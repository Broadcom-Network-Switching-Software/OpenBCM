    ...
    int lock)
{
     ...
     * Lock
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PVT_MON_LOCK, &entry_handle_id));
    for (i = 0; i < nof_pvt_mon; i++)
    {
        dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_PVT_LOCK, i, val);
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    DBAL_HANDLE_FREE(unit, entry_handle_id);
