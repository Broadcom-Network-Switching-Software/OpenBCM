...
#define DEFAULT_DOH_EXT_NAME_SPACE    0
#define DOH_EXT_BIER                  1
#define BIER_DOH_HDR_LENGTH           5
#define BIER_HDR_LENGTH               44
#define DOH_OPT_TYPE_BIER             112
    ...
    bcm_multicast_t group)
{
    ...
    if ((flags & BCM_MULTICAST_BIER_64_GROUP) || (flags & BCM_MULTICAST_BIER_128_GROUP))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "only support BCM_MULTICAST_BIER_256_GROUP\n");
     ...
     */
    if (group > (dnx_data_bier.params.nof_bfr_entries_get(unit) - set_size))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "multicast group (%u) is not at the valid range (>= %u)\n", group,
                     (dnx_data_bier.params.nof_bfr_entries_get(unit) - set_size));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_bier_db.ingress.active.get(unit, group, &active));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_bier_db.egress.active.get(unit, group, &active));
    }
    ...
    uint32 bfr_id)
{
     ...
     */
    if (DNX_MULTICAST_IS_INGRESS(flags) && dnx_data_bier.params.feature_get(unit, dnx_data_bier_params_egress_only))
    {
     ...
     * bfr-id group range check
     */
    if ((group >= dnx_data_bier.params.nof_bfr_entries_get(unit)) || (group < 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "multicast group (%u) is not at the valid range (>= %u)\n", group,
    ...
    bcm_multicast_t group)
{
    ...
    /*
     * Both sides must have the same size
     */
    if (DNX_MULTICAST_IS_INGRESS(flags))
    {
        SHR_IF_ERR_EXIT(dnx_bier_db.ingress.active.set(unit, group, FALSE));
        SHR_IF_ERR_EXIT(dnx_bier_db.egress.active.get(unit, group, &active));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_bier_db.egress.active.set(unit, group, FALSE));
        SHR_IF_ERR_EXIT(dnx_bier_db.ingress.active.get(unit, group, &active));
    }

    both_closed = active ? 0 : 1;
     ...
     */
    SHR_IF_ERR_EXIT(dnx_bier_group_invalid_set(unit, DNX_MULTICAST_IS_INGRESS(flags) ? 1 : 0, group, set_size));
    ...
    bcm_multicast_t group)
{
    ...
    if (DNX_MULTICAST_IS_INGRESS(flags))
    {
        SHR_IF_ERR_EXIT(dnx_bier_db.ingress.active.set(unit, group, TRUE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_bier_db.egress.active.set(unit, group, TRUE));
    }
    ...
    bcm_multicast_replication_t replication)
{
    ...
    uint32 set_bfr_id;
    int set_size;
    uint32 ori_bfr_id = bfr_id;
    int core_id, core_id_idx;
    bcm_multicast_replication_t cur_rep_entry;

    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);

    if (DNX_MULTICAST_IS_INGRESS(flags) &&
        dnx_data_bier.params.feature_get(unit, dnx_data_bier_params_ingress_bfr_pos_msb_start))
    {
        bfr_id = dnx_data_bier.params.bitstring_size_get(unit) - bfr_id - 1;
    }

    SHR_INVOKE_VERIFY_DNXC(dnx_multicast_bfr_id_add_delete_get_verify(unit, flags, group, bfr_id));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_multicast_bfr_id_get(unit, flags, group, ori_bfr_id, &cur_rep_entry));
    if (cur_rep_entry.port != BCM_GPORT_INVALID)
    {
        SHR_IF_ERR_EXIT(dnx_multicast_bfr_id_delete(unit, flags, group, ori_bfr_id, cur_rep_entry));
    }
    ...
    int unit)
{
     ...
     */
    mcdb_entries = dnx_data_bier.params.bier_mcdb_size_get(unit);
    ...
    int unit)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VALUE_OF_5_EN, INST_SINGLE, 1);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    /*
     * BIER type in RCH header init, 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_PEMLA_TRAP, entry_handle_id));
    ...
    if (bier_support)
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_MPLS_GENERAL_CONFIGURATION, entry_handle_id));
