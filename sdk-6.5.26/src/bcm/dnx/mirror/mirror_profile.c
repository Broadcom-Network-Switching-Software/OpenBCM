    ...
    bcm_mirror_destination_t * mirror_dest)
{
    ...
    uint32 snif_type;
    uint32 dest = 0, snif_probability = 0, crop_enable = 0, crop_size = 0,
        tc = 0, tc_ow = 0, dp = 0, dp_ow = 0, in_port = 0, in_port_ow = 0,
        drop_fwd_if_snif_dropped = 0, drop_snif_if_fwd_dropped = 0,
        st_vsq_ptr = 0, st_vsq_ptr_ow = 0, lag_lb_key = 0, lag_lb_key_ow = 0,
        admit_profile = 0, admit_profile_ow = 0, metadata_ow, ext_stat_opcode_ow = 0,
        ext_stat_data_ow = 0, ext_stat_valid_ow = 0, cud_type = 0;
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        {
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_IN_PP_PORT_OW, INST_SINGLE, &in_port_ow);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_IN_PP_PORT, INST_SINGLE, &in_port);
        }
        ...
        {
            crop_size = dnx_data_snif.ingress.crop_size_get(unit);
        }

        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

        /*
         * Only read from the 1st core, as in current implementation,
         * the configuration is the same for all the cores.
         */
        break;
    }
     ...
     */
    {
         ...
         * Crop size field.
         * Set crop size:
         * Crop enable - packet is cropped (trimmed)
         * Crop disable - whole packet is copied
         */
        if (crop_enable)
        {
            mirror_dest->packet_copy_size = crop_size;
        }
        else
        {
        ...
        if (in_port_ow)
        {
            mirror_dest->packet_control_updates.in_port = in_port;
    ...
    /** Set Mirror on Drop flag and bitmap if profile is allocated for the current Action Profile ID */
    {
        int admit_profile_index, nof_admit_profiles;
        int *drop_group_bmp;

        mirror_dest->drop_group_bmp = BCM_MIRROR_DROP_GROUP_BMP_ALL;
        sal_memset(mirror_dest->drop_group_bmp_array, 0, sizeof(mirror_dest->drop_group_bmp_array));
        ...
        /** init drop_group_bmp to default value - BCM_MIRROR_DROP_GROUP_BMP_ALL */
        ...
        else
        {
            ...
            {
                mirror_dest->drop_group_bmp_array[admit_profile_index] = BCM_MIRROR_DROP_GROUP_BMP_ALL;
            }
        }

        /** iterate over admit profiles and retrieve the drop groups */
        for (admit_profile_index = 0; admit_profile_index < nof_admit_profiles; admit_profile_index++)
        {
            /** determine drop group bitmap */
            if (dnx_data_snif.ingress.feature_get(unit, dnx_data_snif_ingress_mirror_on_drop_admit_profile_supported))
            {
            ...
            *drop_group_bmp = 0;
            mod_profile_found = FALSE;

            /** iterate over drop groups */
                 ...
                 mod_profile_index++)
            {
                ...
                {
                    /** Indicate that MoD profile for the current Action Profile ID was found and set flag */
                    mod_profile_found = TRUE;
                    ...
                    {
                        /** On devices where we have more than 1 MoD profile, only set the relevant bits */
                         /* coverity[ARRAY_VS_SINGLETON:FALSE]  */
                        SHR_BITSET(drop_group_bmp, mod_profile_index);
                    }
                }
            }

            if (mod_profile_found == FALSE)
            {
                /**
                 * MoD profile for the current Action Profile ID was not found.
                 * Set bitmap to init value without setting flag
                 */
    ...
    int action_profile_id)
{
    ...
    for (admit_profile_index = 0; admit_profile_index < nof_admit_profiles; admit_profile_index++)
    {
        ...
        SHR_BIT_ITER((uint32 *) drop_group_bmp, dnx_data_snif.ingress.nof_mirror_on_drop_profiles_get(unit),
                     mod_profile_index)
        {
            SHR_IF_ERR_EXIT(dnx_mirror_db.mirror_on_drop_profile.set(unit, admit_profile_index, mod_profile_index,
                                                                     action_profile_id));
        }
    }
    ...
    /** set cud type to be sniff mirror on drop */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CUD_TYPE, INST_SINGLE,
                                 dnx_data_snif.general.cud_type_mirror_on_drop_get(unit));
    ...
    {
        {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id,
                                         DBAL_FIELD_IN_PP_PORT, INST_SINGLE,
    ...
    for (admit_profile_index = 0; admit_profile_index < nof_admit_profiles; admit_profile_index++)
    {
        /** determine the correct bitmap of drop groups */
        {
            drop_group_bmp = &(mirror_dest->drop_group_bmp);
        }

        /** iterate over drop profiles */
         /* coverity[ARRAY_VS_SINGLETON:FALSE]  */
        SHR_BIT_ITER(drop_group_bmp, dnx_data_snif.ingress.nof_mirror_on_drop_profiles_get(unit), mod_profile_index)
        {
            /** set key - drop profile */
    ...
    int action_profile_id)
{
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CUD_TYPE, INST_SINGLE,
                                     dnx_data_snif.general.cud_type_sflow_get(unit));
    }
    else
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CUD_TYPE, INST_SINGLE,
                                     dnx_data_snif.general.cud_type_standard_get(unit));
    }
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INT_STAT_OBJ_META_DATA_OW, INST_SINGLE,
                                         TRUE);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INT_STAT_OBJ_META_DATA, INST_SINGLE,
                                         mirror_dest->packet_control_updates.metadata);
        }
        else
        {
        ...
        if (dnx_data_snif.ingress.feature_get(unit, dnx_data_snif_ingress_ext_stat_dedicated_config))
        {
            ...
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EXT_STAT_DATA_VALID_OW, INST_SINGLE,
                                             TRUE);
            ...
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EXT_STAT_DATA_OPCODE_OW, INST_SINGLE,
                                             TRUE);
                for (i = 0; i < BCM_MIRROR_EXT_STAT_ID_COUNT; i++)
                {
        ...
        /*
         * clearing counting command parameters when creating the mirror profile
         */
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_network_header_termination)
        ...
        dnx_data_snif.ingress.feature_get(unit, dnx_data_snif_ingress_itpp_delta_supported))
    {
    ...
    bcm_mirror_destination_t * mirror_dest)
{
    ...
    uint32 entry_handle_id, is_mc = FALSE, out_lif = 0, out_lif_mask = 0, is_outbound_dis = FALSE,
        fabric_mirror_profile = 0;
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MC_OUTLIF_MASK, INST_SINGLE, &out_lif_mask);
    ...
    mirror_dest->fabric_mirror_flow_profile = fabric_mirror_profile;
    ...
    int action_profile_id)
{
    ...
    else
    {
         ...
         * Encap ID operation
         */
        if (mirror_dest->flags & BCM_MIRROR_DEST_TUNNEL_WITH_ENCAP_ID)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MC_OUTLIF, INST_SINGLE,
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUTBOUND_MIRR_DISABLE, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUTBOUND_MIRR_DISABLE_MASK, INST_SINGLE, 0);
    }
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TC, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TC_MASK, INST_SINGLE, 0);
    }
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ECN_ENABLE, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ECN_ENABLE_MASK, INST_SINGLE, 0);
    }

    /*
     * Fabric mirror flow profile
     */
    ...
    if (mirror_dest->flags2 & BCM_MIRROR_DEST_FLAGS2_MIRROR_ON_DROP)
    {
        int mod_profile_index, nof_admit_profiles, admit_profile_index;
        int *drop_group_bmp;
...
dnx_mirror_egress_profile_hw_get(
    ...
    bcm_mirror_destination_t * mirror_dest)
{
    uint32 rcy_priority, entry_handle_id, snif_type, add_orig_head, crop_segment = 0, probability = 0;
    int action_profile_id;
     ...
     */
    {
         ...
         * key construction
         */
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SNIF_COMMAND_ID,
                                   DNX_MIRROR_EGRESS_PROFILE_GET(unit, action_profile_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SNIF_TYPE, snif_type);
        ...
        {
            dbal_value_field32_request(unit, entry_handle_id,
                                       DBAL_FIELD_SNIF_EGRESS_PROBABILITY, INST_SINGLE, &probability);
        }
        ...
        /** crop */
        {
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_SNIF_CROP_ENABLE, INST_SINGLE, &crop_segment);
        }
     ...
     */
    {
        ...
        if (crop_segment)
        {
            mirror_dest->egress_packet_copy_size = crop_segment * dnx_data_snif.egress.crop_size_granularity_get(unit);
        }
        else
        {
        ...
        {
            uint32 nof_bits, divisor;
            /*
             * Probability field. the dividend is the probability and the divisor is the max value according the field
             */
            mirror_dest->egress_sample_rate_dividend = probability;
            nof_bits = dnx_data_snif.egress.prob_nof_bits_get(unit);
            SHR_IF_ERR_EXIT(dnx_mirror_profile_probability_get(unit, 1, 1, nof_bits, &divisor));
            mirror_dest->egress_sample_rate_divisor = divisor;
        }
        else
        {
            mirror_dest->egress_sample_rate_dividend = 0;
            mirror_dest->egress_sample_rate_divisor = 0;
        }
    }
    ...
    int action_profile_id)
{
     ...
     * For devices that have ibch1 header, original system headers must be always added since they include ibch1.
     *
     */
    ...
    bcm_mirror_destination_t * mirror_dest)
{
    ...
    if (mirror_dest->flags & BCM_MIRROR_DEST_IS_SNOOP)
    {
        ...
        nof_egress_profiles = DNX_MIRROR_EGRESS_NOF_PROFILES_GET(unit, TRUE);
    }
    else if (mirror_dest->flags & BCM_MIRROR_DEST_IS_STAT_SAMPLE)
    {
        BCM_GPORT_MIRROR_STAT_SAMPLE_SET(mirror_dest->mirror_dest_id, action_profile_id);
        nof_egress_profiles = 0;
    }
    else
    {
        BCM_GPORT_MIRROR_MIRROR_SET(mirror_dest->mirror_dest_id, action_profile_id);
        nof_egress_profiles = DNX_MIRROR_EGRESS_NOF_PROFILES_GET(unit, FALSE);
    }
     ...
     */
    if (action_profile_id < nof_egress_profiles)
    {
...
dnx_mirror_profile_get(
    ...
    bcm_mirror_destination_t * mirror_dest)
{
    int action_profile_id, nof_egress_profiles = 0;
     ...
     */
    action_profile_id = BCM_GPORT_MIRROR_GET(mirror_dest_id);
    if (_SHR_GPORT_IS_MIRROR_SNOOP(mirror_dest_id))
    {
        nof_egress_profiles = DNX_MIRROR_EGRESS_NOF_PROFILES_GET(unit, TRUE);
    }
    else if (_SHR_GPORT_IS_MIRROR_MIRROR(mirror_dest_id))
    {
        nof_egress_profiles = DNX_MIRROR_EGRESS_NOF_PROFILES_GET(unit, FALSE);
    }
    else
...
{
    int snif_type = 0;
    int action_profile_id, profile, mod_profile_index, nof_egress_profiles = 0;
    uint32 entry_handle_id;
    int admit_profile_index, nof_admit_profiles;
    int system_headers_mode;
...
{
    uint32 entry_handle_id;
    uint32 profile_id, snif_code;
    int trap_id, nof_egress_profiles = 0;
    ...
    nof_egress_profiles = UTILEX_MAX(DNX_MIRROR_EGRESS_NOF_PROFILES_GET(unit, TRUE),
                                     DNX_MIRROR_EGRESS_NOF_PROFILES_GET(unit, FALSE));
    for (profile_id = 0; profile_id < nof_egress_profiles; profile_id++)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RECYCLE_COMMAND_ID, profile_id);
