    ...
    void *ignored_b)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_port_signal_quality_encoded_val_to_float_get(unit, error_rate_encoded, &error_rate));
    ...
    uint64 *compensated_count)
{
    ...
    const dnx_data_port_general_fec_compensation_info_t *fec_entry = NULL;

    SHR_FUNC_INIT_VARS(unit);

    *compensated_count = count;

    /** get port fec type */
    SHR_IF_ERR_EXIT(bcm_dnx_port_resource_get(unit, port, &resource));

    /** get compensation data based on FEC type */
    fec_entry = dnx_data_port.general.fec_compensation_info_get(unit, resource.fec_type);
    ...
    *error_rate = 0;
    ...
    if (is_ber)
    {
        ...
        if (uncorrectable_er_cnt)
        {
            ...
            min_nof_bad_symbols =
                dnx_data_port.general.fec_compensation_info_get(unit,
    ...
    int unit)
{
    ...
    _SHR_PBMP_ITER(enabled_ports, port)
    {
        ...
        if (sample_interval >= cfg_interval)
        {
            ...
            SHR_IF_ERR_EXIT(soc_dnxc_port_signal_quality_encoded_val_to_float_get
                            (unit, phy_config.fail_thr, &fail_thr));
            SHR_IF_ERR_EXIT(soc_dnxc_port_signal_quality_encoded_val_to_float_get
    ...
    int value)
{
    ...
    switch (type)
    {
        ...
        /** min and max are based on absolute supported values */
            ...
            soc_dnxc_port_signal_quality_man_and_exp_to_float_get(1, min_exp, &min_thr_value);

            max_exp = dnx_data_nif.signal_quality.max_degrade_threshold_get(unit);
            soc_dnxc_port_signal_quality_man_and_exp_to_float_get(1, max_exp, &max_thr_value);

        /** convert user encoded threshold to regular float */
            SHR_IF_ERR_EXIT(soc_dnxc_port_signal_quality_encoded_val_to_float_get(unit, value, &thr_value));
        ...
        /** recover threshold should not be stricter than regular threshold */
            ...
            SHR_IF_ERR_EXIT(soc_dnxc_port_signal_quality_encoded_val_to_float_get(unit, encoded_thr, &max_thr_value));
