...
{
    uint32 entry_handle_id;
    dbal_entry_action_flags_e action_flags;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_PP_PORT, INST_SINGLE, pp_port);
    dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_SOURCE_SYSTEM_PORT, INST_SINGLE, sys_port);
    action_flags = dnx_data_port_pp.general.feature_get(unit, dnx_data_port_pp_general_prt_tcam_is_direct_access) ?
        (DBAL_COMMIT | DBAL_COMMIT_OVERRIDE_DEFAULT) : DBAL_COMMIT_FORCE;
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, action_flags));
    ...
    if (*entry_access_id < max_capacity)
    {
      /** Allocate a virtual port TCAM access id */
        SHR_IF_ERR_EXIT(dnx_port_prt_tcam_virtual_port_tcam_sw_set
                        (unit, core_id, prttcam_key_field_id, prttcam_key_value, *entry_access_id, 0, 0, 0,
                         FC_CHAN_ID_INVALID, 0));
    }
    else
    {
