    ...
    bcm_port_mapping_info_t * mapping_info)
{
        ...
        || (interface_info->interface == BCM_PORT_IF_VIRTUAL_FLEXE_CLIENT))
    {
        ...
        else
        {
            ...
            /** make sure BCM_PORT_ADD_FLEXE_PROTECTION should be use together with BCM_PORT_ADD_FLEXE_MAC */
            if (_SHR_IS_FLAG_SET(interface_info->flags, BCM_PORT_ADD_FLEXE_PROTECTION)
                && !_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FLEXE_MAC))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "BCM_PORT_ADD_FLEXE_PROTECTION should be used together with BCM_PORT_ADD_FLEXE_MAC.\n");
            }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Only L1 ports are allowed in L1 only mode.\n");
    }
    if (dnx_data_nif.global.l1_only_mode_get(unit) == DNX_PORT_NIF_L1_ONLY_MODE_WITHOUT_ILKN &&
        DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE_ELK | DNX_ALGO_PORT_TYPE_INCLUDE_L1))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Only ILKN ports are not allowed in L1 only  without ILKN mode.\n");
    }
