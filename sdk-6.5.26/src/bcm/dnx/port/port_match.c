        ...
        case BCM_PORT_MATCH_PORT:
        {
            if (BCM_GPORT_IS_SET(port))
            {
                     ...
                     BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(port)))
                {
                    ...
                    if (esem_default_result_profile >=
                        dnx_data_esem.default_result_profile.nof_allocable_profiles_get(unit))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    uint8 *found)
{
    ...
    *found = FALSE;
    ...
    else if (!dnx_data_esem.access_cmd.esem_cmd_predefine_allocations_get(unit) &&
    ...
    int *count)
{
    ...
    while ((!DNX_LIF_MNGR_OUTLIF_SW_INFO_ITER_IS_END(&iter)) && (*count < size))
    {
        ...
        if (has_esem_cmd)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_port_get(unit, gport, &lif_esem_cmd));
