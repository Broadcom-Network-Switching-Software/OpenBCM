...
#define PORT_INGR_REASSEMBLY_CONTEXT_MAPPED_PORT_ALLOC_MNG_ID "PORT_INGR_REASSEMBLY_CONTEXT_MAPPED_PORT"
    ...
    int *is_required)
{
    ...
    {
        case DNX_PORT_INGR_REASSEMBLY_ALLOC:
            if (dnx_data_ingr_reassembly.context.rcy_unique_context_get(unit))
            {
                *is_required = TRUE;
            }
            else
    ...
    int *is_deallocated)
{
    ...
    if (dnx_data_ingr_reassembly.context.rcy_unique_context_get(unit))
    {
        SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.alloc.free_single(unit, core_id, reassembly_context, NULL));
        *is_deallocated = TRUE;
    }
    else
    {
        ...
        if (*is_deallocated)
        {
            ...
            for (temp_prio = 0; temp_prio < nof_contexts_per_rcy_if; temp_prio++)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.alloc.free_single(unit, core_id, temp_context, NULL));
    ...
    int *is_allocated)
{
    ...
    switch (port_info.port_type)
    {
        case DNX_ALGO_PORT_TYPE_RCY:
            /** RCY port  */
            SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_rcy_reassembly_context_alloc
                            (unit, port, priority, reassembly_context, is_allocated));
            break;
        case DNX_ALGO_PORT_TYPE_RCY_MIRROR:
            SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_mirror_reassembly_context_alloc
                            (unit, port, priority, reassembly_context, is_allocated));
            break;
        default:
        /** other port types */
    ...
    int unit)
{
    ...
    {
        /*
         * Create internal CGM input port allocation manager
         */
        sal_memset(&data, 0, sizeof(dnx_algo_res_create_data_t));

        data.first_element = 0;
        data.nof_elements = dnx_data_port.general.nof_pp_ports_get(unit);

        sal_strncpy(data.name, PORT_INGR_REASSEMBLY_CONTEXT_MAPPED_PORT_ALLOC_MNG_ID,
                    DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

        SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.
                        mapped_ports.alloc(unit, dnx_data_device.general.nof_cores_get(unit)));

        for (core_id = 0; core_id < dnx_data_device.general.nof_cores_get(unit); core_id++)
        {
            SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.mapped_ports.create(unit, core_id, &data, NULL));
        }
    }
...
/**
 * \brief - should mapped port be allocated ?
 *          check if one of the other priorities has a valid reassembly
 */
static shr_error_e
dnx_port_ingr_reassembly_should_call_mapped_port_alloc(
    int unit,
    bcm_port_t port,
    int priority,
    int *call_alloc,
    uint32 *in_cgm_port)
{
    ...
    *call_alloc = 1;
    ...
    switch (port_info.port_type)
    {
        ...
        case DNX_ALGO_PORT_TYPE_RCY:
        /** always allocate/deallocate for RCY */
            break;
        ...
        default:
            ...
            for (priority_i = 0; priority_i < DNX_DATA_MAX_INGR_REASSEMBLY_PRIORITY_PRIORITIES_NOF; priority_i++)
            {
                /** skip the requested priority */
                if (priority_i == priority)
                {
                    continue;
                }

                if ((reassembly_context[priority_i] != DNX_PORT_INGR_REASSEMBLY_NON_INGRESS_PORT_CONTEXT)
                    && (reassembly_context[priority_i] != invalid_context))
                {
                    *call_alloc = 0;
                    SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_to_in_cgm_port_hw_get
                                    (unit, core, reassembly_context[priority_i], in_cgm_port));
                }
            }
    }
...
/**
 * \brief - should mapped port be deallocated ?
 */
static shr_error_e
dnx_port_ingr_reassembly_should_call_mapped_port_dealloc(
    int unit,
    bcm_port_t port,
    int priority,
    int *call_dealloc)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_should_call_mapped_port_alloc
                    (unit, port, priority, call_dealloc, &in_cgm_port));
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_mapped_port_alloc(
    int unit,
    bcm_port_t port,
    int priority,
    uint32 *mapped_port)
{
    ...
    /** check if mapped port should be allocated */
    SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_should_call_mapped_port_alloc
                    (unit, port, priority, &call_alloc, mapped_port));

    if (call_alloc)
    {
        /** Allocate new port */
        SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.
                        mapped_ports.allocate_single(unit, core, alloc_flags, NULL, (int *) mapped_port));
    }
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_mapped_port_dealloc(
    int unit,
    bcm_port_t port,
    int priority,
    uint32 mapped_port)
{
    ...
    /** check if mapped port should be deallocated */
    SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_should_call_mapped_port_dealloc(unit, port, priority, &call_dealloc));

    if (call_dealloc)
    {
        /** De-Allocate port */
        SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.mapped_ports.free_single(unit, core, mapped_port, NULL));
    }
