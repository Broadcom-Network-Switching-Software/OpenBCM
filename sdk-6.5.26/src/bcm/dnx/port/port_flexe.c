    ...
    int is_add)
{
    ...
    /*
     * Get destination info for removing flow
     */
    if (!is_add)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, dest_port, &port_info));
        SHR_IF_ERR_EXIT(dnx_algo_framer_66bsw_block_id_get(unit, port_info.port_type, channel, &dest_block_id));
        /*
         * Get channel
         */
        if (DNX_ALGO_PORT_TYPE_IS_FRAMER_SAR(unit, port_info, FALSE))
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_flexe_sar_virtual_flexe_channel_get(unit, dest_port, &dest_channel));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, dest_port, &dest_channel));
        }
    }
    /*
     * Get existing flow
    ...
    int is_add)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /*
     * Verify if the path has been configured
     */
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, src_port, &port_info));
    SHR_IF_ERR_EXIT(dnx_algo_framer_66bsw_block_id_get(unit, port_info.port_type, 0, &src_block));

    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, dst_port, &port_info));
    SHR_IF_ERR_EXIT(dnx_algo_framer_66bsw_block_id_get(unit, port_info.port_type, channel, &dst_block));
 ...
 */
static shr_error_e
dnx_port_framer_port_remove_flow_verify(
    int unit,
    bcm_port_t logical_port)
{
    int nof_dests;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_sar_virtual_flexe_channel_get(unit, logical_port, &src_info.channel));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, logical_port, &src_info.channel));
    }
    /*
     * Get flow
     */
    ...
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     "The port %d has configured flexe flow, cannot remove it. Please clear the flow!\n", logical_port);
    }
    ...
    bcm_port_t logical_port)
{
        ...
        || DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, TRUE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_framer_port_remove_flow_verify(unit, logical_port));
