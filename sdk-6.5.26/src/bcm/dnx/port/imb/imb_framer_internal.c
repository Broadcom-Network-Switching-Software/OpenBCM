 ...
 */
static int
imb_framer_internal_sb_fifo_force_set(
    int unit,
    dnx_algo_flexe_link_list_fifo_type_e type)
{
    dbal_tables_e table_id;
...
/**
 * \brief - Set BusB client port speed configuration
 *
 * \param [in] unit - chip unit id
 ...
 */
int
imb_framer_internal_port_fifo_link_list_config_set(
    int unit,
    int client_channel,
    int rmc_id,
    int speed,
    ...
    int enable)
{
    ...
    if (((fifo_type == DNX_ALGO_FLEXE_FIFO_SB_RX) || (fifo_type == DNX_ALGO_FLEXE_FIFO_SB_TX)) &&
        dnx_data_nif.mac_client.feature_get(unit, dnx_data_nif_mac_client_sb_fifo_force_supported))
    {
        SHR_IF_ERR_EXIT(imb_framer_internal_sb_fifo_force_set(unit, fifo_type));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    dbal_fields_e * cal_field)
{
    ...
    switch (cal_type)
    {
        case IMB_FRAMER_NB_CALENDAR:
            *ctrl_table = DBAL_TABLE_NIF_FRAMER_NB_TDM_PATTERN_CTRL;
            *cal_table = DBAL_TABLE_NIF_FRAMER_NB_TDM_PATTERN;
    ...
    uint32 *active_cal)
{
    ...
    SHR_IF_ERR_EXIT(imb_framer_internal_calendar_table_get
                    (unit, cal_type, &ctrl_table, &calendar_table, &calendar_field));
    /*
     * alloc DBAL table handle
     */
    ...
    uint32 active_cal)
{
     ...
     * Configure field
     */
    if ((cal_type == IMB_FRAMER_SB_RX_CALENDAR)
        || ((cal_type == IMB_FRAMER_NB_CALENDAR)
    ...
    uint32 *calendar)
{
    ...
    for (i = 0; i < cal_len; i++)
    {
        ...
        if ((cal_type == IMB_FRAMER_NB_CALENDAR)
 ...
 */
shr_error_e
imb_framer_internal_calendar_config(
    int unit,
    imb_framer_calendar_type_e cal_type,
    int cal_len,
    uint32 *calendar)
{
    ...
    else if ((cal_type == IMB_FRAMER_SB_RX_CALENDAR) && (dnx_data_nif.mac_client.nof_special_clients_get(unit) > 0))
    {
        /*
    ...
    {
        SHR_IF_ERR_EXIT(imb_framer_internal_active_calendar_set(unit, cal_type, cal_to_config));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *nof_slots)
{
    ...
    SHR_IF_ERR_EXIT(imb_framer_internal_active_calendar_get(unit, IMB_FRAMER_NB_CALENDAR, &active_cal));
    ...
    *nof_slots = 0;
     ...
     * alloc DBAL table handle
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_FRAMER_NB_TDM_PATTERN, &entry_handle_id));
...
/**
 * \brief
 *   Insert imb_feu_internal_mac_access_control_set journal to generic ER database
 *
 * see .h file
 */
int
imb_framer_internal_mac_access_control_set_er_journal_insert(
    int unit,
    int port)
{
    SHR_FUNC_INIT_VARS(unit);
