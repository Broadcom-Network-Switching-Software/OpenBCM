...
#define IMB_DIAG_RX_CLOCK_OFFSET                  (8)
#define IMB_DIAG_NANO_SYNC_NIF_LANE_OFFSET        (64)
#define IMB_DIAG_RX_CLOCK_SIGNAL_VALID_AND_LINK   (2)
    ...
    imb_diag_synce_config_t * synce)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
         ...
         * set value fields
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COUNTER_CLK_SEL, INST_SINGLE,
                                     synce->counter_clk_sel);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENABLE, INST_SINGLE, synce->gtimer_mode);
        /*
         * commit values
         */
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
         ...
         * ECI lane select
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_SYNCE_REF_CLK_NIF_FAB_LANE_SEL, entry_handle_id));

        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SYNCE_INDEX, 0);

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_FAB_CLOCK_SEL, INST_SINGLE, synce->clk_sel);
         ...
         * cdpm gate enable
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_SYNCE_NIF_CDU_CLK_VALID_CTRL, entry_handle_id));

        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_SYNCE_CLK_BLK, synce_clk_blk_id);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LANE_ID, synce->array_index);
    ...
    else
    {
         ...
         */
        {
            table_id = DBAL_TABLE_SYNCE_CORE_CTRL;
        }
        /*
         * alloc DBAL table handle
         */
        ...
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_SYNCE_REF_CLK_DIV_NIF_CTRL, entry_handle_id));
            /*
             * set key fields
             */
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
        ...
        if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_nif_clk_en_cfg))
        {
            {
                table_id = DBAL_TABLE_SYNCE_CORE_CTRL;
            }
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CLOCK_ENABLE, INST_SINGLE,
                                         synce->clk_chain_sel);
        ...
        /*
         * Enable NIF chain except mgu block
         */
        if (!is_mgu)
        {
    ...
    imb_diag_synce_config_t * synce)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
         ...
         * set value requests
         */
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_ENABLE, INST_SINGLE, &synce->gtimer_mode);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_COUNTER_CLK_SEL, INST_SINGLE,
                                   &synce->counter_clk_sel);
        /*
         * commit request
         */
         ...
         * set value requests
         */
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_NIF_FAB_CLOCK_SEL, INST_SINGLE, &synce->clk_sel);
        /*
         * commit request
         */
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    else
    {
        ...
        if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_nif_clk_en_cfg))
        {
            ...
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CLOCK_ENABLE, INST_SINGLE,
                                       &synce->clk_chain_sel);
...
imb_diag_ref_clk_int_reminder_get(
    int unit,
    DNXC_SERDES_REF_CLOCK_TYPE ref_clk,
    int *ref_clk_int,
    ...
    soc_dnxc_port_phy_measure_t * phy_measure)
{
    ...
    soc_block_type_t block_type = SOC_BLK_NMG;
    int instance;
    int is_mgu = 0;
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        synce_default.array_index = ethu_info.first_lane_in_port;
    }
     ...
     * Config syncE
     */
    sal_memset(&synce_config, 0, sizeof(synce_config));
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        synce_config.gtimer_mode = 1;
        synce_config.counter_clk_sel = swapped_lane - ethu_info.first_lane_in_ethu + IMB_DIAG_RX_CLOCK_OFFSET;
        synce_config.clk_sel = swapped_lane + IMB_DIAG_NANO_SYNC_NIF_LANE_OFFSET;
        synce_config.link_valid_sel = IMB_DIAG_RX_CLOCK_SIGNAL_VALID_AND_LINK;
        synce_config.clk_chain_sel = 1;
        synce_config.array_index = ethu_info.first_lane_in_port;
        /*
         * Enable CDPM Gtimer
         */
     ...
     */
    SHR_IF_ERR_EXIT(dnx_gtimer_wait(unit, block_type, ethu_info.core));
...
exit:
    ...
    {
        if ((_func_rv = dnx_gtimer_clear(unit, block_type, ethu_info.core)) < 0)
        {
