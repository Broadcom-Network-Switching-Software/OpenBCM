    ...
    if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_scheduler_context_per_port_support))
    {
        ...
        if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
            SHR_IF_ERR_EXIT(dnx_port_ofr_ilkn_rx_config_hw_set
                            (unit, ilkn_access_info.ilkn_id, sch_config.is_tdm_priority_enabled,
    ...
    imb_ilu_sch_config_t * sch_config)
{
    ...
    if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_ofr_ilkn_rx_config_hw_set(unit, ilkn_access_info->ilkn_id,
                                                           sch_config->is_tdm_priority_enabled,
                                                           sch_config->is_high_priority_enabled,
                                                           sch_config->is_low_priority_enabled));
    ...
    int is_start_sync)
{
    ...
    if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_hrf_flush_support) && ilkn_nof_ports > 1)
    {
        ...
        else
        {
             ...
             * Stop flushing
             */
            SHR_IF_ERR_EXIT(imb_ilu_internal_port_rx_core_flush_enable_hw_set
                            (unit, ilkn_access_info->ilkn_core, FALSE));
