    ...
    else
    {
        ...
        if ((is_physical_port == FALSE) && (is_ingress_only == TRUE))
        {
            ...
            SHR_IF_ERR_REPLACE_AND_EXIT(dnx_algo_gpm_gport_to_hw_resources
                                        (unit, lif_gport, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS,
    ...
    int sw_id_key)
{
    ...
    if (is_symmetric == TRUE)
    {
         ...
         * Allocate Egress SW tag-structure:
         */
        SHR_IF_ERR_EXIT(port_tpid_db.tag_struct_allocation.
                        bcm_tag_format_class_id_egress.free_single(unit, sw_id_key, NULL));
    ...
    dnx_port_tpid_egress_llvp_entry_t * egress_llvp_entry)
{
    ...
    if (dnx_data_vlan.llvp.feature_get(unit, dnx_data_vlan_llvp_packet_pcp_dei_support))
    {
    ...
    uint32 *tpid_class_buffer)
{
    ...
    for (idx = 0; idx < llvp_block_size; idx++)
    {
        ...
        llvp_tag_format_enum = llvp_entry.llvp_tag_format;

        /*
         * Set data fields
         */
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value
                        (unit, DBAL_FIELD_INCOMING_TAG_STRUCTURE, llvp_tag_format_enum, &is_illegal));
        if (!is_illegal)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INCOMING_VID_EXIST, INST_SINGLE,
...
/**
 * \brief - Configure ingress LLVP profile per pp port
 *
 ...
 */
static shr_error_e
dnx_port_tpid_class_ingress_llvp_profile_per_pp_port_set(
    int unit,
    uint32 pp_port,
    bcm_core_t core_id,
    int llvp_profile)
    ...
    int llvp_profile)
{
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_port_tpid_class_ingress_llvp_profile_per_pp_port_set
                            (unit, gport_info.internal_port_pp_info.pp_port[pp_port_index],
                             gport_info.internal_port_pp_info.core_id[pp_port_index], llvp_profile));
...
/**
 * \brief - Set egress LLVP profile per pp port
 *
 * \param [in] unit - Unit ID
 * \param [in] pp_port - PP port
 * \param [in] core_id - Core ID
 ...
 */
static shr_error_e
dnx_port_tpid_class_egress_pp_port_llvp_profile_set(
    int unit,
    uint32 pp_port,
    bcm_core_t core_id,
    int llvp_profile)
    ...
    int llvp_profile)
{
    ...
    if (BCM_GPORT_IS_TUNNEL(port) || BCM_GPORT_IS_MPLS_PORT(port))
    {
         ...
         */
        SHR_IF_ERR_REPLACE_AND_EXIT(dnx_algo_gpm_gport_to_hw_resources
                                    (unit, lif_gport, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS,
                                     &gport_hw_resources), _SHR_E_NOT_FOUND, _SHR_E_PARAM);
    ...
    else
    {
        ...
        for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
        {
            SHR_IF_ERR_EXIT(dnx_port_tpid_class_egress_pp_port_llvp_profile_set
                            (unit, gport_info.internal_port_pp_info.pp_port[pp_port_index],
                             gport_info.internal_port_pp_info.core_id[pp_port_index], llvp_profile));
        }
    }
...
/**
 * \brief - Set acceptable frame type profile per pp port
 *
 * \param [in] unit - relevant unit.
 * \param [in] pp_port - PP port
 * \param [in] core_id - Core ID
 * \param [in] acceptable_frame_type_profile - Acceptable frame type profile
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
dnx_port_tpid_egress_pp_port_acceptable_frame_type_profile_set(
    int unit,
    uint32 pp_port,
    bcm_core_t core_id,
    uint32 acceptable_frame_type_profile)
    ...
    uint32 acceptable_frame_type_profile)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        SHR_IF_ERR_EXIT(dnx_port_tpid_egress_pp_port_acceptable_frame_type_profile_set
                        (unit, gport_info.internal_port_pp_info.pp_port[pp_port_index],
                         gport_info.internal_port_pp_info.core_id[pp_port_index], acceptable_frame_type_profile));
    }
    ...
    int unit)
{
     ...
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_INGRESS_LLVP_CLASSIFICATION, entry_handle_id));
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_INNER_TPID, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_OUTER_TPID, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    if (!dnx_data_vlan.llvp.ext_vcp_en_get(unit))
    {
    ...
    bcm_gport_t logical_port)
{
    ...
    if (!BCM_GPORT_IS_TRUNK(logical_port))
    {
        ...
        if (BCM_GPORT_IS_SYSTEM_PORT(logical_port) || BCM_GPORT_IS_MODPORT(logical_port))
        {
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
...
/**
 * \brief
 * See port_tpid.h for reference
 */
shr_error_e
dnx_port_tpid_class_ingress_default_per_pp_port_deinit(
    int unit,
    uint32 pp_port,
    bcm_core_t core_id,
    uint8 tpid_ref_dealloc)
{
    ...
    /** Set the LLVP valid to invalid */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.pp.ingress_llvp_profile_valid.set(unit, core_id, pp_port, 0));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_port_tpid_class_ingress_port_llvp_profile_get(unit, pp_port, core_id, &llvp_profile));

    if (llvp_profile != 0)
    {
        SHR_IF_ERR_EXIT(dnx_port_tpid_class_ingress_llvp_profile_per_pp_port_set(unit, pp_port, core_id, 0));
    }

    /** In case of trunk exists only one reference per trunk gport */
    if (tpid_ref_dealloc)
    {
    ...
    bcm_gport_t logical_port)
{
    ...
    if (llvp_profile != 0)
    {
        /** Set the llvp profile to 0 in INGRESS_PP_PORT table */
        SHR_IF_ERR_EXIT(dnx_port_tpid_class_ingress_llvp_profile_set(unit, logical_port, 0));
    }
    ...
    bcm_gport_t logical_port)
{
    ...
    if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_acceptable_frame_type_filter_support))
    {
        ...
        for (outer_tpid_index = 0; outer_tpid_index < DNX_PORT_TPID_TPID_INDEX_SIZE; outer_tpid_index++)
        {
            for (inner_tpid_index = 0; inner_tpid_index < DNX_PORT_TPID_TPID_INDEX_SIZE; inner_tpid_index++)
            {
                default_acceptable_frame_type_template.acceptable_frame_type_template[outer_tpid_index]
                    [inner_tpid_index] = DBAL_ENUM_FVAL_EGRESS_ACCEPTABLE_FRAME_TYPE_FILTER_ACTION_ACCEPT;
            }
        }
    }
    if (!BCM_GPORT_IS_TRUNK(logical_port))
 ...
 */
shr_error_e
dnx_port_tpid_class_egress_default_per_pp_port_deinit(
    int unit,
    uint32 pp_port,
    bcm_core_t core_id,
    uint8 tpid_ref_dealloc)
{
    ...
    /** Set the LLVP valid to invalid */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.pp.egress_llvp_profile_valid.set(unit, core_id, pp_port, 0));

    /*
     * 1. Free the allocated egress LLVP classification template.
     * 2. Clear the HW if it was the last reference to this LLVP profile.
     */
    SHR_IF_ERR_EXIT(dnx_port_tpid_class_egress_port_llvp_profile_get(unit, pp_port, core_id, &llvp_profile));

    if (llvp_profile != 0)
    {
        /** Set egress LLVP profile to 0 in EGRESS_PP_PORT table */
        SHR_IF_ERR_EXIT(dnx_port_tpid_class_egress_pp_port_llvp_profile_set(unit, pp_port, core_id, 0));
    }
    ...
    if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_acceptable_frame_type_filter_support))
    {
        last_reference = FALSE;
        SHR_IF_ERR_EXIT(dnx_port_tpid_egress_port_acceptable_frame_type_profile_get
                        (unit, pp_port, core_id, &acceptable_frame_type_profile));

        if (acceptable_frame_type_profile != 0)
        {
            /** Set egress acceptable frame type profile to 0 in EGRESS_PP_PORT table */
            SHR_IF_ERR_EXIT(dnx_port_tpid_egress_pp_port_acceptable_frame_type_profile_set(unit, pp_port, core_id, 0));
        }
        /** In case of trunk exists only one reference per trunk gport */
        if (tpid_ref_dealloc)
        {
    ...
    bcm_gport_t logical_port)
{
    ...
    {
        /** Set egress LLVP profile to 0 in EGRESS_PP_PORT table */
        SHR_IF_ERR_EXIT(dnx_port_tpid_class_egress_llvp_profile_set(unit, logical_port, 0));
    }
    ...
    if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_acceptable_frame_type_filter_support))
    {
        ...
        {
            /** Set egress acceptable frame type profile to 0 in EGRESS_PP_PORT table */
            SHR_IF_ERR_EXIT(dnx_port_tpid_egress_acceptable_frame_type_profile_set(unit, logical_port, 0));
        }
    ...
    bcm_port_tpid_class_t * tpid_class)
{
     ...
     * Verify user input
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_tpid_class_get_verify(unit, tpid_class));
    ...
    bcm_port_tpid_class_t * tpid_class)
{
     ...
     * Verify user input
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_tpid_class_set_verify(unit, tpid_class));
    ...
    bcm_port_tag_format_class_t * tag_format_class_id)
{
     ...
     * Verify user input
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_tpid_class_create_verify(unit, flags, tag_struct_type, tag_format_class_id));
...
{
    int is_ingress_only, is_egress_only, is_symmetric;
    int tag_struct_hw_id_ingress = -1, tag_struct_hw_id_egress = -1;
     ...
     * Verify user input
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_tpid_class_destroy_verify(unit, flags, tag_format_class_id));
