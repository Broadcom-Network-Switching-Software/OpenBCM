    ...
    bcm_port_resource_t * resource)
{
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        if (_SHR_PBMP_MEMBER(all_nif_ports, logical_port))
        {
            /*
    ...
    bcm_port_resource_t * resource)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     "Port %d is not framer client, only framer clients support G.HAO", resource[resource_index].port);
    }
    ...
    if (!has_speed)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "Port %d tx speed is not set, can't perfrom G.HAO", resource[resource_index].port);
    }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_has_speed(unit, resource[resource_index].port, DNX_ALGO_PORT_SPEED_F_RX,
                                                &has_speed));
        if (!has_speed)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "Port %d rx speed is not set, can't perform G.HAO", resource[resource_index].port);
        }
    }

    /** G.HAO speed change from a speed above 1.2 Gbps to another speed above 1.2Gbps is not allowed */
    SHR_IF_ERR_EXIT(dnx_algo_port_speed_get(unit, resource[resource_index].port, DNX_ALGO_PORT_SPEED_F_TX, &speed));
    if ((speed >= DNX_ARB_TX_G_HAO_SPEED_LIMIT_MBPS) &&
        (resource[resource_index].speed >= DNX_ARB_TX_G_HAO_SPEED_LIMIT_MBPS))
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     "G.HAO speed change from a speed above 1.2 Gbps to another speed above 1.2 Gbps is not allowed.\n"
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_IMB(unit, port_info, FALSE))
        {
            ...
            if (DNX_ALGO_PORT_TYPE_IS_NIF
                (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, FLEXE, STIF, L1, MGMT, OTN_PHY))
        ...
        if (is_flexe_enabled && DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, TRUE, FALSE, FALSE))
        {
    ...
    bcm_port_flr_timeout_t timeout)
{
    ...
    /*
     * If advanced FLR is supported, configure it to legacy mode
     */
    if (dnx_data_nif.flr.feature_get(unit, dnx_data_nif_flr_is_advanced_flr_supported))
    {
        ...
        SHR_IF_ERR_EXIT(imb_port_flr_link_config_set(unit, port, &link_config));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int is_set)
{
    ...
    if (is_set)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_flr_tick_to_timer_get
                        (unit, dnx_data_nif.flr.max_tick_unit_get(unit), dnx_data_nif.flr.max_tick_count_get(unit),
                         &max_timer));
    ...
    bcm_port_flr_link_config_t * config)
{
    ...
    SHR_IF_ERR_EXIT(imb_port_flr_link_config_get(unit, port, config));
exit:
    SHR_FUNC_EXIT;
}
/**
    ...
    bcm_port_flr_link_config_t * config)
{
    ...
    SHR_IF_ERR_EXIT(imb_port_flr_link_config_set(unit, port, config));
exit:
    SHR_FUNC_EXIT;
}
