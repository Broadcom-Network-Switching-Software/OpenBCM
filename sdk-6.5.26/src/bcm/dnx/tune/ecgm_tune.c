...
#include <soc/dnx/dnx_data/auto_generated/dnx_data_egr_queuing.h>       /* egr_queuing dnx data */
...
/**
 * \brief
 * Init the ECGM thresholds for all ports
 */
static shr_error_e dnx_ecgm_tune_port_init_all(
    int unit);
/*
* }
* Static function declarations.
    ...
    int unit)
{
    ...
    /*
     * Init the ECGM per port thresholds for all ports:
     * Reserved thresholds
     * Unicast thresholds
     * Multicast thresholds and Multicast queues PD SP default mapping
     */
    SHR_IF_ERR_EXIT(dnx_ecgm_tune_port_init_all(unit));
    ...
    int port_rate)
{
    ...
    if (!
        (DNX_ALGO_PORT_TYPE_IS_FRAMER_CLIENT(unit, port_info, TRUE)
         && dnx_data_egr_queuing.params.nof_dedicated_flexe_interfaces_get(unit)))
    {
    ...
    int unit)
{
    ...
    /*
     * Get all TM Master ports, both special interfaces and NIF
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_TM_EGR_QUEUING,
                     DNX_ALGO_PORT_LOGICALS_F_MASTER_ONLY, &tm_master_ports_bitmap));

    /** Set the ECGM tuning for the current master port and all other logical ports on
     * the same interface. The interface rate must be divided to the number of ports (channels) on the interface
     * to get the port rate and properly set the ECGM thresholds for each port. This configuration
     * should be done only one time per interface, so it is called only for master ports. */
    BCM_PBMP_ITER(tm_master_ports_bitmap, master_logical_port)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_has_speed(unit, master_logical_port, DNX_ALGO_PORT_SPEED_F_TX, &has_tx_speed));
        if (has_tx_speed)
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_speed_get(unit, master_logical_port, DNX_ALGO_PORT_SPEED_F_TX, &tx_speed));
            /** Get channels to calculate the channel rate */
            SHR_IF_ERR_EXIT(dnx_algo_port_channels_nof_get(unit, master_logical_port, &nof_channels));
            /** Configure ECGM thresholds */
            SHR_IF_ERR_EXIT(dnx_ecgm_tune_port_init(unit, master_logical_port, (tx_speed / nof_channels)));
        }
    }
    ...
    int unit)
{
    ...
    dnx_ecgm_tune_core_global_thrsholds_t core_sp_0;
    dnx_ecgm_tune_core_global_thrsholds_t core_sp_1;
    ...
    core_sp_0.pd_drop = core_multicast.pd_drop / 3 * 2;
    core_sp_0.pd_fc = core_multicast.pd_fc / 3 * 2;
    core_sp_0.db_drop = core_multicast.db_drop / 3 * 2;
    core_sp_0.db_fc = core_multicast.db_fc / 3 * 2;

    core_sp_1.pd_drop = core_sp_0.pd_drop / 2;
    core_sp_1.pd_fc = core_sp_0.pd_fc / 2;
    core_sp_1.db_drop = core_sp_0.db_drop / 2;
    core_sp_1.db_fc = core_sp_0.db_fc / 2;

    for (mc_tc = 0; mc_tc < DNX_COSQ_NOF_TC; ++mc_tc)
    {
        sp_thresholds.pd_drop[mc_tc] = (core_sp_0.pd_drop * sp_resource_percentage[mc_tc] / 100);
        sp_thresholds.db_drop_max[mc_tc] = (core_sp_0.db_drop * sp_resource_percentage[mc_tc] / 100);
        ...
        mc_tc_pd_fc[mc_tc] = (core_sp_0.pd_fc * sp_resource_percentage[mc_tc] / 100);
        mc_tc_db_fc[mc_tc] = (core_sp_0.db_fc * sp_resource_percentage[mc_tc] / 100);
    }
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ecgm_tune_threshold_core_global_mc_sp_0_pd_fc_set(unit, gport, core_sp_0.pd_fc));
        SHR_IF_ERR_EXIT(dnx_ecgm_tune_threshold_core_global_mc_sp_1_pd_fc_set(unit, gport, core_sp_1.pd_fc));
        SHR_IF_ERR_EXIT(dnx_ecgm_tune_threshold_core_global_mc_sp_0_db_fc_set(unit, gport, core_sp_0.db_fc));
        SHR_IF_ERR_EXIT(dnx_ecgm_tune_threshold_core_global_mc_sp_1_db_fc_set(unit, gport, core_sp_1.db_fc));
        ...
        SHR_IF_ERR_EXIT(dnx_ecgm_tune_threshold_core_global_mc_sp_0_pd_drop_set(unit, gport, core_sp_0.pd_drop));
        SHR_IF_ERR_EXIT(dnx_ecgm_tune_threshold_core_global_mc_sp_1_pd_drop_set(unit, gport, core_sp_1.pd_drop));
        SHR_IF_ERR_EXIT(dnx_ecgm_tune_threshold_core_global_mc_sp_0_db_drop_set(unit, gport, core_sp_0.db_drop));
        SHR_IF_ERR_EXIT(dnx_ecgm_tune_threshold_core_global_mc_sp_1_db_drop_set(unit, gport, core_sp_1.db_drop));
    ...
    int commit_last)
{
    ...
    int pd_fc_max_calc = 0, db_fc_max_calc = 0;
    ...
    port_uc.pd_fc_min = dnx_data_tune.ecgm.port_queue_uc_flow_control_min_thresh_get(unit);
    pd_fc_max_calc = UTILEX_MIN(DNX_ECGM_TUNE_CORE_UNICAST_PD(unit),
                                DNX_ECGM_TUNE_CORE_UNICAST_PD(unit) / max_num_of_ports_per_rate * fadt_max_multiplier);
    /** Set the UC FC Max threshold to the minimum between the calculated value and
     * 75% of the UC Drop threshold. This is done so that the FC threshold will not be
     * too close to the Drop threshold, as it may create a problematic state. */
    port_uc.pd_fc_max = UTILEX_MIN(pd_fc_max_calc, ((port_uc.pd_drop * 75) / 100));
    port_uc.pd_fc_alpha = 0;

    port_uc.db_fc_min = dnx_data_tune.ecgm.port_queue_uc_flow_control_min_thresh_get(unit);
    db_fc_max_calc = UTILEX_MIN(DNX_ECGM_TUNE_CORE_UNICAST_DB(unit),
                                DNX_ECGM_TUNE_CORE_UNICAST_DB(unit) / max_num_of_ports_per_rate * fadt_max_multiplier);
    /** Set the UC FC Max threshold to the minimum between the calculated value and
     * 75% of the UC Drop threshold. This is done so that the FC threshold will not be
     * too close to the Drop threshold, as it may create a problematic state. */
    port_uc.db_fc_max = UTILEX_MIN(db_fc_max_calc, ((port_uc.db_drop * 75) / 100));
    ...
    int commit_last)
{
    ...
    queue_uc.pd_fc_min = dnx_data_tune.ecgm.port_queue_uc_flow_control_min_thresh_get(unit);
    queue_uc.pd_fc_max = UTILEX_MIN(DNX_ECGM_TUNE_CORE_UNICAST_PD(unit),
                                    DNX_ECGM_TUNE_CORE_UNICAST_PD(unit) / max_num_of_ports_per_rate *
                                    fadt_max_multiplier) / 2;
    queue_uc.pd_fc_alpha = -1;

    queue_uc.db_fc_min = dnx_data_tune.ecgm.port_queue_uc_flow_control_min_thresh_get(unit);
    queue_uc.db_fc_max = UTILEX_MIN(DNX_ECGM_TUNE_CORE_UNICAST_DB(unit),
                                    DNX_ECGM_TUNE_CORE_UNICAST_DB(unit) / max_num_of_ports_per_rate *
                                    fadt_max_multiplier) / 2;
