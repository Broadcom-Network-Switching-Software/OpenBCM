    ...
    bcm_pbmp_t pbmp)
{
    ...
    if (_SHR_PBMP_IS_NULL(sfi_pbmp))
    {
        ...
        if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_clock_power_down) ||
            dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_full_power_down))
        {
     ...
     */
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_full_power_down))
    {
        SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fmacs_pbmp_to_be_enabled_get
                        (unit, sfi_pbmp, pbmp, &fmacs_to_be_enabled_bmp));
    ...
    bcm_pbmp_t pbmp)
{
    ...
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_full_power_down))
    {
        SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fmacs_pbmp_to_be_disabled_get
                        (unit, sfi_pbmp, pbmp, &fmacs_to_be_disabled_bmp));
        ...
        dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_full_power_down))
    {
        if (_SHR_PBMP_IS_NULL(sfi_pbmp))
        {
            /** Power down all fabric blocks if dynamically all links were removed*/
    ...
    bcm_pbmp_t * detached)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_fabric_if_port_blocks_power_down(unit, pbmp));
...
/** See .h file */
shr_error_e
dnx_fabric_if_fmac_fsrd_blocks_power_init(
    int unit,
    bcm_pbmp_t pbmp)
{
    int fmac_block = 0;
    bcm_pbmp_t all_fmacs, fmacs_to_be_powered_up;
    int nof_fmacs = dnx_data_fabric.blocks.nof_instances_fmac_get(unit);

    SHR_FUNC_INIT_VARS(unit);

    /**
     * Either we cpnvert the provided by the user pbmp, assuming that it is already validated that is within the valid range
     * Or we return all of the supported lanes based on SKU properties and if FAP is in SA mode
     */
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_full_power_down))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_fabric_pbmp_to_fmac_bmp_convert(unit, pbmp, &fmacs_to_be_powered_up));
    }
    else
    {
    ...
    BCM_PBMP_CLEAR(all_fmacs);
    for (fmac_block = 0; fmac_block < nof_fmacs; ++fmac_block)
    {
        BCM_PBMP_PORT_ADD(all_fmacs, fmac_block);
    }

    /** Disable all FMACs and FSRDs to align sw state with hw state */
    SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_power_set(unit, all_fmacs, 0));

    /*
     * Power up all FMAC and FSRD blocks according to their supported bitmap or provided by user pbmp
     ...
     */
    if (BCM_PBMP_NOT_NULL(fmacs_to_be_powered_up))
    {
        SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_power_set(unit, fmacs_to_be_powered_up, 1));
    }
