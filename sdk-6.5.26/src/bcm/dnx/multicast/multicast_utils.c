 ...
 *   \param [out] entry_to_move_to_start - if entry is replaced, this is the entry to move to the new ID
 *
 * \par INDIRECT INPUT:
 *   * None
 * \par DIRECT OUTPUT:
...
dnx_multicast_v1_entry_remove(
    ...
    uint32 *entry_to_move_to_start)
{
    dnx_multicast_mcdb_dbal_field_t entry_val;
    SHR_FUNC_INIT_VARS(unit);
    *cur_entry_replaced = FALSE;
    *entry_to_move_to_start = DNX_MULTICAST_END_OF_LIST_PTR;
    ...
    if (prev_entry_id == DNX_MULTICAST_END_OF_LIST_PTR)
    {
        ...
        else
        {
            ...
            *entry_to_move_to_start = next_entry_id;
 ...
 */
static shr_error_e
dnx_multicast_v1_entry_relocate(
    int unit,
    uint32 flags,
    ...
    uint32 *rep_exist)
{
    ...
    SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry(unit, group_id, FALSE, FALSE, FALSE, core_id, &entry_id));
    SHR_IF_ERR_EXIT(dnx_multicast_entry_get(unit, entry_id, &entry_val));
    ...
    dnx_multicast_group_region_t * region)
{
    ...
    entry_id -= dnx_data_bier.params.bier_mcdb_size_get(unit);

    {
                 ...
                 dnx_data_multicast.params.max_egr_mc_groups_get(unit))
        {
            ...
            *region = DNX_MULTICAST_GROUP_REGION_EG_CORE0;
                 ...
                 dnx_data_device.general.nof_cores_get(unit) * (dnx_data_multicast.params.max_egr_mc_groups_get(unit) +
                                                                1))
        {
            *id = entry_id - (dnx_data_multicast.params.max_ing_mc_groups_get(unit) + 1) -
    ...
    uint32 entry_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_v1_entry_relocate(unit, flags, group, core_id, entry_id));
    }
    ...
    uint32 *entry_to_move_to_start)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_v1_entry_remove
                        (unit, entry_id, prev_entry_id, next_entry_id, is_bmp, cur_entry_replaced,
                         entry_to_move_to_start));
    }
    ...
    dnx_multicast_trunk_action_t trunk_action)
{
    ...
    else if (BCM_MCAST_GPORT_IS_GROUP_ID(dest))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry(unit, BCM_MCAST_GPORT_GROUP_ID_GET(dest), FALSE,
                                                  DNX_MULTICAST_IS_INGRESS(flags),
    ...
    else if (BCM_MCAST_GPORT_IS_BITMAP_ID(dest))
    {
        SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry(unit, BCM_MCAST_GPORT_BITMAP_ID_GET(dest), TRUE,
                                                  DNX_MULTICAST_IS_INGRESS(flags), FALSE, core_id,
                                                  &entry_val->bmp_ptr));
        entry_val->format = DBAL_RESULT_TYPE_MCDB_BITMAP_PTR_FORMAT;
        *is_belong_to_core = TRUE;
    }
 ...
 */
shr_error_e
dnx_multicast_egress_bitmap_reps_to_bmp(
    int unit,
    int rep_count_in,
    ...
    for (rep_id = 0; rep_id < rep_count_in; rep_id++)
    {
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
        {
            SHR_IF_ERR_EXIT(dnx_multicast_convert_gport_to_hw
                            (unit, rep_array[rep_id].port, rep_array[rep_id].encap1, 0, core_id,
                             &entry_val, &is_belong_to_core, DNX_MULTICAST_TRUNK_ACTION_INVALID));
 ...
 */
shr_error_e
dnx_multicast_egress_bitmap_bmp_to_reps(
    int unit,
    uint32 cud,
    uint32 core_id,
    uint32 *bmp,
