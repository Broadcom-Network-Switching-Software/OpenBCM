    ...
    bcm_trunk_t trunk_id)
{
    ...
    SHR_ALLOC_ERR_EXIT(unique_members_arr,
                       (sizeof(dnx_trunk_unique_member_t) * arr_size),
                       "unique members array", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    ...
    SHR_ALLOC_ERR_EXIT(lb_key_ranges_arr,
                       (sizeof(dnx_algo_trunk_member_lb_key_ranges_t) * (nof_unique_members + 1)),
                       "lb key ranges array", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
