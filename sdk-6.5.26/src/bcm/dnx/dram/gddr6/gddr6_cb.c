...
const int gddr6_cb_dbal_use_flat = 1;
    ...
    uint32 mask)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_gddr6_index_verify(unit, dram_index));
    ...
    g6phy16_training_bist_info_t * info)
{
    ...
    uint64 nof_commands;
    soc_reg_above_64_val_t data_above_64, field_above_64;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_GDDR6_DRAM_CADT_TRAINING_PATTERN, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    ...
    if (gddr6_cb_dbal_use_flat)
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_GDDR6_LDFF_TRAINING_PATTERN_FLAT, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
        for (int pattern_index = 0; pattern_index < SHMOO_G6PHY16_BIST_NOF_PATTERNS; ++pattern_index)
        {
            ...
            SOC_REG_ABOVE_64_RANGE_COPY(field_above_64, 0, info->ldff_data_pattern[pattern_index], 0, 4 * 32);
            soc_reg_above_64_field_set(unit, DCC_LDFF_TRAINING_PATTERNr, data_above_64, DRAM_LDFF_DQ_PATTERN_Nf,
                                       field_above_64);
            soc_reg_above_64_field32_set(unit, DCC_LDFF_TRAINING_PATTERNr, data_above_64, DRAM_LDFF_DBI_PATTERN_Nf,
                                         info->ldff_dbi_pattern[pattern_index]);
            soc_reg_above_64_field32_set(unit, DCC_LDFF_TRAINING_PATTERNr, data_above_64, DRAM_LDFF_EDC_PATTERN_Nf,
                                         info->ldff_edc_pattern[pattern_index]);

            dbal_entry_value_field_arr32_set(unit, entry_handle_id, DBAL_FIELD_PATTERN, pattern_index, data_above_64);
    ...
    if (gddr6_cb_dbal_use_flat)
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_GDDR6_DRAM_BIST_CONFIG_PATTERN_1_FLAT, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
        ...
        for (int pattern_index = 0; pattern_index < SHMOO_G6PHY16_BIST_NOF_PATTERNS / 2; ++pattern_index)
        {
            SOC_REG_ABOVE_64_CLEAR(data_above_64);
            SOC_REG_ABOVE_64_CLEAR(field_above_64);
            SOC_REG_ABOVE_64_RANGE_COPY(field_above_64, 0, info->bist_data_pattern[pattern_index], 0, 8 * 32);
            soc_reg_above_64_field_set(unit, DCC_BIST_PATTERNr, data_above_64, BIST_DQ_PATTERN_Nf, field_above_64);
            soc_reg_above_64_field32_set(unit, DCC_BIST_PATTERNr, data_above_64, BIST_DBI_PATTERN_Nf,
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_GDDR6_DRAM_BIST_CONFIG_PATTERN_2_FLAT, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
        ...
        for (int pattern_index = 0; pattern_index < SHMOO_G6PHY16_BIST_NOF_PATTERNS / 2; ++pattern_index)
        {
            SOC_REG_ABOVE_64_CLEAR(data_above_64);
            SOC_REG_ABOVE_64_CLEAR(field_above_64);
            SOC_REG_ABOVE_64_RANGE_COPY(field_above_64, 0, info->bist_data_pattern[pattern_index + 4], 0, 8 * 32);
            soc_reg_above_64_field_set(unit, DCC_BIST_PATTERNr, data_above_64, BIST_DQ_PATTERN_Nf, field_above_64);
            soc_reg_above_64_field32_set(unit, DCC_BIST_PATTERNr, data_above_64, BIST_DBI_PATTERN_Nf,
                                         info->bist_dbi_pattern[pattern_index + 4]);
