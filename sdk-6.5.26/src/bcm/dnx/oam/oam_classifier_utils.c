...
/*
 * Check if OAM LIF is local port which indicate OAM on trunk member port.
 * If yes, OAM lif key is converted specific for OAM on trunk member port;
 * If no, do nothing.
 */
static shr_error_e
dnx_oam_lif_entry_key_convert(
    int unit,
    const dnx_oam_lif_entry_key_t * oam_lif_entry_key_in,
    dnx_oam_lif_entry_key_t * oam_lif_entry_key_out)
{
    bcm_core_t core_id = 0;
    uint32 ptc = 0xfff;
    SHR_FUNC_INIT_VARS(unit);

    SHR_NULL_CHECK(oam_lif_entry_key_in, _SHR_E_PARAM, "oam_lif_entry_key");
    SHR_NULL_CHECK(oam_lif_entry_key_out, _SHR_E_PARAM, "oam_lif_entry_key");

    sal_memcpy(oam_lif_entry_key_out, oam_lif_entry_key_in, sizeof(dnx_oam_lif_entry_key_t));

    /** Convertion specific for OAM on trunk member port. */
    if (DNX_OAM_LIF_IS_LOCAL_PORT(oam_lif_entry_key_out->oam_lif_id))
    {
        /** Prefix shifted */
        oam_lif_entry_key_out->oam_key_prefix += PREFIX_TRUNK_MEMBER_SHIFT;
        /** Set OAM LIF to PTC*/
        SHR_IF_ERR_EXIT(dnx_port_ptc_get(unit, BCM_GPORT_LOCAL_GET(oam_lif_entry_key_out->oam_lif_id), &ptc, &core_id));
        oam_lif_entry_key_out->oam_lif_id = ptc;
    }
    ...
    dnx_oam_lif_entry_value_t * oam_lif_entry_value)
{
    ...
    dnx_oam_lif_entry_key_t oam_lif_entry_key_tmp;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    sal_memset(&oam_lif_entry_key_tmp, 0, sizeof(dnx_oam_lif_entry_key_t));
    /** Convert OAM LIF entry key */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_entry_key_convert(unit, oam_lif_entry_key, &oam_lif_entry_key_tmp));
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_get_verify(unit, &oam_lif_entry_key_tmp, oam_lif_entry_value));

    /** Get table */
    if (oam_lif_entry_key_tmp.flags & DNX_OAM_CLASSIFIER_INGRESS)
    {
    ...
    /** Set key */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OAM_KEY_PREFIX, oam_lif_entry_key_tmp.oam_key_prefix);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OAM_KEY_BASE, oam_lif_entry_key_tmp.oam_lif_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, oam_lif_entry_key_tmp.core_id);
    ...
    int mp_profile)
{
    ...
    sal_memset(&oam_lif_entry_key_tmp, 0, sizeof(dnx_oam_lif_entry_key_t));
    /** Convert OAM LIF entry key */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_entry_key_convert(unit, oam_lif_db_key, &oam_lif_entry_key_tmp));
    ...
    const dnx_oam_lif_entry_value_t * oam_lif_entry_value)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_entry_key_convert(unit, oam_lif_entry_key, &oam_lif_entry_key_tmp));
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_add_verify(unit, &oam_lif_entry_key_tmp, oam_lif_entry_value));
    ...
    const dnx_oam_lif_entry_key_t * oam_lif_entry_key)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_entry_key_convert(unit, oam_lif_entry_key, &oam_lif_entry_key_tmp));
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_verify(unit, &oam_lif_entry_key_tmp));
    ...
    uint32 *acc_mp_profile)
{
    uint32 entry_handle_id;
    uint32 ptc = 0xfff;
    uint32 oam_lif_tmp = oam_lif;
    bcm_core_t core = 0;
    dbal_tables_e table_id;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    if (DNX_OAM_LIF_IS_LOCAL_PORT(oam_lif))
    {
        /** Set OAM LIF to PTC */
        SHR_IF_ERR_EXIT(dnx_port_ptc_get(unit, BCM_GPORT_LOCAL_GET(oam_lif), &ptc, &core));
        oam_lif_tmp = ptc;
    }

    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_acc_mep_get_verify(unit, core_id, flags, oam_lif_tmp, mdl, mep_id, acc_mp_profile));
    ...
    /** Set key */
    if (_SHR_IS_FLAG_SET(flags, DNX_OAM_CLASSIFIER_BFD) || DNX_OAM_LIF_IS_LOCAL_PORT(oam_lif))
    {
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OAM_LIF, oam_lif_tmp);
    ...
    int acc_mp_profile)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_acc_mep_add_verify(unit, core_id, flags, oam_lif_tmp, mdl, mep_id, acc_mp_profile));
    ...
    int mdl)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_acc_mep_verify(unit, core_id, flags, oam_lif_tmp, mdl));
