    ...
    bcm_vlan_action_set_t * action)
{
     ...
     * Check whether the action_id was allocated:
     */
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dnx_vlan_translate_action_id_is_allocated_verify(unit, flags, action_id),
                                       _SHR_E_NOT_FOUND);
    ...
    uint32 resultType)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_flow_egress_virtual_sw_info_update(unit, local_gport, -1, resultType));
        SHR_EXIT();
    }
    /*
     * Take table handle
    ...
    bcm_vlan_action_set_t * action)
{
    ...
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dnx_vlan_translate_action_id_get_verify(unit, flags, action_id, action),
                                       _SHR_E_NOT_FOUND);
    is_ingress = _SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_INGRESS) ? TRUE : FALSE;
    /*
