 ...
 * {
 */
/** Invalid value of topology ID field*/
#define DNX_VLAN_TOPOLOGY_ID_INVALID        (0)
/*
 * }
 */
 ...
 */
shr_error_e
dnx_vsi_table_initial_result_type_get(
    int unit,
    bcm_vlan_t vsi,
    ...
    uint32 *result_type)
{
    ...
    {
        *result_type = DBAL_RESULT_TYPE_ING_VSI_INFO_DB_VSI_ENTRY_BASIC;
    }
    ...
    bcm_vlan_t vsi)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange(unit, &in_lif_profile_info, old_vsi_profile,
                                                    &new_vsi_profile, ETH_RIF, dbal_table_id, &last_profile));

        new_vsi_profile = CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, new_vsi_profile);

        SHR_IF_ERR_EXIT(dnx_vsi_table_initial_result_type_get(unit, vsi, VSI_TYPE_BRIDGE_ONLY, &result_type));
    ...
    if (dbal_tables_field_info_get_no_err(unit, dbal_table_id, DBAL_FIELD_TOPOLOGY_ID,
                                          FALSE, result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
    {
    ...
    if (dbal_tables_field_info_get_no_err(unit, dbal_table_id, DBAL_FIELD_DA_NOT_FOUND_DESTINATION,
                                          FALSE, result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
    {
        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                        (unit, DBAL_FIELD_DESTINATION, DBAL_FIELD_MC_ID, &mc_id, &destination));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DA_NOT_FOUND_DESTINATION, INST_SINGLE,
                                     destination);
    }

    SHR_IF_ERR_EXIT(dbal_entry_handle_nof_value_fields_on_handle_get(unit, entry_handle_id, &nof_dbal_fields_set));
    if (nof_dbal_fields_set != 0)
    {
    ...
    bcm_vlan_t vsi)
{
    ...
    else
    {
        ...
        {
            in_lif_profile_info.ingress_fields.lif_gport = BCM_L3_ITF_VAL_GET(vsi);
            SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data
                            (unit, old_vsi_profile, &in_lif_profile_info, dbal_table_id, ETH_RIF));
        }
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_dealloc
                        (unit, old_vsi_profile, &new_in_lif_profile, &in_lif_profile_info, ETH_RIF));
    }
    ...
    bcm_vlan_control_vlan_t * control)
{
    ...
    /**check if the multicast group id is valid*/
    group_id = uc;
    if (_SHR_MCAST_GPORT_IS_GROUP_ID(uc))
    {
        group_id = BCM_MCAST_GPORT_GROUP_ID_GET(uc);
    }
    if (group_id > dnx_data_multicast.params.max_ing_mc_groups_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Ingress multicast ID (%d) is not at the valid range (> %u)\n",
                     group_id, dnx_data_multicast.params.max_ing_mc_groups_get(unit));
    }
    ...
    if (_SHR_IS_FLAG_SET(control->flags2, BCM_VLAN_FLAGS2_UNKNOWN_DEST))
    {
        ...
        if ((dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_no_trap_for_unknown_destination))
            && BCM_GPORT_IS_TRAP(control->unknown_dest))
        {
        ...
        if ((dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_no_black_hole_for_unknown_destination))
            && BCM_GPORT_IS_BLACK_HOLE(control->unknown_dest))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Black hole can not be set as a default destination in current device!\n");
        }
    }
    ...
    if (vsi < dnx_data_l3.rif.nof_rifs_get(unit))
    {
        dbal_tables_e table_id = dnx_data_l2.vsi.vsi_table_get(unit);
       ...
       /** Get in_lif_profile_info */
        SHR_IF_ERR_EXIT(in_lif_profile_info_t_init(unit, &in_lif_profile_info));
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data(unit, old_vsi_profile, &in_lif_profile_info, table_id, ETH_RIF));
        ...
        if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_application_v1))
        {
            if (intf_exists
                ...
                (((in_lif_profile_info.ingress_fields.mact_mode == DBAL_ENUM_FVAL_VSI_PROFILE_BRIDGE_SVL)
                  && (control->flags2 & BCM_VLAN_FLAGS2_IVL))
                 || ((in_lif_profile_info.ingress_fields.mact_mode == DBAL_ENUM_FVAL_VSI_PROFILE_BRIDGE_IVL)
                     && !(control->flags2 & BCM_VLAN_FLAGS2_IVL))))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Don't update the SVL/IVL properties of the vsi which is connected to an ETH_RIF interface!\n");
            }
        }
    }
    ...
    bcm_vlan_control_vlan_t * control)
{
    ...
    if (!is_vsi_allocated)
    {
        /** No error message in the case of NOT_FOUND */
    ...
    bcm_vlan_control_vlan_t control)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_control_vlan_set_verify(unit, vlan, &control));
    ...
    /*
     * Get the data for the configured VSI profile in case it is not the default one.
     */
    if (old_vsi_profile != CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, DNX_IN_LIF_PROFILE_DEFAULT))
    {
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange
                    (unit, &in_lif_profile_info, old_vsi_profile, &new_vsi_profile, ETH_RIF, table_id, &last_profile));

    new_vsi_profile = CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, new_vsi_profile);
    /*
     * Set the VSI profile with field bits
     */
    SHR_IF_ERR_EXIT(dnx_field_port_profile_vsi_set(unit, control.if_class, vlan, &new_vsi_profile));
...

    {
        ...
        if (dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_DA_NOT_FOUND_DESTINATION,
                                              FALSE, new_result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
        {
            uint32 destination = 0;
        ...
        if (dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_STAT_OBJECT_ID,
                                              FALSE, new_result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
        {
        ...
        if (dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_MC_ID,
                                              FALSE, new_result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
        {
            if (field_info.field_id == DBAL_FIELD_MC_ID)
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MC_ID, INST_SINGLE,
                                             BCM_GPORT_MCAST_GET(gport_unknown_dest));
            }
        }
        if (dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_FEC,
                                              FALSE, new_result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
        {
            if (field_info.field_id == DBAL_FIELD_FEC)
            {
    ...
    bcm_vlan_control_vlan_t * control)
{
    ...
    rv = dnx_vlan_control_vlan_get_verify(unit, vsi, control);
    ...
    /** Default destination*/
    rv = dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_MC_ID,
    ...
    else
    {
        ...
        rv = dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_FEC,
                                               FALSE, result_type, INST_SINGLE, &field_info);
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(dbal_tables_field_info_get_no_err
                            (unit, table_id, DBAL_FIELD_DA_NOT_FOUND_DESTINATION, FALSE, result_type,
                             INST_SINGLE, &field_info));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, DBAL_FIELD_DA_NOT_FOUND_DESTINATION, INST_SINGLE,
                             &dest_field_value));
    ...
    if (!dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_unified_vsi_info_db))
    {
        table_id = DBAL_TABLE_EGR_VSI_INFO_DB;
        ...
        if (dnx_stat_pp_result_type_verify(unit, table_id, 0) == _SHR_E_NONE)
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit,
                                                                entry_handle_id,
    ...
    SHR_IF_ERR_EXIT(in_lif_profile_info_t_init(unit, &in_lif_profile_info));
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data
                    (unit, old_vsi_profile, &in_lif_profile_info, dnx_data_l2.vsi.vsi_table_get(unit), ETH_RIF));
    
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_ivl_feature_support))
    {
        if (in_lif_profile_info.ingress_fields.mact_mode == DBAL_ENUM_FVAL_VSI_PROFILE_BRIDGE_IVL)
        {
    ...
    bcm_vlan_t vid)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_create_verify(unit, vid));
    ...
    bcm_vlan_t vlan)
{
    ...
    uint32 destination;
    uint32 result_type;
    dbal_tables_e vsi_table_id = dnx_data_l2.vsi.vsi_table_get(unit);
    dbal_table_field_info_t field_info;
    int trap_id;
    ...
    /** First allocate the VSI in the resource manager*/
    SHR_IF_ERR_EXIT(vlan_db.vsi.allocate_single(unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, NULL, &vsi));

    /** Check if the Black hole can be set directly as a destination*/
    if (no_black_hole_for_unknown_destination == 1)
    {
        /** Retrieve the trap_id of bcmRxTrapDfltDroppedPacket*/
        SHR_IF_ERR_EXIT(bcm_rx_trap_type_get(unit, 0, bcmRxTrapDfltDroppedPacket, &trap_id));
        BCM_GPORT_TRAP_SET(gport_dest, trap_id, dnx_data_trap.strength.default_trap_strength_get(unit), 0);
    }
    else
    {
        gport_dest = BCM_GPORT_BLACK_HOLE;
    }

    SHR_IF_ERR_EXIT(algo_gpm_encode_destination_field_from_gport
                    (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, gport_dest, &destination));

    SHR_IF_ERR_EXIT(dnx_vsi_table_initial_result_type_get(unit, vsi, VSI_TYPE_BRIDGE_ONLY, &result_type));

    /** Configure unknown DA destination */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, vsi_table_id, &entry_handle_id));

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, result_type);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI, vsi);

    if (dbal_tables_field_info_get_no_err(unit, vsi_table_id, DBAL_FIELD_MC_ID,
                                          FALSE, result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MC_ID, INST_SINGLE, destination & 0x7ffff);
    }
    if (dbal_tables_field_info_get_no_err(unit, vsi_table_id, DBAL_FIELD_DA_NOT_FOUND_DESTINATION,
                                          FALSE, result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DA_NOT_FOUND_DESTINATION, INST_SINGLE,
                                     destination);
    ...
    bcm_pbmp_t ubmp)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_port_add_verify(unit, vid, ubmp_pp_arr, tx_tag_pp_arr));
    ...
    bcm_gport_t * gport_array)
{
    ...
    /** Initialize the whole gport_array with BCM_GPORT_INVALID */
    sal_memset(gport_array, BCM_GPORT_INVALID, sizeof(bcm_gport_t) * array_max_size);
    ...
    bcm_vlan_t vid)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_destroy_verify(unit, vid));
    vsi = vid;
    ...
    bcm_vlan_t * vid_ptr)
{
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_default_get_verify(unit, vid_ptr));
    ...
    bcm_vlan_t vid)
{
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_default_set_verify(unit, vid));
    /*
    ...
    int *arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_control_port_get_verify(unit, port, type, arg));
    switch (type)
    {
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_control_port_set_verify(unit, port, type, arg));
    ...
    int flags)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_gport_add_verify(unit, vlan, port, flags));
    ...
    sal_memset(tx_tag_pp_arr, 0, sizeof(tx_tag_pp_arr));
    /*
    ...
    int *array_size)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_gport_get_all_verify(unit, vlan, array_max, gport_array, is_untagged, array_size));
    SHR_ALLOC_ERR_EXIT(gport_arr_ingress,
                       sizeof(bcm_gport_t) * array_max, "gport array ingress", "%s%s%s", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_ERR_EXIT(gport_arr_egress,
                       sizeof(bcm_gport_t) * array_max, "gport array egress", "%s%s%s", EMPTY, EMPTY, EMPTY);
    /*
     * Retrieve information of the vlan membership if port bitmap based on given vid
     */
    SHR_IF_ERR_EXIT(dnx_vlan_membership_get(unit, vlan, &vlan_mem_if_bmp_ingress, &vlan_mem_if_bmp_egress, 0));
    ...
    int *countp)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_list_verify(unit, listp, countp));
    ...
    int *countp)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_list_by_pbmp_verify(unit, listp, countp));
    ...
    int *trap_gport)
{
     ...
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, dbal_table_id, &entry_handle_id));
    dbal_value_field32_request(unit, entry_handle_id, field_id, INST_SINGLE, &vtt_default_in_lif);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
     ...
     * Set keys:
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, vtt_default_in_lif);
    /*
     * Set values:
     */
    ...
    int *arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_control_get_verify(unit, type, arg));
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_control_set_verify(unit, type, arg));
