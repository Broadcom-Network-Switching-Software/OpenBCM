...
#define DNX_TDM_SID_STAMP_PARSED_FIELD_SIZE_WIDE_MODE   (24)
/*
 * }
 */
    ...
    bcm_tdm_ingress_context_t * ingress_context)
{
    ...
    /*
     * Verify that together with ID flag valid context id is provided
     */
    if (flags & BCM_TDM_INGRESS_CONTEXT_WITH_ID)
    {
        if (*ingress_context_id <= DNX_TDM_INGRESS_INVALID_CONTEXT)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "CONTEXT_WITH_ID flag provided, context id should be bigger than 0, provided: %d \n",
                         *ingress_context_id);
        }
        if ((ingress_context->is_wide_stream_id) && (*ingress_context_id != DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "CONTEXT_WITH_ID flag provided, and is_wide_stream_id is set. context id should be BCM_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID ( %d ), provided: %d \n",
                         DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID, *ingress_context_id);
        }
        if ((*ingress_context_id == DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID)
            && (!(ingress_context->is_wide_stream_id))
            && (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_sid_wide_mode_supported)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "CONTEXT_WITH_ID flag provided. wide stream id is supported on this device, thus DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID ( %d )  is a reserved context id for the wide mode stream\n",
                         DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID);
        }
    }
    /*
    ...
    /** verify correct usage of wide_stream_id_enable */
    if (ingress_context->is_wide_stream_id)
    {
        if (!ingress_context->is_mc)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "when using wide stream id, is_mc should be true.");
        }
        if (!ingress_context->stamp_mcid_with_sid)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "when using wide stream id, stamp_mcid_with_sid should be true.");
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_tdm_ingress_context_t * ingress_context)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_tdm_ingress_context_create_verify
    ...
    if (!(flags & BCM_TDM_INGRESS_CONTEXT_REPLACE))
    {
        ...
        if (flags & BCM_TDM_INGRESS_CONTEXT_WITH_ID)
        {
            /** id validity was checked in verify */
            algo_flags = RESOURCE_TAG_BITMAP_ALLOC_WITH_ID;
        }
        else if (loc_ingress_context.is_wide_stream_id)
        {
            /** wide stream id has a reserved context in hw , allocate it */
            algo_flags = RESOURCE_TAG_BITMAP_ALLOC_WITH_ID;
            *ingress_context_id_p = DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID;
        }
        else if (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_sid_wide_mode_supported))
        {
            /** if wide sid is supported, context number DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID (1023) is reserved for it.  */
            /** thus, narrow sid should not be allocated on DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID (1023) */
            algo_flags |= RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE;
            alloc_info.range_start = 1;
            alloc_info.range_end = DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID - 1;
        }
        SHR_IF_ERR_EXIT(algo_tdm_db.
                        ingress_context.allocate_single(unit, algo_flags, (void *) &alloc_info, ingress_context_id_p));
    }
    ...
    int ingress_context_id)
{
    ...
    /*
     * Verify that valid context id is provided
     */
    if (ingress_context_id <= DNX_TDM_INGRESS_INVALID_CONTEXT)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "CONTEXT_WITH_ID flag provided, context id is not valid. Should be bigger than 0, provided: %d \n",
                     ingress_context_id);
    }
    ...
    int ingress_context_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_tdm_ingress_context_destroy_verify(unit, flags, ingress_context_id));
    /*
    ...
    SHR_IF_ERR_EXIT(algo_tdm_db.ingress_context.free_single(unit, ingress_context_id, NULL));
    ...
    bcm_tdm_ingress_context_t * ingress_context)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_tdm_ingress_context_get_verify(unit, flags, ingress_context_id, ingress_context));
    ...
    int ingress_context_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_tdm_stream_ingress_context_set_verify(unit, flags, gport, ingress_context_id));
    stream_id = BCM_GPORT_TDM_STREAM_ID_GET(gport);
    ...
    int *ingress_context_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_tdm_stream_ingress_context_get_verify(unit, flags, gport, ingress_context_id));
    stream_id = BCM_GPORT_TDM_STREAM_ID_GET(gport);
    ...
    int *context_en)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_tdm_ingress_context_failover_verify(unit, flags, nof_contexts, context_ids, context_en));
    ...
    bcm_tdm_interface_config_t * interface_config)
{
    ...
    int sid_stamp_parsed_field_size;
    ...
    if (interface_config->stream_id_enable == FALSE)
    {
        ...
        if (interface_config->stream_id_wide_enable)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Stream ID wide cannot be enabled when Stream ID is not enabled. stream_id_enable is %d and stream_id_wide_enable is %d\n",
                         interface_config->stream_id_enable, interface_config->stream_id_wide_enable);
        }
    ...
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Stream ID key offset in bytes:%d is out of valid range 0-%d\n",
                         UTILEX_TRANSLATE_BITS_TO_BYTES(interface_config->stream_id_key_offset),
                         dnx_data_tdm.params.sid_offset_nof_get(unit) - 1);
        }

        /*
         * Verify offset in bits. It is possible that the offset will pass the check for bytes,
         * but will still be over the maximal value in bits
         */
        if ((interface_config->stream_id_key_offset < 0) ||
            (interface_config->stream_id_key_offset >= (dnx_data_tdm.params.sid_offset_nof_in_bits_get(unit) - 1)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Stream ID key offset in bits:%d is out of valid range 0-%d\n",
                         interface_config->stream_id_key_offset,
                         dnx_data_tdm.params.sid_offset_nof_in_bits_get(unit) - 1);
        }
        if (interface_config->stream_id_wide_enable)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Wide Stream ID is not supportted on this device. stream_id_wide_enable is :%d \n",
                             interface_config->stream_id_wide_enable);
            }
            if (interface_config->stream_id_base != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "when using stream_id_wide_enable, stream_id_base must be 0, provided:%d \n",
                             interface_config->stream_id_base);
            }
        }
        ...
        if (interface_config->stream_id_wide_enable)
        {
            sid_stamp_parsed_field_size = DNX_TDM_SID_STAMP_PARSED_FIELD_SIZE_WIDE_MODE;
        }
        else
        {
            sid_stamp_parsed_field_size = DNX_TDM_SID_STAMP_PARSED_FIELD_SIZE;
        }
        if ((interface_config->stream_id_key_offset + interface_config->stream_id_key_size) >
            (UTILEX_ALIGN_DOWN(interface_config->stream_id_key_offset, 8) + sid_stamp_parsed_field_size))
        {
...
bcm_dnx_tdm_interface_config_set(
    ...
    bcm_tdm_interface_config_t * interface_config)
{
    int nif_interface_id = 0, core_id, sid_stamp_parsed_field_size;
    uint32 shift_size;
    bcm_port_t port;

    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_dnx_tdm_interface_config_set_verify(unit, flags, gport, interface_config));
    if (BCM_GPORT_IS_LOCAL(gport))
    {
        port = BCM_GPORT_LOCAL_GET(gport);
    }
    ...
    /**
     * From the packet, starting from stream_id_key_offset, the extracted bits are always DNX_TDM_SID_STAMP_PARSED_FIELD_SIZE.
     * In order to get the actual SID, which has size stream_id_key_size (in bits_ a shift right is performed on the extracted value.
     * However, the stream_id_key_offset must be in bytes in the HW field, so the difference between the user input value and the
     * value in bytes is substracted from the shift.
     */
    if (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_sid_wide_mode_supported) &&
        (interface_config->stream_id_wide_enable))
    {
    ...
    shift_size = sid_stamp_parsed_field_size -
...
/* *INDENT-OFF* */
        SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_TDM_STREAM_CONFIGURATION,
                            ...
                            GEN_DBAL_FIELD32, DBAL_FIELD_SID_WIDE_MODE_ENABLE, INST_SINGLE, interface_config->stream_id_wide_enable,
                            GEN_DBAL_FIELD_LAST_MARK));
/* *INDENT-ON* */
    }
    else
    ...
    bcm_tdm_interface_config_t * interface_config)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_dnx_tdm_interface_config_get_verify(unit, flags, gport, interface_config));
    ...
    if (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_sid_wide_mode_supported))
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_SID_WIDE_MODE_ENABLE, INST_SINGLE,
                         (uint32 *) &interface_config->stream_id_wide_enable));
    }
    else
    {
        interface_config->stream_id_wide_enable = FALSE;
    }
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_SID_BITS_SIZE, INST_SINGLE,
                     (uint32 *) &interface_config->stream_id_key_size));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    if (interface_config->stream_id_enable == TRUE)
    {
         ...
         * In order to get the actual SID, which has size stream_id_key_size (in bits_ a shift right is performed on the extracted value.
         * However, the stream_id_key_offset must be in bytes in the HW field, so the difference between the user input value and the
         * value in bytes is subtracted from the shift. To reverse-calculate the offset, need to add back the difference between
         * shift + stream_id_key_size and DNX_TDM_SID_STAMP_PARSED_FIELD_SIZE to the value read from HW.
         */
        if (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_sid_wide_mode_supported) &&
            (interface_config->stream_id_wide_enable))
        {
        ...
        interface_config->stream_id_key_offset =
            (sid_stamp_parsed_field_size - (shift_size + interface_config->stream_id_key_size)) + bytes_offset * 8;
    }
