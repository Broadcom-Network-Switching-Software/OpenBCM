    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_get(unit, gport, &forward_info));
    ...
    dnx_mpls_l2vpn_in_lif_info_t * lif_info)
{
    ...
    if (fwd_info->fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_OUTLIF)
    {
        ...
        /** set learn_strength */
        SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                        (unit, learn_info_field_id, DBAL_FIELD_LEARN_STRENGTH, &learn_strength, lif_info->learn_info));
    }
    /*
     * For dest only:
     * Use NO_LIF_BASIC
    ...
    /** set destination */
    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                    (unit, learn_info_field_id, DBAL_FIELD_DESTINATION, &learn_strength_dest, lif_info->learn_info));
    ...
    if (l2vpn_info->type != MPLS_L2VPN_TYPE_EVPN)
    {
        ...
        if (is_replace &&
            (dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
             DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE))
        {
            SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_get(unit, l2vpn_info->mpls_port->mpls_port_id, &fwd_info));
        }
        else
        {
