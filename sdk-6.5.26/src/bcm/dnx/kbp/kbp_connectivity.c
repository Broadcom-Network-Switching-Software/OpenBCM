    ...
    struct kbp_device_config *kbp_config)
{
    ...
    if (dnx_kbp_interface_enabled(unit))
    {
        ...
        BCM_PBMP_ITER(port_pbmp, ilkn_port)
        {
            ...
            {
                /*
                 * Here is an assumption that the port0/2 must be connected to the
                 * first ilkn of egw. It means the port0/2 connects to
                 * ile0 or ile2,port1/3 connects to ile1 or ile3
                 */
                if (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_is_egw_support_rr))
                {
                    kbp_port_id = kbp_inst * 2;
                    if (dnx_data_elk.connectivity.topology_get(unit)->port_core_mapping[kbp_port_id] == temp_core_id)
                    {
                ...
                /*
                 * This branch is for jr2 X 2 op2. only for internal.
                 */
                else
                {
        ...
        /*
         *  Case Op2 to act like Tap device
         */
        else if (Kbp_app_data[unit]->device_type == KBP_DEVICE_TAP)
        {
    ...
    bcm_core_t core)
{
    ...
    if (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_elk_nif_if_type_sel))
    {
        ...
        BCM_PBMP_ITER(port_pbmp, ilkn_port)
        {
            ...
            /*
             * Since we have single ELK I/F for J2C we can stop here 
             */
            break;
        }
    }
    ...
    bcm_core_t core)
{
    ...
    /*
     * OP code is not necessary for OP2 since it doesn't support ROP RW.
     */
    ...
    int unit)
{
    ...
    DNX_KBP_INST_ITER(unit, kbp_inst)
    {
        ...
        /** For Tap device, turn off searches */
        ...
        if (Kbp_app_data[unit]->device_type == KBP_DEVICE_OP2 && kbp_config->settings.link_training_enable == 0)
        {
            ...
            /**toggle TX & RX together. Actually toggle TX is enough*/
            if (port_bmp)
            {
    ...
    BCM_PBMP_ITER(elk_pbmp, elk_port)
    {
        ...
        /*
         * Enable RX buffers manually during init sequence
         */
