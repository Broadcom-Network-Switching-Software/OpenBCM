...
#include <soc/dnx/dnx_data/auto_generated/dnx_data_egr_queuing.h>       /* egq dnx data */
 ...
 * while featur v1 exsist
 */
static shr_error_e dnx_ecgm_support_check(
    int unit,
    bcm_gport_t gport,
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_db.port_info.alloc(unit));
    }
    ...
    int unit)
{
    ...
    {
        /** Get the master port. If the current port is slave, the configuration from master needs to be copied to it. */
        ...
        if (logical_port != master_port)
        {
            ...
            if (has_tx_speed)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ecgm_port_tune(unit, logical_port, tx_speed));
            }
        }
    }
exit:
    ...
    int unit)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
    {
        ...
        if (out_tm_ref_count == 1)
        {
            ...
            /*
             * flexe ports at J2X has dedicated interfaces without ECGM interface resource manangement HW support
             */
    ...
    bcm_cosq_egress_multicast_config_t * config)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_multicast_config_verify(unit, system_tc, system_dp, config));
    ...
    bcm_cosq_egress_multicast_config_t * config)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_dbal_mc_se_get
    ...
    bcm_cosq_threshold_t * threshold)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_support_check(unit, gport, cosq, threshold));
    }
    ...
    bcm_cosq_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_gport_threshold_get_verify(unit, threshold));
    if (dnx_data_ecgm.general.feature_get(unit, dnx_data_ecgm_general_v1_supported))
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_support_check(unit, gport, cosq, threshold));
    }
    ...
    bcm_cosq_egress_multicast_config_t * config)
{
    ...
    if ((int) system_dp < 0 || system_dp >= DNX_COSQ_NOF_DP)
    {
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    if (flags & BCM_COSQ_THRESHOLD_DROP)
    {
        ...
        {
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_dp_verify(unit, data->threshold->dp));
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    switch (data->tag)
    {
             ...
             */
        case (PD_DROP):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_global_pd_drop_verify(unit, core, value));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_global_threshold_set(unit, core, DBAL_FIELD_PD_DROP, value));
            break;
        case (PD_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_global_pd_fc_verify(unit, core, value));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_global_threshold_set(unit, core, DBAL_FIELD_PD_FLOW_CONTROL, value));
            break;
        case (DB_DROP):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_global_db_drop_verify(unit, core, value));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_global_threshold_set(unit, core, DBAL_FIELD_DB_DROP, value));
            break;
        case (DB_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_global_db_fc_verify(unit, core, value));
        ...
        case (MC_DB_FC):
             ...
             */
        case (SP_PD_DROP):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_sp_pd_drop_verify(unit, core, data));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_multicast_sp_threshold_set
                            (unit, core, data->sp, DBAL_FIELD_PD_DROP, value));
            break;
        case (SP_PD_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_sp_pd_fc_verify(unit, core, data));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_multicast_sp_threshold_set
                            (unit, core, data->sp, DBAL_FIELD_PD_FLOW_CONTROL, value));
            break;
        case (SP_DB_DROP):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_sp_db_drop_verify(unit, core, data));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_multicast_sp_threshold_set
                            (unit, core, data->sp, DBAL_FIELD_DB_DROP, value));
            break;
        case (SP_DB_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_sp_db_fc_verify(unit, core, data));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_multicast_sp_threshold_set
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    {
        case (MC_PD_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_mc_tc_pd_fc_verify(unit, core, value));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_tc_multicast_threshold_set
                            (unit, core, data->cosq, DBAL_FIELD_PD_FLOW_CONTROL, value));
            break;
        case (MC_DB_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_mc_tc_db_fc_verify(unit, core, value));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_tc_multicast_threshold_set
                            (unit, core, data->cosq, DBAL_FIELD_DB_FLOW_CONTROL, value));
            break;
        case (SP_PD_DROP):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_sp_tc_pd_drop_verify(unit, core, data));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_tc_multicast_sp_threshold_set
                            (unit, core, data->cosq, data->sp, DBAL_FIELD_PD_DROP, value));
            break;
        case (SP_DB_MAX_DROP):
        case (SP_DB_DROP):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_sp_tc_db_drop_verify(unit, core, data));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_tc_multicast_sp_threshold_set
            ...
            break;
        case (SP_DB_MIN_DROP):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_sp_tc_db_min_drop_verify(unit, core, data));
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_tc_multicast_sp_threshold_set
                            (unit, core, data->cosq, data->sp, DBAL_FIELD_DB_DROP_MIN, value));
            break;
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_core_sp_tc_db_reserved_verify(unit, core, data));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_tc_multicast_sp_threshold_get
    ...
    bcm_cosq_threshold_t * threshold)
{
    ...
    if (BCM_COSQ_GPORT_IS_CORE(gport) || gport == 0)
    {
        if (data.tag == DB_DROP || data.tag == DB_FC || data.tag == MC_DB_FC ||
            data.tag == SP_DB_DROP || data.tag == SP_DB_FC || data.tag == MC_DB_FC ||
            data.tag == SP_DB_MIN_DROP || data.tag == SP_DB_MAX_DROP ||
            data.tag == SP_DB_ALPHA_0_DROP || data.tag == SP_DB_ALPHA_1_DROP ||
            data.tag == UC_DB_DROP || data.tag == UC_DB_FC || data.tag == MC_DB_DROP)
        {
    ...
    {
        if (data.tag == DB_DROP || data.tag == DB_DROP_PER_DP)
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Supported by different API, Use user manual");
        }
    }
    else if (BCM_COSQ_GPORT_IS_UCAST_EGRESS_QUEUE(gport))
    {
        if (data.tag == DB_DROP || data.tag == DB_FC || data.tag == DB_MIN_FC ||
            data.tag == DB_MAX_FC || data.tag == DB_ALPHA_FC)
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Supported by different API, Use user manual");
        }
    }
    ...
    {
        if (data.tag == UC_DB_FC || data.tag == UC_DB_MIN_FC || data.tag == UC_DB_MAX_FC || data.tag == UC_DB_ALPHA_FC)
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Supported by different API, Use user manual");
        }
    }
