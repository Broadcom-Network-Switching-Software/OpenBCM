...
/** Maximum reserved PDs per SP must be below the total PDs of the SP minus 2xPDs for each Q-pair */
#define NOF_PDS_BELOW_MAX(unit) (dnx_data_egr_queuing.params.nof_q_pairs_get(unit)*2)
...
static shr_error_e dnx_ecgm_profile_mgmt_queue_info_hw_set(
    int unit,
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle);
    ...
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get(unit, port, &port_profile_handle->core,
                                                  &port_profile_handle->out_tm_port));
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_dbal_out_tm_port_to_port_profile_get
                        (unit, port_profile_handle->core, port_profile_handle->out_tm_port,
        ...
        if (dnx_data_ecgm.queue_resources.feature_get(unit, dnx_data_ecgm_queue_resources_no_profile_management))
        {
            SHR_IF_ERR_EXIT(dnx_ecgm_db.port_info.get(unit, port_profile_handle->core, port_profile_handle->out_tm_port,
                                                      &port_info));
            sal_memcpy(&(port_profile_handle->info.multicast_queue), &(port_info->multicast_queue),
                       sizeof(dnx_ecgm_mc_drop_thresholds_t));
            sal_memcpy(&(port_profile_handle->info.unicast_queue), &(port_info->unicast_queue),
                       sizeof(dnx_ecgm_uc_thresholds_t));
        }
    }
    ...
    int sw_only)
{
    ...
    else
    {
        ...
        if (dnx_data_ecgm.queue_resources.feature_get(unit, dnx_data_ecgm_queue_resources_no_profile_management))
        {
            SHR_IF_ERR_EXIT(dnx_ecgm_db.port_info.set(unit, port_profile_handle->core, port_profile_handle->out_tm_port,
                                                      &port_profile_handle->info));
        }
    ...
    int pd_sp)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_pd_sp_set_verify(unit, port_profile_handle, queue, pd_sp));
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_pd_sp_get_verify(unit, port_profile_handle, queue, pd_sp));
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_port_set_verify(unit, port, data));
    ...
    if (data->cosq == -1)
    {
        ...
        {
            case (PD_DROP):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_unicast_port_pd_drop_verify
                                       (unit, &port_profile_handle, value));
                port_profile_handle.info.unicast_port.pd_drop = value;
                break;
            case (PD_MAX_FC):
            case (PD_FC):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_unicast_port_pd_fc_verify
                                       (unit, &port_profile_handle, value));
                ...
                break;
            case (PD_MIN_FC):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_pd_threshold_verify(unit, value));
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_min_threshold_verify
                                       (unit, port_profile_handle.info.unicast_port.flow_control.pd_fc.max, value));
                port_profile_handle.info.unicast_port.flow_control.pd_fc.min = value;
                break;
            case (PD_ALPHA_FC):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_alpha_verify(unit, value));
                port_profile_handle.info.unicast_port.flow_control.pd_fc.alpha = value;
                break;
            case (DB_DROP):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_unicast_port_db_drop_verify
                                       (unit, &port_profile_handle, value));
                port_profile_handle.info.unicast_port.db_drop = value;
                break;
            case (DB_MAX_FC):
            case (DB_FC):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_unicast_port_db_fc_verify
                                       (unit, &port_profile_handle, value));
                ...
                break;
            case (DB_MIN_FC):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_db_threshold_verify(unit, value));
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_min_threshold_verify
    ...
    else if (data->cosq >= 0 && data->cosq < 8)
    {
        ...
        {
            case (PD_DROP):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_unicast_queue_pd_drop_verify
                                       (unit, &port_profile_handle, data->cosq, value));
                port_profile_handle.info.unicast_queue[data->cosq].pd_drop = value;
                break;
            case (PD_MAX_FC):
            case (PD_FC):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_unicast_queue_pd_fc_verify
                                       (unit, &port_profile_handle, data->cosq, value));
            ...
            case (PD_ALPHA_FC):
                ...
                break;
            case (DB_DROP):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_unicast_queue_db_drop_verify
                                       (unit, &port_profile_handle, data->cosq, value));
                port_profile_handle.info.unicast_queue[data->cosq].db_drop = value;
                break;
            case (DB_MAX_FC):
            case (DB_FC):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_unicast_queue_db_fc_verify
                                       (unit, &port_profile_handle, data->cosq, value));
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    if (data->cosq == -1)
    {
        switch (data->tag)
        {
            case (PD_MAX_DROP):
            case (PD_DROP):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_multicast_port_pd_drop_verify
                                       (unit, &port_profile_handle, value));
            ...
            case (PD_ALPHA_DROP):
                ...
                break;
            case (DB_DROP):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_multicast_port_db_drop_verify
                                       (unit, &port_profile_handle, value));
    ...
    else if (data->cosq >= 0 && data->cosq < DNX_COSQ_NOF_TC)
    {
        switch (data->tag)
        {
            ...
            case (PD_DROP_PER_DP):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_multicast_queue_pd_drop_verify
                                       (unit, &port_profile_handle, value));
            ...
            case (PD_ALPHA_DROP_PER_DP):
                ...
                break;
            case (DB_DROP_PER_DP):
                SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_multicast_queue_db_drop_verify
                                       (unit, &port_profile_handle, value));
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_interface_set_verify(unit, port, data));
    ...
    switch (data->tag)
    {
        case (UC_PD_MAX_FC):
        case (UC_PD_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_interface_uc_pd_fc_verify(unit, port, value));
        ...
        case (UC_PD_MIN_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_pd_threshold_verify(unit, value));
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_min_threshold_verify(unit, priority_ptr->uc_fc.pd_fc.max, value));
            priority_ptr->uc_fc.pd_fc.min = value;
            break;
        case (UC_PD_ALPHA_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_alpha_verify(unit, value));
        ...
        case (UC_DB_MAX_FC):
        case (UC_DB_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_interface_uc_db_fc_verify(unit, port, value));
        ...
        case (UC_DB_MIN_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_db_threshold_verify(unit, value));
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_min_threshold_verify(unit, priority_ptr->uc_fc.db_fc.max, value));
            priority_ptr->uc_fc.db_fc.min = value;
            break;
        case (UC_DB_ALPHA_FC):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_alpha_verify(unit, value));
            ...
            break;
        case (MC_PD_DROP):
            SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_interface_mc_pd_drop_verify(unit, port, value));
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_interface_threshold_verify
    ...
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    info = port_profile_handle->info;

    if (dnx_data_ecgm.queue_resources.feature_get(unit, dnx_data_ecgm_queue_resources_no_profile_management))
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_profile_mgmt_queue_info_hw_set(unit, port_profile_handle));
        /*
         * As queue thresholds are not managed at profile manger they need to be zeroed to have no enfluence
         */
        sal_memset(&(info.multicast_queue), 0, sizeof(dnx_ecgm_mc_drop_thresholds_t));
        sal_memset(&(info.unicast_queue), 0, sizeof(dnx_ecgm_uc_thresholds_t));
    }

    SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_port_profile.exchange
                    (unit, port_profile_handle->core, 0, &info, old_profile, NULL,
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_dbal_out_tm_port_to_port_profile_set
                        (unit, port_profile_handle->core, port_profile_handle->out_tm_port,
...
static shr_error_e
dnx_ecgm_profile_mgmt_queue_info_hw_set(
    int unit,
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle)
{
    uint32 base_qpair_or_port_profile;
    int queue_offset, dp;
    SHR_FUNC_INIT_VARS(unit);

    if (!dnx_data_ecgm.queue_resources.feature_get(unit, dnx_data_ecgm_queue_resources_no_profile_management))
    {
        base_qpair_or_port_profile = port_profile_handle->profile_id;
    }
    else
    {
        base_qpair_or_port_profile = port_profile_handle->base_q_pair;
    }
    ...
    for (queue_offset = 0; queue_offset < DNX_COSQ_NOF_TC; ++queue_offset)
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_dbal_unicast_queue_thresholds_set
                        (unit, port_profile_handle->core, base_qpair_or_port_profile, queue_offset,
        ...
        for (dp = 0; dp < DNX_COSQ_NOF_DP; ++dp)
        {
            SHR_IF_ERR_EXIT(dnx_ecgm_dbal_multicast_queue_tc_dp_thresholds_set
                            (unit, port_profile_handle->core, base_qpair_or_port_profile, queue_offset, dp,
    ...
    dnx_ecgm_threshold_sp_action_e action)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_sp_reserved_pd_threshold_modify_verify(unit, core_id, sp, reserved));
    ...
    uint32 sp_reserved)
{
    ...
    uint32 current_sp_drop;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_multicast_sp_threshold_get
                    (unit, core, ((sp == 0) ? 1 : 0), DBAL_FIELD_RESERVED_PD, &other_sp_reserved));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_multicast_sp_threshold_get
                    (unit, core, sp, DBAL_FIELD_PD_DROP, &current_sp_drop));
    ...
    if (sp_reserved > (current_sp_drop - NOF_PDS_BELOW_MAX(unit)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Invalid new reserved value - the new reserved PDs value %d for SP%d is over the limit. Must be below max-%d, where max PDs is %d!",
                     sp_reserved, sp, NOF_PDS_BELOW_MAX(unit), current_sp_drop);
    }
    ...
    int value)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_profile_mgmt_multicast_queue_sp_pd_reserved_verify
