    ...
    int numq)
{
    ...
    uint32 old_profile;
    int core_idx;
    int new_profile, is_last = FALSE, is_allocated = FALSE;
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_default_tc_map_profile_get(unit, FALSE, &tc_mapping));
    DNXCMN_CORES_ITER(unit, core, core_idx)
    {
        /**
         * Update sw state
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_map_profile_exchange
                        (unit, core_idx, base_voq, TRUE, &tc_mapping, &old_profile, &new_profile, &is_last,
                         &is_allocated));
        /**
         * Update hw
         */
