    ...
    int unit)
{
    ...
    for (mem_index = 0; mem_index < table_info->key_size[0]; mem_index++)
    {
        ...
        switch (default_mem->mode)
        {
            ...
            {
                is_seq_array = (default_mem->mode == dnx_init_mem_default_mode_index_seq_array) ? 1 : 0;
#ifdef PRINT_MEM_INIT_TIME
                if (soc_sand_is_emulation_system(unit))
                {
                    passed_secs = ((usecs = sal_time_usecs()) - start_usecs) / 1000000;
                ...
                SHR_IF_ERR_EXIT(dnxc_fill_table_with_index_val
                                (unit, default_mem->mem, default_mem->field, is_seq_array));
#else
                dnxc_fill_table_with_index_val(unit, default_mem->mem, default_mem->field, is_seq_array);
#endif
                /** mark this map as excluded (will be already handled) */
                SHR_BITSET(mem_exclude_bitmap, default_mem->mem);
    ...
    int *is_non_standard)
{
    ...
    for (mem_index = 0; mem_index < table_info->key_size[0]; mem_index++)
    {
        ...
        {
            new_mem = access_mcm2access_mem_ids[default_mem->mem];
        }
        else
#endif
        {
            new_mem = default_mem->mem;
        }
    ...
    dnx_init_mem_default_mode_e * mode)
{
    ...
    if (SOC_IS_NEW_ACCESS_INITIALIZED(unit))
    {
        access_runtime_info_t *runtime_info = SOC_NEW_ACCESS_RUNTIME_INFO(unit);        /* Access information for the
                                                                                         * device */
        const access_device_type_info_t *device_info = runtime_info->device_type_info;  /* Constant access information
                                                                                         * for the device type */
        access_local_regmem_id_t local_regmem_id;       /* Local register or memory to access */
        const access_device_regmem_t *rm_info;  /* The register or memory to access */

        SHR_IF_ERR_EXIT(access_regmem_global2local(runtime_info, mem, &local_regmem_id));
        rm_info = device_info->local_regs + local_regmem_id;
        sal_memset(entry, 0, ACESSS_REGMEM_T_WIDTH_IN_UINT32S(rm_info));
    }
    else
#endif
    {
