...
#define IPMC_IS_IPV4_DEFAULT_ENTRY(data) ((((data->s_ip_addr & data->s_ip_mask) == 0 && (data->mc_ip_addr & data->mc_ip_mask) == 0) || _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_DEFAULT_ENTRY))\
...
#define IPMC_IS_IPV6_DEFAULT_ENTRY(data, compressed_vrfg) (((compressed_vrfg ? ((dnx_ipmc_ipv6_default_address(data->s_ip6_addr, data->s_ip6_mask) && data->mc_ip_addr == 0)) \
    : (dnx_ipmc_ipv6_default_address(data->s_ip6_addr, data->s_ip6_mask) &&  dnx_ipmc_ipv6_default_address(data->mc_ip6_addr, data->mc_ip6_mask))) \
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        ...
        {
            if (vrfg_vsig_support && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
            {
            ...
            else
            {
                ...
                if (data->mod_id > 0 && _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
                {
                    if (data->s_ip_addr >= dnx_data_l3.fwd.nof_compressed_ivl_sip_get(unit))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "Invalid value for IPv6 compressed SIP, expected value which is less than %d \n",
