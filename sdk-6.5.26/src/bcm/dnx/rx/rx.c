    ...
    bcm_gport_t * dst_gport)
{
    ...
    int core_idx, core_id = dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit);
    ...
    *dst_gport = 0;
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT)))
    {
            ...
            first_lane >= dnx_data_nif.phys.max_phys_in_core_get(unit))
        {
            ...
            global_lane =
                first_lane - dnx_data_nif.phys.max_phys_in_core_get(unit) + dnx_data_nif.phys.first_mgmt_phy_get(unit);
        }
        else
        {
    ...
    else
    {
        ...
        while ((logical_port == DNX_ALGO_PORT_INVALID) && (cosq < 8) && (port_channel >= cosq))
        {
            ...
            {
                core_idx = core_id + i * dnx_data_device.general.nof_cores_per_die_get(unit);
    ...
    void *param)
{
    ...
    {
        if ((dnx_drv_soc_attached(i) == TRUE) && (dnxc_init_is_init_done_get(i) == TRUE))
        {
    ...
    {
        if ((dnx_drv_soc_attached(current_unit) == FALSE) || (dnxc_init_is_init_done_get(current_unit) == FALSE))
        {
            if (toggle_unit)
            {
        ...
        if (rv == _SHR_E_NONE)
        {
             ...
             */
            adapter_build_rx_packet(current_unit, ms_id, nof_signals, port_first_lane,
