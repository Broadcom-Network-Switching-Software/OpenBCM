    ...
    smart_template_alloc_info_t * extra_alloc_info)
{
    ...
    {
        int grain_size = multi_set_template->allocation_bitmap->tag_level_info[0].grain_size;
    ...
    smart_template_alloc_info_t * extra_alloc_info)
{
    ...
    else
    {
        ...
        iter_range_start = multi_set_template->allocation_bitmap->first_element;
        iter_range_end = multi_set_template->allocation_bitmap->count;
    }

    /** Set increment value for the iterator */
    increment = multi_set_template->allocation_bitmap->tag_level_info[0].grain_size;
     ...
     */
    min_allocations = multi_set_template->allocation_bitmap->tag_level_info[0].grain_size;
    while (grain_iter - iter_range_end != 0)
    {
        ...
        if (rv == _SHR_E_NOT_FOUND)
        {
             ...
             */
            chosen_grain = grain_iter / multi_set_template->allocation_bitmap->tag_level_info[0].grain_size;
            break;
        }
    ...
    extra_alloc_info->resource_alloc_info.range_start =
        chosen_grain * multi_set_template->allocation_bitmap->tag_level_info[0].grain_size;
    extra_alloc_info->resource_alloc_info.range_end =
        extra_alloc_info->resource_alloc_info.range_start +
        multi_set_template->allocation_bitmap->tag_level_info[0].grain_size;
exit:
    SHR_FUNC_EXIT;
}
