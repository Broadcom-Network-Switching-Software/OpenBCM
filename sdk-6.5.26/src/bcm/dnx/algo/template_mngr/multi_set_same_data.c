    ...
    uint8 *allocation_needed)
{
    ...
    *allocation_needed = TRUE;
    ...
    while (DNX_SW_STATE_LL_IS_NODE_VALID(node))
    {
        ...
        SHR_IF_ERR_EXIT(sw_state_ll_next_node(unit, node_id, multi_set->ll_list, ll_head_id, node, &node));
    }
    ...
    uint8 *first_appear)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_index_hash_table_entry_lookup
                    (unit, node_id, multi_set->hash_table, (SW_STATE_INDEX_HASH_TABLE_KEY *) data, NULL, &ll_head_id,
                     &exist));
    if (exist != TRUE)
    {
        /*
         ...
         */
        SHR_IF_ERR_EXIT(sw_state_index_hash_table_entry_add
                        (unit, node_id, multi_set->hash_table, (SW_STATE_INDEX_HASH_TABLE_KEY *) data, NULL,
                         &ll_head_id, &success));
        allocation_needed = TRUE;
    }
    else
    ...
    *last_appear = (ref_counter == 0) ? TRUE : FALSE;
    ...
    if (*last_appear)
    {
        ...
        if (is_last_node == TRUE)
        {
             ...
             */
            SHR_IF_ERR_EXIT(sw_state_index_hash_table_entry_remove_by_index
                            (unit, node_id, multi_set->hash_table, ll_head_id));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    void *data)
{
    ...
    SHR_IF_ERR_EXIT(multi_set_same_data_get_head_id(unit, node_id, multi_set, _profile, &ll_head_id));
    SHR_IF_ERR_EXIT(sw_state_index_hash_table_get_by_index
                    (unit, node_id, multi_set->hash_table, ll_head_id, (SW_STATE_INDEX_HASH_TABLE_KEY *) data, &found));
