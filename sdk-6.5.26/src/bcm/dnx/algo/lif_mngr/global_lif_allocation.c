    ...
    int global_lif)
{
    ...
    if (is_one_sided_symmetric && is_ingress &&
        (dnx_data_lif.global_lif.three_intf_glem_get(unit) ||
         (!dnx_data_lif.global_lif.feature_get(unit, dnx_data_lif_global_lif_outlif_profile_in_glem))))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot allocate symmetric in ingress, only in egress");
    }
    ...
    if (alloc_info)
    {
            ...
            (alloc_info->outlif_intf != GLOBAL_LIF_INTF_DISABLED))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "INTF is currently not supported, received invalid value(0x%x), only allwed 0x%x",
                         alloc_info->outlif_intf, GLOBAL_LIF_INTF_DISABLED);
        }
        if ((!with_id) && (!is_egress) && is_ingress && (alloc_info->outlif_intf != 0))
        {
        ...
        if ((!with_id) && (!is_egress) && is_ingress && (alloc_info->outlif_group != 0))
        {
    ...
    int *global_lif)
{
     ...
     */
    SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                    symmetric_global_lif.allocate_single(unit, res_mngr_flags, internal_alloc_info, global_lif));
    ...
    int *global_lif)
{
    ...
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                        ingress_global_lif.allocate_single(unit, res_mngr_flags, internal_alloc_info, global_lif));
    }
    else
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
    ...
    uint32 alloc_flags)
{
    ...
    int element;
    uint8 is_tag_supported = FALSE;

    SHR_FUNC_INIT_VARS(unit);

    if (extra_arguments != NULL)
    {
        is_tag_supported = *((uint8 *) extra_arguments);
    }
    ...
    if (is_tag_supported == TRUE)
    {
        dnx_global_lif_tag_max_val_get(unit, &extra_create_info.max_tags_value[0]);
    }
    ...
    SHR_IF_ERR_EXIT(resource_tag_bitmap_create
                    (unit, node_id, res_tag_bitmap, &res_tag_bitmap_create_info, &extra_create_info, alloc_flags));
    ...
    int unit)
{
    ...
    is_tag_supported = FALSE;
    /*
     * data.desc = "Ingress global lif allocation pool.";
     */
    SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                    ingress_global_lif.create(unit, &global_lif_data, &is_tag_supported));
    ...
    is_tag_supported = TRUE;
    /*
     * data.desc = "Egress global lif allocation pool.";
     */
    SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                    egress_global_lif.create(unit, &global_lif_data, &is_tag_supported));
     ...
     */
    SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                    symmetric_global_lif.create(unit, &global_lif_data, &is_tag_supported));
