    ...
    bcm_pbmp_t * logicals)
{
    ...
    switch (logicals_type)
    {
        ...
        {
            /** throw an error if unsupported flag requested */
            supported_flags = DNX_ALGO_PORT_LOGICALS_F_MASTER_ONLY | DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_REMOVED_PORT;
    ...
    int *local_phy)
{
    ...
    {
        if (global_phy >= dnx_data_nif.phys.first_mgmt_phy_get(unit))
        {
            *local_phy = global_phy - dnx_data_nif.phys.first_mgmt_phy_get(unit);
            SHR_EXIT();
        }
    }
    ...
    dnx_algo_port_diag_info_t * diag_info)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF
        (unit, diag_info->port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT, OTN_PHY)))
    {
        /** first phy for nif */
    ...
    int *pll_index)
{
    ...
    /** if nif port include elk and stif */
             ...
             (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT, OTN_PHY)))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, logical_port, 0, &first_phy));
