    ...
    bcm_sat_ctf_t ctf_id)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PKT_LOST_THRESHOLD, INST_SINGLE, 0x1FFFFFFF);
    ...
    bcm_sat_ctf_t * ctf_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_create_verify(unit, flags, ctf_id));
    ...
    if (dnx_data_sat.general.accum_count_not_collect_during_unavaliable_status_get(unit))
    {
        SHR_IF_ERR_EXIT(dnx_sat_ctf_flow_init(unit, *ctf_id));
    }
exit:
    ...
    bcm_sat_ctf_t ctf_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_destroy_verify(unit, ctf_id));
    ...
    int *max_value)
{
    ...
    switch (type)
    {
        ...
        case dnxSATNumSlotForStateToSwitch:
            SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                            (unit, DBAL_TABLE_SAT_RX_STATE_SET, DBAL_FIELD_WINDOW_SIZE, FALSE, 0, 0, &nof_bits));
            break;
        case dnxSATThersholdPerSlot:
            SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                            (unit, DBAL_TABLE_SAT_RX_STATE_SET, DBAL_FIELD_THRESHOLD, FALSE, 0, 0, &nof_bits));
            break;
...
{
    uint32 entry_handle_id;
    uint32 update_cntr_always;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SAT_RX_STATE_SW, &entry_handle_id));
    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_RX_FLOW_ID, ctf_id);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_UPDATE_CNTR_ALWS, INST_SINGLE, &update_cntr_always);

    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_SAT_RX_STATE_SET, entry_handle_id));

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RX_FLOW_ID, INST_SINGLE, ctf_id);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_WINDOW_SIZE, INST_SINGLE,
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_UPDATE_CNTR_ALWS, INST_SINGLE, update_cntr_always);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RST_STATE_MCHN, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CLEAR_ON_READ, INST_SINGLE, 0);

    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_SAT_RX_STATE_SW, entry_handle_id));
    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_RX_FLOW_ID, ctf_id);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_UPDATE_CNTR_ALWS, INST_SINGLE, update_cntr_always);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_WINDOW_SIZE, INST_SINGLE,
                                 config->switch_state_num_of_slots);
    ...
    bcm_sat_ctf_availability_config_t * config)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_availability_config_set_verify(unit, ctf_id, config));
    ...
    int *bins_count)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_bin_limit_get_verify(unit, max_bins_count, bins, bins_count));
    ...
    bcm_sat_ctf_bin_limit_t * bins)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_bin_limit_set_verify(unit, bins_count, bins));
...
shr_error_e
dnx_sat_dynamic_memory_access_get(
    int unit,
    uint32 *enable)
{
    uint32 entry_handle_id;
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_DYNAMIC_MEMORY_ACCESS_ENABLE, INST_SINGLE, enable);
    ...
    bcm_sat_ctf_packet_info_t * packet_info)
{
    ...
    if (dnx_data_sat.rx_flow_param.feature_get(unit, dnx_data_sat_rx_flow_param_twamp_mode))
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_TWAMP_MODE, INST_SINGLE, &twamp_mode);
    ...
    if (dnx_data_sat.rx_flow_param.feature_get(unit, dnx_data_sat_rx_flow_param_odd_even_id)
        && twamp_mode
        && dnx_data_sat.rx_flow_param.feature_get(unit, dnx_data_sat_rx_flow_param_twamp_rx_time_stamp)
        && (table_id == DBAL_TABLE_SAT_RX_PKT_PARAM_EVEN_CONFIG))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RX_TIME_STAMP_OFFSET, INST_SINGLE,
                                     packet_info->offsets.timestamp_offset - 12);
    }
    ...
    bcm_sat_ctf_packet_info_t * packet_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_packet_config_verify(unit, ctf_id, packet_info));
    ...
    bcm_sat_ctf_t ctf_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_identifier_map_verify(unit, identifier, ctf_id));
    LOG_VERBOSE(BSL_LS_BCM_SAT,
    ...
    if (identifier->trap_id < DNX_DATA_MAX_SAT_COLLECTOR_TRAP_ID_ENTRIES)
    {
        LOG_WARN(BSL_LS_BCM_SAT,
                 ("bcm_sat_ctf_identifier_t.trap_id has been changed to carry trap code instead of trap index in JR2\n"));
    }

    SHR_IF_ERR_EXIT(dnx_sat_trap_idx_get(unit, identifier->trap_id, &trap_idx));

    sal_memset(&dnx_identifier, 0, sizeof(dnx_identifier));
    dnx_identifier.session_id = identifier->session_id;
    dnx_identifier.trap_id = trap_idx;
    ...
    bcm_sat_ctf_identifier_t * identifier)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_identifier_unmap_verify(unit, identifier));
    ...
    bcm_sat_ctf_report_config_t * reports)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_reports_config_set_verify(unit, ctf_id, reports));
    LOG_VERBOSE(BSL_LS_BCM_SAT,
    ...
    bcm_sat_ctf_stat_config_t * stat)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FDV_BIN_COUNT_INTER_PKT_DELAY, INST_SINGLE,
                                 stat->count_packet_delay_delta);
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_THRESHOLD, INST_SINGLE, &lost_threshold);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_THRESHOLD, INST_SINGLE, lost_threshold);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RST_STATE_MCHN, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CLEAR_ON_READ, INST_SINGLE, 0);
    ...
    bcm_sat_ctf_stat_config_t * stat)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_stat_config_set_verify(unit, ctf_id, stat));
    ...
    dnx_sat_ctf_stat_cfg.count_packet_delay_delta = (stat->count_packet_delay_delta ? 1 : 0);
    ...
    bcm_sat_ctf_stat_t * stat)
{
    ...
    uint32 field_array_lost_pkt[2] = { 0 };
    dbal_tables_e table_id = DBAL_TABLE_EMPTY;
    dbal_fields_e key = DBAL_FIELD_EMPTY;
    uint8 key_val = 0;
    uint32 window_size = 0;
    uint32 update_cntr_always = 0;
    uint32 lost_threshold = 0;
    ...
    dbal_value_field_arr32_request(unit, entry_handle_id, DBAL_FIELD_LOST_PKT_CNTR_ACCUM, INST_SINGLE,
                                   field_array_lost_pkt);
    ...
    COMPILER_64_SET(stat->lost_packet_cnt, field_array_lost_pkt[1], field_array_lost_pkt[0]);
    ...
    if ((flags & BCM_SAT_CTF_STAT_DO_NOT_CLR_ON_READ) == 0)
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_WINDOW_SIZE, INST_SINGLE, &window_size);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_THRESHOLD, INST_SINGLE, &lost_threshold);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_UPDATE_CNTR_ALWS, INST_SINGLE,
                                   &update_cntr_always);
    ...
    bcm_sat_ctf_stat_t * stat)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_stat_get_verify(unit, ctf_id, stat));
    ...
    uint32 trap_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_trap_add_verify(unit, trap_id));
    ...
    uint32 session_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_trap_data_to_session_map_verify(unit, trap_data, trap_data_mask, session_id));
    LOG_VERBOSE(BSL_LS_BCM_SAT,
    ...
    uint32 trap_data_mask)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_trap_data_to_session_unmap_verify(unit, trap_data, trap_data_mask));
    ...
    uint32 trap_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_trap_remove_verify(unit, trap_id));
    ...
    void *user_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_ctf_traverse_verify(unit, user_data));
