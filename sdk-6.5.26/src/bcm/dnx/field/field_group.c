    ...
    dnx_field_group_info_t * fg_info_in_p)
{
    ...
    if (fg_info_in_p->fg_type == DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB &&
        (((fg_info_in_p->field_stage == DNX_FIELD_STAGE_IPMF1 ||
           fg_info_in_p->field_stage == DNX_FIELD_STAGE_IPMF2) &&
          dnx_data_field.mdb_dt.pmf_map_stage_get(unit) != DNX_FIELD_STAGE_IPMF1) ||
         (fg_info_in_p->field_stage == DNX_FIELD_STAGE_IPMF3 &&
          dnx_data_field.mdb_dt.pmf_map_stage_get(unit) != DNX_FIELD_STAGE_IPMF3)))
    {
    ...
    uint32 *tcam_handler_id_p)
{
    ...
    /*
     * Since at creation time the FG is not attached to any
     * context, there are no restricted tcam handlers to supply
     * to tcam_handler_create() function.
     */
