...
/* Initiator traverse struct, holds both the user data and the legacy api user defined cb */
typedef struct
{
    void *user_data;
    bcm_tunnel_initiator_traverse_cb tunnel_cb;
} dnx_tunnel_initiator_port_flow_user_data_t;
    ...
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    {
        bcm_flow_initiator_info_t flow_init_info_get;
        bcm_flow_handle_info_t flow_handle_info_get;
        bcm_flow_special_fields_t special_fields_get;
        char *flow_app = NULL;

        bcm_flow_initiator_info_t_init(&flow_init_info_get);
        bcm_flow_handle_info_t_init(&flow_handle_info_get);
        bcm_flow_special_fields_t_init(&special_fields_get);

        /** Get old entry values */
        dnx_tunnel_initiator_flow_app_get(unit, tunnel, &flow_app);
        SHR_IF_ERR_EXIT(bcm_dnx_flow_handle_get(unit, flow_app, &(flow_handle_info_get.flow_handle)));
        flow_handle_info_get.flow_id = flow_id;
        SHR_IF_ERR_EXIT(bcm_dnx_flow_initiator_info_get(unit, &flow_handle_info_get, &flow_init_info_get,
                                                        &special_fields_get));
        /** set SVTAG enable field */
        if ((_SHR_IS_FLAG_SET(flow_init_info_get.valid_elements_set, BCM_FLOW_INITIATOR_ELEMENT_SVTAG_ENABLE_VALID))
            && (!_SHR_IS_FLAG_SET(tunnel->flags, BCM_TUNNEL_INIT_SVTAG_ENABLE)))
        {
            flow_tunnel_init->valid_elements_clear |= BCM_FLOW_INITIATOR_ELEMENT_SVTAG_ENABLE_VALID;
        }
        else if ((!_SHR_IS_FLAG_SET
                  (flow_init_info_get.valid_elements_set, BCM_FLOW_INITIATOR_ELEMENT_SVTAG_ENABLE_VALID))
                 && (_SHR_IS_FLAG_SET(tunnel->flags, BCM_TUNNEL_INIT_SVTAG_ENABLE)))
        {
            flow_tunnel_init->valid_elements_set |= BCM_FLOW_INITIATOR_ELEMENT_SVTAG_ENABLE_VALID;
        }
    }
    else        /* Create */
    {
    ...
    bcm_gport_t * flow_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, tunnel_id, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR, flow_id));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_initiator_create_flow_common_fields_set
                    (unit, flow_handle_info.flow_id, &flow_tunnel_init, tunnel));
    ...
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    if (flow_tunnel_init->valid_elements_set & BCM_FLOW_INITIATOR_ELEMENT_SVTAG_ENABLE_VALID)
    {
...
static shr_error_e
dnx_tunnel_initiator_get_ipv4_flow_special_fields(
    int unit,
    bcm_flow_handle_t flow_handle,
    bcm_flow_special_fields_t * special_fields,
    bcm_tunnel_initiator_t * tunnel)
...
static shr_error_e
dnx_tunnel_initiator_get_ipv6_flow_special_fields(
    int unit,
    bcm_flow_handle_t flow_handle,
    bcm_flow_special_fields_t * special_fields,
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    sal_memcpy(tunnel->dip6, special_field_data.shr_var_uint8_arr, sizeof(bcm_ip6_t));

    /** get sip */
    sal_memset(&special_field_data, 0, sizeof(bcm_flow_special_field_t));
    SHR_IF_ERR_EXIT(dnx_flow_special_field_data_get(unit, special_fields, FLOW_S_F_IPV6_SIP, &special_field_data));
    sal_memcpy(tunnel->sip6, special_field_data.shr_var_uint8_arr, sizeof(bcm_ip6_t));
...
static shr_error_e
dnx_tunnel_initiator_get_ipvx_flow_special_fields(
    int unit,
    bcm_flow_handle_t flow_handle,
    bcm_flow_initiator_info_t * flow_tunnel_init,
    bcm_flow_special_fields_t * special_fields,
...
{
    bcm_flow_special_field_t special_field_data;
    const dnx_flow_app_config_t *flow_app_info = dnx_flow_app_info_get(unit, flow_handle);

    SHR_FUNC_INIT_VARS(unit);

    if (!sal_strncasecmp(flow_app_info->app_name, FLOW_APP_NAME_IPV4_INITIATOR, FLOW_STR_MAX))
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_initiator_get_ipv4_flow_special_fields(unit, flow_handle, special_fields, tunnel));
    }
    else if (!sal_strncasecmp(flow_app_info->app_name, FLOW_APP_NAME_IPV6_INITIATOR, FLOW_STR_MAX))
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_initiator_get_ipv6_flow_special_fields(unit, flow_handle, special_fields, tunnel));
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported application %s", flow_app_info->app_name);
    }
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_initiator_get_ipvx_flow_special_fields
    ...
    char *flow_app)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, tunnel_gport, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR,
...
/* Internal cb, translate the flow initiator properties into legacy tunnel init struct and call the user defined cb */
static shr_error_e
dnx_tunnel_initiator_traverse_flow_translate_cb(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    ...
    void *data)
{
    bcm_tunnel_initiator_t tunnel = { 0 };

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_tunnel_initiator_get_flow_common_fields(unit, initiator_info, &tunnel));
    SHR_IF_ERR_EXIT(dnx_tunnel_initiator_get_flow_special_fields
                    (unit, flow_handle_info->flow_handle, initiator_info, special_fields, &tunnel));
    BCM_GPORT_TUNNEL_ID_SET(tunnel.tunnel_id, _SHR_GPORT_FLOW_LIF_VAL_GET_ID(flow_handle_info->flow_id));

    SHR_IF_ERR_EXIT(((dnx_tunnel_initiator_port_flow_user_data_t *) data)->tunnel_cb(unit, &tunnel,
                                                                                     ((dnx_tunnel_initiator_port_flow_user_data_t *) data)->user_data));
...
static shr_error_e
dnx_tunnel_initiator_traverse_flow(
    int unit,
    bcm_tunnel_initiator_traverse_cb tunnel_cb,
    void *user_data,
    char *flow_app)
{
    bcm_flow_handle_t flow_handle = 0;
    bcm_flow_handle_info_t flow_handle_info = { 0 };
    dnx_tunnel_initiator_port_flow_user_data_t flow_user_data = { 0 };

    SHR_FUNC_INIT_VARS(unit);

    flow_user_data.tunnel_cb = tunnel_cb;
    flow_user_data.user_data = user_data;

    SHR_IF_ERR_EXIT(bcm_dnx_flow_handle_get(unit, flow_app, &flow_handle));
    flow_handle_info.flow_handle = flow_handle;

    SHR_IF_ERR_EXIT(bcm_dnx_flow_initiator_info_traverse
                    (unit, &flow_handle_info, &dnx_tunnel_initiator_traverse_flow_translate_cb, &flow_user_data));
...
shr_error_e
dnx_tunnel_initiator_ipv4_traverse_flow(
    int unit,
    ...
    void *user_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_initiator_traverse_flow(unit, cb, user_data, FLOW_APP_NAME_IPV4_INITIATOR));
...
shr_error_e
dnx_tunnel_initiator_ipv6_traverse_flow(
    int unit,
    bcm_tunnel_initiator_traverse_cb cb,
    void *user_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_initiator_traverse_flow(unit, cb, user_data, FLOW_APP_NAME_IPV6_INITIATOR));
