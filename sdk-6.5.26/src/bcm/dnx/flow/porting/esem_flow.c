...
/*
 *  Return if a given ESEM Namesapce-vsi entry exist.
 * 
 ...
 */
static shr_error_e
dnx_esem_initiator_namespace_vsi_exist(
    int unit,
    bcm_vlan_port_t * vlan_port,
    int *is_exist)
{
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    /*
     * BCM_VLAN_PORT_MATCH_NAMESPACE_VSI:
        ...
        (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_REPLACE)))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_esem_initiator_namespace_vsi_exist(unit, vlan_port, &entry_exist));
    ...
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_REPLACE))
    {
        SHR_IF_ERR_EXIT(dnx_esem_flow_match_info_add(unit, vlan_port, created_by_ac));
    }
