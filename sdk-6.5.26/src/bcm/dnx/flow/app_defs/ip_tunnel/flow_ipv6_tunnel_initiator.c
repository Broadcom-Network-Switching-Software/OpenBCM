    ...
    uint32 *dip6_data_entry_outlif)
{
    ...
    /** set outlif phase */
    lif_table_mngr_outlif_info.logical_phase = logical_phase;
    ...
    uint8 is_replace)
{
    ...
    SHR_IF_ERR_EXIT(flow_ipv6_tunnel_initiator_dip6_hw_update
                    (unit, dip6, logical_phase, arp_outlif, is_replace, &next_outlif));
...
flow_ipv6_tunnel_initiator_l3_intf_id_set(
    ...
    bcm_flow_initiator_info_t * initiator_info)
{
    bcm_gport_t tunnel_gport = BCM_GPORT_INVALID;
    ...
    if (arp_outlif)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif(unit, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS |
                                                    ...
                                                    arp_outlif, &tunnel_gport));
        if (tunnel_gport != BCM_GPORT_INVALID)
        {
            /** Need to understand why it's a tunnel gport */
            bcm_gport_t flow_gport = BCM_GPORT_INVALID;
            BCM_GPORT_FLOW_LIF_ID_SET(flow_gport, _SHR_GPORT_TUNNEL_ID_GET(tunnel_gport));
            BCM_GPORT_FLOW_LIF_TO_L3_ITF_LIF(initiator_info->l3_intf_id, flow_gport);
        }
    }
    else
    {
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_lib_encap_access_to_logical_phase_convert(unit, ipv6_dip_encap_phase, &logical_phase));

    /** set the l3_intf and dip6 */
    SHR_IF_ERR_EXIT(flow_ipv6_tunnel_initiator_ipv6_l3_intf_and_dip6_update
                    (unit, entry_handle_id, initiator_info, logical_phase, special_field_data.shr_var_uint8_arr,
                     next_outlif, FALSE));
    ...
    uint32 *arp_outlif)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_get_outlif_info(unit, data_entry_outlif, entry_handle_id, &outlif_info));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** get the l3_intf */
    SHR_IF_ERR_EXIT(flow_ipv6_tunnel_initiator_l3_intf_id_set(unit, arp_outlif, initiator_info));
    ...
    bcm_flow_special_fields_t * prev_special_fields)
{
    ...
    /** set the l3_intf and dip6 */
    SHR_IF_ERR_EXIT(flow_ipv6_tunnel_initiator_ipv6_l3_intf_and_dip6_update
                    (unit, entry_handle_id, initiator_info, gport_hw_resources->logical_phase + 1,
                     dip6_data.shr_var_uint8_arr, next_outlif, TRUE));
    ...
    uint32 ipv6_entry_handle_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, ipv6_entry_handle_id, DBAL_FIELD_NEXT_OUTLIF_POINTER, INST_SINGLE, &dip6_data_entry_outlif));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_esem_command_set(unit, entry_handle_id, tunnel_type, special_fields));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(flow_ipv6_tunnel_initiator_ipv6_l3_intf_and_dip6_replace
                    (unit, gport_hw_resources, current_entry_handle_id, next_outlif, initiator_info,
                     &prev_initiator_info, special_fields, &prev_special_fields));
    /** setting the esem command */
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_esem_command_set
                    (unit, current_entry_handle_id, tunnel_type, special_fields));
    ...
    uint32 *selectable_result_types)
{
        ...
        && (flow_cmd_control->flow_cb_type == FLOW_CB_TYPE_SET))
    {
        SHR_IF_ERR_EXIT(flow_ipv6_tunnel_initiator_replace
                        (unit, gport_hw_resources, current_entry_handle_id, prev_entry_handle_id,
    ...
    uint32 get_entry_handle)
{
    ...
    /** Call common verifier */
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_special_fields_verify
                    (unit, flow_handle_info, initiator_info, tunnel_type, special_fields));
...
int
flow_ipv6_tunnel_initiator_result_type_cb(
    int unit,
    uint32 entry_handle_id,
    ...
    uint32 *selectable_result_types)
{
    ...
    /*
     * skip ETPS_IPV6_TUNNEL_DOH result type 
     */
    SHR_BITSET(selectable_result_types, DBAL_RESULT_TYPE_EEDB_IPV6_TUNNEL_ETPS_IPV6_TUNNEL_GRE);
    SHR_BITSET(selectable_result_types, DBAL_RESULT_TYPE_EEDB_IPV6_TUNNEL_ETPS_IPV6_TUNNEL_RAW);
    SHR_BITSET(selectable_result_types, DBAL_RESULT_TYPE_EEDB_IPV6_TUNNEL_ETPS_IPV6_TUNNEL_UDP);
    SHR_BITSET(selectable_result_types, DBAL_RESULT_TYPE_EEDB_IPV6_TUNNEL_ETPS_IPV6_TUNNEL_VXLAN);
