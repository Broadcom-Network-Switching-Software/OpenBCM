    ...
    uint8 is_replace)
{
    ...
    if ((rv == _SHR_E_NONE) && !DNX_TUNNEL_TYPE_IS_UDP(tunnel_type))
    {
    ...
    if ((!is_replace) && (rv == _SHR_E_NOT_FOUND) && DNX_TUNNEL_TYPE_IS_UDP(tunnel_type))
    {
    ...
    uint8 enabler_set)
{
    ...
    if (!DNX_TUNNEL_TYPE_IS_GRE(tunnel_type) && enabler_set)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
...
/** common verifier, called in case special fields are configured */
shr_error_e
flow_ipvx_tunnel_initiator_special_fields_verify(
    int unit,
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    initiator_info = (bcm_flow_initiator_info_t *) flow_info;
    ...
    /** Verify for enabling SVTAG */
    valid_elements_set = initiator_info->valid_elements_set;
    if (valid_elements_set & BCM_FLOW_INITIATOR_ELEMENT_SVTAG_ENABLE_VALID)
    {
        udp_ports_t udp_ports;
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_initiator_create_verify_svtag
                        (unit, tunnel_type, udp_ports.dst_port, udp_ports.src_port));
    }
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (DNX_TUNNEL_TYPE_IS_UDP(tunnel_type))
    {
        SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_special_fields_udp_ports_get(unit, special_fields, &udp_ports));
    ...
    if (DNX_TUNNEL_TYPE_IS_GRE(tunnel_type))
    {
    ...
    udp_ports_t * new_udp_ports)
{
    ...
    {
        sal_memcpy(new_udp_ports, &prev_udp_ports, sizeof(udp_ports_t));
    }
    else
    {
        /*
