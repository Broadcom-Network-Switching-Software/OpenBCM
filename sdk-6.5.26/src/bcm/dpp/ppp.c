...
int ppp_terminator_lif_set_sw_state(int unit, int local_in_lif, bcm_ppp_terminator_t *info)
{
    ...
    if(info->type == bcmPPPTypePPPoE && gport_sw_resources.in_lif_sw_resources.lif_type == _bcmDppLifTypeVlan) {
        ...
        /* Clear pppoe terminator information in SW DB */
        if (info->ppp_terminator_id == 0 && 
            info->session_id == 0 &&
            info->network_domain == 0) {
            /* Restore ac_gport in SW DB */
            gport_sw_resources.in_lif_sw_resources.gport_id = gport_sw_resources.in_lif_sw_resources.peer_gport;
            gport_sw_resources.in_lif_sw_resources.match_tunnel = 0;
            gport_sw_resources.in_lif_sw_resources.peer_gport = 0;
        } else {
            gport_sw_resources.in_lif_sw_resources.gport_id = info->ppp_terminator_id; /* tunnel gport */
            gport_sw_resources.in_lif_sw_resources.match_tunnel = info->session_id;
...
int ppp_terminator_lif_get_sw_state(int unit, int local_in_lif, bcm_ppp_terminator_t *info)
{
    ...
    if(gport_sw_resources.in_lif_sw_resources.lif_type == _bcmDppLifTypeVlan && gport_sw_resources.in_lif_sw_resources.peer_gport != 0) {
        ...
        info->session_id = gport_sw_resources.in_lif_sw_resources.match_tunnel;
    ...
    bcm_ppp_terminator_t *info)
{
    ...
    if(info->type == bcmPPPTypePPPoE) {
        ...
        /* verify that match does not exist in SEM table */
        BCMDNX_IF_ERR_EXIT(l2tp_terminator_sem_get(unit, l2tp_session, l2tp_tunnel, vrf, &sem_result, &found));
        if(found)
        ...
        BCMDNX_IF_ERR_EXIT(arad_pp_lif_index_to_sem_result(unit, local_in_lif, 0, (uint32 *)&sem_result));
        
        /* add SEM match */
        BCMDNX_IF_ERR_EXIT(l2tp_terminator_sem_add(unit, l2tp_session, l2tp_tunnel, vrf, sem_result));
