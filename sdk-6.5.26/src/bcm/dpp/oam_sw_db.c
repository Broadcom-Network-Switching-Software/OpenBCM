...
_bcm_dpp_oam_ma_db_delete_ma_cb(int unit, sw_state_htb_key_t key, sw_state_htb_data_t data)
{
    ...
    if ((SOC_IS_QAX(unit) || (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "maid48_enabled", 0))) &&
        (ma_name.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_48_BYTE || ma_name.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE)) {
        int is_last, rv, index,i;
        int nof_extra_data_entries;
        if (ma_name.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE) {
            nof_extra_data_entries = 1;
        } else {
            nof_extra_data_entries = 3;
        }
        if (SOC_IS_QAX(unit)) {
            rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_oam_bfd_mep_db_ext_data_delete,(unit, ma_name.index));
            BCMDNX_IF_ERR_EXIT(rv);
            /* Free the extended entries */
            for (i = 0; i < nof_extra_data_entries; i++, ma_name.index += (SOC_IS_QUX(unit) ? 0x100 : 0x400)) {
