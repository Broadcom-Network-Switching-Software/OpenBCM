...
STATIC soc_reg_t  bs_pll_regs[2] = {
                            ...
                            uint32 divisor)
{
    ...
    switch (event_misc) {
        ...
        case bcmGdpllInputEventGPIO5:
            ...
            READ_NS_REGr(unit, NS_TIMESYNC_GPIO_0_CTRLr, 0, 24*idx, &regVal);
            soc_reg_field_set(unit,  NS_TIMESYNC_GPIO_0_CTRLr, &regVal,
                              DIVISORf, divisor);
            WRITE_NS_REGr(unit, NS_TIMESYNC_GPIO_0_CTRLr, 0, 24*idx, regVal);
            break;

        case bcmGdpllInputEventBS0PLL:
        case bcmGdpllInputEventBS1PLL:
            idx = event_misc - bcmGdpllInputEventBS0PLL;
            READ_NS_REGr(unit, bs_pll_regs[idx], 0, 0, &regVal);
            soc_reg_field_set(unit, bs_pll_regs[idx], &regVal,
                              DIVISORf, divisor);
            WRITE_NS_REGr(unit, bs_pll_regs[idx], 0, 0, regVal);
            break;
...
#if 0
STATIC int
_gdpll_event_divisor_misc_get(int unit, bcm_gdpll_input_event_t event_misc,
                            uint32 *pDivisor)
{
    int rv = BCM_E_NONE;
...
#if 0
STATIC int
_gdpll_event_divisor_port_get(int unit, bcm_port_t port,
...
#if 0
STATIC int
_gdpll_event_dest_misc_get (int unit, bcm_gdpll_input_event_t  event_misc,
                             gdpll_event_dest_cfg_t *pEventDest)
{
    int rv  = BCM_E_NONE;
    return (rv);
...
#if 0
STATIC int
_gdpll_event_dest_port_get(int unit, bcm_port_t port, bcm_gdpll_port_event_t port_event_type,
                             gdpll_event_dest_cfg_t  *pEventDest)
{
    int rv = BCM_E_NONE;
    return (rv);
                               ...
                               int enable)
{
    ...
    switch (event_misc){
        ...
        case bcmGdpllInputEventBS1ConvTC:
            ...
            READ_NS_REGr(unit, NS_MISC_CLK_EVENT_CTRLr, 0, 0, &regVal);
            ...
            soc_reg_field_set(unit, NS_MISC_CLK_EVENT_CTRLr, &regVal,
                              ENABLEf, reg_enable);
            WRITE_NS_REGr(unit, NS_MISC_CLK_EVENT_CTRLr, 0, 0, regVal);
            break;
...
#if 0
STATIC int
_gdpll_event_enable_misc_get(int unit, bcm_gdpll_input_event_t  event_misc,
                              int *pEnable)
{
    int rv = BCM_E_NONE;
    return (rv);
...
#if 0
STATIC int
_gdpll_event_enable_port_get(int unit, bcm_port_t port, bcm_gdpll_port_event_t port_event_type,
                              int *pEnable)
{
    int rv = BCM_E_NONE;
...
#if 0
STATIC int
_gdpll_event_roe_52b_get(int unit, bcm_port_t port, int *pEnable)
{
    int rv = BCM_E_NONE;
...
_gdpll_chan_enable_set(int unit, int chan, int enable)
{
   /* NS_GDPLL_IA_UPDATE_CONFIG */
    ...
    if (enable) {
        ...
        WRITE_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE0r, 0, 4*chan/32, 1<<(chan%32));
        WRITE_NS_REGr(unit, NS_GDPLL_IA_IN_STATUS0r, 0, 4*chan/32, 1<<(chan%32));
        /* Enable the intr for the gdpll channel */
        WRITE_NS_REGr(unit, NS_GDPLL_IA_IN_ENABLE0r, 0, 4*chan/32, (1<<(chan%32)));
    }
...
#if 0
STATIC int
_gdpll_chan_enable_get(int unit, int chan, int *pEnable)
{
    int rv = BCM_E_NONE;
...
#if 0
STATIC int
_gdpll_capture_enable_m7_get(int unit, int *pEnable)
{
    int rv = BCM_E_NONE;
...
#if 0
STATIC int
_gdpll_capture_enable_cpu_get(int unit, int *pEnable)
{
    int rv = BCM_E_NONE;
...
#if 0
STATIC int
_gdpll_chan_out_txpi_get(int unit, int chan, bcm_port_t *pPort)
{
    int rv = BCM_E_NONE;
                        ...
                        bcm_gdpll_output_event_t event)
{
    ...
    if ((event == bcmGdpllOutputEventBCN) || (event == bcmGdpllOutputEventNTPTOD) ||
        ...
        READ_NS_REGr(unit, NS_GDPLL_NCO_OUT_ENABLE0r, 0, 4*chan/32, &enableReg);
        enableReg |= 1<<(chan%32);
        soc_reg_field_set(unit, NS_GDPLL_NCO_OUT_ENABLE0r, &enableReg, INTENf, enableReg);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_ENABLE0r, 0, 4*chan/32, enableReg);

        if (chan < BCM_GDPLL_NUM_CHANNELS) {
            /* Enable in the NS interrupt line */
            sal_usleep(100);
            READ_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, &enableReg);
            enableReg |= INTR_GDPLL_OA_0_STATUS | INTR_GDPLL_OA_1_STATUS |
                         INTR_GDPLL_OA_2_STATUS | INTR_GDPLL_OA_3_STATUS;
            WRITE_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, enableReg);
        }
...
#if 0
STATIC int
_gdpll_chan_out_misc_get(int unit, int chan,
...
STATIC int
_gdpll_chan_out_enable_get(int unit, int chan,
                               int *pEnable)
{
    int rv = BCM_E_NONE;
...
#if 0
STATIC int
_gdpll_debug_enable_set(int unit, int enable)
{
...
gdpll_debug_buffer_send_thread(void *unit_vp)
{
    ...
    while(1) {
     /* Wait on the signal from the ISR */
        ...
        READ_NS_REGr(unit, NS_GDPLL_DEBUG_M7DG_RSTATEr, 0, 0, &regVal);
...
gdpll_intr(int unit, uint32 int_status, uint32 int_enable)
{
    ...
    if (soc_reg_field_get(unit, NS_TS_INT_STATUSr, int_status, M7_CORE_INT_STATUSf)) {
         ...
         */
        READ_NS_REGr(unit, NS_GDPLL_M7_CORE_INT_STATUSr, 0, 0, &regVal);
        READ_NS_REGr(unit, NS_GDPLL_M7_CORE_INT_ENABLEr, 0, 0, &intEnable);
        intEnable &= regVal;
        ...
        if (soc_reg_field_get(unit, NS_GDPLL_M7_CORE_INT_STATUSr, intEnable, ERRORf)) {
            ...
            READ_NS_REGr(unit, NS_GDPLL_M7_ERROR_INFOr, 0, 0, &regVal);
            WRITE_NS_REGr(unit, NS_GDPLL_M7_ERROR_INFO_INTr, 0, 0, 0);
        ...
        if (soc_reg_field_get(unit, NS_GDPLL_M7_CORE_INT_STATUSr, intEnable, LOCKUPf)) {
            ...
            WRITE_NS_REGr(unit, NS_GDPLL_M7_CORE_INT_STATUSr, 0, 0, 0x1);
        }
        ...
        if (soc_reg_field_get(unit, NS_GDPLL_M7_CORE_INT_STATUSr, intEnable, SYSRESET_REQf)) {
            ...
            WRITE_NS_REGr(unit, NS_GDPLL_M7_CORE_INT_STATUSr, 0, 0, 0x2);
        }
        ...
        if (soc_reg_field_get(unit, NS_GDPLL_M7_CORE_INT_STATUSr, intEnable, TXEVf)) {
            ...
            WRITE_NS_REGr(unit, NS_GDPLL_M7_CORE_INT_STATUSr, 0, 0, 0x4);
        }
    ...
    WRITE_NS_REGr(unit, NS_TS_INT_STATUSr, 0, 0, int_status_clear);
    ...
    while (count < (BCM_GDPLL_NUM_CHANNELS/32)) {
        if (soc_reg_field_get(unit, NS_TS_INT_STATUSr, int_status, gdpll_ia_status[count])) {
            READ_NS_REGr(unit, NS_GDPLL_IA_IN_STATUS0r, 0, 4*count, &stat);
            READ_NS_REGr(unit, NS_GDPLL_IA_IN_ENABLE0r, 0, 4*count, &in_enable);
            ...
            while (chan_idx < 32) {
                if ( pGdpllContext->dpll_chan[chan_num+chan_idx].enable &&
                    ...
                    WRITE_NS_REGr(unit, NS_GDPLL_IA_IN_ENABLE0r, 0, 4*count, in_enable);
            ...
            WRITE_NS_REGr(unit, NS_GDPLL_IA_IN_STATUS0r, 0, 4*count, stat);
        }
        count++;
    }
    ...
    if (soc_reg_field_get(unit, NS_TS_INT_STATUSr, int_status, GDPLL_NCO_OVERLOAD_0_STATUSf) ||
        ...
        READ_NS_REGr(unit, NS_GDPLL_NCO_OVERLOAD_STATUS0r, 0, 0, &stat0);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OVERLOAD_STATUS0r, 0, 0, stat0);

        READ_NS_REGr(unit, NS_GDPLL_NCO_OVERLOAD_STATUS1r, 0, 0, &stat1);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OVERLOAD_STATUS1r, 0, 0, stat1);

        READ_NS_REGr(unit, NS_GDPLL_NCO_OVERLOAD_STATUS2r, 0, 0, &stat2);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OVERLOAD_STATUS2r, 0, 0, stat2);

        READ_NS_REGr(unit, NS_GDPLL_NCO_OVERLOAD_STATUS3r, 0, 0, &stat3);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OVERLOAD_STATUS3r, 0, 0, stat3);
    ...
    if (soc_reg_field_get(unit, NS_TS_INT_STATUSr, int_status, TS_CAPTURE_OVRDf)) {
        READ_NS_REGr(unit, NS_RX_CDR_EVENT_OVRD_STATUS0r, 0, 0, &stat0);
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                      "soc_nanosync_intr: NS_RX_CDR_EVENT_OVRD_STATUS0:0x%x\n"), stat0));

        READ_NS_REGr(unit, NS_RX_CDR_EVENT_OVRD_STATUS1r, 0, 0, &stat0);
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                      "soc_nanosync_intr: NS_RX_CDR_EVENT_OVRD_STATUS10:0x%x\n"), stat0));

        READ_NS_REGr(unit, NS_TX_PI_EVENT_OVRD_STATUS0r, 0, 0, &stat0);
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                      "soc_nanosync_intr: NS_TX_PI_EVENT_OVRD_STATUS0:0x%x\n"), stat0));

        READ_NS_REGr(unit, NS_TX_PI_EVENT_OVRD_STATUS1r, 0, 0, &stat0);
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                      "soc_nanosync_intr: NS_TX_PI_EVENT_OVRD_STATUS1:0x%x\n"), stat0));

        READ_NS_REGr(unit, NS_MISC_EVENT_OVRD_STATUSr, 0, 0, &stat0);
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                           ...
                           void *d3, void *d4)
{
    ...
    READ_NS_REGr(unit, NS_GDPLL_DEBUG_M7DG_INT_STATUSr, 0, 0, &int_status);
    ...
    if (soc_reg_field_get(unit, NS_GDPLL_DEBUG_M7DG_INT_STATUSr, int_status, THRESHOLD_REACHEDf)) {
        ...
        WRITE_NS_REGr(unit, NS_GDPLL_DEBUG_M7DG_RSTATEr, 0, 0, regVal);
    ...
    WRITE_NS_REGr(unit, NS_GDPLL_DEBUG_M7DG_INT_STATUSr, 0, 0, int_status);

    /* Re-enable the interrupts */
    READ_NS_REGr(unit, NS_GDPLL_DEBUG_M7DG_INT_ENABLEr, 0, 0, &regVal);
    ...
    WRITE_NS_REGr(unit, NS_GDPLL_DEBUG_M7DG_INT_ENABLEr, 0, 0, int_enable);
...
_bcm_common_gdpll_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_STATUS0r, 0, 0, 0xFFFFFFFF); /* 0:31  */
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_STATUS1r, 0, 0, 0xFFFFFFFF);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_STATUS2r, 0, 0, 0xFFFFFFFF);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_STATUS3r, 0, 0, 0xFFFFFFFF);

#if 0
        /* Enable the Channel Output array status reporting */
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_ENABLE0r, 0, 0, 0xFFFFFFFF); /* 0:31  */
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_ENABLE1r, 0, 0, 0xFFFFFFFF);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_ENABLE2r, 0, 0, 0xFFFFFFFF);
        WRITE_NS_REGr(unit, NS_GDPLL_NCO_OUT_ENABLE3r, 0, 0, 0xFFFFFFFF);
#endif
        ...
        WRITE_NS_REGr(unit, NS_GDPLL_M7_CORE_INT_ENABLEr, 0, 0, 0x8/*0xF*/);
        ...
        for (count = 0; count < TS_GPIO_COUNT; count++) {
            READ_NS_REGr(unit, NS_TIMESYNC_GPIO_0_CTRLr, 0, 24*count, &regVal);
            soc_reg_field_set(unit,  NS_TIMESYNC_GPIO_0_CTRLr, &regVal,
                              CAPTURE_ENABLEf, 1);
            WRITE_NS_REGr(unit, NS_TIMESYNC_GPIO_0_CTRLr, 0, 24*count, regVal);
        ...
        if (!SOC_WARM_BOOT(unit)) {
            ...
            WRITE_NS_REGr(unit, NS_BS0_BS_TC_CTRLr, 0, 0, 0);
            WRITE_NS_REGr(unit, NS_BS1_BS_TC_CTRLr, 0, 0, 0);
        }
        ...
        WRITE_NS_REGr(unit, NS_GDPLL_DEBUG_M7DG_INT_ENABLEr, 0, 0, INTR_NS_DEBUG);
...
STATIC int
_gdpll_chan_alloc(int unit, int *pChan)
{
    int rv = BCM_E_NONE;
    uint32 idx = 0;
    gdpll_context_t *pGdpllContext = pGdpllCtx[unit];
                          ...
                          bcm_gdpll_chan_t *pGdpllChan, int *pChan)
{
    ...
    if(flags & BCM_GDPLL_CHAN_ALLOC) {
        /* Alloc new channel */
        if(_gdpll_chan_alloc(unit, &chan) != BCM_E_NONE) {
            rv = BCM_E_INTERNAL;
            goto exit;
        }
        *pChan = chan;
    ...
    if(flags & BCM_GDPLL_CHAN_OUTPUT_CONF) {
        if (pCurrentGdpllChan->out_event == bcmGdpllOutputEventTXPI) {
            ...
            bsl_printf("_gdpll_chan_out_txpi_set : nco_hwid[%d]\n", pCurrentGdpllChan->port);
...
bcm_common_gdpll_chan_state_get(int unit, int chan, uint32 *dpllState)
{
    ...
    READ_NS_REGr(unit, NS_GDPLL_IA_IN_STATUS0r, 0, 4*chan/32, &regVal);
                          ...
                          bcm_gdpll_cb cb, void *user_data)
{
    ...
    READ_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, &intEnable);
    ...
    case bcmGdpllCbChanState:
        ...
        READ_NS_REGr(unit, NS_GDPLL_M7_CORE_INT_ENABLEr, 0, 0, &regVal);
        regVal |= 0x8;
        WRITE_NS_REGr(unit, NS_GDPLL_M7_CORE_INT_ENABLEr, 0, 0, regVal);
    ...
    WRITE_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, intEnable);
    sal_mutex_give(pGdpllContext->mutex);
...
int bcm_common_gdpll_cb_unregister(int unit, bcm_gdpll_cb_reg_t cb_type)
{
    ...
    case bcmGdpllCbRsvd1:
        if (soc_feature(unit, soc_feature_rsvd1_intf)) {
            ...
            READ_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, &regVal);
            regVal &= ~INTR_RSVD1_MESSAGE_RX_STATUS;
            WRITE_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, regVal);
