...
int
_bcm_ptp_tag_send(
    int unit, uint16 *ptags, int num_tags, int uC)
{
    mos_msg_data_t start_msg;
    int timeout_usec = 1900000;
    ...
    if ((ptags == NULL) || !num_tags || num_tags > MOS_MSG_SUBCLASS_PTP_TAG_COUNT) {
        LOG_WARN(BSL_LS_BCM_PTP, (BSL_META_U(unit,
           "Error: ptp tags info. \n")
           ));
        rc = BCM_E_PARAM;
    ...
    /* Copy all the tags */
    tags = soc_cm_salloc(unit, MOS_MSG_SUBCLASS_PTP_TAG_COUNT*sizeof(uint16), "PTP tags");
    if (!tags) {
        rc = BCM_E_FAIL;
        goto exit;
    }

    for (count = 0; count < MOS_MSG_SUBCLASS_PTP_TAG_COUNT; count++) {
        if (num_tags) {
            tags[count] = bcm_htons(ptags[count]);
            num_tags--;
        } else {
            tags[count] = 0;
        }
    ...
    buf_paddr = soc_cm_l2p(unit, (void*)tags);
    start_msg.s.mclass = MOS_MSG_CLASS_1588;
    start_msg.s.subclass = MOS_MSG_SUBCLASS_PTP_TAG_CONFIG;
    start_msg.s.data = bcm_htonl(PTR_TO_INT(buf_paddr));

    rc = soc_cmic_uc_msg_send(unit, uC, &start_msg, timeout_usec);
    if (BCM_FAILURE(rc)) {
        LOG_WARN(BSL_LS_BCM_PTP, (BSL_META_U(unit,
           "Error: call soc_cmic_uc_msg_send failed for PTP_TAG_CONFIG\n")
           ));
        rc = BCM_E_FAIL;
    }
    ...
    int unit)
{
    ...
    sal_memset(stack_p, 0x00, sizeof (_bcm_ptp_stack_cache_t));
