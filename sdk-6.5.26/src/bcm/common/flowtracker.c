...
void bcm_flowtracker_tracking_param_type_info_t_init(
        bcm_flowtracker_tracking_param_type_info_t *info)
{
    sal_memset(info, 0, sizeof(bcm_flowtracker_tracking_param_type_info_t));
}

/* Initialize a flowtracker resource configuration structure. */
void bcm_flowtracker_cfg_t_init(bcm_flowtracker_cfg_t *cfg)
{
    sal_memset(cfg, 0, sizeof(bcm_flowtracker_cfg_t));
}

/* Initialize a state transition key structure. */
void bcm_flowtracker_state_transition_key_t_init(
    bcm_flowtracker_state_transition_key_t *key)
{
    sal_memset(key, 0, sizeof (bcm_flowtracker_state_transition_key_t));
    key->bidirectional_flow = -1;
    key->new_learn = -1;
}

/* Initialize a state transition policy structure. */
void bcm_flowtracker_state_transition_policy_t_init(
    bcm_flowtracker_state_transition_policy_t *policy)
{
    sal_memset(policy, 0, sizeof (bcm_flowtracker_state_transition_policy_t));
}

/* Initialize a flowtracker bloom hash input structure. */
void bcm_flowtracker_bloom_hash_input_t_init(
            bcm_flowtracker_bloom_hash_input_t *hash_input)
{
    sal_memset(hash_input, 0, sizeof(bcm_flowtracker_bloom_hash_input_t));
}

/* Initialize a flowtracker bloom hash result structure. */
void bcm_flowtracker_bloom_hash_result_t_init(
            bcm_flowtracker_bloom_hash_result_t *hash_result)
{
    sal_memset(hash_result, 0, sizeof(bcm_flowtracker_bloom_hash_result_t));
}

/* Initialize a flowtracker bloom key info structure. */
void bcm_flowtracker_bloom_key_info_t_init(
            bcm_flowtracker_bloom_key_info_t *bl_key)
{
    sal_memset(bl_key, 0, sizeof(bcm_flowtracker_bloom_key_info_t));
}
