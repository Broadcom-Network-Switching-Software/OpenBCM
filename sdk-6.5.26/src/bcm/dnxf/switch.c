    ...
    void *userdata)
{
    ...
    SHR_IF_ERR_EXIT(soc_event_register(unit, (soc_event_cb_t) cb, userdata));
    ...
    void *userdata)
{
    ...
    SHR_IF_ERR_EXIT(soc_event_unregister(unit, (soc_event_cb_t) cb, userdata));
    ...
    uint32 value)
{
    ...
    if (type.event_id != BCM_SWITCH_EVENT_CONTROL_ALL)
    {
        ...
        {
            SHR_IF_ERR_EXIT(soc_is_valid_block_instance(unit, block_types, type.index, &is_valid));
            if (!is_valid)
            {
    ...
    switch (type.action)
    {
        case bcmSwitchEventMask:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                SHR_IF_ERR_EXIT(soc_dnxc_interrupt_all_enable_set(unit, (value ? 0 : 1)));
            }
            else
            {
                /*
                ...
                if (type.index == BCM_CORE_ALL)
                {
                    for (bi_index = 0; SOC_BLOCK_INFO(unit, bi_index).type >= 0; bi_index++)
                    {
                        ...
                        SHR_IF_ERR_EXIT(soc_interrupt_is_valid(unit, bi, &(interrupts[type.event_id]), &is_valid));
                        if (is_valid)
                        {
                            port = soc_interrupt_get_intr_port_from_index(unit, type.event_id, bi->number);
                            if (value)
                            {
                                SHR_IF_ERR_EXIT(soc_interrupt_disable(unit, port, &(interrupts[type.event_id])));
                            }
                            else
                            {
                                SHR_IF_ERR_EXIT(soc_interrupt_enable(unit, port, &(interrupts[type.event_id])));
                            }
                        }
                    }
                }
                else
                {
                    if (value)
                    {
                        SHR_IF_ERR_EXIT(soc_interrupt_disable(unit, type.index, &(interrupts[type.event_id])));
                    }
                    else
                    {
                        SHR_IF_ERR_EXIT(soc_interrupt_enable(unit, type.index, &(interrupts[type.event_id])));
                    }
                }
            }
            break;
        ...
        case bcmSwitchEventForce:
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT(soc_interrupt_force(unit, type.index, &(interrupts[type.event_id]), 1 - value));
            }
            break;
        ...
        case bcmSwitchEventClear:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                if (value)
                {
                    SHR_IF_ERR_EXIT(soc_interrupt_clear_all(unit, 0));
                }
                else
                {
            ...
            else
            {
                ...
                if (value)
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT(interrupts[type.event_id].interrupt_clear(unit, type.index, type.event_id));
                    }
                }
                else
                {
        ...
        case bcmSwitchEventStormTimedCount:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                for (inter = 0; inter < nof_interrupts; inter++)
                {
                    for (bi_index = 0; SOC_BLOCK_INFO(unit, bi_index).type >= 0; bi_index++)
                    {
                        ...
                        if (is_valid)
                        {
                            SHR_IF_ERR_EXIT(soc_interrupt_storm_timed_count_set(unit, inter, value));
            ...
            else
            {
                 ...
                 * Set per interrupt
                 */
                SHR_IF_ERR_EXIT(soc_interrupt_storm_timed_count_set(unit, type.event_id, value));
        ...
        case bcmSwitchEventStormTimedPeriod:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                for (inter = 0; inter < nof_interrupts; inter++)
                {
                    for (bi_index = 0; SOC_BLOCK_INFO(unit, bi_index).type >= 0; bi_index++)
                    {
                        ...
                        if (is_valid)
                        {
                            SHR_IF_ERR_EXIT(soc_interrupt_storm_timed_period_set(unit, inter, value));
            ...
            else
            {
                 ...
                 * Set per interrupt
                 */
                SHR_IF_ERR_EXIT(soc_interrupt_storm_timed_period_set(unit, type.event_id, value));
        ...
        case bcmSwitchEventStormNominal:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                if (value > 0)
                {
                     ...
                     */
                    SHR_IF_ERR_EXIT(dnxf_state.intr.storm_nominal.set(unit, value));
                }
                else if (value == 0)
                {
        ...
        case bcmSwitchEventLog:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                ...
                for (inter = 0; inter < nof_interrupts; inter++)
                {
                    ...
                    SHR_IF_ERR_EXIT(soc_interrupt_flags_set(unit, inter, flags));
                }
            }
            else
            {
                ...
                SHR_IF_ERR_EXIT(soc_interrupt_flags_get(unit, type.event_id, &flags));
                ...
                SHR_IF_ERR_EXIT(soc_interrupt_flags_set(unit, type.event_id, flags));
            }
            break;
    ...
    uint32 *value)
{
    ...
    switch (type.action)
    {
        case bcmSwitchEventMask:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                SHR_IF_ERR_EXIT(soc_dnxc_interrupt_all_enable_get(unit, &is_enable));
                *value = (is_enable == 0);
            }
            else
            {
                 ...
                 * Get per interrupt
                 */
                SHR_IF_ERR_EXIT(soc_interrupt_is_enabled(unit, type.index, &(interrupts[type.event_id]), &is_enable));
                *value = (is_enable == 0);
            }
            break;
        ...
        case bcmSwitchEventForce:
            ...
            {
                SHR_IF_ERR_EXIT(soc_interrupt_force_get(unit, type.index, &(interrupts[type.event_id]), &is_enable));
                *value = is_enable;
            }
            break;
        ...
        case bcmSwitchEventClear:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                SHR_IF_ERR_EXIT(soc_interrupt_is_all_clear(unit, (int *) value));
            }
            else
            {
                /*
                 * Get per interrupt
                 */
                SHR_IF_ERR_EXIT(soc_interrupt_get(unit, type.index, &(interrupts[type.event_id]), &inter_get));
                *value = (inter_get == 0);
            }
            break;
        ...
        case bcmSwitchEventStormTimedPeriod:
            ...
            else
            {
                 ...
                 * Get per interrupt
                 */
                SHR_IF_ERR_EXIT(soc_interrupt_storm_timed_period_get(unit, type.event_id, value));
            }
            break;
        ...
        case bcmSwitchEventStormTimedCount:
            ...
            else
            {
                 ...
                 * Get per interrupt
                 */
                SHR_IF_ERR_EXIT(soc_interrupt_storm_timed_count_get(unit, type.event_id, value));
            }
            break;
    ...
    int *arg)
{
    ...
    switch (bcm_type)
    {
        ...
        case bcmSwitchSynchronousPortClockSourceBkupDivCtrl:
            ...
            break;
        case bcmSwitchSynchronousPortClockSourceSet1:
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_link_get, (unit, 3, arg)));
            break;
        case bcmSwitchSynchronousPortClockSourceBkupSet1:
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_link_get, (unit, 2, arg)));
            break;
        case bcmSwitchSynchronousPortClockSourceSet1DivCtrl:
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_divider_get, (unit, 3, arg)));
            break;
        case bcmSwitchSynchronousPortClockSourceBkupSet1DivCtrl:
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_divider_get, (unit, 2, arg)));
            break;
        default:
    ...
    switch (bcm_type)
    {
        ...
        case bcmSwitchControlSync:
            if (!SOC_WARM_BOOT(unit))
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(_dnxf_switch_control_sync(unit, arg));
            }
            break;
        case bcmSwitchControlAutoSync:
            SHR_SET_CURRENT_ERR(_SHR_E_UNAVAIL);
        ...
        case bcmSwitchSynchronousPortClockSourceBkupDivCtrl:   /* updated slave divider */
            ...
            break;
        case bcmSwitchSynchronousPortClockSourceSet1:  /* update master link for synce */
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_link_set, (unit, 3, arg)));
            break;
        case bcmSwitchSynchronousPortClockSourceBkupSet1:      /* update slave link for synce */
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_link_set, (unit, 2, arg)));
            break;
        case bcmSwitchSynchronousPortClockSourceSet1DivCtrl:   /* updated master divider */
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_divider_set, (unit, 3, arg)));
            break;
        case bcmSwitchSynchronousPortClockSourceBkupSet1DivCtrl:       /* updated slave divider */
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_divider_set, (unit, 2, arg)));
            break;
        default:
    ...
    int *temperature_count)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_temperature_monitors_get,
                     (unit, temperature_max, temperature_array, temperature_count)));
 ...
 */
int
bcm_dnxf_switch_hard_reset_cb_register(
    int unit,
    uint32 flags,
    ...
    void *userdata)
{
    ...
    DNXF_UNIT_LOCK_TAKE(unit);
    SHR_IF_ERR_EXIT(dnxf_state.hard_reset.callback.set(unit, callback));
    SHR_IF_ERR_EXIT(dnxf_state.hard_reset.userdata.set(unit, (uint32 *) userdata));
 ...
 */
int
bcm_dnxf_switch_hard_reset_cb_unregister(
    int unit,
    bcm_switch_hard_reset_callback_t callback,
    void *userdata)
{
    ...
    DNXF_UNIT_LOCK_TAKE(unit);
    SHR_IF_ERR_EXIT(dnxf_state.hard_reset.callback.set(unit, NULL));
    SHR_IF_ERR_EXIT(dnxf_state.hard_reset.userdata.set(unit, NULL));
