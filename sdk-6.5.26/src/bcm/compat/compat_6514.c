 ...
 */
STATIC void
_bcm_compat6514out_l2_addr_t(bcm_compat6525_l2_addr_t *from,
...
{
    int rv = BCM_E_NONE;
    bcm_compat6525_l2_addr_t *naddr = NULL;
    bcm_compat6525_l2_addr_t *ncf_array = NULL;
    int i = 0;

    if ((NULL != addr) && (NULL != cf_array) && (0 < cf_max)) {
        /* Create from heap to avoid the stack to bloat */
        naddr = (bcm_compat6525_l2_addr_t *)
            sal_alloc(sizeof(bcm_compat6525_l2_addr_t), "New naddr");
        if (NULL == naddr) {
            return BCM_E_MEMORY;
        }
