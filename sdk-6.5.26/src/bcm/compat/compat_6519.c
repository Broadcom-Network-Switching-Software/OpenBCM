 ...
 */
STATIC void
_bcm_compat6519out_flow_encap_config_t(bcm_compat6520_flow_encap_config_t *from,
...
{
    int rv;
    bcm_compat6520_flow_encap_config_t *new_info = NULL;

    if (NULL != info) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_compat6520_flow_encap_config_t *)
            sal_alloc(sizeof(bcm_compat6520_flow_encap_config_t), "New flow encap config");
        if (NULL == new_info) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_flow_encap_get(unit, new_info, num_of_fields, field);
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_flow_encap_delete(unit, new_info, num_of_fields, field);
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_flow_encap_add(unit, new_info, num_of_fields, field);
...
{
    bcm_flowtracker_group_stat_t_init(to);
...
{
    bcm_flowtracker_export_trigger_info_t_init(to);
 ...
 */
STATIC void
_bcm_compat6519out_l2_addr_t(bcm_compat6525_l2_addr_t *from,
...
bcm_compat6519_l2_addr_by_struct_get(int unit, bcm_compat6519_l2_addr_t *l2addr)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_addr_by_struct_get(unit, newl2addr);
...
bcm_compat6519_l2_addr_by_struct_delete(int unit, bcm_compat6519_l2_addr_t *l2addr)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_addr_by_struct_delete(unit, newl2addr);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6520_tunnel_terminator_t *new_terminator = NULL;

    if (terminator != NULL) {
        new_terminator = (bcm_compat6520_tunnel_terminator_t *)
                     sal_alloc(sizeof(bcm_compat6520_tunnel_terminator_t),
                     "New terminator");
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_tunnel_terminator_stat_attach(unit, new_terminator, stat_counter_id);
    ...
    bcm_compat6519_tunnel_terminator_t *terminator)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_tunnel_terminator_stat_detach(unit, new_terminator);
    ...
    uint32 *stat_counter_id)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_tunnel_terminator_stat_id_get(unit, new_terminator, stat_counter_id);
    ...
    uint32 value)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_tunnel_terminator_flexctr_object_set(unit, new_terminator, value);
    ...
    uint32 *value)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_tunnel_terminator_flexctr_object_get(unit, new_terminator, value);
...
/*
 * Function:
 *   _bcm_compat6519in_port_flexe_oh_alarm_t
 * Purpose:
 *   Convert the bcm_port_flexe_oh_alarm_t datatype from <=6.5.19 to
 *   SDK 6.5.19+.
 * Parameters:
 *   from        - (IN) The <=6.5.19 version of the datatype.
 *   to          - (OUT) The SDK 6.5.19+ version of the datatype.
...
STATIC void
_bcm_compat6519in_port_flexe_oh_alarm_t(
    bcm_compat6519_port_flexe_oh_alarm_t *from,
    bcm_port_flexe_oh_alarm_t *to)
{

    sal_memset(to, 0, sizeof(*to));

    to->alarm_active = from->alarm_active;
    to->phymap_mismatch = from->phymap_mismatch;
    SHR_BITCOPY_RANGE(to->cal_a_mismatch, 0, from->cal_a_mismatch, 0, BCM_PORT_FLEXE_MAX_NOF_SLOTS);
    SHR_BITCOPY_RANGE(to->cal_b_mismatch, 0, from->cal_b_mismatch, 0, BCM_PORT_FLEXE_MAX_NOF_SLOTS);
    to->group_deskew_alarm = from->group_deskew_alarm;
    to->group_id_mismatch = from->group_id_mismatch;
    to->phy_id_mismatch = from->phy_id_mismatch;
    to->lost_of_frame = from->lost_of_frame;
    to->lost_of_multiframe = from->lost_of_multiframe;
    to->oh_block_1_alarm = from->oh_block_1_alarm;
    to->c_bit_alarm = from->c_bit_alarm;
    to->crc_error = from->crc_error;
    to->rpf_alarm = from->rpf_alarm;
}

/*
 * Function:
 *      _bcm_compat6519out_port_flexe_oh_alarm_t
 * Purpose:
 *      Convert the bcm_port_flexe_oh_alarm_t datatype from 6.5.19+ to
 *      <=6.5.19
 * Parameters:
...
STATIC void
_bcm_compat6519out_port_flexe_oh_alarm_t(
    bcm_port_flexe_oh_alarm_t *from,
    bcm_compat6519_port_flexe_oh_alarm_t *to)
{
    to->alarm_active = from->alarm_active;
    to->phymap_mismatch = from->phymap_mismatch;
    SHR_BITCOPY_RANGE(to->cal_a_mismatch, 0, from->cal_a_mismatch, 0, BCM_PORT_FLEXE_MAX_NOF_SLOTS);
...
/* Get overhead alarms. */
int
bcm_compat6519_port_flexe_oh_alarm_get(
    int unit, 
    bcm_gport_t gport, 
    uint32 flags, 
...
{
    int rv = BCM_E_NONE;
    bcm_port_flexe_oh_alarm_t *new_alarms = NULL;

    if (alarms != NULL) {
        new_alarms = (bcm_port_flexe_oh_alarm_t *)
                      sal_alloc(sizeof(bcm_port_flexe_oh_alarm_t),
                      "New alarm");
        if (new_alarms == NULL) {
            return BCM_E_MEMORY;
        }
        /* Transform the entry from the old format to new one */
        _bcm_compat6519in_port_flexe_oh_alarm_t(alarms, new_alarms);
    } else {
        return BCM_E_PARAM;
    }

    /* Call the BCM API with new format */
    rv = bcm_port_flexe_oh_alarm_get(unit, gport, flags, new_alarms);

    if (new_alarms != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6519out_port_flexe_oh_alarm_t(new_alarms, alarms);
