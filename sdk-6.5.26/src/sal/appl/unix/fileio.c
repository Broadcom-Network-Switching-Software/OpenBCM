...
sal_homedir_set(char *dir)
{
    ...
    if (dir != NULL) {
        ...
        if ((buf = sal_alloc(5 + strlen(dir) + 1,"fileio_sal_alloc")) == NULL) {
            return -1;
        }
...
int sal_cd(char *f)
{
    ...
    if (f == NULL) {
        ...
        if (f == NULL) {
            ...
            /*
             * For Coverity: Make sure size of directory is smaller than 'max_dir_size' (hardcoded)
             */
            if (sal_strnlen(f,max_dir_size) >= max_dir_size)
            {
                rv = EIO;
                goto exit; 
            }
