 ...
 * import config file 
 * to one config file 
 * check if line contains "try_import"
 * when used try import we don't require the file to exist
 * and we indicate the import as silant for errors
 */

static int 
sal_config_line_import(char *str,char *import_fname,int *is_silant_import) {
    char  line[SAL_CONFIG_STR_MAX];
    char *sep=" \t\n\r";
    char *token;
    char *rest = NULL;

    *is_silant_import = FALSE;
    /*
    ...
    */
     ...
     */
    token=sal_strtok_r(line,sep,&rest);
    if (token==NULL) {
        return 0;
    }
     ...
     */
    if (sal_strcmp(token,"import") && sal_strcmp(token,"try_import")) {
        return 0;
    }

    if (!(sal_strcmp(token,"try_import"))) {
        *is_silant_import = TRUE;
    }

    token=sal_strtok_r(NULL,sep,&rest);
    if (token==NULL) {
        return 0;
    }
 ...
 */
int
sal_config_file_process(char *fname,int *nof_imported_config_files, char *already_imported_tables[], char *dirname, int override_duplicates, int silant_import)
{
    FILE  *fp;
    sc_t  *found_sc;
    char  *c;
    ...
    if ((fp = sal_fopen(fname_full_path, "r")) == NULL) {
        /*
         * If trying to parse a file imported with "try_import"
         * don't print error if file not existing
         */
        if (!silant_import) 
        {
    ...
    while (sal_fgets(str, sizeof(str), fp)) {
        ...
        if (sal_config_line_import(str,import_fname, &silant_import_check)) {
            ...
            else
            {
                ...
                if (sal_config_file_process(import_fname,  nof_imported_config_files,already_imported_tables,dirname, override_duplicates, silant_import_check) == -1) {
                    continue; 
                }
                continue;
            }
        ...
        *c = '\0';
        ...
        if (sal_strcasecmp("suppress_unknown_prop_warnings", 
            ...
            /*
             * Free already allocated memory in case of unknow warning.
             */
            FREE_SC(sc);
            continue;
        }
...
sal_config_refresh(void)
{
    ...
    rv =  sal_config_file_process(already_imported_tables[0],&nof_imported_config_files,already_imported_tables, dirname, 0, 0);
