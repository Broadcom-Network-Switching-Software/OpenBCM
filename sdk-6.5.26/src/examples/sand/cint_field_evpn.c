...
int field_jr2_comp_evpn_esi_db(int unit, int presel_id) {
    ...
    if(rv == BCM_E_NONE) {
        /** In J2P and above devices, sys_port locates at learn_payload0[25:5][15:0]}*/
        if (field_action_learn_sys_port[0] == bcmFieldActionCount) {
            /** Destination is 21bit = {5'b01100, sys_port(16b)}, we use [25:0] since no way to pad value to UDF action*/
            bcm_field_action_info_t_init(&action_info);
            action_info.action_type = bcmFieldActionLearnPayload1;
            action_info.prefix_size = 6;
            action_info.prefix_value = 0x30;  /** Learn_Payload_Result_Format*/
            action_info.size = 26;
            action_info.stage = bcmFieldStageIngressPMF2;
            void_ptr = &(action_info.name[0]);
        ...
        /** In other devices, sys_port locates at {learn_raw_data1[28:8][15:0]}*/
        if (field_action_learn_sys_port[0] == bcmFieldActionCount) {
            /** Destination is 21bit = {5'b01100, sys_port(16b)}, we use [28:0] since no way to pad value to UDF action*/
            bcm_field_action_info_t_init(&action_info);
            action_info.action_type = bcmFieldActionLearnRaw1;
            action_info.prefix_size = 3;
            action_info.prefix_value = 0x0; /** Learn_Payload_Result_Format[2:0]*/
            action_info.size = 29;
            action_info.stage = bcmFieldStageIngressPMF2;
            void_ptr = &(action_info.name[0]);
