...
struct cint_vxlan_basic_info_s
{
    ...
    bcm_gport_t egress_native_virtual_ac;   /* for j2 only: egress native vlan port. Represent the AC info in ESEM entry */
};
...
int vxlan_split_horizon_set(int unit, int vpn, int vdc, int vxlan_network_group_id)
{
    ...
    proc_name = "vxlan_split_horizon_set";
    ...
    printf("%s(): unit=%d vsi=%d match_class_id=%d vlan_port_id=0x%08X  \n", proc_name, unit, vpn, vdc, vlan_port.vlan_port_id);

    return rv;
}

int vxlan_split_horizon_clear(int unit)
{
    int rv = BCM_E_NONE;

    if (is_device_or_above(unit,JERICHO2)) {
        bcm_vlan_port_t vlan_port;
        rv = bcm_vlan_port_destroy(unit, cint_vxlan_basic_info.egress_native_virtual_ac);
        if (rv != BCM_E_NONE)
        {
    ...
    int tunnel_idx)
{
    ...
    if (is_device_or_above(unit, JERICHO2))
    {
        ...
        */
        ...
        if (!is_jericho2_mode) {
            rv = fec_id_alignment_field_add(unit, ip_tunnel_basic_info.access_port, BCM_L3_ITF_VAL_GET(ip_tunnel_basic_info.encap_fec_id[tunnel_idx]));
            if (rv != BCM_E_NONE)
            {
