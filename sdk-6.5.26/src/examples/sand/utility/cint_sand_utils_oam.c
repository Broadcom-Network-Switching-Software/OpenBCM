...
/** SVATG enable on vlan port */
uint8 svtag_enable = 0;
/* debug prints */
int verbose = 1;
...
int oam__device_type_get(int unit, device_type_t *device_type) {
    ...
    *device_type =
            ...
            (device_id_masked == 0x8850) ? device_type_jericho2p : (device_id_masked == 0x8280) ? device_type_q2u : (device_id_masked == 0x8290) ? device_type_q2N : -1;
 ...
 */
int
oam__vswitch_metro_run(int unit, int known_mac_lsb, int known_vlan, uint8 svtag_enable){
    int rv;
    /*bcm_vlan_t  vsi*/;
    bcm_mac_t kmac;
    int index, index2;
    ...
    if(svtag_enable)
    {
        flags |= BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE;
    }
    gport1 = 0;
...
int create_vswitch_and_mac_entries(int unit) {
    ...
    rv = oam__vswitch_metro_run(unit, 0xce, 3, svtag_enable);
    if (rv != BCM_E_NONE){
