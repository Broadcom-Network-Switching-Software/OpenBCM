...
int bridge_application_conf_vlan_created = 0;
int
    ...
    int vsi)
{
    ...
    if (bridge_application_conf_vlan_created == 0)
    {
        rv = vlan_create(unit, vid);
        if (rv != BCM_E_NONE)
        {
            printf("Error, vlan_create, vid\n");
        }
        rv = vlan_create(unit, vsi);
        if (rv != BCM_E_NONE)
        {
            printf("Error, vlan_create, vsi\n");
        }
        ...
        {
            /* Set 0x8100 as S-TPID*/
            int jr2_tag_format_s = 4;
            ...
            port_tpid_class.tag_format_class_id = jr2_tag_format_s;
        ...
        bridge_application_conf_vlan_created = 1;
    }
    ...
    int unit)
{
     ...
     */
    if (!BCM_GPORT_IS_TRUNK(bridge_application_info.in_port)) {
    ...
    if (!BCM_GPORT_IS_TRUNK(bridge_application_info.out_port)) {
