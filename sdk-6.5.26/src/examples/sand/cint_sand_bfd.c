...
/** single hop and mpls BFD co_exit*/
int single_hop_bfd_coexit_with_mpls_bfd = 0;
...
/**
*  When gach_chan_type set, 
*  to call bcm_oam_mpls_tp_channel_type_tx_set set gach channel type of tx and rx side
*  for BFD PWE/MPLS-TP
*  Note: bcmOamMplsTpChannelPweBfd and bcmOamMplsTpChannelMplsTpBfd must be the same for channel type
*/
int gach_chan_type = 0;
/**
 * Destination: FEC
...
/**
* indicate the dest ip can be configured for BFD MPLS CcCv
* in that case dst_ip_addr must be set, others follow above is_bfd_mpls_tp_cccv's declare.
*/
uint8 is_bfd_mpls_tp_cccv_with_fixed_dip=0;
...
/*
 * When rand_ip_ttl is not -1, sbfd_reflector_expected_checksum_get will use this ipv4 ttl
 * to calculate checksum as cascade data to lookup and trap packet
 */
int sbfd_rand_ip_ttl = -1;
...
int l3_interface_init(int unit, int in_sysport, int out_sysport, int * _next_hop_mac_encap_id, int * _tunnel_id, int create_mpls_tunnel, int use_my_bfd_dip_destination,int is_muti_hop_mpls){
    ...
    rv = bcm_vlan_control_vlan_get(unit, in_vlan, &vlan_control);
    ...
    rv = bcm_vlan_control_vlan_get(unit, out_vlan, &vlan_control);
    if (rv == BCM_E_NOT_FOUND) {
    ...
    if (single_hop_extended || is_muti_hop_mpls) {
        ...
        /*IOP mode, sdbfd reflector add route for 2nd pass*/
        if (is_device_or_above(unit, JERICHO2) && (system_headers_mode == 0) && is_s_bfd_reflector){
            printf("***************************jr2  iop*********************** \n");
            bfd_route.l3a_subnet = 0x7fffff00;
            ...
            bfd_route.l3a_intf = fec[0];
            rv = bcm_l3_route_add(unit, &bfd_route);
            if (rv != BCM_E_NONE) {
                printf("IOP, JR2 route add failed !!! bcm_l3_route_add \n");
            } 
        }
        else {
...
/**
 * BFD extended SIP - for accelerated BFD IPV4 endpoint take SIP 
 * from  EEDB instead of from OAMP 
 * 
 * @param unit 
 * @param sip - New source IP address
 * @param aux_data - Used for recalculating the checksum, 
 *                 required in Jer1 family devices only (ignored
 *                 in Jer2)
 * @param next_pointer - Should be the arp-pointer
 * @param encap_access - input to bcm_tunnel_initiator_create. 
 *                     In standard cases should be
 *                     bcmEncapAccessInvalid (decided
 *                     automatically), Jer2 only
 * @param output_outlif  - Output of bcm_tunnel_initiator_create
 * 
 * @return int 
 */
int bfd_extended_sip_configuration(int unit, int sip, int aux_data, bcm_if_t next_pointer,bcm_encap_access_t encap_access ,bcm_if_t * output_outlif){
    ...
    tunnel.dip = 0;
    tunnel.l3_intf_id = next_pointer;
    if (is_device_or_above(unit, JERICHO2)) {
        tunnel.encap_access = encap_access;
    }
    ...
    *output_outlif = bfd_extended_l3_intf.l3a_intf_id;
...
int bfd_ipv4_example(int unit, bcm_gport_t remote_gport, uint32 flags, uint32 flags2) {
    ...
    * configured IPv4 SIP and calculated checksum. */
    ...
    if (bfd_extended_sip && !use_vxlan_fec)
    {
        /* In the case of  vxlan fec AND extended SIP, extended SIP is done in cint_sand_ip_tunnel.c  */
        rv = bfd_extended_sip_configuration(unit,0x10203040,0x11ae, next_hop_mac, bcmEncapAccessInvalid /* encap access decided automatically*/,&next_if);
        BCM_IF_ERROR_RETURN(rv);
    }
    ...
    */
            ...
            || is_server || use_trap_with_snoop))
    {
        ...
        else
        {
            ...
            bfd_endpoint_info.remote_discr = single_hop_bfd_coexit_with_mpls_bfd ? 0x10003 : 0x10002;
        }
    ...
    if(single_hop_bfd_coexit_with_mpls_bfd) {
    ...
    if(is_device_or_above(unit, JERICHO2))
	{
         ...
         */
        if ((system_headers_mode != 0) && (full_threshold == 0) && (bfd_endpoint_id != -1))
        {
            bfd_endpoint_info.flags |= BCM_BFD_ENDPOINT_WITH_ID;
            bfd_endpoint_info.id = bfd_endpoint_id;
        }
    }
    ...
    if (is_s_bfd_reflector) {
        rv = sbfd_reflector_endpoint_create(unit, 0x30F0701, 0x10, 3, 0);
        return rv;
    }
...
int bfd_ipv4_run_with_defaults(int unit, int port1, int port2,  int remote_port,  int is_extended_sip, uint32 flags, int remote_port_fwd_strength, int flags2) {
    ...
    if(use_vxlan_fec)
    {
        /** Create VXLAN FEC */
       /**
         * BFD extended SIP + vxlan: 
         * When using extended SIP, FEC pointer should point to the bfd-extended-sip outLIF
         * which then points to the native-arp. This is done in 
         * cint_sand_ip_tunnel.c 
         * When extended SIP is used, RIF pointer may not be used due to 
         * encap access considerations. Instead the RIF will be part of 
         * the native arp (hence is_native_arp_with_rip will be set to 1 
         * as well). 
         * In this case, in this specific cint the resulting native SA
         * will be slightly different - Instead of 00:0c:00:02:00:00
         * without extended-sip, native SA will be 00:0c:00:02:00:22
         */
        ip_tunnel_basic_info.bfd_vxlan_extended_sip = bfd_extended_sip;
        ip_tunnel_basic_info.is_native_arp_with_rif = 1;
        
        rv = vxlan_roo_example(unit, 0 /* is_ipv6 */, port_2, port_1, 1 /* add_routing_table_entry */);
        BCM_IF_ERROR_RETURN(rv);
    ...
    if(is_device_or_above(unit, JERICHO2) && (flags & BCM_BFD_ECHO))
    {
        ...
        rv = cint_field_bfd_echo_entry_add(unit, 0x7fffff03, NULL, bfd_local_discr & 0xFFFF, ep_id);
        if (rv != BCM_E_NONE) {
...
int bfd_pwe_example(int unit , int vccv_type) {
    ...
    */
    ...
    if(gach_chan_type && (is_device_or_above(unit, JERICHO2)))
    {
        /** For channel type : bcmOamMplsTpChannelPweBfd and bcmOamMplsTpChannelMplsTpBfd must be the same*/
        rv=bcm_oam_mpls_tp_channel_type_tx_set(unit, bcmOamMplsTpChannelPweBfd, gach_chan_type);
        if (rv != BCM_E_NONE) {
          printf("(%s) \n",bcm_errmsg(rv));
          return rv;
        }
    
        rv=bcm_oam_mpls_tp_channel_type_tx_set(unit, bcmOamMplsTpChannelMplsTpBfd, gach_chan_type);
        if (rv != BCM_E_NONE) {
          printf("(%s) \n",bcm_errmsg(rv));
          return rv;
        }
    ...
    if(is_device_or_above(unit, JERICHO2))
    {
        ...
        BCM_L3_ITF_SET(bfd_endpoint_info.egress_if, BCM_L3_ITF_TYPE_LIF, cint_vpls_basic_info.encap_id); /* Pwe outlif */
...
int bfd_default_mep_over_lif_example(int unit, int port1, int port2)
{
    ...
    rv = bcm_port_control_get(unit, cint_vpls_basic_info.mpls_port_id_ingress, bcmPortControlOamDefaultProfile, &oam_default_profile);
    if (rv != BCM_E_NONE) {
        printf("Error no %d, in bcm_port_control_get.\n", rv);
        return rv;
    }

    if (oam_default_profile != 2)
    {
        printf(" Mismatch for oam_default_profile");
        return BCM_E_PARAM;
    }
...
int bfd_mpls_example(int unit, int is_muti_hop_mpls) {
    ...
    */
    ...
    if(is_bfd_mpls_tp_cccv_with_fixed_dip)
    {
        bfd_endpoint_info.type = bcmBFDTunnelTypeMplsTpCcCv;
        bfd_endpoint_info.dst_ip_addr = 0x12345678;
        bfd_endpoint_info.egress_label.exp = 255;
        /** ip tll can be configured*/
        bfd_endpoint_info.ip_ttl = 200;
    }
    ...
    if (is_s_bfd_reflector) {
        rv = sbfd_reflector_endpoint_create(unit, 0x7F000001, 0x10, 3, 0);
        return rv;
    }
...
int bfdCC_example(int unit,int use_user_defined_trap) {
    ...
    */
    ...
    printf("created endpoint with id %d\n", bfd_endpoint_info.id);
    bfd_cc_id = bfd_endpoint_info.id;
...
/** This function enable/disable classifying BFD over PHP packet */
int enable_BFD_PHP(int unit,int enable)
{
    uint64 arg;
    int rv = BCM_E_NONE;
    ...
    rv = bcm_oam_control_set(unit, bcmOamControlEnableBfdPhp, arg);
    if (rv != BCM_E_NONE)
    {
...
int get_enable_BFD_PHP(int unit,int *enable)
{
    uint64 is_enable_get;
    int rv = BCM_E_NONE;
   
    COMPILER_64_ZERO(is_enable_get);
    
    rv = bcm_oam_control_get(unit, bcmOamControlEnableBfdPhp, &is_enable_get);
    if(rv != BCM_E_NONE && rv != BCM_E_NOT_FOUND)
    {
        printf("Error no %d, in bcm_oam_control_get.\n", rv);
        return rv;
    }
    *enable = COMPILER_64_LO(is_enable_get);
    return rv;
}

int test_enable_BFD_PHP(int unit)
{
    int get_enable = 0;
    int rv = BCM_E_NONE;
    
    rv = get_enable_BFD_PHP(unit,&get_enable);
    
    if(rv != BCM_E_NONE && (rv != BCM_E_NOT_FOUND))
    {
        printf("Error no %d, in get_enable_BFD_PHP.\n", rv);
        return rv;
    }
    ...
    if(get_enable == 1)
    {
        printf("If BFD_PHP has been enabled,disable it.\n", rv);
        rv = enable_BFD_PHP(unit,0);
        if(rv != BCM_E_NONE)
        {
            printf("Error no %d, in enable_BFD_PHP.\n", rv);
            return rv;
        }
    }
    else
    {
        printf("If BFD_PHP is disabled,enable it.\n", rv);
        rv = enable_BFD_PHP(unit,1);
        if(rv != BCM_E_NONE)
        {
            printf("Error no %d, in enable_BFD_PHP.\n", rv);
            return rv;
