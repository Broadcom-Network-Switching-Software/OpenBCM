...
int ieee_1588_port_set(int unit, bcm_gport_t port, int enable)
{
    ...
    rv = ieee_1588_port_pcs_reset(unit, port, 0);
    if (BCM_FAILURE(rv))
    {
        printf("Error, ieee_1588_port_pcs_reset rv = %d\n", rv);
        return rv;
...
int ieee_1588_port_pcs_reset(int unit, bcm_port_t port, int reset)
{
    ...
    rv = bcm_port_get(unit, port, port_get_flags, intf_info, map_info);
    if (BCM_FAILURE(rv))
    {
        printf("Error, bcm_port_get rv = %d\n", rv);
        return rv;
    }
    ...
    /* do PCS reset */
    rv = bcm_port_phy_control_set(unit, port, BCM_PORT_PHY_CONTROL_TX_RESET, reset);
    if (BCM_FAILURE(rv))
    {
        printf("Error, bcm_port_phy_control_set rv = %d\n", rv);
        return rv;
    }
    sal_sleep(1);
