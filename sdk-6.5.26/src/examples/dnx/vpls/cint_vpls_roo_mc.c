 ...
 * cint ../../../../src/examples/dnx/cint_vpls_roo_mc.c
    ...
    int unit)
{
    ...
    for (i = 0; i < NUM_OF_CORE_PORTS; i++)
    {
        ...
        temp_mac[5] = temp_mac[5] + i;
        rv = l3__egress_only_encap__create_inner(unit, 0, &cint_vpls_roo_mc_info.core_native_arp_id[i], temp_mac, cint_vpls_roo_mc_info.core_native_eth_rif + i, BCM_L3_NATIVE_ENCAP, 0);
        if (rv != BCM_E_NONE)
        {
        ...
        temp_mac[5] = temp_mac[5] + i;
        rv = l3__egress_only_encap__create_inner(unit, 0, &cint_vpls_roo_mc_info.core_overlay_arp_id[i], temp_mac, cint_vpls_roo_mc_info.core_overlay_eth_rif + i, 0, core_arp_flags2);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object ARP only\n");
            return rv;
    ...
    temp_mac = cint_vpls_roo_mc_info.core_native_next_hop_mac;
    temp_mac[5] = temp_mac[5] + 2;
    rv = l3__egress_only_encap__create_inner(unit, 0, &cint_vpls_roo_mc_info.core_native_arp_id[2], temp_mac, cint_vpls_roo_mc_info.access_eth_rif, BCM_L3_NATIVE_ENCAP, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object ARP only\n");
        return rv;
    ...
    temp_mac = cint_vpls_roo_mc_info.core_overlay_next_hop_mac;
    temp_mac[5] = temp_mac[5] + 2;
    rv = l3__egress_only_encap__create_inner(unit, 0, &cint_vpls_roo_mc_info.core_overlay_arp_id[2], temp_mac, cint_vpls_roo_mc_info.core_overlay_eth_rif + 2, 0, core_arp_flags2);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object ARP only\n");
        return rv;
    ...
    for (i = 0; i < NUM_OF_ACCESS_PORTS; i++)
    {
        ...
        temp_mac[5] = temp_mac[5] + i;
        rv = l3__egress_only_encap__create(unit, 0, &cint_vpls_roo_mc_info.access_arp_id[i], temp_mac, cint_vpls_roo_mc_info.access_eth_rif + i);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object ARP only");
            return rv;
    ...
    int unit)
{
    ...
    for (i = 0; i < NUM_OF_CORE_PORTS; i++)
    {
        flags = system_headers_mode?BCM_MULTICAST_INGRESS_GROUP:BCM_MULTICAST_EGRESS_GROUP;
        if (*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL))
        {
            flags |= BCM_MULTICAST_ELEMENT_STATIC;
        }
        ...
        rv = bcm_multicast_add(unit, cint_vpls_roo_mc_info.ipmc_group1, flags, 1, &rep_array);
        if (rv != BCM_E_NONE)
        {
    ...
    int unit)
{
    ...
    for (i = 0; i < NUM_OF_ACCESS_PORTS; i++)
    {
        ...
        rv = bcm_multicast_add(unit, cint_vpls_roo_mc_info.ipmc_group2, flags, 1, &rep_array);
