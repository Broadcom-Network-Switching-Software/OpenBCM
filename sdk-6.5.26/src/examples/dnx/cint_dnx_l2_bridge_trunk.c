...
int dnx_l2_pp_lag_property_set (int unit, int trunk_gport) {
    ...
    rv = bcm_port_untagged_vlan_set(unit, trunk_gport, g_dnx_l2_basic_bridge_trunk.vsi);
    if (rv != BCM_E_NONE) {
    ...
    if (g_dnx_l2_basic_bridge_trunk.is_lag_ingress == 0) {
       ...
       {
           rv = bcm_instru_gport_control_set(unit, g_dnx_l2_basic_bridge_trunk.ports[OUT_PORT_INDEX[i]], BCM_INSTRU_CONTROL_FLAG_DIRECTION_EGRESS, bcmInstruGportControlVisEnable, 1);
           if (rv != BCM_E_NONE) {
               printf("Error, in bcm_instru_gport_control_set\n");
               return rv;
           }

           rv = bcm_instru_gport_control_set(unit, g_dnx_l2_basic_bridge_trunk.ports[OUT_PORT_INDEX[i]], BCM_INSTRU_CONTROL_FLAG_DIRECTION_EGRESS, bcmInstruGportControlVisForce, 1);
           if (rv != BCM_E_NONE) {
               printf("Error, in bcm_instru_gport_control_set\n");
               return rv;
           }
    ...
    dnx_l2_basic_bridge_trunk_s * param)
{
    ...
    rv = dnx_l2_basic_bridge_trunk_modify_init(unit, param);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in dnx_l2_basic_bridge_trunk_modify_init\n");
        return rv;
    }
    ...
    rv = bcm_vlan_create(unit, g_dnx_l2_basic_bridge_trunk.vsi);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_vlan_create(vis=%d)\n", g_dnx_l2_basic_bridge_trunk.vsi);
        return rv;
    }
    ...
    rv = bcm_vlan_port_add(unit, g_dnx_l2_basic_bridge_trunk.vsi, pbmp, untag_pbmp);
    if (rv != BCM_E_NONE)
    {
    ...
    if (g_dnx_l2_basic_bridge_trunk.is_lag_ingress) {
       for (i = 0; i < NUM_OF_IN_PORTS; i++)
       {
          rv = bcm_port_untagged_vlan_set(unit, g_dnx_l2_basic_bridge_trunk.ports[IN_PORT_INDEX[i]], g_dnx_l2_basic_bridge_trunk.vsi);
          if (rv != BCM_E_NONE)
          {
              printf("Error, in bcm_port_untagged_vlan_set\n");
              return rv;
       ...
       rv = bcm_port_untagged_vlan_set(unit, g_dnx_l2_basic_bridge_trunk.ports[IN_PORT_INDEX[0]], g_dnx_l2_basic_bridge_trunk.vsi);
       if (rv != BCM_E_NONE)
       {
           printf("Error, in bcm_port_untagged_vlan_set\n");
           return rv;
     ...
     */
    rv = bcm_port_get(unit, g_dnx_l2_basic_bridge_trunk.ports[0], &flags, &interface_info, &mapping_info);
    if (rv != BCM_E_NONE)
    {
    ...
    for (i = 0; i < nof_ports; i++)
    {
        ...
        if (i != 0)
        {
            ...
            if (rv != BCM_E_NONE)
            {
                printf("Error, in bcm_trunk_member_add( %d)\n", g_dnx_l2_basic_bridge_trunk.ports[port_index[i]]);
                return rv;
            }
        }
        else
     ...
     */
    rv = dnx_l2_pp_lag_property_set(unit, trunk_gport);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in dnx_l2_pp_lag_property_set\n");
        return rv;
    }
     ...
     */
       ...
       rv = bcm_vswitch_port_add(unit, g_dnx_l2_basic_bridge_trunk.vsi,  vlan_port.vlan_port_id);
       if (rv != BCM_E_NONE)
       {
    ...
    rv = bcm_vlan_gport_add(unit, g_dnx_l2_basic_bridge_trunk.vsi, g_dnx_l2_basic_bridge_trunk.trunk_gport, 0);
    if (rv != BCM_E_NONE)
    {
