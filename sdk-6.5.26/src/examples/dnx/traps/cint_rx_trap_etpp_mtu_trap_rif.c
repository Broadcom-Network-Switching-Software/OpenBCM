 ...
 *
 * Main Function:
 *      cint_rx_trap_etpp_mtu_trap_lif_main(unit,out_rif,layer_type,compressed_layer_type,trap_id,fwd_strength,snp_strength,mtu_val);
 * Destroy Function:
    ...
    int mtu_profile)
{
    ...
    rv = cint_etpp_mtu_trap_mtu_rif_set(unit, rif_intf, trap_gport, mtu_val, layer_type, compressed_layer_type, mtu_profile);
    if(rv != BCM_E_NONE)
    {
