    ...
    int out_port)
{
    ...
    for(idx = 0; idx < eg_itf_index; ++idx) {
        rv = add_ipv4_ipmc_to_port(unit, mc_ip, 0xffffffff, src_ip, 0x0, vlan, vrf, ((idx==0)?mc_gport_drop:mc_gport_2_cpu), 0);
        if (rv != BCM_E_NONE)
        {
            return rv;
        }
    ...
    int is_ivl)
{
    ...
    if (ivl_feature_support == 1 && is_ivl)
    {
        ...
        rv = bcm_vlan_control_vlan_set(unit, eth_rif, control);
        if (rv != BCM_E_NONE)
        {
    ...
    for (mc_id = 0; mc_id < nof_mc_groups; mc_id++)
    {
        ...
        rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID | flags, &ipmc_group[mc_id]);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_create %d\n", mc_id);
            return rv;
        }
        rv = bcm_multicast_add(unit, ipmc_group[mc_id], flags, cint_ipmc_info.nof_replications, replications);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_ingress_add\n");
            return rv;
        }
        rv = add_ipv4_ipmc_bridge(unit, dip_addr, dip_mask, sip_addr, sip_mask, fwd_id, eth_rif, ipmc_group[mc_id], 0, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, add_ipv4_ipmc_bridge\n");
            return rv;
    ...
    int vsi_strict)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &fec_start);
    if (rv != BCM_E_NONE)
    {
    ...
    for (idx = 0; idx < nof_ipmc_entries; idx++)
    {
         ...
         * 6. Create FECs
         */
        rv = l3__egress_only_fec__create(unit, fec_start, fec_rifs[idx], 0, mc_gport_l, flags[idx], fec_flags2);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object FEC\n");
            return rv;
        }
        /*
         * 7. Create IPMC entry
    ...
    if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
    {
        ...
        {
            rv = l3__egress_only_fec__create(unit, fec_start, fec_rifs[4 + idx], 0, mc_gport_l, flags[4 + idx], 0);
            if (rv != BCM_E_NONE)
            {
                printf("Error, create egress object FEC\n");
                return rv;
            ...
            rv = add_host_ipv4(unit, cint_ipmc_info.sip + idx, vrf, _l3_itf, 0, 0);
            if (rv != BCM_E_NONE)
            {
    ...
    int trap_id_fec)
{
    ...
    if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
    {
        ...
        */
        ...
        {
            rv = add_ipv4_ipmc_to_port(unit, cint_ipmc_info.mc_dip + idx, no_mask, cint_ipmc_info.sip, 0,
                                    cint_ipmc_info.intf_in, cint_ipmc_info.vrf, trap_gport, flags);
            if (rv != BCM_E_NONE)
            {
    ...
    else
    {
        ...
        rv = l3__egress_only_fec__create_inner(unit, fec, 0, 0, trap_gport, 0, fec_flags2, 0, 0, &fec);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = add_ipv4_ipmc(unit, cint_ipmc_info.mc_dip, no_mask, cint_ipmc_info.sip, part_mask,
                            cint_ipmc_info.intf_in, cint_ipmc_info.vrf, 0, fec, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv4_ipmc to LPM table, \n");
            return rv;
        }
    }
