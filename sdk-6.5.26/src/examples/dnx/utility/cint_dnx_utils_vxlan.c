...
int dnx_utils_vxlan_open_vpn(int unit, int vpn, int port_class, int vni){
    ...
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_multicast_create, flags 0x%08X\n", flags);
        return rv;
    }
...
int dnx_utils_vxlan_port_add(int unit, dnx_utils_vxlan_port_add_s *vxlan_port_add) {
    ...
    if (vxlan_port_add->add_to_mc_group)
    {
        ...
        if (vxlan_port_add->is_ecmp) {
            ...
            rv = multicast__add_multicast_entry(unit, vxlan_port_add->vpn, &(vxlan_port.match_port), &encap_id, 1, egress_mc /* is_egress */);
            if (rv != BCM_E_NONE) {
            ...
            rv = dnx_utils_multicast_vxlan_port_add(unit, vxlan_port_add->vpn, vxlan_port.match_port,
...
int dnx_utils_vxlan_port_s_clear(dnx_utils_vxlan_port_add_s* vxlan_port_add) {
