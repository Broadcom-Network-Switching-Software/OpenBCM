    ...
    bcm_gport_t vlan_default)
{
    ...
    char *proc_name = "mpls_configure_port_properties";
    ...
    for (port_idx = 0; port_idx < nof_ports; port_idx++)
    {
        if (l2_port[port_idx].port != BCM_PORT_INVALID)
        {
            ...
            rv = dnx_utils_l3_port_set(unit, l3_port);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in dnx_utils_l3_port_set for index %d\n", port_idx);
                return rv;
            }
    ...
    int nof_rifs)
{
    ...
    for (rif_idx = 0; rif_idx < nof_rifs; rif_idx++)
    {
        if (rifs[rif_idx].intf != MPLS_UTIL_INVALID_VALUE)
        {
            ...
            rv = dnx_utils_l3_eth_rif_create(unit, l3_eth_rif);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in dnx_utils_l3_eth_rif_create for index %d\n", rif_idx);
                return rv;
            }
        }
    }
    ...
    int nof_fecs)
{
    ...
    for (fec_idx = 0; fec_idx < nof_fecs; fec_idx++)
    {
        if (fecs[fec_idx].fec_id != MPLS_UTIL_INVALID_VALUE)
        {
            ...
            rv = dnx_utils_l3_fec_create(unit, fec_entry);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in dnx_utils_l3_fec_create for index %d\n", fec_idx);
                return rv;
            }
        ...
        if (host[host_idx].ipv4 != MPLS_UTIL_INVALID_VALUE)
        {
            ...
            if ((host[host_idx].ipv4 & 0xf0000000) != 0xe0000000)
            {
                if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
                {
                    ...
                    rv = dnx_utils_l3_host_ipv4_add(unit, l3_host_ipv4);
                    if (rv != BCM_E_NONE)
                    {
                        printf("Error, in dnx_utils_l3_host_ipv4_add for index %d\n", host_idx);
                        return rv;
                    }
                }
                else
                {
                    ...
                    if (rv != BCM_E_NONE)
                    {
                        printf("IPv4 bcm_l3_route_add failed: %x \n", rv);
                    }
                }
            }
            else
            {
                ...
                if (rv != BCM_E_NONE)
                {
                    printf("Error, in bcm_ipmc_add for index %d\n", host_idx);
                    return rv;
                }
            }
        }
        ...
        if (host[host_idx].valid_ipv6)
        {
            ...
            {                
                if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
                {
                    ...
                    rv = dnx_utils_l3_host_ipv6_add(unit, l3_host_ipv6);
                    if (rv != BCM_E_NONE)
                    {
                        printf("Error, in dnx_utils_l3_host_ipv6_add for index %d\n", host_idx);
                        return rv;
                    }
                }
                else
