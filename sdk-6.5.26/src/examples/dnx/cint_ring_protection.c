...
int ring_protection_vlan_port_create(int *units_ids, int nof_units, int out_sysport, int in_sysport)
{
    ...
    rv = vswitch_metro_mp_multi_device_fixed_ports(units_ids, nof_units, 0xce, 3);
    if (rv != BCM_E_NONE){
        printf("Error, in vswitch_metro_mp_multi_device_fixed_ports\n");
        return rv;
    }
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        if (is_system_port_is_local(unit, vswitch_metro_mp_info.sysports[3])) {
            rv = vswitch_metro_add_port(unit, 3, &outlifs[3], flags);
            if (rv != BCM_E_NONE) {
...
int ring_protect_add_l2_addr(int *units_ids, int nof_units, int gport_idx, bcm_mac_t mac_lsbs)
{
    ...
    for (unit_idx = 0 ; unit_idx < nof_units ; unit_idx++) {
        ...
        if (is_system_port_is_local(unit, vswitch_metro_mp_info.sysports[gport_idx])) {
            rv = vswitch_add_l2_addr_w_group_id(unit, outlifs[gport_idx], mac_lsbs, vswitch_metro_mp_info.flush_group_id[gport_idx], vswitch_metro_mp_info.vsi);
            if (rv != BCM_E_NONE) {
                printf("Error, ring_protect_add_l2_addr\n");
                return rv;
            }
        }
    }
