    ...
    int unit)
{
     ...
     int unit)
 {
     ...
     *  2nd pass VTT1.
     ...
     rv = bcm_switch_control_indexed_port_get(unit, cint_mpls_mldp_info.recycle_port_id, key, &get_info);
     if (rv != BCM_E_NONE)
     {
    ...
    rv = bcm_port_control_set(unit, cint_mpls_mldp_info.recycle_port_id, bcmPortControlOverlayRecycle,
    ...
    int access_out_port)
{
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|mc_flags, &cint_mpls_mldp_info.mldp_mc_id);
    if (rv != BCM_E_NONE)
    {
    ...
    /*MCDB setting*/
    rv = bcm_multicast_set(unit, cint_mpls_mldp_info.mldp_mc_id, mc_flags, 3, replications);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|mc_flags, &cint_mpls_mldp_info.ipmc_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_multicast_create \n");
        return rv;
    ...
    /*MCDB setting*/
    rv = bcm_multicast_set(unit, cint_mpls_mldp_info.ipmc_id, mc_flags, 1, replications);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_multicast_set\n");
        return rv;
    ...
    int access_out_port)
{
    ...
    rv = bcm_multicast_create(unit, (BCM_MULTICAST_WITH_ID | flags), &mcid);
    if (rv != BCM_E_NONE) {
    ...
    int access_out_port)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error(%d), in bcm_port_match_add for match_port 0x%08X\n", rv, match_info.port);
        return rv;
    }
