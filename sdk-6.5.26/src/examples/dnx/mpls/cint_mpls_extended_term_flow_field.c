...
bcm_field_group_t mpls_extended_term_fg_id = 0;
       ...
       bcm_if_t rcy_extended_term_encap_id)
 {
    ...
    rv = bcm_switch_control_indexed_port_get(unit, rcy_port, key, &get_info);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = bcm_port_control_set(unit, rcy_port, bcmPortControlRecycleApp,
            ((get_info.value == BCM_SWITCH_PORT_HEADER_TYPE_IBCH1_MODE) ?
                    bcmPortControlRecycleAppDefault : bcmPortControlRecycleAppExtendedTerm));
