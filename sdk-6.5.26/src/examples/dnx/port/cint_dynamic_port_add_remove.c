    ...
    uint32 flags)
{
    ...
    {    /*
         ...
         */
        {
            ...
            rv = cint_dyn_port_is_last_shared_otm(unit, port, &is_last_shared_otm);
            if (rv != BCM_E_NONE)
            {
                printf("Error, cint_dyn_port_is_last_shared_otm: unit=%d, port=%d\n", unit, port);
                return rv;
            }
            if (is_last_shared_otm)
            {
                 ...
                 */
                if (!(flags_get & BCM_PORT_ADD_FLEXE_SAR))
                {
    ...
    int pp_set_header_only)
{
    ...
    for (port_index = 0; port_index < nof_ports_resource_set; ++port_index)
    {
        ...
        if (resource_set_arr[port_index].link_training == 0)
        {
            ...
            if ((interface_info_tmp.interface != BCM_PORT_IF_VIRTUAL_FLEXE_CLIENT) && (interface_info_tmp.interface != BCM_PORT_IF_FLEXE_CLIENT)
                && (interface_info_tmp.interface != BCM_PORT_IF_RCY))
            {
    ...
    int cl72_en)
{
     ...
     * Add the port
     */
    rv = cint_dyn_port_add_flexe_phy_port(unit, port, phy_pbmp, rate_mhz, cl72_en, flags);
    if (rv != BCM_E_NONE)
    {
        printf("Error, cint_dyn_port_add_flexe_phy_port, unit: %d, logical_port: %d\n", unit, port);
        return rv;
        ...
        bcm_port_interface_info_t interface_info)
{
    ...
    if (!(flags & BCM_PORT_ADD_FLEXE_PHY) &&
        ...
        (interface_info.interface != BCM_PORT_IF_OTN) && !sar_is_non_tdm_port &&
