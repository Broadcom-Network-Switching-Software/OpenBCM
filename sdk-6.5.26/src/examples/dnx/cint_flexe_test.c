...
int dnx_flexe_lane_speed_init(int unit0, int unit1) {
    ...
    int out_vlan)
{
    ...
    rv = bcm_l3_host_add(unit, &l3_host);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_l3_host_add(dip6) ****\n", proc_name, rv);
        return rv;
    }
    ...
    int fec_type)
{
    ...
    /* Port creation */
    rv = cint_dyn_port_add_port_full_example(unit, port, mapping_info, interface_info,
              resource, dfe_mode, medium_type, header_type_in, header_type_out, flags);
    if (rv != BCM_E_NONE) {
        printf("Error(%d): fail in cint_dyn_port_add_port_full_example(port %d)\n", rv, port);
        return rv;
    }

    /* TPID setting */
    rv = dnx_flexe_util_port_tpid_cfg(unit, port, 0x8100, 1);
    if (rv != BCM_E_NONE) {
        printf("Error(%d): fail in dnx_flexe_util_port_tpid_cfg(port %d)\n", rv, port);
        return rv;
    }
    ...
    int rate_mhz)
{
    ...
    rv = cint_dyn_port_add_flexe_phy_port(unit, phy_port_id, phy_pbmp, rate_mhz, phy_cl72_enable, 0);
    if (rv != BCM_E_NONE) {
    ...
    bcm_gport_t flexe_grp)
{
    ...
    rv = bcm_port_flexe_group_get(unit, flexe_grp, flags, max_nof_pcs, &grp_phy_info, &actual_nof_pcs);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_get() ****\n", proc_name, rv);
        return rv;
    }
    if (actual_nof_pcs <= 0) {
    ...
    int unit_dis)
{
    ...
    for (i = 0; i < 8; i++) {
        if (synce_flexe_grp_all[i] != 0) {
            ...
            rv = bcm_port_flexe_oh_alarm_get(unit_cen, synce_flexe_grp_all[i], 0, &oh_alarm_cen);
            rv |= bcm_port_flexe_oh_alarm_get(unit_dis, synce_flexe_grp_all[i], 0, &oh_alarm_dis);
            sal_sleep(3);
            rv |= bcm_port_flexe_oh_alarm_get(unit_cen, synce_flexe_grp_all[i], 0, &oh_alarm_cen);
            rv |= bcm_port_flexe_oh_alarm_get(unit_dis, synce_flexe_grp_all[i], 0, &oh_alarm_dis);
            if (rv != BCM_E_NONE) {
                printf("**** Error(%d), fail in bcm_port_flexe_oh_alarm_get in group %d ****\n", rv, synce_flexe_grp_all[i]);
                return rv;
            }
    ...
    for (i = 0; i < 8; i++) {
        if (flexe_phy_check_list[i]) {
            ...
            rv = bcm_port_flexe_oh_alarm_get(unit_cen, flexe_phy_check_list[i], 0, &oh_alarm_cen);
            rv |= bcm_port_flexe_oh_alarm_get(unit_dis, flexe_phy_check_list[i], 0, &oh_alarm_dis);
            sal_sleep(3);
            rv |= bcm_port_flexe_oh_alarm_get(unit_cen, flexe_phy_check_list[i], 0, &oh_alarm_cen);
            rv |= bcm_port_flexe_oh_alarm_get(unit_dis, flexe_phy_check_list[i], 0, &oh_alarm_dis);
            if (rv != BCM_E_NONE) {
                printf("**** Error(%d), fail in bcm_port_flexe_oh_alarm_get on PHY %d ****\n", rv, flexe_phy_check_list[i]);
                return rv;
            }
    ...
    bcm_gport_t flexe_grp)
{
    ...
    sync_error = dnx_flexe_util_oh_alarm_check(unit_cen, unit_dis);
    ...
    uint32 speed_flag)
{
    ...
    rv = cint_dyn_port_add_port_full_example(unit, client_id, mapping_info, interface_info,
              resource, dfe_mode, medium_type,  BCM_SWITCH_PORT_HEADER_TYPE_ETH, BCM_SWITCH_PORT_HEADER_TYPE_ETH, flags);
    if (rv != BCM_E_NONE) {
    ...
    int clear_flag)
{
    ...
    if (!clear_flag) {
        ...
        if (!is_flexe_gen2) {
            ...
            rv = dnx_flexe_util_client_create(unit, client_c, port_type_busC, bandwidth, 0);
            if (rv != BCM_E_NONE) {
    ...
    if (!clear_flag)
    {
        if (!is_flexe_gen2) {
            ...
            rv = bcm_port_flexe_flow_set(unit, flags, client_b_L2, 0, client_c);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting FlexE flow(client_b_L2 to client_c) ****\n", proc_name, rv);
                return rv;
            }
            rv = cint_dnx_tdm_sar_example(unit, client_c, client_c, 1, flexe_gen2_tdm_min_size, flexe_gen2_tdm_max_size, tdm_mc_id);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in cint_dnx_tdm_sar_example() ****\n", proc_name, rv);
                return rv;
            }
            rv = bcm_port_flexe_flow_set(unit, flags, client_c, 0, eth_port_L1);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting FlexE flow(client_c to eth_port_L1) ****\n", proc_name, rv);
                return rv;
            }
        }
    }
    else
    {
        if (!is_flexe_gen2) {
            ...
            rv = bcm_port_flexe_flow_clear(unit, client_b_L2, 0, client_c);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting FlexE flow(client_b_L2 to eth_port_L1) ****\n", proc_name, rv);
                return rv;
            }
            /* Add steps to clear TDM datapath client_c -> client_c */
            rv = bcm_port_flexe_flow_clear(unit, client_c, 0, eth_port_L1);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting FlexE flow(client_b_L2 to eth_port_L1) ****\n", proc_name, rv);
                return rv;
            }
    ...
    if (clear_flag) {
        if (!is_flexe_gen2) {
            ...
            rv = dnx_flexe_util_client_remove(unit, client_c);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_remove(client %d) ****\n", proc_name, rv, client_c);
                return rv;
            }
        }
    ...
    bcm_port_t dst_clientB_ori)
{
    ...
    if (!clear_flag)
    {
        if (!is_flexe_gen2) {
            ...
            rv = bcm_port_flexe_flow_set(unit, flags, eth_port_L1, 0, client_c);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting FlexE flow(eth_port_L1 to client_c) ****\n", proc_name, rv);
                return rv;
            }
            rv = cint_dnx_tdm_sar_example(unit, client_c, client_c, 1, flexe_gen2_tdm_min_size, flexe_gen2_tdm_max_size, tdm_mc_id);
            if (rv != BCM_E_NONE) {
            ...
            rv = bcm_port_flexe_flow_set(unit, flags, client_c, 0, client_b_L2);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting FlexE flow(client_c to client_b_L2) ****\n", proc_name, rv);
                return rv;
            }
            rv = bcm_port_force_forward_set(unit, client_b_L2, eth_port_L2, 1);
            if (rv != BCM_E_NONE) {
    ...
    else
    {
        if (!is_flexe_gen2) {
            ...
            /** Clear FlexE_Gen2 datapath **/
            rv = bcm_port_flexe_flow_clear(unit, eth_port_L1, 0, client_c);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting FlexE flow(eth_port_L1 to client_c) ****\n", proc_name, rv);
                return rv;
            }
            /* Add steps to clear TDM datapath client_c -> client_c */
            rv = bcm_port_flexe_flow_clear(unit, client_c, 0, client_b_L2);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting FlexE flow(client_c to client_b_L2) ****\n", proc_name, rv);
                return rv;
            }
    ...
    bcm_gport_t * mod_gport)
{
    ...
    /* Get system gport */
    rv = bcm_stk_gport_sysport_get(unit, logic_port_id, &system_gport); 
    /* Get mod_gport via system gport */
    rv |= bcm_stk_sysport_gport_get(unit, system_gport, mod_gport);
    ...
    int clear_flag)
{
    ...
    if (!clear_flag)
    {
        if (dnx_flexe_gen2_support(unit, -1)) {
            ...
            rv = bcm_port_flexe_flow_set(unit, flags, client_id_busA, failover_channel_id, dst_eth_port);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting BusA2Eth L1 datapath ****\n", proc_name, rv);
                return rv;
            }
        } else {
    ...
    else
    {
        ...
        if (dnx_flexe_gen2_support(unit, -1)) {
            rv = bcm_port_flexe_flow_clear(unit, client_id_busA, failover_channel_id, dst_eth_port);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in clearing BusA2Eth L1 datapath ****\n", proc_name, rv);
                return rv;
            }
        } else {
    ...
    int clear_flag)
{
    ...
    if (!clear_flag)
    {
        if (dnx_flexe_gen2_support(unit, -1)) {
            ...
            rv = bcm_port_flexe_flow_set(unit, flags, src_eth_port, failover_channel_id, client_id_busA);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in setting Eth2BusA L1 datapath ****\n", proc_name, rv);
                return rv;
            }
        } else {
    ...
    else
    {
        ...
        if (dnx_flexe_gen2_support(unit, -1)) {
            rv = bcm_port_flexe_flow_clear(unit, src_eth_port, failover_channel_id, client_id_busA);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in clearing Eth2BusA L1 datapath ****\n", proc_name, rv);
                return rv;
            }
        } else {
    ...
    int clear_flag)
{
    ...
    if (!is_flexe_gen2) {
        ...
        rv = dnx_flexe_util_port_type_verify(unit, dest_port, port_type_ilkn_L1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Mismached port type ****\n", proc_name);
            return rv;
        }
    ...
    /* Disable ports */
    if (!is_flexe_gen2) {
        rv = bcm_port_enable_set(dst_unit, dst_logic_port, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to disable dest_port ****\n", proc_name, rv);
            return rv;
        }
    }
    ...
    if (!clear_flag)
    {
        if (is_flexe_gen2) {
            ...
            rv = cint_dnx_tdm_sar_example_multi_device(unit, dst_unit, client_id_busC, dest_port, flexe_gen2_tdm_is_mc, flexe_gen2_tdm_grp_id, flexe_gen2_tdm_min_size, flexe_gen2_tdm_max_size);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in cint_dnx_tdm_sar_example_multi_device() ****\n", proc_name, rv);
                return rv;
            }
        } else {
            ...
            rv = bcm_port_flexe_flow_set(unit, flags, client_id_busC, 0, dest_port);
            if (rv != BCM_E_NONE) {
    ...
    else
    {
        if (is_flexe_gen2) {
            ...
            /* Clear above L1 datapath */
            rv = bcm_port_flexe_flow_clear(unit, client_id_busC, 0, dest_port);
            if (rv != BCM_E_NONE) {
    ...
    /* Enable ports */
    if (!is_flexe_gen2) {
        rv = bcm_port_enable_set(dst_unit, dst_logic_port, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to enable dest_port ****\n", proc_name, rv);
            return rv;
        }
    }
    ...
    int clear_flag)
{
    ...
    /* Port type check */
    if (!is_flexe_gen2) {
        rv = dnx_flexe_util_port_type_verify(unit, src_port, port_type_ilkn_L1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Mismached port type ****\n", proc_name);
            return rv;
        }
    }
    rv = dnx_flexe_util_port_type_verify(dst_unit, dst_sar_client, port_type_busC);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Mismached port type ****\n", proc_name);
        return rv;
    }
    ...
    /* Disable ports */
    if (!is_flexe_gen2) {
        rv = bcm_port_enable_set(unit, src_port, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to disable src_port ****\n", proc_name, rv);
            return rv;
        }
    }
    ...
    if (!clear_flag)
    {
        if (is_flexe_gen2) {
            ...
            rv = cint_dnx_tdm_sar_example_multi_device(unit, dst_unit, src_port, client_id_busC, flexe_gen2_tdm_is_mc, flexe_gen2_tdm_grp_id, flexe_gen2_tdm_min_size, flexe_gen2_tdm_max_size);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in cint_dnx_tdm_sar_example_multi_device() ****\n", proc_name, rv);
                return rv;
            }
            rv = bcm_port_flexe_flow_set(dst_unit, flags, dst_sar_client, failover_channel_id, client_id_busA);
            if (rv != BCM_E_NONE) {
            ...
            rv = bcm_port_flexe_flow_set(unit, flags, src_port, 0, client_id_busC);
            if (rv != BCM_E_NONE) {
    ...
    else
    {
        if (is_flexe_gen2) {
            /* WARNING: Need to confirm TDM clear cint here */
            rv = bcm_port_flexe_flow_clear(dst_unit, dst_sar_client, failover_channel_id, client_id_busA);
            if (rv != BCM_E_NONE) {
            ...
            rv = bcm_port_flexe_flow_clear(unit, src_port, 0, client_id_busC);
            if (rv != BCM_E_NONE) {
    ...
    /* Enable ports */
    if (!is_flexe_gen2) {
        rv = bcm_port_enable_set(unit, src_port, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to enable src_port ****\n", proc_name, rv);
            return rv;
        }
    }
    ...
    int clear_flag)
{
    ...
    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, src_eth_port, client_id_busC, dest_port, clear_flag);
    return rv;
}
    ...
    int clear_flag)
{
    ...
    rv = dnx_flexe_util_L1_datapath_src_sar_flexe(unit, src_port, client_id_busC, eth_port, clear_flag);
    return rv;
}
    ...
    int clear_flag)
{
    ...
    rv = bcm_stk_gport_sysport_get(dst_unit, dst_port, &dst_sys_gport);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Fail in bcm_stk_gport_sysport_get() ****\n", proc_name);
        return rv;
    }

    /* Port type check */
    rv = dnx_flexe_util_port_type_verify(src_unit, flexe_client, port_type_busA);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Mismached port type ****\n", proc_name);
        return rv;
    }
    rv = dnx_flexe_util_port_type_verify(src_unit, flexe_tmac, port_type_busB);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Mismached port type ****\n", proc_name);
        return rv;
    }
    ...
    if (!clear_flag)
    {
        rv = bcm_port_flexe_flow_set(src_unit, flags, flexe_client, failover_channel_id, flexe_tmac);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set() ****\n", proc_name, rv);
            return rv;
        }
    }
    else
    {
        rv = bcm_port_flexe_flow_clear(src_unit, flexe_client, failover_channel_id, flexe_tmac);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_clear() ****\n", proc_name, rv);
            return rv;
        }
    }

    rv = bcm_port_force_forward_set(src_unit, flexe_tmac, dst_sys_gport, !clear_flag);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_force_forward_set() ****\n", proc_name, rv);
        return rv;
    }
    ...
    int clear_flag)
{
    ...
    rv = bcm_stk_gport_sysport_get(dst_unit, dst_flexe_tmac, &dst_sys_gport);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Fail in bcm_stk_gport_sysport_get() ****\n", proc_name);
        return rv;
    }
    ...
    /* Port type check */
    rv = dnx_flexe_util_port_type_verify(dst_unit, dst_flexe_client, port_type_busA);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Mismached port type ****\n", proc_name);
        return rv;
    }
    rv = dnx_flexe_util_port_type_verify(dst_unit, dst_flexe_tmac, port_type_busB);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Mismached port type ****\n", proc_name);
        return rv;
    }
    ...
    rv = bcm_port_force_forward_set(src_unit, src_port, dst_sys_gport, !clear_flag);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_force_forward_set() ****\n", proc_name, rv);
        return rv;
    }
    ...
    if (!clear_flag)
    {
        rv = bcm_port_flexe_flow_set(dst_unit, flags, dst_flexe_tmac, failover_channel_id, dst_flexe_client);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set() ****\n", proc_name, rv);
            return rv;
        }
    ...
    {
        rv = bcm_port_flexe_flow_clear(dst_unit, dst_flexe_tmac, failover_channel_id, dst_flexe_client);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_clear() ****\n", proc_name, rv);
            return rv;
        }
    ...
    bcm_port_t phy_port)
{
    ...
    if (is_L1_switch_dis)
    {
         ...
         */
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit_dis, client_a, client_c, ilkn_port, 0);
        if (rv != BCM_E_NONE) {
         ...
         */
        rv = dnx_flexe_util_L1_datapath_src_sar_flexe(unit_dis, ilkn_port, client_c, client_a, 0);
        if (rv != BCM_E_NONE) {
    ...
    bcm_port_t nof_phy)
{
    ...
    rv = bcm_port_ifg_set(unit_cen, eth_400g_cen, 0, BCM_PORT_DUPLEX_FULL, 88);
    rv |= bcm_port_ifg_set(unit_dis, eth_400g_dis, 0, BCM_PORT_DUPLEX_FULL, 88);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_ifg_set ****\n", proc_name, rv);
        return rv;
    }
    ...
    bcm_port_t ilkn_L23)
{
    ...
    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit_dis, client_a_L1, client_c, ilkn_L1, 0);
    if (rv != BCM_E_NONE) {
    ...
    rv = dnx_flexe_util_L1_datapath_src_sar_flexe(unit_dis, ilkn_L1, client_c, client_a_L1, 0);
    if (rv != BCM_E_NONE) {
    ...
    bcm_port_t ilkn_port)
{
    ...
    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit_dis, client_a_src, client_c_dst, ilkn_port, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to create the 3rd failover path(FlexE2ILKN) ****\n", proc_name, rv);
        return rv;
    }
    failover_channel_id = 0;
    rv = dnx_flexe_util_L1_datapath_src_sar_flexe(unit_dis, ilkn_port, client_c_dst, client_a_src, 0);
    if (rv != BCM_E_NONE) {
    ...
    int is_L1_to_L2)
{
    ...
    if (is_L1_to_L2) {
        ...
        rv = dnx_flexe_util_L2_port_add(unit, eth_port_id, eth_lanes[eth_flexe_pair_index], BCM_PORT_IF_NIF_ETH, nof_lanes_per_eth, eth_speed, fec_type);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_port_add(id %d) ****\n", proc_name, rv, eth_port_id);
            return rv;
        }
        /* IPG setting */
        ...
        rv = cint_dyn_port_remove_port_full_example(unit, eth_port_id, 0);
        if (rv != BCM_E_NONE) {
    ...
    int nof_clients)
{
    ...
    for (i = 0; i < nof_clients; i++) {
        ...
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a_base+i, client_c_base+i, ilkn_port_start+i, 0);
        if (rv != BCM_E_NONE) {
        ...
        rv = dnx_flexe_util_L1_datapath_src_sar_flexe(unit, ilkn_port_start+i, client_c_base+i, client_a_base+i, 0);
        if (rv != BCM_E_NONE) {
    ...
    int eth2_is_L1)
{
    ...
    if (eth1_is_L1) {
        ...
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit_dis, client_a_1, client_c_1, ilkn_1, 0);
        rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(unit_dis, ilkn_1, client_c_1, client_a_1, 0);
        if (rv != BCM_E_NONE) {
    ...
    if (eth2_is_L1) {
        ...
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit_dis, client_a_2, client_c_2, ilkn_2, 0);
        rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(unit_dis, ilkn_2, client_c_2, client_a_2, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to set 1st datapath on distributed device ****\n", proc_name, rv);
            return rv;
        }
    ...
    bcm_port_t dis_port_L1)
{
    ...
    /* Set proc_name */
    char *proc_name;
    proc_name = "dnx_flexe_L1_L2_datapath_with_mgmt_ports";
    ...
    int rate_ilkn2flexe)
{
    ...
    if (is_L1_switch_dis)
    {
        ...
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a, client_c, ilkn_port, 0);
        rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(unit, ilkn_port, client_c, client_a, 0);
        if (rv != BCM_E_NONE) {
    ...
    int is_L1)
{
    ...
    if (is_L1) {
        if (!is_flexe_gen2) {
            ...
            rv = bcm_port_flexe_flow_set(unit, 0, eth_1, 0, client_a_1);
            rv |= bcm_port_flexe_flow_set(unit, 0, client_a_1, 0, eth_2);
            rv |= bcm_port_flexe_flow_set(unit, 0, eth_2, 0, client_a_2);
            rv |= bcm_port_flexe_flow_set(unit, 0, client_a_2, 0, eth_1);
        }
        if (rv != BCM_E_NONE) {
    ...
    int port_speed)
{
    ...
    if (dnx_flexe_gen2_support(unit, -1)) {
        ...
        rv = dnx_flexe_basic_cfg_main(unit, flexe_phy_speed, 0, port_speed, eth_L2, flexe_phy, 1, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_basic_cfg_main ****\n", proc_name, rv);
            return rv;
        }
    } else {
        /***************************************************************************
    ...
    bcm_port_t ilkn_port_lo)
{
    ...
    /* Set proc_name */
    char *proc_name;
    proc_name = "dnx_flexe_prd_distributed_mode_main";
    ...
    /* Create client_b */
    rv = dnx_flexe_util_client_create(unit, client_b, port_type_busB_L2_L3, client_speed, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to create client_b ****\n", proc_name, rv);
        return rv;
    }
    ...
    /* Create client_prd */
    rv = dnx_flexe_util_client_create(unit, client_prd, port_type_busB_L2_L3, client_speed_prd, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to create client_prd ****\n", proc_name, rv);
        return rv;
    }
    ...
    rv = bcm_port_enable_set(unit, client_prd, 0);
    rv |= bcm_port_priority_config_set(unit, client_prd, &priority_config);
    rv |= bcm_port_enable_set(unit, client_prd, 1);
    if (rv != BCM_E_NONE) {
    ...
    /* Disable ports */
    rv = bcm_port_enable_set(unit, client_b, 0);
    rv |= bcm_port_enable_set(unit, client_prd, 0);
    rv |= bcm_port_enable_set(unit, ilkn_port_hi, 0);
    rv |= bcm_port_enable_set(unit, ilkn_port_lo, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to disable ports ****\n", proc_name, rv);
        return rv;
    }
    ...
    rv = bcm_port_force_forward_set(unit, eth_port_tx, client_b, 1);
    rv |= bcm_port_flexe_flow_set(unit, 0, client_b, 0, client_prd);
    rv |= bcm_port_flexe_flow_set(unit, 0, gport_pri_hi, 0, ilkn_port_hi);
    rv |= bcm_port_flexe_flow_set(unit, 0, gport_pri_lo, 0, ilkn_port_lo);
    rv |= bcm_port_force_forward_set(unit, ilkn_port_hi, eth_port_rx, 1);
    rv |= bcm_port_force_forward_set(unit, ilkn_port_lo, eth_port_rx, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in FlexE datapath setting ****\n", proc_name, rv);
        return rv;
    }

    /* Enable ports */
    rv = bcm_port_enable_set(unit, client_b, 1);
    rv |= bcm_port_enable_set(unit, client_prd, 1);
    rv |= bcm_port_enable_set(unit, ilkn_port_hi, 1);
    rv |= bcm_port_enable_set(unit, ilkn_port_lo, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to enable ports ****\n", proc_name, rv);
        return rv;
    }
    ...
    /* Loopback ILKN ports */
    rv = bcm_port_loopback_set(unit, ilkn_port_hi, 1);
    rv |= bcm_port_loopback_set(unit, ilkn_port_lo, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_loopback_set ****\n", proc_name, rv);
        return rv;
    }
    ...
    int is_L1_switch_dis)
{
    ...
    {
        /* Clear L1 datapath */
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit_dis, client_a, client_c, ilkn_port, 1);
        rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(unit_dis, ilkn_port, client_c, client_a, 1);
        if (rv != BCM_E_NONE) {
    ...
    int is_flexe_L1)
{
    ...
    /* Set group ID in overhead */
    if (!flexe_bypass_en) {
        rv = bcm_port_flexe_oh_set(unit, flexe_grp, BCM_PORT_FLEXE_TX, bcmPortFlexeOhGroupID, grp_index + 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to set OhGroupID(unit %d) ****\n", proc_name, rv, unit);
            return rv;
        }
    ...
    if (flexe_phy_loopback) {
        for (i = 0; i < nof_pcs; i++) {
            ...
            rv = bcm_port_loopback_set(unit, flexe_phy, BCM_PORT_LOOPBACK_PHY);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_loopback_set(flexe_phy %d) ****\n", proc_name, rv, flexe_phy);
                return rv;
            }
        }
    }
    ...
    int en_dis_sequence)
{
    ...
    for (i = 0; i < nof_clients; i++) {
        ...
        rv = dnx_flexe_util_client_create(unit, busb_clients[i], client_b_type, client_speed, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to create BusB client(index %d) ****\n", proc_name, rv, i);
            return rv;
        }

        if (i == en_dis_sequence) {
            printf("\n**** %s: Disabling BusB client(index %d) ****\n", proc_name, en_dis_sequence);
            rv = bcm_port_enable_set(unit, busb_clients[i], 0);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail to disable BusB client(index %d) ****\n", proc_name, rv, i);
                return rv;
            }
        }
    }
    ...
    rv = bcm_port_enable_set(unit, busb_clients[en_dis_sequence], 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to enable BusB client(index %d) ****\n", proc_name, rv, en_dis_sequence);
        return rv;
    }
    ...
    if (client_type == 1) {
        /* Disable ETH ports */
        rv = bcm_port_enable_set(unit, eth_tx, 0);
        rv |= bcm_port_enable_set(unit, eth_lb, 0);
        rv |= bcm_port_enable_set(unit, eth_rx, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to disable ETH ports ****\n", proc_name, rv);
            return rv;
        }

        /* L1 datapath */
        rv = bcm_port_flexe_flow_set(unit, 0, eth_tx, 0, client_b_0);
        rv |= bcm_port_flexe_flow_set(unit, 0, client_b_0, 0, client_b_1);
        rv |= bcm_port_flexe_flow_set(unit, 0, client_b_1, 0, eth_lb);
        rv |= bcm_port_flexe_flow_set(unit, 0, eth_lb, 0, client_b_2);
        rv |= bcm_port_flexe_flow_set(unit, 0, client_b_2, 0, client_b_3);
        rv |= bcm_port_flexe_flow_set(unit, 0, client_b_3, 0, eth_rx);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in L1 datapath setting ****\n", proc_name, rv);
            return rv;
        }

        /* Enable ETH ports */
        rv = bcm_port_enable_set(unit, eth_tx, 1);
        rv |= bcm_port_enable_set(unit, eth_lb, 1);
        rv |= bcm_port_enable_set(unit, eth_rx, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to disable ETH ports ****\n", proc_name, rv);
            return rv;
        }
        ...
        /* L2 datapath */
        rv = bcm_port_force_forward_set(unit, eth_tx, client_b_0, 1);
        rv |= bcm_port_flexe_flow_set(unit, 0, client_b_0, 0, client_b_1);
        rv |= bcm_port_force_forward_set(unit, client_b_1, eth_lb, 1);
        rv |= bcm_port_force_forward_set(unit, eth_lb, client_b_2, 1);
        rv |= bcm_port_flexe_flow_set(unit, 0, client_b_2, 0, client_b_3);
        rv |= bcm_port_force_forward_set(unit, client_b_3, eth_rx, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in L2 datapath setting ****\n", proc_name, rv);
            return rv;
        }
    } else {
        /* Mixed datapath */
        ...
        rv |= bcm_port_enable_set(unit, eth_lb, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in mixed datapath setting ****\n", proc_name, rv);
            return rv;
        }
    }
    ...
    /* Enable BusB clients */
    for (i = 0; i < nof_clients; i++) {
        rv = bcm_port_enable_set(unit, busb_clients[i], 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to enable BusB client(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    ...
    /* Loopback eth_lb */
    rv = bcm_port_loopback_set(unit, eth_lb, BCM_PORT_LOOPBACK_PHY);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_loopback_set ****\n", proc_name, rv);
        return rv;
    }
    ...
    int eth_lb)
{
    ...
    /* Set proc_name */
    char *proc_name;
    proc_name = "dnx_flexe_busB_creation_with_sequence_clear";
    ...
    if (client_type == 1) {
        ...
        /* L1 datapath */
        rv = bcm_port_flexe_flow_clear(unit, eth_tx, 0, client_b_0);
        rv |= bcm_port_flexe_flow_clear(unit, client_b_0, 0, client_b_1);
        rv |= bcm_port_flexe_flow_clear(unit, client_b_1, 0, eth_lb);
        rv |= bcm_port_flexe_flow_clear(unit, eth_lb, 0, client_b_2);
        rv |= bcm_port_flexe_flow_clear(unit, client_b_2, 0, client_b_3);
        rv |= bcm_port_flexe_flow_clear(unit, client_b_3, 0, eth_rx);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to clear L1 datapath ****\n", proc_name, rv);
            return rv;
        }
        ...
        /* L2 datapath */
        rv = bcm_port_force_forward_set(unit, eth_tx, client_b_0, 0);
        rv |= bcm_port_flexe_flow_clear(unit, client_b_0, 0, client_b_1);
        rv |= bcm_port_force_forward_set(unit, client_b_1, eth_lb, 0);
        rv |= bcm_port_force_forward_set(unit, eth_lb, client_b_2, 0);
        rv |= bcm_port_flexe_flow_clear(unit, client_b_2, 0, client_b_3);
        rv |= bcm_port_force_forward_set(unit, client_b_3, eth_rx, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to clear L2 datapath ****\n", proc_name, rv);
            return rv;
        }
    } else {
        /* Mixed datapath */
        ...
        rv |= bcm_port_enable_set(unit, eth_lb, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to clear mixed datapath ****\n", proc_name, rv);
            return rv;
        }
    }
    ...
    for (i = 0; i < nof_clients; i++) {
        rv = dnx_flexe_util_client_remove(unit, busb_clients[i]);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    }

    /* Loopback eth_lb */
    rv = bcm_port_loopback_set(unit, eth_lb, BCM_PORT_LOOPBACK_NONE);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_loopback_set ****\n", proc_name, rv);
        return rv;
    }
    ...
    bcm_port_t eth_port_out)
{
    ...
    /* Set proc_name */
    char *proc_name;
    proc_name = "dnx_flexe_dpp_busb_l2";
    ...
    /* Create busB clients */
    for (i = 0; i < nof_clients; i++) {
        rv = dnx_flexe_util_client_create(unit, client_b_start + i, port_type_busB_L2_L3, client_speed, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to create busB client(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    ...
    rv = bcm_port_force_forward_set(unit, eth_port_in, client_b_start, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(eth_in->client_b_start) ****\n", proc_name, rv);
        return rv;
    }
    for (i = 0; i < nof_clients; i++) {
        ...
        /** loopback itself */
        rv = bcm_port_enable_set(unit, current_client_id, 0);
        rv |= bcm_port_flexe_flow_set(unit, 0, current_client_id, 0, current_client_id);
        rv |= bcm_port_enable_set(unit, current_client_id, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in BusB to BusB flow setting(index %d) ****\n", proc_name, rv, i);
            return rv;
        }

        /** Force forwarding */
        if (i == nof_clients - 1) {
            rv = bcm_port_force_forward_set(unit, current_client_id, eth_port_out, 1);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(busb-->eth_out, index %d) ****\n", proc_name, rv, i);
                return rv;
            }
        } else {
            rv = bcm_port_force_forward_set(unit, current_client_id, next_client_id, 1);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(busb-->busb, index %d) ****\n", proc_name, rv, i);
                return rv;
            }
        }
    }
    ...
    bcm_port_t eth_port_out)
{
    ...
    /* Set proc_name */
    char *proc_name;
    proc_name = "dnx_flexe_dpp_busb_l2_clear";
    ...
    rv = bcm_port_force_forward_set(unit, eth_port_in, client_b_start, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(eth_in->client_b_start) ****\n", proc_name, rv);
        return rv;
    }
    for (i = 0; i < nof_clients; i++) {
        ...
        /** Clear loopback */
        rv = bcm_port_enable_set(unit, current_client_id, 0);
        rv |= bcm_port_flexe_flow_clear(unit, current_client_id, 0, current_client_id);
        rv |= bcm_port_enable_set(unit, current_client_id, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in BusB to BusB flow clearing(index %d) ****\n", proc_name, rv, i);
            return rv;
        }

        /** Clear force forwarding */
        if (i == nof_clients - 1) {
            rv = bcm_port_force_forward_set(unit, current_client_id, eth_port_out, 0);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(busb-->eth_out, index %d) ****\n", proc_name, rv, i);
                return rv;
            }
        } else {
            rv = bcm_port_force_forward_set(unit, current_client_id, next_client_id, 0);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(busb-->busb, index %d) ****\n", proc_name, rv, i);
                return rv;
            }
    ...
    /* Remove busB clients */
    for (i = 0; i < nof_clients; i++) {
        rv = dnx_flexe_util_client_remove(unit, client_b_start + i);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    ...
    rv = bcm_port_flexe_group_destroy(unit, flexe_grp);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_destroy ****\n", proc_name, rv);
        return rv;
    }
    ...
    int repeat_times)
{
    ...
    /* Set proc_name */
    char *proc_name;
    proc_name = "dnx_flexe_dpp_L2_busb_remove_add";
    ...
    for (i = 0; i < repeat_times; i++) {
        ...
        /* Remove */
        rv = dnx_flexe_util_L1_datapath_client_to_client(unit, client_id, client_id, 1);
        rv |= dnx_flexe_util_client_remove(unit, client_id);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove ****\n", proc_name, rv);
            return rv;
        }
        sal_usleep(300 * 1000);

        /* Re-add */
        rv = dnx_flexe_util_client_create(unit, client_id, port_type_busB_L2_L3, client_speed, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to create busB client %d ****\n", proc_name, rv, client_id);
            return rv;
        }
        rv = bcm_port_enable_set(unit, client_id, 0);
        rv |= bcm_port_flexe_flow_set(unit, 0, client_id, 0, client_id);
        rv |= bcm_port_enable_set(unit, client_id, 1);
        rv |= bcm_port_force_forward_set(unit, client_id, dst_port, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_force_forward_set on client %d ****\n", proc_name, rv, client_id);
            return rv;
        }
        sal_usleep(300 * 1000);
    ...
    uint32 speed_flag)
{
    ...
    rv = cint_dyn_port_add_flexe_client(unit, client_id, intf_type, channel_id, rx_speed, speed, flags, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in cint_dyn_port_add_flexe_client(client %d) ****\n", proc_name, rv, client_id);
        return rv;
    }
    ...
    bcm_port_t client_id)
{
    ...
    rv = cint_dyn_port_remove_port_full_example(unit, client_id, flags);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in cint_dyn_port_remove_port_full_example(client %d) ****\n", proc_name, rv, client_id);
        return rv;
    }
    ...
    int rx_speed)
{
    ...
    /* Set proc_name */
    char *proc_name;
    proc_name = "dnx_flexe_util_client_speed_g_hao_set";
    ...
    /* Set client speed */
    rv = dnx_flexe_util_client_speed_set(unit, client_id, tx_speed, speed_flag);
    if (rv != BCM_E_NONE) {
    ...
    {
        rv = dnx_tune_egr_ofp_rate_set(unit, client_id, tx_speed*1000, 1, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_tune_egr_ofp_rate_set ****\n", proc_name, rv);
            return rv;
        }
        rv = dnx_tune_scheduler_port_rates_set(unit, client_id, 0, tx_speed*1000, 1, 0);
        if (rv != BCM_E_NONE) {
        ...
        rv = appl_dnx_dynamic_port_speed_set(unit, 0, NULL, client_id);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in appl_dnx_dynamic_port_speed_set ****\n", proc_name, rv);
            return rv;
        }
    }
    return rv;
    ...
    int dst_port)
{
    ...
    if (dst_unit == unit) {
        /** SAR to local port */
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a, client_c, dst_port, 0);
    } else {
        /** SAR to remote port */
        dnx_flexe_util_mod_port_get(dst_unit, dst_port, &mod_gport);
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a, client_c, mod_gport, 0);
    }
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_flexe_sar_dst() ****\n", proc_name, rv);
        return rv;
    }
    ...
    if (dst_unit == unit) {
        /** to local SAR */
        rv = dnx_flexe_util_L1_datapath_src_sar_flexe(dst_unit, dst_port, client_c, client_a, 0);
    } else {
        /** to remote SAR */
        dnx_flexe_util_mod_port_get(unit, client_c, &mod_gport);
        rv = dnx_flexe_util_L1_datapath_src_sar_flexe(dst_unit, dst_port, mod_gport, client_a, 0);
    }
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_src_sar_flexe() ****\n", proc_name, rv);
        return rv;
    }
    ...
    /* Loopback FlexE PHY */
    if (flexe_phy_loopback) {
        rv = bcm_port_flexe_group_get(unit, flexe_grp, 0, 16, &grp_phy_info, &nof_pcs);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_group_get() ****\n", proc_name, rv);
            return rv;
        }
        for (i = 0; i < nof_pcs; i++) {
            rv = bcm_port_loopback_set(unit, grp_phy_info[i].port, BCM_PORT_LOOPBACK_PHY);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_loopback_set() ****\n", proc_name, rv);
                return rv;
            }
    ...
    int dst_port)
{
    ...
    if (dst_unit == unit) {
        /** SAR to local port */
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a, client_c, dst_port, 1);
    } else {
        /** SAR to remote port */
        dnx_flexe_util_mod_port_get(dst_unit, dst_port, &mod_gport);
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a, client_c, mod_gport, 1);
    }
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_flexe_sar_dst() ****\n", proc_name, rv);
        return rv;
    ...
    if (dst_unit == unit) {
        /** to local SAR */
        rv = dnx_flexe_util_L1_datapath_src_sar_flexe(dst_unit, dst_port, client_c, client_a, 1);
    } else {
        /** to remote SAR */
        dnx_flexe_util_mod_port_get(unit, client_c, &mod_gport);
        rv = dnx_flexe_util_L1_datapath_src_sar_flexe(dst_unit, dst_port, mod_gport, client_a, 1);
    }
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_src_sar_flexe() ****\n", proc_name, rv);
        return rv;
    ...
    rv = dnx_flexe_util_client_remove(unit, client_a);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to remove busA client ****\n", proc_name, rv);
        return rv;
    }
    rv = dnx_flexe_util_client_remove(unit, client_c);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to remove busB client ****\n", proc_name, rv);
        return rv;
    }
    ...
    for (i = 0; i < nof_pcs; i++) {
        ...
        rv = bcm_port_loopback_set(unit, flexe_phy, BCM_PORT_LOOPBACK_NONE);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_loopback_set(flexe_phy %d) ****\n", proc_name, rv, flexe_phy);
            return rv;
        }
    ...
    int dst_port)
{
    ...
    rv = dnx_flexe_util_client_create(unit, client_b, port_type_busB, client_speed, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail to create busB client() ****\n", proc_name, rv);
        return rv;
    }
    ...
    rv = dnx_flexe_util_L2_datapath_flexe_to_dst(unit, client_a, client_b, dst_unit, dst_port, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_flexe_to_dst() ****\n", proc_name, rv);
        return rv;
    }

    /* Build L2 datapath: dst_port --> client_b --> client_a */
    rv = dnx_flexe_util_L2_datapath_src_to_flexe(dst_unit, dst_port, unit, client_b, client_a, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_src_to_flexe() ****\n", proc_name, rv);
        return rv;
    }
    ...
    int dst_port)
{
    ...
    rv = dnx_flexe_util_L2_datapath_flexe_to_dst(unit, client_a, client_b, dst_unit, dst_port, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_flexe_to_dst() ****\n", proc_name, rv);
        return rv;
    }
    ...
    rv = dnx_flexe_util_L2_datapath_src_to_flexe(dst_unit, dst_port, unit, client_b, client_a, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_src_to_flexe() ****\n", proc_name, rv);
        return rv;
    }
