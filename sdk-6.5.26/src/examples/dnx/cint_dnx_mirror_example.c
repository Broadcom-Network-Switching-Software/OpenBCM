    ...
    int mirror_qualifier)
{
    ...
    rv = bcm_field_group_context_attach(unit, 0, erspan_info.fg_id, BCM_FIELD_CONTEXT_ID_DEFAULT, &attach_info);
    if (rv != BCM_E_NONE)
    {
...
int mirror_erspan_example_clean_up(int unit){
    ...
    if (sand_utils_is_jr2_system_header_mode(unit)) {
        rv = bcm_field_group_context_detach(unit, erspan_info.fg_id, BCM_FIELD_CONTEXT_ID_DEFAULT);
        if (rv != BCM_E_NONE)
        {
