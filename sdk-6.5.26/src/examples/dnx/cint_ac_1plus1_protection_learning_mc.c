...
int ac_1plus1_protection_learning_mc__set_non_protected_ac(int unit, protection_ac_s *ac_info) {
    ...
    rv = multicast__gport_encap_add(unit, mc_group, vlan_port.port, vlan_port.vlan_port_id, 0);
    if (rv != BCM_E_NONE) {
        printf("Error during multicast__gport_encap_add of gport 0x%08x to mc_group %d, rv - %d\n", vlan_port.vlan_port_id, mc_group, rv);
        return rv;
    }
...
int ac_1plus1_protection_learning_mc__set_protected_in_acs(int unit, ac_1plus1_protection_learning_mc_s *ac_1plus1_protection_learning_mc_info) {
    ...
    if (!protect_is_local) {
        ...
        rv = multicast__gport_encap_add(unit, mc_group, in_ac.port, in_ac.vlan_port_id, 0);
        if (rv != BCM_E_NONE) {
            printf("Error, flooding multicast__gport_encap_add, rv - %d\n", rv);
            return rv;
        }
        ...
        rv = multicast__gport_encap_add(unit, mc_group, in_ac.port, in_ac.vlan_port_id, 0);
        if (rv != BCM_E_NONE) {
            printf("Error, failover multicast__gport_encap_add, rv - %d\n", rv);
            return rv;
        }
...
int ac_1plus1_protection_learning_mc__cleanup(int *unit_ids, int nof_units) {
    ...
    for (i = 0 ; i < nof_units ; i++) {
        ...
        rv = bcm_multicast_group_destroy(unit, mc_flags, g_ac_1plus1_protection_learning_mc.failover_mc_group);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_vlan_port_destroy gport: 0x%x\n", g_ac_1plus1_protection_learning_mc.out_ac.vlan_port_id);
            return rv;
        }

        rv = bcm_multicast_group_destroy(unit, mc_flags, g_ac_1plus1_protection_learning_mc.vsi);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_vlan_port_destroy gport: 0x%x\n", g_ac_1plus1_protection_learning_mc.out_ac.vlan_port_id);
            return rv;
        }
