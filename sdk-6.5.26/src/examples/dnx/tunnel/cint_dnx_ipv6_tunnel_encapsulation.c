 ...
 *        ( ipv6_tunnel_encapsulation(unit, access_port, provider_port, bcmIPv6EncapsulationForwardTypeIPv4, bcmIPv6EncapsulationGRE4) )
 *
 *     2. IPv6 tunnel encapsulation with GRE4 additional header for IPv6 forwarding heaer
 *        ( ipv6_tunnel_encapsulation(unit, access_port, provider_port, bcmIPv6EncapsulationForwardTypeIPv6, bcmIPv6EncapsulationGRE4) )
 *
 *     3. IPv6 tunnel encapsulation with GRE4 additional header for MPLS forwarding heaer
 *        ( ipv6_tunnel_encapsulation(unit, access_port, provider_port, bcmIPv6EncapsulationForwardTypeMPLS, bcmIPv6EncapsulationGRE4) )
 *
 * Configuration:
 *
 * ./bcm.user
 ...
 * int access_port = 201;
 * rv = ipv6_tunnel_encapsulation(unit, access_port, provider_port, bcmIPv6EncapsulationForwardTypeIPv4, bcmIPv6EncapsulationGRE8);
 * print rv;
 *
 */
...
int ipv6_tunnel_encapsulation(int unit, int access_port, int provider_port, int forward_type, int gre_type)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_ipv6_tunnel_info.tunnel_fec_id);
    if (rv != BCM_E_NONE)
    {
    ...
    if(forward_type == bcmIPv6EncapsulationForwardTypeIPv4 || forward_type == bcmIPv6EncapsulationForwardTypeIPv6)
    {
        ...
        rv = l3__egress_only_fec__create(unit, cint_ipv6_tunnel_info.tunnel_fec_id, tunnel_outlif, 0, provider_port, 0, fec_flags2);
        if (rv != BCM_E_NONE)
        {
    ...
    else if(forward_type == bcmIPv6EncapsulationForwardTypeMPLS)
    {
         ...
         */
        int fec_flags2 = (*dnxc_data_get(unit, "l3", "feature", "separate_fwd_rpf_dbs", NULL)) ? BCM_L3_FLAGS2_FWD_ONLY : 0;
        rv = l3__egress_only_fec__create(unit, cint_ipv6_tunnel_info.tunnel_fec_id, mpls_intf_id, 0, provider_port, 0, fec_flags2);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error: l3__egress_only_fec__create failed to execute.\n", proc_name);
            return rv;
