...
int ipv6_tunnel_terminator_config_create(int unit)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error: bcm_tunnel_terminator_config_add failed. \n");
        return rv;
    }
...
int ipv6_tunnel_terminator_create(int unit)
{
    ...
    rv = ipv6_tunnel_terminator_config_create(unit);
    if (rv != BCM_E_NONE)
    {
        printf("Error: ipv6_tunnel_terminator_config_create failed. \n");
        return rv;
    }

    rv = ipv6_tunnel_terminator_p2p_create(unit);
    if (rv != BCM_E_NONE)
    {
        printf("Error: ipv6_tunnel_terminator_p2p_create failed. \n");
        return rv;
    }
...
int ipv6_tunnel_term_basic(int unit, int provider_port, int access_port)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_ipv6_basic_tunnel_info.fec_id);
    if (rv != BCM_E_NONE)
    {
