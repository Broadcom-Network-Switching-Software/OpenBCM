    ...
    int unit)
{
    ...
    rv = bcm_multicast_add(unit, l2_external_cpu_info.mc_group, flags, 2, &replications);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = l3__egress_only_fec__create(unit, l2_external_cpu_info.fec_id_enc, 0, 0, mc_gport, 0, fec_flags2);
    if (rv != BCM_E_NONE)
    {
    ...
    int unit)
{
     ...
     * Create FEC
     */
    rv = l3__egress_only_fec__create(unit, cint_ipv4_tunnel_info.tunnel_fec_id, l3_intf.l3a_intf_id, 0, l2_external_cpu_info.remote_port1, 0, fec_flags2);
    if (rv != BCM_E_NONE)
    {
...
int l2_external_cpu_mc_l2_forwarding(int unit) {
    ...
    replications[1].encap1 = BCM_L3_ITF_VAL_GET(l2_external_cpu_info.l2_remote_arp_intf[2]);
    replications[1].port = gport;
    rv = bcm_multicast_add(unit, mc_group, flags, 2, &replications);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_multicast_add\n");
        return rv;
    ...
    bcm_port_t out_port2)
{
    ...
    int rcpu_port_profile = 3;
    rv = bcm_port_class_set(unit, l2_external_cpu_info.remote_port2, bcmPortClassFieldEgressPacketProcessingPortCs, rcpu_port_profile);
    if (rv != BCM_E_NONE)
    {
    ...
    global_outlifs[2] = 0;
    global_outlifs[3] = 0;
    rv = cint_field_external_cpu_tunnel_ptr_set_epmf(unit, global_outlifs);
    if (rv != BCM_E_NONE)
    {
        printf("Error, cint_field_external_cpu_tunnel_ptr_set_epmf \n");
        print rv;
        return rv;
    }
