 ...
 * cint ../../src/./examples/dnx/tunnel/cint_dnx_mvpn.c
 * cint
...
cint_dnx_mvpn_info_s cint_dnx_mvpn_info =
/* ports :
...
mvpn_info_init (
    ...
    int provider_port2)
{
    cint_dnx_mvpn_info.access_port = access_port;
    cint_dnx_mvpn_info.access_port2 = access_port2;
    cint_dnx_mvpn_info.provider_port = provider_port;
    cint_dnx_mvpn_info.provider_port2 = provider_port2;
}
    ...
    int *tunnel_intf)
{
    ...
    rv = l3__egress_only_encap__create(unit, 0, &tunnel_arp_id,
                                       tunnel_next_hop_mac,
                                       cint_dnx_mvpn_info.provider_vlan);
    if (rv != BCM_E_NONE)
    {
    ...
    int unit)
{
    ...
    rv = in_port_intf_set(unit, cint_dnx_mvpn_info.provider_port, cint_dnx_mvpn_info.provider_vlan);
    if (rv != BCM_E_NONE)
    {
    ...
    dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, cint_dnx_mvpn_info.provider_vlan, 0, BCM_L3_INGRESS_GLOBAL_ROUTE, cint_dnx_mvpn_info.my_mac, vrf);
    rv = dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_utils_l3_eth_rif_create provider_vlan\n");
        return rv;
    }
    printf("%s:\t rif configured for provider_vlan(%d)\n", proc_name, cint_dnx_mvpn_info.provider_vlan);

    /* 2. create RIF for IP routing for IP of the overlay: outRif build the LL: SA and VLAN */
    dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, cint_dnx_mvpn_info.provider_vlan2, 0, 0, cint_dnx_mvpn_info.my_mac, vrf);
    rv = dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_utils_l3_eth_rif_create provider_vlan2\n");
        return rv;
    }
    printf("%s:\t rif configured for provider_vlan2(%d)\n", proc_name, cint_dnx_mvpn_info.provider_vlan2);
    ...
    dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, cint_dnx_mvpn_info.internal_vlan, 0, BCM_L3_INGRESS_GLOBAL_ROUTE, cint_dnx_mvpn_info.my_mac, vrf);
    rv = dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_utils_l3_eth_rif_create internal_vlan\n");
        return rv;
    }
    printf("%s:\t rif configured for internal_vlan(%d)\n", proc_name, cint_dnx_mvpn_info.internal_vlan);

    /* add access port to access vlan. */
    rv = in_port_intf_set(unit, cint_dnx_mvpn_info.access_port, cint_dnx_mvpn_info.access_vlan);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in_port_intf_set \n");
        return rv;
    ...
    dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, cint_dnx_mvpn_info.access_vlan, 0, BCM_L3_INGRESS_GLOBAL_ROUTE, cint_dnx_mvpn_info.my_mac, vrf);
    rv = dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_utils_l3_eth_rif_create access_vlan\n");
        return rv;
    }
    printf("%s:\t rif configured for access_vlan(%d)\n", proc_name, cint_dnx_mvpn_info.access_vlan);
    ...
    int unit)
{
    ...
    rv = bcm_port_control_set(unit, cint_dnx_mvpn_info.recycle_port, bcmPortControlOverlayRecycle, 1);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_port_control_set\n");
        return rv;
    }
    printf("%s:\t recycle port(%d) configured\n", proc_name, cint_dnx_mvpn_info.recycle_port);

    /** Create entry, which will be built the recycle header for the copy going on the 2nd pass */
    rv = create_recycle_entry_with_defaults(unit, &cint_dnx_mvpn_info.recycle_entry_encap_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create_recycle_entry_with_defaults \n");
        return rv;
    }
    printf("%s:\t recycle entry(0x%08X) created \n", proc_name, cint_dnx_mvpn_info.recycle_entry_encap_id);
    ...
    int unit )
{
    ...
    int ipmc_index = cint_dnx_mvpn_info.ipmc_index++;
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID, ipmc_index);
    if (rv != BCM_E_NONE) {
     ...
     *  one copy goes to recycle header encap
     */
    int encap_for_recycle =  BCM_L3_ITF_VAL_GET(cint_dnx_mvpn_info.recycle_entry_encap_id) ;
    int members_of_mc[3]  = {cint_dnx_mvpn_info.provider_port,  cint_dnx_mvpn_info.provider_port2, cint_dnx_mvpn_info.recycle_port};
    int encaps[3]         = {cint_dnx_mvpn_info.provider_vlan2, cint_dnx_mvpn_info.provider_vlan2, encap_for_recycle};
    ...
    rv = mvpn_ipmc_add(unit, mc_ip, 0/*src ip*/, ipmc_index, cint_dnx_mvpn_info.provider_vlan/* inRif */, 0/*flags*/);
    if (rv != BCM_E_NONE) {
    ...
    int unit)
{
    ...
    int members_of_mc[2] = {cint_dnx_mvpn_info.provider_port,  cint_dnx_mvpn_info.provider_port2};
    int encaps[2]        = {cint_dnx_mvpn_info.provider_vlan2, cint_dnx_mvpn_info.provider_vlan2};
    ...
    int unit)
{
    ...
    int members_of_mc[2] = {cint_dnx_mvpn_info.access_port,  cint_dnx_mvpn_info.access_port2};
    int encaps[2]        = {cint_dnx_mvpn_info.access_vlan, cint_dnx_mvpn_info.access_vlan};
    int nof_member_of_mc = 2;
    rv = mvpn_multicast_ingress_set(unit, ipmc_index, members_of_mc, nof_member_of_mc, encaps);
    if (rv != BCM_E_NONE) {
        printf("Error, mvpn_multicast_ingress_set\n");
    ...
    rv = mvpn_ipmc_add(unit, mc_ip, 0/*src ip*/, ipmc_index, cint_dnx_mvpn_info.internal_vlan/* inRif */, 0/*flags*/);
    if (rv != BCM_E_NONE) {
        printf("Error, mvpn_ipmc_add\n");
        return rv;
    }
...
/* Leaf configuration
 * for leaf node, one-pass is enough.
    ...
    int unit)
{
    ...
    int members_of_mc[2] = {cint_dnx_mvpn_info.provider_port,  cint_dnx_mvpn_info.access_port2};
    int encaps[2]        = {(tunnel_encap_id&0x3fffff),    cint_dnx_mvpn_info.provider_vlan2};
    int nof_member_of_mc = 2;
    rv = mvpn_multicast_ingress_set(unit, ipmc_index, members_of_mc, nof_member_of_mc, encaps);
    if (rv != BCM_E_NONE) {
        printf("Error, mvpn_multicast_ingress_set\n");
    ...
    rv = mvpn_ipmc_add(unit, mc_ip, 0/*src ip*/, ipmc_index, cint_dnx_mvpn_info.access_vlan/* inRif */, 0/*flags*/);
    if (rv != BCM_E_NONE) {
        printf("Error, mvpn_ipmc_add\n");
        return rv;
    }
