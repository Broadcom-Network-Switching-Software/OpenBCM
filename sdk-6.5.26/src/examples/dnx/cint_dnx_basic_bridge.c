 ...
 */
int dnx_basic_bridge_init(int unit, dnx_basic_bridge_s *param){
     int rv;
...
int dnx_basic_bridge_main_run(int unit,  int iPort, int oPort ,int c_vid, int vsi, uint8 d_mac_0, uint8 d_mac_1, uint8 d_mac_2, uint8 d_mac_3, uint8 d_mac_4, uint8 d_mac_5){
    ...
    rv = dnx_basic_bridge_run(unit, &param);
...
int dnx_basic_bridge_run(int unit,  dnx_basic_bridge_s *param){
    ...
    rv = dnx_basic_bridge_init(unit, param);
    ...
    /* Set vlan domain:*/
    rv = dnx_vlan_domain_create(unit);
    if (rv != BCM_E_NONE) {
        printf("Error, in dnx_vlan_domain_create\n");
        return rv;
    }

    /* Create VSI:*/
    rv = bcm_vlan_create(unit, g_dnx_basic_bridge.vsi);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vlan_create(vsi=%d)\n", g_dnx_basic_bridge.vsi);
        return rv;
    }
    ...
    rv = bcm_vlan_port_add(unit, g_dnx_basic_bridge.c_vid, pbmp, untag_pbmp);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vlan_port_add\n");
        return rv;
    }

    rv = bcm_port_untagged_vlan_set(unit, g_dnx_basic_bridge.in_port, g_dnx_basic_bridge.c_vid);
    if (rv != BCM_E_NONE) {
    ...
    /* Create Lif */
    rv = dnx_create_lif(unit);
    if (rv != BCM_E_NONE) {
        printf("Error, in dnx_create_lif\n");
        return rv;
    }
...
int dnx_vlan_domain_create(int unit) {
    ...
    /*Input Port */
    rv = bcm_port_class_set(unit, g_dnx_basic_bridge.in_port, bcmPortClassId, g_dnx_basic_bridge.in_port);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_port_class_set(port = %d)\n", g_dnx_basic_bridge.in_port);
        return rv;
    }
    
    /*Output Port*/
    rv = bcm_port_class_set(unit, g_dnx_basic_bridge.out_port, bcmPortClassId, g_dnx_basic_bridge.out_port);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_port_class_set(port = %d)\n", g_dnx_basic_bridge.out_port);
        return rv;
    }
...
int dnx_create_lif(int unit) {
    ...
    rv = bcm_vswitch_port_add(unit, g_dnx_basic_bridge.vsi, g_dnx_basic_bridge.lif);
    if (rv != BCM_E_NONE) {
       printf("Error, in bcm_vswitch_port_add(port = %d)\n", g_dnx_basic_bridge.inlif);
       return rv;
    }
...
int dnx_vlan_port_vsi_create(int unit, int vsi, bcm_port_t port, int vlan_domain, int match_criteria, bcm_vlan_t outer_vid, bcm_gport_t* vlan_port_id)
{
    ...
    rv = bcm_port_class_set(unit, port, bcmPortClassId, vlan_domain);
    if (rv != BCM_E_NONE) {
