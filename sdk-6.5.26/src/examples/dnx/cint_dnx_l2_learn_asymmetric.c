...
int dnx_l2_learn_asymmetric_init(int unit, dnx_l2_learn_asymmetric_s *params) {
    ...
    /* Create a vswitch */
    rv = bcm_vswitch_create_with_id(unit, g_dnx_l2_learn_asymmetric.vsi);
    if (rv != BCM_E_NONE) {
        printf("Error, dnx_l2_learn_asymmetric_init failed during bcm_vswitch_create for vsi %d, rv - %d\n", g_dnx_l2_learn_asymmetric.vsi, rv);
        return rv;
    }
...
int dnx_l2_learn_asymmetric__create_acs(int unit, dnx_l2_learn_asymmetric_s *param) {
    ...
    mc_group = param->vsi;
    rv = multicast__gport_encap_add(unit, mc_group, param->in_ac_opt.port, 0, 0);
    if (rv != BCM_E_NONE) {
    ...
    /* Attach to the flooding MC group */
    rv = multicast__gport_encap_add(unit, mc_group, param->in_ac_translation.port, 0, 0);
    if (rv != BCM_E_NONE) {
        printf("%s(): Error during multicast__gport_encap_add of port 0x%08x to mc_group %d, rv - %d\n", proc_name, param->in_ac_translation.port, mc_group, rv);
        return rv;
    }
    ...
    /* Attach to the flooding MC group */
    rv = multicast__gport_encap_add(unit, mc_group, param->out_ac.port, param->out_ac.gport, 0);
    if (rv != BCM_E_NONE) {
...
int dnx_l2_learn_asymmetric__start_run(int unit, dnx_l2_learn_asymmetric_s *params) {
    ...
    rv = dnx_l2_learn_asymmetric_init(unit, params);
    if (rv != BCM_E_NONE) {
        printf("Error in dnx_l2_learn_asymmetric_init, rv - %d\n", rv);
        return rv;
    }

    /* Configure the participating ACs */
    rv = dnx_l2_learn_asymmetric__create_acs(unit, &g_dnx_l2_learn_asymmetric);
    if (rv != BCM_E_NONE) {
        printf("Error in dnx_l2_learn_asymmetric_init, rv - %d\n", rv);
        return rv;
    }
...
int dnx_l2_learn_asymmetric_with_ports__cleanup(int unit) {
    ...
    rv = bcm_vlan_port_destroy(unit, g_dnx_l2_learn_asymmetric.in_ac_opt.gport);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vlan_port_destroy gport: 0x%08x\n", g_dnx_l2_learn_asymmetric.in_ac_opt.gport);
        return rv;
    }

    rv = bcm_vlan_port_destroy(unit, g_dnx_l2_learn_asymmetric.in_ac_translation.gport);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vlan_port_destroy gport: 0x%08x\n", g_dnx_l2_learn_asymmetric.in_ac_translation.gport);
        return rv;
    }

    rv = bcm_vlan_port_destroy(unit, g_dnx_l2_learn_asymmetric.out_ac.gport);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vlan_port_destroy gport: 0x%08x\n", g_dnx_l2_learn_asymmetric.out_ac.gport);
        return rv;
    }
