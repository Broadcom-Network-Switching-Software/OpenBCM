    ...
    int over_mpls)
{
    ...
    if (payload == ipv4)
    {
        rv = add_ipv4_ipmc(unit, cint_bier_info.dip, 0xFFFFFFFF, cint_bier_info.sip, 0xFFFFFFFF,
                           in_lif, cint_bier_info.vrf, cint_bier_info.native_mc_id, 0, 0);
        if (rv != BCM_E_NONE)
        {
    ...
    {
        rv = add_ipv6_ipmc(unit, cint_bier_info.ipv6_dip, full_mask, cint_bier_info.ipv6_sip, part_mask, 
                           in_lif, cint_bier_info.vrf, compressed_vrf?compressed_sip:0, compressed_vrf?0xFFFF:0,
                           cint_bier_info.native_mc_id, 0, 0, 0);
        if (rv != BCM_E_NONE)
        {
    ...
    int over_mpls)
{
    ...
    if (node_type == BFIR)
    {
         ...
         */
        rv = bcm_multicast_create(unit, flag|static_flag|BCM_MULTICAST_WITH_ID, &cint_bier_info.native_mc_id);
        if (rv != BCM_E_NONE)
        {
        ...
        /*MCDB setting*/
        rv = bcm_multicast_set(unit, cint_bier_info.native_mc_id, flag|static_flag, 1, replications);
        if (rv != BCM_E_NONE)
        {
         ...
         */
        rv = bcm_multicast_create(unit, flag|static_flag|BCM_MULTICAST_WITH_ID|BCM_MULTICAST_BIER_256_GROUP, &cint_bier_info.bier_mc_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_create \n");
            return rv;
    ...
    else if (node_type == BFER)
    {
        ...
        rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|flag|static_flag|BCM_MULTICAST_BIER_256_GROUP, &cint_bier_info.bier_mc_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_create \n");
            return rv;
        ...
        rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|flag|static_flag, &cint_bier_info.native_mc_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_create \n");
            return rv;
        ...
        /*MCDB setting*/
        rv = bcm_multicast_set(unit, cint_bier_info.native_mc_id, flag|static_flag, 2, replications);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_set\n");
            return rv;
    ...
    else 
    {
        ...
        if (mode == 1)
        {
            ...
            rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|BCM_MULTICAST_INGRESS_GROUP|BCM_MULTICAST_BIER_256_GROUP|static_flag, 
 ...
 */
    ...
    int payload)
 {
    ...
    if (rv != BCM_E_NONE)
    {
     ...
     rv = mpls_bier_bift_entry_create(unit, node_type,  payload);
     if (rv != BCM_E_NONE)
     {
         printf("Error, mpls_bier_bift_entry_create\n");
         return rv;
     }

     if (node_type == BFER) {
         rv = mpls_bier_2nd_pass_terminator_create(unit, 1);
         if (rv != BCM_E_NONE)
         {
     ...
     rv = mpls_bier_forward_entry_configuration(unit, node_type, payload, 1);
     if (rv != BCM_E_NONE)
     {
     ...
     rv = mpls_bier_multicast_group_configuration(unit, node_type,payload,mc_mode, 1);
     if (rv != BCM_E_NONE)
     {
    ...
    int mode)
 {
    ...
    if (rv != BCM_E_NONE)
    {
     ...
     rv = mpls_bier_bift_entry_create(unit, BFR, payload);
     if (rv != BCM_E_NONE)
     {
         printf("Error, mpls_bier_bift_entry_create\n");
         return rv;
     ...
     rv = mpls_bier_forward_entry_configuration(unit, BFR, payload, 1);
     if (rv != BCM_E_NONE)
     {
         printf("Error, mpls_bier_forward_entry_configuration\n");
         return rv;
     ...
     rv = mpls_bier_multicast_group_configuration(unit, BFR,payload,mode, 1);
     if (rv != BCM_E_NONE)
     {
         printf("Error, mpls_bier_multicast_group_configuration\n");
         return rv;
    ...
    bcm_flow_initiator_info_t info)
{
    ...
    {
        special_fields.actual_nof_special_fields = 1;
        BCM_IF_ERROR_RETURN(bcm_flow_logical_field_id_get(unit, flow_handle, "QOS_DSCP", &field_id));
    ...
    int mode)
{
    ...
    if (mode !=0)
    {
        rv = bcm_multicast_group_destroy(unit, BCM_MULTICAST_INGRESS_GROUP|BCM_MULTICAST_BIER_256_GROUP|static_flag, cint_bier_info.bier_mc_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_group_destroy \n");
            return rv;
        }
    }
    ...
    if (!no_egress_bier_process)
    {
        rv = bcm_multicast_group_destroy(unit, BCM_MULTICAST_EGRESS_GROUP|BCM_MULTICAST_BIER_256_GROUP, cint_bier_info.bier_mc_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_group_destroy \n");
            return rv;
    ...
    for (i = 0; i < 2; i++)
    {
            ...
            && (mode == 0))
        {
            ...
            mreps[0] = 0;
            mreps[1] = 1; /* Ignore it in single core devices*/
            rv = bcm_fabric_multicast_set(unit, bier_mc_id[i], 0, nof_cores, mreps);
            if (rv != BCM_E_NONE) {
                printf("Error, bcm_fabric_multicast_set \n");
                return rv;
            }
        ...
        if (mode == 1)
        {
            ...
            /*MCDB setting*/
            rv = bcm_multicast_bfr_id_add(unit, bier_mc_id[i], BCM_MULTICAST_INGRESS_GROUP, 3, bfr_id, replications);
            if (rv != BCM_E_NONE)
            {
                printf("Error, bcm_multicast_bfr_id_add\n");
            }
        ...
        if (mode == 2)
        {
            ...
            for (j = 0; j < count_erp; j++) {
                rv = bcm_stk_gport_sysport_get(unit, erp_port[j], &erp_sysport_gport);
                if (rv != BCM_E_NONE) {
                    printf("Error, bcm_stk_gport_sysport_get \n");
                    return rv;
                }
            ...
            rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|BCM_MULTICAST_BIER_256_GROUP|BCM_MULTICAST_INGRESS_GROUP, &bier_mc_id[i]);
            if (rv != BCM_E_NONE)
            {
                printf("Error, bcm_multicast_create \n");
                return rv;
        ...
        if (!no_egress_bier_process)
        {
             ...
             * Egress MC
             */
            rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|BCM_MULTICAST_EGRESS_GROUP|BCM_MULTICAST_BIER_256_GROUP, &bier_mc_id[i]);
            if (rv != BCM_E_NONE)
            {
                printf("Error, bcm_multicast_create \n");
                return rv;
            ...
            /*MCDB setting*/
            rv = bcm_multicast_bfr_id_add(unit, bier_mc_id[i], BCM_MULTICAST_EGRESS_GROUP, 3, bfr_id, replications);
            if (rv != BCM_E_NONE)
            {
                printf("Error, bcm_multicast_bfr_id_add\n");
            }
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, fec_direction, &fec_ids[0]);
    if (rv != BCM_E_NONE)
    {
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        rv = l3__egress_only_fec__create(unit, fec_ids[i], 0, 0, mc_gport, 0, flags2);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = l3__ecmp_create(unit, ecmp_intf, 2, fec_ids, BCM_L3_WITH_ID, BCM_L3_ECMP_DYNAMIC_MODE_DISABLED,&ecmp,0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in l3__ecmp_create\n");
        return rv;
    }
