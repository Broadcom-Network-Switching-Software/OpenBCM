...
int ipmc_l3_mcast_l2_ivl_example(int unit, int in_port, int out_port1, int out_port2, int out_port3, bcm_vlan_t vsi, bcm_vlan_t vid1, bcm_vlan_t vid2)
{
    ...
    for (mc_idx = 0; mc_idx < nof_mc_groups; mc_idx++)
    {
        ...
        /* KAPS */
        rv = add_ipv4_ipmc_bridge(unit, dip_addr, dip_mask, sip_addr, sip_mask, kaps_vid, vsi, ipmc_group[mc_idx], 0, 0);
        if (rv != BCM_E_NONE)
        {
        ...
        /* LEM */
        rv = add_ipv4_ipmc_bridge(unit, dip_addr, dip_mask, sip_addr, 0 /*sip_mask*/, lem_vid, vsi, ipmc_group[mc_idx], 0, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, add_ipv4_ipmc_bridge\n");
            return rv;
