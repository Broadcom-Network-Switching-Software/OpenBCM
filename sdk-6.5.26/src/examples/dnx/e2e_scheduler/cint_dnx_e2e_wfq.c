...
int e2e_wfq_start_run(int unit, int port, int mode)
{
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &g_e2e_wfq_gports.gport_ucast_scheduler);
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &g_e2e_wfq_gports.gport_ucast_scheduler_fq0);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_add scheduler failed $rv\n");
        return rv;
    }
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &g_e2e_wfq_gports.gport_ucast_scheduler_fq1);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_add scheduler failed $rv\n");
        return rv;
    }
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &g_e2e_wfq_gports.gport_ucast_scheduler_fq2);
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &g_e2e_wfq_gports.gport_ucast_scheduler_fq3);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_add scheduler failed $rv\n");
        return rv;
    }
    ...
    rv = bcm_cosq_ingress_queue_bundle_gport_add(unit, &conf, &g_e2e_wfq_gports.gport_ucast_queue_group0);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_cosq_ingress_queue_bundle_gport_add(unit, &conf, &g_e2e_wfq_gports.gport_ucast_queue_group1);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_ingress_queue_bundle_gport_add failed\n");
        return rv;
    }
...
int e2e_wfq_verify(int unit)
{
    ...
    rc = bcm_cosq_gport_handle_get(unit, bcmCosqGportTypeSched, &gport_info);
    if (rc != BCM_E_NONE) {
        printf("bcm_dnx_cosq_gport_handle_get scheduler failed $rv\n");
        return(rc);
    }
    g_e2e_wfq_gports.e2e_gport = gport_info.out_gport;
    rc = bcm_cosq_scheduler_gport_get(unit, 0, g_e2e_wfq_gports.e2e_gport, &params_get);
    if (rc != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_get on e2e Sched (0x%x) failed, error (%d)\n", g_e2e_wfq_gports.e2e_gport, rc);
        return(rc);
    }
    ...
    rc = bcm_cosq_scheduler_gport_get(unit, 0, g_e2e_wfq_gports.gport_ucast_scheduler, &params_get);
    if (rc != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_get on Sched (0x%x) failed, error (%d)\n", g_e2e_wfq_gports.gport_ucast_scheduler, rc);
        return(rc);
    }
    ...
    for (nbr_fq_sched = 0; nbr_fq_sched < C3_NBR_FQ_SCHEDULERS; nbr_fq_sched++) {
        ...
        /* retreive type of scheduling element */
        rc = bcm_cosq_scheduler_gport_get(unit, 0, gport_scheduler_fq, &params_get);
        if (rc != BCM_E_NONE) {
    ...
    for (nbr_queue_groups = 0; nbr_queue_groups < C3_NBR_QUEUE_GROUPS; nbr_queue_groups++) {
        ...
        rc = bcm_cosq_voq_connector_gport_get(unit, gport_ucast_voq_connector_group, &config_get);
        if (rc != BCM_E_NONE) {
            printf("bcm_cosq_voq_connector_gport_get on Connector (0x%x) failed, error (%d)\n", gport_ucast_voq_connector_group, rc);
            return(rc);
        }
