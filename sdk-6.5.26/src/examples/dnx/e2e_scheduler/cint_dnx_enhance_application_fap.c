...
int start_enhance_application(int unit, int destination_local_port)
{
    ...
    rv = create_fmq_queue(unit,base_queues[0],num_cos_level, speed,my_modid,hr_classes[0]);
    if (rv != BCM_E_NONE) {
        printf("rv is: $rv \n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    ...
    rv = create_fmq_queue(unit,base_queues[1],num_cos_level, speed,my_modid,hr_classes[0]);
    if (rv != BCM_E_NONE) {
        printf("rv is: $rv \n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    ...
    rv = create_fmq_queue(unit,base_queues[2],num_cos_level, speed,my_modid,hr_classes[1]);
    if (rv != BCM_E_NONE) {
        printf("rv is: $rv \n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    ...
    rv = create_fq(unit, destination_local_port, core, &mg1_fq);
    if (rv != BCM_E_NONE) {
        printf("rv is: $rv \n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    ...
    rv = create_fq(unit, destination_local_port, core, &mg2_fq);
    if (rv != BCM_E_NONE) {
        printf("rv is: $rv \n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    ...
    params.core = core;
    params.shared_order = bcmCosqSchedulerGportSharedOrderNone;
    rv = bcm_cosq_scheduler_gport_add(unit, BCM_COSQ_SCHEDULER_GPORT_REPLACE, params, &hr_classes[0]);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
