...
int C1_DEFAULT_E2E_SCHED_TYPE = bcmCosqSchedulerGportTypeHrSingleWfq;
...
int example_setup(void)
{
    ...
    for (idx=0; idx<C1_NUM_PORTS; idx++) {
        ...
        rv = bcm_cosq_scheduler_gport_add(unit, BCM_COSQ_SCHEDULER_GPORT_WITH_ID, params, &gport_ucast_scheduler[idx]);
        if (rv != BCM_E_NONE) {
...
int example_verify(int unit)
{
    ...
    for (idx = 0; idx < C1_NUM_PORTS; idx++) {
        ...
        rc = bcm_cosq_scheduler_gport_get(unit, 0, gport_ucast_scheduler[idx], &params_get);
        if (rc != BCM_E_NONE) {
            printf("bcm_cosq_scheduler_gport_get on e2e Sched (0x%x) failed, error (%d)\n", e2e_gport[idx], rc);
            return(rc);
        }
        if (params_get.type != C1_DEFAULT_E2E_SCHED_TYPE) {
            printf("gport get mismatch on E2E Sched (0x%x) expected type(%d) retreived type (%d)\n", e2e_gport[idx], C1_DEFAULT_E2E_SCHED_TYPE, params_get.type);
        }
        ...
        rc = bcm_cosq_voq_connector_gport_get(unit, gport_ucast_voq_connector_group[idx], &config_get);
        if (rc != BCM_E_NONE) {
            printf("bcm_cosq_scheduler_gport_get on Connector (0x%x) failed, error (%d)\n", gport_ucast_voq_connector_group[idx], rc);
            return(rc);
        }
