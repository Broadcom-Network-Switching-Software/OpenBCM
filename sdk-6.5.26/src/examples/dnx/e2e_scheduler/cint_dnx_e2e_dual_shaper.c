...
int e2e_dual_shaper_setup(int port_idx)
{
    ...
    rv = bcm_port_get(c6_unit, port, &dummy_flags, &interface_info, &mapping_info);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_cosq_gport_handle_get(c6_unit, bcmCosqGportTypeSched, &gport_info);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_cosq_scheduler_gport_add(c6_unit, BCM_COSQ_SCHEDULER_GPORT_REPLACE, params, &c6_e2e_gport[idx]);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_cosq_scheduler_gport_add(c6_unit, default_flags, params, &c6_gport_ucast_scheduler_fq0[idx]);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_add scheduler failed $rv\n");
        return(rv);
    }
    ...
    rv = bcm_cosq_scheduler_gport_add(c6_unit, default_flags, params, &c6_gport_ucast_scheduler_fq1[idx]);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_add scheduler failed $rv\n");
        return(rv);
    }
    ...
    rv = bcm_cosq_scheduler_gport_add(c6_unit, default_flags, params, &c6_gport_ucast_scheduler[idx]);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_add scheduler failed $rv\n");
        return(rv);
    }
    ...
    rv = bcm_port_resource_get(c6_unit, port, &resource);
    if (rv != BCM_E_NONE) {
        printf("bcm_port_resource_get failed $rv\n");
        return(rv);
    }
    rv = appl_dnx_e2e_voq_profiles_get(c6_unit, resource.speed, &delay_tolerance_level, &rate_class);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_cosq_ingress_queue_bundle_gport_add(c6_unit, &conf, &c6_gport_ucast_queue_group[idx]);
    if (rv != BCM_E_NONE) {
...
int e2e_dual_shaper_verify(int port_idx)
{
    ...
    rc = bcm_cosq_gport_handle_get(c6_unit, bcmCosqGportTypeSched, &gport_info);
    if (rc != BCM_E_NONE) {
        printf("bcm_dnx_cosq_gport_handle_get scheduler failed $rv\n");
        return(rc);
    }
    c6_e2e_gport[idx] = gport_info.out_gport;
    rc = bcm_cosq_scheduler_gport_get(c6_unit, 0, c6_e2e_gport[idx], &params_get);
    if (rc != BCM_E_NONE) {
    ...
    for (nbr_fq_sched = 0; nbr_fq_sched < C6_NBR_FQ_SCHEDULERS; nbr_fq_sched++) {
        ...
        rc = bcm_cosq_scheduler_gport_get(c6_unit, 0, gport_scheduler_fq, &params_get);
        if (rc != BCM_E_NONE) {
    ...
    rc = bcm_cosq_scheduler_gport_get(c6_unit, 0, c6_gport_ucast_scheduler[idx], &params_get);
    if (rc != BCM_E_NONE) {
