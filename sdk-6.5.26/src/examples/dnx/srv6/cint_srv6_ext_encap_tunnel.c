        ...
        int is_uniform)
{
    ...
    if(!is_t_insert)
    {
        ...
        rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_IP_TUNNEL_FEC_ID], 0 , global_lif_encoded, gport, 0, fec_flags2,
        ...
        rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_IP_TUNNEL_L2VPN_FEC_ID], 0 , global_lif_encoded, gport, 0, fec_flags2,
                                               0, 0,&encoded_fec_ip_tunnel);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, create egress object FEC_1 only\n",proc_name);
    ...
    rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FIRST_PASS_FEC_ID], 0 , global_lif_encoded, gport, BCM_L3_2ND_HIERARCHY, fec_flags2,
                                           0, 0,&encoded_fec_srh);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_RCH_1_2_PASS_FEC_ID], 0 , global_lif_encoded, gport, BCM_L3_3RD_HIERARCHY, fec_flags2,
                                           0, 0,&encoded_fec_rch);
    if (rv != BCM_E_NONE)
    {
   ...
   rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_SECOND_PASS_FEC_ID], 0 , global_lif_encoded, gport, BCM_L3_2ND_HIERARCHY, fec_flags2,
                                          0, 0,&encoded_fec_srh);
   if (rv != BCM_E_NONE)
   {
       printf("%s(): Error, create egress object FEC_2 only\n",proc_name);
   ...
   BCM_GPORT_LOCAL_SET(gport, rch_port);
   rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_RCH_2_3_PASS_FEC_ID], 0 , global_lif_encoded, gport, BCM_L3_3RD_HIERARCHY, fec_flags2,
                                          0, 0,&encoded_fec_rch);
   if (rv != BCM_E_NONE)
   {
       printf("%s(): Error, create egress object FEC_3 only\n",proc_name);
   ...
   BCM_GPORT_LOCAL_SET(gport, out_port);
   rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_THIRD_PASS_FEC_ID], 0 , global_lif_encoded, gport, BCM_L3_2ND_HIERARCHY, fec_flags2,
                                          0, 0,&encoded_fec_srh);
   if (rv != BCM_E_NONE)
   {
       printf("%s(): Error, create egress object FEC_2 only\n",proc_name);
    ...
    if(!is_t_insert)
    {
        ...
        fec = cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_IP_TUNNEL_FEC_ID];
    }
    else
    {
        BCM_GPORT_FORWARD_PORT_SET(gport, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FIRST_PASS_FEC_ID]);
        fec = cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FIRST_PASS_FEC_ID];
    }
    ...
    {
        /** L3 Routing */
        rv = add_route_ipv4(unit, cint_srv6_tunnel_info.route_ipv4_dip, cint_srv6_tunnel_info.route_ipv4_mask, cint_srv6_tunnel_info.vrf_in[INGRESS], fec);
        {
            printf("%s(): Error, in function add_route_ipv4() VRF:%d\n",proc_name, cint_srv6_tunnel_info.vrf_in[INGRESS]);
        }

        rv = add_route_ipv6(unit, cint_srv6_tunnel_info.route_ipv6_dip, cint_srv6_tunnel_info.route_ipv6_mask, cint_srv6_tunnel_info.vrf_in[INGRESS], fec);
    ...
    /*
     *  Add cross connect entry for End.B6
     *  End.B6 is a local SID behaviour, no need to look up next SID
     */
    if(is_t_insert)
    {
        rv = srv6_endpoint_tunnel_cross_connect(unit, 0, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FIRST_PASS_FEC_ID], 0);
        if (rv != BCM_E_NONE)
        {
...
/*
 * This is an example to UC nad MC share the same SRv6 tunnel in multiple passes.
 * For UC, it works in 2 passes; for MC, it works in 3 passes;
 * Configuring SRv6 extended encap for Both UC and MC traffic. UC and MC share the same ipv6 and SRv6 encap objects.
 *   MC traffic:
 *      1st pass:
 *        L2 MC -> replication index
 *        PPMC > (IP-tunnel outlif -> DIP outlif) + RCH EE-uncollapse with FEC
 *      2nd pass(extend t.insert):
 *        RCH EE-uncollapse FEC(1st hierarchy) -> (RCH_ENCAP_w_FEC + FEC ->(SRH -> SID_1_2 -> SID_1_1 + RCH_PORT))
 *      3rd pass(extend t.insert):
 *        RCH EE FEC(1st hierarchy) -> (SRH -> SID_2_3 -> SID_2_2 -> SID_2_1 -> SID_2_0 + FEC ->(ARP_AC + OUT_PORT))
 *
 *   UC traffic:
 *      1st pass:
 *        FWD > (IP-tunnel outlif -> DIP outlif) + FEC(1st hierarchy) -> (RCH_ENCAP_w_FEC + FEC -> (SRH -> SID_1_2 -> SID_1_1 + RCH_PORT))
 *      2rd pass(extend encap):
 *        RCH EE FEC(1st hierarchy) -> (SRH -> SID_2_3 -> SID_2_2 -> SID_2_1 -> SID_2_0 + ARP_AC)
 *
 * Note the differences about RCH_ENCAP in MC and UC case:
 *   MC: phase 7, no needs to push out-lif0 to stack after recycle;
 *   UC: phase 8, push out-lif 0 to stack after recycle.
 *
 */
int
srv6_ingress_l2vpn_mc_ext_encap_3_pass(
        int unit,
        int in_port,
        int ce_port,
        int pe_port,
        int rch_port_ee_uncollapse,
        int rch_port_ee,
        int nof_sids_1st_pass,
        int nof_sids_2nd_pass)
{
    int rv;
    char *proc_name = "srv6_ingress_l2vpn_mc_ext_encap_1st_and_2nd_pass";
    int first_fec_in_hier0, first_fec_in_hier1;
    bcm_gport_t gport;
    bcm_gport_t ce_ac_encap_id;
    int reduced_mode = 0;
    int i;
    ...
    if(nof_sids_1st_pass > 2 || nof_sids_1st_pass <= 0)
    {
        printf("Error, nof_sids in the RCYed first pass is limited to 2, %d\n", nof_sids_1st_pass);
        return BCM_E_PARAM;
    }
    if(nof_sids_2nd_pass > 4 || nof_sids_2nd_pass <= 0)
    {
        printf("Error, nof_sids in the RCYed last pass is limited to 4, %d\n", nof_sids_2nd_pass);
        return BCM_E_PARAM;
    }
     ...
     * 5. Configure recycle port
     */
    rv = bcm_port_control_set(unit, rch_port_ee_uncollapse, bcmPortControlRecycleApp, bcmPortControlRecycleAppExtendedEncapUncollapse);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, bcm_port_control_set rch_port %d\n",proc_name,rch_port_ee_uncollapse);
        return rv;
    }

    /** Set port class in order for PMF to crop RCH ad IRPP */
    rv = bcm_port_class_set(unit,rch_port_ee_uncollapse, bcmPortClassFieldIngressPMF3TrafficManagementPortCs,4);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, bcm_port_class_set \n",proc_name);
        return rv;
    ...
    rv = bcm_port_control_set(unit,rch_port_ee, bcmPortControlRecycleApp, bcmPortControlRecycleAppExtendedEncap);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, bcm_port_control_set rch_port %d\n",proc_name,rch_port_ee_uncollapse);
        return rv;
    ...
    /** Set port class in order for PMF Keep RCH to Egress, and set parsing start type to RCY */
    rv = bcm_port_class_set(unit,rch_port_ee,bcmPortClassFieldIngressPMF3TrafficManagementPortCs,5);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_port_class_set \n");
        return rv;
    ...
    /* Update FEC ids */
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 1, 0, &first_fec_in_hier1);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range 0\n");
        return rv;
    ...
    BCM_GPORT_LOCAL_SET(gport, pe_port);
    arp_id = cint_srv6_tunnel_info.tunnel_arp_id[INGRESS];
    ...
    arp_fec_id = first_fec_in_hier1 + 0x100;
    /* FEC */
    rv = l3__egress_only_fec__create_inner(unit, arp_fec_id, 0 , arp_id, gport, BCM_L3_2ND_HIERARCHY, 0, 0, 0,&encoded_fec);
    if (rv != BCM_E_NONE)
    {
    ...
    int global_lif_encoded;
    int encoded_fec_ip_tunnel;
    int encoded_fec_srh_3rd, encoded_fec_srh_2nd;

    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &first_fec_in_hier0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range 0\n");
        return rv;
    ...
    /*
     * Create SRH 3rd pass FEC for MC/2nd pass FEC for UC
     */
    cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_THIRD_PASS_FEC_ID] = first_fec_in_hier0++;

    BCM_L3_ITF_SET(global_lif_encoded, BCM_L3_ITF_TYPE_LIF, cint_srv6_tunnel_info.tunnel_global_lif[INGRESS_SRH_3RD_PASS_GLOBAL_LIF]);
    BCM_GPORT_FORWARD_PORT_SET(gport, ecmp_id);
    rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_THIRD_PASS_FEC_ID], 0 , global_lif_encoded, gport, 0, 0,
                                           0, 0,&encoded_fec_srh_3rd);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC_2 only\n",proc_name);
        return rv;
    ...
    /* Create 2nd_to_3rd pass RCH EE(extended encap), destination is 3rd pass IP tunnel FEC */
    bcm_l2_egress_t recycle_entry_2_to_3;
    bcm_l2_egress_t_init(&recycle_entry_2_to_3);
    recycle_entry_2_to_3.flags = BCM_L2_EGRESS_RECYCLE_HEADER | BCM_L2_EGRESS_DEST_PORT;
    recycle_entry_2_to_3.dest_encap_id = 0;
    BCM_GPORT_FORWARD_PORT_SET(recycle_entry_2_to_3.dest_port, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_THIRD_PASS_FEC_ID]);
    recycle_entry_2_to_3.recycle_app = bcmL2EgressRecycleAppExtendedEncap;
    recycle_entry_2_to_3.encap_access = bcmEncapAccessAc;
    rv = bcm_l2_egress_create(unit, &recycle_entry_2_to_3);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_l2_egress_create \n");
    ...
    /*
     * Create SRH 2nd pass FEC for MC
     */

    /* 2nd hierarchy FEC for the 1st -> 2nd pass rch_port and the 2nd encap*/
    int rch_encap_fec_id = first_fec_in_hier1 + 0x101;
    BCM_GPORT_LOCAL_SET(gport, rch_port_ee);
    BCM_L3_ITF_SET(global_lif_encoded, BCM_L3_ITF_TYPE_LIF, cint_srv6_tunnel_info.tunnel_global_lif[INGRESS_SRH_2ND_PASS_GLOBAL_LIF]);
    rv = l3__egress_only_fec__create_inner(unit, rch_encap_fec_id, 0 , global_lif_encoded, gport, BCM_L3_2ND_HIERARCHY, 0, 0, 0,&encoded_fec);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object only, fec\n");
        return rv;
    ...
    /* 1st hierarchy FEC for the 1st -> 2nd pass rch_encap and the 2nd encap*/
    cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_SECOND_PASS_FEC_ID] = first_fec_in_hier0++;
    BCM_GPORT_FORWARD_PORT_SET(gport, rch_encap_fec_id);
    rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_SECOND_PASS_FEC_ID], 0 , recycle_entry_2_to_3.encap_id, gport, 0, 0,
                                           0, 0,&encoded_fec_srh_2nd);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC_2 only\n",proc_name);
        return rv;
    ...
    BCM_GPORT_FORWARD_PORT_SET(recycle_entry_1_to_2.dest_port, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_SECOND_PASS_FEC_ID]);
    recycle_entry_1_to_2.recycle_app = bcmL2EgressRecycleAppExtendedEncap;
    recycle_entry_1_to_2.encap_access = bcmEncapAccessAc;
    rv = bcm_l2_egress_create(unit, &recycle_entry_1_to_2);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_l2_egress_create \n");
    ...
    /*
     * Create SRH 1st pass FEC for UC (Differ from the 2nd MC FEC because of the RCH encap in different phase)
     */
    /* Create 1st_to_2nd pass RCH EE(extended encap), destination is 2nd pass IP tunnel FEC, using the ip tunnel outlif in the 2nd pass */
    bcm_l2_egress_t_init(&recycle_entry_2_to_3);
    recycle_entry_2_to_3.flags = BCM_L2_EGRESS_RECYCLE_HEADER | BCM_L2_EGRESS_DEST_PORT | BCM_L2_EGRESS_EXTENDED_COPY_DEST_ENCAP;
    recycle_entry_2_to_3.dest_encap_id = 0;
    BCM_GPORT_FORWARD_PORT_SET(recycle_entry_2_to_3.dest_port, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_THIRD_PASS_FEC_ID]);
    recycle_entry_2_to_3.recycle_app = bcmL2EgressRecycleAppExtendedEncap;
    recycle_entry_2_to_3.encap_access = bcmEncapAccessArp;
    rv = bcm_l2_egress_create(unit, &recycle_entry_2_to_3);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_l2_egress_create \n");
    ...
    /* 1st hierarchy FEC for the 1st -> 2nd pass rch_encap and the 2nd encap*/
    int fec_id_2nd_pass_uc = first_fec_in_hier0++;

    BCM_L3_ITF_SET(global_lif_encoded, BCM_L3_ITF_TYPE_LIF, cint_srv6_tunnel_info.tunnel_global_lif[INGRESS_SRH_2ND_PASS_GLOBAL_LIF]);
    BCM_GPORT_FORWARD_PORT_SET(gport, rch_encap_fec_id);
    rv = l3__egress_only_fec__create_inner(unit, fec_id_2nd_pass_uc, 0 , recycle_entry_2_to_3.encap_id, gport, 0, 0,
                                           0, 0,&fec_id_2nd_pass_uc);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC_2 only\n",proc_name);
        return rv;
    ...
    tunnel_init_set.flags = BCM_TUNNEL_WITH_ID;
    tunnel_init_set.ttl = 128;
    /**We want ipv6-tunnel entry in phase 1 and the dip in phase 7. Default encap_access is enough.*/
    /**tunnel_init_set.encap_access = bcmEncapAccessTunnel4;*/
    tunnel_init_set.egress_qos_model.egress_ttl = bcmQosEgressModelPipeMyNameSpace;
    BCM_GPORT_TUNNEL_ID_SET(tunnel_init_set.tunnel_id, cint_srv6_tunnel_info.tunnel_global_lif[INGRESS_IP_TUNNEL_L2VPN_GLOBAL_LIF]);
    if(estimate_encap_size_required)
    ...
    bcm_srv6_sid_initiator_info_t sid_info;
    bcm_srv6_sid_initiator_info_t_init(&sid_info);
    int next_sid_tunnel_id, next_sid_tunnel_id_2nd, next_sid_tunnel_id_1st;
    int sid_idx;
    int sid_location;
    int sid_start_idx = SID1; /* SID0 is brought by IPv6 DIP */
    ...
    printf("Creating 2nd pass SRH tunnel, nof sid = %d\n", nof_sids_2nd_pass);
    int sid_count = 0;
    for (sid_count = 0; sid_count < nof_sids_2nd_pass; sid_count++)
    {
        sid_idx = sid_start_idx - sid_count;
        bcm_srv6_sid_initiator_info_t_init(&sid_info);
    ...
    next_sid_tunnel_id_2nd = next_sid_tunnel_id;


    printf("Creating 1st pass SRH tunnel, nof sid = %d\n", nof_sids_1st_pass);
    sid_count = 0;
    next_sid_tunnel_id = 0;
    for (sid_count = 0; sid_count < nof_sids_1st_pass; sid_count++)
    {
        sid_idx = sid_start_idx - sid_count - nof_sids_2nd_pass;
        bcm_srv6_sid_initiator_info_t_init(&sid_info);
    ...
    next_sid_tunnel_id_1st = next_sid_tunnel_id;
    ...
    /** set number of SIDs*/
    srh_base_info.nof_sids = nof_sids_1st_pass + nof_sids_2nd_pass;
    BCM_GPORT_TUNNEL_TO_L3_ITF_LIF(srh_base_info.next_encap_id, next_sid_tunnel_id_2nd);
    ...
    if(estimate_encap_size_required)
    {
         ...
         * This is the last pass, estimation is:
         * t.insert: 
         * nof_sids*16 + 16(VPN_SID) + 8(SRH) + 40(IPV6) + ARP(at least 14B) - 30(RCH) - (40 + 8 + 16 for t.insert)
         * encap:
         * nof_sids*16 + 16(VPN_SID) + 8(SRH) + 40(IPV6) + ARP(at least 14B) - 30(RCH)
         */
        srh_base_info.estimated_encap_size = (nof_sids_2nd_pass - reduced_mode)*16 + 16 + 8 + 40 + 14 - 30 - (40 + 8 + 16);
    }

    /** Create the SRH pointing to the 2nd pass SID list */
    rv = bcm_srv6_srh_base_initiator_create(unit, &srh_base_info);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_dnx_srv6_srh_base_initiator_create 2nd pass \n");
    ...
    /** Create the SRH pointing to the 1st pass SID list */

    /** convert SRH global-LIF id to GPORT */
    BCM_GPORT_TUNNEL_ID_SET(srh_base_info.tunnel_id, cint_srv6_tunnel_info.tunnel_global_lif[INGRESS_SRH_2ND_PASS_GLOBAL_LIF]);
    
    /** set number of SIDs*/
    BCM_GPORT_TUNNEL_TO_L3_ITF_LIF(srh_base_info.next_encap_id, next_sid_tunnel_id_1st);
    ...
    if(estimate_encap_size_required)
    {
         ...
         * This is the last pass, estimation is:
         * t.insert :
         * nof_sids*16 + 16(DIP) + 8(SRH) + 40(IPv6) + 30(RCH) - (40 + 30)
         * encap :
         * nof_sids*16 + 16(DIP) + 30(RCH)
         */
        srh_base_info.estimated_encap_size = (nof_sids_1st_pass - reduced_mode)*16 + 16;
    }
    rv = bcm_srv6_srh_base_initiator_create(unit, &srh_base_info);
    if (rv != BCM_E_NONE)
    {
    ...
    /* 1st copy to RCH EE, and finally to remote PE */
    bcm_if_t rep_idx = 0x300100;

    rep_array.port = rch_port_ee_uncollapse;
    rep_array.encap1 = rep_idx;
    ...
    /*
     * 10. Add L2 unicast entry
     */
     bcm_l2_addr_t l2_addr_ing;

     bcm_l2_addr_t_init(&l2_addr_ing, cint_srv6_tunnel_info.l2_fwd_mac, cint_srv6_tunnel_info.eth_rif_intf_in[INGRESS]); /* configuring DA address and VSI as keys in the MACT */
     l2_addr_ing.flags = BCM_L2_STATIC;      /* static entry */
     BCM_GPORT_FORWARD_PORT_SET(gport, fec_id_2nd_pass_uc);
     l2_addr_ing.port = gport;               /* destination of 1st FEC */
     /** encap id of IPv6 tunnel */
     BCM_FORWARD_ENCAP_ID_VAL_SET(l2_addr_ing.encap_id, BCM_FORWARD_ENCAP_ID_TYPE_OUTLIF, BCM_FORWARD_ENCAP_ID_OUTLIF_USAGE_GENERAL, cint_srv6_tunnel_info.tunnel_global_lif[INGRESS_IP_TUNNEL_L2VPN_GLOBAL_LIF]);
