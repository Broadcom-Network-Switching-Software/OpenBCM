    ...
    bcm_field_context_t epmf_context_id)
{
    ...
    if (!is_epmf_ace_support)
    {
        /**create pre-select*/
        if (!BCM_GPORT_IS_TRUNK (egress_port)) {
            rv = cint_reflector_l3_internal_fg_presel_create(unit, egress_port, &context_id);
            if (rv != BCM_E_NONE)
            {
            ...
            for (member_index = 0; member_index < trunk_member_count; member_index++) {
                rv = cint_reflector_l3_internal_fg_presel_create(unit, out_trunk_member_array[member_index].gport, &context_id);
                if (rv != BCM_E_NONE)
                {
                    printf("cint_reflector_l3_internal_fg_presel_create failed\n");
                    return rv;
