    ...
    bcm_field_context_t epmf_context_id)
{
    ...
    if (!BCM_GPORT_IS_TRUNK (out_port)) {
        ...
        for (member_index = 0; member_index < trunk_member_count; member_index++) {
            rv = cint_reflector_l2_internal_fp_entry_add(unit, fg_id, &fg_info, in_port, trunk_member_array[member_index].gport, rcy_port, ace_entry_handle, encap_id);
            if (rv != BCM_E_NONE)
            {
                printf("cint_reflector_l2_internal_fp_entry_add failed\n");
                return rv;
        ...
        bcm_field_context_t epmf_context_id)
{
    ...
    else
    {
        for (member_index = 0; member_index < trunk_member_count; member_index++)
        {
            rv = cint_reflector_l2_internal_uc_fp_entry_add(unit, fg_id, &fg_info, in_port, trunk_member_array[member_index].gport, rcy_port, ace_entry_handle, is_ip_swap, is_udp_swap, reflector_encap_id);
            if (rv != BCM_E_NONE)
            {
                printf("cint_reflector_l2_internal_fp_entry_add failed\n");
                return rv;
