    ...
    ip_compatible_mc_s * param)
{
    ...
    {
        rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID | mc_flags, &g_ip_compatible_mc.mcid[temp_j]);
        if (rv != BCM_E_NONE)
        {
        ...
        for (temp_i = 0; temp_i < NUM_OF_OUT_PORTS; temp_i++)
        {
            ...
            rv = bcm_multicast_add(unit, g_ip_compatible_mc.mcid[temp_j], mc_flags, g_ip_compatible_mc.nof_replications,
                                   &rep_array);
            if (rv != BCM_E_NONE)
            {
    ...
    rv = bcm_multicast_add(unit, g_ip_compatible_mc.mcid[0], mc_flags, g_ip_compatible_mc.nof_replications, &rep_array);
    if (rv != BCM_E_NONE)
    {
    ...
    if (ivl_feature_support == 1)
    {
        ...
        rv = bcm_vlan_control_vlan_set(unit, g_ip_compatible_mc.vsi[3], control);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_vlan_control_vlan_set in vsi= %d\n", g_ip_compatible_mc.vsi[3]);
            return rv;
        }
        ...
        rv = bcm_vlan_control_vlan_set(unit, g_ip_compatible_mc.vsi[1], control);

        if (rv != BCM_E_NONE)
       {
            printf("Error, in bcm_vlan_control_vlan_set in vsi= %d\n", g_ip_compatible_mc.vsi[1]);
            return rv;
       }
    }
     ...
     */
    rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID | mc_flags, &g_ip_compatible_mc.mcid[2]);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_multicast_create\n");
        return rv;
    ...
    for (temp_i = 0; temp_i < NUM_OF_OUT_PORTS - 1; temp_i++)
    {
        ...
        rv = bcm_multicast_add(unit, g_ip_compatible_mc.mcid[2], mc_flags, g_ip_compatible_mc.nof_replications,
                               &rep_array);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_multicast_add in Port= %d\n", g_ip_compatible_mc.ports[OUT_PORT_INDEX[temp_i]]);
     ...
     */
    rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID | mc_flags, &g_ip_compatible_mc.mcid[3]);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_multicast_create\n");
        return rv;
    ...
    for (temp_i = 0; temp_i < NUM_OF_OUT_PORTS - 1; temp_i++)
    {
        ...
        rv = bcm_multicast_add(unit, g_ip_compatible_mc.mcid[3], mc_flags, g_ip_compatible_mc.nof_replications,
                               &rep_array);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_multicast_add in Port= %d\n", g_ip_compatible_mc.ports[OUT_PORT_INDEX[temp_i]]);
     ...
     * Multicast Configuration for V4_SVL
     */
    rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID | mc_flags, &g_ip_compatible_mc.mcid[4]);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_multicast_create\n");
        return rv;
    ...
    for (temp_i = 0; temp_i < NUM_OF_OUT_PORTS; temp_i++)
    {
        ...
        rv = bcm_multicast_add(unit, g_ip_compatible_mc.mcid[4], mc_flags, g_ip_compatible_mc.nof_replications,
                               &rep_array);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_multicast_add in Port= %d\n", g_ip_compatible_mc.ports[OUT_PORT_INDEX[temp_i]]);
     ...
     * Replication configuration for unknown multicast
     */
    rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID | mc_flags, &g_ip_compatible_mc.mcid[5]);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_multicast_create\n");
        return rv;
    ...
    for (temp_i = 0; temp_i < NUM_OF_OUT_PORTS; temp_i++)
    {
        ...
        rv = bcm_multicast_add(unit, g_ip_compatible_mc.mcid[5], mc_flags, g_ip_compatible_mc.nof_replications,
                               &rep_array);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_multicast_add in Port= %d\n", g_ip_compatible_mc.ports[OUT_PORT_INDEX[temp_i]]);
    ...
    int unit)
{
     ...
     */
    if (*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL)) {
        rv = bcm_multicast_group_destroy(unit, (BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_ELEMENT_STATIC), g_ip_compatible_mc.mcid);
    } else {
