    ...
    int unit)
{
    ...
    rv = bcm_failover_create(unit, flags, &evpn_protection_info.egress_failover_id);
    if (rv != BCM_E_NONE)
    {
    ...
    if (*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL))
    {
        rv = evpn_iml_failover_mc_add(unit, evpn_protection_info.egress_failover_id,
        ...
        if (rv != BCM_E_NONE)
        {
            printf("ERROR: in evpn_iml_failover_mc_add\n");
            return rv;
        }
    }
    else
    ...
    int secondary_iml_tunnel_if)
{
    ...
    rv = bcm_multicast_encap_extension_create(unit, 0, &primary_mc_ext_id, 2, cuds);
    if (rv != BCM_E_NONE)
    {
        printf("ERROR: bcm_multicast_encap_extension_create\n");
        return rv;
    ...
    rv = bcm_multicast_encap_extension_create(unit, 0, &secondary_mc_ext_id, 2, cuds);
    if (rv != BCM_E_NONE)
    {
        printf("ERROR: bcm_multicast_encap_extension_create\n");
        return rv;
    ...
    rv = bcm_multicast_add(unit, mc_id, flags, 1, rep_array);
    if (rv != BCM_E_NONE)
    {
        printf("evpn_iml_mc_failover_add: Error in bcm_multicast_add!\n");
        return rv;
    }
