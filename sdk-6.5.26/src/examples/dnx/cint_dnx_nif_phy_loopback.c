 ...
 *
 * File:        cint_dnx_nif_phy_loopback.c
 *
 ...
 * tx 1 pbm=1 vl=100
 * bshell(unit,"port sts");
 * bshell(unit,"show co full");
 * print cint_port_phy_loopback_clear (unit, port);
 *
 */

static const int NOF_NIF_PHYS_MAX = 144;
    ...
    bcm_port_t port)
{
    ...
    /*
     * Set back port loopback info after bcm_port_enable_set API.
     * Otherwise, the force sig detect will be clear by bcm_port_enable_set,
     * which causes link down issue for 1G port
     */
    ...
    uint32 *flags)
{
    ...
    for (port_index = 0; port_index < nof_ports; ++port_index)
    {
        ...
        mapping_info[port_index].in_tm_port = -1;
        mapping_info[port_index].out_tm_port = -1;
        mapping_info[port_index].pp_dsp = -1;
        mapping_info[port_index].in_channel = -1;
        mapping_info[port_index].out_channel = -1;
...
)
{
    ...
    int device_id;
    int device_ids[NOF_LANES_PER_PM];
   ...
   /*
    ...
    for(port_index = 0; port_index < nof_ports; ++port_index)
    {
        ...
        device_ids[port_index] = device_id;
    }
    rv = cint_dyn_port_add_ports_full_example(unit, nof_ports, port_list, device_ids, mapping_info, interface_info, resource,
                                              dfe_mode, medium_type, header_type_in, header_type_out, flags);
    if (BCM_FAILURE(rv))
    {
