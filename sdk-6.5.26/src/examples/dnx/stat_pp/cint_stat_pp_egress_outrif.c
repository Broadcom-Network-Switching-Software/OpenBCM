    ...
    int failover_out_port)
{
    ...
    rv = cint_stat_pp_config_main(unit, egress_out_rif_statistics_config, interface_type, 0, BCM_CORE_ALL, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    if (host_format != 2)
    {
        ...
        rv = l3__egress_only_fec__create_inner(unit, fec, ((host_format == 1) || (host_format == 3) ) ? 0 : intf_out, encap_id, gport, 0, flags2,
        ...
        if (failover_id != 0)
        {
            ...
            rv = l3__egress_only_fec__create_inner(unit, fec, (host_format == 1) ? 0 : intf_out, encap_id, gport, 0, flags2,
                                                   failover_id, failover_is_primary,&encoded_fec);
            if (rv != BCM_E_NONE)
            {
                printf("Error, create egress object FEC only\n");
    ...
    else
    {
         ...
         * 8. Add route entry
         */
        rv = add_route_ipv4(unit, host, 0xFFFFFFFF, vrf, fec);
        if (rv != BCM_E_NONE)
        {
         ...
         */
        rv = add_route_ipv6(unit, cint_ip_route_basic_ipv6_host, ipv6_route_as_host_mask, vrf, fec);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = add_ipv4_ipmc(unit, cint_ipmc_info.mc_dip, 0xFFFFFFFF, cint_ipmc_info.sip, 0x0,
