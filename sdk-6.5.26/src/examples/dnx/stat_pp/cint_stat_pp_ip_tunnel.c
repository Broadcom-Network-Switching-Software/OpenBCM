 ...
 * cint ../../src/examples/dnx/cint_dnx_ip_tunnel_encap_basic.c
 * cint
 * 1. IPV4 tunnel term
   ...
   int outP)
{
    ...
    rv = cint_stat_pp_config_main(unit, ipv4_tunnel_outlif_statistics_properties, interface_type, 0, BCM_CORE_ALL, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    print stat_info;
    rv = bcm_gport_stat_set(unit, gport, BCM_CORE_ALL, interface_type, stat_info);
    if (rv != BCM_E_NONE)
    {
         printf("Error in bcm_gport_stat_set in IRPP\n");
    }
