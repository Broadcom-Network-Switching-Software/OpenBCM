    ...
    int vni)
{
    ...
    /* Delete VXLAN port */
    rv = bcm_vxlan_port_delete(unit, cint_vxlan_basic_info.vpn_id, cint_vxlan_basic_info.vxlan_port_id);
    if (rv != BCM_E_NONE)
    {
    ...
    /* Destroy VPN */
    rv = vxlan_vpn_destroy(unit);
    if (rv != BCM_E_NONE)
    {
        printf("Error in vxlan_vpn_destroy \n");
        return rv;
    }
    ...
    int vdc_enable)
{
    ...
    if (learn_mode == LEARN_FEC_ONLY) {
        ...
        rv = l3__egress_only_fec__create(unit, cint_vxlan_basic_info.encap_fec_id, cint_vxlan_basic_info.encap_tunnel_id, 0, provider_port, 0, fwd_flag);
        if (rv != BCM_E_NONE)
        {
    ...
    */
    if (learn_mode == LEARN_SYM_OUTLIF_FEC || learn_mode == LEARN_ASYM_OUTLIF_FEC) {
        int fwd_flag = (*dnxc_data_get(unit, "l3", "feature", "separate_fwd_rpf_dbs", NULL)) ? BCM_L3_FLAGS2_FWD_ONLY : 0;
        rv = l3__egress_only_fec__create(unit, cint_vxlan_basic_info.encap_fec_id, cint_vxlan_basic_info.tunnel_arp_id, 0, provider_port, 0, fwd_flag);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, l3__egress_only_fec__create fec_id=0x%x\n", proc_name, cint_vxlan_basic_info.encap_fec_id);
            return rv;
