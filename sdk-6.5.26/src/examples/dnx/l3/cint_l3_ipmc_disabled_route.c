    ...
    int out_port2)
{
    ...
    rv = bcm_multicast_create(unit, (flags | BCM_MULTICAST_WITH_ID), &l2_mc_group);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_multicast_create \n");
        return rv;
    }
    rv = bcm_multicast_add(unit, l2_mc_group, flags, 2, replications);
    if (rv != BCM_E_NONE)
    {
    ...
    int out_port2)
{
    ...
    for (idx = 0; idx < 4; idx++)
    {
        ...
        rv = bcm_multicast_create(unit, (flags | BCM_MULTICAST_WITH_ID), &ipmc_group[idx]);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_create %d\n", idx);
            return rv;
        }
        rv = bcm_multicast_add(unit, ipmc_group[idx], flags, cint_ipmc_info.nof_replications, replications);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = add_ipv4_ipmc_bridge(unit, dip_addr, dip_mask, sip_addr, sip_mask, vids[0], 0, ipmc_group[0], 0, 0);
    if (rv != BCM_E_NONE)
    {
