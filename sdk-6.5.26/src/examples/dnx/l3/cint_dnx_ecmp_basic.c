        ...
        bcm_l3_egress_ecmp_t * user_profile_ecmp)
{
    ...
    rc = bcm_l3_egress_ecmp_user_profile_create(unit,user_profile_ecmp , offset_count_intf , offset_array);
    if ( rc != BCM_E_NONE)
    {
        printf("bcm_l3_egress_ecmp_user_profile create failed: %d \n", rc);
        return rc;
     }
    return rc;
}
    ...
    int user_profile_id)
{
    ...
    if(user_profile_id)
    {
        ...
        rc = bcm_l3_egress_ecmp_create(unit, ecmp, 1, intf_array);
    }
    else
    {
    ...
    int ecmp_id)
{
    ...
    */
    if (cint_ecmp_info.user_profile_id)
    {
        ...
        rv = l3__ecmp_user_profile_create(unit,BCM_L3_WITH_ID,BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT,cint_ecmp_info.user_profile_id
                ,nof_offsets_user_profiles,intf_offset,&user_profile_ecmp);

        if (rv != BCM_E_NONE)
        {
             printf("Error, create ecmp user profile\n");
             return rv;
        }
    }
    /*
    ...
    {
        rv = l3__ecmp_create(unit, ecmp_id, NOF_OF_MEMBERS_ECMP_GROUP, intf_array, BCM_L3_WITH_ID,
                             BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT, &ecmp, cint_ecmp_info.user_profile_id);
    }
    if (rv != BCM_E_NONE)
    {
    ...
    int ecmp_id)
{
    ...
    */
    ...
    for(tp_iter = 0; tp_iter < traffic_class_max_size; tp_iter++)
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_l3_ecmp_tunnel_priority_map_set!\n");
            return rv;
        }
    }
    ...
    for(index_iter = 0;index_iter < nof_tables;index_iter++)
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("bcm_l3_egress_ecmp_tunnel_priority_set failed: %d \n", rv);
            return rv;
        }
    }
    ...
    {
        rv = add_route_ipv4(unit, cint_ecmp_info.host, mask, cint_ecmp_info.vrf, ecmp_id);
        if (rv != BCM_E_NONE)
        {
    ...
    int out_port)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_ecmp_info.fec_id1);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
        return rv;
    }
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 1, 0, &cint_ecmp_info.fec_id2);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
        return rv;
    }
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 2, 0, &cint_ecmp_info.fec_id3);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
        return rv;
    ...
    for (iter_1st = 0; iter_1st < NOF_1ST_HIER_FECS; iter_1st++)
    {
        ...
        for (iter_2nd = 0; iter_2nd < NOF_2ND_HIER_FECS; iter_2nd++)
        {
             ...
             */
            rv = create_arp_plus_ac_type(unit, 0, cint_ecmp_info.arp_next_hop_mac, &cint_ecmp_info.encap_id1,
            ...
            {
                rv = l3__egress_only_fec__create_inner(unit, cint_ecmp_info.fec_id3, rif, cint_ecmp_info.encap_id2, gport, BCM_L3_3RD_HIERARCHY, flags2, 0, 0, 0);
                if (rv != BCM_E_NONE)
                {
                    printf("Error, create egress object FEC only\n");
                    return rv;
            ...
            rv = l3__ecmp_create(unit, cint_ecmp_info.ecmp_id3, NOF_3RD_HIER_FECS, fec_array_hier3,
                                 BCM_L3_WITH_ID | BCM_L3_3RD_HIERARCHY, BCM_L3_ECMP_DYNAMIC_MODE_DISABLED, &ecmp1, 0);
            if (rv != BCM_E_NONE)
            {
            ...
            rv = l3__egress_only_fec__create_inner(unit, cint_ecmp_info.fec_id2, 0, cint_ecmp_info.encap_id1, gport_ecmp, BCM_L3_2ND_HIERARCHY, flags2, 0, 0, 0);
            if (rv != BCM_E_NONE)
            {
                printf("Error, create egress object FEC only\n");
                return rv;
        ...
        rv = l3__ecmp_create(unit, cint_ecmp_info.ecmp_id2, NOF_2ND_HIER_FECS, fec_array_hier2,
                             BCM_L3_WITH_ID | BCM_L3_2ND_HIERARCHY, BCM_L3_ECMP_DYNAMIC_MODE_DISABLED, &ecmp,0);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = l3__egress_only_fec__create_inner(unit, cint_ecmp_info.fec_id1, 0, cint_ecmp_info.intf_out, gport_ecmp, 0, flags2, 0, 0, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object FEC only\n");
            return rv;
     ...
     */
    rv = add_route_ipv4(unit, cint_ecmp_info.host, cint_ecmp_info.mask, cint_ecmp_info.vrf, ecmp.ecmp_intf);
    if (rv != BCM_E_NONE)
    {
