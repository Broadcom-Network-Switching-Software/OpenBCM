...
int cint_field_flow_id_ext_add(int unit, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_action_info_get
    ...
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &flow_action));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &fg_id_FLOW_id));

    bcm_field_group_attach_info_t_init(&attach_info);
    attach_info.payload_info.nof_actions = 1;
    attach_info.payload_info.action_types[0] = flow_action;

    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, fg_id_FLOW_id, context_id, &attach_info));
                                                                ...
                                                                bcm_field_context_t  *context_id)
{
    ...
    if(stage == bcmFieldStageIngressPMF1)
    {
        ...
        BCM_IF_ERROR_RETURN(bcm_field_context_param_set(unit,0,bcmFieldStageIngressPMF2,*context_id,&context_param));
        ...
        if(flow_id_ext_add && add_flow)
        {
            ...
            BCM_IF_ERROR_RETURN(cint_field_flow_id_ext_add(unit,*context_id));
        }
    }
...
int cint_field_always_hit_contex_destroy(int unit, int presel_id,bcm_field_stage_t stage, bcm_field_context_t context_id)
{
    ...
    if(stage == bcmFieldStageIngressPMF1)
    {
        ...
        if(fg_id_FLOW_id != -1)
        {
            BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit,fg_id_FLOW_id,context_id));

            BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit,fg_id_FLOW_id));
            fg_id_FLOW_id = -1;
            BCM_IF_ERROR_RETURN(bcm_field_action_destroy(unit, flow_action));
            flow_action = -1;
        }
    }
    BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit,stage,context_id));
