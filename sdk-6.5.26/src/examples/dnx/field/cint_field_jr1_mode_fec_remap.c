    ...
    bcm_field_entry_t * entry_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, fg_id, &entry_info, entry_id));
    ...
    int access_port)
{
    ...
    if (!sand_utils_is_jr2_system_header_mode(unit)) {
        BCM_IF_ERROR_RETURN(field_presel_fwd_layer_main(unit, cint_fec_remap.presel_id, cint_fec_remap.ipmf_stage,
                eth_layer, &cint_fec_remap.context_id, ctx_name));

        BCM_IF_ERROR_RETURN(field_fec_id_remapping_group_create(unit, &cint_fec_remap.fg_id, cint_fec_remap.context_id));

        for (idx = 0; idx < cint_fec_remap.nof_entries; idx++)
        {
            bcm_field_entry_t entry_id;
            BCM_IF_ERROR_RETURN(field_fec_id_remapping_entry_add(unit, cint_fec_remap.fg_id, (cint_fec_remap.fec_id_start_jr1 + idx) & cint_fec_remap.mask_15b,
    ...
    int unit)
{
    ...
    if (!sand_utils_is_jr2_system_header_mode(unit)) {
        ...
        for (idx = 0; idx < cint_fec_remap.nof_entries; idx++)
        {
            ...
            BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, cint_fec_remap.fg_id, entry_qual_info, cint_fec_remap.entry_ids[idx]));
        }

        BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, cint_fec_remap.fg_id, cint_fec_remap.context_id));

        BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, cint_fec_remap.fg_id));

        BCM_IF_ERROR_RETURN(field_presel_fwd_layer_destroy(unit, cint_fec_remap.presel_id, cint_fec_remap.ipmf_stage, cint_fec_remap.context_id));
    }
