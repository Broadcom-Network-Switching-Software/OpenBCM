    ...
    bcm_field_context_t context_id_ipmf1)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &action_prefix_stat_action));
    ...
    action_info.size = 1;
    action_info.stage = bcmFieldStageIngressPMF2;
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &action_prefix_udh2_action));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 6;
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &action_prefix_qual_lrn_data_dst_port));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &action_prefix_de_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, action_prefix_de_fg_id, context_id_ipmf1, &attach_info));
    
    printf("Action prefix DE FG_ID(%d) was attached to context (%d) \n", action_prefix_de_fg_id, context_id_ipmf1);

    BCM_IF_ERROR_RETURN(bcm_field_group_info_get(unit, action_prefix_de_fg_id, &fg_info_get));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_info_get(unit, action_prefix_de_fg_id, context_id_ipmf1, &attach_info_get));
    ...
    bcm_field_context_t context_id_ipmf1)
{
     ...
     */
    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, action_prefix_de_fg_id, context_id_ipmf1));
    printf("Action prefix detach DE FG_ID(%d) from context (%d) \n", action_prefix_de_fg_id, context_id_ipmf1);
    
    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, action_prefix_de_fg_id));
    printf("Action prefix delete DE FG_ID(%d) \n", action_prefix_de_fg_id);

    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, action_prefix_qual_lrn_data_dst_port));
    
    BCM_IF_ERROR_RETURN(bcm_field_action_destroy(unit, action_prefix_stat_action));
