    ...
    int unit)
{
    ...
    qual_info.size = 1;
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &Cint_field_dt_multi_qual_id));
    ...
    cint_field_dt_multi_fg_info_t * dt_fg_info)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_context_create(unit, 0, field_stage, &context_info, &dt_fg_info->context_id));
    ...
    if (field_stage == bcmFieldStageIngressPMF3 && dt_fg_info->presel_id == Cint_field_dt_multi_presel_id[1])
    {
        ...
        BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, in_gport, bcmPortClassFieldIngressPMF3PacketProcessingPortCs, port_class_id));
    ...
    cint_field_dt_multi_fg_info_t * dt_fg_info)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_dt_multi_context_create(unit, ipmf3_in_port, field_stage_internal, dt_fg_info));
    ...
    if (field_stage == bcmFieldStageEgress)
    {
        ...
        if (Cint_field_dt_multi_dt_max_key_size_dnx_data_value == 10)
        {
            if (Cint_field_dt_multi_eth_samac_8b_qual_id == -1)
            {
                ...
                BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &eth_qual_info, &Cint_field_dt_multi_eth_samac_8b_qual_id));
            }
    ...
    BCM_IF_ERROR_RETURN(bcm_port_get(unit, internal_port, &flags, &interface_info, &mapping_info));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, dt_fg_info->fg_id, &dt_fg_info->entry_info, &dt_fg_info->entry_handle));
    ...
    bcm_field_stage_t field_stage)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_dt_multi_qual_create(unit));

    for (fg_iter = 0; fg_iter < Cint_field_dt_multi_nof_fg; fg_iter++)
    {
        BCM_IF_ERROR_RETURN(cint_field_dt_multi_fg_create(unit, in_port, ipmf3_in_port, out_port, field_stage, Cint_field_dt_multi_fg_info_array[fg_iter]));
    }
    ...
    cint_field_dt_multi_fg_info_t * dt_fg_info)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, dt_fg_info->fg_id, dt_fg_info->entry_info.entry_qual, dt_fg_info->entry_handle));

    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, dt_fg_info->fg_id, dt_fg_info->context_id));

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, dt_fg_info->fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit, field_stage_internal, dt_fg_info->context_id));

    if (is_second_fg)
    {
        BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, Cint_field_dt_multi_qual_id));

         /** Q2A */
        if (Cint_field_dt_multi_dt_max_key_size_dnx_data_value == 10 && field_stage == bcmFieldStageEgress)
        {
            BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, Cint_field_dt_multi_eth_samac_8b_qual_id));
    ...
    bcm_field_stage_t field_stage)
{
    ...
    for (fg_iter = 0; fg_iter < Cint_field_dt_multi_nof_fg; fg_iter++)
    {
        BCM_IF_ERROR_RETURN(cint_field_dt_multi_destroy_internal(unit, field_stage, fg_iter, Cint_field_dt_multi_fg_info_array[fg_iter]));
    }
