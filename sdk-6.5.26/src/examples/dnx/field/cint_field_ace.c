 ...
 *  
   ...
   cint;
   int unit = 0;
   bcm_field_context_t epmf_context_id = 0;
   int write_value = 1;
...
int field_ace_main(int unit, bcm_field_context_t epmf_context_id, int write_value)
{
   ...
   BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &Cint_field_ace_action_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_ace_format_add(unit, 0, &ace_format_info, &ace_format_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_ace_entry_add(unit, 0, ace_format_id, &ace_entry_info, &ace_entry_handle));
    /*
    ...
    epmf_fg_info.qual_types[0] = bcmFieldQualifyMcastPkt;
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &epmf_fg_info, &epmf_fg_id));
    ...
    /*
     * ACE can only be supported for unicast packet. so McastPkt qualifier should be 0
     */
    epmf_entry_info.entry_qual[0].value[0] = 0;
    epmf_entry_info.entry_qual[0].mask[0] = 0x1;
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, epmf_fg_id, &epmf_entry_info, &epmf_entry_handle));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, epmf_fg_id, epmf_context_id, &epmf_attach_info));
...
int field_ace_destroy(int unit, bcm_field_context_t epmf_context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, epmf_fg_id, epmf_context_id));

    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, epmf_fg_id, &entry_qual_info, epmf_entry_handle));
  
    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, epmf_fg_id));

    BCM_IF_ERROR_RETURN(bcm_field_ace_entry_delete(unit, ace_entry_handle));
  
    BCM_IF_ERROR_RETURN(bcm_field_ace_format_delete(unit, ace_format_id));
    
    BCM_IF_ERROR_RETURN(bcm_field_action_destroy(unit, Cint_field_ace_action_id));
...
int field_ace_update(int unit, int write_value)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_ace_entry_add(unit, flags, ace_format_id, &ace_entry_info, &ace_entry_handle));
