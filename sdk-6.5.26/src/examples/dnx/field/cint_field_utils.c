    ...
    bcm_field_group_info_t * fg_info_p)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, flags, fg_info_p, &tcam_fg_util_p->fg_id));
    ...
    bcm_field_group_attach_info_t * attach_info_p)
{
    ...
    if (tcam_fg_util_p->context != BCM_FIELD_CONTEXT_ID_DEFAULT)
    {
        ...
        if (rv == BCM_E_NOT_FOUND)
        {
            ...
            BCM_IF_ERROR_RETURN(bcm_field_context_create
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, tcam_fg_util_p->fg_id, tcam_fg_util_p->context, attach_info_p));
    ...
    bcm_field_entry_info_t * entry_info_p)
{
   ...
   /** Loop over the supplied array of BCM qualifiers , which we limit to 3 */
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, tcam_fg_util_p->fg_id, entry_info_p, &(tcam_fg_util_p->entry_handle)));
    ...
    field_util_basic_tcam_fg_t * tcam_fg_util_p)
{
     ...
     */
    BCM_IF_ERROR_RETURN(field_util_basic_tcam_fg_add(unit, tcam_fg_util_p, &fg_info));

    /*
     * Creating(if context supplied is not BCM_FIELD_CONTEXT_ID_DEFAULT) context and
     * attaching the supplied FG to it.
     */
    BCM_IF_ERROR_RETURN(field_util_basic_context_attach(unit, tcam_fg_util_p, &fg_info, &attach_info));


    /*
     * Adding single entry containing all qualifiers and actions(for now), to the previously created FG.
     */
    BCM_IF_ERROR_RETURN(field_util_basic_entry_add(unit, tcam_fg_util_p, &attach_info, &entry_info));
    ...
    field_util_fg_t * fg_util_p)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, flags, &fg_info_t, &fg_util_p->fg_id));
...
)
{
    ...
    /** Get the fg info */
    BCM_IF_ERROR_RETURN(bcm_field_group_info_get(unit, fg_util_p->fg_id, &fg_info_get));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, flags, fg_util_p->fg_id, context_id, &attach_info));
    ...
    field_util_entry_info_t * ent_info_util_p)
{
   ...
   /** Loop over the supplied arrays of BCM actions and qualifiers */
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, flags, ent_info_util_p->fg_id, &entry_info, &(ent_info_util_p->entry_handle)));
    ...
    bcm_field_entry_t * entry_id_p)
{
    ...
    BCM_IF_ERROR_RETURN(field_util_field_entry_add(unit, 0, &ent_utlil));
    ...
    int qual_arg)
{
    ...
    bcm_field_context_info_t_init(&context_info);
    BCM_IF_ERROR_RETURN(bcm_field_context_create(unit, flag_with_id, stage, &context_info, &context_id));
    ...
    field_utils_l2_l3_config_t l2_config)
{
    ...
    /** Create VSI */
    BCM_IF_ERROR_RETURN(bcm_vlan_create(unit, l2_config.vid));
    ...
    if (!BCM_GPORT_IS_TRUNK(l2_config.in_port)) {
        ...
        BCM_IF_ERROR_RETURN(bcm_vlan_port_add(unit, l2_config.vid, pbmp, untag_pbmp));

    } else {
        BCM_IF_ERROR_RETURN(bcm_vlan_gport_add (unit, l2_config.vid, l2_config.in_port, BCM_VLAN_PORT_UNTAGGED));
    ...
    BCM_IF_ERROR_RETURN(bcm_vlan_port_create(unit, &vlan_port));
    ...
    field_utils_l2_l3_config_t l3_config)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_vlan_gport_add(unit, l3_config.vid, l3_config.in_port, 0));
    ...
    BCM_IF_ERROR_RETURN(bcm_port_match_add(unit, vlan_port.vlan_port_id, &match_info));
    ...
    bcm_if_t * intf_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_l3_ingress_create(unit, &ingress_intf, intf_id));
    ...
    bcm_field_ace_format_t * ace_format_id_p)
{
     ...
     */
    BCM_IF_ERROR_RETURN(bcm_field_ace_format_add(unit, flags, &ace_format_info, ace_format_id_p));
    ...
    uint32 * entry_handle_p)
{
     ...
     */
    BCM_IF_ERROR_RETURN(bcm_field_ace_entry_add(unit, flags, ace_format_id, &ace_entry_info, entry_handle_p));
