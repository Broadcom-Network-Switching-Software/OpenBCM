...
cint_field_vlan_format_qual_info_t field_vlan_format_qual_info_array[VLAN_FORMAT_NOF_QUALS] = {
  /**  qual_type   |   qual_value   |   qual_mask    |  input_type    |    input_arg  |  qual_offset  */
    {bcmFieldQualifyVlanFormat, bcmTagStructTypeSPrioTag, 0xFFFF, {bcmFieldInputTypeMetaData, 0, 0}}
};
...
bcm_field_presel_qualify_data_t field_vlan_format_cs_info_array[VLAN_FORMAT_NOF_CS_QUALS] = {
  /** qual_type  |        qual_arg    |       qual_value     |     qual_mask  */
    {bcmFieldQualifyVlanFormat, 0, bcmTagStructTypeSPrioTag, 0x1F}
};
    ...
    bcm_field_context_t * context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_context_create(unit, 0, stage, &context_info, context_id));
    ...
    cint_field_vlan_format_fg_info_t * vlan_format_fg_info)
{
     ...
     */
    BCM_IF_ERROR_RETURN(cint_field_vlan_format_context(unit, vlan_format_fg_info->stage, vlan_format_fg_info->vlan_format_cs_info[0], &(vlan_format_fg_info->context_id)));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &(vlan_format_fg_info->fg_id)));


    printf("Field Group ID %d was created. \n", vlan_format_fg_info->fg_id);

    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, vlan_format_fg_info->fg_id, vlan_format_fg_info->context_id, &attach_info));

    printf("Field Group ID %d was attached to Context ID %d. \n", vlan_format_fg_info->fg_id, vlan_format_fg_info->context_id);

    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, vlan_format_fg_info->fg_id, &entry_info, &entry_handle));
    ...
    int fg_info_array_index)
{
     ...
     */
    BCM_IF_ERROR_RETURN(cint_field_vlan_format_group_config(unit, vlan_format_fg_info_array[fg_info_array_index]));
    ...
    int fg_info_array_index)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, vlan_format_fg_info_array[fg_info_array_index].fg_id, NULL, entry_handle));

    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, vlan_format_fg_info_array[fg_info_array_index].fg_id,
                                        vlan_format_fg_info_array[fg_info_array_index].context_id));


    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, vlan_format_fg_info_array[fg_info_array_index].fg_id));

    BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit, vlan_format_fg_info_array[fg_info_array_index].stage,
                                   vlan_format_fg_info_array[fg_info_array_index].context_id));
