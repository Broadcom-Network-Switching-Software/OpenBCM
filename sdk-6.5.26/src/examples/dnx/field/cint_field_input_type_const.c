...
int field_input_type_const_main(int unit ,bcm_field_context_t context_id)
{
    ...
    qual_info.size = 5;
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &input_type_const_qual_lrn_data_dst_port));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 4;
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &input_type_const_qual_const));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &input_type_const_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, input_type_const_fg_id, context_id, &attach_info));
...
int field_input_type_const_destroy(int unit, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit,input_type_const_fg_id,context_id));
    printf("Input type const detach FG_ID(%d) from context (%d) \n",input_type_const_fg_id,context_id);
  
    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, input_type_const_fg_id));

    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, input_type_const_qual_lrn_data_dst_port));
    
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, input_type_const_qual_const));
