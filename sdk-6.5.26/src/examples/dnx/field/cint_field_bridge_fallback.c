...
{
    BCM_IF_ERROR_RETURN(cint_field_bridge_fallback_select_unknown_dest_and_l3_mc_packets_and_vis_profile_update_lif_and_port(unit));

    BCM_IF_ERROR_RETURN(cint_field_bridge_fallback_update_vsi_with_lif_and_pop_lif(unit));
...
int cint_field_bridge_fallback_main_destroy (int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, bridge_fallback_fg_id_casc, bridge_fallback_context_id));
    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, bridge_fallback_fg_id_casc));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, bridge_fallback_fwd_domain_qual_id));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, bridge_fallback_is_pmf_hit_duplicate));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, bridge_fallback_is_pmf_hit));
    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, bridge_fallback_fg_id, bridge_fallback_context_id));
    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, bridge_fallback_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit, bcmFieldStageIngressPMF1, bridge_fallback_context_id));

    BCM_IF_ERROR_RETURN(bcm_field_action_destroy(unit, ipmf_1_action_type_hit));
...
int cint_field_bridge_fallback_select_unknown_dest_and_l3_mc_packets_and_vis_profile_update_lif_and_port(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, ipmf_1_action_type_hit));

    bcm_field_context_info_t context_info;
    bcm_field_context_info_t_init(&context_info);
    sal_strncpy_s(dest_char, "BRIDGE_FALLBACK_CTX", sizeof(context_info.name));

    BCM_IF_ERROR_RETURN(bcm_field_context_create(unit, 0, stage, &context_info, &bridge_fallback_context_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_context_param_set(unit, 0, bcmFieldStageIngressPMF2, bridge_fallback_context_id, &param_info));

    BCM_IF_ERROR_RETURN(cint_field_bridge_fallback_set_preselector_ipv4(unit, stage));
    BCM_IF_ERROR_RETURN(cint_field_bridge_fallback_set_preselector_ipv6(unit, stage));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &bridge_fallback_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, bridge_fallback_fg_id, bridge_fallback_context_id, &attach_info));
...
int cint_field_bridge_fallback_entry_add(int unit, int nof_cores,uint32 rcy_port_core_0, uint32 rcy_port_core_1, int vsi_profile)
{
    ...
    for (core_id = 0; core_id < nof_cores; core_id ++) {
        ...
        BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, bridge_fallback_fg_id, &ent_info, &cint_bridge_fallback_entry_id[core_id]));
...
int cint_field_bridge_fallback_entry_destroy(int unit, int nof_cores, int vsi_profile)
{
    ...
    for (core_id = 0;core_id < nof_cores;core_id++) {
        ...
        BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, bridge_fallback_fg_id, NULL, cint_bridge_fallback_entry_id[core_id]));
    }
    return BCM_E_NONE;
}
...
int cint_field_bridge_fallback_update_vsi_with_lif_and_pop_lif(int unit) {
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &bridge_fallback_is_pmf_hit));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 1;
    dest_char = &(qual_info.name[0]);
    sal_strncpy_s(dest_char, "is_pmf_hit_duplicate", sizeof(qual_info.name));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &bridge_fallback_is_pmf_hit_duplicate));

    /**bcmFieldQualifyVrf = bcmFieldActionVSwitchNew = FWD_Domain*/
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_info_get(unit, bcmFieldQualifyVrf, bcmFieldStageIngressPMF1, &qual_info_get));

    qual_info.size = qual_info_get.size;
    dest_char = &(qual_info.name[0]);
    sal_strncpy_s(dest_char, "BRIDGE_FALLBACK_FWD_DOMAIN", sizeof(qual_info.name));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &bridge_fallback_fwd_domain_qual_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &bridge_fallback_fg_id_casc));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_info_get
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_action_offset_get(unit, 0, bridge_fallback_fg_id, ipmf_1_action_type_hit, &(attach_info.key_info.qual_info[0].offset)));
    attach_info.key_info.qual_info[1].input_type = bcmFieldInputTypeMetaData;
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_action_offset_get(unit, 0, bridge_fallback_fg_id, ipmf_1_action_type_hit, &(attach_info.key_info.qual_info[2].offset)));
    
    attach_info.key_info.qual_info[3].input_type = bcmFieldInputTypeMetaData;

    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, bridge_fallback_fg_id_casc, bridge_fallback_context_id, &attach_info));
