...
int lif_counting_erpp_double_packet_check(int unit, int in_port, int out_port, int pkt_num, int is_last)
{
    int rv;
    int countId = 1024*16;
    int counter_proc = 5;
    ...
    rv = is_device_qax_only(unit, &is_qax);
    if (rv != BCM_E_NONE) {
        printf("Error in is_device_qax_only!\n");
        return rv;
    }

    if (is_qux) {
        countId = 1024*8;
        nof_cnt_per_cnt_processor = 1 * 1024;
    } else if (is_qax) {
        countId = 1024*16;
        nof_cnt_per_cnt_processor = 4 * 1024;
    } else {
        countId = 1024*16;
        nof_cnt_per_cnt_processor = 16 * 1024;
    }
    ...
    counter_config.source.pointer_range.end = countId + nof_cnt_per_cnt_processor - 1;
    ...
    BCM_GPORT_SUB_TYPE_LIF_SET(vlan_port.vlan_port_id, BCM_GPORT_SUB_TYPE_LIF, is_last?countId:(countId + nof_cnt_per_cnt_processor - 1));
    ...
    BCM_FIELD_STAT_ID_SET(stat_id, counter_proc, is_last?countId:(countId + nof_cnt_per_cnt_processor - 1));
    /*check fwd packet*/
