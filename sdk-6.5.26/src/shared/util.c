...
/*
   Function:  _shr_versatile_hash()
   Purpose:   Hash a variable-length key (256bit) into 64bit Hash output
   Inputs:
             hash_key  :  256b Key to Hash
             init_val_0  :  32bit value
  Return:
             vh  :   32bit Versatile Hash
*/

#define rot(x,k) (((x)<<(k)) | ((x)>>(32-(k))))

static void
_shr_compute_mix(uint32 *a, uint32 *b, uint32 *c)
{
    *a ^= *c;
    *a ^= rot(*c, 4);
    *c ^= *b;
...
static void
_shr_compute_final(uint32 *a, uint32 *b, uint32 *c)
{
     *c ^= *b;
     *c -= rot(*b,14);
...
uint32
_shr_versatile_hash(uint32 *hash_key, uint32 init_val_0, uint32 init_val_1)
{
    uint32  a = 0;
    uint32  b = 0;
    uint32  c = 0;
    ...
    uint32 hash_word8 = 0;
     ...
     /* read 32bit chunks (32b = 1x32b chunks) */
     hash_word8 = init_val_0;
     /* read 32bit chunks (256b = 8x32b chunks) */
     hash_word7 = init_val_1;
     hash_word6 = hash_key[6];
     hash_word5 = hash_key[5];
     hash_word4 = hash_key[4];
     hash_word3 = hash_key[3];
     hash_word2 = hash_key[2];
     hash_word1 = hash_key[1];
     hash_word0 = hash_key[0];

     a = hash_word0 - hash_word3 - hash_word6;
     b = hash_word1 - hash_word4 - hash_word7;
     c = hash_word2 - hash_word5 - hash_word8;

     _shr_compute_mix(&a,&b,&c);
     _shr_compute_final(&a,&b,&c);
     return (c);
}
