...
typedef struct
{
    ...
    int total_time;
    /** worst time over all measured intervals*/
    int worst_time;
    ...
    int module_id)
{
        ...
        && utilex_time_analyzer_info[unit]->master_thread_id == sal_thread_id_get())
    {
        ...
        if (utilex_time_analyzer_info[unit]->modules_info[module_id].start_count == 1)
        {
            ...
            if (interval > utilex_time_analyzer_info[unit]->modules_info[module_id].worst_time)
            {
                utilex_time_analyzer_info[unit]->modules_info[module_id].worst_time = interval;
            }
    ...
    if (utilex_time_analyzer_info[unit] != NULL)
    {
        for (module_id = 0; module_id < UTILEX_TIME_ANALYZER_MAX_NOF_MODULES; module_id++)
        {
            if (utilex_time_analyzer_info[unit]->modules_info[module_id].name != NULL)
            {
                ...
                diag_info[module_index].worst = utilex_time_analyzer_info[unit]->modules_info[module_id].worst_time;
