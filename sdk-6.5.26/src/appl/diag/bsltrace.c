...
STATIC int bsltrace_add(const char *format, va_list args)
{
    ...
    str_len = sal_vsnprintf(trace.head, trace.entry_length, format, args);
    if (str_len < 0) {
        /* Handle Error */
    } else {
        if (str_len >= trace.entry_length) {
            /* Insert overrun marker at the end of the string */
            const char *overrun_str = "..>>\n";
            int overrun_len = 0;
            int overrun_offs = 0;
            overrun_len = sal_strlen(overrun_str) + 1;
            overrun_offs = trace.entry_length - overrun_len;
            sal_memcpy(&trace.head[overrun_offs], overrun_str, overrun_len);
        }
        /* Increment head*/
