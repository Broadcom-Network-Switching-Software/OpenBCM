...
static sh_sand_man_t signal_get_man = {
    ...
    .examples =
        ...
        /*
         * tx option currently creates a "memory leak" result in the framework test
         * the packet data is released on a secondary thread
         */
        "from=NIF tx\n"
#endif
...
{
    match_t match_m;
    int core, nof_cores = 0;
    int attr_offset = 0, addr_offset = 0;
    int i_addr;
    rhlist_t *dsig_list = NULL;
#ifdef ADAPTER_SERVER_MODE
    rhlist_t *adapter_dsig_list = NULL;
    int adapter_cores_to_iterae[DNX_DATA_MAX_DEVICE_GENERAL_NOF_DIES] = { 0 };
    int adapter_nof_cores_to_iterate = 0;
#endif
    debug_signal_t *debug_signal;
    ...
    char **show_tokens = NULL;
    uint32 show_tokens_num = 0;
    int tx_enable;
    int nonzero;
    ...
    if ((core < 0 || core >= nof_cores) && (core != BCM_CORE_ALL))
    {
        SHR_CLI_EXIT(_SHR_E_PARAM, "Illegal core id [%d]", core);
    }
    ...
    if ((btokens != NULL) && (btokens_num == 2))
    {
        ...
        if (utilex_str_stoul(btokens[1], &core_id) == _SHR_E_NONE)
        {
            ...
            if (core != BCM_CORE_ALL)
            {
                if (core != core_id)
                {
                    SHR_CLI_EXIT(_SHR_E_PARAM, "core is %d, but block %s is from other core", core, match_m.block);
                }
            }
    ...
    {
        /*
         * Adapter can have signals on the core that is indicated by the swap_core dnx data -
         * usually 0, but in case running with swapped core, it will hold the right one
         *
         * In addition, in case of a device with sub-units. any core from the swap_core+X*cores_per_sub_unit is valid.
         *
         */
        int nof_sub_units = dnx_data_device.general.nof_dies_get(unit);
        int nof_cores_per_subunits = nof_cores / nof_sub_units;
        int index;

        adapter_nof_cores_to_iterate = nof_sub_units;
        for (index = 0; index < nof_sub_units; index++)
        {
            adapter_cores_to_iterae[index] = index * nof_cores_per_subunits +
                dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit);
        }

        if (core != BCM_CORE_ALL)
        {
            int found_legal_core = FALSE;
            for (index = 0; index < nof_sub_units; index++)
            {
                found_legal_core |= core == adapter_cores_to_iterae[index];
            }
            if (!found_legal_core)
            {
                SHR_CLI_EXIT(_SHR_E_PARAM, "requested signals on core %d, btu this is not a traffic core on adapter",
                             core);
            }
            adapter_cores_to_iterae[0] = core;
            adapter_nof_cores_to_iterate = 1;
        }
    }
#endif
    ...
    {   /* Show options parsing */
        ...
        if ((show_tokens = utilex_str_split(show_str, ",", 6, &show_tokens_num)) == NULL)
        {
            SHR_CLI_EXIT(_SHR_E_INTERNAL, "Problem parsing show string\n");
        }

        for (i_token = 0; i_token < show_tokens_num; i_token++)
        {
            if (!sal_strcasecmp(show_tokens[i_token], "detail"))
                print_flags |= SIGNALS_PRINT_DETAIL;
            else if (!sal_strcasecmp(show_tokens[i_token], "hw"))
                print_flags |= SIGNALS_PRINT_HW;
            else if (!sal_strcasecmp(show_tokens[i_token], "source"))
                match_m.flags |= SIGNALS_MATCH_HW;
            else if (!sal_strcasecmp(show_tokens[i_token], "perm"))
                match_m.flags |= SIGNALS_MATCH_PERM;
            else if (!sal_strcasecmp(show_tokens[i_token], "noexpand"))
                match_m.flags &= ~SIGNALS_MATCH_EXPAND;
            else if (!sal_strcasecmp(show_tokens[i_token], "exact"))
                match_m.flags |= SIGNALS_MATCH_EXACT;
            else if (!sal_strcasecmp(show_tokens[i_token], "strict"))
                match_m.flags |= (SIGNALS_MATCH_EXACT | SIGNALS_MATCH_STRICT);
            else if (!sal_strcasecmp(show_tokens[i_token], "nocond"))
                match_m.flags |= SIGNALS_MATCH_NOCOND;
            else if (!sal_strcasecmp(show_tokens[i_token], "noresolve"))
                match_m.flags |= SIGNALS_MATCH_NORESOLVE;
            else if (!sal_strcasecmp(show_tokens[i_token], "lucky"))
                match_m.flags |= SIGNALS_MATCH_ONCE;
            
            else if (!sal_strcasecmp(show_tokens[i_token], "dont_print"))
                print_flags |= SIGNALS_DONT_PRINT;
#ifdef ADAPTER_SERVER_MODE
            else if (!sal_strcasecmp(show_tokens[i_token], "asic"))
                match_m.flags |= SIGNALS_MATCH_ASIC;
            /** Here we want to print adapter signals, but only the ones having asic analogs */
            else if (!sal_strcasecmp(show_tokens[i_token], "asic_only"))
    ...
    if ((pem_flag == TRUE) ||
        (match_m.from && !sal_strncasecmp(match_m.from, "PEM", 3)) ||
        (match_m.to && !sal_strncasecmp(match_m.to, "PEM", 3)))
    {
    ...
    if (core == BCM_CORE_ALL)
    {
        ...
        {
            rv = sand_signal_get(unit, core, &match_m, dsig_list, NULL);
            if (rv == _SHR_E_NONE)
            {
                found_sig = TRUE;
            }
    ...
    for (int core_index = 0; core_index < adapter_nof_cores_to_iterate; core_index++)
    {
        core = adapter_cores_to_iterae[core_index];
        ...
        if (print_flags & SIGNALS_PRINT_ASIC)
        {
            ...
            rv = sand_signal_get(unit, core, &match_m, adapter_dsig_list, NULL);
            if (rv == _SHR_E_NONE)
            {
                found_sig = TRUE;
            }
    ...
    if ((print_flags & SIGNALS_DONT_PRINT))
    {
        SHR_EXIT();
    }
    if (!found_sig)
    {
        SHR_CLI_EXIT_IF_ERR(_SHR_E_NOT_FOUND, "");
    }
    ...
    }
    {
        RHSAFE_ITERATOR(signal_output, dsig_list)
        {
            ...
            if (print_flags & SIGNALS_PRINT_STATUS)
            {
                ...
                {
                    sal_strncat_s(signal_output->status, " reg", RHNAME_MAX_SIZE);
                }
                else if (DEBUG_SIGNAL_STATUS_IS(SIGNAL_STAGE_PORT_VIS_IGNORE))
                {
                    sal_strncat_s(signal_output->status, " glob", RHNAME_MAX_SIZE);
                }
                else if (DEBUG_SIGNAL_STATUS_IS(SIGNAL_STAGE_PREVIOUS))
                {
                    sal_strncat_s(signal_output->status, " prev", RHNAME_MAX_SIZE);
                }
#endif
...
exit:
    ...
    utilex_str_split_free(show_tokens, show_tokens_num);
    PRT_FREE;
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    RHITERATOR(signal_output, dsig_list)
    {
        ...
        if (expansion_n == NULL)
        {
            SHR_CLI_EXIT(_SHR_E_INTERNAL, "expansion_n is NULL\r\n");
        }
...
#ifdef BCM_DNX_SUPPORT
        for (i_core = 0; i_core < dnx_data_device.general.nof_cores_get(unit); i_core++)
#endif
        {
            for (i_bl = 0; i_bl < device->block_num; i_bl++)
            {
                ...
                for (i_st = 0; i_st < cur_pp_block->stage_num; i_st++)
                {
                    ...
                    PRT_CELL_SET("%d", cur_pp_stage->stage_signal_num[i_core]);
#endif
                }
            }
        }
    ...
    args_t * args)
{
    ...
    sh_sand_act(unit, args, sh_sand_signal_cmds, sh_sand_sys_cmds, NULL, SH_CMD_FLAGS_NONE);
    ARG_DISCARD(args);
    /*
...
static void
compare_signal_value_find_direction_from_stage(
    int unit,
    char *from,
    uint8 *is_ingress)
{
    uint8 found_ingress = 0;

    /*
     *  Add manual stages here that should be resolved as ingress
     * This code is only relevant to signal_Validate calls from TCL
     */
    if (sal_strcasestr(from, "vtt"))
    {
        found_ingress = 1;
    }
    if (sal_strcasestr(from, "fwd1"))
    {
        found_ingress = 1;
    }
    if (sal_strcasestr(from, "fwd2"))
    {
        found_ingress = 1;
    }
    if (sal_strcasestr(from, "ipmf"))
    {
        found_ingress = 1;
    }
    if (sal_strcasestr(from, "lbp"))
    {
        found_ingress = 1;
    }
    if (sal_strcasestr(from, "iparser"))
    {
        found_ingress = 1;
    }
    if (sal_strcasestr(from, "llr"))
    {
        found_ingress = 1;
    }
    *is_ingress = found_ingress;
}
    ...
    int strict)
{
...
#ifdef ADAPTER_SERVER_MODE
    uint32 value_zero[DSIG_MAX_SIZE_UINT32] = { 0 };
#endif
    int rc = _SHR_E_NONE;
     ...
     */
    if (sal_tolower(block[0]) == 'i')
    {
        /** Block is an ingress block */
        pp_port = in_port;
    }
    else if (sal_tolower(block[0]) != 'e')
    {
        uint8 is_ingress = 0;
        /** Block is not identified as ingress or egress block. verify it based on "from"*/
        if (from == NULL)
        {
            cli_out("Block name %s is none recognizable - should be ITPP, IRPP, ERPP or ETPP\n", block);
            return _SHR_E_PARAM;
        }
        compare_signal_value_find_direction_from_stage(unit, from, &is_ingress);
        if (is_ingress)
        {
            pp_port = in_port;
        }
    ...
    if (!sal_strncasecmp(from, "PEM", 3) || !sal_strncasecmp(to, "PEM", 3))
    {
    ...
    else if (rc == _SHR_E_FAIL)
    {
        ...
        if (utilex_str_is_number(result_str))
        {
            cli_out("Received value: 0x%s\n", result_str);
        }
        else
        {
            cli_out("Received value: %s\n", result_str);
        }
        cli_out("*****************************************************************************\n");
        if (fail_on_mismatch)
        {
...
#ifdef ADAPTER_SERVER_MODE
        /*
         * After having adapter visibility, non visible packets are not recorded, and might lead to signal not found.
         * If expected value is 0, consider it is as ok 
         */
        if (sal_memcmp(value, value_zero, DSIG_MAX_SIZE_UINT32 * 4) == 0)
        {
            return 0;
        }
#endif
