...
parse_long_integer(uint32 *val, int nval, char *str)
{
    ...
    {
        /*
         * If input string is empty then return the value '0'
         */
        goto done;
    }
    ...
    {
        /*
         * Enter of the string does not start with 0x or 0X
         * If it does then handling is below.
         */
        if (str[0] == '0')
        {
            /*
             * If the string starts with zero then fall back to the original handling
             * of a 32 bits integer without error detection (in case of overflow).
             * This covers the base-2 and base-8 cases.
             */
        ...
        {
            /*
             * If the string does not start with '0' then handle it as a 64 bits integer
             * and return '0' in case of overflow or non-decimal digit.
             */
            /*
             * Container for the 64 bits representation of input decimal number
             */
            COMPILER_UINT64 sum;
            /*
             * Container for value of sum on the previous cycle. Used to detect overflow
             * beyond 64 bits.
             */
            COMPILER_UINT64 prev_sum;
            COMPILER_UINT64 mul;
            /*
             * Container for the current power of 10 to use on current input digit.
             */
            COMPILER_UINT64 power;
            /*
             * Counter of the number of digits already processed from input decimal string.
             */
            int jj;
            /*
             * Pointer to the current digit to handle. Starting from the LS digit and going to
             * the MS digit.
             */
            char *str_p;
    
            ii = 0;
            COMPILER_64_SET(power,0,10);
            in_str_len = sal_strnlen(str,SAL_CONFIG_STR_MAX);
            str_p = str;
            str_p += (in_str_len - 1);
            COMPILER_64_SET(sum,0,(*str_p - '0'));
            str_p--;
            jj = 1;
            while (jj < in_str_len)
            {
                if (!isdigit((unsigned) (*str_p)))
                {
                    /*
                     * If a non-digit is encountered then consider it as an indication
                     * that the whole number is zero (illegal)
                     */
                    cli_out("%s(): A non digit was found in input decimal-representation string: %s.\r\n"
                        "Setting it to zero\r\n",__func__,str);
                    sal_memset(val, 0, nval * sizeof (*val));
                    goto done;
                }
                COMPILER_64_COPY(mul,power);
                /*
                 * Multiply current digit with the correponding power of 10.
                 */
                COMPILER_64_UMUL_32(mul,(*str_p - '0'));
                COMPILER_64_COPY(prev_sum,sum);
                COMPILER_64_ADD_64(sum,mul);
                if (COMPILER_64_GT(prev_sum,sum))
                {
                    /*
                     * Overflow indicating more than 64 bits are required to represent this decimal integer.
                     * Consider the whole decimal value as '0'.
                     */
                    cli_out("%s(): Input decimal-representation string - %s - needs more than a 64 bits container.\r\n"
                        "Setting it to zero. Use hexadecimal representation instead.\r\n",__func__,str);
                    sal_memset(val, 0, nval * sizeof (*val));
                    goto done;
                }
                jj++;
                 ...
                 */
                COMPILER_64_UMUL_32(power,10);
                str_p--;
            }
            val[ii] = COMPILER_64_LO(sum);
            ii++;
            val[ii] = COMPILER_64_HI(sum);
        }
        goto done;
    }
    ...
    do {
        ...
        *--t = '0';
        ...
        val[ii++] = parse_integer(t);
    } while ((*s != 'x' && *s != 'X') && ii < nval);

 done:
    if (neg) {
        uint32 cy = 1;
        for (ii = 0; ii < nval; ii++) {
            if ((val[ii] = (~val[ii]) + cy) != 0) {
                cy = 0;
            }
        }
    }
...
parse_pbmp(int unit, char *s, soc_pbmp_t *pbmp)
{
    ...
    if (SOC_IS_LTSW(unit)) {
        int rv, is_cna;
        rv = soc_ltsw_is_cna(unit, &is_cna);
        if (rv < 0) {
            return rv;
        } else if (is_cna) {
#if defined(BCM_CTSW_SUPPORT)
            return ctsw_parse_pbmp(unit, s, pbmp);
#endif
        } else {
#if defined(BCM_LTSW_SUPPORT)
    ...
    while (*s) {
        ...
        if (sn == s) {                          /* unprefixed number */
            ...
            *sn = '\0';
            ...
            } else if (sal_strcasecmp(s, "d3c") == 0) {
                bmall = PBMP_D3CE_ALL(unit);
		...
		BCM_PBMP_AND(bmall, PBMP_SFI_ALL(unit));
                ...
                SOC_PBMP_REMOVE(bmall, PBMP_OTN_ALL(unit));
#endif
            } else if (sal_strcasecmp(s, "cpu") == 0) {
                bmall = PBMP_CMIC(unit);
            } else if (sal_strcasecmp(s, "rcy") == 0) {
                bmall = PBMP_RCY_ALL(unit);
            ...
            } else if (sal_strcasecmp(s, "otn") == 0) {
                bmall = PBMP_OTN_ALL(unit);
            } else {
                /* parse block names */
                matched = 0;
                SOC_PBMP_CLEAR(bmall);
            ...
            case '\0':
                ...
                for (port = pfirst; port <= plast; port += pstep) {  /* a range */
                    if (SOC_PORT_VALID_RANGE(unit, port)) {
                        if (SOC_PBMP_NOT_NULL(bmall) &&
                            !SOC_PBMP_MEMBER(bmall, port)) {
                            continue;   /* skip gaps in range */
                        }
                        if (complement) {
...
/*                return -1;                       error: range without port */
        ...
        case '\0':
            ...
            for (p = pfirst; p <= plast; p += pstep) {  /* a range */
                if (SOC_PORT_VALID_RANGE(unit, p)) {
...
#if defined(BCM_CTSW_SUPPORT)
            return ctsw_parse_port(unit, s, portp);
#endif
        } else {
#if defined(BCM_LTSW_SUPPORT)
...
format_pbmp(int unit, char *buf, int bufsize, soc_pbmp_t pbmp)
{
...
#if defined(BCM_CTSW_SUPPORT)
            return ctsw_format_pbmp(unit, buf, bufsize, pbmp);
#endif
        } else {
#if defined(BCM_LTSW_SUPPORT)
    ...
    if (SOC_PBMP_NOT_NULL(tpbm)) {
        ...
        _format_pbmp_type(unit, tmp, FORMAT_PBMP_MAX, pbmp, "d3c", PBMP_D3CE_ALL(unit));
        ...
        if (soc_feature(unit, soc_feature_configure_loopback_port)) {
            _format_pbmp_type(unit, tmp, FORMAT_PBMP_MAX, pbmp, "lb", PBMP_LB_ALL(unit));
        }
        _format_pbmp_type(unit, tmp, FORMAT_PBMP_MAX, pbmp, "otn", PBMP_OTN_ALL(unit));
...
#if defined(BCM_CTSW_SUPPORT)
            return ctsw_port_name(unit, port);
#endif
        } else {
#if defined(BCM_LTSW_SUPPORT)
...
format_field_qualifier(char *buf, bcm_field_qualify_t qualifier, int brief)
{
    ...
    if (0 <= (int)qualifier && qualifier < bcmFieldQualifyCount) {
        if (brief) {
...
parse_field_qset(char *str, bcm_field_qset_t *qset) {
    ...
    sal_strlcpy(str_copy, str, BCM_FIELD_QSET_WIDTH_MAX);

    buf = sal_strtok_r(str_copy, delimiters, &tokstr);

    do {
        qual = parse_field_qualifier(buf);
        if ((int)qual != BCM_FIELD_ENTRY_INVALID) {
    ...
    for (qual = 0; qual < bcmFieldQualifyCount; qual++) {
        if(BCM_FIELD_QSET_TEST(qset, qual)) {
            if (first_print == 1) {
                ...
                sal_strncat_s(buf, separator, BCM_FIELD_QSET_WIDTH_MAX);
            }
            format_field_qualifier(buf_local, qual, 1);
            sal_strncat_s(buf, buf_local, BCM_FIELD_QSET_WIDTH_MAX);
        }
    }
    sal_strncat_s(buf, "}", BCM_FIELD_QSET_WIDTH_MAX);
...
format_field_group_mode(char *buf, bcm_field_group_mode_t mode, int brief)
{
    ...
    if (0 <= (int)mode && mode < bcmFieldGroupModeCount) {
        if (brief) {
...
format_field_action(char *buf, bcm_field_action_t action, int brief)
{
    ...
    if (0 <= (int)action && action < bcmFieldActionCount) {
        if (brief) {
...
parse_field_aset(char *str, bcm_field_aset_t *aset, uint8 add) {
    ...
    sal_strlcpy(str_copy, str, BCM_FIELD_ASET_WIDTH_MAX);

    buf = sal_strtok_r(str_copy, delimiters, &tokstr);

    do {
        action = parse_field_action(buf);
        if ((int)action != BCM_FIELD_ENTRY_INVALID) {
            if (add) {
    ...
    for (action = 0; action < bcmFieldActionCount; action++) {
        if(BCM_FIELD_ASET_TEST(aset, action)) {
            if (first_print == 1) {
                ...
                sal_strncat_s(buf, separator, BCM_FIELD_ASET_WIDTH_MAX);
            }
            format_field_action(buf_local, action, 1);
            sal_strncat_s(buf, buf_local, BCM_FIELD_ASET_WIDTH_MAX);
        }
    }
    sal_strncat_s(buf, "}", BCM_FIELD_ASET_WIDTH_MAX);
...
format_field_decap(char *buf, bcm_field_decap_t decap)
{
    ...
    if (0 <= (int)decap && decap < bcmFieldDecapCount) {
