...
#define DIAG_DNXC_PHY_FECSTAT_NOF_COUNTERS       3
...
#define DIAG_DNXC_PHY_RAW_SBUS_FULL_MAN     "Set or Get phy registers through sbus. " \
...
#define DIAG_DNXC_PHY_BER_PROJ_BRIEF_MAN    "Diagnostic for PHY BER projection"

#define DIAG_DNXC_PHY_BER_PROJ_FULL_MAN     "This diag is used to calculate Post FEC BER projection. " \
...
#define DIAG_DNXC_PHY_BER_PROJ_FEC_MODE_FULL_MAN     "This diag is used to calculate Post FEC BER projection. " \
                                            "Option \'hist_errcnt_threshold\' is set to 0 by default, which means BER projection " \
                                            "tool will choose an optimized threshold automatically. The other acceptable values " \
                                            "for this threshold are [1~7]. This command supports Post FEC BER projection calculation across " \
                                            "multiple lanes and multiple ports simultaneously. \n" \
                                            "Please NOTE: \n" \
                                            "1. BER projection is only available with SERDES_API_FLOATING_POINT compilation flag. \n" \
                                            "2. BER projection checker is working only after setting PRBs manually, unless in FEC mode. \n" \
                                            "3. The option \'hist_errcnt_threshold\' is meaningless for Blackhawk 7nm devices.\n" \
                                            "4. BER projection doesn't support working on ILKN across 2 PMs. \n"

#define DIAG_DNXC_PHY_BER_PROJ_SYNOPSIS     "<pbmp>[hist_errcnt_threshold=<val>] [sample_time=<val>]"
#define DIAG_DNXC_PHY_BER_PROJ_FEC_MODE_SYNOPSIS     "<pbmp> [fec] [hist_errcnt_threshold=<val>] [sample_time=<val>]"

#define DIAG_DNXC_PHY_BER_PROJ_EXAMPLES     "1 \n" \
...
extern int dnx_imb_fabric_mdio_address_get(
    int unit,
    int pm_index,
    uint16 *phy_addr);
...
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit) && !dnxc_init_is_init_done_get(unit))
    {
        SHR_EXIT();
    }
#endif
    ...
    bcm_pbmp_t * pbm)
{
    ...
    int pm_id, phy_offset_in_pm;
    int ilkn_lane_arg, nof_lanes;
    ...
    BCM_PBMP_ITER(*pbm, port)
    {
        ...
        if (IS_IL_PORT(unit, port))
        {
            ...
            ilkn_lane_arg = phy_iter->lane;
            if (ilkn_lane_arg >= 0)
            {
                SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_get(unit, port, 0, &ilkn_phys));
                BCM_PBMP_COUNT(ilkn_phys, nof_lanes);
                if (ilkn_lane_arg >= nof_lanes)
                {
                    SHR_CLI_EXIT(_SHR_E_PARAM, "lane=%d is out of the ilkn port! \n", ilkn_lane_arg);
                }
                SHR_IF_ERR_EXIT(bcm_dnx_port_ilkn_lane_map_get(unit, 0, port, nof_lanes, lanes));
                phy = lanes[ilkn_lane_arg];
                SHR_IF_ERR_EXIT(dnx_algo_port_nif_ilkn_is_over_fabric_get(unit, port, &is_over_fabric));
                SHR_IF_ERR_EXIT(dnx_algo_port_ilkn_phy_pms_get(unit, port, is_over_fabric, phy,
                                                               &pm_id, &phy_offset_in_pm));
                /*
                 * Found the real lane id of the PM, replace it since phy mod diag
                 * need the lane id of the PM.
                 */
                phy_iter->lane = phy_offset_in_pm;

                need_check_pm = TRUE;
                if (is_over_fabric)
                {
                    uint16 addr;
                    SHR_IF_ERR_EXIT(dnx_imb_fabric_mdio_address_get(unit, pm_id, &addr));
                    pm_phy_addr = addr;
                }
                else
                {
                    pm_phy_addr = dnx_data_nif.eth.pm_properties_get(unit, pm_id)->phy_addr;
                }
            }
        }
#endif
        ...
        {
            if (need_check_pm && pm_phy_addr != phy_access[i].access.addr)
            {
                continue;
            }
...
/**
 * \brief - helper function to clear prbs counters and set last time cleared (supports DNX ILKN ports spread over multiple PMs)
 */
static shr_error_e
dnxc_phy_prbs_counters_clear(
    int unit,
    bcm_port_t port)
{
    int status;
    ...
    if (SOC_IS_DNX(unit) && !IS_SFI_PORT(unit, port))
    {
        ...
        BCM_PBMP_ITER(nif_phys, lane)
        {
            ...
            SHR_IF_ERR_EXIT(bcm_port_control_get(unit, lane_port, bcmPortControlPrbsRxStatus, &status));
            ++lane_count;
        }
    }
    else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(pbmp, port)
    {
         ...
         * clear errors
         */
        SHR_IF_ERR_EXIT(dnxc_phy_prbs_counters_clear(unit, port));
    }
exit:
...
static shr_error_e
dnxc_phy_diag_fecstat_counter_bit_error_compenaste(
    int unit,
    bcm_port_t port,
    uint64 error_count,
    uint64 *compensated_error_count)
{
    bcm_port_resource_t resource;
    int total_bits = 0, counted_bits = 0;
    ...
    fec_type = resource.fec_type;

    if (SOC_IS_DNX(unit))
    {
#ifdef BCM_DNX_SUPPORT
        counted_bits = dnx_data_port.general.fec_compensation_info_get(unit, fec_type)->counted_bits;
        total_bits = dnx_data_port.general.fec_compensation_info_get(unit, fec_type)->total_bits;
#endif
    }
    else
    {
#ifdef BCM_DNXF_SUPPORT
        counted_bits = dnxf_data_port.general.fec_compensation_info_get(unit, fec_type)->counted_bits;
        total_bits = dnxf_data_port.general.fec_compensation_info_get(unit, fec_type)->total_bits;
#endif
    }

    /** initial value */
    *compensated_error_count = error_count;

    /** if the FEC requires compensation, do the math */
    if (total_bits != 0)
    {
        *compensated_error_count = error_count * total_bits / counted_bits;
    }
...
static shr_error_e
dnxc_phy_diag_fecstat_counter_bit_error_rate_compenaste(
    int unit,
    bcm_port_t port,
    uint32 error_rate,
    uint32 *compensated_error_rate)
{
    bcm_port_resource_t resource;
    int total_bits = 0, counted_bits = 0;
    bcm_port_phy_fec_t fec_type;
    ...
    /** initial value */
    *compensated_error_rate = error_rate;

    /** if the FEC requires compensation, do the math */
    if (total_bits != 0)
    {
        *compensated_error_rate = error_rate * total_bits / counted_bits;
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        SHR_CLI_EXIT(_SHR_E_PARAM,
                     "PHY RAW SBUS can't be used with portmodDispatchTypePm8x50_fabric PM type.\n Please use PHY RAW C45 instead.\n");
    }
...
static shr_error_e
dnxc_phy_ber_proj_fec_mode(
    int unit,
    bcm_pbmp_t pbmp,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    int hist_errcnt_threshold;
    int sample_time;

    SHR_FUNC_INIT_VARS(unit);

    SH_SAND_GET_INT32("hist_errcnt_threshold", hist_errcnt_threshold);
    SH_SAND_GET_INT32("sample_time", sample_time);

    /**
     * hist_errcnt_threshold option is not used in fec mode 
     */
    if (hist_errcnt_threshold)
    {
        SHR_CLI_EXIT(_SHR_E_PARAM, "FEC mode is not supported alongside hist_errcnt_threshold option\n");
    }

    SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fec_ber_projection_display(unit, pbmp, sample_time));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sh_cmd_is_fec_ber_proj_supported(unit, NULL) == _SHR_E_NONE)
    {
        SH_SAND_GET_BOOL("fec", fec_mode);
    }

    if (fec_mode)
    {
        SHR_IF_ERR_EXIT(dnxc_phy_ber_proj_fec_mode(unit, pbmp, args, sand_control));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_phy_diag_pbmp_check(unit, &pbmp));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_stat_val_t counter_type[DIAG_DNXC_PHY_FECSTAT_NOF_COUNTERS] =
        { snmpBcmRxRsFecBitError, snmpBcmRxFecCorrectable, snmpBcmRxFecUncorrectable };
    char *counter_name[DIAG_DNXC_PHY_FECSTAT_NOF_COUNTERS] =
        { "FEC Bit Error Counter", "FEC Correctable Counter", "FEC Uncorrectable Counter" };
    ...
    BCM_PBMP_ITER(logical_ports, port)
    {
        for (i = 0; i < DIAG_DNXC_PHY_FECSTAT_NOF_COUNTERS; ++i)
        {
            ...
            if (i == DIAG_DNXC_PHY_FECSTAT_NOF_COUNTERS - 1)
            {
                /** last counter */
                PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
                PRT_CELL_SET("%s", "");
            ...
            {
                /** not last and not first counter */
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", "");
            }
            ...
            {
                /** prepare counter_val, counter_diff and rate */
                ...
                /** compensate values for snmpBcmRxRsFecBitError */
                if (snmpBcmRxRsFecBitError == counter_type[i])
                {
                    /** compensate counter_val */
                    SHR_IF_ERR_EXIT(dnxc_phy_diag_fecstat_counter_bit_error_compenaste
                                    (unit, port, counter_val, &counter_val));

                    /** compensate counter_diff */
                    SHR_IF_ERR_EXIT(dnxc_phy_diag_fecstat_counter_bit_error_compenaste
                                    (unit, port, counter_diff, &counter_diff));

                    /** compensate rate */
                    SHR_IF_ERR_EXIT(dnxc_phy_diag_fecstat_counter_bit_error_rate_compenaste(unit, port, rate, &rate));
                }

                /*
                 * should be called after compensation (if needed)
                 * since phy_fecstat_val stores compensated values
                 */
                ...
                /** store values towards next sample */
    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(logical_ports, port)
    {
        ...
        SHR_IF_ERR_EXIT(dnxc_phy_diag_fecstat_counter_rate_get(unit, port, snmpBcmRxRsFecBitError, &rate));
        if (rate == 0)
        {
            PRT_CELL_SET("%s", "N/A");
        }
...
/**
 * \brief - CMD to enable/disable port tx squelch
 *
 */
static shr_error_e
cmd_dnxc_port_tx_squelch_set(
    int unit,
    bcm_port_t port,
    phymod_phy_tx_lane_control_t tx_lane_ctrl)
{
    phymod_phy_access_t phy_access_arr[PORTMOD_MAX_ILKN_AGGREGATED_PMS];
    portmod_access_get_params_t params;
    int phys_num = 0, phy_index;
    ...
    SHR_IF_ERR_EXIT(portmod_port_phy_lane_access_get(unit, port, &params, PORTMOD_MAX_ILKN_AGGREGATED_PMS,
                                                     phy_access_arr, &phys_num, NULL));

    for (phy_index = 0; phy_index < phys_num; phy_index++)
    {
        SHR_IF_ERR_EXIT(phymod_phy_tx_lane_control_set(&phy_access_arr[phy_index], tx_lane_ctrl));
    }
...
{
    phymod_phy_tx_lane_control_t tx_lane_ctrl;
    int tx_squelch, tx_squelch_present;
    int enable, enable_present;
    bcm_port_t port;
    bcm_pbmp_t pbmp;
    ...
    SH_SAND_GET_PORT("pbmp", pbmp);
    SH_SAND_IS_PRESENT("enable", enable_present);
    SH_SAND_IS_PRESENT("tx_squelch", tx_squelch_present);

    if (tx_squelch_present && enable_present)
    {
        SHR_CLI_EXIT(_SHR_E_PARAM, "Option ENAble cannot be provided together with TX_SQuelCh option!\n");
    }

    if (tx_squelch_present)
    {
        SH_SAND_GET_BOOL("tx_squelch", tx_squelch);
        tx_lane_ctrl = tx_squelch ? phymodTxSquelchOn : phymodTxSquelchOff;

        BCM_PBMP_ITER(pbmp, port)
        {
            SHR_IF_ERR_EXIT(cmd_dnxc_port_tx_squelch_set(unit, port, tx_lane_ctrl));
        }
    }
...
#ifdef BCM_DNX_SUPPORT
    {"sbus",    cmd_dnxc_phy_raw_sbus,        NULL,       dnxc_phy_raw_sbus_options,     &dnxc_phy_raw_sbus_man,  NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_SKIP_EXEC, sh_cmd_is_dnx},
#endif
    {NULL}
};
...
static sh_sand_man_t dnxc_phy_set_man = {
    ...
    .full     = "Configure the PHY reigster according the given ports and register info. "
                ...
                "each lane and each PLL. For eth port, the lane is representing the lane offset "
                "in the PM; For ILKN port, the lane is representing the ilkn lane id.",
...
static sh_sand_man_t dnxc_phy_get_man = {
    ...
    .full     = "Get the PHY reigster according the given ports, register info and pattern.\n"
                ...
                "For eth port, the lane is representing the lane offset in the PM; For ILKN port, "
                "the lane is representing the ilkn lane id.",
...
static sh_sand_man_t dnxc_phy_ber_proj_man_fec_mode = {
    .brief    = DIAG_DNXC_PHY_BER_PROJ_BRIEF_MAN,
    .full     = DIAG_DNXC_PHY_BER_PROJ_FEC_MODE_FULL_MAN
                "5. FEC mode is supported only on fabric ports."
                ,
    .synopsis = DIAG_DNXC_PHY_BER_PROJ_FEC_MODE_SYNOPSIS,
    .examples = DIAG_DNXC_PHY_BER_PROJ_EXAMPLES
                "\nfec sfi sample_time=30"
};

static sh_sand_man_t dnxc_phy_ber_proj_man = {
    .brief    = DIAG_DNXC_PHY_BER_PROJ_BRIEF_MAN,
    .full     = DIAG_DNXC_PHY_BER_PROJ_FULL_MAN,
    .synopsis = DIAG_DNXC_PHY_BER_PROJ_SYNOPSIS,
    .examples = DIAG_DNXC_PHY_BER_PROJ_EXAMPLES
};

static sh_sand_option_t dnxc_phy_ber_proj_options_fec_mode[] = {
    /*name                    type                  desc          default    ext*/
    {"fec",                   SAL_FIELD_TYPE_BOOL, "Run BER Projection in FEC mode (under normal traffic, without enabling PRBS)", "false", NULL},
    {"pbmp",                  SAL_FIELD_TYPE_PORT, "port # / logical port type / port name", NULL,   NULL, NULL, SH_SAND_ARG_FREE},
...
#ifdef SERDES_API_FLOATING_POINT
    {"berproj",   cmd_dnxc_phy_ber_proj,  NULL,                   dnxc_phy_ber_proj_options_fec_mode,   &dnxc_phy_ber_proj_man_fec_mode, NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, sh_cmd_is_fec_ber_proj_supported},
    {"berproj",   cmd_dnxc_phy_ber_proj,  NULL,                   dnxc_phy_ber_proj_options,            &dnxc_phy_ber_proj_man,          NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, sh_cmd_is_fec_ber_proj_not_supported},
#endif
#ifdef PHYMOD_LINKCAT_SUPPORT
...
sh_sand_man_t sh_dnxc_port_enable_man = {
    ...
    .full     = "Enable or Disable port, enable or disable port tx squelch",
    .synopsis = "<pbmp> [<Yes|No|True|False>] [tx_squelch=<True|False>]",
    .examples = "sfi no \n"
                "sfi yes \n"
                "1 false \n"
                "1 yes \n"
...
sh_sand_option_t sh_dnxc_port_enable_options[] = {
    {"pbmp",   SAL_FIELD_TYPE_PORT, "port # / logical port type / port name", "all", NULL, NULL, SH_SAND_ARG_FREE},
    {"enable", SAL_FIELD_TYPE_BOOL, "Enable or Disable port",                 "true", NULL, NULL, SH_SAND_ARG_FREE},
    {"tx_squelch", SAL_FIELD_TYPE_BOOL, "Enable or disable port tx squelch, 1: enable, 0: disable", "false", NULL, NULL},
    {NULL}
};
