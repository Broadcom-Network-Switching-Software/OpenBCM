...
char cmd_dpp_knet_usage[] =
    ...
    "          [MirrorSts=<val>]    - Match on Mirror status\n"
...
STATIC char *dpp_knet_netif_type[] = {
    "unknown", "Vlan", "Port", "Meta", NULL
};

/* Note:  See knet.h, BCM_KNET_DEST_T_xxx */
STATIC char *dpp_knet_filter_dest_type[] = {
    "Null", "NetIF", "RxAPI", "CallBack", NULL
};

STATIC char *dpp_knet_reason_str[bcmRxReasonCount+1] = BCM_RX_REASON_NAMES_INITIALIZER;

STATIC void
dpp_knet_show_netif(int unit, bcm_knet_netif_t *netif)
{
    char *type_str = "?";
    char *port_str = "n/a";
    ...
    case BCM_KNET_NETIF_T_TX_META_DATA:
        type_str = dpp_knet_netif_type[netif->type];
        break;
...
STATIC int
dpp_knet_trav_netif(int unit, bcm_knet_netif_t *netif, void *user_data)
{
    int *count = (int *)user_data;

    dpp_knet_show_netif(unit, netif);
...
STATIC void
dpp_knet_show_filter(int unit, bcm_knet_filter_t *filter)
{
    char *dest_str = "?";
    char proto_str[16];
    int idx, edx;
    ...
    case BCM_KNET_DEST_T_CALLBACK:
        dest_str = dpp_knet_filter_dest_type[filter->dest_type];
        break;
    default:
        break;
    }
...
STATIC int
dpp_knet_trav_filter(int unit, bcm_knet_filter_t *filter, void *user_data)
{
    int *count = (int *)user_data;

    dpp_knet_show_filter(unit, filter);
...
cmd_dpp_knet(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "netif") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "create") == 0) {
            ...
            parse_table_add(&pt, "Type", PQ_DFL|PQ_MULTI, 0, &if_type, dpp_knet_netif_type);
            ...
            count = 0;
            if (bcm_knet_netif_traverse(unit, dpp_knet_trav_netif, &count) < 0) {
                return CMD_FAIL;
            }
            if (count == 0) {
        ...
        if (sal_strcasecmp(subcmd, "create") == 0) {
            ...
            parse_table_add(&pt, "MirrorSts", PQ_DFL|PQ_INT, 0, &pf_mirror_sts, 0);
            ...
            if (pf_mirror_sts >= 0) {
                filter.m_mirrored = (pf_mirror_sts) ? 1 : 0;
            ...
            count = 0;
            if (bcm_knet_filter_traverse(unit, dpp_knet_trav_filter, &count) < 0) {
                return CMD_FAIL;
            }
            if (count == 0) {
                cli_out("<no filters>\n");
