...
test_run_oam(int unit, int port_1, int port_2, int numEndpoints, int seed, int numUpdate, int testSLM, int is_maid48) {
    ...
    */
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        if (SOC_IS_QUX(unit)) {
            /* First 2 EP create take 125000usec due to configuration of OAM1/2 entries. Due to QUX DMA operation needs more time. */
                 max_total_endpoint_create_time  = 1.1 * (125000 + 4000 * (MEPnum - 2) + 4 * (MEPnum - 2) * (MEPnum - 2)); /* Max time in usec */
          } 
        else if (SOC_IS_JERICHO_PLUS(unit) && is_system_64b) {
            /** Updated from 21.09.2020 from 3000 to 3100 */
            /** Updated from 05.10.2020 from 3100 to 4000 */
            max_total_endpoint_create_time  = 1.1 * (120000 + 4000 * (MEPnum - 2) + 4 * (MEPnum - 2) * (MEPnum - 2)); /* Max time in usec */
        } else {
    ...
    if (time_no > 0) {
        max_total_endpoint_create_time  +=((2600 * time_no) + (testSLM * time_slm * time_no_slm)/*additional time in case of SLM entry*/);
