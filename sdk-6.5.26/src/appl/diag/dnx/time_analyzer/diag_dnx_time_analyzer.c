    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (module_index = 0; module_index < nof_modules; module_index++)
    {
        ...
        /** if bigger than 10 seconds - print in seconds units  - otherwise milliseconds*/
        if (diag_info[module_index].time > /* 10 seconds */ 10 * 1000 * 1000)
        {
        ...
        PRT_CELL_SET("%d [microseconds]", diag_info[module_index].worst);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (; module_idx < last_module; module_idx++)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (COUNTOF(diag_dnx_init_time_analyzer_ops) > UTILEX_TIME_ANALYZER_MAX_NOF_MODULES)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "The number of entries in diag_dnx_init_time_analyzer_ops is bigger that the maximum allowed nof modules: nof_entries:%d, max_nof_modules=%d \n",
                     COUNTOF(diag_dnx_init_time_analyzer_ops), UTILEX_TIME_ANALYZER_MAX_NOF_MODULES);
    }
    if (module_id < 0)
    {
        module_idx = 0;
    ...
    if (last_module > COUNTOF(diag_dnx_init_time_analyzer_ops) || last_module < 0)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Index of the last module is out of bounds: last module:%d, max_index=%d \n",
                     last_module, COUNTOF(diag_dnx_init_time_analyzer_ops));
    }
