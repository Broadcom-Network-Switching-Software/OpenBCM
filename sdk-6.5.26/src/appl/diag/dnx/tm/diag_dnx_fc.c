...
static sh_sand_option_t sh_dnx_fc_cfc_options[] = {
    ...
    {"OOB_InterFace",   SAL_FIELD_TYPE_INT32, "Select which OOB interface to show",             "-1",         NULL, NULL, SH_SAND_ARG_QUIET},
...
static sh_sand_man_t sh_dnx_fc_cfc_man = {
    ...
    .examples =
        ...
        "Direction=GEN Type=ILKN_LLFC ILKN_ID=0\n Direction=REC Type=NIF_PFC Index=8 Count=10"
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_llfc_handled_in_cfc)) &&
        ((type == DNX_FC_NIF_TYPE_STOP_FROM_CFC) || (type == DNX_FC_NIF_TYPE_STOP_TO_PM)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Type is invalid for device\n");
    }
    ...
    for (type = type_start; type <= type_end; type++)
    {
        ...
        BCM_PBMP_ITER(logical_ports, logical_port_i)
        {
            ...
            if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, L1))
                && (type != DNX_FC_NIF_TYPE_LLFC_TO_ILKN))
            {
            ...
            if ((!DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 0)) && (type == DNX_FC_NIF_TYPE_LLFC_TO_ILKN))
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_llfc_handled_in_cfc))
        && ((type == DNX_FC_TYPE_AF_NIF) || (type == DNX_FC_TYPE_NIF_STOP_PM)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Type is not valid for device. \n");
    }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "OOB is not valid for device. \n");
    }
        ...
        for (if_current = if_start; if_current <= if_end; if_current++)
        {
            ...
            switch (type)
            {
                ...
                case DNX_FC_TYPE_AF_NIF:
                     ...
                     * Set maximum ID
                     */
                    max_range = dnx_data_nif.phys.max_phys_in_core_get(unit);
                ...
                case DNX_FC_TYPE_NIF_PFC:
                     ...
                     * Set maximum ID
                     */
                    max_range = dnx_data_nif.phys.max_phys_in_core_get(unit) * BCM_COS_COUNT;
                ...
                case DNX_FC_TYPE_VSQ_LLFC:
                    ...
                    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_fc_from_llfc_vsq_status_sel))
                    {
                        field_select = DBAL_FIELD_VSQ_LLFC_STATUS_SEL;
                    }
                    else
                    {
                        field_select = DBAL_FIELD_EMPTY;
                        table_select = DBAL_TABLE_EMPTY;
                    }
                    break;
            ...
            if ((is_ilkn == 1) && (type != DNX_FC_TYPE_INB_ILKN)
                && (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_ilu_fc_status_per_tdm)))
            {
                ilkn_tdm_status = 1;
            }
            else
            ...
            DNXCMN_CORES_ITER(unit, core_id, core)
            {
                ...
                {
                    for (is_tdm = 0; is_tdm <= ilkn_tdm_status; is_tdm++)
                    {
                        /*
                        ...
                        if (select != select_prev)
                        {
                            ...
                            else if ((is_ilkn == 1) && (type != DNX_FC_TYPE_INB_ILKN))
                            {
                                ...
                                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_PORT_ID,
                                                           (id_current % dnx_data_nif.ilkn.ilkn_unit_if_nof_get(unit)));
                                if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_ilu_fc_status_per_tdm))
                                {
                            ...
                            {
                                dbal_value_field_arr32_request(unit, entry_handle_id, field_status, INST_SINGLE,
                                                               status);
                                SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
                            }
                        ...
                        if (!(only_on_indications) || (enabled))
                        {
                            ...
                            if ((is_ilkn == 1) && (type != DNX_FC_TYPE_INB_ILKN)
                                && (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_ilu_fc_status_per_tdm)))
                            {
                                PRT_CELL_SET("%s", (is_tdm ? "TRUE" : "FALSE"));
                            }
                            PRT_CELL_SET("%s", (enabled ? "ON" : "OFF"));
                        }
                    }
...
sh_sand_cmd_t sh_dnx_fc_cmds[] = {
    ...
    {"IPS",        sh_dnx_fc_ips_cmd,      NULL,                        sh_dnx_fc_ips_options,   &sh_dnx_fc_ips_man, NULL, NULL, SH_CMD_CONDITIONAL * CTEST_ADAPTER_UNSUPPORTED, sh_fc_ips_is_not_8_prio},
    {NULL}
};
/* *INDENT-ON* */
