...
typedef struct
{
    ...
    uint32 latency_voq_reject_probability[DNX_COSQ_NOF_DP][DNX_DATA_MAX_LATENCY_FEATURES_LATENCY_BINS];
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int nof_latency_bins = dnx_data_latency.features.latency_bins_get(unit);
    ...
    if ((!is_sys_red_enabled || !shared_resource)
        && dnx_data_latency.features.feature_get(unit, dnx_data_latency_features_ingress_based_admission))
    {
        PRT_TITLE_SET("Rate class %d VOQ Latency Admission Test Info", rate_class);
        PRT_COLUMN_ADD("Color");
        PRT_COLUMN_ADD("Enable");
        PRT_COLUMN_ADD("Min Threshold");
        for (latency_bin = 0; latency_bin < nof_latency_bins; latency_bin++)
        {
    ...
    for (poll_idx = 0; poll_idx < poll_nof_times; poll_idx++)
    {
        ...
        DNXCMN_CORES_ITER(unit, core_id, core)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_iterator_value_field32_rule_add
                            (unit, entry_handle_id, DBAL_FIELD_SIZE_IN_WORDS, INST_SINGLE, DBAL_CONDITION_NOT_EQUAL_TO,
                             (uint32) 0, 0));
 ...
 * { 
 */
/* Global variable used in both init and deinit callback functions*/
rhhandle_t packet_h = NULL;
/**
 * \brief - init callback used to generate traffic
 * note - the generated traffic must be on a queue different from the default examples used for gold results
 * This way we check if the filter option is working
 */
static shr_error_e
sh_dnx_tm_queue_count_init_cb(
    int unit)
{
    bcm_port_t src_port = 13, dest_port = 14;
    bcm_port_interface_info_t interface_info;
    bcm_port_mapping_info_t mapping_info;
    uint32 flags;
    bcm_core_t core_id_src, core_id_dest;
    int stream_rate = 1000000;  /* Rate 1GB */
    int packet_size = 128, stream_duration = 2, stream_burst = 100000, stream_granularity = 0;
    int type = BCM_SAT_GTF_RATE_IN_BYTES;
    SHR_FUNC_INIT_VARS(unit);

    if (dnx_data_sat.general.feature_get(unit, dnx_data_sat_general_is_sat_supported))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, src_port, &core_id_src));
        SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, dest_port, &core_id_dest));
        SHR_IF_ERR_EXIT(bcm_port_get(unit, src_port, &flags, &interface_info, &mapping_info));

        SHR_IF_ERR_EXIT(bcm_port_force_forward_set(0, src_port, dest_port, 1));
        ...
        SHR_IF_ERR_EXIT(diag_sand_packet_stream_create(unit, core_id_src, packet_h, packet_size, stream_duration,
...
/**
 * \brief - deinit callback used to stop traffic
 */
static shr_error_e
sh_dnx_tm_queue_count_deinit_cb(
    int unit)
{
    bcm_port_t src_port = 13, dest_port = 14;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (dnx_data_sat.general.feature_get(unit, dnx_data_sat_general_is_sat_supported))
    {
        ...
        SHR_IF_ERR_EXIT(bcm_port_force_forward_set(0, src_port, dest_port, 0));
        sh_process_command(unit, "clear cou");
    }
exit:
    if (dnx_data_sat.general.feature_get(unit, dnx_data_sat_general_is_sat_supported))
    {
...
/* *INDENT-OFF* */
    ...
    .full     = "Diagnostic to provide per Queue or per BaseQueue counter information \n"
                ...
                "The time interval is set to 100 microseconds by default",
    .synopsis = "[Queue=<id>] [BaseQueue=<id>] [InTerVal=<val>] [core=<id>]",
    .examples = "q=34 \n"
                "bq=32 \n"
                "q=34 itv=100",
    .init_cb = sh_dnx_tm_queue_count_init_cb,
    .deinit_cb = sh_dnx_tm_queue_count_deinit_cb
...
/* *INDENT-OFF* */
    ...
    {"interval",  SAL_FIELD_TYPE_UINT32, "Time interval",   "100"},
    {NULL}
/* *INDENT-ON* */
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        if (interval != 0)
        {
            gtimer_time.time_units = DNXC_TIME_UNIT_USEC;
...
/* *INDENT-OFF* */
    ...
    {"poll_nof_times",      SAL_FIELD_TYPE_INT32,  "Number of polling times",                    "1"},
    {"poll_delay",          SAL_FIELD_TYPE_INT32,  "Delay inbetween pollings",                   "0"},
    {"oversubscription",    SAL_FIELD_TYPE_BOOL,   "Set when device is oversubscribed. "
                                                   "Can cause degradation in performance!",       "false"},
    {NULL}
/* *INDENT-ON* */
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_INT32("poll_delay", poll_delay);
    SH_SAND_IS_PRESENT("most", most_provided);
    SH_SAND_GET_BOOL("oversubscription", oversubscription);
    ...
    for (poll_idx = 0; poll_idx < poll_nof_times; poll_idx++)
    {
        ...
        DNXCMN_CORES_ITER(unit, core_id, core)
        {
            ...
            while ((!is_end))
            {
                rv = dnx_ingress_congestion_vsq_occupancy_iterator_get_next(unit, core, oversubscription,
                                                                            entry_handle_id, &is_end);
                if (rv != _SHR_E_NONE)
                {
                   /**
                    * If the iterator failed to get the entry, continue with the next one
...
/* *INDENT-OFF* */
    ...
    .examples = "GRouP=a Queue=0 \n"
                ...
                "GRouP=f Queue=10 \n"
                "GRouP=f Queue=10 OVeRSuBscription",
/* *INDENT-ON* */
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_vsq_occupancy_iterator_get_next
                    (unit, core, oversubscription, entry_handle_id, &is_end));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (vsq_group_idx >= dnx_data_ingr_congestion.vsq.info_get(unit, vsq_group)->nof)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "VSQ group %s has only %d queues. Please give smaller number for 'id' input! \n",
                     sh_sand_enum_str(dnx_vsq_group_enum_table, vsq_group), dnx_data_ingr_congestion.vsq.info_get(unit,
                                                                                                                  vsq_group)->nof);
    }
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        SHR_IF_ERR_EXIT(sh_dnx_tm_vsq_occupancy_info_print
                        (unit, core, oversubscription, vsq_group, vsq_group_idx, sand_control));
    }
...
/* *INDENT-OFF* */
    ...
    {"non_empty",       sh_dnx_tm_vsq_non_empty_cmd,    NULL,       sh_dnx_tm_vsq_non_empty_options,    &sh_dnx_tm_vsq_non_empty_man,  NULL, NULL, SH_CMD_CONDITIONAL, sh_dnx_tm_skip_on_emulation},
    {NULL}
/* *INDENT-ON* */
};
