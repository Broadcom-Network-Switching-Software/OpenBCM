...
#include <bcm_int/dnx/auto_generated/dnx_bfd_dispatch.h>
...
#include <include/soc/dnx/dnx_data/auto_generated/dnx_data_meter.h>
#include <include/soc/dnx/swstate/auto_generated/access/oam_access.h>
#include <include/soc/dnxc/drv_dnxc_utils.h>
#include <include/soc/dnx/dbal/dbal_external_defines.h>
...
typedef struct
{
    ...
    int32 no_sbfd_reflector_over_ipv6;
...
#define NUMBER_OF_PROFILES (15)

extern int algo_oam_db_oam_profile_id_ing_is_allocated(
    int unit,
    int element,
    uint8 *is_allocated);

extern int algo_oam_db_oam_profile_id_eg_is_allocated(
    int unit,
    int element,
    uint8 *is_allocated);

extern int algo_oam_db_oam_profile_id_ing_acc_is_allocated(
    int unit,
    int element,
    uint8 *is_allocated);

extern int algo_oam_db_oam_profile_id_eg_acc_is_allocated(
    int unit,
    int element,
    uint8 *is_allocated);
    ...
    oam_cf_inputs_t * oam_cf_inputs)
{
    ...
    if (oam_cf_inputs->core_id > num_of_cores || !DNXCMN_CORE_IS_ENABLED(unit, oam_cf_inputs->core_id))
    {
        /*
         * Default core_id
         */
    ...
    if (oam_cf_inputs->lif == DNX_OAM_INVALID_LIF)
    {
        if (oam_cf_inputs->lif_1 == DNX_OAM_INVALID_LIF)
        {
            ...
            sal_snprintf(disp_line[13], sizeof(disp_line[13]),
                         "LAST     - Display classifier state from last packet\n");
            sal_snprintf(disp_line[0], sizeof(disp_line[0]),
                         "lif      - OAM-LIF - Global for egress, local for ingress\n");
            sal_snprintf(disp_line[1], sizeof(disp_line[1]), "lif_1    - Global for egress, local for ingress");
            if (dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 1)
            {
                sal_snprintf(disp_line[2], sizeof(disp_line[2]), "\nlif_2    - Global for egress, local for ingress\n");
                sal_snprintf(disp_line[3], sizeof(disp_line[3]), "lif_3    - Global for egress, local for ingress\n");
            }
            else
            {
            }
            sal_snprintf(disp_line[4], sizeof(disp_line[4]), "OPcode   - oam-opcode of the incoming packet\n");
            sal_snprintf(disp_line[5], sizeof(disp_line[5]), "mdl      - oam level of the incoming packet\n");
            sal_snprintf(disp_line[6], sizeof(disp_line[6]), "mymac    - Set if packet's dst equals mep's mac\n");
            sal_snprintf(disp_line[7], sizeof(disp_line[7]), "inj      - Set for injected packet\n");
            sal_snprintf(disp_line[8], sizeof(disp_line[8]), "da_is_mc - Set if multicast\n");
            sal_snprintf(disp_line[9], sizeof(disp_line[9]), "ydv      - Your Disc is Valid\n");
            sal_snprintf(disp_line[10], sizeof(disp_line[10]), "INGress - Set for ingress classifier\n");
            sal_snprintf(disp_line[11], sizeof(disp_line[11]), "BFD     - Set for BFD packet\n");
            sal_snprintf(disp_line[12], sizeof(disp_line[12]), "CoLoR   - Display in color. (0 for no-color)\n");
    ...
    {
        if (dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 0)
        {
            if ((oam_cf_inputs->lif_2 != DNX_OAM_INVALID_LIF) || (oam_cf_inputs->lif_3 != DNX_OAM_INVALID_LIF))
            {
                LOG_CLI((BSL_META_U(unit, "Invalid lif value. lif_2 and lif_3 not supported by the device.\n")));
                SHR_IF_ERR_EXIT(_SHR_E_PARAM);
            }
        }
        /*
    ...
    oam_cf_results_t * oam_cf_results)
{
    ...
    if (oam_cf_inputs.ydv == 0)
    {
        ...
        else
        {
            ...
            rv = dnx_lif_mngr_outlif_sw_info_get(unit, local_lif, &dbal_table_id, &dbal_result_type, NULL, NULL,
                                                 NULL, NULL);
    ...
    oam_cf_results_t * oam_cf_results)
{
   ...
   /** Assume - no PCP. */
    oam_cf_results->counter_inc_1 =
        ((!(oam_cf_results->oama_dis_1 | oam_cf_results->
            counter_disable_map_1 | oam_cf_results->profile_en_map_1)) & 1);
    oam_cf_results->counter_inc_2 =
        ((!(oam_cf_results->oama_dis_2 | oam_cf_results->
            counter_disable_map_2 | oam_cf_results->profile_en_map_2)) & 1);
    oam_cf_results->counter_inc_3 =
        ((!(oam_cf_results->oama_dis_3 | oam_cf_results->
            counter_disable_map_3 | oam_cf_results->profile_en_map_3)) & 1);
    SHR_EXIT();
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    oam_cf_results_t * oam_cf_results)
{
    ...
    {
        sal_snprintf(color_line, sizeof(color_line), "\033[96m");
        sal_snprintf(reg_color_line, sizeof(reg_color_line), "\033[0m");
     ...
     *  Left section
     */
    sal_snprintf(disp_line[60], sizeof(disp_line[60]), "pkt_is_bfd(0x%08x)-|                               ",
                 oam_cf_inputs->pkt_is_bfd);
    sal_snprintf(disp_line[0], sizeof(disp_line[0]), "opcode..(0x%08x)---|                               ",
                 oam_cf_inputs->opcode);
    sal_snprintf(disp_line[1], sizeof(disp_line[1]), "mymac...(0x%08x)---|                               ",
                 oam_cf_inputs->mymac);
    sal_snprintf(disp_line[2], sizeof(disp_line[2]), "inject..(0x%08x)---|-------------------------------",
                 oam_cf_inputs->inj);
    sal_snprintf(disp_line[3], sizeof(disp_line[3]), "da_is_mc(0x%08x)---|                               ",
                 oam_cf_inputs->da_is_mc);
    sal_snprintf(disp_line[4], sizeof(disp_line[4]), "                                                       ");
    sal_snprintf(disp_line[5], sizeof(disp_line[5]), "                     _______                           ");
    sal_snprintf(disp_line[6], sizeof(disp_line[6]), "                    |       |                          ");
    if (oam_cf_results->lif_1_entry_found)
    {
        sal_snprintf(disp_line[7], sizeof(disp_line[7]), "LIF-1(0x%08x)-->|       |--lif_key_1(0x%08x)---",
                     oam_cf_inputs->lif_1, oam_cf_results->lif_key_1);
    }
    else
    {
        sal_snprintf(disp_line[7], sizeof(disp_line[7]), "LIF-1(0x%08x)-->|       |%s--lif_key_1(0x%08x)---%s",
    ...
    if (dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 1)
    {
        if (oam_cf_results->lif_2_entry_found)
        {
            sal_snprintf(disp_line[8], sizeof(disp_line[8]), "LIF-2(0x%08x)-->|       |--lif_key_2(0x%08x)---",
                         oam_cf_inputs->lif_2, oam_cf_results->lif_key_2);
        }
        else
        {
            sal_snprintf(disp_line[8], sizeof(disp_line[8]), "LIF-2(0x%08x)-->|       |%s--lif_key_2(0x%08x)---%s",
                         oam_cf_inputs->lif_2, color_line, oam_cf_results->lif_key_2, reg_color_line);
        }
        if (oam_cf_results->lif_3_entry_found)
        {
            sal_snprintf(disp_line[9], sizeof(disp_line[9]), "LIF-3(0x%08x)-->|  KEY  |--lif_key_3(0x%08x)---",
                         oam_cf_inputs->lif_3, oam_cf_results->lif_key_3);
        }
        else
        {
            sal_snprintf(disp_line[9], sizeof(disp_line[9]), "LIF-3(0x%08x)-->|  KEY  |%s--lif_key_3(0x%08x)---%s",
    ...
    sal_snprintf(disp_line[10], sizeof(disp_line[10]), "LIF..(0x%08x)-->|       |                          ",
                 oam_cf_inputs->lif);
    sal_snprintf(disp_line[11], sizeof(disp_line[11]), "                    |  SEL  |                          ");
    sal_snprintf(disp_line[12], sizeof(disp_line[12]), "ydv..(0x%08x)-->|       |                          ",
                 oam_cf_inputs->ydv);
    sal_snprintf(disp_line[13], sizeof(disp_line[13]), "                    |_______|                          ");
    sal_snprintf(disp_line[14], sizeof(disp_line[14]), "                                                       ");
    sal_snprintf(disp_line[15], sizeof(disp_line[15]), "                                                       ");
    if (oam_cf_results->acc_entry_found)
    {
        sal_snprintf(disp_line[16], sizeof(disp_line[16]), "LIF..(0x%08x)--------------------------------------",
                     oam_cf_inputs->lif);
        sal_snprintf(disp_line[17], sizeof(disp_line[17]), "ydv..(0x%08x)--------------------------------------",
                     oam_cf_inputs->ydv);
        sal_snprintf(disp_line[18], sizeof(disp_line[18]), "mdl..(0x%08x)--------------------------------------",
                     oam_cf_inputs->mdl);
    }
    else
    {
        sal_snprintf(disp_line[16], sizeof(disp_line[16]), "%sLIF..(0x%08x)--------------------------------------%s",
                     color_line, oam_cf_inputs->lif, reg_color_line);
        sal_snprintf(disp_line[17], sizeof(disp_line[17]), "%sydv..(0x%08x)--------------------------------------%s",
                     color_line, oam_cf_inputs->ydv, reg_color_line);
        sal_snprintf(disp_line[18], sizeof(disp_line[18]), "%smdl..(0x%08x)--------------------------------------%s",
                     color_line, oam_cf_inputs->mdl, reg_color_line);
    }
    sal_snprintf(disp_line[19], sizeof(disp_line[19]), "                                                       ");
    if (oam_cf_results->acc_entry_found)
    {
        /*
         *  Middle section
         */
        sal_snprintf(disp_line[20], sizeof(disp_line[20]),
                     "                                                                  ");
        sal_snprintf(disp_line[21], sizeof(disp_line[21]),
                     "                                                                  ");
        sal_snprintf(disp_line[22], sizeof(disp_line[22]),
                     "------------------------------                                    ");
        sal_snprintf(disp_line[23], sizeof(disp_line[23]),
                     "                              |                                   ");
        sal_snprintf(disp_line[24], sizeof(disp_line[24]),
                     "     mp-type...(0x%08x)-> |                                   ", oam_cf_results->mp_type);
        if (oam_cf_results->lif_1_entry_found)
        {
            sal_snprintf(disp_line[25], sizeof(disp_line[25]),
                         "    _______                   |___________                        ");
            sal_snprintf(disp_line[26], sizeof(disp_line[26]),
                         "   |       |                              |                       ");
            sal_snprintf(disp_line[27], sizeof(disp_line[27]),
                         "-->|  LIF  |-                             |                       ");
            if (dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 1)
            {
                if (oam_cf_results->lif_2_entry_found)
                {
                    sal_snprintf(disp_line[28], sizeof(disp_line[28]),
                                 "-->|   DB  | |-                           |--oam-2-key(0x%08x)",
                ...
                if (oam_cf_results->lif_3_entry_found)
                {
                    sal_snprintf(disp_line[29], sizeof(disp_line[29]),
                                 "-->|  1-3  | | |                          |                       ");
                }
                else
                {
                ...
                sal_snprintf(disp_line[30], sizeof(disp_line[30]),
                             "   |_______| | |                          |                       ");
                sal_snprintf(disp_line[31], sizeof(disp_line[31]),
                             "    |________| |                          |                       ");
                sal_snprintf(disp_line[32], sizeof(disp_line[32]),
                             "      |________|                          |                       ");
            }
            else
            {
        ...
        sal_snprintf(disp_line[33], sizeof(disp_line[33]),
                     "                                          |                       ");
        if (oam_cf_results->acc_entry_found)
        {
            sal_snprintf(disp_line[34], sizeof(disp_line[34]),
                         "    _______                               |                       ");
            sal_snprintf(disp_line[35], sizeof(disp_line[35]),
                         "   |       |                              |                       ");
            sal_snprintf(disp_line[36], sizeof(disp_line[36]),
                         "-->|  ACC  |-acc-mp_profile(0x%08x)-->|                       ",
                         oam_cf_results->acc_mp_profile);
            sal_snprintf(disp_line[37], sizeof(disp_line[37]),
                         "-->|  LIF  |                                                      ");
            sal_snprintf(disp_line[38], sizeof(disp_line[38]),
                         "-->|  DB   |------------------------------------------------------");
            sal_snprintf(disp_line[39], sizeof(disp_line[39]),
                         "   |_______|                                                      ");
        }
        else
        {
            ...
            sal_snprintf(disp_line[35], sizeof(disp_line[35]),
                         ...
                         reg_color_line);
            sal_snprintf(disp_line[36], sizeof(disp_line[36]),
                         "%s-->|  ACC  |-acc-mp_profile(0x%08x)-->|%s                       ", color_line,
         ...
         *  Right section
         */
        sal_snprintf(disp_line[40], sizeof(disp_line[40]), "                                             ");
        sal_snprintf(disp_line[41], sizeof(disp_line[41]), "                                             ");
        sal_snprintf(disp_line[42], sizeof(disp_line[42]), "                                             ");
        sal_snprintf(disp_line[43], sizeof(disp_line[43]), "                                             ");
        sal_snprintf(disp_line[44], sizeof(disp_line[44]), "                                             ");
        sal_snprintf(disp_line[45], sizeof(disp_line[45]), "     _______                                 ");
        sal_snprintf(disp_line[46], sizeof(disp_line[46]), "    |       | --- oam_subtype----(0x%08x)--->",
                     oam_cf_results->oam_subtype);
        if (oam_cf_inputs->ing)
        {
            sal_snprintf(disp_line[47], sizeof(disp_line[47]), "    |  LIF  | --- trap_code------(0x%08x)--->",
                         oam_cf_results->trap_code);
        }
        else
        {
            sal_snprintf(disp_line[47], sizeof(disp_line[47]), "    |  LIF  | --- action-profile-(0x%08x)--->",
                         oam_cf_results->trap_code);
        }
        sal_snprintf(disp_line[48], sizeof(disp_line[48]), "--->|  MEP  | --- trap_strength--(0x%08x)--->",
                     oam_cf_results->trap_strength);
        sal_snprintf(disp_line[49], sizeof(disp_line[49]), "    | Action| --- snoop_strength-(0x%08x)--->",
                     oam_cf_results->snoop_strength);
        if (oam_cf_inputs->ing)
        {
            sal_snprintf(disp_line[50], sizeof(disp_line[50]), "    |  OAMB | --- meter_disable--(0x%08x)--->",
                         oam_cf_results->meter_disable);
            sal_snprintf(disp_line[51], sizeof(disp_line[51]), "    |_______| --- up_mep---------(0x%08x)--->",
                         oam_cf_results->up_mep);
        }
        else
        {
            sal_snprintf(disp_line[50], sizeof(disp_line[50]), "    |  OAMB |                                ");
            sal_snprintf(disp_line[51], sizeof(disp_line[51]), "    |_______|                                ");
        }
        sal_snprintf(disp_line[52], sizeof(disp_line[52]), "                                             ");
        sal_snprintf(disp_line[53], sizeof(disp_line[53]), "                                             ");
        sal_snprintf(disp_line[54], sizeof(disp_line[54]), "                                             ");
        sal_snprintf(disp_line[55], sizeof(disp_line[55]), "                                             ");
        sal_snprintf(disp_line[56], sizeof(disp_line[56]), "                                             ");
        sal_snprintf(disp_line[57], sizeof(disp_line[57]), "                                             ");
        sal_snprintf(disp_line[58], sizeof(disp_line[58]), "----------------- oam_id---------(0x%08x)--->",
                     oam_cf_results->oam_id);
        sal_snprintf(disp_line[59], sizeof(disp_line[59]), "                                             ");
    }
    else
    {
        /*
        ...
        if (oam_cf_results->lif_1_entry_found)
        {
            ...
            {
                sal_snprintf(disp_line[47], sizeof(disp_line[47]), "    |  LIF  | --- action_profile-(0x%08x)--->",
                             oam_cf_results->trap_code);
            }
            sal_snprintf(disp_line[48], sizeof(disp_line[48]), "--->| Action| --- trap_strength--(0x%08x)--->",
                         oam_cf_results->trap_strength);
            sal_snprintf(disp_line[49], sizeof(disp_line[49]), "    |  OAMA | --- snoop_strength-(0x%08x)--->",
                         oam_cf_results->snoop_strength);
            if (oam_cf_inputs->ing)
            {
                sal_snprintf(disp_line[50], sizeof(disp_line[50]), "    |       | --- meter_disable--(0x%08x)--->",
                             oam_cf_results->meter_disable);
                sal_snprintf(disp_line[51], sizeof(disp_line[51]), "    |_______| --- up_mep---------(0x%08x)--->",
                             oam_cf_results->up_mep);
            }
            ...
            {
                sal_snprintf(disp_line[50], sizeof(disp_line[50]), "    |       |                                ");
                sal_snprintf(disp_line[51], sizeof(disp_line[51]), "    |_______|                                ");
            }
        }
        else
        {
            sal_snprintf(disp_line[45], sizeof(disp_line[45]), "%s     _______                                 %s",
                         color_line, reg_color_line);
            sal_snprintf(disp_line[46], sizeof(disp_line[46]), "%s    |       | --- oam_subtype----(0x%08x)--->%s",
                         color_line, oam_cf_results->oam_subtype, reg_color_line);
            if (oam_cf_inputs->ing)
            {
                sal_snprintf(disp_line[47], sizeof(disp_line[47]), "%s    |  LIF  | --- trap_code------(0x%08x)--->%s",
                             color_line, oam_cf_results->trap_code, reg_color_line);
            }
            else
            {
                sal_snprintf(disp_line[47], sizeof(disp_line[47]), "%s    |  LIF  | --- action_profile-(0x%08x)--->%s",
                             color_line, oam_cf_results->trap_code, reg_color_line);
            }
            sal_snprintf(disp_line[48], sizeof(disp_line[48]), "%s--->| Action| --- trap_strength--(0x%08x)--->%s",
                         color_line, oam_cf_results->trap_strength, reg_color_line);
            sal_snprintf(disp_line[49], sizeof(disp_line[49]), "%s    |  OAMA | --- snoop_strength-(0x%08x)--->%s",
                         color_line, oam_cf_results->snoop_strength, reg_color_line);
            if (oam_cf_inputs->ing)
            {
                sal_snprintf(disp_line[50], sizeof(disp_line[50]), "%s    |       | --- meter_disable--(0x%08x)--->%s",
                             color_line, oam_cf_results->meter_disable, reg_color_line);
                sal_snprintf(disp_line[51], sizeof(disp_line[51]), "%s    |_______| --- up_mep---------(0x%08x)--->%s",
            ...
            else
            {
                sal_snprintf(disp_line[50], sizeof(disp_line[50]),
                             "%s    |       |                                       ", color_line);
                sal_snprintf(disp_line[51], sizeof(disp_line[51]),
                             "%s    |_______|                                       ", color_line);
            }
        }
        sal_snprintf(disp_line[52], sizeof(disp_line[52]), "                                             ");
        sal_snprintf(disp_line[53], sizeof(disp_line[53]), "                                             ");
        ...
        sal_snprintf(disp_line[56], sizeof(disp_line[56]), "                                             ");
        sal_snprintf(disp_line[57], sizeof(disp_line[57]), "                                             ");
        sal_snprintf(disp_line[58], sizeof(disp_line[58]), "%s----------------- oam_id---------(0x%08x)--->%s",
...
/* *INDENT-ON* */
     ...
     */
    sal_snprintf(disp_line[0], sizeof(disp_line[0]), "                                                      ");
    sal_snprintf(disp_line[1], sizeof(disp_line[1]), "                                                      ");
    if (oam_cf_results->oama_dis_1)
    {
        sal_snprintf(disp_line[2], sizeof(disp_line[2]), "                %sOAMA-dis_1---------(0x%08x)------>%s",
                     color_line, oam_cf_results->oama_dis_1, reg_color_line);
    }
    else
    {
        sal_snprintf(disp_line[2], sizeof(disp_line[2]), "                OAMA-dis_1---------(0x%08x)------>",
                     oam_cf_results->oama_dis_1);
    }
    if (dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 1)
    {
        if (oam_cf_results->oama_dis_2)
        {
            sal_snprintf(disp_line[3], sizeof(disp_line[3]), "                %sOAMA-dis_2---------(0x%08x)------>%s",
                         color_line, oam_cf_results->oama_dis_2, reg_color_line);
        }
        else
        {
            sal_snprintf(disp_line[3], sizeof(disp_line[3]), "                OAMA-dis_2---------(0x%08x)------>",
                         oam_cf_results->oama_dis_2);
        }
        if (oam_cf_results->oama_dis_3)
        {
            sal_snprintf(disp_line[4], sizeof(disp_line[4]), "                %sOAMA-dis_3---------(0x%08x)------>%s",
                         color_line, oam_cf_results->oama_dis_3, reg_color_line);
        }
        else
        {
            sal_snprintf(disp_line[4], sizeof(disp_line[4]), "                OAMA-dis_3---------(0x%08x)------>",
                         oam_cf_results->oama_dis_3);
        }
    }
    else
    {
    ...
    {
        sal_snprintf(disp_line[5], sizeof(disp_line[5]), "                %sOAMB-dis-----------(0x%08x)------>%s",
                     color_line, oam_cf_results->oamb_dis, reg_color_line);
    }
    else
    {
        sal_snprintf(disp_line[5], sizeof(disp_line[5]), "                OAMB-dis-----------(0x%08x)------>",
                     oam_cf_results->oamb_dis);
    }
    if (oam_cf_results->lif_1_entry_found)
    {
        sal_snprintf(disp_line[6], sizeof(disp_line[6]), "                mp_profile_1-------(0x%08x)------>",
                     oam_cf_results->lif_db_access_1.mp_profile);
    }
    else
    {
        sal_snprintf(disp_line[6], sizeof(disp_line[6]), "                %smp_profile_1-------(0x%08x)------>%s",
                     color_line, oam_cf_results->lif_db_access_1.mp_profile, reg_color_line);
    }
    if (dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 1)
    {
        if (oam_cf_results->lif_2_entry_found)
        {
            sal_snprintf(disp_line[7], sizeof(disp_line[7]), "                mp_profile_2-------(0x%08x)------>",
                         oam_cf_results->lif_db_access_2.mp_profile);
        }
        else
        {
            sal_snprintf(disp_line[7], sizeof(disp_line[7]), "                %smp_profile_2-------(0x%08x)------>%s",
                         color_line, oam_cf_results->lif_db_access_2.mp_profile, reg_color_line);
        }
        if (oam_cf_results->lif_3_entry_found)
        {
            sal_snprintf(disp_line[8], sizeof(disp_line[8]), "                mp_profile_3-------(0x%08x)------>",
                         oam_cf_results->lif_db_access_3.mp_profile);
        }
        else
        {
            sal_snprintf(disp_line[8], sizeof(disp_line[8]), "                %smp_profile_3-------(0x%08x)------>%s",
    ...
    sal_snprintf(disp_line[9], sizeof(disp_line[9]), "                pkt_is_oam---------(0x%08x)------>",
                 oam_cf_results->pkt_is_oam);
    sal_snprintf(disp_line[10], sizeof(disp_line[10]), "                is_inlif_valid4LM--(0x%08x)------>",
                 oam_cf_results->inlif_valid4lm);
    sal_snprintf(disp_line[11], sizeof(disp_line[11]), "                nof_LM_lifs--------(0x%08x)------>",
                 oam_cf_results->nof_lm_lifs);
    sal_snprintf(disp_line[12], sizeof(disp_line[12]), "                mp_type------------(0x%08x)------>",
                 oam_cf_results->mp_type);
    sal_snprintf(disp_line[13], sizeof(disp_line[13]), "                                                      ");
    sal_snprintf(disp_line[14], sizeof(disp_line[14]), " _________                                 ");
    sal_snprintf(disp_line[15], sizeof(disp_line[15]), "|         |                                ");
    sal_snprintf(disp_line[16], sizeof(disp_line[16]), "|         |                                ");
    sal_snprintf(disp_line[17], sizeof(disp_line[17]), "|         |                                ");
    sal_snprintf(disp_line[18], sizeof(disp_line[18]), "|         |                                ");
    if (oam_cf_results->lif_1_entry_found)
    {
        sal_snprintf(disp_line[19], sizeof(disp_line[19]), "|         |--Counter_if,idx_1--(%01d,0x%08x)----->",
                     oam_cf_results->lif_db_access_1.counter_interface, oam_cf_results->lif_db_access_1.counter_base);
    }
    else
    {
        sal_snprintf(disp_line[19], sizeof(disp_line[19]), "|         |%s--Counter_if,idx_1--(%01d,0x%08x)----->%s",
    ...
    {
        sal_snprintf(disp_line[20], sizeof(disp_line[20]), "| Counter |--Counter_inc_1----------------(%01d)----->",
                     oam_cf_results->counter_inc_1);
    }
    else
    {
        sal_snprintf(disp_line[20], sizeof(disp_line[20]), "| Counter |%s--Counter_inc_1----------------(%01d)----->%s",
    ...
    if (dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 1)
    {
        if (oam_cf_results->lif_2_entry_found)
        {
            sal_snprintf(disp_line[21], sizeof(disp_line[21]), "|         |--Counter_if,idx_2--(%01d,0x%08x)----->",
                         oam_cf_results->lif_db_access_2.counter_interface,
                         oam_cf_results->lif_db_access_2.counter_base);
        }
        else
        {
            sal_snprintf(disp_line[21], sizeof(disp_line[21]), "|         |%s--Counter_if,idx_2--(%01d,0x%08x)----->%s",
        ...
        {
            sal_snprintf(disp_line[22], sizeof(disp_line[22]), "|   Res   |--Counter_inc_2----------------(%01d)----->",
                         oam_cf_results->counter_inc_2);
        }
        else
        {
            sal_snprintf(disp_line[22], sizeof(disp_line[22]),
                         "|   Res   |%s--Counter_inc_2----------------(%01d)----->%s", color_line,
                         oam_cf_results->counter_inc_2, reg_color_line);
        }
        if (oam_cf_results->lif_3_entry_found)
        {
            sal_snprintf(disp_line[23], sizeof(disp_line[23]), "|         |--Counter_if,idx_3--(%01d,0x%08x)----->",
                         oam_cf_results->lif_db_access_3.counter_interface,
                         oam_cf_results->lif_db_access_3.counter_base);
        }
        else
        {
            sal_snprintf(disp_line[23], sizeof(disp_line[23]), "|         |%s--Counter_if,idx_3--(%01d,0x%08x)----->%s",
        ...
        {
            sal_snprintf(disp_line[24], sizeof(disp_line[24]), "|         |--Counter_inc_3----------------(%01d)----->",
                         oam_cf_results->counter_inc_3);
        }
        else
        {
            sal_snprintf(disp_line[24], sizeof(disp_line[24]),
                         "|         |%s--Counter_inc_3----------------(%01d)----->%s", color_line,
                         oam_cf_results->counter_inc_3, reg_color_line);
        }
    }
    else
    {
    }
    sal_snprintf(disp_line[25], sizeof(disp_line[25]), "|         |--Counter_Read_idx----(0x%08x)----->",
                 oam_cf_results->counter_read_idx);
    sal_snprintf(disp_line[26], sizeof(disp_line[26]), "|         |                                ");
    sal_snprintf(disp_line[27], sizeof(disp_line[27]), "|_________|                                ");
    sal_snprintf(disp_line[28], sizeof(disp_line[28]),
                 "                                                                 ");
    LOG_CLI((BSL_META_U
             (unit,
              "%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n%s%s%s\n\n"),
             disp_line[28], disp_line[0], disp_line[14], disp_line[28], disp_line[1], disp_line[15], disp_line[28],
             disp_line[2], disp_line[16], disp_line[28], disp_line[3], disp_line[17], disp_line[28], disp_line[4],
             disp_line[18], disp_line[28], disp_line[5], disp_line[19], disp_line[28], disp_line[6], disp_line[20],
             disp_line[28], disp_line[7], disp_line[21], disp_line[28], disp_line[8], disp_line[22], disp_line[28],
             disp_line[9], disp_line[23], disp_line[28], disp_line[10], disp_line[24], disp_line[28], disp_line[11],
             disp_line[25], disp_line[28], disp_line[12], disp_line[26], disp_line[28], disp_line[13], disp_line[27]));
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
     ...
     *  Verify inputs
     */
    SHR_INVOKE_VERIFY_DNXC(diag_oam_cf_verify(unit, &oam_cf_inputs));
    /*
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_in, core_idx)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get
                        (unit, DBAL_TABLE_OAMP_PE_PROGRAM_SW_HW2ENUM, DBAL_FIELD_OAMP_PE_PROGRAM_ENUM, 0, prog_enum,
                         NULL, 0, FALSE, prog_last_name));
        SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get
                        (unit, DBAL_TABLE_OAMP_PE_TCAM_SW, DBAL_FIELD_OAMP_PE_PROG_TCAM_ENUM, 0, tcam_enum, NULL, 0,
                         FALSE, tcam_last_name));
        SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get
                        (unit, DBAL_TABLE_OAMP_PE_STATUS, DBAL_FIELD_OAMP_PE_LAST_KEY, 0, tcam_key, NULL, 0, FALSE,
                         key_buffer));
        SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get
                        (unit, DBAL_TABLE_OAMP_PE_STATUS, DBAL_FIELD_LAST_TCAM_HIT, 0, hit, NULL, 0, FALSE,
                         hit_buffer));
        SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get
                        (unit, DBAL_TABLE_OAMP_PE_STATUS, DBAL_FIELD_LAST_PKT_VARS, 0, pkt_vars, NULL, 0, FALSE,
                         pkt_vars_buffer));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    while (!is_end)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get
                        (unit, DBAL_TABLE_OAMP_PE_PROGRAMS_TCAM, DBAL_FIELD_OAMP_PE_PROG_TCAM, 0, tcam_key_val,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (oamp_status_outputs->maid_err)
    {
        ...
        SHR_IF_ERR_EXIT(utilex_U8_to_U32(oamp_status_outputs->maid_name_rcv, 16, maid32));
        ...
        SHR_IF_ERR_EXIT(utilex_U8_to_U32(oamp_status_outputs->maid_name_exp, 16, maid32));
        sand_signal_value_to_str(unit, SAL_FIELD_TYPE_NONE, maid32, maid, 16 * 8, PRINT_BIG_ENDIAN);
        PRT_CELL_SET("%s", maid);
    }
    ...
    char *mep_info_str)
{
    ...
    for (i = 0; i < num_meps_in_first_set; i++)
    {
        ...
        {
            num_chars +=
                sal_snprintf(&mep_info_str[num_chars], MAX_MEP_INFO_STR_SIZE - num_chars, "%d", list_of_meps[i]);
        }
        else
        {
            num_chars +=
    ...
    if (num_meps_in_second_set != 0)
    {
         ...
         * Print dots.
         */
        num_chars += sal_snprintf(&mep_info_str[num_chars], MAX_MEP_INFO_STR_SIZE - num_chars, "....");
        /*
         * Second set of meps
         */
        for (i = num_meps_in_second_set; i > 0; i--)
        {
            if (i == 1)
            {
                 ...
                 */
                num_chars +=
                    sal_snprintf(&mep_info_str[num_chars], MAX_MEP_INFO_STR_SIZE - num_chars, "%d",
                                 list_of_meps[num_meps - i]);
            }
            else
            {
                num_chars +=
            ...
            else
            {
                if (in_range == TRUE)
                {
                    if (first_mep_id_in_range != -1)
                    {
                        ...
                        {
                            num_chars +=
                                sal_snprintf(&(mep_info_str[num_chars]), MAX_MEP_INFO_STR_SIZE - num_chars, "%d - %d,",
    ...
    oam_bfdep_results_t * oam_bfdep_results)
{
    ...
    oam_bfdep_results->no_sbfd_reflector_over_ipv6 = 0;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (oam_bfdep_results->endpoint_info.type)
    {
        case bcmBFDTunnelTypeUdp:
        {
            char *ip_version = oam_bfdep_results->endpoint_info.flags & BCM_BFD_ENDPOINT_IPV6 ? "IPv6" : "IPv4";
            {
                ...
                if (oam_bfdep_results->endpoint_info.flags2 & BCM_BFD_ENDPOINT_FLAGS2_SINGLE_HOP_WITH_RANDOM_DIP)
                {
                    PRT_CELL_SET("%s %s", "Single Hop with random DIP", ip_version);
                }
            ...
            if (oam_bfdep_results->endpoint_info.flags & BCM_BFD_ENDPOINT_IPV6)
            {
                ...
                {
                    oam_bfdep_results->no_sbfd_reflector_over_ipv6++;
                }
                else
                {
        ...
        {
            PRT_CELL_SET("%s", "BFD over PWE VCCV Type 1 with UPD/IP encapsulation");
        ...
        {
            PRT_CELL_SET("%s", "BFD over MPLS PHP");
            oam_bfdep_results->no_mpls_php++;
            break;
        }
        default:
        {
    ...
    if (oam_bfdep_results->endpoint_info.flags & BCM_BFD_ENDPOINT_IN_HW)
    {
        ...
        if (oam_bfdep_results->endpoint_info.type == bcmBFDTunnelTypeUdp
            ...
            || oam_bfdep_results->endpoint_info.type == bcmBFDTunnelTypeMplsPhp)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        ...
        if (oam_bfdep_results->endpoint_info.type == bcmBFDTunnelTypeUdp)
        {
            if ((oam_bfdep_results->endpoint_info.flags2 & BCM_BFD_ENDPOINT_FLAGS2_SINGLE_HOP_WITH_RANDOM_DIP) == 0)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            ...
            if (oam_bfdep_results->endpoint_info.flags2 & BCM_BFD_ENDPOINT_FLAGS2_USE_MY_DIP_DESTINATION)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET_SHIFT(1, "%s", "Using my-DIP-destination");
            }
        }
        ...
        if (oam_bfdep_results->endpoint_info.type != bcmBFDTunnelTypeUdp)
        {
            ...
            PRT_CELL_SET("%u", oam_bfdep_results->endpoint_info.label);
            ...
            if (oam_bfdep_results->endpoint_info.type == bcmBFDTunnelTypeMplsTpCcCv)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET_SHIFT(1, "%s", "IP-TTL");
                PRT_CELL_SET("%u", oam_bfdep_results->endpoint_info.ip_ttl);
                ...
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                if (oam_bfdep_results->endpoint_info.dst_ip_addr)
                {
                    PRT_CELL_SET_SHIFT(1, "%s", "Destination IPv4 address");
                    PRT_CELL_SET("%u", oam_bfdep_results->endpoint_info.dst_ip_addr);
                }
                else
                {
                    PRT_CELL_SET_SHIFT(1, "%s", "Destination IPv4 address is random");
                }
            }
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_CELL_SET("%s", "SBFD reflector over IPV6");
    PRT_CELL_SET("%d", oam_bfdep_results->no_sbfd_reflector_over_ipv6);
    ...
    PRT_CELL_SET("%s", "BFD over MPLS PHP");
    PRT_CELL_SET("%d", oam_bfdep_results->no_mpls_php);
    ...
    oam_ep_inputs_t * oam_ep_inputs)
{
     ...
     */
    num_of_cores = dnx_data_device.general.nof_cores_get(unit);
    if (oam_ep_inputs->core_id > num_of_cores || !DNXCMN_CORE_IS_ENABLED(unit, oam_ep_inputs->core_id))
    {
        /*
         * Default core_id
         */
    ...
    oam_cntrs_inputs_t * oam_cntrs_inputs)
{
    ...
    if ((oam_cntrs_inputs->core_id < 0) || (oam_cntrs_inputs->core_id > num_of_cores)
        || !DNXCMN_CORE_IS_ENABLED(unit, oam_cntrs_inputs->core_id))
    {
        /*
         * Default core_id
         */
    ...
    sh_sand_control_t * sand_control)
{
     ...
     *  Verify inputs
     */
    SHR_INVOKE_VERIFY_DNXC(diag_oam_ep_verify(unit, &oam_ep_inputs));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!is_rfc_6374)
    {
        ...
        if (is_mip)
        {
            sal_snprintf(display_str, sizeof(display_str), "MIP");
        ...
        else
        {
            if (is_up)
            {
                sal_snprintf(display_str, sizeof(display_str), "UPMEP");
                oam_ep_results->no_ups++;
            }
            else
            {
                sal_snprintf(display_str, sizeof(display_str), "DOWNMEP");
            }
        }
    ...
    switch (oam_ep_results->endpoint_info.type)
    {
        ...
        {
            {
                sal_snprintf(display_str, sizeof(display_str), "OAM over Ethernet");
            }
        ...
        {
            {
                sal_snprintf(display_str, sizeof(display_str), "BHH MPLS");
            }
        ...
        {
            {
                sal_snprintf(display_str, sizeof(display_str), "BHH - PWE");
            }
        ...
        {
            {
                sal_snprintf(display_str, sizeof(display_str), "BHH - PWEoGAL");
            }
        ...
        {
            {
                sal_snprintf(display_str, sizeof(display_str), "MPLSNetwork");
            }
        ...
        {
            {
                sal_snprintf(display_str, sizeof(display_str), "BHH - Section");
            }
        ...
        {
            sal_snprintf(display_str, sizeof(display_str), "RFC 6374 LM DM - LSP");
            oam_ep_results->no_bhh_mpls++;
            break;
        }
        case bcmOAMEndpointTypeMplsLmDmPw:
        {
            sal_snprintf(display_str, sizeof(display_str), "RFC 6374 LM DM - PWE");
            oam_ep_results->no_bhh_pwe++;
            break;
        }
        case bcmOAMEndpointTypeMplsLmDmSection:
        {
            sal_snprintf(display_str, sizeof(display_str), "RFC 6374 LM DM - Section");
            oam_ep_results->no_bhh_sec++;
            break;
        }
        default:
        {
            sal_snprintf(display_str, sizeof(display_str), "ERROR: Unsupported endpoint type");
    ...
    {
        sal_snprintf(display_str, sizeof(display_str), "ON");
    }
    else
    {
        sal_snprintf(display_str, sizeof(display_str), "OFF");
    }
    ...
    if (is_acc)
    {
        ...
        if (!is_rfc_6374)
        {
            if (oam_ep_results->endpoint_info.type == bcmOAMEndpointTypeEthernet)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            ...
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", "Label");
                PRT_CELL_SET("%d", oam_ep_results->endpoint_info.egress_label.label);
            }
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            ...
            if (oam_ep_results->endpoint_info.vlan)
            {
                ...
                if (oam_ep_results->endpoint_info.inner_vlan)
                {
                    ...
                    PRT_CELL_SET("0x%x", oam_ep_results->endpoint_info.inner_vlan);
            ...
            {
                SHR_IF_ERR_EXIT(bcm_oam_control_get(unit, bcmOamControlIngressVlanEditClassDeleteTwoVlanTags, &ivec));
                sal_snprintf(display_str, sizeof(display_str), "%d - Delete two VLAN tags", (int) ivec);
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", "Down Egress Inj IVEC");
                PRT_CELL_SET("%s", display_str);
            }
            else if (oam_ep_results->
                     endpoint_info.flags2 & BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_ONE_VLAN_TAG)
            {
                SHR_IF_ERR_EXIT(bcm_oam_control_get(unit, bcmOamControlIngressVlanEditClassDeleteOneVlanTag, &ivec));
                sal_snprintf(display_str, sizeof(display_str), "%d - Delete one VLAN tag", (int) ivec);
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", "Down Egress Inj IVEC");
                PRT_CELL_SET("%s", display_str);
            }
            else if (oam_ep_results->endpoint_info.flags2 & BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", "Down Egress Inj IVEC");
                PRT_CELL_SET("%s", "No IVEC");
            }
...
diag_oam_cntrs_get_engine_size(
    ...
    int *num_of_counters)
{
    int first_b_engine_id = 0;
    int first_c_engine_id = 0;
    int nof_engines = 0;
    int nof_counters_in_a_engine = 0;
    int nof_counters_in_b_engine = 0;
    int nof_counters_in_c_engine = 0;
    bcm_stat_counter_format_type_t format_type;

    SHR_FUNC_INIT_VARS(unit);

    first_b_engine_id = dnx_data_crps.engine.nof_engines_get(unit) -
        dnx_data_crps.engine.nof_mid_engines_get(unit) - dnx_data_crps.engine.nof_big_engines_get(unit);
    first_c_engine_id = dnx_data_crps.engine.nof_engines_get(unit) - dnx_data_crps.engine.nof_big_engines_get(unit);
    nof_engines = dnx_data_crps.engine.nof_engines_get(unit);
    format_type = bcmStatCounterFormatDoublePackets;

    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, 0 /* A's engine id */ , format_type, 0  /* state
                                                                                                         * counter size 
                                                                                                         */ , &nof_counters_in_a_engine));
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, first_b_engine_id, format_type, 0       /* state
                                                                                                         * counter size 
                                                                                                         */ , &nof_counters_in_b_engine));
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, first_c_engine_id, format_type, 0       /* state
                                                                                                         * counter size 
                                                                                                         */ , &nof_counters_in_c_engine));

    {
        if ((id >= 0) && (id < first_b_engine_id))
        {
            *num_of_counters = nof_counters_in_a_engine;
        }
        else if (((id >= first_b_engine_id) && (id < first_c_engine_id)))
        {
            *num_of_counters = nof_counters_in_b_engine;
        }
        else if (((id >= first_c_engine_id) && (id < nof_engines)))
        {
            *num_of_counters = nof_counters_in_c_engine;
        }
        else
        {
            *num_of_counters = 0;
    ...
    mep_counter_info_t * counter_info)
{
    ...
    int counter_index;
    int num_of_counters = 0;
    int nof_counters_per_entry = 0;

    SHR_FUNC_INIT_VARS(unit);

    nof_counters_per_entry =
        dnx_data_crps.engine.counter_format_types_get(unit, bcmStatCounterFormatDoublePackets)->nof_counters_per_entry;
    ...
    while (database_id < MAX_NOF_DATABASES)
    {
        if (oam_cntrs_results->database[database_id].is_oam)
        {
            ...
            {
                counter_index = counter_base / nof_counters_per_entry;
    ...
    oam_cntrs_results_t * oam_cntrs_results)
{
    ...
    int nof_a_engines = 0;
    int nof_b_engines = 0;
    int ing_array_index = 0;
    int eg_array_index = 0;
    int nof_of_counter_c_engine = 0;
    int nof_counters_per_entry = 0;
    int nof_bits_for_packets = 0;
    int index;
    int multi_val = 0;
    int section_offset = 0;

/*
 * If counter in in big engine and couter index is 3, valid bits of counter is 18.
 * Otherwise, valid bits of pkt counter is _BITS_PER_COUNTER (34/36)
 */
#define CPY_COUNTER_VALUE_FROM_HW(_DST_CNT, _ENGINE_ID, _INNER_HW_IDX, _BITS_PER_COUNTER, _PCNT_VAL)                 \
{                                                                                                    \
    msb_bits = 2 * (_INNER_HW_IDX + 1);                                                              \
    mask = (1 << msb_bits) - 1;                                                                      \
    COMPILER_64_ZERO(_DST_CNT);                                                                      \
    COMPILER_64_SET(temp_64_a, 0, mask);                                                             \
    COMPILER_64_SET(temp_64, 0, _PCNT_VAL[_INNER_HW_IDX + 1]);                                       \
    COMPILER_64_AND(temp_64, temp_64_a);                                                             \
    COMPILER_64_SHL(temp_64, 32);                                                                    \
    if (_ENGINE_ID >= first_c_engine_id && _INNER_HW_IDX == 3) {                                     \
        COMPILER_64_ADD_32(_DST_CNT, ((_PCNT_VAL[_INNER_HW_IDX] >> (_INNER_HW_IDX*(_BITS_PER_COUNTER-32))) & 0x3FFFF));   \
    } else {                                                                                         \
        COMPILER_64_ADD_32(_DST_CNT, (_PCNT_VAL[_INNER_HW_IDX] >> (_INNER_HW_IDX*(_BITS_PER_COUNTER-32))));               \
        COMPILER_64_ADD_64(_DST_CNT, temp_64);                                                       \
    }                                                                                                \
}
    ...
    nof_counters_per_entry =
        dnx_data_crps.engine.counter_format_types_get(unit, bcmStatCounterFormatDoublePackets)->nof_counters_per_entry;
    nof_bits_for_packets =
        dnx_data_crps.engine.counter_format_types_get(unit, bcmStatCounterFormatDoublePackets)->nof_bits_for_packets;

    first_b_engine_id = dnx_data_crps.engine.nof_engines_get(unit) -
        dnx_data_crps.engine.nof_mid_engines_get(unit) - dnx_data_crps.engine.nof_big_engines_get(unit);
    first_c_engine_id = dnx_data_crps.engine.nof_engines_get(unit) - dnx_data_crps.engine.nof_big_engines_get(unit);
    nof_a_engines = first_b_engine_id;
    nof_b_engines = dnx_data_crps.engine.nof_mid_engines_get(unit);

    section_offset = dnx_data_meter.mem_mgmt.sections_offset_get(unit);
    multi_val = (section_offset == 1) ? 2 : 1;

    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, first_c_engine_id, bcmStatCounterFormatPackets, 0       /* state 
                                                                                                                         * counter 
                                                                                                                         * size */ , &nof_of_counter_c_engine));
    ...
    ing_entry_index = counter_info.ing_index / nof_counters_per_entry;
    eg_entry_index = counter_info.eg_index / nof_counters_per_entry;

    ing_engine = counter_info.ing_engine_id;
    eg_engine = counter_info.eg_engine_id;
    counter_bank = MCP_ENG_DB_A_MEMm;
    if (counter_info.ing_engine_id >= first_b_engine_id)
    {
        counter_bank = MCP_ENG_DB_B_MEMm;
        ing_engine -= nof_a_engines;
        eg_engine -= nof_a_engines;
    }
    if (counter_info.ing_engine_id >= first_c_engine_id)
    {
        counter_bank = MCP_ENG_DB_C_MEMm;
        ing_engine -= nof_b_engines;
        eg_engine -= nof_b_engines;
        if (counter_info.ing_index >= nof_of_counter_c_engine)
        {
            ing_entry_index = ing_entry_index - (nof_of_counter_c_engine >> 2);
        }
        if (counter_info.eg_index >= nof_of_counter_c_engine)
        {
            eg_entry_index = eg_entry_index - (nof_of_counter_c_engine >> 2);
        }

        ing_array_index =
            (counter_info.ing_index >=
             nof_of_counter_c_engine) ? (ing_engine * multi_val + section_offset) : (ing_engine * multi_val);
        eg_array_index =
            (counter_info.eg_index >=
             nof_of_counter_c_engine) ? (eg_engine * multi_val + section_offset) : (eg_engine * multi_val);
    }
    else
    {
        ing_array_index = ing_engine;
        eg_array_index = eg_engine;
    }

    inner_hw_index = endpoint_info.lm_counter_base_id % nof_counters_per_entry;

    /*
     * Read counter values (136/216 bytes for each 8 counters (for pcp))
     */
    block = MCP_BLOCK(unit, core);
    SHR_IF_ERR_EXIT(soc_mem_array_read
                    (unit, counter_bank, ing_array_index, block, ing_entry_index, &ing_entry_above_64));
    SHR_IF_ERR_EXIT(soc_mem_array_read(unit, counter_bank, eg_array_index, block, eg_entry_index, &eg_entry_above_64));

    /*
     * Prepare values of pcp=0 counter. If no-pcp, this is the only value needed.
     */
    CPY_COUNTER_VALUE_FROM_HW(oam_cntrs_results->endpoint_results.ing_counter_value[0], counter_info.ing_engine_id,
                              inner_hw_index, nof_bits_for_packets, ing_entry_above_64);
    CPY_COUNTER_VALUE_FROM_HW(oam_cntrs_results->endpoint_results.eg_counter_value[0], counter_info.ing_engine_id,
                              inner_hw_index, nof_bits_for_packets, eg_entry_above_64);
    ...
    if (oam_cntrs_results->endpoint_results.is_pcp)
    {
        ...
        {
            index = (pcp_index + inner_hw_index) % nof_counters_per_entry;
            if ((pcp_index + inner_hw_index) == nof_counters_per_entry)
            {
                SHR_IF_ERR_EXIT(soc_mem_array_read
                                (unit, counter_bank, ing_array_index, block, ing_entry_index + 1, &ing_entry_above_64));
                SHR_IF_ERR_EXIT(soc_mem_array_read
                                (unit, counter_bank, eg_array_index, block, eg_entry_index + 1, &eg_entry_above_64));
            }
            CPY_COUNTER_VALUE_FROM_HW(oam_cntrs_results->endpoint_results.ing_counter_value[pcp_index],
                                      counter_info.ing_engine_id, index, nof_bits_for_packets, ing_entry_above_64);
            CPY_COUNTER_VALUE_FROM_HW(oam_cntrs_results->endpoint_results.eg_counter_value[pcp_index],
                                      counter_info.ing_engine_id, index, nof_bits_for_packets, eg_entry_above_64);
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_memset(&oam_cntrs_results, 0, sizeof(oam_cntrs_results));
     ...
     *  Verify inputs
     */
    SHR_INVOKE_VERIFY_DNXC(diag_oam_cntrs_verify(unit, &oam_cntrs_inputs));
    ...
    oam_oamid_params_t * oam_oamid_params)
{
    ...
    if (oam_oamid_params->core_id > num_of_cores || !DNXCMN_CORE_IS_ENABLED(unit, oam_oamid_params->core_id))
    {
        /*
         * Default core_id
         */
    ...
    oam_lu_params_t * oam_lu_params)
{
    ...
    if (oam_lu_params->core_id > num_of_cores || !DNXCMN_CORE_IS_ENABLED(unit, oam_lu_params->core_id))
    {
        /*
         * Default core_id
         */
 ...
 *   by dnx_oam_sw we get sw and parse by it the profiles
 *   each set flag will parse.
 */
static shr_error_e
sh_dnx_oam_profile_print(
    int unit,
    bcm_oam_profile_t profile_id,
    char **profile_id_to_string,
    sh_sand_control_t * sand_control)
{
    uint8 is_ingress;
    uint8 is_acc;
    ...
    oam_profile_type = DNX_OAM_PROFILE_TYPE_GET(profile_id);
    ...
    SHR_IF_ERR_EXIT(dnx_oam_sw_db_lm_mode_get(unit, profile_index, is_ingress, is_acc, &lm_type, &profile_is_section));

    PRT_TITLE_SET("Profile diag");
    PRT_COLUMN_ADD("Profile index");
    PRT_COLUMN_ADD("Value");
    PRT_COLUMN_ADD("Flag set");

    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
    PRT_CELL_SET("%s", *profile_id_to_string);
    ...
    if (lm_type == DBAL_ENUM_FVAL_LM_MODE_LM_MODE_IS_DUAL_ENDED)
    {
        ...
        PRT_CELL_SET("%s", *profile_id_to_string);
        PRT_CELL_SET("%d", profile_index);
        PRT_CELL_SET("%s", "LM_TYPE_IS_DUAL_ENDED");
    }
    else if (lm_type == DBAL_ENUM_FVAL_LM_MODE_LM_MODE_IS_NONE)
    {
        ...
        PRT_CELL_SET("%s", *profile_id_to_string);
        PRT_CELL_SET("%d", profile_index);
        PRT_CELL_SET("%s", "LM_TYPE_IS_NONE");

    }
    if (profile_is_section)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("%s", *profile_id_to_string);
        PRT_CELL_SET("%d", profile_index);
        PRT_CELL_SET("%s", "TYPE_SECTION");
    }

    SHR_IF_ERR_EXIT(dnx_oam_sw_db_dm_mode_get(unit, profile_index, is_ingress, is_acc, &dm_type, &profile_is_section));
    ...
    if (dm_type == DBAL_ENUM_FVAL_DM_MODE_DM_MODE_IS_NTP)
    {
        ...
        PRT_CELL_SET("%s", *profile_id_to_string);
        PRT_CELL_SET("%d", profile_index);
        PRT_CELL_SET("%s", "DM_TYPE_IS_NTP");
    }

    if (dm_type == DBAL_ENUM_FVAL_DM_MODE_DM_MODE_IS_NONE)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("%s", *profile_id_to_string);
        PRT_CELL_SET("%d", profile_index);
        PRT_CELL_SET("%s", "DM_TYPE_IS_NONE");
    }

    SHR_IF_ERR_EXIT(dnx_oam_sw_db_hlm_mode_get(unit, profile_index, is_ingress, is_acc, &profile_is_hlm_dis));

    if (profile_is_hlm_dis)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("%s", *profile_id_to_string);
        PRT_CELL_SET("%d", profile_index);
        PRT_CELL_SET("%s", "HIERARCHICAL_LM_DISABLE");
    }
...
/**
 * \brief
 * oam profile_dump command - main function
 * \retval Error indication
 *
 */
static shr_error_e
sh_dnx_oam_profile_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    ...
    for (element = 1; element < NUMBER_OF_PROFILES; element++)
    {

        algo_oam_db_oam_profile_id_ing_acc_is_allocated(unit, element, &is_allocated);

        if (is_allocated)
        {
            profile_id = DNX_OAM_PROFILE_SET(element, bcmOAMProfileIngressAcceleratedEndpoint);
            profile_id_to_string = "Profile Ingress Accelerated Endpoint";
            SHR_IF_ERR_EXIT(sh_dnx_oam_profile_print(unit, profile_id, &profile_id_to_string, sand_control));
        }
        algo_oam_db_oam_profile_id_eg_is_allocated(unit, element, &is_allocated);

        if (is_allocated)
        {
            profile_id = DNX_OAM_PROFILE_SET(element, bcmOAMProfileEgressLIF);
            profile_id_to_string = "Profile Egress LIF";
            SHR_IF_ERR_EXIT(sh_dnx_oam_profile_print(unit, profile_id, &profile_id_to_string, sand_control));
        }
        algo_oam_db_oam_profile_id_ing_is_allocated(unit, element, &is_allocated);

        if (is_allocated)
        {
            profile_id = DNX_OAM_PROFILE_SET(element, bcmOAMProfileIngressLIF);
            profile_id_to_string = "Profile Ingress LIF";
            SHR_IF_ERR_EXIT(sh_dnx_oam_profile_print(unit, profile_id, &profile_id_to_string, sand_control));
        }
        algo_oam_db_oam_profile_id_eg_acc_is_allocated(unit, element, &is_allocated);

        if (is_allocated)
        {
            profile_id = DNX_OAM_PROFILE_SET(element, bcmOAMProfileEgressAcceleratedEndpoint);
            profile_id_to_string = "Profile Egress Accelerated Endpoint";
            SHR_IF_ERR_EXIT(sh_dnx_oam_profile_print(unit, profile_id, &profile_id_to_string, sand_control));
        }
...
sh_sand_cmd_t sh_dnx_oam_oamid_cmds[] =
{
    ...
    {"Get",    sh_dnx_oam_oamid_cmd_get, NULL, dnx_oam_oamid_get_list_options,  &sh_dnx_oam_oamid_get_man,  NULL, NULL, SH_CMD_NO_XML_VERIFY * CTEST_ADAPTER_UNSUPPORTED},
#else
...
/**
 * \brief Profile_dump diagnostic manual
 * This is the manual for "oam oamp Profile_dump"
 */
static sh_sand_man_t sh_dnx_oamp_profile_man =
{
    .brief  = "OAMP Diagnostic",
    .full   = "Display the profiles and parse them\n"
};
...
static sh_sand_cmd_t sh_dnx_oamp_cmds[] =
{
    ...
    {"OAMP_PE" , sh_dnx_oamp_pe_cmd     , NULL,                    dnx_oamp_pe_list_options, &sh_dnx_oamp_pe_man    , NULL, NULL, SH_CMD_NO_XML_VERIFY},
...
sh_sand_cmd_t sh_dnx_oam_cmds[] =
{
    ...
    {"Profile_Dump"  , sh_dnx_oam_profile_cmd , NULL,                    NULL                    , &sh_dnx_oamp_profile_man, NULL, NULL},
    {NULL}
};
/* *INDENT-ON* */
