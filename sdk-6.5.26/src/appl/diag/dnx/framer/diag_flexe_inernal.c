...
/**
 * \brief
 *   Get valid flexe groups
 * \param [in] unit - The unit number.
 * \param [in] groups - flexe groups bitmap.
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 */
shr_error_e
dnx_diag_flexe_groups_get(
    int unit,
    bcm_pbmp_t * groups)
{
    uint32 group_index;
    ...
    for (group_index = 0; group_index < dnx_data_nif.flexe.nof_groups_get(unit); group_index++)
    {
        ...
        {
            BCM_PBMP_PORT_ADD(*groups, group_index);
        }
    }
    SHR_FUNC_EXIT;
}
    ...
    bcm_port_t * logical_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF_ILKN, DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_ELK,
                     &pbmp));
    BCM_PBMP_ITER(pbmp, port)
    {
        if (is_tx)
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_out_channel_get(unit, port, &ilkn_channel));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_in_channel_get(unit, port, &ilkn_channel));
        }
    ...
    uint32 *dst_channel)
{
    ...
    SHR_MAX_VERIFY(src_channel, dnx_data_nif.flexe.nof_clients_get(unit) - 1, _SHR_E_PARAM, "Invald src_channel %d",
                   src_channel);
    ...
    uint32 *tx_chan_map)
{
    ...
    SHR_MAX_VERIFY(sar_channel, dnx_data_nif.flexe.nof_clients_get(unit) - 1, _SHR_E_PARAM, "Invald src_channel %d",
                   sar_channel);
    ...
    bcm_port_t * logical_port)
{
    ...
    {
        port_type = DNX_ALGO_PORT_TYPE_FRAMER_SAR;
        client_index = channel - 80;
    }
    else if (channel < 240)
    {
        port_type = DNX_ALGO_PORT_TYPE_FRAMER_MAC;
    ...
    bcm_port_t * dst_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, src_port, &src_channel));
    if (src_port_info.port_type == DNX_ALGO_PORT_TYPE_FLEXE_CLIENT)
    {
        ram_id = 0;
    }
    else if (src_port_info.port_type == DNX_ALGO_PORT_TYPE_FRAMER_MAC)
    {
        ram_id = 2;
    }
    else if (src_port_info.port_type == DNX_ALGO_PORT_TYPE_FRAMER_SAR)
    {
        ram_id = 1;
    }
    else
    ...
    bcm_port_t * dst_port)
{
             ...
             (traffic_type == DBAL_ENUM_FVAL_FEU_RMC_TRAFFIC_TYPE_ILU_HP))
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_flexe_ilkn_channel_2_logicals_port(unit, port_or_channel, ilkn_port, TRUE, dst_port));
    }
    else
    {
        *dst_port = SOC_PORT_INVALID;
    ...
    bcm_port_t * dst_port)
{
     ...
     * get SAR channel 
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, src_port, &sar_channel));
    ...
    if ((sar_en) && (tx_chan_map != 0x200))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_flexe_ilkn_channel_2_logicals_port(unit, tx_chan_map, ilkn_port_id, TRUE, dst_port));
    }
    else
    {
        *dst_port = SOC_PORT_INVALID;
    ...
    bcm_port_t * dst_port)
{
     ...
     * interlaken channel 
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_in_channel_get(unit, src_port, &ilu_channel));
    SHR_IF_ERR_EXIT(dnx_algo_port_ilkn_access_info_get(unit, src_port, &ilkn_info));

    for (sar_channel = 0; sar_channel < dnx_data_nif.sar_client.nof_clients_get(unit); sar_channel++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_flexe_client_index_to_port_get(unit, DNX_ALGO_PORT_TYPE_FRAMER_SAR,
    ...
    bcm_port_t * dst_port)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_client_index_to_port_get(unit, DNX_ALGO_PORT_TYPE_FRAMER_MAC,
                                                                     target_client, dst_port));
    }
    else
    {
        rc = dnx_algo_port_flexe_client_index_to_port_get(unit, DNX_ALGO_PORT_TYPE_FRAMER_MAC, 0, dst_port);
        if (rc == _SHR_E_NONE)
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        dump_num = 0;
        for (src_channel = 0; src_channel < dnx_data_nif.sar_client.nof_clients_get(unit); src_channel++)
        {
            ...
            if (dst_num)
            {
                ...
                for (ii = 0; ii < dst_num; ii++)
                {
                    ...
                    sal_snprintf(dst_channels_name +
                                 sal_strnlen(dst_channels_name, sizeof(dst_channels_name)),
                                 sizeof(dst_channels_name) - sal_strnlen(dst_channels_name,
                                                                         sizeof(dst_channels_name)),
                                 "%-16s", channel_name);
                }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    dump_num = 0;
    for (channel = 0; channel < dnx_data_nif.sar_client.nof_clients_get(unit); channel++)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    dump_num = 0;
    for (instance_id = 0; instance_id < dnx_data_nif.flexe.nof_flexe_instances_get(unit); instance_id++)
    {
        oh_status = 0;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (ii = 0; ii < test_num; ii++)
    {
        ...
        SHR_IF_ERR_EXIT(_dnx_flexe_ptp_test_pkt_rx(unit, phy_instance, &t1, &t2, 0));
