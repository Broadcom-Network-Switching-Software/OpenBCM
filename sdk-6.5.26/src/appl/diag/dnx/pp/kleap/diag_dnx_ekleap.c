    ...
    signal_output_t * sig)
{
    ...
    if ((table_id != DBAL_NOF_TABLES) && (sw_result_type == -1))
    {
    ...
    if (dnx_data_multicast.params.max_egr_mc_groups_get(unit) != -1)
    {
        ...
        if (ppmc_match)
        {
            ...
            SHR_CLI_EXIT_IF_ERR(dbal_table_printable_entry_get
                                (unit, DBAL_TABLE_ETM_PP_DESCRIPTOR_EXPANSION, NULL, -1, core, sig_output->size,
                                 sig_output->value, entry_info), "Could not parse PPMC result using DBAL\n");
    ...
    uint8 *is_res_valid)
{
    ...
    *is_res_valid = FALSE;
    ...
    sal_memset(entry_info, 0, sizeof(dbal_printable_entry_t));
    SHR_IF_ERR_EXIT_NO_MSG(dbal_table_printable_entry_get
                           (unit, table_id, key_sig->value, -1, core, 0, NULL, entry_info));
    /*
     * Update Key signal
     */
