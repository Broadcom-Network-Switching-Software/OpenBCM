    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!UTILEX_IS_IN_RANGE(user_core_id, 0, dnx_data_device_general_nof_cores_for_traffic_get(unit) - 1)
        && user_core_id != BCM_CORE_ALL)
    {
        LOG_CLI((BSL_META("Illegal Core value %d - device has %u core(s)\n"), user_core_id,
                 dnx_data_device_general_nof_cores_for_traffic_get(unit)));
        SHR_EXIT();
    }
