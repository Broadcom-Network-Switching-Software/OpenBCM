...
/*
 * helper macros for performing the dbal action according to the field type & parameters
 */
...
#define DBAL_ENTRY_RESULT_TYPE_GET(unit, entry_handle_id, res_type, print_err, rt_str)\
do\
{\
    int rv;                                                                   \
    rv = dbal_result_type_string_to_id(unit, table_id, rt_str, &res_type);\
    if(rv)                                                                    \
    {                                                                         \
        if(print_err)                                                         \
        {                                                                     \
            LOG_CLI((BSL_META("result type name not valid %s, please use the string represent the result type value"), rt_str));\
...
#define DBAL_ENTRY_COMMIT_VALUE_FIELD(unit, entry_handle_id, field_id, inst_idx, res_type, print_err, sand_arg)    \
do                                                                                  \
{                                                                                   \
    dbal_field_types_defs_e field_type;                                             \
    SHR_IF_ERR_EXIT(dbal_fields_field_type_get(unit, field_id, &field_type));       \
    ...
    {                                                                               \
      ...
      {                                                                             \
          dbal_entry_value_field_arr32_set(unit, entry_handle_id, field_id, inst_idx,\
                                                 SH_SAND_ARG_ARRAY_DATA(sand_arg)); \
      }                                                                             \
    }                                                                               \
}while(0)
...
shr_error_e diag_dbal_table_autocomplete_cb(
    int unit,
