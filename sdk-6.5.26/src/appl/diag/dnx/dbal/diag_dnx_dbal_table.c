    ...
    sh_sand_control_t * sand_control)
{
    ...
    char_count = sal_snprintf(str, sizeof(buffer_to_print), "%s", "LOGICAL TABLES");
    str += char_count;

    if (label != DBAL_LABEL_NONE)
    {
        char_count = sal_snprintf(str, sizeof(buffer_to_print) - (str - buffer_to_print),
                                  ", with %s %s", "LABEL:", dbal_label_to_string(unit, label));
        str += char_count;
    }
    if (maturity_level != DBAL_NOF_MATURITY_LEVELS)
    {
        char_count = sal_snprintf(str, sizeof(buffer_to_print) - (str - buffer_to_print),
                                  ", with %s %s", "MATURITY:", dbal_maturity_level_to_string(unit, maturity_level));
        str += char_count;
    }
    if (status != DBAL_NOF_TABLE_STATUS)
    {
        char_count = sal_snprintf(str, sizeof(buffer_to_print) - (str - buffer_to_print),
                                  ", with %s %s", "STATUS:", dbal_table_status_to_string(unit, status));
        str += char_count;
    }
    ...
    {
        char_count = sal_snprintf(str, sizeof(buffer_to_print) - (str - buffer_to_print),
                                  ", with %s %s", "ACCESS:", dbal_access_method_to_string(unit, access_method));
        str += char_count;
    }
    ...
    int print_table)
{
    ...
    {
        if (nof_tables_found != AUTOCOMPLETE_PRINT_OUT_OF_SCOPE)
        {
            nof_tables_found++;
        }
        ...
        /*
         * if not printing the table, add table names to matching_table_names
         */
        if (!print_table)
        {
            CONST dbal_logical_table_t *table;
            SHR_IF_ERR_EXIT(dbal_tables_table_get(unit, (*table_id), &table));
            /*
             * check to see that adding the current table's name will not cause a buffer overflow
             * if so, print the table with the matches instead of returning the matches
             */
            if (sal_strnlen(matching_table_names, AUTOCOMPLETE_MAX_STRING_LEN) +
                sal_strnlen(table->table_name, sizeof(table->table_name)) >= AUTOCOMPLETE_MAX_STRING_LEN)
            {
                matching_table_names[0] = '\0';
                print_table = TRUE;
                /*
                 * if the autocomplete options are being printed instead of returned, set nof_tables_found to reflect this
                 */
                nof_tables_found = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                /*
                 * ensures that the top row of the printed table will be neatly aligned with the rest of the table
                 */
                LOG_CLI((BSL_META("\n")));
            }
            else
            {
                sal_strncat(matching_table_names, table->table_name, AUTOCOMPLETE_MAX_STRING_LEN);
                sal_strncat(matching_table_names, " ", AUTOCOMPLETE_MAX_STRING_LEN);
            }
        }
    ...
    else if (nof_tables_found == AUTOCOMPLETE_PRINT_OUT_OF_SCOPE)
    {
        /*
         * if the autocomplete options are being printed instead of returned, set table_id to reflect this
         */
        (*table_id) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
    }
...
/**
* \brief
* Find the dbal table/s that match the user-provided substring and place the found table names in table_full_name
* It is the responsibility of this function, or whomever it calls, to ensure there is no buffer overflow on table_full_name
*/
shr_error_e
diag_dbal_table_autocomplete_cb(
    int unit,
    char *table_substring,
    char *table_full_name,
    int *num_matches_found)
{
    ...
    SHR_IF_ERR_EXIT(diag_dbal_table_from_string(unit, table_substring, &table_id, NULL, table_full_name, FALSE));
    /*
     * if there are no matches or multiple matches, table_id contains the value DBAL_NOF_DYNAMIC_AND_STATIC_TABLES
     * if table_full_name isn't empty, there must be at least one match
     * combined, this condition evaluates to true when there are multiple matches
     */
    if (table_id == DBAL_NOF_DYNAMIC_AND_STATIC_TABLES && table_full_name[0] != 0)
    {
        char *tmp_str;
        char str_cpy[AUTOCOMPLETE_MAX_STRING_LEN] = { 0 };
        sal_strncpy(str_cpy, table_full_name, AUTOCOMPLETE_MAX_STRING_LEN - 1);
        tmp_str = sal_strtok_r(str_cpy, " ", &tokstr);
        /*
         * count the number of tables that were a partial match
         */
        while (tmp_str != NULL)
        {
            (*num_matches_found)++;
            tmp_str = sal_strtok_r(NULL, " ", &tokstr);
        }
    }
    else if (table_id == AUTOCOMPLETE_PRINT_OUT_OF_SCOPE)
    {
        /*
         * if the autocomplete options are being printed instead of returned, set num_matches_found to reflect this
         */
        (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
    }
    /*
     * table_id is not DBAL_NOF_DYNAMIC_AND_STATIC_TABLES only when there is exactly 1 match
     */
    else if (table_id != DBAL_NOF_DYNAMIC_AND_STATIC_TABLES)
    {
        (*num_matches_found) = 1;
    }
    ...
    int *estimated_nof_entries)
{
    ...
    if (table->access_method == DBAL_ACCESS_METHOD_KBP)
    {
        ...
        SHR_IF_ERR_EXIT(kbp_mngr_db_capacity_get(unit, db_id, &num_of_entries, &estimated_capacity, NULL));
    ...
    else if (DBAL_TABLE_IS_NONE_DIRECT(table))
    {
        if (table->access_method == DBAL_ACCESS_METHOD_MDB)
        {
            ...
            if (table->table_type == DBAL_TABLE_TYPE_LPM)
            {
                ...
                /** For LPM pass the result type instead of payload_size */
                payload_size = table->multi_res_info[result_type].phys_table_rt;
            }
            else
            {
    ...
    entry_struct_t * entry)
{
    ...
    {
        if (DBAL_TABLE_IS_TCAM_BY_ID(entry->table))
        {
            ...
            if ((entry->table->access_method == DBAL_ACCESS_METHOD_MDB)
                || (entry->table->access_method == DBAL_ACCESS_METHOD_KBP))
            {
    ...
    entry_struct_t * entry)
{
    ...
    DBAL_ENTRY_COMMIT_VALUE_FIELD(unit, (entry->entry_handle_id), field_id, INST_SINGLE, res_type, 0, (&parsed_param));
...
* them in entry handle for commit
*****************************************************/
static shr_error_e
...
/**
* \brief
* parsing xml file dbal entries and committing them into dbal
*****************************************************/
static shr_error_e
    ...
    sh_sand_control_t * sand_control)
{
    ...
    RHDATA_ITERATOR(cur_sub, curTop, "Entries")
    {
        ...
        SHR_IF_ERR_EXIT(diag_dbal_table_from_string(unit, table_name, &(entry.table_id), sand_control, NULL, TRUE));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        if (sal_strncasecmp(find_str, fields_list[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            if ((sal_strncasecmp("Priority", find_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
                || (sal_strncasecmp("AccessId", find_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0))
            {
                SHR_IF_ERR_EXIT(utilex_str_stoul(values_list[i], &(val->val_uint32)));
                SHR_EXIT();
            }
            else if (sal_strncasecmp("N/A", values_list[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (i = 0; i < num_of_values; i++)
    {
        if (sal_strncasecmp("Priority", fields_list[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0
            || sal_strncasecmp("AccessId", fields_list[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0
            || sal_strncasecmp("Mask", fields_list[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0
            || sal_strncasecmp("N/A", values_list[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            continue;
        }
        mask = NULL;
        if ((i + 1 < num_of_values)
            && sal_strncasecmp("Mask", fields_list[i + 1], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    while (sal_fgets(line, SH_SAND_MAX_TOKEN_SIZE - 1, file))
    {
         ...
         */
        if (sal_strncmp("Table=", tmp_str, sal_strnlen("Table=", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0)
        {
            ...
            sal_memset(table_name, 0, sizeof(table_name));
            sal_strlcpy(table_name, tmp_str + sal_strnlen("Table=", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH),
                        sizeof(table_name));
    ...
    char *filename)
{
    ...
    if (realtokens > 1)
    {
        ...
        if (!sal_strncmp(extension, "dcsv", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
            || !sal_strncmp(extension, "txt", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            ret = DNX_DBAL_FILE_TYPE_DCSV;
        else if (!sal_strncmp(extension, "xml", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (ISEMPTY(table_name))
    {
        ...
        {
            if (sal_strncasecmp(table_status_str, "initialized", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
                table_status = DBAL_TABLE_INITIALIZED;
            }
            else if (sal_strncasecmp(table_status_str, "hwerr", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
                table_status = DBAL_TABLE_HW_ERROR;
            }
            else if (sal_strncasecmp(table_status_str, "NotInit", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
                table_status = DBAL_TABLE_NOT_INITIALIZED;
            }
            else if (sal_strncasecmp
                     (table_status_str, "incompatibleImage", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
    ...
    char **rule_val_str)
{
    ...
    else if (sal_strncmp(rule_str, "odd", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *cond = DBAL_CONDITION_IS_ODD;
    }
    else if (sal_strncmp(rule_str, "even", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        ...
        SH_SAND_GET_ITERATOR(sand_arg)
        {
            ...
            if (field_id == DBAL_FIELD_EMPTY)
            {
                if (sal_strncasecmp(rule_name_str, "HITBIT", strnlen_rule_name_str) == 0)
                {
                    uint32 attr_val;
                    uint32 attr_type =
                        (DBAL_ENTRY_ATTR_FLAG_HIT_GET | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY |
                         DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY);
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot parse rule for attribute %s. Supported attribute: HITBIT\n",
                                 rule_name_str);
                }
            }
            else
            {
                ...
                if (field_id == DBAL_FIELD_RESULT_TYPE)
                {
                    int rv = 0;
                    uint32 res_type = 0;
                    rv = dbal_result_type_string_to_id(unit, table_id, rule_val_str, &res_type);
                    if (rv)
                    {
                        LOG_CLI((BSL_META("result type name not valid %s"), rule_val_str));
                        SHR_SET_CURRENT_ERR(_SHR_E_PARAM);
                        SHR_EXIT();
                    }
                    sal_itoa(rule_val_str, res_type, 10, 0, 0);
                }
                /** Parse rule value, from string to any sal_type */
...
int
diag_table_is_allocator_field_used(
    int unit,
    dbal_tables_e table_id)
{
    switch (table_id)
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (diag_table_is_allocator_field_used(unit, table_id) && (table->allocator_field_id != DBAL_FIELD_EMPTY))
    {
        int result_type, index = 0;
        uint32 core_id = (table->core_mode == DBAL_CORE_MODE_DPC) ? 0 : DBAL_CORE_ALL;
        SHR_IF_ERR_EXIT(dnx_algo_res_dbal_get_next(unit, core_id, 0, table_id, table->allocator_field_id,
                                                   &result_type, &index));
        while ((index != DNX_ALGO_RES_DBAL_ILLEGAL_ELEMENT)
               || ((table->core_mode == DBAL_CORE_MODE_DPC) && (core_id < DBAL_MAX_NUM_OF_CORES - 1)))
        {
            if (index != DNX_ALGO_RES_DBAL_ILLEGAL_ELEMENT)
            {
                SHR_IF_ERR_EXIT(dnx_algo_res_dbal_free
                                (unit, core_id, 0, index, table_id, result_type, table->allocator_field_id));
            }
            /*
             * else is for DPC only, no more values for this core, increment to next_core
             */
            else
            {
                ++core_id;
    ...
    while (!is_end)
    {
        ...
        for (iter = 0; (iter < entry_handle->nof_key_fields) || (iter < entry_handle->nof_result_fields); iter++)
        {
            ...
            if (iter < entry_handle->nof_key_fields)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_field_in_table_printable_string_get(unit, table->keys_info[iter].field_id, 0,
            ...
            if (iter < table->multi_res_info[curr_res_type_index].nof_result_fields)
            {
                ...
                dbal_field_data_t field_data;
                dbal_table_field_info_t *field_table = &table->multi_res_info[curr_res_type_index].results_info[iter];
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, field_table->field_id,
                                                                        field_table->inst_idx, field_data.field_val));
                ...
                SHR_IF_ERR_EXIT(dbal_field_in_table_printable_string_get
                                (unit, table->multi_res_info[curr_res_type_index].results_info[iter].field_id,
                                 table->multi_res_info[curr_res_type_index].results_info[iter].inst_idx,
                                 table_id, field_data.field_val, NULL, curr_res_type_index, FALSE, FALSE, buffer));
                PRT_CELL_SET("%s", field_name);
                PRT_CELL_SET("%s", buffer);
            }
            else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncmp(table_name, "ALL", sal_strnlen(table_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0)
    {
        if (!export_to_file)
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_indication_get(unit, table_id, DBAL_TABLE_IND_IS_HITBIT_EN, &hitbit_en));
    /** check if the requested table support priority */
    SHR_IF_ERR_EXIT(dbal_tables_indication_get(unit, table_id, DBAL_TABLE_IND_IS_PRIORITY_SUPPORTED, &priority_en));
    ...
    for (res_type_index = 0; res_type_index < table->nof_result_types; res_type_index++)
    {
        ...
        PRT_COLUMN_ADD_FLEX_PL_WIDTH(res_type_index, PRT_FLEX_ASCII, PRT_COLUMN_WIDTH_INTEGER, "#");
        ...
        /*
         * Add the priority if it's enabled
         */
        if (priority_en)
        {
            PRT_COLUMN_ADD_FLEX_PL(res_type_index, PRT_FLEX_ASCII, "Priority");
        }
        ...
        {
            /*
             * Optimization of column size to save memory consumption in case field type is VRF
             */
            if (table->keys_info[iter].field_type == DBAL_FIELD_TYPE_DEF_VRF)
            {
                column_width = PRT_COLUMN_WIDTH / 2.5;
            }
            else
            {
                column_width = PRT_COLUMN_WIDTH * 2;
            }
            PRT_COLUMN_ADD_FLEX_PL_WIDTH(res_type_index, PRT_FLEX_ASCII, column_width, "%s",
        ...
        if (!dump_only_key_fields)
        {
            ...
            /*
             * empty column that created for separating between keys and results
             */
            PRT_COLUMN_ADD_FLEX_PL_WIDTH(res_type_index, PRT_FLEX_ASCII, 2, "");
            /*
             * Result columns
             */
            for (iter = first_res_index; iter < table->multi_res_info[res_type_index].nof_result_fields; iter++)
            {
                table_field_info = &table->multi_res_info[res_type_index].results_info[iter];
                dbal_field_with_inst_to_string(unit, table_field_info->field_id, table_field_info->inst_idx,
                                               field_string);
                ...
                {
                    PRT_COLUMN_ADD_FLEX_PL(res_type_index, PRT_FLEX_ASCII, "%s (include valid bit)", field_string);
                }
                else
                {
                    /*
                     * Optimization of column size to save memory consumption in case field type is KAPS_RESULT
                     */
                    if (table->multi_res_info[res_type_index].results_info[iter].field_type ==
                        DBAL_FIELD_TYPE_DEF_KAPS_RESULT)
                    {
                        column_width = PRT_COLUMN_WIDTH;
                    }
                    else
                    {
                        column_width = 2 * PRT_COLUMN_WIDTH;
                    }
                    PRT_COLUMN_ADD_FLEX_PL_WIDTH(res_type_index, PRT_FLEX_ASCII, column_width, "%s", field_string);
                }
            }
        }
        ...
        /*
         * For TCAM tables - add IS_VALID indication
         */
        if (DBAL_TABLE_IS_TCAM(table))
        {
            PRT_COLUMN_ADD_FLEX_PL(res_type_index, PRT_FLEX_ASCII, "IS_VALID");
        }
    }
    ...
    while (!is_end)
    {
        ...
        {
            uint32 priority = -1;
            SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_PRIORITY, &priority));
            PRT_CELL_SET_PL(curr_res_type_index, "%d", priority);
        }
        ...
        for (iter = 0; iter < entry_handle->nof_key_fields; iter++)
        {
            ...
            if (entry_handle->key_field_ids[iter] == DBAL_FIELD_CORE_ID)
            {
                ...
                {
                    sal_snprintf(buffer, sizeof(buffer), "ALL");
                }
                else
                {
                    sal_snprintf(buffer, sizeof(buffer), " %d ", entry_handle->core_id);
                }
            }
            else
            {
        ...
        if (!dump_only_key_fields)
        {
            ...
            for (iter = first_res_index; iter < table->multi_res_info[curr_res_type_index].nof_result_fields; iter++)
            {
                ...
                if (is_no_payload)
                {
                    PRT_CELL_SET_PL(curr_res_type_index, "no_payload");
                    continue;
                }

                rv = dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, field_table->field_id,
                                                             field_table->inst_idx, field_data.field_val);
        ...
        if (hitbit_en)
        {
            ...
            {
                uint32 hitbit;
                SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_HITBIT, &hitbit));
        ...
        {
            uint32 invalid;
            SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_INVALID, &invalid));
            /** Column name is 'IS_VALID' */
            PRT_CELL_SET_PL(curr_res_type_index, invalid ? "FALSE" : "TRUE");
        }
    ...
    for (res_type_index = 0; res_type_index < table->nof_result_types; res_type_index++)
    {
        if ((nof_rules == 0) || (entry_counter[res_type_index] > 0))
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** if no general attribute requested table name must exists */
    if (ISEMPTY(label_str) && (is_image_specific == 0) && (is_dirty == 0))
    {
        if (ISEMPTY(table_name))
        {
            DIAG_DBAL_HEADER_DUMP("table name must exist: dbal table dump Table=<table_name>", "\0");
            SHR_EXIT();
        }
    }
    else
    ...
    rhlist_t * examples_list)
{
    ...
    sal_snprintf(cmd[0], sizeof(cmd[0]), "FILENaMe=%s/tools/sand/db/jericho2_a0/dbalPopulateExample.xml", sdk_dir);
    sal_snprintf(cmd[1], sizeof(cmd[1]), "FILENaMe=%s/tools/sand/db/jericho2_a0/dbalPopulateExample.dcsv", sdk_dir);
...
static sh_sand_option_t diag_dbal_table_list_options[] = {
    {"TaBLe", SAL_FIELD_TYPE_STR, "dbal table name", "", NULL, NULL, SH_SAND_ARG_FREE,.ac_option_cb =
     diag_dbal_table_autocomplete_cb},
    ...
    {"image_specific", SAL_FIELD_TYPE_INT32, "This table is valid for this image only", "0"},
...
static sh_sand_man_t diag_dbal_table_indication_man = {
    .brief = "table indications manual. allow to dump or update table indications",
    ...
    .examples =
        "taBLe=IPV4_UNICAST_PRIVATE_HOST\n"
        "taBLe=IPV4_UNICAST_PRIVATE_HOST indication=validation value=false\n"
        "taBLe=IPV4_UNICAST_PRIVATE_HOST indication=validation value=true"
};
