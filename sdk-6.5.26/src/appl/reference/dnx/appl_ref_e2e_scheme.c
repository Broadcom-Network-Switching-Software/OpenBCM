 ...
 */
#define APPL_DNX_E2E_CREDIT_PROFILE_DELETE            (30)
/**
 * \brief - credit request profile id allocated for push queues
 */
...
/**
 * \brief - rate class profile for push queue. relevant only when credit request profile is unified with rate class.
 *
 */
#define APPL_DNX_E2E_RATE_CLASS_PUSH_QUEUE    (6)
/**
 * \brief - rate class profile id with tail drop 0. destroyed voqs will be mapped to this rate class to discard all traffic.
 *
 */
#define APPL_DNX_E2E_RATE_CLASS_PROFILE_DISCARD    (7)
/*
 * }
 */
    ...
    int *base_voq_connector)
{
    ...
    int remote_device_index, remote_core_id, ftmh_pp_dsp, modid_index;
    ...
    SHR_IF_ERR_EXIT(appl_dnx_modid_to_fap_and_core_convert(unit, remote_modid, &remote_device_index, &remote_core_id));

    /** covert logical port to system port map */
    SHR_IF_ERR_EXIT(appl_dnx_logical_port_to_sysport_map_get(unit, port, &sys_port_map));

    /** extract ftmh_pp_dsp and modid index from system port */
    SHR_IF_ERR_EXIT(appl_dnx_sysport_map_to_ftmh_pp_dsp_and_modid_index_get(unit, &sys_port_map, &ftmh_pp_dsp,
                                                                            &modid_index));

    /** call internal function to get voq_connector id */
    SHR_IF_ERR_EXIT(appl_sand_device_to_base_voq_connector_id_convert(unit, ftmh_pp_dsp, modid_index, BCM_CORE_ALL,
                                                                      remote_device_index, sys_params->nof_devices,
                                                                      sys_params->max_nof_ports_per_modid,
                                                                      sys_params->max_nof_cores, base_voq_connector));
 ...
 */
shr_error_e
appl_dnx_e2e_voq_profiles_final_get(
    int unit,
    ...
    int *rate_class)
{
        ...
        dnx_data_tdm.params.mode_get(unit) == TDM_MODE_PACKET)
    {
        ...
        if (sys_port_map->port_type == APPL_PORT_TYPE_TDM)
        {
            {
 ...
 */
shr_error_e
appl_dnx_e2e_scheme_discard_rate_class_profile_get(
    int unit,
    bcm_gport_t * discard_rate_class)
{
    SHR_FUNC_INIT_VARS(unit);

    BCM_GPORT_PROFILE_SET(*discard_rate_class, APPL_DNX_E2E_RATE_CLASS_PROFILE_DISCARD);
    ...
    int ingress_device_idx)
{
    ...
    int cos, nof_connections_per_hr, if_speed, ftmh_pp_dsp, modid_index;
    ...
    SHR_IF_ERR_EXIT(appl_sand_device_to_base_voq_connector_id_convert(unit, ftmh_pp_dsp, modid_index, BCM_CORE_ALL,
                                                                      ...
                                                                      sys_params->max_nof_ports_per_modid,
                                                                      sys_params->max_nof_cores, &voq_connector_id));
    ...
    voq_connector_config.nof_remote_cores = sys_params->max_nof_cores;
    voq_connector_config.numq = APPL_SAND_E2E_SCHEME_COS;

    /** there is no specific port on remote device - use pp dsp 0 */
    SHR_IF_ERR_EXIT(appl_dnx_fap_and_pp_dsp_to_modid_convert
    ...
    int ingress_device_idx)
{
    ...
    bcm_gport_t local_gport, voq_connector_gport;
    bcm_cosq_gport_info_t gport_info;
    int cos, ftmh_pp_dsp, modid_index;
    int voq_connector_id;
    ...
    int is_disconnect)
{
    ...
    SHR_IF_ERR_EXIT(appl_sand_device_to_base_voq_connector_id_convert
                    (unit, ftmh_pp_dsp, modid_index, BCM_CORE_ALL, sys_params->unit_to_device_index[unit],
                     sys_params->nof_devices, sys_params->max_nof_ports_per_modid, sys_params->max_nof_cores,
                     &voq_connector_id));
    ...
    int is_disconnect)
{
    appl_dnx_sys_device_t *sys_params;
    bcm_cosq_gport_connection_t connection;
    int voq_base_id, voq_connector_id, ftmh_pp_dsp, modid_index;
    ...
    SHR_IF_ERR_EXIT(appl_dnx_sysport_map_to_ftmh_pp_dsp_and_modid_index_get(unit, sys_port_map, &ftmh_pp_dsp,
                                                                            &modid_index));
    SHR_IF_ERR_EXIT(appl_sand_device_to_base_voq_connector_id_convert
                    (unit, ftmh_pp_dsp, modid_index, BCM_CORE_ALL, ingress_device_index, sys_params->nof_devices,
                     sys_params->max_nof_ports_per_modid, sys_params->max_nof_cores, &voq_connector_id));
    BCM_COSQ_GPORT_VOQ_CONNECTOR_CORE_SET(connection.voq_connector, voq_connector_id, sys_port_map->core);

    /** set voq id */
    BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_QUEUE_SET(connection.voq, BCM_CORE_ALL, voq_base_id);

    /** set remote modid, core and PP DSP can be 0 because all that matters at this point is reaching the correct device */
    SHR_IF_ERR_EXIT(appl_dnx_fap_and_pp_dsp_to_modid_convert
    ...
    const system_port_mapping_t * sys_port_map)
{
     ...
     */
    {
        ...
        /** Skip L1 ports */
        if (port_flags & (BCM_PORT_ADD_CROSS_CONNECT | BCM_PORT_ADD_FLEXE_PHY | BCM_PORT_ADD_FLEXE_SAR))
        {
            SHR_EXIT();
        }

        port_flags = BCM_COSQ_SCHEDULER_GPORT_REPLACE;
        ...
        for (sch_priority = 0; sch_priority < mapping_info.num_sch_priorities; sch_priority++)
        {
            ...
            gport_info.cosq = sch_priority;
            SHR_IF_ERR_EXIT(bcm_cosq_gport_handle_get(unit, bcmCosqGportTypeSched, &gport_info));
            e2e_gport = gport_info.out_gport;
            SHR_IF_ERR_EXIT(bcm_cosq_scheduler_gport_add(unit, port_flags, &params, &e2e_gport));
        }
    }
    ...
    for (ingress_device_index = 0; ingress_device_index < sys_params->nof_devices; ingress_device_index++)
    {
        ...
        SHR_IF_ERR_EXIT(appl_dnx_e2e_scheme_voq_connector_create(unit, sys_port_map, ingress_device_index));
    }
    ...
    int port)
{
    ...
    if (is_local_port)
    {
        for (int ingress_device_index = 0; ingress_device_index < sys_params->nof_devices; ingress_device_index++)
        {
            ...
            SHR_IF_ERR_EXIT(appl_dnx_e2e_scheme_voq_connector_destroy(unit, &sys_port_map, ingress_device_index));
        }
    }
        ...
        for (numq = 0; numq < queue_bundle_config.numq; numq++)
        {
            delay_tolerance_level =
                dnx_data_iqs.credit.feature_get(unit,
                                                dnx_data_iqs_credit_profile_is_rate_class) ? -1 :
                APPL_DNX_E2E_CREDIT_PROFILE_MC_SLOW;
            queue_bundle_config.queue_atrributes[numq].delay_tolerance_level = delay_tolerance_level;
    ...
    int unit)
{
    ...
    for (profile_index = 0; profile_index < COUNTOF(profiles); profile_index++)
    {
        ...
        BCM_GPORT_PROFILE_SET(rate_class_gport, profiles[profile_index]);
 ...
 */
shr_error_e
appl_dnx_e2e_discard_rate_class_create(
    int unit)
{
    ...
    create_info.rate = 1;       /* rate is irrelevant for discard */
    create_info.attributes = 0;
    if (dnx_data_iqs.credit.feature_get(unit, dnx_data_iqs_credit_profile_is_rate_class))
    {
        /*
         * credit size is irrelevant for discard
         */
        create_info.credit_size = dnx_data_iqs.credit.worth_get(unit);
    }
    SHR_IF_ERR_EXIT(bcm_cosq_gport_rate_class_create(unit, discard_rate_class, 0, &create_info));

    /** set voq tail drop to 0 */
    flags = BCM_COSQ_GPORT_SIZE_BYTES | BCM_COSQ_GPORT_SIZE_COLOR_BLIND;
    SHR_IF_ERR_EXIT(bcm_cosq_gport_color_size_get(unit, discard_rate_class, 0, 0, flags, &gport_size));
    gport_size.size_max = 0;
    gport_size.size_fadt_min = 0;
    SHR_IF_ERR_EXIT(bcm_cosq_gport_color_size_set(unit, discard_rate_class, 0, 0, flags, &gport_size));
    ...
    int unit)
{
    ...
    int profiles[] = { APPL_DNX_E2E_CREDIT_PROFILE_10G_SLOW, APPL_DNX_E2E_CREDIT_PROFILE_40G_SLOW,
        ...
        APPL_DNX_E2E_CREDIT_PROFILE_PUSH_QUEUE, APPL_DNX_E2E_CREDIT_PROFILE_DELETE
    };
    uint32 root_fmq_max_rate = dnx_data_tune.iqs.fmq_max_rate_get(unit);
    int profile_rates[] = { 10, 40, 100, 200, 400, root_fmq_max_rate, 0, 0 };   /* rates in gbps */
    int profile_index;
    ...
    for (profile_index = 0; profile_index < COUNTOF(profiles); profile_index++)
    {
        ...
        /** delete profile */
        if (profiles[profile_index] == APPL_DNX_E2E_CREDIT_PROFILE_DELETE)
        {
            credit_request_attr.flags = BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_DELETE;
        }
    ...
    int unit)
{
    ...
    /** set egress TCG CIR strict priority if required */
    ...
    {
        /** this configuration is required for devices whose Arch doesn't support TCG mapping */
        /** this configuration should maintain the same default behavior we have in devices that do support TCG mapping */
        BCM_COSQ_GPORT_CORE_SET(gport, BCM_CORE_ALL);
        SHR_IF_ERR_EXIT(bcm_cosq_control_set(unit, gport, -1, bcmCosqControlEgressTCGCirSpEnable, 1));
    }
exit:
    SHR_FUNC_EXIT;
}
