    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(diag_dbal_table_from_string(unit, table_ptr, &table_id, sand_control, NULL, TRUE));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (i = 0; i < minicon_armor_get_num_of_tables(unit); i++)
    {
        SHR_IF_ERR_EXIT(diag_dbal_table_from_string
                        (unit, minicon_armor_get_table_name_by_index(unit, i), &table_id, sand_control, NULL, TRUE));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp(table, "ALL", sal_strnlen(table, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0)
    {
    ...
    else
    {
        if (!ISEMPTY(table))
        {
            SHR_IF_ERR_EXIT(diag_dbal_table_from_string(unit, table, &table_id, sand_control, NULL, TRUE));
            SHR_IF_ERR_EXIT(diag_dnx_armor_clear_table(unit, table_id, args, sand_control));
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_strncpy(algo_path, tmp_algo_path, sal_strnlen(tmp_algo_path, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH));
    sal_strncpy(res_dir, tmp_res_dir, sal_strnlen(tmp_res_dir, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char xml_file_ips[SH_SAND_MAX_TOKEN_SIZE] = { 0 };
    char xml_file_tcams[SH_SAND_MAX_TOKEN_SIZE] = { 0 };
    ...
    if (sal_strncasecmp("load_compressed", action, sal_strnlen(action, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) ==
        0)
    {
    ...
    {
        if (sal_strncasecmp("compress_only", action, sal_strnlen(action, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            == 0)
        {
            acl_compress_only = TRUE;
        }
        else if (sal_strncasecmp
                 ("compress_and_convert", action,
                  sal_strnlen(action, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0)
        {
            convert_only = TRUE;
        }
        else if (sal_strncasecmp
                 ("compress_and_load", action, sal_strnlen(action, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) != 0)
        {
        ...
        {
            if ((sal_strncasecmp(format, "default", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
                && (sal_strncasecmp(format, "format_1", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0))
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp(table, "ALL", sal_strnlen(table, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0)
    {
        ...
        {
            SHR_IF_ERR_EXIT(diag_dbal_table_from_string
                            (unit, minicon_armor_get_table_name_by_index(unit, i), &table_id, sand_control, NULL,
                             TRUE));
