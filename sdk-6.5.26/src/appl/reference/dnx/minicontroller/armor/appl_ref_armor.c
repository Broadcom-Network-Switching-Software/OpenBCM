    ...
    int unit)
{
    ...
    for (table_id = 0; table_id < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); table_id++)
    {
        ...
        if (sal_strnstr(table->table_name, MINICON_ARMOR_NAME_STR, sizeof(table->table_name)) != NULL)
        {
    ...
    bcm_field_context_t context_id)
{
    ...
    int nof_ids;
    char proc_name[] = "pmf_compressed_tcam";
    ...
    sal_strncpy_s((char *) (action_info.name), "TCAM_OVF_INDEX_ACTION", sizeof(action_info.name));
    ...
    sal_strlcpy((char *) (fg_info.name), "ARMOR_TCAM_OVF", sizeof(fg_info.name));

    SHR_IF_ERR_EXIT(bcm_field_group_add(unit, 0 | BCM_FIELD_GROUP_CREATE_WITH_ID, &fg_info, &fg_id));
    ...
    sal_strncpy_s((char *) (name_to_id_info.name), "vw_ARMOR___sip_group", sizeof(name_to_id_info.name));
     ...
     */
    bcm_field_name_to_id_info_t_init(&name_to_id_info);
    sal_strncpy_s((char *) (name_to_id_info.name), "vw_ARMOR___dip_group", sizeof(name_to_id_info.name));
    name_to_id_info.name_to_id_type = bcmFieldNameToIdQualifier;
    SHR_IF_ERR_EXIT(bcm_field_name_to_id(unit, 0, &name_to_id_info, &nof_ids, &id_get));
    if (nof_ids != 1)
    {
    ...
    qual_info.size = 14;
    sal_strncpy_s((char *) (qual_info.name), "TCAM_OVF_INDEX_QUALIFIER", sizeof(qual_info.name));
     ...
     */
    bcm_field_action_info_t_init(&action_info);
    sal_strncpy_s((char *) (action_info.name), "USER_DST_DATA", sizeof(action_info.name));
     ...
     */
    bcm_field_action_info_t_init(&action_info);
    sal_strncpy_s((char *) (action_info.name), "USER_STATISTICS_ID_0", sizeof(action_info.name));
     ...
     */
    bcm_field_action_info_t_init(&action_info);
    sal_strncpy_s((char *) (action_info.name), "USER_STATISTICS_ATR_0", sizeof(action_info.name));
    ...
    sal_strlcpy((char *) (fg_info.name), "ARMOR_TCAM_COMPRESSED", sizeof(fg_info.name));

    SHR_IF_ERR_EXIT(bcm_field_group_add(unit, 0 | BCM_FIELD_GROUP_CREATE_WITH_ID, &fg_info, &fg_id2));
    ...
    char *filename)
{
    ...
    if (cur_res != NULL)
    {
        ...
        RHDATA_ITERATOR(cur_sub, cur_res, "Field")
        {
            ...
            sal_snprintf(old_str, sizeof(old_str), "<Entries Table=\"%s\">", field_name);
            ...
            if ((cur_field = dbx_xml_child_get_first(cur_res, "NewField")) != NULL)
            {
                ...
                if (!ISEMPTY(field_name))
                {
                    ...
                    sal_strlcpy(rename_handle->key_string, old_str, sizeof(rename_handle->key_string));
                    sal_snprintf(rename_handle->rename_string, sizeof(rename_handle->rename_string),
                                 "<Entries Table=\"%s\">", field_name);
                }
            }
        }
    }
    ...
    if (cur_res != NULL)
    {
        RHDATA_ITERATOR(cur_sub, cur_res, "Field")
        {
            ...
            sal_snprintf(old_str, sizeof(old_str), "Name=\"%s\" Value=\"%s\"", field_name, field_val);
            ...
            if ((cur_field = dbx_xml_child_get_first(cur_sub, "NewField")) != NULL)
            {
                ...
                if (!ISEMPTY(field_name) && !ISEMPTY(field_val))
                {
                    ...
                    sal_strlcpy(rename_handle->key_string, old_str, sizeof(rename_handle->key_string));
                    sal_snprintf(rename_handle->rename_string, sizeof(rename_handle->rename_string),
                                 "Name=\"%s\" Value=\"%s\"", field_name, field_val);
                }
            }
            ...
            if ((cur_field = dbx_xml_child_get_first(cur_sub, "AddField")) != NULL)
            {
                ...
                if (!ISEMPTY(field_name) && !ISEMPTY(field_val))
                {
                    ...
                    sal_strlcpy(rename_handle->key_string, old_str, sizeof(rename_handle->key_string));
                    sal_snprintf(rename_handle->rename_string, sizeof(rename_handle->rename_string),
                                 "<Field Name=\"%s\" Value=\"%s\"/>", field_name, field_val);
                }
            }
    ...
    char *out_file)
{
    ...
    while (sal_fgets(read_str, RHSTRING_MAX_SIZE, in_fp) != NULL)
    {
        ...
        sal_strlcpy(copy_str, str_ptr, sizeof(copy_str));

        RHITERATOR(rhhandle, minicon_armor_appl_info[unit]->table_rename_list)
        {
            rename_handle = rhhandle;
            if (sal_strnstr(str_ptr, rename_handle->key_string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != NULL)
            {
        ...
        {
            rename_handle = rhhandle;
            if (sal_strnstr(copy_str, rename_handle->key_string, sizeof(copy_str)) != NULL)
            {
