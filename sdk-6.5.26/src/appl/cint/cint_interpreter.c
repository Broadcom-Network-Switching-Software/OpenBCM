...
_cint_interpreter_include_search(const char* name, char** path_rv)
{
    ...
    for (l = __directory_list; l ; l = l->next) {
        ...
        *p = 0;
        ...
        CINT_STRNCAT(p, name, CINT_STRLEN(l->directory)+1+len_name-CINT_STRLEN(p)); 
...
static char* cint_get_file(void) {
...
int
cint_interpreter_parse(void* handle, const char* prompt, int argc, char** argv, const char* file_name)
{
    cint_ast_t* ast;    
    ...
    cint_cparser_start_handle(cp, handle, file_name); 
