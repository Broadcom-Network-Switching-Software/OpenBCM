                 ...
                 void *cookie)
{
    ...
    LOG_VERBOSE(BSL_LS_TKS_TX,
                (BSL_META("ATP simple %d:  NH %d. ctf %x.\n"),
                 client->client_id, next_hop, ct_flags));
       ...
       void *cookie)
{
    ...
    LOG_DEBUG(BSL_LS_TKS_ATP,
              (BSL_META("TX cli %d, flags 0x%x ctflags 0x%x, cb to "
                        CPUDB_KEY_FMT_EOLN), 
               client_id, client->flags, ct_flags,
...
atp_dump(int verbose)
{
    ...
    LOG_CLI((BSL_META("tx free %p. rx free %p.\n"),
              (void *) tx_trans_freelist,
              (void *) rx_trans_freelist));
            ...
            for (cpu = 0; cpu < CPUDB_CPU_MAX ; cpu++) {
                ...
                for (rx_trans = client->cpu[cpu].rx_trans;
                    ...
                    LOG_CLI((BSL_META("    CPU %d: RX %p: flags 0x%x. cpu %d. rsegs %d. "
                              "ack %d.\n"), cpu, (void *) rx_trans, rx_trans->flags,
                    ...
                    LOG_CLI((BSL_META("      time %u. cli %d. pkt %p. len %d. seq %d\n"),
                              ...
                              (void *) rx_trans->pkt, rx_trans->payload_len,
    ...
    LOG_CLI((BSL_META("BET queue %p tail %p\n"),
              (void *) bet_queue, (void *) bet_queue_tail));
