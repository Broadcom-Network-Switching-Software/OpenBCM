...
flexport_by_config_file(int unit, char *fname)
{
    ...
    while (sal_fgets(line, sizeof(line), fp)) {
        if (sal_strstr(line, "#") != line) {
            ...
            split1 = sal_strtok_r(line_cpy1,"=", &rest);
            port_idx = 0;
            while (split1) {
                port_str[port_idx++] = split1;
                split1 = sal_strtok_r(NULL,"=", &rest);
            }
            ...
            if (sal_strstr(line, dic_portmap)) {
                ...
                split2 = sal_strtok_r(line_cpy2,":", &rest);
                port_data_idx = 0;
                while (split2) {
                    port_data[port_data_idx++] = split2;
                    split2 = sal_strtok_r(NULL,":", &rest);
                }
                ...
                split3 = sal_strtok_r(line_cpy3,"_", &rest);
                l2p_idx = 0;
                while (split3) {
                    l2p_str[l2p_idx++] = split3;
                    split3 = sal_strtok_r(NULL,"_", &rest);
                }
            ...
            if (sal_strstr(line, dic_pbmp_xe)) {
                ...
                sal_strcpy(line_cpy2, port_str[1]);
                split2 = sal_strtok_r(line_cpy2,"x", &rest);
                port_data_idx=0;
                while (split2) {
                    port_data[port_data_idx++] = split2;
                    split2 = sal_strtok_r(NULL,"x", &rest);
                }
