        ...
        if (sal_strstr(line, "#") != line) {
            ...
            if(len > 1) {
                ...
                if (sal_strstr(line, cfg_key_str) == line) {
                    ...
                    cdmac_config_str = sal_strtok_r(line, cfg_key_str, &rest);
                    /*cli_out("CDMAC_CONFIG is %s", cdmac_config_str);*/
                    sal_strtok_r(cdmac_config_str, "\n", &rest);
    ...
    char *flex_entry_str)
{
   ...
   /* get the first token */
   token = sal_strtok_r(flex_entry_str, "_", &rest);
   i = 0;
   is_2nd_mac = 0;
   while( token != NULL ) {
      ...
      token = sal_strtok_r(NULL, "_", &rest);
      i++;
   }
   if (i > 3) { is_2nd_mac = 1;}
   /*cli_out( "%s %s\n", pm_num, cdmac_num);
   ...
   token = sal_strtok_r(pm_cd, "PM", &rest);
   pm_num = sal_atoi(token);
   token = sal_strtok_r(cd, "CD", &rest);
   base_subport[0] = 4 * sal_atoi(token);
   base_subport[1] = 0;
   if(is_2nd_mac == 1) {
       token = sal_strtok_r(cd_1, "CD", &rest);
        ...
        if (sal_strstr(line, "#") != line) {
            ...
            if(len > 1) {
                if ((input_mode == 3) &&
                    ...
                    line_flex_str = sal_strtok_r(line, sec_key_str, &rest);
                    sal_strtok_r(line_flex_str, "\n", &rest);
                    ...
                    (found_section == 1) ) {
                    line_flex_str = sal_strtok_r(line, flex_key_str, &rest);
                    sal_strtok_r(line_flex_str, "\n", &rest);
