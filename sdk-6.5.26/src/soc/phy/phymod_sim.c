...
#include <phymod/chip/tscbh_fe_gen2_sim.h>
#include <soc/phy/phymod_sim.h>

#ifndef PHY_NUM_SIM_ENTRIES
#define PHY_NUM_SIM_ENTRIES     600
#endif
...
phymod_bus_t sim_bus_per_unit[SOC_MAX_NUM_DEVICES] = { { NULL } };

int sim_bus_driver_init(int unit)
{
    /**
     * Initialize the driver per unit only once
     * Regard sim_bus driver as initialized if its read and write functions are assigned
     */
    if (unit < SOC_MAX_NUM_DEVICES)
    {
        if (sim_bus_per_unit[unit].read == NULL && sim_bus_per_unit[unit].write == NULL)
        {
            sim_bus_per_unit[unit].bus_name = "portmod_phy_sim";
            sim_bus_per_unit[unit].read = sim_bus_read;
            sim_bus_per_unit[unit].write = sim_bus_write;
            sim_bus_per_unit[unit].is_write_disabled = NULL;
            sim_bus_per_unit[unit].mutex_take = NULL;
            sim_bus_per_unit[unit].mutex_give = NULL;
            sim_bus_per_unit[unit].mem_read = NULL;
            sim_bus_per_unit[unit].mem_write = NULL;
            sim_bus_per_unit[unit].bus_capabilities = 0;
        }
    }
    else
    {
        LOG_ERROR(BSL_LS_SOC_PHYSIM,
                                 (BSL_META_U(unit,
                                             "Simulator supports up to %d devices\n"), SOC_MAX_NUM_DEVICES));
        return SOC_E_UNIT;
    }
...
soc_physim_check_sim(int unit, phymod_dispatch_type_t type, phymod_access_t* access, uint32 addr_ident, int* is_sim)
{
    ...
    if(use_sim || SAL_BOOT_PLISIM) {
        SOC_IF_ERROR_RETURN(sim_bus_driver_init(unit));
        ...
        switch(type) {
            ...
            case phymodDispatchTypeBlackhawk7_v1l8p1:
                ...
                break;
#endif
#if !defined (EXCLUDE_PHYMOD_TSCBH_FE_GEN2_SIM) && defined (PHYMOD_TSCBH_FE_GEN2_SUPPORT)
            case phymodDispatchTypeTscbh_fe_gen2:
                sim = &tscbh_fe_gen2_sim_drv;
                break;
#endif
            default:
                LOG_ERROR(BSL_LS_SOC_PHYSIM,
        ...
        PHYMOD_ACC_BUS(access) = &sim_bus_per_unit[unit];
