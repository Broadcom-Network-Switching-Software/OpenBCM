...
typedef struct {
    ...
    int serdes_10g_at_25g_vco;      /* 10g speed at 25G vco*/
...
tscf_show_serdes_info(phy_ctrl_t *pc, tscf_dev_info_t *pInfo, phymod_core_info_t *core_info)
{
    ...
    if (core_info->core_version == phymodCoreVersionTscf16Gen3) {
        sal_strcpy(pInfo->name,"TSCF16_GEN3-");
    } else {
 ...
 */
STATIC int
tscf_config_init(int unit, soc_port_t port, phymod_phy_access_t* phy, int logical_lane_offset,
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    */
    ...
    SOC_IF_ERROR_RETURN(phymod_phy_media_type_tx_get(phy, phymodMediaTypeChipToChip, &phymod_tx));

    /* set the default tx parameters */
    p_tx = &pCfg->tx_drive[TXDRV_DFT_INX];
    if (SOC_IS_FIRELIGHT(unit)) {
        sal_memcpy(p_tx, &phymod_tx, sizeof(phymod_tx_t));
    } else {
        p_tx->amp = 0x8;
        p_tx->pre = 0x0;
        p_tx->main = 0x64;
    ...
    pCfg->serdes_10g_at_25g_vco = 0;
    if (SOC_IS_FIRELIGHT(unit)) {
        pCfg->serdes_10g_at_25g_vco = soc_property_port_get(unit, port,
                                            spn_SERDES_10G_AT_25G_VCO,
                                            pCfg->serdes_1000x_at_25g_vco);
    }

    if (pCfg->serdes_10g_at_25g_vco) {
        core_init_config->interface.pll_divider_req = 165;
    }
...
_phy_tscf_init_pass2(int unit, soc_port_t port)
{
    ...
    for (idx = 0; idx < pmc->num_phys; idx++) {
        ...
        SOC_IF_ERROR_RETURN
            (tscf_config_init(unit, port, &phy->pm_phy,
                              logical_lane_offset,
    ...
    /*next check if we need to run speed at 25G VCO */
    if (pCfg->serdes_1000x_at_25g_vco || pCfg->serdes_10g_at_25g_vco) {
    ...
    *link = 0;
    ...
    if (pCfg->link_recovery.enabled) {
        ...
        if ((pm_phy->access.lane_mask == 0xf) && ((pCfg->speed_config.fiber_pref) ||
            (pCfg->speed_config.line_interface == TSCF_IF_CAUI) ||
            (pCfg->speed_config.line_interface == TSCF_IF_CAUI4))) {
            /* get CL91 */
            PHYMOD_FEC_CL91_SET(fec_enable);
...
STATIC int
tscf_power_get(soc_phymod_ctrl_t *pmc, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    ...
    SOC_IF_ERROR_RETURN(phymod_phy_power_get(pm_phy, &power));
...
tscf_autoneg_oui_mode_set(soc_phymod_ctrl_t *pmc, uint32 value)
{
    ...
    pm_phy = &pmc->phy[0]->pm_phy;
    sal_memset(&an_oui, 0, sizeof(phymod_autoneg_oui_t));
...
STATIC int
tscf_fec_cl91_bit_error_counter_get(soc_phymod_ctrl_t *pmc, uint32 *value)
{
   phymod_phy_access_t  *pm_phy;
    ...
    SOC_IF_ERROR_RETURN(phymod_phy_fec_error_bits_counter_get(pm_phy, value));

    return (SOC_E_NONE);
}

STATIC int
tscf_rx_dc_offset_get(soc_phymod_ctrl_t *pmc, uint32 *value)
{
   phymod_phy_access_t  *pm_phy;
    ...
    SOC_IF_ERROR_RETURN(phymod_phy_rx_dc_offset_get(pm_phy, value));
...
phy_tscf_control_get(int unit, soc_port_t port, soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_RS_FEC_BIT_ERROR_COUNT:
        rv = tscf_fec_cl91_bit_error_counter_get(pmc, value);
        break;
    case SOC_PHY_CONTROL_RX_DC_OFFSET:
        rv = tscf_rx_dc_offset_get(pmc, value);
        break;
    ...
    case SOC_PHY_CONTROL_FEC_SYMBOL_ERROR_COUNT:
        ...
        break;
    case SOC_PHY_CONTROL_POWER:
        /* power types like _SHR_PORT_PHY_CONTROL_POWER_LOW doesn't apply to SerDes, limit this for UM usage only */
        if (SOC_IS_FIRELIGHT(unit)) {
            rv = tscf_power_get(pmc, value);
        } else {
            rv = SOC_E_UNAVAIL;
        }
        break;
...
phy_tscf_probe(int unit, phy_ctrl_t *pc)
{
    ...
    if (SOC_IS_FIRELIGHT(unit)) {
        ...
        pmc->symbols = &bcmi_tscf16_gen3_xgxs_symbols;
#else
    ...
    if (SOC_IS_FIRELIGHT(unit)) {
#ifdef PHYMOD_TSCF16_GEN3_SUPPORT
        phy_type = phymodDispatchTypeTscf16_gen3;
#else
...
STATIC int
phy_tscf_timesync_config_set(int unit, soc_port_t port,
...
STATIC int
phy_tscf_timesync_config_get(int unit, soc_port_t port,
                             soc_port_phy_timesync_config_t *conf)
{
    phy_ctrl_t* pc;
    soc_phymod_ctrl_t *pmc;
...
STATIC int
phy_tscf_timesync_control_set(int unit, soc_port_t port,
                              ...
                              uint64 value)
{
    ...
    phymod_core_access_t *pm_core;
    phymod_phy_access_t *pm_phy;
    phymod_timesync_adjust_config_info_t timesync_config;
    ...
    switch (type) {
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_TIMESTAMP_ADJUST:
            SOC_IF_ERROR_RETURN
                (phymod_timesync_adjust_config_info_t_init(&timesync_config));
            timesync_config.am_norm_mode = COMPILER_64_LO(value);
            pm_phy = &pmc->phy[0]->pm_phy;
            SOC_IF_ERROR_RETURN
                (phymod_timesync_adjust_set(pm_phy, 0, &timesync_config));
            break;
        default:
            return SOC_E_UNAVAIL;
            break;
...
STATIC int
phy_tscf_timesync_tx_info_get(int unit, soc_port_t port, soc_port_timesync_tx_info_t *info)
{
    phy_ctrl_t* pc;
    soc_phymod_ctrl_t *pmc;
    phymod_phy_access_t *pm_phy;
    phymod_ts_fifo_status_t tx_info;

    sal_memset(&tx_info, 0, sizeof(phymod_ts_fifo_status_t));

    pc = INT_PHY_SW_STATE(unit, port);
    pmc = &pc->phymod_ctrl;

    /* 1588 two-step tx info get*/
    pm_phy = &pmc->phy[0]->pm_phy;
    SOC_IF_ERROR_RETURN
        (phymod_timesync_tx_info_get(pm_phy, &tx_info));

    info->timestamps_in_fifo    = tx_info.ts_in_fifo_lo;
    info->timestamps_in_fifo_hi = tx_info.ts_in_fifo_hi;
    info->sequence_id           = tx_info.ts_seq_id;
    info->sub_ns                = tx_info.ts_sub_nanosec;

    return SOC_E_NONE;
}

STATIC int
phy_tscf_diag_stats_get(int unit, soc_port_t port, soc_phy_diag_stats_t *err_stat)
{
    phy_ctrl_t* pc;
    soc_phymod_ctrl_t *pmc;
    phymod_phy_access_t *pm_phy;
    phymod_phy_stat_t phy_stat;

    sal_memset(&phy_stat, 0, sizeof(phymod_phy_stat_t));
    ...
    pm_phy = &pmc->phy[0]->pm_phy;
    SOC_IF_ERROR_RETURN
        (phymod_phy_stat_get(pm_phy, &phy_stat));

    sal_memcpy(err_stat, &phy_stat, sizeof(soc_phy_diag_stats_t));

    return SOC_E_NONE;
}

STATIC int
phy_tscf_signal_detect_status_get(int unit, soc_port_t port, soc_phy_signal_detect_status_t *sd_status)
{
    phy_ctrl_t* pc;
    soc_phymod_ctrl_t *pmc;
    phymod_phy_access_t *pm_phy;
    phymod_signal_detect_status_t sd_st;

    sal_memset(&sd_st, 0, sizeof(phymod_signal_detect_status_t));
    ...
    pm_phy = &pmc->phy[0]->pm_phy;
    SOC_IF_ERROR_RETURN
        (phymod_phy_signal_detect_status_get(pm_phy, &sd_st));
    sd_status->signal_detect_bmp = sd_st.signal_detect_bmp;
    sd_status->signal_detect_change_bmp = sd_st.signal_detect_change_bmp;
...
phy_driver_t phy_tscf_drv = {
    /* .drv_name                      = */ "TSC/Falcon PHYMOD PHY Driver",
    ...
    /* .pd_timesync_config_set        = */ phy_tscf_timesync_config_set,
    /* .pd_timesync_config_get        = */ phy_tscf_timesync_config_get,
    /* .pd_timesync_control_set       = */ phy_tscf_timesync_control_set,
    ...
    /* .pd_timesync_tx_info_get            = */ phy_tscf_timesync_tx_info_get,
    /* .pd_diag_stats_get                  = */ phy_tscf_diag_stats_get,
    /* .pd_pause_addr_set                  = */ NULL,
    /* .pd_pause_addr_get                  = */ NULL,
    /* .pd_phy_signal_detect_status_get    = */ phy_tscf_signal_detect_status_get,
};
