...
control_handler_scrambler_set(int unit, phymod_phy_access_t *phy, uint32 value1, uint32 ref_clk)
{
    ...
    if (value1 == SOC_PHY_CONTROL_SCRAMBLE_OFF) {
        PHYMOD_INTF_MODES_SCR_CLR(&config);
        flags = PHYMOD_INTF_F_SCRAMBLE_FORCED_OFF;
    } else if (value1 == SOC_PHY_CONTROL_SCRAMBLE_ON) {
        PHYMOD_INTF_MODES_SCR_SET(&config);
        flags = PHYMOD_INTF_F_SCRAMBLE_FORCED_ON;
    } else if (value1 == SOC_PHY_CONTROL_SCRAMBLE_DEFAULT) {
        PHYMOD_INTF_MODES_SCR_CLR(&config);
        flags =  0;
    } else {
        return SOC_E_PARAM;
    ...
    flags |= PHYMOD_INTF_F_DONT_OVERIDE_TX_PARAMS;
    SOC_IF_ERROR_RETURN(phymod_phy_interface_config_set(phy, flags, &config));
...
STATIC int
control_handler_rs_fec_bit_error_counter_get(int unit, phymod_phy_access_t *phy, uint32 param, uint32 *value)
{
    SOC_IF_ERROR_RETURN(phymod_phy_fec_error_bits_counter_get(phy, value));
...
STATIC int
control_handler_rx_dc_offset_get(int unit,
                                 phymod_phy_access_t *phy,
                                 uint32_t param,
                                 uint32_t *value)
{
    SOC_IF_ERROR_RETURN(phymod_phy_rx_dc_offset_get(phy, value));
    return SOC_E_NONE;
}
...
int soc_port_control_get_wrapper(int unit, phymod_ref_clk_t ref_clock, int is_lane_control, phymod_phy_access_t *phys, int nof_phys, soc_phy_control_t control, uint32 *value)
{
    ...
    *value = 0;
    ...
    case SOC_PHY_CONTROL_RS_FEC_BIT_ERROR_COUNT:
        handler = control_handler_rs_fec_bit_error_counter_get;
        break;
    case SOC_PHY_CONTROL_PARALLEL_DETECTION:
    case SOC_PHY_CONTROL_PARALLEL_DETECTION_10G:
    ...
    case SOC_PHY_CONTROL_RX_DC_OFFSET:
        handler = control_handler_rx_dc_offset_get;
        lane_control_support = TRUE;
        break;
    default:
        return SOC_E_UNAVAIL;
