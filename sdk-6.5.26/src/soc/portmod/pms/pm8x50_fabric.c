 ...
 * Output: fmac_blk_id
 */
#define PM8X50_FABRIC_LANE_FMAC_BLOCK_GET(pm_info, lane_index_in_pm, fmac_blk_id) \
...
int pm8x50_fabric_fmac_blk_ids_get(int unit, int port, pm_info_t pm_info, uint32* fmac_blk_ids, int* nof_fmac_blks, int* first_fmac_index)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
...
{   
    ...
    _SOC_IF_ERR_EXIT(phymod_firmware_load_method_t_validate(pm_add_info->pm_specific_info.pm8x50_fabric.fw_load_method));
    ...
    /*init wb buffer*/
    _SOC_IF_ERR_EXIT(pm8x50_fabric_wb_buffer_init(unit, wb_buffer_index,  pm_info));
    ...
    else 
    {
        ...
        if (is_core_probed)
        {
            ...
            _SOC_IF_ERR_EXIT(portmod_common_serdes_probe(pm8x50_fabric_serdes_list, &core_access, &is_phymod_probed));
            if (!is_phymod_probed)
            {
...
int pm8x50_fabric_pm_lane_mapping_changed_get(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info, int* lane_mapping_changed)
{
    ...
    *lane_mapping_changed = 0;
    ...
    {
        _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL, ("add_info->init_config.lane_map[0].num_of_lanes != PM8X50_FABRIC_LANES_PER_CORE"));
    }

    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_core_access_get(unit, port, pm_info, 0, 1, &core_access, &cores_num, NULL));
...
int pm8x50_fabric_pm_fmac_tx_lane_mapping_set(int unit, int port, pm_info_t pm_info, const phymod_lane_map_t *lane_map)
{
    ...
    PM8X50_FABRIC_PORT_BITMAP_ITER(all_lanes_in_pm_bitmap, lane_index) {
        ...
        _SOC_IF_ERR_EXIT(soc_reg32_get(unit, tx_lane_map_regs[serdes_index_in_fmac], fmac_blk_id, 0, &reg_val));
        ...
        _SOC_IF_ERR_EXIT(soc_reg32_set(unit, tx_lane_map_regs[serdes_index_in_fmac], fmac_blk_id, 0, reg_val));
    }
...
int pm8x50_fabric_pm_fmac_tx_lane_mapping_get(int unit, int port, pm_info_t pm_info, phymod_lane_map_t *lane_map)
{
    ...
    PM8X50_FABRIC_PORT_BITMAP_ITER(all_lanes_in_pm_bitmap, serdes_index) {
        ...
        /* register index is serdes id, register value is logical (FMAC) lane */
        _SOC_IF_ERR_EXIT(soc_reg32_get(unit, tx_lane_map_regs[lane_index_in_fmac], fmac_blk_id, 0, &reg_val));
...
int pm8x50_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if(!PORTMOD_PORT_ADD_F_INIT_PASS2_GET(add_info)) {
        ...
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_core_access_get(unit, port, pm_info, 0, 1, &core_access, &cores_num, NULL));
         ...
         _SOC_IF_ERR_EXIT(portmod_common_serdes_probe(pm8x50_fabric_serdes_list, &core_access, &is_probed));
         if (!is_probed)
         {
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_pm_fmac_tx_lane_mapping_set(unit, port, pm_info, add_info->init_config.lane_map));
    ...
    if (add_info->init_config.lane_map[0].num_of_lanes != PM8X50_FABRIC_LANES_PER_CORE)
    {
        _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL, ("add_info->lane_map.num_of_lanes != PM8X50_FABRIC_LANES_PER_CORE"));
    }
    for(i = 0; i < add_info->init_config.lane_map[0].num_of_lanes; ++i)
    {
    ...
    if (SOC_IS_RELOADING(unit))
    {
                ...
                (!PORTMOD_PORT_ADD_F_INIT_PASS1_GET(add_info) && !PORTMOD_PORT_ADD_F_INIT_PASS2_GET(add_info)))
        {
            ...
            params.phyn = 0;
            _SOC_IF_ERR_EXIT(pm8x50_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1, &phy_access, &nof_phys, NULL));
    ...
    {
        _SOC_IF_ERR_EXIT(phymod_core_init(&core_access, &core_config, &status));
    }
...
int pm8x50_fabric_core_init_phase2(int unit, int port, pm_info_t pm_info)
{
    ...
    {
        _SOC_IF_ERR_EXIT(READ_FSRD_SRD_PLL_1_CTRLr(unit, fsrd_schan_id, &reg_val));
        soc_reg_field_set(unit, FSRD_SRD_PLL_1_CTRLr, &reg_val, SRD_N_PMD_CORE_PLL_1_DP_H_RSTBf, 1);
        _SOC_IF_ERR_EXIT(WRITE_FSRD_SRD_PLL_1_CTRLr(unit, fsrd_schan_id, reg_val));
    }
    _SOC_IF_ERR_EXIT(READ_FSRD_SRD_CTRLr(unit, fsrd_schan_id, reg_above64_val));
...
int pm8x50_fabric_port_disable_init(int unit, int port, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_fmac_blk_ids_get(unit, port, pm_info, fmac_blk_ids, &nof_fmac_blks, &first_fmac_index));
    ...
    for (i = 0 ; i < sizeof(reset_fields)/sizeof(*reset_fields); ++i) {
        PM8X50_FABRIC_PORT_FMACS_ITER(fmac_blk_ids, nof_fmac_blks, first_fmac_index, fmac_index) {
            ...
            *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, reset_fields[i]);
            _SOC_IF_ERR_EXIT(pm8x50_fabric_port_fmac_lanes_bitwrite(unit, port, bitmap, fmac_index, field_val, 1));
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_squelch_set(unit, port, pm_info, flags, 1));
    ...
    int *rx_clk_rptr_dis_req)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_core_access_get(unit, 0, pm_info, 0, 1, &core_access, &cores_num, NULL));
    ...
    uint8 pll_lanes_bitmap)
{
    ...
    _SOC_IF_ERR_EXIT(_pm8x50_fabric_port_clock_repeater_buffer_check(unit, pm_info, pll_index,
...
int pm8x50_fabric_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if(!is_core_probed || (is_core_probed == 1 && PORTMOD_PORT_ADD_F_INIT_PASS2_GET(add_info))){ 
        _SOC_IF_ERR_EXIT(pm8x50_fabric_core_init_phase1(unit, port, pm_info, add_info));
    }
    ...
    {
        _SOC_IF_ERR_EXIT(pm8x50_fabric_pm_lane_mapping_changed_get(unit, port, pm_info, add_info, &lane_mapping_changed));
    ...
    if(!is_core_initialized) {
        _SOC_IF_ERR_EXIT(pm8x50_fabric_core_init_phase2(unit, port, pm_info));
    }
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_power_set(unit, port, pm_info, 1));
   ...
   /* in order to avoid assuming which indexes of init_config.tx phymod will use for the lanes of the port - fill all array indexes */
    for (lane_index = 0; lane_index < PHYMOD_MAX_LANES_PER_CORE; ++lane_index) {
        ...
        _SOC_IF_ERR_EXIT(phymod_phy_tx_taps_default_get(&phy_access, phymodSignallingMethodNRZ, &init_config.tx[lane_index]));
    }
    ...
    if (is_bypass)
    {
        ...
        if (PORTMOD_PBMP_MEMBER(fabric_data->phys, first_lane))
        {
            ...
            _SOC_IF_ERR_EXIT(READ_FMAC_RETIMERr(unit, fmac_blk_id, lane_index_in_fmac, reg_above_64_val));
            soc_reg_above_64_field32_set(unit, FMAC_RETIMERr, reg_above_64_val, FMAL_N_TX_ILKN_MASTER_READ_ENf, 1);
            _SOC_IF_ERR_EXIT(WRITE_FMAC_RETIMERr(unit, fmac_blk_id, lane_index_in_fmac, reg_above_64_val));
        }
    ...
    /*disable phy and mac*/  
    /*_SOC_IF_ERR_EXIT(pm8x50_fabric_port_enable_set(unit, port, pm_info, flags, 0));*/
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_disable_init(unit, port, pm_info));
...
int pm8x50_fabric_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_power_set(unit, port, pm_info, 0));
    ...
    PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        _SOC_IF_ERR_EXIT(PM8X50_FABRIC_PLL_ACTIVE_LANES_BITMAP_GET(unit, pm_info, &pll_active_lanes_bitmap[pll_index], pll_index));
        _SOC_IF_ERR_EXIT(PM8X50_FABRIC_PLL_ACTUAL_LANES_BITMAP_GET(unit, pm_info, &pll_actual_lanes_bitmap[pll_index], pll_index));
        _SOC_IF_ERR_EXIT(PM8X50_FABRIC_PLL_VCO_RATE_GET(unit, pm_info, &pll_vco_rate[pll_index], pll_index));
        ...
        if (SHR_BITGET(&pll_active_lanes_bitmap[pll_index], phy_index))
        {
            ...
            if (PM8X50_FABRIC_PLL_IS_EMPTY(pll_active_lanes_bitmap[pll_index]))
            {
                ...
                _SOC_IF_ERR_EXIT(phymod_phy_pll_pwrdn(&phy_access, pll_index, 1));
            }

            _SOC_IF_ERR_EXIT(PM8X50_FABRIC_PLL_ACTIVE_LANES_BITMAP_SET(unit, pm_info, pll_active_lanes_bitmap[pll_index], pll_index));
            _SOC_IF_ERR_EXIT(PM8X50_FABRIC_PLL_ACTUAL_LANES_BITMAP_SET(unit, pm_info, pll_actual_lanes_bitmap[pll_index], pll_index));
            _SOC_IF_ERR_EXIT(PM8X50_FABRIC_PLL_VCO_RATE_SET(unit, pm_info, pll_vco_rate[pll_index], pll_index));
    ...
    if (fabric_data->clock_buffer_disable_required) {
        PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index) {
            _SOC_IF_ERR_EXIT(_pm8x50_fabric_pll_clock_repeater_buffer_pwrdn(unit, pm_info,
...
soc_error_t pm8x50_fabric_fmac_enable_set(int unit, int port, pm_info_t pm_info, int rx_tx, int enable)
{
    ...
    PM8X50_FABRIC_PORT_FMACS_ITER(fmac_blk_ids, nof_fmac_blks, first_fmac_index, fmac_index) {
        ...
        *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, field);
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_fmac_lanes_bitwrite(unit, port, bitmap, fmac_index, field_val, (enable? 0 : 1)));
...
int pm8x50_fabric_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
             ...
             PORTMOD_PORT_ENABLE_RX_GET(flags_mask) && PORTMOD_PORT_ENABLE_TX_GET(flags_mask))
    {
        ...
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_enable_get(unit, port, pm_info, flags, &cur_enable)); 
        ...
        if(enable){
            _SOC_IF_ERR_EXIT(pm8x50_fabric_squelch_set(unit, port, pm_info, flags_mask, 0));

            /* Enable FMAC Tx*/
            _SOC_IF_ERR_EXIT(pm8x50_fabric_fmac_enable_set(unit, port, pm_info, PM8X50_FABRIC_FMAC_TX, enable));

            /* Enable FMAC Rx*/
            _SOC_IF_ERR_EXIT(pm8x50_fabric_fmac_enable_set(unit, port, pm_info, PM8X50_FABRIC_FMAC_RX, enable));
        ...
        else
        {
            ...
            _SOC_IF_ERR_EXIT(pm8x50_fabric_squelch_set(unit, port, pm_info, flags_mask, 1));
        }
    }
    ...
    {
        if (!is_bypass){
            _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL , ("Fabric doesn't support only Tx MAC option. (Only ILKN over fabric supports that mode).\n"));
        }
        _SOC_IF_ERR_EXIT(pm8x50_fabric_fmac_enable_set(unit, port, pm_info, PM8X50_FABRIC_FMAC_TX, enable));
    }
    else if (!PORTMOD_PORT_ENABLE_MAC_GET(flags_mask) && PORTMOD_PORT_ENABLE_PHY_GET(flags_mask) &&
    ...
    {
        if (!is_bypass){
            _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL , ("Fabric doesn't support only Phy option. (Only ILKN over fabric supports that mode).\n"));
        }
            ...
            !PORTMOD_PORT_ENABLE_RX_GET(flags_mask) && PORTMOD_PORT_ENABLE_TX_GET(flags_mask))
    {
        if (is_bypass)
        {
            _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL , ("ILKN over fabric doesn't support Tx PHY option. (Only fabric supports that mode).\n"));
        }
        if (enable)
        {
            _SOC_IF_ERR_EXIT(pm8x50_fabric_squelch_set(unit, port, pm_info, flags_mask, 0));
...
pm8x50_fabric_port_cdr_lock_get(int unit, int port, pm_info_t pm_info, int* is_locked)
{
    ...
    _SOC_IF_ERR_EXIT(READ_FSRD_SRD_STATUSr(unit, fsrd_schan_id, &reg32_val));
...
int pm8x50_fabric_port_fec_sync_machine_config(int unit, int port, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_encoding_get(unit, port, pm_info, &properties, &pcs));
    if (!PM8X50_FABRIC_ENCODING_IS_RS_FEC(pcs)) {
        SOC_EXIT;
    }

    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_get(unit, port, pm_info, &speed_config));
...
int pm8x50_fabric_port_encoding_set(int unit, int port, pm_info_t pm_info, uint32 properties, portmod_port_pcs_t encoding)
{
    ...
    if(!PORTMOD_ENCODING_LOW_LATENCY_LLFC_GET(properties) && PM8X50_FABRIC_ENCODING_IS_RS_FEC(encoding)){
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, ("RS_FEC pcs: low latency llfc is always enabled"));
    }
    ...
    if(PORTMOD_ENCODING_EXTRCT_CIG_FROM_LLFC_GET(properties) && (encoding != PORTMOD_PCS_64B66B_FEC)){
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, ("extract CIG indications from LLFC cells is supported just for 64/66 kr fec"));
    }
    ...
    flags = 0;
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_enable_get(unit, port, pm_info, flags, &enabled));
    /*disable phy and mac*/
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_enable_set(unit, port, pm_info, flags, 0));
    ...
    PM8X50_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index) {
        PM8X50_FABRIC_LANE_FMAC_DATA_GET(pm_info, lane_index, fmac_blk_id, lane_index_in_fmac);
        _SOC_IF_ERR_EXIT(READ_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, fmac_blk_id, lane_index_in_fmac, &reg_val));
        ...
        _SOC_IF_ERR_EXIT(WRITE_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, fmac_blk_id, lane_index_in_fmac, reg_val));
    }

    PM8X50_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index) {
        PM8X50_FABRIC_LANE_FMAC_DATA_GET(pm_info, lane_index, fmac_blk_id, lane_index_in_fmac);
        _SOC_IF_ERR_EXIT(READ_FMAC_FPS_TX_CONFIGURATIONr(unit, fmac_blk_id, lane_index_in_fmac, &reg_val));
        soc_reg_field_set(unit, FMAC_FPS_TX_CONFIGURATIONr, &reg_val, FPS_N_TX_FEC_ENf, fec_enable);
        _SOC_IF_ERR_EXIT(WRITE_FMAC_FPS_TX_CONFIGURATIONr(unit, fmac_blk_id, lane_index_in_fmac, reg_val));
    }
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_fec_sync_machine_config(unit, port, pm_info));

    /*restore MAC and PHY */
    flags = 0;
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_enable_set(unit, port, pm_info, flags, enabled));
...
_pm8x50_fabric_loopback_serdes_overrides_set(int unit, int port, pm_info_t pm_info)
{
    ...
    for (i = 0; i < sizeof(mac_loopbacks)/sizeof(mac_loopbacks[0]); i++) {
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_loopback_get(unit, port, pm_info, mac_loopbacks[i], &lb_enable));
        if (lb_enable) {
...
int pm8x50_fabric_mac_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    case portmodLoopbackMacRloop:
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_retimer_set(unit, port, pm_info, TRUE, enable));
        break;
    default:
...
int pm8x50_fabric_port_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    case portmodLoopbackMacRloop:
        break;
    case portmodLoopbackMacPCS:
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_encoding_get(unit, port, pm_info, &encoding_properties ,&encoding));
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_loopback_get(unit, port, pm_info, loopback_type, &lb_enable));
    if(enable == lb_enable){
        SOC_EXIT;
    }

    /*loopback set*/
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_are_lanes_swapped(unit, port, pm_info, &lanes_are_swapped));
    if(is_phy_loopback){
        ...
        _SOC_IF_ERR_EXIT(pm8x50_fabric_mac_loopback_set(unit, port, pm_info, loopback_type, enable));
    }
    ...
    _SOC_IF_ERR_EXIT(rv);
    if(!is_bypass) {
        _SOC_IF_ERR_EXIT(_pm8x50_fabric_loopback_serdes_overrides_set(unit, port, pm_info));
    }
...
int pm8x50_fabric_port_loopback_get(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int* enable)
{
    ...
    case portmodLoopbackMacPCS:
        ...
        *enable = soc_reg_field_get(unit, FMAC_KPCS_CONFIGURATIONr, reg_val, KPCS_N_RX_DSC_LOOPBACK_ENf);
        break;
    case portmodLoopbackMacRloop:
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_retimer_get(unit, port, pm_info, &is_remote_loopback, &is_enabled));
...
pm8x50_fabric_port_retimer_set(int unit, soc_port_t port, pm_info_t pm_info, int is_remote_loopback, int enable)
{
    ...
    if(nof_fmac_blks > 1) {
        _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL, ("More than one FMAC per fabric port %d", port));
    }

    /* DC auto adjust settings */
    _SOC_IF_ERR_EXIT(READ_FMAC_FMAL_TX_DC_AUTO_ADJUSTr(unit, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg64_val));
    ...
    _SOC_IF_ERR_EXIT(WRITE_FMAC_FMAL_TX_DC_AUTO_ADJUSTr(unit, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg64_val));

    /* Set allowed fullness level of ILKN FIFO */
    /* Fmal[n]TxIlknFifoThr */
    _SOC_IF_ERR_EXIT(READ_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg64_val));
    ...
    _SOC_IF_ERR_EXIT(WRITE_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg64_val));
    ...
    _SOC_IF_ERR_EXIT(READ_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg64_val));
    soc_reg64_field32_set(unit, FMAC_ASYNC_FIFO_CONFIGURATIONr, &reg64_val, FMAL_N_TX_WIDTHf, fabric_mac_mode);
    _SOC_IF_ERR_EXIT(WRITE_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg64_val));

    /* Set the link as down in MAC leacky bucket */
    _SOC_IF_ERR_EXIT(soc_reg32_get(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg32_val));
    ...
    _SOC_IF_ERR_EXIT(soc_reg32_set(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg32_val));

    /* async FIFO configuration */
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_retimer_fec_enable_get(unit, port, pm_info, &retimer_fec_enable));
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_get(unit, port, pm_info, &speed_config));
    ...
    /* Enable retimer */
    _SOC_IF_ERR_EXIT(READ_FMAC_RETIMERr(unit, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg_above_64_val));
    ...
    _SOC_IF_ERR_EXIT(WRITE_FMAC_RETIMERr(unit, fmac_blk_ids[first_fmac_index], PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg_above_64_val));
...
pm8x50_fabric_port_pass_through_set(int unit, soc_port_t port, pm_info_t pm_info, int loopback_en, int enable)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_retimer_set(unit, port, pm_info, loopback_en, enable));
...
pm8x50_fabric_port_pass_through_get(int unit, soc_port_t port, pm_info_t pm_info, int *loopback_en, int *enable)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_retimer_get(unit, port, pm_info, loopback_en, enable));
...
pm8x50_fabric_port_phase_interpolator_rx_set(int unit, soc_port_t port, pm_info_t pm_info, int enable)
{
    ...
    _SOC_IF_ERR_EXIT(phymod_phy_tx_phase_lock_rx_set(&phy_access, enable));
...
pm8x50_fabric_port_phase_interpolator_tx_set(int unit, soc_port_t port, pm_info_t pm_info, int enable)
{
    ...
    _SOC_IF_ERR_EXIT(phymod_phy_tx_phase_lock_tx_set(&phy_access, enable));
...
pm8x50_fabric_port_retimer_fec_enable_set(int unit, int port, pm_info_t pm_info, int enable)
{
    ...
    if(nof_fmac_blks != 1) {
        _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL, ("invalid number of FMACs for port %d", port));
    }

    lane_index_in_fmac = PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index);
    _SOC_IF_ERR_EXIT(READ_FMAC_RETIMERr(unit, fmac_blk_ids[first_fmac_index], lane_index_in_fmac, reg_above_64_val));
    soc_reg_above_64_field32_set(unit, FMAC_RETIMERr, reg_above_64_val, FMAL_N_RETIMER_FEC_ENf, (enable) ? (1) : (0));
    _SOC_IF_ERR_EXIT(WRITE_FMAC_RETIMERr(unit, fmac_blk_ids[first_fmac_index], lane_index_in_fmac, reg_above_64_val));
...
pm8x50_fabric_port_pass_through_fec_enable_set(int unit, int port, pm_info_t pm_info, int enable)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_retimer_fec_enable_set(unit, port, pm_info, enable));
...
pm8x50_fabric_port_pass_through_fec_enable_get(int unit, int port, pm_info_t pm_info, int* enable)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_retimer_fec_enable_get(unit, port, pm_info, enable));
...
pm8x50_fabric_port_retimer_calibrate(int unit, int port, pm_info_t pm_info)
{
    ...
    /* Start auto-adjust */
    _SOC_IF_ERR_EXIT(READ_FMAC_FMAL_TX_DC_AUTO_ADJUSTr(unit, fmac_blk_ids[first_fmac_index], lane_index_in_fmac, &reg64_val));
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_DC_AUTO_ADJUSTr, &reg64_val, FMAL_03_TX_DC_AUTO_ADJUST_FIELD_7f, 0);
    _SOC_IF_ERR_EXIT(WRITE_FMAC_FMAL_TX_DC_AUTO_ADJUSTr(unit, fmac_blk_ids[first_fmac_index], lane_index_in_fmac, reg64_val));
    ...
    {
        _SOC_EXIT_WITH_ERR(SOC_E_FAIL, ("port %d: retimer calibration failed\n", port));
    }
...
pm8x50_fabric_port_pass_through_calibrate(int unit, int port, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_retimer_calibrate(unit, port, pm_info));
...
int pm8x50_fabric_port_link_get(int unit, int port, pm_info_t pm_info, int flag, int* link)
{
    ...
    *link = 0;
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_enable_get(unit, port, pm_info, 0, &enable));

    if (enable) {
        /* Check if there is a MAC LB set on the lane*/
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_loopback_get(unit, port, pm_info, portmodLoopbackMacAsyncFifo, &is_mac_lb_enabled));
    ...
    int op_cmd, const void *arg) 
{
    ...
    switch(op_cmd) {
        case PHY_DIAG_CTRL_DSC:
            ...
            _SOC_IF_ERR_EXIT(pm8x50_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1, 
...
int pm8x50_fabric_pm_speed_vco_get(int unit, int speed, int is_bypass, portmod_vco_type_t* required_vco)
{
    ...
    {
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_fabric_speed_to_vco_get(unit, speed, required_vco));
    }
exit:
    SOC_FUNC_RETURN;
}
...
static int pm8x50_fabric_pm_speed_config_validate_plls_get(int unit,pm_info_t pm_info, const portmod_pbmp_t* ports, portmod_pm_vco_setting_t* vco_setting, int begin_pll, int end_pll, portmod_vco_type_t* pll_vco_rate_curr, portmod_vco_type_t* pll_vco_rate_curr_updated)
{
    ...
    {
        _SOC_IF_ERR_EXIT(pm8x50_fabric_pm_speed_vco_get(unit, vco_setting->speed_config_list[speed_index].speed, vco_setting->speed_for_pcs_bypass_port[speed_index], &required_vco));
        ...
        if ((required_vco != pll_vco_rate_requested[0]) && (required_vco != pll_vco_rate_requested[1]))
        {
            ...
            if (pll_index < begin_pll)
            {
                ...
                _SOC_EXIT_WITH_ERR(SOC_E_PARAM, NULL);
            }
        }
    }
    ...
    PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        _SOC_IF_ERR_EXIT(PM8X50_FABRIC_PLL_VCO_RATE_GET(unit, pm_info, &pll_vco_rate_curr[pll_index], pll_index));
        _SOC_IF_ERR_EXIT(PM8X50_FABRIC_PLL_ACTIVE_LANES_BITMAP_GET(unit, pm_info, &pll_active_lanes_bitmap_curr[pll_index], pll_index));
...
int pm8x50_fabric_pm_speed_config_validate(int unit, int pm_id, pm_info_t pm_info, const portmod_pbmp_t* ports, int flag, portmod_pm_vco_setting_t* vco_setting)
{
     ...
     *
     */
    _SOC_IF_ERR_EXIT(pm8x50_fabric_pm_speed_config_validate_plls_get(unit, pm_info, ports, vco_setting, begin_pll, end_pll, pll_vco_rate_curr, pll_vco_rate_curr_updated));
            ...
            else
            {
                ...
                if ((pll_vco_rate_curr_updated[other_pll_index] != portmodVCOInvalid &&  pll_vco_rate_curr_updated[other_pll_index] != required_vco) || !other_pll_is_valid)
                {
                    _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                            ("Not enough free PLLs to complete the assignment! Number of allowed PLLs for relevant PM is %d", nof_allowed_plls));
                }
...
int pm8x50_fabric_pm_vco_reconfig(int unit, int pm_id, pm_info_t pm_info, const portmod_vco_type_t* vco)
{
    ...
    PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        ...
        if (vco_aligned[pll_index] != pll_vco_rate[pll_index])
        {
            ...
            _SOC_IF_ERR_EXIT(pm8x50_fabric_vco_to_pll_get(unit, fabric_data->ref_clk, vco_aligned[pll_index], &pll_div));
            ...
            if (!SOC_IS_RELOADING(unit))
            {
                if (pll_div != phymod_TSCBH_PLL_DIVNONE) {
                    _SOC_IF_ERR_EXIT(phymod_phy_pll_reconfig(&phy_access, pll_index, pll_div, fabric_data->ref_clk));
                } else {
                    _SOC_IF_ERR_EXIT(phymod_phy_pll_pwrdn(&phy_access, pll_index, 1));
                }
            }
...
int pm8x50_fabric_port_speed_config_set(int unit, int port, pm_info_t pm_info, const portmod_speed_config_t* speed_config)
{
    ...
    if (is_bypass)
    {
        ...
        if (required_vco == portmodVCOInvalid)
        {
            _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL, ("speed %d not supported", speed_config->speed));
        }
    }
    else
    {
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_fabric_speed_to_vco_get(unit, speed_config->speed, &required_vco));
    }
    PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
    ...
    if (pll_index < begin_pll)
    {
        _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL, ("port %d: the required VCO rate %d was not configured in pm8x50_fabric_pm_vco_reconfig", port, required_vco));
    }
    ...
    if (!SOC_IS_RELOADING(unit))
    {
        ...
        if (fabric_data->clock_buffer_disable_required) {
            ...
            int i, tmp_port;
            for (i = 0; i < PM8X50_FABRIC_MAX_PORTS_PER_PM; i++) {
                _SOC_IF_ERR_EXIT(PM8X50_FABRIC_LANE2PORT_GET(unit, pm_info, i, &tmp_port));
                if(tmp_port >= 0) {
    ...
    {
        _SOC_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_active_lanes_bitmap], &pll_active_lanes_bitmap[0], 0));
        _SOC_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_active_lanes_bitmap], &pll_active_lanes_bitmap[1], 1));
        _SOC_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_actual_lanes_bitmap], &pll_actual_lanes_bitmap[0], 0));
        _SOC_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_actual_lanes_bitmap], &pll_actual_lanes_bitmap[1], 1));
        PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
        {
            _SOC_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_vco_rate], &pll_vco_rate[pll_index], pll_index));
        }
    }
    else
    {
        _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL, ("port %d: didn't get expected PLL bitmaps from phymod actual (%x, %x), expected (%x, %x)",
    ...
    {
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_new_to_legacy_encoding_get(unit, port, pm_info, speed_config->fec, &legacy_encoding));
        ...
        _SOC_IF_ERR_EXIT(pm8x50_fabric_port_encoding_set(unit, port, pm_info, encoding_properties, legacy_encoding));
    }
...
int pm8x50_fabric_port_speed_config_get(int unit, int port, pm_info_t pm_info, portmod_speed_config_t* speed_config)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_encoding_get(unit, port, pm_info, &properties, &legacy_encoding));
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_legacy_to_new_encoding_get(unit, port, pm_info, legacy_encoding, &speed_config->fec));
...
int pm8x50_fabric_port_lane_map_set(int unit, int port, pm_info_t pm_info, uint32 flags, const phymod_lane_map_t* lane_map)
{
    ...
    if (mac_only == 0)
    {
        _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL, ("port %d: pm8x50_fabric supports MAC only lane map set", port));
    }

    _SOC_IF_ERR_EXIT(pm8x50_fabric_pm_fmac_tx_lane_mapping_set(unit, port, pm_info, lane_map));
...
int pm8x50_fabric_port_lane_map_get(int unit, int port, pm_info_t pm_info, uint32 flags,phymod_lane_map_t* lane_map)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_pm_fmac_tx_lane_mapping_get(unit, port, pm_info, lane_map));
...
int pm8x50_fabric_port_credits_enable_set(int unit, int port, pm_info_t pm_info, int enable)
{
    ...
    _SOC_IF_ERR_EXIT(READ_FSRD_TX_ILKN_CREDITr(unit, fsrd_schan_id, &reg_val));
    soc_reg_field_set(unit, FSRD_TX_ILKN_CREDITr, &reg_val, TX_ILKN_CREDIT_ENf, enable? 1 : 0);
    _SOC_IF_ERR_EXIT(WRITE_FSRD_TX_ILKN_CREDITr(unit, fsrd_schan_id, reg_val));
...
int pm8x50_fabric_port_rate_egress_ppt_set(int unit, int port, pm_info_t pm_info, uint32 burst,uint32 nof_tiks)
{
    ...
    _SOC_IF_ERR_EXIT(pm8x50_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
    PM8X50_FABRIC_LANE_FMAC_BLOCK_GET(pm_info, phy_index, fmac_index);

    /** disable shaper handle */
    if (burst == 0)
    {
        COMPILER_64_ZERO(reg64_val);
        _SOC_IF_ERR_EXIT(WRITE_FMAC_TX_CELL_LIMITr(unit, fmac_index, reg64_val));
    }
    else if (burst < 3)
    {
...
int pm8x50_fabric_port_rate_egress_ppt_get(int unit, int port, pm_info_t pm_info, uint32 * burst,uint32 * nof_tiks)
{
    ...
    _SOC_IF_ERR_EXIT(READ_FMAC_TX_CELL_LIMITr(unit, fmac_index, &reg64_val));
