...
int pm4x10_qtc_port_timesync_tx_info_get(int unit, int port, pm_info_t pm_info, portmod_fifo_status_t* tx_info)
{
    ...
    if (!IS_QMOD) {
        ...
        _SOC_IF_ERR_EXIT(unimac_diag_fifo_status_get(unit, port, tx_info));
    }
...
int pm4x10_qtc_port_fec_error_inject_set(int unit, int port, pm_info_t pm_info,
                                         ...
                                         portmod_fec_error_mask_t bit_error_mask)
{
    ...
    if (!IS_QMOD) {
        _SOC_IF_ERR_EXIT(
            PM_DRIVER(PM4x10_INFO(pm_info))->f_portmod_port_fec_error_inject_set(unit, port,
                                             PM4x10_INFO(pm_info), error_control_map, bit_error_mask));
    } else {
        return (SOC_E_UNAVAIL);
    }
...
int pm4x10_qtc_port_fec_error_inject_get(int unit, int port, pm_info_t pm_info,
                                         uint16* error_control_map,
                                         portmod_fec_error_mask_t* bit_error_mask)
{
    PM4x10_QTC_LOCAL_VARIABLE_DEFS;
    ...
    if (!IS_QMOD) {
        _SOC_IF_ERR_EXIT(
            PM_DRIVER(PM4x10_INFO(pm_info))->f_portmod_port_fec_error_inject_get(unit, port,
                                             PM4x10_INFO(pm_info), error_control_map, bit_error_mask));
    } else {
        return (SOC_E_UNAVAIL);
    }
...
int pm4x10_qtc_port_faults_status_get(int unit, int port, pm_info_t pm_info,
                                     portmod_port_fault_status_t* faults)
{
    ...
    if (!IS_QMOD) {
        _SOC_IF_ERR_EXIT(
            PM_DRIVER(PM4x10_INFO(pm_info))->f_portmod_port_faults_status_get(
                              unit, port, PM4x10_INFO(pm_info), faults));
    } else {
        return (SOC_E_UNAVAIL);
    }
