...
#define PM4x25_WB_BUFFER_VERSION        (16)
...
#define PORTMOD_PM4x25_IS_GEN3_5nm(pm_version)         \
                            ((pm_version.type == PM_TYPE_PM4X25) && \
                             (pm_version.gen == PM_GEN3) &&         \
                             (pm_version.tech_process == PM_TECH_5NM))
...
int portmod_pm4x25_wb_upgrade_func(int unit, void *arg, int recovered_version, int new_version)
{
    ...
    if ((recovered_version <= 15) &&
        (new_version >= 16)) {
        roe_frame_format = 0;
        for (i = 0; i < MAX_PORTS_PER_PM4X25; i++) {
            _SOC_IF_ERR_EXIT(
                PM4x25_CPRI_PORT_FRAME_FORMAT_SET(unit, pm_info, roe_frame_format, i));
        }
    }
...
int pm4x25_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
   ...
   _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id,
                          "cpri_port_roe_frame_format", wb_buffer_index,
                          sizeof(uint32_t), NULL, MAX_PORTS_PER_PM4X25,
                          VERSION(16));
    pm_info->wb_vars_ids[roePortFrameFormat] = wb_var_id;
    _SOC_IF_ERR_EXIT(rv);
    ...
    if(!SOC_WARM_BOOT(unit)){
       ...
       rv = SOC_WB_ENGINE_MEMSET_ARR(unit, SOC_WB_ENGINE_PORTMOD,
                              pm_info->wb_vars_ids[roePortFrameFormat], 0);
        _SOC_IF_ERR_EXIT(rv);
...
STATIC portmod_ucode_buf_t pm4x25_flr_ucode_buf[SOC_MAX_NUM_DEVICES] = {{NULL, 0}};
STATIC portmod_ucode_buf_t pm4x25_flr_ucode_buf_2nd[SOC_MAX_NUM_DEVICES] = {{NULL, 0}};
...
int pm4x25_pm_destroy(int unit, pm_info_t pm_info)
{
    ...
    if (pm_info->pm_data.pm4x25_db != NULL) {
        if (PM_4x25_INFO(pm_info)->int_core_access.access.pmd_info_ptr != NULL) {
            sal_free(PM_4x25_INFO(pm_info)->int_core_access.access.pmd_info_ptr);
            PM_4x25_INFO(pm_info)->int_core_access.access.pmd_info_ptr = NULL;
        }
    ...
    /* free pm4x25_flr_ucode_buf */
    if(pm4x25_flr_ucode_buf[unit].ucode_dma_buf != NULL){
        portmod_sys_dma_free(unit, pm4x25_flr_ucode_buf[unit].ucode_dma_buf);
        pm4x25_flr_ucode_buf[unit].ucode_dma_buf = NULL;
    }
    pm4x25_flr_ucode_buf[unit].ucode_alloc_size = 0;

    if(pm4x25_flr_ucode_buf_2nd[unit].ucode_dma_buf != NULL){
        portmod_sys_dma_free(unit, pm4x25_flr_ucode_buf_2nd[unit].ucode_dma_buf);
        pm4x25_flr_ucode_buf_2nd[unit].ucode_dma_buf = NULL;
    }
    pm4x25_flr_ucode_buf_2nd[unit].ucode_alloc_size = 0;
...
pm4x25_default_fw_loader(const phymod_core_access_t* core, uint32_t length, const uint8_t* data)
{
    ...
    rv = portmod_firmware_set(unit,
                              ...
                              load_order,
                              bcast_en,
                              flr_ucode ? &(pm4x25_flr_ucode_buf[unit]) : &(pm4x25_ucode_buf[unit]),
                              flr_ucode ? &(pm4x25_flr_ucode_buf_2nd[unit]) : &(pm4x25_ucode_buf_2nd[unit]),
                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
    ...
    /* No firmware support on 4x25d. */
#ifdef PORTMOD_PM4X25D_SUPPORT
    if (pm_info->type == portmodDispatchTypePm4x25d) {
        pm4x25_data->no_fw_support = 1;
    } else
#endif /* PORTMOD_PM4X25D_SUPPORT */
    {
        pm4x25_data->no_fw_support = 0;
    }
    ...
    if ((pm4x25_data->int_core_access.access.pmd_info_ptr == NULL) &&
        !(pm4x25_data->no_fw_support)) {
        pm4x25_data->int_core_access.access.pmd_info_ptr = sal_alloc(PMD_INFO_DATA_STRUCTURE_SIZE,
                             "PortMod PM PMD info");
        if (pm4x25_data->int_core_access.access.pmd_info_ptr == NULL) {
            return SOC_E_MEMORY;
        }
        sal_memset(pm4x25_data->int_core_access.access.pmd_info_ptr, 0, PMD_INFO_DATA_STRUCTURE_SIZE);
    }
    ...
    if(SOC_WARM_BOOT(unit)){
        ...
        /* Restore first phy number of the PM */
    ...
    if(!SOC_WARM_BOOT(unit)){
        ...
        if (pm_info->type == portmodDispatchTypeCpm4x25) {
            ...
            pll_vco_rate_cpm = -1;
            rv = PM4x25_PLL0_VCO_RATE_SET(unit, pm_info, pll_vco_rate_cpm);
            _SOC_IF_ERR_EXIT(rv);
            rv = PM4x25_PLL1_VCO_RATE_SET(unit, pm_info, pll_vco_rate_cpm);
            _SOC_IF_ERR_EXIT(rv);
        }
#endif
...
int pm4x25_port_detach(int unit, int port, pm_info_t pm_info)
{
...
#if defined(PHYMOD_TSCF16_SUPPORT) || defined(PHYMOD_TSCF16_GEN3_SUPPORT)
    /* Need to disable PCS in case of flexport issues */
    if (!PM_4x25_INFO(pm_info)->in_pm12x10 && ((PM_4x25_INFO(pm_info)->int_core_access.type == phymodDispatchTypeTscf16) ||
        (PM_4x25_INFO(pm_info)->int_core_access.type == phymodDispatchTypeTscf16_gen3))) {
    ...
    if(is_last_one){
       ...
       sal_memset(PM_4x25_INFO(pm_info)->int_core_access.access.pmd_info_ptr, 0, PMD_INFO_DATA_STRUCTURE_SIZE);
    ...
    if(enable){
        ...
        if((PORTMOD_PORT_ENABLE_PHY_GET(actual_flags)) && (!(PM_4x25_INFO(pm_info)->in_pm12x10))){
            _SOC_IF_ERR_EXIT
                ...
                /* Power set works on MOS EXT PHY, skip it for internal SerDes */
                if (nof_phys > 1) {
        ...
        if((PORTMOD_PORT_ENABLE_PHY_GET(actual_flags)) && (!(PM_4x25_INFO(pm_info)->in_pm12x10))){
            ...
            _SOC_IF_ERR_EXIT
                ...
                /* Power set works on MOST EXT PHY, skip it for internal SerDes */
                if (nof_phys > 1) {
                    _SOC_IF_ERR_EXIT (portmod_port_phychain_power_set(unit, port, phy_access, nof_phys, phychain_flag, &phy_power));
                }
        }  
    ...
    if(!(PM_4x25_INFO(pm_info)->in_pm12x10)){
       ...
       _SOC_IF_ERR_EXIT
        ...
        if(PORTMOD_CPM4x25_IS_GEN3_28nm(pm_version)) {
            ...
            _SOC_IF_ERR_EXIT(_pm4x25_speed_to_vco(unit, config->speed, config->port_num_lanes, serdes_1000x_at_25g_vco,
                                                 serdes_10g_at_25g_vco, &vco_rate, &pll_multiplier));
            if(config->flags != PHYMOD_INTF_F_SINGLE_PLL_ONLY) {
...
int pm4x25_port_local_fault_force_set(int unit, int port, pm_info_t pm_info, int enable)
{
    SOC_INIT_FUNC_DEFS;
    SOC_NULL_CHECK(pm_info);

    _SOC_IF_ERR_EXIT(clmac_link_fault_os_set(unit, port, 0, enable));

exit:
    SOC_FUNC_RETURN;
}

int pm4x25_port_local_fault_force_get(int unit, int port, pm_info_t pm_info, int * enable)
{
    SOC_INIT_FUNC_DEFS;
    SOC_NULL_CHECK(pm_info);

    _SOC_IF_ERR_EXIT(clmac_link_fault_os_get(unit, port, 0, (uint32 *)enable));
...
int pm4x25_port_remote_fault_force_set(int unit, int port, pm_info_t pm_info, int enable)
{
    SOC_INIT_FUNC_DEFS;
    SOC_NULL_CHECK(pm_info);

    _SOC_IF_ERR_EXIT(clmac_link_fault_os_set(unit, port, 1, enable));

exit:
    SOC_FUNC_RETURN;
}

int pm4x25_port_remote_fault_force_get(int unit, int port, pm_info_t pm_info, int * enable)
{
    SOC_INIT_FUNC_DEFS;
    SOC_NULL_CHECK(pm_info);

    _SOC_IF_ERR_EXIT(clmac_link_fault_os_get(unit, port, 1, (uint32 *)enable));
...
{
    int i;
    uint32 clmac_rsv_mask = 0;

    for (i = 0; i < portmodMacRsvCount; i++) {
        if (rsv_mask & (1 << i)) {
            clmac_rsv_mask |= (1 << i);
        }
    }
...
int pm4x25_port_mac_rsv_mask_get(int unit, int port, pm_info_t pm_info, uint32* rsv_mask)
{
    uint32 clmac_rsv_mask;
    uint32 rsv_mask_temp = 0;
    int i, rv;
    rv = READ_CLPORT_MAC_RSV_MASKr(unit, port, &clmac_rsv_mask);

    clmac_rsv_mask &= 0x3FFFFF;
    for (i = 0; i <= 21; i++) {
        if (clmac_rsv_mask & (0x1 << i)) {
            rsv_mask_temp |= (0x1 << i);
        }
    }

    *rsv_mask = rsv_mask_temp;
...
int pm4x25_port_lane_map_get(int unit, int port, pm_info_t pm_info,
                             uint32 flags, phymod_lane_map_t* lane_map)
{
    ...
    if(PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy_access[0].device_op_mode)) {
       *lane_map = PM_4x25_INFO(pm_info)->lane_map;
        SOC_EXIT;
    }

    /*
     * Retrieve the lane map information from the hardware.
     * Set lane_map to 0's for missing underlying implementation.
     */
    rv = phymod_core_lane_map_get(&PM_4x25_INFO(pm_info)->int_core_access,
                                  lane_map);
    if (rv == PHYMOD_E_UNAVAIL) {
        rv = PHYMOD_E_NONE;
        LOG_INFO(BSL_LS_SOC_PHY,
                (BSL_META_U(unit,
                            "Lane swap recover not implemented.\n")));
    }
...
int pm4x25_port_faults_status_get(int unit, int port, pm_info_t pm_info,
                                  portmod_port_fault_status_t *faults)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT(clmac_faults_status_get(unit, port, faults));
exit:
    SOC_FUNC_RETURN;
}

STATIC
int _pm4x25_flr_timeout_portmod_to_phymod(portmod_flr_timeout_t timeout, phymod_phy_flr_timeout_t* phymod_timeout)
{
    int rv = SOC_E_NONE;
...
int pm4x25_port_flr_config_set(int unit, int port, pm_info_t pm_info, int enable, portmod_flr_timeout_t timeout_time)
{
    int nof_phys;
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    ...
    if (!SAL_BOOT_SIMULATION) {
        _SOC_IF_ERR_EXIT
            (_pm4x25_flr_timeout_portmod_to_phymod(timeout_time, &phymod_timeout));
        _SOC_IF_ERR_EXIT
            (phymod_phy_flr_config_set(&phy_access, enable, phymod_timeout));
        if (enable) {
            _SOC_IF_ERR_EXIT(clmac_drop_tx_data_on_ordered_set_enable(unit, port, 1));
        } else {
            /*set to the default of HW when disable*/
            _SOC_IF_ERR_EXIT(clmac_drop_tx_data_on_ordered_set_enable(unit, port, 0));
        }
    }
...
STATIC
int _pm4x25_flr_timeout_phymod_to_portmod(phymod_phy_flr_timeout_t phymod_timeout, portmod_flr_timeout_t* timeout)
{
    int rv = SOC_E_NONE;
...
int pm4x25_port_flr_config_get(int unit, int port, pm_info_t pm_info, int *enable, portmod_flr_timeout_t *timeout_time)
{
        int nof_phys;
        portmod_access_get_params_t params;
        phymod_phy_access_t phy_access;
        ...
        *enable = flr_enable ? 1 : 0;
        _SOC_IF_ERR_EXIT
            (_pm4x25_flr_timeout_phymod_to_portmod(phymod_timeout, timeout_time));
...
int pm4x25_port_signal_detect_status_get(int unit, int port, pm_info_t pm_info, portmod_signal_detect_status_t* sd_status)
{
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    int nof_phys;
...
/* portmod mac strip crc set/get*/
int pm4x25_port_mac_strip_crc_set(int unit, int port, pm_info_t pm_info, uint32 enable)
{
    SOC_INIT_FUNC_DEFS;
    _SOC_IF_ERR_EXIT
        (clmac_strip_crc_set(unit, port, enable));
exit:
    SOC_FUNC_RETURN;
}
int pm4x25_port_mac_strip_crc_get(int unit, int port, pm_info_t pm_info, uint32* enable)
{
    SOC_INIT_FUNC_DEFS;
    _SOC_IF_ERR_EXIT
        (clmac_strip_crc_get(unit, port, enable));
exit:
    SOC_FUNC_RETURN;
}

/* portmod tx crc mode set/get*/
int pm4x25_port_mac_tx_crc_mode_set(int unit, int port, pm_info_t pm_info, portmod_mac_tx_crc_mode_t crc_mode)
{
    clmac_tx_crc_mode_t clmac_crc_mode = CLMAC_TX_CRC_MODE_REPLACE;
    SOC_INIT_FUNC_DEFS;

    switch (crc_mode) {
        case portmodMacTxCrcModeAppend:
            clmac_crc_mode = CLMAC_TX_CRC_MODE_APPEND;
            break;
        case portmodMacTxCrcModeKeep:
            clmac_crc_mode = CLMAC_TX_CRC_MODE_KEEP;
            break;
        case portmodMacTxCrcModeReplace:
            clmac_crc_mode = CLMAC_TX_CRC_MODE_REPLACE;
            break;
        case portmodMacTxCrcModeAuto:
            clmac_crc_mode = CLMAC_TX_CRC_MODE_AUTO;
            break;
        default:
            _SOC_IF_ERR_EXIT(SOC_E_PARAM);
    }
    ...
    _SOC_IF_ERR_EXIT
        (clmac_tx_crc_mode_set(unit, port, clmac_crc_mode));
exit:
    SOC_FUNC_RETURN;
}
int pm4x25_port_mac_tx_crc_mode_get(int unit, int port, pm_info_t pm_info, portmod_mac_tx_crc_mode_t* crc_mode)
{
    clmac_tx_crc_mode_t clmac_crc_mode;
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (clmac_tx_crc_mode_get(unit, port, &clmac_crc_mode));

    switch (clmac_crc_mode) {
        case CLMAC_TX_CRC_MODE_APPEND:
            *crc_mode = portmodMacTxCrcModeAppend;
            break;
        case CLMAC_TX_CRC_MODE_KEEP:
            *crc_mode = portmodMacTxCrcModeKeep;
            break;
        case CLMAC_TX_CRC_MODE_REPLACE:
            *crc_mode = portmodMacTxCrcModeReplace;
            break;
        case CLMAC_TX_CRC_MODE_AUTO:
            *crc_mode = portmodMacTxCrcModeAuto;
            break;
        default:
