...
#define ARAD_PORTS_FEM_PP_PORT_PON_PTC_SIZE(unit)  (SOC_DPP_CONFIG(unit)->pp.pon_port_ptc_size)
...
#define ARAD_PORTS_FEM_PON_PP_PORT_OFFSET_CHAN_ENCODE_MODE_1 (1)
...
#define ARAD_PORTS_VIRTUAL_PORT_PON_MAX_CHANNEL_ID(unit) (SOC_DPP_CONFIG(unit)->pp.pon_port_ptc_size > 5 ? 1 : 3)
  ...
  )
{
  ...
  if (!SOC_IS_JERICHO(unit)) {
      ...
      if (SOC_IS_QUX(unit) && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "disable_bad_reassembly_context_check", 0))
      {
          
          SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  8,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IRE_STATIC_CONFIGURATIONr, REG_PORT_ANY, 0, BAD_REASSEMBLY_CONTEXT_VALIDf, 0x0));
      }
      else
      {
      ...
      for (pp_port = 0; pp_port < MAX_PORTS_IN_CORE ; ++pp_port)
      {
          ...
          if (PBMP_MEMBER(pon_port_bm,port)) {
              ...
              else
              {
                  ...
                  if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "gpon_chan_encoding_mode", 0))
                  {
                      fld_val = ARAD_PORTS_FEM_PON_PP_PORT_OFFSET_CHAN_ENCODE_MODE_1;
                  }
              }
          }
  ...
  if (if_pon_pp_port_map_bypass == 0) {
      PBMP_ITER(pon_port_bm, local_port) {
          if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 0) {
              ...
              for (channel_id = 0; channel_id <= ARAD_PORTS_VIRTUAL_PORT_PON_MAX_CHANNEL_ID(unit); channel_id++) {
                  res = READ_IHP_VIRTUAL_PORT_TABLEm(unit, MEM_BLOCK_ANY, (ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(unit, local_port, channel_id)), &tbl_data_32b);
                  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 23, exit);
                  ...
                  fld_val = local_port;
                  soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data_32b, IN_PP_PORTf, fld_val);
                  res = WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, MEM_BLOCK_ANY, (ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(unit, local_port, channel_id)), &tbl_data_32b);
                  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 27, exit);
              }
          }
      }
  ...
  if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable || is_pon_dpp_support) {
    if (if_pon_pp_port_map_bypass == 0) {
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "gpon_application_enable", 0)) {
            ...
            int packet_header_offset = 0;
            if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "gpon_chan_encoding_mode", 0))
            {
                packet_header_offset = 20;
            }
            else
            {
                packet_header_offset = 16;
            }
            for (fem_bit_ndx = 0; fem_bit_ndx < SOC_DPP_DEFS_GET(unit, virtual_port_nof_bits); ++fem_bit_ndx) {
                ...
                virtual_port_fem_field_select_map.virtual_port_fem_field_select[fem_bit_ndx] =
                    ...
                    (fem_bit_ndx >= ARAD_PORTS_FEM_PP_PORT_PON_PTC_SIZE(unit) && 
                     fem_bit_ndx < (ARAD_PORTS_FEM_PP_PORT_PON_PTC_SIZE(unit) + ARAD_PORTS_FEM_PP_PORT_GPON_TID_PORT_SIZE)) ?  
                        (packet_header_offset - ARAD_PORTS_FEM_PP_PORT_PON_PTC_SIZE(unit) + fem_bit_ndx) : 0x30;
	        }
        }
        else if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 0) {
            for (fem_bit_ndx = 0; fem_bit_ndx < SOC_DPP_DEFS_GET(unit, virtual_port_nof_bits); ++fem_bit_ndx) {    
        ...
        if (flags & SOC_PORT_FLAGS_PON_INTERFACE) {
            if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 0) {
                ...
                for (tunnel_id = 0; tunnel_id < ARAD_PORTS_VIRTUAL_PORT_PON_MAX_TUNNEL_ID(unit); tunnel_id++) {
                    ...
                    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 143, exit);
                }
            } else if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 2) {
                
                for (channel_id = 0; channel_id <= ARAD_PORTS_VIRTUAL_PORT_PON_MAX_CHANNEL_ID(unit); channel_id++) {
                    res = READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(unit, local_port, channel_id), &tbl_data_32b);
                    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 144, exit);
                    
                    soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data_32b, VIRTUAL_PORT_SRC_SYS_PORTf, sys_port);
                    res = WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(unit, local_port, channel_id), &tbl_data_32b);
  ...
  )
{
    ...
    if (((info->header_type == SOC_TMC_PORT_HEADER_TYPE_TM) || (arad_port_header_type_is_injected_tm(info->header_type))) && (!(SOC_DPP_CONFIG(unit)->pp.pon_application_enable))) {
        ...
        uint32 pp_port_to_check, pp_port_to_set = 0xff;
        int core_to_check;
        SOC_TMC_PORT_HEADER_TYPE header_type_in_tmp;
        uint8 is_tm_type_port_exist_in_same_core = 0;
        uint32 is_valid;

        
        
        SOCDNX_IF_ERR_EXIT(READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_INJECTED_TM_ENCODED(0), &tbl_data_32b));
        pp_port_to_check = soc_IHP_VIRTUAL_PORT_TABLEm_field32_get(unit, &tbl_data_32b, IN_PP_PORTf);
        
        if (pp_port == pp_port_to_check) {
            
            for (port_encoded = 0; port_encoded < SOC_DPP_DEFS_GET(unit, nof_local_ports); port_encoded++) {
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_valid_port_get(unit, port_encoded, &is_valid));
                if (is_valid) {
                    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_pp_port_get(unit, port_encoded, &pp_port_to_check, &core_to_check));
                    if ((pp_port_to_check == pp_port) && (core_to_check == core)) {
                        continue;
                    }
                    SOCDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.tm.logical_ports_info.header_type_in.get(unit, port_encoded, &header_type_in_tmp));
                    if ((header_type_in_tmp == SOC_TMC_PORT_HEADER_TYPE_TM) || arad_port_header_type_is_injected_tm(header_type_in_tmp)) {                        
                        if (core_to_check == core) {
                            is_tm_type_port_exist_in_same_core = 1;
                            pp_port_to_set = pp_port_to_check;
                        }
                    }
                }
            }
            pp_port_to_set = is_tm_type_port_exist_in_same_core ? pp_port_to_set : 0xff;
            for (port_encoded = 0; port_encoded < ARAD_NOF_FAP_PORTS; ++port_encoded) {
                SOCDNX_IF_ERR_EXIT(READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_INJECTED_TM_ENCODED(port_encoded), &tbl_data_32b));
                soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data_32b, IN_PP_PORTf, pp_port_to_set);
                SOCDNX_IF_ERR_EXIT(WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_INJECTED_TM_ENCODED(port_encoded), &tbl_data_32b));
            }
        }
    }
    ...
    if (((SOC_DPP_CONFIG(unit))->pp.routing_preserve_dscp & SOC_DPP_DSCP_PRESERVE_PORT_CTRL_EN) && info->flags & ARAD_PORTS_PRESERVING_DSCP_PORT_PP_PORT) {
        if (pp_pct_tbl_data.prge_profile == ARAD_PRGE_PP_SELECT_MIXED_QOS_MODEL) {
            prge_prog_select = ARAD_PRGE_PP_SELECT_COPY_DSCP_MIXED_QOS_MODEL;
        } else {
  ...
  )
{
    ...
    switch(pp_pct_tbl_data.prge_profile) {
        ...
        case ARAD_PRGE_PP_SELECT_PRESERVING_DSCP:
            info->flags |= ARAD_PORTS_PRESERVING_DSCP_PORT_PP_PORT;
        ...
        if (_BCM_PPD_PON_PP_PORT_MAPPING_BY_PASS(unit) == 0) {
            if ((SOC_PBMP_MEMBER(pon_port_bm, port))) {
                if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 0) {
                    ...
                    for (channel_id = 0; channel_id <= ARAD_PORTS_VIRTUAL_PORT_PON_MAX_CHANNEL_ID(unit); channel_id++) {
                        SOCDNX_IF_ERR_EXIT(READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(unit, port, channel_id), &tbl_data));
                        soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data, IN_PP_PORTf, pp_port);
                        SOCDNX_IF_ERR_EXIT(WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(unit, port, channel_id), &tbl_data));
                    }
                }
            }
        }
    ...
    )
{
    ...
    uint32 lag_id, lag_member_id, sys_phys_port_id;
    ARAD_SYSPORT master_lag_sysport;
    uint8 update_egq_dsp_ptr_map_tbl = 1;
    ...
    if( nof_replications == 0 ) 
    {
        ...
        if( SOC_DPP_IS_MODID_AND_BASE_MODID_ON_SAME_FAP(unit, fap_id_add_lag_member, sys_fap_id_self) == 1 )
        {
            ...
            SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_get(unit, fap_id_add_lag_member, port_id_add_lag_member, &master_lag_sysport));
            SOCDNX_SAND_IF_ERR_EXIT(arad_ports_logical_sys_id_parse(unit, master_lag_sysport, &is_lag_not_phys, &lag_id, &lag_member_id, &sys_phys_port_id));                
            
            
            if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "hqos_update_dst_sys_port_once", 0)) {
                if (master_lag_sysport != ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT && (lag_id != lag_ndx)) {
                    update_egq_dsp_ptr_map_tbl = 0;
                }
                
                if (!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "hqos_update_src_sys_port_once", 0)) {
                    if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
                        SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_set(unit,fap_id_add_lag_member,port_id_add_lag_member,sys_logic_port_id));
                    } else {
            ...
            if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "hqos_update_src_sys_port_once", 0)) {
                if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
    ...
    if (need_to_disable_not_to_full_remove == 0)
    {
        ...
        if (nof_replications == 1) 
        {
            ...
            if ( SOC_DPP_IS_MODID_AND_BASE_MODID_ON_SAME_FAP(unit, fap_id_removed_lag_member, sys_fap_id_self) == 1)
            {
                ...
                SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_get(unit, fap_id_removed_lag_member, port_id_removed_lag_member, &master_lag_sysport));
                SOCDNX_SAND_IF_ERR_EXIT(arad_ports_logical_sys_id_parse(unit, master_lag_sysport, &is_lag_not_phys, &lag_id, &lag_member_id, &sys_phys_port_id));
                ...
                if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                    ...
                    if ((master_lag_sysport != ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) && (lag_id == lag_ndx)) {
                        ...
                        SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_remove(unit, fap_id_removed_lag_member, port_id_removed_lag_member));
                    }
                } else {
        ...
        if( removed_member_index != moved_lag_member_index)
        {
            ...
            if(( moved_lag_member_nof_replications == 1) || ( (moved_lag_member_first_replication_index != -1) && (moved_lag_member_first_replication_index > removed_member_index) ))
            {
                ...
                {                    
                    ...
                    SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_get(unit, fap_id_moved_lag_member, port_id_moved_lag_member, &master_lag_sysport));
                    SOCDNX_SAND_IF_ERR_EXIT(arad_ports_logical_sys_id_parse(unit, master_lag_sysport, &is_lag_not_phys, &lag_id, &lag_member_id, &sys_phys_port_id));                
                    ...
                    if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                        ...
                        if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
                            ...
                            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("arad_ports_lag_sys_port_remove_unsafe error under hqos_update_src_sys_port_once mode")));
                        } else {
                            if (lag_id == lag_ndx) {
                                ...
                                SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_set(unit,fap_id_moved_lag_member,port_id_moved_lag_member,sys_logic_port_id));
                            }
                        }
                    } else {
    ...
    for(current_member_index = 0; current_member_index < nof_members_in_lag; ++current_member_index)
    {
        ...
        {                    
            ...
            if(current_member_first_replication_index == current_member_index)
            {
                ...
                SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_get(unit, fap_id_current_lag_member, port_id_current_lag_member, &master_lag_sysport));
                SOCDNX_SAND_IF_ERR_EXIT(arad_ports_logical_sys_id_parse(unit, master_lag_sysport, &is_lag_not_phys, &lag_id, &lag_member_id, &sys_phys_port_id));                
            ...
            {                
                ...
                if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                    ...
                    if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
                        ...
                        SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_set(unit,fap_id_current_lag_member,port_id_current_lag_member,sys_logic_port_id));
                    } else {
  ...
  )
{
  ...
  res = READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), tbl_idx, &tbl_data_32b);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  res = WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), tbl_idx, &tbl_data_32b);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
    ...
    SOC_SAND_OUT uint32     enable)
{
    ...
    if(enable) 
    {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ignore_fc_mode", 0)){
                for (q_pair = 0; q_pair < ARAD_EGR_NOF_Q_PAIRS; q_pair++) {
            ...
            for (q_pair = base_q_pair; q_pair < (base_q_pair + nof_queues); q_pair++) {
                SHR_BITCLR(data_above_64, q_pair);
            }
            res = WRITE_EGQ_FORCE_OR_IGNORE_FCr(unit, core, data_above_64);
            SOC_SAND_CHECK_FUNC_RESULT(res , 96, enable_shapers);
        }
   ...
   )
{
    ...
    uint32 tunnel_id, channel_id, tbl_data_32b;
    ...
    if (_BCM_PPD_PON_PP_PORT_MAPPING_BY_PASS(unit) == 0) {
        if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 0) {
            ...
            for (channel_id = 0; channel_id <= ARAD_PORTS_VIRTUAL_PORT_PON_MAX_CHANNEL_ID(unit); channel_id++) {
                SOCDNX_IF_ERR_EXIT(READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(unit, port, channel_id), &tbl_data_32b));
                soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data_32b, IN_PP_PORTf, value? pp_port : 0);
                
                soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data_32b, VIRTUAL_PORT_SRC_SYS_PORTf, value? port : 0);
                SOCDNX_IF_ERR_EXIT(WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(unit, port, channel_id), &tbl_data_32b));
            }
        }
    }
