  ...
  )
{
    ...
    *found = 0;
    ...
    if ((action_type == SOC_PPC_FP_ACTION_TYPE_TRAP) || (action_type == SOC_PPC_FP_ACTION_TYPE_TRAP_REDUCED))
    {
        fem_cycle.is_cycle_fixed = 1;
        fem_cycle.cycle_id = 1;
    }
...
)
{
    ...
    *idx = -1;
    for (fes_idx = (cycle_to_use*nof_fes_group); fes_idx < ARAD_PMF_LOW_LEVEL_NOF_FESS-1; fes_idx++) {
    ...
    if ((*idx == -1) 
        && (fes_info[fes_idx+1].action_type == SOC_PPC_FP_ACTION_TYPE_INVALID_NEXT)) {
        *idx = fes_idx + 1;
    }
...
)
{
    ...
    if (idx_invalid_next > idx_free) {
        ...
        idx_move_from = idx_invalid_next + cycle_to_use * nof_fes_per_group;
        idx_move_to = idx_free + cycle_to_use * nof_fes_per_group;
        ARAD_PP_FP_FES_OPERATION_MOVE(pmf_pgm_ndx, idx_move_from, idx_move_to);
        is_fem_lower[idx_move_to] = is_fem_lower[idx_move_from];
        is_fem_higher[idx_move_to] = is_fem_lower[idx_move_from];
        is_fem_lower[idx_move_from] = FALSE;
        is_fem_higher[idx_move_from] = FALSE;
        
        
        idx_move_start = idx_free + 1 + cycle_to_use * nof_fes_per_group;
        idx_move_end = idx_invalid_next - 1 + cycle_to_use * nof_fes_per_group;
        ...
        for (fem_id_ndx = idx_move_end + 1; fem_id_ndx > idx_move_start; fem_id_ndx--) {
            is_fem_lower[fem_id_ndx] = is_fem_lower[fem_id_ndx-1];
            is_fem_higher[fem_id_ndx] = is_fem_lower[fem_id_ndx-1];
        }
        is_fem_lower[idx_move_start] = FALSE;
        is_fem_higher[idx_move_start] = FALSE;
    ...
    idx_move_start = idx_invalid_next + 1 + group_base;
    idx_move_end = idx_free + group_base;
    res = arad_pmf_db_fes_chunk_move_unsafe(
            unit,
            pmf_pgm_ndx,
            flags,
    ...
    for (fem_free_first = fem_free_min; fem_free_first < nof_fem_fes_per_group; ++fem_free_first)
    {
        if (is_for_fem) {
              ...
              && (fem_free_min_ndx == nof_fem_fes_per_group))
          {
            ...
            {
                if((fes_info[fes_id_indx].action_type == entry_ndx->action_type[action_ndx])
                    || ((fes_info[fes_id_indx].action_type == SOC_PPC_FP_ACTION_TYPE_TRAP) && (entry_ndx->action_type[action_ndx] == SOC_PPC_FP_ACTION_TYPE_TRAP_REDUCED))
                    || ((fes_info[fes_id_indx].action_type == SOC_PPC_FP_ACTION_TYPE_TRAP_REDUCED) && (entry_ndx->action_type[action_ndx] == SOC_PPC_FP_ACTION_TYPE_TRAP))
                ...
                if ((entry_ndx->action_type[action_ndx] == SOC_PPC_FP_ACTION_TYPE_TRAP)
                    || (entry_ndx->action_type[action_ndx] == SOC_PPC_FP_ACTION_TYPE_TRAP_REDUCED))
                {
                    if ((fes_info[fes_id_indx].action_type != SOC_PPC_FP_ACTION_TYPE_TRAP)
                        && (fes_info[fes_id_indx].action_type != SOC_PPC_FP_ACTION_TYPE_TRAP_REDUCED))
                    {
                        is_fem_lower[fem_free_first] = TRUE;
                    }
                }
        ...
        if (!is_for_fem && is_insert_combined) {
            ...
            if (idx_invalid_next != -1) {
                ...
                if (idx_invalid_next > idx_free) {
                    ...
                    idx_invalid_next = idx_free;
                    fem_free_first = idx_free + 1;
                    fem_free_last = fem_free_first;
                    idx_free = idx_free + 1;
                }
            }
