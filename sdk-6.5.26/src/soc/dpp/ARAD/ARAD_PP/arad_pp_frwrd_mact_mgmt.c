  ...
  )
{
    ...
    mact_reg = (SOC_IS_JERICHO(unit)? PPDB_B_LARGE_EM_DEFRAG_CONFIGURATION_REGISTERr: IHP_REG_02B3r);
    if (SOC_IS_JERICHO(unit)) {
    ...
    jer_mact_fifo_thread(void *param)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_L2,
                (BSL_META_U(unit,
                        "%s - channel=0x%x, cmc=%d, ch=%d \n"), __func__, channel_number, cmc, ch));

    while (jer_mact_fifo_terminate[unit] == FALSE)
    {
    ...
    jer_mact_fifo_init(int unit)
{
  ...
  SOCDNX_INIT_FUNC_DEFS;
  if(!jer_mact_fifo_terminate[unit])
  {
  ...
  if(jer_mact_fifo_sem[unit] == NULL) {
    ...
    SOCDNX_EXIT_WITH_ERR(SOC_E_MEMORY, (_BSL_SOCDNX_MSG("Semaphore allocation failure (MACT_fifo_sem.%d"), unit));
  }
