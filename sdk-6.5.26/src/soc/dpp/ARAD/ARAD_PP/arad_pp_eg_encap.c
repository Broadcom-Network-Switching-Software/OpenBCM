...
#define ARAD_PP_EG_ENCAP_AC_OUTLIF_PROFILE_MAX    (SOC_IS_JERICHO_PLUS(unit)?0x3f:0x7)
  ...
  )
{
    ...
    if(eep_type == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_AC) {
        ...
        if ((flags & SOC_PPC_EG_ENCAP_ENTRY_UPDATE_OUT_LIF_PROFILE) && (val > ARAD_PP_EG_ENCAP_AC_OUTLIF_PROFILE_MAX))
        {
            ...
            SOC_SAND_SET_ERROR_CODE(ARAD_PP_EG_ENCAP_ACCESS_ENTRY_OUTLIF_PROFILE_MISMATCH_ERR, 15, exit);
        }
