  ...
  )
{
  ...
  if (hw_q_type != SOC_TMC_ITM_QT_PUSH_Q_NDX) {
      ...
      max_allowed = SOC_IS_JERICHO(unit) ? (63 * (1 << 15)) : (7 * (1 << 12));
      SOC_SAND_CHECK_FUNC_RESULT(arad_itm_cr_request_call_needed_credit_balance_resolution(
        int32_max, max_allowed, &cr_bal_resolution), 20, exit); 
  ...
  if (hw_q_type == ARAD_SW_DB_QUEUE_TYPE_NOT_AVAILABLE) {
      ...
      if (hw_q_type != SOC_TMC_ITM_QT_PUSH_Q_NDX) {
          ...
          if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
              ...
              SOC_SAND_SET_ERROR_CODE(ARAD_UNSUPPORTED_ERR, 405, exit); 
          }
...
shr_error_e
arad_itm_ingress_receive_shaper_grant_size_and_timer_cycles_calc(
    int unit,
    uint32 rate,
    ...
    uint32 *best_timer_cycles)
{
    ...
    uint64 u64_calc;
    uint32 core_clock = SOC_DPP_CONFIG(unit)->arad->init.core_freq.frequency;
    ...
    else
    {
        ...
        *best_grant_size = -1;
        for (current_timer_cycles = 1; current_timer_cycles <= max_nof_timer_cycles; current_timer_cycles++)
        {
            ...
            COMPILER_64_ADD_32(u64_calc, (core_clock / 2));
            COMPILER_64_UDIV_32(u64_calc, core_clock);
            grant_size = COMPILER_64_LO(u64_calc);
            
            COMPILER_64_ZERO(u64_calc);
            COMPILER_64_SET(u64_calc, 0, grant_size);
            COMPILER_64_UMUL_32(u64_calc, core_clock);
    ...
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("The provided rate (%d) cannot be configured.\n"), rate));
    }
...
static int
arad_itm_ingress_receive_shaper_actual_rate_calc(
    int unit,
    uint32 grant_size,
    uint32 timer_cycles,
    uint32 *actual_rate)
{
    ...
    else
    {
        if (timer_cycles == 0)
        {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("Timer cycles value cannot be 0.\n")));
        }
...
uint32
arad_itm_rcy_shaper_set(
    SOC_SAND_IN  int   unit,
    SOC_SAND_IN  int   core,
    SOC_SAND_IN  uint32   kbits_sec)
{
    ...
    soc_field_t grant_size_field, timer_cycles_field;

    SOCDNX_INIT_FUNC_DEFS;

    timer_cycles_field = (nof_cores == 1 ? RCY_SHAPER_TIMER_CYCLESf : RCY_N_SHAPER_TIMER_CYCLESf);
    grant_size_field = (nof_cores == 1 ? RCY_SHAPER_GRANT_SIZEf : RCY_N_SHAPER_GRANT_SIZEf);

    max_nof_timer_cycles = (1 << soc_reg_field_length(unit, IRE_RECYCLING_SHAPERr, timer_cycles_field)) - 1;
    max_grant_size_val   = (1 << soc_reg_field_length(unit, IRE_RECYCLING_SHAPERr, grant_size_field)) - 1;

    
    rate_mbytes_sec = UTILEX_DIV_ROUND(UTILEX_TRANSLATE_BITS_TO_BYTES(kbits_sec), 1000);

    
    SOCDNX_IF_ERR_EXIT(arad_itm_ingress_receive_shaper_grant_size_and_timer_cycles_calc
                       (unit, rate_mbytes_sec, max_nof_timer_cycles, max_grant_size_val,
                        &grant_size, &timer_cycles));


    SOCDNX_IF_ERR_EXIT(READ_IRE_RECYCLING_SHAPERr(unit, core, &reg_val));
    soc_reg64_field_set(unit, IRE_RECYCLING_SHAPERr, &reg_val, timer_cycles_field, timer_cycles - 1);
    soc_reg64_field_set(unit, IRE_RECYCLING_SHAPERr, &reg_val, grant_size_field, grant_size);
    SOCDNX_IF_ERR_EXIT(WRITE_IRE_RECYCLING_SHAPERr(unit, core, reg_val));
...
uint32
arad_itm_rcy_shaper_get(
    SOC_SAND_IN  int   unit,
    SOC_SAND_IN  int   core,
    SOC_SAND_OUT uint32   *kbits_sec)
{
    int nof_cores = SOC_DPP_DEFS_GET(unit, nof_cores);
    uint64 reg_val;
    uint32 timer_cycles = 0, grant_size = 0, rate_mbytes_sec = 0;
    ...
    SOCDNX_IF_ERR_EXIT(READ_IRE_RECYCLING_SHAPERr(unit, core, &reg_val));
    timer_cycles = soc_reg64_field_get(unit, IRE_RECYCLING_SHAPERr, reg_val, timer_cycles_field);
    grant_size = soc_reg64_field_get(unit, IRE_RECYCLING_SHAPERr, reg_val, grant_size_field);

    SOCDNX_IF_ERR_EXIT(arad_itm_ingress_receive_shaper_actual_rate_calc(unit, grant_size, timer_cycles+1, &rate_mbytes_sec));

    
    *kbits_sec = rate_mbytes_sec * UTILEX_NOF_BITS_IN_BYTE * 1000;
