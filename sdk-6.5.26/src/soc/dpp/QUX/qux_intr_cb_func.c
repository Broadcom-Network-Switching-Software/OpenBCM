...
int qux_event_drc_cmd_parity_err_handler(int unit, int block_instance, uint32 en_interrupt, char* msg)
{
    
    qux_interrupt_type drc_interrupt_arr[] = {QUX_INT_DRCA_DRAM_CMD_PARITY_ERR, QUX_INT_DDP_EXT_MEM_ERR_PKUP_CPYDAT_ECC_1B_ERR_INT,
                                              QUX_INT_DDP_EXT_MEM_ERR_PKUP_CPYDAT_ECC_2B_ERR_INT, QUX_INT_DDP_EXT_MEM_ERR_PKUP_CPYDAT_CRC_ERR_INT,
                                              QUX_INT_DDP_EXT_MEM_ERR_PKUP_PACKET_CRC_ERR_INT};
    
    int is_enabled[] =                       {0, 0, 0, 0, 0};
    ...
    for (interrupt_index = 0; interrupt_index < drc_interrupt_arr_size; interrupt_index += 1)
    {
        ...
        SOCDNX_IF_ERR_EXIT(soc_interrupt_is_enabled(unit, block_instance, &interrupt_info, &(is_enabled[interrupt_index])));
        
        if (is_enabled[interrupt_index])
        {
            SOCDNX_IF_ERR_EXIT(soc_interrupt_disable(unit, block_instance, &interrupt_info));
        }
        
        SOCDNX_IF_ERR_EXIT(soc_interrupt_get(unit, block_instance, &interrupt_info, &interrupt_occured));
    ...
    LOG_VERBOSE(BSL_LS_SOC_INTR, (BSL_META_U(unit, "%s(): activating corrective action sequence on DRC index %d"), FUNCTION_NAME(), 0));
    SOCDNX_IF_ERR_EXIT(soc_dpp_drc_combo28_cmd_parity_err_handle_interrupt(unit, 0));
    ...
    for (interrupt_index = 0; interrupt_index < drc_interrupt_arr_size; interrupt_index += 1)
    {
        ...
        if (is_enabled[interrupt_index])
        {
            SOCDNX_IF_ERR_EXIT(soc_interrupt_enable(unit, block_instance, &interrupt_info));
        }
    }
...
void qux_interrupt_cb_init(int unit)
{
    ...
    if (SOC_DPP_CONFIG(unit)->tm.soft_reset_on_dram_cmd_parity_err_enabled)
    {
        dcmn_intr_add_handler_ext(unit, QUX_INT_DRCA_DRAM_CMD_PARITY_ERR, 1000,100, qux_event_drc_cmd_parity_err_handler, qux_event_drc_cmd_parity_err_handler,"Stop traffic to DRAM, re-tune DRAM, then allow traffic to DRAM again");
        dcmn_intr_add_handler_ext(unit, QUX_INT_DDP_EXT_MEM_ERR_PKUP_CPYDAT_ECC_1B_ERR_INT, 1000,100, qux_event_drc_cmd_parity_err_handler, qux_event_drc_cmd_parity_err_handler,"Stop traffic to DRAM, re-tune DRAM, then allow traffic to DRAM again");
        dcmn_intr_add_handler_ext(unit, QUX_INT_DDP_EXT_MEM_ERR_PKUP_CPYDAT_ECC_2B_ERR_INT, 1000,100, qux_event_drc_cmd_parity_err_handler, qux_event_drc_cmd_parity_err_handler,"Stop traffic to DRAM, re-tune DRAM, then allow traffic to DRAM again");
        dcmn_intr_add_handler_ext(unit, QUX_INT_DDP_EXT_MEM_ERR_PKUP_CPYDAT_CRC_ERR_INT, 1000,100, qux_event_drc_cmd_parity_err_handler, qux_event_drc_cmd_parity_err_handler,"Stop traffic to DRAM, re-tune DRAM, then allow traffic to DRAM again");
        dcmn_intr_add_handler_ext(unit, QUX_INT_DDP_EXT_MEM_ERR_PKUP_PACKET_CRC_ERR_INT, 1000,100, qux_event_drc_cmd_parity_err_handler, qux_event_drc_cmd_parity_err_handler,"Stop traffic to DRAM, re-tune DRAM, then allow traffic to DRAM again");
    }
    ...
    dcmn_intr_add_handler(unit, QUX_INT_EDB_ESEM_ONE_ESEM_ERROR_TABLE_COHERENCY, 0,0,dcmn_interrupt_handle_EDB_EsemErrorTableCoherency_Int,NULL);
    dcmn_intr_add_handler(unit, QUX_INT_EDB_GLEM_ONE_GLEM_ERROR_TABLE_COHERENCY, 0,0,dcmn_interrupt_handle_EDB_GlemErrorTableCoherency_Int,NULL);
