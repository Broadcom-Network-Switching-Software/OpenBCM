...
STATIC int jer_kaps_init_device(int unit)
{
    ...
    res = kaps_sw_model_init(JerAppData[unit]->dalloc_p, KAPS_DEVICE_KAPS, KAPS_DEVICE_DEFAULT,
                              KAPS_DEFAULT_DEVICE_ID, 0, 0, &(JerAppData[unit]->kaps_xpt_p));
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
       LOG_ERROR(BSL_LS_SOC_TCAM,
                 (BSL_META_U(unit,
                             "Error in %s(): kaps_sw_model_init with failed: %s!\n"), 
                             FUNCTION_NAME(),
    ...
    KAPS_TRY(kaps_device_init(KAPS_DEVICE_PAIR_TYPE_A,
                            flags, 
