...
STATIC int soc_jer_init_mesh_topology(int unit)
{
    ...
    switch (gt_size) {
    case 0:
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, MESH_TOPOLOGY_MESH_TOPOLOGYr, REG_PORT_ANY, 0, &reg_val)); 
        soc_reg_field_set(unit, MESH_TOPOLOGY_MESH_TOPOLOGYr, &reg_val, RESERVED_2f, gt_size);
        SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, MESH_TOPOLOGY_MESH_TOPOLOGYr, REG_PORT_ANY, 0, reg_val));
        break;
    case 1:
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, MESH_TOPOLOGY_MESH_TOPOLOGYr, REG_PORT_ANY, 0, &reg_val)); 
        soc_reg_field_set(unit, MESH_TOPOLOGY_MESH_TOPOLOGYr, &reg_val, RESERVED_2f, gt_size);
...
int soc_jer_init_sequence_phase1(int unit)
{
...
#ifdef PLISIM
    if (SAL_BOOT_PLISIM) {
      chip_sim_em_init(unit, SOC_SAND_DEV_ARAD, TRUE );
    }
#endif
    ...
    if (!SOC_WARM_BOOT(unit)) {
       ...
       LOG_INFO(BSL_LS_SOC_INIT, (BSL_META_U(unit, "\t+ %d: Configure tables defaults\n"),unit));
        ...
        if (!SOC_IS_QAX(unit) && !SOC_WARM_BOOT(unit)) {
            SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
