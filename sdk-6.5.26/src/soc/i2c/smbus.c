...
#define READ_IPROCPERIPH_SMBUS_REG(regName, unit, val)       \
    if (soc_feature(unit, soc_feature_use_smbus2_for_i2c)) { \
        ...
        if (soc_feature(unit, soc_feature_use_smbus3_for_i2c)) { \
            READ_IPROCPERIPH_SMBUS3_SMBUS_##regName(unit, val);  \
...
#define WRITE_IPROCPERIPH_SMBUS_REG(regName, unit, val)      \
    if (soc_feature(unit, soc_feature_use_smbus2_for_i2c)) { \
        ...
        if (soc_feature(unit, soc_feature_use_smbus3_for_i2c)) { \
            WRITE_IPROCPERIPH_SMBUS3_SMBUS_##regName(unit, val); \
        } else if (soc_feature(unit, soc_feature_use_smbus0_for_i2c)) { \
...
smbus_timeout_recovery(int unit)
{
    ...
    if (soc_reg_field_get(unit, CMIC_I2CM_SMBUS_BIT_BANG_CONTROLr, rval, SMBDAT_INf) == 0) {
        LOG_INFO(BSL_LS_SOC_I2C,
                (BSL_META_U(unit, "i2c%d: smbus_timeout_recovery: SMBDAT pin is high.\n"), unit));
    ...
    LOG_INFO(BSL_LS_SOC_I2C,
                (BSL_META_U(unit, "i2c%d: smbus_timeout_recovery: start reset the bus\n"), unit));
...
smbus_start_wait(int unit)
{
    ...
    soc_timeout_t to;
    uint32 timeout, rval, status;

    /* Check the an operation is in progress */
    ...
    if (soc_reg_field_get(unit, CMIC_I2CM_SMBUS_MASTER_COMMANDr, rval, MASTER_START_BUSY_COMMANDf) != 0) {
        LOG_INFO(BSL_LS_SOC_I2C,
                (BSL_META_U(unit, "ERROR: i2c%d: i2c bus is busy.\n"), unit));
    ...
    timeout = soc_property_get(unit, spn_I2C_TIMEOUT_USEC, I2C_TIMEOUT);
    soc_timeout_init(&to, timeout, 1000);
...
/* Read with RD status check. */
int
smbus_read_with_status_check(int unit, uint8 *data)
{
    int rv = SOC_E_NONE;
    uint32 rval;

    /* Get data from I2C bus and check the RD status */
    READ_CMIC_I2CM_SMBUS_MASTER_DATA_READr(unit, &rval);
    if (soc_reg_field_get(unit, CMIC_I2CM_SMBUS_MASTER_DATA_READr, rval, MASTER_RD_STATUSf) != 0) {
        *data = (uint8) (rval & 0xff);
    } else {
        LOG_INFO(BSL_LS_SOC_I2C,
                (BSL_META_U(unit, "ERROR: i2c%d: data is not ready\n"), unit));
        rv = SOC_E_FAIL;
    }
...
cmicx_smbus_timeout_recovery(int unit)
{
    ...
    LOG_INFO(BSL_LS_SOC_I2C,
                (BSL_META_U(unit, "i2c%d: cmicx_smbus_timeout_recovery: start reset the bus\n"), unit));
...
/* Read with RD status check. */
int
cmicx_smbus_read_with_status_check(int unit, uint8 *data)
{
    int rv = SOC_E_NONE;
    uint32 rval;
    ...
    if (soc_reg_field_get(unit, IPROCPERIPH_SMBUS1_SMBUS_MASTER_DATA_READr, rval, MASTER_RD_STATUSf) != 0) {
        *data = (uint8) (rval & 0xff);
    } else {
        LOG_INFO(BSL_LS_SOC_I2C,
                (BSL_META_U(unit, "ERROR: i2c%d: data is not ready\n"), unit));
...
iproc_smbus_timeout_recovery(int unit)
{
    ...
    LOG_INFO(BSL_LS_SOC_I2C,
                (BSL_META_U(unit, "i2c%d: iproc_smbus_timeout_recovery: start reset the bus\n"), unit));
...
retry:
        ...
        rv = smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            rv = smbus_read_with_status_check(unit, data);
        }
        if (rv != SOC_E_NONE && rt-- > 0) {
            goto retry;
...
retry_cmicx:
        ...
        rv = cmicx_smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            rv = cmicx_smbus_read_with_status_check(unit, data);
        }
        if (rv != SOC_E_NONE && rt-- > 0) {
            goto retry_cmicx;
        } else if (rv != SOC_E_NONE) {
...
retry:
        ...
        rv = smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            rv = smbus_read_with_status_check(unit, &b0);
        }
        if (rv == SOC_E_NONE) {
            rv = smbus_read_with_status_check(unit, &b1);
        }
        if (rv == SOC_E_NONE) {
            rval = (b1 << 8) + b0;
            *value = (uint16) rval;
...
retry_cmicx:
        ...
        rv = cmicx_smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            rv = cmicx_smbus_read_with_status_check(unit, &b0);
        }
        if (rv == SOC_E_NONE) {
            rv = cmicx_smbus_read_with_status_check(unit, &b1);
        }
        if (rv == SOC_E_NONE) {
            rval = (b1 << 8) + b0;
            *value = (uint16) rval;
...
retry:
        ...
        rv = smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            rv = smbus_read_with_status_check(unit, value);
        }
        if (rv != SOC_E_NONE && rt-- > 0) {
            goto retry;
        } else if (rv != SOC_E_NONE) {
...
retry_cmicx:
        ...
        rv = cmicx_smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            rv = cmicx_smbus_read_with_status_check(unit, value);
        }
        if (rv != SOC_E_NONE && rt-- > 0) {
            goto retry_cmicx;
        } else if (rv != SOC_E_NONE) {
...
retry:
        ...
        rv = smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            rv = smbus_read_with_status_check(unit, count);
        }
        if (rv == SOC_E_NONE) {
            ptr = data;
            for( i = 0; i < *count; i++, ptr++) {
                rv = smbus_read_with_status_check(unit, ptr);
                if (rv != SOC_E_NONE) { /* Stop reading */
                    break;
                }
            }
        }
...
retry_cmicx:
        ...
        rv = cmicx_smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            rv = cmicx_smbus_read_with_status_check(unit, count);
        }
        if (rv == SOC_E_NONE) {
            ptr = data;
            for( i = 0; i < *count; i++, ptr++) {
                rv = cmicx_smbus_read_with_status_check(unit, ptr);
                if (rv != SOC_E_NONE) { /* Stop reading */
                    break;
                }
            }
