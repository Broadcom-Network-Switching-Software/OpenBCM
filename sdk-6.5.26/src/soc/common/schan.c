...
* @param dwc [in] size of register,memory in 32bit words
* @param verbosity [in] log level verbosity BSL_FATAL,BSL_ERROR,BSL_WARN,BSL_INFO,BSL_VERBOSE,BSL_DEBUG
*
* @returns SOC_E_NONE
* @returns SOC_E_XXX
*
...
********************************************/
void
soc_schan_dump(int unit, schan_msg_t *msg, int dwc, int verbosity)
{
    char                buf[128];
    int                 i, j;
    ...
    if (soc_feature(unit, soc_feature_sbus_format_v4)) {
        ...
        BSL_LOG(BSL_LS_SOC_SCHAN|verbosity, (BSL_META_U(unit,"  HDR[NACK=%d BANK=%d DMA=%d ECODE=%d ERR=%d "
    ...
    if (soc_feature(unit, soc_feature_generic_table_ops) &&
        ...
        if (soc_feature(unit, soc_feature_new_sbus_format) &&
            ...
            BSL_LOG(BSL_LS_SOC_SCHAN|verbosity, (BSL_META_U(unit, "  RSP[TYPE=%d=%s ERR_INFO=%d=%s "
                     "INDEX=0x%05x]\n"),
            ...
            BSL_LOG(BSL_LS_SOC_SCHAN|verbosity, (BSL_META_U(unit, "  RSP[SRC=%d TYPE=%d=%s ERR_INFO=%d=%s "
                     "INDEX=0x%05x]\n"),
    ...
    for (i = 0; i < dwc; i += 4) {
        ...
        BSL_LOG(BSL_LS_SOC_SCHAN|verbosity, (BSL_META_U(unit, " %s\n"), buf));
    }
}
/*******************************************
...
_soc_schan_check_hw_access_disabled(int unit, schan_msg_t *msg, int *rv)
{
    ...
    const uint32 kt2_ing_pipe_blk = 1;
    const uint32 kt2_addr_ING_Q_BEGINr = 0x2010800;
    uint16 dev_id;
    uint8 rev_id;
#endif
    ...
    if (SOC_HW_ACCESS_DISABLE(unit)) {
        ...
        if (SOC_IS_KATANA2(unit)) {
            ...
            if (rev_id == BCM56450_B1_REV_ID) {
                ...
                /* Always allow access to ING_Q_BEGIN register so that
                 * protection can be enabled when accessing
                 * ING_PHYSICAL_PORT_TABLE table */
                if (msg->header.v3.opcode == WRITE_REGISTER_CMD_MSG &&
                        msg->header.v3.dst_blk == kt2_ing_pipe_blk &&
                        msg->writecmd.address == kt2_addr_ING_Q_BEGINr) {
                    return FALSE;
                } else if (msg->header.v3.opcode == READ_REGISTER_CMD_MSG &&
                        msg->header.v3.dst_blk == kt2_ing_pipe_blk &&
                        msg->readcmd.address == kt2_addr_ING_Q_BEGINr) {
                    return FALSE;
                }
            }
        }
                           ...
                           int dwc_read, int *rv)
{
    *rv = SOC_E_NONE;
    ...
    if (LOG_CHECK(BSL_LS_SOC_SCHAN | BSL_VERBOSE)) {
        ...
        soc_schan_dump(unit, msg, dwc_write, BSL_VERBOSE);
    }
...
/*******************************************
* @function _soc_schan_stat_schan_op_inc
* @Purpose increment schan_op statistic counter.
*
* @param unit [in] unit
*
* @comments this function is not thread safe and is excluded from TSAN validation
*
* @returns None
*
* @end
********************************************/
STATIC void
_soc_schan_stat_schan_op_inc(int unit)
{
...
{
    if (*rv != SOC_E_TIMEOUT) {
        _soc_schan_stat_schan_op_inc(unit);
        return FALSE;
    }
