...
/*
 * Function:
 *      cmicd_dma_chan_restart
 * Purpose:
 *      Restart the CMICd DMA channel for the specified SOC unit.
 * Parameters:
 *      unit - SOC unit #
 ...
 */
static int
cmicd_dma_chan_restart(int unit, sdc_t *sc)
{
    return (soc_cmicm_dma_drv[unit].chan_restart(unit, sc));
}
 ...
 */
static int
cmicd_dma_pci_timeout_handle(int unit, int cmc)
{
    return (soc_cmicm_dma_drv[unit].pci_timeout_handle(unit, cmc));
}
...
soc_cmicd_dma_drv_init(int unit, soc_cmic_dma_drv_t *drv)
{
    ...
    drv->chan_restart               = cmicd_dma_chan_restart;
