                                ...
                                soc_sbusdma_desc_ctrl_t *ctrl, int append, uint8 desc_host_addr)
{
    ...
    if (ctrl->cfg_count == 1) {
        ...
        if (append) {
            ...
            if (soc_feature(unit, soc_feature_xflow_macsec_isec_sa_table_corruption_war) &&
                    (ctrl->flags & SOC_SBUSDMA_CFG_SSRAM_MEMORY)) {
                if (desc_host_addr)
                {
                    desc_build.hostaddrLo = PTR_TO_INT(ctrl->buff);
                    desc_build.hostaddrHi = 0x0;
                }
                ctrl->buff =  (void *)(hptr) + (cfg->count * cfg->width * sizeof(uint32));
            } else
#endif
            {
                if (desc_host_addr)
    ...
    if (!(ctrl->flags & SOC_SBUSDMA_CFG_DATA_BIGENDIAN))
    {
                        ...
                        soc_sbusdma_desc_cfg_t *cfg, uint8 alloc_memory ,void **desc)
{
    ...
    if (ctrl->cfg_count == 1) { /* single mode */
        ...
        if (alloc_memory)
        {
            ...
            if (soc_feature(unit, soc_feature_xflow_macsec_isec_sa_table_corruption_war) &&
                    (ctrl->flags & SOC_SBUSDMA_CFG_SSRAM_MEMORY)) {
                dma_desc = sal_alloc(sizeof(soc_sbusdma_desc_t) *
                        ctrl->cfg_count, "soc_sbusdma_desc_t");
                if (dma_desc == NULL) {
                    return SOC_E_MEMORY;
                }
                sram_addr = soc_iproc_m0ssq_sram_alloc(unit,
                        ctrl->cfg_count * sizeof(soc_sbusdma_desc_cfg_t),
                        "SocHr4MacsecWarCfg");
                if (!sram_addr) {
                    sal_free(dma_desc);
                    return SOC_E_MEMORY;
        ...
        for (count = 0; count < ctrl->cfg_count; count++) {
            ...
            /* Write the config descriptor in Shared SRAM */
#if defined(BCM_HURRICANE4_SUPPORT) && defined(INCLUDE_XFLOW_MACSEC)
            if (soc_feature(unit, soc_feature_xflow_macsec_isec_sa_table_corruption_war) &&
                    (ctrl->flags & SOC_SBUSDMA_CFG_SSRAM_MEMORY)) {
                soc_cm_iproc_write(unit, temp_addr, dma_desc[count].cntrl);
                temp_addr += 4;
                soc_cm_iproc_write(unit, temp_addr, dma_desc[count].req);
                temp_addr += 4;
                soc_cm_iproc_write(unit, temp_addr, dma_desc[count].count);
                temp_addr += 4;
                soc_cm_iproc_write(unit, temp_addr, dma_desc[count].opcode);
                temp_addr += 4;
                soc_cm_iproc_write(unit, temp_addr, dma_desc[count].addr);
                temp_addr += 4;
                soc_cm_iproc_write(unit, temp_addr, dma_desc[count].hostaddrLo);
                temp_addr += 4;
                soc_cm_iproc_write(unit, temp_addr, dma_desc[count].hostaddrHi);
                temp_addr += 4;
                soc_cm_iproc_write(unit, temp_addr, dma_desc[count].rsrv);
                temp_addr += 4;
            }
#endif
        }
...
_cmicx_sbusdma_wait(int unit, int cmc, int ch, int interval, uint8 intrEnb)
{
    ...
    if (rv != SOC_E_NONE) {
        if (rv != SOC_E_TIMEOUT) {
            ...
            cmicx_sbusdma_curr_op_details(unit, cmc, ch, 1);
        } else { /* Internal SW Timeout cleanup */
            soc_timeout_t to;
            SOC_CONTROL(unit)->stat.err_sbusdma_sw_timeout++;
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
                                  "%s timeout\n"),
                       intrEnb ?
                       "interrupt" : "polling"));
            cmicx_sbusdma_curr_op_details(unit, cmc, ch, 0);
            /* Abort DMA */
            ...
            while (1) {
                ...
                if (soc_timeout_check(&to)) {
                    ...
                    sal_dpc(cmicx_sbusdma_event_dpc, INT_TO_PTR(unit),
...
#if defined(BCM_HURRICANE4_SUPPORT) && defined(INCLUDE_XFLOW_MACSEC)
    if (soc_feature(unit, soc_feature_xflow_macsec_isec_sa_table_corruption_war) &&
            (swd->ctrl.flags & SOC_SBUSDMA_CFG_SSRAM_MEMORY)) {
