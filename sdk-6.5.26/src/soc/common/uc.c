...
#define PCIEFW_PENHOLDER_ADDR       0xffff0fe8
#define PCIEFW_RESET_TIMEOUT        100       /* milliSeconds */

/* Time of Day message buffer pointer */
uint32 *tod_buf_ptr;
...
soc_uc_mem_read(int unit, uint32 addr)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        if ((addr >= 0x01100000) && (addr < 0x01200000)) {
            /* mHost TCM. use BAR1 subwindow 2 directly */
            bar1_addr = (addr & 0x000fffff) | 0x00200000;
            return CMREAD(unit, bar1_addr);
        } else if ((addr >= 0x01200000) && (addr < 0x01300000)) {
            /* RTS SRAM. use BAR1 subwindow 1 directly */
            bar1_addr = (addr & 0x000fffff) | 0x00100000;
            return CMREAD(unit, bar1_addr);
        }
     }
#endif
...
soc_uc_addr_to_pcie(int unit, int uC, uint32 addr)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
        uint32 rts_no = uC >> 1;
        uint32 abase = 0x50800000 + (0x1000000 * rts_no) + (0x60000 * (uC & 1));
        uint32 sbase = 0x50900000 + (0x1000000 * rts_no);
        ...
        if (addr < 0x40000) {
            ...
            return abase + addr - 0x20000;
        } else if (addr < 0x900000) {
            assert(0);
        } else if (addr < 0xb00000) {
            /* SRAM address from host perspective*/
            return sbase + addr - 0x900000;
        }
        return addr;
    }
#endif
 ...
 */
{
    ...
    /* Free the tod message buffer */
    if (tod_buf_ptr != NULL) {
        soc_cm_sfree(unit, tod_buf_ptr);
    }
...
soc_uc_mhost_reset(int unit, int uC)
{
    ...
    defined(BCM_DNX_SUPPORT) || defined(BCM_FIRELIGHT_SUPPORT) || defined(BCM_FIREBOLT6_SUPPORT)
    uint32 strap_status, i;
#endif
    ...
    if ((uC == 0) &&
        ...
        if (strap_status != 0) {
            ...
            soc_reg_field_set(unit, MHOST_0_CR5_RST_CTRLr, &rst_control,
                              CPU_HALT_Nf, 1);
            /* Write a pattern to the penholder location.
             * When the Flash FW gets into the penholder loop, it writes a
             * Valid address there
             */
            soc_uc_mem_write(unit, PCIEFW_PENHOLDER_ADDR, 0xabababab);

            sal_msleep(1);
            WRITE_MHOST_0_CR5_RST_CTRLr(unit, rst_control);
            i = PCIEFW_RESET_TIMEOUT;
            do {
                sal_msleep(1);
                if (soc_uc_mem_read(unit, PCIEFW_PENHOLDER_ADDR) != 0xabababab) {
                    break;
                }
                i--;
            } while (i > 0);
            ...
            soc_reg_field_set(unit, MHOST_0_CR5_RST_CTRLr, &rst_control,
                              CPU_HALT_Nf, 0);
            /* Ensure that the uKernel FW is invalidated. his must be done
             * only in the newer FW that supports configurability feature
             */
            ...
            if (i == 0) {
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      "uC %d PCIe FW Loader did not reach penholder loop \n"), uC));
                WRITE_MHOST_0_CR5_RST_CTRLr(unit, rst_control);
    ...
    {
        if (uC == 0) {
            WRITE_IPROCPERIPH_UART0_UART_SRRr(unit,1);
        } else if (uC == 1) {
            WRITE_IPROCPERIPH_UART1_UART_SRRr(unit,1);
        }
    }
#endif
...
soc_uc_mhost_in_reset(int unit, int uC)
{
    ...
    defined(BCM_DNX_SUPPORT) || defined(BCM_FIRELIGHT_SUPPORT) || \
    defined(BCM_FIREBOLT6_SUPPORT)
        if (SOC_IS_TRIDENT3(unit) || SOC_IS_TOMAHAWK2(unit) || SOC_IS_TOMAHAWK3(unit) ||
             SOC_IS_MAVERICK2(unit) || SOC_IS_DNX(unit) || SOC_IS_FIRELIGHT(unit) ||
             SOC_IS_FIREBOLT6(unit)) {
            addr = soc_uc_addr_to_pcie(unit, uC, 0x80);
            if (soc_uc_mem_read(unit, addr) != 0xbadc0de1) {
                /* uC is not in reset, but no valid uKernel FW */
                return 2;
            }
        }
#endif
...
soc_uc_mhost_prestart_config(int unit, int uC)
{
    ...
    if (config_ver >= 2) {
        ...
        /* Config SRAM @ 0x98-0xa0 */
#if defined(BCM_DNX_SUPPORT)
        if (soc_feature(unit, soc_feature_iproc_17)) {
            uint32 sram_addr = 0;
            uint32 sram_size = 0;
            uint32 sram_uncache_size = 0;

            /* 2MB per RTS0/RTS1, allocate 1MB per core */
            sram_size = 0x100000;       /* 1MB per core */
            if ((uC == 1) || (uC == 3)) {
                sram_addr = 0xA00000;
            } else {
                sram_addr = 0x900000;
            ...
            /* Last 8KB of SRAM @ RTS1 is messaging area */
            if (uC == 3) {
                sram_size -= 0x2000;
            }

            /* SRAM address */
            addr = soc_uc_addr_to_pcie(unit, uC, 0x98);
            soc_uc_mem_write(unit, addr, sram_addr);

            /* SRAM size */
            addr = soc_uc_addr_to_pcie(unit, uC, 0x9c);
            soc_uc_mem_write(unit, addr, sram_size);

            /* SRAM size */
            addr = soc_uc_addr_to_pcie(unit, uC, 0xa0);
            soc_uc_mem_write(unit, addr, sram_uncache_size);
        }
#endif
    }
    return SOC_E_NONE;
...
soc_uc_mhost_start(int unit, int uC, uint32 addr)
{
    ...
    if (uC == 0) {
        ...
        if ((BCM56970_DEVICE_ID & 0xFFF0) == (dev_id & 0xFFF0)){
            soc_cm_iproc_write(unit, PCIEFW_PENHOLDER_ADDR, 0);
        }
#endif
...
#define READ_RTSX_MHOST_Y_CR5_RST_CTRLr(_u, _uc, _ctl)      \
    if ((_uc) == 0) {                                       \
        READ_RTS0_MHOST_0_CR5_RST_CTRLr((_u), (_ctl));      \
    } else if ((_uc) == 1) {                                \
        READ_RTS0_MHOST_1_CR5_RST_CTRLr((_u), (_ctl));      \
    } else if ((_uc) == 2) {                                \
        READ_RTS1_MHOST_0_CR5_RST_CTRLr((_u), (_ctl));      \
    } else if ((_uc) == 3) {                                \
        READ_RTS1_MHOST_1_CR5_RST_CTRLr((_u), (_ctl));      \
    }

#define WRITE_RTSX_MHOST_Y_CR5_RST_CTRLr(_u, _uc, _ctl)     \
    if ((_uc) == 0) {                                       \
        WRITE_RTS0_MHOST_0_CR5_RST_CTRLr((_u), (_ctl));     \
    } else if ((_uc) == 1) {                                \
        WRITE_RTS0_MHOST_1_CR5_RST_CTRLr((_u), (_ctl));     \
    } else if ((_uc) == 2) {                                \
        WRITE_RTS1_MHOST_0_CR5_RST_CTRLr((_u), (_ctl));     \
    } else if ((_uc) == 3) {                                \
        WRITE_RTS1_MHOST_1_CR5_RST_CTRLr((_u), (_ctl));     \
    }

static int
soc_uc_cmicx_v4_reset(int unit, int uC)
{
    uint32 rst_control = 0, strap_status;

    if (uC < 0 || uC >= SOC_INFO(unit).num_ucs) {
        return SOC_E_PARAM;
    }

    READ_RTSX_MHOST_Y_CR5_RST_CTRLr(unit, uC, &rst_control);

    /* Assert reset */
    soc_reg_field_set(unit, RTS0_MHOST_0_CR5_RST_CTRLr, &rst_control,
                      SYS_PORESET_Nf, 0);
    soc_reg_field_set(unit, RTS0_MHOST_0_CR5_RST_CTRLr, &rst_control,
                      RESET_Nf, 0);
    soc_reg_field_set(unit, RTS0_MHOST_0_CR5_RST_CTRLr, &rst_control,
    ...
    WRITE_RTSX_MHOST_Y_CR5_RST_CTRLr(unit, uC, rst_control);
    sal_msleep(20); /* Wait for reset Assertion */
    ...
    if (uC == 0) {
        ...
        READ_RTS0_MHOST_0_MHOST_STRAP_STATUSr(unit, &strap_status);
        if (strap_status != 0) {
...
static int
soc_uc_cmicx_v4_in_reset(int unit, int uC)
{
    uint32 rst_control = 0, halt, strap_status, addr;
    ...
    halt = soc_reg_field_get(
                unit, RTS0_MHOST_0_CR5_RST_CTRLr, rst_control, CPU_HALT_Nf);
...
static int
soc_uc_cmicx_v4_start(int unit, int uC, uint32 addr)
{
...
static int
soc_uc_cmicx_v4_preload(int unit, int uC)
{
    COMPILER_REFERENCE(unit);
    COMPILER_REFERENCE(uC);
    return (SOC_E_NONE);
...
#if defined(BCM_CMICX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
        return soc_uc_cmicx_v4_in_reset(unit, uC);
    }
#endif /* BCM_CMICX_SUPPORT */
#if defined(BCM_UC_MHOST_SUPPORT)
    if (soc_feature(unit, soc_feature_uc_mhost)) {
...
#if defined(BCM_CMICX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
        return soc_uc_cmicx_v4_preload(unit, uC);
    }
#endif /* BCM_CMICX_SUPPORT */
#if defined(BCM_UC_MHOST_SUPPORT)
    if (soc_feature(unit, soc_feature_uc_mhost)) {
...
#if defined(BCM_CMICX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
        return soc_uc_cmicx_v4_reset(unit, uC);
    }
#endif /* BCM_CMICX_SUPPORT */
#if defined(BCM_UC_MHOST_SUPPORT)
    if (soc_feature(unit, soc_feature_uc_mhost)) {
...
#if defined(BCM_CMICX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
        return soc_uc_cmicx_v4_start(unit, uC, addr);
    }
#endif /* BCM_CMICX_SUPPORT */
#if defined(BCM_UC_MHOST_SUPPORT)
    if (soc_feature(unit, soc_feature_uc_mhost)) {
...
/*
 * Function: soc_uc_timeofday_set
 * Purpose:	Set timeofday in uKernel.
 * Parameters:	unit - unit number
 *      uC - microcontroller num
 *      timeofday - Time of the day in nanoseconds.
 *      ts - Hardware TS counter in nanoseconds.
 * Returns:	SOC_E_XXX
 *     SOC_E_UNAVAIL : SoC doesn't have uC / MCS support.
 *     SOC_E_PARAM : uC number given is invalid for this SoC
 *     SOC_E_DISABLED : ARM core is in reset.
 ...
 *     SOC_E_NONE : Successful.
 */
int soc_uc_timeofday_set(int unit, int uC, uint64 timeofday, uint64 ts, sal_usecs_t timeout)
{
    mos_msg_data_t send;
    ...
    if (tod_buf_ptr == NULL) {
        tod_buf_ptr = soc_cm_salloc(unit, len, "Tod set buffer");
    }

    if (tod_buf_ptr) {
        buf_paddr = soc_cm_l2p(unit, tod_buf_ptr);
        soc_cm_sinval(unit, tod_buf_ptr, len);

        tod_buf_ptr[0] = COMPILER_64_LO(timeofday);
        tod_buf_ptr[1] = COMPILER_64_HI(timeofday);
        tod_buf_ptr[2] = COMPILER_64_LO(ts);
        tod_buf_ptr[3] = COMPILER_64_HI(ts);

        send.s.mclass = MOS_MSG_CLASS_SYSTEM;
        send.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_TOD_SET;
...
/*
 * Function: soc_uc_timeofday_get
 * Purpose:	Get timeofday in uKernel.
 * Parameters:	unit - unit number
 *      uC - microcontroller num
 *      timeofday - Time of the day in nanoseconds.
 * Returns:	SOC_E_XXX
 ...
 *     SOC_E_DISABLED : ARM core is in reset.
 *     SOC_E_INIT : uKernel messaging is not initialized.
 *     SOC_E_TIMEOUT : timed out (uKernel not responding).
 *     SOC_E_NONE : Successful.
 */
int soc_uc_timeofday_get(int unit, int uC, uint64 *tod, sal_usecs_t timeout)
{
    int rc = SOC_E_NONE;
    sal_paddr_t buf_paddr;
    mos_msg_data_t send, reply;
    uint32 *tod_ns = (uint32 *)tod;
    uint32 *tod_ns_ptr;
    int len = 8;
    ...
    tod_ns_ptr = soc_cm_salloc(unit, len, "Tod get buffer");

    if (tod_ns_ptr) {
        buf_paddr = soc_cm_l2p(unit, tod_ns_ptr);
        soc_cm_sinval(unit, tod_ns_ptr, len);

        send.s.mclass = MOS_MSG_CLASS_SYSTEM;
        send.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_TOD_GET;
        send.s.len = soc_htons(len);
        send.s.data = soc_htonl(PTR_TO_INT(buf_paddr));
        rc = soc_cmic_uc_msg_send(unit, uC, &send, 5 * 1000 * 1000);
        if (rc != SOC_E_NONE) {
            soc_cm_sfree(unit, tod_ns_ptr);
            return rc;
        }
        rc = soc_cmic_uc_msg_receive(unit, uC, MOS_MSG_CLASS_TOD,
                                     &reply, 5 * 1000 * 1000);
        if (rc != SOC_E_NONE) {
            LOG_VERBOSE(BSL_LS_SOC_COMMON,
                        (BSL_META_U(unit,
                                    "uC msg recieve failed to uc %d.\n"), uC));
            soc_cm_sfree(unit, tod_ns_ptr);
            return rc;
        }
        tod_ns[0] = tod_ns_ptr[0];
        tod_ns[1] = tod_ns_ptr[1];
    }
    return rc;
}
...
#if defined(BCM_CMICX_SUPPORT)
/*
 * Function: 	soc_uc_m0_status_version
 * Purpose:	Return Status and firmware version of uC
 * Parameters:	unit            - unit number
 *              uC              - core number
 *              status          - 0|1. 1 - OK
 *              version         - Version (which is uKernel version)
 *              buf             - App type in string format ex: IFA-CC(String)
 *              max_buf_size    - Maximum buffer size
 * Returns:	SOC_E_XXX
 */
int soc_uc_m0_status_version(int unit, int uC, int *status,
                             uint32 *version, char *buf, uint32 max_buf_size)
{
    int reset, rv = SOC_E_PARAM;
    uint32 addr, eapps;

    if (soc_iproc_m0ssq_reset_ucore_get(unit, uC, &reset)) {
        return rv;
    }
    ...
    if (eapps == 0xBADC0DE1) {
#if defined(INCLUDE_IFA)
        uint32        offset, app_status[2], flag = 0;
        ...
        flag = soc_cm_iproc_read(unit,
                (offset + ifa_cc_offset(flag)));
        if ((app_status[0] == BCM_IFA_CC_APP_STATUS_1) &&
            (app_status[1] == BCM_IFA_CC_APP_STATUS_2)) {
            if (reset) {
                soc_uc_mem_write(unit,
                        (addr + BCM_IFA_CC_M0_EAPP_STATUS_OFFSET),
                        (uint32)0);
                soc_uc_mem_write(unit, (offset + ifa_cc_offset(app_status)),
                        (uint32)0);
                soc_uc_mem_write(unit, (offset + ifa_cc_offset(app_status) + 4),
                        (uint32)0);
                return rv;
            }
            *status = 1;
            *version = soc_cm_iproc_read(unit, (addr + BCM_IFA_CC_M0_EAPP_VERSION_OFFSET));
            if (flag & BCM_IFA_CC_IFA2_FLAG) {
                if (max_buf_size > sal_strlen("IFA2-CC")) {
                    sal_strcpy(buf, "IFA2-CC");
                } else {
                    buf[0] = '\0';
                }
            } else {
                if (max_buf_size > sal_strlen("IFA-CC")) {
                    sal_strcpy(buf, "IFA-CC");
                } else {
                    buf[0] = '\0';
                }
            }
...
/*
 * Function: 	soc_uc_m0_reset
 * Purpose:	Put an M0 into reset state
 * Parameters:	unit            - unit number
 *              uC              - core number
 * Returns:	SOC_E_XXX
 */
int soc_uc_m0_reset(int unit, int uC)
{
    int reset, rv = SOC_E_PARAM;
    uint32 addr, eapps;
    ...
    if (eapps == 0xBADC0DE1) {
#if defined(INCLUDE_IFA)
        int                       i, time_count, uc_bit_map = 0;
        uint32                    offset, uc_status, app_status[2];
        ...
        if ((app_status[0] == BCM_IFA_CC_APP_STATUS_1) &&
            ...
            soc_uc_mem_write(unit, (offset + ifa_cc_offset(host_status)),
                             (uint32)BCM_IFA_CC_INIT_STATUS_BIT);

            time_count = 0;
            while (1) {
                uc_status  = soc_uc_mem_read(unit,
                        (offset + ifa_cc_offset(uc_status)));
                if (!(uc_status & BCM_IFA_CC_CONFIG_STATUS_BIT)) {
                    break;
                }
                ++time_count;
            ...
            for (i = 0; i < BCM_IFA_CC_M0_NUM_UCS; i++) {
                ...
                addr = soc_iproc_percore_membase_get(unit, i);
                offset     = (addr + BASEADDR_OFFSET(unit));

                app_status[0] = soc_uc_mem_read(unit,
                                    (offset + BCM_IFA_CC_M0_EAPP_STATUS_OFFSET));
            ...
            if (uc_bit_map &&
                (!(bcm_ifa_cc_config_get(unit, &config_data)))) {
                (void)bcm_ifa_cc_config_set(unit, &config_data);
            }
            rv = SOC_E_NONE;
        }
#endif /* INCLUDE_IFA */
