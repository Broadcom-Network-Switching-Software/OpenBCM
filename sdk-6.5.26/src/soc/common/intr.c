...
soc_intr(void *_unit)
{
    ...
    if (SOC_FLAGS_GET(unit) & SOC_F_BUSY) {
        return;
    }
...
void soc_cmn_error(void *unit_vp, void *d1, void *d2, void *d3, void *d4)
{
    ...
    if (soc_feature(unit, soc_feature_cmicm)) {
        if (!(SOC_FLAGS_GET(unit) & SOC_F_POLLED) && (SOC_IS_ARAD(unit) || SOC_IS_FE3200(unit))) {
#ifdef BCM_CMICM_SUPPORT
            int cmc = SOC_PCI_CMC(unit);
...
int soc_interrupt_get(int unit, int block_instance , const soc_interrupt_db_t* inter, int* inter_val /*out*/)
{
    ...
    if (soc_dnxc_access_available(unit)) {
        return access_interrupt_get(unit, block_instance, inter, inter_val);
    }
#endif
...
#ifdef BCM_ACCESS_SUPPORT
    if (soc_dnxc_access_available(unit)) {
        return access_interrupt_force_get(unit, block_instance, inter, inter_val);
    }
#endif
...
#ifdef BCM_ACCESS_SUPPORT
    if (soc_dnxc_access_available(unit)) {
        return access_interrupt_update(unit, block_instance, inter, 1);
    }
#endif
...
#ifdef BCM_ACCESS_SUPPORT
    if (soc_dnxc_access_available(unit)) {
        return access_interrupt_force(unit, block_instance, inter, 0);
    }
#endif
...
#ifdef BCM_ACCESS_SUPPORT
    if (soc_dnxc_access_available(unit)) {
        return access_interrupt_update(unit, block_instance, inter, 0);
    }
#endif
...
int soc_interrupt_is_enabled(int unit, int block_instance, const soc_interrupt_db_t* inter, int* is_enabled /*out*/)
{
    ...
    if (soc_dnxc_access_available(unit))
    {
        return access_interrupt_is_enabled(unit, block_instance, inter, is_enabled);
    }
#endif
...
int soc_interrupt_is_supported(int unit, int block_instance, int inter_id)
{
    ...
    if (soc_dnxc_access_available(unit))
    {
        return access_interrupt_is_supported(unit, block_instance, inter_id);
    }
#endif
...
static int soc_interrupt_valid(int unit, int block_instance, int inter_id)
{
    ...
    if (soc_dnxc_access_available(unit))
    {
        return access_interrupt_is_valid(unit, block_instance, inter_id);
    }
#endif
...
#ifdef BCM_ACCESS_SUPPORT
    if (soc_dnxc_access_available(unit)){
        return access_interrupt_clear_on_write(unit, block_instance, interrupt_id);
    }
#endif
...
int soc_interrupt_counter_list_get(
    int unit,
    uint32 flags,
    int* intr_number,
    int* intr_list,
...
{
    int rc = SOC_E_NONE;
    int nof_interrupts, intr, intr_enable;
    int blk, port;
    int *blk_list;
    ...
    if ((intr_number == NULL) || (intr_list == NULL) || (count_list == NULL))
    {
        rc = SOC_E_PARAM;
        LOG_ERROR(BSL_LS_SOC_INTR, (BSL_META_U(unit, "%s\n"), soc_errmsg(rc)));
        return rc;
    ...
    for (intr = 0; intr < nof_interrupts; intr++)
    {
        ...
        SOC_BLOCKS_ITER(unit, blk, blk_list)
        {
            ...
            rc = soc_interrupt_is_enabled(unit, port, &(interrupts[intr]), &intr_enable);
            if (SOC_FAILURE(rc))
            {
                LOG_ERROR(BSL_LS_SOC_INTR, 
                          (BSL_META_U(unit, "%s\n"), soc_errmsg(rc)));
        ...
        {
            intr_list[number] = intr;
            count_list[number] = statistics_count;
            number++;
        }
    }
    *intr_number = number;
...
int soc_interrupt_clear_all(int unit, int skip){
...
#ifdef BCM_ACCESS_SUPPORT
    if (soc_dnxc_access_available(unit)) {
        return access_interrupt_clear_all(unit, skip);
    }
#endif
    ...
    for (bi_index = 0;  SOC_BLOCK_INFO(unit, bi_index).type >= 0; bi_index++ ) {
        for (inter = 0; inter < nof_interrupts ; inter++) {
            ...
            /* skip some interrupts from dnxdata */
#ifdef BCM_DNX_SUPPORT
            if (skip) {
                table_info = dnx_data_intr.general.skip_clear_on_init_info_get(unit);
                for (i = 0; i < table_info->key_size[0]; i++) {
                    if (inter == dnx_data_intr.general.skip_clear_on_init_get(unit, i)->intr) {
                        break;
                    }
                }
                /* interrupt clear skipped */
                if (i < table_info->key_size[0]) {
                    continue;
                }
            }
#endif
...
int
soc_interrupt_db_info_get(int unit, soc_interrupt_db_t **intr_db_p)
{
    int rc = SOC_E_NONE;
    ...
    *intr_db_p =  SOC_CONTROL(unit)->interrupts_info->interrupt_db_info;
    return rc;
}
...
soc_cmic_intr_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx_gen2)) {
        rv = access_cmicx_gen2_intr_init(unit, &_cmic_intr_op[unit]);
        return rv;
    }
#endif
