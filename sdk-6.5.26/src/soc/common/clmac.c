...
mac_cl_init(int unit, soc_port_t port)
{
    ...
    if (soc_feature(unit, soc_feature_eee)) {
        SOC_IF_ERROR_RETURN
            (soc_port_eee_timers_init(unit, port, si->port_speed_max[port]));
    }
...
#ifdef BCM_FIRELIGHT_SUPPORT
    if (SOC_IS_FIRELIGHT(unit)) {
        /* Update the speed, used for LED indications and internal buffer sizing */
        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, CLMAC_MODEr, port, SPEED_MODEf, ((speed>= 10000) || (speed==0)) ? 4 : 2));
    }
#endif
                  ...
                  int value)
{
    ...
    case SOC_MAC_CONTROL_TX_SET:
        /* Disabling TX stops egress and hangs if CPU sends */
        SOC_IF_ERROR_RETURN(READ_CLMAC_CTRLr(unit, port, &rval));
        copy = rval;
        soc_reg64_field32_set(unit, CLMAC_CTRLr, &rval, TX_ENf, value ? 1 : 0);
        if (COMPILER_64_NE(rval, copy)) {
            SOC_IF_ERROR_RETURN(WRITE_CLMAC_CTRLr(unit, port, rval));
        }
        break;
    ...
    case SOC_MAC_CONTROL_FAULT_REMOTE_TX_FORCE_ENABLE:
        if (soc_reg_field_valid(unit, CLMAC_RX_LSS_CTRLr, FAULT_SOURCE_FOR_TXf) &&
            soc_reg_field_valid(unit, CLMAC_RX_LSS_CTRLr, FORCE_REMOTE_FAULT_OSf)) {
            SOC_IF_ERROR_RETURN(READ_CLMAC_RX_LSS_CTRLr(unit, port, &rval));
            soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval,
                                  FAULT_SOURCE_FOR_TXf, (value ? 0x2 : 0));
            soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval,
                                   FORCE_REMOTE_FAULT_OSf, (value ? 1 : 0));
            SOC_IF_ERROR_RETURN(WRITE_CLMAC_RX_LSS_CTRLr(unit, port, rval));
        } else {
            return SOC_E_UNAVAIL;
        }
                  ...
                  int *value)
{
    ...
    case SOC_MAC_CONTROL_TX_SET:
        SOC_IF_ERROR_RETURN(READ_CLMAC_CTRLr(unit, port, &rval));
        *value = soc_reg64_field32_get(unit, CLMAC_CTRLr, rval, TX_ENf);
        break;
    ...
    case SOC_MAC_CONTROL_FAULT_REMOTE_TX_FORCE_ENABLE:
        if (soc_reg_field_valid(unit, CLMAC_RX_LSS_CTRLr, FAULT_SOURCE_FOR_TXf) &&
            soc_reg_field_valid(unit, CLMAC_RX_LSS_CTRLr, FORCE_REMOTE_FAULT_OSf)) {
            SOC_IF_ERROR_RETURN(READ_CLMAC_RX_LSS_CTRLr(unit, port, &rval));
            if (soc_reg64_field32_get(unit, CLMAC_RX_LSS_CTRLr, rval, FORCE_REMOTE_FAULT_OSf)) {
                *value = (soc_reg64_field32_get(unit, CLMAC_RX_LSS_CTRLr, rval, FAULT_SOURCE_FOR_TXf) ==  0x2) ? 1 : 0;
            } else {
                *value = 0;
            }
        } else {
