...
soc_knet_cmd_req(kcom_msg_t *kmsg, unsigned int len, unsigned int buf_size)
{
    ...
    if (rv < 0) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META("soc_knet_cmd_req: command failed\n")));
    } else if (rv > 0) {
        /* Synchronous response - no need to wait */
    } else if (sal_sem_take(ctrl->cmd_done, 2000000)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META("soc_knet_cmd_req: command timeout\n")));
        rv = SOC_E_TIMEOUT;
    } else {
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
...
soc_knet_rx_thread(void *context)
{
    ...
    while (knet_rx_thread_run == KNET_RX_THREAD_RUNNING) {
        ...
        if ((len = KNET_RECV(KCOM_CHAN_KNET, &kmsg, sizeof(kmsg))) < 0) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META("KNET RX RECV error - thread aborting "
                                "[state = %d]\n"), len));
            LOG_ERROR(BSL_LS_SOC_COMMON,
...
soc_knet_dune_info_init(int unit, kcom_msg_hw_init_t *kmsg)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        if (system_headers_mode == dnx_data_headers.system_headers.system_headers_mode_jericho_get(unit)) {
            ...
            /* No LIF extension */
            sal_memset(kmsg->pph_lif_ext_size, 0, sizeof(kmsg->pph_lif_ext_size));
        }
        else {
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        kmsg->oam_dm_tod_exist = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_dm_tod_msb_add_enable", SOC_IS_JERICHO_PLUS_AND_BELOW(unit));
    }
#endif
...
soc_knet_hw_init(int unit)
{
    ...
    kmsg.unet_channels = SOC_CONTROL(unit)->soc_unet_channels;
...
soc_knet_check_version(int unit)
{
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META("KNET check version fail rv %d version %d "
                            "netif_max %d filter_max %d\n"),
                  rv, kmsg.version, kmsg.netif_max, kmsg.filter_max));
    }
    return rv;
}
...
#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        soc_iproc_setreg(unit, addr, mask);
    } else
#endif
    {
...
soc_knet_cleanup(void)
{
    ...
    if (knet_chan != NULL) {
        if (knet_rx_thread_run == KNET_RX_THREAD_RUNNING) {
            ...
            if (retry <= 0) {
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META("KNET Rx thread did not exit after 5 "
                                    "seconds [state = %d]\n"),
                          knet_rx_thread_run));
            }
        }
...
soc_knet_init(int unit)
{
    ...
    if (knet_rx_thread_run != KNET_RX_THREAD_STOPPED) {
        ...
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,"knet reprobe failed rv %d\n"), rv));
            return SOC_E_FAIL;
        }
        return SOC_E_NONE;
    }
    ...
    if (ctrl->cmd_lock == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,"knet cmd_lock multex create failed\n")));
        soc_knet_cleanup();
        return SOC_E_RESOURCE;
    }
    ...
    if (ctrl->msg_lock == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,"knet msg_lock multex create failed\n")));
        soc_knet_cleanup();
        return SOC_E_RESOURCE;
    }
    ...
    if (ctrl->cmd_done == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,"knet cmd_done sem create failed\n")));
        soc_knet_cleanup();
        return SOC_E_RESOURCE;
    }
    ...
    if (knet_chan == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "knet open failed\n")));
        soc_knet_cleanup();
        return SOC_E_FAIL;
    }
    ...
    if (rv < 0) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "knet init failed rv %d\n"), rv));
        soc_knet_cleanup();
        return SOC_E_FAIL;
    }
    ...
    if (rxthr == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "knet rx thread create failed\n")));
        soc_knet_cleanup();
        return SOC_E_FAIL;
    }
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "knet version check failed rv %d\n"),rv));
        soc_knet_cleanup();
        return SOC_E_FAIL;
    }
