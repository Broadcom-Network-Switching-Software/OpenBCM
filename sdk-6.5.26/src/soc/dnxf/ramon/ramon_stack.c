    ...
    bcm_module_t module)
{
    ...
    field_val =
        soc_reg_above_64_field32_get(unit, RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, reg_val_above_64,
                                     MAX_BASE_INDEXf);
    if ((module / dnxf_data_fabric.topology.all_reachable_fap_group_size_get(unit)) > field_val)
    {
    ...
    bcm_module_t module)
{
    ...
    SHR_IF_ERR_EXIT(soc_ramon_stk_module_max_all_reachable_verify(unit, module));

    if (module == (dnxf_data_fabric.topology.nof_system_modid_get(unit) - 1))
    {
        fap_group_num = module / dnxf_data_fabric.topology.all_reachable_fap_group_size_get(unit);
    }
    else
    {
        fap_group_num = module / dnxf_data_fabric.topology.all_reachable_fap_group_size_get(unit) - 1;
    }

    SHR_IF_ERR_EXIT(READ_RTP_ALL_REACHABLE_AND_GROUP_REACHABLE_CFGr(unit, &reg_val32));
    soc_reg_field_set(unit, RTP_ALL_REACHABLE_AND_GROUP_REACHABLE_CFGr, &reg_val32, ALRC_MAX_BASE_INDEXf,
                      fap_group_num);
    ...
    bcm_module_t * module)
{
    ...
    mod = dnxf_data_fabric.topology.all_reachable_fap_group_size_get(unit) * (field_val + 1);
    if (mod != dnxf_data_fabric.topology.nof_system_modid_get(unit))
    {
        *module = mod;
    }
    else
    ...
    bcm_module_t module)
{
    ...
    SHR_IF_ERR_EXIT(READ_RTP_ALL_REACHABLE_AND_GROUP_REACHABLE_CFGr(unit, &reg_val32));
    field_val = soc_reg_field_get(unit, RTP_ALL_REACHABLE_AND_GROUP_REACHABLE_CFGr, reg_val32, ALRC_MAX_BASE_INDEXf);
    if (field_val > (module / dnxf_data_fabric.topology.all_reachable_fap_group_size_get(unit)))
    {
        SHR_IF_ERR_EXIT(soc_ramon_stk_module_max_all_reachable_set(unit, module));
        LOG_WARN(BSL_LS_SOC_STK,
    ...
    bcm_module_t module)
{
    ...
    SHR_IF_ERR_EXIT(soc_ramon_stk_module_max_fap_verify(unit, module));
    ...
    soc_reg_above_64_field32_set(unit, RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, reg_val_above_64,
                                 UPDATE_BASE_INDEXf, fap_group_num);
    ...
    for (i = 0; i < nof_qrh; ++i)
    {
        ...
        soc_reg_field_set(unit, QRH_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, &reg_val32, UPDATE_BASE_INDEXf,
                          fap_group_num);
    ...
    for (i = module + 1; i < dnxf_data_fabric.topology.nof_system_modid_get(unit); ++i)
    {
    ...
    soc_reg_above_64_field32_set(unit, RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, reg_val_above_64,
                                 MAX_BASE_INDEXf, fap_group_num);
    SHR_IF_ERR_EXIT(WRITE_RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr(unit, reg_val_above_64));

    for (i = 0; i < nof_qrh; ++i)
    {
        SHR_IF_ERR_EXIT(soc_reg32_get(unit, QRH_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, i, 0, &reg_val32));
        soc_reg_field_set(unit, QRH_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, &reg_val32, MAX_BASE_INDEXf,
                          fap_group_num);
        SHR_IF_ERR_EXIT(soc_reg32_set(unit, QRH_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, i, 0, reg_val32));
    }
    ...
    int arg)
{
    ...
    for (i = 0; i < dnxf_data_fabric.topology.all_reachable_max_nof_ignore_faps_get(unit); i++)
    {
