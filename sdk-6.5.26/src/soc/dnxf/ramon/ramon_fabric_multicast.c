...
#define DNXF_RAMON_MULTICAST_SBUSDMA_RESERVED_CHANNEL 3
    ...
    uint32 *entry_array)
{
    ...
    {
        rv = cmicx_sbusdma_vchan_to_cmc_ch(unit, DNXF_RAMON_MULTICAST_SBUSDMA_RESERVED_CHANNEL, &cmc, &ch);
        SHR_IF_ERR_EXIT(rv);
    }
    ...
    rv = soc_mem_array_write_range_multi_cmc(unit, mem_flags, RTP_MULTI_CAST_TABLE_UPDATEm, 0, MEM_BLOCK_ALL, group_min,
                                             group_max, entry_array, DNXF_RAMON_MULTICAST_SBUSDMA_RESERVED_CHANNEL);
    SHR_IF_ERR_EXIT(rv);
    ...
    uint32 *entry_array)
{
    ...
    rv = soc_mem_array_read_range(unit, RTP_MULTI_CAST_TABLE_UPDATEm, 0, MEM_BLOCK_ALL, group_min, group_max,
                                  entry_array);
    SHR_IF_ERR_EXIT(rv);
    ...
    int unit)
{
    ...
    if (!SAL_BOOT_PLISIM)
    {
        ...
        SHR_IF_ERR_EXIT(sand_fill_partial_table_with_entry
                        (unit, RTP_MULTI_CAST_TABLE_UPDATEm, 0, 0, MEM_BLOCK_ALL, 0, mc_table_size - 1, data));
    ...
    soc_dnxf_fabric_priority_t prio)
{
    ...
    switch (prio)
    {
        case soc_dnxf_fabric_priority_invalid:
    ...
    soc_dnxf_fabric_priority_t * prio)
{
    ...
    {
        case 0x0:
            *prio = soc_dnxf_fabric_priority_invalid;
            break;
        case 0x1:
    ...
    int *local_route_enable_array)
{
    ...
    if (!flags || (flags & _SHR_FABRIC_MULTICAST_SET_ONLY))
    {
        ...
        rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_multicast_multi_read_range,
                                   (unit, index_min, index_max, ramon_multi_entry_array));
        SHR_IF_ERR_EXIT(rv);
    ...
    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_multicast_multi_write_range,
                               (unit, mem_flags, index_min, index_max, ramon_multi_entry_array));
    SHR_IF_ERR_EXIT(rv);
    ...
    int *local_route_enable_array)
{
    ...
    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_multicast_multi_read_range,
                               (unit, index_min, index_max, entry_array));
    SHR_IF_ERR_EXIT(rv);
    ...
    int fap_modid_offset)
{
    ...
    uint32 fap_id, nof_links, local_modid, nof_system_modid;
    ...
    if (nof_mc_groups_created > 0)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     "modid offset configuration needs to be set before creating the first multicast group. %d multicast groups were already created.",
    ...
    if ((fap_modid_offset < 0) || (fap_modid_offset > (nof_system_modid - nof_links)))
    {
