...
STATIC int
_soc_ramon_cosq_pipe_weight_validate(
    int unit,
    int pipe,
...
STATIC void
soc_ramon_cosq_pipe_weight_regs_info_fill(
    int unit,
    ...
    int weight)
{
    ...
    else
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping,
                                   (unit, port, &instance, &inner_link, info.soc_block));
        SHR_IF_ERR_EXIT(rc);
        loop_start = instance;
        loop_end = instance + 1;
    }
    ...
    if (mode == soc_dnxf_cosq_weight_mode_regular)
    {
        ...
        for (i = loop_start; i < loop_end; i++)
        {
            ...
            if (pipe == 0)
            {
                soc_reg_above_64_field32_set(unit, info.pipes_weights_register, reg_val_above_64, info.wfq_weight_p_0,
                                             weight);
            }
            else if (pipe == 1)
            {
                soc_reg_above_64_field32_set(unit, info.pipes_weights_register, reg_val_above_64, info.wfq_weight_p_1,
                                             weight);
            }
            else if (pipe == 2)
            {
                soc_reg_above_64_field32_set(unit, info.pipes_weights_register, reg_val_above_64, info.wfq_weight_p_2,
                                             weight);
            }
    ...
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_0)
    {
        for (i = loop_start; i < loop_end; i++)
        {
            ...
            if (pipe == 0)
            {
                soc_reg_field_set(unit, info.dyn_pipes_weights_register, &reg_val32, info.wfq_weight_p_0_p_0_gt_p_1,
                                  weight);
            }

            else if (pipe == 1)
            {
                soc_reg_field_set(unit, info.dyn_pipes_weights_register, &reg_val32, info.wfq_weight_p_1_p_0_gt_p_1,
                                  weight);
            }
    ...
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_1)
    {
        for (i = loop_start; i < loop_end; i++)
        {
            ...
            if (pipe == 0)
            {
                soc_reg_field_set(unit, info.dyn_pipes_weights_register, &reg_val32, info.wfq_weight_p_0_p_1_gt_p_0,
                                  weight);
            }

            else if (pipe == 1)
            {
                soc_reg_field_set(unit, info.dyn_pipes_weights_register, &reg_val32, info.wfq_weight_p_1_p_1_gt_p_0,
                                  weight);
            }
    ...
    int *weight)
{
    ...
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_0)
    {
        ...
        {
            *weight =
                soc_reg_field_get(unit, info.dyn_pipes_weights_register, reg_val32, info.wfq_weight_p_0_p_0_gt_p_1);
        }
        else if (pipe == 1)
        {
            *weight =
                soc_reg_field_get(unit, info.dyn_pipes_weights_register, reg_val32, info.wfq_weight_p_1_p_0_gt_p_1);
        }
    }
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_1)
    {
        ...
        {
            *weight =
                soc_reg_field_get(unit, info.dyn_pipes_weights_register, reg_val32, info.wfq_weight_p_0_p_1_gt_p_0);
        }
        else if (pipe == 1)
        {
            *weight =
                soc_reg_field_get(unit, info.dyn_pipes_weights_register, reg_val32, info.wfq_weight_p_1_p_1_gt_p_0);
        }
    }
