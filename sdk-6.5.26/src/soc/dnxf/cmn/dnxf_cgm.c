...
static _shr_error_t
soc_dnxf_cgm_register_field_max_value_get(
    int unit,
    soc_reg_t reg_name,
    soc_field_t field_name,
    int *max_value)
{
    ...
    *max_value = soc_reg_field_length(unit, reg_name, field_name);
        ...
        for (dim0 = start_index[0]; dim0 <= end_index[0]; dim0++)
        {
                 ...
                 dim1++)
            {
                ...
                if (set_or_get == _SHR_FABRIC_FLAG_SET)
                {
                    ...
                    soc_dnxf_cgm_register_field_max_value_get(unit, reg_name, field_name, &max_th_value);
                    ...
                    {
                        SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_set
                                        (unit, reg_name, field_name, j, profile_id, threshold_value_tmp));
                    }
                }
                else if (set_or_get == _SHR_FABRIC_FLAG_GET)
                {
                    SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_get
                                    (unit, reg_name, field_name, first_block, profile_id, threshold_value));
        ...
        for (dim0 = start_index[0]; dim0 <= end_index[0]; dim0++)
        {
            ...
            for (dim1 = start_index[1]; dim1 <= end_index[1]; dim1++)
            {
                ...
                if (set_or_get == _SHR_FABRIC_FLAG_SET)
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_set
                                        (unit, reg_name, field_name, j, profile_id, *threshold_value));
                    }
                }
                else if (set_or_get == _SHR_FABRIC_FLAG_GET)
                {
    ...
    for (dim0 = start_index[0]; dim0 <= end_index[0]; dim0++)
    {
        for (dim1 = start_index[1]; dim1 <= end_index[1]; dim1++)
        {
            ...
            if (set_or_get == _SHR_FABRIC_FLAG_SET)
            {
                ...
                SOC_PBMP_ITER(block_bmp, j)
                {
                    SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_set(unit, reg_name, field_name, j, dim1, *threshold_value));
                }
            }
            else if (set_or_get == _SHR_FABRIC_FLAG_GET)
            {
                SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_get
                                (unit, reg_name, field_name, first_block, dim1, threshold_value));
    ...
    int *threshold_value)
{
    ...
    for (dim1 = start_index[1]; dim1 <= end_index[1]; dim1++)
    {
        ...
        if (set_or_get == _SHR_FABRIC_FLAG_SET)
        {
            ...
            SOC_PBMP_ITER(block_bmp, j)
            {
                SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_set(unit, reg_name, field_name, j, 0, *threshold_value));
            }
        }
        else if (set_or_get == _SHR_FABRIC_FLAG_GET)
        {
            SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_get(unit, reg_name, field_name, first_block, 0, threshold_value));
    ...
    for (dim0 = start_index[0]; dim0 <= end_index[0]; dim0++)
    {
        ...
        for (dim1 = start_index[1]; dim1 <= end_index[1]; dim1++)
        {
            ...
            if (set_or_get == _SHR_FABRIC_FLAG_SET)
            {
                ...
                SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_set
                                (unit, reg_name, field_name, fmac_index, fmac_inner_link, *threshold_value));
            }
            else if (set_or_get == _SHR_FABRIC_FLAG_GET)
            {
                SHR_IF_ERR_EXIT(soc_dnxf_cgm_register_get
