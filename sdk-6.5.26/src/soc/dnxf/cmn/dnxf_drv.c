    ...
    int unit)
{
    ...
    soc->miimMutex = sal_mutex_create("MIIM");
    SHR_NULL_CHECK(soc->miimMutex, _SHR_E_MEMORY, "Failed to allocate MIIM lock");
    ...
    SHR_NULL_CHECK(soc->socControlMutex, _SHR_E_MEMORY, "failed to allocate socControlMutex");

    soc->counterMutex = sal_mutex_create("Counter");
    SHR_NULL_CHECK(soc->counterMutex, _SHR_E_MEMORY, "failed to allocate counterMutex");

    soc->schanMutex = sal_mutex_create("SCHAN");
    SHR_NULL_CHECK(soc->schanMutex, _SHR_E_MEMORY, "failed to allocate schanMutex");

    for (cmc = 0; cmc < SOC_PCI_CMCS_NUM(unit) + 1; cmc++)
    {
        soc->schanIntr[cmc] = sal_sem_create("SCHAN interrupt", sal_sem_BINARY, 0);
        SHR_NULL_CHECK(soc->schanIntr[cmc], _SHR_E_MEMORY, "failed to allocate schanSem");
    }

    soc->schan_wb_mutex = sal_mutex_create("SchanWB");
    SHR_NULL_CHECK(soc->schan_wb_mutex, _SHR_E_MEMORY, "Failed to allocate SchanWB");
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_mutexes_deinit(unit));
...
static shr_error_e
soc_dnxf_configure_soc_control(
    int unit)
{
    uint8 rev_id;
    uint16 dev_id;
    ...
    {
        uint32 use_new_access = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "use_new_access", 1);
        if (use_new_access != 0)
        {
            SHR_IF_ERR_EXIT(access_init(unit));
            if (use_new_access < 10)
            {
                SOC_CONTROL(unit)->device_access_runtime_info->flags |= ACCESS_RUNTIME_INFO_FLAGS_ALOW_MCM;
            }
        }
    }
#endif
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_configure_soc_control(unit));
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_mutexes_init(unit));
    ...
    int *block_id)
{
    ...
    _SHR_PBMP_FIRST(block_bmp, *block_id);
...
{
    int fmac_id, nof_fmacs_in_block, block_id, fmac_lane_iter, block_id_inner_link;
