...
static int is_mem_ecc_type_wide(int unit, uint32 mem)
{
    int rc = 0;
#ifdef BCM_ACCESS_SUPPORT
    if (SOC_IS_NEW_ACCESS_INITIALIZED(unit))
    {
        ...
        rm_info = ACCESS_GET_REGMEM_INFO_BY_GLOBAL_ID(runtime_info, mem);
        if (ACCESS_REGMEM_IS_ECC_TYPE_WIDE(rm_info))
        {
            rc = 1;
        }
    }
        ...
        uint32 *ecc)
{
    ...
    if (is_mem_ecc_type_wide(unit,mem))
    {
