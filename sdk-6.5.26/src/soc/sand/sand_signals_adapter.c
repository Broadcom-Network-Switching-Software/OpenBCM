...
#define ADAPTER_INFO_MDB_GET_MEM(mc_mem, mc_mem_length, mc_info)                                               \
    if(mc_mem_length > DSIG_MDB_MAX_SIZE_STR)                                                                  \
    {                                                                                                          \
        SHR_EXIT_WITH_LOG(_SHR_E_NOT_FOUND, "Signal size:%u is more than maximum supported:%u%s\n",            \
                                                mc_mem_length, DSIG_MAX_SIZE_STR, EMPTY);                      \
    }                                                                                                          \
    ...
    pp_block_t *cur_pp_block)
{
...
#endif
    {
        for(i_st = 0; i_st < cur_pp_block->stage_num; i_st++)
        {
            ...
            if(cur_pp_stage->stage_debug_signals[i_core] != NULL)
            {
                sal_memset(cur_pp_stage->stage_debug_signals[i_core], 0, sizeof(debug_signal_t) * DNX_MAX_NOF_DEBUG_SIGNALS);
                cur_pp_stage->stage_signal_num[i_core] = 0;
            }
        }
    }
    ...
    )
{
    ...
    debug_signal = &cur_pp_stage->stage_debug_signals[core][cur_pp_stage->stage_signal_num[core]++];
    ...
    char *res)
{
       ...
       )
    {
        ...
        else if (sal_strncmp(stage_name, "IPMF", 4) == 0 && sal_strncmp(dnx_data_adapter.mdb.lookup_caller_id_mapping_get(unit, interface_index, stage_and_mdb_key)->interface,
                "NotFound", sal_strlen("NotFound")) != 0)
        {
            sal_sprintf(res, "%s", dnx_data_adapter.mdb.lookup_caller_id_mapping_get(unit, interface_index, stage_and_mdb_key)->interface);
        }
    ...
    
    {
                        ...
                        "NotFound", sal_strlen("NotFound")) == 0)
        {
            ...
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "\n TEMP_COMMIT_PRINTING Signal interface not found. caller_id_key: %d stage_and_mdb_key: %d stage_name: %s stage_id: %d db_name: %s db_id: %d \n",
    ...
    pp_stage_t *cur_pp_stage)
{
    ...
    while ( data_index > 0)
    {
        ...
        for ( i = 0; i < nof_keys; i++ )
        {
            ...
            if ( skip_signal_flag == 0 )
            {
                SHR_IF_ERR_EXIT(add_stage_signal(unit, &app_db, 0, app_db_size, app_db_size, core,
            ...
            if ( skip_signal_flag == 0 )
            {
                SHR_IF_ERR_EXIT(add_stage_signal(unit, adapter_value, data_index, key_sizes[i], key_sizes[i], core,
        ...
        for ( i = 0; i < nof_data_payloads; i++ )
        {
            ...
            if (skip_signal_flag == 0)
            {
                ...
                SHR_IF_ERR_EXIT(add_stage_signal(unit, result_array, 0, result_size, result_size, core, cur_pp_stage->name,
        ...
        if ( skip_signal_flag == 0 )
        {
            SHR_IF_ERR_EXIT(add_stage_signal(unit, adapter_value, data_index, 8, 1, core,
        ...
        int flags)
{
    ...
    if (cur_pp_stage->stage_debug_signals[core] == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "cur_pp_stage->stage_debug_signals not allocated \n");
    }
    cur_pp_stage->stage_signal_num[core] = 0;
    ...
    if(cur_pp_stage->status & SIGNAL_STAGE_IS_SUBSTITUTE)
    {
        ...
        if (subst_pp_stage->stage_debug_signals[core] == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "subst_pp_stage->stage_debug_signals not allocated \n");
        }
        subst_pp_stage->stage_signal_num[core] = 0;
    }
    ...
    for(i_sig = 0; i_sig < adapter_info.nof; i_sig++)
    {
        ...
        if (strcmp(signal_name,"MDB_memory_access") == 0) {
            uint32 mdb_memory_access_value[DSIG_MDB_MAX_SIZE_UINT32];
            char mdb_signal_str[DSIG_MDB_MAX_SIZE_STR + 1];

            if(signal_size > DSIG_MDB_MAX_SIZE_BITS)
            {
                LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META("Signal:%s-%s-%s size:%d is more than maximum:%d\n"),
                                               block_n, cur_pp_stage->name, signal_name, signal_size, DSIG_MDB_MAX_SIZE_BITS));
                ADAPTER_INFO_SKIP_MEM((signal_size + 3) / 4 + 2, &adapter_info);
                continue;
            }

            sal_memset(mdb_memory_access_value, 0, sizeof(mdb_memory_access_value));
            sal_memset(mdb_signal_str, 0, DSIG_MDB_MAX_SIZE_STR + 1);
            ADAPTER_INFO_MDB_GET_MEM(mdb_signal_str, (signal_size + 3) / 4 + 2, &adapter_info);
            parse_long_integer(mdb_memory_access_value, DSIG_MDB_MAX_SIZE_UINT32, mdb_signal_str);
            SHR_IF_ERR_EXIT(sand_adapter_parse_mdb_signal(unit, signal_size, core, mdb_memory_access_value, cur_pp_stage));
            continue;
        }
        ...
        {
            debug_signal = &subst_pp_stage->stage_debug_signals[core][subst_pp_stage->stage_signal_num[core]++];
        }
        else
        {
            debug_signal = &cur_pp_stage->stage_debug_signals[core][cur_pp_stage->stage_signal_num[core]++];
