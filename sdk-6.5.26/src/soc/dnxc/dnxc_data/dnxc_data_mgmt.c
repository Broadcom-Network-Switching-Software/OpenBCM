    ...
    dnxc_data_value_set_f set_func)
{
    ...
    {
        if ((state_flags & DNXC_DATA_STATE_F_DEV_DATA_DONE) == 0)
        {
...
uint32
dnxc_data_mgmt_state_get(
    int unit)
{
    return _dnxc_data[unit].state;
}
    ...
    int unit)
{
    ...
    state = dnxc_data_mgmt_state_get(unit);
    if (state != 0)
    {
...
void
dnxc_data_mgmt_inaccessible_data_get(
    int unit,
    char *field_name,
    char *family_exclusive)
{
    LOG_ERROR(BSL_LOG_MODULE,
              (BSL_META_U
               (unit,
                "DNX DATA - trying to access unsupported field. the field %s is inaccessible since is exclusive to %s devices family only"),
               field_name, family_exclusive));
    assert(0);
}

void
dnxc_data_mgmt_inaccessible_data_set(
    int unit,
    char *field_name,
    char *family_exclusive)
{
    LOG_ERROR(BSL_LOG_MODULE,
              (BSL_META_U
               (unit,
                "DNX DATA - trying to set a field that is unsupported for the device %s. the field %s can't be set since is exclusive to %s only"),
               _dnxc_data[unit].name, field_name, family_exclusive));
    assert(0);
}
    ...
    int define_index)
{
    ...
    if (state == 0)
    {
        ...
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "DNX DATA - dnx data access attempted before init\n")));
        assert(0);
    }
    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];

    state = dnxc_data_mgmt_state_get(unit);
    rv = dnxc_data_mgmt_access_verify(unit, state, &(define->flags), define->set);
    if (rv != _SHR_E_NONE)
    {
    ...
    int feature_index)
{
    ...
    if (state == 0)
    {
        ...
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "DNX DATA - dnx data feature access attempted before init\n")));
        assert(0);
    }
    feature = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].features[feature_index];

    state = dnxc_data_mgmt_state_get(unit);
    rv = dnxc_data_mgmt_access_verify(unit, state, &(feature->flags), feature->set);
    ...
    int table_index)
{
    ...
    if (state == 0)
    {
        ...
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "DNX DATA - dnx data table access attempted before init\n")));
        assert(0);
    }
    ...
    int table_index)
{
    ...
    if (state == 0)
    {
        ...
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "DNX DATA - dnx data table info get attempted before init\n")));
        assert(0);
    }
    table = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].tables[table_index];

    state = dnxc_data_mgmt_state_get(unit);
    rv = dnxc_data_mgmt_access_verify(unit, state, &(table->flags), table->set);
    if (rv != _SHR_E_NONE)
    {
...
char *
dnxc_data_mgmt_shell_table_data_get(
    int unit,
    dnxc_data_table_t * table,
    int key0,
    int key1)
{
    shr_error_e rv = _SHR_E_NONE;
    char *data;
    uint32 data_flags, state;

    data_flags = IGNORE_INIT_ONLY(table->flags);
    state = dnxc_data_mgmt_state_get(unit);
    rv = dnxc_data_mgmt_access_verify(unit, state, &(data_flags), table->set);
    if (rv != _SHR_E_NONE)
    {

        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "DNX DATA - table %s access denied for device %s\n"),
                   table->name, _dnxc_data[unit].name));
        return NULL;
    }

    data = dnxc_data_mgmt_table_data_internal_get(unit, table, key0, key1, 0);
...
dnxc_data_mgmt_table_data_diag_get(
    ...
    int key1)
{
    char *data = dnxc_data_mgmt_table_data_internal_get(unit, table, key0, key1, 0);
    if (data == NULL)
    {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "DNX DATA - Data for table %s, with key0 = %d and key1 = %d could not be found \n"),
                   table->name, key0, key1));
        assert(0);
    }
    return data;
}
...
dnxc_data_mgmt_is_jr2_b0(
    int unit)
{
    return SOC_IS_JERICHO2_B0(unit);
}
...
dnxc_data_mgmt_is_jr2_b1(
    int unit)
{
    return SOC_IS_JERICHO2_B1(unit);
}
...
dnxc_data_mgmt_is_j2p_a0(
    int unit)
{
    return SOC_IS_J2P_A0(unit);
}

int
dnxc_data_mgmt_is_j2p_a1(
    int unit)
{
    return SOC_IS_J2P_A1(unit);
}

int
dnxc_data_mgmt_is_j2p_a2(
    int unit)
{
    return SOC_IS_J2P_A2(unit);
}

int
dnxc_data_mgmt_is_j2x_a0(
    int unit)
{
