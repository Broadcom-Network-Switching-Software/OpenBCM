    ...
    char *value_str)
{
    ...
    SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "value must be provided\n");
    ...
    int *value)
{
    ...
    *value = defl;
    ...
    ret = dnxc_data_property_csv_dict_str_get(unit, propval, arg_name, value_str);
    if (ret == _SHR_E_NONE)
    {
    ...
    else if (is_mandatory && ret == _SHR_E_NOT_FOUND)
    {
    ...
    {
        SHR_ERR_EXIT(ret, "'%s' key failed to parse from property '%s'", arg_name, propval);
    }
    ...
    bcm_port_phy_tx_t * tx)
{
    ...
    if (propval)
    {
        ...
        SHR_IF_ERR_EXIT(dnxc_data_property_csv_dict_str_get(unit, propval, "mode", value_str));
        if (!sal_strncasecmp("nrz", value_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            tx->signalling_mode = bcmPortPhySignallingModeNRZ;
        }
