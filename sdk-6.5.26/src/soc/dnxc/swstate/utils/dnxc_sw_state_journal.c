    ...
    char *stamp)
{
    ...
    LOG_ERROR(BSL_LOG_MODULE, (BSL_META("Difference found in swstate, associated stamp is: %s\n%s%s"), stamp, EMPTY, EMPTY));
...
static shr_error_e
dnxc_sw_state_comparison_journal_roll_back_idx_HTBL_entry_cb(
    int unit,
    int er_thread_id,
    uint8 *entry_raw,
    uint8 *payload,
    char *stamp)
{
    ...
    entry = (dnxc_sw_state_journal_entry_t *)(entry_raw);
    hash_table = (sw_state_idx_htbl_t)DNXC_SWSTATE_JOURNAL_CALCULATE_POINTER(unit, entry, data.htbl_data.hash_table_offset);

    
    key_section = payload;
    data_section = payload + hash_table->key_size;

    DNXC_SWSTATE_JOURNAL_ALLOC(unit, data_buffer, uint8 *, data_size, "sw state comparison cur val buf", DNX_ROLLBACK_JOURNAL_TYPE_COMPARISON, DNX_ROLLBACK_JOURNAL_SUBTYPE_SWSTATE);
    ...
    uint8 *payload)
{
    ...
    LOG_INFO(BSL_LOG_MODULE, (BSL_META("******************************* SW STATE Journal *******************************\n")));
...
static shr_error_e
dnxc_sw_state_journal_comparison_idx_htbl_entry_print_cb(
    int unit,
    uint8 *entry_raw,
    uint8 *payload)
{
...
static inline shr_error_e
dnxc_sw_state_journal_log_idx_htbl_internal(
    int unit,
    int er_thread_id,
    uint32 node_id,
    ...
    SW_STATE_INDEX_HASH_TABLE_KEY * const key)
{
    ...
    sal_memcpy(key_section, key, hash_table->key_size);
    SHR_IF_ERR_EXIT(sw_state_index_hash_table_entry_lookup(unit, node_id, hash_table, key, data_section, &data_idx, &found));
    ...
    SW_STATE_INDEX_HASH_TABLE_KEY * const key)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_journal_log_idx_htbl_internal(unit, er_thread_id, node_id, hash_table, key));
