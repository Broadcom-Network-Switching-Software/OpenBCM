    ...
    uint32 sw_state_max_size)
{
    ...
    switch (init_mode) {
        ...
        case socDnxcSwStateDataBlockRestoreAndOveride:
            ...
            scache_ptr = (uint8*)DNXC_SW_STATE_ALIGN_SIZE((unsigned long)scache_ptr);
            ...
            for (i = 0; i < NOF_MODULE_ID; i++) {
                if (retrieved_data_block_header->roots_array[i] != NULL) {
                    dnxc_sw_state_dispatcher_module_init(unit, DNXC_SW_STATE_LAYOUT_NODE_ID_GET(i, 0), DNXC_SW_STATE_IMPLEMENTATION_WB);
                } else {
                    dnxc_sw_state_dispatcher_module_init(unit, DNXC_SW_STATE_LAYOUT_NODE_ID_GET(i, 0), DNXC_SW_STATE_IMPLEMENTATION_PLAIN);
                }
            }
    ...
    sw_state_layout_array[unit] = (dnxc_sw_state_layout_t **)dnxc_sw_state_data_block_header[unit]->layout_array; 
...
int dnxc_sw_state_data_block_header_init(int unit, uint32 size) {
    ...
    alignment_offset = DNXC_SW_STATE_ALIGN_SIZE((unsigned long)scache_ptr) - (unsigned long)scache_ptr;
    size = size - alignment_offset;
    total_buffer_size = total_buffer_size - alignment_offset;

    
    dnxc_sw_state_data_block_header[unit] = (dnxc_sw_state_data_block_header_t *)(scache_ptr + alignment_offset);
    ...
    sal_memset(dnxc_sw_state_data_block_header[unit]->module_sizes, 0, NOF_MODULE_ID * sizeof(uint32));
    ...
    dnxc_sw_state_data_block_header[unit]->layout_array = (dnxc_sw_state_layout_t**)((uint8*)(dnxc_sw_state_data_block_header[unit]->roots_array) + sizeof(void*) * NOF_MODULE_ID);

    
    dnxc_sw_state_data_block_header[unit]->next_free_data_slot = (uint8*)(dnxc_sw_state_data_block_header[unit]->layout_array) + sizeof(dnxc_sw_state_layout_t*) * NOF_MODULE_ID;
...
int dnxc_sw_state_module_init_wb(
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_alloc_wb(unit, node_id, (uint8**)&(dnxc_sw_state_data_block_header[unit]->roots_array[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)]), 1, size, flags, dbg_string));
    sw_state_roots_array[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)] = dnxc_sw_state_data_block_header[unit]->roots_array[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)];
    SOC_EXIT;
...
int dnxc_sw_state_layout_init_wb(
    ...
    DNXC_SW_STATE_INIT_FUNC_DEFS;
    SHR_IF_ERR_EXIT(dnxc_sw_state_alloc_wb(unit, node_id, (uint8**)&(dnxc_sw_state_data_block_header[unit]->layout_array[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)]), 1,  size * nof_params, flags, dbg_string));
    sw_state_layout_array[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)] = dnxc_sw_state_data_block_header[unit]->layout_array[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)];
    SOC_EXIT;
...
int dnxc_sw_state_module_deinit_wb(
    ...
    DNXC_SW_STATE_INIT_FUNC_DEFS;
    SHR_IF_ERR_EXIT(dnxc_sw_state_free_wb(unit, node_id, (uint8**)&(dnxc_sw_state_data_block_header[unit]->roots_array[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)]), flags, dbg_string));
    sw_state_roots_array[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)] = NULL;
    dnxc_sw_state_data_block_header[unit]->roots_array[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)] = NULL;
    SOC_EXIT;
...
int dnxc_sw_state_layout_deinit_wb(
    ...
    DNXC_SW_STATE_INIT_FUNC_DEFS;
    SHR_IF_ERR_EXIT(dnxc_sw_state_free_wb(unit, node_id, (uint8**)&(dnxc_sw_state_data_block_header[unit]->layout_array[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)]), flags, dbg_string));
    sw_state_layout_array[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)] = NULL;
    dnxc_sw_state_data_block_header[unit]->layout_array[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)] = NULL;
    SOC_EXIT;
...
int dnxc_sw_state_alloc_wb(
    ...
    DNXC_VERIFY_INVOKE(DNXC_SW_STATE_IF_ERR_EXIT(dnxc_sw_state_alloc_verify_common(unit, node_id, ptr_location, _nof_elements, element_size, flags, dbg_string)));
    ...
    alloc_size = DNXC_SW_STATE_ALIGN_SIZE(sizeof(dnxc_sw_state_allocation_data_prefix_wb_t)) + size + sizeof(dnxc_sw_state_allocation_data_suffix_wb_t);
    alloc_size = DNXC_SW_STATE_ALIGN_SIZE(alloc_size);
    ...
    if(UINTPTR_TO_PTR(PTR_TO_UINTPTR(p)) != p)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "dnxc_sw_state_alloc_wb: Invalid Pointer");
    }
    ...
    ptr_suffix = (dnxc_sw_state_allocation_data_suffix_wb_t *)(((uint8 *)ptr_prefix) + DNXC_SW_STATE_ALIGN_SIZE(sizeof(dnxc_sw_state_allocation_data_prefix_wb_t)) + size);
    ptr_suffix->end_santinel = DNXC_SW_STATE_MEMORY_ALLOCATION_END_SANTINEL;

    *ptr_location = ((uint8*)p) + DNXC_SW_STATE_ALIGN_SIZE(sizeof(dnxc_sw_state_allocation_data_prefix_wb_t));
    ...
    if(dnxc_sw_state_journal_is_done_init(unit) && !sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_log_alloc(unit, node_id, ptr_location));
    }
#endif 
...
int dnxc_sw_state_free_wb(
    ...
    DNXC_VERIFY_INVOKE(DNXC_SW_STATE_IF_ERR_EXIT(dnxc_sw_state_free_verify_common(unit, node_id, ptr_location, flags, dbg_string)));
    ...
    while (p <= stack_base) {
        if (p->ptr_value == *ptr_location) {
            ...
            if (p->type == SW_STATE_ALLOC_ELEMENT_POINTER) {
                ...
                if(*freed_location != *ptr_location)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "dnxc_sw_state_free_wb: Different location of the freed pointer");
                }
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "dnxc_sw_state_free_wb: ptr_location has invalid range");
                }

                
                
                SHR_IF_ERR_EXIT(dnxc_sw_state_alloc_size_wb(unit, node_id, *ptr_location, &nof_elements, &element_size));

#ifdef BCM_DNX_SUPPORT
                if(dnxc_sw_state_journal_is_done_init(unit) && !sw_state_is_flag_on(flags, DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
                    SHR_IF_ERR_EXIT(dnxc_sw_state_journal_log_free(unit, node_id, nof_elements, element_size, ptr_location));
                }
#endif 

                size = nof_elements * element_size;
                size = size + DNXC_SW_STATE_ALIGN_SIZE(sizeof(dnxc_sw_state_allocation_data_prefix_wb_t)) + sizeof(dnxc_sw_state_allocation_data_suffix_wb_t);
                
                size = DNXC_SW_STATE_ALIGN_SIZE(size);
                
                sal_memset((uint8*)(*ptr_location) - DNXC_SW_STATE_ALIGN_SIZE(sizeof(dnxc_sw_state_allocation_data_prefix_wb_t)), 0, size);                
...
int dnxc_sw_state_alloc_size_wb(
    ...
    DNXC_VERIFY_INVOKE(DNXC_SW_STATE_IF_ERR_EXIT(dnxc_sw_state_alloc_size_verify_common(unit, node_id, location, nof_elements, element_size)));

    prefix = (dnxc_sw_state_allocation_data_prefix_wb_t*)(location - DNXC_SW_STATE_ALIGN_SIZE(sizeof(dnxc_sw_state_allocation_data_prefix_wb_t)));
...
void dnxc_sw_state_wb_module_size_update(int unit, uint32 node_id, uint32 allocated_size, uint32 increment){

    
    if (increment == 0)
    {
        dnxc_sw_state_data_block_header[unit]->module_sizes[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)]-= allocated_size;
    }
    else
    {
        dnxc_sw_state_data_block_header[unit]->module_sizes[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)]+= allocated_size;
...
uint32 dnxc_sw_state_wb_module_size_get(int unit, uint32 node_id){

    return dnxc_sw_state_data_block_header[unit]->module_sizes[DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)];
