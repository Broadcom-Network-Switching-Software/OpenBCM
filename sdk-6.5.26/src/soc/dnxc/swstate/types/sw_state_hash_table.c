    ...
    uint32 alloc_flags)
{
    ...
    sw_state_hash_table_info_clear(unit, node_id, *hash_table);
    ...
    DNX_SW_STATE_ALLOC(unit, node_id, (*hash_table)->data, uint8, max_nof_elements * data_size,
    ...
    sw_state_htbl_t *hash_table_ptr)
{
    ...
    DNX_SW_STATE_FREE(unit, node_id, (*hash_table_ptr)->data, "sw state hashtable data");
    ...
    uint8 *success)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_journal_log_htbl(unit, node_id, hash_table, key));
#endif 

    
    SHR_IF_ERR_EXIT(sw_state_hash_table_find_entry
                    (unit, node_id, hash_table, key, TRUE, &entry_offset, &found, &prev_entry, &first));
    ...
    DNX_SW_STATE_MEMWRITE(unit, node_id, data, hash_table->data, data_size * entry_offset, data_size, 0, "swstate hashtable add data");
    ...
    SW_STATE_HASH_TABLE_KEY * const key)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_hash_table_find_entry
                    (unit, node_id, hash_table, key, FALSE, &entry_offset, &found, &prev_entry, &first));
    ...
    DNX_SW_STATE_MEMSET(unit, node_id, hash_table->data, data_size * entry_offset, 0x0, data_size, 0, "swstate hashtable remove data");
    ...
    sw_state_htbl_t hash_table)
{
    ...
    do {
        ...
        SHR_IF_ERR_EXIT(sw_state_hash_table_get_next(unit, node_id, hash_table, &iter, (SW_STATE_HASH_TABLE_KEY*) cur_key, (SW_STATE_HASH_TABLE_DATA*)data));
        if (iter == UTILEX_U32_MAX)
        {
            SHR_EXIT();
        }
        
        SHR_IF_ERR_EXIT(sw_state_hash_table_entry_remove(unit, node_id, hash_table, (SW_STATE_HASH_TABLE_KEY*) cur_key));
    ...
    uint8 *first)
{
    ...
    *first = TRUE;
    ...
    SHR_IF_ERR_EXIT(sw_state_hash_table_simple_hash(unit, node_id, hash_table, key, 0, &hash_val));
    *prev_entry = hash_val;
    ...
    uint8 *found)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_hash_table_find_entry(unit,
                                                 node_id, hash_table, key, FALSE, &entry_offset, &is_found, &prev_entry,
                                                 &first));
    ...
    sw_state_htbl_t hash_table)
{
    ...
    DNX_SW_STATE_MEMSET(unit, node_id, hash_table->data, offset, 0x00, max_nof_elements * data_size, 0, "");
    ...
    sw_state_htbl_t hash_table)
{
    ...
    sw_state_hash_table_get_num_active(unit, node_id, hash_table, &active_entries);
