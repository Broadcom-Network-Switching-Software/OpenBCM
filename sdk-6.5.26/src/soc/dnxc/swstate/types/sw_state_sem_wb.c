...
STATIC int dnxc_sw_state_sem_create_wb_internal_verify(int unit, uint32 node_id, sw_state_sem_t *ptr_sem, char *desc)
{
...
int dnxc_sw_state_sem_create_wb(int unit, uint32 node_id, sw_state_sem_t *ptr_sem, int is_binary, int initial_count, char *desc, uint32 flags) {
    ...
    DNXC_VERIFY_INVOKE(SHR_IF_ERR_EXIT(dnxc_sw_state_sem_create_wb_internal_verify(unit, node_id, ptr_sem, desc)));
    ...
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_sem_create(unit, node_id, ptr_sem));
    }
#endif 
...
int dnxc_sw_state_sem_destroy_wb(int unit, uint32 node_id, sw_state_sem_t *ptr_sem, uint32 flags)
{
#ifdef BCM_DNX_SUPPORT
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(dnxc_sw_state_free_wb(unit, node_id, (uint8**)ptr_sem, flags,"sw_state_sem_destroy"),
        "unit:%d sw_state semaphore destroy failed\n%s%s", unit, EMPTY, EMPTY);

#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_sem_destroy(unit, node_id, ptr_sem, is_binary, initial_count));
    }
#endif 
    DNXC_SW_STATE_FUNC_RETURN;
}
...
int dnxc_sw_state_sem_take_wb(int unit, uint32 node_id, sw_state_sem_t *ptr_sem, int usec, uint32 flags)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_sem_take(unit, node_id, ptr_sem, usec));
    }
#endif 
...
int dnxc_sw_state_sem_give_wb(int unit, uint32 node_id, sw_state_sem_t *ptr_sem, uint32 flags)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_sem_give(unit, node_id, ptr_sem));
    }
#endif 
