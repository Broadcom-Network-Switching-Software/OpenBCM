    ...
    uint32 alloc_flags)
{
    ...
    DNX_SW_STATE_ALLOC(unit, node_id, *bit_map_ptr, **bit_map_ptr,  1, DNXC_SW_STATE_NO_FLAGS, "sw_state occ bitmap");
    ...
    DNX_SW_STATE_ALLOC(unit, node_id, (*bit_map_ptr)->levels, uint32, nof_levels, alloc_flags, "occ_bitmap levels array");
    ...
    if (init_info->support_cache)
    {
        ...
        DNX_SW_STATE_ALLOC(unit, node_id, (*bit_map_ptr)->levels_cache, uint32, nof_levels, alloc_flags, "occ_bitmap levels_cache array");
    }

    DNX_SW_STATE_ALLOC(unit, node_id, (*bit_map_ptr)->levels_size, uint32, nof_levels, alloc_flags, "occ_bitmap levels_size array");
    ...
    if (init_info->support_cache)
    {
         DNX_SW_STATE_ALLOC(unit, node_id, (*bit_map_ptr)->levels_cache_buffer, uint8, all_level_size, alloc_flags, "occ_bitmap levels_cache_buffer");
    }
    DNX_SW_STATE_ALLOC(unit, node_id, (*bit_map_ptr)->levels_buffer, uint8, all_level_size, alloc_flags, "occ_bitmap levels_buffer");

    
    if (init_val)
    {
        DNX_SW_STATE_MEMSET(unit, node_id, (*bit_map_ptr)->levels_buffer, 0, init_val, all_level_size, 0, "levels_buffer memset");
    }
   ...
   sw_state_occ_bm_t *bit_map_ptr)
{
    ...
    DNX_SW_STATE_FREE(unit, node_id, (*bit_map_ptr)->levels, "bitmap levels");
    DNX_SW_STATE_FREE(unit, node_id, (*bit_map_ptr)->levels_buffer, "bitmap level buffers");

    support_cache = (*bit_map_ptr)->support_cache;

    if (support_cache)
    {
        DNX_SW_STATE_FREE(unit, node_id, (*bit_map_ptr)->levels_cache, "bitmap level cache");
        DNX_SW_STATE_FREE(unit, node_id, (*bit_map_ptr)->levels_cache_buffer, "bitmap levels cache buffer");
    }

    DNX_SW_STATE_FREE(unit, node_id, (*bit_map_ptr)->levels_size, "bitmap levels size");
    DNX_SW_STATE_FREE(unit, node_id, (*bit_map_ptr), "swstate occ bitmap"); 
    ...
    uint8 *found)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_occ_bm_get_next(unit, node_id, bit_map, place, found));
    size = bit_map->size;
    ...
    SHR_IF_ERR_EXIT(sw_state_occ_bm_occup_status_set(unit, node_id, bit_map, *place, 1));
