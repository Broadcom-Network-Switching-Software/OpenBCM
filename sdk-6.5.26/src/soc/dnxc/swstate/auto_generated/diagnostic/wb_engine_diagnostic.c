...
sw_state_wb_engine_buffer_instance_dump(int  unit,  int  engine_id,  int  buffer_id,  int  instance_idx_0,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    else
    {
        ...
        for(; i0 < I0; i0++) {
            ...
            if(i1 >= dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_SW_STATE_WB_ENGINE__BUFFER__INSTANCE, ((sw_state_wb_engine_t*)sw_state_roots_array[unit][WB_ENGINE_MODULE_ID])->buffer[i0]))
            {
                LOG_CLI((BSL_META("sw_state_wb_engine[]->((sw_state_wb_engine_t*)sw_state_roots_array[][WB_ENGINE_MODULE_ID])->buffer[][].instance[]: ")));
                LOG_CLI((BSL_META("Invalid index: %d \n"),i1));
                SHR_EXIT();
            ...
            if(dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_SW_STATE_WB_ENGINE__BUFFER__INSTANCE, ((sw_state_wb_engine_t*)sw_state_roots_array[unit][WB_ENGINE_MODULE_ID])->buffer[i0]) == 0)
            {
                SHR_EXIT();
            }
            ...
            for(; i1 < I1; i1++) {
                ...
                if(i2 >= dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_SW_STATE_WB_ENGINE__BUFFER__INSTANCE, ((sw_state_wb_engine_t*)sw_state_roots_array[unit][WB_ENGINE_MODULE_ID])->buffer[i0][i1].instance))
                {
                    LOG_CLI((BSL_META("sw_state_wb_engine[]->((sw_state_wb_engine_t*)sw_state_roots_array[][WB_ENGINE_MODULE_ID])->buffer[][].instance[]: ")));
                    LOG_CLI((BSL_META("Invalid index: %d \n"),i2));
                    SHR_EXIT();
                ...
                if(dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_SW_STATE_WB_ENGINE__BUFFER__INSTANCE, ((sw_state_wb_engine_t*)sw_state_roots_array[unit][WB_ENGINE_MODULE_ID])->buffer[i0][i1].instance) == 0)
                {
                    SHR_EXIT();
                }
