...
int
multithread_analyzer_htb_create(int unit, sw_state_idx_htbl_init_info_t *init_info)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
...
multithread_analyzer_htb_destroy(int unit)
{
    ...
    SW_STATE_INDEX_HTB_DESTROY(
        DNX_SW_STATE_MULTITHREAD_ANALYZER__HTB,
        ((multithread_analyzer_t*)multithread_analyzer_root[unit])->htb);

    DNX_SW_STATE_LOG(
        unit,
        DNX_SW_STATE_INDEX_HTB_DESTROY_LOGGING,
        BSL_LS_SWSTATEDNX_INDEX_HTBDESTROY,
        DNX_SW_STATE_MULTITHREAD_ANALYZER__HTB,
        &((multithread_analyzer_t*)multithread_analyzer_root[unit])->htb,
        "multithread_analyzer[%d]->->htb",
        unit);
...
int
multithread_analyzer_htb_get_next(int unit, SW_STATE_INDEX_HASH_TABLE_ITER *iter, const thread_access_log_entry_t *key, const uint32 *data_index)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
