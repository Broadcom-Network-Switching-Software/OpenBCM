    ...
    uint32 flags)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
    {
        ...
        else
        {
            ...
            if (!valid)
            {
                entry->attr_flags |= DBAL_ENTRY_ATTR_FLAG_INVALID;
            }
        }
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_handler_entry_valid_set
                    (unit, core, entry->entry_hw_id, !(entry->attr_flags & DBAL_ENTRY_ATTR_FLAG_INVALID)));
    ...
    uint8 *is_end)
{
    ...
    while (TRUE)
    {
        ...
        if (found)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_tcam_handler_entry_valid_get(unit, core, entry->entry_hw_id, &valid));
            if (!valid)
            {
                entry->attr_flags |= DBAL_ENTRY_ATTR_FLAG_INVALID;
            }
