    ...
    int er_thread_id)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_journal_clear(unit, er_thread_id, FALSE));
    SHR_IF_ERR_EXIT(dnx_dbal_journal_clear(unit, er_thread_id, FALSE));
...
exit:
    if (dnx_err_recovery_common_is_validation_enabled(unit) == TRUE)
    {
    ...
    int api_result)
{
    ...
    if (1 == (dnx_err_recovery_manager[unit][er_thread_id].transaction_counter))
    {
        ...
        if (SHR_FAILURE(api_result) || dnx_er_regress_transaction_end_should_abort(unit, er_thread_id))
        {
