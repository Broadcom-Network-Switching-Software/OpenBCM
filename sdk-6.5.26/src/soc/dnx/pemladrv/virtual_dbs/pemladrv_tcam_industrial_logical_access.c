    ...
    void* entry_data)
{
    ...
    if(!dnx_data_field.features.tcam_cs_is_tcam_direct_access_get(unit))
    {
        ...
        data[0] = 2;
        SHR_IF_ERR_EXIT(dnx_pem_direct_access_wrapper(unit, sub_unit_idx, FALSE, TRUE, (int)mem->block_identifier, tcam_mask_line_address,
            mem->mem_width_in_bits, data));

        
        sal_memset(data, 0, SOC_REG_ABOVE_64_MAX_SIZE_U32*sizeof(uint32));
        sal_memset(field_data, 0, SOC_REG_ABOVE_64_MAX_SIZE_U32*sizeof(uint32));
        SHR_IF_ERR_EXIT(dnx_pem_direct_access_wrapper(unit, sub_unit_idx, TRUE, TRUE, (int)mem->block_identifier, tcam_mask_line_address,
            mem->mem_width_in_bits, data));
        ...
        sal_memset(data, 0, SOC_REG_ABOVE_64_MAX_SIZE_U32*sizeof(uint32));
        data[0] = 2;
        SHR_IF_ERR_EXIT(dnx_pem_direct_access_wrapper(unit, sub_unit_idx, FALSE, TRUE, (int)mem->block_identifier, tcam_key_line_address,
            mem->mem_width_in_bits, data));

        
        sal_memset(data, 0, SOC_REG_ABOVE_64_MAX_SIZE_U32*sizeof(uint32));
        sal_memset(field_data1, 0, SOC_REG_ABOVE_64_MAX_SIZE_U32*sizeof(uint32));
        SHR_IF_ERR_EXIT(dnx_pem_direct_access_wrapper(unit, sub_unit_idx, TRUE, TRUE, (int)mem->block_identifier, tcam_key_line_address,
            mem->mem_width_in_bits, data));
