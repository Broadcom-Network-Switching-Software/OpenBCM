...
shr_error_e hexstr2uints(int unit, char *str, unsigned int *len, unsigned int** arr)
{
...
shr_error_e hexstr2addr(int unit, char *str, unsigned int *block_id, unsigned int* addr)
{
  unsigned int len;
...
shr_error_e parse_meminfo_definition_file(int unit, int restore_after_reset, uint32 use_file, const char *rel_file_path, const char *file_name)
{
    int read_lines = 1;
    ...
    if(use_file)
    {
        ...
        if(sal_fgets(line, sizeof(*line)*PEMLA_UCODE_LINE_MAX_WIDTH, fp) == NULL) {read_lines = -1; continue;}
    }
    else
    {
        line = lines[line_no];
    ...
    if (sal_strnlen(line, PEMLA_UCODE_LINE_MAX_WIDTH) == 0) {line_no = line_no + 1; continue;}

    
    if (sal_strnstr(line, END_COMMENT, PEMLA_UCODE_LINE_MAX_WIDTH) != 0)    { in_comment = 0; ++line_no; continue;} 
    if (sal_strnstr(line, START_COMMENT, PEMLA_UCODE_LINE_MAX_WIDTH) != 0)  { in_comment = 1; ++line_no; continue;}
    if (in_comment) { ++line_no; continue;}

    
    if ((sal_strncmp(line, KEYWORD_UCODE_MEM_WRITE_INFO, KEYWORD_UCODE_MEM_WRITE_INFO_SIZE) == 0) &&   !restore_after_reset )   { SHR_IF_ERR_EXIT(dnx_pemladrv_mem_line_write(unit, line));  ++line_no; continue; }
    if ((sal_strncmp(line, KEYWORD_UCODE_REG_WRITE_INFO, KEYWORD_UCODE_REG_WRITE_INFO_SIZE) == 0) &&   !restore_after_reset )   { SHR_IF_ERR_EXIT(dnx_pemladrv_reg_line_write(unit, line));  ++line_no; continue; }
    
    if (sal_strncmp(line, KEYWORD_DB, KEYWORD_DB_SIZE) == 0)                                                                    { SHR_IF_ERR_EXIT(dnx_pemladrv_db_info_insert(unit, line))                      ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_KEY, KEYWORD_KEY_SIZE) == 0)                                                                  { SHR_IF_ERR_EXIT(dnx_pemladrv_db_field_info_insert(unit, line))                ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_RESULT, KEYWORD_RESULT_SIZE) == 0)                                                            { SHR_IF_ERR_EXIT(dnx_pemladrv_db_field_info_insert(unit, line))                ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_DIRECT_INFO, KEYWORD_DB_DIRECT_INFO_SIZE) == 0)                                            { SHR_IF_ERR_EXIT(dnx_pemladrv_direct_result_chunk_insert(unit, line))          ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_TCAM_KEY_INFO, KEYWORD_DB_TCAM_KEY_INFO_SIZE) == 0)                                        { SHR_IF_ERR_EXIT(dnx_pemladrv_tcam_key_chunk_insert(unit, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_TCAM_RESULT_INFO, KEYWORD_DB_TCAM_RESULT_INFO_SIZE) == 0)                                  { SHR_IF_ERR_EXIT(dnx_pemladrv_tcam_result_chunk_insert(unit, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_EXACT_MATCH_KEY_INFO, KEYWORD_DB_EXACT_MATCH_KEY_INFO_SIZE) == 0)                          { SHR_IF_ERR_EXIT(dnx_pemladrv_em_key_chunk_insert(unit, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_EXACT_MATCH_RESULT_INFO, KEYWORD_DB_EXACT_MATCH_RESULT_INFO_SIZE) == 0)                    { SHR_IF_ERR_EXIT(dnx_pemladrv_em_result_chunk_insert(unit, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_LONGEST_PERFIX_MATCH_KEY_INFO, KEYWORD_DB_LONGEST_PERFIX_MATCH_KEY_INFO_SIZE) == 0)        { SHR_IF_ERR_EXIT(dnx_pemladrv_lpm_key_chunk_insert(unit, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_LONGEST_PERFIX_MATCH_RESULT_INFO, KEYWORD_DB_LONGEST_PERFIX_MATCH_RESULT_INFO_SIZE) == 0)  { SHR_IF_ERR_EXIT(dnx_pemladrv_lpm_result_chunk_insert(unit, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_VIRTUAL_REGISTER_MAPPING, KEYWORD_REGISTER_INFO_SIZE) == 0)                                   { SHR_IF_ERR_EXIT(dnx_pemladrv_register_insert(unit, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_REG_AND_DBS_NUM_INFO, KEYWORD_REG_AND_DBS_NUM_INFO_SIZE) == 0)                                { SHR_IF_ERR_EXIT(dnx_pemladrv_dnx_init_all_db_arr_by_size(unit, line))         ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_SINGLE_CHUNCK_INFO, KEYWORD_DB_SINGLE_CHUNCK_INFO_SIZE) == 0)                              { SHR_IF_ERR_EXIT(dnx_pemladrv_init_logical_db_chunk_mapper(unit, line))        ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_VIRTUAL_REGISTER_NOF_FIELDS, KEYWORD_VIRTUAL_REGISTER_NOF_FIELDS_SIZE) == 0)                  { SHR_IF_ERR_EXIT(dnx_pemladrv_init_reg_field_info(unit, line))                 ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_VIRTUAL_REGISTER_FIELD_NOF_MAPPINGS, KEYWORD_VIRTUAL_REGISTER_FIELD_NOF_MAPPINGS_SIZE) == 0)  { SHR_IF_ERR_EXIT(dnx_pemladrv_init_reg_field_mapper(unit, line))               ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_PEM_VER, KEYWORD_PEM_VER_SIZE) == 0)                                                          { SHR_IF_ERR_EXIT(init_pem_version_control(unit, line))      ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_PEM_APPLET_REG, KEYWORD_PEM_APPLET_REG_SIZE) == 0)                                            { SHR_IF_ERR_EXIT(init_pem_applet_reg(unit, line))                 ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_PEM_APPLET_MEM, KEYWORD_PEM_APPLET_MEM_SIZE) == 0)                                            { SHR_IF_ERR_EXIT(init_pem_applet_mem(unit, line))                 ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_GENERAL_INFO_FOR_APPLET, KEYWORD_GENERAL_INFO_FOR_APPLET_SIZE) == 0)                          { SHR_IF_ERR_EXIT(init_meminfo_array_for_applets(unit, line))      ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_MEMINFO_FOR_APPLET, KEYWORD_MEMINFO_FOR_APPLET_SIZE) == 0)                                    { SHR_IF_ERR_EXIT(insert_meminfo_to_array_for_applets(unit, line)) ;  ++line_no; continue; }

    if (sal_strncmp(line, KEYWORD_UCODE_DUMP_SINGLE_VIRTUAL_WIRE_MAPPING, KEYWORD_UCODE_DUMP_SINGLE_VIRTUAL_WIRE_MAPPING_SIZE) == 0) { SHR_IF_ERR_EXIT(dnx_pemladrv_vw_mapping_insert(unit, line))      ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_UCODE_VIRTUAL_WIRE_MAPPING_INFO, KEYWORD_UCODE_VIRTUAL_WIRE_MAPPING_INFO_SIZE) == 0)               { SHR_IF_ERR_EXIT(dnx_pemladrv_init_vw_info(unit, line))           ;  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_UCODE_DUMP_NOF_VIRTUAL_WIRES, KEYWORD_UCODE_DUMP_NOF_VIRTUAL_WIRES_SIZE) == 0)                     { SHR_IF_ERR_EXIT(dnx_pemladrv_init_vw_arr_by_size(unit, line))    ;  ++line_no; continue; }
...
void free_db_direct_container(const int unit) {
  ...
  for (i = 0; i < api_info[unit].db_direct_container.nof_direct_dbs; ++i) {
    if (sal_strncmp(api_info[unit].db_direct_container.db_direct_info_arr[i].db_direct_info_per_core_arr[core_id].name, "/0", 1) ==
        ...
        continue;
    if (NULL ==
        api_info[unit].db_direct_container.db_direct_info_arr[i].db_direct_info_per_core_arr[core_id].result_chunk_mapper_matrix_arr) {    
        continue;
    }
...
void free_db_tcam_container(const int unit) {
  ...
  {
   if(sal_strncmp(api_info[unit].db_tcam_container.db_tcam_info_arr[i].db_tcam_per_core_arr[core_id].name, "/0", 1) == 0) 
     continue;
    if (NULL == api_info[unit].db_tcam_container.db_tcam_info_arr[i].db_tcam_per_core_arr[core_id].result_chunk_mapper_matrix_arr){     
      SHR_FREE(api_info[unit].db_tcam_container.db_tcam_info_arr[i].db_tcam_per_core_arr[core_id].key_field_bit_range_arr);
      SHR_FREE(api_info[unit].db_tcam_container.db_tcam_info_arr[i].db_tcam_per_core_arr[core_id].result_field_bit_range_arr);
...
void free_db_lpm_container(const int unit) {
  ...
  for (i = 0; i < api_info[unit].db_lpm_container.nof_lpm_dbs; ++i){
    if(sal_strncmp(api_info[unit].db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.db_tcam_per_core_arr[core_id].name, "/0", 1) == 0) 
      continue;
    if (NULL == api_info[unit].db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.db_tcam_per_core_arr[core_id].result_chunk_mapper_matrix_arr){      
      SHR_FREE(api_info[unit].db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.db_tcam_per_core_arr[core_id].key_field_bit_range_arr);
      SHR_FREE(api_info[unit].db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.db_tcam_per_core_arr[core_id].result_field_bit_range_arr);
...
void free_db_em_container(const int unit) {
  ...
  {
    if(sal_strncmp(api_info[unit].db_em_container.db_em_info_arr[i].logical_em_info.db_tcam_per_core_arr[core_id].name, "/0", 1) == 0) 
      continue;
    if (NULL == api_info[unit].db_em_container.db_em_info_arr[i].logical_em_info.db_tcam_per_core_arr[core_id].result_chunk_mapper_matrix_arr){      
      SHR_FREE(api_info[unit].db_em_container.db_em_info_arr[i].logical_em_info.db_tcam_per_core_arr[core_id].key_field_bit_range_arr);
      SHR_FREE(api_info[unit].db_em_container.db_em_info_arr[i].logical_em_info.db_tcam_per_core_arr[core_id].result_field_bit_range_arr);
  ...
  if(chunk_mapper_matrix)
  {
    for (implamentation_index = 0; implamentation_index < nof_implamentations; ++implamentation_index)
    {
      for (row_index = 0; row_index < chunk_mapper_matrix->nof_rows_in_chunk_matrix; ++row_index)
  	{
  	  ...
  	  {
          SHR_FREE(chunk_mapper_matrix[implamentation_index].db_chunk_mapper[row_index][col_index]->phy_mem_name);
...
shr_error_e dnx_pemladrv_mem_line_write(int unit, const char* line) {
  char pem_mem_address[MEM_ADDR_LENGTH] = {0};                              
  char str_value[MAX_MEM_DATA_LENGTH] = {0};                   
  char shtut[MAX_SHTUT_LENGTH] = {0};
  int width_in_bits;
  char key_word[MAX_NAME_LENGTH] = {0}, pem_mem_name[MAX_NAME_LENGTH] = {0};
  ...
  else
  {
    ...
    {
        SHR_IF_ERR_EXIT(pem_write(unit, sub_unit_idx, &phy_mem, 1, data_value));
    }
  }
exit:
  SHR_FUNC_EXIT;
...
shr_error_e dnx_pemladrv_reg_line_write(int unit, const char* line) {
  char pem_reg_address[MEM_ADDR_LENGTH] = {0};                              
  char str_value[MAX_MEM_DATA_LENGTH] = {0};                   
  char shtut[MAX_SHTUT_LENGTH] = {0};
  int width_in_bits;
  char key_word[MAX_NAME_LENGTH] = {0}, pem_reg_name[MAX_NAME_LENGTH] = {0};
  phy_mem_t phy_mem = {0};
  ...
  if (sscanf(line, "%s %s %s %d %2s %s", key_word, pem_reg_name, pem_reg_address, &width_in_bits, shtut, str_value) != PEM_NOF_REG_WRITE_TOKEN) {
    ...
    for(int sub_unit_idx = 0; sub_unit_idx < PEM_CFG_NOF_SUB_UNITS; sub_unit_idx++)
    {
        SHR_IF_ERR_EXIT(pem_write(unit, sub_unit_idx, &phy_mem, 0, data_value));
    }
  }
...
shr_error_e dnx_pemladrv_db_info_insert(const int unit, const char* line) {
  char key_word[MAX_NAME_LENGTH] = {0},db_type[MAX_NAME_LENGTH] = {0}, db_name[MAX_NAME_LENGTH] = {0};
  ...
  if (sscanf( line, "%s %s %s %d %d %d %d %d %d ", key_word, db_type, db_name, &db_id, &nof_entries, &key_width, &result_width, &nof_key_fields, &nof_result_fields) != PEM_NOF_DB_TOKEN) {
      ...
      for (int core_id=0; core_id<PEM_CFG_API_NOF_CORES; ++core_id) {
          if ((sal_strncmp(db_type, KEYWORD_DB_DIRECT, sizeof(db_type)) == 0)) {
              ...
              SHR_IF_ERR_EXIT(init_logical_fields_location(unit,
...
shr_error_e dnx_pemladrv_db_field_info_insert(const int unit, const char* line) {
  char key_word[MAX_NAME_LENGTH] = {0}, db_name_debug[MAX_NAME_LENGTH] = {0}, field_name_debug[MAX_NAME_LENGTH] = {0}, db_type[MAX_NAME_LENGTH] = {0};
...
shr_error_e dnx_pemladrv_direct_result_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  int core_id = 0;
...
exit:
    if(SHR_FUNC_ERR())
        SHR_FREE(chunk_info_ptr);
    SHR_FUNC_EXIT;
}


shr_error_e dnx_pemladrv_tcam_key_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  int core_id = 0;
  DbChunkMapper* chunk_info_ptr = NULL;
...
shr_error_e dnx_pemladrv_tcam_result_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  int core_id = 0;
  DbChunkMapper* chunk_info_ptr = NULL;
...
shr_error_e dnx_pemladrv_em_key_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  int core_id = 0;
  DbChunkMapper* chunk_info_ptr = NULL;
...
shr_error_e dnx_pemladrv_em_result_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  int core_id = 0;
  DbChunkMapper* chunk_info_ptr = NULL;
...
shr_error_e dnx_pemladrv_lpm_key_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  int core_id = 0;
  DbChunkMapper* chunk_info_ptr = NULL;
...
shr_error_e dnx_pemladrv_lpm_result_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  int core_id = 0;
  DbChunkMapper* chunk_info_ptr = NULL;
...
shr_error_e build_direct_chunk_from_ucode(const int unit, int* core_id, const char* line, DbChunkMapper* chunk_info_ptr, unsigned int* db_id) {
  char pem_mem_address[44] = {0};                
  unsigned int pem_mem_width;
  unsigned int curr_offset, previous_chunks_to_add;
  unsigned int field_id, field_width, pe_matrix_col, phy_mem_idx, mem_start_row, mem_start_colomn, implementation_index;
  unsigned int index_range[2] = {0}, bits_range[2] = {0}, db_dim[2] = {0};
  unsigned int is_ingress;
  char key_word[MAX_NAME_LENGTH] = {0},db_type[MAX_NAME_LENGTH] = {0}, db_name_debug[MAX_NAME_LENGTH] = {0}, field_name_debug[MAX_NAME_LENGTH] = {0};
  ...
  if (sscanf( line, "%s %s %d %s %d %u %d%c%d %d%c%d %d%c%d %s %d %d %s %s %d %d %d %d %d %d",
    ...
    SHR_IF_ERR_EXIT(hexstr2addr(unit, pem_mem_address, &chunk_info_ptr->mem_block_id, &chunk_info_ptr->phy_mem_addr));
    chunk_info_ptr->phy_mem_entry_offset = mem_start_row;
    chunk_info_ptr->phy_mem_index = phy_mem_idx;
    SHR_ALLOC_SET_ZERO_ERR_EXIT(chunk_info_ptr->phy_mem_name, sal_strnlen(pem_mem_name, MAX_NAME_LENGTH) + 1, "chunk_info_ptr->phy_mem_name", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
...
shr_error_e build_cam_key_chunk_from_ucode(const int unit, const char* line, int* core_id, DbChunkMapper* chunk_info_ptr, unsigned int* db_id) {
  char pem_mem_address[44] = {0};                
  ...
  unsigned int index_range[2] = {0}, mask_bit_range[2] = {0}, key_bit_range[2] = {0};
  unsigned int is_ingress;
  char key_word[MAX_NAME_LENGTH] = {0},db_type[MAX_NAME_LENGTH] = {0}, db_name_debug[MAX_NAME_LENGTH] = {0}, field_name_debug[MAX_NAME_LENGTH] = {0};
  char pem_mem_name[MAX_NAME_LENGTH] = {0};
  char colon, x;
...
shr_error_e build_cam_result_chunk_from_ucode(const int unit, const char* line, int* core_id, DbChunkMapper* chunk_info_ptr, unsigned int* db_id) {
  char pem_mem_address[44] = {0};                
  unsigned int pem_mem_width, mem_nof_rows_debug, total_width;
  unsigned int pe_matrix_col, phy_mem_idx, mem_start_row, mem_start_colomn, implementation_index;
  unsigned int index_range[2] = {0}, bits_range[2] = {0};
  unsigned int is_ingress;
  char key_word[MAX_NAME_LENGTH] = {0}, db_type[MAX_NAME_LENGTH] = {0}, db_name_debug[MAX_NAME_LENGTH] = {0}, field_name_debug[MAX_NAME_LENGTH] = {0};
  char pem_mem_name[MAX_NAME_LENGTH] = {0};
  char colon, x;
...
shr_error_e dnx_pemladrv_register_insert(const int unit, const char* line) {
  ...
  char key_word[MAX_NAME_LENGTH] = {0}, db_type[MAX_NAME_LENGTH] = {0}, reg_name[MAX_NAME_LENGTH] = {0}, field_name_debug[MAX_NAME_LENGTH] = {0}, pem_mem_name[MAX_NAME_LENGTH] = {0};
  char colon;
  ...
  if (sscanf(line, "%s %s %d %d %s %d %d %d%c%d %s %s %s %d %d %d %d %d %d",
    ...
    SHR_IF_ERR_EXIT(hexstr2addr(unit, pem_mem_address, &reg_field_mapping_ptr->pem_mem_block_id, &reg_field_mapping_ptr->pem_mem_address));
...
shr_error_e dnx_pemladrv_init_logical_db_chunk_mapper(const int unit, const char* line){
    ...
    for (int core_id=0; core_id<PEM_CFG_API_NOF_CORES; ++core_id) {
        ...
        if ((sal_strncmp(line + KEYWORD_DB_SINGLE_CHUNCK_INFO_SIZE + 1, KEYWORD_DB_TCAM, KEYWORD_DB_TCAM_SIZE) == 0)) {
            ...
            SHR_IF_ERR_EXIT(init_logical_db_mapper_matrix(unit,
                    ...
                    nof_result_chunk_rows, nof_result_chunk_cols, nof_implementations_per_core));
            SHR_IF_ERR_EXIT(init_em_cache(unit, &api_info[unit].db_em_container.db_em_info_arr[db_id].em_cache[core_id]));
            ...
            SHR_IF_ERR_EXIT(init_logical_db_mapper_matrix(unit,
                    ...
                    nof_result_chunk_rows, nof_result_chunk_cols, nof_implementations_per_core));
            SHR_IF_ERR_EXIT(init_lpm_cache(unit, &api_info[unit].db_lpm_container.db_lpm_info_arr[db_id].lpm_cache[core_id]));
...
shr_error_e init_logical_db_mapper_matrix(int unit, LogicalDbChunkMapperMatrix** logical_db_mapper_matrix_ptr, const int nof_chunk_rows, const int nof_chunk_cols, const int nof_implementations) {
  int i, j;
  LogicalDbChunkMapperMatrix* logical_db_mapper_matrix = NULL;

  SHR_FUNC_INIT_VARS(unit);

  SHR_ALLOC_SET_ZERO_ERR_EXIT(*logical_db_mapper_matrix_ptr, nof_implementations*sizeof(LogicalDbChunkMapperMatrix), "logical_db_mapper_matrix_ptr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  ...
  for ( i = 0; i < nof_implementations; i = i + 1 ) {
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(logical_db_mapper_matrix[i].db_chunk_mapper, nof_chunk_rows* sizeof(DbChunkMapper**), "logical_db_mapper_matrix[].db_chunk_mapper", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    for ( j = 0; j < nof_chunk_rows; j = j + 1 ) {
      SHR_ALLOC_SET_ZERO_ERR_EXIT(logical_db_mapper_matrix[i].db_chunk_mapper[j], nof_chunk_cols* sizeof(DbChunkMapper*), "logical_db_mapper_matrix[].db_chunk_mapper[]", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    }
  }
...
shr_error_e realloc_logical_db_mapper_matrix(int unit, LogicalDbChunkMapperMatrix** logical_db_mapper_matrix_ptr, const int nof_chunk_cols_to_add, const int nof_implementations) {
...
shr_error_e init_logical_fields_location(int unit, FieldBitRange** field_bit_range_arr, const int nof_fields) {
    SHR_FUNC_INIT_VARS(unit);

  SHR_ALLOC_SET_ZERO_ERR_EXIT(*field_bit_range_arr, nof_fields* sizeof(FieldBitRange), "field_bit_range_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
...
shr_error_e dnx_pemladrv_init_reg_field_info(const int unit, const char* line) {
  ...
  if (sscanf(line, "%s %d %d %d %d", key_word, &reg_id, &nof_fields, &register_total_width, &is_binded) != VIRTUAL_REGISTER_NOF_FIELDS_TOKEN) {
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_info_arr, nof_fields* sizeof(LogicalRegFieldInfo), "api_info[unit].reg_container.reg_info_arr[].reg_field_info_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr, nof_fields* sizeof(FieldBitRange), "api_info[unit].reg_container.reg_info_arr[].reg_field_bit_range_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  }
...
shr_error_e dnx_pemladrv_init_reg_field_mapper(const int unit, const char* line) {
  char key_word[MAX_NAME_LENGTH] = {0};
  ...
  if (sscanf( line, "%s %d %d %d %d %d %d", key_word, &reg_id, &field_id, &lsb_bit, &msb_bit, &nof_mappings, &nof_cores) != VIRTUAL_REGISTER_FIELD_NOF_MAPPINGS_TOKEN) {
    ...
    for (int core_id=0; core_id < PEM_CFG_API_NOF_CORES; ++core_id){
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT(api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_info_arr[field_id].reg_field_mapping_per_core_arr[core_id].reg_field_mapping_arr, nof_mappings* sizeof(RegFieldMapper), "reg_field_mapping_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    }
  }
...
shr_error_e init_meminfo_array_for_applets(const int unit, const char* line) {
  char key_word[MAX_NAME_LENGTH] = {0};
  ...
  SHR_ALLOC_SET_ZERO_ERR_EXIT(api_info[unit].applet_info.meminfo_array_for_applet, nof_mems*sizeof(PemMemInfoForApplet*), "api_info[unit].applet_info.meminfo_array_for_applet", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
...
shr_error_e insert_meminfo_to_array_for_applets(const int unit, const char* line) {
  char key_word[MAX_NAME_LENGTH] = {0}, mem_name[MAX_NAME_LENGTH] = {0};
...
shr_error_e init_pem_version_control(const int unit, const char* line) {
  ...
  else
  {
    ...
    api_info[unit].version_info.iterator++;
    SHR_IF_ERR_EXIT(dnx_pemladrv_reg_line_write(unit, line));
  }
...
shr_error_e init_pem_applet_reg(const int unit, const char* line) {
  char key_word[MAX_NAME_LENGTH] = {0}, name[MAX_NAME_LENGTH] = {0}, address_s[MAX_NAME_LENGTH] = {0};
...
shr_error_e init_pem_applet_mem(const int unit, const char* line) {
  char key_word[MAX_NAME_LENGTH] = {0}, name[MAX_NAME_LENGTH] = {0}, address_s[MAX_NAME_LENGTH] = {0};
  unsigned int ingress0_egress_1, address, block_id, width_in_bits;
  int core;
...
shr_error_e init_em_cache(const int unit, EmDbCache* em_cache_info){
  int i;

  SHR_FUNC_INIT_VARS(unit);

  SHR_ALLOC_SET_ZERO_ERR_EXIT(em_cache_info->em_key_entry_arr, PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES* sizeof(unsigned int*), "em_cache_info->em_key_entry_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  SHR_ALLOC_SET_ZERO_ERR_EXIT(em_cache_info->em_result_entry_arr, PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES* sizeof(unsigned int*), "em_cache_info->em_result_entry_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  for(i = 0; i < PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES; ++i) {
    SHR_ALLOC_SET_ZERO_ERR_EXIT(em_cache_info->em_key_entry_arr[i], sizeof(unsigned int), "em_cache_info->em_key_entry_arr[]", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(em_cache_info->em_result_entry_arr[i], sizeof(unsigned int), "em_cache_info->em_result_entry_arr[]", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  }
  SHR_ALLOC_SET_ZERO_ERR_EXIT(em_cache_info->physical_entry_occupation, PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES* sizeof(unsigned char), "em_cache_info->physical_entry_occupation", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
...
shr_error_e init_lpm_cache(const int unit, LpmDbCache* lpm_cache_info){
  int i;

  SHR_FUNC_INIT_VARS(unit);

  SHR_ALLOC_SET_ZERO_ERR_EXIT(lpm_cache_info->lpm_key_entry_arr, PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES* sizeof(unsigned int*), "lpm_cache_info->lpm_key_entry_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  SHR_ALLOC_SET_ZERO_ERR_EXIT(lpm_cache_info->lpm_result_entry_arr, PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES* sizeof(unsigned int*), "lpm_cache_info->lpm_result_entry_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  for(i = 0; i < PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES; ++i) {
    SHR_ALLOC_SET_ZERO_ERR_EXIT(lpm_cache_info->lpm_key_entry_arr[i], sizeof(unsigned int), "lpm_cache_info->lpm_key_entry_arr[]", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(lpm_cache_info->lpm_result_entry_arr[i], sizeof(unsigned int), "lpm_cache_info->lpm_result_entry_arr[]", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  }
  SHR_ALLOC_SET_ZERO_ERR_EXIT(lpm_cache_info->lpm_key_entry_prefix_length_arr, PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES* sizeof(unsigned int), "lpm_cache_info->lpm_key_entry_prefix_length_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  SHR_ALLOC_SET_ZERO_ERR_EXIT(lpm_cache_info->physical_entry_occupation, PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES* sizeof(unsigned char), "lpm_cache_info->physical_entry_occupation", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
...
shr_error_e dnx_pemladrv_vw_mapping_insert(const int unit, const char* line){
  char key_word[MAX_NAME_LENGTH] = {0};
...
shr_error_e dnx_pemladrv_init_vw_info(const int unit, const char* line){
  char key_word[MAX_NAME_LENGTH] = {0};
  ...
  if (sscanf(line, "%s %d %s %d %d %d %d", key_word, &vw_id, vw_name, &vw_width, &nof_mappings, &start_stage, &end_stage) != VIRTUAL_WIRE_INFO_NOF_FIELDS_TOKEN) {
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(api_info[unit].vw_container.vw_info_arr[vw_id].vw_mappings_arr, nof_mappings* sizeof(VirtualWireMappingInfo), "api_info[unit].vw_container.vw_info_arr[].vw_mappings_arr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
  }
...
shr_error_e dnx_pemladrv_init_vw_arr_by_size(const int unit, const char* line){
  int nof_vws;
  char key_word[MAX_NAME_LENGTH] = {0};
...
shr_error_e db_direct_write_command(const int unit, const int core_id, const char* write_command);
shr_error_e db_tcam_write_command(const int unit, const int core_id, const char* write_command);
shr_error_e db_lpm_write_command(const int unit, const int core_id, const char* write_command);
shr_error_e db_em_write_command(const int unit, const int core_id, const char* write_command);
shr_error_e reg_write_command(const int unit, int core_id, const char* write_command);
shr_error_e program_select_tcam_write_command(const int unit, const char* write_command);
    ...
    const char* write_commands_file_name)
{
  ...
  while((sal_fgets(line, sizeof(line), fp) != NULL)) {
    ...
    if (sal_strnstr(line, END_COMMENT, PEMLADRV_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)    { in_comment = 0; ++line_no; continue;} 
    if (sal_strnstr(line, START_COMMENT, PEMLADRV_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)  { in_comment = 1; ++line_no; continue;}
    if (in_comment) { ++line_no; continue;}


    if (sal_strncmp(line, KEYWORD_DB_DIRECT_WRITE_COMMAND, KEYWORD_DB_DIRECT_WRITE_COMMAND_SIZE) == 0)          { SHR_IF_ERR_EXIT(db_direct_write_command(unit, core_id, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_TCAM_WRITE_COMMAND, KEYWORD_DB_TCAM_WRITE_COMMAND_SIZE) == 0)              { SHR_IF_ERR_EXIT(db_tcam_write_command(unit, core_id, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_LPM_WRITE_COMMAND, KEYWORD_DB_LPM_WRITE_COMMAND_SIZE) == 0)                { SHR_IF_ERR_EXIT(db_lpm_write_command(unit, core_id, line));  ++line_no; continue; }
    
    if (sal_strncmp(line, KEYWORD_DB_EM_WRITE_COMMAND, KEYWORD_DB_EM_WRITE_COMMAND_SIZE) == 0)                  { SHR_IF_ERR_EXIT(db_em_write_command(unit, core_id, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_REG_WRITE_COMMAND, KEYWORD_REG_WRITE_COMMAND_SIZE) == 0)                      { SHR_IF_ERR_EXIT(reg_write_command(unit, core_id, line));  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_PROGRAM_SELECT_CONFIG_WRITE_COMMAND, KEYWORD_PROGRAM_SELECT_CONFIG_WRITE_COMMAND_SIZE) == 0)  { SHR_IF_ERR_EXIT(program_select_tcam_write_command(unit, line));  ++line_no; continue; }
...
{
  char key_word[MAX_NAME_LENGTH] = {0}, res_value_s[MAX_NAME_LENGTH] = {0};  
...
shr_error_e db_tcam_write_command(
    ...
    const char* write_command)
{
  char key_word[MAX_NAME_LENGTH] = {0}, key_value_s[MAX_NAME_LENGTH] = {0}, mask_value_s[MAX_NAME_LENGTH] = {0}, res_value_s[MAX_NAME_LENGTH] = {0};  
  ...
  SHR_IF_ERR_EXIT(dnx_pemladrv_allocate_pemladrv_mem_struct(unit, &mask_mem_access, nof_fields_in_key, key_bit_range));

  SHR_IF_ERR_EXIT(dnx_pemladrv_allocate_pemladrv_mem_struct(unit, &result_mem_access, nof_fields_in_res, result_bit_range));
  ...
  SHR_IF_ERR_EXIT(pemladrv_tcam_write(unit, core_id, db_id, row_ndx, key_mem_access, mask_mem_access, valid_mem_access, result_mem_access));
...
shr_error_e db_lpm_write_command(
    ...
    const char* write_command)
{
  char key_word[MAX_NAME_LENGTH] = {0}, key_value_s[MAX_NAME_LENGTH] = {0}, res_value_s[MAX_NAME_LENGTH] = {0};  
  ...
  dnx_set_pem_mem_accesss_fldbuf(0, key_value, key_bit_range, key_mem_access);
  dnx_set_pem_mem_accesss_fldbuf(0, res_value, result_bit_range, result_mem_access);
  SHR_IF_ERR_EXIT(pemladrv_lpm_insert(unit, core_id, db_id, key_mem_access, nof_bits_in_prefix, result_mem_access, &index));
...
{
  char key_word[MAX_NAME_LENGTH] = {0}, reg_value_s[MAX_NAME_LENGTH] = {0};  
...
shr_error_e program_select_tcam_write_command(
    ...
    const char* write_command)
{
  char key_word[MAX_NAME_LENGTH] = {0}, mem_address_s[MAX_NAME_LENGTH] = {0}, entry_val_s[MAX_NAME_LENGTH] = {0};  
  ...
  for(int sub_unit_idx = 0; sub_unit_idx < PEM_CFG_NOF_SUB_UNITS; sub_unit_idx++)
  {
      SHR_IF_ERR_EXIT(pem_write(unit, sub_unit_idx, &mem_write, 0, data_value));
  }
#endif
