...
shr_error_e  dnx_pemladrv_allocate_pemladrv_mem_struct(int unit, pemladrv_mem_t** mem_access, unsigned int nof_fields, const FieldBitRange* bit_range_arr);
...
shr_error_e pem_write(int          unit,
              int sub_unit_idx,
              phy_mem_t*    mem,
              int is_mem,
              void *entry_data);
...
unsigned int dnx_pemladrv_get_nof_unsigned_int_chunks(const int field_width);
...
void set_bits(const unsigned int *in_buff,
...
shr_error_e dnx_pemladrv_set_valid_bit_to_zero(const int unit, const int core_id, const int virtual_row_index, LogicalTcamInfo* tcam_info);
...
shr_error_e pemladrv_lpm_remove_by_index (const int unit, const int core_id, table_id_t table_id, int index);
...
shr_error_e pem_cs_tcam_read_entry(int unit, int sub_unit_idx, phy_mem_t * mem, void* entry_data);
#endif
