    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(soc_dnx_chip_driver_find(unit, device_id, revision_id, &(soc->chip_driver)),
                             "Couldn't find driver for unit %d (device 0x%04x, revision 0x%02x)\n",
                             unit, device_id, revision_id);

    SHR_IF_ERR_EXIT(soc_dnx_chip_type_set(unit, device_id));
    ...
    {
        uint32 use_new_access = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "use_new_access",
                                                            0);
        if (use_new_access != 0)
        {
...
static shr_error_e soc_dnx_detach_no_soc_ndev_attached(
    int unit);
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnx_configure_soc_control(unit));
    ...
    if (!CM_IS_SUB_DEVICE(unit))
    {
        SHR_ALLOC_SET_ZERO_ERR_EXIT(SOC_PERSIST(unit),
                                    sizeof(soc_persist_t), "soc_persist", "%s%s%s", "failed to allocate soc_persist",
                                    EMPTY, EMPTY);
...
exit:
    if (SHR_FUNC_ERR())
    {
        ...
        soc_dnx_detach_no_soc_ndev_attached(unit);
    }
    else
    {
        ++soc_ndev_attached;
...
static shr_error_e
soc_dnx_detach_no_soc_ndev_attached(
    int unit)
{
    soc_control_t *soc;
    int spl;
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnx_detach_no_soc_ndev_attached(unit));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_CONT(soc_dnx_configure_soc_control(unit));
