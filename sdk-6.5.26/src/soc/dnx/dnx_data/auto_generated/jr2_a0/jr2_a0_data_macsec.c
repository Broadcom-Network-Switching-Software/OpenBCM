...
static shr_error_e
jr2_a0_dnx_data_macsec_egress_multiple_nh_profiles_set(
    int unit)
{
    dnxc_data_feature_t *feature;
    int module_index = dnx_data_module_macsec;
    int submodule_index = dnx_data_macsec_submodule_egress;
    int feature_index = dnx_data_macsec_egress_multiple_nh_profiles;
    SHR_FUNC_INIT_VARS(unit);

    feature = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].features[feature_index];
    
    feature->default_data = dnx_data_macsec.egress.nh_profiles_nof_get(unit)>1;

    
    feature->data = dnx_data_macsec.egress.nh_profiles_nof_get(unit)>1;
...
static shr_error_e
jr2_a0_dnx_data_macsec_egress_nh_profiles_nof_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_macsec;
    ...
    int unit)
{
    ...
    define = &submodule->defines[data_index];
    define->set = jr2_a0_dnx_data_macsec_egress_nh_profiles_nof_set;

    
    data_index = dnx_data_macsec_egress_multiple_nh_profiles;
    feature = &submodule->features[data_index];
    feature->set = jr2_a0_dnx_data_macsec_egress_multiple_nh_profiles_set;
