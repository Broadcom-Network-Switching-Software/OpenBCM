    ...
    int unit)
{
    ...
    if (DNX_FIELD_STAGE_EXTERNAL < table->keys[0].size && DNX_FIELD_CS_QUAL_VT2_PEM_CS_VAR < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_EXTERNAL, DNX_FIELD_CS_QUAL_VT2_PEM_CS_VAR);
        data->dbal_field = DBAL_FIELD_VT2_PEM_CS_VAR;
    }
    ...
    if (DNX_FIELD_STAGE_EPMF < table->keys[0].size && DNX_FIELD_CS_QUAL_FWD_CONTEXT_PROFILE < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_EPMF, DNX_FIELD_CS_QUAL_FWD_CONTEXT_PROFILE);
        data->dbal_field = DBAL_FIELD_FWD_CONTEXT_PROFILE;
    }
