...
static shr_error_e
jr2_a0_dnx_data_mpls_general_fwd_trap_destination_set(
    int unit)
{
    int index_index;
    dnx_data_mpls_general_fwd_trap_destination_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_mpls;
    int submodule_index = dnx_data_mpls_submodule_general;
    int table_index = dnx_data_mpls_general_table_fwd_trap_destination;
    SHR_FUNC_INIT_VARS(unit);
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_mpls_general_fwd_trap_destination_t, (1 * (table->keys[0].size) + 1  ), "data of dnx_data_mpls_general_table_fwd_trap_destination");

    
    default_data = (dnx_data_mpls_general_fwd_trap_destination_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->context = DBAL_FIELD_EMPTY;
    
    for (index_index = 0; index_index < table->keys[0].size; index_index++)
    {
        data = (dnx_data_mpls_general_fwd_trap_destination_t *) dnxc_data_mgmt_table_data_get(unit, table, index_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    
    if (0 < table->keys[0].size)
    {
        data = (dnx_data_mpls_general_fwd_trap_destination_t *) dnxc_data_mgmt_table_data_get(unit, table, 0, 0);
        data->context = DBAL_FIELD_FWD1___MPLS___FWD;
    }
    if (1 < table->keys[0].size)
    {
        data = (dnx_data_mpls_general_fwd_trap_destination_t *) dnxc_data_mgmt_table_data_get(unit, table, 1, 0);
    ...
    int unit)
{
    ...
    data_index = dnx_data_mpls_general_table_fwd_trap_destination;
    table = &submodule->tables[data_index];
    table->set = jr2_a0_dnx_data_mpls_general_fwd_trap_destination_set;
