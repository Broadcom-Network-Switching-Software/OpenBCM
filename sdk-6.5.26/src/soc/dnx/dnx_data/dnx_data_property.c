    ...
    dnx_data_fabric_pipes_map_t * map)
{
    ...
    else
    {
        ...
        if ((dnx_drv_soc_property_get(unit, spn_FABRIC_PIPE_MAP, SOC_DNX_PROPERTY_UNAVAIL) != SOC_DNX_PROPERTY_UNAVAIL)
            && (dnx_drv_soc_property_get(unit, spn_FABRIC_PIPE_MAP, SOC_DNX_PROPERTY_UNAVAIL) != 0))
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "Invalid configuration of fabric_pipe_map SoC property for fabric single pipe mapping");
        }

        for (i_prio = 0; i_prio < DNX_DATA_MAX_FABRIC_CELL_NOF_PRIORITIES; ++i_prio)
        {

            if (((dnx_drv_soc_property_suffix_num_get(unit, i_prio, spn_FABRIC_PIPE_MAP, "uc", SOC_DNX_PROPERTY_UNAVAIL)
                  != 0)
                 ||
                 (dnx_drv_soc_property_suffix_num_get(unit, i_prio, spn_FABRIC_PIPE_MAP, "mc", SOC_DNX_PROPERTY_UNAVAIL)
                  != 0))
                &&
                ((dnx_drv_soc_property_suffix_num_get(unit, i_prio, spn_FABRIC_PIPE_MAP, "uc", SOC_DNX_PROPERTY_UNAVAIL)
                  != SOC_DNX_PROPERTY_UNAVAIL)
                 ||
                 (dnx_drv_soc_property_suffix_num_get(unit, i_prio, spn_FABRIC_PIPE_MAP, "mc", SOC_DNX_PROPERTY_UNAVAIL)
                  != SOC_DNX_PROPERTY_UNAVAIL)))
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             "Invalid configuration of fabric_pipe_map_uc/mc SoC properties for fabric single pipe mapping");
            }
        }
    }
    ...
    if (!dnx_data_fabric.tdm.feature_get(unit, dnx_data_fabric_tdm_blocks_dont_exist))
    {
        ...
        map->tdm_pipe =
            (dnx_data_tdm.params.mode_get(unit) ==
             TDM_MODE_NONE) ? -1 : dnx_data_fabric.pipes.valid_map_config_get(unit, i_config)->tdm_pipe;
...

    {
        ...
        if (dnx_data_fabric.general.connect_mode_get(unit) == DNX_FABRIC_CONNECT_MODE_MESH)
        {
            ...
            if (dnx_data_fabric.mesh.multicast_enable_get(unit) == 1)
            {
                if (is_tdm_supported)
                {
                    if ((map->type != soc_dnxc_fabric_pipe_map_triple_uc_mc_tdm)
...
static dnx_data_property_ucode_port_info_t ucode_ports_info[] = {
    ...
    {"D3CE",        BCM_PORT_IF_NIF_ETH,    "d3c",      8,       0},
    ...
    {"OLP",         BCM_PORT_IF_OLP,        "olp",      0,       DNX_DATA_PROPERTY_UCODE_FLAG_OPTIONAL_INDEX},
    ...
    {"OTN",         BCM_PORT_IF_OTN,        "otn",      0,       DNX_DATA_PROPERTY_UCODE_FLAG_NOT_REQUIRED_INDEX},
    ...
    dnx_data_port_static_add_ucode_port_t * ucode_port)
{
    ...
    if (propval)
    {
        ...
        while (ucode_ports_info[idx].interface_type_name)
        {
            ...
            interface_type_str_length = sal_strnlen(interface_type_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
        ...
        if (str_pointer && (*str_pointer == ':'))
        {
            ...
            prefix = "core_";
            prefix_len = sal_strnlen(prefix, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
        ...
        while (str_pointer && (*str_pointer == ':'))
        {
            ...
            prefix = "flexe_unaware";
            prefix_len = sal_strnlen(prefix, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
            if (!sal_strncasecmp(str_pointer, prefix, prefix_len))
            {
                ucode_port->is_flexe_phy = 1;
                ucode_port->is_flexe_unaware = 1;
                str_pointer += prefix_len;
                continue;
            }
            ...
            prefix = "frame_preemptable";
            prefix_len = sal_strnlen(prefix, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
            if (!sal_strncasecmp(str_pointer, prefix, prefix_len))
            {
                ucode_port->frame_preemptable = 1;
                str_pointer += prefix_len;
                continue;
            }
        }
        ...
        ucode_port->out_tm_port = dnx_drv_soc_property_port_get(unit, port, spn_PORT_OUT_TM, -1);
    }
    ...
    dnx_data_port_static_add_ext_stat_speed_t * ext_stat_speed)
{
    ...
    else
    {
        ...
        if (sal_strncmp(&tokens[1][1], "lanes", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
    ...
    dnx_data_elk_connectivity_topology_t * topology)
{
    ...
    for (kbp_port = 0; kbp_port < dnx_data_elk.general.max_ports_get(unit); ++kbp_port)
    {
        ...
        else
        {
            ...
            if (*sub_str != '\0')
            {
                ...
                sub_str++;
                if (sal_strncmp(sub_str, "core0", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
                {
                    topology->port_core_mapping[kbp_port] = 0;
                }
                else if (sal_strncmp(sub_str, "core1", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
                {
    ...
    if (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_is_egw_support_rr))
    {
        ...
        if (topology->start_lane[2] >= dnx_data_elk.general.max_lane_id_get(unit))
        {
            

            if ((temp_mapping[0] != temp_mapping[1]) || (temp_mapping[2] != temp_mapping[3]))
            {
                SHR_ERR_EXIT(SOC_E_CONFIG, "Cannot attach two KBP ports of one KBP in the different core\n");
            }
        }
    }
...
{
    char *propkey;
    char core_string[RHNAME_MAX_SIZE];
    int user_region;
        ...
        {
            {
                ...
                {
                    sal_snprintf(core_string, RHNAME_MAX_SIZE, "core%d_", core);
                    type =
                        dnx_drv_soc_property_suffix_num_get_only_suffix(unit, user_region, propkey, core_string,
                                                                        DNX_SCH_REGION_INVALID_MAPPING_MODE);

                    if (type == DNX_DATA_SCH_REGION_INVALID_MAPPING_MODE)
                    {
                        sal_snprintf(core_string, RHNAME_MAX_SIZE, "core%d", core);
                        type =
                            dnx_drv_soc_property_suffix_num_get_only_suffix(unit, user_region, propkey, core_string,
                                                                            DNX_SCH_REGION_INVALID_MAPPING_MODE);
                    }
            ...
            if (type == DNX_DATA_SCH_REGION_TYPE_CON)
            {
                
                region_type->type = type;
            }
            else if (type == DNX_DATA_SCH_REGION_TYPE_INTERDIG_1)
            {
                if (!dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_interdigitated_supported))
                {
...
typedef struct
{
    ...
    dnx_sch_flow_quartet_type_e type;

} dnx_data_property_quartet_type_info_t;


static dnx_data_property_quartet_type_info_t quartet_type_info[] = {
    {"CL_FQ_CL_FQ", DNX_SCH_FLOW_QUARTET_TYPE_CL_FQ_CL_FQ},
    {"CL_CL_FQ_FQ", DNX_SCH_FLOW_QUARTET_TYPE_CL_CL_FQ_FQ},
    {"CL_FQ_FQ_CL", DNX_SCH_FLOW_QUARTET_TYPE_CL_FQ_FQ_CL},
    {"FQ_CL_FQ_CL", DNX_SCH_FLOW_QUARTET_TYPE_FQ_CL_FQ_CL},
    {"FQ_FQ_CL_CL", DNX_SCH_FLOW_QUARTET_TYPE_FQ_FQ_CL_CL},
    {"FQ_CL_CL_FQ", DNX_SCH_FLOW_QUARTET_TYPE_FQ_CL_CL_FQ},
    {NULL, DNX_SCH_FLOW_QUARTET_TYPE_INVALID}   
};


shr_error_e
dnx_data_property_sch_flow_quartet_type_read(
    int unit,
    int core,
    int group,
    dnx_data_sch_flow_quartet_type_t * quartet_type)
{
    ...
    propkey = spn_FLOW_QUARTET_ORDER;

    if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_flex_quartet_supported))
    {
        
        region = group * dnx_data_sch.flow.nof_regions_in_quartet_order_group_get(unit);
        user_region = region + 1; 

        
        quartet_type->type = DNX_SCH_FLOW_QUARTET_TYPE_CL_FQ_CL_FQ;

        
        if (region >= dnx_data_sch.flow.nof_connectors_only_regions_get(unit))
        {
            propval = NULL;
            sal_snprintf(core_string, RHNAME_MAX_SIZE, "base_region_core%d_", core);
            propval = dnx_drv_soc_property_suffix_num_only_suffix_str_get(unit, user_region, propkey, core_string);
            ...
            if (propval == NULL)
            {
                propval = dnx_drv_soc_property_suffix_num_str_get(unit, user_region, propkey, "base_region_");
            }
            ...
            if (propval)
            {
                ...
                while (quartet_type_info[idx].name)
                {
                    type_name_str = quartet_type_info[idx].name;
                    type_name_str_length = sal_strnlen(type_name_str, 20);

                    if (!sal_strncasecmp(propval, type_name_str, type_name_str_length))
                    {
                        quartet_type->type = quartet_type_info[idx].type;
                        break;
                    }
                ...
                if (quartet_type_info[idx].name == NULL  )
                {
                    SHR_ERR_EXIT(_SHR_E_CONFIG, "Invalid quartet configuration (\"%s\")\n", propval);
                }
            }

            
            region_start = user_region + 1;
            region_end = user_region + dnx_data_sch.flow.nof_regions_in_quartet_order_group_get(unit) - 1;

            for (idx = region_start; idx < region_end; idx++)
            {
                propval = NULL;
                ...
                {
                    propval =
                        dnx_drv_soc_property_suffix_num_only_suffix_str_get(unit, idx, propkey, "base_region_core0_");
                }
                else if (core == 1)
                {
                    propval =
                        dnx_drv_soc_property_suffix_num_only_suffix_str_get(unit, idx, propkey, "base_region_core1_");
                }

                if (propval == NULL)
                {
                    propval = dnx_drv_soc_property_suffix_num_only_suffix_str_get(unit, idx, propkey, "base_region_");
                }

                if (propval)
                {
                    SHR_ERR_EXIT(_SHR_E_CONFIG,
                                 "The following region cannot be configured: %d. Configure the base region %d.\n",
                                 idx, user_region);
                }
            }
        }
    ...
    else
    {
        ...
        {
            quartet_type->type = DNX_SCH_FLOW_QUARTET_TYPE_CL_FQ_FQ_FQ;
        }
    }
...
shr_error_e
dnx_data_property_lane_map_otn_logical_lanes_val_read(
    int unit,
    int port,
    bcm_pbmp_t * val)
{
    char *propval;
    uint32 temp_array[DNX_DATA_MAX_NIF_PHYS_NOF_PHYS];
    uint8 is_hex = FALSE;
    SHR_FUNC_INIT_VARS(unit);

    
    propval = dnx_drv_soc_property_suffix_num_str_get(unit, port, spn_OTN_LANES, "");
    if (propval)
    {
        if (_shr_pbmp_parse(propval, val, temp_array, &is_hex) != 0)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "OTN lane bitmap parsing failed \n");
        }
    }
    ...
    uint32 *is_over_fabric)
{
    ...
    if (propval == DEFAULT_VAL)
    {
        ...
        if (propval == DEFAULT_VAL)
        {
            ...
            *is_over_fabric = dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_ilkn_over_fabric_only);
        }
        else
        {
    ...
    uint32 *nof_rifs)
{
    ...
    else
    {
        ...
        if (rif_id_max % (outlif_physical_bank_size / 2) != 0)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "Soc property rif_id_max must be a multiple of %d. Given 0x%08x.",
                         outlif_physical_bank_size, rif_id_max);
        }
    ...
    uint32 *enable)
{
    ...
    char *module_names[SBUSDMA_DESC_MODULE_ENABLE_NOF_MODULES] =
        ...
        "MDB_ISEM", "MDB_LEM", "MDB_OEM", "MDB_PPMC", "MDB_GLEM", "MDB_ESEM", "MDB_EXEM", "MDB_RMEP_EM", "INIT"
    };
    ...
    dnx_data_dram_general_info_timing_params_t * timing_params)
{
    ...
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_WR, TRUE, &timing_params->twr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RP, TRUE, &timing_params->trp));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RTP_S, TRUE, &timing_params->trtps));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RTP_L, TRUE, &timing_params->trtpl));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RRD_S, TRUE, &timing_params->trrds));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RRD_L, TRUE, &timing_params->trrdl));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_FAW, TRUE, &timing_params->tfaw));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RCD_WR, TRUE, &timing_params->trcdwr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RCD_RD, TRUE, &timing_params->trcdrd));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RAS, TRUE, &timing_params->tras));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RC, TRUE, &timing_params->trc));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_WTR_L, TRUE, &timing_params->twtrl));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_WTR_S, TRUE, &timing_params->twtrs));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_RTW, FALSE, &timing_params->trtw));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_CCD_S, TRUE, &timing_params->tccds));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_CCD_L, TRUE, &timing_params->tccdl));
    
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_rrefd", TRUE, &timing_params->trrefd));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_rfcsb", TRUE, &timing_params->trfcsb));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_rfc", TRUE, &timing_params->trfc));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_mod", TRUE, &timing_params->tmod));
    
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_wtrtr", TRUE, &timing_params->twtrtr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_wrwtr", TRUE, &timing_params->twrwtr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_reftr", TRUE, &timing_params->treftr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_rdtlt", TRUE, &timing_params->trdtlt));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_rcdwtr", TRUE, &timing_params->trcdwtr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_rcdrtr", TRUE, &timing_params->trcdrtr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_rcdltr", TRUE, &timing_params->trcdltr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_ltrtr", TRUE, &timing_params->tltrtr));
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_ltltr", TRUE, &timing_params->tltltr));
...
shr_error_e
dnx_data_property_dram_general_info_tertw_read(
    int unit,
    uint32 *tertw)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, spn_EXT_RAM_T_E_RTW, FALSE, tertw));
    if ((int) *tertw != -1)
    {
        SHR_MAX_VERIFY(*tertw, 0x3F, _SHR_E_PARAM, "ext_ram_t_e_rtw is out of bounds.\n");
    }
    ...
    uint32 *write_latency)
{
    ...
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_write_latency", TRUE, write_latency));
    if ((dnx_data_dram.general_info.dram_info_get(unit)->dram_bitmap != 0))
    {
    ...
    uint32 *read_latency)
{
    ...
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_read_latency", TRUE, read_latency));
    if ((dnx_data_dram.general_info.dram_info_get(unit)->dram_bitmap != 0))
    {
    ...
    uint32 *crc_write_latency)
{
    ...
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_crc_wr_latency", TRUE, crc_write_latency));
    if ((dnx_data_dram.general_info.dram_info_get(unit)->dram_bitmap != 0))
    {
    ...
    uint32 *crc_read_latency)
{
    ...
    SHR_IF_ERR_EXIT(dnx_gddr6_dram_timing_param_get(unit, "ext_ram_t_crc_rd_latency", TRUE, crc_read_latency));
    if ((dnx_data_dram.general_info.dram_info_get(unit)->dram_bitmap != 0))
    {
...
shr_error_e
dnx_data_property_dram_general_info_dram_bitmap_internal_read(
    int unit,
    uint32 *dram_bitmap_internal)
{
    uint32 supported_dram_bitmap[1] = { dnx_data_dram.general_info.supported_dram_bitmap_get(unit) };
    ...
    int dram_index;
    int is_gddr = dnx_data_dram.gddr6.feature_get(unit, dnx_data_dram_gddr6_is_supported);
    SHR_FUNC_INIT_VARS(unit);

    *dram_bitmap_internal = dnx_drv_soc_property_get(unit, spn_EXT_RAM_ENABLED_BITMAP, *dram_bitmap_internal);
    SHR_MAX_VERIFY(*dram_bitmap_internal, all_drams_bitmap, _SHR_E_PARAM,
                   "ext_ram_enabled_bitmap 0x%x is out of bounds (0x0-0x%x).\n", *dram_bitmap_internal,
                   all_drams_bitmap);

    
    SHR_BIT_ITER(dram_bitmap_internal, nof_drams, dram_index)
    {
        if (!SHR_IS_BITSET(supported_dram_bitmap, dram_index)
            || (is_gddr
                && !SHR_IS_BITSET(dram_bitmap_internal,
                                  dnx_data_dram.gddr6.master_phy_get(unit, dram_index)->master_dram_index)))
        {
    ...
    uint32 *vsq_f_default)
{
    ...
    reduced_lane =
        dnx_drv_soc_property_get(unit, spn_REDUCED_PRIORITY_GROUPS_LANE,
                                 dnx_data_nif.phys.max_phys_in_core_get(unit) - 1);
...
shr_error_e
dnx_data_property_mdb_em_flush_max_supported_payload_read(
    int unit,
    uint32 *flush_max_supported_payload)
{
    uint32 flush_max_supported_payload_tmp = 64;
    SHR_FUNC_INIT_VARS(unit);

    *flush_max_supported_payload = flush_max_supported_payload_tmp;
...
shr_error_e
dnx_data_property_macsec_general_is_macsec_enabled_read(
    int unit,
    uint32 *is_macsec_enabled)
{
    ...
    {
        int macsec_block_exists =
            dnx_data_macsec.general.feature_get(unit, dnx_data_macsec_general_macsec_block_exists);
        int macsec_enabled = dnx_drv_soc_property_get(unit, spn_MACSEC_ENABLED, *is_macsec_enabled);

        if (macsec_enabled > 1)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "Invalid value %u for is_macsec_enabled. Value must be only 0 or 1.", *is_macsec_enabled);
        }

        *is_macsec_enabled = (macsec_block_exists & macsec_enabled);
    }
#else
    *is_macsec_enabled = 0;
    SHR_EXIT();
...
shr_error_e
dnx_data_property_port_static_add_flr_config_read(
    int unit,
    int port,
    dnx_data_port_static_add_flr_config_t * flr_config)
{
    ...
    propval = dnx_drv_soc_property_port_get_str(unit, port, spn_PHY_INIT_FLR_ENABLE);
    
    if (propval)
    {
        str_pointer = propval;
        
        flr_config->flr_fw_support = sal_ctoi(str_pointer, &str_pointer_end);

        if (str_pointer == str_pointer_end)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "The FLR enable info is missed!\n");
        }
        SHR_BOOL_VERIFY(flr_config->flr_fw_support, _SHR_E_CONFIG, "Invalid value for FLR enable, must be 1 or 0.\n");
        
        str_pointer = str_pointer_end;

        if (*str_pointer == '\0')
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "The FLR timeout info is missed!\n");
        }
        else
        {
            if (*str_pointer != ':')
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "Bad config string. Please use \':\' instead of \"%s\"!\n", str_pointer);
            }
            str_pointer++;
        }
        flr_config->timeout = sal_ctoi(str_pointer, &str_pointer_end);

        if (str_pointer == str_pointer_end)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "The FLR timeout info is missed!\n");
        }
        SHR_RANGE_VERIFY(flr_config->timeout, bcmPortFlrTimeoutInvaild, (bcmPortFlrTimeoutCount - 1), _SHR_E_CONFIG,
                         "Invalid value for FLR timeout.\n");

        str_pointer = str_pointer_end;

        if (*str_pointer != '\0')
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "Bad configure string!\n");
        }
    }
exit:
    SHR_FUNC_EXIT;
