    ...
    else
    {
        ...
        memory_feature_jr2 = ((entry_handle_regular_fields->value_field_ids[field_pos] != DBAL_FIELD_EMPTY)
                              ...
                              && (entry_handle->table->
                                  ...
                                  entry_handle->table->hl_mapping_multi_res[entry_handle->
                                                                            cur_res_type].l2p_hl_info
                                  [DBAL_HL_ACCESS_MEMORY].l2p_fields_info[iter - 1].field_id) && ((SOC_IS_JERICHO2(unit)
                                                                                                   && *entry_handle->
                                                                                                   table->
    ...
    uint32 decode)
{
    ...
    switch (table_id)
    {
        ...
        case DBAL_TABLE_MACSEC_INGRESS_SVTAG_CPU_FLEX_MAP_TCAM:
        {
            first_tcam_key = DBAL_FIELD_SRC_PBMP;
            break;
        }
    }
    ...
    int *continue_regular_process)
{
    ...
    *continue_regular_process = FALSE;
    ...
    switch (entry_handle->table_id)
    {
        ...
        case DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_EPMF:
        {
            *continue_regular_process = TRUE;
            break;
        }
