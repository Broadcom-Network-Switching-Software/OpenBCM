    ...
    uint8 is_dbal_error)
{
    ...
    if (((ptr_handle->table->access_method == DBAL_ACCESS_METHOD_KBP)) && (ptr_handle->nof_ranged_fields > 0))
    {
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT(ptr_handle->key_field_ranges, ptr_handle->nof_ranged_fields * sizeof(uint32),
                                    ptr_handle->table->table_name, "%s%s%s", EMPTY, EMPTY, EMPTY);

        sal_memcpy(ptr_handle->key_field_ranges, range_info_ptr_orig, ptr_handle->nof_ranged_fields * sizeof(uint32));

        dbal_handles_range_ptr_store(unit, ptr_handle->key_field_ranges);
    }
    ...
    uint8 is_comparison)
{
    ...
    SHR_IF_ERR_EXIT(dbal_handles_range_ptr_free_all(unit));
    ...
    uint8 is_dbal_error)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Handle is null");
    }
