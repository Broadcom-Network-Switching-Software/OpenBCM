...
typedef enum
{
    DBAL_ENCODING_TYPE_XY,
    DBAL_ENCODING_TYPE_LPT,
    DBAL_NOF_ENCODING_TYPES
} dbal_encoding_types_t;
...
#define DBAL_FIELD_ERR_HANDLE(is_error, string_to_save, inst_idx)                                 \
do                                                                                                \
{                                                                                                 \
    if(is_error)                                                                                  \
    {                                                                                             \
        ...
        {                                                                                         \
            ...
            string_to_save, dbal_field_with_inst_to_string(unit, field_id, inst_idx, field_string), entry_handle->table->table_name,                        \
        ...
        DBAL_ENCODE_FIELD_ID_WITH_INSTANCE(entry_handle->error_info.field_and_instance, field_id, inst_idx);   \
...
{                                                                                                 \
    ...
    {                                                                                             \
        ...
        DBAL_ENCODE_FIELD_ID_WITH_INSTANCE(entry_handle->error_info.field_and_instance, field_id, table_field->inst_idx);\
...
{ \
    ...
    { \
        ...
        { \
            SHR_ALLOC_SET_ZERO_ERR_EXIT((_dst_entry_handle).user_output_info, \
                    sizeof(dbal_user_output_info_t) * DBAL_TABLE_MAX_NUM_OF_RESULT_FIELDS_PER_RES_TYPE, \
                    (_dst_entry_handle).table->table_name, "%s%s%s",EMPTY,EMPTY,EMPTY) ;\
        } \
...
#define DBAL_ENTRY_HANDLE_FREE(_entry_handle) \
do \
{\
    if (_entry_handle != NULL)\
    {\
        SHR_FREE(_entry_handle->key_field_ranges);\
    }\
    SHR_FREE(_entry_handle);\
} while (0)

#define DBAL_ENTRY_HANDLE_KEY_FIELD_RANGES_COPY(_src_entry_handle, _dst_entry_handle) \
do \
{ \
    if ((_src_entry_handle).key_field_ranges != NULL) \
    { \
        { \
            (_dst_entry_handle).key_field_ranges = NULL;\
            SHR_ALLOC_SET_ZERO_ERR_EXIT((_dst_entry_handle).key_field_ranges, \
                            DBAL_TABLE_MAX_NUM_OF_KEY_FIELDS * sizeof(uint32), \
                        (_dst_entry_handle).table->table_name, "%s%s%s",EMPTY,EMPTY,EMPTY) ;\
        } \
        sal_memcpy((_dst_entry_handle).key_field_ranges, (_src_entry_handle).key_field_ranges, \
                                        DBAL_TABLE_MAX_NUM_OF_KEY_FIELDS * sizeof(uint32)); \
    } \
} while (0)
...
shr_error_e dbal_handles_range_ptr_free_all(
    int unit);
...
shr_error_e dbal_handles_range_ptr_store(
    int unit,
    uint32 *range_info_ptr);
...
shr_error_e dbal_entry_direct_is_valid_internal(
    int unit,
    dbal_entry_handle_t * entry_handle,
    int *is_valid);
...
shr_error_e dbal_tables_field_id_update(
    int unit,
    dbal_tables_e table_id,
    dbal_fields_e field_id,
    uint8 is_key,
...
void dbal_internal_key_field_ranges_alloc(
    int unit,
    dbal_entry_handle_t * entry_handle);
...
shr_error_e dbal_internal_encode(
    int unit,
    uint32 *in_data,
    uint32 *in_mask,
    uint32 *out_data,
    uint32 *out_mask,
    uint32 length,
    dbal_encoding_types_t type);

shr_error_e dbal_internal_decode(
    int unit,
    uint32 *in_data,
    uint32 *in_mask,
    uint32 *out_data,
...
shr_error_e dbal_db_init_table_interface_key_post_process(
    int unit,
    dbal_logical_table_t * table);

shr_error_e dbal_db_init_table_interface_result_post_process(
    int unit,
    dbal_tables_e table_id,
    uint32 result_lsb_padding,
    dbal_logical_table_t * table);

shr_error_e dbal_db_init_table_hl_direct_post_process(
    int unit,
    dbal_logical_table_t * table);
...
shr_error_e dbal_entry_direct_is_default_internal(
    int unit,
    dbal_entry_handle_t * entry_handle,
    int *is_default);
