...
err_code_t merlin7_pcieg3_core_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    case PWRDN_DEEP:
        EFUN(merlin7_pcieg3_core_pwrdn(sa__, PWRDN));
        break;
    default:
...
err_code_t merlin7_pcieg3_lane_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    case PWR_ON:
        ...
        EFUN(merlin7_pcieg3_lane_pwrdn(sa__, PWR_ON_RX));
        EFUN(merlin7_pcieg3_lane_pwrdn(sa__, PWR_ON_TX));
        break;
    case PWR_ON_TX:
    ...
    case PWR_ON_RX:
        ...
        break;
    case PWRDN:
        EFUN(merlin7_pcieg3_lane_pwrdn(sa__, PWRDN_TX));
        EFUN(merlin7_pcieg3_lane_pwrdn(sa__, PWRDN_RX));
        break;
    case PWROFF_DEEP: /* PWROFF_DEEP is identical to PWRDN_DEEP for lane pwrdn. FALL THROUGH */
    case PWRDN_DEEP:
        EFUN(merlin7_pcieg3_lane_pwrdn(sa__, PWRDN_TX_DEEP));
        EFUN(merlin7_pcieg3_lane_pwrdn(sa__, PWRDN_RX_DEEP));
...
err_code_t merlin7_pcieg3_pwrdn_all(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    INIT_SRDS_ERR_CODE
    ...
    for (lane = 0; lane < lanes_per_core; ++lane) {
        ...
        EFUN(merlin7_pcieg3_lane_pwrdn(sa__, mode));
    }
    EFUN(merlin7_pcieg3_acc_set_lane(sa__, lane_orig));
    {
      uint8_t pll_orig = merlin7_pcieg3_acc_get_pll_idx(sa__);
      uint8_t pll;
      for (pll = 0; pll < NUM_PLLS; ++pll) {
        EFUN(merlin7_pcieg3_acc_set_pll_idx(sa__, pll));
        EFUN(merlin7_pcieg3_core_pwrdn(sa__, mode));
      }
