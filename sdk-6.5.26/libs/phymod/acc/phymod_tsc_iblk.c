...
/*
 * TBD: Need to update the block read and write functions.
 */ 
int
phymod_tscd_iblk_write(const phymod_access_t *pa, uint32_t addr, uint32_t data)
{
    int ioerr = 0;
    uint32_t lane_map, hwlane, is_hwsupport = 1;
...
int
phymod_tscd_iblk_read(const phymod_access_t *pa, uint32_t addr, uint32_t *data)
{
    int ioerr = 0;
    uint32_t devad = (addr >> 16) & 0xf;
    uint32_t blkaddr, regaddr;
