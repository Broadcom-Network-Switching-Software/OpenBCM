...
extern unsigned char  falcon16_v1l4p1_flr_ucode_image[];
extern unsigned short falcon16_v1l4p1_flr_ucode_crc;
extern unsigned short falcon16_v1l4p1_flr_ucode_len;
...
STATIC
int _tscf16_gen3_core_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config)
{
    uint32_t ucode_len;
    unsigned char *ucode_image;
    phymod_firmware_load_method_t load_method = init_config->firmware_load_method;
    phymod_firmware_loader_f fw_loader = init_config->firmware_loader;

    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        ucode_image = falcon16_v1l4p1_flr_ucode_image;
        ucode_len = falcon16_v1l4p1_flr_ucode_len;
    } else {
        ucode_image = falcon16_v1l4p1_ucode;
        ucode_len = falcon16_v1l4p1_ucode_len;
    }

    switch(load_method){
    case phymodFirmwareLoadMethodInternal:
        PHYMOD_IF_ERR_RETURN(falcon16_v1l4p1_ucode_mdio_load(&core->access, ucode_image, ucode_len));
        break;
    case phymodFirmwareLoadMethodExternal:
        ...
        PHYMOD_IF_ERR_RETURN(fw_loader(core, ucode_len, ucode_image));
        PHYMOD_IF_ERR_RETURN
...
int _tscf16_gen3_core_init_post_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        ...
        rv = falcon16_v1l4p1_ucode_load_verify(&core_copy.access, (uint8_t *) &ucode_image, ucode_len);
        if (rv != PHYMOD_E_NONE) {
    ...
    if (PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_GET(init_config)) {
        ...
        PHYMOD_IF_ERR_RETURN(
            falcon16_v1l4p1_start_ucode_crc_calc(&core_copy.access, ucode_len));
#endif
    }
...
int _tscf16_gen3_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    /* 4. Micro code load */
    rv = _tscf16_gen3_core_firmware_load(&core_copy, init_config);
    if (rv != PHYMOD_E_NONE) {
...
int _tscf16_gen3_core_init_pass2(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    if (PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_GET(init_config)) {
...
#else
        {
            uint16_t ucode_crc;
            if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
                ucode_crc = falcon16_v1l4p1_flr_ucode_crc;
            } else {
                ucode_crc = falcon16_v1l4p1_ucode_crc;
            }
            PHYMOD_IF_ERR_RETURN(
                falcon16_v1l4p1_check_ucode_crc(&core_copy.access, ucode_crc, 200));
        }
#endif
    }
...
{
    int lane;
    uint32_t ts_enable, pcs_enable;
    phymod_phy_access_t phy_copy;
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    PHYMOD_IF_ERR_RETURN
        (tefmod16_gen3_1588_ts_enable_get(&phy_copy.access, &ts_enable));

    /* 1588 timestamp is per core in TSCF.
     * We should NOT be toggling the TS_EN unless ALL ports at the PCS level are in the reset/disable state (speed control bit = 0).
     */
    if (ts_enable != enable) {
        for (lane = 0; lane < TSCF16_GEN3_NOF_LANES_IN_CORE; lane++) {
            phy_copy.access.lane_mask = 1 << lane;
            PHYMOD_IF_ERR_RETURN (tefmod16_gen3_enable_get(&phy_copy.access, &pcs_enable));
            if (pcs_enable) {
                PHYMOD_DEBUG_ERROR(("ERROR :: PCS lane %d is not disabled. All PCS lanes should be disabled before changing TSCF TS.\n", lane));
                return PHYMOD_E_CONFIG;
            }
        }
    }
...
int tscf16_gen3_timesync_adjust_set(const phymod_phy_access_t* phy, uint32_t flags,
                                    const phymod_timesync_adjust_config_info_t *config)
{
    ...
    phymod_timesync_compensation_mode_t ts_am_norm_mode = config->am_norm_mode;
...
STATIC
int _tscf16_gen3_flr_timeout_phymod_to_falcon16_v1l4p1(phymod_phy_flr_timeout_t timeout_time, enum falcon16_v1l4p1_flr_timeout_enum *falcon16_v1l4p1_timeout_time)
{
    switch (timeout_time) {
    case phymodFlrTimeout50ms:
        *falcon16_v1l4p1_timeout_time = FALCON16_V1L4P1_FLR_TIMEOUT_50MS;
        break;
    case phymodFlrTimeout75ms:
        *falcon16_v1l4p1_timeout_time = FALCON16_V1L4P1_FLR_TIMEOUT_75MS;
        break;
    case phymodFlrTimeout100ms:
        *falcon16_v1l4p1_timeout_time = FALCON16_V1L4P1_FLR_TIMEOUT_100MS;
        break;
    case phymodFlrTimeout150ms:
        *falcon16_v1l4p1_timeout_time = FALCON16_V1L4P1_FLR_TIMEOUT_150MS;
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for falcon16_v1l4p1 %u"), timeout_time));
        return PHYMOD_E_PARAM;
    }
    return PHYMOD_E_NONE;
}
...
int tscf16_gen3_phy_flr_config_set(const phymod_phy_access_t* phy, uint32_t enable, phymod_phy_flr_timeout_t timeout_time)
{
    int start_lane, num_lane, i;
    phymod_phy_access_t phy_copy;
    uint32_t pcs_enable, lane_enable;
    enum falcon16_v1l4p1_flr_timeout_enum falcon16_v1l4p1_timeout_time;
    ...
    if (enable) {
        PHYMOD_IF_ERR_RETURN
            (_tscf16_gen3_flr_timeout_phymod_to_falcon16_v1l4p1(timeout_time, &falcon16_v1l4p1_timeout_time));
    } else {
    ...
    PHYMOD_IF_ERR_RETURN
        (falcon16_v1l4p1_lane_soft_reset_release_get(&phy_copy.access, &lane_enable));

    /*next check if PCS lane is in reset */
    PHYMOD_IF_ERR_RETURN
        (tefmod16_gen3_enable_get(&phy_copy.access, &pcs_enable));

    /* disable pcs lane if pcs lane not in rset */
    if (pcs_enable) {
        phy_copy.access.lane_mask = 1 << start_lane;
        PHYMOD_IF_ERR_RETURN
            (tefmod16_gen3_disable_set(&phy_copy.access));
    }
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (falcon16_v1l4p1_configure_fast_link_recovery(&phy_copy.access, enable,
                                                        enable, falcon16_v1l4p1_timeout_time, 0 , 0));
    }
    ...
    if (pcs_enable) {
        phy_copy.access.lane_mask = 1 << start_lane;
        PHYMOD_IF_ERR_RETURN
            (tefmod16_gen3_enable_set(&phy_copy.access));
    }
...
STATIC
int _tscf16_gen3_flr_timeout_falcon16_v1l4p1_to_phymod(enum falcon16_v1l4p1_flr_timeout_enum falcon16_v1l4p1_timeout_time, phymod_phy_flr_timeout_t *timeout_time)
{
    switch (falcon16_v1l4p1_timeout_time) {
        case FALCON16_V1L4P1_FLR_TIMEOUT_50MS:
            *timeout_time = phymodFlrTimeout50ms;
            break;
        case FALCON16_V1L4P1_FLR_TIMEOUT_75MS:
            *timeout_time = phymodFlrTimeout75ms;
            break;
        case FALCON16_V1L4P1_FLR_TIMEOUT_100MS:
            *timeout_time = phymodFlrTimeout100ms;
            break;
        case FALCON16_V1L4P1_FLR_TIMEOUT_150MS:
            *timeout_time = phymodFlrTimeout150ms;
            break;
        default:
            PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for falcon16_v1l4p1 %u"), falcon16_v1l4p1_timeout_time));
            return PHYMOD_E_PARAM;
    }
    return PHYMOD_E_NONE;
}
...
int tscf16_gen3_phy_flr_config_get(const phymod_phy_access_t* phy, uint32_t *enable, phymod_phy_flr_timeout_t *timeout_time)
{
    ...
    uint8_t enable_2ms_yield = 0, filter_count = 0;
    enum falcon16_v1l4p1_flr_timeout_enum falcon16_v1l4p1_timeout_time;
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (falcon16_v1l4p1_get_configure_fast_link_recovery(&phy_copy.access,
                                                          &flr_enable,
                                                          &flr_timeout_en,
                                                          &falcon16_v1l4p1_timeout_time,
                                                          &enable_2ms_yield,
                                                          &filter_count));
    ...
    *enable = flr_enable ? 1 : 0;
    if (flr_timeout_en) {
        PHYMOD_IF_ERR_RETURN
            (_tscf16_gen3_flr_timeout_falcon16_v1l4p1_to_phymod(falcon16_v1l4p1_timeout_time,
                                                                timeout_time));
    } else {
