...
int tscf_gen3_timesync_adjust_set(const phymod_phy_access_t* phy,
                                  uint32_t flags,
                                  const phymod_timesync_adjust_config_info_t *config)
{
    ...
    if ((no_of_vl > 1) && (config->am_norm_mode == 0)) {
    ...
    if ((no_of_vl == 1) && (config->am_norm_mode != 0)) {
        PHYMOD_DEBUG_WARN(("Warning: phy_addr 0x%x, lane mask 0x%x, virtual lane number %d. The ts adjust mode is %s (%d), it should be None (0).\n",
            phy_copy.access.addr, phy_copy.access.lane_mask, no_of_vl, config->am_norm_mode == 1 ? "EarliestLane" : "Latestlane", config->am_norm_mode));
    }

    /* Program deskew adjustment for multi-lane ports */
    if (config->am_norm_mode != phymodTimesyncCompensationModeNone) {
        PHYMOD_IF_ERR_RETURN
            (tefmod_gen3_1588_rx_deskew_set(&phy_copy.access, config->am_norm_mode));
    }
