...
*/
err_code_t
blackhawk_tsc_ext_loop_timing_tx(phymod_access_t *sa__, uint8_t enable)
{
...
err_code_t blackhawk_tsc_ext_loop_timing_rx(phymod_access_t *sa__, uint8_t enable)
{
    uint8_t is_enable = enable ? 0x1 : 0x0;
...
err_code_t
blackhawk_rx_pmd_lock_status_get(phymod_access_t *sa__, uint32_t *pmd_lock, uint32_t *pmd_lock_change)
{
...
err_code_t
blackhawk_signal_detect_status_get(phymod_access_t *sa__, uint32_t *sd_bmp, uint32_t *sd_change_bmp)
{
    int start_lane, num_lane, lane_offset;
    phymod_access_t phy_access_copy;
...
err_code_t
blackhawk_lane_tx_pll_selection_set(phymod_access_t *sa__, uint32_t pll_index) {
    err_code_t __err = ERR_CODE_NONE;

    __err = (uint32_t) wr_tx_pll_select(pll_index);
    if (__err) {
        return (__err);
    }
...
err_code_t
blackhawk_lane_rx_pll_selection_set(phymod_access_t *sa__, uint32_t pll_index) {
    err_code_t __err = ERR_CODE_NONE;

    __err = (uint32_t) wr_rx_pll_select(pll_index);
    if (__err) {
        return (__err);
    }
...
err_code_t
blackhawk_rx_dc_offset_get(phymod_access_t *sa__, uint32_t *dc_offset)
{
    err_code_t __err;
    __err = ERR_CODE_NONE;
