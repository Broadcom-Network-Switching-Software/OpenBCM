       ...
       PHYMOD_IF_ERR_RETURN
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            default:
                ...
                if (i == 0) {
                    PHYMOD_IF_ERR_RETURN
                        (falcon16_tsc_display_diag_data(&phy_copy.access, SRDS_DIAG_CORE|SRDS_DIAG_UC_CORE|SRDS_DIAG_LANE|SRDS_DIAG_REG_CORE|SRDS_DIAG_UC_LANE|SRDS_DIAG_EYE));
                } else {
                    PHYMOD_IF_ERR_RETURN
                        (falcon16_tsc_display_diag_data(&phy_copy.access, SRDS_DIAG_LANE|SRDS_DIAG_REG_CORE|SRDS_DIAG_UC_LANE|SRDS_DIAG_EYE));
                }
...
int tscf16_phy_fec_error_bits_counter_get(const phymod_phy_access_t* phy, uint32_t* count)
{
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (tefmod16_fec_cl91_bit_error_counter_get(&phy_copy.access, &local_count));

     /* Check overflow */
     if (local_count > 0xffffffff) {
        *count = 0xffffffff;
     } else {
         *count = local_count;
     }
                                    ...
                                    phymod_fec_error_mask_t bit_error_mask)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (tefmod16_FEC_get(&phy->access, TEFMOD16_CL91, &cl91_en));
    PHYMOD_IF_ERR_RETURN
        (tefmod16_FEC_get(&phy->access, TEFMOD16_CL108, &cl108_en));
