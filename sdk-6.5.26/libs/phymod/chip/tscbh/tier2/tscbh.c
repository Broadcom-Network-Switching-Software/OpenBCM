...
int _tscbh_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
     ...
     */
    for (i = 0; i < TBHMOD_TS_400G_MEM_SIZE; i++) {
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemTxLkup1588400G, i, init_data));
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemRxLkup1588400G, i, init_data));
    }

    for (i = 0; i < TBHMOD_TS_MPP_MEM_SIZE; i++) {
        PHYMOD_IF_ERR_RETURN
...
/*Set timesync adjust*/
int tscbh_timesync_adjust_set(const phymod_phy_access_t* phy, uint32_t flags,
                              const phymod_timesync_adjust_config_info_t *config)
{
    ...
    int num_retry = 0;
    int rv = 0;
    ts_table_entry_mem *rx_shadow_mem;
    ts_table_entry_mem *tx_shadow_mem;
    ...
    tx_shadow_mem = (ts_table_entry_mem *)NULL;
    rx_shadow_mem = (ts_table_entry_mem *)NULL;

    if (num_lane == 8) {
        tx_mem = phymodMemTxLkup1588400G;
        rx_mem = phymodMemRxLkup1588400G;
        intr_type = phymodIntrEccRx1588400g;
        tx_shadow_mem = (ts_table_entry_mem *)config->tx_lkup_1588_mem_400g;
        rx_shadow_mem = (ts_table_entry_mem *)config->rx_lkup_1588_mem_400g;
        ...
        tx_shadow_mem = (ts_table_entry_mem *)config->tx_lkup_1588_mem_mpp0;
        rx_shadow_mem = (ts_table_entry_mem *)config->rx_lkup_1588_mem_mpp0;
        ...
        tx_shadow_mem = (ts_table_entry_mem *)config->tx_lkup_1588_mem_mpp1;
        rx_shadow_mem = (ts_table_entry_mem *)config->rx_lkup_1588_mem_mpp1;
    ...
    */
    ...
    for (i = 0; i < (int)ts_table_size; i++) {
        ...
        if (tx_shadow_mem != NULL) {
            sal_memcpy(&tx_shadow_mem[i + mem_offset][0], &(*ts_tx_entry)[i][0], sizeof(uint32_t)*3);
        }
    }
    ...
    if (config->am_norm_mode == 0x1) {
        ...
        for (i = 0; i < (int)ts_table_size; i++) {
            ...
            if (rx_shadow_mem != NULL) {
                sal_memcpy(&rx_shadow_mem[i + mem_offset][0], &ts_update_table[i][0], sizeof(uint32_t)*3);
            }
        }
    } else {
        /* 2-stage SW WAR for RX 1588 memory accessing:
                       ...
                       phymod_intr_info_t *mem_info)
{
    ...
    uint32_t *tscbh_am_table_entry;
    uint32_t *tscbh_um_table_entry;
    uint32_t ts_table_entry_error_marker[TBHMOD_TS_ENTRY_SIZE]={0x0badecc0,0x0badecc0, 0xbecc};
    int index;
    ...
    switch (type) {
        ...
        case phymodIntrEccRx1588400g:
            if (intr_status.is_2b_err) {
                ...
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemRxLkup1588400G, index, ts_table_entry_error_marker));
        ...
        case phymodIntrEccRx1588Mpp1:
           if (intr_status.is_2b_err) {
                ...
                mem_info->mem_addr = index;
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemRxLkup1588Mpp1, index, ts_table_entry_error_marker));
            } else if (intr_status.is_1b_err) {
                index = intr_status.err_addr;
                if (index >= TBHMOD_TS_MPP_MEM_SIZE) {
                    return PHYMOD_E_FAIL;
        ...
        case phymodIntrEccRx1588Mpp0:
           if (intr_status.is_2b_err) {
                ...
                mem_info->mem_addr = index;
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemRxLkup1588Mpp0, index, ts_table_entry_error_marker));
            } else if (intr_status.is_1b_err) {
                index = intr_status.err_addr;
                if (index >= TBHMOD_TS_MPP_MEM_SIZE) {
                    return PHYMOD_E_FAIL;
        ...
        case phymodIntrEccTx1588400g:
            if (intr_status.is_2b_err) {
                ...
                mem_info->mem_addr = index;
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemTxLkup1588400G, index, ts_table_entry_error_marker));
            } else if (intr_status.is_1b_err) {
                index = intr_status.err_addr;
                if (index >= TBHMOD_TS_DEFAULT_TABLE_SIZE) {
                    return PHYMOD_E_FAIL;
        ...
        case phymodIntrEccTx1588Mpp1:
          if (intr_status.is_2b_err) {
                ...
                mem_info->mem_addr = index;
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemTxLkup1588Mpp1, index, ts_table_entry_error_marker));
            } else  if (intr_status.is_1b_err) {
                index = intr_status.err_addr;
                if (index >= TBHMOD_TS_MPP_MEM_SIZE) {
                    return PHYMOD_E_FAIL;
        ...
        case phymodIntrEccTx1588Mpp0:
           if (intr_status.is_2b_err) {
                ...
                mem_info->mem_addr = index;
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemTxLkup1588Mpp0, index, ts_table_entry_error_marker));
            } else if (intr_status.is_1b_err) {
                index = intr_status.err_addr;
                if (index >= TBHMOD_TS_MPP_MEM_SIZE) {
                    return PHYMOD_E_FAIL;
...
STATIC
int _tscbh_flr_timeout_blackhawk_to_phymod(enum blackhawk_tsc_flr_timeout_enum blackhawk_timeout_time, phymod_phy_flr_timeout_t *timeout_time)
{
    switch (blackhawk_timeout_time) {
    case BLACKHAWK_TSC_FLR_TIMEOUT_50MS:
        *timeout_time = phymodFlrTimeout50ms;
        break;
    case BLACKHAWK_TSC_FLR_TIMEOUT_75MS:
        *timeout_time = phymodFlrTimeout75ms;
        break;
    case BLACKHAWK_TSC_FLR_TIMEOUT_100MS:
        *timeout_time = phymodFlrTimeout100ms;
        break;
    case BLACKHAWK_TSC_FLR_TIMEOUT_150MS:
        *timeout_time = phymodFlrTimeout150ms;
        break;
    case BLACKHAWK_TSC_FLR_TIMEOUT_250MS:
        *timeout_time = phymodFlrTimeout250ms;
        break;
    case BLACKHAWK_TSC_FLR_TIMEOUT_500MS:
        *timeout_time = phymodFlrTimeout500ms;
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for blackhawk %u"), blackhawk_timeout_time));
        return PHYMOD_E_PARAM;
    }
    return PHYMOD_E_NONE;
}
...
int tscbh_phy_flr_config_get(const phymod_phy_access_t* phy, uint32_t *enable, phymod_phy_flr_timeout_t *timeout_time)
{
    ...
    phy_copy.access.lane_mask = 0x1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (blackhawk_tsc_get_configure_fast_link_recovery(&phy_copy.access,
                                                        &flr_enable,
                                                        &flr_timeout_en,
                                                        &blackhawk_timeout_time));
    if (flr_enable) {
        *enable = 1;
    } else {
        *enable = 0;
    ...
    if (flr_timeout_en) {
        PHYMOD_IF_ERR_RETURN
            (_tscbh_flr_timeout_blackhawk_to_phymod(blackhawk_timeout_time,
                                                    timeout_time));
    } else {
