...
int _tscbh_gen3_core_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config)
{
    ...
    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        blackhawk_ucode = blackhawk7_v1l8p1_flr_ucode_get();
        blackhawk_ucode_len = BLACKHAWK7_V1L8P1_FLR_UCODE_IMAGE_SIZE;
    } else {
...
static int _tscbh_gen3_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_INTERNAL_is_flr_supported(&phy_copy.access, &flr_support));
                                   ...
                                   phymod_phy_pll_state_t* new_pll_state)
{
   ...
   for (i = 0; i < num_lane; i++) {
    ...
    if (osr_2p5_is_required) {
        osr_mode = TBHMOD_GEN3_OS_MODE_2p5;
    } else {
        osr_mode = tbhmod_gen3_mapped_speed_get_osmode(mapped_speed_id, ref_clk);
    }

   /* Set osr_mode on register in PHYSIM environment. */
    if(PHYMOD_ACC_F_PHYSIM_GET(&pm_phy_copy.access)) {
        ...
        /* Set uC RAM's variable when running on HW */
        for ( i = 0; i < num_lane; i++) {
            pm_phy_copy.access.lane_mask =  1 << (start_lane + i);
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
                continue;
            ...
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p1_set_osr_mode(&pm_phy_copy.access,
                                             (enum blackhawk7_v1l8p1_osr_mode_enum)osr_mode));
        }
    }
                                        ...
                                        phymod_phy_speed_config_t *speed_config)
{
    ...
    int num_lane, osr_mode_value = 0, start_lane, lanes, an_en= 0, an_done = 0;
    enum blackhawk7_v1l8p1_osr_mode_enum tx_osr_mode, rx_osr_mode;
   ...
   /* Get osr_mode from register that is programmed by _osr_mode_set() in PHYSIM environment. */
    if(PHYMOD_ACC_F_PHYSIM_GET(&phy_copy.access)) {
        PHYMOD_IF_ERR_RETURN
             (blackhawk7_v1l8p1_osr_mode_get(&phy_copy.access, &osr_mode_value));
    } else {
        PHYMOD_IF_ERR_RETURN
           (tbhmod_gen3_autoneg_status_get(&phy_copy.access, &an_en, &an_done));
        if (an_en && !an_done) {
            PHYMOD_DEBUG_ERROR(("WARNING :: AN is not resolved yet, speed may not be accurate on 0x%x\n",
                                 phy_copy.access.addr));
        }
        /* Get osr_mode from uC RAM's OSR control status when running on HW */
        phy_copy.access.lane_mask = 1 << start_lane;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p1_get_osr_mode(&phy_copy.access, &tx_osr_mode, &rx_osr_mode));
    ...
    if (firmware_lane_config.ForcePAM4Mode) {
        ...
        if (osr_mode_value == TSCBH_GEN3_PMD_OSR_2P5) {
            data_rate_lane = 10000;
        } else {
            data_rate_lane = data_rate_lane >> osr_mode_value;
        }
    }
...
int tscbh_gen3_phy_cl72_set(const phymod_phy_access_t* phy, uint32_t cl72_en)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_INTERNAL_is_flr_supported(&pm_phy_copy.access, &flr_support));
    if (flr_support) {
...
int _tscbh_gen3_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        ucode.ucode_size = BLACKHAWK7_V1L8P1_FLR_UCODE_IMAGE_SIZE;
        ucode.stack_size = BLACKHAWK7_V1L8P1_FLR_UCODE_STACK_SIZE;
        blackhawk7_ucode_len = BLACKHAWK7_V1L8P1_FLR_UCODE_IMAGE_SIZE;
        blackhawk7_ucode = blackhawk7_v1l8p1_flr_ucode_get();
    } else {
...
int _tscbh_gen3_core_init_pass2(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        ...
        if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
            ucode.ucode_size = BLACKHAWK7_V1L8P1_FLR_UCODE_IMAGE_SIZE;
            ucode.stack_size = BLACKHAWK7_V1L8P1_FLR_UCODE_STACK_SIZE;
            ucode.crc_value  = BLACKHAWK7_V1L8P1_FLR_UCODE_IMAGE_CRC;
        } else {
            ucode.ucode_size = BLACKHAWK7_V1L8P1_UCODE_IMAGE_SIZE;
            ucode.stack_size = BLACKHAWK7_V1L8P1_UCODE_STACK_SIZE;
        ...
        if (!PHYMOD_CORE_INIT_F_BYPASS_CRC_CHECK_GET(init_config)) {
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p1_ucode_crc_verify(&core_copy.access, 0, ucode.crc_value));
        }
    ...
    if ((int) (init_config->pll0_div_init_value) != phymod_TSCBH_PLL_DIVNONE) {
        core_copy.access.pll_idx = 0;
        if ((init_config->pll0_div_init_value == phymod_TSCBH_PLL_DIV132) ||
            (init_config->pll0_div_init_value == phymod_TSCBH_PLL_DIV66)) {
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p1_configure_20P625G_pll_div(&core_copy.access,
                                         ...
                                         phymod_phy_pll_state_t* new_pll_adv_state)
{
    ...
    int start_lane, num_lane, i = 0, tier1_ref_clk = 0;
    phymod_phy_access_t phy_copy;
    tbhmod_gen3_refclk_t ref_clk;
    ...
    if ((i == 1) && (request_pll_div[0] != tvco_pll_div)) {
        ...
        if (ref_clk == TBHMOD_GEN3_REF_CLK_312P5MHZ) {
            tier1_ref_clk = BLACKHAWK7_V1L8P1_PLL_REFCLK_312P5MHZ;
        } else {
            tier1_ref_clk = BLACKHAWK7_V1L8P1_PLL_REFCLK_156P25MHZ;
        }

        if ((request_pll_div[0] == phymod_TSCBH_PLL_DIV132) ||
            (request_pll_div[0] == phymod_TSCBH_PLL_DIV66)) {
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p1_configure_20P625G_pll_div(&phy_copy.access,
                                                             request_pll_div[0]));
        } else {
            PHYMOD_IF_ERR_RETURN
                                ...
                                phymod_ref_clk_t ref_clk1)
{
    ...
    if ((pll_div == phymod_TSCBH_PLL_DIV132 ) ||
        (pll_div == phymod_TSCBH_PLL_DIV66)) {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p1_configure_20P625G_pll_div(&pm_phy_copy.access, pll_div));
...
/*Set timesync adjust*/
int tscbh_gen3_timesync_adjust_set(const phymod_phy_access_t* phy,
                                   uint32_t flags,
                                   const phymod_timesync_adjust_config_info_t *config)
{
    ...
    enum blackhawk7_v1l8p1_osr_mode_enum tx_osr_mode, rx_osr_mode;
    tbhmod_gen3_ts_table_entry_mem *rx_shadow_mem;
    tbhmod_gen3_ts_table_entry_mem *tx_shadow_mem;
    ...
    tx_shadow_mem = (tbhmod_gen3_ts_table_entry_mem *)config->tx_lkup_1588_mem_mpp0;

    if (start_lane < 4) {
        /* MPP0 */
        rx_mem_offset = (start_lane >> 1) * TBHMOD_GEN3_TS_DEFAULT_TABLE_SIZE;
        rx_mem = phymodMemRxLkup1588Mpp0;
        rx_shadow_mem = (tbhmod_gen3_ts_table_entry_mem *)config->rx_lkup_1588_mem_mpp0;
    } else {
        /* MPP1. */
        rx_mem_offset = ((start_lane - 4) >> 1) * TBHMOD_GEN3_TS_DEFAULT_TABLE_SIZE;
        rx_mem = phymodMemRxLkup1588Mpp1;
        rx_shadow_mem = (tbhmod_gen3_ts_table_entry_mem *)config->rx_lkup_1588_mem_mpp1;
    }
    ...
    for (i = 0; i < (int) ts_table_size; i++) {
        ...
        if (tx_shadow_mem != NULL) {
            sal_memcpy( &tx_shadow_mem[i + tx_mem_offset][0],
                        &(*ts_tx_entry)[i][0],
                        sizeof(uint32_t)*TBHMOD_GEN3_TS_ENTRY_SIZE);
        }
    }
    ...
    /* 3.2 Add PM offset and Rx Clk period to each PSLL. */
    PHYMOD_IF_ERR_RETURN
        (tbhmod_gen3_ts_rx_table_adjust_set(&phy_copy.access, vco,
                                            osr_mode_value, is_pam4,
                                            psll_entry_size, psll_entry));
    ...
    PHYMOD_IF_ERR_RETURN
        (tbhmod_gen3_pcs_set_1588_ui(&phy_copy.access, vco, osr_mode_value, is_pam4));

    /* 5. Program PMD latency. */
    PHYMOD_IF_ERR_RETURN
        (tbhmod_gen3_tx_pmd_latency_set(&phy_copy.access, vco, osr_mode_value, is_pam4));
    ...
    if (config->am_norm_mode) {
        ...
        PHYMOD_IF_ERR_RETURN
            (tbhmod_gen3_mod_rx_1588_tbl_val(&phy_copy.access, speed_config_entry.bit_mux_mode,
                                             vco, osr_mode_value, is_pam4, normalize_to_latest,
                                 ...
                                 phymod_synce_clk_ctrl_t cfg)
{
    ...
    uint32_t sdm_val;
    int32_t pll_div;
    int osr_mode_value, start_lane, num_lane;
    enum blackhawk7_v1l8p1_osr_mode_enum tx_osr_mode, rx_osr_mode;
    ...
    switch (type) {
        ...
        case phymodIntrEccRx1588Mpp1: /* ODD bank */
            ...
            if ((intr_status.is_1b_err) || (intr_status.is_2b_err)) {
                ...
                if ( native_addr < TBHMOD_GEN3_TS_RX_MPP1_NATIVE_START_ADDR) {
                    mem_type = phymodMemRxLkup1588Mpp0;
                    mem_addr = TBHMOD_GEN3_TS_RX_MPP_ODD_BANK_NATIVE_TO_ADDR(native_addr) ;
                } else {
                    mem_type = phymodMemRxLkup1588Mpp1;
                    mem_addr = TBHMOD_GEN3_TS_RX_MPP_ODD_BANK_NATIVE_TO_ADDR(native_addr)-
                                TBHMOD_GEN3_TS_RX_MPP_MEM_SIZE ;
                }
            }

           if (intr_status.is_2b_err) {
                if (mem_addr >= TBHMOD_GEN3_TS_RX_MPP_MEM_SIZE) {
                    return PHYMOD_E_FAIL;
                }
        ...
        case phymodIntrEccRx1588Mpp0: /* EVEN Bank */
            ...
            if ((intr_status.is_1b_err) || (intr_status.is_2b_err)) {
                ...
                if ( native_addr < TBHMOD_GEN3_TS_RX_MPP1_NATIVE_START_ADDR) {
                    mem_type = phymodMemRxLkup1588Mpp0;
                    mem_addr = TBHMOD_GEN3_TS_RX_MPP_EVEN_BANK_NATIVE_TO_ADDR(native_addr) ;
                } else {
                    mem_type = phymodMemRxLkup1588Mpp1;
                    mem_addr = TBHMOD_GEN3_TS_RX_MPP_EVEN_BANK_NATIVE_TO_ADDR(native_addr)-
                                TBHMOD_GEN3_TS_RX_MPP_MEM_SIZE ;
...
STATIC
int _tscbh_gen3_flr_timeout_phymod_to_blackhawk(phymod_phy_flr_timeout_t timeout_time,
                                                enum blackhawk7_v1l8p1_flr_timeout_enum *bhk_timeout_time)
{
    switch (timeout_time) {
    case phymodFlrTimeout50ms:
        *bhk_timeout_time = BLACKHAWK7_V1L8P1_FLR_TIMEOUT_50MS;
        break;
    case phymodFlrTimeout75ms:
        *bhk_timeout_time = BLACKHAWK7_V1L8P1_FLR_TIMEOUT_75MS;
        break;
    case phymodFlrTimeout100ms:
        *bhk_timeout_time = BLACKHAWK7_V1L8P1_FLR_TIMEOUT_100MS;
        break;
    case phymodFlrTimeout150ms:
        *bhk_timeout_time = BLACKHAWK7_V1L8P1_FLR_TIMEOUT_150MS;
        break;
    case phymodFlrTimeout250ms:
        *bhk_timeout_time = BLACKHAWK7_V1L8P1_FLR_TIMEOUT_250MS;
        break;
    case phymodFlrTimeout500ms:
        *bhk_timeout_time = BLACKHAWK7_V1L8P1_FLR_TIMEOUT_500MS;
        break;
    default:
...
STATIC
int _tscbh_gen3_flr_timeout_blackhawk_to_phymod(enum blackhawk7_v1l8p1_flr_timeout_enum bhk_timeout_time,
                                          phymod_phy_flr_timeout_t *timeout_time)
{
    switch (bhk_timeout_time) {
    case BLACKHAWK7_V1L8P1_FLR_TIMEOUT_50MS:
        *timeout_time = phymodFlrTimeout50ms;
        break;
    case BLACKHAWK7_V1L8P1_FLR_TIMEOUT_75MS:
        *timeout_time = phymodFlrTimeout75ms;
        break;
    case BLACKHAWK7_V1L8P1_FLR_TIMEOUT_100MS:
        *timeout_time = phymodFlrTimeout100ms;
        break;
    case BLACKHAWK7_V1L8P1_FLR_TIMEOUT_150MS:
        *timeout_time = phymodFlrTimeout150ms;
        break;
    case BLACKHAWK7_V1L8P1_FLR_TIMEOUT_250MS:
        *timeout_time = phymodFlrTimeout250ms;
        break;
    case BLACKHAWK7_V1L8P1_FLR_TIMEOUT_500MS:
        *timeout_time = phymodFlrTimeout500ms;
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for blackhawk %u"), bhk_timeout_time));
        return PHYMOD_E_PARAM;
    }
    return PHYMOD_E_NONE;
}

int tscbh_gen3_phy_flr_config_set(const phymod_phy_access_t* phy, uint32_t enable, phymod_phy_flr_timeout_t timeout_time)
{
    int start_lane, num_lane, i;
    phymod_phy_access_t phy_copy;
    uint8_t flr_support = 0;
    ...
    if (enable) {
        PHYMOD_IF_ERR_RETURN
            (_tscbh_gen3_flr_timeout_phymod_to_blackhawk(timeout_time, &bhk_timeout_time));
    } else {
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p1_configure_fast_link_recovery(&phy_copy.access, enable,
                                                            enable, bhk_timeout_time));
    }
...
int tscbh_gen3_phy_flr_config_get(const phymod_phy_access_t* phy, uint32_t* enable, phymod_phy_flr_timeout_t* timeout_time)
{
    int start_lane, num_lane;
    ...
    phy_copy.access.lane_mask = 1 << start_lane ;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_get_configure_fast_link_recovery(&phy_copy.access,
                                                           &flr_enable,
                                                           &flr_timeout_en,
                                                           &bhk_timeout_time));
    if (flr_enable) {
    ...
    if (flr_timeout_en) {
        PHYMOD_IF_ERR_RETURN
            (_tscbh_gen3_flr_timeout_blackhawk_to_phymod(bhk_timeout_time,
                                                         timeout_time));
    } else {
...
int tscbh_gen3_phy_fw_is_flr_supported(const phymod_phy_access_t* phy, uint32_t* flr_support)
{
    uint8_t support = 0;
    phymod_phy_access_t pm_phy_copy;
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    ...
    pm_phy_copy.access.lane_mask = 0x1;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_INTERNAL_is_flr_supported(&pm_phy_copy.access, &support));
    *flr_support = support;
