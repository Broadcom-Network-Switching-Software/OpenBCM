...
STATIC int merlin7_diagnostics_eyescan_run_uc(const phymod_phy_access_t* phy, uint32_t flags)
{
    ...
    if ((PHYMOD_EYESCAN_F_ENABLE_GET(flags)) ||
        ...
        /*merlin7 eyescan does not support the speed lower that 8G per lane*/
        PHYMOD_IF_ERR_RETURN
            (merlin7_osr_mode_get(&phy->access, &osr_mode));
        PHYMOD_IF_ERR_RETURN
            (phymod_osr_mode_to_actual_os(osr_mode, &actual_osr, &actual_osr_rem));
        data_rate = ((core_cfg.vco_rate_in_Mhz * 1000) / (actual_osr * 1000 + actual_osr_rem));
        if (data_rate < 8000) {
            PHYMOD_RETURN_WITH_ERR(PHYMOD_E_FAIL, (_PHYMOD_MSG("eyescan is not supported for the lane rate lower than 8G")));
        }
    }
