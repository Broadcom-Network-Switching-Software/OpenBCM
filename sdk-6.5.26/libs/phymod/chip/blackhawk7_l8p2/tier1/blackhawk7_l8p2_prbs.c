...
static err_code_t _blackhawk7_l8p2_get_prbs_error_analyzer_prbs_chk_lock(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t *prbs_chk_lock);
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        EFUN(blackhawk7_l8p2_INTERNAL_compute_bin(sa__, pattern[i],bin, sizeof(bin)));
    ...
    for (i=1; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        EFUN(blackhawk7_l8p2_INTERNAL_pam4_to_bin(sa__, pattern[i], bin, sizeof(bin)));
        bytes_written = USR_SNPRINTF(patt_mod + mod_count, sizeof(patt_mod) - mod_count, "%s",  bin);
        mod_count += (size_t)bytes_written;
        if(( bytes_written < 0) || (mod_count >= sizeof(patt_mod))) {
          return ERR_CODE_REACHED_BUF_SIZE_LIMIT;
...
err_code_t blackhawk7_l8p2_get_prbs_chk_hw_timer_ctrl(srds_access_t *sa__, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl_bak) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l8p2_set_prbs_chk_hw_timer_ctrl(srds_access_t *sa__, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st const * const prbs_chk_hw_timer_ctrl_bak) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l8p2_config_prbs_chk_hw_timer(srds_access_t *sa__, uint16_t time_ms, uint16_t *time_ms_adjusted) {
    ...
    ENULL_MEMSET(&prbs_chk_hw_timer_ctrl, 0, sizeof(struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st));
...
err_code_t blackhawk7_l8p2_BER_setup_measurement(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l8p2_ber_data_st *ber_data, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) { 
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    uint8_t clk_gate = 0;
    ...
    return(ERR_CODE_NONE);
} /* blackhawk7_l8p2_BER_setup_measurement() */

err_code_t blackhawk7_l8p2_BER_poll_measurement(srds_access_t *sa__, struct blackhawk7_l8p2_ber_data_st *ber_data, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) { 
    ...
    /* Poll prbs_chk_lock until done or timeout has expired */
        ...
        /* In IP not having the new timer mode, prbs checker with the hardware timer mode disables the entire checker logic when timer times out. */
        /* If prbs chk hardware timer is already expired before entering the current function, */
        /* we have only one chance to read the latched high bit prbs_chk_lock_state which is done above. */
        if (iter == 0) {
            /* the following code checks at what point did the timers expire in relation to the events - 
             * (a)reading prbs_chk_lock_state and  (b)reading prbs_chk_done_lh */
            uint8_t lock_state;
            ESTM(lock_state = rd_prbs_chk_lock_state());
            if(lock_state == PRBS_CHECKER_NOT_ENABLED) {
                /* timers expired before reading the prbs_chk_lock_state */
                ber_data->prbs_lck_state = prbs_chk_lock_state;
            } else {
                /* timers expired after reading the prbs_chk_lock_state and before reading prbs_chk_done_lh */
                ber_data->prbs_lck_state = lock_state;
            }
        }
        else {
...
err_code_t blackhawk7_l8p2_BER_get_measurement(srds_access_t *sa__, struct blackhawk7_l8p2_ber_data_st *ber_data, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) { 
    uint8_t  lcklost = 0;
    ...
    lock_state = ber_data->prbs_lck_state;
    ...
    return(ERR_CODE_NONE);
} /* blackhawk7_l8p2_BER_get_measurement() */
...
const char* blackhawk7_l8p2_get_e2s_prbs_mode_enum(enum srds_prbs_polynomial_enum prbs_poly_mode) {
    ...
    if(prbs_poly_mode >= sizeof(blackhawk7_l8p2_e2s_prbs_mode_enum)/sizeof(blackhawk7_l8p2_e2s_prbs_mode_enum[0])) {
        return "UNKNOWN";
    }
    return blackhawk7_l8p2_e2s_prbs_mode_enum[prbs_poly_mode];
}
...
err_code_t blackhawk7_l8p2_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    if(enabled) {
        ...
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            ...
            prbs_string =  blackhawk7_l8p2_get_e2s_prbs_mode_enum(prbs_poly_mode);
        }
...
err_code_t blackhawk7_l8p2_prbs_error_analyzer_reset(srds_access_t *sa__) {
    ...
    EFUN(blackhawk7_l8p2_clear_dac_pattern_memory(sa__));
...
err_code_t blackhawk7_l8p2_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    fec_size = (uint8_t)(err_analyzer->prbs_err_fec_size / BLACKHAWK7_L8P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
...
/* Stop error analyzer and Clear DAC pattern memory, legacy */
...
err_code_t blackhawk7_l8p2_clear_dac_pattern_memory(srds_access_t *sa__) {
    ...
    /* Set clock to comclk when writing to SC bit dac_test_mem_write_pulse.
       The following is added for 7 and new Blackhawk7 cores. */
    ...
    /* Set clock to fast clock(clk4sync_div2) for running PRBS error analyzer.
       This is added for 7 and new Blackhawk7 cores. */
...
err_code_t blackhawk7_l8p2_read_dac_pattern_memory(srds_access_t *sa__, uint32_t *dac_mem, uint8_t read_addr, uint8_t num_of_lines) {
    INIT_SRDS_ERR_CODE
     ...
     /* Disable error analyzer writing error counts to DAC memory */
...
err_code_t blackhawk7_l8p2_get_prbs_error_analyzer_lane_err_count(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t lane) {
    ...
    EFUN(blackhawk7_l8p2_read_dac_pattern_memory(sa__,dacmem2,(uint8_t)(4*(num_lanes+1)),2));
    ...
    for (i=BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i > 0; i--) {
        if (i!=BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS) {
...
err_code_t blackhawk7_l8p2_get_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(_blackhawk7_l8p2_get_prbs_error_analyzer_prbs_chk_lock(sa__, err_analyzer, &prbs_lock));
    if (!prbs_lock) {
        return (ERR_CODE_NONE);
    }
    ...
    /* Stop Error Analyzer on single lane */
...
err_code_t blackhawk7_l8p2_get_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(blackhawk7_l8p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, num_lanes));
...
err_code_t blackhawk7_l8p2_prbs_error_analyzer_aggregate_config(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer->lane_mask_aggregate >> lane) & 0x1) {
            ...
            EFUN(blackhawk7_l8p2_prbs_error_analyzer_config(sa__, err_analyzer));
...
err_code_t blackhawk7_l8p2_display_aggregate_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, enum srds_prbs_error_analyzer_aggregate_mode_enum prbs_err_aggregate_mode, uint8_t lane_mask_aggregate, uint32_t timeout_s, uint8_t diag_mode) {
    ...
    EFUN(blackhawk7_l8p2_prbs_error_analyzer_reset(sa__));
    /* Configure PRBS Error Analyzer Aggregation Mode*/
    EFUN(blackhawk7_l8p2_prbs_error_analyzer_aggregate_config(sa__, &err_analyzer));
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            ...
            EFUN(blackhawk7_l8p2_prbs_error_analyzer_start(sa__));
        }
    }
...
err_code_t blackhawk7_l8p2_prbs_error_analyzer_report_proj(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s, blackhawk7_l8p2_prbs_err_analyzer_report_st *err_analyzer_report) {
    ...
    if (err_analyzer.prbs_err_fec_size == 10560) {
        if (delta_n >= 2) {
            ...
            proj_ber[7] = pow(proj_ber[7],1.0/BLACKHAWK7_L8P2_PRBS_ERR_ANALYZER_LINEARITY_ADJUST);
...
/*  ii) Need at least 2 valid data points to generate projected data                                             */
/*----------------------------------------------------------------------------------------------------------------*/
err_code_t blackhawk7_l8p2_prbs_error_analyzer_extrapolate_err_count(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t err_cnt_idx ) {
#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    uint8_t    delta_n;
    uint8_t    i;
    ...
    if (delta_n >= 2) { /* There are at least 2 points to trace a line for extrapolation */
        ...
        EFUN(blackhawk7_l8p2_INTERNAL_fit_second_order(sa__, delta_n, x, y, a));
    }
