...
static err_code_t _blackhawk7_v1l8p1_get_prbs_error_analyzer_prbs_chk_lock (srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t *prbs_chk_lock);
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_compute_bin(sa__, pattern[i], bin, sizeof(bin)));
    ...
    for (i=1; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_pam4_to_bin(sa__, pattern[i], bin, sizeof(bin)));
        bytes_written = USR_SNPRINTF(patt_mod + mod_count, sizeof(patt_mod) - mod_count, "%s",  bin);
        mod_count += (size_t)bytes_written;
        if(( bytes_written < 0) || (mod_count >= sizeof(patt_mod))) {
          return ERR_CODE_REACHED_BUF_SIZE_LIMIT;
...
err_code_t blackhawk7_v1l8p1_BER_setup_measurement(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_v1l8p1_ber_data_st *ber_data, struct blackhawk7_v1l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) { 
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    uint8_t clk_gate = 0;
    ...
    return(ERR_CODE_NONE);
} /* blackhawk7_v1l8p1_BER_setup_measurement() */

err_code_t blackhawk7_v1l8p1_BER_poll_measurement(srds_access_t *sa__, struct blackhawk7_v1l8p1_ber_data_st *ber_data, struct blackhawk7_v1l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) { 
...
err_code_t blackhawk7_v1l8p1_BER_get_measurement(srds_access_t *sa__, struct blackhawk7_v1l8p1_ber_data_st *ber_data, struct blackhawk7_v1l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) { 
    uint8_t  lcklost = 0;
    ...
    return(ERR_CODE_NONE);
} /* blackhawk7_v1l8p1_BER_get_measurement() */
...
const char* blackhawk7_v1l8p1_get_e2s_prbs_mode_enum(enum srds_prbs_polynomial_enum prbs_poly_mode) {
  ...
  if(prbs_poly_mode >= sizeof(blackhawk7_v1l8p1_e2s_prbs_mode_enum)/sizeof(blackhawk7_v1l8p1_e2s_prbs_mode_enum[0])) {
        return "UNKNOWN";
    }
    return blackhawk7_v1l8p1_e2s_prbs_mode_enum[prbs_poly_mode];
}
...
err_code_t blackhawk7_v1l8p1_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    if(enabled) {
        ...
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            ...
            prbs_string =  blackhawk7_v1l8p1_get_e2s_prbs_mode_enum(prbs_poly_mode);
        }
...
err_code_t blackhawk7_v1l8p1_get_prbs_error_analyzer_lane_err_count(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t lane) {
    ...
    EFUN(blackhawk7_v1l8p1_read_dac_pattern_memory(sa__,dacmem2,(uint8_t)(4*(num_lanes+1)),2));
    ...
    /* Calculating cumulative error count for respective values of "t" */
        ...
        if (i!=BLACKHAWK7_V1L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS) {
...
err_code_t blackhawk7_v1l8p1_get_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(_blackhawk7_v1l8p1_get_prbs_error_analyzer_prbs_chk_lock(sa__, err_analyzer, &prbs_lock));
    if(!prbs_lock) {
        return (ERR_CODE_NONE);
    }
...
/*  ii) Need at least 2 valid data points to generated projected data                                             */
/*----------------------------------------------------------------------------------------------------------------*/
err_code_t blackhawk7_v1l8p1_prbs_error_analyzer_extrapolate_err_count(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t err_cnt_idx ) {
#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    uint8_t    delta_n;
    uint8_t    i;
    ...
    if (delta_n >= 2) { /* There are at least 2 points to trace a line for extrapolation */
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_fit_second_order(sa__, delta_n, x, y, a));
    }
