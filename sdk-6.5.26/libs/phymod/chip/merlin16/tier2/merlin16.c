...
int _merlin16_pll_multiplier_get(uint32_t pll_div, uint32_t *pll_multiplier)
{
    switch (pll_div) {
        ...
        case MERLIN16_PLL_DIV_80: *pll_multiplier = 80;
            break;
        case MERLIN16_PLL_DIV_54P4: *pll_multiplier = 54;
            break;
...
int merlin16_phy_rx_pmd_lock_status_get(const phymod_phy_access_t* phy,
                                         uint32_t* pmd_lock,
                                         uint32_t* pmd_lock_change)
{
    ...
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (merlin16_rx_pmd_lock_status_get(&pm_phy_copy.access,
                                          pmd_lock,
                                          pmd_lock_change));
    return PHYMOD_E_NONE;
}
