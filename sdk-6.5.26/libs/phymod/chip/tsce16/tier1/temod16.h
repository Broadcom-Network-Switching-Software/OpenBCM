...
typedef enum {
    TEMOD16_PORT_MODE_QUAD  = 0,    /* quad port mode.  */
    TEMOD16_PORT_MODE_TR1,          /* lanes 2,3 aggregated and 0,1 as 3 ports.   */
    TEMOD16_PORT_MODE_TR2,          /* lanes 1,0 aggregated and 3,2 as 3 ports.  */
    TEMOD16_PORT_MODE_DUAL,         /* dual port mode.  */
    TEMOD16_PORT_MODE_SINGLE,       /* single port mode. */
    TEMOD16_PORT_MODE_COUNT
} temod16_port_mode_type;
...
typedef enum {
    TEMOD16_ENCODER_DISABLE = 0,
    TEMOD16_ENCODER_CL48,
    TEMOD16_ENCODER_RXAUI_CL48,
    TEMOD16_ENCODER_CL36,
    TEMOD16_ENCODER_CL82,
    TEMOD16_ENCODER_CL49,
    TEMOD16_ENCODER_BRCMCL49,
    TEMOD16_ENCODER_TYPE_COUNT
}temod16_encoder_type_t;

typedef enum {
    TEMOD16_SCRAMBLER_BYPASS = 0,  /* bypass scrambler */
    TEMOD16_SCRAMBLER_64B66B_SH,   /* 64b66b scrambler (all 66 bits) */
    TEMOD16_SCRAMBLER_8B10B,       /* 8b10b scrambler (all 80 bits) */
    TEMOD16_SCRAMBLER_64B66B,      /* 64b66b scrambler (sync bits not scrambled) */
    TEMOD16_SCRAMBLER_TYPE_COUNT
}temod16_scrambler_type_t;
...
typedef struct temod16_fec_error_mask_s {
...
extern int temod16_enable_set(PHYMOD_ST* pc);
...
int temod16_pcs_lane_swap_get(PHYMOD_ST *pa,  uint32_t *tx_rx_swap);
...
extern int temod16_refclk_get(PHYMOD_ST* pc, phymod_ref_clk_t *ref_clock);
...
extern int temod16_pll_div_get(uint32_t pll_div);
...
extern int temod16_port_start_lane_get(PHYMOD_ST *pc, int *port_starting_lane, int *port_num_lane);
extern int temod16_fec_error_inject_set(PHYMOD_ST *pc,
                                        uint16_t error_control_map,
                                        temod16_fec_error_mask_t bit_error_mask);
extern int temod16_fec_error_inject_get(PHYMOD_ST *pc,
                                        uint16_t* error_control_map,
                                        temod16_fec_error_mask_t* bit_error_mask);

extern int temod16_encode_mode_get(PHYMOD_ST* pc, temod16_encoder_type_t *mode);
extern int temod16_scramble_mode_get(PHYMOD_ST* pc, temod16_scrambler_type_t *mode);
extern int temod16_fec_correctable_counter_get(PHYMOD_ST* pc, uint32_t* count);
extern int temod16_fec_uncorrectable_counter_get(PHYMOD_ST* pc, uint32_t* count);
extern int temod16_bip_error_counter_get(PHYMOD_ST* pc, uint32_t* count);
extern int temod16_cl49_ber_counter_get(PHYMOD_ST* pc, uint32_t* count);
extern int temod16_cl82_ber_counter_get(PHYMOD_ST* pc, uint32_t* count);
