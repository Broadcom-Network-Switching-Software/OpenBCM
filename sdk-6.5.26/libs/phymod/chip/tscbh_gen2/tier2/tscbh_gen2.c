...
STATIC int
_tscbh_gen2_phy_lane_swap_validate(const phymod_phy_access_t* phy)
{
    int i;
    ...
    PHYMOD_IF_ERR_RETURN
        (tscbh_gen2_core_lane_map_get(&core, &lane_map));
...
static int _tscbh_gen2_100g_2lane_sw_workaround(const phymod_phy_access_t* phy)
{
    ...
    /*select the tvco*/
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_lane_pll_selection_set(&phy_copy.access, phy_copy.access.tvco_pll_index));
...
int _tscbh_gen2_core_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config)
{
    ...
    uint8_t afe_ver_a0 = 0, flr_ucode = 0;
    ...
    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        if (PHYMOD_CORE_INIT_F_TOP_DEV_REV_ID_A0_GET(init_config)) {
            PHYMOD_DEBUG_ERROR(("devad 0x%"PRIx32" lane 0x%"PRIx32": FLR not supported\n", core->access.addr, core->access.lane_mask));
            return PHYMOD_E_CONFIG;
        } else {
            flr_ucode = 1;
            blackhawk_ucode_len = BLACKHAWK7_V2L8P2_FLR_UCODE_IMAGE_SIZE;
        }
    } else {
        if (PHYMOD_CORE_INIT_F_TOP_DEV_REV_ID_A0_GET(init_config)) {
            afe_ver_a0 = 1;
    ...
    blackhawk_ucode = blackhawk7_l8p2_ucode_get(afe_ver_a0, flr_ucode);
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        if (an_en && an_done) {
            ...
            if ((speed_id == SPEED_ID_INDEX_100G_4_LANE_CL73_KR4) ||
                ...
                PHYMOD_IF_ERR_RETURN
                    (tbhmod_gen2_fec_align_status_get(&phy_copy.access, &fec_align_live));
                if (!fec_align_live) {
...
int tscbh_gen2_phy_loopback_set(const phymod_phy_access_t* phy, phymod_loopback_mode_t loopback, uint32_t enable)
{
    ...
    if (enable && (num_lane != TSCBH_NOF_LANES_IN_CORE)) {
        if (_tscbh_gen2_phy_lane_swap_validate(phy)) {
...
int _tscbh_gen2_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        if (PHYMOD_CORE_INIT_F_TOP_DEV_REV_ID_A0_GET(init_config)) {
            ...
            flr_ucode = 1;
            ucode.stack_size = BLACKHAWK7_V2L8P2_FLR_UCODE_STACK_SIZE;
            ucode.ucode_size = BLACKHAWK7_V2L8P2_FLR_UCODE_IMAGE_SIZE;
            ucode.crc_value  = BLACKHAWK7_V2L8P2_FLR_UCODE_IMAGE_CRC;
        }
    } else {
        if (PHYMOD_CORE_INIT_F_TOP_DEV_REV_ID_A0_GET(init_config)) {
            afe_ver_a0 = 1;
    ...
    blackhawk7_ucode = blackhawk7_l8p2_ucode_get(afe_ver_a0, flr_ucode);
    ...
    for (i = 0; i < am_table_load_size; i++) {
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemAMTable, i,
                              (tscbh_gen2_am_table_entry + i * TSCBH_AM_ENTRY_SIZE)));
    }
    ...
    /*
     * Initialize 1588 look up table.
     */
    for (i = 0; i < TBHMOD_GEN2_TS_MPP_MEM_SIZE; i++) {
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemTxLkup1588Mpp0, i, init_data));
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemTxLkup1588Mpp1, i, init_data));
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemRxLkup1588Mpp0, i, init_data));
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemRxLkup1588Mpp1, i, init_data));
    }
...
int tscbh_gen2_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    if (PHYMOD_PHY_INIT_F_TX_PCS_INTF_DLY_MODE_EN_GET(init_config)) {
        for (i = 0; i < num_lane; i++) {
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
                continue;
            }
    ...
    /* Bypass this SW WAR if 8 lane port */
    if (num_lane != 8) {
        /* SW WAR is required for lane 0 when MPP0 is active. */
        ...
        /* SW WAR is required for lane 4 when MPP1 is active. */
    ...
    *new_pll_adv_state = *old_pll_adv_state;
    ...
    for (i = requested_pll_num; i >= 0; i--) {
        if (request_pll_div[i] == ovco_pll_div) {
            ...
            if (ref_clk == TBHMOD_REF_CLK_312P5MHZ) {
                tier1_ref_clk = BLACKHAWK7_L8P2_PLL_REFCLK_312P5MHZ;
            } else {
                tier1_ref_clk = BLACKHAWK7_L8P2_PLL_REFCLK_156P25MHZ;
            }

            /* Delay 300ms as PLL not lock Workaround */
            PHYMOD_USLEEP(300000);

            if (request_pll_div[i] == TBHMOD_PLL_MODE_DIV_132 ||
                request_pll_div[i] == TBHMOD_PLL_MODE_DIV_66) {

                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_configure_20P625G_pll_div(&phy_copy.access,
                                                               request_pll_div[i]));
            } else {
                /* Assert core dp reset */
                PHYMOD_IF_ERR_RETURN
...
int tscbh_gen2_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    if (an->enable) {
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            phy_copy.access.lane_mask = 0x1 << (i + start_lane);
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_l8p2_tx_pam4_precoder_enable_set(&phy_copy.access, 0));
        }
    }
                                ...
                                phymod_ref_clk_t ref_clk1)
{
    ...
    /*
     * Delay 300ms as PLL not lock Workaround
     * Delay time is applied only for OVCO configuration.
     */
    if (!tvco_reconfig) {
        PHYMOD_USLEEP(300000);
    }
    ...
    if (pll_div == TBHMOD_PLL_MODE_DIV_132 ||
...
/*Set timesync adjust*/
int tscbh_gen2_timesync_adjust_set(const phymod_phy_access_t* phy,
                                   uint32_t flags,
                                   const phymod_timesync_adjust_config_info_t *config)
{
    ...
    tbhmod_gen2_ts_table_entry_mem *rx_shadow_mem;
    tbhmod_gen2_ts_table_entry_mem *tx_shadow_mem;

    tbhmod_gen2_ts_table_entry* tbhmod_gen2_ts_table_tx_sop;
    tbhmod_gen2_ts_table_entry* tbhmod_gen2_ts_table_rx_sop;
    tbhmod_gen2_ts_table_entry* tbhmod_gen2_ts_table_tx_sfd;
    tbhmod_gen2_ts_table_entry* tbhmod_gen2_ts_table_rx_sfd;

    tbhmod_gen2_ts_table_tx_sop = tbhmod_gen2_ts_table_tx_sop_get();
    tbhmod_gen2_ts_table_rx_sop = tbhmod_gen2_ts_table_rx_sop_get();
    tbhmod_gen2_ts_table_tx_sfd = tbhmod_gen2_ts_table_tx_sfd_get();
    tbhmod_gen2_ts_table_rx_sfd = tbhmod_gen2_ts_table_rx_sfd_get();
    ...
    if (PHYMOD_TIMESYNC_F_SOP_GET(flags)) {
        ts_offset = TSCBH_GEN2_TS_OFFSET_SOP;
    } else {
        if (PHYMOD_TIMESYNC_F_MAC_DA_GET(flags)) {
            ts_offset = TSCBH_GEN2_TS_OFFSET_MAC_DA;
        } else {
            ts_offset = TSCBH_GEN2_TS_OFFSET_SFD;
            ts_use_sfd_table = 1;
        }
    }
    ...
    if ((PHYMOD_TIMESYNC_F_REDUCED_PREAMBLE_MODE_GET(flags)) &&
        (ts_offset != TSCBH_GEN2_TS_OFFSET_SOP)){
        ts_offset -= TSCBH_GEN2_TS_RPM_OFFSET;
    }
    ...
    /* Base-R CL74 does not support 1588 */
    ...
    if (ts_use_sfd_table) {
        ts_tx_entry = tbhmod_gen2_ts_table_tx_sfd + ts_table_index;
        ts_rx_entry = tbhmod_gen2_ts_table_rx_sfd + ts_table_index;
    } else {
        ts_tx_entry = tbhmod_gen2_ts_table_tx_sop + ts_table_index;
        ts_rx_entry = tbhmod_gen2_ts_table_rx_sop + ts_table_index;
    }

    if (start_lane < 4) {
        tx_mem = phymodMemTxLkup1588Mpp0;
        rx_mem = phymodMemRxLkup1588Mpp0;
        tx_shadow_mem = (tbhmod_gen2_ts_table_entry_mem *)config->tx_lkup_1588_mem_mpp0;
        rx_shadow_mem = (tbhmod_gen2_ts_table_entry_mem *)config->rx_lkup_1588_mem_mpp0;
    } else {
        tx_mem = phymodMemTxLkup1588Mpp1;
        rx_mem = phymodMemRxLkup1588Mpp1;
        tx_shadow_mem = (tbhmod_gen2_ts_table_entry_mem *)config->tx_lkup_1588_mem_mpp1;
        rx_shadow_mem = (tbhmod_gen2_ts_table_entry_mem *)config->rx_lkup_1588_mem_mpp1;
    }
    ...
    for (i = 0; i < (int) ts_table_size; i++) {
        ...
        if (tx_shadow_mem != NULL) {
            sal_memcpy( &tx_shadow_mem[i + mem_offset][0],
                        &(*ts_tx_entry)[i][0],
                        sizeof(uint32_t)*TBHMOD_GEN2_TS_ENTRY_SIZE);
        }
    }
    ...
    for (i = 0; i < (int) ts_table_size; i++) {
        ...
        if (rx_shadow_mem != NULL) {
            sal_memcpy( &rx_shadow_mem[i + mem_offset][0],
                        &ts_update_table[i][0],
                        sizeof(uint32_t)*TBHMOD_GEN2_TS_ENTRY_SIZE);
        }
    }
    ...
    */
    ...
    /* 7. Set timestamp  byte offset without enabling RX TS update. */
    phy_copy.access.lane_mask = 0x1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (tbhmod_gen2_pcs_ts_config(&phy_copy.access, ts_offset));
...
int tscbh_gen2_phy_autoneg_speed_id_table_reload(const phymod_phy_access_t* phy, uint32_t gsh_header_enable)
{
    ...
    uint32_t* tscbh_spd_id_entry_25;
    uint32_t* tscbh_spd_id_entry_20;
    uint32_t* tscbh_spd_id_entry_26_gsh;
    uint32_t* tscbh_spd_id_entry_25_gsh;
    uint32_t* tscbh_spd_id_entry_20_gsh;
    ...
    tscbh_spd_id_entry_25 = tscbh_spd_id_entry_25_get();
    tscbh_spd_id_entry_20 = tscbh_spd_id_entry_20_get();
    tscbh_spd_id_entry_26_gsh = tscbh_spd_id_entry_26_gsh_get();
    tscbh_spd_id_entry_25_gsh = tscbh_spd_id_entry_25_gsh_get();
    tscbh_spd_id_entry_20_gsh = tscbh_spd_id_entry_20_gsh_get();
    ...
    /* first set the lane mask to be 0x1 */
    phy_copy.access.lane_mask = 1 << 0;
    PHYMOD_IF_ERR_RETURN(tbhmod_gen2_fec_override_get(&phy_copy.access, &an_fec_override_enable));
    ...
    if ((tvco_pll_div == TBHMOD_PLL_MODE_DIV_170) || (tvco_pll_div == TBHMOD_PLL_MODE_DIV_85))  {
            ...
            if (gsh_header_enable) {
                PHYMOD_IF_ERR_RETURN
                   (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable, i,
                                      (tscbh_spd_id_entry_26_gsh + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
            } else {
                if (an_fec_override_enable &&
                    (i == SPEED_ID_INDEX_100G_4_LANE_CL73_KR4 ||
                     i == SPEED_ID_INDEX_100G_4_LANE_CL73_CR4)) {
                    ...
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                          i, tscbh_spd_id_entry_100g_4lane_no_fec_26));
                } else {
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable, i,
                                          (tscbh_spd_id_entry_26 + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
        ...
        for (i = 0; i < TSCBH_GEN2_AUTONEG_SPEED_ID_COUNT; i++) {
            if (gsh_header_enable) {
                PHYMOD_IF_ERR_RETURN
                   (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable, i,
                                      (tscbh_spd_id_entry_25_gsh + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
            } else {
                if (an_fec_override_enable &&
                    (i == SPEED_ID_INDEX_100G_4_LANE_CL73_KR4 ||
                     i == SPEED_ID_INDEX_100G_4_LANE_CL73_CR4)) {
                    ...
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                          i, tscbh_spd_id_entry_100g_4lane_no_fec_25));
                } else {
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable, i,
                                          (tscbh_spd_id_entry_25 + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
        ...
        for (i = 0; i < TSCBH_GEN2_AUTONEG_SPEED_ID_COUNT; i++) {
            if (gsh_header_enable) {
                PHYMOD_IF_ERR_RETURN
                   (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable, i,
                                      (tscbh_spd_id_entry_20_gsh + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
            } else {
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable, i,
                                      (tscbh_spd_id_entry_20 + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
...
static
int _tscbh_gen2_phy_power_mode_map(phymod_power_mode_t power_mode, enum srds_core_pwrdn_mode_enum *pwrdn_mode)
{
    switch (power_mode) {
...
int tscbh_gen2_phy_serdes_power_set(const phymod_phy_access_t* phy, phymod_power_option_t power_option, phymod_power_mode_t power_mode)
{
    phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;
    ...
    PHYMOD_IF_ERR_RETURN
        (_tscbh_gen2_phy_power_mode_map(power_mode, &pwrdn_mode));
    switch (power_option) {
 ...
 * for this purpose
 */
int tscbh_gen2_phy_fec_override_set(const phymod_phy_access_t* phy, uint32_t enable)
{
    phymod_phy_access_t phy_copy;
    uint32_t tvco_pll_div;
    uint32_t* tscbh_spd_id_entry_25;
    ...
    if ((enable ==  0) || (enable == 1)) {
         PHYMOD_IF_ERR_RETURN(tbhmod_gen2_fec_override_set(&phy_copy.access, enable));
    } else {
...
int tscbh_gen2_phy_fec_override_get(const phymod_phy_access_t* phy, uint32_t* enable)
{
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    phy_copy.access.lane_mask = 1;
    PHYMOD_IF_ERR_RETURN(tbhmod_gen2_fec_override_get(&phy_copy.access, enable));
...
STATIC
int _tscbh_gen2_flr_timeout_phymod_to_blackhawk(phymod_phy_flr_timeout_t timeout_time, enum blackhawk7_l8p2_flr_timeout_enum *bh7_l8p2_timeout_time)
{
    switch (timeout_time) {
    case phymodFlrTimeout50ms:
        *bh7_l8p2_timeout_time = BLACKHAWK7_L8P2_FLR_TIMEOUT_50MS;
        break;
    case phymodFlrTimeout75ms:
        *bh7_l8p2_timeout_time = BLACKHAWK7_L8P2_FLR_TIMEOUT_75MS;
        break;
    case phymodFlrTimeout100ms:
        *bh7_l8p2_timeout_time = BLACKHAWK7_L8P2_FLR_TIMEOUT_100MS;
        break;
    case phymodFlrTimeout150ms:
        *bh7_l8p2_timeout_time = BLACKHAWK7_L8P2_FLR_TIMEOUT_150MS;
        break;
    case phymodFlrTimeout250ms:
        *bh7_l8p2_timeout_time = BLACKHAWK7_L8P2_FLR_TIMEOUT_250MS;
        break;
    case phymodFlrTimeout500ms:
        *bh7_l8p2_timeout_time = BLACKHAWK7_L8P2_FLR_TIMEOUT_500MS;
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for blackhawk7_l8p2 %u"), timeout_time));
        return PHYMOD_E_PARAM;
    }
    return PHYMOD_E_NONE;
}
...
STATIC
int _tscbh_gen2_flr_timeout_blackhawk_to_phymod(enum blackhawk7_l8p2_flr_timeout_enum bh7_l8p2_timeout_time, phymod_phy_flr_timeout_t *timeout_time)
{
    switch (bh7_l8p2_timeout_time) {
    case BLACKHAWK7_L8P2_FLR_TIMEOUT_50MS:
        *timeout_time = phymodFlrTimeout50ms;
        break;
    case BLACKHAWK7_L8P2_FLR_TIMEOUT_75MS:
        *timeout_time = phymodFlrTimeout75ms;
        break;
    case BLACKHAWK7_L8P2_FLR_TIMEOUT_100MS:
        *timeout_time = phymodFlrTimeout100ms;
        break;
    case BLACKHAWK7_L8P2_FLR_TIMEOUT_150MS:
        *timeout_time = phymodFlrTimeout150ms;
        break;
    case BLACKHAWK7_L8P2_FLR_TIMEOUT_250MS:
        *timeout_time = phymodFlrTimeout250ms;
        break;
    case BLACKHAWK7_L8P2_FLR_TIMEOUT_500MS:
        *timeout_time = phymodFlrTimeout500ms;
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for blackhawk7_L8P2 %u"), bh7_l8p2_timeout_time));
        return PHYMOD_E_PARAM;
    }
...
int tscbh_gen2_phy_flr_config_set(const phymod_phy_access_t* phy, uint32_t enable, phymod_phy_flr_timeout_t timeout_time)
{
    int start_lane, num_lane, i;
    phymod_phy_access_t phy_copy;
    uint8_t flr_support = 0;
    uint32_t pcs_enable, lane_reset;
    enum blackhawk7_l8p2_flr_timeout_enum bh7_l8p2_timeout_time;
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_INTERNAL_is_flr_supported(&phy_copy.access, &flr_support));
    if (!flr_support) {
    ...
    if (enable) {
        PHYMOD_IF_ERR_RETURN
            (_tscbh_gen2_flr_timeout_phymod_to_blackhawk(timeout_time, &bh7_l8p2_timeout_time));
    } else {
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l8p2_configure_fast_link_recovery(&phy_copy.access, enable,
                                                          enable, bh7_l8p2_timeout_time));
    }
...
int tscbh_gen2_phy_flr_config_get(const phymod_phy_access_t* phy, uint32_t *enable, phymod_phy_flr_timeout_t *timeout_time)
{
    uint8_t flr_enable =0, flr_timeout_en = 0;
    int start_lane, num_lane;
    enum blackhawk7_l8p2_flr_timeout_enum bh7_l8p2_timeout_time;
    phymod_phy_access_t phy_copy;
    ...
    phy_copy.access.lane_mask = 0x1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_get_configure_fast_link_recovery(&phy_copy.access,
                                                          &flr_enable,
                                                          &flr_timeout_en,
                                                          &bh7_l8p2_timeout_time));

    *enable =  flr_enable ? 1 : 0;
    if (flr_timeout_en) {
        PHYMOD_IF_ERR_RETURN
            (_tscbh_gen2_flr_timeout_blackhawk_to_phymod(bh7_l8p2_timeout_time,
                                                         timeout_time));
    } else {
        *timeout_time = phymodFlrTimeoutInvaild;
    }
    return PHYMOD_E_NONE;
}
...
STATIC
int _tscbh_gen2_speed_table_entry_to_nums_fec_lane_get(spd_id_tbl_entry_t* speed_config_entry,
                                                       int* nums_fec_lane)
{
...
int tscbh_gen2_phy_am_loss_align_multi_state_get(const phymod_phy_access_t* phy, uint32_t* am_loss_align_state)
{
   ...
   /* Just return when port speed is no_fec. */
    ...
    PHYMOD_IF_ERR_RETURN
        (_tscbh_gen2_speed_table_entry_to_nums_fec_lane_get(&speed_config_entry, &nums_fec_lane));
...
int tscbh_gen2_phy_fw_is_flr_supported(const phymod_phy_access_t* phy, uint32_t* flr_support)
{
    return PHYMOD_E_NONE;
 ...
 */
int tscbh_gen2_intr_handler(const phymod_phy_access_t* phy,
                       ...
                       phymod_intr_info_t *mem_info)
{
    ...
    uint32_t ts_table_entry[TBHMOD_GEN2_TS_ENTRY_SIZE];
    ...
    switch (type) {
        ...
        case phymodIntrEccRx1588Mpp1: /* ODD bank */
            ...
            if ((intr_status.is_1b_err) || (intr_status.is_2b_err)) {
                ...
                if ( native_addr < TBHMOD_GEN2_TS_RX_MPP1_NATIVE_START_ADDR) {
                    mem_type = phymodMemRxLkup1588Mpp0;
                    mem_addr = TBHMOD_GEN2_TS_RX_MPP_ODD_BANK_NATIVE_TO_ADDR(native_addr) ;
                } else {
                    mem_type = phymodMemRxLkup1588Mpp1;
                    mem_addr = TBHMOD_GEN2_TS_RX_MPP_ODD_BANK_NATIVE_TO_ADDR(native_addr)-
                                TBHMOD_GEN2_TS_MPP_MEM_SIZE ;
                }
            }

           if (intr_status.is_2b_err) {
                if (mem_addr >= TBHMOD_GEN2_TS_MPP_MEM_SIZE) {
                    return PHYMOD_E_FAIL;
                }
        ...
        case phymodIntrEccRx1588Mpp0: /* EVEN Bank */
            if ((intr_status.is_1b_err) || (intr_status.is_2b_err)) {
                ...
                if ( native_addr < TBHMOD_GEN2_TS_RX_MPP1_NATIVE_START_ADDR) {
                    mem_type = phymodMemRxLkup1588Mpp0;
                    mem_addr = TBHMOD_GEN2_TS_RX_MPP_EVEN_BANK_NATIVE_TO_ADDR(native_addr) ;
                } else {
                    mem_type = phymodMemRxLkup1588Mpp1;
                    mem_addr = TBHMOD_GEN2_TS_RX_MPP_EVEN_BANK_NATIVE_TO_ADDR(native_addr)-
                                TBHMOD_GEN2_TS_MPP_MEM_SIZE ;
        ...
        case phymodIntrEccTx1588Mpp1:
          if (intr_status.is_2b_err) {
                index = intr_status.err_addr;
                if (index >= TBHMOD_GEN2_TS_MPP_MEM_SIZE) {
                    return PHYMOD_E_FAIL;
                }
                mem_info->mem_type = phymodMemTxLkup1588Mpp1;
