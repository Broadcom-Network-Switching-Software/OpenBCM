...
typedef struct
{
    ...
    bcm_field_input_types_t qual_type;  /* Field Qualifier Input Types, indicate base layer type for qualifier. */
    int qual_arg;
    int offset;
    uint32 mask;
} sh_pkt_qualifier_t;

/* SAT CTF identifier information.*/
typedef struct
{
    int color;
    int tc;
} sh_pkt_sat_identifier_info_t;
...
/**
 * \brief
 *    Generate gtf for SAT packet streaming with configurable rate pattern
 * \param [in] unit - Unit #
 * \param [in] core_id - core id  #*
 * \param [in] packet_handle - handle to the object which holds all packet/stream related params
 * \param [in] pkt_length - full packet length (units: bytes)
 ...
 * \param [in] rate_unit - unit for rate parameter - either kbits or packets per second
 * \param [in] rate - stream rate (units: kbits per second)
 * \param [in] rate_pattern - configure stream rate pattern mode
 * \param [in] burst - stream burst size (units: bytes)
 * \param [in] granularity - globally clocks per cycle,
 *                         Special values
 *                         SAT_GRANULARITY_SET_BY_DIAG - when there is a need to have optimized flat flow with high
 ...
 *   diag_sand_packet_create
 */
shr_error_e diag_sand_packet_stream_pattern_create(
    int unit,
    int core_id,
    rhhandle_t packet_handle,
    int pkt_length,
 ...
 *                                                       stream use BY_DIAG, for the rest BY_BCM
 *                         SAT_GRANULARITY_SET_BY_BCM  - for multiple flows with lower rates
 * \param [in] compensation - stream rate compensation
 *
 * \retval _SHR_E_NONE for success
 * \retval shr_error_e Standard error handling
 * \see
 *   diag_sand_packet_create
 */
shr_error_e diag_sand_packet_stream_with_compensation_create(
    int unit,
    int core_id,
    rhhandle_t packet_handle,
    int pkt_length,
...
/**
 * \brief
 *    Destroy stream for packet handle without freeing it
 * \param [in] unit - Unit #
 * \param [in] packet_handle   - pointer to array of handles to the object which holds all packet/stream related params
 *
 * \retval _SHR_E_NONE for success
 * \retval shr_error_e Standard error handling
 */
shr_error_e diag_sand_packet_stream_destroy(
    int unit,
    rhhandle_t packet_handle);
...
/**
 * \brief
 *    Start stream for all packet handles in array
 * \param [in] unit - Unit #
 * \param [in] packet_h   - a handle to the object which holds all packet/stream related params
 *
 * \retval _SHR_E_NONE for success
 * \retval shr_error_e Standard error handling
 * \see
 ...
 */
shr_error_e diag_sand_packet_stream_start(
    int unit,
    rhhandle_t packet_h);

/**
 * \brief
 *    Stop stream for all packet handles in array
 * \param [in] unit - Unit #
 * \param [in] packet_h   - a handle to the object which holds all packet/stream related params
 *
 * \retval _SHR_E_NONE for success
 ...
 */
shr_error_e diag_sand_packet_stream_stop(
    int unit,
    rhhandle_t packet_h);
