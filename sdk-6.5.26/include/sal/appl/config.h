...
extern int 	sal_config_refresh(void) SAL_ATTR_WEAK;
extern int	sal_config_file_set(const char *fname, const char *tname) SAL_ATTR_WEAK;
extern int	sal_config_file_get(const char **fname, const char **tname) SAL_ATTR_WEAK;
extern int	sal_config_flush(void) SAL_ATTR_WEAK;
extern int	sal_config_save(char *file, char *pattern, int append) SAL_ATTR_WEAK;
extern char	*sal_config_get(const char *name) SAL_ATTR_WEAK;
extern int	sal_config_get_next(char **name, char **value) SAL_ATTR_WEAK;
extern int	sal_config_set(char *name, char *value) SAL_ATTR_WEAK;
extern void	sal_config_show(void) SAL_ATTR_WEAK;
extern void	sal_config_dump(void) SAL_ATTR_WEAK;
extern int	sal_config_init(void) SAL_ATTR_WEAK;
extern int	sal_config_file_process(char *fname,int *nof_imported_config_files,
                                        char *already_imported_tables[], char *dirname,
                                        int override_duplicates, int silant_import) SAL_ATTR_WEAK;
#ifdef BCM_SHARED_LIB_SDK
extern int sal_config_refresh_fileless(void) SAL_ATTR_WEAK;
extern int sal_config_set_full_info(char *name, char *value, uint16 flag, uint16 line, sc_hash_t sc_hash) SAL_ATTR_WEAK;
 ...
 */
extern void     sal_config_init_defaults(void) SAL_ATTR_WEAK;
