...
typedef enum bcm_switch_control_e {
    ...
    bcmSwitchL3VpbrTunnel = 1583,       /* In case enabled L3 VPBR tunnel can be
                                           ...
                                           DB is disabled. */
    bcmSwitchVxlanGbpFlexEnable = 1584, /* Requires BROADCOM_PREMIUM license */
    bcmSwitchVxlanGbpFlags = 1585,      /* Requires BROADCOM_PREMIUM license */
    bcmSwitchHigig3EthertypeMask = 1586, /* Set HIGIG3 ethernet type mask. */
    bcmSwitchLoopbackToLoopbackDrop = 1587, /* Enable loopback to loopback drop. */
    bcmSwitchHashUseFlowSelMplsEcmpSecondary = 1588, /* Enable/Disable flow based hashing of
                                           secondary ECMP for MPLS packets. */
    bcmSwitchHashEnable = 1589,         /* Enable/Disable RTAG7 hash. */
    bcmSwitchL3EgressFailoverEnable = 1590, /* Enable/Disable L3 egress object
                                           failover when failover is triggered
                                           by bcm_failover_status_set. */
    bcmSwitchPortL3EgressFailoverEnable = 1591, /* Enable/Disable L3 egress object
                                           failover when the egress port is
                                           down. */
    bcmSwitchTunnelTermEnable = 1592,   /* Enable/Disable tunnel termination. */
    bcmSwitchTunnelTermExactMatchEnable = 1593, /* Enable/Disable tunnel termination on
                                           exact match. */
    bcmSwitchTunnelInitDisableOverride = 1594, /* Override tunnel initiation disabling. */
    bcmSwitchTunnelTermDisableOverride = 1595, /* Override tunnel termination
                                           disabling. */
    bcmSwitchEcmpEnable = 1596,         /* Enable/Disable ECMP. */
    bcmSwitchInputPriBase = 1597,       /* Input priority base. */
    bcmSwitchTwampStatelessModeEnable = 1598, /* Enable/Disable TWAMP stateless mode. */
    bcmSwitchSamplingTxStatType = 1599, /* TX stat type for sampling. Valid
                                           values in
                                           bcm_switch_sampling_tx_stat_type_t. */
    bcmSwitchDosAttackInnerIcmpPktOversize = 1600, /* Max packet size for Inner ICMP size
                                           check. */
    bcmSwitchDosAttackMinInnerTcpHdrSize = 1601, /* Header length for min Inner TCP
                                           header checking. */
    bcmSwitchDosAttackInnerIcmpV6PingSize = 1602, /* Max packet size for Inner v6 ping
                                           check. */
    bcmSwitchDosAttackInnerV6MinFragSize = 1603, /* Min packet size for fragmented Inner
                                           IPv6. */
    bcmSwitch__Count = 1604             /* Must be last.  Not a usable value. */
...
#define BCM_SWITCH_EP_RECIRC_CPU_Q_HIGH_PRIORITY_SELECT (1U << 8)  /* Select the high
                                                          priority queue for
                                                          copy to CPU packets. */
#define BCM_SWITCH_EP_RECIRC_QOS_REPLACE    (1U << 9)  /* Replace QOS
                                                          attributes, including
                                                          internal priority,
                                                          internal congestion
                                                          notification and
                                                          color/drop precedence. */
#define BCM_SWITCH_EP_RECIRC_QOS_APPLY      (1U << 10) /* Apply the QOS
                                                          attributes (internal
                                                          priority/internal
                                                          congestion
                                                          notification/color/drop
                                                          precedence) carried in
                                                          the EPRC loopback
...
#define BCM_SWITCH_EP_RECIRC_OVERRIDE_PROFILE_OPTIONS_WITH_ID (1U << 0)  /* Create the EPRC
                                                          ...
                                                          ID. */
#define BCM_SWITCH_EP_RECIRC_OVERRIDE_PROFILE_OPTIONS_REPLACE (1U << 1)  /* Replace the EPRC
                                                          override profile. */
...
/* Set status of the EPRC override offset. */
...
/* Get status of the EPRC override offset. */
...
typedef struct bcm_switch_ep_recirc_profile_control_s {
    ...
    uint8 ingress_mirror_profile_strength; /* Profile strength for above mirrored
                                           packets. */
    int stateful_mod_profile_id;        /* Ingress/MMU stateful MOD. */
    uint8 stateful_mod_profile_strength; /* Profile strength for above
