...
/* BCM_MULTICAST_ENCAP_xxx flag definitions. */
#define BCM_MULTICAST_ENCAP_DST_MAC_REPLACE (1 << 0)   /* Replace the
                                                          destination MAC for
                                                          L3. */
#define BCM_MULTICAST_ENCAP_PAYLOAD_DST_MAC_REPLACE (1 << 1)   /* Replace the payload
                                                          destination MAC for L2
                                                          tunnel. */

/* Describes multicast encapsulation info. */
typedef struct bcm_multicast_encap_s {
    uint32 flags;                       /* BCM_MULTICAST_ENCAP_xxx flag
                                           definitions. */
    ...
    bcm_if_t egress_if;                 /* Egress object ID shared by overlay
                                           and L3 encapsulation. */
