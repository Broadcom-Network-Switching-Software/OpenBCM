...
typedef struct
{
    ...
    /**
     * Linked list to store the allocated profiles (will be used only in the same data advanced algorithm)
     */
    sw_state_ll_t ll_list;
    /**
     * Buffer to store a profile to ll head id map (will be used only in the same data advanced algorithm)
     */
    DNX_SW_STATE_BUFF *head_ids;
}  *multi_set_t;
