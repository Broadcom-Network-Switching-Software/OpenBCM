...
} resource_tag_bitmap_tag_level_info_t;
...
typedef struct
{
    ...
    int **next_alloc;
    /*
     * holding multihead linked list of the free grains per tags in case of
     * using RESOURCE_TAG_BITMAP_CREATE_GET_NOF_ELEMENTS_PER_GRAIN flag
     * */
    sw_state_ll_t free_grains;
    /*
     * linked list head index of the empty groups - (linked list that hold the grains that were not tagged)
     * Relevant only if using RESOURCE_TAG_BITMAP_CREATE_GET_NOF_ELEMENTS_PER_GRAIN flag
     */
     ...
     * */
    resource_tag_bitmap_tag_level_info_t tag_level_info[RESOURCE_TAG_BITMAP_MAX_NOF_TAGS_LEVELS];
