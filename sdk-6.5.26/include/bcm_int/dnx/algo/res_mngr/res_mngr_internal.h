...
#define RES_MGR_IF_ERR_EXIT_CONSIDER_SIMULATION(flags,expr)\
    do{\
        if(_SHR_IS_FLAG_SET(flags, DNX_ALGO_RES_ALLOCATE_SIMULATION) || _SHR_IS_FLAG_SET(flags, DNX_ALGO_RES_ALLOCATE_SILENTLY))\
        {\
...
#define DNX_ALGO_RES_MNGR_CREATE(node_id, algo_res, data, extra_arguments, alloc_flags)\
        SHR_IF_ERR_EXIT(dnx_algo_res_create(unit, node_id, &algo_res, data, extra_arguments,  alloc_flags))

#define DNX_ALGO_RES_MNGR_DESTROY(node_id, algo_res, extra_arguments)\
        SHR_IF_ERR_EXIT(dnx_algo_res_destroy(unit, node_id, &algo_res, extra_arguments))

#define DNX_ALGO_RES_MNGR_ALLOCATE_SINGLE(node_id, algo_res, flags, extra_arguments, element)\
        RES_MGR_IF_ERR_EXIT_CONSIDER_SIMULATION(flags, dnx_algo_res_allocate(unit, node_id, algo_res, flags, extra_arguments, element))

#define DNX_ALGO_RES_MNGR_ALLOCATE_SEVERAL(node_id, algo_res, flags, nof_elements, extra_arguments, element)\
        RES_MGR_IF_ERR_EXIT_CONSIDER_SIMULATION(flags, dnx_algo_res_allocate_several(unit, node_id, algo_res, flags, nof_elements, extra_arguments, element))

#define DNX_ALGO_RES_MNGR_IS_ALLOCATED(node_id, algo_res, element, is_allocated)\
        SHR_IF_ERR_EXIT(dnx_algo_res_is_allocated(unit, node_id, algo_res, element, is_allocated))

#define DNX_ALGO_RES_MNGR_FREE_SINGLE(node_id, algo_res, element, extra_arguments)\
        SHR_IF_ERR_EXIT(dnx_algo_res_free(unit, node_id, algo_res, element, extra_arguments))

#define DNX_ALGO_RES_MNGR_FREE_SEVERAL(node_id, algo_res, nof_elements, element, extra_arguments)\
        SHR_IF_ERR_EXIT(dnx_algo_res_free_several(unit, node_id, algo_res, nof_elements, element, extra_arguments))

#define DNX_ALGO_RES_MNGR_CLEAR(node_id, algo_res)\
        SHR_IF_ERR_EXIT(dnx_algo_res_clear(unit, node_id, algo_res))

#define DNX_ALGO_RES_MNGR_NOF_FREE_ELEMENTS_GET(node_id, algo_res, nof_free_elements)\
        SHR_IF_ERR_EXIT(dnx_algo_res_nof_free_elements_get(unit, node_id, algo_res, nof_free_elements))

#define DNX_ALGO_RES_MNGR_GET_NEXT(node_id, algo_res, element)\
        SHR_IF_ERR_EXIT(dnx_algo_res_get_next(unit, node_id, algo_res, element))

#define DNX_ALGO_RES_MNGR_DUMP_INFO_GET(node_id, algo_res, info)\
        SHR_IF_ERR_EXIT(dnx_algo_res_dump_info_get(unit, node_id, algo_res, info))

#define DNX_ALGO_RES_MNGR_PRINT(node_id, algo_res)\
        SHR_IF_ERR_EXIT(dnx_algo_res_print(unit, node_id, &algo_res))

#define DNX_ALGO_RES_MNGR_NOF_ALLOCATED_ELEMENTS_IN_RANGE_GET(node_id, algo_res, range_start, nof_elements_in_range, nof_allocated_elements)\
        SHR_IF_ERR_EXIT(dnx_algo_res_nof_allocated_elements_in_range_get\
                (unit, node_id, algo_res, range_start, nof_elements_in_range, nof_allocated_elements))

#define DNX_ALGO_RES_MNGR_ADVANCED_ALGORITHM_INFO_SET(node_id, algo_res, algorithm_info)\
        SHR_IF_ERR_EXIT(dnx_algo_res_advanced_algorithm_info_set\
                (unit, node_id, algo_res, algorithm_info))

#define DNX_ALGO_RES_MNGR_ADVANCED_ALGORITHM_INFO_GET(node_id, algo_res, algorithm_info)\
...
*  \param [in, out] extra_arguments - Pointer to memory holding extra arguments to be passed to the allocation function and recieved from the allocation function of an advanced algorithm.
...
*  \param [out] extra_arguments - Pointer to memory holding extra arguments to be recieved from the freeing function.
*                                 relevant only for advanced algorithm.
*
*  \return
*    shr_error_e
*
