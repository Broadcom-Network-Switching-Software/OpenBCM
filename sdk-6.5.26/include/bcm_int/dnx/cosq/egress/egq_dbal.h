...
/**
 * \brief -
 *   Enable / disable FQP to go to HP context in middle of packet .
 ...
 *   None
 */
shr_error_e dnx_egq_dbal_egq_preemption_set(
    int unit,
    int egq_if,
    int core_id,
    uint32 enable);
...
/**
 * \brief -
 *   configure if we use single TXI between FQP and ESB.
 * \param [in] unit - HW Unit-ID
 * \param [in] egq_if - Egq if
 * \param [in] core_id - core
 * \param [in] enable - enable/disable param
 ...
 *   None
 */
shr_error_e dnx_egq_dbal_use_single_txi_set(
    int unit,
    int egq_if,
    int core_id,
    uint32 enable);
