 ...
 */
/**
 * The initial size for TCAM entries cache.
 */
#define DNX_FIELD_TCAM_INITIAL_CACHE_SIZE       100
/*
 * }
 */
/*
...
/**
 * TCAM information for cache entries.
...
} dnx_field_group_tcam_cache_entry_t;
...
typedef struct
{
    ...
    int cache_size;
    /**
     * The list of entries saved on cache.
     * This list is only valid for TCAM field groups.
     */
    dnx_field_group_tcam_cache_entry_t *tcam_entries;
} dnx_field_group_tcam_cache_t;
...
typedef struct
{
    ...
    /**
     * Array that holds the core ID for each TCAM bank
     */
...
/**
 * Information for adding properties of external TCAM.
 */
typedef struct
{
     ...
     */
    uint32 flags;
} dnx_field_group_external_tcam_info_t;
...
typedef struct
{
    ...
    /**
     * Use the PPMC range for entries
     */
...
typedef struct
{
     ...
     * Only external TCAM field groups support ranged qualifiers.
     */
    ...
    dnx_field_group_tcam_info_t tcam_info;
    /**
      * This struct holds all advanced parameters for an external TCAM field group.
      * Specifically, it indicated properties to be set and minimum capacity.
      */
    dnx_field_group_external_tcam_info_t external_tcam_info;
    /**
...
typedef struct
{
    ...
    /**
     * Payload Offset - Relevant ONLY for external TCAM/Range FG
     * Payload Offset is the offset of the result of current FG inside the result buffer received from
     * the external TCAM.
     * This offset may vary for the same FG when it is attached to different contexts.
     */
    uint32 payload_offset;
    /**
     * Payload id - Relevant ONLY for the TCAM/Range FG
     * Payload id is the [0-7] id of the result of current FG inside the result buffer received from the external TCAM.
     * The external TCAM result buffer is a concatenation of the result buffers of it's field groups.
...
typedef struct
{
    ...
    dnx_field_key_length_type_e key_length_type;
    /**
     * The field_io that refers to the result of the PDB (for key_select).
     */
    dbal_enum_value_field_field_io_e field_io;
    /**
 ...
 * {
 */
extern dnx_field_group_tcam_cache_t
    dnx_field_group_tcam_entry_cache[BCM_MAX_NUM_UNITS][DNX_DATA_MAX_FIELD_GROUP_NOF_FGS];
/*
 * }
 */
...
*   * None
*/
shr_error_e dnx_field_group_tcam_external_swstate_init(
    int unit);
 ...
 *  Get context ID info from SW state of Field group
 * \param [in] unit              - Device ID
 * \param [in] fg_id             - Field group id that context ID is attached to
 * \param [in] context_id        - Context ID
 * \param [out] nof_quals_p      - Number of elements in key_qual_map and qual_attach_info.
 * \param [out] key_qual_map     - key template info per qualifier
 * \param [out] qual_attach_info - key attach info per qualifier
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
 ...
 *   * None
 */
shr_error_e dnx_field_group_key_quals_template_attach_info_get(
    int unit,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
    int *nof_quals_p,
...
*   * None
*/
shr_error_e dnx_field_group_exem_info(
    int unit,
    dnx_field_stage_e field_stage,
 ...
 */
shr_error_e dnx_field_group_fes_config(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
...
/**
* \brief
*  Set TCAM field group cache mode
* \param [in] unit         - Device id
...
*   * None
*/
shr_error_e dnx_field_group_cache(
    int unit,
    uint32 flags,
    dnx_field_group_t fg_id,
...
*  Note that we remove added EFES as well,
*  unlike dnx_field_group_context_create_fes_update() that doesn't add added EFES.
* \param [in] unit                - Device ID
* \param [in] stage               - Field Stage to create context for
* \param [in] context_id          - The created context ID.
...
/**
 * \brief
 *  Get whether an external tcam field group supports packing (has the packing flag used, BCM_FIELD_FLAG_QUAL_PACKING).
 * \param [in] unit     - Device ID
 * \param [in] fg_id    - Field group ID
 ...
 *   * None
 */
shr_error_e dnx_field_group_external_tcam_support_packing(
    int unit,
    dnx_field_group_t fg_id,
...
/**
 * \brief
 *  Retrieve if an ACE format uses the PPMC range for its entries.
 * \param [in] unit     - Device ID
 * \param [in] ace_id   - ACE ID
 * \param [out] use_ppmc_range_p - If TRUE uses PPMC entries range instead of ACE.
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
 ...
 *   * None
 */
shr_error_e dnx_field_ace_format_uses_ppmc_range(
    int unit,
    dnx_field_ace_id_t ace_id,
...
*   * None
*/
shr_error_e dnx_field_group_actions_template_create(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_action_in_group_info_t actions_info_arr[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP],
...
* \param [in] unit                  - Device ID
* \param [in] field_stage           - The stage for which to get the block size.
...
shr_error_e dnx_field_group_external_tcam_sync_set(
    int unit);
 ...
 * \param[in] core_ids   - Array of core ID per bank ID
 *
* \return
*   shr_error_e - Error Type
 */
...
/**
 * \brief
 *  Returns the list of field groups along with their entry count on the given
 *  TCAM bank for the given core.
 *
 * \param [in] unit            - Device ID
 * \param [in] core            - Core ID to get the status for
 * \param [in] bank_id         - Bank ID to get the status for
 * \param [out] fg_ids         - List of FG-IDs on the bank terminated by INVALID
 * \param [out] nof_entries    - List of entry count per each FG-ID on the bank
 *
 * \return
 ...
 *  None
 */
shr_error_e dnx_field_group_tcam_bank_status_get(
    int unit,
    int core,
    int bank_id,
...
/**
* \brief
*  run init for Field Group module.
* \param [in] unit       - Device ID
* \return
*   shr_error_e - Error Type
* \remark
...
*   * None
*/
shr_error_e dnx_field_group_init(
    int unit);

/**
* \brief
*  run deinit for Field Group module.
* \param [in] unit       - Device ID
* \return
*   shr_error_e - Error Type
* \remark
...
*   * None
*/
shr_error_e dnx_field_group_deinit(
    int unit);
