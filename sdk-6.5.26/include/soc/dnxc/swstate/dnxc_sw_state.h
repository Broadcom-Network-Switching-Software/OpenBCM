...
#define DNX_SW_STATE_RANGE_COPY(unit, node_id, location, input, type, num_of_elements, flags, dbg_string)\
    DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, location, input, num_of_elements * sizeof(type), flags, dbg_string);

#define DNX_SW_STATE_RANGE_READ(unit, node_id, location, input, type, num_of_elements, flags, dbg_string)\
...
#define DNX_SW_STATE_NO_WB_MODULE_INIT(unit, node_id, root, root_layout, implementation, type, nof_params,flags, dbg_string, function, function_init_cb, function_assigment)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher_module_init(unit, node_id, implementation));\
    DNX_SW_STATE_DIAG_INFO_PRE(unit, node_id, function);\
