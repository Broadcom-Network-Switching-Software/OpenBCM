...
/* { */
/*
 * For TSAN, we use wrappers for accessing 'sof flags' so we can suppress the 'get'
 * operation. However, this degrades performance so we keep the wrapeprs for TSAN
 * compilation only and, for all others, we directly access the bits.
 */
extern uint32 soc_flags_get(int unit);
extern int soc_flags_set(int unit, uint32 value);

#define SOC_FLAGS_GET(unit)             soc_flags_get(unit)
#define SOC_FLAGS_SET(unit,value)       soc_flags_set(unit,value)
/* } */
#else
/* { */
#define SOC_FLAGS_GET(unit)             (SOC_CONTROL(unit)->soc_flags)
#define SOC_FLAGS_SET(unit,value)       (SOC_CONTROL(unit)->soc_flags = value)
/* } */
#endif
...
#define SOC_UCHAN(unit,chan)            (SOC_CONTROL(unit)->soc_unet_channels & 0x1 << (chan))
#define SOC_KCHAN(unit,chan)            (SOC_UCHAN(unit, chan) == 0)
...
#define SOC_IS_RCPU_SCHAN(unit)         (SOC_FLAGS_GET(unit) & SOC_F_RCPU_SCHAN)
#define SOC_IS_RCPU_ONLY(unit)          (SOC_FLAGS_GET(unit) & SOC_F_RCPU_ONLY)
...
/* New access i/s is used by newer DNX devices, and here macros for it are defined */

/* Access information for the device */
#ifdef BCM_ACCESS_SUPPORT
/* Is the new access initialized for the device and therfore should be used for access */
#define SOC_NEW_ACCESS_RUNTIME_INFO(unit) (SOC_CONTROL(unit)->device_access_runtime_info)
#define SOC_IS_NEW_ACCESS_INITIALIZED(unit) (SOC_NEW_ACCESS_RUNTIME_INFO(unit) != NULL)
#define SOC_MAY_USE_NEW_ACCESS_BY_MCM(unit) ((SOC_NEW_ACCESS_RUNTIME_INFO(unit)->flags & ACCESS_RUNTIME_INFO_FLAGS_ALOW_MCM) != 0)
#ifdef NDEBUG
#define SOC_PREPARE_TO_USE_NEW_ACCESS(unit) do { if (!SOC_MAY_USE_NEW_ACCESS_BY_MCM(unit)) return SOC_E_UNAVAIL;} while (0)
#else
#define SOC_PREPARE_TO_USE_NEW_ACCESS(unit) assert(SOC_MAY_USE_NEW_ACCESS_BY_MCM(unit))
#endif
#else
#define SOC_NEW_ACCESS_RUNTIME_INFO(unit) NULL
#define SOC_IS_NEW_ACCESS_INITIALIZED(unit) 0
#define SOC_MAY_USE_NEW_ACCESS_BY_MCM(unit) 0
#define SOC_PREPARE_TO_USE_NEW_ACCESS(unit) do {} while (0)
...
#define SOC_IS_JERICHO(unit)      ((SOC_INFO(unit).chip_type  == SOC_INFO_CHIP_TYPE_JERICHO) || (SOC_IS_QMX(unit)) || (SOC_IS_QMX_B0(unit))  || (SOC_IS_QAX(unit))  || (SOC_IS_JERICHO_A0(unit))  || (SOC_IS_JERICHO_B0(unit)) || (SOC_IS_JERICHO_PLUS(unit)) )
...
#define SOC_IS_DNX2(unit)               ((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_JERICHO2) || (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_J2C) || (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_Q2A) || (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_J2P) || (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_J2X))
...
#define SOC_IS_JERICHO2_B1(unit)       ((SOC_INFO(unit).driver_type == SOC_CHIP_BCM88690_B0) && CMDEV(unit).dev.rev_id >= BCM88690_B1_REV_ID)
#define SOC_IS_JERICHO2_B0(unit)       ((SOC_INFO(unit).driver_type == SOC_CHIP_BCM88690_B0) && CMDEV(unit).dev.rev_id == BCM88690_B0_REV_ID)
#define SOC_IS_JERICHO2(unit)          (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_JERICHO2)
/* SKU Support*/
...
#define SOC_IS_J2P_A0(unit)             ((SOC_INFO(unit).driver_type == SOC_CHIP_BCM88850_A0) && CMDEV(unit).dev.rev_id == BCM88850_A0_REV_ID)
#define SOC_IS_J2P_A1(unit)             ((SOC_INFO(unit).driver_type == SOC_CHIP_BCM88850_A0) && CMDEV(unit).dev.rev_id == BCM88850_A1_REV_ID)
#define SOC_IS_J2P_A2(unit)             ((SOC_INFO(unit).driver_type == SOC_CHIP_BCM88850_A0) && CMDEV(unit).dev.rev_id >= BCM88850_A2_REV_ID)
...
#define SOC_IS_J2X(unit)                (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_J2X)
...
#define SOC_UNSIGNED_PORT_VALID_RANGE(unit,port) \
...
#define SOC_PORT_VALID_RANGE(unit,port) \
             ((port) >= 0 && SOC_UNSIGNED_PORT_VALID_RANGE(unit,port))
...
#define SOC_UNSIGNED_PORT_VALID(unit,port)   \
    (SOC_UNSIGNED_PORT_VALID_RANGE(unit, port) && \
        ((SOC_PORT_TYPE(unit, port) != 0) || \
        SOC_PP_PORT_VALID(unit, port)))
#else
...
#define NANOSYNC_BLOCK(unit)            (SOC_INFO(unit).nanosync_block)
...
#ifdef SOC_MAX_NUM_FMAC_BLKS
#undef SOC_MAX_NUM_FMAC_BLKS
#endif
#if defined(BCM_88790_A0) || defined(BCM_88850_A0)
#ifndef SOC_MAX_NUM_FMAC_BLKS
...
#ifdef SOC_MAX_NUM_DDHA_BLKS
#undef SOC_MAX_NUM_DDHA_BLKS
#endif
#ifdef BCM_88850_A0
#ifndef SOC_MAX_NUM_DDHA_BLKS
...
#define SOC_MAX_NUM_DDHB_BLKS 8
...
#define SOC_MAX_NUM_MACT_BLKS 4
...
#define SOC_MAX_NUM_MSU_BLKS 36
#define MSU_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_MSU_BLKS)) ? SOC_INFO(unit).msu_blocks[instance] : (-1))
#define SOC_MAX_NUM_PMU_BLKS 36
#define PMU_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_PMU_BLKS)) ? SOC_INFO(unit).pmu_blocks[instance] : (-1))
#define SOC_MAX_NUM_NBU_BLKS 36
#define NBU_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_NBU_BLKS)) ? SOC_INFO(unit).nbu_blocks[instance] : (-1))
#define SOC_MAX_NUM_DDHX_BLKS 12
#define DDHX_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_DDHX_BLKS)) ? SOC_INFO(unit).ddhx_blocks[instance] : (-1))
#define SOC_MAX_NUM_TCMX_BLKS 16
#define TCMX_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_TCMX_BLKS)) ? SOC_INFO(unit).tcmx_blocks[instance] : (-1))
#define SOC_MAX_NUM_DTQ_BLKS 4
#define DTQ_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_DTQ_BLKS)) ? SOC_INFO(unit).dtq_blocks[instance] : (-1))
#define SOC_MAX_NUM_NBUTX_BLKS 36
#define NBUTX_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_NBUTX_BLKS)) ? SOC_INFO(unit).nbutx_blocks[instance] : (-1))
#define SOC_MAX_NUM_PBU_BLKS 4
#define PBU_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_PBU_BLKS)) ? SOC_INFO(unit).pbu_blocks[instance] : (-1))
#define SOC_MAX_NUM_PHB_BLKS 36
#define PHB_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_PHB_BLKS)) ? SOC_INFO(unit).phb_blocks[instance] : (-1))
#define SOC_MAX_NUM_QHC_BLKS 6
#define QHC_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_QHC_BLKS)) ? SOC_INFO(unit).qhc_blocks[instance] : (-1))
...
#define IPC_BLOCK(unit) (SOC_INFO(unit).ipc_block)
#define ARB_BLOCK(unit) (SOC_INFO(unit).arb_block)
#define FAWR_BLOCK(unit) (SOC_INFO(unit).fawr_block)
#define FAWT_BLOCK(unit) (SOC_INFO(unit).fawt_block)
#define FBSW_BLOCK(unit) (SOC_INFO(unit).fbsw_block)
#define FDCRL_BLOCK(unit) (SOC_INFO(unit).fdcrl_block)
#define FDCRS_BLOCK(unit) (SOC_INFO(unit).fdcrs_block)
#define FDMP_BLOCK(unit) (SOC_INFO(unit).fdmp_block)
#define FDMXA_BLOCK(unit) (SOC_INFO(unit).fdmxa_block)
#define FDMXB_BLOCK(unit) (SOC_INFO(unit).fdmxb_block)
#define FDMXC_BLOCK(unit) (SOC_INFO(unit).fdmxc_block)
#define FDMXT_BLOCK(unit) (SOC_INFO(unit).fdmxt_block)
#define FECRL_BLOCK(unit) (SOC_INFO(unit).fecrl_block)
#define FECRS_BLOCK(unit) (SOC_INFO(unit).fecrs_block)
#define FFLXO_BLOCK(unit) (SOC_INFO(unit).fflxo_block)
#define FFOA_BLOCK(unit) (SOC_INFO(unit).ffoa_block)
#define FFOB_BLOCK(unit) (SOC_INFO(unit).ffob_block)
#define FGDMP_BLOCK(unit) (SOC_INFO(unit).fgdmp_block)
#define FGMAP_BLOCK(unit) (SOC_INFO(unit).fgmap_block)
#define FLEXMAC_BLOCK(unit) (SOC_INFO(unit).flexmac_block)
#define FLOTN_BLOCK(unit) (SOC_INFO(unit).flotn_block)
#define FMACR_BLOCK(unit) (SOC_INFO(unit).fmacr_block)
#define FMACT_BLOCK(unit) (SOC_INFO(unit).fmact_block)
#define FMAP_BLOCK(unit) (SOC_INFO(unit).fmap_block)
#define FMXA_BLOCK(unit) (SOC_INFO(unit).fmxa_block)
#define FMXBI_BLOCK(unit) (SOC_INFO(unit).fmxbi_block)
#define FMXBJ_BLOCK(unit) (SOC_INFO(unit).fmxbj_block)
#define FMXC_BLOCK(unit) (SOC_INFO(unit).fmxc_block)
#define FMXT_BLOCK(unit) (SOC_INFO(unit).fmxt_block)
#define FOSW_BLOCK(unit) (SOC_INFO(unit).fosw_block)
#define FPMR_BLOCK(unit) (SOC_INFO(unit).fpmr_block)
#define FPMT_BLOCK(unit) (SOC_INFO(unit).fpmt_block)
#define FPTPR_BLOCK(unit) (SOC_INFO(unit).fptpr_block)
#define FPTPT_BLOCK(unit) (SOC_INFO(unit).fptpt_block)
#define FRWA_BLOCK(unit) (SOC_INFO(unit).frwa_block)
#define FRWBC_BLOCK(unit) (SOC_INFO(unit).frwbc_block)
#define OFR_BLOCK(unit) (SOC_INFO(unit).ofr_block)
#define OFT_BLOCK(unit) (SOC_INFO(unit).oft_block)
#define CIG_BLOCK(unit) (SOC_INFO(unit).cig_block)
#define FECPB_BLOCK(unit) (SOC_INFO(unit).fecpb_block)
#define FMXBK_BLOCK(unit) (SOC_INFO(unit).fmxbk_block)
#define WB_BLOCK(unit) (SOC_INFO(unit).wb_block)
#define FICPB_BLOCK(unit) (SOC_INFO(unit).ficpb_block)
#define FDMXBJ_BLOCK(unit) (SOC_INFO(unit).fdmxbj_block)
#define W40_BLOCK(unit) (SOC_INFO(unit).w40_block)
#define FMXAJ_BLOCK(unit) (SOC_INFO(unit).fmxaj_block)
#define TSN_BLOCK(unit) (SOC_INFO(unit).tsn_block)
#define FMXAI_BLOCK(unit) (SOC_INFO(unit).fmxai_block)
#define FDMXBI_BLOCK(unit) (SOC_INFO(unit).fdmxbi_block)

#define SOC_MAX_NUM_DC3MAC_BLKS 36
#define DC3MAC_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_DC3MAC_BLKS)) ? SOC_INFO(unit).dc3mac_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_DC3PORT_BLKS 36
#define DC3PORT_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_DC3PORT_BLKS)) ? SOC_INFO(unit).dc3port_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_HBW_BLKS 4
#define HBW_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_HBW_BLKS)) ? SOC_INFO(unit).hbw_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_HPW_BLKS 4
#define HPW_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_HPW_BLKS)) ? SOC_INFO(unit).hpw_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_FDMX_BLKS 2
#define FDMX_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_FDMX_BLKS)) ? SOC_INFO(unit).fdmx_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_FLFR_BLKS 4
#define FLFR_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_FLFR_BLKS)) ? SOC_INFO(unit).flfr_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_FLFT_BLKS 2
#define FLFT_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_FLFT_BLKS)) ? SOC_INFO(unit).flft_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_FMX_BLKS 2
#define FMX_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_FMX_BLKS)) ? SOC_INFO(unit).fmx_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_FOAM_BLKS 2
#define FOAM_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_FOAM_BLKS)) ? SOC_INFO(unit).foam_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_FODUO_BLKS 2
#define FODUO_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_FODUO_BLKS)) ? SOC_INFO(unit).foduo_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_FRA_BLKS 2
#define FRA_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_FRA_BLKS)) ? SOC_INFO(unit).fra_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_IPW_BLKS 8
#define IPW_BLOCK(unit, instance) ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_IPW_BLKS)) ? SOC_INFO(unit).ipw_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
#define SOC_MAX_NUM_MDBK_BLKS 8
...
#define SOC_MAX_NUM_EPPS_BLKS 8
...
#ifdef SOC_MAX_NUM_FSRD_BLKS
#undef SOC_MAX_NUM_FSRD_BLKS
#endif
#if defined(BCM_88790_A0) || defined(BCM_88850_A0)
#ifndef SOC_MAX_NUM_FSRD_BLKS
...
#define SOC_MAX_NUM_RTP_BLKS 1
#define RTP_BLOCK(unit, instance) \
( ((instance >= 0) && (instance < SOC_MAX_NUM_RTP_BLKS)) ? \
   SOC_INFO(unit).rtp_blocks[instance] : (-1))
...
#define SOC_MAX_NUM_MSW_BLKS 2
#define MSW_BLOCK(unit, instance) \
( (((instance) >= 0) && ((instance) < SOC_MAX_NUM_MSW_BLKS)) ? \
   SOC_INFO(unit).msw_blocks[instance] : (instance == SOC_CORE_ALL ? SOC_CORE_ALL : -1))
...
#define SOC_MAX_NUM_CCML_BLKS 16
#define CCML_BLOCK(unit, instance) \
( ((instance >= 0) && (instance < SOC_MAX_NUM_CCML_BLKS)) ? \
   SOC_INFO(unit).ccml_block[instance] : (-1))
#define SOC_MAX_NUM_MESH_TOPOLOGY_WRAP_BLKS 2
#define MESH_TOPOLOGY_WRAP_BLOCK(unit, instance) \
( ((instance >= 0) && (instance < SOC_MAX_NUM_MESH_TOPOLOGY_WRAP_BLKS)) ? \
   SOC_INFO(unit).mesh_topology_wrap_block[instance] : (-1))
#define SOC_MAX_NUM_DTM_BLKS 16
#define DTM_BLOCK(unit, instance) \
( ((instance >= 0) && (instance < SOC_MAX_NUM_DTM_BLKS)) ? \
   SOC_INFO(unit).dtm_block[instance] : (-1))
#define SOC_MAX_NUM_DTL_BLKS 16
#define DTL_BLOCK(unit, instance) \
( ((instance >= 0) && (instance < SOC_MAX_NUM_DTL_BLKS)) ? \
   SOC_INFO(unit).dtl_block[instance] : (-1))
#define SOC_MAX_NUM_DFL_BLKS 16
#define DFL_BLOCK(unit, instance) \
( ((instance >= 0) && (instance < SOC_MAX_NUM_DFL_BLKS)) ? \
   SOC_INFO(unit).dfl_block[instance] : (-1))
#define SOC_MAX_NUM_DFL_FBC_BLKS 32
#define DFL_FBC_BLOCK(unit, instance) \
( ((instance >= 0) && (instance < SOC_MAX_NUM_DFL_FBC_BLKS)) ? \
   SOC_INFO(unit).dfl_fbc_block[instance] : (-1))
#define SOC_MAX_NUM_VTMON_BLKS 8
#define VTMON_BLOCK(unit, instance) \
( ((instance >= 0) && (instance < SOC_MAX_NUM_VTMON_BLKS)) ? \
   SOC_INFO(unit).vtmon_block[instance] : (-1))


#define MCT_BLOCK(unit)         (SOC_INFO(unit).mct_block)
#define PCU_BLOCK(unit)         (SOC_INFO(unit).pcu_block)
...
#define BRDC_CCML_BLOCK(unit)  (SOC_INFO(unit).brdc_ccml_block)
#define BRDC_DFL_BLOCK(unit)  (SOC_INFO(unit).brdc_dfl_block)
#define BRDC_DTL_BLOCK(unit)  (SOC_INFO(unit).brdc_dtl_block)
#define BRDC_DTM_BLOCK(unit)  (SOC_INFO(unit).brdc_dtm_block)
#define BRDC_RTP_BLOCK(unit)  (SOC_INFO(unit).brdc_rtp_block)
#define BRDC_MCT_BLOCK(unit)  (SOC_INFO(unit).brdc_mct_block)
#define BRDC_DFL_FBC_BLOCK(unit)  (SOC_INFO(unit).brdc_dfl_fbc_block)
#define BRDC_CCH_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_cch_vd0_block)
#define BRDC_CCH_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_cch_vd1_block)
#define BRDC_CCML_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_ccml_vd0_block)
#define BRDC_CCML_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_ccml_vd1_block)
#define BRDC_DCH_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_dch_vd0_block)
#define BRDC_DCH_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_dch_vd1_block)
#define BRDC_DFL_FBC_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_dfl_fbc_vd0_block)
#define BRDC_DFL_FBC_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_dfl_fbc_vd1_block)
#define BRDC_DFL_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_dfl_vd0_block)
#define BRDC_DFL_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_dfl_vd1_block)
#define BRDC_DTL_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_dtl_vd0_block)
#define BRDC_DTL_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_dtl_vd1_block)
#define BRDC_DTM_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_dtm_vd0_block)
#define BRDC_DTM_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_dtm_vd1_block)
#define BRDC_FMAC_HIGH_BLOCK(unit)  (SOC_INFO(unit).brdc_fmac_high_block)
#define BRDC_FMAC_LOW_BLOCK(unit)  (SOC_INFO(unit).brdc_fmac_low_block)
#define BRDC_FMAC_HIGH_BLOCK_VD0(unit)  (SOC_INFO(unit).brdc_fmac_high_block_vd0)
#define BRDC_FMAC_HIGH_BLOCK_VD1(unit)  (SOC_INFO(unit).brdc_fmac_high_block_vd1)
#define BRDC_FMAC_LOW_BLOCK_VD0(unit)  (SOC_INFO(unit).brdc_fmac_low_block_vd0)
#define BRDC_FMAC_LOW_BLOCK_VD1(unit)  (SOC_INFO(unit).brdc_fmac_low_block_vd1)
#define BRDC_FSRD_HIGH_BLOCK(unit)  (SOC_INFO(unit).brdc_fsrd_high_block)
#define BRDC_FSRD_LOW_BLOCK(unit)  (SOC_INFO(unit).brdc_fsrd_low_block)
#define BRDC_LCM_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_lcm_vd0_block)
#define BRDC_LCM_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_lcm_vd1_block)
#define BRDC_QRH_VD0_BLOCK(unit)  (SOC_INFO(unit).brdc_qrh_vd0_block)
#define BRDC_QRH_VD1_BLOCK(unit)  (SOC_INFO(unit).brdc_qrh_vd1_block)

#if defined(BCM_88850_A0) 
#define SOC_MAX_NUM_BRDC_FSRD_BLKS 4
#else
...
#if defined(BCM_88850_A0) 
#define SOC_MAX_NUM_BRDC_FMAC_BLKS 4
#else
...
#if defined(BCM_88850_A0) 
#define SOC_MAX_NUM_BRDC_HBC_BLKS 6
#else
...
#define PBMP_D3CE_ALL(unit)             SOC_PORT_BITMAP(unit,d3c)
...
#define PBMP_OTN_ALL(unit)              SOC_PORT_BITMAP(unit, otn)
...
#define IS_FLEXE_MAC_PORT(unit,port)           \
        (SOC_PBMP_MEMBER(PBMP_FLEXE_MAC_ALL(unit), port))
#define IS_OTN_PORT(unit,port)           \
        (SOC_PBMP_MEMBER(PBMP_OTN_ALL(unit), port))
...
#define SOC_SL_MODE(unit) \
    ((SOC_FLAGS_GET(unit) & SOC_F_SL_MODE) != 0)

#define SOC_SL_MODE_SET(unit, sl_mode) \
    if (sl_mode) SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_SL_MODE); \
    else SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_SL_MODE)

/* KNET Mode set/get macros */
#define SOC_KNET_MODE(unit) \
    ((SOC_FLAGS_GET(unit) & SOC_F_KNET_MODE) != 0)

#define SOC_KNET_MODE_SET(unit, knet_mode) \
    if (knet_mode) SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_KNET_MODE); \
    else SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_KNET_MODE)

/* URPF on/off set/get macros */
#define SOC_URPF_STATUS_GET(unit) \
    ((SOC_FLAGS_GET(unit) & SOC_F_URPF_ENABLED) != 0)

#define SOC_URPF_STATUS_SET(unit, status) \
    if (status) SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_URPF_ENABLED); \
    else SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_URPF_ENABLED)
...
#define SOC_MEM_CLEAR_USE_DMA(unit) \
    ((SOC_FLAGS_GET(unit) & SOC_F_MEM_CLEAR_USE_DMA) != 0)

#define SOC_MEM_CLEAR_USE_DMA_SET(unit, status) \
    if (status) SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_MEM_CLEAR_USE_DMA); \
    else SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_MEM_CLEAR_USE_DMA)

/* Device should use hw pipe clear for memory clear ops */
#define SOC_MEM_CLEAR_HW_ACC(unit) \
    ((SOC_FLAGS_GET(unit) & SOC_F_MEM_CLEAR_HW_ACC) != 0)

#define SOC_MEM_CLEAR_HW_ACC_SET(unit, status) \
    if (status) SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_MEM_CLEAR_HW_ACC); \
    else SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_MEM_CLEAR_HW_ACC)
...
#define SOC_USE_GPORT(unit) \
    ((SOC_FLAGS_GET(unit) & SOC_F_GPORT) != 0)

#define SOC_USE_GPORT_SET(unit, status) \
    if (status) SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_GPORT); \
    else SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_GPORT)
...
#define SOC_IPMCREPLSHR_GET(unit) \
    ((SOC_FLAGS_GET(unit) & SOC_F_IPMCREPLSHR) != 0)

#define SOC_IPMCREPLSHR_SET(unit, status) \
    if (status) SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_IPMCREPLSHR); \
    else SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_IPMCREPLSHR)

/* Device should use gport for source/destination notation */
#define SOC_REMOTE_ENCAP(unit) \
    ((SOC_FLAGS_GET(unit) & SOC_F_REMOTE_ENCAP) != 0)

#define SOC_REMOTE_ENCAP_SET(unit, status) \
    if (status) SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_REMOTE_ENCAP); \
    else SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_REMOTE_ENCAP)
...
typedef struct soc_stat_s {
    ...
    uint32      schan_op;       /* Number of S-Channel operations. Please note that this caunter is not thread safe abd its value is only approximate and may be inaccurate*/
    ...
    uint32      err_sbusdma_sw_timeout; /* SBUSDMA SW timeout error count */
    uint32      err_l2fifo_dma; /* DMA error in L2FIFO */
    uint32      err_dma_addr_decode; /* DMA address decode error */
    ...
    uint32      schan_timeout_mem_read;     /* Number of timeout on schan_op READ_MEMORY */
    uint32      schan_timeout_mem_write;    /* Number of timeout on schan_op WRITE_MEMORY */
    uint32      schan_timeout_reg_read;     /* Number of timeout on schan_op READ_REGISTER */
    uint32      schan_timeout_reg_write;    /* Number of timeout on schan_op WRITE_REGISTER */
    uint32      schan_timeout_tbl_insert;   /* Number of timeout on schan_op TABLE_INSERT */
    uint32      schan_timeout_tbl_delete;   /* Number of timeout on schan_op TABLE_DELETE */
    uint32      schan_timeout_tbl_lookup;   /* Number of timeout on schan_op TABLE_LOOKUP */
} soc_stat_t;
...
typedef struct soc_driver_s {
    ...
    uint8                   *indirect_write_reg_nof_uint32s;  /* size in words of indirect write data register per block */
...
typedef struct {
    ...
    soc_ptype_t     otn;                    /* OTN physical port */
    ...
    pbmp_t          pbmp_mmu_lossless;      /* bitmap of ports lossless is enabled */
    int             lossless_portmax;       /* max lossless ports supportd in switch*/
    int             lossless_headroom_reserve; /* max headroom to be reserved for lossless*/
    ...
    int             cw;                     /* CW Block */
    int             iproc_block;
    int             nanosync_block;         /* NANOSYNC block */
    int             crypto_block;
    ...
    int             nof_pmqs;
        ...
        int             ipt_blocks[SOC_MAX_NUM_IPT_BLKS];
        int             iqm_blocks[SOC_MAX_NUM_IQM_BLKS];
        int             pqp_blocks[SOC_MAX_NUM_PQP_BLKS];
        int             ire_blocks[SOC_MAX_NUM_IRE_BLKS];
        int             irr_block;
        ...
        int             dtq_blocks[SOC_MAX_NUM_DTQ_BLKS];
        int             nbutx_blocks[SOC_MAX_NUM_NBUTX_BLKS];
        int             pbu_blocks[SOC_MAX_NUM_PBU_BLKS];
        int             phb_blocks[SOC_MAX_NUM_PHB_BLKS];
        int             qhc_blocks[SOC_MAX_NUM_QHC_BLKS];
        ...
        int             oft_block;
        int             cig_block;
        int             ipw_blocks[SOC_MAX_NUM_IPW_BLKS];
        int             dc3mac_blocks[SOC_MAX_NUM_DC3MAC_BLKS];
        int             dc3port_blocks[SOC_MAX_NUM_DC3PORT_BLKS];
        int             hbw_blocks[SOC_MAX_NUM_HBW_BLKS];
        int             hpw_blocks[SOC_MAX_NUM_HPW_BLKS];
        int             fdmx_blocks[SOC_MAX_NUM_FDMX_BLKS];
        int             flfr_blocks[SOC_MAX_NUM_FLFR_BLKS];
        int             flft_blocks[SOC_MAX_NUM_FLFT_BLKS];
        int             fmx_blocks[SOC_MAX_NUM_FMX_BLKS];
        int             foam_blocks[SOC_MAX_NUM_FOAM_BLKS];
        int             foduo_blocks[SOC_MAX_NUM_FODUO_BLKS];
        int             fra_blocks[SOC_MAX_NUM_FRA_BLKS];
        ...
        int             rtp_blocks[SOC_MAX_NUM_RTP_BLKS];
        ...
        int             brdc_ccml_block;
        int             brdc_dfl_block;
        int             brdc_dfl_fbc_block;
        int             brdc_cch_vd0_block;
        int             brdc_cch_vd1_block;
        int             brdc_ccml_vd0_block;
        int             brdc_ccml_vd1_block;
        int             brdc_dch_vd0_block;
        int             brdc_dch_vd1_block;
        int             brdc_dfl_fbc_vd0_block;
        int             brdc_dfl_fbc_vd1_block;
        int             brdc_dfl_vd0_block;
        int             brdc_dfl_vd1_block;
        int             brdc_dtl_vd0_block;
        int             brdc_dtl_vd1_block;
        int             brdc_dtm_vd0_block;
        int             brdc_dtm_vd1_block;
        int             brdc_fmac_high_block;
        int             brdc_fmac_low_block;
        int             brdc_fmac_high_block_vd0;
        int             brdc_fmac_high_block_vd1;
        int             brdc_fmac_low_block_vd0;
        int             brdc_fmac_low_block_vd1;
        int             brdc_fsrd_high_block;
        int             brdc_fsrd_low_block;
        int             brdc_lcm_vd0_block;
        int             brdc_lcm_vd1_block;
        int             brdc_qrh_vd0_block;
        int             brdc_qrh_vd1_block;
        int             brdc_dtl_block;
        int             brdc_dtm_block;
        int             brdc_rtp_block;
        int             brdc_mct_block;
        int             brdc_fsrd_block[SOC_MAX_NUM_BRDC_FSRD_BLKS];
        ...
        int             brdc_fmac_block[SOC_MAX_NUM_BRDC_FMAC_BLKS];
        int             brdc_hbc_block[SOC_MAX_NUM_BRDC_HBC_BLKS];
        int             brdc_cgm_block;
        int             brdc_egq_block;
        int             brdc_epni_block;
        int             brdc_ihb_block;
        ...
        int             ccml_block[SOC_MAX_NUM_CCML_BLKS];
        int             mesh_topology_wrap_block[SOC_MAX_NUM_MESH_TOPOLOGY_WRAP_BLKS];
        int             dtm_block[SOC_MAX_NUM_DTM_BLKS];
        int             dtl_block[SOC_MAX_NUM_DTL_BLKS];
        int             dfl_block[SOC_MAX_NUM_DFL_BLKS];
        int             dfl_fbc_block[SOC_MAX_NUM_DFL_FBC_BLKS];
        int             vtmon_block[SOC_MAX_NUM_VTMON_BLKS];
        int             brdc_cch_block;
        int             brdc_dcml_block;
        int             brdc_lcm_block;
        int             brdc_qrh_block;
        ...
        int             fscl_block;
        /* J2X */
        int             msw_blocks[SOC_MAX_NUM_MSW_BLKS];
    ...
    */
    ...
    int cosq_buf_mode; /* Auto (0) or Manual (1) mode for MMU on speed change */

#if defined(BCM_HURRICANE4_SUPPORT)
    uint8 clk_to_pm_clk_factor;
...
#define   SOC_F_RESET              (SOC_F_ATTACHED|SOC_F_INITED|SOC_F_POLLED|SOC_F_HW_RESETING|SOC_F_KNET_MODE)
    ...
    void        *drv;
#ifdef BCM_ACCESS_SUPPORT
    access_runtime_info_t *device_access_runtime_info; /* Access information for the device */
#endif
    ...
    int         sbusdma_flush_cache_enable;
#endif /* BCM_SBUSDMA_SUPPORT */
...
#define  MULTI_MOVE_MODE_BREADTH            0x4
    ...
    /* Physical port bit map of Fast links recovery. */
    pbmp_t      hw_link_flr_support_ppbmp;

    /* FLR linkup callback for hardware linkscan. */
    soc_linkscan_flr_linkup_callback_f hw_linkscan_flr_linkup_cb;
...
#define         SOC_DMA_UNLOCK(l)    (void)sal_spl(l)
    ...
    uint32      soc_unet_channels;      /* Unet channel bitmap */
    ...
    int         soc_dv_rx_dv_done_unmatch_cnt;     /* # on RX dv done mismatch on chain done */
    int         soc_dv_rx_dv_done_proc_retry_cnt;  /* # on RX dv done process retry count on mismatch */
    int         soc_dcb_error_mitigate_enable;     /* Enable retry and DMA restart */
    ...
    int         soc_max_pdma_err_count; /* Max count of consective PKTDMA error */
    int         soc_pdma_err_count[SOC_CMCS_NUM_MAX][MAX_N_DMA_CHAN]; /* Count of consective PKTDMA error */
    ...
    VOL int     counter_max_dma_err_count;  /* Max count of consective DMA error */
    VOL int     counter_port_dma_err_count; /* Current count of consective port DMA error */
    VOL int     counter_blk_dma_err_count;  /* Current count of consective bulk DMA error */
    VOL sal_sem_t
...
#define SOC_PSTATS_UNLOCK(unit) \
    ...
    uint32      ctr_evict_enable;       /* Counter eviction enabled in
    ...
    int ls_mbox_id;
    int led_mbox_id;
    sal_mutex_t cmicx_link_lock;
    pbmp_t cmicx_link_stat;
    pbmp_t cmicx_link_stat_cache;
    pbmp_t cmicx_link_latch_down;
    pbmp_t cmicx_link_latch_down_cache;
    ...
    int interrupt_all_enable;

    /* SW lookup on Station-Move of old entries when in L2 FIFO mode */
    int l2_fifo_mode_station_move_sw_lookup;
    int eee_phy_config_ignore;

    /* Allow to retry the timed out S-Channel operations */
    int schan_timeout_retry_enable;
} soc_control_t;
...
typedef struct soc_persist_s {
    ...
    pbmp_t      lc_pbm_flr_enable;      /* Enable FLR(Fast link recovery). */
    uint32      lc_flr_timeout[SOC_MAX_NUM_PORTS]; /* FLR timeout value in ms */
...
/* map 0..ndev-1 to unit=logical device number, used for looping over units */
...
#if (THREAD_SANITIZER_IS_ACTIVE == 1)
/* { */
extern int soc_state_set(int unit, uint32 value);
extern uint32 soc_state_get(int unit);
#define SOC_STATE_SET(unit,value) \
    soc_state_set(unit, value)
#define SOC_STATE_GET(unit) \
    soc_state_get(unit)
/* } */
#else
/* { */
#define SOC_STATE_SET(unit,value) \
    soc_state[unit] = value
#define SOC_STATE_GET(unit) \
    soc_state[unit]
/* } */
#endif
...
#define SOC_FAST_REBOOT(unit)       (SOC_STATE_GET(unit) & SOC_ST_F_FAST_REBOOTING)
#define SOC_FAST_REBOOT_START(unit) (SOC_STATE_SET(unit,SOC_STATE_GET(unit) | SOC_ST_F_FAST_REBOOTING))
#define SOC_FAST_REBOOT_DONE(unit)   (SOC_STATE_SET(unit, SOC_STATE_GET(unit) & ~SOC_ST_F_FAST_REBOOTING))

#define SOC_ST_F_RELOADING         0x1    /* Warm boot in progress, device is reloading */
#define SOC_WARM_BOOT(unit)       (SOC_STATE_GET(unit) & SOC_ST_F_RELOADING)
#define SOC_WARM_BOOT_START(unit) (SOC_STATE_SET(unit,SOC_STATE_GET(unit) | SOC_ST_F_RELOADING))
#define SOC_WARM_BOOT_DONE(unit)  (SOC_STATE_SET(unit,SOC_STATE_GET(unit) & ~SOC_ST_F_RELOADING))
#define SOC_IS_DONE_INIT(unit) 		((SOC_FLAGS_GET(unit) & SOC_F_ALL_MODULES_INITED) && \
                                     (SOC_FLAGS_GET(unit) & SOC_F_ATTACHED) && \
                                     (SOC_FLAGS_GET(unit) & SOC_F_INITED))
#define SOC_IS_INIT(unit) 		    ((SOC_FLAGS_GET(unit) & SOC_F_ATTACHED) && \
                                     (SOC_FLAGS_GET(unit) & SOC_F_INITED))
/*(((SOC_FLAGS_GET(unit) & SOC_F_RESET) == SOC_F_RESET) ? TRUE : FALSE)*/
...
#define SOC_HW_RESET(unit)          (SOC_FLAGS_GET(unit) & SOC_F_HW_RESETING)
#define SOC_HW_RESET_START(unit)    (SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_HW_RESETING))
#define SOC_HW_RESET_DONE(unit)     (SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) & ~SOC_F_HW_RESETING))
...
#define SOC_IS_RELOADING(unit)    (SOC_STATE_GET(unit) & SOC_ST_F_EASY_RELOAD)
#define SOC_RELOAD_MODE_SET(unit, reload_mode) \
    if (reload_mode) {              \
    SOC_STATE_SET(unit,SOC_STATE_GET(unit) | SOC_ST_F_EASY_RELOAD); \
    } else {                    \
        SOC_STATE_SET(unit, SOC_STATE_GET(unit) & ~SOC_ST_F_EASY_RELOAD);           \
    }
...
#define SOC_IS_RELOADING(unit)    (soc_feature(unit, soc_feature_easy_reload_wb_compat) &&  \
                                    SOC_STATE_GET(unit) & SOC_ST_F_EASY_RELOAD)
#define SOC_RELOAD_MODE_SET(unit, reload_mode)                                              \
...
/* Reload mode set/get macros */
#define SOC_IS_DETACHING(_unit_)    (SOC_STATE_GET(_unit_) & SOC_ST_F_DETACH)
#define SOC_DETACH(_unit_, _detach_) \
    if (_detach_) {              \
        SOC_STATE_SET(unit,SOC_STATE_GET(_unit_) | SOC_ST_F_DETACH); \
    } else {                    \
        SOC_STATE_SET(unit,SOC_STATE_GET(_unit_) & ~SOC_ST_F_DETACH);            \
    }
...
#if (SRC_SOC_COMMON_ACTIVATE_UNUSED) || defined(BCM_PETRA_SUPPORT) || defined(BCM_JERICHO_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_DNXF_SUPPORT) || defined(INCLUDE_INTR)
/* { */
...
#if (SRC_SOC_COMMON_ACTIVATE_UNUSED) || defined(BCM_FIREBOLT_SUPPORT) || defined(BCM_BRADLEY_SUPPORT) || defined(BCM_ESW_SUPPORT)
/* { */
extern void soc_xport_type_mode_update(int unit, soc_port_t port, int to_hg_port);
/* } */
#endif
#if defined(BCM_ESW_SUPPORT) || defined(BCM_BIGMAC_SUPPORT) || defined(BCM_FIREBOLT_SUPPORT) ||  \
    defined(BCM_BRADLEY_SUPPORT) || defined(BCM_CMAC_SUPPORT) || defined(BCM_CLMAC_SUPPORT) ||   \
    defined(BCM_XLMAC_SUPPORT) || defined(BCM_XMAC_SUPPORT)
/* { */
...
#define SOC_SUB_DEV_TYPE       SAL_SUB_DEV_TYPE       /* A sub-device (with a bus) of a composite device */
...
extern void soc_ser_stat_update(int unit, uint32 flags, int blk_type,
                                ...
                                soc_ser_correction_type_t ctype,
                                soc_stat_t *stat);
extern int soc_add_log_generic(int unit, _soc_ser_correct_info_t *spci);
...
extern int soc_pcie_hot_swap_disable(int unit, int mode);
...
/* bit 19 of CMC IRQ STAT2 for CMICM */
#define PCI_COMPLETION_TIMEOUT_ERROR       0x00080000
/* Default value of PCI completion timeout */
#define PCI_COMPLETION_WAIT_TIMEOUT_USEC   50000
extern int
