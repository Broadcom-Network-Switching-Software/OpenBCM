...
static err_code_t _blackhawk7_v1l8p1_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section);
static err_code_t _blackhawk7_v1l8p1_reg_print_with_buf(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index);
static err_code_t _blackhawk7_v1l8p1_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index);
...
static err_code_t _blackhawk7_v1l8p1_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section) {
    ...
    for (addr = blackhawk7_v1l8p1_reg_dump_arr[reg_section][0]; addr < blackhawk7_v1l8p1_reg_dump_arr[reg_section][1]; addr++) {
        if (!(addr & 0x000F)) {
            EFUN_PRINTF(("\n%04x ",addr));
        }
        EFUN(blackhawk7_v1l8p1_acc_rdt_reg(sa__, addr,&rddata));
...
static err_code_t _blackhawk7_v1l8p1_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index) {
    INIT_SRDS_ERR_CODE
    if(reg_buffer == NULL) {
         EFUN(_blackhawk7_v1l8p1_reg_print_no_buf(sa__, reg_section));
    }
    else {
        EFUN(_blackhawk7_v1l8p1_reg_print_with_buf(sa__, reg_section, reg_buffer, buf_index));
    }
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_reg_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char reg_buffer[][SRDS_DUMP_BUF_SIZE]) {
    ...
    while(reg_section < BLACKHAWK7_V1L8P1_REG_DUMP_SECTIONS) {
        if(blackhawk7_v1l8p1_reg_dump_arr[reg_section][0] == BLACKHAWK7_V1L8P1_REG_PLL_START) {
            ...
            for (pll_idx = 0; pll_idx < NUM_PLLS; pll_idx++) {
                ...
                while( reg_section < BLACKHAWK7_V1L8P1_REG_DUMP_SECTIONS && blackhawk7_v1l8p1_reg_dump_arr[reg_section][0] != BLACKHAWK7_V1L8P1_REG_UC_START) {
                    EFUN(_blackhawk7_v1l8p1_reg_print(sa__, reg_section, reg_buffer, &buf_index));
                    reg_section++;
                }
            }
            EFUN(blackhawk7_v1l8p1_acc_set_pll_idx(sa__,pll_orig));
        }
        else if (blackhawk7_v1l8p1_reg_dump_arr[reg_section][0] == BLACKHAWK7_V1L8P1_REG_UC_START) {
...
err_code_t blackhawk7_v1l8p1_uc_core_var_dump(srds_access_t *sa__, uint8_t core, char ram_buffer[][SRDS_DUMP_BUF_SIZE]) {
  ...
  core_var_ram_size = (uint8_t)blackhawk7_v1l8p1_info_ptr->core_var_ram_size;
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < core_var_ram_size; addr++) {
          ...
          ESTM_PRINTF(("%02x ", blackhawk7_v1l8p1_rdbc_uc_var(sa__, __ERR, addr)));
      }
  }
  else {
      uint8_t index=0;
      ...
      for (addr = 0x0; addr < core_var_ram_size; addr++) {
          ...
          ESTM(rddata = blackhawk7_v1l8p1_rdbc_uc_var(sa__, __ERR, addr));
...
err_code_t blackhawk7_v1l8p1_uc_micro_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][SRDS_DUMP_BUF_SIZE]) {
  ...
  srds_info_t * blackhawk7_v1l8p1_info_ptr = blackhawk7_v1l8p1_INTERNAL_get_blackhawk7_v1l8p1_info_ptr_with_check(sa__);
  EFUN(blackhawk7_v1l8p1_INTERNAL_match_ucode_from_info(sa__, blackhawk7_v1l8p1_info_ptr));
  micro_var_ram_size = (uint8_t)blackhawk7_v1l8p1_info_ptr->micro_var_ram_size;
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < micro_var_ram_size; addr++) {
         ...
         ESTM_PRINTF(("%02x ", blackhawk7_v1l8p1_rdbuc_uc_var(sa__, __ERR, addr)));
      }
  }
  else {
      ...
      for (addr = 0x0; addr < micro_var_ram_size; addr++) {
         ...
         ESTM(rddata = blackhawk7_v1l8p1_rdbuc_uc_var(sa__, __ERR, addr));
         count += USR_SNPRINTF(ram_buffer[index] + count, (size_t)(SRDS_DUMP_BUF_SIZE - count - 1), "%02x ", rddata);
     }
     ram_buffer[++index][0] = 0xA;
     ram_buffer[++index][0] = 0;
...
err_code_t blackhawk7_v1l8p1_uc_lane_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][SRDS_DUMP_BUF_SIZE]) {
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < lane_var_ram_size; addr++) {
         ...
         ESTM_PRINTF(("%02x ", blackhawk7_v1l8p1_rdbl_uc_var(sa__, __ERR, addr)));
      }
  }
  else {
      uint8_t rddata, index = 0;
      ...
      for (addr = 0x0; addr < lane_var_ram_size; addr++) {
         ...
         ESTM(rddata = blackhawk7_v1l8p1_rdbl_uc_var(sa__, __ERR, addr));
         count += USR_SNPRINTF(ram_buffer[index] + count,  (size_t)(SRDS_DUMP_BUF_SIZE - count - 1), "%02x ", rddata);
      }
      ram_buffer[++index][0] = 0xA;
      ram_buffer[++index][0] = 0;
...
err_code_t blackhawk7_v1l8p1_isolate_lane_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    ...
    blackhawk7_v1l8p1_isolate_lane_ctrl_tx_pins(sa__, enable);
    blackhawk7_v1l8p1_isolate_lane_ctrl_rx_pins(sa__, enable);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_log_full_pmd_state_noPRBS (srds_access_t *sa__, struct blackhawk7_v1l8p1_detailed_lane_status_st *lane_st) {
...

  {
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_get_lms_thresh_bin(sa__, &tmp)); lane_st->lms_thresh = tmp;
...
err_code_t blackhawk7_v1l8p1_log_full_pmd_state (srds_access_t *sa__, struct blackhawk7_v1l8p1_detailed_lane_status_st *lane_st) {
    ...
    EFUN(blackhawk7_v1l8p1_prbs_err_count_ll(sa__, &lane_st->prbs_chk_errcnt));
    if (lane_st->prbs_chk_lock && (lane_st->rx_pam_mode != NRZ)) {
       EFUN(blackhawk7_v1l8p1_collect_dbgfb_stats(sa__, 8000, &lane_st->dbgfb_stats));
    }
    EFUN(blackhawk7_v1l8p1_INTERNAL_get_BER_string(sa__,100,&lane_st->ber_string[0], MEMBER_SIZE(struct blackhawk7_v1l8p1_detailed_lane_status_st, ber_string)));
...
void blackhawk7_v1l8p1_INTERNAL_print_triage_info(srds_access_t *sa__, err_code_t err_code, uint8_t print_header, uint8_t print_data, uint16_t line)
{
    ...
    if (blackhawk7_v1l8p1_INTERNAL_pmd_lock_status(sa__, &info.pmd_lock, &info.pmd_lock_chg)) {
        error_seen = 1;
    }
