...
/*---------------------------------------*/
enum falcon16_tsc_prbs_chk_timer_selection_enum {
    USE_HW_TIMERS = 0,
    USE_SW_TIMERS = 1
};
...
/** Checks to see if the stored ucode_version in falcon16_tsc_info is matching with the current thread ucode
 * @param sa__ is an opaque state vector passed through to device access functions.
 ...
 * @param *falcon16_tsc_info_ptr Info_table to be verified
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @return Error code, if generated (returns ERR_CODE_NONE if no errors)
 */
 ...
 * @param *srds_div Divider value (enumerated), or FALCON16_TSC_PLL_DIV_UNKNOWN to auto-compute.
 ...
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors)
 */
err_code_t falcon16_tsc_INTERNAL_get_vco_from_refclk_div(srds_access_t *sa__,
                                                   uint32_t refclk_freq_hz,
                                                   enum falcon16_tsc_pll_div_enum srds_div,
                                                   uint32_t *vco_freq_khz,
                                                   enum falcon16_tsc_pll_option_enum pll_option);
