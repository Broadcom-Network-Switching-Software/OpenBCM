...
#undef SERDES_EXTERNAL_INFO_TABLE_EN

#if !defined(SERDES_EXTERNAL_INFO_TABLE_EN)
static srds_info_t falcon16_tsc_info[NUM_SERDES_INFO_TABLES];
#endif


srds_info_t *falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr(srds_access_t *sa__) {    
#if defined(SERDES_EXTERNAL_INFO_TABLE_EN)
    return falcon16_tsc_get_info_table_address(sa__);
#else
    uint8_t index = 0;
    index = 0;
    return &falcon16_tsc_info[index];
#endif
}

srds_info_t *falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr_with_check(srds_access_t *sa__) {
    err_code_t err_code = ERR_CODE_NONE;
    srds_info_t * falcon16_tsc_info_ptr = falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr(sa__);
    CHECK_AND_RETURN_IF_NULL(falcon16_tsc_info_ptr, 0);
    if (falcon16_tsc_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        err_code = falcon16_tsc_init_falcon16_tsc_info(sa__);
        if (err_code != ERR_CODE_NONE){
            EFUN_PRINTF(("ERROR:  %s Failed: 0x%04X \n", API_FUNCTION_NAME, err_code));
            return 0;
        }
    } 
    return falcon16_tsc_info_ptr;
}

err_code_t falcon16_tsc_INTERNAL_match_ucode_from_info(srds_access_t *sa__) {
    uint16_t ucode_version_major;
    uint8_t ucode_version_minor;
    uint32_t ucode_version;

    srds_info_t * falcon16_tsc_info_ptr = falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr(sa__);
    CHECK_AND_RETURN_IF_NULL(falcon16_tsc_info_ptr, ERR_CODE_INVALID_INFO_TABLE_ADDR);
    ...
    if (ucode_version == falcon16_tsc_info_ptr->ucode_version) {
        return(ERR_CODE_NONE);
    } else {
        EFUN_PRINTF(("ERROR:  ucode version of the current thread not matching with stored falcon16_tsc_info->ucode_version, Expected 0x%08X, but received 0x%08X.\n",
                    falcon16_tsc_info_ptr->ucode_version, ucode_version));
        return(ERR_CODE_UCODE_VERIFY_FAIL);
    }
}

err_code_t falcon16_tsc_INTERNAL_verify_falcon16_tsc_info(srds_info_t *falcon16_tsc_info_ptr, srds_access_t *sa__)
{
    err_code_t err_code = ERR_CODE_NONE;
    ...
    if (falcon16_tsc_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        EFUN_PRINTF(("ERROR:  Mismatch in falcon16_tsc_info signature.  Expected 0x%08X, but received 0x%08X.\n",
                     SRDS_INFO_SIGNATURE, falcon16_tsc_info_ptr->signature));
        return (falcon16_tsc_error(sa__, ERR_CODE_INFO_TABLE_ERROR));
    }
    err_code = falcon16_tsc_INTERNAL_match_ucode_from_info(sa__);
    if (err_code != ERR_CODE_NONE) {
...
int falcon16_tsc_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    switch(osr_mode) {
    case FALCON16_TSC_OSX1:      return 1000;
    case FALCON16_TSC_OSX2:      return 2000;
    case FALCON16_TSC_OSX4:      return 4000;
    case FALCON16_TSC_OSX16P5:   return 16500;
    case FALCON16_TSC_OSX20P625: return 20625;
#ifdef wr_ams_rx_rxclk_div2p5
    case FALCON16_TSC_OSX2P5:    return 2500;
#endif
    default:                    return 1000;
    }
}

err_code_t falcon16_tsc_INTERNAL_get_num_bits_per_ms(srds_access_t *sa__, uint32_t *num_bits_per_ms) {
    uint8_t osr_mode = 0;
#if defined(rd_rx_pam4_mode)
    uint8_t pam4_mode = 0;
#endif
    struct falcon16_tsc_uc_core_config_st core_config = {{0}};

    { 
       falcon16_tsc_osr_mode_st osr_mode_st;
       ENULL_MEMSET(&osr_mode_st, 0, sizeof(falcon16_tsc_osr_mode_st));
       EFUN(falcon16_tsc_INTERNAL_get_osr_mode(sa__, &osr_mode_st));
    ...
    ESTM(pam4_mode = rd_rx_pam4_mode());
        ...
        *num_bits_per_ms = (((uint32_t)core_config.vco_rate_in_Mhz * 100000UL) / falcon16_tsc_osr_mode_enum_to_int_x1000(osr_mode))*10;
...
err_code_t falcon16_tsc_INTERNAL_display_BER(srds_access_t *sa__, uint16_t time_ms) {
    char string[10];
      EFUN(falcon16_tsc_INTERNAL_get_BER_string(sa__,time_ms,string));
      USR_PRINTF(("%s",string));
    return(ERR_CODE_NONE);
}

err_code_t falcon16_tsc_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string) {
    char string2[3];
    ...
    EFUN(falcon16_tsc_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local, USE_HW_TIMERS));
...
err_code_t falcon16_tsc_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, enum falcon16_tsc_prbs_chk_timer_selection_enum timer_sel) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
#if defined(FALCON16_TSC_PRBS_CHK_HARDWARE_TIMERS)
    ...
    if((time_ms <= PRBS_MAX_HW_TIMER_TIMEOUT) && (timer_sel == USE_HW_TIMERS)) {
        ...
        EFUN(falcon16_tsc_get_prbs_chk_hw_timer_ctrl(sa__, &prbs_chk_hw_timer_ctrl_bak));
        
        /* STEP 4: Configure hardware timer to count prbs errors */
        EFUN(falcon16_tsc_config_prbs_chk_hw_timer(sa__, time_ms, &time_ms_adjusted));
        
        /* STEP 5: Clear prbs error counters and lock lost status */
        EFUN(falcon16_tsc_prbs_err_count_state(sa__, &err_cnt,&lcklost)); /* clear error counters */
        
        /* STEP 6: toggle checker enable to start hardware timers and error counters */
        EFUN(falcon16_tsc_prbs_chk_en_toggle(sa__));
        ...
        if(!prbs_chk_lock) {
            ...
            EFUN(falcon16_tsc_prbs_err_count_state(sa__, &err_cnt,&lcklost));
            
            /* STEP 11: Update the BER data structure */
            EFUN(falcon16_tsc_INTERNAL_get_num_bits_per_ms(sa__,&num_bits_per_ms));
        ...
        EFUN(falcon16_tsc_set_prbs_chk_hw_timer_ctrl(sa__, &prbs_chk_hw_timer_ctrl_bak));
...
#if defined(FALCON16_TSC_PRBS_CHK_HARDWARE_TIMERS)
err_code_t falcon16_tsc_INTERNAL_get_prbs_timeout_count_from_time(uint16_t time_ms, uint16_t * time_ms_adjusted, struct prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl ) {
...
err_code_t falcon16_tsc_INTERNAL_compute_bin(srds_access_t *sa__, char var, char bin[]) {
    if(!bin) {
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (falcon16_tsc_error(sa__, ERR_CODE_CFG_PATT_INVALID_HEX));
  }
  return (ERR_CODE_NONE);
}


err_code_t falcon16_tsc_INTERNAL_compute_hex(srds_access_t *sa__, char bin[],uint8_t *hex) {
  if(!hex) {
    return(falcon16_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
  ...
  if (!USR_STRCMP(bin,"0000")) {
    ...
    *hex = 0x0;
    return (falcon16_tsc_error(sa__, ERR_CODE_CFG_PATT_INVALID_BIN2HEX));
  } 
  return (ERR_CODE_NONE);
}

uint8_t falcon16_tsc_INTERNAL_stop_micro(srds_access_t *sa__, uint8_t graceful, err_code_t *err_code_p) {
   ...
   if (!(stop_state & 0x7F)) {
       if (graceful) {
           EPFUN2((falcon16_tsc_stop_rx_adaptation(sa__, 1)),0xFF);
       }
       else {
           EPFUN2((falcon16_tsc_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_IMMEDIATE,GRACEFUL_STOP_TIME)),0xFF);
       }
   }
...
err_code_t falcon16_tsc_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
    ...
    srds_info_t * falcon16_tsc_info_ptr = falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr_with_check(sa__);
    CHECK_AND_RETURN_IF_NULL(falcon16_tsc_info_ptr, ERR_CODE_INVALID_INFO_TABLE_ADDR);

    EFUN(falcon16_tsc_INTERNAL_verify_falcon16_tsc_info(falcon16_tsc_info_ptr, sa__));

    if (micro_num >= falcon16_tsc_info_ptr->micro_count) {
        return (falcon16_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    /* Read Current lane so that it can be restored at the end of function */
    current_lane = falcon16_tsc_get_lane(sa__);
    EFUN_PRINTF(("\n\n********************************************\n"));
    ...
    if (bypass_micro) {
        ...
        EFUN(falcon16_tsc_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, GRACEFUL_STOP_TIME));
    ...
    if (falcon16_tsc_info_ptr->trace_memory_descending_writes) {
        ...
        EFUN(falcon16_tsc_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                  falcon16_tsc_info_ptr->trace_mem_ram_base + falcon16_tsc_info_ptr->grp_ram_size*micro_num,
                                                  falcon16_tsc_info_ptr->trace_mem_ram_size,
                                                  rd_idx,
                                                  falcon16_tsc_info_ptr->trace_mem_ram_size,
                                                  arg,
    ...
    if (!bypass_micro) {
        ...
        EFUN(falcon16_tsc_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 50));
    }
    EFUN(falcon16_tsc_set_lane(sa__,current_lane));
    return(ERR_CODE_NONE);
}
...
uint32_t falcon16_tsc_INTERNAL_float12_to_uint32(uint8_t input, uint8_t multi) {
...
err_code_t falcon16_tsc_INTERNAL_set_rx_pf_main(srds_access_t *sa__, uint8_t val) {
    if (val > 15) 
    {
      return (falcon16_tsc_error(sa__, ERR_CODE_PF_INVALID));
    }
    EFUN(WR_RX_PF_CTRL(val));
    return(ERR_CODE_NONE); 
}

err_code_t falcon16_tsc_INTERNAL_get_rx_pf_main(srds_access_t *sa__, uint8_t *val) {
...
err_code_t falcon16_tsc_INTERNAL_get_rx_pf2(srds_access_t *sa__, uint8_t *val) {
...
err_code_t falcon16_tsc_INTERNAL_set_rx_vga(srds_access_t *sa__, uint8_t val) {
    ...
    if (val > RX_VGA_CTRL_VAL_MAX) {
      return (falcon16_tsc_error(sa__, ERR_CODE_VGA_INVALID));
    }
...
err_code_t falcon16_tsc_INTERNAL_get_rx_vga(srds_access_t *sa__, uint8_t *val) {
...
err_code_t falcon16_tsc_INTERNAL_set_rx_dfe2(srds_access_t *sa__, int8_t val) {
    if ((val > 15) || (val < -15))
      return (falcon16_tsc_error(sa__, ERR_CODE_DFE_TAP));  
...
err_code_t falcon16_tsc_INTERNAL_get_rx_dfe2(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = ((rd_rxa_dfe_tap2() + rd_rxb_dfe_tap2() + rd_rxc_dfe_tap2() + rd_rxd_dfe_tap2())>>2));
    return (ERR_CODE_NONE);
}
...
err_code_t falcon16_tsc_INTERNAL_set_tx_pre(srds_access_t *sa__, uint8_t val) {
  if (val > 31) {
    return (falcon16_tsc_error(sa__, ERR_CODE_TXFIR_PRE_INVALID));  
  }
...
err_code_t falcon16_tsc_INTERNAL_set_tx_main(srds_access_t *sa__, uint8_t val) {
  if (val > 127) {
    return (falcon16_tsc_error(sa__, ERR_CODE_TXFIR_MAIN_INVALID));  
  }
...
err_code_t falcon16_tsc_INTERNAL_set_tx_post1(srds_access_t *sa__, int8_t val) {
  ...
  if ((val > 63) || (val < -63)) {
    return (falcon16_tsc_error(sa__, ERR_CODE_TXFIR_POST1_INVALID));  
  }
...
err_code_t falcon16_tsc_INTERNAL_set_tx_post2(srds_access_t *sa__, int8_t val) {
  if ((val > 15) || (val < -15)) {
    return (falcon16_tsc_error(sa__, ERR_CODE_TXFIR_POST2_INVALID));  
  }
...
err_code_t falcon16_tsc_INTERNAL_set_tx_post3(srds_access_t *sa__, int8_t val) {
  if ((val > 7) || (val < -7)) {
    return (falcon16_tsc_error(sa__, ERR_CODE_TXFIR_POST3_INVALID));  
  }
...
/*-----------------------------------------------*/
err_code_t falcon16_tsc_INTERNAL_get_eye_margin_est(srds_access_t *sa__, uint16_t *left_eye_mUI, uint16_t *right_eye_mUI, uint16_t  *upper_eye_mV, uint16_t *lower_eye_mV) {
  uint8_t ladder_range = 0;
  ESTM(*left_eye_mUI  = falcon16_tsc_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_left()));
  ESTM(*right_eye_mUI = falcon16_tsc_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_right()));
  ESTM(*upper_eye_mV = falcon16_tsc_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_upper(), ladder_range));
  ESTM(*lower_eye_mV = falcon16_tsc_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_lower(), ladder_range));
...
err_code_t falcon16_tsc_INTERNAL_get_osr_mode(srds_access_t *sa__, falcon16_tsc_osr_mode_st *imode) {
...
#ifdef wr_ams_rx_rxclk_div2p5    /* Analog 2p5 OSR mode */
  {
      ...
      if ((mode.tx == FALCON16_TSC_OSX1) && (osr_2p5 == 1)  && (osr_2p5_pwrup == 1)) {
          mode.tx = FALCON16_TSC_OSX2P5;
      }
      ESTM(osr_2p5       = rd_ams_rx_rxclk_div2p5());
      ESTM(osr_2p5_pwrup = rd_ams_rx_rxclk_div2p5_pwrup());
      if ((mode.rx == FALCON16_TSC_OSX1) && (osr_2p5 == 1)  && (osr_2p5_pwrup == 1)) {
          mode.rx = FALCON16_TSC_OSX2P5;
      }
  }
#endif
  *imode = mode;
...
#else
#error "Unknown core for falcon16_tsc_INTERNAL_pmd_lock_status()"    
#endif
    return(ERR_CODE_NONE);
}
...
err_code_t falcon16_tsc_INTERNAL_read_lane_state(srds_access_t *sa__, falcon16_tsc_lane_state_st *istate) {
  ...
  EFUN(falcon16_tsc_INTERNAL_pmd_lock_status(sa__,&state.rx_lock, &state.rx_lock_chg));
  {
      ...
      state.stop_state = falcon16_tsc_INTERNAL_stop_micro(sa__,state.rx_lock,&err_code);
  ...
  EFUN(falcon16_tsc_read_rx_afe(sa__, RX_AFE_PF, &state.pf_main));
  state.pf_hiz = 0;
  EFUN(falcon16_tsc_read_rx_afe(sa__, RX_AFE_PF2, &state.pf2_ctrl));
  EFUN(falcon16_tsc_read_rx_afe(sa__, RX_AFE_VGA, &state.vga));
  ...
  EFUN(falcon16_tsc_INTERNAL_get_eye_margin_est(sa__, &state.heye_left, &state.heye_right, &state.veye_upper, &state.veye_lower));
  ...
  ESTM(state.link_time = (((uint32_t)rdv_usr_sts_link_time())*8)/10);     /* convert from 80us to 0.1 ms units */
  {
      ...
      EFUN(falcon16_tsc_INTERNAL_pmd_lock_status(sa__,&rx_lock_at_end, &rx_lock_chg_at_end));
  ...
  if (!state.stop_state || (state.stop_state == 0xFF)) {
      /* manually check error code instead of EFUN*/
      {
          ...
          resume_status = falcon16_tsc_stop_rx_adaptation(sa__, 0);
          if (resume_status) {
...
err_code_t falcon16_tsc_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {     
  ...
  EFUN(falcon16_tsc_INTERNAL_display_BER(sa__,100));
...
err_code_t falcon16_tsc_INTERNAL_read_core_state(srds_access_t *sa__, falcon16_tsc_core_state_st *istate) {  
  ...
  EFUN(falcon16_tsc_version(sa__, &state.api_version));
  ...
  EFUN(falcon16_tsc_INTERNAL_measure_pll_comp_thresh(sa__, &state.pll_comp_thresh));
...
err_code_t falcon16_tsc_INTERNAL_measure_pll_comp_thresh(srds_access_t *sa__, int16_t *thresh) {
...
err_code_t falcon16_tsc_INTERNAL_update_uc_core_config_st(struct falcon16_tsc_uc_core_config_st *st) {
  ...
  st->field.osr_2p5_en = in & BFMASK(1); in >>= 1;
...
err_code_t falcon16_tsc_INTERNAL_update_uc_core_config_word(struct falcon16_tsc_uc_core_config_st *st) {
  ...
  in <<= 1; in |= st->field.osr_2p5_en & BFMASK(1);
...
err_code_t falcon16_tsc_INTERNAL_check_uc_lane_stopped(srds_access_t *sa__) {
  ...
  if (!is_micro_stopped) {
      return(falcon16_tsc_error(sa__, ERR_CODE_UC_NOT_STOPPED));
  } else {
      return(ERR_CODE_NONE);
  }
}

err_code_t falcon16_tsc_INTERNAL_calc_patt_gen_mode_sel(srds_access_t *sa__, uint8_t *mode_sel, uint8_t *zero_pad_len, uint8_t patt_length) {
  ...
  if (!(140 % patt_length)) {
    ...
    *zero_pad_len = 0;
    ...
    return (falcon16_tsc_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATT_LENGTH));
  }
  return(ERR_CODE_NONE);
}
...
err_code_t _falcon16_tsc_get_refclk_in_hz(srds_access_t *sa__, enum falcon16_tsc_pll_refclk_enum refclk, uint32_t *refclk_in_hz) {
    switch (refclk) {
        ...
        default:
            ...
            *refclk_in_hz = 0xFFFFFFFF;
            return (falcon16_tsc_error(sa__, ERR_CODE_REFCLK_FREQUENCY_INVALID));
    }
    return (ERR_CODE_NONE);
}
 ...
 */
static err_code_t _falcon16_tsc_get_divisor_ratio(srds_access_t *sa__, enum falcon16_tsc_pll_div_enum srds_div, uint16_t *numerator, uint16_t *denominator)
{
    switch (srds_div) {
        case FALCON16_TSC_PLL_DIV_64:         *denominator =   8;  *numerator =  125; break;
        case FALCON16_TSC_PLL_DIV_66:         *denominator =  33;  *numerator =  500; break;
        ...
        case FALCON16_TSC_PLL_DIV_330:        *denominator =  33;  *numerator =  100; break;
        case FALCON16_TSC_PLL_DIV_350:        *denominator =   7;  *numerator =   20; break;
        default:
...
static err_code_t _falcon16_tsc_get_refclk_from_div_vco(srds_access_t *sa__, uint32_t *refclk_freq_hz, enum falcon16_tsc_pll_div_enum srds_div, uint32_t vco_freq_khz, enum falcon16_tsc_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_falcon16_tsc_get_divisor_ratio(sa__, srds_div, &numerator, &denominator));
    *refclk_freq_hz = ((vco_freq_khz+(denominator>>1)) / denominator) * numerator;
    if (pll_option == FALCON16_TSC_PLL_OPTION_REFCLK_DOUBLER_EN) *refclk_freq_hz /= 2;
    if (pll_option == FALCON16_TSC_PLL_OPTION_REFCLK_DIV2_EN)    *refclk_freq_hz *= 2;
    if (pll_option == FALCON16_TSC_PLL_OPTION_REFCLK_DIV4_EN)    *refclk_freq_hz *= 4;
    return (ERR_CODE_NONE);
}

/** Get the VCO frequency in kHz, based on the reference clock frequency and falcon16_tsc_pll_div_enum value. */
err_code_t falcon16_tsc_INTERNAL_get_vco_from_refclk_div(srds_access_t *sa__, uint32_t refclk_freq_hz, enum falcon16_tsc_pll_div_enum srds_div, uint32_t *vco_freq_khz, enum falcon16_tsc_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_falcon16_tsc_get_divisor_ratio(sa__, srds_div, &numerator, &denominator));
    if (pll_option == FALCON16_TSC_PLL_OPTION_REFCLK_DOUBLER_EN) refclk_freq_hz *= 2;
    if (pll_option == FALCON16_TSC_PLL_OPTION_REFCLK_DIV2_EN)    refclk_freq_hz /= 2;
...
static const enum falcon16_tsc_pll_div_enum _falcon16_tsc_div_candidates[] = {
    FALCON16_TSC_PLL_DIV_64,
    FALCON16_TSC_PLL_DIV_66,
    ...
    FALCON16_TSC_PLL_DIV_330,
    FALCON16_TSC_PLL_DIV_350
};
...
static err_code_t _falcon16_tsc_check_div(srds_access_t *sa__, enum falcon16_tsc_pll_div_enum srds_div) {
    uint8_t i, found = 0;
    for (i=0; i<_falcon16_tsc_div_candidates_count; i++) {
        found |= srds_div == _falcon16_tsc_div_candidates[i];
    }
    if (!found) {
...
static err_code_t _falcon16_tsc_check_vco_freq_khz(srds_access_t *sa__, uint32_t vco_freq_khz) {
    if (vco_freq_khz < SERDES_VCO_FREQ_KHZ_MIN - SERDES_VCO_FREQ_KHZ_TOLERANCE) {
        ...
        return (falcon16_tsc_error(sa__, ERR_CODE_VCO_FREQUENCY_INVALID));
    }
...
static err_code_t _falcon16_tsc_get_div(srds_access_t *sa__, uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum falcon16_tsc_pll_div_enum *srds_div, enum falcon16_tsc_pll_option_enum pll_option) {
    int32_t vco_freq_khz_min_error = 0x7FFFFFFF;
    uint8_t i;
    for (i=0; i<_falcon16_tsc_div_candidates_count; i++) {
        uint32_t actual_vco_freq_khz = 0;
        int32_t  vco_freq_khz_error;
        EFUN(falcon16_tsc_INTERNAL_get_vco_from_refclk_div(sa__, refclk_freq_hz, _falcon16_tsc_div_candidates[i], &actual_vco_freq_khz, pll_option));
        ...
        if (vco_freq_khz_min_error > vco_freq_khz_error) {
            ...
            *srds_div = _falcon16_tsc_div_candidates[i];
        }
    }
    if (vco_freq_khz_min_error == 0xFFFFFFFF) {
      return (falcon16_tsc_error(sa__, ERR_CODE_CONFLICTING_PARAMETERS));
    }
    return (ERR_CODE_NONE);
}

err_code_t falcon16_tsc_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    const uint8_t given_param_count = (((refclk == FALCON16_TSC_PLL_REFCLK_UNKNOWN) ? 0 : 1)
                                       + ((*srds_div == FALCON16_TSC_PLL_DIV_UNKNOWN) ? 0 : 1)
                                       + ((*vco_freq_khz == 0) ? 0 : 1));
    const uint32_t original_vco_freq_khz = *vco_freq_khz;
    enum falcon16_tsc_pll_div_enum auto_div = FALCON16_TSC_PLL_DIV_UNKNOWN;
    ...
    if (given_param_count < 2) {
        return (falcon16_tsc_error(sa__, ERR_CODE_INSUFFICIENT_PARAMETERS));
    }

    /* Skip verification if option is selected. Error if all parameters not given. */
    if ((pll_option & FALCON16_TSC_PLL_OPTION_DISABLE_VERIFY) == FALCON16_TSC_PLL_OPTION_DISABLE_VERIFY) {
        if (given_param_count < 3) {
            return (falcon16_tsc_error(sa__, ERR_CODE_INSUFFICIENT_PARAMETERS));
        } else {
            EFUN(_falcon16_tsc_get_refclk_in_hz(sa__, refclk, refclk_freq_hz));
            return(ERR_CODE_NONE);
        }
    }
    ...
    if (*srds_div != FALCON16_TSC_PLL_DIV_UNKNOWN) {
        EFUN(_falcon16_tsc_check_div(sa__, *srds_div));
    }

    /* Verify that the requested VCO frequency is allowed. */
    if (*vco_freq_khz != 0) {
        EFUN(_falcon16_tsc_check_vco_freq_khz(sa__, *vco_freq_khz));
    }

    if (refclk == FALCON16_TSC_PLL_REFCLK_UNKNOWN) {
        /* Determine refclk from vco frequency and div */
        EFUN(_falcon16_tsc_get_refclk_from_div_vco(sa__, refclk_freq_hz, *srds_div, *vco_freq_khz, pll_option));
    } else {
        EFUN(_falcon16_tsc_get_refclk_in_hz(sa__, refclk, refclk_freq_hz));
    }
    ...
    if (*vco_freq_khz == 0) {
        ...
        EFUN(falcon16_tsc_INTERNAL_get_vco_from_refclk_div(sa__, *refclk_freq_hz, *srds_div, vco_freq_khz, pll_option));
    }
     ...
     */
    EFUN(_falcon16_tsc_get_div(sa__, *refclk_freq_hz, *vco_freq_khz, &auto_div, pll_option));
    if (*srds_div == FALCON16_TSC_PLL_DIV_UNKNOWN) {
...
err_code_t falcon16_tsc_INTERNAL_display_pll_to_divider(srds_access_t *sa__, uint32_t srds_div) {
...
err_code_t falcon16_tsc_INTERNAL_poll_diag_done(srds_access_t *sa__, uint16_t *status, uint32_t timeout_ms) {
 ...
 return(falcon16_tsc_error(sa__, ERR_CODE_DIAG_TIMEOUT));
}
...
err_code_t falcon16_tsc_INTERNAL_poll_uc_dsc_ready_for_cmd_equals_1(srds_access_t *sa__, uint32_t timeout_ms, enum srds_pmd_uc_cmd_enum cmd) {
    ...
    for (loop = 0; loop < 100; loop++) {
        ...
        if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
            if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
                ...
                return(falcon16_tsc_error(sa__, ERR_CODE_UC_CMD_RETURN_ERROR));
            }
            return (ERR_CODE_NONE);
        }
        if(loop>10) {
    ...
    }
    {
        ...
        falcon16_tsc_INTERNAL_print_triage_info(sa__, ERR_CODE_UC_CMD_POLLING_TIMEOUT, 1, 1, __LINE__);
...
err_code_t falcon16_tsc_INTERNAL_poll_dsc_state_equals_uc_tune(srds_access_t *sa__, uint32_t timeout_ms) {
  ...
  return (falcon16_tsc_error(sa__, ERR_CODE_POLLING_TIMEOUT));          /* Error Code for polling timeout */
}
...
uint8_t falcon16_tsc_INTERNAL_is_big_endian(void)
{
...
int falcon16_tsc_INTERNAL_get_endian_offset(uint8_t *addr) {
...
err_code_t falcon16_tsc_INTERNAL_rdblk_callback(void *arg, uint8_t byte_count, uint16_t data) {
    ...
    ESTM(endian_offset = falcon16_tsc_INTERNAL_get_endian_offset(cast_arg->mem_ptr));
...
uint8_t falcon16_tsc_INTERNAL_grp_idx_from_lane(uint8_t lane) {
    return(0);
}


err_code_t falcon16_tsc_INTERNAL_display_info_table (srds_access_t *sa__) {
    srds_info_t * const falcon16_tsc_info_ptr = falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr_with_check(sa__);
    EFUN(falcon16_tsc_INTERNAL_verify_falcon16_tsc_info(falcon16_tsc_info_ptr, sa__));

    EFUN_PRINTF(("\n********** SERDES INFO TABLE DUMP **********\n"));
    EFUN_PRINTF(("signature                 = 0x%X\n", falcon16_tsc_info_ptr->signature));
    EFUN_PRINTF(("diag_mem_ram_base         = 0x%X\n", falcon16_tsc_info_ptr->diag_mem_ram_base));
    EFUN_PRINTF(("diag_mem_ram_size         = 0x%X\n", falcon16_tsc_info_ptr->diag_mem_ram_size));
    EFUN_PRINTF(("core_var_ram_base         = 0x%X\n", falcon16_tsc_info_ptr->core_var_ram_base));
    EFUN_PRINTF(("core_var_ram_size         = 0x%X\n", falcon16_tsc_info_ptr->core_var_ram_size));
    EFUN_PRINTF(("lane_var_ram_base         = 0x%X\n", falcon16_tsc_info_ptr->lane_var_ram_base));
    EFUN_PRINTF(("lane_var_ram_size         = 0x%X\n", falcon16_tsc_info_ptr->lane_var_ram_size));
    EFUN_PRINTF(("trace_mem_ram_base        = 0x%X\n", falcon16_tsc_info_ptr->trace_mem_ram_base));
    EFUN_PRINTF(("trace_mem_ram_size        = 0x%X\n", falcon16_tsc_info_ptr->trace_mem_ram_size));
    EFUN_PRINTF(("micro_var_ram_base        = 0x%X\n", falcon16_tsc_info_ptr->micro_var_ram_base));
    EFUN_PRINTF(("lane_count                = 0x%X\n", falcon16_tsc_info_ptr->lane_count));
    EFUN_PRINTF(("trace_memory_descending_writes = 0x%X\n", falcon16_tsc_info_ptr->trace_memory_descending_writes));
    EFUN_PRINTF(("micro_count               = 0x%X\n", falcon16_tsc_info_ptr->micro_count));
    EFUN_PRINTF(("micro_var_ram_size        = 0x%X\n", falcon16_tsc_info_ptr->micro_var_ram_size));
    EFUN_PRINTF(("grp_ram_size              = 0x%X\n", falcon16_tsc_info_ptr->grp_ram_size));
    EFUN_PRINTF(("ucode_version             = 0x%X\n", falcon16_tsc_info_ptr->ucode_version));
