 ...
 *  Revision      :  $Id: eagle2_tsc2pll_dv_functions.c 1565 2017-08-18 23:22:09Z kirand $ *
 *                                                                                    *
...
err_code_t eagle2_tsc2pll_core_pwrdn (const phymod_access_t *phy_pa, enum srds_core_pwrdn_mode_enum mode) {
    ...
    pa = &pa_copy;
    ENULL_MEMCPY(pa,phy_pa,sizeof(phymod_access_t));switch(mode) {
    case PWR_ON:
...
err_code_t eagle2_tsc2pll_configure_fast_link_recovery (const phymod_access_t *pa, uint8_t enable, uint8_t enable_timeout, enum eagle2_tsc2pll_flr_timeout_enum timeout_time, uint8_t enable_2ms_yield, uint8_t filter_count) {
    ...
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: eagle2_tsc2pll_configure_fast_link_recovery (pa, ..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",eagle2_tsc2pll_get_lane(),reset_state));
        return (_error(ERR_CODE_LANE_DP_NOT_RESET));
    }
#endif

    if (timeout_time_val > 3) {
        ESTM_PRINTF(("timeout_time value for eagle2_tsc2pll_configure_fast_link_recovery (pa, ) is too large:  %d\n", timeout_time_val));
        return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    if (filter_count > 7) {
        ESTM_PRINTF(("filter_count value for eagle2_tsc2pll_configure_fast_link_recovery (pa, ) is too large:  %d\n", filter_count));
        return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
