...
/* Comment out display of PRBS burst error if BLACKHAWK7 & 7 */
...
err_code_t blackhawk7_l8p2_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l8p2_ber_data_st *ber_data, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    INIT_SRDS_ERR_CODE
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    uint8_t clk_gate = 0;
...
err_code_t blackhawk7_l8p2_get_BER_data_HW_mode(srds_access_t *sa__, struct blackhawk7_l8p2_ber_data_st *ber_data, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    INIT_SRDS_ERR_CODE
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    if (prbs_chk_lock_state == PRBS_CHECKER_NOT_ENABLED) {
        ...
        if (iter == 0) {
            /* the following code checks at what point did the timers expire in relation to the events - (a) reading prbs_chk_lock_state and (b) reading prbs_chk_done_lh */
            uint8_t lock_state = rd_prbs_chk_lock_state();
