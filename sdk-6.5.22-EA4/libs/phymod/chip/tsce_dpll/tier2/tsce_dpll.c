...
int tsce_dpll_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_lane_config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (temod2pll_port_start_lane_get(&phy->access,
                                       &port_starting_lane,
                                       &port_num_lane));
...
STATIC
int _tsce_dpll_phy_drivermode_phymod_to_tier1(int8_t phymod_drivermode, int8_t* drivermode)
{
...
STATIC
int _tsce_dpll_phy_drivermode_tier1_to_phymod(int8_t drivermode, int8_t* phymod_drivermode)
{
    switch (drivermode) {
        case DM_DEFAULT:
...
int tsce_dpll_phy_tx_set(const phymod_phy_access_t* phy, const phymod_tx_t* tx)
{
    ...
    if((tx->drivermode != -1) &&
       (phy->device_op_mode & PHYMOD_INTF_CONFIG_TX_FIR_DRIVERMODE_ENABLE)) {
        PHYMOD_IF_ERR_RETURN
            (_tsce_dpll_phy_drivermode_phymod_to_tier1(tx->drivermode, &drivermode));
        PHYMOD_IF_ERR_RETURN
            (eagle2_tsc2pll_write_tx_afe(&phy_copy.access, TX_AFE_DRIVERMODE, drivermode));
    }
...
int tsce_dpll_phy_tx_get(const phymod_phy_access_t* phy, phymod_tx_t* tx)
{
    ...
    tx->amp = value;
    PHYMOD_IF_ERR_RETURN
        (eagle2_tsc2pll_read_tx_afe(&phy->access, TX_AFE_DRIVERMODE, &drivermode));
    PHYMOD_IF_ERR_RETURN
        (_tsce_dpll_phy_drivermode_tier1_to_phymod(drivermode, &value));
    tx->drivermode = value;
...
/* Get timesync ts_offset */
int tsce_dpll_timesync_offset_get(const phymod_core_access_t* core, uint32_t *ts_offset)
{
    phymod_core_access_t core_copy;
    ...
    TSCE_DPLL_PHY_ACCESS_PLLIDX_SET(&core_copy);
    PHYMOD_IF_ERR_RETURN
        (temod2pll_1588_ts_offset_get(&core_copy.access, ts_offset));
    return PHYMOD_E_NONE;
}
