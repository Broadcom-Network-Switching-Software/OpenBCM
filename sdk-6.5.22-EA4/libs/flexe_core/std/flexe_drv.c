...
soc_flexe_mux_timeslot_convert(
    ...
    int is_100g)
{
    int timeslot_mod_20 = inner_timeslot % 20;
    int hw_timeslot = global_timeslot;
...
_soc_flexe_mux_channel_add(
    ...
    SHR_BITDCL ts_mask[_SHR_BITDCLSIZE(FLEXE_CORE_NOF_TIMESLOTS)],
    int calendar_sel)
{
    SHR_BITDCL hw_tsmap[_SHR_BITDCLSIZE(FLEXE_CORE_NOF_TIMESLOTS)];
    uint32 timeslots[FLEXE_CORE_NOF_TIMESLOTS] = { 0 };
    uint32 inner_timeslots[FLEXE_CORE_NOF_TIMESLOTS] = { 0 };
    ...
    slot_index = 0;
    SHR_BIT_ITER(ts_mask, FLEXE_CORE_NOF_TIMESLOTS, timeslot)
    {
        inner_timeslots[slot_index] = timeslot;
        slot_index++;
    }
    ...
    SHR_BIT_ITER(global_ts_mask, FLEXE_CORE_NOF_TIMESLOTS, timeslot)
    {
        hw_timeslot = soc_flexe_mux_timeslot_convert(timeslot, inner_timeslots[slot_index], (mux_mode != FLEXE_MUX_MODE_50G));
    ...
    int calendar_sel)
{
    ...
    SHR_IF_ERR_EXIT(_soc_flexe_mux_channel_add(unit, channel, global_ts_mask, ts_mask, calendar_sel));
    ...
    SHR_IF_ERR_EXIT(soc_flexe_std_mux_rateadp_channel_add(unit, channel, global_ts_mask));
#endif
    ...
    int calendar_sel)
{
    ...
    SHR_IF_ERR_EXIT(_soc_flexe_mux_channel_del(unit, channel, global_ts_mask, ts_mask, calendar_sel));

#ifdef FLEXE_DRV_ALLOC_TIMESLOT_IN_RATE_ADAPTER
    
    SHR_IF_ERR_EXIT(soc_flexe_std_mux_rateadp_channel_del(unit, channel, global_ts_mask));
#endif
