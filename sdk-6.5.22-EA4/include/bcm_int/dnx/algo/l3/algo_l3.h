...
#define DNX_ALGO_L3_PHYSICAL_FEC_GET(unit,fec_id) (DNX_ALGO_L3_FEC_ID_TO_SUPER_FEC_ID(fec_id) * dnx_data_l3.fec.super_fec_size_get(unit) /  dnx_data_mdb.direct.fec_row_width_get(unit))
...
/**
 * \brief
 *   Return the MAX capacity that can be used for the VRRP exem table.
 * \param [in] unit - The unit number.
 * \param [out] capacity - The EXEM capacity.
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 * \see
