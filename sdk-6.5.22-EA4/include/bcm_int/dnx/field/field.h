...
#define DNX_FIELD_INPUT_SIZE_OFFSET_VERIFY(_unit, _input_offset, _input_size, _fg_id)  \
    { \
        ...
        {    \
            ...
            {    \
                SHR_IF_ERR_EXIT(dnx_field_group_action_length_type_get(_unit,_fg_id,&_action_length_type)); \
                if (_action_length_type == DNX_FIELD_ACTION_LENGTH_TYPE_LEXEM) \
                { \
                    _block_size = dnx_data_field.exem.large_max_result_size_get(unit); \
                } \
                else if (_action_length_type == DNX_FIELD_ACTION_LENGTH_TYPE_SEXEM) \
                { \
                     _block_size = dnx_data_field.exem.small_max_result_size_get(unit); \
                } \
                else \
                { \
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, \
                             "\r\n"  \
                             "Unknown action_length_type (%d) for EXEM. Quit\r\n", \
                             _action_length_type);    \
                } \
                break;    \
            }    \
