    ...
    uint32 *filter_type_val)
{
    ...
    DNXF_UNIT_LOCK_TAKE(unit);
    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cell_filter_set,
                               (unit, flags, array_size, filter_type_arr, filter_type_val));
    SHR_IF_ERR_EXIT(rc);
    ...
    soc_dnxc_fabric_cell_info_t * cell_info)
{
    ...
    SHR_ALLOC_SET_ZERO(parse_table, (soc_dnxc_fabric_cell_field_nof + 1) * sizeof(soc_dnxc_fabric_cell_parse_table_t),
                       "parse_table", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_cell_parse_table_get,
    ...
    int unit)
{
    ...
    {
        rv = dnxc_captured_buffer_add(unit, &SOC_DNXF_CONTROL(unit)->captured_cells_buffer,
