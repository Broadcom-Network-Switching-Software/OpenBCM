    ...
    int unit)
{
    ...
    for (quad = 0;
         quad < dnxf_data_port.general.nof_links_get(unit) / dnxf_data_device.blocks.nof_links_in_fmac_get(unit);
         quad++)
    {
        ...
        if (!quad_active)
        {
            for (quad_inner_link = 0; quad_inner_link < dnxf_data_device.blocks.nof_links_in_fmac_get(unit);
                 quad_inner_link++)
            {
                SOC_PBMP_PORT_ADD(pbmp_disabled_links,
                                  quad * dnxf_data_device.blocks.nof_links_in_fmac_get(unit) + quad_inner_link);
            }
        }
    }
    ...
    for (port = 0; port < dnxf_data_port.general.nof_links_get(unit); port++)
    {
        ...
        if (!(PBMP_MEMBER(dnxf_data_port.general.supported_phys_get(unit)->pbmp, port))
            || PBMP_MEMBER(pbmp_disabled_links, port))
        {
    ...
    int unit)
{
    ...
    rc = soc_dnxc_interrupt_event_init(unit);
    SHR_IF_ERR_EXIT(rc);
    ...
    for (blk = 0; SOC_BLOCK_INFO(unit, blk).type >= 0; blk++)
    {
        ...
        switch (blktype)
        {
            ...
            case SOC_BLK_LCM:
                ...
                break;
            case SOC_BLK_CCML:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_CCML_BLKS)
                {
                    si->ccml_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much CCML blocks");
                }
                break;
            case SOC_BLK_GCK_WRAP:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_GCK_WRAP_BLKS)
                {
                    si->gck_wrap_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much GCK_WRAP blocks");
                }
                break;
            case SOC_BLK_GCK:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_GCK_BLKS)
                {
                    si->gck_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much GCK blocks");
                }
                break;
            case SOC_BLK_DTM:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_DTM_BLKS)
                {
                    si->dtm_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much DTM blocks");
                }
                break;
            case SOC_BLK_DTL:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_DTL_BLKS)
                {
                    si->dtl_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much DTL blocks");
                }
                break;
            case SOC_BLK_DFL:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_DFL_BLKS)
                {
                    si->dfl_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much DFL blocks");
                }
                break;
            case SOC_BLK_BRDC_FSRD:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_BRDC_FSRD_BLKS)
                {
                    si->brdc_fsrd_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->broadcast_blocks[blk] = si->fsrd_block;
                    si->broadcast_blocks_size[blk] = SOC_MAX_NUM_FSRD_BLKS;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much BRDC_FSRD blocks");
                }
                break;
            ...
            case SOC_BLK_BRDC_FMAC:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_BRDC_FMAC_BLKS)
                {
                    si->brdc_fmac_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
            ...
            case SOC_BLK_BRDC_GCK_WRAP:
                si->brdc_gck_wrap_block = blk;
                si->broadcast_blocks[blk] = si->gck_wrap_block;
                si->broadcast_blocks_size[blk] = SOC_MAX_NUM_GCK_WRAP_BLKS;
                break;
            ...
            case SOC_BLK_BRDC_GCK:
                si->brdc_gck_block = blk;
                si->broadcast_blocks[blk] = si->gck_block;
                si->broadcast_blocks_size[blk] = SOC_MAX_NUM_GCK_BLKS;
                break;
    ...
    int unit)
{
    ...
    rc = soc_dnxc_interrupt_event_deinit(unit);
    SHR_IF_ERR_CONT(rc);
