    ...
    )
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    kaps_db_get_prefix_handle(db_p, data, prefix_len, &db_entry);
    if (db_entry != NULL) {
        is_update = 1;
    }

    if (!is_update) {
        res = kaps_db_add_prefix(
                db_p, 
                data, 
                prefix_len,
                &db_entry);
        ...
        if (SOC_IS_JERICHO_PLUS(unit) && (hitbit_enabled != 0)) {
            res = kaps_hb_db_add_entry(hb_db_p, &hb_entry);
            if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                kaps_db_delete_entry(db_p, db_entry); 
                kaps_ad_db_delete_entry(ad_db_p, ad_entry); 
                *success = SOC_SAND_FAILURE_INTERNAL_ERR;
            ...
            if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                kaps_db_delete_entry(db_p, db_entry); 
                kaps_ad_db_delete_entry(ad_db_p, ad_entry); 
                kaps_hb_db_delete_entry(hb_db_p, hb_entry); 
                *success = SOC_SAND_FAILURE_INTERNAL_ERR;
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
            }
        }
        ...
        res = kaps_entry_get_ad(db_p, db_entry, &ad_entry);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            *success = SOC_SAND_FAILURE_INTERNAL_ERR;
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
        }

        res = kaps_ad_db_update_entry(ad_db_p, ad_entry, assoData);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            *success = SOC_SAND_FAILURE_INTERNAL_ERR;
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
        }
...
#endif
    {
        ...
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            ...
            LOG_ERROR(BSL_LS_SOC_TCAM,
                     (BSL_META_U(unit,
                                 "Error in %s(): Entry add : kaps_db_install failed with: %s!\n"),
                                 FUNCTION_NAME(),
                      kaps_get_status_string(res)));
        if(res == KAPS_OUT_OF_DBA || res == KAPS_OUT_OF_UIT || res == KAPS_OUT_OF_UDA || res == KAPS_OUT_OF_AD){
    ...
    )
{
    ...
    res = kaps_db_get_prefix_handle(db_p, data, prefix_len, &db_entry);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    if (SOC_IS_JERICHO_PLUS(unit) && (hitbit_enabled != 0)) {
        ...
        res = kaps_entry_get_hb(db_p, db_entry, &hb_entry);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
       LOG_ERROR(BSL_LS_SOC_TCAM,
                 (BSL_META_U(unit,
                             "Error in %s(): kaps_ad_db_delete_entry failed with error: %s!\n"), 
                             FUNCTION_NAME(),
                  kaps_get_status_string(res)));
    ...
    if (SOC_IS_JERICHO_PLUS(unit) && (hitbit_enabled != 0)) {
        ...
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
           LOG_ERROR(BSL_LS_SOC_TCAM,
                     (BSL_META_U(unit,
                                 "Error in %s(): kaps_hb_db_delete_entry failed with error: %s!\n"), 
                                 FUNCTION_NAME(),
                      kaps_get_status_string(res)));
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 90, exit);
        }
...
#endif
    {
        ...
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                      (BSL_META_U(unit,
                                  "Error in %s(): kaps_db_install failed with error: %s!\n"), 
                                  FUNCTION_NAME(),
                       kaps_get_status_string(res)));
             SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 90, exit);
        }
...
uint32 jer_pp_kaps_get_hit_bit(
         ...
         SOC_SAND_INOUT JER_KAPS_ENTRY  *db_entry,
      ...
      )
{
    ...
    res = kaps_hb_entry_get_bit_value(hb_db_p, hb_entry, &bit_value, clear);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    )
{
    ...
    *found = FALSE;
    ...
    res = kaps_ad_db_get(ad_db_p, ad_entry, assoData);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
...
{
    JER_KAPS_ENTRY_ITER *iter=NULL;
    JER_KAPS_ENTRY *kaps_e = NULL;
    JER_KAPS_ENTRY_INFO kaps_e_info;

    JER_KAPS_AD_DB *ad_db_p = NULL;
    ...
    do {
        ++entry_index;
        res = kaps_db_entry_iter_next(tbl_p, iter, &kaps_e);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 10, exit);
        }
    }
    while ( kaps_e && (entry_index < block_range_key->start.payload.arr[0]));

    do {
        if (kaps_e == NULL)
            break;

        res = kaps_entry_get_info(tbl_p, kaps_e, &kaps_e_info);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 30, exit);
        }
        ...
        res = arad_pp_dbal_kaps_buffer_to_entry_key(unit, dbal_table, kaps_e_info.prio_len, kaps_e_info.data, qual_vals_array + (entry_index - block_range_key->start.payload.arr[0])*SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
        ...
        {
            res = kaps_ad_db_get(ad_db_p, kaps_e_info.ad_handle, ad_bytes_array);
            if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    if((!kaps_e) || ((*nof_entries) < block_range_key->entries_to_act))
    {
...
exit:
    ...
    res = kaps_db_entry_iter_destroy(tbl_p, iter);
    ...
    )
{
    ...
    res = kaps_db_entry_iter_init(db_p, &iter_p);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
    }
    SOC_SAND_CHECK_NULL_PTR(iter_p, 90, exit);

    res = kaps_db_entry_iter_next(db_p, iter_p, &db_entry);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
    }

    while (db_entry != NULL) {
        res = kaps_entry_get_info(db_p, db_entry, &kaps_e_info);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 90, exit);
        }

        
        if (kaps_e_info.data[0] >> (SOC_SAND_NOF_BITS_IN_BYTE - prefix_len) == prefix) {
            
            res = jer_pp_kaps_entry_remove_hw(unit, table_id, kaps_e_info.prio_len, hit_bit_enabled, kaps_e_info.data);
            if (res != SOC_E_NONE) {
                SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 75, exit);
            }

            
            res = kaps_db_entry_iter_destroy(db_p, iter_p);
            if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
            }
    ...
    )
{
    ...
    for (offset = 0; offset < JER_KAPS_DMA_DB_NOF_PMF_ENTRIES; offset++) {
        ...
        res = kaps_dma_db_add_entry(db_dma_p,
                                   entry_data,
                                   offset);
    ...
    )
{
    ...
    res = kaps_db_delete_all_entries(db_p);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
    }

    res = kaps_ad_db_delete_unused_entries(ad_db_p);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 90, exit);
    }
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in jer_pp_kaps_entry_table_clear_hw()",table_id,hit_bit_enabled);
}
                   ...
                   SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE   *success)
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    res = kaps_dma_db_read_entry(db_p, 
                               offset, 
                               entry_data);
    ...
    res = kaps_dma_db_add_entry(db_p, 
                               entry_data, 
                               offset);
                   ...
                   SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE    *success)
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    res = kaps_dma_db_delete_entry(db_p,  
                               offset);
