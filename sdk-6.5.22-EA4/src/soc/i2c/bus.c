...
soc_i2c_reset(int unit)
{
    ...
    if(soc_feature(unit, soc_feature_cmicx)) {
        if (soc_feature(unit, soc_feature_use_smbus3_for_i2c)) {
            READ_IPROCPERIPH_SMBUS3_SMBUS_CONFIGr(unit,&rval);
            soc_reg_field_set(unit, IPROCPERIPH_SMBUS3_SMBUS_CONFIGr, &rval, RESETf, 1);
            WRITE_IPROCPERIPH_SMBUS3_SMBUS_CONFIGr(unit,rval);
            soc_reg_field_set(unit, IPROCPERIPH_SMBUS3_SMBUS_CONFIGr, &rval, RESETf, 0);
            WRITE_IPROCPERIPH_SMBUS3_SMBUS_CONFIGr(unit,rval);
        } else if (soc_feature(unit, soc_feature_use_smbus2_for_i2c)) {
...
soc_i2c_attach(int unit, uint32 flags, int speed_khz)
{
    ...
    if(soc_feature(unit, soc_feature_cmicx)) {
        /* 1) Enable CPU access to I2C controller */
        if (soc_feature(unit, soc_feature_use_smbus3_for_i2c)) {
            READ_IPROCPERIPH_SMBUS3_SMBUS_CONFIGr(unit,&reg);
            soc_reg_field_set(unit, IPROCPERIPH_SMBUS3_SMBUS_CONFIGr, &reg, SMB_ENf, 1);
            WRITE_IPROCPERIPH_SMBUS3_SMBUS_CONFIGr(unit,reg);

            READ_IPROC_WRAP_IPROC_STRAP_CTRL_1r(unit, &reg);
            soc_reg_field_set(unit, IPROC_WRAP_IPROC_STRAP_CTRL_1r, &reg,
                                                              I2C4_OR_I2C3_MUX_SELf, 0);
            WRITE_IPROC_WRAP_IPROC_STRAP_CTRL_1r(unit, reg);
        } else if (soc_feature(unit, soc_feature_use_smbus2_for_i2c)) {
