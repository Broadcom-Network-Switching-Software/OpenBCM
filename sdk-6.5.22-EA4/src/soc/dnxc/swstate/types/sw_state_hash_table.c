    ...
    uint32 alloc_flags)
{
    ...
    DNX_SW_STATE_ALLOC_BITMAP(unit, module_id, (*hash_table)->next, (max_nof_elements * ptr_size_in_bits),
    ...
    DNX_SW_STATE_ALLOC_BITMAP(unit, module_id, (*hash_table)->lists_head, max_nof_elements * ptr_size_in_bits,
    ...
    SW_STATE_HASH_TABLE_KEY * const key)
{
    ...
    if (first)
    {
        ...
        DNX_SW_STATE_BIT_RANGE_WRITE(unit, module_id, 0, hash_table->lists_head, ptr_size * prev_entry,
                                     ptr_size * entry_offset, ptr_size ,hash_table->next);
    }
    else
    {
        
        DNX_SW_STATE_BIT_RANGE_WRITE(unit, module_id, 0, hash_table->next, ptr_size * prev_entry,
                                     ptr_size * entry_offset, ptr_size, hash_table->next);
    }
    ...
    DNX_SW_STATE_BIT_RANGE_WRITE(unit, module_id, 0, hash_table->next, entry_offset * ptr_size, 0, ptr_size, null_ptr);
exit:
    if(is_suppressed)
    {
        DNXC_SW_STATE_COMPARISON_UNSUPPRESS(unit);
    ...
    uint8 *first)
{
    ...
    *ptr_long = 0;
    DNX_SW_STATE_BIT_RANGE_READ(unit, module_id, 0, hash_table->lists_head, ptr_size * hash_val, 0, ptr_size, ptr_long);
    ...
    if (*ptr_long == null_ptr)
    {
        if (get_first_empty)
        {
            ...
            else
            {
                ...
                DNX_SW_STATE_BIT_RANGE_WRITE(unit, module_id, 0, hash_table->lists_head, ptr_size * hash_val, 0, ptr_size, ptr_long);
                *entry = *ptr_long;
            }
            *found = FALSE;
            goto exit;
    ...
    while (*ptr_long != null_ptr)
    {
        ...
        *first = FALSE;
        ...
        sal_memset(tmp_shr_bitdcl_buf, 0, tmp_buf_size);
        DNX_SW_STATE_BIT_RANGE_READ(unit, module_id, 0, hash_table->next, ptr_size * (*ptr_long), 0, ptr_size, (uint32*)tmp_shr_bitdcl_buf);
    ...
    *found = FALSE;
    if (get_first_empty)
    {
        ...
        else
        {
            ...
            DNX_SW_STATE_BIT_RANGE_WRITE(unit, module_id, 0, hash_table->next,
...
sw_state_hash_table_clear(
    ...
    sw_state_htbl_t hash_table)
{
    uint32 max_buf_size, data_size, ptr_size, max_nof_elements, key_size;
    int32 offset;
    uint8 is_suppressed = FALSE;
    ...
    sw_state_htbl_t hash_table)
{
    ...
    for (indx = 0; indx < expected_nof_elements; ++indx)
    {
        ...
        *ptr_long = 0;
        DNX_SW_STATE_BIT_RANGE_READ(unit, module_id, 0, hash_table->lists_head, ptr_size * indx, 0, ptr_size, ptr_long);
        ...
        while (*ptr_long != null_ptr)
        {
            ...
            if (hash_table->is_index) {
                ...
                cur_data = (uint8*)(hash_table->data) + data_size * ptr_long[0];
            }
            if (sal_strncmp(hash_table->print_cb_name, "", SW_STATE_CB_DB_NAME_STR_SIZE-1) != 0) {
                ...
                if (hash_table->is_index) {
                    ...
                    DNX_SW_STATE_PRINT(unit,"%u", ptr_long[0]);
                }
            }
            DNX_SW_STATE_PRINT(unit, "\n");
            ...
            DNX_SW_STATE_BIT_RANGE_READ(unit, module_id, 0, hash_table->next, ptr_size * ptr_long[0], 0, ptr_size, ptr_long_tmp);
            ptr_long[0] = 0;
    ...
    uint32 *num_active_ptr)
{
    ...
    for (indx = 0; indx < expected_nof_elements; ++indx)
    {
        ...
        DNX_SW_STATE_BIT_RANGE_READ(unit, module_id, 0, hash_table->lists_head, ptr_size * indx, 0, ptr_size, tmp_buf_ptr);
        ...
        while (*tmp_buf_ptr != null_ptr)
        {
            ...
            DNX_SW_STATE_BIT_RANGE_READ(unit, module_id, 0, hash_table->next, ptr_size * (*tmp_buf_ptr), 0, ptr_size, tmp_buf_ptr);
        }
    }
exit:
