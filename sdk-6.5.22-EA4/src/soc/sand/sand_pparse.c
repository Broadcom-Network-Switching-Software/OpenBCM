    ...
    int *header_type)
{
    ...
    *header_type = -1;
    ...
    SHR_IF_ERR_EXIT(dnx_switch_header_type_in_get(unit, gport, header_type));
    ...
    uint32 *enabled)
{
    ...
    uint32 fwd_layer_index, fwd_layer_index_vt4;
    ...
    *enabled = FALSE;
    ...
    rv = sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", "VTT5", "FWD1", "Fwd_Layer_Index", &sig_fwd_index_p);
    if (rv == _SHR_E_NONE)
    {
    ...
    if (fwd_layer_index <= PPARSE_MPLS_UNTERM_LAYER_INDEX)
    {
    ...
    if (fwd_layer_index_vt4 <= PPARSE_MPLS_UNTERM_LAYER_INDEX)
    {
    ...
    
    {
        ...
        rv = dnx_algo_lif_mapping_global_to_local_get(unit, DNX_ALGO_LIF_INGRESS, global_in_lif, &local_lif_info);
        if (rv != _SHR_E_NONE)
        {
            SHR_EXIT();
        }
    ...
    {
        SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", "VTT5", "FWD1", "Layer_Protocols.3", &sig_proto_p));
