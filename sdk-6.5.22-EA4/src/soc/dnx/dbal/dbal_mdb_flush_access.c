...
void *flush_buffer[BCM_MAX_NUM_UNITS] = { NULL };

int flush_buffer_next_entry[BCM_MAX_NUM_UNITS] = { 0 };

int flush_buffer_entries_count[BCM_MAX_NUM_UNITS] = { 0 };
    ...
    soc_reg_above_64_val_t mdb_entry_mask)
{
    ...
    lem_entry_size = dnx_data_mdb.em.em_info_get(unit, DBAL_PHYSICAL_TABLE_LEM)->entry_size;
         ...
         entry_encoding_index--)
    {
        ...
        entry_size =
            dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, DBAL_PHYSICAL_TABLE_LEM)->row_width / (1 << entry_encoding_index);

        vmv_aligned_offset =
            lem_entry_size - dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, DBAL_PHYSICAL_TABLE_LEM)->row_width + entry_size;
    ...
    int unit)
{
    ...
    lem_emp_scan_status_field = dnx_data_mdb.em.em_emp_get(unit, DBAL_PHYSICAL_TABLE_LEM)->emp_scan_status_field;
#endif
    ...
    int unit)
{
...
#if defined(BCM_DNX2_SUPPORT)
        SHR_IF_ERR_EXIT(mdb_get_capacity(unit, DBAL_PHYSICAL_TABLE_LEM, &capacity));
#endif
        flush_buffer_entries_max =
            capacity * MDB_DIRECT_BASIC_ENTRY_SIZE / (MDB_NOF_CLUSTER_ROW_BITS / (MDB_EM_ENTRY_ENCODING_HALF << 1));
    }
    ...
    while (flush_in_work)
    {
        ...
        else
        {
            ...
            if (flush_buffer_entries_max - flush_buffer_entries_count[unit] < entries_count)
            {
                entries_to_copy = flush_buffer_entries_max - flush_buffer_entries_count[unit];
                buffer_overflow = 1;
            }
            else
            {
            ...
            sal_memcpy((uint8 *) flush_buffer[unit] + (flush_buffer_entries_count[unit] * entry_size), retrievd_entries,
                       entries_to_copy * entry_size);

            flush_buffer_entries_count[unit] += entries_to_copy;

            SHR_IF_ERR_EXIT(soc_dnx_fifodma_set_entries_read(unit, dma_channel_id, entries_count));
        }
    }

    for (flush_done_iter = 0; flush_done_iter < 2; flush_done_iter++)
    {
    ...
    uint8 non_blocking)
{
    ...
    if (bulk_commit || !flush_in_bulk)
    {
        ...
        if (!hw_accelerated)
        {
            dbal_flush_dma_copy_entries(unit);
        }
        else
        {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    while ((found_valid_entry == 0) && (flush_buffer_next_entry[unit] < flush_buffer_entries_count[unit]))
    {
        entry = (uint8 *) flush_buffer[unit] + (flush_buffer_next_entry[unit] * entry_size);
        entry_32 = (uint32 *) entry;
        flush_buffer_next_entry[unit]++;
    ...
    else
    {
        ...
        flush_buffer_next_entry[unit] = 0;
        flush_buffer_entries_count[unit] = 0;
    ...
    int unit)
{
    ...
    flush_buffer_entries = dnx_data_l2.age_and_flush_machine.flush_buffer_nof_entries_get(unit);

    if (flush_buffer_entries == -1)
    {

#if defined(BCM_DNX2_SUPPORT)
        SHR_IF_ERR_EXIT(mdb_get_capacity(unit, DBAL_PHYSICAL_TABLE_LEM, &capacity));
#endif
        flush_buffer_entries =
            capacity * MDB_DIRECT_BASIC_ENTRY_SIZE / (MDB_NOF_CLUSTER_ROW_BITS / (MDB_EM_ENTRY_ENCODING_HALF << 1));
    }
    entry_size = 4 * BYTES2WORDS(soc_reg_bytes(unit, MACT_LARGE_EM_REPLYr));
    host_mem_size_in_bytes = entry_size * flush_buffer_entries;
    flush_buffer[unit] = sal_alloc(host_mem_size_in_bytes, "L2 Flush buffer");
    flush_buffer_entries_count[unit] = 0;
    flush_buffer_next_entry[unit] = 0;
...
dbal_flush_buffer_deinit(
    int unit)
{
    if (flush_buffer[unit] != NULL)
    {
        sal_free(flush_buffer[unit]);
    }

    flush_buffer[unit] = NULL;
}
