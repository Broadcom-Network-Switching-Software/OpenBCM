    ...
    int unit)
{
    ...
    if (dnx_data_mdb.global.feature_get(unit, dnx_data_mdb_global_init_enable))
    {
    ...
    uint32 *is_mact)
{
    ...
    if (((app_id == mact_app_id) || (app_id == mact_ivl_app_id) || (app_id == mact_pcc_app_id)))
    {
        *is_mact = TRUE;
    }
    else
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else
    {
        ...
        if (physical_table_actions->iterator_init == NULL)
        {
    ...
    dbal_iterator_info_t * iterator_info)
{
    ...
    while (!entry_found && !iterator_info->is_end)
    {
        ...
        else
        {
            ...
            if (physical_table_actions->iterator_get_next == NULL)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "ERROR missing implementation for %s\n",
                             dbal_physical_table_to_string(unit, physical_db_id));
            }
    ...
    dbal_iterator_info_t * iterator_info)
{
    ...
    if (iterator_info->mdb_iterator.iterate_in_flush_machine)
    {
        SHR_IF_ERR_EXIT(dbal_iterator_rules_shadow_set(unit, entry_handle, iterator_info->actions_bitmap,
                                                       result_types_in_flush_bitmap[0], nof_flush_rules));
    ...
    uint32 app_id)
{
           ...
           && (iterator_info->physical_db_index < (table->nof_physical_tables - 1)))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_mdb_phy_table_actions_get
                        (unit, table->physical_db_id[iterator_info->physical_db_index], &physical_table_actions));
        if (physical_table_actions->entry_get == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "missing get implementation %s\n",
                         dbal_physical_table_to_string(unit, table->physical_db_id[iterator_info->physical_db_index]));
        }

        SHR_IF_ERR_EXIT(physical_table_actions->entry_get(unit,
                                                          table->physical_db_id
                                                          [iterator_info->physical_db_index], app_id, phy_entry, 0));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else
    {
        ...
        if (physical_table_actions->iterator_deinit == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "ERROR missing implementation for %s\n",
                         dbal_physical_table_to_string(unit, physical_db_id));
        }
    ...
    uint8 is_eedb_ll)
{
    ...
    if (physical_table_actions->entry_delete == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "missing implementation %s\n",
                     dbal_physical_table_to_string(unit, physical_db_ids[0]));
    }
    ...
    uint8 is_eedb_ll)
{
    ...
    SHR_IF_ERR_EXIT(dbal_mdb_phy_table_actions_get(unit, physical_db_ids[0], &physical_table_actions));
    if (physical_table_actions->entry_add == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "missing implementation %s\n",
                     dbal_physical_table_to_string(unit, physical_db_ids[0]));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT(dbal_mdb_phy_table_actions_get(unit, physical_db_id, &physical_table_actions));
    if (physical_table_actions->table_clear == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "missing implementation %s\n",
                     dbal_physical_table_to_string(unit, physical_db_id));
    }
