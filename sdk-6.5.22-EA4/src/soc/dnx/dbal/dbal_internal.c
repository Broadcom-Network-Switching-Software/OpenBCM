    ...
    int pool_id)
{
    ...
    sal_sem_give(dbal_mngr[unit]->collect_info.collection_semaphore_array[pool_id]);
    ...
    void *cookie)
{
    ...
    while (1)
    {
        ...
        int curr_handle;
        sal_sem_take(dbal_mngr[unit]->collect_info.collection_semaphore_array[my_id], sal_sem_FOREVER);

        sal_mutex_take(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[my_id], sal_mutex_FOREVER);
        sal_mutex_take(dbal_mngr[unit]->collect_info.collection_worker_thread_sync, sal_mutex_FOREVER);

        if (dbal_mngr[unit]->collect_info.collection_signals[my_id] == DBAL_COLLECTION_SIGNAL_DESTROY)
        {
        ...
        for (curr_handle = first_entry_in_pool; curr_handle < first_entry_in_pool + collection_pool_size; curr_handle++)
        {
            ...
            else
            {
                ...
                if (rv == _SHR_E_NONE)
                {
                    ...
                    {
                        DBAL_COLLECTION_PRINTS((BSL_META("pool %d - entry %d added\n"), my_id, entries_counter));
                    }
                }
                else
                {
                    ++dbal_mngr[unit]->collect_info.nof_collect_operation_failures;
        ...
        sal_mutex_give(dbal_mngr[unit]->collect_info.collection_worker_thread_sync);

        sal_mutex_give(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[my_id]);
    }

    sal_sem_destroy(dbal_mngr[unit]->collect_info.collection_semaphore_array[my_id]);

    sal_mutex_give(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[my_id]);

    sal_mutex_destroy(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[my_id]);
    ...
    int unit)
{
    ...
    dbal_mngr[unit]->collect_info.collection_counter = 0;
    dbal_mngr[unit]->collect_info.nof_collect_operation_failures = 0;

    if (dnx_data_dbal.db_init.dbal_collection_pool_size_get(unit))
    {
        worker_thread_params.unit = unit;
        dbal_mngr[unit]->collect_info.collection_worker_thread_sync = sal_mutex_create("worker sync mutex");
        if (dbal_mngr[unit]->collect_info.collection_worker_thread_sync == NULL)
        {
        ...
        {
            dbal_mngr[unit]->collect_info.collection_signals[worker_thread_id] = DBAL_COLLECTION_SIGNAL_PROCESS_POOL;
            worker_thread_params.mutex_id = worker_thread_id;

            dbal_mngr[unit]->collect_info.collection_pool_mutex_array[worker_thread_id] =
                sal_mutex_create("collection pool");
            if (dbal_mngr[unit]->collect_info.collection_pool_mutex_array[worker_thread_id] == NULL)
            {
                SHR_ERR_EXIT(SOC_E_MEMORY, "collection pool mutex allocation failure.");
            }
            dbal_mngr[unit]->collect_info.collection_semaphore_array[worker_thread_id] =
                sal_sem_create("collection sem", sal_sem_COUNTING, 0);
            if (dbal_mngr[unit]->collect_info.collection_semaphore_array[worker_thread_id] == NULL)
            {
            ...
            dbal_mngr[unit]->collect_info.worker_threads[worker_thread_id] =
                sal_thread_create(thread_name, 4 * SAL_THREAD_STKSZ, 50, dbal_collection_worker_thread,
    ...
    int unit)
{
    ...
    if (dnx_data_dbal.db_init.dbal_collection_pool_size_get(unit))
    {
        for (worker_thread_id = 0; worker_thread_id < DBAL_NOF_COLLECTION_POOLS; worker_thread_id++)
        {
            ...
            sal_mutex_take(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[worker_thread_id],
                           sal_mutex_FOREVER);
            dbal_mngr[unit]->collect_info.collection_signals[worker_thread_id] = DBAL_COLLECTION_SIGNAL_DESTROY;
            sal_mutex_give(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[worker_thread_id]);

            sal_sem_give(dbal_mngr[unit]->collect_info.collection_semaphore_array[worker_thread_id]);
        }
        for (worker_thread_id = 0; worker_thread_id < DBAL_NOF_COLLECTION_POOLS; worker_thread_id++)
        {
            rv = sal_thread_join(dbal_mngr[unit]->collect_info.worker_threads[worker_thread_id], NULL);
            if (rv)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "sal_thread_join failed with error %d\n", rv);
            }
            DBAL_COLLECTION_PRINTS((BSL_META("===== Worker thread %d was joined =====\n"), worker_thread_id));
        }
        sal_mutex_destroy(dbal_mngr[unit]->collect_info.collection_worker_thread_sync);
    }
    ...
    dbal_collection_trigger_type_e trigger_type)
{
    ...
    pool_id = DBAL_COLLECTION_POOL_ID_GET(dbal_mngr[unit]->collect_info.collection_counter);

    first_entry_in_pool = DBAL_SW_NOF_ENTRY_HANDLES + (pool_id * collection_pool_size);
    nof_entries_in_pool = DBAL_COLLECTION_NOF_ENTRIES_IN_POOL(dbal_mngr[unit]->collect_info.collection_counter);
    ...
    if (trigger_type == DBAL_COLLECTION_TRIGGER_PROCESS_AND_WAIT)
    {
        ...
        while (!is_task_completed)
        {
            ...
            sal_mutex_take(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[pool_id], sal_mutex_FOREVER);
            ...
            sal_mutex_give(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[(pool_id)]);
        }
        ...
        if (dbal_mngr[unit]->collect_info.nof_collect_operation_failures > 0)
        {
            LOG_ERROR(BSL_LS_SOC_DMA,
                      (BSL_META_U(unit, "\nThere were %d errors in collect mode process\n"),
                       dbal_mngr[unit]->collect_info.nof_collect_operation_failures));
            dbal_mngr[unit]->collect_info.nof_collect_operation_failures = 0;
        }
    }
    ...
    int *pool_id)
{
    ...
    int pool_size = dnx_data_dbal.db_init.dbal_collection_pool_size_get(unit);

    SHR_FUNC_INIT_VARS(unit);

    if (dbal_mngr[unit]->collect_info.collection_counter + 1 == DBAL_SW_NOF_ENTRY_HANDLES_FOR_COLLECTION)
    {
    ...
    else
    {
        ...
        pool_id_to_check = DBAL_COLLECTION_POOL_ID_GET(dbal_mngr[unit]->collect_info.collection_counter + 1);
    }
    ...
    while (!is_found)
    {
        ...
        sal_mutex_take(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[(pool_id_to_check)],
                       sal_mutex_FOREVER);

        handle_index = pool_size ? DBAL_SW_NOF_ENTRY_HANDLES + pool_id_to_check * DBAL_COLLECTION_POOL_SIZE : 0;
        if (dbal_mngr[unit]->entry_handles_pool[handle_index].handle_status != DBAL_HANDLE_STATUS_ACTION_COLLECTED)
        {
            is_found = TRUE;
            (*pool_id) = pool_id_to_check;
        }
        sal_mutex_give(dbal_mngr[unit]->collect_info.collection_pool_mutex_array[(pool_id_to_check)]);
    ...
    uint8 operation)
{
    ...
    if (operation == DBAL_HANDLE_TAKE_ALLOC)
    {
        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_COLLECT_MODE))
        {
            ...
            if (DBAL_COLLECTION_IS_POOL_FULL(dbal_mngr[unit]->collect_info.collection_counter))
            {
                int pool_id, old_pool_id;

                old_pool_id = DBAL_COLLECTION_POOL_ID_GET(dbal_mngr[unit]->collect_info.collection_counter);

                SHR_IF_ERR_EXIT(dbal_collection_get_next_available_pool(unit, &pool_id));
                dbal_mngr[unit]->collect_info.collection_counter =
                ...
                DBAL_COLLECTION_PRINTS((BSL_META
                                        ...
                                        old_pool_id, pool_id, dbal_mngr[unit]->collect_info.collection_counter));
            ...
            handle_index = dbal_mngr[unit]->collect_info.collection_counter + DBAL_SW_NOF_ENTRY_HANDLES;
...
exit:
    if ((entry_handle_id < DBAL_SW_NOF_ENTRY_HANDLES) && mutex_taken)
    {
        SHR_IF_ERR_EXIT(sal_mutex_give(dbal_mngr[unit]->handle_mutex));
    }
    SHR_FUNC_EXIT;
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    DBAL_FIELD_ERR_HANDLE((table_field == NULL), "table field is NULL, KEY field not found");
