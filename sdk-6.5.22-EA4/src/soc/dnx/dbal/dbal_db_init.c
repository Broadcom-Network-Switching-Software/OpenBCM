    ...
    dbal_parsed_db_dnx_data_struct_t * int_or_dnx_data)
{
    ...
    dnx_data_label += strlen(DB_INIT_DNX_DATA_SDK_MODULE_PREFIX);
    ...
    data_str = strstr(dnx_data_label, DB_INIT_DNX_DATA_GET_SUFFIX);
    if (data_str != NULL)
    {
        dnx_data_label[strlen(dnx_data_label) - strlen(DB_INIT_DNX_DATA_GET_SUFFIX)] = 0;
    ...
    str_len = sal_strlen(str_formula);
    {
        ...
        else if (nof_operands == 0)
        {
            ...
            else
            {
                ...
                else if (sal_strstr(curr_str, DB_INIT_DNX_DATA_SDK_MODULE_PREFIX) != NULL)
                {
                    uint8 negative_check = FALSE;
                    uint32 dnx_data_val = 0;
    ...
    uint8 *has_valid_interface_field)
{
    ...
    if (table_entry->table_type == DBAL_TABLE_TYPE_DIRECT)
    {
        ...
        int rv = dbal_tables_field_index_in_table_get_no_err(unit, table_id, field_id, FALSE, result_idx, 0,
    ...
    int nof_valid_access_fields[])
{
    ...
    for (access_param_index = 0; access_param_index < hl_access_param->nof_access; access_param_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_db_init_l2p_field_id_has_valid_interface_field
                        (unit, table_id, table_entry, access_field_id, result_idx, &has_valid_interface_field));
    ...
    int result_idx)
{
    ...
    for (access_param_index = 0; access_param_index < hl_access_param->nof_access; access_param_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_db_init_l2p_field_id_has_valid_interface_field
                        (unit, table_id, table_entry, access_param->access_field_id, result_idx,
    ...
    dbal_logical_table_t * table)
{
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_table_interface_result_post_process(unit, table_id, table));
...
#else
/* *INDENT-OFF* */
shr_error_e dbal_db_init_table_kbp_post_process(int unit,dbal_tables_e table_id,dbal_logical_table_t * table){DNX_KBP_DUMMY_PROTOTYPE_ERR_MSG("dbal_db_init_table_kbp_post_process")}
    ...
    dbal_logical_table_t * table)
{
    ...
    for (res_type = 0; res_type < table->nof_result_types; res_type++)
    {
        ...
        if (total_of_of_mapped_fields < result_type_set->nof_result_fields)
        {
