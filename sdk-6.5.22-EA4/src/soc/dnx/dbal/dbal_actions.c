    ...
    dbal_entry_action_flags_e flags)
{
    ...
    if (actions_info->action_validation_flags & ALL_KEY_FIELDS_MUST_EXIST)
    {
        ...
        if (!DBAL_TABLE_IS_TCAM(table) && (table->table_type != DBAL_TABLE_TYPE_LPM))
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "illegal number of key fields for action %s in table %s, requested %d set %d\n",
    ...
    dbal_actions_e * action)
{
    ...
    if (*action == DBAL_ACTION_ENTRY_GET)
    {
        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_COLLECT_MODE))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Collect mode enabled for table %s: Get operation is not allowed\n",
                         table->table_name);
        }
    }
