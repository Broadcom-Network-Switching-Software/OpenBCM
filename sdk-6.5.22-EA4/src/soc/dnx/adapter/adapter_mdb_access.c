...
#define UPDATE_FRM_SOC_LOGICAL_MEM_TYPE_XLTCAM 195
...
shr_error_e
adapter_mdb_access_acl_prep_buffer(
    int unit,
    dbal_physical_tables_e dbal_physical_table_id,
    uint32 app_id,
    dbal_physical_entry_t * entry,
    ...
    dbal_physical_entry_t * entry)
{
    ...
    SHR_IF_ERR_EXIT(adapter_mdb_access_acl_prep_buffer
                    (unit, dbal_physical_table_id, app_id, entry, UPDATE_FRM_SOC_LOGICAL_ACCESS_WRITE,
                     &write_index, buffer));
    ...
    {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Error: adapter_mdb_access_acl_write: writing the opcode failed\n")));
        SHR_SET_CURRENT_ERR(_SHR_E_FAIL);
        SHR_EXIT();
    }
    ...
    dbal_physical_entry_t * entry)
{
    ...
    {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Error: adapter_mdb_access_acl_delete: writing the opcode failed\n")));
        SHR_SET_CURRENT_ERR(_SHR_E_FAIL);
        SHR_EXIT();
    }
    ...
    dbal_physical_entry_t * entry)
{
    ...
    {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Error: adapter_mdb_access_acl_read: writing the opcode failed\n")));
        SHR_SET_CURRENT_ERR(_SHR_E_FAIL);
        SHR_EXIT();
    }
    ...
    if (FD_ISSET(adapter_mem_reg_fd, &read_vect))
    {
        ...
        SHR_BITCOPY_RANGE(entry->payload, 0, entry_payload, ntohl(nw_order_payload_bit_size), entry->payload_size);
    }
    else
    {
        SHR_SET_CURRENT_ERR(_SHR_E_TIMEOUT);
    ...
    uint8 buffer[ADAPTER_MDB_ACCESS_MAX_BUFFER_SIZE_U8])
{
    ...
    table_entry_size =
        MDB_PAYLOAD_SIZE_TYPE_TO_PAYLOAD_SIZE(dnx_data_mdb.pdbs.
                                              mdb_pdb_info_get(unit, mdb_physical_table_id)->direct_max_payload_type);
