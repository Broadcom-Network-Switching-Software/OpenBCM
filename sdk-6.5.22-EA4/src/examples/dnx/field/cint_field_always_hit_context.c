...
int cint_field_flow_id_ext_add(int unit, bcm_field_context_t context_id)
{
    ...
    printf("Flow FG ID (%d) was created for context(%d)\n",fg_id_FLOW_id,context_id);
                                                                ...
                                                                bcm_field_context_t  *context_id)
{
    ...
    printf("add_flow_fg is set to (%d) \n", add_flow);
    ...
    if(stage == bcmFieldStageIngressPMF1)
    {
        ...
        flow_id_ext_add = *(dnxc_data_get(unit, "headers", "feature", "flow_id_ext_enable", NULL));

        if(flow_id_ext_add && add_flow && (fg_id_FLOW_id != -1))
        {
