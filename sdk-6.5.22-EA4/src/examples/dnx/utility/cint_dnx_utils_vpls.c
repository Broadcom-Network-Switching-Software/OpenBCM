    ...
    int unit)
{
    ...
    estimate_encap_size_required = (is_device_or_above(unit, JERICHO2)) ?
            *dnxc_data_get(unit, "dev_init", "general", "egress_estimated_bta_btr_config", NULL) : 0;
    ...
    mpls_encap_tunnel[0].tunnel_id = 16386;
    ...
    mpls_tunnel_initiator_create_s * mpls_tunnel)
{
    ...
    estimate_encap_size_required = 0;
...
vpls_mpls_next_layer_network_domain_value(int unit, int next_layer_network_domain)
    ...
    int arr_prefix_value = *(dnxc_data_get(unit, "dnx2_arr", "prefix", "IRPP_IN_LIF_FORMATS___prefix_11", NULL));
