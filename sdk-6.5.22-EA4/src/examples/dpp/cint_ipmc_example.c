...
/* Add an IPv4 MC entry with VRF */
int add_ip4mc_entry_with_vrf(int unit, bcm_ip_t mc_ip, bcm_ip_t src_ip, int vrf, int vlan, bcm_multicast_t mc_id){
    bcm_error_t rv = BCM_E_NONE;

    rv = add_ip4mc_prefix_entry_with_vrf(unit, mc_ip, src_ip, vrf, vlan, mc_id, 0xFFFFFFFF, 0xFFFFFFFF);
    if (rv != BCM_E_NONE) {
        printf("Error, in ipmc_add, mc_ip $mc_ip src_ip $src_ip\n");
        return rv;
    }
...
/* Add an IPv4 MC entry with an LPM s_ip_mask, with VRF */
int add_ip4mc_prefix_entry_with_vrf(int unit, bcm_ip_t mc_ip, bcm_ip_t src_ip, int vrf, int vlan, bcm_multicast_t mc_id, bcm_ip_t s_ip_mask, bcm_ip_t mc_ip_mask){
    bcm_error_t rv = BCM_E_NONE;
    bcm_ipmc_addr_t data;
    int flags =  BCM_IPMC_SOURCE_PORT_NOCHECK;
...
int ipv4_mc_example(int unit){
	...
	bcm_ip_t src_ip = 0xC0800101; /* 192.128.1.1 */
	int ipmc_index = 6000;
    bcm_mac_t mac_l3_ingress = {0};

    mac_l3_ingress[4] = mac4;
    mac_l3_ingress[5] = mac5;
	...
	/* Create the IP MC entry */
	rv = add_ip4mc_entry_with_vrf(unit, mc_ip, src_ip, intf.vrf, vlan, mc_id);
	if (rv != BCM_E_NONE) return rv;
