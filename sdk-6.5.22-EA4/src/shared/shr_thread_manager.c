...
typedef struct shr_thread_manager_object_s {
    ...
    volatile int is_active;                         /* Indicator if the thread is active */
...
static void shr_thread_manager_thread(shr_thread_manager_object_t* obj)
{
    ...
    /* we need this seperate indicator as a volatile indicator as we cannot change sal_thread_t to be volatile pointer */
    obj->is_active = 0;

    /* in case thread is RUN_TO_COMPLETION, we destroy it but keep the handle alive. If thread is joinable destruction is done at the join() function */
...
int shr_thread_manager_create(int unit, const shr_thread_manager_config_t* config, shr_thread_manager_handler_t* h)
{
    ...
    *h = NULL;
    ...
    obj->unit = unit;
    sal_strncpy(obj->name, config->name, SHR_THREAD_MANAGER_MAX_NAME_SIZE-1);
...
static int is_thread_created(shr_thread_manager_handler_t h) 
{
    ...
    /* The obj->is_active is used to avoid compiler's optimizations that may cause obj->tid
       to have a non up to date value if it was set by a different thread than the current thread */
    if(obj == NULL || obj->tid == SAL_THREAD_ERROR || obj->is_active == 0) {
...
shr_thread_manager_info_data_get(int unit, int* num_of_elements, shr_thread_manager_info_t *info)
{
    ...
    if (thread_lock[unit]) {
        ...
        while (shr_thread_manager_control)
        {
            ...
            {
                sal_strncpy((info[idx]).name, shr_thread_manager_control->name, SHR_THREAD_MANAGER_MAX_NAME_SIZE-1);
