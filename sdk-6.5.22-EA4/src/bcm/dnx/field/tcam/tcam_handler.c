    ...
    uint32 entry_id)
{
    ...
    dnx_field_tcam_access_fg_params_t fg_params;
    uint32 absolute_location;
    dnx_field_tcam_core_e core = DNX_FIELD_ENTRY_TCAM_INTERNAL_DT_CORE_ID(entry_id);
    dnx_field_tcam_core_e core_iter;
    uint32 entries_left = 0;
    ...
    for (core_iter = 0; core_iter < dnx_data_device.general.nof_cores_get(unit); core_iter++)
    {
        if (core == DNX_FIELD_TCAM_CORE_ALL || core == core_iter)
        {
             ...
             * *NOTE* That this procedure does not clear out the entry from HW, but rather
             * removes the entry from our SW database (SWSTATE)
             */
    ...
    /*
     * When no entries left on both cores for AUTO BAM (DT included), remove bank from handler's allocated banks list 
     */
    if (fg_params.bank_allocation_mode == DNX_FIELD_TCAM_BANK_ALLOCATION_MODE_AUTO && entries_left == 0)
    {
        int bank_to_evac_id[1];
        bank_to_evac_id[0] = location.bank_id;
        SHR_IF_ERR_EXIT(dnx_field_tcam_defrag_bank_evacuate(unit, handler_id, 1, bank_to_evac_id));
    }
...
dnx_field_tcam_handler_entry_location(
    ...
    dnx_field_tcam_location_t * location)
{
    dnx_field_tcam_core_e ent_core = DNX_FIELD_ENTRY_TCAM_INTERNAL_DT_CORE_ID(entry_id);
    SHR_FUNC_INIT_VARS(unit);
...
dnx_field_tcam_handler_entry_handler(
    ...
    uint32 *handler_id)
{
    int core = DNX_FIELD_ENTRY_TCAM_INTERNAL_DT_CORE_ID(entry_id);
    dnx_field_tcam_location_t location;
    SHR_FUNC_INIT_VARS(unit);
