    ...
    bcm_gport_t profile_gport)
{
    ...
    {
        profile_id = BCM_GPORT_LATENCY_END_TO_END_AQM_LATENCY_PROFILE_GET(profile_gport);
        SHR_IF_ERR_EXIT(dnx_cosq_latency_db.end_to_end_aqm_latency_res.free_single(unit, profile_id, NULL));
    }
    else
    {
...
*  probabilistic latency based marking cni/drop mechanism
*
* \param [in] unit -unit id
    ...
    int arg)
{
    ...
    switch (type)
    {
        case bcmCosqControlLatencyEgressAqmFlowId:
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get(unit, logical_port, &core_id, &out_tm_port));
    ...
    bcm_gport_t gport)
{
    ...
    if (BCM_GPORT_IS_LATENCY_AQM_FLOW_ID(gport))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_cosq_latency_db.aqm_flow_id_res.free_single(unit, flow_id, NULL));
    }
    else
    {
