 ...
 * \param [out] esb_intlv_queue_id - additional ESB queue ID for TDM interleave
 *
 * \return
 *   shr_error_e
 * \remark
    ...
    */
    ...
    else
    {
        if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
        {
             ...
             */
            if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, interface_type) && interface_offset)
            {
                if_id = dnx_data_egr_queuing.params.egr_if_txi_rcy_get(unit);
            }
        }
        else
        {
