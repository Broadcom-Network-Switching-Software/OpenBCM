    ...
    if (BCM_GPORT_IS_TRUNK(port))
    {
        ...
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Direction BOTH (in and out) API is only available for ETH, RAW, INJECTED and MPLS_RAW header type trunk gports. Other types are non-directional. Please use bcm_switch_control_indexed_port_get API with direction IN or OUT instead. \n");
        }
    }
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Direction BOTH (in and out) API is only available for ETH, RAW, TM, INJECTED and MPLS_RAW header types. Other types are non-directional. Please use bcm_switch_control_indexed_port_get API with direction IN or OUT instead. \n");
        }
    ...
    int switch_header_type)
{
    ...
    if (((direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_BOTH) || (direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_OUT))
        && (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_TM))
    {
        if (!dnx_data_device.general.feature_get(unit, dnx_data_device_general_otmh_supported))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmSwitchHashLayersDisable:
        {
            ...
            /*
             * Used as a WA to test only the MACSEC PP without enabling the NIF MACSEC option.
             */
    ...
    int arg)
{
    ...
    if (dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_routed_learning))
    {
    ...
    int trap_id)
{
    ...
    if (is_default_image)
    {
        table_id = DBAL_TABLE_PEMLA_FWD1_DEFAULT_FORWARD_RESULT;
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id, &entry_handle_id));
        SHR_IF_ERR_EXIT(dbal_tables_next_field_get(unit, table_id, field_id, 0, 0, &field_id));
        while (field_id != DBAL_FIELD_EMPTY)
        {
            if ((field_id == DBAL_FIELD_FWD1___MPLS___FWD) || (field_id == DBAL_FIELD_FWD1___MPLS___FWD_PER_INTERFACE))
            {
                SHR_IF_ERR_EXIT(dbal_tables_next_field_get(unit, table_id, field_id, 0, 0, &field_id));
                continue;
            }
            dbal_entry_value_field32_set(unit, entry_handle_id, field_id, INST_SINGLE, trap_dest_w_default_bit);
            SHR_IF_ERR_EXIT(dbal_tables_next_field_get(unit, table_id, field_id, 0, 0, &field_id));
        }
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

        /** Iterate over the result fields of FWD1 table and set the trap ID as destination */
        table_id = DBAL_TABLE_PEMLA_FWD2_DEFAULT_FORWARD_RESULT;
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id, &entry_handle_id));
        SHR_IF_ERR_EXIT(dbal_tables_next_field_get(unit, table_id, field_id, 0, 0, &field_id));
        while (field_id != DBAL_FIELD_EMPTY)
        {
    ...
    if (is_default_image)
    {
        ...
        while (field_id != DBAL_FIELD_EMPTY)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, field_id, INST_SINGLE, &trap_dest_w_default_bit));
...
*  attempting to modify it's FHEI size.
* \param [in] unit  -
*   Relevant unit.
* \param [in] last_ivec  -
    ...
    uint32 fhei_size)
{
    ...
    /*
     * Verify that the last IVEC isn't allocated
     */
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_switch_control_last_ivec_fhei_size_set_verify(unit, last_ivec, fhei_size));
