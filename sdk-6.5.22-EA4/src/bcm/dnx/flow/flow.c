    ...
    int *sip_profile)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.allocate_single
                    (unit, 0, &source_address_entry, NULL, sip_profile, &is_first_sip_ref));
    ...
    for (field = 0; field < special_fields->actual_nof_special_fields; field++)
    {
        if (special_fields->special_fields[field].field_id == DBAL_FIELD_IPV4_SIP)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_flow_initiator_sip_create
                            (unit, special_fields->special_fields[field].shr_var_uint32, &sip_profile));
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SOURCE_IDX, INST_SINGLE, sip_profile);
        }
        else if (special_fields->special_fields[field].field_id == DBAL_FIELD_BITSTR)
        {
            dbal_entry_value_field_arr32_set(unit, entry_handle_id, DBAL_FIELD_BITSTR, INST_SINGLE,
                                             special_fields->special_fields[field].shr_var_uint32_arr);
        }
        else
        {
        ...
        if (!special_fields->special_fields[field].is_clear)
        {
            if (special_fields->special_fields[field].field_id == DBAL_FIELD_IPV4_SIP)
            {
                SHR_IF_ERR_EXIT(dnx_flow_initiator_sip_replace
                                (unit, special_fields->special_fields[field].shr_var_uint32, entry_handle_id));
            }
            else if (special_fields->special_fields[field].field_id == DBAL_FIELD_BITSTR)
            {
                dbal_entry_value_field_arr32_set(unit, entry_handle_id, DBAL_FIELD_BITSTR, INST_SINGLE,
    ...
    if (flow_app_info->special_fields[0] != DBAL_FIELD_EMPTY)
    {
        ...
        for (field = 0; field < num_special_fields_in_app; field++)
        {
            if (flow_app_info->special_fields[field] == DBAL_FIELD_IPV4_SIP)
            {
                ...
                if (rv == _SHR_E_NONE)
                {
                    SHR_IF_ERR_EXIT(dnx_flow_initiator_sip_get
                                    (unit, value, &special_fields->special_fields[idx].shr_var_uint32));
                }
            }
            else if (flow_app_info->special_fields[field] == DBAL_FIELD_BITSTR)
            {

                rv = dbal_entry_handle_value_field_arr32_get
                    (unit, entry_handle_id, DBAL_FIELD_BITSTR, INST_SINGLE,
            ...
            if (dnx_flow_term_common_fields[field]->field_cbs.field_replace_cb)
            {
                SHR_IF_ERR_EXIT(dnx_flow_term_common_fields[field]->
                                field_cbs.field_replace_cb(unit, entry_handle_id, get_entry_handle_id,
                                                           FLOW_APP_TYPE_TERM, flow_handle_info, gport_hw_resources,
                                                           terminator_info));
            }
    ...
    for (field = 0; field < num_common_fields; field++)
    {
        ...
        if (_SHR_IS_FLAG_SET(initiator_info->valid_elements_set, (1 << field)))
        {
            ...
            if (dnx_flow_init_common_fields[field]->field_cbs.field_replace_cb)
            {
                SHR_IF_ERR_EXIT(dnx_flow_init_common_fields[field]->
                                field_cbs.field_replace_cb(unit, entry_handle_id, get_entry_handle_id,
                                                           FLOW_APP_TYPE_INIT, flow_handle_info, gport_hw_resources,
                                                           initiator_info));
            }
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    {
        if (_SHR_IS_FLAG_SET(flow_app_info->valid_common_fields_bitmap, (1 << field))
            && (dnx_flow_term_common_fields[field]->field_cbs.field_get_cb))
        {
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    {
        if (_SHR_IS_FLAG_SET(flow_app_info->valid_common_fields_bitmap, (1 << field))
            && (dnx_flow_init_common_fields[field]->field_cbs.field_get_cb))
        {
            ...
            for (mask_counter = 0; mask_counter < nof_mask_fields; mask_counter++)
            {
                ...
                if (FLOW_FIELD_IS_MASK_MATCHING(maskable_field_id, mask_field_id))
                {
                    dbal_entry_key_field32_masked_set(unit, entry_handle_id, maskable_field_id,
                                                      ...
                                                      key_special_fields->
                                                      special_fields[mask_fields_pos[mask_counter]].shr_var_uint32);
                    break;
                }
            }
    ...
    bcm_flow_special_fields_t * key_special_fields)
{
    ...
    /** setting the core_id to default (get require to be done on only one of the cores) */
    ...
    bcm_flow_handle_info_t * flow_handle_info)
{
    ...
    {
        if (_SHR_IS_FLAG_SET(flow_app_info->valid_common_fields_bitmap, (1 << field))
            && (dnx_flow_term_common_fields[field]->field_cbs.field_delete_cb))
        {
    ...
    bcm_flow_handle_info_t * flow_handle_info)
{
    ...
    {
        if (_SHR_IS_FLAG_SET(flow_app_info->valid_common_fields_bitmap, (1 << field))
            && (dnx_flow_init_common_fields[field]->field_cbs.field_delete_cb))
        {
