...
dnx_multicast_linklist_create(
    ...
    bcm_multicast_replication_t * rep_array)
{
    uint32 is_belong_to_core, rep_id;
    bcm_core_t core_id;
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        /** get rep count per core */
        if (DNX_MULTICAST_IS_INGRESS(flags))
        {
            /** for ingress there's no core differentiation */
    ...
    SHR_IF_ERR_EXIT(multicast_db.mcdb.nof_free_elements_get(unit, &nof_free_elements));
    if (nof_free_elements < core_rep_count[0] + core_rep_count[1]) /** this check is faulty for future devices with more than 2 cores */
    {
