 ...
 */
#define MPLS_TUNNEL_ENCAP_SUPPORTED_FLAGS_2 (BCM_MPLS_EGRESS_LABEL_FLAGS2_SVTAG_ENABLE | BCM_MPLS_EGRESS_LABEL_FLAGS2_ESI_INTF_NAMESPACE)
/*
 * }
 */
/*
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    for (ii = 0; ii < num_labels; ii++)
    {
        ...
        if (ii > 0)
        {
            ...
            if (encap_access != label_array[ii].encap_access)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "encap_access are not aligned: label_array[0].encap_access (%d) != label_array[%d].encap_access (%d)",
                             encap_access, ii, label_array[ii].encap_access);
            }
        }
    }
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    SHR_IF_ERR_EXIT(dnx_mpls_tunnel_encap_lif_allocate
                    (unit, num_labels, label_array, &out_global_lif, &egr_pointed_id));
    ...
    else
    {
        ...
        *label_count = encap_info.labels_nof;
        ...
        if (gport_hw_resources.global_out_lif != DNX_ALGO_GPM_LIF_INVALID)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_global_out_lif_bta_get
                            (unit, gport_hw_resources.global_out_lif, &label_array[0].estimated_encap_size));
        }
        else
        {
            /*
        ...
        if (encap_info.esem_command_profile != dnx_data_esem.access_cmd.no_action_get(unit))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_switch_svtag_is_present_in_esem_cmd
                            (unit, encap_info.esem_command_profile, &svtag_present));
