    ...
    uint32 first_header_size_to_skip)
{
    ...
    ffc_instruction_5 =
        BUILD_PRT_FFC(prt_configuration->ffc_5_offset, prt_configuration->ffc_5_width, prt_ffc_width_start_offset,
                      ffc_5_input_offset, prt_ffc_start_input_offset);
    ...
    int first_header_size_to_skip_value)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Trunk gport %d is not supported! The configuration should be per trunk member!\n",
                     port);
    ...
    int *layer_offset)
{
    ...
    /** Get layer_offset */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PRT_INFO, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PRT_RECYCLE_PROFILE, prt_recycle_cmd);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PORT_TERMINATION_PTC_PROFILE, ptc_profile);
    ...
    int value)
{
    ...
    switch (type)
    {
        ...
        {
            SHR_RANGE_VERIFY(value, 0, 1, _SHR_E_PARAM, "Value should be 0 or 1");
            SHR_IF_ERR_EXIT(dnx_port_pp_lif_speculative_parsing_set(unit, port, value));
            break;
        }
        default:
        {
    ...
    int *value_p)
{
    ...
    switch (type)
    {
        ...
        case bcmPortControlMplsSpeculativeParse:
        {
            SHR_IF_ERR_EXIT(dnx_port_pp_lif_speculative_parsing_get(unit, port, value_p));
            break;
        }
        default:
        {
 ...
 */
shr_error_e
dnx_port_pp_dsp_to_out_tm_mapping_get(
    int unit,
    int core,
    int pp_dsp,
    int *out_tm_port)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_OUT_TM_PORT, INST_SINGLE, (uint32 *) out_tm_port);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    uint32 is_lag)
{
    ...
    {
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_in_tm_port_get(unit, logical_port, &core_id, &ptc));

            /** Provide mapping between source system port and PTC */
            SHR_IF_ERR_EXIT(dnx_port_pp_ptc_to_src_system_port_set(unit, core_id, ptc, system_port));
        }
    }
