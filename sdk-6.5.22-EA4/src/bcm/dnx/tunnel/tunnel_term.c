...
#define DNX_TUNNEL_TERM_IS_SRV6_WITH_GENERALIZED_SEGMENT_ID_EOC(_info)  (_SHR_IS_FLAG_SET(_info->flags, BCM_TUNNEL_TERM_GENERALIZED_SEGMENT_ID_END_OF_COMPRESSION))
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    if ((info->vlan != 0) && (!tunnel_type_is_l2vpn && (info->type != bcmTunnelTypeCascadedFunct)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "vlan (%d) is supported only for tunnel types bcmTunnelTypeEthSR6, bcmTunnelTypeEthIn6 and bcmTunnelTypeCascadedFunct",
                     info->vlan);
    }
...
dnx_tunnel_terminator_srv6_funct_nof_bits(
    int unit,
    bcm_gport_t default_tunnel_id,
    int *funct_nof_bits)
{
    dbal_enum_value_field_srv6_sid_formats_e sid_format;
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_gport_to_sid_format_get(unit, default_tunnel_id, &sid_format));
    ...
    dnx_tunnel_term_range_t * expected_unmasked_range)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_gport_to_locator_nof_bits(unit, default_tunnel_id, &locator_nof_bits));
    expected_unmasked_range->start_range = DNX_TUNNEL_TERM_SRV6_SID_ARG_START_BIT;
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_funct_nof_bits(unit, default_tunnel_id, &funct_nof_bits));
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_funct_dip6_mask_expected_unmasked_lsb_range
                    (unit, info->default_tunnel_id, &expected_unmasked_ranges[0]));
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_funct_dip6_mask_expected_unmasked_msb_range
                    (unit, info->default_tunnel_id, &expected_unmasked_ranges[1]));
    ...
    dnx_tunnel_term_range_t * expected_mask_range)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_funct_nof_bits(unit, info->default_tunnel_id, &funct_nof_bits));
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    /** that GSID End-of-Compression flag doesn't come without is GSID flag */
    if (DNX_TUNNEL_TERM_IS_SRV6_WITH_GENERALIZED_SEGMENT_ID_EOC(info) &&
        !DNX_TUNNEL_TERM_IS_SRV6_WITH_GENERALIZED_SEGMENT_ID(info))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "BCM_TUNNEL_TERM_GENERALIZED_SEGMENT_ID_END_OF_COMPRESSION flag cannot exist without BCM_TUNNEL_TERM_GENERALIZED_SEGMENT_ID");
    }
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_tunnel_terminator_key_verify(unit, info));

    /** verification of lif parameters integrity */
    SHR_INVOKE_VERIFY_DNX(dnx_tunnel_terminator_lif_verify(unit, info));

    /** verification of unsupported parameters integrity */
    SHR_INVOKE_VERIFY_DNX(dnx_tunnel_terminator_unsupported_params_verify(unit, info));
    ...
    bcm_vlan_t default_vsi)
{
    ...
    else if (((info->type == bcmTunnelTypeVxlan) || (info->type == bcmTunnelTypeVxlan6)) && (default_vsi))
    {
    ...
    uint32 *vrf)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_get_inlif_info(unit, _SHR_CORE_ALL, local_inlif, entry_handle_id, &inlif_info));
    {
        ...
        *ecn_mapping_profile = dbal_field[0];
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_optional_vsi_get(unit, *fodo, default_vsi));
    }
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                        (unit, gport_hw_resources.local_in_lif,
                         (uint32 *) &global_lif, &propag_profile, (uint32 *) &old_in_lif_profile, &ecn_mapping_profile,
                         &flags, &fodo, &default_vsi, &vrf));
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    /** VERIFICATION of input parameters */
    SHR_INVOKE_VERIFY_DNX(dnx_tunnel_terminator_get_delete_verify(unit, info));
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                    (unit, local_inlif, &global_inlif, &propagation_prof, &in_lif_profile, &ecn_mapping_profile,
                     &(info->flags), &fodo, &default_vsi, &vrf));

    /** VSI could be set only with tunnel types of bcmTunnelTypeEthSR6, bcmTunnelTypeEthIn6 and bcmTunnelTypeCascadedFunct*/
     ...
     */
    if (DNX_TUNNEL_TERM_IS_SRV6_WITH_GENERALIZED_SEGMENT_ID(info))
    {
        in_lif_profile_info_t in_lif_profile_info;
        SHR_IF_ERR_EXIT(in_lif_profile_info_t_init(unit, &in_lif_profile_info));
        ...
        {
            info->flags |= BCM_TUNNEL_TERM_GENERALIZED_SEGMENT_ID_END_OF_COMPRESSION;
        }
    }
...
shr_error_e
dnx_tunnel_terminator_get_ipv6_as_funct_from_traversed_entry(
    int unit,
    uint32 funct,
    bcm_gport_t default_tunnel_id,
    bcm_ip6_t * dip6)
{
    dbal_enum_value_field_srv6_sid_formats_e sid_format;
    int funct_nof_bits, funct_start_bit, locator_nof_bits;
    uint32 dip6_long[UTILEX_PP_IPV6_ADDRESS_NOF_UINT32S];

    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_gport_to_sid_format_get(unit, default_tunnel_id, &sid_format));
    /*
     * get funct lsb and nof bits 
     */
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_gport_to_locator_nof_bits(unit, default_tunnel_id, &locator_nof_bits));
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_sid_format_to_funct_nof_bits(unit, sid_format, &funct_nof_bits));
    funct_start_bit = DNX_TUNNEL_TERM_SRV6_SID_FUNCT_START_BIT(locator_nof_bits, funct_nof_bits);
    /*
     * update dip6 
     */
    sal_memset(dip6_long, 0, sizeof(dip6_long));
    SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(&funct, funct_start_bit, funct_nof_bits, dip6_long));
    ...
    void *user_data)
{
    ...
    uint32 dbal_tables[] =
        ...
        DBAL_TABLE_IPVX_TT_IPSEC, DBAL_TABLE_IPV6_TT_MP_EM_16
    };
    ...
    for (table = 0; table < nof_tables; table++)
    {
        ...
        while (!is_end)
        {
                            ...
                            DNX_TUNNEL_TERM_LOCAL_IN_LIF_ENCODING))
                    {
                        ...
                        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                                        (unit, local_inlif_p2p, &global_inlif, &propagation_prof, &in_lif_profile,
                                         &ecn_mapping_profile, &flags, &fodo, &dummy_vsi, &dummy_vrf));
                        break;
                    }
                    SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id_internal, &is_end_ipv6));
                }
            ...
            else if (dbal_tables[table] == DBAL_TABLE_IPV6_TT_MP_EM_16)
            {
                uint32 funct = 0, dip_idx_data = 0;
                ...
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_dip_idx_data_to_gport
                                (unit, dip_idx_data, &info.default_tunnel_id));
                /*
                 * get funct bits in dip6 
                 */
                SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                                (unit, entry_handle_id, DBAL_FIELD_IPV6_ADDRESS_16, &funct));
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_get_ipv6_as_funct_from_traversed_entry
                                (unit, funct, info.default_tunnel_id, &info.dip6));
                ...
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                                (unit, local_inlif, &global_inlif, &propagation_prof, &in_lif_profile,
                                 &ecn_mapping_profile, &info.flags, &fodo, &dummy_vsi, &dummy_vrf));
                info.type = bcmTunnelTypeCascadedFunct;
            }
