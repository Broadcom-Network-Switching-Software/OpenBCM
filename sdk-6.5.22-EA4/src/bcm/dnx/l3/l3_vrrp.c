    ...
    int group_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_value_get(unit, DBAL_FIELD_LAYER_TYPES, protocol_layer, &protocol_layer_enum));
    ...
    uint32 *members_bmp)
{
    ...
    /*
     * input bitmap is 32 bits size
     * can access only types 0-31
     */
    int bitmap_size = UTILEX_MIN(DBAL_NOF_ENUM_LAYER_TYPES_VALUES, 32);
...
/**
* \brief
*   function set my mac vrid vmv shift value (according to sexem1 phy db vmv)
* \par DIRECT INPUT:
...
*/
static shr_error_e
dnx_l3_vrrp_my_mac_sexem1_vmv_shift_set(
    int unit)
{
    uint32 entry_handle_id;
    SHR_FUNC_INIT_VARS(unit);
    ...
    /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_L3_MY_MAC_VRID_VMV_SHIFT_VAL, &entry_handle_id));
    /** value must match the vmv_size written in sexem1 phyDB */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VMV_SHIFT_VAL, INST_SINGLE, 2);
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_vrrp_my_mac_sexem1_vmv_shift_set(unit));
