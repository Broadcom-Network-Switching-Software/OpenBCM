    ...
    int is_single_hop_with_random_dip)
{
    ...
    if (mep_entry_values->mep_type != DBAL_ENUM_FVAL_OAMP_MEP_TYPE_BFD_PWE)
    {
        ...
        {
            /** Seamless BFD */
            SHR_IF_ERR_EXIT(algo_bfd_db.bfd_oamp_v1_seamless_udp_sport.free_single(unit, 0, &dummy));
        }
        else
        {
    ...
    dnx_bfd_oamp_v1_temp_mep_db_data_t * mep_hw_write_data)
{
    ...
    {
        /** Seamless BFD */
        rv = algo_bfd_db.bfd_oamp_v1_seamless_udp_sport.allocate_single(unit, NO_FLAGS, &param_port, NULL, &dummy,
                                                                        &write_hw);
    }
    else
    {
    ...
    if (rv == _SHR_E_RESOURCE)
    {
        ...
        if (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_INITIATOR))
        {
            /** Seamless BFD */
            SHR_IF_ERR_EXIT(algo_bfd_db.
                            bfd_oamp_v1_seamless_udp_sport.profile_data_get(unit, DISCR_PROF, &dummy, &actual_port));
        }
        else
        {
            if (endpoint_info->type != bcmBFDTunnelTypeUdp)
    ...
    uint8 current_profile)
{
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_UPDATE) && (endpoint_info->bfd_period_cluster != 0))
    {
         ...
         * will be modified
         */
        SHR_IF_ERR_EXIT(algo_bfd_db.bfd_oamp_v1_tx_rate.profile_data_get(unit, current_profile, &dummy, &tx_rate_data));

        tx_rate_data.tx_period = new_tx_rate;
        SHR_IF_ERR_EXIT(algo_bfd_db.bfd_oamp_v1_tx_rate.replace_data(unit, current_profile, &tx_rate_data));
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_INITIATOR))
    {
        /** Seamless BFD */
        SHR_IF_ERR_EXIT(algo_bfd_db.
                        bfd_oamp_v1_seamless_udp_sport.profile_data_get(unit, DISCR_PROF, &dummy, &udp_port));
    }
    else
    {
        if (endpoint_info->type != bcmBFDTunnelTypeUdp)
    ...
    dnx_bfd_oamp_v1_temp_mep_db_data_t * mep_hw_write_data)
{
    ...
    if ((new_endpoint_info->bfd_period != existing_endpoint_info->bfd_period) &&
        (BCM_OAMP_SW_ENTRY_INFO_PERIOD_TO_MEP_DB_SCAN_RATE(unit, new_endpoint_info->bfd_period) !=
         BCM_OAMP_SW_ENTRY_INFO_PERIOD_TO_MEP_DB_SCAN_RATE(unit, existing_endpoint_info->bfd_period)))
    {
        ...
        if (mep_hw_write_data->tx_rate_write_index != mep_entry_values->tx_rate_profile)
        {
            ...
            SHR_IF_ERR_EXIT(algo_bfd_db.bfd_oamp_v1_tx_rate.free_single(unit, mep_entry_values->tx_rate_profile,
    ...
    uint8 *ipv6_extra_data_header)
{
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_INITIATOR))
    {
        ...
        SHR_IF_ERR_EXIT(algo_bfd_db.bfd_oamp_v1_seamless_udp_sport.profile_data_get(unit, DISCR_PROF, &dummy, &port));
        udp_src_port = port;
    }
    else
    {
