...
/*
 * Defines the value of preserve DSCP per inlif and outlif
 */
    ...
    dnx_in_lif_profile_last_info_t * last_profile)
{
    ...
    last_profile->old_cs_in_lif_profile = DNX_IN_LIF_PROFILE_CS_DEFAULT;
    last_profile->old_cs_in_lif_fwd_profile = DNX_IN_LIF_FWD_PROFILE_CS_DEFAULT;
    ...
    int unit)
{
    ...
    /**Decide the number of egres bits to support*/
    ...
    *l3_ingress_flags = 0;
    if ((in_lif_profile_template_data.lif_type == LIF) || (in_lif_profile_template_data.lif_type == ETH_RIF))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                        (unit, DBAL_FIELD_ROUTING_INLIF_CS_PROFILE, DBAL_FIELD_NAT_DIRECTION_OR_VIP_MODE_OR_SRV6_GSID,
                         &nat_or_virtual_ip_or_srv6_gsid, &cs_in_lif_proifile));
        SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
    ...
    in_lif_profile_info_t * in_lif_profile_info)
{
    ...
    {
        int offsets_to_enable[2 * NOF_ROUTING_ENABLE_TYPES];
        int offset_index = 0;
        offsets_to_enable[offset_index++] = dnx_init_pp_layer_types_value_get(unit, DBAL_ENUM_FVAL_LAYER_TYPES_UDP) *
            NOF_ROUTING_ENABLE_TYPES + ROUTING_ENABLE_UC;
        offsets_to_enable[offset_index++] = dnx_init_pp_layer_types_value_get(unit, DBAL_ENUM_FVAL_LAYER_TYPES_UDP) *
            NOF_ROUTING_ENABLE_TYPES + ROUTING_ENABLE_MC;
        offsets_to_enable[offset_index++] = dnx_init_pp_layer_types_value_get(unit, DBAL_ENUM_FVAL_LAYER_TYPES_TCP) *
            NOF_ROUTING_ENABLE_TYPES + ROUTING_ENABLE_UC;
        offsets_to_enable[offset_index++] = dnx_init_pp_layer_types_value_get(unit, DBAL_ENUM_FVAL_LAYER_TYPES_TCP) *
            NOF_ROUTING_ENABLE_TYPES + ROUTING_ENABLE_MC;

        for (offset_index = 0; offset_index < 2 * NOF_ROUTING_ENABLE_TYPES; offset_index++)
        {
            uint32 index_32 = offsets_to_enable[offset_index] / SAL_UINT32_NOF_BITS;
            uint32 offset_32 = offsets_to_enable[offset_index] % SAL_UINT32_NOF_BITS;
            routing_enablers_vector[index_32] |= SAL_BIT(offset_32);
        }
    }
exit:
 ...
 *   the concluded cs_in_lif_profile flags
    ...
    in_lif_intf_type_e * lif_type)
{
    ...
    *cs_in_lif_profile_flags = in_lif_profile_info->ingress_fields.cs_in_lif_profile_flags;
        ...
        (inlif_dbal_table_id == DBAL_TABLE_IN_LIF_FORMAT_LSP) || (inlif_dbal_table_id == DBAL_TABLE_ING_VSI_INFO_DB))
    {
        ...
        *lif_type = LIF;
            ...
            (in_lif_profile_info->ingress_fields.l3_ingress_flags & BCM_L3_INGRESS_FIELD_COMPRESSION_ENABLE))
        {
            if (in_lif_profile_info->ingress_fields.l3_ingress_flags & BCM_L3_INGRESS_MPLS_INTF_NAMESPACE)
            {
                *cs_in_lif_profile_flags |= DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_MPLS_NAMESPACE;
            }
            if (in_lif_profile_info->ingress_fields.l3_ingress_flags & BCM_L3_INGRESS_FIELD_COMPRESSION_ENABLE)
            {
                *cs_in_lif_profile_flags |= DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_AACL;
            }
        ...
        if (in_lif_profile_info->ingress_fields.l3_ingress_flags & BCM_L3_INGRESS_VIP_ENABLED)
        {
            *cs_in_lif_profile_flags |= DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_VIRTUAL_IP;
            nat_or_virtual_ip_or_srv6_gsid =
                DBAL_ENUM_FVAL_LIF_VTT5_CSP_1BIT_NAT_DIRECTION_AND_VIRTUAL_IP_AND_SRV6_GSID_NAT_DOWNSTREAM_VIP_ENABLED_SRV6_GSID_EOC;
        }
        else
...
dnx_in_lif_profile_destroy(
    ...
    in_lif_intf_type_e intf_type)
{
    int pmf_reserved_nof_bits, egress_reserved_nof_bits, indexed_mode_nof_bits,
        eth_rif_profile_used_bits_mask, in_lif_profile_egress_reserved_start_bit, in_lif_profile_used_bits_mask;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_table_init
                        (unit, last_profile.old_in_lif_profile, TRUE, pmf_reserved_nof_bits, egress_reserved_nof_bits,
                         indexed_mode_nof_bits, intf_type));
        SHR_IF_ERR_EXIT(dnx_vsi_profile_table_init
                        (unit, last_profile.old_in_lif_profile, TRUE, pmf_reserved_nof_bits, egress_reserved_nof_bits,
                         intf_type));
    ...
    dnx_in_lif_profile_last_info_t * last_profile)
{
    ...
    uint32 entry_handle_id, cs_in_lif_profile_flags;
    ...
    uint8 cs_in_lif_profile_value, rpf_mode, is_first, routing_profile_is_first, routing_profile_is_last,
        cs_in_lif_fwd_profile_value, native_indexed_mode_in_lif_profile, lif_lb_enablers_profile_first,
        lif_lb_enablers_profile_last, flood_profile_is_last, lif_profile_is_last;
    in_lif_intf_type_e lif_type;
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_algo_convert_to_cs_in_lif_profile
                    (unit, intf_type, inlif_dbal_table_id, in_lif_profile_info, &cs_in_lif_profile_value,
                     &cs_in_lif_profile_flags, &cs_in_lif_fwd_profile_value, &lif_type));
    /*
    ...
    in_lif_profile_template_data.cs_in_lif_profile_flags = cs_in_lif_profile_flags;
    ...
    in_lif_intf_type_e intf_type)
{
    ...
    in_lif_profile_info->ingress_fields.cs_in_lif_profile_flags = in_lif_profile_template_data.cs_in_lif_profile_flags;
    /*
     * assign RPF mode
     */
     ...
     */
    {
        int offset_to_check = dnx_init_pp_layer_types_value_get(unit, DBAL_ENUM_FVAL_LAYER_TYPES_UDP) *
            NOF_ROUTING_ENABLE_TYPES + ROUTING_ENABLE_UC;
