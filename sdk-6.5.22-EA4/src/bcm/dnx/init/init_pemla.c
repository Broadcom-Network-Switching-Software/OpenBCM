    ...
    char *internal_build_release)
{
    ...
    strncpy(internal_build_release, &build_release[begin_from_id], (end_at_id - begin_from_id));
        ...
        line_no++;
        {
            ...
            else if (read_sdk_version)
            {
                ...
                else
                {
                    ...
                    if (strncmp(internal_sdk_version_value, internal_build_release, sizeof(internal_build_release)) !=
                        0)
                    {
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEM_CONTROLLER, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PEMA_ID, DBAL_ENUM_FVAL_PEMA_ID_VTT1);
