    ...
    for (cmd_id = 0; cmd_id < DNX_INIT_MEM_INDIRECT_CMDS_PER_BLOCK_MAX; cmd_id++)
    {
        for (blk_index = 0; blk_index < SOC_MAX_NUM_BLKS; blk_index++)
        {
            if (nof_cmds[blk_index] > cmd_id)
            {
                ...
                sand_mem_indirect_reset_write(unit, cmd->mem, cmd->blk_id, mem_addr, cmd->nof_entries, entry);
    ...
    int unit)
{
    ...
    if (dnx_data_device.general.device_id_get(unit) == 0x8850)
    {
        ...
        {
            {
                ...
                SHR_IF_ERR_EXIT(READ_CGM_REP_CMD_FIFO_OCCUPANCY_RJCT_THm(unit, SOC_CORE_ALL, 0, &entry));
                if (entry != 0)
                {
