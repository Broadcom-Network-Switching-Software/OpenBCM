...
int set_classifier_action_general(int unit, int mep_id, bcm_oam_endpoint_action_t *action, int opcode_map, SOC_PPC_OAM_MEP_TYPE mep_type) {
    ...
    BCMDNX_IF_ERR_EXIT(rv);
    rv = bcm_petra_oam_endpoint_action_set_internal(unit, mep_id, action, 2/*ignore*/, 2/*ignore*/, 2/*is_loss_unset:ignore*/);
    BCMDNX_IF_ERR_EXIT(rv);
    BCM_EXIT;
    ...
    while (valid) {
        if (event_type == SOC_PPC_OAM_DMA_EVENT_TYPE_EVENT) {
            ...
            if (!found) {
                ...
                LOG_WARN(BSL_LS_BCM_OAM,
                         (BSL_META_U(unit,
                                     "Warning: Processing event for RMEP ID 0x%x failed. RMEP not found. Continuing.\n"),rmeb_db_ndx));
                soc_sand_rv = soc_ppd_oam_event_fifo_read(unit, event_type, &rmeb_db_ndx_internal, event_id_bitmap, &valid, event_found.event_data, interrupt_data);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                continue;
            }
            ...
            if (!found) {
            	/**
            	 * If the endpoint was destroyed between _bcm_dpp_oam_bfd_rmep_info_db_get() and
            	 * _bcm_dpp_oam_bfd_mep_info_db_get(), found in this case will be 0 as well.
            	 * Continue in this case also
            	 */
                LOG_WARN(BSL_LS_BCM_OAM,
                         (BSL_META_U(unit,
                                                  ...
                                                  void *user_data)
{
    ...
    int non_acc_profile_exchange = 0;
          ...
          if (_BCM_OAM_DISSECT_IS_ENDPOINT_MIP(endpoint_info)) {
             ...
             */
             if (SOC_IS_JERICHO_PLUS(unit) && profile_info->profile_data->is_slm && is_passive) {
                ...
                if ((profile != old_profile) && (non_acc_profile_exchange == 1)) {
...
STATIC int _bcm_oam_endpoint_local_create(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl)
{
    ...
    if(SOC_DPP_CONFIG(unit)->pp.oam_ccm_rx_wo_tx){
        ...
        if (!endpoint_info_lcl->ccm_period)
        {
            classifier_mep_entry.dip_profile_ndx = endpoint_info_lcl->tx_gport;
        }
    ...
    if ((endpoint_info_lcl->gport == BCM_GPORT_INVALID) &&
        ...
        for (is_passive = 0; is_passive <= 1; is_passive++) {
            ...
            */
            ...
            if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit) ?
               ...
               if (endpoint_info_lcl->action_reference_id != BCM_OAM_ENDPOINT_INVALID) {
                  if (!SOC_IS_JERICHO_PLUS(unit) || (ref_profile != ARAD_PP_SLM_PASSIVE_PROFILE)) {
            ...
            if (endpoint_info_lcl->action_reference_id != BCM_OAM_ENDPOINT_INVALID) {
               profile = ref_profile;
               if (!SOC_IS_JERICHO_PLUS(unit) || (profile != ARAD_PP_SLM_PASSIVE_PROFILE)) {
        ...
        if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) && (updated_mp_type)) {
            soc_sand_rv = soc_ppd_oam_classifier_oem_mep_profile_replace(unit, &classifier_mep_entry, 1 /*update mp-type*/, 0);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
...
STATIC int _bcm_oam_endpoint_remote_replace(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl)
{
    ...
    if (endpoint_info_lcl->name != rmep_info.rmep_id) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
          (_BSL_BCM_MSG("Error:remote endpoint name must equal to rmep_id 0x%x\n"),rmep_info.rmep_id));		
    }
    if (endpoint_info_lcl->loc_clear_threshold < 1) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: loc_clear_threshold must be positive. Current value:%d\n"),
                                endpoint_info_lcl->loc_clear_threshold));
...
int bcm_petra_oam_endpoint_destroy (int unit, bcm_oam_endpoint_t endpoint)
{
    ...
    sal_memset(&mep_del_info, 0, sizeof(SOC_PPC_OAM_MEP_DEL_INFO));
        ...
        /* local endpoint */
            ...
            if (_BCM_OAM_DISSECT_IS_CLASSIFIER_MEP_ACCELERATED(&mep_info)) {
                if (SOC_IS_ARADPLUS(unit)) {
                    ...
                    if (SOC_IS_JERICHO_PLUS(unit) && !SOC_IS_QUX(unit)) {
                        /* is_slm will be cleared when calling bcm_petra_oam_loss_delete. Here getting the info before calling bcm_petra_oam_loss_delete */
                        SOC_PPC_OAM_LIF_PROFILE_DATA_clear(&profile_data);
                        rv = _bcm_dpp_am_template_oam_mep_profile_non_accelerated_mapping_get(unit, endpoint, 0 /*is_passive*/, &old_profile);
                        BCMDNX_IF_ERR_EXIT(rv);

                        if (old_profile != ARAD_PP_SLM_PASSIVE_PROFILE && old_profile != 0) {
                            rv = _bcm_dpp_am_template_oam_mep_profile_non_accelerated_data_get(unit, endpoint, 0/*is_passive*/, &profile_data);
                            BCMDNX_IF_ERR_EXIT(rv);
                            mep_del_info.is_slm |= profile_data.is_slm;
                        }
                        SOC_PPC_OAM_LIF_PROFILE_DATA_clear(&profile_data);
                    }
            ...
            if ((mep_info.lif != _BCM_OAM_INVALID_LIF) ||
                ...
                soc_sand_rv = soc_ppd_oam_classifier_mep_delete(unit, endpoint, &mep_info, &mep_del_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    ...
                    if (!found) { /* No more meps on this lif */
                        ...
                        if (_BCM_OAM_DISSECT_IS_CLASSIFIER_MIP(&mep_info)) {
                            ...
                            if (!found) {
                                ...
                                soc_sand_rv = soc_ppd_oam_classifier_oem_mep_profile_replace(unit, &classifier_mep_entry_dummy, 1 /*update mp-type*/,0);
                                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                            }
                        }
                    }
    ...
    int is_loss_unset)
{
    ...
    if (!is_mip) {
        ...
        profile_data->is_slm = is_slm = (is_loss_unset == 1) ? 0 : ((BCM_OAM_ACTION_GET(*action_lcl, bcmOAMActionSLMEnable) || old_profile_data->is_slm) ? 1 : 0);
        if (is_accelerated) {
            profile_data_acc->is_slm =  (is_loss_unset == 1) ? 0 : ((BCM_OAM_ACTION_GET(*action_lcl, bcmOAMActionSLMEnable) || old_profile_data_acc->is_slm) ? 1 : 0);
             ...
             if (profile_info.is_passive_profile_allocated) {
                ...
                BCMDNX_IF_ERR_EXIT(rv);
                 ...
                 if (classifier_mep_entry.passive_side_lif != _BCM_OAM_INVALID_LIF) {
                     oem1_key.ingress = !profile_info.is_ingress;
                     oem1_key.your_discr = 0;
                     oem1_key.oam_lif = classifier_mep_entry.passive_side_lif;
                     soc_sand_rv = soc_ppd_oam_classifier_oem1_entry_get(unit, &oem1_key, &oem1_payload, &is_found);
                     BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                     if (is_found && (oem1_payload.mp_profile == 0)) {
                         profile_change = 1;
    ...
    if (profile_change) {
        if (id != -1) {
            soc_sand_rv = soc_ppd_oam_classifier_oem_mep_profile_replace(unit, &classifier_mep_entry, 0 /*update mp type*/,SOC_PPD_OAM_MEP_INDEX_IS_SERVER_CLIENT_SIDE(id));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
...
int bcm_petra_oam_endpoint_action_set(
    ...
    rv = bcm_petra_oam_endpoint_action_set_internal(unit,id,action,2/*ignore*/,2/*ignore*/,2/*is_loss_unset:ignore*/);
    BCMDNX_IF_ERR_EXIT(rv);
...
int _manage_DA_with_existing_entries_internal(int unit , int mep_id ,SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep, uint32 msb, uint32 lsb, 
    ...
    uint32 tx_rate = 0, other_tx_rate = 0, rate_index = 0;
    uint32 user_header_size, user_header_0_size, user_header_1_size;
    uint32 user_header_egress_pmf_offset_0_dummy, user_header_egress_pmf_offset_1_dummy;
    ...
    if (_BCM_OAM_BCM_MEP_TYPE_IS_RFC6374(mep_db_entry.mep_type) && eth_1731_profile!=NULL)
    {
        ...
        if (has_other_entry)
        {
            rate_index = is_dm ? old_eth_data.lmm_rate : old_eth_data.dmm_rate;
            rv = BFD_ACCESS.tx_rate_value.get(unit,
                                rate_index,
                                &other_tx_rate);
            BCMDNX_IF_ERR_EXIT(rv);

            if (other_tx_rate > tx_rate && ((other_tx_rate + 1) % (tx_rate + 1) != 0))
            {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG(
                             "Invalid rate passed, the rate should be multiple of already configured ones, but in units of 1.6667.")));
            }
            else if (tx_rate > other_tx_rate && ((tx_rate + 1) % (other_tx_rate + 1) != 0))
            {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG(
                             "Invalid rate passed, the rate should be multiple of already configured ones, but in units of 1.6667.")));
            }
        ...
        if (is_dm)
        {
            ...
            eth_1731_profile->dmm_cnt = lm_dm_period ? (has_other_entry ? 1 : 0) : 14;
            eth_1731_profile->lmm_cnt = old_eth_data.lmm_cnt;
        } 
        else
        {
            ...
            /** Set the tx phase to bigger value to disable the transmission, any value bigger then 0 will work */
            eth_1731_profile->lmm_cnt = lm_dm_period ? (has_other_entry ? 1 : 0) : 15;
            eth_1731_profile->dmm_cnt = old_eth_data.dmm_cnt;
        }
...
int _bcm_oam_set_classifier_action(int unit, uint32 is_add, uint32 mep_id, bcm_oam_endpoint_action_t action,int is_lm,int is_1dm_or_piggy_back,SOC_PPC_OAM_MEP_TYPE mep_type ,int is_slm) {
    ...
    rv = _bcm_oam_endpoint_action_set_validity_check(unit,mep_id,&action);
    BCMDNX_IF_ERR_EXIT(rv);
    rv = bcm_petra_oam_endpoint_action_set_internal(unit,mep_id,&action,is_piggy_backed,2/*ignore*/,2/*is_loss_unset:ignore*/);
    BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_oam_lm_dm_remove_shared(int unit,bcm_oam_endpoint_t endpoint_id , int is_lm, SOC_PPC_OAM_OAMP_LM_DM_MEP_DB_ENTRY *mep_db_entry, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    uint8 is_upmep;
    int trap_code = 0;
    int is_not_report_mode_qax = FALSE;
    ...
    if (_BCM_OAM_MEP_INDEX_IS_SERVER_CLIENT_SIDE(endpoint_id) == 0) {
        ...
        BCMDNX_IF_ERR_EXIT(rv);
        is_not_report_mode_qax = !SOC_IS_QAX(unit) ? FALSE : \
                                   (is_lm ? (!(old_profile->report_mode & 1) ? TRUE : FALSE) : \
                                   (!(old_profile->report_mode & 2) ? TRUE : FALSE));

        if (!only_on_demand_dmm && (!old_profile->report_mode || is_not_report_mode_qax)) {
...
STATIC int loss_delay_add_client_side(int unit,int endpoint_id, int is_single_ended_or_1dm, int is_lm,
    ...
    rv = bcm_petra_oam_endpoint_action_set_internal(unit,endpoint_id, &action,is_lm && !is_single_ended_or_1dm,2/*ignore*/,2/*is_loss_unset:ignore*/);
    BCMDNX_IF_ERR_EXIT(rv);
                                                       ...
                                                       int set_unset /* 1 = set, 0 = unset*/)
{
    ...
    BCMDNX_IF_ERR_EXIT(bcm_petra_oam_endpoint_action_set_internal(unit,id,action,set_unset /* piggyback */,2/*sat-loopback*/,is_loss_unset));
...
int sat_loopback_add_rx(int unit, bcm_oam_loopback_t *loopback_ptr) {
    ...
    if ((classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) == 0) { /* Down mep */
        ...
        BCM_GPORT_TRAP_SET(action.destination, trap_id, 7, 0); /*Taken from default values*/
        rv = bcm_petra_oam_endpoint_action_set_internal(unit, loopback_ptr->id, &action, 2/*piggyback(ignored)*/, 1/*sat_loopback*/, 2/*is_loss_unset:ignore*/);
        BCMDNX_IF_ERR_EXIT(rv);
        trap_id = (int)lb_trap_id;
    ...
    for (index = 0; index < loopback_ptr->num_tlvs; index++) {
...
int bcm_petra_oam_tst_rx_add(int unit,bcm_oam_tst_rx_t *tst_ptr) {
    ...
    else 
    {
        ...
        BCM_GPORT_TRAP_SET(action.destination, trap_id, 7, 0); /*Taken from default values*/
        rv = bcm_petra_oam_endpoint_action_set_internal(unit, tst_ptr->endpoint_id, &action, 2/*piggyback(ignored)*/, 1/*sat_loopback*/, 2/*is_loss_unset:ignore*/);
        BCMDNX_IF_ERR_EXIT(rv);
        trap_id = (int)tst_trap_id;
