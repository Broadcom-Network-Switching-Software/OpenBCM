...
bcm_instru_synced_counters_records_key_t_init(bcm_instru_synced_counters_records_key_t *key)
{
    ...
    key->source_type = bcmInstruSyncedCountersTypeCount;
    key->gport = BCM_GPORT_INVALID;
...
/*
 * Function:
 *     bcm_instru_ifit_encap_info_t_init
 * Description:
 *     Initialize The structure bcm_instru_ifit_encap_info_t.
 * Parameters:
 *     config - pointer to struct to be initialized
 * Return: none
 */
extern void
bcm_instru_ifit_encap_info_t_init(bcm_instru_ifit_encap_info_t *ifit_encap_info)
{
    if (ifit_encap_info != NULL) {
        sal_memset(ifit_encap_info, 0, sizeof(bcm_instru_ifit_encap_info_t));
    }
}
