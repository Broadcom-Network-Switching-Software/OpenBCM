...
static int bcm_tdpll_input_clock_reference_selector(
    int unit, int stack_id, int dpll_index);
    ...
    void **unused1)
{
    ...
    for (i = 0; i < TDPLL_DPLL_INSTANCE_NUM_MAX; ++i) {
        ...
        if (BCM_FAILURE(rv = bcm_tdpll_input_clock_reference_selector(unit, stack_id, i))) {
            /*
    ...
    int dpll_index)
{
    ...
    /* APTS : Check for Synce availability and change APTS mode accordingly */
    if (INPUT_CLOCK(BCM_TDPLL_INPUT_CLOCK_NUM_GPIO).dpll_use[dpll_index] ||
            INPUT_CLOCK(BCM_TDPLL_INPUT_CLOCK_NUM_GPIO+1).dpll_use[dpll_index]) {
        ...
        if (g_apts_enabled && (g_apts_usr_cfgd_source_state & bcmPtpClockAptsSourceSynceInternalDpll)){
            ...
            _bcm_ptp_apts_update_current_mode(unit, stack_id);
            /* Updating Synce Frequency correction value from Internal TDPLL monitoring functionality */
            if ((g_apts_current_op_mode == bcmPtpClockAptsModeSyncEPtp) ||
                    (g_apts_current_op_mode == bcmPtpClockAptsModeSyncEGps)) {
                bcm_tdpll_input_clock_frequency_error_get(unit, stack_id,
                    objdata.selector_state.selected_clock[dpll_index], &g_apts_synce_freq_corr_ppb);
            }
        }
    }
