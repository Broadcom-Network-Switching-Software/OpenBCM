    ...
    bcm_ptp_clock_info_t *clock_info)
{
    ...
    if (soc_feature(unit, soc_feature_ptp)) {
         ...
         *    Octet 56-59   : sync_receipt_timeout_msecs
         *    Octet 60-63   : delay_resp_receipt_timeout_msecs
         */
        i = 0;
        payload[i++] = 'B';
        payload[i++] = 'C';
        ...
        if (PTP_UC_FEATURE_CHECK(PTP_SYNC_DRESP_RECEIPT_TO)){
            _bcm_ptp_uint32_write(payload+i, clock_info->sync_receipt_timeout_msecs);
            i += sizeof(uint32);
            _bcm_ptp_uint32_write(payload+i, clock_info->delay_resp_receipt_timeout_msecs);
            i += sizeof(uint32);
        }
        ...
        _bcm_ptp_servo_init(unit, ptp_id, clock_info->clock_num, bcmPTPOscTypeOCXO, E_srvAdaptiveTimeReferenceTracker, clock_info->holdover_in_spec_secs);
#endif /* BCM_PTP_EXT_SERVO_SUPPORT */
        }
