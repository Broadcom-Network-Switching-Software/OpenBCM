    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                        (unit, mbcm_dnxf_port_speed_max, (unit, i, &(SOC_INFO(unit).port_speed_max[i]))));
    }
    ...
    bcm_module_t * modid_array)
{
    ...
    for (i = 0; i < modid_count; i++)
    {
        if (BCM_FABRIC_MODID_IS_LOCAL(modid_array[i]))
        {
            SHR_IF_ERR_EXIT(dnxf_modid_local_map_get(unit, BCM_FABRIC_LOCAL_MODID_UNSET(modid_array[i]), &module));
        }
        else
        {
    ...
    bcm_module_t * destid_array)
{
            ...
            else
            {
                if (BCM_FABRIC_MODID_IS_LOCAL(destid_array[i]))
                {
                    ...
                    if (soc_dnxf_multicast_mode_direct == dnxf_data_fabric.multicast.mode_get(unit))
                    {
                        ...
                        SHR_IF_ERR_EXIT(dnxf_modid_local_map_get(unit, local_module_id, &port_array[i]));
                        if (port_array[i] >= mc_table_entry_size)
                        {
                            SHR_ERR_EXIT(_SHR_E_PARAM,
                                         ...
                                         local_module_id, port_array[i], mc_table_entry_size);
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        SHR_IF_ERR_EXIT(dnxf_modid_local_map_modid_to_local(unit, destid_array[i], &(port_array[i])));
                    }
                }
            }
        }
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_multicast_egress_set,
                                          (unit, group, destid_count, (soc_gport_t *) port_array)));
