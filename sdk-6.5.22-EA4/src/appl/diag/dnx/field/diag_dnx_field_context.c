    ...
    for (stage_index = stage_lower; stage_index <= stage_upper; stage_index++)
    {
        ...
        if (field_stage == DNX_FIELD_STAGE_EXTERNAL)
        {
            ...
            if ((ctx_id_lower_in == 0xFFFFFFFF) && (ctx_id_upper_in == 0xFFFFFFFF))
            {
                ...
                {
                    ctx_id_upper = DNX_KBP_MAX_STATIC_OPCODES_NOF - 1;
                }
            }
            else
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (fwd_context_idx = 0; fwd_context_idx < fwd_nof_contexts; fwd_context_idx++)
    {
        ...
        DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(NULL, "%d, ", acl_contexts[fwd_context_idx]);
        sal_snprintf(acl_ctx_buff, sizeof(acl_ctx_buff), "%d, ", acl_contexts[fwd_context_idx]);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (field_stage == DNX_FIELD_STAGE_EXTERNAL)
    {
        ...
        if (dnx_kbp_device_enabled(unit) && (DNX_KBP_STATUS_IS_LOCKED(kbp_mngr_status) == FALSE))
        {
