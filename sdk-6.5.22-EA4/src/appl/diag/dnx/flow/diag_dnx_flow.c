    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (flow_app_info->special_fields[0] != DBAL_FIELD_EMPTY)
    {
        ...
        PRT_COLUMN_ADD("is field Maskable");
        ...
        for (field_idx = 0; field_idx < BCM_FLOW_SPECIAL_FIELD_MAX_NOF_FIELDS; field_idx++)
        {
            ...
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%s", dnx_flow_special_field_to_string(unit, flow_app_info->special_fields[field_idx]));
            PRT_CELL_SET("%d", flow_app_info->special_fields[field_idx]);
            if (flow_app_info->special_fields[field_idx] == DBAL_FIELD_BITSTR)
            {
                PRT_CELL_SET("shr_var_uint32_arr");
            }
            else
            {
            ...
            {
                if (FLOW_FIELD_IS_MASKABLE(flow_app_info->special_fields[field_idx]))
                {
                    PRT_CELL_SET("Maskable");
                }
                else
                {
                    PRT_CELL_SET("Complete field can be valid/not valid");
                }
            }
            else
            {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_field_type_get(unit, flow_app_info->special_fields[field_idx], &field_type));
            ...
            if (field_type_info->nof_enum_values > 0)
            {
                ...
                {
                    if (!(field_type_info->enum_val_info[enum_idx].is_invalid))
                    {
                        char_count =
                            sal_sprintf(cur_str_pos, "%s: 0x%x\n", field_type_info->enum_val_info[enum_idx].name,
                                        enum_idx);
     ...
     */
    LOG_CLI((BSL_META("\nApplication valid flow handler flags:\n")));
    ...
    uint8 is_key)
{
    ...
    for (field_idx = 0; field_idx < BCM_FLOW_SPECIAL_FIELD_MAX_NOF_FIELDS; field_idx++)
    {
        ...
        if (flow_app_info->special_fields[field_idx] == special_fields->special_fields[actual_field_idx].field_id)
        {
            ...
            field_val[0] = special_fields->special_fields[actual_field_idx].shr_var_uint32;
        ...
        if (SAL_BIT(field_idx) & flow_app_info->valid_common_fields_bitmap)
        {
            ...
            switch (common_fields_desc->term_field_enabler)
            {
                ...
                case BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID:
                    PRT_CELL_SET("%d", terminator_info->l2_ingress_info.ingress_network_group_id);
                    break;
        ...
        if (SAL_BIT(field_idx) & flow_app_info->valid_common_fields_bitmap)
        {
            ...
            switch (common_fields_desc->init_field_enabler)
            {
                ...
                case BCM_FLOW_INITIATOR_ELEMENT_ENCAP_ACCESS_VALID:
                    SHR_IF_ERR_EXIT(common_fields_desc->print_cb
                                    (unit, (void *) (&(initiator_info->encap_access)), print_string));
                    PRT_CELL_SET("%s", print_string);
                    break;
                ...
                case BCM_FLOW_INITIATOR_ELEMENT_QOS_EGRESS_MODEL_VALID:
                    ...
                    PRT_CELL_SET("%s", print_string);
                    break;
                case BCM_FLOW_INITIATOR_ELEMENT_TTL_VALID:
                    PRT_CELL_SET("%d", initiator_info->ttl);
                    break;
                case BCM_FLOW_INITIATOR_ELEMENT_DSCP_VALID:
                    PRT_CELL_SET("%d", initiator_info->dscp);
                    break;
                case BCM_FLOW_INITIATOR_ELEMENT_EXP_VALID:
                    PRT_CELL_SET("%d", initiator_info->exp);
                    break;
                case BCM_FLOW_INITIATOR_ELEMENT_PKT_PRI_CFI_VALID:
                    PRT_CELL_SET("%d , %d", initiator_info->pkt_pri, initiator_info->pkt_cfi);
                    break;
