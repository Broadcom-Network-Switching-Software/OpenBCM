...
#define MAX_PORT_SIGNALS        5
        ...
        SHR_CLI_EXIT_IF_ERR(sand_signal_output_find(unit, core_id, 0, "IRPP", "VTT5", "", "layer_offsets",
                                                    &offsets_list_element_head),
                            "no signal record for layer_offsets\n");
        ...
        if (!short_mode)
        {
            ...
            RHITERATOR(field_list_element, sig_list_element->field_list)
            {
                ...
                if (sub_layer_idx != -1)
                {
                    ...
                    if (!sal_strstr(name, "UDP") || sal_strstr(next_layer_name, "VXLAN"))
                    {
                        sub_layer_flag = 1;
                ...
                if (next_field_list_element != NULL)
                {
                    ...
                    {
                        /** UDP is considered as a sublayer in VXLAN cases.*/
                        if (!sal_strstr(next_layer_name, "UDP") || sal_strstr(next_next_layer_name, "VXLAN"))
                        {
