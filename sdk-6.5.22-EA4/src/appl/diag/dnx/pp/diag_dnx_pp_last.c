...
#define PP_LAST_SRV6_SGEMENT_SIZE_BYTES        16
...
#define __PP_LAST_GET_FWD_LAYER_LOOKUP_TERMS(__fwd_layer)                                                                                                                           \
    ...
    {                                                                                                                                                                       \
        ...
        {                                                                                                                                                                            \
            SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "The header was not parsed correctly for the signal: %s !\n", pp_last_fwd_layers_lookup_terms[__fwd_layer].lookup_term_signals[i_sig_fwd_lookup_term]); \
        }                                                                                                                                                                            \
                                                                                                                                                                            \
        if (field_header_sig)                                                                                                                                               \
        {                                                                                                                                                                   \
    ...
    char srv6_printable_dip[RHSTRING_MAX_SIZE])
{
    ...
    uint32 fwd2_context;
    uint32 ipv6_addr[PP_LAST_SRV6_SGEMENT_SIZE_BYTES / 4] = { 0 }, index;
    ...
    if (fwd2_context == DBAL_ENUM_FVAL_FWD2_CONTEXT_ID_SRV6___PRIVATE_UC
        || fwd2_context == DBAL_ENUM_FVAL_FWD2_CONTEXT_ID_SRV6___USID_PRIVATE_UC
        || fwd2_context == DBAL_ENUM_FVAL_FWD2_CONTEXT_ID_SRV6___GSID_PRIVATE_UC)
    {
        for (index = 0; index < PP_LAST_SRV6_SGEMENT_SIZE_BYTES / 4; index++)
        {
            ...
            SHR_CLI_EXIT_IF_ERR(sand_signal_output_find(unit, core_id, 0, "IRPP", "LBP", "ITMR", udh_header_name,
                                                        &udh_data_sig), "no signal record for UDH_DATA_x\n");
            ipv6_addr[PP_LAST_SRV6_SGEMENT_SIZE_BYTES / 4 - 1 - index] = udh_data_sig->value[0];
        }

        /** Format the IPv6 address by a router table*/
        SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get
                        (unit, DBAL_TABLE_IPV6_UNICAST_PRIVATE_HOST, DBAL_FIELD_IPV6, ipv6_addr, NULL, 0, TRUE,
                         buffer_to_print));
 ...
 */
    ...
    SHR_CLI_EXIT_IF_ERR(sand_signal_output_find(unit, core_id, 0, "IRPP", "VTT5", "", "layer_protocols",
                                                &protocols_sig_head), "no signal record for layer_protocols\n");

    if (sand_signal_output_find(unit, core_id, 0, "IRPP", "NIF", "", "Header", &pkt_header_sig_head) != _SHR_E_NONE)
    {
        /** Try to get the header again from another block.*/
        SHR_CLI_EXIT_IF_ERR(sand_signal_output_find(unit, core_id, 0, "IRPP", "", "IParser", "Pkt_Header",
                                                    &pkt_header_sig_head), "no signal record for header\n");
    }

    SHR_CLI_EXIT_IF_ERR(sand_signal_output_find(unit, core_id, 0, "IRPP", "VTT5", "", "layer_offsets",
                                                &layer_offsets_sig_head), "no signal record for layer_offsets\n");
