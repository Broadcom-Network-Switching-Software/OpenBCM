             ...
             && !dnx_data_lif.feature.feature_get(unit, dnx_data_lif_feature_iop_mode_orientation_selection))
    {
        if (dnx_data_qos.qos.routing_dscp_preserve_mode_get(unit) == 1)
        {
            ...
            nof_ingress_profile_bits =
                ...
                IN_LIF_PROFILE_EGRESS_NOF_BITS_DSCP_MODE - NATIVE_INDEXED_MODE_NOF_BITS;
        }
        else
        {
           /** If policer is enabled, it is allocated only 1 bit, IVE_OUTER_QOS_DP_PROFILE_NOF_BITS should not be calculated */
            nof_egress_profile_bits =
                IN_LIF_ORIENTATION_NOF_BITS + IVE_INNER_QOS_DP_PROFILE_NOF_BITS + SAME_INTERFACE_MODE_NOF_BITS;
            nof_ingress_profile_bits =
                NOF_IN_LIF_PROFILE_BITS - IN_LIF_PROFILE_PMF_RESERVED_NOF_BITS - nof_egress_profile_bits -
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char acl_profile_bits[DBAL_MAX_STRING_LENGTH];
    char egress_profile_bits[DBAL_MAX_STRING_LENGTH];
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
        ...
        && (dnx_data_qos.qos.routing_dscp_preserve_mode_get(unit) == 1))
    {
       /** When routing_dscp_preserve_mode_get = 1 means all other egress properties are 0 */
    ...
    sh_sand_control_t * sand_control)
{
    uint32 entry_handle_id, rpf_mode, in_lif_profile, icmpv4_trap_handling_profile, entry_handle_id_vsi_profile,
        ...
        da_not_found_destination_profile, dhcp_trap_handling_profile, routing_enablers_profile,
        l2tp_trap_handling_profile;
    int is_end, nof_ingress_bits, system_headers_mode;
    char prefix_value_binary_buffer[8];
    char prefix_value_binary_buffer_full[8];
    char buffer[DBAL_MAX_PRINTABLE_BUFFER_SIZE];
             ...
             && !dnx_data_lif.feature.feature_get(unit, dnx_data_lif_feature_iop_mode_orientation_selection))
    {
        if (dnx_data_qos.qos.routing_dscp_preserve_mode_get(unit) == 1)
        {
            SHR_ALLOC(egress_bits_p, sizeof(*egress_bits_p) * (IN_LIF_PROFILE_EGRESS_NOF_BITS_DSCP_MODE + 1),
                      "egress_bits_p", "%s%s%s", EMPTY, EMPTY, EMPTY);
            nof_ingress_bits =
                NOF_IN_LIF_PROFILE_BITS - IN_LIF_PROFILE_EGRESS_NOF_BITS_DSCP_MODE -
                IN_LIF_PROFILE_PMF_RESERVED_NOF_BITS - NATIVE_INDEXED_MODE_NOF_BITS;
        }
        else
        {
            /** IVE_OUTER_QOS_DP_PROFILE_NOF_BITS should not be calculated */
            SHR_ALLOC(egress_bits_p,
                      sizeof(*egress_bits_p) * (IN_LIF_ORIENTATION_NOF_BITS + IVE_INNER_QOS_DP_PROFILE_NOF_BITS +
                                                SAME_INTERFACE_MODE_NOF_BITS + 1), "egress_bits_p", "%s%s%s", EMPTY,
                      EMPTY, EMPTY);
            nof_ingress_bits =
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char prefix_value_binary_buffer[7];
    char prefix_value_binary_buffer_full[8];
    ...
    else
    {
        ...
        PRT_TITLE_SET("Ingress-PP VSI profile table");
        PRT_INFO_ADD
            ("Profile bits separation: %s, Egress profile bits = [%d : %d], Ingress ETH RIF profile bits = [%d : %d]",
             acl_profile_bits, ETH_RIF_PROFILE_PMF_RESERVED_START_BIT - 1,
             ETH_RIF_PROFILE_PMF_RESERVED_START_BIT - IN_LIF_PROFILE_EGRESS_NOF_BITS_DSCP_MODE,
             ETH_RIF_PROFILE_PMF_RESERVED_START_BIT - IN_LIF_PROFILE_EGRESS_NOF_BITS_DSCP_MODE - 1, 0);
        PRT_COMMITX;
    }
