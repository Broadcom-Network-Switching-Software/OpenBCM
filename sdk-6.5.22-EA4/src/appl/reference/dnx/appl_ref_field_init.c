    ...
    int unit)
{
    ...
    bcm_field_context_info_t_init(&context_info);
    dest_char = &(context_info.name[0]);
    sal_strncpy(dest_char, "MPLS_FHEI_8B_WA", sizeof(context_info.name));
    ...
    qual_info.size = 8;
    dest_char = &(qual_info.name[0]);
    sal_strncpy(dest_char, "MPLS_FHEI_TTL", sizeof(qual_info.name));
    SHR_IF_ERR_EXIT(bcm_field_qualifier_create(unit, 0, &qual_info, &mpls_fhei_ttl_qual_id));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 3;
    dest_char = &(qual_info.name[0]);
    sal_strncpy(dest_char, "MPLS_FHEI_EXP", sizeof(qual_info.name));
    ...
    dest_char = &(fg_info.name[0]);
    sal_strncpy(dest_char, "MPLS_FHEI_8B", sizeof(fg_info.name));
    SHR_IF_ERR_EXIT(bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &fg_id_fhei_8b));
    ...
    ace_format_info.action_types[ace_format_info.nof_actions++] = bcmFieldActionReserved370;
    ace_format_info.action_types[ace_format_info.nof_actions++] = bcmFieldActionReserved371;
    ace_format_info.action_types[ace_format_info.nof_actions++] = bcmFieldActionReserved372;

    dest_char = &(ace_format_info.name[0]);
    sal_strncpy_s(dest_char, "MPLS_FHEI_8B", sizeof(ace_format_info.name));
    SHR_IF_ERR_EXIT(bcm_field_ace_format_add(unit, 0, &ace_format_info, &ace_format_id));

    bcm_field_ace_entry_info_t_init(&ace_entry_info);

    ace_entry_info.nof_entry_actions = 3;
    ace_entry_info.entry_action[0].type = bcmFieldActionReserved370;
    ace_entry_info.entry_action[1].type = bcmFieldActionReserved371;
    ace_entry_info.entry_action[2].type = bcmFieldActionReserved372;
