                                     ...
                                     int bufsz)
{
    ...
    void *arg_ptrs_save[CINT_CONFIG_MAX_FPARAMS+1];
    char _rstr[CINT_CONFIG_MAX_STACK_PRINT_BUFFER] = {0};
    int ii, nargs, copy_size, done, len;
    char *dst;
 ...
 */
            ...
            if (pds[ii].pcount) {
                ...
                if (SHR_BITGET(ctxt->is_charptr, ii) && (pds[ii].pcount == 1) &&
                    arg_ptrs_save[ii] && (copy_size = array_size_save[ii])) {
 ...
 */
        ...
        if (pds[ii].pcount) {
            ...
            if (SHR_BITGET(ctxt->is_charptr, ii) && (params[ii].pcount == 1)) {
                src += CINT_STRLEN(src) + 1;
            }
            ...
            if (pds[ii].pcount) {
                ...
                if (orig_ptr) {
                    ...
                    if (SHR_BITGET(ctxt->is_charptr, ii) && (pds[ii].pcount == 1)) {
                        ...
                        dummy_ptrs[ii][cur_dummy_ptr[ii]] = src;
                        src += CINT_STRLEN(src) + 1;
                        pds[ii].pcount--;
                    } else {
    ...
    for (ii = 0; (ii < nargs) && params[ii].name; ii++) {
        ...
        for (pcount = 0; pcount < params[ii].pcount; pcount++) {
            ...
            if (pcount == (params[ii].pcount -1)) {
                ...
                if (SHR_BITGET(ctxt->is_charptr, ii)) {
                    continue;
                }
