...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin7_pcieg3_diag.c                                             *
...
err_code_t merlin7_pcieg3_test_uc_timestamp(srds_access_t *sa__) {
    return merlin7_pcieg3_INTERNAL_test_uc_timestamp_with_print_options(sa__, 0xFF);
}

static void _merlin7_pcieg3_display_eye_scan_ignored_lane(srds_access_t *sa__, uint8_t eye_slice) {
    if (eye_slice) {
        USR_PRINTF(("         : -"));
    } else {
        USR_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
...
err_code_t merlin7_pcieg3_display_eye_scan(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    ...
    Exit_Status = merlin7_pcieg3_INTERNAL_eye_scan_setup(sa__, 0, &y_max, &y_step, &usr_diag_status);
    ...
    if (y_step == 0) {
        merlin7_pcieg3_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
        Exit_Status = ERR_CODE_BAD_PTR_OR_INVALID_INPUT;
        goto Exit;
    }
    CFUN(merlin7_pcieg3_display_eye_scan_header(sa__, 1, &usr_diag_status));
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        if(enable_read) {
            Exit_Status = merlin7_pcieg3_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (Exit_Status) {
                EFUN_PRINTF(("ERROR: merlin7_pcieg3_read_eye_scan_stripe() returned: %s, (status = 0x%x)\n", merlin7_pcieg3_INTERNAL_e2s_err_code(Exit_Status), status));
                goto Exit;
            }
        }
        CFUN(merlin7_pcieg3_display_eye_scan_stripe(sa__, y, &stripe[buffer_idx], usr_diag_status));
        USR_PRINTF(("\n"));
    }
    CFUN(merlin7_pcieg3_display_eye_scan_footer(sa__, 1, &usr_diag_status));
    /* stop acquisition */
    CFUN(merlin7_pcieg3_meas_eye_scan_done(sa__));
Exit:
    return(merlin7_pcieg3_error_report(sa__, Exit_Status));
}

err_code_t merlin7_pcieg3_display_all_eye_scan(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint8_t num_lanes, lane;
    uint8_t lane_orig = merlin7_pcieg3_acc_get_lane(sa__);
    ...
    for ( lane=0; lane < num_lanes; lane++) {
        err_code = merlin7_pcieg3_acc_set_lane(sa__, lane);
        if (err_code) {
            USR_PRINTF(("%s WARN: Unable to switch to lane(%d) and so ignoring lane for for 2D eye scan\n", API_FUNCTION_NAME, lane));
            continue;
        }
        err_code = merlin7_pcieg3_INTERNAL_eye_scan_setup(sa__, 0, &i_max[lane], &i_step[lane], &usr_diag_status[lane]);
        if (err_code) {
            uint8_t core;
            core = merlin7_pcieg3_acc_get_core(sa__);
    ...
    if(lane_mask) {
        CFUN(merlin7_pcieg3_display_eye_scan_header(sa__, num_lanes, &usr_diag_status[0]));
    }
    else {
        goto Exit;
    }
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                ...
                if(i[lane] >= -i_max[lane]) {
                    local_error = merlin7_pcieg3_acc_set_lane(sa__, lane);
                    if (local_error) {
                        lane_mask &= (uint8_t)~(1<<lane);
                        _merlin7_pcieg3_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                        continue;
                    }
                    status = 0;
                    if(eye_slice[lane]) {
                    ...
                    if(enable_read) {
                        local_error = merlin7_pcieg3_read_eye_scan_stripe(sa__, &stripe[0], &status);
                        if (local_error) {
                            lane_mask &= (uint8_t)~(1<<lane);
                            _merlin7_pcieg3_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                            continue;
                    ...
                    local_error = merlin7_pcieg3_display_eye_scan_stripe(sa__, i[lane], &stripe[buffer_idx], usr_diag_status[lane]);
                    if (local_error) {
                        lane_mask &= (uint8_t)~(1<<lane);
                        _merlin7_pcieg3_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                        continue;
    ...
    CFUN(merlin7_pcieg3_display_eye_scan_footer(sa__, num_lanes, &usr_diag_status[0]));
    for(lane=0;lane < num_lanes;lane++) {
        if(lane_mask & (1<<lane)) {
            CFUN(merlin7_pcieg3_acc_set_lane(sa__, lane));
            CFUN(merlin7_pcieg3_meas_eye_scan_done(sa__));
        }
    }
Exit:

    if (merlin7_pcieg3_acc_set_lane(sa__, lane_orig)) {
...
err_code_t merlin7_pcieg3_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    INIT_SRDS_ERR_CODE
    uint8_t lock;
    EFUN(merlin7_pcieg3_pmd_lock_status(sa__,&lock));
    {
        uint8_t sts;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(merlin7_pcieg3_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_VSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_HORIZ) {
        EFUN(merlin7_pcieg3_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_HSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_SLICE) {
        EFUN(merlin7_pcieg3_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_EYE_SLICE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_INTR) {
        EFUN(merlin7_pcieg3_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_INTR_EYE,GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}

err_code_t merlin7_pcieg3_INTERNAL_poll_diag_data(srds_access_t *sa__, const srds_info_t *merlin7_pcieg3_info_ptr, uint16_t *status,
                                          uint8_t * diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms)
{
    INIT_SRDS_ERR_CODE
    const uint32_t lane_diag_size = merlin7_pcieg3_info_ptr->diag_mem_ram_size;
    uint16_t loop;
    ...
    *diag_rd_ptr = 0;
    ...
    if (byte_count > (lane_diag_size / 2)) {
        EFUN_PRINTF(("\nERROR : merlin7_pcieg3_INTERNAL_poll_diag_data() has excessive byte count of %d.\n", byte_count));
...
typedef struct {
    uint32_t *buffer_ptr;
    } merlin7_pcieg3_read_eye_scan_stripe_callback_arg_t;

static err_code_t merlin7_pcieg3_read_eye_scan_stripe_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    merlin7_pcieg3_read_eye_scan_stripe_callback_arg_t * const cast_arg = (merlin7_pcieg3_read_eye_scan_stripe_callback_arg_t *)arg;
    UNUSED(sa__);
    *(cast_arg->buffer_ptr++) = merlin7_pcieg3_INTERNAL_float8_to_uint32((float8_t)(data & 0xFF));
    if (byte_count > 1) {
        *(cast_arg->buffer_ptr++) = merlin7_pcieg3_INTERNAL_float8_to_uint32((float8_t)(data >> 8));
    }
    return (ERR_CODE_NONE);
}

err_code_t merlin7_pcieg3_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    INIT_SRDS_ERR_CODE
    srds_info_t const * const merlin7_pcieg3_info_ptr = merlin7_pcieg3_INTERNAL_get_merlin7_pcieg3_info_ptr_with_check(sa__);
    const uint8_t lane = merlin7_pcieg3_acc_get_lane(sa__);
    const uint8_t stripe_size = 64;

    merlin7_pcieg3_read_eye_scan_stripe_callback_arg_t arg;
    ...
    EFUN(merlin7_pcieg3_INTERNAL_match_ucode_from_info(sa__, merlin7_pcieg3_info_ptr));
        ...
        lane_diag_base = merlin7_pcieg3_info_ptr->diag_mem_ram_base + ((lane%merlin7_pcieg3_info_ptr->lane_count) * merlin7_pcieg3_info_ptr->diag_mem_ram_size) +
            (uint32_t)(merlin7_pcieg3_info_ptr->grp_ram_size*merlin7_pcieg3_INTERNAL_grp_idx_from_lane(merlin7_pcieg3_acc_get_physical_lane(sa__)));

    EFUN(merlin7_pcieg3_INTERNAL_poll_diag_data(sa__, merlin7_pcieg3_info_ptr, status, &diag_rd_ptr, stripe_size, timeout_ms));

    arg.buffer_ptr = buffer;
    EFUN(merlin7_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
                                              (uint16_t)merlin7_pcieg3_info_ptr->diag_mem_ram_size,
                                              diag_rd_ptr,
                                              stripe_size,
                                              &arg,
                                              merlin7_pcieg3_read_eye_scan_stripe_callback));

    diag_rd_ptr = (uint8_t)(((uint32_t)diag_rd_ptr + stripe_size) % merlin7_pcieg3_info_ptr->diag_mem_ram_size);
    EFUN(wrv_usr_diag_rd_ptr(diag_rd_ptr));
    ESTM(*status = rdv_usr_diag_status() & 0xFF);
    return(ERR_CODE_NONE);
}

err_code_t merlin7_pcieg3_display_eye_scan_stripe(srds_access_t *sa__, int16_t y,uint32_t *buffer, uint16_t usr_diag_status) {
    INIT_SRDS_ERR_CODE
    ...
    slicer_range = (data_thresh ? 29 : 35);
    ...
    EFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, slicer_range, data_thresh, slicer, &slicer_range_mV));
...
err_code_t merlin7_pcieg3_display_eye_scan_header(srds_access_t *sa__, uint8_t i, uint16_t *usr_diag_status) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    ...
    for(x=1;x<=i;x++) {
        {
            _merlin7_pcieg3_display_eye_scan_ignored_lane(sa__, 0);
        }
    }
    EFUN_PRINTF(("\n"));
    return(ERR_CODE_NONE);
...
err_code_t merlin7_pcieg3_display_eye_scan_footer(srds_access_t *sa__, uint8_t i, uint16_t *usr_diag_status) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    uint8_t stepsize_var[NUM_LANES_MAX];
    uint8_t lane, num_lanes;
...
err_code_t merlin7_pcieg3_read_eye_scan_status(srds_access_t *sa__, uint16_t *status) {
    INIT_SRDS_ERR_CODE
...
err_code_t merlin7_pcieg3_meas_eye_scan_done(srds_access_t *sa__) {
  INIT_SRDS_ERR_CODE
  EFUN(merlin7_pcieg3_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_DISABLE,GRACEFUL_STOP_TIME));
  return(ERR_CODE_NONE);
}


err_code_t merlin7_pcieg3_start_ber_scan_test(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    INIT_SRDS_ERR_CODE
    uint8_t lock,sts;
    EFUN(merlin7_pcieg3_pmd_lock_status(sa__,&lock));
    if(lock == 0) {
    ...
    EFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_START, ber_scan_mode,GRACEFUL_STOP_TIME));
    return(ERR_CODE_NONE);
}

err_code_t merlin7_pcieg3_read_ber_scan_data(srds_access_t *sa__, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
    INIT_SRDS_ERR_CODE
    ...
    if((sts & 0x8000) == 0) {
        return(merlin7_pcieg3_error(sa__, ERR_CODE_DATA_NOTAVAIL));
    }
    *cnt = (uint8_t)((sts & 0x00FF)/3);
    for(i=0;i < *cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, timeout));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors[i] = merlin7_pcieg3_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values[i] = (merlin7_pcieg3_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
...
err_code_t merlin7_pcieg3_eye_margin_proj_with_data(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(merlin7_pcieg3_start_ber_scan_test(sa__, ber_scan_mode, timer_control, max_error_control));
    ESTM(offset_start = (int16_t)rd_uc_dsc_data());
    {
        int16_t nlmv_val = 0;
        afe_override_slicer_sel_t slicer = INVALID_SLICER;
        EFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, offset_start, 0, slicer, &nlmv_val));
    ...
    EFUN(merlin7_pcieg3_INTERNAL_poll_diag_done(sa__, &sts,(uint32_t)(timer_control*2000)));

    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(merlin7_pcieg3_read_ber_scan_data(sa__, &errs[0], &time[0], &cnt, 2000));

    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,200));
 ...
 /* }                                                                  */
    ...
    EFUN(merlin7_pcieg3_display_ber_scan_data(sa__, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)SRDS_ABS(offset_start)));
...
err_code_t merlin7_pcieg3_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    INIT_SRDS_ERR_CODE
    EFUN(merlin7_pcieg3_eye_margin_proj_with_data(sa__, rate, ber_scan_mode, timer_control, max_error_control));
    return(ERR_CODE_NONE);
}



err_code_t merlin7_pcieg3_display_ber_scan_data (srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
...
#ifndef STANDALONE_EVENT
            if(intrusive) {
                margins[i] = direction*intrusive_margins_mv[(uint8_t)offset[i]];
            } else {
                CFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250, slicer, &nlmv_val));
...
err_code_t merlin7_pcieg3_display_lane_state_hdr(srds_access_t *sa__) {
  EFUN_PRINTF(("LN  P STP RATE "));
  EFUN_PRINTF(("DSC0(0,1) SD(D,C) LCK RXPPM "));
...
err_code_t merlin7_pcieg3_display_lane_state_legend(srds_access_t *sa__) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
  ...
  EFUN_PRINTF(("RATE      : Rate select\n"));
  EFUN_PRINTF(("DSC0(0,1) : Double read of dsc_one_hot_0 \n"));
  EFUN_PRINTF(("SD(D,C)   : Signal detect and counter\n"));
...
err_code_t merlin7_pcieg3_display_lane_state(srds_access_t *sa__) {
  err_code_t err_code = merlin7_pcieg3_INTERNAL_display_lane_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin7_pcieg3_display_core_state_hdr(srds_access_t *sa__) {
    char core_type[25] = "merlin7_pcieg3";
    EFUN_PRINTF(("SerDes type = %s\n",core_type));

  EFUN_PRINTF(("CORE  ST   PLL_PWDN   COM_CLK   UCODE_VER  API_VER  AFE_VER   LIVE_TEMP   AVG_TMON   RESCAL   RATE ANA_VCO_RANGE PLL_LOCK"));
  EFUN_PRINTF(("\n"));
  return (ERR_CODE_NONE);
}

err_code_t merlin7_pcieg3_display_core_state_line(srds_access_t *sa__) {
  err_code_t err_code = merlin7_pcieg3_INTERNAL_display_core_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin7_pcieg3_display_core_state_legend(srds_access_t *sa__) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
err_code_t merlin7_pcieg3_display_core_state(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    EFUN(merlin7_pcieg3_display_core_state_hdr(sa__));
    EFUN(merlin7_pcieg3_display_core_state_line(sa__));
    EFUN(merlin7_pcieg3_display_core_state_legend(sa__));
    return ERR_CODE_NONE;
}
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    struct merlin7_pcieg3_uc_core_config_st core_config;
    err_code_t Exit_Status = ERR_CODE_NONE;
    ...
    CFUN(merlin7_pcieg3_INTERNAL_get_osr_mode(sa__, &osr_mode));

    if(osr_mode.rx > MERLIN7_PCIEG3_OSX4)
    {
    ...
    {
        uint8_t rate_sel;
        CSTM(rate_sel = rd_rate_sel());
        switch (rate_sel) {
        case 0: rate = 2500; break;
        case 1: rate = 5000; break;
        case 2: rate = 8000; break;
        default:
                {
                    Exit_Status = ERR_CODE_CONFLICTING_PARAMETERS;
                    goto Exit;
                }
        }
    }
    ...
    CFUN(merlin7_pcieg3_start_ber_scan_test(sa__, ber_scan_mode, timer_control, max_error_control));
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        CFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, offset_start, range250, slicer, &nlmv_val));
    ...
    CFUN(merlin7_pcieg3_INTERNAL_poll_diag_done(sa__, &sts,(uint32_t)(timer_control*2000)));
    ...
    if((sts & 0x8000) == 0) {
        ...
        merlin7_pcieg3_error(sa__, Exit_Status);
        goto Exit;
    }
    cnt = (uint8_t)((sts & 0x00FF)/3);
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        CFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        CSTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        CFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        CSTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors = merlin7_pcieg3_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values = (merlin7_pcieg3_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
        if(DIAG_VERBOSE < 5) {
            if (!(i % 4))  {
                USR_PRINTF(("\n"));
            }
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                CFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)(SRDS_ABS(offset_start)-i), range250, slicer, &nlmv_val));
    ...
    USR_PRINTF(("\n"));
    CFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
Exit:
...
err_code_t merlin7_pcieg3_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    INIT_SRDS_ERR_CODE
    uint8_t lane = merlin7_pcieg3_acc_get_lane(sa__), lane_select=0;

    if (lane < 8) {
        lane_select = (uint8_t)(0x1<<lane);
        EFUN(merlin7_pcieg3_display_diag_data_core(sa__, diag_level, lane_select));
    }
    else {
...
err_code_t merlin7_pcieg3_display_diag_data_core(srds_access_t *sa__, uint16_t  diag_level, uint8_t lane_select) {
    INIT_SRDS_ERR_CODE
    ...
    ESTM_PRINTF(("Core  = %d;    LANE  = %d\n",merlin7_pcieg3_acc_get_core(sa__),merlin7_pcieg3_acc_get_lane(sa__)));
    EFUN(merlin7_pcieg3_version(sa__, &api_version));
    ...
    EFUN_PRINTF(("\n"));
    ESTM(num_lanes = rdc_revid_multiplicity());
    save_lane = merlin7_pcieg3_acc_get_lane(sa__);
    ...
    for (lane=0; lane<num_lanes; lane++) {
        if (lane_select & (0x1<<lane)) {
            ...
            EFUN(merlin7_pcieg3_pmd_lock_status(sa__,&rx_lock));
            micro_stop[lane] = merlin7_pcieg3_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
            if(err_code) USR_PRINTF(("Unable to stop lane %d micro,  following data is suspect\n", lane));
        }
    }
    EFUN(merlin7_pcieg3_acc_set_lane(sa__, save_lane));
    if((diag_level & SRDS_DIAG_TIMESTAMP) || (diag_level & SRDS_DIAG_CORE)) {
        diag_data_err = merlin7_pcieg3_test_uc_timestamp(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
            EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_TIMESTAMP returned error %s ****\n", merlin7_pcieg3_INTERNAL_e2s_err_code(diag_data_err)));
            diag_level_error |= SRDS_DIAG_TIMESTAMP;
        }
    }
    if(diag_level & SRDS_DIAG_CORE) {
        EFUN(merlin7_pcieg3_display_core_state_hdr(sa__));
        diag_data_err = merlin7_pcieg3_display_core_state_line(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    EFUN(merlin7_pcieg3_display_lane_state_hdr(sa__));
    diag_data_err = merlin7_pcieg3_display_lane_state(sa__);
    if(diag_data_err != ERR_CODE_NONE) {
        EFUN_PRINTF(("\n**** WARNING: merlin7_pcieg3_display_lane_state() returned error %s ****\n", merlin7_pcieg3_INTERNAL_e2s_err_code(diag_data_err)));
    }
    ...
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        ...
        uint8_t micro_num = 0;
        {
            ...
            state.zero_cnt = 0;
            save_lane = merlin7_pcieg3_acc_get_lane(sa__);
            diag_data_err = merlin7_pcieg3_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, merlin7_pcieg3_INTERNAL_event_log_dump_callback);
            EFUN(merlin7_pcieg3_acc_set_lane(sa__,save_lane));
            if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        save_lane = merlin7_pcieg3_acc_get_lane(sa__);
        diag_data_err = merlin7_pcieg3_read_event_log(sa__);
        EFUN(merlin7_pcieg3_acc_set_lane(sa__,save_lane));
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if (diag_level & SRDS_DIAG_REG_CORE) {
        diag_data_err = merlin7_pcieg3_reg_select_dump(sa__, SRDS_CORE_REG_ONLY, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if (diag_level & SRDS_DIAG_REG_LANE) {
        ...
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                EFUN(merlin7_pcieg3_acc_set_lane(sa__, lane));
                diag_data_err = merlin7_pcieg3_reg_select_dump(sa__, SRDS_LANE_REG_ONLY, NULL);
                if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        data_cnt = (uint16_t)merlin7_pcieg3_info_ptr->core_var_ram_size;

        diag_data_err = merlin7_pcieg3_uc_core_var_blk_dump(sa__, core, data_cnt, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_UC_LANE) {
        ...
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                EFUN(merlin7_pcieg3_acc_set_lane(sa__, lane));
                diag_data_err = merlin7_pcieg3_uc_lane_var_blk_dump(sa__, core, lane, data_cnt, NULL);
                if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_EYE) {
        diag_data_err = merlin7_pcieg3_display_eye_scan(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        diag_data_err = merlin7_pcieg3_INTERNAL_display_info_table(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
...
} merlin7_pcieg3_diag_access_callback_state_t;

static err_code_t merlin7_pcieg3_diag_access_read_byte_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    merlin7_pcieg3_diag_access_callback_state_t * const state_ptr = (merlin7_pcieg3_diag_access_callback_state_t *)arg;
    while (byte_count > 0) {
...
static err_code_t merlin7_pcieg3_diag_access_read_word_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    merlin7_pcieg3_diag_access_callback_state_t * const state_ptr = (merlin7_pcieg3_diag_access_callback_state_t *)arg;
    UNUSED(byte_count);
...
/* Required Diagnostic Functions */
err_code_t merlin7_pcieg3_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint32_t addr, uint32_t data, uint16_t param) {
    INIT_SRDS_ERR_CODE
    srds_info_t * const merlin7_pcieg3_info_ptr = merlin7_pcieg3_INTERNAL_get_merlin7_pcieg3_info_ptr_with_check(sa__);
    merlin7_pcieg3_diag_access_callback_state_t state;
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(merlin7_pcieg3_acc_rdt_reg(sa__, (uint16_t)(i+addr),&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(merlin7_pcieg3_acc_rdt_reg(sa__, (uint16_t)addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin7_pcieg3_acc_wr_reg(sa__, (uint16_t)addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin7_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           addr + merlin7_pcieg3_info_ptr->core_var_ram_base,
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                                                           merlin7_pcieg3_diag_access_read_byte_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,merlin7_pcieg3_rdbc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 EFUN_PRINTF(("\n****  SERDES BLK LANE RAM READ BYTE   ****"));
                                 EFUN(merlin7_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + merlin7_pcieg3_info_ptr->lane_var_ram_base + (merlin7_pcieg3_acc_get_physical_lane(sa__) * merlin7_pcieg3_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,merlin7_pcieg3_rdbl_uc_var(sa__, __ERR, (uint16_t)addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin7_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           ...
                                                                           ((data+1)>>1)<<1, /* Round up to nearest word count */
                                                                           &state,
                                                                           merlin7_pcieg3_diag_access_read_word_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,merlin7_pcieg3_rdwc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,merlin7_pcieg3_rdwl_uc_var(sa__, __ERR, (uint16_t)addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_rdbc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(merlin7_pcieg3_wrbc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_rdbl_uc_var(sa__, __ERR, (uint16_t)addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(merlin7_pcieg3_wrbl_uc_var(sa__, (uint16_t)addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_rdwc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & (uint16_t)data));
                             EFUN(merlin7_pcieg3_wrwc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_rdwl_uc_var(sa__, __ERR,(uint16_t)addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & (uint16_t)data));
                             EFUN(merlin7_pcieg3_wrwl_uc_var(sa__, (uint16_t)addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,merlin7_pcieg3_INTERNAL_rdb_uc_var(sa__, __ERR, DATA_RAM_BASE | addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_INTERNAL_rdb_uc_var(sa__, __ERR, DATA_RAM_BASE | addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(merlin7_pcieg3_INTERNAL_wrb_uc_var(sa__, DATA_RAM_BASE | addr, tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,merlin7_pcieg3_INTERNAL_rdw_uc_var(sa__, __ERR, DATA_RAM_BASE | addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_INTERNAL_rdw_uc_var(sa__, __ERR, DATA_RAM_BASE | addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & (uint16_t)data));
                             EFUN(merlin7_pcieg3_INTERNAL_wrw_uc_var(sa__, DATA_RAM_BASE | addr, tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin7_pcieg3_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, (uint16_t)data, GRACEFUL_STOP_TIME));
    ...
    case SRDS_PROG_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Prog RAM Read byte: x%04x = x%02x\n",addr,merlin7_pcieg3_INTERNAL_rdb_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    case SRDS_PROG_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Prog RAM Read word: x%04x = x%04x\n",addr,merlin7_pcieg3_INTERNAL_rdw_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    case SRDS_EN_BREAKPOINT: {
        EFUN(merlin7_pcieg3_INTERNAL_en_breakpoint(sa__, (uint8_t)addr));
                             } break;
    case SRDS_GOTO_BREAKPOINT: {
        EFUN(merlin7_pcieg3_INTERNAL_goto_breakpoint(sa__, (uint8_t)addr));
                             } break;
    case SRDS_RD_BREAKPOINT: {
        EFUN(merlin7_pcieg3_INTERNAL_rd_breakpoint(sa__));
                             } break;
    case SRDS_DIS_BREAKPOINT: {
        EFUN(merlin7_pcieg3_INTERNAL_dis_breakpoint(sa__));
                             } break;
    ...
    default: EFUN_PRINTF(("Invalid request type merlin7_pcieg3_diag_access\n"));
    }

    return(ERR_CODE_NONE);
}

err_code_t merlin7_pcieg3_display_state (srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    EFUN(merlin7_pcieg3_display_core_state(sa__));
    EFUN(merlin7_pcieg3_display_lane_state_hdr(sa__));
    EFUN(merlin7_pcieg3_display_lane_state(sa__));
    EFUN(merlin7_pcieg3_display_lane_state_legend(sa__));
    return(ERR_CODE_NONE);
}
...
err_code_t merlin7_pcieg3_force_die_temperature (srds_access_t *sa__, int16_t die_temp) {
    INIT_SRDS_ERR_CODE
    /* disable force */
    if(die_temp == -255) {
