...
uint8_t merlin7_pcieg3_rdbl_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    ...
    {
        lane = merlin7_pcieg3_acc_get_physical_lane(sa__);


        lane_addr_offset = _merlin7_pcieg3_INTERNAL_get_addr_from_lane(sa__, addr,lane);
...
err_code_t merlin7_pcieg3_wrbls_uc_var(srds_access_t *sa__, uint16_t addr, int8_t wr_val) {
  return (merlin7_pcieg3_wrbl_uc_var(sa__, addr, (uint8_t)wr_val));
}
...
err_code_t merlin7_pcieg3_wrwls_uc_var(srds_access_t *sa__, uint16_t addr, int16_t wr_val) {
  return (merlin7_pcieg3_wrwl_uc_var(sa__, addr, (uint16_t)wr_val));
}

/* Micro RAM Lane Double Word Write */
err_code_t merlin7_pcieg3_wrdwl_uc_var(srds_access_t *sa__, uint16_t addr, uint32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(merlin7_pcieg3_wrwl_uc_var(sa__,addr,(uint16_t)(wr_val&0xFFFF)));
    EFUN(merlin7_pcieg3_wrwl_uc_var(sa__,(uint16_t)(addr + 0x2),(uint16_t)((wr_val>>16)&0xFFFF)));
    return ERR_CODE_NONE;
}

/* Micro RAM Lane Double Word Signed Write */
err_code_t merlin7_pcieg3_wrdwls_uc_var(srds_access_t *sa__, uint16_t addr, int32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(merlin7_pcieg3_wrdwl_uc_var(sa__, addr, (uint32_t)wr_val));
    return ERR_CODE_NONE;
}
...
uint8_t merlin7_pcieg3_rdbc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint8_t addr) {
...

    {
        ...
        core_addr_offset = _merlin7_pcieg3_INTERNAL_get_addr_from_core(sa__, addr);
...
err_code_t merlin7_pcieg3_wrbcs_uc_var(srds_access_t *sa__, uint8_t addr, int8_t wr_val) {
  return (merlin7_pcieg3_wrbc_uc_var(sa__, addr, (uint8_t)wr_val));
}
...
err_code_t merlin7_pcieg3_wrwcs_uc_var(srds_access_t *sa__, uint8_t addr, int16_t wr_val) {
  return(merlin7_pcieg3_wrwc_uc_var(sa__, addr, (uint16_t)wr_val));
}

/* Micro RAM Core Double Word Write */
err_code_t merlin7_pcieg3_wrdwc_uc_var(srds_access_t *sa__, uint8_t addr, uint32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(merlin7_pcieg3_wrwc_uc_var(sa__,addr,(uint16_t)(wr_val&0xFFFF)));
    EFUN(merlin7_pcieg3_wrwc_uc_var(sa__,(uint8_t)(addr + 0x2),(uint16_t)((wr_val>>16)&0xFFFF)));
    return ERR_CODE_NONE;
}

/* Micro RAM Core Double Word Signed Write */
err_code_t merlin7_pcieg3_wrdwcs_uc_var(srds_access_t *sa__, uint8_t addr, int32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(merlin7_pcieg3_wrdwc_uc_var(sa__, addr, (uint32_t)wr_val));
    return ERR_CODE_NONE;
}
...
uint32_t merlin7_pcieg3_rd_long_uc_prog_ram(srds_access_t *sa__, err_code_t *err_code_p, uint32_t addr) {
   INIT_SRDS_ERR_CODE
   uint32_t rddata;
   if(!err_code_p) {
        return(0);
...
err_code_t merlin7_pcieg3_rdblk_uc_prog_ram(srds_access_t *sa__, uint8_t *mem, uint32_t addr, uint32_t cnt) {
...
err_code_t merlin7_pcieg3_wr_long_uc_ram(srds_access_t *sa__, uint32_t addr, uint32_t value) {
    INIT_SRDS_ERR_CODE
    EFUN(merlin7_pcieg3_wrw_uc_ram(sa__, addr+2, (uint16_t)(value >> 16)));
    EFUN(merlin7_pcieg3_wrw_uc_ram(sa__, addr, value & 0xFFFF));
    return (ERR_CODE_NONE);
}

/* Micro RAM Word Write */
err_code_t merlin7_pcieg3_wrw_uc_ram(srds_access_t *sa__, uint32_t addr, uint16_t wr_val) {
    INIT_SRDS_ERR_CODE
...
err_code_t merlin7_pcieg3_wrb_uc_ram(srds_access_t *sa__, uint32_t addr, uint8_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(wrc_micro_autoinc_wraddr_en(0));
...
/* Micro RAM Long Read */
uint32_t merlin7_pcieg3_rd_long_uc_ram(srds_access_t *sa__, err_code_t *err, uint32_t addr) {
    INIT_SRDS_ERR_CODE
    uint32_t result;
    ESTM(result = (uint32_t)(merlin7_pcieg3_rdw_uc_ram(sa__, err, addr+2) << 16));
    ESTM(result |= merlin7_pcieg3_rdw_uc_ram(sa__, err, addr) & 0xFFFF);
    return result;
}
/* Micro RAM Word Read */
uint16_t merlin7_pcieg3_rdw_uc_ram(srds_access_t *sa__, err_code_t *err_code_p, uint32_t addr) {
   INIT_SRDS_ERR_CODE
   uint16_t rddata;
   if(!err_code_p) {
        return(0);
...
/* Micro RAM Byte Read */
uint8_t merlin7_pcieg3_rdb_uc_ram(srds_access_t *sa__, err_code_t *err_code_p, uint32_t addr) {
    INIT_SRDS_ERR_CODE
    uint8_t rddata;
    if(!err_code_p) {
        return(0);
...
err_code_t merlin7_pcieg3_wrws_uc_ram(srds_access_t *sa__, uint32_t addr, int16_t wr_val) {
    return (merlin7_pcieg3_wrw_uc_ram(sa__, addr, (uint16_t)wr_val));
}

/* Micro RAM Byte Signed Write */
err_code_t merlin7_pcieg3_wrbs_uc_ram(srds_access_t *sa__, uint32_t addr, int8_t wr_val) {
    return (merlin7_pcieg3_wrb_uc_ram(sa__, addr, (uint8_t)wr_val));
}

/* Micro RAM Word Signed Read */
int16_t merlin7_pcieg3_rdws_uc_ram(srds_access_t *sa__, err_code_t *err_code_p, uint32_t addr) {
    return ((int16_t)merlin7_pcieg3_rdw_uc_ram(sa__, err_code_p, addr));
}

/* Micro RAM Byte Signed Read */
int8_t merlin7_pcieg3_rdbs_uc_ram(srds_access_t *sa__, err_code_t *err_code_p, uint32_t addr) {
    return ((int8_t)merlin7_pcieg3_rdb_uc_ram(sa__, err_code_p, addr));
}

/* Micro RAM Block Read */
err_code_t merlin7_pcieg3_rdblk_uc_ram(srds_access_t *sa__, uint8_t *mem, uint32_t addr, uint32_t cnt) {
    merlin7_pcieg3_INTERNAL_rdblk_callback_arg_t arg;
    ...
    arg.mem_ptr = mem;
    return merlin7_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__, addr | DATA_RAM_BASE, cnt, 0, cnt, &arg, merlin7_pcieg3_INTERNAL_rdblk_callback);
}
