...
int tscf_phy_fec_error_bits_counter_get(const phymod_phy_access_t* phy, uint32_t* count)
{
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (tefmod_fec_cl91_bit_error_counter_get(&phy_copy.access, &local_count));
                                       ...
                                       phymod_fec_error_mask_t bit_error_mask)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (tefmod_FEC_get(&phy->access, TEFMOD_CL91, &cl91_en));

    if (model == TSCF_GEN2_MODEL) {
        PHYMOD_IF_ERR_RETURN
            (tefmod_FEC_get(&phy->access, TEFMOD_CL108, &cl108_en));
    }
