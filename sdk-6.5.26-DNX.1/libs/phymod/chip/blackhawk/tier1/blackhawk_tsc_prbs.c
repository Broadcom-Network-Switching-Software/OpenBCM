...
err_code_t blackhawk_tsc_get_prbs_chk_hw_timer_ctrl(srds_access_t *sa__, struct blackhawk_tsc_prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl_bak) {
...
err_code_t blackhawk_tsc_set_prbs_chk_hw_timer_ctrl(srds_access_t *sa__, struct blackhawk_tsc_prbs_chk_hw_timer_ctrl_st const * const prbs_chk_hw_timer_ctrl_bak) {
...
err_code_t blackhawk_tsc_config_prbs_chk_hw_timer(srds_access_t *sa__, uint16_t time_ms, uint16_t *time_ms_adjusted) {
    ...
    struct blackhawk_tsc_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl;
    USR_MEMSET(&prbs_chk_hw_timer_ctrl, 0, sizeof(struct blackhawk_tsc_prbs_chk_hw_timer_ctrl_st));
...
err_code_t blackhawk_tsc_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk_tsc_prbs_err_analyzer_errcnt_st *err_analyzer) {
    ...
    if(!prbs_lock) {
        ...
        for (i=9; i > 0; i--) {
            if ((frame_errors[i-1] != 0xFFFF) && (frame_errors[i-1] != 0xFFFFF)) {
                if ((i-1)!=8) {
                    if (frame_errors[i] == 0xFFFFFFFF) {
                        frame_errors[i-1] = 0xFFFFFFFF;
                    }
                    else {
                        frame_errors[i-1] = (frame_errors[i-1] + frame_errors[i]);
                    }
                }
            }
            else {
...
err_code_t blackhawk_tsc_optimize_hist_errcnt_thresh(srds_access_t *sa__, blackhawk_tsc_prbs_err_analyzer_errcnt_st *err_analyzer, uint32_t timeout_check, uint32_t timeout_s) {
...
#ifdef PHYMOD_SUPPORT
    for(i = BLACKHAWK_TSC_PRBS_ERR_ANALYZER_NUM_COUNTERS; i>0 ; i--) {
        /*
         * 64-bit operations use COMPILER_64_XXX macros
         */
        /* tmp_err = (uint64_t)timeout_s * err_analyzer->prbs_errcnt[i-1] */
        COMPILER_64_SET(tmp_err, 0, (uint32_t)1);
        COMPILER_64_UMUL_32(tmp_err, timeout_s);
        COMPILER_64_UMUL_32(tmp_err, err_analyzer->prbs_errcnt[i-1]);
        ...
        if (COMPILER_64_GT(tmp_err, tmp_sat)) {
            errcnt_sat = 1;
            break;
        }
    }
#else
    for(i = BLACKHAWK_TSC_PRBS_ERR_ANALYZER_NUM_COUNTERS; i>0 ; i--) {
        if((uint64_t)timeout_s * err_analyzer->prbs_errcnt[i-1] > (uint64_t)timeout_check * (0xFFFF)) {
            errcnt_sat = 1;
            break;
        }
    }
    ...
    if((errcnt_sat == 1) && ((i-1) != 0)) {
        if(((i-1)+err_analyzer->hist_errcnt_thresh) >= BLACKHAWK_TSC_PRBS_ERR_ANALYZER_HIST_ERR_THRESH_MAX) {
            ...
            err_analyzer->hist_errcnt_thresh = (uint8_t)(err_analyzer->hist_errcnt_thresh + (i-1));
