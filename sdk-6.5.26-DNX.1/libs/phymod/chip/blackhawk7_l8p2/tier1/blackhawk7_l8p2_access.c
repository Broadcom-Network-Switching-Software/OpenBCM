...
uint8_t blackhawk7_l8p2_rdbl_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
...

    {
        ...
        lane_addr_offset = _blackhawk7_l8p2_INTERNAL_get_addr_from_lane(sa__, addr,lane);
...
err_code_t blackhawk7_l8p2_wrdwl_uc_var(srds_access_t *sa__, uint16_t addr, uint32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(blackhawk7_l8p2_wrwl_uc_var(sa__,addr,(uint16_t)(wr_val&0xFFFF)));
    EFUN(blackhawk7_l8p2_wrwl_uc_var(sa__,(uint16_t)(addr + 0x2),(uint16_t)((wr_val>>16)&0xFFFF)));
    return ERR_CODE_NONE;
}

/* Micro RAM Lane Double Word Signed Write */
err_code_t blackhawk7_l8p2_wrdwls_uc_var(srds_access_t *sa__, uint16_t addr, int32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(blackhawk7_l8p2_wrdwl_uc_var(sa__, addr, (uint32_t)wr_val));
    return ERR_CODE_NONE;
}
...
uint8_t blackhawk7_l8p2_rdbuc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
  ...
  uc_addr_offset = _blackhawk7_l8p2_INTERNAL_get_uc_addr_from_lane(sa__, addr,lane);
...
err_code_t blackhawk7_l8p2_wrdwuc_uc_var(srds_access_t *sa__, uint16_t addr, uint32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(blackhawk7_l8p2_wrwuc_uc_var(sa__,addr,(uint16_t)(wr_val&0xFFFF)));
    EFUN(blackhawk7_l8p2_wrwuc_uc_var(sa__,(uint16_t)(addr + 0x2),(uint16_t)((wr_val>>16)&0xFFFF)));
    return ERR_CODE_NONE;
}

/* Micro RAM UC Double Word Signed Write */
err_code_t blackhawk7_l8p2_wrdwucs_uc_var(srds_access_t *sa__, uint16_t addr, int32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(blackhawk7_l8p2_wrdwuc_uc_var(sa__, addr, (uint32_t)wr_val));
    return ERR_CODE_NONE;
}
...
uint8_t blackhawk7_l8p2_rdbc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint8_t addr) {
...

    {
        ...
        core_addr_offset = _blackhawk7_l8p2_INTERNAL_get_addr_from_core(sa__, addr);
...
err_code_t blackhawk7_l8p2_wrdwc_uc_var(srds_access_t *sa__, uint8_t addr, uint32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(blackhawk7_l8p2_wrwc_uc_var(sa__,addr,(uint16_t)(wr_val&0xFFFF)));
    EFUN(blackhawk7_l8p2_wrwc_uc_var(sa__,(uint8_t)(addr + 0x2),(uint16_t)((wr_val>>16)&0xFFFF)));
    return ERR_CODE_NONE;
}

/* Micro RAM Core Double Word Signed Write */
err_code_t blackhawk7_l8p2_wrdwcs_uc_var(srds_access_t *sa__, uint8_t addr, int32_t wr_val) {
    INIT_SRDS_ERR_CODE
    EFUN(blackhawk7_l8p2_wrdwc_uc_var(sa__, addr, (uint32_t)wr_val));
    return ERR_CODE_NONE;
}
