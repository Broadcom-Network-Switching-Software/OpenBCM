...
int tsce_core_lane_map_get(const phymod_core_access_t* core,
                           phymod_lane_map_t* lane_map)
{
    ...
    PHYMOD_IF_ERR_RETURN(temod_pmd_lane_swap_tx_get(&core->access,
                                                    &pmd_tx_swap));
    ...
    for (lane = 0 ; lane < TSCE_NOF_LANES_IN_CORE ; lane++) {
        ...
        lane_map->lane_map_rx[lane] =
                             (pcs_swap >> (lane*4)) & TSCE_LANE_SWAP_LANE_MASK;
        /*
         * considering the pcs lane swap:
         * tx_map[lane] = pmd_map[pcs_map[lane]]
         */
        lane_map->lane_map_tx[lane] =
                             (pmd_swap >> (lane_map->lane_map_rx[lane] * 4)) &
                              TSCE_LANE_SWAP_LANE_MASK;
    }
...
int tsce_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    if (fw_config.LpDfeOn != 0) {
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_UNAVAIL, (_PHYMOD_MSG("LpDfe is not supported!\n")));
    }
    ...
    PHYMOD_IF_ERR_RETURN
        (temod_port_start_lane_get(&phy->access,
                                   &port_starting_lane,
                                   &port_num_lane));
...
int tsce_core_pll_sequencer_restart(const phymod_core_access_t* core, uint32_t flags, phymod_sequencer_operation_t operation)
{
    switch (operation) {
        ...
        case phymodSeqOpStart:
            ...
            /* need to fill out the timeout value properly */
            /* PHYMOD_IF_ERR_RETURN
                          ...
                          const phymod_core_status_t* core_status)
{
  ...
  /* NEED TO REVISIT */
    ...
    /*cl48 lfrfli has 2 register copies*/
    phy_access_copy.access.lane_mask = 0x5;
    PHYMOD_IF_ERR_RETURN
        (temod_cl48_lfrfli_init(&phy_access_copy.access));
    phy_access_copy.access.lane_mask = 0x1;
