...
int _blackhawk7_v1l8p1_core_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config)
{
    ...
    case phymodFirmwareLoadMethodExternal:
        if(!PHYMOD_CORE_INIT_F_RESUME_AFTER_FW_LOAD_GET(init_config)) {
            ...
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p1_pram_firmware_enable(&core_copy.access, 1, wait));
...
/* This function gives the PMD rx signal detect live status*/
...
int blackhawk7_v1l8p1_phy_signal_detect_status_get(const phymod_phy_access_t* phy,
                                                   phymod_signal_detect_status_t* sd_status)
{
    ...
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_signal_detect_status_get(&pm_phy_copy.access,
...
int blackhawk7_v1l8p1_phy_rx_dc_offset_get(const phymod_phy_access_t* phy,  uint32_t *rx_dc_offset)
{
    int start_lane, num_lane;
    phymod_phy_access_t phy_copy;
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_rx_dc_offset_get(&phy_copy.access, rx_dc_offset));
