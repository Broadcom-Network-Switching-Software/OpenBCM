...
err_code_t falcon_tsc_cl93n72_frc_byte_set(const phymod_access_t *pa, uint32_t data)
{
    ...
    __err = wrv_usr_ctrl_cl93n72_frc_byte(data);
    if(__err) return(__err);
...
err_code_t falcon_tsc_cl93n72_frc_byte_get(const phymod_access_t *pa, uint32_t *data)
{
    ...
    *data = rdv_usr_ctrl_cl93n72_frc_byte();

    return ERR_CODE_NONE;
}

err_code_t
falcon_rx_pmd_lock_status_get(const phymod_access_t *pa, uint32_t *pmd_lock, uint32_t *pmd_lock_change)
{
    int start_lane, num_lane, lane_offset;
    phymod_access_t phy_access_copy;
    TLB_RX_PMD_RX_LOCK_STSr_t pmd_lock_status_reg;

    PHYMOD_MEMCPY(&phy_access_copy, pa, sizeof(phy_access_copy));
    *pmd_lock_change = 0;
    *pmd_lock        = 0;

    PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(pa, &start_lane, &num_lane));

    for (lane_offset = 0; lane_offset < num_lane; lane_offset++) {
        if (!PHYMOD_LANEPBMP_MEMBER(pa->lane_mask, start_lane + lane_offset)) {
            continue;
        }
...
err_code_t falcon_tsc_rx_dc_offset_get(const phymod_access_t *pa, uint32_t *dc_offset)
{
    err_code_t __err;
    __err = ERR_CODE_NONE;

    *dc_offset = rd_dc_offset_bin() & 0x7f;
    if(__err) return(__err);
