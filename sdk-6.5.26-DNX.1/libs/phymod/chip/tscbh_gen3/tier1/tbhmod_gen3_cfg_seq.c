...
int tbhmod_gen3_set_an_timers(PHYMOD_ST* pc, phymod_ref_clk_t refclk, uint32_t *pam4_an_timer_value)    /* SET_AN_TIMERS */
{
    ...
    if (refclk == phymodRefClk312Mhz) {
        ...
        MAIN0_TICK_CTL0r_CLR(MAIN0_TICK_CONTROL_0r_reg);
        MAIN0_TICK_CTL0r_TICK_DENOMINATORf_SET(MAIN0_TICK_CONTROL_0r_reg, 1);
        MAIN0_TICK_CTL0r_TICK_NUMERATOR_LOWERf_SET(MAIN0_TICK_CONTROL_0r_reg, 0xf);

        PHYMOD_IF_ERR_RETURN (MODIFY_MAIN0_TICK_CTL0r(pc, MAIN0_TICK_CONTROL_0r_reg));

        MAIN0_TICK_CTL1r_CLR(MAIN0_TICK_CONTROL_1r_reg);
        MAIN0_TICK_CTL1r_TICK_OVERRIDEf_SET(MAIN0_TICK_CONTROL_1r_reg, 1);
        MAIN0_TICK_CTL1r_TICK_NUMERATOR_UPPERf_SET(MAIN0_TICK_CONTROL_1r_reg, 0x249);
        PHYMOD_IF_ERR_RETURN (MODIFY_MAIN0_TICK_CTL1r(pc, MAIN0_TICK_CONTROL_1r_reg));
    }
    ...
    AN_X1_LNK_FAIL_INHBT_TMR_CL72r_SET(AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_CL72r_reg, 0x208e);
    ...
    AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72r_SET(AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_NOT_CL72r_reg, 0xa6c);
    ...
    if (pam4_an_timer_value == NULL) {
        ...
        AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_SET(AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_CL72_PAM4r_reg, 0x3416);
        ...
        AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72_PAM4r_SET(AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_NOT_CL72_PAM4r_reg, 0x106a);
...
int tbhmod_gen3_an_link_fail_inhibit_timer_set(PHYMOD_ST* pc, tbhmod_gen3_an_timer_t timer_type, uint32_t period)
{
    AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_t AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_CL72_PAM4r_reg;
    uint16_t tick_count;

    switch (timer_type) {
        case TBHMOD_GEN3_AN_FAIL_INHIBIT_TIMER_LT_PAM4:
...
int tbhmod_gen3_an_link_fail_inhibit_timer_get(PHYMOD_ST* pc, tbhmod_gen3_an_timer_t timer_type, uint32_t* period)
{
    AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_t AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_CL72_PAM4r_reg;
    uint16_t tick_count;
...
int tbhmod_gen3_fec_align_status_get(PHYMOD_ST* pc, uint32_t *fec_align_live)
{
...
int tbhmod_gen3_pcs_ts_config(PHYMOD_ST *pc, int ts_offset)
{
    RX_X2_RX_TS_CTLr_t RX_X2_RX_TS_CTLr_reg;
...
float _tbhmod_gen3_pcs_vco_to_rx_clk_period(uint32_t vco, int os_mode, int pam4)
{
    float clk_period;
    float bit_ui_val;
    ...
    clk_period = bit_ui_val * (pam4 ? 40:20);
...
/*
 * Update the Rx Table with
 * pm_offset (pmd staging delay)
 * rx_clk adjustment
 */

int tbhmod_gen3_ts_rx_table_adjust_set(PHYMOD_ST* pc, uint32_t vco,
                                       int os_mode, int is_pam4,
                                       int tbl_ln, uint32_t *table)
{
#ifdef PHYMOD_CONFIG_INCLUDE_FLOATING_POINT
    PMD_X1_PM_TMR_OFFSr_t PMD_X1_PM_TMR_OFFSr_reg;
    ...
    int i;
    float rx_clk_period;
    uint32_t rx_clk_offset_ns = 0;
    uint32_t rx_clk_offset_sub_ns = 0;
    uint32_t adjustment_ns = 0;
    uint32_t adjustment_sub_ns = 0;

    /*
     * Calculate the rx clock adjustment.
     * Get sub ns by multiplying the fraction with 256 (2^8, 8 bit sub ns)
     */
    rx_clk_period = _tbhmod_gen3_pcs_vco_to_rx_clk_period(vco, os_mode, is_pam4);

    rx_clk_offset_ns        =  4.0 * rx_clk_period;
    rx_clk_offset_sub_ns    = ((4.0 * rx_clk_period) - rx_clk_offset_ns) * 256.0;

    /*
     * Get the PM Offset, PMD staging delay.
     */
    PMD_X1_PM_TMR_OFFSr_CLR(PMD_X1_PM_TMR_OFFSr_reg);
     ...
     * PM offset + Rx Clk adjustment.
     * If total sub ns is more than 0xFF, added to ns.
     */
    adjustment_sub_ns   = rx_clk_offset_sub_ns + pm_offset_sub_ns;
    adjustment_ns       = pm_offset_ns + rx_clk_offset_ns + (adjustment_sub_ns >> 8);
     ...
     */
    pm_offset = ((adjustment_ns & 0xff) << 8) | (adjustment_sub_ns & 0xff);
...
#endif /* PHYMOD_CONFIG_INCLUDE_FLOATING_POINT */
    return PHYMOD_E_NONE;
}
...
int tbhmod_gen3_1588_ts_valid_get(PHYMOD_ST* pc, uint16_t* ts_valid)
{
...
int tbhmod_gen3_fec_error_inject_set(const PHYMOD_ST *pc,
                                     uint16_t error_control_map,
                                     tbhmod_gen3_fec_error_mask_t bit_error_mask)
{
...
int tbhmod_gen3_fec_error_inject_get(const PHYMOD_ST *pc,
                                     uint16_t* error_control_map,
                                     tbhmod_gen3_fec_error_mask_t* bit_error_mask)
{
    int start_lane = 0, num_lane = 0;
...
int tbhmod_gen3_fec_error_bits_counter_get (PHYMOD_ST* pc, uint32_t* count)
{
    int start_lane, num_lane;
    uint32_t count_32 = 0;
...
int tbhmod_gen3_mapped_speed_get_osmode (int mapped_speed_id, tbhmod_gen3_refclk_t ref_clk)
{
  if (ref_clk == TBHMOD_GEN3_REF_CLK_312P5MHZ) {
     return tscbh_gen3_sc_pmd_entry_312M_ref[mapped_speed_id].t_pma_os_mode;
  } else {
     return tscbh_gen3_sc_pmd_entry[mapped_speed_id].t_pma_os_mode;
  }
}
