...
/**************************************************************************************
 **************************************************************************************
 *  File Name     :  blackhawk7_l2p2_pwr_mgt.c                                             *
...
err_code_t blackhawk7_l2p2_core_config_for_iddq(srds_access_t *sa__) {
    UNUSED(sa__);
    return (ERR_CODE_NONE);
}


err_code_t blackhawk7_l2p2_lane_config_for_iddq(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_core_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    INIT_SRDS_ERR_CODE

    switch(mode) {
    case PWR_ON:
        EFUN(blackhawk7_l2p2_INTERNAL_core_clkgate(sa__, 0));
    ...
    case PWRDN_DEEP:
        ...
        EFUN(blackhawk7_l2p2_INTERNAL_core_clkgate(sa__, 1));
        BLACKHAWK7_L2P2_UAPI_NOT_AMS_A0_ERR  /* if not BLACKHAWK7_V1LXP2, print error */
        break;
    default:
...
err_code_t blackhawk7_l2p2_lane_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    INIT_SRDS_ERR_CODE

    switch(mode) {
    case PWR_ON:
        EFUN(blackhawk7_l2p2_INTERNAL_lane_clkgate(sa__, 0));
        EFUN(blackhawk7_l2p2_lane_pwrdn(sa__, PWR_ON_RX));
        EFUN(blackhawk7_l2p2_lane_pwrdn(sa__, PWR_ON_TX));
        break;
    case PWR_ON_TX:
    ...
    case PWR_ON_RX:
        ...
        break;
    case PWRDN:
        EFUN(blackhawk7_l2p2_lane_pwrdn(sa__, PWRDN_TX));
        EFUN(blackhawk7_l2p2_lane_pwrdn(sa__, PWRDN_RX));
        break;
    case PWROFF_DEEP: /* PWROFF_DEEP is identical to PWRDN_DEEP for lane pwrdn. FALL THROUGH */
    case PWRDN_DEEP:
        EFUN(blackhawk7_l2p2_lane_pwrdn(sa__, PWRDN_TX_DEEP));
        EFUN(blackhawk7_l2p2_lane_pwrdn(sa__, PWRDN_RX_DEEP));
        EFUN(blackhawk7_l2p2_INTERNAL_lane_clkgate(sa__, 1));
        break;
    case PWRDN_TX:
    ...
    case PWRDN_TX_DEEP:
        ...
        EFUN(blackhawk7_l2p2_set_tx_osr_mode(sa__, BLACKHAWK7_L2P2_OSX4));
        BLACKHAWK7_L2P2_UAPI_NOT_AMS_B0_ERR /* if !BLACKHAWK7_V2LXP2 and !BLACKHAWK7_V3LXP2, print error */
        EFUN(wr_ln_tx_s_pwrdn(0x1));
        break;
    case PWRDN_RX_DEEP:
        EFUN(wr_rx_ln_dp_s_rstb(0x0));
        BLACKHAWK7_L2P2_UAPI_INIT_AMS_A0    /* if BLACKHAWK7_V1LXP2 */
        EFUN(blackhawk7_l2p2_set_rx_osr_mode(sa__, BLACKHAWK7_L2P2_OSX4));
        BLACKHAWK7_L2P2_UAPI_NOT_AMS_B0_ERR /* if !BLACKHAWK7_V2LXP2 and !BLACKHAWK7_V3LXP2, print error */
        EFUN(wr_ln_rx_s_pwrdn(0x1));
        break;
    default:
...
err_code_t blackhawk7_l2p2_pwrdn_all(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    INIT_SRDS_ERR_CODE
    ...
    for (lane = 0; lane < lanes_per_core; ++lane) {
        ...
        EFUN(blackhawk7_l2p2_lane_pwrdn(sa__, mode));
    }
    EFUN(blackhawk7_l2p2_acc_set_lane(sa__, lane_orig));
    {
      ...
      for (pll = 0; pll < NUM_PLLS; ++pll) {
        ...
        EFUN(blackhawk7_l2p2_core_pwrdn(sa__, mode));
      }
