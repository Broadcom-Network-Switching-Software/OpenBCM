...
#ifndef BLACKHAWK7_L2P2_API_PWR_MGT_H
#define BLACKHAWK7_L2P2_API_PWR_MGT_H
 ...
 * To complete a blackhawk7_l2p2_core_pwrdn(), all of the lane(s) belonging to the core being powered down.
 * should also be issued a lane power down using blackhawk7_l2p2_lane_pwrdn().
 * Note: To wake up from any powerdown mode, first call PWR_ON and then de-assert core_dp_reset [blackhawk7_l2p2_core_dp_reset(sa__, 0)].
 ...
 * Note: User also needs to configure all lanes through blackhawk7_l2p2_lane_config_for_iddq()
 ...
 * Note: User needs to configure all lanes through blackhawk7_l2p2_lane_config_for_iddq() and also call blackhawk7_l2p2_core_config_for_iddq()
 * before enabling IDDQ by asserting IDDQ pin.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
 ...
 *       De-assert with blackhawk7_l2p2_tx_dp_reset(..) when waking up from a TX only PWRDN, blackhawk7_l2p2_rx_dp_reset(..)
 *       for a RX only PWRDN, and blackhawk7_l2p2_ln_dp_reset(..) for both RX and TX.
