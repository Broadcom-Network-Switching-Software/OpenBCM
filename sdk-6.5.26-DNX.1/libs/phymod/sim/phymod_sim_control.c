...
#include <phymod/chip/tscbh_sub10g_sim.h>
#endif
#ifdef PHYMOD_TSCD_SUPPORT
#include <phymod/chip/tscd_sim.h>
#endif
...
phymod_control_physim_drv_get(int unit, phymod_dispatch_type_t dispatch_type, phymod_sim_drv_t **drv)
{
    ...
    switch(dispatch_type) {
        ...
        case phymodDispatchTypeBlackhawk7_v1l8p1:
            *drv = &blackhawk7_v1l8p1_sim_drv;
            break;
#endif
#ifdef PHYMOD_TSCBH_SUB10G_SUPPORT
        case phymodDispatchTypeTscbh_sub10g:
            *drv = &tscbh_sub10g_sim_drv;
            break;
#endif
#ifdef PHYMOD_TSCD_SUPPORT
        case phymodDispatchTypeTscd:
            *drv = &tscd_sim_drv;
            break;
#endif
        default:
...
int
phymod_control_physim_read32(int unit, uint32_t phy_id,
                             uint32_t phy_reg_addr,
                             uint32_t *phy_rd_data32)
{
    phymod_control_phy_sim_t *psim = phymod_control_physim_find(unit, phy_id);
    ...
    PHYMOD_IF_ERR_RETURN(
        phymod_sim_read32(&psim->pms, phy_reg_addr, phy_rd_data32));

    PHYMOD_DEBUG_VERBOSE(
            ("phymod_control_physim_read32 0x%03x:0x%04x = 0x%08x\n",
              phy_id, phy_reg_addr, *phy_rd_data32));
    return PHYMOD_E_NONE;
}

int
phymod_control_physim_write32(int unit, uint32_t phy_id,
                              uint32_t phy_reg_addr,
                              uint32_t phy_wr_data32)
{
    phymod_control_phy_sim_t *psim = phymod_control_physim_find(unit, phy_id);
    ...
    PHYMOD_IF_ERR_RETURN(
        phymod_sim_write32(&psim->pms, phy_reg_addr, phy_wr_data32));

    PHYMOD_DEBUG_VERBOSE(
            ("phymod_control_physim_wrmask 0x%03x:0x%04x = 0x%08x\n",
              phy_id, phy_reg_addr, phy_wr_data32));
    return PHYMOD_E_NONE;
}
