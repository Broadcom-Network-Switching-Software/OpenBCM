...
/* An environment variable name used to override the default device name */
#define LUBDE_DEVICE_NAME_ENV_VAR "BCM_USER_BDE_DEVICE_NAME"
...
/* An environment variable name used to override the default device name */
#define LKBDE_DEVICE_NAME_ENV_VAR "BCM_KERNEL_BDE_DEVICE_NAME"
 ...
 */
static linux_bde_device_bitmap_t _inst_dev_mask = {0}; /* A bitmap specifying which devices this SDK instance controls */
#ifdef BCM_INSTANCE_SUPPORT
static unsigned int              _inst_dma_size = 0; /* DMA memory size available for this SDK instance */
#endif
static unsigned int              _inst_num = BDE_DEV_INST_ID_INVALID; /* the number of the current SDK instance */
#ifdef BCM_INSTANCE_SUPPORT
static int                       _is_an_instance = 0; /* Does this SDK not control all the devices */
#define IS_AN_INSTANCE() (_is_an_instance)
#else
#define IS_AN_INSTANCE() 0
#endif
...
static sal_mutex_t iproc_map_lock[LINUX_BDE_MAX_DEVICES] = {NULL};
...
_devio_remap(unsigned int command, lubde_ioctl_t *pdevio)
{
    ...
    idx = pdevio->dev;
    if (idx >= LINUX_BDE_MAX_DEVICES || _devices[idx] == NULL) {
        printf("Device %u of the SDK instance does not exist.\n",idx);
        return -1;
    }
...
{
    pdevio->rc = -1;
    if (_devio_remap(command, pdevio) != 0 ) {
        return -1;
    }
...
#ifdef BCM_INSTANCE_SUPPORT
static int
_bde_instance_attach()
{
    lubde_ioctl_t devio;

#ifdef static_assert
    static_assert(sizeof(devio.dx.dw) >= sizeof(linux_bde_device_bitmap_t), "dx_too_small");
#endif

    /* Initialize the variable */
    memset(&devio, 0, sizeof(lubde_ioctl_t));

    memcpy(devio.dx.dw, _inst_dev_mask, sizeof(linux_bde_device_bitmap_t));
    devio.d0 = _inst_num;
    devio.d1 = _inst_dma_size;
 ...
 *
 * Purpose:
 *    Helper function for calling system() library function
 * Parameters:
...
_system(const char *command)
{
    ...
    if (rc == -1){
        printf("linux-user-bde: could not run %s.\n", command);
    } else {
        if (WIFEXITED(rc) && (WEXITSTATUS(rc) == 0)) {
            return;
        } else {
            printf("linux-user-bde: exit status of %s was %d.\n", command, WEXITSTATUS(rc));
        }
    }
}
 ...
 * Purpose:
 *    BDE probe function.
 *    Probe all available devices and initialize the devices information.
 *    Reprobe the device to update information in hotswap case.
 * Parameters:
 *    None
 * Returns:
 *    0 - Success
...
_probe()
{
    ...
    if(!IS_AN_INSTANCE()) {
        ...
        /* Include all possible devices for non-instance mode */
        for (i = 0; i <  (LINUX_BDE_MAX_DEVICES - 1) / 32; ++i) {
          _inst_dev_mask[i] = 0xffffffff;
        }
        _inst_dev_mask[i] = (((uint32)1) << (LINUX_BDE_MAX_DEVICES % 32)) - 1;
    }

    /* get dma pool information and its handling mode */
    _get_dma_info(&_cpu_pbase, &_dma_pbase, &_dma_size);
    /* If there is no DMA memory, Check that there is no device requiring it */
    if (_dma_size == 0) {
        for (dev_no = 0; dev_no < num_devices; dev_no++) {
            /* Check if the device is being controlled by this instance ans is no removed */
            if ((_inst_dev_mask[dev_no / 32] & (1 << (dev_no % 32))) != 0 &&
                _get_dev_state(dev_no) != BDE_DEV_STATE_REMOVED) {

                /* Get the type of device */
                memset(&devio, 0, sizeof(lubde_ioctl_t));
                devio.dev = dev_no;
                devio.rc = -1;
                assert(ioctl(_devfd, LUBDE_GET_DEVICE_TYPE, &devio) == 0 && devio.rc == 0);

                if ((devio.d0 & BDE_USER_DEV_TYPE) == 0) { /* Check if the device needs DMA memory like most devices do */
                    printf("linux-user-bde: There is no DMA memory, and we found that device %d requires it\n", dev_no);
                    assert(0);
                    break;
                }
            }
    ...
    for (dev_no = 0; dev_no < num_devices; dev_no++) {
        /* Skip the dev not belong to this SDK instance. */
        if ((_inst_dev_mask[dev_no / 32] & (1 << (dev_no % 32))) == 0) {
            continue;
        }
        /* Check if unit has been probed before. */
        for (u = 0; u < _ndevices; u++) {
            if (_devices[u] != NULL) {
                if (_devices[u]->dev_id == dev_no) {
                    break;
                }
            }
        }
        ...
        if (dev_state == BDE_DEV_STATE_REMOVED) {
            printf("linux-user-bde: device(%d) was removed.\n", dev_no);
            continue;
        } else if (dev_state == -1) {
        ...
        if (u >= _ndevices) {
            /* New probed device */
            /* Invoke kernel space user bde to reprobe */
#ifndef NDEBUG
            _ioctl_LUBDE_REPROBE =
#endif /* !NDEBUG */
            _ioctl(LUBDE_REPROBE, &devio);
            assert(_ioctl_LUBDE_REPROBE == 0);

            if (iproc_map_lock[u] == NULL) {
                iproc_map_lock[u] = sal_mutex_create("iproc_map_mutex");
                if (iproc_map_lock[u] == NULL) {
                    return -1;
                }
            }

            _devices[u] = (bde_dev_t*)malloc(sizeof(bde_dev_t));
            memset(_devices[u], 0, sizeof(bde_dev_t));
            iproc_map[u] = iproc_map_default;

            _devices[u]->dev_id = dev_no;
            /* Get the type of device */
            memset(&devio, 0, sizeof(lubde_ioctl_t));
            devio.dev = u;
#ifndef NDEBUG
            ...
            _ndevices++;
            _devices[u]->dev_type = dev_type;
            reprobe = 1;
            printf("linux-user-bde:new probed device "
                   "unit %d dev_no %d _ndevices %d\n",
                   u, dev_no, _ndevices);
        } else {
            /* Device probed before */
            if (dev_state == BDE_DEV_STATE_CHANGED) {
                dev_type = _devices[u]->dev_type;
                reprobe = 1;
                printf("linux-user-bde:reprobe unit %d\n",u);
            } else {
                printf("linux-user-bde:skip unit %d\n",u);
                continue;
            }
        }
        if (!reprobe) {
        ...
        _devices[u]->bde_dev.device = devio.d0;
        _devices[u]->bde_dev.rev = devio.d1;
        _devices[u]->bde_dev.dev_unique_id = devio.dx.dw[0];
        ...
        if (BDE_DEV_MEM_MAPPED(dev_type)) {
            ...
            if (dev_type & BDE_128K_REG_SPACE) {
                size = 128 * 1024;
#if defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT) || defined(BCM_CMICX_SUPPORT)
            ...
            pbase = devio.d2;
#endif /* PHYS_ADDRS_ARE_64BITS */
            _devices[u]->pbase = pbase;

            switch (_devices[u]->bde_dev.device) {
            case GEDI_DEVICE_ID:
            ...
            {
            /* vbase Will either hold the DMA buffer pool in th process virtual address space, or NULL if DMA may not be used */
            _devices[u]->vbase = (dev_type & BDE_USER_DEV_TYPE) == 0 ? _mmap(pbase, size) : NULL;
            _devices[u]->bde_dev.base_address = (sal_vaddr_t)_devices[u]->vbase;
            }
            ...
            if (_ioctl(LUBDE_DEV_RESOURCE, &devio) == 0) {
                ...
                if (pbase && !(dev_type & BDE_AXI_DEV_TYPE)) {
                    _devices[u]->vbase1 = _mmap(pbase, size);
                    if (!(dev_type & BDE_NO_IPROC)) {
                        /* Cache IMAP0 registers from the current PAXB */
                        for (j = 0; j < IPROC_SUBWIN_MAX; j++) {
                            rval = _devices[u]->vbase1[(BAR0_PAXB_IMAP0_0/sizeof(uint32))+j];
                            if (rval & 1) { /* Valid Bit */
                                iproc_map[u].subwin[j].addr_min = rval & ~(0xfff);
                                iproc_map[u].subwin[j].addr_max = rval | 0xfff;
                            } else {
                                iproc_map[u].subwin[j].addr_min = 0;
                                iproc_map[u].subwin[j].addr_max = 0;
                            }
                        }
                        if (iproc_map[u].subwin[2].addr_min  & 0x1000) {
                            /* PAXB_1 */
...
#ifdef DUNE_GTO_I2C
        _devices[u]->dev_type |= BDE_I2C_DEV_TYPE;
        _devices[u]->dev_type &= ~BDE_PCI_DEV_TYPE;
        switch (_devices[u]->bde_dev.device) {
        case GEDI_DEVICE_ID:
            _devices[u]->i2c_addr = GFA_BI_I2C_PCP_DEVICE_ADDR;
            _devices[u]->i2c_access_type = CPU_I2C_ALEN_LONG_DLEN_LONG;
            break;
    ...
    case BCM88687_DEVICE_ID:
        ...
        switch (_devices[u]->bde_dev.device & DNXC_DEVID_FAMILY_MASK) {
...
#endif
#ifdef INCLUDE_CPU_I2C
            _devices[u]->i2c_bus = cpu_i2c_bus_num_default; /* The number of the CPU I2C bus the device is connected to on Broadcom CPU cards */
            _devices[u]->i2c_dev = 0x44;  /* the device slave address on most Broadcom demo boards */
            _devices[u]->use_i2c_access = 0;
#endif /* INCLUDE_CPU_I2C */
            break;
          default:
#ifdef INCLUDE_CPU_I2C
            _devices[u]->i2c_bus = -1;
            _devices[u]->i2c_dev = -1;
            _devices[u]->use_i2c_access = 0;
#endif /* INCLUDE_CPU_I2C */
        }
...
_open(void)
{
    ...
    char *env_var_value = getenv(LUBDE_DEVICE_NAME_ENV_VAR); /* value of environment variable used to override the default device file path */
    char *user_bde_dev_name = env_var_value ? env_var_value : LUBDE_DEVICE_NAME; /* device file path */
    ...
    if ((_devfd = open(user_bde_dev_name,
                       O_RDWR | O_SYNC | O_DSYNC | O_RSYNC)) < 0) {
        /* Check legacy modules */
        if (access(LINUX_KERNEL_BDE_NAME".ko", F_OK) < 0 && soc_ndev) {
            return -1;
        }
        if (access(LINUX_USER_BDE_NAME".ko", F_OK) < 0 && soc_ndev) {
            return -1;
        }

        /* Try inserting modules from the current directory */
        if (linux24) {
            _system("/sbin/insmod linux-kernel-bde.o");
            _system("/sbin/insmod linux-user-bde.o");
        } else {
            _system("/sbin/insmod linux-kernel-bde.ko");
            _system("/sbin/insmod linux-user-bde.ko");
        }

        if ((_devfd = open(user_bde_dev_name,
                           O_RDWR | O_SYNC | O_DSYNC | O_RSYNC)) < 0) {
            perror(user_bde_dev_name);
            return -1;
        }
    }
...
/*  knet is only supported on Jericho and above */
    ...
    if ((devknetfd = open("/dev/linux-bcm-knet", O_RDONLY | O_NONBLOCK  )) < 0) {
        ...
        if (linux24) {
            ...
            _system("/sbin/insmod linux-bcm-knet.ko use_rx_skb=1 use_napi=1 rx_buffer_size=5120");
#else
            _system("/sbin/insmod linux-bcm-knet.ko");
#endif
        }
    }
#else
    ...
    if ((devknetfd = open("/dev/linux-bcm-knet", O_RDONLY | O_NONBLOCK  )) >= 0) {
        ...
        if (linux24) {
            ...
            _system("/sbin/rmmod linux_bcm_knet");
        }
    }
#endif
#endif
    ...
    env_var_value = getenv(LKBDE_DEVICE_NAME_ENV_VAR); /* value of environment variable used to override the default device file path */
    user_bde_dev_name = env_var_value ? env_var_value : LKBDE_DEVICE_NAME; /* device file path */
    if ((_kdevfd = open(user_bde_dev_name, O_RDWR | O_SYNC | O_DSYNC | O_RSYNC)) < 0) {
        perror(user_bde_dev_name);
        close(_devfd);
        return -1;
    }
...
_read(int d, unsigned int addr)
{
    ...
    if (_devices[d]->dev_type & (BDE_DEV_BUS_RD_16BIT | BDE_USER_DEV_TYPE)) {
        _devio.dev = d;
        _devio.d0 = addr;
...
_write(int d, uint32 addr, uint32 data)
{
    ...
    if (_devices[d]->dev_type & (BDE_DEV_BUS_WR_16BIT | BDE_USER_DEV_TYPE)) {
        _devio.dev = d;
        _devio.d0 = addr;
        _devio.d1 = data;
 ...
 *
 * Purpose:
 *    Read a 64 bits from the bar used to access CMIC.
 *    64 bit access is meant for better efficiency, and as such should not
 *    use the BDE for access to the BAR.
 * Parameters:
 *    d - device number
 *    addr - register address
 * Returns:
 ...
 */
static uint64
_read64(int d, unsigned int addr)
{
#if defined(SOC_PCI_DEBUG) && defined(INCLUDE_CPU_I2C)
    assert(!SAL_BOOT_I2C_ACCESS);
#endif

    if (_devices[d]->dev_type & (
#if CMIC_SPI_SUPPORT
          BDE_SPI_DEV_TYPE |
#endif
#ifdef DUNE_GTO_I2C
          ...
          BDE_DEV_BUS_RD_16BIT)) {
        printf("device %d is unsupported for 64 bit read operations\n", d);
#if defined(LONGS_ARE_64BITS) || defined(COMPILER_HAS_LONGLONG)
        return COMPILER_64_LITERAL(-1);
#else
        return *(volatile uint64*) -1;
#endif
    }

    assert(_devices[d]->vbase);
    /* byte/word swapping by the CPU is not supported */
    return *(volatile uint64*)(((volatile uint8*)_devices[d]->vbase) + addr);
}
 ...
 *
 * Purpose:
 *    Write 64 bits to the BAR containing CMIC.
 *    64 bit access is meant for better efficiency, and as such should not
 *    use the BDE for access to the BAR.
 * Parameters:
 *    d - device number
 ...
 */
static void
_write64(int d, uint32 addr, uint64 data)
{
#if defined(SOC_PCI_DEBUG) && defined(INCLUDE_CPU_I2C)
    assert(!SAL_BOOT_I2C_ACCESS);
#endif
    ...
    if (_devices[d]->dev_type & (
          ...
          BDE_DEV_BUS_WR_16BIT)) {
        printf("device %d is unsupported for 64 bit write operations\n", d);
        return;
    }

    assert(_devices[d]->vbase);
    /* byte/word swapping by the CPU is not supported */
    *(volatile uint64*)(((volatile uint8*)_devices[d]->vbase) + addr) = data;
    return;
}
...
#endif
   if (tmp == NULL) {
       printf("DMA allocation failed, pool usage:  %d bytes.\n",
              _dma_get_usage());
   }
   return tmp;
}
...
/*
 * Function:
 *      ether_intr_thread_get
 * Purpose:
 *      Read value of '_ether_intr_thread'
 * Parameters:
 *      NONE.
 * Returns:
 *      sal_thread_t - Value of _ether_intr_thread.
 */
static sal_thread_t
ether_intr_thread_get(void)
{
    sal_thread_t ret;

    ret = _ether_intr_thread;
    return (ret);
}
/*
 * Function:
 *      ether_intr_thread_set
 * Purpose:
 *      Write 'value' into '_ether_intr_thread'
 * Parameters:
 *      value -
            Value to load into _ether_intr_thread
 * Returns:
 *      NONE
 */
static void
ether_intr_thread_set(sal_thread_t value)
{
    _ether_intr_thread = value;
    return;
}
/*
 * Function:
 *      intr_thread_get
 * Purpose:
 *      Read value of '_intr_thread'
 * Parameters:
 *      NONE.
 * Returns:
 *      sal_thread_t - Value of _intr_thread.
 */
static sal_thread_t
intr_thread_get(void)
{
    sal_thread_t ret;
 ...
 *      intr_thread_set
 * Purpose:
 *      Write 'value' into '_intr_thread'
 * Parameters:
 *      value -
            Value to load into _intr_thread
 * Returns:
 *      NONE
 */
static void
intr_thread_set(sal_thread_t value)
{
    _intr_thread = value;
    return;
}
#define ETHER_INTR_THREAD_GET() \
    ether_intr_thread_get()
#define ETHER_INTR_THREAD_SET(value) \
    ether_intr_thread_set(value)
#define INTR_THREAD_GET() \
    intr_thread_get()
#define INTR_THREAD_SET(value) \
    intr_thread_set(value)
/* } */
#else
/* { */
#define ETHER_INTR_THREAD_GET() \
...
{
    return ((INTR_THREAD_GET() == sal_thread_self()) ||
            (ETHER_INTR_THREAD_GET() == sal_thread_self()));
}
...
_interrupt_thread(void *d)
{
    ...
    INTR_THREAD_SET(sal_thread_self());
...
_ether_interrupt_thread(void* d)
{
    ...
    /* We are the interrupt thread for intr_int_context() */
    ETHER_INTR_THREAD_SET(sal_thread_self());
...
_iproc_read(int d, unsigned int addr)
{
    ...
    assert(!SAL_BOOT_I2C_ACCESS);
#endif
    if (_devices[d]->dev_type & (BDE_AXI_DEV_TYPE | BDE_USER_DEV_TYPE)) {
        return _iproc_ihost_read(d, addr);
    }
    sal_mutex_take(iproc_map_lock[d], sal_mutex_FOREVER);

    if (!(_devices[d]->dev_type & BDE_NO_IPROC)) {
        if ((offset = _iproc_offset(d, addr)) == 0) {
            sal_mutex_give(iproc_map_lock[d]);
            return -1;
        }
        addr -= offset;
    }
...
#ifdef BCM_INSTANCE_SUPPORT
/*
* Call this function once to configure the SDK instance,
* before calling linux_bde_create().
*/
int
linux_bde_instance_config(linux_bde_device_bitmap_t dev_mask, unsigned int dma_size)
{
    unsigned i;
    uint32 *bitmap_ptr, bits;
    memcpy(_inst_dev_mask, dev_mask, sizeof(linux_bde_device_bitmap_t));
    /* check that we have devices controlled by the instance and set the instance number/ID to the first device controlled by the instance */
    for (bitmap_ptr = dev_mask, i = 0; i < LINUX_BDE_MAX_DEVICES; ++bitmap_ptr, i += 32) {
        bits = *bitmap_ptr;
        if (bits == 0) {
            continue;
        }
        for (; (bits & 1) == 0; bits >>= 1, ++i);
        break;
    }
    if (i >= LINUX_BDE_MAX_DEVICES) {
        printf("linux-user-bde: no devices specified for the instance\n");
        return -1;
    }
    _inst_num = i;
...
/*
* Call this function once to configure the SDK instance,
* before calling linux_bde_create() if all the controlled device IDs are below 32.
* This API is replaced by linux_bde_instance_config().
*/
int
linux_bde_instance_attach(unsigned int dev_mask,unsigned int dma_size)
{
    linux_bde_device_bitmap_t bigger_dev_mask = {0};
    bigger_dev_mask[0] = dev_mask;
    return linux_bde_instance_config(bigger_dev_mask, dma_size);
}
#endif
...
linux_bde_destroy(ibde_t* ibde)
{
    ...
    for (u = 0; u < LINUX_BDE_MAX_DEVICES; u++) {
        if (iproc_map_lock[u]) {
            sal_mutex_destroy(iproc_map_lock[u]);
            iproc_map_lock[u] = NULL;
        }
    }
#if LINUX_SPIDEV_SUPPORT
#endif /* LINUX_SPIDEV_SUPPORT */
