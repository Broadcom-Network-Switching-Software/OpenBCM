    ...
    int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx_gen2))
    {
        ...
        SHR_IF_ERR_EXIT(access_chip_intr_init(unit));
#endif
        SHR_EXIT();
    }
#endif
...
#ifdef BCM_ACCESS_SUPPORT
    if (access_new_interrupt_available(unit) && soc_feature(unit, soc_feature_iproc_20))
    {
        int ii;

        for (ii = 0; ii < 8; ii++)
        {
            soc_cm_iproc_write(unit, ICFG_CHIP_INTR_ENABLE_REGx_OFFSET(ii), 0);
        }
    }
    else
#endif
