    ...
    int unit)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        rc = dnx_data_intr.general.ser_event_generate_support_get(unit);
    }
#endif
    ...
    if (SOC_IS_DNXF(unit))
    {
        rc = dnxf_data_intr.general.ser_event_generate_support_get(unit);
    }
#endif
    ...
    int unit)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        rc = dnx_data_intr.general.ser_reset_cb_en_support_get(unit);
    }
#endif
    ...
    if (SOC_IS_DNXF(unit))
    {
        rc = dnxf_data_intr.general.ser_reset_cb_en_support_get(unit);
    }
#endif
    ...
    uint32 *entry_idx)
{
    ...
    block = SOC_BLOCK_INFO(unit, copyno).schan;
    *mem = soc_addr_to_mem_extended(unit, block, 0xff, addrf);
    if (*mem == INVALIDm)
    {
        return _SHR_E_PARAM;
    }
    rc = soc_mem_addr_to_array_element_and_index(unit, *mem, addrf, array_idx, (int *) entry_idx);
    return rc;
}

static int
_dnxc_ser_event_generate(
    int unit,
    uint32 blk,
    uint32 addrf,
    sand_memory_dc_t dc_type)
{
    int rc = _SHR_E_NONE;
    uint32 ser_type = 0xFFFFFFFF;

    switch (dc_type)
    {
        case SAND_ECC_PARITY_DC:
            ser_type = SOC_SWITCH_EVENT_DATA_ERROR_PARITY;
            break;
        case SAND_ECC_ECC1B_DC:
        case SAND_P_1_ECC_ECC1B_DC:
        case SAND_P_2_ECC_ECC1B_DC:
        case SAND_P_3_ECC_ECC1B_DC:
            ser_type = SOC_SWITCH_EVENT_DATA_ERROR_ECC_1BIT_CORRECTED;
            break;
        case SAND_ECC_ECC2B_DC:
        case SAND_P_1_ECC_ECC2B_DC:
        case SAND_P_2_ECC_ECC2B_DC:
        case SAND_P_3_ECC_ECC2B_DC:
            ser_type = SOC_SWITCH_EVENT_DATA_ERROR_ECC_2BIT_CORRECTED;
            break;
        default:
            break;
    }

    if (ser_type != 0xFFFFFFFF)
    {
        rc = soc_event_generate(unit, SOC_SWITCH_EVENT_PARITY_ERROR, ser_type, addrf, blk);
    }
    return rc;
}
    ...
    if (addr_validf != 0)
    {
        ...
        switch (mem)
        {
            ...
            default:
                ...
                if (_dnxc_ser_event_support(unit))
                {
                    SHR_IF_ERR_EXIT(_dnxc_ser_event_generate(unit, shadow_correct_info->copyno, addrf, type));
                }
        }
    }
    else
    ...
    char *msg)
{
    ...
    {
        int blk_is_fdrc = 0;
        uint32 soft_reset_cb_type;
        int is_ile = 0;
#ifdef BCM_ACCESS_SUPPORT
        if (access_new_interrupt_available(unit))
        {
            access_global_block_ids_enum_t block;
            ...
            access_local_regmem_id_t local_reg_id;
            const access_device_regmem_t *rm_info;

            local_reg_id = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[interrupt_id].reg;
            rm_info = ACCESS_LOCAL_REGMEM_RMINFO(runtime_info, local_reg_id);
            block = device_info->blocks[rm_info->local_block_id].block_type;
...
#endif
        {
            ...
            is_ile = (blktype == SOC_BLK_ILE) ? 1 : 0;
        }
        if (_dnxc_ser_reset_cb_en_support(unit))
        {
            if (blk_is_fdrc)
            {
                soft_reset_cb_type = ASIC_SOFT_RESET_BLOCKS_FABRIC_ILE;
            }
            else if (is_ile)
            {
                soft_reset_cb_type = ASIC_SOFT_RESET_BLOCKS_ILE;
            }
            else
            {
                soft_reset_cb_type = ASIC_SOFT_RESET_BLOCKS;
            }

            soc_event_generate(unit, SOC_SWITCH_EVENT_DEVICE_INTERRUPT, interrupt_id, block_instance,
                               soft_reset_cb_type);
            SHR_EXIT();
        }
...
#ifdef BCM_DNXF_SUPPORT
int
dnxf_interrupt_hard_reset_call_register_func(
    int unit,
    int block_instance,
    uint32 interrupt_id)
{
    bcm_switch_hard_reset_callback_t callback = NULL;
    uint32 *userdata;

    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnxf_state.hard_reset.callback.get(unit, &callback));
    SHR_IF_ERR_EXIT(dnxf_state.hard_reset.userdata.get(unit, &userdata));
    if (callback != NULL)
    {
        callback(unit, 0, (void *) userdata);
    }
    else
    {
    ...
    char *msg)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        SHR_IF_ERR_EXIT(dnx_interrupt_hard_reset_call_register_func(unit, block_instance, interrupt_id));
    }
#endif
    ...
    if (SOC_IS_DNXF(unit))
    {
        SHR_IF_ERR_EXIT(dnxf_interrupt_hard_reset_call_register_func(unit, block_instance, interrupt_id));
    }
#endif
    ...
    sand_intr_tcam_location * location)
{
    ...
    if (access_new_interrupt_available(unit))
    {
        protection_reg = rTCAM_TCAM_PROTECTION_ERROR;
        SHR_IF_ERR_EXIT(access_regmem(unit, 0, protection_reg, block_instance, 0, 0, &reg_value));

        SHR_IF_ERR_EXIT(access_field_get(unit, rTCAM_TCAM_PROTECTION_ERROR_fTCAM_PROTECTION_ERROR_TCAM_BANK,
                                         &reg_value, &location->bank_id));
        SHR_IF_ERR_EXIT(access_field_get(unit, rTCAM_TCAM_PROTECTION_ERROR_fTCAM_PROTECTION_ERROR_TCAM_BANK,
                                         &reg_value, &location->entry));
    }
    else
#endif
    {
...
int
dnxc_interrupt_handles_corrective_action_tcam_ecc_2bits_done(
    int unit,
    int block_instance)
{
    uint32 rc;
    ...
    if (access_new_interrupt_available(unit))
    {
        ...
        rc = access_regmem_field_modify(unit, 0, rTCAM_TCAM_PROTECTION_2_BIT_ECC_HANDLING_DONE, block_instance, 0, 0,
                                        rTCAM_TCAM_PROTECTION_2_BIT_ECC_HANDLING_DONE_fTCAM_PROTECTION_2_BIT_ECC_HANDLING_DONE,
                                        &field_value);
    }
    else
#endif
    {
        rc = soc_reg_field32_modify(unit, TCAM_TCAM_PROTECTION_2_BIT_ECC_HANDLING_DONEr,
                                    block_instance, TCAM_PROTECTION_2_BIT_ECC_HANDLING_DONEf, 1);
    }
    SHR_IF_ERR_EXIT(rc);
    ...
    uint32 en_interrupt)
{
    ...
    if (access_new_interrupt_available(unit))
    {
        ...
        SHR_IF_ERR_EXIT(access_regmem(unit, 0, rCRPS_CRPS_ENGINE_ERROR, block_instance, 0, index, &reg_value));

        SHR_IF_ERR_EXIT(access_field_get(unit, rCRPS_CRPS_ENGINE_ERROR_fENGINE_N_CNT_OVF_ADDRESS,
                                         &reg_value, &CntOvfAddress));
    }
    else
#endif
    {
    ...
    char *msg)
{
    ...
    if (access_new_interrupt_available(unit))
    {
        ...
        SHR_IF_ERR_EXIT(access_regmem(unit, 0, rTCAM_TCAM_MOVE_ECC_FAILURE, block_instance, 0, 0, &reg_value));

        SHR_IF_ERR_EXIT(access_field_get(unit, rTCAM_TCAM_MOVE_ECC_FAILURE_fTCAM_MOVE_ECC_FAILURE_VALID,
                                         &reg_value, &valid));
        SHR_IF_ERR_EXIT(access_field_get(unit, rTCAM_TCAM_MOVE_ECC_FAILURE_fTCAM_MOVE_ECC_BANK, &reg_value, &bank));
        SHR_IF_ERR_EXIT(access_field_get(unit, rTCAM_TCAM_MOVE_ECC_FAILURE_fTCAM_MOVE_ECC_LINE, &reg_value, &line));
        SHR_IF_ERR_EXIT(access_field_get(unit, rTCAM_TCAM_MOVE_ECC_FAILURE_fTCAM_MOVE_1_BIT_ECC,
                                         &reg_value, &ecc_1bit_error));
        SHR_IF_ERR_EXIT(access_field_get(unit, rTCAM_TCAM_MOVE_ECC_FAILURE_fTCAM_MOVE_2_BIT_ECC,
                                         &reg_value, &ecc_2bit_error));
    }
    else
#endif
    {
    ...
    char *msg)
{
    ...
    if (access_new_interrupt_available(unit))
    {
        access_field_id_t access_field[] =
            { rMCP_ENG_DB_BANK_SOURCE_fENG_DB_A_BANK_0_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_A_BANK_1_SOURCE };
        if (bank >= COUNTOF(access_field))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported bank: %d\n", bank);
        }

        SHR_IF_ERR_EXIT(access_regmem(unit, 0, rMCP_ENG_DB_BANK_SOURCE, block_instance, 0, 0, reg_value));

        SHR_IF_ERR_EXIT(access_field_get(unit, access_field[bank], reg_value, &EngineDbSource));
    }
    else
#endif
    {
    ...
    char *msg)
{
    int rc;
    soc_reg_above_64_val_t reg_value;
    uint32 EngineDbSource, EngineDbErrSource;
    soc_field_t field[] = {
    ...
    {
        access_field_id_t access_field[] = {
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_0_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_1_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_2_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_3_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_4_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_5_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_6_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_7_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_8_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_9_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_10_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_11_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_12_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_13_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_14_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_15_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_16_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_17_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_18_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_19_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_20_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_21_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_22_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_23_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_24_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_25_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_26_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_27_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_28_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_29_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_30_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_31_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_32_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_33_SOURCE,
            rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_34_SOURCE, rMCP_ENG_DB_BANK_SOURCE_fENG_DB_C_BANK_35_SOURCE
        };

        access_field_id_t access_errfield[] = {
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_0_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_1_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_2_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_3_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_4_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_5_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_6_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_7_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_8_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_9_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_10_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_11_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_12_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_13_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_14_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_15_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_16_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_17_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_18_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_19_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_20_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_21_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_22_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_23_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_24_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_25_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_26_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_27_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_28_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_29_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_30_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_31_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_32_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_33_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_34_ACCESS_ERR_SOURCE,
            rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE_fENG_DB_C_BANK_35_ACCESS_ERR_SOURCE
        };
        ...
        SHR_IF_ERR_EXIT(access_regmem(unit, 0, rMCP_ENG_DB_BANK_ACCESS_ERR_SOURCE, block_instance, 0, 0, reg_value));
        SHR_IF_ERR_EXIT(access_field_get(unit, access_errfield[bank], reg_value, &EngineDbErrSource));
    }
    else
#endif
    {
    ...
    sand_intr_tcam_location * location)
{
    ...
    if (access_new_interrupt_available(unit))
    {
        ...
        reg = rKAPS_RPB_TCAM_SCAN_INDEX;
        if (is_rpb_a)
        {
            field = rKAPS_RPB_TCAM_SCAN_INDEX_fRPB_A_TCAM_SCAN_INDEX_N;
        }
        else
        {
            field = rKAPS_RPB_TCAM_SCAN_INDEX_fRPB_B_TCAM_SCAN_INDEX_N;
        }
        SHR_IF_ERR_EXIT(access_regmem(unit, 0, reg, block_instance, 0, 0, &reg_value));
        SHR_IF_ERR_EXIT(access_field_get(unit, field, &reg_value, &scan_idx));
        location->entry = scan_idx;

        if (is_rpb_a)
        {
            reg = rKAPS_RPB_TCAM_A_SCAN_PAUSE;
            field = rKAPS_RPB_TCAM_A_SCAN_PAUSE_fRPB_A_TCAM_SCAN_PAUSE_N;
        }
        else
        {
            reg = rKAPS_RPB_TCAM_B_SCAN_PAUSE;
            field = rKAPS_RPB_TCAM_B_SCAN_PAUSE_fRPB_B_TCAM_SCAN_PAUSE_N;
        }
        SHR_IF_ERR_EXIT(access_regmem(unit, 0, reg, block_instance, 0, 0, &reg_value));
        SHR_IF_ERR_EXIT(access_field_get(unit, field, &reg_value, &scan_pause));
    ...
    char *special_msg)
{
    ...
    if ((print_msg = (char *) sal_alloc(DNXC_INTERRUPT_PRINT_MSG_SIZE, "print_msg")) == NULL)
    {
        SHR_IF_ERR_EXIT(_SHR_E_MEMORY);
    }
    ...
    switch (corr_act)
    {
        ...
        case DNXC_INT_CORR_ACT_TCAM_SHADOW_FROM_SW_DB:
            ...
            break;
        case DNXC_INT_CORR_ACT_TCAM_ECC2BITS_HANDLE_DONE:
            sal_snprintf(cur_corr_act_msg, DNXC_INTERRUPT_COR_ACT_MSG_SIZE, "TCAM 2B ECC DONE");
            break;
        ...
        case DNXC_INT_CORR_ACT_IPPD_INVALID_DESTINATION_VALID:
            ...
            break;
        case DNXC_INT_CORR_ACT_MSS_MACSEC_SA_EXPIRY:
        case DNXC_INT_CORR_ACT_MSD_MACSEC_SA_EXPIRY:
        case DNXC_INT_CORR_ACT_MSW_MACSEC_SA_EXPIRY:
        ...
        case DNXC_INT_CORR_ACT_KAPS_TECC:
            ...
            break;
        case DNXC_INT_CORR_ACT_SIGNAL_QUALITY_ERR_INT:
            sal_snprintf(cur_corr_act_msg, DNXC_INTERRUPT_COR_ACT_MSG_SIZE, "Signal quality event");
            break;
        case DNXC_INT_CORR_ACT_RETRAIN_LINK:
            sal_snprintf(cur_corr_act_msg, DNXC_INTERRUPT_COR_ACT_MSG_SIZE, "Retrain link");
            break;
        default:
    ...
    soc_port_t * p_link)
{
    ...
    if (SOC_IS_DNXF(unit))
    {
        SHR_IF_ERR_EXIT(soc_dnxf_drv_block_to_link_mapping
                        (unit, fmac_block_instance, bit_in_field, SOC_BLK_FMAC, p_link));
    ...
    soc_port_t * p_link)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        uint32 nof_instances_fsrd = dnx_data_fabric.blocks.nof_instances_fsrd_get(unit);
        uint32 nof_links_in_fsrd = dnx_data_fabric.blocks.nof_links_in_fsrd_get(unit);
    ...
    {
        SHR_IF_ERR_EXIT(soc_dnxf_drv_block_to_link_mapping
                        (unit, fsrd_block_instance, bit_in_field, SOC_BLK_FSRD, p_link));
    }
#endif
    ...
    char *msg)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        rc = soc_dnxc_port_enable_set(unit, port, 0, 0);
        SHR_IF_ERR_EXIT(rc);
    }
#endif
exit:
    ...
    char *msg)
{
    ...
    for (ii = 0; ii < 8; ii++)
    {
        ...
        sal_strncat_s(msg, str, DNXC_INTERRUPT_SPECIAL_MSG_SIZE);
    }
    for (ii = 0; ii < 2; ii++)
    {
...
static shr_error_e
dnx_interrupt_handles_corrective_retrain_link(
    int unit,
    int block_instance,
    char *msg)
{
    ...
    sal_snprintf(msg, DNXC_INTERRUPT_SPECIAL_MSG_SIZE - 1,
                 "Link status change on instace%d, need retrain link when link change from down to up", block_instance);
...
static int
dnxc_interrupt_signal_quality_interrupt_state_get(
    int unit,
    bcm_port_signal_quality_error_rate_state_t * state,
    char *state_str)
{
    SHR_FUNC_INIT_VARS(unit);
    if (0 == sal_strncmp(state_str, "DEGRADE_RECOVER", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        *state = BCM_PORT_SIGNAL_QUALITY_ERROR_RATE_STATE_DEGRADE_RECOVER;
    }
    else if (0 == sal_strncmp(state_str, "FAIL_RECOVER", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        *state = BCM_PORT_SIGNAL_QUALITY_ERROR_RATE_STATE_FAIL_RECOVER;
    }
    else if (0 == sal_strncmp(state_str, "FAIL", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        *state = BCM_PORT_SIGNAL_QUALITY_ERROR_RATE_STATE_FAIL;
    }
    else if (0 == sal_strncmp(state_str, "DEGRADE", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        *state = BCM_PORT_SIGNAL_QUALITY_ERROR_RATE_STATE_DEGRADE;
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "UnKnown Signal-Quality event");
    }
...
static int
dnxc_interrupt_handles_corrective_action_for_signal_quality(
    int unit,
    int block_instance,
    uint32 interrupt_id,
    dnxc_int_corr_act_type corr_act,
    ...
    char *state_str)
{
    ...
    soc_interrupt_db_t *interrupt;
    bcm_port_signal_quality_error_rate_state_t state = BCM_PORT_SIGNAL_QUALITY_ERROR_RATE_STATE_DEGRADE_RECOVER;

    SHR_FUNC_INIT_VARS(unit);

    SHR_NULL_CHECK(msg, _SHR_E_PARAM, "msg");
    SHR_NULL_CHECK(state_str, _SHR_E_PARAM, "state_str");

    interrupt = &(SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[interrupt_id]);

    SHR_IF_ERR_EXIT(dnxc_interrupt_fmac_link_get(unit, block_instance, interrupt->bit_in_field, &link));
    port = link;
    ...
    SHR_IF_ERR_EXIT(dnxc_interrupt_signal_quality_interrupt_state_get(unit, &state, msg));

    sal_snprintf(msg, (DNXC_INTERRUPT_PRINT_MSG_SIZE - 1), "port= %d link= %d state: %-15.15s\n ", port, link,
                 state_str);

    SHR_IF_ERR_EXIT(dnxc_interrupt_print_info(unit, block_instance, interrupt_id, 0, corr_act, msg));
...
int
dnxc_signal_quality_interrupt_handling(
    int unit,
    int block_instance,
    uint32 en_interrupt,
    char *msg)
{
    dnxc_int_corr_act_type p_corrective_action = DNXC_INT_CORR_ACT_SIGNAL_QUALITY_ERR_INT;
    char special_msg[DNXC_INTERRUPT_SPECIAL_MSG_SIZE] = { 0 };
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnxc_interrupt_handles_corrective_action
                    (unit, block_instance, en_interrupt, special_msg, p_corrective_action, msg, NULL));
    ...
    void *param2)
{
    ...
    switch (corr_act)
    {
        ...
        case DNXC_INT_CORR_ACT_SHADOW:
            ...
            if (access_new_interrupt_available(unit))
            {
                rc = dnxc_access_handle_action_shadow(unit,
                                                      block_instance,
                                                      interrupt_id, (dnxc_interrupt_mem_err_info *) param1, msg);
            }
            else
#endif
        ...
        case DNXC_INT_CORR_ACT_ECC_1B_FIX:
            ...
            if (access_new_interrupt_available(unit))
            {
                rc = dnxc_access_handle_action_ecc_1b(unit,
                                                      block_instance,
                                                      interrupt_id, (dnxc_interrupt_mem_err_info *) param1, msg);
            }
            else
        ...
        case DNXC_INT_CORR_ACT_RTP_SLSCT:
            ...
            if (access_new_interrupt_available(unit))
            {
                rc = dnxc_access_handles_action_rtp_slsct(unit, block_instance, interrupt_id,
                                                          (dnxc_interrupt_mem_err_info *) param1, msg);
            }
            else
#endif
            {
                rc = dnxc_interrupt_handles_corrective_action_for_rtp_slsct(unit, block_instance, interrupt_id,
                                                                            (dnxc_interrupt_mem_err_info *) param1,
                                                                            msg);
            }
            SHR_IF_ERR_EXIT(rc);
            break;
        ...
        case DNXC_INT_CORR_ACT_XOR_FIX:
            ...
            if (access_new_interrupt_available(unit))
            {
                rc = dnxc_access_handle_action_xor(unit, block_instance, interrupt_id,
        ...
        case DNXC_INT_CORR_ACT_MDB_ECC_1B_FIX:
            ...
            if (access_new_interrupt_available(unit))
            {
                rc = dnxc_access_handle_action_mdb_1b(unit, block_instance, interrupt_id,
                                                      (dnxc_interrupt_mem_err_info *) param1, msg);
            }
            else
#endif
        ...
        case DNXC_INT_CORR_ACT_MDB_ECC_2B_FIX:
            ...
            if (access_new_interrupt_available(unit))
            {
                rc = dnxc_access_handle_action_mdb_2b(unit, block_instance, interrupt_id,
                                                      (dnxc_interrupt_mem_err_info *) param1, msg);
            }
            else
#endif
        ...
        case DNXC_INT_CORR_ACT_HANDLE_OAMP_EVENT_FIFO:
            dnx_oam_oamp_event_handler(unit, 1);
            break;
        ...
        case DNXC_INT_CORR_ACT_TCAM_ECC2BITS_HANDLE_DONE:
            rc = dnxc_interrupt_handles_corrective_action_tcam_ecc_2bits_done(unit, block_instance);
            SHR_IF_ERR_EXIT(rc);
            break;
        ...
        case DNXC_INT_CORR_ACT_MSS_MACSEC_SA_EXPIRY:
        case DNXC_INT_CORR_ACT_MSW_MACSEC_SA_EXPIRY:
            rc = dnx_sec_interrupt_handles_corrective_action_sa_expiry(unit, block_instance, 1, msg);
            SHR_IF_ERR_EXIT(rc);
            break;
        case DNXC_INT_CORR_ACT_MSD_MACSEC_SA_EXPIRY:
            rc = dnx_sec_interrupt_handles_corrective_action_sa_expiry(unit, block_instance, 0, msg);
            SHR_IF_ERR_EXIT(rc);
            break;
#endif
        case DNXC_INT_CORR_ACT_EPNI_TOTAL_INC_ABOVE_TH_WITH_EBD:
        case DNXC_INT_CORR_ACT_EPNI_EBD_ERR_INT:
            break;
#endif
        case DNXC_INT_CORR_ACT_TCAM_QUERY_FAILURE_VALID:
            ...
            break;
        case DNXC_INT_CORR_ACT_SIGNAL_QUALITY_ERR_INT:
            rc = dnxc_interrupt_handles_corrective_action_for_signal_quality(unit, block_instance, interrupt_id,
                                                                             corr_act, msg, (char *) param1);
            SHR_IF_ERR_EXIT(rc);
            break;
        case DNXC_INT_CORR_ACT_RETRAIN_LINK:
            rc = dnx_interrupt_handles_corrective_retrain_link(unit, block_instance, msg);
            SHR_IF_ERR_EXIT(rc);
            break;
        default:
