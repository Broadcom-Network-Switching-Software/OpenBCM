...
{
    if (!sal_strncmp(cb_db->function_name,"dnx_algo_consistent_hashing_manager_profile_move", 48))
    {
        if(!dryRun && cb != NULL)
        {
    ...
    if (!sal_strncmp(cb_db->function_name,"sw_state_defragmented_chunk_move_test", 37))
    {
        if(!dryRun && cb != NULL)
        {
