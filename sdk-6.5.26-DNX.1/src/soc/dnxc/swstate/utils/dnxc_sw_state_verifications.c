    ...
    uint32 flags)
{
    ...
    if (DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id) == EXAMPLE_MODULE_ID) {
    ...
    DNXC_MTA(SHR_IF_ERR_EXIT(dnxc_multithread_analyzer_log_resource_use(unit, MTA_RESOURCE_SW_STATE, DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id), is_write)));
    ...
    if ((sw_state_is_syncing(unit) &&
        !SW_STATE_VERF_MATRIX_CHECK(DNX_SW_STATE_VERF_MATRIX_WB)) &&
        !_SHR_IS_FLAG_SET(flags, DNXC_SW_STATE_ALLOW_WRITES_DURING_WB)) {
        SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,
                                 "Trying to modify state during sync\n%s%s%s",
                                 EMPTY, EMPTY, EMPTY);
    }
    ...
    void* module_ptr)
{
    ...
    if (!sw_state_roots_array[unit]) {
        SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,
                                 ...
                                 dnxc_module_id_e_get_name(DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)), DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id), EMPTY);
    }

    if (!module_ptr) {
        SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,
                                 "Trying to access SW State module that wasn't initialized (module name=%s id=%d)\n%s",
                                 dnxc_module_id_e_get_name(DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)), DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id), EMPTY);
    }
    ...
    void* ptr)
{
    ...
    if (!ptr) {
        SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,
                                 ...
                                 DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id), EMPTY, EMPTY);
    }
...
#define NOT_DNX2_DEVICE_STR ""
        ...
        if(!soc_is(unit, DNX2_DEVICE)){
            SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,"sw state ERROR: Trying to access %s only parameter while the unit is not from the same family(module_id=%d)\n%s",
                                    (soc_device == DNX2_DEVICE) ? "dnx2" : NOT_DNX2_DEVICE_STR, DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id), EMPTY);
        }
    }
...
int dnxc_sw_state_verifications_init(
    ...
    SW_STATE_VERF_MATRIX_INIT(SW_STATE_FUNC_REPLACE,                    1,    1,    0,    0);
    ...
    SW_STATE_VERF_MATRIX_INIT(SW_STATE_FUNC_OFFSET_UNRESERVE,           1,    1,    0,    0);
    SW_STATE_VERF_MATRIX_INIT(SW_STATE_FUNC_OFFSET_RESERVE,             1,    1,    0,    0);
    SW_STATE_VERF_MATRIX_INIT(SW_STATE_FUNC_DEFRAG,                     1,    1,    0,    0);
    ...
    SW_STATE_VERF_MATRIX_INIT(SW_STATE_FUNC_DATA_PER_ENTRY_SET,                 1,    1,    1,    0);
    SW_STATE_VERF_MATRIX_INIT(SW_STATE_FUNC_DATA_PER_ENTRY_GET,                 1,    1,    1,    0);
    return SOC_E_NONE;
}
    ...
    uint32 type_size)
{
    ...
    rv = dnx_sw_state_oob_dynamic_array_check(unit, node_id, idx, ptr_void, type_size);
    if(rv != SOC_E_NONE)
    {
    ...
    uint32 flags)
{
    ...
    {
        SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,
                                 "sw state ERROR: size allocated %d  is bigger than permitted by DNXC_SW_STATE_ALLOC_MAX_BYTES %d\n%s",
                                 alloc_size, DNXC_SW_STATE_ALLOC_MAX_BYTES, EMPTY);
    }
    ...
    char *dbg_string)
{
    ...
    SHR_RANGE_VERIFY(nof_elements, 0, DNXC_SW_STATE_MAX_NOF_ELEMENTS_IN_ARRAY, SOC_E_INTERNAL, "sw state alloc ERROR: nof_elements %d not in range 0-%d", nof_elements, DNXC_SW_STATE_MAX_NOF_ELEMENTS_IN_ARRAY);
    SHR_RANGE_VERIFY(element_size, 0, DNXC_SW_STATE_MAX_ARRAY_ENTRY_SIZE, SOC_E_INTERNAL, "sw state alloc ERROR: element_size %d not in range 0-%d", element_size, DNXC_SW_STATE_MAX_ARRAY_ENTRY_SIZE);
