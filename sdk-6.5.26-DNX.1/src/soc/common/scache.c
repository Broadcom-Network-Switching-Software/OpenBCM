                           ...
                           scache_handle_state_t **handle_state)
{
    ...
    scache_descriptor_t *descriptor;
        ...
        /*
         * Ignore allocation counters (in main thread), scache memory should not be taken into account with regards to memory tests.
         * Note: cannot use soc_is here because under certain circumstances the soc_control variable is uninitialized.
         */
        hs = sal_alloc2(sizeof(*hs), SAL_ALLOC_F_IGNORE_COUNTERS, "WB cache info");
#else
...
#if (defined(BCM_DNX_SUPPORT) || (defined(BCM_DNXF_SUPPORT)))
        /*
         * Ignore free counters (in main thread), scache memory should not be taken into account with regards to memory tests.
         */
        sal_free2(temp, SAL_ALLOC_F_IGNORE_COUNTERS);
#else
        sal_free(temp);
#endif
    }
...
/* those defaults are different in order to ignore the allocation and free
...
soc_scache_defl_dnx_alloc(uint32 sz)
{
     ...
     * Ignore allocation counters (in main thread), scache memory should not be taken into account with regards to memory tests.
     */
    return sal_alloc2(sz, SAL_ALLOC_F_IGNORE_COUNTERS, "wb handle");
}
...
soc_scache_defl_dnx_free(void *p)
{
     ...
     * Ignore free counters (in main thread), scache memory should not be taken into account with regards to memory tests.
     */
    sal_free2(p, SAL_ALLOC_F_IGNORE_COUNTERS);
}
