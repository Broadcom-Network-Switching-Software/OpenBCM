...
soc_knet_pcie_link_status_set(int unit, int pcie_link_status)
{
    int rv;
    unsigned int len;
    kcom_msg_pcie_link_status_t kmsg;

    sal_memset(&kmsg, 0, sizeof(kmsg));
    kmsg.hdr.opcode = KCOM_M_PCIE_LINK_STATUS;
    kmsg.hdr.unit = unit;
    if (pcie_link_status == 0) {
        kmsg.pcie_link_status = PCIE_LINK_STATUS_UP;
    } else {
        kmsg.pcie_link_status = PCIE_LINK_STATUS_DOWN;
    }
    len = sizeof(kmsg);
