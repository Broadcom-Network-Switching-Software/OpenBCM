 ...
 */
int
soc_features_bcm56070_a0(int unit, soc_feature_t feature)
{
    SOC_FEATURE_DEBUG_PRINT((" 56070_a0"));

    return _soc_firelight_features(unit,feature);
}
#endif /* BCM_56070 */
...
soc_features_bcm56640_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_ptp:
    case soc_feature_ptp_cf_sw_update:
    case soc_feature_timesync_support:
    case soc_feature_field_oam_actions:
    case soc_feature_egr_mirror_true:
    case soc_feature_flex_port:
...
soc_features_bcm56850_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_alpm_hit_bits_not_support:
        return TRUE;
    case soc_feature_knetsync_mtp_ts_update:
        return TRUE;
    case soc_feature_configure_loopback_port:
        return TRUE;
    default:
        return soc_features_bcm56840_b0(unit, feature);
    }
...
soc_features_bcm56960_a0(int unit, soc_feature_t feature)
{
    ...
    if ((dev_id == BCM56168_DEVICE_ID) || (dev_id == BCM56169_DEVICE_ID)) {
        if ((feature == soc_feature_l3_1k_defip_table) ||
            ...
            (feature == soc_feature_no_ip_tunnel)) {
            return TRUE;
        }
    ...
    case soc_feature_ctr_collection_preempt:
        return TRUE;
...
/*    case soc_feature_portmod: */
    ...
    case soc_feature_configure_loopback_port:
        return TRUE;
    case soc_feature_l2_mac_move_monitoring:
        return TRUE;
...
soc_features_bcm56980_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_l2_mac_move_monitoring:
    case soc_feature_bfd_l3iif_vrf:
    case soc_feature_l2_wb_recover_pause_cbs:
    case soc_feature_ctr_collection_preempt:
        return TRUE;
    ...
    case soc_feature_dlb_flow_monitoring:
        return   (soc_property_get(unit, spn_DLB_ENABLE, 1)) ? TRUE : FALSE;
    case soc_feature_sdklt_pktio:
#ifdef INCLUDE_PKTIO
        return (soc_property_get(unit, spn_PKTIO_DRIVER_TYPE, 0)) ? TRUE : FALSE;
#else
        return FALSE;
#endif /* INCLUDE_PKTIO */
    ...
    case soc_feature_mirror_rspan_no_mtp0:
        return FALSE;
    case soc_feature_no_l2_del_witk_key_only_match:
    case soc_feature_egr_dscp_map_ext_ref_check:
        return TRUE;
...
soc_features_bcm56870_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_stm_tpid_support:
    case soc_feature_egress_vlan_xlate_with_pci_dei_mapping:
    case soc_feature_pp_port_control:
    case soc_feature_td3_style_obm_classifier_map:
    case soc_feature_ipep_clock_gating:
    case soc_feature_misc_i2e_hgclass_combo_profile:
    case soc_feature_bfd_l3iif_vrf:
    case soc_feature_tunnel_init_ecn:
    case soc_feature_egr_dscp_map_ext_ref_check:
        return TRUE;
    case soc_feature_ptp_cf_sw_update:
        return FALSE;
    ...
    /* will be determined by cancun version and config variable */
    case soc_feature_mim_decoupled_mode:
        return FALSE;
    case soc_feature_vxlan:
    case soc_feature_l2gre:
        if (flow_init_mode) {
     ...
     */
        return FALSE;
    case soc_feature_flex_stat_port_group_support:
        if (soc_property_get(unit, spn_FLEX_STAT_PORT_GROUP_SUPPORT, 0)) {
            return TRUE;
        } else {
            return FALSE;
        }
    case soc_feature_separate_key_for_ipmc_route:
    case soc_feature_derive_phb_based_on_vfi:
    case soc_feature_enable_flow_based_udf_extraction:
    case soc_feature_enable_ifa_2:
    case soc_feature_ifa_adapt_table_select:
    case soc_feature_egr_vlan_xlate2_enable:
    case soc_feature_pt2pt_access_vlan_check:
    case soc_feature_pt2pt_tunnel_term_learn_disable:
    case soc_feature_nsh_over_l2:
    case soc_feature_vxlan_tunnel_vlan_egress_translation:
    case soc_feature_per_port_mpls_label_removal:
    case soc_feature_mpls_frr_bottom_label:
    case soc_feature_ipmc_riot_l3_iif_check:
    case soc_feature_ipuc_riot_l3_iif_check:
    case soc_feature_snap_nonzero_oui:
    case soc_feature_ifa_truncated_pkt_hdr_len_update:
    case soc_feature_mpls_evpn:
    case soc_feature_hx5_flex_gbp_support: /* feature imported from HX5 */
    case soc_feature_l2_tunnel_pyld_sgpp_tpid_disable:
    case soc_feature_srv6_enable:
    case soc_feature_vxlan_gbp_flex:
    case soc_feature_mirror_erspan_gre_key_enable:
    /* will be determined by cancun version */
        return FALSE;

    case soc_feature_l2_fifo_mode_station_move_mac_invalid:
        return TRUE;
    case soc_feature_knetsync_mtp_ts_update:
        return FALSE;
    case soc_feature_configure_loopback_port:
        return FALSE;
    case soc_feature_vp_lag_underlay_ecmp:
        return TRUE;
    case soc_feature_l2_mac_move_monitoring:
        return TRUE;
    case soc_feature_l2_mem_acc_err_handling:
        return TRUE;
    default:
        return soc_features_bcm56960_a0(unit, feature);
    }
 ...
 */
int
soc_features_bcm56275_a0(int unit, soc_feature_t feature)
{
    uint16      dev_id;
    uint8       rev_id;

    SOC_FEATURE_DEBUG_PRINT((DK_VERBOSE," 56275_a0"));
    soc_cm_get_id(unit, &dev_id, &rev_id);
    switch (feature) {
        /* No ALPM */
    case soc_feature_alpm:
    ...
    case soc_feature_field_exact_match_support:
        /* Disable 10 FP slices */
    case soc_feature_field_slices10:
        /* No TCB support */
    case soc_feature_tcb:
        /* No ECMP underlay/overlay protected access */
    case soc_feature_l3_ecmp_protected_access:
    case soc_feature_chip_config_read_war:
    case soc_feature_egress_vlan_xlate_with_pci_dei_mapping:
        /* HW TCAM SCAN will not be enabled in HR4 */
    case soc_feature_tcam_scan_engine:
        return FALSE;

        /* L3_IIF size is less than 4k (2k) */
    case soc_feature_l3_iif_under_4k:
    case soc_feature_l3_iif_zero_invalid:
    case soc_feature_vlan_default_l3_iif_as_one:
#ifdef INCLUDE_XFLOW_MACSEC
        /* MACSEC */
    case soc_feature_xflow_macsec_crypto_aes_256:
    case soc_feature_xflow_macsec_svtag:
    ...
    case soc_feature_xflow_macsec_misc_counters_ecc_error_war:
#endif /* INCLUDE_XFLOW_MACSEC */
#if defined(BCM_FLOWTRACKER_V2_SUPPORT)
        /* BroadScan 2.0 */
    case soc_feature_flex_flowtracker_ver_2_check_slice:
#endif /* BCM_FLOWTRACKER_V2_SUPPORT */
        /* QOS COMBO profile */
    case soc_feature_mpls_shadow_qos:
        /*
         * Some ports are managed with legacy port driver and
         * some with PORTMOD driver.
         */
    case soc_feature_mixed_legacy_and_portmod:
        /* GPORT EEE LPI counter tx/rx registers swapped */
    case soc_feature_gport_eee_lpi_tx_rx_swap:
        /* GBP support at l2 entry */
    case soc_feature_l2x_gbp_support:
    case soc_feature_timesync_nanosync:
    case soc_feature_nanosync_time_capture:
#if defined(BCM_TIMESYNC_LIVE_WIRE_TOD_SUPPORT)
    case soc_feature_timesync_live_wire_tod:
#endif /* BCM_TIMESYNC_LIVE_WIRE_TOD_SUPPORT */
    case soc_feature_unimac_no_rx_fifo_reset:
    case soc_feature_egphy_port_leverage:
    case soc_feature_vxlan_gpe_support:
        return TRUE;
    case soc_feature_vlan_ctrl_updates_l3_iif_under_4k_disable:
        if (soc_property_get(unit,
                    spn_VLAN_CTRL_UPDATES_L3_IIF_DISABLE, 0)) {
            return TRUE;
        }
        return FALSE;
#if defined(BCM_FLOWTRACKER_V2_SUPPORT)
        /* BroadScan 2.0 */
    case soc_feature_flex_flowtracker_indirect_memory_access:
        if (SAL_BOOT_XGSSIM || SAL_BOOT_BCMSIM) {
            return FALSE;
        }
    case soc_feature_flex_flowtracker_ver_2:
    case soc_feature_flex_flowtracker_ver_2_setid_war:
    case soc_feature_flowtracker_9banks_ft_table:
    case soc_feature_flowtracker_ver_2_drop_code:
    case soc_feature_flowtracker_ver_2_chip_e2e_delay:
    case soc_feature_flowtracker_ver_2_alu_delay:
    case soc_feature_flowtracker_ver_2_iat_idt_delay:
    case soc_feature_flowtracker_ver_2_mmu_congestion_level:
    case soc_feature_flowtracker_ver_2_load_balancing:
    case soc_feature_flowtracker_coupled_data_engines:
    case soc_feature_flowtracker_ts_64_sw_war_support:
    case soc_feature_flowtracker_ver_2_ftfp_tcam_atomic_hw_write:
    case soc_feature_flowtracker_ver_2_first_pkt_delay_sw_war:
    case soc_feature_flex_ctr_lsb_port_ctrl:
        return TRUE;
    case soc_feature_flowtracker_5banks_ft_table:
    case soc_feature_flowtracker_3banks_ft_table:
    case soc_feature_flowtracker_half_entries_ft_group:
    case soc_feature_flowtracker_ts_ntp64_support:
    case soc_feature_hx5_ft_32bit_param_update:
        ...
        /* UTT 1.0 */
    case soc_feature_utt:
        if (SAL_BOOT_XGSSIM || SAL_BOOT_BCMSIM ||
            !soc_property_get(unit, "l3_defip_sizing", TRUE)) {
            return FALSE;
        }
        return TRUE;
    case soc_feature_td3x1_reduced_utt:
        if (SAL_BOOT_XGSSIM || SAL_BOOT_BCMSIM ||
            !soc_property_get(unit, "l3_defip_sizing", TRUE)) {
            return FALSE;
        }
        if ((dev_id == BCM56175_DEVICE_ID) ||
            (dev_id == BCM56176_DEVICE_ID) ||
            (dev_id == BCM53642_DEVICE_ID)) {
            return TRUE;
        }
        return FALSE;
    case soc_feature_wh3_reduced_utt:
        if (SAL_BOOT_XGSSIM || SAL_BOOT_BCMSIM ||
            !soc_property_get(unit, "l3_defip_sizing", TRUE)) {
            return FALSE;
        }
        if (dev_id == BCM53642_DEVICE_ID) {
            return TRUE;
        }
        return FALSE;
#endif  /* BCM_UTT_SUPPORT */

        /* Supported on SVK only */
    case soc_feature_visibility:
        if (SAL_BOOT_XGSSIM || SAL_BOOT_BCMSIM) {
            return FALSE;
        }
    ...
    case soc_feature_twamp_owamp_support:
    case soc_feature_ioam_support:
    case soc_feature_ip_checksum_validate:
    case soc_feature_separate_key_for_ipmc_route:
    case soc_feature_enable_flow_based_udf_extraction:
    case soc_feature_allow_l3_ing_tag_actions:
    case soc_feature_pt2pt_access_vlan_check:
        return TRUE;
    case soc_feature_td3_lpm_ipmc_war:
    case soc_feature_l3mtu_fail_tocpu:
    case soc_feature_pt2pt_tunnel_term_learn_disable:
       ...
       /* L3MTU Fail to CPU WAR*/
    case soc_feature_hr4_l3mtu_fail_tocpu_war:
        /* Mutually exclusive with
         * soc_feature_nat and soc_feature_large_scale_nat */
        return TRUE;
       /* L2X static bit WAR */
    case soc_feature_hr4_l2x_static_bit_war:
        return TRUE;
        /* SOBMH WAR */
    case soc_feature_hr4_a0_sobmh_war:
        if (SAL_BOOT_XGSSIM || SAL_BOOT_BCMSIM) {
            return FALSE;
        } else if ((rev_id != BCM56275_A0_REV_ID) ||
                   (dev_id == BCM53642_DEVICE_ID)) {
            return FALSE;
        }
        return TRUE;
    case soc_feature_vlan_vp_sync_and_recover:
        return TRUE;

    case soc_feature_td3x1_enhanced_tdm:
    case soc_feature_td3x1_novrf:
    case soc_feature_td3x1_reduced_mmu_internal_buffer:
    case soc_feature_td3x1_reduced_ipmc:
    case soc_feature_no_vrf_stats:
        if ((dev_id == BCM56175_DEVICE_ID) ||
            (dev_id == BCM56176_DEVICE_ID)) {
            return TRUE;
        ...
        return FALSE;
    case soc_feature_td3x1_reduced_uft:
    case soc_feature_td3x1_reduced_nhop:
    case soc_feature_td3x1_reduced_ecmp:
    case soc_feature_td3x1_reduced_l3intf:
    case soc_feature_td3x1_reduced_vfi:
    case soc_feature_td3x1_reduced_flex_counter:
    case soc_feature_use_smbus3_for_i2c:
        if ((dev_id == BCM56175_DEVICE_ID) ||
            (dev_id == BCM56176_DEVICE_ID) ||
            (dev_id == BCM53642_DEVICE_ID)) {
            return TRUE;
        ...
        return FALSE;
    case soc_feature_td3x1_macsec_port_speed:
        return FALSE;
    case soc_feature_use_smbus2_for_i2c:
        ...
        return TRUE;
    case soc_feature_wh3_reduced_vrf:
    case soc_feature_wh3_reduced_mmu_internal_buffer:
    case soc_feature_wh3_reduced_ipmc:
    case soc_feature_wh3_reduced_iuat:
        if (dev_id == BCM53642_DEVICE_ID) {
            return TRUE;
        }
        return FALSE;
    case soc_feature_l3_intf_mcast_nhi_auto_del:
        return soc_property_get(unit, spn_BCM_L3_INTF_MCAST_NHI_AUTO_DEL, FALSE);

#ifdef INCLUDE_XFLOW_MACSEC
    case soc_feature_xflow_macsec_sa_table_corruption_war:
        if (SAL_BOOT_XGSSIM || SAL_BOOT_BCMSIM) {
            return FALSE;
        }
        return TRUE;
    case soc_feature_xflow_macsec:
        if (soc_property_get(unit, spn_XFLOW_MACSEC_DISABLE, 0)) {
            return FALSE;
        }
        return TRUE;
#endif /* INCLUDE_XFLOW_MACSEC */
    ...
    default:
        return soc_features_bcm56370_a0(unit, feature);
    }
}
#endif  /* BCM_56275 */
...
soc_features_bcm56370_a0(int unit, soc_feature_t feature)
{
    ...
    /* GPORT EEE LPI counter tx/rx registers swapped */
    case soc_feature_gport_eee_lpi_tx_rx_swap:
    case soc_feature_hx5_flex_gbp_support:
    case soc_feature_chip_icid_info:
    case soc_feature_update_defip_pair_data_only:
    case soc_feature_hecmp_failover_no_support:
        return TRUE;
    ...
    case soc_feature_twamp_owamp_support:
    case soc_feature_pt2pt_tunnel_term_learn_disable:
    case soc_feature_hgoe_line_card_mode:
        return FALSE;
    ...
    case soc_feature_flex_flowtracker_state_transition:
        if (soc_property_get(unit,
                    "flowtracker_flex_state_transition_mode", 0)) {
            return TRUE;
        } else {
            return FALSE;
        }
 ...
 */
int
soc_features_bcm56470_a0(int unit, soc_feature_t feature)
{
    uint16 dev_id;
    uint8  rev_id;

    soc_cm_get_id(unit, &dev_id, &rev_id);
    SOC_FEATURE_DEBUG_PRINT((DK_VERBOSE," 56470_a0"));
    switch (feature) {

    case soc_feature_repl_port_agg_map_is_mem:
    case soc_feature_split_4_repl_head_table:
    case soc_feature_split_2_repl_list_table:
    case soc_feature_time_support:
    case soc_feature_timesync_nanosync:
    case soc_feature_nanosync_time_capture:
#if defined(BCM_TIMESYNC_LIVE_WIRE_TOD_SUPPORT)
    case soc_feature_timesync_live_wire_tod:
#endif /* BCM_TIMESYNC_LIVE_WIRE_TOD_SUPPORT */
    case soc_feature_channelized_switching:
    case soc_feature_subport_forwarding_support:
    case soc_feature_ifp_meter_control_is_mem:
    case soc_feature_storm_control_meter_is_mem:
    case soc_feature_flex_flowtracker_ver_2:
    case soc_feature_flex_flowtracker_indirect_memory_access:
    case soc_feature_clmac_16byte_interface_mode:
    case soc_feature_l3_egress_idx_scale_200K:
    case soc_feature_l3_defip_v4_64k:
    case soc_feature_subport_per_line_card:
    case soc_feature_8k_l3_nh_per_bank:
    case soc_feature_field_flowtracker_v3_support:
    case soc_feature_flex_flowtracker_ver_3:
    case soc_feature_flowtracker_ver_2_drop_code:
    case soc_feature_flowtracker_ver_2_chip_e2e_delay:
    case soc_feature_flowtracker_ver_2_alu_delay:
    case soc_feature_flowtracker_ver_2_iat_idt_delay:
    ...
    case soc_feature_flowtracker_coupled_data_engines:
    case soc_feature_flowtracker_ts_ntp64_support:
    case soc_feature_flowtracker_sw_agg_periodic_export:
    case soc_feature_repl_head_icc:
    case soc_feature_lpm_prefilter:
    case soc_feature_dscp_map_index_sw_war:
    case soc_feature_efp_meter_noread:
    case soc_feature_vco_freq_config_enable:
    case soc_feature_uft_shared_8_banks_32k:
    case soc_feature_l3_ecmp_4k_groups:
    case soc_feature_egr_pvlan_eport_control_is_mem:
    case soc_feature_hla:
    case soc_feature_appl_signature_support:
    case soc_feature_vxlan_gpe_support:
    case soc_feature_l2x_bulk_improvement:
    case soc_feature_fb6_ecmp_group_partitioning:
    ...
    case soc_feature_bfd:
        return TRUE;
#ifdef INCLUDE_L3
    case soc_feature_field_aacl_compression:
        return soc_property_get(unit, spn_ALPM_PRE_FILTER, FALSE);
#endif
#ifdef BCM_UTT_SUPPORT
    case soc_feature_utt:
        if (SAL_BOOT_XGSSIM || SAL_BOOT_BCMSIM ||
    ...
    case soc_feature_utt_ver1_5:
        return TRUE;
    case soc_feature_td3x4_ax_reduced_utt:
        if(dev_id == BCM56474_DEVICE_ID) {
            return TRUE;
        } else {
            return FALSE;
        }
    ...
    case soc_feature_alpm_addr_xlate:
    case soc_feature_vlan_wirelss_traffic_select:
    /* Use iProc15.2 which is similar to iProc14(TD3)
       and hence, i2c will be on smbus1.
     */
    case soc_feature_use_smbus2_for_i2c:
    case soc_feature_tcb:
    case soc_feature_td3_lpm_ipmc_war:
    ...
    case soc_feature_pt2pt_tunnel_term_learn_disable:
        return FALSE;
    case soc_feature_td3x4_ax_reduced_nhop:
    case soc_feature_td3x4_ax_reduced_ifp_meter_table:
    case soc_feature_td3x4_ax_reduced_ing_flex_ctr:
    case soc_feature_td3x4_ax_uft_shared_8_banks_16k:
        if(dev_id == BCM56474_DEVICE_ID) {
            return TRUE;
        } else {
            return FALSE;
...
soc_features_bcm88675_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_generic_counters:
    case soc_feature_linkscan_lock_per_unit:
    case soc_feature_hw_linkscan_fault_support:
#ifdef INCLUDE_RCPU
    case soc_feature_rcpu_1:
#endif /* INCLUDE_RCPU */
    case soc_feature_time_support:
    ...
    case soc_feature_sat:
        return TRUE;
    case soc_feature_linkscan_fault_ignore_an_speed:
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "linkscan_fault_ignore_an_speed", 0)) {
            return TRUE;
        } else {
            return FALSE;
        }
...
soc_features_bcm88690_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_prop_suffix_full_dev_id:
    case soc_feature_prop_ignore:
    case soc_feature_iproc:
    case soc_feature_cmicx:
    ...
    case soc_feature_single_block_type_per_register:
    /* DMA */
    case soc_feature_sbusdma:
    ...
    case soc_feature_linkscan_flr_support:
#ifdef INCLUDE_RCPU
    case soc_feature_rcpu_1:
#endif /* INCLUDE_RCPU */
    case soc_feature_time_support:
...
#ifdef BCM_88850_A0
int
soc_features_bcm88850_a0(int unit, soc_feature_t feature)
{
    COMPILER_REFERENCE(unit);
    SOC_FEATURE_DEBUG_PRINT((" 88850_a0"));
    ...
    case soc_feature_prop_ignore:
    case soc_feature_iproc:
    case soc_feature_iproc_17:
    case soc_feature_access:
    case soc_feature_sbus_width_120B:
    case soc_feature_cmicx:
    case soc_feature_cmicx_v4:
    case soc_feature_no_ep_to_cpu:
    case soc_feature_extended_cmic_error:
    case soc_feature_cmicm_multi_schan_cmc:
    case soc_feature_cmicd_v2:
    ...
    case soc_feature_mem_direct_acc_last_used_first:
    case soc_feature_single_block_type_per_register:
    case soc_feature_reg_port_any_set_all_instance:
    /* DMA */
    case soc_feature_sbusdma:
    case soc_feature_dcb_type39:
    case soc_feature_cos_rx_dma:
    ...
    case soc_feature_linkscan_lock_per_unit:
    case soc_feature_linkscan_remove_port_info:
    case soc_feature_single_instance_cdmac:
    case soc_feature_linkscan_flr_support:
#ifdef INCLUDE_RCPU
    case soc_feature_rcpu_1:
#endif /* INCLUDE_RCPU */
...
#ifdef BCM_88830_A0
int
soc_features_bcm88830_a0(int unit, soc_feature_t feature)
{
    COMPILER_REFERENCE(unit);
    SOC_FEATURE_DEBUG_PRINT((" 88830_a0"));
...
#ifdef BCM_88480_A0
int
soc_features_bcm88480_a0(int unit, soc_feature_t feature)
{
    COMPILER_REFERENCE(unit);
    SOC_FEATURE_DEBUG_PRINT((" 88480_a0"));
    ...
    case soc_feature_single_instance_cdmac:
    case soc_feature_linkscan_flr_support:
    case soc_feature_sw_linkscan_flr_support:
#ifdef INCLUDE_RCPU
    case soc_feature_rcpu_1:
#endif /* INCLUDE_RCPU */
    case soc_feature_time_support:
...
#ifdef BCM_88480_B0
int
soc_features_bcm88480_b0(int unit, soc_feature_t feature)
{
    COMPILER_REFERENCE(unit);
    SOC_FEATURE_DEBUG_PRINT((" 88480_b0"));
...
soc_features_bcm88790_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_schan_err_check:
        ...
        return TRUE;
    case soc_feature_linkscan_remove_port_info:
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "linkscan_remove_port_info", 0)) {
            return TRUE;
        }
        return FALSE;
    case soc_feature_iproc_14_tx_arbiter_priority:
...
soc_features_bcm56450_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_queue_flush_war_enable:
        return TRUE;
    case soc_feature_urpf:
    case soc_feature_lpm_tcam:
    case soc_feature_ip_mcast:
...
soc_features_bcm56270_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_egr_dscp_map_ext_ref_check:
        return TRUE;
    case soc_feature_per_port_ing_egr_inner_outer_tpid_enable:
        return TRUE;
    case soc_feature_use_local_tod:
        return TRUE;
    case soc_feature_subtag_counters:
        return TRUE;
...
int
soc_features_bcm56670_c0(int unit, soc_feature_t feature)
{
    switch (feature) {
        ...
        default :
            return soc_features_bcm56670_b0(unit, feature);
    }
