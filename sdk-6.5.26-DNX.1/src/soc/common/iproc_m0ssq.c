...
/*
 * Function:
 *      soc_get_ptr_for_iproc_m0ssq_ctrl_get
 * Purpose:
 *      Get pointer to 'soc->iproc_m0ssq_ctrl[ucnum]' for the purpose
 *      reading structure elements from specified unit.
 * Parameters:
 *      unit - Device unit #
 *      ucnum - Core # to apply 'get' on
 * Returns:
 *      uint32 - Value of '&soc->iproc_m0ssq_ctrl[ucnum]'.
 *      If 'unit' is illegal or
 *      If 'ucnum' is illegal then return 'NULL'
 */
static soc_iproc_m0ssq_control_t
   *soc_get_ptr_for_iproc_m0ssq_ctrl_get(int unit,int ucnum)
{
    soc_control_t *soc;
    soc_iproc_m0ssq_control_t *ret;
    ...
    else if ((uint32)ucnum >= (uint32)MAX_UCORES)
    {
        ret = NULL;
    }
    else
    {
        ret = &soc->iproc_m0ssq_ctrl[ucnum];
    }
    return (ret);
}
#define SOC_GET_PTR_FOR_IPROC_M0SSQ_CTRL_GET(unit,ucnum) \
    soc_get_ptr_for_iproc_m0ssq_ctrl_get(unit,ucnum)
/* } */
#else
/* { */
#define SOC_GET_PTR_FOR_IPROC_M0SSQ_CTRL_GET(unit,ucnum) \
    &(SOC_CONTROL(unit)->iproc_m0ssq_ctrl[ucnum])
/* } */
#endif
...
soc_iproc_m0ssq_reset_ucore(int unit, int ucore, int reset)
{
    ...
    if (cur_reset == 1 && reset == 0 && ucore == 0 && fwdbg_enable != 0) {
        ...
        fwlog.ctrl = 0;
        fwlog.base = soc_iproc_sram_membase_get(unit) + U0_LOG_BUFFER;
...
soc_iproc_m0ssq_uc_fw_load(int unit, int uc, int offset, const uint8 *data, int len)
{
    ...
    for (i = 0; i < wlen; i += 4) {
        data32 = (((uint32)data[i + 3]) << 24) | (((uint32)data[i + 2]) << 16) | (((uint32)data[i + 1]) << 8) | data[i];
...
void soc_iproc_quad_event_thread(void *unit_vp)
{
    ...
    soc_iproc_m0ssq_control_t *iproc_m0ssq_for_get = SOC_GET_PTR_FOR_IPROC_M0SSQ_CTRL_GET(unit,ucnum);

    while (iproc_m0ssq_for_get->thread_interval != 0) {
        LOG_DEBUG(BSL_LS_SOC_M0, (BSL_META_U(unit,"soc_iproc_quad_event_thread: sleep %d\n"),
                                                iproc_m0ssq->thread_interval));

        (void) sal_sem_take(iproc_m0ssq_for_get->event_sema, sal_sem_FOREVER);
...
int soc_iproc_m0ssq_init(int unit)
{
   ...
   soc_iproc_m0ssq_control_t *iproc_m0ssq_for_get = NULL;
   ...
   for (ucnum = 0; ucnum < max_ucores; ucnum++) {
       ...
       iproc_m0ssq_for_get = SOC_GET_PTR_FOR_IPROC_M0SSQ_CTRL_GET(unit,ucnum);

       iproc_m0ssq->event_sema = sal_sem_create("m0ssq_intr", sal_sem_BINARY, 0);
       if (iproc_m0ssq_for_get->event_sema == NULL) {
       ...
       iproc_m0ssq->thread_pid = sal_thread_create(iproc_m0ssq_for_get->thread_name,
                                                  SAL_THREAD_STKSZ,
       ...
       if (iproc_m0ssq_for_get->thread_pid == SAL_THREAD_ERROR) {
           iproc_m0ssq->thread_interval = 0;
           sal_sem_destroy(iproc_m0ssq_for_get->event_sema);
...
int soc_iproc_m0ssq_exit(int unit)
{
    ...
    for (ucnum = 0; ucnum < MAX_UCORES; ucnum++) {
        ...
        if (iproc_m0ssq_for_get->event_sema) {
            sal_sem_give(iproc_m0ssq_for_get->event_sema);
        }
    }
...
/*
 * Function:
 *      soc_cmicx_icfg_sw_intr
 * Purpose:
 *      Process a S/W generated interrupt from m0 on iproc17
 * Parameters:
...
void
soc_cmicx_icfg_sw_intr(int unit, void *data)
{
    soc_control_t       *soc = SOC_CONTROL(unit);
    ...
    if (m0_u0_sw_prg_intr) {
        ...
        iproc_m0ssq_for_get = SOC_GET_PTR_FOR_IPROC_M0SSQ_CTRL_GET(unit,0);
        if (iproc_m0ssq_for_get->event_sema) {
            sal_sem_give(iproc_m0ssq_for_get->event_sema);
        }
    }
    if (m0_u1_sw_prg_intr) {
        ...
        iproc_m0ssq_for_get = SOC_GET_PTR_FOR_IPROC_M0SSQ_CTRL_GET(unit,1);
        if (iproc_m0ssq_for_get->event_sema) {
            sal_sem_give(iproc_m0ssq_for_get->event_sema);
        }
    }
    if (m0_u2_sw_prg_intr) {
        ...
        iproc_m0ssq_for_get = SOC_GET_PTR_FOR_IPROC_M0SSQ_CTRL_GET(unit,2);
        if (iproc_m0ssq_for_get->event_sema) {
            sal_sem_give(iproc_m0ssq_for_get->event_sema);
        }
    }
    if (m0_u3_sw_prg_intr) {
        ...
        iproc_m0ssq_for_get = SOC_GET_PTR_FOR_IPROC_M0SSQ_CTRL_GET(unit,3);
        if (iproc_m0ssq_for_get->event_sema) {
            sal_sem_give(iproc_m0ssq_for_get->event_sema);
        }
    }
...
/*
 * Function:
 *      soc_cmicx_icfg_sw_m0ssq_init
 * Purpose:
 *      Initialize m0 ssq message on iproc 17
 * Parameters:
 *      unit - unit number
 * Returns:
 *      standard SOC_E_XXX
 */
int
soc_cmicx_icfg_sw_m0ssq_init(int unit) {
    soc_cmic_intr_handler_t *handle;
    int rv = SOC_E_NONE;

    /* Register interrupts */
    handle = sal_alloc(sizeof(soc_cmic_intr_handler_t), "icfg_sw_interrupt");
    if (handle == NULL) {
        return SOC_E_MEMORY;
    }

    handle->intr_fn = soc_cmicx_icfg_sw_intr;
    handle->intr_data = (void *)0;
    handle->num = IPROC17_ICFG_PROG_INTR;
...
/*
 * Function:
 *      soc_iproc_m0ssq_sram_alloc
 * Purpose:
 *      Allocate a M0SSQ SRAM block. This function will return
 *      iproc address of SRAM block.
 *      User need to use soc_iproc_setreg/soc_iproc_getreg to
 *      access the SRAM.
 * Parameters:
 *      unit - unit number
 *      size - size of allocation.
 *      name - name of memory block.
 * Returns:
 *      0: allocation fail. otherwise return iproc address of SRAM block.
 */
uint32
soc_iproc_m0ssq_sram_alloc(int unit, uint32 size, const char *name)
{
    uint32 addr, i;
    ...
    if (name == NULL) {
        LOG_ERROR(BSL_LS_SOC_M0,
                 (BSL_META_U(unit, "Name is NULL.\n")));
        return 0;
    }

    /* Fixed allocation. */
    if (SOC_IS_HURRICANE4(unit)) {
        if (sal_strcmp(name, "SocHr4MacsecWarCfg") == 0 &&
            size < M0SSQ_SRAM_HR4_MACSEC_WAR_CFG_SIZE) {
            addr = soc_iproc_sram_membase_get(unit) +
                   M0SSQ_SRAM_HR4_MACSEC_WAR_CFG_OFFSET;
        } else if (sal_strcmp(name, "SocHr4MacsecWarData") == 0 &&
                   size < M0SSQ_SRAM_HR4_MACSEC_WAR_DATA_SIZE) {
            addr = soc_iproc_sram_membase_get(unit) +
                   M0SSQ_SRAM_HR4_MACSEC_WAR_DATA_OFFSET;
        }
    }
    ...
    if (addr == 0) {
        LOG_ERROR(BSL_LS_SOC_M0,
                 (BSL_META_U(unit, "Fail to allocate M0SSQ SRAM %s.\n"), name));
        return 0;
    }

    /* Clear to zero. */
    for (i = addr; i < (addr + size); i += 4) {
         soc_iproc_setreg(unit, i, 0);
    }
...
/*
 * Function:
 *      soc_iproc_m0ssq_sram_free
 * Purpose:
 *      Free allocated M0SSQ SRAM block.
 * Parameters:
 *      unit - unit number
 *      addr - iproc address of allocation.
 * Returns:
 *      N/A
 */
void
soc_iproc_m0ssq_sram_free(int unit, uint32 addr)
{
    return;
}
