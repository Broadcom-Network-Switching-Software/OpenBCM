    ...
    if (fix == 0) {
        ...
        if (EGR_IP_TUNNELm == reported_mem) {
            ...
            if (SOC_IS_TOMAHAWK2(unit) || SOC_IS_TOMAHAWK3(unit)) {
                ...
                * If no, clear EGR_IP_TUNNEL_IPV6 view.
                */
                cache = SOC_MEM_STATE(unit, check_mem_list).cache[copyno];
                vmap = SOC_MEM_STATE(unit, check_mem_list).vmap[copyno];
...
_soc_td2x_tdm_calendar_ser_correction(int unit, soc_mem_t mem, int index, int copyno, _soc_ser_correct_info_t *si)
{
    ...
    if (found) {
        ...
        if (((calendar_id == 0) && (i >= 0 && i <=1)) ||
            ...
            switch_mem = INVALIDm;
            cache = SOC_MEM_STATE(unit, tdm[i][1]).cache[copyno];
            vmap = SOC_MEM_STATE(unit, tdm[i][1]).vmap[copyno];
        }
                           ...
                           uint32 *entry)
{
    ...
    if (SOC_IS_TOMAHAWK3(unit) && (mem == IFP_TCAMm)) {
        ...
        cache = SOC_MEM_STATE(unit, mem_cache).cache[copyno];
        vmap = SOC_MEM_STATE(unit, mem_cache).vmap[copyno];
