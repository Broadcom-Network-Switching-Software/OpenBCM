...
unsigned int gh2_ddr4_init_tab_1600[] = {
    0,   0x00000a00,
    1,   0x00000000,
    2,   0x00000000,
    3,   0x00000000,
...
unsigned int gh2_ddr3_init_tab_1600[] = {
      0, 0x00000600,
      1, 0x00000000,
      2, 0x00000000,
      3, 0x00000000,
...
/* GREYHOUND2 DDR4 */
#define CONFIG_DDR4_GH2_ROW_BITS                15
#define CONFIG_DDR4_GH2_COL_BITS                10
#define CONFIG_DDR4_GH2_BANK_BITS               2
#define CONFIG_DDR4_GH2_BANK_GROUPS             2

/* GREYHOUND2 DDR3 */
#define CONFIG_DDR3_GH2_ROW_BITS                16
#define CONFIG_DDR3_GH2_COL_BITS                10
#define CONFIG_DDR3_GH2_BANK_BITS               3
#define CONFIG_DDR3_GH2_BANK_GROUPS             1

#define DDR_DENALI_CTL_177_ECC_MASK             0x78

/*
 * Function:
 *      soc_iproc_gh2_ddr_init
 * Purpose:
 *      Initialize DDR of the embedded iProc subsystem for Greyhound2
 * Parameters:
 *      unit - unit number
 * Returns:
 *      SOC_E_XXX
 */
STATIC int
soc_iproc_gh2_ddr_init(int unit)
{
    soc_timeout_t to;
    sal_usecs_t to_val = 50000; /* 50ms */
    uint32 ddr_type;            /* 1 if DDR4 */
    ...
    uint16 dev_id;
    uint8 rev_id;
    and28_f4_shmoo_dram_info_t sdi;
    and28_shmoo_config_param_t config_param;
    uint32 genpll_clk_tbl[] =
    {
        /*  mode,   ndiv,   MDIV
    ...
    if (ddr_type >= 2) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit,
                "Unsupported DDR type for Greyhound2: %d\n"), ddr_type));
        return SOC_E_UNAVAIL;
    }
    ...
    if (((rev_id & 0xff) == 0x11) && (ddr_type == 1)) {
        /* Greyhound2_B0 with DDR4 only */
        SOC_IF_ERROR_RETURN(READ_DDR_S2_IDM_IO_CONTROL_DIRECTr(unit, &rval));
        soc_reg_field_set(unit, DDR_S2_IDM_IO_CONTROL_DIRECTr, &rval,
                            DM_INVERTf, 1);
        SOC_IF_ERROR_RETURN(WRITE_DDR_S2_IDM_IO_CONTROL_DIRECTr(unit, rval));
    ...
    if (ddr_type) {
        /* DDR4 */
        sdi.dram_type = SHMOO_AND28_DRAM_TYPE_DDR4;
        sdi.num_columns     = (1 << CONFIG_DDR4_GH2_COL_BITS);
        sdi.num_rows        = (1 << CONFIG_DDR4_GH2_ROW_BITS);
        sdi.num_banks       = (1 << CONFIG_DDR4_GH2_BANK_BITS);
        sdi.num_bank_groups = (1 << CONFIG_DDR4_GH2_BANK_GROUPS);
    } else {
        /* DDR3 */
        sdi.dram_type = SHMOO_AND28_DRAM_TYPE_DDR3;
        sdi.num_columns     = (1 << CONFIG_DDR3_GH2_COL_BITS);
        sdi.num_rows        = (1 << CONFIG_DDR3_GH2_ROW_BITS);
        sdi.num_banks       = (1 << CONFIG_DDR3_GH2_BANK_BITS);
        sdi.num_bank_groups = (1 << CONFIG_DDR3_GH2_BANK_GROUPS);
    }

    SOC_IF_ERROR_RETURN(soc_and28_f4_shmoo_dram_info_set(unit, &sdi));
    SOC_IF_ERROR_RETURN(soc_and28_f4_shmoo_phy_init(unit, 0));

    sal_usleep(5000);

    /* DDR controller initialization */
    soc_iproc_ddr_reg_table_init(unit, DDR_DENALI_CTL_00r,
        (ddr_type ? gh2_ddr4_init_tab_1600 : gh2_ddr3_init_tab_1600));
    ...
    soc_timeout_init(&to, to_val, 0);
    for(;;) {
        /* INT_STATUS : Bit [8] = The MC initialization has been completed. */
        SOC_IF_ERROR_RETURN(READ_DDR_DENALI_CTL_175r(unit, &rval));
        if (soc_reg_field_get(unit, DDR_DENALI_CTL_175r, rval, INT_STATUSf)
            & 0x100) {
            break;
    ...
    /* Run or restore SHMOO */
    if (soc_property_get(unit, spn_DDR3_AUTO_TUNE, TRUE)) {
        soc_and28_f4_shmoo_ctl(unit, 0, SHMOO_AND28_SHMOO_RSVP, 0,
            SHMOO_AND28_CTL_FLAGS_PLOT_BIT, SHMOO_AND28_ACTION_RUN,
            &config_param);
        LOG_INFO(BSL_LS_SOC_COMMON,
            (BSL_META_U(unit, "DDR tuning completed.\n")));
...
int
soc_gh2_ydc_ddr_bist_config_set(int unit, int phy_ndx, ydc_ddr_bist_info_t *bist_info)
{
    uint32 data;

    SOC_IF_ERROR_RETURN(READ_DDR_BISTCONFIGURATIONSr(unit, &data));
    soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data,
                            WRITEWEIGHTf, bist_info->write_weight);
    soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data,
                            READWEIGHTf, bist_info->read_weight);
    soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data,
                            PATTERNBITMODEf, 0);
    soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data,
                            PRBSMODEf, bist_info->prbs_mode);
    soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data,
                            CONSADDR8BANKSf, 1);
    soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data,
                            ADDRESSSHIFTMODEf, 0);
    ...
    soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data,
                            INDWRRDADDRMODEf, 1);
    SOC_IF_ERROR_RETURN(WRITE_DDR_BISTCONFIGURATIONSr(unit, data));
...
int
soc_gh2_ydc_ddr_bist_run(int unit, int phy_ndx, ydc_ddr_bist_err_cnt_t *error_count)
{
    uint32 data;
    uint32 poll_count;

    SOC_IF_ERROR_RETURN(READ_DDR_BISTCONFIGURATIONSr(unit, &data));
    soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data, BISTENf, 1);
    SOC_IF_ERROR_RETURN(WRITE_DDR_BISTCONFIGURATIONSr(unit, data));
    ...
    while(TRUE)
    {
        ...
        if(soc_reg_field_get(unit, DDR_BISTSTATUSESr, data, BISTFINISHEDf))
        {
            SOC_IF_ERROR_RETURN(READ_DDR_BISTCONFIGURATIONSr(unit, &data));
            soc_reg_field_set(unit, DDR_BISTCONFIGURATIONSr, &data, BISTENf, 0);
            SOC_IF_ERROR_RETURN(WRITE_DDR_BISTCONFIGURATIONSr(unit, data));
            break;
        }
    ...
    error_count->bist_err_occur = soc_reg_field_get(unit, DDR_BISTERROROCCURREDr, data, ERROCCURREDf);

    SOC_IF_ERROR_RETURN(READ_DDR_BISTFULLMASKERRORCOUNTERr(unit, &data));
    error_count->bist_full_err_cnt = soc_reg_field_get(unit, DDR_BISTFULLMASKERRORCOUNTERr, data, FULLERRCNTf);

    SOC_IF_ERROR_RETURN(READ_DDR_BISTSINGLEBITMASKERRORCOUNTERr(unit, &data));
    error_count->bist_single_err_cnt = soc_reg_field_get(unit, DDR_BISTSINGLEBITMASKERRORCOUNTERr, data, SINGLEERRCNTf);

    SOC_IF_ERROR_RETURN(READ_DDR_BISTGLOBALERRORCOUNTERr(unit, &data));
    error_count->bist_global_err_cnt = soc_reg_field_get(unit, DDR_BISTGLOBALERRORCOUNTERr, data, GLOBALERRCNTf);

    return SOC_E_NONE;
}

int iproc28_enable_ecc(int unit)
{
    uint32 rval;
    ...
    SOC_IF_ERROR_RETURN(READ_DDR_DENALI_CTL_148r(unit, &rval));
    soc_reg_field_set(unit, DDR_DENALI_CTL_148r, &rval,
                                  ECC_DISABLE_W_UC_ERRf, 0);
    SOC_IF_ERROR_RETURN(WRITE_DDR_DENALI_CTL_148r(unit, rval));

    /* Disable ECC correction and reporting */
    SOC_IF_ERROR_RETURN(READ_DDR_DENALI_CTL_146r(unit, &rval));
    soc_reg_field_set(unit, DDR_DENALI_CTL_146r, &rval,
                                  ECC_ENf, 0);
    SOC_IF_ERROR_RETURN(WRITE_DDR_DENALI_CTL_146r(unit, rval));

    /* Disable auto corruption */
    SOC_IF_ERROR_RETURN(READ_DDR_DENALI_CTL_148r(unit, &rval));
    soc_reg_field_set(unit, DDR_DENALI_CTL_148r, &rval,
                                  ECC_DISABLE_W_UC_ERRf, 1);
    SOC_IF_ERROR_RETURN(WRITE_DDR_DENALI_CTL_148r(unit, rval));
    ...
    SOC_IF_ERROR_RETURN(READ_DDR_DENALI_CTL_177r(unit, &rval));
    soc_reg_field_set(unit, DDR_DENALI_CTL_177r, &rval,
                                  INT_ACKf, DDR_DENALI_CTL_177_ECC_MASK);
    SOC_IF_ERROR_RETURN(WRITE_DDR_DENALI_CTL_177r(unit, rval));

    return 0;
}

int iproc28_read_ecc_syndrome(int unit)
{
    uint32 syndrome = 0;
    SOC_IF_ERROR_RETURN(READ_DDR_DENALI_CTL_175r(unit, &syndrome));
    return(syndrome & DDR_DENALI_CTL_177_ECC_MASK);
}

int iproc28_clear_ecc_syndrome(int unit)
{
    /* Clear the interrupts, bits 6:3 */
    SOC_IF_ERROR_RETURN(WRITE_DDR_DENALI_CTL_177r(unit, DDR_DENALI_CTL_177_ECC_MASK));
    sal_udelay(100);
    return 0;
}
#endif /* BCM_GREYHOUND2_SUPPORT */
...
int soc_iproc_ddr_init(int unit)
{
    ...
    if (SOC_IS_GREYHOUND2(unit)) {
        return soc_iproc_gh2_ddr_init(unit);
    }
#endif /* BCM_GREYHOUND2_SUPPORT */
