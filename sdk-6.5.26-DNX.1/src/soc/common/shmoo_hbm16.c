...
soc_hbm16_calculate_step_size(int unit, int hbm_ndx, hbm16_step_size_t *ssPtr)
{
    ...
    {
        LOG_ERROR(BSL_LS_SOC_DDR, (BSL_META_U(unit, "VDL step size computation failed. Check power rails and clocks.\n")));
        return SOC_E_FAIL;
    }
    else
    {
    ...
    if(!stat)
    {
        ...
        for(; ndx < ndxEnd; ndx++)
        {
            ...
            for(; chn < chnEnd; chn++)
            {
                ...
                /*
                 * Save the config params in order to later save them on a flash file.
                 * Save also in case of 'restore' action, to handle the case where the params were restored from OTP.
                 * In that case, there is a version difference between the 'OTP' and 'regular tune', and saving the
                 * config_param will make the file we save on flash after 'restore from OTP' to be the same as after
                 * running 'regular tune'.
                 */
                if((action == SHMOO_HBM16_ACTION_RUN_AND_SAVE) || (action == SHMOO_HBM16_ACTION_SAVE) || (action == SHMOO_HBM16_ACTION_RESTORE))
                {
