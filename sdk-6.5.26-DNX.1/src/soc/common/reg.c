...
#include <shared/utilex/utilex_seq_dma.h>
#endif
    ...
    if (opcode != READ_REGISTER_ACK_MSG || err != 0) {
        {
            ...
            soc_schan_dump(unit, &schan_msg, 2, BSL_VERBOSE);
...
soc_direct_memreg_get(int unit, int cmic_block, uint32 addr, uint32 dwc_read, int is_mem, uint32 *data)
{
    ...
    uint32 allow_intr = 0;
    int data_byte_len;
    int opcode, err, rv = SOC_E_NONE;
#ifdef BCM_SAND_SUPPORT
    int block_inst = 0, block_inst_locked = 0;
#endif
    ...
    if (ACCESS_IS_INITIALIZED(unit)) {
        ...
        flags = is_mem ? FLAG_ACCESS_IS_MEMORY : 0;
        return access_direct_regmem(unit, flags, cmic_block, addr, dwc_read, data);
    }
#endif
...
#ifdef BCM_SAND_SUPPORT
    if (is_mem && SOC_IS_SAND(unit))
    {
        /* find the block instance from its sbus ID */
        for (; ; ++block_inst) {
            if (SOC_BLOCK_TYPE(unit, block_inst) < 0) {
                return SOC_E_PARAM; /* block instance not found */
            } else if (SOC_BLOCK_INFO(unit, block_inst).cmic == cmic_block) {
                break;
            }
        }

        SOC_IF_ERROR_RETURN(soc_sand_indirect_mem_access_block_instance_lock(unit, block_inst));
        block_inst_locked = 1;
    }
#endif

    /* Write header word + address DWORD, read header word + data DWORD */
    rv = soc_schan_op(unit, &schan_msg, 2, dwc_read+1, allow_intr);
...
#ifdef BCM_SAND_SUPPORT
    if (block_inst_locked) {
        i = soc_sand_indirect_mem_access_block_instance_unlock(unit, block_inst);
        if (rv == SOC_E_NONE) {
            rv = i;
        }
    }
...
/* set memory entry cache given soc block instance ID and memory address */
STATIC void soc_direct_blk_inst_mem_set_cache_update(int unit, int block_inst, uint32 addr, uint32 *entry_data)
{
    int rc;
    soc_mem_t mem;
    int index;
    ...
    mem = soc_blk_inst_and_addr_to_mem_extended(unit, block_inst, 0xff, addr);
    if(mem == INVALIDm)
    {
        return;
    }
    ...
    _soc_mem_write_cache_update(unit, mem, block_inst, 0, index, (int)array_index, entry_data, NULL, NULL, NULL);

    return;
}

/* set memory entry cache given sbus block instance ID and memory address */
void soc_direct_mem_set_cache_update(int unit, int cmic_block, uint32 addr, uint32 *entry_data)
{
    ...
    soc_direct_blk_inst_mem_set_cache_update(unit, blk, addr, entry_data);
    return;
}
...
/*
 * Function:
 *     soc_reg_dnx_global_reg_set
 * Purpose:
 *      Write to global registers for DNX.
 * Parameters:
 ...
 *      FAIL or PASS
 */
int soc_reg_dnx_global_reg_set(int unit, soc_reg_t reg, soc_reg_above_64_val_t data)
{
    int array_index, array_index_min = 0, array_index_max = 0;
    int rv = SOC_E_NONE;
    array_index_min = SOC_REG_FIRST_ARRAY_INDEX(unit, reg);
    array_index_max = SOC_REG_NUMELS(unit, reg) + array_index_min;
    for (array_index = array_index_min; array_index < array_index_max; array_index++)
    {
        rv = soc_reg_above_64_set_all_instances(unit, reg, array_index, data);
        if (rv != SOC_E_NONE) {
...
{
    schan_msg_t schan_msg;
    int i, rv, block_inst = 0;
#ifdef BCM_SAND_SUPPORT
    int block_inst_locked = 0;
#endif
    uint32 allow_intr = 0;
    ...
    if (ACCESS_IS_INITIALIZED(unit)) {
        ...
        flags = is_mem ? (FLAG_ACCESS_IS_WRITE | FLAG_ACCESS_IS_MEMORY) : FLAG_ACCESS_IS_WRITE;
        return access_direct_regmem(unit, flags, cmic_block, addr, dwc_write, data);
    }
#endif
...
#endif
        soc_direct_blk_inst_mem_set_cache_update(unit, block_inst, addr, data);
    }
    /*
    ...
    rv = soc_schan_op(unit, &schan_msg, dwc_write+2, 0, allow_intr);
#ifdef BCM_SAND_SUPPORT
    if (block_inst_locked) {
        i = soc_sand_indirect_mem_access_block_instance_unlock(unit, block_inst);
        if (rv == SOC_E_NONE) {
...
#ifdef BCM_ACCESS_SUPPORT
/* call access_regmem() to access the given register, adapting the instance
 * number and flags according to the received port argument */
STATIC int access_regmem_wrapper(
    const int unit,
    uint32 flags,                    /* flags for the operation */
    soc_reg_t reg,                   /* the register to access */
    int port,                        /* The block instance number to access. */
    uint16 array_index,              /* The array index if this regmem is an array, or 0 */
    uint32 *value                    /* input/output: value read form register/memory */
)
{
    int actual_port;
    access_block_instance_num_t inst;
    access_regmem_id_t regmem_id = /* get the access regmem ID of the register */
      (unsigned)reg >= ACCESS_NOF_MCM_MAPPED_REGS ? ACCESS_INVALID_REGMEM_ID : access_mcm2access_reg_ids[(unsigned)reg];
    if (regmem_id == ACCESS_INVALID_REGMEM_ID) {
        return SOC_E_PARAM;
    }
    if (port == REG_PORT_ANY || port == SOC_CORE_ALL) {
        inst = ACCESS_ALL_BLOCK_INSTANCES;
    } else {
        actual_port = port & (~(SOC_REG_ADDR_INSTANCE_MASK | SOC_REG_ADDR_BLOCK_ID_MASK 
                      | SOC_REG_ADDR_SCHAN_ID_MASK | SOC_REG_ADDR_PHY_ACC_MASK));
        inst = actual_port;
        if (port == actual_port) {
            inst = port;
        } else if ((port & SOC_REG_ADDR_PHY_ACC_MASK) != 0) {
            flags |= FLAG_ACCESS_INST_TYPE_PHY_PORT;
        } else if ((port & (SOC_REG_ADDR_SCHAN_ID_MASK | SOC_REG_ADDR_BLOCK_ID_MASK)) != 0) {
            if ((port & SOC_REG_ADDR_BLOCK_ID_MASK) != 0) {
                /* convert soc block instance ID to sbus block instance ID */
                inst = SOC_BLOCK_INFO(unit, actual_port).cmic;
            }
            flags |= FLAG_ACCESS_INST_TYPE_SBUS_ID;
        } else if (SOC_REG_INFO(unit, reg).regtype == soc_portreg) {
            /* used to mark which CDMAC instance to use or just the block instance for per-port regmems */
            flags |= FLAG_ACCESS_INST_TYPE_CUSTOM;
        }
    }

    /* coverity[callee_ptr_arith:FALSE] */
    return access_regmem(unit, flags, regmem_id, inst, array_index, 0, value);
}
...
#ifdef BCM_ACCESS_SUPPORT
    if (ACCESS_IS_INITIALIZED(unit)) {
        uint32 converted_data[2] = {0};
        ACCESS_PREPARE_TO_USE_FROM_MCM(unit);
        rv = access_regmem_wrapper(unit, 0, reg, port, index, converted_data);
        COMPILER_64_SET(*data, converted_data[1], converted_data[0]);
        return rv;
    }
#endif /* BCM_ACCESS_SUPPORT */
...
soc_reg_above_64_get(int unit, soc_reg_t reg, int port, int index, soc_reg_above_64_val_t data)
{
    ...
    if (ACCESS_IS_INITIALIZED(unit)) {
        ACCESS_PREPARE_TO_USE_FROM_MCM(unit);
        return access_regmem_wrapper(unit, 0, reg, port, index, data);
    }
#endif /* BCM_ACCESS_SUPPORT */
               ...
               uint32 *data)
{
...
#endif
    {
        ...
        {
            src_blk = 
                SOC_BLOCK2SCH(unit, CMIC_BLOCK(unit));
...
#ifdef BCM_ACCESS_SUPPORT
    if (ACCESS_IS_INITIALIZED(unit)) {
        uint32 converted_data[2];
        ACCESS_PREPARE_TO_USE_FROM_MCM(unit);
        converted_data[0] = COMPILER_64_LO(data);
        converted_data[1] = COMPILER_64_HI(data);
        return access_regmem_wrapper(unit, FLAG_ACCESS_IS_WRITE, reg, port, index, converted_data);
    }
#endif /* BCM_ACCESS_SUPPORT */
...
/* For global registers in devices that need it, the CPU writes to all blocks instead of ASIC */
#ifdef BCM_DNX_SUPPORT
    if ((SOC_IS_J2P(unit) || SOC_IS_J2X(unit)) && (SOC_REG_BLOCK_IS(unit, reg, SOC_BLK_ECI)) && (addr < 256))
    {
        int i = 0;
        soc_reg_above_64_val_t data_above_64;
        const soc_reg_t *global_register_block_registers = NULL;
        SOC_REG_ABOVE_64_CLEAR(data_above_64);
        data_above_64[0] = COMPILER_64_LO(data);
        data_above_64[1] = COMPILER_64_HI(data);
        global_register_block_registers = SOC_IS_J2P(unit) ? j2p_common_regs_addr_2_global_register_block_registers[addr]
                                                             : j2x_common_regs_addr_2_global_register_block_registers[addr];
        if (global_register_block_registers != NULL)
        {
            for (i = 0; global_register_block_registers[i] != -1; i++)
            {
                rv = soc_reg_dnx_global_reg_set(unit, global_register_block_registers[i], data_above_64);
                if (rv != SOC_E_NONE) {
                    LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                    "soc_reg_dnx_global_reg_set: failed to set global register")));
                    return rv;
                }
            }
        }
...
soc_reg32_set_all_instances(int unit, soc_reg_t reg, int array_index, uint32 data)
{
    ...
    if (soc_feature(unit,soc_feature_single_block_type_per_register)) {
        ...
        {
            if (SOC_INFO(unit).block_valid[first_instance + instance] == 0) {
                continue;
            }
...
soc_reg_above_64_set(int unit, soc_reg_t reg, int port, int index, soc_reg_above_64_val_t data)
{
    ...
    if (ACCESS_IS_INITIALIZED(unit)) {
        ACCESS_PREPARE_TO_USE_FROM_MCM(unit);
        return access_regmem_wrapper(unit, FLAG_ACCESS_IS_WRITE, reg, port, index, data);
    }
#endif /* BCM_ACCESS_SUPPORT */
...
#ifdef BCM_DNXC_SUPPORT
    UTILEX_SEQ_SBUSDMA_DESC_ADD_REG(unit, reg, port, index, data);
#endif /* BCM_DNXC_SUPPORT */
...
/* For global registers in devices that need it, the CPU writes to all blocks instead of ASIC */
        ...
        if ((SOC_IS_J2P(unit) || SOC_IS_J2X(unit)) && (SOC_REG_BLOCK_IS(unit, reg, SOC_BLK_ECI)) && (addr < 256))
        {
            ...
            if (global_register_block_registers != NULL)
            {
                for (i = 0; global_register_block_registers[i] != -1; i++)
                {
                    rv = soc_reg_dnx_global_reg_set(unit, global_register_block_registers[i], data);
                    if (rv != SOC_E_NONE) {
                        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                   "soc_reg_dnx_global_reg_set: failed to set global register")));
                        return rv;
    ...
    if(!instance_mask) {
        ...
        if (REG_PORT_ANY == port ||instance_mask || !SOC_BLOCK_IN_LIST(unit, regblktype, portblktype)) {
            ...
            case SOC_BLK_IPROC:
                ...
                break;
            case SOC_BLK_NANOSYNC:
                block = NANOSYNC_BLOCK(unit);
                break;
            case SOC_BLK_IDI:
                block = IDI_BLOCK(unit, instance);
                break;
            case SOC_BLK_CRYPTO:
            ...
            case SOC_BLK_RTP:
                if(REG_PORT_ANY != port)
                    block = RTP_BLOCK(unit, port);
                else
                    block = RTP_BLOCK(unit, 0);
                break;
            ...
            case SOC_BLK_BRDC_FSRD:
                if(REG_PORT_ANY != port)
                    block = BRDC_FSRD_BLOCK(unit, port);
                else
                    block = BRDC_FSRD_BLOCK(unit, 0);
                break;
            case SOC_BLK_BRDC_HBC:
                if(REG_PORT_ANY != port)
                    block = BRDC_HBC_BLOCK(unit, port);
                else
                    block = BRDC_HBC_BLOCK(unit, 0);
                break;
            case SOC_BLK_MXQ:
                if(REG_PORT_ANY != port)
...
#define SOC_REG_ADDR_INVALID_PORT 0 /* for asserts */
    ...
    if ((!block_id_mask) && (!schan_id_mask) && (REG_PORT_ANY == port || instance_mask || !SOC_BLOCK_IN_LIST(unit, regblktype, portblktype))) {
        ...
        case SOC_BLK_BRDC_FSRD:
            ...
            if (SOC_IS_J2P(unit)) {
                if (block_core == SOC_CORE_ALL) {
                    block = SOC_CORE_ALL;
                } else {
                    block = BRDC_FSRD_BLOCK(unit, block_core);
                }
            } else
#endif
            block = BRDC_FSRD_BLOCK(unit, 0);
        ...
        case SOC_BLK_BRDC_FMAC:
            ...
            if (SOC_IS_J2P(unit)) {
                if (block_core == SOC_CORE_ALL) {
                    ...
                    block = BRDC_FMAC_BLOCK(unit, block_core);
                }
            } else
#endif
            block = BRDC_FMAC_BLOCK(unit, 0);
            break;
        case SOC_BLK_BRDC_HBC:
#if defined(BCM_88850_A0)
            if (SOC_IS_J2P(unit)) {
                if (block_core == SOC_CORE_ALL) {
                    ...
                    block = BRDC_HBC_BLOCK(unit, block_core);
                }
            } else
#endif
            block = BRDC_HBC_BLOCK(unit, 0);
        ...
        case SOC_BLK_LCM:
            ...
            break;
        case SOC_BLK_CCML:
            block=CCML_BLOCK(unit, blkport);
            break;
        case SOC_BLK_MESH_TOPOLOGY_WRAP:
            block=MESH_TOPOLOGY_WRAP_BLOCK(unit, blkport);
            break;
        case SOC_BLK_DTM:
            block=DTM_BLOCK(unit, blkport);
            break;
        case SOC_BLK_DTL:
            block=DTL_BLOCK(unit, blkport);
            break;
        case SOC_BLK_DFL:
            block=DFL_BLOCK(unit, blkport);
            break;
        case SOC_BLK_DFL_FBC:
            block=DFL_FBC_BLOCK(unit, blkport);
            break;
        case SOC_BLK_VTMON:
            block=VTMON_BLOCK(unit, blkport);
            break;
        ...
        case SOC_BLK_BRDC_CCML:
            block=BRDC_CCML_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DFL:
            block=BRDC_DFL_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DFL_FBC:
            block=BRDC_DFL_FBC_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_CCH_VD0:
            block=BRDC_CCH_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_CCH_VD1:
            block=BRDC_CCH_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_CCML_VD0:
            block=BRDC_CCML_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_CCML_VD1:
            block=BRDC_CCML_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DCH_VD0:
            block=BRDC_DCH_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DCH_VD1:
            block=BRDC_DCH_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DFL_FBC_VD0:
            block=BRDC_DFL_FBC_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DFL_FBC_VD1:
            block=BRDC_DFL_FBC_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DFL_VD0:
            block=BRDC_DFL_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DFL_VD1:
            block=BRDC_DFL_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DTL_VD0:
            block=BRDC_DTL_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DTL_VD1:
            block=BRDC_DTL_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DTM_VD0:
            block=BRDC_DTM_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DTM_VD1:
            block=BRDC_DTM_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_FMAC_HIGH:
            block=BRDC_FMAC_HIGH_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_FMAC_LOW:
            block=BRDC_FMAC_LOW_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_FMAC_HIGH_VD0:
            block=BRDC_FMAC_HIGH_BLOCK_VD0(unit);
            break;
        case SOC_BLK_BRDC_FMAC_HIGH_VD1:
            block=BRDC_FMAC_HIGH_BLOCK_VD1(unit);
            break;
        case SOC_BLK_BRDC_FMAC_LOW_VD0:
            block=BRDC_FMAC_LOW_BLOCK_VD0(unit);
            break;
        case SOC_BLK_BRDC_FMAC_LOW_VD1:
            block=BRDC_FMAC_LOW_BLOCK_VD1(unit);
            break;
        case SOC_BLK_BRDC_FSRD_HIGH:
            block=BRDC_FSRD_HIGH_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_FSRD_LOW:
            block=BRDC_FSRD_LOW_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_LCM_VD0:
            block=BRDC_LCM_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_LCM_VD1:
            block=BRDC_LCM_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_QRH_VD0:
            block=BRDC_QRH_VD0_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_QRH_VD1:
            block=BRDC_QRH_VD1_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DTL:
            block=BRDC_DTL_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_RTP:
            block=BRDC_RTP_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_MCT:
            block=BRDC_MCT_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_DTM:
            block=BRDC_DTM_BLOCK(unit);
            break;
        case SOC_BLK_PCU:
            block=PCU_BLOCK(unit);
            break;
        case SOC_BLK_GPORT:
        ...
        case SOC_BLK_TCMX:
            ...
            break;
        case SOC_BLK_DTQ:
            block = DTQ_BLOCK(unit, blkport);
            break;
        case SOC_BLK_NBUTX:
            block = NBUTX_BLOCK(unit, blkport);
            break;
        case SOC_BLK_PBU:
            block = PBU_BLOCK(unit, blkport);
            break;
        case SOC_BLK_PHB:
            block = PHB_BLOCK(unit, blkport);
            break;
        case SOC_BLK_QHC:
            block = QHC_BLOCK(unit, blkport);
            break;
        case SOC_BLK_MKX:
        ...
        case SOC_BLK_MDBB:
            ...
            break;
        case SOC_BLK_PADS:
            block = PADS_BLOCK(unit);
            break;
        case SOC_BLK_IPC:
            block = IPC_BLOCK(unit);
            break;
        case SOC_BLK_ARB:
            block = ARB_BLOCK(unit);
            break;
        case SOC_BLK_FAWR:
            block = FAWR_BLOCK(unit);
            break;
        case SOC_BLK_FAWT:
            block = FAWT_BLOCK(unit);
            break;
        case SOC_BLK_FBSW:
            block = FBSW_BLOCK(unit);
            break;
        case SOC_BLK_FDCRL:
            block = FDCRL_BLOCK(unit);
            break;
        case SOC_BLK_FDCRS:
            block = FDCRS_BLOCK(unit);
            break;
        case SOC_BLK_FDMP:
            block = FDMP_BLOCK(unit);
            break;
        case SOC_BLK_FDMXA:
            block = FDMXA_BLOCK(unit);
            break;
        case SOC_BLK_FDMXB:
            block = FDMXB_BLOCK(unit);
            break;
        case SOC_BLK_FDMXC:
            block = FDMXC_BLOCK(unit);
            break;
        case SOC_BLK_FDMXT:
            block = FDMXT_BLOCK(unit);
            break;
        case SOC_BLK_FECRL:
            block = FECRL_BLOCK(unit);
            break;
        case SOC_BLK_FECRS:
            block = FECRS_BLOCK(unit);
            break;
        case SOC_BLK_FFLXO:
            block = FFLXO_BLOCK(unit);
            break;
        case SOC_BLK_FFOA:
            block = FFOA_BLOCK(unit);
            break;
        case SOC_BLK_FFOB:
            block = FFOB_BLOCK(unit);
            break;
        case SOC_BLK_FGDMP:
            block = FGDMP_BLOCK(unit);
            break;
        case SOC_BLK_FGMAP:
            block = FGMAP_BLOCK(unit);
            break;
        case SOC_BLK_FLEXMAC:
            block = FLEXMAC_BLOCK(unit);
            break;
        case SOC_BLK_FLOTN:
            block = FLOTN_BLOCK(unit);
            break;
        case SOC_BLK_FMACR:
            block = FMACR_BLOCK(unit);
            break;
        case SOC_BLK_FMACT:
            block = FMACT_BLOCK(unit);
            break;
        case SOC_BLK_FMAP:
            block = FMAP_BLOCK(unit);
            break;
        case SOC_BLK_FMXA:
            block = FMXA_BLOCK(unit);
            break;
        case SOC_BLK_FMXBI:
            block = FMXBI_BLOCK(unit);
            break;
        case SOC_BLK_FMXBJ:
            block = FMXBJ_BLOCK(unit);
            break;
        case SOC_BLK_FMXC:
            block = FMXC_BLOCK(unit);
            break;
        case SOC_BLK_FMXT:
            block = FMXT_BLOCK(unit);
            break;
        case SOC_BLK_FOSW:
            block = FOSW_BLOCK(unit);
            break;
        case SOC_BLK_FPMR:
            block = FPMR_BLOCK(unit);
            break;
        case SOC_BLK_FPMT:
            block = FPMT_BLOCK(unit);
            break;
        case SOC_BLK_FPTPR:
            block = FPTPR_BLOCK(unit);
            break;
        case SOC_BLK_FPTPT:
            block = FPTPT_BLOCK(unit);
            break;
        case SOC_BLK_FRWA:
            block = FRWA_BLOCK(unit);
            break;
        case SOC_BLK_FRWBC:
            block = FRWBC_BLOCK(unit);
            break;
        case SOC_BLK_OFR:
            block = OFR_BLOCK(unit);
            break;
        case SOC_BLK_OFT:
            block = OFT_BLOCK(unit);
            break;
        case SOC_BLK_CIG:
            block = CIG_BLOCK(unit);
            break;
        case SOC_BLK_FECPB:
            block = FECPB_BLOCK(unit);
            break;
        case SOC_BLK_FMXBK:
            block = FMXBK_BLOCK(unit);
            break;
        case SOC_BLK_WB:
            block = WB_BLOCK(unit);
            break;
        case SOC_BLK_FICPB:
            block = FICPB_BLOCK(unit);
            break;
        case SOC_BLK_FDMXBI:
            block = FDMXBI_BLOCK(unit);
            break;
        case SOC_BLK_FDMXBJ:
            block = FDMXBJ_BLOCK(unit);
            break;
        case SOC_BLK_W40:
            block = W40_BLOCK(unit);
            break;
        case SOC_BLK_FMXAI:
            block = FMXAI_BLOCK(unit);
            break;
        case SOC_BLK_FMXAJ:
            block = FMXAJ_BLOCK(unit);
            break;
        case SOC_BLK_TSN:
            block = TSN_BLOCK(unit);
            break;
        case SOC_BLK_IPW:
            block = IPW_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).ipw_blocks;
            break;
        case SOC_BLK_DC3MAC:
            block = DC3MAC_BLOCK(unit, blkport);
            break;
        case SOC_BLK_DC3PORT:
            block = DC3PORT_BLOCK(unit, blkport);
            break;
        case SOC_BLK_HBW:
            block = HBW_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).hbw_blocks;
            break;
        case SOC_BLK_HPW:
            block = HPW_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).hpw_blocks;
            break;
        case SOC_BLK_FDMX:
            block = FDMX_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).fdmx_blocks;
            break;
        case SOC_BLK_FLFR:
            block = FLFR_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).flfr_blocks;
            break;
        case SOC_BLK_FLFT:
            block = FLFT_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).flft_blocks;
            break;
        case SOC_BLK_FMX:
            block = FMX_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).fmx_blocks;
            break;
        case SOC_BLK_FOAM:
            block = FOAM_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).foam_blocks;
            break;
        case SOC_BLK_FODUO:
            block = FODUO_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).foduo_blocks;
            break;
        case SOC_BLK_FRA:
            block = FRA_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).fra_blocks;
            break;
        case SOC_BLK_MDBK:
        ...
        case SOC_BLK_FDTS:
            ...
            break;
        case SOC_BLK_MSW:
            block = MSW_BLOCK(unit, block_core);
            soc_blocks = SOC_INFO(unit).msw_blocks;
            break;
        default:
            block = -1; /* unknown non-port block */
            break;
...
soc_regaddrlist_alloc(soc_regaddrlist_t *addrlist)
{
    ...
    addrlist->count = 0;
    sal_memset(addrlist->ainfo, 0, sizeof(soc_regaddrinfo_t));
...
#if defined(BCM_CMICM_SUPPORT) && defined(BCM_ESW_SUPPORT)
    ...
    uint32 *is_write_only)
{
    ...
    *is_write_only = 0;
    ...
    if (finfop == NULL)
    {
        ...
        assert(finfop);
        /*
         * Code above does not necessarily quit this object.
         * Some other solution needs to be implemented.
         * Temporarily, add 'return' below.
         */
        return;
    }
 ...
 *     soc_is_reg_framer
 * Purpose:
 *      indicate if a register is customer for framer.
 * Parameters:
 *      unit         -  (IN) BCM device number.
 *      reg          -  (IN) Register
 * Returns:
 *      TRUE or FALSE
 */
int soc_is_reg_framer(int unit, soc_reg_t reg)
{
    int rc = 0;
    soc_block_types_t regblktype;
