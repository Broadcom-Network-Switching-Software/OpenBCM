...
cmicx_pktdma_ch_desc_done(int unit, void *data)
{
    ...
    if (sc->sc_q_cnt > 0) {
        ...
        if (SOC_IS_DNX2(unit))
        {
            /* Given chain_done interrupt arrives earlier than desc-done and no chain is started, make sure desc_done interrupt is enabled */
            if (!(sc->sc_flags & SOC_DMA_F_POLL)) {
                uint32 intr_base = 0;

                intr_base = cmicx_dma_get_intr_base(in_d->cmc);
                if (SOC_DMA_MODE(unit) == SOC_DMA_MODE_CONTINUOUS) {
                    soc_cmic_intr_enable(unit, intr_base + (4 * in_d->ch) + CH_DESC_CONTROLLED_INTR);
                } else {
                    soc_cmic_intr_enable(unit, intr_base + (4 * in_d->ch) + CH_DESC_DONE);
                }
            }
        }
#endif
