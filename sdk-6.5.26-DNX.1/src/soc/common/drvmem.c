...
/* Find a memory given its soc block instance ID and address */
/* Note: acc_type = -1 is used as don't care */
soc_mem_t
soc_blk_inst_and_addr_to_mem_extended(int unit, int block_inst, int acc_type, uint32 address)
{
    soc_mem_t       mem;
    uint32          offset, sbus_blk_inst_id = SOC_BLOCK2OFFSET(unit, block_inst);
    int block = -1; /* block (block type) */
    offset = address;
    if (SOC_IS_SAND(unit) || SOC_IS_GREYHOUND(unit) ||
        SOC_IS_HURRICANE3(unit) || SOC_IS_GREYHOUND2(unit)) {

#ifdef BCM_JERICHO_SUPPORT
        if (SOC_IS_JERICHO(unit) && SOC_BLOCK_IS_BROADCAST(unit, block_inst)) {
            block_inst = SOC_BLOCK_BROADCAST_MEMBER(unit, block_inst, 0); /* For broadcast blocks we want the type of the broadcast members */
        }
#endif /* BCM_JERICHO_SUPPORT */
        block = SOC_BLOCK_TYPE(unit, block_inst); /* Get block type for later comparisons from block */
    }
    ...
    if (soc_feature(unit, soc_feature_mem_direct_acc_last_used_first))
    {
        ...
        if (soc_addr_is_mem(unit, acc_type, sbus_blk_inst_id, block, offset, last_used_mem))
        {
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (soc_addr_is_mem(unit, acc_type, sbus_blk_inst_id, block, offset, mem))
        {
            if (soc_feature(unit, soc_feature_mem_direct_acc_last_used_first))
            {
...
/* Find a memory given its sbus block instance ID and address */
/* Note: acc_type = -1 is used as don't care */
soc_mem_t
