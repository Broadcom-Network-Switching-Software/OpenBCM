...
#define LOG_PKTDMA(err, ls_, stuff_)          \
    if (err) {                                \
        LOG_ERROR(ls_, stuff_);               \
    } else {                                  \
        LOG_VERBOSE(ls_, stuff_);             \
    }                                         \
 ...
 */
static void
cmicm_pdma_chan_reg_dump(int unit, int vchan, int err)
{
    uint32 val = 0, val2 = 0;
    int cmc, chan;
    ...
    LOG_PKTDMA(err, BSL_LS_SOC_PACKETDMA,
                (BSL_META_U(unit,
...
#ifdef INCLUDE_KNET
    if (SOC_KCHAN(unit, vchan) && SOC_WARM_BOOT(unit) &&
...
cmicm_dma_chan_start(int unit, sdc_t *sc)
{
    ...
    if (bsl_check(bslLayerSoc, bslSourcePacketdma, bslSeverityNormal, unit)) {
        cmicm_pdma_chan_reg_dump(unit ,sc->sc_channel, 0);
    }
    val = soc_pci_read(unit,CMIC_CMCx_CHy_DMA_CTRL_OFFSET(cmc, chan));
    val |= PKTDMA_ENABLE;
                    ...
                    int * detected)
{
    ...
    case SOC_DMA_POLL_CHAIN_DONE:
        *detected = ( soc_pci_read(unit, CMIC_CMCx_DMA_STAT_OFFSET(cmc))
        ...
        break;
    case SOC_DMA_POLL_ADDR_DECODE_ERR:
        *detected = ( soc_pci_read(unit, CMIC_CMCx_DMA_STAT_OFFSET(cmc))
                     & DS_CMCx_DMA_ADDR_DECODE_ERR(chan) );
        break;
    default:
        break;
    }
...
/*
 * Function:
 *      cmicm_dma_chan_restart
 * Purpose:
 *      Restart the CMICM DMA channel for the specified SOC unit.
 * Parameters:
 *      unit - SOC unit #
 *      sc   - SOC channel data structure pointer
 * Returns:
 ...
 */
static int
cmicm_dma_chan_restart(int unit, sdc_t *sc)
{
    int             rv = SOC_E_NONE;
    int             cmc;
    int             chan;
    ...
    /* Abort the active DMA */
    rv = cmicm_dma_chan_abort(unit, sc->sc_channel);
    if (SOC_FAILURE(rv)) {
        cmicm_pdma_chan_reg_dump(unit, sc->sc_channel, 1);
        return rv;
    }
 ...
 */
static int
cmicm_dma_pci_timeout_handle(int unit, int cmc)
{
    int rv = SOC_E_NONE;
    ...
    if (SOC_FEATURE_CMICD(unit)) {
        int timeout  =  PCI_COMPLETION_WAIT_TIMEOUT_USEC;
        soc_timeout_t to;
        ...
        for (chan = 0; chan < N_DMA_CHAN; chan++) {
            ...
            if (!(DS_CMCx_DMA_ACTIVE(chan) & stat)) {
                continue;
            }

            /* Check if DMA_ADDR_DECODE_ERR is set */
            ...
            /* Decode error is not set, DMA is locked */
            if (rv !=  SOC_E_NONE) {
                cmicm_pdma_chan_reg_dump(unit, vchan, 1);
                break;
            }

            sc = &soc->soc_channels[vchan];

            /* Restart the halted DMA channel */
            rv = cmicm_dma_chan_restart(unit, sc);
            if (SOC_FAILURE(rv)) {
                break;
            }
        }
...
/*
 * Function:
 *      cmicm_dma_chan_cur_op_datails
 * Purpose:
 *      Print CMICM DMA channel operation details.
 * Parameters:
 *      unit - SOC unit #
 *      sc   - SOC channel data structure pointer
 * Returns:
 ...
 */
static int
cmicm_dma_chan_cur_op_datails(int unit, sdc_t *sc)
{
    int rv = SOC_E_NONE;
#ifdef BCM_TOMAHAWK_SUPPORT
     uint32          val, addr;

    /* Enable dump  for Tomahawk/Tomahawk2 */
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)) {
        cmicm_pdma_chan_reg_dump(unit, sc->sc_channel, 1);

        /* Read CMIC_CMC0_HOSTMEM_ADDR_REMAP_0/1/2/3/4 */
        val = soc_pci_read(unit, CMIC_CMC0_HOSTMEM_ADDR_REMAP_0_OFFSET);
        LOG_ERROR(BSL_LS_SOC_PACKETDMA,
             (BSL_META_U(unit,
              "HOSTMEM_ADDR_REMAP_0: 0x%x\n"), val));

        val = soc_pci_read(unit, CMIC_CMC0_HOSTMEM_ADDR_REMAP_1_OFFSET);
        LOG_ERROR(BSL_LS_SOC_PACKETDMA,
            (BSL_META_U(unit,
             "HOSTMEM_ADDR_REMAP_1: 0x%x\n"), val));

        val = soc_pci_read(unit, CMIC_CMC0_HOSTMEM_ADDR_REMAP_2_OFFSET);
        LOG_ERROR(BSL_LS_SOC_PACKETDMA,
            (BSL_META_U(unit,
             "HOSTMEM_ADDR_REMAP_2: 0x%x\n"), val));

        val = soc_pci_read(unit, CMIC_CMC0_HOSTMEM_ADDR_REMAP_3_OFFSET);
        LOG_ERROR(BSL_LS_SOC_PACKETDMA,
             (BSL_META_U(unit,
             "HOSTMEM_ADDR_REMAP_3: 0x%x\n"), val));


        val = soc_pci_read(unit, CMIC_CMC0_CONFIG_OFFSET);
        LOG_ERROR(BSL_LS_SOC_PACKETDMA,
            (BSL_META_U(unit,
             "CMIC_CMC0_CONFIG: 0x%x\n"), val));

        (void)READ_PAXB_0_PAXB_TX_ARBITER_PRIORITYr(unit, &val);
         LOG_ERROR(BSL_LS_SOC_PACKETDMA,
             (BSL_META_U(unit,
            "_PAXB_TX_ARBITER_PRIORITY: 0x%x\n"), val));

        /* Dump PCI controller register */
        for (addr = 0x980; addr <= 0x994; addr += 4) {
            (void)WRITE_PAXB_0_CONFIG_IND_ADDRr(unit, addr);
            (void)READ_PAXB_0_CONFIG_IND_DATAr(unit, &val);
            LOG_ERROR(BSL_LS_SOC_PACKETDMA,
                (BSL_META_U(unit,
                "PCI_CTRL: Addr = 0x%x, val = 0x%x\n"), addr, val));
        }
     }
#endif
 ...
 *
 */
#define CMIC_DMA_STAT_ERR_MASK    0xfffff000
static int
cmicm_dma_chan_chain_done(int unit, int vchan, int mitigation)
{
    ...
    /* Dump all the registers in case any error bit is set */
    sc.sc_channel = vchan;
    if (val & CMIC_DMA_STAT_ERR_MASK) {
        cmicm_dma_chan_cur_op_datails(unit, &sc);
#if defined(BCM_ESW_SUPPORT) && defined(BCM_IPROC_SUPPORT)
        if (SOC_FEATURE_CMICD(unit)) {
            soc->soc_pdma_err_count[cmc][chan]++;
            if (soc->soc_pdma_err_count[cmc][chan] >= soc->soc_max_pdma_err_count) {
                LOG_ERROR(BSL_LS_SOC_PACKETDMA,
                      (BSL_META_U(unit,
                                  "Max consective PKTDMA error unit %d: "
                                  "cmc = %d, channel %d\n"), unit, cmc,  chan));
                soc->soc_pdma_err_count[cmc][chan] = 0;
                return SOC_E_FAIL;
            }
        }
#endif
        ...
        soc->soc_pdma_err_count[cmc][chan] = 0;
        if (bsl_check(bslLayerSoc, bslSourcePacketdma,
                      bslSeverityNormal, unit)) {
            cmicm_pdma_chan_reg_dump(unit, sc.sc_channel, 0);
        }
    }
...
soc_cmicm_dma_drv_init(int unit, soc_cmic_dma_drv_t *drv)
{
    ...
    drv->chan_restart               = cmicm_dma_chan_restart;
    ...
    drv->chan_cur_op_datails        = cmicm_dma_chan_cur_op_datails;
