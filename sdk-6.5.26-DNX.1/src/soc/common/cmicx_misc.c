...
/* Per design team credit value should be within 64, otherwise
 * cpu ip intf credit flow can potentially go into error state
 * CMIC_TOP_IPINTF_INTERFACE_CREDITSr.IP_INTF_CREDITSf */
#define CMICX_TOP_IPINTF_CREDIT_SAFE 64
                                      ...
                                      unsigned int *cmic_rev)
{
    ...
    if (cap_base) {
        ...
        /* return iproc version from the capability, and handle the bug of the capability showing 14 for iproc 15 */
        *iproc_ver = rval & 0xff;
        if (*iproc_ver == 14 && get_iproc_version_from_unit(unit) == 15)  {
            *iproc_ver = 15;
        }
...
int soc_cmicx_paxb_tx_arbiter_set(int unit, uint8 enable, uint8 priority)
{
    ...
    if (SOC_IS_TRIDENT3X(unit) || SOC_IS_JERICHO2(unit) || SOC_IS_RAMON(unit)) {
...
/*
 * Function:
 *   soc_cmicx_verify_before_idb_reset
 * Purpose:
 *   Verify if CMIC pipe is clean before idb reset is called
 * Parameters:
 *   unit - SOC unit #
 * Returns:
 * Notes:
 *   As per HW design team input before IDB reset is called CMIC pipe
 *   should be clean. Ideally there should not be any traffic during
 *   init so not taking any action here. Just flagging warning
 *   message.
 */
void
soc_cmicx_verify_before_idb_reset(int unit)
{
    uint32 rval = 0;
    uint32 buf_depth = 0;                  /* Buffer Depth */
    uint32 txbuf_num_free_entries = 0;     /* Tx buffer free entries */
    uint32 txbuf_def_num_free_entries = 0; /* Default Tx buffer free entries */

    /* Validate if pre idb reset conditions are proper */
    READ_CMIC_TOP_IPINTF_BUF_DEPTHr(unit, &rval);
    buf_depth = soc_reg_field_get(unit, CMIC_TOP_IPINTF_BUF_DEPTHr,
                                                              rval, BUF_DEPTHf);
    /* if buf depth is non zero it means there is still traffic in progress.
     * Ideally should not happen in init, but if we hit this then
     * we should try to put some delay for packet to go through */
    if (buf_depth != 0) {
        sal_udelay(10);
        LOG_WARN(BSL_LS_SOC_INIT,
                    (BSL_META_U(unit,
                     "CMIC_TOP_IPINTF_BUF_DEPTH buffer depth is non-zero"
                     ", current value %d\n"), buf_depth));
    }

    /* Should be set to def value, if it is not equal to default it means some
     * traffic is present. Add delay before reset for packet to go through */
    txbuf_def_num_free_entries =
      SOC_REG_INFO(unit, CMIC_CMC0_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr).rst_val_lo;
    READ_CMIC_CMC0_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr(unit, &rval);
    txbuf_num_free_entries = soc_reg_field_get(unit,
                              CMIC_CMC0_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr,
                                                 rval, NUMBER_OF_FREE_ENTRIESf);
    if (txbuf_num_free_entries != txbuf_def_num_free_entries) {
        LOG_WARN(BSL_LS_SOC_DMA,
                    (BSL_META_U(unit,
                     "CMIC_CMC0_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIES is"
                     " non-default, current value %d\n"),
                     txbuf_num_free_entries));
    }

    /* Should be set to def value, if it is not equal to default it means some
     * traffic is present. Add delay before reset for packet to go through */
    txbuf_def_num_free_entries =
      SOC_REG_INFO(unit, CMIC_CMC1_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr).rst_val_lo;
    READ_CMIC_CMC1_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr(unit, &rval);
    txbuf_num_free_entries = soc_reg_field_get(unit,
                              CMIC_CMC1_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr,
                              rval, NUMBER_OF_FREE_ENTRIESf);
    if (txbuf_num_free_entries != txbuf_def_num_free_entries) {
        LOG_WARN(BSL_LS_SOC_DMA,
                    (BSL_META_U(unit,
                     "CMIC_CMC1_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIES is not"
                     " reset properly to default, current value %d\n"),
                     txbuf_num_free_entries));
    }
     ...
     * traffic is present. Add delay before reset for packet to go through */
    txbuf_def_num_free_entries =
        SOC_REG_INFO(unit, CMIC_RPE_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr).rst_val_lo;
    READ_CMIC_RPE_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr(unit, &rval);
    txbuf_num_free_entries = soc_reg_field_get(unit,
                              CMIC_RPE_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIESr,
                              rval, NUMBER_OF_FREE_ENTRIESf);
    if (txbuf_num_free_entries != txbuf_def_num_free_entries) {
        LOG_WARN(BSL_LS_SOC_DMA,
                    (BSL_META_U(unit,
                    "CMIC_RPE_SHARED_TXBUF_DATABUF_NUM_FREE_ENTRIES is not"
                    " reset properly to default, current value %d\n"),
                    txbuf_num_free_entries));
    }
}
...
/*
 * Function:
 *   soc_cmicx_top_intf_credit_reset
 * Purpose:
 *   Reset the CMICX TOP IP INTF
 * Parameters:
 *   unit - SOC unit #
 * Returns:
 * Notes:
 *   Reset CMICX IP Interface Credit
 *      a) First flush IP_INTF buffer by asserting FLUSH_IP_INTF_BUFFERf
 *      b) Program IP_INTF_CREDITSf for max credit available(Max Value - 64)
 *      c) Write credit by programming WR_IP_INTF_CREDITSf
 */
void
soc_cmicx_top_ip_intf_credit_reset(int unit)
{
    uint32 rval = 0;

    /* Reset sequence */
    READ_CMIC_TOP_IPINTF_INTERFACE_CREDITSr(unit, &rval);
    soc_reg_field_set(unit, CMIC_TOP_IPINTF_INTERFACE_CREDITSr,
                                              &rval, FLUSH_IP_INTF_BUFFERf, 1);
    WRITE_CMIC_TOP_IPINTF_INTERFACE_CREDITSr(unit, rval);

    soc_reg_field_set(unit, CMIC_TOP_IPINTF_INTERFACE_CREDITSr,
                                              &rval, FLUSH_IP_INTF_BUFFERf, 0);
    soc_reg_field_set(unit, CMIC_TOP_IPINTF_INTERFACE_CREDITSr,
                                                   &rval, IP_INTF_CREDITSf, 0);
    WRITE_CMIC_TOP_IPINTF_INTERFACE_CREDITSr(unit, rval);

    soc_reg_field_set(unit, CMIC_TOP_IPINTF_INTERFACE_CREDITSr,
                                                &rval, WR_IP_INTF_CREDITSf, 1);
    WRITE_CMIC_TOP_IPINTF_INTERFACE_CREDITSr(unit, rval);

    soc_reg_field_set(unit, CMIC_TOP_IPINTF_INTERFACE_CREDITSr,
                                                &rval, WR_IP_INTF_CREDITSf, 0);
    WRITE_CMIC_TOP_IPINTF_INTERFACE_CREDITSr(unit, rval);
}
/*
 * Function:
 *   soc_cmicx_verify_after_idb_reset
 * Purpose:
 *   Verify if CMICX TOP IP INTF is reset properly
 * Parameters:
 *   unit - SOC unit #
 * Returns:
 * Notes:
 *   As per HW design team input after IDB reset is called
 *   credit should be less than64. If init seq is not proper
 *   investage why outstanding credit is more than recommeneded
 *   credit value.
 */
void
soc_cmicx_verify_after_idb_reset(int unit)
{
    uint32 rval = 0;
    uint32 outs_credit = 0;                /* Outstanding credit */

    /* Validate if credit reset was proper */
    READ_CMIC_TOP_IPINTF_BUF_DEPTHr(unit, &rval);
    outs_credit = soc_reg_field_get(unit, CMIC_TOP_IPINTF_BUF_DEPTHr,
                                                    rval, OUTSTANDING_CREDITSf);
    /* As per HW design team it should be less than 64 in ideal conditions */
    if (outs_credit > CMICX_TOP_IPINTF_CREDIT_SAFE) {
        LOG_WARN(BSL_LS_SOC_INIT,
                    (BSL_META_U(unit,
                     "CMIC_TOP_IPINTF_BUF_DEPTH outstanding credit is not set"
                     " to safe limit %d, current value %d\n"),
                      CMICX_TOP_IPINTF_CREDIT_SAFE, outs_credit));
    }
}
/* } */
#endif
