...
cmicx_dnxf_fifodma_done(int unit, void *data)
{
    ...
    if (dnxf_data_fabric.fifo_dma.enable_get(unit) && SOC_DNXF_CONTROL(unit)->rx_reception_tmngr_hndlr != NULL) {
        SOC_CONTROL(unit)->stat.intr_fifo_dma[in_d->ch]++;
        (void)shr_thread_manager_trigger(SOC_DNXF_CONTROL(unit)->rx_reception_tmngr_hndlr);
    }
}
#endif
...
cmicx_dnx_fifodma_done(int unit, void *data)
{
    ...
    {
        rv = soc_dnx_fifodma_channel_get(unit, soc_dnx_fifodma_src_oam_event, &oam_oamp_event_channel);
        SHR_IF_ERR_EXIT(rv);
        dnx_fifodma_info.fifodma_thread_shr_thread_manager_handler.get(unit, &dnx_oam_oamp_event_fifodma_thread_info);
    }
    ...
    if (in_d->ch == olp_channel)
    {
        ...
        {
            rv = shr_thread_manager_trigger(dnx_l2_cpu_learn_thread_info[unit]);
        }
    }

    else if((dma_event_interface_enable == 1) && (in_d->ch == oam_oamp_event_channel))
    {
        if (dnx_oam_oamp_event_fifodma_thread_info != NULL)
        {
            rv = shr_thread_manager_trigger(dnx_oam_oamp_event_fifodma_thread_info);
        }
    }
    else
    {
 ...
 exit:
    if (rv) 
    {
         ...
         * In the case of DPC failure or get olp/oamp channel failure,
         * interrupt is disabled forerver, so enable it here.
         */
...
int cmicx_fifodma_init(int unit, soc_fifodma_drv_t *drv)
{
    ...
    handle = sal_alloc(sizeof(soc_cmic_intr_handler_t)*CMICX_N_FIFODMA_CHAN,
                       "fifodma_interrupt");
    sal_memset(handle, 0, sizeof(soc_cmic_intr_handler_t)*CMICX_N_FIFODMA_CHAN);
    if (handle == NULL) {
        return SOC_E_MEMORY;
    }
    ...
    if (SOC_IS_DNXF(unit))
    {
        ...
        nof_channels = dnxf_data_fabric.fifo_dma.nof_channels_get(unit);
