...
#define MAX_NOF_FLASH_VENDORS 10
#define MAX_NOF_FLASH_PROP_VALUES (SOC_PROP_NOF_VALUES*MAX_NOF_FLASH_VENDORS)
...
static int
soc_flash_params_soc_prop_get(int unit, uint8 *flash_id, soc_flash_user_params_t *params)
{
    int vals[MAX_NOF_FLASH_PROP_VALUES];
    int nof_flash_prop_values;
    int prop_set_index;

    nof_flash_prop_values = soc_property_get_csv(unit, spn_PCIE_FLASH_MEM_PARAMS, MAX_NOF_FLASH_PROP_VALUES, vals);
    if (nof_flash_prop_values == 0) {
        return SOC_E_UNAVAIL;
    } else if ((nof_flash_prop_values % SOC_PROP_NOF_VALUES) != 0) {
        LOG_ERROR(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit, "soc property %s must have a number of comma separated values which is a multiple of %u full sets of flash properties\n"),
                spn_PCIE_FLASH_MEM_PARAMS, SOC_PROP_NOF_VALUES));
        return SOC_E_PARAM;

    } else if (nof_flash_prop_values > MAX_NOF_FLASH_PROP_VALUES) {
        LOG_ERROR(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit, "soc property %s can not have more than %u flash properties comma separated values\n"),
                spn_PCIE_FLASH_MEM_PARAMS, MAX_NOF_FLASH_PROP_VALUES));
        return SOC_E_PARAM;
    }

    for (prop_set_index = 0; prop_set_index<nof_flash_prop_values;)
    {
        /* select the correct properties set according to manufacturer ID*/
        if (vals[prop_set_index + SOC_PROP_MANF_ID_VALUE_INDEX] == flash_id[0])
        {
            if (vals[prop_set_index + SOC_PROP_SECT_SIZE_VALUE_INDEX] < 0 || vals[prop_set_index + SOC_PROP_PAGE_SIZE_VALUE_INDEX] < 0 ||
                vals[prop_set_index + SOC_PROP_PAGE_SIZE_VALUE_INDEX] > vals[prop_set_index + SOC_PROP_SECT_SIZE_VALUE_INDEX]) {
                LOG_ERROR(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit, "Invalid flash chip parameters\n")));
                return SOC_E_PARAM;
            } else if (vals[prop_set_index + SOC_PROP_SECT_SIZE_VALUE_INDEX] % vals[prop_set_index + SOC_PROP_PAGE_SIZE_VALUE_INDEX]) {
                LOG_ERROR(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit, "Warning: flash chip sector size %d is not a multiple of the page size %d\n"),
                          vals[prop_set_index + SOC_PROP_PAGE_SIZE_VALUE_INDEX], vals[prop_set_index + SOC_PROP_SECT_SIZE_VALUE_INDEX]));
            }
            params->manf_id = vals[prop_set_index + SOC_PROP_MANF_ID_VALUE_INDEX];
            params->sector_size = vals[prop_set_index + SOC_PROP_SECT_SIZE_VALUE_INDEX];
            params->page_size = vals[prop_set_index + SOC_PROP_PAGE_SIZE_VALUE_INDEX];
            return SOC_E_NONE;
        }
        prop_set_index += SOC_PROP_NOF_VALUES;
    }
                   ...
                   soc_flash_conf_t *flash_conf)
{
    ...
    int rv = SOC_E_NONE;
    uint8 status;
    soc_flash_user_params_t flash_user_params = {0};
    ...
    if (SOC_SUCCESS(soc_flash_params_soc_prop_get(unit, id, &flash_user_params)) &&
