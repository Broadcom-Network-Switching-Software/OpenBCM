                ...
                void *p4, void *p5)
{
    ...
    if ((!p4) && (!p5)) {
        /* No params, consecutive DMA errors in RX, restart */
        if (!tx) {
            soc_event_generate(unit, SOC_SWITCH_EVENT_IO_ERROR,
...
soc_dma_done_chain(int unit, uint32 vchan)
{
    ...
    rv = cmic_drv[unit].chan_chain_done(unit, vchan, sdc_mtg->mitigation_started);
    if(SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_PACKETDMA,
                  (BSL_META_U(unit, "Error in PKDMA chain Done: rv=%d\n"), rv));
        /* Only apply DMA error mitigation for Tomahawk/Tomahawk2 */
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)) {
            int tx;
            SOC_DMA_LOCK(soc_dma_lock);
            ...
            tx = (dv_chain->dv_op == DV_TX);
            sal_dpc(dma_dpc_restart, INT_TO_PTR(unit),
                        INT_TO_PTR(tx), INT_TO_PTR(vchan), NULL, NULL);
        }
#endif
    }
    SOC_DMA_LOCK(soc_dma_lock);
...
soc_dma_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx_gen2)) {
        rx_chans = CMICX_GEN2_N_DMA_CHAN;
    ...
    flags &= ~SOC_DMA_F_DEFAULT;
    if (SOC_IS_DNX(unit))
    {
        /** Init all channels on CMC0 except channel 0 as RX */
...
soc_dma_attach(int unit, int reset)
{
    ...
    soc->soc_max_pdma_err_count =
         soc_property_get(unit, spn_PDMA_MAX_ERR_COUNT, 3);
    ...
    } else if (soc_feature(unit, soc_feature_cmicx_gen2)) {
        soc->soc_max_channels = SOC_CMCS_NUM(unit) * CMICX_GEN2_N_DMA_CHAN;
        soc->soc_dma_channels = SOC_PCI_CMCS_NUM(unit) * CMICX_GEN2_N_DMA_CHAN;
    } else if (soc_feature(unit, soc_feature_cmicx)) {
