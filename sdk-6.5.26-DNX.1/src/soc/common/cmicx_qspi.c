              ...
              uint32 mode)
{
    ...
    if ((SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) &&
        ...
        /* Set read Command byte for BSPI. Unless overridden, IPROC_BSPI_ADDR_LANES==4 */
        val = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "qspi_fast_read_cmd", (
          (IPROC_BSPI_ADDR_LANES == 4 && (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit))) ? IPROC_BSPI_QUAD_READ_CMD : IPROC_BSPI_READ_CMD));
        IPROC_QSPI_WRITE(unit, QSPI_BSPI_REGISTERS_CMD_AND_MODE_BYTEr, val);
        /* Enable flex mode to take effect */
        IPROC_QSPI_WRITE(unit, QSPI_BSPI_REGISTERS_FLEX_MODE_ENABLEr, 1);
