    ...
    uint32 *errored_token_count)
{
    ...
    uint32 serdes_err = 0;
    uint32 sig_lock[1] = { 0 };
    uint32 sig_lock_link = 0;
    int blk_id, reg_select, links_in_fmac;
    soc_port_t inner_link;
    int srd_id;
    int rv;
    ...
    inner_link = link_id % dnxf_data_device.blocks.nof_links_in_fsrd_get(unit);
    ...
    *sig_lock = soc_reg_field_get(unit, FSRD_SRD_STATUSr, reg_val, SRD_N_SYNC_STATUSf);
    sig_lock_link = SHR_BITGET(sig_lock, inner_link) >> inner_link;
    if (!sig_lock_link)
    {
        link_status_tmp |= DNXC_FABRIC_LINK_STATUS_NO_SIG_LOCK;
    }

    SHR_IF_ERR_EXIT(soc_dnxc_port_rx_locked_get(unit, link_id, &sig_acc));
    if (!sig_acc)
    {
        link_status_tmp |= DNXC_FABRIC_LINK_STATUS_NO_SIG_ACCEP;
    }

    serdes_err = (sig_lock_link & sig_acc) ? 0 : 1;

    inner_link = link_id % links_in_fmac;
    ...
    *field_val = soc_reg_field_get(unit, FMAC_FMAC_INTERRUPT_REGISTER_1r, reg_val, RX_CRC_ERR_N_INTf);
    if (SHR_BITGET(field_val, inner_link) || serdes_err)
    {
    ...
    bcm_fabric_link_connectivity_t * link_partner)
{
    ...
    if (SHR_BITGET(intergrity_vec, link_id))
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnxf_diag_connectivity_status_reg_parse(unit, reg_val, link_partner));
    }
    else
    {
