...
static shr_error_e
soc_ramon_fifo_dma_channel_enable(
    int unit,
    int channel)
{
    int rv;
...
{
    int blk;
    uint32 reg, fifo_dma_timeout, host_mem_size_in_bytes;
    int rv;
    uint64 reg64_val;
    int max_entries;
    schan_msg_t msg;
    ...
    switch (channel)
    {
        ...
        case soc_ramon_fifo_dma_channel_fabric_cell_dcml_7:
            ...
            max_entries = dnxf_data_fabric.fifo_dma.buffer_size_get(unit) / fifo_dma_info->config.entry_size;
    ...
    LOG_VERBOSE(BSL_LS_SOC_FABRIC, (BSL_META_U(unit, "Allocating %d memory \n"), host_mem_size_in_bytes));
    ...
    rv = soc_fifodma_start(unit, channel, TRUE,
                           fifo_dma_info->config.mem,
                           0,
                           channel,
                           fifo_dma_info->config.entry_size, fifo_dma_info->config.max_entries, fifo_dma_info->buffer);
    SHR_IF_ERR_EXIT(rv);
    ...
    soc_reg_field_set(unit, CMIC_COMMON_POOL_FIFO_CH0_RD_DMA_HOSTMEM_THRESHOLDr, &reg, THRESHOLDf,
                      dnxf_data_fabric.fifo_dma.threshold_get(unit));
    rv = soc_pci_write(unit, CMIC_COMMON_POOL_FIFO_CHy_RD_DMA_HOSTMEM_THRESHOLD_OFFSET(channel), reg);
    SHR_IF_ERR_EXIT(rv);

    fifo_dma_timeout = dnxf_data_fabric.fifo_dma.timeout_get(unit);
    ...
    int unit)
{
    ...
    if (dnxf_data_fabric.fifo_dma.enable_get(unit))
    {
        ...
        host_mem_num_entries =
            dnxf_data_fabric.fifo_dma.buffer_size_get(unit) / _SOC_RAMON_FIFO_DMA_FABRIC_CELL_ENTRY_SIZE;
        if (host_mem_num_entries < dnxf_data_fabric.fifo_dma.threshold_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "fabric_cell_fifo_dma_buffer size is small compared to fabric_cell_fifo_dma_threhold");
        }

        if (dnxf_data_fabric.fifo_dma.buffer_size_get(unit) <
            dnxf_data_fabric.fifo_dma.min_nof_entries_get(unit) *
            dnxf_data_fabric.fifo_dma.entry_size_get(unit)
            || dnxf_data_fabric.fifo_dma.buffer_size_get(unit) >
            dnxf_data_fabric.fifo_dma.max_nof_entries_get(unit) * dnxf_data_fabric.fifo_dma.entry_size_get(unit))
        {
