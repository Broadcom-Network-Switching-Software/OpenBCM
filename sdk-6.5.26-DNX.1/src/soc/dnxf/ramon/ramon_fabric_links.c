...
#define SOC_RAMON_FABRIC_LINKS_RETIMER_DELAY_BEFORE_PI_TX_USEC  (5)

#define SOC_RAMON_FABRIC_LINKS_RETIMER_DELAY_AFTER_PI_TX_USEC   (100)
    ...
    soc_port_t link)
{
    ...
    if (!IS_SFI_PORT(unit, link))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Port %d is not probed. \n", link);
    }
...
{
    uint32 reg32_val, bitmap[1];
    int blk, inner_link, nof_links_in_cch;
...
{
    uint32 reg32_val, bitmap[1];
    int blk, inner_link, is_vsc256_v1, nof_links_in_cch;
    ...
    *val = is_vsc256_v1 ? soc_dnxf_fabric_link_cell_size_VSC256_V1 : soc_dnxf_fabric_link_cell_size_VSC256_V2;
...
soc_ramon_fabric_link_repeater_enable_set(
    ...
    int enable)
{
    int nof_links_in_fmac = 0, fmac_instance = 0, include_empties = 0, link = 0, first_link_in_fmac = 0;
    ...
    COMPILER_64_ZERO(reg64_val);
    SHR_IF_ERR_EXIT(soc_reg_get(unit, FMAC_GENERAL_CONFIGURATION_REGISTERr, fmac_instance, 0, &reg64_val));
    soc_reg64_field32_set(unit, FMAC_GENERAL_CONFIGURATION_REGISTERr, &reg64_val,
                          GENERAL_CONFIGURATION_REGISTER_FIELD_1f, include_empties);
    SHR_IF_ERR_EXIT(soc_reg_set(unit, FMAC_GENERAL_CONFIGURATION_REGISTERr, fmac_instance, 0, reg64_val));
    ...
    int enable)
{
    ...
    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping,
                               (unit, port, &dch_instance, &dch_inner_link, SOC_BLK_DCH));
    SHR_IF_ERR_EXIT(rv);

    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping,
                               (unit, port, &fmac_instance, &fmac_inner_link, SOC_BLK_FMAC));
    SHR_IF_ERR_EXIT(rv);
    ...
    soc_reg64_field_set(unit, FMAC_FMAL_TX_GENERAL_CONFIGURATIONr, &reg64_val, FMAL_N_TX_SIMPLE_UNPACKING_ENf,
                        enable ? 0 : 1);
    ...
    soc_dnxf_fabric_link_remote_pipe_mapping_t pipe_map)
{
    ...
    int block_i = 0;
    soc_reg_t two_pipes_bmp_array[_SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS] =
        { DCH_TWO_PIPES_BMPr, DCML_TWO_PIPES_BMPr, LCM_TWO_PIPES_BMPr };
    soc_reg_t three_pipes_bmp_array[_SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS] =
        { DCH_THREE_PIPES_BMPr, DCML_THREE_PIPES_BMPr, LCM_THREE_PIPES_BMPr };
    soc_reg_t remote_pri_pipe_idx_array[_SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS] =
        { DCH_REMOTE_PRI_PIPE_IDXr, DCML_REMOTE_PRI_PIPE_IDXr, LCM_REMOTE_PRI_PIPE_IDXr };
    soc_reg_t remote_sec_pipe_idx_array[_SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS] =
        { DCH_REMOTE_SEC_PIPE_IDXr, DCML_REMOTE_SEC_PIPE_IDXr, LCM_REMOTE_SEC_PIPE_IDXr };
    soc_reg_t pipes_mapping_en_array[_SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS] =
        { DCH_PIPES_MAPPING_ENr, DCML_PIPES_MAPPING_ENr, LCM_PIPES_MAPPING_ENr };
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &instance_array[0], &inner_link_array[0], SOC_BLK_DCH)));
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &instance_array[1], &inner_link_array[1], SOC_BLK_DCML)));
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &instance_array[2], &inner_link_array[2], SOC_BLK_LCM)));
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &fmac_instance, &fmac_inner_link, SOC_BLK_FMAC)));
    ...
    for (block_i = 0; block_i < _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS; ++block_i)
    {
        ...
        SHR_IF_ERR_EXIT(soc_reg32_set(unit, two_pipes_bmp_array[block_i], instance_array[block_i], 0, reg32_val[0]));
        ...
        SHR_IF_ERR_EXIT(soc_reg32_set(unit, three_pipes_bmp_array[block_i], instance_array[block_i], 0, reg32_val[0]));
    }
    ...
    soc_reg_field_set(unit, FMAC_FMAL_GENERAL_CONFIGURATIONr, reg32_val, FMAL_N_PARALLEL_DATA_PATHf,
                      pipe_map.num_of_remote_pipes ==
                      0 ? dnxf_data_fabric.pipes.nof_pipes_get(unit) - 1 : pipe_map.num_of_remote_pipes - 1);
    SHR_IF_ERR_EXIT(WRITE_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, fmac_instance, fmac_inner_link, reg32_val[0]));
    ...
    if (pipe_map.num_of_remote_pipes >= 1)
    {
        ...
        for (block_i = 0; block_i < _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS; ++block_i)
        {
            ...
            SHR_IF_ERR_EXIT(soc_reg_above_64_get
                            (unit, remote_pri_pipe_idx_array[block_i], instance_array[block_i], 0, reg_above_64_val));
            SOC_REG_ABOVE_64_RANGE_COPY(reg_above_64_val, inner_link_array[block_i] * _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE,
                                        &pipe_map.remote_pipe_mapping[0], 0, _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE);
    ...
    if (pipe_map.num_of_remote_pipes >= 2)
    {
        ...
        for (block_i = 0; block_i < _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS; ++block_i)
        {
            ...
            SHR_IF_ERR_EXIT(soc_reg_above_64_get
                            (unit, remote_sec_pipe_idx_array[block_i], instance_array[block_i], 0, reg_above_64_val));
            SOC_REG_ABOVE_64_RANGE_COPY(reg_above_64_val, inner_link_array[block_i] * _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE,
                                        &pipe_map.remote_pipe_mapping[1], 0, _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE);
            SHR_IF_ERR_EXIT(soc_reg_above_64_set
                            (unit, remote_sec_pipe_idx_array[block_i], instance_array[block_i], 0, reg_above_64_val));
        }
    }
    ...
    for (block_i = 0; block_i < _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS; ++block_i)
    {
        ...
        soc_reg32_get(unit, pipes_mapping_en_array[block_i], instance_array[block_i], 0, &reg32_val[0]);
        ...
        soc_reg32_set(unit, pipes_mapping_en_array[block_i], instance_array[block_i], 0, reg32_val[0]);
    }
    ...
    soc_dnxf_fabric_link_remote_pipe_mapping_t * pipe_map)
{
    ...
    if (pipe_map->num_of_remote_pipes >= 1)
    {
        ...
        SHR_IF_ERR_EXIT(rv);
        SOC_REG_ABOVE_64_RANGE_COPY(&pipe_map->remote_pipe_mapping[0], 0, reg_above_64_val,
                                    dch_inner_link * _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE,
                                    _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE);
    }
    ...
    if (pipe_map->num_of_remote_pipes >= 2)
    {
        ...
        SHR_IF_ERR_EXIT(rv);
        SOC_REG_ABOVE_64_RANGE_COPY(&pipe_map->remote_pipe_mapping[1], 0, reg_above_64_val,
                                    dch_inner_link * _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE,
                                    _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE);
    }
    ...
    soc_dnxc_isolation_status_t val)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(soc_reg_get(unit, RTP_UNICAST_ROUTE_UPDATE_DIRTYr, REG_PORT_ANY, 0, &reg64_val));
        COMPILER_64_ALLONES(reg64_ones);
        SHR_IF_ERR_EXIT(soc_reg_set(unit, RTP_UNICAST_ROUTE_UPDATE_DIRTYr, REG_PORT_ANY, 0, reg64_ones));
        ...
        SHR_IF_ERR_EXIT(soc_reg_set(unit, RTP_UNICAST_ROUTE_UPDATE_DIRTYr, REG_PORT_ANY, 0, reg64_val));
    }
    ...
    soc_dnxc_isolation_status_t * val)
{
    ...
    if (!SOC_REG_ABOVE_64_IS_ZERO(specific_link_bm))
    ...
    soc_dnxc_fabric_pipe_t val)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_port_disable_and_save_enable_state(unit, link, 0, &link_original_state));

    SHR_IF_ERR_EXIT(soc_reg_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, blk_id, 0, &reg_val));
    *field_val_rx =
        soc_reg64_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg_val, LNK_LVL_FC_RX_ENf);
    *field_val_tx =
        soc_reg64_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg_val, LNK_LVL_FC_TX_ENf);
    ...
    soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg_val, LNK_LVL_FC_RX_ENf,
                        *field_val_rx);
    soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg_val, LNK_LVL_FC_TX_ENf,
                        *field_val_tx);
    SHR_IF_ERR_EXIT(soc_reg_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, blk_id, 0, reg_val));

    SHR_IF_ERR_EXIT(soc_dnxf_port_restore_enable_state(unit, link, 0, link_original_state));
exit:
    SHR_FUNC_EXIT;
}
    ...
    soc_port_t port)
{
    ...
    int dch_instance, dch_inner_link;
    int offset = 0;
    soc_dnxf_fabric_device_stage_t device_mode;
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_fabric_link_device_stage_get(unit, port, 1, &device_mode));
    ...
    if (highest_aldwp != 0)
    {
        if (device_mode == soc_dnxf_fabric_device_stage_fe2)
        {
            offset = 1;
        }
        else if (device_mode == soc_dnxf_fabric_device_stage_fe3)
        {
            offset = 2;
        }
    ...
    if (device_mode == soc_dnxf_fabric_device_stage_repeater)
    {
        highest_aldwp = 0;
    }
...
{
    int rv;
    int dch_instance, nof_dch, nof_links_in_dcq;
    SHR_FUNC_INIT_VARS(unit);
...
static shr_error_e
soc_ramon_fabric_links_tx_swap_set(
    int unit,
    soc_port_t link_a,
    soc_port_t link_b,
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(soc_ramon_lane_map_db_pm_map_get
                    (unit, pm_index, dnxf_data_device.blocks.nof_links_in_phy_core_get(unit), port_config.lane2serdes));
...
static shr_error_e
soc_ramon_fabric_links_retimer_loopback_peer_get(
    int unit,
    soc_port_t link_a,
    soc_port_t * link_b)
{
    ...
    *link_b = -1;
    ...
    for (lane_index = 0; lane_index < dnxf_data_device.blocks.nof_links_in_phy_core_get(unit); lane_index++)
    {
        if (lane_map.lane_map_tx[link_a_index] == port_config.lane2serdes[lane_index].tx_id)
        {
            ...
            *link_b =
                lane_index + SOC_RAMON_FABRIC_LINKS_CORE_INDEX_GET(unit,
                                                                   link_a) *
                dnxf_data_device.blocks.nof_links_in_phy_core_get(unit);
            break;
        }
    }
...
static shr_error_e
soc_ramon_fabric_links_retimer_cdr_lock_poll(
    int unit,
    soc_port_t link,
    int min_nof_polls,
    int usec_timeout,
...
shr_error_e
soc_ramon_fabric_link_retimer_cdr_lock_poll_check(
    int unit,
    soc_port_t link_a,
    soc_port_t link_b)
{
    ...
    usec_timeout = 6000000;

    SHR_IF_ERR_EXIT(soc_ramon_fabric_links_retimer_cdr_lock_poll
                    (unit, link_a, min_nof_polls, usec_timeout, &rx_locked));
    if (!rx_locked)
    {
    ...
    {
        SHR_IF_ERR_EXIT(soc_ramon_fabric_links_retimer_cdr_lock_poll
                        (unit, link_b, min_nof_polls, usec_timeout, &rx_locked));
        if (!rx_locked)
        {
            SHR_ERR_EXIT(_SHR_E_TIMEOUT, "Link %d: timeout - Rx is not locked\n", link_b);
        }
...
static shr_error_e
soc_ramon_fabric_links_link_status_lock_poll(
    int unit,
    soc_port_t link,
    int min_nof_polls,
    int usec_timeout,
    int *rx_locked)
{
    ...
    while (1)
    {
        SHR_IF_ERR_EXIT(soc_ramon_port_link_status_get(unit, link, rx_locked));
        if (*rx_locked)
        {
            break;
        }
...
static shr_error_e
soc_ramon_fabric_links_retimer_link_status_poll_check(
    int unit,
    soc_port_t link_a,
    soc_port_t link_b)
{
    ...
    SHR_IF_ERR_EXIT(soc_ramon_fabric_links_link_status_lock_poll
                    (unit, link_a, min_nof_polls, usec_timeout, &rx_locked));
    if (!rx_locked)
    {
        SHR_ERR_EXIT(_SHR_E_TIMEOUT, "Link %d: timeout - Rx FEC is not locked\n", link_a);
    }
    ...
    soc_port_t link_b)
{
    ...
    if (link_b != -1)
    {
        ...
        if (link_a_fec_enabled != link_b_fec_enabled)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "retimer FEC enable is %d for link %d, but %d for link %d. it should be the same for both links ",
        ...
        if (link_a_fec_enabled && link_b_fec_enabled)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             "FEC type is %d for link %d, but %d for link %d. it should be the same for both links ",
    ...
    if (link_b != -1)
    {
        ...
        else if (SOC_RAMON_FABRIC_LINKS_FMAC_INDEX_GET(unit, link_a) ==
                 SOC_RAMON_FABRIC_LINKS_FMAC_INDEX_GET(unit, link_b))
        {
            loopback_en = TRUE;
        ...
        PORTMOD_PORT_ENABLE_PHY_SET(squelch_flags);
        PORTMOD_PORT_ENABLE_TX_SET(squelch_flags);
        SHR_IF_ERR_EXIT(portmod_port_enable_set(unit, link_a, squelch_flags, 0));
        if (connected_link != link_a)
        {
            SHR_IF_ERR_EXIT(portmod_port_enable_set(unit, connected_link, squelch_flags, 0));
        }

        SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                        (unit, mbcm_dnxf_fabric_links_link_integrity_down_set, (unit, link_a, enable)));
        if (connected_link != link_a)
        {
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                            (unit, mbcm_dnxf_fabric_links_link_integrity_down_set, (unit, connected_link, enable)));
        }
        ...
        SHR_IF_ERR_EXIT(soc_ramon_fabric_link_retimer_cdr_lock_poll_check(unit, link_a, connected_link));

        SHR_IF_ERR_EXIT(portmod_port_phase_interpolator_rx_set(unit, link_a, enable));
        if (connected_link != link_a)
        {
            SHR_IF_ERR_EXIT(portmod_port_phase_interpolator_rx_set(unit, connected_link, enable));
        }
        ...
        if (link_a_fec_enabled)
        {
            SHR_IF_ERR_EXIT(soc_ramon_fabric_links_retimer_link_status_poll_check(unit, link_a, link_b));
        }

        sal_usleep(SOC_RAMON_FABRIC_LINKS_RETIMER_DELAY_BEFORE_PI_TX_USEC);
        SHR_IF_ERR_EXIT(portmod_port_phase_interpolator_tx_set(unit, link_a, enable));
        if (connected_link != link_a)
        {
            SHR_IF_ERR_EXIT(portmod_port_phase_interpolator_tx_set(unit, connected_link, enable));
        }
        sal_usleep(SOC_RAMON_FABRIC_LINKS_RETIMER_DELAY_AFTER_PI_TX_USEC);
        ...
        SHR_IF_ERR_EXIT(portmod_port_enable_set(unit, link_a, squelch_flags, 1));
        if (connected_link != link_a)
        {
            SHR_IF_ERR_EXIT(portmod_port_enable_set(unit, connected_link, squelch_flags, 1));
        }
    }
    ...
    else
    {
        ...
        if (link_a == connected_link)
        {
            tx_swap_required = FALSE;
                 ...
                 SOC_RAMON_FABRIC_LINKS_FMAC_INDEX_GET(unit, connected_link))
        {
            tx_swap_required = TRUE;
        }
