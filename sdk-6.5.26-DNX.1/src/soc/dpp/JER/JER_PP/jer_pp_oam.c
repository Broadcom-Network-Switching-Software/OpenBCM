...
soc_error_t soc_jer_pp_oam_oem1_mep_add(
    ...
    if ((is_active) || (!(SOC_PPC_OAM_IS_MEP_TYPE_Y1731(classifier_mep_entry->mep_type) &&
        ...
        if (new_mdl_mp_type == _JER_PP_OAM_MDL_MP_TYPE_MIP) {
           if (!(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_mip_standard_fwd", 0) == 1))
           {
     ...
     SOC_SAND_OUT int *header_offset)
{
    ...
    SOCDNX_INIT_FUNC_DEFS;
    SOCDNX_NULL_CHECK(header_buffer);
    SOCDNX_NULL_CHECK(header_offset);
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, OAMP_DOWN_PTCHr, REG_PORT_ANY, 0, DOWN_PTCH_PP_SSPf, &ssp));
    ...
    if (next_is_itmh) {
     ...
     SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, OAMP_PR_2_FW_DTCr, REG_PORT_ANY, 0, fwd_tc[mep_db_entry.priority], &tc));
     SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, OAMP_PR_2_FWDDPr, REG_PORT_ANY, 0, fwd_dp[mep_db_entry.priority], &dp));

     if (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) {
         if(SOC_IS_QAX(unit)){
             SOCDNX_IF_ERR_EXIT(READ_OAMP_LOCAL_PORT_2_SYSTEM_PORTm(unit, MEM_BLOCK_ANY, mep_db_entry.local_port, &entry));
         }
         else{
             SOCDNX_IF_ERR_EXIT(READ_OAMP_MEM_20000m(unit, MEM_BLOCK_ANY, mep_db_entry.local_port, &entry)); 
     ...
     if(soc_property_get(unit, spn_ITMH_PROGRAMMABLE_MODE_ENABLE, FALSE) ==0 ){
       if (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) {
             ...
             SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL,  (_BSL_SOCDNX_MSG("currently just support lb & tst in  ETH /MPLS-TP/PWE OAM mode")));
       }
       ...
       offset = _insert_itmh_for_tst_lb_arad(header_buffer, offset, fwd_dst_info, tc, dp, 0, 0, (mep_db_entry.mep_type==SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP  || mep_db_entry.mep_type==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE || mep_db_entry.mep_type==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL));
         ...
         if (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) {
            fwd_dst_info =  0x10000 | (entry&0xffff);
                ...
                (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL)) {
             _sys_phy_port_id = mep_db_entry.local_port;
             fwd_dst_info =  0x10000 | _sys_phy_port_id;
             extern_flag = 1;
         }
         else {
             SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL,  (_BSL_SOCDNX_MSG("currently just support lb & tst in  ETH /MPLS-TP OAM mode")));
         }
    ...
    if (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) {
     ...
     SOCDNX_IF_ERR_EXIT(_ARAD_PP_OAM_INTERNAL_READ_OAMP_MEP_DBm(unit, MEM_BLOCK_ANY, lb_tst_info->endpoint_id, entry_above_64)); 
     ...
     SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field64_read(unit, OAMP_CCM_TPID_MAPr, REG_PORT_ANY, 0, tpid_field[mep_db_entry.outer_tpid], &uint64_field));
     ...
     if (mep_db_entry.tags_num == 2) {
         ...
         SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field64_read(unit, OAMP_CCM_TPID_MAPr, REG_PORT_ANY, 0, tpid_field[mep_db_entry.inner_tpid], &uint64_field));
    ...
    offset = offset + udh_size;
     ...
     SOCDNX_IF_ERR_EXIT(READ_OAMP_MPLS_PWE_PROFILEr(unit, reg_data));
     ...
     exp = (reg_field[0] & 0x700) >> 8 ; 
      ...
      if (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) {
         ...
         SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, OAMP_Y_1731O_MPLSTP_GALr, REG_PORT_ANY, 0, Y_1731O_MPLSTP_GALf, &gal_gach));
         offset= _insert_4_bytes_for_tst_lb(header_buffer ,gal_gach,offset);
         SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, OAMP_Y_1731O_MPLSTP_GACHr, REG_PORT_ANY, 0, Y_1731O_MPLSTP_GACHf, &gal_gach));
         offset= _insert_4_bytes_for_tst_lb(header_buffer ,gal_gach,offset);
         ...
         SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit,OAMP_Y_1731O_PWE_GACHr, REG_PORT_ANY, 0, Y_1731O_PWE_GACHf, &pwe_gach));
