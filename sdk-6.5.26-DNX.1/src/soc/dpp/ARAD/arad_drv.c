...
#include <bcm_int/dpp/shr_occupation.h>
...
soc_arad_default_config_tm_get(int unit)
{
    ...
    SOC_DPP_CONFIG(unit)->tm.nof_flow_profile = 8;
    ...
    SOC_DPP_CONFIG(unit)->tm.soft_reset_on_dram_cmd_parity_err_enabled = FALSE;
...
soc_arad_default_config_general_get(int unit)
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ipmc_l2_ssm_mode_lem", 0)
        && (SOC_DPP_CONFIG(unit)->pp.local_switching_enable == 0)) {
    ...
    *channel=0;
    ...
    if (propval) {
        ...
        if (!found) {
            ...
            if (!sal_strncasecmp(s, prefix, prefix_len)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("TM_INTERNAL_PKT Interface is not enabled. property value (\"%s\") for %s"), propval, propkey));
            }
        }
...
soc_arad_str_prop_nif_elk_tcam_dev_type_get(int unit, ARAD_NIF_ELK_TCAM_DEV_TYPE *tcam_dev_type)
{
    ...
    if (propval) {
        ...
        } else if (dcmn_device_block_for_feature(unit, DCMN_NO_KBP_FEATURE)) {
           SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s does not support KBP."),
                                             soc_dev_name(unit)));
...
soc_arad_ports_info_config(int unit)
{
    ...
    SOC_DPP_CONFIG(unit)->pp.pon_port_ptc_size = 3;
    SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_tunnel_id_alloc_mode", 1);
...
#if defined(INCLUDE_KBP)
{
    ...
    if (dpp_arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311 || dpp_arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52321) {
        ...
        elk->kbp_port_cross_connectivity = soc_property_get(unit, spn_EXT_TCAM_PORT_CROSS_CONNECTIVITY, 0);
    }
...
soc_arad_info_config(int unit)
{
    ...
    switch(val)
    {
      ...
      if (SOC_IS_QUX(unit))
      {
          SOC_DPP_CONFIG(unit)->tm.soft_reset_on_dram_cmd_parity_err_enabled = (dpp_arad->init.dram.nof_drams != 0) &&
                  soc_property_suffix_num_get(unit, 0, "custom_feature", "soft_reset_on_dram_cmd_parity_err", FALSE);
      }
          ...
          if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
              ...
              if (val == 1) { 
                  ...
                  if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mixed_qos_model_enable", 0)) &&
                    (dpp_pp->routing_preserve_dscp & SOC_DPP_DSCP_PRESERVE_OUTLIF_CTRL_EN)) {
                      SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("DSCP preserve OutLIF control and mixed_qos_model could not be enabled at the same time.")));
                  }
              }
          }
      }
