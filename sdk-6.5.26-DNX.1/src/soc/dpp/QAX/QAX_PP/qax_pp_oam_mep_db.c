...
soc_error_t soc_qax_pp_oam_oamp_lm_dm_set(int unit,
    ...
    if((lm_dm_info->out_lif != 0) && (lm_dm_info->mep_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) && (lm_dm_mep_db_entry->is_piggyback_down)) {
        ...
        } else if (lm_dm_info->is_maid_20){
            if(user_header_size) {
                arad_pp_oamp_pe_program_profile_get(unit,  ARAD_PP_OAMP_PE_PROGS_20_MAID_UDH, &reg32_field);
            }else{
                arad_pp_oamp_pe_program_profile_get(unit,  ARAD_PP_OAMP_PE_PROGS_20_MAID, &reg32_field);
            }
        } else{
...
soc_error_t soc_qax_pp_oam_oamp_lm_dm_delete(int unit,
    ...
    case ARAD_PP_OAM_OAMP_MEP_DB_LM_DM_ACTION_REMOVE_LM:
        ...
        entry = lm_dm_info->dm_entry ? lm_dm_info->dm_entry : lm_dm_info->dm_one_way_entry;
        if (entry) {
            if (entry != lm_dm_info->last_entry) {
                
                res = soc_qax_pp_oam_mep_db_lm_dm_last_bit_write(unit, lm_dm_info, entry, 1);
                SOCDNX_IF_ERR_EXIT(res);
            }
            if (entry != lm_dm_info->mep_entry.lm_dm_ptr) {
    ...
    case ARAD_PP_OAM_OAMP_MEP_DB_LM_DM_ACTION_REMOVE_LM_STAT:
        entry = (lm_dm_info->action == ARAD_PP_OAM_OAMP_MEP_DB_LM_DM_ACTION_REMOVE_DM) ?
           (lm_dm_info->dm_entry ? lm_dm_info->dm_entry: lm_dm_info->dm_one_way_entry) :
...
soc_qax_pp_oam_oamp_lm_dm_search(int unit, uint32 endpoint_id, uint32 *found_bmp) {
    ...
    *found_bitmap_lcl = 0;
    ...
    if (lm_dm_info.dm_entry > 0 || lm_dm_info.dm_one_way_entry > 0) {
...
soc_qax_pp_oam_oamp_dm_get(int unit, SOC_PPC_OAM_OAMP_DM_INFO_GET *dm_info, uint8 *is_1DM) {
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    res = _arad_pp_oam_internal_no_cache_read_oamp_mep_DBm(unit, flags, MEM_BLOCK_ANY, entry, &reg_data);
    SOCDNX_IF_ERR_EXIT(res);
    ...
    case SOC_PPC_OAM_MEP_TYPE_DM:
        *is_1DM = FALSE;
        ...
        res = SOC_QAX_PP_OAM_INTERNAL_WRITE_OAMP_MEP_DB_DM_STAT_TWO_WAYm(unit, MEM_BLOCK_ALL, entry, reg_data);
        SOCDNX_IF_ERR_EXIT(res);
        break;
    case SOC_PPC_OAM_MEP_TYPE_DM_ONE_WAY:
        *is_1DM = TRUE;
        _soc_qax_pp_oam_dm_one_way_entry_read(unit, dm_info, reg_data);
        res = SOC_QAX_PP_OAM_INTERNAL_WRITE_OAMP_MEP_DB_DM_STAT_ONE_WAYm(unit, MEM_BLOCK_ALL, entry, reg_data);
        SOCDNX_IF_ERR_EXIT(res);
        break;
    default:
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL,
    ...
    if (lm_dm_info->mep_entry.lm_dm_ptr) {
        ...
        for (chain_count = 0;
            ...
            case SOC_PPC_OAM_MEP_TYPE_DM:
                ...
                break;
            case SOC_PPC_OAM_MEP_TYPE_DM_ONE_WAY:
                lm_dm_info->dm_one_way_entry = entry;
...
soc_error_t soc_qax_pp_oam_mep_db_lm_dm_last_bit_write(int unit,
    ...
    } else if (lm_dm_info->dm_one_way_entry == entry) {
        soc_OAMP_MEP_DB_DM_STAT_ONE_WAYm_field32_set(unit, reg_data, LAST_ENTRYf, val);
    } else {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL,
                             (_BSL_SOCDNX_MSG("Something went wrong.")));
