...
static int _cpm4x25_port_index_get(int unit, int port, pm_info_t pm_info,
...
int _cpm4x25_pm_port_init(int unit, int port, pm_info_t pm_info, int enable)
{
   ...
   if (first_index == -1) {
    ...
    if (enable) {
        ...
        /* Disable CLPORT to save power in CPRI mode. */
...
int cpm4x25_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    int pcs_lane, vco_freq, pll0_vco_rate, pll1_vco_rate;
    int master_pll = 0;
    ...
    if((add_info->interface_config.encap_mode != _SHR_PORT_ENCAP_CPRI) &&
      (add_info->interface_config.encap_mode != _SHR_PORT_ENCAP_RSVD4)){
        _SOC_IF_ERR_EXIT(cprimod_cpri_or_enet_port_set(unit, port, 0));
        _SOC_IF_ERR_EXIT
            (pm4x25_port_attach(unit, port, pm_info, add_info));

        /*
         * Disabled all CPRI clocks for power saving in ETH mode.
         */
        _SOC_IF_ERR_EXIT
            (cprimod_cpri_port_clock_disable_set(unit, port, 1));
        SOC_FUNC_RETURN;
    }
    ...
    */
    ...
    if (!PORTMOD_CORE_INIT_FLAG_CPRI_CORE_INITIALZIED_GET(core_is_initialized)) {
        if(!PORTMOD_CORE_INIT_FLAG_FIRMWARE_LOADED_GET(core_is_initialized) ||
            ...
            master_pll =  pm4x25_port_tsc_is_master_pll(unit, port, pm_info);
            _SOC_IF_ERR_EXIT(cprimod_core_init(unit, port, &PM_4x25_INFO(pm_info)->int_core_access,
                                           &init_cfg, master_pll, &core_sts));
    ...
    _SOC_IF_ERR_EXIT
        (_cpm4x25_pm_port_init(unit, port, pm_info, 1));
    /*
     *  Enabled all CPRI clocks.
     */
    _SOC_IF_ERR_EXIT
        (cprimod_cpri_port_clock_disable_set(unit, port, 0));
    ...
    /* Check if the VCO for the port is available In all CPRI mode check both VCO */
    /* cprimodInUsePll0_1 => both plls used (either cpri or eth) */
    if((port_config.pll_in_use == cprimodInUsePll0_1) && (vco_freq != pll0_vco_rate) && (vco_freq != pll1_vco_rate) &&
       (add_info->init_config.cpm_core_mode != portmodCpmMixEthCpri)) {
    /* portmodCpmMixEthCpri => cpri-only  configuration */
        LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit, "fail to get vcos -- Both PLL in use with vco %d : %d vco needed %d\n"),
        pll0_vco_rate, pll1_vco_rate, vco_freq));
        return SOC_E_PARAM;
    }

    /* Check if the VCO for the port is available In mix mode check VCO0 */
    if(((port_config.pll_in_use & cprimodInUsePll0) != 0) && (vco_freq != pll0_vco_rate) &&
       (add_info->init_config.cpm_core_mode == portmodCpmMixEthCpri)) {
        LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit, "fail to get vcos -- PLL0 in use in MIX mode with vco %d vco needed %d\n"),
        pll0_vco_rate, vco_freq));
        return SOC_E_PARAM;
    }
...
int cpm4x25_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
    ...
    if (PORTMOD_PORT_ENABLE_TX_GET(actual_flags)) {
        ...
        if (!enable) {
            _SOC_IF_ERR_EXIT
                (cprimod_tx_framer_tgen_enable_set(unit, port, FALSE));
            _SOC_IF_ERR_EXIT
...
int cpm4x25_port_interface_config_set(int unit, int port, pm_info_t pm_info, const portmod_port_interface_config_t* config, int phy_init_flags)
{
    ...
    cprimod_port_interface_config_t_init(&config_t);
    sal_memcpy(&phy_access, &(PM_4x25_INFO(pm_info)->int_core_access), sizeof(phymod_core_access_t));
    lane = (SOC_INFO(unit).port_l2p_mapping[port]-1)%CPRIMOD_MAX_LANES_PER_CORE;
    phy_access.access.lane_mask = (1<<lane);
    ...
    if (phy_init_flags == PORTMOD_INIT_F_TX_ONLY) {
        config_t.port_encap_type = cprimodPortEncapCpri;
...
int cpm4x25_cpri_port_rx_frame_optional_config_set(int unit, int port, pm_info_t pm_info, portmod_rx_config_field_t field, int value)
{
    SOC_INIT_FUNC_DEFS;
...
int cpm4x25_cpri_port_rx_frame_optional_config_get(int unit, int port, pm_info_t pm_info, portmod_rx_config_field_t field, int* value)
{
    SOC_INIT_FUNC_DEFS;
...
int cpm4x25_cpri_port_tx_frame_optional_config_set(int unit, int port, pm_info_t pm_info, portmod_tx_config_field_t field, uint32 value)
{
    SOC_INIT_FUNC_DEFS;
...
int cpm4x25_cpri_port_tx_frame_optional_config_get(int unit, int port, pm_info_t pm_info, portmod_tx_config_field_t field, uint32* value)
{
    SOC_INIT_FUNC_DEFS;
...
int cpm4x25_cpri_port_encap_set(int unit, int port, pm_info_t pm_info, uint32 queue_num, cprimod_encap_header_config_t* config)
{
    int lane_index=0;
    uint32 lane_map=0;
    uint32 frame_format=0;
    SOC_INIT_FUNC_DEFS;

    /* retrieved the frame format type. */
    _SOC_IF_ERR_EXIT
        (_cpm4x25_port_index_get(unit, port, pm_info, &lane_index, &lane_map));
    _SOC_IF_ERR_EXIT
        (PM4x25_CPRI_PORT_FRAME_FORMAT_GET(unit, pm_info, &frame_format, lane_index));

    if (frame_format != config->roe_frame_format) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
            (_SOC_MSG("Frame Format Does not Match Port frame format %d vs %d \n"),
             frame_format,
             config->roe_frame_format));
    }
    _SOC_IF_ERR_EXIT
...
int cpm4x25_cpri_port_encap_get(int unit, int port, pm_info_t pm_info, uint32 queue_num, cprimod_encap_header_config_t* config)
{
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_encap_header_config_get(unit, port, queue_num, frame_format, config));
...
int cpm4x25_cpri_port_tx_frame_tgen_enable_set(int unit, int port, pm_info_t pm_info, uint32 enable)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprimod_tx_framer_tgen_enable_set(unit, port, enable));

exit:
    SOC_FUNC_RETURN;
}

int cpm4x25_cpri_port_tx_frame_tgen_enable_get(int unit, int port, pm_info_t pm_info, uint32 *enable)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprimod_tx_framer_tgen_enable_get(unit, port, enable));
...
int cpm4x25_cpri_port_cw_fast_eth_config_set(int unit, int port, pm_info_t pm_info, const portmod_cpri_fast_eth_config_info_t* config_info)
{
    ...
    /*
     * clear the decap buffer and status.
     */
    encap_decap_data.sample_size          = 16;
    encap_decap_data.truncation_enable    = FALSE;
    encap_decap_data.truncation_type      = 0;
    ...
    encap_decap_data.mux_enable           = 0;
    encap_decap_data.bit_reversal         = FALSE;
    encap_decap_data.buffer_size          = 0;
    encap_decap_data.priority             = config_info->priority;
    encap_decap_data.cycle_size           = 0;
    encap_decap_data.is_agnostic          = 0;

    _SOC_IF_ERR_EXIT
        (cprimod_cpri_decap_data_config_set(unit, port, config_info->queue_num, &encap_decap_data));
    _SOC_IF_ERR_EXIT
        (cprimod_decap_memory_clear(unit, port, config_info->queue_num));
...
int cpm4x25_rsvd4_rx_control_flow_get (int unit, int port, pm_info_t pm_info, uint32 flow_id, portmod_control_flow_config_t* config)
{
    cprimod_control_flow_config_t flow_config;
    cprimod_rsvd4_encap_data_config_t encap_decap_data;
    cprimod_header_compare_entry_t hdr_entry;
    ...
    cprimod_control_flow_config_t_init(&flow_config);
    _SOC_IF_ERR_EXIT
        (cprimod_rsvd4_rx_control_flow_config_get(unit, port, flow_id, &flow_config));

    config->proc_type       = flow_config.proc_type;
    config->queue_num       = flow_config.queue_num;
    config->sync_profile    = flow_config.sync_profile;
    config->sync_enable     = flow_config.sync_enable;

    cprimod_rsvd4_encap_data_config_t_init(&encap_decap_data);
    _SOC_IF_ERR_EXIT
        (cprimod_rsvd4_encap_data_config_get(unit, port, config->queue_num, &encap_decap_data));

    config->queue_size = encap_decap_data.buffer_size;
    config->priority = encap_decap_data.priority;

    cprimod_header_compare_entry_t_init(&hdr_entry);
    _SOC_IF_ERR_EXIT
        (cprimod_header_compare_entry_for_flow_get(unit, port, flow_id, cprimodFlowTypeCtrl, &hdr_entry));
    config->match_data = hdr_entry.match_data;
    config->match_mask = hdr_entry.mask;
...
int cpm4x25_rsvd4_fast_eth_config_get(int unit, int port, pm_info_t pm_info, portmod_cpri_fast_eth_config_t* config_info)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprimod_rsvd4_fast_eth_config_get(unit, port, config_info));
...
int cpm4x25_rx_tag_config_get(int unit, int port, pm_info_t pm_info, uint32 *default_tag, uint32 *no_match_tag)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprimod_rx_tag_config_get(unit, port, default_tag, no_match_tag));
...
int cpm4x25_rsvd4_tx_control_flow_group_member_get(int unit, int port, pm_info_t pm_info,
                                                   uint32 group_id, uint32 priority,
                                                   uint32 *queue_num,
                                                   portmod_control_msg_proc_type_t *proc_type)
{
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_rsvd4_tx_control_flow_group_member_get(unit , port, group_id,
                                                      priority, queue_num,
                                                      proc_type));
...
int cpm4x25_rsvd4_tx_control_flow_group_member_get_all(int unit, int port, pm_info_t pm_info, uint32 group_id, portmod_rsvd4_control_group_entry_t* group_config)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprimod_rsvd4_tx_control_flow_group_member_get_all (unit , port, group_id, group_config));
...
int cpm4x25_rsvd4_tx_control_flow_config_set(int unit, int port, pm_info_t pm_info, uint32 flow_id, const portmod_rsvd4_tx_config_info_t* config_info)
{
    ...
    /*
     * Add extra buffer entry (16 bytes) for control flow.
     */
    decap_data_info.buffer_size             = config_info->queue_size + 16;
...
int cpm4x25_rsvd4_tx_single_tunnel_message_config_get(int unit, int port, pm_info_t pm_info, cprimod_cpri_crc_option_t *crc_option)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprimod_rsvd4_tx_single_tunnel_message_config_get(unit, port, crc_option));
                                     ...
                                     portmod_pm_vco_setting_t* vco_setting)
{
    ...
    int pll0_active_lane_bitmap=0, pll1_active_lane_bitmap=0;
    portmod_vcos_speed_config_t speed_config_list[MAX_PORTS_PER_PM4X25]={{0}};
                                             ...
                                             portmod_cpri_ecc_intr_info_t* ecc_err_info)
{
    ...
    for (lane_index = 0 ; lane_index < MAX_PORTS_PER_PM4X25; lane_index++) {
        ...
        if (cpri_port) {
            ...
            if (ecc_info->ecc_num_bits_err != CPRIMOD_PM_ECC_ERR_NONE) {
                break;
            }
        }
    }
...
int cpm4x25_cpri_port_frame_config_set(int unit, int port, pm_info_t pm_info, const portmod_cpri_port_frame_config_t* config)
{
    ...
    /* saved the frame format type. */
    _SOC_IF_ERR_EXIT
        (_cpm4x25_port_index_get(unit, port, pm_info, &lane_index, &lane_map));
    _SOC_IF_ERR_EXIT
        (PM4x25_CPRI_PORT_FRAME_FORMAT_SET(unit, pm_info, config->frame_type, lane_index));

exit:
    SOC_FUNC_RETURN;
}

int cpm4x25_cpri_port_frame_config_get (int unit, int port, pm_info_t pm_info, 
                                        portmod_cpri_port_frame_config_t* config)
{
    cprimod_cpri_decap_header_field_mapping_t decap_map;
    int lane_index;
    uint32 lane_map;
    ...
    /* retrieve the frame format type. */
    _SOC_IF_ERR_EXIT
        (_cpm4x25_port_index_get(unit, port, pm_info, &lane_index, &lane_map));
    _SOC_IF_ERR_EXIT
        (PM4x25_CPRI_PORT_FRAME_FORMAT_GET(unit, pm_info, &frame_format, lane_index));
    config->frame_type          = frame_format;
    config->decap_primary_key   = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_0;
    config->decap_secondary_key = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_1;

    if ((frame_format == PORTMOD_CPRI_ROE_FRAME_FORMAT_ECPRI12) ||
        (frame_format == PORTMOD_CPRI_ROE_FRAME_FORMAT_ECPRI8)) {

        _SOC_IF_ERR_EXIT
            (cprimod_cpri_decap_header_field_mapping_t_init(&decap_map));
        _SOC_IF_ERR_EXIT
            (cprimod_cpri_decap_header_field_mapping_get(unit, port, &decap_map));
            config->decap_primary_key   = decap_map.subtype_sel;
            config->decap_secondary_key = decap_map.flowid_sel;
    }
exit:
    SOC_FUNC_RETURN;
...
int cpm4x25_port_lane_map_get(int unit, int port, pm_info_t pm_info,
                              uint32 flags, phymod_lane_map_t* lane_map)
{
    ...
    if (val == 0) {
        /* Ethernet port. */
        _rv = pm4x25_port_lane_map_get(unit, port, pm_info, flags, lane_map);
        SOC_FUNC_RETURN;
    } else {
        _SOC_IF_ERR_EXIT
            (cprimod_cpri_port_lane_map_get(unit, port, flags, lane_map));
    }
...
int cpm4x25_port_lane_map_set(int unit, int port, pm_info_t pm_info,
                              uint32 flags, phymod_lane_map_t* lane_map)
{
    SOC_INIT_FUNC_DEFS;

    /* Ethernet port on CPRI core. */
    _SOC_IF_ERR_EXIT
        (pm4x25_port_lane_map_set(unit, port, pm_info, flags, lane_map));
