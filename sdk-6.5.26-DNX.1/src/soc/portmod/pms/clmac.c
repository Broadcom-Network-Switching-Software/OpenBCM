...
{
    uint64 rval;
    soc_field_t field, other_field;

    SOC_INIT_FUNC_DEFS;

    enable = enable ? 1 : 0;
    field = is_remote ? FORCE_REMOTE_FAULT_OSf : FORCE_LOCAL_FAULT_OSf;
    other_field = is_remote ? FORCE_LOCAL_FAULT_OSf : FORCE_REMOTE_FAULT_OSf;

    RD_CLMAC(RX_LSS_CTRLr, unit, port, &rval);

    if (enable) {
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, field, 1);
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, other_field, 0);
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, FAULT_SOURCE_FOR_TXf, 2);

    } else {
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, FORCE_REMOTE_FAULT_OSf, 0);
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, FORCE_LOCAL_FAULT_OSf, 0);
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, FAULT_SOURCE_FOR_TXf, 0);
    }

    WR_CLMAC(RX_LSS_CTRLr, unit, port, rval);
...
int clmac_link_fault_os_get(int unit, int port, int is_remote, uint32 * enable)
{
    ...
    if (is_remote) {
        *enable = soc_reg64_field32_get(unit, CLMAC_RX_LSS_CTRLr, rval,
...
int clmac_drop_tx_data_on_ordered_set_enable(int unit, int port, uint32 enable)
{
    ...
    RD_CLMAC(RX_LSS_CTRLr, unit, port, &rval);
    if (enable) {
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_LINK_INTERRUPTf, 1);
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_REMOTE_FAULTf, 1);
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_LOCAL_FAULTf, 1);
    } else {
        /* Set to the default of  HW when disable*/
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_LINK_INTERRUPTf, 0);
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_REMOTE_FAULTf, 1);
        soc_reg64_field32_set(unit, CLMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_LOCAL_FAULTf, 1);
    }
    WR_CLMAC(RX_LSS_CTRLr, unit, port, rval);
...
int clmac_tx_crc_mode_set(int unit, int port, clmac_tx_crc_mode_t crc_mode)
{
    ...
    RD_CLMAC(TX_CTRLr, unit, port, &mac_tx_ctrl);
    soc_reg64_field32_set(unit, CLMAC_TX_CTRLr, &mac_tx_ctrl, CRC_MODEf, crc_mode);
    WR_CLMAC(TX_CTRLr, unit, port, mac_tx_ctrl);
...
int clmac_tx_crc_mode_get(int unit, int port, clmac_tx_crc_mode_t* crc_mode)
{
    ...
    RD_CLMAC(TX_CTRLr, unit, port, &mac_tx_ctrl);
    *crc_mode = soc_reg64_field32_get(unit, CLMAC_TX_CTRLr, mac_tx_ctrl, CRC_MODEf);
...
int clmac_strip_crc_set(int unit, int port, uint32 enable)
{
    ...
    RD_CLMAC(RX_CTRLr, unit, port, &mac_rx_ctrl);
    soc_reg64_field32_set(unit, CLMAC_RX_CTRLr, &mac_rx_ctrl, STRIP_CRCf, enable);
    WR_CLMAC(RX_CTRLr, unit, port, mac_rx_ctrl);
...
int clmac_strip_crc_get(int unit, int port, uint32* enable)
{
    ...
    RD_CLMAC(RX_CTRLr, unit, port, &mac_rx_ctrl);
    * enable = soc_reg64_field32_get(unit, CLMAC_RX_CTRLr, mac_rx_ctrl, STRIP_CRCf);
