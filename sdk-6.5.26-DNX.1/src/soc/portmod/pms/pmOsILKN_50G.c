...
#define OS_ILKN_LANE_CONFIG_RX  (0x1)
#define OS_ILKN_LANE_CONFIG_TX  (0x2)
...
/**
 * \brief - get ILKN enabled lanes from HW
 * 
 * \param [in] unit - chip unit id
 * \param [in] port - logical port
 * \param [out] enabled_rx_serdes - uint32 to represent enabled RX lanes bitmap.
 * \param [out] enabled_tx_serdes - uint32 to represent enabled TX lanes bitmap.
 *   
 * \return
 ...
 */
STATIC
int _pmOsILKN_50G_ilkn_enabled_lanes_get(int unit, int flags, int port, uint32 *enabled_rx_serdes, uint32 *enabled_tx_serdes)
{
    uint32 reg_val;
    uint32 rx_lane_disabled, tx_lane_disabled;
    SOC_INIT_FUNC_DEFS;

    /* The register has reverse logic  - so we negate the lanes bitmap */
    if (flags & OS_ILKN_LANE_CONFIG_TX) {
        _SOC_IF_ERR_EXIT(READ_ILKN_SLE_TX_LANEr(unit, port, &reg_val)); 
        tx_lane_disabled = soc_reg_field_get(unit, ILKN_SLE_TX_LANEr, reg_val, SLE_TX_LANE_DISABLEf);
        *enabled_tx_serdes = ~tx_lane_disabled;
    }

    if (flags & OS_ILKN_LANE_CONFIG_RX) {
        _SOC_IF_ERR_EXIT(READ_ILKN_SLE_RX_LANEr(unit, port, &reg_val)); 
        rx_lane_disabled = soc_reg_field_get(unit, ILKN_SLE_RX_LANEr, reg_val, SLE_RX_LANE_DISABLEf);
        *enabled_rx_serdes = ~rx_lane_disabled;
    }
exit:
    SOC_FUNC_RETURN;
}
 ...
 */
STATIC
int _pmOsILKN_50G_ilkn_enabled_lanes_set(int unit, int flags, int port, uint32 enabled_rx_serdes, uint32 enabled_tx_serdes)
{
    uint32 reg_val;
    SOC_INIT_FUNC_DEFS;
...
int _pmOsILKN_50G_ilkn_lanes_config(int unit, int port, pm_info_t pm_info, pmOsIlkn_core_lane_map_t *lane_map)
{
    ...
    /* If there's fec denpendency, RX lanes will be enabled in pmOsILKN_50G_port_periodic_callback() */
    if (pm_info->pm_data.pmOsIlkn_db->fec_dependency)
    {
        rx_reg_val = 0;
    }
    else
    ...
    _SOC_IF_ERR_EXIT(_pmOsILKN_50G_ilkn_enabled_lanes_set(unit, OS_ILKN_LANE_CONFIG_RX|OS_ILKN_LANE_CONFIG_TX, port, rx_reg_val, tx_reg_val));
...
static int pmOsILKN_50G_port_periodic_callback(int unit, int port)
{
    pm_info_t pm_info;
    int fec_enable, fec_bypass_enable, fec_dec_en;
    pmOsIlkn_internal_t* ilkn_data;
    int core_id, nof_lanes, lane, valid, is_mac_lb;
    /* rx_lane_status = 0 to make GCC happy */
    uint32 fec_dec_align, fec_dec_config, rx_lane_config, rx_lane_status = 0;
    pmOsIlkn_core_lane_map_t lane_map;
    portmod_pbmp_t serdes_rx_bmp, fec_rx_bmp;
    SOC_INIT_FUNC_DEFS;
    ...
    fec_dec_en = (!pm_info->pm_data.pmOsIlkn_db->fec_disable_by_bypass && fec_enable) ||
                    (pm_info->pm_data.pmOsIlkn_db->fec_disable_by_bypass && !fec_bypass_enable);
    ...
    PORTMOD_PBMP_CLEAR(fec_rx_bmp);
    PORTMOD_PBMP_CLEAR(serdes_rx_bmp);

    for (lane = 0 ; lane < nof_lanes ; lane++)
    {
        PORTMOD_PBMP_PORT_ADD(fec_rx_bmp, lane_map.rx_lane_map[lane]/2);
        PORTMOD_PBMP_PORT_ADD(serdes_rx_bmp, lane_map.rx_lane_map[lane]);
    }

    fec_dec_config = SOC_PBMP_WORD_GET(fec_rx_bmp, 0);
    rx_lane_config = SOC_PBMP_WORD_GET(serdes_rx_bmp, 0);

    _SOC_IF_ERR_EXIT(READ_ILKN_SLE_FEC_DEC_ALIGN_STATUSr(unit, port, &fec_dec_align));

    _SOC_IF_ERR_EXIT(_pmOsILKN_50G_ilkn_enabled_lanes_get(unit, OS_ILKN_LANE_CONFIG_RX, port, &rx_lane_status, &rx_lane_status));
    _SOC_IF_ERR_EXIT(_pmOsILKN_50G_port_loopback_get(unit, port, &is_mac_lb));
    /*
     * Check FEC decoder alignment status and ILKN MAC loopback status
     * For ILKN MAC loopback, the FEC aligner cannot be aligned when
     * ILKN Rx lanes are disabled, so need to force to enable Rx lanes
     * for ILKN MAC loopback.
     */
    if (!fec_dec_en || ((fec_dec_align & fec_dec_config) == fec_dec_config) || is_mac_lb)
    {
        /* Enable RX lanes if aligned */
        if (rx_lane_status != rx_lane_config)
        {
            _SOC_IF_ERR_EXIT(_pmOsILKN_50G_ilkn_enabled_lanes_set(unit, OS_ILKN_LANE_CONFIG_RX, port, rx_lane_config, 0));
        }
    } else {
        if (rx_lane_status != 0)
        {
            _SOC_IF_ERR_EXIT(_pmOsILKN_50G_ilkn_enabled_lanes_set(unit, OS_ILKN_LANE_CONFIG_RX, port, 0, 0));
        }
    }
...
{        
    ...
    if (pm_info->pm_data.pmOsIlkn_db->fec_dependency)
    {
        _SOC_IF_ERR_EXIT(portmod_port_periodic_callback_register(unit, port, pmOsILKN_50G_port_periodic_callback, 1000000));
    }
...
{        
    ...
    _SOC_IF_ERR_EXIT(_pmOsILKN_50G_ilkn_enabled_lanes_set(unit, OS_ILKN_LANE_CONFIG_RX|OS_ILKN_LANE_CONFIG_TX, port, 0, 0));
...
int pmOsILKN_50G_port_logical_lane_order_set(int unit, int port, pm_info_t pm_info, const int* lane_order, int lane_order_size)
{
    ...
    if (fec_enable)
    {
        int ilkn_id_to_logic_order[OS_ILKN_TOTAL_LANES_PER_CORE];
        for (i = 0; i < nof_lanes; i++)
        {
            ilkn_id_to_logic_order[lane_order[i]] = i;
        }

        /*lane_id is representing ilkn id*/
        for (lane_id = 0 ; lane_id < nof_lanes ; lane_id++)
        {
            rx_lane_order[lane_id] = lane_map.rx_lane_map[ilkn_id_to_logic_order[lane_id]];
            if (lane_id % 2 == 0)
            {
                continue;
            }
            pre_rx_field_index = active_rx_serdes[lane_id - 1];
            pre_rx_reg_index = pre_rx_field_index / OS_ILKN_LANE_MAP_REGISTER_RESOLUTION;

            rx_field_index = active_rx_serdes[lane_id];
            rx_reg_index = rx_field_index / OS_ILKN_LANE_MAP_REGISTER_RESOLUTION;

            /*Need to adjust previous RX remap due to ILKN FEC doing the Rx swap automatically.*/
            if (OS_ILKN_PHY_ADDR_PAIRS_IS_CROSS(rx_lane_order[lane_id - 1], rx_lane_order[lane_id]))
            {
                /*configure current ilkn lane id rx map*/
                _SOC_IF_ERR_EXIT(soc_custom_reg32_get(unit, remap_lanes_rx_regs[rx_reg_index], port, 0, &reg_val));
                soc_reg_field_set(unit, remap_lanes_rx_regs[rx_reg_index], &reg_val, remap_lanes_rx_fields[rx_field_index], rx_lane_order[lane_id -1]);
                _SOC_IF_ERR_EXIT(soc_custom_reg32_set(unit, remap_lanes_rx_regs[rx_reg_index], port, 0, reg_val));

                /*configure the previous ilkn lane id rx remap */
                _SOC_IF_ERR_EXIT(soc_custom_reg32_get(unit, remap_lanes_rx_regs[pre_rx_reg_index], port, 0, &reg_val));
                soc_reg_field_set(unit, remap_lanes_rx_regs[pre_rx_reg_index], &reg_val, remap_lanes_rx_fields[pre_rx_field_index], rx_lane_order[lane_id]);
...
{
    portmod_speed_config_t speed_config_aggregated_pm;
    pmOsIlkn_internal_t* ilkn_data;
    int core_id, pm, is_pm_aggregated = 0;
    int pm_found = FALSE;
    int fec_enable = 0;
    ...
    for (pm = 0; pm < OS_ILKN_MAX_ILKN_AGGREGATED_PMS; ++pm)
    {
        ...
        if (is_pm_aggregated) {
            _SOC_IF_ERR_EXIT(__portmod__dispatch__[ilkn_data->pms[pm]->type]->f_portmod_port_speed_config_get(unit, port, ilkn_data->pms[pm], &speed_config_aggregated_pm));
            nof_lanes += speed_config_aggregated_pm.num_lane;
            if (!pm_found)
            {
                sal_memcpy(speed_config, &speed_config_aggregated_pm, sizeof(speed_config_aggregated_pm));
                pm_found = TRUE;
            }
        }
    }

    speed_config->num_lane = nof_lanes;
    _SOC_IF_ERR_EXIT(_pmOsILKN_50G_port_fec_enable_get(unit, port, pm_info, &fec_enable, &fec_bypass_enable));
...
int pmOsILKN_50G_ilkn_port_lane_enable(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
    ...
    if (!pm_info->pm_data.pmOsIlkn_db->fec_dependency)
    {
        /* No need to touch RX lane if there's no fec dependency */
        SOC_EXIT;
    }
    ...
    {
	    _SOC_IF_ERR_EXIT(portmod_port_periodic_callback_unregister(unit, port));
        _SOC_IF_ERR_EXIT(_pmOsILKN_50G_ilkn_enabled_lanes_set(unit, flags, port, 0, 0));
    }
