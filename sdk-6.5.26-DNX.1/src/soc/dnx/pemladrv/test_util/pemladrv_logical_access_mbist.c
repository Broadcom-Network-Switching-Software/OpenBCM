    ...
    LogicalDirectInfo * db_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_allocate_pemladrv_mem_struct(unit, mem_access,
                                                              ...
                                                              db_info->db_direct_info_per_core_arr
    ...
    int unit)
{
    ...
    for (db_ndx = 0; db_ndx < nof_direct_dbs; ++db_ndx)
    {
        ...
        if (!dnx_pemladrv_is_db_mapped_to_pes
            (db_info->db_direct_info_per_core_arr[core_id].result_chunk_mapper_matrix_arr))
            continue;
    ...
    int unit)
{
    ...
    for (db_ndx = 0; db_ndx < nof_dbs; ++db_ndx)
    {
        ...
        dnx_pemladrv_create_pemladrv_mem_struct_db_tcam(unit, &key_write, &mask_write, &valid_write, &result_write,
                                                        db_info);
        dnx_pemladrv_create_pemladrv_mem_struct_db_tcam(unit, &key_read, &mask_read, &valid_read, &result_read,
                                                        db_info);
        ...
        for (entry_ndx = 0; entry_ndx < nof_entries; ++entry_ndx)
        {
            ...
            for (mbist_val_ndx = 0; mbist_val_ndx < nof_mbist_values; ++mbist_val_ndx)
            {
                ...
                SHR_IF_ERR_EXIT(pemladrv_tcam_write
                                (unit, core_id, db_ndx, entry_ndx, key_write, mask_write, valid_write, result_write));

                SHR_IF_ERR_EXIT(pemladrv_tcam_read
                                (unit, core_id, db_ndx, entry_ndx, key_read, mask_read, valid_read, result_read));
    ...
    LogicalTcamInfo * db_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_allocate_pemladrv_mem_struct
                    (unit, result, nof_fields_in_result,
                     db_info->db_tcam_per_core_arr[core_id].result_field_bit_range_arr));
    ...
    for (db_ndx = 0; db_ndx < nof_dbs; ++db_ndx)
    {
        ...
        for (entry_ndx = 0; entry_ndx < nof_entries; ++entry_ndx)
        {
            ...
            for (mbist_val_ndx = 0; mbist_val_ndx < nof_mbist_values; ++mbist_val_ndx)
            {
                dnx_pemladrv_set_mbist_value_to_field_db_em_lpm(key_write, result_write, db_info,
                                                                get_mbist_val_by_ndx(mbist_val_ndx));
                SHR_IF_ERR_EXIT(dnx_pem_lpm_set_by_id
                                (unit, core_id, db_ndx, entry_ndx, key_write, &length_in_bits_write, result_write));
                SHR_IF_ERR_EXIT(dnx_pem_lpm_get_by_id
                                (unit, core_id, db_ndx, entry_ndx, key_read, &length_in_bits_read, result_read));
                if (dnx_pemladrv_compare_pemladrv_mem_structs_db_em_lpm
                    (key_write, key_read, result_write, result_read, db_info))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "");
        ...
        for (int core_id = 0; core_id < PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_id)
        {
            ...
            for (mbist_val_ndx = 0; mbist_val_ndx < nof_mbist_values; ++mbist_val_ndx)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_pemladrv_reg_read_and_retreive_read_bits_mask
                                (unit, core_id, reg_ndx, data_read, mask));
...
shr_error_e
dnx_pemladrv_pem_mem_virtual_bist(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
