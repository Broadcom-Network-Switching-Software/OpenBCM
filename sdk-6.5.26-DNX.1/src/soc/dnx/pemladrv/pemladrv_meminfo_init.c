    ...
    int use_dma)
{
    ...
    else
    {
        ...
        if (use_dma)
        {
            ...
            dnx_sbusdma_desc_addr_add(unit, block_id, pem_reg_address_val, 0, entry_size, FALSE, FALSE, 1, data_value);
#endif
        }
        else
        {
    ...
    else
    {
        for (int core_id = 0; core_id < PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_id)
        {
            ...
            if (!is_cfg_core)
            {
                ...
                api_info[unit].reg_container.reg_info_arr[reg_id].
                    reg_field_info_arr[field_id].reg_field_mapping_per_core_arr[core_id].reg_field_mapping_arr = NULL;
                continue;
            }
    ...
    const char *line)
{
    ...
    core = get_core_from_memory_address(address);
    assert(core >= 0);
