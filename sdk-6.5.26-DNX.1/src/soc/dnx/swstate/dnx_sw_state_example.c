    ...
    int unit)
{
    ...
    init_info.max_nof_elements = dnx_data_infra_tests.sw_state.htb_max_nof_elements_get(unit);
    init_info.hash_func = dnx_data_infra_tests.sw_state.htb_hashing_get(unit);
    init_info.load_factor = 0;
    init_info.key_size_in_bits = BYTES2BITS(sizeof(uint32));
    init_info.data_size_in_bits = BYTES2BITS(sizeof(uint32));
    init_info.flags = 0;
    init_info.nof_htbs = 0;
