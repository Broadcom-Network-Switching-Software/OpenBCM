    ...
    else
    {
        ...
        for(; i0 < I0; i0++) {
            ...
            for(; i1 < I1; i1++) {
                ...
                if(i2 >= dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_MDB_DB_INFOS__MDB_CLUSTER_INFOS__MACRO_CLUSTER_CACHE__DATA, ((mdb_tables_info*)sw_state_roots_array[unit][MDB_MODULE_ID])->mdb_cluster_infos.macro_cluster_cache[i0][i1].data))
                {
                    LOG_CLI((BSL_META("mdb_db_infos[]->((mdb_tables_info*)sw_state_roots_array[][MDB_MODULE_ID])->mdb_cluster_infos.macro_cluster_cache[][].data[][]: ")));
                    LOG_CLI((BSL_META("Invalid index: %d \n"),i2));
                    SHR_EXIT();
                ...
                if(dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_MDB_DB_INFOS__MDB_CLUSTER_INFOS__MACRO_CLUSTER_CACHE__DATA, ((mdb_tables_info*)sw_state_roots_array[unit][MDB_MODULE_ID])->mdb_cluster_infos.macro_cluster_cache[i0][i1].data) == 0)
                {
                    SHR_EXIT();
                }
