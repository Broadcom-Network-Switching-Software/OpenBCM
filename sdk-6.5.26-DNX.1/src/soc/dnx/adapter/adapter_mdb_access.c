    ...
    int *is_valid)
{
    ...
    for (die_idx = 0; die_idx < dnx_data_device.general.nof_core_groups_get(unit); die_idx++)
    {
        if (core_id == (dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit) +
                        die_idx * dnx_data_device.general.nof_cores_per_core_group_get(unit)))
        {
            break;
        }
    }
    if (die_idx == dnx_data_device.general.nof_core_groups_get(unit))
    {
        *is_valid = FALSE;
    }
    else
    ...
    dbal_physical_entry_t * entry)
{
    ...
    adapter_context_db.params.adapter_mem_reg_mutex.take(unit, sub_unit_id, sal_mutex_FOREVER);
    adapter_context_db.params.adapter_mem_reg_fd.get(unit, sub_unit_id, &adapter_mem_reg_fd);
    SHR_IF_ERR_EXIT(adapter_mdb_verify_valid_core(unit, entry->core_id, &valid_core));
    if (valid_core == FALSE)
    {
        SHR_EXIT();
    }
