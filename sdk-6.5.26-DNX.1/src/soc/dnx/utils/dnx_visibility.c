...
static shr_error_e
dnx_visibility_dbal_field_type_enum_max_value_get(
    int unit,
    ...
    uint32 *max_value)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get(unit, field_type, INVALID_ENUM, &i));
    while (i != INVALID_ENUM)
    {
        max_value_internal++;
        SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get(unit, field_type, i, &i));
    }
    ...
    dbal_enum_value_field_pp_asic_block_e asic_block)
{
    ...
    shr_error_e rv =
        dnx_visibility_dbal_field_type_enum_max_value_get(unit, DBAL_FIELD_TYPE_DEF_PP_ASIC_BLOCK, &block_max_index);
    if (rv != _SHR_E_NONE)
    {
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_visibility_dbal_field_type_enum_max_value_get
                    (unit, DBAL_FIELD_TYPE_DEF_PP_ASIC_BLOCK, &block_max_index));
    ...
    for (i_bl = 0; i_bl < block_max_index; i_bl++)
    {
        ...
        sal_strncpy(dnx_visibility_block_map[unit][i_bl].name,
                    dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_PP_ASIC_BLOCK,
                                                dnx_data_debug.mem.block_map_get(unit, i_bl)->block_id),
                    RHNAME_MAX_SIZE);
    ...
    for (i_bl = 0; i_bl < block_max_index; i_bl++)
    {
        SHR_ALLOC_ERR_EXIT(mem_name_per_block_array[unit][i_bl], sizeof(int) * (2), "mem_name_per_block_array",
                           "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
...
#if defined(BCM_ACCESS_SUPPORT)
        if (ACCESS_IS_INITIALIZED(unit) && dnx_data_debug.mem.feature_get(unit, dnx_data_debug_mem_new_access)
            && (mem_name_per_block_array[unit][i_bl][1] != 0))
        {
            mem_name_per_block_array[unit][i_bl][1] =   
                (unsigned) mem_name_per_block_array[unit][i_bl][1] >= ACCESS_NOF_MCM_MAPPED_MEMS ?
                ACCESS_INVALID_REGMEM_ID :
                access_mcm2access_mem_ids[(unsigned) mem_name_per_block_array[unit][i_bl][1]];
            if (mem_name_per_block_array[unit][i_bl][1] == ACCESS_INVALID_REGMEM_ID)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Memory ID (%d) is invalid\n",
                             dnx_data_debug.mem.mem_array_get(unit, i_bl)->mem2);
            }
        }
#endif
    }
    ...
    uint32 *value_array)
{
    ...
    int size;
    dbal_tables_e signal_window_table, signal_data_table;
    ...
    switch (pp_stage)
    {
        case DNX_PP_STAGE_VTT1:
            signal_window_table = DBAL_TABLE_VTT1_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_VTT1_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_VTT3:
            signal_window_table = DBAL_TABLE_VTT3_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_VTT3_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_VTT5:
            signal_window_table = DBAL_TABLE_VTT5_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_VTT5_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_IFWD1:
            signal_window_table = DBAL_TABLE_FWD1_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_FWD1_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_IFWD2:
            signal_window_table = DBAL_TABLE_FWD2_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_FWD2_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_IPMF1:
            signal_window_table = DBAL_TABLE_IPMF1_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_IPMF1_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_IPMF3:
            signal_window_table = DBAL_TABLE_IPMF3_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_IPMF3_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_ETPRP2:
            signal_window_table = DBAL_TABLE_PRP2_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_PRP2_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_TERM:
            signal_window_table = DBAL_TABLE_TERM_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_TERM_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_EFWD:
            signal_window_table = DBAL_TABLE_FWD_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_FWD_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_ENC1:
            signal_window_table = DBAL_TABLE_ENC1_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_ENC1_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_ENC2:
            signal_window_table = DBAL_TABLE_ENC2_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_ENC2_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_ENC3:
            signal_window_table = DBAL_TABLE_ENC3_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_ENC3_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_ENC4:
            signal_window_table = DBAL_TABLE_ENC4_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_ENC4_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_ENC5:
            signal_window_table = DBAL_TABLE_ENC5_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_ENC3_PEMA_DEBUG_SIGNALS_DATA;
            break;
        case DNX_PP_STAGE_TRAP:
            signal_window_table = DBAL_TABLE_TRAP_PEMA_DEBUG_SIGNALS_WINDOW_CONFIG;
            signal_data_table = DBAL_TABLE_TRAP_PEMA_DEBUG_SIGNALS_DATA;
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_EMPTY, "PP stage %d is invalid or has no PEM access enabled", pp_stage);
    }

    
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, signal_window_table, &entry_handle_id));
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, signal_data_table, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);

    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));

    SHR_IF_ERR_EXIT(dbal_tables_field_size_get(unit, signal_data_table, DBAL_FIELD_PEMA_DATA, FALSE, 0, 0, &size));
    ...
    uint32 *value_array)
{
    ...
    line_index = DNX_VISIBILITY_ENGINE_CALC_IDX(pp_stage, direction, window);

    
    if (dnx_visibility_block_info[unit][asic_block]->sig_cash_status[core][line_index] ==
        DNX_VISIBILITY_SIG_CACHE_STATUS_EMPTY)
    {
        SHR_IF_ERR_EXIT(dnx_debug_pema_fetch(unit, core, direction, pp_stage, window,
    ...
    int line_index)
{
    ...
    {
        SHR_IF_ERR_EXIT_WITH_LOG(access_regmem(unit,
                                               0,
                                               mem_name_per_block_array[unit][asic_block][mem_array_index],
                                               ACCESS_ALL_BLOCK_INSTANCES,
                                               0,
                                               line_index,
                                               temp_buffer),
                                 "Failed clearing memory %s arr_ind=%d %s",
                                 SOC_MEM_NAME(unit, mem_name_per_block_array[unit][asic_block][mem_array_index]), 0,
                                 "");
    }
    else
#endif
    {
        ...
        block =
            SOC_MEM_BLOCK_MIN(unit,
                              mem_name_per_block_array[unit][asic_block][mem_array_index]) + (core * num_of_blocks);

        SHR_IF_ERR_EXIT_WITH_LOG(soc_mem_array_read
                                 (unit, mem_name_per_block_array[unit][asic_block][mem_array_index], 0, block,
                                  line_index, temp_buffer), "Failed reading memory %s port=%d arr_ind=%d",
                                 SOC_MEM_NAME(unit, mem_name_per_block_array[unit][asic_block][mem_array_index]), block,
                                 0);
    }
    ...
    int line_index)
{
    ...
    if (ACCESS_IS_INITIALIZED(unit))
    {
        SHR_IF_ERR_EXIT_WITH_LOG(access_regmem(unit,
                                               ...
                                               line_index,
                                               temp_buffer),
                                 "Failed clearing memory %s arr_ind=%d %s", SOC_MEM_NAME(unit, memory), array_index,
                                 "");
    }
    else
#endif
    {
        ...
        SHR_IF_ERR_EXIT_WITH_LOG(soc_mem_array_write(unit,
                                                     ...
                                                     line_index,
                                                     temp_buffer),
                                 "Failed clearing memory %s arr_ind=%d block=%d", SOC_MEM_NAME(unit, memory),
                                 array_index, block);
    }
    ...
    dnx_pp_block_e pp_block)
{
...
#ifdef ADAPTER_SERVER_MODE
    {
        ...
        SHR_SET_CURRENT_ERR(sand_signal_get(unit, core, &match_m, dsig_list, NULL));
...
static shr_error_e
dnx_visibility_port_egress_verify(
    int unit,
    bcm_gport_t gport)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Port 0x%08x cannot be configured for direction Egress as it does not have PP_DSP!\n", gport);
    }
    ...
    int enable)
{
    ...
    if (direction & BCM_INSTRU_CONTROL_FLAG_DIRECTION_EGRESS)
    {
        SHR_IF_ERR_EXIT(dnx_visibility_port_egress_verify(unit, gport));
    }
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_idx)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get
                            (unit, DBAL_FIELD_TYPE_DEF_PP_ASIC_BLOCK, INVALID_ENUM, &i_bl));
            while (i_bl != INVALID_ENUM)
            {
                ...
                {
                    SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get
                                    (unit, DBAL_FIELD_TYPE_DEF_PP_ASIC_BLOCK, i_bl, &i_bl));
                    continue;
                }
                for (i_line = 0; i_line < dnx_visibility_block_info[unit][i_bl]->line_nof; i_line++)
                {
    ...
    int core)
{
    ...
    for (line_index = 0; line_index < DNX_VISIBILITY_BLOCK_MAX_NOF_LINES; line_index++)
    {
        dnx_visibility_block_info[unit][DBAL_ENUM_FVAL_PP_ASIC_BLOCK_TO_ENGINE]->sig_cash_status[core][line_index] =
            DNX_VISIBILITY_SIG_CACHE_STATUS_EMPTY;
        dnx_visibility_block_info[unit][DBAL_ENUM_FVAL_PP_ASIC_BLOCK_FROM_ENGINE]->sig_cash_status[core][line_index] =
            DNX_VISIBILITY_SIG_CACHE_STATUS_EMPTY;
    }
