    ...
    char *action)
{
    ...
    int logger_action = 0;
    int core_id = (entry_handle->table->core_mode == DBAL_CORE_MODE_SBC) ? 0 : entry_handle->core_id;
    ...
    uint32 *payload_size)
{
    ...
    DNX_KBP_HANDLES_GET(entry_handle->table, kbp_handles, entry_handle->core_id);
    ...
    uint32 *payload_size)
{
    ...
    entry_handle->phy_entry.attr_flags |= (entry_info.active == 0) ? DBAL_ENTRY_ATTR_FLAG_CACHED : 0;
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    entry_handle->core_id =
        (kbp_mngr_use_different_dbs_per_core(unit, entry_handle->table->core_mode)) ? entry_handle->core_id : 0;
        ...
        if (kpb_entry != NULL)
        {
            if (entry_info.ad_handle != NULL)
            {
                kbp_db_handles_t *db_handles =
                    &((kbp_db_handles_t *) entry_handle->table->kbp_handles)[entry_handle->core_id];
