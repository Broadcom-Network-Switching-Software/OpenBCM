    ...
    int unit)
{
    ...
    for (handle_index = 0; handle_index < DBAL_SW_NOF_ENTRY_HANDLES; handle_index++)
    {
        ...
        dbal_mngr[unit]->range_info_ptr_array[handle_index] = NULL;
    }

    dbal_mngr[unit]->nof_range_info_ptr = 0;
    ...
    int unit)
{
    ...
    for (handle_index = 0; handle_index < DBAL_SW_NOF_ENTRY_HANDLES; handle_index++)
    {
        ...
        if (dbal_mngr[unit]->range_info_ptr_array[handle_index] != NULL)
        {
            LOG_CLI((BSL_META("range_info_ptr_array at index %d was not freed\n"), handle_index));
            SHR_FREE(dbal_mngr[unit]->range_info_ptr_array[handle_index]);
        }
    }
    ...
    int unit)
{
    ...
    for (ii = 0; ii < dbal_mngr[unit]->nof_range_info_ptr; ii++)
    {
        SHR_FREE(dbal_mngr[unit]->range_info_ptr_array[ii]);
    }
    dbal_mngr[unit]->nof_range_info_ptr = 0;
...
shr_error_e
dbal_handles_range_ptr_store(
    int unit,
    uint32 *range_info_ptr)
{
    SHR_FUNC_INIT_VARS(unit);

    if (dbal_mngr[unit]->nof_range_info_ptr == DBAL_SW_NOF_ENTRY_HANDLES)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Could not find a slot to save range info\n");
    }

    dbal_mngr[unit]->range_info_ptr_array[dbal_mngr[unit]->nof_range_info_ptr] = range_info_ptr;
    ++dbal_mngr[unit]->nof_range_info_ptr;
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    if (entry_handle->phy_entry.attr_flags & DBAL_ENTRY_ATTR_FLAG_NO_PAYLOAD)
    {
    ...
    uint32 flags)
{
    ...
    if (attr_type == DBAL_ENTRY_ATTR_AGE)
    {
        ...
        entry_handle->phy_entry.attr_flags |= DBAL_ENTRY_ATTR_FLAG_AGE;
    }
