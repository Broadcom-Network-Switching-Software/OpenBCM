    ...
    char str[DBAL_MAX_STRING_LENGTH])
{
    ...
    sal_snprintf(str, DBAL_MAX_STRING_LENGTH - 1, "%s", EMPTY);

    rv = dbal_field_string_to_id_no_error(unit, field_name, &field_id);
    if (!SHR_FAILURE(rv))
    {
    ...
    {
        dbal_field_types_defs_e field_type_id;
        SHR_IF_ERR_EXIT(dbal_field_types_def_string_to_id(unit, field_name, &field_type_id));
        SHR_IF_ERR_EXIT(dbal_field_types_info_get(unit, field_type_id,
    ...
    if (field_info->nof_defines_values > 0)
    {
        for (ii = 0; ii < field_info->nof_defines_values; ii++)
        {
            if (field_info->defines_info[ii].value == hw_value)
            {
                ...
                sal_snprintf(str, DBAL_MAX_STRING_LENGTH - 1, "%s", field_info->defines_info[ii].name);
                SHR_EXIT();
            }
        }
    }
    ...
    if (field_info->nof_enum_values > 0)
    {
        for (ii = 0; ii < field_info->nof_enum_values; ii++)
        {
            if ((field_info->enum_val_info[ii].value == hw_value) && (!field_info->enum_val_info[ii].is_invalid))
            {
                ...
                sal_snprintf(str, DBAL_MAX_STRING_LENGTH - 1, "%s", field_info->enum_val_info[ii].name);
                SHR_EXIT();
            }
        }
    ...
    if (field_info->nof_child_fields > 0)
    {
        ...
        {
            sal_snprintf(str, DBAL_MAX_STRING_LENGTH - 1, "%s %x", dbal_field_to_string(unit, sub_field_id),
                         sub_field_val);
        }
    }
    ...
    int *hw_val)
{
    ...
    {
        if ((sal_strncasecmp
             (enum_val_name, field_info->enum_val_info[ii].name, sizeof(field_info->enum_val_info[ii].name)) == 0)
...
int
dnx_dbal_fields_field_type_enum_value_get(
    int unit,
    ...
    int *hw_val)
{
    ...
    SHR_IF_ERR_EXIT(dbal_field_types_def_string_to_id(unit, field_type_name, &field_type_id));
    SHR_IF_ERR_EXIT(dbal_field_types_info_get
                    (unit, field_type_id, (const dbal_field_types_basic_info_t **) &field_info));

    SHR_IF_ERR_EXIT(dnx_dbal_hw_enum_value_get(unit, field_info, enum_val_name, hw_val));
...
int
dnx_dbal_fields_enum_dbal_value_get_no_err(
    int unit,
    char *field_name,
    char *enum_val_name,
    int *dbal_val)
    ...
    int *dbal_val)
{
    ...
    rv = dnx_dbal_fields_enum_dbal_value_get_no_err(unit, field_name, enum_val_name, dbal_val);
    ...
    int *sub_field_val)
{
    ...
    if (field_info->nof_child_fields > 0)
    {
        ...
        if (sub_field_id != DBAL_FIELD_EMPTY)
        {
            sal_snprintf(sub_field_name, DBAL_MAX_STRING_LENGTH - 1, "%s", dbal_field_to_string(unit, sub_field_id));
            SHR_EXIT();
        }
    }

    SHR_ERR_EXIT(_SHR_E_PARAM, "illegal input no subfields found or/ non of them match to he value\n");
    ...
    char *result_field_name)
{
    ...
    sal_strncpy_s(result_field_name, field_name, sal_strnlen(field_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH));
exit:
    SHR_FUNC_EXIT;
}
    ...
    char *symbol_name)
{
    ...
    if (field_id == DBAL_FIELD_RESULT_TYPE)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle));
        sal_snprintf(symbol_name, DBAL_MAX_STRING_LENGTH - 1, "%s",
                     dbal_result_type_to_string(unit, entry_handle->table_id, field_val[0]));
    }
    else
    {
        sal_snprintf(symbol_name, DBAL_MAX_STRING_LENGTH - 1, "%s",
                     dbal_enum_id_to_string(unit, field_id, field_val[0]));
    }
    ...
    signal_output_t * sig_format)
{
    ...
    if ((index > 1) && (field_info->nof_struct_fields > 0))
    {
        if ((index & 1) == 0)
        {
        ...
        if (!sal_strncmp(decoded_values_list[index + 1], "True", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            sig_format->value[0] = 1;
        }
        else if (!sal_strncmp(decoded_values_list[index + 1], "False", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
    ...
    rhlist_t * decoded_values)
{
    ...
    if (field_info->nof_child_fields > 0)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_types_info_get(unit, sub_field_id, &subfield_info));
    }

    if (field_info->nof_struct_fields > 0 || (subfield_info != 0 && subfield_info->nof_struct_fields > 0))
    {
        ...
        for (index = 0; index < nof_values - 1; index++)
        {
            ...
            else if (field_info->nof_child_fields > 0)
            {
                SHR_IF_ERR_EXIT(dbal_parent_field_string_convert_to_signal_string
                                (unit, decoded_val, index, decoded_values_list,
                                 sub_field_id, field_info, data_size_bits, sig_format));
                if ((index > 1) && (subfield_info->nof_struct_fields > 0))
                {
                    SHR_IF_ERR_EXIT(dbal_field_string_to_id(unit, decoded_values_list[index], &struct_member_field_id));
                    ...
                    if (subfield_info->nof_enum_values > 0)
                    {
                        ...
                        index++;
                             ...
                             (decoded_values_list[index], "True", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
                             || !sal_strncmp(decoded_values_list[index], "False",
            ...
            else
            {
                ...
                if (!sal_strncmp(decoded_values_list[index + 1], "True", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                {
                    sig_format->value[0] = 1;
                         ...
                         (decoded_values_list[index + 1], "False", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                {
                    sig_format->value[0] = 0;
                }
                else
                ...
                if (field_info->nof_struct_fields > 0)
                {
                    if (struct_member_field_info->nof_enum_values > 0)
                    {
    ...
    rhlist_t * decoded_values)
{
    ...
    SHR_IF_ERR_EXIT(dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle, FALSE));
    SHR_ALLOC_SET_ZERO_ERR_EXIT(entry_info,
                                sizeof(dbal_printable_entry_t), "dbal_printable_entry", "%s%s%s", "entry_info", EMPTY,
                                EMPTY);
    ...
    SHR_IF_ERR_EXIT_NO_MSG(dbal_table_printable_entry_get
                           (unit, table_id, NULL, DBAL_RESULT_TYPE_NOT_INITIALIZED, core, payload_size,
                            get_entry_handle->phy_entry.payload, entry_info));
    ...
    char to_n[RHNAME_MAX_SIZE])
{
    ...
    if (is_dbal_field)
    {
        ...
        if ((field_info->nof_defines_values == 0) && (field_info->nof_enum_values == 0)
            && (field_info->nof_child_fields == 0) && (field_info->nof_struct_fields == 0))
        {
        ...
        if (sal_strncmp
            (decoded_data_string, "No child field found!",
             sal_strnlen("No child field found!", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0)
        {
    ...
    if (is_dbal_field)
    {
        SHR_IF_ERR_EXIT(dbal_string_convert_to_signal_string
                        (unit, field_id, data, decoded_data_string, field_info, data_size_bits, decoded_values));
    }
    else
    {
