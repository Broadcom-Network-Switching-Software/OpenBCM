    ...
    uint32 *attr_val)
{
    ...
    {
        if (!(entry_handle->phy_entry.attr_flags & DBAL_ENTRY_ATTR_FLAG_AGE))
        {
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_INVALID)
    {
        if (DBAL_TABLE_IS_TCAM(entry_handle->table))
        {
            (*attr_val) = (entry_handle->phy_entry.attr_flags & DBAL_ENTRY_ATTR_FLAG_INVALID) ? TRUE : FALSE;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Attribute Invalid is supported for TCAM tables only\n");
    ...
    {
        (*attr_val) = FALSE;
        if (entry_handle->phy_entry.attr_flags & DBAL_ENTRY_ATTR_CACHED)
        {
            (*attr_val) = TRUE;
        }
    }
