...
#define DNX_DBAL_ROLLBACK_JOURNAL_TRANSACTION dnx_er_db->dnx_dbal_journal_manager->journals[DNX_DBAL_ROLLBACK_JOURNAL_NUM].transaction
#define DNX_DBAL_COMPARISON_JOURNAL_TRANSACTION dnx_er_db->dnx_dbal_journal_manager->journals[DNX_DBAL_COMPARISON_JOURNAL_NUM].transaction

#define DNX_DBAL_JOURNAL_ACCESS_NOTIFY(_unit, _dnx_er_db, _table_ptr)                                                                  \
    do {                                                                                                                               \
        if(NULL != _dnx_er_db->dnx_dbal_journal_manager->journals[DNX_DBAL_COMPARISON_JOURNAL_NUM].access_cb) {                        \
            _dnx_er_db->dnx_dbal_journal_manager->journals[DNX_DBAL_COMPARISON_JOURNAL_NUM].access_cb(_unit, _dnx_er_db, _table_ptr);  \
        }                                                                                                                              \
        if(NULL != _dnx_er_db->dnx_dbal_journal_manager->journals[DNX_DBAL_ROLLBACK_JOURNAL_NUM].access_cb) {                          \
            _dnx_er_db->dnx_dbal_journal_manager->journals[DNX_DBAL_ROLLBACK_JOURNAL_NUM].access_cb(_unit, _dnx_er_db, _table_ptr);    \
        }                                                                                                                              \
    } while(0)
    ...
    uint8 *payload)
{
    ...
    bsl_severity_t dbaldnx_severity = bslSeverityNormal;
    ...
    SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SOCDNX_DBALDNX, dbaldnx_severity);
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META("********************************* DBAL Journal *********************************\n")));

    switch (ptr_entry->action)
    {
        case DBAL_ACTION_ENTRY_CLEAR:
            LOG_INFO(BSL_LOG_MODULE, (BSL_META("Type: CLEAR\n")));
            break;
        case DBAL_ACTION_ENTRY_COMMIT:
            LOG_INFO(BSL_LOG_MODULE, (BSL_META("Type: COMMIT\n")));
            break;
        default:
    ...
    LOG_INFO(BSL_LOG_MODULE, (BSL_META("Table name: %s\n"), ptr_payload->table->table_name));

    LOG_INFO(BSL_LOG_MODULE, (BSL_META("Data associated with DBAL entry:\n")));

    SHR_IF_ERR_EXIT(dbal_entry_print(unit, ptr_payload, (dbaldnx_severity >= bslSeverityError) ? TRUE : FALSE));

    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META("********************************* DBAL Journal End *****************************\n\n")));
    ...
    char *stamp)
{
    ...
    if (bsl_fast_check_two(BSL_SOURCE_DBAL_DNX_CHK, BSL_SOURCE_DBALACCESS_DNX_CHK))
    {
        SHR_IF_ERR_EXIT(dbal_logger_table_internal_lock(unit, ptr_payload->table_id));
    }

    SHR_IF_ERR_EXIT(dbal_action_prints(unit, ptr_payload, 0, ptr_entry->action));
    ...
    switch (ptr_entry->action)
    {
        case DBAL_ACTION_ENTRY_COMMIT:
            ...
            break;
        case DBAL_ACTION_ENTRY_CLEAR:
            SHR_IF_ERR_EXIT(dbal_actions_access_entry_clear(unit, ptr_payload, NULL));
            break;
        default:
            SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "dbal journal ERROR: unrecognized entry type.\n%s%s%s", EMPTY,
                                     EMPTY, EMPTY);
    ...
    SHR_IF_ERR_EXIT(dbal_logger_table_internal_unlock(unit, ptr_payload->table_id));
...
static shr_error_e
dnx_dbal_journal_compare_clear(
    int unit,
    uint8 entry_not_found,
    dbal_entry_handle_t * ptr_payload,
    dbal_entry_handle_t * get_entry_handle,
    bsl_severity_t dbaldnx_severity)
{
    dbal_logical_table_t *table = NULL;
        ...
        && !(table->table_type == DBAL_TABLE_TYPE_TCAM && table->access_method == DBAL_ACCESS_METHOD_MDB))
    {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META("\n################################################################################\n")));
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META("DBAL entry found when it should not exist for table stamp: %s, core id: %d\n"),
                   table->table_name, (get_entry_handle->core_id)));
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META("Data associated with DBAL entry:\n")));
        SHR_IF_ERR_EXIT(dbal_entry_print
                        (unit, get_entry_handle, (dbaldnx_severity >= bslSeverityError) ? TRUE : FALSE));
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META("################################################################################\n")));
...
static shr_error_e
dnx_dbal_journal_compare_commit(
    int unit,
    uint8 entry_not_found,
    dbal_entry_handle_t * ptr_payload,
    dbal_entry_handle_t * get_entry_handle,
    uint8 is_journaled_entry_default,
    uint8 is_compare_payload,
    bsl_severity_t dbaldnx_severity)
{
    ...
    if (entry_not_found && !is_journaled_entry_default)
    {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META("\n################################################################################\n")));
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META("DBAL entry not found when it should exist for table stamp: %s, core id: %d\n"),
    ...
    if (DBAL_TABLE_IS_TCAM(table))
    {
        ...
        if (entries_match)
        {
            entries_match =
                (sal_memcmp
                 (ptr_payload->phy_entry.key, get_entry_handle->phy_entry.key, DBAL_PHYSICAL_KEY_SIZE_IN_BYTES) == 0);
        }
    ...
    if (!entries_match)
    {
        ...
        if (!
            ((ptr_payload->table_id == DBAL_TABLE_MDB_7)
             && ((ptr_payload->phy_entry.key[0] == 10) || (ptr_payload->phy_entry.key[0] == 11))))
        {
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META
                       ("\n################################################################################\n")));
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META
                       ...
                       table->table_name, (get_entry_handle->core_id)));
            LOG_ERROR(BSL_LOG_MODULE, (BSL_META("Entry data before comparison journal start:\n")));
            SHR_IF_ERR_EXIT(dbal_entry_print(unit, ptr_payload, (dbaldnx_severity >= bslSeverityError) ? TRUE : FALSE));
            LOG_ERROR(BSL_LOG_MODULE, (BSL_META("Entry data after comparison journal end:\n")));
            SHR_IF_ERR_EXIT(dbal_entry_print
                            (unit, get_entry_handle, (dbaldnx_severity >= bslSeverityError) ? TRUE : FALSE));
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META("################################################################################\n")));
...
static shr_error_e
dnx_dbal_journal_compare_common(
    int unit,
    dnx_er_db_t * dnx_er_db,
    uint8 *metadata,
    uint8 *payload,
    char *stamp)
{
    ...
    rv = dbal_actions_access_entry_get(unit, ptr_payload, get_entry_handle, FALSE);
    ...
    {
        case DBAL_ACTION_ENTRY_COMMIT:
            SHR_IF_ERR_EXIT(dnx_dbal_journal_compare_commit
                            (unit, entry_not_found, ptr_payload, get_entry_handle, is_journaled_entry_default,
                             is_compare_payload, dbaldnx_severity));
            break;
        case DBAL_ACTION_ENTRY_CLEAR:
            SHR_IF_ERR_EXIT(dnx_dbal_journal_compare_clear
                            (unit, entry_not_found, ptr_payload, get_entry_handle, dbaldnx_severity));
            break;
        default:
...
static shr_error_e
dnx_dbal_journal_compare_dbal_table_create(
    int unit,
    dnx_er_db_t * dnx_er_db,
    uint8 *metadata,
    uint8 *payload,
...
static shr_error_e
dnx_dbal_journal_compare_dbal_table_destroy(
    int unit,
    dnx_er_db_t * dnx_er_db,
    uint8 *metadata,
    uint8 *payload,
    ...
    dnx_rollback_journal_entry_t * second_entry)
{
        ...
        if (first_dbal_metadata->action == DBAL_JOURNAL_ACTION_TABLE_CREATE)
        {
            ...
            if (table_exists)
            {
                LOG_ERROR(BSL_LOG_MODULE, (BSL_META
                                           ("\n################################################################################\n")));
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META("DBAL table ID %d creation entry found without a table destruction entry.\n"),
        ...
        if (first_dbal_metadata->action == DBAL_JOURNAL_ACTION_TABLE_DESTROY)
        {
            ...
            if (!table_exists)
            {
                LOG_ERROR(BSL_LOG_MODULE, (BSL_META
                                           ("\n################################################################################\n")));
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META("DBAL table ID %d destruction entry found without a table creation entry.\n"),
                           first_dbal_payload->table_id));
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META
                           ("################################################################################\n")));
    ...
    if (first_dbal_payload->table_id == second_dbal_payload->table_id)
    {
        ...
        if (((first_dbal_metadata->action == DBAL_JOURNAL_ACTION_TABLE_CREATE)
             && (second_dbal_metadata->action == DBAL_JOURNAL_ACTION_TABLE_CREATE))
            || ((first_dbal_metadata->action == DBAL_JOURNAL_ACTION_TABLE_DESTROY)
            ...
            && (second_dbal_metadata->action == DBAL_JOURNAL_ACTION_TABLE_DESTROY))
        {
            ...
            if (!(second_entry->state == DNX_ROLLBACK_JOURNAL_ENTRY_STATE_CONDITIONALLY_INVALID))
            {
                first_entry->state = DNX_ROLLBACK_JOURNAL_ENTRY_STATE_INVALID;

                second_entry->state = DNX_ROLLBACK_JOURNAL_ENTRY_STATE_CONDITIONALLY_INVALID;
            }

            SHR_EXIT();
        }
        else if (second_dbal_metadata->action == DBAL_JOURNAL_ACTION_TABLE_DESTROY)
        {

            first_entry->state = DNX_ROLLBACK_JOURNAL_ENTRY_STATE_INVALID;
            SHR_EXIT();
        }
        if (first_dbal_payload->table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT
            || first_dbal_payload->table->table_type == DBAL_TABLE_TYPE_TCAM_BY_ID)
        {
            entries_match = (first_dbal_payload->phy_entry.entry_hw_id
                             == second_dbal_payload->phy_entry.entry_hw_id) ? 1 : 0;
        }
        else
        {
            entries_match =
...
dnx_dbal_journal_range_info_copy_if_exists(
    int unit,
    dnx_er_db_t * dnx_er_db,
    dbal_entry_handle_t * ptr_handle,
    uint32 *range_info_ptr_orig,
    dnx_er_db_journal_handle_e handle)
{
    SHR_FUNC_INIT_VARS(unit);

    if (((ptr_handle->table->access_method == DBAL_ACCESS_METHOD_KBP)) && (ptr_handle->nof_ranged_fields > 0))
    {
        ptr_handle->key_field_ranges = NULL;

        SHR_IF_ERR_EXIT(dnx_rollback_journal_custom_alloc
                        (unit, dnx_er_db, handle, ptr_handle->nof_ranged_fields * sizeof(uint32),
                         (void *) (&(ptr_handle->key_field_ranges))));

        sal_memcpy(ptr_handle->key_field_ranges, range_info_ptr_orig, ptr_handle->nof_ranged_fields * sizeof(uint32));
    }
...
dnx_dbal_journal_entry_insert(
    ...
    dnx_rollback_journal_rollback_handler comparison_handler,
    uint8 is_er,
    uint8 is_cmp,
    uint8 is_dbal_error)
{
    ...
    uint32 *range_info_ptr_orig = ptr_handle->key_field_ranges;
    ...
    if (is_er && !dnx_dbal_journal_is_table_excluded(unit, dnx_er_db, ptr_handle->table_id, FALSE)
        && (!is_dbal_error || (DNX_ROLLBACK_JOURNAL_ENTRY_STATE_ALWAYS_VALID == entry_state)))
    {

        SHR_IF_ERR_EXIT(dnx_dbal_journal_range_info_copy_if_exists
                        (unit, dnx_er_db, ptr_handle, range_info_ptr_orig, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK));

        SHR_IF_ERR_EXIT(dnx_rollback_journal_entry_new(unit,
                                                       dnx_er_db,
                                                       DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK,
                                                       (uint8 *) (&journal_entry), sizeof(dnx_dbal_journal_entry_t),
                                                       (uint8 *) ptr_handle,
    ...
    if (is_cmp && !dnx_dbal_journal_is_table_excluded(unit, dnx_er_db, ptr_handle->table_id, TRUE))
    {

        SHR_IF_ERR_EXIT(dnx_dbal_journal_range_info_copy_if_exists
                        (unit, dnx_er_db, ptr_handle, range_info_ptr_orig, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON));
        ...
        SHR_IF_ERR_EXIT(dnx_rollback_journal_entry_new(unit,
                                                       ...
                                                       comparison_handler, NULL, dnx_dbal_journal_entry_print_cb));
    }

    ptr_handle->key_field_ranges = range_info_ptr_orig;
    ...
    uint8 is_comparison)
{
    ...
    dnx_er_db->dnx_dbal_journal_manager->unsupported_table_bypass_counter = 0;
    ...
    if (is_comparison)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_rollback_journal_new(unit,
                                                 ...
                                                 rollback_journal_cbs, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON));
        dnx_er_db->dnx_dbal_journal_manager->journals[DNX_DBAL_COMPARISON_JOURNAL_NUM].access_cb = access_cb;
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_new(unit,
                                                 ...
                                                 DNX_ROLLBACK_JOURNAL_SUBTYPE_DBAL,
                                                 rollback_journal_cbs, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK));
        dnx_er_db->dnx_dbal_journal_manager->journals[DNX_DBAL_ROLLBACK_JOURNAL_NUM].access_cb = access_cb;
    }
...
static uint8
dnx_dbal_journal_is_journal_on(
    int unit,
    dnx_er_db_t * dnx_er_db,
    dnx_err_recovery_journal_e journals)
{
    switch (journals)
    {
        case DNX_DBAL_JOURNAL_ALL:
            return (dnx_rollback_journal_is_on(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK)
                    || dnx_rollback_journal_is_on(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON));
        case DNX_DBAL_JOURNAL_ROLLBACK:
            return dnx_rollback_journal_is_on(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK);
        case DNX_DBAL_JOURNAL_COMPARISON:
            return dnx_rollback_journal_is_on(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON);
        default:
            return FALSE;
    }
}
    ...
    dnx_err_recovery_journal_e journals)
{
    ...
    return dnx_dbal_journal_is_journal_on(unit, dnx_er_db, journals);
}

static inline shr_error_e
dnx_dbal_journal_get_if_on(
    int unit,
    dnx_er_db_t ** dnx_er_db,
    dnx_err_recovery_journal_e journals)
{
    ...
    if (!dnx_dbal_journal_is_journal_on(unit, *dnx_er_db, journals))
    {
        SHR_IF_ERR_EXIT_NO_MSG(_SHR_E_INTERNAL);
    }
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_destroy(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_destroy(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK));
    }
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_clear(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_clear(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK));
    }
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        ...
        distinct_rv = dnx_rollback_journal_distinct
            (unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON,
             &dnx_dbal_comparison_journal_entries_make_distinct);

        rollback_rv = dnx_rollback_journal_rollback
            (unit, dnx_er_db, only_head_rollback, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON);
    ...
    else
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_rollback
                        (unit, dnx_er_db, only_head_rollback, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK));
    }
    ...
    uint8 is_dbal_error)
{
    ...
    if (dnx_dbal_journal_is_unsupported_table(unit, ptr_handle->table_id))
    {
        ...
        if (dnx_er_db->dnx_dbal_journal_manager->unsupported_table_bypass_counter > 0)
        {
            SHR_EXIT();
        }
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_journal_entry_insert
                    (unit, dnx_er_db, ptr_handle, action, &dnx_dbal_journal_rollback_common,
                     &dnx_dbal_journal_compare_common, is_er, is_cmp, is_dbal_error));
    ...
    uint32 *seq_id)
{
    ...
    if (is_comparison)
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_get_head_seq_id
                        (unit, dnx_er_db, seq_id, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_get_head_seq_id
                        (unit, dnx_er_db, seq_id, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK));
    }
    ...
    uint8 is_disable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rollback_journal_disabled_counter_change
                    (unit, dnx_er_db, is_disable, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK, TRUE));
    SHR_IF_ERR_EXIT(dnx_rollback_journal_disabled_counter_change
                    (unit, dnx_er_db, is_disable, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON, TRUE));
    ...
    uint8 is_disable)
{
    ...
    if (dnx_er_db_get_if_initialized(unit, &dnx_er_db) != _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "dbal journal ERROR: cannot suppress comparison journal in transactions where error recovery is NOT_NEEDED/NO_SUPPORT.\n");
    }
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        dnx_rollback_journal_disabled_counter_get(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON, &counter);
    }
    else
    {
        dnx_rollback_journal_disabled_counter_get(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK, &counter);
    }
    ...
    uint8 is_dbal_error)
{
    ...
    if (dnx_dbal_journal_get_if_on(unit, &dnx_er_db, DNX_DBAL_JOURNAL_ALL) != _SHR_E_NONE)
    {
        SHR_EXIT();
    }

    DNX_DBAL_JOURNAL_ACCESS_NOTIFY(unit, dnx_er_db, entry_handle->table);
    ...
    if (0 != entry_handle->er_flags)
    {
        if (get_handle == NULL)
        {
            ...
            if (!is_dbal_error)
            {
                SHR_IF_ERR_EXIT(dnx_dbal_journal_entry_access_id_update
                                (unit, entry_handle, entry_handle->phy_entry.entry_hw_id));
            }

            SHR_IF_ERR_EXIT(dnx_dbal_journal_log_common
                            (unit, dnx_er_db, entry_handle, DBAL_ACTION_ENTRY_CLEAR, entry_handle->er_flags,
                             is_dbal_error));
        }
        else
        {

            if (entry_handle->table->access_method == DBAL_ACCESS_METHOD_MDB && get_handle != entry_handle)
            {
                sal_memcpy(get_handle->phy_entry.p_mask, entry_handle->phy_entry.p_mask,
            ...
            {
                SHR_IF_ERR_EXIT(dnx_dbal_journal_entry_access_id_update
                                (unit, get_handle, entry_handle->phy_entry.entry_hw_id));
            }

            SHR_IF_ERR_EXIT(dnx_dbal_journal_log_common
                            (unit, dnx_er_db, get_handle, DBAL_ACTION_ENTRY_COMMIT, entry_handle->er_flags,
                             is_dbal_error));

            LOG_DEBUG(BSL_LOG_MODULE,
                      (BSL_META("entry add to journal table %s, key %d, restype %d payload %x %x %x%x\n"),
                       get_handle->table->table_name, get_handle->phy_entry.key[0], get_handle->cur_res_type,
                       get_handle->phy_entry.payload[0], get_handle->phy_entry.payload[1],
                       get_handle->phy_entry.payload[2], get_handle->phy_entry.payload[3]));
        }
    }
    ...
    uint8 is_dbal_error)
{
    ...
    SHR_NULL_CHECK(entry_handle, _SHR_E_PARAM, "Handle is null");
    ...
    {
        SHR_IF_ERR_EXIT(dnx_dbal_journal_log_common
                        (unit, dnx_er_db, entry_handle, DBAL_ACTION_ENTRY_COMMIT, entry_handle->er_flags,
                         is_dbal_error));
    }
    ...
    uint8 is_bypassed)
{
    ...
    if (is_bypassed)
    {
        dnx_er_db->dnx_dbal_journal_manager->unsupported_table_bypass_counter++;
    }
    else
    {
        if (0 == dnx_er_db->dnx_dbal_journal_manager->unsupported_table_bypass_counter)
        {
            SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL,
...
dnx_dbal_journal_are_unsupported_tables_bypassed(
    ...
    dnx_er_db_t * dnx_er_db)
{
    return (dnx_er_db->dnx_dbal_journal_manager->unsupported_table_bypass_counter > 0) ? TRUE : FALSE;
}
    ...
    uint8 is_comparison)
{
    ...
    if (!dnx_rollback_journal_is_on(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON))
    {
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL,
    ...
    uint8 is_comparison)
{
    ...
    if (!is_comparison)
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_print(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_print(unit, dnx_er_db, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON));
    }
    ...
    uint8 is_comparison)
{
    ...
    if (!is_comparison)
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_logger_state_change_by_type(unit, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_logger_state_change_by_type
                        (unit, DNX_ER_DB_JOURNAL_HANDLE_DBAL_COMPARISON));
    }
    ...
    uint8 *is_logging)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rollback_journal_is_logging(unit, DNX_ER_DB_JOURNAL_HANDLE_DBAL_ROLLBACK, is_logging));
    ...
    dbal_tables_e table_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_journal_entry_insert
                    (unit, dnx_er_db, &entry, DBAL_JOURNAL_ACTION_TABLE_CREATE,
                     &dnx_dbal_journal_rollback_dbal_table_create, &dnx_dbal_journal_compare_dbal_table_create, TRUE,
                     add_to_cmp, FALSE));
    ...
    dbal_tables_e table_id)
{
    ...
    add_to_cmp = dnx_dbal_journal_is_on(unit, DNX_DBAL_JOURNAL_COMPARISON);
    SHR_IF_ERR_EXIT(dnx_dbal_journal_entry_insert
                    (unit, dnx_er_db, &entry, DBAL_JOURNAL_ACTION_TABLE_DESTROY,
                     &dnx_dbal_journal_rollback_dbal_table_destroy, &dnx_dbal_journal_compare_dbal_table_destroy, TRUE,
                     add_to_cmp, FALSE));
