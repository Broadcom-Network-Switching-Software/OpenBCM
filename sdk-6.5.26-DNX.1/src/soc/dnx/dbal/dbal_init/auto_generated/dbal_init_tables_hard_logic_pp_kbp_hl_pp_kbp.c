...
dbal_logical_table_t * table_info)
{
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_table_general_info_and_image_handle(unit, table_entry, DBAL_TABLE_KBP_OPCODE_MAPPING, is_valid, maturity_level, "KBP_OPCODE_MAPPING", DBAL_ACCESS_METHOD_HARD_LOGIC, custom_image, is_compatible_with_all_images, &is_skip_table_init));
    ...
    table_entry->table_labels[1] = DBAL_LABEL_KBP_MNGM;
    {
        ...
        {
            dbal_db_field = &table_entry->keys_info[field_index];
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_mandatory_values(unit, DBAL_FIELD_ACL_CONTEXT, DBAL_FIELD_TYPE_DEF_ACL_CONTEXT, dbal_db_field));
            dbal_db_init_table_field_params_init(&db_field);
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_optional_or_default_values(unit, table_entry, dbal_db_field, &db_field, TRUE));
            field_index += dbal_db_field->nof_instances;
        }
    ...
    }
    {
        ...
        DBAL_DB_INIT_TABLE_INTERFACE_RESULT_FIELDS_ALLOC(table_entry, table_entry->multi_res_info[result_type_counter]);
        {
            ...
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_mandatory_values(unit, DBAL_FIELD_KBP_OPCODE_ID, DBAL_FIELD_TYPE_DEF_KBP_OPCODE_ID, dbal_db_field));
            dbal_db_init_table_field_params_init(&db_field);
            ...
            {
                {
                    ...
                    for (field_instance = 0; field_instance < 1; field_instance++)
                    {
                        ...
                        access_counter++;
                        access_params->access_field_id = DBAL_FIELD_KBP_OPCODE_ID;
                        access_params->access_type = DBAL_HL_ACCESS_MEMORY;
                        access_params->inst_idx = field_instance;
                        {
                            SHR_IF_ERR_EXIT(dbal_db_init_table_set_access_register_memory(unit, access_params, IPPB_ELK_OPCODE_MAPPING_MEMm, OPCODEf));
                        }
                    }
                }
            }
...
dbal_logical_table_t * table_info)
{
    ...
    dbal_init_table_db_struct_clear(cur_table_param);
    maturity_level = DBAL_MATURITY_HIGH;
    SHR_IF_ERR_EXIT(dbal_db_init_table_general_info_and_image_handle(unit, table_entry, DBAL_TABLE_KBP_FORWARD_RESULT_SIZE, is_valid, maturity_level, "KBP_FORWARD_RESULT_SIZE", DBAL_ACCESS_METHOD_HARD_LOGIC, custom_image, is_compatible_with_all_images, &is_skip_table_init));
    if (is_skip_table_init)
    {
        SHR_EXIT();
    }
    ...
    }
    {
        ...
        {
            dbal_db_field = &table_entry->multi_res_info[result_type_counter].results_info[field_index];
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_mandatory_values(unit, DBAL_FIELD_KBP_FWD_RESULT_SIZE, DBAL_FIELD_TYPE_DEF_UINT, dbal_db_field));
            dbal_db_init_table_field_params_init(&db_field);
            db_field.size = 7;
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_optional_or_default_values(unit, table_entry, dbal_db_field, &db_field, FALSE));
            field_index += dbal_db_field->nof_instances;
    ...
    dbal_db_init_table_add_result_type_physical_values_by_result_type_index(table_entry);
    {
        ...
        sal_strncpy(cur_table_param->hl_access[map_idx].mapping_result_name, EMPTY, sizeof(cur_table_param->hl_access[map_idx].mapping_result_name));
        {
            table_db_access_params_struct_t * access_params;
            {
                ...
                dbal_db_init_table_db_struct_hl_access_params_clear(access_params);
                access_counter++;
                access_params->access_field_id = DBAL_FIELD_KBP_FWD_RESULT_SIZE;
                access_params->access_type = DBAL_HL_ACCESS_REGISTER;
                {
                    SHR_IF_ERR_EXIT(dbal_db_init_table_set_access_register_memory(unit, access_params, IPPB_ELK_FWD_RESULT_SIZE_MAPr, DBAL_HL_ACCESS_HW_FIELD_INVALID));
