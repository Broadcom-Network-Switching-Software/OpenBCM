...
dbal_logical_table_t * table_info)
{
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_table_general_info_and_image_handle(unit, table_entry, DBAL_TABLE_VLAN_PORT_GENERAL_INFO_SW, is_valid, maturity_level, "VLAN_PORT_GENERAL_INFO_SW", DBAL_ACCESS_METHOD_SW_STATE, custom_image, is_compatible_with_all_images, &is_skip_table_init));
...
dbal_logical_table_t * table_info)
{
    ...
    dbal_init_table_db_struct_clear(cur_table_param);
    maturity_level = DBAL_MATURITY_HIGH;
    SHR_IF_ERR_EXIT(dbal_db_init_table_general_info_and_image_handle(unit, table_entry, DBAL_TABLE_VLAN_PORT_MATCH_KEY_SW, is_valid, maturity_level, "VLAN_PORT_MATCH_KEY_SW", DBAL_ACCESS_METHOD_SW_STATE, custom_image, is_compatible_with_all_images, &is_skip_table_init));
    if (is_skip_table_init)
    {
        SHR_EXIT();
    }
    ...
    table_entry->table_labels[0] = DBAL_LABEL_L2;
    {
        ...
        {
            dbal_db_field = &table_entry->keys_info[field_index];
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_mandatory_values(unit, DBAL_FIELD_PON_CHANNEL_ID, DBAL_FIELD_TYPE_DEF_UINT, dbal_db_field));
            dbal_db_init_table_field_params_init(&db_field);
            db_field.size = 16;
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_optional_or_default_values(unit, table_entry, dbal_db_field, &db_field, TRUE));
            field_index += dbal_db_field->nof_instances;
    ...
    table_entry->core_mode = DBAL_CORE_MODE_SBC;
    dbal_db_init_table_add_result_type_physical_values_by_result_type_index(table_entry);
    dbal_db_init_sw_state_set_indirect_table_size(unit, table_entry, vlan_port_match_key_sw_indirect_table_size_0_cb, DBAL_DB_INVALID);
