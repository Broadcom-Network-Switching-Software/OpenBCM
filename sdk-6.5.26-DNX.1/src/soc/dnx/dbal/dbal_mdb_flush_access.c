    ...
    int *flush_in_work)
{
    ...
    uint32 lem_em_status_reg;
    uint32 lem_emp_scan_status_field;
    uint32 flush_status;
...

    {
        ...
        flush_status =
            soc_reg_above_64_field32_get(unit, lem_em_status_reg, reg_above_64_val, lem_emp_scan_status_field);
    }
    ...
    dnx_l2_action_type_t l2_action_type)
{
    ...
    uint32 flush_data_mem, flush_tcam_mem;
    ...
    dbal_flush_shadow_info_t * rules_info)
{
    ...
    uint32 valid_field;
    uint32 entry_mask_field;
    uint32 entry_filter_field;
    uint32 src_mask_field;
    uint32 src_filter_field;
    uint32 appdb_id_mask_field;
    uint32 appdb_id_filter_field;
    uint32 accessed_mask_field;
    uint32 accessed_filter_field;
    uint32 command_field;
    uint32 payload_mask_field;
    ...
    for (rule_index = 0; rule_index < nof_rules; rule_index++)
    {
...

        {
            ...
            SHR_IF_ERR_EXIT(soc_mem_write
                            (unit, flush_data_mem, MEM_BLOCK_ALL, next_available_rule, (void *) rule_data));
        }
    ...
    dnx_l2_action_type_t l2_action_type)
{
    ...
    uint32 lem_emp_age_reg, lem_emp_age_scan_pulse_reg, lem_em_status_reg = MDB_ACCESS_INVALID_ID;
    uint32 disable_age_field, lem_emp_scan_status_field = INVALIDf;
    dbal_physical_tables_e physical_table_id;
    SHR_FUNC_INIT_VARS(unit);
        ...
        *flush_buffer_entries_max =
            (capacity * MDB_DIRECT_BASIC_ENTRY_SIZE /
             (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->row_width /
              (1 << MDB_EM_ENTRY_ENCODING_HALF))) + dnx_data_mdb.em.em_info_get(unit,
                                                                                dbal_physical_table_id)->ovf_cam_size;
    }
exit:
    SHR_FUNC_EXIT;
}
                ...
                if (reply_fifo_empty_counter == 0)
                {
...

                    {
                        SHR_IF_ERR_EXIT(soc_reg32_get
                                        (unit, MACT_LARGE_EM_REPLY_FIFO_ENTRY_COUNTr, REG_PORT_ANY, 0,
                                         &reply_fifo_count));
                    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (found_valid_entry)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_payload_update
                        (unit, entry_handle->handle_id, DBAL_RESULT_TYPE_NOT_INITIALIZED, entry_32, NULL, _SHR_CORE_ALL,
                         FLUSH_MACT_ENTRY_SIZE, entry_32));
    }
    else
    {
