    ...
    *continue_calling_mdb_get_next = FALSE;
                   ...
                   DBAL_CORE_MODE_DPC)) && (table->core_mode == DBAL_CORE_MODE_DPC))) && (DBAL_MAX_NUM_OF_CORES > 1)))
    {
        ...
        while (iterator_info->is_end)
        {
            ...
            else if (iterator_info->mdb_iterator.core_id < (DBAL_MAX_NUM_OF_CORES - 1))
            {
    ...
    if (!iterator_info->is_end)
    {
            ...
            (!SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_RESULT_TYPE_MAPPED_TO_SW)))
        {
            ...
            {
                entry_handle->core_id = iterator_info->mdb_iterator.core_id;
            }
        }
    ...
    if (!support_mdb_general_phys_db)
    {
        ...
        else
        {
            SHR_IF_ERR_EXIT(dbal_mdb_access_delete_one_table
                            (unit, app_id, entry_handle->table->physical_db_id[entry_handle->core_id], entry_handle,
                             0));
        }
    }
    else
