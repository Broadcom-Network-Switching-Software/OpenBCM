    ...
    dbal_tcam_cs_stage_info_t * stage_info)
{
    ...
    if (SOC_MEM_IS_VALID(unit, stage_info->reply_memory))
    {
    ...
    {
        SHR_ERR_EXIT(_SHR_E_ACCESS, "Invalid reply memory for stage %d\n", dbal_stage);
    }
    ...
    int check_validity_only)
{
    ...
    if ((entry_handle->table->core_mode != DBAL_CORE_MODE_SBC) && (entry_handle->core_id != DBAL_CORE_ALL))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_hl_access_block_calc(unit, entry_handle, entry_handle->core_id, stage_info->cmd_memory,
                                                  &block_index_info, NULL, 1, &first_block, &num_of_blocks));
    }
    ...
    int check_validity_only)
{
    ...
    if (stage_info->supports_half_entries)
    {
        rv = dbal_tcam_cs_key_read_half_entry_supported(unit, stage_info, entry_num, entry_handle, check_validity_only);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND) SHR_IF_ERR_EXIT_NO_MSG(rv);
    }
    else
    {
        rv = dbal_tcam_cs_key_read_full_line(unit, stage_info, entry_num, &valid_bits, entry_handle->phy_entry.key,
                                             entry_handle->phy_entry.k_mask, entry_handle, check_validity_only);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND) SHR_IF_ERR_EXIT_NO_MSG(rv);
    }
    ...
    dbal_actions_e dbal_action)
{
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF(dbal_tcam_cs_key_read_full_line
                              (unit, stage_info, (entry_num / 2), &valid_bits_read, key, k_mask, entry_handle, FALSE),
                              _SHR_E_NOT_FOUND);
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (memory == INVALIDm)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "memory illegal for table %s \n", table->table_name);
    }
    ...
    SHR_IF_ERR_EXIT(dbal_hl_access_block_calc(unit, entry_handle, entry_handle->core_id, memory,
                                              &(block_index_info), NULL, 1, &block, &num_of_blocks));
    ...
    dbal_actions_e dbal_action)
{
    ...
    if (entry_handle->phy_entry.indirect_commit_mode != DBAL_INDIRECT_COMMIT_MODE_FORCE)
    {
        int rv;
        rv = dbal_tcam_cs_key_read(unit, &stage_info, entry_handle->phy_entry.entry_hw_id, entry_handle, TRUE);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);

        if ((rv == _SHR_E_NONE) && (entry_handle->phy_entry.indirect_commit_mode == DBAL_INDIRECT_COMMIT_MODE_NORMAL))
        {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_tcam_cs_entry_add_internal(unit, entry_handle, DBAL_ACTION_ENTRY_COMMIT),
                                       _SHR_E_NOT_FOUND);
    SHR_EXIT();
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "result type not initialized table = %s\n", entry_handle->table->table_name);
    }
    ...
    SHR_SET_CURRENT_ERR(dbal_tcam_cs_key_read
                        (unit, &stage_info, entry_handle->phy_entry.entry_hw_id, entry_handle, FALSE));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (entry_handle->phy_entry.indirect_commit_mode != DBAL_INDIRECT_COMMIT_MODE_FORCE)
    {
        default_handle->phy_entry.indirect_commit_mode = DBAL_INDIRECT_COMMIT_MODE_UPDATE;
    }
    SHR_SET_CURRENT_ERR(dbal_tcam_cs_entry_add_internal(unit, default_handle, DBAL_ACTION_ENTRY_CLEAR));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (iter = 0; iter < entry_handle->table->max_capacity; iter++)
    {
        ...
        SHR_IF_ERR_EXIT_EXCEPT_IF(dbal_tcam_cs_entry_delete(unit, entry_handle), _SHR_E_NOT_FOUND);
    }
