...
#define DBAL_SWSTATE_DPC_TBL_IGNORE(unit, core_mode, core_id) \
...
{
    sw_state_idx_htbl_init_info_t hash_tbl_init_info;
...
static shr_error_e
dbal_sw_access_first_valid_result_get(
    int unit,
    dbal_logical_table_t * table,
    multi_res_info_t * first_valid_result)
{
    ...
    {
        if (!table->multi_res_info[result_type_index].is_disabled)
        {
            *first_valid_result = table->multi_res_info[result_type_index];
            SHR_EXIT();
        }
    }
    SHR_ERR_EXIT(_SHR_E_INTERNAL, "table %s has only invalid results\n", table->table_name);
exit:
    SHR_FUNC_EXIT;
}
    ...
    for (ii = 0; ii < DBAL_NOF_TABLES; ii++)
    {
        ...
        switch (table->access_method)
        {
            ...
            case DBAL_ACCESS_METHOD_MDB:
                ...
                {
                    multi_res_info_t first_valid_result;
                    SHR_IF_ERR_EXIT(dbal_sw_access_first_valid_result_get(unit, table, &first_valid_result));
                    max_result_length_bytes = BITS2BYTES(first_valid_result.results_info[0].field_nof_bits);
                }
                break;
    ...
    dbal_entry_handle_t * entry_handle)
{
...

    {
        ...
        {
            DBAL_SWSTATE_DPC_TBL_IGNORE(unit, entry_handle->table->core_mode, core_id);
                                                   ...
                                                   key_val));
    {
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            if (entry_handle->phy_entry.indirect_commit_mode != DBAL_INDIRECT_COMMIT_MODE_FORCE)
            {
                ...
                if (found && (entry_handle->phy_entry.indirect_commit_mode == DBAL_INDIRECT_COMMIT_MODE_NORMAL))
                {
                    SHR_IF_ERR_EXIT_NO_MSG(_SHR_E_EXISTS);
                }

                if (!found && (entry_handle->phy_entry.indirect_commit_mode == DBAL_INDIRECT_COMMIT_MODE_UPDATE))
                {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    do
    {
        ...
        if (SW_STATE_INDEX_HASH_TABLE_ITER_IS_END(&iterator_info->sw_iterator.hash_entry_index))
        {
            iterator_info->is_end = TRUE;
            break;
        }
    ...
    while (!SW_STATE_INDEX_HASH_TABLE_ITER_IS_END(&iterator_info->sw_iterator.hash_entry_index));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    switch (table_type)
    {
        case DBAL_SW_TABLE_DIRECT:
            ...
            break;
        case DBAL_SW_TABLE_HASH:
            SHR_IF_ERR_EXIT_NO_MSG(dbal_sw_table_hash_entry_add(unit, entry_handle));
            break;
        default:
            break;
    }
    ...
    else
    {
        if (table->max_capacity == 0)
        {
            ...
            if (table->access_method == DBAL_ACCESS_METHOD_MDB)
            {
                ...
                (*num_of_entries) = max_capacity;
            }
            else if (table->access_method == DBAL_ACCESS_METHOD_SW_STATE)
            {
                if (table->sw_access_info.table_size_formula.cb)
                {

                    SHR_IF_ERR_EXIT(table->sw_access_info.
                                    table_size_formula.cb(unit, NULL, DBAL_FIELD_EMPTY, &max_capacity));
                }
                else
                {
                    max_capacity = table->sw_access_info.table_size_formula.val;
                }
                (*num_of_entries) = max_capacity;
