...
setup_itmh_tx(xd_t *xd)
{
    ...
    if (!SOC_IS_ARAD(xd->xd_tx_unit)) {
        ...
        if (SOC_IS_JERICHO(xd->xd_tx_unit) && !soc_property_get(xd->xd_tx_unit, spn_ITMH_ARAD_MODE_ENABLE, 0)) {
            /** Jericho Mode */
            ...
            if (xd->itmh_fwd_type == DPP_HDR_ITMH_FWD_TYPE_UNICAST_DIRECT) {
                ...
                itmh.base.dest_info.dest_info_mi = (xd->itmh_dest_sys_port >> 8) & 0xff;
                itmh.base.dest_info.dest_info_lo = xd->itmh_dest_sys_port & 0xff;
                pkt_info->_dpp_hdr_type = DPP_HDR_itmh_base;
            } else if (xd->itmh_fwd_type == DPP_HDR_ITMH_FWD_TYPE_SYSTEM_MULTICAST) {
                itmh.base.dest_info.dest_info_hi = 0x0 | ((xd->itmh_multicast_id >> 16) & 0x7);
                itmh.base.dest_info.dest_info_mi = (xd->itmh_multicast_id >> 8) & 0xff;
                itmh.base.dest_info.dest_info_lo = xd->itmh_multicast_id & 0xff;
                pkt_info->_dpp_hdr_type = DPP_HDR_itmh_base;
            } else if (xd->itmh_fwd_type == DPP_HDR_ITMH_FWD_TYPE_UNICAST_FLOW) {
                itmh.base.dest_info.dest_info_hi = 0xe | ((xd->itmh_flow_id >> 16) & 0x1);
                itmh.base.dest_info.dest_info_mi = (xd->itmh_flow_id >> 8) & 0xff;
                itmh.base.dest_info.dest_info_lo = xd->itmh_flow_id & 0xff;
                pkt_info->_dpp_hdr_type = DPP_HDR_itmh_base;
            } else if (xd->itmh_fwd_type == DPP_HDR_ITMH_FWD_TYPE_ISQ_FLOW) {
                itmh.base.dest_info.dest_info_hi = 0x8 | ((xd->itmh_isq_flow >> 16) & 0x1);
                itmh.base.dest_info.dest_info_mi = (xd->itmh_isq_flow >> 8) & 0xff;
                itmh.base.dest_info.dest_info_lo = xd->itmh_isq_flow & 0xff;
                ...
                itmh.base.dest_info.dest_info_mi = (xd->itmh_out_lif >> 8) & 0xff;
                itmh.base.dest_info.dest_info_lo = xd->itmh_out_lif & 0xff;
                itmh.dest_ext.dest.dest_info_hi = (xd->itmh_dest_ext >> 16) & 0xf;
                itmh.dest_ext.dest.dest_info_mi = (xd->itmh_dest_ext >> 8) & 0xff;
                itmh.dest_ext.dest.dest_info_lo = xd->itmh_dest_ext & 0xff;
                pkt_info->_dpp_hdr_type = DPP_HDR_itmh_dest_ext;
                ...
                itmh.base.dest_info.dest_info_mi = (xd->itmh_multicast_flow >> 8) & 0xff;
                itmh.base.dest_info.dest_info_lo = xd->itmh_multicast_flow & 0xff;
                itmh.dest_ext.dest.dest_info_hi = (xd->itmh_dest_ext >> 16) & 0xf;
                itmh.dest_ext.dest.dest_info_mi = (xd->itmh_dest_ext >> 8) & 0xff;
                itmh.dest_ext.dest.dest_info_lo = xd->itmh_dest_ext & 0xff;
                pkt_info->_dpp_hdr_type = DPP_HDR_itmh_dest_ext;
...
/*
 *  Simplest test command for the SOC DMA ROM interface
 */
char cmd_dpp_drt_usage[] =
    "Parameters: [init|detach|rx|tx]\n"
    "  init     - Reconfigure TX/RX channels in polled mode \n"
    "                 RX channel is specified by Macro SOC_DMA_ROM_RX_CHANNEL, channel 2 by default\n"
    "                 RX is started up and ready for packets to arrive\n"
    "                 TX channel is specified by Macro SOC_DMA_ROM_TX_CHANNEL, channel 3 by default\n"
    "                 Packet can be transmited out via DmaRomTest tx.\n"
    "  detach   - Finalize the polled TX/RX module \n"
    "                 Abort DMA active on the all DMA channels\n"
    "  rx       - Check whether the packet is received, and if so, dump it\n"
    "                 Poll RX DMA channel \n"
    "                 If packet is recevied, dump packet data in human readable form\n"
    "  tx       - Simple packet transmit using soc_dma_rom_tx_start() \n"
    "                 Packet in fixed format of ETH1oPTCH2 is injected from fixed CPU port 200\n"
    "                     PTCH2 delivers PP port of CPU port 200\n"
    "                     ETH1 is boardcasted with VID=1\n";

/**
 * \brief - DmaRomTest command implementation
 * \param [in] unit - Unit #
 ...
 */
cmd_result_t
cmd_dpp_drt(int unit, args_t *args)
{
    char* arg = NULL;
    int rv = CMD_OK;
    ...
    if(!sal_strcmp(arg, "init")) {
        if((rv = soc_dma_rom_init(unit, 1600, 1, 1)) < 0) {
            cli_out("cmd_dpp_drt: soc_dma_rom_init() returned %d: %s\n",
                    rv, soc_errmsg(rv));
            return CMD_FAIL;
        }
    }
        ...
        if((rv = soc_dma_rom_detach(unit)) < 0) {
            cli_out("cmd_dpp_drt: soc_dma_rom_detach() returned %d: %s\n",
                    rv, soc_errmsg(rv));
            return CMD_FAIL;
        }
    }
        ...
        if((rv=soc_dma_rom_rx_poll(unit, &dcb)) < 0) {
            cli_out("cmd_dpp_drt: soc_dma_rom_rx_poll() returned %d: %s\n",
                    rv, soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if(dcb) {
            ...
            pkt = (uint8 *)SOC_DCB_ADDR_GET(unit, dcb);
            len = SOC_DCB_XFERCOUNT_GET(unit, dcb);

            cli_out("Packet Received:\n");
            soc_dma_dump_pkt(unit, "rx: ", pkt, len, TRUE);

            soc_dma_rom_dcb_free(unit, dcb);
        }
        else {
            cli_out("Operation failed. No packet was detected\n");
        }
    }
    else if(!sal_strcmp(arg, "tx")) {
        uint8 pkt_data[100] = { 0x80, 0x00, /* PTCH2 */
        ...
        int polls = 0;
        int done = 0;
        /* Packet is injected from CPU port 200 */
        uint8 cpu_port = 200;
        uint32 pp_port = 0;
        int core = 0;
        uint32 channel = 0;
        /* TX poll timeout limit 1000000 us*/
        int timeout = 1000000;
        
        dcb = soc_dma_rom_dcb_alloc(unit, sizeof(pkt_data));
        if (dcb == NULL) {
            cli_out("cmd_dpp_drt: soc_dma_rom_dcb_alloc() failed\n");
            return CMD_FAIL;
        }

        if (SOC_IS_JERICHO(unit)) {
            dcb28_t* dcb28 = dcb;
            if((rv=soc_port_sw_db_local_to_pp_port_get(unit, cpu_port, &pp_port, &core)) < 0) {
                cli_out("cmd_dpp_drt: soc_port_sw_db_local_to_pp_port_get() returned %d: %s\n",
                        rv, soc_errmsg(rv));
                soc_dma_rom_dcb_free(unit, dcb);
                return CMD_FAIL;
            }
            /* set PTCH2.SSP */
            pkt_data[1] = pp_port&0xff;
            /* c_hg is set to indicate Module Header is present*/
            dcb28->c_hg = 1;

            /* get CPU channel of CPU port 200 */
            if((rv=soc_port_sw_db_channel_get(unit, cpu_port, &channel)) < 0) {
                cli_out("cmd_dpp_drt: soc_port_sw_db_channel_get() returned %d: %s\n",
                        rv, soc_errmsg(rv));
                soc_dma_rom_dcb_free(unit, dcb);
                return CMD_FAIL;
            }
            /* set CPU channel to Module header */
            dcb28->mh0 = soc_htonl((channel & 0xff) << 24);
        }

        dcb_addr = (uint8 *)SOC_DCB_ADDR_GET(unit, dcb);
        /* packet buffer is fixed 1600 bytes, the pkt_data is fixed 100 bytes, packet buffer must have enough space */
        sal_memcpy(dcb_addr, pkt_data, sizeof(pkt_data));

        if ((rv = soc_dma_rom_tx_start(unit, dcb)) < 0) {
            cli_out("cmd_dpp_drt: soc_dma_rom_tx_start() returned %d: %s\n",
                    rv, soc_errmsg(rv));
            soc_dma_rom_dcb_free(unit, dcb);
            return CMD_FAIL;
        }
        ...
        /* check if TX done polling timed out */
        if (timeout < 0)
        {
            cli_out("Operation failed. Tx done polling timed out\n");
            soc_dma_rom_dcb_free(unit, dcb);
            return CMD_FAIL;
        }
