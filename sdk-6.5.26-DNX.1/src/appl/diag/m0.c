...
_soc_m0fw_default_filename(int unit, char *filename) {
    ...
    for(i = 0; i < _SOC_M0FW_DEFAULT_PATHS_NUM; i++) {
        sal_strlcpy(filepath, paths[i], 257);
        /* M0 FW binary filename is fixed */
        sal_strncat(filepath, filename, 257 - sal_strlen(filepath) - 1);
        fp = sal_fopen(filepath, "rb");
        if (fp) {
            found = 1;
            sal_strlcpy(filename, filepath, M0FW_FILENAME_SIZE);
            sal_fclose(fp);
            break;
        }
    }
...
char m0_usage[] =
    ...
    "\tcommand = [load/unload/status/stats/start/stop/log]\n"
 ...
 */
{
    ...
    if (!sal_strcasecmp(c, "load")) {
        ...
        sal_strlcpy(filename, c, M0FW_FILENAME_SIZE);
        ...
        for(ucnum = 0; ucnum < MAX_UCORES; ucnum++) {
            ...
            err = soc_uc_m0_status_version(unit, ucnum, &status,
                                               &version, buf, len);
            if (SOC_SUCCESS(err) && status) {
                cli_out("uC %d status: Ok\n", ucnum);
                cli_out("version:  %02d.%02d.%02d.%02d  %s\n",
...
#endif
    } else if (!sal_strcasecmp(c, "unload")) {
        c = ARG_GET(a);
        if (c == NULL) {
            return(CMD_USAGE);
        }
        ...
        err = soc_uc_m0_reset(unit, ucnum);
        if (SOC_FAILURE(err)) {
            cli_out("M0 uC: %d unload failed\n", ucnum);
        }
    }
