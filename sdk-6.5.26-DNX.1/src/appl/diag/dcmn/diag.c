...
#include <appl/diag/dcmn/diag_pp.h>
#include <appl/diag/dcmn/diag_oam.h>
#include <appl/diag/dcmn/diag_buffers.h>
#include <appl/diag/dcmn/diag_mmu.h>
#include <appl/diag/dcmn/diag_field.h>
#include <appl/diag/dcmn/diag_alloc.h>
#include <appl/diag/dcmn/fabric.h>
#include <sal/appl/sal.h>
#include <appl/diag/dcmn/diag.h>
#include <appl/diag/system.h>
#ifdef INCLUDE_IPSEC
#include <appl/diag/dcmn/diag_ipsec.h>
#endif
...
STATIC int diag_print_iqm_rejects_and_erpp_discards(int unit, uint8 core_disp) {
    ...
    {
        if (SOC_IS_QAX(unit)){
            if (BCM_E_NONE != soc_reg_get(unit, CGM_REJECT_STATUS_BITMAPr, core, 0, &cgm_reg_val)) {
               return CMD_FAIL;
            }
        }
        else{
...
int jericho_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp) {
    ...
    jericho_graphic_counters_e i, start, end;

    counters_data = sal_alloc(NOF_GRAPHIC_COUNTERS * sizeof(uint64), "jericho_counters_graphic_print.counters_data");
    if(counters_data == NULL) {
        cli_out("Memory allocation failure\n");
        return CMD_FAIL;
    }
    sal_memset(counters_data, 0x0, NOF_GRAPHIC_COUNTERS * sizeof(uint64));

    dual_counters_data = sal_alloc(NOF_GRAPHIC_COUNTERS * sizeof(uint64), "jericho_counters_graphic_print.dual_counters_data");
    if(dual_counters_data == NULL) {
        sal_free(counters_data);
        cli_out("Memory allocation failure\n");
        return CMD_FAIL;
    }
    sal_memset(dual_counters_data, 0x0, NOF_GRAPHIC_COUNTERS * sizeof(uint64));
    ...
    for (i = NBIH_PRD_PKT_DROP_CNT_PORT_0; i <= NBIH_PRD_PKT_DROP_CNT_PORT_23; i++) {
        COMPILER_64_ADD_64(sum_counters, counters_data[i]);
    }

    if(SOC_IS_JERICHO_PLUS_ONLY(unit)) {
        start = NBIL_PRD_PKT_DROP_CNT_PORT_0;
        end = NBIL_PRD_PKT_DROP_CNT_PORT_23;
    } else {
        start = NBIL_RX_PRD_PKT_DROP_CNT_0;
        end = NBIL_RX_PRD_PKT_DROP_CNT_23;
    }
    for (i = start; i <= end; i++) {
        COMPILER_64_ADD_64(sum_counters, counters_data[i]);
        if (core_disp) {
            COMPILER_64_ADD_64(sum_counters, dual_counters_data[i]);
        }
    }
...
cmd_dpp_lag(int unit, args_t *a) {
    ...
    /*Allocate abd clear member_array*/
    member_array = sal_alloc(BCM_TRUNK_MAX_PORTCNT * sizeof(bcm_trunk_member_t), "Trunk Members Array");
...
cmd_dpp_retransmit(int unit, args_t *a) {
    ...
    if (SOC_IS_DFE(unit)) {
        cli_out("Diag retransmit unavailable for DFE \n");
        return CMD_FAIL;
    }
    ...
    if (res == CMD_OK) {
        ...
        BCM_PBMP_ITER(valid_ports, port) {
            ...
            if (nif_type == SOC_PORT_IF_ERP || SOC_PORT_IS_VIRTUAL_RCY_INTERFACE(flags) || SOC_PORT_IS_LB_MODEM(flags)
                    || SOC_PORT_IS_ELK_INTERFACE(flags) || SOC_PORT_IS_STAT_INTERFACE(flags)) {
                continue;
            }
         ...
         for (core=first_core; core < last_core; core++) {
             ...
             if (local_to_sys) {
                ...
                BCM_PBMP_ITER(bcm_pbmp, local_port_id) {
                   ...
                   /*get system gport*/
                   res = bcm_stk_gport_sysport_get(unit, gport, &sys_gport);
                   if (SOC_FAILURE(res)) {
                       cli_out("\nfailed to get local to system port information.\n\n");
                       res = CMD_FAIL;
                   } else {
                       /*get system port*/
