...
appl_dpp_itmh_prog_mode_diag_init(int unit)
{
    ...
    bcm_field_extraction_field_t ext_field[7];
    bcm_field_data_qualifier_t data_qual;
    int qual_id[7] = {58,59,60,61,62,63,57};
    int fwd_offset_qual_id[7] = {50,51,52,53,54,55,56}; /* data qualifier created for fwd_offset_fix, 50~56 */
    int fwd_offset_qual_bits[7] = {1, 1, 1, 3, 1, 3, 3};
    bcm_field_group_t entry_groups[17]={122,123,122,121,127,122,122,124,124,124,124,124,125,126,126,126,126};
    ...
    int ii, stat_en, fwd_offset_fix_en;
    uint32
        udh2_size,
    ...
    fwd_offset_fix_en = SOC_IS_QAX(unit) && soc_property_get(unit, "ingress_congestion_management_pkt_header_compensation_enable", 0);

    if (fwd_offset_fix_en) {
        /*
         * Added one more entry to update bcmFieldActionForwardingTypeNew
         * fwd_offset should point to the end of ITMH,
         * this fix is required on QAX when ingress_congestion_management_pkt_header_compensation_enable
         * is set.
         * Otherwise Network_Header_Truncate_Size will be a negative value.
         */
        nof_itmh_entries=17;
    }
   ...
   */
    ...
    /*
     * fwd_offset should point to the end of ITMH for QAX.
     *
     * Create data qualifiers to build the following key:
     *     key = {3'b111, 3'b000, 1'b1, 3'b000, 1'b1, 1'bITMH_BASE_EXTENSION_EXISTS, 1'bITMH_BASE_EXTENSION_EXISTS}
     *     {4'bfwd_code, 9'bfwd_offset} = key
     *     fwd_code = 4'b1110 = TM
     *     fwd_offset_index = 3'b001
     *     fwd_offset = ITMH_BASE_EXTENSION_EXISTS ? 7 : 4
     */
    if(fwd_offset_fix_en)
    {
        for (ii=0; ii<sizeof(fwd_offset_qual_id)/sizeof(fwd_offset_qual_id[0]); ++ii) {
            bcm_field_data_qualifier_t_init(&data_qual);

            if (ii<2) {
                /*Data Qualifier for ITMH extension exist*/
                data_qual.flags = BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_OFFSET_BIT_RES  | BCM_FIELD_DATA_QUALIFIER_WITH_ID;
                data_qual.offset_base = bcmFieldDataOffsetBaseL2Header;
                data_qual.offset = 24;
            }
            else {
                data_qual.flags = BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_OFFSET_BIT_RES  | BCM_FIELD_DATA_QUALIFIER_WITH_ID | BCM_FIELD_DATA_QUALIFIER_OFFSET_PREDEFINED;
                data_qual.qualifier = ((ii==3) || (ii==5)) ? bcmFieldQualifyConstantZero : bcmFieldQualifyConstantOne;
                data_qual.offset = 0;
            }

            data_qual.length = fwd_offset_qual_bits[ii];
            data_qual.qual_id = fwd_offset_qual_id[ii];
            rv = bcm_field_data_qualifier_create(unit, &data_qual);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_APPL_COMMON,
                          (BSL_META_U(unit,
    ...
    */
    ...
    if (fwd_offset_fix_en) {
        for (ii=0; ii<sizeof(fwd_offset_qual_id)/sizeof(fwd_offset_qual_id[0]); ++ii) {
            rv = bcm_field_qset_data_qualifier_add(unit, &grp.qset, fwd_offset_qual_id[ii]);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_APPL_COMMON,
                        (BSL_META_U(unit,
                                "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),
    ...
    grp.priority = 126;
    grp.flags = BCM_FIELD_GROUP_CREATE_WITH_MODE | BCM_FIELD_GROUP_CREATE_WITH_ID | BCM_FIELD_GROUP_CREATE_WITH_PRESELSET;
    grp.flags |= fwd_offset_fix_en ? BCM_FIELD_GROUP_CREATE_LARGE : 0;
    grp.mode = bcmFieldGroupModeDirectExtraction;
    grp.preselset = psset;
    rv = bcm_field_group_config_create(unit, &grp);
    if (BCM_FAILURE(rv)) {
    ...
    for (entry_ndx = 0; entry_ndx < nof_itmh_entries; entry_ndx++) {
     ...
     /*build extraction fields*/
      ...
      case 0: /* PPH type */
          ...
          ext_field[0].qualifier = qual_id[0];
          ext_field[0].lsb = 30;
          ext_field[0].bits = 2;
      ...
      case 4: /* multicast destination id */
          ...
          bcm_field_extraction_field_t_init(&ext_field[0]);
          ext_field[0].flags = BCM_FIELD_EXTRACTION_FLAG_DATA_FIELD;
          ext_field[0].qualifier = qual_id[5];
          ext_field[0].lsb = 0;
          ext_field[0].bits = 17;
      ...
      case 7: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = outlif*/
          ...
          bcm_field_extraction_field_t_init(&ext_field[0]);
          ext_field[0].flags = BCM_FIELD_EXTRACTION_FLAG_DATA_FIELD;
          ext_field[0].qualifier = qual_id[3];
          ext_field[0].lsb = 0;
          ext_field[0].bits = 16;
          if (SOC_IS_JERICHO(unit)) {
              ext_field[0].bits = 18;
     ...
     case 9:  /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = outlif*/
      ...
      case 12: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = ISQ*/
       ...
       case 14: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS =0- remove ITMH = 4 Bytes*/
        ...
        case 15: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = ISQ*/
          ...
          break;
          case 16: /* Fix fwd_offset for QAX */
              if (fwd_offset_fix_en) {
				  for (ii=0; ii<sizeof(fwd_offset_qual_id)/sizeof(fwd_offset_qual_id[0]); ++ii) {
                      bcm_field_extraction_field_t_init(&ext_field[ii]);
                      ext_field[ii].flags = BCM_FIELD_EXTRACTION_FLAG_DATA_FIELD;
                      ext_field[ii].qualifier = fwd_offset_qual_id[ii];
                      ext_field[ii].lsb = 0;
                      ext_field[ii].bits = fwd_offset_qual_bits[ii];
                  }

                  bcm_field_extraction_action_t_init(&ext_action);
                  ext_action.action = bcmFieldActionForwardingTypeNew;
              }
      ...
      rv = bcm_field_direct_extraction_action_add(unit,
                                                  ent,
                                                  ext_action,
                                                  (entry_ndx == 5 || entry_ndx == 13 || entry_ndx == 14 || entry_ndx == 15) ? 2 :
                                                  (entry_ndx == 16) ? 7 : 1 /* count */,
                                                  ext_field );
