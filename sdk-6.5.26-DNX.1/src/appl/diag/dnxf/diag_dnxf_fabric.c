    ...
    sh_sand_control_t * sand_control)
{
    ...
    int pipe_idx, block_idx, fifo_idx, dfl_bank_idx, ifm_block_idx, aligned_block_idx;
     ...
     */
    SHR_IF_ERR_EXIT(soc_dnxf_block_active_instance_bmp_get(unit, SOC_BLK_DCH, &dch_bmp));
    SHR_IF_ERR_EXIT(soc_dnxf_block_active_instance_bmp_get(unit, SOC_BLK_DCML, &dcml_bmp));
        ...
        for (pipe_idx = 0; pipe_idx < nof_pipes; pipe_idx++)
        {
            for (block_idx = 0; block_idx < dnxf_data_device.blocks.nof_instances_dch_get(unit); block_idx++)
            {
                     ...
                     ifm_block_idx++)
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT(soc_dnxf_drv_inner_block_align
                                        (unit, SOC_BLK_DCH, block_idx, &aligned_block_idx));
        ...
        for (pipe_idx = 0; pipe_idx < nof_pipes; pipe_idx++)
        {
            for (block_idx = 0; block_idx < dnxf_data_device.blocks.nof_instances_dcml_get(unit); block_idx++)
            {
                ...
                for (fifo_idx = 0; fifo_idx < dnxf_data_device.blocks.nof_dtm_fifos_get(unit); fifo_idx++)
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT(soc_dnxf_drv_inner_block_align
                                        (unit, SOC_BLK_DCML, block_idx, &aligned_block_idx));
        ...
        for (pipe_idx = 0; pipe_idx < nof_pipes; pipe_idx++)
        {
            for (block_idx = 0; block_idx < dnxf_data_device.blocks.nof_instances_dcml_get(unit); block_idx++)
            {
                ...
                if (current_dtl_block->max_occupancy_value)
                {
                    ...
                    SHR_IF_ERR_EXIT(soc_dnxf_drv_inner_block_align(unit, SOC_BLK_DCML, block_idx, &aligned_block_idx));
