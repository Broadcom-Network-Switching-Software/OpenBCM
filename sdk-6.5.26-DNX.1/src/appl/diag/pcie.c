...
STATIC cmd_result_t
_pcie_phy_fw_cmp(int unit, args_t *args, pciephy_access_t *sa)
{
#ifndef NO_FILEIO
    int rv = SOC_E_NONE;
    char *file_str;
    long len;
    uint8 *rbuf = NULL, *fbuf = NULL;
    soc_flash_conf_t flash;
    FILE *in = NULL;
    int i, j;
    ...
    fbuf = sal_alloc(len, "LOADER_FILE");
    if (!fbuf) {
        rv = SOC_E_MEMORY;
        goto error;
    }

    if (sal_fread(fbuf, sizeof(uint8), len, in) != len) {
        rv = _SHR_E_INTERNAL;
        goto error;
    }

    /* Read data from flash. */
    rbuf = sal_alloc(len, "FLASH_READ");
    if (!rbuf) {
        rv = SOC_E_MEMORY;
        goto error;
    }

    rv = soc_flash_read(unit, 0, len, rbuf);
    if (SOC_FAILURE(rv)) {
        goto error;
    }

    /* Compare and print the first mismatch. */
    cli_out("Comparing PCIE firmware loader...\n");
    for (i = 0; i <  len; i++) {
        if (rbuf[i] != fbuf[i]) {
            break;
        }
    ...
    if (i < len) {
        cli_out("Mismatches at %08x:\n", i);
        j = (i / 16) * 16;
        cli_out("File content -\n");
        for (i = 0; i < 128 && j + i < len; i++) {
            if (i % 16 == 0) {
                cli_out("%08x ", i + j);
            } else if (i % 2 == 0) {
                cli_out(" ");
            }
            cli_out("%02x", fbuf[j + i]);
            if (i % 16 == 15) {
                cli_out("\n");
            }
        }
...
_pcie_phy_fw(int unit, args_t *args, pciephy_access_t *sa)
{
    ...
    if (sal_strcasecmp(cmd_str, "load") == 0) {
    ...
    } else if (sal_strcasecmp(cmd_str, "cmp") == 0) {
        rv = _pcie_phy_fw_cmp(unit, args, sa);
        ...
        cli_out("\n%s is not a valid command option for pciephy fw\n", cmd_str);
        rv = CMD_FAIL;
    }
...
char pciephy_usage[] =
    ...
    "        fw - sub commands:\n\t"
    "          load <file> - programs contents of given firmware file to QSPI flash\n\t"
    "          cmp <file> - Compare flash content with given file\n\t"
...
cmd_result_t cmd_pciephy(int u, args_t *a)
{
    ...
    if (c != NULL && sal_strcasecmp(c, "access") == 0) {
            return _pcie_phy_fw_access(u, a);
    }
