...
_iproc_write_from_args(int unit, int ce, uint32 addr, args_t *args) {
    ...
    if (SOC_IS_J2X(unit)) {
        nsip_wr = 0;
        if ((addr >= 0x600000) && (addr < 0x650000)) {
            nsip_wr = 1;
        }
    }
#endif
    ...
    for (i = 0; i < len; i++) {
        ...
        if (nsip_wr) {
            soc_pci_write(unit, (addr + (i * 4)), data);
        } else {
...
_iproc_write_from_bin_file(int unit, int ce, uint32 addr, FILE *fp) {
    ...
    while ((i = sal_fread(&data,4,1,fp)) != 0) {
        ...
        if (nsip_wr) {
            soc_pci_write(unit, (addr + (len * 4)), data);
        } else {
...
_iproc_dump(int unit, int ce, void *fp, uint32 addr, int len) {
    ...
    for (i = 0; i < len; i++) {
        if (nsip_rd) {
            rval = soc_pci_read(unit, (addr + (i * 4)));
        } else {
