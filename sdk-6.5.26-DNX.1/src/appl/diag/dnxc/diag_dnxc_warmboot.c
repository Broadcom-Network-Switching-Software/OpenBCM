    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (bcm_switch_control_set(unit, bcmSwitchControlSync, BCM_SWITCH_CONTROL_SYNC_FULL) != _SHR_E_NONE)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        cli_out("total: %d.%d MB. (%d bytes)\n", BYTES_TO_MBS((left + in_use)), BYTES_TO_MBS_CARRY((left + in_use)),
                left + in_use);
        cli_out("in use: %d.%d MB. (%d bytes)\n", BYTES_TO_MBS(in_use), BYTES_TO_MBS_CARRY(in_use), in_use);
        cli_out("left: %d.%d MB (%d bytes)\n", BYTES_TO_MBS(left), BYTES_TO_MBS_CARRY(left), left);
    }
    else
    {
        cli_out("total: %d.%d KB. (%d bytes)\n", BYTES_TO_KBS((left + in_use)), BYTES_TO_KBS_CARRY((left + in_use)),
                left + in_use);
        cli_out("in use: %d.%d KB. (%d bytes)\n", BYTES_TO_KBS(in_use), BYTES_TO_KBS_CARRY(in_use), in_use);
        cli_out("left: %d.%d KB. (%d bytes)\n", BYTES_TO_KBS(left), BYTES_TO_KBS_CARRY(left), left);
    }
