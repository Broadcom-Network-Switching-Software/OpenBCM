...
#if ((defined(BCM_PETRA_SUPPORT) || defined (BCM_DPP_SUPPORT) || defined (BCM_DFE_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)) 
...
sh_swap_unit_vars(int new_unit)
{
    ...
    if (old_unit >= 0) {
        ...
        if (NULL != SOC_CONTROL(old_unit)) {
            ...
            if (SOC_IS_DNX2(old_unit)) {
                var_unset("BCM_DNX2_DEV", FALSE, TRUE, FALSE);
                if (SOC_IS_JERICHO2(old_unit)) {
                ...
                if (SOC_IS_Q2A(old_unit)) {
                    if (var_get("BCM_Q2A_DEV"))
                    {
                        var_unset("BCM_Q2A_DEV", FALSE, TRUE, FALSE);
                    }
                    else if (var_get("BCM_Q2N_DEV"))
                    {
                        var_unset("BCM_Q2N_DEV", FALSE, TRUE, FALSE);
                    }
                }
                if (SOC_IS_J2P(old_unit)) {
                    var_unset("BCM_J2P_DEV", FALSE, TRUE, FALSE);
                }
                if (SOC_IS_J2X(old_unit)) {
                    var_unset("BCM_J2X_DEV", FALSE, TRUE, FALSE);
                }
            }
...
#if defined(BCM_DNX_SUPPORT)
            is_q2n_sku = ((dev_id_driver & DNXC_DEVID_FAMILY_MASK) == Q2N_DEVICE_ID);
#endif
        }
        ...
        if (soc_feature(new_unit, soc_feature_uc_image_name_with_no_chip_rev)) {
            ...
            if (SOC_IS_METROLITE(new_unit)) {
                if ((dev_id == BCM53460_DEVICE_ID) || (dev_id == BCM53461_DEVICE_ID)) {
                    sal_strcpy(uc_image_name_prefix, (char *)"BCM53460_A0");
                } else {
            ...
            } else if (SOC_IS_FIREBOLT6(new_unit)) {
                sal_strcpy(uc_image_name_prefix, (char *)"BCM56470_A0");
            } else {
    ...
    if (new_unit >= 0) {
        ...
        for (i = 0; i < soc_ndev; i++) {
            if (NULL != SOC_CONTROL(i))
            {
                if (SOC_IS_J2P(i)) {
                     var_set_integer("BCM_J2P_IN_SYSTEM", 1, FALSE, FALSE);
                     break;
                }
                else if (SOC_IS_J2X(i)) {
                    var_set_integer("BCM_J2X_IN_SYSTEM", 1, FALSE, FALSE);
                    break;
                }
            }
        }
...
#endif
#if defined(BCM_DNX_SUPPORT)
        if (SOC_IS_DNX2(new_unit)) {

            var_set_integer("BCM_DNX2_DEV", 1, FALSE, FALSE);
            if (SOC_IS_JERICHO2(new_unit)) {
            ...
            if (SOC_IS_Q2A(new_unit)) {
                if (is_q2n_sku)
                {
                    var_set_integer("BCM_Q2N_DEV", 1, FALSE, FALSE);
                }
                else
                {
            ...
            if (SOC_IS_J2X(new_unit)) {
                var_set_integer("BCM_J2X_DEV", 1, FALSE, FALSE);
            }
        ...
        if (SOC_IS_LTSW(new_unit)) {
            ...
            command_mode_set(new_unit, CTSW_CMD_MODE);
#else
 ...
 */
{
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        if (SOC_IS_LTSW(SOC_NDEV_IDX2DEV(i))) {
            if (soc_ltsw_shutdown(SOC_NDEV_IDX2DEV(i)) < 0) {
                return(CMD_FAIL);
            }
 ...
 */
{
    ...
    if (arg_count == 1) {
        ...
        if (!sal_strcasecmp(arg_str, "clean") && !SOC_IS_DNX(u)) {
#ifdef BCM_WARM_BOOT_SUPPORT
#if (defined(LINUX) && !defined(__KERNEL__)) || defined(UNIX)
            if (!SOC_IS_LTSW(u)) {
                if (soc_property_get(u, spn_DIAG_SHELL_EXIT_DO_NOTHING, FALSE)) {
 ...
 */
{
    ...
    if (sal_config_file_process(fname, NULL, NULL, ".", 1, 0) != 0)
    {
        return CMD_FAIL;
    }
 ...
 */
{
    ...
    sal_memset(source_list, 0, sizeof(bsl_source_list_t));
 ...
 */
{
    ...
    } else if (!sal_strcasecmp("restore", c)) {
        bslerror_enable_restore();
    } else {
        return(CMD_USAGE);
    }
    return(CMD_OK);
...
char sh_version_usage[] =
    "Parameters: [all]\n\t"
    "Prints current build information (default)\n\t"
    "all - Lists all the supported chips and variants in addition to the build information.\n";
 ...
 */
{
    ...
    if (sysconf_ltsw_probe() < 0) {
        ...
        if ((sysconf_ltsw_ha_issu_start()) < 0) {
 ...
 */
{
    ...
    for (dev = 0; dev < SOC_MAX_NUM_DEVICES; dev++) {
        if (CMDEV(dev).dev.info) {
            cli_out("%s: Unit %d (%s%s): %sattached%s\n",
                    ARG_CMD(a),
 ...
 * Remark:
 *   We assume here that the size of the woking directory string is smaller than 1024.
 *   If it is larger, this procedure will fail.
 */
{
    char    pwd[1024];
    ...
    if (!sal_getcwd(pwd, sizeof(pwd))) {
        cli_out(
              "%s: Error: Unable to determine current directory\r\n"
              "==> (Failure may be due to Current Working Directory string larger than %d.)\r\n",
              ARG_CMD(a),(int)sizeof(pwd));
        return(CMD_FAIL);
    } else {
...
#if ((defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined (BCM_DPP_SUPPORT) || defined (BCM_DFE_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)) 
char sh_state_usage[] =
    "Parameters: [sw | usage]\n"
    "sw: \n"
    "    [dump size]\n"
...
/*
 * Function to test the easy reload
 *
 * Easy reload does a boot without HW access
 * Still all global variables including the SW state are deleted
 * Once the init under easy reload is done however the SW state should be the same
 * as the one under cold boot because we should have done the same steps but with no HW access.
 * The same applies if we also loaded with the applications
 *
 * The test checks the above statement by:
 * 1. Making a pre easy reload SW state dump
 * 2. Enters easy reload mode
 * 3. Does a deinit/init
 * 4. Makes post easy reload SW state dump
 * 5. Compares the two dumps.
 */
static cmd_result_t
xxreload_test(int unit)
{
    int rv = 0;
    char cwd[1024] = {0};
    char pre_reboot_file_name[1024] = {0};
    char post_reboot_file_name[1024] = {0};
    char shell_command[1024] = {0};

    if (getcwd(cwd, sizeof(cwd)) == NULL) {
        cli_out("Cannot get working directory\n");
        return CMD_FAIL;
    }

    sal_strncpy(pre_reboot_file_name, cwd, sizeof(cwd) - 1);
    sal_strncpy(post_reboot_file_name, cwd, sizeof(cwd) - 1);
    sal_strncat(pre_reboot_file_name, "/sw_state_pre_easy_reload", sizeof(pre_reboot_file_name) - sal_strnlen(pre_reboot_file_name, sizeof(pre_reboot_file_name)) - 1);
    sal_strncat(post_reboot_file_name, "/sw_state_post_easy_reload", sizeof(post_reboot_file_name) - sal_strnlen(post_reboot_file_name, sizeof(post_reboot_file_name)) - 1);

    cli_out("Set pre Easy reload file location\n");
    sal_strncpy(shell_command, "swstate config dump file_path=", sizeof(shell_command) - 1);
    sal_strncat(shell_command, pre_reboot_file_name, sizeof(shell_command) - sal_strnlen(shell_command, sizeof(shell_command)) - 1);

    rv = sh_process_command(unit, shell_command);
    if (rv != CMD_OK)
    {
        cli_out("Cannot set config file for SW state\n");
        return CMD_FAIL;
    }

    cli_out("Make a SW state dump\n");
    rv = sh_process_command(unit, "swstate dump");
    if (rv != CMD_OK)
    {
        cli_out("Cannot dump SW state\n");
        return CMD_FAIL;
    }

    rv = sh_process_command(unit, "swstate config dump file_unset");
    if (rv != CMD_OK)
    {
        cli_out("Fail saving file\n");
        return CMD_FAIL;
    }

    cli_out("Enter easy reload mode\n");
    rv = xxreload_set(unit, TRUE);
    if (BCM_FAILURE(rv)) {
        cli_out("EASY RELOAD set failed\n");
        return CMD_FAIL;
    }

    cli_out("Reboot under Easy Reload mode\n");
    rv = sh_process_command(unit, "tr 141");
    if (rv != CMD_OK)
    {
        cli_out("Reboot under Easy Reload mode failed!\n");
        return CMD_FAIL;
    }

    cli_out("Set post Easy Reload dump file location\n");
    sal_strncpy(shell_command, "swstate config dump file_path=", sizeof(shell_command) - 1);
    sal_strncat(shell_command, post_reboot_file_name, sizeof(shell_command) - sal_strnlen(shell_command, sizeof(shell_command)) - 1);
    rv = sh_process_command(unit, shell_command);
    if (rv != CMD_OK)
    {
        cli_out("Cannot set config file for SW state\n");
    ...
    cli_out("Diff the two files\n");
    sal_strncpy(shell_command, "diff -q ", sizeof(shell_command) - 1);
    strncat(shell_command, pre_reboot_file_name, sizeof(shell_command) - sal_strnlen(shell_command,sizeof(shell_command)) - 1);
    strncat(shell_command, " ", sizeof(shell_command) - 1);
    strncat(shell_command, post_reboot_file_name, sizeof(shell_command) - sal_strnlen(shell_command,sizeof(shell_command)) - 1);
    rv = system(shell_command);
    if (rv != CMD_OK)
    {
        cli_out("Compare failed. Here is the diff\n");
        sal_strncpy(shell_command, "diff -u ", sizeof(shell_command) - 1);
        strncat(shell_command, pre_reboot_file_name, sizeof(shell_command) - sal_strnlen(shell_command,sizeof(shell_command)) - 1);
        strncat(shell_command, " ", sizeof(shell_command) - 1);
        strncat(shell_command, post_reboot_file_name, sizeof(shell_command) - sal_strnlen(shell_command,sizeof(shell_command)) - 1);
        rv = system(shell_command);
    ...
    sal_strncpy(shell_command, "rm ", sizeof(shell_command) - 1);
    strncat(shell_command, pre_reboot_file_name, sizeof(shell_command) - sal_strnlen(shell_command,sizeof(shell_command)) - 1);
    rv = system(shell_command);
...
#ifdef BCM_DNXF_SUPPORT
    else if (!sal_strcasecmp(c, "test")) 
    {
        return (xxreload_test(unit));
    }
#endif
    else 
    {
...
#if defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)
    "  rx_los - init RX LOS application of BCM88660 devices\n\n"
#endif
 ...
 */
{
    ...
    if ((bcm_rv = system_init(u)) < 0) {
        ...
        if (!(SOC_FLAGS_GET(u) & SOC_F_INITED)) {
 ...
 */
{
    ...
    if (c == NULL)
    {
        cli_out("%s, #01: Failed ARG_GET\n", "sh_cd");
        return(CMD_FAIL);
    }
    if (strcmp(c, "-sethome") == 0) {
        sethome = 1;
        c = ARG_GET(a);
    }
    ...
    if (c == NULL)
    {
        cli_out("%s, #02: Failed ARG_GET\n", "sh_cd");
        return(CMD_FAIL);
    }
    if (sethome && sal_homedir_set(c) < 0) {
...
cmd_sh_restest(int unit, args_t *a){
    ...
    if (!sal_strncasecmp(c,"create",resTestMax(sal_strlen(c),1))) {
        ...
        c = ARG_GET(a);
        if (localRes) {
            /* Coverity complains if we print pointer after it is freed */
            sal_snprintf(ptrstr, sizeof(ptrstr), "%p", (void *)localRes);
            result = shr_mres_destroy(localRes);
            if (BCM_E_NONE == result) {
                cli_out("destroyed %s resources successfully\n", ptrstr);
                localRes = NULL;
            } else {
