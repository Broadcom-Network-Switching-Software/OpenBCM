...
#ifdef BCM_CTSW_SUPPORT
extern cmd_result_t cmd_ctsw_ledproc(int unit, args_t *a);
#endif
 ...
 *          at linkchange event and warmboot recovery.
 *          Customers should revise this function by their own requirement.
...
void
unregister_ledproc_linkscan_cb(int unit)
{
    bcm_linkscan_unregister(unit, ledproc_linkscan_cb);
    return;
}
/*
 * Function: ledproc_sync_status
 * Purpose: To sync port and LED status for all ports.
 *
 * Parameters: unit - unit number.
 *
 * Returns: nothing
...
STATIC void
ledproc_sync_status(int unit, bcm_pbmp_t pbmp)
{
    int port;
    ...
    PBMP_ITER(pbmp, port) {
        ...
        ledproc_linkscan_cb(unit, port, &info);
    }
}
 ...
 */
{
    ...
    if (rv < 0) {
        cli_out("%s: Fail to check CNA.\n", ARG_CMD(a));
        return CMD_FAIL;
    }
    if (is_cna) {
        return cmd_ctsw_ledproc(unit, a);
    }
#endif
...
#if (defined(BCM_ESW_SUPPORT) || defined(BCM_PETRA_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)
