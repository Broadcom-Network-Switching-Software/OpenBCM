...
typedef enum
{
    DIAG_FLOW_ENTRY_TYPE_LIF,
    DIAG_FLOW_ENTRY_TYPE_MATCH,
    DIAG_FLOW_ENTRY_TYPE_ALL,

    DIAG_NOF_FLOW_ENTRY_TYPES
} diag_flow_entry_type_e;
    ...
    while (encap_access_bitmap[0])
    {
        if (encap_access_bitmap[0] & 0x1)
        {
            ...
            while (res_type_bitmap[0])
            {
                ...
                {
                    char_count = sal_snprintf(cur_str_pos, DBAL_MAX_STRING_LENGTH, "%s\n",
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (field_idx = 0; field_idx < nof_special_fields; field_idx++)
    {
        ...
        for (idx = 0; idx < FLOW_NOF_SPECIAL_FIELDS_IND; idx++)
        {
            ...
            if (is_external)
            {
                sal_strncat_s(indications_to_print, dnx_flow_special_fields_indications_to_string(unit, idx),
                              sizeof(indications_to_print));
                sal_strncat_s(indications_to_print, "\n", sizeof(indications_to_print));
            }
        }
        if (sal_strnlen(indications_to_print, sizeof(indications_to_print)))
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (flow_app_info->valid_common_fields_bitmap)
    {
        ...
        PRT_COLUMN_ADD("Mapped DBAL Field");
        ...
        for (field_idx = 0; field_idx < nof_common_fields; field_idx++)
        {
            if (SAL_BIT(field_idx) & flow_app_info->valid_common_fields_bitmap)
            {
                ...
                PRT_CELL_SET("%s",
                             (common_fields_desc->mapped_dbal_field != DBAL_FIELD_EMPTY) ? dbal_field_to_string(unit,
                                                                                                                common_fields_desc->mapped_dbal_field)
                             : "N.A.");
            }
        }
        PRT_COMMITX;
    ...
    void *user_data)
{
    ...
    for (field_idx = 0; field_idx < nof_common_fields; field_idx++)
    {
        if (SAL_BIT(field_idx) & flow_app_info->valid_common_fields_bitmap)
        {
            ...
            SHR_IF_ERR_EXIT(common_fields_desc->print_cb(unit, FLOW_APP_TYPE_TERM,
                                                         (void *) terminator_info, print_string));
    ...
    void *user_data)
{
    ...
    for (field_idx = 0; field_idx < nof_common_fields; field_idx++)
    {
        if (SAL_BIT(field_idx) & flow_app_info->valid_common_fields_bitmap)
        {
            ...
            SHR_IF_ERR_EXIT(common_fields_desc->print_cb(unit, FLOW_APP_TYPE_INIT,
                                                         (void *) initiator_info, print_string));
            PRT_CELL_SET("%s", print_string);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(diag_flow_app_prepare_columns(unit, flow_handle, sand_control, prt_ctr));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (rv == _SHR_E_NONE)
    {
        SHR_IF_ERR_EXIT(diag_flow_app_dump_print(unit, flow_handle_id, 0, sand_control));
    }
...
static shr_error_e
diag_flow_entry_get_lif_output(
    int unit,
    int gport,
    sh_sand_control_t * sand_control)
{
    ...
    {
        rv = algo_gpm_gport_flow_convert
            (unit, gport, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_CALC, &gport);
        if (rv != _SHR_E_NONE)
        {
            SHR_ERR_EXIT(_SHR_E_NONE, "Gport %d is invalid or not set", gport);
        }
    }
    ...
    if ((gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS) || (gport_subtype == BCM_GPORT_SUB_TYPE_IN_LIF))
    {
        rv = bcm_flow_terminator_info_get(unit, &flow_handle_info, &terminator_info, &special_fields);
    }
             ...
             (gport_subtype == BCM_GPORT_SUB_TYPE_OUT_LIF))
    {
        is_egress = TRUE;
        rv = bcm_flow_initiator_info_get(unit, &flow_handle_info, &initiator_info, &special_fields);
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_NONE, "Unknown subtype 0x%x for gport 0x%x\n", gport_subtype, gport);
    }

    if (rv != _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_NONE, "Couldn't find gport 0x%x", gport);
    }

    flow_app_info = dnx_flow_app_info_get(unit, flow_handle_id);

    PRT_TITLE_SET("Lif related information for gport 0x%x, %s", gport, flow_app_info->app_name);

    diag_flow_data.prt_ctr = prt_ctr;
    SHR_IF_ERR_EXIT(diag_flow_app_prepare_columns(unit, flow_handle_id, sand_control, prt_ctr));

    if (is_egress)
    {
        SHR_IF_ERR_EXIT(diag_flow_app_init_entry_print_cb
                        (unit, &flow_handle_info, &initiator_info, &special_fields, &diag_flow_data));
    }
    else
    {
        SHR_IF_ERR_EXIT(diag_flow_app_term_entry_print_cb
                        (unit, &flow_handle_info, &terminator_info, &special_fields, &diag_flow_data));
    }
...
static shr_error_e
diag_flow_entry_get_match_output(
    int unit,
    int gport,
    sh_sand_control_t * sand_control)
{
    dnx_diag_flow_traverse_data_t diag_flow_data;
    const dnx_flow_app_config_t *flow_app_info;
    flow_match_entries_t *match_entries = NULL;
    int ent_idx, rv;

    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);

    SHR_ALLOC_SET_ZERO_ERR_EXIT(match_entries, sizeof(flow_match_entries_t),
                                "match_entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    sal_memset(match_entries, 0, sizeof(flow_match_entries_t));
    if (!BCM_GPORT_IS_FLOW_LIF(gport))
    {
        rv = algo_gpm_gport_flow_convert
            (unit, gport, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_CALC, &gport);
    ...
    rv = dnx_flow_match_info_by_gport_multiple_get(unit, gport, match_entries);
    if (rv != _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_NONE, "Couldn't find gport %d related match entries", gport);
    }

    for (ent_idx = 0; ent_idx < match_entries->nof_entries; ent_idx++)
    {
        flow_app_info = dnx_flow_app_info_get(unit, match_entries->match_entries[ent_idx].flow_handle_info.flow_handle);

        PRT_TITLE_SET("Match entry dump for gport 0x%x in App %s", gport, flow_app_info->app_name);

        diag_flow_data.prt_ctr = prt_ctr;
        SHR_IF_ERR_EXIT(diag_flow_app_prepare_columns
                        (unit, match_entries->match_entries[ent_idx].flow_handle_info.flow_handle, sand_control,
                         prt_ctr));

        SHR_IF_ERR_EXIT(diag_flow_app_match_entry_print_cb
                        (unit, &match_entries->match_entries[ent_idx].flow_handle_info,
                         &match_entries->match_entries[ent_idx].special_fields, &diag_flow_data));
    }

    PRT_COMMITX;

exit:
    SHR_FREE(match_entries);
    PRT_FREE;
    SHR_FUNC_EXIT;
}
...
/**
 * \brief
 * Display a specific entry (identified by gport given)
 *
 * \return
 * _SHR_E_NONE - Successful command
 */
static shr_error_e
diag_flow_entry_get_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    diag_flow_entry_type_e output_type;
    int gport;

    SHR_FUNC_INIT_VARS(unit);

    SH_SAND_GET_ENUM("Type", output_type);
    SH_SAND_GET_UINT32("Gport", gport);

    if (output_type == DIAG_FLOW_ENTRY_TYPE_LIF)
    {
        SHR_IF_ERR_EXIT(diag_flow_entry_get_lif_output(unit, gport, sand_control));
    }
    else if (output_type == DIAG_FLOW_ENTRY_TYPE_MATCH)
    {
        SHR_IF_ERR_EXIT(diag_flow_entry_get_match_output(unit, gport, sand_control));
    }
    else if (output_type == DIAG_FLOW_ENTRY_TYPE_ALL)
    {
        SHR_IF_ERR_EXIT(diag_flow_entry_get_lif_output(unit, gport, sand_control));
        SHR_IF_ERR_EXIT(diag_flow_entry_get_match_output(unit, gport, sand_control));
    }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported output type\n");
    }
...
static sh_sand_enum_t sand_flow_entry_type_enum_table[] = {
    {"lif", DIAG_FLOW_ENTRY_TYPE_LIF, "Returns the lif related info for the gport"},
    {"match", DIAG_FLOW_ENTRY_TYPE_MATCH, "Returns the match keys entries related to the gport"},
    {NULL}
};

static sh_sand_option_t diag_flow_entry_get_options[] = {
    {"Type", SAL_FIELD_TYPE_ENUM, "Field type LIF", "LIF",
     (void *) sand_flow_entry_type_enum_table},
    {"Gport", SAL_FIELD_TYPE_UINT32, "Gport", "0"},
    {NULL}
};

static sh_sand_man_t dnx_flow_entry_get_man = {
    .brief = "Dump matching entry based on gport, application",
    .full = "Dump entry (all fields) based on gport, application",
    .synopsis = "[Type=<Field type>] [Name=<Field name>] [Gport=<Gport val>]",
    .examples = "Type=lif Gport=5\n"
};

static sh_sand_cmd_t diag_flow_entry_cmds[] = {
    /******************************************************************************************************************
     * CMD    *     CMD_ACTION           * Next *        Options               *            MAN             * CB      *
     * NAME   *                          * Level*                              *                            *         *
     *        *                          * CMD  *                              *                            *         *
     ******************************************************************************************************************/
    {"get", diag_flow_entry_get_cmd, NULL, diag_flow_entry_get_options, &dnx_flow_entry_get_man},
    {NULL}
};

#define SEVERITY_PRINT (bslSeverityVerbose + 1)
static sh_sand_enum_t flowl_logger_severity_enum_table[] = {
    {"OFF", bslSeverityOff, "Logger is off"},
    {"ERROR", bslSeverityError, "Logger active level is ERROR"},
    {"WARN", bslSeverityWarn, "Logger active level is WARNING"},
    {"INFO", bslSeverityInfo, "Logger active level is INFO"},
    {"VERB", bslSeverityVerbose, "Logger active level is VERBOSE"},
    {"PRINT", SEVERITY_PRINT, "PRINT Logger settings"},
    {NULL}
};

static sh_sand_option_t diag_flow_logger_options[] = {
    {"SeVerity", SAL_FIELD_TYPE_ENUM, "logger active level", "PRINT", (void *) flowl_logger_severity_enum_table},
    {NULL}
};

static shr_error_e
diag_flow_logger_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    ...
    /** set for all but PRINT */
    if (severity != SEVERITY_PRINT)
    {
        bslenable_set(bslLayerBcmdnx, bslSourceFlow, severity);
    }
    else        /* for SEVERITY_PRINT, need only to print the current severity, rather than setting it */
    {
        severity = bslenable_get(bslLayerBcmdnx, bslSourceFlow);
    }

    LOG_CLI((BSL_META("Flow logger severity was set to %s\n"), bsl_severity2str(severity)));
...
static sh_sand_man_t dnx_flow_entry_man = {
...
sh_sand_man_t dnx_flow_logger_man = {
    .brief = "FLOW logger diagnostic commands",
    .full = NULL,
    .examples = "Severity=WARN"
};
...
sh_sand_cmd_t sh_dnx_flow_cmds[] = {
   /************************************************************************************************************
    *   CMD_NAME   *          CMD        *       Next          *         Options        *       MAN            *
    *              *        ACTION       *       Level         *                        *                      *
    *              *                     *        CMD          *                        *                      *
    ************************************************************************************************************/
    {"APPLication", NULL,                 diag_flow_app_cmds,   NULL,                     &dnx_flow_app_man},
    {"Field",       NULL,                 diag_flow_field_cmds, NULL,                     &dnx_flow_field_man},
    {"entry",       NULL,                 diag_flow_entry_cmds, NULL,                     &dnx_flow_entry_man},
    {"LoGger",      diag_flow_logger_cmd, NULL,                 diag_flow_logger_options, &dnx_flow_logger_man},
    {NULL}
};
/* *INDENT-ON* */
