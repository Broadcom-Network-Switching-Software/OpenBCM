...
char cmd_dnx_knet_usage[] = "Usages:\n"
    ...
    "  knetctrl pcie [up|down]\n" "        Up/down to indicate PCIE link status, this option is only for debug.\n";
    ...
    args_t * args)
{
    ...
    if (sal_strncasecmp(subcmd, "netif", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if ((subcmd = ARG_GET(args)) == NULL)
        {
        ...
        if (sal_strncasecmp(subcmd, "create", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
        ...
        else if (sal_strncasecmp(subcmd, "destroy", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
        ...
        else if (sal_strncasecmp(subcmd, "show", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            count = 0;
    ...
    else if (sal_strncasecmp(subcmd, "filter", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if ((subcmd = ARG_GET(args)) == NULL)
        {
            cli_out("Requires string argument\n");
    ...
    else if (sal_strncasecmp(subcmd, "pcie", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if ((subcmd = ARG_GET(args)) == NULL)
        {
            cli_out("Requires string argument\n");
        ...
        if (sal_strcasecmp(subcmd, "up") == 0)
        {
            if (soc_knet_pcie_link_status_set(unit, 0) < 0)
            {
                return CMD_FAIL;
            }
        }
        else if (sal_strcasecmp(subcmd, "down") == 0)
        {
            if (soc_knet_pcie_link_status_set(unit, 1) < 0)
            {
                return CMD_FAIL;
            }
        }
