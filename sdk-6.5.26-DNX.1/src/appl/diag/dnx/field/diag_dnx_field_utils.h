 ...
 */
#define DIAG_DNX_FIELD_UTILS_STR_SIZE               250
/**
 * Maximum string size, to be used when we expect larger strings.
 */
#define DIAG_DNX_FIELD_UTILS_STR_SIZE_LARGER        1200
/**
 * \brief
...
{                                                                                                       \
    ...
    int _str_len = 0, _current_str_len = 0, _current_str_total_len = 0;                                 \
    SHR_ALLOC(_str, DIAG_DNX_FIELD_UTILS_INPUT_STR_SIZE, "_str", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);    \
    if (_str == NULL)                                                                                   \
    {                                                                                                   \
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Failed to allocated memory.\r\n");                               \
    }                                                                                                   \
    sal_snprintf(_str, DIAG_DNX_FIELD_UTILS_INPUT_STR_SIZE, _format, __VA_ARGS__);                      \
    _str_len = sal_strnlen(_str, DIAG_DNX_FIELD_UTILS_INPUT_STR_SIZE);                                  \
    if (_current_str_addr != 0)                                                                         \
    {                                                                                                   \
        _current_str_total_len = sizeof(_current_str) - 1;                                              \
        _current_str_len = sal_strnlen(_current_str, sizeof(_current_str));                             \
        if ((_current_str_len + _str_len) > _current_str_total_len)                                     \
        {                                                                                               \
            LOG_ERROR_EX(BSL_LOG_MODULE,                                                                \
                     "\r\n"                                                                             \
                     "Given string (\"%s\") size (%d) plus old sting size (%d) is bigger then the "     \
                     "current string maximum size %d!\r\n",                                             \
                     _str, _str_len, _current_str_len, _current_str_total_len);                         \
            SHR_FREE(_str);                                                                             \
            SHR_SET_CURRENT_ERR(_SHR_E_INTERNAL);                                                       \
            SHR_EXIT();                                                                                 \
        }                                                                                               \
    }                                                                                                   \
    else if (_str_len > (DIAG_DNX_FIELD_UTILS_STR_SIZE -1))                                             \
    {                                                                                                   \
        LOG_ERROR_EX(BSL_LOG_MODULE,                                                                    \
                     "\r\n"                                                                             \
                     "Given string (%s) size (%d) is bigger then the maximum allowed size %d!%s\r\n",   \
                     _str, _str_len, (DIAG_DNX_FIELD_UTILS_STR_SIZE - 1), EMPTY);                       \
        SHR_FREE(_str);                                                                                 \
        SHR_SET_CURRENT_ERR(_SHR_E_INTERNAL);                                                           \
        SHR_EXIT();                                                                                     \
    }                                                                                                   \
 ...
 * \param [out] converted_fg_type - The converted string of FG type
 * \return
 ...
 * \param [out] converted_mode - The converted string of Context cmp, hash, state mode
 * \return
 *  \retval _SHR_E_NONE  - success
 */
 ...
 * \param [in] template_offset_on_key -
 *  What bit on key the field group starts.
