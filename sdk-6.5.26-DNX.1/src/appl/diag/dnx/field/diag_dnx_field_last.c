    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (DIAG_DNX_FIELD_LAST_IS_SIM)
    {
        ...
        if (adapter_core == -1)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "adapter_core is -1.\n");
        }

        if ((core_id != adapter_core) && (core_id != _SHR_CORE_ALL))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Invalid CORE (%d) was given. CORE values for adapter, can be '%d' or 'all'\r\n\n",
                         core_id, adapter_core);
        }
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_index)
    {
        ...
        if (core_index == -1)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "core_index is -1.\n");
        }

        if (DIAG_DNX_FIELD_LAST_IS_SIM)
        {
            if (core_index != adapter_core)
            {
                continue;
            }
        }
