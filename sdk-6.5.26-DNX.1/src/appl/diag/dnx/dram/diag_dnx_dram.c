    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    DNX_DRAM_BMP_FIRST(dram_bitmap, max_nof_dram, first_enabled_dram);
    SHR_IF_ERR_EXIT(dnx_gddr6_dynamic_calibration_is_enabled
                    (unit, first_enabled_dram, 0, &dynamic_calibration_is_enabled_in_hw));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** before running BIST, clear ECC counters, so we know any ECC found later happened during BIST */
...
static shr_error_e
dnx_dram_bist_main_section_print(
    int unit,
    sh_sand_control_t * sand_control,
    uint32 dram_start,
    uint32 dram_end,
...
/** print dram parity errors table in case any were found */
static shr_error_e
dnx_dram_bist_hbm_parity_print(
    int unit,
    sh_sand_control_t * sand_control,
    uint32 dram_start,
    uint32 dram_end,
    ...
    uint32 channel_end)
{
    ...
    int is_dram_valid = 0;
    uint32 channel;
    int parity_errors_found = FALSE;

    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);

    /** print parity error counters */
    /** title */
    PRT_TITLE_SET("Dram parity errors");
    /** columns */
    PRT_COLUMN_ADD("Dram index");
    PRT_COLUMN_ADD("Channel");
    PRT_COLUMN_ADD("Write DWORD0");
    PRT_COLUMN_ADD("Write DWORD1");
    PRT_COLUMN_ADD("Write DWORD2");
    PRT_COLUMN_ADD("Write DWORD3");
    PRT_COLUMN_ADD("Read DWORD0");
    PRT_COLUMN_ADD("Read DWORD1");
    PRT_COLUMN_ADD("Read DWORD2");
    PRT_COLUMN_ADD("Read DWORD3");

    for (dram_index = dram_start; dram_index <= dram_end; ++dram_index)
    {
        SHR_IF_ERR_EXIT(dnx_dram_index_is_valid(unit, dram_index, &is_dram_valid));
        if (!is_dram_valid)
        {
            continue;
        }
        for (channel = channel_start; channel <= channel_end; ++channel)
        {
            hbmc_parity_err_cnt_t parity_error_counters;

            /** get error counters */
            SHR_IF_ERR_EXIT(dnx_hbmc_parity_errors_get(unit, dram_index, channel, &parity_error_counters));

            /** info */
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%d", dram_index);
            PRT_CELL_SET("%d", channel);
            /** Print write parity errors */
            for (i_dword = 0; i_dword < nof_channel_dwords; ++i_dword)
            {
                PRT_CELL_SET("0x%02x%08x", parity_error_counters.write_parity_err_cnt[i_dword][1],
                             parity_error_counters.write_parity_err_cnt[i_dword][0]);

                /** flag if parity errors were found */
                if (parity_error_counters.write_parity_err_cnt[i_dword][1] != 0
                    || parity_error_counters.write_parity_err_cnt[i_dword][0] != 0)
                {
                    parity_errors_found = TRUE;
                }
            }
            /** Print read parity errors */
            for (i_dword = 0; i_dword < nof_channel_dwords; ++i_dword)
            {
                if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_apd_phy))
                {
                     ...
                     * This will result with incorrect read parity errors when write parity is enabled, thus read parity errors should be
                     * ignored in this case.
                     */
                    uint32 write_parity_enable = dnx_data_dram.general_info.dq_write_parity_get(unit);
                    if (write_parity_enable)
                    {
                        PRT_CELL_SET("N/A");
                        continue;
                    }
                ...
                PRT_CELL_SET("0x%02x%08x", parity_error_counters.read_parity_err_cnt[i_dword][1],
                             parity_error_counters.read_parity_err_cnt[i_dword][0]);

                /** flag if parity errors were found */
                if (parity_error_counters.read_parity_err_cnt[i_dword][1] != 0
                    || parity_error_counters.read_parity_err_cnt[i_dword][0] != 0)
                {
                    parity_errors_found = TRUE;
                }
            }
    ...
    /** print the table only if parity errors were found */
    if (parity_errors_found)
    {
        PRT_COMMITX;
    }
    else
    {
        LOG_CLI((BSL_META("No DRAM parity errors\n")));
    }
    ...
    uint32 channel_end)
{
    ...
    uint32 dram_index;
    uint32 channel;
    int ecc_errors_found = FALSE;

    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);

    PRT_TITLE_SET("Dram ECC errors");
    PRT_COLUMN_ADD("Dram index");
    PRT_COLUMN_ADD("Channel");
    PRT_COLUMN_ADD("Lower channel corrected ECC counter");
    PRT_COLUMN_ADD("Upper channel corrected ECC counter");
    PRT_COLUMN_ADD("Lower channel detected ECC counter");
    PRT_COLUMN_ADD("Upper channel detected ECC counter");
    ...
    for (dram_index = dram_start; dram_index <= dram_end; ++dram_index)
    {
        ...
        for (channel = channel_start; channel <= channel_end; ++channel)
        {
            ...
            uint32 is_ecc_enabled = TRUE;

            SHR_IF_ERR_EXIT(dnx_hbmc_ecc_enable_get(unit, dram_index, channel, &is_ecc_enabled));
            if (!is_ecc_enabled)
            {
                continue;
            }
            ...
            /** get error counters */
            SHR_IF_ERR_EXIT(dnx_hbmc_ecc_errors_get(unit, dram_index, channel, &ecc_err_cnt));
            ...
            if (ecc_err_cnt.corrected_err_cnt_lower_channel[0] > 0 ||
                ecc_err_cnt.corrected_err_cnt_lower_channel[1] > 0 ||
                ecc_err_cnt.corrected_err_cnt_upper_channel[0] > 0 ||
                ecc_err_cnt.corrected_err_cnt_upper_channel[1] > 0 ||
                ecc_err_cnt.detected_err_cnt_lower_channel[0] > 0 ||
                ecc_err_cnt.detected_err_cnt_lower_channel[1] > 0 ||
                ecc_err_cnt.detected_err_cnt_upper_channel[0] > 0 || ecc_err_cnt.detected_err_cnt_upper_channel[1] > 0)
            {
                ecc_errors_found = TRUE;
            }

            PRT_CELL_SET("0x%08x%08x", ecc_err_cnt.corrected_err_cnt_lower_channel[1],
                         ecc_err_cnt.corrected_err_cnt_lower_channel[0]);
            PRT_CELL_SET("0x%08x%08x", ecc_err_cnt.corrected_err_cnt_upper_channel[1],
                         ecc_err_cnt.corrected_err_cnt_upper_channel[0]);
            PRT_CELL_SET("0x%08x%08x", ecc_err_cnt.detected_err_cnt_lower_channel[1],
                         ecc_err_cnt.detected_err_cnt_lower_channel[0]);
            PRT_CELL_SET("0x%08x%08x", ecc_err_cnt.detected_err_cnt_upper_channel[1],
                         ecc_err_cnt.detected_err_cnt_upper_channel[0]);
        }
    }
    /** print the table only if ecc errors were found */
    if (ecc_errors_found)
    {
        PRT_COMMITX;
    }
    ...
    {
        LOG_CLI((BSL_META("No DRAM ECC errors\n")));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** print results */
    SHR_IF_ERR_EXIT(dnx_dram_bist_main_section_print
                    (unit, sand_control, dram_start, dram_end, channel_start, channel_end));
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_is_supported))
    {
        SHR_IF_ERR_EXIT(dnx_dram_bist_hbm_parity_print
                        (unit, sand_control, dram_start, dram_end, channel_start, channel_end));
        SHR_IF_ERR_EXIT(dnx_dram_bist_hbm_ecc_print
                        (unit, sand_control, dram_start, dram_end, channel_start, channel_end));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (index_str = 2, index_arr = 0;
         (index_str < sal_strnlen(pattern_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) && (index_arr < arr_size);
         index_str += 8, ++index_arr)
    {
        char current_uint[11] = "0x";
        sal_strncat_s(current_uint, &pattern_str[index_str], sizeof(current_uint));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        LOG_CLI((BSL_META("Cannot run DRAM PHY RESET on device which doesn't support HBM\n")));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (rate)
    {
        dnxc_time_t gtimer_time = {.time_units = DNXC_TIME_UNIT_SEC,
        ...
        SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &gtimer_time, &nof_clock_cycles));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 force_command = 0;
    uint32 dram_index;
    int temp_monitoring_is_active = FALSE;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (dram_index = dram_start; dram_index <= dram_end; ++dram_index)
    {
...

        {
            ...
            /** Read Temperature for all requested drams */
            rv = bcm_dnx_switch_thermo_sensor_read
                (unit, bcmSwitchThermoSensorDram, dram_index, &sensor_data[dram_index]);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (force_command && dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_vendor_info_disable))
    {
        ...
        LOG_WARN(BSL_LOG_MODULE,
                 (BSL_META_U(unit, "\nIMPORTANT!!! Device should be power cycled after using force\n")));
    }
...
static sh_sand_option_t dnx_debug_vendor_info_option_list[] = {
    /* Option name      Option Type             Option Description          Option Default      Option Extension                    Valid Range */
    {"dram",            SAL_FIELD_TYPE_UINT32,  "dram interface",           "0-max",            (void*)sand_diag_dram_enum,         "0-max"},
    {"force",           SAL_FIELD_TYPE_BOOL,    "force command. Device should be power cycled after using force",            "0"},
    {NULL}
};
...
static sh_sand_man_t sh_dnx_dram_bist_man = {
    .brief = "DRAM BIST diagnostics and commands.\n"
             "Used for debug purposes only. Please refer to DRAM Application Notes for usage instructions.\n",
    ...
    rhlist_t * examples_list)
{
    ...
    {
        sal_snprintf(cmd, DIAG_DNX_DRAM_CMD_MAX_LENGTH, examples_str[i_example_str], dram_index);
 ...
 */
static shr_error_e
dram_bist_start_example_generate_hbm(
    int unit,
    int flags,
    struct sh_sand_cmd_s *sh_sand_cmd,
    rhlist_t * examples_list)
 ...
 */
static shr_error_e
dram_bist_start_example_generate_gddr6(
    int unit,
    int flags,
    struct sh_sand_cmd_s *sh_sand_cmd,
    rhlist_t * examples_list)
{
    char examples_str[][DIAG_DNX_DRAM_CMD_MAX_LENGTH] = {
        "dram=%d channel=0 count=0x1000 mode=prbs prbs_data_seed=0x3451231231234353245432523412345432598098230495893427589798798732290754123421232545789135548903af4554bd48574747578487438412345678 \n",
        "dram=%d channel=0-1 read_weight=0x100 write_weight=0x100 same_row_count=0x8 bank=0-max column=0-max row=0-max \n",
        "dram=%d channel=1 pattern_0=0 pattern_1=1 pattern_2=2 pattern_3=3 pattern_4=4 pattern_5=5 pattern_6=6 pattern_7=7 \n",
    };
    int examples_str_size = sizeof(examples_str) / sizeof(examples_str[0]);
    SHR_FUNC_INIT_VARS(unit);
...
static sh_sand_man_t sh_dnx_dram_bist_start_man_hbm = {
    ...
    .example_generate_cb = dram_bist_start_example_generate_hbm
};
...
static sh_sand_man_t sh_dnx_dram_bist_start_man_gddr6 = {
    ...
    .full = "start a dram bist run.\n"
            ...
            "seed needs to be 128 hex digit long for all data bits to be randomized. "
            "pattern mode will use pattern provided in pattern_<n>",
    .example_generate_cb = dram_bist_start_example_generate_gddr6
};
...
{
    char examples_str[][DIAG_DNX_DRAM_CMD_MAX_LENGTH] = {
        "dram=%d channel=0 address=0x16 value=0x3 \n",
 ...
 */
static shr_error_e
dram_phy_tune_example_generate_hbm(
    int unit,
    int flags,
    struct sh_sand_cmd_s *sh_sand_cmd,
    rhlist_t * examples_list)
 ...
 */
static shr_error_e
dram_phy_tune_example_generate_gddr6(
    int unit,
    int flags,
    struct sh_sand_cmd_s *sh_sand_cmd,
    rhlist_t * examples_list)
{
    char examples_str[][DIAG_DNX_DRAM_CMD_MAX_LENGTH] = {
        "dram=%d shmoo_type=all plot=1 action=run \n",
        "dram=%d extended_vref_range=1 \n",
        "dram=%d last=1 \n",
    };
    int examples_str_size = sizeof(examples_str) / sizeof(examples_str[0]);
    SHR_FUNC_INIT_VARS(unit);
...
static sh_sand_man_t sh_dnx_dram_phy_tune_man_hbm = {
    ...
    .example_generate_cb = dram_phy_tune_example_generate_hbm
};

static sh_sand_man_t sh_dnx_dram_phy_tune_man_gddr6 = {
    .brief = "perform tuning on dram phy\n",
    .full = "run dram phy tuning.\n"
            "it is not required to rerun tuning if it was already ran during init\n"
            "the phy tuning process calibrates the dram phy to successfully read/write from/to the dram\n"
            ...
            "options for action are: restore from file, restore from otp, run\n"
            "when running with reset option, certain shmoo_types can pass only once per interface\n",
    .example_generate_cb = dram_phy_tune_example_generate_gddr6
};
...
static sh_sand_man_t sh_dnx_dram_debug_eye_scan_man_hbm = {
    .brief = "run eye scan",
    .full = "run different types of phy eye scans, AKA shmoo or phy tuning.",
    .examples = "dram=0 channel=0 type=all"
};

static sh_sand_man_t sh_dnx_dram_debug_eye_scan_man_gddr6 = {
    .brief = "run eye scan",
    .full = "run different types of phy eye scans, AKA shmoo or phy tuning.",
    .examples = "dram=0 type=all"
};
...
static sh_sand_man_t sh_dnx_dram_debug_bist_man = {
    ...
    .examples = "dram=0 channel=0 type=all_address"
};
...
static sh_sand_cmd_t sh_dnx_dram_bist_cmds[] = {
    ...
    {"start",  sh_dnx_dram_hbm_bist_start_cmd,    NULL,  dnx_bist_start_options_list_hbm,   &sh_dnx_dram_bist_start_man_hbm,       NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, sh_cmd_is_hbm_available},
...
static sh_sand_cmd_t sh_dnx_dram_buffer_cmds[] = {
    /* Name           | Leaf Action                   | Junction Array Pointer        | Options for Leaf              | Usage */
    {"status",        sh_dnx_dram_buffer_status_cmd,  NULL,                           dnx_buffer_status_options_list, &sh_dnx_dram_buffer_status_man, NULL, NULL, SH_CMD_NO_XML_VERIFY},
    {NULL}
};
...
static sh_sand_cmd_t sh_dnx_dram_phy_cmds[] = {
    /* Name           | Leaf Action               | Junction Array Pointer       | Options for Leaf          | Usage */
    {"tune",      sh_dnx_dram_phy_tune,   NULL,                          dnx_phy_tune_options_list_gddr6, &sh_dnx_dram_phy_tune_man_gddr6,        NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, sh_cmd_is_gddr6_available},
    {"tune",      sh_dnx_dram_phy_tune,   NULL,                          dnx_phy_tune_options_list_hbm,   &sh_dnx_dram_phy_tune_man_hbm,        NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, sh_cmd_is_hbm_available},
...
static sh_sand_cmd_t sh_dnx_dram_debug_cmds[] = {
    ...
    {"EyeScan",             sh_dnx_dram_debug_eye_scan_cmd,             NULL,               dnx_debug_eye_scan_option_list,         &sh_dnx_dram_debug_eye_scan_man_hbm,    NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY,  sh_cmd_is_hbm_available},
    {"EyeScan",             sh_dnx_dram_debug_eye_scan_cmd,             NULL,               dnx_debug_eye_scan_option_list_gddr6,   &sh_dnx_dram_debug_eye_scan_man_gddr6,  NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY,  sh_cmd_is_gddr6_available},
