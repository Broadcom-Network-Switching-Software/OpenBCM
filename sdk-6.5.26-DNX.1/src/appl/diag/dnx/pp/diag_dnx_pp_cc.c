...
extern shr_error_e dnx_vswitch_cross_connect_get_flow(
    int unit,
    ...
    int encap)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(gport) || BCM_GPORT_IS_MPLS_PORT(gport) || BCM_GPORT_IS_EXTENDER_PORT(gport))
    {
        ...
        if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
        {
            ...
            rv = dnx_vswitch_cross_connect_get_flow(unit, &gports);
            /*
             * gport hasn't forward info when only create 1 direction cross connect 
             */
            SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
            forward_info.destination = gports.port2;
        }
        else
        {
    ...
    else
    {
        ...
        if (0 == sal_strncmp(dest_type_str, "ecmp", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
