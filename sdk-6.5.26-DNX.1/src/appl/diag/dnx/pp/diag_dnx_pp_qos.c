    ...
    uint32 layer_index)
{
    ...
    if (layer_type == DBAL_ENUM_FVAL_LAYER_TYPES_IPV6)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_value_get
                        (unit, DBAL_FIELD_TYPE_DEF_IPV6_ADDITIONAL_HEADER, ipv6_list_element->value[0],
                         &additional_header));
        /** check if 1st tag exist */
    ...
    else if (shared_data->shifted_etps < shared_data->etps_data.entries_nof)
    {
        ...
        if (stage == EKLEAP_STAGE_FWD)
        {
            ...
            *in_qos_dp = (dp << DNX_QOS_DP_OFFSET) | in_nwk_qos;
            ...
            /** get qos_var_type*/
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, DBAL_FIELD_FWD_QOS_VAR_TYPE, INST_SINGLE, qos_var_type));
            ...
            {
                int signal_version = dnx_data_debug.general.signal_version_get(unit);

                if (signal_version == 2)
                {
                    /** get remark_profile from port*/
                    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", cur_stage_name, next_stage_name,
                                                  "fwd_and_encap_pp_port_attributes.Remark_Profile", qos_profile, 1));
                }
                else
                {
                     /** get remark_profile from port*/
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ENCAP_QOS_VAR_TYPE, *qos_var_type);
    ...
    prt_control_t * prt_ctr)
{
    ...
    SHR_IF_ERR_EXIT(sh_dnx_pp_cos_print_cell_set(unit, qos_var_type, DNX_PP_COS_PRINT_WIDTH_NONE, prt_ctr, ENUM,
                                                 DBAL_TABLE_QOS_EGRESS_MAP_TYPE_SELECT, DBAL_FIELD_FWD_QOS_VAR_TYPE));
