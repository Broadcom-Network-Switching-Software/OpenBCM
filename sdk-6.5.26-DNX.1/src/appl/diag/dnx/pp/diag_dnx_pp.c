...
/** The commands are verified by traffic tests*/
#define SH_CMD_IN_MEGA_CINT  SH_CMD_NO_XML_VERIFY
...
char *sh_dnx_pp_sub_layer_names[] = { "VLAN", "GRE", "VXLAN", "UDP", "PPP_CONST", "" };
...
extern sh_sand_option_t dnx_pp_info_fec_options[];
extern shr_error_e sh_dnx_pp_info_fec_cmds(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control);
extern shr_error_e sh_dnx_pp_info_fec_fwd_rpf_not_separated(
    int unit,
    rhlist_t * list);
    ...
    char **str_arr)
{
    ...
    while (sal_strncmp(*str_arr, "", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != -0)
    {
        if (exact_match)
        {
            /** exact match */
            if (sal_strncmp(str, *str_arr, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
                return index;
            }
        }
        else
        {
            ...
            if (sal_strnstr(str, *str_arr, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            {
                return index;
            }
        }
    ...
    char mpls_label_array[SH_DNX_PP_MAX_LAYERS_NUMBER][DSIG_MAX_SIZE_UINT32])
{
    ...
    sal_memset(packet_struct_str, 0, sal_strnlen(packet_struct_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH));
    if (layer_list != NULL)
    {
        /*
        ...
        RHITERATOR(layer_list_element, layer_list)
        {
          ...
          /** Use CW instead of Control_Word in the structure.*/
            if (!sal_strncmp(layer_name, "Control_Word", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            {
                layer_name = cw_name;
            }
          ...
          /** if the layer is not sub layer add to linked list of layers */
            if ((next_protocol_array) && (string_in_array(layer_name, FALSE, sh_dnx_pp_sub_layer_names) == -1))
            {
              ...
              /** mark the next string as NULL to mark end of layers */
                ...
                if (mpls_label_array
                    && (sal_strncmp(layer_name, "MPLS", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0))
                {
                    ...
                    RHITERATOR(mpls_list_element, layer_list_element->field_list)
                    {
                        ...
                        if (sal_strncmp(mpls_attr_name, "Label", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
                        {
                ...
                else if (mpls_label_array
                         && (sal_strncmp(layer_name, "CW", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0))
                {
                    sand_signal_value_to_str(unit, SAL_FIELD_TYPE_NONE, (uint32 *) &layer_list_element->value,
                                             mpls_label_array[mpls_idx], 32, PRINT_BIG_ENDIAN);
                    mpls_idx++;
                }
            }
    ...
    {
        packet_struct_str[sal_strnlen(packet_struct_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) - 1] = 0;
    }
    SHR_FUNC_EXIT;
}
...
{
    signal_output_t *sig_pp_port_element = NULL;
    signal_output_t *sig_pp_ssp_element = NULL;
    uint8 header_found = FALSE;
    int rv = BCM_E_NONE;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        SHR_IF_ERR_EXIT(sand_signal_output_find(unit, core_id, 0, "IRPP", "", "", "in_port", &sig_pp_port_element));
        rv = sand_signal_output_find(unit, core_id, 0, "IRPP", "NIF", "", "header.ptch_2.pp_ssp", &sig_pp_ssp_element);
        if (rv == BCM_E_NOT_FOUND ||
            (rv == BCM_E_NONE && sig_pp_ssp_element->value[0] == sig_pp_port_element->value[0]))

        {
            rv = sand_signal_output_find(unit, core_id, 0, "IRPP", "NIF", "", "header", sig_list_element);
            if (rv == BCM_E_NONE)
            {
                header_found = TRUE;
            }
    ...
    if (header_found == FALSE)
    {
        ...
        rv = sand_signal_output_find(unit, core_id, 0, "IRPP", "PRT", "", "Pkt_Header", sig_list_element);
        if (rv == BCM_E_NOT_FOUND)
        {
            /** Try again, generally for adapter.*/
            SHR_IF_ERR_EXIT(sand_signal_output_find
                            (unit, core_id, 0, "IRPP", "LLR", "", "packet_header", sig_list_element));
        }
    }

exit:
    sand_signal_output_free(sig_pp_port_element);
    sand_signal_output_free(sig_pp_ssp_element);
    SHR_FUNC_EXIT;
}
    ...
    RHITERATOR(field_list_element, field_list)
    {
        ...
        if (nvs == 1)
        {
            ...
            if (value_is_number)
            {
                 ...
                 * Note:
                 * An empty string or just a sign indicator are, both, considered to
                 * represent the number '0'.
                 */
                if (sal_strnlen(field_list_element->print_value, DSIG_MAX_SIZE_STR) == 0)
                {
                    signal_numeric_value = 0;
                }
                else if ((sal_strnlen(field_list_element->print_value, DSIG_MAX_SIZE_STR) == 1)
                         && (field_list_element->print_value[0] == '-'))
                {
                    signal_numeric_value = 0;
                }
                else
            ...
            else
            {
                ...
                print_result_accum[sal_strnlen(print_result_accum, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) - 1] =
                    '\0';
            }   /** of if rc */
...
static sh_sand_cmd_t sh_dnx_pp_vis_sub_cmds[] = {
    ...
    *********************************************************************************************************************************************************************************************/
    {"PacketParsingInfo",    sh_dnx_pp_ppi_cmds,        NULL, dnx_pp_ppi_options,       &dnx_pp_ppi_man,            NULL, NULL, SH_CMD_IN_MEGA_CINT},
    {"PacKeTTM",             sh_dnx_pp_pkttm_cmds,      NULL, dnx_pp_pkttm_options,     &dnx_pp_pkttm_man,          NULL, NULL, SH_CMD_IN_MEGA_CINT},
    {"ForwardDecisionTrace", sh_dnx_pp_fdt_cmds,        NULL, dnx_pp_fdt_options,       &dnx_pp_fdt_man,            NULL, NULL, SH_CMD_IN_MEGA_CINT},
    {"SYS_Header",           sh_dnx_pp_sh_cmds,         NULL, dnx_pp_sh_options,        &dnx_pp_sh_man,             NULL, NULL, SH_CMD_IN_MEGA_CINT},
    {"FER",                  NULL,     sh_dnx_pp_fer_fwd_rpf_sub_cmd, NULL,         &sh_dnx_pp_fer_fwd_rpf_sub_man, NULL, NULL, SH_CMD_IN_MEGA_CINT},
    {"TERMination",          sh_dnx_pp_termi_cmds,      NULL, dnx_pp_termi_options,     &dnx_pp_termi_man,          NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_IN_MEGA_CINT, diag_pp_bare_metal_check},
    {"IngressVlanEditing",   sh_dnx_pp_ive_cmds,        NULL, dnx_pp_ive_options,       &dnx_pp_ive_man,            NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_IN_MEGA_CINT, diag_pp_bare_metal_check},
    {"EgressVlanEditing",    sh_dnx_pp_eve_cmds,        NULL, dnx_pp_eve_options,       &dnx_pp_eve_man,            NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_IN_MEGA_CINT, diag_pp_bare_metal_check},
    {"LAST",                 sh_dnx_pp_last_cmds,       NULL, dnx_pp_last_options,      &dnx_pp_last_man,           NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_IN_MEGA_CINT, diag_pp_bare_metal_check},
    {"CONTEXT",              sh_dnx_pp_context_cmd,     NULL, dnx_pp_context_options,   &dnx_pp_context_man,        NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_IN_MEGA_CINT, diag_pp_bare_metal_check},
    {"load_balancing",       sh_dnx_pp_lb_vis_cmd,      NULL, NULL,                     &sh_dnx_diag_pp_lb_vis_man, NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_IN_MEGA_CINT, diag_pp_bare_metal_check},
    {"ClassOfService",       sh_dnx_pp_cos_cmds,        NULL, dnx_pp_cos_options,       &dnx_pp_cos_man,            NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_IN_MEGA_CINT, diag_pp_bare_metal_check},
    {"stats",                stat_pp_vis_cmd,           NULL, dnx_stat_pp_options,      &sh_dnx_stat_pp_man,        NULL, NULL, SH_CMD_CONDITIONAL, diag_pp_bare_metal_check},
    {"ikleap",               sh_dnx_ikleap_visib_cmds,  NULL, dnx_ikleap_visib_options, &dnx_ikleap_visib_man,      NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, NULL},
    {"ekleap",               sh_dnx_ekleap_vis_cmd,     NULL, dnx_ekleap_vis_options,   &dnx_ekleap_vis_man,        NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, NULL},
    {NULL}
};
...
static sh_sand_cmd_t sh_dnx_pp_info_sub_cmds[] = {
    ...
    * Command        * Command Callback       * Next Level Cmd Array       * Options                     *  MAN Struct               * OptCB Invokes *Flags        * CondCN
    *********************************************************************************************************************************************************************************************/
    {"FEC",            sh_dnx_pp_info_fec_cmds, NULL,                        dnx_pp_info_fec_options,     &dnx_pp_info_fec_man,        NULL, NULL, SH_CMD_CONDITIONAL, sh_dnx_pp_info_fec_fwd_rpf_not_separated},
