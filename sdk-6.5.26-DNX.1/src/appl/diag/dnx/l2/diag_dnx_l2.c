    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (learn_mode)
    {
        case BCM_L2_INGRESS_CENT:
            sal_snprintf(learn_mode_string, sizeof(learn_mode_string), "Ingress Centralized");
            break;
        case BCM_L2_INGRESS_DIST:
            sal_snprintf(learn_mode_string, sizeof(learn_mode_string), "Ingress Distributed");
            break;
        case BCM_L2_EGRESS_DIST:
            sal_snprintf(learn_mode_string, sizeof(learn_mode_string), "Egress Distributed");
            break;
        case BCM_L2_EGRESS_CENT:
            sal_snprintf(learn_mode_string, sizeof(learn_mode_string), "Egress Centralized");
            break;
        case (BCM_L2_INGRESS_CENT | BCM_L2_LEARN_CPU):
            sal_snprintf(learn_mode_string, sizeof(learn_mode_string), "Ingress Centralized with CPU Learning");
            break;
        case (BCM_L2_EGRESS_CENT | BCM_L2_LEARN_CPU):
            sal_snprintf(learn_mode_string, sizeof(learn_mode_string), "Egress Centralized with CPU Learning");
            break;
    }
    ...
    void *user_data)
{
    ...
    if ((MACT_ENTRY_STRENGTH_ENCODE_SET(l2addr->learn_strength, l2addr->age_profile) == CONFIGURED_MACT_ENTRY_STRENGTH)
        || (MACT_ENTRY_STRENGTH_ENCODE_SET(l2addr->learn_strength, l2addr->age_profile) ==
            CONFIGURED_MACT_ENTRY_STRENGTH_2))
    {
        PRT_CELL_SET("%s", "S");
    }
    else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int mac_ivl = 0;
    int del_flags = 0;
    uint8 is_ivl_supported = dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_ivl_feature_support);
    ...
    if (all)
    {
        ...
        {
            ivl = is_ivl_supported ? TRUE : FALSE;
            svl = TRUE;
        }
    ...
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact))
    {
        fwd_mact_str = "FWD_MACT_DA/FWD_MACT_SA";
    }
    else
    {
        fwd_mact_str = (ivl == TRUE) ? "FWD_MACT_IVL" : "FWD_MACT";
    }
    if ((is_ivl_supported) && (ivl == TRUE))
    {
        mac_ivl = BCM_L2_DELETE_IVL;
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "vsi or vlan should be set. Use only one of the fields for SVL and only vsi for IVL if supported.\n");
    }
    if (is_ivl_supported)
    {
    ...
    /** For SVL can be configured either vlan or vsi for vsi, while for IVL(if supported) can be configured only vsi */
    if (svl_vsi_value != 0)
    {
        vsi = svl_vsi_value;
    }
    ...
    if ((nof_ports != 0) && (tgid < 0) && (l2_mc_group == 0))
    {
        BCM_PBMP_ITER(logical_ports, port)
        {
            ...
            /** Adding the l2 entry to MACT table */
 ...
 * If IVL is supported and vsi is provided, delete l2 entry from FWD_MACT_IVL.
 */
static shr_error_e
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (index = 0; index < nof_entries_to_delete; index++)
    {
        if (ENET_CMP_MACADDR(mac_addr, _soc_mac_all_zeroes))
        {
            ...
            /** Delete the l2 entry from MACT table */
...
static shr_error_e
sh_cmd_separate_fwd_learn_mact_not_supported(
    int unit,
    rhlist_t * test_list)
{
    SHR_FUNC_INIT_VARS(unit);
...
sh_sand_cmd_t sh_dnx_l2_cmds[] = {
    ...
    ,
    {"add", sh_dnx_l2_add_cmd, NULL, dnx_l2_add_options, &sh_dnx_l2_add_man, NULL, NULL,
     SH_CMD_CONDITIONAL, sh_cmd_separate_fwd_learn_mact_not_supported}
    ,
    ...
    ,
    {"delete", sh_dnx_l2_delete_cmd, NULL, dnx_l2_delete_options, &sh_dnx_l2_delete_man, NULL, NULL,
     SH_CMD_CONDITIONAL, sh_cmd_separate_fwd_learn_mact_not_supported}
    ,
    {"clear", sh_dnx_l2_clear_cmd, NULL, dnx_l2_clear_options, &sh_dnx_l2_clear_man, NULL, NULL,
     SH_CMD_CONDITIONAL, sh_cmd_separate_fwd_learn_mact_not_supported}
    ,
