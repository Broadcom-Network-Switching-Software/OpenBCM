    ...
    sh_sand_control_t * sand_control)
{
    ...
    rv = dnx_algo_global_out_lif_alloc_info_get(unit, global_lif, &alloc_info);
    if (rv != _SHR_E_NONE)
    {
        LOG_CLI((BSL_META_U(unit, "Something went wrong, dnx_algo_global_out_lif_alloc_info_get returned status %d\n"),
                 rv));
        SHR_EXIT();
    }
    ...
    LOG_CLI((BSL_META_U(unit, "\nGlobal lif 0x%x has the tag 0x%x with the parameters:\n"), global_lif, tag));
