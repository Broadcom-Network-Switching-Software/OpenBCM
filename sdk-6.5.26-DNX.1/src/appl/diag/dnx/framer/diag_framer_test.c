    ...
    void *user_data)
{
    ...
    test_sym_t *dm = (test_sym_t *) user_data;

    SHR_FUNC_INIT_VARS(unit);

    if (dm->test_flags & TEST_F_ALL)
    {
    ...
    args_t * a)
{
    ...
    sal_memset(&dm, 0, sizeof(test_sym_t));
    arg2 = ARG_GET(a);
    if (arg2 == NULL)
    {
        arg2 = "*";
        dm.test_flags |= TEST_F_ALL;
    }
    else
    {
    ...
    void *user_data)
{
    ...
    test_sym_t *dm = (test_sym_t *) user_data;
    test_pattern_t *mp = (test_pattern_t *) dm->cookie;
    uint32 pattern;
    ...
    args_t * a)
{
    ...
    if ((arg3) && (isint(arg3)))
    {
        dm.start_index = parse_integer(arg3);
        if ((arg4) && (isint(arg4)))
        {
            dm.count = parse_integer(arg4);
        }
        else
        {
            dm.count = 1;
