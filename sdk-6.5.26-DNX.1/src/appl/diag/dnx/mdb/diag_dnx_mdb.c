...
#define MDB_DIAG_STR_INSTANCE "instance"
...
{
    if (mdb_db_type >= DBAL_ENUM_FVAL_MDB_DB_TYPE_NONE)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_INFO_ADD("MAP (PMF-Direct) entry size is controlled by the %s soc property.", spn_PMF_MAPS_PAYLOAD_SIZE);
         ...
         dbal_physical_table_id_iter++)
    {
        ...
        else if (db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_KAPS)
        {
            ...
            SHR_IF_ERR_EXIT(mdb_lpm_get_ip_capacity
                            (unit, dbal_physical_table_id_iter, 0, NULL, &ipv4_capacity, &ipv6_capacity));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /*
     * KAPS cfg is only valid when there is only a single pair
     */
        ...
        if (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM)
        {
            ...
            for (ii = 0; ii < MDB_EM_ENTRY_ENCODING_EMPTY; ii++)
            {
                ...
                if (vmv_size != 0)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_em_get_entry_encoding_count(unit, dbal_physical_table_id, ii, &entry_count));
    ...
    sh_sand_control_t * sand_control)
{
...

    {
        ...
        else
        {
            ...
            if (add_cmd != 0)
            {
                SHR_IF_ERR_EXIT(mdb_lpm_entry_add(unit, dbal_physical_table_id, app_id, &entry, 0));
            ...
            {
                SHR_IF_ERR_EXIT(mdb_lpm_entry_delete(unit, dbal_physical_table_id, app_id, &entry, 0));
            ...
            {
                SHR_IF_ERR_EXIT(mdb_lpm_table_clear(unit, dbal_physical_table_id, app_id));
            }
#ifndef ADAPTER_SERVER_MODE
            ...
            else if (stats_cmd != 0)
            {
                ...
                uint8 result_type_iter;

                SHR_IF_ERR_EXIT(mdb_lpm_dbal_to_db(unit, dbal_physical_table_id, &db_idx));

                nof_result_types = dnx_data_mdb.kaps.db_info_get(unit, db_idx)->nof_result_types;

                PRT_TITLE_SET("MDB LPM Stats");
                PRT_INFO_ADD("The KAPS SDK capacity is an estimate based on the history of inserted routes");
                PRT_COLUMN_ADD("Database");
                PRT_COLUMN_ADD("Num Entries");
                PRT_COLUMN_ADD("Num Entries Iterator");
                PRT_COLUMN_ADD("KAPS SDK capacity");
                ...
                PRT_TITLE_SET("Result type info");
                ...
                for (result_type_iter = 0; result_type_iter < nof_result_types; result_type_iter++)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_lpm_get_ip_capacity
                                    (unit, dbal_physical_table_id, result_type_iter, &payload_size, &ipv4_capacity,
                                     &ipv6_capacity));

                    PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                    PRT_CELL_SET("%d", result_type_iter);
                    PRT_CELL_SET("%d", payload_size);
    ...
    rhlist_t * examples_list)
{
     ...
     */
    sal_memset(&cmd, 0, sizeof(cmd));
    sal_snprintf(cmd, sizeof(cmd), "%s %s=%d %s=%s %s=%d %s=%d %s=%s", MDB_DIAG_STR_ADD, MDB_DIAG_STR_INSTANCE,
                 DBAL_CORE_ALL, MDB_DIAG_STR_TABLE, table_name, MDB_DIAG_STR_KEY, key, MDB_DIAG_STR_PAYLOAD_SIZE,
                 payload_size, MDB_DIAG_STR_PAYLOAD, payload_str);
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, cmd), "");
...
static sh_sand_option_t dnx_mdb_direct_options[] = {
    ...
    {MDB_DIAG_STR_HIT_CLR, SAL_FIELD_TYPE_BOOL, "Perform a hitbit clear operation", "no"},
#endif
    {MDB_DIAG_STR_INSTANCE, SAL_FIELD_TYPE_INT32, "The instance (core/die) ID", "0"},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_memset(&physical_entry_iterator, 0, sizeof(physical_entry_iterator));
    physical_entry_iterator.core_id = core_die_id;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_INT32(MDB_DIAG_STR_INSTANCE, core_die_id);
    entry.core_id = core_die_id;
    ...
    else
    {
        ...
        if (add_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_direct_table_entry_add(unit, dbal_physical_table_id, app_id, &entry, 0));
        ...
        {
            SHR_IF_ERR_EXIT(mdb_direct_table_entry_delete(unit, dbal_physical_table_id, app_id, &entry, 0));
            LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "Delete operation completed successfully on direct table.\n")));
        }
        else if (dump_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_diag_direct_dump
                            (unit, core_die_id, dbal_physical_table_id, app_id, hw_cmd, sand_control));
        }
        else if (clear_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_direct_table_clear(unit, dbal_physical_table_id, app_id));
        }
        else
        {
            LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "An operation must be specified.\n")));
...
static sh_sand_man_t dnx_mdb_eedb_man = {
    ...
    .examples = "add table=EEDB1 key=100 payload_size=120 payload=0x12345678abcdef01030507 eedb_ll=0\n"
        "get table=EEDB1 instance=0 key=100 payload_size=120 eedb_ll=0\n"
#ifndef ADAPTER_SERVER_MODE
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_memset(&physical_entry_iterator, 0, sizeof(physical_entry_iterator));
    physical_entry_iterator.core_id = die_id;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_INT32(MDB_DIAG_STR_INSTANCE, die_id);
    entry.core_id = die_id;
    ...
    else
    {
        ...
        if (add_cmd != 0)
        {
            ...
            SHR_IF_ERR_EXIT(mdb_eedb_table_entry_add(unit, dbal_physical_table_id, app_id, &entry, entry_flags[0]));
        ...
        {
            SHR_IF_ERR_EXIT(mdb_eedb_table_entry_delete(unit, dbal_physical_table_id, app_id, &entry, 0));
            LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "Delete operation completed successfully on eedb table.\n")));
        }
        else if (dump_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_diag_eedb_dump(unit, die_id, dbal_physical_table_id, app_id, hw_cmd, sand_control));
        }
        else if (clear_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_eedb_table_clear(unit, dbal_physical_table_id, app_id));
        }
        else
        {
            LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "An operation must be specified.\n")));
    ...
    rhlist_t * examples_list)
{
     ...
     */
    sal_memset(&cmd, 0, sizeof(cmd));
    sal_snprintf(cmd, sizeof(cmd), "%s %s=%d %s=%s %s=0x%x %s=%d %s=0x%x %s=%d", MDB_DIAG_STR_ADD,
                 MDB_DIAG_STR_INSTANCE, DBAL_CORE_ALL, MDB_DIAG_STR_TABLE, table_name, MDB_DIAG_STR_KEY, key,
                 MDB_DIAG_STR_PAYLOAD_SIZE, payload_size, MDB_DIAG_STR_PAYLOAD, payload, MDB_DIAG_STR_APP_ID, app_id);
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, cmd), "");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        if (dump_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_diag_em_dump(unit, core_die_id, dbal_physical_table_id, app_id, hw_cmd, sand_control));
        }
        else if (clear_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_em_table_clear(unit, dbal_physical_table_id, app_id));
        }
        else
        {
            /*
            ...
            if (add_cmd != 0)
            {
                SHR_IF_ERR_EXIT(mdb_em_entry_add(unit, dbal_physical_table_id, app_id, &entry, 0));
            ...
            {
                SHR_IF_ERR_EXIT(mdb_em_entry_delete(unit, dbal_physical_table_id, app_id, &entry, 0));
