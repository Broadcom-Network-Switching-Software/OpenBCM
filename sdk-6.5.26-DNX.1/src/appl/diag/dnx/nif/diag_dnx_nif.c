...
shr_error_e
sh_dnx_is_flr_supported(
    int unit,
    rhlist_t * test_list)
{
    int pm_index, is_supported = 0;
    SHR_FUNC_INIT_VARS(unit);

    for (pm_index = 0; pm_index < dnx_data_nif.eth.total_nof_ethu_pms_in_device_get(unit); ++pm_index)
    {
        is_supported |= dnx_data_nif.eth.pm_properties_get(unit, pm_index)->flr_support;
    }
    if (!is_supported)
    {
        SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
...
sh_sand_cmd_t sh_dnx_nif_cmds[] = {
    ...
    {"flr",      sh_dnx_nif_flr_cmd,      NULL,                 sh_dnx_nif_flr_options,      &sh_dnx_nif_flr_man, NULL, NULL, SH_CMD_NO_XML_VERIFY, sh_dnx_is_flr_supported},
    {NULL},
};
