...
#include "../../sand/diag_sand_framework_internal.h"
/*
 * }
 */
...
/**
 * \brief - Keyword for custom verify indication
 */
#define DNX_DIAG_EXPORT_OPTION_CUSTOM_VERIFY   "CuSToM_VeRiFY"
    ...
    export_execution_info_t * info)
{
     ...
     */
    /* *INDENT-OFF* */
    cmd_buffer_size = sal_strnlen(info->cmd,sizeof(info->cmd))
            + sal_strnlen(" core=x",SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
            + sal_strnlen(" file=",SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
            + sal_strnlen(info->out_file,SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
            + sal_strnlen(" xpath=",SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
            + sal_strnlen(info->category,sizeof(info->category))
            + sal_strnlen(" gold new",SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
            + 1;
    /* *INDENT-ON* */
    ...
    sal_memset(full_cmd_buffer, 0, cmd_buffer_size);
    if (info->core_mode == EXPORT_CORE_MODE_SBC)
    {
         ...
         */
        LOG_CLI_EX("Executing: '%s' %s%s%s\n", full_cmd_buffer, EMPTY, EMPTY, EMPTY);
    ...
    char **out_file_p)
{
     ...
     */
    path_len = sal_strnlen(sdk_path, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 1 /* '/' */  +
        sizeof(DNX_DIAG_EXPORT_DEFAULT_DUMP_DIR) + 1 /* '/' */  +
        sizeof(DNX_DIAG_EXPORT_DEFAULT_OUTPUT_FILENAME) + 1 /* '_' */  + sal_strnlen(timestamp, sizeof(timestamp)) + 1  /* '.' 
                                                                                                                         */  +
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int overwrite = 1, custom_verify = 0;
    ...
    SH_SAND_IS_PRESENT(DNX_DIAG_EXPORT_OPTION_CUSTOM_VERIFY, custom_verify);
    /*
     * Validate input
     */
    ...
    if (overwrite)
    {
        char located_output_file[RHFILE_MAX_SIZE];

        /** Determine the file path*/
#if !defined(NO_FILEIO)
        if (info->create_gold == TRUE)
        {
            int path_flags = 0;
            if (custom_verify == TRUE)
            {
                utilex_prime_flag_set(&path_flags, SH_CMD_CUSTOM_XML_VERIFY);
            }
            SHR_CLI_EXIT_IF_ERR(sh_sand_gold_path(unit, path_flags, info->out_file, located_output_file),
                                "Gold result file:'%s' cannot be obtained\n", info->out_file);
        }
        else
#endif /* #if !defined(NO_FILEIO) */
        {
            sal_strncpy(located_output_file, info->out_file, RHFILE_MAX_SIZE - 1);
        }

        /** Verify the file writability and remove it for the space to create a new one.*/
        dbx_file_remove(located_output_file);
    }
    /*
    ...
    while (cur_cmd != NULL)
    {
        ...
        if (sal_strnlen(info->cmd, sizeof(info->cmd)) > 0)
        {
             ...
             * Exit immediately when error happens for the convenience of reporting the command error quickly.
             */
            SHR_IF_ERR_EXIT_WITH_LOG(dnx_export_cmd_execute(unit, info),
    ...
    /** All command should execute successfully when getting here. */
