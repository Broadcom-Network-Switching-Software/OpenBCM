    ...
    for (pool = 0; pool < dnx_data_trunk.parameters.nof_pools_get(unit); ++pool)
    {
        ...
        for (group = 0; group < max_nof_groups_in_pool; ++group)
        {
            ...
            DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
            {
                ...
                if (utilex_bitstream_test_bit(&allocation_info.core_bitmap, core))
                {
                    PRT_CELL_SET("%d", pp_port);
    ...
    else
    {
        ...
        if (force != DNX_COMMAND_NONE)
        {
            DNXCMN_CORES_ITER(unit, in_core, core)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "The LAG (pool=%d and group=%d) has no pp_port allocated on core %d!\n",
                                 pool, group, core);
                }
            }
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get
                        (unit, DBAL_FIELD_TYPE_DEF_PP_ASIC_BLOCK, INVALID_ENUM, &asic_block));
        while (asic_block != INVALID_ENUM)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get
                                (unit, DBAL_FIELD_TYPE_DEF_PP_ASIC_BLOCK, asic_block, &asic_block));
                continue;
            }
