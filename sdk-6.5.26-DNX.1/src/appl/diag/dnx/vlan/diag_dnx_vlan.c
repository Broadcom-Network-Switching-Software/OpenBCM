    ...
    else
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            BCM_PBMP_ITER(list[i].port_bitmap, port)
            {
                ...
                port_name_len = sal_strnlen(port_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((sal_strncasecmp(action, "delete", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0) &&
        (sal_strncasecmp(action, "add", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0) &&
        (sal_strncasecmp(action, "get", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0) &&
        (sal_strncasecmp(action, "clear", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
        && (sal_strncasecmp(action, "show", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "%s: ERROR: Invalid gport subcommand\n", action);
    }
    if ((sal_strncasecmp(action, "clear", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
        && (sal_strncasecmp(action, "show", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0))
    {
        /*
    ...
    if (sal_strncasecmp(action, "show", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        int i, count;
    ...
    else
    {
        BCM_PBMP_ITER(logical_ports, port)
        {
            ...
            else if (sal_strncasecmp(action, "add", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
                SHR_IF_ERR_EXIT(bcm_vlan_gport_add(unit, vlan, port, 0));
            }
            else if (sal_strncasecmp(action, "get", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
                SHR_IF_ERR_EXIT(bcm_vlan_gport_get(unit, vlan, port, &is_untagged));
            }
            else if (sal_strncasecmp(action, "clear", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
...
sh_sand_cmd_t sh_dnx_vlan_cmds[] = {
     ...
     */
    {"show", sh_dnx_vlan_show_cmd, NULL, vlan_show_options, &vlan_show_man, NULL, NULL, SH_CMD_NO_XML_VERIFY},
