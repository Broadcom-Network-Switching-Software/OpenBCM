...
#include <src/soc/dnx/dbal/dbal_hl_access.h>
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (to_print)
    {
        ...
        for (ii = 0; ii < DBAL_NOF_HL_ACCESS_TYPES; ii++)
        {
            if ((ii == DBAL_HL_ACCESS_SW) || (ii == DBAL_HL_ACCESS_UNMAPPED))
            {
                continue;
            }
            for (jj = 0; jj < l2p_hl_info[ii].num_of_access_fields; jj++)
            {
                ...
                {
                    is_memory = 1;
                    regmem = l2p_hl_info[ii].l2p_fields_info[jj].memory[0];
                    alias_regmem = l2p_hl_info[ii].l2p_fields_info[jj].alias_memory;
                }
                else if (ii == DBAL_HL_ACCESS_REGISTER)
                {
                    is_memory = 0;
                    regmem = l2p_hl_info[ii].l2p_fields_info[jj].reg[0];
                    alias_regmem = l2p_hl_info[ii].l2p_fields_info[jj].alias_reg;
                }
                else
                {
                ...
                memory_name = dbal_hl_access_regmem_name_get(unit, table, regmem, is_memory);
                if (alias_regmem != DBAL_HL_ACCESS_HW_ENTITY_INVALID)
                {
                    alias_name = dbal_hl_access_regmem_name_get(unit, table, alias_regmem, is_memory);
                    diag_dbal_offset_encode_info_to_str(unit,
                                                        &(l2p_hl_info[ii].l2p_fields_info[jj].alias_data_offset_info),
                                                        info_string);
                    has_alias = TRUE;
                }

                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", dbal_field_with_inst_to_string(unit,
                                                                  l2p_hl_info[ii].l2p_fields_info[jj].field_id,
                                                                  l2p_hl_info[ii].l2p_fields_info[jj].inst_idx,
                                                                  field_str));
                ...
                if (has_alias)
                {
                    if (sal_strncasecmp(info_string, "", sizeof(info_string)) == 0)
                    {
                ...
                if ((l2p_hl_info[ii].l2p_fields_info[jj].hw_field != 0) &&
                    (l2p_hl_info[ii].l2p_fields_info[jj].hw_field != DBAL_HL_ACCESS_HW_FIELD_INVALID))
                {
                    int field_offset, field_len;
                    char field_name[DBAL_MAX_SHORT_STRING_LENGTH];
                    int regmem = is_memory ? l2p_hl_info[ii].l2p_fields_info[jj].memory[0] :
                        l2p_hl_info[ii].l2p_fields_info[jj].reg[0];

                    SHR_IF_ERR_EXIT(dbal_hl_access_field_info_get_per_access(unit, table,
                                                                             regmem,
                                                                             l2p_hl_info[ii].
                                                                             l2p_fields_info[jj].hw_field,
                                                                             DBAL_HL_NEW_ACCESS_SCOPE_GLOBAL_FIELD,
                                                                             is_memory, &field_offset, &field_len,
                                                                             field_name));
                    PRT_CELL_SET("%s, (%d, %d)", field_name, field_offset, field_len);
                }
                else
                {
                    PRT_CELL_SET(" - ");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (to_print)
    {
        int entry_offset_formula_set = 0;
        PRT_TITLE_SET("HL access logical to physical info");
        PRT_INFO_ADD("Default entry: %s",
                     access_info->is_default_non_standard ? "Non-standard" : "Standard (all zero)");
        ...
        for (ii = 0; ii < l2p_hl_info[DBAL_HL_ACCESS_MEMORY].num_of_access_fields; ii++)
        {
            diag_dbal_offset_encode_info_to_str(unit,
                                                &(l2p_hl_info[DBAL_HL_ACCESS_MEMORY].
                                                  l2p_fields_info[ii].entry_offset_info), info_string);
            if (sal_strncasecmp(info_string, "", sizeof(info_string)) != 0)
            {
                PRT_COLUMN_ADD("Entry Offset");
                entry_offset_formula_set = 1;
                break;
            }
        }
        ...
        for (ii = 0; ii < DBAL_NOF_HL_ACCESS_TYPES; ii++)
        {
            ...
            for (jj = 0; jj < l2p_hl_info[ii].num_of_access_fields; jj++)
            {
                ...
                {
                    memory_name =
                        dbal_hl_access_regmem_name_get(unit, table, l2p_hl_info[ii].l2p_fields_info[jj].memory[0], 1);
                }
                else
                {
                    LOG_ERROR_EX(BSL_LOG_MODULE, "\n access type not parsed %d %s %s %s\n", ii, EMPTY, EMPTY, EMPTY);
                ...
                if ((l2p_hl_info[ii].l2p_fields_info[jj].hw_field != 0) &&
                    (l2p_hl_info[ii].l2p_fields_info[jj].hw_field != DBAL_HL_ACCESS_HW_FIELD_INVALID))
                {
                    field_name = dbal_hl_access_field_name_get(unit, table, &l2p_hl_info[ii].l2p_fields_info[jj]);
                ...
                if (entry_offset_formula_set)
                {
                    diag_dbal_offset_encode_info_to_str(unit, &(l2p_hl_info[ii].l2p_fields_info[jj].entry_offset_info),
                                                        info_string);
                    if (sal_strncasecmp(info_string, "", sizeof(info_string)) == 0)
                    {
                        sal_snprintf(info_string, CMD_MAX_STRING_LENGTH, "%s", "default (access id)");
                    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (jj = 0; jj < l2p_hl_info[DBAL_HL_ACCESS_SW].num_of_access_fields; jj++)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("%s", dbal_field_with_inst_to_string(unit,
                                                          l2p_hl_info[DBAL_HL_ACCESS_SW].l2p_fields_info[jj].field_id,
                                                          l2p_hl_info[DBAL_HL_ACCESS_SW].l2p_fields_info[jj].inst_idx,
                                                          field_str));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADD("Asymmetric Mapping");
    ...
    {
        PRT_CELL_SET("YES %s",
                     (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_SW_RESULT_TYPE_NO_ACCESS) ? "(No access)" :
                      ""));
    }
    ...
    PRT_CELL_SET("%s",
                 (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_NONE_DIRECT_FORCE_OPTIMIZED) ? "Optimized" :
                  "Regular"));
    ...
    PRT_CELL_SET("%s",
                 (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED) ? "Optimized" :
                  "Regular"));
    PRT_CELL_SET("%s",
    ...
    PRT_CELL_SET("%s", (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_ASYMMETRIC) ? "YES" : "NO"));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char phys_table_rt[DBAL_MAX_STRING_LENGTH] = "";
    ...
    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
    {
        ...
        {
            if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_LART_SUPPORT))
            {
                sal_snprintf(phys_table_rt, DBAL_MAX_STRING_LENGTH - 1, ", LART index=%d",
                             table->multi_res_info[result_idx].phys_table_rt);
            }
            PRT_TITLE_SET("result type=%s (hw %d), payload size=%d bits%s",
                          table->multi_res_info[result_idx].result_type_name,
                          table->multi_res_info[result_idx].result_type_hw_value[0],
                          table->multi_res_info[result_idx].entry_payload_size, phys_table_rt);

        }
    }
    else
    {
        /** Title set is mandatory, set to empty string  */
    ...
    {
        results_info = &table->multi_res_info[result_idx].results_info[ii];
        dbal_field_with_inst_to_string(unit, results_info->field_id, results_info->inst_idx, field_string);

        SHR_IF_ERR_EXIT(dbal_fields_field_types_info_get(unit, results_info->field_id, &field_type_info));

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("%s", field_string);
        PRT_CELL_SET("%s", field_type_info->name);
        ...
        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
        {
            if (results_info->field_id == DBAL_FIELD_RESULT_TYPE)
            {
                ...
                char_count =
                    sal_snprintf(buf_ptr, CMD_MAX_STRING_LENGTH - 1, "%d",
                                 table->multi_res_info[result_idx].result_type_hw_value[0]);
                buf_ptr += char_count;
                ...
                for (jj = 1; jj < table->multi_res_info[result_idx].result_type_nof_hw_values; jj++)
                {
                    char_count =
                        sal_snprintf(buf_ptr, CMD_MAX_STRING_LENGTH - 1, ", %d",
                                     table->multi_res_info[result_idx].result_type_hw_value[jj]);
                    buf_ptr += char_count;
                }
...
/** dump table access related information */
static shr_error_e
diag_dbal_logical_table_access_info_dump(
    int unit,
    dbal_tables_e table_id,
    CONST dbal_logical_table_t * table,
    sh_sand_control_t * sand_control)
{
        ...
        for (ii = 0; ii < table->nof_result_types; ii++)
        {
            ...
            if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
            {
                    ...
                    (table->hl_mapping_multi_res[ii].l2p_hl_info[DBAL_HL_ACCESS_REGISTER].num_of_access_fields > 0))
                {
                    ...
                    SHR_IF_ERR_EXIT(diag_dbal_hard_logic_direct_hw_info_dump
                                    (unit, table, &(table->hl_mapping_multi_res[ii]), sand_control));
                    LOG_CLI((BSL_META("\n")));
                }
            ...
            else if (table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT)
            {
                SHR_IF_ERR_EXIT(diag_dbal_hard_logic_tcam_hw_info_dump
                                (unit, table, &(table->hl_mapping_multi_res[0]), sand_control));
                LOG_CLI((BSL_META("\n")));
            }
            else
            {
    ...
    else if (table->access_method == DBAL_ACCESS_METHOD_PEMLA)
    {
        ...
        if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
        {
            ...
            LOG_CLI((BSL_META("  Access Type: %s\n"),
                     dbal_hl_access_type_to_string(unit, table->pemla_mapping.access_type)));
            LOG_CLI((BSL_META("  %s ID %d\n"),
                     dbal_hl_access_type_to_string(unit, table->pemla_mapping.access_type), table->app_id));

            LOG_CLI((BSL_META("  Result fields: \n")));
            for (ii = 0; ii < table->multi_res_info[0].nof_result_fields; ii++)
            {
                field_info = &table->multi_res_info[0].results_info[ii];
                LOG_CLI((BSL_META("   %s mapped to HW ID %02d\n"),
                         dbal_field_with_inst_to_string(unit,
                                                        field_info->field_id,
                                                        field_info->inst_idx,
                                                        field_str), table->pemla_mapping.result_fields_mapping[ii]));
            }
        }
        else
        {
    ...
    else if (table->access_method == DBAL_ACCESS_METHOD_MDB)
    {
        ...
        for (jj = 0; jj < table->nof_physical_tables; jj++)
        {
            ...
            /*
             * Display generic name for EEDB table (without index)
             */
            if (table->physical_db_id[jj] == DBAL_PHYSICAL_TABLE_EEDB_1)
            {
                is_remove_index = 1;
            }
            LOG_CLI((BSL_META("\tPhysical table: %s"),
                     dbal_physical_table_to_string_2(unit, table->physical_db_id[jj], is_remove_index)));

            if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_LART_SUPPORT))
            {
                LOG_CLI((BSL_META(", LART supported\n")));
            }
            else
            {
                LOG_CLI((BSL_META("\t\n")));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (table->max_capacity != 0)
    {
        ...
        {
            LOG_CLI((BSL_META("\tEntries Status: Max Capacity (May be less in case other tables are mapped "
                              "to the same physical tables): %d, Committed %d, \n"), table->max_capacity, nof_entries));
        }
    }
    ...
    for (ii = 0; ii < table->nof_result_types; ii++)
    {
        if (table->multi_res_info[ii].is_disabled)
        {
            continue;
        }
    ...
    if (print_access)
    {
        SHR_IF_ERR_EXIT(diag_dbal_logical_table_access_info_dump(unit, table_id, table, sand_control));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dbal_mdb_phy_table_is_valid(unit, phy_table_id));
    ...
    PRT_CELL_SET("%s", dbal_core_mode_to_string(unit, dbal_mdb_phy_table_core_mode_get(unit, phy_table_id)));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (field_type_info->nof_illegal_values != 0)
    {
        ...
        for (ii = 0; ii < field_type_info->nof_illegal_values; ii++)
        {
            ...
            {
                char_count =
                    sal_snprintf(current_loc, CMD_MAX_STRING_LENGTH - 1, "0x%x", field_type_info->illegal_values[ii]);
                current_loc += char_count;
            }
            else
            {
                char_count =
                    sal_snprintf(current_loc, CMD_MAX_STRING_LENGTH - 1, "%d", field_type_info->illegal_values[ii]);
                current_loc += char_count;
            }
            if (ii != (field_type_info->nof_illegal_values - 1))
            {
                char_count = sal_snprintf(current_loc, CMD_MAX_STRING_LENGTH - 1, ", ");
                current_loc += char_count;
            }
        }
    ...
    if (field_type_info->nof_defines_values > 0)
    {
        ...
        {
            LOG_CLI((BSL_META("\tName %s,\tValue %d\n"), field_type_info->defines_info[ii].name,
