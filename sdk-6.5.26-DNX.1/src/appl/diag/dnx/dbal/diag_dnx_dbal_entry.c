...
extern shr_error_e dnx_algo_res_dbal_alloc(
    int unit,
    int core_id,
...
extern shr_error_e dnx_algo_res_dbal_is_allocated(
    int unit,
    int core_id,
    int sub_resource_index,
    int index,
...
extern shr_error_e dnx_algo_res_dbal_free(
    int unit,
    int core_id,
    int sub_resource_index,
    int index,
    ...
    dbal_tables_e table_id)
{
    ...
    sal_snprintf(buffer, DBAL_MAX_PRINTABLE_BUFFER_SIZE - 1, "Example: dbal ENTry Get TaBLe=%s AccessId=<value>",
    ...
    uint8 key_field_only)
{
    ...
    /*
     * Result type check is relevant only for multiple RT tables and for operations that request the result fields
     * (commit, update...) 
     */
    if ((table->nof_result_types > 1) && (!key_field_only))
    {
        if (res_type == DBAL_RESULT_TYPE_NOT_INITIALIZED)
        {
    ...
    if (!(SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE)) || key_field_only)
    {
    ...
    char *cmd)
{
    ...
    if (((dbal_table_is_in_lif(unit, table)) || (dbal_table_is_out_lif(table))))
    {
        char *rt_str;
        SH_SAND_GET_STR("result_type", rt_str);
        DBAL_ENTRY_RESULT_TYPE_GET(unit, entry_handle_id, res_type, 1, rt_str);
    }
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        ...
        field_value = *SH_SAND_ARG_ARRAY_DATA(sand_arg);
        ...
        if (is_key == TRUE)
        {
            ...
            else if (table->allocator_field_id == field_id)
            {
                uint8 is_allocated;

                /*
                 * Check if the resource is already allocated by any table
                 */
                SHR_IF_ERR_EXIT(dnx_algo_res_dbal_is_allocated(unit, core_id,
                                                               0, field_value, table_id, res_type, field_id, TRUE,
                                                               &is_allocated));
                /*
                 * If not, we can allocate
                 */
                if (!is_allocated)
                {
                    SHR_IF_ERR_EXIT(dnx_algo_res_dbal_alloc
                                    (unit, core_id, 0, field_value, table_id, res_type, field_id));
                    LOG_CLI((BSL_META("Resource for field %s was allocated with entry insertion\n"),
                             dbal_field_to_string(unit, field_id)));
                }
                else
                {
                    /*
                     * If the resource is already allocated, make sure it is by this table and RT
                     */
                    SHR_IF_ERR_EXIT(dnx_algo_res_dbal_is_allocated(unit, core_id,
                                                                   0, field_value, table_id, res_type, field_id,
                                                                   FALSE, &is_allocated));
                    if (!is_allocated)
                    {
                        /*
                         * Next line can be removed for debugging, so we can commit entry no matter if already
                         * allocated
                         */
                        /*
                         * SHR_IF_ERR_EXIT(dnx_algo_res_dbal_free(unit, core_id, 0, field_value, table_id, res_type,
                         * field_id));
                         */
                        LOG_CLI((BSL_META
                                 ("Resource value %d for allocator field %s is already allocated by another table\n"),
                                 field_value, dbal_field_to_string(unit, field_id)));
                        SHR_EXIT();
                    }
                }
            }
    ...
    uint32 entry_handle_id)
{
    ...
    /** check if the requested table entry support priority */
    ...
    if (priority_en)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_PRIORITY, &attr_val));
        LOG_CLI((BSL_META("Priority=%d \n"), attr_val));
    }
    if (DBAL_TABLE_IS_TCAM(table))
    {
        /** For TCAM tables - add is_valid indication */
        uint32 attr_val;
        SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_INVALID, &attr_val));
        LOG_CLI((BSL_META("IsValid=%s \n"), attr_val ? "FALSE" : "TRUE"));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (is_clear_hitbit)
    {
        ...
        clear_hitbit_flag = DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR;
    }
...
*/
    ...
    else
    {
        ...
        if ((result_type_set == 0) && ((dbal_table_is_in_lif(unit, table)) || (dbal_table_is_out_lif(table))))
        {
            SHR_IF_ERR_EXIT(diag_dbal_entry_lif_res_type_set(unit, entry_handle_id, lif_val, NULL));
        }

        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_HITBIT_EN))
        {
            SHR_IF_ERR_EXIT(dbal_entry_attribute_request(unit, entry_handle_id, DBAL_ENTRY_ATTR_HITBIT,
                                                         (DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY |
                                                          DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY |
                                                          DBAL_ENTRY_ATTR_FLAG_HIT_GET | clear_hitbit_flag)));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int allocator_field_val = 0, result_type_set = 0;
    int is_hw_only;
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        field_value = SH_SAND_ARG_ARRAY_DATA(sand_arg)[0];
        field_id = SH_SAND_GET_ID(sand_arg);
        SHR_IF_ERR_EXIT(dbal_tables_field_is_key_get(unit, table_id, field_id, &is_key));
        SHR_IF_ERR_EXIT(dbal_tables_field_is_key_const_get(unit, table_id, field_id, &is_key_const));
        if ((is_key == TRUE) && (is_key_const == FALSE))
        {
            if (field_id == table->allocator_field_id)
            {
                allocator_field_val = field_value;
            }
            else if (field_id == DBAL_FIELD_CORE_ID)
            {
                core_id = field_value;
            }
            /*
    ...
    if ((result_type_set == 0) && ((dbal_table_is_in_lif(unit, table)) || (dbal_table_is_out_lif(table))))
    {
        SHR_IF_ERR_EXIT(diag_dbal_entry_lif_res_type_set(unit, entry_handle_id, allocator_field_val, &result_type));
    }
    ...
    else
    {
        ...
        /*
         * If the table has an allocator field, we want to free the allocated resource
         */
        if (table->allocator_field_id != DBAL_FIELD_EMPTY)
        {
            uint8 is_allocated;
            SHR_IF_ERR_EXIT(dnx_algo_res_dbal_is_allocated(unit, core_id,
                                                           0, allocator_field_val, table_id, result_type,
                                                           table->allocator_field_id, 0, &is_allocated));
            if (!is_allocated)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Trying to clear a key that is not "
                             "allocated. Allocator field:%s, value: %d\n",
                             dbal_field_to_string(unit, table->allocator_field_id), allocator_field_val);
            }
            SHR_IF_ERR_EXIT(dnx_algo_res_dbal_free
                            (unit, core_id, 0, allocator_field_val, table_id, result_type, table->allocator_field_id));
            LOG_CLI((BSL_META("Resource for allocator field %s was freed with entry deletion\n"),
...
static sh_sand_man_t diag_dbal_entry_commit_man = {
    .brief = "Set and commit a table entry.\n"
        "If the key field is an allocator, diag handles allocation, only if the resource is not already allocated "
        "by an other table, or an other result type in the same table (for RT dependent allocators)",
    .full = NULL,
    ...
    .examples =
        ...
        "TaBLe=fwd_mact Result_Type=FWD_MACT_RESULT_DEST__STAT MAC_STRENGTH=1 VSI=1 L2_MAC=12:12:12:12:12:12 HwOnly\n"
...
static sh_sand_man_t diag_dbal_entry_update_man = {
    ...
    .examples = "table=IN_AC_C_VLAN_DB Priority=1 VLAN_DOMAIN=1 C_VID=2 IN_LIF=3\n"
        "table=PRT_VIRTUAL_PORT_TCAM AccessId=1 PRT_KBR_INFO_HALF=2 CORE_ID=0\n"
        "TaBLe=fwd_mact Result_Type=FWD_MACT_RESULT_DEST__STAT MAC_STRENGTH=2 VSI=1 L2_MAC=12:12:12:12:12:12 hwonly\n"
        "TaBLe=IPV4_RPF_FIB VRF=1 IPV4=2 NoPayload"
};
...
static sh_sand_man_t diag_dbal_entry_get_man = {
    ...
    .examples = "table=IN_AC_C_VLAN_DB VLAN_DOMAIN=1 C_VID=2 IN_LIF=4 ClearHitBit\n"
        "table=PRT_VIRTUAL_PORT_TCAM AccessId=1 CORE_ID=0\n" "TaBLe=fwd_mact VSI=1 L2_MAC=12:12:12:12:12:12 hwonly\n"
...
static sh_sand_man_t diag_dbal_entry_clear_man = {
    ...
    .examples = "table=IN_AC_C_VLAN_DB VLAN_DOMAIN=1 C_VID=2 IN_LIF=4\n"
        ...
        "TaBLe=fwd_mact Result_Type=FWD_MACT_RESULT_DEST__STAT VSI=1 L2_MAC=12:12:12:12:12:12 hwonly\n"
        "TaBLe=IPV4_RPF_FIB VRF=1 IPV4=2"
};
