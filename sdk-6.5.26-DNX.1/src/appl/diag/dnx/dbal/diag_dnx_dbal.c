...
/** this is not the actual size, just an estimation */
 ...
 **********************************************************************************************************************/
/*
 * enum table for dynamic fields support. size is doubled for supporting both short
 * enum name and full enum name.
 */
    ...
    uint8 *is_full_match)
{
    ...
    *is_full_match = 0;
    ...
    {
        SHR_SET_CURRENT_ERR((dbal_field_types_info_get(unit, iter, &field_type_info)));
        if (SHR_GET_CURRENT_ERR() == _SHR_E_UNAVAIL)
        {
            continue;
        }

        if (sal_strncasecmp(substr_match, field_type_info->name, sizeof(field_type_info->name)) == 0)
        {
            *is_full_match = 1;
            *field_type = iter;
            break;
    ...
    char *field_type_name)
{
    ...
    LOG_CLI((BSL_META("\tOptional field types are: \n")));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (ii = DBAL_FIELD_TYPE_DEF_EMPTY + 1; ii < DBAL_NOF_FIELD_TYPES_DEF; ii++)
    {
        ...
        rv = dbal_field_types_info_get(unit, ii, &field_type_info);
        ...
        else
        {
            ...
            if (ii == nof_fields)
            {
                ...
                for (jj = 0; jj < nof_fields_in_mem; jj++)
                {
                    ...
                    if (sal_strncasecmp(field_name, "ecc", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
                    {
                        SHR_IF_ERR_EXIT(dbal_tables_next_table_get
    ...
    int *is_full_match)
{
    ...
    *is_full_match = 0;
    ...
    {
        if (!sal_strncasecmp
            (substr_match, dbal_hw_entity_group_to_string(unit, group_index),
    ...
    sh_sand_control_t * sand_control)
{
    ...
    first_group_id = (dbal_tables_e) (*group_id);
    ...
    {
        (*group_id) = (dbal_hw_entity_group_e) first_group_id;
        SHR_EXIT();
    }
    ...
    uint8 *is_full_match)
{
    ...
    *is_full_match = 0;
    ...
    for (iter = curr_phy_table; iter < DBAL_NOF_PHYSICAL_TABLES; iter++)
    {
        ...
        if (sal_strncasecmp(substr_match, curr_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            *is_full_match = 1;
            *phy_table = iter;
            break;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADD("Payload Size (Bytes)");
    PRT_COLUMN_ADD("Total Payload Size (KBytes)");
    PRT_COLUMN_ADD("Total Key Size (KBytes)");

    for (table_iter = 0; table_iter < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); table_iter++)
    {
        int sw_hash_size_bytes = 0;
        SHR_IF_ERR_EXIT(dbal_tables_table_get(unit, table_iter, &table));
        ...
        {
            sw_hash_size_bytes = (DBAL_HASH_TABLE_ENTRY_SIZE * sw_nof_entries);
            PRT_CELL_SET("%d", sw_hash_size_bytes / 1000);
        }
        ...
        {
            size_real_tables += table->sw_access_info.sw_payload_length_bytes * sw_nof_entries + sw_hash_size_bytes;
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
     ...
     * Get parameters
     */
    SH_SAND_GET_BOOL("MEMory", is_mem);
    SH_SAND_GET_BOOL("REGister", is_reg);

    if ((is_mem == 0) && (is_reg == 0))
    {
        is_mem = is_reg = 1;
    }
    ...
    PRT_COLUMN_ADD("Table Name");
    PRT_COLUMN_ADD("Db Type");
    PRT_COLUMN_ADD("IsBinded");
    PRT_COLUMN_ADD("Db Id");
    ...
    {
        pemladrv_db_info *pem_mem_info_ptr = &pemla_mem_info[pem_mem_index];

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("%s", pem_mem_info_ptr->db_name);
        PRT_CELL_SET("Memory");

        PRT_CELL_SET("NotBinded");

        PRT_CELL_SET("%d", pem_mem_info_ptr->db_id);

        PRT_CELL_SET("%d", pem_mem_info_ptr->nof_fields);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    pemladrv_reg_info *pemla_reg_info = NULL;
    pemladrv_db_info *pemla_mem_info = NULL;
    int pemla_reg_info_size, pemla_mem_info_size;
    int pem_reg_index, pem_field_index, pem_mem_index;
    ...
    {
        if (sal_strncasecmp
            (pemla_reg_info[pem_reg_index].reg_name, table_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            break;
        }
    }
    ...
    {
        if (sal_strncasecmp
            (pemla_mem_info[pem_mem_index].db_name, table_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            break;
        }
    }

    if ((pem_reg_index == pemla_reg_info_size) && (pem_mem_index == pemla_mem_info_size))
    {
        /*
         * Table info not found in Pemla driver nothing to display.
         */
        LOG_CLI((BSL_META("Table info not found in Pemla driver for table %s nothing to display\n"), table_name));
        SHR_EXIT();
    ...
    {
        pemladrv_db_field_info *pem_mem_field_info;
        ...
        {
            pem_mem_field_info = &pemla_mem_info[pem_mem_index].ptr[pem_field_index];

            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%s", "Unknown");
            PRT_CELL_SET("%d", pem_mem_field_info->field_id);
            PRT_CELL_SET("%d", pem_mem_field_info->field_size_in_bits);
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        if (sal_strncasecmp
            (pemla_reg_info[pem_reg_index].reg_name, reg_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            break;
        }
    }
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_full_reg_write(unit, -1, pem_reg_index, -1, value));
...
static sh_sand_option_t dnx_dbal_access_pemla_dump_options[] = {
    {"REGister", SAL_FIELD_TYPE_BOOL, "dump Pemla Registers only", "false"},
    {"MEMory", SAL_FIELD_TYPE_BOOL, "dump Pemla Memories only", "false"},
    {NULL}
};
...
static sh_sand_man_t dnx_dbal_access_mdb_shadow_man = {
    ...
    .full = "MDB reads from Shadow by default. Setting the skip flag, \n"
        "will lead to read from HW only. No effect on LPM tables " "(read only from shadow)",
...
static sh_sand_man_t dnx_dbal_access_pemla_dump_man = {
    .brief = "Dump information about PEMLA tables",
    .full = "Dump information about PEMLA tables (memories and/or registers).\n"
        "Use memory/register argument to dump memories or registers only.\n" "Both are dumped by default.",
    .examples = "register\n" "memory"
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    CONST dbal_logical_table_t *table;
    uint32 sw_nof_entries = 0;
    uint32 payload_total_size_bytes = 0, size_hash_bytes = 0;
    uint32 curr_table_sw_size = 0;
    uint32 total_bitmap_alloc = 0, total_sw_alloc = 0, total_hash_alloc = 0;
    uint32 iterator_bitmap_size_bytes = 0;
    char *access_method_str;
    uint32 min_size_to_display = 0;
    dbal_access_method_e access_method;
    int is_basic_info_printed;

    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);

    SH_SAND_GET_UINT32("size", min_size_to_display);
    SH_SAND_GET_STR("access", access_method_str);

    PRT_TITLE_SET("All SW tables info");

    PRT_COLUMN_ADD("Table Name");
    PRT_COLUMN_ADD("Access Method");
    PRT_COLUMN_ADD("Table type");
    PRT_COLUMN_ADD("Memory Type");
    PRT_COLUMN_ADD("Size (Bytes)");

    for (table_iter = 0; table_iter < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); table_iter++)
    {
        payload_total_size_bytes = 0;
        curr_table_sw_size = 0;
        size_hash_bytes = 0;
        iterator_bitmap_size_bytes = 0;
        SHR_IF_ERR_EXIT(dbal_tables_table_get(unit, table_iter, &table));

        /*
         * Proceed only for tables with some sw part in it. 1. Tables with at least SW access 2. MDB tables with result 
         * mapped to sw 3. Tables with iterator optimized 
         */
        if ((table->sw_access_info.sw_payload_length_bytes == 0)
            && !SHR_BITGET(table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
        {
            continue;
        }
        ...
        if (!ISEMPTY(access_method_str))
        {
            ...
            if (access_method != table->access_method)
            {
                continue;
            }
        }

        is_basic_info_printed = 0;

        if (table->sw_access_info.sw_payload_length_bytes > 0)
        {
            SHR_IF_ERR_EXIT(dbal_sw_table_nof_entries_calculate(unit, (dbal_logical_table_t *) table, &sw_nof_entries));
            payload_total_size_bytes = sw_nof_entries * table->sw_access_info.sw_payload_length_bytes;
            /*
             * for hash software state table
             */
            if ((table->table_type != DBAL_TABLE_TYPE_DIRECT))
            {
                size_hash_bytes = DBAL_HASH_TABLE_ENTRY_SIZE * sw_nof_entries;
                total_hash_alloc += size_hash_bytes;
            }
            curr_table_sw_size = payload_total_size_bytes + size_hash_bytes;
            if (curr_table_sw_size > min_size_to_display)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET(" %s", table->table_name);
                ...
                PRT_CELL_SET(" %s", (table->table_type == DBAL_TABLE_TYPE_DIRECT) ? "Direct" : "Hash");
                PRT_CELL_SET(" %s", "SW");
                total_sw_alloc += payload_total_size_bytes;
                PRT_CELL_SET(" %d", curr_table_sw_size);
                is_basic_info_printed = 1;
            }
        }

        if (SHR_BITGET(table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
        {
            if (is_basic_info_printed == 1)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET(" %s", "\"");
                PRT_CELL_SET(" %s", "\"");
            ...
            PRT_CELL_SET(" %s", "bitmap alloc");

            /*
             * For outlif and in_lif tables, the nof bits to allocate is calculated according to max value of the key before table
             * max_capacity is updated (at mdb init step). So here we use the same calculation made at allocation time
             * (see dbal_db_init.c: dbal_db_init_alloc_bit_num_get)
             */
            if (dbal_table_is_out_lif(table) || dbal_table_is_in_lif(unit, table))
            {
                uint32 max_entry_key_value;
                SHR_IF_ERR_EXIT(dbal_tables_max_key_value_get(unit, table_iter, &max_entry_key_value));
                iterator_bitmap_size_bytes = BITS2BYTES(max_entry_key_value);
            }
            else
            {
                iterator_bitmap_size_bytes = BITS2BYTES(table->max_capacity);
            }
            if (table->core_mode == DBAL_CORE_MODE_DPC)
            {
                iterator_bitmap_size_bytes *= DBAL_MAX_NUM_OF_CORES;
            }
            total_bitmap_alloc += iterator_bitmap_size_bytes;
            PRT_CELL_SET(" %d", iterator_bitmap_size_bytes);
        }
    }

    /*
     * It is more accurate to sum everything as bytes, then divide, info lost is negligible 
     */
    PRT_INFO_ADD("Total SW Size: %d MBytes [SW: %d MBytes, Hash: %d MBytes, bitmap alloc: %d MBytes]",
                 (total_sw_alloc + total_hash_alloc + total_bitmap_alloc) / 1000000,
                 total_sw_alloc / 1000000, total_hash_alloc / 1000000, total_bitmap_alloc / 1000000);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_CELL_SET("%s", dbal_logger_print_type_to_string(unit, logger_info->print_type));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ENUM("Print", print_type);

    if (!ISEMPTY(file_name))
    {
        if (sal_strncasecmp("close", file_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    if (!ISEMPTY(table_name))
    {
        if (sal_strncasecmp("off", table_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    {
        dbal_logger_print_type_set(unit, print_type);
        LOG_CLI((BSL_META("logger print type set to %s.\n"), dbal_logger_print_type_to_string(unit, print_type)));
    }

    if (!ISEMPTY(mode))
    {
        if (sal_strncasecmp("write", mode, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            dbal_logger_table_mode_set(unit, DBAL_LOGGER_MODE_WRITE_ONLY);
            LOG_CLI((BSL_META(" logger mode set to write only.\n")));
        }

        if (sal_strncasecmp("regular", mode, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
...
static sh_sand_enum_t sand_test_dbal_logger_print_types_enum_table[] = {
    {"DEFAULT", DBAL_LOGGER_PRINT_TYPE_DEFAULT, "Default field print type"},
    {"HEX", DBAL_LOGGER_PRINT_TYPE_AS_HEX, "Print all numeric types as hexa"},
    {"EMPTY", DBAL_LOGGER_NOF_PRINT_TYPES, "dbal logger print type is empty"},
    {NULL}
};
...
static sh_sand_option_t dnx_dbal_logger_options[] = {
    ...
    {"Print", SAL_FIELD_TYPE_ENUM, "default / hex", "EMPTY", (void *) sand_test_dbal_logger_print_types_enum_table},
    {NULL}
};
...
static sh_sand_man_t dnx_dbal_logger_man = {
    ...
    .examples =
        ...
        "SeVerity=warn mode=regular print=default"
};
...
static sh_sand_man_t dnx_dbal_sw_stats_man = {
    .brief = "DBAL SW statistics information operation menu",
    .full = NULL,
    .synopsis = NULL,
    .examples = "access=SW_STATE size=1000"
};
...
static sh_sand_man_t dnx_dbal_dcd_man = {
    ...
    .examples =
        ...
        "data=1 size=2 name=MACT_APP_DB_IDX"
};

static sh_sand_option_t dnx_dbal_sw_stats_dump_options[] = {
    {"size", SAL_FIELD_TYPE_UINT32, "Minimum sw size to display (bytes)", "0"},
    {"access", SAL_FIELD_TYPE_STR, "dbal table access", ""},
    {NULL}
};

sh_sand_cmd_t diag_dbal_cmds[] = {
  /**********************************************************************************************************
   * CMD_NAME *     CMD_ACTION              * Next *        Options                   *       MAN           *
   *          *                             * Level*                                  *                     *
   *          *                             * CMD  *                                  *                     *
   **********************************************************************************************************/
    {"Info", cmd_dbal_statistics_dump, NULL, NULL, &dnx_dbal_stats_man, NULL, NULL},
    {"Sw", cmd_dbal_sw_statistics_dump, NULL, dnx_dbal_sw_stats_dump_options, &dnx_dbal_sw_stats_man, NULL, NULL},
    {NULL}
};
...
sh_sand_cmd_t sh_dnx_dbal_cmds[] = {
    ...
    ********************************************************************************************************************************/
    ...
    {"STATS", NULL, diag_dbal_cmds, NULL, &dnx_dbal_stats_man},
