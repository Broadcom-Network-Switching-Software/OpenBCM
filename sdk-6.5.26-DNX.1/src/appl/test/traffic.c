...
traffic_test_free(traffic_test_t* test)
{
    ...
    if (test->cleanup) {
        ...
        /* Restore TD3 HG3 ethertype setting */
        if (SOC_IS_TRIDENT3X(unit) &&
            soc_feature(unit, soc_feature_higig3)) {
            if (test->saved_hg3_ethertype) {
                (void)bcm_switch_control_set(unit, bcmSwitchHigig3Ethertype, test->saved_hg3_ethertype);
            }
        }
    }
 done:
...
traffic_test_send_frames(traffic_test_t* test, int port_pair_index)
{
    ...
    rv = bcm_pkt_alloc(unit, test->pkt_size + pad, 0, &pktA);
    if (rv < 0) {
        cli_out("ERROR in traffic_test_send_frames(): 1. bcm_pkt_alloc() has failed: %s\n", bcm_errmsg(rv));
        goto error;
    }
    rv = bcm_pkt_alloc(unit, test->pkt_size + pad, 0, &pktB);
    if (rv < 0) {
        cli_out("ERROR in traffic_test_send_frames(): 2. bcm_pkt_alloc() has failed: %s\n", bcm_errmsg(rv));
        goto error;
    }
    ...
    if ((SOC_IS_TRIDENT3X(unit)) &&
        ...
        uint8 is_hgoe = 0;
        rv = bcm_td3_port_hgoe_mode_get(
                 unit, portA, &is_hgoe);
        if (rv < 0) {
            goto error;
        }
        if (is_hgoe) {
        /* Remove needless setting of HGoE ports
         * in untagged bitmap
         */
            BCM_PKT_PORT_SET(pktA, portA, FALSE, FALSE);
            BCM_PKT_PORT_SET(pktB, portB, FALSE, FALSE);
        }
    }
#endif /*BCM_TRIDENT3_SUPPORT */
...
traffic_run_test(traffic_test_t* test)
{
    ...
    if (SOC_IS_TRIDENT3X(unit) &&
        soc_feature(unit, soc_feature_higig3)) {
        (void)bcm_switch_control_get(unit, bcmSwitchHigig3Ethertype, &test->saved_hg3_ethertype);
        (void)bcm_switch_control_set(unit, bcmSwitchHigig3Ethertype, 0x387);
    }
