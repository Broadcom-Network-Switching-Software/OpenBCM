...
{
    int rv;
    access_sbusdma_param_t param;

    sal_memset(&param, 0, sizeof(access_sbusdma_param_t));
    param.regmem_id = regmem;
    param.inst = instance;
    param.array_id_start = array_idx;
    param.array_id_end = array_idx;
    param.index_begin = 0;
    param.index_end = entry_cnt - 1;
    param.buffer = dma_buffer;
    /* don't enable MOR */
    param.pend_clocks = 0;
    param.flags = flags;
    rv = access_regmem_range(unit, &param);
    return rv;
}
    ...
    for (inst=0; inst<nof_instances; inst++)
    {
        ...
        for (ii=0; ii<rm_info->nof_array_indices; ii++)
        {
            ...
            if (dma_access)
            {
                ...
                flags = ACCESS_SBUSDMA_FLAG_GLOBAL_REGMEM | ACCESS_SBUSDMA_FLAG_CACHE_DISABLED;
            ...
            if (dma_access)
            {
                ...
                flags = ACCESS_SBUSDMA_FLAG_WRITE_CMD | ACCESS_SBUSDMA_FLAG_GLOBAL_REGMEM;
                rv = access_cachemem_dma_array_range(runtime_info->unit, flags, rm_info->global_regmem_id,
                                                     inst, array_idx, entry_cnt, dma_buffer);
                if (_SHR_E_FAILURE(rv))
                {
            ...
            if (dma_access)
            {
                ...
                flags = ACCESS_SBUSDMA_FLAG_GLOBAL_REGMEM | ACCESS_SBUSDMA_FLAG_WRITE_CMD;
