    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (table->physical_db_id[DBAL_PHY_DB_DEFAULT_INDEX] == DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM)
    {
    ...
    else
    {
        for (i = 0; i < DBAL_MAX_NUM_OF_PHY_DB_IN_TABLE; i++)
        {
            ...
            if (print_vals->phy_dbs_num >= DIAG_DNX_ARMOR_NUM_OF_DBS)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "DIAG_DNX_ARMOR_NUM_OF_DBS must be increased\n");
            }
            SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, table->physical_db_id[i],
                                                      &(print_vals->phy_db_capacity[print_vals->phy_dbs_num])));
            SHR_IF_ERR_EXIT(DBAL_SW_STATE_MDB_PHY_TBL.
                            nof_entries.get(unit, table->physical_db_id[i], &nof_phy_entries));
            print_vals->phy_db_entries[print_vals->phy_dbs_num] = nof_phy_entries;
            print_vals->phy_db_name[print_vals->phy_dbs_num] =
                dbal_physical_table_to_string(unit, table->physical_db_id[print_vals->phy_dbs_num]);
