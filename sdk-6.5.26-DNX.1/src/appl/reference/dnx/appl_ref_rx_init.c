    ...
    int unit)
{
    ...
    /*
     * VALGRIND requires memory of its own for memory monitoring, reduce the size of pre-allocated RX pool as much as possible in order to reserve enough memory for VALGRIND.
     */
    ...
    if (nof_cpu_ports <= RX_DMA_CHANNEL_AVAILABLE)
    {
        ...
        BCM_PBMP_ITER(port_config.cpu, logical_port)
        {
             ...
             * VALGRIND requires memory of its own for memory monitoring, reduce the size of pre-allocated RX pool as much as possible in order to reserve enough memory for VALGRIND.
             */
            cfg.chan_cfg[channel_index].chains = 1;
#else
