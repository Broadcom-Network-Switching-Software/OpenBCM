    ...
    int app_type)
{
    ...
    ipmf_presel_entry_data.qual_data[0].qual_value = app_type;
    ...
    int is_ext_cut)
{
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_application_v2))
    {
        ...
        if (is_ext_cut)
        {
            ipmf_presel_entry_data.qual_data[1].qual_value = 0x802a;
        }
        else
        {
            ipmf_presel_entry_data.qual_data[1].qual_value = 0x402a;
        }
        ipmf_presel_entry_data.qual_data[1].qual_mask = 0xFFFF;
    }
    else
    {
        ipmf_presel_entry_data.nof_qualifiers = 3;
    ...
    field_srv6_endpoint_info_t * appl_ref_field_srv6_usid_endpoint_info)
{
    ...
    bcm_field_action_info_t action_info;
    int appl_usid_prefix_size;
    uint8 usid_prefix_found = FALSE;
    int usids_offset_start = 80;
    int ii = 0;
    SHR_FUNC_INIT_VARS(unit);

    /** Get the configured usid prefix size from swstate */
    SHR_IF_ERR_EXIT(bcm_switch_control_get(unit, bcmSwitchSrv6UsidPrefixBits, &appl_usid_prefix_size));
    for (ii = 0; ii < SRV6_USID_PREFIX_NOF; ii++)
    {
        if (srv6_usid_prefix_sizes[ii] == appl_usid_prefix_size)
        {
            usid_prefix_found = TRUE;
            usids_offset_start = (appl_usid_prefix_size == 32) ? usids_offset_start : 64;
            break;
        }
    }
    if (!usid_prefix_found)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Switch control USID Prefix size %d not supported !", appl_usid_prefix_size);
    }
    ...
    /** If the prefix is 32 bits, we have 80b for the other SIDs, if it is 48 - we are left with 64b*/
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = usids_offset_start;
    ...
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = appl_usid_prefix_size;
    ...
    attach_info.key_info.qual_info[1].offset = 320 - usids_offset_start;
 ...
 */
static shr_error_e
field_srv6_config_usid_end_point_no_default_ipmf2_udh(
    int unit,
    field_srv6_endpoint_info_t * appl_ref_field_srv6_usid_endpoint_info)
{
    bcm_field_group_info_t fg_info;
    ...
    for (ii = 0; ii < SRV6_USID_PREFIX_NOF; ii++)
    {
        ...
        {
            usid_prefix_found = TRUE;
            usids_offset_start = (appl_usid_prefix_size == 32) ? usids_offset_start : 48;
            break;
        }
    }
    if (!usid_prefix_found)
    ...
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 32;
    sal_strncpy_s((char *) (qual_info.name), "srv6_usid_endp_no_default_16b_lsb_0", sizeof(qual_info.name));
    SHR_IF_ERR_EXIT(bcm_field_qualifier_create(unit, 0, &qual_info, &udh_qual_0));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = usids_offset_start;
    sal_strncpy_s((char *) (qual_info.name), "srv6_usid_endp_no_default_x5_lsb_usids", sizeof(qual_info.name));
    SHR_IF_ERR_EXIT(bcm_field_qualifier_create(unit, 0, &qual_info, &udh_qual_1));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = appl_usid_prefix_size;
    sal_strncpy_s((char *) (qual_info.name), "srv6_usid_endp_no_default_prefix", sizeof(qual_info.name));
    SHR_IF_ERR_EXIT(bcm_field_qualifier_create(unit, 0, &qual_info, &udh_qual_2));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 8;
    sal_strncpy_s((char *) (qual_info.name), "qual_usid_endpoint_no_default_udh_size_name", sizeof(qual_info.name));
    SHR_IF_ERR_EXIT(bcm_field_qualifier_create
                    (unit, 0, &qual_info, &appl_ref_field_srv6_usid_endpoint_info->udh_type_qual));
    ...
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 1;
    sal_strncpy_s((char *) (qual_info.name), "srv6_usid_zero_no_default_container_q", sizeof(qual_info.name));
    SHR_IF_ERR_EXIT(bcm_field_qualifier_create
                    (unit, 0, &qual_info, &appl_ref_field_srv6_usid_endpoint_info->pmf2_zero_container_q));
    ...
    fg_info.fg_type = bcmFieldGroupTypeDirectExtraction;
    fg_info.stage = bcmFieldStageIngressPMF2;
    sal_strncpy_s((char *) (fg_info.name), "SRv6_usid_no_default_endpoint_UDH", sizeof(fg_info.name));
    ...
    action_info.stage = bcmFieldStageIngressPMF2;
    action_info.action_type = bcmFieldActionContainer;
    sal_strncpy_s((char *) (action_info.name), "PMF2_usid_no_default_container_zero_a", sizeof(action_info.name));
    SHR_IF_ERR_EXIT(bcm_field_action_create
                    (unit, 0, &action_info, &appl_ref_field_srv6_usid_endpoint_info->pmf2_container_zero_a));
    ...
    /** 32bits LSB which are 0 for shift left */
    attach_info.key_info.qual_info[0].input_type = bcmFieldInputTypeConst;
    attach_info.key_info.qual_info[0].input_arg = 0;
    ...
    /** Indicates the layer relative to FWD layer - in Endpoint we set FWD layer back to IPv6 for FWD hash */
    attach_info.key_info.qual_info[1].input_arg = 0;
    /** Take from IPv6 DIP the last x4 LSB usids for left shift by one sid in next DIP */
    attach_info.key_info.qual_info[1].offset = 320 - usids_offset_start;
    ...
    field_srv6_endpoint_gsid_info_t * appl_ref_field_srv6_gsid_endpoint_info)
{
     ...
     */
    fg_info.nof_quals = 6;
    fg_info.qual_types[0] = bcmFieldQualifySrv6GsidNextSi;
    ...
    fg_info.qual_types[3] = bcmFieldQualifySrv6GsidNextGsid;
    ...
    field_srv6_endpoint_psp_info_t * appl_ref_field_srv6_endpoint_psp_info)
{
    ...
    sal_strncpy_s((char *) (action_info.name), "SRv6_endpoint_psp_ipm3_void_ep_action", sizeof(action_info.name));
    SHR_IF_ERR_EXIT(bcm_field_action_create
                    (unit, 0, &action_info,
    ...
    action_info.prefix_value = 0;
    action_info.stage = bcmFieldStageIngressPMF3;
    sal_strncpy_s((char *) (action_info.name), "SRv6_endpoint_psp_ipm3_void_psp_action", sizeof(action_info.name));
    SHR_IF_ERR_EXIT(bcm_field_action_create
    ...
    field_srv6_endpoint_psp_info_t * appl_ref_field_srv6_endpoint_psp_info)
{
    ...
    action_info.prefix_value = 0;
    action_info.stage = bcmFieldStageIngressPMF3;
    sal_strncpy_s((char *) (action_info.name), "SRv6_usid_ep_ipmf3", sizeof(action_info.name));
    SHR_IF_ERR_EXIT(bcm_field_action_create
                    (unit, 0, &action_info,
                     &appl_ref_field_srv6_endpoint_psp_info->endpoint_parsing_start_void_action));
    ...
    action_info.prefix_value = 0;
    action_info.stage = bcmFieldStageIngressPMF3;
    sal_strncpy_s((char *) (action_info.name), "SRv6_usid_psp_ipmf3", sizeof(action_info.name));
    SHR_IF_ERR_EXIT(bcm_field_action_create
                    (unit, 0, &action_info, &appl_ref_field_srv6_endpoint_psp_info->psp_parsing_start_void_action));
 ...
 */
static shr_error_e
field_srv6_config_psp_extended_cut_ipmf3_de(
    int unit,
    field_srv6_endpoint_psp_info_t * appl_ref_field_srv6_endpoint_psp_ext_cut_info)
{
    bcm_field_group_info_t fg_info;
    ...
    action_info.prefix_value = 0;
    action_info.stage = bcmFieldStageIngressPMF3;
    sal_strncpy_s((char *) (action_info.name), "SRv6_endpoint_psp_ext_ipm3_void_ep_action", sizeof(action_info.name));
    SHR_IF_ERR_EXIT(bcm_field_action_create
                    (unit, 0, &action_info,
    ...
    action_info.prefix_value = 0;
    action_info.stage = bcmFieldStageIngressPMF3;
    sal_strncpy_s((char *) (action_info.name), "SRv6_endpoint_psp_ext_ipm3_void_psp_action", sizeof(action_info.name));
    SHR_IF_ERR_EXIT(bcm_field_action_create
                    (unit, 0, &action_info,
 ...
 *          -it sets the Parsing-Start-Type to 'IPv4/IPv6/ETH/MPLS' according to SRH Next-Header field
 *
 *   Field Group -  TCAM:
 ...
 *          -In TCAM 2 entries of Next-Header to Parsing-Start-Type exist
 *                  -IANA MPLS (137) --> Layer_Types_MPLS
    ...
    field_srv6_egress_usd_info_t appl_ref_field_srv6_egress_usd_info)
{
     ...
     * - Create 2 TCAM entries between SRH Next-Header to  Egress Parsing Start Type and write
     * - MPLS (137) --> PARSING_START_TYPE_ENCODING_MPLS_DUMMY (10)
    ...
    field_srv6_endpoint_psp_info_t * appl_ref_field_srv6_endpoint_psp_info)
{
    ...
    SHR_IF_ERR_EXIT(field_srv6_pmf1_usid_endpoint_presel_set
                    (unit, appl_ref_field_srv6_usid_endpoint_info->ipmf1_context_id,
                     ...
                     bcmFieldAppTypeSrv6UsidEndpoint));
 ...
 */
static shr_error_e
appl_ref_srv6_endpoint_usid_no_default(
    int unit,
    field_srv6_endpoint_info_t * appl_ref_field_srv6_usid_endpoint_no_default_info,
    field_srv6_endpoint_psp_info_t appl_ref_field_srv6_endpoint_psp_info)
{
    SHR_FUNC_INIT_VARS(unit);
    /*
    ...
    /** End-Point - create the context in PMF1 and name it */
    SHR_IF_ERR_EXIT(field_srv6_context_set
                    (unit, bcmFieldStageIngressPMF1, "SRv6_EP_usid_no_def_pmf1", 0, 0,
                     &appl_ref_field_srv6_usid_endpoint_no_default_info->ipmf1_context_id));

    /** End-Point - create the Presel and attach it to the above context */
    SHR_IF_ERR_EXIT(field_srv6_pmf1_usid_endpoint_presel_set
                    (unit, appl_ref_field_srv6_usid_endpoint_no_default_info->ipmf1_context_id,
                     dnx_data_field.preselector.srv6_endpoint_usid_no_default_presel_id_ipmf1_get(unit),
                     bcmFieldAppTypeSrv6UsidExtendedEndpoint));

    /** End-Point- FG Action: Save IPv6 DIP in x4 UDH Containers in PMF2 */
    SHR_IF_ERR_EXIT(field_srv6_config_usid_end_point_no_default_ipmf2_udh
                    (unit, appl_ref_field_srv6_usid_endpoint_no_default_info));
    ...
    SHR_IF_ERR_EXIT(field_srv6_pmf3_by_pmf1_context_presel_set
                    (unit, appl_ref_field_srv6_usid_endpoint_no_default_info->ipmf1_context_id,
                     appl_ref_field_srv6_endpoint_psp_info.ipmf3_context_id,
                     dnx_data_field.preselector.srv6_endpoint_usid_no_default_presel_id_ipmf3_get(unit)));
    ...
    int unit)
{
     ...
     * -Actions:
     *        - get the NOF SIDs(not used now) and next-heder to store for PMF3
     *
     * Will be peformed for following cases:
    ...
    SHR_IF_ERR_EXIT(bcm_field_context_param_set
                    (unit, 0, bcmFieldStageIngressPMF2, appl_ref_field_srv6_egress_usd_info.ipmf1_context_id,
                     &context_param));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_hw_value_get(unit, DBAL_FIELD_TYPE_DEF_IPV6_ADDITIONAL_HEADER,
    ...
    field_srv6_endpoint_psp_info_t * appl_ref_field_srv6_endpoint_psp_ext_cut_info)
{
    ...
    SHR_IF_ERR_EXIT(field_srv6_config_psp_extended_cut_ipmf3_de(unit, appl_ref_field_srv6_endpoint_psp_ext_cut_info));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(appl_ref_field_srv6_usid_endpoint_info_t_init
                    (unit, &appl_ref_field_srv6_usid_endpoint_no_default_info));
    ...
    SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchSrv6GsidPrefixBits, ((appl_gsid_64b_enable == 1) ? 64 : 48)));

    /** configure the usid mode */
    appl_usid_48b_enable = dnx_data_srv6.general.feature_get(unit, dnx_data_srv6_general_srv6_usid_prefix_48b_enable);
    SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchSrv6UsidPrefixBits, ((appl_usid_48b_enable == 1) ? 48 : 32)));
    ...
    /*
     * uSID Extended End point handling - handle the uSID middle case, when SL is not decreased
     */
    SHR_IF_ERR_EXIT(appl_ref_srv6_endpoint_usid_no_default
                    (unit, &appl_ref_field_srv6_usid_endpoint_no_default_info, appl_ref_field_srv6_endpoint_psp_info));
