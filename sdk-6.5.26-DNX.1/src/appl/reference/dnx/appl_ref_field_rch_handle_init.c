    ...
    int unit)
{
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_rch_remove_field_program_enable))
    {
        /*
        ...
        /*
         * We now take the offset of the first header from the laye record and use it in the
...
/**
 * \brief
 *   This function Configures a new context and field group for usage of RCH extended encapsulation application.
...
/**
 * \brief
 *   This function configure EPMF rule which copy the outlif0 on egress container.
 *   This is used for RCH purpose, which should set outlif0 on the RCH header.
 * \param [in] unit - The unit number.
 *
 * \return
 *   Error code (as per 'shr_error_e').
 ...
 */
shr_error_e
appl_dnx_field_rch_outlif_zero_init(
    int unit)
{
    bcm_field_group_info_t fg_info;
    bcm_field_group_attach_info_t attach_info;
    ...
    if (dnx_data_lif.feature.feature_get(unit, dnx_data_lif_feature_copy_outlif0_to_egress_container_by_epmf))
    {
        /*
         * Create a direct extraction field group to copy the IP outlif0 to the container.
         */
        bcm_field_group_info_t_init(&fg_info);
        sal_strncpy_s((char *) (fg_info.name), "OUTLIF0_TO_CONTAINER", sizeof(fg_info.name));
        fg_info.fg_type = bcmFieldGroupTypeDirectExtraction;
        fg_info.stage = bcmFieldStageEgress;
        fg_info.nof_quals = 1;
        fg_info.qual_types[0] = bcmFieldQualifyOutVPort0Raw;
        fg_info.nof_actions = 1;
        fg_info.action_with_valid_bit[0] = FALSE;
        fg_info.action_types[0] = bcmFieldActionOutVportToStamp;
        SHR_IF_ERR_EXIT(bcm_field_group_add(unit, 0, &fg_info, &fg_id));
        ...
        SHR_IF_ERR_EXIT(bcm_field_group_context_attach
                        (unit, 0, fg_id, dnx_data_field.context.default_context_get(unit), &attach_info));
    }
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(appl_dnx_field_rch_outlif_zero_init(unit));
