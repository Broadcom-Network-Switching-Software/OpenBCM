...
{    
    ...
    CINT_MEMSET(&desc, 0, sizeof(desc));

#ifdef CINT_COV
    if(ast->utype.declaration.init){
      init_coverage(&ast->file, &ast->utype.declaration.identifier->line, NULL, NULL);
      executions_coverage(ast->file, &ast->utype.declaration.identifier->line, NULL);
    }
#endif
...
#ifdef CINT_COV
void operator_cov(cint_ast_t* ast){
    cint_ast_t* ast_left = ast->utype.operator.left;
    cint_ast_t* ast_right = ast->utype.operator.right;
    int operator = ast->utype.operator.op;
    if(operator == cintOpAssign || operator == cintOpIncrement || operator == cintOpDecrement){
      if(ast_left){
         executions_coverage(ast->file, &ast_left->line, NULL);
      }
      else if(ast_right){
         executions_coverage(ast->file, &ast_right->line, NULL);
      }
    }
}
#endif
...
__cint_eval_ast_Function(cint_ast_t* ast)
{
    ...
    fp = dtp->basetype.fp; 
#ifdef CINT_COV
    /*Counting function execution in decl file*/
    if(fp->body.statements){
      executions_coverage(NULL, &fp->body.statements->line, ast->utype.function.name);
    }
#endif
    ...
    for(count = 0, p = ast->utype.function.parameters, pd = fp->params+1; 
        ...
        fparams_args_length = sizeof(fparams.args)/sizeof(fparams.args[0]);
        if(count >= fparams_args_length){
            cint_ast_error(ast, CINT_E_INTERNAL, "cint function %s takes too many input parameters. The maximum input parameters in CINT functions are %d.",
                           ast->utype.function.name ,fparams_args_length);
            return NULL;
        }
...
#ifdef CINT_COV
    executions_coverage(ast->file, &ast->utype._while.condition->line, NULL);
#endif
...
#ifdef CINT_COV
        executions_coverage(ast->file, &ast->line, NULL);
#endif
    }
...
#ifdef CINT_COV
    /*Counting function execution in decl file*/
    if(dt.basetype.fp->body.statements){
      executions_coverage(dt.basetype.fp->body.statements->file, &dt.basetype.fp->body.statements->line, dt.basetype.fp->name);
    }
#endif
...
cint_eval_asts(cint_ast_t* ast)
{
    ...
    for(p = ast; p && (cint_errno == CINT_E_NONE); p = p->next) {
        rv = cint_eval_ast(p);
#ifdef CINT_COV
        if(p->ntype != cintAstOperator && p->ntype != cintAstFunctionDef){
            executions_coverage(p->file, &p->line, NULL);
        }
#endif
    }
