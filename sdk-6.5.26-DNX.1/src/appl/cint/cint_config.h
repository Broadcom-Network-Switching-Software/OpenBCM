 ...
 * print into without truncation. Currently BCM_PBMP_PORT_MAX is 1280.
 * Accordingly the size is increased to 5400. The formula to get from 1280 to
 * 5400 is mentioned in cint_logger.c -> cint_logger_cint_init
 *
 */
#ifndef CINT_CONFIG_MAX_STACK_PRINT_BUFFER
#if 100 > _SHR_PBMP_PORT_MAX
#define CINT_CONFIG_MAX_STACK_PRINT_BUFFER  (4 + 2 + 18 + (_SHR_PBMP_PORT_MAX - 9) * 3)
#elif 1000 > _SHR_PBMP_PORT_MAX
#define CINT_CONFIG_MAX_STACK_PRINT_BUFFER  (4 + 2 + 18 + 270 + (_SHR_PBMP_PORT_MAX - 99) * 4)
#else
#define CINT_CONFIG_MAX_STACK_PRINT_BUFFER  (4 + 2 + 18 + 270 + 900 * 4 + (_SHR_PBMP_PORT_MAX - 999) * 5)
#endif
#endif
/*
 ...
 *
 * API Logger test infra creates variable names as function of the API name,
 * and the longest API name is 60 characters wide already:
 * bcm_field_qualify_EthernetOamClassVlanTranslateKeySecond_get
 * Increasing the max variable length to (arbitrarily chosen) 128
 */
#ifndef CINT_CONFIG_MAX_VARIABLE_NAME
#define CINT_CONFIG_MAX_VARIABLE_NAME 128
#endif
