...
{
    SHR_UTIL_PACK_U16(buf, ipfix->version);
    SHR_UTIL_PACK_U16(buf, ipfix->length);
    SHR_UTIL_PACK_U32(buf, ipfix->export_time);
    SHR_UTIL_PACK_U32(buf, ipfix->sequence_num);
    SHR_UTIL_PACK_U32(buf, ipfix->observation_id);
...
{
    SHR_UTIL_PACK_U32(buf, vxlan->flags);
    SHR_UTIL_PACK_U32(buf, vxlan->vnid);
...
uint8 *
shr_util_pack_ipv4_header_pack(uint8 *buf, shr_util_pack_ipv4_header_t *ip)
{
    uint32 tmp;
...
uint8 *
shr_util_pack_ipv6_header_pack(uint8 *buf, shr_util_pack_ipv6_header_t *ip)
{
    uint32 tmp, idx;
...
uint8 *
shr_util_pack_mpls_label_pack(uint8 *buf, shr_util_pack_mpls_label_t *mpls)
{
    uint32 tmp;
...
uint32
shr_util_pack_initial_chksum_get(uint16 length, uint8 *data)
{
    uint32 chksum, idx;
    uint16 w16;
    ...
    idx = 0;
    while (length > 1) {
        w16 = (((uint16)data[idx]) << 8) + data[idx+1];
        chksum += w16;
        idx += 2;
        length -= 2;
    }
    if (length) {
        w16 = (((uint16)data[idx]) << 8) + 0;
        chksum += w16;
    }
...
{
    uint8 buf[SHR_UDP_HDR_LEN + (SHR_IPV6_ADDR_LEN *2 ) +
