    ...
    uint32 *field_p)
{
    uint32 end_bit;
    /*
     * word_first - index of the firts word containing the field.
     * word_last  - index of the last word containing the field.
     */
    uint32 word_first, word_last;
    COMPILER_UINT64 dword_data;
    uint32 word_data;
    uint32 word_first_bit, shift, mask;
    COMPILER_UINT64 dmask;
    /*
     * The bits in bit_stream are counted as follows (from LS to MS):
     * index 0, bits 0  - 31
     * index 1, bits 32 - 63
     * index 2, bits 64 - 95
     * ...
    ...
    SHR_NULL_CHECK(field_p, _func_rv, "field_p");
    if (nof_bits == 0)
    {
        *field_p = 0;
        SHR_EXIT();
    ...
    /*
     * word offset is the bit index divided by 32
     */
    word_first = start_bit >> 5;
    word_first_bit = word_first << 5;
    shift = start_bit - word_first_bit;
    /*
     * If the field crosses the border between two 32-bits words then use 64
     * bits as containiner. Otherwise, use 32-bits container.
     */
    if (word_last > word_first)
    {
        COMPILER_UINT64 tmp_64;

        dword_data = COMPILER_64_INIT(bit_stream[word_last], bit_stream[word_first]);
        COMPILER_64_SET(dmask, 0, 1);
        COMPILER_64_SHL(dmask, nof_bits);
        COMPILER_64_SET(tmp_64, 0, 1);
        COMPILER_64_SUB_64(dmask, tmp_64);
        /*
         * Shift-right the data, first, and then AND (and not the other way round).
         * This will make sure no bits are dragged from the MS direction.
         */
        COMPILER_64_SHR(dword_data, shift);
        COMPILER_64_AND(dword_data, dmask);
        *field_p = COMPILER_64_LO(dword_data);
    }
    else
    {
        word_data = bit_stream[word_first];
        /*
         * Shift-right the data, first, and then AND (and not the other way round).
         * This will make sure no bits are dragged from the MS direction.
         */
        word_data >>= shift;
        /*
         * Some compilers complain about shifting out of a variable.
         */
        if (nof_bits == (sizeof(mask) * SAL_UINT8_NOF_BITS))
        {
            mask = (uint32) (-1);
        }
        else
        {
            mask = SAL_BIT(nof_bits) - 1;
        }
        word_data &= mask;
        *field_p = word_data;
    }
    ...
    uint32 silent)
{
    ...
    /*
     * At this point, the bits stream looks like this:
     * 0000FFFF FFF23450
     */
    field = 0;
    ...
    field = 0;
    res = utilex_bitstream_get_field(bit_stream, 0, 32, &field);
    if (res != _SHR_E_NONE)
    {
        pass = FALSE;
        goto exit;
    }
    else if (field != 0xFFF23450)
    {
        pass = FALSE;
        goto exit;
    }
    field = 0;
    res = utilex_bitstream_get_field(bit_stream, 4, 28, &field);
    if (res != _SHR_E_NONE)
    {
        pass = FALSE;
        goto exit;
    }
    else if (field != 0x0FFF2345)
    {
        pass = FALSE;
        goto exit;
    }
    SHR_IF_ERR_EXIT(utilex_bitstream_set_field(bit_stream, 56, 16, 0x6789A));
    if (!silent)
    {
        utilex_bitstream_print(bit_stream, 4);
        utilex_bitstream_print_beautify_1(bit_stream, 2, 3, 96);
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META("\r\n")));
    }
    /*
     * At this point, bits stream looks like this:
     * 00000000 00000078 9A00FFFF FFF23450
     */
    field = 0;
    res = utilex_bitstream_get_field(bit_stream, 60, 12, &field);
    if (res != _SHR_E_NONE)
    {
        pass = FALSE;
        goto exit;
    ...
    field = 0;
    res = utilex_bitstream_get_field(bit_stream, 40, 32, &field);
    if (res != _SHR_E_NONE)
    {
        pass = FALSE;
        goto exit;
    }
    else if (field != 0x789A00FF)
    {
        pass = FALSE;
        goto exit;
    }
    ...
    uint32 max_nof_printed_items)
{
    ...
    sal_snprintf(format, sizeof(format), "%%-%uu ", max_dec_digits);
    /*
     */
    if (nof_on_bits == 0)
    {
    ...
    uint32 nof_columns)
{
    ...
    for (curr_line = 0; curr_line < max_nof_lines; curr_line++)
    {
        ...
        sal_memset(str_line, 0x0, sizeof(str_line));

        str_ptr = str_line;

        str_ptr +=
            sal_snprintf(str_ptr, sizeof(str_line), "OFFSET LONGS: %6d", (uint32) (curr_offset / sizeof(uint32)));
        /*
         * ITERATION INITIALIZATIONS }
         */
        ...
        for (curr_col = 0; curr_col < max_columns; curr_col++)
        {
            ...
            if (curr_offset < buff_size)
            {
                ...
                {
                    str_ptr +=
                        sal_snprintf(str_ptr, sizeof(str_line) - (uint32) (str_ptr - str_line), " %02X", curr_char);
                }
                else
                {
                    str_ptr +=
        ...
        str_ptr += sal_snprintf(str_ptr, sizeof(str_line) - (uint32) (str_ptr - str_line), "\n\r");
        /*
