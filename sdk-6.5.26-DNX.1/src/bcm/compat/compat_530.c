    ...
    bcm_compat6525_l2_addr_t *to)
{
    ...
    to->encap_id = BCM_FORWARD_ENCAP_ID_INVALID;
    to->taf_gate_primap = BCM_TSN_PRI_MAP_INVALID;
    to->flags = from->flags;
...
{
    int			rv;
    bcm_compat6525_l2_addr_t	nl2addr;

    if (l2addr == NULL) {
        return BCM_E_PARAM;
    }
    _bcm_compat530in_l2_addr_add_t(l2addr, &nl2addr);
    rv = bcm_compat6525_l2_addr_add(unit, &nl2addr);
    return rv;
}
    ...
    int *cf_count)
{
    ...
    bcm_compat6525_l2_addr_t	*ncf_array;

    if (addr == NULL || cf_array == NULL) {
        return BCM_E_PARAM;
    }
    ncf_array = sal_alloc(sizeof(bcm_compat6525_l2_addr_t) * cf_max,
    ...
    bcm_compat530_l2_addr_t *l2addr)
{
    ...
    rv = bcm_compat6525_l2_addr_get(unit, mac_addr, vid, &nl2addr);
    if (rv >= 0) {
