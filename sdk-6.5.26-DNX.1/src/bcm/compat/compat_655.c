...
{
    int rv, i;
    bcm_compat6521_trunk_member_t * n_mem = NULL;

    if (member_count) {
        n_mem = (bcm_compat6521_trunk_member_t *)sal_alloc(
                        member_count*sizeof(bcm_compat6521_trunk_member_t),
                        "rpc member_array");
        if(n_mem == NULL) {
            return BCM_E_MEMORY;
        }
     ...
     * to be set
     */
    if ((n_mem != NULL) && (member_array != NULL))
    {
...
{
    int rv, i;
    bcm_compat6521_trunk_member_t * n_mem;
    ...
    n_mem = (bcm_compat6521_trunk_member_t *)sal_alloc(
                member_max*sizeof(bcm_compat6521_trunk_member_t), "rpc member_array");
    if(n_mem == NULL) {
        return BCM_E_MEMORY;
    }
