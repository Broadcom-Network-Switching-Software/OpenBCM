...
{
    int rv;
    bcm_compat6526_flow_match_config_t *new_info = NULL;

    if (info != NULL) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_compat6526_flow_match_config_t *)
            sal_alloc(sizeof(bcm_compat6526_flow_match_config_t), "New flow match config");
        if (new_info == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flow_match_add(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flow_match_delete(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_match_config_t(new_info, info);
        sal_free(new_info);
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flow_match_get(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_match_config_t(new_info, info);
        sal_free(new_info);
    ...
    uint32 value)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flow_match_flexctr_object_set(unit, new_info, value);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_match_config_t(new_info, info);
        sal_free(new_info);
    ...
    uint32 *value)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flow_match_flexctr_object_get(unit, new_info, value);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_match_config_t(new_info, info);
        sal_free(new_info);
    ...
    uint32 stat_counter_id)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flow_match_stat_attach(unit, new_info, num_of_fields, field, stat_counter_id);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_match_config_t(new_info, info);
        sal_free(new_info);
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flow_match_stat_detach(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_match_config_t(new_info, info);
        sal_free(new_info);
    ...
    uint32 *stat_counter_id)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flow_match_stat_id_get(unit, new_info, num_of_fields, field, stat_counter_id);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_match_config_t(new_info, info);
        sal_free(new_info);
