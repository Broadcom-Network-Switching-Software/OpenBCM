 ...
 */
STATIC void
_bcm_compat6516out_l2_addr_t(bcm_compat6525_l2_addr_t *from,
...
{
    int rv = BCM_E_NONE;
    bcm_compat6525_l2_addr_t *newl2addr = NULL;

    if (NULL != l2addr) {
        /* Create from heap to avoid the stack to bloat */
        newl2addr = (bcm_compat6525_l2_addr_t *)
            sal_alloc(sizeof(bcm_compat6525_l2_addr_t), "New l2 addr");
        if (NULL == newl2addr) {
            return BCM_E_MEMORY;
        }
    ...
    rv = bcm_compat6525_l2_addr_add(unit, newl2addr);
                                 ...
                                 int count)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_addr_multi_add(unit, newl2addr,count);
                                    ...
                                    int count)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_addr_multi_delete(unit, newl2addr,count);
                           ...
                           bcm_compat6516_l2_addr_t *l2addr)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_addr_get(unit, mac_addr, vid, newl2addr);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6525_l2_addr_t naddr;
    bcm_compat6525_l2_addr_t *ncf_array = NULL;
    int i = 0;

    if ((NULL != addr) && (NULL != cf_array) && (0 < cf_max)) {
        /* Create from heap to avoid the stack to bloat */
        ncf_array = (bcm_compat6525_l2_addr_t *)
                     sal_alloc(cf_max * sizeof(bcm_compat6525_l2_addr_t),
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_conflict_get(unit, &naddr, ncf_array, cf_max, cf_count);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6525_l2_addr_t *newmatch_addr = NULL;

    if (NULL != match_addr) {
        /* Create from heap to avoid the stack to bloat */
        newmatch_addr = (bcm_compat6525_l2_addr_t *)
            sal_alloc(sizeof(bcm_compat6525_l2_addr_t), "New l2 addr");
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_replace(unit, flags, newmatch_addr, new_module, new_port, new_trunk);
                                ...
                                bcm_compat6516_l2_addr_t *replace_mask_addr)
{
    int rv = BCM_E_NONE;
    bcm_compat6525_l2_addr_t *newmatch_addr = NULL;
    bcm_compat6525_l2_addr_t *newmask_addr = NULL;
    bcm_compat6525_l2_addr_t *newreplace_addr = NULL;
    bcm_compat6525_l2_addr_t *newreplace_mask_addr = NULL;
    ...
    if ((NULL != match_addr) && (NULL != mask_addr) &&
        ...
        newmask_addr = (bcm_compat6525_l2_addr_t *)
            sal_alloc(sizeof(bcm_compat6525_l2_addr_t), "New l2 addr mask");
        if (NULL == newmask_addr) {
            sal_free(newmatch_addr);
            return BCM_E_MEMORY;
        }

        newreplace_addr = (bcm_compat6525_l2_addr_t *)
            sal_alloc(sizeof(bcm_compat6525_l2_addr_t), "Replace l2 addr");
        ...
        newreplace_mask_addr = (bcm_compat6525_l2_addr_t *)
            sal_alloc(sizeof(bcm_compat6525_l2_addr_t), "Replace l2 addr mask");
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_replace_match(unit, flags, newmatch_addr, newmask_addr,
                           ...
                           bcm_l2_stat_t stat, uint64 *val)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_stat_get(unit, newl2addr, stat, val);
                             ...
                             bcm_l2_stat_t stat, uint32 *val)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_stat_get32(unit, newl2addr, stat, val);
                           ...
                           bcm_l2_stat_t stat, uint64 val)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_stat_set(unit, newl2addr, stat, val);
                             ...
                             bcm_l2_stat_t stat, uint32 val)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_stat_set32(unit, newl2addr, stat, val);
                                  ...
                                  int enable)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_l2_stat_enable_set(unit, newl2addr, enable);
 ...
 */
STATIC void
_bcm_compat6516out_trunk_info_t( bcm_compat6521_trunk_info_t *from,
...
{
    int rv;
    bcm_compat6521_trunk_info_t *new_info = NULL;

    if (NULL != trunk_info) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_compat6521_trunk_info_t *)
            sal_alloc(sizeof(bcm_compat6521_trunk_info_t), "New trunk info");
        if (NULL == new_info) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6521_trunk_get(unit, tid, new_info, member_max, member_array, member_count);
    ...
    bcm_compat6521_trunk_member_t *member_array)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6521_trunk_set(unit, tid, new_info, member_count, member_array);
    ...
    bcm_compat6516_ecn_traffic_map_info_t *map)
{
    ...
    if (NULL != map) {
        ...
        new_map = (bcm_compat6520_ecn_traffic_map_info_t *)
            sal_alloc(sizeof(bcm_compat6520_ecn_traffic_map_info_t), "New ECN traffic map");
        if (NULL == new_map) {
            return BCM_E_MEMORY;
        }
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    if (info != NULL) {
        ...
        new_info = (bcm_compat6521_flow_tunnel_initiator_t *)
            sal_alloc(sizeof(bcm_compat6521_flow_tunnel_initiator_t), "New flow tunnel");
        if (new_info == NULL) {
            return BCM_E_MEMORY;
        }
...
{
    int rv;
    bcm_compat6523_flow_match_config_t *new_info = NULL;

    if (info != NULL) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_compat6523_flow_match_config_t *)
            sal_alloc(sizeof(bcm_compat6523_flow_match_config_t), "New flow match config");
        if (new_info == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_flow_match_add(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_flow_match_delete(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6516out_flow_match_config_t(new_info, info);
        sal_free(new_info);
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_flow_match_get(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6516out_flow_match_config_t(new_info, info);
        sal_free(new_info);
 ...
 */
STATIC void
_bcm_compat6516out_vxlan_port_t(bcm_compat6523_vxlan_port_t *from,
...
{
    int rv;
    bcm_compat6523_vxlan_port_t *new_info = NULL;

    if (NULL != vxlan_port) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_compat6523_vxlan_port_t *)
            sal_alloc(sizeof(bcm_compat6523_vxlan_port_t), "New VxLAN Port");
        if (NULL == new_info) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_vxlan_port_add(unit, l2vpn, new_info);
                                  ...
                                  bcm_compat6516_vxlan_port_t *vxlan_port)
{
    ...
    if (NULL != vxlan_port) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_compat6523_vxlan_port_t*)
            sal_alloc(sizeof(bcm_compat6523_vxlan_port_t), "New VxLAN Port Info");
        if (NULL == new_info) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_vxlan_port_get(unit, l2vpn, new_info);
                                      ...
                                      int *port_count)
{
    ...
    if ((port_max > 0) && (port_array != NULL)) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_compat6523_vxlan_port_t *)
            sal_alloc(sizeof(bcm_compat6523_vxlan_port_t) * port_max, "New VxLAN Port");
        if (NULL == new_info) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_vxlan_port_get_all(unit, l2vpn, port_max, new_info, port_count);
