 ...
 *      gport_is_fec  - (IN) gport encoding is fec ID
 * Returns:
 *      BCM_E_XXX
 */
int
_bcm_dpp_in_lif_ac_match_add(int unit, bcm_vlan_port_t *vlan_port, int in_lif, int gport_is_fec)
{
    ...
    if (gport_is_fec)
    {
        gport_sw_resources.in_lif_sw_resources.additional_flags |= _BCM_DPP_INFLIF_MATCH_INFO_GPORT_IS_FEC;
    }
...
{   
  ...
  /**pcp mapping is initialized in arad_pp_eg_vlan_edit_init_unsafe()*/
    ...
    int unit)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop != SOC_PPC_AC_ID_INVALID) {
        ...
        rv = _bcm_dpp_in_lif_ac_match_add(unit, &vlan_port, lif_drop_local_lif, 0);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple != SOC_PPC_AC_ID_INVALID) {
        ...
        BCM_GPORT_VLAN_PORT_ID_SET(vlan_port.vlan_port_id, gport);
        rv = _bcm_dpp_in_lif_ac_match_add(unit, &vlan_port, lif_simple_local_lif, 0);
        BCMDNX_IF_ERR_EXIT(rv);
...
exit:
    /**add GLEM entry when EEDB entry is ready*/
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, global_out_ac, local_out_ac, TRUE);
    ...
    bcm_vlan_action_set_t *action)
{
    ...
    /* Compute the Out-AC */
    rv = _bcm_petra_vlan_translate_action_out_ac_get(unit, port_class, outer_vlan, inner_vlan, TRUE, &is_found, &out_ac, &global_lif_id, &is_need_add_glem);
    BCMDNX_IF_ERR_EXIT(rv);
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, global_lif_id, out_ac, TRUE);
        BCMDNX_IF_ERR_REPORT(rv1);
    }
    ...
    bcm_vlan_action_set_t *action)
{
    ...
    /* Compute the Out-AC */
    rv = _bcm_petra_vlan_translate_action_out_ac_get(unit, port_class, outer_vlan, inner_vlan, FALSE, &is_found, &out_ac, &global_lif_id, &is_need_add_glem);
    BCMDNX_IF_ERR_EXIT(rv);
...
exit:
    if (rv != BCM_E_NONE) {
        if (outer_vlan_low != BCM_VLAN_INVALID) {
            /* In case of outer_vlan_low != BCM_VLAN_INVALID, outer_vlan_range is initialized and the function is called */
            /* coverity[UNINIT:FALSE]  */
            soc_ppd_l2_lif_vlan_compression_remove(unit, &outer_vlan_range);
        }
        
        if (inner_vlan_low != BCM_VLAN_INVALID) {
            /* In case of inner_vlan_range != BCM_VLAN_INVALID, inner_vlan_range is initialized and the function is called */
            /* coverity[UNINIT:FALSE]  */
    ...
    if (is_port_flooding || (old_template != new_template)) {
        ...
        if (is_lif_flooding) {        
          ...
          if ((lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_AC) && (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP) &&
              ...
              (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_MPLS_TUNNEL_RIF) && (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_IP_TUNNEL_RIF)) {
          ...
          if (lif_entry_info->type == SOC_PPC_LIF_ENTRY_TYPE_PWE) {
              ...
              lif_entry_info->value.ip_term_info.default_forward_profile = new_template;
          } else {
    ...
    bcm_vlan_port_t *vlan_port)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(vlan_port->vlan_port_id) ) {
        ...
        /* Get fec_id from SW DB when couldn't get fec_id from LIF.learn_info */
        if (fec_id == (-1)) {
            rv = _bcm_dpp_local_lif_sw_resources_learn_gport_get(unit, in_lif_id, &learn_gport);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = _bcm_dpp_gport_to_fwd_decision(unit,learn_gport,&learn_info);
            BCMDNX_IF_ERR_EXIT(rv);
            if (learn_info.type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) {
                fec_id = learn_info.dest_id;
            }
        }

        /* In case don't add fec as learn_dest  */
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_l2_intf_calculate_port_id(unit, fec_id, global_lif_id, vlan_port, BCM_GPORT_VLAN_PORT, &gport_is_fec));
        BCM_ENCAP_ID_SET(vlan_port->encap_id, global_lif_id);
    ...
    if (fec_id != -1 && (!BCM_GPORT_SUB_TYPE_IS_MULTICAST(BCM_GPORT_VLAN_PORT_ID_GET(vlan_port->vlan_port_id))) && \
        ...
        if (phy_port.type != _bcmDppGportResolveTypeRing) {
            ...
            if (protect_type != SOC_PPC_FRWRD_FEC_PROTECT_TYPE_PATH && 
                ...
                BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("FEC is not of protection type path/facility")));      /* not expected */
            }
            ...
            if (protect_type == SOC_PPC_FRWRD_FEC_PROTECT_TYPE_PATH)
            {
                ...
                failover_id = (SOC_IS_QUX(unit) ? (SOC_DPP_DEFS_GET(unit, nof_failover_ingress_ids) - 2) : (SOC_DPP_DEFS_GET(unit, nof_failover_fec_ids)));
                DPP_FAILOVER_TYPE_SET(vlan_port->failover_id, failover_id, DPP_FAILOVER_TYPE_FEC);
            }
...
int _bcm_dpp_vlan_port_create(int unit, bcm_vlan_port_t *vlan_port)
{
    ...
    int gport_is_fec = 0;
    int is_need_add_glem = FALSE;
    int is_glem_added = FALSE;
    int glif_id_to_add_glem = 0, local_lif_id_to_add_glem = 0; 
    ...
    if (encap_with_id) {
        ...
        rv = _bcm_dpp_gport_l2_intf_calculate_port_id(unit, act_fec, global_lif_id, vlan_port, BCM_GPORT_VLAN_PORT, &gport_is_fec);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    {
        uint32 pp_port_tmp, local_port_tmp = local_port;
        uint8 is_first_port = 1;
        int core_tmp = 0;
        SOC_PPC_PORT_INFO port_info_tmp;

        SOC_PPC_PORT_INFO_clear(&port_info_tmp);
        /* set port info */
        BCM_PBMP_ITER(gport_info.pbmp_local_ports,local_port_tmp) {
            /* set port attributes to act as metro port */
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, local_port_tmp, &pp_port_tmp, &core_tmp)));

            soc_sand_rv = soc_ppd_port_info_get(unit,core_tmp,pp_port_tmp,&port_info_tmp);

            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            /* this information not to be change at update */

            if ((SOC_DPP_CONFIG(unit)->trill.mode) && 
                (port_info_tmp.vlan_translation_profile == SOC_PPC_PORT_DEFINED_VT_PROFILE_TRILL || 
                port_info_tmp.vlan_translation_profile == SOC_PPC_PORT_DEFINED_VT_PROFILE_TRILL_USE_INITIAL_VID) ) {
                /* Assume all pbmp are trill ports */
                is_trill_port = TRUE;
            }

            /* verify CEP port */
            if (port_info_tmp.port_type == SOC_SAND_PP_PORT_L2_TYPE_CEP) {
                cep_port = 1;
            }
            ...
            if (is_first_port) {
                core = core_tmp;
                pp_port = pp_port_tmp;
                memcpy(&port_info, &port_info_tmp, sizeof(SOC_PPC_PORT_INFO));
            }
            if(update) {
                break;
            }
            ...
            if (port_info_tmp.enable_learning != TRUE ||
                port_info_tmp.orientation != SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE ||
                port_info_tmp.is_learn_ac == 0 ) {
                /* enable learning in port */
                port_info_tmp.enable_learning = TRUE;
                /* 
                 ...
                 */
                 if (port_info_tmp.port_type != SOC_SAND_PP_PORT_L2_TYPE_CEP &&
                     !(is_forward_group)) {
                     port_info_tmp.is_learn_ac = 1;
                 }
                 else if (port_info_tmp.port_type != SOC_SAND_PP_PORT_L2_TYPE_CEP && 
                          is_forward_group &&
                          (SOC_PPC_FRWRD_MACT_LEARN_LIMIT_MODE == SOC_PPC_FRWRD_MACT_LEARN_LIMIT_TYPE_LIF)) {
                    port_info_tmp.is_learn_ac = 1;
                }
                port_info_tmp.orientation = SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE;
                port_info_tmp.flags = (SOC_PPC_PORT_IHB_PINFO_FLP_TBL |
                                   SOC_PPC_PORT_IHP_VTT_PP_PORT_CONFIG_TBL |
                                   SOC_PPC_PORT_EGQ_PP_PPCT_TBL);
                soc_sand_rv = soc_ppd_port_info_set(unit,core_tmp,pp_port_tmp,&port_info_tmp);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
            /* Update port information */
            if (is_first_port) {
                port_info.enable_learning = port_info_tmp.enable_learning;
                port_info.is_learn_ac = port_info_tmp.is_learn_ac;
                port_info.orientation = port_info_tmp.orientation;
                port_info.flags = port_info_tmp.flags;
                is_first_port = 0;
            }
            /* 
             * Update other port property in trunk.
             */
            if (!(gport_info.flags & _BCM_DPP_GPORT_INFO_F_IS_LAG)){
                break;
            }
        }
    }
        ...
        if ((!update) ||
           ((encap_with_id) && BCM_ENCAP_ID_GET(vlan_port->encap_id) != BCM_ENCAP_ID_GET(vlan_port_o.encap_id))) {
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                if (local_out_lif_info_p != NULL) {
                    is_need_add_glem = TRUE;
                    glif_id_to_add_glem = global_lif_id;
                    local_lif_id_to_add_glem = local_out_lif_info_p->base_lif_id;
                }
    ...
    if (!remote_lif) {
        ...
        if (is_ingress) {
            rv = _bcm_dpp_in_lif_ac_match_add(unit, vlan_port, local_in_lif_id, gport_is_fec);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif) {
            ...
            /* non-protected ac */
            if (is_1_plus_1 == 0) {
                in_ac_info.protection_pointer = _BCM_INGRESS_PROTECTION_POINTER_DEFAULT;
                in_ac_info.protection_pass_value = 0;
            } else {
                /* 1+1 protection info */
            ...
            if (is_egress && !skip_egress_alloc) {
                ...
                if (egress_vlan_filtering) {
                    ...
                    if (is_need_add_glem && !is_glem_added) {
                        rv = _bcm_dpp_lif_glem_access_entry_operation(unit, glif_id_to_add_glem, local_lif_id_to_add_glem, TRUE);
                        BCMDNX_IF_ERR_EXIT(rv);
                        is_glem_added = TRUE;
                    }
                ...
                /* 
                 * In case of Port match set also port default AC 
                 */
                if (vlan_port->criteria == BCM_VLAN_PORT_MATCH_PORT) { 
                ...
                if ((vlan_port->vsi <= BCM_VLAN_MAX) && (vlan_port->vsi != BCM_VLAN_NONE) && (vlan_port->criteria != BCM_VLAN_PORT_MATCH_NONE)) {
                    ...
                    if(update) {
                        if (!cep_port ||
                            ...
                            /*
                             * For ingress key, when match critearia is port_only,
                             * the Vlan_domain is actually pp_port
                             * But For Egress key, vlan_domain is always needed 
                             */
                            vlan_port_o.criteria = BCM_VLAN_PORT_MATCH_NONE;
    ...
    /*add glem for !remote and !local_port*/
    if (is_need_add_glem && !is_glem_added) {
        rv = _bcm_dpp_lif_glem_access_entry_operation(unit, glif_id_to_add_glem, local_lif_id_to_add_glem, TRUE);
        BCMDNX_IF_ERR_EXIT(rv);
        is_glem_added = TRUE;
...
exit:
    /*add glem in case of failure before adding the glem entry*/
    if (is_need_add_glem && !is_glem_added && (_rv != SOC_E_NONE)) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, glif_id_to_add_glem, local_lif_id_to_add_glem, TRUE);
        BCMDNX_IF_ERR_REPORT(rv1);
    }
