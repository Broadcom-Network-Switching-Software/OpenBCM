                              ...
                              int unit)
{
    ...
    /* clear virtual trap resources */
    rv = _bcm_dpp_am_l2_virtual_trap_res_clear(unit);
    BCMDNX_IF_ERR_EXIT(rv);
                ...
                int is_for_delete,bcm_dpp_rx_virtual_traps_t *virtual_traps_p)
{
    ...
    if(virtual_trap_was_set == FALSE)
    {
        if (new_trap_allocated) /*In new trap was allocated by template exchange of MC reserved or Prog trap*/
        {
            ...
            virtual_trap_id = ppc_trap_code - SOC_PPC_TRAP_CODE_RESERVED_MC_0;
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_trap_virtual,SW_STATE_RES_ALLOC_WITH_ID,1,&virtual_trap_id);
            BCMDNX_IF_ERR_EXIT(rv);
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, global_outlif, local_ll_outlif, TRUE);
