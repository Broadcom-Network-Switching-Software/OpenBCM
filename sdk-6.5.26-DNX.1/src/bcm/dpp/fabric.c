                                 ...
                                 bcm_fabric_tdm_editing_t *editing)
{
    ...
    if (SOC_DPP_CONFIG(unit)->tdm.is_bypass) { 
        ...
        /* Add packet CRC should be done when sending to ARAD or Soc_petra packet mode */
                                     ...
                                     int maximum_size)
{
    ...
    if (SOC_IS_DPP_TDM_STANDARD(unit) || !SOC_DPP_CONFIG(unit)->tdm.is_fixed_opt_cell_size) { /* Standard or not fixed Optimize size  */       
...
bcm_petra_fabric_route_tx(
    ...
    uint32 *data_in)
{
    soc_dcmn_fabric_route_t soc_route = { 0 };
    BCMDNX_INIT_FUNC_DEFS;
    ...
    bcm_module_t *destid_array)
{
    ...
    max_mesh_replication = ((SOC_DPP_DEFS_GET(unit, fabric_max_dest_mesh_mc_mode)) + (SOC_DPP_DEFS_GET(unit, nof_cores))); /*ejacobi - changing the behavior for Petra and ARAD.*/
