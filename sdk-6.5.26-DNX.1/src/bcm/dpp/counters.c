...
/* Maximum delay access: 1 sec in Arad. Possible enlargement in Arad to up to 3 minutes  */
                        ...
                        uint64 *counter)
{
    ...
    if (SOC_IS_JERICHO(unit) 
        && (unitData->proc[proc].mode.format == SOC_TMC_CNT_FORMAT_PKTS_AND_PKTS)
        && (unitData->proc[proc].mode.custom_mode_params.set_size == 2)) {
        base = ((set_id - lfirst) * 2) / nof_counters_per_entry; 
    } else {
        ...
        if ((base < unitData->proc[proc].mode.num_counters) || ((base >= base_limit_down) && (base < base_limit_up)))
        {
            if ( ((proc+1) < (SOC_DPP_DEFS_GET(unit, nof_counter_processors))) && 
                (unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP || unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EPNI ||
                 unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP || unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPE_VOQ)
                 && (bit_count != 0) )
            {
    ...
    /* already aligned, used for split engines case */
...
#if (defined(BCM_ARAD_SUPPORT) || defined(BCM_88650_B0))
/* { */
...
int
bcm_dpp_counter_voq_base_val_get(int unit,
                         int core_id,
                         int range_start,
                         int *base_val)
{
    ...
    *base_val = range_start;

    if (!SOC_IS_JERICHO(unit)) {
        /*
         * For ARAD+ we always override the base_val,
         * the max_we is always 0.
         */
        BCM_EXIT;
    }

    /*
     * For VoQ counting, each core shares the same base_val.
     * It is decided by the first counter engine configured on this core.
     */
    for (index = 0; index < unitData->num_counter_procs; index++) {
        if (SOC_TMC_CNT_SRC_TYPE_VOQ == unitData->proc[index].mode.src_type && unitData->proc[index].mode.src_core == core_id) {
            *base_val = unitData->proc[index].mode.mode_eg.base_val;
            break;
        }
    }
                         ...
                         unsigned int *last)
{
    ...
    unsigned int lindex;
    unsigned int index;
    unsigned int is_split_at_end=0, is_split_at_begin=0;
    ...
    BCM_DPP_CORES_ITER(core_id, core_index) {
        for (index = 0; index < unitData->num_counter_procs; index++) {
            if (SOC_TMC_CNT_SRC_TYPE_VOQ == unitData->proc[index].mode.src_type && unitData->proc[index].mode.src_core == core_index) {
                ...
                /*
                 * Check if counter set splitted in two engines
                 */
                is_split_at_end = unitData->proc[index].mode.num_counters % unitData->proc[index].mode.custom_mode_params.set_size;
                is_split_at_end = is_split_at_end ? 1 : 0;
                /*if split at end, then may split at begin*/
                if (is_split_at_end) {
                    is_split_at_begin = (unitData->proc[index].mode.we_val * unitData->proc[index].mode.num_counters) % unitData->proc[index].mode.custom_mode_params.set_size;
                    is_split_at_begin = is_split_at_begin ? 1 : 0;
                }

                /*
                 * In order to support full voq range of system port, more than two processors may needed
                 * For set_size=3, then no split between 3rd and 4th processor.
                 */
                if (is_split_at_end) {
                    is_split_at_end = (unitData->proc[index].mode.we_val + 1) % unitData->proc[index].mode.custom_mode_params.set_size;
                    is_split_at_end = is_split_at_end ? 1 : 0;
                }

                /*
                 * re-calc the lfirst when split:
                 *  |-------------------------------------------|--------------|--------------|
                 *  |           |1|2|           |2|1|           |            |1|2|          |2| ...
                 *  |-------------------------------------------|--------------|--------------|
                 *                |               |
                 ...
                 *                  
                 */
                if (is_split_at_end || is_split_at_begin) {            
                    /* if split beginning, then add one for first */
                    lfirst = ((unitData->proc[index].mode.num_counters * unitData->proc[index].mode.we_val) / unitData->proc[index].mode.custom_mode_params.set_size) + is_split_at_begin;
                    /* if split ending, then add one for count */
                    lcount = ((unitData->proc[index].mode.num_counters * (unitData->proc[index].mode.we_val+1)) / unitData->proc[index].mode.custom_mode_params.set_size) + is_split_at_end - lfirst;
                    lcount = lcount * lscale;
                    lfirst = lfirst * lscale + unitData->proc[index].mode.mode_eg.base_val;
                }
