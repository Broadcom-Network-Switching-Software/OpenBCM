 ...
 */
int
bcm_petra_mirror_destination_erspan_lif_alloc(int unit, uint32 alloc_flags, int *global_outlif, int *local_outlif, int *is_need_add_glem) {
        ...
        if(mirror_dest->flags & BCM_MIRROR_DEST_TUNNEL_WITH_SPAN_ID) {/*span_id*/
            ...
            else if (SOC_IS_JERICHO(unit))/* Jericho and above*/
            {
                ...
                if (update) {
                    ...
                    rv = bcm_petra_mirror_destination_erspan_lif_alloc(unit, alloc_flags, global_outlif, &local_outlif, &is_need_add_glem);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, *global_outlif, local_outlif, TRUE);
                                    ...
                                    bcm_mirror_destination_t *mirror_dest, uint8 set_dest)
{
    ...
    /*  Override up meter pointer, if enabled. Value Range: 0 - 8K-1. Petra only since Meter block is after mirroring in Arad */
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (mirror_dest->packet_control_updates.valid & BCM_MIRROR_PKT_HEADER_UPDATE_FABRIC_HEADER_EDITING) {
            ...
            if (mirror_dest->packet_control_updates.header_fabric.ftmh.lb_ext.valid == TRUE) {
                if (mirror_dest->packet_control_updates.header_fabric.ftmh.lb_ext.lb_key > 255)
                {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("LB Key for FTMH stamping is above the max value!")));
                }
                info.stamping.lb_key_ext_valid = TRUE;
                info.stamping.lb_key = mirror_dest->packet_control_updates.header_fabric.ftmh.lb_ext.lb_key;
            }
        }
    }
                                 ...
                                 bcm_mirror_destination_t *mirror_dest)
{
   ...
   /* in case i ERSPAN mirroring initialize the ERSPAN related data */
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (info.stamping.lb_key_ext_valid == TRUE) {
            mirror_dest->packet_control_updates.valid |= BCM_MIRROR_PKT_HEADER_UPDATE_FABRIC_HEADER_EDITING;
            mirror_dest->packet_control_updates.header_fabric.ftmh.lb_ext.valid = TRUE;
            mirror_dest->packet_control_updates.header_fabric.ftmh.lb_ext.lb_key = info.stamping.lb_key;
        }
