    ...
    int                 is_replace)
{
    ...
    if (is_egress) {
        ...
        /**get operation is lookup in GLEM by _bcm_dpp_global_lif_mapping_egress_global_to_local_get*/
        if (!is_need_add_glem) {
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, trill_global_out_lif_id, trill_local_out_lif_id, TRUE);
