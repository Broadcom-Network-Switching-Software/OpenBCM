                                          ...
                                          bcm_dpp_am_local_out_lif_info_t   *local_out_lif_info) 
{
    ...
    if (is_egress) {
        ...
        /* If lif is mapped, the GLEM entry should write after eedb entry is ready. */
    }
...
_bcm_dpp_lif_glem_access_entry_operation(int unit, int global_lif_id, int egress_lif_id, int is_add) 
{
    ...
    if (SOC_IS_JERICHO(unit) && is_mapped_lif) {
        ...
        if (is_add) {
            rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_add, (unit, global_lif_id, egress_lif_id));
        } else {
            rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_remove, (unit, global_lif_id));
        }
        BCMDNX_IF_ERR_EXIT(rv);
    }
