...
int _bcm_oam_mep_db_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPC_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int icc_ndx, _bcm_oam_ma_name_t *group_name_struct, int do_not_set_mep_db_ccm_interval)
{
    ...
    int         core;
    int bank, entry_idx;
    int using_fake_destination = (SOC_DPP_CONFIG(unit)->pp.oam_ccm_rx_wo_tx && !endpoint_info->ccm_period && !SOC_IS_QAX(unit));
    ...
    if (!_BCM_OAM_BCM_MEP_TYPE_IS_RFC6374(mep_db_entry->mep_type))
    {
        ...
        if (((group_name_struct->name_type == _BCM_OAM_MA_NAME_TYPE_STRING_48_BYTE) ||
             (group_name_struct->name_type == _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE)) && SOC_IS_QAX(unit)) {
            /* QAX only - pointer to extended data header */
            mep_db_entry->is_maid_48 = (group_name_struct->name_type == _BCM_OAM_MA_NAME_TYPE_STRING_48_BYTE) ? TRUE : FALSE;
            mep_db_entry->is_maid_20 = (group_name_struct->name_type == _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE) ? TRUE : FALSE;
    ...
    if (using_fake_destination) {
        ...
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit) &&  PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type)) {
            ...
            /* Set ccm_interval to non-zero value to allow receiveing CCMS when creating new MEP with CCM_period ==0*/
            if(!(endpoint_info->flags & BCM_OAM_ENDPOINT_REPLACE))
            {
                mep_db_entry->do_not_set_interval = 0;
                ccm_period_ms = BCM_OAM_ENDPOINT_CCM_PERIOD_3MS;
...
int _bcm_oam_classifier_mep_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if (endpoint_info->type == bcmOAMEndpointTypeEthernet) {
        ...
        classifier_mep_entry->gport = endpoint_info->gport;
    }
