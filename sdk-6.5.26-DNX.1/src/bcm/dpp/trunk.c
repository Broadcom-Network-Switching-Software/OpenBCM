...
bcm_petra_trunk_find(int unit, bcm_module_t modid, bcm_port_t port, bcm_trunk_t *tid)
{
    ...
    *tid = BCM_TRUNK_INVALID;
    ...
    if (found == 0) 
    {
        ...
        BCMDNX_IF_ERR_VERB_EXIT( BCM_E_NOT_FOUND);
    }
    ...
    uint32* system_port_aggregate)
{
    ...
    /** verify input is system port */
    if (!BCM_GPORT_IS_SYSTEM_PORT(system_port))
    {
        BCMDNX_ERR_EXIT_VERB_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("input system_port 0x%x is not a system port"), system_port));
    }
    sys_port_id = BCM_GPORT_SYSTEM_PORT_ID_GET(system_port);

    /** null check for return pointer */
    BCMDNX_NULL_CHECK(system_port_aggregate);

    /** check trunk is valid */
    TRUNK_CHECK_TID(unit, trunk_id);
    ...
    /* check trunk in use */
    BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.t_info.in_use.get(unit, trunk_id, &trunk_in_use));
    if(!trunk_in_use)
    {
        BCMDNX_ERR_EXIT_VERB_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("trunk ID %d is not in use"), trunk_id));
    }

    /** get first replication of system port in trunk using */
    SOC_PPC_LAG_MEMBER_clear(&lag_member);
    lag_member.sys_port = sys_port_id;
    BCMDNX_IF_ERR_EXIT(soc_dpp_trunk_sw_db_get_first_replication_index(unit, trunk_id, &lag_member, &first_replication_index));
    if (first_replication_index == -1)
    {
        BCMDNX_ERR_EXIT_VERB_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("system port 0x%x is not found in trunk ID %d"), system_port, trunk_id));
    }

    /** build SPA using */
    BCM_SAND_IF_ERR_EXIT(arad_ports_logical_sys_id_build( unit, TRUE, trunk_id, first_replication_index, dummy_sys_phys_id, system_port_aggregate));
