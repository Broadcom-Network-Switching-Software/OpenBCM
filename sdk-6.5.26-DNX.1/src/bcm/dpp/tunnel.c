    ...
    bcm_tunnel_initiator_t *tunnel)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
            ...
            /*
             * directly add GLEM as eedb entry is set by bcm_petra_port_encap_config_set
             */
            rv = _bcm_dpp_lif_glem_access_entry_operation(unit, global_outlif, local_outlif, TRUE);
            BCM_IF_ERROR_RETURN(rv);
...
exit:
    if (is_need_add_glem) {
        int rv0;
        rv0 = _bcm_dpp_lif_glem_access_entry_operation(unit, global_lif_id, local_out_lif, TRUE);
        BCMDNX_IF_ERR_REPORT(rv0);
    }
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, global_lif_id, local_lif_id, TRUE);
        BCMDNX_IF_ERR_REPORT(rv1);
    }
