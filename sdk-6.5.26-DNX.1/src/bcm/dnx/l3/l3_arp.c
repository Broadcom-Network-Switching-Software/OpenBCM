    ...
    bcm_if_t intf)
{
    ...
    if (src_mac_profile != SOURCE_ADDRESS_TABLE_DEFAULT_PROFILE)
    {
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.source_address_table.exchange
                        (unit, 0, &source_address_entry, src_mac_profile, NULL, &new_profile, &first_ref, &last_ref));
    }
    ...
    if (last_ref)
    {
        SHR_IF_ERR_EXIT(dnx_l3_intf_egress_mymac_clear(unit, TRUE, src_mac_profile));
    }
    /**free qos resource*/
    ...
    bcm_l3_egress_t * old_egr)
{
    ...
    if (!((flags & (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN)) == (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN)))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_l3_intf_egress_mymac_clear(unit, TRUE, old_src_mac_addr_profile));
        }
