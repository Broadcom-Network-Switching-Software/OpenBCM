...
#define DNX_NANO_SYNC_LANE_NIF_BASE       64
#define DNX_NANO_SYNC_LANE_FRAMER_BASE    128
#define DNX_NANO_SYNC_LANE_MAS_BASE       192
#define DNX_NANO_SYNC_LANE_VALID_MAX      193

typedef enum synce_rx_clock_signal
{
    synce_rx_clock_signal_valid = 0,
    synce_rx_clock_signal_link = 1,
    synce_rx_clock_signal_valid_and_link = 2,
    synce_rx_clock_signal_na = 3
} synce_rx_clock_valid_sigal_t;

/*
 * }
 */
static int dnx_synce_output_clk_pad_enable_set(
    int unit,
    int synce_index,
    int enable);

static int dnx_fabric_synce_fdr_link_sel_get(
    int unit,
    int synce_index,
    int core_id,
    uint32 *fdr_link);
    ...
    int unit)
{
    ...
    for (synce_index = 0; synce_index < nof_synce_plls; synce_index++)
    {
        ...
        /*
         * Configure the SyncE PAD to output
         */
        SHR_IF_ERR_EXIT(dnx_synce_output_clk_pad_enable_set(unit, synce_index, 1));
    }
...
/**
 * \brief
 *   Set nif/fabric synce clocks mux towards PADS
 * \param [in] unit -
 *   The unit number.
 * \param [in] synce_index -
 *   SyncE index.
 * \param [in] valid_sel -
 *   recovered clock valid
 * \param [in] clock_sel -
 *   recovered clock 
 * \return
 *   See \ref shr_error_e
...
static int
dnx_nanosync_fabric_nif_mux_hw_set(
    int unit,
    int synce_index,
    uint32 valid_sel,
    uint32 clock_sel)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SYNCE_REF_CLK_NIF_FAB_LANE_SEL, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SYNCE_INDEX, synce_index);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_FAB_VALID_SEL, INST_SINGLE, valid_sel);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_FAB_CLOCK_SEL, INST_SINGLE, clock_sel);
...
static int
dnx_nanosync_fabric_nif_mux_hw_get(
    int unit,
    int synce_index,
    uint32 *valid_sel,
    uint32 *clock_sel)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_NIF_FAB_VALID_SEL, INST_SINGLE, valid_sel);

    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_NIF_FAB_CLOCK_SEL, INST_SINGLE, clock_sel);
 ...
 */
static int
dnx_nanosync_nif_lane_gate_enable_set(
    int unit,
    uint32 synce_index,
    bcm_port_t port,
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SYNCE_NIF_CDU_CLK_VALID_CTRL, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_SYNCE_CLK_BLK, ethu_info.ethu_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LANE_ID, ethu_info.first_lane_in_port);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LINK_VALID_SEL, INST_SINGLE,
                                 enable ? synce_rx_clock_signal_valid_and_link : synce_rx_clock_signal_na);
 ...
 */
static int
dnx_nanosync_nif_lane_gate_enable_get(
    int unit,
    uint32 synce_index,
    uint32 global_lane,
    uint32 *enable)
{
    uint32 entry_handle_id;
    uint32 ethu_id, lane_id;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    ethu_id = global_lane / dnx_data_nif.eth.nof_cdu_lanes_in_pm_get(unit);
    lane_id = global_lane % dnx_data_nif.eth.nof_cdu_lanes_in_pm_get(unit);

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SYNCE_NIF_CDU_CLK_VALID_CTRL, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_SYNCE_CLK_BLK, ethu_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LANE_ID, lane_id);
 ...
 */
static int
dnx_nanosync_nif_lane_sel_set(
    int unit,
    uint32 synce_index,
    bcm_port_t port)
{
    dnx_algo_port_ethu_access_info_t ethu_info;
    int first_phy, phy_lane, map_size, swapped_lane;
     ...
     */
    phy_lane = first_phy;
    swapped_lane = lane2serdes[first_phy].rx_id;

    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    /*
     * For ethL1,ethL2 and flexphy, the base offset is 64
     */
    phy_lane += DNX_NANO_SYNC_LANE_NIF_BASE;
    swapped_lane += DNX_NANO_SYNC_LANE_NIF_BASE;
    /*
     * commit the value to HW
     */
    SHR_IF_ERR_EXIT(dnx_nanosync_fabric_nif_mux_hw_set(unit, synce_index, phy_lane, swapped_lane));
...
static int
dnx_nanosync_fabric_mas_sel_set(
    int unit,
    int synce_index,
    int fsrd_block)
{
    ...
    mas_id = fsrd_block / DNX_NOF_FSRD_PER_MAS;
    mas_id += DNX_NANO_SYNC_LANE_MAS_BASE;
    SHR_IF_ERR_EXIT(dnx_nanosync_fabric_nif_mux_hw_set(unit, synce_index, mas_id, mas_id));
...
/**
 * \brief
 *   Get output clock state(output or input).
 * \param [in] unit -
 *   The unit number.
 * \param [in] synce_index -
 *   Master or Slave SyncE.
 ...
 */
static int
dnx_nanosync_output_clk_pad_enable_get(
    int unit,
    int synce_index,
    uint32 *enable)
{
    uint32 valid_sel, clock_sel;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_nanosync_fabric_nif_mux_hw_get(unit, synce_index, &valid_sel, &clock_sel));
    if (valid_sel >= DNX_NANO_SYNC_LANE_NIF_BASE && valid_sel < DNX_NANO_SYNC_LANE_FRAMER_BASE)
    {
        valid_sel -= DNX_NANO_SYNC_LANE_NIF_BASE;
        SHR_IF_ERR_EXIT(dnx_nanosync_nif_lane_gate_enable_get(unit, synce_index, valid_sel, enable));
    }
    else if (valid_sel >= DNX_NANO_SYNC_LANE_FRAMER_BASE && valid_sel <= DNX_NANO_SYNC_LANE_VALID_MAX)
    {
        *enable = 1;
    }
    else
 ...
 */
static int
dnx_nanosync_nif_clk_port_sel_get(
    int unit,
    uint32 synce_index,
    bcm_port_t * port)
{
    ...
    if (valid_sel < DNX_NANO_SYNC_LANE_NIF_BASE || valid_sel >= DNX_NANO_SYNC_LANE_MAS_BASE)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "valid lane %d is unexpected!\n", valid_sel);
    }

    if (valid_sel >= DNX_NANO_SYNC_LANE_FRAMER_BASE)
    {
        valid_sel -= DNX_NANO_SYNC_LANE_FRAMER_BASE;
    }
    else if (valid_sel >= DNX_NANO_SYNC_LANE_NIF_BASE)
    {
        valid_sel -= DNX_NANO_SYNC_LANE_NIF_BASE;
    }
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phy_to_logical_get(unit, valid_sel, 0, 0, port));
    ...
    uint32 *fabric_link)
{
    ...
    if (valid_sel < DNX_NANO_SYNC_LANE_MAS_BASE)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "valid lane %d is unexpected!\n", valid_sel);
    }

    SHR_IF_ERR_EXIT(dnx_fabric_synce_fdr_link_sel_get(unit, synce_index, 0, fabric_link));
    ...
    bcm_port_t port)
{
    ...
    if ((dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_blocks_exist) ||
        ...
        !dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        ...
        if (dnx_data_device.general.feature_get(unit, dnx_data_device_general_multi_core)
            && (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, MGMT))
    ...
    uint32 *fabric_link)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_nanosync_fabric_link_get(unit, synce_index, fabric_link));
    }
    else
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SYNCE_NIF_FAB_CTRL, &entry_handle_id));
    ...
    uint32 *is_nif)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_nanosync_fabric_nif_mux_hw_get(unit, synce_index, &valid_sel, &clock_sel));
        *is_nif = valid_sel >= DNX_NANO_SYNC_LANE_MAS_BASE ? 0 : 1;
    }
    else
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SYNCE_NIF_FAB_CTRL, &entry_handle_id));
...
/**
 * \brief
 *   Get the Fabric link ID in FDR block.
 * \param [in] unit -
 *   The unit number.
 * \param [in] synce_index -
 *   SyncE index.
...
static int
dnx_fabric_synce_fdr_link_sel_get(
    int unit,
    int synce_index,
    int core_id,
    uint32 *fdr_link)
{
    ...
    if (synce_index == DBAL_ENUM_FVAL_SYNCE_INDEX_MASTER_SYNCE)
    {
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MASTER_CLK_SEL, INST_SINGLE, fdr_link);
    }
    else
    {
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_SLAVE_CLK_SEL, INST_SINGLE, fdr_link);
    }
    ...
    int fmac_id)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MAS_ID, fsrd_block / DNX_NOF_FSRD_PER_MAS);

    if (synce_index == DBAL_ENUM_FVAL_SYNCE_INDEX_MASTER_SYNCE)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MASTER_FMAC_INSTANCE, INST_SINGLE, fmac_id);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SLAVE_FMAC_INSTANCE, INST_SINGLE, 0);
    }
    else
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SLAVE_FMAC_INSTANCE, INST_SINGLE, fmac_id);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MASTER_FMAC_INSTANCE, INST_SINGLE, 0);
    }
    ...
    bcm_port_t port)
{
    ...
    fmac_id = fsrd_block % DNX_NOF_FSRD_PER_MAS;
    fsrd_link = fabric_link % DNX_NOF_LINKS_IN_FSRD;
    fdr_link = fabric_link % DNX_NOF_LINKS_PER_CORE;

    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        SHR_IF_ERR_EXIT(dnx_nanosync_fabric_mas_sel_set(unit, synce_index, fsrd_block));
    }
    else
    {
    ...
    if (!dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_no_fdr_cfg))
    {
...
static int
dnx_fabric_synce_div_set_verify(
    int unit,
    int synce_index,
    int divider)
{
    ...
    bcm_port_t port)
{
    ...
    {
        table_id = DBAL_TABLE_SYNCE_REF_CLK_NIF_CLU_LANE_SEL;
    }
    else if (ethu_info.imb_type == imbDispatchTypeImb_cdu)
    {
        table_id = DBAL_TABLE_SYNCE_REF_CLK_NIF_CDU_LANE_SEL;
    }
    /*
     * alloc DBAL table handle
     */
 ...
 */
static int
dnx_synce_output_clk_pad_enable_set(
    int unit,
    int synce_index,
    int enable)
{
    ...
    if (!dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, field, INST_SINGLE, !enable);
        /*
         * commit the value
         */
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
static int
dnx_synce_output_clk_pad_enable_get(
    int unit,
    int synce_index,
    uint32 *enable)
{
     ...
     */
    field = synce_index ? DBAL_FIELD_CLK1_OUTPUT_ENABLE : DBAL_FIELD_CLK0_OUTPUT_ENABLE;
    dbal_value_field32_request(unit, entry_handle_id, field, INST_SINGLE, enable);
    /*
     * get the value
     */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int *nif_synce_div)
{
    ...
    {
        if (synce_cfg.stg0_mode == DNX_STAGE0_SDM_FRAC_DIV_MODE)
        {
            *pm_synce_div = synce_cfg.sdm_val / 256;
            *nif_synce_div = 1;
        }
        else if (synce_cfg.stg1_mode == DNX_NIF_SYNCE_PM_DIV_11)
        {
            *pm_synce_div = 11;
            *nif_synce_div = 1;
        }
    ...
    int enable)
{
    ...
    {
        table_id = DBAL_TABLE_SYNCE_ENABLE_CTRL;
    }
...
dnx_nif_synce_pll_div_get(
    int unit,
    bcm_port_t port,
    DNXC_SERDES_REF_CLOCK_TYPE ref_clock_out,
    uint32 *pdiv,
    uint32 *mdiv,
    uint32 *ndiv,
    uint32 *fref,
        ...
        *is_bypass = 0;
        SHR_IF_ERR_EXIT(portmod_port_speed_config_get(unit, port, &speed_config));
        {
                                                     ...
                                                     || speed_config.fec == PORTMOD_PORT_PHY_FEC_RS_544_2XN))
            {
                ...
                *pdiv = 10;
                *fref = 20;
                if (DNXC_SERDES_REF_CLOCK_156_25 == ref_clock_out)
                {
                    *mdiv = 17;
                    *ndiv = 220;
                }
                else if (DNXC_SERDES_REF_CLOCK_125 == ref_clock_out)
                {
                    *mdiv = 17;
                    *ndiv = 176;
                }
                else if (DNXC_SERDES_REF_CLOCK_25 == ref_clock_out)
                {
                    *mdiv = 85;
                    *ndiv = 176;
                }
    ...
    DNXC_SERDES_REF_CLOCK_TYPE ref_clock_out)
{
    ...
    {
        /*
         * Select the global CDU lane in ECI
         */
        SHR_IF_ERR_EXIT(dnx_nanosync_nif_lane_sel_set(unit, synce_index, port));
        /*
         * Enable the ref clock towards to the nanosync
         */
        SHR_IF_ERR_EXIT(dnx_nanosync_nif_lane_gate_enable_set(unit, synce_index, port, 1));
    }
    else
    {
        /*
         * Select the lane in CDU CLU
         */
    ...
    if (!dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_no_nif_div_cfg))
    {
     ...
     * Enable synce
     */
    SHR_IF_ERR_EXIT(dnx_nif_synce_enable_set(unit, core_id, is_mgu, 1));
    ...
    if (!dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync) &&
        dnx_data_synce.general.cfg_get(unit, synce_index)->squelch_enable)
    {
...
dnx_nif_synce_clk_div_get(
    int unit,
    uint32 synce_index,
    DNXC_SERDES_REF_CLOCK_TYPE * clk_div)
{
    uint32 entry_handle_id;
    uint32 mdiv, ndiv;
    ...
    switch (mdiv)
    {
        ...
        case 0:
             ...
             */
        case 1:
            *clk_div = DNXC_SERDES_REF_CLOCK_25;
            break;
        case 20:
            *clk_div = DNXC_SERDES_REF_CLOCK_156_25;
            break;
        case 17:
        {
            {
                if (ndiv == 176)
                {
                    *clk_div = DNXC_SERDES_REF_CLOCK_125;
                }
                else if (ndiv == 220)
                {
                    *clk_div = DNXC_SERDES_REF_CLOCK_156_25;
    ...
    bcm_port_t * port)
{
    ...
    int is_end, phy_lane, first_phy, nof_lanes;
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        SHR_IF_ERR_EXIT(dnx_nanosync_nif_clk_port_sel_get(unit, synce_index, port));
    }
    else
    {
        /*
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_global_ethu_id_lane_info_get(unit, synce_clk_blk_id, &first_phy, &nof_lanes));
        /*
         * Get serdes to lane map
         */
        SHR_IF_ERR_EXIT(dnx_algo_lane_map_serdes_to_lane_map_get
                        (unit, DNX_ALGO_LANE_MAP_NIF_SIDE, map_size, serdes2lane));
    ...
    portmod_port_synce_clk_ctrl_t * config)
{
    ...
    if (DNXC_SERDES_REF_CLOCK_25 != divider)
    {
...
/**
 * \brief
 *   get the SyncE clock configurations in SDM mode of TSCBH_fe
 * \param [in] unit -
 *   The unit number.
 * \param [in] port -
 *   Source link for SyncE
 ...
 */
static int
dnx_synce_tscbh_fe_sdm_config_get(
    int unit,
    bcm_port_t port,
    uint32 divider,
    portmod_port_synce_clk_ctrl_t * config)
{
    ...
    SHR_IF_ERR_EXIT(portmod_port_speed_config_get(unit, port, &speed_config));
    {
         ...
         * OTN speed
         */
        if (speed_config.speed == 10709)
        {
            /*
             * VCO: 26773.06329114,OS: 2.5  SDM_DIVISOR_26P773G_VCO  13707
             */
            config->sdm_val = 5483;
        }
        else if (speed_config.speed == 11049)
        {
            /*
             * VCO: 27622.76785714, OS: 2.5 SDM_DIVISOR_27P622G_VCO  14142
             */
            config->sdm_val = 5657;
        }
        else if (speed_config.speed == 11095)
        {
            /*
             * VCO: 27739.31962025, OS: 2.5 SDM_DIVISOR_27P739G_VCO  14202
             */
            config->sdm_val = 5681;
        }
        else if (speed_config.speed == 25781)
        {
            /*
             * VCO: 25781.65149137, OS: 1 DIVISOR_25GP78125_VCO  13200
             */
            config->sdm_val = 13200;
        }
        else if (speed_config.speed == 26562)
        {
            /*
             * VCO: 26562.91365777, OS: 1 SDM_DIVISOR_26G_VCO  13600
             */
            config->sdm_val = 13600;
        }
        else if (speed_config.speed == 27952)
        {
            /*
             * VCO: 27952.36861161, OS: 1  SDM_DIVISOR_27P952G_VCO  14311
             */
            config->sdm_val = 14311;
        }
        else if (speed_config.speed == 27952)
        {
            /*
        ...
        else if (speed_config.speed == 28076)
        {
            /*
             * VCO: 28076.17699115, OS: 1  SDM_DIVISOR_28P076G_VCO  14375
             */
            config->sdm_val = 14375;
        }
        /*
         * Next is PAM4 speed
         */
        else if (speed_config.speed == 53125)
        {
            /*
             * VCO: 26562.91365777, OS: 1  SDM_DIVISOR_26G_VCO  13600
             */
        ...
        else if (speed_config.speed == 55904)
        {
            /*
             * VCO: 27952.36861161, OS: 1  SDM_DIVISOR_27P952G_VCO  14311
             */
        ...
        else if (speed_config.speed == 56152)
        {
            /*
             * VCO: 28076.17699115, OS: 1  SDM_DIVISOR_28P076G_VCO  14375
             */
    ...
    portmod_port_synce_clk_ctrl_t * config)
{
    ...
    switch (pm_type)
    {
        case portmodDispatchTypePm8x50:
            ...
            break;
#ifdef PORTMOD_PM8X50_FLEXE_GEN2_SUPPORT
        case portmodDispatchTypePm8x50_flexe_gen2:
            SHR_IF_ERR_EXIT(dnx_synce_tscbh_fe_sdm_config_get(unit, port, divider, config));
            break;
#endif
...
/**
 * \brief
 *   output clock force enable or not, this will change the pad behaviors.
 * \param [in] unit -
 *   The unit number.
 * \param [in] synce_index -
 *   Master or Slave SyncE.
 ...
 */
static int
dnx_synce_output_clk_force_enable_set(
    int unit,
    int synce_index,
    int enable)
{
     ...
     * alloc DBAL table handle
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SYNCE_OUTPUT_CLK_FORCE_CTRL, &entry_handle_id));
    /*
     * set value fields
     */
    field = synce_index ? DBAL_FIELD_CLK1_OUTPUT_FORCE_ENABLE : DBAL_FIELD_CLK0_OUTPUT_FORCE_ENABLE;
    dbal_entry_value_field32_set(unit, entry_handle_id, field, INST_SINGLE, (enable != 0));
    /*
     * commit the value
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/**
 * \brief
 *   Disable synce output clock and valid signal,this is for PAD 
 *   configuration(Q2A/J2P) or synce pll configuration(JR2/J2C)
 * \param [in] unit -
 *   The unit number.
 * \param [in] synce_index -
 *   Master or Slave SyncE.
 * \param [in] enable -
 *   synce clock output enable or not.
 * \return
 *   See \ref shr_error_e
 * \remark
 *   None.
 ...
 */
static int
dnx_synce_nif_fab_synce_clk_output_enable_set(
    int unit,
    int synce_index,
    int enable)
{
    SHR_FUNC_INIT_VARS(unit);
    /*
     * Power down SyncE PLL
     */
    if (dnx_data_pll.synce_pll.feature_get(unit, dnx_data_pll_synce_pll_present))
    {
        SHR_IF_ERR_EXIT(soc_dnx_synce_pll_power_enable_set(unit, synce_index, enable));
    }
    else if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        /*
         * For J2X, do nothing when enable since it will be enabled in port set.
         */
    }
    else
    {
        if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_ouput_force_is_supported))
        {
            SHR_IF_ERR_EXIT(dnx_synce_output_clk_force_enable_set(unit, synce_index, (enable != 0)));
        }

        /*
         * Configure the SyncE PAD to output(enable) or input(disable)
         */
        SHR_IF_ERR_EXIT(dnx_synce_output_clk_pad_enable_set(unit, synce_index, (enable != 0)));
 ...
 */
static int
dnx_synce_nif_fab_synce_clk_output_enable_get(
    int unit,
    int synce_index,
    uint32 *enable)
{
    SHR_FUNC_INIT_VARS(unit);
    /*
     * Get SyncE PLL Power down state 1 is power up; 0 is power down
     */
    if (dnx_data_pll.synce_pll.feature_get(unit, dnx_data_pll_synce_pll_present))
    {
        SHR_IF_ERR_EXIT(soc_dnx_synce_pll_power_enable_get(unit, synce_index, enable));
    }
    else if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        SHR_IF_ERR_EXIT(dnx_nanosync_output_clk_pad_enable_get(unit, synce_index, enable));
    }
    else
    {
        /*
         * Get the PAD state(1 output or 0 is input)
         */
        SHR_IF_ERR_EXIT(dnx_synce_output_clk_pad_enable_get(unit, synce_index, enable));
...
/**
 * \brief
 *   Verify function for dnx_synce_port_set
 * \param [in] unit -
 *   The unit number.
 * \param [in] synce_index -
 *   Master or slave synce
    ...
    bcm_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ext_txpi_enable_get(unit, port, &is_ext_txpi_en));
    if (is_ext_txpi_en)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "SyncE source clock port can't be configured on TXPI enabled port!\n");
    }
    ...
    bcm_port_t port)
{
    ...
    if (port == DNX_INVALID_SYNCE_SRC_PORT)
    {
        SHR_IF_ERR_EXIT(dnx_synce_nif_fab_synce_clk_output_enable_set(unit, synce_index, 0));
    }
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_synce_port_set_verify(unit, synce_index, port));
        ...
        /*
         * Configure the SyncE output enable
         */
        SHR_IF_ERR_EXIT(dnx_synce_nif_fab_synce_clk_output_enable_set(unit, synce_index, 1));
    }
    ...
    bcm_port_t * port)
{
    ...
    uint32 synce_clk_state = 1;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_synce_nif_fab_synce_clk_output_enable_get(unit, synce_index, &synce_clk_state));
    if (synce_clk_state == 0)
    {
        *port = DNX_INVALID_SYNCE_SRC_PORT;
        SHR_EXIT();
    }
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_no_sdm_mode))
    {
        if (is_nif_synce)
        {
            SHR_RANGE_VERIFY(divider, dnx_data_synce.general.nif_div_min_get(unit),
                             dnx_data_synce.general.nif_div_max_get(unit), _SHR_E_PARAM,
                             "SyncE divider is out of bound!\n");
            SHR_IF_ERR_EXIT(dnx_nif_synce_clk_div_set(unit, synce_index, (DNXC_SERDES_REF_CLOCK_TYPE) divider));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_fabric_synce_div_set_verify(unit, synce_index, divider));
    ...
    int *divider)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_no_sdm_mode))
    {
        ...
        if (is_nif_synce)
        {
            SHR_IF_ERR_EXIT(dnx_nif_synce_clk_div_get(unit, synce_index, (DNXC_SERDES_REF_CLOCK_TYPE *) divider));
        }
        else
        {
    ...
    {
        *divider = DNXC_SERDES_REF_CLOCK_25;
    }
exit:
    SHR_FUNC_EXIT;
}
