    ...
    int *preserve_dscp_mode)
{
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_vsi_profile_get(unit, gport, (uint32 *) &in_lif_profile, &result_type, intf_type));
    ...
    dnx_port_pp_egress_port_info_t * egress_port_info)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_per_pp_port_get(unit,
                                                                gport_info.internal_port_pp_info.pp_port[pp_port_index],
                                                                gport_info.internal_port_pp_info.core_id[pp_port_index],
                                                                &old_esem_cmd));
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_is_predefine(unit, old_esem_cmd, &is_predefined));
        ...
        if (!is_predefined)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "PP port %d on core %d, has uncleared ESEM configuration,"
                         "please clear the ESEM configuration before calling this API",
                         gport_info.internal_port_pp_info.pp_port[pp_port_index],
                         gport_info.internal_port_pp_info.core_id[pp_port_index]);
        }

        SHR_IF_ERR_EXIT(dnx_port_pp_egress_hw_per_pp_port_set
                        (unit, gport_info.internal_port_pp_info.pp_port[pp_port_index],
                         gport_info.internal_port_pp_info.core_id[pp_port_index], egress_port_info));
    }
    ...
    int switch_header_type)
{
    ...
    switch (switch_header_type)
    {
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_RCH_SRV6_USP_PSP:
        {
            ...
            /*
             * Set port default AC for eth ports  (recycle and Eventor port Default ESEM CMD should be "no action")
             */
        ...
        {
            egress_port_info.egress_pp_filters_enable = FALSE;
            egress_port_info.trap_context_port_profile = DBAL_ENUM_FVAL_ETPP_TRAP_CONTEXT_PORT_PROFILE_TDM_STAMP_MC_CUD;
            break;
        }
        case BCM_SWITCH_PORT_HEADER_TYPE_RCH_0:
        {
    ...
    uint8 tpid_ref_dealloc)
{
    ...
    if (tpid_ref_dealloc)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_per_pp_port_get(unit, pp_port, core_id, &old_esem_cmd));
        /** If the old ESEM command is not one of the default commands then there is an uncleared configuration */
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_is_predefine(unit, old_esem_cmd, &is_predefined));
        if (!is_predefined)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "PP port %d on core %d, has uncleared ESEM configuration,"
                         "please clear the ESEM configuration before calling this API", pp_port, core_id);
        }
    }
    SHR_IF_ERR_EXIT(dnx_port_pp_egress_hw_per_pp_port_set(unit, pp_port, core_id, &egress_port_info));
    if (is_last)
    {
    ...
    uint32 header_type)
{
    ...
    switch (header_type)
    {
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_RAW:
        {
            ...
            {
                port_termination_ptc_profile = DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_COE;
                prt_qualifier_raw = PRT_TCAM_APP_DB_COE;
            }
            else
            {
                port_termination_ptc_profile = DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_RAW;
...
* \param [out] is_large_group - indicates the gport has large group
    ...
    dbal_table_field_info_t * dbal_info)
{
    ...
    if (rv == _SHR_E_NOT_FOUND)
    {
        if (dbal_tables_field_info_get_no_err
            (unit, gport_hw_resources->inlif_dbal_table_id, DBAL_FIELD_GLOBAL_IN_LIF_AND_SOME_PMF_DATA, 0,
             gport_hw_resources->inlif_dbal_result_type, 0, dbal_info) == _SHR_E_NOT_FOUND)
        {
...
{
    dbal_table_field_info_t dbal_info;
    uint32 is_large_group = 0;

    SHR_FUNC_INIT_VARS(unit);

    /** Verify parameters for flags and gport information */
    sal_memset(&dbal_info, 0, sizeof(dbal_table_field_info_t));
    SHR_IF_ERR_EXIT(dnx_port_pp_wide_data_common_verify(unit, gport_hw_resources, flags, &is_large_group, &dbal_info));
    ...
    if (is_large_group &&
        ...
        (COMPILER_64_LO(data) > SAL_UPTO_BIT(DNX_PORT_PP_PMF_ADDITIONAL_DATA_SIZE)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "data={0x%X, 0x%X}. max allowed=0x%X\n",
                     COMPILER_64_HI(data), COMPILER_64_LO(data), SAL_UPTO_BIT(DNX_PORT_PP_PMF_ADDITIONAL_DATA_SIZE));
    }
...
{
    uint32 entry_handle_id;
    dbal_table_field_info_t field_info_0, field_info_1, field_info_glob_inlif;
    ...
    rv = dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_GLOBAL_IN_LIF_AND_SOME_PMF_DATA, 0, type, 0,
                                           &field_info_glob_inlif);
    if (rv == _SHR_E_NONE)
    {
        data_mask = 0;
        ...
        {
            SHR_BITSET_RANGE(&data_mask, 0, DNX_PORT_PP_PMF_ADDITIONAL_DATA_SIZE);
        }
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_GLOBAL_IN_LIF_AND_SOME_PMF_DATA, INST_SINGLE, &data_tmp));
        if (glob_lif_data != NULL)
        {
            *glob_lif_data = data_tmp;
        }
    ...
    uint32 data)
{
    ...
    int type;
    int local_in_lif;
    uint32 value = 0, global_lif_data = 0;
    shr_error_e rv;
    ...
    if (rv == _SHR_E_NONE)
    {
        ...
        if (gport_hw_resources.global_in_lif != DNX_ALGO_GPM_LIF_INVALID)
        {
            ...
            /** GLOBAL_IN_LIF mask  */
            SHR_BITSET_RANGE(&data_mask, DNX_PORT_PP_PMF_ADDITIONAL_DATA_SIZE, field_info_glob_inlif.field_nof_bits);
            global_lif_data = (global_lif_data & data_mask);
            /** PMF additional data mask  */
            SHR_BITSET_RANGE(&data_mask, 0, DNX_PORT_PP_PMF_ADDITIONAL_DATA_SIZE);
            global_lif_data = (global_lif_data | (data & data_mask));
        }
        else
        {
            global_lif_data = data;
        }
        dbal_entry_value_field32_set(unit, entry_handle_id,
                                     DBAL_FIELD_GLOBAL_IN_LIF_AND_SOME_PMF_DATA, INST_SINGLE, global_lif_data);
    }
    else
    {
        /*
