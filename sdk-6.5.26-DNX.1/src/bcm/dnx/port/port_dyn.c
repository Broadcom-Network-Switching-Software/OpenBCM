...
#define DNX_PORT_ADD_IF_INFO_SUPPORTED_FLAGS  (BCM_PORT_ADD_FLEXE_PROTECTION)
    ...
    int is_ilkn_over_fabric)
{
    ...
    if (((!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, FLEXE, OTN_PHY, FRAMER_MGMT))) &&
         ...
         && !DNX_ALGO_PORT_TYPE_IS_SCH_ONLY(unit, port_info)) || is_ilkn_over_fabric)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Only NIF port (excluding ELK / STIF/ ILKNoFabric), RCY, mirror-recycle, FlexE client and SCH only can be dymamically added or removed. port=%d\n",
                     port);
    }
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_port_mapping_info_t * mapping_info)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT, FRAMER_MGMT))
