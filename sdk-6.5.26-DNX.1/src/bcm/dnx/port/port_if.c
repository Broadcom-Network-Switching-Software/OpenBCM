    ...
    bcm_port_resource_t * resource)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF,
                     ...
                     DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_L1 | DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FRAMER_MGMT, &all_ports));
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
    ...
    bcm_port_resource_t * resource)
{
    ...
    if (resource[resource_index].flags & BCM_PORT_RESOURCE_ASYMMETRICAL)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_has_speed(unit, resource[resource_index].port, DNX_ALGO_PORT_SPEED_F_RX,
                                                &has_speed));
        if (!has_speed && resource[resource_index].rx_speed != 0)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_IMB(unit, port_info, FALSE))
        {
            ...
            if (DNX_ALGO_PORT_TYPE_IS_NIF
                (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, FLEXE, STIF, L1, MGMT, OTN_PHY, FRAMER_MGMT))
    ...
    dnx_port_resource_port_info_t * ports_info)
{
    ...
    {
        if (DNX_ALGO_PORT_TYPE_IS_NIF
            (unit, ports_info[resource_index].port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, FLEXE, OTN_PHY)))
        {
    ...
    if (!dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        ...
        for (resource_index = 0; resource_index < nport; ++resource_index)
        {
            if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, ports_info[resource_index].port_info,
                                          DNX_ALGO_PORT_TYPE_INCLUDE(STIF, L1)))
            {
    ...
    int enable)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(FLEXE, OTN_PHY, FRAMER_MGMT, MGMT)))
    {
