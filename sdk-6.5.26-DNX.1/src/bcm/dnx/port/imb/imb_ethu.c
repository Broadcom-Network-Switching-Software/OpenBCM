...
#include "../nif/dnx_port_nif_link_list_internal.h"

#include "../nif/dnx_port_nif_ofr_internal.h"
...
#define IMB_ETHU_IS_RX_RMC_PORT(unit, port_info) (DNX_ALGO_PORT_TYPE_IS_ING_TM(unit, port_info) ||                  \
                                                  DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, FALSE))
 ...
 * \param [in] ethu_info - ptr to ETHU ID info struct
 *
 * \return
 *   int - see _SHR_E_*
 *
    ...
    dnx_algo_port_ethu_access_info_t * ethu_info)
{
    ...
    int first_lane_in_core = 0, first_lane_in_ethu = 0;
    int lane_idx;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_local_to_global_get
                    (unit, ethu_info->core, ethu_info->ethu_id_in_core, &(ethu_info->ethu_id)));

    SHR_IF_ERR_EXIT(dnx_algo_global_ethu_id_lane_info_get(unit, ethu_info->ethu_id, &first_lane, &nof_lanes_in_ethu));
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, first_lane, &first_lane_in_core));

    SHR_IF_ERR_EXIT(dnx_algo_global_ethu_id_lane_info_get
                    (unit, ethu_info->ethu_id, &first_lane_in_ethu, &ethu_info->nof_lanes_in_ethu));
    /*
     * Disable all lanes in the given ETHU
     */
    for (lane_idx = 0; lane_idx < nof_lanes_in_ethu; lane_idx++)
    {
        ethu_info->first_lane_in_port = first_lane_in_ethu + lane_idx;
        ethu_info->lane_in_core = first_lane_in_core + lane_idx;
        SHR_IF_ERR_EXIT(imb_ethu_tx_data_to_pm_enable_hw_set(unit, ethu_info, 0));
    }
...
/**
 * \brief - calculated the new ETHU MAC mode.
 * Suitable for both add and remove sequences.
 * see .h file
 */
int
imb_ethu_port_mac_mode_calc(
    int unit,
...
/**
 * \brief - get all  ports on the same MAC
 *
 * \param [in] unit - chip unit id.
    ...
    bcm_pbmp_t * mac_ports)
{
    ...
    dnx_algo_port_ethu_access_info_t ethu_info, port_i_ethu_info;
    SHR_FUNC_INIT_VARS(unit);
    ...
    /*
     * iterate over all ETHU ports and check if MAC is is the same as given port
     */
    BCM_PBMP_ITER(ethu_ports, port_i)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_ethu_access_info_get(unit, port_i, &port_i_ethu_info));
        if (port_i_ethu_info.mac_id == ethu_info.mac_id)
        {
...
/**
 * \brief - get all ETHU ports on the same MAC mode config
 *
 * \param [in] unit - chip unit id.
 * \param [in] port - logical port
 * \param [out] mac_ports - bitmap of ports on the same mac
 ...
 */
int
imb_ethu_mac_mode_config_ports_get(
    int unit,
    bcm_port_t port,
    bcm_pbmp_t * mac_ports)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_ethu_access_info_get(unit, port_i, &port_i_ethu_info));
        if (port_i_ethu_info.mac_mode_config_id == ethu_info.mac_mode_config_id)
        {
            BCM_PBMP_PORT_ADD(*mac_ports, port_i);
        }
    }
    ...
    int *mac_nof_ports)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_mac_ports_get(unit, port, &mac_ports));
...
int
imb_ethu_mac_mode_config_nof_ports_get(
    int unit,
    bcm_port_t port,
    int *mac_mode_config_nof_ports)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_mac_mode_config_ports_get(unit, port, &mac_ports));
    ...
    int weight)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_hw_set(unit, ethu_info, dbal_sch_prio, dbal_sch_weight));
    ...
    int weight)
{
     ...
     *
     */
    SHR_IF_ERR_EXIT(imb_ethu_rmc_low_prio_scheduler_config_hw_set(unit, &ethu_info, rmc_id, dbal_sch_weight));
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 *active)
{
    ...
    SHR_IF_ERR_EXIT(imb_common_dbal_sch_prio_from_bcm_sch_prio_get(unit, sch_prio, &dbal_sch_prio));
    SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_hw_get(unit, &ethu_info, dbal_sch_prio, &weight));
    ...
    dnx_algo_port_rmc_info_t * rmc)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_ethu_access_info_get(unit, port, &ethu_info));
    SHR_IF_ERR_EXIT(imb_ethu_port_rmc_thr_after_ovf_hw_set(unit, &ethu_info, rmc->rmc_id, rmc->thr_after_ovf));
    ...
    int is_map_valid)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_rmc_to_lane_map_hw_set
                    (unit, &ethu_info, is_map_valid ? ethu_info.first_lane_in_port : 0, rmc));
    ...
    int flush_enable)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_ethu_access_info_get(unit, port, &ethu_info));
    SHR_IF_ERR_EXIT(imb_ethu_tx_egress_flush_hw_set(unit, &ethu_info, flush_enable));
exit:
    SHR_FUNC_EXIT;
}
/**
 * \brief - Additional sequences for egress flush rollback
 *
 */
static int
imb_ethu_port_er_tx_egress_flush_journal_rollback(
    int unit,
 ...
 */
static int
imb_ethu_port_er_tx_egress_flush_journal_set(
    int unit,
    bcm_port_t port,
    int orig_enable,
    int curr_enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_log_entry(unit,
                                                        ...
                                                        (uint8 *) (&reset_credit),
                                                        sizeof(int),
                                                        &imb_ethu_port_er_tx_egress_flush_journal_rollback, FALSE));
 ...
 */
static int
imb_ethu_port_er_tx_egress_flush_set(
    int unit,
    bcm_port_t port,
    int flush_enable)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_ethu_tx_egress_flush_hw_get(unit, &ethu_info, &orig_enable));
    /*
     * Register sequences after rollback egress flush
     */
    SHR_IF_ERR_EXIT(imb_ethu_port_er_tx_egress_flush_journal_set(unit, port, orig_enable, flush_enable));
    /*
     * Configure flush
     */
    SHR_IF_ERR_EXIT(imb_ethu_tx_egress_flush_hw_set(unit, &ethu_info, flush_enable));
    ...
    bcm_port_t port)
{
    ...
    for (priority_group = 0; priority_group < nof_priority_groups; ++priority_group)
    {
        ...
        if (!dnx_data_nif.scheduler.feature_get(unit, dnx_data_nif_scheduler_dynamic_scheduling))
        {
            ...
            SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_hw_get
                            (unit, &ethu_info, dbal_sch_prio, &priority_weight[priority_group]));
        }
    }
    weight_arr_set = TRUE;
    ...
    for (priority_group = 0; priority_group < nof_priority_groups; ++priority_group)
    {
       ...
       /**
         ...
         */
        SHR_IF_ERR_EXIT(imb_ethu_port_rx_drop_data_hw_set(unit, &ethu_info, rmc.rmc_id, TRUE));
        ...
        if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_rmc_flush_support))
        {
             ...
             */
            SHR_IF_ERR_EXIT(imb_ethu_port_rx_rmc_counter_get(unit, &ethu_info, rmc.rmc_id, &counter));
        ...
        if (!rmc_empty)
        {
            ...
            SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_hw_set
                            (unit, &ethu_info, dbal_sch_prio, DBAL_ENUM_FVAL_NIF_SCH_WEIGHT_WEIGHT_0));
            ...
            if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_rmc_flush_support))
            {
                ...
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_nmg_flush_context_enable_hw_set
                                (unit, &ethu_info, dbal_sch_prio, TRUE));
                 ...
                 */
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_rmc_flush_mode_enable_hw_set(unit, &ethu_info, rmc.rmc_id, TRUE));

                /*
                 * Start flushing
                 */
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_flush_enable_hw_set(unit, &ethu_info, TRUE));

                /*
                 * Wait until RMC counter is empty
                 */
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_rmc_counter_polling(unit, &ethu_info, rmc.rmc_id));
            }
            else
            {
                /*
        ...
        if (!rmc_empty)
        {
            if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_rmc_flush_support))
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_rmc_flush_mode_enable_hw_set(unit, &ethu_info, rmc.rmc_id, FALSE));

                /*
                 * Stop flushing
                 */
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_flush_enable_hw_set(unit, &ethu_info, FALSE));
                ...
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_nmg_flush_context_enable_hw_set
                                (unit, &ethu_info, dbal_sch_prio, FALSE));
            }
         ...
         */
        SHR_IF_ERR_EXIT(imb_ethu_port_rx_drop_data_hw_set(unit, &ethu_info, rmc.rmc_id, FALSE));
    }
...
{
    dnx_algo_port_rmc_info_t rmc;
    dnx_algo_port_info_s port_info = { 0 };
    uint32 max_rmcs_per_port = dnx_data_nif.eth.priority_groups_nof_get(unit);
    int rmc_index = 0;
    bcm_port_nif_scheduler_t sch_priority_to_reset;
    SHR_FUNC_INIT_VARS(unit);

    /** Not relevant for MGMT port */
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    ...
    int enable)
{
    ...
    if (enable)
    {
        if (IMB_ETHU_IS_TX_CREDIT_PORT(unit, port_info))
        {
             ...
             * stop the egr flush
             */
            SHR_IF_ERR_EXIT(imb_ethu_port_er_tx_egress_flush_set(unit, port, 0));
        }
        ...
        {
            /** No MACSEC on MGU port */
            if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH_MGMT(unit, port_info))
            {
                /** increase tx threshold for macsec enabled ports */

                if (dnx_data_macsec.general.is_macsec_enabled_get(unit) && IMB_ETHU_IS_TX_CREDIT_PORT(unit, port_info))
                {
                    bcm_xflow_macsec_port_info_t port_info;
                    int tx_threshold;

                    SHR_IF_ERR_EXIT(bcm_dnx_xflow_macsec_port_control_get
                                    (unit, XFLOW_MACSEC_ENCRYPT_DECRYPT_NONE, port, bcmXflowMacsecPortEnable,
                                     &port_info));
                    tx_threshold =
                        COMPILER_64_IS_ZERO(port_info.value) ? 1 : dnx_data_macsec.general.tx_threshold_get(unit);

                    SHR_IF_ERR_EXIT(imb_er_portmod_port_tx_threshold_set(unit, port, tx_threshold));
                }
            }
        }
#endif
    ...
    else
    {
        ...
        /*
         * flush the egress, the TX MLF will send credit to EGQ
         */
        if (IMB_ETHU_IS_TX_CREDIT_PORT(unit, port_info))
        {
            SHR_IF_ERR_EXIT(imb_ethu_port_er_tx_egress_flush_set(unit, port, 1));
        }
        ...
        if (!(dnx_data_nif.portmod.feature_get(unit, dnx_data_nif_portmod_not_implemented)))
        {
            ...
            if (dnx_data_macsec.general.is_macsec_enabled_get(unit))
            {
                ...
                {
                    /*
                     * lock critical section. Both user app (shell) and linkscan thread
                     * might call dnx_xflow_macsec_port_soft_reset_set
                     */
                    SHR_IF_ERR_EXIT(imb_portmod_soft_reset_lock(unit, port));

                    /** toggle soft reset. Will be toggled only if macsec is enabled */
                    rv = dnx_xflow_macsec_port_soft_reset_set(unit, port, IMB_COMMON_IN_RESET);

                    /** unlock critical section */
                    SHR_IF_ERR_EXIT(imb_portmod_soft_reset_unlock(unit, port));

                    if (rv != BCM_E_NONE)
                    {
                        SHR_ERR_EXIT(rv, "Failed to perform macsec soft reset");
                    }
                }
            }
        }
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_STIF(unit, port_info))
        {
            ...
            /** if stif_hard_wire_to_nif no need to reset its credits separately*/
            if (is_connected && !dnx_data_stif.config.feature_get(unit, dnx_data_stif_config_is_stif_hard_wire_to_nif))
            {
    ...
    int *enable)
{
    ...
    /*
     * Reset enable value to 0
     */
    sal_memset(enable, 0, sizeof(int));
    ...
    int enable)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_ethu_tx_data_to_pm_enable_hw_set(unit, &ethu_info, enable));
exit:
    SHR_FUNC_EXIT;
}

int
imb_ethu_port_phy_loopback_adapter_set(
    int unit,
    ...
    int loopback)
{
    ...
    /*
     * In case of PHY loopback perform also MAC loopback in Portmod (set LOCAL_LPBK bit)
     */
    if (loopback == BCM_PORT_LOOPBACK_PHY || curr_loopback == BCM_PORT_LOOPBACK_PHY)
    {
        enable = (loopback == BCM_PORT_LOOPBACK_PHY) ? TRUE : FALSE;
        SHR_IF_ERR_EXIT(portmod_port_loopback_set(unit, logical_port, portmodLoopbackMacOuter, enable));
    }
    ...
    int loopback)
{
    ...
    if (curr_loopback != BCM_PORT_LOOPBACK_NONE)
    {
        ...
        if ((is_linkscan_available == TRUE) && (curr_loopback == BCM_PORT_LOOPBACK_MAC))
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_channels_get
                            (unit, port, DNX_ALGO_PORT_CHANNELS_F_NON_L1_ONLY, &pbmp_same_if));
            _SHR_PBMP_ITER(pbmp_same_if, port_i)
            {
                SHR_IF_ERR_EXIT(_bcm_link_force(unit, port_i, FALSE, FALSE));
            }
        }
    ...
    if (loopback != BCM_PORT_LOOPBACK_NONE)
    {
        ...
        if ((is_linkscan_available == TRUE) && (loopback == BCM_PORT_LOOPBACK_MAC))
        {
            ...
            _SHR_PBMP_ITER(pbmp_same_if, port_i)
            {
                SHR_IF_ERR_EXIT(_bcm_link_force(unit, port_i, TRUE, link));
            }
        }
    }

#ifdef ADAPTER_SERVER_MODE
    SHR_IF_ERR_EXIT(imb_port_phy_loopback_adapter_set(unit, port, curr_loopback, loopback));
#endif
    ...
    uint32 enable)
{
    ...
    {
        rmc_id_in_prio_group = rmc;
        if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
            if (rmc >= dnx_data_nif.ofr.nof_rmc_per_priority_group_get(unit))
            {
                rmc_prio = bcmPortNifSchedulerLow;
                /** Remove the offset from the RMC ID */
                rmc_id_in_prio_group = rmc - dnx_data_nif.ofr.nof_rmc_per_priority_group_get(unit);
            }
            else
            {
                rmc_prio = bcmPortNifSchedulerHigh;
                rmc_id_in_prio_group = rmc;
            }
        }
        ...
        SHR_IF_ERR_EXIT(imb_ethu_internal_port_gen_pfc_from_rmc_enable_set
                        (unit, ethu_access_info.core, ethu_access_info.ethu_id_in_core, rmc_id_in_prio_group, rmc_prio,
                         priority, enable));
    }
    ...
    uint32 enable)
{
    ...
    _SHR_PBMP_ITER(lanes, global_lane)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, global_lane, &lane_in_core));
        _SHR_PBMP_ITER(rmcs, rmc)
        {
            rmc_id_in_prio_group = rmc;
            if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
            ...
            SHR_IF_ERR_EXIT(imb_ethu_internal_port_gen_llfc_from_rmc_enable_set
                            (unit, ethu_access_info.core, lane_in_core, rmc_id_in_prio_group, rmc_prio, enable));
        }
    }
    ...
    uint32 enable)
{
    ...
    /** Set HW to stop transmission on the port according to received PFC signal */
    SHR_IF_ERR_EXIT(imb_ethu_internal_port_pfc_force_enable_set
                    (unit, ethu_access_info.core, ethu_access_info.ethu_id_in_core, pfc, enable));
    ...
    uint32 enable)
{
    ...
    for (prio_group = 0; prio_group < nof_prio_groups_supported; ++prio_group)
    {
        ...
        if (rmc_info.rmc_id != -1)
        {
            if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
            {
                /** When The RMCs are of Low priority, the IDs are in the high range. Need to remove the offset for DBAL */
                rmc_id =
                    (rmc_info.sch_priority ==
                     bcmPortNifSchedulerLow) ? (rmc_info.rmc_id -
                                                dnx_data_nif.ofr.
                                                nof_rmc_per_priority_group_get(unit)) : rmc_info.rmc_id;
            }
            else
            {
                rmc_id = rmc_info.rmc_id;
            }
            /** Set the threshold value to HW */
            SHR_IF_ERR_EXIT(imb_ethu_internal_port_gen_pfc_from_llfc_thresh_enable_set
                            (unit, ethu_access_info.core, ethu_access_info.ethu_id_in_core, rmc_id,
                             rmc_info.sch_priority, enable));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    uint8 is_set)
{
    ...
    uint32 rmc_fifo_size = 0;
    uint32 rmc_fifo_resolution = 0;
    int nif_interface_id = 0, core, rmc_machine_id;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    if (!(DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF))))
    {
        ...
        for (prio_group = 0; prio_group < nof_prio_groups_supported; ++prio_group)
        {
            ...
            if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
            {
                SHR_IF_ERR_EXIT(dnx_port_ofr_rmc_machine_id_internal_get
                                (unit, port, rmc.sch_priority, &rmc_machine_id));
                SHR_IF_ERR_EXIT(dnx_port_link_list_db_double_priority_nof_sections_allocated_get
                                (unit, DNX_PORT_NIF_OFR_LINK_LIST, nif_interface_id, rmc_machine_id, &rmc_fifo_size));
                max_threshold = rmc_fifo_size * rmc_fifo_resolution;
            }
            else
            {
                max_threshold = rmc.last_entry - rmc.first_entry + 1;
            }
    ...
    uint32 threshold)
{
    ...
    dnx_algo_port_rmc_info_t rmc_info;
    dnx_algo_port_ethu_access_info_t ethu_access_info;
    dnx_rmc_thresholds_t rmc_thresholds;
    int rmc_id = 0;
    int ref_count = 0;
    int profile_old = 0;
    int profile_new = 0;
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(imb_ethu_port_fc_rx_qmlf_threshold_set_get_verify(unit, port, flags, &threshold, TRUE));
    ...
    for (prio_group = 0; prio_group < nof_prio_groups_supported; ++prio_group)
    {
        ...
        if (rmc_info.rmc_id != -1)
        {
            ...
            {
                if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_qmlf_rmcs_thresholds_per_profile_support))
                {
                    /** Calculate the RMC ID. PFC/LLFC is only relevant for ETH ports, which take the lower 64 RMCs out of 384 in each priority */
                    rmc_id =
                        (rmc_info.sch_priority ==
                         bcmPortNifSchedulerLow) ? (rmc_info.rmc_id -
                                                    dnx_data_nif.ofr.
                    ...
                    SHR_IF_ERR_EXIT(imb_ethu_internal_port_fc_rx_qmlf_rmc_to_profile_get
                                    (unit, prio_group, rmc_id, &profile_old));
                    /** Get the profile's value */
                    SHR_IF_ERR_EXIT(flow_control_db.
                                    rmc_thresholds.profile_data_get(unit, profile_old, &ref_count, &rmc_thresholds));
                    ...
                    if (flags & BCM_COSQ_THRESHOLD_ETH_PORT_LLFC)
                    {
                        ...
                        {
                            rmc_thresholds.llfc_set_thresh = threshold;
                        }

                        if (flags & BCM_COSQ_THRESHOLD_CLEAR)
                        {
                            rmc_thresholds.llfc_clr_thresh = threshold;
                        }
                    }
                    ...
                    if (flags & BCM_COSQ_THRESHOLD_ETH_PORT_PFC)
                    {
                        ...
                        if (flags & BCM_COSQ_THRESHOLD_SET)
                        {
                            rmc_thresholds.pfc_set_thresh = threshold;
                        }
                        if (flags & BCM_COSQ_THRESHOLD_CLEAR)
                        {
                            rmc_thresholds.pfc_clr_thresh = threshold;
                        }
                    }

                    /** Exchange the existing profile with a new one */
                    SHR_IF_ERR_EXIT(flow_control_db.
                                    rmc_thresholds.exchange(unit, 0, &rmc_thresholds, profile_old, NULL, &profile_new,
                                                            &is_first, &is_last));

                    /** Set values to HW */
                    SHR_IF_ERR_EXIT(imb_ethu_internal_port_fc_rx_qmlf_rmc_to_profile_set
                                    (unit, prio_group, rmc_id, profile_new));
                    SHR_IF_ERR_EXIT(imb_ethu_internal_port_fc_rx_qmlf_profile_value_set
                                    (unit, profile_new, rmc_thresholds));
                }
                else
                {
                    /** Set the threshold value to HW */
    ...
    uint32 *threshold)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(imb_ethu_port_fc_rx_qmlf_threshold_set_get_verify(unit, port, flags, threshold, FALSE));
          ...
          BCM_COSQ_THRESHOLD_ETH_PORT_HIGH_FIFO)))
    {
        ...
        for (prio_group = 0; prio_group < nof_prio_groups_supported; ++prio_group)
        {
            ...
            if (rmc_info.rmc_id != -1)
            {
                if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_qmlf_rmcs_thresholds_per_profile_support))
                {
                    ...
                    /** Get the current profile */
                    SHR_IF_ERR_EXIT(imb_ethu_internal_port_fc_rx_qmlf_rmc_to_profile_get
                                    (unit, prio_group, rmc_id, &profile));
                    /** Get the profile's value */
                    SHR_IF_ERR_EXIT(flow_control_db.
                                    rmc_thresholds.profile_data_get(unit, profile, &ref_count, &rmc_thresholds));
                    ...
                    if (flags & BCM_COSQ_THRESHOLD_ETH_PORT_LLFC)
                    {
                        ...
                        if (flags & BCM_COSQ_THRESHOLD_SET)
                        {
                            thresh_get[prio_group] = rmc_thresholds.llfc_set_thresh;
                        }

                        if (flags & BCM_COSQ_THRESHOLD_CLEAR)
                        {
                            thresh_get[prio_group] = rmc_thresholds.llfc_clr_thresh;
                        }
                    }
                    ...
                    if (flags & BCM_COSQ_THRESHOLD_ETH_PORT_PFC)
                    {
                        ...
                        if (flags & BCM_COSQ_THRESHOLD_SET)
                        {
                            thresh_get[prio_group] = rmc_thresholds.pfc_set_thresh;
                        }
                        if (flags & BCM_COSQ_THRESHOLD_CLEAR)
                        {
                            thresh_get[prio_group] = rmc_thresholds.pfc_clr_thresh;
                        }
                    }
                }
                else
        ...
        for (prio_group = 0; prio_group < nof_prio_groups_supported; ++prio_group)
        {
             /** Get the first value that is different from invalid */
            if ((thresh_temp == -1) && (thresh_get[prio_group] != -1))
            {
       ...
       /** The threshold requested is for a specific FIFO
        ...
        for (prio_group = 0; prio_group < nof_prio_groups_supported; ++prio_group)
        {
            ...
            if (rmc_info.rmc_id != -1)
            {
                ...
                {
                    if (dnx_data_fc.
                        general.feature_get(unit, dnx_data_fc_general_qmlf_rmcs_thresholds_per_profile_support))
                    {
                        /** Calculate the RMC ID. PFC/LLFC is only relevant for ETH ports, which take the lower 64 RMCs out of 384 in each priority */
                        rmc_id =
                            (rmc_info.sch_priority ==
                        ...
                        if (flags & BCM_COSQ_THRESHOLD_ETH_PORT_LLFC)
                        {
                            ...
                            if (flags & BCM_COSQ_THRESHOLD_SET)
                            {
                                thresh_temp = rmc_thresholds.llfc_set_thresh;
                            }

                            if (flags & BCM_COSQ_THRESHOLD_CLEAR)
                            {
                                thresh_temp = rmc_thresholds.llfc_clr_thresh;
                            }
                        }
                        ...
                        if (flags & BCM_COSQ_THRESHOLD_ETH_PORT_PFC)
                        {
                            ...
                            if (flags & BCM_COSQ_THRESHOLD_SET)
                            {
                                thresh_temp = rmc_thresholds.pfc_set_thresh;
                            }
                            if (flags & BCM_COSQ_THRESHOLD_CLEAR)
                            {
                                thresh_temp = rmc_thresholds.pfc_clr_thresh;
                            }
                        }
                    }
                    else
    ...
    uint32 in_reset)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_global_to_local_and_core_get(unit, imb_info->inst_id,
                                                                        &core, &ethu_id_in_core));
    ...
    uint32 enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_global_to_local_and_core_get(unit, imb_info->inst_id,
                                                                        &core, &ethu_id_in_core));
    SHR_IF_ERR_EXIT(dnx_algo_global_ethu_id_lane_info_get
                    (unit, imb_info->inst_id, &first_lane_in_ethu, &nof_lanes_in_ethu));
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, lane, &lane_in_core));
    ...
    uint32 enable)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, lane, &lane_in_core));
        SHR_IF_ERR_EXIT(imb_ethu_internal_pfc_mask_on_link_fail_enable_set(unit, core, lane_in_core, enable));
    }
    ...
    bcm_pbmp_t * ethu_ports)
{
    ...
    SHR_RANGE_VERIFY(ethu_info.pm_id_in_ethu, 0,
                     dnx_data_nif.eth.total_nof_ethu_pms_in_device_get(unit) - 1,
                     _SHR_E_PORT, "pm id is out of range.\n");
    ...
    uint32 is_active)
{
    ...
    if (sch_prio == bcmPortNifSchedulerLow)
    {
        ...
        SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_set(unit, &ethu_info, sch_prio, weight));
    }
    else        /* High / TDM scheduler */
    {
        ...
        else if (is_active)
        {
            ...
            SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_set
                            (unit, &ethu_info, sch_prio, dnx_data_nif.scheduler.nof_weight_bits_get(unit)));
        }
        else if (is_active == 0)
        {
            /*
            ...
            {
                SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_set(unit, &ethu_info, sch_prio, 0));
            }
        }
    }
    ...
    int nof_new_priority_groups)
{
    ...
    {
        if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_is_rmc_allocation_needed))
        {
    ...
    const bcm_port_prio_config_t * priority_config)
{
    ...
    for (ii = 0; ii < nof_prio_groups; ++ii)
    {
        ...
        {
            /*
             * RMCs 0-7 for High priority, 8-15 for Low 
             */
            rmc.rmc_id =
                ethu_info.first_lane_in_port +
                dnx_data_nif.eth.nof_rmcs_per_priority_group_get(unit) *
                (priority_config->priority_groups[ii].sch_priority == bcmPortNifSchedulerLow ? 1 : 0);
        }
        /*
        ...
        rmc.nof_entries = rmc_fifo_size;
        SHR_IF_ERR_EXIT(dnx_algo_port_imb_threshold_after_ovf_get(unit, rmc_fifo_size, &rmc.thr_after_ovf));
    ...
    int enable)
{
    ...
    for (ii = 0; ii < nof_rmcs; ++ii)
    {
        ...
        if (!dnx_data_nif.scheduler.feature_get(unit, dnx_data_nif_scheduler_hardcoded_lanes_and_priority))
        {
            /*
    ...
    int *bandwidth)
{
    ...
    BCM_PBMP_ITER(ethu_ports, port_i)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_has_speed(unit, port_i, 0, &port_has_speed));
        if (port_has_speed)
        {
...
/**
 * imb_ethu_port_mib_counter_get
 *
 * \brief Get ethu counter
 *
 * \param [in]  unit            - unit id
 * \param [in]  port            - logical port
 * \param [in]  counter_type    - counter type
 * \param [out]  counter_val     - ethu counter value
 *
 * \return
 *   int - see _SHR_E_*
 *
    ...
    uint64 *counter_val)
{
    ...
    COMPILER_64_SET(rmc_counter, 0, 0);

    /** May need support other counter type in the future */
    switch (counter_type)
    {
        case dnx_mib_counter_nif_rx_stats_discards:
            ...
            for (rmc_index = 0; rmc_index < max_rmcs_per_port; ++rmc_index)
            {
                ...
                if (rmc.rmc_id != IMB_COSQ_RMC_INVALID)
                {
                    SHR_IF_ERR_EXIT(imb_ethu_rx_fifo_drop_hw_get(unit, port, rmc.rmc_id, &rmc_counter));
                    COMPILER_64_ADD_64(*counter_val, rmc_counter);
                }
            }
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "imb_ethu_port_mib_counter_get isn't implemented for counter type %d",
                         counter_type);
    }
 ...
 * \param [out] mapped - indicate if port is mapped to a counter(only for relevant devices)
 *
 * \return
 *   int - see _SHR_E_*
 *
    ...
    uint32 *mapped)
{
    ...
    /*
     * in non-OFR devices the port is always mapped
     */
    *mapped = 1;
    ...
    uint32 tpid_value)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_prd_tpid_hw_set(unit, ethu_info, prd_profile, tpid_index, tpid_value));
    ...
    uint32 *tpid_value)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_prd_tpid_hw_get(unit, ethu_info, prd_profile, tpid_index, tpid_value));
    ...
    uint32 val)
{
    ...
    switch (property)
    {
        case imbImbPrdIgnoreIpDscp:
            SHR_IF_ERR_EXIT(imb_ethu_prd_ignore_ip_dscp_hw_set(unit, ethu_info, prd_profile, val));
            break;
        case imbImbPrdIgnoreMplsExp:
            SHR_IF_ERR_EXIT(imb_ethu_prd_ignore_mpls_exp_hw_set(unit, ethu_info, prd_profile, val));
            break;
        case imbImbPrdIgnoreInnerTag:
            SHR_IF_ERR_EXIT(imb_ethu_prd_ignore_inner_tag_hw_set(unit, ethu_info, prd_profile, val));
            break;
        case imbImbPrdIgnoreOuterTag:
            SHR_IF_ERR_EXIT(imb_ethu_prd_ignore_outer_tag_hw_set(unit, ethu_info, prd_profile, val));
            break;
        case imbImbPrdDefaultPriority:
            SHR_IF_ERR_EXIT(imb_ethu_prd_default_priority_hw_set(unit, ethu_info, prd_profile, val));
            break;
        default:
    ...
    uint32 *val)
{
    ...
    {
        case imbImbPrdIgnoreIpDscp:
            SHR_IF_ERR_EXIT(imb_ethu_prd_ignore_ip_dscp_hw_get(unit, ethu_info, prd_profile, val));
            break;
        case imbImbPrdIgnoreMplsExp:
            SHR_IF_ERR_EXIT(imb_ethu_prd_ignore_mpls_exp_hw_get(unit, ethu_info, prd_profile, val));
            break;
        case imbImbPrdIgnoreInnerTag:
            SHR_IF_ERR_EXIT(imb_ethu_prd_ignore_inner_tag_hw_get(unit, ethu_info, prd_profile, val));
            break;
        case imbImbPrdIgnoreOuterTag:
            SHR_IF_ERR_EXIT(imb_ethu_prd_ignore_outer_tag_hw_get(unit, ethu_info, prd_profile, val));
            break;
        case imbImbPrdDefaultPriority:
            SHR_IF_ERR_EXIT(imb_ethu_prd_default_priority_hw_get(unit, ethu_info, prd_profile, val));
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid PRD property %d\n", property);
    }
    ...
    int enable_mode)
{
    ...
    if (enable_mode == bcmCosqIngressPortDropDisable)
    {
         ...
         * Disable Soft Stage
         */
        SHR_IF_ERR_EXIT(imb_ethu_prd_soft_stage_enable_hw_set(unit, ethu_info, prd_profile, 0, 0));
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(imb_ethu_prd_outer_tag_size_hw_set(unit, ethu_info, prd_profile, outer_tag_size));
        ...
        SHR_IF_ERR_EXIT(imb_ethu_prd_port_type_hw_set(unit, ethu_info, prd_profile, port_type));
        ...
        SHR_IF_ERR_EXIT(imb_ethu_prd_soft_stage_enable_hw_set
                        (unit, ethu_info, prd_profile, en_soft_stage_eth, en_soft_stage_tm));
 ...
 */
int
imb_ethu_port_fec_speed_is_valid_get(
    int unit,
    ...
    *is_valid = FALSE;
    ...
    for (idx = 0; idx < dnx_data_nif.eth.supported_interfaces_info_get(unit)->key_size[0]; idx++)
    {
        if (dnx_data_nif.eth.supported_interfaces_get(unit, idx)->is_valid &&
            (!for_autoneg || dnx_data_nif.eth.supported_interfaces_get(unit, idx)->autoneg_support))
        {
    ...
    int speed)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_port_fec_speed_is_valid_get(unit, pm_type, nof_lanes, fec_type, speed, FALSE, &is_valid));
...
imb_ethu_instru_traffic_counters_reset(
    ...
    const imb_create_info_t * imb_info)
{
    int nof_ethus_per_core, lane, ethu_id_in_core;
    int lane_in_core, first_lane_in_ethu, nof_lanes_in_ethu;
    bcm_core_t core;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_global_to_local_and_core_get
                    (unit, imb_info->inst_id, &core, &ethu_id_in_core));
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_nof_ethus_per_core_get(unit, core, &nof_ethus_per_core));
    SHR_IF_ERR_EXIT(dnx_algo_global_ethu_id_lane_info_get
                    (unit, imb_info->inst_id, &first_lane_in_ethu, &nof_lanes_in_ethu));
    ...
    for (lane = first_lane_in_ethu; lane < (first_lane_in_ethu + nof_lanes_in_ethu); lane++)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, lane, &lane_in_core));
        SHR_IF_ERR_EXIT(imb_ethu_internal_instru_traffic_pfc_deadlock_counters_reset
                        (unit, core, lane_in_core, TRUE, FALSE));
    }
    ...
    bcm_port_t port)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_traffic_pfc_deadlock_counters_reset
                    (unit, ethu_info.core, ethu_info.lane_in_core, TRUE, FALSE));
    ...
    const imb_create_info_t * imb_info)
{
    ...
    for (lane = first_lane_in_ethu; lane < (first_lane_in_ethu + nof_lanes_in_ethu); lane++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, lane, &lane_in_core));
        SHR_IF_ERR_EXIT(imb_ethu_internal_instru_traffic_pfc_deadlock_counters_reset
                        (unit, core, lane_in_core, FALSE, TRUE));
    }
    ...
    bcm_port_t port)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_traffic_pfc_deadlock_counters_reset
                    (unit, ethu_info.core, ethu_info.lane_in_core, FALSE, TRUE));
...
/**
 * \brief - Set per IMB instrumentation sync counters configuration
 *
 * \param [in] unit - chip unit id
    ...
    uint8 is_continuous)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_synced_counters_config_set
                    (unit, core, ethu_id_in_core, nof_intervals, is_continuous));
...
/**
 * \brief - Get per IMB instrumentation sync counters configuration
 *
 * \param [in] unit - chip unit id
 * \param [in] imb_id - IMB ID
 ...
 */
int
imb_ethu_instru_synced_counters_config_get(
    int unit,
    int imb_id,
    uint8 *nof_intervals,
    uint8 *is_continuous)
{
    ...
    /** Set HW to put in/out of reset all instrumentation counters of the ETHU */
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_synced_counters_config_get
                    (unit, core, ethu_id_in_core, nof_intervals, is_continuous));
...
/**
 * \brief - Get per IMB instrumentation sync counters last write
 *
 * \param [in] unit - chip unit id
 * \param [in] port - logical port
    ...
    uint32 *nof_writes)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_synced_counters_last_write_get
                    (unit, ethu_info.core, ethu_info.ethu_id_in_core, last_write, nof_writes));
    ...
    uint64 counter_values[])
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_synced_counters_per_record_get(unit,
                                                                            ethu_info.core, ethu_info.ethu_id_in_core,
                                                                            port_in_ethu, ethu_info.imb_type,
                                                                            record_index, nof_counters,
 ...
 * \param [in] unit - chip unit id
 * \param [in] imb_id - IMB ID
 * \param [in] reset - If true take the counters into reset.
 * \return
 *   int - see _SHR_E_ *
 *
 * \remarks
 ...
 */
int
imb_ethu_instru_counters_reset_set(
    int unit,
    int imb_id,
    uint8 reset)
{
    ...
    /** Set HW to put in/out of reset all instrumentation counters of the ETHU */
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_counters_reset_set(unit, core, ethu_id_in_core, reset));
...
/**
 * \brief - Get the reset status per ETHU instrumentation counters
 *
 * \param [in] unit - chip unit id
 * \param [in] imb_id - IMB ID
 * \param [out] reset - reset value of the instrumentation counters.
 * \return
 *   int - see _SHR_E_ *
 *
 * \remarks
 ...
 */
int
imb_ethu_instru_counters_reset_get(
    int unit,
    int imb_id,
    uint8 *reset)
{
    ...
    /** Get from HW the reset value of the NIF instrumentation counters per ETHU */
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_counters_reset_get(unit, core, ethu_id_in_core, reset));
    ...
    int *pfc_num)
{
    ...
    int imb_type = imbDispatchTypeNone;
    int first_lane = 0, first_lane_in_core = 0;
    int nof_lanes_in_ethu;
    int lane_in_core1;
    int lane_in_core2;
    uint32 duration_temp1[2] = { 0, 0 };
    uint32 duration_temp2[2] = { 0, 0 };
    uint64 duration_temp1_64;
    uint64 duration_temp2_64;
    SHR_FUNC_INIT_VARS(unit);
    /**
     * Get ETHU core and ETHU in core
     */
    if (dnx_data_nif.eth.nbu_nof_get(unit) != 0)
    {
        ethu_core = dnx_data_nif.eth.ethu_per_core_get(unit, ethu_id)->core_id;
        ethu_id_in_core = ethu_id - dnx_data_nif.eth.nif_cores_ethus_get(unit, ethu_core)->first_ethu;
    }
    else
    {
    ...
    /**
     * For CLU the debug information is presented in 3 bit, but the CLUs contain 16 PHYs.
     * An additional calculation is needed to determine the PHY that triggered the FC.
     */
    imb_type = dnx_data_nif.eth.ethu_properties_get(unit, ethu_id)->type;
    if (imb_type == imbDispatchTypeImb_clu)
    {
        /**
         * Calculate first lane in core
         */
        SHR_IF_ERR_EXIT(dnx_algo_global_ethu_id_lane_info_get(unit, ethu_id, &first_lane, &nof_lanes_in_ethu));
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, first_lane, &first_lane_in_core));

        /**
         * Need to check which PHY has bigger PFC duration
         */
        lane_in_core1 = first_lane_in_core + *ethu_port_num;
        lane_in_core2 = first_lane_in_core + *ethu_port_num + (dnx_data_nif.eth.nof_lanes_in_clu_get(unit) / 2);;
        SHR_IF_ERR_EXIT(imb_ethu_internal_pfc_deadlock_current_duration_get
                        (unit, lane_in_core1, ethu_core, *pfc_num, duration_temp1));
        SHR_IF_ERR_EXIT(imb_ethu_internal_pfc_deadlock_current_duration_get
                        (unit, lane_in_core2, ethu_core, *pfc_num, duration_temp2));
        COMPILER_64_SET(duration_temp1_64, duration_temp1[1], duration_temp1[0]);
        COMPILER_64_SET(duration_temp2_64, duration_temp2[1], duration_temp2[0]);
        *ethu_port_num =
            (COMPILER_64_GT(duration_temp1_64, duration_temp2_64) ? *ethu_port_num
             : (*ethu_port_num + (dnx_data_nif.eth.nof_lanes_in_clu_get(unit) / 2)));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int enable)
{
    ...
    {
        if ((rx_speed == dnx_data_nif.mac_client.l1_mismatch_rate_table_get(unit, idx)->eth_speed) &&
            (tx_speed == dnx_data_nif.mac_client.l1_mismatch_rate_table_get(unit, idx)->client_speed))
        {
            is_found = 1;
            break;
        }
    ...
    int power_down)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_power_down_set(unit, &ethu_info, power_down));
...
/**
 * \brief - Get two-step PTP PHY timesync tx info for eth port.
 *
 */
int
 ...
 */
int
imb_ethu_prd_port_last_packet_priority_get(
    int unit,
    bcm_port_t port,
    uint32 *priority,
    uint32 *tdm,
    uint32 *valid)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_ethu_prd_port_last_packet_priority_tdm_hw_get(unit, ethu_info, priority, tdm, valid));
