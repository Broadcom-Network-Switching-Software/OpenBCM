    ...
    bcm_port_t port)
{
    ...
    if (dnx_data_macsec.general.feature_get(unit, dnx_data_macsec_general_macsec_block_exists))
    {
        /** disable both the MACSec and core clocks ONLY in case this is the last port in the entire MACSec instance. */
        SHR_IF_ERR_EXIT(dnx_xflow_macsec_control_power_down_set(unit, port, DNX_MACSEC_POWER_DOWN_CORE, TRUE));
        SHR_IF_ERR_EXIT(dnx_xflow_macsec_control_power_down_set(unit, port, DNX_MACSEC_POWER_DOWN_MACSEC, TRUE));
    }
#endif
exit:
    SHR_FUNC_EXIT;
