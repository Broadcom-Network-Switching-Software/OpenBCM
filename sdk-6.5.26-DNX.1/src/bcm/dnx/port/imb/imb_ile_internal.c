    ...
    int enable)
{
    ...
    if ((flags & (IMB_COMMON_DIRECTION_RX | IMB_COMMON_DIRECTION_TX)) == 0)
    {
    ...
    int enable)
{
     ...
     */
    is_high_updated = (ilkn_access_info.port_in_core == 1 || nof_segments == max_segments) ? TRUE : FALSE;

    if (is_low_updated)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RX_SEGMENTS_SELECTION_LOW, INST_SINGLE, enable);
    }
    if (is_high_updated)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RX_SEGMENTS_SELECTION_HIGH, INST_SINGLE, enable);
    }
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_NIF_ILE_CONNECTIVITY, entry_handle_id));
    ...
    if (is_low_updated)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TX_SEGMENTS_SELECTION_LOW, INST_SINGLE, enable);
    }
    if (is_high_updated)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TX_SEGMENTS_SELECTION_HIGH, INST_SINGLE, enable);
    }
    ...
    int in_reset)
{
     ...
     * set value fields
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ELK_RX_RESET, INST_SINGLE,
 ...
 */
int
imb_ile_port_frmr_ilkn_select_set(
    int unit,
    bcm_port_t port,
    int select_ilkn)
{
     ...
     * alloc DBAL table handle
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_FRMR_ILKN_SELECT, &entry_handle_id));
    ...
    _SHR_PBMP_ITER(ilkn_phys, iter_phy)
    {
         ...
         * set value fields
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_SELECT, INST_SINGLE, select_ilkn);
        /*
         * commit the value
         */
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
static int
imb_ilu_burst_drop_count_hw_get(
    int unit,
    bcm_port_t port,
    uint32 is_tdm,
    uint64 *burst_count)
{
     ...
     * alloc DBAL table handle
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_ILU_HRF_BURST_DROP_COUNTERS, &entry_handle_id));
    /*
     * set key fields
     */
     ...
     */
    dbal_value_field64_request(unit, entry_handle_id, DBAL_FIELD_BURST_DROP_COUNTER, INST_SINGLE, burst_count);
    /*
     * commit value
     */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    uint64 *burst_count)
{
    ...
    /** Get burst drop counter for master port only, return 0 for other port */
    if (is_master_port)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_ilkn_priority_get(unit, port, &hrf_sch_priority));

        if (hrf_sch_priority == bcmPortNifSchedulerTDM)
        {
            is_tdm = 1;
        }
        else
        ...
        SHR_IF_ERR_EXIT(imb_ilu_burst_drop_count_hw_get(unit, port, is_tdm, burst_count));
    }
    else
    {
        COMPILER_64_ZERO(*burst_count);
    }
    ...
    uint8 in_reset)
{
    ...
    if (flags & IMB_COMMON_DIRECTION_RX)
    {
         ...
         * alloc DBAL table handle
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_ILKN_ALIGNER_CTRL, &entry_handle_id));
         ...
         * Clear the handle before re-use
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_NIF_ILKN_ALIGNER_CTRL, entry_handle_id));
    }
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(imb_cdu_port_xpmd_lane_enable(unit, port, flags, enable));
exit:
    SHR_FUNC_EXIT;
    ...
    int fec_bypass_enable)
{
    ...
    int iter_phy, first_lane_in_cdu, ethu_id, ethu_id_in_core, core, phy_in_core, is_over_fabric, pm_index,
        first_phy_in_pm, tx_serdes_source, map_size, nof_phys;
    ...
    SHR_IF_ERR_EXIT(dnx_algo_lane_map_serdes_to_lane_map_get(unit,
                                                             (is_over_fabric) ? DNX_ALGO_LANE_MAP_FABRIC_SIDE :
                                                             DNX_ALGO_LANE_MAP_NIF_SIDE, map_size, serdes2lane));
    ...
    {
        SHR_IF_ERR_EXIT(imb_ile_internal_fec_global_enable_set(unit, ilkn_access_info.ilkn_core, fec_enable));
    }
    ...
    _SHR_PBMP_ITER(tx_phys, iter_phy)
    {
         ...
         * set key fields
         */
        if (!dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_is_fec_bypass_supported))
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_CORE_ID, ilkn_access_info.ilkn_core);
        }
        if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_12lanes_mode))
        {
            SHR_IF_ERR_EXIT(dnx_algo_lane_map_ilkn_lane_info_get
                            (unit, port, (is_over_fabric) ? DNX_ALGO_LANE_MAP_FABRIC_SIDE : DNX_ALGO_LANE_MAP_NIF_SIDE,
                             serdes2lane[iter_phy].tx_id, &ilkn_lane_info));
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FEC_UNIT,
                                       (ilkn_lane_info.tx_id / dnx_data_nif.ilkn.nof_lanes_per_fec_unit_get(unit)));
        }
        else
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FEC_UNIT,
 ...
 */
int
imb_ile_internal_fec_global_enable_set(
    int unit,
    int ilkn_core,
    int fec_enable)
{
     ...
     * set key fields
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_CORE_ID, ilkn_core);
 ...
 */
int
imb_ile_port_ilkn_is_fec_bypass_enable(
    int unit,
    int port,
    int *fec_bypass_enable)
{
    ...
    *fec_bypass_enable = (fec_type == (bcm_port_phy_fec_t) PORTMOD_PORT_PHY_FEC_RS_544) ? FALSE : TRUE;
