...
#define DNX_PORT_ESEM_CMD_IS_DEFAULT(esem_cmd) \
    ((esem_cmd == dnx_data_esem.access_cmd.default_ac_get(unit)) ||  \
     ...
     (esem_cmd == dnx_data_esem.access_cmd.sflow_sample_interface_get(unit)) || \
     (esem_cmd == dnx_data_esem.access_cmd.ip_tunnel_no_action_get(unit)) || \
     (esem_cmd == dnx_data_esem.access_cmd.arp_plus_ac_port_svtag_get(unit)))
    ...
    dnx_esem_cmd_data_t esem_cmd_data)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                    (unit, dnx_data_esem.access_cmd.esem_cmd_table_map_get(unit)->table_name, &entry_handle_id));
    dbal_entry_key_field16_set(unit, entry_handle_id, dnx_data_esem.access_cmd.esem_cmd_table_map_get(unit)->esem_cmd,
                               esem_cmd);
    for (access_id = 0; access_id < ESEM_ACCESS_IF_COUNT; access_id++)
    {
        dbal_entry_value_field8_set(unit, entry_handle_id,
                                    dnx_data_esem.access_cmd.esem_cmd_table_map_get(unit)->access_valid[access_id],
                                    INST_SINGLE, esem_cmd_data.esem[access_id].valid);
        dbal_entry_value_field8_set(unit, entry_handle_id,
                                    dnx_data_esem.access_cmd.esem_cmd_table_map_get(unit)->access_program[access_id],
                                    INST_SINGLE, esem_cmd_data.esem[access_id].app_db_id);
        dbal_entry_value_field8_set(unit, entry_handle_id,
                                    dnx_data_esem.access_cmd.
                                    esem_cmd_table_map_get(unit)->access_designated_offset[access_id], INST_SINGLE,
                                    esem_cmd_data.esem[access_id].designated_offset);
        dbal_entry_value_field8_set(unit, entry_handle_id,
                                    dnx_data_esem.access_cmd.
                                    esem_cmd_table_map_get(unit)->access_default_result[access_id], INST_SINGLE,
                                    esem_cmd_data.esem[access_id].default_result_profile);
    }
...
{
    uint32 entry_handle_id;
    uint8 is_predefined = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_port_esem_cmd_is_predefine(unit, esem_cmd, &is_predefined));
    if (!is_predefined)
    {
        /** Remove the attributes from esem-cmd table in the cmd-id*/
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, dnx_data_esem.access_cmd.esem_cmd_table_map_get(unit)->table_name,
                                          &entry_handle_id));
        dbal_entry_key_field16_set(unit, entry_handle_id,
                                   dnx_data_esem.access_cmd.esem_cmd_table_map_get(unit)->esem_cmd, esem_cmd);
    ...
    dnx_esem_access_type_t * access_type)
{
    ...
    else
    {
        ...
        if (result_type == DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_AC_STAT)
        {
        ...
        else if (result_type == DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_DUAL_HOMING)
        {
            *access_type = ESEM_ACCESS_TYPE_DUAL_HOME;
        }
        else if (result_type == DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_NULL)
        {
            *access_type = ESEM_ACCESS_TYPE_DUAL_HOME;
        }
        else
    ...
    int *base_cmd_idx)
{
    ...
    for (access_cnt = 0; access_cnt < max_nof_access; access_cnt++)
    {
        if (esem_if != NULL)
        {
            esem_if[access_cnt] = ESEM_ACCESS_IF_COUNT;
        }
        if (esem_offset != NULL)
        {
            esem_offset[access_cnt] = DBAL_NOF_ENUM_ESEM_OFFSET_VALUES;
        }
    }
...
dnx_port_esem_cmd_algo_exchange(
    ...
    uint8 *is_last)
{
    int rv, tmp_new_esem_cmd, tmp_old_esem_cmd;
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_esem_cmd_algo_exchange_verify
    ...
    while (rv != _SHR_E_NOT_FOUND)
    {
        ...
        {
            int esem_cmd_i, ref_count;
            dnx_esem_cmd_data_t esem_cmd_data_i;
            /*
             * If the cmd data match with default cmd, do not increase the reference count of the cmd,
             * since we always keep the reference count of the default cmds that are not created during
             * the template initialization unchanged, and update the real default cmd instead.
             */
            if (DNX_PORT_ESEM_CMD_IS_DEFAULT(tmp_new_esem_cmd))
            {
                esem_cmd_i = dnx_data_esem.access_cmd.no_action_get(unit);
                SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.profile_data_get
                                (unit, esem_cmd_i, &ref_count, &esem_cmd_data_i));
            }
            else
            {
                sal_memcpy(&esem_cmd_data_i, esem_cmd_data, sizeof(esem_cmd_data_i));
            }
            if (DNX_PORT_ESEM_CMD_IS_DEFAULT(old_esem_cmd))
            {
                esem_cmd_i = dnx_data_esem.access_cmd.no_action_get(unit);
            }
            else
            {
                esem_cmd_i = old_esem_cmd;
            }
            SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.exchange
                            (unit, flags, &esem_cmd_data_i, esem_cmd_i, (void *) &extra_alloc_info, new_esem_cmd,
                             is_first, is_last));
            *new_esem_cmd = tmp_new_esem_cmd;
    ...
    if (DNX_PORT_ESEM_CMD_IS_DEFAULT(old_esem_cmd))
    {
        tmp_old_esem_cmd = dnx_data_esem.access_cmd.no_action_get(unit);
    }
    else
    {
        tmp_old_esem_cmd = old_esem_cmd;
    }
    SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.exchange
                    (unit, flags, esem_cmd_data, tmp_old_esem_cmd, (void *) &extra_alloc_info, new_esem_cmd, is_first,
                     is_last));
    ...
    int unit)
{
    ...
    esem_cmd_default = dnx_data_esem.access_cmd.allocation_mngr_default_get(unit);
    ...
    if (is_first)
    {
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_data_hw_set(unit, esem_cmd_alloc, esem_cmd_data));
    }
    algo_port_pp_db.esem.predefine_esem_cmds.set(unit, esem_cmd_default, TRUE);
    /*
    ...
    algo_port_pp_db.esem.predefine_esem_cmds.set(unit, esem_cmd_alloc, TRUE);
    /*
    ...
    {
        /*
         * Create ESEM command for SVTAG port, used by ARP+AC LIF
         */
        esem_cmd_info.nof_accesses = 0;
        sal_memset(&esem_cmd_data, 0, sizeof(dnx_esem_cmd_data_t));
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_access_add(unit, 0, ESEM_ACCESS_TYPE_SVTAG,
                                                     ...
                                                     DNX_PORT_ESEM_DEFAULT_RESULT_PROFILE_INVALID, &esem_cmd_info));
        esem_cmd_alloc = dnx_data_esem.access_cmd.arp_plus_ac_port_svtag_get(unit);
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_exchange(unit, flags, DNX_ALGO_ESEM_CMD_SUFFIX_SIZE_BIT_2
                                                   /** insignificant for ID allocation*/ ,
    ...
    /*
     * Init PEM esem_default_result_profile with default_native_ac initially
     * It is required by the encapsolation(from eedb) of snooping to remote CPU cases.
     */
    {
        uint32 entry_handle_id;
        ...
        for (access_idx = 0; access_idx < esem_cmd_info.nof_accesses_remove; access_idx++)
        {
            if (esem_cmd_info.remove_accesses[access_idx] != ESEM_ACCESS_TYPE_NONE)
            {
                SHR_IF_ERR_EXIT(dnx_port_esem_cmd_access_info_get(unit, 0, esem_cmd_info.remove_accesses[access_idx],
                                                                  DNX_PORT_ESEM_NOF_ACCESS, access_if, NULL,
                                                                  &base_cmd_idx));
    ...
    uint8 *is_last)
{
     ...
     */
    free_needed = TRUE;
    if (DNX_PORT_ESEM_CMD_IS_DEFAULT(esem_cmd))
    {
        free_needed = FALSE;
    }
 ...
 */
shr_error_e
dnx_port_esem_cmd_is_predefine(
    int unit,
    ...
    uint8 *is_predefined)
{
    ...
    *is_predefined = FALSE;
    {
        if (DNX_PORT_ESEM_CMD_IS_DEFAULT(esem_cmd_profile))
        {
            *is_predefined = TRUE;
        }
    }
