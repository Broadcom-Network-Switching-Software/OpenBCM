    ...
    bcm_core_t core)
{
     ...
     *
     * Encoding Efficiency(Fixed): 64/67
     * Framing Efficiency(Variable): The impact of the 8-byte control word overhead
     * as a percentage of the frame or cell size (256-byte BurstMax for this example)
     * Alignment Efficiency(Variable): The impact of invalid characters inserted to
     * pad the end of a frame to an 8-byte word boundary
     * Meta Frame Efficiency(Fixed):
     * The 99.8% efficiency created by the Synchronization, Scrambler State,
     * Diagnostic, and Skip Words (assuming a MetaFrameLength of 2K words)
     * (meta_frame_len - 4) / meta_frame_len = (2048-4)/2048 = 99.8%
     * 
     * Since Factor Framing Efficiency and Alignment Efficiency are variable, it is a
     * experience value, we take it 64/72 in the following formula.
     *
     * in case of overflow, EGW_TO_ILKN_DATA_WIDTH=64;
     *  (64 * EGW_TO_ILKN_DATA_WIDTH) / (67 * (EGW_TO_ILKN_DATA_WIDTH + 8))
     *   = (64 * 64)/ (67 *72) = 4096 /4,824 = 17/20
     */
    efficiency_rate = (ilkn_total_burst_rate * (meta_frame_len - 4) * 17) / (meta_frame_len * 20);
    /*
     * Consider RS(544, 514) Encoder, see Interlaken Reed-Solomon Forward Error Correction Extension
     * Protocol Definition, 3.1 Transmit Functions 
     * Step 1: 67b to 65b Transcode and Interleave, 2x67  ==>130bit
     * Step 2: AM Generation and Insertion 1240bit per 4096 codewords.
     * step 3: The final result is: Data field of 4096 FEC codewords contains
     * exactly 161940 blocks of tx_data_interleaved[n][129:0]
     *
     * Hence, the RS(544, 514 )Efficiency:
     * 161940*130 * 67 / 4096*5440 * 65 = 1,410,497,400/1,448,345,600 =0.9738679
     */
    /*
     * Since Factor Framing Efficiency and Alignment Efficiency are variable,
     * It is easy to think that the more lane num, the efficiency is lower,
     * there will be the same lookups during the same time, if distributing to the more num,
     * the more pad and control. Hence, the the efficiency is lower.
     * According the experience for lane num is > 8, we take it it as 95% of 64/72.
     * Since there is no customer issue so far for 8 lanes, we only adjust 12 lane PAM4 cases 
     */
