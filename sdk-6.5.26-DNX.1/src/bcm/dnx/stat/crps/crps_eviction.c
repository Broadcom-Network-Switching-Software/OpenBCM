...
#define DNX_CRPS_SEQUENCE_NUMBER_MAX_VALUE (15)
...
/** see .h file */
shr_error_e
dnx_crps_eviction_A_or_B_hw_selection_set(
    int unit,
    bcm_stat_engine_t * engine,
    bcm_stat_eviction_t * eviction)
{
    ...
    is_B_engine = (IS_B_TYPE_ENGINE(unit, engine->engine_id));

    /** Setting value fields */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IRE_ENGINE_TYPE_A_OR_B_SELECT,
                                 (engine->engine_id) % dnx_data_crps.eviction.nof_crps_network_channels_get(unit),
                                 is_B_engine);
...
shr_error_e
dnx_crps_eviction_scan_mode_get(
    int unit,
    bcm_stat_engine_t * engine,
    bcm_stat_eviction_t * eviction,
    ...
    *scan_mode = DBAL_ENUM_FVAL_CRPS_ENGINE_SCAN_MODE_SEQUENTIAL_SCAN;
    ...
    if (dnx_data_crps.eviction.feature_get(unit, dnx_data_crps_eviction_probabilistic_is_supported))
    {
        SHR_IF_ERR_EXIT(dnx_crps_db.proc.interface.
                        format.get(unit, engine->core_id, engine->engine_id, &counter_format));

        *scan_mode =
            (counter_format ==
             bcmStatCounterFormatSlimPacketsAndBytes) ? DBAL_ENUM_FVAL_CRPS_ENGINE_SCAN_MODE_NO_SCAN :
            DBAL_ENUM_FVAL_CRPS_ENGINE_SCAN_MODE_SEQUENTIAL_SCAN;
    }

    else if (!eviction->eviction_algorithmic_disable &&
             dnx_data_crps.eviction.feature_get(unit, dnx_data_crps_eviction_algorithmic_is_supported))
    {
    ...
    bcm_stat_eviction_t * eviction)
{
    ...
    /** eviction record configurations */
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CRPS_ENGINE_SCAN_OBJECT_TYPE_FOR_DMA,
                                     INST_SINGLE, eviction->eviction_event_id);
        if (eviction->type != bcmStatEvictionDestinationNetwork)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CRPS_ENGINE_SCAN_DMA_RECORD_LOGICAL_SEL,
                                         INST_SINGLE, (eviction->record_format == bcmStatEvictionRecordFormatLogical));
    ...
    /** eviction destination selection */
    {
        if (eviction->type == bcmStatEvictionDestinationNetwork)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CRPS_ENGINE_SCAN_DESTINATION_IRE_ENABLE,
                                         INST_SINGLE, TRUE);
        }
    }

    /** Eviction bubble parameters */
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CRPS_ENGINE_SCAN_BUBBLE_REQUEST_ENABLE,
                                     INST_SINGLE, TRUE);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CRPS_ENGINE_SCAN_FORCE_BUBBLE, INST_SINGLE,
                                     FALSE);
    }

    /** Eviction scan mode selection */
    {
        SHR_IF_ERR_EXIT(dnx_crps_eviction_scan_mode_get(unit, engine, eviction, &scan_mode));
     ...
     */
    {
        ...
        SHR_IF_ERR_EXIT(dnx_crps_db.proc.eviction.
                        sequential_timer_value.get(unit, engine->core_id, engine->engine_id, &seq_timer_interval));
        COMPILER_64_SET(time.time, 0, seq_timer_interval);
        time.time_units = DNXC_TIME_UNIT_USEC;
        SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &time, &seq_timer_interval));
    ...
    /*
     * Algorithmic scan parameters 
     */
    if (dnx_data_crps.eviction.feature_get(unit, dnx_data_crps_eviction_algorithmic_is_supported))
    {
        ...
        SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &time, &alg_timer_interval));
    ...
    /** Note: eviction boundaries is set in different function once all the relevant parameters are known */
    /** Performing the action */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT(dnx_crps_eviction_conditional_engine_hw_set(unit, engine, &eviction->cond));

    /** Global configuration, relevant for network eviction only */
    if (eviction->type == bcmStatEvictionDestinationNetwork)
    {
        SHR_IF_ERR_EXIT(dnx_crps_eviction_A_or_B_hw_selection_set(unit, engine, eviction));
    }
...
static shr_error_e
dnx_crps_ctrs_rate_calculation_shadows_update(
    int unit,
    int core_id,
    int engine_id,
    int counter_id,
    ...
    uint64 entry_cnt2)
{
    ...
    uint32 shadow_idx;
    uint64 record_data, algo_shadow_val;
    int nof_counters_to_update, sub_counter;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_crps_db.proc.interface.format.get(unit, core_id, engine_id, &format));
    nof_counters_to_update = DNX_CRPS_MGMT_IS_DOUAL_COUNTER_FORMAT(unit, format) ? 2 : 1;

    /** Rate calculation shadows update for each sub counter */
    for (sub_counter = 0; sub_counter < nof_counters_to_update; sub_counter++)
    {
        shadow_idx = counter_id * DNX_CRPS_MGMT_SUB_COUNT + sub_counter;
        record_data = (sub_counter == 0) ? entry_cnt1 : entry_cnt2;

        SHR_IF_ERR_EXIT(dnx_crps_mgmt_rate_algo_shadow_get(unit, core_id, engine_id, shadow_idx, &algo_shadow_val));

        if (is_seq_record)
        {
            /** Update the sequential shadow */
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_rate_seq_shadow_set
                            (unit, core_id, engine_id, shadow_idx, record_data + algo_shadow_val));

            /** Reset the algorithmic shadow*/
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_rate_algo_shadow_set(unit, core_id, engine_id, shadow_idx, 0));
        }
        else
        {
            /** Update the algorithmic shadow */
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_rate_algo_shadow_set
                            (unit, core_id, engine_id, shadow_idx, algo_shadow_val + record_data));
        }
    }
...
static shr_error_e
dnx_crps_ctrs_rate_calculation_next_expected_counter_update(
    int unit,
    int core_id,
    int engine_id)
{
    uint32 next_seq_counter_idx, logical_object_id_first, logical_object_id_last;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_crps_db.proc.rate.next_seq_counter_idx.get(unit, core_id, engine_id, &next_seq_counter_idx));
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.log_obj_id_range.
                    logical_object_id_first.get(unit, core_id, engine_id, &logical_object_id_first));
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.log_obj_id_range.
                    logical_object_id_last.get(unit, core_id, engine_id, &logical_object_id_last));
    if (next_seq_counter_idx == (logical_object_id_last - logical_object_id_first))
    {
        next_seq_counter_idx = 0;
    }
    else
    {
        next_seq_counter_idx++;
    }

    SHR_IF_ERR_EXIT(dnx_crps_db.proc.rate.next_seq_counter_idx.set(unit, core_id, engine_id, next_seq_counter_idx));
...
static shr_error_e
dnx_crps_ctrs_rate_calculation_sequence_number_update(
    int unit,
    int core_id,
    int engine_id,
    int counter_id)
{
    uint8 sequence_noumber_val;
    int shadow_idx;
    ...
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.rate.
                    sequence_num.get(unit, core_id, engine_id, shadow_idx, &sequence_noumber_val));
    sequence_noumber_val = (sequence_noumber_val == DNX_CRPS_SEQUENCE_NUMBER_MAX_VALUE) ? 0 : sequence_noumber_val + 1;
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.rate.sequence_num.set(unit, core_id, engine_id, shadow_idx, sequence_noumber_val));
...
static shr_error_e
dnx_crps_eviction_rate_calculation_update(
    int unit,
    int core_id,
    int engine_id,
    int counter_id,
    ...
    uint64 entry_cnt2)
{
    ...
    SHR_IF_ERR_EXIT(dnx_crps_ctrs_rate_calculation_shadows_update
                    (unit, core_id, engine_id, counter_id, is_seq_record, entry_cnt1, entry_cnt2));

    if (is_seq_record)
    {

        SHR_IF_ERR_EXIT(dnx_crps_ctrs_rate_calculation_next_expected_counter_update(unit, core_id, engine_id));

        SHR_IF_ERR_EXIT(dnx_crps_ctrs_rate_calculation_sequence_number_update(unit, core_id, engine_id, counter_id));
    }
...
static shr_error_e
dnx_crps_eviction_is_sequential_record_calculate(
    int unit,
    int core_id,
    int engine_id,
    uint32 counter_stat_id,
    uint8 *is_seq_record)
{
    uint32 next_seq_counter_stat_id;
    SHR_FUNC_INIT_VARS(unit);

    /*
     * In case the eviction record doesn't contain the is_sequential_scan bit,
     * the current record is from sequential scan if the counter-stat-is equal to the expected next counter id
     */

    /** Get the next expected stat id for sequential scan */
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.rate.
                    next_seq_counter_idx.get(unit, core_id, engine_id, &next_seq_counter_stat_id));

    if (counter_stat_id == next_seq_counter_stat_id)
    {
        *is_seq_record = TRUE;
    }
    else
    ...
    uint32 *curr_entry)
{
    ...
    int ctr_size_second_section;
    uint32 temp32_arr[2] = { 0 };
    uint64 temp64;
    int nof_counters_per_entry, nof_counters_per_shaddow;
    int is_zero = TRUE, rate_calculation_is_enable;
    uint32 nof_counters;
    SHR_FUNC_INIT_VARS(unit);
    ...
    /** extract the core_id */
    SHR_IF_ERR_EXIT(utilex_bitstream_get_field
                    (curr_entry, rcd_format->core_id_offset, rcd_format->core_id_size, (uint32 *) (&core_id)));
    ...
    SHR_IF_ERR_EXIT(dnx_crps_eviction_rate_calculation_is_enable
                    (unit, core_id, engine_id, &rate_calculation_is_enable));

    /** if counter is zero and rate calculation is disabled, do not update anything */
    if (!is_zero || rate_calculation_is_enable)
    {
        ...
        /** Extract the counter_id */
        {
            ...
            /** validate counter id is in the engine range  */
            engine_info = dnx_data_crps.engine.engines_info_get(unit, engine_id);
            SHR_IF_ERR_EXIT(dnx_crps_db.proc.interface.format.get(unit, core_id, engine_id, &format));
            nof_counters_per_entry =
                dnx_data_crps.engine.counter_format_types_get(unit, format)->nof_counters_per_entry;
            if (ctr_id >= engine_info->nof_entries * nof_counters_per_entry)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "counter_id=%d. bigger than nof_counters(%d) for engine=%d\n",
                             ctr_id, engine_info->nof_entries * nof_counters_per_entry, engine_id);

            }

        }

        /** Set the Entry's core_id*/
        SHR_IF_ERR_EXIT(dnx_crps_eviction_db.fifo_results.cnt_result.core_id.set(unit, nof_counters, core_id));

        /** Set the Entry's engine_id*/
        SHR_IF_ERR_EXIT(dnx_crps_eviction_db.fifo_results.cnt_result.engine_id.set(unit, nof_counters, engine_id));
        ...
        /** Set entry data */
        {
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_in_counter_shaddow_get(unit, format, &nof_counters_per_shaddow));

            /**
             * SW has two counters_int for each full/dual counter and one for single counter (narrow/slim),
             * Counter_entry contains two SW counters, s.t two single counters has the same counter_entry
             * */
            SHR_IF_ERR_EXIT(dnx_crps_eviction_db.fifo_results.cnt_result.counter_entry.set
                            (unit, nof_counters, (ctr_id / nof_counters_per_shaddow)));

        }

        /** In case the record contains the is_sequential_record bit, copy this to the crps eviction DB */
        {
            if (rate_calculation_is_enable)
            {
                /** Extract parameter from the record if exist*/
                if (rcd_format->is_probabilistic_size > 0)
                {
                    SHR_IF_ERR_EXIT(utilex_bitstream_get_field
                                    (curr_entry, rcd_format->is_probabilistic_offset, rcd_format->is_probabilistic_size,
                                     (uint32 *) &is_seq_record));

                    SHR_IF_ERR_EXIT(dnx_crps_eviction_db.fifo_results.cnt_result.
                                    is_seq_record.set(unit, nof_counters, is_seq_record));
                }
        ...
        switch (format)
        {
            case bcmStatCounterFormatTimestamp:
            ...
            case bcmStatCounterFormatPacketsAndBytes:
                ctr_size_first_section =
                    dnx_data_crps.engine.counter_format_types_get(unit, format)->nof_bits_for_packets;
                ...
                ctr_size_second_section =
                    dnx_data_crps.engine.counter_format_types_get(unit, format)->nof_bits_for_bytes;
                SHR_BITCOPY_RANGE(temp32_arr, 0, ctr, ctr_size_first_section, ctr_size_second_section);
                COMPILER_64_ZERO(temp64);
                COMPILER_64_SET(temp64, temp32_arr[1], temp32_arr[0]);
                SHR_IF_ERR_EXIT(dnx_crps_eviction_db.fifo_results.cnt_result.cnt2.set(unit, nof_counters, temp64));
                break;
            case bcmStatCounterFormatSlimPackets:
            case bcmStatCounterFormatDoublePackets:
            case bcmStatCounterFormatDoubleBytes:
    ...
    int *depth)
{
    ...
    int fifo_channel, rate_calculation_is_enable;
    SHR_FUNC_INIT_VARS(unit);
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_index)
    {
        DNX_CRPS_FIFOS_ITER(unit, fifo_id, fifo_index)
        {
            ...
            if (fifo_channel != DNX_CRPS_INVALID_DMA_CHANNEL)
            {
                /** read the data records from the current sw FIFO to eviction sw-state*/
                ...
                for (ctr_idx = 0; ctr_idx < nof_counter_entries; ctr_idx++)
                {
                    ...
                    if (valid)
                    {
                        ...
                        /** Rate calculation parameters update*/
                        {
                            SHR_IF_ERR_EXIT(dnx_crps_eviction_rate_calculation_is_enable
                                            (unit, entry_core_id, entry_engine_id, &rate_calculation_is_enable));
                            ...
                            if (rate_calculation_is_enable)
                            {
                                ...
                                {
                                    SHR_IF_ERR_EXIT(dnx_crps_eviction_db.fifo_results.cnt_result.
                                                    is_seq_record.get(unit, ctr_idx, &is_seq_record));
                                }
                                else
                                {
                                    SHR_IF_ERR_EXIT(dnx_crps_eviction_is_sequential_record_calculate
                                                    (unit, entry_core_id, entry_engine_id, counter_entry,
                                                     &is_seq_record));
                                }
                                SHR_IF_ERR_EXIT(dnx_crps_eviction_rate_calculation_update
                                                (unit, entry_core_id, entry_engine_id, counter_entry, is_seq_record,
                                                 entry_cnt1, entry_cnt2));
                            }
                        }
    ...
    void *user_data)
{
    ...
    SHR_IF_ERR_EXIT(shr_thread_manager_interval_get(crps_eviction_handler, &bgWait));
    ...
    {
        interval = DNX_CRPS_BACKGROUND_THREAD_ITER_MIN;
        SHR_IF_ERR_EXIT(shr_thread_manager_interval_set(crps_eviction_handler, 1, interval));
    ...
    else if (DNX_CRPS_FIFO_DEPTH_PREF < depth)
    {
        ...
        if (bgWait >= (DNX_CRPS_BACKGROUND_THREAD_ITER_ADJUST(bgWait) + DNX_CRPS_BACKGROUND_THREAD_ITER_MIN))
        {
            ...
            interval = bgWait - DNX_CRPS_BACKGROUND_THREAD_ITER_ADJUST(bgWait);
        }
        else
        {
            interval = DNX_CRPS_BACKGROUND_THREAD_ITER_MIN;
    ...
    else if (DNX_CRPS_FIFO_DEPTH_PREF > depth)
    {
        ...
        interval = bgWait + DNX_CRPS_BACKGROUND_THREAD_ITER_ADJUST(bgWait);
        if (interval > DNX_CRPS_BACKGROUND_THREAD_ITER_MAX)
        {
            interval = DNX_CRPS_BACKGROUND_THREAD_ITER_MAX;
        }
        SHR_IF_ERR_EXIT(shr_thread_manager_interval_set(crps_eviction_handler, 1, interval));
        LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit, "%s unit %d counter background thread delay to %dus\n"),
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(shr_thread_manager_start(crps_eviction_handler));
    ...
    int unit)
{
    ...
    if (crps_eviction_handler != NULL)
    {
        SHR_IF_ERR_EXIT(shr_thread_manager_is_active_get(crps_eviction_handler, &is_active));
    }
    if (is_active)
    {
        SHR_IF_ERR_EXIT(shr_thread_manager_stop(crps_eviction_handler, 0));
    }
exit:
    SHR_FUNC_EXIT;
    ...
    if (sw_state_is_warm_boot(unit))
    {
        DNXCMN_CORES_ITER(unit, _SHR_CORE_ALL, core_idx)
        {
            for (engine_idx = 0; engine_idx < dnx_data_crps.engine.nof_engines_get(unit); engine_idx++)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_crps_eviction_collection_handle(unit, core_idx, engine_idx));
            }
        }
    }
...
dnx_crps_eviction_init(
    int unit)
{
    shr_thread_manager_config_t crps_eviction_cfg;
    shr_thread_manager_handler_t crps_eviction_handler;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (dnx_data_crps.general.feature_get(unit, dnx_data_crps_general_is_used))
    {
        ...
        shr_thread_manager_config_t_init(&crps_eviction_cfg);
        crps_eviction_cfg.name = "DnxCtrBg";
        crps_eviction_cfg.type = SHR_THREAD_MANAGER_TYPE_EVENT;
        crps_eviction_cfg.bsl_module = BSL_LOG_MODULE;
        crps_eviction_cfg.callback = &dnx_crps_eviction_counter_bg;
        crps_eviction_cfg.interval = sal_sem_FOREVER;
        crps_eviction_cfg.start_operation = 0;
        SHR_IF_ERR_EXIT(shr_thread_manager_create(unit, &crps_eviction_cfg, &crps_eviction_handler));

        /** Save thread handler to SW-State */
        SHR_IF_ERR_EXIT(dnx_crps_eviction_db.bg_thread_data.
                        crps_eviction_thread_handler.set(unit, crps_eviction_handler));
    ...
    int unit)
{
    ...
    if (is_init)
    {
        /** Get thread handle from SW-State */
        SHR_IF_ERR_EXIT(dnx_crps_eviction_db.bg_thread_data.
                        crps_eviction_thread_handler.get(unit, &crps_eviction_handler));
        /** disable the eviction from all engines */
        SHR_IF_ERR_CONT(dnx_crps_eviction_disable_wrapper(unit));
        /** destroy thread manager*/
        SHR_IF_ERR_CONT(shr_thread_manager_destroy(&crps_eviction_handler));
        SHR_IF_ERR_EXIT(dnx_crps_eviction_db.bg_thread_data.crps_eviction_thread_handler.set(unit, NULL));
        /** detach the DMA FIFO */
...
static shr_error_e
dnx_crps_ctr_db_counter_rate_parameters_get(
    int unit,
    int core_id,
    int engine_id,
    int counter_stat_id,
    int sub_counter,
    uint64 *last_period_change,
    uint8 *seq_number)
{
    uint32 counter_idx;
    SHR_FUNC_INIT_VARS(unit);

    counter_idx = counter_stat_id * DNX_CRPS_MGMT_SUB_COUNT;
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.rate.sequence_num.get(unit, core_id, engine_id, counter_idx, seq_number));

    SHR_IF_ERR_EXIT(dnx_crps_mgmt_rate_seq_shadow_get
                    (unit, core_id, engine_id, counter_idx + sub_counter, last_period_change));
    ...
    bcm_stat_counter_output_data_t * output_data)
{
    ...
    uint64 value = 0;
    uint8 seq_number;
    uint64 last_period_change = 0;
    int lock_taken = FALSE, rate_calculation_is_enable;
    SHR_FUNC_INIT_VARS(unit);
    ...
    for (cnt_idx = 0; cnt_idx < nstat; cnt_idx++)
    {
        if ((counters_id[cnt_idx] != -1) && (sub_count[cnt_idx] != -1))
        {
            ...
            if (rate_calculation_is_enable)
            {
                SHR_IF_ERR_EXIT(dnx_crps_ctr_db_counter_rate_parameters_get
                                (unit, core_id, engine_id, counters_id[cnt_idx], sub_count[cnt_idx],
                                 &last_period_change, &seq_number));
                COMPILER_64_COPY(output_data->value_arr[cnt_idx].last_period_change, last_period_change);
                output_data->value_arr[cnt_idx].sequence_number = seq_number;
            }
        }
    }
    ...
    uint32 *time_between_scan_usec)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_time_clock_cycles_to_time_get(unit, seq_timer_interval, DNXC_TIME_UNIT_USEC, &time));
    ...
    int engine_id)
{
    ...
    if (valid)
    {
        ...
        if (is_active == FALSE)
        {
            ...
            if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_application_v1))
            {
                SHR_IF_ERR_EXIT(shr_thread_manager_interval_set
                                (crps_eviction_handler, 1, DNX_CRPS_BACKGROUND_THREAD_ITER_DEFAULT));
    ...
    int *type_counter_id_end)
{
    ...
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_in_counter_shaddow_get(unit, format, &nof_counters_per_entry));
