...
/**
 * \brief - Verify egress counter interface type
 *
 ...
 */
static shr_error_e
dnx_stat_pp_egress_interface_verify(
    int unit,
    bcm_stat_counter_interface_type_t engine_source)
{
    SHR_FUNC_INIT_VARS(unit);

    if (dnx_data_stat.stat_pp.feature_get(unit, dnx_data_stat_stat_pp_epp_combined_interfaces))
    {
        if ((engine_source == bcmStatCounterInterfaceEgressReceivePp) ||
            (engine_source == bcmStatCounterInterfaceEgressTransmitPp))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Engine source %d is not valid when Egress PP interfaces are combined\n", engine_source);
        }
    }
    else
    {
        if (engine_source == bcmStatCounterInterfaceEgressPp)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Engine source %d is not valid when Egress PP interfaces are separate\n", engine_source);
        }
    }
    ...
    bcm_stat_pp_profile_info_t * stat_pp_profile_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_stat_pp_egress_interface_verify(unit, engine_source));
    ...
    bcm_stat_counter_interface_type_t engine_source)
{
    ...
    if (engine_source == bcmStatCounterInterfaceIngressReceivePp)
    {
        {
            /*
             * JR2 FEC stats by default are two indirect counters.
             * FEC with protection is supported by single indirect counters.
             */
            if (stat_pp_profile_info->fec_profile != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Usage of fec_profile is not supported in this device");
            }
    ...
    bcm_stat_pp_info_t stat_info)
{
    ...
    if (engine_source != STAT_PP_PROFILE_ENGINE_GET(unit, stat_info.stat_pp_profile))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Engine source %d is inconsistent with stat_info.stat_pp_profile %d engine %d",
                     engine_source, stat_info.stat_pp_profile, stat_info.stat_pp_profile);
    }
    ...
    {
        if (BCM_GPORT_IS_VLAN_PORT(gport) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(gport))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "No statistic support for egress esem default entry (gport = 0x%08x)\n", gport);
        }
        else if (BCM_GPORT_IS_VLAN_PORT(gport) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(gport))
        {
            /*
             * The LIF is Egress Virtual, only valid for ETPP:
             */
            if ((engine_source != bcmStatCounterInterfaceEgressTransmitPp) &&
                (engine_source != bcmStatCounterInterfaceEgressPp))
            {
                bcm_stat_counter_interface_type_t tmp_source =
                    (dnx_data_stat.stat_pp.supported_egress_interface_counter_enum_get(unit));
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error, gport = 0x%08X is Egress Virtual but engine_source = %d is expected to be %d\n",
                             gport, engine_source, tmp_source);
            }

            /*
             * Verify that the ESEM entry contains the STAT fields.
             */
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
                            (unit, gport, DNX_ALGO_GPM_GPORT_HW_RESOURCES_EGRESS_LOCAL_LIF_OR_VIRTUAL,
                             &gport_hw_resources));

            dbal_table_id = gport_hw_resources.outlif_dbal_table_id;
            dbal_table_result_type = gport_hw_resources.outlif_dbal_result_type;

            if ((dbal_tables_field_info_get_no_err(unit, dbal_table_id, DBAL_FIELD_STAT_OBJECT_ID, 0,
                                                   dbal_table_result_type, 0, &dbal_info) == _SHR_E_NOT_FOUND) ||
                (dbal_tables_field_info_get_no_err(unit, dbal_table_id, DBAL_FIELD_STAT_OBJECT_CMD, 0,
                                                   dbal_table_result_type, 0, &dbal_info) == _SHR_E_NOT_FOUND))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Entry type with no statistics. Make sure the statistics flag is on when calling the lif creation API");
            }
        }
        /*
         * All VLAN-Port Virtual Ingress Native result types support stats, but the application 
         * isn't supported by the LIF-Table-Mngr or by dnx_algo_gpm_gport_to_hw_resources
         */
        else if (!(BCM_GPORT_IS_VLAN_PORT(gport) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS_NATIVE(gport)))
        {
            ...
            {
                lif_flags =
                    is_ingress ? DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS :
                    DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS;
                flow_flags =
                    is_ingress ? DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR :
                    DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR;
                SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                                (unit, gport, BCM_GPORT_TYPE_FLOW_LIF, flow_flags, &tmp_gport));
            }
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, tmp_gport, lif_flags, &gport_hw_resources));
            /*
             * Check if statistics were set via the LIF-Table-Mngr. 
             * SRv6 doesn't use LIF-Table-Mngr, but only has one result type that has stat
             */
            if (gport_hw_resources.outlif_dbal_table_id != DBAL_TABLE_EEDB_SRV6)
            {
                /*
                 * SRV6 DATA SID table doesn't use lif table manager and doesn't have a result type with statistics on
                 * it 
                 */
                if (gport_hw_resources.outlif_dbal_table_id == DBAL_TABLE_EEDB_SRV6_DATA_SID)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "EEDB_SRV6_DATA_SID table - statistics is not supported");
                }
                ...
                SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_valid_field
                                (unit, core_id,
                                 ...
                                 (is_ingress) ? gport_hw_resources.inlif_dbal_table_id :
                                 gport_hw_resources.outlif_dbal_table_id, is_ingress,
                                 DBAL_FIELD_STAT_OBJECT_CMD, &is_stat_cmd_valid));
                if (is_stat_cmd_valid == FALSE)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Entry type with no statistics. Make sure the statistics flag is on when calling the lif creation API for gport = 0x%08x\n",
                                 gport);
                }
            }
        }
    ...
    int *size)
{
    ...
    switch (header_type)
    {
        ...
        case bcmStatPpMetadataHeaderTypeOthers:
            ...
            dbal_layer_type_a[2] = DBAL_ENUM_FVAL_LAYER_TYPES_ITMH;
            ...
            dbal_layer_type_a[11] = DBAL_ENUM_FVAL_LAYER_TYPES_L2TP;
            dbal_layer_type_a[12] = DBAL_ENUM_FVAL_LAYER_TYPES_PTP;
            dbal_layer_type_a[13] = DBAL_ENUM_FVAL_LAYER_TYPES_Y_1731;
            dbal_layer_type_a[14] = DBAL_ENUM_FVAL_LAYER_TYPES_BIER_NON_MPLS;
            dbal_layer_type_a[15] = DBAL_ENUM_FVAL_LAYER_TYPES_IN_BIER_BASE_EG_BIER_MPLS;
            *size = 16;
            break;
        default:
 ...
 * \param [in] unit - relevant unit
 * \param [in] header_type - Header type to be converted
 * \param [in] address_type - cast in case of header_type is IPv4 or IPv6
    ...
    int *size)
{
    ...
    switch (header_type)
    {
        ...
        case bcmStatPpMetadataHeaderTypeIpv4:
            /*
             * Backward-compatitility: If address_type is 0 set both
             */
            if (address_type == 0)
            {
            ...
            else
            {
                *size = 0;
                ...
                if (_SHR_IS_FLAG_SET(address_type, BCM_STAT_PP_METADATA_ADDRESS_TYPE_UCAST))
                {
                    dbal_current_protocol_type_a[0] = DBAL_ENUM_FVAL_CURRENT_PROTOCOL_TYPE_IPV4_UC;
                    *size += 1;
                }

                if (_SHR_IS_FLAG_SET(address_type, BCM_STAT_PP_METADATA_ADDRESS_TYPE_MCAST))
                {
                    dbal_current_protocol_type_a[*size] = DBAL_ENUM_FVAL_CURRENT_PROTOCOL_TYPE_IPV4_MC;
                    *size += 1;
                }
            }
            break;
        case bcmStatPpMetadataHeaderTypeIpv6:
            ...
            else
            {
                *size = 0;
                ...
                if (_SHR_IS_FLAG_SET(address_type, BCM_STAT_PP_METADATA_ADDRESS_TYPE_MCAST))
                {
                    dbal_current_protocol_type_a[*size] = DBAL_ENUM_FVAL_CURRENT_PROTOCOL_TYPE_IPV6_MC;
                    *size += 1;
                }
            }
            break;
        ...
        case bcmStatPpMetadataHeaderTypeOthers:
            ...
            dbal_current_protocol_type_a[14] = DBAL_ENUM_FVAL_CURRENT_PROTOCOL_TYPE_BIER_MPLS;
            dbal_current_protocol_type_a[15] = DBAL_ENUM_FVAL_CURRENT_PROTOCOL_TYPE_BIER_NON_MPLS;
            *size = 16;
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported header type %d", header_type);
...
{
    int rv, enum_val;
    uint8 is_fwd_code_expanded = 1;
    SHR_FUNC_INIT_VARS(unit);

    rv = dnx_dbal_fields_enum_dbal_value_get_no_err(unit, "EGRESS_FWD_CODE", "IPV4_UC_R0", &enum_val);
    if (rv == _SHR_E_PARAM)
    {
        is_fwd_code_expanded = 0;
    }
    else
    {
        SHR_IF_ERR_EXIT(rv);
    ...
    switch (header_type)
    {
        ...
        case bcmStatPpMetadataHeaderTypeOthers:
            if (is_fwd_code_expanded)
            {
                ...
                dbal_egress_fwd_code_a[14] = DBAL_ENUM_FVAL_EGRESS_FWD_CODE_BIER_MPLS_BFR;
                dbal_egress_fwd_code_a[15] = DBAL_ENUM_FVAL_EGRESS_FWD_CODE_BIER_NON_MPLS_BFR;
    ...
    bcm_stat_pp_metadata_info_t * metadata_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_stat_pp_egress_header_types_to_dbal_convert
                    (unit,
                     metadata_info->header_type,
                     metadata_info->address_type_flags, dbal_current_protocol_type_a, &size));
...
/**
 * Set metadata value for header type specified in metadata_info - for forward plus one step
 */
static shr_error_e
dnx_stat_pp_fwd_plus_one_metadata_set(
    int unit,
    bcm_stat_pp_metadata_info_t * metadata_info)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_STMD, &entry_handle_id));

    switch (metadata_info->header_type)
    {
        case bcmStatPpMetadataHeaderTypeIpv4:
            /*
             * Note: address type cannot be set to ucast | mcast, it is necessarily one of them
             */
            if (metadata_info->address_type_flags == BCM_STAT_PP_METADATA_ADDRESS_TYPE_UCAST)
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV4UC,
                                             INST_SINGLE, metadata_info->metadata);
            }
            else if (metadata_info->address_type_flags == BCM_STAT_PP_METADATA_ADDRESS_TYPE_MCAST)
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV4MC,
                                             INST_SINGLE, metadata_info->metadata);
            }
            else
            {
        ...
        case bcmStatPpMetadataHeaderTypeIpv6:
            ...
            if (metadata_info->address_type_flags == BCM_STAT_PP_METADATA_ADDRESS_TYPE_UCAST)
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6UC,
                                             INST_SINGLE, metadata_info->metadata);
            }
            else if (metadata_info->address_type_flags == BCM_STAT_PP_METADATA_ADDRESS_TYPE_MCAST)
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6MC,
                                             INST_SINGLE, metadata_info->metadata);
            }
            else
            {
            ...
            break;
        case bcmStatPpMetadataHeaderTypeMpls:
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS, INST_SINGLE, metadata_info->metadata);
            break;
        default:
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OTHER, INST_SINGLE, metadata_info->metadata);
            break;
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/**
 * Get metadata value for header type specified in metadata_info - for forward plus one step
 */
static shr_error_e
dnx_stat_pp_fwd_plus_one_metadata_get(
    int unit,
    bcm_stat_pp_metadata_info_t * metadata_info)
{
    uint32 entry_handle_id;
    ...
    switch (metadata_info->header_type)
    {
        case bcmStatPpMetadataHeaderTypeIpv4:
            ...
            if (metadata_info->address_type_flags == BCM_STAT_PP_METADATA_ADDRESS_TYPE_UCAST)
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                                    DBAL_FIELD_IPV4UC, INST_SINGLE,
                                                                    &metadata_info->metadata));
            }
            else if (metadata_info->address_type_flags == BCM_STAT_PP_METADATA_ADDRESS_TYPE_MCAST)
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                                    DBAL_FIELD_IPV4MC, INST_SINGLE,
                                                                    &metadata_info->metadata));
            }
            else
            {
        ...
        case bcmStatPpMetadataHeaderTypeIpv6:
            ...
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                                    DBAL_FIELD_IPV6UC, INST_SINGLE,
                                                                    &metadata_info->metadata));
            }
            else if (metadata_info->address_type_flags == BCM_STAT_PP_METADATA_ADDRESS_TYPE_MCAST)
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                                    DBAL_FIELD_IPV6MC, INST_SINGLE,
                                                                    &metadata_info->metadata));
            }
            else
            {
        ...
        case bcmStatPpMetadataHeaderTypeMpls:
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                                DBAL_FIELD_MPLS, INST_SINGLE,
                                                                &metadata_info->metadata));
            break;
        default:
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                                DBAL_FIELD_OTHER, INST_SINGLE,
                                                                &metadata_info->metadata));
            break;
    }
    ...
    bcm_stat_pp_metadata_info_t * metadata_info)
{
    ...
    else if ((_SHR_IS_FLAG_SET(flow_step, BCM_STAT_PP_METADATA_FORWARD_PLUS_ONE)))
    {
        SHR_IF_ERR_EXIT(dnx_stat_pp_fwd_plus_one_metadata_set(unit, metadata_info));
    }
    ...
    bcm_stat_pp_metadata_info_t * metadata_info)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_stat_pp_fwd_plus_one_metadata_get(unit, metadata_info));
    }
    ...
    bcm_stat_pp_metadata_info_t * metadata_info)
{
    ...
    if (metadata_info->flags != (BCM_STAT_PP_METADATA_EGRESS | BCM_STAT_PP_METADATA_TUNNEL) &&
        ...
        metadata_info->flags != (BCM_STAT_PP_METADATA_EGRESS | BCM_STAT_PP_METADATA_FORWARD_PLUS_ONE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported flags %d", metadata_info->flags);
    }
    ...
    bcm_stat_pp_metadata_info_t * metadata_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_pp_metadata_check_args(unit, metadata_info));
...
/** see .h file */
shr_error_e
dnx_stat_pp_egress_l2_fwd_plus_one_metadata_enable_set(
    int unit,
    int arg)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_STMD, &entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PAE_ENABLE, INST_SINGLE, arg ? 1 : 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/** see .h file */
shr_error_e
dnx_stat_pp_egress_l2_fwd_plus_one_metadata_enable_get(
    int unit,
    int *arg)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_PAE_ENABLE, INST_SINGLE, &field_val));
    *arg = field_val;
    ...
    bcm_stat_pp_info_t stat_info)
{
    ...
    {
        /*
         * engine_source == bcmStatCounterInterfaceEgressTransmitPp or bcmStatCounterInterfaceEgressPp
                    ...
                    DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_PHP_STAT)))
        {
            ...
            STAT_PP_ETPP_SWITCH_CMD_SET(stat_id, stat_pp_profile, engine_source);
    ...
    int interface_type)
{
    ...
    else if (interface_type == bcmStatCounterInterfaceEgressReceivePp)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "ERPP counting engine is not supported");
             ...
             (interface_type == bcmStatCounterInterfaceEgressPp))
    {
    ...
    if (is_meter == FALSE)
    {
        ...
        {
            SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_generation_verify
...
/**
 * \brief - Function to check that the values used for ERPP_COUNTER_PROFILE match the register controlling them.
 * \param [in] unit - relevant unit
 *
 * \return
 *   Negative in case of an error. See \ref shr_error_e
 ...
 */
static shr_error_e
dnx_stat_pp_erpp_counter_profile_init_verify(
    int unit)
{
    uint32 entry_handle_id;
    uint32 invalid_value_from_reg;
    uint32 skip_value_from_reg;
    uint32 invalid_value_from_dbal;
    uint32 skip_value_from_dbal;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_STAT_PP_ERPP_COUNTER_PROFILE_VALUES, &entry_handle_id));
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_ERPP_COUNTER_PROFILE_INVALID_VALUE, INST_SINGLE,
                               &invalid_value_from_reg);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_ERPP_COUNTER_PROFILE_SKIP_VALUE, INST_SINGLE,
                               &skip_value_from_reg);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_hw_value_get
                    (unit, DBAL_FIELD_TYPE_DEF_ERPP_COUNTER_PROFILE, DBAL_ENUM_FVAL_ERPP_COUNTER_PROFILE_INVALID,
                     &invalid_value_from_dbal));
    SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_hw_value_get
                    (unit, DBAL_FIELD_TYPE_DEF_ERPP_COUNTER_PROFILE, DBAL_ENUM_FVAL_ERPP_COUNTER_PROFILE_SKIP,
                     &skip_value_from_dbal));

    if (invalid_value_from_reg != invalid_value_from_dbal)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "Invalid ERPP counter profile value is %d, but in register %d.\r\n",
                     invalid_value_from_dbal, invalid_value_from_reg);
    }
    if (skip_value_from_reg != skip_value_from_dbal)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "Skip ERPP counter profile value is %d, but in register %d.\r\n",
                     skip_value_from_dbal, skip_value_from_reg);
    }
...
/**
 * \brief - init function, performed during bcm init stage.
 *          Initilaize the per outlif profile and per pp ports tables to not sent anything to CRPS.
 * \param [in] unit - relevant unit
 *
 * \return
 *   Negative in case of an error. See \ref shr_error_e
 ...
 */
static shr_error_e
dnx_stat_pp_erpp_counter_profile_init(
    int unit)
{
    uint32 entry_handle_id;
    int outlif_profile_max_val;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_stat_pp_erpp_counter_profile_init_verify(unit));

    SHR_IF_ERR_EXIT(dbal_tables_field_max_value_get
                    (unit, DBAL_TABLE_STAT_PP_ERPP_COUNTER_PROFILE_PER_OUTLIF, DBAL_FIELD_OUT_LIF_PROFILE, TRUE, 0,
                     INST_SINGLE, &outlif_profile_max_val));

    /*
     * Per outlif_profile table.
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_STAT_PP_ERPP_COUNTER_PROFILE_PER_OUTLIF, &entry_handle_id));
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_OUT_LIF_PROFILE, 0, outlif_profile_max_val);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COUNTER_0_PROFILE, INST_SINGLE,
                                 DBAL_ENUM_FVAL_ERPP_COUNTER_PROFILE_INVALID);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COUNTER_1_PROFILE, INST_SINGLE,
                                 DBAL_ENUM_FVAL_ERPP_COUNTER_PROFILE_INVALID);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    /*
     * Per PP port table is not set.
     * Should not be reached since per outlif_profile table is set to invalid and not skip.
     */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_stat_pp_profile_init(unit));

    if (dnx_data_stat.stat_pp.feature_get(unit, dnx_data_stat_stat_pp_erpp_counter_profile_per_outlif))
    {
        SHR_IF_ERR_EXIT(dnx_stat_pp_erpp_counter_profile_init(unit));
    }
...
bcm_dnx_stat_pp_profile_delete(
    ...
    int stat_pp_profile)
{
    uint32 entry_handle_id = (uint32) (-1);
    uint32 engine_source = 0;
    uint32 profile_id;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);

    engine_source = STAT_PP_PROFILE_ENGINE_GET(unit, stat_pp_profile);
    profile_id = STAT_PP_PROFILE_ID_GET(stat_pp_profile);

    SHR_INVOKE_VERIFY_DNXC(dnx_stat_pp_profile_get_delete_verify(unit, engine_source, profile_id));
    ...
    switch (engine_source)
    {
        ...
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Profile %d is assigned to an unsupported stat counter interface",
                         stat_pp_profile);
            break;
    }
    ...
    SHR_IF_ERR_EXIT(STAT_PP_SW_STATE_RESOURCE_EXEC_BY_ENGINE_PROFILE
                    (engine_source, profile_id, STAT_PP_IS_ETPP_METERING_PROFILE(profile_id),
                     free_single(unit, profile_id, NULL)));
    ...
    bcm_stat_pp_profile_info_t * stat_pp_profile_info)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_pp_profile_create_verify
    ...
    bcm_stat_pp_info_t stat_info)
{
    ...
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_pp_gport_set_verify(unit, gport, core_id, engine_source, stat_info));
    ...
    bcm_stat_pp_info_t * stat_info)
{
    ...
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_pp_gport_get_delete_verify(unit, engine_source));
