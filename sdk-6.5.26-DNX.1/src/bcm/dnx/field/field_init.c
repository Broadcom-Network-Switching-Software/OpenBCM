    ...
    int unit)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        /**Init SW of external TACM*/
        SHR_IF_ERR_EXIT(dnx_field_group_tcam_external_swstate_init(unit));
        ...
        /** Init SW state flush module */
        SHR_IF_ERR_EXIT(dnx_field_flush_sw_state_init(unit));
    ...
    int unit)
{
    ...
    if (state_table_uses_rmw)
    {
        ...
        dbal_enum_value_field_state_table_ipmf1_key_select_e ipmf1_key_select = -1;
        dnx_field_stage_e field_stage;
        int use_acr;
        dbal_enum_value_field_field_key_e key_id;
        int lsb_on_key;
        int state_table_key_size_get;
...
*/
static shr_error_e
dnx_field_init_ecmp_tunnel_priority_dummy_table(
    int unit)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_PMF, &entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DUMMY_ZERO_BIT, INST_ALL, 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    int unit)
{
     ...
     */
    exem_key_select_1 = dnx_data_field.exem.ipmf1_key_get(unit);
    exem_key_select_2 = dnx_data_field.exem.small_ipmf2_key_hw_value_get(unit);
    map_key_select_1 = dnx_data_field.mdb_dt.ipmf1_key_select_get(unit);
    map_key_select_2 = dnx_data_field.mdb_dt.ipmf2_key_select_get(unit);
    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        if (dnx_data_field.mdb_dt.supported_get(unit))
        {
            if (dnx_data_field.mdb_dt.ipmf3_key_select_supported_get(unit))
            {
                map_key_select = dnx_data_field.mdb_dt.ipmf3_key_select_get(unit);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MAP_KEY_SELECT, INST_SINGLE,
                                             map_key_select);
            }
        }
...
/**
* \brief
*  Checks that the VMV size is the same for all entry encodings.
* \param [in] unit                  - Device ID
* \param [in] nof_tables            - Number of elements in array dbal_physical_tables.
* \param [in] dbal_physical_tables  - Array of physical tables. Number of elements is nof_tables.
* \param [in] is_available          - If FALSE, we expect the physical tables to not be available for devices.
...
*/
static shr_error_e
dnx_field_init_exem_vmv_size_same_for_all_encoding_verify(
    int unit,
    int nof_tables,
    ...
    int is_available)
{
    ...
    min_vmv_size = DNX_FIELD_EXEM_MIN_VMV_SIZE_NOT_INCLUDING_2MSB;
    if (dnx_data_field.exem.feature_get(unit, dnx_data_field_exem_exem_vmv_padded_for_2msb))
    {
        min_vmv_size += 2;
    }

    for (dbal_physical_tables_idx = 0; dbal_physical_tables_idx < nof_tables; dbal_physical_tables_idx++)
    {
        vmv_size_first = -1;
        for (entry_encoding_idx = 0; entry_encoding_idx < (COUNTOF(entry_encoding)); entry_encoding_idx++)
        {
            ...
            else if (vmv_size != vmv_size_first)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "VMV size changing by entry encoding not supported. "
                             ...
                             dbal_physical_tables[dbal_physical_tables_idx],
                             entry_encoding[entry_encoding_idx], vmv_size, vmv_size_first);
            }
        }
        if (is_available)
        {
            if (vmv_size < min_vmv_size)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "VMV size for dbal_physical_tables %d is %d, under minimum %d.\n",
                             dbal_physical_tables[dbal_physical_tables_idx], vmv_size, min_vmv_size);
            }
        }
        else
        {
            if (vmv_size != 0)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "VMV size for dbal_physical_tables %d is %d, expected 0.\n",
                             dbal_physical_tables[dbal_physical_tables_idx], vmv_size);
            }
        }
    }
...
*/
static shr_error_e
dnx_field_init_exem_vmv_configuration_verify(
    int unit)
{
    dbal_physical_tables_e dbal_physical_tables_v1[] = {
    ...
    if (dnx_data_field.exem.feature_get(unit, dnx_data_field_exem_stage_selection_includes_ipmf1))
    {
        SHR_IF_ERR_EXIT(dnx_field_init_exem_vmv_size_same_for_all_encoding_verify
                        (unit, COUNTOF(dbal_physical_tables_v1), dbal_physical_tables_v1, FALSE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_field_init_exem_vmv_size_same_for_all_encoding_verify
                        (unit, COUNTOF(dbal_physical_tables_v1), dbal_physical_tables_v1, TRUE));
    }
...
*/
static shr_error_e
dnx_field_init_epmf_kbr(
    int unit)
{
    dbal_enum_value_field_field_key_e key_id = DBAL_ENUM_FVAL_FIELD_KEY_B;
    int nof_contexts = dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_EPMF)->nof_contexts;
    ...
    if (dnx_data_field.tests.epmf_debug_rely_on_tcam0_get(unit))
    {
        ...
        fg_tcam.stage = DNX_FIELD_TCAM_STAGE_EPMF;

        /*
         * Set occupied of last access profile to TRUE
         */
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_profile_reserve(unit, app_db_id));

        for (ctx_id = 0; ctx_id < nof_contexts; ctx_id++)
        {
            SHR_IF_ERR_EXIT(dnx_field_key_epmf_key_b_kbr_init(unit, DNX_FIELD_STAGE_EPMF, ctx_id, key_id));
        }
    }
...
/**
* \brief
*  Init CS Outlif profile mapping
* \param [in] unit  - Device ID
* \param [in] table_name  - Table ID used for handle allocation
* \return
*   shr_error_e - Error Type
...
*/
static shr_error_e
dnx_field_init_cs_outlif_profile_map(
    int unit,
    dbal_tables_e table_name)
{
    uint32 entry_handle_id;
    ...
    uint32 field_profile, outlif_profile, outrif_profile;
    field_profile = 1;
    outlif_profile = field_profile;
    outrif_profile = field_profile << (dnx_data_lif.out_lif.outrif_profile_width_get(unit) - 1);

    if (dnx_data_field.features.epmf_outlif_profile_map_get(unit))
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_name, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CS_OUTLIF_PROFILE_ENTRY, outlif_profile);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUTLIF_PROFILE, INST_SINGLE, outlif_profile);
        /** writing the entry */
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CS_OUTLIF_PROFILE_ENTRY, outrif_profile);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUTLIF_PROFILE, INST_SINGLE, outrif_profile);
        /** writing the entry */
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    int unit)
{
    ...
    dnx_field_context_t context_id_max = dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_contexts;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FIELD_PMF_CONTEXT_GENERAL, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FIELD_PMF_STAGE, DBAL_ENUM_FVAL_FIELD_PMF_STAGE_IPMF3);
    for (context_id = 0; context_id < context_id_max; context_id++)
    {
         ...
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ACL_CONTEXT_PROFILE, INST_SINGLE, context_id);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
...
/**
* \brief
*  Init MDB intefarce MUX register for the EXEM lookups
* \param [in] unit  - Device ID
* \return
*   shr_error_e - Error Type
* \remark
...
*/
static shr_error_e
dnx_field_init_mux_mdb_exem_interfaces(
    int unit)
{
    uint32 entry_handle_id;
    ...
    */
    ...
    if (dnx_data_field.exem.feature_get(unit, dnx_data_field_exem_stage_selection_includes_ipmf1))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Error, compilation mismatch for exem stage_selection_includes_ipmf1 feature.\n");
    }
    else
    {
        /*
         * The value of the SEXEM MUX is dependent on the SOC property pmf_sexem3_stage.
         */
        int sexem_mux_value;
        dnx_field_stage_e sexem_stage = dnx_data_field.exem.pmf_sexem3_stage_get(unit);
        ...
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_MUX_SEXEM_INTERFACE, INST_SINGLE,
                                    sexem_mux_value);
    }
...
/**
* \brief
*  Init MDB intefarce MUX register for the MDB direct table lookups.
* \param [in] unit  - Device ID
* \return
*   shr_error_e - Error Type
* \remark
...
*/
static shr_error_e
dnx_field_init_mux_mdb_dt_interfaces(
    int unit)
{
    uint32 entry_handle_id;
    ...
    {
        int map_mux_value;
        int ipmf1_ipmf2_concurrent_support = dnx_data_field.mdb_dt.ipmf1_ipmf2_concurrent_support_get(unit);
        dnx_field_stage_e map_stage = dnx_data_field.mdb_dt.pmf_map_stage_get(unit);
        /**
         * writing data foe relevant table
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FIELD_PMF_MUX_MDB_INTERFACES, &entry_handle_id));
        ...
        else if ((ipmf1_ipmf2_concurrent_support == 0 && map_stage == DNX_FIELD_STAGE_IPMF2) ||
                 (ipmf1_ipmf2_concurrent_support == 1 && map_stage == DNX_FIELD_STAGE_IPMF3))
        {
            /*
             * Mux is used for IPMF2 when no ipmf1-ipmf2 concurrent support, otherwise it's used for IPMF3 
             */
            map_mux_value = 1;
        }
        else
...
/**
* \brief
*  Init MDB inteface MUX register for the SCE lookups.
* \param [in] unit  - Device ID
* \return
*   shr_error_e - Error Type
* \remark
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_init_mux_mdb_exem_interfaces(unit));
    SHR_IF_ERR_EXIT(dnx_field_init_mux_mdb_dt_interfaces(unit));
    ...
    {
        /** If inlif profile doesn't use PMF mapping, init the mapping */
        SHR_IF_ERR_EXIT(dnx_field_init_cs_inlif_profile_map(unit, DBAL_TABLE_FIELD_IPMF1_CS_INLIF_PROFILE_MAP));
    }
    
    if (dnx_data_field.features.epmf_outlif_profile_map_get(unit))
    {
        SHR_IF_ERR_EXIT(dnx_field_init_cs_outlif_profile_map(unit, DBAL_TABLE_FIELD_EPMF_CS_OUTLIF_PROFILE_MAP));
    }
    SHR_IF_ERR_EXIT(dnx_field_init_epmf_kbr(unit));

    /** Verify EXEM VMV configuration */
    SHR_INVOKE_VERIFY_DNXC(dnx_field_init_exem_vmv_configuration_verify(unit));
...
/**
 * \brief
 *  Creates a new context that sets system header profile to none and bytes to remove to 0.
 *  A preselector is then created for this context when PRT_QUALIFIER is equal to the given PRT_QUAL
 *
 * \param [in] unit      - Device ID
 * \param [in] prt_qual  - qual value
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
 ...
 */
shr_error_e
dnx_field_init_prt_append64_sys_hdr_set(
    int unit,
    int prt_qual)
{
    ...
    sal_strncpy_s((char *) (context_info.name), "RCY_MIRROR_SYS_HDR_CHG", sizeof(context_info.name));
    ...
    p_id.presel_id = dnx_data_field.preselector.default_rcy_mirror_sys_hdr_presel_id_ipmf1_get(unit);
    p_id.stage = bcmFieldStageIngressPMF1;
    p_data.entry_valid = TRUE;
    p_data.context_id = context_id;
    p_data.nof_qualifiers = 1;

    p_data.qual_data[0].qual_type = bcmFieldQualifyPtch;
    p_data.qual_data[0].qual_arg = 0;
    p_data.qual_data[0].qual_value = prt_qual;
    ...
    int unit)
{
    ...
    for (ii = 0; ii < DNX_FIELD_CONTEXT_SYS_HDR_PROFILE_NOF; ii++)
    {
         ...
         */
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SYSTEM_HEADER_PROFILE, ii);
    ...
    if (dnx_data_headers.ftmh.add_dsp_ext_get(unit))
    {
         ...
         */
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_SYSTEM_HEADER_PROFILE, DBAL_RANGE_ALL,
                                         DBAL_RANGE_ALL);
    ...
    if (dnx_data_port_pp.general.rcy_info_prt_qualifier_eth_over_append64_get(unit) != 0)
    {
        SHR_IF_ERR_EXIT(dnx_field_init_prt_append64_sys_hdr_set(unit,
                                                                dnx_data_port_pp.
                                                                general.rcy_info_prt_qualifier_eth_over_append64_get
                                                                (unit)));
    }
...
shr_error_e
dnx_field_udh_len_jericho_mode_get(
    int unit,
    int arr_size,
    uint32 *udh_type_len)
{
    ...
    if (arr_size < DNX_DATA_MAX_FIELD_UDH_TYPE_COUNT)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "wrong arr_size. should be DNX_DATA_MAX_FIELD_UDH_TYPE_COUNT \n");
    }
...
static shr_error_e
dnx_field_udh_init_without_extensions(
    int unit)
{
    uint32 entry_handle_id;
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_udh_init_without_extensions(unit));
    }
    ...
    int unit)
{
    ...
    while (table_id != DBAL_TABLE_EMPTY)
    {
        ...
        prefix_size = dnx_data_field.tcam.max_prefix_size_get(unit);
        db.prefix_size = UTILEX_MIN(prefix_size, app_db_size);
    ...
    int unit)
{
    ...
    /**
     * FES and KEY are initialized by HW init mechanism! (dev_init)
     *
    ...
    if (dnx_data_field.features.ecmp_tunnel_priority_get(unit))
    {
        SHR_IF_ERR_EXIT(dnx_field_init_ecmp_tunnel_priority_dummy_table(unit));
    }
