    ...
    dnx_field_ace_format_info_for_sw_t * ace_format_info_p)
{
    ...
    ace_format_info_sw.use_ppmc_range = ace_format_info_p->use_ppmc_range;
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
     ...
     */
    for (action_idx = 0; action_idx < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP; action_idx++)
    {
        fg_info_p->actions_payload_info.actions_on_payload_info[action_idx].dnx_action = DNX_FIELD_ACTION_INVALID;
    }
    /*
    ...
    dnx_field_ace_format_info_for_sw_t * ace_format_info_p)
{
    ...
    ace_format_info_p->use_ppmc_range = ace_format_info_sw.use_ppmc_range;
    ...
    dnx_field_ace_format_info_t * ace_format_info_in_p)
{
    ...
    ace_format_info_in_p->use_ppmc_range = FALSE;
    ...
    dnx_field_actions_fg_payload_info_t * actions_payload_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_actions_template_get(unit, &actions_payload_info_sw, actions_payload_info_p));
    ...
    int *buffer_shift_p)
{
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_EXEM:
        {
            ...
            {
                (*buffer_shift_p) += dnx_data_field.efes.epmf_exem_zero_padding_added_to_field_io_get(unit);
            }
            break;
        }
 ...
 */
shr_error_e
dnx_field_ace_format_uses_ppmc_range(
    int unit,
    dnx_field_ace_id_t ace_id,
    uint8 *use_ppmc_range_p)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(use_ppmc_range_p, _SHR_E_PARAM, "use_ppmc_range_p");
    DNX_FIELD_ACE_ID_VERIFY(unit, ace_id);

    SHR_IF_ERR_EXIT(dnx_field_group_sw.ace_format_info.use_ppmc_range.get(unit, ace_id, use_ppmc_range_p));
    ...
    dnx_field_action_attach_info_t attach_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_info_payload_info_get
                    (unit, fg_id, context_id, field_stage, &actions_payload_info_sw, fes_group_info_p, attach_info));
exit:
    SHR_FUNC_EXIT;
}
    ...
    dnx_field_group_info_t * fg_info_in_p)
{
         ...
         && fg_info_in_p->dnx_actions[action_ndx] != DNX_FIELD_ACTION_INVALID; action_ndx++)
    {
        if (fg_info_in_p->use_valid_bit[action_ndx])
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Action number %d (%s) is assigned a valid bit, "
    ...
    {
        dnx_field_stage_e equivalent_stage;
        if (dnx_data_field.mdb_dt.ipmf1_ipmf2_concurrent_support_get(unit) &&
            (fg_info_in_p->field_stage == DNX_FIELD_STAGE_IPMF2))
        {
            equivalent_stage = DNX_FIELD_STAGE_IPMF1;
        }
        else
        {
            equivalent_stage = fg_info_in_p->field_stage;
        }
        if (dnx_data_field.mdb_dt.pmf_map_stage_get(unit) != equivalent_stage)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "MDB DT type field group in stage \"%s\", but "
                         "SOC property \"pmf_map_stage\" selected stage \"%s\" for MDB DT.\n",
                         dnx_field_stage_text(unit, fg_info_in_p->field_stage),
                         dnx_field_stage_text(unit, dnx_data_field.mdb_dt.pmf_map_stage_get(unit)));
        }
    }
    ...
    /*
     * Verify that LPM fdoes not use user ranged qualifiers.
     */
    if ((fg_info_in_p->fg_type == DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM) &&
        ((fg_info_in_p->external_tcam_info.flags & BCM_FIELD_GROUP_EXTERNAL_FLAG_LPM) != 0) && (nof_ranged_qual != 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "External field group is to be LPM, but a ranged qualifier is used.\r\n");
    }
    ...
    if ((fg_info_in_p->fg_type == DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM) &&
        ...
        ((fg_info_in_p->external_tcam_info.flags & BCM_FIELD_GROUP_EXTERNAL_FLAG_ENTRIES_PER_CORE) != 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "External field group set to have entries per core, but there is only one core on device.\r\n");
    }
    ...
    unsigned int *block_size)
{
    ...
    switch (action_length_type)
    {
        ...
        case DNX_FIELD_ACTION_LENGTH_TYPE_MDB_DT:
        {
             ...
             * size)
             */
            (*block_size) = dnx_data_field.mdb_dt.action_size_large_get(unit);
            break;
        }
            ...
            else
            {
                ...
                if (fg_stage == DNX_FIELD_STAGE_IPMF1)
                {
                    ...
                    if (is_attached)
                    {
                        ...
                        /*
                         * Find if the field group is direct extraction.
                         */
                        if (fg_type == DNX_FIELD_GROUP_TYPE_DIRECT_EXTRACTION)
                        {
                            is_dir_ext = TRUE;
...
* \param [in] core_mode            - If entries can be added by core (DPC or SBC)
    ...
    dbal_tables_e dbal_table_id)
{
    ...
    SHR_IF_ERR_EXIT(mdb_init_logical_table(unit, dbal_table_id));
    ...
    dnx_field_group_attach_info_t * attach_info_p)
{
    ...
    /*
     * Verify no input type qualifier Const is attached to external field group
     */
    if (fg_type == DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM)
    {
        for (qual_ndx = 0; qual_ndx < nof_quals; qual_ndx++)
        {
            if (attach_info_p->qual_info[qual_ndx].input_type == DNX_FIELD_INPUT_TYPE_CONST)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Input type Const is not supported for External TCAM field groups (%d).",
                             fg_id);
            }
        }
    }
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    for (fg_id_index = 0; fg_id_index < nof_fgs; fg_id_index++)
    {
        ...
        if (is_fg_on_context)
        {
            ...
            if (fg_to_cmp_type != fg_type)
            {
                continue;
            }
            if (fg_key != DNX_FIELD_KEY_ID_INVALID)
            {
                dnx_field_key_id_t fg_to_cmp_key_id;
                dbal_enum_value_field_field_key_e fg_to_cmp_one_key_id;

                SHR_IF_ERR_EXIT(dnx_field_group_context_key_id_get(unit, fg_id_index, context_id, &fg_to_cmp_key_id));
                fg_to_cmp_one_key_id = (fg_to_cmp_key_id.id[1] != DNX_FIELD_KEY_ID_INVALID) ?
                    (fg_to_cmp_key_id.id[1]) : (fg_to_cmp_key_id.id[0]);
                if (fg_to_cmp_one_key_id != fg_key)
                {
                    continue;
                }
            }
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM:
        {
            switch (dnx_data_field.external_tcam.type_get(unit))
            {
                ...
                {
#if defined(INCLUDE_KBP)
                    SHR_IF_ERR_EXIT(dnx_field_kbp_group_opcode_attach(unit, fg_id, context_id,
                                                                      group_attach_info_p, &context_attach_info));
#else
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
                    break;
...
/**
 * \brief
 *  Returns one field group that cascades from a field group.
 *
 * \param[in] unit                 - Device ID
 * \param[in] cascaded_fg_id       - The cascading field group.
 * \param[out] found_cascading_p   - If true there is a cascading field group.
 * \param[out] cascading_fg_id_p   - If found_cascading_p is true, returns one of the cascading field groups.
 *
 * \return
 *   shr_error_e - Error Type
 * \remark
 ...
 */
static shr_error_e
dnx_field_group_find_cascading_group(
    int unit,
    dnx_field_group_t cascaded_fg_id,
    int *found_cascading_p,
    dnx_field_group_t * cascading_fg_id_p)
{
    int nof_fgs;
    int nof_contexts;
    int fg_id_ndx;
    ...
    dnx_field_stage_e field_stage_cascaded;
    dnx_field_stage_e field_stage_cascading;
    dnx_field_qual_t dnx_quals[DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG];
    unsigned int nof_quals;
    dnx_field_qual_attach_info_t qual_attach_info[DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG];
    ...
    (*cascading_fg_id_p) = DNX_FIELD_QUAL_TYPE_INVALID;

    /** Sanity check*/
    SHR_IF_ERR_EXIT(dnx_field_group_field_stage_get(unit, cascaded_fg_id, &field_stage_cascaded));
    if (field_stage_cascaded != DNX_FIELD_STAGE_IPMF1)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Cannot cascade from stage %d, must be iPMF1. fg_id %d.\n",
                     field_stage_cascaded, cascaded_fg_id);
    }

    nof_fgs = dnx_data_field.group.nof_fgs_get(unit);
    nof_contexts = dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF2)->nof_contexts;

    for (fg_id_ndx = 0; (fg_id_ndx < nof_fgs) && ((*found_cascading_p) == FALSE); fg_id_ndx++)
    {
        SHR_IF_ERR_EXIT(dnx_algo_field_group_is_allocated(unit, fg_id_ndx, &fg_is_alloc));
        if (fg_is_alloc == FALSE)
        {
            continue;
        }
        SHR_IF_ERR_EXIT(dnx_field_group_field_stage_get(unit, fg_id_ndx, &field_stage_cascading));
        if (field_stage_cascading != DNX_FIELD_STAGE_IPMF2)
        {
            continue;
        }
        for (context_ndx = 0; context_ndx < nof_contexts; context_ndx++)
        {
            SHR_IF_ERR_EXIT(dnx_field_group_is_context_id_on_arr(unit, fg_id_ndx, context_ndx, &is_attached));
            if (is_attached == FALSE)
            {
                continue;
            }
            /** Note that the qualifier types are only collected for sanity check.*/
            SHR_IF_ERR_EXIT(dnx_field_group_dnx_qual_arr_get(unit, fg_id_ndx, dnx_quals, &nof_quals));
            SHR_IF_ERR_EXIT(dnx_field_group_context_qualifier_attach_info_get
                            (unit, fg_id_ndx, context_ndx, qual_attach_info));
            for (qual_ndx = 0; (qual_ndx < nof_quals) && ((*found_cascading_p) == FALSE); qual_ndx++)
            {
                if (qual_attach_info[qual_ndx].input_type == DNX_FIELD_INPUT_TYPE_CASCADED)
                {
                    /** Sanity check*/
                    if (DNX_QUAL_CLASS(dnx_quals[qual_ndx]) != DNX_FIELD_QUAL_CLASS_USER)
                    {
                        SHR_ERR_EXIT(_SHR_E_INTERNAL, "fg_id %d has qualifier %d of type 0x%x with input type "
                                     "cascaded, but is not a user defined qualifer.\n",
                                     cascaded_fg_id, qual_ndx, dnx_quals[qual_ndx]);
                    }
                    if (qual_attach_info[qual_ndx].input_arg == cascaded_fg_id)
                    {
                        (*found_cascading_p) = TRUE;
                        (*cascading_fg_id_p) = fg_id_ndx;
                    }
                }
            }
        }
    ...
    dnx_field_context_t context_id)
{
    ...
    if (field_stage == DNX_FIELD_STAGE_IPMF1)
    {
        ...
        if (nof_cascading_refs > 0)
        {
            int found_cascading_fg;
            dnx_field_group_t cascading_fg_id;
            SHR_IF_ERR_EXIT(dnx_field_group_find_cascading_group(unit, fg_id, &found_cascading_fg, &cascading_fg_id));
            if (found_cascading_fg)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Can't detach FG %d from Context %d: "
                             "FG payload is being used for cascading by %d qualifiers. "
                             "Field group %d is cascading from it.\r\n",
                             fg_id, context_id, nof_cascading_refs, cascading_fg_id);
            }
            else
            {
                dnx_field_presel_entry_id_t cascading_presel_entry_id;
                int found_cascading_presel;
                SHR_IF_ERR_EXIT(dnx_field_presel_find_cascading_presel
                                (unit, fg_id, &found_cascading_presel, &cascading_presel_entry_id));
                if (found_cascading_presel)
                {
                    dnx_field_presel_entry_data_t presel_entry_data;
                    SHR_IF_ERR_EXIT(dnx_field_presel_entry_data_t_init(unit, &presel_entry_data));
                    SHR_IF_ERR_EXIT(dnx_field_presel_get(unit, 0, &cascading_presel_entry_id, &presel_entry_data));
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Can't detach FG %d from Context %d: "
                                 "FG payload is being used for cascading by %d qualifiers. "
                                 "presel_id %d in stage \"%s\" is cascading from it to context %d.\r\n",
                                 fg_id, context_id, nof_cascading_refs, cascading_presel_entry_id.presel_id,
                                 dnx_field_stage_text(unit, cascading_presel_entry_id.stage),
                                 presel_entry_data.context_id);
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "Can't detach FG %d from Context %d: FG payload is being used for "
                                 "cascading by %d qualifiers. Failed to find cascading field groups or presels.\r\n",
        ...
        case DNX_FIELD_GROUP_CACHE_MODE_INSTALL:
            ...
            for (entry_idx = 0; entry_idx < nof_valid_entries; entry_idx++)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_field_entry_tcam_install
                                (unit, DNX_FIELD_ENTRY_GROUP_ADD_FLAG_WITH_ID, fg_id,
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM:
        {
            switch (dnx_data_field.external_tcam.type_get(unit))
            {
                ...
                {
#if defined(INCLUDE_KBP)
                    SHR_IF_ERR_EXIT(dnx_field_kbp_group_cache(unit, flags, fg_id, mode));
#else
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
                    break;
    ...
    unsigned int *nof_quals_p)
{
    ...
    /** Sanity check.*/
    SHR_IF_ERR_EXIT(dnx_field_group_type_get(unit, fg_id, &fg_type));
    if (qual_is_ranged_found && (fg_type != DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM))
    {
    ...
    dnx_field_group_context_full_info_t * attach_full_info_p)
{
    ...
    else
    {
        dnx_field_key_qualifier_ffc_info_init(group_ffc_info_p->key.qualifier_ffc_info,
                                              sizeof(group_ffc_info_p->key.qualifier_ffc_info) /
                                              sizeof(group_ffc_info_p->key.qualifier_ffc_info[0]));
    ...
    if (fg_type == DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM)
    {
        ...
        if (device_locked)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_lookup_fwd_contexts
                            (unit, apptype, &nof_fwd_contexts, fwd_contexts));
            SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_acl_contexts(unit, apptype, &nof_acl_contexts, acl_contexts));
            if (nof_fwd_contexts <= 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Apptype %d (\"%s\") has no FWD contexts (%d).\n",
                             apptype, dnx_field_bcm_apptype_text(unit, apptype), nof_fwd_contexts);
            }
            if (nof_acl_contexts <= 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Apptype %d (\"%s\") has no ACL contexts (%d).\n",
                             apptype, dnx_field_bcm_apptype_text(unit, apptype), nof_acl_contexts);
            }
            switch (dnx_data_field.external_tcam.type_get(unit))
            {
                case DNX_FIELD_EXTERNAL_TCAM_TYPE_KBP:
                {
#if defined(INCLUDE_KBP)
                    SHR_IF_ERR_EXIT(dnx_field_kbp_key_ffc_info_get
                                    (unit, fg_id, opcode_id, fwd_contexts[0], acl_contexts[0], group_ffc_info_p));
#else
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
                    break;
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     ...
                     DBAL_TABLE_TYPE_TCAM_BY_ID, DBAL_ACCESS_METHOD_MDB, DBAL_CORE_MODE_DPC, 0, FALSE, use_hitbit,
    ...
    dnx_field_action_length_type_e * actions_length_type_p)
{
     ...
     */
    if (key_size_in_bits >
        dnx_data_field.mdb_dt.key_size_get(unit) - dnx_data_mdb.direct.map_payload_size_get(unit) / 60)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Key size=%d for MDB DT is not supported. Maximum supported key size is %d.\r\n",
                     key_size_in_bits, dnx_data_field.mdb_dt.key_size_get(unit));
    }
    /*
     * Verify payload
     */
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     fg_info_in_p->dnx_quals, fg_info_in_p->qual_is_ranged, actions_info_arr, fg_info_in_p->name,
                     DBAL_TABLE_TYPE_DIRECT, DBAL_ACCESS_METHOD_MDB, DBAL_CORE_MODE_SBC, 0, FALSE, FALSE,
                     &fg_info_p->dbal_table_id));
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     fg_info_in_p->dnx_quals, fg_info_in_p->qual_is_ranged, actions_info_arr, fg_info_in_p->name,
                     DBAL_TABLE_TYPE_EM, DBAL_ACCESS_METHOD_MDB, DBAL_CORE_MODE_SBC, 0, FALSE, FALSE,
                     &fg_info_p->dbal_table_id));
    ...
    int payload_size_bits)
{
    ...
    switch (dnx_data_field.external_tcam.type_get(unit))
    {
        ...
        {
#if defined(INCLUDE_KBP)
            SHR_IF_ERR_EXIT(dnx_field_kbp_payload_size_verify(unit, payload_size_bits));
#else
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
            break;
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    if ((fg_info_in_p->external_tcam_info.flags & BCM_FIELD_GROUP_EXTERNAL_FLAG_ENTRIES_PER_CORE) != 0)
    {
        core_mode = DBAL_CORE_MODE_DPC;
    }
    else
    {
        core_mode = DBAL_CORE_MODE_SBC;
    }
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     fg_info_in_p->dnx_quals, fg_info_in_p->qual_is_ranged, actions_info_arr, fg_info_in_p->name,
                     DBAL_TABLE_TYPE_TCAM_BY_ID, dbal_access_method, core_mode, SAL_UINT8_NOF_BITS, pack_qualifiers,
                     FALSE, &(fg_info_p->dbal_table_id)));
    ...
    switch (dnx_data_field.external_tcam.type_get(unit))
    {
        ...
        {
#if defined(INCLUDE_KBP)
            SHR_IF_ERR_EXIT(dnx_field_kbp_group_external_tcam_add
                            (unit, fg_id, fg_info_p, &(fg_info_in_p->external_tcam_info), actions_size_in_bits));
#else
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
            break;
...
/**
* \brief
*  verify function for dnx_field_group_delete.
* \param [in] unit   - Device Id
...
*/
shr_error_e
dnx_field_group_delete_verify(
    int unit,
    dnx_field_group_t fg_id)
{
    dnx_field_context_t context_id_arr[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS];
    uint32 nof_flush_profiles = dnx_data_field.exem_learn_flush_machine.nof_flush_profiles_get(unit);
    uint32 flush_profile_iter;
    SHR_FUNC_INIT_VARS(unit);
    ...
    /*
     * Verify no flush profile is attached.
     */
    for (flush_profile_iter = 0; flush_profile_iter < nof_flush_profiles; flush_profile_iter++)
    {
        bcm_field_group_t flush_profile_attached_fg_id;
        SHR_IF_ERR_EXIT(dnx_field_flush_sw.flush_profile.
                        mapped_fg.get(unit, flush_profile_iter, &flush_profile_attached_fg_id));
        if (flush_profile_attached_fg_id == fg_id)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Before deleting Field Group=%d need to detach (delete) flush profile %d\n",
                         fg_id, flush_profile_iter);
        }
    }
    ...
    dnx_field_group_t fg_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_group_delete_verify(unit, fg_id));
        ...
        && (fg_type != DNX_FIELD_GROUP_TYPE_CONST))
    {
        ...
        {
            /*
             * Destroy MDB table in case of EXEM
             */
            SHR_IF_ERR_EXIT(mdb_init_logical_table_destroy(unit, dbal_table_id));
        }
    ...
    dnx_field_group_info_t * fg_basic_info_p)
{
   ...
   /**
    ...
    if (fg_basic_info_p->fg_type == DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM)
    {
        ...
        dbal_core_mode_e core_mode;
        dbal_tables_e dbal_table_id;
        uint8 lpm_instead_of_tcam = FALSE;
        ...
        switch (dnx_data_field.external_tcam.type_get(unit))
        {
            ...
            {
#if defined(INCLUDE_KBP)
                SHR_IF_ERR_EXIT(dnx_field_kbp_db_properties_get(unit, fg_id, &counters_enable,
                                                                &(fg_basic_info_p->
                                                                  external_tcam_info.min_capacity_valid),
                                                                &(fg_basic_info_p->external_tcam_info.min_capacity),
                                                                &(fg_basic_info_p->
                                                                  external_tcam_info.algorithmic_mode_valid),
                                                                &(fg_basic_info_p->external_tcam_info.algorithmic_mode),
                                                                &lpm_instead_of_tcam));
#else
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
        ...
        /*
         * Find the external TCAM flags.
         */
        fg_basic_info_p->external_tcam_info.flags = 0;
        SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_id_get(unit, fg_id, &dbal_table_id));
        SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, dbal_table_id, &core_mode));
        if (core_mode == DBAL_CORE_MODE_DPC)
        {
            fg_basic_info_p->external_tcam_info.flags |= BCM_FIELD_GROUP_EXTERNAL_FLAG_ENTRIES_PER_CORE;
        }
        else if (core_mode != DBAL_CORE_MODE_SBC)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unknown core mode %d for fg_id %d. \n", core_mode, fg_id);
        }
        if (lpm_instead_of_tcam)
        {
            fg_basic_info_p->external_tcam_info.flags |= BCM_FIELD_GROUP_EXTERNAL_FLAG_LPM;
        }
    }
    ...
    dnx_field_ace_id_t * ace_id_p)
{
    ...
    ace_format_info.use_ppmc_range = ace_format_info_in_p->use_ppmc_range;
    ...
    dnx_field_ace_format_full_info_t * ace_id_info_p)
{
    ...
    ace_id_info_p->format_basic_info.use_ppmc_range = ace_id_info_p->format_full_info.use_ppmc_range;
    /*
     * Collect FES info from sw state and from HW.
     */
    ...
    int unit)
{
    ...
    switch (dnx_data_field.external_tcam.type_get(unit))
    {
        ...
        {
#if defined(INCLUDE_KBP)
            SHR_IF_ERR_EXIT(dnx_field_kbp_group_set(unit));
#else
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
            break;
...
dnx_field_group_tcam_external_swstate_init(
    int unit)
{
    int external_tcam_device_exists;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_field_map_is_external_tcam_available(unit, &external_tcam_device_exists));
    if (external_tcam_device_exists)
    {
        switch (dnx_data_field.external_tcam.type_get(unit))
        {
            case DNX_FIELD_EXTERNAL_TCAM_TYPE_KBP:
            {
#if defined(INCLUDE_KBP)
                SHR_IF_ERR_EXIT(dnx_field_kbp_tcam_external_swstate_init(unit));
#else
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
                break;
