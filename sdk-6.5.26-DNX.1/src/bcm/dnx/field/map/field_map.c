    ...
    bcm_field_AppType_t bcm_apptype)
{
    ...
    else
    {
        ...
        rv = dbal_fields_field_type_enum_value_get(unit, DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES, bcm_apptype,
                                                   &opcode_enum_val);
        if (rv != _SHR_E_NONE)
        {
            apptype_name = "Invalid Apptype";
    ...
    uint8 static_opcode_is_valid[DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF])
{
    ...
    /*
     * Test that all contexts are mapped to apptypes.
     */
    {
        uint32 dbal_fwd2_context_enum_iterator;
        int checked_one_context = FALSE;
        bcm_field_AppType_t i_apptype;
        uint8 nof_fwd2_context_get;
        int i_context_in_apptype;
        int found_mapped_apptype;
        uint32 fwd2_context_id_hw;

        SHR_IF_ERR_EXIT(dbal_fields_enum_next_enum_value_get
                        (unit, DBAL_FIELD_FWD2_CONTEXT_ID, INVALID_ENUM, &dbal_fwd2_context_enum_iterator));
        while (dbal_fwd2_context_enum_iterator != INVALID_ENUM)
        {
            SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                            (unit, DBAL_FIELD_FWD2_CONTEXT_ID, dbal_fwd2_context_enum_iterator, &fwd2_context_id_hw));
            found_mapped_apptype = FALSE;
            for (i_apptype = 0; (i_apptype < bcmFieldAppTypeCount) && (found_mapped_apptype == FALSE); i_apptype++)
            {
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.
                                nof_fwd2_context_ids.get(unit, i_apptype, &nof_fwd2_context_get));
                for (i_context_in_apptype = 0; i_context_in_apptype < nof_fwd2_context_get; i_context_in_apptype++)
                {
                    uint8 fwd2_context_id_sw;
                    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.
                                    fwd2_context_ids.get(unit, i_apptype, i_context_in_apptype, &fwd2_context_id_sw));
                    if (fwd2_context_id_sw == fwd2_context_id_hw)
                    {
                        found_mapped_apptype = TRUE;
                        break;
                    }
                }
            }
            
            if (found_mapped_apptype == FALSE && 0)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "For FWD2 context %d (\"%s\"), no apptype was found.\n", fwd2_context_id_hw,
                             dbal_enum_id_to_string(unit, DBAL_FIELD_FWD2_CONTEXT_ID, dbal_fwd2_context_enum_iterator));
            }
            checked_one_context = TRUE;
            SHR_IF_ERR_EXIT(dbal_fields_enum_next_enum_value_get
                            (unit, DBAL_FIELD_FWD2_CONTEXT_ID, dbal_fwd2_context_enum_iterator,
                             &dbal_fwd2_context_enum_iterator));
        }
        if (checked_one_context == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Could not find any FWD2 contexts.\n");
        }
    }
    ...
    int unit)
{
    ...
    }
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get
                        (unit, DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES, INVALID_ENUM, &apptype_enum_iter));
        while (apptype_enum_iter != INVALID_ENUM)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get
                            (unit, DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES, apptype_enum_iter, &apptype_enum_iter));
        }
    }
    ...
    uint32 *bcm_vlan_format_p)
{
    ...
    switch (dbal_vlan_format)
    {
        ...
        case DBAL_ENUM_FVAL_INCOMING_TAG_STRUCTURE_S_PRIORITY:
        {
            *bcm_vlan_format_p = bcmTagStructTypeSPrioTag;
            break;
        }
        default:
        {
