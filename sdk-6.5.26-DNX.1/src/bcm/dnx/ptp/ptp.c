...
/*22*/ {DNX_PTP_ETH , DNX_PTP_ANY , DNX_PTP_ANY , DNX_PTP_ANY , DNX_PTP_ANY , DNX_PTP_UDP, DNX_PTP_1588 , DNX_PTP_INIT,             FALSE,     5        },
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RECORD_TS, INST_SINGLE, FALSE);
 ...
 * \param [in] action - True for add and False for delete
 * \return
 *   See \ref shr_error_e
 * \remark
 *   * None
...
{
    dnx_ptp_port_profile_info_t profile_info;
    dnx_ptp_port_profile_info_t old_profile_info;
    int ref_count;
    ...
    sal_memset(&old_profile_info, 0x0, sizeof(dnx_ptp_port_profile_info_t));
    ...
    if (port_in_lag)
    {
        ...
        SHR_IF_ERR_EXIT(algo_ptp_db.
                        ptp_port_profile.profile_data_get(unit, old_profile, &ref_count, &old_profile_info));
        ...
        if (last_reference)
        {
            SHR_IF_ERR_EXIT(dnx_ptp_action_table_update(unit, old_profile, &old_profile_info, FALSE));
        }
        if (first_reference)
        {
            SHR_IF_ERR_EXIT(dnx_ptp_action_table_update(unit, new_profile, &profile_info, TRUE));
        }
        SHR_IF_ERR_EXIT(dnx_ptp_port_profile_set(unit, core,
                                                 allocation_info.pp_port_per_core_array[core], new_profile));
        {
            /** Configure TX timestamping per pp_port*/
    ...
    else
    {
        ...
        for (port_idx = 0; port_idx < gport_info.internal_port_pp_info.nof_pp_ports; port_idx++)
        {
                                                     ...
                                                     gport_info.internal_port_pp_info.pp_port[port_idx], new_profile));
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ptp_tx_timestamp_config_set
    ...
    bcm_port_timesync_config_t * config_array)
{
    ...
    if (dnx_drv_soc_feature(unit, soc_feature_timesync_support))
    {
        ...
        if (config_count)
        {
            ...
            {
                if (config_array->flags & BCM_PORT_TIMESYNC_F1588)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_PORT_TIMESYNC_F1588 flag is not supported");
                }
            }
...
/*
 * Function:
 *      bcm_dnx_ptp_ntp_tod_offset_set
 * Purpose:
    ...
    bcm_ptp_time_spec_t * ntp_tod_offset)
{
    ...
    SHR_IF_ERR_EXIT(bcm_common_ptp_ntp_tod_offset_set(unit, ptp_id, clock_num, flags, ptp_tod_offset, ntp_tod_offset));
...
/*
 * Function:
 *      bcm_dnx_ptp_ntp_tod_offset_get
 * Purpose:
    ...
    bcm_ptp_time_spec_t * ntp_tod_offset)
{
    ...
    SHR_IF_ERR_EXIT(bcm_common_ptp_ntp_tod_offset_get(unit, ptp_id, clock_num, flags, ptp_tod_offset, ntp_tod_offset));
