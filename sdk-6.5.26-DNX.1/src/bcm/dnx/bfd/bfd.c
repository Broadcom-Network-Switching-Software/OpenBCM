...
#define BFD_ACC_ONLY_FLAGS (BCM_BFD_ENDPOINT_UPDATE | BCM_BFD_ENDPOINT_WITH_ID | BCM_BFD_ENDPOINT_PWE_ACH | BCM_BFD_ENDPOINT_IN_HW | \
                            ...
                            BCM_BFD_ECHO | BCM_BFD_ENDPOINT_RX_RDI_AUTO_UPDATE | \
                            BCM_BFD_ENDPOINT_EXPLICIT_DETECTION_TIME | BCM_BFD_ENDPOINT_RDI_AUTO_UPDATE )
...
#define BFD_ACC_FLAGS2 (BCM_BFD_ENDPOINT_FLAGS2_USE_MY_DIP_DESTINATION | BCM_BFD_ENDPOINT_FLAGS2_APPLY_JITTER | BCM_BFD_ENDPOINT_FLAGS2_TX_STATISTICS | BCM_BFD_ENDPOINT_FLAGS2_RX_STATISTICS | \
...
/** "Server-Client" flag */
#define ENDPOINT_IS_BFD_SERVER_CLIENT(endpoint) UTILEX_GET_BITS_RANGE(endpoint, BFD_MEP_ID_SERVER_CLIENT_BIT_4, BFD_MEP_ID_SERVER_CLIENT_BIT_0)
...
/** SPECIAL_DIAG_2 local_diag or remote can be map to special number */
#define SPECIAL_DIAG_27 27

/** SPECIAL_DIAG_28 local_diag or remote can be map to special number */
#define SPECIAL_DIAG_28 28

/** EXCLUDE_9 local_diag or remote can be map to special number */
#define EXCLUDE_9 9

 /** EXCLUDE_10 local_diag or remote can be map to special number */
#define EXCLUDE_10 10
 ...
 * \param [in] bfd_sw_state_flags - bfd sw flags
    ...
    int *trap_id)
{
    ...
    switch (endpoint_info->type)
    {
        ...
        {
            SHR_IF_ERR_EXIT(bcm_dnx_rx_trap_type_get(unit, flags, bcmRxTrapOamBfdCcMplsTp, trap_id));
        }
            break;
    ...
    bfd_temp_endpoint_data_t * hw_write_data)
{
    ...
    */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_bfd_endpoint_trap_id_get(unit, endpoint_info, hw_write_data->bfd_sw_state_flags, &trap_id));
...
dnx_bfd_endpoint_mod_verify(
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
    int cluster_profile_exists = 0;
    uint32 cluster_period = 0;
    uint32 cluster_scan_rate = 0;
    uint32 new_scan_rate = 0;
    uint32 tx_cnt_enable = 0;
    uint32 rx_cnt_enable = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        {
            /** in case cluster exists, check if period of this endpoint is same as period of cluster */
            new_scan_rate =
                endpoint_info->bfd_period ? BCM_OAMP_SW_ENTRY_INFO_PERIOD_TO_MEP_DB_SCAN_RATE(unit,
                                                                                              endpoint_info->bfd_period)
                : 0;

            if (endpoint_info->bfd_period_cluster != existing_endpoint_info->bfd_period_cluster)
            {
                SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_existing_cluster_info_get
                                (unit, endpoint_info->id, endpoint_info->bfd_period_cluster, &cluster_profile_exists,
                                 &cluster_period, &cluster_scan_rate, &tx_cnt_enable, &rx_cnt_enable));

                if (cluster_profile_exists && (cluster_scan_rate != new_scan_rate))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Error, cluster %d already exists, and has a different tx_period.\n"
                                 ...
                                 endpoint_info->bfd_period_cluster, cluster_period, endpoint_info->bfd_period);
                }
            }
        }
    }
    ...
    bfd_temp_endpoint_data_t * hw_write_data)
{
    ...
    rv = algo_bfd_db.bfd_your_discr_range.allocate_single(unit, NO_FLAGS, &range, NULL, &dummy, &write_hw);
    if (rv == _SHR_E_RESOURCE)
    {
        /** Resource error means no match - display error message */
        SHR_IF_ERR_EXIT(algo_bfd_db.bfd_your_discr_range.profile_data_get(unit, DISCR_PROF, &dummy, &range));
    ...
    if (write_hw == TRUE)
    {
         ...
         *  dnx_bfd_endpoint_classifier_db_write_to_hw
         */
    ...
    bfd_temp_endpoint_data_t * hw_write_data)
{
        ...
        && (endpoint_info->remote_gport != BCM_GPORT_INVALID))
    {
        ...
        if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_MULTIHOP) ||
            _SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_INITIATOR) ||
            (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ECHO) && DNX_OAMP_IS_V2(unit)))
        {
    ...
    const bfd_temp_endpoint_data_t * hw_write_data)
{
    ...
    {
        endpoint_id |= DNX_BFD_CLIENT_MEP_SERVER_DEVICE_SET(hw_write_data->gathered_data.server_device_index);
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BFD_ENDPOINT_SW_STATE_FLAGS, INST_SINGLE,
                                 (endpoint_info->ip_subnet_length ==
                                  32) ? (hw_write_data->bfd_sw_state_flags |
                                         DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_IP_SUBNET_FULL_CHECK) :
                                 hw_write_data->bfd_sw_state_flags);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BCM_BFD_TUNNEL_TYPE, INST_SINGLE,
                                 endpoint_info->type);
    ...
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_BFD_SERVER_INDEX, INST_SINGLE,
                                (uint8) hw_write_data->gathered_data.server_device_index);
    ...
    bfd_temp_endpoint_data_t * sw_read_data)
{
    ...
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_BFD_SERVER_INDEX, INST_SINGLE,
                              &sw_read_data->gathered_data.server_device_index);
    ...
    bfd_temp_endpoint_data_t * hw_delete_data)
{
    ...
    if (_SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_IS_ACCELERATED))
    {
        if (BFD_TYPE_NOT_PWE(hw_delete_data->endpoint_type)
            && (hw_delete_data->endpoint_type != bcmBFDTunnelTypeMplsTpCcCv))
        {
...
dnx_bfd_check_dip_update(
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
    /** Is DIP even relevant? - only for multi-hop, BFD echo and seamless BFD  */
    if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_MULTIHOP) ||
        _SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_INITIATOR) ||
        (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ECHO) && DNX_OAMP_IS_V2(unit)))
    {
    ...
    int is_set)
{
    ...
    if ((endpoint_type == bcmBFDTunnelTypeMpls) && (0x7F == (src_ip_addr >> 24)))
    {
        /** for label_switch type, match DIP==127/8*/
    ...
    bfd_temp_endpoint_data_t * hw_delete_data)
{
    ...
    if (hw_delete_data->deleted_data.action_delete)
    {
        ...
        /** clear ipv4 BFD multicast trap action */
        if (hw_delete_data->endpoint_type == bcmBFDTunnelTypeUdp)
        {
            hw_delete_data->deleted_data.action_delete_key.da_is_mc = 1;
            SHR_IF_ERR_EXIT(dnx_oam_action_clear
                            (unit, hw_delete_data->action_flags, &hw_delete_data->deleted_data.action_delete_key));
        }
    }
    ...
    uint32 *flags)
{
    ...
    if ((endpoint_info->type == bcmBFDTunnelTypeMplsTpCcCv) && ((endpoint_info->dst_ip_addr != 0)))
    {
        return_flags |= DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_VCCV_TYPE_FIXED_DIP;
    }
    ...
    else if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_IN_HW)
             && (endpoint_info->tx_gport == BCM_GPORT_INVALID) && (endpoint_info->egress_if == 0))
    {
        return_flags |= DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_SERVER_CLIENT;
    }
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
        ...
        && endpoint_info != NULL)
    {
        ...
        /**  Configure virtual register for dest ip, src ip, required min Rx */
        if ((endpoint_info->src_ip_addr != 0) || (endpoint_info->local_min_echo != 0)
            || (endpoint_info->local_min_rx != 0) || (endpoint_info->local_state != 0))
        {
    ...
    /** BFD Echo doesn't use OAM Classifier in OAMPv1*/
        ...
        || DNX_OAMP_IS_V2(unit))
    {
        if (hw_write_data->gathered_data.action_write)
        {
            ...
            /** support ipv4 BFD multicast trap*/
            if ((endpoint_info != NULL) && (endpoint_info->type == bcmBFDTunnelTypeUdp))
            {
                hw_write_data->gathered_data.action_write_key.da_is_mc = 1;
                SHR_IF_ERR_EXIT(dnx_oam_action_set
                                (unit, hw_write_data->action_flags, &hw_write_data->gathered_data.action_write_key,
                                 &hw_write_data->gathered_data.action_content));
            }
    ...
    bfd_temp_endpoint_data_t * hw_update_data)
{
    ...
    if (!BFD_MEP_TYPE_CLASSIFIED_BY_DISCR(hw_update_data->endpoint_type)
        || (hw_update_data->endpoint_type == bcmBFDTunnelTypeMplsTpCcCv))
    {
        /**
    ...
    bfd_temp_endpoint_data_t * hw_update_data)
{
    ...
    dbal_enum_value_field_oamp_mep_type_e mep_type = 0;
    int is_bfd_php = 0;
    uint32 entry_handle_id;
    uint8 rmep_punt_profile_delete = 0;
    dnx_bfd_oamp_v1_temp_mep_db_data_t *mep_oamp_v1_hw_write_data = NULL;
    dnx_bfd_oamp_v1_endpoint_t *mep_oamp_v1_entry_values = NULL;
    dnx_bfd_oamp_v1_endpoint_t *new_mep_oamp_v1_entry_values = NULL;
    dnx_bfd_oamp_v1_temp_db_delete_data_t *oamp_v1_hw_delete_data = NULL;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    SHR_INVOKE_VERIFY_DNXC(bcm_dnx_bfd_endpoint_get(unit, endpoint_info->id, existing_endpoint_info));
    ...
    dnx_bfd_endpoint_flags_to_sw_state_flags(unit, endpoint_info, &hw_update_data->bfd_sw_state_flags);
 ...
 */
        ...
        (hw_update_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_SEAMLESS_BFD_REFLECTOR))
    {
        ...
        if (!_SHR_IS_FLAG_SET(hw_update_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_IS_IPV6))
        {
    ...
    if (_SHR_IS_FLAG_SET(hw_update_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_FROM_OAMP))
    {
        ...
        SHR_ALLOC_SET_ZERO(mep_oamp_v1_hw_write_data, sizeof(dnx_bfd_oamp_v1_temp_mep_db_data_t),
        ...
        SHR_ALLOC_SET_ZERO(new_mep_oamp_v1_entry_values, sizeof(dnx_bfd_oamp_v1_endpoint_t),
                           "Structure used to call OAMP MEP DB create", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
        SHR_ALLOC_SET_ZERO(oamp_v1_hw_delete_data, sizeof(dnx_bfd_oamp_v1_temp_db_delete_data_t),
                           "Structure used to delete OAMP DB data", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            ...
            if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_IPV6))
            {
                mep_oamp_v1_entry_values->flags = DNX_OAMP_BFD_IPV6;
            }
            ...
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_modify_data_in_templates(unit, endpoint_info, existing_endpoint_info,
                                                                     mep_oamp_v1_entry_values,
                                                                     oamp_v1_hw_delete_data,
                                                                     mep_oamp_v1_hw_write_data));
            /** Build structure for creating OAMP MEP DB entry */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_endpoint_db_build_param_for_mep_db
                            (unit, endpoint_info, mep_oamp_v1_hw_write_data, hw_update_data->bfd_sw_state_flags,
                             new_mep_oamp_v1_entry_values));

            mep_type = mep_oamp_v1_entry_values->mep_type;
        }
        /** Modify the relevant reference counts */
        SHR_IF_ERR_EXIT(dnx_bfd_oamp_modify_rmep_data_in_templates(unit, endpoint_info, existing_endpoint_info,
                                                                   rmep_entry_values, &rmep_punt_profile_delete,
                                                                   rmep_hw_write_data));
    ...
    if (existing_endpoint_info->remote_gport != endpoint_info->remote_gport)
    {
            ...
            existing_endpoint_info->remote_gport != BCM_GPORT_INVALID)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_bfd_write_trap_tcam_entry_to_template(unit, endpoint_info, mep_type, hw_update_data));
        ...
        else
        {
             ...
             */
            if (!_SHR_IS_FLAG_SET(existing_endpoint_info->flags, BCM_BFD_ECHO) || DNX_OAMP_IS_V2(unit))
            {
                /** Echo BFD doesn't have an MP profile in OAMPv1*/
    ...
    endpoint_info->type = is_bfd_php ? bcmBFDTunnelTypeMplsPhp : endpoint_info->type;
    SHR_IF_ERR_EXIT(dnx_bfd_save_endpoint_to_sw_state_table(unit, endpoint_info, hw_update_data));
    endpoint_info->type = is_bfd_php ? bcmBFDTunnelTypeMpls : endpoint_info->type;
...
/** Second section: writing to hardware tables   */
    ...
    if ((existing_endpoint_info->remote_gport != endpoint_info->remote_gport) &&
        (!_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ECHO) || DNX_OAMP_IS_V2(unit)))
    {
    ...
    SHR_IF_ERR_EXIT(dnx_bfd_endpoint_classifier_db_write_to_hw(unit, hw_update_data, NULL));
    ...
    if (_SHR_IS_FLAG_SET(hw_update_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_FROM_OAMP))
    {

        {
            ...
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_endpoint_db_write_to_hw(unit, mep_oamp_v1_hw_write_data));

            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_endpoint_db_delete_from_hw
                            (unit, mep_oamp_v1_entry_values, oamp_v1_hw_delete_data));

            /** Write to OAMP MEP DB table */
            new_mep_oamp_v1_entry_values->flags |= DNX_OAMP_BFD_MEP_UPDATE;
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_mep_db_endpoint_set(unit, endpoint_info, new_mep_oamp_v1_entry_values));
        }

        SHR_IF_ERR_EXIT(dnx_bfd_oamp_endpoint_rmep_db_write_to_hw(unit, rmep_hw_write_data));

        SHR_IF_ERR_EXIT(dnx_bfd_oamp_endpoint_rmep_profiles_delete
                        (unit, rmep_entry_values, rmep_punt_profile_delete, &rmep_hw_write_data->punt_good_profile));

        /** Check if an update is required */
        if (dnx_oam_oamp_rmep_compare(unit, FALSE, rmep_entry_values, new_rmep_entry_values))
        {
...
exit:
    ...
    SHR_FREE(new_mep_oamp_v1_entry_values);
    SHR_FREE(mep_oamp_v1_entry_values);
    SHR_FREE(mep_oamp_v1_hw_write_data);
    SHR_FREE(oamp_v1_hw_delete_data);
    ...
    bfd_temp_endpoint_data_t * hw_write_data)
{
    ...
    /** Save the resulting endpoint to the databases   */
    SHR_IF_ERR_EXIT(dnx_bfd_endpoint_classifier_db_write_to_hw(unit, hw_write_data, NULL));
    ...
    bfd_temp_endpoint_data_t * hw_write_data)
{
    ...
    dbal_enum_value_field_oamp_mep_type_e mep_type = 0;
    int is_bfd_php = 0;
    int bfd_server_device_index = 0;
    uint8 first_reference = 0;
    uint8 bfd_server_device = 0;
    dnx_bfd_oamp_v1_temp_mep_db_data_t *mep_oamp_v1_hw_write_data = NULL;
    dnx_bfd_oamp_v1_endpoint_t *mep_oamp_v1_entry_values = NULL;
    SHR_FUNC_INIT_VARS(unit);
 ...
 */
    ...
    dnx_bfd_endpoint_flags_to_sw_state_flags(unit, endpoint_info, &hw_write_data->bfd_sw_state_flags);
    ...
    if ((_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_MULTI_HOP_BFD) ||
         (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_INITIATOR)) ||
         ((_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_ECHO)
           && DNX_OAMP_IS_V2(unit))))
    ...
    /** Prepare classification for the MEP */
    SHR_IF_ERR_EXIT(dnx_bfd_mep_type_classification_set(unit, endpoint_info, hw_write_data));
    ...
    if (ENDPOINT_FROM_OAMP(endpoint_info))
    {
        /** Calculate the BFD endpoint ID */
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_build_endpoint_id(unit, endpoint_info, hw_write_data));

            /** After calculating the endpoint ID, some more verification can be applied */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_verify_params_after_mep_id_calc(unit, endpoint_info));
        }
    }

    /** BFD Echo doesn't use OAM Classifier in OAMPv1*/
    if ((!_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_ECHO)
         || DNX_OAMP_IS_V2(unit)) || dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 0)
    {
        /** Connect to a new/existing MEP profile */
        SHR_IF_ERR_EXIT(dnx_bfd_mp_profile_sw_update(unit, endpoint_info, hw_write_data));
    }

    /** Template for BFD server device index in multi-servers */
    if (_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_SERVER_CLIENT))
    {
        /** For client endpoints, server device is carried in remote_id. */
        bfd_server_device = (uint8) (endpoint_info->remote_id + 1);
        SHR_IF_ERR_EXIT(algo_bfd_db.bfd_server_device_index.allocate_single
                        (unit, 0, &bfd_server_device, NULL, &bfd_server_device_index, &first_reference));
        hw_write_data->gathered_data.server_device_index = (uint8) bfd_server_device_index;
    }
    ...
    if (_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_FROM_OAMP))
    {
        ...
        /** Set the Remote MEP ID in RMEP HW Data */
        rmep_hw_write_data->endpoint_id = endpoint_info->id;
        ...
        {
            int dummy;
            mep_oamp_v1_hw_write_data->endpoint_id = endpoint_info->id;
            if (_SHR_IS_FLAG_SET(hw_write_data->action_flags, DNX_OAM_MPLS_LM_DM_MEP))
            {
                /** For MPLS-LM-DM, avoid allocating the MEP DB entry */
                mep_oamp_v1_entry_values->flags |= DNX_OAMP_OAM_CCM_MEP_UPDATE;
                mep_oamp_v1_entry_values->mpls_lm_dm_entry = hw_write_data->gathered_data.mpls_lm_dm_entry;
                /** This endpoint already has a MEP profile - get it */
                mep_oamp_v1_hw_write_data->mep_profile = hw_write_data->gathered_data.mep_profile;
                SHR_IF_ERR_EXIT(algo_oam_db.oam_mep_profile.profile_data_get
                                (unit, hw_write_data->gathered_data.mep_profile, &dummy,
                                 &mep_oamp_v1_hw_write_data->mep_profile_data));
            }
            else
            {
                mep_oamp_v1_hw_write_data->mep_profile = 0xFF;
            }

            /** Templates for endpoint accelerated to local OAMP */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_write_data_to_templates(unit, endpoint_info, mep_oamp_v1_hw_write_data));

            /** Build structure for creating OAMP MEP DB entry */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_endpoint_db_build_param_for_mep_db
                            (unit, endpoint_info, mep_oamp_v1_hw_write_data, hw_write_data->bfd_sw_state_flags,
                             mep_oamp_v1_entry_values));
        ...
        /** Templates for endpoint accelerated to local OAMP for RMEP */
        SHR_IF_ERR_EXIT(dnx_bfd_oamp_write_rmep_data_to_templates(unit, endpoint_info, rmep_hw_write_data));

        /** Template for OAMP trap TCAM table entry allocation */
        if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_HW_ACCELERATION_SET) &&
            (endpoint_info->remote_gport != BCM_GPORT_INVALID))
        {
            SHR_IF_ERR_EXIT(dnx_bfd_write_trap_tcam_entry_to_template(unit, endpoint_info, mep_type, hw_write_data));
        }
...
/** Second section: writing to hardware tables   */
     ...
     */
    if ((!_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_ECHO)
         || DNX_OAMP_IS_V2(unit)) && !_SHR_IS_FLAG_SET(hw_write_data->action_flags, DNX_OAM_MPLS_LM_DM_MEP))
    {
    ...
    SHR_IF_ERR_EXIT(dnx_bfd_endpoint_classifier_db_write_to_hw(unit, hw_write_data, endpoint_info));

    /** Write to the HW tables that begin with "OAMP_"   */
    if (_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_FROM_OAMP))
    {
        {
            uint16 rmep_index_const = 0;
            /** Write data to the HW tables for the OAMP profiles */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_endpoint_db_write_to_hw(unit, mep_oamp_v1_hw_write_data));

            /** Write to OAMP MEP DB table */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_mep_db_endpoint_set(unit, endpoint_info, mep_oamp_v1_entry_values));
            ...
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_endpoint_db_get_const_for_oamp_rmep_index_db
                            (unit, mep_oamp_v1_entry_values->mep_type, &rmep_index_const));

            /** Write to RMEP index DB table */
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_rmep_index_db_add
                            (unit, endpoint_info->id, rmep_index_const, endpoint_info->remote_id));
        }

        /** Write to RMEP profiles */
        SHR_IF_ERR_EXIT(dnx_bfd_oamp_endpoint_rmep_db_write_to_hw(unit, rmep_hw_write_data));
    }

    if (_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_SERVER_CLIENT))
    {
        endpoint_info->id |= DNX_BFD_CLIENT_MEP_SERVER_DEVICE_SET(hw_write_data->gathered_data.server_device_index);
    }
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
    ...
    else
    {
        ...
        {
            /**oamp-v1 verifications */
            /** Are we adding to an existing MplsLmDm endpoint? */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_pwe_with_lm_dm_verify(unit, endpoint_info));

            /*
             * In following cases, it's need to check if full entry id is valid
             *   case 1 JR1 mode
             *   case 2 full entry threshold is greater than 0
             *   case 3 JR2 mode && full entry threshold is 0 && flag with_id is not set
             * In case JR2 mode && full entry threshold is 0 && flag with_id is set, endpoint id is decoupled from local_discr
             */
            if ((is_jr1_mode || (full_entry_threshold > 0) ||
                 (!_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_WITH_ID))) &&
                ((endpoint_info->local_discr & 0xffff) > dnx_data_oam.oamp.oamp_mep_full_entry_threshold_get(unit))
    ...
    if (!dnx_data_bfd.general.feature_get(unit, dnx_data_bfd_general_bfd_over_php_support)
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_MICRO_BFD)
        && _SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_MULTIHOP))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error: Micro BFD can't set multihop flag");
    }
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_INITIATOR))
    {
        ...
        if (endpoint_info->local_state == bcmBFDStateInit)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error: Seamless BFD Initiator endpoint cannot have INIT as local state.");
        }
    }
    if (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_REFLECTOR))
    {
    ...
    if (ENDPOINT_FROM_OAMP(endpoint_info))
    {
        ...
        if (endpoint_info->bfd_period_cluster > MAX_BFD_PERIOD_CLUSTER)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error: bfd_period_cluster %d not valid, valid value must be between 1 and %d",
                         endpoint_info->bfd_period_cluster, MAX_BFD_PERIOD_CLUSTER);
        }
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
                ...
                && (endpoint_info->bfd_period_cluster != 0))
            {
                ...
                if (cluster_profile_exists && (new_scan_rate != cluster_scan_rate))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Error, cluster %d already exists, and has a different tx_period.\n"
                                 "Cluster Tx period: 0x%08x, requested Tx period: 0x%08x\n",
                                 endpoint_info->bfd_period_cluster, cluster_period, endpoint_info->bfd_period);
                ...
                if (cluster_profile_exists
                    &&
                    ((tx_cnt_enable != _SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_TX_STATISTICS))
                     || (rx_cnt_enable !=
        ...
        if (endpoint_info->local_diag > MAX_DIAG
            || (endpoint_info->local_diag == EXCLUDE_9 || endpoint_info->local_diag == EXCLUDE_10))
        {
            if ((endpoint_info->local_diag != SPECIAL_DIAG_27 && endpoint_info->local_diag != SPECIAL_DIAG_28))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error: local_diag %d not valid, value must be between 0 and %d except speical cases 27,28",
        ...
        if (endpoint_info->remote_diag > MAX_DIAG
            || (endpoint_info->remote_diag == EXCLUDE_9 || endpoint_info->remote_diag == EXCLUDE_10))
        {
            if ((endpoint_info->remote_diag != SPECIAL_DIAG_27 && endpoint_info->remote_diag != SPECIAL_DIAG_28))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error: remote_diag %d not valid, value must be between 0 and %d except speical cases 27,28",
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error: for vccv bfd endpoints the ip-ttl must be set to 255.");
        }

        /** remote_gport require valid trap or default value BCM_GPORT_INVALID  */
        if ((!_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_HW_ACCELERATION_SET))
            && !BCM_GPORT_IS_TRAP(endpoint_info->remote_gport) && (endpoint_info->remote_gport != BCM_GPORT_INVALID))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error: value for remote gport, %d, is not valid value",
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_bfd_endpoint_create_verify(unit, endpoint_info));
    ...
    bfd_temp_endpoint_data_t * sw_read_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_bfd_endpoint_trap_id_get
                    (unit, endpoint_info, sw_read_data->bfd_sw_state_flags, &default_trap_id));
...
dnx_bfd_get_classifying_value(
    ...
    bcm_bfd_endpoint_t endpoint,
    bcm_bfd_endpoint_info_t * endpoint_info,
    const bfd_temp_endpoint_data_t * sw_read_data)
{
    uint32 lif_id;
    int dummy;
    dnx_bfd_discr_range_t range;
    ...
    if (ENDPOINT_CLASSIFIED_BY_DISCR(endpoint) || ENDPOINT_IS_ACC(endpoint))
    {
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
                                   ...
                                   DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_TX_GPORT_IS_INVALID)))
            {
                ...
                SHR_IF_ERR_EXIT(algo_bfd_db.bfd_oamp_v1_my_discr_range_start.profile_data_get(unit, 0, &dummy, &start));
                endpoint_info->local_discr |= start;
            }
            else
            {
                /** The upper bits should match the discriminator range   */
                SHR_IF_ERR_EXIT(algo_bfd_db.bfd_your_discr_range.profile_data_get(unit, 0, &dummy, &range));
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
    ...
    uint8 bfd_server_device = 0;
    int bfd_server_device_index = 0;
    dnx_bfd_oamp_v1_temp_mep_db_data_t *mep_oamp_v1_sw_read_data = NULL;
    dnx_bfd_oamp_v1_endpoint_t *mep_oamp_v1_entry_values = NULL;
    int is_jr1_mode = 0;
    uint32 full_entry_threshold = 0;
    int is_bfd_php = 0;
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_bfd_endpoint_get_verify(unit, endpoint, endpoint_info));
        ...
        (sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_USE_MY_BFD_DIP_DESTINATION))
    {
        ...
        if (_SHR_IS_FLAG_SET(sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_MULTI_HOP_BFD)
            || _SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_INITIATOR) ||
            (_SHR_IS_FLAG_SET(sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_ECHO)
             && DNX_OAMP_IS_V2(unit)))
        {
    ...
    if ((!_SHR_IS_FLAG_SET(sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_ECHO)
         || DNX_OAMP_IS_V2(unit)) || dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 0)
    {
        /** BFD Echo doesn't use classifier in OAMPv1*/
        SHR_IF_ERR_EXIT(dnx_bfd_get_mp_profile_from_template(unit, endpoint_info, sw_read_data));
    }
    ...
    if (_SHR_IS_FLAG_SET(sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_SERVER_CLIENT))
    {
        /*
         * Keep upper 14 bits, only update lower DISCR_RANGE_BIT_SHIFT(18) bits of local_discr from sw oam_lif 
         */
        endpoint_info->local_discr &= 0xFFFC0000;
        endpoint_info->local_discr |= sw_read_data->oam_lif;

        bfd_server_device_index = (int) sw_read_data->gathered_data.server_device_index;
        SHR_IF_ERR_EXIT(algo_bfd_db.bfd_server_device_index.profile_data_get
                        (unit, bfd_server_device_index, &dummy, &bfd_server_device));
        endpoint_info->remote_id = bfd_server_device - 1;
    }
    ...
    if (_SHR_IS_FLAG_SET(sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_FROM_OAMP))
    {
        ...
        SHR_ALLOC_SET_ZERO(mep_oamp_v1_sw_read_data, sizeof(dnx_bfd_oamp_v1_temp_mep_db_data_t),
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            ...
            /** Read from OAMP MEP DB table */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_mep_db_endpoint_get(unit, endpoint, mep_oamp_v1_entry_values));

            sal_memcpy(endpoint_info->dst_ip6_addr, mep_oamp_v1_entry_values->ipv6_dip, sizeof(bcm_ip6_t));
            sal_memcpy(endpoint_info->src_ip6_addr, mep_oamp_v1_entry_values->ipv6_sip, sizeof(bcm_ip6_t));

            /*
             * Update lower 16bits of local_discr
             *   in case of J2 mode and full_entry_threshold==0, oam_id is decoupled from local_discr
             *   lower 16bits of local_discr are stored in oamp_mep_db
             */
            if ((!is_jr1_mode) && (full_entry_threshold == 0))
            {
                endpoint_info->local_discr &= 0xFFFF0000;
                endpoint_info->local_discr |= mep_oamp_v1_entry_values->local_discr_lsb;
            }
            if (_SHR_IS_FLAG_SET
                (sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_VCCV_TYPE_FIXED_DIP))
            {
                mep_oamp_v1_entry_values->flags |= DNX_OAMP_BFD_VCCV_BFD_FIXED_DIP;
            }
            /** Write OAMP MEP DB data to target and temporary structures */
            dnx_bfd_oamp_v1_endpoint_db_analyze_param_from_oamp_mep_db(unit, endpoint_info,
                                                                       mep_oamp_v1_sw_read_data,
                                                                       sw_read_data->bfd_sw_state_flags,
                                                                       mep_oamp_v1_entry_values);
            /** Read data from templates for endpoint accelerated to local OAMP   */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_read_data_from_templates(unit, endpoint_info, mep_oamp_v1_sw_read_data));
        }
        ...
        /** Read data from templates for endpoint accelerated to local OAMP   */
        SHR_IF_ERR_EXIT(dnx_bfd_oamp_read_rmep_data_from_templates(unit, endpoint_info, rmep_sw_read_data));
...
exit:
    ...
    SHR_FREE(mep_oamp_v1_entry_values);
    SHR_FREE(mep_oamp_v1_sw_read_data);
    SHR_FREE(rmep_entry_values);
    ...
    bfd_temp_endpoint_data_t * hw_delete_data)
{
    ...
    SHR_IF_ERR_EXIT(algo_oam_db.oam_default_mep_profiles.free_single(unit, default_id, NULL));
    ...
    bcm_bfd_endpoint_t endpoint)
{
    ...
    dnx_algo_gpm_gport_hw_resources_t gport_hw_resources;
    uint8 is_mpls_lm_dm = FALSE, rmep_punt_profile_delete = 0;
    uint32 entry_handle_id;
    uint8 dummy;
    ...
    if (hw_delete_data->endpoint_type == bcmBFDTunnelTypeMplsPhp)
    {
        hw_delete_data->endpoint_type = bcmBFDTunnelTypeMpls;
    }
        ...
        && BFD_MEP_TYPE_CLASSIFIED_BY_DISCR(hw_delete_data->endpoint_type))
    {
         ...
         */
        SHR_IF_ERR_EXIT(algo_bfd_db.bfd_your_discr_range.free_single(unit, 0, &dummy));
    }
    ...
    if ((_SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_MULTI_HOP_BFD)
         || _SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags,
                             DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_SEAMLESS_BFD_INITIATOR) ||
         (_SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_ECHO)
          && DNX_OAMP_IS_V2(unit)))
        && !_SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags,
    ...
    /*
     *  Free BFD server device index from sw state
     */
    if (hw_delete_data->gathered_data.server_device_index)
    {
        SHR_IF_ERR_EXIT(algo_bfd_db.bfd_server_device_index.free_single
                        (unit, (int) hw_delete_data->gathered_data.server_device_index, &last_reference));
    }
    ...
    if (ENDPOINT_IS_ACC(endpoint))
    {
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_get_endpoint_is_mpls_lm_dm_type(unit, endpoint, &is_mpls_lm_dm));
            ...
            {
                /** Free MEP ID */
                SHR_IF_ERR_EXIT(dnx_algo_oamp_v1_mep_id_dealloc(unit, endpoint));
            }
        }
        else
        {
    ...
    if (_SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_FROM_OAMP))
    {
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            ...
            oamp_v1_hw_delete_data->seamless_bfd =
                (_SHR_IS_FLAG_SET
                 (hw_delete_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_SEAMLESS_BFD_INITIATOR));
            oamp_v1_hw_delete_data->mpls_lm_dm = is_mpls_lm_dm;

            /** Set IPv6 indication relevant for 1/4 extra data entries*/
            if (_SHR_IS_FLAG_SET
                (hw_delete_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_IS_IPV6))
            {
                mep_oamp_v1_entry_values->flags = DNX_OAMP_BFD_IPV6;
            }
            /** Read from OAMP MEP DB table */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_mep_db_endpoint_get(unit, endpoint, mep_oamp_v1_entry_values));
            /** Delete BFD profiles from templates */
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_delete_data_from_templates
                            (unit, endpoint, mep_oamp_v1_entry_values, oamp_v1_hw_delete_data,
                             _SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags,
                                              DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_SINGLE_HOP_RANDOM_DIP)));
        }
        ...
        SHR_IF_ERR_EXIT(dnx_bfd_oamp_delete_rmep_data_from_templates
                        (unit, rmep_entry_values, &rmep_punt_profile_delete, punt_good_hw_write_data));
    ...
    if (_SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_FROM_OAMP))
    {

        {
            ...
            if (is_mpls_lm_dm)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_set_mpls_lm_dm_mep_profile(unit, endpoint,
                                                                           oamp_v1_hw_delete_data->mpls_lm_dm_only_profile));
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_mep_db_endpoint_clear
                                (unit, endpoint,
                                 _SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags,
...

        {
            ...
            SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_endpoint_db_get_const_for_oamp_rmep_index_db
                            (unit, mep_oamp_v1_entry_values->mep_type, &rmep_index_const));
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_rmep_index_db_remove(unit, endpoint, rmep_index_const));
        }
