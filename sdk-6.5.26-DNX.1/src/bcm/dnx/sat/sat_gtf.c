    ...
    for (priority = 0; priority < (BCM_SAT_GTF_NUM_OF_PRIORITIES - 1); priority++)
    {
        ...
        for (ii = 0; ii < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; ii++)
        {
            if ((config->packet_edit[priority].packet_length[ii] != 0)
                && (config->packet_edit[priority].packet_length[ii] < DNX_SAT_GTF_PACKET_LENGTH_MIN))
            {
    ...
    bcm_sat_gtf_packet_config_t * config)
{
    ...
    if (is_last)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_sat_gtf_packet_header_config_clean
                        (unit, saved_pkt_header_base, header_tbl_data.pkt_header_len));

        SHR_IF_ERR_EXIT(dnx_sat_pkt_header_mem_free
                        (unit, (header_tbl_data.pkt_header_len + 4) / 4, saved_pkt_header_base));
