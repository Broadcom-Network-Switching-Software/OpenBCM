...
#define LEARN_PAYLOAD_AGE_PROFILE_WIDTH     1

#define LEARN_PAYLOAD_ENTRY_LARGE_GROUP_WIDTH     11
#define LEARN_PAYLOAD_TYPE_WIDTH_5B               5
...
/** Widths in the learn data buffer that is the source */
#define LEARN_PAYLOAD_SOURCE_LEARN_DATA_WIDTH      48
#define LEARN_PAYLOAD_SOURCE_SRC_SYSTEM_PORT_WIDTH 16
#define LEARN_PAYLOAD_SOURCE_LPKGV_WIDTH           (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_enhanced_learn_key_generation)?16:8)
#define LEARN_PAYLOAD_SOURCE_GLOBAL_LIF_ID_WIDTH   22
#define LEARN_PAYLOAD_SOURCE_LEARN_LIF_WIDTH       1

/** Offsets in the learn data buffer that is the source*/
#define LEARN_PAYLOAD_SOURCE_LEARN_LIF_OFFSET        0
#define LEARN_PAYLOAD_SOURCE_GLOBAL_LIF_ID_OFFSET    (LEARN_PAYLOAD_SOURCE_LEARN_LIF_OFFSET + LEARN_PAYLOAD_SOURCE_LEARN_LIF_WIDTH)
#define LEARN_PAYLOAD_SOURCE_LPKGV_OFFSET            (LEARN_PAYLOAD_SOURCE_GLOBAL_LIF_ID_OFFSET + LEARN_PAYLOAD_SOURCE_GLOBAL_LIF_ID_WIDTH)
#define LEARN_PAYLOAD_SOURCE_SRC_SYSTEM_PORT_OFFSET  (LEARN_PAYLOAD_SOURCE_LPKGV_OFFSET + LEARN_PAYLOAD_SOURCE_LPKGV_WIDTH)
#define LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET       (LEARN_PAYLOAD_SOURCE_SRC_SYSTEM_PORT_OFFSET + LEARN_PAYLOAD_SOURCE_SRC_SYSTEM_PORT_WIDTH)

#define DNX_LEARN_PAYLOAD_LPKGV_STRENGTH_BIT 0
#define DNX_LEARN_PAYLOAD_LPKGV_DEST_BIT     2
...
/*
 *  Increased stack size for MACT_fifo thread.
 */
#define MACT_FIFO_THREAD_STKSZ SAL_THREAD_STKSZ*2
/*
 * }
 */
/*
...
int dnx_l2_channel_number[BCM_MAX_NUM_UNITS];
/** Holds the l2 CPU learning thread ctrls per unit */
shr_thread_manager_handler_t dnx_l2_cpu_learn_thread_info[SOC_MAX_NUM_DEVICES];
/*
 * }
 */
...
dnx_l2_olp_jr_mode_init(
    int unit)
{
    uint8 vmv_value = 0, vmv_size, offset;
    uint32 app_db_id;
    ...
    int result_type_size, single_lif_grouping_size, eei_fec_grouping_size, strength_size, age_profile_size, dest_size;
    ...
    SHR_IF_ERR_EXIT(mdb_em_get_vmv_size_value
                    (unit, dnx_data_mdb.em.mact_dbal_id_get(unit), MDB_EM_ENTRY_ENCODING_HALF, &vmv_size, &vmv_value));
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                    (unit, DBAL_FIELD_IRPP_FWD_STAGE_FORMATS,
                     DBAL_ENUM_FVAL_IRPP_FWD_STAGE_FORMATS_FWD_MACT_RESULT_DEST__LIF__XL_FGID,
                     &result_type_single_outlif));

    /** Construct init value of learn payload for type eei and fec */
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                    (unit, DBAL_FIELD_IRPP_FWD_STAGE_FORMATS,
                     DBAL_ENUM_FVAL_IRPP_FWD_STAGE_FORMATS_FWD_MACT_RESULT_EEI__FEC, &result_type_eei_fec));
    ...
    single_lif_grouping_size = dnx_data_l2.dma.flush_group_size_get(unit);
    eei_fec_grouping_size = LEARN_PAYLOAD_ENTRY_GROUP_WIDTH;
    SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                    (unit, DBAL_TABLE_FWD_MACT, DBAL_FIELD_MAC_STRENGTH, 0, 0, INST_SINGLE, &strength_size));

    SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                    (unit, DBAL_TABLE_FWD_MACT, DBAL_FIELD_MACT_ENTRY_AGE_PROFILE, 0, 0, INST_SINGLE,
                     &age_profile_size));

    SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                    (unit, DBAL_TABLE_FWD_MACT, DBAL_FIELD_DESTINATION, 0, 0, INST_SINGLE, &dest_size));
    ...
    /*
     * set dynamic entry strength - result type(5), dest(21), strength(2), group(4), age_profile(1)
     */
    lif_and_destination_init_lsb |=
        DYNAMIC_MACT_ENTRY_STRENGTH << (SAL_UINT64_NOF_BITS - result_type_size - dest_size - strength_size -
                                        single_lif_grouping_size - age_profile_size);
     ...
     * set dynamic entry strength - result type(5), dest(21), strength(2), group(4), age_profile(1)
     */
    eei_and_fec_init_lsb |=
        (uint32) DYNAMIC_MACT_ENTRY_STRENGTH << (SAL_UINT64_NOF_BITS - result_type_size - dest_size - strength_size -
                                                 eei_fec_grouping_size - age_profile_size);
    ...
    /** set OLP JR mode for interwork/native DSP structure*/
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_OLP_GENERAL_CONFIGURATION, entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_JR_MODE, INST_SINGLE, is_jr_mode);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    void **user_data_unused)
{
    ...
    uint32 app_db_id_svl, app_db_id_svl_mask;
    uint32 table_id_ivl = DBAL_TABLE_FWD_MACT_IVL;
    uint32 table_id_svl;
    int app_db_size_ivl;
    int app_db_size_svl;
    ...
    table_id_svl = dnx_data_l2.general.fwd_mact_info_get(unit, l2ActionTypeLearn, FALSE)->logical_data_base;

    SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, table_id_svl, &app_db_id_svl, &app_db_size_svl));
    app_db_id_svl_mask = SAL_UPTO_BIT(app_db_size_svl);
    
    SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, table_id_ivl, &app_db_id_ivl, &app_db_size_ivl));
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_key_payload_update
                    (unit, entry_handle_id, DBAL_RESULT_TYPE_NOT_INITIALIZED, NULL, NULL, _SHR_CORE_ALL,
                     dma_register_size_in_bits, entry));
    ...
    if (received_appdb_id == app_db_id_ivl)
    {
        table_id = table_id_ivl;
    }
    else if ((received_appdb_id & app_db_id_svl_mask) == app_db_id_svl)
    {
        table_id = table_id_svl;
    }
    else
    {
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_key_payload_update
                    (unit, entry_handle_id, DBAL_RESULT_TYPE_NOT_INITIALIZED, dsp_key, NULL, _SHR_CORE_ALL,
                     dsp_payload_size_in_bits, dsp_payload));
...
dnx_l2_learn_mact_fifodma_cpu_learn_thread_info_init(
    int unit)
{
    sal_memset(&dnx_l2_cpu_learn_thread_info[unit], 0, sizeof(shr_thread_manager_handler_t));
}
    ...
    int unit)
{
    ...
    if (dnx_l2_cpu_learn_thread_info[unit] != NULL)
    {
        SHR_IF_ERR_EXIT(shr_thread_manager_destroy(&(dnx_l2_cpu_learn_thread_info[unit])));
    }
exit:
    /*
     * Disable the l2 cmic interrupt.
     */
    soc_cmic_intr_disable(unit, INTR_FIFO_DMA(dnx_l2_channel_number[unit]));
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    thread_priority = dnx_data_l2.dma.l2_dma_cpu_learn_thread_priority_get(unit);

    SHR_IF_ERR_EXIT_WITH_LOG(soc_dnx_fifodma_channel_get(unit, soc_dnx_fifodma_src_olp, &dnx_l2_channel_number[unit]),
                             "Error getting channel%s%s%s", EMPTY, EMPTY, EMPTY);

    shr_thread_manager_config_t_init(&thread_config_info);
    thread_config_info.name = "bcmL2CPULearn";
    ...
    thread_config_info.callback = dnx_l2_learn_mact_fifodma_cpu_learn_thread;
    thread_config_info.user_data = INT_TO_PTR(dnx_l2_channel_number[unit]);
    thread_config_info.thread_priority = thread_priority;
    thread_config_info.stop_timeout_usec = (10 * SECOND_USEC);
    SHR_IF_ERR_EXIT(shr_thread_manager_create(unit, &thread_config_info, &(dnx_l2_cpu_learn_thread_info[unit])));

    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "dnx_l2_learn_mact_fifodma_cpu_learn_init - Task %s\n"), "bcmL2CPULearn"));
 ...
 *
 * \param [in] unit - unit id
 * \param [out] olp_pp_port - pp port configured for the OLP
 * \param [out] olp_in_port_header_type - type of the port from the OLP
 * \param [out] olp_out_port_header_type - type of the port to the OLP
 *
 * \return
 *   int
 *
    ...
    dnx_vsi_learning_profile_info_t * vsi_learning_profile_info)
{
    ...
    if (is_last)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INTERRUPT_ENABLE, INST_SINGLE, 1);
    ...
    uint32 forwarding_distribution)
{
    ...
    uint32 nof_olp_blocks, olp_block;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    nof_olp_blocks = dnx_data_l2.general.lem_nof_dbs_get(unit) * dnx_data_device.general.nof_core_groups_get(unit);
    ...
    for (olp_block = dnx_data_l2.olp.olp_learning_block_id_get(unit); olp_block < nof_olp_blocks; olp_block += 2)
    {
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    int unit)
{
    uint32 entry_handle_id;
    dnx_vsi_learning_profile_info_t vsi_learning_profile_info;
    uint32 message_enable = !dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_olp_always_enters_source_cpu);
    ...
    bcm_l2_addr_distribute_t * distribution)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_addr_msg_distribute_verify(unit, distribution));
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_addr_msg_distribute_get_verify(unit, distribution));
    ...
    bcm_l2_addr_distribute_t * distribution)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_addr_msg_distribute_verify(unit, distribution));
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_addr_msg_distribute_set_verify(unit, distribution));
    ...
    int bcm_learn_mode)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_learn_mode_set_verify(unit, bcm_learn_mode));
    ...
    int unit)
{
    ...
    if ((learn_limit_bug == 1) ||
        ((dnx_data_field.init.j1_learning_get(unit) ||
          dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_refresh_events_wrong_key_msbs)) &&
    ...
    /** Key fields for command destination learning */
    ...
    /** Result values for command destination learning */
    ...
    /** Key fields for command destination shadow */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OLP_COMMAND_DESTINATION,
    ...
    if (dbal_tables_field_info_get_no_err(unit, DBAL_TABLE_OLP_GENERAL_CONFIGURATION, DBAL_FIELD_IS_JR_MODE,
                                          FALSE, 0, INST_SINGLE, &field_info) == _SHR_E_NONE)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_JR_MODE, INST_SINGLE, FALSE);
    }

    /** setting the entry with the default values */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    if (DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE ==
        dnx_data_headers.system_headers.system_headers_mode_get(unit))
    {
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_capacity_get
                    (unit, dnx_data_l2.general.fwd_mact_info_get(unit, l2ActionTypeFwdAndNA, FALSE)->logical_data_base,
                     &max_capacity));
    ...
    if (!dnx_data_mdb.feature.feature_get(unit, dnx_data_mdb_feature_em_mact_use_refresh_on_insert) &&
        !dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_ignore_limit_check))
    {
        /*
         * Set all values as 1 to allow MACT exceed limit
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_MACT_LIMIT_EXCEED_CONFIGURATION, entry_handle_id));
        dbal_entry_key_field8_range_set(unit, entry_handle_id, DBAL_FIELD_LIMIT_CHECK_PASS, DBAL_RANGE_ALL,
                                        DBAL_RANGE_ALL);
        dbal_entry_key_field8_range_set(unit, entry_handle_id, DBAL_FIELD_IS_HOST_REQUEST, DBAL_RANGE_ALL,
                                        DBAL_RANGE_ALL);
        dbal_entry_key_field8_range_set(unit, entry_handle_id, DBAL_FIELD_IS_STATIC, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ALLOW_IF_EXCEED_MACT_LIMIT, INST_SINGLE, 1);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ALLOW_IF_EXCEED_VSI_LIMIT, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        /*
         * Doesn't allow MACT exceed limit when mact_limit_check_pass[0], is_host_request[0,1], request_is_static[0,1]
         */
        dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_LIMIT_CHECK_PASS, 0);
        dbal_entry_key_field8_range_set(unit, entry_handle_id, DBAL_FIELD_IS_HOST_REQUEST, DBAL_RANGE_ALL,
                                        DBAL_RANGE_ALL);
        dbal_entry_key_field8_range_set(unit, entry_handle_id, DBAL_FIELD_IS_STATIC, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ALLOW_IF_EXCEED_MACT_LIMIT, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ALLOW_IF_EXCEED_VSI_LIMIT, INST_SINGLE, 0);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
...
{
    uint32 entry_handle_id;
    dbal_fields_e vlan_source_field;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    {
        vlan_source_field = DBAL_FIELD_VLAN_SOURCE;
    }
    ...
    /** VLAN source */
    dbal_entry_value_field32_set(unit, entry_handle_id, vlan_source_field, INST_SINGLE, DBAL_ENUM_FVAL_VLAN_SOURCE_VSI);
    ...
    /** VLAN source */
    dbal_entry_value_field32_set(unit, entry_handle_id, vlan_source_field, INST_SINGLE,
...
shr_error_e
dnx_l2_init_symmetric_lif_fec_spa_trap_dest_extended_2l_strength(
    int unit,
    int lpkgv_one_outlif)
{
    ...
    uint32 current_len;
    uint32 learn_payload_context_val =
    ...
    current_offset = LEARN_PAYLOAD_TOTAL_WIDTH;

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_PAYLOAD_CONTEXT, learn_payload_context_val);

    /** LPKGV[7:2]: type(5) + 1msb dest(1) */
    current_len = LEARN_PAYLOAD_TYPE_WIDTH_5B + 1;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LPKGV_OFFSET + 2));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 0, learn_ffc_instruction);

    /** Learn data[19:0]: destination[19:0] */
    current_len = 20;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 1, learn_ffc_instruction);

    /** LPKGV[1]: 1msb strength - 0 */
    current_len = 1;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LPKGV_OFFSET + 1));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 2, learn_ffc_instruction);

    /** Learn_Data[20]: second part of learn strength */
    current_len = 1;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 20));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 3, learn_ffc_instruction);

    /** Learn data[31:21]: entry group */
    current_len = LEARN_PAYLOAD_ENTRY_LARGE_GROUP_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(current_offset,
                                                    (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 21));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 4, learn_ffc_instruction);

    /** LPKGV[0]: Age_profile(1) + In_LIF[21:0]: OutLIF0 */
    current_len = LEARN_PAYLOAD_OUT_LIF_WIDTH + LEARN_PAYLOAD_AGE_PROFILE_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(current_offset,
                                                    (current_len - 1), LEARN_PAYLOAD_SOURCE_GLOBAL_LIF_ID_OFFSET);
...
shr_error_e
dnx_l2_init_symmetric_lif_tmf_mc_dest_extended_2l_strength(
    int unit,
    int lpkgv_one_outlif)
{
    uint32 entry_handle_id;
    ...
    /** TM flow or mc dest: set value as 1 */
    SHR_BITSET(&lpkgv_one_outlif, DNX_LEARN_PAYLOAD_LPKGV_DEST_BIT);
...
shr_error_e
dnx_l2_init_asymmetric_lif_any_dest_extended_2l_strength(
    int unit,
    int lpkgv_one_outlif)
{
    uint32 entry_handle_id;
    ...
    /** LPKGV[7:3]: type(5) */
    current_len = LEARN_PAYLOAD_TYPE_WIDTH_5B;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LPKGV_OFFSET + 3));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 0, learn_ffc_instruction);

    /** Learn data[20:0]: destination[20:0] */
    current_len = LEARN_PAYLOAD_DESTINATION_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 1, learn_ffc_instruction);

    /** Keep 0x0(1), Learn_Data[47] - learn strength */
    current_len = 1;
    current_offset -= (current_len + 1);
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 47));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 2, learn_ffc_instruction);

    /** Keep 0x0(7), Learn_Data[46:43] Learn_Data[46:43] - entry group */
    current_len = LEARN_PAYLOAD_ENTRY_GROUP_WIDTH;
    current_offset -= (current_len + LEARN_PAYLOAD_ENTRY_LARGE_GROUP_WIDTH - LEARN_PAYLOAD_ENTRY_GROUP_WIDTH);
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 43));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 3, learn_ffc_instruction);

    /** LPKGV[0]: Age_profile(1) */
    current_len = LEARN_PAYLOAD_AGE_PROFILE_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(current_offset,
                                                    (current_len - 1), LEARN_PAYLOAD_SOURCE_LPKGV_OFFSET);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 4, learn_ffc_instruction);

    /** Learn_Data[42:21]: OutLIF0 */
    current_len = LEARN_PAYLOAD_OUT_LIF_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(current_offset,
                                                    (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 21));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 5, learn_ffc_instruction);
...
shr_error_e
dnx_l2_init_symmetric_lif_symmetric_dest_extended_2l_strength(
    int unit,
    int lpkgv_one_outlif)
{
    uint32 entry_handle_id;
    ...
    /** 
     * Update lpkgv_one_outlif
     * from:
     *     LPKGV[7:3]: result type, LPKGV[2:1]:0x0, LPKGV[0]:age_profile(1)
     * to 
     *     LPKGV[7:6]:0x0, LPKGV[5:1]: result type, LPKGV[0]:age_profile(1)
     */
    lpkgv_one_outlif = (((lpkgv_one_outlif >> 3) << 1) | 1);
    ...
    /** Learn_Data[47:43]: DESTINATION_SSPA_PREFIX*/
    current_len = 5;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 43);
    ...
    /** SSPA(16]: destination[15:0] + LPKGV[7]==0x0(1) */
    current_len = LEARN_PAYLOAD_SOURCE_SRC_SYSTEM_PORT_WIDTH + 1;
    current_offset -= current_len;
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(current_offset,
                                                    (current_len - 1),
                                                    (LEARN_PAYLOAD_SOURCE_SRC_SYSTEM_PORT_OFFSET - 1));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 2, learn_ffc_instruction);

    /** Learn_Data[0]: second part of learn strength */
    current_len = 1;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET);
    ...
    /** Learn_Data[11:1]: entry group */
    current_len = LEARN_PAYLOAD_ENTRY_LARGE_GROUP_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(current_offset,
                                                    (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 1));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 4, learn_ffc_instruction);
...
shr_error_e
dnx_l2_init_no_lif_fec_spa_trap_dest_extended_2l_strength_both(
    int unit,
    int lpkgv_no_outlif)
{
    uint32 entry_handle_id;
...
shr_error_e
dnx_l2_init_no_lif_tmf_mc_dest_extended_2l_strength_both(
    int unit,
    int lpkgv_no_outlif)
{
    uint32 entry_handle_id;
    ...
    /** TM flow or mc dest: set value as 1 */
    SHR_BITSET(&lpkgv_no_outlif, DNX_LEARN_PAYLOAD_LPKGV_DEST_BIT);
...
shr_error_e
dnx_l2_init_no_lif_symmetric_dest_extended_2l_strength(
    int unit,
    int lpkgv_no_outlif)
{
    uint32 entry_handle_id;
     ...
     *     LPKGV[7:6]:0x0, LPKGV[5:1]: result type, LPKGV[0]:age_profile(1)
     */
    lpkgv_no_outlif = (((lpkgv_no_outlif >> 3) << 1) | 1);
...
shr_error_e
dnx_l2_init_asymmetric_eei_any_dest_extended_2l_strength(
    int unit,
    int lpkgv_one_outlif)
{
    uint32 entry_handle_id;
    ...
    uint32 current_len;
    uint32 learn_payload_context_val =
        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_ASYMMETRIC_EEI__ANY_DEST__16_GROUP__2L_STRENGTH__SVL;
    ...
    /** Learn_Data[47] - second part of learn strength */
    current_len = 1;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 47));
    ...
    current_len = LEARN_PAYLOAD_AGE_PROFILE_WIDTH;
    /** Skip entry_group */
    current_offset -= (current_len + LEARN_PAYLOAD_ENTRY_GROUP_WIDTH);
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(current_offset,
                                                    (current_len - 1), LEARN_PAYLOAD_SOURCE_LPKGV_OFFSET);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 4, learn_ffc_instruction);

    /** Learn_Data[42:21]: OutLIF0 */
    current_len = LEARN_PAYLOAD_EEI_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(current_offset,
                                                    (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 21));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 5, learn_ffc_instruction);
...
shr_error_e
dnx_l2_init_symmetric_lif0_any_dest_extended_2l_strength(
    int unit,
    int lpkgv_one_outlif)
{
    uint32 entry_handle_id;
    ...
    /** Learn_Data[31:21]: entry group */
    current_len = LEARN_PAYLOAD_ENTRY_LARGE_GROUP_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 21));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 3, learn_ffc_instruction);
...
shr_error_e
dnx_l2_init_symmetric_lif_any_dest_extended_2l_strength(
    int unit,
    int lpkgv_one_outlif)
{
    uint32 entry_handle_id;
    ...
    /** Learn_Data[21]: second part of learn strength */
    current_len = 1;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 21));
    ...
    /** Learn_Data[32:22] - entry group */
    current_len = LEARN_PAYLOAD_ENTRY_LARGE_GROUP_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction =
        BUILD_LEARN_PAYLOAD_FFC(current_offset, (current_len - 1), (LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET + 22));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 4, learn_ffc_instruction);
...
shr_error_e
dnx_l2_init_symmetric_no_lif_any_dest_extended_2l_strength(
    int unit,
    int lpkgv_one_outlif)
{
    uint32 entry_handle_id;
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                    (unit, DBAL_FIELD_IRPP_FWD_STAGE_FORMATS,
                     DBAL_ENUM_FVAL_IRPP_FWD_STAGE_FORMATS_FWD_MACT_RESULT_DEST__XL_FGID, &lpkgv_hw_value_no_outlif));
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                    (unit, DBAL_FIELD_IRPP_FWD_STAGE_FORMATS,
                     DBAL_ENUM_FVAL_IRPP_FWD_STAGE_FORMATS_FWD_MACT_RESULT_DEST__LIF__XL_FGID,
     ...
     * LPKGV[7:3]: Type, LPKGV[2]: 1MSB DEST bit for fec_spa_trap_dest or tmf_mc_dest, LPKGV[1]:0x0, LPKGV[0]:Age_Profile,
     */
    lpkgv_hw_value_no_outlif = ((lpkgv_hw_value_no_outlif << 3) | (1 << DNX_LEARN_PAYLOAD_LPKGV_STRENGTH_BIT));
    lpkgv_hw_value_single_outlif = ((lpkgv_hw_value_single_outlif << 3) | (1 << DNX_LEARN_PAYLOAD_LPKGV_STRENGTH_BIT));

    SHR_IF_ERR_EXIT(dnx_l2_init_symmetric_lif_fec_spa_trap_dest_extended_2l_strength
                    (unit, lpkgv_hw_value_single_outlif));
    SHR_IF_ERR_EXIT(dnx_l2_init_symmetric_lif_tmf_mc_dest_extended_2l_strength(unit, lpkgv_hw_value_single_outlif));
    SHR_IF_ERR_EXIT(dnx_l2_init_asymmetric_lif_any_dest_extended_2l_strength(unit, lpkgv_hw_value_single_outlif));
    SHR_IF_ERR_EXIT(dnx_l2_init_symmetric_lif_symmetric_dest_extended_2l_strength(unit, lpkgv_hw_value_single_outlif));
    SHR_IF_ERR_EXIT(dnx_l2_init_no_lif_fec_spa_trap_dest_extended_2l_strength_both(unit, lpkgv_hw_value_no_outlif));
    SHR_IF_ERR_EXIT(dnx_l2_init_no_lif_tmf_mc_dest_extended_2l_strength_both(unit, lpkgv_hw_value_no_outlif));
    SHR_IF_ERR_EXIT(dnx_l2_init_no_lif_symmetric_dest_extended_2l_strength(unit, lpkgv_hw_value_no_outlif));
    SHR_IF_ERR_EXIT(dnx_l2_init_symmetric_lif_any_dest_extended_2l_strength(unit, lpkgv_hw_value_single_outlif));
    SHR_IF_ERR_EXIT(dnx_l2_init_symmetric_no_lif_any_dest_extended_2l_strength(unit, lpkgv_hw_value_no_outlif));
    ...
    if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_IRPP_FWD_STAGE_FORMATS,
                         DBAL_ENUM_FVAL_IRPP_FWD_STAGE_FORMATS_FWD_MACT_RESULT_EEI__FEC, &lpkgv_hw_value_eei_fec));
        lpkgv_hw_value_eei_fec = ((lpkgv_hw_value_eei_fec << 3) | (1 << DNX_LEARN_PAYLOAD_LPKGV_STRENGTH_BIT));

        /*
         * MACT Payload Context for Asymmetric EEI+FEC of standard size
         */
        SHR_IF_ERR_EXIT(dnx_l2_init_asymmetric_eei_any_dest_extended_2l_strength(unit, lpkgv_hw_value_eei_fec));
        SHR_IF_ERR_EXIT(dnx_l2_init_symmetric_lif0_any_dest_extended_2l_strength(unit, lpkgv_hw_value_single_outlif));
    }
    /*
     * }
     * Jericho mode end
...
dnx_l2_select_dsp_result_type(
    int unit,
    uint32 olp_headers_bitmap,
    dbal_result_type_t * res_type_idx)
{
    ...
    bcm_l2_learn_msgs_config_t * learn_msg_config)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_nof_res_type_get
                    (unit, DBAL_TABLE_OLP_HEADER_CONFIGURATION, (int *) &res_type_idx));
    ...
    if (olp_headers_bitmap & OLP_HEADER_BITMAP_UDH)
    {
        ...
        SHR_ALLOC_SET_ZERO(p_udh_field, dsp_header_size, "UDH field", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_UDH_BASE, INST_SINGLE, p_udh_field);
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    bcm_l2_learn_msgs_config_t * learn_msg_config)
{
     ...
     */
    if ((no_black_hole_for_unknown_destination) && BCM_GPORT_IS_BLACK_HOLE(learn_msg_config->dest_port))
    {
    ...
    bcm_l2_learn_msgs_config_t * learn_msg_config)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_learn_msgs_config_set_verify(unit, learn_msg_config));
    ...
    void *userdata)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_addr_register_verify(unit, callback, userdata));
...
{
    int callback_idx;
    uint8 callback_found = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_l2_addr_unregister_verify(unit, callback, userdata));
    ...
    for (callback_idx = 0; callback_idx < DNX_L2_CB_MAX; callback_idx++)
    {
        if (dnx_l2_cbs[unit].entry[callback_idx].callback == callback
            && dnx_l2_cbs[unit].entry[callback_idx].userdata == userdata)
        {
            dnx_l2_cbs[unit].entry[callback_idx].callback = NULL;
            dnx_l2_cbs[unit].entry[callback_idx].userdata = NULL;
            dnx_l2_cbs[unit].count--;
            callback_found = TRUE;
            break;
        }
    ...
    if (!callback_found)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "The callback given is not registered");
    }
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief - Common verifier for learn resolution
 *          Verify that the learn action resolution strength inputs are valid
 *
 * \param [in] unit - unit id
 * \param [in] action_res - existing and learn strengths
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
dnx_l2_learn_action_resolution_common_verify(
    int unit,
    bcm_l2_learn_action_resolution_t * action_res)
{
    SHR_FUNC_INIT_VARS(unit);
    {
        /** Verify existing_strength value */
        if (action_res->existing_strength || action_res->learn_strength)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "invalid configuration, existing_strength=%d and learn_strength=%d should be nullified.\n",
                         action_res->existing_strength, action_res->learn_strength);
        }
    }
...
/**
 * \brief - Verifies input for action resolution set
 *
 * \param [in] unit - unit id
 * \param [in] action_res - existing and learn strengths, trap gport
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
dnx_l2_learn_action_resolution_set_verify(
    int unit,
    bcm_l2_learn_action_resolution_t * action_res)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_l2_learn_action_resolution_common_verify(unit, action_res));

    /** gport must be encoded as a trap/invalid */
    if ((!BCM_GPORT_IS_TRAP(action_res->gport)) && (BCM_GPORT_INVALID != action_res->gport))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "gport must be encoded as a trap or invalid\n");
    }
...
static shr_error_e
dnx_l2_learn_action_resolution_set(
    int unit,
    bcm_l2_learn_action_resolution_t * action_res)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                    (unit, DBAL_TABLE_TRANSPLANT_WITH_WEAKER_STRENGTH_TRAP_PROPERTIES, &entry_handle_id));

    if (action_res->gport == BCM_GPORT_INVALID)
    {
        snoop_str = 0;
        trap_str = 0;
        trap_id = 0;
    ...
    {
        snoop_str = BCM_GPORT_TRAP_GET_SNOOP_STRENGTH(action_res->gport);
        trap_str = BCM_GPORT_TRAP_GET_STRENGTH(action_res->gport);
        trap_id = BCM_GPORT_TRAP_GET_ID(action_res->gport);
    }

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARNING_ACTION_FWD_CODE, INST_SINGLE, trap_id);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_LEARNING_ACTION_FWD_STRENGTH, INST_SINGLE, trap_str);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_LEARNING_ACTION_SNOOP_STRENGTH, INST_SINGLE,
                                snoop_str);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
static shr_error_e
dnx_l2_learn_action_resolution_get(
    int unit,
    bcm_l2_learn_action_resolution_t * action_res)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                    (unit, DBAL_TABLE_TRANSPLANT_WITH_WEAKER_STRENGTH_TRAP_PROPERTIES, &entry_handle_id));
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_LEARNING_ACTION_FWD_CODE, INST_SINGLE, &trap_id);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_LEARNING_ACTION_FWD_STRENGTH, INST_SINGLE, &trap_str);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_LEARNING_ACTION_SNOOP_STRENGTH, INST_SINGLE,
                              &snoop_str);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    if (trap_id | trap_str | snoop_str)
    {
        BCM_GPORT_TRAP_SET(action_res->gport, trap_id, trap_str, snoop_str);
    }
    else
    {
        action_res->gport = BCM_GPORT_INVALID;
    }
...
/**
 * \brief - Set learn action resolution
 *
 * \param [in] unit - unit id
 * \param [in] action_res - existing and learn strengths, gport action
 *
 * \return
 *   shr_error_e
 *
 ...
 */
shr_error_e
bcm_dnx_l2_learn_action_resolution_set(
    int unit,
    bcm_l2_learn_action_resolution_t * action_res)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_learn_action_resolution_set_verify(unit, action_res));

    {
        SHR_IF_ERR_EXIT(dnx_l2_learn_action_resolution_set(unit, action_res));
    }
...
/**
 * \brief - Get learn action resolution
 *
 * \param [in] unit - unit id
 * \param [in, out] action_res - existing and learn strengths, gport action
 *
 * \return
 *   shr_error_e
 *
 ...
 */
int
bcm_dnx_l2_learn_action_resolution_get(
    int unit,
    bcm_l2_learn_action_resolution_t * action_res)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_learn_action_resolution_common_verify(unit, action_res));
    {
        SHR_IF_ERR_EXIT(dnx_l2_learn_action_resolution_get(unit, action_res));
    }
