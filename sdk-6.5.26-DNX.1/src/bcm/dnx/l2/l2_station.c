...
/** bit 31: OAM CFM l2 station range indication. */
#define DNX_L2_STATION_OAM_CFM_RANGE_BIT SAL_BIT(31)
...
/**
 * brief - The supported flags for VSI based multiple mymac
 */
#define DNX_L2_STATION_VSI_BASED_FLAGS (BCM_L2_STATION_IPV4 | BCM_L2_STATION_IPV6 | BCM_L2_STATION_ARP_RARP | BCM_L2_STATION_MPLS | BCM_L2_STATION_MIM | BCM_L2_STATION_FCOE)
/**
 * brief - Wether the station id is a vrrp station
 */
...
/**
 * \brief Build station ID for OAM range
 *
 * \param [out] _station_id - Result is written to it
 * \param [in] _port - global port ID (gport)
 * \param [in] _lsb - LSB of MY_CFM_MAC for this station
 ...
 *   * None
 */
#define DNX_L2_STATION_OAM_CFM_RANGE_SET(_station_id, _port, _lsb)              \
    ((_station_id) = (((_port) << DNX_L2_STATION_OAM_PORT_SHIFT) |         \
                    ((_lsb) << DNX_L2_STATION_OAM_MYMAC_LSB_SHIFT) |      \
                    (DNX_L2_STATION_OAM_CFM_BIT) | (DNX_L2_STATION_OAM_CFM_RANGE_BIT)))
 ...
 *   * None
 */
#define DNX_L2_STATION_IS_OAM_CFM_RANGE(_station_id) \
    ((_station_id) & DNX_L2_STATION_OAM_CFM_RANGE_BIT)
    ...
    bcm_l2_station_t * station)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        {
            if (station->flags & BCM_L2_STATION_OAM_RANGE)
            {
                SHR_IF_ERR_EXIT(dnx_oam_my_cfm_mac_lsb_range_add
                                (unit, flags, gport_info.internal_port_pp_info.core_id[pp_port_index],
                                 ...
                                 station->my_cfm_mac_lsb_range_min, station->my_cfm_mac_lsb_range_max));
            }
            else
            {
    ...
    if (station->flags & BCM_L2_STATION_OAM_RANGE)
    {
        DNX_L2_STATION_OAM_CFM_RANGE_SET(*station_id, station->src_port,
                                         station->dst_mac[UTILEX_PP_MAC_ADDRESS_NOF_U8 - 1]);
    }
    else
    {
    ...
    bcm_l2_station_t * station)
{
    ...
    if ((station->flags & BCM_L2_STATION_MPLS) && (station->src_port_mask != 0))
    {
    ...
    bcm_l2_station_t * station)
{
    ...
    else if (_SHR_IS_FLAG_SET(station->flags, BCM_L2_STATION_EXTENDED))
    {
        ...
        /*
         * Go over the physical databases and check if they are enabled or not
         */
        ...
        if (station->src_port_mask != 0)
        {
            ...
            /** Check the supported flags when working in SRC port mode*/
            if (arg == L2_STATION_SRC_PORT_MODE)
            {
                /** only BCM_L2_STATION_EXTENDED flag is supported when working in SRC port mode */
                SHR_MASK_VERIFY(station->flags, BCM_L2_STATION_EXTENDED, _SHR_E_PARAM, "non supported flags specified,"
                                "l3 protocols are irrelevant when working in SRC port mode, only BCM_L2_STATION_EXTENDED is supported.\n");
            }
            /** Check the supported flags when working in SRC port - protocol(ignore MAC) mode*/
            if (arg == L2_STATION_SRC_PORT_PROTOCOL_MODE)
            {
                uint32 supported_flags;
                supported_flags = BCM_L2_STATION_EXTENDED | BCM_L2_STATION_MPLS;
                /** only MPLS protocol is supported when working in SRC port-protocol mode */
                SHR_MASK_VERIFY(station->flags, supported_flags, _SHR_E_PARAM, "non supported flags specified,"
                                "Only MPLS protocol is supported when working in SRC port-protocol(ignore MAC) mode.\n");
            }
        }
        ...
        if (station->vlan != 0)
        {
            ...
            SHR_MASK_VERIFY(station->flags, BCM_L2_STATION_EXTENDED, _SHR_E_PARAM,
                            "non supported flags specified,"
                            "l3 protocols are irrelevant when working in VSI mode, only BCM_L2_STATION_EXTENDED is supported.\n");
    ...
    else
    {
        ...
        /** Verify supported flags */
        SHR_MASK_VERIFY(station->flags, DNX_L2_STATION_VSI_BASED_FLAGS, _SHR_E_PARAM,
                        "non supported flags specified.\n");
    }
    ...
    bcm_l2_station_t * station)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_station_my_mac_add_verify(unit, station_id, station));
    ...
    bcm_l2_station_t * station)
{
    ...
    if (!(station->flags & BCM_L2_STATION_OAM) && (station->flags & BCM_L2_STATION_OAM_RANGE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_L2_STATION_OAM_RANGE flag is not supported without BCM_L2_STATION_OAM");
    }

    if (!my_cfm_mac_by_range && (station->flags & BCM_L2_STATION_OAM_RANGE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_L2_STATION_OAM_RANGE flag is not supported");
    }
    ...
    bcm_l2_station_t * station)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_station_add_verify(unit, station_id, station));
    ...
    bcm_l2_station_t * station)
{
    ...
    if (DNX_L2_STATION_IS_OAM_CFM_RANGE(station_id))
    {
        station->flags |= BCM_L2_STATION_OAM_RANGE;
    }
...
{
    algo_dnx_l3_vrrp_tcam_key_t tcam_info;
    uint32 tcam_index, internal_flags[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] = { 0 };
    int ref_count;
    ...
    bcm_l2_station_t * station)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_station_my_mac_get_verify(unit, station_id, station));
    ...
    bcm_l2_station_t * station)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_station_get_verify(unit, station_id, station));
    ...
    int station_id)
{
    ...
    uint32 pp_port_index;
    int my_cfm_mac_by_range;
    uint8 my_cfm_mac_lsb_range_min = 0;
    uint8 my_cfm_mac_lsb_range_max = 0;
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_my_cfm_mac_get(unit, 0, gport_info.internal_port_pp_info.core_id[pp_port_index],
                                               ...
                                               &my_cfm_mac_lsb_range_min, &my_cfm_mac_lsb_range_max));
        ...
        if (my_cfm_mac_by_range)
        {
            if (DNX_L2_STATION_IS_OAM_CFM_RANGE(station_id))
            {
                /** Clear the resulting MAC address from ingress */
                SHR_IF_ERR_EXIT(dnx_oam_my_cfm_mac_range_clear
                                (unit, DNX_OAM_CLASSIFIER_INGRESS,
                                 gport_info.internal_port_pp_info.core_id[pp_port_index],
                                 gport_info.internal_port_pp_info.pp_port[pp_port_index]));

                /** Clear the resulting MAC address from egress */
                SHR_IF_ERR_EXIT(dnx_oam_my_cfm_mac_range_clear
                                (unit, DNX_OAM_CLASSIFIER_EGRESS,
                                 gport_info.internal_port_pp_info.core_id[pp_port_index],
                                 gport_info.internal_port_pp_info.pp_port[pp_port_index]));
            }
            else
    ...
    int station_id)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_station_my_mac_get_verify(unit, station_id, NULL));
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_station_multi_my_mac_exem_mode_set_verify(unit, arg));

    /** set mode to hw (tcam da profile) */
    {
        algo_dnx_l3_vrrp_tcam_key_t tcam_data;
        uint32 tcam_index = 0, tcam_index_msb = 0, da_profile;

        /** clear TCAM data since its data is irrelevant */
        sal_memset(&tcam_data, 0, sizeof(algo_dnx_l3_vrrp_tcam_key_t));

        /** get dedicated tcam index for EXEM: VSI/SRC port/SRC port-protocol ignore MAC
         *  All the three system modes used the same TCAM index */
        tcam_index = dnx_data_l3.vrrp.exem_default_tcam_index_get(unit);
        /** In case the table is per MSB, additional TCAM index is required*/
        ...
        else if (arg == L2_STATION_SRC_PORT_MODE)
        {
            da_profile = DBAL_ENUM_FVAL_MY_MAC_EXEM_DA_PROFILE_PER_PORT_MY_MAC;
            /** set TCAM */
            SHR_IF_ERR_EXIT(dnx_l3_vrrp_tcam_info_set(unit, tcam_index, &tcam_data, da_profile));

            /** In case the table is per MSB, additional TCAM index is required in VSI mode that need to be cleared*/
    ...
    /** set mode in sw for easy access */
    {
        ...
        else if (arg == L2_STATION_SRC_PORT_PROTOCOL_MODE)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_EXEM_MODE, INST_SINGLE,
    ...
    int *arg)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_EXEM_MODE, INST_SINGLE, exem_mode);
    /** commit */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
