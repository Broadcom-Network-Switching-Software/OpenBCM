 ...
 * \param [in] dest_group - destination group for the current topology that we are setting.
    ...
    int disable_link_delay_measurement)
{
    ...
    int nof_dest_get = dnx_data_fabric.mesh.nof_dest_get(unit);
    int status;
    int enable_link_delay;
    int enable_sync_cells;
    int dest_group_idx;
    ...
    /*
     * Link delay and sync cells transmit are enabled by default for all links.
     * In this step we disable link delay and sync cells transmit for links that are not part of any topology.
     * Delay measurement will be disabled by force, for a link of the current topology, only if disable_link_delay_measurement = 1.
     */
    for (i_link = 0; i_link < nof_links; ++i_link)
    {
        ...
        /*
         * Check if current link is part of a destination group 
         */
        for (dest_group_idx = 0; dest_group_idx < nof_dest_get; ++dest_group_idx)
        {
            /*
             * Mesh topology is populated in SW state and can be cheked from there.
             */
            SHR_IF_ERR_EXIT(dnx_fabric_db.mesh_topology.links.get(unit, dest_group_idx, i_link, &status));
            if (status == 1)
            {
                /*
                 * If link delay measurement has to be stoped. 
                 */
                if (disable_link_delay_measurement)
                {
                    /*
                     * And link is used by the current topology we are setting,
                     * then disable the link delay measuremet.
                     */
                    enable_link_delay = (dest_group_idx == dest_group) ? 0 : 1;
                }
                else
                {
                    /*
                     * Else we always enable link delay for links used by any topology. 
                     */
                    enable_link_delay = 1;
                }
                enable_sync_cells = 1;
    ...
    int enable)
{
    ...
    if (disable_link_delay_measurement)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_fabric_update_link_delay_measurement_and_sync_cells
                        (unit, links_count, links_array, local_dest_id, disable_link_delay_measurement));
    }
    else
    {
        /*
