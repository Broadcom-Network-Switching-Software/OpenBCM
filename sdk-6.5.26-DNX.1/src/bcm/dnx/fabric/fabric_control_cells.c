    ...
    int *success)
{
    ...
    if (iter_index % 2 != 0 && is_soft_reset)
    {
    ...
    int *any_up)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fabric_ctrl_cells_all_links_disable_polling(unit, iter_index, is_soft_reset, &all_down));
    ...
    dnx_fabric_ctrl_cells_polling_t * data)
{
    ...
    for (iter_index = 0; iter_index < data->nof_polling_iters; ++iter_index)
    {
        ...
        {
            SHR_IF_ERR_EXIT(data->polling_func(unit, iter_index, data->is_soft_reset, &success));
        }
    ...
    int is_soft_reset)
{
    ...
    data.is_soft_reset = is_soft_reset;
    SHR_IF_ERR_EXIT(dnx_fabric_ctrl_cells_enable_polling(unit, &data));
    ...
    int enable_fabric_traffic)
{
    ...
    if (enable_fabric_traffic)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_fabric_ctrl_cells_mesh_topology_sync(unit, is_soft_reset));
 ...
 *   there is a good chance that it will be needed to be done also to the second
 *   function. 
 *   One exception to that is that we DON'T disable Flow Status and Credits, so that local
 *   traffic is not harmed when isolating fabric.
 *
    ...
    int is_soft_reset)
{
    ...
    /*
     * Discard control cells
     * Leave ON Flow Status and Credit cells in order to not stop local traffic when isolating Fabric.
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FABRIC_CONTROL_CELLS_TX_DISCARD, entry_handle_id));
    ...
    int is_soft_reset)
{
    ...
    else
    {
         ...
         * cells as disabled.
         */
        if (is_control_cells_enabled || !dnxc_init_is_init_done_get(unit))
        {
    ...
    bcm_fabric_link_connectivity_t * link_connectivity)
{
    ...
    link_connectivity->local_link_id = link_id;
    /*
     * Get is link active
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FABRIC_CONTROL_CELLS_LINK_ACTIVE, &link_active_handle_id));
    ...
    bcm_fabric_link_connectivity_t * link_partner_info)
{
     ...
     * Note that param 'link' is actually refering to logical port or fabric gport.
     */
