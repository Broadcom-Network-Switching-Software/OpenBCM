    ...
    bcm_pbmp_t pbmp_to_probe)
{
    ...
    if (_SHR_PBMP_IS_NULL(sfi_pbmp))
    {
        ...
        /**
         * Devices the support full power down won't enable dynamically internal fabric blocks (like FDT and FDR)
         * This is considered to be misconfiguration as these blocks should be always enabled or always disabled 
         * based on soc property static configuration, and to not be affected by dynamic sequences
         */
    ...
    bcm_pbmp_t * okay_pbmp)
{
    ...
    bcm_pbmp_t fmacs_bmp, fmacs_to_be_enabled_bmp;
    int fmac, fmac_enabled;
    ...
    /*
     * Stat counters, only set the reg for FMACs that are about to be enabled but not yet enabled.
     */
    ...
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_full_power_down))
    {
        if (!is_init_sequence)
        {
            BCM_PBMP_CLEAR(fmacs_to_be_enabled_bmp);
            SHR_IF_ERR_EXIT(dnx_algo_port_fabric_pbmp_to_fmac_bmp_convert(unit, pbmp, &fmacs_bmp));
            BCM_PBMP_ITER(fmacs_bmp, fmac)
            {
                SHR_IF_ERR_EXIT(dnxc_fabric_if_fmac_block_enable_get(unit, fmac, &fmac_enabled));
                if (!fmac_enabled)
                {
                    BCM_PBMP_PORT_ADD(fmacs_to_be_enabled_bmp, fmac);
                }
            }
        }
    }
     ...
     * All relevant blocks were powered up during init inside dnx_fabric_if_fmac_fsrd_blocks_power_init
     */
    if (!is_init_sequence)
    {
    ...
    if (!is_init_sequence)
    {
        ...
        if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_full_power_down))
        {
            BCM_PBMP_ITER(fmacs_to_be_enabled_bmp, fmac)
            {
                SHR_IF_ERR_EXIT(mib_stat_fabric_fmac_hw_init(unit, fmac));
            }
        }
        /*
...
/**
 * \brief
 *   Controls per block valid database and restricts access to this block.
 */
static shr_error_e
dnx_fabric_if_fmac_fsrd_block_valid_set(
    int unit,
    int block_type,
    int block_idx,
    int is_valid)
{
    ...
    /**
     * This is the relevant power mode / validity indicator if using new access
     * soc_info pointer modification should be irrelevant for the device state but is used to monitor 
     * in a common way the expected FMAC/FSRD state
     */
    {
        int access_block_type;

        if (block_type == FMAC_BLOCK_INT)
        {
            access_block_type = BLOCK_FMAC;
        }
        else if (block_type == FSRD_BLOCK_INT)
        {
            access_block_type = BLOCK_FSRD;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Invalid block type provided  %d", block_type);
        }

        SHR_IF_ERR_EXIT(access_valid_block_enable(unit, access_block_type, block_idx, is_valid ? TRUE : FALSE));
    }
#endif
    
    /** Mark FAMC blocks as valid/non-valid */
    SHR_IF_ERR_EXIT(soc_info_int_address_get(unit, block_type, &block_p));
    block_p = &(block_p[block_idx]);
    blk = *block_p;
    ...
    int enable)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnxc_fabric_if_fsrd_block_enable_get(unit, master_fsrd_block, &is_master_fsrd_enabled));

    if (is_master_fsrd_enabled)
    {
        int nof_slaves_enabled = 0, slave_enabled, fsrd_idx;
        for (fsrd_idx = 0; fsrd_idx < dnx_data_fabric.blocks.nof_instances_fsrd_get(unit); fsrd_idx++)
        {
            if (master_fsrd_block == dnx_data_fabric.blocks.fsrd_ref_clk_master_get(unit, fsrd_idx)->master_fsrd_index)
            {
                SHR_IF_ERR_EXIT(dnxc_fabric_if_fsrd_block_enable_get(unit, master_fsrd_block, &slave_enabled));
                nof_slaves_enabled += slave_enabled;
            }
        }

        if (nof_slaves_enabled == 0)
        {
            SHR_IF_ERR_EXIT(dnx_fabric_if_fsrd_clock_power_set(unit, master_fsrd_block, 0));
        }
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_valid_set(unit, FSRD_BLOCK_INT, master_fsrd_block, 1));
    ...
    int power_up)
{
    ...
    BCM_PBMP_ITER(fsrds_bmp, fsrd_block)
    {
        if (power_up)
        {
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_valid_set(unit, FSRD_BLOCK_INT, fsrd_block, power_up));
    ...
    if (power_up)
    {
        BCM_PBMP_ITER(fmacs_bmp, fmac_block)
        {
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_valid_set(unit, FMAC_BLOCK_INT, fmac_block, power_up));
    ...
    bcm_pbmp_t pbmp)
{
     ...
     * Later only the required ones will be powered up and marked as such
     */
    ...
    int unit)
{
    ...
    for (fsrd = 0; fsrd < nof_fsrds; fsrd++)
    {
        ...
        SHR_IF_ERR_EXIT(dnxc_fabric_if_fsrd_block_enable_get(unit, fsrd, &enable));
        if (enable)
        {
