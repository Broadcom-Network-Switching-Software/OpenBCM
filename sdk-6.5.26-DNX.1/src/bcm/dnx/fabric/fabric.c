    ...
    int unit)
{
    ...
    /*
     * Init power of all supported FMACs FSRDs. Power down unsupported FMACs and FSRDs.
     * Actual power down for FSRDs happen in the PM code. Here only the blocks are marked as disabled
     * Power up happens on base of port bitmap so the FSRDs were never powered up if not needed
     */
    SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_blocks_power_init(unit, pbmp));
...
/**
 * \brief
 * Power down fabric blocks that are not use.
...
{
    dnx_fabric_power_mode_e power_mode;
    bcm_pbmp_t fmacs_to_power_down;
    SHR_FUNC_INIT_VARS(unit);

    /**
     * For devices supporting full power down the unneeded fabric blocks were never powered up from the soft reset
     * so no need to be powered down here 
     */
    if (!dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_full_power_down))
    {
        if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_clock_power_down))
        {
            ...
            if (power_mode != DNX_FABRIC_POWER_MODE_ON)
            {
                ...
                /** 
                 * FSRDs and FMACs are powered up or down inside dnx_fabric_ports_init.
                 * dnx_fabric_ports_init is not executed when all fabric blocks are powered down.
                 * This is why we power down FMACs and FSRDs explicitly here.
                 */
                if (power_mode == DNX_FABRIC_POWER_MODE_OFF)
                {
                    /** Power down FMACs */
                    SHR_IF_ERR_EXIT(dnx_fabric_if_supported_fmacs_get(unit, &fmacs_to_power_down));
                    SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_power_set(unit, fmacs_to_power_down, FALSE));
                }
            }
        }
    }
    ...
    int unit)
{
    ...
    /*
     * Disable unused fabric blocks.
     * If fabric full power down is active this means that we powered up only the blocks that were not marked as disabled by 
     * dnx_drv_soc_block_enable_set while initing the access
     */
    ...
    dnx_fabric_power_mode_e power_mode)
{
    ...
    switch (power_mode)
    {
        ...
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_POWER_DOWN_STANDALONE, INST_SINGLE, FALSE);
            break;
        }
            /*
        ...
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_POWER_DOWN_STANDALONE, INST_SINGLE, TRUE);
            break;
        }
            /*
        ...
        case DNX_FABRIC_POWER_MODE_OFF:
        {
             ...
             */
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_POWER_DOWN_FULL, INST_SINGLE, TRUE);
            break;
        }
        default:
