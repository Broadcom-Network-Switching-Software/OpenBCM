    ...
    bcm_cos_queue_t offset)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_gport_egress_map_set(unit, gport, ingress_pri, ingress_dp, offset));
    }
    else
    {
    ...
    bcm_cosq_egress_multicast_config_t * config_p)
{
    ...
    if (config_p->priority > num_priorities)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid priority parameter %d. Legal range is %d,%d", config_p->priority, 0,
                     (num_priorities - 1));
    }
 ...
 * \param [in] ingress_pri - Priority (TC) of traffic entering egress. Must be between 0 and DNX_DEVICE_COSQ_ING_NOF_TC-1 (7).
 * \param [in] ingress_dp - Drop precedence of traffic entering egress. Must be between 0 and DNX_DEVICE_COSQ_ING_NOF_DP-1 (3).
 * \param [in] offset_p - bcm_cos_queue_t. This procedure loads pointed memory by priority assigned to outgoing traffic. Must be between 0 and DNX_EGR_NOF_Q_PRIO-1 (7).
 * \return
 *   If zero (_SHR_E_NONE), then no error was encountered.
 *   Otherwise, see shr_error_e
 * \remark
    ...
    bcm_cos_queue_t * offset_p)
{
    ...
    if (BCM_COSQ_GPORT_IS_UCAST_EGRESS_QUEUE(gport) || BCM_COSQ_GPORT_IS_MCAST_EGRESS_QUEUE(gport) ||
        BCM_GPORT_IS_TRUNK(gport))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_gport_egress_map_get(unit, gport, ingress_pri, ingress_dp, offset_p));
    }
    else
    {
