    ...
    uint8 *is_cdu)
{
    ...
    int ethu_port_num, first_ethu_in_core, ethu_id_in_core;
    shr_thread_manager_handler_t pfc_deadlock_recovery_handler;
    ...
    if (dnx_data_nif.eth.cdum_nof_get(unit) != 0)
    {
        ...
        if (*is_cdu == FALSE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethus_first_ethu_per_core_get(unit, core, &first_ethu_in_core));
            ethu_id = (block_instance % cdum_blks_in_core) + first_ethu_in_core;
        }
        else
        {
            /** CDU */
    ...
    else
    {
        ...
        /** Determine the core */
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_global_to_local_and_core_get(unit, ethu_id, &core, &ethu_id_in_core));
    }
    ...
    SHR_IF_ERR_EXIT(shr_thread_manager_interval_set(pfc_deadlock_recovery_handler, 1, recovery_timer));
    ...
    int unit)
{
...

    {
        ...
        /** Reset the instrumentation counters */
            SHR_IF_ERR_EXIT(imb_instru_counters_reset_set_all(unit, TRUE));
            SHR_IF_ERR_EXIT(imb_instru_counters_reset_set_all(unit, FALSE));
