...
#include "ingress/ingress_congestion.h"
    ...
    switch (index & _SHR_COSQ_TH_INDEX_INDICATION_MASK)
    {
        ...
        case _SHR_COSQ_TH_INDEX_POOL_DP_INDICATION:
            *index_type = DNX_COSQ_THRESHOLD_INDEX_POOL_DP;
            ...
            break;
        case _SHR_COSQ_TH_INDEX_CAST_PRIO_FWDACT_DP_INDICATION:
            *index_type = DNX_COSQ_THRESHOLD_INDEX_CAST_PRIO_FDWACT_DP;
            ...
            index_info->dp = (index >> 3 * _SHR_COSQ_TH_INDEX_FIELD_NOF_BITS) & _SHR_COSQ_TH_INDEX_FIELD_MASK;
            break;
        ...
        case _SHR_COSQ_TH_INDEX_POOL_PRIO_INDICATION:
            *index_type = DNX_COSQ_THRESHOLD_INDEX_POOL_PRIO;
            ...
            index_info->pool_id = (index >> _SHR_COSQ_TH_INDEX_FIELD_NOF_BITS) & _SHR_COSQ_TH_INDEX_FIELD_MASK;
            break;
        case _SHR_COSQ_TH_INDEX_CAST_INDICATION:
            *index_type = DNX_COSQ_THRESHOLD_INDEX_CAST;
            index_info->cast = (index & _SHR_COSQ_TH_INDEX_FIELD_MASK);
            break;
        default:
...
/**
 * helper function for decoding ICGM resource stat index into pool id
 * Opposite direction is done by BCM Macros.
 */
static int
dnx_cosq_icgm_resource_stat_index_convert(
    int unit,
    uint32 index,
    int *pool_id)
{
    ...
    if (index & _SHR_COSQ_ICGM_STAT_INDEX_INDICATION_MASK)
    {
        *pool_id = (index & _SHR_COSQ_ICGM_STAT_INDEX_FIELD_MASK);
    }
    else
    {
        *pool_id = -1;
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_static_threshold_verify(unit, flags, thresh_info, threshold));
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    if (thresh_info->resource_type == bcmResourceDataBuffers)
    {
        allowed_flags |= BCM_COSQ_STATIC_THRESHOLD_EGRESS;
    }

    SHR_MASK_VERIFY(flags, allowed_flags, _SHR_E_PARAM, "unsupported flag specified\n");
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_static_threshold_internal_verify(unit, flags, thresh_info, threshold));
    ...
    {
        if (thresh_info->resource_type == bcmResourceDataBuffers && flags == BCM_COSQ_STATIC_THRESHOLD_EGRESS)
        {
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "resource type not supported in this device");
    ...
    bcm_cosq_pfc_config_t * config)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_pfc_config_verify(unit, gport, cosq, flags, config));
    ...
    bcm_cosq_range_t * range)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqStatIfScrubberQueues:
        {
            /** verify */
            SHR_VAL_VERIFY(flags, 0, _SHR_E_PARAM, "expecting flags == 0");
            SHR_INVOKE_VERIFY_DNXC(dnx_stif_mgmt_control_range_verify(unit, STIF_MGMT_VERIFY_SET, port, type, range));
    ...
    bcm_cosq_range_t * range)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqStatIfScrubberQueues:
        {
            /** verify */
            SHR_VAL_VERIFY(flags, 0, _SHR_E_PARAM, "expecting flags == 0");
            SHR_INVOKE_VERIFY_DNXC(dnx_stif_mgmt_control_range_verify(unit, STIF_MGMT_VERIFY_GET, port, type, range));
            core = BCM_COSQ_GPORT_CORE_GET(port);
    ...
    dnx_fabric_wfq_type_e fabric_wfq_type)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_fabric_gport_sched_verify(unit, pipe, cosq));
    SHR_VAL_VERIFY(mode, 0, _SHR_E_PARAM, "expecting mode == 0");
    SHR_IF_ERR_EXIT(dbal_tables_field_predefine_value_get
                    (unit, DBAL_TABLE_FABRIC_WFQ_DYNAMIC_WEIGHTS, DBAL_FIELD_WEIGHT, FALSE, 0, 0,
    ...
    int weight)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_queue_wfq_set(unit, gport, cosq, mode, weight));
    }
    /*
     * Use case: "RCY EIR shaper weights"
     */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_rcy_eir_weight_set(unit, gport, weight));
    }
    /*
     * Use case: "Mirror RCY EIR shaper weights"
     */
    ...
    else if (BCM_COSQ_GPORT_IS_PORT_TC(gport))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "TCG to TC mapping is not supported on this device\n");
        }
    ...
    else if (BCM_COSQ_GPORT_IS_PORT_TCG(gport))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "TCG gport is not supported on this device\n");
        }
        SHR_IF_ERR_EXIT(dnx_cosq_egq_gport_egress_port_tcg_sched_set(unit, gport, cosq, mode, weight));
    }
    /** Use case: "Port TC Scheduler EIR weights" */
    else if (BCM_COSQ_GPORT_IS_PORT_TC_SCHED(gport))
    {
        if (!dnx_data_egr_queuing_v1.general.feature_get(unit, dnx_data_egr_queuing_v1_general_v1_supported))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "PORT TC Scheduler gport is not supported on this device\n");
        }
    }
    /*
     * Use case: "fabric pipes WFQ"
    ...
    {
        if (dnx_data_fabric.cgm.feature_get(unit, dnx_data_fabric_cgm_is_ingress_dynamic_wfq_supported))
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                         "Gport type bcmCosqGportTypeFabricPipeIngress is not supported on this device.\n");
        }
    ...
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE(gport))
    {
        if (dnx_data_fabric.cgm.feature_get(unit, dnx_data_fabric_cgm_is_ingress_dynamic_wfq_supported))
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Gport type bcmCosqGportTypeFabricPipe is not supported on this device.\n");
        }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_port_priority_set(unit, gport, cosq, mode, weight));
    }
    /** Use case: IPC */
    else if (BCM_GPORT_IS_LOCAL_INTERFACE(gport))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_ipc_set(unit, gport, weight));
    }
    /*
     * Gport not supported
     */
    ...
    int *weight)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_queue_wfq_get(unit, gport, cosq, mode, weight));
    }
    /*
     * Use case: "RCY EIR shaper weights"
     */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_rcy_eir_weight_get(unit, gport, weight));
    }
    /*
     * Use case: "Mirror RCY EIR shaper weights"
     */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_port_priority_get(unit, gport, cosq, mode, weight));
    }
    /** Use case: IPC */
    else if (BCM_GPORT_IS_LOCAL_INTERFACE(gport))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_ipc_get(unit, gport, weight));
    }
    /*
     * Gport not supported
     */
    ...
    int weight)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport) || BCM_COSQ_GPORT_IS_FABRIC_PIPE(gport))
    {
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqControlHeaderTruncateSchedCompensationEnable:
        ...
        case bcmCosqControlEgressTCGCirSpEnable:
        {
            ...
            SHR_RANGE_VERIFY(cosq, -1, 0, _SHR_E_PARAM, "Cosq should be -1 or 0 only\n");
            SHR_RANGE_VERIFY(arg, 0, 1, _SHR_E_PARAM, "Invalid set argument %d, should be 0 or 1", arg);
            if (port != 0)
            {
                core = BCM_COSQ_GPORT_CORE_GET(port);
                DNXCMN_CORE_VALIDATE(unit, core, TRUE);
        ...
        case bcmCosqControlEgressPortTCSchedSpEnable:
        {
            if (!dnx_data_egr_queuing_v1.general.feature_get(unit, dnx_data_egr_queuing_v1_general_v1_supported))
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Control type %d is not supported on this device\n", type);
            ...
            SHR_RANGE_VERIFY(cosq, -1, 0, _SHR_E_PARAM, "Cosq should be -1 or 0 only\n");
            SHR_RANGE_VERIFY(arg, 0, 1, _SHR_E_PARAM, "Invalid argument %d, should be 0 or 1", arg);
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid gport (0x%x), has to be core gport", port);
            }
            break;
        }
        ...
        case bcmCosqControlPolicerTCSMThresh:
        {
            ...
            SHR_RANGE_VERIFY(cosq, 0, BCM_COS_COUNT - 1, _SHR_E_PARAM, "Invalid COSQ! \n");
            break;
        }
        case bcmCosqControlServicePoolUcSize:
        case bcmCosqControlServicePoolMcSize:
        {
            if (dnx_data_ecgm.general.feature_get(unit, dnx_data_ecgm_general_v1_supported) == FALSE)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "This API is not supported for this device type.\n");
            }
            else if (BCM_COSQ_GPORT_IS_CORE(port) || port == 0)
            {
                core = (port == 0) ? BCM_CORE_ALL : BCM_COSQ_GPORT_CORE_GET(port);
                DNXCMN_CORE_VALIDATE(unit, core, TRUE);
            }
            else
            {
            ...
            SHR_RANGE_VERIFY(arg, 0, dnx_data_ecgm.core_resources.total_dbs_get(unit) - 1, _SHR_E_PARAM,
                             "Pool size out of bound");
            break;
        }
        default:
        {
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_control_device_verify(unit, port, cosq, type, arg));
    ...
    switch (type)
    {
        ...
        case bcmCosqControlHeaderTruncateSchedCompensationEnable:
        {
            SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_scheduler_enable_set(unit, type, arg));
            break;
        }
        case bcmCosqControlEgressTCGCirSpEnable:
        {
        ...
        {
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "type not supported in this device");
            }
            break;
        }
        default:
    ...
    int *arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_control_device_verify(unit, port, cosq, type, 0));
    ...
    switch (type)
    {
        ...
        case bcmCosqControlHeaderTruncateSchedCompensationEnable:
        {
            SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_scheduler_enable_get(unit, type, arg));
            break;
        }
        case bcmCosqControlEgressTCGCirSpEnable:
        {
        ...
        {
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "type not supported in this device");
            }
            break;
        }
        default:
    ...
    bcm_cosq_control_t type)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqControlEgressShaperEnable:
        {
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_rate_class_gport_verify(unit, port, cosq, is_vsq_allowed));
    ...
    bcm_gport_t port)
{
    ...
    if (port != BCM_GPORT_INVALID
        && (BCM_GPORT_INGRESS_LATENCY_PROFILE_GET(port) >
            dnx_data_latency.based_admission.max_supported_profile_get(unit)))
    {
    ...
    bcm_cosq_control_t type)
{
    ...
    /*
     * check if it is supported for egress
     */
    if (BCM_GPORT_IS_END_TO_END_LATENCY_PROFILE(port) || BCM_GPORT_IS_END_TO_END_AQM_LATENCY_PROFILE(port))
    {
        if (!dnx_data_latency.features.feature_get(unit, dnx_data_latency_features_egress_latency_track_support))
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Egress latency track is not supported on this device.");
        }
    }
    /*
    ...
    int is_set)
{
    ...
    /** Verify cosq param */
    if (type == bcmCosqControlLatencyVoqRejectBinThreshold || type == bcmCosqControlLatencyBinThreshold)
    {
        if (cosq >= dnx_data_latency.features.latency_bins_get(unit))
        {
    ...
    int arg)
{
    ...
    /*
     * Use cases: "Port scheduler compensation" , "Low Rate Factor", Global VOQ ....
     */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_local_set(unit, port, cosq, type, arg));
    }
    else if (BCM_GPORT_IS_LOCAL_INTERFACE(port))
    {
        SHR_INVOKE_VERIFY_DNXC(dnx_cosq_control_interface_verify(unit, port, cosq, type));
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_local_interface_set(unit, port, cosq, type, arg));
    }
    else if (BCM_COSQ_GPORT_IS_RCY(port))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_rcy_set(unit, port, cosq, type, FALSE, arg));
    }
    else if (BCM_COSQ_GPORT_IS_RCY_MIRR(port))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_rcy_set(unit, port, cosq, type, TRUE, arg));
    }
    else if (BCM_COSQ_GPORT_IS_PORT_TC(port))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_tc_set(unit, port, cosq, type, arg));
    }
    else if (BCM_COSQ_GPORT_IS_PORT_TCG(port))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_tcg_set(unit, port, cosq, type, arg));
    }
    /*
     ...
     * AQM profile
     */
    else if (BCM_GPORT_IS_AQM_PROFILE(port))
    {
        SHR_IF_ERR_EXIT(dnx_cosq_aqm_config_set(unit, port, cosq, type, arg));
    }
    /*
     * aqm flow id
     */
    else if (BCM_GPORT_IS_AQM_FLOW_ID(port))
    {
        /*
         * Same flow applies for BCM_GPORT_IS_LATENCY_AQM_FLOW_ID
         */
        SHR_IF_ERR_EXIT(dnx_cosq_aqm_flow_profile_mapping_set(unit, port, cosq, type, arg));
    }
    /*
     * Use cases: "OCB Only"
     */
    ...
    int *arg)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_rcy_get(unit, port, cosq, type, FALSE, arg));
    }
    else if (BCM_COSQ_GPORT_IS_RCY_MIRR(port))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_rcy_get(unit, port, cosq, type, TRUE, arg));
    }
    else if (BCM_GPORT_IS_LOCAL(port))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_local_get(unit, port, cosq, type, arg));
    }
    else if (BCM_GPORT_IS_LOCAL_INTERFACE(port))
    {
        SHR_INVOKE_VERIFY_DNXC(dnx_cosq_control_interface_verify(unit, port, cosq, type));
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_local_interface_get(unit, port, cosq, type, arg));
    }
    else if (BCM_COSQ_GPORT_IS_PORT_TC(port))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_tc_get(unit, port, cosq, type, arg));
    }
    else if (BCM_COSQ_GPORT_IS_PORT_TCG(port))
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_cosq_control_port_tcg_get(unit, port, cosq, type, arg));
    }
    /*
     * latency profile
     */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_cosq_aqm_config_get(unit, port, cosq, type, arg));
    }
    /*
     * aqm flow id
     */
    else if (BCM_GPORT_IS_AQM_FLOW_ID(port))
    {
         ...
         * Same flow applies for BCM_GPORT_IS_LATENCY_AQM_FLOW_ID
         */
        SHR_IF_ERR_EXIT(dnx_cosq_aqm_flow_profile_mapping_get(unit, port, cosq, type, (uint32 *) arg));
    }
    /*
     * Use cases: "OCB Only"
     */
    ...
    bcm_cosq_gport_info_t * gport_info)
{
    ...
    switch (gport_type)
    {
        ...
        case bcmCosqGportTypePortTCSched:
            core = -1;
            break;
    ...
    bcm_gport_t * out_gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_handle_core_verify(unit, core, gport_info, out_gport));
        ...
        case bcmCosqGportTypePortTCSched:
        {
            ...
            switch (gport_info->gport_type)
            {
                ...
                case bcmCosqGportTypeMulticastEgress:
                    ...
                    break;
                case bcmCosqGportTypePortTCSched:
                    BCM_COSQ_GPORT_PORT_TC_SCHED_SET(*out_gport, local_port);
                    break;
                    /*
    ...
    uint32 flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_discard_set_verify(unit, flags));
    ...
    bcm_cosq_threshold_t * threshold)
{
    ...
    switch (threshold->type)
    {
        ...
        case (bcmCosqThresholdPacketDescriptorsAlpha):
            if (dnx_data_ecgm.general.feature_get(unit, dnx_data_ecgm_general_v1_supported))
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Feature is not supported in this device\n");
            }
            else
            {
    ...
    bcm_cosq_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_threshold_verify(unit, gport, cosq, threshold));
    ...
    uint32 flags)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_recycle_shaper_bandwidth_set(unit, gport, FALSE, kbits_sec_max, flags));
    }
    /*
     * Use case: "Mirror"
     */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_recycle_shaper_bandwidth_set(unit, gport, TRUE, kbits_sec_max, flags));
    }
    /*
     * Use case: "EGQ port/interface"
     */
    ...
    {
        /** Egress port */
        SHR_IF_ERR_EXIT(dnx_egr_tm_port_shaper_bandwidth_set
                        (unit, parent_port, cosq, kbits_sec_min, kbits_sec_max, flags));
    }
    else if (BCM_GPORT_IS_LOCAL_INTERFACE(parent_port))
    {
        /** Egress interface */
        SHR_IF_ERR_EXIT(dnx_egr_tm_interface_shaper_bandwidth_set(unit, parent_port, cosq,
                                                                  kbits_sec_min, kbits_sec_max, flags));
    }
    /*
     * Use case: "EGQ port TC"
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_queue_shaper_bandwidth_set(unit, parent_port, cosq,
                                                              kbits_sec_min, kbits_sec_max, flags));
    }
    /*
     * Use case: "EGQ port TCG"
    ...
    uint32 *flags)
{
    ...
    {
        /** Egress port */
        SHR_IF_ERR_EXIT(dnx_egr_tm_port_shaper_bandwidth_get(unit, gport, cosq, kbits_sec_min, kbits_sec_max, flags));

    }
    else if (BCM_GPORT_IS_LOCAL_INTERFACE(gport))
    {
        /** Egress interface */
        SHR_IF_ERR_EXIT(dnx_egr_tm_interface_shaper_bandwidth_get
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_recycle_shaper_bandwidth_get(unit, gport, FALSE, kbits_sec_max, flags));
    }
    /*
     * Use case: "Mirror"
     */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_recycle_shaper_bandwidth_get(unit, gport, TRUE, kbits_sec_max, flags));
    }
    /*
     * Use case: "EGQ port TC"
     * (Legacy implementation)
    ...
    {
        SHR_IF_ERR_EXIT(dnx_egr_tm_queue_shaper_bandwidth_get(unit, res_gport, cosq,
                                                              kbits_sec_min, kbits_sec_max, flags));
    }
    /*
     * Use case: "EGQ port TCG"
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
    ...
    if (is_egress)
    {
        ...
        /** Validate if virtual flows are enabled, if true we cannot allocate on VOQ 0*/
        ...
        if ((base_qid == dnx_data_sch.flow.virtual_flow_mapped_queue_get(unit))
            && (is_enabled == SCH_VIRTUAL_FLOWS_ENABLED))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot map connector to VOQ=0 when virtual flows are enabled\n");
        }
    }
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_connection_set_verify(unit, gport_connect));
    ...
    {
        /** is_egress */
        SHR_IF_ERR_EXIT(dnx_scheduler_connection_set(unit, gport_connect, FALSE));
    }
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_scheduler_is_voq_connector_virtual(unit, gport_connect->voq_connector, &is_virtual));
        if (is_virtual)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Not supported for virtual flow voq connector 0x%x\n",
                         gport_connect->voq_connector);
        }
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_connection_get_verify(unit, gport_connect));
    ...
    bcm_gport_t * gport)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_add_verify(unit, port, numq, flags, gport));
    ...
    {
        SHR_IF_ERR_EXIT(dnx_cosq_aqm_flow_id_gport_add(unit, flags, gport));
    }
    else
    {
    ...
    bcm_gport_t gport)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_cosq_aqm_flow_id_gport_delete(unit, gport));
    }
    else
    {
    ...
    uint32 *flags)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_get_verify(unit, gport, physical_port, num_cos_levels, flags));
    ...
    bcm_switch_profile_mapping_t * profile_mapping)
{
    ...
    if (profile_mapping->profile_type == bcmCosqIngressQueueToRateClass)
    {
        ...
        SHR_INVOKE_VERIFY_DNXC(dnx_cosq_rate_class_gport_verify
    ...
    int *queue_id)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_to_voq_get_verify(unit, *core, base_queue, cosq, force_core_all));
    ...
    bcm_switch_profile_mapping_t * profile_mapping)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_profile_mapping_set_verify(unit, gport_to_map, cosq, flags, profile_mapping));
    ...
    switch (profile_mapping->profile_type)
    {
        ...
        case bcmCosqPortToEndToEndLatencyProfile:
            ...
            break;
        case bcmCosqPortToOcbGlobalDropProfile:
            profile_id = BCM_GPORT_OCB_GLOBAL_DROP_PROFILE_GET(profile_mapping->mapped_profile);
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_port_ocb_global_drop_profile_map_set
                            (unit, flags, gport_to_map, cosq, profile_id));
            break;
        default:
    ...
    bcm_switch_profile_mapping_t * profile_mapping)
{
    ...
    switch (profile_mapping->profile_type)
    {
        ...
        case bcmCosqPortToEndToEndLatencyProfile:
            ...
            break;
        case bcmCosqPortToOcbGlobalDropProfile:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_port_ocb_global_drop_profile_map_get
                            (unit, flags, gport_to_map, cosq, &profile_id));
            BCM_GPORT_OCB_GLOBAL_DROP_PROFILE_SET(profile_mapping->mapped_profile, profile_id);
            break;
        default:
    ...
    bcm_cosq_fadt_threshold_t * threshold)
{
    ...
    if (fadt_info->thresh_type != bcmCosqFadtEgressReceiveDrop &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueDrop &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueFcHigh &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueFcLow &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueFcPfcOn &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueFcPfcOff)
    {
        /*
    ...
    switch (fadt_info->thresh_type)
    {
        ...
        case bcmCosqFadtEgressReceiveDrop:
        case bcmCosqFadtEgressEnqueueDrop:
        case bcmCosqFadtEgressEnqueueFcHigh:
        case bcmCosqFadtEgressEnqueueFcLow:
        case bcmCosqFadtEgressEnqueueFcPfcOn:
        case bcmCosqFadtEgressEnqueueFcPfcOff:
            break;
        default:
    ...
    bcm_cosq_fadt_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_fadt_threshold_verify(unit, flags, fadt_info, threshold));
    ...
    switch (fadt_info->thresh_type)
    {
        ...
        case bcmCosqFadtEgressReceiveDrop:
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "thresh type not supported in this device");
        }
            break;
        case bcmCosqFadtEgressEnqueueDrop:
        case bcmCosqFadtEgressEnqueueFcHigh:
...
/*
 * brief - Function checks if WRED or ECN WRED is enabled
 */
static shr_error_e
dnx_cosq_icgm_wred_enable_verify(
    int unit,
    int core_id,
    int qid,
    int cosq,
    int *wred_en)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_rate_class_get(unit, core_id, qid + cosq, &rate_class));
    BCM_GPORT_PROFILE_SET(rate_class_gport, rate_class);

    /*
     * Verify if WRED is enabled for atleast one of the colors
     */
    for (dp = 0; dp < 4; dp++)
    {
        switch (dp)
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_voq_discard_get(unit, rate_class_gport, 0, &discard));
    ...
    /*
     * If WRED is not enabled for one of the colors , check if ECN WRED is enabled
     */
    if (*wred_en == 0)
    {
    ...
    uint64 *value)
{
    ...
    if (BCM_COSQ_GPORT_IS_VSQ(gport))
    {
        if (!BCM_COSQ_GPORT_IS_VSQ_PG(gport) && !BCM_COSQ_GPORT_IS_VSQ_GL(gport))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported vsq gport 0x%x\n", gport);
        }
    }
    else if ((BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport)) || (BCM_GPORT_IS_MCAST_QUEUE_GROUP(gport)))
    {
        SHR_IF_ERR_EXIT(dnx_cosq_gport_to_voq_get(unit, gport, stat_key->cosq, FALSE, &core_id, &qid));
    }
    else if (BCM_COSQ_GPORT_IS_CORE(gport))
    {
        core_id = BCM_COSQ_GPORT_CORE_GET(gport);
    ...
    if (BCM_COSQ_GPORT_IS_VSQ_GL(gport))
    {
        SHR_IF_ERR_EXIT(dnx_cosq_icgm_resource_stat_index_convert(unit, stat_key->index, &pool_id));
        switch (stat_key->stat_type)
        {
            ...
            case bcmCosqICgmMaxGrntdBytes:
            case bcmCosqICgmMaxHdrmBytes:
                SHR_RANGE_VERIFY(pool_id, 0, dnx_data_ingr_congestion.vsq.pool_nof_get(unit) - 1, _SHR_E_PARAM,
                                 "index %d is out of range\n", pool_id);
                break;

            default:
                SHR_ERR_EXIT(_SHR_E_PARAM, "resource statistic type not supported for Global vsq gport %d.",
                             stat_key->stat_type);
        }
    }
    else if (BCM_COSQ_GPORT_IS_VSQ_PG(gport))
    {
        switch (stat_key->stat_type)
        {
            case bcmCosqICgmCurrSharedBytes:
    ...
    else if ((BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport)) || (BCM_GPORT_IS_MCAST_QUEUE_GROUP(gport)))
    {
        switch (stat_key->stat_type)
        {
            case bcmCosqICgmCurrTotalBytes:
            case bcmCosqICgmCurrOcbBytes:
            case bcmCosqICgmCurrOcbBuffers:
                break;
            case bcmCosqICgmAvrgTotalBytes:
                /*
                 * Average is enabled when WRED is enabled 
                 */
                SHR_IF_ERR_EXIT(dnx_cosq_icgm_wred_enable_verify(unit, core_id, qid, stat_key->cosq, &wred_en));
                if (!wred_en)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "WRED is not enabled");
                }
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_PARAM, "resource statistic type not supported for voq gport %d.",
                             stat_key->stat_type);
        }
    }
    else
    ...
    uint64 *value)
{
    ...
    dnx_ingress_congestion_voq_occupancy_info_t voq_queue_info;
    dnx_ingress_congestion_gl_vsq_occupancy_info_t gl_vsq_occupancy_info;
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_icgm_resource_stat_get_verify(unit, stat_key, value));

    gport = stat_key->gport;

    if (BCM_COSQ_GPORT_IS_VSQ(gport))
    {
        /** Global VSQ Statistics */
        if (BCM_COSQ_GPORT_IS_VSQ_GL(gport))
        {
            core_id = BCM_COSQ_GPORT_VSQ_CORE_ID_GET(gport);
            SHR_IF_ERR_EXIT(dnx_cosq_icgm_resource_stat_index_convert(unit, stat_key->index, &pool_id));
            ...
            sal_memset(&gl_vsq_occupancy_info, 0, sizeof(dnx_ingress_congestion_gl_vsq_occupancy_info_t));

            /** Get lossless pool */
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.port_based_vsq.
                            lossless_pool_id.get(unit, core_id, &lossless_pool));
            /*
            ...
            switch (stat_key->stat_type)
            {
                ...
                case bcmCosqICgmMaxGrntdBytes:
                case bcmCosqICgmMaxHdrmBytes:
                    /** Get current occupancy statistics for Global VSQ */
                    SHR_IF_ERR_EXIT(dnx_ingress_congestion_gl_vsq_occupancy_info_get
                                    (unit, core_id, &gl_vsq_occupancy_info));
                    break;
            ...
            {
                case bcmCosqICgmCurrSharedBytes:
                    COMPILER_64_SET(*value, 0, gl_vsq_occupancy_info.shared[pool_id]);
                    break;
                case bcmCosqICgmCurrGrntdBytes:
                    COMPILER_64_SET(*value, 0, gl_vsq_occupancy_info.guaranteed[pool_id]);
                    break;
                case bcmCosqICgmCurrHdrmBytes:
                    /*
                     * Only one pool can be associated with lossless traffic.
                     */
                    if (pool_id == lossless_pool)
                    {
                        COMPILER_64_SET(*value, 0, gl_vsq_occupancy_info.headroom);
                    }
                    else
                    {
                    ...
                    break;
                case bcmCosqICgmMaxSharedBytes:
                    COMPILER_64_SET(*value, 0, gl_vsq_occupancy_info.shared_max[pool_id]);
                    break;
                case bcmCosqICgmMaxGrntdBytes:
                    COMPILER_64_SET(*value, 0, gl_vsq_occupancy_info.guaranteed_max[pool_id]);
                    break;
                case bcmCosqICgmMaxHdrmBytes:
                    /*
                     * Only one pool can be associated with lossless traffic.
            ...
            /**
             * Clear SW state if needed
             */
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_gl_vsq_occupancy_max_value_clear
                            (unit, core_id, pool_id, lossless_pool, stat_key->stat_type));
        }
        /** VSQ Statistics */
    ...
    {
        /** Clear occupancy structure */
        sal_memset(&voq_queue_info, 0, sizeof(dnx_ingress_congestion_voq_occupancy_info_t));
        /*
         * Collect occupancy statistics
         */
        switch (stat_key->stat_type)
        {
            ...
            case bcmCosqICgmCurrOcbBuffers:
                ...
                {
                    voq_id = BCM_GPORT_MCAST_QUEUE_GROUP_QID_GET(gport);
                    core_id = BCM_GPORT_MCAST_QUEUE_GROUP_CORE_GET(gport);
                }
                voq_id += stat_key->cosq;
                SHR_IF_ERR_EXIT(dnx_ingress_congestion_voq_occupancy_info_get(unit, core_id, voq_id, &voq_queue_info));
                break;
            default:
                break;
        }
        ...
        {
            case bcmCosqICgmCurrTotalBytes:
                COMPILER_64_SET(*value, 0, voq_queue_info.size);
                break;
            case bcmCosqICgmAvrgTotalBytes:
                COMPILER_64_SET(*value, 0, voq_queue_info.size_avrg);
                break;
            case bcmCosqICgmCurrOcbBytes:
                COMPILER_64_SET(*value, 0, voq_queue_info.size_sram);
                break;
            case bcmCosqICgmCurrOcbBuffers:
                COMPILER_64_SET(*value, 0, voq_queue_info.buffer_size);
                break;
            default:
                break;
        }
    ...
    bcm_cosq_rate_class_create_info_t * create_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_rate_class_create_verify(unit, gport, flags, create_info));
    ...
    int is_set)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqGenericControlLatencyVoqRejectProb:
            /** check if key is correct */
            if (BCM_COSQ_GENERIC_KEY_LAT_BIN_COLOR_BIN_GET(key) >= dnx_data_latency.features.latency_bins_get(unit))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "key=%d not correct for type=%d \n", key, type);
            }
            SHR_IF_ERR_EXIT(dnx_cosq_control_validate_latency_based_admission(unit, type, BCM_GPORT_INVALID));
        ...
        case bcmCosqGenericControlLatencyVoqRejectMinThreshold:
            ...
            break;
        case bcmCosqGenericControlOcbGlobalDropPriorityMap:
        case bcmCosqGenericControlOcbGlobalDropThreshold:
            /** Validated in inner functions */
            break;
        default:
    ...
    uint32 arg)
{
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_generic_control_validate(unit, flags, gport, key, type, arg, TRUE));
    switch (type)
    {
        case bcmCosqGenericControlLatencyVoqRejectEnable:
        case bcmCosqGenericControlLatencyVoqRejectMinThreshold:
        case bcmCosqGenericControlLatencyVoqRejectProb:
            ...
            break;
        case bcmCosqGenericControlOcbGlobalDropPriorityMap:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_ocb_global_drop_profile_priority_map_set
                            (unit, flags, gport, key, arg));
            break;
        case bcmCosqGenericControlOcbGlobalDropThreshold:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_ocb_global_drop_profile_threshold_set(unit, flags, gport, key, arg));
            break;
        default:
    ...
    uint32 *arg)
{
    ...
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_NULL_CHECK(arg, _SHR_E_PARAM, "arg");
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_generic_control_validate(unit, flags, gport, key, type, 0, FALSE));
    switch (type)
    {
        case bcmCosqGenericControlLatencyVoqRejectEnable:
        case bcmCosqGenericControlLatencyVoqRejectMinThreshold:
        case bcmCosqGenericControlLatencyVoqRejectProb:
            ...
            break;
        case bcmCosqGenericControlOcbGlobalDropPriorityMap:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_ocb_global_drop_profile_priority_map_get
                            (unit, flags, gport, key, arg));
            break;
        case bcmCosqGenericControlOcbGlobalDropThreshold:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_ocb_global_drop_profile_threshold_get(unit, flags, gport, key, arg));
            break;
        default:
    ...
    uint32 *is_set)
{
    ...
    SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Global VOQ is not supported\n");
    ...
    bcm_cos_queue_t * cosq)
{
...

    {
        ...
        SHR_INVOKE_VERIFY_DNXC(dnx_cosq_port_mapping_multi_verify(unit, port, count, priority, cosq));

        SHR_ALLOC_ERR_EXIT(key_array, sizeof(bcm_cosq_queue_offset_mapping_key_t) * count, "key_array", "%s%d%s",
                           "Failed allocating key_array for port ", port, EMPTY);
        SHR_ALLOC_ERR_EXIT(offset_array, sizeof(bcm_cosq_queue_offset_t) * count, "offset_array", "%s%d%s",
                           "Failed allocating offset_array for port ", port, EMPTY);
...
*  probabilistic latency/Global_VOQ based marking cni/drop mechanism
*
* \param [in] unit -unit id
...
*/
void
dnx_cosq_probability_marking_alpha_value_calculation(
    int unit,
    uint64 delta_value,
    uint32 *shift_value)
{
    uint64 lfsr_calc;
    uint64 work_reg_64;
    COMPILER_UINT64 shift_value_log2_calc;
    ...
    if (COMPILER_64_GT(delta_value, work_reg_64))
    {
        ...
        COMPILER_64_SET(shift_value_log2_calc, 0, 0);
        COMPILER_64_COPY(shift_value_log2_calc, lfsr_calc);
    ...
    bcm_cosq_tas_profile_id_t pid)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tsn_tas_profile_commit(unit, port, pid));
    ...
    bcm_cosq_tas_profile_id_t * pid)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tsn_tas_profile_create(unit, port, profile, pid));
    ...
    bcm_cosq_tas_profile_id_t pid)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tsn_tas_profile_destroy(unit, port, pid));
    ...
    bcm_cosq_tas_profile_t * profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tsn_tas_profile_get(unit, port, pid, profile));
    ...
    bcm_cosq_tas_profile_t * profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tsn_tas_profile_set(unit, port, pid, profile));
    ...
    bcm_cosq_tas_profile_status_t * status)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tsn_tas_status_get(unit, port, pid, status));
...
/**
 * \brief -
 * Generic API function used for various TAS controls get
 *
 * Use cases:
 *
 * ** Use case: "Reset TAS port"
 * Do nothing, return 0
 *  Parameters:
 *  - port - logical port
 *  - type - bcmCosqTASControlTASReset
 *  - arg - valid pointer
 *
 * \param [in] unit -  Unit-ID
 * \param [in] port - description per use case.
    ...
    int *arg)
{
    ...
    switch (type)
    {
        case bcmCosqTASControlTASReset:
            SHR_NULL_CHECK(arg, _SHR_E_PARAM, "arg");
            *arg = 0;
            break;
        default:
...
/**
 * \brief -
 * Generic API function used for configuring various TAS controls
 *
 * Use cases:
 *
 * ** Use case: "Reset TAS port"
 * Reset TAS port, deactivated all port's profiles
 *  Parameters:
 *  - port - logical port
 *  - type - bcmCosqTASControlTASReset
 *  - arg - should be 1
    ...
    int arg)
{
    ...
    {
        case bcmCosqTASControlTASReset:
            SHR_IF_ERR_EXIT(dnx_tsn_tas_port_reset(unit, port, arg));
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "type not supported %d", type);
    }
