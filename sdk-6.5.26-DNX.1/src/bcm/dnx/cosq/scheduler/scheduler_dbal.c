    ...
    dnx_sch_flow_credit_source_cos_data_t * credit_source_cos)
{
    ...
    /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SCH_FLOW_CREDIT_SRC_COS, &entry_handle_id));
    ...
    dnx_sch_flow_credit_source_cos_data_t * credit_source_cos)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_CREDIT_SRC_HR_SEL_DUAL, INST_SINGLE,
    ...
    int full)
{
    ...
    for (i = 0; i < nof_remote_cores; i++)
    {
        ...
        dbal_entry_value_field32_set(unit, token_count_entry_handle_id, DBAL_FIELD_TOKEN_COUNT, INST_SINGLE,
                                     token_count);
        dbal_entry_value_field32_set(unit, token_count_entry_handle_id, DBAL_FIELD_SLOW_TOKEN_COUNT, INST_SINGLE, 1);

        dbal_entry_value_field32_set(unit, token_count_entry_handle_id, DBAL_FIELD_FULL, INST_SINGLE, full);
    ...
    int flow_id)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_sch_flow_shaper_token_count_set(unit, core_id, &flow_id, 1,
                                                        dnx_data_sch.dbal.token_count_default_get(unit), 0));
    ...
    uint32 max_burst)
{
    ...
    /** Updating max_burst requires updating token count, unless we are under traffic */
    if (!is_under_traffic)
    {
        SHR_IF_ERR_EXIT(dnx_sch_flow_shaper_token_count_set
                        (unit, core_id, actual_flow_id, nof_remote_cores, max_burst, 1));
    }
    ...
    int is_high_priority)
{
    ...
    if (!is_high_priority && !dnx_data_sch.general.feature_get(unit, dnx_data_sch_general_fc_on_channelized_if_only))
    {
        egq_if += dnx_data_egr_queuing.params.first_lp_egr_interface_get(unit);
    }
    ...
    int *is_high_priority)
{
   ...
   /** Setting pointers value to receive the fields */
    ...
    {
        if (*egq_if < dnx_data_egr_queuing.params.first_lp_egr_interface_get(unit))
        {
            *is_high_priority = TRUE;
        }
        else
    ...
    uint32 *enable)
{
    ...
    *flow_min = hw_min * 16;
    /** 15 flows are always lost in the conversion from SW to HW and back */
    *flow_max = hw_max * 16 + 15;
 ...
 */
shr_error_e
dnx_sch_dbal_lag_port_config_set(
    int unit,
    int core_id,
    int pp_dsp,
    int member,
    int sch_core)
{
    ...
    if (dnx_data_device.general.feature_get(unit, dnx_data_device_general_multi_core))
    {
        ...
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_FLIP, INST_SINGLE,
                                         core_id == sch_core ? FALSE : TRUE);
        }
...
/**
 * \brief clear the configuration of LAG port core
 */
shr_error_e
dnx_sch_dbal_lag_port_config_clear(
    int unit,
    int core_id,
    int pp_dsp)
{
    ...
    int pp_dsp)
{
    ...
    if (dnx_data_device.general.feature_get(unit, dnx_data_device_general_multi_core))
    {
        ...
        /** Setting value fields */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DUMMY_PP_DSP, INST_SINGLE, pp_dsp);
    ...
    int egq_pfc_fc)
{
    ...
    /** Taking a handle to DBAL table */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SCH_FC_ENABLERS, &entry_handle_id));

    /** Setting key fields */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);

    /** Set the fields values  */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EGQ_DEVICE_FC_ENABLE, INST_SINGLE, egq_device_fc);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EGQ_ERP_FC_ENABLE, INST_SINGLE, egq_erp_fc);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EGQ_ERP_TC_FC_ENABLE, INST_SINGLE, egq_erp_tc_fc);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EGQ_IF_FC_ENABLE, INST_SINGLE, egq_if_fc);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EGQ_PFC_FC_ENABLE, INST_SINGLE, egq_pfc_fc);
 ...
 */
shr_error_e
dnx_sch_flow_quartet_order_set(
    int unit,
    int core_id,
    uint32 se_group,
    dbal_enum_value_field_quartet_order_e quartet_order)
{
    uint32 entry_handle_id;
    ...
    /** Taking a handle to DBAL table */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SCH_FLOW_QUARTET_ORDER, &entry_handle_id));

    /** Setting key fields */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SE_GROUP, se_group);

    /** Set the fields values  */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_QUARTET_ORDER, INST_SINGLE, quartet_order);
...
/**
 * \brief - Enable/disable virtual flows
 */
shr_error_e
dnx_sch_dbal_virtual_flow_enable_set(
    int unit,
    int enable)
{
    uint32 entry_handle_id;
    ...
    /** Setting value fields */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VIRTUAL_FLOW_ENABLE, INST_SINGLE, enable);
...
/**
 * \brief - Get if virtual flows are enabled/disabled
 */
shr_error_e
dnx_sch_dbal_virtual_flow_enable_get(
    int unit,
    int *enable)
{
    uint32 entry_handle_id;
    ...
    /** Setting pointers value to receive the fields */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_VIRTUAL_FLOW_ENABLE, INST_SINGLE, (uint32 *) enable);
...
/**
 *\brief - Generate mannually flow status message for specific flow
 */
shr_error_e
dnx_sch_dbal_manual_fsm_generate(
    int unit,
    int core_id,
    int flow_id,
    dbal_enum_value_field_flow_slow_status_e slow_status)
{
    ...
    /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SCH_MANUAL_FSM_GENERATE, &entry_handle_id));
    ...
    /** Set the fields values  */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SCH_FLOW_ID, INST_SINGLE, flow_id);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FLOW_SLOW_STATUS, INST_SINGLE, slow_status);
    ...
    int power_down)
{
   ...
   /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SCH_POWER_DOWN, &entry_handle_id));
