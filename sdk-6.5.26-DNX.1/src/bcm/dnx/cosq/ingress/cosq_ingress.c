    ...
    int unit)
{
    ...
    {
        /*
         * Init ingress shapers
         */
        if (dnx_data_device.general.max_mpps_get(unit) != -1)
        {
            SHR_IF_ERR_EXIT(dnx_cosq_ingress_receive_packet_rate_limit_set(unit,
                                                                           dnx_data_device.general.max_mpps_get(unit)));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    dnx_ingress_ire_counter_info_t * counter_info)
{
    ...
    sal_memset(counter_info, 0, sizeof(dnx_ingress_ire_counter_info_t));
   ...
   /** Select the core */
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_NIF_PACKET_COUNTER_OVF, INST_SINGLE,
                               (uint32 *) &counter_info->nif_pkt_ovf);

    if (dnx_data_ingr_reassembly.priority.rcy_priorities_nof_get(unit) == 2)
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_RCYH_PACKET_COUNTER_OVF, INST_SINGLE,
                                   (uint32 *) &counter_info->rcyh_pkt_ovf);
    }
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_RCYL_PACKET_COUNTER_OVF, INST_SINGLE,
                               (uint32 *) &counter_info->rcyl_pkt_ovf);

    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CPU_PACKET_COUNTER_OVF, INST_SINGLE,
                                   (uint32 *) &counter_info->cpu_pkt_ovf);
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_OLP_PACKET_COUNTER_OVF, INST_SINGLE,
                                   (uint32 *) &counter_info->olp_pkt_ovf);
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_OAMP_PACKET_COUNTER_OVF, INST_SINGLE,
                                   (uint32 *) &counter_info->oamp_pkt_ovf);
    }
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_REGISTER_INTERFACE_PACKET_COUNTER_OVF, INST_SINGLE,
                               (uint32 *) &counter_info->reg_if_pkt_ovf);
    ...
    if (is_gtimer_enabled)
    {
        /*
         * when gtimer is enabled, OVF field is used to indicate that the timer has finished
         */
        counter_info->nif_pkt_ovf = 0;
        counter_info->rcyh_pkt_ovf = 0;
        counter_info->rcyl_pkt_ovf = 0;
        counter_info->cpu_pkt_ovf = 0;
        counter_info->olp_pkt_ovf = 0;
        counter_info->oamp_pkt_ovf = 0;
        counter_info->reg_if_pkt_ovf = 0;
        counter_info->ipc_pkt_ovf = 0;
    }
    ...
    dnx_ingress_sqm_counter_info_t * counter_info)
{
   ...
   /** Select the core */
    ...
    dbal_value_field64_request(unit, entry_handle_id, DBAL_FIELD_PKT_ENQ_CTR, INST_SINGLE, &counter_info->pkt_enq_cnt);
    {
        ...
        dbal_value_field64_request(unit, entry_handle_id, DBAL_FIELD_PKT_DEQ_CTR, INST_SINGLE,
                                   &counter_info->pkt_deq_cnt);
    }
...

    {
        ...
        dbal_tables_field_size_get(unit, DBAL_TABLE_INGRESS_SQM_COUNTER, DBAL_FIELD_PKT_DEQ_CTR, 0, 0, 0,
                                   &counter_size);
        /*
    ...
    uint32 flags)
{
    ...
    /** Validate inputs */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_receive_bandwith_set_validate
    ...
    uint32 *flags)
{
    ...
    /** Validate inputs */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_receive_bandwith_get_validate
                           (unit, gport, cosq, kbits_sec_min, kbits_sec_max, flags));
 ...
 */
static int
dnx_cosq_ingress_receive_packet_rate_limit_hw_set(
    int unit,
    int rate_mpps)
{
...
/**
 * 
 * \brief - Function to verify the device overall ingress 
 *        receive rate. The rate is given in Mega packets per
 *        second
 * 
 * \param unit 
 ...
 */
static int
dnx_cosq_ingress_receive_packet_rate_limit_verify(
    int unit,
    int rate_mpps)
{
    SHR_FUNC_INIT_VARS(unit);

    if (dnx_data_device.general.max_mpps_get(unit) != -1 && rate_mpps > dnx_data_device.general.max_mpps_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "rate_mpps (%d) should be below %d\n", rate_mpps,
                     dnx_data_device.general.max_mpps_get(unit));
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int rate_mpps)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ingress_receive_packet_rate_limit_verify(unit, rate_mpps));
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_receive_packet_rate_limit_hw_set(unit, rate_mpps));
