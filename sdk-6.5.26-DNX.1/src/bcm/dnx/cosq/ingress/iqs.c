...
/**
 * \brief - timeout for manual credit operation (micro second)
 */
#define DNX_IQS_MANUAL_CREDIT_TIMEOUT_US (1)
/*
 * }
 */
/*
...
/* { */
    if (dnx_data_iqs.deq_default.feature_get(unit, dnx_data_iqs_deq_default_dequeue_params_configuration))
    {
        SHR_IF_ERR_EXIT(dnx_iqs_dequeue_params_init(unit));
    }
/* } */
    /*
    ...
    bcm_cosq_delay_tolerance_t * thresholds)
{
    ...
    uint32 is_high_priority, is_low_delay;
    uint32 value;
    uint32 is_ocb_only;
    ...
    int unit)
{
    ...
    if (enable)
    {
        ...
        SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &time, &cycle_clocks));
    ...
    int unit)
{
    ...
    if (enable)
    {
        ...
        time.time_units = DNXC_TIME_UNIT_USEC;
        SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &time, &period_clocks));
        nof_queues = last_queue - first_queue + 1;
    ...
    int unit)
{
    ...
    if (dnx_data_iqs.dbal.feature_get(unit, dnx_data_iqs_dbal_fsm_sn_mask_supported))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FSM_SN_MASK, INST_SINGLE,
                                     dnx_data_iqs.dbal.fsm_sn_mask_get(unit));
    }
    ...
    uint32 *enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_iqs_db.delete_credit_request_profile.id.get(unit, profile_id));
    SHR_IF_ERR_EXIT(dnx_iqs_db.delete_credit_request_profile.valid.get(unit, enable));
    ...
    uint32 enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_iqs_db.delete_credit_request_profile.id.set(unit, profile_id));
    SHR_IF_ERR_EXIT(dnx_iqs_db.delete_credit_request_profile.valid.set(unit, enable));
 ...
 */
shr_error_e
dnx_iqs_queue_manual_operation_queue_id_set(
    int unit,
    int core,
    uint32 voq)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_IQS_QUEUE_MANUAL_OPERATION, &entry_handle_id));
 ...
 */
shr_error_e
dnx_iqs_queue_manual_credit_trigger_set(
    int unit,
    int core,
    uint32 enable)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CREDIT_TRIGGER, INST_SINGLE, enable);
 ...
 */
shr_error_e
dnx_iqs_queue_manual_credit_trigger_down_polling(
    int unit,
    int core)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(dnxcmn_polling
                    (unit, DNX_IQS_MANUAL_CREDIT_TIMEOUT_US, 100, entry_handle_id, DBAL_FIELD_CREDIT_TRIGGER, 0));
...
static shr_error_e
dnx_iqs_watchdog_delete_special_mode(
    int unit)
{
    int fix_exists = dnx_data_iqs.credit.feature_get(unit, dnx_data_iqs_credit_watchdog_delete_special_mode);
    uint32 entry_handle_id;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    if (fix_exists == 1)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENABLE_WATCHDOG_DELETE_SPECIAL_MODE, INST_SINGLE,
                                     1);
    ...
    int power_down)
{
   ...
   /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_IQS_POWER_DOWN, &entry_handle_id));
    ...
    int unit)
{
    ...
    if (dnx_data_iqs.general.feature_get(unit, dnx_data_iqs_general_is_used))
    {
        /*
         * Alloc SW state
         */
        ...
        /*
         * watchdog_delete_special_mode
         */
        SHR_IF_ERR_EXIT(dnx_iqs_watchdog_delete_special_mode(unit));
    }
    else
    {
        if (dnx_data_iqs.general.feature_get(unit, dnx_data_iqs_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_iqs_power_down_set(unit, 1));
        }
    }
