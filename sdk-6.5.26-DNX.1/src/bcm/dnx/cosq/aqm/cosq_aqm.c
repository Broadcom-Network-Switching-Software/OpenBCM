...
#define DNX_COSQ_AQM_DEFAULT_FLOW (0)
/*
 * }
 */
    ...
    int unit)
{
    ...
    if (dnx_data_cosq_aqm.general.feature_get(unit, dnx_data_cosq_aqm_general_is_aqm_supported))
    {
        ...
        /** allocate the first flow as default flow that do not perform any action */
        BCM_GPORT_LATENCY_AQM_FLOW_ID_SET(gport, DNX_COSQ_AQM_DEFAULT_FLOW);
        BCM_IF_ERROR_RETURN(dnx_cosq_aqm_flow_id_gport_add(unit, BCM_COSQ_GPORT_WITH_ID, &gport));
...
/**
 * \brief Validate the E2E-AQM-profile is valid, relevant for profiles that are already created
 *
 ...
 */
static shr_error_e
dnx_cosq_allocated_aqm_profile_verify(
    int unit,
    int profile_id)
{
    uint8 is_allocated;
    ...
    SHR_MAX_VERIFY(profile_id, profile_nof - 1, _SHR_E_PARAM,
                   "AQM-Latency-profile %d is out of range 0-%d\n", profile_id, profile_nof - 1);

    /** Canno't set properties for default profile*/
    if (profile_id == DNX_COSQ_AQM_DEFAULT_PROFILE)
    {
    ...
    if (!is_allocated)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unallocated AQM-Latency-profile profile_id=%d \n", profile_id);
    }
    ...
    int *gport_profile)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_COSQ_GPORT_WITH_ID))
    {
        ...
        profile_id = BCM_GPORT_AQM_PROFILE_GET(*gport_profile);

        /** Validate that the profile id within the valid range */
        SHR_MAX_VERIFY(profile_id, profile_nof - 1, _SHR_E_PARAM,
                       "AQM-Latency-profile %d is out of range 0-%d\n", profile_id, profile_nof - 1);

        /** Validate that the profile didn't allocated */
        SHR_IF_ERR_EXIT(dnx_cosq_aqm_db.aqm_profile_res.is_allocated(unit, profile_id, &is_allocated));
        if (is_allocated)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "AQM-profile already allocated, profile_id=%d \n", profile_id);
        }
    ...
    bcm_gport_t profile_gport)
{
    ...
    if (profile_id == DNX_COSQ_AQM_DEFAULT_PROFILE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The default AQM-profile cannot be deleted, profile_id=%d \n", profile_id);
    }
    ...
    uint8 is_set)
{
    ...
    if (type == bcmCosqControlEgressAqmFlowIdToProfile)
    {
        ...
        /** Verify Flow ID range*/
        SHR_MAX_VERIFY(flow_id, flows_nof - 1, _SHR_E_PARAM,
                       "AQM Flow ID %d is out of range 0-%d\n", flow_id, flows_nof - 1);

        if (is_set == TRUE)
        {

            /** In case of SET verify, validate that the flow was created */
            SHR_IF_ERR_EXIT(dnx_cosq_aqm_db.aqm_flow_id_res.is_allocated(unit, flow_id, &is_allocated));
            if (!is_allocated)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Unallocated AQM-Flow flow_id=%d \n", flow_id);
            }

            /** In case of SET verify, validate that the flow was created */
            if (flow_id == DNX_COSQ_AQM_DEFAULT_FLOW)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot set properties for default flow, flow_id=%d \n", flow_id);
            }

            /** In Case of SET verify, validate that the Profile is valid */
            SHR_IF_ERR_EXIT(dnx_cosq_allocated_aqm_profile_verify(unit, profile_id));
    ...
    if (type == bcmCosqControlEgressAqmFlowId)
    {
        ...
        if (is_set)
        {
            ...
            /** In case of SET verify, validate the flow was created */
            SHR_IF_ERR_EXIT(dnx_cosq_aqm_db.aqm_flow_id_res.is_allocated(unit, flow_id, &is_allocated));
            if (!is_allocated)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Unallocated AQM-Flow flow_id=%d \n", flow_id);
    ...
    bcm_gport_t * gport)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_COSQ_GPORT_WITH_ID))
    {
        ...
        if (is_allocated)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "AQM-flow already allocated, flow_id=%d \n", flow_id);
        }
    ...
    bcm_gport_t gport)
{
    ...
    SHR_MAX_VERIFY(flow_id, flow_nof - 1, _SHR_E_PARAM, "AQM Flow ID %d is out of range 0-%d\n", flow_id, flow_nof - 1);

    if (flow_id == DNX_COSQ_AQM_DEFAULT_FLOW)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The default AQM-flow cannot be deleted, flow_id=%d \n", flow_id);
    }
    ...
    int arg)
{
    ...
    SHR_MAX_VERIFY(aqm_profile_index, aqm_profile_nof - 1, _SHR_E_PARAM,
