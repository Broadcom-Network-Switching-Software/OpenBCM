...
#include <bcm_int/dnx/auto_generated/dnx_mpls_dispatch.h>
...
#include "mpls_evpn.h"
#include "src/bcm/dnx/flow/porting/mpls_flow.h"
#include "src/bcm/dnx/flow/porting/mpls_evpn_flow.h"
...
#define MPLS_TUNNEL_ENCAP_SUPPORTED_FLAGS_2 (BCM_MPLS_EGRESS_LABEL_FLAGS2_SVTAG_ENABLE | \
                                             BCM_MPLS_EGRESS_LABEL_FLAGS2_ESI_INTF_NAMESPACE | \
                                             BCM_MPLS_EGRESS_LABEL_FLAGS2_NO_TPID_CLASS)
/*
 * }
 */
/*
    ...
    dnx_egr_pointed_t * egr_pointed_id_p)
{
    ...
    else
    {
        ...
        if (!(this_is_egr_pointed_case == TRUE))
        {
            ...
            if (label_array->estimated_encap_size == -1)
            {
                alloc_info.bta = dnx_data_lif.out_lif.global_mpls_outlif_bta_sop_get(unit);
            }
            else
            {
                alloc_info.bta = label_array->estimated_encap_size;
            }
            ...
            *egr_pointed_id_p = DNX_ALGO_EGR_POINTED_INVALID;
            ...
            SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_allocate
                            (unit, lif_flags, DNX_ALGO_LIF_EGRESS, &alloc_info, global_lif_id_p));
        }
        else
        {
            /*
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    if (DNX_QOS_MAP_IS_MPLS_PHP(label_array[0].qos_map_id))
    {
        if (DNX_QOS_MAP_PROFILE_GET(label_array[0].qos_map_id) >=
            dnx_data_qos.qos.nof_egress_mpls_php_profiles_get(unit))
        {
    ...
    {
        if (DNX_QOS_MAP_PROFILE_GET(label_array[0].qos_map_id) >= dnx_data_qos.qos.nof_egress_profiles_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "qos map id (0x%x) is out of range ", label_array[0].qos_map_id);
        }
    }
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    bcm_encap_access_t encap_access = label_array[0].encap_access;
    int l3_itf_dummy;
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_REPLACE) &&
        !_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_WITH_ID))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        ...
        if ((num_labels < DNX_MPLS_PUSH_MIN_NUM_LABELS) || (num_labels > DNX_MPLS_PUSH_DATA_MAX_NUM_LABELS))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "num_labels(%d) is out of range. The range should be %d - %d", num_labels,
                         DNX_MPLS_PUSH_MIN_NUM_LABELS, DNX_MPLS_PUSH_DATA_MAX_NUM_LABELS);
        }
        /** if data entry used all labels must be valid */
        ...
        if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_REPLACE))
        {
            for (ii = 0; ii < DNX_MPLS_PUSH_MAX_NUM_LABELS; ii++)
            {
                bcm_mpls_egress_label_t_init(&existing_label_entry[ii]);
            }
            SHR_IF_ERR_EXIT(bcm_dnx_mpls_tunnel_initiator_get
                            (unit, label_array[0].tunnel_id, DNX_MPLS_PUSH_MAX_NUM_LABELS, existing_label_entry,
                             &label_count));
            
            if (label_count > DNX_MPLS_TUNNEL_ENCAP_TWO_LABELS)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "tunnel_id %d - REPLACE on tunnel with DATA not supported",
    ...
    for (ii = 0; (ii < num_labels) && (ii < DNX_MPLS_PUSH_MAX_NUM_LABELS); ii++)
    {
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_WITH_ID))
    {
        ...
        BCM_L3_ITF_SUB_TYPE_VIRTUAL_EGRESS_POINTED_SET(l3_itf_dummy, label_array[0].tunnel_id);
        if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_VIRTUAL_EGRESS_POINTED)
            && (BCM_L3_ITF_SUB_TYPE_VIRTUAL_EGRESS_POINTED_GET(l3_itf_dummy) >=
                dnx_data_l3.egr_pointed.nof_egr_pointed_ids_get(unit)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Egress pointed encap_id(0x%x) is out of range (0 - %d)", label_array[0].tunnel_id,
                         dnx_data_l3.egr_pointed.nof_egr_pointed_ids_get(unit));
        }
    }
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_REPLACE))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "replacing entry with logical phase(%d) with new value(%d) - not supported, logical phase must not change in REPLACE",
                         gport_hw_resources.logical_phase, logical_phase);
        }
    ...
    if (!DNX_MPLS_TUNNEL_INIT_IS_EVPN(label_array))
    {
        for (ii = 0; ii < num_labels; ii++)
        {
            if (_SHR_IS_FLAG_SET(label_array[ii].flags2, BCM_MPLS_EGRESS_LABEL_FLAGS2_NO_TPID_CLASS))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "BCM_MPLS_EGRESS_LABEL_FLAGS2_NO_TPID_CLASS is for EVPN only. MPLS tunnel never need it!");
            }
        }
    ...
    if ((label_array[0].l3_intf_id != 0) && (label_array[0].l3_intf_id != BCM_IF_INVALID))
    {
        ...
        dnx_algo_gpm_gport_hw_resources_t l3_intf_gport_hw_resources;
        ...
        if ((dnx_algo_gpm_gport_to_hw_resources
             (unit, next_pointer, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS, &l3_intf_gport_hw_resources)))
        {
    ...
    /** encap stage */
    if ((encap_access < bcmEncapAccessInvalid) || (encap_access > bcmEncapAccessRif))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "label_array[0].encap_access (%d) is invalid, supported values are %d - %d",
                     encap_access, bcmEncapAccessInvalid, bcmEncapAccessRif);
    }
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_PROTECTION))
    {
        SHR_IF_ERR_EXIT(dnx_failover_id_valid_verify
                        (unit, BCM_FAILOVER_TYPE_ENCAP, 0, label_array[0].egress_failover_id));
    }

    if (label_array[0].estimated_encap_size < -1)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "estimated_encap_size=%d. Invalid value.", label_array[0].estimated_encap_size);
    }
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_REPLACE))
    {
        ...
        /** SVTAG flag can't be replaced */
        if (_SHR_IS_FLAG_SET
            ((existing_label_entry[0].flags2 ^ label_array[0].flags2), BCM_MPLS_EGRESS_LABEL_FLAGS2_SVTAG_ENABLE))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Can't replace SVTAG configuration");
        }
    }

    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_TANDEM))
    {
        if (dnx_data_mpls.general.mpls_special_label_encap_handling_get(unit) &&
            (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_ENTROPY_ENABLE) ||
     ...
     */
    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_ALTERNATE_MARKING) ||
        _SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_ALTERNATE_MARKING_LOSS_SET) ||
...
dnx_mpls_tunnel_initiator_get_verify(
    ...
    int *label_count)
{
    int max_nof_labels = DNX_MPLS_PUSH_MAX_NUM_LABELS;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        max_nof_labels = DNX_MPLS_PUSH_DATA_MAX_NUM_LABELS;
    }
    if ((label_max < DNX_MPLS_PUSH_MIN_NUM_LABELS) || (label_max > max_nof_labels))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "label_max(%d) is out of range. The range should be %d - %d", label_max,
                     DNX_MPLS_PUSH_MIN_NUM_LABELS, max_nof_labels);
    }
...
dnx_mpls_encap_calc_lif_additional_header_profile(
    int has_cw,
    int entropy_label_enable,
    int entropy_lable_indication_enable)
{
    if (has_cw)
    {
        if (entropy_label_enable && entropy_lable_indication_enable)
        {
            return DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_CWOELOELI;
        }
        else if (entropy_label_enable)
        {
    ...
    else
    {
        ...
        else if (entropy_label_enable && entropy_lable_indication_enable)
        {
...
/**
* \brief
*  Calculates the needed lif additional header profile and allocate the related ETPP additional headers profile.
*/
shr_error_e
dnx_mpls_encap_additional_header_profile_alloc(
    int unit,
    uint8 is_replace,
    dnx_mpls_encap_t * encap_info_org,
    dnx_mpls_encap_t * encap_info,
    dnx_egress_headers_profile_mngr_info_t * headers_profile_mngr_info)
{
    dbal_enum_value_field_lif_ahp_mpls_e lif_ahp, lif_ahp_old;
    ...
    {
        ahs_namespace = AHS_NAMESPACE_MPLS_IOAM;
        if (!is_replace)
        {
            SHR_IF_ERR_EXIT(egress_additional_headers_profile_alloc
                            (unit, is_replace, DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_NONE,
                             DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_NONE, ahs_namespace, headers_profile_mngr_info));

            /** Config the header profile table if needed.*/
            SHR_IF_ERR_EXIT(egress_additional_headers_profile_first_set(unit, headers_profile_mngr_info));

            SHR_IF_ERR_EXIT(egress_additional_headers_profile_alloc
                            (unit, is_replace, DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_NONE,
                             DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_CW, ahs_namespace, headers_profile_mngr_info));
    ...
    {
        lif_ahp_old =
            encap_info_org ? encap_info_org->lif_additional_headers_profile : DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_NONE;

        lif_ahp = dnx_mpls_encap_calc_lif_additional_header_profile(encap_info->has_cw,
                                                                    (encap_info->label_array[0].flags &
                                                                     BCM_MPLS_EGRESS_LABEL_ENTROPY_ENABLE),
                                                                    (encap_info->label_array[0].flags &
                                                                     BCM_MPLS_EGRESS_LABEL_ENTROPY_INDICATION_ENABLE));

        ahs_namespace = encap_info->is_iml ? AHS_NAMESPACE_MPLS_IML : AHS_NAMESPACE_MPLS;
        SHR_IF_ERR_EXIT(egress_additional_headers_profile_alloc
                        (unit, is_replace, lif_ahp_old, lif_ahp, ahs_namespace, headers_profile_mngr_info));

        encap_info->lif_additional_headers_profile = lif_ahp;
...
/**
* \brief
*  De-allocate the related ETPP additional headers profile per the caculated namespace and lif additional headers profile.
*/
shr_error_e
dnx_mpls_encap_additional_header_profile_free(
    int unit,
    dnx_mpls_encap_t * encap_info)
{
    dnx_ahs_namespace_e ahs_namespace;
    ...
    if (_SHR_IS_FLAG_SET(encap_info->label_array[0].flags, BCM_MPLS_EGRESS_LABEL_ALTERNATE_MARKING))
    {
        ...
        SHR_IF_ERR_EXIT(egress_additional_headers_profile_free
                        (unit, DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_NONE, ahs_namespace, &headers_profile_mngr_info));
        SHR_IF_ERR_EXIT(egress_additional_headers_profile_last_clear(unit, &headers_profile_mngr_info));

        SHR_IF_ERR_EXIT(egress_additional_headers_profile_free
                        (unit, DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_CW, ahs_namespace, &headers_profile_mngr_info));
    }
    else
    {
        ahs_namespace = encap_info->is_iml ? AHS_NAMESPACE_MPLS_IML : AHS_NAMESPACE_MPLS;
        ...
        SHR_IF_ERR_EXIT(egress_additional_headers_profile_free
                        (unit, encap_info->lif_additional_headers_profile, ahs_namespace, &headers_profile_mngr_info));
    }
    ...
    int *entropy_lable_indication_enable)
{
    ...
    *has_cw = FALSE;
    *entropy_label_enable = FALSE;
    *entropy_lable_indication_enable = FALSE;
    switch (additional_headers_profile)
    {
        case DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_CWOELOELI:
            *has_cw = TRUE;
            *entropy_label_enable = TRUE;
            *entropy_lable_indication_enable = TRUE;
            break;
...
dnx_mpls_egress_lif_ttl_qos_profiles_sw_allocate(
    ...
    uint8 *out_lif_profile_last_reference)
{
    ...
    if (encap_info_orig->out_lif_profile_valid)
    {
        out_lif_profile_orig = encap_info_orig->out_lif_profile;
    }
    else
    {
        if (dbal_table_id == DBAL_TABLE_EEDB_PWE)
        {
            out_lif_profile_orig = DNX_OUT_LIF_PROFILE_DEFAULT;
        }
        else
        {
            out_lif_profile_orig = DNX_OUT_LIF_PROFILE_DEFAULT_MPLS_PROFILE;
        }
    }
    if (encap_info->labels_nof < DNX_MPLS_TUNNEL_ENCAP_TWO_LABELS)
    {
        ...
        encap_info->out_lif_profile_valid = TRUE;
    }
    else if (encap_info_orig->out_lif_profile_valid)
    {
        int new_out_lif_profile = 0;

        SHR_IF_ERR_EXIT(dnx_etpp_out_lif_profile_exchange
                        (unit, dbal_table_id, 0, &out_lif_profile_info, out_lif_profile_orig,
                         &new_out_lif_profile, out_lif_profile_last_reference));
    }
    ...
    lif_table_mngr_outlif_info_t * outlif_info)
{
     ...
     */
    outlif_info->logical_phase = encap_info->logical_phase;
    ...
    /*
     * Handle IOAM over VPLS, only one field exists
     */
    if (_SHR_IS_FLAG_SET(encap_info->label_array[0].flags, BCM_MPLS_EGRESS_LABEL_ALTERNATE_MARKING)
        && (dbal_table_id == DBAL_TABLE_EEDB_PWE))
    {
        outlif_info->table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_MPLS_ENCAP_IOAM_DUAL_HOM;

        label_w_l_bit = encap_info->label_array[0].label;
        if (_SHR_IS_FLAG_SET(encap_info->label_array[0].flags, BCM_MPLS_EGRESS_LABEL_ALTERNATE_MARKING_LOSS_SET))
        {
            label_w_l_bit |= (1 << DNX_MPLS_IOAM_PWE_EEDB_L_BIT_SHIFT);
        }

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DUAL_HOMING, INST_SINGLE, label_w_l_bit);
        SHR_EXIT();
    }
    ...
    if (encap_info->lif_additional_headers_profile != DBAL_ENUM_FVAL_LIF_AHP_MPLS_LIF_AHP_NONE)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LIF_AHP_MPLS,
                                     INST_SINGLE, encap_info->lif_additional_headers_profile);
    }

    if (encap_info->label_array[0].action == BCM_MPLS_EGRESS_ACTION_PHP)
    {
        uint32 map_id = encap_info->label_array[0].qos_map_id;

        if (DNX_QOS_MAP_IS_MPLS_PHP(map_id))
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TERMINATION_REMARK_PROFILE, INST_SINGLE,
                                         DNX_QOS_MAP_PROFILE_GET(map_id));
        }
     ...
     */
    if (encap_info->out_lif_profile_valid)
    {
    ...
    dnx_mpls_encap_t * encap_info)
{
    ...
    uint32 ttl_mode;
    uint32 qos_map_id = 0;
    dbal_enum_value_field_lif_ahp_mpls_e additional_headers_profile = 0;
    dbal_enum_value_field_encap_qos_model_e qos_model;
    bcm_qos_egress_model_t model;
    dbal_tables_e dbal_table_id;
    uint8 is_field_valid;
    int entropy_lable_enable = 0, entropy_lable_indication_enable = 0;
    uint32 label_w_l_bit = 0;
    ...
    encap_info->logical_phase = outlif_info->logical_phase;

    /*
     * Handle IOAM over VPLS, only one field exists
     */
    if (_SHR_IS_FLAG_SET(outlif_info->table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_MPLS_ENCAP_IOAM_DUAL_HOM))
    {
        encap_info->labels_nof = 1;

        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_DUAL_HOMING,
                                                            INST_SINGLE, &label_w_l_bit));

        encap_info->label_array[0].flags |= BCM_MPLS_EGRESS_LABEL_ALTERNATE_MARKING;
        if (_SHR_IS_FLAG_SET(label_w_l_bit, (1 << DNX_MPLS_IOAM_PWE_EEDB_L_BIT_SHIFT)))
        {
            encap_info->label_array[0].flags |= BCM_MPLS_EGRESS_LABEL_ALTERNATE_MARKING_LOSS_SET;
        }
        encap_info->label_array[0].label = label_w_l_bit & (~(1 << DNX_MPLS_IOAM_PWE_EEDB_L_BIT_SHIFT));
    ...
    if (_SHR_IS_FLAG_SET(outlif_info->table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_MPLS_ENCAP_PHP))
    {
        uint32 termination_remark_profile = 0;
        ...
        if ((is_field_valid == TRUE) && (termination_remark_profile != 0))
        {
            qos_map_id = termination_remark_profile;
    ...
    encap_info->label_array[0].qos_map_id = qos_map_id;
    if (encap_info->labels_nof == 2)
    {
        encap_info->label_array[1].qos_map_id = qos_map_id;
    }
    ...
    if (!_SHR_IS_FLAG_SET(outlif_info->table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_MPLS_ENCAP_PHP))
    {
        ...
        if (is_field_valid == TRUE)
        {
            encap_info->lif_additional_headers_profile = additional_headers_profile;

            dnx_mpls_encap_lif_additional_header_profile_to_label_flags(additional_headers_profile, &encap_info->has_cw,
                                                                        &entropy_lable_enable,
                                                                        &entropy_lable_indication_enable);
            if (entropy_lable_enable)
            {
                encap_info->label_array[0].flags |= BCM_MPLS_EGRESS_LABEL_ENTROPY_ENABLE;
            }
            if (entropy_lable_indication_enable)
            {
                encap_info->label_array[0].flags |= BCM_MPLS_EGRESS_LABEL_ENTROPY_INDICATION_ENABLE;
            }
        }
    }
    ...
    if ((dbal_table_id == DBAL_TABLE_EEDB_PWE) || (dbal_table_id == DBAL_TABLE_EEDB_EVPN))
    {
        ...
        if (is_field_valid)
        {
            encap_info->skip_llvp_profile = 0;
        }
        else
        {
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    is_replace = _SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_REPLACE);
    ...
    /** Set outlif eedb phase. */
    encap_access = label_array[0].encap_access;
    if (encap_access == bcmEncapAccessInvalid)
    {
        encap_access = bcmEncapAccessTunnel2;
    }
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_PROTECTION))
    {
        ...
        BCM_FAILOVER_ID_GET(encap_info.protection_pointer, encap_info.label_array[0].egress_failover_id);
    }

    /** allocate lif, TTL and qos profiles if needed */
    SHR_IF_ERR_EXIT(dnx_mpls_egress_lif_ttl_qos_profiles_sw_allocate
                    (unit, is_replace,
                     DNX_OUT_LIF_PROFILE_DEFAULT_ORIENTATION, DBAL_TABLE_EEDB_MPLS_TUNNEL, &encap_info_orig,
                     &encap_info, &ttl_pipe_profile_first_reference, &ttl_pipe_profile_last_reference,
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags2, BCM_MPLS_EGRESS_LABEL_FLAGS2_SVTAG_ENABLE))
    {
        ...
        int old_esem_profile = _SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_REPLACE) ?
            encap_info_orig.esem_command_profile : dnx_data_esem.access_cmd.no_action_get(unit);
    ...
    if (!_SHR_IS_FLAG_SET(encap_info.label_array[0].flags, BCM_MPLS_EGRESS_LABEL_ALTERNATE_MARKING) || !is_replace)
    {
        SHR_IF_ERR_EXIT(egress_additional_headers_profile_last_clear(unit, &headers_profile_mngr_info));
    }
    ...
    bcm_mpls_egress_label_t * label_array)
{
     ...
     * Verification
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_initiator_create_php_verify(unit, label_array));
    ...
    /** calculate lif additional_headers_profile*/
    SHR_IF_ERR_EXIT(dnx_mpls_encap_additional_header_profile_alloc
                    (unit, is_replace, NULL, &encap_info, &headers_profile_mngr_info));
    ...
    int push_profile)
{
    ...
    dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                 DBAL_RESULT_TYPE_JR1_MPLS_PUSH_COMMAND_ETPS_MPLS_1_STAT);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LIF_AHP_MPLS, INST_SINGLE,
                                 encap_info->lif_additional_headers_profile);
    /*
    ...
    int *push_cmd)
{
    ...
    /** Allocate the QoS/TTL profile*/
    if (is_first_push_profile_reference || is_last_push_profile_reference)
    {
        /** add  mpls labels to be pushed */
        dnx_mpls_encap_t_init(unit, &encap_info);
        ...
        SHR_IF_ERR_EXIT(dnx_mpls_egress_lif_ttl_qos_profiles_sw_allocate
                        (unit, is_replace, DNX_OUT_LIF_PROFILE_DEFAULT_ORIENTATION, DBAL_TABLE_EEDB_MPLS_TUNNEL,
                         ...
                         &qos_pipe_profile_last_reference, &out_lif_profile_last_reference));
        ...
        if (qos_pipe_profile_first_reference)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_set
                            (unit, encap_info.nwk_qos_id, nwk_qos, nwk_qos, 0));
        }
    ...
    int push_cmd)
{
    ...
    if (is_last_push_profile_reference)
    {
        ...
        /** Free and clear the additional_headers_profile*/
    ...
    void *user_data)
{
     ...
     * Verification.
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_initiator_traverse_verify(unit, additional_info, cb, user_data));
    ...
    bcm_mpls_egress_label_t * label_array)
{
     ...
     * Verification.
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_initiator_create_verify(unit, intf, num_labels, label_array));
    ...
    {
        {
            if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_ACTION_VALID)
                && (label_array[0].action == BCM_MPLS_EGRESS_ACTION_PHP))
            {
                SHR_IF_ERR_EXIT(dnx_mpls_tunnel_initiator_create_php(unit, num_labels, label_array));
            }
            else
            {
    ...
    int *label_count)
{
     ...
     * Verification.
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_initiator_get_verify(unit, intf, label_max, label_array, label_count));
    ...
    else
    {
        ...
        *label_count = encap_info.labels_nof;
        ...
        {
            BCM_FAILOVER_SET(encap_info.label_array[0].egress_failover_id,
        ...
        SHR_IF_ERR_EXIT(dnx_lif_lib_logical_phase_to_encap_access_convert
                        (unit, gport_hw_resources.logical_phase, &label_array[0].encap_access));
        /*
         * Whenever the returned value is the same as the Invalid, the Invalid value is returned
         */
        if (label_array[0].encap_access == bcmEncapAccessTunnel2)
        {
            label_array[0].encap_access = bcmEncapAccessInvalid;
        }
        ...
        if (gport_hw_resources.global_out_lif != DNX_ALGO_GPM_LIF_INVALID)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_global_out_lif_alloc_info_get(unit, gport_hw_resources.global_out_lif,
                                                                   &alloc_info));
        ...
        if (encap_info.esem_command_profile != dnx_data_esem.access_cmd.no_action_get(unit))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_switch_svtag_is_present_in_esem_cmd
                            (unit, encap_info.esem_command_profile, &app_db_id, &svtag_present));
    ...
    bcm_if_t intf)
{
    ...
    uint32 lif_flags;
    bcm_gport_t gport;
    int new_out_lif_profile = 0, rv = _SHR_E_NONE;
    etpp_out_lif_profile_info_t out_lif_profile_info;
    dnx_mpls_encap_t encap_info;
     ...
     * Verification.
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_initiator_clear_verify(unit, intf));
    /*
    ...
    else
    {
        ...
        rv = dbal_tables_field_info_get_no_err(unit, DBAL_TABLE_EEDB_MPLS_TUNNEL, DBAL_FIELD_OUT_LIF_PROFILE,
                                               FALSE, gport_hw_resources.outlif_dbal_result_type, INST_SINGLE,
                                               &field_info);
        if (rv == _SHR_E_NONE)
        {
            /*
            ...
            if (new_out_lif_profile != DNX_OUT_LIF_PROFILE_DEFAULT_MPLS_PROFILE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Error when freeing out_lif_profile! It didn't get the correct default out_lif_profile, expected = %d, new_out_lif_profile = %d",
                             DNX_OUT_LIF_PROFILE_DEFAULT_MPLS_PROFILE, new_out_lif_profile);
            }
            if (is_last)
            {
                /** Clear the old ETPP outlif profile in case there are no more references to it*/
