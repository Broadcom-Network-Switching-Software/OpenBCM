    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    info->class_id = pmport->class_id;
    info->qos_map_id = pmport->qos_map_id;
    sal_memcpy(&info->ingress_qos_model, &pmport->ingress_qos_model, sizeof(bcm_qos_ingress_model_t));
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_EVPN_IML))
    {
        ...
        if (support_mpls_term_till_bos)
        {
            ...
            if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_ENTROPY_ENABLE))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "BCM_MPLS_SWITCH_ENTROPY_ENABLE is not needed for IML termination. It is always terminated together with IML!\n");
            }
        }
        else if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_ENTROPY_ENABLE))
        {
            /** Check if the entropy was enabled properly.*/
            int enabler = 0;
            SHR_IF_ERR_EXIT(bcm_switch_control_get(unit, bcmSwitchMplsEvpnEntropyEnable, &enabler));
            if (!(enabler & MPLS_EVPN_IML_FL_ENABLE))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Entropy label was not enabled in EVPN. It can be enabled by bcmSwitchMplsEvpnEntropyEnable!\n");
            }

            if (_SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_CONTROL_WORD)
                && !(enabler & MPLS_EVPN_IML_FL_WITH_CW_ENABLE))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Entropy label with Control Word was not enabled in EVPN. It can be enabled by bcmSwitchMplsEvpnEntropyEnable!\n");
            }
        }
    }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_evpn_term_add_verify(unit, info));
    ...
    uint8 *is_evpn)
{
    ...
    sal_memset(&match, 0, sizeof(dnx_mpls_term_match_t));
    *local_in_lif = 0;
    *is_evpn = FALSE;

    {
        ...
        if (info != NULL)
        {
            ...
            if (is_iml && !dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_mpls_term_till_bos))
            {
                ...
                match.vmpi = info->class_id;
                match.bos_val = _SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_EXPECT_BOS) ? 1 : 0;
            }
            else
            {
                /** Non-IML label or feature mpls_term_till_bos is supported. Can still be EVPN. */
        ...
        SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                        (unit, *local_in_lif, dnx_drv_soc_core_default(), physical_table_id, &dbal_table_id,
                         &dbal_result_type));
...
static const uint32 egress_label_flags2_mpls_port_flags2_pairs[][2] = {
    {BCM_MPLS_EGRESS_LABEL_FLAGS2_NO_TPID_CLASS, BCM_MPLS_PORT2_NO_TPID_CLASS}
};
    ...
    dnx_mpls_l2vpn_info_t * l2vpn)
{
    ...
    for (pair_i = 0; pair_i < sizeof(egress_label_flags2_mpls_port_flags2_pairs) / (2 * sizeof(uint32)); ++pair_i)
    {
        l2vpn->mpls_port->flags2 |=
            _SHR_IS_FLAG_SET(eg_label->flags2, egress_label_flags2_mpls_port_flags2_pairs[pair_i][0]) ?
            egress_label_flags2_mpls_port_flags2_pairs[pair_i][1] : 0;
    }
    /*
    ...
    bcm_mpls_egress_label_t * eg_label)
{
    ...
    {
        eg_label->flags2 |=
            _SHR_IS_FLAG_SET(l2vpn->mpls_port->flags2, egress_label_flags2_mpls_port_flags2_pairs[pair_i][1]) ?
            egress_label_flags2_mpls_port_flags2_pairs[pair_i][0] : 0;
    }
    ...
    dnx_mpls_l2vpn_info_t * l2vpn)
{
    ...
    pmport->egress_label.flags2 = eg_label->flags2;
    ...
    pmport->egress_label.estimated_encap_size = eg_label->estimated_encap_size;
    ...
    bcm_mpls_egress_label_t * eg_label)
{
    ...
    eg_label->flags2 |= pmport->egress_label.flags2;
    ...
    eg_label->estimated_encap_size = pmport->egress_label.estimated_encap_size;
    ...
    bcm_mpls_egress_label_t * eg_label)
{
    ...
    if (_SHR_IS_FLAG_SET(eg_label->flags2, BCM_MPLS_EGRESS_LABEL_FLAGS2_SVTAG_ENABLE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported flag set for EVPN/IML"
                     "Flag: 0x%08x\n", BCM_MPLS_EGRESS_LABEL_FLAGS2_SVTAG_ENABLE);
    }
    ...
    bcm_mpls_egress_label_t * eg_label)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_evpn_encap_add_verify(unit, eg_label));
    ...
    int *label_count)
{
    ...
    l2vpn.mpls_port->flags2 |= BCM_MPLS_PORT2_EGRESS_ONLY;
    l2vpn.mpls_port->egress_label.estimated_encap_size = eg_label->estimated_encap_size;
