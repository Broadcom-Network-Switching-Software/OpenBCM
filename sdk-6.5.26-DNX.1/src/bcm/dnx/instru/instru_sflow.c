    ...
    bcm_instru_sflow_encap_extended_src_info_t * sflow_encap_ext_src_info)
{
    ...
    if (!dnx_data_instru.sflow.feature_get(unit, dnx_data_instru_sflow_hard_logic_supported))
    {
        if (sflow_encap_ext_src_info->sflow_encap_id != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Error! sflow_encap_id = 0x%08x is only relevant for sFlow hard-logic! Please set it to 0!\n",
                         sflow_encap_ext_src_info->sflow_encap_id);
        }
    }
