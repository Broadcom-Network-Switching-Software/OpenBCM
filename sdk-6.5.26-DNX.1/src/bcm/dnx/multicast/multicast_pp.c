...
/** Utility macro for getting only the valid DBAL table result type by result type input
 * Currently we enable only double or triple outlifs for multicast */
#define MULTICAST_DBAL_RESULT_TYPE_BY_ENCAP_EXTENTION(encap_extension_count) \
        (encap_extension_count == 2) ? \
                   DBAL_RESULT_TYPE_PPMC_CUD_PPMC_DOUBLE_OUTLIF : \
                    (encap_extension_count == 3) ? \
                        DBAL_RESULT_TYPE_PPMC_CUD_PPMC_TRIPLE_OUTLIF : -1;

/** Utility macro for getting encap extention count by DBAL table result type
 * Currently we enable only double or triple outlifs for multicast */
#define MULTICAST_ENCAP_EXTENTION_BY_DBAL_RESULT_TYPE(dbal_result_type) \
        (dbal_result_type == DBAL_RESULT_TYPE_PPMC_CUD_PPMC_TRIPLE_OUTLIF) ? \
                  3 : (dbal_result_type == DBAL_RESULT_TYPE_PPMC_CUD_PPMC_DOUBLE_OUTLIF) ? \
                        2 : -1;
    ...
    bcm_if_t * encap_extension_array)
{
    ...
    SHR_NULL_CHECK(multicast_replication_index, _SHR_E_PARAM, "multicast_replication_index");
    /*
    ...
    bcm_if_t * encap_extension_array)
{
    ...
    /**Add the entry*/
        ...
        SHR_IF_ERR_EXIT(dnx_field_ace_entry_add
                        (unit, ace_flags, ace_id[encap_extension_count - 1], &payload, &ace_key));
    ...
    bcm_if_t * encap_id)
{
    ...
    bcm_gport_t gport = vxlan_port_id;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, vxlan_port_id, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
                         &gport));
    }
