...
#define DNX_MULTICAST_HASH_KEY_DEST_ELEMENT_DYNAMIC 0x4
#define DNX_MULTICAST_HASH_KEY_DEST_ELEMENT_STATIC 0x5
/*
 *  }
 */
...
static shr_error_e
dnx_multicast_hash_key_dest_and_type_get(
    int unit,
    bcm_gport_t dest_gport,
    uint32 dest_converted,
    uint32 *destination,
    uint8 *destination_type)
{
    ...
    else if (BCM_MCAST_GPORT_IS_ELEMENT_ID(dest_gport))
    {
        *destination = BCM_MCAST_GPORT_ELEMENT_ID_GET(dest_gport);
        if (BCM_MCAST_GPORT_IS_ELEMENT_STATIC(dest_gport))
        {
            *destination_type = DNX_MULTICAST_HASH_KEY_DEST_ELEMENT_STATIC;
        }
        else
        {
            *destination_type = DNX_MULTICAST_HASH_KEY_DEST_ELEMENT_DYNAMIC;
        }
    }
 ...
 *   \param [in] dest2_gport - Second Destination Gport
 *   \param [in] entry_val - Entry value
    ...
    mcdb_hash_table_key_t * hash_key)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(key_temp,
                                (sizeof(uint32) *
                                 BYTES2WORDS(DNX_DATA_MAX_MULTICAST_MCDB_HASH_TABLE_MAX_KEY_SIZE_IN_WORDS)), "key_temp",
                                "%s%s%s", EMPTY, EMPTY, EMPTY);

    /** Determine the destination and type */
    SHR_IF_ERR_EXIT(dnx_multicast_hash_key_dest_and_type_get
                    (unit, dest_gport, entry_val.dest[0], &destination, &destination_type));

    /** Set the common field of the key */
    SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                    (key_temp, dnx_data_multicast.mcdb_hash_table.destination_start_bit_get(unit),
                     dnx_data_multicast.mcdb_hash_table.destination_nof_bits_get(unit), destination));
    SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                    (key_temp, dnx_data_multicast.mcdb_hash_table.reference_num_start_bit_get(unit),
                     dnx_data_multicast.mcdb_hash_table.reference_num_nof_bits_get(unit), (uint32) ref_num));
    SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                    (key_temp, dnx_data_multicast.mcdb_hash_table.destination_type_start_bit_get(unit),
                     dnx_data_multicast.mcdb_hash_table.destination_type_nof_bits_get(unit), destination_type));
    SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                    (key_temp, dnx_data_multicast.mcdb_hash_table.group_id_start_bit_get(unit),
                     dnx_data_multicast.mcdb_hash_table.group_id_nof_bits_get(unit), (uint32) group_id));

    {
        /** Set V1 specific key fields */
        SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                        (key_temp, dnx_data_multicast.mcdb_hash_table.is_ingress_start_bit_get(unit),
                         dnx_data_multicast.mcdb_hash_table.is_ingress_nof_bits_get(unit), is_ingress));
        SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                        (key_temp, dnx_data_multicast.mcdb_hash_table.info_start_bit_get(unit),
                         dnx_data_multicast.mcdb_hash_table.info_nof_bits_get(unit), entry_val.cud));
        SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                        (key_temp, dnx_data_multicast.mcdb_hash_table.core_id_start_bit_get(unit),
                         dnx_data_multicast.mcdb_hash_table.core_id_nof_bits_get(unit), core_id));
    }
    ...
    *last_reference = 0;
    ...
    for (ref_num = 0; ref_num < DNX_MULTICAST_MAX_NOF_REF_PER_REP; ref_num++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_multicast_hash_key_construct
                        (unit, group_id, is_ingress, core_id, dest_gport, dest2_gport, entry_val, ref_num,
                         &hash_key_temp));
    ...
    uint8 *found)
{
    ...
    *found = FALSE;
    sal_memset(hash_key, 0, sizeof(*hash_key));
    /** Go over the references to find the reference that is previous the provided MCDB entry ID */
    for (ref_num = 0; ref_num < DNX_MULTICAST_MAX_NOF_REF_PER_REP; ref_num++)
    {
        found_temp = 0;
        SHR_IF_ERR_EXIT(dnx_multicast_hash_key_construct
 ...
 */
static shr_error_e
dnx_multicast_egress_bitmap_iter(
    int unit,
    uint32 bitmap_ptr,
    uint32 cud,
    ...
    uint32 *bmp)
{
    ...
    while (bitmap_ptr != DNX_MULTICAST_END_OF_LIST_PTR)
    {
        ...
        /** Need to check if the previous entry is the BMP_PTR */
        ...
        switch (action)
        {
            case DNX_MULTICAST_ENTRY_ITER_REMOVE_LIST:
                ...
                /** If the entry is empty it can be removed from the link list, unless it is the first bitmap of the group.
                 * The first bitmap is not removed, because a BMP_PTR must always point to a Bitmap */
                if ((entry_val.bmp_val != 0)
                ...
                else
                {
                    SHR_IF_ERR_EXIT(dnx_multicast_entry_remove
                                    (unit, bitmap_ptr, entry_val.prev_entry, entry_val.next_entry_id, TRUE,
                                     &cur_entry_replaced, &entry_to_move));
                }
                break;
 ...
 */
static shr_error_e
dnx_multicast_egress_bitmap_delete(
    int unit,
    ...
    bcm_multicast_replication_t * rep_array)
{
    ...
    {
        SHR_ALLOC_ERR_EXIT(bmp[core_id],
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_egress_bitmap_reps_to_bmp(unit, nof_replications, rep_array, bmp, bmp_cud));
    }
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry
                        (unit, group, TRUE, DNX_MULTICAST_IS_INGRESS(flags), FALSE, core_id, &entry_id));
        ...
        if (entry_val.next_entry_id != DNX_MULTICAST_END_OF_LIST_PTR)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_multicast_egress_bitmap_iter
                            (unit, entry_val.bmp_ptr, entry_val.cud, action, bmp[core_id]));
        }
    }
 ...
 */
static shr_error_e
dnx_multicast_egress_bitmap_get(
    int unit,
    bcm_multicast_t group,
    uint32 flags,
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        if (!DNX_MULTICAST_IS_EMPTY_BMP_GROUP(entry_val))
        {
            if (max_replications != 0)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_multicast_egress_bitmap_bmp_to_reps(unit, entry_val.cud, core_id, bmp,
 ...
 */
static shr_error_e
dnx_multicast_egress_bitmap_create(
    int unit,
    bcm_multicast_t group,
    uint32 flags,
    uint32 is_set,
    ...
    bcm_multicast_replication_t * rep_array)
{
    ...
    int ext_mode;
    uint8 first_bitmap_allocation = FALSE;
    ...
    SHR_IF_ERR_EXIT(dnx_multicast_egress_bitmap_reps_to_bmp(unit, rep_count, rep_array, bmp_new_reps, bmp_cud));
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        /** Get the existing bitmap */
        SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry
                        (unit, group, TRUE, DNX_MULTICAST_IS_INGRESS(flags), FALSE, core_id,
                         &bitmap_first_entry_id[core_id]));
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_entry_get(unit, bitmap_first_entry_id[core_id], &entry_val));
        /** If BMP_PTR is empty, it means that the group was just created and an empty bitmap needs to be allocat3ed to it */
        if (entry_val.bmp_ptr == DNX_MULTICAST_END_OF_LIST_PTR)
        {
            first_bitmap_allocation = TRUE;
        }

        /** remove the entries user wish to remove. relevant for set API only */
        if (is_set)
        {
            SHR_IF_ERR_EXIT(dnx_multicast_egress_bitmap_iter(unit, entry_val.bmp_ptr, entry_val.cud,
        ...
        for (bmp_entry_id = 0; bmp_entry_id < DNX_MULTICAST_BMP_NOF_ENTRIES_PER_DEV(unit); bmp_entry_id++)
        {
            if (bmp_new_reps[core_id][bmp_entry_id] || (first_bitmap_allocation == TRUE))
            {
                ...
                /** Mark that the first bitmap was allocated.
                 * After that allocate only in case reolications exist */
                first_bitmap_allocation = FALSE;
            }
        }
 ...
 */
static shr_error_e
dnx_multicast_v1_linklist_create(
    int unit,
    bcm_multicast_t group,
    uint32 flags,
    uint32 is_set,
    ...
    bcm_multicast_replication_t * rep_array)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry
                        (unit, group, FALSE, DNX_MULTICAST_IS_INGRESS(flags),
                         DNX_MULTICAST_IS_MC_ELEMENT_DYNAMIC(flags), core_id, &group_first_entry_id[core_id]));
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        for (rep_id = 0; rep_id < rep_count; rep_id++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_multicast_hash_last_reference_get
                            (unit, group, DNX_MULTICAST_IS_INGRESS_GROUP(flags),
                             core_id, rep_array[rep_id].port, rep_array[rep_id].port2, entry_values[core_id][rep_id],
                             &last_reference));
            ...
            SHR_IF_ERR_EXIT(dnx_multicast_hash_key_construct
                            (unit, group, DNX_MULTICAST_IS_INGRESS_GROUP(flags), core_id, rep_array[rep_id].port,
                             rep_array[rep_id].port2, entry_values[core_id][rep_id], first_free_reference, &hash_key));
            ...
            if (nof_reps_at_entry == 2)
            {
                SHR_IF_ERR_EXIT(dnx_multicast_hash_last_reference_get
                                (unit, group, DNX_MULTICAST_IS_INGRESS_GROUP(flags), core_id,
                                 rep_array[rep_id + 1].port, rep_array[rep_id + 1].port2,
                                 entry_values[core_id][rep_id + 1], &last_reference));
                /** Return the hash key and table index of the first free reference */
                first_free_reference = last_reference + 1;
                if (first_free_reference >= DNX_MULTICAST_MAX_NOF_REF_PER_REP)
                {
                ...
                SHR_IF_ERR_EXIT(dnx_multicast_hash_key_construct
                                (unit, group, DNX_MULTICAST_IS_INGRESS_GROUP(flags), core_id,
                                 rep_array[rep_id + 1].port, rep_array[rep_id + 1].port2,
                                 entry_values[core_id][rep_id + 1], first_free_reference, &hash_key));
                SHR_IF_ERR_EXIT(multicast_db.key_2_mcdb_id_hash.insert(unit, &hash_key, &cur_entry_id, &success));
                if (!success)
                {
                    SHR_ERR_EXIT(_SHR_E_FULL,
    ...
    bcm_multicast_replication_t * rep_array)
{
    SHR_FUNC_INIT_VARS(unit);
    {
        SHR_IF_ERR_EXIT(dnx_multicast_egress_bitmap_delete(unit, group, flags, action, nof_replications, rep_array));
    }
    ...
    int *rep_count)
{
    SHR_FUNC_INIT_VARS(unit);
    {
        SHR_IF_ERR_EXIT(dnx_multicast_egress_bitmap_get(unit, group, flags, max_replications, rep_array, rep_count));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_multicast_replication_t * rep_array)
{
    SHR_FUNC_INIT_VARS(unit);
    {
        SHR_IF_ERR_EXIT(dnx_multicast_egress_bitmap_create(unit, group, flags, is_set, rep_count, rep_array));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 new_entry_id)
{
    ...
    uint32 is_open = 0;
    uint8 is_ingress = 0;
    uint8 skip_hash_table_error = FALSE;
    SHR_FUNC_INIT_VARS(unit);
...

    {
        ...
        if (entry_val.format == DBAL_RESULT_TYPE_MCDB_LINK_LIST_FORMAT)
        {
            /** For Link List format entry the CUD value is not relevant,
             * so it is not written in HW. However, an invalid CUD value that is 
             * received from the convert function is used in the Hash table, so the struct myst be updated */
    ...
    if (is_open)
    {
        SHR_IF_ERR_EXIT(dnx_multicast_hash_mcdb_hash_key_get
                        (unit, group_id, is_ingress, dest_core_id, dest1, dest2, entry_val, old_entry_id,
                         &hash_key, &found));
    }

    {
        /** The entry is the first entry of a Bitmap group, no need to update hash table */
        skip_hash_table_error = (entry_val.format == DBAL_RESULT_TYPE_MCDB_BITMAP_PTR_FORMAT) ? TRUE : FALSE;
    }

    if (found == FALSE)
    {
        if (skip_hash_table_error == TRUE)
        {
            /** Do not return error, entry does not exist in hash table. */
            SHR_EXIT();
        }
        else
        {
    ...
    /** For Multicast V1 implementation each destination is handled separately in the Hash table */
    if ((dest2 != BCM_GPORT_INVALID)
        && (!dnx_data_multicast.params.feature_get(unit, dnx_data_multicast_params_mcdb_formats_v2)))
    {
        /** Remove also the second entry from the hash table. Key function is using only dest[0] in the key creation,
         * so the second destination must be set in field dest[0] in order to generate a key for it */
        entry_val.dest[0] = entry_val.dest[1];
        SHR_IF_ERR_EXIT(dnx_multicast_hash_mcdb_hash_key_get
                        (unit, group_id, is_ingress, dest_core_id, dest2, BCM_GPORT_INVALID, entry_val, old_entry_id,
                         &hash_key, &found));
        if (found == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "MCDB entry %d not found in hash table! \n", old_entry_id);
    ...
    bcm_multicast_replication_t * rep_array)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        for (rep_id = 0; rep_id < rep_count_in; rep_id++)
        {
            ...
            {
                /** Convert replication's destinations to HW */
                SHR_IF_ERR_EXIT(dnx_multicast_convert_gport_to_hw
                                (unit, rep_array[rep_id].port, rep_array[rep_id].encap1, flags,
                                 core_id, &rep_entry_val, &is_belong_to_core, DNX_MULTICAST_TRUNK_ACTION_DESTROY));
            ...
            SHR_IF_ERR_EXIT(dnx_multicast_hash_last_reference_get
                            (unit, group,
                             (DNX_MULTICAST_IS_INGRESS_GROUP(flags)
                              && (!DNX_MULTICAST_IS_MC_ELEMENT_DYNAMIC(flags))), core_id, rep_array[rep_id].port,
                             rep_array[rep_id].port2, rep_entry_val, &last_reference));
            /** Return the hash key and table index of the last used reference */
            SHR_IF_ERR_EXIT(dnx_multicast_hash_key_construct
                            (unit, group,
                             (DNX_MULTICAST_IS_INGRESS_GROUP(flags)
                              && (!DNX_MULTICAST_IS_MC_ELEMENT_DYNAMIC(flags))), core_id, rep_array[rep_id].port,
                             rep_array[rep_id].port2, rep_entry_val, last_reference, &hash_key));
            ...
            if (cur_entry_replaced == TRUE)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_multicast_hash_table_entry_relocate(unit, entry_to_move, cur_entry_id));
            }
        }
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        else
        {
            SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry(unit, group, FALSE, DNX_MULTICAST_IS_INGRESS(flags),
                                                      DNX_MULTICAST_IS_MC_ELEMENT_DYNAMIC(flags), core_id,
                                                      &cur_entry_id));
        }
        ...
        while (cur_entry_id != DNX_MULTICAST_END_OF_LIST_PTR)
        {
            ...
            switch (action)
            {
                case DNX_MULTICAST_ENTRY_ITER_GET:
                    ...
                    {
                        /** Convert entry to Gport */
                        SHR_IF_ERR_EXIT(dnx_multicast_convert_hw_to_gport
                                        (unit, entry_val, core_id, &dest1, &dest2, &cud));
                ...
                case DNX_MULTICAST_ENTRY_ITER_REMOVE_ALL:
                {
                    ...
                    /** Remove the entry from the hash table mapping unique destinations to MCDB Index */
                    SHR_IF_ERR_EXIT(dnx_multicast_hash_mcdb_hash_key_get
                                    (unit, group, DNX_MULTICAST_IS_INGRESS_GROUP(flags), core_id, dest1, dest2,
                                     entry_val, cur_entry_id, &hash_key, &found));
                    SHR_IF_ERR_EXIT(multicast_db.key_2_mcdb_id_hash.delete(unit, &hash_key));
                        ...
                        && (!dnx_data_multicast.params.feature_get(unit, dnx_data_multicast_params_mcdb_formats_v2)))
                    {
                        ...
                        entry_val.dest[0] = entry_val.dest[1];
                        SHR_IF_ERR_EXIT(dnx_multicast_hash_mcdb_hash_key_get
                                        (unit, group, DNX_MULTICAST_IS_INGRESS_GROUP(flags), core_id, dest2,
                                         BCM_GPORT_INVALID, entry_val, cur_entry_id, &hash_key, &found));
                        SHR_IF_ERR_EXIT(multicast_db.key_2_mcdb_id_hash.delete(unit, &hash_key));
                    }
    ...
    bcm_multicast_replication_t * rep_array)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_v1_linklist_create(unit, group, flags, is_set, rep_count, rep_array));
    }
    ...
    bcm_multicast_replication_t * rep_array)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry(unit, group, FALSE, FALSE, FALSE, core_id, &cur_entry_id));
    ...
    uint32 *is_open)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_v1_group_open_hw_get(unit, group, flags, is_open));
    }
    ...
    uint32 is_open)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_v1_group_open_hw_set(unit, group, flags, is_open));
    }
    ...
    dnx_multicast_mcdb_dbal_field_t * entry_val)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_v1_entry_get(unit, entry_id, entry_val));
    }
    ...
    dnx_multicast_mcdb_dbal_field_t * entry_val)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_multicast_v1_entry_set(unit, entry_id, entry_val));
    }
...
/**
 * \brief - Power down the MTM module
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit -  Unit-ID
    ...
    int power_down)
{
   ...
   /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_MULTICAST_SETTINGS, &entry_handle_id));
    ...
    int unit)
{
    ...
    if (!dnx_data_multicast.general.feature_get(unit, dnx_data_multicast_general_is_used))
    {
        if (dnx_data_multicast.general.feature_get(unit, dnx_data_multicast_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_multicast_power_down_set(unit, 1));
        }
    }
    else
    {
         ...
         * Get BIER size
         */
        bier_size = dnx_data_bier.params.bier_mcdb_size_get(unit);

        /*
         * For V2 MC implementation the below check is not valid. The IDs pool is shared and
         * the user does not set the ranges
         */
