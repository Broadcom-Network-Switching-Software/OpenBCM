    ...
    uint32 *extra_enc_hdr_len)
{
    ...
    uint8 tc, dp, qos, bos = 1;
    uint32 gal, fwd_dst_info;
    int gach;
    uint16 ether_type = 0x8902;
    dbal_field_types_defs_e destination_field_type;
    ...
    if (next_is_itmh)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_oamp_itmh_priority_profile_get(unit, mep_db_entry->itmh_tc_dp_profile, &tc, &dp, &qos));
        ...
        destination_field_type =
            is_jericho_mode ? DBAL_FIELD_TYPE_DEF_DESTINATION_JR1 : DBAL_FIELD_TYPE_DEF_DESTINATION;
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_parent_field32_value_set(unit, destination_field_type,
                                                                        port_id_field, &_sys_phy_port_id,
                                                                        &fwd_dst_info));
    ...
    bcm_oam_tst_tx_t * tst_ptr)
{
    ...
    SHR_ALLOC_ERR_EXIT(p_config, sizeof(bcm_sat_gtf_packet_config_t),
                       "bcm_petra_oam_tst_tx_add", "%s%s%s", "p_config", EMPTY, EMPTY);
