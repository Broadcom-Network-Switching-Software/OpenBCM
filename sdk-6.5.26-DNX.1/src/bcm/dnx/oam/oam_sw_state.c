    ...
    int *ll_index)
{
...

    {
         ...
         */
        if (endpoint_id >= mep_db_threshold)
        {
            *ll_index = mep_db_threshold + endpoint_id / 4;
        }
        else
        {
    ...
    dnx_oam_lif_info_t * lif_info)
{
    ...
    if (DNX_OAM_LIF_IS_LOCAL_PORT(oam_lif))
    {
        is_lif = 0;
        /** Set OAM LIF to PTC */
        SHR_IF_ERR_EXIT(dnx_port_ptc_get(unit, BCM_GPORT_LOCAL_GET(oam_lif), &ptc, &core_id));
        oam_lif_tmp = ptc;
    }
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_LIF, is_lif);
...
/**
 * \brief - This function deletes oam-lif info to the OAM SW state
 *        LIF table
 *
 * \param [in] unit - Number of hardware unit used.
 ...
 */
shr_error_e
dnx_oam_sw_state_oam_lif_info_delete(
    int unit,
    uint32 oam_lif,
    uint8 is_ingress)
{
    ...
    rv = dnx_oam_sw_state_oam_lif_info_get(unit, oam_lif, is_ingress, &oam_lif_info);
    /** Entry might never been set */
    SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);

    sal_memset(&field_info, 0, sizeof(field_info));
    SHR_IF_ERR_EXIT(dbal_tables_field_info_get
                    (unit, DBAL_TABLE_OAM_LIF_INFO_SW, DBAL_FIELD_OAM_ENDPOINT_ID, FALSE, 0, 0, &field_info));
    ...
    /** In case the entry is found and not used by MEP, remove it*/
    if (rv != _SHR_E_NOT_FOUND)
    {
        for (mdl = 0; mdl <= DNX_OAM_MAX_MDL; mdl++)
        {
            if (oam_lif_info.endpoint_id[mdl] != (field_info.max_value - 1))
            {
                is_lif_used = 1;
            }
        }
        /** Can not delete it if mep created on this lif, otherwise, it will cause mep's error*/
        if (is_lif_used)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "please delete the mep fisrtly before you delete oam lif sw entry\r\n");
        }
    ...
    dnx_oam_endpoint_info_t * sw_endpoint_info)
{
    ...
    dbal_value_field16_request(unit, entry_handle_id, DBAL_FIELD_DUPLICATED_MEP_ID, INST_SINGLE,
                               &sw_endpoint_info->duplicated_mep_id);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_SERVER_DEVICE_INDEX, INST_SINGLE,
                              &sw_endpoint_info->server_device_index);
    ...
    uint8 is_update)
{
    ...
    dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_DUPLICATED_MEP_ID, INST_SINGLE,
                                 sw_endpoint_info->duplicated_mep_id);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_SERVER_DEVICE_INDEX, INST_SINGLE,
                                sw_endpoint_info->server_device_index);
    ...
    bcm_oam_endpoint_t endpoint_id)
{
    ...
    SHR_ALLOC_ERR_EXIT(rmep_id_list, num_rmeps_in_list * sizeof(bcm_oam_endpoint_t),
                       "oam event fifo", "%s%s%s", "rmep_id_list", EMPTY, EMPTY);
    SHR_IF_ERR_EXIT(oam_sw_db_info.oam_endpoint_sw_db_info.
                    oam_endpoint_array_of_rmep_linked_lists.get_first(unit, ll_index, &node));
