    ...
    bcm_oam_profile_t egress_oam_action_profile)
{
    ...
    else
    {
       ...
       /** Get Gport information */
        if (!DNX_OAM_LIF_IS_LOCAL_PORT(gport))
        {
    ...
    bcm_oam_action_result_t * oam_action_result)
{
    ...
    uint8 is_level_or_passive_trap = 0, is_valid_trap = 0, dummy = 0;
    ...
    is_valid_trap = (BCM_GPORT_TRAP_GET_ID(oam_action_result->destination) > 0) ? TRUE : FALSE;

    /** Set sub-type to NULL for inject passive CCM without piggy-back */
    if (!is_valid_trap && is_ingress && action_key.is_inject &&
        (oam_action_key->endpoint_type == bcmOAMMatchTypePassive) &&
        (lm_type != DBAL_ENUM_FVAL_LM_MODE_LM_MODE_IS_DUAL_ENDED) &&
        (action_key.oam_internal_opcode == DBAL_ENUM_FVAL_OAM_INTERNAL_OPCODE_ETH_CCM))
    {
        action_content.oam_sub_type = DBAL_ENUM_FVAL_OAM_SUB_TYPE_NULL;
    }
        ...
        for (mp_type = 0; mp_type < DBAL_NOF_ENUM_MP_TYPE_VALUES; mp_type++)
        {
            ...
            /** Calculate counter disable values based on mp_type */
                    ...
                    && (action_key.is_inject == 0))
                {
                    ...
                    if (is_ingress && action_key.is_inject && (lm_type != DBAL_ENUM_FVAL_LM_MODE_LM_MODE_IS_DUAL_ENDED)
                        && (action_key.oam_internal_opcode == DBAL_ENUM_FVAL_OAM_INTERNAL_OPCODE_ETH_CCM))
                    {
                        action_content.oam_sub_type = DBAL_ENUM_FVAL_OAM_SUB_TYPE_NULL;
                    }
