    ...
    int is_virtual_ac)
{
    ...
    supported_flags2 = BCM_VLAN_PORT_FLAGS2_OAM_TRUNK_MEMBER | BCM_VLAN_PORT_FLAGS2_MAC_SAV;
    ...
    int *is_p2p)
{
    ...
    uint32 global_inlif_pmf_data = 0;
    ...
    *is_p2p = FALSE;
    ...
    if (has_global_lif)
    {
        ...
        if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_RECYCLE) &&
            !DNX_L2_IS_LARGE_ENTRY_GROUP(vlan_port->group))
        {
    ...
    else
    {
        if (has_global_lif)
        {
            global_inlif_pmf_data = (ingress_ac_table_info->global_lif << DNX_PORT_PP_PMF_ADDITIONAL_DATA_SIZE);
        }
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOBAL_IN_LIF_AND_SOME_PMF_DATA, INST_SINGLE,
                                     global_inlif_pmf_data);
    }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    int indexed_mode_enable = 0, native_indexed_mode = 0, lif_on_lag_is_shown = 0;
    ...
    if (!is_replace)
    {
        SHR_IF_ERR_EXIT(dnx_ingress_inlif_profile_alloc
                        (unit, vlan_port->ingress_network_group_id, lif_on_lag_is_shown, native_indexed_mode,
                         DNX_IN_LIF_PROFILE_DEFAULT, &(ingress_ac_table_info->in_lif_profile),
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_inlif_profile_alloc
                        (unit, vlan_port->ingress_network_group_id, lif_on_lag_is_shown, native_indexed_mode,
                         old_in_lif_profile, &(ingress_ac_table_info->in_lif_profile),
                         ingress_ac_table_info->dbal_table_id, &last_profile));
    }
