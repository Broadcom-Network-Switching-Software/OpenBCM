    ...
    uint32 buffer)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "%s(): Trying to restore BDB %d in core %d\n"), FUNCTION_NAME(), buffer, core));
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "%s(): Successfully restored BDB %d in core %d\n"), FUNCTION_NAME(), unit, core));
    ...
    uint32 buffer)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "%s(): Schedule BDB %d in core %d to be quarantined\n"), FUNCTION_NAME(), buffer,
                 core));
    ...
    bcm_core_t core)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "%s(): BDB in core %d was successfully quarantined\n"), FUNCTION_NAME(), core));
    ...
    uint32 buffer_identifier)
{
    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_is_delete_bdb_supported))
    {
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "%s(): Attempting to delete BDB %d in core %d to be quarantined\n"),
                     FUNCTION_NAME(), buffer_identifier, core));
        ...
        {
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit, "%s(): Successfully deleted BDB %d in core %d to be quarantined\n"),
                         FUNCTION_NAME(), buffer_identifier, core));
        }
    }
exit:
