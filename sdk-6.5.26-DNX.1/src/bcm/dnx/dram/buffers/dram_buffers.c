...
static shr_error_e dnx_dram_buffers_power_down_enable_set(
    int,
    uint32);
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_dram_buffers_power_down_enable_set(unit, 1));
    }
    else
    {
...
static shr_error_e
dnx_dram_buffers_power_down_enable_set(
    int unit,
    uint32 enable)
{
    uint32 entry_handle_id;
    ...
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_BDM_POWER_DOWN, &entry_handle_id));
