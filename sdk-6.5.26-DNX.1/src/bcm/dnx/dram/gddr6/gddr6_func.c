...
#include <soc/dnx/shmoo_combo16.h>
...
dnx_gddr6_set_dram_interface_static_values(
    ...
    int dram_index)
{
    const dnx_data_dram_gddr6_ca_system_config_fields_t *conf_field;
    const uint32 nof_ca_bits = dnx_data_dram.gddr6.nof_ca_bits_get(unit);
    uint32 field_buffer[1] = { 0 };
    uint32 ca_ch_a[1] = { 0 };
    uint32 ca_ch_b[1] = { 0 };
    uint32 cabi_n;
    uint32 i;
    ...
    SHR_IF_ERR_EXIT(utilex_bitstream_set_bit_range(ca_ch_a, 0, nof_ca_bits - 1));
    SHR_IF_ERR_EXIT(utilex_bitstream_set_bit_range(ca_ch_b, 0, nof_ca_bits - 1));

    for (i = 0; i < dnx_data_dram.gddr6.ca_system_config_fields_info_get(unit)->key_size[0]; i++)
    {
        conf_field = dnx_data_dram.gddr6.ca_system_config_fields_get(unit, i);
        field_buffer[0] = conf_field->field_val;
        SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field
                        (field_buffer, conf_field->ch_a_start_bit, conf_field->nof_bits, ca_ch_a));
        SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field
                        (field_buffer, conf_field->ch_b_start_bit, conf_field->nof_bits, ca_ch_b));
    }

    SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(ca_ch_a, nof_ca_bits, nof_ca_bits, ca_ch_a));
    SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(ca_ch_b, nof_ca_bits, nof_ca_bits, ca_ch_b));

    cabi_n = 0x3;

    if (dnx_data_dram.gddr6.dq_channel_swap_get(unit, dram_index)->dq_channel_swap)
    {
        SHR_IF_ERR_EXIT(dnx_gddr6_dbal_force_constant_cpu_value_set(unit, dram_index, 0, ca_ch_b[0], cabi_n));
        SHR_IF_ERR_EXIT(dnx_gddr6_dbal_force_constant_cpu_value_set(unit, dram_index, 1, ca_ch_a[0], cabi_n));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_gddr6_dbal_force_constant_cpu_value_set(unit, dram_index, 0, ca_ch_a[0], cabi_n));
        SHR_IF_ERR_EXIT(dnx_gddr6_dbal_force_constant_cpu_value_set(unit, dram_index, 1, ca_ch_b[0], cabi_n));
    }
