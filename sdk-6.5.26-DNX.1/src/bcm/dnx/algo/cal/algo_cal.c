    ...
    uint32 *calendar)
{
    ...
    SHR_ALLOC_ERR_EXIT(slots_per_group, sizeof(*slots_per_group) * nof_groups, "slots_per_group", "%s%s%s",
                       EMPTY, EMPTY, EMPTY);

    for (group_id = 0; group_id < nof_groups; group_id++)
    {
        slots_per_group[group_id] = ids_per_group[group_id].size * ids_per_group[group_id].nof_slots;
    }
    ...
    uint32 *calendar)
{
    ...
    uint32 *slots_per_object_sub_cal = NULL;
    uint32 select_slots[2];
    ...
    if (nof_objects > 2)
    {
        select_index = utilex_get_index_of_max_member_in_array(slots_per_object, nof_objects);

        SHR_ALLOC_ERR_EXIT(objects_sub_cal, sizeof(uint32) * nof_objects, "objects_sub_cal", "%s%s%s",
                           EMPTY, EMPTY, EMPTY);

        SHR_ALLOC_ERR_EXIT(slots_per_object_sub_cal, sizeof(uint32) * nof_objects, "slots_per_object_sub_cal",
                           "%s%s%s", EMPTY, EMPTY, EMPTY);
        ...
        for (i = 0; i < nof_objects; i++)
        {
            if (i != select_index)
            {
                ...
                slots_per_object_sub_cal[nof_objects_sub_cal] = slots_per_object[i];
                nof_objects_sub_cal++;
            }
        }
        total_slots_sub_cal = calendar_len - slots_per_object[select_index];
        ...
        select_slots[0] = total_slots_sub_cal;
        select_slots[1] = slots_per_object[select_index];
    ...
    uint32 *calendar)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_cal_recursive_ratio_based_build_cb(unit, slots_per_object, nof_objects, calendar_len,
    ...
    dnx_algo_cal_detailed_entry_t * cal_dist_info)
{
    ...
    for (p = 0; p < nof_objects; p++)
    {
        ...
        {
            if (slots_per_object[p] > 1)
            {
                for (i = 0; i < calendar_len; i++)
                {
                    if (calendar[i] == p)
                    {
                        cal_dist_info[i].movable = FALSE;
                    }
                }
            }
            continue;
        ...
        for (curr_test = 0; curr_test < nof_tests_to_fix; curr_test++)
        {
            ...
            for (i = calendar_length - 1; i >= 0; i--)
            {
                ...
                if (actual_dist_left_i < cal_dist_info_curr[i].min_allowed_gap
                    && cal_dist_info_curr[i].min_allowed_gap != -1)
                {
                    j = i;
                    for (gap = 1;; gap++)
                    {
                        ...
                        if (!cal_dist_info_curr[k].movable)
                        {
                            continue;
                        }
                        delta = k - j;
                ...
                if (actual_dist_right_i > cal_dist_info_curr[i].max_allowed_gap
                    && cal_dist_info_curr[i].max_allowed_gap != -1)
                {
                    j = i;
                    for (gap = 1;; gap++)
                    {
