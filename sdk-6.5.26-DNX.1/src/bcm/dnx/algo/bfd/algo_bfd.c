 ...
 */
void
dnx_bfd_server_device_index_print_cb(
    int unit,
    const void *data)
{
#ifdef DNX_SW_STATE_DIAGNOSTIC
    ...
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT8, "BFD server device", server_device_index, NULL,
                                   "0x%x");
    SW_STATE_PRETTY_PRINT_FINISH();
...
/**
 * \brief - This function creates the template for the BFD server device index.
 *          Used for BFD server-client model
 *
 * \param [in] unit - Number of hardware unit used.
 *
 * \return
 ...
 */
static shr_error_e
dnx_algo_bfd_server_device_index_template_init(
    int unit)
{
    ...
    server_device_index_data.nof_profiles = dnx_data_bfd.general.bfd_nof_server_device_profile_get(unit);
    server_device_index_data.max_references = dnx_data_bfd.general.nof_endpoints_get(unit);
    ...
    sal_strncpy(server_device_index_data.name, DNX_BFD_SERVER_DEVICE_INDEX_TEMPLATE,
                DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);

    SHR_IF_ERR_EXIT(algo_bfd_db.bfd_server_device_index.create(unit, &server_device_index_data, NULL));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_bfd_server_device_index_template_init(unit));
