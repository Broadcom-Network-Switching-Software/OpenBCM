    ...
    uint32 alloc_flags)
{
    ...
    extra_create_info.grains_size[0] = DNX_FIELD_ALGO_FFC_BMP_REGION_SIZE;
    extra_create_info.max_tags_value[0] = TRUE;
    ...
    int *element)
{
    ...
    res_tag_bitmap_alloc_info.tags[0] = enable_tag;
    ...
    for (ii = 0; ii < DNX_FIELD_ALGO_FFC_BMP_REGION_COUNT; ii++)
    {
        ...
        {
            tag_info.tags[0] = enable_tag;
            tag_info.force_tag = TRUE;
            tag_info.element = DNX_FFC_REGION(cur_range);
            tag_info.nof_elements = DNX_FIELD_ALGO_FFC_BMP_REGION_SIZE;

            SHR_IF_ERR_EXIT(resource_tag_bitmap_tag_set
                            (unit, node_id, (resource_tag_bitmap_t) res_tag_bitmap, &tag_info));
