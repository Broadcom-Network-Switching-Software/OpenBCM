 ...
 * \param [in] node_id - relevant node_id
...
dnx_algo_template_mngr_even_distribution_set_cnt_tag(
    ...
    uint8 increment)
{
    resource_tag_bitmap_utils_tag_info_t tag_set_info, tag_get_info;
    int rv;
    SHR_FUNC_INIT_VARS(unit);

    sal_memset(&tag_set_info, 0, sizeof(resource_tag_bitmap_utils_tag_info_t));
    sal_memset(&tag_get_info, 0, sizeof(resource_tag_bitmap_utils_tag_info_t));

    /** Get the the current counter value associated with 'profile' */
    tag_get_info.element = profile;
    rv = dnx_algo_smart_template_tag_get(unit, node_id, multi_set_template, &tag_get_info);
    ...
    if (rv != _SHR_E_NOT_FOUND)
    {
        ...
        tag_set_info.tags[0] =
            (increment) ? tag_get_info.tags[0] + 1 : ((tag_get_info.tags[0] > 0) ? tag_get_info.tags[0] - 1 : 0);
        /*
        ...
        SHR_IF_ERR_EXIT(dnx_algo_smart_template_tag_set(unit, node_id, multi_set_template, &tag_set_info));
    }
exit:
    SHR_FUNC_EXIT;
}
...
dnx_algo_template_mngr_even_distribution_set_range(
    ...
    smart_template_alloc_info_t * extra_alloc_info)
{
    resource_tag_bitmap_utils_tag_info_t tag_get_info;
    ...
    else
    {
        ...
        iter_range_start = multi_set_template->allocation_bitmap->resource->first_element;
        iter_range_end = multi_set_template->allocation_bitmap->resource->count;
    }

    /** Set increment value for the iterator */
    increment = multi_set_template->allocation_bitmap->resource->tag_level_info[0].grain_size;
     ...
     */
    min_allocations = multi_set_template->allocation_bitmap->resource->tag_level_info[0].grain_size;
    while (grain_iter - iter_range_end != 0)
    {
        ...
        if (rv == _SHR_E_NOT_FOUND)
        {
             ...
             */
            chosen_grain = grain_iter / multi_set_template->allocation_bitmap->resource->tag_level_info[0].grain_size;
            break;
        }
        ...
        else if (tag_get_info.tags[0] < min_allocations)
        {
            /** Update the minimum value and the grain that matches the value */
            min_allocations = tag_get_info.tags[0];
            chosen_grain = grain_iter / multi_set_template->allocation_bitmap->resource->tag_level_info[0].grain_size;
        }
        /** Increment iterator */
    ...
    extra_alloc_info->resource_alloc_info.range_start =
        chosen_grain * multi_set_template->allocation_bitmap->resource->tag_level_info[0].grain_size;
    extra_alloc_info->resource_alloc_info.range_end =
        extra_alloc_info->resource_alloc_info.range_start +
        multi_set_template->allocation_bitmap->resource->tag_level_info[0].grain_size;
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint8 *first_reference)
{
    ...
    if (!_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID) && (rv == _SHR_E_NOT_FOUND))
    {
        /*
        ...
        SHR_IF_ERR_EXIT(dnx_algo_template_mngr_even_distribution_set_range(unit,
                                                                           node_id, multi_set_template, flags,
                                                                           &extra_alloc_info));
    }
    ...
    if (*first_reference)
    {
        SHR_IF_ERR_EXIT(dnx_algo_template_mngr_even_distribution_set_cnt_tag
                        (unit, node_id, multi_set_template, *profile, TRUE));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint8 *last_reference)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_template_mngr_even_distribution_set_cnt_tag
                        (unit, node_id, multi_set_template, profile, FALSE));
    }
