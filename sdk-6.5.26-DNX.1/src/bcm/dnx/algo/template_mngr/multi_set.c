    ...
    uint8 *first_appear)
{
    ...
    if (with_id)
    {
        ...
        /*
         * Note: Procedure sw_state_index_hash_table_entry_add_at_index() reports 'success' on trying to add
         * an existing entry.
         */
        SHR_IF_ERR_EXIT(sw_state_index_hash_table_entry_add_at_index
                        (unit, node_id, multi_set->hash_table, (MULTI_SET_KEY *) key, found_index, &success));
    }
    else
    {
        /*
         * Note: Procedure sw_state_index_hash_table_entry_add() reports 'success' on trying to add
         * an existing entry.
         */
     ...
     *
     * If it is a singleton, and we didn't find it in the lookup (sw_state_index_hash_table_entry_lookup),
     * then mark it as 'first appearance' and quit.
     * Note that *success is as loaded by 'sw_state_index_hash_table_entry_add()'
     */
    /*
     * Save the index. 
     */
    ...
    uint32 alloc_flags)
{
    ...
    uint32 member_size;
    sw_state_idx_htbl_init_info_t hash_table_init_info;
    ...
    max_duplications = 0;
    member_size = 0;
    sal_memset(&hash_table_init_info, 0, sizeof(sw_state_idx_htbl_init_info_t));
    ...
    SHR_IF_ERR_EXIT(sw_state_index_hash_table_create(unit, node_id, &((*multi_set)->hash_table), &hash_table_init_info, member_size, member_size, 0     /* nof_members 
                                                                                                                                                         */ , alloc_flags));
    ...
    void *extra_arguments)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_index_hash_table_destroy(unit, node_id, &((*multi_set)->hash_table)));
    ...
    *last_appear = (ref_counter == 0) ? TRUE : FALSE;
    ...
    if (*last_appear)
    {
        ...
        SHR_IF_ERR_EXIT(sw_state_index_hash_table_entry_remove_by_index
                        (unit, node_id, multi_set->hash_table, _data_indx));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    multi_set_t multi_set)
{
     ...
     * clear mutli-set.
     */
    SHR_IF_ERR_EXIT(sw_state_index_hash_table_clear(unit, node_id, multi_set->hash_table));
