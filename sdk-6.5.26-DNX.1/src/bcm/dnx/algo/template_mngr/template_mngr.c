    ...
    dnx_algo_template_create_data_t * data)
{
    ...
    sw_state_string_strncpy
        (unit, node_id, algo_temp->name, sizeof(char) * (DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1), data->name);
    ...
    uint32 alloc_flags)
{
     ...
     * 1. Allocate template.
     */
    DNX_SW_STATE_ALLOC(unit, node_id, *algo_temp, **algo_temp, /* number of elements */ 1,
                       /*
                        * flags 
                        */ 0, "sw_state algo_temp");
    ...
    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[data->advanced_algorithm].create_cb
                    (unit, node_id, &((*algo_temp)->multi_set_template), data, extra_arguments, alloc_flags));

    if (_SHR_IS_FLAG_SET(data->flags, DNX_ALGO_TEMPLATE_CREATE_USE_DEFAULT_PROFILE))
    {
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[data->advanced_algorithm].allocate_cb
                        (unit, node_id, (*algo_temp)->multi_set_template,
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_template_register_data(unit, node_id, *algo_temp, data));
    ...
    void *extra_arguments)
{
    ...
    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[(*algo_temp)->advanced_algorithm].destroy_cb
                    (unit, node_id, &((*algo_temp)->multi_set_template), extra_arguments));

    DNX_SW_STATE_FREE(unit, node_id, (*algo_temp)->multi_set_template, "sw_state algo_temp multiset");

    DNX_SW_STATE_FREE(unit, node_id, *algo_temp, "sw_state algo_temp");
    ...
    uint8 *first_reference)
{
    ...
    SHR_NULL_CHECK(first_reference, _SHR_E_PARAM, "first_reference")
         ...
         */
        SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_verify(unit, node_id, algo_temp));
    ...
    {
        SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_profile_verify(unit, node_id, algo_temp, *profile));
    }
    ...
    uint8 *first_reference)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_allocate_verify
                           (unit, node_id, algo_temp, flags, profile_data, profile, first_reference));
    ...
    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].allocate_cb
                    (unit, node_id, algo_temp->multi_set_template, flags, 1 /** nof refs */ ,
    ...
    int profile)
{
     ...
     * Verify profile.
     */
    SHR_IF_ERR_EXIT(dnx_algo_template_profile_verify(unit, node_id, algo_temp, profile));
    ...
    void *profile_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_and_profile_verify(unit, node_id, algo_temp, profile));
    ...
    void *profile_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_profile_data_get_verify
                           (unit, node_id, algo_temp, profile, ref_count, profile_data));

    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].profile_data_get_cb
                    (unit, node_id, algo_temp->multi_set_template, profile, ref_count, profile_data));
    ...
    uint8 *last_reference)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_free_verify(unit, node_id, algo_temp, profile, last_reference));

    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].free_cb
                    (unit, node_id, algo_temp->multi_set_template, profile, 1, last_reference));
    ...
    uint8 *last_reference)
{
     ...
     * Verify old profile.
     */
    SHR_IF_ERR_EXIT(dnx_algo_template_profile_verify(unit, node_id, algo_temp, old_profile));

    if (!_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_EXCHANGE_IGNORE_NOT_EXIST_OLD_PROFILE))
    {
        SHR_IF_ERR_EXIT(dnx_algo_template_profile_data_get(unit, node_id, algo_temp, old_profile, &ref_count, NULL));
        if (ref_count == 0)
        {
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID))
    {
         ...
         */
        SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_profile_verify(unit, node_id, algo_temp, *new_profile));

        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_EXCHANGE_IGNORE_DATA))
        {
            SHR_IF_ERR_EXIT(dnx_algo_template_profile_data_get
                            (unit, node_id, algo_temp, *(int *) new_profile, &ref_count, NULL));
            if (ref_count == 0)
            {
                SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
        ...
        {
            rv = dnx_algo_template_profile_get(unit, node_id, algo_temp, profile_data, &profile);
    ...
    const void *new_profile_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_template_profile_data_get(unit, node_id, algo_temp, profile, &ref_count, NULL));
    if (ref_count == 0)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "We can't change the data of a non-existing profile.");
    }

    rv = dnx_algo_template_profile_get(unit, node_id, algo_temp, new_profile_data, &tmp_profile);
    ...
    const void *new_profile_data)
{
     ...
     */
    SHR_ALLOC_ERR_EXIT(profile_old_data, algo_temp->data_size, "Old data of new profile..", "%s%s%s",
                       "profile_old_data", EMPTY, EMPTY);

    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].profile_data_get_cb
                    (unit, node_id, algo_temp->multi_set_template, profile, &ref_count, profile_old_data));

    /*
     *  Free all members using the old data.
     */
    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].free_cb
                    (unit, node_id, algo_temp->multi_set_template, profile, ref_count, &last_reference));
     ...
     */
    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].allocate_cb
                    (unit, node_id, algo_temp->multi_set_template, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID,
    ...
    const void *new_profile_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_replace_data_verify(unit, node_id, algo_temp, profile, new_profile_data));
    ...
    if (Template_callbacks_map_sw[algo_temp->advanced_algorithm].replace_data_cb != NULL)
    {
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].replace_data_cb
                        (unit, node_id, algo_temp->multi_set_template, profile, new_profile_data));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_algo_template_generic_replace_data(unit, node_id, algo_temp, profile, new_profile_data));
    }
    ...
    int *profile)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_profile_get_verify(unit, node_id, algo_temp, profile_data, profile));

    rv = Template_callbacks_map_sw[algo_temp->advanced_algorithm].profile_get_cb(unit, node_id,
    ...
    dnx_algo_template_t algo_temp)
{
    ...
    else
    {
        while (TRUE)
        {
            SHR_IF_ERR_EXIT(dnx_algo_template_get_next(unit, node_id, algo_temp, &current_profile));
            if (current_profile == DNX_ALGO_TEMPLATE_ILLEGAL_PROFILE)
            {
                break;
            }
            SHR_IF_ERR_EXIT(dnx_algo_template_free_all(unit, node_id, algo_temp, current_profile++));
        }
    }
    ...
    if (_SHR_IS_FLAG_SET(algo_temp->flags, DNX_ALGO_TEMPLATE_CREATE_USE_DEFAULT_PROFILE))
    {
        ...
        SHR_ALLOC_ERR_EXIT(default_data, algo_temp->data_size, "Default data backup", "%s%s%s", "default_data",
                           EMPTY, EMPTY);
    ...
    int *current_profile)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_get_next_verify(unit, node_id, algo_temp, current_profile));
    ...
    if (Template_callbacks_map_sw[algo_temp->advanced_algorithm].get_next_cb != NULL)
    {
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].get_next_cb
                        (unit, node_id, algo_temp->multi_set_template, current_profile));
    }
    else
    {
        SHR_ALLOC_ERR_EXIT(tmp_profile_data, algo_temp->data_size, "Data buffer for tmp template data.", "%s%s%s",
                           "tmp_profile_data", EMPTY, EMPTY);
        ...
        {
            SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].profile_data_get_cb
                            (unit, node_id, algo_temp->multi_set_template, profile_iter, &ref_count, tmp_profile_data));
            if (ref_count > 0)
            {
                break;
            }
    ...
    int profile)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_free_all_verify(unit, node_id, algo_temp, profile));
    ...
    if (Template_callbacks_map_sw[algo_temp->advanced_algorithm].free_all_cb != NULL)
    {
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].free_all_cb
                        (unit, node_id, algo_temp->multi_set_template, profile));
    }
    else
    {
        SHR_ALLOC_ERR_EXIT(tmp_profile_data, algo_temp->data_size, "Data buffer for tmp template data.", "%s%s%s",
        ...
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].profile_data_get_cb
                        (unit, node_id, algo_temp->multi_set_template, profile, &ref_count, tmp_profile_data));
    ...
    uint8 *last_reference)
{
     ...
     */
    SHR_ALLOC_ERR_EXIT(old_profile_data, algo_temp->data_size, "Data buffer old.", "%s%s%s", "old_profile_data",
                       EMPTY, EMPTY);

    SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].profile_data_get_cb
                    (unit, node_id, algo_temp->multi_set_template, old_profile, &ref_count_old, old_profile_data));
    ...
    if (ref_count_old != 0)
    {
         ...
         */
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].free_cb
                        (unit, node_id, algo_temp->multi_set_template, old_profile, nof_refs, last_reference));
        /*
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID))
    {
        SHR_ALLOC_ERR_EXIT(new_profile_old_data, algo_temp->data_size, "Old data of new profile", "%s%s%s",
                           "new_profile_old_data", EMPTY, EMPTY);

        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].profile_data_get_cb
                        (unit, node_id, algo_temp->multi_set_template, *new_profile,
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_EXCHANGE_IGNORE_DATA))
    {
         ...
         */
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].allocate_cb
                        (unit, node_id, algo_temp->multi_set_template, flags, nof_refs,
    ...
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].allocate_cb
                        (unit, node_id, algo_temp->multi_set_template, flags, nof_refs, (void *) profile_data,
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_EXCHANGE_TEST))
    {
         ...
         */
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].free_cb
                        (unit, node_id, algo_temp->multi_set_template, *new_profile, nof_refs, &test_last_reference));
...
exit:
    if (SHR_FUNC_ERR() || !add_success || _SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_EXCHANGE_TEST))
    {
        ...
        if (old_profile_freed)
        {
            ...
            rv = Template_callbacks_map_sw[algo_temp->advanced_algorithm].allocate_cb(unit, node_id,
                                                                                      algo_temp->multi_set_template,
    ...
    uint8 *last_reference)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_exchange_verify
                           (unit, node_id, algo_temp, flags, profile_data, old_profile, new_profile,
                            first_reference, last_reference));

    /*
     * check the profile that is mapped to the input data
     */
    rv = dnx_algo_template_profile_get(unit, node_id, algo_temp, profile_data, &tmp_profile);
    if (rv == _SHR_E_NOT_FOUND)
    {
        /*
    ...
    else
    {
        if (Template_callbacks_map_sw[algo_temp->advanced_algorithm].exchange_cb != NULL)
        {
            SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].exchange_cb
                            (unit, node_id, algo_temp->multi_set_template, flags, 1, profile_data,
                             old_profile, extra_arguments, new_profile, first_reference, last_reference));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_template_generic_exchange
                            (unit, node_id, algo_temp, flags, (void *) profile_data, old_profile,
    ...
    dnx_algo_template_print_data_cb print_cb)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_template_dump_info_get(unit, node_id, (*algo_temp), &data));
    ...
    if (data.nof_used_profiles)
    {
        ...
        tmp_ref_count = 0;
        SHR_ALLOC_ERR_EXIT(tmp_data, (*algo_temp)->data_size, "Tmp data", "%s%s%s", "tmp_data", EMPTY, EMPTY);
        ...
        while (1)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_template_profile_data_get
                            (unit, node_id, (*algo_temp), current_profile, &tmp_ref_count, tmp_data));
