...
#undef _SOC_DRV_H
#endif
#include <soc/drv.h> /** needed for access soc flr linkupcb */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnx_linkctrl_deinit(unit));
    ...
    int mode)
{
    ...
    BCM_PBMP_ITER(gport_info.local_port_bmp, port_ndx)
    {
        if (!dnx_drv_is_sfi_port(unit, port))
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Stat port is not supported by linkscan");
            }
            /*
             * FlexE phy ports are not supported in HW mode
             */
            if (DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info, TRUE) && mode == BCM_LINKSCAN_MODE_HW)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "FlexE phy port is not supported by linkscan HW mode");
            }
            /*
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "ILKN port is not supported by linkscan HW mode");
            }
        }
    ...
    int port)
{
    ...
    SOC_PBMP_CLEAR(flr_linkup_pbmp);
    SOC_PBMP_PORT_ADD(flr_linkup_pbmp, port);

    if (soc->hw_linkscan_flr_linkup_cb)
    {
        SOC_PBMP_CLEAR(flr_linkup_pbmp);
        SOC_PBMP_PORT_ADD(flr_linkup_pbmp, port);
        soc->hw_linkscan_flr_linkup_cb(unit, flr_linkup_pbmp);
    }
    return;
}
    ...
    else
    {
        ...
        if (link)
        {
            ...
            if (flr_delay_check)
            {
                ...
                /*
                 * FLR linkup CB, similar as soc_iproc_flr_link_up which is for HW linkscan 
                 */
                _dnx_fw_sw_flr_link_up(unit, port);
            }
            else
            {
                /*
