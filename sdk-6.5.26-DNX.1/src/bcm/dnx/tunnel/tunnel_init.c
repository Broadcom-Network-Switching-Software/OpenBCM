...
#include <src/bcm/dnx/flow/porting/mirror_flow.h>
...
#define TUNNEL_INIT_IS_UDP_TUNNEL_WITH_USER_DEFINED_UDP_PORTS(_tunnel_type, _udp_src_port, _udp_dst_port) (DNX_TUNNEL_TYPE_IS_UDP(_tunnel_type) && ((_udp_src_port != 0) || (_udp_dst_port!= 0)))
...
/**
 * \brief
 * Used by bcm_dnx_tunnel_initiator_create to verify API input regarding SVTAG use.
 */
shr_error_e
dnx_tunnel_initiator_create_verify_svtag(
    int unit,
    ...
    uint16 udp_src_port)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "SVTAG is not supported in this device.");
    }

    if (TUNNEL_INIT_IS_UDP_TUNNEL_WITH_USER_DEFINED_UDP_PORTS(type, udp_src_port, udp_dst_port))
    {
     ...
     */
    if ((type == bcmTunnelTypeGreAnyIn4) || (type == bcmTunnelTypeGreAnyIn6)
        || (type == bcmTunnelTypeSR6) || (type == bcmTunnelTypeEthSR6))
    {
    ...
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    if (_SHR_IS_FLAG_SET(tunnel->flags, BCM_TUNNEL_INIT_SVTAG_ENABLE))
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_initiator_create_verify_svtag
                        (unit, tunnel->type, tunnel->udp_dst_port, tunnel->udp_src_port));
    }
 ...
 *       header profile
 * \param [out] ahs_namespace - Additional headers stack namespace.  
 *
 */
static shr_error_e
    ...
    dnx_ahs_namespace_e * ahs_namespace)
{
    ...
    switch (bcm_tunnel_type)
    {
        ...
        case bcmTunnelTypeGreAnyIn6:
        {
            ...
            *ahs_namespace = AHS_NAMESPACE_GRE_OR_GENEVE;
            break;
        }
        case bcmTunnelTypeIpAnyIn4:
        case bcmTunnelTypeIpAnyIn6:
        case bcmTunnelTypeEthIn6:
        {
            ...
            *ahs_namespace = AHS_NAMESPACE_IP_TUNNEL;
            break;
        }
        case bcmTunnelTypeUdp:
        case bcmTunnelTypeUdp6:
        {
            ...
            *ahs_namespace = AHS_NAMESPACE_UDP;
            break;
        }
        case bcmTunnelTypeVxlan:
        case bcmTunnelTypeVxlan6:
        {
            ...
            *ahs_namespace = AHS_NAMESPACE_VXLAN;
            break;
        }
        case bcmTunnelTypeVxlanGpe:
        case bcmTunnelTypeVxlan6Gpe:
        ...
        case bcmTunnelTypeEthSR6:
        {
            ...
            *ahs_namespace = AHS_NAMESPACE_TOTAL_NUM;
            break;
        }
        default:
        {
    ...
    uint32 *local_outlif)
{
    ...
    /**
     * If the ESEM command profile is not configured in the caller function then the ESEM lookup is not required
     */
    if (esem_cmd_profile != 0)
    {
    ...
    if (set_new_esem_cmd_profile == TRUE)
    {
        ...
        if (_SHR_IS_FLAG_SET(tunnel_esem_access_flags, TUNNEL_INIT_ESEM_ACCESS_TYPE_VNI))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_rx_trap_lif_etpp_trap_profile_allocate_and_hw_update
                            (unit, old_lif_trap_profile, trap_gport, &new_lif_trap_profile, &is_last));
    ...
    uint32 arp_out_lif)
{
    ...
    uint32 new_lif_additional_header_profile = 0;
    ...
    dnx_ahs_namespace_e ahs_namespace;
    uint8 is_udp_tunnel_with_user_defined_udp_ports =
        TUNNEL_INIT_IS_UDP_TUNNEL_WITH_USER_DEFINED_UDP_PORTS(tunnel->type, tunnel->udp_src_port, tunnel->udp_dst_port);
    ...
    if (!_SHR_IS_FLAG_SET(tunnel->flags, BCM_TUNNEL_REPLACE))
    {
         ...
         */
        if (is_udp_tunnel_with_user_defined_udp_ports)
        {
            /*
            ...
            new_lif_additional_header_profile = new_udp_ports_profile;
            ahs_namespace = AHS_NAMESPACE_UDP;
        }
        ...
        else
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_init_bcm_to_lif_additional_header_profile
                            (unit, tunnel->type, tunnel->flags, &new_lif_additional_header_profile, &ahs_namespace));
        }

        /** Alloc and set the additional headers profile.*/
        SHR_IF_ERR_EXIT(egress_additional_headers_profile_alloc
                        (unit, FALSE, 0, new_lif_additional_header_profile, ahs_namespace, &headers_profile_mngr_info));
        ...
        /** ESEM command configuration */
        /*
         *  For bcmTunnelTypeEthIn6 and bcmTunnelTypeEthSR6 the default native is set
         */
        if ((tunnel->type == bcmTunnelTypeEthSR6) || (tunnel->type == bcmTunnelTypeEthIn6))
        {
            new_esem_cmd_profile = dnx_data_esem.access_cmd.default_native_get(unit);
        }
        else if (esem_cmd_flag != TUNNEL_INIT_ESEM_ACCESS_NONE)
        {
            /*
             * For vxlan or geneve, set esem command:
             * indicate that an ESEM lookup is expected, the ESEM result is the VNI.
             */
            /** Allocate an esem access cmd and configure the cmd table. */
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_initiator_allocate_local_lif_and_out_lif_table_set
                        (unit, tunnel, dbal_table_id, global_lif_id, next_local_outlif, new_sip_profile,
                         ...
                         new_lif_additional_header_profile, &local_outlif));
    }
    ...
    else
    {
        ...
        uint8 is_old_udp_tunnel_with_user_defined_udp_ports = FALSE;
        uint32 old_lif_additional_header_profile = 0;
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_init_ipv4_ipv6_eedb_entry_get
                        (unit, gport_hw_resources.outlif_dbal_table_id, gport_hw_resources.local_out_lif,
                         ...
                         &old_esem_cmd_profile, &old_lif_additional_header_profile, &ipv4_df_flag_dummy_var,
        ...
        if (_SHR_IS_FLAG_SET((tunnel->flags ^ svtag_flag_dummy_var), BCM_TUNNEL_INIT_SVTAG_ENABLE))
        {
         ...
         *   If the entry type requires lif additional header profile that needs allocation, set it here.
         */
        is_old_udp_tunnel_with_user_defined_udp_ports =
            DNX_TUNNEL_TYPE_IS_UDP(old_tunnel_type)
            && TUNNEL_INIT_IS_IN_LIF_AHP_UDP_SESSION_PROFILE_RANGE(unit, old_lif_additional_header_profile);
        if (is_udp_tunnel_with_user_defined_udp_ports || is_old_udp_tunnel_with_user_defined_udp_ports)
        {
            /*
             ...
             * the old  profile.
             */
            old_udp_ports_profile = old_lif_additional_header_profile;
            SHR_IF_ERR_EXIT(dnx_tunnel_udp_ports_profile_allocator_and_set_hw
        ...
        /** Alloc and set the additional headers profile.*/
        SHR_IF_ERR_EXIT(egress_additional_headers_profile_alloc
                        (unit, TRUE, old_lif_additional_header_profile, new_lif_additional_header_profile,
                         ahs_namespace, &headers_profile_mngr_info));
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_initiator_allocate_local_lif_and_out_lif_table_set
                        (unit, tunnel, gport_hw_resources.outlif_dbal_table_id,
                         ...
                         new_ttl_pipe_profile, nwk_qos_profile,
                         old_esem_cmd_profile, srv6_with_dip, new_lif_additional_header_profile, &local_outlif));
    ...
    bcm_l3_intf_t * intf)
{
    ...
    uint32 lif_additional_header_profile = 0;
    uint32 lif_additional_header_profile_dummy;
    uint32 remove_old_udp_ports_profile = FALSE;
    uint32 old_udp_ports_profile = 0;
    bcm_tunnel_type_t tunnel_type;
    bcm_encap_access_t encap_access;
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_init_ipv4_ipv6_eedb_entry_get
                    (unit, gport_hw_resources.outlif_dbal_table_id, gport_hw_resources.local_out_lif,
                     ...
                     &encap_access, &srv6_scale_flag, &stat_flag, &svtag_flag, ipv6_dip_dummy_var));

    SHR_IF_ERR_EXIT(dnx_tunnel_init_lif_additional_header_to_tunnel_flags
                    (unit, tunnel_type, lif_additional_header_profile, &tunnel_flags));
    tunnel_flags |= srv6_scale_flag;
    tunnel_flags |= stat_flag;
    tunnel_flags |= svtag_flag;
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_init_bcm_to_lif_additional_header_profile
                    (unit, tunnel_type, tunnel_flags, &lif_additional_header_profile_dummy, &ahs_namespace));
    SHR_IF_ERR_EXIT(egress_additional_headers_profile_free
                    (unit, lif_additional_header_profile, ahs_namespace, &headers_profile_mngr_info));
    SHR_IF_ERR_EXIT(egress_additional_headers_profile_last_clear(unit, &headers_profile_mngr_info));
