    ...
    uint32 *local_outlif)
{
    ...
    /** set outlif phase */
    lif_table_mngr_outlif_info.logical_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_2;
    ...
    if (tunnel->flags & BCM_TUNNEL_INIT_WIDE)
    {
        content_id = tunnel->aux_data2;
        content_id <<= 32;
        content_id |= tunnel->aux_data;

        /*
         *  LI content ID is 8 bytes
         *  LI content ID (8B) = {aux_data2, aux_data}
         */
        dbal_entry_value_field64_set(unit, entry_handle_id, DBAL_FIELD_LI_CONTENT_ID, INST_SINGLE, content_id);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RAW_DATA_SIZE, INST_SINGLE, 8);
    }
    else
    {
        content_id = tunnel->aux_data;
        ...
        /** LI content ID is 4 bytes, put in MSB */
        dbal_entry_value_field64_set(unit, entry_handle_id, DBAL_FIELD_LI_CONTENT_ID, INST_SINGLE, content_id);
    ...
    if (tunnel != NULL)
    {
        ...
        if (is_field_valid)
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, DBAL_FIELD_RAW_DATA_SIZE, INST_SINGLE, &header_size));
            ...
            if (header_size == 8)
            {
                 ...
                 *  LI content ID (8B) = {aux_data2, aux_data}
                 */
                dbal_entry_handle_value_field64_get(unit, entry_handle_id, DBAL_FIELD_LI_CONTENT_ID, INST_SINGLE,
                                                    &content_id);
                tunnel->aux_data2 = (uint32) ((content_id >> 32) & 0xFFFFFFFF);
                tunnel->aux_data = (uint32) (content_id & 0xFFFFFFFF);
            }
            else
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field64_get
                                (unit, entry_handle_id, DBAL_FIELD_LI_CONTENT_ID, INST_SINGLE, &content_id));
                tunnel->aux_data = (uint32) ((content_id >> 32) & 0xFFFFFFFF);
            }
        }
    }
