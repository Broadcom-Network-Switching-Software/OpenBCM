    ...
    int protection_pointer)
{
    ...
    else if (failover_type == BCM_FAILOVER_TYPE_FEC)
    {
        ...
        if (protection_pointer == dnx_data_failover.path_select.fec_no_protection_get(unit) ||
            ...
            protection_pointer == DNX_FAILOVER_PROTECTION_DISABLE_VALUE)
        {
    ...
    {
        SHR_RANGE_VERIFY(protection_pointer, 0, dnx_data_failover.path_select.ingress_mc_size_get(unit) - 1,
                         _SHR_E_PARAM, "ingress MC failover-id out of bound.\n");
        if (protection_pointer == dnx_data_failover.path_select.ing_mc_no_protection_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "failover id %d pre-allocated for ingress MC no protection.\n",
                         protection_pointer);
        }
    }
    else
    ...
    int unit)
{
    ...
    int facility_protection_ptr, bank_id;
    ...
    for (bank_id = dnx_data_failover.path_select.nof_fec_path_select_banks_get(unit);
         bank_id < dnx_data_failover.path_select.nof_fec_path_select_banks_in_hw_get(unit); bank_id++)
    {
        int first_failover_id_in_bank = bank_id * dnx_data_failover.path_select.fec_bank_size_get(unit);
        SHR_IF_ERR_EXIT(dnx_failover_fec_hierarchy_set
                        (unit, first_failover_id_in_bank, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_NO_HIERARCHY));
    }
    ...
    int unit)
{
    ...
    if (!dnx_data_multicast.params.feature_get(unit, dnx_data_multicast_params_mc_pruning_actions_not_supported))
    {
        ...
        no_protection_element = dnx_data_failover.path_select.ing_mc_no_protection_get(unit);
        SHR_IF_ERR_EXIT(algo_failover_db.ing_mc_path_select.allocate_single(unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, NULL,
                                                                            &no_protection_element));
    }
    ...
    bcm_failover_t failover_id)
{
    ...
    BCM_FAILOVER_TYPE_GET(actual_type, failover_id);
    BCM_FAILOVER_ID_GET(protection_ptr, failover_id);

    if (actual_type == BCM_FAILOVER_TYPE_FEC)
    {
...
shr_error_e
dnx_failover_table_id_get(
    int unit,
    bcm_failover_t failover_id,
    dbal_tables_e * failover_table_id)
{
    int failover_type;
    SHR_FUNC_INIT_VARS(unit);
    ...
    switch (failover_type)
    {
        case BCM_FAILOVER_TYPE_INGRESS:
            *failover_table_id = DBAL_TABLE_FAILOVER_INGRESS_PATH_SELECT;
            break;
        case BCM_FAILOVER_TYPE_ENCAP:
            *failover_table_id = DBAL_TABLE_FAILOVER_EGRESS_PATH_SELECT;
            break;
        case BCM_FAILOVER_TYPE_FEC:
            *failover_table_id = DBAL_TABLE_FAILOVER_FEC_PATH_SELECT;
            break;
        default:
    ...
    int enable)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_failover_id_verify(unit, failover_id));

    BCM_FAILOVER_ID_GET(protection_pointer, failover_id);

    SHR_IF_ERR_EXIT(dnx_failover_table_id_get(unit, failover_id, &table_id));
    ...
    bcm_failover_t * failover_id)
{
    ...
    int is_fec, is_ingress, is_ingress_mc, is_egress, failover_type;
    int protection_pointer;
    ...
    supported_flags =
        ...
        BCM_FAILOVER_FEC_3RD_HIERARCHY | BCM_FAILOVER_INGRESS_MC | BCM_FAILOVER_WITH_ID;
    ...
    is_ingress_mc = (flags & BCM_FAILOVER_INGRESS_MC) ? 1 : 0;
    is_egress = (flags & BCM_FAILOVER_ENCAP) ? 1 : 0;
    is_fec = (flags & BCM_FAILOVER_FEC) ? 1 : 0;
    if (is_ingress + is_egress + is_fec + is_ingress_mc != 1)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "failover id can be created for specific type only.\n");
    }
    else if (is_ingress)
    {
        failover_type = BCM_FAILOVER_TYPE_INGRESS;
    }
    else if (is_egress)
    {
        failover_type = BCM_FAILOVER_TYPE_ENCAP;
    }
    else if (is_ingress_mc)
    {
        failover_type = BCM_FAILOVER_TYPE_INGRESS_MC;
    }
    else
    {
    ...
    if (is_ingress_mc
        && dnx_data_multicast.params.feature_get(unit, dnx_data_multicast_params_mc_pruning_actions_not_supported))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_FAILOVER_TYPE_INGRESS_MC is not supported.\n");
    }
...
bcm_dnx_failover_create(
    ...
    bcm_failover_t * failover_id)
{
    int is_fec, is_ingress, is_ingress_mc, is_egress;
    int failover_type;
    uint8 hierarchy = 0;
    uint32 alloc_flags;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_failover_create_verify(unit, flags, failover_id));
    ...
    {
        SHR_IF_ERR_EXIT(algo_failover_db.ing_mc_path_select.allocate_single(unit, alloc_flags, NULL, failover_id));
    }
    else
    {
    ...
    BCM_FAILOVER_SET(*failover_id, *failover_id, failover_type);
    ...
    bcm_failover_t failover_id)
{
    ...
    if (failover_type == BCM_FAILOVER_TYPE_FEC)
    {
        ...
        SHR_IF_ERR_EXIT(algo_failover_db.fec_path_select.free_single(unit, protection_ptr, NULL));
        ...
        else
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_failover_fec_hierarchy_set
                                (unit, protection_ptr, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1));
            }
        }
    }
    else if (failover_type == BCM_FAILOVER_TYPE_INGRESS)
    {
        SHR_IF_ERR_EXIT(algo_failover_db.ing_path_select.free_single(unit, protection_ptr, NULL));
    }
    else if (failover_type == BCM_FAILOVER_TYPE_INGRESS_MC)
    {
        SHR_IF_ERR_EXIT(algo_failover_db.ing_mc_path_select.free_single(unit, protection_ptr, NULL));
    }
    else
    {
        SHR_IF_ERR_EXIT(algo_failover_db.egr_path_select.free_single(unit, protection_ptr, NULL));
    }
    ...
    if (is_ingress == TRUE)
    {
        if (DNX_FAILOVER_IS_PROTECTION_ENABLED(protection_info.ingress_failover_id))
        {
            SHR_IF_ERR_EXIT(dnx_failover_id_valid_verify
                            (unit, BCM_FAILOVER_TYPE_INGRESS, 0, protection_info.ingress_failover_id));
        }
    }
    if (is_egress == TRUE)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_failover_id_valid_verify
                            (unit, BCM_FAILOVER_TYPE_ENCAP, 0, protection_info.egress_failover_id));
        }
    }
