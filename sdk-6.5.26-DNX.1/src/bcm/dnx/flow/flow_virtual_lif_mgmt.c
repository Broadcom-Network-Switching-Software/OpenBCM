...
shr_error_e
dnx_flow_egress_virtual_entry_not_created_by_flow_set(
    int unit,
    bcm_gport_t virtual_lif_gport)
{
...
shr_error_e
dnx_flow_egress_virtual_is_created_by_flow(
    int unit,
    bcm_gport_t virtual_lif_gport,
    uint32 *is_created_by_flow)
{
    uint32 sw_entry_handle_id = 0;
    uint32 is_not_created_by_flow = 0;
    shr_error_e rv;
    ...
    rv = dbal_entry_get(unit, sw_entry_handle_id, DBAL_GET_ALL_FIELDS);
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);

    (*is_created_by_flow) = (!is_not_created_by_flow);
