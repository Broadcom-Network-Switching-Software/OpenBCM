    ...
    else
    {
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_terminator_info_symmetric_replace
                        (unit, flow_handle_info, l2_learn_info, l2_learn_data_param, gport_hw_resources,
                         entry_handle_id, selectable_result_types));
    }
    ...
    dnx_flow_app_config_t * flow_app_info)
{
    ...
    if (!sal_strncasecmp(flow_app_info->app_name, FLOW_APP_NAME_VLAN_PORT_ESEM_NAMESPACE_VSI_INITIATOR, FLOW_STR_MAX))
    {
        ...
        while (next_field_id != DBAL_FIELD_EMPTY)
        {
            ...
            if (next_field_id != DBAL_FIELD_FWD_DOMAIN_NWK_NAME)
            {
...
/** checks if the entry related to flow application */
static shr_error_e
dnx_flow_is_entry_related_to_flow(
    int unit,
    uint32 entry_handle_id,
    const dnx_flow_app_config_t * flow_app_info,
    int *is_entry_related)
{
    ...
    if (!sal_strncasecmp(flow_app_info->app_name, FLOW_APP_NAME_VLAN_PORT_ESEM_NAMESPACE_VSI_INITIATOR, FLOW_STR_MAX))
    {
        uint32 zero_buffer[DBAL_MAX_SIZE_FOR_GLOBAL_ZERO_BUFFER] = { 0 };
        dbal_table_field_info_t table_field;

        SHR_IF_ERR_EXIT(dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle));

        SHR_IF_ERR_EXIT(dbal_tables_field_info_get(unit, entry_handle->table_id, DBAL_FIELD_RESULT_TYPE, 0,
                                                   entry_handle->cur_res_type, 0, &table_field));

        /** Setting the result type location in the buffer to 0 */
        SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(zero_buffer, table_field.bits_offset_in_buffer,
                                                       table_field.field_nof_bits,
                                                       &(entry_handle->phy_entry.payload[0])));

        /** Setting the vxlan field with 0, and checking now that the entry is default */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FWD_DOMAIN_NWK_NAME, INST_SINGLE, 0);

        if (sal_memcmp
            (zero_buffer, entry_handle->phy_entry.payload,
             WORDS2BYTES(BITS2WORDS(entry_handle->table->max_payload_size))) == 0)
        {
            (*is_entry_related) = 0;
        }
    }
    ...
    dnx_flow_app_config_t * flow_app_info)
{
    uint32 entry_handle_id;
    dbal_tables_e dbal_table_id = gport_hw_resources->outlif_dbal_table_id;
    uint32 is_created_by_flow = TRUE;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_flow_egress_virtual_is_created_by_flow(unit, no_local_lif_gport, &is_created_by_flow));
        if (!is_created_by_flow)
        {
    ...
    uint32 selected_result_type)
{
    ...
    if (dnx_flow_app_is_ind_set(unit, flow_app_info, FLOW_APP_INIT_IND_SHARED_PHY_DB_MANAGEMENT))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_shared_db_management_set_value_fields
                        (unit, flow_app_info, entry_handle_id, entry_handle_id_for_hw_set, dbal_table_id,
                         selected_result_type));
        DBAL_HANDLE_COPY(unit, entry_handle_id_for_hw_set, &get_entry_handle);
        if (dnx_data_esem.feature.feature_get(unit, dnx_data_esem_feature_esem_is_dpc))
        {
            if (dbal_table_id != DBAL_TABLE_ESEM_DEFAULT_RESULT_TABLE)
            {
                dbal_entry_key_field32_set(unit, get_entry_handle, DBAL_FIELD_CORE_ID, 0);
            }
        }
        rv = dbal_entry_get(unit, get_entry_handle, DBAL_GET_ALL_FIELDS);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
        if (rv == _SHR_E_NONE)
        {
            int is_entry_related;
            SHR_IF_ERR_EXIT(dnx_flow_is_entry_related_to_flow
                            (unit, get_entry_handle, flow_app_info, &is_entry_related));
            if (is_entry_related)
            {
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT(dnx_flow_egress_virtual_entry_not_created_by_flow_set(unit, flow_handle_info->flow_id));
            }
        }
    }
    else
