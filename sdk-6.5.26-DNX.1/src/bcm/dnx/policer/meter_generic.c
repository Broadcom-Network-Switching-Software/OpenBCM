    ...
    int global_sharing)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_meter_db.generic_meter_info.db_info.use_single_bucket_bank.
                    get(unit, core_id, is_ingress, database_id, &use_single_bucket_bank));
    /** disable refresh mechanism in order to get access to the HW table */
    SHR_IF_ERR_EXIT(dnx_meter_generic_refresh_mechanism_enable(unit, core_id, is_ingress, database_id, FALSE));
    if (use_single_bucket_bank)
    {
    ...
    int *valid)
{
    ...
    *valid = TRUE;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_meter_db.generic_meter_info.db_info.use_single_bucket_bank.
                        get(unit, core_idx, is_ingress, database_id, &single_bucket_bank));
