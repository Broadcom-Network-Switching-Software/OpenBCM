...
/* convert rate compensation from float to int */
#define _BCM_SAT_GTF_RATE_CMP_CONVERSION (1000000.0)
...
#define _BCM_SAT_GTF_DFT_GRANULARITY_IN_PPS_MODE             1                       /* One packet per second */
#define _BCM_SAT_GTF_DFT_GRANULARITY_IN_BPS_MODE             125                     /* 1 byte per 1/125 second, 1kbps */
#define _BCM_SAT_GTF_DFT_GRANULARITY_IN_BPS_MODE_JR          8000                    /* 1 byte per 1/8000 second, 64kbps */
...
typedef struct _bcm_sat_data_s {
    ...
    uint8         granularity_set;
    uint8         is_dirty;
    uint8         is_initialized;
    uint32        gtf_length_compensation[_BCM_SAT_GTF_ID_MAX+1];
    uint32        gtf_rate_factor[_BCM_SAT_GTF_ID_MAX+1][BCM_SAT_GTF_NUM_OF_PRIORITIES];
    uint32        gtf_cfg_rate[_BCM_SAT_GTF_ID_MAX+1][BCM_SAT_GTF_NUM_OF_PRIORITIES];
} _bcm_sat_data_t;
...
static uint8 _bcm_common_sat_granularity_is_set(int unit) {
    return  sat_data[unit].granularity_set;
}

static uint8 _bcm_common_sat_granularity_set(int unit, int granularity, int clr_0_set_1) {
    uint32 freq_hz = 0;
    ...
    if (SOC_IS_JERICHO(unit)) {
        freq_hz = arad_chip_ticks_per_sec_get(unit);
    }
#endif

    if (clr_0_set_1 == 0) {
        granularity = _BCM_SAT_GTF_DFT_GRANULARITY_IN_BPS_MODE;

        if (SOC_IS_JERICHO(unit)) {
            granularity = _BCM_SAT_GTF_DFT_GRANULARITY_IN_BPS_MODE_JR;
        }
    }

    clocks_per_cycle = freq_hz/granularity;
    rv = MBCM_SAT_DRIVER_CALL(unit, mbcm_sat_clock_per_cycle_set, (unit, clocks_per_cycle));

    if (BCM_SUCCESS(rv)) {
        sat_data[unit].granularity_set = clr_0_set_1;
    }

    return rv;
}

STATIC int _bcm_common_gtf_cfg_rate_get (
    int unit,
    ...
    int *packet_length_compensation)
{
    ...
    *packet_length_compensation = psat_data->gtf_length_compensation[gtf_id];
    *gtf_rate_factor = psat_data->gtf_rate_factor[gtf_id][priority];
    *gtf_config_rate = psat_data->gtf_cfg_rate[gtf_id][priority];

    return rv;
}

STATIC int _bcm_common_gtf_cfg_rate_set (
    int unit,
    _bcm_sat_data_t* psat_data,
    bcm_sat_gtf_t gtf_id,
	int priority,
    ...
    int packet_length_compensation)
{
    int rv = BCM_E_NONE;
    uint32 max_rate;
	max_rate = gtf_config_rate * ((float) gtf_rate_factor / _BCM_SAT_GTF_RATE_CMP_CONVERSION);

	if (SOC_IS_QUX(unit)) {
		 SAT_VALUE_MAX_CHECK(max_rate, (_BCM_SAT_GTF_BW_RATE_MAX_QUX+1), "rate");
	} else if (SOC_IS_QAX(unit)) {
		 SAT_VALUE_MAX_CHECK(max_rate, (_BCM_SAT_GTF_BW_RATE_MAX_QAX+1), "rate");
	} else {
		 SAT_VALUE_MAX_CHECK(max_rate, (_BCM_SAT_GTF_BW_RATE_MAX+1), "rate");
	}

    psat_data->gtf_length_compensation[gtf_id] = packet_length_compensation;
    psat_data->gtf_rate_factor[gtf_id][priority] = gtf_rate_factor;
    psat_data->gtf_cfg_rate[gtf_id][priority] = gtf_config_rate;
...
int bcm_common_sat_init(int unit)
{
    ...
    {
        rv = _bcm_common_gtf_cfg_rate_set(unit, psat_data, gtf_id, bcmSatGtfPriCir, 0, _BCM_SAT_GTF_RATE_CMP_CONVERSION, 0);
        if (rv != SOC_E_NONE)
        {
            return rv;
        }
        rv = _bcm_common_gtf_cfg_rate_set(unit, psat_data, gtf_id, bcmSatGtfPriEir, 0, _BCM_SAT_GTF_RATE_CMP_CONVERSION, 0);
        if (rv != SOC_E_NONE)
        {
            return rv;
        }
    ...
    bcm_sat_config_t* conf)
{
    ...
    conf->timestamp_format =
        (bcm_sat_timestamp_format_t)soc_conf.timestamp_format;
#endif /* BCM_SAT_SUPPORT */
...

{
    ...
    soc_conf.timestamp_format =
        (soc_sat_timestamp_format_t)conf->timestamp_format;
    _BCM_SAT_SYSTEM_LOCK(unit);
    ...
    )
{
    ...
    if(psat_data->gtf_id_bitmap == 0){
        /* Restore granularity to default if all GTFs are destroyed */
        rv =_bcm_common_sat_granularity_set(unit, 0, 0);
        if (rv != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_SAT,
                    (BSL_META_U(unit,"Fail(%s) restoring default granularity\n"), soc_errmsg(rv)));
        }
    }
#endif/* BCM_SAT_SUPPORT */
    ...
    )
{
    ...
    uint32 gtf_config_rate = 0, gtf_rate_factor = 0;
    uint32 cir_rate_factor = 0;
    uint32 eir_rate_factor = 0;
    int packet_length_compensation;
    uint32 granularity;
    ...
    if (!SOC_IS_QAX(unit) && (bw->flags & BCM_SAT_GTF_RATE_IN_PACKETS)) {
        LOG_ERROR(BSL_LS_BCM_SAT,(BSL_META_U(unit,"Fail(%s) pps mode is not supported by this device \n"), soc_errmsg(BCM_E_PARAM)));
        return BCM_E_PARAM;
    }

    /*
     * Granularity should be set in the first time we configuring the bandwidth
     * for a GTF. Otherwise the default granularity is used.
     * It should not be changed when there are GTFs configured with bandwidth.
     */
    if (bw->flags & BCM_SAT_GTF_RATE_WITH_GRANULARITY) {
        ...
        if (_bcm_common_sat_granularity_is_set(unit)) {
        ...
        /* setting granularity*/
        rv =_bcm_common_sat_granularity_set(unit, bw->granularity, 1);
        if (rv != BCM_E_NONE) {
            return rv;
        }
    }
    ...
    /*
     * If we reach here with granularity not set, this is the case
     * when we configure GTF bandwidth without with_granularity flag set
     * for the first GTF. Set granularity to default value.
     */
    if (!_bcm_common_sat_granularity_is_set(unit)) {
        granularity = (SOC_IS_QAX(unit) && bw->flags & BCM_SAT_GTF_RATE_IN_PACKETS) ? _BCM_SAT_GTF_DFT_GRANULARITY_IN_PPS_MODE
                      : (SOC_IS_JERICHO(unit) ? _BCM_SAT_GTF_DFT_GRANULARITY_IN_BPS_MODE_JR : _BCM_SAT_GTF_DFT_GRANULARITY_IN_BPS_MODE);

        rv =_bcm_common_sat_granularity_set(unit, granularity, 1);
        if (rv != BCM_E_NONE) {
            return rv;
        }
    }
    ...
    if(priority != bcmSatGtfPriAll){
        /*get old rate compensation*/
        rv = _bcm_common_gtf_cfg_rate_get(unit, psat_data, gtf_id, priority, &gtf_config_rate, &gtf_rate_factor, &packet_length_compensation);
        if (rv != BCM_E_NONE) {
            return rv;
        }
        /*update old rate compensation*/
        rv = _bcm_common_gtf_cfg_rate_set(unit,psat_data,gtf_id, priority, bw->rate, gtf_rate_factor,packet_length_compensation);
        if (rv != BCM_E_NONE) {
            return rv;
        }
    }
    else {
        /* Ensure rate to cover both cir&eir, EVC rate factor should get the max of cir/eir*/
        rv = _bcm_common_gtf_cfg_rate_get(unit, psat_data, gtf_id, bcmSatGtfPriCir, &gtf_config_rate, &cir_rate_factor, &packet_length_compensation);
        if (rv != BCM_E_NONE) {
            return rv;
        }
        rv = _bcm_common_gtf_cfg_rate_get(unit, psat_data, gtf_id, bcmSatGtfPriEir, &gtf_config_rate, &eir_rate_factor, &packet_length_compensation);
        if (rv != BCM_E_NONE) {
            return rv;
        }
        if(cir_rate_factor > eir_rate_factor){
            gtf_rate_factor = cir_rate_factor;
        }
        else{
            gtf_rate_factor = eir_rate_factor;
        }
    }
    ...
    _BCM_SAT_SYSTEM_LOCK(unit);
     rv = MBCM_SAT_DRIVER_CALL(unit, mbcm_sat_gtf_bandwidth_set, (unit, gtf_id, priority, &soc_bandwidth, gtf_rate_factor));
    _BCM_SAT_SYSTEM_UNLOCK(unit);
#endif/* BCM_SAT_SUPPORT */
    ...
    )
{
    ...
    _bcm_sat_data_t *psat_data;
    soc_sat_gtf_bandwidth_t soc_bandwidth;
    uint32 gtf_config_rate = 0; 
    uint32 gtf_rate_factor = 0;
    int packet_length_compensation = 0;
    SAT_INIT_CHECK(unit);
    ...
    sal_memset(&soc_bandwidth, 0, sizeof(soc_bandwidth));
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        bw->rate = gtf_config_rate ? gtf_config_rate : soc_bandwidth.rate;
        if (!(soc_bandwidth.flags & BCM_SAT_GTF_RATE_IN_PACKETS) && gtf_rate_factor){
            /*
             * Reverse the compensation for IPG and SAT headers
             */
            bw->rate = bw->rate * (_BCM_SAT_GTF_RATE_CMP_CONVERSION / gtf_rate_factor);
        }
        bw->granularity = soc_bandwidth.granularity;
        bw->max_burst = soc_bandwidth.max_burst;
        bw->flags = soc_bandwidth.flags;

        if (((bw->flags & BCM_SAT_GTF_RATE_IN_PACKETS) && (bw->granularity != _BCM_SAT_GTF_DFT_GRANULARITY_IN_PPS_MODE))
            || (!(bw->flags & BCM_SAT_GTF_RATE_IN_PACKETS) && (bw->granularity !=
                (SOC_IS_JERICHO(unit) ? _BCM_SAT_GTF_DFT_GRANULARITY_IN_BPS_MODE_JR : _BCM_SAT_GTF_DFT_GRANULARITY_IN_BPS_MODE)))) {
    ...
    )
{
    ...
    int idx_len;
    int priority;
    uint32 gtf_rate_factor[BCM_SAT_GTF_NUM_OF_PRIORITIES] = {_BCM_SAT_GTF_RATE_CMP_CONVERSION,_BCM_SAT_GTF_RATE_CMP_CONVERSION};
    int stamp_count[bcmSatStampCount] = {0};
    uint8 pkt_data[128];
    int pkt_len_idx = 0;
    bcm_sat_gtf_bandwidth_t bw[BCM_SAT_GTF_NUM_OF_PRIORITIES];
    soc_sat_gtf_bandwidth_t soc_bandwidth;
    int num_of_len_pattern = 0;
    float avg_gtf_pkt_len[BCM_SAT_GTF_NUM_OF_PRIORITIES] = {0};
    int total_gtf_pkt_len[BCM_SAT_GTF_NUM_OF_PRIORITIES] = {0};
    SAT_INIT_CHECK(unit);
    ...
    soc_pkt_cfg->sat_header_type = (soc_sat_header_type_t)config->sat_header_type;
    ...
    soc_pkt_cfg->payload.payload_type =
        (soc_sat_payload_type_t)config->payload.payload_type;
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
        ...
        for (i = 0; i < config->packet_edit[priority].number_of_stamps; i++) {
            soc_pkt_cfg->packet_edit[priority].stamps[i].stamp_type = (soc_sat_stamp_type_t) config->packet_edit[priority].stamps[i].stamp_type;
            soc_pkt_cfg->packet_edit[priority].stamps[i].field_type = (soc_sat_stamp_field_t) config->packet_edit[priority].stamps[i].field_type;
            if ((config->packet_edit[priority].stamps[i].stamp_type == bcmSatStampCounter8Bit) || 
                (config->packet_edit[priority].stamps[i].stamp_type == bcmSatStampCounter16Bit)) {
        ...
        bcm_sat_gtf_bandwidth_t_init(&bw[priority]);
        rv = bcm_common_sat_gtf_bandwidth_get (unit,gtf_id,priority,&bw[priority]);
        if (rv != BCM_E_NONE) {
            sal_free(soc_pkt_cfg);
            return rv;
        }
        num_of_len_pattern = 0;
        for (idx_len = 0; idx_len < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; idx_len++)
        {
            if (config->packet_edit[priority].packet_length[idx_len])
            {
                total_gtf_pkt_len[priority] += config->packet_edit[priority].packet_length[idx_len];
                num_of_len_pattern++;
            }
        }
        if (num_of_len_pattern)
        {
            avg_gtf_pkt_len[priority] = (float) total_gtf_pkt_len[priority] / num_of_len_pattern;
            gtf_rate_factor[priority] =
                (int) ((avg_gtf_pkt_len[priority] / (avg_gtf_pkt_len[priority] + config->offsets.packet_length_compensation))* _BCM_SAT_GTF_RATE_CMP_CONVERSION);
        }
        if (config->offsets.packet_length_compensation < ((int) (0 - avg_gtf_pkt_len[priority] + 1)) || config->offsets.packet_length_compensation >= ((int) avg_gtf_pkt_len[priority])) {
            LOG_ERROR(BSL_LS_BCM_SAT,(BSL_META_U(unit,"Fail(%s) gtf packet length compensation\n"), soc_errmsg(BCM_E_PARAM)));
            sal_free(soc_pkt_cfg);
            return BCM_E_PARAM;
        }
        rv = _bcm_common_gtf_cfg_rate_set(unit,psat_data,gtf_id, priority, bw[priority].rate, gtf_rate_factor[priority], config->offsets.packet_length_compensation);
        if (rv != BCM_E_NONE) {
            sal_free(soc_pkt_cfg);
            return rv;
        }
        soc_bandwidth.rate = bw[priority].rate;
        soc_bandwidth.max_burst = bw[priority].max_burst;
        soc_bandwidth.flags = bw[priority].flags;
        soc_bandwidth.granularity = bw[priority].granularity;
		
        _BCM_SAT_SYSTEM_LOCK(unit);
        rv = MBCM_SAT_DRIVER_CALL(unit, mbcm_sat_gtf_bandwidth_set, (unit, gtf_id, priority, &soc_bandwidth,gtf_rate_factor[priority]));
        _BCM_SAT_SYSTEM_UNLOCK(unit);
    }
    ...
    )
{
    ...
    soc_rate_pattern.rate_pattern_mode =
        (soc_sat_gtf_rate_pattern_mode_t) config->rate_pattern_mode;
    ...
    )
{
    ...
    if (BCM_SUCCESS(rv)) {
        config->rate_pattern_mode =
           (bcm_sat_gtf_rate_pattern_mode_t) soc_rate_pattern.rate_pattern_mode;
    ...
    )
{
    ...
    stat_type = (soc_sat_gtf_stat_counter_t) type;
    ...
    )
{
    ...
    soc_packet_info.sat_header_type = (soc_sat_header_type_t)packet_info->sat_header_type;
    soc_packet_info.payload.payload_type =
        (soc_sat_payload_type_t) packet_info->payload.payload_type;
...
bcm_common_sat_wb_sync(int unit, int sync)
{
    ...
    if (SOC_FAILURE(rv) && (rv != SOC_E_NOT_FOUND)) {
        LOG_ERROR(BSL_LS_BCM_SAT,
                  (BSL_META_U(unit,
                              "Error(%s) reading scache. scache_ptr:%p and len:%d\n"),
                   soc_errmsg(rv), (void *)scache_ptr, scache_len));
        return rv;
    }
