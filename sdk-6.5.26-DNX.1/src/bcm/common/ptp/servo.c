...
{   
    ...
    {
        if (g_apts_enabled == 1){
            /*This is a special case where the profile is G.8275.1 but Ext servo is used only for GPS filtering.
             *This particular configuration is only supported in the case where ALL PTP ports are MASTER_ONLY
             *i.e., This is a GM configuration with EXT servo being used for filtering the GPS reference*/
    ...
    bcm_ptp_servo_performance_t *perf)
{
    ...
    sal_memset(&serv_perf_details, 0, sizeof(_bcm_ptp_servo_performance_t));
