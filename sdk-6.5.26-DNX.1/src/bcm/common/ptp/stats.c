...
static bcm_ptp_packet_counters_t g_sdk_packet_counters;
    ...
    uint32 counter_bitmap)
{
    ...
    /* FW counter were cleared successfully. Now clear SDK counters as well*/
    sal_memset(&g_sdk_packet_counters, 0, sizeof(bcm_ptp_packet_counters_t));
...
{   
    int rv = BCM_E_NONE;
    rv = _bcm_ptp_packet_counters_msg_get(unit, ptp_id, clock_num, counters);

    if (rv != BCM_E_NONE){
        return rv;
    } else {
        int index = 0;
        counters->enduro_sync_packets_received += g_sdk_packet_counters.enduro_sync_packets_received;
        counters->enduro_sync_packets_transmitted += g_sdk_packet_counters.enduro_sync_packets_transmitted;
        counters->ipv4_packets_received += g_sdk_packet_counters.ipv4_packets_received;
        counters->ipv6_packets_received += g_sdk_packet_counters.ipv6_packets_received;
        counters->l2_ptp_packets_received += g_sdk_packet_counters.l2_ptp_packets_received;
        counters->packets_discarded += g_sdk_packet_counters.packets_discarded;
        counters->packets_received += g_sdk_packet_counters.packets_received;
        counters->packets_transmitted += g_sdk_packet_counters.packets_transmitted;
        counters->rcpu_encap_packets_received += g_sdk_packet_counters.rcpu_encap_packets_received;
        counters->rx_queue_overflows += g_sdk_packet_counters.rx_queue_overflows;
        counters->udp_ptp_packets_received += g_sdk_packet_counters.udp_ptp_packets_received;

        for (index = 0; index < PTP_MAX_CLOCK_INSTANCE_PORTS; index++){
            counters->port_packets_discarded[index] += g_sdk_packet_counters.port_packets_discarded[index];
            counters->port_packets_received[index] += g_sdk_packet_counters.port_packets_received[index];
            counters->port_packets_transmitted[index] += g_sdk_packet_counters.port_packets_transmitted[index];
        }
    }
        ...
        uint32 packets_discarded)
{
    ...
    if ((ptpPortNum < 1) || (ptpPortNum > PTP_MAX_CLOCK_INSTANCE_PORTS)) {
        rv = BCM_E_PARAM;
        goto exit;
    }
    g_sdk_packet_counters.packets_discarded += packets_discarded;
    g_sdk_packet_counters.packets_transmitted += packets_transmitted;
    g_sdk_packet_counters.packets_received += packets_received;

    switch (protocol){
    case bcmPTPUDPIPv4:
        g_sdk_packet_counters.ipv4_packets_received += packets_received;
        break;
    case bcmPTPUDPIPv6:
        g_sdk_packet_counters.ipv6_packets_received += packets_received;
        break;
    case bcmPTPIEEE8023:
        g_sdk_packet_counters.l2_ptp_packets_received += packets_received;
        break;
    default:
        rv = BCM_E_PARAM;
        goto exit;
    ...
    g_sdk_packet_counters.port_packets_discarded[ptpPortNum - 1] += packets_discarded;
    g_sdk_packet_counters.port_packets_received[ptpPortNum - 1] += packets_received;
    g_sdk_packet_counters.port_packets_transmitted[ptpPortNum - 1] += packets_transmitted;
    ...
    unsigned remote_port_num)
{
    ...
    for (index = 0; index < peerTable->num_peers; index++) {
        if ((localPortNum == 0xffff) ||
            ...
            if (!_bcm_ptp_port_address_cmp(port_addr, &peerTable->peer[index].port_address)) {
                ...
                if (clockID) {
                    sal_memcpy(peerTable->peer[index].clock_identity, clockID, BCM_PTP_CLOCK_EUID_IEEE1588_SIZE);
                }
                ...
                if (PTP_UC_FEATURE_CHECK(PTP_REMOTE_PORT_NUMBER)) {
                    peerTable->peer[index].remote_port_number = remote_port_num;
                }
    ...
    if (clockID) {
        ...
        sal_memset(peerTable->peer[index].clock_identity, 0, BCM_PTP_CLOCK_EUID_IEEE1588_SIZE);
    }
    ...
    int port_num)
{
    ...
    while (1) {
         ...
         *    Octet 45..46 PEER#0  peer port number
         *    Octet 47..81 PEER#1 (as PEER#0)
         *    Octet 82....
         */
        ...
        if (PTP_UC_FEATURE_CHECK(PTP_REMOTE_PORT_NUMBER)) {
            expected_per_peerdata_resp_len += 2;
        }
        ...
        for (index = 0; index < resp[0]; index++) {
            ...
            unsigned remote_port_num = 0;

            localPortNum = _bcm_ptp_uint16_read(resp + offset);
            offset += 2;

            sal_memcpy(clockIDStorage, resp + offset, BCM_PTP_CLOCK_EUID_IEEE1588_SIZE);
            ...
            if (addr_len) {
                sal_memcpy(port_addr.address, resp + offset, addr_len);
            } else {
                return BCM_E_INTERNAL;
            }
            offset += 16;
            ...
            if (PTP_UC_FEATURE_CHECK(PTP_REMOTE_PORT_NUMBER)) {
                remote_port_num = _bcm_ptp_uint16_read(resp + offset);
            }
            ...
            if (total_pkt) {
                if (BCM_FAILURE(rv = _bcm_ptp_update_peer_counts(unit, ptp_id, clock_num, localPortNum, clockID,
                        ...
                        delayresps, mgmts, signals, rejected, &phy_port, remote_port_num))) {
                    return rv;
                }
            }
        }
    ...
    _bcm_ptp_clock_description_t *description)
{
    ...
    sal_memcpy(description->data,resp,resp_len);
    return BCM_E_NONE;
}
