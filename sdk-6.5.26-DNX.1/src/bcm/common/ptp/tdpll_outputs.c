...
#if defined (BCM_MONTEREY_SUPPORT) && defined (INCLUDE_GDPLL)
extern int _bcm_common_gdpll_holdover_set(int unit, int chan, uint8 enable);
extern int _bcm_common_gdpll_holdover_get(int unit, int chan, uint8 *enable);
extern int _bcm_common_gdpll_pbo_set(int unit, int chan);
extern int _bcm_common_gdpll_LoopFilter_set(int unit, int chan, int64 loop_filter);
#endif
...
extern int _bcm_common_gdpll_tdpllinstance_bind(int unit, int gdpllChan, uint32 out_clock_offset);
    ...
    int *enable)
{
    ...
    if (clock_index < BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC) {
        ...
        if (gdpllChan != -1) {
            rv = bcm_gdpll_chan_state_get(unit, gdpllChan, &status);
            if (BCM_FAILURE(rv)) {
                PTP_ERROR_FUNC("bcm_gdpll_chan_state_get()");
                return rv;
            }
        }
        ...
        int reference_clock_idx)
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        rv = _bcm_common_gdpll_tdpllinstance_bind(unit, gdpllChan, clock_index);
        if (BCM_FAILURE(rv)) {
            PTP_ERROR_FUNC("_bcm_common_gdpll_tdpllinstance_bind()");
        }
    }
    return rv;
}
         ...
         int reference_clock_idx)
{
    ...
    if (tdpllout_gdpll_state.tdpll_output_synceport[synce_idx][port-1] == 1) {
        ...
        if (gdpllChan != -1) {
            prev_event_div = _gdpll_event_fb_divisor_get(unit, gdpllChan);
    ...
    int enable)
{
    ...
    if (enable) {
        ...
        _bcm_common_gdpll_LoopFilter_set(unit, apts_gdpll_gps1pps_ts1, curr_loop_filter);

        LOG_INFO(BSL_LS_BCM_PTP,
                (BSL_META_U(0, "APTS: Enabling PBO gdpll_chan:%d\n"),
                apts_gdpll_gps1pps_ts1));

        _bcm_common_gdpll_pbo_set(unit, apts_gdpll_gps1pps_ts1);
    }
...
void _bcm_common_tdpll_gdpll_holdover_get_set(int enable_ho, int gdpll_chan) {
    ...
    _bcm_common_gdpll_holdover_get(0,gdpll_chan,&enabled);
    ...
    if (enable_ho != enabled) {
        ...
        _bcm_common_gdpll_holdover_set(0,gdpll_chan, enable_ho);
    }
}
...
_bcm_common_tdpll_output_pbo_set(int tdpll_outClk_index, int enable_ho)
{
    ...
    if (tdpll_outClk_index < BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC) {
        gdpll_chan = tdpllout_gdpll_state.gdpllChan_miscEvent[tdpll_outClk_index];
        _bcm_common_gdpll_pbo_set(0, gdpll_chan);
    }  else if (tdpll_outClk_index < (BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC + BCM_TDPLL_OUTPUT_CLOCK_NUM_SYNCE)) {
        int port;
        for (port = 1; port <= BCM_GDPLL_NUM_PORTS; port++) {
            if(tdpllout_gdpll_state.tdpll_output_synceport[tdpll_outClk_index - BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC][port-1] == 1){
