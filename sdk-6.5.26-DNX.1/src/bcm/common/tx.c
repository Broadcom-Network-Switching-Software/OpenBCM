 ...
 * 	_bcm_tx_run_get
 * Purpose:
 *	Get value of the static/global '_bcm_tx_run'. Used
 *      for suppressing the 'get' operation under tsan.
 *      Applied only on code which is NOT protected by 'spinlock'.
 * Parameters:
 *	None
 * Returns:
 *	int - Current value of _bcm_tx_run.
 */
static int _bcm_tx_run_get(void)
{
    int ret;
    ret = _bcm_tx_run;
    return (ret);
 ...
 * 	_bcm_tx_run_set
 * Purpose:
 *	Set value of the static/global '_bcm_tx_run'. Used
 *      for potentially controlling the 'set' operation under tsan.
 *      Applied only on code which is NOT protected by 'spinlock'.
 * Parameters:
 *	int value -
 *        Value to load into _bcm_tx_run.
 * Returns:
 *	void
 */
static void _bcm_tx_run_set(int value)
{
    _bcm_tx_run = value;
    return;
}
#define _BCM_TX_RUN_GET() \
    _bcm_tx_run_get()
#define _BCM_TX_RUN_SET(value) \
    _bcm_tx_run_set(value)
/* } */
#else
/* { */
#define _BCM_TX_RUN_GET() \
...
/*
 * Function:
 *      _bcm_xgs3_pbmp_word_get
 * Purpose:
 *      The port is extracted from pbmp.
 * Parameters:
 *      pkt - Packet layout
 *      max_num_port - max port number.
 *      bmap - Port bit map (output)
 *      port - Port number (outpur)
 *      port_adj - Port adjustment (output)
 * Returns:
 *      BCM_E_XXX
 */
static INLINE int
_bcm_xgs3_pbmp_word_get (bcm_pkt_t *pkt, int max_num_port,
                         uint32 *bmap, int *port, int *port_adj)
{
...
_bcm_xgs3_tx_pipe_bypass_header_setup(int unit, bcm_pkt_t *pkt)
{
    ...
    if (soc_feature(unit, soc_feature_visibility)) {
        if (pkt->flags2 & BCM_PKT_F2_RX_PORT) {
            BCM_IF_ERROR_RETURN(_bcm_xgs3_pbmp_word_get(pkt, max_num_port,
                                                      &bmap, &port, &port_adj));
            if (port != -1) {
                /*
                 * For visibility/masquerade, user is not expected to set the
                 * tx_pbmp.
                 */
                return BCM_E_PARAM;
            }
...
/*
 * Function:
 *      _bcm_tx_hg3hdr_setup
 * Purpose:
 *      Setup HiGig3 header local buffer consistently with packet's
 *      data members.
 * Parameters:
 *      pkt - Packet to set up header from
 * Returns:
 ...
 */
STATIC INLINE int
_bcm_tx_hg3hdr_setup(bcm_pkt_t *pkt)
{
    soc_higig3_hdr_t *xgh = (soc_higig3_hdr_t *)pkt->_higig;
    int unit = pkt->unit;
    uint32 prio_val, color;
    uint32 src_port, src_mod;
    bcm_port_t dst_vp = 0, src_vp = 0;
    int multicast, mc_index_offset, mc_index;
    bcm_vlan_t vfi = 0;
#if defined(INCLUDE_L3) && defined(BCM_XGS3_SWITCH_SUPPORT)
    uint32 repl_id;
#endif
    ...
    /* This must be first, so all of the following operate properly */
    soc_higig3_field_set(unit, xgh, HG_ether_type, bcm_td3_tx_hg3_ethertype_get(unit));
    /* Only Base Header + Extension Header 0 is supported regardless of
     * the presence of BCM_PKT_F2_EXT_HG_HDR or not */
    soc_higig3_field_set(unit, xgh, HG_ehv, 2);
    ...
    /* This field is common to all PPDs */
    soc_higig3_field_set(unit, xgh, HG_opcode, pkt->opcode);
    /* Get multicast from opcode.  Override if PPD2 */
    multicast = ((pkt->opcode == SOC_HIGIG_OP_CPU) ||
                 (pkt->opcode == SOC_HIGIG_OP_UC)) ? 0 : pkt->opcode;

    soc_higig3_field_set(unit, xgh, HG_multipoint, multicast ? 1 : 0);
#if defined(INCLUDE_L3) && defined(BCM_XGS3_SWITCH_SUPPORT)
    if (pkt->stk_flags & BCM_PKT_STK_F_ENCAP_ID) {
        BCM_IF_ERROR_RETURN
            (_bcm_tx_pkt_stk_encap_to_repl_id(unit, pkt->stk_encap_id,
                                              &repl_id));
        soc_higig3_field_set(unit, xgh, HG_replication_id, repl_id);
    } else
#endif
    {
        if (multicast) {
             ...
             * of multicast packets with ppd2 header injected by CPU.
             */
            soc_higig3_field_set(unit, xgh, HG_dst_vp, mc_index);
        } else {
            soc_higig3_field_set(unit, xgh, HG_dst_vp, dst_vp);
        }
    }
    soc_higig3_field_set(unit, xgh, HG_src_vp, src_vp);
    soc_higig3_field_set(unit, xgh, HG_vni, vfi);
    if (pkt->stk_flags & BCM_PKT_STK_F_MIRROR) {
        soc_higig3_field_set(unit, xgh, HG_mirror, 1);
    }
    if (pkt->stk_flags & BCM_PKT_STK_F_DO_NOT_LEARN) {
        soc_higig3_field_set(unit, xgh, HG_donot_learn, 1);
    }
    if (pkt->stk_flags & BCM_PKT_STK_F_TRUNK_FAILOVER) {
        soc_higig3_field_set(unit, xgh, HG_lag_failover, 1);
    }
    /* Must transmit VLAN tag with this format */
    pkt->stk_flags |= BCM_PKT_STK_F_TX_TAG;
    ...
    /* Finally, fill in the common FRC fields */
    prio_val = (pkt->flags & BCM_TX_PRIO_INT) ? pkt->prio_int : pkt->cos;
    soc_higig3_field_set(unit, xgh, HG_tc, prio_val);

    /* multicast from opcode or flags */
    if (multicast) {
        mc_index_offset = 0;
        soc_higig3_field_set(unit, xgh, HG_mcst, 1);
        mc_index = _BCM_MULTICAST_ID_GET(pkt->multicast_group);
        soc_higig3_field_set(unit, xgh, HG_mgid, mc_index + mc_index_offset);
    } else {
        soc_higig3_field_set(unit, xgh, HG_dst_mod, pkt->dest_mod);
        soc_higig3_field_set(unit, xgh, HG_dst_port, pkt->dest_port);
    }

    src_mod = (pkt->flags & BCM_TX_SRC_MOD) ?
        pkt->src_mod : SOC_DEFAULT_DMA_SRCMOD_GET(unit);
    soc_higig3_field_set(unit, xgh, HG_src_mod, src_mod);

    src_port = (pkt->flags & BCM_TX_SRC_PORT) ?
        pkt->src_port : SOC_DEFAULT_DMA_SRCPORT_GET(unit);
    soc_higig3_field_set(unit, xgh, HG_src_port, src_port);

    soc_higig3_field_set(unit, xgh, HG_lbid, pkt->stk_load_balancing_number);
    ...
    soc_higig3_field_set(unit, xgh, HG_dp, color);
        ...
        if (!BCM_PKT_TX_ETHER(pkt)) { /* Raw mode */
            ...
            BCM_PBMP_ITER(tx_pbmp, port) {
                ...
                if (SOC_IS_TRIDENT3X(unit) &&
                    ...
                    return _bcm_tx_hg3hdr_setup(pkt);
                } else
#endif
...
_bcm_tx_pkt_tag_setup(int unit, bcm_pkt_t *pkt)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        ...
        if (SOC_IS_TRIDENT3X(unit) &&
            ...
            bcm_port_t tmp_port = 0;
            BCM_PBMP_ITER(pkt->tx_pbmp, tmp_port) {
                if (bcm_td3_is_hg3_enabled_port(unit, tmp_port)) {
                   /* clear HG flag in packet*/
                   BCM_PKT_HGHDR_CLR(pkt);
                   break;
                }
...
bcm_common_tx_deinit(int unit)
{
    ...
    if(SOC_IS_ARAD(unit)) {
        if ((soc_ndev_attached == 0) && (_tx_tid != SAL_THREAD_ERROR)) {
            return BCM_E_INTERNAL;
        }
    }
#endif
...
_xgs3_tx(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    if (NULL != pkt->call_back) {
        ...
        if (SOC_IS_TRIDENT3X(unit) &&
                    soc_feature(unit, soc_feature_higig3)) {
            rv = bcm_td3_tx_hg3_array(unit, packet_pointers_p, pkt_cnt,
                                      &_xgs3_tx_cb, _xgs3_tx_cb_cookie);
        } else
#endif
        {
        ...
        if (SOC_IS_TRIDENT3X(unit) &&
                    soc_feature(unit, soc_feature_higig3)) {
           rv = bcm_td3_tx_hg3_array(unit, packet_pointers_p,
                                     pkt_cnt, NULL, NULL);
        } else
#endif
        {
            ...
            if (SOC_IS_TRIDENT3X(unit) &&
                ...
                if (1 == pkt_cnt && !BCM_PKT_TX_ETHER(pkt)) {
                    ...
                    BCM_PBMP_ITER(pkt->tx_pbmp, tmp_port) {
                        if (bcm_td3_is_hg3_enabled_port(unit, tmp_port)) {
                           /* Tx a HG3 packet */
                           rv = bcm_td3_tx_hg3_pkt(unit, pkt, cookie, _bcm_tx);
...
_tx_pkt_olp_desc_add(int unit, bcm_pkt_t *pkt, dv_t *dv, int pkt_idx)
{
    ...
    sal_memset(&ohdr, 0, sizeof(soc_olp_tx_hdr_t));
    BCM_PKT_HGHDR_CLR(pkt);
    (pkt)->flags &= (~BCM_TX_HG_READY);
    pkt_hg_hdr = NULL;
    ...
    if(!BCM_PKT_TX_HAS_OLP_READY(pkt)) {
        _bcm_tx_olp_hdr_fill(unit, pkt, &ohdr);
    }
    sal_memcpy((uint8 *)&ohdr,BCM_PKT_OLP_HDR(pkt),sizeof(soc_olp_tx_hdr_t));
    opkt.tx_hdr = ohdr;
    ...
    */
    ...
    if (soc_feature(unit, soc_feature_cmicx) && (pkt_hg_hdr != NULL)) {
        ...
        if (soc_feature(unit, soc_feature_hgoe_line_card_mode) &&
            ((pkt->pkt_data[0].data[12] == ((BCM_TD3_HGOE_HDR_TYPE >> 8) & 0xff)) &&
             (pkt->pkt_data[0].data[13] == (BCM_TD3_HGOE_HDR_TYPE & 0xff)))) {

          /* For a HGoE packet, HiGig extension header is already
           * added along with HG2 header while constructing the HGoE
           * header; hence do not check again for extension header.
           */
        } else
#endif
...
_sand_tx_pkt_desc_add(int unit, bcm_pkt_t *pkt, dv_t *dv, int pkt_idx)
{
    ...
    /* Pad runt packets */  
    if ((pkt_len < min_len) && !(pkt->flags & BCM_TX_NO_PAD)) {
        int pad_len = min_len - pkt_len;
        int index_pad = 0;
        LOG_INFO(BSL_LS_BCM_TX,
                 (BSL_META_U(unit,
                             "bcm_tx: packet_pad: ")));
        for (index_pad = 0; index_pad < pad_len ; index_pad ++) {
            LOG_INFO(BSL_LS_BCM_TX,
                     (BSL_META_U(unit,
                                 "%.2x"),_pkt_pad_ptr[index_pad]));
        }
        LOG_INFO(BSL_LS_BCM_TX,
                 (BSL_META_U(unit,
                             "\n")));
