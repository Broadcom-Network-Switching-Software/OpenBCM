    ...
    switch (type.action)
    {
        case bcmSwitchEventMask:
            ...
            else
            {
                ...
                if (access_new_interrupt_available(unit))
                {
                    if (type.index == BCM_CORE_ALL)
                    {
                        SHR_IF_ERR_EXIT(access_interrupt_enable_all_instances
                                        (unit, &interrupts[type.event_id], value ? 0 : 1));
                    }
                    else
                    {
                        SHR_IF_ERR_EXIT(access_interrupt_update
                                        (unit, type.index, &interrupts[type.event_id], value ? 0 : 1));
                    }
                    break;
                }
#endif
        ...
        case bcmSwitchEventForce:
            ...
            if (access_new_interrupt_available(unit))
            {
                if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
                {
                    SHR_IF_ERR_EXIT(access_interrupt_force_all(unit, value ? 0 : 1));
                }
                else
                {
                    SHR_IF_ERR_EXIT(access_interrupt_force
                                    (unit, type.index, &(interrupts[type.event_id]), value ? 0 : 1));
                }
                break;
            }
        ...
        case bcmSwitchEventStormTimedCount:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                ...
                if (access_new_interrupt_available(unit))
                {
                    access_interrupt_storm_timed_count_set_all(unit, value);
                }
                else
#endif
                {
        ...
        case bcmSwitchEventStormTimedPeriod:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                ...
                if (access_new_interrupt_available(unit))
                {
                    access_interrupt_storm_timed_period_set_all(unit, value);
                }
                else
#endif
                {
        ...
        case bcmSwitchEventStormNominal:
            if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL)
            {
                ...
                else if (value == 0)
                {
                    ...
                    if (access_new_interrupt_available(unit))
                    {
                        access_interrupt_storm_nominal_clear_all(unit);
                    }
                    else
#endif
                    {
    ...
    int *temperature_count)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "temperature_max expected to be bigger than 0.\n");
    }
    DNXF_UNIT_LOCK_TAKE(unit);
