...
static unsigned long    sal_alloc_bytes_main_thr_ignore;
static unsigned long    sal_free_bytes_main_thr_ignore;
...
/**
 * \brief
 *     Get the amount of ignored memory allocated and freed bytes counters.
 * \param [out] alloc_bytes_count - Pointer to ignored alloc bytes counter storage.
 * \param [out] free_bytes_count  - Pointer to ignored free bytes counter storage.
 * \return
 *     (void)
 * \remark
 * * None
 ...
 * * None
 */
void sal_get_alloc_counters_main_thr_ignore(unsigned long *alloc_bytes_count, unsigned long *free_bytes_count)
{
    *alloc_bytes_count = sal_alloc_bytes_main_thr_ignore;
    *free_bytes_count = sal_free_bytes_main_thr_ignore;
}

/**
 * \brief
 *     Increment the amount of ignored memory allocated and freed bytes counters.
 * \param [in] alloc_bytes_count - The number of alloc bytes to increment the ignored alloc counter by.
 * \param [in] free_bytes_count  - The number of free bytes to increment the ignored free counter by.
 * \return
 *     (void)
 * \remark
 * * None
 ...
 * * None
 */
void sal_inc_alloc_counters_main_thr_ignore(unsigned long alloc_bytes_inc, unsigned long free_bytes_inc)
{
    sal_alloc_bytes_main_thr_ignore += alloc_bytes_inc;
    sal_free_bytes_main_thr_ignore += free_bytes_inc;
}
 ...
 *      \ref SAL_ALLOC_F_IGNORE - Add the amount of allocated memory to a counter of allocated memory to ignore for testing.
...
/* } */
    ...
    if (sal_thread_self() == sal_thread_main_get())
    {
        ...
        /*
         * If SAL_ALLOC_F_IGNORE flag was provided, add amount of allocated memory to ignored memory allocated counter.
         */
        if (flags & SAL_ALLOC_F_IGNORE_COUNTERS)
        {
           sal_alloc_bytes_main_thr_ignore += sz;
        }
    }
 ...
 * Parameters:
 * \param [in] addr  - Address returned by sal_alloc
   \param [in] flags - Allocation flags.
 *      \ref SAL_ALLOC_F_IGNORE - Add the amount of freed memory to a counter of freed memory to ignore for testing.
 */

void 
sal_free2(void *addr, unsigned int flags)
{
...
/* } */
    ...
    if (sal_thread_self() == sal_thread_main_get())
    {
        ...
        /*
         * If SAL_ALLOC_F_IGNORE flag was provided, add amount of freed memory to ignored memory freed counter.
         */
        if (flags & SAL_ALLOC_F_IGNORE_COUNTERS)
        {
            sal_free_bytes_main_thr_ignore += ap[0] * 4;
        }
    }
...
sal_free(void *addr)
{
    sal_free2(addr, SAL_ALLOC_F_NONE);
}
