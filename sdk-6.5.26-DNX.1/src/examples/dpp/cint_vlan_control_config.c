...
int vlan_control_config_vpn_lower_4k(int unit, int vid, int dflt_frwrd)
{
    ...
    if (is_device_or_above(unit, JERICHO2)){
        ...
        rv = bcm_multicast_create(unit, BCM_MULTICAST_EGRESS_GROUP | BCM_MULTICAST_WITH_ID, &mc_group);
        if (rv != BCM_E_NONE) {
...
int vlan_control_config_vpn_upper_4k(int unit, int vid, int dflt_frwrd)
{
	...
	int cuds[2]={0};
	rv = multicast__open_ingress_mc_group_with_gports(unit, vsi, vlan_control_info.ports, cuds, vlan_control_info.nof_ports, is_device_or_above(unit, JERICHO2) ? 0 : BCM_MULTICAST_TYPE_L2);
    if (rv != BCM_E_NONE) {
...
int vlan_control_config_run(int unit, bcm_vlan_t vid, int vpn_lower_4k, int dflt_frwrd)
{
    ...
    if (!dflt_frwrd)
    {
		rv = multicast__open_ingress_mc_group_with_gports(unit, vlan_control_info.mc_group, vlan_control_info.ports, cuds, vlan_control_info.nof_ports, is_device_or_above(unit, JERICHO2) ? 0 : BCM_MULTICAST_TYPE_L2);
		if (rv != BCM_E_NONE) {
			printf("Error, multicast__open_ingress_mc_group_with_local_ports \n");
			return rv;
		}

		rv = multicast__open_ingress_mc_group_with_gports(unit, vlan_control_info.bc_group, vlan_control_info.ports, cuds, vlan_control_info.nof_ports, is_device_or_above(unit, JERICHO2) ? 0 : BCM_MULTICAST_TYPE_L2);
		if (rv != BCM_E_NONE) {
			printf("Error, multicast__open_ingress_mc_group_with_local_ports \n");
			return rv;
		}
