...
int stacking_mc_lag_func(int unit, int vlan, int mc_type, int mc_id, int *nxt_mc_id, int mc_member_count, int *mc_port_members)
{
    int
        rv = BCM_E_NONE,
        i;
    bcm_gport_t
        mc_gport_members[16];
    bcm_mac_t 
        mc_da  = {0x01, 0x00, 0x5e, 0x00, 0x00, 0x01};
    bcm_l2_addr_t 
        l2addr;

    /* Open Multicast Group  */
    printf("stacking_mc_lag_func(): Open Multicast Group, mc_id=%d, mc_type=%d\n", mc_id, mc_type);
    rv = open_multicast_group(unit, mc_id, mc_type);
    if (!(rv == BCM_E_NONE || rv == BCM_E_EXISTS)) 
    {
        printf("Error, open_multicast_group(). mc_id=%d\n",mc_id);
    ...
    /* Add MC members */
    printf("stacking_mc_lag_func():Add MC members\n");
    for (i = 0; i < mc_member_count; i++) {
        BCM_GPORT_TRUNK_SET(mc_gport_members[i], mc_port_members[i]);
        rv = add_multicast_replication(unit, mc_id, mc_gport_members[i], nxt_mc_id[i], mc_type);
      if (rv != BCM_E_NONE) {
          printf("Error, add_multicast_replication(). mc_gport_members[%d]=0x%x\n", i, mc_gport_members[i]);
          return rv;
    ...
    /* Config mact to send packet to MC-id */
    printf("stacking_mc_lag_func(): Calling bcm_l2_addr_add(), vlan=%d, mc_id=%d\n", vlan, mc_id);
    bcm_l2_addr_t_init(&l2addr, mc_da, vlan);
...
int stacking_flooding_lag_appl(int unit, int tmd, bcm_trunk_t trunk_id)
{
    int
        rv = BCM_E_NONE,
        vlan = 1,
        mc_type = 0, /* 0=ING, 1=EG, Other=Both */
        mc_id = 1,
        mc_member_count = 1;
    int
        mc_port_members[16],
    ...
    if (info.device == 0x8680) {
        /* Allocate outlif to keep the same in mgmt system */
        outlif = 0x1000;
    }

    BCM_GPORT_SUB_TYPE_LIF_SET(vlan_port_id, 0, outlif);
    BCM_GPORT_VLAN_PORT_ID_SET(vlan_port_id, outlif);

    /* Create Outlif in EEDB */
    bcm_vlan_port_t_init(&vlan_port);
    vlan_port.flags = BCM_VLAN_PORT_WITH_ID | BCM_VLAN_PORT_CREATE_EGRESS_ONLY;
    vlan_port.criteria = BCM_VLAN_PORT_MATCH_NONE;
    vlan_port.vlan_port_id = vlan_port_id;
    print bcm_vlan_port_create(unit, &vlan_port);

    mc_port_members[0] = trunk_id; /* trunk id */
    nxt_mc_id[0] = outlif;
    ...
    rv = stacking_mc_lag_func(unit, vlan, mc_type, mc_id, nxt_mc_id, mc_member_count, mc_port_members);
    if (rv != BCM_E_NONE) {
        printf("Error, stacking_mc_func(), rv=%d.\n", rv);
        return rv;
    }
