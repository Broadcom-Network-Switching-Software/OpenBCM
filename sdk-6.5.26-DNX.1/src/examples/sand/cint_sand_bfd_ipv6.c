...
/**
 * SBFD reflector ipv6 endpoint
 */
uint8 is_s_bfd_ipv6_reflector_ep = 0;
...
/*
 * When bfd_endpoint_with_id is not -1, create bfd endpoint with given id
 */
int bfd_endpoint_with_id = 0;
...
int bfd_ipv6_run_with_defaults(int unit, int endpoint_id, int is_acc, int is_single_hop, bcm_bfd_auth_type_t auth_type, int auth_index, int type, int is_trunk, int punt_rate, int use_my_bfd_dip) {
    ...
    if (is_device_or_above(unit,JERICHO2)) {
        ...
        if(is_s_bfd_ipv6_reflector_ep)
        {
            rv = basic_sbfd_reflector_ipv6(unit,bfd_in_port[0],bfd_out_port[0]);
        }
        else
        {
            cint_ip_route_info.force_vsi = 50;
            rv = basic_example_ipv6(unit,bfd_in_port[0],bfd_out_port[0],0,0,arp_plus_ac_type);
            rv = intf_eth_rif_create(unit, cint_ip_route_info.force_vsi, cint_ip_route_info.intf_in_mac_address);
            if (rv != BCM_E_NONE)
            {
                printf("Error, intf_eth_rif_create intf_out = %d, err_id = %d\n", cint_ip_route_info.force_vsi, rv);
                return rv;
            }
        }
        if(use_vxlan6_fec) {
            ...
            ip_tunnel_basic_info.is_native_arp_with_rif = 1;
    ...
    */
            ...
            || use_trap_with_snoop))
    {
        ...
        if ((is_device_or_above(unit,JERICHO2)) && (is_acc)) {
            ...
            full_threshold = *(dnxc_data_get(unit, "oam", "oamp", "oamp_mep_full_entry_threshold", NULL));
            /* 
             * On JR2 with JR2 mode, in case full entry threshold is equal to 0, endpoint id is decoupled from local discr. 
             * in that case, creating endpoint with id is supported
            */
            if ((system_headers_mode != 0) && (full_threshold == 0) && (bfd_endpoint_with_id == 1))
            {
                is_local_dicr_as_session_id = 0;
        ...
        bfd_endpoint_info.remote_detect_mult = 0;
    ...
    if (is_s_bfd_ipv6_reflector_ep) {
        rv = sbfd_reflector_endpoint_create(unit, 0, 0x10, 3,1);
        return rv;
    }
    ...
    if(is_bfd_echo)
    {
        if(is_device_or_above(unit, JERICHO2))
        {
            ...
            rv = cint_field_bfd_echo_entry_add(unit, 0, &src_ipv6, local_disc & 0xFFFF, bfd_endpoint_info.id);
            if (rv != BCM_E_NONE) {
        ...
        {
            int echo_remote_gport;
            echo_remote_gport = bfd_endpoint_info.remote_gport;
            if (!is_acc)  {
                /* Create a new trap for the incoming ECHO packet if the endpoint is non-accelerate */
                bcm_rx_trap_config_t trap_to_host_cpu;
                rv =  bcm_rx_trap_type_create(unit, 0, bcmRxTrapUserDefine, &echo_trap_code);
                if (rv != BCM_E_NONE) {
                   printf("Error in bcm_rx_trap_type_create (%s) \n",bcm_errmsg(rv));
                   return rv;
                }
                bcm_rx_trap_config_t_init(&trap_to_host_cpu);
                trap_to_host_cpu.flags = BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_TRAP;
                trap_to_host_cpu.dest_port = 0;
                rv = bcm_rx_trap_set(unit, echo_trap_code, &trap_to_host_cpu);
                if (rv != BCM_E_NONE) {
                    printf("Error in bcm_rx_trap_set (%s) \n",bcm_errmsg(rv));
                    return rv;
                }
                printf(">>> Trap created echo_trap_code=%d \n",echo_trap_code);
                BCM_GPORT_TRAP_SET(echo_remote_gport, echo_trap_code, 7, 0); /*Taken from default values*/
            }
            if (verbose >= 1) {
                printf("Step: bfd Ipv6 echo configuration -local disc %d, remote gport %d \n", local_disc, echo_remote_gport);
            }
            rv = bfd_ipv6_echo_config(unit, type, bfd_in_port[0], local_disc, echo_remote_gport);
            if (rv != BCM_E_NONE) {
                printf("Error bfd_ipv6_echo_config.\n");
                return rv;
            }
        }
    }
