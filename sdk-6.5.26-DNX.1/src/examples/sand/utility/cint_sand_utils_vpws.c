    ...
    int unit)
{
    ...
    if ((!is_device_or_above(unit, JERICHO2)) || 
        (!soc_property_get(unit, "system_headers_mode", 1) && is_device_or_above(unit, JERICHO2)))
    {
    ...
    int unit)
{
    ...
    int estimate_encap_size_required = (is_device_or_above(unit, JERICHO2)) ?
            *dnxc_data_get(unit, "dev_init", "general", "egress_estimated_bta_btr_hw_config", NULL) : 0;
    ...
    int unit)
{
    ...
    if (!(is_device_or_above(unit, JERICHO_PLUS) && !is_qax_a0))
    {
