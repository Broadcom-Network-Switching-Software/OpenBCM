...
/*
 * When set to 1, decouple local_discr with endpoint-ID, local_discr is allow to use the value not multiple of 4.
 */
int decouple_local_discriminator = 0;
...
/*
 * When set to 1, support N:1 bfd server-to-clinet mode, should set decouple_local_discriminator = 1.
 */
int multi_servers = 0;
...
int bfd_server_example(int server_unit, int client_unit) {
    ...
    server_ep.local_discr = decouple_local_discriminator ? (bfd_local_discr - 1) : bfd_local_discr;
    if(decouple_local_discriminator == 1)
    {
    ...
    if((multi_servers == 1) && (decouple_local_discriminator == 1))
    {
        bcm_bfd_endpoint_info_t_init(&server_ep);
        if(is_mpls)
        {
        ...
        server_ep.remote_detect_mult = 30;
        server_ep.flags |= BCM_BFD_ENDPOINT_HW_ACCELERATION_SET;
        server_ep.local_discr = bfd_local_discr;
        server_ep.flags |= BCM_BFD_ENDPOINT_WITH_ID;
        server_ep.id = 4;
     ...
     * SBFD does not have limitations on UDP src port range */
        ...
        rv = bcm_bfd_endpoint_create(client_unit, &server_ep);
        if (rv != BCM_E_NONE)
        {
            printf("(%s) \n", bcm_errmsg(rv));
            return rv;
        }
        client_server_endpoint_id = server_ep.id;
        printf("client_server_endpoint_id = 0x%x\r\n", client_server_endpoint_id);
    }
    ...
    if (bfd_echo != 1) {
        ...
        client_ep.local_discr = decouple_local_discriminator ? (bfd_local_discr - 1) : bfd_local_discr;
        if(decouple_local_discriminator == 1)
        {
            client_ep.flags |= BCM_BFD_ENDPOINT_WITH_ID;
        }
        ...
        if((multi_servers == 1) && (decouple_local_discriminator == 1))
        {
            if(!is_ipv6)
            {
               rv = bcm_rx_trap_type_get(client_unit,0/* flags */ , bcmRxTrapOamBfdIpv4, &trap_code);
               if (rv != BCM_E_NONE)
               {
                   printf("(%s) \n",bcm_errmsg(rv));
               }
           ...
           BCM_GPORT_TRAP_SET(remote_gport, trap_code, 7, 0);
           client_ep_remote_gport = remote_gport;
            ...
            client_ep.id = client_server_endpoint_id;
            ...
            client_ep.remote_gport = remote_gport;
            client_ep.type = bcmBFDTunnelTypeUdp;
            client_ep.local_discr = bfd_local_discr;
            client_ep.flags |= BCM_BFD_ENDPOINT_WITH_ID;
            ...
            client_endpoint_id_2 = client_ep.id;
            printf("client_endpoint_id_2 = 0x%x\r\n", client_endpoint_id_2);
        }
    }
