    ...
    bcm_mac_t dst_mac)
{
    ...
    /* Create MC group */
    rv = bcm_multicast_create(unit, flags, &mc_grp);
    if (rv != BCM_E_NONE) {
    ...
    for (i = 0; i < nof_copies; i++) {
        ...
        rv = bcm_multicast_add(unit, mc_grp, BCM_MULTICAST_INGRESS_GROUP, 1, &rep_array);
        if (rv != BCM_E_NONE) {
            printf("Error in bcm_multicast_add(). Index %d\n", i);
            return rv;
        }
    }
    ...
    int mgmt_channel)
{
    ...
    char *proc_name;
    proc_name = "dnx_flexe_util_L1_eth_add";
    ...
    rv = cint_dyn_port_add_eth_port(unit, port_id, phy_pbmp, rate_mhz, fec_type, flags, mgmt_channel);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in cint_dyn_port_add_eth_port(port %d) ****\n", proc_name, rv, port_id);
        return rv;
    }
    ...
    int get_tx_speed)
{
    ...
    /* Get port resource */
    rv = bcm_port_resource_get(unit, client_id, &resource_get);
    if (rv != BCM_E_NONE) {
        printf("**** Error(%d), fail in bcm_port_resource_get() ****\n", rv);
        return rv;
    }
    ...
    int nof_slots)
{
    ...
    if (dnx_flexe_gen2_support(unit, -1)) {
        rv = bcm_port_flexe_group_cal_active_get(unit, flexe_grp_id, BCM_PORT_FLEXE_RX, &active_rx_cal);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_active_get() ****\n", proc_name, rv);
            return rv;
        }
    }
    ...
    if ( dnx_flexe_gen2_support(unit, -1) && ((cal_flags==0) || (cal_flags & BCM_PORT_FLEXE_RX)) ) {
        rv = bcm_port_flexe_group_cal_active_set(unit, flexe_grp_id, BCM_PORT_FLEXE_RX, active_rx_cal);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_active_set() ****\n", proc_name, rv);
            return rv;
        }
    }
    ...
    int clear_flag)
{
    ...
    if (!clear_flag)
    {
        if (!is_flexe_gen2) {
            ...
            rv = cint_dnx_tdm_sar_example(unit, client_c, client_c, 0, flexe_gen2_tdm_min_size, flexe_gen2_tdm_max_size, tdm_mc_id);
            if (rv != BCM_E_NONE) {
    ...
    bcm_gport_t * mod_gport)
{
    ...
    rv = bcm_port_gport_get(unit, logic_port_id, mod_gport);
    ...
    int client_rx_rate)
{
    ...
    if (is_flexe_gen2) {
        /* Compute grp speed */
        rv = bcm_port_resource_get(unit, flexe_phy_ports[0], &port_res);
        if (rv != BCM_E_NONE) {
    ...
    uint32 speed_flag)
{
    ...
    if (!dnx_flexe_gen2_support(unit, -1) && (client_type == port_type_L1_monitor || client_type == port_type_L2_mgmt_oam || client_type == port_type_L2_mgmt_ptp)) {
        printf("**** %s: Warning - FlexE_Gen1 doesn't support port_type_L1_monitor/port_type_L2_mgmt_oam/port_type_L2_mgmt_ptp. Quit. ****\n", proc_name);
        return BCM_E_NONE;
    }

    /* Create the FlexE client */
    rv = cint_dyn_port_add_flexe_client(unit, client_id, intf_type, channel_id, rx_speed, speed, flags, if_flags, mgmt_channel);
    if (rv != BCM_E_NONE) {
    ...
    int with_c_bit)
{
    ...
    rv = bcm_port_flexe_group_cal_active_get(unit_mux, flexe_grp, BCM_PORT_FLEXE_TX, &active_cal_mux);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_active_get(mux)****\n", proc_name, rv);
        return rv;
    }
    ...
    if (!is_bcm_adapter) {
        rv = bcm_port_flexe_group_cal_active_get(unit_demux, flexe_grp, BCM_PORT_FLEXE_RX, &active_cal_demux);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_active_get(demux) ****\n", proc_name, rv);
            return rv;
        }
    } else {
    ...
    rv = bcm_port_flexe_group_cal_slots_get(unit_mux, flexe_grp, BCM_PORT_FLEXE_TX, active_cal_mux, 800, cal_slots, &total_slots);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_slots_get(mux) ****\n", proc_name, rv);
        return rv;
    }
    for (i = 0; i < total_slots; i++) {
        if (is_rate_increase) {
    ...
    rv = bcm_port_flexe_group_cal_slots_set(unit_mux, flexe_grp, BCM_PORT_FLEXE_TX, inactive_cal_mux, total_slots, cal_slots);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_slots_set(mux) ****\n", proc_name, rv);
        return rv;
    }

    /* Step 2. Set demux.inactive_cal.rx  */
    rv = bcm_port_flexe_group_cal_slots_set(unit_demux, flexe_grp, BCM_PORT_FLEXE_RX, inactive_cal_demux, total_slots, cal_slots);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_slots_set(demux) ****\n", proc_name, rv);
        return rv;
    }

    /* Step 3. mux.set_CR to inactive calendar */
    rv = bcm_port_flexe_oh_set(unit_mux, flexe_grp, BCM_PORT_FLEXE_TX, bcmPortFlexeOhCalRequest, inactive_cal_mux);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_oh_set(CR) ****\n", proc_name, rv);
        return rv;
    }

    /* Step 4. demux.set_g_hao_action AToB/BToA and send CA */
    rv = bcm_port_flexe_group_ghao_action_set(unit_demux, flexe_grp, BCM_PORT_FLEXE_RX, ghao_action);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_ghao_action_set(demux A/B) ****\n", proc_name, rv);
        return rv;
    }
    rv = bcm_port_flexe_oh_set(unit_demux, flexe_grp, BCM_PORT_FLEXE_TX, bcmPortFlexeOhCalAck, inactive_cal_mux);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_oh_set(CA) ****\n", proc_name, rv);
        return rv;
    }

    /* Step 5. mux.set_g_hao_action AToB/BToA and complete */
    rv = bcm_port_flexe_group_ghao_action_set(unit_mux, flexe_grp, BCM_PORT_FLEXE_TX, ghao_action);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_ghao_action_set(mux A/B) ****\n", proc_name, rv);
        return rv;
    }
    rv = bcm_port_flexe_group_ghao_action_set(unit_mux, flexe_grp, BCM_PORT_FLEXE_TX, bcmPortFlexeGHaoComplete);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_ghao_action_set(mux complete) ****\n", proc_name, rv);
        return rv;
    }

    /* Step 6. mux side send C bit */
    if (with_c_bit) {
        rv = bcm_port_flexe_oh_set(unit_mux, flexe_grp, BCM_PORT_FLEXE_TX, bcmPortFlexeOhCalInUse, inactive_cal_mux);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_oh_set(C) ****\n", proc_name, rv);
            return rv;
        }
    } else {
        printf("**** %s: Mux side can't send out C bit. Restoring calendar settings ****\n", proc_name);
        rv = bcm_port_flexe_group_cal_active_set(unit_mux, flexe_grp, BCM_PORT_FLEXE_TX, active_cal_mux);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_active_set() ****\n", proc_name, rv);
            return rv;
        }
    ...
    if (with_c_bit) {
        ...
        rv = bcm_port_flexe_group_ghao_action_set(unit_demux, flexe_grp, BCM_PORT_FLEXE_RX, bcmPortFlexeGHaoComplete);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_group_ghao_action_set(demux complete) ****\n", proc_name, rv);
            return rv;
        }
    } else {
        printf("**** %s: Demux side can't receive C bit. Setting G.HAO action 'Stop' to restore settings ****\n", proc_name);
        rv = bcm_port_flexe_group_ghao_action_set(unit_demux, flexe_grp, BCM_PORT_FLEXE_RX, bcmPortFlexeGHaoStop);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_group_ghao_action_set(demux complete) ****\n", proc_name, rv);
            return rv;
        }
    ...
    int new_speed)
{
    ...
    if (is_speed_increase) {
        ...
        get_tx_speed = 1;
        tx_speed = dnx_flexe_util_client_speed_get(unit_demux, client_dst, get_tx_speed);
        rx_speed = new_speed;
        rv = dnx_flexe_util_client_g_hao_speed_set(unit_demux, client_dst, tx_speed, rx_speed);
        if (rv != BCM_E_NONE) {
            printf("**** Error(%d), fail in dnx_flexe_util_client_g_hao_speed_set(client_dst) ****\n", rv);
            return rv;
        }

        /** Step 2. Change FlexE client(client_busa) speed at both mux and demux side */
        speed_diff = new_speed - old_speed;
        rv = dnx_flexe_g_hao_mux_demux_set(unit_mux, unit_demux, flexe_grp, client_busa, speed_diff, is_speed_increase, 1);
        if (rv != BCM_E_NONE) {
            printf("**** Error(%d), fail in dnx_flexe_g_hao_mux_demux_set() ****\n", rv);
            return rv;
        }
        ...
        get_tx_speed = 0;
        tx_speed = new_speed;
        rx_speed = dnx_flexe_util_client_speed_get(unit_mux, client_src, get_tx_speed);
        rv = dnx_flexe_util_client_g_hao_speed_set(unit_mux, client_src, tx_speed, rx_speed);
        if (rv != BCM_E_NONE) {
            printf("**** Error(%d), fail in dnx_flexe_util_client_g_hao_speed_set(client_src) ****\n", rv);
            return rv;
        }
    } else {
    ...
    int dst_port_2)
{
    ...
    rv |= dnx_flexe_util_client_create(unit, client_a_2, port_type_busA, client_speed, 0);
    if (client_1_is_L1) {
        rv |= dnx_flexe_util_client_create(unit, client_c_1, port_type_busC, client_speed, 0);
    } else {
        rv |= dnx_flexe_util_client_create(unit, client_b_1, port_type_busB, client_speed, 0);
    }
    if (client_2_is_L1) {
        rv |= dnx_flexe_util_client_create(unit, client_c_2, port_type_busC, client_speed, 0);
    } else {
        rv |= dnx_flexe_util_client_create(unit, client_b_2, port_type_busB, client_speed, 0);
    }
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_client_create() ****\n", proc_name, rv);
        return rv;
    ...
    if (client_1_is_L1) {
        ...
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a_1, client_c_1, dst_port_1, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_flexe_sar_dst() ****\n", proc_name, rv);
            return rv;
        }
        /* Build L1 datapath: dst_port_1 --> client_c_1 --> client_a_1 */
        rv = dnx_flexe_util_L1_datapath_src_sar_flexe(unit, dst_port_1, client_c_1, client_a_1, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_src_sar_flexe() ****\n", proc_name, rv);
            return rv;
        }
        ...
        rv = dnx_flexe_util_L2_datapath_flexe_to_dst(unit, client_a_1, client_b_1, unit, dst_port_1, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_flexe_to_dst() ****\n", proc_name, rv);
            return rv;
        }
        /* Build L2 datapath: dst_port_1 --> client_b_1 --> client_a_1 */
        rv = dnx_flexe_util_L2_datapath_src_to_flexe(unit, dst_port_1, unit, client_b_1, client_a_1, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_src_to_flexe() ****\n", proc_name, rv);
            return rv;
        }
    ...
    if (client_2_is_L1) {
        ...
        rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a_2, client_c_2, dst_port_2, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_flexe_sar_dst() ****\n", proc_name, rv);
            return rv;
        }
        /* Build L1 datapath: dst_port_2 --> client_c_2 --> client_a_2 */
        rv = dnx_flexe_util_L1_datapath_src_sar_flexe(unit, dst_port_2, client_c_2, client_a_2, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_src_sar_flexe() ****\n", proc_name, rv);
            return rv;
        }
        ...
        rv = dnx_flexe_util_L2_datapath_flexe_to_dst(unit, client_a_2, client_b_2, unit, dst_port_2, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_flexe_to_dst() ****\n", proc_name, rv);
            return rv;
        }
        /* Build L2 datapath: dst_port_2 --> client_b_2 --> client_a_2 */
        rv = dnx_flexe_util_L2_datapath_src_to_flexe(unit, dst_port_2, unit, client_b_2, client_a_2, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_src_to_flexe() ****\n", proc_name, rv);
            return rv;
        }
...
int dnx_flexe_gen2_mix_datapaths_in_1_grp(
    ...
    int dapath_bmp)
{
    int i, rv, unit_iter, unit, dst_unit, dst_port, valid_datapaths;
    bcm_gport_t flexe_grp;
    int grp_index = 0;
    int nof_pcs = 0;
    int max_datapaths = 7;
    ...
    for (unit_iter = 0; unit_iter < 2; unit_iter++) {
        ...
        for (i = 0; i < max_datapaths; i++) {
            ...
            /** Create BusA clients */
            rv = dnx_flexe_util_client_create(unit, client_a[i], port_type_busA, client_speed, 0);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in dnx_flexe_util_client_create(BusA, unit %d, index %d) ****\n", proc_name, rv, unit, i);
                return rv;
            }
            ...
            if (unit == unit1 || (unit == unit2 && i < 2)) {
                rv = dnx_flexe_util_client_create(unit, client_b[i], port_type_busB, client_speed, 0);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in dnx_flexe_util_client_create(BusB, unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }
            }
            ...
            if ( (unit == unit2 && client_c[i] > 0) || (unit == unit1 && i == 5) ) {
                rv = dnx_flexe_util_client_create(unit, client_c[i], port_type_busC, client_speed, 0);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in dnx_flexe_util_client_create(BusC, unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }
            }
        }
        ...
        for (i = 0; i < max_datapaths; i ++) {
            ...
            if (unit == unit1) {
                ...
                rv = dnx_flexe_util_L2_entry_advanced_set(unit, eth_tx, client_b[i], dst_mac, 1, client_b[i], 1);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_entry_advanced_set(unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }

                /** TX: client_b[i] -> client_a[i] */
                rv = bcm_port_flexe_flow_set(unit, 0, client_b[i], 0, client_a[i]);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set(unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }

                /** RX: client_a[i] -> client_b[i] -> Force_Forward -> eth_rx */
                rv = dnx_flexe_util_L2_datapath_flexe_to_dst(unit, client_a[i], client_b[i], unit, eth_rx, 0);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_flexe_to_dst(unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }
            } else {
                ...
                if (i < 2) {
                    ...
                    rv = dnx_flexe_util_L2_datapath_flexe_to_dst(unit, client_a[i], client_b[i], dst_unit, eth_L2, 0);
                    rv |= dnx_flexe_util_L2_datapath_src_to_flexe(dst_unit, eth_L2, unit, client_b[i], client_a[i], 0);
                    rv |= bcm_port_loopback_set(dst_unit, eth_L2, BCM_PORT_LOOPBACK_MAC);
                } else if (i < 4) {
                    /** L1 datapath: client_a[i] -> client_c[i] -> ilkn_tdm(local or remote)
                     *  i=2, local eth; i=3, remote eth
                     */
                    dst_unit = i == 2 ? unit2 : unit1;
                    dnx_flexe_util_mod_port_get(dst_unit, ilkn_tdm, &mod_gport);
                    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a[i], client_c[i], mod_gport, 0);

                    /** L1 datapath: client_a[i] <- client_c[i] <- dst_unit.ilkn_tdm */
                    dnx_flexe_util_mod_port_get(unit, client_c[i], &mod_gport);
                    rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(dst_unit, ilkn_tdm, mod_gport, client_a[i], 0);
                    rv |= bcm_port_loopback_set(dst_unit, ilkn_tdm, BCM_PORT_LOOPBACK_MAC);
                } else if (i == 4) {
                    /** L1 datapath: client_a[i] <-> eth_L1(local) */
                    rv = bcm_port_flexe_flow_set(unit, 0, client_a[i], 0, eth_L1);
                    rv |= bcm_port_flexe_flow_set(unit, 0, eth_L1, 0, client_a[i]);
                    rv |= bcm_port_loopback_set(unit, eth_L1, BCM_PORT_LOOPBACK_PHY);
                } else if (i == 5) {
                    /** L1 datapath: unit2.client_a[5] <--> unit2.client_c[5] <--> unit1.client_c[5] <--> unit1.eth_L1(to remote ETH_L1, and loopback) */
                    dnx_flexe_util_mod_port_get(unit1, client_c[i], &mod_gport);
                    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit2, client_a[i], client_c[i], mod_gport, 0);
                    dnx_flexe_util_mod_port_get(unit2, client_c[i], &mod_gport);
                    rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(unit1, client_c[i], mod_gport, client_a[i], 0);
                    rv |= bcm_port_flexe_flow_set(unit1, 0, client_c[i], 0, eth_L1);
                    rv |= bcm_port_flexe_flow_set(unit1, 0, eth_L1, 0, client_c[i]);
                    rv |= bcm_port_loopback_set(unit1, eth_L1, BCM_PORT_LOOPBACK_PHY);
                } else if (i == 6) {
                    /** L1 datapath: client_a[6] <--> client_c[6](TDM loopback) */
                    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a[i], client_c[i], client_c[i], 0);
                    rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(unit, client_c[i], client_c[i], client_a[i], 0);
                }
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail to set FlexE datapath at unit2(index %d) ****\n", proc_name, rv, i);
                    return rv;
                }
            }
        }
        ...
        /* Assign calendar slots for BusA clients */
        dnx_flexe_util_cal_slots_clear(FLEXE_CAL_A);
        for (i = 0; i < client_slots * valid_datapaths; i++) {
            Cal_Slots_A[i] = valid_client_a[i/client_slots];
        }
        rv = dnx_flexe_util_calendar_slots_set(unit, flexe_grp, 0, FLEXE_CAL_A, total_slots);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_calendar_slots_set(unit %d) ****\n", proc_name, rv, unit);
...
int dnx_flexe_gen2_mix_datapaths_in_1_grp_clear(
    ...
    int dapath_bmp)
{
    int i, rv, unit_iter, unit, dst_unit, dst_port;
    bcm_gport_t flexe_grp;
    int grp_index = 0;
    int max_datapaths = 7;
    int client_a[7] = {30, 31, 32, 33, 34, 35, 36};
    ...
    for (unit_iter = 0; unit_iter < 2; unit_iter++) {
        ...
        for (i = 0; i < max_datapaths; i ++) {
            ...
            if (unit == unit1) {
                ...
                /** TX: client_b[i] -> client_a[i] */
                rv = bcm_port_flexe_flow_clear(unit, client_b[i], 0, client_a[i]);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_clear(unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }

                /** RX: client_a[i] -> client_b[i] -> Force_Forward -> eth_rx */
                rv = dnx_flexe_util_L2_datapath_flexe_to_dst(unit, client_a[i], client_b[i], unit, eth_rx, 1);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_datapath_flexe_to_dst(unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }
                ...
                if (i < 2) {
                     ...
                     */
                    dst_unit = i == 0 ? unit2 : unit1;
                    rv = dnx_flexe_util_L2_datapath_flexe_to_dst(unit, client_a[i], client_b[i], dst_unit, eth_L2, 1);
                    rv |= dnx_flexe_util_L2_datapath_src_to_flexe(dst_unit, eth_L2, unit, client_b[i], client_a[i], 1);
                    rv |= bcm_port_loopback_set(dst_unit, eth_L2, BCM_PORT_LOOPBACK_NONE);
                } else if (i < 4) {
                    /** L1 datapath: client_a[i] -> client_c[i] -> ilkn_tdm(local or remote)
                     *  i=2, local eth; i=3, remote eth
                     */
                    dst_unit = i == 2 ? unit2 : unit1;
                    dnx_flexe_util_mod_port_get(dst_unit, ilkn_tdm, &mod_gport);
                    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a[i], client_c[i], mod_gport, 1);

                    /** L1 datapath: client_a[i] <- client_c[i] <- dst_unit.ilkn_tdm */
                    dnx_flexe_util_mod_port_get(unit, client_c[i], &mod_gport);
                    rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(dst_unit, ilkn_tdm, mod_gport, client_a[i], 1);
                    rv |= bcm_port_loopback_set(dst_unit, ilkn_tdm, BCM_PORT_LOOPBACK_NONE);
                } else if (i == 4) {
                    /** L1 datapath: client_a[i] <-> eth_L1 */
                    rv = bcm_port_flexe_flow_clear(unit, client_a[i], 0, eth_L1);
                    rv |= bcm_port_flexe_flow_clear(unit, eth_L1, 0, client_a[i]);
                    rv |= bcm_port_loopback_set(unit, eth_L1, BCM_PORT_LOOPBACK_NONE);
                } else if (i == 5) {
                    /** L1 datapath: unit2.client_a[5] <--> unit2.client_c[5] <--> unit1.client_c[5] <--> unit1.eth_L1(to remote ETH_L1, and loopback) */
                    dnx_flexe_util_mod_port_get(unit1, client_c[i], &mod_gport);
                    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit2, client_a[i], client_c[i], mod_gport, 1);
                    dnx_flexe_util_mod_port_get(unit2, client_c[i], &mod_gport);
                    rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(unit1, client_c[i], mod_gport, client_a[i], 1);
                    rv |= bcm_port_flexe_flow_clear(unit1, client_c[i], 0, eth_L1);
                    rv |= bcm_port_flexe_flow_clear(unit1, eth_L1, 0, client_c[i]);
                    rv |= bcm_port_loopback_set(unit1, eth_L1, BCM_PORT_LOOPBACK_NONE);
                } else if (i == 6) {
                    /** L1 datapath: client_a[6] <--> client_c[6](TDM loopback) */
                    rv = dnx_flexe_util_L1_datapath_flexe_sar_dst(unit, client_a[i], client_c[i], client_c[i], 1);
                    rv |= dnx_flexe_util_L1_datapath_src_sar_flexe(unit, client_c[i], client_c[i], client_a[i], 1);
                }
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail to clear FlexE datapath at unit2(index %d) ****\n", proc_name, rv, i);
                    return rv;
                }
            }
        }
        ...
        for (i = 0; i < max_datapaths; i++) {
            ...
            /** Remove BusA clients */
            rv = dnx_flexe_util_client_remove(unit, client_a[i]);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove(BusA, unit %d, index %d) ****\n", proc_name, rv, unit, i);
                return rv;
            }

            /** Remove BusB clients */
            if (unit == unit1 || (unit == unit2 && i < 2)) {
                rv = dnx_flexe_util_client_remove(unit, client_b[i]);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove(BusB, unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }
            }
            ...
            /** Remove BusC clients */
            if ( (unit == unit2 && client_c[i] > 0) || (unit == unit1 && i == 5) ) {
                rv = dnx_flexe_util_client_remove(unit, client_c[i]);
                if (rv != BCM_E_NONE) {
                    printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove(BusC, unit %d, index %d) ****\n", proc_name, rv, unit, i);
                    return rv;
                }
            }
        }
    ...
    int client_b_start)
{
    ...
    while (i < nof_clients) {
        ...
        rv = dnx_flexe_util_client_create(unit, client_id, port_type_busA, client_speed, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to create busA client(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    ...
    while (nof_clients_added < nof_clients) {
        ...
        rv = dnx_flexe_util_client_create(unit, client_id, port_type_busB, client_speed, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to create busB client(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    ...
    for (i = 0; i < nof_clients; i++) {
        rv = bcm_port_flexe_flow_set(unit, 0, client_b_ids[i], 0, client_a_ids[i]);
        rv |= bcm_port_flexe_flow_set(unit, 0, client_a_ids[i], 0, client_b_ids[i]);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    }
    ...
    for (i = 0; i < nof_clients; i++) {
        ...
        rv = egr_vlan_edit(unit, client_b_ids[i], client_b_ids[i+1], in_vid, out_vid);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in egr_vlan_edit(index %d) ****\n", proc_name, rv, i);
            return rv;
        }

        /** Add L2 entry */
        rv = dnx_flexe_util_L2_entry_advanced_set(unit, client_b_ids[i], client_b_ids[i+1], mac_da, 1, in_vid, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L2_entry_advanced_set(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    }

    /* Hook eth ports */
    rv = bcm_port_force_forward_set(unit, eth_tx, client_b_ids[0], 1);
    rv |= bcm_port_force_forward_set(unit, client_b_ids[nof_clients-1], eth_rx, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_force_forward_set() ****\n", proc_name, rv);
        return rv;
    }
    ...
    int client_b_start)
{
    ...
    for (client_a_iter = 0; client_a_iter < nof_clients; client_a_iter++) {
        ...
        if (client_b_id == reserved_port) {
            client_b_iter++;
            client_b_id = client_b_start + client_b_iter;
        }

        /* Clear FlexE flows */
        rv = bcm_port_flexe_flow_clear(unit, client_b_id, 0, client_a_id);
        rv |= bcm_port_flexe_flow_clear(unit, client_a_id, 0, client_b_id);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_clear(index %d) ****\n", proc_name, rv, client_a_iter);
            return rv;
        }

        /* Remove busA client and busB client */
        rv = dnx_flexe_util_client_remove(unit, client_a_id);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to remove busA client(index %d) ****\n", proc_name, rv, client_a_iter);
            return rv;
        }
        rv = dnx_flexe_util_client_remove(unit, client_b_id);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to remove busB client(index %d) ****\n", proc_name, rv, client_a_iter);
            return rv;
        }
        client_b_iter++;
    }
    ...
    int client_c_start)
{
    ...
    while (nof_clients_added < nof_clients) {
        ...
        rv = dnx_flexe_util_client_create(unit, client_id, port_type_busC, client_speed, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to create busC client(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    ...
    /* Hook eth_L1 */
    rv = bcm_port_flexe_flow_set(unit, 0, eth_L1, 0, client_c_ids[0]);
    rv |= bcm_port_flexe_flow_set(unit, 0, client_c_ids[nof_clients-1], 0, eth_L1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set(eth_L1) ****\n", proc_name, rv);
        return rv;
    }
    ...
    for (i = 0; i < nof_clients; i++) {
        if ((i % 2) == 0) {
            rv = cint_dnx_tdm_sar_example_multi_device(unit, unit, client_c_ids[i], client_c_ids[i+1], 0, flexe_gen2_tdm_grp_id, flexe_gen2_tdm_min_size, flexe_gen2_tdm_max_size);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in cint_dnx_tdm_sar_example_multi_device(index %d) ****\n", proc_name, rv, i);
                return rv;
            }
        }
    }
    ...
    for (i = 0; i < nof_clients - 1; i++) {
        if (i % 2) {
            rv = bcm_port_flexe_flow_set(unit, 0, client_c_ids[i], 0, client_c_ids[i+1]);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set(C->A, index %d) ****\n", proc_name, rv, i);
                return rv;
            }
        }
    }
    ...
    int client_c_start)
{
    int i, rv;
    int is_flexe_gen2 = dnx_flexe_gen2_support(unit, -1);
    int client_b_ids[160];
    int client_c_ids[160];
    ...
    for (i = 0; i < nof_clients; i++) {
        ...
        rv = bcm_port_flexe_flow_set(unit, 0, client_b_ids[i], 0, client_c_ids[i]);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set(B->C, index %d) ****\n", proc_name, rv, i);
            return rv;
        }

        /* FlexE flow: BusC_X --> BusB_X+1 */
        dest_client = i == nof_clients -1 ? client_b_ids[0] : client_b_ids[i+1];
        rv = bcm_port_flexe_flow_set(unit, 0, client_c_ids[i], 0, dest_client);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set(C->B, index %d) ****\n", proc_name, rv, i);
            return rv;
        }

        /* Loopback BusB clients(L2_force_fwd) */
        if (i) {
            rv = bcm_port_force_forward_set(unit, client_b_ids[i], client_b_ids[i], 1);
            if (rv != BCM_E_NONE) {
                printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(B->B, index %d) ****\n", proc_name, rv, i);
                return rv;
            }
        }

        /* Loopback BusC clients(TDM_Bypass) */
        rv = cint_dnx_tdm_sar_example_multi_device(unit, unit, client_c_ids[i], client_c_ids[i], 0, flexe_gen2_tdm_grp_id, flexe_gen2_tdm_min_size, flexe_gen2_tdm_max_size);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in cint_dnx_tdm_sar_example_multi_device(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    ...
    /* Hook eth ports */
    rv = bcm_port_force_forward_set(unit, eth_tx, client_b_ids[0], 1);
    rv |= bcm_port_force_forward_set(unit, client_b_ids[0], eth_rx, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_force_forward_set() ****\n", proc_name, rv);
        return rv;
    }
    ...
    int client_c_start)
{
    ...
    for (i = 0; i < nof_clients; i++) {
        /* FlexE flow: BusB_X --> BusC_X */
        rv = bcm_port_flexe_flow_clear(unit, client_b_ids[i], 0, client_c_ids[i]);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_clear(B->C, index %d) ****\n", proc_name, rv, i);
            return rv;
        }

        /* FlexE flow: BusC_X --> BusB_X+1 */
        dest_client = i == nof_clients -1 ? client_b_ids[0] : client_b_ids[i+1];
        rv = bcm_port_flexe_flow_clear(unit, client_c_ids[i], 0, dest_client);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_clear(C->B, index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    }
    ...
    for (i = 0; i < nof_clients; i++) {
        rv = dnx_flexe_util_client_remove(unit, client_b_ids[i]);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove(BusB, index %d) ****\n", proc_name, rv, i);
            return rv;
        }
        rv = dnx_flexe_util_client_remove(unit, client_c_ids[i]);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove(BusC, index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    }
    ...
    bcm_port_t eth_rx)
{
    ...
    /* Create FlexE clients */
    rv = dnx_flexe_util_client_create(unit, client_a_0, port_type_busA, client_speed, 0);
    rv |= dnx_flexe_util_client_create(unit, client_a_1, port_type_busA, client_speed, 0);
    rv |= dnx_flexe_util_client_create(unit, client_a_2, port_type_busA, client_speed, 0);
    rv |= dnx_flexe_util_client_create(unit, client_b, port_type_busB, client_speed, 0);
    rv |= dnx_flexe_util_client_create(unit, client_c_0, port_type_busC, client_speed, 0);
    rv |= dnx_flexe_util_client_create(unit, client_c_1, port_type_busC, client_speed, 0);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_client_create ****\n", proc_name, rv);
        return rv;
    ...
    /* Hook eth ports */
    rv = bcm_port_force_forward_set(unit, eth_tx, client_b, 1);
    rv |= bcm_port_force_forward_set(unit, client_b, eth_rx, 1);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_force_forward_set() ****\n", proc_name, rv);
        return rv;
    }
    ...
    /* FlexE flows */
    rv = bcm_port_flexe_flow_set(unit, 0, client_b, 0, client_a_0);
    rv |= bcm_port_flexe_flow_set(unit, 0, client_a_0, 0, client_c_0);
    rv |= bcm_port_flexe_flow_set(unit, 0, client_c_1, 0, client_a_1);
    rv |= bcm_port_flexe_flow_set(unit, 0, client_a_1, 0, eth_L1);
    rv |= bcm_port_flexe_flow_set(unit, 0, eth_L1, 0, client_a_2);
    rv |= bcm_port_flexe_flow_set(unit, 0, client_a_2, 0, client_b);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_flow_set() ****\n", proc_name, rv);
        return rv;
    }
    ...
    /* TDM setting */
    rv = cint_dnx_tdm_sar_example_multi_device(unit, unit, client_c_0, client_c_1, 0, flexe_gen2_tdm_grp_id, flexe_gen2_tdm_min_size, flexe_gen2_tdm_max_size);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in cint_dnx_tdm_sar_example_multi_device() ****\n", proc_name, rv);
        return rv;
    }
    ...
    for (i = 0; i < nof_pcs; i++) {
        rv = bcm_port_loopback_set(unit, grp_phy_info[i].port, BCM_PORT_LOOPBACK_PHY);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_loopback_set(phy) ****\n", proc_name, rv);
            return rv;
        }
    }
    rv = bcm_port_loopback_set(unit, eth_L1, BCM_PORT_LOOPBACK_PHY);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_loopback_set(eth_L1) ****\n", proc_name, rv);
        return rv;
    }
    ...
    int cfg_index)
{
    ...
    for (i = 0; i < nof_clients; i++) {
        rv = dnx_flexe_util_client_create(unit, client_a_base+i, port_type_busA, client_speed[i], 0);
        rv |= dnx_flexe_util_client_create(unit, client_b_base+i, port_type_busB, client_speed[i], 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to create BusA/BusB client(unit %d, index %d) ****\n", proc_name, rv, unit, i);
            return rv;
        }
    }
    ...
    for (i = 0; i < nof_clients; i++) {
        ...
        rv |= dnx_flexe_util_L1_datapath_client_to_client(unit, client_b_base+i, client_a_base+i, 0);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail to set BusB<->BusA FlexE flow(unit %d, index %d) ****\n", proc_name, rv, unit, i);
            return rv;
        }
    }
    ...
    for (i = 0; i < nof_clients; i++) {
        rv =  bcm_port_force_forward_set(unit, client_b_base+i, BCM_GPORT_BLACK_HOLE, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(unit %d, index %d) ****\n", proc_name, rv, unit, i);
            return rv;
        }
    ...
    for (i = 0; i < nof_clients; i++) {
        rv =  bcm_port_force_forward_set(unit, client_b_base+i, client_b_base+i, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_force_forward_set(unit %d, index %d) ****\n", proc_name, rv, unit, i);
            return rv;
        }
    ...
    if (calendar_cfg == 2) {
        ...
        rv = dnx_flexe_util_calendar_slots_set(unit_0, flexe_grp, BCM_PORT_FLEXE_RX, FLEXE_CAL_A, total_slots);
        rv |= dnx_flexe_util_calendar_slots_set(unit_0, flexe_grp, BCM_PORT_FLEXE_RX, FLEXE_CAL_B, total_slots);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_calendar_slots_set(unit %d, Rx) ****\n", proc_name, rv, unit_0);
            return rv;
        }
        ...
        rv = dnx_flexe_util_calendar_slots_set(unit_1, flexe_grp, BCM_PORT_FLEXE_RX, FLEXE_CAL_A, total_slots);
        rv |= dnx_flexe_util_calendar_slots_set(unit_1, flexe_grp, BCM_PORT_FLEXE_RX, FLEXE_CAL_B, total_slots);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_calendar_slots_set(unit %d, Rx) ****\n", proc_name, rv, unit_1);
            return rv;
        }
    ...
    int nof_clients)
{
    ...
    /* Clear calendar */
    dnx_flexe_util_cal_slots_clear(FLEXE_CAL_A);
    dnx_flexe_util_cal_slots_clear(FLEXE_CAL_B);
    rv = dnx_flexe_util_calendar_slots_set(unit_0, flexe_grp, cal_flags, FLEXE_CAL_A, total_slots);
    rv |= dnx_flexe_util_calendar_slots_set(unit_0, flexe_grp, cal_flags, FLEXE_CAL_B, total_slots);
    rv |= dnx_flexe_util_calendar_slots_set(unit_1, flexe_grp, cal_flags, FLEXE_CAL_A, total_slots);
    rv |= dnx_flexe_util_calendar_slots_set(unit_1, flexe_grp, cal_flags, FLEXE_CAL_B, total_slots);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_calendar_slots_set() ****\n", proc_name, rv);
        return rv;
    }
    ...
    /* Clear FlexE flows */
    for (i = 0; i < nof_clients; i++) {
        rv = dnx_flexe_util_L1_datapath_client_to_client(unit_0, client_a_base+i, client_b_base+i, 1);
        rv |= dnx_flexe_util_L1_datapath_client_to_client(unit_0, client_b_base+i, client_a_base+i, 1);
        rv |= dnx_flexe_util_L1_datapath_client_to_client(unit_1, client_a_base+i, client_b_base+i, 1);
        rv |= dnx_flexe_util_L1_datapath_client_to_client(unit_1, client_b_base+i, client_a_base+i, 1);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_L1_datapath_client_to_client(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    }
    ...
    /* Remove FlexE clients */
    for (i = 0; i < nof_clients; i++) {
        rv = dnx_flexe_util_client_remove(unit_0, client_a_base+i);
        rv |= dnx_flexe_util_client_remove(unit_0, client_b_base+i);
        rv |= dnx_flexe_util_client_remove(unit_1, client_a_base+i);
        rv |= dnx_flexe_util_client_remove(unit_1, client_b_base+i);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in dnx_flexe_util_client_remove(index %d) ****\n", proc_name, rv, i);
            return rv;
        }
    ...
    /* Destroy FlexE group */
    rv = bcm_port_flexe_group_destroy(unit_0, flexe_grp);
    rv |= bcm_port_flexe_group_destroy(unit_1, flexe_grp);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in bcm_port_flexe_group_destroy() ****\n", proc_name, rv);
        return rv;
    }
    ...
    int is_sync_slots)
{
    ...
    for (unit_iter = 0; unit_iter < 2; unit_iter++) {
        ...
        /* Get slot assignment on active calendar */
        rv = bcm_port_flexe_group_cal_slots_get(unit, flexe_grp, BCM_PORT_FLEXE_TX, cal_active, 800, active_cal_slots_tx, &i);
        rv |= bcm_port_flexe_group_cal_slots_get(unit, flexe_grp, BCM_PORT_FLEXE_RX, cal_active, 800, active_cal_slots_rx, &i);
        if (rv != BCM_E_NONE) {
            printf("**** %s: Error(%d), fail in bcm_port_flexe_group_cal_slots_get(unit %d) ****\n", proc_name, rv, unit);
            return rv;
        }
    ...
    rv = dnx_flexe_util_calendar_slots_set(unit_0, flexe_grp, BCM_PORT_FLEXE_RX, !calendar_id, total_slots);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_calendar_slots_set(cen_rx, cal_backup) ****\n", proc_name, rv);
        return rv;
    }
    rv = dnx_flexe_util_calendar_slots_set(unit_1, flexe_grp, BCM_PORT_FLEXE_TX, !calendar_id, total_slots);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_calendar_slots_set(dis_tx, cal_backup) ****\n", proc_name, rv);
        return rv;
    }
    ...
    rv = dnx_flexe_util_calendar_slots_set(unit_0, flexe_grp, BCM_PORT_FLEXE_TX, !calendar_id, total_slots);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_calendar_slots_set(cen_tx, cal_backup) ****\n", proc_name, rv);
        return rv;
    }
    rv = dnx_flexe_util_calendar_slots_set(unit_1, flexe_grp, BCM_PORT_FLEXE_RX, !calendar_id, total_slots);
    if (rv != BCM_E_NONE) {
        printf("**** %s: Error(%d), fail in dnx_flexe_util_calendar_slots_set(dis_rx, cal_backup) ****\n", proc_name, rv);
        return rv;
    }
    ...
    bcm_field_context_t context_id)
{
    ...
    rv = bcm_field_group_add(unit, flags, &fg_info, &fg_id);
    if (rv != BCM_E_NONE) {
        printf("**** Error(%d): fail in bcm_field_group_add() ****\n", rv);
        return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
        printf("**** Error(%d): fail in bcm_field_group_context_attach() ****\n", rv);
        return rv;
    }
    ...
    rv = bcm_field_entry_add(unit, 0, fg_id, &ent_info, &oh_oam_filter_ent_def);
    if (rv != BCM_E_NONE) {
        printf("**** Error(%d): fail in bcm_field_entry_add() ****\n", rv);
        return rv;
    }
    ...
    uint32 priority)
{
    ...
    rv = bcm_field_entry_add(unit, 0, fg_id, &ent_info, &oh_oam_filter_ent);
    if (rv != BCM_E_NONE) {
        printf("**** Error(%d): fail in bcm_field_entry_add() ****\n", rv);
        return rv;
    }
