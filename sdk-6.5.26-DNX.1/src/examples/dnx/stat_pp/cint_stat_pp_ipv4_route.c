 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_stat_pp_ipv4_route.c Purpose: Statistics examples for SDK ingress APIs using an IPV4 application.
 */
 ...
 * cint ../../src/examples/dnx/cint_dnx_ip_route_hierarchical_fec.c
    ...
    int unit)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_stat_pp_ipv4_route_info.fec_id1);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 1, 0, &cint_stat_pp_ipv4_route_info.fec_id2);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 2, 0, &cint_stat_pp_ipv4_route_info.fec_id3);
    if (rv != BCM_E_NONE)
    {
    ...
    int out_port)
{
    ...
    uint32 fec_1_logical_pp_profile = fec_1_statistics_config.pp_profile;
    uint32 fec_2_logical_pp_profile = fec_2_statistics_config.pp_profile;
    uint32 fec_3_logical_pp_profile = fec_3_statistics_config.pp_profile;
    ...
    int reduced_fec_stat_cmd = *(dnxc_data_get(unit, "stat", "stat_pp", "reduced_fec_stat_cmd", NULL));
    ...
    if (reduced_fec_stat_cmd)
    {
        /** Valid FEC PP profiles are from 28 to 31 */
        if (fec_1_statistics_config.pp_profile != 0)
        {
            fec_1_statistics_config.pp_profile = 29;
        }
        if (fec_2_statistics_config.pp_profile != 0)
        {
            fec_2_statistics_config.pp_profile = 30;
        }
        if (fec_3_statistics_config.pp_profile != 0)
        {
            fec_3_statistics_config.pp_profile = 31;
        }
    }
    ...
    /** In case of direct stats, update the CRPS engine range according to the selected FEC ID */
    if (fec_1_statistics_config.stat_id == BCM_STAT_ID_DIRECT)
    {
        fec_1_statistics_config.crps_start = cint_stat_pp_ipv4_route_info.fec_id1 - 10;
        fec_1_statistics_config.crps_end = cint_stat_pp_ipv4_route_info.fec_id1 + 10;
    }
    if (fec_2_statistics_config.stat_id == BCM_STAT_ID_DIRECT)
    {
        fec_2_statistics_config.crps_start = cint_stat_pp_ipv4_route_info.fec_id2 - 10;
        fec_2_statistics_config.crps_end = cint_stat_pp_ipv4_route_info.fec_id2 + 10;
    }
    if (fec_3_statistics_config.stat_id == BCM_STAT_ID_DIRECT)
    {
        fec_3_statistics_config.crps_start = cint_stat_pp_ipv4_route_info.fec_id3 - 10;
        fec_3_statistics_config.crps_end = cint_stat_pp_ipv4_route_info.fec_id3 + 10;
    }
    ...
    fec_structure.flags2 = l3_flags2_fwd_only;
    fec_structure.stat_id = fec_3_statistics_config.stat_id;
    fec_structure.stat_pp_profile = fec_3_logical_pp_profile;
    ...
    fec_structure.stat_pp_profile = fec_2_logical_pp_profile;
    rv = dnx_utils_l3_fec_create(unit, &fec_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object FEC only\n");
    ...
    fec_structure.stat_pp_profile = fec_1_logical_pp_profile;
    rv = dnx_utils_l3_fec_create(unit, &fec_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object FEC only\n");
    ...
    {
        /** Add route entry - dst lookup */
        bcm_l3_route_t_init(route);
        route.l3a_subnet = cint_stat_pp_ipv4_route_info.host;
        route.l3a_ip_mask = 0xFFFFFFFF;
        route.l3a_vrf = cint_stat_pp_ipv4_route_info.vrf;
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function bcm_l3_route_add\n");
        }

        /** Add route entry - src lookup */
        bcm_l3_route_t_init(route);
        route.l3a_subnet = cint_stat_pp_ipv4_route_info.host_sip_uc;
        route.l3a_ip_mask = 0xFFFFFFFF;
        route.l3a_vrf = cint_stat_pp_ipv4_route_info.vrf;
        route.l3a_intf = ecmp.ecmp_intf;
        route.l3a_flags2 = l3_flags2_rpf_only;
