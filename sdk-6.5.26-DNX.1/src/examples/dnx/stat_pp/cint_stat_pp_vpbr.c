    ...
    int out_port)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_stat_pp_vpbr_info.fec_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = l3__egress_only_fec__create_inner(unit, cint_stat_pp_vpbr_info.fec_id, cint_stat_pp_vpbr_info.intf_out,
                                           cint_stat_pp_vpbr_info.encap_id, gport, 0, l3_flags2_fwd_only, 0, 0, &encoded_fec);
    if (rv != BCM_E_NONE)
    {
