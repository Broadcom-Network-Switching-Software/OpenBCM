    ...
    int out_sys_port)
{
     ...
     */
    rv = mpls_lsr_ilm_swap_entry_replace_with_stat_pp(ingress_unit, out_sys_port, cint_mpls_lsr_info.in_rif);
    if (rv != BCM_E_NONE)
    {
    ...
    int out_port)
{
     ...
     */
    rv = cint_stat_pp_config_main(unit, mpls_fwd_nop_statistics_config, bcmStatCounterInterfaceIngressReceivePp, gport, BCM_CORE_ALL, is_lif);
    if (rv != BCM_E_NONE)
    {
     ...
     * step 2.MPLS LSR NOP
     */
    rv = mpls_encapsulation_basic_main(unit, in_port, out_port, 0);
    if (rv != BCM_E_NONE) {
        printf("Error, in mpls_encapsulation_basic_main\n");
        return rv;
    }
