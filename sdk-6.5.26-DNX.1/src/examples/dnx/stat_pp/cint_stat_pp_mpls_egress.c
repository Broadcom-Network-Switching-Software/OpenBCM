 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_stat_pp_mpls_egress.c Purpose: Statistics examples for SDK egress APIs using an mpls encapsulation.
 */
    ...
    int port2)
{
    ...
    rv = cint_stat_pp_config_main(unit, etps_1_statistics_config, interface_type, gport, BCM_CORE_ALL, is_lif);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = cint_stat_pp_config_main(unit, etps_2_statistics_config, interface_type, gport, BCM_CORE_ALL, is_lif);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = cint_stat_pp_config_main(unit, etps_3_statistics_config, interface_type, gport, BCM_CORE_ALL, is_lif);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = cint_stat_pp_config_main(unit, etps_4_statistics_config, interface_type, gport, BCM_CORE_ALL, is_lif);
    if (rv != BCM_E_NONE)
    {
    ...
    int port2)
{
    ...
    mpls_util_entity[0].mpls_encap_tunnel[0].flags |= BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT | BCM_MPLS_EGRESS_LABEL_ACTION_VALID | BCM_MPLS_EGRESS_LABEL_STAT_ENABLE;
    ...
    BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(gport, mpls_util_entity[0].mpls_encap_tunnel[0].tunnel_id);
    rv = cint_stat_pp_config_main(unit, mpls_php_statistics_config, interface_type, gport, BCM_CORE_ALL, is_lif);
    if (rv != BCM_E_NONE)
    {
        printf("cint_stat_pp_config_main failed for etps_1_statistics_config\n");
        return rv;
