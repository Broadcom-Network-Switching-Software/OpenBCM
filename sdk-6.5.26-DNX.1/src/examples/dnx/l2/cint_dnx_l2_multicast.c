...
int l2_multicast_flood__main_config__start_run(int unit,  l2_multicast_flood_s *param, int step){
    ...
    rv = bcm_multicast_create(unit, (BCM_MULTICAST_WITH_ID | flags), &g_l2_multicast_flood.mcid);
...
int l2_multicast_flood__cleanup__start_run(int unit){
     ...
     */
    if (*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL)) {
        rv = bcm_multicast_group_destroy(unit, (BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_ELEMENT_STATIC), g_l2_multicast_flood.mcid);
    } else {
