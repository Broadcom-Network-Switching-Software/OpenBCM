 ...
 *
 * ./bcm.user
 * config add system_red_enable.0=1
 *
 * 0: tr 141 NoInit=1
 *
 ...
 * cint ../../../../src/examples/dnx/cint_dnx_system_red.c
 * cint
 * dnx_config_system_red_param_app(0, 14);
 * dnx_system_red_rsrc_thrs_example(0, 14);
 * exit;
 *
 * Test Scenario - end
 *
 * Remarks:
 *   1. config_system_red_param() thresholds  adjusted to 10Gbps port.
 *   2. Dbuff, BDB thresholds are global, there for API does not use gport or cosq (need to insert valid values).
 *   3. In bcm_cosq_gport_threshold_set(), number of values (thresholds) is 4, and number of dps id 3 (4 bit Source-Q-size)
 *   4. formula for system RED drop probability:
...
int
dnx_config_system_red_param(int unit, int out_port_voq, int tc_high, int tc_low)
{
    ...
    rv = bcm_cosq_profile_mapping_get(unit, voq_gport, cosq, 0, &profile_mapping);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_cosq_profile_mapping_get(), rv=%d.\n", rv);
        return rv;
    }
...
int
dnx_sysport_to_voq_get(int unit, int out_port_sysport, int *out_port_voq)
{
    int rv = BCM_E_NONE;
 ...
 */
int
dnx_config_system_red_param_app(int unit, int destination)
{
    ...
    rv = dnx_sysport_to_voq_get(unit, out_port_sysport, &out_port_voq);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_sysport_to_voq_get(), rv=%d.\n", rv);
        return rv;
    }

    rv = dnx_config_system_red_param(unit, out_port_voq, tc_high, tc_low);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_config_system_red_param(), rv=%d.\n", rv);
        return rv;
    }
 ...
 */
int dnx_system_red_rsrc_thrs_example(int unit, int destination)
{
    int rv = BCM_E_NONE, i, val, out_port_voq, out_port_sysport, is_dram_present, nof_calls_to_api;
    bcm_gport_t          gport;
    ...
    for (i = 0; i < nof_calls_to_api; i++)
    {
        ...
        if (i < 4)
        {
            threshold.type = bcmCosqThresholdOcbPacketDescriptorBuffers;
        } else if (i < 8)
        {
            threshold.type = bcmCosqThresholdOcbDbuffs;
        } else if (i < 12)
        {
        ...
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_cosq_gport_threshold_set(), rv=%d.\n", rv);
            return rv;
        }
