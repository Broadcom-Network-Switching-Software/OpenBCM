    ...
    int unit)
{
    ...
    {
        rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_vpls_basic_info.mpls_encap_fec_id);
    ...
    if (pwe_protection_with_one_fec)
    {
        ...
        rv = l3__egress_only_fec__create_inner(unit, cint_vpls_basic_info.mpls_encap_fec_id,
                                               out_rif_or_tunnel, 0, cint_vpls_basic_info.pwe_port, flags, 0, cint_vpls_basic_info.pwe_failover_id_fec, 0, NULL);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = l3__egress_only_fec__create_inner(unit, cint_vpls_basic_info.mpls_encap_fec_id_primary,
                                               out_rif_or_tunnel, 0, cint_vpls_basic_info.pwe_port, flags, 0, cint_vpls_basic_info.pwe_failover_id_fec, 1, NULL);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object FEC only\n");
        }
