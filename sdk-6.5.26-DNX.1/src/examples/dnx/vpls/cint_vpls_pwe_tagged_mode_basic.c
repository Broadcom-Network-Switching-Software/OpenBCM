    ...
    int port)
{
    ...
    rv = l3__egress_only_fec__create(unit, fec_id, tunnel_id, 0, port, flags, fec_flags2);
    if (rv != BCM_E_NONE)
    {
    ...
    int is_virtual)
{
    ...
    rv = vpls_pwe_tagged_mode_basic_mpls_port_add_encapsulation_with_tunnel(unit,
                mpls_port_id, pwe_label, encap_id, is_virtual, 0);
    {
        printf("Error(%d), in vpls_pwe_tagged_mode_basic_mpls_port_add_encapsulation_with_tunnel\n", rv);
        return rv;
    }
    ...
    int is_set_global_lif)
{
    ...
    *ingress_vlan_port_id = vlan_port.vlan_port_id;
     ...
     * Add VLAN port in egress
     */
    rv = vpls_pwe_tagged_mode_add_network_egress_outer_ac(unit, vsi, mac,
                                                          egress_vlan_port_id,
                                                          port,
                                                          is_cross_connect);
    if (rv != BCM_E_NONE)
    {
        printf("Error(%d), in vpls_pwe_tagged_mode_add_network_egress_outer_ac!\n", rv);
        return rv;
    }
    ...
    int unit)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_port_tpid_class_set, port=0x%08x, \n", port_tpid_class.port);
        return rv;
    }
    ...
    int core_port)
{
     ...
     * For Egress flow
     */
    rv = vpls_pwe_tagged_mode_add_core_native_ac(unit, 0 /*cint_vpls_pwe_tagged_mode_basic_info.service_vsi */,
                                                 cint_vpls_pwe_tagged_mode_basic_info.mpls_port_id_eg,
                                                 &(cint_vpls_pwe_tagged_mode_basic_info.egress_native_ac_vlan_port_id),
                                                 1, &dummy_encap_id);
    if (rv != BCM_E_NONE)
    ...
    int core_port)
{
     ...
     */
    rv = vpls_pwe_tagged_mode_add_symmetric_native_ac(unit, cint_vpls_pwe_tagged_mode_basic_info.service_vsi,
                                                      &(cint_vpls_pwe_tagged_mode_basic_info.ingress_native_ac_vlan_port_id),
                                                      cint_vpls_pwe_tagged_mode_basic_info.mpls_port_id_ing,
                                                      cint_vpls_pwe_tagged_mode_basic_info.mpls_port_id_eg,
                                                      cint_vpls_pwe_tagged_mode_basic_info.ingress_canonical_outer_vlan,
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error(%d), in vpls_pwe_tagged_mode_add_symmetric_native_ac\n", rv);
        return rv;
    }
     ...
     */
    rv = vpls_pwe_tagged_mode_add_network_egress_outer_ac(unit, cint_vpls_pwe_tagged_mode_basic_info.service_vsi,
                                                          cint_vpls_pwe_tagged_mode_basic_info.service_mac_1,
                                                          &(cint_vpls_pwe_tagged_mode_basic_info.access_egress_vlan_port_id),
                                                          cint_vpls_pwe_tagged_mode_basic_info.access_port,
                                                          0);
    ...
    int core_port)
{
    ...
    for (idx = 0; idx < cint_vpls_pwe_tagged_mode_extended_p2p_info.nof_service_tag_types - 1; idx++) {
        ...
        rv = vpls_pwe_tagged_mode_basic_mpls_port_add_termination_p2p(unit,
                  core_port,
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error(%d), idx %d, in vpls_pwe_tagged_mode_basic_mpls_port_add_termination\n", rv, idx);
            return rv;
        }

        /** Also add the outer AC that is the result of Ethernet forwarding with the exposed DA.*/
        rv = vpls_pwe_tagged_mode_add_network_egress_outer_ac(unit,
                   cint_vpls_pwe_tagged_mode_basic_info.service_vsi,
                   cint_vpls_pwe_tagged_mode_basic_info.service_mac_1,
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error(%d), idx %d, in vpls_pwe_tagged_mode_add_network_egress_outer_ac\n", rv, idx);
            return rv;
        }
        ...
        rv = vpls_pwe_tagged_mode_basic_flexible_connect_add(unit,
                  cint_vpls_pwe_tagged_mode_extended_p2p_info.mpls_port_id_ing[idx],
                  idx,
                  cint_vpls_pwe_tagged_mode_extended_p2p_info.ing_outer_stag,
                  cint_vpls_pwe_tagged_mode_extended_p2p_info.ing_inner_stag,
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error(%d), idx %d, in vpls_pwe_tagged_mode_basic_flexible_connect_add\n", rv, idx);
            return rv;
        }
        ...
        rv = vlan_translate_ive_eve_translation_set(unit,
                  ...
                  cint_vpls_pwe_tagged_mode_basic_info.egress_vlan_edit_profile,
                  cint_vpls_pwe_tagged_mode_extended_p2p_info.egr_tag_format[idx], 0);

        if (rv != BCM_E_NONE)
        {
            printf("Error, idx %d, in vlan_translate_ive_eve_translation_set, port=0x%08x, \n", idx,
                   cint_vpls_pwe_tagged_mode_extended_p2p_info.access_egress_vlan_port_id[0]);
            return rv;
        }
     ...
     */
    int gport, trap_id;
    rv = bcm_rx_trap_type_get(unit, 0, bcmRxTrapDfltDroppedPacket, &trap_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_rx_trap_type_get\n");
        return rv;
    }

    BCM_GPORT_TRAP_SET(gport, 0, 0, trap_id);
    rv = bcm_vlan_control_set (unit, bcmVlanNativeVplsServiceTaggedMiss, gport);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_vlan_control_set bcmVlanNativeVplsServiceTaggedMiss\n");
        return rv;
    }
    ...
    int unit)
{
    ...
    for (idx = 0; idx < 3; idx ++ )
    {
        ...
        egr_ac_encap_id = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(cint_vpls_pwe_tagged_mode_extended_p2p_info.access_egress_vlan_port_id[idx]);
        rv = vpls_pwe_tagged_mode_basic_flexible_connect_delete(unit,
                      cint_vpls_pwe_tagged_mode_extended_p2p_info.mpls_port_id_ing[idx],
                      idx,
                      cint_vpls_pwe_tagged_mode_extended_p2p_info.ing_outer_stag,
                      cint_vpls_pwe_tagged_mode_extended_p2p_info.ing_inner_stag);
        if (rv != BCM_E_NONE)
        {
            printf("Error, vpls_pwe_tagged_mode_basic_flexible_connect_delete for 2_tag\n");
            return rv;
        }
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, vpls_pwe_tagged_mode_basic_flexible_connect_delete for x_tag\n");
        return rv;
    }
    ...
    int unit)
{
    ...
    for (idx = 1; idx < 3; idx ++ )
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error, vpls_pwe_tagged_mode_basic_flexible_connect_delete for 2/1_stag\n");
            return rv;
        }
    }
