...
vpls_hvpls_mpls_tunnels_create(int unit)
{
    ...
    mpls_encap_tunnel_2[0].flags |= BCM_MPLS_EGRESS_LABEL_WITH_ID | BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
    ...
    {
        mpls_encap_tunnel_2[0].flags |= BCM_MPLS_EGRESS_LABEL_PROTECTION;
    ...
    if (!*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL))
    {
        mpls_encap_tunnel_1[0].flags |= BCM_MPLS_EGRESS_LABEL_PROTECTION;
...
vpls_hvpls_mpls_fec_create(int unit)
{
    ...
    rv = l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.mpls_fec, vpls_hvpls_info.mpls_tunnel_id_protected,
                                           0, gport, BCM_L3_2ND_HIERARCHY, fec_flags2, vpls_hvpls_info.mpls_failover_id_fec, 0, &encoded_fec);
    ...
    rv = l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.mpls_fec, vpls_hvpls_info.mpls_tunnel_id_primary,
                                           0, gport, BCM_L3_2ND_HIERARCHY, fec_flags2, vpls_hvpls_info.mpls_failover_id_fec, 1, &encoded_fec);
...
vpls_hvpls_mpls_port_add_encapsulation(int unit)
{
    ...
    if (!*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL)) 
    {
        mpls_port_egress.flags2 |= BCM_MPLS_PORT2_EGRESS_PROTECTION;
...
vpls_hvpls_pwe_fec_create(int unit)
{
    ...
    if (mc_in_fec) {
        ...
        rv = l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.pwe_fec, 0,
                                               0, gport, 0, fec_flags2, vpls_hvpls_info.pwe_failover_id_fec, 0, &encoded_fec);
    } else {
    ...
    if (mc_in_fec) {
        rv = l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.pwe_fec, 0,
                                               0, gport, 0, fec_flags2, vpls_hvpls_info.pwe_failover_id_fec, 1, &encoded_fec);
    } else {
...
int
vpls_hvpls_ingress_prune_mc_set(
    int unit)
{
    int rv = BCM_E_NONE;
    char *proc_name;
    proc_name = "vpls_hvpls_ingress_prune_mc_set";
    bcm_multicast_replication_t reps[2] = {{0}};
    int mc_id;
    int flags;
    int gport;
    int pri_pwe_prune_group_id = 0x30001, backup_pwe_prune_group_id = 0x30002;

    rv = bcm_failover_create(unit, BCM_FAILOVER_INGRESS_MC, &vpls_hvpls_info.mpls_failover_id_outlif);
    if (rv != BCM_E_NONE)
    {
        printf("%sERROR: in bcm_failover_create of LSP egress\n",proc_name);
        return rv;
    }

    /* create egress failover id for PWE */
    rv = bcm_failover_create(unit, BCM_FAILOVER_INGRESS_MC, &vpls_hvpls_info.pwe_failover_id_outlif);
    if (rv != BCM_E_NONE) {
        printf("%sError, bcm_failover_create for PWE Egress\n",proc_name);
        return rv;
    } 
    ...
    bcm_multicast_group_destroy(unit, flags, pri_pwe_prune_group_id);
    bcm_multicast_group_destroy(unit, flags, backup_pwe_prune_group_id);

    flags = BCM_MULTICAST_WITH_ID | BCM_MULTICAST_INGRESS_GROUP
            | BCM_MULTICAST_ELEMENT_STATIC;
    rv = bcm_multicast_create(unit, flags, pri_pwe_prune_group_id);
    if (rv != BCM_E_NONE) {
        printf("%sError, bcm_multicast_create, \n",proc_name);
        return rv;
    }

    rv = bcm_multicast_create(unit, flags, backup_pwe_prune_group_id);
    if (rv != BCM_E_NONE) {
        printf("%sError, bcm_multicast_create, \n",proc_name);
        return rv;
    }

    mc_id = pri_pwe_prune_group_id;
    ...
    if (rv != BCM_E_NONE) {
        printf("Error , bcm_multicast_encap_extension_create. cud0 = 0x%08X, cud1 = 0x%08X\n", cuds[0],cuds[1]);
        return rv;
    }
    reps[0].encap1 = rep_index;
    BCM_GPORT_LOCAL_SET(reps[0].port, pwe_port.port);
    ...
    reps[0].encap2 = rep_index;
    BCM_GPORT_LOCAL_SET(reps[0].port2, pwe_port.port);

    flags = BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_ELEMENT_STATIC;
    reps[0].failover_id = vpls_hvpls_info.mpls_failover_id_outlif;
    reps[0].flags = BCM_MULTICAST_REP_EITHER;
    rv = bcm_multicast_add(unit, mc_id, flags, 1, reps);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error in bcm_multicast_add!\n",proc_name);
        return rv;
    }

    mc_id = backup_pwe_prune_group_id;
    ...
    BCM_GPORT_MCAST_SET(reps[1].port, pri_pwe_prune_group_id);
    BCM_GPORT_MCAST_SET(reps[1].port2, backup_pwe_prune_group_id);
    reps[1].flags = BCM_MULTICAST_REP_EITHER;
    reps[1].failover_id = vpls_hvpls_info.pwe_failover_id_outlif;

    rv = bcm_multicast_add(unit, mc_id, flags, 2, reps);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error in bcm_multicast_add!\n",proc_name);
        return rv;
...
int
vpls_hvpls_legacy_mc_set(
    int unit)
{
    int rv = BCM_E_NONE;
    char *proc_name;
    proc_name = "vpls_hvpls_legacy_mc_set";
    ...
    int no_learning)
{
    ...
    if (no_learning)
    {
        ...
        if (*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL))
        {
            rv = vpls_hvpls_ingress_prune_mc_set(unit);
            if (rv != BCM_E_NONE)
            {
                printf("%s(): Error, in vpls_hvpls_ingress_prune_mc_set\n", proc_name);
                return rv;
            }
        ...
        {
            rv = vpls_hvpls_legacy_mc_set(unit);
            if (rv != BCM_E_NONE)
            {   
                printf("%s(): Error,vpls_hvpls_legacy_mc_set\n", proc_name);
                return rv;
            }
        }
   }
