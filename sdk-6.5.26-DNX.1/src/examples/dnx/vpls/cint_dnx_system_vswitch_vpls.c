    ...
    int port3)
{
    ...
    rv = l3__egress_only_fec__create_inner(unit1, pwe__port_2.encap_fec_id, gport, 0, port3, 0, flags2, failover_id, 0, NULL);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = l3__egress_only_fec__create_inner(unit1, pwe__port_1.encap_fec_id, gport, 0, port2, 0, flags2, failover_id, 1, NULL);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC only\n", proc_name);
        return rv;
