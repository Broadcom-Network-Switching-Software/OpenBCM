...
int cint_sflow_field_extended_gateway_first_pass_group_create_and_attach(int unit, int nof_cores, bcm_gport_t gport_stat_samp_core_0, bcm_gport_t gport_stat_samp_core_1, bcm_gport_t gport_stat_samp_core_2, bcm_gport_t gport_stat_samp_core_3, bcm_field_context_t context_id)
{
   ...
   for (core_id = 0; core_id < nof_cores; core_id ++) {
       ...
       if (gport_stat_samp[core_id] != -1)
       {
          ...
          rv = bcm_field_entry_add(unit, 0, cint_sflow_snoop_field_fg_id, &ent_info, &(cint_sflow_core_id_entry_id[core_id]));
          if (rv != BCM_E_NONE)
          {
...
int sflow_field_extended_gateway_second_pass_next_hop_const_create_and_attach(int unit, bcm_field_context_t context_id)
{
   ...
   {
       rv = BCM_E_UNAVAIL;
       printf("%s: Error (%d), NEXT-HOP lookup is only supported in sFlow hard-logic platform! \n", proc_name, rv);
       return rv;
   }
   ...
   rv = bcm_field_action_create(unit, 0, &action_info, &cint_sflow_trap_next_hop_action_id);
   if (rv != BCM_E_NONE)
   {
      printf("%s: Error (%d), in bcm_field_action_create\n", proc_name, rv);
      return rv;
   ...
   rv = bcm_field_group_add(unit, 0, &field_group_info, &cint_sflow_trap_next_hop_field_fg_id);
   if (rv != BCM_E_NONE)
   {
      printf("%s: Error (%d), in bcm_field_group_add\n", proc_name, rv);
      return rv;
   ...
   rv = bcm_field_group_context_attach(unit, 0, cint_sflow_trap_next_hop_field_fg_id, context_id, &attach_info);
   if (rv != BCM_E_NONE)
   {
      printf("%s: Error (%d), in bcm_field_group_context_attach\n", proc_name, rv);
      return rv;
...
int sflow_field_extended_gateway_second_pass_tcam_next_hop_create_and_attach(int unit, bcm_field_context_t context_id)
{
   ...
   rv = bcm_field_group_add(unit, 0, &field_group_info, &cint_sflow_extended_next_hop_field_fg_id);
   if (rv != BCM_E_NONE)
   {
      printf("%s: Error (%d), in bcm_field_group_add\n", proc_name, rv);
      return rv;
   ...
   rv = bcm_field_group_context_attach(unit, 0, cint_sflow_extended_next_hop_field_fg_id, context_id, &attach_info);
   if (rv != BCM_E_NONE)
   {
      printf("%s: Error (%d), in bcm_field_group_context_attach\n", proc_name, rv);
      return rv;
...
int cint_sflow_field_extended_gateway_tcam_next_hop_entry_add(int unit, int dst_port, int sflow_encap_extended_next_hop_id)
{
   ...
   rv = bcm_field_entry_add(unit, 0, cint_sflow_extended_next_hop_field_fg_id, &ent_info, &cint_sflow_next_hop_entry_id);
   if (rv != BCM_E_NONE)
   {
      printf("%s: Error (%d), in bcm_field_entry_add\n", proc_name, rv);
      return rv;
...
int cint_sflow_field_extended_gateway_destroy(int unit,bcm_field_context_t context_id, int nof_cores)
{
   ...
   for (core_id = 0; core_id < nof_cores; core_id ++) {
      ...
      rv = bcm_field_entry_delete(unit, cint_sflow_snoop_field_fg_id, NULL, cint_sflow_core_id_entry_id[core_id]);
      if (rv != BCM_E_NONE)
      {
   ...
   if (cint_sflow_eventor_port_entry_id != BCM_FIELD_INVALID) {
      rv = bcm_field_entry_delete(unit, cint_sflow_snoop_field_fg_id, NULL, cint_sflow_eventor_port_entry_id);
      if (rv != BCM_E_NONE)
      {
         printf("%s: Error (%d), in bcm_field_entry_delete cint_sflow_eventor_port_entry_id = %d \n", proc_name, rv, cint_sflow_eventor_port_entry_id);
         return rv;
      }
