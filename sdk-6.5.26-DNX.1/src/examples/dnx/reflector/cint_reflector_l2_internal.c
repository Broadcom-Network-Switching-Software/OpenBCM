...
int cint_reflector_l2_internal_clear_mir_snp_code(int unit, int *trap_gport)
{
    ...
    rv = bcm_rx_trap_type_create(unit, 0, bcmRxTrapEgTxUserDefine, &trap_id);
    if (rv != BCM_E_NONE)
    {
    ...
    if (rv != BCM_E_NONE)
    {
        printf("bcm_rx_trap_set to clear mir/snp code failed\n");
        return rv;
    }
    ...
    bcm_field_context_t epmf_context_id)
{
    ...
    rv = cint_reflector_l2_internal_clear_mir_snp_code(unit, &trap_gport);
    if (rv != BCM_E_NONE)
    {
        printf("cint_reflector_l2_internal_clear_mir_snp_code failed\n");
        return rv;
    }
    ...
    else
    {
        ...
        if (!BCM_GPORT_IS_TRUNK (out_port)) {
            ...
            rv = bcm_trunk_get(unit, out_port, &trunk_info, 4, trunk_member_array, &trunk_member_count);
            if (rv != BCM_E_NONE)
            {
            ...
            for (member_index = 0; member_index < trunk_member_count; member_index++) {
                /** assume that mirror recycle port for destination already created by reference application */
                /** just enable high priority */
                rv = bcm_mirror_port_to_rcy_port_map_get(unit, 0, trunk_member_array[member_index].gport, &rcy_map_info);
                if (rv != BCM_E_NONE)
                {
                    printf("bcm_mirror_port_to_rcy_port_map_get failed\n");
                    return rv;
                ...
                sal_memset(&rcy_map_info_clear, 0, sizeof(rcy_map_info_clear));
                rcy_map_info_clear.rcy_mirror_port = BCM_PORT_INVALID;
                rv = bcm_mirror_port_to_rcy_port_map_set(unit, 0, trunk_member_array[member_index].gport, &rcy_map_info_clear);
                if (rv != BCM_E_NONE)
                {
                    printf("bcm_mirror_port_to_rcy_port_map_set failed\n");
                    return rv;
                ...
                rv = bcm_mirror_port_to_rcy_port_map_set(unit, 0, trunk_member_array[member_index].gport, &rcy_map_info);
                if (rv != BCM_E_NONE)
                {
                    printf("bcm_mirror_port_to_rcy_port_map_set failed\n");
                    return rv;
    ...
    if (!BCM_GPORT_IS_TRUNK (out_port)) {
        rv = cint_reflector_l2_internal_fp_entry_add(unit, fg_id, &fg_info, in_port, out_port, rcy_port, ace_entry_handle, encap_id, trap_gport);
        if (rv != BCM_E_NONE)
        {
        ...
        for (member_index = 0; member_index < trunk_member_count; member_index++) {
            rv = cint_reflector_l2_internal_fp_entry_add(unit, fg_id, &fg_info, in_port, trunk_member_array[member_index].gport, rcy_port, ace_entry_handle, encap_id, trap_gport);
            if (rv != BCM_E_NONE)
            {
                printf("cint_reflector_l2_internal_fp_entry_add failed\n");
                return rv;
    ...
    bcm_field_context_t epmf_context_id)
{
    ...
    /** add entry to field processor program */
    rv = cint_reflector_l2_internal_fp_entry_add(unit, fg_id, &fg_info, in_port, out_port, 0, ace_entry_handle, encap_id, 0);
    if (rv != BCM_E_NONE)
    {
        printf("cint_reflector_l2_internal_fp_entry_add failed\n");
        return rv;
        ...
        bcm_field_context_t epmf_context_id)
{
    ...
    {
        rv = bcm_trunk_get(unit, egress_port, &trunk_info, 4, trunk_member_array, &trunk_member_count);
        if (rv != BCM_E_NONE)
        {
            printf("bcm_trunk_get failed\n");
            return rv;
    ...
    if (!BCM_GPORT_IS_TRUNK (egress_port))
    {
        rv = cint_reflector_l2_internal_uc_fp_entry_add(unit, fg_id, &fg_info, in_port, egress_port, rcy_port, ace_entry_handle, is_ip_swap, is_udp_swap, reflector_encap_id, trap_gport);
        if (rv != BCM_E_NONE)
        {
            printf("cint_reflector_l2_internal_fp_entry_add failed\n");
            return rv;
    ...
    else
    {
        for (member_index = 0; member_index < trunk_member_count; member_index++)
        {
            rv = cint_reflector_l2_internal_uc_fp_entry_add(unit, fg_id, &fg_info, in_port, trunk_member_array[member_index].gport, rcy_port, ace_entry_handle, is_ip_swap, is_udp_swap, reflector_encap_id, trap_gport);
            if (rv != BCM_E_NONE)
            {
                printf("cint_reflector_l2_internal_fp_entry_add failed\n");
                return rv;
        ...
        bcm_field_context_t epmf_context_id)
{
     ...
     * add entry to field processor program 
     */  
    rv = cint_reflector_l2_internal_uc_fp_entry_add(unit, fg_id, &fg_info, in_port, egress_port, 0, ace_entry_handle, is_ip_swap, is_udp_swap, reflector_encap_id, 0);
    if (rv != BCM_E_NONE)
    {
        printf("cint_reflector_l2_internal_fp_entry_add failed\n");
        return rv;
