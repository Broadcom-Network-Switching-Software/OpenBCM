    ...
    int port4)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 1, 0, &cint_mpls_encapsulation_basic_info.fec_id_enc);
    if (rv != BCM_E_NONE)
    {
    ...
    for (i = 0; i < MEMBERS_IN_ECMP_GROUP; i++)
    {
        ...
        rv = l3__egress_only_fec__create(unit, cint_mpls_ecmp_basic_info.fec_ids[i], cint_mpls_encapsulation_basic_info.push_tunnel_id, 0, dest_port, BCM_L3_2ND_HIERARCHY, fec_flags2);
        if (rv != BCM_E_NONE)
        {
