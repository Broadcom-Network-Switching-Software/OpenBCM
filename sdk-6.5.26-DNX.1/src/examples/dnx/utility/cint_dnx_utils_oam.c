 ...
 * @return int
 */
int oam__vswitch_gport_add(int unit, bcm_port_t port, int vlan)
{
    bcm_error_t rv;
    gport4 = 0;
    bcm_vlan_port_t vp;
    ...
    rv = bcm_vswitch_port_add(unit, dnx_oam_endpoint_info_glb.vsi, gport4);
    if (rv != BCM_E_NONE)
    {
    ...
    if(verbose)
    {
        printf("Added port 0x%08x to vswitch 0x%08x\n", gport4, dnx_oam_endpoint_info_glb.vsi);
    }

    /* Add vlan-port to multicast group */
    rv = multicast__gport_encap_add(unit, dnx_oam_endpoint_info_glb.vsi, port, gport4, 0);
    if (rv != BCM_E_NONE)
    {
