 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_dnx_utils_vpls.c Purpose: utility for VPLS. 
 */
...
void
vpls_mpls_tunnel_initiator_create_s_init(
...
void
vpls_mpls_tunnel_switch_create_s_init(
    ...
    int unit)
{
    ...
    estimate_encap_size_required = *dnxc_data_get(unit, "dev_init", "general", "egress_estimated_bta_btr_hw_config", NULL);
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &ac_port.encap_fec_id);
    if (rv != BCM_E_NONE)
    {
    ...
    if (vpls_util_cw_present) {
        pwe_encap.label_flags |= BCM_MPLS_PORT_CONTROL_WORD;
    }
    ...
    if (vpls_util_cw_present) {
        pwe_term.label_flags |= BCM_MPLS_PORT_CONTROL_WORD;
    }
    ...
    uint8 create_rif)
{
    ...
    /* port must be added to the vlan for membership check */
    ...
    int flag)
{
     ...
     * 4) Give the out port. 
     */
    rv = l3__egress_only_fec__create(unit, fec_id, mpls_tunnel_id, 0, port, flags, flags2);
    if (rv != BCM_E_NONE)
    {
    ...
    mpls_tunnel_initiator_create_s * mpls_tunnel)
{
    ...
    estimate_encap_size_required = estimate_encap_size_required = *dnxc_data_get(unit, "dev_init", "general", "egress_estimated_bta_btr_hw_config", NULL);
    ...
    for (i = MAX_NOF_TUNNELS - 1; i >= 0; i--)
    {
        ...
        if ((mpls_tunnel[i].label[0] >= MIN_LABEL) && (mpls_tunnel[i].label[0] <= MAX_LABEL))
        {
            ...
            if ((i == 0) && (estimate_encap_size_required))
            {
                uint32 mpls_encap_size = *dnxc_data_get(unit, "lif", "out_lif", "global_mpls_outlif_bta_sop", NULL);
                label_array[i].estimated_encap_size  =
                        nof_actual_tunnels * mpls_tunnel[i].num_labels * mpls_encap_size;
            }
    ...
    int port2)
{
    ...
    if (*dnxc_data_get(unit, "mpls", "general", "mpls_control_word_supported", NULL))
    {
...
vpls_mpls_next_layer_network_domain_value(int unit, int next_layer_network_domain)
    ...
    int arr_prefix_value = *(dnxc_data_get(unit, "dnx2_arr", "prefix", "IRPP_IN_LIF_FORMATS___prefix_NEXT_LAYER_NETWORK_DOMAIN_PWE_WO_SD_TAGS", NULL));
    ...
    int port2)
{
    ...
    mpls_encap_tunnel[1].flags  |= BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
