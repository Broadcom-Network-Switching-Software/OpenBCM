...
int lawful_interception_mirror_dest_create(int unit)
{
    ...
    if (rv = bcm_mirror_header_info_set(unit, flags, dest.mirror_dest_id, &mirror_header_info)) {
...
int lawful_interception_filter_config(int unit, int in_port,bcm_field_context_t context_id, int is_remove_ll)
{
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &li_mirror_fg_id);
    if (rv != BCM_E_NONE) {
        printf("Error (%d), in bcm_field_group_add for %s sys_header mode\n", rv, is_jr1_system_header_mode?"JR1":"JR2");
        return rv;
    }
...
int lawful_interception_main_config(int unit, int in_port, int mir_port, int is_remove_ll, int is_ipv4_encap)
{
     ...
     * filter config
     */
    rv = lawful_interception_filter_config(unit, in_port, BCM_FIELD_CONTEXT_ID_DEFAULT, is_remove_ll);
    if (rv != BCM_E_NONE) {
