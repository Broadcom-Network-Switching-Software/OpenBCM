...
int ac_1to1_coupled_protection_init(int unit, ac_1to1_coupled_protection_s *params) {
    ...
    rv = bcm_failover_create(unit, (BCM_FAILOVER_WITH_ID | failover_create_flags), &g_ac_1to1_coupled_protection.eg_in_failover_id);
    if (rv != BCM_E_NONE) {
    ...
    if (failover_create_flags == BCM_FAILOVER_INGRESS_MC)
    {
        ...
        rv = bcm_multicast_create(unit, mc_create_flags, mc_group);
        if (rv != BCM_E_NONE)
        {
...
int ac_1to1_coupled_protection__set_protection_acs(int unit, ac_1to1_coupled_protection_s *ac_1to1_coupled_protection_info) {
    ...
    bcm_if_t encap_id;
    int rv, mc_group, is_mc_pruning = FALSE;
    int check_pass = 1;
    bcm_l3_egress_t fec;
    ...
    if (is_mc_pruning)
    {
        ...
        rv = bcm_multicast_set(unit, mc_group, set_flags, 1, replications);
        if (rv != BCM_E_NONE)
        {
