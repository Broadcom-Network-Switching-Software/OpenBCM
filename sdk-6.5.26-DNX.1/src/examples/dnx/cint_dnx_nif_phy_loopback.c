    ...
    int *port_list)
{
     ...
     */
    rv = dnx_port_phy_loopback_port_lane_boundary_get(unit, port, &port_first_lane, &port_last_lane);
    if (BCM_FAILURE(rv))
    {
        printf("dnx_port_phy_loopback_port_lane_boundary_get failed for port %d - unit %d.\n", port, unit);
        return rv;
    }
    ...
    *nof_ports = 0;
    ...
    {
        rv = dnx_port_phy_loopback_port_lane_boundary_get(unit, nif_port, &port_first_lane, &port_last_lane);
        if (BCM_FAILURE(rv))
        {
            printf("dnx_port_phy_loopback_port_lane_boundary_get failed for port %d - unit %d.\n", nif_port, unit);
            return rv;
    ...
    bcm_port_t port)
{
    ...
    rv = dnx_port_phy_loopback_medium_type_and_dfe_mode_get(unit, port_info_db[port].resource.phy_lane_config,
                                                             &port_info_db[port].medium_type, &port_info_db[port].dfe_mode);
    if (BCM_FAILURE(rv))
    {
        printf("dnx_port_phy_loopback_medium_type_and_dfe_mode_get failed for port %d - unit %d.\n", port, unit);
        return rv;
    }
...
)
{
   ...
   /*
...

    {
        ...
        tmp_port = port_list[port_index];
        rv = dnx_port_phy_loopback_port_info_get(unit, tmp_port);
        if (BCM_FAILURE(rv))
        {
            printf("dnx_port_phy_loopback_port_info_get failed for port %d - unit %d.\n", tmp_port, unit);
            return rv;
        }
    ...
    rv = dnx_dyn_port_properties_construct(unit, nof_ports, port_list, mapping_info, interface_info, resource,
                                            dfe_mode, medium_type, header_type_in, header_type_out, flags);
    if (BCM_FAILURE(rv))
    {
        printf("dnx_dyn_port_properties_construct failed - unit %d.\n", unit);
        return rv;
    }
    /*
     * Add the ports back 
    ...
    for (port_index = 0; port_index < nof_ports; ++port_index)
    {
         ...
         */
        tmp_port = port_list[port_index];
        rv = dnx_port_phy_loopback_port_info_set(unit, tmp_port);
        if (BCM_FAILURE(rv))
        {
            printf("dnx_port_phy_loopback_port_info_set failed for port %d - unit %d.\n", tmp_port, unit);
            return rv;
        }
        /*
    ...
    bcm_port_t port)
{
     ...
     */
    rv = dnx_port_phy_loopback_pm_ports_get(unit, port, &pm_first_lane, &pm_last_lane, &nof_ports, port_list);
    if (BCM_FAILURE(rv))
    {
        printf("dnx_port_phy_loopback_pm_ports_get failed for port %d - unit %d.\n", port, unit);
        return rv;
    }
    ...
    rv = dnx_port_pm_lane_swap_adjust(unit, nof_ports, port_list, *(dnxc_data_get(unit, "nif", "phys", "nof_phys", NULL)), serdes_map, 1);
    if (BCM_FAILURE(rv))
    {
        printf("dnx_port_pm_lane_swap_adjust failed for port %d - unit %d.\n", port, unit);
        return rv;
    }
    ...
    int port)
{
    ...
    rv = dnx_port_pm_lane_swap_adjust(unit, nof_ports, port_list, *(dnxc_data_get(unit, "nif", "phys", "nof_phys", NULL)), serdes_map, 0);
    if (BCM_FAILURE(rv))
    {
        printf("dnx_port_pm_lane_swap_adjust failed for port %d - unit %d.\n", port, unit);
        return rv;
    ...
    bcm_port_t port)
{
    ...
    if (is_dnx)
    {
        need_to_align_swap = dnx_port_phy_has_lane_swap(unit, port);
        if (need_to_align_swap)
        {
            ...
            rv = dnx_port_phy_loopback_lane_swap_align(unit, port);
            if (BCM_FAILURE(rv))
            {
                printf("dnx_port_phy_loopback_lane_swap_align failed for port %d - unit %d.\n", port, unit);
                return rv;
            }
        }
    }
    ...
    bcm_port_t port)
{
     ...
     */
    rv = dnx_port_phy_loopback_lane_swap_recover(unit, port);
    if (BCM_FAILURE(rv))
    {
        printf("dnx_port_phy_loopback_lane_swap_recover failed for port %d - unit %d.\n", port, unit);
        return rv;
    }
    return rv;
}
    ...
    int en)
{
    ...
    if (en)
    {
        ...
        if (BCM_FAILURE(rv))
        {
            printf("dnx_port_phy_loopback_set failed for port %d - unit %d.\n", port, unit);
            return rv;
        }
    }
    else
    {
        rv = dnx_port_phy_loopback_clear(unit, port);
        if (BCM_FAILURE(rv))
        {
            printf("dnx_port_phy_loopback_clear failed for port %d - unit %d.\n", port, unit);
            return rv;
        }
    }
