 ...
 * In different chip, its value is different, see regression case .
 */
    ...
    int out_port)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error in mpls_deep_stack_alternate_marking_snoop\n");
        return rv;
    }
    ...
    int nof_tunnel)
{
    ...
    rv = mpls_deep_stack_alternate_marking_enacp_configuration(unit, in_port, out_port);
    if (rv) {
        printf("Error, mpls_deep_stack_alternate_marking_enacp_configuration\n");
        return rv;
    }
    return rv;
}
