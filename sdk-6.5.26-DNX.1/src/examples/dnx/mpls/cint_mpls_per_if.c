 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_mpls_per_if.c Purpose: MPLS lookup per-interface example
 */
    ...
    int out_port)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_mpls_per_if_basic_info.ipv4_info.fec_id);
    if (rv != BCM_E_NONE)
    {
    ...
    int unit)
{
    ...
    rv = l3__egress_only_fec__create(unit, cint_mpls_per_if_basic_info.ipv4_info.fec_id,
                                     ...
                                     cint_mpls_per_if_basic_info.out_port, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    int unit)
{
    ...
    rv = l3__egress_only_fec__create(unit, cint_mpls_per_if_basic_info.mpls_swap_info.fec_id,
                                     eg_label.tunnel_id, 0, cint_mpls_per_if_basic_info.out_port, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), l3__egress_only_fec__create\n", rv);
        return rv;
