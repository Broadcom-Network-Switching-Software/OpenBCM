   ...
   bcm_if_t fec_id) 
{
    ...
    rc = l3__egress_only_fec__create(unit, fec_id, tunnel_id, 0, cint_mpls_ecmp_basic_info.out_port, 0,flags2);
    if (rc != BCM_E_NONE)
    {
   ...
   int intf)
{
    ...
    {
        rc = add_route_ipv4(unit, cint_mpls_ecmp_basic_info.dip, 0xFFFFFFFF, cint_mpls_ecmp_basic_info.vrf,
                intf);
        if (rc != BCM_E_NONE)
        {
   ...
   int out_port) 
{
    ...
    rc = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, fec_direction, &cint_mpls_ecmp_basic_info.fec_ids[0]);
    if (rc != BCM_E_NONE)
    {
