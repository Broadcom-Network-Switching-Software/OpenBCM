...
int cint_field_inlif_profile_main(int unit, bcm_field_context_t context_id, uint32 inlif, int inlif_profile)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_class_set(unit,inlif,bcmPortClassFieldIngressVport,inlif_profile));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &cint_inlif_profile_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, cint_inlif_profile_fg_id, context_id, &attach_info));
    ...
    printf("Adding Entry\n");
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, cint_inlif_profile_fg_id, &ent_info, &cint_inlif_profile_ent_id));
...
{
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, cint_inlif_profile_fg_id, NULL, cint_inlif_profile_ent_id));

    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, cint_inlif_profile_fg_id, context_id));

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, cint_inlif_profile_fg_id));
