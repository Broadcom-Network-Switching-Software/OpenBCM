    ...
    bcm_field_context_t * context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_context_param_set(unit, 0, stage, *context_id, &param_info));
    ...
    cint_field_sys_headers_qual_info_t * sys_hdr_qual_info)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &(sys_hdr_qual_info->qual_id)));
    ...
    cint_field_sys_headers_fg_info_t * sys_hdr_fg_info)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_sys_headers_context(unit, sys_hdr_fg_info->stage, &(sys_hdr_fg_info->context_id)));
    ...
    {
        BCM_IF_ERROR_RETURN(cint_field_sys_headers_user_qual_config(unit, fg_info.stage, &(sys_hdr_fg_info->sys_hdr_qual_info[0])));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &(sys_hdr_fg_info->fg_id)));


    printf("Field Group ID %d was created. \n", sys_hdr_fg_info->fg_id);

    
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, sys_hdr_fg_info->fg_id, sys_hdr_fg_info->context_id, &attach_info));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, sys_hdr_fg_info->fg_id, &entry_info, &entry_handle));
    ...
    int fg_info_array_index)
{
     ...
     */
    BCM_IF_ERROR_RETURN(cint_field_sys_headers_group_config(unit, sys_headers_fg_info_array[fg_info_array_index]));
    ...
    int fg_info_array_index)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, sys_headers_fg_info_array[fg_info_array_index].fg_id, NULL, entry_handle));

    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, sys_headers_fg_info_array[fg_info_array_index].fg_id, sys_headers_fg_info_array[fg_info_array_index].context_id));

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, sys_headers_fg_info_array[fg_info_array_index].fg_id));

    if (sys_headers_fg_info_array[fg_info_array_index].stage == bcmFieldStageIngressPMF2)
    {
        BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit, bcmFieldStageIngressPMF1, sys_headers_fg_info_array[fg_info_array_index].context_id));
    }
    else
    {
        BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit, sys_headers_fg_info_array[fg_info_array_index].stage, sys_headers_fg_info_array[fg_info_array_index].context_id));
    }

    if (sys_headers_fg_info_array[fg_info_array_index].sys_hdr_qual_info[0].name != NULL)
    {
        BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, sys_headers_fg_info_array[fg_info_array_index].sys_hdr_qual_info[0].qual_id));
    }
