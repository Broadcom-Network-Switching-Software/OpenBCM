                    ...
                    int is_ss,    bcm_gport_t ss_cmd, bcm_field_entry_t *cint_field_sniff_ent_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, fg_id, &ent_info, cint_field_sniff_ent_id));
...
{
    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, cint_field_sniff_ingress_fg_id, ipmf1_context_id));

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, cint_field_sniff_ingress_fg_id));

    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, cint_field_sniff_egress_fg_id, epmf_context_id));

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, cint_field_sniff_egress_fg_id));
...
int cint_field_sniff_ingress_main(int unit, bcm_field_context_t ipmf1_context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &cinf_field_sniff_ingress_fg_info, &cint_field_sniff_ingress_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, cint_field_sniff_ingress_fg_id, ipmf1_context_id, &attach_info));
...
int cint_field_sniff_egress_main(int unit, bcm_field_context_t epmf_context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &cinf_field_sniff_egress_fg_info, &cint_field_sniff_egress_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, cint_field_sniff_egress_fg_id, epmf_context_id, &attach_info));
...
int cint_field_sniff_ingress_config(int unit, int destroy_config, bcm_field_context_t ipmf1_context_id)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_sniff_ingress_main(unit, ipmf1_context_id));


    int trap_id;
    BCM_IF_ERROR_RETURN(bcm_rx_trap_type_create(unit,0,bcmRxTrapUserDefine, &trap_id));

    bcm_rx_trap_config_t trap_config;
    bcm_rx_trap_config_t_init(&trap_config);
    trap_config.snoop_cmnd = BCM_GPORT_MIRROR_GET(snoop_cmd);
    BCM_IF_ERROR_RETURN(bcm_rx_trap_set(unit, trap_id, &trap_config));

    
    BCM_GPORT_TRAP_SET (snoop_gport, trap_id, 0, 7);

    /**Add 1 entry for Mirror,snoop,ss in ingress PMF*/
    BCM_IF_ERROR_RETURN(cint_field_sniff_entry_add(unit, FALSE, condition_port, TRUE, mirror_cmd, TRUE, snoop_gport, TRUE, ss_cmd, &ipmf1_ent_id));


    if(destroy_config)
    {
        BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit,cint_field_sniff_ingress_fg_id,NULL,ipmf1_ent_id));
...
int cint_field_sniff_egress_config(int unit, int destroy_config, bcm_field_context_t epmf_context_id)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_sniff_egress_main(unit, epmf_context_id));
    ...
    BCM_IF_ERROR_RETURN(cint_field_sniff_entry_add(unit, TRUE, condition_port, TRUE, mirror_cmd, TRUE, snoop_gport, FALSE, ss_cmd, &epmf_ent_id));


    if(destroy_config)
    {
        BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit,cint_field_sniff_egress_fg_id,NULL,epmf_ent_id));
...
int cint_field_sniff_example(int unit, int destroy_config, bcm_field_context_t ipmf1_context_id, bcm_field_context_t epmf_context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_mirror_destination_create(unit,&snoop_dest));
    ...
    BCM_IF_ERROR_RETURN(bcm_mirror_destination_create(unit,&ss_dest));

    ss_cmd = ss_dest.mirror_dest_id;
    printf("Created ss Profile 0x%x \n",ss_cmd);

    /** Configure Ingress PMF */
    BCM_IF_ERROR_RETURN(cint_field_sniff_ingress_config(unit, destroy_config, ipmf1_context_id));


    /** Configure Egress PMF */
    BCM_IF_ERROR_RETURN(cint_field_sniff_egress_config(unit, destroy_config, epmf_context_id));

    /** Destroy all configuration that was done */
    if(destroy_config)
    {
        BCM_IF_ERROR_RETURN(bcm_mirror_destination_destroy(unit,ss_cmd));
        BCM_IF_ERROR_RETURN(bcm_mirror_destination_destroy(unit,snoop_cmd));
        BCM_IF_ERROR_RETURN(bcm_mirror_destination_destroy(unit,mirror_cmd));
        BCM_IF_ERROR_RETURN(cint_field_sniff_destroy(unit,ipmf1_context_id,epmf_context_id));
