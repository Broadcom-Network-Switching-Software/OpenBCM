    ...
    uint8 is_rif)
{
    ...
    if (is_rif == 0)
    {
        ...
        BCM_IF_ERROR_RETURN(bcm_port_class_get(unit, outlif_gport, bcmPortClassFieldEgressVport, &outlif_profile_get));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &cint_epmf_outlif_profile_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, cint_epmf_outlif_profile_fg_id, context_id, &attach_info));
    ...
    printf("Adding Entry\n");
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, cint_epmf_outlif_profile_fg_id, &ent_info, &cint_epmf_outlif_profile_ent_id));
    ...
    bcm_field_context_t context_id)
{
     ...
     */
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, cint_epmf_outlif_profile_fg_id, NULL, cint_epmf_outlif_profile_ent_id));

    /*
     * Detach the IPMF1 FG from its context 
     */
    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, cint_epmf_outlif_profile_fg_id, context_id));

    /*
     * Delete FG 
     */
    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, cint_epmf_outlif_profile_fg_id));
    ...
    int outlif_profile)
{
     ...
     */
    BCM_IF_ERROR_RETURN(dnx_basic_example(unit, in_port, out_port, kaps_result));


    BCM_GPORT_TUNNEL_ID_SET(intf_gport, intf_out);
    BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, intf_gport, bcmPortClassFieldEgressVport, outlif_profile));


    /* Verify outlif_profile was set */
    BCM_IF_ERROR_RETURN(bcm_port_class_get(unit, intf_gport, bcmPortClassFieldEgressVport, &outlif_profile_get));
    ...
    for (i = 0; i < 3; i++)
    {
         ...
         */
        BCM_IF_ERROR_RETURN(intf_eth_rif_create(unit, intf_out, intf_out_mac_address));

        /*
         * Add host entry
         */
        BCM_GPORT_LOCAL_SET(gport, out_port);
        BCM_IF_ERROR_RETURN(add_host_ipv4(unit, host, vrf, intf_out, encap_id /* arp id */ , gport));
