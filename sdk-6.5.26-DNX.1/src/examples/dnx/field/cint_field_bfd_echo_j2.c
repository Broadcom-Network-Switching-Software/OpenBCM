    ...
    int offset_of_valid_bit_of_ipmf1_tcam_action)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &zero_qual_id));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 1;
    dest_char = &(qual_info.name[0]);
    sal_strncpy_s(dest_char, "hit_bit_bfd", sizeof(qual_info.name));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &hit_qual_id));

    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &oam_action));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, fg_id, context_id_of_ipmf1_tcam_fg, &attach_info));
    ...
    int offset_of_valid_bit_of_ipmf1_tcam_action)
{
    ...
    action_info.prefix_value = 0;
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &fem_increment_void_action_id));
...
int cint_field_bfd_echo_main(int unit,bcm_field_context_t context_id_bfd_echo, int is_ipv6)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit,0,&qual_info,&cint_field_bfd_echo_desc_qual_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &cint_field_echo_bfd_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, cint_field_echo_bfd_fg_id, context_id_bfd_echo, &attach_info));

    printf("Attached  FG (%d) to context (%d)\n",cint_field_echo_bfd_fg_id,context_id_bfd_echo);

    BCM_IF_ERROR_RETURN(bcm_field_group_action_offset_get(unit,0,cint_field_echo_bfd_fg_id,bcmFieldActionTrap,&offset));
    ...
    if (traffic_with_ptch && !use_oam_offset)
    {
        ...
        BCM_IF_ERROR_RETURN(cint_field_echo_bfd_de_for_oam_offset_with_ptch(unit,context_id_bfd_echo,cint_field_echo_bfd_fg_id,offset-1));
    }
    else{
        /*traffice doesn't take ptch header as it's from NIF*/
        BCM_IF_ERROR_RETURN(cint_field_echo_bfd_de_for_oam_offset(unit,context_id_bfd_echo,cint_field_echo_bfd_fg_id,offset-1));
    }

    BCM_IF_ERROR_RETURN(bcm_rx_trap_type_get(unit,0, is_ipv6 ? bcmRxTrapOamBfdIpv6 : bcmRxTrapOamBfdIpv4, &cint_field_echo_trap_id));
...
int cint_field_bfd_echo_entry_add(int unit, uint32 dest_ip, bcm_ip6_t *dest_ip6, uint16 local_discr, uint16 ep_id)
{
    ...
    for (ii = 0; ii < 8; ii++) {
        ...
        BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, cint_field_echo_bfd_fg_id, &ent_info, &(cint_bfd_echo_entry_id)));
