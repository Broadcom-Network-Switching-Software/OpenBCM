    ...
    char * name)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &bcm_action));
    ...
    bcm_field_group_info_t *fg_info_p)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, external_fg_id, &entry_info, &entry_id));
    ...
    bcm_field_group_info_t *fg_info_p)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, BCM_FIELD_FLAG_UPDATE, external_fg_id, &entry_info, &entry_id));
...
cint_field_external_main(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_external_fg_add(unit, &fg_info, &external_fg_id, "main"));

    BCM_IF_ERROR_RETURN(cint_field_external_fg_attach(unit, external_fg_id, &fg_info, external_app_type, payload_id, external_fg_payload_offset));

    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit, bcmSwitchExternalTcamSync, 0));

    BCM_IF_ERROR_RETURN(cint_field_external_entry_add(unit, external_fg_id, &fg_info));
...
cint_field_external_shared_acl_main(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_external_fg_add(unit, &fg_info, &external_fg_id, "1"));

    BCM_IF_ERROR_RETURN(cint_field_external_fg_attach(unit, external_fg_id, &fg_info, external_app_type, payload_id, external_fg_payload_offset));

    BCM_IF_ERROR_RETURN(cint_field_external_fg_add(unit, &fg_info_2, &external_fg_2_id, "2"));

    BCM_IF_ERROR_RETURN(cint_field_external_fg_attach(unit, external_fg_2_id, &fg_info_2, external_app_type, payload_id_2, external_fg_payload_offset_2));

    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit, bcmSwitchExternalTcamSync, 0));

    BCM_IF_ERROR_RETURN(cint_field_external_entry_add(unit, external_fg_id, &fg_info));

    BCM_IF_ERROR_RETURN(cint_field_external_entry_add(unit, external_fg_2_id, &fg_info_2));
...
cint_field_external_destroy(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, external_fg_id, NULL, entry_id));

    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, external_fg_id, NULL, traffic_entry_id));
