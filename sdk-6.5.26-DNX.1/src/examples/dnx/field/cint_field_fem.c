    ...
    int unit)
{
    ...
    */
     ...
     */
    BCM_IF_ERROR_RETURN(bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF1, &context_info, &Cint_field_fem_context_id));
   /**
    ...
    cint_field_fem_actions_info_t * action_info_p)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &action_info_p->action_id));
    return BCM_E_NONE;
}
/**
    ...
    int fem_iterator)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_fem_action_add(unit, 0, fg_id, action_priority, &fem_action_info));
    ...
    int is_fem_fg)
{
    ...
    for (qual_index = 0; qual_index < fem_fg_info_p->nof_quals; qual_index++)
    {
        ...
        {
            BCM_IF_ERROR_RETURN(cint_field_fem_user_qual_config(unit, fg_info.stage, &(fem_fg_info_p->qual_info[qual_index])));
        }
    ...
    for (action_index = 0; action_index < fem_fg_info_p->nof_actions; action_index++)
    {
        ...
        {
            BCM_IF_ERROR_RETURN(cint_field_fem_user_action_config(unit, fg_info.stage, &(fem_fg_info_p->action_info[action_index])));
        }
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &fem_fg_info_p->fg_id));
    /**
    ...
    if (is_fem_fg == TRUE)
    {
        ...
        {
            BCM_IF_ERROR_RETURN(cint_field_fem_action_config(unit, fem_fg_info_p->fg_id, &fem_action_info_p[fem_iterator], fem_iterator));
        }
    }

    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, fem_fg_info_p->fg_id, Cint_field_fem_context_id, &attach_info));
    /**
     * IF the FG is TCAM we should create an entry!
     */
    if (fg_info.fg_type == bcmFieldGroupTypeTcam)
    {
         BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, fem_fg_info_p->fg_id, &entry_info, &(fem_fg_info_p->entry_id)));
    }
    return BCM_E_NONE;
}
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_fem_context_create_presel_set(unit));
    ...
    BCM_IF_ERROR_RETURN(cint_field_fem_fg_config(unit, bcmFieldStageIngressPMF2, Cint_field_fem_DE_fg_info_arr,
    ...
    int unit)
{
     ...
     */
    printf("Create DE Fg which is using FEM actions and Attach it to the context.. \n");
    BCM_IF_ERROR_RETURN(cint_field_fem_fg_config(unit, bcmFieldStageIngressPMF2, Cint_field_fem_TCAM_fg_info_arr,
            Cint_field_fem_action_info_arr, TRUE));
    return BCM_E_NONE;
}
    ...
    int fg_type)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, fg_info_p->fg_id,
    ...
    for (fem_index = 0; fem_index < Cint_field_fem_action_info_arr[0].nof_fem_actions; fem_index++)
    {
        ...
        if (!fem_was_removed)
        {
            ...
            action_priority = BCM_FIELD_ACTION_POSITION(array_id,fem_position_in_array);
            BCM_IF_ERROR_RETURN(bcm_field_fem_action_delete(unit, fg_info_p->fg_id,action_priority));
        }
    }
    if(fg_type == FG_type_TCAM)
    {
        BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, fg_info_p->fg_id, NULL, fg_info_p->entry_id));
    }

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, fg_info_p->fg_id));

    BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit, bcmFieldStageIngressPMF1, Cint_field_fem_context_id));
    /**
    ...
    for (qual_index = 0; qual_index < fg_info_p->nof_quals; qual_index++)
    {
        ...
        {
            BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, fg_info_p->qual_info[qual_index].qual_id));
        }
    }
    /**
    ...
    for (action_index = 0; action_index < fg_info_p->nof_actions; action_index++)
    {
        ...
        {
            BCM_IF_ERROR_RETURN(bcm_field_action_destroy(unit, fg_info_p->action_info[action_index].action_id));
        }
    }
    return BCM_E_NONE;
}
