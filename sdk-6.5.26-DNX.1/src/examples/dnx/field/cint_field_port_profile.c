    ...
    uint32 profile_id_bit)
{
    ...
    qual_info.size = 1;
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &Port_profile_bitmap_profile_qual));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_info_get(unit, predefined_qual, stage, &qual_info_get));

    qual_offset = qual_info_get.offset;

    /* Set the port profile in the wanted bit.*/
    BCM_IF_ERROR_RETURN(bcm_port_class_get(unit, gport, port_class, &profile_total_value));

    profile_total_value |= (1 << profile_id_bit);
    BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, gport, port_class, profile_total_value));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &Port_profile_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, Port_profile_fg_id, context_id, &attach_info));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, Port_profile_fg_id, &entry_info, &Port_profile_entry_handle));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, Port_profile_fg_id, &entry_info, &Port_profile_entry_handle_default));
    ...
    uint32 profile_id_num)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, gport, port_class, profile_id_num));
    ...
    uint32 profile_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_class_get(unit, gport_in, port_class, &Port_profile_old_value));


    /* Perform configurations.*/
    if (bitmap_or_single)
    {
        BCM_IF_ERROR_RETURN(field_port_profile_bitmap_configure(unit, bcmFieldStageIngressPMF1, context_id, gport_in, port_class, pp_or_tm, profile_id));

    }
    else
    {
        BCM_IF_ERROR_RETURN(field_port_profile_single_configure(unit, bcmFieldStageIngressPMF1, context_id, gport_in, port_class, pp_or_tm, profile_id));
    ...
    int pp_or_tm)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, Port_profile_fg_id, &entry_qual_info, Port_profile_entry_handle));


    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, Port_profile_fg_id, &entry_qual_info, Port_profile_entry_handle_default));


    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, Port_profile_fg_id, context_id));


    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, Port_profile_fg_id));


    if (Port_profile_bitmap_profile_qual != bcmFieldQualifyCount)
    {
        BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, Port_profile_bitmap_profile_qual));

        Port_profile_bitmap_profile_qual = bcmFieldQualifyCount;
    }

    BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, gport_in, port_class, Port_profile_old_value));
