    ...
    bcm_field_context_t *context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, recycle_port_mc, bcmPortClassFieldIngressPMF1PacketProcessingPortCs, ing_port_pmf_cs_profile));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_presel_set(unit, 0, &mc_ipmf1_presel_entry_id, &presel_entry_data));
    ...
    field_ftmh_action_info_t * ftmh_action_info)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &(ftmh_action_info->action_id)));
    return BCM_E_NONE;
}
    ...
    field_ftmh_fg_info_t * ftmh_fg_info)
{
    ...
    BCM_IF_ERROR_RETURN(field_ftmh_context(unit, &(ftmh_fg_info->context_id)));
    ...
    {
        BCM_IF_ERROR_RETURN(field_ftmh_user_qual_config(unit, fg_info.stage, &(ftmh_fg_info->ftmh_qual_info[qual_index])));
    ...
    for (action_index = 0; action_index < fg_info.nof_actions; action_index++)
    {
        ...
        {
            BCM_IF_ERROR_RETURN(field_ftmh_user_action_config(unit, fg_info.stage, ftmh_fg_info->ftmh_action_info[action_index]));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &(ftmh_fg_info->fg_id)));

    for (fem_index = 0; fem_index < ftmh_fg_info->nof_fems; fem_index++)
    {
        BCM_IF_ERROR_RETURN(field_ftmh_fem_action_config(unit, ftmh_fg_info->fg_id, ftmh_fg_info->ftmh_fem_action_info[fem_index]));
    }

    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, ftmh_fg_info->fg_id, ftmh_fg_info->context_id, &attach_info));
    ...
    int unit)
{
    ...
    {
        BCM_IF_ERROR_RETURN(field_ftmh_group_config(unit, &(cint_ftmh_fg_info_array[fg_index])));
    }
    ...
    int unit)
{
    ...
    for (fg_index = 0; fg_index < FTMH_NOF_FG; fg_index++)
    {
        ...
        BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, ftmh_fg_info.fg_id,
        ...
        for (fem_index = 0; fem_index < ftmh_fg_info.nof_fems; fem_index++)
        {
            ...
            action_priority = BCM_FIELD_ACTION_POSITION(array_id,position);
            BCM_IF_ERROR_RETURN(bcm_field_fem_action_delete(unit, ftmh_fg_info.fg_id,action_priority));
        }

        BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, ftmh_fg_info.fg_id));

        BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit, bcmFieldStageIngressPMF1, ftmh_fg_info.context_id));
        for (qual_index = 0; qual_index < ftmh_fg_info.nof_quals; qual_index++)
        {
            BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, ftmh_fg_info.ftmh_qual_info[qual_index].qual_id));
        }
        ...
        for (action_index = 0; action_index < ftmh_fg_info.nof_actions; action_index++)
        {
            if (ftmh_fg_info.ftmh_action_info[action_index].name != NULL)
            {
                BCM_IF_ERROR_RETURN(bcm_field_action_destroy(unit, ftmh_fg_info.ftmh_action_info[action_index].action_id));
            }
        }
    }
    return BCM_E_NONE;
