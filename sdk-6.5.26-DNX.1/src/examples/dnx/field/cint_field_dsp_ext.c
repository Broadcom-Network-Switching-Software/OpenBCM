    ...
    int dsp_ext_value)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &Cint_field_dsp_ext_fg_id));

    printf("Field Group ID %d was created. \n", Cint_field_dsp_ext_fg_id);

    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, Cint_field_dsp_ext_fg_id, context_id, &attach_info));

    printf("Field Group ID %d was attached to Context ID %d. \n", Cint_field_dsp_ext_fg_id, context_id);

    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, Cint_field_dsp_ext_fg_id, &entry_info, &Cint_field_dsp_ext_entry_handle));
    ...
    bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, Cint_field_dsp_ext_fg_id, NULL, Cint_field_dsp_ext_entry_handle));

    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, Cint_field_dsp_ext_fg_id, context_id));

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, Cint_field_dsp_ext_fg_id));
