...
int field_de_main(int unit, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_context_create(unit, BCM_FIELD_FLAG_WITH_ID, bcmFieldStageIngressPMF1, &context_info, &context_id));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 3;
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &itmh_trap_strength_3_bit));
    ...
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 21;
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &dest_qual));

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 9;
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &trap_code_ones));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &dest_action));
    ...
    action_info.size = 13;
    action_info.stage = bcmFieldStageIngressPMF2;
    BCM_IF_ERROR_RETURN(bcm_field_action_create(unit, 0, &action_info, &trap_action));
...
int field_basic_de_destroy(int unit, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_context_destroy(unit,bcmFieldStageIngressPMF1,context_id));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, itmh_trap_strength_3_bit));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, pph_trap_strength_1_bit));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, trap_code_ones));
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, dest_qual));
    BCM_IF_ERROR_RETURN(bcm_field_action_destroy(unit, dest_action));
    BCM_IF_ERROR_RETURN(bcm_field_action_destroy(unit, trap_action));
