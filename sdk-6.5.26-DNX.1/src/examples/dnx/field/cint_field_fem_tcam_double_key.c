    ...
    cint_field_fem_tcam_double_key_qual_info_t * fem_qual_info)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualifier_create(unit, 0, &qual_info, &(fem_qual_info->qual_type)));
    ...
    cint_field_fem_tcam_double_key_fg_info_t * fem_fg_info)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &(fem_fg_info->fg_id)));
    ...
    if (fem_fg_info->fem_action_info != NULL)
    {
        ...
        {
            BCM_IF_ERROR_RETURN(cint_field_fem_tcam_double_key_fem_action_config
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, fem_fg_info->fg_id, context_id, &attach_info));

    printf("Field Group ID %d was attached to Context ID %d. \n", fem_fg_info->fg_id, context_id);

    /** Add an entry to the created field group. */
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, fem_fg_info->fg_id, &entry_info, &(fem_fg_info->entry_handle)));
    ...
    bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_fem_tcam_double_key_user_qual_config(unit, &(Cint_field_fem_tcam_double_key_fg_info_array[0].qual_info[0])));

    /** Creating FEM TCAM FG. */
    BCM_IF_ERROR_RETURN(cint_field_fem_tcam_double_key_group_config(unit, context_id, Cint_field_fem_tcam_double_key_fg_info_array[0]));
    ...
    cint_field_fem_tcam_double_key_fg_info_t * fem_fg_info)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, fem_fg_info->fg_id, NULL, fem_fg_info->entry_handle));

    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, fem_fg_info->fg_id, context_id));
    ...
    for (fem_index = 0; fem_index < fem_fg_info->nof_fems; fem_index++)
    {
        ...
        BCM_IF_ERROR_RETURN(bcm_field_fem_action_delete(unit, fem_fg_info->fg_id, action_priority));
    }

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, fem_fg_info->fg_id));

    for (qual_index = 0; qual_index < fem_fg_info->nof_quals; qual_index++)
    {
        BCM_IF_ERROR_RETURN(bcm_field_qualifier_destroy(unit, fem_fg_info->qual_info[qual_index].qual_type));
    }
    ...
    bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN(cint_field_fem_tcam_double_key_destroy_internal(unit, context_id, Cint_field_fem_tcam_double_key_fg_info_array[0]));
