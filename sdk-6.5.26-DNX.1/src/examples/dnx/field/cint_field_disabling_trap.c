       ...
       int *trap_id)
{
    ...
    /* Create the trap */
    BCM_IF_ERROR_RETURN(bcm_rx_trap_type_create(unit, 0, bcmRxTrapLinkLayerSaEqualsDa, &cint_field_disabling_trap_trap_id));
    printf("Trap %d was created. \n", cint_field_disabling_trap_trap_id);

    /* Set the trap */
    BCM_IF_ERROR_RETURN(bcm_rx_trap_set(unit, cint_field_disabling_trap_trap_id, trap_config));
       ...
       int *default_trap_id)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_add(unit, 0, &fg_info, &cint_field_disabling_trap_fg_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_context_attach(unit, 0, cint_field_disabling_trap_fg_id, cint_field_disabling_trap_ctx_id, &attach_info));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_add(unit, 0, cint_field_disabling_trap_fg_id, &entry_info, cint_field_disabling_trap_entry_id));
       ...
       int *default_trap_id)
{
   ...
   BCM_IF_ERROR_RETURN(cint_field_disabling_trap_add_fg(unit, default_trap_id));
...
int cint_field_disabling_trap_destroy(
        ...
        int ipmf1_ctx_id)
{
    BCM_IF_ERROR_RETURN(bcm_field_entry_delete(unit, cint_field_disabling_trap_fg_id, NULL, cint_field_disabling_trap_entry_id));
    printf("Entry 0x%X (%d) was deleted. \n", cint_field_disabling_trap_entry_id, cint_field_disabling_trap_entry_id);

    BCM_IF_ERROR_RETURN(bcm_field_group_context_detach(unit, cint_field_disabling_trap_fg_id, ipmf1_ctx_id));
    printf("Field Group %d was detach from context %d. \n", cint_field_disabling_trap_fg_id, ipmf1_ctx_id);

    BCM_IF_ERROR_RETURN(bcm_field_group_delete(unit, cint_field_disabling_trap_fg_id));
    printf("Field Group %d was deleted. \n", cint_field_disabling_trap_fg_id);

    BCM_IF_ERROR_RETURN(bcm_rx_trap_type_destroy(unit, cint_field_disabling_trap_trap_id));
