    ...
    bcm_port_t port2)
{
    ...
    rv = dnx_oam_accelerated_endpoint_create(unit, oam_perf_up_mep_id, gport2, group_info.id,
                                             counter_if_0, lm_counter_base_id_0, acc_egress_profile, 0, 1, BCM_OAM_OPCODE_CCM_IN_HW);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = oam_set_opcode_destination_to_oamp(unit, 1/**ingress */, acc_ingress_profile, bcmOamOpcodeDMM, bcmOAMEndpointTypeEthernet);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %s, could not create action for DMM.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Create action of trapping incoming loss measurement packet for down-MEPs */
    rv = oam_set_opcode_destination_to_oamp(unit, 1/**ingress */, acc_ingress_profile, bcmOamOpcodeLMM, bcmOAMEndpointTypeEthernet);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %s, could not create action for LMM.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Create action of trapping incoming delay measurement response packet for down-MEPs */
    rv = oam_set_opcode_destination_to_oamp(unit, 1/**ingress */, acc_ingress_profile, bcmOamOpcodeDMR, bcmOAMEndpointTypeEthernet);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %s, could not create action for DMM.\n", bcm_errmsg(rv));
        return rv;
    ...
    rv = oam_set_opcode_destination_to_oamp(unit, 1/**ingress */, acc_ingress_profile, bcmOamOpcodeLMR, bcmOAMEndpointTypeEthernet);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %s, could not create action for LMM.\n", bcm_errmsg(rv));
        return rv;
    ...
    if(device_type_v2)
    {
         ...
         */
        rv = dnx_oam_accelerated_endpoint_create(unit, oam_perf_lmm_up_mep_id, gport2, group_info.id,
                                                 counter_if_0, lm_counter_base_id_0, acc_egress_profile, 0, 1, BCM_OAM_OPCODE_LMM_IN_HW);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %s, in dnx_oam_accelerated_endpoint_create (Up MEP LMM).\n", bcm_errmsg(rv));
            return rv;
        }
         ...
         * as well.
         */
        rv = dnx_oam_accelerated_endpoint_create(unit, oam_perf_dmm_down_mep_id, gport2, group_info.id,
                                                 counter_if_0, lm_counter_base_id_0, acc_ingress_profile, port2, 0, BCM_OAM_OPCODE_DMM_IN_HW);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %s, in dnx_oam_accelerated_endpoint_create (Down MEP DMM).\n", bcm_errmsg(rv));
            return rv;
        }
         ...
         */
        rv = dnx_oam_accelerated_endpoint_create(unit, oam_perf_dmm_up_mep_id, gport2, group_info.id,
                                                 counter_if_0, lm_counter_base_id_0, acc_egress_profile, 0, 1, BCM_OAM_OPCODE_DMM_IN_HW);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %s, in dnx_oam_accelerated_endpoint_create (Up MEP DMM).\n", bcm_errmsg(rv));
            return rv;
        }
    }
    else
    ...
    bcm_port_t port2)
{
    ...
    if (oam_lm_dm_reports_enabled)
    {
        ...
        rv = bcm_oam_control_set(unit, bcmOamControlReportMode, arg);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n", bcm_errmsg(rv));
            return rv;
        }
        ...
        rv = bcm_oam_performance_event_register(unit, loss_event, cb, (void*)1);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n", bcm_errmsg(rv));
            return rv;
        }
        ...
        rv = bcm_oam_performance_event_register(unit, delay_event, cb, (void*)2);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n", bcm_errmsg(rv));
            return rv;
        }
