...
n.c 
n.c Copyright 2007-2022 Broadcom Inc. All rights reserved.File: cint_oam_slm.c Purpose: Examples for JR OAM SLM. 
...
struct oam_lif_profiles_s
{
   ...
   bcm_oam_profile_t oam_profile_eth_slm;
   ...
   bcm_oam_profile_t oam_profile_mpls_slm;
   ...
   bcm_oam_profile_t oam_profile_pwe_slm;
};
...
bcm_oam_profile_t * oam_lif_profiles_t[]=
{
   ...
   &oam_lif_profiles.oam_profile_eth_slm,
   ...
   &oam_lif_profiles.oam_profile_mpls_slm,
   ...
   &oam_lif_profiles.oam_profile_pwe_slm
};
    ...
    oam_destinations_s * oam_destinations)
{
     ...
     */
    rv = bcm_rx_trap_type_get(unit,0 , bcmRxTrapOamReflector, &trap_code);
    if (rv != BCM_E_NONE)
    {
        printf("(%s) \n", bcm_errmsg(rv));
    }
    ...
    oam_destinations_s oam_destinations)
{
    ...
    is_slm_profile_supported = *(dnxc_data_get(unit,  "oam", "general", "oam_slm_dedicated_profile_supported", NULL));

    last_lm_type = LM_TYPE_NONE;

    for (lif_type = MEP_TYPE_ETH; lif_type <= MEP_TYPE_PWE; lif_type++)
    {
        for (lm_type = LM_TYPE_SINGLE_ENDED; lm_type <= last_lm_type; lm_type++)
        {
            /*
             * Create OAM ingress profile
             */
            ...
            *(oam_lif_profiles_t[lif_profile_index]) = ingress_profile_id;
            ...
            if (lif_type == MEP_TYPE_ETH)
            {
                egress_profile_id = ingress_profile_id & 0xF;
                profile_type = bcmOAMProfileEgressLIF;
                rv = bcm_oam_profile_create(unit, flags|BCM_OAM_PROFILE_WITH_ID, profile_type, &egress_profile_id);
                if (rv != BCM_E_NONE)
                {
                    printf("Error no %d, in bcm_oam_profile_create.\n", rv);
                    return rv;
            ...
            lif_profile_index++;
            /*
             * oam_profile_eth_slm should be skipped in oam_lif_profiles_t[] 
             * for slm profile unsupported cases
             */
            if ((is_slm_profile_supported == 0) && (lm_type == LM_TYPE_NONE))
            {
                lif_profile_index++;
            }
        }
    ...
    bcm_oam_profile_t *acc_ingress_profile)
{
   ...
   is_acc = 1;
   profile_type = bcmOAMProfileIngressAcceleratedEndpoint;
   /** No need to create SLM specific ACC profile */
...
int oam_set_opcode_destination_to_oamp(int unit,int is_ingress ,int acc_profile_id, int opcode, bcm_oam_endpoint_type_t type) {
    ...
    if (type == bcmOAMEndpointTypeBHHMPLS) {
        if (device_type_v2 && (opcode == bcmOamOpcodeLMR || opcode == bcmOamOpcodeDMR || opcode == bcmOamOpcode1DM))
        {
            bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceY1731MplsTpAcceleratedResponse, &trap_code);
        }
        else if (device_type_v2 && (opcode == bcmOamOpcodeSLR))
        {
            bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamSlrEthAccelerated, &trap_code);
        }
        else if (opcode == bcmOamOpcode1DM || opcode == bcmOamOpcodeLMM || opcode == bcmOamOpcodeDMM ||
                (!device_type_v2 && (opcode == bcmOamOpcodeLMR || opcode == bcmOamOpcodeDMR)))
        {
        ...
        if (device_type_v2 && (opcode == bcmOamOpcodeLMR || opcode == bcmOamOpcodeDMR))
        {
            if (is_pcp_counting_enabled)
            {
                bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceEthAcceleratedResponsePriority, &trap_code);
            }
            else
            {
                bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceEthAcceleratedResponse, &trap_code);
            }
        }
        else if (device_type_v2 && (opcode == bcmOamOpcodeSLM))
        {
            bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamSlmEthAccelerated, &trap_code);
        }
        else if (device_type_v2 && (opcode == bcmOamOpcodeSLR))
        {
            bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamSlrEthAccelerated, &trap_code);
        ...
        if (!device_type_v2 && is_pcp_counting_enabled)
        {
            rv = bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamUpMEPDest2, &trap_code);
        }
        else
        {
            if(device_type_v2 && (opcode == bcmOamOpcodeLMR || opcode == bcmOamOpcodeDMR || opcode == bcmOamOpcodeSLR))
            {
                if(is_pcp_counting_enabled)
                {
                    rv = bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamUpMEPOampPerformenceResponsePriority, &trap_code);
                }
                else
                {
                rv = bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamUpMEPOampPerformenceResponse, &trap_code);
            }
            }
            else
            {
    ...
    bcm_oam_profile_t profile_id)
{
    ...
    if (dest_port != BCM_GPORT_INVALID && !BCM_GPORT_IS_TRAP(dest_port))
    {
        ...
        rv = dnx_oam__switch_contol_indexed_port_set(unit, dest_port, BCM_SWITCH_PORT_HEADER_TYPE_CPU, 2);
        if (rv != BCM_E_NONE)
        {
 ...
 * Relevant for Jer2 only. Jer3 have own dedicated trap bcmRxTrapEgTxOamUpMEPOampPerformenceResponsePriority
 * @param unit
 * @return error code
 */
