...
struct cint_vpls_fec_info_s
{
    ...
    int use_gal_with_value_14;                   /* Special case where packets are TX'ed with GAL value 14*/
};
    ...
    int unit)
{
    ...
    mpls_port.egress_label.flags |= BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT|BCM_MPLS_EGRESS_LABEL_STAT_ENABLE;
    ...
    int pwe_port)
{
    ...
    if (cint_vpls_fec_info.use_gal_with_value_14)
    {
        /* Set GAL 14 with switch control*/
        rv = bcm_switch_control_set(unit,bcmSwitchOamAdditionalGalSpecialLabel,14); 
        if (rv != BCM_E_NONE)
        {
            printf("Error(%d), in bcmSwitchOamAdditionalGalSpecialLabel config\n", rv);
            return rv;
        }
    }
    ...
    int is_dual_ended_lm)
{
    ...
    if (cint_vpls_fec_info.use_gal_with_value_14 && bcmOAMEndpointTypeBhhSection != mep_type)
    {
        /* Optional additional GAL label*/
