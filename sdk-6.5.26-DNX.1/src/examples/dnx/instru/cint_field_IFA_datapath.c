...
int IPT_PROFILE_IFA1 = 3; /* Used for initiator ?? node */
 ...
 * The Actions:
 *  1. Set IPT profile as IPT_PROFILE_IFA1
...
int cint_field_IFA_datapath_intermediate_ingress_entry_add(int unit, uint32 probe_1, uint32 probe_2, bcm_field_entry_t *entry_handle)
{
    ...
    ent_info.entry_action[0].value[0] = IPT_PROFILE_IFA1;
...
*             1- Snoop Packet
*             2- Set Tail Edit Profile = IPT_PROFILE_IFA1
...
*            Action: Set Tail Edit Profile = IPT_PROFILE_IFA1
*
* Parameters:
*  unit        -  Device id
...
int cint_field_group_const_example_IFA_gen(int unit,int Outlif,int recycle_port)
{
    ...
    if (*dnxc_data_get(unit,"instru","ifa","ingress_tod_feature",NULL))
    {
      ...
      rv = bcm_field_group_add(unit, 0, &fg_info, &cint_field_IFA_datapath_ingress_fg_id_2p_tsh);
      if (rv != BCM_E_NONE)
      {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
    ...
    /* *********************** */
      ...
      */
      ...
      ace_entry_info.entry_action[0].value[0] = IPT_PROFILE_IFA1;
