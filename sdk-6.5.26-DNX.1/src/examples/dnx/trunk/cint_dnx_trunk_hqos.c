...
create_trunks(int unit, bcm_port_t src_port)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_trunk_create,unit $unit trunk_id: %x, $rv\n",master_trunk_id);
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error in bcm_switch_control_indexed_port_set (LAG gport)\n");
        return rv;
    }
...
cint_dnx_trunk_e2e_scheme_destory(int unit, bcm_port_t port)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        for (cos = 0; cos < num_cos; cos++)
        {
            ...
            if (rv != BCM_E_NONE)
            {
                printf("Error, bcm_cosq_gport_detach,unit $unit gport: %x, $rv\n",gport_info.out_gport);
                return rv;
            }
        }
...
cint_dnx_trunk_hqos_clean(int unit, bcm_port_t dst_port)
{
    ...
    /* disable forwarding */
    rv = cint_dnx_trunk_hqos_forward(unit, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in cint_dnx_trunk_hqos_forward when disabling forwarding during cleanup, $rv \n");
        return rv;
    }
    ...
    /* destroy e2e scheme */
    rv = cint_dnx_trunk_e2e_scheme_destory(unit, dst_port);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in cint_dnx_trunk_e2e_scheme_destory, $rv \n");
        return rv;
    }
