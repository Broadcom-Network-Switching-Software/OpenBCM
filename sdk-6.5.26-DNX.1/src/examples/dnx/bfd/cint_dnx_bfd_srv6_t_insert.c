  ...
  int unit)
{
    ...
    if (rc != BCM_E_NONE)
    {
        printf("Error, bcm_vlan_port_create failed\n");
        return rc;
    }
    ...
    rc = bcm_l2_egress_create(unit, &recycle_entry);
    if (rc != BCM_E_NONE)
    {
        printf("Error, bcm_l2_egress_create failed\n");
        return rc;
    }
  ...
  int unit)
{
    ...
    /** create recycle encap */
    rc = dnx_recycle_encap_create(unit);
    if (rc != BCM_E_NONE)
    {
        printf("Error, dnx_recycle_encap_create failed\n");
        return rc;
    }
    ...
    rc = bcm_bfd_endpoint_create(unit, &ep);
    if (rc != BCM_E_NONE)
    {
        printf("Error, bcm_l2_egress_create failed\n");
        return rc;
    ...
    uint32 new_fai)
{
    ...
    rc = bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF1, &context_info, &pmf1_ctx_id);
    ...
    rc = bcm_field_presel_set(unit, 0, &ipmf_presel_entry_id, &ipmf_presel_entry_data);
    if (rc != BCM_E_NONE)
    {
        printf("Error, bcm_field_presel_set failed\n");
        return rc;
    }
    ...
    rc = bcm_field_qualifier_create(unit, 0, &qual_info, &qual_id_fai_3_0);
    if (rc != BCM_E_NONE)
    {
        printf("Error, bcm_field_qualifier_create failed\n");
        return rc;
    }
    ...
    rc = bcm_field_group_add(unit, 0, &fg_info, &bfd_fg_id);
    if (rc != BCM_E_NONE)
    {
        printf("Error, bcm_field_group_add failed\n");
        return rc;
    }
    ...
    rc = bcm_field_group_context_attach(unit, 0, bfd_fg_id, pmf1_ctx_id, &fg_attach_info);
    if (rc != BCM_E_NONE)
    {
        printf("Error, bcm_field_group_context_attach failed\n");
        return rc;
    }
