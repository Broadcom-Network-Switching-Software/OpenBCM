 ...
 *          o   fap_tdm_bypass=TDM_OPTIMIZED/fap_tdm_bypass=TDM_STANDARD
 *          o   tm_port_header_type_<incoming_port_id>=TDM (for example tm_port_header_type_13=TDM)
 ...
 *          o   fap_tdm_bypass=TDM_OPTIMIZED/fap_tdm_bypass=TDM_STANDARD
 *          o   tm_port_header_type_<incoming_port_id>=TDM (for example tm_port_header_type_13=TDM)
 * For the CINT consists of two applications: TDM standard and TDM optimize.
 *  o   TDM standard:
 ...
 *      o   You can configure different settings for the port editing, including a different
 *          destination port, multicast group settings, FTMH actions, and user defined fields.
 *      Note: Before running the following TDM standard application, be sure the device is configured for TDM Standard
 *
 *  o   TDM optimize:
 *      Note: Before running the following TDM optimize application, be sure the device is configured for TDM Optimize and add fap_device_mode=TDM_OPTIMIZED.
 ...
 *
 * ./bcm.user
 * config delete ucode_port_5.*
 * config delete ucode_port_20.*
 * config add tdm_mode=TDM_OPTIMIZED
 * config delete flexe_device_mode.*
 * config add flexe_device_mode.0=DISABLED
 * config delete ucode_port_17.*
 * config add ucode_port_17=CGE20:core_0.17:if_tdm_only:tdm
 * tr 141
 *
 * cint ../../../../src/examples/dnx/utility/cint_dnx_utils_global.c
 * cint ../../../../src/examples/dnx/utility/cint_dnx_utils_multicast.c
 * cint ../../../../src/examples/dnx/tdm/cint_dnx_tdm_examples.c
 * cint
 * cint_dnx_tdm_example(0, 17, 17, 0, 1 ,64, 511, 5005, 3, 14, 15);
 * exit;
 *
 * Test Scenario - end
 */
    ...
    int *user_field)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error in bcm_fabric_tdm_editing_set failed \n");
        return rv;
    }
    ...
    int mc_port2)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error in cint_tdm_example_multi_device failed \n");
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error in bcm_fabric_static_replication_set failed \n");
        return rv;
    }
    ...
    int *ingress_context_id_p)
{
    ...
    /*
     * Indicate all available fabric links should be assigned to TDM (as well as other sources).
     */
    flags = 0;
    type_as_int = type;
    if (detailed_dnx_print)
    ...
    rv = bcm_tdm_ingress_context_create(unit, flags, &context_id, &ingress_context);
    if (rv != BCM_E_NONE)
    {
    ...
    *ingress_context_id_p = context_id;
    ...
    if (set_interface_config_p->stream_id_enable == TRUE)
    {
        for (i_stream = 0; i_stream < SAL_BIT(set_interface_config_p->stream_id_key_size); i_stream++)
        {
            ...
            rv = bcm_tdm_stream_ingress_context_set(unit, 0, gport, context_id);
            if (rv != BCM_E_NONE)
            {
    ...
    else
    {
        ...
        if (detailed_dnx_print)
        {
            printf("\r\n %s(): 2. Call %s() stream_context %d stream_id %d gport 0x%X\r\n", proc_name, "bcm_tdm_stream_ingress_context_set", context_id, stream_id, gport);
        }
        rv = bcm_tdm_stream_ingress_context_set(unit, 0, gport, context_id);
        if (rv != BCM_E_NONE)
        {
 ...
 *   For zero mc_id, 'dest_id' is the destination gport. It is either modport or local port or, otherwise, logical port.
 */
int setup_dnx_tdm(
    int unit,
    ...
    int *user_field)
{
    ...
    rv = BCM_E_NONE;
    {
        ...
        rv = bcm_port_get(unit, local_port_id, &flags, &interface_info, &mapping_info);
        if (BCM_FAILURE(rv))
           {
               printf("Error, in bcm_port_get, unit %d.\n", unit);
               return rv;
           }
    ...
    tdm_interface_config.stream_id_base = 1000 + local_port_id;
    tdm_interface_config.stream_id_enable = FALSE;
    tdm_interface_config.stream_id_key_size = 1;
    rv = setup_dnx_tdm_ire_configure(unit,local_port_id,channel_id,is_mc,dest_id,ingress_editing_type, stamp_mcid_with_sid, mc_id, &tdm_interface_config, &context_id_value);
    if (rv != BCM_E_NONE)
    {
...
/*
 * SAR TDM Example:
 * This example can configure a MC or UC to dest_gport.
 * The optimized FTMH will be appended in the ingress, and removed in the egress.
 * It also sets the Cells Size Range: min_size to max_size
 * local_port_id = Incoming BCM local port id.
 * dest_gport = Outgoing BCM local port id or mod port id or logical port id.
 * is_mc = is it multicast configuration or unicast configuration
 ...
 * max_size = max Cells Size Range: up to 512/511
 */
int cint_dnx_tdm_sar_example_multi_device(
    int src_unit,
    int dst_unit,
    bcm_port_t local_port_id,
    bcm_gport_t dest_gport,
    ...
    int max_size)
{
    ...
    int user_field_data[2] = {0};
    bcm_tdm_egress_editing_type_t egress_edit_type = bcmTdmEgressEditingNoStamp;

    proc_name = "cint_dnx_tdm_sar_example_multi_device";
    if (detailed_dnx_print)
    {
        printf("\r\n %s(): ENTER. src_port %d dst_port %d (0x%08X) is_mc %d mc_group_id %d \r\n",
                                        proc_name, local_port_id, dest_gport, dest_gport, is_mc, mc_group_id);
    }
    ...
    if (is_mc)
    {
        if (detailed_dnx_print)
        {
            printf("\r\n %s(): Build multicast group %d :\r\n", proc_name, mc_group_id);
            for (ii = 0; ii < 1; ii++)
            {
                printf("%s(): ii %d mc_ports[ii] %d mc_cuds[ii] %d:\r\n", proc_name, ii, mc_ports[ii], mc_cuds[ii]);
            }
        }
        rv = multicast__open_egress_mc_group_with_local_ports(dst_unit, mc_group_id, mc_ports, mc_cuds, 1, BCM_MULTICAST_SAR_GROUP);
        if (rv != BCM_E_NONE)
        {
            printf("\r\n %s(): 'multicast__open_egress_mc_group_with_local_ports' has failed with code %d. Quit.\r\n", proc_name, rv);
            return rv;
...
/*
 * Simple example for TDM SAR
 * See cint_dnx_tdm_sar_example_multi_device description.
 *                                              .
 */
int cint_dnx_tdm_sar_example(
    int unit,
    bcm_port_t local_port_id,
    bcm_port_t dest_port_id,
    int is_mc,
    ...
    int mc_id)
{
    ...
    rv = cint_dnx_tdm_sar_example_multi_device(unit, unit,
                                         local_port_id,
                                         dest_port_id,
                                         is_mc,
                                         mc_id,
    ...
    if (rv != BCM_E_NONE)
    {
        printf("\r\n %s(): 'cint_dnx_tdm_sar_example_multi_device' has failed. Code (%d). Quit.\r\n", proc_name, rv);
        return rv;
    }
    ...
    int mc_port2)
{
    ...
    if (is_mc)
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error in multicast__open_egress_mc_group_with_local_ports failed \n");
            return rv;
        }
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error in setup_tdm failed \n");
            return rv;
        }
    }
    else
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error in bcm_fabric_control_set failed \n");
        return rv;
    }
    ...
    int mc_port2)
{
    ...
    if (is_mc)
    {
        bcm_fabric_tdm_editing_type_t egress_edit_type = bcmFabricTdmEditingNoChange;
...
/*
 * Destroy SAR TDM bypass datapath
 *
 * local_port_id = Incoming BCM local port id.
 * is_mc = is it multicast configuration or unicast configuration
 * ingress_context_id = context id to destroy, relevant only if context id is known
 */
int cint_tdm_sar_destroy_example (
    int unit,
    bcm_port_t local_port_id,
    int is_mc,
    int mc_group_id,
    ...
    )
{
    ...
    /*
     * Mapped stream to context ID 0 (default value)
     */
    rv = bcm_tdm_stream_ingress_context_set(unit, flags, loc_gport, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error in bcm_fabric_control_set failed \n");
        return rv;
    ...
    if (context_id != -1)
    {
        ingress_context_id = context_id;
        context_id = -1;
    }

    rv = bcm_tdm_ingress_context_destroy(unit, flags, ingress_context_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error in bcm_dnx_tdm_ingress_context_destroy failed \n");
        return rv;
    }
