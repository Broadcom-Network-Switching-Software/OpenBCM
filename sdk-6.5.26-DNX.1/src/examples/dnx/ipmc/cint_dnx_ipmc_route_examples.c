    ...
    int ttl_threshold)
{
    ...
    if (!lpm_raw_data_enabled)
    {
        ...
        mc_id = create_ip_mc_group(unit, mc_flags, ipmc_index, dest_local_port_id, nof_dest_ports, vlan, TRUE);
    }
    ...
    for (port_ndx = 0; port_ndx < nof_dest_ports; ++port_ndx)
    {
        ...
        if (lpm_raw_data_enabled)
        {
            ...
            if (rv != BCM_E_NONE)
            {
                printf("Error, add_ipv4_ipmc_raw\n");
                return rv;
            }
        }
        else
...
int bcm_l3_ip6_mc_rpf(int unit, int port0, int port1, int port2, int port3) {
    ...
    int kbp_dev_type =  *(dnxc_data_get(unit,  "elk", "general", "device_type", NULL));
    int is_kbp = (kbp_dev_type > 0) ? 1 : 0;
    ...
    int out_port)
{
    ...
    for (idx = 0; idx < eg_itf_index; ++idx) {
        ...
        {
            rv = add_ipv4_ipmc_to_port(unit, mc_ip, 0xffffffff, src_ip, 0x0, vlan_l, vrf, ((idx == 0) ? mc_gport_drop : mc_gport_2_cpu), 0);
            if (rv != BCM_E_NONE)
            {
                printf("Error, add_ipv4_ipmc_to_port\n");
                return rv;
            }
        }
        else
        {
            ...
            rv = l3__egress_only_fec__create_inner(unit, fec_id, 0, 0, ((idx == 0) ? mc_gport_drop : mc_gport_2_cpu), 0, fec_flags2, 0, 0, &fec_id);
            if (rv != BCM_E_NONE)
            {
            ...
            rv = add_ipv4_ipmc(unit, mc_ip, 0xffffffff, src_ip, 0, vlan_l, vrf, 0, fec_id, 0);
            if (rv != BCM_E_NONE)
            {
    ...
    int recycle_port_core_1)
{
    ...
    rv = intf_eth_rif_create(unit, vid_out, cint_ipmc_info.intf_out_mac_address);
    if (rv != BCM_E_NONE)
    {
        printf("Error, intf_eth_rif_create intf_in\n");
        return rv;
