    ...
    int out_port2)
{
     ...
     */
    rv = add_ipv4_ipmc(unit, cint_ipmc_info.mc_dip, all_valid, cint_ipmc_info.sip, all_valid, cint_ipmc_info.intf_in,
    ...
    {
        rv = add_ipv4_ipmc(unit, cint_ipmc_info.mc_dip, all_valid, cint_ipmc_info.sip, 0xffffff00,
                           cint_ipmc_info.intf_in, cint_ipmc_info.vrf, mc_group[1], 0, BCM_IPMC_TCAM);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = add_ipv4_ipmc(unit, cint_ipmc_info.mc_dip, all_valid, cint_ipmc_info.sip, 0xffff0000, 0,
    ...
    {
        rv = add_ipv4_ipmc(unit, cint_ipmc_info.mc_dip, all_valid, cint_ipmc_info.sip, none_valid,
    ...
    rv = add_ipv4_ipmc(unit, cint_ipmc_info.mc_dip, 0xffffff00, cint_ipmc_info.sip, none_valid, 0,
    ...
    rv = l3__egress_only_fec__create(unit, fec_id, cint_ipmc_info.intf_in, 0, mc_gport, 0, fec_flags2);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object FEC inner, fecs[0]\n");
        return rv;
    }
    rv = add_ipv4_ipmc(unit, cint_ipmc_info.mc_dip, none_valid, cint_ipmc_info.sip, none_valid, 0,
    ...
    int out_port2)
{
    ...
    rv = l3__egress_only_fec__create(unit, fec_id, 0, 0, mc_gport, 0, fec_flags2);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object FEC inner, fecs[0]\n");
        return rv;
    ...
    if (*(dnxc_data_get(unit, "l3", "fwd", "ipmc_config_cmprs_vrf_group", NULL)))
    {
        ...
        rv = add_ipv6_ipmc(unit, dip_addr, all_valid, sip_addr, all_valid, 0, cint_ipmc_info.vrf,
                        cint_ipmc_info.compressed_sip, 0xFFFF, mc_group[0], 0, 0, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv6_ipmc 1 to TCAM + LPM table \n");
            return rv;
        }
        dip_addr[15]++;
        ...
        rv = add_ipv6_ipmc(unit, dip_addr, all_valid, sip_addr, all_valid, 0, cint_ipmc_info.vrf,
                        cint_ipmc_info.compressed_sip, 0xFFFF, mc_group[1], 0, 0, BCM_IPMC_TCAM);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv6_ipmc 2 to TCAM + LPM table \n");
            return rv;
        }
        ...
        rv = add_ipv6_ipmc(unit, dip_addr, all_valid, sip_addr, none_valid, 0, cint_ipmc_info.vrf,
                        cint_ipmc_info.compressed_sip, 0xFF00, mc_group[4], 0, 0, BCM_IPMC_TCAM);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv6_ipmc 5 to TCAM + LPM table \n");
            return rv;
        }
        cint_ipmc_info.compressed_sip += 0x100;

        /** TCAM C_VRFG (VRF + partial DIP) + LPM fully masked SIP */
        rv = add_ipv6_ipmc(unit, dip_addr, part_mask, sip_addr, none_valid, 0, cint_ipmc_info.vrf,
                        cint_ipmc_info.compressed_sip, 0xFF00, mc_group[5], 0, 0, BCM_IPMC_TCAM);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv6_ipmc 6 to TCAM + LPM table \n");
            return rv;
        }
        cint_ipmc_info.compressed_sip += 0x100;

        /** TCAM C_VRF (VRF + masked DIP) + LPM fully masked SIP */
        rv = add_ipv6_ipmc(unit, dip_addr, none_valid, sip_addr, none_valid, 0, cint_ipmc_info.vrf,
                        cint_ipmc_info.compressed_sip, 0xFF00, mc_group[6], 0, 0, BCM_IPMC_TCAM);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv6_ipmc 7 to TCAM + LPM table \n");
            return rv;
        }
    }
    else
    {
        rv = add_ipv6_ipmc(unit, dip_addr, all_valid, sip_addr, all_valid, cint_ipmc_info.intf_in, cint_ipmc_info.vrf,
                        cint_ipmc_info.compressed_sip, 0xFFFF, mc_group[0], 0, flags, 0);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = add_ipv6_ipmc(unit, dip_addr, all_valid, none_valid, none_valid, cint_ipmc_info.intf_in,
                        cint_ipmc_info.vrf, 0, 0, mc_group[4], 0, 0, 0);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = add_ipv6_ipmc(unit, dip_addr, part_mask, none_valid, none_valid, 0, cint_ipmc_info.vrf, 0, 0, mc_group[5], 0, 0, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv6_ipmc to LPM table (prefix on G), \n");
            return rv;
        }
        rv = add_ipv6_ipmc(unit, dip_addr, none_valid, none_valid, none_valid, 0, cint_ipmc_info.vrf, 0, 0, 0, fec_id, 0, 0);
        if (rv != BCM_E_NONE)
        {
