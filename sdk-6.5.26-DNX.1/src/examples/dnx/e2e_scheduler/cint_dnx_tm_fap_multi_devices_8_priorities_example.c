...
cosq_init_ing_egr_alloc(int unit) {
    ...
    for (idx = 0; idx < g_info.num_ports + g_info.num_internal_ports; idx++) {
        ...
        for (cos = 0; cos < g_info.num_cos; cos++)
        {
            ...
            if (BCM_FAILURE(rv)) {
                printf("bcm_cosq_scheduler_gport_add replace hr scheduler idx:%d failed. Error:%d (%s)\n", idx, rv, bcm_errmsg(rv));
                return rv;
            }
            ...
            params.core = mapping_info.core;
            params.shared_order = bcmCosqSchedulerGportSharedOrderNone;
            rv = bcm_cosq_scheduler_gport_add(unit, 0, params, &g_info.gport_ucast_scheduler[idx*MAX_COS+cos]);
            if (BCM_FAILURE(rv)) {
                printf("bcm_cosq_scheduler_gport_add fq scheduler idx:%d cos: %d failed. Error:%d (%s)\n", idx, cos, rv, bcm_errmsg(rv));
                return rv;
            }
    ...
    for (modid_idx = 0; modid_idx < g_info.nof_devices; modid_idx++) {
        for (idx = 0; idx < g_info.num_ports + g_info.num_internal_ports; idx++) {
            ...
            rv = bcm_cosq_ingress_queue_bundle_gport_add(unit, &conf, &g_info.gport_ucast_queue_group[modid_idx*MAX_NUM_PORTS+idx]);
            if (rv != BCM_E_NONE) {
