...
int tm_config2_setup(int port_id, int port_type, int res_type)
{
    ...
    rv = bcm_cosq_scheduler_gport_add(c2_egr_unit, default_flags, params, &c2_gport_ucast_scheduler[idx]);
    ...
    params.core = mapping_info.core;
    params.shared_order = bcmCosqSchedulerGportSharedOrderNone;
    rv = bcm_cosq_scheduler_gport_add(c2_egr_unit, default_flags, params, &c2_gport_ucast_scheduler_fq0[idx]);
    ...
    params.core = mapping_info.core;
    params.shared_order = bcmCosqSchedulerGportSharedOrderNone;
    rv = bcm_cosq_scheduler_gport_add(c2_egr_unit, default_flags, params, &c2_gport_ucast_scheduler_fq1[idx]);
    ...
    params.core = mapping_info.core;
    params.shared_order = bcmCosqSchedulerGportSharedOrderNone;
    rv = bcm_cosq_scheduler_gport_add(c2_egr_unit, default_flags, params, &c2_gport_ucast_scheduler_fq2[idx]);
    ...
    params.core = mapping_info.core;
    params.shared_order = bcmCosqSchedulerGportSharedOrderNone;
    rv = bcm_cosq_scheduler_gport_add(c2_egr_unit, default_flags, params, &c2_gport_ucast_scheduler_fq3[idx]);
    ...
    rv = bcm_port_resource_get(c2_ingr_unit, port, &resource);
    if (rv != BCM_E_NONE) {
        printf("bcm_port_resource_get failed $rv\n");
        return(rv);
    }
    rv = appl_dnx_e2e_voq_profiles_get(c2_ingr_unit, resource.speed, &delay_tolerance_level, &rate_class);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_cosq_ingress_queue_bundle_gport_add(c2_ingr_unit, &conf, &c2_gport_ucast_queue_group0[idx]);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_port_resource_get(c2_egr_unit, port, &resource);
    if (rv != BCM_E_NONE) {
        printf("bcm_port_resource_get failed $rv\n");
        return(rv);
    }
    rv = appl_dnx_e2e_voq_profiles_get(c2_egr_unit, resource.speed, &delay_tolerance_level, &rate_class);
    if (rv != BCM_E_NONE) {
        printf("appl_dnx_e2e_voq_profiles_get failed $rv\n");
        return(rv);
    }
    ...
    rv = bcm_cosq_ingress_queue_bundle_gport_add(c2_egr_unit, &conf, &c2_gport_ucast_queue_group1[idx]);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_ingress_queue_bundle_gport_add failed\n");
        return rv;
    }
...
tm_config2_verify(int port_id, int port_type, int res_type)
{
    ...
    rc = bcm_cosq_scheduler_gport_get(c2_egr_unit, 0, c2_e2e_gport[idx], &params_get);
    if (rc != BCM_E_NONE) {
    ...
    rc = bcm_cosq_scheduler_gport_get(c2_egr_unit, 0, c2_gport_ucast_scheduler[idx], &params_get);
    if (rc != BCM_E_NONE) {
    ...
    for (nbr_fq_sched = 0; nbr_fq_sched < C2_NBR_FQ_SCHEDULERS; nbr_fq_sched++) {
        ...
        /* retreive type of scheduling element */
        rc = bcm_cosq_scheduler_gport_get(c2_egr_unit, 0, gport_scheduler_fq, &params_get);
        if (rc != BCM_E_NONE) {
    ...
    for (nbr_queue_groups = 0; nbr_queue_groups < C2_NBR_QUEUE_GROUPS; nbr_queue_groups++) {
        ...
        rc = bcm_cosq_voq_connector_gport_get(c2_egr_unit, gport_ucast_voq_connector_group, &config_get);
        if (rc != BCM_E_NONE) {
