...
int e2e_composite_connectors_setup(int unit, int kbits_sec_max_port, int port_idx)
{
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &c5_gport_ucast_scheduler[idx]);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &c5_gport_ucast_scheduler_fq0[idx]);
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &c5_gport_ucast_scheduler_fq1[idx]);
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &c5_gport_ucast_scheduler_fq2[idx]);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_add scheduler failed $rv\n");
        return rv;
    }
    ...
    rv = bcm_cosq_scheduler_gport_add(unit, default_flags, params, &c5_gport_ucast_scheduler_fq3[idx]);
    ...
    rv = bcm_cosq_ingress_queue_bundle_gport_add(unit, &conf, &c5_gport_ucast_queue_group0[idx]);
    if (rv != BCM_E_NONE) {
...
e2e_scheduling_elements_verify_type(int unit, bcm_gport_t gport, int exp_type)
{
    ...
    rc = bcm_cosq_scheduler_gport_get(unit, 0, gport, &params_get);
    if (rc != BCM_E_NONE) {
...
e2e_composite_connectors_verify_flags(int unit, bcm_gport_t gport, int exp_flags)
{
    ...
    rc = bcm_cosq_voq_connector_gport_get(unit, gport, &config_get);
    if (rc != BCM_E_NONE) {
        printf("bcm_cosq_voq_connector_gport_get on Sched (0x%x) failed, error (%d)\n", gport, rc);
        return(rc);
    }
...
int e2e_composite_connectors_verify(int unit, int kbits_sec_max_port, int port_idx)
{
    ...
    rc = bcm_cosq_scheduler_gport_get(unit, 0, c5_e2e_gport[idx], &params_get);
    if (rc != BCM_E_NONE) {
        printf("bcm_cosq_scheduler_gport_get on e2e Sched (0x%x) failed, error (%d)\n", c5_e2e_gport[idx], rc);
        return(rc);
    }
    ...
    /* retreive type of scheduling element */
    rc = e2e_scheduling_elements_verify_type(unit, c5_gport_ucast_scheduler[idx], C5_SCHED_CL_TYPE);
    if (rc != BCM_E_NONE) {
        return(rc);
    }
    ...
    for (nbr_fq_sched = 0; nbr_fq_sched < C5_NBR_FQ_SCHEDULERS; nbr_fq_sched++) {
        ...
        /* retreive type of scheduling element */
        rc = e2e_scheduling_elements_verify_type(unit, gport_scheduler_fq, C5_SCHED_FQ_TYPE);
        if (rc != BCM_E_NONE) {
            return(rc);
        }
