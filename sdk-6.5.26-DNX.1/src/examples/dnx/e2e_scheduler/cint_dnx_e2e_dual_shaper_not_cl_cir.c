 ...
 *
 * File: cint_dnx_e2e_dual_shaper_not_cl_cir.c
...
int e2e_dual_shaper_setup(int port_idx)
{
    ...
    printf("cint_dnx_e2e_dual_shaper_not_cl_cir.c completed with status (%s)\n", bcm_errmsg(rv));
...
int e2e_dual_shaper_teardown(int port_idx)
{
    ...
    printf("cint_dnx_e2e_dual_shaper_not_cl_cir.c (teardown) completed with status (%s)\n", bcm_errmsg(rc));
...
int e2e_dual_shaper_verify(int port_idx)
{
    ...
    if (params_get.type != C6_DEFAULT_E2E_SCHED_TYPE) {
        printf("gport get mismatch on E2E Sched (0x%x) expected type(0%d) retrieved type (%d)\n", c6_e2e_gport[idx], C6_DEFAULT_E2E_SCHED_TYPE, params_get.type);
    }
    ...
    printf("cint_dnx_e2e_dual_shaper_not_cl_cir.c (verify) completed with status (%s)\n", bcm_errmsg(rc));
