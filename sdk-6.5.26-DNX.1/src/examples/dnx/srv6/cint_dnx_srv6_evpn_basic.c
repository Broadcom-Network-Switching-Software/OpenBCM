...
/*
 * Configurations for ESI encapsulation in SRv6 EVPN.
 * Unlike MPLS EVPN in which the ESI label is retrieved from ESEM, currently the ESI DB is
 * the same one as SRv6 IPv6 tunnel, but always with VPN SID whose 16 LSB is the ESI label.
 * The ePMF is responsible for replacing the right ipv6-tunnel-outlif with the one with ESI label.
 * Generaly, the user can configure the ePMF per their applications.
 * In this example, the recommended configurations can be found in cint_field_srv6_evpn_esi_encap_egress_fg_add.
 * The function is mainly for ESI encapsulation. The traffic service should be based on additional
 * SRv6 encapsulation configurations(single-pass, multi-pass, etc.), such as:
 * srv6_ingress_l2vpn_mc_ext_encap_1st_and_2nd_pass(...).
 *
 * same_es_p_port - PE port for the traffic to the same ES as the original port.
    ...
    int is_virtual_esi)
{
    ...
    /** Add the L2VPN tunnel with ESI label*/
    ...
    sal_memcpy(tunnel_init_set.dip6, cint_srv6_evpn_info.ip6_dip_with_esi[is_virtual_esi], 16);
    tunnel_init_set.flags = 0;
    ...
    {
        /** This is the 1st pass, estimation is:40(IPv6 header)*/
    ...
    printf("The IPv6 tunnel ID with ESI is 0x%08X \n", tunnel_init_set.tunnel_id);



    /** Add the ESI encapsulation ePMF configurations.*/
    rv = cint_field_srv6_evpn_esi_encap_egress_fg_add(unit, is_virtual_esi, same_es_p_port);
    if (rv != BCM_E_NONE)
    {
       printf("Error, cint_field_srv6_evpn_esi_encap_egress_fg_add \n");
       return rv;
    }

    /** Add the ESI encapsulation ePMF entries.*/
    rv = cint_field_srv6_evpn_esi_encap_egress_entry_add(unit, is_virtual_esi, src_vp, dst_vp, tunnel_init_set.tunnel_id);
    if (rv != BCM_E_NONE)
    {
       printf("Error, cint_field_srv6_evpn_esi_encap_egress_entry_add \n");
       return rv;
    }
