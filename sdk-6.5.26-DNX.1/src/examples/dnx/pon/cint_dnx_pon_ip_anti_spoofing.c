...
bcm_ip_t sav_sip_mask = 0xffffffff;
bcm_ip6_t sav_sip6_mask = { 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF };
...
/*
 * Set SIP mask for PON ip anti spoofing service.
 * enable: 1  disable: 0
 */

int pon_service_ip_anti_spoofing_set_mask(int unit, int is_mask_enable)
{
    if (is_mask_enable) {
        sav_sip_mask = 0xfffffff0;
        sav_sip6_mask[15] = 0xF0;
    } else {
        sav_sip_mask = 0xffffffff;
        sav_sip6_mask[15] = 0xFF;
    }
...
int pon_service_ip_anti_spoofing_set(int unit,int start_index,int end_index,uint32 add,int is_dhcp,int is_n1)
{
    ...
    for (index = start_index; index <= end_index; index ++)
    {
        ...
        info.ip_mask = sav_sip_mask;

        sal_memcpy(info.ip6, ip6_add, 16);
        sal_memcpy(info.ip6_mask, sav_sip6_mask, 16);
        if(is_dhcp)
        {
