 ...
 * cint cint dnx/utility/cint_dnx_utils_global.c
 * cint dnx/utility/cint_dnx_utils_multicast.c
 * cint dnx/utility/cint_dnx_utils_vlan.c
 * cint dnx/utility/cint_dnx_utils_l3.c
 * cint dnx/l3/cint_dnx_ip_route_basic.c
 * cint dnx/crps/cint_crps_oam_config.c
 * cint dnx/instru/cint_field_IFA_datapath.c
...
int twamp_reflector_example(int unit, int is_ipv6, int is_stateless, int in_port, int out_port, int rcy_port) {
    ...
    if (is_stateless) {
        rv = bcm_switch_control_set(unit, bcmSwitchTwampStatelessModeEnable, 1);
        if (rv != BCM_E_NONE) {
            printf("bcm_switch_control_set $rv");
        }
    }
    ...
    /*** create PMF rule ***/
    rv = cint_field_twamp_reflector_rule(unit, is_ipv6, 0, is_stateless, local_ip_addr, remote_ip_addr, &local_ip6_addr, &remote_ip6_addr, 0, 0, gport_trap, 0, twamp_info.gport_fec[1], 0);
    if (rv != BCM_E_NONE) {
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_port_get $rv");
        return rv;
    }
    if (interface_info.interface == BCM_PORT_IF_NIF_ETH) {
        rv = twamp_rx_timestamp_enable(unit, in_port);
        if (rv != BCM_E_NONE) {
            printf("twamp_rx_timestamp_enable $rv");
        }
    }
...
int twamp_tx_rx_example(int unit, int is_ipv6, int in_port, int out_port, int trap_port, int ctf_id) {
    ...
    /*** create PMF rule ***/
    rv = cint_field_twamp_tx_rx_rule(unit, is_ipv6, gport_trap, ctf_id, 
...
int twamp_reflector_lag_example(int unit, int is_ipv6, int in_port, int out_port, int rcy_port) {
    ...
    /*** create PMF rule ***/
    rv = cint_field_twamp_reflector_rule(unit, is_ipv6, 1, 0, local_ip_addr, remote_ip_addr, &local_ip6_addr, &remote_ip6_addr,
