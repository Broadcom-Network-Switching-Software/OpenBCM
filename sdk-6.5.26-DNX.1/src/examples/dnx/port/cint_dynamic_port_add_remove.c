    ...
    if (!port_is_rcy_dynamic)
    {
        ...
        if (is_last_in_if)
        {
            ...
            if (*(dnxc_data_get(unit, "nif", "global", "not_implemented", NULL)) == 0)
            {
                if (port_is_nif_dynamic)
                {
                    ...
                    if (loopback == BCM_PORT_LOOPBACK_PHY)
                    {
                        ...
                        if (rv != BCM_E_NONE)
                        {
                            printf("Error in dnx_port_phy_loopback_clear: unit=%d, port=%d\n", unit, port);
                            return rv;
                        }
                    }
                }
            ...
            if (*(dnxc_data_get(unit, "nif", "global", "not_implemented", NULL)) == 0)
            {
                ...
                if (loopback == BCM_PORT_LOOPBACK_PHY)
                {
                    rv = dnx_port_phy_loopback_db_clear(unit, port);
                    if (rv != BCM_E_NONE)
                    {
                        printf("Error, dnx_port_phy_loopback_db_clear: unit=%d, port=%d\n", unit, port);
                        return rv;
                    }
                }
            }
    ...
    for (port_index = 0; port_index < nof_ports; ++port_index)
    {
        ...
        if (port_is_scheduler_required)
        {
            if (port_is_local)
            {
                ...
                if (!(flags[port_index] & BCM_PORT_ADD_FLEXE_SAR))
                {
                    ...
                    rv = cint_dyn_port_add_packet_processing_properties_flexible_set(unit, port_list[port_index], header_type_in[port_index],
                                                                                              header_type_out[port_index], g_config->pp_set_header_only);
                    if (rv != BCM_E_NONE)
                    {
                        printf("Error, bcm_dnx_port_enable_set: unit=%d, port=%d\n", unit, port_list[port_index]);
                        return rv;
...

            {
                ...
                rv = appl_dnx_dynamic_port_create(unit, appl_flags, &sys_port_map, flags[port_index], port_list[port_index]);
                if (rv != BCM_E_NONE)
                {
                    printf("Error, bcm_dnx_port_enable_set: unit=%d, port=%d\n", unit, port_list[port_index]);
                    return rv;
