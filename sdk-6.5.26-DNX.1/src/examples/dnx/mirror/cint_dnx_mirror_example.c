 ...
 * cint dnx/utility/cint_dnx_utils_tpid.c
 * cint dnx/utility/cint_dnx_utils_vlan_translate.c
 * cint dnx/field/cint_field_utils.c
 * cint dnx/l3/cint_dnx_ip_route_basic.c
 * cint dnx/dbal/cint_dbal.c
 * cint dnx/mirror/cint_dnx_mirror_example.c
...
int mirror_with_span_lag_lb_example(int unit, int in_port, int in_vlan, int dest_port, int trunk_id) {
     ...
     int out_port)
 {
     ...
     get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &kaps_result);
...
int mirror_erspan_example(int unit, int in_port, bcm_vlan_t in_vlan, int out_port, int mirror_port){
    ...
    if (dnx_utils_is_jr2_system_header_mode(unit)) {
        rv = mirror_erspan_update_mirror_qualifier(unit, erspan_info.session_id);
        if (rv != BCM_E_NONE) {
            printf("Error in mirror_erspan_update_mirror_qualifier %d\n", rv);
            return rv;
        }
    }
    ...
    int mirror_qualifier)
{
    ...
    rv = bcm_field_action_create(unit, 0, &action_info, &erspan_info.action_id);
    if (rv != BCM_E_NONE)
    {
        printf("bcm_field_action_create failed: %d\n", rv);
        return rv;
    }
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &erspan_info.fg_id);
    if (rv != BCM_E_NONE)
    {
        printf("bcm_field_group_add failed: %d\n", rv);
        return rv;
    }
    ...
    rv = bcm_field_fem_action_add(unit, 0, erspan_info.fg_id, action_priority, &fem_action_info);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_fem_action_add for fg_id = %d | array_id = %d | fem_position_in_array = %d\n", rv,
    ...
    if ((G_mirror_type == BCM_MIRROR_PORT_EGRESS) &&
        *(dnxc_data_get(unit, "port_pp", "general", "rcy_info_prt_qualifier_eth_over_append64", NULL))!= 0)
    {
    ...
    rv = bcm_field_group_context_attach(unit, 0, erspan_info.fg_id, erspan_info.context_id, &attach_info);
    if (rv != BCM_E_NONE)
    {
        printf("bcm_field_group_context_attach failed: %d\n", rv);
        return rv;
    }
...
int mirror_erspan_example_clean_up(int unit){
    ...
    if (dnx_utils_is_jr2_system_header_mode(unit)) {
        rv = bcm_field_group_context_detach(unit, erspan_info.fg_id, erspan_info.context_id);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = bcm_field_fem_action_delete(unit, erspan_info.fg_id, erspan_info.action_priority);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = bcm_field_group_delete(unit, erspan_info.fg_id);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = bcm_field_action_destroy(unit, erspan_info.action_id);
        if (rv != BCM_E_NONE)
        {
...
int mirror_vxlan_example(int unit, int in_port, int out_port, int mirror_port, int access_port, int provider_port) {
    ...
    rv = vxlan_example(unit, 0, access_port, provider_port, 0);
    if (rv != BCM_E_NONE)
    {
