...
struct cint_srv6_tunnel_info_s
{
    ...
    bcm_ip6_t tunnel_ip6_dip_vpn_common_prefix;         /* IPv6 of Tunnel to Hold the Common Prefix 56b for VPN SID */
...
cint_srv6_tunnel_info_s cint_srv6_tunnel_info =
{
        ...
        {{ 0x11, 0x11, 0x22, 0x22, 0x33, 0x33, 0x44, 0x44, 0x55, 0x55, 0x66, 0x66, 0x77, 0x77, 0x88, 0x88 },
         ...
         { 0xaa, 0xaa, 0xff, 0xff, 0xee, 0xee, 0xbb, 0x55, 0x44, 0x44, 0x33, 0x33, 0x22, 0x22, 0x11, 0x11 },
         { 0xaa, 0xaa, 0xff, 0xff, 0xee, 0xee, 0xbb, 0x55, 0x44, 0x44, 0x33, 0x33, 0x22, 0x22, 0x11, 0x10 }},


         /*
          *tunnel_ip6_dip_vpn_common_prefix
          */
         { 0xAA, 0xAA, 0xFF, 0xFF, 0xEE, 0xEE, 0xBB, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
                            ...
                            uint8 lif_cs_profile)     /** if not 0, terminated LIFs are set with this CS Profile */
{
    ...
    rv = l3__egress_only_fec__create(unit, cint_srv6_tunnel_info.tunnel_fec_id[END_POINT_2_FEC_ID], cint_srv6_tunnel_info.eth_rif_intf_out[END_POINT_2], cint_srv6_tunnel_info.tunnel_arp_id[END_POINT_2], gport, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf
     ...
     */
    {
        ...
        tunnel_init_set.flags = BCM_TUNNEL_WITH_ID | BCM_TUNNEL_INIT_SRV6_SCALE_VPN_SID;
        sal_memcpy(tunnel_init_set.dip6, cint_srv6_tunnel_info.tunnel_ip6_dip_vpn_common_prefix, 16);
     ...
     */
    {
        ...
        if (next_header_is_eth)
        {
            srv6_tunnel_termination(unit, SIDPASS, 1, cint_srv6_tunnel_info.vrf_in[EGRESS], cint_srv6_tunnel_info.l2_termination_vsi, lif_cs_profile);
            srv6_tunnel_termination(unit, SIDDROP, 1, cint_srv6_tunnel_info.vrf_in[EGRESS], cint_srv6_tunnel_info.l2_termination_vsi, lif_cs_profile);
        }
        else
        {
            srv6_tunnel_termination(unit, SIDPASS, 1, cint_srv6_tunnel_info.vrf_in[EGRESS], 0, lif_cs_profile);
            srv6_tunnel_termination(unit, SIDDROP, 1, cint_srv6_tunnel_info.vrf_in[EGRESS], 0, lif_cs_profile);
        }
