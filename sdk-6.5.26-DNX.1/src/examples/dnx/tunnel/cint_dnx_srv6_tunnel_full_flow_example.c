                    ...
                    uint8 lif_cs_profile) /** if not 0, LIF CS Profile is set with this value */
{
    ...
    if ((is_egress_tunnel) && (cint_srv6_tunnel_info.reclassification_tunnel_wide_data))
    {
        ...
        rv = bcm_port_wide_data_set(unit, tunnel_term_set.tunnel_id, BCM_PORT_WIDE_DATA_INGRESS, wide_data);
        if (rv != BCM_E_NONE)
        {
...
int trap_config(int unit)
{
    ...
    /* set trap */
    rv = bcm_rx_trap_set(unit, ingress_trap_id_ttl0, trap_config);
    if (rv != BCM_E_NONE) {
        printf("Error. bcm_rx_trap_set failed(%s) \n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_rx_trap_set(unit, ingress_trap_id_ttl1, trap_config);
    if (rv != BCM_E_NONE) {
        printf("Error. bcm_rx_trap_set failed(%s) \n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_rx_trap_set(unit, ingress_trap_id_ipv6_ttl0, trap_config);
    if (rv != BCM_E_NONE) {
        printf("Error. bcm_rx_trap_set failed(%s) \n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_rx_trap_set(unit, ingress_trap_id_ipv6_ttl1, trap_config);
    if (rv != BCM_E_NONE) {
        printf("Error. bcm_rx_trap_set failed(%s) \n", bcm_errmsg(rv));
        return rv;
    }
                            ...
                            uint8 lif_cs_profile)     /** if not 0, terminated LIFs are set with this CS Profile */
{
    ...
    else
    {
        ...
        BCM_L3_ITF_SET(srv6_basic_lif_encode, BCM_L3_ITF_TYPE_LIF, cint_srv6_tunnel_info.tunnel_global_lif[INGRESS_SRH_GLOBAL_LIF]);
        BCM_GPORT_LOCAL_SET(gport, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_FIRST_SID_FEC_ID]);

        rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID_2ND_HIER], srv6_basic_lif_encode, 0, gport, BCM_L3_2ND_HIERARCHY, fec_flags2,
                                               0, 0,&encoded_fec);
        ...
        rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID], 0, cint_srv6_tunnel_info.tunnel_arp_id[INGRESS], gport, 0, fec_flags2, 0, 0, NULL);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = l3__egress_only_fec__create(unit, cint_srv6_tunnel_info.tunnel_fec_id[END_POINT_2_FEC_ID], cint_srv6_tunnel_info.eth_rif_intf_out[END_POINT_2], cint_srv6_tunnel_info.tunnel_arp_id[END_POINT_2], gport, 0, fec_flags2);
    if (rv != BCM_E_NONE)
    {
        printf
    ...
    if(*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
    {
        ...
        if (cross_connect)
        {
           rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[EGRESS_FEC_ID], cint_srv6_tunnel_info.eth_rif_intf_out[EGRESS] , cint_srv6_tunnel_info.tunnel_arp_id[EGRESS], gport, 0, 0, 0, 0,&encoded_fec);
        }
        else
        {
    ...
    else
    {
       ...
       rv = bcm_l2_addr_add(unit, &l2_addr_ing);
        ...
        rv = add_route_ipv6(unit, cint_srv6_tunnel_info.tunnel_ip6_dip[SID1], ip6_mask, cint_srv6_tunnel_info.vrf_in[END_POINT_1], cint_srv6_tunnel_info.tunnel_fec_id[END_POINT_1_FEC_ID]);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, create route faild\n",proc_name);
            return rv;
        }

        rv = add_route_ipv6(unit, cint_srv6_tunnel_info.tunnel_ip6_dip[SID0], ip6_mask, cint_srv6_tunnel_info.vrf_in[END_POINT_2], cint_srv6_tunnel_info.tunnel_fec_id[END_POINT_2_FEC_ID]);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, create route faild\n",proc_name);
            return rv;
     ...
     */
    {
        ...
        for (sid_idx = 0; sid_idx < nof_2pass_recycle_entries; sid_idx++)
        {
            ...
            rv = srv6_create_extended_termination_recycle_entry(unit, sid_idx, &recycle_entry_encap_id[sid_idx]);
            if (rv != BCM_E_NONE)
            {
            ...
            BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(extension_terminator_info.encap_id , recycle_entry_encap_id[sid_idx]);
     ...
     */
    {
        ...
        srv6_tunnel_termination(unit, SID2, 0, cint_srv6_tunnel_info.vrf_in[END_POINT_1], 0, lif_cs_profile);
        srv6_tunnel_termination(unit, SID1, 0, cint_srv6_tunnel_info.vrf_in[END_POINT_2], 0, lif_cs_profile);
        ...
        if (next_header_is_eth)
        {
            srv6_tunnel_termination(unit, SID0, 1, cint_srv6_tunnel_info.vrf_in[EGRESS], cint_srv6_tunnel_info.l2_termination_vsi, lif_cs_profile);
        }
        else
        {
            srv6_tunnel_termination(unit, SID0, 1, cint_srv6_tunnel_info.vrf_in[EGRESS], 0, lif_cs_profile);
        }
