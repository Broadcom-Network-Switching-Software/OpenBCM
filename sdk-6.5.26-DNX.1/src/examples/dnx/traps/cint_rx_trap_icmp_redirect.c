...
int fwd_strength)
{
    ...
    {
        rv = bcm_rx_trap_type_get(unit, 0, bcmRxTrapIcmpRedirect, &cint_rx_trap_icmp_redirect_trap_id);
    }
    rv = bcm_rx_trap_set(unit, cint_rx_trap_icmp_redirect_trap_id, config);
    if (rv != BCM_E_NONE) 
    {
