    ...
    int mtu_profile)
{
    ...
    rv = cint_etpp_mtu_trap_mtu_lif_set(unit, lif_gport, trap_gport, mtu_val, layer_type, compressed_layer_type, mtu_profile);
    if(rv != BCM_E_NONE)
    {
        printf("Error in cint_etpp_mtu_trap_mtu_lif_set \n");
        return rv;
    }
    ...
    int mtu_profile)
{
    ...
    int rv = BCM_E_NONE, mtu_layer_to_compressed_layer_mapping_support = 0;
    ...
    int mtu_profile)
{
    ...
    {
        /* delete trap */
        rv = bcm_rx_trap_type_destroy(unit, cint_etpp_mtu_trap_lif_trap_id);
        if (rv != BCM_E_NONE)
        {
