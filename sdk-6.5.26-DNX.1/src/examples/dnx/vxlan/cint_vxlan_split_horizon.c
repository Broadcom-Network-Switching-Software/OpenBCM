    ...
    int virtual_machine_port)
{
     ...
     */
    rv = multicast__open_mc_group(unit, cint_vxlan_global_info.vpn_id, 0);
    if (rv != BCM_E_NONE)
    {
     ...
     */
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_vxlan_global_info.fec_id);
    if (rv != BCM_E_NONE)
    {
