    ...
    bcm_multicast_t mc_group)
{
    ...
    rv = bcm_multicast_add(unit, mc_group, multicast_add_flags, nof_replications, replications);
    if (rv != BCM_E_NONE)
    {
    ...
    int * recycle_entry_encap_id)
{
    ...
    {
        int cs_profile_id = 5;
        rv = cint_field_epmf_cs_outlif_profile_disable_egress_filters_main(unit, *recycle_entry_encap_id, cs_profile_id);
        if (rv != BCM_E_NONE)
        {
    ...
    int recycle_port)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, vxlan_bud_node_create_recycle_entry_with_control_lif \n", proc_name);
        return rv;
    }
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_vxlan_tunnel_bud_node_info.fec_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = l3__egress_only_fec__create(unit, cint_vxlan_tunnel_bud_node_info.fec_id, encap_tunnel_intf, 0, virtual_machine_mc_port, 0, fwd_flag);
    if (rv != BCM_E_NONE)
    {
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, l2_unknown_multicast_da_create\n", proc_name);
        return rv;
    }
   return rv;
}
