...
/* is_wide_data_16b used to select 16bits or 8bits wide data for AC MP:
 *      is_wide_data_16b=1: MP AC contains 16bits wide data + protection not include flush group + VSI(15bits)
 *      is_wide_data_16b=0: MP AC contains 8bits wide data + protection include flush group + VSI(17bit)
 * 
 * selection rule: {group + VSI}
 *      (VSI > 0x7fff)  + (group==0/!0)      : select 8bits wide-data
 *      (VSI <= 0x7fff) + (group==0)         : select 16bits wide-data
 *      (VSI <= 0x7fff) + (group!=0)         : select 8bits wide-data
 */
int is_wide_data_16b=0;
    ...
    int vsi)
{
    ...
    /* 
     * Setting the group isn't required by the user. 
     * It's done only for the purpose of testing, in order ensure that the selected In-LIF format has 8bits wide data.
     * On Q2A, VSI alway < 0x8000, so the group is needed to select between 8/16 bits
     */
    if (!is_wide_data_16b)
    {
        vlan_port.group = 1;
    ...
    bcm_field_context_t ipmf1_context_id)
{
    ...
    if (first_run)
    {
        rc = bcm_field_group_add(unit, 0, &fg_info, &g_inlif_wide_data.acl_ipmf1_fg_id);
        if (rc != BCM_E_NONE)
        {
        ...
        rc = bcm_field_group_context_attach(unit, 0, g_inlif_wide_data.acl_ipmf1_fg_id, ipmf1_context_id, &attach_info);
        if (rc != BCM_E_NONE)
        {
...
{       
    ...
    rc = inlif_wide_data_acl_run(unit, 1, bcmFieldQualifyAcInLifWideData, 
                                 is_wide_data_16b?0xFFFF:0XFF, 0, COMPILER_64_LO(g_inlif_wide_data.wide_data), 0,
                                 ipmf1_context_id);
    if (rc != BCM_E_NONE)
    {
...
int inlif_wide_data_acl_destroy(int unit, int nof_entries, bcm_field_context_t ipmf1_context_id)
{
    ...
    rv = bcm_field_group_context_detach(unit, g_inlif_wide_data.acl_ipmf1_fg_id, ipmf1_context_id);
    if (rv != BCM_E_NONE)
    {
    ...
    bcm_field_context_t ipmf1_context_id)
{
    ...
    rc = bcm_switch_control_get(unit, bcmSwitchWideDataExtKeyMode, &mode);
    if (rc != BCM_E_NONE)
    {
        printf("Error, bcm_switch_control_get, type=bcmSwitchWideDataExtKeyMode\n");
        return rc;
    }
    ...
    rc = bcm_switch_wide_data_extension_add(unit, flags, &info);
    if (rc != BCM_E_NONE)
    {
...
int inlif_wide_data_general_pmf_configure(int unit, int vtt_stage, int dest_port, uint32 wide_data, bcm_field_context_t ipmf1_context_id)
{
    ...
    */
    ...
    rv = bcm_field_qualifier_info_get(unit, bcmFieldQualifyPweInLifWideData, bcmFieldStageIngressPMF1, &qual_info_get);
    if (rv != BCM_E_NONE)
    {
...
int inlif_wide_data_general_service(int unit, int port1, int port2, int redirect_port, int vtt_stage, bcm_field_context_t ipmf1_context_id)
{
    ...
    rv = inlif_wide_data_general_pmf_configure(unit, vtt_stage, redirect_port, wide_data, ipmf1_context_id);
    if (rv != BCM_E_NONE) {
...
int inlif_large_wide_data_pmf_clean(int unit, bcm_field_context_t ipmf1_context_id)
{
    ...
    rv = bcm_field_group_context_detach(unit, fg_id, ipmf1_context_id);
    if (rv != BCM_E_NONE) {
        printf("Error in inlif_wide_data_general_create_udf_qual\n");
        return rv;
    }
...
int inlif_large_wide_data_pmf_configure(int unit, int dest_port, uint32 data, uint32 mask, bcm_field_context_t ipmf1_context_id)
{
    ...
    if (is_pmf_config) {
        rv = inlif_large_wide_data_pmf_clean(unit, ipmf1_context_id);
        if (rv != BCM_E_NONE) {
    ...
    rv = bcm_field_group_context_attach(unit, 0, fg_id, ipmf1_context_id, &attach_info);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_context_attach\n", rv);
        return rv;
...
int inlif_large_wide_data_general_service(int unit, int in_port, int vsi, int redirect_port, bcm_field_context_t ipmf1_context_id)
{
    ...
    rv = inlif_large_wide_data_pmf_configure(unit, redirect_port, cint_inlif_wide_data_val, 0xfffffff, ipmf1_context_id);
    if (rv != BCM_E_NONE)
    {
...
int inlif_large_wide_data_l3_fwd_config(int unit, int vsi, int out_port)
{
    ...
    if (!*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error (%d), in bcm_l3_route_add\n", rv);
            return rv;
        }
    }
    else
...
int inlif_large_wide_data_l3_test(int unit, int vsi, int in_port, int out_port, int redirect_port, bcm_field_context_t ipmf1_context_id)
{
     ...
     * Set In AC wide data
     */
    rv = inlif_large_wide_data_general_service(unit, in_port, vsi, redirect_port, ipmf1_context_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in inlif_large_wide_data_general_service\n", rv);
        return rv;
