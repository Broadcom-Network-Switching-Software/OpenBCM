    ...
    int fec)
{
    ...
    rv = l3__egress_only_fec__create_inner(unit, fec, encap_id, 0, gport, 0,fec_flags2, 0, 0, &encoded_fec);
    if (rv != BCM_E_NONE)
    {
    ...
    int fec)
{
    ...
    {
         get_first_fec_in_range_which_not_in_ecmp_range(unit,0,1,&fec_rpf_loose);
    ...
    BCM_GPORT_LOCAL_SET(gport, out_port);
    rv = l3__egress_only_fec__create(unit, fec_rpf_loose, intf_in_loose, 0, gport, 0,fec_flags2_rpf);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = l3__egress_only_fec__create(unit, fec_rpf_strict, intf_in_strict, 0, gport, 0,fec_flags2_rpf);
    if (rv != BCM_E_NONE)
    {
     ...
     */
    printf("Create main FEC and set its properties.\r\n");
    rv = l3__egress_only_fec__create_inner(unit, fec, encap_id, 0, gport, 0,fec_flags2_fwd, 0, 0, &encoded_fec);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object FEC only\n");
        return rv;
    ...
    rv = add_route_ipv4_rpf(unit, sip_loose_kaps, mask, vrf, fec_rpf_loose);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_route_ipv4(), \n");
        return rv;
    }
    rv = add_route_ipv4_rpf(unit, sip_strict_kaps, mask, vrf, fec_rpf_strict);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_route_ipv4(), \n");
        return rv;
    ...
    rv = add_route_ipv6_rpf(unit, ipv6_sip_loose_kaps, ipv6_mask, vrf, fec_rpf_loose);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_route_ipv6: vrf = %d, fec = %d, err_id = %d \n", vrf, fec_rpf_loose, rv);
        return rv;
    }
    rv = add_route_ipv6_rpf(unit, ipv6_sip_strict_kaps, ipv6_mask, vrf, fec_rpf_strict);
    if (rv != BCM_E_NONE)
    {
    ...
    int out_port2)
{
    ...
    if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
    {
        rv = add_ipv6_ipmc(unit, ipv6_host, ipv6_mask, no_addr, no_addr, intf_in, vrf, 0, 0, mc_group, 0, 0, 0);
        if (rv != BCM_E_NONE)
        {
    ...
    {
        rv = add_ipv6_ipmc(unit, ipv6_route, ipv6_mask, no_addr, no_addr, 0, vrf, 0x123, 0xFFFF, mc_group, 0, 0, 0);
        if (rv != BCM_E_NONE)
        {
    ...
    {
        rv = add_ipv6_ipmc(unit, ipv6_route, ipv6_mask, no_addr, no_addr, 0, vrf, 0, 0, mc_group, 0, 0, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv6_ipmc to LPM table, \n");
            return rv;
    ...
    if (*dnxc_data_get(unit, "l3", "fwd", "tcam_entry_support", NULL))
    {
        rv = add_ipv6_ipmc(unit, ipv6_tcam, ipv6_mask, ipv6_sip, ipv6_mask, intf_in, vrf, 0, 0, mc_group, 0, BCM_IPMC_TCAM, 0);
        if (rv != BCM_E_NONE)
        {
