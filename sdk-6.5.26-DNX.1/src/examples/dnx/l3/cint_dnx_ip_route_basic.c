    ...
    int kaps_result)
{
    ...
    {
        rv = out_port_set(unit, cint_ip_route_info.failover_out_port);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, out_port_set failover_out_port %d\n",proc_name, cint_ip_route_info.failover_out_port);
            return rv;
        }
    }
    ...
    if (cint_ip_route_info.host_format != 2)
    {
        ...
        rv = l3__egress_only_fec__create_inner(unit, fec, (cint_ip_route_info.is_arp_rif_mode ? 0 : intf_out), encap_id,
                                               gport, 0, fec_flags2, cint_ip_route_info.failover_id, failover_is_primary, &encoded_fec);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, create egress object FEC only\n", proc_name);
            return rv;
    ...
    int arp_plus_ac_type)
{
    ...
    rv = l3__egress_only_fec__create(unit, fec, 0, encap_id_itf, gport, 0, fec_flags2);
    if (rv != BCM_E_NONE)
    {
        printf
            ("%s(): Error, create egress object FEC only: fec = %d, intf_out = %d, encap_id = %d, out_port = %d, err_id = %d\n",
    ...
    int redir_encap_id)
{
     ...
     */
    {
        ...
        }
        {
            ...
            rv = l3__egress_only_fec__create_inner(unit, redir_fec, intf_out, redir_encap_id, redir_gport, 0, fec_flags2,
