    ...
    int out_port)
{
    ...
    for (prefix = 124; prefix >= 0; prefix -= 4)
    {
        ...
        {
            printf
                ("%s(): Error, dnx_utils_l3_fec_create: fec = %d, intf_out = %d, encap_id = %d, out_port = %d, err_id = %d\n",
    ...
    int tcam)
{
    ...
    rc = bcm_l3_route_add(unit, l3_route_host);
    if (rc != BCM_E_NONE) {
      printf ("Create unmasked route failed: %d \n", rc);
      return rc;
    }
    ...
    int out_sysport)
{
    ...
    rv = vlan_translate_ive_eve_translation_set(unit, src_encap_out_vlan_port_id, 0x8100, 0, bcmVlanActionAdd, 0,
                                                (out_vlan + 1), 0, 2, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    int redir_encap_id)
{
    ...
    rv = dnx_basic_example_inner_redirect(unit, in_port, out_port, redir_port, modid, kaps_result, redir_kaps_result,
                                          redir_encap_id);
    printf("%s(): Exit\r\n", proc_name);
    return rv;
}
    ...
    int out_port)
{
    ...
    proc_name = "dnx_l3_host_destination_example";
    ...
    rv = l3__egress_only_fec__create(unit, fec, intf_out, encap_id, gport, 0, 0);
    if (rv != BCM_E_NONE)
    {
