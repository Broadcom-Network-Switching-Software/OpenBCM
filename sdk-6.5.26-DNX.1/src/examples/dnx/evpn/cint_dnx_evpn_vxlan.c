    ...
    int peer2_port)
{
    ...
    printf("Create VPN (%d)\n", evpn_vxlan_info.vpn);
    rv = dnx_utils_vxlan_open_vpn(unit, evpn_vxlan_info.vpn, 0, evpn_vxlan_info.vni);
    if (rv != BCM_E_NONE)
    {
        printf("ERROR: in dnx_utils_vxlan_open_vpn\n");
        return rv;
    }

    evpn_vxlan_split_horizon_set(unit);
    ...
    int add_to_flooding)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("ERROR: in dnx_utils_l3_eth_rif_create\n");
        return rv;
    }
    ...
    rv = dnx_utils_vxlan_port_add(unit, &info->vxlan_port);
    if (rv != BCM_E_NONE) {
        printf("ERROR: in dnx_utils_vxlan_port_add\n");
        return rv;
    }

    /* update tunnel init orientation.
     * It's not done by vxlan_port_add since we configure it with tunnel term lif and FEC */
    rv = bcm_port_class_set(unit, info->tunnel_init.tunnel_id, bcmPortClassForwardEgress, evpn_vxlan_info.vxlan_nwk_grp_id);
