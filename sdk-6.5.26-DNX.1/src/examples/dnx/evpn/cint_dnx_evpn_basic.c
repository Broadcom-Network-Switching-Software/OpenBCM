    ...
    evpn_es_info_s * esi)
{
    ...
    if (is_evpn_etree && vport->group == 8)
    {
        ...
        rv = bcm_port_class_set(unit,vport->vlan_port_id, pclass, evpn_global_info.in_lif_acl_profile_esi);
        if (rv != BCM_E_NONE)
        {
    ...
    int p2_port)
{
    ...
    if (vmpi_support)
    {
        rv = bcm_port_class_set(unit,p1_port,bcmPortClassMplsForwardIngress,cint_evpn_info.vmpi);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = bcm_port_class_set(unit,p2_port,bcmPortClassMplsForwardIngress,cint_evpn_info.vmpi);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_port_class_set\n");
            return rv;
    ...
    bcm_mpls_egress_label_t * plabel)
{
    ...
    if (iml_info->add_to_flooding)
    {
        if (iml_info->lsp_tunnel_id > 0)
        {
            ...
            if (rv != BCM_E_NONE)
            {
                printf("ERROR: bcm_multicast_encap_extension_create ID = 0x%08x\n", mc_ext_id);
                return rv;
            }
    ...
    int p2_port)
{
    ...
    rv = bcm_port_class_set(unit, recyle_port, bcmPortClassFieldIngressPMF3TrafficManagementPortCs, 4);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_port_class_set \n");
        return rv;
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_dnx_mpls_tunnel_switch_delete pe1_pe2_lsp_label\n");
        return rv;
    }
    bcm_mpls_tunnel_switch_t_init(&entry);
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_dnx_mpls_tunnel_switch_delete pe1_pe3_lsp_label\n");
        return rv;
    }
    ...
    int enable)
{
    ...
    rv = bcm_stk_gport_sysport_get(unit, port, &org_sys_port);
    if (BCM_E_NONE != rv)
    {
        printf("bcm_stk_gport_sysport_get failed with port %d \n", port);
        return rv;
    }

    /** Check if the virtual_sys_port is mapped to a modport already, if not add the maps.*/
    int modport_gport = 0, org_modport_gport;
    ...
    if (rv = BCM_E_NOT_FOUND)
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_port_get with port %d\n", port);
            return rv;
        }

        rv = bcm_stk_sysport_gport_get(unit, org_sys_port, &org_modport_gport);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_port_get with port %d\n", org_sys_port);
            return rv;
        ...
        rv = bcm_stk_sysport_gport_set(unit, virtual_sys_port, org_modport_gport);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_stk_sysport_gport_set with port %d\n", org_sys_port);
            return rv;
        }
    }

    rv = bcm_cosq_sysport_ingress_queue_map_get(unit, 0, org_sys_port, &voq_gport);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_cosq_sysport_ingress_queue_map_get with port %d\n", port);
        return rv;
    }

    /** Map the virtual system port to the same VOQ as the UC port*/
    flags = enable ? 0 : BCM_COSQ_SYSPORT_INGRESS_QUEUE_UNMAP;
    rv = bcm_cosq_sysport_ingress_queue_map_set(unit, flags, virtual_sys_port, voq_gport);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_cosq_sysport_ingress_queue_map_set with port 0x%x, voq 0x%x\n", virtual_sys_port, voq_gport);
        return rv;
    }
    ...
    int p2_port)
{
    ...
    rv = evpn_bum_df_switch_port_set(unit, virtual_ce3_sys_port, ce3_port, TRUE);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in evpn_bum_df_switch_port_set with port %d\n", virtual_ce3_sys_port);
        return rv;
    }
