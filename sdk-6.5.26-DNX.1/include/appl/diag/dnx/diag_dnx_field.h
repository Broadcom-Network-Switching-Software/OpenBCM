...
/**
 * \brief
 *   Keyword for the field presel.
 */
#define DIAG_DNX_FIELD_OPTION_PRESEL_ID     "presel"
/**
 * \brief
 *   Keyword for the field context.
...
/**
 * \brief
 * Enum for option requiring 'pmf' and none 'pmf' stages for 'presel' diagnostic.
 */
extern sh_sand_enum_t Field_presel_diag_stage_enum_table[];
/**
 * \brief
...
/**
 * \brief
 *     Generic init function for configuring device to be potentially used by the init functions of different
 *     field diag command (for producing gold results)
 * \param [in] unit - Identifier of HW platform.
 * \param [in]  add_entries- If FALSE do not perform entry add.
 * \return
 *    \retval Zero - On success
 *    \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 *   * None
 */
shr_error_e diag_dnx_field_generic_init(
    int unit,
    int add_entries);
