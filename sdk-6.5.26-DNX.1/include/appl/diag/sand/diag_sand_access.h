...
SAND_DCL_CMD(cmd_sand_mem_modify) SAND_DCL_CMD(cmd_sand_dma) SAND_DCL_CMD(cmd_sand_intr)
    ...
    SAND_DCL_CMD(cmd_sand_cpu_i2c) SAND_DCL_CMD(cmd_sand_cpu_regs) SAND_DCL_CMD(cmd_sand_i2c_smbus)
#endif
    SAND_DCL_CMD(cmd_sand_pcie_reg) SAND_DCL_CMD(cmd_sand_pcic_access)
/** Print information for the PCI interface */
     void
     diag_sand_pci_print_config(
    int unit);

/** Dumping a memory by address and optional count */
     int
