...
/*
 * VALGRIND requires memory of its own for memory monitoring, together with the pre-allocated RX
 * pool, the system probably doesn't have that much memory.
 */
#define RX_PKT_SIZE_DFLT       (4 * 1024)    /* 4 K packets */
#else
...
#if (VALGRIND_SUPPORT == 1)
/* VALGRIND requires memory of its own for memory monitoring, together with the pre-allocated RX pool, the system probably doesn't have that much memory. */
#define BCM_RX_POOL_COUNT_DEFAULT      64
#else
...
#define RX_CONTROL_THREAD_RUNNING_GET() \
    rx_control_thread_running_get()
#define RX_CONTROL_THREAD_RUNNING_SET(value) \
    rx_control_thread_running_set(value)
/* } */
#else
/* { */
#define RX_CONTROL_THREAD_RUNNING_GET() \
...
#define _BCM_RX_CHECK_THREAD_DONE  if (!RX_CONTROL_THREAD_RUNNING_GET()) break

#define RX_IS_SETUP(unit)     ((((unit) >= 0) && \
                                ((unit) < BCM_CONTROL_MAX)) \
                                && (rx_ctl[unit] != NULL))
#define RX_IS_LOCAL_SETUP(unit) ((((unit) >= 0) && \
                                 ((unit) < BCM_LOCAL_UNITS_MAX)) \
                                 && (rx_ctl[unit] != NULL))
...
#define RX_IS_LOCAL(unit) \
    (RX_IS_LOCAL_SETUP(unit) && (!BCM_UNIT_VALID(unit) || \
      (BCM_IS_LOCAL(unit) && !RX_IS_RCPU(unit))))
#define RX_IS_REMOTE(unit)    (RX_IS_SETUP(unit) && !RX_IS_LOCAL(unit))
#else
#define RX_IS_LOCAL(unit)     (RX_IS_LOCAL_SETUP(unit) && !RX_IS_RCPU(unit))
