...
/*
 * Nof bits in mpls identification array
 */
#define SWITCH_MPLS_IDENTIFICATION_SIZE_IN_UINT32 BITS2WORDS(DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_LAYER_PROTOCOLS)
 ...
 *   * bcm_dnx_switch_control_indexed_set() (for 'bcmSwitchHashSe1')
 */
typedef enum
{
...
/**
* Enum that lists all devices methods for creating the hash key inside the ingress FWD block
*/
typedef enum
{
    /** Using CRC functions to create the hash keys */
    DNX_SWITCH_LB_FWD_HASH_METHOD_CRC_FUNCTION
/* { */
        ,
    /** Using SPN to create the hash keys */
    DNX_SWITCH_LB_FWD_HASH_METHOD_SPN_KEY_GENERATION
/* } */
} dnx_switch_lb_fwd_hash_method_e;
/*
 * }
 */
/*
...
{
    /**
     * Bit map in the size of nof MPLS protocols.
     * Contains the MPLS protocols identification bitmap in which each bit represent a protocol.
     * The bits, which are set, identify MPLS protocols to FWD hw.
     */
    uint32 mpls_identification[SWITCH_MPLS_IDENTIFICATION_SIZE_IN_UINT32];
 ...
 * \param [in] general_seeds_p - Pointer to structure of type dnx_switch_lb_general_seeds_t which is
 *                               loaded, by this procedure, by all 3 fields to load. Each field is of 32 bits.
 * \return
 *    \retval Zero if no error was detected
 *    \retval Negative if error was detected. See \ref shr_error_e
 * \remark
 * For now, IPPE_PRSE_LOAD_BALANCING_SEED has not been included in the register file and, therefore, is not accessed.
 * \see
 *   * dnx_switch_lb_general_seeds_t
...
/**
 * \brief - Include termianted L2 Layer in Load Balancing calculation
 *
 * \param [in] unit - Relevant unit.
 * \param [in] valid - True indicates to include termianted L2 Layer in Load Balancing calculation
 * \return
 *   Error indication according to shr_error_e enum
 *
 * \remark
 ...
 *   * None
 */
shr_error_e dnx_switch_control_load_balancing_hash_force_l2_set(
    int unit,
    uint32 valid);

/**
 * \brief - Get status whether termianted L2 Layer is included in Load Balancing calculation
 *
 * \param [in] unit - Relevant unit.
 * \param [out] valid - TRUE if termianted L2 Layer is included in Load Balancing calculation
 * \return
 *   Error indication according to shr_error_e enum
 *
 * \remark
 ...
 *   * None
 */
shr_error_e dnx_switch_control_load_balancing_hash_force_l2_get(
    int unit,
    uint32 *valid);
