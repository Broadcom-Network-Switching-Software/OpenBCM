 ...
 */
#define TEMPLATE_PFC_GENERIC_BMP "TEMPLATE_PFC_GENERIC_BMP"
...
/**
 * \brief - Data type for Rx Quad MAC Lane FIFO (QMLF) Flow Control generation thresholds profiles template manager
 */
typedef struct dnx_rmc_thresholds
{
    uint16 pfc_set_thresh;
    uint16 pfc_clr_thresh;
    uint16 llfc_set_thresh;
    uint16 llfc_clr_thresh;
} dnx_rmc_thresholds_t;
...
/**
 * \brief -
 *   Print the QMLF Threshold template content.
 * \param [in] unit - relevant unit
 * \param [in] data - template data
 * \return
 *   shr_error_e
 ...
 * \see
 */
void dnx_qlmf_thresholds_profile_template_print_cb(
    int unit,
    const void *data);
