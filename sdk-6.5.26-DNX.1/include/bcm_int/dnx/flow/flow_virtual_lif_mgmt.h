...
/**
 * \brief - indicates that the entry was not created by flow, hence it should not be deleted by flow.
 *  
 *   \param [in] unit - unit id
 *   \param [in] virtual_lif_gport - The gport of the added
 *          virtual LIF
 ...
 *   shr_error_e
 */
shr_error_e dnx_flow_egress_virtual_entry_not_created_by_flow_set(
    int unit,
    bcm_gport_t virtual_lif_gport);
...
/**
 * \brief - Retrieve an indication if entry was created by flow. API returns not_found in case the entry was not updated 
 *        by flow at all (without a log). So to know if the entry was not created by flow it can be (rv == not_found) or
 *        !is_created_by_flow.
 *  
 *   \param [in] unit - unit id
 *   \param [in] virtual_lif_gport - The gport of the virtual LIF for which the result-type is retrieved.
 *   \param [out] is_created_by_flow - inidcation returned to the user.
 *  
 * \return DIRECT OUTPUT:
 *   shr_error_e
 */
shr_error_e dnx_flow_egress_virtual_is_created_by_flow(
    int unit,
    bcm_gport_t virtual_lif_gport,
    uint32 *is_created_by_flow);
