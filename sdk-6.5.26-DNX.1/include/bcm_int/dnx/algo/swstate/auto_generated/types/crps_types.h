...
typedef struct {
    ...
    /**
     * Counter size, relevant for state-counters-formats only
     */
    int counter_size;
    /**
     * The action performed on the engine's counters (relevant only for formats that supports dynamic mode)
     */
...
typedef struct {
    ...
    /**
     * holds the next counter id expected to be evicted by the sequential scan
     */
    uint32 next_seq_counter_idx;
    /**
     * sample's sequence number, increments every sample update
     */
    uint8* sequence_num;
    /**
     * SW counter-memory for rate calculation,              holds the change in the counter value during sequential scan iteration.
     */
    uint8 (*seq_period_shadow)[6];
    /**
     * SW counter-memory for rate calculation,              holds the sum of the data evicted due to an algorithmic scan in the current iteration.
     */
    uint8 (*algo_shadow)[6];
...
typedef struct {
    ...
    /**
     * crps counters rate calculation information for the current engine
     */
