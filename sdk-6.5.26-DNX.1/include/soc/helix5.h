...
#define SOC_HX5_PORT_BLOCK_BASE_PHY_PORT(phy_port)  \
        phy_port <= 48 ?                            \
        (1 + ((phy_port - 1) & ~0xF)) :             \
        (1 + ((phy_port - 1) & ~0x3));
...
extern int soc_hx5_cosq_threshold_dynamic_change_check(
    int unit,
    int new_threshold,
    soc_reg_t ctr_reg,
...
extern int soc_helix5_port_blk_reset(int unit, int blk);
