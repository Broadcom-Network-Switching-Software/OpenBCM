...
#define DBAL_SW_HASH_TABLES_MAX_HASH_KEY_SIZE_BITS 164

#define HW_ENT_DIRECT_MAP_HASH_KEY_SIZE 5

#define HW_ENT_GROUPS_MAP_HASH_KEY_SIZE 5

#define DBAL_NOF_FIELD_NAME_OVERRIDE_INSTANCES 100
...
typedef enum {
    ...
    DBAL_SW_STATE_TABLE_TYPE_E_DUMMY
...
typedef enum {
    ...
    DBAL_MULTI_RES_INFO_STATUS_E_DUMMY
...
typedef struct {
    char string[DBAL_MAX_STRING_LENGTH];
} encap_string_type_t;
...
typedef struct {
    ...
    dbal_dynamic_fields_t dbal_fields[DBAL_NOF_DYNAMIC_FIELDS];
    dbal_dynamic_table_res_types_t dbal_ppmc_table_res_types;
    sw_state_idx_htbl_t hw_ent_direct_map_hash_tbl;
    sw_state_idx_htbl_t hw_ent_groups_map_hash_tbl;
    sw_state_idx_htbl_t field_names_override_locations;
    encap_string_type_t field_names_override_strings[DBAL_NOF_FIELD_NAME_OVERRIDE_INSTANCES];
} dnx_dbal_sw_state_t;
