...
#define DBAL_HL_MAX_REG_LINE_SIZE_IN_WORDS               (30)


#define DBAL_HL_MAX_REG_LINE_SIZE_IN_BYTES            (DBAL_HL_MAX_REG_LINE_SIZE_IN_WORDS*sizeof(uint32))
...
#define DBAL_HANDLE_IS_MERGE_NEEDED(entry_handle)        \
   (!(entry_handle->phy_entry.attr_flags & DBAL_ENTRY_ATTR_FLAG_NO_PAYLOAD) && \
        (entry_handle->nof_result_fields != entry_handle->table->multi_res_info[entry_handle->cur_res_type].nof_result_fields))
...
#define DBAL_FIELD_MAX_NUM_OF_DEFINE_VALUES             41
...
#define DBAL_FIELD_IND_IS_ALLOCATOR_DISABLED            16
#define DBAL_NOF_FIELD_IND                             (17)
...
typedef enum {
    ...
    DBAL_TABLE_IND_IS_NONE_DIRECT_FORCE_OPTIMIZED,      
    ...
    DBAL_TABLE_IND_KEY_CONST_VALUE_EXISTS,              
    DBAL_TABLE_IND_RESULT_CONST_VALUE_EXISTS,           
    DBAL_TABLE_IND_IS_ASYMMETRIC,                       
    DBAL_TABLE_IND_ZERO_SIZE_PAYLOAD_SUPPORT,           
    DBAL_TABLE_IND_LART_SUPPORT,                        
    DBAL_TABLE_IND_IS_NEW_ACCESS,                       
    DBAL_TABLE_IND_SW_RESULT_TYPE_NO_ACCESS,            
    DBAL_NOF_TABLE_IND
...
typedef enum
{
    ...
    DBAL_ENTRY_ATTR_PRIORITY = 0,
        
    DBAL_ENTRY_ATTR_AGE,
    
    DBAL_ENTRY_ATTR_HITBIT,
    
    DBAL_ENTRY_ATTR_CACHED,
    
    DBAL_ENTRY_ATTR_INVALID,
    
    DBAL_ENTRY_ATTR_NO_PAYLOAD,
    DBAL_NOF_ENTRY_ATTR_TYPE         
} dbal_entry_attribute_t;
...
#define DBAL_ENTRY_ATTR_FLAG_AGE           SAL_BIT(1)

#define DBAL_ENTRY_ATTR_FLAG_HIT_GET       SAL_BIT(2)
#define DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR     SAL_BIT(3)

#define DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY   SAL_BIT(4)
#define DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY SAL_BIT(5)

#define DBAL_ENTRY_ATTR_FLAG_CACHED        SAL_BIT(6)
#define DBAL_ENTRY_ATTR_FLAG_INVALID       SAL_BIT(7)
#define DBAL_ENTRY_ATTR_FLAG_NO_PAYLOAD    SAL_BIT(8)
...
#define DBAL_INDIRECT_COMMIT_MODE_NORMAL (0)

#define DBAL_INDIRECT_COMMIT_MODE_UPDATE (1)

#define DBAL_INDIRECT_COMMIT_MODE_FORCE (2)
#define DBAL_NOF_INDIRECT_COMMIT_MODES (3)
...
#define DBAL_HANDLE_STATUS_AVAILABLE (0)

#define DBAL_HANDLE_STATUS_IN_USE (1)

#define DBAL_HANDLE_STATUS_ACTION_PERFORMED (2)
#define DBAL_NOF_ENTRY_HANDLE_STATUSES (3)
...
#define DBAL_CORE_NOT_INITIALIZED   (-1) 
...
#define DBAL_MDB_ACTION_APPLY_ALL (0)
#define DBAL_MDB_ACTION_APPLY_NONE (1)
#define DBAL_MDB_ACTION_APPLY_HW_ONLY (2)
#define DBAL_MDB_ACTION_APPLY_SW_SHADOW (3)

#define DBAL_MDB_ACTION_APPLY_ENTRY_INVALID (4)
#define DBAL_NOF_MDB_ACTION_APPLY_TYPES (5)
...
typedef struct
{
    ...
    dbal_defines_info_t *defines_info;
...
typedef enum
{
    ...
    DBAL_HL_TCAM_ACCESS_TYPE_VALID_BIT,
...
typedef struct
{
    ...
    SW_STATE_INDEX_HASH_TABLE_ITER mdb_em_htb_iter;     
    SW_STATE_INDEX_HASH_TABLE_ITER mdb_em_key_size;
    
    SW_STATE_INDEX_HASH_TABLE_ITER mdb_em_payload_size;
...
typedef struct
{
    SW_STATE_INDEX_HASH_TABLE_PTR hash_table_id;
    SW_STATE_INDEX_HASH_TABLE_ITER hash_entry_index;
...
typedef struct
{
    ...
    dbal_entry_handle_t *internal_entry_handles_pool;

    dbal_iterator_info_t iterators_pool[DBAL_SW_NOF_ENTRY_HANDLES];
    
    uint32 * range_info_ptr_array[DBAL_SW_NOF_ENTRY_HANDLES];
    uint8 nof_range_info_ptr;
