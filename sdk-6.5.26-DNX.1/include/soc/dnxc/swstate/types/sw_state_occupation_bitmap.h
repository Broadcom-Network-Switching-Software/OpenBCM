...
#define SW_STATE_OCC_BM_CREATE(node_id, occ_bm, init_info, occ_size, alloc_flags) \
    SHR_IF_ERR_EXIT(sw_state_occ_bm_create(unit, node_id, init_info, &occ_bm, occ_size, alloc_flags));

#define SW_STATE_OCC_BM_DESTROY(node_id, occ_bm) \
    sw_state_occ_bm_destroy(unit, node_id, &occ_bm)

#define SW_STATE_OCC_BM_INIT_INFO_CLEAR(init_info) \
    sw_state_occ_bm_init_info_clear(init_info)

#define SW_STATE_OCC_BM_GET_NEXT_IN_RANGE(node_id, occ_bm, start, end, forward, val, place, found) \
    sw_state_occ_bm_get_next_in_range(unit, node_id, occ_bm, start, end, forward, val, place, found)

#define SW_STATE_OCC_BM_GET_NEXT(node_id, occ_bm, place, found) \
    sw_state_occ_bm_get_next(unit, node_id, occ_bm, place, found)

#define SW_STATE_OCC_BM_STATUS_SET(node_id, occ_bm, place, occupied) \
    sw_state_occ_bm_occup_status_set(unit, node_id, occ_bm, place, occupied)

#define SW_STATE_OCC_BM_IS_OCCUPIED(node_id, occ_bm, place, occupied) \
    sw_state_occ_bm_is_occupied(unit, node_id, occ_bm, place, occupied)

#define SW_STATE_OCC_BM_ALLOC_NEXT(node_id, occ_bm, place, found) \
    sw_state_occ_bm_alloc_next(unit, node_id, occ_bm, place, found)

#define SW_STATE_OCC_BM_CLEAR(node_id, occ_bm) \
    sw_state_occ_bm_clear(unit, node_id, occ_bm)

#define SW_STATE_OCC_BM_PRINT(node_id, occ_bm) \
