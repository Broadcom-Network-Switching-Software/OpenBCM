...
#define IGNORE_INIT_ONLY(FLAGS) FLAGS & ~DNXC_DATA_F_INIT_ONLY
...
#define DNXC_DATA_MGMT_ARR_STR(buffer, size, data)\
    ...
    {\
        ...
        {\
            ...
            offset += sal_snprintf(buffer + offset, DNXC_DATA_MGMT_MAX_TABLE_VALUE_LENGTH, "%08x ", data[i]);\
        }\
    } while(0)
...
#define DNXC_DATA_MGMT_PBMP_STR(buffer, data)\
    ...
    {\
        ...
        {\
            if (offset + 10 > DNXC_DATA_MGMT_MAX_TABLE_VALUE_LENGTH) break;\
            offset += sal_snprintf(buffer + offset, DNXC_DATA_MGMT_MAX_TABLE_VALUE_LENGTH, "%08x ", _SHR_PBMP_WORD_GET(data, i));\
        }\
    } while(0)

#define DNXC_DATA_FEATURE_GET(_unit, _module, _submodule, _feature)\
        _module._submodule.feature_get(_unit, _module##_##_submodule##_##_feature)
...
uint32 dnxc_data_mgmt_state_get(
    int unit);
...
void dnxc_data_mgmt_inaccessible_data_get(
    int unit,
    char *field_name,
    char *family_exclusive);

void dnxc_data_mgmt_inaccessible_data_set(
    int unit,
    char *field_name,
    char *family_exclusive);
...
char *dnxc_data_mgmt_shell_table_data_get(
    int unit,
...
int dnxc_data_mgmt_is_j2p_a0(
    int unit);
int dnxc_data_mgmt_is_j2p_a1(
    int unit);
int dnxc_data_mgmt_is_j2p_a2(
    int unit);
int dnxc_data_mgmt_is_j2x_a0(
    int unit);
