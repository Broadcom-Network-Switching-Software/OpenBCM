...
#define SOC_SBUSDMA_CTRL_LAST      (1U<<31)
#define SOC_SBUSDMA_CTRL_NOT_LAST ~(1U<<31)
...
/* Indicates 64bit swap data */
#define SOC_SBUSDMA_64BIT_SWAP            0x2000
/* Indicates access address reverse, high to low */
#define SOC_SBUSDMA_DEC_ADDR              0x4000
/* Indicates descriptor data big endian or indicated by host CPU */
#define SOC_SBUSDMA_CFG_DATA_BIGENDIAN    0x8000
/* Indicates config and data descriptors are from Shared SRAM memory not from host memory*/
#define SOC_SBUSDMA_CFG_SSRAM_MEMORY      0x10000
/*
 * AXI interface select, used on cmicx gen2
 * used by PCI-e master be mapped to AXI M0 and those channels used by RTS master be mapped to AXI M1.. 
 * if specify the flag, must specify SOC_SBUSDMA_CFG_DATA_BIGENDIAN is set or not.
 */
#define SOC_SBUSDMA_AXI_SELECT_M1         0x10000
...
extern int soc_sbusdma_pci_timeout_handle(int unit, int cmc);
