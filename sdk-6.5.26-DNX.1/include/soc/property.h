...
/* Maximum number of consecutive PKTDMA errors. */
#define spn_PDMA_MAX_ERR_COUNT  "pdma_max_err_count"
...
/* Maximum number of consecutive DMA errors during counter collection. */
#define spn_SOC_COUNTER_MAX_DMA_ERR_COUNT  "soc_counter_max_dma_err_count"
/*
...
/*
 * Used during warmboot recovery stage on BCM5698x devices.
 * When value is set to 1, the sync thread will not issue callbacks until it
 * rebuilds its shadow table; that is, during the first cycle of the sync
 * thread's processing. Callbacks resume after the shadow table is rebuilt
 * (at the end of the first cycle of sync thread processing)
 * When value is set to 0, the current behavior of issuing callbacks during
 * table rebuild is followed
 */
#define spn_L2XMSG_NO_CB_DURING_TABLE_REBUILD  "l2xmsg_no_cb_during_table_rebuild"
/*
...
/*
 * Retry S-Channel operations in timeout.
 * By default, value 0 disables the retry.
 */
#define spn_SCHAN_TIMEOUT_RETRY_ENABLE  "schan_timeout_retry_enable"
...
/* Fabric debug feature to use only PLL 0 when setting speed if possible */
#define spn_PLL0_ONLY  "pll0_only"
/*
 ...
 * Bits [17:16] - Linkup delay in PHY chip before forwarding Ingress Traffic.
 * Bits [17:16] = 0b00 - 1500ms, 0b01 - 500ms, 0b10 - 1000ms, 0b11 - 2000ms.
 */
...
/*
 * maximum ports that supports MMU lossless configuration (valid only if
 * mmu_lossless=1
 * This property is only required for specific chips that have limited MMU
 * lossless ports, such as BCM5667X.
 */
#define spn_MMU_LOSSLESS_PORTS_MAX  "mmu_lossless_ports_max"
/*
 * maximum headroom that will be reserved for lossless ports (valid only if
 * mmu_lossless=1
 * This property is only required for specific chips that have limited MMU
 * lossless ports, such as BCM5667X.
 */
#define spn_MMU_LOSSLESS_RESERVE_HEADROOM_SIZE  "mmu_lossless_reserve_headroom_size"
/*
 ...
 * By default, it sets the same divisor on all MDIO rings.
 * For CMICX devices, it can be suffixed with "_#" to selectively set it per MDIO ring.
 */
...
/* Enable/Disable the configuration of resetting remote discriminator variable under certain circumstances such as
                       receiving admin_down or down messages when local session state is UP.
                       The default value of the config property is 0 (disabled) */
#define spn_BFD_REMOTE_DISCRIMINATOR_RESET  "bfd_remote_discriminator_reset"
...
/* Configure number of tx log packet buffers for BFD sessions with flags2 set to
                        BCM_BFD_ENDPOINT_FLAGS2_RX_PKT_COPY2CPU_ON_STATE_CHANGE. */
#define spn_BFD_NUM_TX_LOG_BUFFERS  "bfd_num_tx_log_buffers"

/* Enable/Disable L3IIF to VRF lookup in the FW. */
#define spn_BFD_L3_IIF_TO_VRF_MAP_ENABLE  "bfd_l3_iif_to_vrf_map_enable"
...
/* PTP BroadSync1/10Mhz PLL value for fref parameter */
#define spn_PTP_BS1_FREF  "ptp_bs1_fref"
 ...
 * By default, it sets the same clock delay on all MDIO rings.
 * For CMICX devices,it can be suffixed with "_#" to selectively set it per MDIO ring.
 */
#define spn_MDIO_OUTPUT_DELAY  "mdio_output_delay"
/*
 * The config property is used to select packet,s egress tag behavior
 * on the device. User can select "egress vlan translate based tag action"
 * or "hash based egress VP vlan membership" on the devices that support
 * both methods. The valid values are 0 and 1.
 * 0: hash based egress VP vlan membership is used for egress tag
 * 1: egress vlan translate based tag action is used for egress tag
 * default: 0
 */
#define spn_VLAN_TAG_CTRL_ON_EGRESS  "vlan_tag_ctrl_on_egress"
...
/* credit worth size. Configuration units are in bytes. Supported on 88650 devices */
...
/*
 * Port egress recycling scheduler configuration. Supported on 88650 device
 * 
 ...
 * If  there are Peter-A devices in the system it must match their frequency or 2/3 of their frequency
 */
...
/*
 * Sets the LLFC type. Valid options are:
 * NONE
 * LLFC
 * CLLFC
 */
#define spn_LLFC_TYPE  "llfc_type"
...
/*
 * Number of ticks the 88650 devices clock ticks per second (about a tick every
 * 4.00 nanoseconds). Default value 88650: 600. Units: MHz.
 * Range for 88650: 150-900
 */
...
/*
 * PM cable swap master/slave configurations.
 *   0 - slave
 *   1 - master
 *   if not set, master will be set according to the fabric device mode
 *     FE13 - slave
 *     FE2 single-stage - slave
 *     FE2 multi-stage - master
 *     FAP - master
 */
#define spn_PM_CABLE_SWAP_MASTER  "pm_cable_swap_master"
/*
 * PM cable swap enable.
 *   0 - disable
 *   1 - enable
 */
#define spn_PM_CABLE_SWAP_ENABLE  "pm_cable_swap_enable"
/*
 ...
 * UDH_ETH: Port header processing type: UDH_ETH. Supported direction:
 *   Outgoing. Designed to support outgoing packet encapsulated with whole UDH Data.
...
/*
 * Explicity control OUT TM port of a local port.
 * Range: 0-255
 */
#define spn_PORT_OUT_TM  "port_out_tm"
/*
 ...
 * mmu_lossless=1).
 * This property is only required for specific chips that have limited MMU
 * lossless ports, such as BCM5354x,BCM5667X.
...
/*
 * Enable the counter collection preemption.
 * Current counter collection will be preempted when the following APIs are active:
 ...
 *    1: Enable
 */
#define spn_COUNTER_COLLECTION_PREEMPT_ENABLE  "counter_collection_preempt_enable"
/*
 * The maximum time period in microseconds for counter collection preemption.
 * When continous preemption time exceeds this maximum period,
 * the preemption will be paused to allow a full counter collection completes
 * Valid values:
 *    0: No maximum period
 *    1 ~ (2^31-1): 1 ~ 2,147,483,647 microseconds
 * Note if this value is non-zero but less than current counter collection interval,
 * the maximum preempt time period will be current counter collection interval,
 */
#define spn_COUNTER_COLLECTION_PREEMPT_MAX_PERIOD_USEC  "counter_collection_preempt_max_period_usec"
/*
...
/* Link delay for 20 GbE port (ns) */
#define spn_LINK_DELAY_20GBE_NS  "link_delay_20gbe_ns"
...
/* Timestamp osts adjust for 20 GbE port (ns) */
#define spn_TIMESTAMP_ADJUST_20GBE_NS  "timestamp_adjust_20gbe_ns"
...
/* Timestamp tsts adjust for 20 GbE port (ns) */
#define spn_TIMESTAMP_TSTS_ADJUST_20GBE_NS  "timestamp_tsts_adjust_20gbe_ns"
...
/*
 * Allows default TPID on ports to be disabled.
 * When all TPID are deleted from a port, the default TPID is added back.
 * If enabled, this control will prevent a port from automatically adding
 * the default TPID back to that port.
 * port_allow_tpid_disable{logical port number}=0 or 1
 * Valid values are 0 or 1. Default value is 0.
 */
#define spn_PORT_ALLOW_TPID_DISABLE  "port_allow_tpid_disable"
/*
 * BCM5696X :
...
/*
 * ext_tcam_port_cross_connectivity to specify if two JR units connected to KBP with cross or straight.
 * 1 is representing cross connectivity, 0 is representing straight connectivity.
 * It is only applicable to dual host mode (2xJR+ with 1xOP)
 */
#define spn_EXT_TCAM_PORT_CROSS_CONNECTIVITY  "ext_tcam_port_cross_connectivity"
/*
 ...
 * If IPFIX observation domain needs to be configured at runtime, this
 * config property must not be configured.
 */
...
/*
 * Maximum number of collectors that can be attached to flowtracker groups. Valid only if flowtracker_enable=2
 * default value: 1
 */
#define spn_FLOWTRACKER_MAX_COLLECTORS  "flowtracker_max_collectors"
...
/* Configure up mep dm tod recycle port for core0 */
#define spn_UP_MEP_TOD_RECYCLE_PORT_CORE_0  "up_mep_tod_recycle_port_core_0"

/* Configure up mep dm tod recycle port for core1. */
#define spn_UP_MEP_TOD_RECYCLE_PORT_CORE_1  "up_mep_tod_recycle_port_core_1"
...
/*
 * 0x0 - System does not have any DNX2 devices
 * 0x1 - System has DNX2 devices
 */
#define spn_IS_DNX2_IN_SYSTEM  "is_dnx2_in_system"
/*
...
/*
 * This property is defined for BCM5698x which does not support pending learn
 *  functionality in hardware. When enabled, software simulates per-port pending
 *  learn for learning L2 entries
 * Valid values: 0 (disable) or 1 (enable)
 */
#define spn_L2_SW_PENDING_LEARN  "l2_sw_pending_learn"
/*
...
/* Maximum number of IFA VXLAN terminated flows supported by IFA embedded app. */
#define spn_IFA_MAX_VXLAN_TERM_FLOWS  "ifa_max_vxlan_term_flows"
/*
 * LNS format to which the tailstamped metadata is converted
 * prior to insertion in the metadata stack.
 */
#define spn_IFA_TAILSTAMP_METADATA_LNS_ID  "ifa_tailstamp_metadata_lns_id"
/*
...
/*
 * Enable/Disable the DLB feature. By default, we enable DLB.
 * For disable, dlb_en = 0
 * For enable,  dlb_en = 1
 */
#define spn_DLB_ENABLE  "dlb_enable"
...
/*
 * This soc property is used to disable IP-EP clock gating for BCM5687x,
 * BCM5677x, BCM5637x, BCM5647x and BCM5627x devices.
...
/* Read To Write command delay, not including other time params added to final value, Only used if ext_ram_t_e_rtw is not set */
#define spn_EXT_RAM_T_RTW  "ext_ram_t_rtw"

/* Read To Write command delay, given explicitly as final value */
#define spn_EXT_RAM_T_E_RTW  "ext_ram_t_e_rtw"
...
/*
 * Decide which PMF stage will have access to NEXEM2.
 * Valid values:
 * IPMF1:  Ingress PMF 1 will be able to perform a lookup in NEXEM2.
 * IPMF3:  Ingress PMF 3 will be able to perform a lookup in NEXEM2.
 */
#define spn_PMF_NEXEM2_STAGE  "pmf_nexem2_stage"
/*
 ...
 * For Example: flowtracker_aggregate_alu32_mem_banks = 8:4:4
 *              flowtracker_aggregate_alu32_mem_banks = 8::4
...
/*
 * Enable TD3 ALPM bucket sharing optimization between IPv4 and IPv6,
 * when bucket used exceeds its corresponding start boundary
 * (alpm_bkt_share_start_ipv4 or alpm_bkt_share_start_ipv6), and
 * bank usage within used buckets is below this threshold (in %).
 * For example, alpm_bkt_share_bnk_usage_thres=100 means bank_usage_thres=100%.
...
/*
 * Percentage of IPv4 buckets over total buckets where to start TD3 ALPM bucket sharing optimization.
 * This property is used only when TD3 bucket sharing optimization is enabled (alpm_bkt_share_bnk_usage_thres).
 * By default it is half of the total buckets (50%).
 */
#define spn_ALPM_BKT_SHARE_START_IPV4  "alpm_bkt_share_start_ipv4"
/*
 * Percentage of IPv6 buckets over total buckets where to start TD3 ALPM bucket sharing optimization.
 * This property is used only when TD3 bucket sharing optimization is enabled (alpm_bkt_share_bnk_usage_thres).
 * By default it is half of the total buckets (50%).
 */
#define spn_ALPM_BKT_SHARE_START_IPV6  "alpm_bkt_share_start_ipv6"
 ...
 * ID: The one byte manufacturer ID of the flash chip. The rest of the parameters will
 * be applied only if the read ID is equal to this one.
 * sector size in bytes: The smallest part of the chip that can be erased, should be a multiple of the page size.
 ...
 *    3: Enable large multicast mode 3 (Enables ACL algo mode 3 and does not expect VRF and DIP to be fully specified).
 */
...
/*
 * This soc property is applicable in L2 FIFO mode (l2xmsg_mode = 1),
 * it is used to pause L2 learning during L2 bulk operations having record to L2 FIFO enabled.
 * This soc property can be used in cases where there is very high L2 churn, which may cause
 * contention for L2 FIFO table during L2 bulk operation, causing L2 bulk operation to timeout.
 */
#define spn_L2X_BULK_OP_LEARN_PAUSE  "l2x_bulk_op_learn_pause"
/*
...
/* Count of CMICX SCHAN channels cofigured to be used in SDK. */
#define spn_CMICX_SCHAN_NUM_CFG_COUNT  "cmicx_schan_num_cfg_count"
/*
 * Adaptive IRQ poll mode. In case lot of interrupts are happening, interrupt thread
 * switches to interrupt poll mode and switch back to kernel mode to wait for interrupt
 * only when no interrupt status is pending.
 * This feature is specific to cmicx based devices. Adaptive IRQ poll mode is enabled by default.
 * It can be disabled by setting the configuration property to be 0.
 */
#define spn_IRQ_ADAPTIVE_POLL_MODE  "irq_adaptive_poll_mode"
/*
...
/* Set the CPU learning thread priority. */
#define spn_L2_DMA_CPU_LEARN_THREAD_PRIORITY  "l2_dma_cpu_learn_thread_priority"
/*
 * Select whether the QoS egress DSCP table entries are non-sharable.
 * 0 - The entries within the table are sharable between QoS maps (Default option).
 * 1 - The entries within the table are non-sharable. Each QoS map has an exclusive set for itself.
 */
#define spn_QOS_MAP_EGR_DSCP_NONSHARABLE_MODE  "qos_map_egr_dscp_nonsharable_mode"
/*
 * Select DSCP map profile sharable mode.
 * 0 - DSCP map profile shares among some map IDs (Default option).
 * 1 - Each QoS map ID holds an exclusive DSCP map profile.
 */
#define spn_QOS_MAP_DSCP_NONSHARABLE_MODE  "qos_map_dscp_nonsharable_mode"
/*
 * Flush buffer size.
 * 0 - Invalid.
 * -1 - Max nof entries (for used MDB profile) will be used.
 * > 0 - It will be used to indicate the size of the buffer (in nof entries) used for Flush DMA copied entries.
 */
#define spn_L2_FLUSH_BUFFER_NOF_ENTRIES  "l2_flush_buffer_nof_entries"

/* Maximum number of OAM groups. */
#define spn_ETH_OAM_MAX_GROUPS  "eth_oam_max_groups"

/* Maximum number of remote MEPs. */
#define spn_ETH_OAM_MAX_RMEPS  "eth_oam_max_rmeps"

/* Maximum number of CCM sessions. */
#define spn_ETH_OAM_MAX_CCM_SESSIONS  "eth_oam_max_ccm_sessions"

/* Maximum number of Loss measurement sessions */
#define spn_ETH_OAM_MAX_LM_SESSIONS  "eth_oam_max_lm_sessions"
/*
 * Enable removing all MPLS labels on a per port basis for BCM5687x, BCM5677x,
 * BCM5637x and BCM5647x series of devices.
 * With this enabled, traditional adaptation lookup will not happen on individual ports that opt to use this feature
 * by using port control API.
 * This is applicable for both L3MPLS and L2MPLS packets based on values selected
 * Valid values: 0,1 and 3.
 * 0 -> This is the default value and device behaves in the existing way where MPLS tunnel termination
 *      is based on results from adaption lookup.
 * 1 -> This enables per port MPLS termination for ONLY L2MPLS applications like VPLS/VPWS.
 * 2 -> Reserved.
 * 3 -> Enables per port MPLS termination for BOTH L2MPLS and L3MPLS applications.
 */
#define spn_MPLS_LABEL_PER_PORT_DELETE_ALL_ENABLE  "mpls_label_per_port_delete_all_enable"
/*
 * Select VLAN_XLATE table to use for VLAN assignment based on incoming MAC address.
 *     Valid values are 0 and 1. The default value is 0.
 *         0: Use VLAN_XLATE_2_DOUBLE to program MAC based VLAN translations (Default behavior)
 *         1: Use VLAN_XLATE_1_DOUBLE to program MAC based VLAN translations.
 *     The APIs impacted are:
 *         bcm_vlan_mac_add, bcm_esw_vlan_mac_delete,
 *         bcm_esw_vlan_mac_delete_all, bcm_esw_vlan_mac_get and
 *         bcm_esw_vlan_mac_action_traverse.
 */
#define spn_VLAN_MAC_ADD_XLATE_TABLE_SELECT  "vlan_mac_add_xlate_table_select"

/* Number of MEPs that allocate 2 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_2_LEN_ENTRIES  "oamp_mep_db_max_nof_2_len_entries"

/* Number of MEPs that allocate 3 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_3_LEN_ENTRIES  "oamp_mep_db_max_nof_3_len_entries"

/* Number of MEPs that allocate 4 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_4_LEN_ENTRIES  "oamp_mep_db_max_nof_4_len_entries"

/* Number of MEPs that allocate 5 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_5_LEN_ENTRIES  "oamp_mep_db_max_nof_5_len_entries"

/* Number of MEPs that allocate 6 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_6_LEN_ENTRIES  "oamp_mep_db_max_nof_6_len_entries"

/* Number of MEPs that allocate 7 from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_7_LEN_ENTRIES  "oamp_mep_db_max_nof_7_len_entries"

/* Number of MEPs that allocate 10 or 11 from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_11_LEN_ENTRIES  "oamp_mep_db_max_nof_11_len_entries"

/* Number of Jitter MEPs that allocate 2 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_JITTER_2_LEN_ENTRIES  "oamp_mep_db_max_nof_jitter_2_len_entries"

/* Number of Jitter MEPs that allocate 3 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_JITTER_3_LEN_ENTRIES  "oamp_mep_db_max_nof_jitter_3_len_entries"

/* Number of Jitter MEPs that allocate 4 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_JITTER_4_LEN_ENTRIES  "oamp_mep_db_max_nof_jitter_4_len_entries"

/* Number of Jitter MEPs that allocate 5 entries from MEP-DB */
#define spn_OAMP_MEP_DB_MAX_NOF_JITTER_5_LEN_ENTRIES  "oamp_mep_db_max_nof_jitter_5_len_entries"

/* Priority of OAMP event FIFODMA thread */
#define spn_OAMP_FIFO_DMA_EVENT_THREAD_PRIORITY  "oamp_fifo_dma_event_thread_priority"

/* Enable retry and DMA restart incase of DCB count mismatch in CMICM. */
#define spn_CMICM_DCB_ERROR_MITIGATE_ENABLE  "cmicm_dcb_error_mitigate_enable"

/* Enable cache flush for SBUSDMA. */
#define spn_SBUSDMA_FLUSH_CACHE_ENABLE  "sbusdma_flush_cache_enable"

/* Enable warm boot on pci non-recoverable PCIe completion timeout error in CMICM. */
#define spn_CMICM_PCI_COMPLETION_TIMOUT_WARM_BOOT_ENABLE  "cmicm_pci_completion_timout_warm_boot_enable"

/* MACSec support enable */
#define spn_MACSEC_ENABLED  "macsec_enabled"
/*
 * Enable the API logger
 * This property can be used to initiate logging as soon as the first API is called
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_ENABLE  "api_logger_enable"
/*
 * API logger mode
 *   0 - invoke optional cint function, log to an in-memory buffer, and optionally write to raw_logfile
 *   1 - only log to an in-memory buffer.
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_MODE  "api_logger_mode"
/*
 * Size of circular buffer to hold all API call data. This is malloc'd during SDK init and never freed
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_CACHE_SIZE  "api_logger_cache_size"
/*
 * Number of API call data entries in the circular buffer to never overwrite / lose
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_CACHE_NFREEZE  "api_logger_cache_nfreeze"
/*
 * Number of spaces in one indent level
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_BASE_INDENT  "api_logger_base_indent"
/*
 * Stack size to assign for API logger consumer thread
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_THREAD_STKSZ  "api_logger_thread_stksz"
/*
 * Priority for API logger consumer thread
 * Unix sal_thread_create ignores the thread priority attribute
 * This property is retained only for those systems which support thread priority
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_THREAD_PRIO  "api_logger_thread_prio"
/*
 * Output type for the logfile
 *   0 => None Disabled
 *   1 => RAW binary data
 *   2 => CINT-like output
 * Typically set to 2 for CINT-like output but can be used to generate RAW output also.
 * RAW output is in the same format as the one generated by the raw_logfile property below.
 * The only difference is, that file is written inline with the BCM API call whereas this is
 * relevant only for LOGFILE which is written from a SW thread meant to offload that task from
 * the thread executing the BCM API.
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_LOGFILE_OPTYPE  "api_logger_logfile_optype"
/*
 * Name of logfile to which to write the API logs
 * Typically CINT-like output is written but some users may choose to write raw output also.
 * This file is written from a dedicated SW thread and can write output in two formats.
 * See api_logger_logfile_optype.
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_LOGFILE  "api_logger_logfile"
/*
 * Name of logfile to which to write the raw API logs
 * When set, this file is written to inline with the creation of log entries in circular buffer
 * This property is referred only once ever, so changing it between init/deinit has no effect
 * unless deinit/init is performed from tr141 with api_logger_reset_on_tr141=1
 */
#define spn_API_LOGGER_RAW_LOGFILE  "api_logger_raw_logfile"
/*
 * When set, this SOC property causes the logger to reset all configurations to their default 
 * values at the end of SDK deinitialization sequence of 'tr 141'. The logger will then 
 * completely reinitialize next time it is invoked, processing all the relevant SOC properties
 * which may have been added or changed.
 */
#define spn_API_LOGGER_RESET_ON_TR141  "api_logger_reset_on_tr141"
/*
 * CL,FQ scheduling elements order in flow quartet.
 * The order is configured per group of 1K regions.
 */
#define spn_FLOW_QUARTET_ORDER  "flow_quartet_order"

/* Global lif allocation property */
#define spn_GLOBAL_LIF_ALLOCATION  "global_lif_allocation"
...
#define spn_VLAN_LOOKUP_PHYSICAL_DATABASE_MODE  "vlan_lookup_physical_database_mode"

/* PHY config ingnored for EEE controls */
#define spn_EEE_PHY_CONFIG_IGNORE  "eee_phy_config_ignore"

/* Set the number of Load Balancing keys for Egress trunks. Valid values: 128, 256 */
#define spn_TRUNK_EGRESS_NOF_LOAD_BALANCING_KEYS  "trunk_egress_nof_load_balancing_keys"
/*
 * Select the payload size per VSI range.
 * If the larger VSI entry types are not required, a smaller payload size would support more entries.
 */
#define spn_VSI_PAYLOAD_SIZE  "vsi_payload_size"

/* Allows to set the position of the FEC hierarchies in the FEC ID range. */
#define spn_FEC_ID_RANGE_POSITION  "fec_id_range_position"

/* Disable the support of destination in MPLS tunnel */
#define spn_MPLS_LSP_DESTINATION_DISABLE  "mpls_lsp_destination_disable"
/*
 * 0/1 (0 - default, 1 - JR2/J2c compatibility mode)
 * If value = 1, limit the device to JR2 SRv6 capabilities
 * value = 0, use max SRv6 capabilities of the current device.
 */
#define spn_SRV6_MODE  "srv6_mode"
/*
 * WCK granularity, see GDDR6 JEDEC
 * Supported values: WCK_PER_BYTE, WCK_PER_WORD
 */
#define spn_EXT_RAM_WCK_GRANULARITY  "ext_ram_wck_granularity"
/*
 * CK termination, see GDDR6 JEDEC
 * Supported values: DISABLED, 60_OHM, 120_OHM
 */
#define spn_EXT_RAM_CK_TERMINATION  "ext_ram_ck_termination"
/*
 * CA termination, see GDDR6 JEDEC
 * Supported values: DISABLED, 60_OHM, 120_OHM
 */
#define spn_EXT_RAM_CA_TERMINATION  "ext_ram_ca_termination"

/* Enable external DRAM temperature read */
#define spn_EXT_RAM_TEMP_READ_ENABLE  "ext_ram_temp_read_enable"

/* 0/1 (0 - default, 1 - Delete the NHI during mutlicast egress delete.) */
#define spn_BCM_L3_INTF_MCAST_NHI_AUTO_DEL  "bcm_l3_intf_mcast_nhi_auto_del"
/*
 * This property specifies the flr support and flr enable for the certain port.
 * Format: phy_init_flr_enable_lport=flr_support:timeout_time
 *   lport:         logic port id.
 *   flr_support :  choose if to download the flr ucode.
 ...
 * For Example:
 *     phy_init_flr_enable_1=1:0    Download flr ucode on the PM core that port 1 is attached, but not enable flr on port 1.
 *     phy_init_flr_enable_1=1:1    Download flr ucode on the PM core that port 1 is attached, and enable flr on port 1 with timeout of 50ms.
 */
#define spn_PHY_INIT_FLR_ENABLE  "phy_init_flr_enable"

/* Used by DNX only. Map rcy_mirror port to forward_port */
#define spn_RCY_MIRROR_TO_FORWARD_PORT_MAP  "rcy_mirror_to_forward_port_map"

/* Enable SRv6 function */
#define spn_L3_SRV6_ENABLE  "l3_srv6_enable"
/*
 * Enable/disable power optimization for L1 only, and set its mode
 * DISABLED -------------------------disable feature
 * L1_ONLY_WITHOUT_ILKN -------------Enable feature for L1 only without ILKN
 * L1_ONLY_WITH_ILKN ----------------Enable feature for L1 only with ILKN
 */
#define spn_L1_ONLY_MODE  "l1_only_mode"
/*
 * This configuration property is used to enable/disable default QoS mapping
 * loading when creating a QoS map.
 * Note:
 *  qos_default_map = 0 (default): disable loading default Qos mapping.
 *  qos_default_map = 1: enable loading default Qos mapping.
 */
#define spn_QOS_DEFAULT_MAP  "qos_default_map"
/*
 * Select MOD map mode of remote unit from global MODID to local MODID or vice versa.
 * 0 - Convert remote (MODID, port) based on dual MOD config of local unit;
 * 1 - Do not convert remote (MODID, port) based on dual MOD config of local unit.
 *     Instead, applications need to register a callback to convert remote
 *     (MODID, port).
 */
#define spn_REMOTE_MOD_MAP_MODE  "remote_mod_map_mode"
/*
 * Specifies the number of alu64 memory banks allocated to flowtracker stages.
 * If this config is not specified or there is any un-allocated resource left,
 * it will be allocated to IFT by default.
 * flowtracker_alu64_mem_banks = <no. of ift alu64>:<no. of mft alu64>:<no. of eft alu64>
 * For Example: flowtracker_alu64_mem_banks = 2:1:1
 *              flowtracker_alu64_mem_banks = :2:2
 *              flowtracker_alu64_mem_banks = ::
 *              flowtracker_alu64_mem_banks = ::4
 */
#define spn_FLOWTRACKER_ALU64_MEM_BANKS  "flowtracker_alu64_mem_banks"
/*
 * This configuration property is used to enable/disable Serdes PHY TimeSync.
 * This property may be configured with the base name to control all ports,
 * or with the suffix "_lport#" to configure a specific port.
 * This is only for Tomahawk3.
 */
#define spn_SERDES_TIMESYNC_ENABLE  "serdes_timesync_enable"
/*
 * This configuration property is used to enable One Step Serdes PHY TimeSync
 * One step is diabled or not specifiy, two step will be enabled.
 * This property may be configured with the base name to control all ports,
 * or with the suffix "_lport#" to configure a specific port.
 * This is only for Tomahawk3.
 */
...
/*
 * IPMC aging mode.
 * 0: (default): Application management mode. IPMC aging process triggered
 * by applications and bcm_ipmc_age can be used to clear hit or delete
 * entries.
 * 1: SDK management mode. IPMC aging triggered by SDK and
 * bcm_ipmc_age_timer_set can be used to set the interval
 * of aging thread.
 */
#define spn_IPMC_AGE_MODE  "ipmc_age_mode"
/*
 * Specifies the GPHY 100M Base Test Amplitude Value.
 * Used by BCM5627x devices for IEEE compliance settings init time.
 * gphy_100bt_amp_<port>=<new value>
 */
#define spn_GPHY_100BT_AMP  "gphy_100bt_amp"

/* Is OTN supported. */
#define spn_OTN_SUPPORTED  "otn_supported"
/*
 * Random seed for flow based RTAG7 hash shuffling.
 * 0 :(default) Use the random seed generated from timestamp.
 * Non Zero: Configure the given non zero value as random seed.
 */
#define spn_MACRO_FLOW_HASH_SHUFFLE_RANDOM_SEED  "macro_flow_hash_shuffle_random_seed"
/*
 * VLAN Control L3 Ingress Interface Update Disable.
 * Applicable only for devices having l3 iif less than 4K size.
 * Valid values are 0 and 1. Default:0
 * 0: L3_IIF will be updated on calling bcm_vlan_control_vlan_set.
 * 1: L3_IIF will not be updated on calling bcm_vlan_control_vlan_set.
 */
#define spn_VLAN_CTRL_UPDATES_L3_IIF_DISABLE  "vlan_ctrl_updates_l3_iif_disable"

/* When a bit is set, the equivalent lane (PHY) is used for the corresponding port. */
#define spn_OTN_LANES  "otn_lanes"
/*
 * Allocate the framer logical lanes for OTN interface.
 * -1: Auto-allocated the framer logical lanes by SW.
 * Positive: Specify the framer logical lanes by customers.
 */
#define spn_PORT_INIT_LOGICAL_OTN_LANE  "port_init_logical_otn_lane"
/*
 * Configure the device FlexE slot mode.
 * Valid values:
 * 1G_AND_5G: Indicate only 1G and 5G FlexE slots are supported on the device.
 * 1P25G_AND_5G: Indicate only 1.25G and 5G FlexE slots are supported on the device.
 */
#define spn_FLEXE_SLOT_MODE  "flexe_slot_mode"
/*
 * Specify if resource manager test instance should be init. Disabled in default
 * Valid values are 0 and 1. Default:0
 */
#define spn_RES_MNGR_TEST  "res_mngr_test"
/*
 * Specify if template manager test instance should be init. Disabled in default
 * Valid values are 0 and 1. Default:0
 */
#define spn_TEMP_MNGR_TEST  "temp_mngr_test"
/*
 * Specify if Sw State test instance should be init. Disabled in default
 * Valid values are 0 and 1. Default:0
 */
#define spn_SW_STATE_TEST  "sw_state_test"
/*
 * De-multiplex BFDoVxLAN session for zero Your-discriminator packets.
 * 0    --   Only VNID for de-multiplex BFD session.
 * 1    --   VNID + VTEP SIP both for de-multiplex BFD session.
 */
#define spn_BFD_OVER_VXLAN_ZERO_DISCR_DEMUX  "bfd_over_vxlan_zero_discr_demux"
/*
 * Allocate unique reassembly context to each RCY channel.
 * 0    --   All RCY channels share the same reassembly context.
 * 1    --   each RCY channel has a unique reassmbly context.
 */
#define spn_INGRESS_REASSEMBLY_UNIQUE_CONTEXT_RCY  "ingress_reassembly_unique_context_rcy"
/*
 * Control whether to enable hybrid PFC deadlock.
 * 0    --   Do not enable hybrid PFC deadlock. Only support HW based PFC deadlock (default).
 * 1    --   Enable hybrid PFC deadlock. Support both HW and SW based PFC deadlock.
 */
#define spn_HYBRID_PFC_DEADLOCK_ENABLE  "hybrid_pfc_deadlock_enable"
/*
 * Specify mode for flexible state transition in flowtracker.
 * 0    --   State transition table is pre-defined at SDK initialization. It cannot be modified using APIs after SDK initialization (default).
 * 1    --   State transition table is pre-defined at SDK initialization. It can be modified using APIs after SDK initialization.
 * 2    --   State transition table is not pre-defined at SDK initialization.
 */
#define spn_FLOWTRACKER_FLEX_STATE_TRANSITION_MODE  "flowtracker_flex_state_transition_mode"
/*
 * Specifies the record assembler credit values for export records originating from learning/timer/data based triggers.
 * In BCM5689x devices, total RA credit values sum cannot exceed max of 128.
 * If this config is not specified then total RA credit values will be divided equally as 43:43:42.
 * If there is unallocated credit then it will be assigned to learning by default.
 * flowtracker_collector_ra_credit = <no. of ra credit learning>:<no. of ra credit timer>:<no. of ra credit data>
 * For Example: flowtracker_collector_ra_credit = 43:42:42
 *              flowtracker_collector_ra_credit = :42:42
 *              flowtracker_collector_ra_credit = ::
 *              flowtracker_collector_ra_credit = ::42
 */
#define spn_FLOWTRACKER_COLLECTOR_RA_CREDIT  "flowtracker_collector_ra_credit"
/*
 * Disable xflow_macsec initialization.
 * 0    --   Enable xflow_macsec initialization (default).
 * 1    --   Disable xflow_macsec initialization.
 */
#define spn_XFLOW_MACSEC_DISABLE  "xflow_macsec_disable"
...
{ \
  ...
  "alpm_bkt_share_start_ipv4", \
  "alpm_bkt_share_start_ipv6", \
  "alpm_data_autosave", \
  "alpm_flex_stat_support", \
  "alpm_pre_filter", \
  "ancillary_bandwidth_mode", \
  "api_logger_base_indent", \
  "api_logger_cache_nfreeze", \
  "api_logger_cache_size", \
  "api_logger_enable", \
  "api_logger_logfile", \
  "api_logger_logfile_optype", \
  "api_logger_mode", \
  "api_logger_raw_logfile", \
  "api_logger_reset_on_tr141", \
  "api_logger_thread_prio", \
  "api_logger_thread_stksz", \
  "app_queues_num", \
  "app_queues_start", \
  "appl_enable", \
  "appl_param", \
  ...
  "bcm_l3_intf_mcast_nhi_auto_del", \
  "bcm_linkscan_errdelay", \
  ...
  "bfd_ipv6_trap_port", \
  "bfd_l3_iif_to_vrf_map_enable", \
  "bfd_mask_flags_bitfield", \
  "bfd_num_sessions", \
  "bfd_num_tx_log_buffers", \
  "bfd_over_vxlan_zero_discr_demux", \
  "bfd_remote_discriminator", \
  "bfd_remote_discriminator_reset", \
  ...
  "cmic_ingress_dqueue", \
  "cmic_ingress_squeue", \
  "cmicm_dcb_error_mitigate_enable", \
  "cmicm_pci_completion_timout_warm_boot_enable", \
  "cmicx_link_sbus_channel_id", \
  "cmicx_schan_num_cfg_count", \
  ...
  "counter_collection_preempt_enable", \
  "counter_collection_preempt_max_period_usec", \
  ...
  "eth_oam_max_ccm_sessions", \
  "eth_oam_max_groups", \
  "eth_oam_max_lm_sessions", \
  "eth_oam_max_rmeps", \
  "evb_enable", \
  ...
  "ext_ram_ca_termination", \
  "ext_ram_channel_swap_en", \
  "ext_ram_ck_termination", \
  ...
  "ext_ram_t_wtrtr", \
  "ext_ram_t_zqcs", \
  "ext_ram_temp_read_enable", \
  "ext_ram_total_size", \
  "ext_ram_type", \
  "ext_ram_wck_granularity", \
  "ext_ram_write_crc", \
  "ext_ram_write_dbi", \
  ...
  "ext_tcam_mode", \
  "ext_tcam_port_cross_connectivity", \
  "ext_tcam_pvt", \
  ...
  "ifa_max_vxlan_term_flows", \
  "ifa_rx_pkt_max_length", \
  "ifa_tailstamp_metadata_lns_id", \
  ...
  "ingress_max_pages", \
  "ingress_pool", \
  "ingress_reassembly_unique_context_rcy", \
  ...
  "l2_dma_cpu_learn_thread_priority", \
  "l2_entry_used_as_my_station", \
  "l2_flush_buffer_nof_entries", \
  ...
  "macro_flow_hash_shuffle_random_seed", \
  "macsec_dev_addr", \
  "macsec_enable", \
  "macsec_enabled", \
  ...
  "mmu_lossless_pbmp", \
  "mmu_lossless_ports_max", \
  "mmu_lossless_reserve_headroom_size", \
  ...
  "oamp_fifo_dma_event_thread_priority", \
  ...
  "oamp_mep_db_max_nof_11_len_entries", \
  "oamp_mep_db_max_nof_2_len_entries", \
  "oamp_mep_db_max_nof_3_len_entries", \
  "oamp_mep_db_max_nof_4_len_entries", \
  "oamp_mep_db_max_nof_5_len_entries", \
  "oamp_mep_db_max_nof_6_len_entries", \
  "oamp_mep_db_max_nof_7_len_entries", \
  "oamp_mep_db_max_nof_jitter_2_len_entries", \
  "oamp_mep_db_max_nof_jitter_3_len_entries", \
  "oamp_mep_db_max_nof_jitter_4_len_entries", \
  "oamp_mep_db_max_nof_jitter_5_len_entries", \
  ...
  "pll0_only", \
  "pll600_slowclk", \
  "pm_cable_swap_enable", \
  "pm_cable_swap_master", \
  ...
  "qgroup_id", \
  "qos_default_map", \
  "qos_map_dscp_nonsharable_mode", \
  "qos_map_egr_dscp_nonsharable_mode", \
  ...
  "sbusdma_flush_cache_enable", \
  "schan_error_block_usec", \
  "schan_intr_enable", \
  "schan_timeout_retry_enable", \
  "schan_timeout_usec", \
  ...
  "serdes_timesync_enable", \
  "serdes_timesync_one_step_enable", \
  "serdes_tx_los_usec", \
  "serdes_tx_taps", \
  "serdes_txpi_mode", \
  ...
  "timestamp_tsts_adjust_20gbe_ns", \
  ...
  "trunk_egress_nof_load_balancing_keys", \
  "trunk_extend", \
  ...
  "ucode_port", \
  "udp_tunnel_enable", \
  "up_mep_tod_recycle_port_core_0", \
  "up_mep_tod_recycle_port_core_1", \
  ...
  "vlan_auto_stack", \
  "vlan_ctrl_updates_l3_iif_disable", \
  "vlan_lookup_physical_database_mode", \
  "vlan_mac_add_xlate_table_select", \
  "vlan_match_criteria_mode", \
  "vlan_match_db_mode", \
  "vlan_queue_enable", \
  "vlan_queue_levels_max", \
  "vlan_tag_ctrl_on_egress", \
