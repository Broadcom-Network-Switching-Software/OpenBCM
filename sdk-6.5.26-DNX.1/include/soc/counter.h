...
#define _SOC_CONTROLLED_COUNTER_FLAG_RS544  0x200
...
typedef int (*soc_get_counter_length_f)(int unit, int counter_id, int port, int *length);
...
extern int soc_counter_port_collect_enable_set(int unit,
                                               int port, int enable);
#if (SRC_SOC_COMMON_ACTIVATE_UNUSED) || defined(BCM_ESW_SUPPORT)
extern int soc_counter_port_collect_enable_get(int unit,
                                               int port, int * enable);
...
extern int soc_controlled_counter_clear_by_counter_idx(int unit, soc_port_t port, int counter_idx);
extern int soc_controlled_counter_get_delta_by_counter_idx(int unit, soc_port_t port, int counter_id,uint64 *delta_cnt);
#if (SRC_SOC_COMMON_ACTIVATE_UNUSED) || defined(BCM_ESW_SUPPORT)
...
#if defined(BCM_XGS5_SWITCH_PORT_SUPPORT) || defined(BCM_DNX_SUPPORT)
extern int
...
extern int
soc_counter_port_dma_stat_get(int unit, soc_port_t port, int dma_offset, uint64 *val);
extern int
soc_counter_port_dma_stat_set(int unit, soc_port_t port, int dma_offset, uint64 val);
#endif
...
/**
 *
 * Returns the amount of microseconds since last iteration of counter sampling
 *
 * NOTE:
 * Should be called only from within counter thread execution context.
 * May be called from functions that were registered using soc_counter_extra_register.
 */
int soc_counter_actual_interval_get(int unit, int *interval);

/*
 * Function:
 *      soc_counter_collection_preempt_active
 * Purpose:
 *      Inform counter module about active counter collection preempt calls.
 * Parameters:
 *      unit - The SOC unit number.
 *      active - A preempt call is active (1) or inactive (0).
 * Returns:
 *      SOC_E_NONE
 */
int soc_counter_collection_preempt_active(int unit, int active);
