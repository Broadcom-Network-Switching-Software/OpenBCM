...
typedef enum
{
    ...
    SOC_DPP_DBAL_SW_TABLE_ID_PPPOE_SESSION_ID_ANTI_SPOOFING,
...
uint32 arad_pp_dbal_entry_key_to_kaps_buffer(int unit, SOC_DPP_DBAL_TABLE_INFO *dbal_table, uint32 table_size_in_bytes, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], uint32 *prefix_len, uint8 *data_bytes);
#endif
...
#if defined(BCM_88675_A0)
uint32 arad_pp_dbal_kaps_buffer_to_entry_key(int unit, const SOC_DPP_DBAL_TABLE_INFO *dbal_table, uint32 prefix_len, uint8 *data_bytes,
                                            SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX]);
#endif
