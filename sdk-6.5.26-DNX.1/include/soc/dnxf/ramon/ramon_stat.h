...
typedef enum soc_ramon_counters_e
{
    ...
    soc_ramon_counters_rx_rs_fec_bit_error_rate = 23
} soc_ramon_counters_t;

shr_error_e soc_ramon_signal_quality_bit_error_rate_clear_dummy(
    int unit,
    soc_port_t port,
    int counter_idx);

shr_error_e soc_ramon_signal_quality_bit_error_rate_get(
    int unit,
    int counter_id,
    int port,
    uint64 *val,
...
shr_error_e soc_ramon_controlled_counter_set(
    int unit);

shr_error_e soc_ramon_stat_fmac_init(
    int unit);

shr_error_e soc_ramon_mapping_stat_get(
    int unit,
    bcm_port_t port,
    int *counters,
    int *array_size,
...
shr_error_e soc_ramon_stat_counter_length_get(
    int unit,
    int counter_id,
    int *length);

shr_error_e soc_ramon_stat_is_supported_type(
    int unit,
    bcm_port_t port,
    bcm_stat_val_t type);

shr_error_e soc_ramon_fabric_stat_get(
    int unit,
    bcm_fabric_stat_index_t index,
    bcm_fabric_stat_t stat,
    uint64 *value);

int soc_ramon_controlled_counter_get(
    int unit,
    int counter_id,
    int port,
    uint64 *val,
