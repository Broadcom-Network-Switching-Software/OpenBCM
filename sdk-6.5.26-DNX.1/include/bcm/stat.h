...
typedef enum bcm_stat_val_e {
    ...
    snmpBcmRxCodeErrors = 125,          /* Fabric RX 8b/10b code errors.
                                           (Broadcom-specific) */
    snmpBcmRxRsFecBitError = 126,       /* RX RS-FEC bit error counter.
                                           (Broadcom-specific) */
    ...
    snmpBcmRxControlOverflowDrops = 290, /* RX control overflow drop
                                           counter(Broadcom-specific) */
    snmpBcmRxCorruptedTranscodedWordInFrame = 291, /* RX corrupted transcoded words in
                                           frame counter(Broadcom-specific) */
    snmpBcmSignalQualityLastErrorRate = 292, /* Signal Quality error rate as
...
{ \
    ...
    "snmpBcmRxControlOverflowDrops", \
    "snmpBcmRxCorruptedTranscodedWordInFrame", \
    "snmpBcmSignalQualityLastErrorRate"  \
}
...
typedef enum bcm_custom_stat_trigger_e {
    ...
    bcmDbgCntRxEcmpProtectionMemberCollisionDrop = 213, /* Rx: Enable to count packets dropped
                                           due to ECMP member table bank
                                           collision is detected while resolving
                                           primary and secondary next hops. */
    bcmDbgCntRxIntMtuError = 214,       /* Rx: Enable to count In-band Network
                                           Telemetry (INT) packets experiencing
                                           L3 MTU check failure due to metadata
                                           insertion. */
    bcmDbgCntRxEcmpMemberNotResolvedDrop = 215, /* Rx: Enable to count packets dropped
                                           due to primary, secondary and
                                           tertiary next hop selections are down
                                           or programmatically disabled and a
                                           valid next hop could not be resolved. */
    bcmDbgCntRxNonFirstPass = 216,      /* Rx: Enable to count packets marked as
                                           non-first pass packets. */
    bcmDbgCntRxEcmpProtectionMemberActive = 217, /* Rx: Enable to count packets due to
                                           secondary or tertiary next hop is
                                           picked during next hop resolution
                                           because primary/secondary next hop is
                                           either down or programmatically
                                           disabled. */
    bcmDbgCntRxStatefulModSecondPassDrop = 218, /* Rx: Enable to count switch copies
                                           dropped due to 2nd pass processing of
                                           stateful MOD. */
    bcmDbgCntRxHigig3Drop = 219,        /* Rx: Enable to count packets dropped
                                           due to the drop configuration on
                                           HIGIG3 packets. */
    bcmDbgCntRxNonHigig3Drop = 220,     /* Rx: Enable to count packets dropped
                                           due to the drop configuration on
                                           non-HIGIG3 packets. */
    bcmDbgCntRxL2TunnelSvpAssignFailDrop = 221, /* Rx: Enable to indicate packets
                                           dropped due to source virtual port
                                           assignment failure. Applicable for
                                           VxLAN and SRv6. */
    bcmDbgCntRxSrv6ProcessErrorDrop = 222, /* Rx: Enable to count packets dropped
                                           due to SRV6 process error. */
    bcmDbgCntRxSrv6ValidationErrorDrop = 223, /* Rx: Enable to count packets dropped
                                           due to SRV6 validation error. */
    bcmDbgCntRxSrv6UnknownRoutingTypeWithNonZeroSlDrop = 224, /* Rx: Enable to count packets dropped
                                           due to unknown routing type in SRV6
                                           packets with nonzero segments left
                                           field. */
    bcmDbgCntTxDcnPktRoutingPlaneMapDrop = 225, /* Rx: Enable to count drop congestion
                                           notification packets dropped due to
                                           routing plane mapping. */
    bcmDbgCntTxIfaActionNotApplied = 226, /* Rx: Enable to count packets IFA
                                           action not applied. */
    bcmDbgCntTxNonFirstPass = 227,      /* Rx: Enable to count packets marked as
                                           non-first pass packets. */
    bcmDbgCntTxEpRecircDrop = 228,      /* Rx: Enable to count packets dropped
                                           due to EP recirculation drop. */
    bcmDbgCntTxIfaTermMirrorCopySuppress = 229, /* Rx: Enable to count IFA terminated
                                           packets experiencing mirror copy
                                           suppressing. */
    bcmDbgCntTxEpRecircSupress = 230,   /* Rx: Enable to count packets
                                           experiencing EP recirculation
                                           suppressing. */
    bcmDbgCntRxSvtagLookupMiss = 231,   /* Rx: Packets dropped due to SVTAG
                                           lookup miss. */
    bcmDbgCntNum = 232                  /* Must be last one. */
...
/* Get the rate of a specified statistic from the device. */
extern int bcm_stat_rate_get(
    int unit, 
    bcm_port_t port, 
    bcm_stat_val_t type, 
    uint64 *rate);
...
/* Get the rate for multiple statistics of the device. */
extern int bcm_stat_rate_multi_get(
    int unit, 
    bcm_port_t port, 
    int nstat, 
    bcm_stat_val_t *stat_arr, 
...
#define BCM_STAT_ID_DIRECT      0xFFFFFFFF /*  Use direct stat ID */
...
typedef enum bcm_stat_counter_format_type_e {
    ...
    bcmStatCounterFormatSlimPacketsAndBytes = 6, /* Slim dual counter format and add one
                                           action */
    bcmStatCounterFormatDoubleBytes = 7, /* Single narrow counter, counts bytes */
    bcmStatCounterFormatSlimPackets = 8, /* Single Slim counter, counts packets */
    bcmStatCounterFormatTimestamp = 9,  /* Single Wide counter holds the 48bits
                                           Time of day (TOD) from the ECI */
    bcmStatCounterFormatNarrowState = 10, 
    bcmStatCounterFormatWideState = 11, 
    bcmStatCounterFormatCount = 12 
} bcm_stat_counter_format_type_t;

/* Defines the action that occurs on the counters. */
typedef enum bcm_stat_counter_mode_e {
    bcmStatCounterModeNone = 0,         /* Use the default mode for each counter
                                           format */
    bcmStatCounterModeIncrement = 1,    /* Add one to the counter */
    bcmStatCounterModeAdd = 2,          /* Add data to the counter */
    bcmStatCounterModeOpcode = 3,       /* Use a dynamic opcode */
    bcmStatCounterModeCount = 4         /* Must be the last one */
...
typedef struct bcm_stat_counter_value_s {
    ...
    uint64 last_period_change;  /* The difference in the counter value in the
                                   previous sequential scan iteration */
    uint8 sequence_number;      /* sampel sequence noumber */
...
typedef enum bcm_stat_counter_database_control_e {
    ...
    bcmStatCounterOverflowMode = 1,     /* Allow the user to choose the counter
                                           overflow mode */
    bcmStatCounterDatabaseSequentialSkipZerosEnable = 2, /* Enable/disable sequential scan skip
                                           zero counters */
    bcmStatCounterDatabaseSequentialSamplingZeroInterval = 3, /*  Setting the sequential sampling
                                           Interval after zero counter */
    bcmStatCounterDatabaseSequentialIterationInterval = 4, /* Setting the sequential iteration
                                           Interval */
    bcmStatCounterDatabaseRateCalculationEnable = 5, /* Enable/disable counters rate
                                           calculation */
    bcmStatCounterDatabaseCount = 6     /* Must be the last one */
...
#define BCM_STAT_MAX_NUMBER_OF_COUNTER_SETS_PER_DB 2          /* Defines the number of
                                                          different sets allowed
                                                          per crps database */
...
typedef enum bcm_stat_counter_interface_type_e {
    ...
    bcmStatCounterInterfaceEgressPp = 9, 
    bcmStatCounterInterfaceIngressPMF = 10, 
    bcmStatCounterInterfaceEgressPMF = 11, 
    bcmStatCounterInterfaceCount = 12 
} bcm_stat_counter_interface_type_t;

/* Set of counter engine parameters per set. */
typedef struct bcm_stat_counter_set_config_s {
    uint32 start;       /* The first entry address (offset in the database)
                           belongs to the counter set. */
    uint32 end;         /* The last entry address (offset in the database)
                           belongs to the counter set. */
    uint32 set_size;    /* Counter set size */
} bcm_stat_counter_set_config_t;
...
/* Defines the counter interface configuration */
typedef struct bcm_stat_counter_interface_s {
    ...
    bcm_stat_counter_set_config_t set_config[BCM_STAT_MAX_NUMBER_OF_COUNTER_SETS_PER_DB]; /* The size of the counter set. */
    uint32 counter_size;                /* The size of the counter set. */
    bcm_stat_counter_mode_t mode;       /* The action made on the counter. */
...
#define BCM_STAT_COUNTER_END_TO_END_LATENCY 0x00000002 /* If flag set, driver
                                                          ...
                                                          select packet size. */
#define BCM_STAT_COUNTER_MULTI_SETS_DATABASE 0x00000004 /* Allow to config
                                                          database with two
                                                          different counters
                                                          sets */
...
typedef enum bcm_stat_expansion_types_e {
    ...
    bcmStatExpansionTypeVoqQueueSizeBin = 22, 
    bcmStatExpansionTypeTrafficClassInput = 23, 
    bcmStatExpansionTypeIsUnscheduled = 24, /* Is ERP */
    bcmStatExpansionTypePpPortMetaData = 25, 
    bcmStatExpansionTypeTmPortMetaData = 26, 
    bcmStatExpansionTypeAqmProfile = 27, 
    bcmStatExpansionTypePortRateBin = 29, 
    bcmStatExpansionTypeTrafficClassMapProfile = 30, 
    bcmStatExpansionTypeGlobalVoqAgg = 31, 
    bcmStatExpansionTypeMax = 32 
...
typedef struct bcm_stat_expansion_data_value_s {
    ...
    int set_id;             /* The index of the set the expansion data is
                               related to. */
...
typedef enum bcm_eviction_destination_type_e {
    ...
    bcmStatEvictionDestinationCrpsXl = 2, /* Configure the counters eviction
                                           destination to be crps XL */
    bcmStatEvictionDestinationSize = 3  /* enum size */
...
typedef enum bcm_stat_counter_group_filter_e {
    ...
    bcmStatCounterGroupFilterDropPrecedenceGroup = 8, 
    bcmStatCounterGroupFilterPpGroup = 9, 
    bcmStatCounterGroupFilterDramGroup = 10, 
    bcmStatCounterGroupFilterPruningGroup = 11, 
    bcmStatCounterGroupFilterCount = 12 
...
typedef enum bcm_stat_fec_profile_e {
    bcmStatFecProfileSingleIndirect = 0, /* FEC statistic profile with single
                                           indirect counters */
    bcmStatFecProfileTwoIndirect = 1,   /* FEC statistic profile with two
                                           indirect counters */
    bcmStatFecProfileDirect = 2         /* EC statistic profile with direct
                                           counters */
} bcm_stat_fec_profile_t;
...
typedef struct bcm_stat_pp_profile_info_s {
    ...
    bcm_stat_fec_profile_t fec_profile; /* Statistic profile for FEC entrie */
