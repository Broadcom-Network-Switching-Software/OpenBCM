...
#define BCM_SUBPORT_GROUP_WITH_ID           0x00000001 /* Add subport group
                                                          using the specified
                                                          subport group index
                                                          which is stored in the
                                                          LSB
                                                          9 bits in the subport
                                                          group gport (refer
                                                          group parameter in API
...
#define BCM_SUBPORT_GROUP_TYPE_GENERAL      0x00000020 /* Add a general purpose
                                                          subport group. */
#define BCM_SUBPORT_GROUP_TYPE_SUBPORT_TAG_VER_3 0x00000040 /* Add a subport tag type
                                                          3 subport group. */
...
/* 
 * Create a subport group. After subport group is created with this
 * API, all the subsequent APIs referring to the subport group gport
 * should use gport even for case where group config.flags is set
 * to BCM_SUBPORT_GROUP_WITH_ID.
 */
...
typedef struct bcm_subport_config_s {
    ...
    bcm_gport_t subport_modport;        /* CoE - System port associated with the
                                           subport tag */
    int svtag_signature;                /* CES - SVTAG signature field in the
                                           SVTAG header. */
    int svtag_payload_offset;           /* CES - Payload offset field in the
                                           SVTAG header. */
    int svtag_pri;                      /* CES - Priority field in the SVTAG
                                           header. */
    int svtag_devid;                    /* CES - Device ID field in the SVTAG
                                           header. */
    int svtag_pkt_type;                 /* CES - Packet type field in the SVTAG
                                           header. */
    int svtag_ipv6;                     /* CES - IPv6 field in the SVTAG type 3
                                           header. */
} bcm_subport_config_t;
...
/* Flags for subport ID map */
#define BCM_SUBPORT_ID_MAP_FLAG_SUBPORT_ID_VALID (1 << 0)   /* Subport ID field is
                                                          valid. */
#define BCM_SUBPORT_ID_MAP_FLAG_NAME_SPACE_VALID (1 << 1)   /* Name space field is
                                                          valid. */
#define BCM_SUBPORT_ID_MAP_FLAG_DEST_PORT_VALID (1 << 2)   /* Destination port field
                                                          is valid. */
#define BCM_SUBPORT_ID_MAP_FLAG_CLASS_ID_VALID (1 << 3)   /* Class ID field is
                                                          valid. */

/* bcm_subport_id_map_entry_t */
typedef struct bcm_subport_id_map_entry_s {
    uint32 flags;           /* Refer to BCM_SUBPORT_ID_MAP_FLAG_xxx. */
    int subport_id;         /* Subport ID. Key field. */
    int name_space;         /* Name space of the port. Key field. */
    bcm_port_t dest_port;   /* Destination port. */
    uint32 class_id;        /* Class ID used for FP. */
} bcm_subport_id_map_entry_t;

/* Initialize a subport ID map entry structure. */
extern void bcm_subport_id_map_entry_t_init(
    bcm_subport_id_map_entry_t *subport_entry);

#ifndef BCM_HIDE_DISPATCHABLE

/* 
 * Add an entry to the subport ID map.
 * 
 * This function is to add an entry to the subport ID map. The subport ID
 * map entry is indexed by key fields. The key fields and value fields
 * that are applicable for the device must be completely specified (along
 * with the valid flags set) when adding an entry.
 */
extern int bcm_subport_id_map_add(
    int unit, 
    bcm_subport_id_map_entry_t *subport_entry);

/* 
 * Delete an entry to subport ID map.
 * 
 * This function is to delete an entry from the subport ID map based on
 * the key fields. All the key fields applicable for the device must be
 * specified (with the valid flags set) for lookup. Only key fields are
 * required in the entry structure.
 */
extern int bcm_subport_id_map_delete(
    int unit, 
    bcm_subport_id_map_entry_t *subport_entry);

/* 
 * Get an entry from the subport ID map.
 * 
 * This function is to retrieve an entry from the subport ID map based on
 * the key fields. All the key fields applicable for the device must be
 * specified (with the valid flags set) for lookup. Only key fields are
 * required in the entry structure.
 */
extern int bcm_subport_id_map_get(
    int unit, 
    bcm_subport_id_map_entry_t *subport_entry);

#endif /* BCM_HIDE_DISPATCHABLE */

/* bcm_subport_id_map_traverse_cb */
typedef int (*bcm_subport_id_map_traverse_cb)(
    int unit, 
    bcm_subport_id_map_entry_t *subport_entry, 
    void *user_data);
 ...
 * function for each one.
 */
extern int bcm_subport_id_map_traverse(
    int unit, 
    bcm_subport_id_map_traverse_cb cb, 
    void *user_data);
