...
#define BCM_TUNNEL_TERM_GENERALIZED_SEGMENT_ID_END_OF_COMPRESSION 0x00000080 /* Indicate that this
                                                          GSID is End of
                                                          Compression */
...
#define BCM_TUNNEL_INIT_ERSPAN_WITH_SN      BCM_TUNNEL_INIT_GRE_WITH_SN /* Enable sequence number
                                                          ...
                                                          ERSPAN tunnel. */
#define BCM_TUNNEL_TERM_MASK                BCM_TUNNEL_INIT_IPV4_SET_DF /* Tunnel termination by
                                                          mask. */
#define BCM_TUNNEL_INIT_DISABLE_QOS_REMARK  BCM_TUNNEL_TERM_WLAN_REMOTE_TERMINATE /* Disable egress QOS
                                                          remark on the tunnel
                                                          initiator. */
#define BCM_TUNNEL_TERM_CASCADED_MISS_DISABLE_TERM BCM_TUNNEL_TERM_ECN_MAP /* In case of Cascaded
                                                          miss, disable
                                                          termination */
#define BCM_TUNNEL_TERM_USE_TUNNEL_TRAFFIC_CLASS BCM_TUNNEL_INIT_WLAN_TUNNEL_WITH_ID /* Pick traffic class
                                                          either from payload IP
                                                          header or tunnel IP
                                                          header. */
#define BCM_TUNNEL_INIT_SRV6_SCALE_VPN_SID  BCM_TUNNEL_TERM_UDP_CHECKSUM_ENABLE /* In case set, scale the
                                                          number of VPN SID. */
#define BCM_TUNNEL_TERM_L2_PHB              BCM_TUNNEL_TERM_DSCP_TRUST /* Tunnel termination PHB
                                                          valid for L2. */
#define BCM_TUNNEL_TERM_L3_PHB              BCM_TUNNEL_TERM_USE_TUNNEL_CLASS /* Tunnel termination PHB
                                                          valid for L3. */
...
typedef enum bcm_tunnel_type_e {
    ...
    bcmTunnelTypeVxlanEsp = 51,         /* VXLAN ESP Tunnel */
    bcmTunnelTypeEthCascadedFunct = 52, /* Ethernet Cascaded FUNCT */
    bcmTunnelTypeL2Gre6 = 53,           /* L2-GRE over v6 tunnel */
    bcmTunnelTypeL2Ip4 = 54,            /* L2-Tunnel over v4 */
    bcmTunnelTypeL2Ip6 = 55,            /* L2-Tunnel over v6 */
    bcmTunnelTypeGeneve = 56,           /* Geneve over IPv4 */
    bcmTunnelTypeGeneve6 = 57,          /* Geneve over IPv6 */
    bcmTunnelTypeCount = 58             /* Must be last */
} bcm_tunnel_type_t;
...
typedef enum bcm_tunnel_pri_select_e {
    bcmTunnelPriNone = 0,       /* No copy action. */
    bcmTunnelPriPayload = 1,    /* Copy payload priority and CFI to outer
                                   header. */
    bcmTunnelPriCount = 2       /* Unused always last. */
} bcm_tunnel_pri_select_t;

/* bcm_tunnel_dscp_ecn_select_e */
typedef enum bcm_tunnel_dscp_ecn_select_e {
    bcmTunnelDscpEcnAssign = 0,         /* Set outer IP header DSCP and ECN to
                                           tunnel initiator DSCP and ECN value. */
    bcmTunnelDscpEcnMap = 1,            /* Use DSCP and ECN value from DSCP and
                                           ECN map. */
    bcmTunnelDscpEcnPayloadDscp = 2,    /* Copy payload DSCP to tunnel outer
                                           header. */
    bcmTunnelDscpEcnCount = 3           /* Unused always last. */
...
typedef struct bcm_tunnel_initiator_s {
    ...
    uint8 counter_command_id;           /* Counter interface associated with the
                                           tunnel. */
    int estimated_encap_size;           /* Esitimated encapsulation size
                                           expected by this object including the
                                           next interfaces that are pointed from
                                           it. */
    uint32 aux_data2;                   /* High 32b for tunnel associated data
                                           in case it has more than 32 bits. */
...
/* 
 * Attach counter entries to the IP tunnel initiator associated with a
 * given L3 interface.
 */
extern int bcm_tunnel_initiator_stat_attach(
    int unit, 
    bcm_if_t intf_id, 
    uint32 stat_counter_id);

/* 
 * Detach counter entries from the IP tunnel initiator associated with a
 * given L3 interface.
 */
extern int bcm_tunnel_initiator_stat_detach(
    int unit, 
    bcm_if_t intf_id);

/* 
 * Get a stat counter ID on the IP tunnel initiator associated with a
 * given L3 interface.
 */
extern int bcm_tunnel_initiator_stat_id_get(
    int unit, 
    bcm_if_t intf_id, 
    uint32 *stat_counter_id);

/* 
 * Get the flex counter object value associated to IP tunnel initiator on
 * a given L3 interface.
 */
extern int bcm_tunnel_initiator_flexctr_object_get(
    int unit, 
    bcm_if_t intf, 
    uint32 *value);
...
/* Attach counter entries to a given tunnel terminator. */
extern int bcm_tunnel_terminator_stat_attach(
    int unit, 
    bcm_tunnel_terminator_t *terminator, 
    uint32 stat_counter_id);

/* Detach counter entries from a given tunnel terminator. */
extern int bcm_tunnel_terminator_stat_detach(
    int unit, 
    bcm_tunnel_terminator_t *terminator);

/* Get a stat counter ID on a given tunnel terminator. */
extern int bcm_tunnel_terminator_stat_id_get(
    int unit, 
    bcm_tunnel_terminator_t *terminator, 
    uint32 *stat_counter_id);

/* Set the flex counter object value on a given tunnel terminator. */
extern int bcm_tunnel_terminator_flexctr_object_set(
    int unit, 
    bcm_tunnel_terminator_t *terminator, 
    uint32 value);

/* Get the flex counter object value on a given tunnel terminator. */
