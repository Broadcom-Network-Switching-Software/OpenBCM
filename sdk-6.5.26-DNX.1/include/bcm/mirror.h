...
#define BCM_MIRROR_DEST_FLAGS2_OVERWRITE    (1U << 12) /* Overwrite a mirror
                                                          ...
                                                          re-attach to the new
                                                          mirror destination. */
#define BCM_MIRROR_DEST_FLAGS2_STATEFUL_MOD (1U << 13) /* Stateful mirror on
                                                          drop. */
...
typedef enum bcm_mirror_ingress_mod_event_e {
    ...
    bcmMirrorIngModEventCount = 169     /* Must be last. Not a valid value. */
...
typedef struct bcm_mirror_ingress_mod_event_profile_s {
    uint8 reason_code;                  /* Configurable reason code for ingress
                                           mirror-on-drop packets. */
    uint32 sample_rate;                 /* Sampler threshold. Sample the mirror
                                           packets if the generated sFlow random
                                           number is less than or equal to the
                                           threshold. Set 0 to disable sampler. */
    uint32 priority;                    /* Profile priority. Higher value means
                                           higher priority. When multiple
                                           profiles are hit simultaneously, the
                                           profile with the highest priority
                                           will be chosen to mirror. */
    uint8 uncompressed_drop_reason_enable; /* Enable uncompressed drop reasons to
                                           be carried in mirror on drop
                                           encapsulation. */
