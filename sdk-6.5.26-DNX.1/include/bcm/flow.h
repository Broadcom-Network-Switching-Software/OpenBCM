...
#define BCM_FLOW_MATCH_FLAG_SRV6_LE_CHECK_FAIL (1 << 2)   /* Match SRv6 last entry
                                                          check fail condition. */
#define BCM_FLOW_MATCH_FLAG_SRV6_SL_CHECK_FAIL (1 << 3)   /* Match SRv6 segment
                                                          left check fail
                                                          condition. */
#define BCM_FLOW_MATCH_FLAG_SRV6_ROUTING_TYPE_ERROR (1 << 4)   /* Match SRv6 SRH routing
                                                          type error condition. */
#define BCM_FLOW_MATCH_FLAG_SRV6_SRH_FLAG_ERROR (1 << 5)   /* Match SRv6 SRH flag
                                                          error condition. */
#define BCM_FLOW_MATCH_FLAG_CROSS_CONNECT   (1 << 6)   /* Indicate
                                                          point-to-point
                                                          service. */
#define BCM_FLOW_MATCH_FLAG_USE_OUTER_TTL   (1 << 7)   /* Use outer header's
                                                          TTL. */
#define BCM_FLOW_MATCH_FLAG_USE_OUTER_PHB   (1 << 8)   /*  Use outer header to
...
/* bcm_flow_es_id_shift_t for ethernet segment ID. */
typedef enum bcm_flow_es_id_shift_e {
    bcmFlowEsIdShift0Bit = 0,   /* 0bits. */
    bcmFlowEsIdShift4Bit = 1,   /* 4bits. */
    bcmFlowEsIdShift8Bit = 2,   /* 8bits. */
    bcmFlowEsIdShift12Bit = 3,  /* 12bits. */
    bcmFlowEsIdShift16Bit = 4,  /* 16bits. */
    bcmFlowEsIdShift20Bit = 5,  /* 20bits. */
    bcmFlowEsIdShift24Bit = 6,  /* 24bits. */
    bcmFlowEsIdShift28Bit = 7,  /* 28bits. */
    bcmFlowEsIdShift32Bit = 8,  /* 32bits. */
    bcmFlowEsIdShift36Bit = 9,  /* 36bits. */
    bcmFlowEsIdShift40Bit = 10, /* 40bits */
    bcmFlowEsIdShift44Bit = 11, /* 44bits. */
    bcmFlowEsIdShift48Bit = 12, /* 48bits. */
    bcmFlowEsIdShiftCount = 13  /* Unused always last. */
} bcm_flow_es_id_shift_t;
...
#define BCM_FLOW_TUNNEL_TERM_FLAGS2_SRV6_USD (1 << 0)   /* SRv6 Ultimate Segment
                                                          Decapsulation action. */
#define BCM_FLOW_TUNNEL_TERM_FLAGS2_DROP    (1 << 1)   /* Drop the packet. */
#define BCM_FLOW_TUNNEL_TERM_FLAGS2_COPY_TO_CPU (1 << 2)   /* Copy to cpu. */
#define BCM_FLOW_TUNNEL_TERM_FLAGS2_SRV6_LE_CHECK_FAIL (1 << 3)   /* Match SRv6 last entry
                                                          check fail condition. */
#define BCM_FLOW_TUNNEL_TERM_FLAGS2_SRV6_SL_CHECK_FAIL (1 << 4)   /* Match SRv6 segment
                                                          left check fail
                                                          condition. */
#define BCM_FLOW_TUNNEL_TERM_FLAGS2_SRV6_ROUTING_TYPE_ERROR (1 << 5)   /* Match SRv6 SRH routing
                                                          type error condition. */
#define BCM_FLOW_TUNNEL_TERM_FLAGS2_SRV6_SRH_FLAG_ERROR (1 << 6)   /* Match SRv6 SRH flag
                                                          error condition. */
#endif
...
typedef struct bcm_flow_port_s {
    ...
    uint64 es_id_mask;                  /* Destination IP mask to get ethernet
                                           segment ID. */
    bcm_flow_es_id_shift_t es_id_right_shift; /* Right shift bits to get ethernet
                                           segment ID. */
} bcm_flow_port_t;
