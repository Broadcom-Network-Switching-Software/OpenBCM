...
#define BCM_POLICER_WITH_SHARED_POOL_ID     0x00400000 /* Indicates that this
                                                          ...
                                                          given shared meter
                                                          pool id. */
#define BCM_POLICER_L2L3                    0x00800000 /* Indicates that this
                                                          policer will be used
                                                          by L2 or L3 tables. */
#define BCM_POLICER_WITH_METER_PAIR_INDEX   0x01000000 /* Indicates that this
                                                          policer will be
                                                          installed at the
                                                          specified
                                                          meter_pair_index. */
...
/* Packet colors. */
#define BCM_POLICER_COLOR_PRESERVE  0          
#define BCM_POLICER_COLOR_GREEN     1          
#define BCM_POLICER_COLOR_YELLOW    2          
#define BCM_POLICER_COLOR_RED       3          
...
typedef enum bcm_policer_action_e {
    ...
    bcmPolicerActionRpIntCongestionNotificationNew = 20, /* Replace the internal congestion
                                           ...
                                           param0: Internal congestion
                                           notification value. */
    bcmPolicerActionGpDropPrecedence = 21, /* Set Green Priority Drop Precedence;
                                           param0: BCM_POLICER_COLOR_xxx. */
    bcmPolicerActionYpDropPrecedence = 22, /* Set Yellow Priority Drop Precedence;
                                           param0: BCM_POLICER_COLOR_xxx. */
    bcmPolicerActionRpDropPrecedence = 23, /* Set Red Priority Drop Precedence;
                                           param0: BCM_POLICER_COLOR_xxx. */
    bcmPolicerActionGpCopyToCpu = 24,   /* Green Priority Copy to CPU. */
    bcmPolicerActionYpCopyToCpu = 25,   /* Yellow Priority Copy to CPU. */
    bcmPolicerActionRpCopyToCpu = 26,   /* Red Priority Copy to CPU. */
    bcmPolicerActionGpCopyToCpuCancel = 27, /* Override another rule. */
    bcmPolicerActionYpCopyToCpuCancel = 28, /* Override another rule. */
    bcmPolicerActionRpCopyToCpuCancel = 29, /* Override another rule. */
    bcmPolicerActionGpDropCancel = 30,  /* Override drop rule for green packets. */
    bcmPolicerActionYpDropCancel = 31,  /* Override drop rule for yellow
                                           packets. */
    bcmPolicerActionRpDropCancel = 32,  /* Override drop rule for red packets. */
    bcmPolicerActionGpOpaqueCtrlId = 33, /* Assign Opaque control ID for green
                                           colored packets */
    bcmPolicerActionYpOpaqueCtrlId = 34, /* Assign Opaque control ID for yellow
                                           colored packets */
    bcmPolicerActionRpOpaqueCtrlId = 35, /* Assign Opaque control ID for red
                                           colored packets */
    bcmPolicerActionGpPrioPktNew = 36,  /* Green packet priority from argument0;
                                           param0: New priority. */
    bcmPolicerActionYpPrioPktNew = 37,  /* Yellow packet priority from
                                           argument0; param0: New priority. */
    bcmPolicerActionRpPrioPktNew = 38,  /* Red packet priority from argument0;
                                           param0: New priority. */
    bcmPolicerActionGpOuterVlanCfiNew = 39, /* Green packet 802.1p CFI from arg;
                                           param0: New CFI bit value. */
    bcmPolicerActionYpOuterVlanCfiNew = 40, /* Yellow packet 802.1p CFI from arg;
                                           param0: New CFI bit value. */
    bcmPolicerActionRpOuterVlanCfiNew = 41, /* Red packet 802.1p CFI from arg;
                                           param0: New CFI bit value. */
    bcmPolicerActionGpCosMapNew = 42,   /* Override the default internal
                                           priority to cosq mapping derived in
                                           ingress pipeline using new IFP Cos
                                           Map for Green Packets only. */
    bcmPolicerActionYpCosMapNew = 43,   /* Override the default internal
                                           priority to cosq mapping derived in
                                           ingress pipeline using new IFP Cos
                                           Map for Yellow Packets only. */
    bcmPolicerActionRpCosMapNew = 44,   /* Override the default internal
                                           priority to cosq mapping derived in
                                           ingress pipeline using new IFP Cos
                                           Map for Red Packets only. */
    bcmPolicerActionGpFlexCtrOpaqueObj1 = 45, /* Policer opaque 1 flex counter index
                                           for Green packets. */
    bcmPolicerActionYpFlexCtrOpaqueObj1 = 46, /* Policer opaque 1 flex counter index
                                           for Yellow packets. */
    bcmPolicerActionRpFlexCtrOpaqueObj1 = 47, /* Policer opaque 1 flex counter index
                                           for Red packets. */
    bcmPolicerActionGpFlexCtrOpaqueObj2 = 48, /* Policer opaque 2 flex counter index
                                           for Green packets. */
    bcmPolicerActionYpFlexCtrOpaqueObj2 = 49, /* Policer opaque 2 flex counter index
                                           for Yellow packets. */
    bcmPolicerActionRpFlexCtrOpaqueObj2 = 50, /* Policer opaque 2 flex counter index
                                           for Red packets. */
    bcmPolicerActionGpFlexCtrOpaqueObj3 = 51, /* Policer opaque 3 flex counter index
                                           for Green packets. */
    bcmPolicerActionYpFlexCtrOpaqueObj3 = 52, /* Policer opaque 3 flex counter index
                                           for Yellow packets. */
    bcmPolicerActionRpFlexCtrOpaqueObj3 = 53, /* Policer opaque 3 flex counter index
                                           for Red packets. */
    bcmPolicerActionCount = 54          /* Last value. Not to be used */
} bcm_policer_action_t;

#define BCM_POLICER_ACTION_STRINGS \
{ \
    "GpDrop", \
    "GpDscpNew", \
    "GpEcnNew", \
...
/* Policer action pool number. */
typedef enum bcm_policer_action_pool_num_e {
    bcmPolicerActionPool0 = 0,      /* Policer action pool 0. */
    bcmPolicerActionPool1 = 1,      /* Policer action pool 1. */
    bcmPolicerActionPool2 = 2,      /* Policer action pool 2. */
    bcmPolicerActionPool3 = 3,      /* Policer action pool 3. */
    bcmPolicerActionPoolAny = 4,    /* Any Policer action pool. */
    bcmPolicerActionPoolCount = 5   /* Always Last. Not a usable value. */
} bcm_policer_action_pool_num_t;

#define BCM_POLICER_ACTION_POOL_NUM_STRINGS \
{ \
    "0", \
    "1", \
    "2", \
...
/* Set the packet length adjustment profile. */
extern int bcm_policer_pkt_len_adjust_profile_set(
    int unit, 
    uint32 profile_id, 
    uint32 flags, 
    uint32 bytes);

/* Get the packet length adjustment bytes. */
extern int bcm_policer_pkt_len_adjust_profile_get(
    int unit, 
    uint32 profile_id, 
    uint32 *flags, 
    uint32 *bytes);

/* Delete the packet length adjustment profile. */
extern int bcm_policer_pkt_len_adjust_profile_delete(
    int unit, 
    uint32 profile_id, 
    uint32 flags);
...
#define BCM_POLICER_ACTION_INGRESS  0x00000001 
#define BCM_POLICER_ACTION_EGRESS   0x00000002 
#define BCM_POLICER_ACTION_L2L3     0x00000004 

typedef struct bcm_policer_action_config_s {
    int priority;                       /* Policer action priority. */
    uint32 flags;                       /* BCM_POLICER_ACTION_* Flags. */
    bcm_policer_t policer_id;           /* Policer id. */
    bcm_policer_action_pool_num_t action_pool; /* Action pool. */
    bcm_policer_action_id_t action_id;  /* Policer action id. */
} bcm_policer_action_config_t;

typedef struct bcm_policer_action_param_s {
    bcm_policer_action_t action;    /* Policer action. */
    uint32 param0;                  /* Policer action param 0. */
    uint32 param1;                  /* Policer action param 1. */
} bcm_policer_action_param_t;

typedef struct bcm_policer_action_profile_s {
    uint32 policer_index;           /* Policer pair index. */
    uint32 policer_profile_index;   /* Policer profile index. */
} bcm_policer_action_profile_t;

/* Initialize a policer action config structure. */
extern void bcm_policer_action_config_t_init(
    bcm_policer_action_config_t *action_cfg);

/* Initialize a policer action structure. */
extern void bcm_policer_action_param_t_init(
    bcm_policer_action_param_t *action_param);

/* Initialize a policer action profile structure. */
extern void bcm_policer_action_profile_t_init(
    bcm_policer_action_profile_t *action_profile);

#ifndef BCM_HIDE_DISPATCHABLE

/* Create a policer action entry. */
extern int bcm_policer_action_config_create(
    int unit, 
    bcm_policer_action_config_t *action_cfg);

/* Destroy a policer action entry. */
extern int bcm_policer_action_config_destroy(
    int unit, 
    bcm_policer_action_id_t action_id);

/* Get a policer action entry. */
extern int bcm_policer_action_config_get(
    int unit, 
    bcm_policer_action_config_t *action_cfg);

/* Destroy all policer action entries. */
extern int bcm_policer_action_config_destroy_all(
    int unit);

/* Add a policer action to the action entry. */
extern int bcm_policer_action_param_add(
    int unit, 
    bcm_policer_action_id_t action_id, 
    bcm_policer_action_param_t *action);

/* Get a policer action from the action entry. */
extern int bcm_policer_action_param_get(
    int unit, 
    bcm_policer_action_id_t action_id, 
    bcm_policer_action_param_t *action);

/* Remove a policer action from the action entry. */
extern int bcm_policer_action_param_remove(
    int unit, 
    bcm_policer_action_id_t action_id, 
    bcm_policer_action_t action);

/* Remove all policer action from the action entry. */
extern int bcm_policer_action_param_remove_all(
    int unit, 
    bcm_policer_action_id_t action_id);

/* Create a policer action profile entry. */
extern int bcm_policer_action_profile_create(
    int unit, 
    bcm_policer_action_id_t action_id, 
    bcm_policer_action_profile_t *action_profile);

/* Get a policer action profile entry. */
extern int bcm_policer_action_profile_get(
    int unit, 
    bcm_policer_action_id_t action_id, 
    bcm_policer_action_profile_t *action_profile);

/* Destroy a policer action profile entry. */
extern int bcm_policer_action_profile_destroy(
    int unit, 
    bcm_policer_action_id_t action_id);
