...
typedef struct bcm_tdm_ingress_context_s {
    ...
    int is_wide_stream_id;              /* indicates a wide sid mode. depands
                                           whether wide stream id enabled on
                                           this device */
...
typedef struct bcm_tdm_interface_config_s {
    ...
    int stream_id_key_size;     /* Relevant for stream_id_enable TRUE only. Key
                                   size in bits 1-14 */
    int stream_id_wide_enable;  /* Relevant for stream_id_enable TRUE only. This
                                   enables wide stream ID. In this case
                                   stream_id_key_size is bigger than 14 */
