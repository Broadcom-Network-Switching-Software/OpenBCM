...
#define BCM_FAILOVER_FEC_3RD_HIERARCHY  (1 << 13)  /* flag to allocate failover
                                                      id to be used for FEC
                                                      third hierarchy */
#define BCM_FAILOVER_INGRESS_MC         (1 << 14)  /* Flag to allocate
...
/* Failover type defines */
#define BCM_FAILOVER_TYPE_NONE          0          /* No Failover type */
#define BCM_FAILOVER_TYPE_FEC           2          /* Failover ID of type FEC
                                                      (1:1 UC) */
#define BCM_FAILOVER_TYPE_INGRESS       3          /* Failover ID of type
                                                      Ingress (1+1) */
#define BCM_FAILOVER_TYPE_ENCAP         4          /* Failover ID of type Egress
                                                      (1:1 MC) */
#define BCM_FAILOVER_TYPE_INGRESS_MC    5          /* Failover ID of type
                                                      Ingress MC (1:1 MC) */

/* Failover Type and Value location within the Failover ID */
#define _BCM_FAILOVER_TYPE_SHIFT    29         
#define _BCM_FAILOVER_VAL_SHIFT     0          
#define _BCM_FAILOVER_TYPE_MASK     7          
#define _BCM_FAILOVER_VAL_MASK      0x1fffffff 

#define BCM_FAILOVER_TYPE_GET(failover_type, failover_id)  \
 ((failover_type) = (((failover_id) >> _BCM_FAILOVER_TYPE_SHIFT) & _BCM_FAILOVER_TYPE_MASK))    

#define BCM_FAILOVER_ID_GET(failover_id_val, failover_id)  \
 ((failover_id_val) = (((failover_id) >> _BCM_FAILOVER_VAL_SHIFT) & _BCM_FAILOVER_VAL_MASK))    

#define BCM_FAILOVER_SET(failover_id, failover_id_val, failover_type)  \
 ((failover_id) = (((failover_type) << _BCM_FAILOVER_TYPE_SHIFT) | (((failover_id_val) & _BCM_FAILOVER_VAL_MASK) << _BCM_FAILOVER_VAL_SHIFT)))   
