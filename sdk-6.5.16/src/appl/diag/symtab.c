...
parse_symbolic_reference(int unit, soc_regaddrlist_t *alist, char *ref)
{
    ...
    if (diag_parse_range(idx1, idx2, &minidx, &maxidx,
                         SOC_REG_ARRAY(unit, reg) ? SOC_REG_INFO(unit, reg).first_array_index : -1,
                         SOC_REG_ARRAY(unit, reg) ? SOC_REG_NUMELS(unit,reg) + SOC_REG_INFO(unit, reg).first_array_index - 1 : -1) < 0) {
        LOG_WARN(BSL_LS_APPL_SYMTAB,
                 (BSL_META_U(unit,
