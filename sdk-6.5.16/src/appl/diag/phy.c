...
_phy_diag_linkcat(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    char *mode = NULL;
    int mode_val = LINKCAT_LPBK_MODE; /* mode specified on command line. Default to lpbk */
    phymod_dispatch_type_t type = phymodDispatchTypeCount;
    ...
    parse_table_add(&pt, "mode", PQ_DFL | PQ_STRING,
               (void *)(0), &mode, NULL);
    parse_table_add(&pt, "prefix", PQ_DFL | PQ_STRING,
               (void *)(0), &prefix, NULL);
    if ( parse_arg_eq(args, &pt) < 0) {
        cli_out("Error: invalid option: %s\n", ARG_CUR(args));
        parse_arg_eq_done(&pt);
    ...
    if ( mode != NULL ) {
        if ( !sal_strcasecmp(mode, "tx") ) {
            mode_val = LINKCAT_LPTX_MODE;
        } else if ( !sal_strcasecmp(mode, "rx") ) {
            mode_val = LINKCAT_RX_MODE;
        } else if ( !sal_strcasecmp(mode, "lpbk") ) {
            mode_val = LINKCAT_LPBK_MODE;
        } else {
            cli_out("Invalid mode parameter %s\n", mode); 
            return CMD_OK;
        }
    }
    sal_memset(prefix_str, 0, 50); 
    if ( prefix != NULL ) {
        sal_strncpy(prefix_str, prefix, 49); 
    }

    parse_arg_eq_done(&pt);

    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        sal_memset(&phy_acc, 0, sizeof(phymod_access_t)); 
        ...
        if(soc_feature(unit, soc_feature_portmod)) {
            ...
            sal_memcpy(&phy_acc, &internal_core.access, sizeof(phymod_access_t));
            type = internal_core.type;
            ...
            phy_acc.lane_mask = access.access.lane_mask;
...
#endif /* PORTMOD_SUPPORT */
        {
            ...
            sal_memcpy(&phy_acc, &pc->phymod_ctrl.phy[0]->pm_phy.access, sizeof(phymod_access_t));
            type = pc->phymod_ctrl.phy[0]->pm_phy.type; 
        }

        if ( phy_acc.bus == NULL || !phy_acc.addr || !phy_acc.lane_mask ) {
        ...
        for(first_bit = 0; first_bit < 32; first_bit++) {
           if ( (phy_acc.lane_mask & mask) ) { 
                phy_acc.lane_mask = 1 << ( first_bit + lane ); 
                break;
            }
            mask = mask << 1;
        }
        ...
        linkcat_config.portaddress = phy_acc.addr; 
        ...
        switch(type) {
            ...
            case phymodDispatchTypeTscbh: 
                ...
                blackhawk_tsc_LinkCat(&phy_acc, &linkcat_config, &linkcat_status);
                break;
            default:
...
STATIC void
prbs_stat_ber_update(int unit, bcm_port_t port, int lane, uint32 delta)
{
    double ber;
    prbs_stat_cb_t *pscb;
    prbs_stat_pinfo_t *pspi;
    ...
    for (lane = 0; lane < pspi->lanes; lane++) {
        ...
        default:
            ...
            if (pspi->ber[lane] != -2) {
                 ...
                 */
                prbs_stat_ber_update(unit, port, lane, status);
            }
...
prbs_stat_counter_init(int unit)/*, bcm_pbmp_t pbmp)*/
{
    ...
    BCM_PBMP_ITER(pscb->pbmp, port) {
        ...
        for (lane = 0; lane < pspi->lanes; lane++) {
            BCM_PHY_GPORT_LANE_PORT_SET(gport, lane, port);
            bcm_port_phy_control_get(unit, gport,
                                     BCM_PORT_PHY_CONTROL_PRBS_RX_STATUS,
                                     &status);
            pspi->intervals[lane] = 0;
        }
