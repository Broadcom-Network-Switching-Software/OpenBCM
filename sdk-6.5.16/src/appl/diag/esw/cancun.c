    ...
    /* Operation: load */
        ...
        if(!c) {
            ...
            if (!sal_strcasecmp(c, "cih")) {
                ...
                sal_strncpy(filename, c, CANCUN_FILENAME_SIZE-1);
            }
...
int _cmd_cancun_file_load(int unit, char* filename, uint32 file_type,
    ...
    if(ccf) {
        sal_strncpy(ccf->filename, filename, filename_len);
    }
...
#define _SOC_CANCUN_DEFAULT_PATHS_NUM   (7)
    ...
    sal_strncpy(p, soc_chip_type_names[SOC_DRIVER(unit)->type], CANCUN_FILENAME_SIZE-1);
    sal_strcat(p, "_");
    sal_strcat(p, soc_cancun_file_type_string[file_type]);
    sal_strcat(p, ".pkg");
    ...
    /* searching default file path if not available */
        for(i = 0; i < _SOC_CANCUN_DEFAULT_PATHS_NUM; i++) {
            sal_strncpy(filepath, paths[i], CANCUN_FILENAME_SIZE-1);
            sal_strncat(filepath, filename, chipname_len);
            sal_strncat(filepath, "/default/", 9);
            sal_strncat(filepath, filename, CANCUN_FILENAME_SIZE-1);
            fp = sal_fopen(filepath, "rb");
            if (fp) {
                sal_fclose(fp);
                sal_getcwd(cc->default_path, CANCUN_FILENAME_SIZE-1);
                sal_strcat(cc->default_path, paths[i]);
                sal_strncat(cc->default_path, filename, chipname_len);
                sal_strncat(cc->default_path, "/default/", 9);
                break;
            }
        }
    } else {
    ...
    sal_strncpy(filepath, cc->default_path, CANCUN_FILENAME_SIZE-1);
    sal_strncat(filepath, filename, CANCUN_FILENAME_SIZE-1);
    sal_strncpy(filename, filepath, CANCUN_FILENAME_SIZE-1);
...
int soc_cancun_generic_load(int unit, uint32 cancun_type) {
    ...
    if ((cancun_type < CANCUN_SOC_FILE_TYPE_CIH) ||
        (cancun_type > CANCUN_SOC_FILE_TYPE_CEH)) {
        LOG_ERROR(BSL_LS_SOC_CANCUN, (BSL_META_U(unit,
