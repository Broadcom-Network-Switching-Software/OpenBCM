...
th3_check_req_grant_per_port(int unit, int port) {
    ...
    if ((!IS_CPU_PORT(unit, port)) && (!IS_LB_PORT(unit, port))) {
        ...
        soc_reg_get(unit, reg, port, REG_PORT_ANY, &rval);
        /* coverity[checked_return:FALSE]*/
        port_ep_req = soc_reg64_field32_get(unit, reg, rval, REQ_CNTf);
        /* coverity[checked_return:FALSE]*/
        soc_mem_read(unit, EGR_PORT_REQUESTSm, MEM_BLOCK_ANY, phy_port, read_entry);
        /* coverity[checked_return:FALSE]*/
    ...
    /* coverity[checked_return:FALSE]*/
    soc_reg32_get(unit, MMU_EBPTS_EDB_CREDIT_COUNTERr, port, REG_PORT_ANY,
            &rval_1);
    /* coverity[checked_return:FALSE]*/
    mmu_credit = soc_reg_field_get(unit, MMU_EBPTS_EDB_CREDIT_COUNTERr, rval_1,
            NUM_EDB_CREDITSf);
    /* coverity[checked_return:FALSE]*/
    soc_mem_read(unit, EGR_MMU_REQUESTSm, MEM_BLOCK_ANY, phy_port, read_entry);
    /* coverity[checked_return:FALSE]*/
