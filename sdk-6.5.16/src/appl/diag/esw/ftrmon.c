...
void
format_ts(char buf[20], uint8 *ts)
{
    ...
    if (report & REPORT_DECODE) {
        if (pkt->_dv) {
            ...
            for (ix = 0; ix < (pkt->pkt_len); ix+=8) {
                cli_out("%04d: %02x %02x %02x %02x %02x %02x %02x %02x\n",
                        (ix/8), data[ix], data[ix+1], data[ix+2], data[ix+3], data[ix+4], data[ix+5], data[ix+6], data[ix+7]);
            }
            cli_out("*************************************************\n");
