               ...
               void *userdata)
{
    ...
    if (handled == 0) {
        ...
        return BCM_FLOWTRACKER_EXPORT_RECORD_HANDLED_OWNED;
    }
}
...
cmd_ftmon(int unit, args_t *arg)
{
...
#ifdef BCM_FLOWTRACKER_SUPPORT
    callback_options.callback_prio = 0;
    if (!sal_strcasecmp(subcmd, "start")) {
        ...
        rv = bcma_ft_export_fifo_control(unit, 1000000);
        if (rv < 0) {
        ...
        rv = bcm_collector_export_record_register(unit, collector_id,
                                                    callback_options,
                                                    ftmon_callback, NULL);
        ...
        rv = bcma_ft_export_fifo_control(unit, 0);
        if (rv < 0) {
            cli_out("%s: ERROR: %s\n", ARG_CMD(arg), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        rv = bcm_collector_export_record_unregister(unit, collector_id,
                                                      callback_options,
                                                      ftmon_callback);
