...
cmd_ptp_syncphy(int unit, args_t *a)
{
    ...
    if (UsePHYSYNCcb) {
        ...
        /* Adding 1.5 sec */
        extphy_config.cpu_latency_nsec = 0;
...
int get_output_clock_index(int arg_clock_index, int arg_l1mux_port)
{
    int clock_index = arg_clock_index;

    /* "BS0", "BS1", "SyncE0", "SyncE1",..., "SyncE63", "1588-Freq", "GPIO_0",...,"GPIO_5" */
    if ((clock_index == 2) && (arg_l1mux_port > 0) && (arg_l1mux_port <= 64)) {
        /* syncE: 2 -> 65 */
        clock_index = clock_index + (arg_l1mux_port-1);
    } else if (clock_index == 3) {
        /* 1588-Freq:66 */
        clock_index = (BCM_TDPLL_OUTPUT_CLOCK_NUM_SYNCE + 2);   /*66;*/
    } else if ((clock_index >= 4) && (clock_index <= 9)) {
        /* 1588-Phase:67...72 */
        clock_index = (BCM_TDPLL_OUTPUT_CLOCK_NUM_SYNCE + 3) + (clock_index-4);  /* 67+(clock_index-4);*/
    } else {
        cli_out("### get_output_clock_index: Error !! Invalid clock_index\n");
    }
cli_out("### get_output_clock_index: ip:clock_index-%d op:clock_index-%d\n", arg_clock_index, clock_index);

    return clock_index;
}
extern int _bcm_esw_tdpll_outClk_portSet(int unit, int outclock_index, uint32 port);
#endif
    ...
    if (arg) {
        ...
        if (clock_index_str) {
            ...
            if ((arg_clock_index >= 2) && (arg_clock_index <= 5)) {
                char *pPort = ARG_GET(a);
                if (!isint(pPort)) {
                    cli_out("### Invalid port\n");
                    return CMD_FAIL;
                }
                arg_l1mux_port = bcm_esw_ptp_map_synce_clock_port(parse_integer(pPort));
            }
            /* arg_clock_index = get_output_clock_index(arg_clock_index, arg_l1mux_port); */
...
#if defined(BCM_MONTEREY_SUPPORT) & defined(INCLUDE_GDPLL)
        if ((arg_clock_index >= 2) &&
            (arg_clock_index < (BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC + BCM_TDPLL_OUTPUT_CLOCK_NUM_SYNCE))) {
            /* Output clock is port */
            if (arg_l1mux_port > 0 && arg_l1mux_port <= 64) {
                PTP_IF_ERROR_RETURN(_bcm_esw_tdpll_outClk_portSet(
                   unit, arg_clock_index, arg_l1mux_port));
            } else {
                cli_out("Error! Invalid port number\n");
                return CMD_FAIL;
            }
        }
#endif
