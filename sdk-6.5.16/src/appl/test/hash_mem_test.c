...
#ifdef BCM_HELIX5_SUPPORT
            if (SOC_IS_HELIX5(unit)) {
                r = soc_hx5_hash_bucket_get(unit, htd->mem, bank, entry, &soft_bucket);
            } else
#endif /* BCM_HELIX5_SUPPORT */
#ifdef BCM_TRIDENT3_SUPPORT
            if (SOC_IS_TRIDENT3X(unit)) {
...
#ifdef BCM_HELIX5_SUPPORT
            if (SOC_IS_HELIX5(unit)) {
                index = soc_hx5_hash_index_get(unit, htd->mem, bank, soft_bucket);
            } else
#endif /* BCM_HELIX5_SUPPORT */
#ifdef BCM_TRIDENT3_SUPPORT
            if (SOC_IS_TRIDENT3X(unit)) {
...
#ifdef BCM_HELIX5_SUPPORT
                if (SOC_IS_HELIX5(unit)) {
                    soc_hx5_hash_bucket_get(unit, htd->mem, 0, &(entry_tmp[offset]), &bucket);
                    idx = soc_hx5_hash_index_get(unit, htd->mem, 0, bucket);
                } else
#endif
#ifdef BCM_TRIDENT3_SUPPORT
                if (SOC_IS_TRIDENT3X(unit)) {
