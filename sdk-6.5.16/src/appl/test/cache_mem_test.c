    ...
    *p = (void *) test_params;
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        if (test_params->test_part != cache_mem_test_cache_only && test_params->test_part != cache_mem_test_read_only)
        {
            ...
            if ((rv = bcm_switch_event_control_set(unit, BCM_SWITCH_EVENT_DEVICE_INTERRUPT, type, 1)) < 0)
            {
