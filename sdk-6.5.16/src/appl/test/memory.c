...
/* Function that checks if memory is an alias */
static int
is_main_mem_read(
    int unit,
    soc_mem_t mem)
...
/* Skip function needed when looping over all memories */
int
test_clear_mem_filter(int unit, soc_mem_t mem)
{
    int rv = 0;

    if (!is_main_mem_read(unit, mem))
    {
        return 1;
    }
    switch (mem)
    ...
    soc_mem_t mem)
{
    ...
    /* Check if the bit numbers are calculated correctly */
    assert(((nof_bits_in_mem + 7) / 8 == soc_mem_entry_bytes(unit, mem)) && nof_data_bits > 0 && nof_data_bits <= nof_bits_in_mem);

    return nof_data_bits;
}
...
static int tr10_init_callback(int unit) {
    soc_mem_t mem;
    int i;
    int rv = 0;
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++)
    {
        ...
        if (test_clear_mem_filter(unit, mem))
        {
            continue;
        }
        /*
...
memories_clear_test(int unit, args_t *a, void *pa)
{
    ...
    uint32 write_data[SOC_MAX_MEM_WORDS];
    dnx_startup_test_functions[unit] = tr10_init_callback;
