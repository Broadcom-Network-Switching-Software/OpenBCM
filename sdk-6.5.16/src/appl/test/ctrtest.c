    ...
    int counter_id)
{
    ...
    rv = dnx_cdmib_counter_id_to_dbal_field_id_get(unit, port, counter_id, &field_id);
    if (rv != 0)
    {
    ...
    rv = soc_jer2_stat_counter_length_get(unit, counter_id, &length);
    if (rv != 0)
    {
    ...
    for (loop = 0; loop < 4; loop++)
    {
        ...
        rv = dnx_cdmib_stat_dbal_set(unit, port, field_id, tmp);
        if (rv != 0)
        {
        ...
        rv = dnx_cdmib_stat_dbal_get(unit, port, field_id, &val);
        if (rv != 0)
        {
...
int ctr_dnx_test_rw(int unit, args_t *a, void *pa)
{
    ...
    {
        for (counter_id = soc_jer2_counters_cdmib_r64; counter_id <= soc_jer2_counters_cdmib_tbyt; counter_id++)
        {
    ...
    int counter_id)
{
    ...
    rv = dnx_cdmib_stat_dbal_set(unit, port, field_id, val);
    if (rv != 0)
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - set value %08x%08x for counter type %d failed"),
                                  COMPILER_64_HI(val),COMPILER_64_LO(val), counter_id));
        res = 1;
    }
    ...
    rv = soc_jer2_stat_counter_length_get(unit, counter_id, &defined_width);
    if (rv != 0)
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - get length failed for counter type %d"), counter_id));
        res = 1;
    ...
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit,
                                    "Counter type %d width mismatch"
                                    "defined width: %d bits, actual: %d bits\n"),
                                    counter_id, defined_width, actual_width));
        res = 1;
    }
