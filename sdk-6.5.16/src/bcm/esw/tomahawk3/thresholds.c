    ...
    int cable_len, int arr_size, bcm_cosq_pkt_size_dist_t *pkt_dist_array)
{
    ...
    uint32 entry[SOC_MAX_MEM_WORDS];
    soc_mem_t mem = INVALIDm;
    int delta = 0, old_limit;
    int itm;
    uint32 old_hp_limit = 0, itm_map = 0;
    ...
    for (pg_i = 0; pg_i < _TH3_MMU_NUM_PG; pg_i++) {
        if (pg_bmp & (1U << pg_i)) {
            ...
            old_limit = soc_mem_field32_get(unit, mem, entry,
                    field_pghdrm_limit[pg_i]);
            delta += (hdrm - old_limit);
    ...
    itm_map = si->ipipe_itm_map[pipe];
    for (itm = 0; itm < NUM_ITM(unit); itm++) {
        if (itm_map & (1 << itm)) {
            /* Retrieve old value of headroom pool limit */
            rv = soc_tomahawk3_itm_reg32_get(unit,
                    MMU_THDI_HDRM_BUFFER_CELL_LIMIT_HPr, itm, itm, 0,
                    &old_hp_limit);
            if (rv) {
                goto exit;
            }
            rv = _bcm_th3_adjust_delta_hdrm_service_pools(unit, itm,
                            0, 0, (old_hp_limit + delta));
            if (rv) {
                goto exit;
            }
        }
