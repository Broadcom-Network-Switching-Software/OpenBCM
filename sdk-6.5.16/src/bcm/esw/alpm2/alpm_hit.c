 ...
 */
int
alpm_rte_hit_get(int u, int vrf_id, _alpm_tbl_t tbl, int idx, int ent)
{
    int rv = BCM_E_UNAVAIL;
    rv = ALPM_DRV(u)->alpm_bkt_hit_get(u, vrf_id, ACB_VRF_BTM(u, vrf_id),
                                       tbl, idx, ent);
    return rv;
}
...
int
alpm_rte_hit_set(int u, int vrf_id, _alpm_tbl_t tbl, int idx, int ent)
{
    int rv = BCM_E_UNAVAIL;
    rv = ALPM_DRV(u)->alpm_bkt_hit_set(u, vrf_id, ACB_VRF_BTM(u, vrf_id),
                                       tbl, idx, ent, 1);
    return rv;
}
...
alpm_pvt_hit_get(int u, _alpm_cb_t *acb, _alpm_pvt_node_t *pvt_node)
{
    ...
    if (ACB_HAS_TCAM(acb)) { /* TCAM pre-pivot cb=0 */
        ...
        rv = tcam_cache_hit_get(u, pkm, tcam_idx);
                ...
                _alpm_pvt_node_t *pvt_node, void *user_data)
{
    ...
    if (def_node != NULL) { /* pivot BPM route exists */
        ...
        rv = alpm_rte_hit_get(u, vrf_id, tbl, idx, ent);
        ...
        rv = alpm_rte_hit_set(u, vrf_id, tbl, idx, ent);
    }
...
int
alpm_hitbit_sync_cache(int u, int i, int pid)
{
    int rv = BCM_E_UNAVAIL;
    rv = ALPM_DRV(u)->alpm_hit_cache_sync(u, i, pid);
...
int
alpm_hitbit_write_hw(int u, int i, int pid)
{
    int rv = BCM_E_UNAVAIL;
    rv = ALPM_DRV(u)->alpm_hit_hw_write(u, i, pid);
...
int
tcam_entry_hit_get(int u, int pkm, void *e, int sub_idx)
{
    int         rv = BCM_E_UNAVAIL;

    rv = ALPM_DRV(u)->tcam_entry_hit_get(u, pkm, e, sub_idx);
...
int
tcam_cache_hit_get(int u, int pkm, int tcam_idx)
{
    int         rv = BCM_E_UNAVAIL;

    rv = ALPM_DRV(u)->tcam_cache_hit_get(u, pkm, tcam_idx);
    ...
    while (ALPMDH(u)->interval != 0) {
        ...
        for (i = 0; i < ACB_CNT(u); i++) {
            ...
            for (pid = 0; pid < ACB_PID_CNT(acb); pid++) {
                alpm_hitbit_sync_cache(u, i, pid);
            }
        }
