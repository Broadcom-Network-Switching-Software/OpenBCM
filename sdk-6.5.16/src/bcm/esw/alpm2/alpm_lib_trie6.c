...
static int taps_key_shift(uint32 *key, uint32 length, int32 shift)
{
    ...
    if (shift > 0) {
        /* right shift */
        for (lsb = shift, word_idx=BITS2WORDS(_MAX_KEY_LEN_144_)-1;
             word_idx >=0;
             lsb+=32, word_idx--) {
            if (lsb < length) {
        ...
        for (word_idx = 0;
             ((shift/32)!=0) && (word_idx < BITS2WORDS(_MAX_KEY_LEN_144_));
             word_idx++) {
            if ((word_idx + (shift/32)) >= BITS2WORDS(_MAX_KEY_LEN_144_)) {
                key[word_idx]=0;
            } else {
        ...
        for (word_idx = 0;
             ((shift%32)!=0) && (word_idx < BITS2WORDS(_MAX_KEY_LEN_144_));
             word_idx++) {
            if (word_idx == TP_BITS2IDX(0)) {
                /* at bit 0 word, next word doesn't exist */
                key[word_idx] = _SHL(key[word_idx], (shift%_NUM_WORD_BITS_));
            } else {
                key[word_idx] = _SHL(key[word_idx], (shift%_NUM_WORD_BITS_)) | \
                    TRIE_SHR(key[word_idx+1], _NUM_WORD_BITS_-(shift%_NUM_WORD_BITS_), _NUM_WORD_BITS_);
            }
        }
 ...
 */
static int _key_append(uint32 *key,
               uint32 *length,
               uint32 skip_addr,
               uint32 skip_len)
{
 ...
 */
static uint32
lcplen(uint32 *key, uint32 len1, uint32 skip_addr, uint32 len2)
{
    uint32 diff;
...
/*
 * Function:
 *   _alpm_lib_trie_v6_skip_node_free
 * Purpose:
 ...
 *   free_end --  free
 */
int _alpm_lib_trie_v6_skip_node_free(alpm_lib_trie_node_t *trie,
                            uint32 *key,
                            uint32 length)
{
    uint32 lcp=0;
    ...
    if (length > trie->skip_len) {
        ...
        if (lcp == trie->skip_len) {
            ...
            /* based on next bit branch left or right */
            if (trie->child[bit]) {
                rv = _alpm_lib_trie_v6_skip_node_free(trie->child[bit], key,
                        length - lcp - 1);
                if (SOC_SUCCESS(rv)) {
               ...
               alpm_lib_trie_node_t **new_parent)
{
    ...
    if (child != child_trie) {
        rv = _alpm_lib_trie_v6_skip_node_free(child_trie, child_pivot, child_length);
        if (rv < 0) {
            return SOC_E_PARAM;
        }
    }
