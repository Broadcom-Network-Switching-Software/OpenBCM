...
STATIC uint32 taf_gate_refcount[SOC_MAX_NUM_DEVICES][BCMI_TSN_TAF_GATE_NUM];

/*
 * taf gate pri map
 */
#define TAF_GATE_PRI_MAP_PROFILE_OFFSET     (0)
#define TAF_GATE_PRI_MAP_PROFILE_WIDTH      (7)
#define TAF_GATE_PRI_MAP_PROFILE_INVALID_ID (0)
#define TAF_GATE_PRI_MAP_PROFILE_NUM    (1 << TAF_GATE_PRI_MAP_PROFILE_WIDTH)
#define TAF_GATE_PRI_MAP_PROFILE_MASK                       \
            (((1 << TAF_GATE_PRI_MAP_PROFILE_WIDTH) - 1) << \
             TAF_GATE_PRI_MAP_PROFILE_OFFSET)
#define TAF_GATE_PRI_MAP_INTPRI_OFFSET (7)
#define TAF_GATE_PRI_MAP_INTPRI_WIDTH  (4)
#define TAF_GATE_PRI_MAP_INTPRI_NUM    (1 << TAF_GATE_PRI_MAP_INTPRI_WIDTH)
#define TAF_GATE_PRI_MAP_INTPRI_MASK                       \
            (((1 << TAF_GATE_PRI_MAP_INTPRI_WIDTH) - 1) << \
             TAF_GATE_PRI_MAP_INTPRI_OFFSET)
#define TAF_GATE_PRI_MAP_PREEMPT_OFFSET (11)
#define TAF_GATE_PRI_MAP_PREEMPT_WIDTH  (1)
#define TAF_GATE_PRI_MAP_PREEMPT_NUM    (1 << TAF_GATE_PRI_MAP_PREEMPT_WIDTH)
#define TAF_GATE_PRI_MAP_PREEMPT_MASK                       \
            (((1 << TAF_GATE_PRI_MAP_PREEMPT_WIDTH) - 1) << \
             TAF_GATE_PRI_MAP_PREEMPT_OFFSET)
...
STATIC const soc_mem_t taf_schedule_calendar_mem[2][BCMI_TSN_TAF_GATE_NUM]
...
#define TAF_INTERRUPT_SUB_FIELD_NUM \
        ((BCMI_TSN_TAF_GATE_NUM + TAF_INTERRUPT_SUB_FIELD_WIDTH - 1)\
...
bcmi_gh2_taf_gate_create(int unit, int flags, int *taf_gate_id)
{
    ...
    if (*taf_gate_id < 0 || *taf_gate_id >= BCMI_TSN_TAF_GATE_NUM ||
    ...
    void *user_data)
{
    ...
    for (taf_gate_id = 0; taf_gate_id < BCMI_TSN_TAF_GATE_NUM; taf_gate_id++) {
...
{
    assert(taf_gate_id >= 0 && taf_gate_id < BCMI_TSN_TAF_GATE_NUM);
    ...
    int taf_gate_id)
{
    if (taf_gate_id < 0 ||
        taf_gate_id == TAF_GATE_RESERVED_ID ||
        taf_gate_id >= BCMI_TSN_TAF_GATE_NUM) {
        return BCM_E_PARAM;
    }
    ...
    void *user_data)
{
    ...
    assert(taf_gate >= 0 && taf_gate < BCMI_TSN_TAF_GATE_NUM);
...
{
    if (NULL == hw_idx ||
        TAF_GATE_PRI_MAP_PROFILE_NUM <= sw_idx ||
    ...
    uint32 *sw_idx)
{
    ...
    if (hw_idx >= TAF_GATE_PRI_MAP_PROFILE_NUM ||
    ...
    uint32 new_gate_id)
{
    ...
    if (preempt >= TAF_GATE_PRI_MAP_PREEMPT_NUM ||
        ...
        profile_idx >= TAF_GATE_PRI_MAP_PROFILE_NUM ||
        profile_idx == TAF_GATE_PRI_MAP_PROFILE_INVALID_ID) {
        return BCM_E_PARAM;
    }
    hw_idx = (preempt << TAF_GATE_PRI_MAP_PREEMPT_OFFSET) |
             (priority << TAF_GATE_PRI_MAP_INTPRI_OFFSET) |
    ...
    bcm_tsn_pri_map_config_t *config)
{
    ...
    for (prio = 0; prio < TAF_GATE_PRI_MAP_INTPRI_NUM; prio++) {
    ...
    bcm_tsn_pri_map_config_t *config)
{
    ...
    config->num_map_entries = TAF_GATE_PRI_MAP_INTPRI_NUM;
    ...
    uint32 bmp_size)
{
    ...
    if (bmp_size < _SHR_BITDCLSIZE(BCMI_TSN_TAF_GATE_NUM)) {
        return BCM_E_PARAM;
    }
    ...
    SHR_BITTEST_RANGE(ret_bmp_gate, 0, BCMI_TSN_TAF_GATE_NUM, *ret_need);
    return BCM_E_NONE;
}
...
bcmi_gh2_taf_cleanup(int unit)
{
    ...
    for (i = 0; i < BCMI_TSN_TAF_GATE_NUM; i++) {
        bcmi_tsn_taf_schedule_entry_t *entry_iter = NULL;
...
bcmi_gh2_taf_init(int unit)
{
    ...
    if ((TAF_GATE_PRI_MAP_PROFILE_MASK | TAF_GATE_PRI_MAP_INTPRI_MASK |
         TAF_GATE_PRI_MAP_PREEMPT_MASK) >= entry_count) {
    ...
    for (stat_type = 0; stat_type < bcmTsnTafGateStatCount; stat_type++) {
        ...
        taf_gate_counter[unit][stat_type] =
                sal_alloc(BCMI_TSN_TAF_GATE_NUM * sizeof(uint64),
                          "taf gate counter");
        if (NULL == taf_gate_counter[unit][stat_type]) {
        ...
        taf_gate_counter_dma_buffer[unit][stat_type] =
            soc_cm_salloc(unit, BCMI_TSN_TAF_GATE_NUM *
                          WORDS2BYTES(
    ...
    int unit, int *size)
{
    ...
    *size = 0;
    ...
    /* taf gate */
    *size += BCMI_TSN_TAF_GATE_NUM * sizeof(uint32);

    /* maxbyte profile */
    *size += BCMI_TSN_TAF_GATE_NUM * TAF_MAXBYTE_PROFILE_NUM * sizeof(uint32);
    ...
    int unit, uint8 *scache_ptr)
{
    ...
    /* taf gate */
    WRITE_PTR_RANGE(scache_ptr, BCMI_TSN_TAF_GATE_NUM * sizeof(uint32),
                    taf_gate_refcount[unit]);

    /* maxbyte profile */
    for (gate_id = 0; gate_id < BCMI_TSN_TAF_GATE_NUM; gate_id++) {
    ...
    int unit, uint8 *scache_ptr)
{
    ...
    /* taf gate */
    READ_PTR_RANGE(scache_ptr, BCMI_TSN_TAF_GATE_NUM * sizeof(uint32),
                   taf_gate_refcount[unit]);
...
#endif /* BCM_WARM_BOOT_SUPPORT */
    /* data set */
    TAF_GATE_PRI_MAP_PROFILE_NUM, /* exclude invalid index 0 */
