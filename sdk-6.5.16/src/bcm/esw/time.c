...
#define HX5_PORT_BLOCK_FIRST_PORT(port)                              \
    ...
    if (SOC_HAS_TIME_V3_OR_IPROC_MANAGED_BROADSYNC(unit)) {
        ...
        if (SOC_IS_TOMAHAWK3(unit) || SOC_IS_MONTEREY(unit)) {
            ...
            if (0 == soc_reg_field_get(
                            unit, NS_TIMESYNC_INPUT_TIME_FIFO_TS_3r, regval3, 
                            TS_VALIDf)) {
                return (BCM_E_FAIL);
            }
...
_bcm_esw_time_interface_get(int unit, bcm_time_if_t id, bcm_time_interface_t *intf)
{
...
#endif /* defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT) */
    {
        if (SOC_IS_TOMAHAWK3(unit) || SOC_IS_MONTEREY(unit)) {
            ...
            if (soc_reg_field_get(unit, NS_BS0_BS_CONFIGr, regval, LOCKf)) {
                intf_ptr->flags |= BCM_TIME_LOCKED;
            } else {
                intf_ptr->flags &= ~BCM_TIME_LOCKED;
            }
...
bcm_esw_time_init (int unit)
{
    ...
    if (SOC_IS_TOMAHAWK3(unit) || SOC_IS_MONTEREY(unit)) {
        ...
        eventId = 192;
        for (idx = 128; idx < 150; idx++) {
            BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TS_EVENT_FWD_CFGr, idx, &regVal);
            soc_reg_field_set(unit, NS_TIMESYNC_TS_EVENT_FWD_CFGr, &regVal, EVENT_IDf, eventId);
    ...
    tsfifo_flag = soc_property_get(unit, "tsfifo_available_flag", 0);
#endif
...
bcm_esw_time_interface_add (int unit, bcm_time_interface_t *intf)
{
    ...
    if (intf->flags & BCM_TIME_SYNC_STAMPER) {
        ...
        {
              if (SOC_IS_TOMAHAWK3(unit) || SOC_IS_MONTEREY(unit)) {  /* IPROC/CMICD */
                    if (needs_restart) {
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) || defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_APACHE_SUPPORT) || defined(BCM_GREYHOUND2_SUPPORT)
                                         ...
                                         int *adjusted_phy_port)
{
    ...
    if (SOC_IS_HELIX5(unit)) {
        first_phy_port = HX5_PORT_BLOCK_FIRST_PORT(port);
                               ...
                               bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    case bcmTimeSynceClockSourcePrimary:
            ...
            } else if (dispatch_type == phymodDispatchTypeTscf) {
...
#define BCM_TIME_HX5_PHY_PORT_VALID(x) ((x) >= 1 && (x) <= 76) /* Section 9.2, HX5 top spec */

STATIC int
_bcm_esw_time_hx5_synce_L1_rcvd_clk_port_get(int phy_port)
{
...
STATIC int
_bcm_esw_time_hx5_phy_port_get(int port_sel)
{
    if (port_sel < 4) {
        return (port_sel + 1);
    }
...
STATIC int
_bcm_esw_time_hx5_synce_clock_set_by_port(int unit,
                               bcm_time_synce_clock_src_type_t clk_src,
                               bcm_time_synce_divisor_setting_t *divisor)
{   
    /* Assumes input index is logical port number */
    ...
    if (!BCM_TIME_SYNCE_CLK_SRC_VALID(clk_src) ||
        !BCM_TIME_HX5_PHY_PORT_VALID(phy_port)) {
        return BCM_E_PARAM;
    }
    ...
    port_sel = _bcm_esw_time_hx5_synce_L1_rcvd_clk_port_get(phy_port);
                              ...
                              bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    if (val < 40) {
        ...
        phy_port = _bcm_esw_time_hx5_phy_port_get(val);
        _bcm_esw_time_synce_phy_port_get(unit, phy_port, &phy_port);
        divisor->input_src = bcmTimeSynceInputSourceTypePort;
        divisor->index = SOC_INFO(unit).port_p2l_mapping[phy_port];
    } else {
        divisor->input_src = bcmTimeSynceInputSourceTypePLL;
        divisor->index = val - 40;
    }
...
#if defined(BCM_GREYHOUND2_SUPPORT)
#define BCM_TIME_GH2_PHY_PORT_VALID(x) ((x) >= 2 && (x) <= 89) /* Section 7.1.3, GH2 top design spec */
                               ...
                               bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    if (phy_port > 1 && phy_port < 26) {  /* Viper */
        port_sel = phy_port + 6;
    ...
    if (phy_port >= 58) { /*Eagle and Falcon */
        /* Set TSCF SDM divisors */
        /* Set SDM stage 1 mode to bypass */
                              ...
                              bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    if (val >= 8 && val <= 31) { /* viper */
        phy_port = val - 6;
    } else if (val >= 40) { /*Eagle or Falcon */
        phy_port = val + 18;
        _bcm_esw_time_synce_phy_port_get(unit, phy_port, &phy_port);
    }
...

{
      ...
      /* GH2 */
        if (div_out.input_src == bcmTimeSynceInputSourceTypePort) {
            ...
            if ((div_out.stage0_mode == bcmTimeSynceModeSDMFracDiv) &&
                ...
                if (dispatch_type == phymodDispatchTypeTsce_dpll || dispatch_type == phymodDispatchTypeTscf ) {
                    *frequency = bcmTimeSyncE25MHz;
                }
            } else {
    ...
    if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3(unit) || SOC_IS_APACHE(unit) || SOC_IS_MAVERICK2(unit) || SOC_IS_HELIX5(unit) || SOC_IS_MONTEREY(unit)) {
        /* tomahawk  & TD3 */
        if ((div_out.stage0_mode == bcmTimeSynceModeSDMFracDiv) &&
                (div_out.stage1_div == bcmTimeSynceStage1Div1)) {
            ...
            if ((dispatch_type == phymodDispatchTypeTsce) || 
                (dispatch_type == phymodDispatchTypeTsce16)) {
            ...
            } else if (dispatch_type == phymodDispatchTypeTscf16) {
                switch (port_speed) {
                    case 100:
                        /* 1G ports  - VCO 25.78125 */
                        if (sdm_divisor == 0x19c8) {
...

{
    ...
    if (SOC_IS_HELIX5(unit)) {
        ...
        if ((clk_src_config->input_src == bcmTimeSynceInputSourceTypePort) && (phy_port <= 48)) {
        ...
        if (clk_src_config->input_src == bcmTimeSynceInputSourceTypePort) {
            ...
            switch (frequency) {
                case bcmTimeSyncE25MHz: /* 25MHz using SDM mode */
                    if (type == phymodDispatchTypeTsce_dpll) {
                        div_in.stage0_mode      = bcmTimeSynceModeSDMFracDiv;
                    ...
                    } else if (type == phymodDispatchTypeTscf) {
                        div_in.stage0_mode      = bcmTimeSynceModeSDMFracDiv;
                        div_in.stage1_div = bcmTimeSynceStage1Div1;
                        /* falcon ports */
                        if (speed ==  100 || speed == 10000 || speed== 40000) {
        ...
        bcm_time_ts_counter_t  *counter)
{
    ...
    if(SOC_IS_MONTEREY(unit) || SOC_IS_TOMAHAWK3(unit)) {
        ...
        if (ts_counter == 1) {
            SOC_IF_ERROR_RETURN(READ_NS_TIMESYNC_TS1_COUNTER_ENABLEr(unit, &regval));
            counter->enable = soc_reg_field_get(unit, NS_TIMESYNC_TS1_COUNTER_ENABLEr, regval, ENABLEf);
        } else if(ts_counter == 0) {
            SOC_IF_ERROR_RETURN(READ_NS_TIMESYNC_TS0_COUNTER_ENABLEr(unit, &regval));
...

{
    ...
    bsc_ts_utc_conversion_entry_t ts_entry;
#else
    egr_ts_utc_conversion_entry_t ts_entry;
...
#if defined(BCM_FLOWTRACKER_SUPPORT)
    sal_memset(&ts_entry, 0, sizeof(bsc_ts_utc_conversion_entry_t));
#else
    sal_memset(&ts_entry, 0, sizeof(egr_ts_utc_conversion_entry_t));
#endif /* BCM_FLOWTRACKER_SUPPORT */
    COMPILER_64_SET(check, u64_hi, u64_low);
...

{
    ...
    sal_memset(&ts_entry, 0, sizeof(egr_ts_utc_conversion_entry_t));
#endif
    rv = soc_mem_read(unit, mem, MEM_BLOCK_ALL, 0,  &ts_entry);
        ...
        bcm_time_tod_t *tod)
{
    ...
    if (stages & BCM_TIME_STAGE_ALL) {
        rv = bcmi_time_tod_set(unit, tod, 1);
        if (rv) {
            return rv;
        }

        rv = bcmi_time_tod_set(unit, tod, 0);
        ...
        bcm_time_tod_t *tod)
{
    ...
    if (tod == NULL) {
     ...
     defined(BCM_TOMAHAWK2_SUPPORT)|| defined(BCM_MAVERICK2_SUPPORT))
    if (stages & BCM_TIME_STAGE_ALL) {
        rv = bcmi_time_tod_get(unit, tod, 1);
    } else if (stages & BCM_TIME_STAGE_EGRESS) {
        rv = bcmi_time_tod_get(unit, tod, 1);
    }  else if (stages & BCM_TIME_STAGE_INGRESS_FLOWTRACKER) {
        rv = bcmi_time_tod_get(unit, tod, 0);
    } else {
        return BCM_E_PARAM;
    }
#endif
                                 ...
                                 int enable)
{
    ...
    switch (port_event_type) {
        ...
        case bcmTimePortEventRXCDR:
            ...
            BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_RX_CDR_EVENT_CTRLr, port, &regVal);
            soc_reg_field_set(unit, NS_TIMESYNC_RX_CDR_EVENT_CTRLr, &regVal, ENABLEf, enable ? 1:0);
            BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_RX_CDR_EVENT_CTRLr, port, regVal);
            break;
...
#endif /* BCM_MONTEREY_SUPPORT  || defined(BCM_TOMAHAWK3_SUPPORT)*/
        ...
        int *capture_handle)
{
    ...
    if (_bcm_esw_get_event_config_from_time_capture(unit,
            time_capture,
            &time_capture_input) != BCM_E_NONE) {
        rv = BCM_E_INTERNAL;
        goto exit;
    }
...
#if defined(BCM_MONTEREY_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT) || defined (BCM_TIMESYNC_TIME_CAPTURE_SUPPORT)
...
soc_esw_nanosync_ts_cmicx_intr(int unit, void *data)
{
    ...
    /* Read the GDPLL interrupt */
    BCM_TIME_READ_NS_REGr(unit, NS_TS_INT_STATUSr, 0, &int_status);
    BCM_TIME_READ_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, &int_enable);
