                                ...
                                _field_stage_t *stage_fc)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit,
            "FP(unit %d) Error: _field_hx5_ft_session_keygen_profiles_init=%d\n"),
                  unit, rv));
    ...
    rv = _field_ft_tracking_param_qual_map_db_init(unit, ft_info);
    if (rv != BCM_E_NONE) {
    ...
    /* Flow key Qualifiers cfg is same as stage qualifers */
    ft_info->flow_key_qual_cfg = stage_fc->qual_cfg_info_db;

    /* Flow key Extractors cfg is same as stage extractors cfg */
    ft_info->flow_key_ext_cfg_arr = stage_fc->ext_cfg_db_arr;

    /* Flow data Qualifiers cfg is same as stage qualifiers */
    ft_info->flow_data_qual_cfg = stage_fc->qual_cfg_info_db;
    ...
    if (rv != BCM_E_NONE) {
        if (stage_fc->ft_info != NULL) {
            if (stage_fc->ft_info->flow_data_ext_cfg != NULL) {
                sal_free(stage_fc->ft_info->flow_data_ext_cfg);
            }
            (void) _field_ft_tracking_param_qual_map_db_cleanup(unit, ft_info);
            sal_free(stage_fc->ft_info);
            stage_fc->ft_info = NULL;
                             ...
                             _field_stage_t *stage_fc)
{
    ...
    /* VPN */
    BCMI_KEYGEN_QUAL_CFG_INIT(unit);
    BCMI_KEYGEN_QUAL_CFG_OFFSET_ADD(0, BCMI_KEYGEN_EXT_SECTION_L1E16, 138, 2208, 10);
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    /* Decrease tcam_sz by 4 */
    /* 1 for Single-wide, 2 for Double-wide and 1 Reserved */
    stage_fc->tcam_sz -= 4;
    /* Move tcam stat Idx up by 2 */
    stage_fc->slices[0]->start_tcam_idx +=4;
    /* Reduce number of Free Entry in the slice by 2 */
    stage_fc->slices[0]->free_count -= 4;
    /* Reduce number of entries in the slice by 2 */
    stage_fc->slices[0]->entry_count -= 4;
    ...
    rv = _bcm_field_hx5_ft_tcam_policy_mem_get(unit, &tcam_mem, &policy_mem);
    BCM_IF_ERROR_RETURN(rv);
                                   ...
                                   _field_group_t *fg)
{
    ...
    SHR_BITCLR(ft_info->key_type_bmp, keytype);
                                   ...
                                   _field_group_t *fg)
{
    ...
    for (keytype = 0;
         ...
         keytype++) {
        if (!SHR_BITGET(stage_fc->ft_info->key_type_bmp, keytype)) {
            SHR_BITSET(stage_fc->ft_info->key_type_bmp, keytype);
            break;
        }
    }
                                        ...
                                        int tcam_idx)
{
    ...
    /* Write to h/w Table */
    if (BCM_SUCCESS(rv)) {
        soc_mem_write(unit, tcam_mem, MEM_BLOCK_ALL,
                tcam_idx, entry_buf);
 ...
 *    tcam_idx_old- (IN) Old tcam Indices of entry
 *    tcam_idx_new- (IN) New tcam Indices of entry
 *
 * Returns:
 *    BCM_E_PARAM       - Null field stage control structure.
 *    BCM_E_INTERNAL    - Invalid CAP Stage ID.
                             ...
                             int *tcam_idx_new)
{
    ...
    if ((f_ent == NULL) || (tcam_idx_old == NULL) || (tcam_idx_new == NULL)) {
        return (BCM_E_PARAM);
    }
    ...
    for (part = 0; part < parts_count; part++) {
        if ((tcam_idx_old[part] > tcam_idx_max) ||
            (tcam_idx_old[part] < tcam_idx_min) ||
            (tcam_idx_new[part] > tcam_idx_max) ||
            (tcam_idx_new[part] < tcam_idx_min)) {
            return (BCM_E_PARAM);
        }
    }
                                     ...
                                     _field_action_t *fa)
{
    ...
    switch(fa->action) {
        case bcmFieldActionFlowtrackerGroupId:
            ...
            rv = bcmi_ft_group_entry_add_check(unit, id);
            break;
                                ...
                                _field_entry_t *f_ent)
{
    ...
    sal_memset(&profiles, 0, sizeof(bcmi_ft_session_profiles_t));
     ...
     for (fa = f_ent->actions;
            (fa != NULL) && (fa->flags & _FP_ACTION_VALID);
            fa = fa->next) {
         if (fa->action != bcmFieldActionFlowtrackerGroupId) {
             continue;
         }
                                ...
                                int flags)
{
    ...
    for (fa = f_ent->actions; NULL != fa; fa = fa->next) {
        if (bcmFieldActionFlowtrackerGroupId != fa->action) {
            continue;
        }
        if (fa->flags & _BCM_FIELD_ACTION_REF_STALE) {
            /* Flowtracker Group is Stale */
            continue;
        }
        ...
        for (offset = 0; offset < BCMI_KEYGEN_QUAL_OFFSETS_MAX; offset++) {
            ...
            BCM_IF_ERROR_RETURN(
                _bcm_field_ft_keygen_profile_extractors_mask_set(unit,
                        q_offset, q_width, ext_num, shift_offsets_arr, ext_codes));
