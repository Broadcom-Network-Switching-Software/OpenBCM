                        ...
                        int *shift_offsets_arr)
{
    ...
    for (e_num = 1; e_num < 10; e_num++) {
        if ((extractors->l1_e16_sel[e_num - 1] == -1) ||
                (extractors->l2_e16_ext_mode[e_num - 1] == 0)) {
            gran = 16;
            ...
            _field_ft_keygen_ext_codes_t *ext_codes)
{
    ...
    /* Set keygen memory entry buffers */
    if (BCM_SUCCESS(rv)) {
        rv = _bcm_field_ft_keygen_type_c_extractors_mem_set(unit, ext_codes,
                    COUNTOF(keygen_mem_arr), keygen_mem_arr, ent_buf_arr);
    }
            ...
            bcmi_ft_session_profiles_t *profiles)
{
    ...
    user_entry = (BCMI_FT_GROUP(unit, flow_group_id)->flags
    ...
    if (profiles->session_key_profile_idx != 0) {
        ...
        rv = soc_profile_mem_delete(unit, profile, profile_index);
    ...
    if (profiles->session_data_profile_idx != 0) {
        ...
        rv = soc_profile_mem_delete(unit,
                &stage_fc->ft_info->session_data_profile,
                profile_index);
    ...
    if (profiles->alu_data_profile_idx != 0) {
        ...
        rv = soc_profile_mem_delete(unit,
                &stage_fc->ft_info->alu_data_profile,
                profile_index);
            ...
            bcmi_ft_session_profiles_t *profiles)
{
    ...
    BCM_IF_ERROR_RETURN(
            bcmi_esw_ft_group_extraction_hw_info_get(unit,
                    1, flow_group_id, 0, NULL, &key_ext_count));

    BCM_IF_ERROR_RETURN(
            bcmi_esw_ft_group_extraction_hw_info_get(unit,
                    0, flow_group_id, 0, NULL, &data_ext_count));
    ...
    rv = bcmi_esw_ft_group_extraction_hw_info_get(unit, 1,
                flow_group_id, ext_count, ext_ctrl_info, &key_ext_count);
    ...
    if (BCM_SUCCESS(rv)) {
        rv = _field_hx5_ft_session_key_profile_install(unit,
                    (BCMI_FT_GROUP(unit, flow_group_id)->flags
                     & BCM_FLOWTRACKER_GROUP_USER_ENTRIES_ONLY),
                    profiles, &ext_codes);
    }
    ...
    if (BCM_SUCCESS(rv)) {
        rv = bcmi_esw_ft_group_extraction_hw_info_get(unit, 0,
                flow_group_id, ext_count, ext_ctrl_info, &data_ext_count);
    }
...
/*
 * Function:
 *    _field_hx5_ft_keygen_cfg_qual_ext_ctrl_skip
 *
 * Purpose:
 *    Skip given qualifier from extraction. This qualifier
 *    will be inserted to keygen_oper after extraction at
 *    fixed position.
 *
 * Parameters:
 *    unit       - (IN) BCM device number.
        ...
        uint16 *skipped_size)
{
    ...
    switch(skip_qual) {
        case bcmFieldQualifySrcIp6:
            /* Use 16 ext ctrl sel info */
            ext_ctrl_info_count = 16;
            /* Use ext from 8 in part(1) and 0 from part(2) */
            num_exts[0] = 0; num_exts[1] = 2; num_exts[2] = 6;
            start_ext[0] = -1; start_ext[1] = 8; start_ext[2] = 0;
            /* First container id */
            start_cont = 14;
            break;
    ...
    for (level = 1; level <= 2; level++) {
        ...
        for (part_num = 0; part_num < 3; part_num++) {
            ...
            for (idx = 0; idx < num_exts[part_num]; idx++) {
                ...
                /* Calc size of skipped qualifier */
                if (level == 2) {
                    temp_size += ext_ctrl_info[ext_idx].gran;
                }
            }
        }
    }
    ...
    if (skipped_size != NULL) {
        *skipped_size = temp_size;
    }
        ...
        _field_ft_keygen_type_a_ext_t *extractors)
{
    ...
    if (filler_extractor_required == TRUE) {
...
/*
 * Function:
 *    _field_hx5_ft_keygen_oper_qual_ext_ctrl_apply
 *
 * Purpose:
 *    Apply qual_offset_info and extractor ctrl sel info
 ...
 *    BCM_E_XXX
 * Notes: Steps
 *   If CFG ext_ctrl_sel_info == 0
 *      Return - Nothing to do
 *
 *  -- Updating qual_offset_info --
 *  Form qual_offset_info for static qualifier
 *  Loop through each part
 *  Combine Old and New list.
 *
 *  -- Updating ext ctrl sel info --
        ...
        bcmi_keygen_oper_t *keygen_oper)
{
    ...
    bcm_field_qualify_t qid_arr[3][5];
    bcmi_keygen_qual_offset_t offset_arr[3];
    ...
    sal_memset(qid_arr, 0, sizeof(qid_arr));
    sal_memset(offset_arr, 0, sizeof(offset_arr));

    _FP_XGS3_ALLOC(qual_offset_info, (keygen_cfg->num_parts * \
    ...
    switch(skip_qual) {
        ...
        case bcmFieldQualifyInnerSrcIp6:
        case bcmFieldQualifyInnerDstIp6:
            for (part = 1; part < keygen_cfg->num_parts; part++) {
                qid_arr[part][0] = skip_qual;
                if (part == 1) {
                    offset_arr[part].num_offsets = 2;
                    for (offset = 0; offset < 2; offset++) {
                        offset_arr[part].offset[offset] = 128 + offset * 16;
                        offset_arr[part].width[offset] = 16;
                    }
                } else {
                    offset_arr[part].num_offsets = 6;
                    for (offset = 2; offset < 8; offset++) {
                        offset_arr[part].offset[offset] = offset * 16 - 32;
                        offset_arr[part].width[offset] = 16;
                    }
                }
                qual_offset_info[part].qid_arr = &qid_arr[part][0];
                qual_offset_info[part].offset_arr = &offset_arr[part];
    ...
    rv = _field_ft_hx5_keygen_oper_qual_offset_info_merge(unit,
            keygen_cfg->num_parts, keygen_oper, qual_offset_info);
    BCMI_IF_ERROR_CLEANUP(rv);
    ...
    ext_ctrl_sel_info = keygen_cfg->ext_ctrl_sel_info;
    ext_ctrl_sel_info_count = keygen_cfg->ext_ctrl_sel_info_count;
    rv = _bcm_field_hx5_ft_keygen_extractors_set(unit, keygen_profile,
              ext_ctrl_sel_info_count, ext_ctrl_sel_info, &ext_codes);
    BCMI_IF_ERROR_CLEANUP(rv);
