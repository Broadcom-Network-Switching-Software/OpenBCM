    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcmi_esw_portctrl_detach(unit, si->cpri.bitmap, &detached));
...
#ifdef CPRIMOD_SUPPORT
    int port;
    PBMP_ITER(PBMP_CPRI_ALL(unit), port) {
...
{
#ifdef CPRIMOD_SUPPORT
    portmod_cpri_cw_l1_protocol_info_t l1_data;

    BCM_IF_ERROR_RETURN(_bcm_esw_port_gport_validate(unit, port, &port));

    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_rx_cw_l1_signal_info_get(unit, port, &l1_data));

    l1_info->protocol_ver   = l1_data.protocol_ver;
    l1_info->hdlc_rate      = l1_data.hdlc_rate;
    l1_info->reset          = l1_data.reset;
    l1_info->l1_lof         = l1_data.l1_lof;
    l1_info->l1_los         = l1_data.l1_los;
    l1_info->l1_sdi         = l1_data.l1_sdi;
    l1_info->l1_rai         = l1_data.l1_rai;
    l1_info->eth_pointer    = l1_data.eth_pointer;
    ...
    bcm_cpri_fast_eth_config_info_t *config)
{
    ...
    config->drop_min_pkt_size   = portmod_config.min_packet_drop;
    config->drop_max_pkt_size   = portmod_config.max_packet_drop;
    ...
    bcm_cpri_cw_l1_config_info_t *entry)
{
    ...
    if ((entry->enet_ptr < 20) || (entry->enet_ptr > 63)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                 (BSL_META("enet_ptr p= %d Valid Eth Channel should be p=20..63\n"), entry->enet_ptr));
        return BCM_E_PARAM;
    }

    int_entry.enet_ptr      = entry->enet_ptr;
    int_entry.l1_fun        = entry->l1_fun;
    int_entry.hdlc_rate     = entry->hdlc_rate;
    int_entry.protocol_ver  = entry->protocol_ver;

    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_tx_cw_l1_config_set(unit, port, &int_entry));
    return (BCM_E_NONE);
#else
    return (BCM_E_UNAVAIL);
#endif
