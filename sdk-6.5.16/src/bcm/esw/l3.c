...
#define BCM_WB_DEFAULT_VERSION            BCM_WB_VERSION_1_17
...
bcm_esw_l3_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cancun)) {
        /* These two features are supported on TD3 from 6.0.0 and above */
        uint32 cancun_ver;
        ...
        if (SOC_IS_TRIDENT3(unit) && (cancun_ver >= SOC_CANCUN_VERSION_DEF_6_0_0)) {
            SOC_FEATURE_SET(unit, soc_feature_large_scale_nat);
            SOC_FEATURE_SET(unit, soc_feature_vfi_switched_l2_change_fields);
        }
    }
#endif /* BCM_TRIDENT3_SUPPORT */
