...
bcmi_xgs5_bfd_hw_init(int unit)
{
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if(SOC_IS_TRIDENT3X(unit)) {
        num_of_cpu_queues = SOC_TD3_NUM_CPU_QUEUES;
    } else
#endif
    {
    ...
    if (soc_feature(unit, soc_feature_cmicm)) {
        max_rx_channels = BCM_RX_CHANNELS;
    }
#endif /* BCM_CMICM_SUPPORT */
    chan_id = (max_rx_channels * (SOC_ARM_CMC(unit, bfd_info->uc_num))) +
               bfd_info->rx_channel;
                                 ...
                                 bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    uint8 *gal_label_info = (uint8 *)&user_gal_label;

#ifdef LE_HOST
    user_gal_label =  _shr_swap32(endpoint_info->gal_label);
#else
    user_gal_label = endpoint_info->gal_label;
#endif

    /* Decode 32bit gal label,  parse the exp, ttl value and ignore gal label field */
    user_specified_exp = (gal_label_info[2] >> 1) & 0x7;
    user_specified_ttl = gal_label_info[3];
