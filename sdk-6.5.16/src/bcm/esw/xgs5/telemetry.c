                               ...
                               int config_count)
{
    ...
    uint16 pports[SHR_ST_MAX_PORTS], mports[SHR_ST_MAX_PORTS];
...
_bcm_xgs5_st_eapp_collector_encap_build_pack (int unit,
        ...
        shr_st_msg_ctrl_collector_create_t *collector_create_msg)
{
                    ...
                    bcm_collector_t collector_id, int export_profile_id)
{
    ...
    buffer_req      = telemetry_per_unit_global_info->dma_buffer;
    buffer_ptr      = shr_st_msg_ctrl_collector_create_pack(buffer_req,
                                                        &collector_create_msg);
    buffer_len      = buffer_ptr - buffer_req;
...
int bcm_xgs5_telemetry_eapp_init(int unit)
{
     ...
     */
    telemetry_config_per_unit->dma_buffer_len = sizeof(st_sdk_msg_ctrl_init_t);
...
int _bcm_xgs5_telemetry_detach(int unit)
{
    ...
    if (BCM_FAILURE(rv)) {
     ...
     rv = soc_counter_tomahawk_telemetry_detach(unit);
     if (BCM_FAILURE(rv)) {
         LOG_ERROR(BSL_LS_BCM_TELEMETRY,
                 (BSL_META_U(unit,
                             "TELEMETRY(unit %d) Error: telemetry "
                             " counter Init failed %s\n"), unit, bcm_errmsg(rv)));
         return rv;
     }
    ...
    SOC_CONTROL(unit)->uc_telemetry_enable = 0;
...
int _bcm_xgs5_telemetry_init(int unit)
{
    ...
    telemetry_config_per_unit->max_num_ports = max_num_ports;
     ...
     SOC_CONTROL(unit)->uc_telemetry_enable = 1;

     /* Counter module settings */
     rv = soc_counter_tomahawk_telemetry_init(unit);
     if (BCM_FAILURE(rv)) {
         LOG_ERROR(BSL_LS_BCM_TELEMETRY,
                 (BSL_META_U(unit,
                             "TELEMETRY(unit %d) Error: telemetry "
...
int _bcm_xgs5_telemetry_not_empty;
