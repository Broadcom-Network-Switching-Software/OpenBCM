 ...
 */
STATIC int
_bcm_l2_station_entry_install(int unit, _bcm_l2_station_entry_t *ent_p, uint32 station_flags)
{
    int     rv;         /* Operation return status. */ 
    soc_mem_t tcam_mem; /* TCAM memory name.        */
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit) && (station_flags & BCM_L2_STATION_FCOE)) {
        tcam_mem = MY_STATION_TCAM_2m;
    } else 
#endif /* BCM_TRIDENT3_SUPPORT */
    {
 ...
 */
STATIC int
_bcm_l2_station_entry_prio_install(int unit, _bcm_l2_station_entry_t *ent_p, uint32 station_flags)
{
...
only_move:
    ...
    rv = _bcm_l2_station_entry_install(unit, ent_p, station_flags);
    if (BCM_FAILURE(rv)) {
        return rv;
    }
    return (BCM_E_NONE);
                      ...
                      bcm_l2_station_t *station)
{
    ...
    rv = _bcm_l2_station_entry_prio_install(unit, ent, station->flags);
    if (BCM_FAILURE(rv)) {
