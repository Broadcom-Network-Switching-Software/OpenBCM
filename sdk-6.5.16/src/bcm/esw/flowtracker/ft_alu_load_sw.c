...
#define BCMI_FT_ALU_LOAD_HASH_HEAD(_u) \
            (bcmi_ft_alu_table_hash[_u]->head)
...
bcmi_ft_alu_hash_init(int unit)
{
    ...
    if (NULL == bcmi_ft_alu_table_hash[unit]) {
        ...
        if (bcmi_ft_alu_table_hash[unit] == NULL) {
            /* See if you want to clear all resources. */
            return SOC_E_MEMORY;
        }
...
bcmi_ft_alu_hash_cleanup(int unit)
{
    ...
    bcmi_ft_alu_entry_list_t *temp;
    bcmi_ft_alu_table_hash_t *hash;
    int i = 0;

    for (; i < BCMI_FT_ALU_HASH_BUCKETS; i++) {
        if (bcmi_ft_alu_table_hash[unit]) {
            hash =  &(bcmi_ft_alu_table_hash[unit][i]);
            temp = hash->head_link;
                ...
                bcmi_ft_alu_entry_list_t **entry)
{
    ...
    bcmi_ft_alu_entry_list_t *new_entry;
            ...
            bcmi_ft_alu_hash_info_t *hash_alu_info)
{
    ...
    /* Load the key into hash alu state. */
    ...
    hash_alu_info->direction = BCMI_FT_GROUP_FTFP_DATA(unit, id).direction;
}

void
bcmi_ft_group_alu_load_info_set(
                int unit,
                bcm_flowtracker_group_t id,
                ...
                bcmi_ft_alu_hash_info_t *hash_alu_info)
{
    ...
    bcmi_ft_group_alu_info_t *local, *group_temp = NULL;
    int i = 0;

    if ((alu_load_type == bcmiFtAluLoadTypeAlu32) ||
       (alu_load_type == bcmiFtAluLoadTypeAlu16)) {

        /* Assign other ALU params here.
         * For loads, nothing to be assigned.
         * Loads add multiple keys into one to
         * form one key so cant assign multiple keys
         * to one hash_entry.
         */
        bcmi_ft_alu_hash_info_set(unit, id, group_alu_info, hash_alu_info);
...
/*    bcmi_ft_group_alu_info_t *local, *group_temp = NULL;*/
            ...
            if (bcmi_ft_alu_check_free_index
                ...
                *alu_load_index = temp->alu_info.alu_load_index;
                ...
                bcmi_ft_group_alu_load_info_set(unit, id, alu_load_type,
                     group_alu_info, load_indexes, *alu_load_index, NULL);
                return BCM_E_EXISTS;
            }
        }
    ...
    BCM_IF_ERROR_RETURN(bcmi_ft_alu_hash_entry_alloc(unit, &new_entry));

    /* If no index is provided by caller then get new index. */
    if (*alu_load_index == -1) {

        if (bcmi_ft_tracking_param_in_Debug_mode
            (unit, id, group_alu_info, alu_load_type)) {

            rv = bcmi_ft_chip_debug_alu_index_get
                (unit, id, group_alu_info, alu_load_type, alu_load_index);

        } else {
            rv = bcmi_ft_alu_get_free_index
                (unit, id, alu_load_type, alu_load_index);
        }
...
#if 0
    /* Insert data in hash table for future lookups. */
    if ((alu_load_type == bcmiFtAluLoadTypeAlu32) ||
       (alu_load_type == bcmiFtAluLoadTypeAlu16)) {
         ...
         * to one hash_entry.
         */
        bcmi_ft_alu_hash_info_set(unit, id, group_alu_info, &(new_entry->alu_info));
    }
#endif

    bcmi_ft_group_alu_load_info_set(unit, id, alu_load_type,
        group_alu_info, load_indexes, *alu_load_index, &(new_entry->alu_info));
...
bcmi_ft_alu_hash_entry_remove(
                ...
                int *alu_load_index)
{
    bcmi_ft_alu_entry_list_t *prev, *temp = NULL;
    ...
    while (temp != NULL) {
        if (BCM_SUCCESS(bcmi_ft_alu_load_compare(unit, id, group_alu_info,
            ...
            if ((indexes != NULL) && (indexes[0] != -1)) {
                ...
                temp_group_alu_info = group_alu_info;
            }

            if (temp->alu_info.alu_load_index == temp_group_alu_info->alu_load_index) {
                *alu_load_index = temp->alu_info.alu_load_index;

                bcmi_ft_flowchecker_list_delete(unit, (&(temp->alu_info.head)),
                    temp_group_alu_info->flowchecker_id);
...

{
    ...
    bcmi_ft_group_alu_info_t *local;
    int max_entry;
    int j = 0;

    max_entry = ((alu_load_type == bcmiFtAluLoadTypeLoad16)  ? 16 : 8);

    sal_memset(key, 0, sizeof(int) * BCMI_FT_ALU_HASH_KEY_LEN_IN_WORDS);
    ...
    LOG_INFO(BSL_LS_SOC_MPLS, (BSL_META_U(unit,
        "Printing Key : Location = %d , length = %d , is_alu = %d\n "),
         key[0], key[1], key[2]));
...

{
    ...
    bcmi_ft_hash_bucket_get(unit, alu_load_type, group_alu_info, load_indexes,
        &hash_idx);
                ...
                int *alu_load_index)
{
    ...
    /* First get the index of bucket.*/
    bcmi_ft_hash_bucket_get(unit, alu_load_type, group_alu_info,
            load_indexes, &hash_idx);
                ...
                int *alu_load_index)
{
    ...
    /* We have got the hash bucket. Now remove the ALU from this list. */
    rv = bcmi_ft_alu_hash_entry_index_get(unit, id, alu_load_type, group_alu_info,
            hash_input, load_indexes, alu_load_index);
...
bcmi_ft_alu_load_hash_dump(int unit)
{
    ...
    char *strlist[] = {"LOAD8", "LOAD16", "ALU16", "ALU32"};
        ...
        for (i = 0; i < BCMI_FT_ALU_HASH_BUCKETS; i++) {
            ...
            if (alu_hash->head_link != NULL) {
                ...
                while(alu_entry != NULL) {
                    LOG_CLI((BSL_META_U(unit, "Type: %s Idx: %d key:"
                                    " (%d - %d / %d) \n"),
    ...
    for (i=bcmiFtAluLoadTypeLoad8; i<bcmiFtAluLoadTypeNone; i++) {
        ...
        for (j = 0; j < total_indexes_per_mem; j++) {
            if (alu_load_index_info[unit][i].idx_refcount[j] != 0) {
                LOG_CLI((BSL_META_U(unit, "%d(%d), "), j,
                            alu_load_index_info[unit][i].idx_refcount[j]));
            }
        }
        LOG_CLI((BSL_META_U(unit, "\n")));
    }
...
int bcmi_ft_alu_load_sw_not_empty;
