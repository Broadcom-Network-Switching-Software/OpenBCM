...
bcm_ptp_clock_apts_mode_t g_apts_current_op_mode = bcmPtpClockAptsModePtpPtp;
...
void _bcm_ptp_apts_update_current_mode()
{
    ...
    {
        if((g_apts_op_mode_lists[g_apts_op_mode_list_idx].op_mode_list[i].required_sources & g_apts_current_source_state) == g_apts_op_mode_lists[g_apts_op_mode_list_idx].op_mode_list[i].required_sources){
    ...
    if (old_op_mode != g_apts_current_op_mode){
        ...
        cli_out("[APTS] Operating mode changed <Freq source|Time source>: %s -> %s", old_op_mode_str, new_op_mode_str);
    }
    return;
}
...
{
    int rv = BCM_E_NONE;
    priority_list = g_apts_usr_cfgd_priority_list;
    return rv;
}
    ...
    int enable)
{
    ...
    if (source >= 0x08) {
        ...
        g_apts_current_source_state &= (source_avail | 0x4);
        _bcm_ptp_apts_update_current_mode();
        rv = BCM_E_NONE;
    }
    return rv;
