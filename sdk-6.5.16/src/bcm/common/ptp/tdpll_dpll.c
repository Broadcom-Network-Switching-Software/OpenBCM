...
extern int _bcm_esw_tdpllOutinstance_bind(int unit, int outclock_index, uint32 tdpll_instance_num);
#endif
    ...
    bcm_tdpll_dpll_bindings_t *bindings)
{
...
#if defined(BCM_MONTEREY_SUPPORT) & defined(INCLUDE_GDPLL)
    /* Bind the output clock(synthesizer) to the instance */
    for (j = 0; j < TDPLL_OUTPUT_CLOCK_NUM_MAX; j++) {
        output_mask = DPLL(dpll_index).bindings.output_clocks[_SHR_BITDCLSIZE(j+1)-1];

        /* Get the bit position */
        i = j - (j/32)*32;

        /* If the mask is set for outclock, call the gdpll instance bind to output */
        if (output_mask & (1 << i)) {
            _bcm_esw_tdpllOutinstance_bind(unit, j, dpll_index);
        }
    }
#endif
    ...
    for (el = 0; el < TDPLL_DPLL_INSTANCE_NUM_MAX; ++el) {
        ...
        DPLL(el).bindings.output_clocks[el/SHR_BITWID] ^= (output_mask & DPLL(el % SHR_BITWID).bindings.output_clocks[el/SHR_BITWID]);
...
/*
 * Function:
 *      _bcm_common_tdpll_dpll_binding_lookup()
 * Purpose:
 *      Get DPLL instance number associated with output clock/synthesizer.
 * Parameters:
 *      clock_index - (IN)  Output clock number.
 *      dpll_index  - (OUT) DPLL instance number.
 * Returns:
 *      None.
 * Notes:
 *      A negative answer (DPLL index) signifies that the synthesizer is not
 *      bound to any logical DPLL instances.
 *
 *      ASSUMPTION(s) - Host code guarantees that a synthesizer is bound to
 *      only one DPLL instance. Design allows N-to-1 mappings, i.e. multiple
 *      synthesizers to one DPLL, but does not permit 1-to-N mappings, i.e.
 *      one synthesizer to multiple DPLLs.
 */
int _bcm_common_tdpll_dpll_outclock_binding_lookup(int clock_index, int *dpll_index)
{
    int i;
    *dpll_index = -1;
    ...
    for (i = 0; i < TDPLL_DPLL_INSTANCE_NUM_MAX; ++i) {
#if defined(BCM_MONTEREY_SUPPORT)
        if (DPLL(i).bindings.output_clocks[i/SHR_BITWID] & (1 << (clock_index % SHR_BITWID))) {
#else
        if (DPLL(i).bindings.output_clocks & (1 << clock_index)) {
#endif
            /* Synthesizer is bound to this DPLL. */
            *dpll_index = i;
            break;
        }
    }
...
int _bcm_common_tdpll_dpll_outclock_bandwidth_get(int clock_index, bcm_tdpll_dpll_bandwidth_t *bandwidth)
{
    int dpll_index = -1;
    int rv = BCM_E_UNAVAIL;
    bandwidth->value = -1;

    if (BCM_FAILURE(rv = _bcm_common_tdpll_dpll_outclock_binding_lookup(clock_index, &dpll_index))) {
        PTP_ERROR_FUNC("_bcm_common_tdpll_dpll_outclock_binding_lookup()");
        return rv;
    }

    *bandwidth = DPLL(dpll_index).properties.bandwidth;
    return BCM_E_NONE;
}
