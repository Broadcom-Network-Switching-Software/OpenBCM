...
/*
 * Function:
 *      bcm_srv6_terminator_next_protocol_mapping_t_init
 * Purpose:
 *      Initialize an SRv6 next protocol mapping struct
 * Parameters:
 *      intf - pointer to the next protocol mapping struct
 * Returns:
 *      NONE
 */

extern void
bcm_srv6_terminator_next_protocol_mapping_t_init(bcm_srv6_terminator_next_protocol_mapping_t *info)
{
    if (info != NULL) {
        sal_memset(info, 0, sizeof (*info));
    }
