 ...
 */
STATIC int
_bcm_th3_modules_deinit(int unit)
{
    ...
    rv =  bcm_tomahawk3_flowtracker_init(unit);
...
_bcm_th3_modules_init(int unit)
{
    ...
    if (SOC_FAST_REBOOT(unit)) {
        ...
        PBMP_PORT_ITER(unit, port_iter) {
            BCM_IF_ERROR_RETURN(bcm_esw_port_control_set(unit, port_iter,
                        bcmPortControlMmuTrafficEnable, FALSE));
    ...
    if (SOC_FAST_REBOOT(unit)) {
        ...
        PBMP_PORT_ITER(unit, port_iter) {
            BCM_IF_ERROR_RETURN(bcm_esw_port_control_set(unit, port_iter,
                        bcmPortControlMmuTrafficEnable, TRUE));
            /* In TH3, MMU is drained when queuing is disabled. No need to do
             * bcmPortControlMmuDrain, BUT PROBABLY NEED TO IN OTHER DEVICES! */
        }
    }
...
_bcm_th3_detach(int unit)
{
    ...
    rv = _bcm_th3_modules_deinit(unit);
