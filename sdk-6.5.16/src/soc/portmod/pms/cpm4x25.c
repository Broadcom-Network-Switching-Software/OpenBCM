...
#define CPM4x25_PHY_ACCESS_GET(unit, port, pm_info, phy_acc) \
    do { \
        uint32 is_bypass; \
        phy_acc = 0; \
        _SOC_IF_ERR_EXIT(CPM4x25_IS_BYPASSED_GET(unit, pm_info, &is_bypass)); \
...
int _cpm4x25_pm_port_init(int unit, int port, pm_info_t pm_info, int enable)
{
    ...
    for (i = 0 ; i < MAX_PORTS_PER_PM4X25; i++) {
       rv = CPM4x25_LANE2PORT_GET(unit, pm_info, i, &tmp_port);
       _SOC_IF_ERR_EXIT(rv);
...
int cpm4x25_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    */
    ...
    rv = CPM4x25_IS_ACTIVE_GET(unit, pm_info, &pm_is_active);
    _SOC_IF_ERR_EXIT(rv);
    if(!pm_is_active){
        three_ports_mode = (add_info->init_config.port_mode_aux_info == portmodModeInfoThreePorts) ? 1 : 0;
        rv = CPM4x25_THREE_PORTS_MODE_SET(unit, pm_info, three_ports_mode);
        _SOC_IF_ERR_EXIT(rv);
        rv = pm4x25_to_cpm_pm_enable(unit, port, pm_info, port_index, add_info, 1);
        _SOC_IF_ERR_EXIT(rv);
        pm_is_active = 1;
        rv = CPM4x25_IS_ACTIVE_SET(unit, pm_info, pm_is_active);
        _SOC_IF_ERR_EXIT(rv);
    }
    /*
    ...
    */
    ...
    if (!PORTMOD_CORE_INIT_FLAG_CPRI_CORE_INITIALZIED_GET(core_is_initialized)) {
        if(!PORTMOD_CORE_INIT_FLAG_FIRMWARE_LOADED_GET(core_is_initialized) ||
            ...
            _SOC_IF_ERR_EXIT(_pm4x25_txpi_lane_select_set(unit, port, pm_info, (uint32 *)add_info->init_config.lane_map[0].lane_map_tx));
...
int cpm4x25_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    for( i = 0 ; i < MAX_PORTS_PER_PM4X25; i++){
        ...
        if(tmp_port == port){
            ...
            rv = CPM4x25_LANE2PORT_SET(unit, pm_info, i, invalid_port);
            _SOC_IF_ERR_EXIT(rv);
        } else if (tmp_port != -1){
            is_last_one = FALSE;
        }
    ...
    if(is_last_one){
       ...
       rv = CPM4x25_IS_ACTIVE_SET(unit, pm_info, inactive);
       _SOC_IF_ERR_EXIT(rv);

       rv = CPM4x25_IS_INITIALIZED_SET(unit, pm_info, inactive);
       _SOC_IF_ERR_EXIT(rv);

       rv = CPM4x25_IS_BYPASSED_SET(unit, pm_info, inactive);
       _SOC_IF_ERR_EXIT(rv);
...
int cpm4x25_port_replace(int unit, int port, pm_info_t pm_info, int new_port)
{
    ...
    for(i = 0; i < MAX_PORTS_PER_PM4X25; i++) {
        ...
        _SOC_IF_ERR_EXIT(rv);
        if (tmp_port == port) {
            rv = CPM4x25_LANE2PORT_SET(unit, pm_info, i, new_port);
            _SOC_IF_ERR_EXIT(rv);
        }
    }
...
int cpm4x25_port_interface_config_get(int unit, int port, pm_info_t pm_info, portmod_port_interface_config_t* config, int phy_init_flags)
{
    ...
    if (config_t.interface == cprimodCpri) {
...
{
    cprimod_cpri_container_config_t container_config;
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_cpri_container_config_t_init(&container_config));

    _SOC_IF_ERR_EXIT
        (cprimod_cpri_rx_axc_container_config_set(unit, port, axc_id, &container_config));
...
int cpm4x25_cpri_port_tx_axc_frame_delete(int unit, int port, pm_info_t pm_info, uint32 axc_id)
{
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_cpri_tx_axc_container_config_set(unit, port, axc_id, &container_config));
                                        ...
                                        portmod_rsvd4_rx_flow_info_t* info)
{
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_rx_roe_payload_size_set(unit, port, container_info.axc_id, &payload_info));
...
int cpm4x25_cpri_port_rsvd4_rx_flow_delete(int unit, int port, pm_info_t pm_info, uint32 flow_id)
{
   ...
   _SOC_IF_ERR_EXIT
    ...
    /* clear the relate container parser entry. */
    _SOC_IF_ERR_EXIT
        (cprimod_rsvd4_container_config_t_init(&container_config));

    _SOC_IF_ERR_EXIT
        (cprimod_rsvd4_rx_axc_container_config_set(unit, port, flow_id, &container_config));
   ...
   _SOC_IF_ERR_EXIT
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_rx_roe_payload_size_set(unit, port, queue_num, &roe_payload_info));
...
int cpm4x25_cpri_port_rsvd4_tx_flow_add(int unit, int port, pm_info_t pm_info, uint32 flow_id, portmod_rsvd4_tx_flow_info_t* info)
{
    ...
    payload_info.last_packet_size   = info->last_packet_size ;
    _SOC_IF_ERR_EXIT
        (cprimod_tx_roe_payload_size_set(unit, port, container_info.axc_id, &payload_info));
...
int cpm4x25_cpri_port_rsvd4_tx_flow_delete(int unit, int port, pm_info_t pm_info, uint32 flow_id)
{
   ...
   _SOC_IF_ERR_EXIT
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_tx_roe_payload_size_set(unit, port, queue_num, &roe_payload_info));
exit:
    SOC_FUNC_RETURN;
...
int cpm4x25_cpri_port_tx_vsd_flow_add(int unit, int port, pm_info_t pm_info, uint32 group_id, const portmod_cpri_tx_vsd_flow_info_t* flow_info)
{
    ...
    CPM4x25_CPRI_PORT_VSD_ACTIVE_GROUP_GET(unit, pm_info, group_list, lane_index);

    _SOC_IF_ERR_EXIT
        (cprimod_cpri_port_tx_vsd_ctrl_flow_add(unit, port, group_id, group_list, flow_info));

    _SOC_IF_ERR_EXIT
        (CPM4x25_CPRI_PORT_VSD_ACTIVE_GROUP_SET(unit, pm_info, group_list, lane_index));
exit:
    SOC_FUNC_RETURN;
...
int cpm4x25_cpri_port_tx_cw_sync_info_get(int unit, int port, pm_info_t pm_info, uint32* hyper_frame_num, uint32* radio_frame_num)
{
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_tx_framer_tgen_next_hfn_config_get(unit,port,hyper_frame_num));
    _SOC_IF_ERR_EXIT
        (cprimod_tx_framer_tgen_next_bfn_config_get(unit,port,radio_frame_num));
                                                ...
                                                int* is_link_intr_status)
{
     ...
     */
    PORTMOD_PBMP_ITER(PM_4x25_INFO(pm_info)->phys, phy){
      _SOC_IF_ERR_EXIT(CPM4x25_LANE2PORT_GET(unit, pm_info, ii, &lport));
...
int cpm4x25_cpri_port_interrupt_process(int unit, int port, pm_info_t pm_info)
{
    ...
    PORTMOD_PBMP_ITER(PM_4x25_INFO(pm_info)->phys, phy){
        ...
        /* Only if CPRI port */
        if (val) {
            _SOC_IF_ERR_EXIT(cprimod_cpri_port_interrupt_process(unit, lport));
        }
         ii++;
    }
...
int cpm4x25_cpri_port_ecc_interrupt_status_get(int unit, int port,
                                    pm_info_t pm_info,
                                    portmod_cpri_ecc_intr_info_t* ecc_err_info)
{
    ...
    if(val) {
        _SOC_IF_ERR_EXIT(cprimod_cpri_port_ecc_interrupt_status_get(unit,
                                                          port, ecc_info));
    } else {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
              (_SOC_MSG("Can't operate on Non-CRPI port %d"), port));
    }
                                                ...
                                                const portmod_cpri_rx_tunnel_mode_config_t* config_info)
{
     ...
     */
    _SOC_IF_ERR_EXIT
        (cprimod_cpri_port_rx_tunnel_mode_set(unit, port, enable, config_info->payload_size));

    /*
     *  Setup the basic frame and framer. For both enable and disable
     */
    _SOC_IF_ERR_EXIT
        (cprimod_cpri_tunnel_mode_basic_frame_config_set(unit, port, CPRIMOD_DIR_RX, enable));
...
int cpm4x25_cpri_port_tx_tunnel_mode_config_set(int unit, int port, pm_info_t pm_info, int enable, const portmod_cpri_tx_tunnel_mode_config_t* config_info)
{
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_cpri_tunnel_mode_basic_frame_config_set(unit, port, CPRIMOD_DIR_TX, enable));
...
int cpm4x25_rsvd4_port_rx_agnostic_config_set(int unit, int port, pm_info_t pm_info, int enable, const portmod_rsvd4_rx_agnostic_config_t* config_info)
{
    ...
    if (enable) {
        ...
        container_info.message_number_offset    = config_info->message_group_number*21;
    ...
    _SOC_IF_ERR_EXIT
        (cprimod_port_rsvd4_rx_frame_optional_config_set(unit, port,
                                                         cprimodRsvd4RxConfigFwdIdlesToFramer, 
                                                         enable));
    /*
...
int cpm4x25_rsvd4_port_tx_agnostic_config_get(int unit, int port, pm_info_t pm_info, int* enable, portmod_rsvd4_tx_agnostic_config_t* config_info)
{
    ...
    if (*enable && (mode == cprimod_agnostic_mode_rsvd4)) {
        ...
        config_info->message_group_number   = container_info.message_number_offset/21;
...
int cpm4x25_cpri_port_frame_config_set(int unit, int port, pm_info_t pm_info, const portmod_cpri_port_frame_config_t* config)
{
    ...
    switch (config->frame_type) {
        ...
        case PORTMOD_CPRI_ROE_FRAME_FORMAT_ECPRI12:
            ...
            encap_map.hdr_byte6_sel  = CPRIMOD_CPRI_ROE_HEADER_FIELD_SUBTYPE ;
            encap_map.hdr_byte7_sel  = CPRIMOD_CPRI_ROE_HEADER_FIELD_FLOW_ID;
            encap_map.hdr_byte8_sel  = CPRIMOD_CPRI_ROE_HEADER_FIELD_UNMAPPED;
