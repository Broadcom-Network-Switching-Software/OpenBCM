...
#define PM4x25_WB_BUFFER_VERSION        (13)
...
typedef enum pm4x25_fabric_wb_vars{
    ...
    portDynamicState,
    tsAdjustOld, /* the time stamp adjust setting on MAC ** removed ** */
...
int _pm4x25_pm_version_get(int unit, pm_info_t pm_info, pm_version_t *version)
{
    ...
    if (SOC_REG_IS_VALID(unit, CLPORT_PM_VERSION_IDr) &&
        ...
        raddr = soc_reg_addr_get(unit, CLPORT_PM_VERSION_IDr, phy_port, 0,
...
int _pm4x25_port_tsc_is_master_pll(int unit, int port, pm_info_t pm_info)
{
   ...
   if (PORTMOD_PM4x25_IS_GEN1_GEN2(pm_version)) {
        _SOC_IF_ERR_EXIT(READ_CLPORT_XGXS0_CTRL_REGr(unit, phy_acc, &reg_val));
                                     ...
                                     int phy_init_flags)
{
    ...
    if (config->flags & PHYMOD_INTF_F_UPDATE_SPEED_LINKUP) {
        ...
        if (mac_ts_enable) {
            ...
            /* PM VERSION and CLMAC BYTE ADJUST aren't valid before TH2 */
