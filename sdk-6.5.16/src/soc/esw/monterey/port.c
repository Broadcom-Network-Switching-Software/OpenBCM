    ...
    for (phy_port = 0; phy_port < SOC_MAX_NUM_PORTS; phy_port++) {
        ...
        for (i = 0; i < SOC_DRIVER(unit)->port_num_blktype; i++) {
            ...
            if (SOC_BLOCK_TYPE(unit, blk) == SOC_BLK_CXXPORT) {
                MN_PHY_PORT(unit, phy_port).lanes_valid |=
                    MN_PHY_XLPORT_LANES_10 | MN_PHY_XLPORT_LANES_12;
            }
                            ...
                            soc_info_mn_misc_t *post_si)
{
    ...
    _soc_mn_soc_info_mn_misc_t_init(unit, post_si);
                                         ...
                                         soc_info_mn_misc_t *post_si)
{
    ...
    uint32 sp_spacing = 0;
    int sister_speed,lanes_sister[MN_PORTS_PER_TSC];
     ...
     for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (type != portmodDispatchTypePm4x10) {
            rv =  soc_esw_portmod_port_vcos_get(unit , phy_port, type, lanes_sister,
                                      speed_sister , encap_sister, size); 
        }
        if (rv != SOC_E_NONE) {  
...
soc_mn_port_mixed_sister_speed_validate(int unit)
{
    ...
    /* coverity[overrun-local] */
             ...
             } else if ((SOC_PBMP_MEMBER(pbmp_xport_cpri, sister_lport)) && (roe_speed_cpri > 0)) {
                 encap_sister[size] = SOC_ENCAP_CPRI;
                 speed_sister[size] = roe_speed_cpri;
             } else if ((SOC_PBMP_MEMBER(pbmp_xport_cpri, sister_lport)) && (roe_speed_rsvd4 > 0)) {
        ...
        for (i = 1; i < MN_PORTS_PER_TSC; i++) {
            sister_port = MN_PHY_PORT(unit, phy_port).sisters_ports[i];
                                   ...
                                   soc_port_resource_t *post_resource)
{
    ...
    for (i = 0, pr = &post_resource[0]; i < post_count; i++, pr++) {
        ...
        if (SOC_SUCCESS(soc_monterey_port_reg_blk_index_get
                    (unit, logic_port, SOC_BLK_CXXPORT, &blk))) {
            si->block_valid[blk] += 1;

            if (si->block_port[blk] < 0) si->block_port[blk] = logic_port;
        }
    ...
    for (i = 0, pr = &post_resource[0]; i < post_count; i++, pr++) {
        ...
        for (port_idx = 0; port_idx < SOC_DRIVER(unit)->port_num_blktype; port_idx++) {
            ...
            case SOC_BLK_CLG2PORT:
                ...
                break;
            case SOC_BLK_CPRI:
               if(IS_CPRI_PORT(unit, logic_port) &&
...
soc_monterey_flexport_scache_recovery(int unit)
{
    ...
    sal_memcpy(&si->all.disabled_bitmap,
           ...
           sizeof(pbmp_t));
    scache_offset += sizeof(pbmp_t);
    if (recovered_ver > SOC_FLEXPORT_WB_VERSION_1_0) {
