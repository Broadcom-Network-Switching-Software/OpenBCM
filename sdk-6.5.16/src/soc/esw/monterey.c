...
#if defined(BCM_56670_B0) && defined(INCLUDE_XFLOW_MACSEC)
 ...
 */
    tdm_freq = soc_property_get(unit, spn_BCM_TDM_FREQUENCY,si->frequency); 
    if (IS_ROE_ENABLED(unit)) {
        if (tdm_freq == 815 || tdm_freq == 666) {
            mn_mmu_express_buffer_total = _MN_MMU_EXPRESS_BUFFER_TOTAL_1;
        } else {
            mn_mmu_express_buffer_total = _MN_MMU_EXPRESS_BUFFER_TOTAL_2;
        }
    } else {
...
soc_mn_port_resource_tdm_calculate(int unit, soc_monterey_tdm_t *tdm)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        if(IS_CPRI_PORT(unit,port) || IS_RSVD4_PORT(unit,port)) {
           if (si->port_speed_max[port] > 10100)  {
...
#ifdef INCLUDE_XFLOW_MACSEC
            /* ESEC_SA_TABLEm has 4 banks of memory with individyal SER control and status */
            if (soc_feature(unit, soc_feature_xflow_macsec) &&
                    mem == ESEC_SA_TABLEm) {
                switch (index % 4) {
                    case 0:
                                 ...
                                 void *d2, void *d3, void *d4)
{
    ...
    for (rbi = 0; ; rbi++) {
        ...
        if(soc_reg64_field32_get(unit, rb->status_reg, rb_rval64,
                                 ECCERR_INTf)) {
            (void)xflow_macsec_process_ecc_intr(unit);
        }
