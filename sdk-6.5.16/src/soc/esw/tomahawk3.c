...
#define BCM_TH3_MAX_LOGICAL_PORT_NUM    (160)
    ...
    for (port = 0; port < BCM_TH3_MAX_LOGICAL_PORT_NUM; port++) {
        ...
        /* If the port is mapped, find out its fec, link training, lane config
         * from config file */
     ...
     * or WB, calculate the port macro VCOs for later PM init
     */
...
_soc_th3_pp_pll_config(int unit, int freq)
{
    ...
    for (pp_idx = 0; pp_idx < sizeof(pp_pll)/sizeof(pp_pll[0]); ++pp_idx) {
...
STATIC
int _soc_tomahawk3_activate_pm8x50(int unit, int phy_port)
{
    uint8 at;
    uint32 raddr, reg_val;
    soc_block_t block;
    soc_reg_t reg = CDPORT_XGXS0_CTRL_REGr;
    int sleep_usec = SAL_BOOT_QUICKTURN ? 500000 : 1100;

    /* index the register by phy port to activate the PM */
    raddr = soc_reg_addr_get(unit, reg, phy_port, 0,
...
{
    int i, blk, pm, port, phy_port = 1;
    uint32 pipe_map;
    soc_info_t * si;
    int pm_reset[_TH3_PBLKS_PER_DEV];
    uint16 dev_id;
    uint8 rev_id;

    si = &SOC_INFO(unit);

    sal_memset(pm_reset, 0, sizeof(pm_reset));

    /* reset the PMs that have mappings */
    SOC_BLOCK_ITER(unit, blk, SOC_BLK_CDPORT) {
        port = SOC_BLOCK_PORT(unit, blk);

        phy_port = si->port_l2p_mapping[port];
        /* mark that this PM has been reset */
        pm_reset[(phy_port-1)/_TH3_PORTS_PER_PBLK] = 1;
    ...
    /* reset any master PMs that haven't been reset yet.
     * In TH3 A0 the master PMs are 1, 2, 5, 6, 9, 10, 13, 14, 17,
     *                              18, 22, 22, 26, 26, 29, 30 */
    if (rev_id == BCM56980_A0_REV_ID) {
        for (i = 0; i < 2; i++) {
            for (pm = 1 + i; pm < _TH3_PBLKS_PER_DEV; pm+=4) {
                /* skip if the pm has already been reset */
                if (pm_reset[pm] == 1) {
                    continue;
                }
                ...
                /* get the first phy port of the PM */
                phy_port = (pm * _TH3_PORTS_PER_PBLK) + 1;

                /* Activate the PM */
                SOC_IF_ERROR_RETURN(
                        _soc_tomahawk3_activate_pm8x50(unit, phy_port));
            }
        }
    } else {
        /* In TH3 B0 the master PMs are 3, 4, 11, 12, 19, 20, 27, 28 */
        for (pm = 0; pm < _TH3_PBLKS_PER_DEV; pm++) {
            /* Skip if not a master PM */
            if ( !(pm == 3 || pm == 4 || pm == 11 || pm == 12 ||
                   pm == 19 || pm == 20 || pm == 27 || pm == 28) ) {
                continue;
            }
