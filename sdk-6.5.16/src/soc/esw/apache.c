    ...
    do {                                                    \
        ...
        if ((speed > 0) && (speed < 10000)) {               \
            if (SOC_INFO(unit).oversub_mode != 2) {         \
                speed = 10000;                              \
            } else if (IS_OVERSUB_PORT(unit, port)) {       \
                speed = 10000;                              \
            } else if (speed < 1000) {                      \
                speed = 1000;                               \
            } else if (speed == 5000) {                     \
...
soc_apache_port_is_11g_forced(int unit, int phy_port)
{
     ...
     */
    bit_pos = (tsc_id >= APACHE_TSCS_PER_PGW) ? (tsc_id - APACHE_NUM_XLP_PER_PGW) : tsc_id;

    /* Subtract 7 TSC eagles for PGW 0 */
    bit_pos -= APACHE_NUM_XLP_PER_PGW;
...
#if defined(BCM_56560_A0) || defined(BCM_56560_B0)
                    if (SOC_PBMP_MEMBER(pbmp, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 16) + 1]) &&
                            !IS_OVERSUB_PORT(unit, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 16) + 1]) &&
                            SOC_PBMP_MEMBER(pbmp, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 17) + 1]) &&
                            !IS_OVERSUB_PORT(unit, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 17) + 1])) {
                        /* TSC16 and TSC 17 is in line rate */
                        *core_clk_freq = 797;
                    } else if ( SOC_PBMP_MEMBER(pbmp, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 6) + 1]) &&
                            !IS_OVERSUB_PORT(unit, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 6) + 1]) &&
                            SOC_PBMP_MEMBER(pbmp, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 9) + 1]) &&
                            !IS_OVERSUB_PORT(unit, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 9) + 1])) {
