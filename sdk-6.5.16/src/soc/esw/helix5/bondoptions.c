...
soc_helix5_bond_info_init(int unit)
{
    ...
    if (val == 1) {
        SOC_BOND_INFO_FEATURE_SET(unit, socBondInfoBskSessionDepth8k);
    } else if (val == 2) {
        SOC_BOND_INFO_FEATURE_SET(unit, socBondInfoBskSessionDepth16k);
    } else if (val == 3) {
        SOC_BOND_INFO_FEATURE_SET(unit, socBondInfoBskSessionDepth32k);
    }

    /* ICID */
    for (i = 0; i < (HX5_ICID_LENGTH_BYTES/4); i++) {
        /* For HX5 the ICID start from 1440 {20*(64+8)}. 64 are user bits and 8 are ECC bits. */
        bit_val =  1440 + (i * 32);
        SOC_BOND_OPTIONS_COPY(bond_info->ic_id[i], uint32, \
                bond_info_regval, bit_val, bit_val + 32);
    }
