...
soc_pstats_sbusdma_desc_setup(int unit)
{
    ...
    for (ti = 0; ti < soc->pstats_tbl_desc_count; ti++) {
        ...
        if (tbl_desc[ti].mor_dma) {
            if (soc->pstats_flags & (_TD3_MMU_PSTATS_MOR_EN | _TH2_MMU_PSTATS_MOR_EN | _HX5_MMU_PSTATS_MOR_EN)) {
...
soc_pstats_sync(int unit)
{
    ...
    for (ti = 0; ti < soc->pstats_tbl_desc_count; ti++) {
        tbl_ctrl = pstats_tbl_ctrl[unit][ti];
        if (!tbl_desc[ti].mor_dma ||
            !!(soc->pstats_flags & (_TD3_MMU_PSTATS_MOR_EN | _TH2_MMU_PSTATS_MOR_EN | _HX5_MMU_PSTATS_MOR_EN)) ==
        ...
        if (tbl_ctrl && desc_handle) {
            ...
            LOG_INFO(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                      ...
                      unit, ti,
                      tbl_desc[ti].mor_dma && (soc->pstats_flags & (_TD3_MMU_PSTATS_MOR_EN | _TH2_MMU_PSTATS_MOR_EN | _HX5_MMU_PSTATS_MOR_EN)) ? "MOR" : "Non-MOR",
