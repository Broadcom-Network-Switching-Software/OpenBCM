...
{
    soc_port_details_t *port_details, *port_save;
    int count = 0, port;


    port_details = sal_alloc(
                   sizeof(soc_port_details_t) * (SB2_MAX_PHYSICAL_PORTS+1),
                  "sb2_block_ports_t");

    if (port_details == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
           "INTERNAL: Memory allocation failed for port_details\n")));
        return 0;
    }

    sal_memcpy(port_details,
            sb2_port_details[_sb2_config_id[unit]],
            sizeof(soc_port_details_t) * (SB2_MAX_PHYSICAL_PORTS+1));
    port_save = port_details;
    while (count < SB2_MAX_PHYSICAL_PORTS) {
           if ((port_details->port_type == 0) &&
              (port_details->port_speed == 0)) {
              for (port = port_details->start_port_no;
                   port <= port_details->end_port_no;
                   port++) {
                   if (port == 0) {
                       continue;
                   }
