...
soc_counter_tomahawk_eviction_flags_update(int unit, uint32 flags, int enable)
{
    ...
    for (idx = 0; idx < SOC_COUNTER_NON_DMA_END - SOC_COUNTER_NON_DMA_START;
        ...
        if (enable && SOC_CONTROL(unit)->uc_telemetry_enable) {
            /* Disable CLEAR_ON_READ for telemetry feature */
...
#if defined(INCLUDE_TELEMETRY)
        if (SOC_CONTROL(unit)->uc_telemetry_enable) {
            for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
...
/*
 * Function:
 *      soc_counter_tomahawk_telemetry_init
 * Purpose:
 *      Disable counter eviction and clear on read for counters
 *      used by the Telemetry EApp
 * Parameters:
 *      unit         - (IN) BCM device number
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int soc_counter_tomahawk_telemetry_init(int unit)
{
    soc_control_t *soc;
    soc_counter_non_dma_t *non_dma = NULL;
    soc_reg_t reg;
...
/*
 * Function:
 *      soc_counter_tomahawk_telemetry_detach
 * Purpose:
 *      Reset the telemetry speicifc settings to default
 *
 * Parameters:
 *      unit         - (IN) BCM device number
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int soc_counter_tomahawk_telemetry_detach(int unit)
{
    soc_control_t *soc;
    soc_counter_non_dma_t *non_dma = NULL;
    soc_reg_t reg;
