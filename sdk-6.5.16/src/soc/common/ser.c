    ...
    for (m = 0; m < num_mems; m++) {
        ...
        if (SOC_IS_TRIDENT3X(unit)) {
            if ((mem == IFP_POLICY_TABLE_WIDEm) && (reported_mem == IFP_POLICY_TABLEm)) {
                vindex = vindex / SOC_TD3_FP_SLICE_ENTRY_NARROW_VIEW *
                         SOC_TD3_FP_SLICE_ENTRY_WIDE_VIEW +
                         vindex % SOC_TD3_FP_SLICE_ENTRY_WIDE_VIEW;
                 ...
                 */
                if (m == 0) {
                    vindex = vindex / SOC_TD3_FP_SLICE_ENTRY_WIDE_VIEW *
                             SOC_TD3_FP_SLICE_ENTRY_NARROW_VIEW +
                             vindex % SOC_TD3_FP_SLICE_ENTRY_WIDE_VIEW;
                } else if (m == 1) {
                    vindex = vindex / SOC_TD3_FP_SLICE_ENTRY_WIDE_VIEW *
                             SOC_TD3_FP_SLICE_ENTRY_NARROW_VIEW +
...
for (i = 0; hash_mem[i] != INVALIDm; i++) {
    ...
    } else if (SOC_FAILURE(soc_l2x_freeze(unit))) {
    ...
    } else if (SOC_FAILURE(soc_l2x_thaw(unit))) {
