...
#ifdef BCM_DNX_SUPPORT
                if (SOC_IS_DNX(unit) && ((regblktype[0] == SOC_BLK_CDPORT) ||
...
#ifdef BCM_MONTEREY_SUPPORT
                if (regblktype[0] == SOC_BLK_CPRI && !IS_CPRI_PORT(unit, ainfo.port )) { 
		    continue;  	
                }  
#endif
                             ...
                             soc_block_t block, int acc_type, uint32 addr)
{
    ...
    for (reg = 0; reg < NUM_SOC_REG; reg++) {
        ...
        if (SOC_IS_DNX(unit) && (SOC_BLOCK_TYPE(unit, actual_blk) == SOC_BLK_CDMAC || SOC_BLOCK_TYPE(unit, actual_blk) == SOC_BLK_CDPORT)) {
            ...
            if (SOC_IS_DNX(unit) &&
              ((SOC_BLOCK_TYPE(unit, actual_blk) == SOC_BLK_CDPORT && *SOC_REG_INFO(unit, reg).block == SOC_BLK_CDMAC) ||
               (SOC_BLOCK_TYPE(unit, actual_blk) == SOC_BLK_CDMAC && *SOC_REG_INFO(unit, reg).block == SOC_BLK_CDPORT))) {
                /* In Jer2 CDPORT and CDMAC are the same sbus blocks */
            } else
#endif
            continue;
        }
