...
soc_cmicx_miim_divider_ring_delay_set(int unit, int ring_index, int out_delay)
{
    ...
    if (out_delay != MIIM_INTERNAL_EXTERNAL_DELAY_INVALID) {
...
/** see .h file   */
int
soc_cmicx_miim_operation(int unit, int is_write, int clause, int int_delay, int ext_delay,
                            uint32 phy_id, uint32 phy_reg_addr, uint16 *phy_data)
{
    ...
    int current_out_delay = 0, new_out_delay = 0;
    SOC_INIT_FUNC_DEFS;

#if (defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT))
    if (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) {
        if (is_write && dnxc_ha_is_access_disabled(unit,DNXC_HA_ALLOW_SCHAN))
        {
    ...
    */
     ...
     */
    _SOC_IF_ERR_EXIT(soc_cmicx_miim_divider_ring_delay_get(unit, real_bus_id, &current_out_delay));
    new_out_delay = internal_select ? int_delay : ext_delay;
    /**
     * Change only if new and old are different and if an actual valid delay value was rovided
     */
    if(new_out_delay != current_out_delay && new_out_delay != MIIM_INTERNAL_EXTERNAL_DELAY_INVALID) {
    ...
    if (clause45){
        LOG_VERBOSE(BSL_LS_SOC_MIIM,
                 (BSL_META_U(unit,
                  ...
                  is_write ? "write" : "read ", phy_id, SOC_PHY_CLAUSE45_REGAD(phy_reg_addr), SOC_PHY_CLAUSE45_DEVAD(phy_reg_addr),
