...
soc_pcie_fw_status_get(int unit, uint32 *valid)
{
    ...
    if(soc_feature(unit, soc_feature_cmicx) && 
       (soc_cm_get_bus_type(unit) & SOC_PCI_DEV_TYPE)) {
        ...
        rv = pcie_phy_diag_reg_read(&sa, 0xd230, &loaded);
        if(rv != 0) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit, "pcie_phy_diag_reg_read failed, %d\n"), rv));
        }
        ...
        else if (!SOC_IS_HELIX5(unit) && (mhost_en == 1) && 
                 (bootdev == 0 || bootdev == 6) &&
                 (loaded == 0)) {
           *valid = 0;
        }
