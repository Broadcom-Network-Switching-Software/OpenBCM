...

{
    ...
    for(j = 0 ; j < nof_ports ; j++) {
        ...
        if ((soc_feature(unit, soc_feature_portmod)) && !is_legacy) {
#ifdef PORTMOD_SUPPORT
...
#if defined(BCM_TOMAHAWK3_SUPPORT) || defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)
    /* For TH3, only front panel ports support this eyescan mode.
     * For front panel ports (PM8x50), we do not need line_rates here.
     * So pass NULL as input for lint_rates.
     */
    if (SOC_IS_TOMAHAWK3(unit) || SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) {
    ...
    soc_port_phy_eyescan_results_t* results)    /*array of result per port*/
{
    ...
    if ((IS_CPRI_PORT(unit, ports[0])) && SOC_IS_MONTEREY(unit)) {
        rv = _soc_port_generate_phy_array(unit, 
                                     inst, 
                                     nof_ports, 
                                     ports, 
