...
soc_uc_mhost_reset(int unit, int uC)
{
    ...
    defined(BCM_JERICHO_SUPPORT) || defined(BCM_JERICHO_2_SUPPORT)
    /* Take the processor in and out of reset but halted */
...
soc_uc_mhost_prestart_config(int unit, int uC)
{
    ...
    if (config_ver >= 1) {
        size = (2*1024*1024); /* HostRAM Fixed 2MB for now */
        soc->uc_hostram_buf[uC] = _soc_uc_mhost_hostram_alloc(unit, uC, size);
