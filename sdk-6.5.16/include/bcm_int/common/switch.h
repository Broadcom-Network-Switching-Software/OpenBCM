...
#define DNXC_WARM_BOOT_API_TEST_FUNCTION_NAME_UPDATE(unit) \
    sal_strncpy(dnxc_warmboot_api_function_name[unit], __FUNCTION__, DNXC_WARMBOOT_API_FUNCTION_NAME_MAX_LEN - 1)
...
#define BCM_PRE_API_LOGIC_HOOK(unit)                 \
    ...
    DNXC_MTA(dnxc_multithread_analyzer_declare_api_in_play(unit, __FUNCTION__, MTA_FLAG_DISPATCHER, TRUE));\

#define BCM_POST_API_LOGIC_HOOK(unit)        \
    DNXC_MTA(dnxc_multithread_analyzer_declare_api_in_play(unit, __FUNCTION__, MTA_FLAG_DISPATCHER, FALSE));\
