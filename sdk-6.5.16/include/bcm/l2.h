...
#if defined(INCLUDE_L3)
/* Change L2 Fields entry specification. */
typedef struct bcm_l2_change_fields_s {
    uint32 flags;                       /* See BCM_L2_CHANGE_FIELDS_XXX flag
                                           definitions. */
    bcm_mac_t macda;                    /* New payload MACDA. */
    bcm_mac_t macsa;                    /* New payload MACSA. */
    uint8 pri;                          /* payload tag priority. */
    uint8 cfi;                          /* payload tag CFI. */
    uint16 tpid;                        /* payload tag TIP value. */
    uint16 vlan;                        /* payload vlan tag. */
    uint32 change_l2_fields_class_id;   /* Change L2 Fields class ID assigned by
                                           ingress FP. */
    bcm_gport_t dvp;                    /* Optional DVP key. */
} bcm_l2_change_fields_t;
#endif

/* bcm_l2_change_fields_traverse_cb */
#if defined(INCLUDE_L3)
...
#if defined(INCLUDE_L3)
/* 
 * Add a Change L2 Fields entry in the Egress Adaptation Table. If
 * REPLACE flag is specified then update the entry identified by the
 * l2_fields_info.
 */
...
#if defined(INCLUDE_L3)
/* Delete a Change L2 Fields entry from the Egress Adaptation table. */
...
#if defined(INCLUDE_L3)
/* 
 * Retrieve the Change L2 Fields entry specified in l2_fields_info
 * structure
 */
...
#if defined(INCLUDE_L3)
/* 
 * Traverse through the Change L2 Fields entries in the Egress Adaptation
 * table and invoke a user provided callback for each valid entry.
 */
