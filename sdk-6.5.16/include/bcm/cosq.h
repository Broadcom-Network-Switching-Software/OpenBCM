...
typedef enum bcm_cosq_control_e {
    ...
    bcmCosqControlMirrorOnDropAgingVsq = 202, /* Mirror on Drop aging time for vsq
                                           type */
    bcmCosqControlShaperPacketMode = 203 /* Control shaper resolution mode:
...
/* Create rate class with the specified attributes. */
...
/* 
 * Getting set of recommended values for queueus with the specified
 * attributes
 */
...
typedef enum bcm_cosq_stat_e {
    ...
    bcmCosqStatDiscardUCDroppedPackets = 89, /* Per port per queue WRED dropped
                                           packets. */
    bcmCosqStatMaxCount = 90            /* this should be the last one. */
} bcm_cosq_stat_t;
...
/* 
 * set a PFC-tx priority to PG mapping profile, pg_arary is
 * indexed by PFC priority
 */
...
/* 
 * get a PFC-tx priority to PG mapping profile, pg_arary is
 * indexed by PFC priority
 */
...
/* 
 * set a PFC class config profile, config_arary is
 * indexed by PFC priority
 */
...
/* 
 * get a PFC class config profile, config_array is
 * indexed by PFC priority
 */
...
/* Set port to profile id for given profile type */
...
/* Get port to profile id for given profile type */
...
/* Get profile dynamic changeable property */
extern int bcm_cosq_profile_property_dynamic_get(
    int unit, 
    bcm_cosq_profile_type_t profile_type, 
    bcm_cosq_dynamic_setting_type_t *dynamic);

/* Check if a particular profile ID of a profile Type is in use */
...
/* set per port per priority group properties */
...
/* get per port per priority group properties */
...
/* get if cosq_control_type setting can be changed dynamically */
...
/* Set the cosq to queue mapping from a profile */
...
/* Get the cosq to queue mapping from a profile */
...
/* Set OBM Traffic class based on lookup priority */
...
/* Get OBM Traffic class mapped to lookup priority */
...
/* Get OBM max watermark mode for a port */
extern int bcm_obm_port_max_usage_mode_get(
    int unit, 
    bcm_port_t port, 
    bcm_obm_max_watermark_mode_t *obm_wm_mode);

/* Set OBM max watermark mode for a port */
extern int bcm_obm_port_max_usage_mode_set(
    int unit, 
    bcm_port_t port, 
    bcm_obm_max_watermark_mode_t obm_wm_mode);

/* Setup OBM traffic class to priority mapping for different classes */
...
/* Get OBM traffic class to priority mapping for different classes */
...
/* Unset obm callback funtion to be called when obm interrupt occur. */
extern int bcm_obm_cb_unregister(
    int unit, 
    bcm_obm_callback_fn fn, 
    void *user_data);

/* Get the maximum number of  PFC optimized groups supported for a port. */
extern int bcm_cosq_port_optimized_pfc_group_num_get(
    int unit, 
    bcm_port_t port, 
    int *arg);

/* Set obm callback funtion to be called when obm interrupt occur. */
...
/* Get scheduler element parameters */
...
/* 
 * Program the PG headroom for port's priorities based on cable
 *         length and traffic distribution parameters
 */
...
/* 
 * Program the Headroom Pool limit based on traffic distribution
 *         parameters and number of lossless classes
 */
...
#define BCM_COSQ_GENERIC_KEY_COLOR_SET(key, color)  (1 | (color << 8)) 
#define BCM_COSQ_GENERIC_KEY_COLOR_GET(key)  (key >> 8) 
#define BCM_COSQ_GENERIC_KEY_LAT_BIN_COLOR_SET(key, bin, color)  (2 | (color << 8) | (bin << 10)) 
#define BCM_COSQ_GENERIC_KEY_LAT_BIN_COLOR_BIN_GET(key)  (key >> 10) 
#define BCM_COSQ_GENERIC_KEY_LAT_BIN_COLOR_COLOR_GET(key)  ((key >> 8) & 3) 
...
/* API for setting the generic control */
...
/* API for getting the generic control */
