...
err_code_t blackhawk_tsc_ucode_init(  phymod_access_t *sa__ )
{
    ...
    __err = blackhawk_tsc_INTERNAL_poll_micro_ra_initdone(sa__, 250); /* Poll status of data RAM initialization */
    if(__err) return(__err);
...
err_code_t blackhawk_tsc_config_shared_tx_pattern_idx_get(  phymod_access_t *sa__, uint32_t *pattern_len, uint32_t *pattern) {
  err_code_t __err;
  uint8_t mode_sel;
