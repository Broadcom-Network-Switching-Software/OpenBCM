...
int blackhawk_diagnostics_eye_margin_proj( const phymod_phy_access_t* phy, uint32_t flags,
  ...
  if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
     ...
     for (i = 0; i < num_lane; i++) {
         ...
         if (found == 0) {
             ...
             PHYMOD_IF_ERR_RETURN(phymod_phy_speed_config_t_init(&speed_config));
             PHYMOD_IF_ERR_RETURN(blackhawk_phy_speed_config_get(phy, &speed_config));
             data_rate = speed_config.data_rate * 1000.0 * 1000.0;
             data_rate_in_Mhz = speed_config.data_rate;
             found = 1;
         }
         if(num_lane > 1) {
