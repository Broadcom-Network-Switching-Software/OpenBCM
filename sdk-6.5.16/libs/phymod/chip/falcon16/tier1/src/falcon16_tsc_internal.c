...
static falcon16_tsc_info_t falcon16_tsc_info[NUM_SERDES_INFO_TABLES];


falcon16_tsc_info_t *falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr(srds_access_t *sa__) {
    uint8_t index;
    index = 0;
    return &falcon16_tsc_info[index];
}

falcon16_tsc_info_t *falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr_with_check(srds_access_t *sa__) {
    err_code_t err_code = ERR_CODE_NONE;
    falcon16_tsc_info_t * falcon16_tsc_info_ptr = falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr(sa__);
...
err_code_t falcon16_tsc_INTERNAL_verify_falcon16_tsc_info(falcon16_tsc_info_t *falcon16_tsc_info_ptr, srds_access_t *sa__)
{
    err_code_t err_code = ERR_CODE_NONE;
...
err_code_t falcon16_tsc_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
    ...
    falcon16_tsc_info_t * falcon16_tsc_info_ptr = falcon16_tsc_INTERNAL_get_falcon16_tsc_info_ptr_with_check(sa__);
    CHECK_AND_RETURN_IF_NULL(falcon16_tsc_info_ptr, ERR_CODE_INVALID_INFO_TABLE_ADDR);
...
err_code_t falcon16_tsc_INTERNAL_update_uc_core_config_st(struct falcon16_tsc_uc_core_config_st *st) {
  ...
  st->field.osr_2p5_available = in & BFMASK(1); in >>= 1;
...
err_code_t falcon16_tsc_INTERNAL_update_uc_core_config_word(struct falcon16_tsc_uc_core_config_st *st) {
  ...
  in <<= 1; in |= st->field.osr_2p5_available & BFMASK(1);
 ...
 */
static err_code_t _falcon16_tsc_get_divisor_ratio(srds_access_t *sa__, enum falcon16_tsc_pll_div_enum div, uint16_t *numerator, uint16_t *denominator)
{
    switch (div) {
...
static err_code_t _falcon16_tsc_get_refclk_from_div_vco(srds_access_t *sa__, uint32_t *refclk_freq_hz, enum falcon16_tsc_pll_div_enum div, uint32_t vco_freq_khz, enum falcon16_tsc_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_falcon16_tsc_get_divisor_ratio(sa__, div, &numerator, &denominator));
...
err_code_t falcon16_tsc_INTERNAL_get_vco_from_refclk_div(srds_access_t *sa__, uint32_t refclk_freq_hz, enum falcon16_tsc_pll_div_enum div, uint32_t *vco_freq_khz, enum falcon16_tsc_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_falcon16_tsc_get_divisor_ratio(sa__, div, &numerator, &denominator));
    if (pll_option == FALCON16_TSC_PLL_OPTION_REFCLK_DOUBLER_EN) refclk_freq_hz *= 2;
    if (pll_option == FALCON16_TSC_PLL_OPTION_REFCLK_DIV2_EN)    refclk_freq_hz /= 2;
...
static err_code_t _falcon16_tsc_check_div(srds_access_t *sa__, enum falcon16_tsc_pll_div_enum div) {
    uint8_t i, found = 0;
...
static err_code_t _falcon16_tsc_get_div(srds_access_t *sa__, uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum falcon16_tsc_pll_div_enum *div, enum falcon16_tsc_pll_option_enum pll_option) {
...
err_code_t falcon16_tsc_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    if (pll_option == FALCON16_TSC_PLL_OPTION_DISABLE_VERIFY) {
    ...
    if (*div != FALCON16_TSC_PLL_DIV_UNKNOWN) {
        EFUN(_falcon16_tsc_check_div(sa__, *div));
    }
    ...
    if (refclk == FALCON16_TSC_PLL_REFCLK_UNKNOWN) {
        ...
        EFUN(_falcon16_tsc_get_refclk_from_div_vco(sa__, refclk_freq_hz, *div, *vco_freq_khz, pll_option));
    } else {
        EFUN(_falcon16_tsc_get_refclk_in_hz(sa__, refclk, refclk_freq_hz));
    }
    ...
    if (*vco_freq_khz == 0) {
        ...
        EFUN(falcon16_tsc_INTERNAL_get_vco_from_refclk_div(sa__, *refclk_freq_hz, *div, vco_freq_khz, pll_option));
    }
