...
err_code_t falcon16_tsc_stop_rx_adaptation(srds_access_t *sa__, uint8_t enable) {
  ...
  if (enable) {
      err_code_t err_code;
      err_code = falcon16_tsc_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_GRACEFULLY,GRACEFUL_STOP_TIME);
      if(err_code) {
          uint8_t temp=0;
          USR_PRINTF(("Warning Graceful stop request returned error %d; Requesting a forceful stop\n",err_code));
          temp = falcon16_tsc_INTERNAL_stop_micro(sa__,0,&err_code);
          /* return value from falcon16_tsc_INTERNAL_stop_micro - immediate stop is not required */
...
err_code_t falcon16_tsc_ucode_crc_verify(srds_access_t *sa__, uint16_t ucode_len,uint16_t expected_crc_value) {
    ...
    if(calc_crc != expected_crc_value) {
        ...
        return(falcon16_tsc_error(sa__, ERR_CODE_UC_CRC_NOT_MATCH));
    }
...
err_code_t falcon16_tsc_set_uc_lane_cfg(srds_access_t *sa__, struct falcon16_tsc_uc_lane_config_st struct_val) {
  ...
  if(reset_state < 7) {
      ...
      return (falcon16_tsc_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
  }
...
err_code_t falcon16_tsc_ucode_pram_load(srds_access_t *sa__, char const * ucode_image, uint32_t ucode_len) {
    ...
    if (ucode_len > UCODE_MAX_SIZE) {                       /* uCode size should be less than UCODE_MAX_SIZE  */
        return (falcon16_tsc_error(sa__, ERR_CODE_INVALID_UCODE_LEN));
    }
...
err_code_t falcon16_tsc_set_osr_2p5_available(srds_access_t *sa__, uint8_t osr_2p5_available) {
    struct falcon16_tsc_uc_core_config_st core_config = {{0}};
    EFUN(falcon16_tsc_get_uc_core_config(sa__, &core_config));
    core_config.field.osr_2p5_available = osr_2p5_available;
    EFUN(falcon16_tsc_INTERNAL_set_uc_core_config(sa__, core_config));
    return (ERR_CODE_NONE);
}
...
err_code_t falcon16_tsc_read_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t *val) {
    ...
    switch(param) {
    case RX_AFE_PF:
        EFUN(falcon16_tsc_INTERNAL_get_rx_pf_main(sa__, &rd_val));
        *val = (int8_t)rd_val;
        break;
    case RX_AFE_PF2:
        EFUN(falcon16_tsc_INTERNAL_get_rx_pf2(sa__, &rd_val));
        *val = (int8_t)rd_val;
        break;
    case RX_AFE_VGA:
        EFUN(falcon16_tsc_INTERNAL_get_rx_vga(sa__, &rd_val));
        *val = (int8_t)rd_val;
      break;
...
err_code_t falcon16_tsc_validate_txfir_cfg(srds_access_t *sa__, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3) {
  ...
  return (falcon16_tsc_error(sa__, err_code));
}
...
err_code_t falcon16_tsc_apply_txfir_cfg(srds_access_t *sa__, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3) {
  ...
  err_code_t failcode = falcon16_tsc_validate_txfir_cfg(sa__, pre, main, post1, post2, post3);
  if (!failcode) {
  ...
  return (falcon16_tsc_error(sa__, failcode));
}
...
err_code_t falcon16_tsc_uc_reset(srds_access_t *sa__, uint8_t enable) {
  ...
  return falcon16_tsc_uc_reset_with_info(sa__, enable, ucode_info);
}
...
err_code_t falcon16_tsc_uc_reset_with_info(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
  if (enable) {
    ...
    EFUN(falcon16_tsc_pmd_wr_reg(sa__, 0xD225, 0x8701));
...
err_code_t falcon16_tsc_wait_uc_active(srds_access_t *sa__) {
    ...
    return (falcon16_tsc_error(sa__, ERR_CODE_MICRO_INIT_NOT_DONE));
}
err_code_t falcon16_tsc_init_falcon16_tsc_info(srds_access_t *sa__) {
    ...
    USR_ACQUIRE_LOCK;
    /* Handle error cases when falcon16_tsc_info_ptr returned as zero */
    if (falcon16_tsc_info_ptr == NULL) {
        EFUN_PRINTF(("ERROR:  Info table pointer is null.\n"));
        RELEASE_LOCK_AND_RETURN (falcon16_tsc_error(sa__, ERR_CODE_INFO_TABLE_ERROR));
    }
    if (falcon16_tsc_info_ptr->signature == SRDS_INFO_SIGNATURE) {
        ...
        if (err_code == ERR_CODE_NONE) {
            ...
            RELEASE_LOCK_AND_RETURN (falcon16_tsc_error(sa__, ERR_CODE_MICRO_INIT_NOT_DONE));
        }
    }
    /*Never initialized so far, continue with initialization */

    err_code = falcon16_tsc_rdblk_uc_prog_ram(sa__, (uint8_t *)info_table, INFO_TABLE_RAM_BASE, INFO_TABLE_END);
    /* Release lock before returning from the call to avoid deadlock */
    if (ERR_CODE_NONE != err_code) {
       RELEASE_LOCK_AND_RETURN(falcon16_tsc_error(sa__, err_code));
    }
    ...
    if (info_table_version < 0x36) {
        falcon16_tsc_info_ptr->grp_ram_size = 0;
    } else {
        falcon16_tsc_info_ptr->grp_ram_size = info_table[INFO_TABLE_OFFS_GROUP_RAM_SIZE / sizeof(uint32_t)] & 0xFFFF;
    }
    falcon16_tsc_info_ptr->ucode_version = info_table[INFO_TABLE_OFFS_UC_VERSION /sizeof(uint32_t)];
...
err_code_t falcon16_tsc_configure_pll_refclk_div(srds_access_t *sa__,
    ...
    return falcon16_tsc_INTERNAL_configure_pll(sa__, refclk, div, 0, FALCON16_TSC_PLL_OPTION_NONE);
}

err_code_t falcon16_tsc_configure_pll_refclk_vco(srds_access_t *sa__,
                                           enum falcon16_tsc_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, refclk, FALCON16_TSC_PLL_DIV_UNKNOWN, vco_freq_khz, FALCON16_TSC_PLL_OPTION_NONE);
}

err_code_t falcon16_tsc_configure_pll_div_vco(srds_access_t *sa__,
                                        enum falcon16_tsc_pll_div_enum div,
                                        uint32_t vco_freq_khz) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, FALCON16_TSC_PLL_REFCLK_UNKNOWN, div, vco_freq_khz, FALCON16_TSC_PLL_OPTION_NONE);
}
...
err_code_t falcon16_tsc_configure_pll_refclk_div_div2refclk(srds_access_t *sa__,
                                           enum falcon16_tsc_pll_refclk_enum refclk,
                                           enum falcon16_tsc_pll_div_enum div) {
   return falcon16_tsc_INTERNAL_configure_pll(sa__, refclk, div, 0, FALCON16_TSC_PLL_OPTION_REFCLK_DIV2_EN);
}

err_code_t falcon16_tsc_configure_pll_refclk_vco_div2refclk(srds_access_t *sa__,
                                           enum falcon16_tsc_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, refclk, FALCON16_TSC_PLL_DIV_UNKNOWN, vco_freq_khz, FALCON16_TSC_PLL_OPTION_REFCLK_DIV2_EN);
}

err_code_t falcon16_tsc_configure_pll_div_vco_div2refclk(srds_access_t *sa__,
                                        enum falcon16_tsc_pll_div_enum div,
                                        uint32_t vco_freq_khz) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, FALCON16_TSC_PLL_REFCLK_UNKNOWN, div, vco_freq_khz, FALCON16_TSC_PLL_OPTION_REFCLK_DIV2_EN);
}
/* following routines divide refclk input by 2 to prevent fracn mode */
err_code_t falcon16_tsc_configure_pll_refclk_div_div4refclk(srds_access_t *sa__,
                                           enum falcon16_tsc_pll_refclk_enum refclk,
                                           enum falcon16_tsc_pll_div_enum div) {
   return falcon16_tsc_INTERNAL_configure_pll(sa__, refclk, div, 0, FALCON16_TSC_PLL_OPTION_REFCLK_DIV4_EN);
}

err_code_t falcon16_tsc_configure_pll_refclk_vco_div4refclk(srds_access_t *sa__,
                                           enum falcon16_tsc_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, refclk, FALCON16_TSC_PLL_DIV_UNKNOWN, vco_freq_khz, FALCON16_TSC_PLL_OPTION_REFCLK_DIV4_EN);
}

err_code_t falcon16_tsc_configure_pll_div_vco_div4refclk(srds_access_t *sa__,
                                        enum falcon16_tsc_pll_div_enum div,
                                        uint32_t vco_freq_khz) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, FALCON16_TSC_PLL_REFCLK_UNKNOWN, div, vco_freq_khz, FALCON16_TSC_PLL_OPTION_REFCLK_DIV4_EN);
}

err_code_t falcon16_tsc_configure_pll_refclk_div_refclk_2x(srds_access_t *sa__,
                                           enum falcon16_tsc_pll_refclk_enum refclk,
                                           enum falcon16_tsc_pll_div_enum div) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, refclk, div, 0, FALCON16_TSC_PLL_OPTION_REFCLK_DOUBLER_EN);
}

err_code_t falcon16_tsc_configure_pll_refclk_vco_refclk_2x(srds_access_t *sa__,
                                           enum falcon16_tsc_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, refclk, FALCON16_TSC_PLL_DIV_UNKNOWN, vco_freq_khz, FALCON16_TSC_PLL_OPTION_REFCLK_DOUBLER_EN);
}

err_code_t falcon16_tsc_configure_pll_div_vco_refclk_2x(srds_access_t *sa__,
                                        enum falcon16_tsc_pll_div_enum div,
                                        uint32_t vco_freq_khz) {
    return falcon16_tsc_INTERNAL_configure_pll(sa__, FALCON16_TSC_PLL_REFCLK_UNKNOWN, div, vco_freq_khz, FALCON16_TSC_PLL_OPTION_REFCLK_DOUBLER_EN);
}
...
err_code_t falcon16_tsc_ucode_load_verify(srds_access_t *sa__, uint8_t *ucode_image, uint32_t ucode_len) {
    ...
    do {                                                  /* ucode_image read 16bits at a time */
        ...
        if (data_lsw != rddata_lsw) {
            ...
            return (falcon16_tsc_error(sa__, ERR_CODE_UCODE_VERIFY_FAIL));             /* Verify uCode FAIL */
        }
