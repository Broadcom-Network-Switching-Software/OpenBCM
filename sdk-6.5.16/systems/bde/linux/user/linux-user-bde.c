...
{  
    ...
    if (p & ~page_mask) {
        ...
        */
        ...
        if ((map == MAP_FAILED) && (dev_fd == _kdevfd)) {
...
/*  knet is only supported on jericho and qmx of DUNE */
    ...
    assert(_ioctl_LUBDE_GET_DEVICE == 0);
        ...
        if (SOC_IS_JERICHO_2_TYPE(_devices[i]->bde_dev.device)) {
            _devices[i]->i2c_addr = 0x44;
            _devices[i]->i2c_access_type = CPU_I2C_ALEN_LONG_DLEN_LONG;
        }

        /*All Ramon devices from 0x8790 to 0x879F*/
        if ((_devices[i]->bde_dev.device & BCM_DNXF_DEVID_MASK) == BCM88790_DEVICE_ID) {
            _devices[i]->i2c_addr = 0x44;
            _devices[i]->i2c_access_type = CPU_I2C_ALEN_LONG_DLEN_LONG;
        }
#endif
        ...
        switch (_devices[i]->bde_dev.device & BCM_DNXF_DEVID_MASK) {
