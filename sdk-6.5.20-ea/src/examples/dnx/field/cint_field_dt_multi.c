    ...
    cint_field_dt_multi_fg_info_t * dt_fg_info)
{
    ...
    {
        /* Q2A */
        if (is_device_or_above(unit, QUMRAN2A) && !is_device_or_above(unit, JERICHO2P))
        {
    ...
    /**
     * None zero constant qualifiers are not allowed on EPMF stage. Use header qualifier
     * to get value from the ETH-Outer-PCP field.
     */
    if (field_stage == bcmFieldStageEgress)
    {
