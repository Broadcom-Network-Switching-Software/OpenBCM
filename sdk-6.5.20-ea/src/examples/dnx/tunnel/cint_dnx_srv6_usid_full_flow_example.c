 ...
 * cint;
 * int rv;
 * rv = dnx_basic_usid_demo(0,200,202,202,202,201,40,203);
 * print rv;
 * exit;
 *
 *
                    ...
                    int out_port_egress)     /** Outgoing port of SRV6 Egress */
{
     ...
     */
    {
        ...
        tunnel_init_set.type = bcmTunnelTypeIpAnyIn6; /** since no SRH is present, creating a regular IPv6 tunnel type */
     ...
     */
    {
        ...
        BCM_GPORT_LOCAL_SET(extension_terminator_info.port, cint_srv6_tunnel_info.recycle_port);


        int max_sid_idx;
        max_sid_idx = *dnxc_data_get(unit, "srv6", "termination", "max_nof_terminated_sids_usp", NULL);
        int min_sid_idx;
        min_sid_idx = *dnxc_data_get(unit, "srv6", "termination", "min_nof_terminated_sids_usp", NULL);

        /** create LIF entries for all possible nof_sids in an SRv6 packet that would lead to RCY port + RCH Encapsulation*/
        for (sid_idx = min_sid_idx; sid_idx <= max_sid_idx; sid_idx++)
        {
            /*
     ...
     */
    {
        ...
        srv6_tunnel_termination(unit, END_POINT_1_DIP, cint_srv6_tunnel_info.vrf_in[END_POINT_1], bcmTunnelTypeIpAnyIn6);

        /** End-Point-2: Add MyDIP of current End-Point, getting same VRF for all End-Points */
        srv6_tunnel_termination(unit, END_POINT_2_DIP, cint_srv6_tunnel_info.vrf_in[END_POINT_1], bcmTunnelTypeIpAnyIn6);

        /** End-Point-3: Add MyDIP of current End-Point, getting same VRF for all End-Points */
        srv6_tunnel_termination(unit, END_POINT_3_DIP, cint_srv6_tunnel_info.vrf_in[END_POINT_1], bcmTunnelTypeIpAnyIn6);

        /** End-Point-4: Add MyDIP of current End-Point, getting same VRF for all End-Points */
        srv6_tunnel_termination(unit, END_POINT_4_DIP, cint_srv6_tunnel_info.vrf_in[END_POINT_1], bcmTunnelTypeIpAnyIn6);

        /** End-Point-5: Add MyDIP of current End-Point, getting same VRF for all End-Points */
        srv6_tunnel_termination(unit, END_POINT_5_DIP, cint_srv6_tunnel_info.vrf_in[END_POINT_1], bcmTunnelTypeIpAnyIn6);

        /** Egress: Add MyDIP according to the SIDs, because we need to terminate the IPV6 header with the current SID */
        srv6_tunnel_termination(unit, EGRESS_DIP, cint_srv6_tunnel_info.vrf_in[EGRESS], bcmTunnelTypeIpAnyIn6);
