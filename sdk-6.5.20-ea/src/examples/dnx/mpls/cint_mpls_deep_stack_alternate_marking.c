 ...
 *
 *  Should recieve 2 packets: forward copy + snoop copy.
 ...
 * cint
 * mpls_alternate_marking_termination(0,200,201,0xabc);
 * exit;
 * 
 * 
 ...
 * And snooped copy, including system headers
...
int mpls_alternate_marking_intermediate_field(int unit) {
    bcm_field_entry_t entry_handle;
    int rv = BCM_E_NONE;
 ...
 * @return int 
 */
int mpls_alternate_marking_termination(int unit, int in_port, int out_port, int flow_id) {
    int cpu_port;
    bcm_port_config_t port_config;
    ...
    BCM_IF_ERROR_RETURN(mpls_alternate_marking_intermediate_field(unit));
