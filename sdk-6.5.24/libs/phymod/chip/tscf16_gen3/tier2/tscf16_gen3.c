...
int tscf16_gen3_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_lane_config)
{
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        PHYMOD_IF_ERR_RETURN
            (tefmod16_gen3_port_start_lane_get(&phy->access,
                                               &port_starting_lane,
                                               &port_num_lane));
        phy_copy.access.lane_mask = 1 << port_starting_lane;
        PHYMOD_IF_ERR_RETURN
            (tefmod16_gen3_trigger_speed_change(&phy_copy.access));
    }
...
{
    /* Use reset->tx to reset Phy, ignore reset->rx */
...
int tscf16_gen3_phy_power_set(const phymod_phy_access_t* phy, const phymod_phy_power_t* power)
{
    ...
    PHYMOD_IF_ERR_RETURN
       (tefmod16_gen3_autoneg_control_get(&phy->access, &an_control, &an_complete));
    ...
    for (i = 0; i < num_lane; i++) {
         ...
         if ((power->tx == phymodPowerOff) && (power->rx == phymodPowerNoChange)) {
             mode = PWRDN_TX;
             PHYMOD_IF_ERR_RETURN(falcon16_v1l4p1_lane_pwrdn(&pm_phy_copy.access, mode));
         }

         /*enable tx on the PMD side */
         if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerNoChange)) {
             PHYMOD_IF_ERR_RETURN(falcon16_v1l4p1_pwrdn_set(&pm_phy_copy.access, 1, 0));
         }
         ...
         if ((power->tx == phymodPowerNoChange) && (power->rx == phymodPowerOn)) {
             PHYMOD_IF_ERR_RETURN(falcon16_v1l4p1_pwrdn_set(&pm_phy_copy.access, 0, 0));
         }

         /* Both Tx and Rx power on */
         if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerOn)) {
             if (an_control.enable) {
                 ...
                 if (0 == i) {
                     an_control.enable = 0;
                     PHYMOD_IF_ERR_RETURN
                         (tefmod16_gen3_autoneg_control(&pm_phy_copy.access, &an_control));
                     an_control.enable = 1;
                 }
             } else {
                 /* pcs disable */
             ...
             /* Check if the last lane, then enable PCS/AN */
             if ((num_lane - 1) == i) {
                 ...
                 if (an_control.enable) {
                     ...
                     /* pcs enable */
                     PHYMOD_IF_ERR_RETURN
                         (tefmod16_gen3_enable_set(&pm_phy_copy.access));
                 }
                 pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
             }
         }
...
int tscf16_gen3_phy_power_get(const phymod_phy_access_t* phy, phymod_phy_power_t* power)
{
    ...
    PHYMOD_IF_ERR_RETURN(falcon16_v1l4p1_pwrdn_get(&phy->access, &pwrdn));
...
int tscf16_gen3_phy_fec_enable_set(const phymod_phy_access_t* phy, uint32_t enable)
{
    ...
    /*  There are different regsiters for rsfec and base-r.
     *  Need to check and disable the fec_types in the check list,
     *  to make sure the target fec_type works without conflicts.
     */
    if (fec_en) {
        if (fec_type == TEFMOD_CL74) {
            PHYMOD_IF_ERR_RETURN(tefmod16_gen3_FEC_get(&phy->access, TEFMOD_CL91, &fec_en_cur));
            if (fec_en_cur) {
                PHYMOD_DEBUG_WARN(("FEC enable previous state :: %x :: fec_type :: %x\n", fec_en_cur, TEFMOD_CL91));
                PHYMOD_IF_ERR_RETURN(tefmod16_gen3_FEC_control(&phy->access, TEFMOD_CL91, 0));
            }

            PHYMOD_IF_ERR_RETURN(tefmod16_gen3_FEC_get(&phy->access, TEFMOD_CL108, &fec_en_cur));
            if (fec_en_cur) {
                PHYMOD_DEBUG_WARN(("FEC enable previous state :: %x :: fec_type :: %x\n", fec_en_cur, TEFMOD_CL108));
                PHYMOD_IF_ERR_RETURN(tefmod16_gen3_FEC_control(&phy->access, TEFMOD_CL108, 0));
            }
        } else {
            PHYMOD_IF_ERR_RETURN(tefmod16_gen3_FEC_get(&phy->access, TEFMOD_CL74, &fec_en_cur));
            if (fec_en_cur) {
                PHYMOD_DEBUG_WARN(("FEC enable previous state :: %x :: fec_type :: %x\n", fec_en_cur, TEFMOD_CL74));
                PHYMOD_IF_ERR_RETURN(tefmod16_gen3_FEC_control(&phy->access, TEFMOD_CL74, 0));
            }
        }
    }
...
int _tscf16_gen3_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    PHYMOD_IF_ERR_RETURN(falcon16_v1l4p1_uc_reset(&core_copy.access, 1));
...
/* Get timesync offset */
int tscf16_gen3_timesync_offset_get(const phymod_core_access_t* core, uint32_t *ts_offset)
{
    phymod_core_access_t core_copy;
    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));

    PHYMOD_IF_ERR_RETURN
        (tefmod16_gen3_1588_ts_offset_get(&core_copy.access, ts_offset));
...
{
    int i, deskew_calc_count = 3; /* calc average deskew value with 3 times */
    phymod_phy_access_t phy_copy;
    int speed_id;
    uint16_t no_of_vl = 1;  /* virtual lanes per port */
    ...
    PHYMOD_IF_ERR_RETURN
        (tefmod16_gen3_speed_id_get(&phy_copy.access, &speed_id));

    tefmod16_gen3_vl_bml_get(speed_id, &no_of_vl, &no_of_bml);

    if ((no_of_vl > 1) && (ts_am_norm_mode == 0)) {
        PHYMOD_DEBUG_WARN(("Warning: phy_addr 0x%x, lane mask 0x%x, virtual lane number %d. The ts adjust mode is None (0), it should be EarliestLane (1) or Latestlane (2).\n",
            phy_copy.access.addr, phy_copy.access.lane_mask, no_of_vl));
    }

    if ((no_of_vl == 1) && (ts_am_norm_mode != 0)) {
        PHYMOD_DEBUG_WARN(("Warning: phy_addr 0x%x, lane mask 0x%x, virtual lane number %d. The ts adjust mode is %s (%d), it should be None (0).\n",
            phy_copy.access.addr, phy_copy.access.lane_mask, no_of_vl, ts_am_norm_mode == 1 ? "EarliestLane" : "Latestlane", ts_am_norm_mode));
    }

    /* Program deskew adjustment for multi-lane ports */
    if (ts_am_norm_mode != phymodTimesyncCompensationModeNone) {
        /* To set rx deskew adjust, there are three steps: TEFMOD_RX_DESKEW_INIT, TEFMOD_RX_DESKEW_CALC, and  TEFMOD_RX_DESKEW_ADJUST.
         * The global variable my_am_ts_calc_value_avg is used to calculate average adjust value.
         * The my_am_ts_calc_value_avg is cleared every time in TEFMOD_RX_DESKEW_INIT of this API, and used for calculate average value in TEFMOD_RX_DESKEW_CALC,
         * and then it is set to registers in TEFMOD_RX_DESKEW_ADJUST.
         * Therefore, the global variable is not shared with other thread (protect by port/portmod lock) and
         * do not need Warmboot protection (always clear at TEFMOD_RX_DESKEW_INIT). It is an internal global variable in this API.
         */
