...
uint32_t blackhawk7_l8p2_INTERNAL_mult_with_overflow_check(uint32_t a, uint32_t b, uint8_t *of) {
    uint16_t c,d;
    uint32_t r,s;
    if (a > b) return blackhawk7_l8p2_INTERNAL_mult_with_overflow_check(b, a, of);
    *of = 0;
    c = (uint16_t)(b >> 16);
    d = UINT16_MAX & b;
    r = a * c;
...
err_code_t blackhawk7_l8p2_INTERNAL_test_uc_timestamp_with_print_options(srds_access_t *sa__, uint8_t console_print_options) {
#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    uint16_t time = 0;
    uint16_t time1 = 0;
...
err_code_t blackhawk7_l8p2_INTERNAL_get_num_bits_per_ms(srds_access_t *sa__, uint8_t select_rx, uint32_t *num_bits_per_ms) {
    ...
    EFUN(blackhawk7_l8p2_acc_set_pll_idx(sa__,pll_select));
        ...
        /* *num_bits_per_ms = (uint32_t)((((uint64_t)core_config.vco_rate_in_Mhz * 100000UL) / blackhawk7_l8p2_osr_mode_enum_to_int_x1000(osr_mode))*10); */
    ...
    /* blackhawk7_l8p2_osr_mode_enum_to_int_x1000(osr_mode) */
    COMPILER_64_ZERO(temp_divisor);
    COMPILER_64_ADD_32(temp_divisor, ((uint32_t)blackhawk7_l8p2_osr_mode_enum_to_int_x1000(osr_mode)));
    /* (((uint64_t)core_config.vco_rate_in_Mhz * 100000UL) / blackhawk7_l8p2_osr_mode_enum_to_int_x1000(osr_mode)) */
    COMPILER_64_UDIV_64(temp_dividend, temp_divisor);
    /* ((((uint64_t)core_config.vco_rate_in_Mhz * 100000UL) / blackhawk7_l8p2_osr_mode_enum_to_int_x1000(osr_mode))*10) */
...
err_code_t blackhawk7_l8p2_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string, uint8_t string_size) {
    char string2[4];
    char string_extra[3] = "";
    struct blackhawk7_l8p2_ber_data_st ber_data_local;
    err_code_t err = ERR_CODE_NONE;
    enum blackhawk7_l8p2_prbs_chk_timer_selection_enum timer_sel = USE_HW_TIMERS;
...

    {
        ...
        err = blackhawk7_l8p2_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local, timer_sel);
    }
...
err_code_t blackhawk7_l8p2_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l8p2_ber_data_st *ber_data, enum blackhawk7_l8p2_prbs_chk_timer_selection_enum timer_sel) {
    INIT_SRDS_ERR_CODE
    uint8_t  lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    if (timer_sel == USE_HW_TIMERS) {
        ...
        struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl;

        /* Call functionality step by step */
        EFUN(blackhawk7_l8p2_BER_setup_measurement(sa__, time_ms, ber_data, &prbs_chk_hw_timer_ctrl));
        EFUN(blackhawk7_l8p2_BER_poll_measurement(sa__, ber_data, &prbs_chk_hw_timer_ctrl));
        EFUN(blackhawk7_l8p2_BER_get_measurement(sa__, ber_data, &prbs_chk_hw_timer_ctrl));
    }
...
err_code_t blackhawk7_l8p2_INTERNAL_get_prbs_timeout_count_from_time(uint16_t time_ms, uint16_t * time_ms_adjusted, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl) {
...
err_code_t blackhawk7_l8p2_INTERNAL_pam4_to_bin(srds_access_t *sa__, char var, char bin[], size_t bin_size) {
    if(!bin) {
...
err_code_t blackhawk7_l8p2_INTERNAL_compute_bin(srds_access_t *sa__, char var, char bin[], size_t bin_size) {
    if(!bin) {
        return(blackhawk7_l8p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t blackhawk7_l8p2_INTERNAL_set_rx_pf_main(srds_access_t *sa__, uint8_t val) {
    INIT_SRDS_ERR_CODE
    uint8_t pf_max;
    BLACKHAWK7_L8P2_UAPI_INIT_AMS_B0         /* if BLACKHAWK7_V2LXP2 || BLACKHAWK7_V3LXP2 */
    pf_max = PF_MAX_VALUE_B0;
    BLACKHAWK7_L8P2_UAPI_ELSE_AMS_A0   /* else if BLACKHAWK7_V1LXP2 */
    pf_max = PF_MAX_VALUE;
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_pf3(srds_access_t *sa__, uint8_t *val) {
    ...
    BLACKHAWK7_L8P2_UAPI_INIT_AMS_B0         /* if !BLACKHAWK7_V1LXP2 */
    {
        uint8_t pf3_msb;
    ...
    BLACKHAWK7_L8P2_UAPI_NOT_AMS_A0_ERR  /* if not BLACKHAWK7_V1LXP2, print error */
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_set_rx_dfe5(srds_access_t *sa__, int8_t val) {
    ...
    val = blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(val);
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe5(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap5())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap5())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe6(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap6())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap6())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe7(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap7())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap7())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe8(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap8())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap8())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe9(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap9())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap9())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe10(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap10())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap10())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe11(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap11())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap11())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe12(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap12())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap12())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe13(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap13())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap13())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_get_rx_dfe14(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    int8_t temp = 0;
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap14())));
    ESTM(temp = (int8_t)(temp + blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap14())));
    *val = (int8_t)(temp >> 1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_INTERNAL_eye_scan_setup(srds_access_t *sa__, uint8_t intr_pass_b, int16_t *y_max, int16_t *y_step, uint16_t *usr_diag_status) {
    INIT_SRDS_ERR_CODE
    enum blackhawk7_l8p2_rx_pam_mode_enum pam_mode = NRZ;
    uint8_t dig_lpbk_en;
    ...
    *usr_diag_status = 0;
    ...
    EFUN_PRINTF(("\n\n****  SERDES %sEYE SCAN CORE %d LANE %d   ****\n", intr_pass_b?"INTRUSIVE ":"",blackhawk7_l8p2_acc_get_core(sa__),blackhawk7_l8p2_acc_get_lane(sa__)));
...
/*----------------------------------------------------*/
err_code_t blackhawk7_l8p2_INTERNAL_get_pam_eye_margin_est(srds_access_t *sa__, blackhawk7_l8p2_eye_margin_t *eye_margin) {
    INIT_SRDS_ERR_CODE
  uint8_t ladder_range = 0;
...
err_code_t blackhawk7_l8p2_display_pam_eye_margin_est(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint8_t i;
    blackhawk7_l8p2_eye_margin_t eye_margin = {{0},{0},{0},{0}};
    enum blackhawk7_l8p2_rx_pam_mode_enum pam_mode = NRZ;
...
uint32_t blackhawk7_l8p2_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    int32_t c;
...
const char* blackhawk7_l8p2_INTERNAL_get_osr_str(uint8_t osr_val, uint8_t osr_str_length) {
...
err_code_t blackhawk7_l8p2_INTERNAL_get_tx_ppm(srds_access_t *sa__, int16_t *tx_ppm) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l8p2_INTERNAL_read_lane_state(srds_access_t *sa__, blackhawk7_l8p2_lane_state_st_t *istate) {
  INIT_SRDS_ERR_CODE
  const char *tx;
  const char *rx;
  ...
  istate->laneid = blackhawk7_l8p2_acc_get_lane(sa__);

  EFUN(blackhawk7_l8p2_INTERNAL_pmd_lock_status(sa__,&istate->pmd_lock, &istate->pmd_lock_chg));
  {
      err_code_t err_code = 0;
      istate->stop_state = blackhawk7_l8p2_INTERNAL_stop_micro(sa__,istate->pmd_lock,&err_code);

      if(err_code) USR_PRINTF(("WARNING: Unable to stop microcontroller,  following data is suspect\n"));
  }
  EFUN(blackhawk7_l8p2_get_rx_tuning_status(sa__, &istate->rx_tuning_done));
...

  {
      ...
      istate->osr_mode = osr_mode;
  }

  if(istate->osr_mode.tx_rx != 255) {
      tx= blackhawk7_l8p2_INTERNAL_get_osr_str(istate->osr_mode.tx_rx, GET_OSR_STR_LONG);
      USR_SNPRINTF(istate->tx_osr_mode_str, sizeof(istate->tx_osr_mode_str), "%s", tx);
  } else {
      tx = blackhawk7_l8p2_INTERNAL_get_osr_str(istate->osr_mode.tx, GET_OSR_STR_SHORT);
      rx = blackhawk7_l8p2_INTERNAL_get_osr_str(istate->osr_mode.rx, GET_OSR_STR_SHORT);
      USR_SNPRINTF(istate->tx_osr_mode_str, sizeof(istate->tx_osr_mode_str), "%s", tx);
      USR_SNPRINTF(istate->rx_osr_mode_str, sizeof(istate->rx_osr_mode_str), "%s", rx);
 }
  {
     enum blackhawk7_l8p2_rx_pam_mode_enum pam_mode = NRZ;
     EFUN(blackhawk7_l8p2_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));
     istate->rx_pam_mode = (uint8_t)pam_mode;
  ...
  EFUN(blackhawk7_l8p2_INTERNAL_sigdet_status(sa__,&istate->sig_det, &istate->sig_det_chg));
  ESTM(istate->ppm = rd_cdr_integ_reg()/84);

  ESTM(istate->clk90 = rdv_usr_sts_phase_hoffset());

  EFUN(blackhawk7_l8p2_INTERNAL_decode_br_os_mode(sa__, &istate->CDR));

  EFUN(blackhawk7_l8p2_read_rx_afe(sa__, RX_AFE_PF, &istate->pf_main));
  EFUN(blackhawk7_l8p2_read_rx_afe(sa__, RX_AFE_PF2, &istate->pf2_ctrl));
  ESTM(istate->bwr_ctrl = rdv_afe_bw_adjust());

  EFUN(blackhawk7_l8p2_read_rx_afe(sa__, RX_AFE_VGA, &istate->VGA));

  ESTM(istate->DCO = rd_dc_offset_bin());
  EFUN(blackhawk7_l8p2_read_rx_afe(sa__, RX_AFE_PF3, &istate->pf3_ctrl));
  ESTM(istate->P1mV = rdv_usr_main_tap_est()/32);
  EFUN(blackhawk7_l8p2_INTERNAL_ladder_setting_to_mV(sa__, istate->P1mV, 0, INVALID_SLICER, &istate->P1mV));

  if (istate->rx_pam_mode == NRZ) {
      ESTM(istate->TP0  = rd_rx_phase02_status());
      EFUN(blackhawk7_l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)istate->TP0, 0, INVALID_SLICER, &istate->TP0));
      istate->usr_status_eq_debug1 = 0;
      istate->afe_bw = 0;
  }
  else {
      ESTM(istate->TP0 = rdv_usr_ctrl_pam4_chn_loss());

      ESTM(istate->usr_status_eq_debug1 = rdv_usr_status_eq_debug1());
      ESTM(istate->afe_bw = rdv_usr_status_afe_bw_dbg_0());
  }
  ...
  if ((istate->TP1 == 0) && (istate->blind_ctle_trnsum < 0)) {
      istate->afe_bw_metric = istate->blind_ctle_trnsum;
  } else {
      istate->afe_bw_metric = istate->TP1;
  }
  ESTM(istate->tp_metric_prec1 = rdv_usr_status_tp_metric_1());
  if (istate->rx_pam_mode == NRZ) {
      EFUN(blackhawk7_l8p2_INTERNAL_get_rx_dfe1(sa__, &istate->DFE1));
  }

  EFUN(blackhawk7_l8p2_INTERNAL_get_rx_dfe2(sa__, &istate->DFE2));
  EFUN(blackhawk7_l8p2_INTERNAL_get_rx_dfe3(sa__, &istate->DFE3));
  EFUN(blackhawk7_l8p2_INTERNAL_get_rx_dfe4(sa__, &istate->DFE4));
  EFUN(blackhawk7_l8p2_INTERNAL_get_rx_dfe5(sa__, &istate->DFE5));

  EFUN(blackhawk7_l8p2_INTERNAL_get_rx_dfe6(sa__, &istate->dfe6));

  EFUN(blackhawk7_l8p2_INTERNAL_get_tx_ppm(sa__, &istate->tx_ppm));
  ...
  ESTM(istate->txfir_use_pam4_range = rd_txfir_nrz_tap_range_sel() ? 0 : 1);

  {
      uint8_t tap_num;
      for (tap_num=0; tap_num<TXFIR_ST_NUM_TAPS; ++tap_num) {
          EFUN(blackhawk7_l8p2_INTERNAL_get_tx_tap(sa__, tap_num, &istate->txfir.tap[tap_num]));
      }
      ESTM(istate->enable_6taps = rd_txfir_tap_en());
      if (istate->enable_6taps) {
          istate->TXEQ_n2  = istate->txfir.tap[0];
          istate->TXEQ_n1  = istate->txfir.tap[1];
          istate->TXEQ_m   = istate->txfir.tap[2];
          istate->TXEQ_p1  =  istate->txfir.tap[3];
          istate->TXEQ_p2  =  istate->txfir.tap[4];
          istate->TXEQ_p3  = istate->txfir.tap[5];
      } else {
          istate->TXEQ_n1 = istate->txfir.tap[0];
          istate->TXEQ_m  = istate->txfir.tap[1];
          istate->TXEQ_p1 = istate->txfir.tap[2];
      }
  }

  EFUN(blackhawk7_l8p2_INTERNAL_get_eye_margin_est(sa__, &istate->heye_left, &istate->heye_right, &istate->veye_upper, &istate->veye_lower));
  {
      struct blackhawk7_l8p2_usr_ctrl_disable_functions_st dsu, dss;
      uint8_t nrz_mode_en=0, pam4_mode_en=0;
      EFUN(blackhawk7_l8p2_get_usr_ctrl_disable_startup(sa__, &dsu));
      EFUN(blackhawk7_l8p2_get_usr_ctrl_disable_steady_state(sa__, &dss));
      nrz_mode_en = (istate->rx_pam_mode == NRZ) && ((dsu.field.eye_margin_estimation) || (dss.field.eye_margin_estimation));
      pam4_mode_en = (istate->rx_pam_mode != NRZ) && ((dsu.field.pam_eye_margin_estimation) || (dss.field.pam_eye_margin_estimation));
      istate->disable_eye_display = nrz_mode_en || pam4_mode_en;
  }
  EFUN(blackhawk7_l8p2_INTERNAL_get_link_time(sa__, &istate->link_time));
  {
      ...
      if (istate->pmd_lock != rx_lock_at_end) {
          USR_PRINTF(("WARNING: Lane %d rx_lock status changed while reading lane state (rx_lock_start=%d, rx_lock_chg_start=%d, rx_lock_at_end=%d, rx_lock_chg_at_end=%d) \n",
                      blackhawk7_l8p2_acc_get_lane(sa__), istate->pmd_lock, istate->pmd_lock_chg, rx_lock_at_end, rx_lock_chg_at_end));
      }
  ...
  EFUN(blackhawk7_l8p2_INTERNAL_get_BER_string(sa__, 100, istate->BER, sizeof(istate->BER)));

  {
    const char * const prepend = "    ";

    if(istate->UC_STS != 0) {
        EFUN(blackhawk7_l8p2_decode_uc_sts(sa__, istate->UC_STS, istate->uc_sts_decoded, sizeof(istate->uc_sts_decoded), prepend));
    }

    if(istate->UC_STS_EXT != 0) {
        EFUN(blackhawk7_l8p2_decode_uc_sts_ext(sa__, istate->UC_STS_EXT, istate->uc_sts_ext_decoded, sizeof(istate->uc_sts_ext_decoded), prepend));
    }
  }
...
err_code_t blackhawk7_l8p2_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {
  ...
  blackhawk7_l8p2_lane_state_st_t *state;

  
  state = (blackhawk7_l8p2_lane_state_st_t *)USR_CALLOC(1, sizeof(blackhawk7_l8p2_lane_state_st_t));
  if(!state) {
      return (ERR_CODE_MEM_ALLOC_FAIL);
  }
  CFUN(blackhawk7_l8p2_INTERNAL_read_lane_state(sa__, state));
  ...
  if (state->link_time == ((0xFFFFL<<BLACKHAWK7_L8P2_LANE_TIMER_SHIFT)/10)) {
...
err_code_t blackhawk7_l8p2_INTERNAL_read_core_state(srds_access_t *sa__, blackhawk7_l8p2_core_state_st *istate) {
  ...
  EFUN(blackhawk7_l8p2_read_die_temperature(sa__, &state.die_temp));
...
err_code_t blackhawk7_l8p2_INTERNAL_display_core_state_no_newline(srds_access_t *sa__) {
    ...
    EFUN_PRINTF((" %01d%s"             ,  state_pll1.pll_lock, state_pll1.pll_lock_chg ? "*" : " "));
  {
    ...
    if(state.core_status != 0) {
        EFUN_PRINTF(("\n"));
      EFUN(blackhawk7_l8p2_decode_core_sts(sa__, state.core_status, decode_buf, sizeof(decode_buf), ""));
...
static err_code_t _blackhawk7_l8p2_minimize_divisor_ratio(uint32_t md, uint32_t *simple_numerator, uint32_t *simple_denominator) {
...
static err_code_t _blackhawk7_l8p2_get_divisor_ratio(srds_access_t *sa__, enum blackhawk7_l8p2_pll_div_enum srds_div, uint16_t *simple_numerator, uint16_t *simple_denominator){
    /*
    ...
    */
    ...
    if (lowest_numerator >= 0x10000){
        _blackhawk7_l8p2_minimize_divisor_ratio(0x10000,&lowest_numerator, &lowest_denominator);
...
static const enum blackhawk7_l8p2_pll_div_enum _blackhawk7_l8p2_div_candidates[] = {
    ...
    BLACKHAWK7_L8P2_PLL_DIV_137P721519,
    BLACKHAWK7_L8P2_PLL_DIV_140,
    BLACKHAWK7_L8P2_PLL_DIV_142P0253165,
    BLACKHAWK7_L8P2_PLL_DIV_144,
    BLACKHAWK7_L8P2_PLL_DIV_152,
    BLACKHAWK7_L8P2_PLL_DIV_165P7770801,
    BLACKHAWK7_L8P2_PLL_DIV_170P800628,
    BLACKHAWK7_L8P2_PLL_DIV_165P0025695,
    BLACKHAWK7_L8P2_PLL_DIV_170P0026474,
    BLACKHAWK7_L8P2_PLL_DIV_178P8959577,
    BLACKHAWK7_L8P2_PLL_DIV_178P8951591,
    BLACKHAWK7_L8P2_PLL_DIV_179P7348805,
    BLACKHAWK7_L8P2_PLL_DIV_179P7356828,
    BLACKHAWK7_L8P2_PLL_DIV_180P5309734,
...
err_code_t blackhawk7_l8p2_INTERNAL_check_vco_freq_khz(srds_access_t *sa__, uint32_t vco_freq_khz) {
...
err_code_t blackhawk7_l8p2_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    if (*vco_freq_khz != 0) {
        EFUN(blackhawk7_l8p2_INTERNAL_check_vco_freq_khz(sa__,*vco_freq_khz));
    }
...
err_code_t blackhawk7_l8p2_INTERNAL_ram_dump_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk7_l8p2_INTERNAL_ram_dump_state_t * const state_ptr = (blackhawk7_l8p2_INTERNAL_ram_dump_state_t *)arg;
...
err_code_t blackhawk7_l8p2_INTERNAL_rdblk_ram_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    INIT_SRDS_ERR_CODE

    blackhawk7_l8p2_INTERNAL_rdblk_ram_arg_t *  cast_arg = (blackhawk7_l8p2_INTERNAL_rdblk_ram_arg_t *)arg;
    blackhawk7_l8p2_INTERNAL_ram_dump_state_t * const state_ptr = (blackhawk7_l8p2_INTERNAL_ram_dump_state_t *)(cast_arg->dump_state_ptr);
    ...
    if (cast_arg->ram_buff == NULL) {
        EFUN(blackhawk7_l8p2_INTERNAL_ram_dump_callback(sa__, cast_arg->dump_state_ptr, byte_count,data));
    }
...
uint32_t _blackhawk7_l8p2_INTERNAL_get_addr_from_lane(srds_access_t *sa__, uint16_t addr, uint8_t lane) {
 ...
 uint32_t _blackhawk7_l8p2_INTERNAL_get_uc_addr_from_lane(srds_access_t *sa__, uint16_t addr, uint8_t lane) {
 ...
 uint32_t _blackhawk7_l8p2_INTERNAL_get_addr_from_core(srds_access_t *sa__, uint16_t addr) {
...
err_code_t blackhawk7_l8p2_INTERNAL_rdblk_ram_read_callback(srds_access_t * sa__,void *arg,uint8_t byte_count,uint16_t data) {
    INIT_SRDS_ERR_CODE

    blackhawk7_l8p2_INTERNAL_rdblk_ram_arg_t * const cast_arg = (blackhawk7_l8p2_INTERNAL_rdblk_ram_arg_t *)arg;
...
err_code_t blackhawk7_l8p2_get_dbgfb_summary(srds_access_t *sa__, blackhawk7_l8p2_dbgfb_cfg_st *dbgfb_cfg) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l8p2_dbgfb_collect_stats(srds_access_t *sa__, uint32_t time_in_us, blackhawk7_l8p2_dbgfb_stats_st *dbgfb_stats) {
    INIT_SRDS_ERR_CODE
    EFUN(blackhawk7_l8p2_dbgfb_collect_stats_api_method(sa__, time_in_us, dbgfb_stats));
    return (ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_dbgfb_collect_stats_api_method(srds_access_t *sa__, uint32_t time_in_us, blackhawk7_l8p2_dbgfb_stats_st *dbgfb_stats) {
    INIT_SRDS_ERR_CODE
    uint8_t y, x;
    uint8_t max_y;
    enum blackhawk7_l8p2_rx_pam_mode_enum pam_mode = NRZ;
    uint8_t pam4es_en = 0;
    blackhawk7_l8p2_dbgfb_cfg_st dbgfb_cfg = DBGFB_CFG_ST_INIT;
...
int8_t blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(int8_t sgb){
    int16_t local_sgb = 0;
...
err_code_t blackhawk7_l8p2_INTERNAL_fit_second_order(srds_access_t *sa__, uint8_t n, USR_DOUBLE *x, USR_DOUBLE *y, USR_DOUBLE *a) {
#ifdef SERDES_API_FLOATING_POINT
    uint8_t    i, j, k;
...
err_code_t blackhawk7_l8p2_INTERNAL_is_flr_supported(srds_access_t *sa__, uint8_t *flr_support) {
    INIT_SRDS_ERR_CODE
    srds_info_t * const blackhawk7_l8p2_info_ptr = blackhawk7_l8p2_INTERNAL_get_blackhawk7_l8p2_info_ptr_with_check(sa__);
    EFUN(blackhawk7_l8p2_INTERNAL_match_ucode_from_info(sa__, blackhawk7_l8p2_info_ptr));

    *flr_support = (blackhawk7_l8p2_info_ptr->lane_var_ram_size >= 0x950);
    return ERR_CODE_NONE;
}
