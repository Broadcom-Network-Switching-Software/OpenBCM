...
static err_code_t _blackhawk7_l8p2_reg_print_no_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr);
static err_code_t _blackhawk7_l8p2_reg_print_with_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr, char reg_buffer[SRDS_DUMP_BUF_SIZE]);
static err_code_t _blackhawk7_l8p2_reg_print(srds_access_t *sa__, uint8_t reg_start_index, uint8_t reg_end_index, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index);
...
static err_code_t _blackhawk7_l8p2_reg_print_no_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr) {
    ...
    for (offset = 0; offset < 0x10; offset++) {
        if(valid_addr & (0x1 << offset)) {
            EFUN(blackhawk7_l8p2_acc_rdt_reg(sa__, (uint16_t)(start_addr + offset), &rddata));
        }
        else {
            rddata = 0x0;
        }
...
static err_code_t _blackhawk7_l8p2_reg_print(srds_access_t *sa__, uint8_t reg_start_index, uint8_t reg_end_index, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index) {
    ...
    if(reg_buffer == NULL) {
        for(reg_section = reg_start_index; reg_section < reg_end_index; reg_section++) {
            EFUN(_blackhawk7_l8p2_reg_print_no_buf(sa__, blackhawk7_l8p2_reg_dump_arr[reg_section][0], blackhawk7_l8p2_reg_dump_arr[reg_section][1]));
        }
    }
    else {
        for(reg_section = reg_start_index; reg_section < reg_end_index; reg_section++) {
            EFUN(_blackhawk7_l8p2_reg_print_with_buf(sa__, blackhawk7_l8p2_reg_dump_arr[reg_section][0], blackhawk7_l8p2_reg_dump_arr[reg_section][1], reg_buffer[*buf_index]));
            (*buf_index)++;
        }
    }
    return (ERR_CODE_NONE);
...
err_code_t blackhawk7_l8p2_reg_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char reg_buffer[][SRDS_DUMP_BUF_SIZE]) {
    ...
    UNUSED(core);
    UNUSED(lane);
    EFUN(blackhawk7_l8p2_reg_select_dump(sa__, SRDS_LANE_AND_CORE_REG, reg_buffer));
    return (ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_reg_select_dump(srds_access_t *sa__, uint8_t reg_dump_control, char reg_buffer[][SRDS_DUMP_BUF_SIZE]) {
    INIT_SRDS_ERR_CODE
    uint8_t core = blackhawk7_l8p2_acc_get_core(sa__);
    uint8_t lane = blackhawk7_l8p2_acc_get_lane(sa__);
    uint8_t buf_index = 0;
    ...
    if (reg_dump_control & SRDS_LANE_REG_ONLY) {
        /* Lane Registers */
        EFUN(_blackhawk7_l8p2_reg_print(sa__, 0, BLACKHAWK7_L8P2_REG_CORE_START_INDEX, reg_buffer, &buf_index));
    }
    if (reg_dump_control & SRDS_CORE_REG_ONLY) {
        /* Core Registers */
        EFUN(_blackhawk7_l8p2_reg_print(sa__, BLACKHAWK7_L8P2_REG_CORE_START_INDEX, BLACKHAWK7_L8P2_REG_PLL_START_INDEX, reg_buffer, &buf_index));
        ...
        for (pll_idx = 0; pll_idx < NUM_PLLS; pll_idx++) {
            ...
            EFUN(_blackhawk7_l8p2_reg_print(sa__, BLACKHAWK7_L8P2_REG_PLL_START_INDEX, BLACKHAWK7_L8P2_REG_UC_START_INDEX, reg_buffer, &buf_index));
        }
        ...
        for(micro_idx = 0; micro_idx < num_micros; micro_idx++) {
            ...
            EFUN(_blackhawk7_l8p2_reg_print(sa__, BLACKHAWK7_L8P2_REG_UC_START_INDEX, BLACKHAWK7_L8P2_REG_DUMP_SECTIONS, reg_buffer, &buf_index));
        }
...
err_code_t blackhawk7_l8p2_uc_core_var_blk_dump(srds_access_t *sa__,uint8_t core, uint16_t data_buff_size,uint8_t *data_buffer) {
      INIT_SRDS_ERR_CODE

      srds_info_t * blackhawk7_l8p2_info_ptr = blackhawk7_l8p2_INTERNAL_get_blackhawk7_l8p2_info_ptr_with_check(sa__);
      blackhawk7_l8p2_INTERNAL_rdblk_ram_arg_t rdblk_state;
      blackhawk7_l8p2_INTERNAL_ram_dump_state_t state;
      uint32_t core_var_ram_size = 0;
      EFUN(blackhawk7_l8p2_INTERNAL_match_ucode_from_info(sa__, blackhawk7_l8p2_info_ptr));
      core_var_ram_size = (uint32_t)blackhawk7_l8p2_info_ptr->core_var_ram_size;
      state.index = 0;
      ...
      EFUN(blackhawk7_l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                      ...
                                                      blackhawk7_l8p2_info_ptr->core_var_ram_size,
                                                      &rdblk_state,
                                                      blackhawk7_l8p2_INTERNAL_rdblk_ram_read_callback));
...
err_code_t blackhawk7_l8p2_uc_micro_var_blk_dump(srds_access_t *sa__,uint8_t core,uint8_t lane,uint16_t data_buff_size,uint8_t *data_buffer) {
      ...
      srds_info_t * blackhawk7_l8p2_info_ptr = blackhawk7_l8p2_INTERNAL_get_blackhawk7_l8p2_info_ptr_with_check(sa__);
      EFUN(blackhawk7_l8p2_INTERNAL_match_ucode_from_info(sa__, blackhawk7_l8p2_info_ptr));
      micro_var_ram_size = blackhawk7_l8p2_info_ptr->micro_var_ram_size;
      ...
      EFUN_PRINTF(("\n**** SERDES UC CORE %d MICRO %d RAM VARIABLE DECODED ****\n", core,lane/2));
          ...
          uc_addr_offset = _blackhawk7_l8p2_INTERNAL_get_uc_addr_from_lane(sa__, 0,lane);
...
err_code_t blackhawk7_l8p2_uc_lane_var_blk_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, uint32_t data_buff_size,uint8_t *data_buffer) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(blackhawk7_l8p2_pmd_lock_status(sa__, &rx_lock));
    ...
    lane = blackhawk7_l8p2_acc_get_physical_lane(sa__);
    lane_addr_offset = _blackhawk7_l8p2_INTERNAL_get_addr_from_lane(sa__, 0 ,lane);
...
err_code_t blackhawk7_l8p2_internal_lpbk(srds_access_t *sa__, uint8_t enable) {
    ...
    switch (lane) {
        ...
        default:
            ...
            return(blackhawk7_l8p2_error(sa__, ERR_CODE_BAD_LANE));
    }
    ...
    if(enable) {
        ...
        EFUN(blackhawk7_l8p2_reset_lane_to_default(sa__));                                /* Toggle lane soft reset */
...
err_code_t blackhawk7_l8p2_dig_lpbk(srds_access_t *sa__, uint8_t enable) {
    ...
    if(blackhawk7_l8p2_info_ptr->info_table_version < 0x39) {
...
err_code_t blackhawk7_l8p2_isolate_lane_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    ...
    EFUN(blackhawk7_l8p2_isolate_lane_ctrl_tx_pins(sa__, enable));
    EFUN(blackhawk7_l8p2_isolate_lane_ctrl_rx_pins(sa__, enable));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_log_full_pmd_state_noPRBS(srds_access_t *sa__, struct blackhawk7_l8p2_detailed_lane_status_st *lane_st) {
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_pmd_lock_status(sa__,&lane_st->pmd_lock, &lane_st->pmd_lock_chg));
    {
        err_code_t err_code=ERR_CODE_NONE;
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_sigdet_status(sa__, &lane_st->sigdet, &lane_st->sigdet_chg));
    ...
    {
        int8_t lms_thresh;
        EFUN(blackhawk7_l8p2_INTERNAL_get_lms_thresh_bin(sa__, &lms_thresh)); lane_st->lms_thresh = lms_thresh;
    }
    ...
    ESTM(lane_st->dfe[4][0]  = blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap5()));
    ESTM(lane_st->dfe[4][1]  = blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap5()));
    ESTM(lane_st->dfe[5][0]  = blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap6()));
    ESTM(lane_st->dfe[5][1]  = blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap6()));
    ESTM(lane_st->dfe[6][0]  = ((rd_rxa_dfe_tap7_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap7()):0));
    ESTM(lane_st->dfe[6][1]  = ((rd_rxb_dfe_tap7_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap7()):0));
    ESTM(lane_st->dfe[7][0]  = ((rd_rxa_dfe_tap8_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap8()):0));
    ESTM(lane_st->dfe[7][1]  = ((rd_rxb_dfe_tap8_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap8()):0));
    ESTM(lane_st->dfe[8][0]  = ((rd_rxa_dfe_tap9_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap9()):0));
    ESTM(lane_st->dfe[8][1]  = ((rd_rxb_dfe_tap9_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap9()):0));
    ESTM(lane_st->dfe[9][0]  = ((rd_rxa_dfe_tap10_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap10()):0));
    ESTM(lane_st->dfe[9][1]  = ((rd_rxb_dfe_tap10_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap10()):0));
    ESTM(lane_st->dfe[10][0] = ((rd_rxa_dfe_tap7_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap7()):(rd_rxa_dfe_tap11_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap11()):0));
    ESTM(lane_st->dfe[10][1] = ((rd_rxb_dfe_tap7_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap7()):(rd_rxb_dfe_tap11_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap11()):0));
    ESTM(lane_st->dfe[11][0] = ((rd_rxa_dfe_tap8_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap8()):(rd_rxa_dfe_tap12_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap12()):0));
    ESTM(lane_st->dfe[11][1] = ((rd_rxb_dfe_tap8_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap8()):(rd_rxb_dfe_tap12_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap12()):0));
    ESTM(lane_st->dfe[12][0] = ((rd_rxa_dfe_tap9_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap9()):(rd_rxa_dfe_tap13_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap13()):0));
    ESTM(lane_st->dfe[12][1] = ((rd_rxb_dfe_tap9_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap9()):(rd_rxb_dfe_tap13_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap13()):0));
    ESTM(lane_st->dfe[13][0] = ((rd_rxa_dfe_tap10_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap10()):(rd_rxa_dfe_tap14_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap14()):0));
    ESTM(lane_st->dfe[13][1] = ((rd_rxb_dfe_tap10_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap10()):(rd_rxb_dfe_tap14_mux()==0)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap14()):0));
    ESTM(lane_st->dfe[14][0] = ((rd_rxa_dfe_tap7_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap7()):(rd_rxa_dfe_tap11_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap11()):0));
    ESTM(lane_st->dfe[14][1] = ((rd_rxb_dfe_tap7_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap7()):(rd_rxb_dfe_tap11_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap11()):0));
    ESTM(lane_st->dfe[15][0] = ((rd_rxa_dfe_tap8_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap8()):(rd_rxa_dfe_tap12_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap12()):0));
    ESTM(lane_st->dfe[15][1] = ((rd_rxb_dfe_tap8_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap8()):(rd_rxb_dfe_tap12_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap12()):0));
    ESTM(lane_st->dfe[16][0] = ((rd_rxa_dfe_tap9_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap9()):(rd_rxa_dfe_tap13_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap13()):0));
    ESTM(lane_st->dfe[16][1] = ((rd_rxb_dfe_tap9_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap9()):(rd_rxb_dfe_tap13_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap13()):0));
    ESTM(lane_st->dfe[17][0] = ((rd_rxa_dfe_tap10_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap10()):(rd_rxa_dfe_tap14_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap14()):0));
    ESTM(lane_st->dfe[17][1] = ((rd_rxb_dfe_tap10_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap10()):(rd_rxb_dfe_tap14_mux()==1)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap14()):0));
    ESTM(lane_st->dfe[18][0] = ((rd_rxa_dfe_tap7_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap7()):(rd_rxa_dfe_tap11_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap11()):0));
    ESTM(lane_st->dfe[18][1] = ((rd_rxb_dfe_tap7_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap7()):(rd_rxb_dfe_tap11_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap11()):0));
    ESTM(lane_st->dfe[19][0] = ((rd_rxa_dfe_tap8_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap8()):(rd_rxa_dfe_tap12_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap12()):0));
    ESTM(lane_st->dfe[19][1] = ((rd_rxb_dfe_tap8_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap8()):(rd_rxb_dfe_tap12_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap12()):0));
    ESTM(lane_st->dfe[20][0] = ((rd_rxa_dfe_tap9_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap9()):(rd_rxa_dfe_tap13_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap13()):0));
    ESTM(lane_st->dfe[20][1] = ((rd_rxb_dfe_tap9_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap9()):(rd_rxb_dfe_tap13_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap13()):0));
    ESTM(lane_st->dfe[21][0] = ((rd_rxa_dfe_tap10_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap10()):(rd_rxa_dfe_tap14_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap14()):0));
    ESTM(lane_st->dfe[21][1] = ((rd_rxb_dfe_tap10_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap10()):(rd_rxb_dfe_tap14_mux()==2)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap14()):0));
    ESTM(lane_st->dfe[22][0] = ((rd_rxa_dfe_tap11_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap11()):0));
    ESTM(lane_st->dfe[22][1] = ((rd_rxb_dfe_tap11_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap11()):0));
    ESTM(lane_st->dfe[23][0] = ((rd_rxa_dfe_tap12_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap12()):0));
    ESTM(lane_st->dfe[23][1] = ((rd_rxb_dfe_tap12_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap12()):0));
    ESTM(lane_st->dfe[24][0] = ((rd_rxa_dfe_tap13_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap13()):0));
    ESTM(lane_st->dfe[24][1] = ((rd_rxb_dfe_tap13_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap13()):0));
    ESTM(lane_st->dfe[25][0] = ((rd_rxa_dfe_tap14_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxa_dfe_tap14()):0));
    ESTM(lane_st->dfe[25][1] = ((rd_rxb_dfe_tap14_mux()==3)?blackhawk7_l8p2_INTERNAL_dfe_tap_sgb2sg(rd_rxb_dfe_tap14()):0));
...
err_code_t blackhawk7_l8p2_log_full_pmd_state (srds_access_t *sa__, struct blackhawk7_l8p2_detailed_lane_status_st *lane_st) {
...

  {
    ...
    EFUN(blackhawk7_l8p2_prbs_err_count_state(sa__, &lane_st->prbs_chk_errcnt, &lane_st->prbs_chk_lock_lost));
    if (lane_st->prbs_chk_lock && (lane_st->rx_pam_mode != NRZ)) {
        EFUN(blackhawk7_l8p2_dbgfb_collect_stats(sa__, 8000, &lane_st->dbgfb_stats));
    }
    EFUN(blackhawk7_l8p2_INTERNAL_get_BER_string(sa__,100,&lane_st->ber_string[0], sizeof(lane_st->ber_string)));
...
err_code_t blackhawk7_l8p2_disp_full_pmd_state (srds_access_t *sa__, struct blackhawk7_l8p2_detailed_lane_status_st const * const lane_st, uint8_t num_lanes) {
    ...
    if((num_lanes != 1) && (num_lanes != 2) && (num_lanes != 4) && (num_lanes != 8)) {
      ...
      ESTM_PRINTF(("%02x ", (uint8_t)sizeof(struct blackhawk7_l8p2_check_platform_info_st)));
...
void blackhawk7_l8p2_INTERNAL_print_triage_info(srds_access_t *sa__, err_code_t err_code, uint8_t print_header, uint8_t print_data, uint16_t line)
{
    ...
    if (blackhawk7_l8p2_pmd_lock_status(sa__, &info.pmd_lock)) {
        error_seen = 1;
    }
