...
STATIC
int _falcon_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    switch(osr_mode) {
    case FALCON_TSC_OSX1:      return 1000;
    case FALCON_TSC_OSX2:      return 2000;
    case FALCON_TSC_OSX4:      return 4000;
    case FALCON_TSC_OSX16P5:   return 16500;
    case FALCON_TSC_OSX20P625: return 20625;
    default:                   return 1000;
    }
}

/* get number bits per ms */
STATIC
int _falcon_get_num_bits_per_ms(const phymod_access_t *pa, uint32_t *num_bits_per_ms) {
    int osr_mode = 0;
    struct falcon_tsc_uc_core_config_st core_cfg;

    PHYMOD_IF_ERR_RETURN
        (falcon_osr_mode_get(pa, &osr_mode));

    PHYMOD_IF_ERR_RETURN(falcon_tsc_get_uc_core_config(pa, &core_cfg));
    *num_bits_per_ms = (((uint32_t)core_cfg.vco_rate_in_Mhz * 100000UL) / _falcon_osr_mode_enum_to_int_x1000(osr_mode))*10;
...
STATIC
int _falcon_get_BER_data(const phymod_access_t *pa, uint16_t time_ms, uint8_t *prbs_chk_en, uint8_t *lcklost, uint64_t *num_errs, uint64_t *num_bits)
{
    uint32_t err_cnt = 0;
    /* prbs status */
    PHYMOD_IF_ERR_RETURN
        (falcon_tsc_get_rx_prbs_en(pa, prbs_chk_en));
    if (*prbs_chk_en == 0) {
        return PHYMOD_E_NONE;
    }
    /* clear error counters */
    PHYMOD_IF_ERR_RETURN
        (falcon_tsc_prbs_err_count_state(pa, &err_cnt, lcklost));

    falcon_tsc_delay_us(time_ms*1000);
    /* read error counters */
    PHYMOD_IF_ERR_RETURN
        (falcon_tsc_prbs_err_count_state(pa, &err_cnt, lcklost));
    if (*lcklost == 0) {
        ...
        PHYMOD_IF_ERR_RETURN
            (_falcon_get_num_bits_per_ms(pa, &num_bits_per_ms));
        COMPILER_64_SET(*num_bits, 0, num_bits_per_ms);
        COMPILER_64_UMUL_32(*num_bits, time_ms);
        COMPILER_64_SET(*num_errs, 0, err_cnt);
    }
...
int falcon_phy_fast_ber_proj_get(const phymod_phy_access_t* phy, uint32_t* ber_proj_data)
{
    int start_lane, num_lane;
    phymod_phy_access_t phy_copy;
    uint8_t prbs_chk_en = 0, lcklost = 0;
    uint64_t num_errs;
    uint64_t num_bits;
    ...
    /* collect prbs in 1000 ms */
    PHYMOD_IF_ERR_RETURN
        (_falcon_get_BER_data(&phy_copy.access, 1000, &prbs_chk_en, &lcklost, &num_errs, &num_bits));

    /* first check if PRBS enabled or prbs lost lock */
    if ((prbs_chk_en == 0) || (lcklost == 1)) {
        *ber_proj_data = 0xffffffff;
    } else {
        uint16_t x = 0,y = 0,z = 0,div;

        if (COMPILER_64_GE(num_errs, num_bits)) {
            x = 1;
            y = 0;
            z = 0;
        } else if (COMPILER_64_IS_ZERO(num_errs)) {
            x = 0;
            y = 0;
            z = 0;
        } else {
            ...
            while (1) {
                 ...
                 */
                COMPILER_64_COPY(tmp_num_errs, num_errs);
                COMPILER_64_COPY(tmp_num_bits, num_bits);

                /* ber_data_local.num_errs << 1 */
                COMPILER_64_SHL(tmp_num_errs, 1);

                /* (ber_data_local.num_errs << 1) + ber_data_local.num_bits */
                COMPILER_64_ADD_64(tmp_num_errs, num_bits);
                 ...
                 */
                COMPILER_64_UMUL_32(num_errs, (uint32_t) 10);
                z = z + 1;
            }
            if(div >= 100) {
                div = div / 10;
