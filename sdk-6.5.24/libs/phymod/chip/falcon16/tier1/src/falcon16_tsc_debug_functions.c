...
#include "../include/falcon16_tsc_reg_dump.h"
static err_code_t _falcon16_tsc_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section);
static err_code_t _falcon16_tsc_reg_print_with_buf(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
static err_code_t _falcon16_tsc_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
...
err_code_t falcon16_tsc_stop_uc_lane(srds_access_t *sa__, uint8_t enable) {
    ...
    return(falcon16_tsc_stop_rx_adaptation(sa__,enable));
}
...
static err_code_t _falcon16_tsc_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section)
{
    ...
    for (addr = falcon16_tsc_reg_dump_arr[reg_section][0]; addr < falcon16_tsc_reg_dump_arr[reg_section][1]; addr++) 
    {
        if (!(addr & 0x000F))  
        {
            EFUN_PRINTF(("\n%04x ",addr));
...
static err_code_t _falcon16_tsc_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index)
{
    if(reg_buffer == NULL)
    {
         EFUN(_falcon16_tsc_reg_print_no_buf(sa__, reg_section));
    }
    else
    {
        EFUN(_falcon16_tsc_reg_print_with_buf(sa__, reg_section, reg_buffer, buf_index));
    }
    return (ERR_CODE_NONE);
}
err_code_t falcon16_tsc_reg_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char reg_buffer[][250])
{    
    ...
    while(reg_section < FALCON16_TSC_REG_DUMP_SECTIONS) 
    {
        if(falcon16_tsc_reg_dump_arr[reg_section][0] == FALCON16_TSC_REG_PLL_START)
        {
                while( reg_section < FALCON16_TSC_REG_DUMP_SECTIONS && falcon16_tsc_reg_dump_arr[reg_section][0] != FALCON16_TSC_REG_UC_START)
                {
                    EFUN(_falcon16_tsc_reg_print(sa__, reg_section, reg_buffer, &buf_index));
                    reg_section++;
                }
        }
        else if (falcon16_tsc_reg_dump_arr[reg_section][0] == FALCON16_TSC_REG_UC_START)
        {
            ...
            ESTM(micro_orig = falcon16_tsc_get_micro_idx(sa__));
            for(micro_idx = 0; micro_idx < num_micros; micro_idx++)
            {
                reg_section = uc_start_section;
                EFUN(falcon16_tsc_set_micro_idx(sa__,micro_idx));
#endif
                while(reg_section < FALCON16_TSC_REG_DUMP_SECTIONS)
                {
                    EFUN(_falcon16_tsc_reg_print(sa__, reg_section, reg_buffer, &buf_index));
            ...
            EFUN(falcon16_tsc_set_micro_idx(sa__,micro_orig));
#endif
        }
        else
        {
...
err_code_t falcon16_tsc_uc_core_var_dump(srds_access_t *sa__, uint8_t core, char ram_buffer[][250]) {
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < core_var_ram_size; addr++) {
          ...
          ESTM(rddata = falcon16_tsc_rdbc_uc_var(sa__, __ERR, addr));
...
err_code_t falcon16_tsc_uc_lane_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][250]) {
...

  {
      ...
      uc_stopped = falcon16_tsc_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < lane_var_ram_size; addr++) {
         ...
         ESTM(rddata = falcon16_tsc_rdbl_uc_var(sa__, __ERR, addr));
         USR_SPRINTF(tempBuf,"%02x ", rddata);
         USR_STRNCAT(ram_buffer[index], tempBuf, SPC_LEFT(ram_buffer[index]));
      }
      ram_buffer[index+1][0] = 0xA;
...
err_code_t falcon16_tsc_isolate_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    ...
    ESTM(lane_orig = falcon16_tsc_get_lane(sa__));
    ...
    EFUN(falcon16_tsc_set_lane(sa__, lane_orig));
...
err_code_t falcon16_tsc_log_full_pmd_state_noPRBS (srds_access_t *sa__, struct falcon16_tsc_detailed_lane_status_st *lane_st) {
    ...
    int8_t   tmp;
    uint8_t  tmp1;
    const uint8_t big_endian = falcon16_tsc_INTERNAL_is_big_endian();
    ...
    {
        err_code_t err_code=ERR_CODE_NONE;
        lane_st->stop_state = falcon16_tsc_INTERNAL_stop_micro(sa__,lane_st->pmd_lock,&err_code);
        if(err_code) USR_PRINTF(("Unable to stop microcontroller,  following data is suspect\n"));
    }
    ...
    ESTM(falcon16_tsc_INTERNAL_get_rx_vga(sa__, &tmp1)); lane_st->vga = tmp1;
    EFUN(falcon16_tsc_INTERNAL_get_rx_pf_main(sa__, &tmp1)); lane_st->pf = tmp1;
    EFUN(falcon16_tsc_INTERNAL_get_rx_pf2(sa__, &tmp1)); lane_st->pf2 = tmp1;
    ...
    EFUN(falcon16_tsc_INTERNAL_get_eye_margin_est(sa__, &lane_st->heye_left, &lane_st->heye_right, &lane_st->veye_upper, &lane_st->veye_lower));
...
err_code_t falcon16_tsc_log_full_pmd_state (srds_access_t *sa__, struct falcon16_tsc_detailed_lane_status_st *lane_st) {
    ...
    ENULL_MEMSET(lane_st, 0, sizeof(struct falcon16_tsc_detailed_lane_status_st));
    EFUN(falcon16_tsc_log_full_pmd_state_noPRBS(sa__, lane_st));

    ESTM(lane_st->prbs_chk_en = rd_prbs_chk_en());
    EFUN(falcon16_tsc_get_rx_prbs_config(sa__, &prbs_poly_mode, &prbs_checker_mode, &lane_st->prbs_chk_inv));
    ...
    EFUN(falcon16_tsc_INTERNAL_get_BER_string(sa__,100,&lane_st->ber_string[0]));

    return(ERR_CODE_NONE);
}


err_code_t falcon16_tsc_disp_full_pmd_state (srds_access_t *sa__, struct falcon16_tsc_detailed_lane_status_st const * const lane_st, uint8_t num_lanes) {
    ...
    if ((num_lanes != 4) && (num_lanes != 8) && (num_lanes != 1)) {
      return(falcon16_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));  /* Number of lanes has to be 1, 4 or 8 */
    }

    size_of_lane_st = sizeof(struct falcon16_tsc_detailed_lane_status_st);

    EFUN_PRINTF(("\n**** SERDES EXTENDED LANE DIAG DATA CORE %d LANE %d DUMP ****", falcon16_tsc_get_core(sa__), falcon16_tsc_get_lane(sa__)));
    EFUN_PRINTF(("\n0000 "));
...
void falcon16_tsc_INTERNAL_print_triage_info(srds_access_t *sa__, err_code_t err_code, uint8_t print_header, uint8_t print_data, uint16_t line)
{
/*  Note: No EFUNs or ESTMs should be used in this function as this print routine is called by _error() handler. */
    falcon16_tsc_triage_info info;
    err_code_t error;
    ...
    UNUSED(error);
    USR_MEMSET(&info, 0, sizeof(falcon16_tsc_triage_info));
    info.error = err_code;
    info.line = line;
    if (print_header) {
    ...
    error = falcon16_tsc_version(sa__, &info.api_ver);
    if (error) {
    ...
    if(falcon16_tsc_INTERNAL_sigdet_status(sa__, &info.sig_det, &info.sig_det_chg)) {
        error_seen = 1;
    }
    if (falcon16_tsc_INTERNAL_pmd_lock_status(sa__, &info.pmd_lock, &info.pmd_lock_chg)) {
        error_seen = 1;
    }
    ...
    info.core = falcon16_tsc_get_core(sa__);
    info.lane = falcon16_tsc_get_lane(sa__);
    if (print_data) {
        if ((err_code == ERR_CODE_UC_CMD_POLLING_TIMEOUT) || (err_code == ERR_CODE_UC_NOT_STOPPED)) {
            USR_PRINTF(("%4d, %4d,  %X_%X,    %X_%X, %17d, %7d,%d       , %11d,   0x%04x, %8d, %6d,    0x%x,0x%x   , %s\n",
                        ...
                        info.dsc_one_hot[0],
                        info.dsc_one_hot[1],
                        falcon16_tsc_INTERNAL_e2s_err_code(info.error) ));
        } else {
