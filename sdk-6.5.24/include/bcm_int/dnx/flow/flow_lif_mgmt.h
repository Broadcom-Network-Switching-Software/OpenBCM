...
/**
 * \brief - Retrieve a virtual LIF entry from DBAL to the
 *        handle, by using the sw mapping from a virtual gport
 *        to its match keys.
 *  
 *   \param [in] unit - unit id
 ...
 *   shr_error_e
 */
shr_error_e dnx_flow_lif_mgmt_virtual_entry_on_handle_get(
    int unit,
    bcm_gport_t virtual_lif_gport,
    uint32 entry_handle_id);

/**
 * \brief - Perform virtual gport entry removal - DBAL entry
 *        clear, but also valid fields sw state and sw match
 *        mapping. The function assumes that the virtual key is
 *        already set in the DBAL handle.
 *  
 *   \param [in] unit - unit id
 *   \param [in] virtual_lif_gport - The gport of the removed
 *          virtual LIF
 ...
 *   shr_error_e
 */
shr_error_e dnx_flow_lif_mgmt_virtual_entry_clear(
    int unit,
    bcm_gport_t virtual_lif_gport,
    uint32 entry_handle_id);
