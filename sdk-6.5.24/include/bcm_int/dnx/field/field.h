...
#define DNX_FIELD_FEM_CONDITION_ENTRY_VERIFY(_unit,_fem_condition_entry)  \
        ...
        { \
            ...
            { \
                SHR_ERR_EXIT(_SHR_E_PARAM, \
                     ...
                     ((_fem_condition_entry).fem_map_index), 0, (dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_fem_map_index - 1)); \
            } \
        } \
    } \
    else    \
    { \
        SHR_ERR_EXIT(_SHR_E_PARAM, \
             ...
             ((_fem_condition_entry).fem_action_valid), 0, (DBAL_NOF_ENUM_FIELD_FEM_ACTION_VALID_VALUES - 1)); \
    }
/*
...
#define DNX_TCAM_EXTERNAL_TCAM_FUNCTION(_unit, _func_kbp) \
{ \
    if (dnx_data_field.external_tcam.use_kbp_get(_unit)) \
    { \
        SHR_IF_ERR_EXIT(_func_kbp); \
    } \
    else \
    { \
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached external TCAM function but no external TCAM available for device.\n"); \
    } \
}
