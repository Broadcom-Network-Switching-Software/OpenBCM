...
typedef struct _alpm_cb_s {
    ...
    int         split_retry;    /* Current split retry number */
...
#define _ALPM_MERGE_UNCHANGE    1
    ...
    int _alpm_bkt_share_start[ALPM_IPT_CNT];
...
typedef struct _alpm_cb_merge_ctrl_s {
    ...
    int l2_merge_cnt;               /* Accumulated l2 merge counter (include shrink merge) */
    int l3_merge_cnt;               /* Accummulated l3 merge counter (include shrink merge) */
    int l2_shrink_cnt;              /* Accumulated l2 shrink merge counter */
    int l3_shrink_cnt;              /* Accummulated l3 shrink merge counter */
...
typedef int (*tcam_hit_get_f)(int u, _alpm_pvt_node_t *pvt_node,
...
#define ALPM_BKT_SHARE_START(u, ipt) (ALPMC(u)->_alpm_bkt_share_start[ipt])
...
#define ALPM_SPLIT_RETRY_MAX(u)     (ALPMC(u)->_alpm_split_retry_max)
...
#define ACB_SPLIT_RETRY(acb)        ((acb)->split_retry)
...
extern int alpm_bkt_pfx_shrink(int u, _alpm_cb_t *acb,
                               _alpm_pvt_node_t *pvt_node, void *user_data, int *shrink_bnks);
...
extern int tcam_hit_get(int u, _alpm_pvt_node_t *pvt_node, int app, int pkm, int tcam_idx);
