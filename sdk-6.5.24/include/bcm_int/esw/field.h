...
/* Version change to 35 - for 6.5.23
 * This is to sync and recover the extrator codes missing
 * (l3_e1_sel and l2_e1_sel) in TH devices.
 * There is no change in scache but is a checkpoint for the missing
 * extractor codes that ae read from hardware
 */
#define BCM_FIELD_WB_VERSION_1_35               SOC_SCACHE_VERSION(1, 35)

/* Version change to 34 - for 6.5.22
 * This is to sync and recover the extrator codes newly added for th3
 * Starting this version, the extractors are synced and recovered using TLS.
 * for upgrade from lower version, the missing extractors are recovered from
 * hardware and regenerated for a group.
 * Sync and recover of stat_types is added
 */
#define BCM_FIELD_WB_VERSION_1_34               SOC_SCACHE_VERSION(1, 34)
/* Version change to 33 - for 6.5.21
 * This is to sync and recover the max_num_pipes as it is different in case
 * of multipart build with th and th3.
 */
#define BCM_FIELD_WB_VERSION_1_33               SOC_SCACHE_VERSION(1, 33)
...
#define BCM_FIELD_WB_DEFAULT_VERSION            BCM_FIELD_WB_VERSION_1_35
#endif
...
/*
 * Typedef:
 *      _field_ft_tracking_param_qual_stage_map_t
 * Purpose:
 *      Flow group tracking param and qualifier map
 *      for the given stage.
 */
typedef struct _field_ft_tracking_param_qual_stage_map_s {
    bcm_flowtracker_stage_t stage;
    uint32 flags;
    bcm_field_qualify_t qual;
    uint16 offset;
    uint16 width;
    struct _field_ft_tracking_param_qual_stage_map_s *next;
} _field_ft_tracking_param_qual_stage_map_t;
 ...
 *      Flow group tracking param and qualifier map.
 */
typedef struct _field_ft_tracking_param_qual_map_s {
    uint16 param;
    int default_stage;
    uint16 db_ft_type_bmp;
    int num_pq_smap;
    _field_ft_tracking_param_qual_stage_map_t *pq_smap;
} _field_ft_tracking_param_qual_map_t;
...
typedef struct _field_funct_s {
    ...
    int (*fp_dredirect_action_alloc)(int unit, _field_entry_t *f_ent,
                                        _field_action_t *fa);
    int (*fp_control_drop_mask_set)(int unit,
                                    _field_control_t *fc,
                                    bcm_field_control_t control,
                                    bcm_field_control_config_t *fc_cfg);
    int (*fp_control_drop_mask_get)(int unit,
                                    _field_control_t *fc,
                                    bcm_field_control_t control,
                                    bcm_field_control_config_t *fc_cfg);
...
typedef enum _bcm_field_internal_element_e {
    ...
    _bcmFieldInternalGroupDropMaskProfileAction = 392,
    _bcmFieldInternalExtl2e4Sel = 393,
    _bcmFieldInternalExtl2e2Sel = 394,
    _bcmFieldInternalExtl2e1Sel = 395,
    _bcmFieldInternalExtDestContASel = 396,
    _bcmFieldInternalExtDestContBSel = 397,
    _bcmFieldInternalExtUdfSel = 398,
    _bcmFieldInternalExtIpbmSourceSel = 399,
    _bcmFieldInternalEMActionPbmLoopbackCustomSubtype = 400,
    _bcmFieldInternalElementCount = 401
...
typedef struct _field_entry_wb_info_s {
    ...
    uint32                            stat_types;                 /*sync and recover stat types */
...
typedef struct _field_group_add_fsm_s {
    ...
    int                    action_res_id;  /* Action resolution id */
    bcm_field_qset_t       *qset_prev; /* to be used only for group update */
...
#define _BCM_FP_GROUP_SLICE_SHARE_ENABLE       (1 << 5)
...
typedef enum _bcmFieldActionNoParam_e {
    ...
    _bcmFieldActionNoParamRedirectVlan = 68,
    _bcmFieldActionNoParamRedirectIpmcExtender = 69,
    _bcmFieldActionNoParamCount /* 70 */
...
#if defined (BCM_TOMAHAWK_SUPPORT)
extern int
_bcm_esw_th_field_entry_aset_get(
        int unit,
        _field_entry_t *f_ent,
        bcm_field_aset_t *ent_aset);

extern int
_bcm_esw_th_field_entry_qset_get(
        int unit,
        _field_entry_t *f_ent,
        bcm_field_qset_t *ent_qset);
...
extern int
_bcm_field_th_group_mode_slice_get(int unit,
                                   _field_stage_t *stage_fc,
                                   _field_group_t *group,
                                   _field_slice_t **fs);
#endif /* BCM_TOMAHAWK_SUPPORT */
extern int
_bcm_esw_field_entry_enable_set(
        int unit,
        bcm_field_entry_t entry,
        int enable_flag);

extern int
_bcm_esw_field_tcam_hw_idx_info_get(
    int unit,
    _field_stage_id_t stage_id,
    int slice_idx,
    int slice_number,
...
extern int
_bcm_esw_field_entry_reinstall(int unit, bcm_field_entry_t entry);

extern int
_bcm_esw_field_action_remove_from_group(int unit,
        _field_group_t *fg,
