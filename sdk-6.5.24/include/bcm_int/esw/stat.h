...
#define BCM_STAT_GENERIC_REG_VALIDATE(unit, port, ctr, valid)                  \
    if (soc_feature(unit, soc_feature_cxl_mib) ||                              \
        ...
        if (valid == FALSE && SOC_REG_IS_VALID(unit, ctr##r)) {                \
            valid = TRUE;                                                      \
        }                                                                      \
    } else {                                                                   \
        valid = FALSE;                                                         \
...
/*
 * Internal function to enable the L2 source port pruning pkt debug 
 * counter for VP flows.
 */
extern int _bcm_esw_stat_l2_src_port_prune_drop_debug_ctr_set(int unit,
                                                                 int value);
extern int _bcm_esw_stat_l2_src_port_prune_drop_debug_ctr_get(int unit,
                                                                 int *value);
 ...
 * at the egress.
 */
extern int _bcm_esw_stat_egr_mcast_drop_debug_ctr_set(int unit, int value);
extern int _bcm_esw_stat_egr_mcast_drop_debug_ctr_get(int unit, int *value);
