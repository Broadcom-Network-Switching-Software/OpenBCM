...
SHR_G_ENTRY(field_stage_egress_3k_half_slice),/*  Only half entries/slice are used when EFP_TCAM has 3k entries/slice */\
...
SHR_G_ENTRY(access),          /* New access infrastructure             */ \
...
SHR_G_ENTRY(hr4_l3mtu_fail_tocpu_war), /* L3 MTU fail copy to cpu WAR*/ \
...
SHR_G_ENTRY(linkscan_flr_support), /* linkscan flr support */ \
SHR_G_ENTRY(linkscan_fault_ignore_an_speed), /* Not call bcm_port_speed_get in linkscan to improve performance */ \
...
SHR_G_ENTRY(iproc_20),                       /* iProc rev 20 */ \
...
SHR_G_ENTRY(td3x1_reduced_mmu_internal_buffer), /* Feature for TD3 x1 reduced MMU internal buffer to 2MB */ \
SHR_G_ENTRY(td3x1_reduced_flex_counter), /* Feature for TD3 x1 reduced IP/EP flex counters */ \
...
SHR_G_ENTRY(use_smbus3_for_i2c), /* TD3x1 uses SMBUS3 for I2C access (TD3 uses smbus1, HX5/HR4 uses smbus2) */ \
SHR_G_ENTRY(mpls_evpn), /* mpls evpn feature */ \
SHR_G_ENTRY(pt2pt_tunnel_term_learn_disable), /* disable mac-learning on vpws tunnel termination flow. */ \
SHR_G_ENTRY(tunnel_term_dscp_phb), /* Derive PHB from Tunnel DSCP during termination */ \
SHR_G_ENTRY(bfd_l3iif_vrf), /* BFD over VRF for L3IIF assigned VRF */ \
SHR_G_ENTRY(packet_rate_limit_hw_only), /* Packet-based rate limiting to CPU for HW only. */ \
SHR_G_ENTRY(vxlan_sip_dip_based_svp_assignment), /* Support to assign SVP based on SIP+DIP */ \
SHR_G_ENTRY(hgoe_line_card_mode), /* Device supporting HGoE is used in line_card mode */ \
SHR_G_ENTRY(td3x4_ax_reduced_nhop), /* Feature for TD3 x4_AX reduced Ingress and Egress Next Hop Tables to 32k */ \
SHR_G_ENTRY(td3x4_ax_reduced_ifp_meter_table), /* Feature for TD3 x4_AX reduced IFP meter  table to 6k */ \
SHR_G_ENTRY(td3x4_ax_reduced_ing_flex_ctr), /* Feature for TD3 x4_ax reduced ingress flex counters */ \
SHR_G_ENTRY(td3x4_ax_uft_shared_8_banks_16k), /* Total 8 shared banks of 16k each. */ \
SHR_G_ENTRY(tunnel_init_ecn), /* Support packet ECN to tunnel header ECN mapping */ \
SHR_G_ENTRY(td3x4_ax_reduced_utt), /* Feature for TD3 x4_AX reduced UTT with 12 bank support instead of 16 */ \
SHR_G_ENTRY(prop_suffix_full_dev_id), /* Enable parsing soc properties according to full device id (no digit is ignored) */ \
SHR_G_ENTRY(l3_intf_mcast_nhi_auto_del), /* Delete the NHI during mutlicast egress delete */ \
SHR_G_ENTRY(xflow_macsec_misc_counters_ecc_error_war), /* WAR to adress ISEC and ESEC MISC_COUNTERS memory ECC errors. */\
SHR_G_ENTRY(l2_tunnel_pyld_sgpp_tpid_disable), /* disable SGPP based TPID enable for Payload OTPID parsing after L2 tunnel termination) */ \
SHR_G_ENTRY(l2_vp_cml), /* Use stored vp cml flags during L2 freeze */ \
SHR_G_ENTRY(ipuc_riot_l3_iif_check), /* Feature to allow L2 switching of VXLAN terminated packets with unicast IPv4/IPv6 payload. */ \
SHR_G_ENTRY(no_ip_tunnel), /* No IP tunnel. */ \
SHR_G_ENTRY(per_port_ing_egr_inner_outer_tpid_enable), /* Feature to allow inner and outer TPID checks on ingress and egress. */ \
SHR_G_ENTRY(srv6_enable), /* Enable SRv6 feature. */ \
SHR_G_ENTRY(l2_mem_acc_err_handling), /* Enable retry mechanism for recoverable soc mem read errors. Can be used for mem writes, if required */ \
