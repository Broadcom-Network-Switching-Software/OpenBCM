...
/* Flag values used with soc_th3_lrn_shadow_lookup to return search results */
#define SOC_TH3_L2_LRN_SHADOW_TBL_ENTRY_FOUND             0x1
#define SOC_TH3_L2_LRN_SHADOW_TBL_PENDING                 0x2
#define SOC_TH3_L2_LRN_SHADOW_TBL_PENDING_ENTRY_IN_HW     0x4

/* Flag values specified by the bcm layer to indicate state of entry to be
 * inserted into the lean shadow table
 */
/* Entry added to hardware */
#define SOC_TH3_L2_LRN_SHADOW_TBL_SET_IN_HW 0x1

typedef struct soc_l2_pending_learn_info_s {
    sal_mac_addr_t mac; /* MAC address to be learned */

    vlan_id_t vlan; /* VLAN id to which the mac is associated */

    /* If dst_type is TRUE, portnum_or_trunk_id will hold the trunk number.
     * If dst_type is FALSE, portnum_or_trunk_id will hold logical port
     * number
     */
    int dst_type;
    /* Logical port number, or trunk id associated with the MAC address */
    int portnum_or_trunk_id; /* Port number, or trunk id */
    soc_module_t modid;     /* Module ID - always 0 */
} soc_l2_pending_learn_info_t;

typedef void (*soc_l2_pending_learn_cb)(
    int unit,
    soc_l2_pending_learn_info_t *info,
    void *user_data);

/* Per-port pending learn feature configuration */
typedef struct soc_th3_port_pending_lrn_cfg_s {
    /* Bitmap to maintain per-port setting of pending learn feature */
    soc_pbmp_t pending_lrn_port_bmap;
    /* Mutex to control access between port module and L2 module */
    sal_mutex_t pending_lrn_cfg_mutex;
} soc_th3_port_pending_lrn_cfg_t;
...
extern int soc_th3_mmu_config_reinit_thd_from_hw(int unit, int *shared_limit);
...
extern int soc_th3_lrn_shadow_delete(int unit, l2x_entry_t *entry,
                                     int del_static);
extern int soc_th3_lrn_shadow_entry_lookup(int unit, sal_mac_addr_t mac, vlan_id_t vlan, int use_lock, uint32 *found);
extern int soc_th3_lrn_shadow_entry_delete(int unit, sal_mac_addr_t mac, vlan_id_t vlan, int use_lock, uint32 *found);
extern int soc_th3_lrn_shadow_pending_entry_set(int unit,
               soc_l2_pending_learn_info_t *info, int use_lock, uint32 flags);
extern int soc_th3_lrn_shadow_pending_entry_get(int unit,
               soc_l2_pending_learn_info_t *info, int use_lock, uint32 *found);
extern int soc_th3_lrn_shadow_pending_entry_cb_check(int unit, l2x_entry_t *entry, uint32 *callback);
...
extern int soc_l2_pending_learn_cb_register(int unit,
                                            soc_l2_pending_learn_cb fn,
                                            void *fn_data);
extern int soc_l2_pending_learn_cb_unregister(int unit);

extern int soc_th3_port_pending_learn_cfg_init(int unit);
extern int soc_th3_port_pending_learn_cfg_cleanup(int unit);
extern int soc_th3_port_pending_learn_port_add(int unit, soc_port_t port);
extern int soc_th3_port_pending_learn_port_remove(int unit, soc_port_t port);
extern int soc_th3_port_pending_learn_port_set(int unit, soc_pbmp_t *pbmp);
extern int soc_th3_port_pending_learn_state_get(int unit, soc_port_t port, int *state);
extern int soc_th3_port_pending_learn_bitmap_get(int unit, soc_pbmp_t *pbmp);
extern int soc_th3_l2_pending_learn_resources_destroy(int unit);
#ifdef BCM_WARM_BOOT_SUPPORT
extern int soc_th3_port_pending_learn_cfg_memsize_get(int unit, int *size);
extern int soc_th3_port_pending_learn_cfg_wb_sync(int unit, uint8 *scache_ptr);
extern int soc_th3_port_pending_learn_cfg_wb_recover(int unit,
               uint8 *scache_ptr);
#endif /* BCM_WARM_BOOT_SUPPORT */
