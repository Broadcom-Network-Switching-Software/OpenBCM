 ...
 * \brief
 *
 * Will not dump error message for the specified error.
 * The expression can be a function call or a fixed error code.
 *
 * Behaves as:
 * 1. SHR_IF_ERR_EXIT, if (error != _rv_except).
 * 2. SHR_IF_ERR_EXIT_NO_MSG if (error == _rv_except)
 *
 * \par DIRECT INPUT:
 *   \param [in] _expr
 *     Expression to evaluate.
 *   \param [in] _rv_except
 *     Error code for which we don't want to print message.
 *
 * \par INDIRECT OUTPUT:
 *   * Newly set exit error value.
 *   * Exit procedure.
 ...
 *   SHR_FUNC_INIT_VARS(unit) ;
 *   ...
 *   SHR_LOG_AND_EXIT(_SHE_E_PARAM ," var1 %d var2 %s\r\n", var1,var2,EMPTY ) ;
 *   ...
 *
 * exit:
 *   SHR_FUNC_EXIT ;
 ...
 *     Expression representing error. One of shr_error_e (_shr_error_t).
 *   \param [in] _stuff
 *     printf format string to print, and possibl additional arguments to be
 *     printed, encapsulated in parenthesis. May use BSL_META_U().
 * \par INDIRECT INPUT:
 *    BSL_LOG_MODULE       -             \n
 *      Module (layer/source) to report on. Defined at head of file.  \n
 *    LOG_ERROR            -             \n
 ...
 *
 *   SHR_FUNC_INIT_VARS(unit) ;
 *   SHR_INVOKE_VERIFY_DNXC(verification_procedure(unit,index,value)) ;
 *   ...
 *
 * exit:
 *   SHR_FUNC_EXIT ;
 ...
 *
 * The macro should be used in conjunction with \ref SHR_FREE.
 * This macro is different than \ref SHR_ALLOC by supporting allocation flags.
 *
 * \par DIRECT INPUT:
 *   \param [in] _ptr
 *     Pointer variable to use for memory allocation.
 ...
 * Only relvant to files that incude this file (Apply this code-style standard)
 */
