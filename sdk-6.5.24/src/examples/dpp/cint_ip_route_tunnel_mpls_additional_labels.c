...
ip_route_to_tunnel_additional_labels_init(int unit)
{
    ...
    additional_labels_info.B3_label = 0x23;
...
int example_ip_route_to_tunnel_mpls_additional_labels(int unit, int in_sysport, int out_sysport, int add_EL_ELI, int add_B1_B2, int add_C1_C2, int add_C3, int add_D1, int add_B3) {
    ...
    if (add_B1_B2) {
        ...
        if (add_B3) {
            ...
            COMPILER_64_SET(wide_data, 0, additional_labels_info.B3_label);

            rv = bcm_port_wide_data_set(unit, additional_labels_info.B_gport, BCM_PORT_WIDE_DATA_EGRESS/*flags*/, wide_data);
            if (rv != BCM_E_NONE) {
                printf("Error, in bcm_port_wide_data_set\n");
                return rv;
            }

            rv = bcm_port_control_set(unit, additional_labels_info.B_gport, bcmPortControlMPLSEncapsulateAdditionalLabel, 1);
            if (rv != BCM_E_NONE) {
                printf("Error, in bcm_port_control_set\n");
                return rv;
            }
            ...
            int get_value;
            rv = bcm_port_control_get(unit, additional_labels_info.B_gport, bcmPortControlMPLSEncapsulateAdditionalLabel, &get_value);
            if (rv != BCM_E_NONE) {
                printf("Error, in bcm_port_control_get\n");
                return rv;
            }
