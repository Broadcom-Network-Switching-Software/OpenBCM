        ...
        int offset, int push, int nof_operations)
{
    ...
    {
        diag_mem_info_get(unit, "EVNT_EVENTS_MAIN_MEM.EVNT0", &mem_id, &block_instance_id, &block_instance_number, &array_index);
        rv = bcm_instru_get_memory_access_info(unit, 0, mem_id, block_instance_number, 0, 1, &operations[operation_index].access_info);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_instru_get_memory_access_info\n");
            return rv;
    ...
    for (operation_index=0;operation_index<nof_operations; operation_index++)
    {
        ...
        operations[operation_index].buf_offset = operation_index + offset;
    }
    printf("cint_eventor_utils_context_set: bank1 = %d, bank2 = %d\n", context_conf_set.bank1, context_conf_set.bank2);
...
{
    int rv;
    bcm_instru_synced_counters_source_type_t sources[3];
    uint32 enable[3];
