...
int cint_field_bfd_echo_main(int unit,bcm_field_context_t context_id_bfd_echo, int is_ipv6)
{
    ...
    int offset;
    int ii;
    int use_oam_offset = *(dnxc_data_get(unit,"oam","feature","oam_use_offset_supported",NULL));
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &cint_field_echo_bfd_fg_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
    ...
    if (traffic_with_ptch && !use_oam_offset)
    {
...
* \param [in] ep_id        -  endpoint id(oam_id)
* \return
*   int - Error Type
* \remark
*   * None
...
int cint_field_bfd_echo_entry_add(int unit, uint32 dest_ip, bcm_ip6_t *dest_ip6, uint16 local_discr, uint16 ep_id)
{
    ...
    ent_info.entry_action[0].value[1] = ep_id;
