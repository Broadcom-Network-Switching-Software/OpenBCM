    ...
    bcm_field_context_t context_id_ipmf1)
{
    ...
    rv = bcm_field_group_info_get(unit, action_prefix_de_fg_id, &fg_info_get);
    if(rv != BCM_E_NONE)
    {
        printf("Error (%d) in bcm_dnx_field_group_info_get\n", rv);
        return rv;
    }
    ...
    rv = bcm_field_group_context_info_get(unit, action_prefix_de_fg_id, context_id_ipmf1, &attach_info_get);
    if(rv != BCM_E_NONE)
    {
        printf("Error (%d) in bcm_field_group_context_info_get\n", rv);
        return rv;
    }
