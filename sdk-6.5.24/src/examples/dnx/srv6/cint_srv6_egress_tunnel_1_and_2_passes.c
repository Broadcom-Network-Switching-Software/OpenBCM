        ...
        int access_port)
{
    ...
    rv = intf_eth_rif_create(unit, cint_srv6_tunnel_info.eth_rif_intf_tunnel_disable_out, cint_srv6_tunnel_info.intf_out_mac_address);
    if (rv != BCM_E_NONE)
    {
     ...
     */
    rv = l3__egress_only_encap__create(unit, 0, &(cint_srv6_tunnel_info.tunnel_disable_arp_id),
     ...
     */
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &(cint_srv6_tunnel_info.tunnel_disable_fec_id));
    if (rv != BCM_E_NONE)
    {
    ...
    rv = l3__egress_only_fec__create(unit, cint_srv6_tunnel_info.tunnel_disable_fec_id,
                                        cint_srv6_tunnel_info.eth_rif_intf_tunnel_disable_out,
                                        cint_srv6_tunnel_info.tunnel_disable_arp_id, access_port, 0,0);
    if (rv != BCM_E_NONE)
    {
     ...
     */
    rv = add_route_ipv6(unit, cint_srv6_tunnel_info.tunnel_ipv6_dip,
    ...
    int recycle_port)
{
    ...
    rv = bcm_port_class_set(unit,recycle_port,bcmPortClassFieldIngressPMF3TrafficManagementPortCs,4);
    if (rv != BCM_E_NONE)
    {
     ...
     * Create FEC and configure its: Outlif (ARP), Destination port, Global Out-EthRIF
     */
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &(cint_srv6_tunnel_info.tunnel_fec_id));
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
        return rv;
    ...
    rv = l3__egress_only_fec__create(unit, cint_srv6_tunnel_info.tunnel_fec_id,
                                        cint_srv6_tunnel_info.eth_rif_intf_out,
                                        cint_srv6_tunnel_info.tunnel_arp_id, access_port, 0,0);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error,l3__egress_only_fec__create\n", proc_name);
        return rv;
