...
int dnx_flexe_rx_in_invalid_status(int unit) {
    ...
    rx_valid_flag = *(dnxc_data_get(unit, "nif", "flexe", "rx_valid_in_lf", NULL));
    ...
    uint32 speed_flag)
{
    ...
    if ((client_type == port_type_busB_L2_L3) || (client_type == port_type_1588_L2))
    {
        ...
        rv = appl_dnx_dynamic_port_create(unit, 0, NULL, client_id);
        if (rv != BCM_E_NONE) {
...
int dnx_flexe_util_client_remove_backup(
    int unit,
    bcm_port_t client_id)
{
    int i, rv;
    ...
    uint32 speed_flag)
{
    ...
    rv = cint_dyn_port_add_flexe_client(unit, client_id, intf_type, channel_id, rx_speed, speed, flags);
    if (rv != BCM_E_NONE) {
