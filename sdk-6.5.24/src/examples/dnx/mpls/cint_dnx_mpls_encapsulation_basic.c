 ...
 * 
 ...
 Copyright 2007-2021 Broadcom Inc. All rights reserved. File: cint_mpls_encapsulation_basic.c Purpose: Various examples for MPLS/VPLS.
 */
    ...
    uint8 ttl_value)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit,0,fec_direction,&first_fec_id_in_hierarchy);
    ...
    int unit)
{
    ...
    rv = l3__egress_only_fec__create(unit, cint_mpls_encapsulation_basic_info.fec_id_enc,
                                     ...
                                     cint_mpls_encapsulation_basic_info.out_port, flag, flags2);
    if (rv != BCM_E_NONE)
    {
    ...
    int tunnel_id)
{
     ...
     * 4) Give the out port.
     */
    rv = l3__egress_only_fec__create(unit, fec_id, tunnel_id, 0, cint_mpls_encapsulation_basic_info.out_port, flags, flags2);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object FEC only\n");
        return rv;
    ...
    int unit)
{
    ...
    if (*dnxc_data_get(unit, "flow", "arp", "virtual_lif_support", NULL))
    {
    ...
    int unit)
{
    ...
    {
        rv = add_route_ipv4(unit, cint_mpls_encapsulation_basic_info.dip, 0xFFFFFFFF, cint_mpls_encapsulation_basic_info.vrf,
    ...
    else
    {
        ...
        rv = add_route_ipv6(unit, cint_mpls_encapsulation_basic_info.ipv6_dip, full_mask, cint_mpls_encapsulation_basic_info.vrf,
                           cint_mpls_encapsulation_basic_info.fec_id_enc);
        if (rv != BCM_E_NONE)
        {
