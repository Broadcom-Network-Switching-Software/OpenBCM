...
struct cint_termination_mpls_basic_info_s
{
    ...
    bcm_mpls_label_t termination_stack[5];      /* stack of labels to be terminated */
    bcm_gport_t tunnel_id[5];   /* stack of tunnel_id created for the labels */
    bcm_if_t tunnel_if;         /* RIF, not relevant only for Jericho2 devices */
    bcm_mpls_label_t extend_term_label; /*  extended term label */
    bcm_if_t rcy_extended_term_encap_id; /* Encapsulation index of rcycle extended term */
    int vmpi;                    /*mpls vmpi*/
};
    ...
    int port2)
{
     ...
     */
    if (cint_termination_mpls_basic_info.extend_term_label)
    {
        nof_labels = 5;
    }

    rv = mpls_termination_basic_create_termination_stack(unit, nof_labels);
    if (rv != BCM_E_NONE)
    {
        print rv;
    ...
    bcm_gport_t * tunnel_id)
{
    ...
    if (terminated_label == cint_termination_mpls_basic_info.extend_term_label)
    {
    ...
    int nof_labels)
{
    ...
    if (nof_labels == 5)
    {
        ...
        {
            rv = BCM_E_PARAM;
            printf("Invalid configuration, Couldn't term 5 label without extended termination flow\n");
            return rv;
        }
