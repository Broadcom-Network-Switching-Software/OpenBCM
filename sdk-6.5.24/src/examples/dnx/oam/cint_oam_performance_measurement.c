 ...
 * 
 ...
 Copyright 2007-2021 Broadcom Inc. All rights reserved. File: cint_oam_performance_measurement.c Purpose: basic examples for OAM loss and delay measurement.
 */
    ...
    bcm_port_t port2)
{
    ...
    rv = dnx_oam_accelerated_endpoint_create(unit, oam_perf_down_mep_id, gport2, group_info.id,
                                             ...
                                             device_type_v2 ? BCM_OAM_OPCODE_DUAL_ENDED_LM : BCM_OAM_OPCODE_CCM_IN_HW);
    if (rv != BCM_E_NONE)
    {
    ...
    else 
    {
        if(device_type_v2)
        {
            /* Create action of incoming delay measurement packet */
            ...
            /* Create action of incoming loss measurement packet */
            ...
            rv = dnx_oam_accelerated_endpoint_create(unit, oam_perf_lmm_up_mep_id, gport2, group_info.id,
                                                     counter_if_0, lm_counter_base_id_0, acc_ingress_profile, 0, 0, BCM_OAM_OPCODE_LMM_IN_HW);
            if (rv != BCM_E_NONE)
            {
            ...
            /*
             * Create loss measurement endpoint - in OAMP v2, delay measurement is done with a separate endpoint,
             * as well.
             */
            rv = dnx_oam_accelerated_endpoint_create(unit, oam_perf_dmm_up_mep_id, gport2, group_info.id,
                                                     counter_if_0, lm_counter_base_id_0, acc_ingress_profile, 0, 0, BCM_OAM_OPCODE_DMM_IN_HW);
            if (rv != BCM_E_NONE)
            {
    ...
    uint32 opcode_flags)
{
    ...
    is_dm = ((opcode_flags == BCM_OAM_OPCODE_DMM_IN_HW) ||
             ...
             (opcode_flags == BCM_OAM_OPCODE_DMM_ONE_WAY_STATS));
