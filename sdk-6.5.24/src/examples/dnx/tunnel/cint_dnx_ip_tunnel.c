...
struct ip_tunnel_basic_info_s{
    ...
    int is_native_arp_with_rif;    /* When set, RIF will be part of the native ARP, not the FEC. Required for bfd_vxlan_extended_sip*/
    int bfd_vxlan_extended_sip; /*May be set to 1 through bfd cints. 
                                  When set to 1, the cint cint_dnx_bfd.c must also be loaded*/
};
...
ip_tunnel_basic_info_s ip_tunnel_basic_info = 
{
    ...
    /* SVTAG */
    0,
    /* is_native_arp_with_rif*/
    0,
    /* bfd extended sip*/
    0
};
int verbose_ip_tunnel = 1;
    ...
    char *proc_name)
{
    ...
    dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, ip_tunnel_basic_info.access_eth_rif, 0, 0, ip_tunnel_basic_info.access_eth_rif_mac, ip_tunnel_basic_info.access_eth_vrf);
    eth_rif_structure.ing_qos_map_id = ip_tunnel_basic_info.ing_qos_id;
    rv = dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_utils_l3_eth_rif_create access_eth_rif \n");
        return rv;
    } 
    ...
    char *proc_name)
{
    ...
    /* Set RIF -> My-Mac  and RIF -> VRF */
    dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, ip_tunnel_basic_info.provider_eth_rif, 0, 0, ip_tunnel_basic_info.provider_eth_rif_mac, ip_tunnel_basic_info.provider_eth_vrf);
    eth_rif_structure.ing_qos_map_id = ip_tunnel_basic_info.ing_qos_id;
    rv = dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure);
    if (rv != BCM_E_NONE)
    {
    ...
    if (ip_tunnel_basic_info.roo) {
        /* Set RIF -> My-Mac  and RIF -> VRF */
        dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, ip_tunnel_basic_info.provider_native_eth_rif, 0, 0, ip_tunnel_basic_info.provider_native_eth_rif_mac, ip_tunnel_basic_info.provider_native_eth_vrf);
        eth_rif_structure.ing_qos_map_id = ip_tunnel_basic_info.ing_qos_id;
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error, dnx_utils_l3_eth_rif_create provider_native_eth_rif \n");
            return rv;
        } 
    }
    ...
    int tunnel_idx)
{
     ...
     */
    dnx_utils_l3_arp_s_common_init(unit, 0, &arp_structure, ip_tunnel_basic_info.term_arp_id[tunnel_idx], 0, 0,
    ...
    int tunnel_idx)
{
    ...
    if (ip_tunnel_basic_info.roo) {
        /* RIF may either come from the FEC or from the ARP. This is controlled through is_native_arp_with_rif*/
        int eth_rif = ip_tunnel_basic_info.is_native_arp_with_rif? ip_tunnel_basic_info.provider_native_eth_rif : 0 ;
        dnx_utils_l3_arp_s_common_init(unit, 0, &arp_structure, ip_tunnel_basic_info.encap_native_arp_id[tunnel_idx], l3_flags, 0,
                                        ip_tunnel_basic_info.encap_native_next_hop_mac[tunnel_idx], eth_rif);
        rv = dnx_utils_l3_arp_create(unit, &arp_structure);
        if (rv != BCM_E_NONE)
        {
            printf("Error, dnx_utils_l3_arp_create roo\n");
            return rv;
        }
    ...
    int tunnel_idx)
{
    ...
    dnx_utils_l3_arp_s_common_init(unit, 0, &arp_structure, ip_tunnel_basic_info.encap_arp_id[tunnel_idx], 0, l3_flags2,
    ...
    int tunnel_idx)
{
     ...
     */
    dnx_utils_l3_fec_s_common_init(unit, BCM_L3_WITH_ID, 0, &fec_structure, ip_tunnel_basic_info.term_fec_id[tunnel_idx], 0, 0, ip_tunnel_basic_info.access_port,
    ...
    int tunnel_idx)
{
    ...
    if (ip_tunnel_basic_info.roo) {
        ...
        bcm_if_t next_if =  ip_tunnel_basic_info.encap_native_arp_id[tunnel_idx];
        bcm_if_t tunnel_gport = ip_tunnel_basic_info.is_native_arp_with_rif? 0 : ip_tunnel_basic_info.provider_native_eth_rif;
        if (ip_tunnel_basic_info.bfd_vxlan_extended_sip)
        {
            /* When BFD extended SIP is used, FEC points to extrened-sip outlif which points to native ARP and includes the RIF.
               ...
               bcmEncapAccessRif*/
            rv = bfd_extended_sip_configuration(unit, 0x10203040, 0, next_if, bcmEncapAccessRif, &next_if);
         ...
         * Create native FEC for routing into overlay:
         *  1) Give a fec id.
         *  2) Native out rif, unless it is provided through the ARP.
         *  3) Native arp id.
         *  4) Point to overlay fec or overlay ECMP.
         */
        dnx_utils_l3_fec_s_common_init(unit, BCM_L3_WITH_ID, 0, &fec_structure, ip_tunnel_basic_info.encap_native_fec_id[tunnel_idx], l3_flags, 0, gport,
                                   tunnel_gport ,next_if);
        rv = dnx_utils_l3_fec_create(unit, &fec_structure);
        if (rv != BCM_E_NONE)
        {
            printf("Error, dnx_utils_l3_fec_create\n");
    ...
    int tunnel_idx)
{
    ...
    dnx_utils_l3_fec_s_common_init(unit, BCM_L3_WITH_ID, 0, &fec_structure, ip_tunnel_basic_info.encap_fec_id[tunnel_idx], l3_flags, 0, gport,
    ...
    int tunnel_idx)
{
     ...
     */  
     ...
     dnx_utils_l3_fec_s_common_init(unit,
...
int
mpls_encapsulation_basic_create_tunnel(
    int unit,
    int tunnel_idx)
{
    ...
    label_array[0].l3_intf_id = ip_tunnel_basic_info.encap_arp_id[tunnel_idx];;

    /** qos remark profile */
    rv = bcm_qos_map_id_get_by_profile(unit, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_REMARK, 1, &egress_qos_profile);
    if (rv != BCM_E_NONE)
    {
    ...
    ip_tunnel_basic_info.encap_tunnel_intf[tunnel_idx] = label_array[0].l3_intf_id;
    ip_tunnel_basic_info.encap_tunnel_id[tunnel_idx] = label_array[0].tunnel_id;
    if(verbose_ip_tunnel >= 1) {
        printf("created MPLS tunnel encap on intf:0x%08x id:0x%08x \n",label_array[0].l3_intf_id, label_array[0].tunnel_id);
    }
    ...
    int tunnel_idx)
{
    ...
    if ((tunnel_term.type != bcmTunnelTypeVxlan) && (tunnel_term.type != bcmTunnelTypeVxlan6) && (tunnel_term.type != bcmTunnelTypeVxlanGpe) && (tunnel_term.type != bcmTunnelTypeVxlan6Gpe))
    {
    ...
    else if (ip_tunnel_basic_info.ing_qos_id != 0)
    {
        rv = bcm_qos_port_map_set(unit, tunnel_term.tunnel_id,  ip_tunnel_basic_info.ing_qos_id, -1);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_l3_ingress_create \n");
            return rv;
