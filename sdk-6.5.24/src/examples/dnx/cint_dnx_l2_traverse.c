 ...
 *
 * Run Script:
 *      BCM> examples/dnx/l3/cint_dnx_ip_route_basic.c
 *      BCM> examples/dnx/utility/cint_dnx_utils_multicast.c
 *      BCM> examples/dnx/cint_dnx_l2_traverse.c
 *      BCM> cint
...
int nof_iterations_before_error = 20;
/*
 * l2 mac table init
...
void l2_mac_table_info_init(int unit, int port1, int port2) {
    ...
    l2_mact_table_info1.aging_time = 200; /* 200 seconds */
...
/*
 * Depending on: l2_mact_table_info2
...
l2_mac_table_init2(int unit){
    ...
    if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
    {
        ...
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_l3_host_add, rv= %d \n", rv );
            return rv;
        }
    ...
    {
        rv = add_route_ipv4(unit, l3_host.l3a_ip_addr, 0,l3_host.l3a_vrf, l3_host.l3a_intf);
        if (rv != BCM_E_NONE) {
            printf("Error, in add_route_ipv4, rv= %d \n", rv );
            return rv;
        }
    }
    ...
    int unit)
{
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID, &l2_mact_table_info1.mc_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = bcm_multicast_add(unit, l2_mact_table_info1.mc_id, BCM_MULTICAST_INGRESS_GROUP, 2, replications);
    if (rv != BCM_E_NONE)
    {
...
/* call back that return an error after processing some entries */
int call_back_traverse_and_return_error(int unit, bcm_l2_addr_t *l2e, int* ud) {
    l2_addr_print_entry(l2e);
    ++*ud;
    if(*ud == nof_iterations_before_error)
    {
        return BCM_E_FAIL;
    }
    return BCM_E_NONE;
...
int l2_traverse(int unit, int api_idx, int flags, bcm_l2_addr_t *l2_addr, bcm_l2_addr_t *l2_addr_mask)
{
    ...
    switch(api_idx) {
        ...
        case l2_traverse_and_abort:
            ...
            break;
        case l2_match_traverse_and_abort:
            rv = bcm_l2_matched_traverse(unit, flags, l2_addr, call_back_traverse_and_return_error, &count);
            if (rv != BCM_E_NONE)
            {
                printf("Error, bcm_l2_matched_traverse returned %s failed\n", bcm_errmsg(rv));
                return rv;
