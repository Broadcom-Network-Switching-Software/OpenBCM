...
int kaps_hitbits_ipv_4_6_uc_mc_fill_entries(int unit, int in_port, int out_port) {
    ...
    rc = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &egress_intf);
    if (rc != BCM_E_NONE)
    {
        printf("Error, in function get_first_fec_in_range_which_not_in_ecmp_range(), \n");
        return rc;
    }
    ...
    rc = l3__egress_only_fec__create_inner(unit, egress_intf, l3if.l3a_intf_id, encap_id, out_port,0, 0, 0, 0,&l3egid);
    if (rc != BCM_E_NONE)
    {
