    ...
    int out_sys_port)
{
    ...
    if (l3_fec_cascade == BCM_L3_2ND_HIERARCHY ) {
        rv = get_first_fec_in_range_which_not_in_ecmp_range(ingress_unit, 1, 0, &fec_id);
    } else if (l3_fec_cascade == BCM_L3_3RD_HIERARCHY) {
        rv = get_first_fec_in_range_which_not_in_ecmp_range(ingress_unit, 2, 0, &fec_id);
    } else {
        rv = get_first_fec_in_range_which_not_in_ecmp_range(ingress_unit, 0, 0, &fec_id);
    }
...
{
    int rv;
    rv = add_host_ipv4(unit, cint_multi_dev_ip_route_info.dip, cint_multi_dev_ip_route_info.vrf, cint_multi_dev_ip_route_info.level1_fec_id, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    int out_sys_port)
{
    ...
    rv = multi_dev_ingress_port_configuration(ingress_unit,in_port, cint_multi_dev_ip_route_info.in_rif);
    if (rv != BCM_E_NONE)
    {
     printf("Error, multi_dev_ingress_port_configuration\n");
     return rv;
    ...
    rv = multi_dev_ip_route_fec_entry_configuration(ingress_unit,out_sys_port);
    if (rv != BCM_E_NONE)
    {
     printf("Error, multi_dev_ip_route_fec_entry_configuration\n");
     return rv;
    }

    rv = multi_dev_ip_route_forward_entry_configuration(ingress_unit);
    if (rv != BCM_E_NONE)
    {
     printf("Error, multi_dev_ip_route_forward_entry_configuration\n");
     return rv;
    }
    ...
    int out_port)
{
    ...
    rv = multi_dev_ip_route_l3_intf_configuration(egress_unit);
    if (rv != BCM_E_NONE)
    {
     printf("Error, multi_dev_ip_route_l3_intf_configuration\n");
     return rv;
    ...
    rv = multi_dev_ip_route_arp_configuration(egress_unit);
    if (rv != BCM_E_NONE)
    {
     printf("Error, multi_dev_ip_route_arp_configuration\n");
     return rv;
    }
    ...
    int out_sys_port)
{
    ...
    if (rv != BCM_E_NONE)
    {
     printf("Error, multi_dev_ip_route_init\n");
     return rv;
    }

    rv = multi_dev_ip_route_ingress_configuration(ingress_unit,cint_multi_dev_ip_route_info.in_local_port,out_sys_port);
    if (rv != BCM_E_NONE)
    {
     printf("Error, multi_dev_ip_route_ingress_configuration\n");
     return rv;
    }

    rv = multi_dev_ip_route_egress_configuration(egress_unit,cint_multi_dev_ip_route_info.out_local_port);
    if (rv != BCM_E_NONE)
    {
     printf("Error, multi_dev_ip_route_egress_configuration\n");
     return rv;
    }
    ...
    int out_sysport)
{
    ...
    rv = multi_dev_ingress_port_configuration(ingress_unit, in_sysport, in_vlan);
    if (rv != BCM_E_NONE)
    {
        printf("Error, multi_dev_ingress_port_configuration for ingress_unit\n");
        return rv;
    }

    rv = multi_dev_egress_port_configuration(egress_unit, out_sysport);
    if (rv != BCM_E_NONE)
    {
        printf("Error, multi_dev_egress_port_configuration for egress_unit\n");
        return rv;
    }
    ...
    /* Set VLAN with MC */
    rv = bcm_vlan_control_vlan_get(ingress_unit, in_vlan, &control_vlan);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_vlan_control_vlan_set(ingress_unit, in_vlan, control_vlan);
    if (rv != BCM_E_NONE) {
    ...
    /* Set VLAN with MC */
    rv = bcm_vlan_control_vlan_get(egress_unit, out_vlan, &control_vlan);
    if (rv != BCM_E_NONE) {
        printf("fail get control vlan(%d)\n", out_vlan);
        return rv;
    }
    control_vlan.unknown_unicast_group = control_vlan.unknown_multicast_group = control_vlan.broadcast_group = 0x1;
    rv = bcm_vlan_control_vlan_set(egress_unit, out_vlan, control_vlan);
    if (rv != BCM_E_NONE) {
        printf("fail set control vlan(%d)\n", out_vlan);
        return rv;
    }
    ...
    eth_rif_structure.mtu_valid  = 1;
    eth_rif_structure.mtu  = default_mtu;
    rv = dnx_utils_l3_eth_rif_create(ingress_unit, &eth_rif_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_utils_l3_eth_rif_create core_native_eth_rif\n");
        return rv;
    ...
    eth_rif_structure.mtu_valid  = 1;
    eth_rif_structure.mtu  = default_mtu;
    rv = dnx_utils_l3_eth_rif_create(egress_unit, &eth_rif_structure);
    if (rv != BCM_E_NONE)
    {
        printf("Error, dnx_utils_l3_eth_rif_create core_native_eth_rif\n");
        return rv;
    ...
    rv = dnx_utils_l3_arp_create(egress_unit, &l3_arp);
    if (rv != BCM_E_NONE)
    {
    ...
    l3_fec.fec_id = fec[0];
    rv = dnx_utils_l3_fec_create(ingress_unit, &l3_fec);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object FEC only\n");
        return rv;
    ...
    if (!lpm_raw_data_enabled) {
        ...
        dnx_utils_l3_route_ipv4_s_common_init(ingress_unit, 0, &route_ipv4_structure, route, mask, vrf, flags, 0, fec[0]);
        rv = dnx_utils_l3_route_ipv4_add(ingress_unit, &route_ipv4_structure);
        if (rv != BCM_E_NONE)
        {
    ...
    if (lpm_raw_data_enabled) {
        ...
        rv = dnx_kaps_raw_data_redirect(ingress_unit, lpm_payload, 0, 0, dst_gport);
        if (rv != BCM_E_NONE) {
            printf("Error, in function dnx_kaps_raw_data_redirect, \n");
            return rv;
        }
    }
    ...
    int header_code)
{
    ...
    if(res != BCM_E_NONE) {
        printf("Error in mtu_check_header_code_enable: $res\n", res);
        return res;
    }
    return res;
}
    ...
    int link_mtu)
{
    ...
    res = multi_dev_ipv4_uc_example(ing_unit, egr_unit, in_sysport, out_sysport);
    if (res != BCM_E_NONE) {
        return res;
    }
    ...
    int unit)
{
    ...
    if (res != BCM_E_NONE) {
        printf("Error in mtu_check_setup_destroy\n");
        return res;
    }
    return res;
}
