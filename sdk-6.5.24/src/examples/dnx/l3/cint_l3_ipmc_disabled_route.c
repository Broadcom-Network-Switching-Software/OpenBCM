    ...
    int out_port2)
{
    ...
    rv = add_ipv4_ipmc_bridge(unit, cint_ipmc_info.mc_dip, mask, 0, 0, intf_in, 0, l2_mc_group, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_ipv4_ipmc_bridge, \n");
        return rv;
    }
    ...
    int out_port2)
{
    ...
    rv = add_ipv4_ipmc_bridge(unit, dip_addr, dip_mask, sip_addr, sip_mask, 0, vids[0], ipmc_group[0], 0, 0);
    if (rv != BCM_E_NONE)
    {
