...
int create_protection_fec(int unit, ac_1to1_coupled_protection_s *ac_1to1_coupled_protection_info)
{
    ...
    if(device_is_jericho2)
    {
        ...
        if (!is_jericho2_mode) {
            rv = fec_id_alignment_field_add(unit, ac_1to1_coupled_protection_info->out_ac[FAILOVER_WORKING].port, BCM_L3_ITF_VAL_GET(fec_primary));
            if (rv != BCM_E_NONE){
...
int ac_1to1_coupled_protection__cleanup(int *unit_ids, int nof_units) {
    ...
    for (i = 0 ; i < nof_units ; i++) {
        ...
        if (device_is_jericho2)
        {
            ...
            if (!is_jericho2_mode) {
                rv = fec_id_alignment_field_destroy(unit);
                if (rv != BCM_E_NONE){
                     printf("Error, fec_id_alignment_field_destroy\n");
                     return rv;
                }
            }
        }
