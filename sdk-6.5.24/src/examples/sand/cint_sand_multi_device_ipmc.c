...
/**
 * IPv4 MC with VRF redirect
 * INPUT:
 *   in_sys_port    - traffic incoming port
 *   out_sys_port - traffic outgoing port
 *   rch_port - recycle port for VRF redirect
*/
 int
 multi_dev_ipmc_vrf_redirect(
    int ingress_unit,
    int egress_unit,
    int rch_port,
    int in_sys_port,
    int out_sys_port)
{
    ...
    /* Configure in port */
    rv = multi_dev_ingress_port_configuration(ingress_unit,cint_ipmc_route_info.in_local_port,cint_ipmc_route_info.in_rif);
    if (rv != BCM_E_NONE)
    {
        printf("Error, multi_dev_ingress_port_configuration\n");
        return rv;
    ...
    /* Configure in RIF */
    rv = multi_dev_ipmc_l3_intf_configuration(ingress_unit,cint_ipmc_route_info.in_local_port);
    if (rv != BCM_E_NONE)
    {
        printf("Error, multi_dev_ipv4_uc_l3_intf_configuration\n");
        return rv;
    ...
    /* Set in RIF with a different VRF as 2nd pass */
    l3_ingress_intf ingr_itf;
    ingr_itf.intf_id = cint_ipmc_route_info.in_rif;
    ingr_itf.vrf = cint_ipmc_info.vrf;
    rv = intf_ingress_rif_set(ingress_unit, &ingr_itf);
    if (rv != BCM_E_NONE)
    {
    ...
    /* Recycle entry for VRF redirect */
    ...
    recycle_entry_params.vrf_redirect_params.vrf = cint_ipmc_route_info.vrf;
    rv = create_vrf_redirect_recycle_entry(ingress_unit, recycle_entry_params, recycle_entry_encap_id);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create_vrf_redirect_recycle_entry\n", proc_name);
    }

    /* Create IP tunnel and set next pointer to recycle entry */
    BCM_L3_ITF_SET(ip_tunnel_basic_info.encap_arp_id[0],BCM_L3_ITF_TYPE_LIF,recycle_entry_encap_id);
    ip_tunnel_basic_info.dip[0] = 0xE0020202;
    ip_tunnel_basic_info.sip[0] = 0x0A0A0A02;
    rv = ip_tunnel_encap_create(ingress_unit,0);
    if (rv != BCM_E_NONE)
    {
    ...
    /* 1st pass replication to IP tunnel */
    cint_ipmc_info.mc_group = 5000;

    bcm_multicast_replication_t rep[1];
    /* 1st copy: to In-RIF*/
    bcm_multicast_replication_t_init(&rep[0]);
    rep[0].encap1 = ip_tunnel_basic_info.encap_tunnel_id[0] & 0x3fffff;
    rep[0].port = rch_port;

    rv = multi_dev_ipmc_create_multicast(ingress_unit, egress_unit, rep, 1, cint_ipmc_info.mc_group, 1);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function create_multicast, \n");
        return rv;
    ...
    /* 1st pass IPMC */
    cint_ipmc_info.mc_host_dip = 0xE00A0A0A;
    cint_ipmc_info.host_sip = 0xC0800202;

    rv = add_ipv4_ipmc(ingress_unit, cint_ipmc_info.mc_host_dip, 0xFFFFFFFF, cint_ipmc_info.host_sip, 0xFFFFFFFF,
                       0, cint_ipmc_info.vrf, cint_ipmc_info.mc_group, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    /* 2nd pass IPMC config, same MC with new VRF */
    ...
    /* Add IPv4 IPMC entry */
    rv = add_ipv4_ipmc(ingress_unit, cint_ipmc_route_info.mc_group_ip, 0xFFFFFFFF, cint_ipmc_route_info.host_ip, 0xFFFFFFFF,
                       0, cint_ipmc_route_info.vrf, cint_ipmc_route_info.mc_group, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in function add_ipv4_ipmc to LEM table, \n",proc_name);
        return rv;
