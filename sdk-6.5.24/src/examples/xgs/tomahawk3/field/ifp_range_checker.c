...
bcm_vlan_t test_vid1_min = 101;	// = 100; // Here VID itself is a variable in RANGE
bcm_vlan_t test_vid1_max = 105;	// = 100; // Here VID itself is a variable in RANGE

bcm_vlan_t test_vid2_min = 111;	// = 100; // Here VID itself is a variable in RANGE
bcm_vlan_t test_vid2_max = 115;	// = 100; // Here VID itself is a variable in RANGE

bcm_vlan_t test_vid_range_max = 120;
...
configure_fp(int unit, fp_cfg_t *ifp_cfg)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualify_SrcMac(unit, entry, ifp_cfg->src_mac, ifp_cfg->mac_mask));
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualify_RangeCheckGroup(unit, entry, rangeGroupId, rangeGroupId));
...
bcm_error_t test_setup(int unit)
{
	...
	for(vid=test_vid1_min; vid<test_vid_range_max; ++vid)
	{
...
bcm_error_t verify()
{
	...
	printf("Finally the STAT count should be 5 + 5 = %d, 0x%08x\n", 0x0000000a,
																	0x0000000a);
