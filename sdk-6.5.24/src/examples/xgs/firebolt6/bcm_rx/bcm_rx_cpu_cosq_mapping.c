...
/*
 * Helix5 Rx Queue Channel Configuration
 *
 * The mechanism for configuring Rx Queue channels on Helix5 
 * (BCM56370) differs from previous devices. Previous devices used
 * bcm_tx_queue_channel_set(). This API does not apply to Helix5. For
 * Helix5 devices, the user needs to use a combination of cosq attach/detach
 * operations on L0 gports. On Helix5, the L0 nodes are directly wired
 ...
 * 
 * bcm_error_t
 * device_rx_queue_channel_set(int unit,
 ...
 */
bcm_error_t
device_cpu_queues_get(int unit, cpu_queues_t * cpu_queues)
{
    int cos_idx;
    int input_idx;
    int rv = BCM_E_NONE;
...
int rx_cpu_cosq_setup(int unit)
{
     ...
     */
    rv = device_cpu_queues_get(unit, &cpu_cosq_map_db);
    if(BCM_FAILURE(rv)) {
        printf("\nError in device_cpu_queues_get(): %s.\n",bcm_errmsg(rv));
        return rv;
    }
...
test_harness(int unit)
{
    ...
    */ 
    for (cos = 0; cos < max_rx_cos; cos++) {
        ...
        rv = device_rx_queue_channel_set(unit, &cpu_cosq_map_db, cos, rx_queue_channel);
        if(BCM_FAILURE(rv)) {
