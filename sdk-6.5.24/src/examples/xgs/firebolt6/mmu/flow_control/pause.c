 ...
 *         at linerate to xe1 (Port-2)
 *         (Pause frame: dmac=0x0180C2000001, smac=0x1, type=0x8808, opcode=0x0001)
 *      c) Expected Result:
 *         ================
 *         For test case mentioned in a)
 *         On xe0 (Port-1): verify that Pause frames should be transmitted at regular intervals as
 *         egress port is congested.
 *         In 'show c' output Pause frame Tx should be seen on xe0 (port-1)
 *
 *         On xe1 (Port-2): verify that packets egress at configured rate
 *
 *         For test case mentioned in b)
 *         On xe1(Port-2): verify that Port does not egress any packets as pause frames are being
