...
soc_tr3_l2_port_age(int unit, soc_reg_t reg0, soc_reg_t reg1)
{
    ...
    /* If L2 mode is FIFO, the bulk op records to FIFO and config
       l2x_bulk_op_learn_pause is enabled, pause L2 learning */
    if ((mode == L2MODE_FIFO) &&
        (reg0 == L2_BULK_CONTROLr) &&
        (soc_reg_field_valid(unit, reg0, L2_MOD_FIFO_RECORDf))) {
        if (soc_property_get(unit, spn_L2X_BULK_OP_LEARN_PAUSE, 0)) {
            rv = soc_reg32_get(unit, reg0, REG_PORT_ANY, 0, &rval);
            if (SOC_SUCCESS(rv)) {
                if (soc_reg_field_get(unit, reg0, rval, L2_MOD_FIFO_RECORDf)) {
                    l2_learn_pause = 1;
                }
            }
        }
    ...
    if (mode != L2MODE_FIFO || l2_learn_pause == 1) {
