...
soc_th2_ledup_set(int unit)
{
    ...
    /*
     * LED0 scan chain order: 64, 63, ...., 1
     * LED1 scan chain order: 65, 66, ...., 128
     * LED2 scan chain order: 192, 191, ...., 129
     * LED3 scan chain order: 193, 194, ...., 256
     * LED4 scan chain order: 260, 259, 258, 257
     */

    /* Build a LED chain hardware descriptor for TH2. */
    ...
    for (phy_port = 1; phy_port <= 260; phy_port++) {
        if (phy_port <= 64) {
            uc = 0;
            pos = 63 - (phy_port - 1);
        } else if (phy_port <= 128) {
            uc = 1;
            pos = (3 - (((phy_port - 65) & 3))) + ((phy_port - 65) & ~(3));
        } else if (phy_port <= 192) {
            uc = 2;
            pos = 63 - (phy_port - 129);
        } else if (phy_port <= 256) {
            uc = 3;
            pos = (3 - (((phy_port - 193) & 3))) + ((phy_port - 193) & ~(3));
        } else {
            uc = 4;
            pos = 3 - (phy_port - 257);
        }
    ...
    /* Free local CMICd LED chain data. */
    sal_free(chain);
...
int
bcm_th2_mmu_pfc_tx_config_set(int unit, soc_port_t port, int pfc_enable)
{
    soc_reg_t reg = INVALIDr;
    soc_info_t *si;
    uint64 rval64;
