...
/**
@name: tdm_fb6_chk_get_speed_slots_5G
@param:

Return the number of slots (5G per slot) of the given speed
 */
int
tdm_fb6_chk_get_speed_slots_5G(enum port_speed_e port_speed)
{
    int slot_num;

    slot_num = tdm_fb6_cmn_get_speed_slots(port_speed);
    if (slot_num > 1) {
        slot_num /= 2;
    }
...
/**
@name: tdm_fb6_chk_get_pipe_token_cnt
@param:

Return number of slots of the given token in selected calendar
 */
int
tdm_fb6_chk_get_pipe_token_cnt(tdm_mod_t *_tdm, int cal_id, int port_token)
{
    int i, cnt=0;
    int param_cal_len;
    int *param_cal_main=NULL;

    param_cal_len = tdm_fb6_cmn_get_pipe_cal_len(cal_id, _tdm);
...
/**
@name: tdm_fb6_chk_get_port_lanes
@param:

Return number of lanes of the given port
 */
int
tdm_fb6_chk_get_port_lanes(tdm_mod_t *_tdm, int port)
{
    ...
    if (port >= param_phy_lo && port <= param_phy_hi) {
        ...
        if (param_speeds[port] == SPEED_40G || param_speeds[port] == SPEED_42G) {
            port_start = ((port - 1) / FB6_NUM_PM_LNS) * FB6_NUM_PM_LNS + 1;
            if (port_start == port &&
...
/**
@name: tdm_fb6_chk_struct_os_ratio
@param:

Check oversub ratio for each pipe and/or half-pipe
 */
int
tdm_fb6_chk_struct_os_ratio(tdm_mod_t *_tdm, int cal_id, int os_ratio_limit)
{
        ...
        for (port_phy=phy_lo; port_phy<=phy_hi; port_phy++) {
            ...
            if (port_state==PORT_STATE__OVERSUB    ||
                ...
                if (port_phy>port_phy_mid) {
                    hp0_os_bw_req += tdm_fb6_cmn_get_speed_slots(port_speed);
                } else {
                    hp1_os_bw_req += tdm_fb6_cmn_get_speed_slots(port_speed);
                }
            }
        }
        /* os_bw_max */
...
/**
@name: tdm_fb6_chk_struct_lr_bw
@param:

Check overall bandwidth for linerate ports for the given pipe (for DV only)
 */
int
tdm_fb6_chk_struct_lr_bw(tdm_mod_t *_tdm, int cal_id)
{
    ...
    if (phy_lo>=param_phy_lo && phy_hi<=param_phy_hi) {
        for (port_phy=phy_lo; port_phy<=phy_hi; port_phy++) {
            if(port_phy<1 || port_phy>4)  {
              /*TDM_NORMAL("Port Phy %d", port_phy);*/
            port_state = param_states[port_phy-1];
            port_speed = param_speeds[port_phy];
            if (port_state==PORT_STATE__LINERATE    ||
                port_state==PORT_STATE__LINERATE_HG ){
                lr_slot_req += tdm_fb6_cmn_get_speed_slots(port_speed);
            }
            }
        }
        if (lr_slot_req>0){
            lr_slot_limit = tdm_fb6_cmn_get_pipe_cal_len(cal_id, _tdm) - FB6_NUM_ANCL ;
 ...
 */
int
tdm_fb6_chk_struct(tdm_mod_t *_tdm)
{
    ...
    for (cal_id=0; cal_id<8; cal_id++){
        if (cal_id == FB6_IDB_PIPE_2_CAL_ID ||
            cal_id == FB6_IDB_PIPE_3_CAL_ID ||
            cal_id == FB6_IDB_PIPE_1_CAL_ID ||
            cal_id == FB6_MMU_PIPE_1_CAL_ID ||
            cal_id == FB6_MMU_PIPE_2_CAL_ID ||
            cal_id == FB6_MMU_PIPE_3_CAL_ID) {
            continue;
        }
        cal_len_act = tdm_fb6_cmn_get_pipe_cal_len_max(cal_id, _tdm);
        cal_len_limit = tdm_fb6_cmn_get_pipe_cal_len(cal_id, _tdm);
        TDM_SEL_CAL(cal_id, param_cal_main);
    ...
    if (PASS!=tdm_fb6_chk_struct_os_ratio(_tdm, FB6_IDB_PIPE_0_CAL_ID, 2) ||   /* IDB pipe 0 */
        PASS!=tdm_fb6_chk_struct_os_ratio(_tdm, FB6_MMU_PIPE_0_CAL_ID, 2))  {  /* MMU pipe 0 */
        result = FAIL;
    }
    /* linerate bandwidth */
    if (PASS!=tdm_fb6_chk_struct_lr_bw(_tdm, FB6_IDB_PIPE_0_CAL_ID)) {   /* IDB pipe 0 */
        result = FAIL;
    }
     /* linerate bandwidth */
    if (PASS!=tdm_fb6_chk_struct_lr_bw(_tdm, FB6_MMU_PIPE_0_CAL_ID)) {   /* MMU  pipe 0 */
        result = FAIL;
    }
    ...
    for (port_phy=param_phy_lo; port_phy<=param_phy_hi; port_phy++) {
        ...
        if ((port_speed > SPEED_0) &&
            (port_state == PORT_STATE__LINERATE    ||
             ...
             port_state == PORT_STATE__OVERSUB_HG  ||
             port_state == PORT_STATE__MANAGEMENT ) ) {
            port_tsc = tdm_fb6_cmn_get_port_pm(port_phy, _tdm);
            if (port_tsc<param_pm_num) {
                port_lanes_exp = tdm_fb6_cmn_get_speed_lanes(port_speed);
                port_lanes = 0;
...
/**
@name: tdm_fb6_chk_sister
@param:

Check sister port spacing constraint
 */
int
tdm_fb6_chk_sister(tdm_mod_t *_tdm)
{
    ...
    for (cal_id=0; cal_id<8; cal_id++){
        ...
        for (i=0; i<param_cal_len; i++){
            ...
            if (port_i>=param_phy_lo && port_i<=param_phy_hi) {
                tsc_i = tdm_fb6_cmn_get_port_pm_chk(port_i, _tdm);
		/* just setting parameters for falcon to be 2 and eagle as 4 */
		if(tsc_i < 16)
		param_sister_min = FB6_MIN_SPACING_SISTER_EAGLE_PORT;
		else
		param_sister_min = FB6_MIN_SPACING_SISTER_FALCON_PORT;

                for (j=1; j<param_sister_min; j++){
                    k = (i+j)%param_cal_len;
                    port_k = param_cal_main[k];
                    if (port_k>=param_phy_lo && port_k<=param_phy_hi) {
                        tsc_k = tdm_fb6_cmn_get_port_pm_chk(port_k, _tdm);
                        if (tsc_i == tsc_k &&
...
/**
@name: tdm_fb6_chk_same (Version 2.0)
@param:

Check same port spacing constraint
 */
int
tdm_fb6_chk_same(tdm_mod_t *_tdm)
{
    ...
    for (cal_id=0; cal_id<8; cal_id++) {
        ...
        for (i=0; i<param_cal_len; i++) {
            ...
            if (port_i>=param_phy_lo && port_i<=param_phy_hi) {
                ...
                if((cal_id == FB6_MMU_PIPE_0_CAL_ID
                    ) &&
                   (port_speed == SPEED_10G ||
                    port_speed == SPEED_11G ||
                    port_speed == SPEED_25G ||
                    port_speed == SPEED_27G)) {
                    param_same_min = FB6_MIN_SPACING_SAME_PORT_LSP;
                } else {
                    param_same_min = FB6_MIN_SPACING_SAME_PORT_HSP;
                }
...
/**
@name: tdm_fb6_chk_bandwidth_ancl
@param:
 ...
 */
int
tdm_fb6_chk_bandwidth_ancl(tdm_mod_t *_tdm)
{
    ...
    param_token_cmic= FB6_CMIC_TOKEN;
    param_token_idl1= FB6_IDL1_TOKEN;
    param_token_idl2= FB6_IDL2_TOKEN;
    param_token_null= FB6_NULL_TOKEN;
    param_token_lpb0= FB6_LPB0_TOKEN;
    param_token_ext = _tdm->_chip_data.soc_pkg.num_ext_ports;

    for (cal_id=0; cal_id<8; cal_id++) {
        if (cal_id == FB6_IDB_PIPE_0_CAL_ID) {
            /* IDB 0 */
            slot_cpu  = FB6_NUM_ANCL_CPU;      token_cpu  = param_token_cmic;
            slot_lpbk = FB6_NUM_ANCL_LPBK;     token_lpbk = param_token_lpb0;
            slot_opt1 = FB6_NUM_ANCL_IDB_OPT1; token_opt1 = param_token_idl1;
            slot_null = FB6_NUM_ANCL_IDB_NULL; token_null = param_token_null;
            slot_idle = FB6_NUM_ANCL_IDB_IDLE; token_idle = param_token_idl2;
        }  else if (cal_id == FB6_MMU_PIPE_0_CAL_ID) {
            /* MMU 0 */
            slot_cpu  = FB6_NUM_ANCL_CPU;      token_cpu  = param_token_cmic;
            slot_lpbk = FB6_NUM_ANCL_LPBK;     token_lpbk = param_token_lpb0;
            slot_opt1 = FB6_NUM_ANCL_MMU_OPT1; token_opt1 = param_token_ext;
            slot_null = FB6_NUM_ANCL_MMU_NULL; token_null = param_token_null;
            slot_idle = FB6_NUM_ANCL_MMU_IDLE; token_idle = param_token_idl2;
        }
	 else {
        ...
        if (slot_cpu>0) {
            ...
            slot_req   = slot_cpu;
            slot_cnt   = tdm_fb6_chk_get_pipe_token_cnt(_tdm, cal_id, slot_token);
...
/**
@name: tdm_fb6_chk_bandwidth_lr_pipe
@param:

Check bandwidth for linerate ports for the given pipe
 */
int
tdm_fb6_chk_bandwidth_lr_pipe(tdm_mod_t *_tdm, int cal_id)
{
    ...
    if (phy_lo>=param_phy_lo && phy_hi<=param_phy_hi) {
        for (port_phy=phy_lo; port_phy<=phy_hi; port_phy++) {
            ...
            if (port_state==PORT_STATE__LINERATE    ||
                port_state==PORT_STATE__LINERATE_HG ){
                slot_req = tdm_fb6_cmn_get_speed_slots(port_speed);
                slot_cnt = tdm_fb6_chk_get_pipe_token_cnt(_tdm, cal_id, port_phy);
                if (slot_cnt<slot_req){
                    result = FAIL;
...
/**
@name: tdm_fb6_chk_bandwidth_os_pipe
@param:

Check bandwidth for Oversub ports for the given pipe
 */
int
tdm_fb6_chk_bandwidth_os_pipe(tdm_mod_t *_tdm, int cal_id)
{
            ...
            for (j=1; j<param_cal_grp_len; j++){
                ...
                if (port_phy>=param_phy_lo && port_phy<=param_phy_hi) {
                    ...
                    if (grp_speed == SPEED_0) {
                        ...
                        grp_speed_slots = tdm_fb6_cmn_get_speed_slots(grp_speed);
                    }
                    if ( (grp_speed==SPEED_20G  || grp_speed==SPEED_40G) &&
                        ...
                        port_speed_slots = tdm_fb6_cmn_get_speed_slots(port_speed);
 ...
 */
int
tdm_fb6_chk_bandwidth_lr(tdm_mod_t *_tdm)
{
    int result=PASS;

    /* IDB pipe 0 */
    if (tdm_fb6_chk_bandwidth_lr_pipe(_tdm, FB6_IDB_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* IDB pipe 1
    if (tdm_fb6_chk_bandwidth_lr_pipe(_tdm, FB6_IDB_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }*/
    /* MMU pipe 0 */
    if ( tdm_fb6_chk_bandwidth_lr_pipe(_tdm, FB6_MMU_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* MMU pipe 1
    if (tdm_fb6_chk_bandwidth_lr_pipe(_tdm, FB6_MMU_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }
    */
    return result;
 ...
 */
int
tdm_fb6_chk_bandwidth_os(tdm_mod_t *_tdm)
{
    int result=PASS;

    /* IDB pipe 0 */
    if (tdm_fb6_chk_bandwidth_os_pipe(_tdm, FB6_IDB_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* IDB pipe 1
    if (tdm_fb6_chk_bandwidth_os_pipe(_tdm, FB6_IDB_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }
    */
...
/**
@name: tdm_fb6_chk_bandwidth
@param:

Verifies IDB/MMU calendars allocating enough slots for port bandwidth
 */
int
tdm_fb6_chk_bandwidth(tdm_mod_t *_tdm)
{
    int result=PASS;

    /* Ancillary ports */
    if (tdm_fb6_chk_bandwidth_ancl(_tdm)!=PASS){
        result = FAIL;
    }
    /* Linerate ports */
    if(tdm_fb6_chk_bandwidth_lr(_tdm)!=PASS){
        result = FAIL;
    }
    /* Oversub ports */
    if(tdm_fb6_chk_bandwidth_os(_tdm)!=PASS){
        result = FAIL;
    }
...
/**
@name: tdm_fb6_chk_jitter_cmic
@param:

Checks distribution of cmic port for the given pipe
 */
int
tdm_fb6_chk_jitter_cmic(tdm_mod_t *_tdm, int cal_id)
{
    ...
    param_cal_len = tdm_fb6_cmn_get_pipe_cal_len(cal_id, _tdm);
	TDM_SEL_CAL(cal_id, param_cal_main);
	cmic_num = tdm_fb6_chk_get_pipe_token_cnt(_tdm, cal_id, FB6_CMIC_TOKEN);
	switch(cmic_num) {
		case 4:
			port_speed = SPEED_10G;
			break;
    ...
    tdm_fb6_cmn_get_speed_jitter(port_speed, param_cal_len, &space_min, &space_max);
	for(i = 0; i < param_cal_len; i++) {
		if(param_cal_main[i] != FB6_CMIC_TOKEN) {
            continue;
        }
		for(j=1; j< param_cal_len;j++) {
			if(param_cal_main[(i+j)%param_cal_len] == FB6_CMIC_TOKEN) {
...
/**
@name: tdm_fb6_chk_jitter_lr_pipe
@param:

Checks distribution of linerate slots of the same port for the given pipe
 */
int
tdm_fb6_chk_jitter_lr_pipe(tdm_mod_t *_tdm, int cal_id)
{
                    ...
                    if (param_cal_main[pos_curr] == port_phy) {
                        ...
                        if (port_space > space_max) {
                            if (port_speed<=SPEED_25G) { /* Low speed port: 10G, 20G, 25G */
                                ...
                                if (port_space > (space_max+FB6_LR_CHK_TRHD)) {
                                    result = FAIL;
...
/**
@name: tdm_fb6_chk_jitter_lr
@param:

Checks distribution of linerate tokens for IDB/MMU calendars
 */
int
tdm_fb6_chk_jitter_lr(tdm_mod_t *_tdm)
{
    int result=PASS;

    /* IDB pipe 0 */
    if (tdm_fb6_chk_jitter_lr_pipe(_tdm, FB6_IDB_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* IDB pipe 1
    if (tdm_fb6_chk_jitter_lr_pipe(_tdm, FB6_IDB_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }
    */
    /* MMU pipe 0 */
    if (tdm_fb6_chk_jitter_lr_pipe(_tdm, FB6_MMU_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* MMU pipe 1
    if (tdm_fb6_chk_jitter_lr_pipe(_tdm, FB6_MMU_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }
    */
...
/**
@name: tdm_fb6_chk_os_halfpipe
@param:

Check oversub Half Pipes (only for IDB)
 */
int
tdm_fb6_chk_os_halfpipe(tdm_mod_t *_tdm, int cal_id)
{
    ...
    if (hp0_port_cnt==0 && hp1_port_cnt==0) {
        ...
        for (i=0; i<param_cal_grp_num; i++) {
            for (j=0; j<param_cal_grp_len; j++) {
                port_phy = param_cal_grp[i][j];
	/* fb6 is a fixed unbalanced half pipe */
	...
	/* hp1 falcon 1 , 2 & egl3 :- pm's 61,69,73 */
                port_hp_id = (port_phy <(49)) ? (0) :
                    ...
                    port_pm = tdm_fb6_cmn_get_port_pm(port_phy, _tdm);
...
/**
@name: tdm_fb6_chk_pkt_sched_print
@param:

Print out PKT scheduler calender
 */
int
tdm_fb6_chk_pkt_sched_print(tdm_mod_t *_tdm, int cal_id, int hpipe_id)
{
    int i, j, x, y, port_phy, port_space, slot_cnt,
        hp_id, result=PASS;
    int dist_max[FB6_NUM_PHY_PORTS_PER_HPIPE],
        dist_min[FB6_NUM_PHY_PORTS_PER_HPIPE];
    int port_cnt = 0, port_buff[FB6_NUM_PHY_PORTS_PER_HPIPE];
    int max_tmp, min_tmp,
        max_10g = 0, max_20g = 0, max_25g = 0,
        max_40g = 0, max_50g = 0, max_100g = 0,
        min_10g = FB6_SHAPING_GRP_LEN, min_20g = FB6_SHAPING_GRP_LEN,
        min_25g = FB6_SHAPING_GRP_LEN, min_40g = FB6_SHAPING_GRP_LEN,
        min_50g = FB6_SHAPING_GRP_LEN, min_100g= FB6_SHAPING_GRP_LEN;
    int param_phy_lo, param_phy_hi,
        param_cal_grp_num, param_cal_grp_len;
    int *param_pkt_cal;
    int **param_cal_grp=NULL;
    ...
    for (i = 0; i < FB6_NUM_PHY_PORTS_PER_HPIPE; i++) {
        port_buff[i]= FB6_NUM_EXT_PORTS;
        dist_max[i] = 0;
        dist_min[i] = FB6_SHAPING_GRP_LEN;
    }
    ...
    switch (hpipe_id) {
        case 0: param_pkt_cal = param_cal_grp[FB6_SHAPING_GRP_IDX_0]; break;
        case 1: param_pkt_cal = param_cal_grp[FB6_SHAPING_GRP_IDX_1]; break;
    ...
    for (i=0; i<param_cal_grp_num; i++) {
        ...
        for (j=0; j<param_cal_grp_len; j++){
            port_phy = param_cal_grp[i][j];
            if (port_phy >= param_phy_lo && port_phy <= param_phy_hi) {
                if (port_cnt < FB6_NUM_PHY_PORTS_PER_HPIPE) {
    ...
    for (i = 0; i < port_cnt; i++) {
        ...
        for (j=0; j <FB6_SHAPING_GRP_LEN; j++) {
            if (port_phy == param_pkt_cal[j]) {
                port_space = 0;
                for (x = 0; x < FB6_SHAPING_GRP_LEN; x++) {
                    y = (x + j + 1) % FB6_SHAPING_GRP_LEN;
                    if (param_pkt_cal[y] == FB6_NUM_EXT_PORTS) {
                        continue;
                        ...
                        slot_cnt = (slot_cnt + 1) % FB6_NUM_PKT_SLOTS_PER_PM;
                        break;
                    } else {
                        port_space++;
                    }
    ...
    if (min_10g == FB6_SHAPING_GRP_LEN && max_10g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "10G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "10G", min_10g, max_10g, max_10g - min_10g);
    }
    if (min_20g == FB6_SHAPING_GRP_LEN && max_20g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "20G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "20G", min_20g, max_20g, max_20g - min_20g);
    }
    if (min_25g == FB6_SHAPING_GRP_LEN && max_25g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "25G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "25G", min_25g, max_25g, max_25g - min_25g);
    }
    if (min_40g == FB6_SHAPING_GRP_LEN && max_40g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "40G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "40G", min_40g, max_40g, max_40g - min_40g);
    }
    if (min_50g == FB6_SHAPING_GRP_LEN && max_50g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "50G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "50G", min_50g, max_50g, max_50g - min_50g);
    }
    if (min_100g == FB6_SHAPING_GRP_LEN && max_100g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "100G", "--", "--", "--");
    } else {
...
/**
@name: tdm_fb6_chk_pkt_sched
@param:

Checks same port jitter for PKT scheduler calender
 */
int
tdm_fb6_chk_pkt_sched(tdm_mod_t *_tdm, int cal_id, int hpipe_id)
{
	int i, j, k, port_phy, port_speed, slot_req, slot_cnt,
        hp_id, result=PASS;
    int dist_max[FB6_NUM_PKT_SLOTS_PER_PM], dist_min[FB6_NUM_PKT_SLOTS_PER_PM];
    int x, y, port_space, pkt_jitter, slot_pos_base, slot_pos,
        port_lanes, pos_step;
    int port_cnt = 0, port_buff[FB6_NUM_PKT_SLOTS_PER_PM];
    int param_phy_lo, param_phy_hi,
        param_cal_grp_num, param_cal_grp_len;
    int *param_pkt_cal;
    int **param_cal_grp=NULL;
    ...
    for (i = 0; i < FB6_NUM_PKT_SLOTS_PER_PM; i++) {
        dist_max[i] = 0;
        dist_min[i] = FB6_SHAPING_GRP_LEN;
        port_buff[i]= FB6_NUM_EXT_PORTS;
    }
    ...
    for (i=0; i<param_cal_grp_num; i++) {
        ...
        for (j=0; j<param_cal_grp_len; j++){
            port_phy = param_cal_grp[i][j];
            if (port_phy >= param_phy_lo && port_phy <= param_phy_hi) {
                if (port_cnt < FB6_NUM_PKT_SLOTS_PER_PM) {
                    port_buff[port_cnt++] = port_phy;
                }
            }
        }
    ...
    for (i = 0; i < port_cnt; i++) {
        port_phy = port_buff[i];
        port_speed   = param_speeds[port_phy];
        slot_req = tdm_fb6_chk_get_speed_slots_5G(port_speed);
        slot_cnt = 0;
        for (k=0; k <FB6_SHAPING_GRP_LEN; k++) {
    ...
    for (i = 0; i < port_cnt; i++) {
        ...
        slot_pos_base = 0;
        port_speed = param_speeds[port_phy];
        port_lanes = tdm_fb6_chk_get_port_lanes(_tdm, port_phy);
        switch (port_lanes) {
        ...
        for (k=0; k <FB6_SHAPING_GRP_LEN/8; k++) {
            /* quad mode (lanes == 1) */
            ...
            slot_pos = (slot_pos_base + k * 8) % FB6_SHAPING_GRP_LEN;
            if(param_pkt_cal[slot_pos] != port_phy &&
               param_pkt_cal[slot_pos] != FB6_NUM_EXT_PORTS) {
...
/*             TDM_PRINT5("%s port %3d, speed %dG, jitter/limit %d/%d\n",
    ...
    tdm_fb6_chk_pkt_sched_print(_tdm, cal_id, hpipe_id);
...
tdm_fb6_chk(tdm_mod_t *_tdm)
{
    ...
    if (tdm_fb6_chk_struct(_tdm) != PASS) {
        result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_fb6_chk_transcription(_tdm) != PASS) {
        result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_fb6_chk_sister(_tdm) != PASS) {
        result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_fb6_chk_bandwidth(_tdm) != PASS) {
         result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_fb6_chk_jitter_lr(_tdm) != PASS) {
        result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_fb6_chk_pkt_sched(_tdm, FB6_IDB_PIPE_0_CAL_ID, 0) != PASS ||
        tdm_fb6_chk_pkt_sched(_tdm, FB6_IDB_PIPE_0_CAL_ID, 1) != PASS ||
        tdm_fb6_chk_pkt_sched(_tdm, FB6_IDB_PIPE_1_CAL_ID, 0) != PASS ||
        tdm_fb6_chk_pkt_sched(_tdm, FB6_IDB_PIPE_1_CAL_ID, 1) != PASS ) {
        result = FAIL;
    }
    */
    TDM_SML_BAR
