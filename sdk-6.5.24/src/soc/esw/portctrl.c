...
#ifdef BCM_HURRICANE4_SUPPORT
        /* At lower frequency <= 200Mhz and HG port at 21G frame alignment
         * error can occur guideline is to set TX_THRESHOLD to 4 to avoid
         * mac overspeeding mac-tsc interface */
        if (SOC_IS_HURRICANE4(unit)) {
            soc_info_t *si = &SOC_INFO(unit);
            if ((si->frequency <= 200) &&
                            (IS_HG_PORT(unit, port)) && (speed >= 21000)) {
                uint64 reg_val;
    ...
    if (SOC_IS_FIRELIGHT(unit)) {
        ...
        if (pm_type == portmodDispatchTypePm8x50) {
            ...
            if(timeout != 0) {
                ...
                rv = soc_linkscan_flr_enable_set(unit, port, 1, timeout);
                if (rv != SOC_E_NONE) {
                    if (local_add_info != NULL) {
                        sal_free(local_add_info);
                    }
