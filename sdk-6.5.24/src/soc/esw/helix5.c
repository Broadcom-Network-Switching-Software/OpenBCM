...
_soc_hx5_port_flex_init(int unit, int is_any_cap)
{
    ...
    if (SOC_PBMP_NOT_NULL(SOC_PORT_DISABLED_BITMAP(unit, all)) || is_any_cap) {
        ...
        for (phy_port = 1; phy_port <= HELIX5_GPHY_PORTS_PER_PIPE; phy_port++) {
            if (phy_port <= 48) {
                pm = (phy_port - 1) / _HX5_PORTS_PER_PMQ_PBLK;
            } else {
                pm = ((phy_port - 1) / _HX5_PORTS_PER_PBLK) - 9;
            }
            phy_base = SOC_HX5_PORT_BLOCK_BASE_PHY_PORT(phy_port);
...
int
soc_helix5_port_blk_reset(int unit, int blk)
{
    int port, blktype;
    uint32 rval;
    uint64 rval64;
...
soc_helix5_port_reset(int unit)
{
    ...
    SOC_BLOCK_ITER(unit, blk, SOC_BLK_CLPORT) {
        SOC_IF_ERROR_RETURN(soc_helix5_port_blk_reset(unit, blk));
    }
...
#define _SOC_HX5_DEF_TEMP_MAX  110
    ...
    * IPEP clock gating should be disabled if visibility feature support is needed. */
    if (soc_property_get(unit, spn_IPEP_CLOCK_GATING_DISABLE, 0)) {
        ...
        SOC_IF_ERROR_RETURN(WRITE_TOP_CLOCKING_ENFORCE_PSGr(unit, 0x1fffff));
        SOC_IF_ERROR_RETURN(WRITE_TOP_CLOCKING_ENFORCE_PCGr(unit, 0x7ffff));
    } else {
                               ...
                               _soc_hx5_mmu_sw_config_info_t *swcfg)
{
    ...
    int default_mtu_cells, limit = 0, rlimit;
    int pipe;
...
int
soc_helix5_mmu_pfc_tx_config_set(int unit, bcm_port_t port, int pfc_enable)
{
    ...
    soc_reg_t obm_fc_config_regs[] = {
        IDB_OBM0_Q_FLOW_CONTROL_CONFIGr, IDB_OBM1_Q_FLOW_CONTROL_CONFIGr,
        IDB_OBM2_Q_FLOW_CONTROL_CONFIGr, IDB_OBM0_FLOW_CONTROL_CONFIGr,
        IDB_OBM1_FLOW_CONTROL_CONFIGr, IDB_OBM2_FLOW_CONTROL_CONFIGr,
        IDB_OBM3_FLOW_CONTROL_CONFIGr, IDB_OBM0_48_FLOW_CONTROL_CONFIGr,
        IDB_OBM1_48_FLOW_CONTROL_CONFIGr, IDB_OBM2_48_FLOW_CONTROL_CONFIGr,
    };
