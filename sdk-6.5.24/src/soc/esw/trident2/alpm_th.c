    ...
    if (ipv6) {
        ...
        if (len > 32) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u, "Invalid prefix length %d\n"), len));
        }
                                ...
                                int *shuffled)
{
    ...
    if (max_split_count < 0) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u, "Unexpected max_split_count %d!\n"),
                              max_split_count));
    } else if (max_split_count == 0) {
        return SOC_E_NONE;
    }
...
_soc_th_alpm_phy_bkt_trie_count(int u, int phy_bkt)
{
    ...
    for (sub_bkt = 0; sub_bkt < SOC_TH_MAX_SUB_BUCKETS; sub_bkt++) {
        if (SOC_ALPM_BU_SUB_BKT_IS_SET(u, phy_bkt, sub_bkt)) {
            ...
            if (ALPM_LOG_BKT(phy_bkt, sub_bkt) != PIVOT_BUCKET_INDEX(pivot)) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u, "LOG BKT IDX mismatched!\n")));
            }
...
#ifdef CHECK_CLEAN_BKT
    if (count != _soc_th_alpm_phy_bkt_trie_count(u, phy_bkt)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u, "PHY BKT trie count mismatched!\n")));
    }
#endif
    return;
}
                                  ...
                                  int w_index, int bpm_len, int v4_half)
{
    ...
    if (bpm_len == INVALID_BPM_LEN) {
        ...
        if (v6 != (v4_half == 0)) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u, "Unexpected MODE1 field value %d!\n"),
                                  v6));
        }
        if (v6) {
                              ...
                              defip_entry_t *lpm_entry, int init)
{
    ...
    if (ipv6) {
        ...
        if (len > 32) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u, "Unexpected prefix length %d!\n"), len));
        }
        mask = (len == 32) ? 0xffffffff : ~(0xffffffff >> len);
        SOC_MEM_OPT_F32_SET(u, L3_DEFIPm, lpm_entry, IP_ADDR_MASK0f, mask);
    }
                        ...
                        int *key_index, int v6)
{
    ...
    rv = soc_mem_alpm_delete(u, mem, phy_bkt, MEM_BLOCK_ALL,
                             bank_disable, bufp2, e, key_index, &ser_retry);
    if (SOC_SUCCESS(rv) && (ser_retry == 0)) {
        return rv;
    }
    if (ALPM_CTRL(u).double_wide[v6]) {
        rv2 = soc_mem_alpm_delete(u, mem, phy_bkt + 1,
                                   MEM_BLOCK_ALL, bank_disable, bufp2, e,
                                   key_index, NULL);
        if (rv2 != SOC_E_NOT_FOUND) {
            return rv2;
        }
    }
    return rv;
                    ...
                    int tcam_index, int key_index)  /* TCAM entry data */
{
    ...
    if (!(ALPM_PREFIX_IN_TCAM(u, vrf_id))) {
        ...
        if (empty_bucket) { /* Empty bucket */
            ...
            if (_soc_th_alpm_bkt_count(u, mem, 1, delete_bucket, vrf,
                                       bufp) != 0) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
...
/* clear alpm bucket trie related internal state */
static int soc_th_alpm_tcam_pivot_state_clear(int u) {
    int i, rv = SOC_E_NONE;
    alpm_pivot_t *tmp;

    /* clear all pivot bucket trie state */
    for (i = 0; i < TH_MAX_PIVOT_COUNT(u); i++) {
            ...
            if (PIVOT_BUCKET_HANDLE(tmp)) {
                if (PIVOT_BUCKET_TRIE(tmp)) {
                    ...
                    rv = trie_traverse(PIVOT_BUCKET_TRIE(tmp),
                            alpm_delete_node_cb, NULL, _TRIE_POSTORDER_TRAVERSE);
                    if (SOC_SUCCESS(rv)) {
...
static int soc_th_alpm_state_clear(int u)
{
    ...
    for (i = 0; i <= SOC_VRF_MAX(u) + 1; i++) {
        ...
        rv = trie_traverse(VRF_PIVOT_TRIE_IPV4(u, i), alpm_delete_node_cb,
                           NULL, _TRIE_POSTORDER_TRAVERSE);
        if (SOC_SUCCESS(rv)) {
            trie_destroy(VRF_PIVOT_TRIE_IPV4(u, i));
        } else {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u,
                                  "unit: %d Unable to clear v4 pivot trie for vrf %d\n"),
                       u, i));
            return rv;
        }
        rv = trie_traverse(VRF_PIVOT_TRIE_IPV6(u, i), alpm_delete_node_cb,
                           NULL, _TRIE_POSTORDER_TRAVERSE);
        if (SOC_SUCCESS(rv)) {
            trie_destroy(VRF_PIVOT_TRIE_IPV6(u, i));
        } else {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u,
                                  "unit: %d Unable to clear v6 pivot trie for vrf %d\n"),
                       u, i));
            return rv;
        }
...
soc_th_alpm_deinit(int u)
{
    if (SOC_ALPM_LPM_INIT_CHECK(u)) {
        ...
        SOC_IF_ERROR_RETURN(soc_th_alpm_tcam_pivot_state_clear(u));
...
soc_th_alpm_vrf_add(int u, int vrf, int v6)
{
    ...
    rv = soc_th_alpm_lpm_insert(u, &tmp_lpm, &index, 0, 0, 0);
    if (SOC_FAILURE(rv)) {
        goto _err_exit;
    }
               ...
               defip_aux_scratch_entry_t *aux_entry)
{
    ...
    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, key_data, DEFAULT_MISS0f);
    soc_mem_field32_set(u, L3_DEFIP_AUX_SCRATCHm, aux_entry, DEFAULT_MISSf, ipv4a);
                          ...
                          int flex)
{
    ...
    if (ipv6) {
        ...
        if (ipv4a > 32) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u, "Unexpected IP prefix length %d!\n"),
                                     ipv4a));
        }
