                    ...
                    int tcam_index, int key_index)  /* TCAM entry data */
{
    ...
    if (!(ALPM_PREFIX_IN_TCAM(u, vrf_id))) {
        ...
        if (empty_bucket) { /* Empty bucket */
            ...
            if (_soc_th_alpm_128_bkt_count(u, mem, 1, delete_log_bkt, vrf,
                                           bufp) != 0) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
...
int soc_th_alpm_128_state_clear(int u)
{
    ...
    for (i = 0; i <= SOC_VRF_MAX(u) + 1; i++) {
        ...
        rv = trie_traverse(VRF_PIVOT_TRIE_IPV6_128(u, i), alpm_delete_node_cb,
                           NULL, _TRIE_POSTORDER_TRAVERSE);
        if (SOC_SUCCESS(rv)) {
            trie_destroy(VRF_PIVOT_TRIE_IPV6_128(u, i));
        } else {
                 LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
                          "unit: %d Unable to clear v6_128 pivot trie for "
                          "vrf %d\n"), u, i));
            return rv;
        }
               ...
               defip_aux_scratch_entry_t *aux_entry)
{
    ...
    ipv4a = SOC_MEM_OPT_F32_GET_128(u, L3_DEFIP_PAIR_128m, key_data,
                                DEFAULT_MISSf);
    soc_mem_field32_set(u, L3_DEFIP_AUX_SCRATCHm, aux_entry, DEFAULT_MISSf,
                        ipv4a);
