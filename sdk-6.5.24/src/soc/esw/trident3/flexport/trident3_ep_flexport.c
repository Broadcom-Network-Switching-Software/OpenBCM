...
static const soc_td3_ep_core_cfg_t
    soc_td3_ep_core_cfg_tbl[] = {
    /* L2_lat & L3_lat not used in TD3
	 * speed   ep2mmu  ep2_mmu_max    L2_lat      L3 lat       full_lat
     * LR  3:2 2:1   LR  3:2 2:1   LR  3:2 2:1
     */
    {    -1,  0,  0, { 7, 13, 16}, { 7, 13, 16}, { 7, 13, 16} },   /* SAF     */
    { 10000, 11,  8, {29, 42, 48}, {29, 42, 48}, {29, 42, 48} },   /* 10GE    */
    { 11000, 11,  8, {29, 42, 48}, {29, 42, 48}, {29, 42, 48} },   /* HG[11]  */
    { 20000, 14,  9, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* 20GE    */
    { 21000, 14,  9, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* HG[21]  */
    { 25000, 16, 12, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* 25GE    */
    { 27000, 16, 12, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* HG[27]  */
    { 40000, 25, 16, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* 40GE    */
    { 42000, 25, 16, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* HG[42]  */
    { 50000, 26, 19, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* 50GE    */
    { 53000, 26, 19, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* HG[53]  */
    {100000, 41, 35, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} },   /* 100GE   */
    {106000, 41, 35, {13, 26, 32}, {13, 26, 32}, {13, 26, 32} }    /* HG[106] */
};


/*! @fn int soc_trident3_ep_flexport_get_max_ovs_ratio(int unit,
 ...
 */
int
soc_trident3_ep_flexport_get_max_ovs_ratio(
...
/*! @fn int soc_trident3_ep_get_ep2mmu_credit(int unit, 
 ...
 */
int
soc_trident3_ep_get_ep2mmu_credit(
    int unit, soc_port_schedule_state_t *port_schedule_state, int logical_port)
{
    ...
    soc_trident3_ep_flexport_get_max_ovs_ratio(unit,
                                                port_schedule_state,
                                                &old_max_ovs_ratio,
                                                &new_max_ovs_ratio);
    ...
    ct_class = soc_trident3_get_ct_class(speed);
    ...
    if ((1525 == port_schedule_state->frequency) &&
        ...
        return (soc_td3_ep_core_cfg_tbl[ct_class].egr_mmu_credit_max_freq + credit_incr);
    } else {
        return soc_td3_ep_core_cfg_tbl[ct_class].egr_mmu_credit;
    }
}


/*! @fn int soc_trident3_ep_get_ct_xmit_start_cnt(int ct_class, int latency_mode,
 *              int ovs_ratio)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct 
 *         variable
 ...
 */
int
soc_trident3_ep_get_ct_xmit_start_cnt(int ct_class, int latency_mode,
                                       int ovs_ratio)
{
    int xmit_cnt;
    ...
    case 0:
        if (ovs_ratio <= 10) {     /* LR */
            xmit_cnt =
                soc_td3_ep_core_cfg_tbl[ct_class].xmit_cnt_full_latency.
                line_rate;
    ...
    case 1:
        if (ovs_ratio <= 10) {     /* LR */
            xmit_cnt =
                soc_td3_ep_core_cfg_tbl[ct_class].xmit_cnt_l3_latency.line_rate;
        } else if (ovs_ratio <= 15) {     /* OVS 1.61 */
            xmit_cnt =
                soc_td3_ep_core_cfg_tbl[ct_class].xmit_cnt_l3_latency.
                oversub_3_2;
        } else {     /* OVS 2:1 */
            xmit_cnt =
                soc_td3_ep_core_cfg_tbl[ct_class].xmit_cnt_l3_latency.
    ...
    case 2:
        if (ovs_ratio <= 10) {     /* LR */
            xmit_cnt =
                soc_td3_ep_core_cfg_tbl[ct_class].xmit_cnt_l2_latency.line_rate;
        } else if (ovs_ratio <= 15) {     /* OVS 1.61 */
            xmit_cnt =
                soc_td3_ep_core_cfg_tbl[ct_class].xmit_cnt_l2_latency.
                oversub_3_2;
        } else {     /* OVS 2:1 */
            xmit_cnt =
                soc_td3_ep_core_cfg_tbl[ct_class].xmit_cnt_l2_latency.
...
/*! @fn int soc_trident3_ep_get_xmit_start_count(int unit,
 *              soc_port_schedule_state_t *port_schedule_state,
 ...
 */
int
soc_trident3_ep_get_xmit_start_count(
    int  unit, soc_port_schedule_state_t *port_schedule_state,
    int  logical_port, int *xmit_start_cnt)
{
    ...
    cookie = port_schedule_state->cookie;
    speed = cookie->exact_out_log_port_speed[logical_port];
    dst_ct_class = soc_trident3_get_ct_class(speed);
    ...
    saf_xmit_start_cnt = soc_trident3_ep_get_ct_xmit_start_cnt(
                                      0, latency_mode, new_max_ovs_ratio);                                                            /* 0 means SAF */
    dst_ct_xmit_start_cnt = soc_trident3_ep_get_ct_xmit_start_cnt(
...
/*! @fn int soc_trident3_ep_flexport_sft_rst_pm_intf(int unit,
 ...
 */
int
soc_trident3_ep_flexport_sft_rst_pm_intf(
    int unit, soc_port_schedule_state_t *port_schedule_state, int rst_on)
{
    ...
    uint32 pm_rst_values[TRIDENT3_PIPES_PER_DEV][TRIDENT3_PBLKS_PER_PIPE];

    uint64 rval64;
    soc_reg_t reg;
    static const soc_reg_t edb_sft_rst_regs[TRIDENT3_PIPES_PER_DEV] = {
        EGR_PORT_BUFFER_SFT_RESET_0_PIPE0r, EGR_PORT_BUFFER_SFT_RESET_0_PIPE1r
    };


    /* pipe_map indicates which pipe has flexing ports */
    _soc_td3_tdm_get_pipe_map(unit, port_schedule_state, &pipe_map);
    ...
    if (1 == rst_on) {
        for (i = 0; i < port_schedule_state->nport; i++) {
            ...
            pm_num = (physical_port - 1) / TRIDENT3_PORTS_PER_PBLK; /* device pm_num*/
            pm_indx = (pm_num % TRIDENT3_PBLKS_PER_PIPE);
            /* pm_indx is pipe local PM indx and swapped for odd pipes */
            pm_indx =
                ((pipe % 2) == 0) ? pm_indx : (TRIDENT3_PBLKS_PER_PIPE-1 - pm_indx);
            subport = (physical_port - 1) % TRIDENT3_PORTS_PER_PBLK;
            switch (subport) {
            case 0:
            case 1:
                if (num_lanes == 4) {
        ...
        for (pipe = 0; pipe < TRIDENT3_PIPES_PER_DEV; pipe++) {
            for (pm_indx = 0; pm_indx < TRIDENT3_PBLKS_PER_PIPE; pm_indx++) {
...
/*! @fn int soc_trident3_ep_flexport_sft_rst_ports(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 ...
 */
int
soc_trident3_ep_flexport_sft_rst_ports(
    int unit, soc_port_schedule_state_t *port_schedule_state, int rst_on)
{
    int i;
    ...
    uint32 memfld;
    soc_mem_t mem;
    int port_rst_serviced[TRIDENT3_PHY_PORTS_PER_DEV];
...
/*! @fn int soc_trident3_ep_enable_disable(int unit,
 *              soc_port_schedule_state_t *port_schedule_state,
 ...
 */
int
soc_trident3_ep_enable_disable(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        logical_port,
                                int                        down_or_up)
{
...
/*! @fn int soc_trident3_ep_set_egr_mmu_credit(int unit,
 *              soc_port_schedule_state_t *port_schedule_state
 *              int logical_port, int down_or_up)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct 
 ...
 */
int
soc_trident3_ep_set_egr_mmu_credit(
    int unit, soc_port_schedule_state_t *port_schedule_state,
    int logical_port, int down_or_up)
{
    uint32 entry[SOC_MAX_MEM_WORDS];
    ...
    if (0 == down_or_up ) { /* port down */
        ...
        memfld = soc_trident3_ep_get_ep2mmu_credit(unit, port_schedule_state,
                                                    logical_port);
        physical_port =
            port_schedule_state->out_port_map.port_l2p_mapping[logical_port];
    }
...
/*! @fn int soc_trident3_ep_set_dev_to_phy(int unit,
 *              soc_port_schedule_state_t *port_schedule_state
 *              int logical_port, int down_or_up)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 ...
 */
int
soc_trident3_ep_set_dev_to_phy(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        logical_port,
                                int                        down_or_up)
{
...
/*! @fn int soc_trident3_ep_set_edb_1dbg_b(int unit,
 *              soc_port_schedule_state_t *port_schedule_state
 *              int logical_port, int down_or_up)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 ...
 */
int
soc_trident3_ep_set_edb_1dbg_b(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        logical_port,
                                int                        down_or_up)
{
...
/*! @fn int soc_trident3_ep_flexport_extra_holding_set(int unit,
 ...
 */
int
soc_trident3_ep_flexport_extra_holding_set(
    int unit, soc_port_schedule_state_t *port_schedule_state,
    int cell_counts)
{
    int pipe;
    ...
    uint32 pipe_map;
    soc_reg_t reg;
    static const soc_reg_t extr_hold_regs[TRIDENT3_PIPES_PER_DEV] = {
...
/*! @fn int soc_trident3_ep_flexport_xmit_cnt_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state, int logical_port)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_ep_flexport_xmit_cnt_set(
    int unit, soc_port_schedule_state_t *port_schedule_state, int logical_port)
{
    soc_mem_t mem;
    ...
    static const soc_mem_t ep_xmit_cnt_mems[TRIDENT3_PIPES_PER_DEV] = {
    ...
    soc_trident3_ep_get_xmit_start_count(unit, port_schedule_state,
                                          logical_port,
                                          xmit_start_cnt);
    for (class = 0; class < 13; class++) {
        sal_memset(entry, 0, sizeof(uint32) *
                   soc_mem_entry_words(unit, EGR_XMIT_START_COUNT_PIPE0m));
        mem_index = (logical_port % TRIDENT3_DEV_PORTS_PER_PIPE) * 16 + class;
...
/*! @fn int soc_trident3_ep_flexport_xmit_cnt_set_all_ports(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_ep_flexport_xmit_cnt_set_all_ports(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int old_max_ovs_ratio, new_max_ovs_ratio;
    ...
    if (new_max_ovs_ratio != old_max_ovs_ratio) {
        for (logical_port = 0; logical_port < TRIDENT3_DEV_PORTS_PER_DEV;
             logical_port++) {
            ...
            if ((port_schedule_state->out_port_map.log_port_speed[logical_port]
                 > 0 ) &&
                (is_ovs == 1)) {
                soc_trident3_ep_flexport_xmit_cnt_set(unit,
                                                       port_schedule_state,
                                                       logical_port);
            }
        }
...
/*! @fn int soc_trident3_ep_flexport_ct_class_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state, int logical_port)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_ep_flexport_ct_class_set(
    int unit, soc_port_schedule_state_t *port_schedule_state, int logical_port)
{
    soc_mem_t mem;
    uint32 memfld;
    ...
    cookie = port_schedule_state->cookie;
    speed  = cookie->exact_out_log_port_speed[logical_port];
    memfld = soc_trident3_get_ct_class(speed);
...
/*! @fn int soc_trident3_ep_flexport_drain_port(int unit,
 *              soc_port_schedule_state_t *port_schedule_state, int logical_port)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_ep_flexport_drain_port(
    int unit, soc_port_schedule_state_t *port_schedule_state, int logical_port)
{
    int pipe;
    ...
    static const soc_reg_t misc_ctrl_regs[TRIDENT3_PIPES_PER_DEV] = {
...
/*! @fn int soc_trident3_flex_ep_port_down(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_flex_ep_port_down(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port == -1) {
            logical_port = port_schedule_state->resource[i].logical_port;
            SOC_IF_ERROR_RETURN(soc_trident3_ep_flexport_drain_port(unit,
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if (-1 == physical_port) { /* that is, port DOWN */
            ...
            soc_trident3_ep_enable_disable(unit, port_schedule_state,
                                            logical_port,
                                            down_or_up);
        }
    }
    ...
    rst_on = 1;
    SOC_IF_ERROR_RETURN(soc_trident3_ep_flexport_sft_rst_ports(unit,
                                                                port_schedule_state,
                                                                rst_on));
...
/*! @fn int soc_trident3_flex_ep_reconfigure_remove(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if (-1 == physical_port) { /* that is, port DOWN */
             ...
             * EGR_DEVICE_TO_PHYSICAL_PORT_NUMBER_MAPPINGr
             */
            soc_trident3_ep_set_dev_to_phy(unit, port_schedule_state,
                                            logical_port,
                                            down_or_up);
            /* Reset EP2MMU credits to 0 : write EGR_MMU_CELL_CREDITm */
            soc_trident3_ep_set_egr_mmu_credit(unit, port_schedule_state,
                                                logical_port,
                                                down_or_up);
            /* Reset edb_1dbg_b : write EDB_1DBG_Bm */
            soc_trident3_ep_set_edb_1dbg_b(unit, port_schedule_state,
                                            logical_port,
                                            down_or_up);
        }
    }
    ...
    SOC_IF_ERROR_RETURN(soc_trident3_ep_flexport_extra_holding_set(unit,
                            port_schedule_state, 8)); /* 8 as per TD3 Chip Uarch Delta Spec */

    return SOC_E_NONE;
}


/*! @fn int soc_trident3_flex_ep_reconfigure_add(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if (-1 != physical_port) { /* that is, port UP */
            ...
            soc_trident3_ep_flexport_ct_class_set(unit, port_schedule_state,
                                                   logical_port);
     ...
     * write EGR_XMIT_START_COUNT_PIPE0/1m
     */
    SOC_IF_ERROR_RETURN(soc_trident3_ep_flexport_xmit_cnt_set_all_ports(unit,
                            port_schedule_state));
...
/*! @fn int soc_trident3_flex_ep_port_up(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_flex_ep_port_up(int                        unit,
                              soc_port_schedule_state_t *port_schedule_state)
{
    int i, rst_on, down_or_up;
    int logical_port, physical_port;
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (ovs_ratio = 2; ovs_ratio < 4; ovs_ratio++) {
        ...
        for (ct_class=0; ct_class < 13; ct_class++) {
            LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                     ...
                     ct_class, soc_td3_ep_core_cfg_tbl[ct_class].egr_mmu_credit,
                     soc_td3_ep_core_cfg_tbl[ct_class].egr_mmu_credit_max_freq));
            for (lat=2; lat>=0; lat--) {
                xmit_cnt = soc_trident3_ep_get_ct_xmit_start_cnt(ct_class, lat,
                                                                  5*ovs_ratio);
    ...
    for (port = 1; port < SOC_MAX_NUM_PORTS; port++) {
        if (port_schedule_state->out_port_map.log_port_speed[port] > 0 ) {
            ep2mmu_cred = soc_trident3_ep_get_ep2mmu_credit(
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_trident3_ep_flexport_remove_ports_shim(): "
                        "calling soc_trident3_flex_start")));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_start(unit, port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_trident3_ep_flexport_remove_ports_shim(): "
                        "calling soc_trident3_tdm_calculation_flexport")));
    SOC_IF_ERROR_RETURN(soc_trident3_tdm_calculation_flexport(unit,
                                                               port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_trident3_ep_flexport_remove_ports_shim(): "
                        "calling soc_trident3_flex_ep_port_down")));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_ep_port_down(unit,
                                                        port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_trident3_ep_flexport_remove_ports_shim(): "
                        "calling soc_trident3_ep_flexport_reconfigure_remove")));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_ep_reconfigure_remove(unit,
                            port_schedule_state));
...
/* Comment out these functions from soc_trident3_ep_flexport_add_ports function
 * 1. soc_trident3_ep_flexport_ct_class_set
 * 2. soc_trident3_ep_flexport_xmit_cnt_set_all_ports
 * 3. soc_trident3_ep_enable_disable
 */
int
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_trident3_ep_flexport_add_ports_shim(): "
                        "calling soc_trident3_ep_flexport_reconfigure_add")));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_ep_reconfigure_add(unit,
                                                              port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_trident3_ep_flexport_add_ports_shim(): "
                        "calling soc_trident3_ep_flexport_add_ports")));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_ep_port_up(unit, port_schedule_state));
