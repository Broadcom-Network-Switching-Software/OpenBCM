                               ...
                               soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
	    ...
	    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "Bringing Falcon mac rx port %0d up\n"), phy_port));
            ...
            SOC_IF_ERROR_RETURN(soc_reg_rawport_get(unit, CLPORT_MAC_CONTROLr,
                                                    phy_port, 0, &skip_rval64));
            COMPILER_64_TO_32_LO(rval64_low, skip_rval64);
            if((rval64_low & 0x1) == 1) {
              return SOC_E_NONE;
            }
            else {

            /*  soc_reg64_field32_set(unit, CLPORT_MAC_CONTROLr, &skip_rval64, XMAC0_RESETf, 0);
            }*/
            /*SOC_IF_ERROR_RETURN(soc_reg_rawport_set(unit, CLPORT_MAC_CONTROLr,
                                                    phy_port, 0, skip_rval64));*/
