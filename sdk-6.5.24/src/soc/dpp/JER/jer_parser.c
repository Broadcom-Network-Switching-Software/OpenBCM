...
static int jer_parser_ipv6_ext(int unit)
{
    ...
    {
        SOCDNX_IF_ERR_EXIT(arad_parser_ipv6_extension_header_size_set(unit));
    }
...
static int jer_parser_segment_add(int unit, DPP_SEGMENT_E segment_id)
{
    ...
    ARAD_PP_IHP_PACKET_FORMAT_TABLE_TBL_DATA format_data[JER_SEGMENT_FULL_SIZE] = { { 0 } };
        ...
        int unit)
{
    ...
    if(!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "non_bfd_forwarding", 0)) {      
        UDPoIPv4_leafs[0].plc_sw = DPP_PLC_BFD_SINGLE_HOP;
        UDPoIPv4_2_leafs[0].plc_sw = DPP_PLC_BFD_SINGLE_HOP;
        UDPoIPv4_gtp_leafs[0].plc_sw = DPP_PLC_BFD_SINGLE_HOP;
    }
