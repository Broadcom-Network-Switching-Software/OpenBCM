    ...
    )
{
  ...
  ARAD_TCAM_PRIO_LOCATION 
      prio_location;
  ARAD_TCAM_ACTION_SIZE action_bitmap_ndx;
  ARAD_TCAM_BANK_OWNER db_bank_owner;
  ...
  for (bank_ndx = 0; bank_ndx < nof_banks; bank_ndx++) {
      ...
      if ((!move_block_not_needed) || (!move_per_block)) {
        ...
        if ((!move_block_not_needed) || (!move_per_block))
        {
            ...
            if ((db_bank_owner == SOC_TMC_TCAM_BANK_OWNER_PMF_0) || (db_bank_owner == SOC_TMC_TCAM_BANK_OWNER_PMF_1)) {
                res = arad_pp_fp_action_bitmap_ndx_get_unsafe(unit, ARAD_PP_FP_TCAM_DB_TO_FP_ID(tcam_db_id), &action_bitmap_ndx);
                SOC_SAND_CHECK_FUNC_RESULT(res, 18, exit);
                
                if (!(action_bitmap_ndx & ARAD_TCAM_ACTION_SIZE_FORTH_20_BITS)
                    && (entry_size == ARAD_TCAM_BANK_ENTRY_SIZE_320_BITS)
                    && (!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "tcam_sw_move_disabled_for_shared_action", 0))) {
                    move_per_block = FALSE;
                }
            }
  ...
  )
{
  ...
  bank_id_shift = (entry_size == ARAD_TCAM_BANK_ENTRY_SIZE_320_BITS) ? 2:1;

  for (bank_id = 0; bank_id < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); bank_id += bank_id_shift)
  {
    res = sw_state_access[unit].dpp.soc.arad.tm.tcam.tcam_db.bank_used.get(unit, tcam_db_id, bank_id, &is_used);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
    if (is_used)
    {
      used_banks_bitmap |= SAL_BIT(bank_id);
    }
  }
  ...
  while (found)
  {
      ...
      if (nof_entries_free == 0) {
          ...
          for (bank_id_to += bank_id_shift; bank_id_to < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); bank_id_to += bank_id_shift)
          {
            if (used_banks_bitmap & SAL_BIT(bank_id_to))
            {
              break;
            }
          }
  ...
  for (bank_id = 0; bank_id < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); bank_id += bank_id_shift)
  {
      if (used_banks_bitmap & SAL_BIT(bank_id))
      {
          ...
          if (bank_nof_entries == 0) {
              ...
              if (entry_size == ARAD_TCAM_BANK_ENTRY_SIZE_320_BITS) {
                  res = arad_tcam_db_bank_remove_unsafe(unit, tcam_db_id, bank_id + 1);
                  SOC_SAND_CHECK_FUNC_RESULT(res, 34, exit);
              }
          }
      }
