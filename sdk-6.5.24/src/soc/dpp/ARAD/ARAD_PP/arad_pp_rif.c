...
{
  uint32                                    res;
  ARAD_PP_IHP_IN_RIF_CONFIG_TABLE_TBL_DATA  tbl_data = { 0 };
  ...
  )
{
  ...
  disable_lsp_destination = soc_property_get(unit, spn_MPLS_LSP_DESTINATION_DISABLE, 0);
  ...
  )
{
  ...
  ARAD_PP_ISEM_ACCESS_ENTRY                isem_entry;
  ARAD_PP_IHP_LIF_TABLE_LABEL_PROTOCOL_OR_LSP_TBL_DATA sem_res_tbl_data;
  uint32  disable_lsp_destination;
  ...
  *lif_index = arad_pp_sem_result_to_lif_index(unit,isem_entry.sem_result_ndx);
  ...
  if (disable_lsp_destination) {
        term_info->term_profile = (sem_res_tbl_data.termination_profile & ARAD_PP_RIF_MPLS_TERM_PROFILE_LSB_MASK) |
                                  (sem_res_tbl_data.destination_valid << ARAD_PP_RIF_MPLS_TERM_PROFILE_LSB_NOF_BITS);
  } else {
        term_info->term_profile = sem_res_tbl_data.termination_profile;
  }

  if (sem_res_tbl_data.destination_valid && !disable_lsp_destination) {
  ...
  )
{
    ...
    uint32 is_ipv6_tunnel = term_key->flags & SOC_PPC_RIF_IP_TERM_FLAG_IPV6;
#endif 
    *dip_only = ((SOC_DPP_CONFIG(unit)->pp.ingress_ipv4_tunnel_term_mode == SOC_DPP_IP_TUNNEL_L2_LKUP_MODE_SIP_DIP_SEPERATED) ||
        ...
        (SOC_IS_DIP_SIP_VRF_WITH_MY_VTEP_INDEX_ENABLE(is_ipv6_tunnel, unit) && (term_key->flags & SOC_PPC_RIF_IP_TERM_FLAG_KEY_ONLY)));
    ...
    *my_vtep_index_sip_vrf_sem =  (SOC_IS_DIP_SIP_VRF_WITH_MY_VTEP_INDEX_ENABLE(is_ipv6_tunnel, unit) 
