  ...
  )
{
    ...
    uint32 i, bfd_gach_cc, y1731_gach, bfd_pwe_cw, oam_pwe_cw, oam_pwe_channel;
    uint32 num_clocks_per_mep_scan;
    uint32 bfd_cc_channel, y1731_channel, bfd_pwe_channel;    
    uint32 use_event_dma=0;
    uint32 use_report_dma=0;
    uint32 event_ndx;
    int core;
...
uint32
  arad_pp_oam_classifier_oem_mep_profile_replace_unsafe(
    ...
    SOC_SAND_IN  SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY *classifier_mep_entry,
    SOC_SAND_IN  uint32                           update_mp_type,
    SOC_SAND_IN  uint32                           is_server_client_update
  )
{
  uint32 res;
  uint8  found;
  ...
  if (((SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MULTIPLE_MEPS_ON_LIF_MODE(unit) && 
        ...
        (((classifier_mep_entry->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED) != 0) || (is_server_client_update))) {
...
uint32
  arad_pp_oam_classifier_oem1_slm_passive_profile_update (
    SOC_SAND_IN  int                           unit,
    SOC_SAND_INOUT  SOC_PPC_OAM_CLASSIFIER_OEM1_ENTRY_PAYLOAD  *oem1_payload
  )
{
  int level;
  uint8 mdl_mp_type;
  int nof_active_meps = 0, nof_passive_meps = 0, nof_mips = 0;
  ...
  for (level = 0; level < SOC_PPC_OAM_NOF_ETH_MP_LEVELS; level++) {
    ...
    if (mdl_mp_type == _JER_PP_OAM_MDL_MP_TYPE_ACTIVE_MATCH) {
      ...
      nof_passive_meps++;
    } else if (mdl_mp_type == _JER_PP_OAM_MDL_MP_TYPE_MIP) {
      nof_mips++;
    }
  }

  if (nof_passive_meps && (nof_active_meps == 0) && (nof_mips == 0)) {
    oem1_payload->mp_profile = ARAD_PP_SLM_PASSIVE_PROFILE;
  }

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_oam_classifier_oem1_slm_passive_profile_update()", 0, 0);
}
...
uint32
  arad_pp_oam_classifier_mep_delete_unsafe(
    ...
    SOC_SAND_IN  uint32                   mep_index,
    SOC_SAND_IN  SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY   *classifier_mep_entry,
    SOC_SAND_IN  SOC_PPC_OAM_MEP_DEL_INFO           *mep_del
  )
{
  uint32 res;
  uint8  new_mip_bitmap = 0;
      ...
      if (!((classifier_mep_entry->lif == _BCM_OAM_INVALID_LIF) &&
          ...
          if ((new_mip_bitmap != 0) || (new_mep_bitmap != 0) || new_mp_type_vector) {
              ...
              if (SOC_IS_QAX(unit) && SOC_DPP_CONFIG(unit)->pp.oam_hierarchical_lm &&
                  ...
                  if (SOC_IS_JERICHO_PLUS(unit) && !SOC_IS_QUX(unit) && mep_del->is_slm) {
                      res = arad_pp_oam_classifier_oem1_slm_passive_profile_update(unit, &oem1_payload);
                      SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
                  }
                  res = arad_pp_oam_classifier_oem1_entry_set_unsafe(unit, &oem1_key, &oem1_payload);
                  SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
  ...
  )
{
  ...
  rmep_db_entry->lifetime = lifetime;
  rmep_db_entry->lifetime_units = lifetime_units;
  ...
  soc_OAMP_RMEP_DBm_field_get(unit, entry, LAST_CCM_LIFETIME_VALIDf, &field);
  rmep_db_entry->last_ccm_lifetime_valid_on_create = (uint8)field;
        ...
        *rmeb_db_ndx = *rmeb_db_ndx_lcl;
        ...
        if (!report_mode) {
            ...
            if (report_mode == 2) {
                ...
                if (rx_report_data.event_size > 9) {
                    
                    LOG_VERBOSE(BSL_LS_SOC_OAM,
                                (BSL_META_U(unit, "Invalid event size : %d \n"), rx_report_data.event_size));
                    SOC_SAND_SET_ERROR_CODE(ARAD_PP_OAM_INTERNAL_ERROR, 53, exit);
                }
                field_32 = 0;
                SHR_BITCOPY_RANGE(&field_32, 0, interrupt_data->interrupt_message, start_of_message+20, 1);
                ...
                if (rx_report_data.event_type == 1 ) {
                    if (SOC_IS_QAX(unit)) {
                        ...
                        dm_info.entry_id = *rmeb_db_ndx;
                        SOC_SAND_SOC_IF_ERROR_RETURN(res, 43, exit, soc_qax_pp_oam_oamp_dm_get(unit, &dm_info, &is_1dm));
                    }
                    ...
                    if (rx_report_event_mode[unit] == SOC_PPC_OAM_REPORT_MODE_COMPACT) {
                        if (SOC_IS_QAX(unit)) {
                            if (is_1dm) {
                                ...
                                event_buffer[0] = (event_buffer[1] & 0xFFFFF) << 12;
                                event_buffer[1] = (event_buffer[1] & 0x1FF00000) >> 20;
                            } else {
                                
                                event_buffer[1] = 0;
                                SHR_BITCOPY_RANGE(&event_buffer[1], 0, interrupt_data->interrupt_message, start_of_message+23, 31 - 23 + 1);
                                event_buffer[1] = (event_buffer[1] << 2) + (event_buffer[0]>>30);
                                event_buffer[0] = (event_buffer[0] & 0x1FFFFFFF) << 2;
                            }
                        } else {
                    ...
                    } else if (rx_report_event_mode[unit] == SOC_PPC_OAM_REPORT_MODE_NORMAL) {
                        if (SOC_IS_QAX(unit) && is_1dm) {
                            
                            event_buffer[0] = event_buffer[2];
                            event_buffer[1] = event_buffer[3];
                        }
                    }
  ...
  )
{
    ...
    for (i=0 ; i<20/2; i++) {
        temp= pkt_header[20-(i+1)];
        pkt_header[20-(i+1)]= pkt_header[i];
        pkt_header[i]= temp;
    }
...
{ 
    ...
    soc_reg_t sticky_fields[] = {STICKY_PORT_ERR_PACKETf, STICKY_PORT_CHAN_TYPE_MISSf, STICKY_PORT_TYPE_MISSf, STICKY_PORT_EMC_MISSf, STICKY_PORT_MAID_MISSf,
        ...
        STICKY_PORT_SRC_PORT_MISSf ,

		STICKY_PORT_RMEP_STATE_CHANGEf, STICKY_PORT_PARITY_ERRORf, STICKY_PORT_TIMESTAMP_MISSf,

        STICKY_PORT_TRAP_CODE_MISSf};
    char *stikcy_port_msg[] = {"error packet", "channel type miss", "type miss", "emc miss", "maid miss", "crc miss", "mdl miss", "ccm interval miss", "my discriminator miss","error src IP miss" , "error your discriminator miss"  ,
        "source port miss", "RMEP state change", "parity error", "time stamp miss", "trap miss"};
#else
    ...
    char *stikcy_port_msg[] = {"error packet", "trap miss", "type miss", "emc miss", "maid miss", "mdl miss", "ccm interval miss", "my discriminator miss","error src IP miss" , "error your discriminator miss"  ,
        "source port miss", "RMEP state change", "parity error", "time stamp miss"};
#endif
    uint8 ll_found;
