...
/*******************************************
* @function _cmicm_sbusdma_reg_dpc
 ...
 */
STATIC void
_cmicm_sbusdma_reg_event_dpc(void *unit, void *event_id, void *cmc, void *chan,
                             void *p5)
{
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META_U(PTR_TO_INT(unit),
                          "Generate I/O error event for ID%d\n"),
               PTR_TO_INT(event_id)));

    soc_event_generate(PTR_TO_INT(unit), SOC_SWITCH_EVENT_IO_ERROR,
...
retry:
    ...
    if (rv != SOC_E_NONE) {
        if (rv != SOC_E_TIMEOUT) {
            ...
            sal_dpc(_cmicm_sbusdma_reg_event_dpc, INT_TO_PTR(unit),
