...
static int
pci_hot_swap_debug_dump(int unit)
{
    int rv = SOC_E_NONE;
    uint32 rval = 0;
    uint32 val = 0;

    if (soc_feature(unit, soc_feature_cmicm) && SOC_IS_MONTEREY(unit)) {
        for(val = 0; val < 8; val++) {
            SOC_IF_ERROR_RETURN(READ_PAXB_0_PAXB_HOTSWAP_DEBUG_CTRLr(unit, &rval));
            soc_reg_field_set(unit, PAXB_0_PAXB_HOTSWAP_DEBUG_CTRLr, &rval,
                          DEBUG_FSM_FLIST_SELf, val);
            SOC_IF_ERROR_RETURN(WRITE_PAXB_0_PAXB_HOTSWAP_DEBUG_CTRLr(unit, rval));
            SOC_IF_ERROR_RETURN(READ_PAXB_0_PAXB_HOTSWAP_DEBUG_STATr(unit, &rval));
            LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                "DEBUG_FSM_FLIST_SELf = 0x%x, PAXB_0_PAXB_HOTSWAP_DEBUG_STATr = 0x%x\n"),
                        val, rval));
        }
        SOC_IF_ERROR_RETURN(READ_AXI_PCIE_S0_IDM_IDM_RESET_STATUSr(unit, &rval));
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                "AXI_PCIE_S0_IDM_IDM_RESET_STATUSr = 0x%x\n"), rval));
        SOC_IF_ERROR_RETURN(READ_CMICD_M0_IDM_IDM_RESET_STATUSr(unit, &rval));
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                "CMICD_M0_IDM_IDM_RESET_STATUSr = 0x%x\n"), rval));
        SOC_IF_ERROR_RETURN(READ_CMICD_S0_IDM_IDM_RESET_STATUSr(unit, &rval));
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                "CMICD_S0_IDM_IDM_RESET_STATUSr = 0x%x\n"), rval));
        ...
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                "AXI_PCIE_S0_IDM_IDM_RESET_STATUSr = 0x%x\n"), rval));
        SOC_IF_ERROR_RETURN(READ_CMICX_M0_IDM_IDM_RESET_STATUSr(unit, &rval));
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                "CMICX_M0_IDM_IDM_RESET_STATUSr = 0x%x\n"), rval));
        SOC_IF_ERROR_RETURN(READ_CMICX_S0_IDM_IDM_RESET_STATUSr(unit, &rval));
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                "CMICX_S0_IDM_IDM_RESET_STATUSr = 0x%x\n"), rval));
    }
    ...
    if (hot_swap_cause != 0 || fsm_state == 0x3 /* DUMMYRESP */) {
        ...
        if ((flags & SOC_PCIE_HOTSWAP_HANDLING_FLAG_DONOT_ABORT) == 0) {
            ...
            if (SOC_FAILURE(rv)) {
                ...
                (void)pci_hot_swap_debug_dump(unit);
        ...
        if (SOC_REG_FIELD_VALID(unit, PAXB_0_PAXB_HOTSWAP_STATr, PAXB_RDYf)) {
            ...
            /* Check if time out has happened before "ready" is asserted */
            if (ready == 0) {
                LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                    "Hot Swap Wait Time Out for READY: %d usecs\n"),
                    SAL_USECS_SUB(etime, stime)));
                /* Debug information */
                (void)pci_hot_swap_debug_dump(unit);
                rv = SOC_E_TIMEOUT;
 ...
 * Purpose:
 *      Disable/enable hot swap manager.
 * Parameters:
 *      unit - unit #
 * Returns:
 *      SOC_E_XXX
*/
int
soc_pcie_hot_swap_disable(int unit, int mode)
{
        ...
        if (soc_cm_get_bus_type(unit) & SOC_PCI_DEV_TYPE) {
            if (SOC_REG_IS_VALID(unit, PAXB_0_PAXB_HOTSWAP_CTRLr) &&
                ...
                if (mode) {
                    ...
                    /* Disable reset to propogate to paxb data path logic when pcie link is down. */
                    SOC_IF_ERROR_RETURN
                    ...
                    SOC_IF_ERROR_RETURN
                        (WRITE_PAXB_0_RESET_ENABLE_IN_PCIE_LINK_DOWNr(unit, rval));
                    LOG_INFO(BSL_LS_SOC_COMMON,
                        (BSL_META_U(unit, "PCIE HOTSWAP Manager is enabled!\n")));
                    rv = SOC_E_NONE;
                }
            }
        }
