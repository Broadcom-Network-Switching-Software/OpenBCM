...
_soc_cmicx_schan_err_handle(int unit, int schanCtrl, int ch, uint32 flags)
{
    ...
    if (schanCtrl & SC_CHx_MSG_TIMEOUT_TST) {
    ...
    } else if ((schanCtrl & SC_CHx_MSG_SCHAN_ERR) && soc_feature(unit, soc_feature_schan_err_check)) {
        ...
        if (schan_err & SC_CHx_SCHAN_ERR_ERR_BIT) {
            switch(err_code)
            {
                ...
                case 1:
                    ...
                    {
                        LOG_ERROR(BSL_LS_SOC_SCHAN, (BSL_META_U(unit,
                            "  allowed block timeout: Continue.\n")));
                    }
                    rv = SOC_E_TIMEOUT;
                    break;
                case 2:
                    LOG_ERROR(BSL_LS_SOC_SCHAN, (BSL_META_U(unit, "ECC error.\n")));
                    rv = SOC_E_FAIL;
                    break;
                default:
                    LOG_ERROR(BSL_LS_SOC_SCHAN, (BSL_META_U(unit, "Unexpected error.\n")));
                    rv = SOC_E_FAIL;
            }
        }
        ...
        if ((flags & SCHAN_OP_FLAG_DO_NOT_LOG_TIMEOUTS) == 0)
        {
                LOG_ERROR(BSL_LS_SOC_SCHAN, (BSL_META_U(unit,"CMIC_SCHAN_ERR data dump: "
                        "err_code=%u, data_len=%u, src_port=%u, dst_port=%u, op_code=%u Full reg value=0x%x\n"),
                        err_code, data_len, src_port, dst_port, op_code, schan_err));
        }
