...
soc_mem_test_skip(int unit, soc_mem_t mem, int index)
{
    ...
    if (soc_feature(unit, soc_feature_field_stage_half_slice)
        ...
        if (soc_feature(unit, soc_feature_field_stage_egress_3k_half_slice)) {
            if (mem == EFP_TCAMm || mem == EFP_TCAM_PIPE0m) {
                if ((index / 1536) % 2) {
                    return TRUE;
                }
        ...
        if (soc_feature(unit, soc_feature_field_stage_lookup_256_half_slice)) {
            if (mem == VFP_TCAMm || mem == VFP_TCAM_PIPE0m) {
                if ((index / 64) % 2) {
                    return TRUE;
                }
            }
                         ...
                         int index, ser_test_data_t *test_data)
{
    ...
    sal_strlcpy(test_data->mem_name, SOC_MEM_NAME(unit, test_data->mem), sizeof(test_data->mem_name));
    /* coverity[secure_coding] */
    sal_strlcpy(test_data->field_name,
               _SER_TEST_MEM_SOC_FIELD_NAME(unit, test_data->test_field), sizeof(test_data->field_name));
#else
                     ...
                     (test_data->parity_enable_reg == MMU_MB_TCB_ENABLE_ECCP_MEMr)) {
                        ...
                        for (i = 0; i < NUM_PIPE(unit); i++) {
                            rv = soc_td3_ser_reg_field32_modify(
                               ...
                               (test_data->mem == TDM_CALENDAR0m ||
                                test_data->mem == TDM_CALENDAR0_PIPE0m ||
                                test_data->mem == TDM_CALENDAR0_PIPE1m ||
                                test_data->mem == TDM_CALENDAR1m ||
                                test_data->mem == TDM_CALENDAR1_PIPE0m ||
                                test_data->mem == TDM_CALENDAR1_PIPE1m)){
                        rv = soc_reg_field32_modify(unit,
                                                    test_data->parity_enable_reg,
                                                    REG_PORT_ANY,
                                                    test_data->parity_enable_field,
                                                    enable);
                        rv = soc_reg_field32_modify(unit,
