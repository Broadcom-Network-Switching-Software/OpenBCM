...
soc_wb_engine_restore(int unit, int engine_id, uint32 buffer_id, uint16 recovered_ver, int *var_ndx)
{
    ...
    if (buffer->version < recovered_ver) {
        ...
        LOG_ERROR(BSL_LS_SOC_COMMON,
            (BSL_META_U(unit, "Downgrade scenario is not supported\n")));
...
int soc_wb_engine_init_buffer(int unit, int engine_id, int buffer_id, uint8 is_reinit)
{
    ...
    if (SOC_WARM_BOOT(unit) &&
        ...
        if (SOC_FAILURE(rc)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
                                  "soc_wb_engine_scache_ptr_get() failed: %s\n"), soc_errmsg(rc)));
            sal_free(var_ndx);
            return rc;
        }
        ...
        if (SOC_FAILURE(rc)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
                                  "soc_wb_engine_calculate_offsets() failed: %s\n"), soc_errmsg(rc)));
            sal_free(var_ndx);
            return rc;
        }
        ...
        if (SOC_FAILURE(rc)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
                                  "soc_wb_engine_restore() failed: %s\n"), soc_errmsg(rc)));
            sal_free(var_ndx);
            sal_free(tmp_scache_copy);
            return rc;
        }
        ...
        if (version != recovered_ver) {
            ...
            if (is_only_copy_workaround) {
                ...
                if (SOC_FAILURE(rc)) {
                    LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      "_wb_engine_var_info_header_update() failed: %s\n"), soc_errmsg(rc)));
                    sal_free(var_ndx);
                    sal_free(tmp_scache_copy);
                    return rc;
                }
