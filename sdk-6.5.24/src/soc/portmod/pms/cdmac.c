                    ...
                    portmod_encap_t encap)
{
    ...
    switch(encap){
        ...
        /* 4 Bytes Header (K.SOP + 2 byte Preamble + SFD) */
            val = 6;
            break;
        default:
            _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
...
int cdmac_tx_threshold_set(int unit, soc_port_t port, int value)
{
    ...
    _SOC_IF_ERR_EXIT(READ_CDMAC_TX_CTRLr(unit, port, &rval));
    soc_reg_field_set(unit, CDMAC_TX_CTRLr, &rval, TX_THRESHOLDf, value);
    _SOC_IF_ERR_EXIT(WRITE_CDMAC_TX_CTRLr(unit, port, rval));
...
int cdmac_tx_threshold_get(int unit, soc_port_t port, int *value)
{
    ...
    _SOC_IF_ERR_EXIT(READ_CDMAC_TX_CTRLr(unit, port, &rval));
    *value = soc_reg_field_get(unit, CDMAC_TX_CTRLr, rval, TX_THRESHOLDf);
...
int cdmac_rsv_mask_control_set(int unit, int port, uint32 flags, uint32 value)
{
    ...
    _SOC_IF_ERR_EXIT(cdmac_rsv_mask_get(unit, port, rsv_mask_bmap));
    ...
    _SOC_IF_ERR_EXIT(cdmac_rsv_mask_set(unit, port, rsv_mask_bmap[0]));
                            ...
                            portmod_port_fault_status_t *faults)
{
    ...
    faults->local = soc_reg_field_get(unit, CDMAC_RX_LSS_STATUSr, rval,
                                      LOCAL_FAULT_STATUSf);

    faults->remote = soc_reg_field_get(unit, CDMAC_RX_LSS_STATUSr, rval,
                                       REMOTE_FAULT_STATUSf);
exit:
    SOC_FUNC_RETURN;
}

int cdmac_drop_tx_data_on_ordered_set_enable(int unit, int port, uint32 enable)
{
    uint32 rval;
    SOC_INIT_FUNC_DEFS;
    ...
    _SOC_IF_ERR_EXIT(READ_CDMAC_RX_LSS_CTRLr(unit, port, &rval));
    if (enable) {
        soc_reg_field_set(unit, CDMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_LINK_INTERRUPTf, 1);
        soc_reg_field_set(unit, CDMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_REMOTE_FAULTf, 1);
        soc_reg_field_set(unit, CDMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_LOCAL_FAULTf, 1);
    } else {
        /*Set to the default of  HW when disable*/
        soc_reg_field_set(unit, CDMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_LINK_INTERRUPTf, 0);
        soc_reg_field_set(unit, CDMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_REMOTE_FAULTf, 1);
        soc_reg_field_set(unit, CDMAC_RX_LSS_CTRLr, &rval, DROP_TX_DATA_ON_LOCAL_FAULTf, 1);
    }
    _SOC_IF_ERR_EXIT(WRITE_CDMAC_RX_LSS_CTRLr(unit, port, rval));
