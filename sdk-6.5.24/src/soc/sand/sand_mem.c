        ...
        uint32 *value)
{
    ...
    int cmic_blk;
    int entry_size;
    int max_words = (soc_feature(unit, soc_feature_sbus_width_120B) && !SOC_IS_J2P(unit)) ? 30 : 20;
    int rv = 0;
    ...
    if (entry_size > max_words)
    {
        uint32 val = 1;
        SHR_IF_ERR_EXIT(soc_direct_reg_set(unit, cmic_blk, reg_wide, 1, &val));
    }

    for (i = 0; i < entry_size / max_words + 1; i++)
    {
        SHR_IF_ERR_EXIT(soc_direct_reg_set(unit, cmic_blk, reg_data, max_words, value));
    }
