       ...
       int *db_id)
{
    ...
    if ( mdb_type == EM_MDB_TYPE && memory_id == dnx_data_mdb.pdbs.adapter_mapping_crps_id_get(unit))
    {
...
sand_signals_adapter_get_interface_index(
    ...
    char *res)
{
    int interface_index, caller_id_key;
    stage_t STAGE_INFO;
    SHR_FUNC_INIT_VARS(unit);
    ...
    
    {
        ...
        if(sal_strncmp(dnx_data_adapter.mdb.lookup_caller_id_mapping_get(unit, caller_id_key, stage_id)->interface,
                        "NotFound", sal_strlen("NotFound")) == 0)
        {
            sal_sprintf(res, "%s", "SkipSignal");
            SHR_EXIT();
        }
        else if (dnx_data_adapter.mdb.lookup_caller_id_mapping_get(unit, caller_id_key, stage_id)->mdb != db_id)
        {
            sal_sprintf(res, "%s", "SkipSignal");
            SHR_EXIT();
        }
        else if(sal_strncmp(dnx_data_adapter.mdb.lookup_caller_id_mapping_get(unit, caller_id_key, stage_id)->interface,
        ...
        {
            sal_sprintf(res, "%s", dnx_data_adapter.mdb.lookup_caller_id_mapping_get(unit, caller_id_key, stage_id)->interface);
            SHR_EXIT();
        }
    }
    ...
    pp_stage_t *cur_pp_stage)
{
    ...
    int db_id = 0;
    uint32 app_db_size = 0, is_app_db_part_of_key = 0, app_db = 0;
    ...
    while ( data_index > 0)
    {
        ...
        if (sand_signals_adapter_get_interface_index(unit, stage_and_intrf/1000, stage_and_intrf%1000, memory_id,
                                         db_id, cur_pp_stage->pp_id, cur_pp_stage->name, interface_index) != _SHR_E_NONE )
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Interface was not found. stage: %s, db: %s, stage_and_intrf: %d, memory_id: %d\n",
                                                                    cur_pp_stage->name, db_name, stage_and_intrf, memory_id);
        }
