...
dbal_logical_table_t * table_info)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    dbal_init_table_db_struct_clear(cur_table_param);
    SHR_IF_ERR_EXIT(dbal_db_init_general_info_set(unit, table_entry, DBAL_TABLE_ETM_PP_DESCRIPTOR_EXPANSION, is_valid, "ETM_PP_DESCRIPTOR_EXPANSION", DBAL_ACCESS_METHOD_MDB));
    table_entry->maturity_level = DBAL_MATURITY_HIGH;
    table_entry->table_type = DBAL_TABLE_TYPE_EM;
    table_entry->nof_labels = 3;
    DBAL_DB_INIT_TABLE_LABELS_ALLOC(table_entry)
    ...
    dbal_db_init_table_db_struct_access_clear(cur_table_param, table_entry);
    {
        ...
        uint8 is_compatible_with_all_images = TRUE;
        DBAL_DB_INIT_TABLE_IMAGE_SPECIFIC_TABLE_INDICATION_SET(table_entry, is_standard_1, is_compatible_with_all_images);
        SHR_IF_ERR_EXIT(dbal_db_init_table_set_table_incompatible_image_sw_state_indication(unit, DBAL_TABLE_ETM_PP_DESCRIPTOR_EXPANSION, is_standard_1, is_compatible_with_all_images));
    }
    table_entry->core_mode = DBAL_CORE_MODE_SBC;
