...
uint32 * offset)
{
    dbal_entry_handle_t *  eh = (dbal_entry_handle_t *) entry_handle;
    uint32 key_value__in_lif_profile;
    uint32 result_size__cs_in_lif_vtt_to_vtt_profile = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_IN_LIF_PROFILE, key_value__in_lif_profile);
    result_size__cs_in_lif_vtt_to_vtt_profile += 4;
    *offset = key_value__in_lif_profile%128*result_size__cs_in_lif_vtt_to_vtt_profile;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
...
uint32 * offset)
{
    dbal_entry_handle_t *  eh = (dbal_entry_handle_t *) entry_handle;
    uint32 key_value__in_lif_profile;
    uint32 result_size__cs_in_lif_flp_to_flp_profile = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_IN_LIF_PROFILE, key_value__in_lif_profile);
    result_size__cs_in_lif_flp_to_flp_profile += 4;
    *offset = key_value__in_lif_profile%128*result_size__cs_in_lif_flp_to_flp_profile;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
