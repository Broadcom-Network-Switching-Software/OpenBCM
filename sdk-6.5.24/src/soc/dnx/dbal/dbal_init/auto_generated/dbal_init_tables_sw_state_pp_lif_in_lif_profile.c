...
shr_error_e
_dbal_init_tables_sw_state_pp_lif_in_lif_profile_cs_lif_profile_flags_per_gport_sw_init(
int unit,
table_db_struct_t * cur_table_param,
dbal_logical_table_t * table_info)
{
    dbal_logical_table_t * table_entry = table_info + DBAL_TABLE_CS_LIF_PROFILE_FLAGS_PER_GPORT_SW;
    int is_valid = TRUE;
    SHR_FUNC_INIT_VARS(unit);
    dbal_init_table_db_struct_clear(cur_table_param);
    SHR_IF_ERR_EXIT(dbal_db_init_general_info_set(unit, table_entry, DBAL_TABLE_CS_LIF_PROFILE_FLAGS_PER_GPORT_SW, is_valid, "CS_LIF_PROFILE_FLAGS_PER_GPORT_SW", DBAL_ACCESS_METHOD_SW_STATE));
    ...
    }
    {
        ...
        DBAL_DB_INIT_TABLE_INTERFACE_RESULT_FIELDS_ALLOC(table_entry, table_entry->multi_res_info[result_type_counter]);
        {
            ...
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_mandatory_values(unit, DBAL_FIELD_CS_LIF_PROFILE_FLAGS, DBAL_FIELD_TYPE_DEF_CS_LIF_PROFILE_FLAGS, dbal_db_field));
            dbal_db_init_table_field_params_init(&db_field);
        ...
        {
            dbal_db_field = &table_entry->multi_res_info[result_type_counter].results_info[field_index];
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_mandatory_values(unit, DBAL_FIELD_CS_LIF_PROFILE_NAMESPACE, DBAL_FIELD_TYPE_DEF_UINT, dbal_db_field));
            dbal_db_init_table_field_params_init(&db_field);
            db_field.size = 3;
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_optional_or_default_values(unit, table_entry, dbal_db_field, &db_field, FALSE));
            field_index += dbal_db_field->nof_instances;
    ...
    dbal_db_init_table_db_struct_access_clear(cur_table_param, table_entry);
    {
        ...
        SHR_IF_ERR_EXIT(dbal_db_init_table_set_table_incompatible_image_sw_state_indication(unit, DBAL_TABLE_CS_LIF_PROFILE_FLAGS_PER_GPORT_SW, is_standard_1, is_compatible_with_all_images));
    }
    ...
    dbal_db_init_sw_state_set_indirect_table_size(unit, table_entry, cs_lif_profile_flags_per_gport_sw_indirect_table_size_0_cb, DBAL_DB_INVALID);
    SHR_IF_ERR_EXIT(dbal_db_init_table_add(unit, cur_table_param, DBAL_TABLE_CS_LIF_PROFILE_FLAGS_PER_GPORT_SW, table_info));
exit:
    SHR_FUNC_EXIT;
}
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(_dbal_init_tables_sw_state_pp_lif_in_lif_profile_cs_lif_profile_flags_per_gport_sw_init(unit, cur_table_param, table_info));
exit:
    SHR_FUNC_EXIT;
}
