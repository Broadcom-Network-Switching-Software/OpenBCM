    ...
    int unit)
{
    ...
    for (table_id = 0; table_id < DBAL_NOF_TABLES; table_id++)
    {
        ...
        if ((table->access_method != DBAL_ACCESS_METHOD_PEMLA) || (table->nof_key_fields > 0))
        {
            continue;
        }
...
{
    dbal_logical_table_t *table;
    pemladrv_db_info *pemla_mem_info;
...
exit:
    if (pemla_mem_info)
    {
        SHR_IF_ERR_EXIT(dnx_pemladrv_direct_info_de_allocate(unit, pemla_mem_info));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT(pemladrv_direct_write
                    (unit, entry_handle->table->pemla_mapping.reg_mem_id, entry_handle->phy_entry.key[0],
                     pemla_drv_mem));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT(dbal_pemla_table_pemla_set_default_entry(unit, entry_handle, &pemla_drv_mem));
    SHR_IF_ERR_EXIT(pemladrv_direct_write(unit, entry_handle->table->pemla_mapping.reg_mem_id,
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    sal_memcpy(entry_handle->phy_entry.payload, pemla_mem_result->fields[0]->fldbuf,
               sizeof(pemla_mem_result->fields[0]->fldbuf[0]));
