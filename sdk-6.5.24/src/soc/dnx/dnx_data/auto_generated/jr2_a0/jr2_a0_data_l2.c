    ...
    int unit)
{
    ...
    if ((uint8)l2AddrNA < table->keys[0].size && (uint8)l2AddrMactSvl < table->keys[1].size)
    {
        data = (dnx_data_l2_general_mact_result_type_map_t *) dnxc_data_mgmt_table_data_get(unit, table, (uint8)l2AddrNA, (uint8)l2AddrMactSvl);
    ...
    if ((uint8)l2AddrNA < table->keys[0].size && (uint8)l2AddrMactIvl < table->keys[1].size)
    {
        data = (dnx_data_l2_general_mact_result_type_map_t *) dnxc_data_mgmt_table_data_get(unit, table, (uint8)l2AddrNA, (uint8)l2AddrMactIvl);
    ...
    int unit)
{
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_l2_general_fwd_mact_info_t, (1 * (table->keys[0].size) + 1  ), "data of dnx_data_l2_general_table_fwd_mact_info");

    
    default_data = (dnx_data_l2_general_fwd_mact_info_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->data_base = DBAL_TABLE_EMPTY;
    
    for (data_base_id_index = 0; data_base_id_index < table->keys[0].size; data_base_id_index++)
    {
        data = (dnx_data_l2_general_fwd_mact_info_t *) dnxc_data_mgmt_table_data_get(unit, table, data_base_id_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    
    if ((uint8)l2AddrFwdMact < table->keys[0].size)
    {
        data = (dnx_data_l2_general_fwd_mact_info_t *) dnxc_data_mgmt_table_data_get(unit, table, (uint8)l2AddrFwdMact, 0);
        data->data_base = DBAL_TABLE_FWD_MACT;
    }
    if ((uint8)l2AddrLearnMact < table->keys[0].size)
    {
        data = (dnx_data_l2_general_fwd_mact_info_t *) dnxc_data_mgmt_table_data_get(unit, table, (uint8)l2AddrLearnMact, 0);
        data->data_base = DBAL_TABLE_FWD_MACT;
    }
