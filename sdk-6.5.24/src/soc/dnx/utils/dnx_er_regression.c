...
extern err_recovery_manager_t
    dnx_err_recovery_manager[SOC_MAX_NUM_DEVICES][DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION];
...
dnx_er_regress_is_top_level_api(
    ...
    int er_thread_id)
{
    return (1 == dnx_err_recovery_manager[unit][er_thread_id].api_counter);
}
    ...
    int er_thread_id)
{
    ...
    uint32 trans_num_before_api = dnx_err_recovery_manager[unit][er_thread_id].api_to_trans_map[0];

    return (dnx_err_recovery_manager[unit][er_thread_id].transaction_counter == (trans_num_before_api + 1));
}
...
dnx_er_regress_is_outside_of_transaction(
    ...
    int er_thread_id)
{
    return (0 == dnx_err_recovery_manager[unit][er_thread_id].transaction_counter);
}
    ...
    int er_thread_id)
{
    return (er_thread_id != -1
            ...
            && dnx_er_regress_is_top_level_api(unit, er_thread_id)
            && dnx_er_regression[unit].is_appl_init_done
            && dnx_er_regression[unit].is_testing_enabled
            && (!dnx_er_regression[unit].trans_info[er_thread_id].is_api_excluded));
}
    ...
    int er_thread_id)
{
    ...
    if (dnx_er_regress_testing_conditions_are_met(unit, er_thread_id)
        && dnx_er_regress_is_outside_of_transaction(unit, er_thread_id))
    {
        dnx_er_regression[unit].trans_info[er_thread_id].has_access_outside_trans = TRUE;

        if (0 != dnx_er_regression[unit].trans_info[er_thread_id].top_level_trans_in_api_count)
        {
            LOG_CLI((BSL_META_U
...
{
    return (dnx_er_regression[unit].trans_info[er_thread_id].err_type_suppress_counters[type] > 0);
}
...
dnx_err_recovery_is_top_level_trans_in_top_level_api(
    ...
    int er_thread_id)
{
    return (dnx_er_regress_is_top_level_api(unit, er_thread_id)
            && dnx_er_regress_is_top_level_api_transaction(unit, er_thread_id));
}
...
dnx_er_regress_is_first_test_iteration(
    ...
    int er_thread_id)
{
    return (dnx_er_regress_testing_conditions_are_met(unit, er_thread_id)
            && dnx_err_recovery_is_top_level_trans_in_top_level_api(unit, er_thread_id)
            && (!dnx_er_regression[unit].trans_info[er_thread_id].is_api_ran));
}
    ...
    uint8 is_begin)
{
    if (is_begin)
    {
        if (dnx_er_regress_is_first_test_iteration(unit, er_thread_id))
        {
            LOG_CLI((BSL_META_U(unit, "**** ER BCM API %s **** ERROR SIMULATION RUN ****\n"),
                     dnx_er_regression[unit].trans_info[er_thread_id].dnx_er_regress_testing_api_name));
        }
        else if (dnx_er_regress_is_second_test_iteration(unit, er_thread_id))
        {
    ...
    {
        if (dnx_er_regress_is_second_test_iteration(unit, er_thread_id))
        {
    ...
    int er_thread_id)
{
    ...
    {
        dnx_er_regression[unit].trans_info[er_thread_id].err_type_suppress_counters[idx] = 0;
    }

    dnx_er_regression[unit].is_appl_init_done = FALSE;

    dnx_er_regression[unit].trans_info[er_thread_id].is_api_excluded = FALSE;
    dnx_er_regression[unit].is_testing_enabled = FALSE;
    dnx_er_regression[unit].trans_info[er_thread_id].has_access_outside_trans = FALSE;
    ...
    int unit)
{
    ...
    {
        dnx_er_regress_init_deinit_counters_reset(unit, er_thread_id);
    }
    ...
    int er_thread_id)
{
    ...
    dnx_er_regression[unit].trans_info[er_thread_id].top_level_trans_in_api_count = 0;
    dnx_er_regression[unit].trans_info[er_thread_id].is_api_excluded = FALSE;
    dnx_er_regression[unit].trans_info[er_thread_id].is_api_ran = FALSE;
    dnx_er_regression[unit].trans_info[er_thread_id].has_access_outside_trans = FALSE;
    ...
    int er_thread_id)
{
    ...
    if (dnx_er_regression[unit].trans_info[er_thread_id].has_access_outside_trans)
    {
        LOG_CLI((BSL_META_U
                 (unit, "#### #### #### #### ####"
    ...
    uint8 is_top_level_transaction)
{
    ...
    if (is_top_level_transaction && dnx_er_regress_testing_conditions_are_met(unit, er_thread_id))
    {
        SHR_IF_ERR_EXIT(dnx_er_regress_transaction_start_top_level_verify(unit, er_thread_id));
    }
    ...
    {
        dnx_er_regress_print_api_and_iteration(unit, er_thread_id, TRUE);
    }

    if (dnx_err_recovery_is_top_level_trans_in_top_level_api(unit, er_thread_id))
    {
        dnx_er_regression[unit].trans_info[er_thread_id].top_level_trans_in_api_count++;

        if (dnx_er_regress_testing_conditions_are_met(unit, er_thread_id)
            && (dnx_er_regression[unit].trans_info[er_thread_id].top_level_trans_in_api_count > 1))
        {
            LOG_CLI((BSL_META_U
                     (unit,
...
dnx_er_regress_transaction_end_should_abort(
    ...
    int er_thread_id)
{
    return dnx_er_regress_is_first_test_iteration(unit, er_thread_id);
}
    ...
    int er_thread_id)
{
    ...
    if (dnx_data_dev_init.ha.error_recovery_support_get(unit) == DNX_ER_DEBUG_MODE)
    {
        dnx_er_regress_print_api_and_iteration(unit, er_thread_id, FALSE);
    }
    ...
    const char *func_name)
{
    ...
    if (dnx_er_regress_testing_conditions_are_met(unit, er_thread_id))
    {
        sal_strncpy(dnx_er_regression[unit].trans_info[er_thread_id].dnx_er_regress_testing_api_name, func_name,
    ...
    int unit)
{
    ...
    if ((dnx_er_regress_testing_conditions_are_met(unit, er_thread_id)
         && (1 == dnx_er_regression[unit].trans_info[er_thread_id].top_level_trans_in_api_count)
         && (!dnx_er_regression[unit].trans_info[er_thread_id].is_api_ran)))
    {

        dnx_er_regression[unit].trans_info[er_thread_id].is_api_ran = TRUE;
        dnx_er_regression[unit].trans_info[er_thread_id].top_level_trans_in_api_count = 0;
    ...
    int unit)
{
    ...
    if ((er_thread_id != -1) && dnx_er_regress_testing_conditions_are_met(unit, er_thread_id)
        && dnx_er_regress_is_outside_of_transaction(unit, er_thread_id))
    {
        dnx_er_regression[unit].trans_info[er_thread_id].is_api_excluded = TRUE;
    }
}
    ...
    int unit)
{
    ...
    else
    {
        LOG_CLI((BSL_META_U
                 (unit,
                  "error recovery regression ERROR: attempted 'is_api_excluded' flag reset, without a valid er_thread_id.\n")));
        assert(0);
    }
}
    ...
    uint8 *ptr)
{
    ...
    if ((er_thread_id != -1) && dnx_er_regress_is_first_test_iteration(unit, er_thread_id))
    {
    ...
    uint8 is_on)
{
    ...
    if (!is_on)
    {
        dnx_er_regression[unit].trans_info[er_thread_id].err_type_suppress_counters[type]++;
    }
    else
    {
        if (dnx_er_regression[unit].trans_info[er_thread_id].err_type_suppress_counters[type] == 0)
        {
    ...
    int er_thread_id)
{
    if (dnx_er_regression[unit].is_testing_enabled
        && !dnx_err_recovery_regress_is_err_type_suppressed(unit, er_thread_id,
                                                            DNX_ERR_RECOVERY_REGRESS_ERROR_SW_ACCESS_OUTSIDE_TRANS))
    {
        dnx_er_regress_system_state_modified(unit, er_thread_id);
    }
}
