...
/* { */
/*
 * When compiling for Valgrind, do not use 'calloc' since, in some cases, Vargrind does not recognize this
 * operation as a 'set' for specified memory.
 */
#define EXT_DEBUG_ALLOC(_sz, _flags)  \
    {  \
        ...
        if ((_flags & SAL_ALLOC_F_ZERO) == SAL_ALLOC_F_ZERO)  \
        {  \
            sal_memset(pp,0,_sz);  \
        }  \
        return pp;  \
...
/*
 * The following is code which is only activated when 'thread sanitizer' is embedded
 * in the SDK. On standard compilation, we do not ise it since it consumes too much real-time)
 */
#if (THREAD_SANITIZER_IS_ACTIVE == 1)
/* { */
static int thread_sanitizer_alloc_started = 0;
static pthread_mutex_t thread_sanitizer_alloc_mutex;
static void
thread_sanitizer_alloc_lock_mutex(void)
{
    if (!thread_sanitizer_alloc_started) {
        thread_sanitizer_alloc_started = 1;
        pthread_mutex_init(&thread_sanitizer_alloc_mutex, NULL);
    }
    pthread_mutex_lock(&thread_sanitizer_alloc_mutex);
}
/* } */
#endif
...
#define SAL_ALLOC_RESOURCE_USAGE_INCR(a_curr, a_max, a_size, ilock)     \
    thread_sanitizer_alloc_lock_mutex();                                \
    a_curr += (a_size);                                                 \
    a_max = ((a_curr) > (a_max)) ? (a_curr) : (a_max);                  \
    pthread_mutex_unlock(&thread_sanitizer_alloc_mutex)
/* } */
#else
/* { */
#define SAL_ALLOC_RESOURCE_USAGE_INCR(a_curr, a_max, a_size, ilock)     \
 ...
 *      Note that when compiling for Valgrind, 'calloc' is not used since Valgrind does
 *      not process it properly, in some cases. In that case, only 'malloc' is used.
 ...
 *      \ref SAL_ALLOC_F_ZERO - Allocate the memory with calloc. (Not for Valgrind)
...
/* { */
    ...
    if ((flags & SAL_ALLOC_F_ZERO) == SAL_ALLOC_F_ZERO)
    {
        sal_memset(p,0,alloc_sz);
    }
/* } */
#else
/* { */
