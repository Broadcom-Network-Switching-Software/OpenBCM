...
sal_spinlock_create(char *desc)
{
    ...
    if (sl != NULL) {
        ...
        sl->cpus = sysconf(_SC_NPROCESSORS_ONLN);
#else
        sl->cpus = 1;
#endif
        sl->desc = desc;
        sl->ts_max.tv_sec = 0;
        sl->ts_max.tv_nsec = (SECOND_USEC / sysconf(_SC_CLK_TCK)) * 1000;
    }
...
sal_spinlock_lock(sal_spinlock_t lock)
{
    ...
    int busy_retries = 0, sched_retries = 0;
    int rv;

    assert(sl);
    /*
     * On some systems pthread_spin_lock() would be unsafe due to the thread
     * schedule policy such as SCHED_FIFO, CPU affinity and so on which could
     * lead to deadlock situation. So pthread_spin_trylock() is used instead
     * to avoid this kind of issue.
     */
    while ((rv = pthread_spin_trylock(&sl->spinlock)) == EBUSY) {
        if (sl->cpus > 1) {
            if (busy_retries++ < 1000) {
                continue;
            }
            if (sched_retries++ < sl->cpus * 2) {
                sched_yield();
                continue;
            }
        }
        ts.tv_sec = 0;
        ts.tv_nsec *= 2;
        if (ts.tv_nsec > sl->ts_max.tv_nsec) {
            ts.tv_nsec = sl->ts_max.tv_nsec;
        }
        nanosleep(&ts, NULL);
    }
    return rv ? -1 : 0;
...
sal_spinlock_unlock(sal_spinlock_t lock)
{
    ...
    assert(sl);
    rv = pthread_spin_unlock(&sl->spinlock);
    return rv ? -1 : 0;
#else
