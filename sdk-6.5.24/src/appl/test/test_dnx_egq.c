...
static int
test_dnx_egq_global_egq_dbal_configurations(
    int unit,
    uint32 enable,
    int half_q_pairs)
{
    ...
    {
        rv = dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_SHAPER_GLOBAL_CONFIGURATION,
                                         1, 8,
        ...
        if (rv != 0)
        {
            LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't configure global configurations.\n")));
            return BCM_E_FAIL;
        }
        rv = soc_reg_above_64_set(unit, EPS_PS_MODEr, core, 0, reg_above_64_val);
        if (rv != 0)
        {
            LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't configure Port scheduler mode configurations.\n")));
            return BCM_E_FAIL;
        }
    }
    ...
    bcm_gport_t src_port)
{
    ...
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't get the params for source port.\n")));
        return BCM_E_FAIL;
    }
    rv = diag_sand_packet_create(unit, packet_header);
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't create packet header.\n")));
        /** make coverity happy */
        sal_free(*packet_header);
        return BCM_E_FAIL;
    }
    rv = diag_sand_packet_proto_add(unit,*packet_header, "PTCH_2");
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't set packet properties.\n")));
        return BCM_E_FAIL;
    }
    rv = diag_sand_packet_field_add_uint32(unit, *packet_header, "PTCH_2.PP_SSP", &mapping_info.pp_port, 16);
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't set packet field.\n")));
        return BCM_E_FAIL;
    }
    rv = diag_sand_packet_proto_add(unit, *packet_header, "ETH1");
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't set packet properties.\n")));
        return BCM_E_FAIL;
    ...
    uint32 pcp)
{
    ...
    rv = diag_sand_packet_field_add_uint32(unit, *packet_header, "ETH1.VLAN.PCP", &pcp, 3);
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't set packet field.\n")));
        return BCM_E_FAIL;
    }
    rv = diag_sand_packet_send(unit, src_port, *packet_header, SAND_PACKET_RESUME);
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't send packet.\n")));
        return BCM_E_FAIL;
    }
...
test_dnx_egq_qpair_test(int unit, args_t *a, void *pa)
{
    ...
    rv = ctest_dnxc_set_soc_properties(unit, ctest_soc_property, &ctest_soc_set_h);
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't configure the soc properties.\n")));
        return BCM_E_FAIL;
    }
#endif /** INCLUDE_CTEST */

    rv = test_dnx_egq_global_egq_dbal_configurations(unit, 0, half_q_pairs);
    if (rv != 0)
    {
        LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't configure ttest.\n")));
        return BCM_E_FAIL;
    }
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        rv = dnx_algo_port_egr_if_get(unit, nif_port, &interface);
        if (rv != 0)
        {
            LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't get egress interface.\n")));
            return BCM_E_FAIL;
        }

        rv = dnx_algo_port_priorities_nof_get(unit, nif_port, &nof_priorities);
        if (rv != 0)
        {
            LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't get nof priorities.\n")));
            return BCM_E_FAIL;
        }
        rv = bcm_port_force_forward_set(unit, cpu_port, nif_port, 1);
        if (rv != 0)
        {
            LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't configure force forward.\n")));
            return BCM_E_FAIL;
        }

        /** create packet structure that will be used for the test */
        rv = test_dnx_egq_create_packet(unit, &packet_header, cpu_port);
        if (rv != 0)
        {
            LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - creating the packet failed.\n")));
            return BCM_E_FAIL;
        }
        ...
        for (base_q_pair = 0; base_q_pair < nof_q_pairs; base_q_pair = base_q_pair + 8) {
            ...
            for(cosq  =  0; cosq  <  nof_priorities; cosq++) {
                rv = test_dnx_egq_send_packet(unit, &packet_header, cpu_port, cosq);
                if (rv != 0)
                {
                    LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - something wrong when sending a packet.\n")));
                    return BCM_E_FAIL;
                }
            }
            ...
            for(cosq  =  0; cosq  <  nof_priorities; cosq++) {
                ...
                rv = dnx_ecgm_dbal_queue_resources_statistics_max_get(unit, core_id, 0, base_q_pair + cosq, &pd, &db);
                if (rv != 0)
                {
                    LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't get statistics.\n")));
                    return BCM_E_FAIL;
                }
            ...
            rv = bcm_stat_get(unit, nif_port, snmpEtherStatsTXNoErrors, &tx_packet_count);
            if (rv != 0)
            {
                LOG_ERROR(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Error - couldn't get stats for packets.\n")));
                return BCM_E_FAIL;
            }
