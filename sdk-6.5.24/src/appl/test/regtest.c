...
/* Function used for testing a register in TR 3. Skip list for TR 3 included here */
STATIC int
 ...
 */
    ...
    if(SOC_IS_DNX(unit)) {
        ...
        switch(ainfo->reg) {
            ...
            case CDBM_INDIRECT_COMMANDr:
            case CDB_INDIRECT_COMMANDr:
            ...
            case OCBM_INDIRECT_COMMANDr:
            case OLP_INDIRECT_COMMANDr:
            case PEM_INDIRECT_COMMANDr:
            case PDM_INDIRECT_COMMANDr:
            ...
            case CDBM_GTIMER_CONFIGURATIONr:
            case CDB_GTIMER_CONFIGURATIONr:
            case CDPM_GTIMER_CONFIGURATIONr:
            ...
            case HBMC_GTIMER_CONFIGURATIONr:
            case HRC_GTIMER_CONFIGURATIONr:
            ...
            case MSD_GTIMER_CONFIGURATIONr:
            case MSS_GTIMER_CONFIGURATIONr:
            ...
            case OCBM_GTIMER_CONFIGURATIONr:
            ...
            /* J2P investigated */
            case SCH_SPECIAL_CLOCK_CONTROLSr:
            case TCAM_SPECIAL_CLOCK_CONTROLSr:
            case TCAM_TCAM_VTT_IF_SELECTr:
            case IPPB_REG_00C5r:
            case MACSEC_ESEC_FIPS_OVR_CONTROLr:
            case MACSEC_ISEC_FIPS_OVR_CONTROLr:
            case MACSEC_MACSEC_HW_RESET_CONTROLr:
            case MACSEC_MACSEC_MIB_MODEr:
               return 1; /* Skip these registers */
            default:
                break;
        }
...
static int
test_brdc_instance(int unit, int instance, int brdc_block, soc_block_types_t regblktype)
{
    soc_block_type_t blktype = SOC_REG_FIRST_BLK_TYPE(regblktype);
    int inst, blknum;
    ...
    switch (blktype)
    {
        case SOC_BLK_FMAC:
        {
            inst = dnx_data_fabric.blocks.fmac_sbus_chain_get(unit, instance)->chain;
            blknum = BRDC_FMAC_BLOCK(unit, inst);
            break;
        }
        case SOC_BLK_FSRD:
        {
            inst = dnx_data_fabric.blocks.fsrd_sbus_chain_get(unit, instance)->chain;
            blknum = BRDC_FSRD_BLOCK(unit,inst);
            break;
        }
        case SOC_BLK_HBC:
        {
            inst = dnx_data_dram.hbm.hbc_sbus_chain_get(unit, instance)->sbus_chain;
            blknum = BRDC_HBC_BLOCK(unit,inst);
            break;
        }
        default:
        {
        ...
        for (i = 0; i < brdc_count; i++) {
            ...
            for (reg = 0; reg < NUM_SOC_REG; reg++) {
                ...
                /* Check the size of  the register by counting the fields and storing the number in reg_size */
                    ...
                    LOG_INFO(BSL_LS_APPL_TESTS,
                            (BSL_META("Write using the ADDR reg %s.%s: value %u \n"),
                                SOC_REG_NAME(unit,reg), SOC_BLOCK_NAME(unit,brdc_block[i]), *reg_above_64));
                    if (rv < 0)
                    {
                         LOG_ERROR(BSL_LS_APPL_TESTS,
...
#ifdef BCM_DNX_SUPPORT
                        if (SOC_IS_J2P(unit) && !test_brdc_instance(unit, instance, brdc_block[i], regblktype))
                        {
                            continue;
                        }
#endif
                    ...
                    for (brdc_reg = 0; brdc_reg < NUM_SOC_REG; brdc_reg++) {
                        if (sal_strcmp(reg_name, SOC_REG_NAME(unit, brdc_reg)) == 0) {
                            blockNum = SOC_BLOCK_INFO(unit, brdc_block[i]).number;
                            rv = soc_reg_above_64_set(unit, brdc_reg, blockNum, 0 , reg_above_64);
                            if (rv < 0)
                            {
                                LOG_INFO(BSL_LS_APPL_TESTS,
                            ...
                            LOG_INFO(BSL_LS_APPL_TESTS,
                                (BSL_META("Write to BRDC reg %s: value %u, port %d \n"),
                                 reg_name, *reg_above_64, blockNum));
                            for (instance = 0; instance < SOC_BLOCK_BROADCAST_SIZE(unit, brdc_block[i]) ; ++instance) {
                                blk = SOC_BLOCK_BROADCAST_MEMBER(unit, brdc_block[i], instance);
                                if (!SOC_INFO(unit).block_valid[blk])
                                {
