    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 instance_num, enable_bmp[2], assert_bmp[2], valid_flags;
    ...
    if (out_info)
    {
        ...
        else if (sal_strncasecmp(out_info, "flags", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            out_format = 1;
        }
        else if (sal_strncasecmp(out_info, "storm", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            out_format = 2;
        }
        else if (sal_strncasecmp(out_info, "stat", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            out_format = 3;
        }
    }
    ...
    for (intr = 0; intr < nof_interrupts; intr++)
    {
        ...
        if (out_format == 0)
        {
            ...
            if (enable_flag)
            {
                if ((enable_bmp[0] == 0) && (enable_bmp[1] == 0))
                {
                    continue;
                }
            }
            if (assert_flag)
            {
                if ((assert_bmp[0] == 0) && (assert_bmp[1] == 0))
                {
                    continue;
                }
            }
...
/* } */
#endif
            PRT_CELL_SET("enable=0x%x/0x%x, assert=0x%x/0x%x", enable_bmp[0], enable_bmp[1], assert_bmp[0],
                         assert_bmp[1]);
        }
...
/* } */
        ...
        else if (out_format == 3)
        {
            ...
            SOC_BLOCKS_ITER(unit, blk, blk_list)
            {
                ...
                {
                    valid_flags = 1;
                    sal_snprintf(str_stat_info + sal_strnlen(str_stat_info, sizeof(str_stat_info)),
                                 sizeof(str_stat_info) - sal_strnlen(str_stat_info, sizeof(str_stat_info)),
