    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncmp(directory, "keep_the_same_path", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
    {
        dnx_sw_state_dump_directory_set(unit, directory);
    }

    if (sal_strncmp(mode, "short", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        dnx_sw_state_dump_mode_set(unit, DNX_SW_STATE_DUMP_MODE_SHORT);
    }
    else if (sal_strncmp(mode, "full", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        dnx_sw_state_dump_mode_set(unit, DNX_SW_STATE_DUMP_MODE_FULL);
    }

    if (sal_strncmp(file, "", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
    {
