    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (soc_is(unit, DNXF_DEVICE))
    {
        ...
        if (dnxf_data_port.features.feature_get(unit, dnxf_data_port_features_link_local_remote_fault_support))
        {
    ...
    BCM_PBMP_ITER(diag_info_bmp, logical_port)
    {
        ...
        {
            /**
             * Some DNXF devices can display link local and remote fault.
             * If not supported, just display "-" meaning no fault.
             */
            if (dnxf_data_port.features.feature_get(unit, dnxf_data_port_features_link_local_remote_fault_support))
            {
