...
/*
 * Imported from src/appl/diag/dfe/utils_fe3200_card.c
 * {
 */
/*************                                                       
 * INCLUDES  *                                                       
...
#define UTILS_FE3200_INIT_ERR_DEFS(e_proc_name)  \
  uint32 m_ret = 0;                       \
  const char* m_proc_name = e_proc_name;


#define UTILS_FE3200_EXIT_AND_PRINT_ERR           \
{                                          \
  if (m_ret)                               \
    {                                      \
      cli_out(\
...
{ 
  ...
  uint8 msb; 
  uint8 lsb; 
}FE3200_CARD_FLD_ADDR; 

typedef enum
{
    FE3200_CARD_SYNT_SERDES_0 =   0,
    FE3200_CARD_SYNT_SERDES_1 =   1,
    FE3200_CARD_SYNT_SERDES_2 =   2,
    FE3200_CARD_SYNT_SERDES_3 =   3,
    FE3200_CARD_SYNT_CORE     =   4,
    FE3200_CARD_NOF_SYNT_TYPES
} FE3200_CARD_SYNT_TYPE;

typedef uint8 (*FE3200_CARD_FPGA_IO_FIELD_GET) (const FE3200_CARD_FLD_ADDR *); 
typedef void (*FE3200_CARD_FPGA_IO_FIELD_SET) (const FE3200_CARD_FLD_ADDR *, uint8);     

typedef struct   
{ 
  SOC_BSP_CARDS_DEFINES           card_type; 
  char                        *agent_file; 
  FE3200_CARD_FPGA_IO_FIELD_GET io_agent_field_get_func; 
  FE3200_CARD_FPGA_IO_FIELD_SET io_agent_field_set_func; 
 
  int  
    (*i2c_syntmuxset) ( 
    const FE3200_CARD_SYNT_TYPE     target 
    ); 
...
}FE3200_CARD_BOARD_INFO; 
/*
 * }
 */
static FE3200_CARD_BOARD_INFO 
   Fe3200_card_board_info;

static int   
  fe3200_card_i2c_read ( 
    uint8            unit, 
    uint8            internal_address, 
    uint8            buffer_size, 
    uint8           *buffer 
...
static uint32 
  fe3200_card_board_synt_set_imp( 
   const FE3200_CARD_SYNT_TYPE   targetsynt, 
...
{ 
  ...
  UTILS_FE3200_INIT_ERR_DEFS("dfe_card_board_synt_set_imp"); 
  ...
  do  
  { 
    Fe3200_card_board_info.i2c_read(synt_id,SYNTHESIZER_REG135,1,&reg135);   
 
    /* Reset synt, in order for fout to be nominal rate. */ 
    databuffer[0] = (reg135 | 0x1); 
    buffersize = 1; 
 
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG135,buffersize,databuffer); 
    sal_msleep(100); 
 
    Fe3200_card_board_info.i2c_read(synt_id,SYNTHESIZER_REG137,1,&reg137);       
 
    Fe3200_card_board_info.i2c_read(synt_id,SYNTHESIZER_REG7TO12,6,databuffer);       
  ...
  */ 
  ...
  { 
    ...
    reg137 = reg137 | 0x10; 
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG137,1,&reg137); 
 
    /* modify synthesizer values to generate new frequency */ 
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG7TO12+0,1,&reg7); 
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG7TO12+1,1,&reg8); 
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG7TO12+2,1,&reg9); 
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG7TO12+3,1,&reg10); 
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG7TO12+4,1,&reg11); 
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG7TO12+5,1,&reg12); 
    ...
    reg135 = reg135 | 0x40;  
    Fe3200_card_board_info.i2c_write(synt_id,SYNTHESIZER_REG135,1,&reg135); 
...
exit: 
  UTILS_FE3200_EXIT_AND_PRINT_ERR; 
} 
...
{ 
   uint8 synt_id = 0x55;
   const FE3200_CARD_SYNT_TYPE targetsynt = FE3200_CARD_NOF_SYNT_TYPES;

   Fe3200_card_board_info.i2c_read = fe3200_card_i2c_read; 
   Fe3200_card_board_info.i2c_write = fe3200_card_i2c_write; 
    
   cli_out("%s(): targetfreq=%d, synt_id=0x%x, nominalfreq=%d\n", FUNCTION_NAME(),  targetfreq, synt_id, nominalfreq); 
 
   fe3200_card_board_synt_set_imp(targetsynt, targetfreq, FALSE, synt_id, nominalfreq); 
...
appl_dpp_convert_modport_to_sysport_id(int unit, int modid_idx,int port)
{
    ...
    else /* Run DPP system ports allocation */
    {
        ...
        /* Petra ITMH cannot be sent to system port 0, so we're using system port
                   different than 0 instead (for CPU). ARAD can use system port 0 but we will do the same as Petra */
        if (port == 0) {
...
appl_dpp_convert_modport_to_base_voq_connector(int unit, int local_port, int core_id, int ingress_modid)
{
    ...
    if (is_dnx_app)
    {
        ...
        rv = appl_sand_device_to_base_voq_connector_id_convert(unit, mapping_info.tm_port, mapping_info.core, core_id,
                                                               ingress_device, g_dii.nof_devices,
                                                               APPL_SAND_NOF_PORTS_PER_MODID, 2, &voq_conn_id);
        if (rv < 0)
        {
            return -1;
        }
...
int appl_dpp_cosq_diag_credit_request_thresholds_config(int unit, int flags, bcm_port_t port, bcm_gport_t modport_gport, bcm_port_interface_info_t *interface_info, bcm_cosq_ingress_queue_bundle_gport_config_t *config)
{
    ...
    /**
     * When working with dnx devices (such as J2C) set push queue on queues connected to ERP ports since
     * in multi-modid devices (such as J2C) we can't create multiple ERP ports on the same core thus masking it impossible (due to the symmetric assumption of the default application)
     * to imitate different cores using modids (which is the method we use to connect dpp multi core devices to dnx multi modid devices)
     */
    if (is_dnx_app)
    {
        if (interface_info->interface == BCM_PORT_IF_ERP)
        {
            default_delay_tolerance_level = BCM_COSQ_DELAY_TOLERANCE_15;
        }
    }
...
/*
 * Purpose: Initialize basic components of Petra on GFA-BI card.
