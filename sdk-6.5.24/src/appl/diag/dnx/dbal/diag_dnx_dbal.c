    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 total_bitmap_alloc = 0, total_sw_alloc = 0;
    ...
    for (table_iter = 0; table_iter < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); table_iter++)
    {
        ...
        if (table->sw_access_info.sw_payload_length_bytes > 0)
        {
             ...
             */
            if ((table->table_type != DBAL_TABLE_TYPE_DIRECT) && (table->access_method == DBAL_ACCESS_METHOD_SW_STATE))
            {
            ...
            if (curr_table_sw_size > min_size_to_display)
            {
                ...
                PRT_CELL_SET(" %s", (table->table_type == DBAL_TABLE_TYPE_DIRECT) ? "Direct" : "Hash");
                PRT_CELL_SET(" %s", "SW");
                total_sw_alloc += curr_table_sw_size;
     ...
     */
    PRT_INFO_ADD("Total SW Size: %d MBytes [SW: %d MBytes, bitmap alloc: %d MBytes]",
                 (total_sw_alloc + total_bitmap_alloc) / 1000000,
                 total_sw_alloc / 1000000, total_bitmap_alloc / 1000000);
