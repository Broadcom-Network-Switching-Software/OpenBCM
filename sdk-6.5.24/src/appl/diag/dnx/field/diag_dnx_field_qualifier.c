    ...
    sh_sand_control_t * sand_control)
{
    ...
    RHITERATOR(cs_dnx_dump, cs_dnx_dump_list)
    {
        ...
        if (show_desc == TRUE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_qual_description_get_dispatch(unit, cs_dnx_dump->bcm_qual, &qual_desc));
    ...
    prt_control_t * prt_ctr)
{
    ...
    for (bcm_qual = 0; bcm_qual < bcmFieldQualifyCount; bcm_qual++)
    {
        ...
        if (show_desc == TRUE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_qual_description_get_dispatch(unit, bcm_qual, &qual_desc));
            PRT_CELL_SET("%s", qual_desc);
        }
