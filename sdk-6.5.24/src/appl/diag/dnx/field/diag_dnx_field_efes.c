...
static sh_sand_man_t Field_efes_list_man = {
    ...
    .init_cb =  diag_dnx_field_efes_init,
};
        ...
        for (fes_pgm_iter = 0; fes_pgm_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_PROGRAMS; fes_pgm_iter++)
        {
                 ...
                 fes_mask_iter++)
            {
                    ...
                    && fes_id_info->fes_pgm_info[fes_pgm_iter].masks_id_used[fes_mask_iter])
                {
                    ...
                    {
                        DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(efes_masks_string, "%s", ", ");
                        sal_strncat_s(efes_masks_string, ", ", sizeof(efes_masks_string));
                    }
            ...
            for (ctx_iter = 0; ctx_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS; ctx_iter++)
            {
                ...
                if (utilex_bitstream_test_bit(fes_id_info->fes_pgm_info[fes_pgm_iter].context_bmp, ctx_iter))
                {
                    ...
                    if (sal_strncmp(ctx_string, "", sizeof(ctx_string)))
                    {
                        DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(ctx_string, "%s", ", ");
                        sal_strncat_s(ctx_string, ", ", sizeof(ctx_string));
                    }
        ...
        for (fes_mask_iter = 0; fes_mask_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_MASKS_PER_FES; fes_mask_iter++)
        {
            ...
            for (fes_pgm_iter = 0; fes_pgm_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_PROGRAMS; fes_pgm_iter++)
            {
                ...
                if (fes_id_info->fes_pgm_info[fes_pgm_iter].masks_id_used[fes_mask_iter])
                {
                    ...
                    {
                        DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(efes_pgm_string, "%s", ", ");
                        sal_strncat_s(efes_pgm_string, ", ", sizeof(efes_pgm_string));
                    }
                ...
                if (fes_id_info->fes_pgm_info[fes_pgm_iter].masks_id_used[fes_mask_iter])
                {
                    ...
                    for (ctx_iter = 0; ctx_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS; ctx_iter++)
                    {
                        ...
                        if (utilex_bitstream_test_bit(fes_id_info->fes_pgm_info[fes_pgm_iter].context_bmp, ctx_iter))
                        {
                            ...
                            {
                                DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(efes_ctx_string, "%s", ", ");
                                sal_strncat_s(efes_ctx_string, ", ", sizeof(efes_ctx_string));
                            }
        ...
        for (fes_id_iter = 0; fes_id_iter < context_fes_get_info.nof_fes_quartets; fes_id_iter++)
        {
            ...
            else
            {
                     ...
                     fes2msb_index++)
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT(dnx_field_map_action_type_to_predef_dnx_action(unit, field_stage,
                                                                                       context_fes_get_info.fes_quartets
                                                                                       [fes_id_iter].fes2msb_info
                ...
                if (efes_action_info.is_condition_valid == TRUE)
                {
                    ...
                    else
                    {
                             ...
                             fes2msb_index++)
                        {
                            ...
                            if ((int) (efes_action_info.efes_condition_conf[fes2msb_index].action_type) ==
                                BCM_FIELD_INVALID)
                            {
                                /** Set 'Action Type' cell. */
                                PRT_CELL_SET("%s", "Invalid");
