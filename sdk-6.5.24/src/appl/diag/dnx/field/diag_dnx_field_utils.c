    ...
    char *converted_fg_type_p)
{
    ...
    switch (fg_type)
    {
        ...
        {
            sal_strncpy_s(converted_fg_type_p, "TCAM", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            break;
        }
        case DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM:
        {
            sal_strncpy_s(converted_fg_type_p, "DIRECT_TABLE_TCAM", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            break;
        }
        case DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB:
        {
            sal_strncpy_s(converted_fg_type_p, "DIRECT_TABLE_MDB", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            break;
        }
        case DNX_FIELD_GROUP_TYPE_EXEM:
        {
            sal_strncpy_s(converted_fg_type_p, "EXEM", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            break;
        }
        case DNX_FIELD_GROUP_TYPE_STATE_TABLE:
        {
            sal_strncpy_s(converted_fg_type_p, "STATE_TABLE", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            break;
        }
        case DNX_FIELD_GROUP_TYPE_DIRECT_EXTRACTION:
        {
            sal_strncpy_s(converted_fg_type_p, "DIRECT_EXTRACTION", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            break;
        }
        case DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM:
        {
            sal_strncpy_s(converted_fg_type_p, "EXTERNAL_TCAM", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            break;
        }
        case DNX_FIELD_GROUP_TYPE_CONST:
        {
            sal_strncpy_s(converted_fg_type_p, "CONST", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            break;
        }
        default:
        {
    ...
    int is_state)
{
    ...
    sal_strncpy_s(converted_mode_p, "", DIAG_DNX_FIELD_UTILS_STR_SIZE);

    if (mode == DNX_FIELD_CONTEXT_COMPARE_MODE_SINGLE && !is_hash && !is_state)
    {
        sal_strncpy_s(converted_mode_p, "S", DIAG_DNX_FIELD_UTILS_STR_SIZE);
    }
    else if (mode == DNX_FIELD_CONTEXT_COMPARE_MODE_DOUBLE)
    {
        sal_strncpy_s(converted_mode_p, "D", DIAG_DNX_FIELD_UTILS_STR_SIZE);
    }
    else if ((mode == DNX_FIELD_CONTEXT_HASH_MODE_ENABLED) && is_hash)
    {
        sal_strncpy_s(converted_mode_p, "HASH.E", DIAG_DNX_FIELD_UTILS_STR_SIZE);
    }
    else if ((mode == DNX_FIELD_CONTEXT_STATE_TABLE_MODE_ENABLED) && is_state)
    {
        sal_strncpy_s(converted_mode_p, "ST.E", DIAG_DNX_FIELD_UTILS_STR_SIZE);
    }
    else
    {
    ...
    char out_qual_value[DIAG_DNX_FIELD_UTILS_STR_SIZE])
{
    ...
    int qual_offsets_arr[DNX_FIELD_QAUL_MAX_NOF_MAPPINGS];
    uint32 qual_size;
    uint8 qual_found;
    ...
    for (qual_index = 0; qual_index < bcmFieldQualifyCount && user_qual_size; qual_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_meta_qual_map_get_dispatch(unit, field_stage, qual_index, &meta_qual_map));

        /** Check if this bcm qual is mapped on this stage, if not continue. */
        if (meta_qual_map.dnx_qual == DNX_FIELD_QUAL_TYPE_INVALID)
        {
            continue;
        }
        ...
        rv = dnx_field_map_dnx_qual_size(unit, field_stage, pbus_dnx_qual, NULL, NULL, &qual_size);
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_offset
                        (unit, field_stage, pbus_dnx_qual, &qual_nof_mappings, qual_offsets_arr));
        
        if (qual_nof_mappings != 1)
        {
            continue;
        }
        qual_offset = qual_offsets_arr[0];
    ...
    char out_qual_value[DIAG_DNX_FIELD_UTILS_STR_SIZE])
{
    ...
    qual_offset = (attach_info_p->offset >= 0) ?
        (dnx_qual_info_single.offset) : (attach_info_p->offset + dnx_qual_info_single.offset);
