    ...
    prt_control_t * prt_ctr)
{
    ...
    dnx_field_action_map_t action_map;
    shr_error_e rv;
    ...
    for (bcm_action = 0; bcm_action < bcmFieldActionCount; bcm_action++)
    {
        ...
        if (show_desc == TRUE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_action_description_get_dispatch(unit, bcm_action, &action_desc));
         ...
         */
        if (BCM_TO_DNX_IS_BAISC_OBJ(action_map.flags))
        {
            PRT_CELL_SET("%s", bcm_action_n);
        }
        else
        {
            ...
            for (bcm_action_raw = 0; bcm_action_raw < bcmFieldActionCount; bcm_action_raw++)
            {
                ...
                SHR_IF_ERR_EXIT(rv);
                if (BCM_TO_DNX_IS_BAISC_OBJ(action_map.flags) && (dnx_action == dnx_action_raw))
                {
