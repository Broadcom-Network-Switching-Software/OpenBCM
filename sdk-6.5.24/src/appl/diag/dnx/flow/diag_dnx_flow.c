    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (flow_app_info->flow_app_type == FLOW_APP_TYPE_INIT)
    {
        ...
        /*
         * Encap access info, not applicable for virtual applications
         */
        ...
        if (!is_virtual_lif)
        {
            SHR_IF_ERR_EXIT(diag_flow_app_init_encap_info_print
                            (unit, flow_handle, flow_app_info->flow_table, sand_control));
        }
    }
    ...
    uint8 is_print_virtual_keys)
{
     ...
     */
    SHR_IF_ERR_EXIT(dbal_tables_table_get_internal(unit, flow_app_info->flow_table, &table));
