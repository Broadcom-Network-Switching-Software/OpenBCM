    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 mpls_identifier;
    uint32 num_mpls_identifiers;
    uint32 mpls_identifiers_bitmap [SAL_BIT(DNX_DATA_MAX_SWITCH_LOAD_BALANCING_NOF_BITS_MPLS_PROTOCOL_IDENTIFIER)]={0};
    int prev_identifier=0;
    int first_identifier=0;
    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);
    num_mpls_identifiers = SAL_BIT(dnx_data_switch.load_balancing.nof_bits_mpls_protocol_identifier_get(unit));
    if (detail_level > DISPLAY_LEVEL_MEDIUM)
    {
        /*
         * Print legend for 'mpls_identification' table below.
         */
        PRT_TITLE_SET("Legend  - table of mpls identification, below.");
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "column name");
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "description");
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("%s", "First index");
        PRT_CELL_SET("%s", "First mpls bit that gets the current mpls identification value");
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("%s", "Last index");
        PRT_CELL_SET("%s", "Last mpls bit that gets the current mpls identification value");
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("%s", "Value");
        PRT_CELL_SET("%s", "mpls identification value of the current bit range");
        PRT_COMMITX;
    ...
    {
        if (mpls_identification.mpls_identification[mpls_identifier/SAL_UINT32_NOF_BITS] & SAL_BIT(mpls_identifier%SAL_UINT32_NOF_BITS))
        {
            mpls_identifiers_bitmap[mpls_identifier]=1;
        }
    }
    PRT_TITLE_SET("Bitmap of MPLS protocol identifiers - MPLS_LAYER_IDENTIFICATION_BITMAP_CFG");
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "First bit");
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Last bit");
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Value");

    for (mpls_identifier = 1; mpls_identifier < num_mpls_identifiers; mpls_identifier++)
    {
        if(mpls_identifiers_bitmap[mpls_identifier]!=mpls_identifiers_bitmap[prev_identifier])
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%d ",first_identifier);
            PRT_CELL_SET("%d ",prev_identifier);
            PRT_CELL_SET("%d",mpls_identifiers_bitmap[prev_identifier]);
            first_identifier = mpls_identifier;
        }
        prev_identifier+=1;
    }
    /*Adding last identifier*/
    PRT_ROW_ADD(PRT_ROW_SEP_NONE);
    PRT_CELL_SET("%d ",first_identifier);
    PRT_CELL_SET("%d ",prev_identifier);
    PRT_CELL_SET("%d",mpls_identifiers_bitmap[prev_identifier]);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
        ...
        for (idx = 0; idx < max_layer_id; idx++)
        {
            ...
            /* Break in case the layer type is LAYER_TYPES_INITIALIZATION*/
            if (layer_type == 0)
                break;

            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get(unit, DBAL_TABLE_IN_AC_TCAM_DB, DBAL_FIELD_LAYER_TYPE, 0,
            ...
            {
                SHR_CLI_EXIT_IF_ERR(dpp_dsig_read(unit, core_id, "IRPP", dnx_data_pp.debug.from_signal_str_get(unit)->ingress_layers, NULL, "is_fragmented",
            ...
            if ((idx >= forwarding_layer || (layer_header_type == DBAL_ENUM_FVAL_LAYER_TYPES_MPLS && (dnx_data_switch.feature.feature_get(unit, dnx_data_switch_feature_mpls_labels_included_in_hash)))) && is_fragmented == FALSE)
            {
                PRT_CELL_SET("TRUE");
            }
            else
