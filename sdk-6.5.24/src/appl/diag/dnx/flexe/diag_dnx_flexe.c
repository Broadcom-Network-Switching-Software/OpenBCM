    ...
    rhlist_t * test_list)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        if (!dnx_data_nif.flexe.feature_get(unit, dnx_data_nif_flexe_is_supported) && !(test_list && test_list->include_unsupported))
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADD("TimeSlot(in Hex)");
    PRT_COLUMN_ADD("OH Group Slot(in Hex)");
