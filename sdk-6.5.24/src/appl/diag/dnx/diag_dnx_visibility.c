    ...
    bcm_pbmp_t * logical_ports_p)
{
    ...
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "PP DSP");
    PRT_COLUMN_ADD("Type");
    PRT_COLUMN_ADD("Core");
    ...
    BCM_PBMP_ITER(logical_ports, logical_port)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_in_lag(unit, logical_port, &port_is_in_lag));
        if (port_is_in_lag)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get(unit, trunk_gport,
            ...
            {
                if (port_mapping_info.core == gport_info.internal_port_pp_info.core_id[pp_port_index])
                {
        ...
        pp_dsp = port_mapping_info.pp_dsp;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 direction;
    uint32 nof_trunk_gports = 0;
    int index = 0;
    bcm_gport_t *trunk_gport_array = NULL;
    uint32 is_added = 0;
    uint32 visib_flags = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    else
    {
        ...
        BCM_PBMP_COUNT(pbmp, nof_ports);
        trunk_gport_array = sal_alloc(sizeof(bcm_gport_t) * nof_ports, "trunk_gport_array");

        BCM_PBMP_ITER(logical_ports, port)
        {
            bcm_trunk_t trunk_id = BCM_TRUNK_INVALID;
            bcm_gport_t trunk_gport = 0;
            visib_flags = direction;
            ...
            if (DNX_ALGO_PORT_TYPE_IS_PP(unit, port_info, FALSE) && port_is_in_lag)
            {
                /** Get trunk_id */
                ...
                BCM_GPORT_TRUNK_SET(trunk_gport, trunk_id);
                is_added = 0;
                for (index = 0; index < nof_trunk_gports; index++)
                {
                    /** Check if the trunk_gport is already added to trunk_gport_array */
                    if (trunk_gport_array[index] == trunk_gport)
                    {
                        is_added = 1;
                        break;
                    }
                ...
                if (is_added == 0)
                {
                    trunk_gport_array[nof_trunk_gports] = trunk_gport;
                    nof_trunk_gports = nof_trunk_gports + 1;
                }
            }
        ...
        /** Handling trunk gports*/
        if (nof_trunk_gports != 0)
        {
            visib_flags = direction;
            /** Trunk gports are supported only for ingress direction */
            visib_flags &= ~BCM_INSTRU_CONTROL_FLAG_DIRECTION_EGRESS;
            for (index = 0; index < nof_trunk_gports; index++)
            {
                if (force != DNX_COMMAND_NONE)
                {
                    SHR_CLI_EXIT_IF_ERR(dnx_visibility_port_force_set
                                        (unit, visib_flags, trunk_gport_array[index], force), "Port Force failed\n");
                }
                if (enable != DNX_COMMAND_NONE)
                {
                    SHR_CLI_EXIT_IF_ERR(dnx_visibility_port_enable_set
                                        (unit, visib_flags, trunk_gport_array[index], enable), "Port Enable failed\n");
                }
            }
        }
    }
...
exit:
    if (trunk_gport_array)
    {
        sal_free(trunk_gport_array);
    }
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORE_VALIDATE(unit, in_core, 1);
    if ((force == DNX_COMMAND_NONE) && (enable == DNX_COMMAND_NONE))
    {
        /*
         * Show port status
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        SHR_IF_ERR_EXIT(dbal_fields_enum_next_enum_value_get
                        (unit, DBAL_FIELD_PP_ASIC_BLOCK, INVALID_ENUM, &asic_block));
        while (asic_block != INVALID_ENUM)
        {
            int size, cache_status;
            if (dnx_data_debug.mem.block_map_get(unit, asic_block)->dbal_table == DBAL_TABLE_EMPTY)
            {
                SHR_IF_ERR_EXIT(dbal_fields_enum_next_enum_value_get
                                (unit, DBAL_FIELD_PP_ASIC_BLOCK, asic_block, &asic_block));
                continue;
            }
