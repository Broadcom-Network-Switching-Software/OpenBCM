...
int dpp_kbp_file_open(int unit, char *filename, int device_type)
{
...
#endif
    /* Using the enum definitions of the same value in both kbp and kaps libs */
    ...
    if (file_fp != NULL && *file_fp == NULL) {
        ...
        if ((*file_fp = sal_fopen(prefixed_file_name, is_warmboot ? "r+" : "w+")) == 0) {
...
cmd_dpp_kbp_callback_reset_device(void *handle, int32_t s_reset_low, int32_t c_reset_low){
    ...
    i2c_access_disable = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "kbp_i2c_access_disable", 0);
    if (!i2c_access_disable) {
    ...
    uint32 kbp_ilkn_rev)
{
    ...
    if (elk->is_master) {
         ...
         */
        if (ARAD_KBP_IS_OP_OR_OP2 && !SOC_WARM_BOOT(unit)) {
...
int dpp_kbp_init_kbp_interface(int unit, uint32 core) {
    ...
    if (ARAD_KBP_IS_OP_OR_OP2) {
        if (ARAD_KBP_OP_IS_DUAL_MODE) {
            rv = arad_kbp_op_phmf_fifo_reset(unit-1 , 0, elk->kbp_user_data[core].kbp_mdio_id);
            if(BCM_FAILURE(rv)){
                cli_out("Error: arad_kbp_op_phmf_fifo_reset for unit 0 core 0\n");
                return rv;
            }
            rv = arad_kbp_op_phmf_fifo_reset(unit-1, 1, elk->kbp_user_data[core].kbp_mdio_id);
            if(BCM_FAILURE(rv)){
                cli_out("Error: arad_kbp_op_phmf_fifo_reset for unit 0 core 1\n");
                return rv;
            }
            rv = arad_kbp_op_phmf_fifo_reset(unit, 0, elk->kbp_user_data[core].kbp_mdio_id);
            if(BCM_FAILURE(rv)){
                cli_out("Error: arad_kbp_op_phmf_fifo_reset for unit 1 core 0\n");
                return rv;
            }
            rv = arad_kbp_op_phmf_fifo_reset(unit, 1, elk->kbp_user_data[core].kbp_mdio_id);
            if(BCM_FAILURE(rv)){
                cli_out("Error: arad_kbp_op_phmf_fifo_reset for unit 1 core 1\n");
                return rv;
            }
        } else {
            rv = arad_kbp_op_phmf_fifo_reset(unit, 0, elk->kbp_user_data[core].kbp_mdio_id);
...
STATIC cmd_result_t cmd_dpp_kbp_dump_xml(int unit, args_t *a)
{
    ...
    if (kaps_dump == 1) {
        ...
        if (kaps_device_dump(JerAppData[unit]->kaps_device_p, fulldump, kbp_fp) < 0) {
...
cmd_dpp_kbp_test_ipv6_performance_measure(int unit, args_t *a)
{
    ...
    double AddWorstPerformance = 500000.0;
    double DelWorstPerformance = 600000.0;

    routesArray = (bcm_l3_route_t *)sal_alloc(sizeof(bcm_l3_route_t)*60000,"routes_array");

    if(SOC_IS_QUX(unit)){
        IPv6Number = 8000;
        AddWorstPerformance = 700000.0;
        DelWorstPerformance = 700000.0;
    }
    ...
    if(avgPerformance > AddWorstPerformance){
        printf("ERROR : the average time for bcm_l3_route_add is higher than %fs us for 2k entries : %f us\n\n",avgPerformance, AddWorstPerformance);
        sal_free(routesArray);
        return BCM_E_FAIL;
    }
    else{
    ...
    avgPerformance = sum/num;
    if(avgPerformance > DelWorstPerformance){
        printf("ERROR : the average time for bcm_l3_route_delete is higher than %f us for 2k entries : %f us\n\n",avgPerformance, DelWorstPerformance);
        sal_free(routesArray);
        return BCM_E_FAIL;
    }
    else{
...
cmd_dpp_kbp_test_ip_uc_scale_capacity(int unit, args_t *a)
{
    ...
    rv = jer_pp_kaps_search_test(unit, 1/*add_entries*/, 1/*search_entries*/, 0/*delete_entries*/, 1/*loop_test*/, 0/*cache_test*/, 0/*ser_test*/);
    if (rv != BCM_E_NONE) {
    ...
    rv = jer_pp_kaps_search_test(unit, 1/*add_entries*/, 1/*search_entries*/, 1/*delete_entries*/, 1/*loop_test*/, 0/*cache_test*/, 0/*ser_test*/);
    if (rv != BCM_E_NONE) {
        printf("Error, jer_pp_kaps_search_test Failed, rv=0x%x\n", rv);
        return CMD_FAIL;
    }
...
STATIC cmd_result_t
cmd_dpp_kbp_test_ser(int unit, args_t *a)
{
#ifdef BCM_88675_A0
    bcm_error_t     rv = BCM_E_NONE;
#endif /* BCM_88675_A0 */
...
#ifdef BCM_88675_A0
    cli_out("KAPS TCAM SER testing.\n");
    rv = jer_pp_kaps_search_test(unit, 1/*add_entries*/, 1/*search_entries*/, 1/*delete_entries*/, 1/*loop_test*/, 0/*cache_test*/, enable/*ser_test*/);
    if (rv != BCM_E_NONE) {
        printf("Error, jer_pp_kaps_search_test Failed, rv=0x%x\n", rv);
        return CMD_FAIL;
    }
...
int dpp_kaps_init(int unit) {
    ...
    soc_sand_rv = dpp_kbp_file_open(unit, "kaps", KAPS_DEVICE_KAPS);
    rv = handle_sand_result(soc_sand_rv);
    if (BCM_FAILURE(rv)) {
        cli_out("Error: dpp_kbp_file_open(%d)\n", unit);
        return rv;
    ...
    if (ARG_CNT(a) > 0) {
        ...
        if (raw_mode != 0) {
            ...
            for (i=0; i < JER_KAPS_MAX_NUM_OF_PARALLEL_SEARCHES; i++) {
                ...
                rv = arad_pp_dbal_entry_key_to_kaps_buffer(unit, &table, JER_KAPS_KEY_BUFFER_NOF_BYTES, qual_vals, &prefix_len, key[i]);
                if (BCM_FAILURE(rv)) {
                    cli_out("Error: jer_kaps_search_generic(%d), key buffer construction failed\n", unit);
                    return rv;
                }
...
cmd_dpp_kbp(int unit, args_t *a)
{
...
#endif
    if (!sal_strcasecmp(func, "TestIPv6Performance")) {
    ...
    } else if (!sal_strcasecmp(func, "test_ser")) {

        return cmd_dpp_kbp_test_ser(unit, a);
