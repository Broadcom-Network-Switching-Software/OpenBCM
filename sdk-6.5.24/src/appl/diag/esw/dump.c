...
do_dump_pcim(int unit, uint32 off_start, uint32 count)
{
    ...
    for (off = off_start; count--; off += 4) {
        ...
        /* IPROCGENRES_* regs are not applicable on HX5/HR4 */
        if ((SOC_IS_HELIX5(unit) || SOC_IS_HURRICANE4(unit))) {
            if (sal_strstr(pcireg_name,"IPROCGENRES_")) {
                continue;
            }
        }
    ...
    for (k = index; k < index + count; k++) {
        ...
        if (soc_feature(unit, soc_feature_fb6_ecmp_group_partitioning)) {
            if (((mem == INITIAL_L3_ECMP_GROUPm) || (mem == L3_ECMP_COUNTm)) &&
                 flags != SOC_MEM_NO_FLAGS) {
                /* Skip reserved index range for 2 level ECMP */
                if ((soc_property_get(unit, spn_L3_ECMP_LEVELS, 1) > 1) &&
                        (k >= BCMI_L3_ECMP_GROUP_FIRST_LKUP_MEM_SIZE_A &&
                         k < BCMI_L3_ECMP_GROUP_FIRST_LKUP_MEM_SIZE_B))  {
                    continue;
                }
            }
        }
                        ...
                        if (acc_type == 9) {
                            ...
                            for (pipe = 0; pipe < 8; pipe++) {
                                ...
                                if (si->active_pm_map[pm] == 1) {
                                    i = soc_mem_read_physical_index(unit, read_flags, temp_mem,
                                                        copyno, k, entry);
                                    if (i == SOC_E_UNAVAIL) {
                                        continue;
                                    }
                                    if (flags & DUMP_TABLE_HEX) {
                                        ...
                                        sal_sprintf(lineprefix, "%s.%s[%d]: ",
                                                SOC_MEM_UFNAME(unit, temp_mem),
                                                SOC_BLOCK_NAME(unit, copyno),
                                                k);
                                        if (field_names == NULL) {
                                            soc_mem_entry_dump_if_changed(unit,
                                                               temp_mem, entry,
                                                               lineprefix);
                                        } else {
                                            soc_mem_entry_dump_if_changed_fields(
                                                               unit, temp_mem,
                                                               entry,
                                                               lineprefix,
                                                               field_names);
                                        ...
                                        if (flags & DUMP_TABLE_RAW) {
                                           ...
                                           cli_out("\n");
                                            ...
                                            if (field_names == NULL) {
                                                soc_mem_entry_dump(unit,
                                                        temp_mem, entry,
                                                        BSL_LSS_CLI);
                                            } else {
                                                soc_mem_entry_dump_fields(unit,
                                                              temp_mem, entry,
                                                              field_names);
                                            }
                                        }
...
#endif
            th_alpm_table_ent_dump(unit, 0, 1, mem, entry, copyno, k, fmt,
                                   -1, flags, fmt_pfx[fmt]);
        }
#endif
#endif
...
do_dump_sw(int unit, args_t *a)
{
    ...
    if (dump_all || !sal_strcasecmp(c, "mim")) {
        _bcm_mim_sw_dump(unit);
    }
   ...
   if (soc_feature(unit, soc_feature_uc_flowtracker_learn) ||
       ...
       if (soc_property_get(unit, spn_QCM_FLOW_ENABLE, 0)) {
           if (dump_all || !sal_strcasecmp(c, "qcm")) {
                _bcm_esw_qcm_sw_dump(unit);
          }
       }
   }
#endif /* INCLUDE_FLOWTRACKER */
...
bypass_dump_table(int unit, soc_mem_t mem, uint32 copyno, int flags)
{
    ...
    /* Skip memories for which read access is not allowed while
       traffic is on. 0 indicates read access is not allowed.
     */
    if (memp->dyn_rd_type == 0) {
        return TRUE;
    }
    ...
    if (!(flags & DUMP_TABLE_EXT)) {
        ...
        if ((SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_MMU) ||
            (SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_MMU_GLB) ||
            (SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_MMU_SC) ||
            (SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_MMU_SED) ||
            (SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_MMU_XPE)) {
            return TRUE;
        }
    }
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        switch(mem) {
            case MMU_CELL_LINKm:
            case MMU_CELL_LINK_XPE0m:
            case MMU_CELL_LINK_XPE1m:
            case MMU_CELL_LINK_XPE2m:
            case MMU_CELL_LINK_XPE3m:
            case MMU_ENQS_CT_FIFOm:
            case MMU_ENQS_CT_FIFO_PIPE0m:
            case MMU_ENQS_CT_FIFO_PIPE1m:
            case MMU_ENQS_CT_FIFO_PIPE2m:
            case MMU_ENQS_CT_FIFO_PIPE3m:
            case MMU_ENQS_PBI_DBm:
            ...
            case MMU_MCFP_XPE3m:
            case MMU_MCQDB_X_Am:
            case MMU_MCQDB_X_A_XPE0m:
            case MMU_MCQDB_X_A_XPE1m:
            case MMU_MCQDB_X_A_XPE2m:
            case MMU_MCQDB_X_A_XPE3m:
            case MMU_MCQDB_X_Bm:
            case MMU_MCQDB_X_B_XPE0m:
            case MMU_MCQDB_X_B_XPE1m:
            case MMU_MCQDB_X_B_XPE2m:
            case MMU_MCQDB_X_B_XPE3m:
            case MMU_MCQDB_Y_Am:
            case MMU_MCQDB_Y_A_XPE0m:
            case MMU_MCQDB_Y_A_XPE1m:
            case MMU_MCQDB_Y_A_XPE2m:
            case MMU_MCQDB_Y_A_XPE3m:
            case MMU_MCQDB_Y_Bm:
            case MMU_MCQDB_Y_B_XPE0m:
            case MMU_MCQDB_Y_B_XPE1m:
            case MMU_MCQDB_Y_B_XPE2m:
            case MMU_MCQDB_Y_B_XPE3m:
            case MMU_MCQEm:
            case MMU_MCQE_XPE0m:
            case MMU_MCQE_XPE1m:
            case MMU_MCQE_XPE2m:
            ...
            case MMU_MCQN_XPE3m:
            case MMU_PDB_Xm:
            case MMU_PDB_X_XPE0m:
            case MMU_PDB_X_XPE1m:
            case MMU_PDB_X_XPE2m:
            case MMU_PDB_X_XPE3m:
            case MMU_PDB_Ym:
            case MMU_PDB_Y_XPE0m:
            case MMU_PDB_Y_XPE1m:
            case MMU_PDB_Y_XPE2m:
            case MMU_PDB_Y_XPE3m:
            case MMU_PKT_LINKm:
            case MMU_PKT_LINK_XPE0m:
            case MMU_PKT_LINK_XPE1m:
            case MMU_PKT_LINK_XPE2m:
            case MMU_PKT_LINK_XPE3m:
            case MMU_UCQDB_Xm:
            case MMU_UCQDB_X_XPE0m:
            case MMU_UCQDB_X_XPE1m:
            case MMU_UCQDB_X_XPE2m:
            case MMU_UCQDB_X_XPE3m:
            case MMU_UCQDB_Ym:
            case MMU_UCQDB_Y_XPE0m:
            case MMU_UCQDB_Y_XPE1m:
            case MMU_UCQDB_Y_XPE2m:
            case MMU_UCQDB_Y_XPE3m:
            case MMU_CCP_MEMm:
            case MMU_CCP_MEM_XPE0m:
            case MMU_CCP_MEM_XPE1m:
            case MMU_CCP_MEM_XPE2m:
            case MMU_CCP_MEM_XPE3m:
            case MMU_CCP_RESEQ_MEMm:
            ...
            case MMU_PKTHDRm:
            case MMU_PKTHDR_XPE0m:
            case MMU_PKTHDR_XPE1m:
            case MMU_PKTHDR_XPE2m:
            case MMU_PKTHDR_XPE3m:
            case MMU_PORTCNTm:
            case MMU_PORTCNT_XPE0m:
            case MMU_PORTCNT_XPE1m:
            case MMU_PORTCNT_XPE2m:
            case MMU_PORTCNT_XPE3m:
            case MMU_PQE0_MEMm:
            ...
            case REPLICATION_FIFO_BANK0m:
            case REPLICATION_FIFO_BANK0_XPE0m:
            case REPLICATION_FIFO_BANK0_XPE1m:
            case REPLICATION_FIFO_BANK0_XPE2m:
            case REPLICATION_FIFO_BANK0_XPE3m:
            case REPLICATION_FIFO_BANK1m:
            case REPLICATION_FIFO_BANK1_XPE0m:
            case REPLICATION_FIFO_BANK1_XPE1m:
            case REPLICATION_FIFO_BANK1_XPE2m:
            case REPLICATION_FIFO_BANK1_XPE3m:
            case RQE_FREE_LISTm:
            case RQE_FREE_LIST_XPE0m:
            case RQE_FREE_LIST_XPE1m:
            case RQE_FREE_LIST_XPE2m:
            case RQE_FREE_LIST_XPE3m:
            case RQE_LINK_LISTm:
            case RQE_LINK_LIST_XPE0m:
            case RQE_LINK_LIST_XPE1m:
            case RQE_LINK_LIST_XPE2m:
            case RQE_LINK_LIST_XPE3m:
                return TRUE;
        }

        if (mem >= MMU_CBPDATA0m && mem <= MMU_CBPDATA9_XPE3m) {
            return TRUE;
        }

        if (mem >= MMU_CFAP_BANK0m && mem <= MMU_CFAP_BANK9_XPE3m) {
            return TRUE;
        }
    }
#endif
    ...
    if (SOC_IS_TOMAHAWK3(unit)) {
        ...
        if (mem >= MMU_EBMB_PAYLOAD_SLICE0_CPUm && mem <= MMU_EBMB_PAYLOAD_SLICE7_CPU_PIPE7m) {
            return TRUE;
        }
    }
#endif
...
{
    uint32     index_min, count;
    int        copyno;
    int        rv = SOC_E_NONE;
    int        flags = PTR_TO_INT(data);
    int port = 0 , phy_port = 0, blk_num = 0, cpri_blk=0;
    if (!SOC_MEM_IS_VALID(unit, mem) ||
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, copyno) {
        if (SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_CPRI) {
            cpri_blk = 0 ;
	...
	if ((arg2) && (isint(arg2))) {
        ...
        while ((temp_field_name = ARG_GET(a)) != NULL) {
                ...
                sal_strncat(field_names, temp_field_name,
                            ARGS_BUFFER - sal_strlen(field_names) - 1);
                sal_strncat(field_names, ",",
                            ARGS_BUFFER - sal_strlen(field_names) - 1);
        }
        if (SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_CPRI && dumpall == 1) {
            rv = mem_dump_iter_callback( unit, mem, NULL );
         } else {
