    ...
    uint32 interrupt_id)
{
    ...
    /*
     * Attempt to clear and unmask interrupt, since the interrupt cannot be unmasked before the FIFO is read.
     * You can use bcmSwitchEventUnmaskAndClearDisable to prevent a failed attempt to clear the mask before
     * calling this function.
     */
    {
        bcm_switch_event_control_t switch_event_control;
        switch_event_control.event_id = interrupt_id;
        switch_event_control.index = core_id;
        switch_event_control.action = bcmSwitchEventClear;
        SHR_IF_ERR_EXIT(bcm_switch_event_control_set(0, BCM_SWITCH_EVENT_DEVICE_INTERRUPT, switch_event_control, 1));
        switch_event_control.action = bcmSwitchEventMask;
        SHR_IF_ERR_EXIT(bcm_switch_event_control_set(0, BCM_SWITCH_EVENT_DEVICE_INTERRUPT, switch_event_control, 0));
    }
    ...
    int unit)
{
    ...
    sal_memset((void *) &bcm_switch_event_control, 0, sizeof(bcm_switch_event_control_t));
