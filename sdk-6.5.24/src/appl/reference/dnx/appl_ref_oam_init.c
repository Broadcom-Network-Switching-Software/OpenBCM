    ...
    oam_traps_t * oam_traps)
{
    ...
    else
    {
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamEthAccelerated, &oam_traps->oamp_eth_trap_id,
                                                    &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamPerformanceEthAccelerated,
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamY1731MplsTp, &trap_id, &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "appl_dnx_oam_trap_config failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamPerformanceY1731MplsTp,
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamY1731Pwe, &trap_id, &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "appl_dnx_oam_trap_config failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamPerformanceY1731Pwe,
    ...
    else
    {
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapBfdOamDownMEP, &oam_traps->downmep_trap_id,
                                                    &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
         ...
         * Set OAM Level trap
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamLevel, &oam_traps->level_trap_id, &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "appl_dnx_oam_trap_config failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamPassive, &oam_traps->passive_trap_id,
                                                    &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
         ...
         */
        {
            ...
            rv = (bcm_error_t) appl_dnx_oam_snoop_config(unit, cpu_port, oam_traps);
            if (BCM_FAILURE(rv))
            {
                LOG_ERROR(BSL_LS_APPL_COMMON,
        ...
        if (count >= 1)
        {
            ...
            {
                rv = (bcm_error_t) appl_dnx_oam_punt_config(unit, cpu_port);
                if (BCM_FAILURE(rv))
                {
                    LOG_ERROR(BSL_LS_APPL_COMMON,
            ...
            }
            {
                ...
                rv = (bcm_error_t) appl_dnx_oam_up_mep_trap_init(unit, cpu_port, oamp_gport[0], oam_traps);
                if (BCM_FAILURE(rv))
                {
                    LOG_ERROR(BSL_LS_APPL_COMMON,
    ...
    int unit)
{
    ...
    else
    {
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapEgBfdIpv6InvalidUdpChecksum, &udp_checksum_trap_id,
                                                    &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
    ...
    bfd_traps_t * bfd_traps)
{
     ...
     * Configuring destination(OAMP) for accelerated endpoints
     */
    rv = (bcm_error_t) bcm_port_internal_get(unit, BCM_PORT_INTERNAL_OAMP, 2, oamp_gport, &count);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "bcm_port_internal_get failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
    ...
    else
    {
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamBfdIpv4, &bfd_traps->ipv4_trap_id, &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
        ...
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamBfdIpv6, &bfd_traps->ipv6_trap_id,
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamBfdMpls, &bfd_traps->mpls_trap_id, &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "appl_dnx_bfd_trap_config failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamBfdPwe, &bfd_traps->pwe_trap_id, &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "appl_dnx_bfd_trap_config failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
        ...
        rv = (bcm_error_t) appl_dnx_oam_trap_config(unit, bcmRxTrapOamBfdCcMplsTp, &bfd_traps->mpls_vccv_trap_id,
                                                    &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
    ...
    oam_traps_t oam_traps)
{
     ...
     */
    rv = (bcm_error_t) appl_dnx_oam_default_profile_set(unit, is_ingress, NOT_ACCELERATED, profile_id,
    ...
    oam_traps_t oam_traps)
{
     ...
     */
    rv = (bcm_error_t) appl_dnx_oam_default_profile_set(unit, is_ingress, ACCELERATED, acc_profile_id,
                                                        active_dest_gport, &oam_traps);
    ...
    int counter_enable)
{
    ...
    profile_id =
        reserved_acc_egr_profile_id | (uint32) bcmOAMProfileEgressAcceleratedEndpoint << 24 | (uint32) 1 << 31;;
    ...
    if (rv != BCM_E_NONE)
    {
        sal_printf("Error rv, in bcm_oam_profile_action_get\n");
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        sal_printf("Error rv, in bcm_oam_profile_action_set\n");
        return rv;
    }
    ...
    int unit)
{
    ...
    rv = (bcm_error_t) appl_dnx_oam_trap_init(unit, &oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "appl_dnx_oam_trap_config failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
    ...
    {
        rv = (bcm_error_t) appl_dnx_bfd_trap_init(unit, &bfd_traps);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "appl_dnx_bfd_trap_config failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
        ...
        rv = (bcm_error_t) appl_dnx_bfd_ipv6_invalid_udp_checksum(unit);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
         ...
         */
        rv = (bcm_error_t) appl_dnx_oam_upmep_reflector_init(unit, &oam_traps);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
     ...
     */
    rv = (bcm_error_t) appl_dnx_oam_downmep_reflector_init(unit, &oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
     ...
     */
    rv = (bcm_error_t) appl_dnx_oam_configure_default_profile(unit, INGRESS_SIDE, oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
     ...
     */
    rv = (bcm_error_t) appl_dnx_oam_configure_default_acc_profile(unit, INGRESS_SIDE, oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U
     ...
     */
    rv = (bcm_error_t) appl_dnx_oam_configure_default_profile(unit, EGRESS_SIDE, oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
     ...
     */
    rv = (bcm_error_t) appl_dnx_oam_configure_default_acc_profile(unit, EGRESS_SIDE, oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U
...

    {
         ...
         */
        rv = (bcm_error_t) appl_bcm_dnx_oam_failover_state_ignore_set(unit);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U
        ...
        {
            rv = (bcm_error_t) appl_dnx_oam_hlm_egress_with_update_counter_in_egr_acc_profile(unit, opcodes_dis_cnt[i],
                                                                                              0);
            if (rv != BCM_E_NONE)
            {
                LOG_ERROR(BSL_LS_APPL_COMMON,
