...
xmlAddDefAttrs(xmlParserCtxtPtr ctxt,
	...
	if (xmlHashUpdateEntry2(ctxt->attsDefault, name, prefix,
        ...
        temp = (xmlDefAttrsPtr) xmlRealloc(defaults, sizeof(xmlDefAttrs),sizeof(xmlDefAttrs) +
...
nsPush(xmlParserCtxtPtr ctxt, const xmlChar *prefix, const xmlChar *URL)
{
	...
	if (ctxt->nsTab == NULL) {
        ...
        const xmlChar ** tmp;
        ctxt->nsMax *= 2;
        tmp = (const xmlChar **) xmlRealloc((char *) ctxt->nsTab, (ctxt->nsMax/2) * sizeof(ctxt->nsTab[0]),
...
xmlCtxtGrowAttrs(xmlParserCtxtPtr ctxt, int nr) {
	...
	atts = (const xmlChar **) xmlRealloc((void *) ctxt->atts, ctxt->maxatts * sizeof(const xmlChar *),
				     maxatts * sizeof(const xmlChar *));
	if (atts == NULL) goto mem_error;
	ctxt->atts = atts;

	attallocs = (int *) xmlRealloc((void *) ctxt->attallocs, (ctxt->maxatts/5) * sizeof(int),
...
inputPush(xmlParserCtxtPtr ctxt, xmlParserInputPtr value)
{
    ...
    if (ctxt->inputNr >= ctxt->inputMax) {
        ctxt->inputMax *= 2;
        ctxt->inputTab =
            (xmlParserInputPtr *) xmlRealloc(ctxt->inputTab,(ctxt->inputMax/2) * sizeof(ctxt->inputTab[0]) ,
                                             ctxt->inputMax *
           ...
           const xmlChar *prefix, const xmlChar *URI, int nsNr)
{
	...
	ctxt->nameTab = tmp;
        tmp2 = (void **) xmlRealloc((void * *)ctxt->pushTab, ctxt->nameMax * sizeof(ctxt->pushTab[0]),
                                    ctxt->nameMax * 3 *
	...
	ctxt->pushTab = tmp2;
        ...
        ctxt->pushTab = (void **) xmlMalloc(ctxt->nameMax * 3 * sizeof(ctxt->pushTab[0]));
...
static int spacePush(xmlParserCtxtPtr ctxt, int val) {
	...
	ctxt->spaceMax *= 2;
        tmp = (int *) xmlRealloc(ctxt->spaceTab, (ctxt->spaceMax/2) * sizeof(ctxt->spaceTab[0]),
...
#define growBuffer(buffer, n) {						\
    ...
    tmp = (xmlChar *) xmlRealloc(buffer, buffer##_size, new_size);                     \
...
xmlSplitQName(xmlParserCtxtPtr ctxt, const xmlChar *name, xmlChar **prefix) {
    ...
    *prefix = NULL;
	...
	while ((c != 0) && (c != ':')) { /* tested bigname.xml */
		...
		max *= 2;
		tmp = (xmlChar *) xmlRealloc(buffer, (max/2) * sizeof(xmlChar),
						max * sizeof(xmlChar));
		if (tmp == NULL) {
		    xmlFree(buffer);
		    xmlErrMemory(ctxt, NULL);
...
xmlParseEntityValue(xmlParserCtxtPtr ctxt, xmlChar **orig) {
	...
	if (len + 5 >= size) {
	    ...
	    size *= 2;
	    tmp = (xmlChar *) xmlRealloc(buf, (size/2) * sizeof(xmlChar),size * sizeof(xmlChar));
	    if (tmp == NULL) {
		xmlErrMemory(ctxt, NULL);
                goto error;
	    }
...
xmlParseCommentComplex(xmlParserCtxtPtr ctxt, xmlChar *buf,
	...
	if (len + 5 >= size) {
	    ...
	    new_size = size * 2;
	    new_buf = (xmlChar *) xmlRealloc(buf, size,new_size);
	    if (new_buf == NULL) {
		xmlFree (buf);
		xmlErrMemory(ctxt, NULL);
		return;
...
get_more:
	...
	if (nbchar > 0) {
		...
		if (buf == NULL) {
		    ...
		    new_buf = (xmlChar *) xmlRealloc(buf, size - (len + nbchar + XML_PARSER_BUFFER_SIZE),
		                                     size * sizeof(xmlChar));
		    if (new_buf == NULL) {
		        xmlFree (buf);
			xmlErrMemory(ctxt, NULL);
...
xmlParsePI(xmlParserCtxtPtr ctxt) {
	...
	if (target != NULL) {
		...
		if (len + 5 >= size) {
		    ...
		    tmp = (xmlChar *) xmlRealloc(buf, size, new_size);
		    if (tmp == NULL) {
			xmlErrMemory(ctxt, NULL);
			xmlFree(buf);
			ctxt->instate = state;
        ...
        if ((RAW == '<') && (NXT(1) == '!') && (NXT(2) == '[')) {
            ...
            if (CMP7(CUR_PTR, 'I', 'N', 'C', 'L', 'U', 'D', 'E')) {
                ...
                if (inputIdsSize <= depth) {
                    int *tmp;
                    size_t oldInputIdsSize = inputIdsSize;
                    inputIdsSize = (inputIdsSize == 0 ? 4 : inputIdsSize * 2);
                    tmp = (int *) xmlRealloc(inputIds, oldInputIdsSize * sizeof(int),
...
xmlParseReference(xmlParserCtxtPtr ctxt) {
    ...
    if (NXT(1) == '#') {
	int i = 0;
	xmlChar out[16]; //maybe change back to 12?
	int hex = NXT(2);
...
xmlParseStartTag(xmlParserCtxtPtr ctxt) {
	...
	attname = xmlParseAttribute(ctxt, &attvalue);
		...
		ctxt->maxatts = maxatts;
	        ...
	        maxatts *= 2;
	        n = (const xmlChar **) xmlRealloc((void *) atts, (maxatts/2) * sizeof(const xmlChar *),
					     maxatts * sizeof(const xmlChar *));
		if (n == NULL) {
		    xmlErrMemory(ctxt, NULL);
		    if (attvalue != NULL)
...
xmlParseCDSect(xmlParserCtxtPtr ctxt) {
	...
	if (len + 5 >= size) {
	    ...
	    tmp = (xmlChar *) xmlRealloc(buf, size * sizeof(xmlChar), size * 2 * sizeof(xmlChar));
	    if (tmp == NULL) {
	        xmlFree(buf);
		xmlErrMemory(ctxt, NULL);
		return;
...
xmlParseBalancedChunkMemoryInternal(xmlParserCtxtPtr oldctxt,
    ...
    for (i = 0;i < oldctxt->nsNr;i += 2) {
        if(nsPush(ctxt, oldctxt->nsTab[i], oldctxt->nsTab[i+1]) < 0) {
            xmlErrMemory(ctxt, NULL);
        }
    }
#endif
