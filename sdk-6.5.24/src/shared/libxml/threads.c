...
xmlNewMutex(void)
{
    ...
    if ((tok = xmlMalloc(sizeof(xmlMutex))) == NULL)
        return (NULL);
#ifdef HAVE_PTHREAD_H
...
xmlNewRMutex(void)
{
    ...
    if ((tok = xmlMalloc(sizeof(xmlRMutex))) == NULL)
        return (NULL);
#ifdef HAVE_PTHREAD_H
    if (libxml_is_threaded != 0) {
        pthread_mutex_init(&tok->lock, NULL);
...
__xmlGlobalInitMutexLock(void)
{
    ...
    if (global_init_lock == NULL) {
        cs = xmlMalloc(sizeof(CRITICAL_SECTION));
        if (cs == NULL) {
            xmlGenericError(xmlGenericErrorContext,
...
xmlNewGlobalState(void)
{
    ...
    gs = xmlMalloc(sizeof(xmlGlobalState));
    if (gs == NULL) {
	xmlGenericError(xmlGenericErrorContext,
...
xmlGetGlobalState(void)
{
    ...
    if (globalval == NULL) {
        ...
        p = (xmlGlobalStateCleanupHelperParams *)
            xmlMalloc(sizeof(xmlGlobalStateCleanupHelperParams));
	if (p == NULL) {
            xmlGenericError(xmlGenericErrorContext,
                            "xmlGetGlobalState: out of memory\n");
