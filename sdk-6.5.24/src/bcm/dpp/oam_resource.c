...
#define _BCM_OAM_TRAP_CODE_UPMEP_FTMH(trap_code_upmep)      \
  ...
  (trap_code_upmep == SOC_PPC_TRAP_CODE_SAT_2) || \
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        if (mep_id < SOC_PPC_OAM_SIZE_OF_UMC_TABLE(unit)) {
            if (mep_id % 4 == 0) {
                ...
                /*if the endpoint is for long group, the _bcm_dpp_oam_mep_id_alloc may alloc  mep_id = 0,it is non  48_maid*/
                if(mep_id == 0 && rv != BCM_E_EXISTS) {
...
int _bcm_oam_trap_and_mirror_profile_set(int unit,
    ...
    case SOC_PPC_OAM_TRAP_ID_OAMP:
        ...
        break;
    case SOC_PPC_OAM_TRAP_ID_SAT_LB_TST_UP:
        break;
	...
	if(trap_index != SOC_PPC_OAM_TRAP_ID_SAT_LB_TST_UP){
	...
	if (set_upmep) {
		...
		if (_BCM_OAM_TRAP_CODE_UPMEP_FTMH(trap_code_upmep)) {
            ...
            if(upmep_trap_index != SOC_PPC_OAM_UPMEP_TRAP_ID_SAT){		
