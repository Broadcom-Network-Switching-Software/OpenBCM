...
bcm_petra_init(int unit)
{
  ...
  if (SOC_IS_QAX(unit)) {
    ...
    /* BCM_IF_ERROR_RETURN(bcm_petra_knetsync_init(unit)); */
    (void)bcm_petra_knetsync_init(unit);
#endif
...
_bcm_petra_modules_deinit(int unit, int silent)
{
    ...
    SOC_PETRA_DETACH(unit, _bcm_petra_stk_detach, rv); /* no dynamic allocs in Soc_petra */
...
#ifdef INCLUDE_KNETSYNC
    /* SOC_PETRA_DETACH(unit, bcm_petra_knetsync_deinit, rv); */
    (void)bcm_petra_knetsync_deinit (unit);
#endif
