    ...
    uint32 value)
{
    ...
    if (info != NULL) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_flow_tunnel_terminator_t *)
            sal_alloc(sizeof(bcm_flow_tunnel_terminator_t), "New flow tunnel_terminator");
        if (new_info == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    uint32 stat_counter_id)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_flow_tunnel_terminator_stat_attach(unit, new_info, num_of_fields,
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_flow_tunnel_terminator_stat_detach(unit, new_info, num_of_fields,
                     field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_tunnel_terminator_t(new_info, info);
    ...
    uint32 *stat_counter_id)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_flow_tunnel_terminator_stat_id_get(unit, new_info, num_of_fields,
                     field, stat_counter_id);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6523out_flow_tunnel_terminator_t(new_info, info);
