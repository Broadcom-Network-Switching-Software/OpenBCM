        ...
        for (mbox = 0; mbox < BCM_PTP_MAX_BUFFERS; ++mbox) {
            ...
            case MBOX_STATUS_ATTN_HOST_TUNNEL: {
                ...
                case bcmPTP_MESSAGE_TYPE_SIGNALING:
                    ...
                    if (BCM_FAILURE(rv = _bcm_ptp_update_packet_counts(port_num, protocol, 0, 1, 0))) {
                        SOC_CMIC_UCDBG_LOG_ADD((unit, _bcmCmicUcDbgLogHostPtpRx,
                                             "Error in _bcm_ptp_update_packet_counts()\n"));
                        PTP_ERROR_FUNC("_bcm_ptp_update_packet_counts()");
                    }
    ...
    int *ev_internal)
{
    ...
    if (event_code == _bcm_ptp_state_change_event) {
        ...
        *ev_internal = 1;
        ...
        return BCM_E_NONE;
#endif
    } else if (event_code == _bcm_ptp_master_avail_event ||
        event_code == _bcm_ptp_master_unavail_event) {

        /* handle G.8265.1 PTSF loss-Announce signaling */
        bcm_ptp_clock_port_address_t address;

        address.addr_type = ev_data[cursor + 25];
        if (address.addr_type == bcmPTPIEEE8023) {
            sal_memcpy(address.address, &ev_data[cursor + 28], PTP_MAC_ADDR_SIZE_BYTES);
        } else if (address.addr_type == bcmPTPUDPIPv4) {
            sal_memcpy(address.address, &ev_data[cursor + 28], PTP_IPV4_ADDR_SIZE_BYTES);
        } else if (address.addr_type == bcmPTPUDPIPv6) {
            sal_memcpy(address.address, &ev_data[cursor + 28], PTP_IPV6_ADDR_SIZE_BYTES);
        }
        if (BCM_FAILURE(rv = _bcm_ptp_telecom_g8265_packet_master_ptsf_loss_announce_set(unit, ptp_id, PTP_CLOCK_NUMBER_DEFAULT,
                &address, (_bcm_ptp_event_t)event_code))) {
            return rv;
        }
    }
