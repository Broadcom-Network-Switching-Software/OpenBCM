...
{   
    ...
    if ((bcmPTPServoTypeExt == config->servo) &&
        ((config->osc_type != bcmPTPOscTypeOCXO) || (config->flags & BCM_PTP_SERVO_FREQUENCY_ONLY))) {
        ...
        if (BCM_FAILURE(rv = _bcm_ptp_servo_init(unit, ptp_id, clock_num, config->osc_type, tracker_type, clock_info->holdover_in_spec_secs))) {
            PTP_ERROR_FUNC("_bcm_ptp_servo_init()");
        }
    } else if (bcmPTPServoTypeBCM == config->servo) {
        if (BCM_FAILURE(rv = _bcm_ptp_servo_deinit(unit, ptp_id, clock_num))) {
            PTP_ERROR_FUNC("_bcm_ptp_servo_deinit()");
        }
    } else {
    ...
    *offset_freq_ppt = _bcm_ptp_uint32_read(curs);  curs += 4;
    ...
    if (resp_len > 14) {
        if (lf_hi && lf_lo) {
            ...
            *lf_hi = _bcm_ptp_uint32_read(curs);  curs += 4;
            *lf_lo = _bcm_ptp_uint32_read(curs);  curs += 4;
        }
    }
    return BCM_E_NONE;
}
...
#endif /* BCM_PTP_EXT_SERVO_SUPPORT */
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_apts_apply_servo_issued_freq_corr())) {
 ...
 */
int
bcm_common_ptp_servo_performance_get(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
...
{
    int rv = BCM_E_NONE;
    _bcm_ptp_servo_performance_t serv_perf_details;

    rv = _bcm_ptp_servo_performance_get(unit, ptp_id, clock_num, &serv_perf_details);

    perf->status = serv_perf_details.status;
    perf->freq_correction = serv_perf_details.freq_correction;
    perf->phase_correction = serv_perf_details.phase_correction;
    perf->residual_phase_error = serv_perf_details.residual_phase_error;
    perf->min_round_trip_delay = serv_perf_details.min_round_trip_delay;
    perf->fwd_pkt_rate = serv_perf_details.fwd_pkt_rate;
    perf->rev_pkt_rate = serv_perf_details.rev_pkt_rate;
