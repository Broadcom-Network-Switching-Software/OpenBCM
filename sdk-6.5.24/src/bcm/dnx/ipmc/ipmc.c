...
#define IPMC_IS_IPV6_DEFAULT_ENTRY(data) (((dnx_ipmc_ipv6_default_address(data->s_ip6_addr, data->s_ip6_mask) &&  dnx_ipmc_ipv6_default_address(data->mc_ip6_addr, data->mc_ip6_mask)) || _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_DEFAULT_ENTRY))\
                                          && _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
...
#define IPMC_COMPRESSED_VAL_FULL_MASK(unit) (UTILEX_BITS_MASK(dnx_data_l3.fwd.compressed_sip_svl_size_get(unit), 0))
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    if (data->l3a_intf == 0 && (IPMC_IS_IPV4_DEFAULT_ENTRY(data) || IPMC_IS_IPV6_DEFAULT_ENTRY(data)))
    {
    ...
    else if (data->l3a_intf != 0)
    {
        if (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_RAW_ENTRY))
        {
            ...
            if ((IPMC_IS_IPV4_DEFAULT_ENTRY(data) && !use_kbp_ipv4)
                || (IPMC_IS_IPV6_DEFAULT_ENTRY(data) && !use_kbp_ipv6))
            {
    ...
    uint32 *result_type)
{
    ...
    if (IPMC_TABLE_IS_KBP(dbal_table))
    {
        ...
        if (IPMC_IS_IPV4_DEFAULT_ENTRY(data) || IPMC_IS_IPV6_DEFAULT_ENTRY(data))
        {
