...
/**
 * \brief - Send the hardware info to Kernel module.
 *
 * \param [in] unit - Unit id
 *
 * \return
 ...
 */
static int
dnx_knet_hw_info_notify(
    int unit)
{
    unsigned int len;
    kcom_msg_hw_info_t kmsg;
    bcm_gport_t oamp_gport[KCOM_HW_INFO_OAMP_PORT_MAX] = { 0 };
    int i, oamp_port_count = 0;
    dnx_algo_gpm_gport_phy_info_t gport_info;

    SHR_FUNC_INIT_VARS(unit);

    sal_memset(&kmsg, 0, sizeof(kmsg));
    kmsg.hdr.opcode = KCOM_M_HW_INFO;
    kmsg.hdr.unit = unit;
    /*
     * Record OAMP system ports
     */
    SHR_IF_ERR_EXIT(bcm_dnx_port_internal_get(unit, BCM_PORT_INTERNAL_OAMP,
                                              KCOM_HW_INFO_OAMP_PORT_MAX, oamp_gport, &oamp_port_count));
    if (oamp_port_count > KCOM_HW_INFO_OAMP_PORT_MAX)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "oamp_port_count > KCOM_HW_INFO_OAMP_PORT_MAX(%d) (now adding OAMP)\n",
    ...
    for (i = 0; i < oamp_port_count; i++)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get(unit, oamp_gport[i],
                                                        DNX_ALGO_GPM_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT, &gport_info));
        kmsg.oamp_info.oamp_ports[kmsg.oamp_info.oamp_port_number++] = gport_info.sys_port;
    }
    len = sizeof(kmsg);

    SHR_IF_ERR_EXIT(soc_knet_cmd_req((kcom_msg_t *) & kmsg, len, sizeof(kmsg)));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_knet_netif_t * netif)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_knet_netif_create_verify(unit, netif));
    ...
    {
        /** Module Header */
        SHR_IF_ERR_EXIT(dnx_algo_port_in_channel_get(unit, logical_port, &cpu_channel));
    ...
    bcm_knet_filter_t * filter)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_knet_filter_create_verify(unit, filter));
            ...
            && (filter->match_flags & BCM_KNET_FILTER_M_SRC_MODPORT)))
    {
        ...
        else
        {
                     ...
                     && (filter->match_flags & BCM_KNET_FILTER_M_SRC_MODPORT))
            {
                ...
                SHR_IF_ERR_EXIT(bcm_port_get(spa_unit, gport_modport, &flags, &interface_info, &mapping_info));
                /** Get logical port */
                SHR_IF_ERR_EXIT(dnx_algo_port_pp_dsp_to_logical_get
                                (spa_unit, mapping_info.core, mapping_info.pp_dsp, &logical_port));
                /** Get system gport from logical port */
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get(spa_unit, logical_port,
                                                                DNX_ALGO_GPM_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT,
                                                                &gport_info));
                BCM_GPORT_SYSTEM_PORT_ID_SET(gport_sysport, gport_info.sys_port);
            ...
            if (mapping_info.pp_port == 0xFFFFFFFF)
            {
                ...
                if (value.value == BCM_SWITCH_PORT_HEADER_TYPE_ETH)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_trunk_system_port_to_spa_map_get(unit, gport_sysport, tid_find, &spa));
                }
                else
                {
                    /** Get the pp port allocation info of a given trunk */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_knet_hw_info_notify(unit));
    bcm_knet_filter_t_init(&filter);
    filter.type = BCM_KNET_FILTER_T_RX_PKT;
    filter.dest_type = BCM_KNET_DEST_T_BCM_RX_API;
    filter.priority = 255;
