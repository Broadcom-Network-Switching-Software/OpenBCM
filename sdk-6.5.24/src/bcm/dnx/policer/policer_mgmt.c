    ...
    uint32 enable)
{
    uint32 entry_handle_id;
    uint32 bit_val;
    /** Below var is defined as an array of 1
      * to solve ARRAY_VS_SINGLETON Coverity issue */
    uint32 tc_bitmap[1];
    bcm_core_t core_idx;
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_TC_ENABLE_BITMAP, INST_SINGLE, &tc_bitmap[0]);
        ...
        SHR_IF_ERR_EXIT(utilex_bitstream_set(&tc_bitmap[0], tc, bit_val));
    ...
    int *reconfigure_hierarchical_meters)
{
    ...
    for (i = 0; i < DNX_POLICER_CASCADE_GROUP_SIZE; i++)
    {
        ...
        {
            sal_memcpy(&meter_profile_info_array[i], &meter_profile_info[0], sizeof(dnx_meter_profile_info_t));
        }
        else
        {
    ...
    if (other_ir_rev_exp != current_ir_rev_exp)
    {
        *reconfigure_hierarchical_meters = TRUE;
        ...
        for (i = 0; i < DNX_POLICER_CASCADE_GROUP_SIZE; i++)
        {
            ...
            {
                sal_memcpy(&meter_profile_info[i], &meter_profile_info_array[i], sizeof(dnx_meter_profile_info_t));
                meter_profile_info[i].ir_rev_exp = current_ir_rev_exp;
            }
        }
    }
    ...
    bcm_policer_config_t * pol_cfg)
{
    ...
    if (pol_cfg->mode == bcmPolicerModeCascade || pol_cfg->mode == bcmPolicerModeCoupledCascade)
    {
        ...
        {
            sal_memcpy(&meter_profile_info[i], &meter_profile_info[0], sizeof(dnx_meter_profile_info_t));
        }
        if (flags & BCM_POLICER_REPLACE)
        {
    ...
    bcm_policer_config_t * pol_cfg)
{
    ...
    if (is_global)
    {
        ...
        /** Global database operates always in a single-bucket mode */
    ...
    else
    {
        ...
        /** Check if the database that the policer belongs to is Single or Dual bucket mode */
        SHR_IF_ERR_EXIT(dnx_meter_db.generic_meter_info.db_info.
                        is_single_bucket.get(unit, core_id, is_ingress, database_id, &is_single_bucket));
    }
    ...
    bcm_policer_database_config_t * config)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        ...
        /** set it to HW */
        SHR_IF_ERR_EXIT(dnx_meter_generic_profile_set(unit, core_idx, is_ingress, database_id,
    ...
    bcm_policer_engine_t * engine)
{
    ...
    DNXCMN_CORES_ITER(unit, engine->core_id, core_idx)
    {
        ...
        if (DNX_CRPS_METER_SHMEM_IS_METER_SINGLE_BUCKET_ENGINE(unit, engine->engine_id) == FALSE)
        {
            ...
            SHR_INVOKE_VERIFY_DNXC(dnx_policer_mgmt_section_verify
    ...
    int *nof_meters)
{
    ...
    if (use_single_bucket_bank == FALSE)
    {
        ...
        *nof_meters =
            nof_banks * engine_info->nof_entries * dnx_data_meter.mem_mgmt.nof_dual_buckets_per_entry_get(unit);
    }
    else
    {
        ...
        *nof_meters = engine_info->nof_entries * dnx_data_meter.mem_mgmt.nof_single_buckets_per_entry_get(unit);
    }
    ...
    int delta)
{
    ...
    uint32 cgm_input_port[DNX_DATA_MAX_INGR_REASSEMBLY_PRIORITY_CGM_PRIORITIES_NOF] = { 0 };
    uint32 temp_port_indx;
    int nof_cgm_entries;
    bcm_core_t core_id;
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_policer_mgmt_ingress_port_compensation_delta_and_port_verify(unit, logical_port, delta));
            ...
            for (database_idx = 0; database_idx < nof_databases; database_idx++)
            {
                ...
                if (is_created == TRUE)
                {
                    ...
                    else
                    {
                        ...
                        engine_info = dnx_data_crps.engine.engines_info_get(unit, engine_id);
                        nof_meters =
                            (engine_info->nof_entries * dnx_data_meter.mem_mgmt.nof_single_buckets_per_entry_get(unit));
    ...
    int *tcsm_threshold)
{
    ...
    *tcsm_threshold = UTILEX_DIV_ROUND_UP((per_tc_mantissa * 100), per_tc_mantissa_max);
    ...
    uint32 use_pp_tc)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DATABASE_ID, database_id);
    /** Setting value field */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TCSM_MANT_MAP_SEL, INST_SINGLE,
                                 ((use_pp_tc == TRUE) ? 0 : 1));
    /** Performing the action */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    uint32 *use_pp_tc)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DATABASE_ID, database_id);
    /** Getting value field */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_TCSM_MANT_MAP_SEL, INST_SINGLE, &map_sel);
    /** Performing the action */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    *use_pp_tc = (map_sel == TRUE) ? 0 : 1;
