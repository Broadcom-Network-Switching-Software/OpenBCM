    ...
    int unit)
{
     ...
     */
    {
        ...
        extra_create_info.grain_size = dnx_data_l3.ecmp.ecmp_bank_size_get(unit);
        extra_create_info.max_tag_value = dnx_data_mdb.direct.nof_fec_ecmp_hierarchies_get(unit) - 1;
    ...
    uint32 bank_id)
{
    ...
    alloc_info.tag = DBAL_ENUM_FVAL_HIERARCHY_LEVEL_NO_HIERARCHY;
    ...
    void *tag_info)
{
    ...
    grain_indx = ALGO_L3_ECMP_BANK_GET(unit, ((resource_tag_bitmap_tag_info_t *) tag_info)->element);

    /** get tag information */
    SHR_IF_ERR_EXIT(resource_tag_bitmap_tag_get(unit, module_id, res_tag_bitmap, tag_info));
    /** get nof used elements information */
    if (((resource_tag_bitmap_tag_info_t *) tag_info)->tag != DBAL_ENUM_FVAL_HIERARCHY_LEVEL_NO_HIERARCHY)
    {
        SHR_IF_ERR_EXIT(resource_tag_bitmap_nof_used_elements_in_grain_get
                        (unit, module_id, res_tag_bitmap, grain_indx, &nof_elements_per_grain));
    }
    ((resource_tag_bitmap_tag_info_t *) tag_info)->nof_elements = nof_elements_per_grain;
    ...
    if (fec_index < dnx_data_l3.fec.nof_fecs_get(unit))
    {
        ...
        *hierarchy_p = dnx_data_l3.fec.fec_tables_info_get(unit, fec_db)->hierarchy;
            ...
            && ((*hierarchy_p) == DBAL_ENUM_FVAL_HIERARCHY_LEVEL_NO_HIERARCHY))
        {
            ...
            {
                SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fecz.hierarchy.get(unit, fec_index, &hierarchy_temp));
    ...
    int *element)
{
    ...
    if (!with_id)
    {
        uint32 temp_internal_flags =
            internal_flags | RESOURCE_TAG_BITMAP_ALWAYS_CHECK_TAG | RESOURCE_TAG_BITMAP_ALLOC_CHECK_ONLY;
        int rv;
        res_tag_bitmap_alloc_info.flags = temp_internal_flags;
    ...
    int *start_if_id)
{
    ...
    *start_if_id = -1;
    ...
    for (subr_id = range_start; (subr_id < (range_start + range_size)); subr_id++)
    {
        ...
        {
            for (;; next_fec += increment)
            {
                int free_fec_range_size;
                uint32 range_end = 0;
                SHR_IF_ERR_EXIT(algo_l3_db.fec_alloc.l3_fec_res_mngr.
                ...
                {
                    range_end =
                        (next_fec == DNX_ALGO_RES_ILLEGAL_ELEMENT) ? fec_bank_size : ALGO_L3_GET_PREV_EVEN_ID(next_fec);
                    free_fec_range_size = range_end - ALGO_L3_GET_NEXT_EVEN_ID((prev_fec + 1));
                    if (free_fec_range_size >= nof_members)
                    {
                        *start_if_id = ALGO_L3_GET_NEXT_EVEN_ID((prev_fec + 1));
                        break;
                    }
                }
                else
                {
                    ...
                    free_fec_range_size = range_end - (prev_fec + 1);
                    if (free_fec_range_size >= nof_members)
                    {
