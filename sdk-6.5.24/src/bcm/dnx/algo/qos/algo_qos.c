    ...
    const void *data)
{
    ...
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT8, "NETWORK_QOS_PIPE",
                                   template_data->network_qos_pipe,
                                   "piped NWK_QOS, EXP for two labels, PRI&CFI for two tags", "0x%x");

    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT8, "NETWORK_QOS_PIPE_MAPPED",
                                   template_data->network_qos_pipe_mapped, "NETWORK_QOS_PIPE_MAPPED", "0x%x");

    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT8, "DP_PIPE_MAPPED",
                                   template_data->dp_pipe_mapped, "DP_PIPE_MAPPED", "%d");
    ...
    const void *data)
{
    ...
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_INT, "TTL", *ttl_ptr, NULL, "%d");
    ...
    int unit)
{
    ...
    sal_memset(&data, 0, sizeof(dnx_algo_res_create_data_t));
    data.first_element = 1;
    data.nof_elements = dnx_data_qos.qos.nof_egress_php_opcode_profiles_get(unit) - 1;
    data.flags = 0;
