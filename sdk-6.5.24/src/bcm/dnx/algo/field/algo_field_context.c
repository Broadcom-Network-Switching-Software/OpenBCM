    ...
    int profile_num)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.link_profiles.free_single(unit, profile_num, NULL));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    sal_memset(&data, 0, sizeof(data));
    data.first_element = dnx_data_field.external_tcam.apptype_user_1st_get(unit);
    data.nof_elements = dnx_data_field.external_tcam.apptype_user_nof_get(unit);
    data.flags = 0;
    sal_strncpy(data.name, DNX_ALGO_FIELD_EXTERNAL_TCAM_USER_APPTYPE, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    int *context_alloc_id_p)
{
    ...
    if (alloc_flags & DNX_ALGO_RES_ALLOCATE_WITH_ID)
    {
        ...
        if (stage == DNX_FIELD_STAGE_IPMF1)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_EXISTS, "Context ID %d is already allocated for iPMF1.\r\n", *context_alloc_id_p);
            }
            SHR_IF_ERR_EXIT(dnx_algo_field_context_id_is_allocated
                            (unit, DNX_FIELD_STAGE_IPMF2, *context_alloc_id_p, &is_allocated_ipmf2));
            if (is_allocated_ipmf2)
            {
                SHR_ERR_EXIT(_SHR_E_EXISTS, "Context ID %d is already allocated for iPMF2. "
    ...
    else
    {
        ...
        if (is_allocated_ipmf1)
        {
            SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf2_context_id.free_single(unit, *context_alloc_id_p, NULL));
    ...
    dnx_field_context_t context_id)
{
    ...
    if (stage == DNX_FIELD_STAGE_IPMF1)
    {
         ...
         */
        SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf1_context_id.free_single(unit, context_id, NULL));
        SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf2_context_id.free_single(unit, context_id, NULL));
    }
    else if (stage == DNX_FIELD_STAGE_IPMF2)
    {
        /*
    ...
    dnx_field_context_t context_id)
{
    ...
    switch (stage)
    {
        ...
        case DNX_FIELD_STAGE_IPMF2:
            ...
            break;
        case DNX_FIELD_STAGE_IPMF3:
            SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf3_context_id.free_single(unit, context_id, NULL));
            break;
        case DNX_FIELD_STAGE_EPMF:
            SHR_IF_ERR_EXIT(algo_field_info_sw.epmf_context_id.free_single(unit, context_id, NULL));
            break;
        default:
