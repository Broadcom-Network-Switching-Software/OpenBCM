...
dnx_algo_chc_fully_populate(
    ...
    int *member_array)
{
    int min_entries_per_member;
    int remaining;
    int max_entries_per_member;
    int entry = 0;

    SHR_FUNC_INIT_VARS(unit);

    /** Fix COVERITY */
    if (nof_members_in_calendar == 0)
    {
        SHR_IF_ERR_EXIT(_SHR_E_PARAM);
    }
    min_entries_per_member = nof_entries_in_calendar / nof_members_in_calendar;
    remaining = nof_entries_in_calendar % nof_members_in_calendar;
    max_entries_per_member = min_entries_per_member + (remaining ? 1 : 0);
    ...
    int *nof_needed_changes)
{
    ...
    /** handle corner case of empty calendar */
    if (nof_unique_members == 0)
    {
        /** no changes are needed, calendar has no members anyways */
        *nof_needed_changes = 0;
        SHR_EXIT();
    }
