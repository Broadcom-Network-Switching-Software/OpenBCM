    ...
    dnx_egr_pointed_t * egr_pointed_p)
{
    ...
    *egr_pointed_p = DNX_ALGO_EGR_POINTED_INVALID;
    ...
    else if (_SHR_GPORT_IS_FLOW_LIF(gport))
    {
        if (!(BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_POINTED(gport)))
        {
            *global_in_lif_id = *global_out_lif_id = BCM_GPORT_TUNNEL_ID_GET(gport);
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    if (gport_hw_resources->local_out_lif != DNX_ALGO_GPM_LIF_INVALID)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
              ...
              (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit) && _SHR_GPORT_IS_FLOW_LIF(gport)))
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(gport) || _SHR_GPORT_IS_FLOW_LIF(gport))
    {
    ...
    if ((local_lif_payload->is_symmetric) || (!dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc)))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                        (unit, local_in_lif_per_core->local_in_lif[0], _SHR_CORE_ALL,
                         ...
                         &gport_hw_resources->inlif_dbal_table_id, &gport_hw_resources->inlif_dbal_result_type, NULL));
    ...
    else        /* Non symmetric */
    {
        ...
        *local_in_lif = DNX_ALGO_GPM_LIF_INVALID;
        ...
        DNXCMN_CORES_ITER(unit, _SHR_CORE_ALL, current_core)
        {
            ...
            {
                local_in_lif_per_core->local_in_lif[current_core] =
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                                    (unit, local_in_lif_per_core->local_in_lif[current_core], current_core,
                                     local_lif_payload->phy_table,
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    dnx_algo_gpm_local_in_lif_per_core_t *local_in_lif_per_core = NULL;
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS))
    {
        ...
        local_in_lif_per_core = &(gport_hw_resources->local_in_lif_per_core);
    }
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS))
    {
    ...
    else
    {
         ...
         */
        if (local_in_lif && local_in_lif_per_core)
        {
            ...
            if ((BCM_GPORT_IS_VLAN_PORT(gport) ||
                 (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit) && _SHR_GPORT_IS_FLOW_LIF(gport)))
                && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS_NATIVE(gport))
            {
                /*
                ...
                {
                    rv = dnx_vlan_port_ingress_virtual_native_ac_local_lif_get
                        (unit, core_idx, gport, (uint32 *) &local_in_lif_per_core->local_in_lif[core_idx]);

                    if (rv != _SHR_E_NOT_FOUND)
                    {
                        SHR_IF_ERR_EXIT(rv);
                        /** if it is the first core found, update the table_id and result_type */
                        if (local_in_lif_per_core->nof_in_lifs == 0)
                        {
                            SHR_IF_ERR_EXIT(algo_gpm_dbal_in_lif_dpc_physical_table_get(unit, core_idx, &phy_table));
                            SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                                            (unit, local_in_lif_per_core->local_in_lif[core_idx], core_idx, phy_table,
                                             &gport_hw_resources->inlif_dbal_table_id,
                                             &gport_hw_resources->inlif_dbal_result_type, NULL));
                            *local_in_lif = local_in_lif_per_core->local_in_lif[core_idx];
                        }
                        if (*local_in_lif != local_in_lif_per_core->local_in_lif[core_idx])
                        {
                            *local_in_lif = DNX_ALGO_GPM_LIF_INVALID;
                        }
                        local_in_lif_per_core->core_id[local_in_lif_per_core->nof_in_lifs] = core_idx;
                        local_in_lif_per_core->nof_in_lifs++;
                    }
                }
            }
            else if (global_in_lif_dummy != DNX_ALGO_GPM_LIF_INVALID)
            {
                SHR_IF_ERR_EXIT(dnx_algo_gpm_local_inlifs_get
                                (unit, gport_hw_resources, &local_lif_payload, global_in_lif_dummy, local_in_lif,
                                 local_in_lif_per_core));
            }
        }
        ...
        if (local_out_lif)
        {
            ...
            if (egr_pointed_id != DNX_ALGO_EGR_POINTED_INVALID)
            {
                ...
                if (*local_out_lif != DNX_ALGO_GPM_LIF_INVALID)
                {
                    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get(unit, *local_out_lif,
                                                                    &gport_hw_resources->outlif_dbal_table_id,
                                                                    &gport_hw_resources->outlif_dbal_result_type,
                                                                    &gport_hw_resources->logical_phase, NULL, NULL,
                                                                    NULL));
                }
            }
            else if (global_out_lif_dummy != DNX_ALGO_GPM_LIF_INVALID)
            {
                ...
                SHR_IF_ERR_EXIT_EXCEPT_IF(dnx_algo_lif_mapping_global_to_local_get
                                          ...
                                          _SHR_E_NOT_FOUND);
                *local_out_lif = local_lif_payload.local_lifs_array.local_lifs[0];
                if (*local_out_lif != DNX_ALGO_GPM_LIF_INVALID)
                {
                    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get(unit, *local_out_lif,
                                                                    &gport_hw_resources->outlif_dbal_table_id,
    ...
    bcm_gport_t * gport)
{
    ...
    if (is_egress && (shr_error == _SHR_E_NONE))
    {
        ...
        switch (dbal_table_id)
        {
            ...
            case DBAL_TABLE_EEDB_SRV6_SID:
            case DBAL_TABLE_EEDB_SFLOW_DATA:
            case DBAL_TABLE_EEDB_IPFIX_CODE:
            case DBAL_TABLE_EEDB_IPV6_DIP:
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    {
        bcm_gport_t converted_gport = gport;
        /*
         * If GPORT is not valid destination - probably logical gport, try to get forward info from SW state
         */
        ...
        if (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit))
        {
            SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, gport, _SHR_GPORT_TYPE_FLOW_LIF, &converted_gport));
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_L2_LOGICAL_GPORT, converted_gport);
        /*
    ...
    bcm_gport_t * flow_gport)
{
    ...
    if (_SHR_GPORT_IS_FLOW_LIF(in_gport))
    {
        *flow_gport = in_gport;
    }
    else
    {
        if (BCM_GPORT_IS_VLAN_PORT(in_gport))
        {
            ...
            if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(in_gport))
            {
                /** refer to dnx_flow_initiator_info_virtual_gport_allocate - see how the virtual flow gport is encoded */
                global_lif = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_GET(BCM_GPORT_VLAN_PORT_ID_GET(in_gport));
                _SHR_GPORT_FLOW_LIF_SET(global_lif, _SHR_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY, global_lif);
                _SHR_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_SET(global_lif, global_lif);
            }
            else if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(in_gport))
            {
                /** refer to dnx_flow_initiator_info_virtual_gport_allocate - see how the virtual flow gport is encoded */
                global_lif = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_GET(BCM_GPORT_VLAN_PORT_ID_GET(in_gport));
                _SHR_GPORT_FLOW_LIF_SET(global_lif, _SHR_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY, global_lif);
                _SHR_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_SET(global_lif, global_lif);
            }
            else if (BCM_GPORT_SUB_TYPE_IS_L3_VLAN_TRANSLATION(in_gport))
            {
                global_lif = BCM_GPORT_SUB_TYPE_L3_VLAN_TRANSLATION_GET(BCM_GPORT_VLAN_PORT_ID_GET(in_gport));
            }
            else
            {
                global_lif = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(BCM_GPORT_VLAN_PORT_ID_GET(in_gport));
            }
        }
        else if (BCM_GPORT_IS_MPLS_PORT(in_gport))
        {
            global_lif = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(BCM_GPORT_MPLS_PORT_ID_GET(in_gport));
        }
        else if (BCM_GPORT_IS_EXTENDER_PORT(in_gport))
        {
            global_lif = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(BCM_GPORT_EXTENDER_PORT_ID_GET(in_gport));
        }
        ...
        else if (BCM_GPORT_IS_L2GRE_PORT(in_gport))
        {
            global_lif = BCM_GPORT_L2GRE_PORT_ID_GET(in_gport);
        }
        ...
        /** lif_exc = BCM_GPORT_SUB_TYPE_LIF_EXC_GET(in_gport); */
        _SHR_GPORT_FLOW_LIF_ID_SET(*flow_gport, global_lif);
    }
    ...
    bcm_gport_t * out_gport)
{
    ...
    if (!_SHR_GPORT_IS_FLOW_LIF(flow_gport))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "flow lif gport given is not from type flow_lif (flow_gport=0x%x)\n", flow_gport);
    }

    global_lif = _SHR_GPORT_FLOW_LIF_VAL_GET(flow_gport);

    if (out_gport_type == _SHR_GPORT_TYPE_FLOW_LIF)
    {
    ...
    else if (out_gport_type == BCM_GPORT_VLAN_PORT)
    {
        ...
        int gport_subtype = _SHR_GPORT_SUB_TYPE_GET(flow_gport);

        BCM_GPORT_SUB_TYPE_LIF_SET(*out_gport, _SHR_GPORT_FLOW_LIF_EXC_GET(flow_gport), global_lif);

        if (gport_subtype == _SHR_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH)
        {
            _SHR_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_SET(*out_gport, *out_gport);
        }
        else if (gport_subtype == _SHR_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT)
        {
            _SHR_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_SET(*out_gport, *out_gport);
        }
    ...
    {
        BCM_GPORT_SUB_TYPE_LIF_SET(*out_gport, 0, global_lif);
    ...
    else if (out_gport_type == BCM_GPORT_L2GRE_PORT)
    {
        BCM_GPORT_L2GRE_PORT_ID_SET(*out_gport, global_lif);
    }
    else if (out_gport_type == BCM_GPORT_VXLAN_PORT)
    {
        BCM_GPORT_VXLAN_PORT_ID_SET(*out_gport, global_lif);
    }
    else
    {
    ...
    bcm_gport_t * out_gport)
{
    ...
    if (out_gport_type == _SHR_GPORT_TYPE_FLOW_LIF)
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_to_flow_gport_convert(unit, in_gport, out_gport));
    }
    /** convert flow lif gport to legacy gport */
    else
    {
        SHR_IF_ERR_EXIT(algo_gpm_flow_gport_to_gport_convert(unit, in_gport, out_gport_type, out_gport));
    }
