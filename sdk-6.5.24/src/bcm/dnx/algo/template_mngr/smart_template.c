    ...
    uint8 *first_reference)
{
    ...
    if (rv != _SHR_E_NOT_FOUND)
    {
        ...
        if (resource_alloc_info != NULL)
        {
            ...
            /*
             * In simulation mode, the range validation is ignored inside resource_tag_bitmap.c, thus the need to do it here.
             */
            if (_SHR_IS_FLAG_SET(extra_alloc_info->resource_flags, RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE))
            {
                SHR_RANGE_VERIFY(tmp_profile, extra_alloc_info->resource_alloc_info.range_start,
                                 extra_alloc_info->resource_alloc_info.range_end - 1, _SHR_E_INTERNAL,
                                 "The profile allocated: %d, is out of range(%d - %d)", tmp_profile,
