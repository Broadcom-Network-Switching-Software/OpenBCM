...
exit:
 ...
 /**
  *  \brief
  *  Verification function for dnx_algo_lif_mapping create
...
dnx_algo_lif_mapping_create_verify(
    int unit,
    uint32 flags,
    lif_mapping_local_lif_payload_t * local_lif_info,
    int *global_lif)
{
    int is_ingress;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (is_ingress)
    {
        ...
        {
            LIF_MNGR_LOCAL_SBC_IN_LIF_VERIFY(unit, local_lif_info->local_lifs_array.local_lifs[0]);
        }
        else if ((local_lif_info->phy_table == DBAL_PHYSICAL_TABLE_INLIF_2)
                 || (local_lif_info->phy_table == DBAL_PHYSICAL_TABLE_INLIF_3))
        {
            LIF_MNGR_LOCAL_DPC_IN_LIF_VERIFY(unit, local_lif_info->local_lifs_array.local_lifs[0]);
    ...
    else
    {
         ...
         */
        LIF_MNGR_LOCAL_OUT_LIF_VERIFY(unit, local_lif_info->local_lifs_array.local_lifs[0]);
        if (*global_lif != LIF_MNGR_INVALID)
        {
...
/**
 *  \brief
 *  Verification function for dnx_algo_lif_mapping global get.
 *  Check flags, NULL pointers and that lifs are in legal ID range.
 */
static shr_error_e
dnx_algo_lif_mapping_get_verify(
    int unit,
    uint32 flags,
    ...
    int is_validate)
{
    ...
    /*
     * In case of phy_db_dpc, inlifs - use flow inlif algo mapping
     */

    if (is_validate)
    {
        SHR_INVOKE_VERIFY_DNXC(dnx_algo_lif_mapping_create_verify(unit, flags, local_lif_payload, &global_lif));
    }
    ...
    if (is_egress)
    {
         ...
         *  Global to local
         */
        SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.egress_global_lif_to_lif_htb.insert(unit, &global_lif,
                                                                                    &local_lif_payload->
                                                                                    local_lifs_array.local_lifs[0],
                                                                                    &success));
        if (success == 0)
        {
        ...
        SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.
                        egress_lif_to_global_lif_map.set(unit, local_lif_payload->local_lifs_array.local_lifs[0],
                                                         global_lif));
    }
    else
    {
         ...
         *  Global to local
         */
        local_lif_info.local_lif = local_lif_payload->local_lifs_array.local_lifs[0];
        local_lif_info.phy_table = local_lif_payload->phy_table;
        SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.
         ...
         *  Local to global
         */
        if (local_lif_payload->phy_table == DBAL_PHYSICAL_TABLE_INLIF_1)
        {
            SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.
                            ingress_lif1_to_global_lif_map.set(unit, local_lif_payload->local_lifs_array.local_lifs[0],
                                                               global_lif));
        }
        else
        {
            SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.
                            ingress_lif2_to_global_lif_map.set(unit, local_lif_payload->local_lifs_array.local_lifs[0],
                                                               global_lif));
        }
    }
    ...
    int global_lif)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_lif_mapping_remove_verify(unit, flags, global_lif));
    ...
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.egress_global_lif_to_lif_htb.find(unit, &global_lif,
                                                                                  &local_lif_info.local_lif, &found));
        if (found == 0)
        {
            SHR_IF_ERR_EXIT(_SHR_E_NOT_FOUND);
        }
    ...
    if (is_ingress)
    {
        if (local_lif_info.phy_table == DBAL_PHYSICAL_TABLE_INLIF_1)
        {
            SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.ingress_lif1_to_global_lif_map.set(unit, local_lif_info.local_lif,
                                                                                       LIF_MNGR_INVALID));
        }
        else
        {
            SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.ingress_lif2_to_global_lif_map.set(unit, local_lif_info.local_lif,
                                                                                       LIF_MNGR_INVALID));
        }
    }
    else
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.egress_lif_to_global_lif_map.set(unit, local_lif_info.local_lif,
                                                                                 LIF_MNGR_INVALID));
    }
    ...
    int *global_lif)
{
    ...
    *global_lif = LIF_MNGR_INVALID;
     ...
     *  Check input
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_lif_mapping_get_verify(unit, flags, local_lif_info, global_lif));
...
dnx_algo_lif_mapping_global_to_local_get(
    ...
    lif_mapping_local_lif_payload_t * local_lif_payload)
{
    lif_mapping_local_lif_info_t local_lif_info = { 0 };
    uint8 is_egress;
    uint8 found;

    SHR_FUNC_INIT_VARS(unit);

    SHR_NULL_CHECK(local_lif_payload, _SHR_E_PARAM, "local_lif_payload");
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_lif_mapping_global_to_local_get_verify(unit, flags, global_lif));
    ...
    if (global_lif < dnx_data_l3.rif.nof_rifs_get(unit))
    {
         ...
         */
        local_lif_payload->local_lifs_array.local_lifs[0] = global_lif;
        SHR_EXIT();
    }
    ...
    {
        local_lif_payload->is_symmetric = TRUE;
        /*
         * Find the mapped lif in the hash table.
         */
        ...
        if (found)
        {
            local_lif_payload->local_lifs_array.local_lifs[0] = local_lif_info.local_lif;
            local_lif_payload->phy_table = local_lif_info.phy_table;
        }
    }

    if (found == 0)
    {
        local_lif_payload->local_lifs_array.local_lifs[0] = LIF_MNGR_INVALID;
        SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
    }
