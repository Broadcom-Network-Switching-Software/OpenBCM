    ...
    int unit)
{
    ...
    for (current_stack = 0; current_stack < 2; current_stack++)
    {
             ...
             current_logical_phase_index < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT; current_logical_phase_index++)
        {
            ...
            local_outlif_info.dbal_table_id = DBAL_TABLE_EEDB_IPV6_DIP;
            local_outlif_info.dbal_result_type = DBAL_RESULT_TYPE_EEDB_IPV6_DIP_ETPS_DATA_128B;
    ...
    DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EEDB_IPV6_DIP, &entry_handle_id);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                 DBAL_RESULT_TYPE_EEDB_IPV6_DIP_ETPS_DATA_128B);
