    ...
    bcm_l3_egress_t * old_egr)
{
    ...
    if (egr->encap_id != 0)
    {
        ...
        BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(flow_handle_info.flow_id, egr->encap_id);
    }
    else
    {
    ...
    if ((flags & (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN)) != (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN))
    {
        ...
        if (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION))
        {
            ...
            {
                /*
                 * Indication that 2 VLAN tags are required
                 */
        ...
        else
        {
            ...
            else
            {
                dnx_algo_local_outlif_logical_phase_e phase_selected =
                    dnx_l3_egress_arp_outlif_phase_get(egr->flags, egr->flags2, egr->encap_access);
        ...
        BCM_L3_ITF_SET(egr->encap_id, BCM_L3_ITF_TYPE_LIF, BCM_GPORT_TUNNEL_ID_GET(flow_handle_info.flow_id));
    }
    ...
    if (egr->intf != 0)
    {
        ...
        BCM_IF_ERROR_RETURN(dnx_algo_gpm_gport_to_hw_resources(unit, flow_handle_info.flow_id,
    ...
    bcm_l3_egress_t * egr)
{
    ...
    {
        BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(flow_gport, initiator_info->l3_intf_id);
        /*
         * Get VLAN-PORT gport from FLOW gport
         */
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, flow_gport, BCM_GPORT_VLAN_PORT, &egr->vlan_port_id));
    }
    ...
    bcm_l3_egress_t * egr)
{
    ...
    flow_handle_info.flow_handle = flow_handle;
    BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(flow_handle_info.flow_id, intf);
    ...
    void *data)
{
    ...
    BCM_L3_ITF_SET(intf, BCM_L3_ITF_TYPE_LIF, BCM_GPORT_TUNNEL_ID_GET(flow_handle_info->flow_id));
