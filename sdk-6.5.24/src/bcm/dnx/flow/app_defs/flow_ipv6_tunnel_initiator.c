    ...
    bcm_tunnel_type_t tunnel_type)
{
    ...
    switch (tunnel_type)
    {
        ...
        case bcmTunnelTypeVxlan6Gpe:
            ...
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid tunnel type (%d) was set", (int) tunnel_type);
            break;
    }
    ...
    uint32 *arp_outlif)
{
    ...
    {
        BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(arp_id, initiator_info->l3_intf_id);
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** tunnel_type bcmTunnelTypeGreAnyIn6 type */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_LIF_AHP_GRE_GENEVE, &is_field_valid, &lif_ahp_data));
    if (is_field_valid)
    {
        SPECIAL_FIELD_SYMBOL_DATA_ADD(special_fields, FLOW_S_F_IPV6_TUNNEL_TYPE, bcmTunnelTypeGreAnyIn6);

        flow_ipvx_tunnel_initiator_special_field_gre_flags_add(unit, lif_ahp_data, special_fields);

        SHR_EXIT();
    }

    /** tunnel_type bcmTunnelTypeUdp6 type */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_LIF_AHP_UDP, &is_field_valid, &lif_ahp_data));
    if (is_field_valid)
    {
        SPECIAL_FIELD_SYMBOL_DATA_ADD(special_fields, FLOW_S_F_IPV6_TUNNEL_TYPE, bcmTunnelTypeUdp6);

        SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_special_field_udp_ports_add
                        (unit, lif_ahp_data, &udp_ports, special_fields));

        SHR_EXIT();
    }

    /** tunnel_type bcmTunnelTypeIpAnyIn6 type */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_LIF_AHP_IP, &is_field_valid, &lif_ahp_data));
    if (is_field_valid)
    {
        SPECIAL_FIELD_SYMBOL_DATA_ADD(special_fields, FLOW_S_F_IPV6_TUNNEL_TYPE, bcmTunnelTypeIpAnyIn6);
        SHR_EXIT();
    }

    /** tunnel_type vxlan (2 options) */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_LIF_AHP_VXLAN_VXLAN_GPE, &is_field_valid, &lif_ahp_data));
    if (is_field_valid)
    {
        if (lif_ahp_data == DBAL_ENUM_FVAL_LIF_AHP_VXLAN_VXLAN_GPE_LIF_AHP_VXLAN_GPE)
        {
            SPECIAL_FIELD_SYMBOL_DATA_ADD(special_fields, FLOW_S_F_IPV6_TUNNEL_TYPE, bcmTunnelTypeVxlan6Gpe);
        }
        else
        {
            SPECIAL_FIELD_SYMBOL_DATA_ADD(special_fields, FLOW_S_F_IPV6_TUNNEL_TYPE, bcmTunnelTypeVxlan6);
        }
        SHR_EXIT();
    }
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** setting the lif_ahp */
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_lif_ahp_set(unit, entry_handle_id, tunnel_type, special_fields));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_lif_ahp_replace(unit, current_entry_handle_id, prev_entry_handle_id,
                                                               tunnel_type, special_fields, &prev_special_fields));
