...
#define VALID_VLAN_PORT_LL_INITIATOR_INDICATIONS SAL_BIT(FLOW_APP_INIT_IND_L2_GLOBAL_LIF)
...
/*
* IN_AC_UNTAGGED_DB:
* This tables has two options:
* keys: IN_PP_PORT + CORE_ID
* or
* keys: MAPPED_PP_PORT
*
* The indication which option is the right one is defined in device data.
* Please refer to DNX-DATA: 
*       - module="lif"
*       - sub_module name="in_lif"
*       - feature name="in_lif_phy_db_dpc"
*/
#define VALID_VLAN_PORT_LL_MATCH_IN_AC_UNTAGGED_SPECIAL_FIELDS      {FLOW_S_F_MAPPED_PORT}
    ...
    int *is_p2p)
{
    ...
    if (_SHR_IS_FLAG_SET(terminator_info->valid_elements_set, BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID))
    {
        ...
        if (terminator_info->l2_ingress_info.service_type == bcmFlowServiceTypeCrossConnect)
        {
            *is_p2p = TRUE;
            SHR_EXIT();
        }
    ...
    if (is_replace)
    {
        ...
        if (is_exist)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, get_entry_handle_id, DBAL_FIELD_LEARN_INFO_NO_LIF_EXT_SYM_LIF_EXT, INST_SINGLE,
                             &dbal_struct_data));
        ...
        SHR_IF_ERR_EXIT_NO_MSG(dnx_flow_lif_mgmt_field_is_set_in_hw
                               (unit, flow_handle_info, gport_hw_resources, DBAL_FIELD_GENERAL_DATA_0, &is_exist));
        ...
        case LIF_LEARN_INFO_REQ_TYPE_EXTENDED_LIF:
            SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                            (unit, DBAL_FIELD_LEARN_INFO_NO_LIF_EXT_SYM_LIF_EXT, DBAL_FIELD_DESTINATION,
                             &(forward_info->destination), &full_dbal_struct_data));
            SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                            (unit, DBAL_FIELD_LEARN_INFO_NO_LIF_EXT_SYM_LIF_EXT, DBAL_FIELD_FLUSH_GROUP,
        ...
        case LIF_LEARN_INFO_REQ_TYPE_BASIC_LIF:
            ...
            SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                            (unit, DBAL_FIELD_LEARN_INFO_NO_LIF_BASIC_SYM_LIF_BASIC, DBAL_FIELD_DESTINATION,
                             (uint32 *) &(forward_info->destination), &full_dbal_struct_data));
            dbal_entry_value_field32_set(unit, entry_handle_id,
    ...
    uint32 *selectable_result_types)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_vlan_port_ll_l2_learn_info_set_to_hw
                    (unit, entry_handle_id, l2_learn_info, learn_info_type, &forward_info, selectable_result_types));
    ...
    uint32 *selectable_result_types)
{
     ...
     * Reset the value of GLOB_IN_LIF for Non-Symmetric LIFs
     * Note:
     * Apply only for LIF creat, on replace operation, the field should remain as is!
     */
    ...
    uint32 *selectable_result_types)
{
    ...
    else
    {
        ...
        SHR_BITSET(selectable_result_types, DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_MP_OPTIMIZED_LEARN_PAYLOAD);
        {
            SHR_BITSET(selectable_result_types,
...
const dnx_flow_app_config_t dnx_flow_app_vlan_port_ll_initiator = {
     ...
     * Bitmap for supported flow handler flags
     */
    FLOW_APP_BASE_FLAGS | BCM_FLOW_HANDLE_INFO_SYMMETRIC_ALLOC | BCM_FLOW_HANDLE_NATIVE,
    ...
    for (idx = 0; idx < special_fields->actual_nof_special_fields; idx++)
    {
        if (special_fields->special_fields[idx].field_id == FLOW_S_F_PORT)
        {
                     ...
                     || (flow_cmd_control->flow_cb_type == FLOW_CB_TYPE_TRAVERSE))
            {
                ...
                {
                    algo_flags = DNX_ALGO_GPM_GPORT_HW_RESOURCES_SBC_LOCAL_LIF_INGRESS;
                    core_id = _SHR_CORE_ALL;
                }
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif(unit, algo_flags, core_id,
...
shr_error_e
flow_vlan_port_ll_ac_untagged_term_match_verify_cb(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_cmd_control_info_t * flow_cmd_control,
    void *flow_info,
    bcm_flow_special_fields_t * special_fields)
{
    ...
    for (idx = 0; idx < special_fields->actual_nof_special_fields; idx++)
    {
        if (special_fields->special_fields[idx].field_id == FLOW_S_F_MAPPED_PORT)
        {
            break;
        }
    }
...
int
flow_vlan_port_ll_ac_untagged_term_match_is_valid(
    int unit)
{
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
    {
 ...
 */
int
flow_vlan_port_ll_ac_untagged_term_match_app_specific_operations_cb(
    int unit,
    uint32 current_entry_handle_id,
    uint32 prev_entry_handle_id,
    bcm_flow_handle_info_t * flow_handle_info,
    ...
    for (idx = 0; idx < special_fields->actual_nof_special_fields; idx++)
    {
        if (special_fields->special_fields[idx].field_id == FLOW_S_F_MAPPED_PORT)
        {
            ...
            if (flow_cmd_control->flow_cb_type == FLOW_CB_TYPE_TRAVERSE)
            {
                ...
                SHR_IF_ERR_EXIT(flow_special_field_in_mapped_port_traverse_get(unit,
                                                                               current_entry_handle_id,
                                                                               flow_app_info,
            ...
            else
            {
                ...
                if (flow_cmd_control->flow_cb_type == FLOW_CB_TYPE_SET)
                {
                         ...
                         pp_port_index++)
                    {
                        ...
                        uint32 mapped_pp_port =
                            pp_port | (core_id << dnx_data_port.general.pp_port_bits_size_get(unit));
                        dbal_entry_key_field32_set(unit, current_entry_handle_id, DBAL_FIELD_MAPPED_PP_PORT,
                                                   mapped_pp_port);
...
const dnx_flow_app_config_t dnx_flow_app_vlan_port_ll_ac_untagged_term_match = {
    /*
     * Application name
     */
    FLOW_APP_NAME_VLAN_PORT_LL_AC_UNTAGGED_TERM_MATCH,
...
int
flow_vlan_port_ll_ac_tcam_term_match_is_valid(
    int unit)
{
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
    {
