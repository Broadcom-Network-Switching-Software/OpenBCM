    ...
    bcm_gport_t mirror_dest_id)
{
    ...
    {
        /** snoop profile */
        SHR_IF_ERR_EXIT(dnx_mirror_db.snif_ingress_profiles_snoop.free_single(unit, action_profile_id, NULL));
    }
    else if (_SHR_GPORT_IS_MIRROR_STAT_SAMPLE(mirror_dest_id))
    {
        /** mirror profile */
        SHR_IF_ERR_EXIT(dnx_mirror_db.snif_ingress_profiles_stat_sampling.free_single(unit, action_profile_id, NULL));
    }
    else if (BCM_GPORT_IS_MIRROR_MIRROR(mirror_dest_id))
    {
        /** mirror profile */
        SHR_IF_ERR_EXIT(dnx_mirror_db.snif_ingress_profiles_mirror.free_single(unit, action_profile_id, NULL));
    }
    else
    {
    ...
    int unit)
{
    ...
    /** mirror on drop profile sw state */
    for (admit_profile_index = 0; admit_profile_index < BCM_MIRROR_ADMIT_PROFILES_NOF; admit_profile_index++)
    {
        SHR_IF_ERR_EXIT(dnx_mirror_db.mirror_on_drop_profile.alloc(unit, admit_profile_index));
    }

    /** Egress probability profiles template manager */
 ...
 */
void
dnx_mirror_egr_probability_profiles_print_cb(
    int unit,
    const void *data)
{
#ifdef DNX_SW_STATE_DIAGNOSTIC
    uint32 *probability = (uint32 *) data;
#endif
    SW_STATE_PRETTY_PRINT_INIT_VARIABLES();

    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "PROBABILITY", *probability, NULL, "%u");
 ...
 */
void
dnx_mirror_egr_crop_size_profiles_print_cb(
    int unit,
    const void *data)
{
#ifdef DNX_SW_STATE_DIAGNOSTIC
    uint32 *crop_size = (uint32 *) data;
#endif
    SW_STATE_PRETTY_PRINT_INIT_VARIABLES();

    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "CROP_SIZE", *crop_size, NULL, "%u");
