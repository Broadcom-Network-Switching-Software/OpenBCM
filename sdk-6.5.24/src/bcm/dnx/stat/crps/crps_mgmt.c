    ...
    int *nof_counters)
{
    ...
    *nof_counters = 0;
    ...
    nof_counters_per_entry = dnx_data_crps.engine.counter_format_types_get(unit, format_type)->nof_counters_per_entry;
 ...
 * \param [out] total_nof_prev_entries - total number of engine's entries, in all prev engines counted.
    ...
    uint32 *basic_offset)
{
    ...
    while (cur_engine != -1)
    {
        ...
        if (prev_engine != -1)
        {
            ...
            range_size += engine_info->nof_entries;
    ...
    *total_nof_prev_entries = range_size;
    ...
    bcm_stat_engine_t * engine)
{
    ...
    /** 2. get how many counters in each engine entry */
    nof_counters_per_entry = dnx_data_crps.engine.counter_format_types_get(unit, inf.format)->nof_counters_per_entry;
    /**  3. find the total previous number of counters */
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_base_engine_get(unit, engine, &total_prev_range, &basic_offset));

     /** 4. find the range of each engine untill the current engine (in stat-obj-id index)*/
    input_engine_range.start = basic_offset + total_prev_range * nof_counters_per_entry;
    input_engine_range.end = input_engine_range.start + (engine_info->nof_entries * nof_counters_per_entry) - 1;
    ...
    for (type_id = 0; type_id < BCM_STAT_MAX_NUMBER_OF_OBJECT_TYPES; type_id++)
    {
        ...
        if (type.valid == TRUE)
        {
            ...
            if (type_valid[type_id] == TRUE)
            {
                ...
                {
                    last_index = first_index + (engine_info->nof_entries * nof_counters_per_entry) - 1;
                }
    ...
    uint32 *selection_bitmap)
{
    ...
    total_nof_bits = dnx_data_crps.expansion.source_type_get(unit, interface->interface_source,
