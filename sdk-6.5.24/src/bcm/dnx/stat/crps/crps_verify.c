    ...
    bcm_stat_counter_interface_t * config)
{
    ...
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        ...
        if ((flags & BCM_STAT_COUNTER_MODIFY_RANGE_ONLY) == FALSE)
        {
            ...
            /** check engine source */
            if (config->source >= bcmStatCounterInterfaceCount || (int) (config->source) < 0)
            {
...
static shr_error_e
dnx_crps_mgmt_counter_eviction_parameters_network_type_verify(
    int unit,
    int core_idx,
    bcm_stat_eviction_t * eviction,
    int engine_id)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unssuported eviction type=%d \n", eviction->type);
    }
    ...
    bcm_stat_eviction_t * eviction)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_crps_mgmt_counter_eviction_parameters_network_type_verify
                        (unit, core_idx, eviction, engine_id));
    }
