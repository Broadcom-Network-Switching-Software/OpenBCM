    ...
    bcm_port_t port)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_clear_verify(unit, port));
    ...
    uint64 *value_arr)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_multi_get_verify(unit, port, nstat, stat_arr, value_arr));
    for (i = 0; i < nstat; i++)
    {
...
/**
 * \brief
 *   Insert counter related info to generic ER database
 */
static shr_error_e
dnx_counter_port_er_journal_insert(
    int unit,
    int is_add,
    bcm_port_t logical_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_entry_new(unit,
                                                        ...
                                                        sizeof(int),
                                                        (uint8 *) &logical_port,
                                                        sizeof(bcm_port_t), &dnx_counter_port_er_rollback));
exit:
    SHR_FUNC_EXIT;
}
 ...
 */
static shr_error_e
dnx_counter_port_update_er_journal_insert(
    int unit,
    int next_master_port,
    bcm_port_t logical_port)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_entry_new(unit,
                                                        ...
                                                        sizeof(int),
                                                        (uint8 *) &logical_port,
                                                        sizeof(bcm_port_t), &dnx_counter_port_update_er_rollback));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    if ((DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, TRUE, TRUE) && is_master_port)
        ...
        || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, port_info, TRUE) || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info))
    {
        if (is_init_sequence)
        {
            stat_pbmp = dnx_data_mib.general.stat_pbmp_get(unit);
            if (BCM_PBMP_MEMBER(stat_pbmp->pbmp, port))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_counter_port_er_journal_insert(unit, 1, port));
            }
        }
        else
        {
    ...
    int unit)
{
        ...
        || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, port_info, TRUE) || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_counter_port_er_journal_insert(unit, 0, port));
        if (next_master_port != DNX_ALGO_PORT_INVALID)
        {
            if (BCM_PBMP_MEMBER(counter_pbmp, port))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_counter_port_update_er_journal_insert(unit, next_master_port, port));
            }
        }
        else
        {
    ...
    switch (counter_id)
    {
        ...
        case dnx_mib_counter_nif_rx_bip_errors:
            if (!DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, TRUE /** ELK*/ , TRUE /** L1 */ ))
            {
                ...
                {
                    nof_lane_in_pm = dnx_data_nif.eth.nof_cdu_lanes_in_pm_get(unit);
                }
                else if (DNX_ALGO_PORT_IMB_TYPE_IS_CLU(ethu_info.imb_type))
                {
                    nof_lane_in_pm = dnx_data_nif.eth.nof_clu_lanes_in_pm_get(unit);;
                }
                else
#endif
                {
    ...
    uint32 *clear_on_read)
{
     ...
     */
#ifdef BCM_DNX2_SUPPORT
    if (DNX_ALGO_PORT_IMB_TYPE_IS_CDU(ethu_info.imb_type) && *counter_interval_p != 0)
    {
