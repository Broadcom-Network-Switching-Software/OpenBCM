...
/**
 * \brief - Function to check that the vlaues used for ERPP_COUNTER_PROFILE match the register controlling them.
 * \param [in] unit - relevant unit
 *
 * \return
 *   Negative in case of an error. See \ref shr_error_e
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                    (unit, DBAL_FIELD_ERPP_COUNTER_PROFILE, DBAL_ENUM_FVAL_ERPP_COUNTER_PROFILE_INVALID,
...
#else
        SHR_ERR_EXIT(_SHR_E_CONFIG, "Device expected to support per outlif profile counter profile, but not compiled.");
#endif
    }
