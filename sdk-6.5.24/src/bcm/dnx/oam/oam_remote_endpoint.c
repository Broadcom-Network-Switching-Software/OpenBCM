...
#endif
    uint32 ccm_period_ms = 0, ccm_period_micro_s = 0, mep_db_period = 0;

    SHR_FUNC_INIT_VARS(unit);

#ifdef BCM_DNX2_SUPPORT
    SHR_ALLOC(entry_values, sizeof(dnx_oam_oamp_ccm_endpoint_t),
              "Endpoint data read to get Tx period", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
...
#ifdef BCM_DNX2_SUPPORT
/* { */
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mep_db_ccm_endpoint_get(unit, endpoint_info->local_id, 0, entry_values));
            mep_db_period = entry_values->ccm_interval;
/* } */
#endif
        }
        else
        ...
        SHR_IF_ERR_EXIT(dnx_oam_oamp_ccm_interval_to_ccm_period(unit, mep_db_period, (int *) &(ccm_period_ms)));
    ...
    bcm_oam_endpoint_info_t * endpoint_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_remote_endpoint_create_verify(unit, endpoint_info));
    ...
    {
        /**
        * forbid APP to change the endpoint name to a value which doesn't equal to the mep_id of this endpoint
        */
        sal_memset(&rmep_sw_info, 0, sizeof(rmep_sw_info));
        SHR_IF_ERR_EXIT(dnx_oam_sw_state_remote_endpoint_info_get(unit, rmep_idx, &rmep_sw_info));
        if (endpoint_info->name != rmep_sw_info.rmep_id)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error:remote endpoint name must equal to rmep_id 0x%x\n", rmep_sw_info.rmep_id);
        }
    ...
    if (is_update)
    {
        ...
        if (dnx_oam_oamp_rmep_compare(unit, TRUE, &old_oamp_rmep_entry, &oamp_rmep_entry))
        {
            /*
    ...
    if (!is_update)
    {
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_rmep_index_db_add
                            (unit, endpoint_info->local_id, endpoint_info->name, rmep_idx));
        }
        else
#endif
        {
    ...
    uint8 del_rmep_from_ll)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_rmep_index_db_remove(unit, endpoint_info->local_id, endpoint_info->name));
    }
#endif
 ...
 */
shr_error_e
dnx_oam_lmep_rmep_index_db_add_traverse_callback(
    int unit,
    bcm_oam_endpoint_t local_endpoint,
    bcm_oam_endpoint_t remote_endpoint)
{
    ...
    SHR_IF_ERR_EXIT(dnx_oam_sw_state_endpoint_info_get(unit, local_endpoint, &sw_endpoint_info));
    SHR_IF_ERR_EXIT(dnx_oam_remote_endpoint_get(unit, remote_endpoint, &remote_ep_info));
#ifdef BCM_DNX2_SUPPORT
/* { */
    SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_rmep_index_db_add(unit, sw_endpoint_info.duplicated_mep_id,
                                                      remote_ep_info.name, remote_endpoint));
/* } */
#endif
 ...
 */
shr_error_e
dnx_oam_lmep_rmep_index_db_remove_traverse_callback(
    int unit,
    bcm_oam_endpoint_t local_endpoint,
    bcm_oam_endpoint_t remote_endpoint)
{
...
#ifdef BCM_DNX2_SUPPORT
/* { */
    SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_rmep_index_db_remove
                    (unit, sw_endpoint_info.duplicated_mep_id, remote_ep_info.name));
/* } */
#endif
exit:
    SHR_FUNC_EXIT;
