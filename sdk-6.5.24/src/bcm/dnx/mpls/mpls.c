    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (!dnx_data_flow.mpls.feature_get(unit, dnx_data_flow_mpls_ingress_flow_in_use))
    {
        if (!DNX_MPLS_LABEL_IN_RANGE(unit, info->label))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "MPLS label (%d) is out of range. The range should be 0 - %d", info->label,
    ...
    for (first_nibble_index = 0; first_nibble_index < DBAL_NOF_ENUM_FIRST_NIBBLE_INDEX_VALUES; first_nibble_index++)
    {
        switch (first_nibble_index)
        {
            case DBAL_ENUM_FVAL_FIRST_NIBBLE_INDEX_IPV4:
                ...
                if (dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_mpls_module_init_enabled))
                {
...
{
    uint32 entry_handle_id;
    uint32 bier_enable, bier_support;
    int system_headers_mode;
    ...
    bier_support = dnx_data_bier.params.feature_get(unit, dnx_data_bier_params_hw_supported) ? TRUE : FALSE;
    bier_enable = dnx_data_bier.params.feature_get(unit, dnx_data_bier_params_is_supported) ? TRUE : FALSE;
    if (bier_support)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BIER_ENABLE, INST_SINGLE, bier_enable);
    }
    if (dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_mpls_module_init_enabled))
    {
    ...
    if (dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_mpls_module_init_enabled))
    {
        ...
        /*
         * Enable Parsing for MPLS BIER, 
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_PARSERIN1_MPLS_SPECULATIVE, &entry_handle_id));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VALUE_OF_5, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    /*
     * BIER type in RCH header init, 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_TRAP, &entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BIER_MPLS_O_RCH,
                                 INST_SINGLE, DBAL_DEFINE_CURRENT_NEXT_PROTOCOL_SET_2_BIER_MPLS_OVER_RCH);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
