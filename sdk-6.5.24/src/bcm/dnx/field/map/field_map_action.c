    ...
    dnx_field_map_action_info_t * dnx_action_info_p)
{
    ...
    switch (dnx_action_class)
    {
        ...
        case DNX_FIELD_ACTION_CLASS_STATIC:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_static_action_info_get_dispatch(unit, stage, action_id, &static_action_info));

            if (static_action_info.field_id == DBAL_FIELD_EMPTY)
            {
                SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "The action(%d) in stage %s does not exist!", action_id,
                             dnx_field_stage_text(unit, dnx_stage));
            }
            dnx_action_info_p->field_id = static_action_info.field_id;
            /**
            ...
            for (signal_iter = 0; signal_iter < DNX_DATA_MAX_FIELD_DIAG_NOF_SIGNALS_PER_ACTION; signal_iter++)
            {
                ...
                if (static_action_info.signal_name[signal_iter] == 0)
                {
                    break;
                }
                dnx_action_info_p->signal_name[signal_iter] = static_action_info.signal_name[signal_iter];
            }
            break;
        }
    ...
    dnx_field_action_map_t * action_map_info_p)
{
    ...
    action_map_info_p->dnx_action = DNX_FIELD_ACTION_TYPE_INVALID;
...
dnx_field_map_action_bcm_to_dnx_int(
    ...
    bcm_field_action_t bcm_action,
    dnx_field_action_t * dnx_action_p,
    dnx_field_action_map_t * action_map_entry_p)
{
    ...
    SHR_NULL_CHECK(action_map_entry_p, _SHR_E_PARAM, "action_map_entry_p");
    SHR_IF_ERR_EXIT(dnx_field_action_map_t_init(unit, action_map_entry_p));
    /*
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_map_global_action_map_get_dispatch(unit, bcm_action, action_map_entry_p));
        /*
         * Static action
         */
        /*
         ...
         */
        if (action_map_entry_p->dnx_action != DNX_FIELD_ACTION_TYPE_INVALID)
        {
            dnx_action = action_map_entry_p->dnx_action;
        ...
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_field_map_static_action_map_get_dispatch
                            (unit, dnx_stage, bcm_action, action_map_entry_p));
            if (action_map_entry_p->dnx_action == DNX_FIELD_ACTION_TYPE_INVALID)
            {
                if (print_errors)
                {
            ...
            if (!is_std_1 && (action_map_entry_p->conversion_cb != NULL))
            {
                SHR_IF_ERR_EXIT(dnx_field_bare_metal_support_check
                                (unit, action_map_entry_p->conversion_cb, &bare_metal_support));
            ...
            dnx_action = DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, dnx_stage, action_map_entry_p->dnx_action);
        }
    }
...
dnx_field_map_action_bcm_to_dnx(
    ...
    dnx_field_action_t * dnx_action_p)
{
    dnx_field_action_map_t action_map_entry;
    SHR_FUNC_INIT_VARS(unit);
    ...
    dnx_field_entry_action_t * dnx_action_info)
{
    ...
    if (action_map_entry.conversion_cb == NULL)
    {
    ...
    uint8 *found_p)
{
    ...
    dnx_field_action_map_t global_action_info;
    SHR_FUNC_INIT_VARS(unit);
    ...
    *found_p = FALSE;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_map_global_action_map_get_dispatch(unit, i_bcm_act, &global_action_info));
        /*
         * If Additional means not main object
         */
        if ((global_action_info.flags & BCM_TO_DNX_ADDITIONAL_OBJ))
        {
            continue;
        }
        /*
         ...
         */
        if (DNX_ACTION_ID(global_action_info.dnx_action) == action_id &&
            DNX_ACTION_CLASS(global_action_info.dnx_action) == action_class)
        {
    ...
    uint8 *found_p)
{
    bcm_field_action_t i_bcm_act;
    dnx_field_action_id_t action_id = DNX_ACTION_ID(dnx_action);
    dnx_field_action_map_t static_action_info;
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(found_p, _SHR_E_INTERNAL, "found_p");
    ...
    *found_p = FALSE;
    ...
    for (i_bcm_act = 0; i_bcm_act < bcmFieldActionCount; i_bcm_act++)
    {
        SHR_IF_ERR_EXIT(dnx_field_map_static_action_map_get_dispatch(unit, stage, i_bcm_act, &static_action_info));
        /*
         * If Additional means not main object
         */
        if ((static_action_info.flags & BCM_TO_DNX_ADDITIONAL_OBJ))
        {
            continue;
        }

        if (static_action_info.dnx_action == action_id)
        {
            *bcm_action_p = i_bcm_act;
            *found_p = TRUE;
            break;
    ...
    int *dnx_action_nof_p)
{
    ...
    if ((dnx_action_list =
         sal_alloc(field_map_stage_info.static_action_nof * sizeof(dnx_field_action_t), "act_list")) == NULL)
    {
    ...
    i_action = 0;
    for (i_info = 0; i_info < field_map_stage_info.static_action_nof; i_info++)
    {
        SHR_IF_ERR_EXIT(dnx_field_map_static_action_info_get_dispatch(unit, stage, i_info, &static_action_info));
        if (static_action_info.field_id != DBAL_FIELD_EMPTY)
        {
            ...
            if (i_action >= field_map_stage_info.static_action_nof)
            {
    ...
    int *action_is_legal_p)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value
                        (unit, field_map_stage_info.action_field_id, action_type, &dbal_enum_val_illegal));
    ...
    dnx_field_action_t * dnx_action_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_static_action_info_get_dispatch(unit, dnx_stage, action_id, &static_action_info));
    /*
    ...
    switch (dnx_action_class)
    {
        ...
        case DNX_FIELD_ACTION_CLASS_STATIC:
        {
            ...
            SHR_IF_ERR_EXIT(dbal_field_struct_field_size_get(unit,
                                                             field_map_stage_info.container_act_field_type,
                                                             static_action_info.field_id, &size));
            break;
        }
        case DNX_FIELD_ACTION_CLASS_SW:
        {
    ...
    dnx_field_action_t * dnx_action_p)
{
    ...
    if (field_map_stage_info.static_action_info == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Static Action list is not defined on Stage:%d\n", dnx_stage);
    }

    *dnx_action_p = DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, dnx_stage, field_map_stage_info.static_action_nof - 1);
    ...
    int unit)
{
    ...
    dnx_field_stage_e stage;
    dnx_field_map_stage_info_t field_map_stage_info;
    dnx_field_action_map_t action_map_info;
    dnx_field_base_action_info_t sw_action_info;
     ...
     */
    DNX_FIELD_STAGE_ACTION_ITERATOR(stage, field_map_stage_info)
    {
        int i_info;
        for (i_info = 0; i_info < field_map_stage_info.static_action_nof; i_info++)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value
                            (unit, field_map_stage_info.action_field_id, static_action_info.field_id,
    ...
    for (i_bcm_act = 0; i_bcm_act < bcmFieldActionCount; i_bcm_act++)
    {
        ...
        dnx_field_action_id_t action_id;
        dnx_field_stage_e dnx_stage;
        SHR_IF_ERR_EXIT(dnx_field_map_global_action_map_get_dispatch(unit, i_bcm_act, &action_map_info));
        if (action_map_info.dnx_action != DNX_FIELD_ACTION_TYPE_INVALID)
        {
            action_id = DNX_ACTION_ID(action_map_info.dnx_action);
            action_class = DNX_ACTION_CLASS(action_map_info.dnx_action);
            ...
            SHR_IF_ERR_EXIT(dnx_field_action_description_is_empty_dispatch(unit, i_bcm_act, &is_empty));
            if (is_empty)
            {
        ...
        else
        {
            ...
            DNX_FIELD_STAGE_ACTION_ITERATOR(dnx_stage, field_map_stage_info)
            {
                uint8 dbal_enum_val_illegal;
                SHR_IF_ERR_EXIT(dnx_field_map_static_action_map_get_dispatch
                                (unit, dnx_stage, i_bcm_act, &action_map_info));
                if (action_map_info.dnx_action == DNX_FIELD_ACTION_TYPE_INVALID)
                    continue;
                /*
                 * Here dnx_action is not encoded - it is actually DBAL enum for the ACTION field
                 */
                action_id = action_map_info.dnx_action;
                /*
                 * Check if this enum is enabled for this stage.
                 * Only write to debug, as we need to support multiple devices.
                 */
                SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value
                                (unit, field_map_stage_info.action_field_id, action_id, &dbal_enum_val_illegal));
                if (dbal_enum_val_illegal == FALSE)
                {
                ...
                SHR_IF_ERR_EXIT(dnx_field_map_static_action_info_get_dispatch
                                (unit, dnx_stage, action_id, &static_action_info));
                if (static_action_info.field_id == DBAL_FIELD_EMPTY)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Action:%s mapped to invalid static dnx action type:%s\n",
    ...
    for (i_bcm_act = 0; i_bcm_act < bcmFieldActionCount; i_bcm_act++)
    {
        ...
        DNX_FIELD_STAGE_ACTION_ITERATOR(dnx_stage, field_map_stage_info)
        {
            SHR_IF_ERR_EXIT(dnx_field_map_static_action_map_get_dispatch(unit, dnx_stage, i_bcm_act, &action_map_info));
            if (action_map_info.conversion_cb != NULL)
            {
                SHR_IF_ERR_EXIT(dnx_field_bare_metal_support_check
                                (unit, action_map_info.conversion_cb, &bare_metal_support));
            }
        }
    }
