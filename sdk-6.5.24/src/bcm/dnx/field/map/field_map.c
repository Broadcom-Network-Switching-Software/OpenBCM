    ...
    dnx_field_stage_e stage)
{
    ...
    else
    {
        ...
        rv = dnx_field_map_stage_info_get_dispatch(unit, stage, &field_map_stage_info);
    ...
    uint8 static_opcode_is_valid[DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF])
{
    ...
    RHDATA_ITERATOR(opcode_node, catalogue, "AppType")
    {
        ...
        if (is_standard_image)
        {
            ...
            {
                rv = dnx_field_map_standard_1_predefined_apptype_to_opcode_get_dispatch(unit, apptype_index,
                                                                                        &mapped_opcode);
                if (rv == _SHR_E_NOT_FOUND)
                {
                    continue;
    ...
    int unit)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "The number of static opcodes (%d) is smaller then the number of opcode in standard_1 %d.\n",
    ...
    for (i_apptype = 0; i_apptype < nof_static_apptypes; i_apptype++)
    {
        ...
        if (is_standard_image)
        {
            rv = dnx_field_map_standard_1_predefined_apptype_to_opcode_get_dispatch(unit, i_apptype, &mapped_opcode);
            if (rv == _SHR_E_NOT_FOUND)
            {
                continue;
            }
     ...
     */
    if (is_standard_image && dnx_data_field.external_tcam.use_kbp_get(unit) && (!sw_state_is_warm_boot(unit)))
    {
        int kbp_found;
                ...
                if (rv != _SHR_E_NOT_FOUND)
                {
                    ...
                    if ((!sw_state_is_warm_boot(unit)) && kbp_found)
                    {
                        ...
                        for (fwd_context_idx = 0; fwd_context_idx < fwd_nof_contexts; fwd_context_idx++)
                        {
                            ...
                            if (mapped_acl_contexts[fwd_context_idx] != acl_contexts[fwd_context_idx])
                            {
                                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                             "Apptype %d FWD context %d mapped alternatively to ACL contexts %d and %d.\n",
                                             i_apptype, mapped_fwd_contexts[fwd_context_idx],
                                             mapped_acl_contexts[fwd_context_idx], acl_contexts[fwd_context_idx]);
                            }
                        }
                    }
                }
...
/**
* \brief
*  Verifies that the external TCAM defined are aligned with the KBP defines.
* \param [in] unit - Device ID.
* \return
*   shr_error_e - Error Type
* \remark
    ...
    dnx_field_stage_e * dnx_stage_p)
{
    ...
    DNX_FIELD_STAGE_ITERATOR(dnx_stage, field_map_stage_info)
    {
    ...
    dbal_enum_value_field_layer_types_e * dnx_layer_type)
{
    ...
    switch (bcm_layer_type)
    {
        ...
        case bcmFieldLayerTypeTm:
            *dnx_layer_type = DBAL_ENUM_FVAL_LAYER_TYPES_ITMH;
            break;
    ...
    dbal_tables_e * table_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get_dispatch(unit, dnx_stage, &field_map_stage_info));
    /** We do not assume here that DBAL_TABLE_EMPTY would always be 0.*/
    if (field_map_stage_info.lookup_enabler_table_id == 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "lookup DBAL table for stage:%s is not provided.",
                     dnx_field_stage_text(unit, dnx_stage));
    }
    if (field_map_stage_info.lookup_enabler_table_id > DBAL_NOF_TABLES ||
        ((int) field_map_stage_info.lookup_enabler_table_id) < 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "lookup DBAL table for stage:%s (%d) is %d, invalid value.",
                     dnx_field_stage_text(unit, dnx_stage), dnx_stage, field_map_stage_info.lookup_enabler_table_id);
    }
    ...
    dnx_field_dbal_entry_t * field_dbal_entry_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_context_param_info_get_dispatch
                    (unit, bcm_context_param_set_p->param_type, bcm_stage, &dnx_context_param_set));
    ...
    dbal_enum_value_field_context_hash_function_e * dbal_hash_func_p)
{
    ...
    if ((bcm_hash_func >= bcmFieldContextHashFunctionSpnResult0
         || bcm_hash_func < bcmFieldContextHashFunctionFirstReserved))
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Invalid Hash Function type: %d . Valid ranges from: %d to %d \n",
                     bcm_hash_func, bcmFieldContextHashFunctionFirst, (bcmFieldContextHashFunctionSpnResult0 - 1));
    }
...
{
    dnx_field_range_map_t range_map_legacy;
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(range_type_info_p, _SHR_E_PARAM, "range_type_info_p");

    SHR_IF_ERR_EXIT(dnx_field_range_map_legacy_get_dispatch(unit, dnx_range_type, field_stage, &range_map_legacy));
    if (range_map_legacy.table_id == DBAL_TABLE_EMPTY)
    {
     ...
     * Fetch table id
     */
    range_type_info_p->table_id = range_map_legacy.table_id;
    /**
     *  Fill the dbal fields.
     */
    range_type_info_p->range_id_dbal_field = range_map_legacy.range_id_dbal_field;
    range_type_info_p->range_min_dbal_field = range_map_legacy.range_min_dbal_field;
    range_type_info_p->range_max_dbal_field = range_map_legacy.range_max_dbal_field;

    /*
     * Indicate whether the HW returns the range ID or a bitmap of range IDs.
     * Used to determine the default value for the range.
     */
    range_type_info_p->id_not_bitmap = range_map_legacy.id_not_bitmap;
    ...
    uint32 *opcode_id_p)
{
    ...
    else
    {
        ...
        if (is_standard_image)
        {
            ...
            rv = dnx_field_map_standard_1_predefined_apptype_to_opcode_get_dispatch(unit, app_type, &opcode_id);
            if (rv == _SHR_E_NOT_FOUND)
            {
                SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
                SHR_EXIT();
        ...
        }
        {
            ...
            rv = dnx_field_map_static_opcode_to_apptype_get_dispatch(unit, opcode_id, &mapped_apptype);
            if (rv == _SHR_E_NOT_FOUND)
            {
                SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
                SHR_EXIT();
    ...
    if (rv == _SHR_E_NOT_FOUND)
    {
        ...
        else
        {
            ...
            if (is_standard_image)
            {
                ...
                rv_apptype_map =
                    dnx_field_map_standard_1_predefined_apptype_to_opcode_get_dispatch(unit, app_type, &opcode_id);
            ...
            }
            {
                ...
                bcm_field_AppType_t mapped_apptype;
                rv_opcode_map = dnx_field_map_static_opcode_to_apptype_get_dispatch(unit, opcode_id, &mapped_apptype);
    ...
    bcm_field_AppType_t * app_type_p)
{
    ...
    else
    {
        ...
        rv = dnx_field_map_static_opcode_to_apptype_get_dispatch(unit, opcode_id, &app_type);
        if (rv == _SHR_E_NOT_FOUND)
        {
    ...
    uint32 *cs_profile_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_static_opcode_to_apptype_get_dispatch(unit, opcode_id, &app_type));
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_static_opcode_to_cs_profile_get_dispatch(unit, opcode_id, &cs_profile));
    ...
    dnx_field_context_t acl_contexts[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE])
{
        ...
        for (context_index = 0; context_index < nof_contexts; context_index++)
        {
            ...
            if (((unsigned int) fwd_context) >= nof_contexts_stage)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Apptype %d (\"%s\") opcode %d context number %d is %d. "
                             ...
                             apptype, dnx_field_bcm_apptype_text(unit, apptype), opcode_id, context_index, fwd_context,
                             nof_contexts_stage);
            }
    ...
    else
    {
        ...
        DNX_TCAM_EXTERNAL_TCAM_FUNCTION(unit,
                                        dnx_field_kbp_opcode_to_contexts(unit, opcode_id, nof_contexts_p, fwd_contexts,
                                                                         acl_contexts));
    }

    if (dnx_data_field.features.apptype_based_on_fwd2_get(unit) == FALSE)
    {
    ...
    int *device_locked_p)
{
    ...
    DNX_TCAM_EXTERNAL_TCAM_FUNCTION(unit, dnx_field_kbp_is_device_locked(unit, device_locked_p));
    ...
    bcm_field_AppType_t * app_type_p)
{
...

    {
        uint32 opcode_id;
        /** Sanity check. */
        if (fwd_context != ((uint8) fwd_context))
        {
        ...
        DNX_TCAM_EXTERNAL_TCAM_FUNCTION(unit,
                                        dnx_field_kbp_contexts_to_opcode_get(unit, fwd_context, acl_context,
                                                                             &opcode_id));
...
{
    dnx_field_pp_app_map_t pp_app_map;
    SHR_FUNC_INIT_VARS(unit);

    SHR_NULL_CHECK(dbal_table_id_p, _SHR_E_PARAM, "dbal_table_id_p");

    SHR_IF_ERR_EXIT(dnx_field_map_pp_app_map_get_dispatch(unit, bcm_pp_app, &pp_app_map));
    if (!pp_app_map.valid)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "No valid table found for the given PP APP DB ID (%d)\n", bcm_pp_app);
    }

    *dbal_table_id_p = pp_app_map.dbal_table_id;
