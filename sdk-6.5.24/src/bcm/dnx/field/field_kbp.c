    ...
    int *offset_on_last_key_p)
{
    ...
    if (nof_fwd_contexts <= 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "apptype %d had %d FWD contexts.\n", apptype, nof_fwd_contexts);
    }
    ...
    int unit)
{
    ...
    if (nof_ones_bmp > DBAL_NOF_ENUM_KBP_KBR_IDX_VALUES)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "The KBP key bitmap 0x%x has %d ones, maximum is %d.\n",
                     kbp_key_bmp, nof_ones_bmp, DBAL_NOF_ENUM_KBP_KBR_IDX_VALUES);
    }
    ...
    bcm_field_AppType_t * apptype_p)
{
    ...
    DNX_FIELD_KBP_OPCODE_ITERATOR(opcode_id)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_static_opcode_to_apptype_get_dispatch(unit, opcode_id, &app_type_internal));
        if (sal_strncmp
    ...
    uint32 algorithmic_mode)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_db_create(unit, counters_enable, dbal_table_id, initial_capacity, algo_mode));
    ...
    uint32 *algorithmic_mode_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_id_get(unit, fg_id, &dbal_table_id));
    SHR_IF_ERR_EXIT(kbp_mngr_db_properties_get(unit, dbal_table_id, counters_enable_p, &initial_capacity, &algo_mode));
    ...
    unsigned int actions_size_in_bits)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_kbp_db_create(unit, counters_enable, fg_info_p->dbal_table_id,
