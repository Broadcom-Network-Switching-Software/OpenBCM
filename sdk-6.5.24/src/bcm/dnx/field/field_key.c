    ...
    dnx_field_key_attached_qual_info_t dnx_qual_info[DNX_FIELD_KEY_MAX_SUB_QUAL])
{
    ...
    if (qual_attach_info_p->input_type == DNX_FIELD_INPUT_TYPE_EXTERNAL)
    {
        DNX_TCAM_EXTERNAL_TCAM_FUNCTION(unit,
                                        dnx_field_kbp_key_qual_info_get_verify(unit, field_stage, qual_type,
                                                                               qual_attach_info_p));
    }
    ...
    dnx_field_key_attached_qual_info_t dnx_qual_info[DNX_FIELD_KEY_MAX_SUB_QUAL])
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_key_qual_info_get_verify
                           (unit, field_stage, context_id, qual_type, offset_on_key, qual_attach_info_p,
    ...
    switch (key_in_info_p->field_stage)
    {
        ...
        case DNX_FIELD_STAGE_IPMF2:
        {
            mdb_dt_key_id = dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_keys_alloc +
                dnx_data_field.map.ipmf2_key_select_get(unit);
            break;
        }
        case DNX_FIELD_STAGE_IPMF3:
        {
        ...
        if (dnx_quals[qual_index] != DNX_FIELD_QUAL_TYPE_INVALID)
        {
            ...
            switch (cur_input_type)
            {
                ...
                case DNX_FIELD_INPUT_TYPE_META_DATA:
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size(unit, field_stage, dnx_quals[qual_index],
                                                                NULL, NULL, &qual_size));
                ...
                case DNX_FIELD_INPUT_TYPE_EXTERNAL:
                {
                    ...
                    if (cur_class == DNX_FIELD_QUAL_CLASS_USER && qual_info[qual_index].offset < 0)
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "The supplied offset(%d) for user defined qual %s in FG %d context %d is negative. "
                                     "Offset may not be negative for user defined qualifier with input type external.\n",
                                     qual_info[qual_index].offset, dnx_field_dnx_qual_text(unit, dnx_quals[qual_index]),
                                     fg_id, context_id);
                    }
                    break;
