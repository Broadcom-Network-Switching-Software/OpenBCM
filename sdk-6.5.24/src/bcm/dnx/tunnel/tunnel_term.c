...
#include <src/bcm/dnx/flow/porting/tunnel_term_flow.h>
...
#define DNX_TUNNEL_TERM_TYPE_IS_VXLAN(_tunnel_type)              ((_tunnel_type == bcmTunnelTypeVxlan)    || (_tunnel_type == bcmTunnelTypeVxlan6) || (_tunnel_type == bcmTunnelTypeVxlanEsp))
...
#define DNX_TUNNEL_TERM_TYPE_IS_VXLAN_OR_VXLAN_GPE(_tunnel_type) (DNX_TUNNEL_TERM_TYPE_IS_VXLAN(_tunnel_type) || DNX_TUNNEL_TERM_TYPE_IS_VXLAN_GPE(_tunnel_type))
 ...
 */
static shr_error_e
dnx_tunnel_terminator_tunnel_vxlan_type_verify(
    int unit,
    bcm_tunnel_terminator_t * info,
    uint32 fodo_assignment_mode)
{
    ...
    {
        uint8 next_set_type_is_vxlan, previous_set_type_is_vxlan;

        previous_set_type_is_vxlan =
            (fodo_assignment_mode == DBAL_ENUM_FVAL_FORWARD_DOMAIN_ASSIGNMENT_MODE_ENUM_FORWARD_DOMAIN_FROM_LOOKUP);
        next_set_type_is_vxlan = DNX_TUNNEL_TERM_TYPE_IS_VXLAN_OR_VXLAN_GPE(info->type);

        if (previous_set_type_is_vxlan && (!next_set_type_is_vxlan))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "BCM_TUNNEL_REPLACE vxlan tunnel type can't be replaced with non-vxlan tunnel type");
        }
        if ((!previous_set_type_is_vxlan) && next_set_type_is_vxlan)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "BCM_TUNNEL_REPLACE non-vxlan tunnel type can't be replaced with vxlan tunnel type");
        }
    }
 ...
 */
static shr_error_e
dnx_tunnel_terminator_type_to_ipv6_additional_headers_internal(
    int unit,
    bcm_tunnel_type_t tunnel_type,
    ...
    uint32 *second_additional_header)
{
    ...
    if (DNX_TUNNEL_TERM_SRV6_IS_STRICT_SRV6_TYPE(tunnel_type))
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_type_to_ipv6_additional_headers
                        (unit, tunnel_type, first_additional_header, second_additional_header));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_type_to_ipv6_additional_headers_internal
                        (unit, tunnel_type, first_additional_header, second_additional_header));
    }
exit:
    SHR_FUNC_EXIT;
 ...
 */
static shr_error_e
dnx_tunnel_terminator_additional_headers_to_ipv6_type_internal(
    int unit,
    uint32 first_additional_header,
    uint32 second_additional_header,
    bcm_tunnel_type_t * tunnel_type)
    ...
    bcm_tunnel_type_t * tunnel_type)
{
    ...
    if (DNX_TUNNEL_TERM_SRV6_IS_SRV6_ADDITIONAL_HEADER(first_additional_header, second_additional_header))
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_additional_headers_to_ipv6_type
                        (unit, first_additional_header, second_additional_header, tunnel_type));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_additional_headers_to_ipv6_type_internal
                        (unit, first_additional_header, second_additional_header, tunnel_type));
    }
exit:
    SHR_FUNC_EXIT;
    ...
    dnx_tunnel_term_ipv6_tcam_table_key_t * tcam_table_key)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_type_to_ipv6_additional_headers
                    (unit, tcam_table_key->type, &first_additional_header, &second_additional_header));
    ...
    dnx_tunnel_term_ipv6_tcam_table_result_t * tcam_table_result)
{
    ...
    dnx_tunnel_terminator_srv6_to_ipv6_tcam_table_result_dbal_field(info, &tcam_table_result->result_dbal_field);
}
...
dnx_tunnel_terminator_ipv6_tcam_result_get(
    int unit,
    uint32 entry_handle_id,
    dbal_fields_e * tcam_result_sub_field_id,
    uint32 *local_inlif)
{
    uint32 encoded_tcam_result;
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_IRPP_CROSS_STAGE_VAR_2, INST_SINGLE, &encoded_tcam_result));
    SHR_IF_ERR_EXIT(dbal_fields_uint32_sub_field_info_get
                    (unit, DBAL_FIELD_IRPP_CROSS_STAGE_VAR_2, encoded_tcam_result, tcam_result_sub_field_id,
                     local_inlif));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_tunnel_type_t * tunnel_type)
{
    ...
    *vrf = vrf_value;
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_ipv6_tcam_key_field_tunnel_type(unit, entry_handle_id, tunnel_type));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_vlan_t default_vsi)
{
    ...
    else if (DNX_TUNNEL_TERM_TYPE_IS_VXLAN(info->type) && (default_vsi))
    {
    ...
    uint32 *local_inlif)
{
     ...
     */
    if (DNX_TUNNEL_TERM_TYPE_IS_VXLAN_OR_VXLAN_GPE(info->type))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO_ASSIGNMENT_MODE, INST_SINGLE,
                                     fodo_assignment_mode);
    }

    /** for vxlan, we set the learn enable as TRUE only at lif creation */
    if (DNX_TUNNEL_TERM_TYPE_IS_VXLAN(info->type) && !is_replace)
    {
    ...
    dnx_tunnel_term_inlif_info_t * tunnel_term_inlif_info)
{
                    ...
                    (unit, _SHR_CORE_ALL, local_inlif, entry_handle_id, &lif_table_mngr_inlif_info));
    {
        ...
        if (is_field_valid == TRUE)
        {
            ...
            dnx_tunnel_terminator_srv6_optional_micro_segment_id_flag_set(service_per_flow,
                                                                          &tunnel_term_inlif_info->flags);
        }
    ...
    uint32 *fodo_assignment_mode)
{
     ...
     */
    if (DNX_TUNNEL_TERM_TYPE_IS_VXLAN_OR_VXLAN_GPE(tunnel_type))
    {
        *fodo_assignment_mode = DBAL_ENUM_FVAL_FORWARD_DOMAIN_ASSIGNMENT_MODE_ENUM_FORWARD_DOMAIN_FROM_LOOKUP;
    }
    else
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    if (info->type == bcmTunnelTypeGreAnyIn4)
    {
        if (dnx_data_flow.ipv4.use_flow_lif_term_get(unit))
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_terminator_create_flow(unit, info));
            SHR_EXIT();
        }
    }
    ...
    if (!_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_REPLACE))
    {
        ...
        dnx_tunnel_terminator_srv6_optional_in_lif_profile_info_set(info, &in_lif_profile_info);
        ...
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_gport_to_sid_format_add(unit, info));
        }
    }
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_tunnel_vxlan_type_verify
                        (unit, info, tunnel_term_inlif_info.fodo_assignment_mode));
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    if (info->type == bcmTunnelTypeGre4In4)
    {
        if (dnx_data_flow.ipv4.use_flow_lif_term_get(unit))
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_terminator_get_flow(unit, info));
            SHR_EXIT();
        }
    }
    ...
    /** VSI could be set only with tunnel types of bcmTunnelTypeEthSR6, bcmTunnelTypeEthIn6 and bcmTunnelTypeEthCascadedFunct*/
    if (DNX_TUNNEL_TERM_SRV6_IS_L2_VPN(info->type))
    {
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    if (info->type == bcmTunnelTypeGre4In4)
    {
        if (dnx_data_flow.ipv4.use_flow_lif_term_get(unit))
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_terminator_delete_flow(unit, info));
            SHR_EXIT();
        }
    }
...
dnx_tunnel_terminator_ipv6_tcam_table_result_is_lif(
    ...
    uint8 *result_is_lif)
{
    *result_is_lif = UTILEX_NUM2BOOL(result_dbal_field == DBAL_FIELD_IN_LIF);
}
    ...
    uint32 *local_inlif)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_ipv6_tcam_result_get(unit, entry_handle_id, &sub_field_id, local_inlif));
    dnx_tunnel_terminator_srv6_ipv6_tcam_table_result_is_lif(sub_field_id, &srv6_result_is_local_inlif);
    dnx_tunnel_terminator_ipv6_tcam_table_result_is_lif(sub_field_id, &result_is_local_inlif_internal);
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    else if ((dbal_table == DBAL_TABLE_IPV6_MP_TT_TCAM_2ND_PASS) || (dbal_table == DBAL_TABLE_IPV6_MP_TT_TCAM_BASIC))
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_ipv6_tcam_key_fields
                        (unit, entry_handle_id, &info->dip6, &info->dip6_mask, &info->vrf, &info->type));
    }
    ...
    bcm_tunnel_terminator_t * info)
{
     ...
     * bcmTunnelTypeEthIn6, bcmTunnelTypeEthSR6 or bcmTunnelTypeCascadedFunct*/
    ...
    if ((dbal_table == DBAL_TABLE_IPV6_MP_TT_TCAM_2ND_PASS) || (dbal_table == DBAL_TABLE_IPV6_MP_TT_TCAM_BASIC))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_optional_sid_format_flag_get
