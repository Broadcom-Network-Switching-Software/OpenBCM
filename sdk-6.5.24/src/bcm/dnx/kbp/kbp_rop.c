...
#define DNX_KBP_OP_LAST_RESPONSE_PORT0_REG_ADDR       0xf16d
#define DNX_KBP_OP_LAST_RESPONSE_PORT1_REG_ADDR       0xf1f2
    ...
    dnx_kbp_lut_data_t * lut_data)
{
    ...
    DNX_KBP_TRY(dnx_kbp_callback_mdio_write
    ...
    DNX_KBP_TRY(dnx_kbp_callback_mdio_read
    ...
    dnx_kbp_rop_cbw_cmp_t * cbw_cmp_data)
{
    ...
    SOC_REG_ABOVE_64_CLEAR(msb_data);
    SOC_REG_ABOVE_64_CLEAR(lsb_data);
    SOC_REG_ABOVE_64_CLEAR(above_64_cmp_data);
    ...
    uint32 *index)
{
    ...
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OFFSET, offset);
    }
    ...
    if ((dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_is_egw_support_rr)) && type == DNX_KBP_ROP_REQUEST)
    {
        hw_index_dec = 2;
    }
    else
    ...
    *index =
        (fifo_index ==
         0) ? (dnx_data_elk.general.rop_fifo_depth_get(unit) - hw_index_dec) : (fifo_index - hw_index_dec);
    ...
    dnx_kbp_rop_packet_t * pkt)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_rop_fifo_index_get(unit, core, offset, type, &fifo_index));
    ...
    pkt->sequence_num = bcm_ntohs(pkt->sequence_num);
    ...
    dnx_kbp_rop_packet_t * pkt)
{
    ...
    {
        DNX_KBP_TRY(dnx_kbp_callback_mdio_read
                    (&Kbp_app_data[unit]->user_data[0], 0, DNX_KBP_OP_LAST_RESPONSE_REG_DEVAD, reg_addr, &reg_data[i]));
        reg_addr++;
    ...
    dnx_kbp_rop_packet_t * pkt)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_kbp_rop_last_get_verify(unit, core, flags, pkt));

    sal_memset(pkt, 0, sizeof(dnx_kbp_rop_packet_t));

    if (flags & DNX_KBP_ROP_GET_DNX_SIDE)
    {
        SHR_IF_ERR_EXIT(dnx_kbp_rop_last_dnx_get(unit, core, offset, DNX_KBP_ROP_REQUEST, pkt));
    }
    else
    {
    ...
    dnx_kbp_rop_packet_t * pkt)
{
    ...
    if (flags & DNX_KBP_ROP_GET_DNX_SIDE)
    {
        SHR_IF_ERR_EXIT(dnx_kbp_rop_last_dnx_get(unit, core, offset, DNX_KBP_ROP_RESPONSE, pkt));
    }
    else
    {
