    ...
    int unit)
{
    ...
    /*
     * Iterate over the supported PDQ contexts and only enable FCs for HP and LP coming from the relevant DTQs 
     */
    for (idx = 0; idx < dnx_data_fabric.transmit.dtq_to_pdq_fc_mapping_info_get(unit)->key_size[0]; idx++)
    {
        /*
         * Some configuration are valid and some not based on mode (FE/MESH). Handled inside dnx data.
         */
        if (dnx_data_fabric.transmit.dtq_to_pdq_fc_mapping_get(unit, idx)->valid)
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FABRIC_PDQ_CONTEXT,
                                       dnx_data_fabric.transmit.dtq_to_pdq_fc_mapping_get(unit, idx)->context);
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FABRIC_DTQ_FC,
                                       dnx_data_fabric.transmit.dtq_to_pdq_fc_mapping_get(unit, idx)->fc);

            switch (dnx_data_fabric.dtqs.nof_active_dtqs_get(unit))
            {
                case 1:
                {
                    field = dnx_data_fabric.transmit.dtq_to_pdq_fc_mapping_get(unit, idx)->single_dtq;
                    break;
                }
                case 2:
                {
                    field = dnx_data_fabric.transmit.dtq_to_pdq_fc_mapping_get(unit, idx)->dual_dtq;
                    break;
                }
                case 3:
                {
                    field = dnx_data_fabric.transmit.dtq_to_pdq_fc_mapping_get(unit, idx)->triple_dtq;
                    break;
                }
                default:
                {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, field, INST_SINGLE, 1);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

            /*
             * We clear the configured field so it won't be enabled wrongly on the next iterration.
             */
    ...
    int unit)
{
    ...
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_blocks_exist))
    {
        ...
        for (dtq_fc = 0; dtq_fc < DBAL_NOF_ENUM_FABRIC_DTQ_FC_VALUES; ++dtq_fc)
        {
            ...
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FABRIC_DTQ_FC, dtq_fc);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_THRESHOLD, INST_SINGLE,
                                         dnx_data_fabric.transmit.dtq_fc_fabric_thresholds_get(unit,
                                                                                               dtq_fc)->threshold);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }
        ...
        for (dtq_ocb_fc = 0; dtq_ocb_fc < DBAL_NOF_ENUM_FABRIC_DTQ_OCB_FC_VALUES; ++dtq_ocb_fc)
        {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_THRESHOLD, INST_SINGLE,
                                         dnx_data_fabric.transmit.dtq_fc_ocb_thresholds_get(unit,
                                                                                            dtq_ocb_fc)->threshold);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }
    }
    ...
    int unit)
{
    ...
    if (dnx_data_fabric.general.connect_mode_get(unit) == DNX_FABRIC_CONNECT_MODE_FE)
    {
        switch (dnx_data_fabric.dtqs.nof_active_dtqs_get(unit))
        {
            ...
            {
                /*
                 * Disable FC from unused DTQs based on max nof dtqs.
                 */
                SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                                (unit, DBAL_TABLE_FABRIC_TRANSMIT_RDF_DTQ_FC_ENABLE, &entry_handle_id));
                for (dtq_fc = 0; dtq_fc < DBAL_NOF_ENUM_FABRIC_DTQ_FC_VALUES; ++dtq_fc)
                {
                    int dtq = 0;
                    dbal_fields_e dtq_fc_enable_fields[3] =
                        { DBAL_FIELD_FABRIC_0_DTQ_FC_ENABLE, DBAL_FIELD_FABRIC_1_DTQ_FC_ENABLE,
                        DBAL_FIELD_FABRIC_2_DTQ_FC_ENABLE
                    };

                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FABRIC_DTQ_FC, dtq_fc);

                    for (dtq = 1; dtq < dnx_data_fabric.dtqs.max_nof_dtqs_get(unit); ++dtq)
                    {
                        dbal_entry_value_field32_set(unit, entry_handle_id, dtq_fc_enable_fields[dtq], INST_SINGLE, 0);
                    }
    ...
    int unit)
{
    ...
    /*
     * If feature is available this configuration will be done in fabric cgm tuning step. 
     */
