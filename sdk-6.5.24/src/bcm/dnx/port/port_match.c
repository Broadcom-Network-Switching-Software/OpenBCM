                ...
                if (BCM_GPORT_IS_SET(port))
                {
                         ...
                         BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(port)))
                    {
                        ...
                        SHR_IF_ERR_EXIT(algo_port_pp_db.
                                        esem_default_result_profile.is_allocated(unit, esem_default_result_profile,
                                                                                 &is_allocated));
                        if (is_allocated == FALSE)
                        {
                            SHR_ERR_EXIT(_SHR_E_PARAM,
                                         "Given port(0x%08X) is not a valid virtual egress default port.\n", port);
                        }
                    }
                    else
                    {
    ...
    int *is_match_exist)
{
    ...
    if (is_ingress == TRUE)
    {
        ...
        if (port != BCM_GPORT_INVALID)
        {
            SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
                            (unit, entry_handle_id, DBAL_FIELD_GPORT, 0, DBAL_CONDITION_EQUAL_TO, (uint32 *) &port,
                             NULL));
        }
    ...
    else
    {
        ...
        else
        {
            ...
            /** Get the port esem cmd */
            SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_port_get(unit, match_info->port, &port_esem_cmd));
            ...
            if ((port_esem_cmd_data.esem[ESEM_ACCESS_IF_1].valid &&
                ...
                (port_esem_cmd_data.esem[ESEM_ACCESS_IF_3].valid &&
                 port_esem_cmd_data.esem[ESEM_ACCESS_IF_3].default_result_profile == esem_default_profile))
            {
                match_found = TRUE;
            }
        }
    ...
    bcm_port_match_info_t * match_info)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Wrong setting! match = %d with these keys for port = %d does not exist !\n",
                     match_info->match, port);
    }
    ...
    dnx_port_match_esem_cmd_oper_e add_or_delete)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_port_hw_set(unit, match_info->port, new_esem_cmd));
    ...
    int *count)
{
    ...
    *count = 0;
    ...
    for (esem_cmd = 0; esem_cmd < dnx_data_esem.access_cmd.nof_cmds_get(unit); esem_cmd++)
    {
        ...
        /** Check if the result profile was used by the cmd in any access interface.*/
        if ((esem_cmd_data.esem[ESEM_ACCESS_IF_1].valid &&
             esem_cmd_data.esem[ESEM_ACCESS_IF_1].default_result_profile == esem_default_profile) ||
            (esem_cmd_data.esem[ESEM_ACCESS_IF_2].valid &&
             esem_cmd_data.esem[ESEM_ACCESS_IF_2].default_result_profile == esem_default_profile) ||
            (esem_cmd_data.esem[ESEM_ACCESS_IF_3].valid &&
             esem_cmd_data.esem[ESEM_ACCESS_IF_3].default_result_profile == esem_default_profile))
        {
    ...
    bcm_port_match_info_t * match_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_match_add_verify(unit, port, match_info));
    ...
    if (_SHR_IS_FLAG_SET(match_info->flags, BCM_PORT_MATCH_INGRESS_ONLY))
    {
        if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use) &&
            !(_SHR_IS_FLAG_SET(match_info->flags, BCM_PORT_MATCH_NATIVE)))
        {
            SHR_IF_ERR_EXIT(dnx_port_match_create_flow(unit, port, match_info));
        }
        else
        {
    ...
    bcm_port_match_info_t * match_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_match_delete_verify(unit, port, match_info));
    ...
    if (_SHR_IS_FLAG_SET(match_info->flags, BCM_PORT_MATCH_INGRESS_ONLY))
    {
        if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use) &&
            !(_SHR_IS_FLAG_SET(match_info->flags, BCM_PORT_MATCH_NATIVE)))
        {
            SHR_IF_ERR_EXIT(dnx_port_match_clear_flow(unit, port, match_info));
            SHR_EXIT();
        }
        /*
         * Convert the match struct in order to use the VLAN-Port set function
    ...
    bcm_gport_t port)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_match_delete_all_verify(unit, port));
    ...
    int *count)
{
     ...
     * Verify the inputs
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_match_multi_get_verify(unit, port, size, match_array, count));
    ...
    bcm_port_match_info_t * new_match)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_match_replace_verify(unit, port, old_match, new_match));
    ...
    bcm_port_match_info_t * match_array)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_match_set_verify(unit, port, size, match_array));
    ...
    uint8 enable)
{
    ...
    if (enable || svtag_present)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_port_hw_set(unit, gport, new_esem_cmd));
    ...
    uint32 new_local_in_lif_id)
{
    ...
    else
    {
        if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif(unit, algo_flags, 0, old_local_in_lif_id, &gport));
        }
        else
        {
