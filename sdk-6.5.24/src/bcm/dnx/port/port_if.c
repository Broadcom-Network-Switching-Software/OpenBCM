    ...
    int *bit_times)
{
     ...
     * Verify input parameters
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_ifg_verify(unit, port, duplex, bit_times));
    SHR_IF_ERR_EXIT(dnx_port_logical_get(unit, port, &logical_port));
    ...
    bcm_port_resource_t * resource)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "total speed in the device for core%d (%dMbps) exceed the egress TM limitation(%dMbps).",
    ...
    dnx_port_resource_port_info_t * ports_info)
{
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_calendar_refresh(unit, BCM_CORE_ALL));
            break;
        }
    }
    ...
    dnx_port_resource_port_info_t * ports_info)
{
    ...
    int total_special_if_speed = 0, rcy_speed_i = 0;
    int has_speed;
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
            ...
            && (resource[resource_index].speed != 0) && ports_info[resource_index].is_tx_speed_change)
        {
            ...
            if ((!DNX_ALGO_PORT_TYPE_IS_NIF(unit, ports_info[resource_index].port_info, TRUE, TRUE, TRUE, TRUE))
                && (!DNX_ALGO_PORT_TYPE_IS_FLEXE(unit, ports_info[resource_index].port_info, TRUE, TRUE)))
            {
                if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, ports_info[resource_index].port_info))
                {
                    /** For RCY interfaces, this function must be called with the total speed for both RCY interfaces */
                    total_special_if_speed = 0;
                    SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, resource[resource_index].port, &core));
                    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    ...
                    {
                        SHR_IF_ERR_EXIT(dnx_algo_port_has_speed
                                        (unit, rcy_port_i, DNX_ALGO_PORT_SPEED_F_TX, &has_speed));
                        if (has_speed == TRUE)
                        {
                            SHR_IF_ERR_EXIT(dnx_algo_port_interface_rate_get
                ...
                {
                    total_special_if_speed = resource[resource_index].speed;
                }
                SHR_IF_ERR_EXIT(dnx_egr_queuing_special_if_credit_set
                                (unit, resource[resource_index].port, total_special_if_speed));
            }
        }
    }
    /*
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, ports_info[resource_index].port_info))
        {
            SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_calendar_activate(unit, BCM_CORE_ALL));
            break;
        }
    }
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        /** STIF - FALSE, L1 - TRUE */
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index].port_info, FALSE, TRUE)
            || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, ports_info[resource_index].port_info, TRUE)
            /** ELK - FALSE, L1 - TRUE */
            || DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, ports_info[resource_index].port_info, FALSE, TRUE))
        {
    ...
    bcm_port_resource_t * resource)
{
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        {
            port_to_remove =
                (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index].port_info, FALSE, FALSE)) ?
    ...
    SHR_IF_ERR_EXIT(imb_common_post_resource_multi_set(unit, nport, resource));
    ...
    int *remote_fault)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_remote_local_fault_verify(unit, port, local_fault));
    SHR_INVOKE_VERIFY_DNXC(dnx_port_remote_local_fault_verify(unit, port, remote_fault));
    SHR_IF_ERR_EXIT(dnx_port_logical_get(unit, port, &logical_port));
    is_cdu = 0;
#ifdef BCM_DNX2_SUPPORT
    /** get port info */
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, FALSE, TRUE))
    {
        ...
        if (DNX_ALGO_PORT_IMB_TYPE_IS_CDU(ethu_info.imb_type))
        {
            is_cdu = 1;
        }
    }
    ...
    {
        /*
         * For CDU, get remote fault may result in clearing local fault 
         */
        SHR_IF_ERR_EXIT(imb_port_faults_status_get(unit, logical_port, local_fault, remote_fault));
    }
    else
    {
        SHR_IF_ERR_EXIT(imb_port_local_fault_status_get(unit, logical_port, local_fault));
        SHR_IF_ERR_EXIT(imb_port_remote_fault_status_get(unit, logical_port, remote_fault));
    }
    ...
    int *value)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_remote_local_fault_verify(unit, port, value));
    SHR_IF_ERR_EXIT(dnx_port_logical_get(unit, port, &logical_port));
    /** call imb layer  */
    ...
    int *enable)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_remote_local_fault_verify(unit, port, enable));
    SHR_IF_ERR_EXIT(dnx_port_logical_get(unit, port, &logical_port));
    /** call imb layer  */
...
/**
 * \brief - verify parameters for link fault force APIs
 */
static shr_error_e
dnx_port_link_fault_force_verify(
    int unit,
    int port,
    int *enable,
    int is_set)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Wrong port type, only FlexE MAC type is supported!\r\n");
    }
    if (is_set)
    {
    ...
    int enable)
{
    SHR_FUNC_INIT_VARS(unit);
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_link_fault_force_verify(unit, port, &enable, 1));
    /** call imb layer  */
    SHR_IF_ERR_EXIT(imb_port_link_fault_force_set(unit, port, type, enable));
exit:
    SHR_FUNC_EXIT;
}
/**
    ...
    int *enable)
{
    SHR_FUNC_INIT_VARS(unit);
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_link_fault_force_verify(unit, port, enable, 0));
    /** call imb layer  */
    SHR_IF_ERR_EXIT(imb_port_link_fault_force_get(unit, port, type, enable));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *start_tx)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    if (!DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, port_info, 1     /* L1 */
        ))
    {
...
static shr_error_e
dnx_port_fabric_control_verify(
    int unit,
    bcm_port_t port,
    ...
    int value)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_fabric_control_verify(unit, port, type));
    ...
    bcm_port_flr_timeout_t timeout)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0, 1) && !DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info))
    {
    ...
    bcm_port_flr_timeout_t timeout)
{
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_port_fast_link_recovery_enable_verify(unit, port, enable, timeout));
    /*
     * call api
     */
