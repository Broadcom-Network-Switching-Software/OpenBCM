    ...
    int enable)
{
    ...
    {
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(portmod_ilkn_port_lane_enable(unit, port, IMB_ILE_DIRECTION_RX, enable));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
    ...
    switch (type)
    {
        ...
        case BCM_PORT_PHY_CONTROL_TX_PPM_ADJUST:
        {
            if ((pm_type == portmodDispatchTypePm8x50)
                && !dnx_data_nif.features.feature_get(unit, dnx_data_nif_features_txpi_supported_in_pm8x50))
            {
