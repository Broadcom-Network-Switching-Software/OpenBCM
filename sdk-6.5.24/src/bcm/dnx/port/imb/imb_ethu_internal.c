    ...
    uint32 enable)
{
    ...
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_qmlf_fc_high_and_low_rmcs_support) == 0)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, ethu_id_in_core);
    }
    else
    {
        ...
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_RMC_PRIO, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    }
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_PRIORITY, priority);
    ...
    uint32 *enable)
{
    ...
    else
    {
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RMC_PRIO, 0);
    }
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_PRIORITY, priority);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RMC, rmc);
 ...
 */
int
imb_ethu_global_to_local_id_and_core_get(
    int unit,
    int inst_id,
    bcm_core_t * core,
    int *ethu_id_in_core)
{
    ...
    if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_is_ethu_nof_per_core_supported))
    {
        *core = dnx_data_nif.eth.ethu_per_core_get(unit, inst_id)->core_id;
        *ethu_id_in_core = inst_id - dnx_data_nif.eth.nif_cores_ethus_get(unit, *core)->first_ethu;
    }
    else
    {
        *core = inst_id >= dnx_data_nif.eth.ethu_nof_per_core_get(unit) ? 1 : 0;
        *ethu_id_in_core = inst_id % dnx_data_nif.eth.ethu_nof_per_core_get(unit);
    }
