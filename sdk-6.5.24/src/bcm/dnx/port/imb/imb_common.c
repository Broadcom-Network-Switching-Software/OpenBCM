    ...
    const bcm_port_resource_t * resource)
{
    ...
    uint8 nif_eth_framer_port_exist = FALSE;
    int resource_index;
    SHR_FUNC_INIT_VARS(unit);
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index], FALSE, TRUE) ||
            DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, ports_info[resource_index], FALSE))
        {
            nif_eth_framer_port_exist = TRUE;
        }
    }
    ...
    if (dnx_data_nif.arb.feature_get(unit, dnx_data_nif_arb_is_supported))
    {
         ...
         */
        if (nif_eth_framer_port_exist)
        {
            SHR_IF_ERR_EXIT(dnx_port_arb_calendar_set(unit, (ARB_RX_DIRECTION | ARB_TX_DIRECTION)));
        }
    ...
    if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        ...
        for (resource_index = 0; resource_index < nport; ++resource_index)
        {
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index], FALSE, TRUE)
                || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, ports_info[resource_index], FALSE)
                            /** ELK - FALSE, L1 - TRUE */
                || DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, ports_info[resource_index], FALSE, TRUE))
            {
    ...
    int link)
{
    ...
    if (DNX_ALGO_PORT_IMB_TYPE_IS_CDU(ethu_info.imb_type))
    {
        ...
        if ((an_status.enabled && an_status.locked && link) ||
            (an_status.enabled && link && (an_status.resolved_num_lane == DNX_DATA_MAX_NIF_ETH_NOF_LANES_IN_CDU)))
        {
