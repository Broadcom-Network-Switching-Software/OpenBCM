    ...
    int flush_enable)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_ethu_tx_egress_flush_hw_get(unit, ethu_info.core, ethu_info.lane_in_core, &orig_enable));
    /*
    ...
    if (enable)
    {
        ...
        if (dnx_data_macsec.general.is_macsec_enabled_get(unit) && IMB_ETHU_IS_TX_CREDIT_PORT(unit, port_info))
        {
            ...
            SHR_IF_ERR_EXIT(bcm_dnx_xflow_macsec_port_control_get
                            (unit, XFLOW_MACSEC_ENCRYPT_DECRYPT_NONE, port, bcmXflowMacsecPortEnable, &port_info));
            tx_threshold = COMPILER_64_IS_ZERO(port_info.value) ? 1 : dnx_data_macsec.general.tx_threshold_get(unit);
    ...
    int loopback)
{
    ...
    {
        SHR_IF_ERR_EXIT(portmod_port_loopback_set(unit, port, portmodLoopbackMacOuter,
                                                  (loopback == BCM_PORT_LOOPBACK_NONE ? 0 : 1)));
    ...
    uint32 in_reset)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_global_to_local_id_and_core_get(unit, imb_info->inst_id, &core, &ethu_id_in_core));
 ...
 * \param [in] unit - chip unit id
 * \param [in] imb_id - IMB ID
 * \param [in] bring_back_up - If true enable the counters after reset.
 * \return
 *   int - see _SHR_E_ *
 *
 * \remarks
    ...
    int bring_back_up)
{
    ...
    /** Set HW to put in/out of reset all instrumentation counters of the ETHU */
    SHR_IF_ERR_EXIT(imb_ethu_internal_instru_counters_reset(unit, core, ethu_id_in_core, bring_back_up));
    ...
    int *pfc_num)
{
    ...
    if (imb_type == imbDispatchTypeImb_clu)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_ethu_id_lane_info_get(unit, ethu_core, ethu_id, &first_lane, &nof_lanes_in_ethu));
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, first_lane, &first_lane_in_core));
