    ...
    bcm_port_mapping_info_t * mapping_info)
{
    ...
    if (is_channelized)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Port %d - cannot specify in_channel and out_channel must be equal for this port type \n",
                         port);
        }
    }
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, FALSE, TRUE) || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_flr_fw_support_get(unit, port, &is_flr_fw_support));
        if (is_flr_fw_support)
        {
