    ...
    int unit)
{
    ...
    if (!dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_application_v2))
    {
        ...
        {
            uint32 eei_identifer_offset;
            uint32 prefix;

            eei_identifer_offset = 20;

            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FEC_ARR_PREFIX, entry_handle_id));
            dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_FEC_TYPE,
                                      DBAL_ENUM_FVAL_FEC_TYPE_EEI_POP_COMMAND);
            prefix = DNX_EEI_IDENTIFIER_POP << eei_identifer_offset;
            ...
            prefix = DNX_EEI_IDENTIFIER_SWAP << eei_identifer_offset;
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PREFIX_VALUE, INST_SINGLE, prefix);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }
    }
 ...
 */
static shr_error_e
dnx_l3_egress_arp_bcm_api_create_arp(
    int unit,
    bcm_l3_egress_traverse_cb trav_fn,
    void *user_data)
{
    ...
    void *user_data)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_arp_bcm_api_create_arp(unit, trav_fn, user_data));
    }
