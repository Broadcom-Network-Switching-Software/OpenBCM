    ...
    dnx_l3_ecmp_profile_t * ecmp_profile)
{
    ...
    if (dnx_data_l3.ecmp.tunnel_priority_support_get(unit))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_ECMP_TUNNEL_PRIORITY_MODE, INST_SINGLE,
                         &(ecmp_profile->tunnel_priority_mode)));

        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_ECMP_TUNNEL_PRIORITY_MAP_PROFILE, INST_SINGLE,
    ...
    else
    {
         ...
         */
        ecmp_profile->consistent_hashing_id = address;
    ...
    bcm_if_t * intf_array)
{
    ...
    if (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_USER_PROFILE))
    {
        ...
        if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_REPLACE))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot update an ECMP with an user profile.\n");
        }
        if (intf_count != 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "When creating an ECMP group with an user-defined profile, dynamic mode must be RESILIENT.\n");
        }
        if (ecmp->max_paths == 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Expected max_paths value to be different than 0 when creating group with user profile.\n");
        }
    }
    /*
