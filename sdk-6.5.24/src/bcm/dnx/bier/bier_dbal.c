    ...
    uint32 region_size)
{
     ...
     */
    if (!dnx_data_bier.params.feature_get(unit, dnx_data_bier_params_egress_only))
    {
    ...
    uint32 region_size)
{
    uint32 entry_handle_id;
    uint32 bier_start_offset = 0;
    int ext_mode, copies_per_entry;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    copies_per_entry = dnx_bier_mcdb_copies_per_entry_get(unit);
    region_size /= copies_per_entry;

    /*
     * Base field is SW cached value, need to be recovered
     */
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INGRESS_OFFSET, INST_SINGLE, bier_start_offset);
    ...
    int set_size)
{
    ...
    if (ingress)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_BIER_ING_TABLE, &entry_handle_id));
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_BFR_ID, group, group + set_size - 1);
        dbal_entry_key_field_predefine_value_set(unit, entry_handle_id, DBAL_FIELD_BASE, DBAL_PREDEF_VAL_MIN_VALUE);
