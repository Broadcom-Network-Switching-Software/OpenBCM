    ...
    bcm_gport_t * voq_connector_gport)
{
    ...
    /** Generate FSM for each member of the VOQ bundle to active it*/
    for (i = 0; i < nof_remote_cores; i++)
    {
        for (cur_connector = 0; cur_connector < numq; cur_connector++)
        {
            actual_flow_id = DNX_SCH_FLOW_ID_FROM_REMOTE_CORE(flow_id + cur_connector, nof_remote_cores, i);
    ...
    for (cur_connector = 0; cur_connector < numq; cur_connector++)
    {
        ...
        for (subflow_indx = 0; subflow_indx < nof_subflows; subflow_indx++)
        {
            ...
            for (i = 0; i < nof_remote_cores; i++)
            {
                ...
                /** If flow is virtual , we also need to send FSM to deactive it*/
                if (is_virtual)
                {
                    SHR_IF_ERR_EXIT(dnx_sch_dbal_manual_fsm_generate
                                    (unit, core, actual_flow_id[i], DBAL_ENUM_FVAL_FLOW_SLOW_STATUS_OFF));
                }
            }
        }
    }
    ...
    *is_virtual = FALSE;
    ...
    if (is_allocated)
    {
        ...
        if (is_enabled == SCH_VIRTUAL_FLOWS_ENABLED)
        {
            ...
            if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport_connect.voq))
            {
                base_queue_id = BCM_GPORT_UNICAST_QUEUE_GROUP_QID_GET(gport_connect.voq);
            }
            else
            {
                base_queue_id = BCM_GPORT_MCAST_QUEUE_GROUP_GET(gport_connect.voq);
            }

            if (base_queue_id == 0)
            {
                /** Need also to verify that the flow is active */
                SHR_IF_ERR_EXIT(dnx_sch_flow_slow_status_get(unit, core, base_flow_id, &slow_status));
                if (slow_status == DBAL_ENUM_FVAL_FLOW_SLOW_STATUS_NORMAL)
                {
                    *is_virtual = TRUE;
                }
            }
