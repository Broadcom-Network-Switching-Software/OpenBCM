    ...
    if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_interdigitated_supported))
    {
        ...
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
        {
                 ...
                 region_index < dnx_data_sch.flow.nof_regions_get(unit); region_index++)
            {
                 ...
                 * Interdigitated
                 */
                SHR_IF_ERR_EXIT(dnx_scheduler_region_is_interdigitated_get
                                (unit, core, region_index, &is_interdigitated));
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INTERDIGITATED, k_idx,
                                             is_interdigitated);
            }
