    ...
    uint32 bandwidth)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_api_bandwidth_fabric_adjust_set_verify(unit, flags, key, bandwidth));
    ...
    uint32 *bandwidth)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_api_bandwidth_fabric_adjust_get_verify(unit, flags, key, bandwidth));
    ...
    bcm_cosq_slow_profile_attributes_t * attr)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_api_slow_profile_set_verify(unit, slow_level, attr));
    ...
    bcm_gport_t * gport)
{
    ...
    int fmq_mode;
    uint8 is_composite_fq_supported = 0;
    ...
    switch (params->mode)
    {
        ...
        case bcmCosqSchedulerGportModeComposite:
            if (se_type == DNX_SCH_ELEMENT_SE_TYPE_FQ)
            {
                SHR_IF_ERR_EXIT(dnx_scheduler_db.general.
                                is_composite_fq_supported.get(unit, params->core, &is_composite_fq_supported));

                if (!is_composite_fq_supported)
                {
                    if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_flex_quartet_supported))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM, "Device configuration does not support composite FQ on core %d\n",
                                     params->core);
                    }
                    else
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM, "Composite FQ is not supported on this device\n");
                    }
                }
            }
    ...
    if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_flex_quartet_supported))
    {
        switch (params->shared_order)
        {
            case bcmCosqSchedulerGportSharedOrderNone:
                SHR_IF_ERR_EXIT(dnx_scheduler_db.general.
                                non_default_flow_order.get(unit, params->core, &non_default_flow_order));

                if (((params->mode == bcmCosqSchedulerGportModeSharedDual) ||
                     (params->mode == bcmCosqSchedulerGportModeSharedQuad) ||
                     (params->mode == bcmCosqSchedulerGportModeSharedOcta)) && non_default_flow_order)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Shared shaper order should be configured for shared shaper flows when quartet order is non-default\n");
                }
                break;
            ...
            case bcmCosqSchedulerGportSharedOrderFqCl:
                if (params->mode != bcmCosqSchedulerGportModeSharedDual)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Dual shaper order should be configured for dual shaper flows only\n");
                }
                break;
            ...
            case bcmCosqSchedulerGportSharedOrderFqClClFq:
                if ((params->mode != bcmCosqSchedulerGportModeSharedQuad) &&
                    (params->mode != bcmCosqSchedulerGportModeSharedOcta))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Quad shaper order should be configured for quad/octa shaper flows only\n");
                }
                break;
        }
    }
    ...
    bcm_gport_t * gport)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_add_verify(unit, flags, params, gport));
 ...
 */
static int
dnx_cosq_scheduler_gport_get_verify(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    bcm_cosq_scheduler_gport_params_t * params)
    ...
    bcm_cosq_scheduler_gport_params_t * params)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_scheduler_gport_get_verify(unit, flags, gport, params));
    ...
    specific_element_in_shared_shaper = BCM_COSQ_GPORT_IS_SHARED_SHAPER_ELEM(gport);

    /** actual implementation */
    SHR_IF_ERR_EXIT(dnx_scheduler_gport_get(unit, core, flow_id, specific_element_in_shared_shaper, params));
