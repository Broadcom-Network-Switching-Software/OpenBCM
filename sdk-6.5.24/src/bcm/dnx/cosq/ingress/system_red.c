    ...
    bcm_fabric_config_discard_t * discard)
{
    ...
    uint32 nof_out_tm_ports = dnx_data_port.general.nof_out_tm_ports_get(unit);
    uint32 factor = dnx_data_system_red.info.sch_aging_period_factor_get(unit);
    uint32 ing_aging_period, aging_period_clocks;
    dnxc_time_t time;
    SHR_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_system_red_aging_config_set_verify(unit, discard));
    ...
    SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &time, &aging_period_clocks));

    aging_params.sch_aging_period = UTILEX_DIV_ROUND_UP(aging_period_clocks, nof_out_tm_ports * factor);
    ...
    bcm_fabric_config_discard_t * discard)
{
     ...
     *  aging_period = nof_ports * 4 clocks * HW value
     */
    aging_period_clocks = aging_params.sch_aging_period * nof_out_tm_ports * factor;

    SHR_IF_ERR_EXIT(dnxc_time_clock_cycles_to_time_get(unit, aging_period_clocks, DNXC_TIME_UNIT_MILISEC, &time));
    ...
    bcm_cosq_gport_size_t * gport_size)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_system_red_voq_max_size_set_verify(unit, gport, red_q_size, gport_size));
    ...
    bcm_cosq_gport_size_t * gport_size)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_system_red_q_size_thr_verify(unit, red_q_size));
    ...
    bcm_cosq_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_system_red_free_res_threshold_verify(unit, gport, cosq, threshold, is_set));
    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_system_red_discard_set_verify(unit, gport, cosq, discard));
    ...
    uint32 *red_index)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_system_red_sysport_verify(unit, system_port));
