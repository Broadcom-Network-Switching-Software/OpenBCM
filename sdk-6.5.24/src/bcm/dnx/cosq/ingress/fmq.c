    ...
    int weight)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_fmq_sched_set_verify(unit, gport, cosq, mode, weight));
    ...
    int *weight)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_fmq_basic_verify(unit, gport, cosq));
    ...
    uint32 flags)
{
    ...
    COMPILER_64_SET(kbps_rate_max, 0, dnx_data_iqs.credit.fmq_shp_rate_max_get(unit));
    COMPILER_64_UMUL_32(kbps_rate_max, 1000);

    /** Validate kbits_sec_max */
    if (COMPILER_64_HI(kbps_rate_max) == 0)
    {
        SHR_MAX_VERIFY(kbits_sec_max, COMPILER_64_LO(kbps_rate_max), _SHR_E_PARAM,
                       "maximum kbits per second %u is out of bounds (0 - %u)", kbits_sec_max,
                       COMPILER_64_LO(kbps_rate_max));
    }
    ...
    uint32 flags)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_fmq_bandwidth_set_verify(unit, gport, cosq, kbits_sec_min, kbits_sec_max, flags));
    ...
    uint32 *flags)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_fmq_bandwidth_get_verify(unit, gport, cosq, kbits_sec_min, kbits_sec_max, flags));
    ...
    int arg)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_control_fmq_set_verify(unit, port, cosq, type, arg));
    ...
    int *arg)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_control_fmq_get_verify(unit, port, cosq, type, arg));
    ...
    int mode)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_fmq_scheduler_mode_set_verify(unit, mode));
