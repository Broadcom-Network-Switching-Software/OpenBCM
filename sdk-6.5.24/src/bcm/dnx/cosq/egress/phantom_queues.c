    ...
    int unit)
{
    ...
    /*
     * Init the Phantom queues probability threshold to the max vallue (probability 100%)
     */
    if (dnx_data_egr_queuing.params.feature_get(unit, dnx_data_egr_queuing_params_phantom_queues_prob_thresh))
    {
        SHR_IF_ERR_EXIT(dnx_phantom_queues_dbal_global_prob_thresh_init(unit));
    }
    ...
    int threshold)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_phantom_queues_threshold_set_verify(unit, port, threshold));
