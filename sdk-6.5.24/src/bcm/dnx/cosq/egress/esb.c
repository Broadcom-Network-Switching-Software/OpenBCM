    ...
    */
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0, 0))
    {
        ...
        *esb_queue_id = local_nif_if;
        if ((*is_interleave == TRUE) && (dnx_data_esb.general.nof_eth_tdm_queues_get(unit) != 0))
        {
            /**
             * In case of dedicated interleave queues for, they are located after all regular queues
             */
            *esb_intlv_queue_id = local_nif_if + dnx_data_esb.general.eth_tdm_queue_id_base_get(unit);
        }
    }
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 0, 0))
    {
        ...
        */
        if (dnx_data_esb.general.nof_ilkn_tdm_queues_get(unit) != 0)
        {
            ...
            */
            *esb_intlv_queue_id = ilkn_if + dnx_data_esb.general.ilkn_tdm_queue_id_base_get(unit);
        }
        else
        {
           /**
            ...
            *esb_intlv_queue_id = local_ilkn_first_phy;
    ...
    int is_add)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_entry_new(unit,
                                                        ...
                                                        sizeof(int),
                                                        (uint8 *) &logical_port,
                                                        sizeof(bcm_port_t), &dnx_esb_port_er_rollback));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_port_t logical_port)
{
    ...
    DNX_ERR_RECOVERY_EXPR_SUPPRESS_IF_ERR_EXIT(dnx_esb_port_add(unit, logical_port));
    ...
    bcm_core_t core)
{
    ...
    uint32 esb_intlv_queue_id = 0, esb_queue_id = 0, min_gap, flags = 0;
