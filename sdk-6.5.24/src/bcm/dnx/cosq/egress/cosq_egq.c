    ...
    bcm_cos_queue_t offset)
{
    ...
    if ((int) ingress_dp < 0 || ingress_dp >= DNX_COSQ_NOF_DP)
    {
    ...
    /** when port is local check that it has sufficient priorities to support provided offset*/
    if (gport_info.flags & DNX_ALGO_GPM_GPORT_INFO_F_IS_LOCAL_PORT)
    {
        int num_priorities;
        SHR_IF_ERR_EXIT(dnx_algo_port_priorities_nof_get(unit, gport_info.local_port, &num_priorities));
        if (offset >= num_priorities)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid offset parameter %d. Legal range is %d,%d", offset, 0,
                         (num_priorities - 1));
        }
    }
    ...
    bcm_cosq_egress_multicast_config_t * config)
{
     ...
     * Validate input params 
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_egq_gport_egress_multicast_config_get_validate
    ...
    bcm_cosq_egress_multicast_config_t * config_p)
{
    ...
    if ((int) (config_p->scheduled_dp) < 0 || config_p->scheduled_dp > DNX_COSQ_NOF_DP)
    {
    ...
    if ((int) queue_id < 0 || queue_id >= DNX_EGR_NOF_Q_PRIO)
    {
    ...
    bcm_cosq_egress_multicast_config_t * config_p)
{
             ...
             (flags & BCM_COSQ_MULTICAST_SCHEDULED))
    {
        ...
        /** Validate input params */
        SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_egress_multicast_config_set_verify
    ...
    bcm_cos_queue_t * offset_p)
{
     ...
     * Validate input params 
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_egress_map_get_verify(unit, gport, ingress_pri, ingress_dp, offset_p));
    if (BCM_COSQ_GPORT_IS_UCAST_EGRESS_QUEUE(gport) || BCM_COSQ_GPORT_IS_MCAST_EGRESS_QUEUE(gport) ||
        BCM_GPORT_IS_TRUNK(gport))
    {
