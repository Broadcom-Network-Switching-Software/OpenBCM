...
/*
 * This semaphore protect the shared memory that is being accessed by the traverse API via main thread and traverse thread
 */
sal_sem_t dnx_l2_traverse_sem[BCM_MAX_NUM_UNITS] = { NULL };
    ...
    void *user_data)
{
    ...
    if ((dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_ivl_feature_support) == 0)
        && ((match_l2addr->modid != 0) || (flags & BCM_L2_REPLACE_VLAN_AND_VPN_TYPE)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "No support ivl on current device");
    }
...
*/
shr_error_e
dnx_l2_traverse_thread_sem_create(
    int unit)
{

    SHR_FUNC_INIT_VARS(unit);

    if (dnx_l2_traverse_sem[unit] == NULL)
    {
        dnx_l2_traverse_sem[unit] = sal_sem_create("MACT_fifo_sem", sal_sem_BINARY, 1);
        if (dnx_l2_traverse_sem[unit] == NULL)
        {
            SHR_IF_ERR_EXIT(_SHR_E_RESOURCE);
        }
        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "Sem dnx_l2_traverse_sem.%d created\n"), unit));
    }
exit:
    SHR_FUNC_EXIT;
}
...
*/
void
dnx_l2_traverse_thread_sem_destroy(
    int unit)
{
    if (dnx_l2_traverse_sem[unit] != NULL)
    {
        sal_sem_destroy(dnx_l2_traverse_sem[unit]);
        dnx_l2_traverse_sem[unit] = NULL;
    }
    LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "Sem dnx_l2_traverse_sem%d destroyed\n"), unit));
}
 ...
 *
 * \param [in] unit - unit id
 * \param [in] table_id - the id of the table that will be traversed
    ...
    void *user_data)
{
    ...
    uint8 traverse_by_thread = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /**
     * This semaphore is placed in order to prevent multiple thread access to the overflow buffer
     */
    if (sal_sem_take(dnx_l2_traverse_sem[unit], sal_sem_FOREVER))
    {
        SHR_IF_ERR_EXIT(SOC_E_RESOURCE);
    }
    ...
    if ((flags & DNX_L2_NO_CALLBACKS) || (flags & DNX_L2_MATCH_GET_NON_BLOCKING))
    {
        ...
        if (flags & DNX_L2_MATCH_GET_NON_BLOCKING)
        {
            traverse_by_thread = TRUE;
            /**
             * Waking the traverse thread to perform the action required. Semaphore, in this case, will be released from
    ...
    else
    {
        ...
        while (!is_end)
        {
            ...
            if (return_val)
            {
                ...
                dbal_flush_reset_flush_work_buffers(unit);
...
exit:
    if (!traverse_by_thread)
    {
        /*
         * Release the semaphore if the work over the overflow buffer was done via main thread
         */
        sal_sem_give(dnx_l2_traverse_sem[unit]);
    }
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
    ...
    void *user_data)
{
    ...
    table_id = (_SHR_IS_FLAG_SET(flags, DNX_L2_MATCH_IVL)) ? DBAL_TABLE_FWD_MACT_IVL : DBAL_TABLE_FWD_MACT;

    SHR_IF_ERR_EXIT(dnx_l2_addr_traverse_by_rule
                    (unit, table_id, flags, match_l2addr, match_mask_l2addr, replace_l2addr, replace_mask_l2addr,
                     trav_fn, user_data));
...
exit:
    /**
     * Release the traverse semaphore after the work over the entries is done
     */
    sal_sem_give(dnx_l2_traverse_sem[unit]);
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    int unit)
{
    ...
    thread_config_info.stop_timeout_usec = (5 * SECOND_USEC);
