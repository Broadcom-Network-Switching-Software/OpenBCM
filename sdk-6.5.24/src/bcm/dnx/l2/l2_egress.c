    ...
    bcm_l2_egress_t * egr)
{
    ...
    if (egr->recycle_app == bcmL2EgressRecycleAppRedirectVrf)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "VRF %d is out of range, should be bigger than 0 and lower than %d.", egr->vrf,
    ...
    bcm_l2_egress_t * egr)
{
    ...
    if (dnx_data_flow.rch.feature_get(unit, dnx_data_flow_rch_flow_in_use))
    {
    ...
    if (!_SHR_IS_FLAG_SET(egr->flags, BCM_L2_EGRESS_REPLACE))
    {
        ...
        if (egr->recycle_app == bcmL2EgressRecycleAppExtendedEncap)
        {
            outlif_info.logical_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_7;
        }
        else
        {
