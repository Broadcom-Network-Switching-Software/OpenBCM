    ...
    uint32 *field_flags)
{
    ...
    *result_type = 0;
    *eei = 0;
    mact_vlan_learning_type = (l2addr->modid) ? l2AddrMactIvl : l2AddrMactSvl;
    mact_type =
        (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact))
        ? (_SHR_IS_FLAG_SET(flags, L2_ADDR_ADD_LRN) ? l2AddrLearnMact : l2AddrFwdMact) : l2AddrNA;
    ...
    if (_SHR_IS_FLAG_SET(l2addr->flags, BCM_L2_MCAST))
    {
        ...
        {
            *result_type =
                dnx_data_l2.general.mact_result_type_map_get(unit, mact_type, mact_vlan_learning_type)->dest_stat;
        ...
        else
        {
            ...
            {
                *result_type =
                    dnx_data_l2.general.mact_result_type_map_get(unit, mact_type,
            ...
            {
                *result_type =
                    dnx_data_l2.general.mact_result_type_map_get(unit, mact_type, mact_vlan_learning_type)->no_outlif;
    ...
    else        /* Currently assuming GPORT encoding for destination + outLIF. */
    {
        ...
        *destination = forward_info.destination;
        ...
        else if (forward_info.fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_EEI)
        {
            ...
            *result_type = dnx_data_l2.general.mact_result_type_map_get(unit, mact_type, mact_vlan_learning_type)->eei;
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    {
        uint8 key;
        key = (_SHR_IS_FLAG_SET(flags, L2_ADDR_ADD_LRN)) ? l2AddrLearnMact : l2AddrFwdMact;
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, (dbal_tables_e) (dnx_data_l2.general.fwd_mact_info_get(unit, key)->data_base),
                         &entry_handle_id));
    }
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    {
        table_id = (dbal_tables_e) (dnx_data_l2.general.fwd_mact_info_get(unit, l2AddrFwdMact)->data_base);
    }
