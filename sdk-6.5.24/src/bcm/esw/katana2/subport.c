...
#define SCACHE_BYTE_MIN_RESTORE(_scache_ptr_, _local_var_, _scache_sz_, _local_sz_) \
    do {                                                                      \
        sal_memcpy(_local_var_, _scache_ptr_,                                 \
...
bcm_kt2_subport_wb_recover(int unit, uint16 version, uint8 **scache_ptr)
{
    ...
    if (BCM_PBMP_NOT_NULL(SOC_INFO(unit).subtag_allowed_pbm)) {
        ...
        *scache_ptr += (length * max_num_pp_ports);
    }
    ...
    if (BCM_PBMP_NOT_NULL(SOC_INFO(unit).linkphy_allowed_pbm)) {
        ...
        for (index = 0; index < max_num_pp_ports; index++) {
            SCACHE_BYTE_RESTORE(*scache_ptr,
    ...
    if (version >= BCM_SUBPORT_WB_VERSION_1_1) {
        SCACHE_BYTE_MIN_RESTORE(*scache_ptr,
                                &(si->enabled_subtag_pp_port_pbm),
                                pbmp_scache_sz, sizeof(pbmp_t));
        SCACHE_BYTE_MIN_RESTORE(*scache_ptr,
                                &(si->enabled_linkphy_pp_port_pbm),
                                pbmp_scache_sz, sizeof(pbmp_t));
    }
    return BCM_E_NONE;
}
                                          ...
                                          int ext_stream_id)
{
    ...
    sal_memset(&sp_config, 0, sizeof(sp_config));
                              ...
                              int *local_port)
{
        ...
        if (BCM_PBMP_MEMBER(SOC_INFO(unit).linkphy_pp_port_pbm, pp_port)) {
            ...
            if (!BCM_GPORT_IS_SUBPORT_GROUP(subport_group)) {
                subport_group_valid = 0;
            }
            /* For Metrolite, the same phy-port pp-port can be re-used as
             * a pp-port for subports. If at init time, this is marked to be
             * used as such and now the user wants to reuse the same as a
             * regular phy-port, we should allow that, as long as the
             * run time bcmPortControlLinkPhyEnable is not setup on that phy-port.
             * In such a case just return the pp-port as the same phy-port, else
             * if control is already set, return FAIL. (This is indicated using
             * the 'linkphy_pbm' pbmp)
             *
             * Similar discussion for subtag below.
             */
            if (soc_feature(unit, soc_feature_discontinuous_pp_port) &&
                !subport_group_valid) {
                if (!BCM_PBMP_MEMBER(SOC_INFO(unit).linkphy_pbm, pp_port)) {
                    *local_port = pp_port;
                    return BCM_E_NONE;
                } else {
                    return BCM_E_PARAM;
                }
            } else if (!subport_group_valid) {
                return BCM_E_PARAM;
            } 
            ...
            if (soc_feature(unit, soc_feature_discontinuous_pp_port) &&
                !subport_group_valid) {
                if (!BCM_PBMP_MEMBER(SOC_INFO(unit).subtag_pbm, pp_port)) {
                    *local_port = pp_port;
                    return BCM_E_NONE;
                } else {
                return BCM_E_PARAM;
                              ...
                              _bcm_kt2_subport_info_t *subport_info)
{
    ...
    sal_memset(&subport_port_config, 0, sizeof(subport_port_config));
