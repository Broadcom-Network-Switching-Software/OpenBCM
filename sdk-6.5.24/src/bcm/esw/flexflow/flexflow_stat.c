...
#define BCM_FLEXFLOW_FLEX_COUNTER_MAX_TABLE 4
STATIC
                            ...
                            int  *index)
{
    ...
    sal_memset(&vinfo, 0, sizeof(soc_flow_db_mem_view_info_t));
    ...
    if (info->function_type == bcmFlowFuncTypeL3Route) {
	    ...
	    vfi_f = VFIf;
            ...
            if (info->valid_elements & BCM_FLOW_STAT_PORT_VALID) {
                ...
                if (BCM_GPORT_IS_SET(info->port)) {
                    if (BCM_GPORT_IS_TRUNK(info->port)) {
                        return BCM_E_PARAM; /* trunk is not supported */
                    }
                    BCM_IF_ERROR_RETURN(_bcm_esw_gport_resolve(unit, info->port,
                          &mod_out, &port_out, &trunk_id, &gport_id));
                 
                    if (gport_id != -1) {  /* a vp */
                        if (!SOC_MEM_FIELD_VALID(unit, mem_view_id, SVPf)) {
                             return BCM_E_PARAM;
                        }
                        soc_mem_field32_set(unit,mem_view_id, entry, SVPf,
                            gport_id);
                    } else {
                        if (mod_out == -1) {
                            mod_out = 0;
                ...
                if (gport_id == -1) {
                    if (!SOC_MEM_FIELD_VALID(unit, mem_view_id, DGPPf)) {
                         return BCM_E_PARAM;
                    }
                    soc_mem_field32_set(unit,mem_view_id, entry, DGPPf,
                        (mod_out << 8) | port_out);
                }
            }
            if (info->valid_elements & BCM_FLOW_STAT_VPN_VALID) {
        ...
        if (info->match_criteria == BCM_FLOW_MATCH_CRITERIA_PORT_VLAN ) {
            ...
            soc_mem_field32_set(unit, mem_view_id, entry, OTAGf, info->vlan);
                   ...
                   (info->match_criteria == BCM_FLOW_MATCH_CRITERIA_SIP_DIP)) {
            ovid_f = OVIDf;
            ...
            if (info->match_criteria == BCM_FLOW_MATCH_CRITERIA_SIP_DIP) {
               soc_mem_field32_set(unit, mem_view_id, entry, dip_f, info->dip); 
            }
                          ...
                          uint32 stat_counter_id)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if ((direction == bcmStatFlexDirectionIngress) &&
            (!SOC_MEM_FIELD_VALID(unit, mem_view_id, FLEX_CTR_ACTION_SETf))) {
            return BCM_E_UNAVAIL;
        } else if (!SOC_MEM_FIELD_VALID(unit, mem_view_id,
                   EGR_FLEX_CTR_0_ACTION_SETf) &&
                   !SOC_MEM_FIELD_VALID(unit, mem_view_id,
                   EGR_FLEX_CTR_1_ACTION_SETf)) {
            return BCM_E_UNAVAIL;
        }
