...
#include <esw/xflow_macsec_common.h>
#endif
                       ...
                       int *vntag_used)
{
    ...
    int vntag_action;
    int vntag_dst;
    bcm_if_t member_arr[1];
    int member_cnt = 0;

    *vntag_used = FALSE;
    if (mpath_flag) {
        BCM_IF_ERROR_RETURN(bcm_xgs3_l3_egress_multipath_get(unit,
                               (bcm_if_t)nh_inx,
                               1, member_arr, &member_cnt));
        if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, member_arr[0])) {
            nh_inx = member_arr[0] - BCM_XGS3_EGRESS_IDX_MIN(unit);
        } else {
            return BCM_E_NONE;
        }
    }
                       ...
                       uint32 mpath_flag)
{
    ...
    if (!_BCM_FLOW_IS_FLEX_VIEW(info)) {
         ...
         */
        BCM_IF_ERROR_RETURN(_bcm_flow_egress_nh_vntag_check (unit,
                 mpath_flag? (int)info->egress_if: nh_idx,
                 mpath_flag, &vntag_used));
        soc_mem_field32_set(unit, mem_id, entry, del_vntag_f,
                            vntag_used == FALSE ? 1 : 0);
        ...
        int port_encap2config)
{
    ...
    uint32 pe_flg[] = {BCM_FLOW_PORT_ENCAP_FLAG_SERVICE_VLAN_ADD,
                     ...
                     BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE_OUTER_IF_DTAG,
                     BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE_IF_STAG,
                     BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE_OUTER_IF_DTAG,
                     BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE_IF_STAG,
                     0};
    ...
    uint32 e_flg[] = {BCM_FLOW_ENCAP_FLAG_SERVICE_VLAN_ADD,
                    ...
                    BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_OUTER_IF_DTAG,
                    BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_IF_STAG,
                    BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_OUTER_IF_DTAG,
                    BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_IF_STAG,
                    0};
    int i;
...
STATIC
int _bcm_flow_num_of_flags(uint32 flags)
{
    int count = 0;
...
STATIC
int _bcm_flow_vlan_tag_flags_validate(
    int unit,
    uint32 e_flags,  /* from flow_encap_add */
    uint32 pe_flags) /* from flow_port_encap_set */
{
    uint32 flags;
    char *flag_str;

    if (e_flags) {
        flags = BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_OUTER_IF_DTAG |
                 BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_IF_STAG |
                 BCM_FLOW_ENCAP_FLAG_VLAN_DELETE;
        flags &= e_flags;
        if (_bcm_flow_num_of_flags(flags) > 1) {
            flag_str = "BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_OUTER_IF_DTAG\n"
                       "BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_IF_STAG\n"
                       "BCM_FLOW_ENCAP_FLAG_VLAN_DELETE";
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
              "Only use one of these flags:\n%s\n"), flag_str));
            return BCM_E_PARAM;
        }
        flags = BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_OUTER_IF_DTAG |
                 BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_IF_STAG |
                 BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE;
        flags &= e_flags;
        if (_bcm_flow_num_of_flags(flags) > 1) {
            flag_str = "BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_OUTER_IF_DTAG\n"
                       "BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_IF_STAG\n"
                       "BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE";
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
              "Only use one of these flags:\n%s\n"), flag_str));
        ...
        flags = BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_IF_STAG |
                BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_IF_STAG;
        flags &= e_flags;
        if (_bcm_flow_num_of_flags(flags) > 1) {
            flag_str = "BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_IF_STAG\n"
                       "BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_IF_STAG\n";
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
              "Only use one of these flags:\n%s\n"), flag_str));
            return BCM_E_PARAM;
        ...
        flags = BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE_OUTER_IF_DTAG |
                BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE_IF_STAG |
                BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE;
        flags &= pe_flags;
        if (_bcm_flow_num_of_flags(flags) > 1) {
            flag_str = "BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE_OUTER_IF_DTAG\n"
                       "BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE_IF_STAG\n"
                       "BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE";
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
              "Only use one of these flags:\n%s\n"), flag_str));
        ...
        flags = BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE_OUTER_IF_DTAG |
                 BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE_IF_STAG |
                 BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE;
        flags &= pe_flags;
        if (_bcm_flow_num_of_flags(flags) > 1) {
            flag_str = "BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE_OUTER_IF_DTAG\n"
                       "BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE_IF_STAG\n"
                       "BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE";
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
              "Only use one of these flags:\n%s\n"), flag_str));
        ...
        flags = BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE_IF_STAG |
                BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE_IF_STAG;
        flags &= pe_flags;
        if (_bcm_flow_num_of_flags(flags) > 1) {
            flag_str = "BCM_FLOW_PORT_ENCAP_FLAG_VLAN_REPLACE_IF_STAG\n"
                       "BCM_FLOW_PORT_ENCAP_FLAG_VLAN_DELETE_IF_STAG\n";
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
              "Only use one of these flags:\n%s\n"), flag_str));
            return BCM_E_PARAM;
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    if ((info->valid_elements & BCM_FLOW_PORT_ENCAP_FLAGS_VALID) &&
        ...
        rv = _bcm_flow_vlan_tag_flags_validate(unit, 0, info->flags);
        BCM_IF_ERROR_RETURN(rv);
    }
    ...
    int network_port)
{
    ...
    if (info->valid_elements & BCM_FLOW_ENCAP_FLOW_PORT_VALID) {
        if (!BCM_GPORT_IS_MODPORT(info->flow_port)) {
            if (BCM_GPORT_IS_MPLS_PORT(info->flow_port)) {
                dvp = _SHR_GPORT_MPLS_PORT_ID_GET(info->flow_port);
            } else {
                dvp = _SHR_GPORT_FLOW_PORT_ID_GET(info->flow_port);
            }
        }
    ...
    if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_DVP) {
        ...
        if (info->valid_elements & BCM_FLOW_ENCAP_SRC_FLOW_PORT_VALID) {
            ...
            } else if (BCM_GPORT_IS_MPLS_PORT(info->src_flow_port)) {
                svp = _SHR_GPORT_MPLS_PORT_ID_GET(info->src_flow_port);
            } else {
                return BCM_E_PARAM;
            }
                         ...
                         uint32 *xlate_data)
{
    ...
    if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_DVP_GROUP ||
        ...
        if (_BCM_FLOW_IS_FLEX_VIEW(info)) {
            ...
            if ((hw_mem == EGR_VLAN_XLATE_1_SINGLEm) &&
                soc_mem_field_valid(unit, hw_mem, VXLAN_VFI_FLEX__OVIDf)) {
                info->vlan = soc_mem_field32_get(unit, hw_mem, entry, VXLAN_VFI_FLEX__OVIDf);
            } else {
                info->vlan = soc_format_field32_get(unit,xlate_format,
                                         xlate_data, NEW_OVIDf);
            }
            if ((hw_mem == EGR_VLAN_XLATE_1_SINGLEm) &&
                soc_mem_field_valid(unit, hw_mem, VXLAN_VFI_FLEX__OPRIf)) {
                info->pri = soc_mem_field32_get(unit, hw_mem, entry, VXLAN_VFI_FLEX__OPRIf);
            } else {
                info->pri = soc_format_field32_get(unit,xlate_format,
                                         xlate_data, NEW_OPRIf);
            }
            if ((hw_mem == EGR_VLAN_XLATE_1_SINGLEm) &&
                soc_mem_field_valid(unit, hw_mem, VXLAN_VFI_FLEX__OCFIf)) {
                info->cfi = soc_mem_field32_get(unit, hw_mem, entry, VXLAN_VFI_FLEX__OCFIf);
            } else {
        ...
        int *tpid_index)
{
    ...
    int delete_type = _BCM_TD3_TAG_ACTION_TYPE_NONE;
    int replace_type = _BCM_TD3_TAG_ACTION_TYPE_NONE;
    ...
    if (info->flags & BCM_FLOW_ENCAP_FLAG_SERVICE_VLAN_ADD) {
        if ((info->valid_elements & BCM_FLOW_ENCAP_PKT_PRI_VALID) &&
            (info->valid_elements & BCM_FLOW_ENCAP_PKT_CFI_VALID)) {
            sd_tag_remark_cfi_set = TRUE;
        } 
        action_not_present = 0x1;  /* ADD */
    }

    if (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_OUTER_IF_DTAG) {
        replace_type = _BCM_TD3_TAG_ACTION_TYPE_DTAG;
        action_present = 0x2;
    } else if (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_IF_STAG) {
    ...
    if (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_OUTER_IF_DTAG) {
        delete_type = _BCM_TD3_TAG_ACTION_TYPE_DTAG;
    } else if (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_IF_STAG) {
        delete_type = _BCM_TD3_TAG_ACTION_TYPE_STAG;
    }
    ...
    _bcm_td3_sd_tag_action_profile_get(unit, action_present,
            action_not_present, &profile_index, replace_type, delete_type);
        ...
        int  *tpid_idx)
{
    ...
    (void)_bcm_td3_sd_tag_action_get(unit, profile_index, &action_present,
                    &action_not_present, &replace_type, &delete_type);
    ...
    if (delete_type == _BCM_TD3_TAG_ACTION_TYPE_STAG) {
        info->flags |= BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_IF_STAG;
        info->flags &= ~BCM_FLOW_ENCAP_FLAG_VLAN_DELETE;
    } else if (delete_type == _BCM_TD3_TAG_ACTION_TYPE_DTAG) {
        info->flags |= BCM_FLOW_ENCAP_FLAG_VLAN_DELETE_OUTER_IF_DTAG;
        info->flags &= ~BCM_FLOW_ENCAP_FLAG_VLAN_DELETE;
    }
    if (replace_type == _BCM_TD3_TAG_ACTION_TYPE_STAG) {
        info->flags |= BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_IF_STAG;
        info->flags &= ~BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE;
    } else if (replace_type == _BCM_TD3_TAG_ACTION_TYPE_DTAG) {
        info->flags |= BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE_OUTER_IF_DTAG;
        info->flags &= ~BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE;
    }
        ...
        for (i = 0; i < num_of_fields; i++) {
            for (j = 0; j < cnt; j++) {
                if (field[i].id == data_ids[j]) {
                    ...
                    len = soc_mem_field_length(unit, mem, field[i].id);
                    if (len >= 32) {
                        /* logical field value max is 0xffffffff */
                        mval = 0xffffffff;
                    } else {
                        mval = (1 << len) - 1;
                    }
                    if (field[i].value > mval) {
                        char *fname;

                        SOC_IF_ERROR_RETURN(soc_flow_db_mem_view_field_name_get(
                            unit, mem, field[i].id, &fname));

                        LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
                           "%s field value %u > max %u\n"),fname, field[i].value, mval));
                        return BCM_E_PARAM;
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    if ((info->valid_elements & BCM_FLOW_ENCAP_FLAGS_VALID) &&
        (info->flags & BCM_FLOW_ENCAP_FLAG_SERVICE_TAGGED)) {
        rv = _bcm_flow_vlan_tag_flags_validate(unit, info->flags, 0);
        BCM_IF_ERROR_RETURN(rv);
    }
    ...
    if (info->valid_elements & BCM_FLOW_ENCAP_CLASS_ID_VALID) {
        ...
        bcm_common_xflow_macsec_port_info_get(unit, info->flow_port, &port_info);
        if (port_info.enable) {
    ...
    if ((info->valid_elements & BCM_FLOW_ENCAP_FLOW_PORT_VALID) &&
        (!(BCM_GPORT_IS_MODPORT(info->flow_port) ||
          (BCM_GPORT_IS_SUBPORT_PORT(info->flow_port))))) {
        if (BCM_GPORT_IS_FLOW_PORT(info->flow_port)) {
            dvp = _SHR_GPORT_FLOW_PORT_ID_GET(info->flow_port);
        } else if (BCM_GPORT_IS_MPLS_PORT(info->flow_port)) {
            dvp = _SHR_GPORT_MPLS_PORT_ID_GET(info->flow_port);
        } else {
            return BCM_E_PARAM;
        } 
        if (!(_bcm_vp_used_get(unit, dvp, _bcmVpTypeFlow) ||
              _bcm_vp_used_get(unit, dvp, _bcmVpTypeMpls))) {
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
