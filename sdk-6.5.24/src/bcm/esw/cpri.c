    ...
    bcm_cpri_axc_frame_info_t *axc_info)
{
     ...
     *  truncation modes in different setting. The following confgiration of truncation
     *  and compression need to process in that particular order.
     */

    adjusted_sample_size = axc_info_t->encap_data_info.sample_size;

    if ((axc_info_t->encap_data_info.truncation_enable) &&
    ...
    if (axc_info_t->encap_data_info.compression_type == cprimod_compress) {
        adjusted_sample_size = 9;
    } else if (axc_info_t->encap_data_info.compression_type == cprimod_decompress) {
        adjusted_sample_size = 15;
    }
    ...
    bcm_cpri_encap_info_t *encap_config)
{
    ...
    encap_config->hdr_type                  = (bcm_cpri_hdr_encap_type_t)
                                                config.header_type;
    ...
    encap_config->ecpri_msg_type            = config.ecpri_msg_type;
    encap_config->ecpri_pc_id               = config.ecpri_pc_id;
    ...
    bcm_cpri_encap_info_t *encap_config)
{
    ...
    config.header_type = (cprimod_hdr_encap_type_t) encap_config->hdr_type;
...
int
bcm_esw_cpri_port_rsvd4_rx_bfn_sync_config_get(
    int unit,
    bcm_gport_t port,
    uint16 *master_frame_count,
    uint64 *master_frame_start)
{
#ifdef CPRIMOD_SUPPORT
    uint32 local_master_frame_count= 0;
    BCM_IF_ERROR_RETURN(_bcm_esw_port_gport_validate(unit, port, &port));
    SOC_IF_ERROR_RETURN
        (portmod_cpri_port_rsvd4_rx_master_frame_sync_config_get( unit, port, 
                                                                  &local_master_frame_count,
                                                                  master_frame_start));
    *master_frame_count = local_master_frame_count;
    return BCM_E_NONE;
#else
    return BCM_E_UNAVAIL;
#endif
...

{
    ...
    mod_rule.modulo_value   = (cprimod_modulo_rule_mod_t)
                                modulo_rule->modulo_value;
    ...
    mod_rule.flow_type      = (cprimod_flow_type_t) modulo_rule->flow_type;
...

{
    ...
    modulo_rule->modulo_value   = (bcm_cpri_modulo_rule_mod_t)
                                    mod_rule.modulo_value;
    ...
    modulo_rule->flow_type      = (bcm_cpri_flow_type_t) mod_rule.flow_type;
    return BCM_E_NONE;
#else
    return BCM_E_UNAVAIL;
#endif
...

{
    ...
    position_entry.flow_type    = (cprimod_flow_type_t) pos_entry->flow_type;
...

{
    ...
    pos_entry->flow_type    = (bcm_cpri_flow_type_t) position_entry.flow_type;
    ...
    bcm_cpri_tx_frame_tgen_config_t *tgen_config)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_tx_frame_tgen_enable_set(unit, port, enable));
    return BCM_E_NONE;
#else
    return BCM_E_UNAVAIL;
#endif
...
int
bcm_esw_cpri_port_tx_frame_tgen_config_get(
    int unit,
    bcm_gport_t port,
    int *enable,
    bcm_cpri_tx_frame_tgen_config_t *tgen_config)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_tx_frame_tgen_enable_get(unit, port, &int_enable));
    *enable =  int_enable ? TRUE:FALSE;

    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_tx_frame_tgen_config_get(unit, port, &tgen_cfg_t));

    tgen_config->tx_tgen_hfn = tgen_cfg_t.tx_tgen_hfn;
    tgen_config->tx_tgen_bfn = tgen_cfg_t.tx_tgen_bfn;
    COMPILER_64_COPY (tgen_config->tx_tgen_ts_offset, tgen_cfg_t.tx_tgen_ts_offset);
...
/* Get Rsvd4 Rx control message flow. */
int bcm_esw_cpri_port_rsvd4_rx_control_flow_get(
        int unit,
        int port,
        int control_flow_id,
        bcm_rsvd4_control_flow_config_t *config)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_rsvd4_rx_control_flow_get(unit, port, control_flow_id, &portmod_config));

    config->match_mask      = portmod_config.match_mask;
    config->match_data      = portmod_config.match_data;
    config->type            = portmod_config.proc_type;
    config->queue_num       = portmod_config.queue_num;
    config->sync_profile    = portmod_config.sync_profile;
    config->sync_en         = portmod_config.sync_enable;
...
/* Get the fast Ethernet control configuration. */
int bcm_esw_cpri_port_rsvd4_cm_fast_eth_config_get(
    int unit,
    int port,
    bcm_rsvd4_fast_eth_config_info_t *config)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_rsvd4_fast_eth_config_get(unit, port, &int_config));

    config->queue_num           = int_config.queue_num;
    config->no_fcs_err_check    = int_config.no_fcs_err_check;
    config->min_packet_size     = int_config.min_packet_size;
    config->max_packet_size     = int_config.max_packet_size;
    config->min_packet_drop     = int_config.min_packet_drop;
    config->max_packet_drop     = int_config.max_packet_drop;
    config->strip_crc           = int_config.strip_crc;
...
/* Get control  flow from rsvd4 tx control group. */
int bcm_esw_cpri_port_rsvd4_tx_control_flow_group_member_get(
    int unit,
    int port,
    bcm_rsvd4_control_group_id_t group_num,
    bcm_rsvd4_priority_t priority,
...
{
#ifdef CPRIMOD_SUPPORT
    uint32 local_queue_num =0;
    BCM_IF_ERROR_RETURN(_bcm_esw_port_gport_validate(unit, port, &port));

    BCM_IF_ERROR_RETURN
        (portmod_rsvd4_tx_control_flow_group_member_get( unit, port,
                                                         group_num, priority,
                                                         &local_queue_num,
                                                         &config->type));
    config->priority    = priority;
    config->queue_num   = local_queue_num;
    return BCM_E_NONE;
#else
    return BCM_E_UNAVAIL;
#endif
...
/* Get all control  flow from rsvd4 tx control group. */
int bcm_esw_cpri_port_rsvd4_tx_control_flow_group_member_get_all(
    int unit,
    int port,
    bcm_rsvd4_control_group_id_t group_num,
    bcm_rsvd4_control_group_entry_t *group_config)
{
#ifdef CPRIMOD_SUPPORT
    portmod_rsvd4_control_group_entry_t local_group_config;
    int priority;

    BCM_IF_ERROR_RETURN(_bcm_esw_port_gport_validate(unit, port, &port));

    BCM_IF_ERROR_RETURN
        (portmod_rsvd4_tx_control_flow_group_member_get_all (unit, port, group_num,
                                                             &local_group_config));
    group_config->valid_mask = local_group_config.valid_mask;
    for (priority = bcmRsvd4Priority0; priority <= bcmRsvd4Priority3; priority++) {
        group_config->queue_num[priority]   = local_group_config.queue_num[priority];
        group_config->type[priority]        = local_group_config.type[priority];
    }
    ...
    bcm_rsvd4_tx_control_flow_config_t *config)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_port_gport_validate(unit, port, &port));
    if (config->cycle_size != 1) {
        LOG_ERROR (BSL_LS_BCM_CPRI,
            (BSL_META("cycle_size %d is not supported in control flow,\
                       forcing to 1.\n"),
                       config->cycle_size));
    }
    ...
    config_info.cycle_size = 1 ; /* Fixed to 1 for control flow. */
...
/* Get tx rsvd4 control flow. */
int bcm_esw_cpri_port_rsvd4_tx_control_flow_get(
    int unit,
    int port,
    int control_flow_id,
    bcm_rsvd4_tx_control_flow_config_t *config)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_rsvd4_tx_control_flow_config_get(unit, port, control_flow_id, &config_info));

    config->type        = config_info.proc_type;
    config->queue_num   = config_info.queue_num;
    config->queue_size  = config_info.queue_size;
    config->cycle_size  = config_info.cycle_size;
    config->crc_mode    = config_info.crc_mode;
...
/* Get tx rsvd4 tunnel control message crc option. */
int bcm_esw_cpri_port_rsvd4_tx_cm_tunnel_crc_config_get(
    int unit,
    int port,
    bcm_rsvd4_cm_tunnel_crc_option_t *crc_option)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_rsvd4_tx_single_tunnel_message_config_get(unit, port, crc_option));
    ...
    bcm_rsvd4_tx_cm_hdr_entry_t* entry)
{
    ...
    if (entry->rsvd4_header_subnode > 0x1F) {
        LOG_ERROR(BSL_LS_BCM_CPRI,
                 (BSL_META("Invalid header subnode %d, max is 31.\n"), entry->rsvd4_header_subnode));
        return BCM_E_PARAM;
    }
...
/* Get rsvd4 tx control message header. */
int bcm_esw_cpri_port_rsvd4_tx_cm_hdr_config_entry_get(
    int unit,
    int port,
    int index,
    bcm_rsvd4_tx_cm_hdr_entry_t *entry)
{
    ...
    uint32 header_payload_node;
    BCM_IF_ERROR_RETURN(_bcm_esw_port_gport_validate(unit, port, &port));

    BCM_IF_ERROR_RETURN
        (portmod_rsvd4_tx_control_header_entry_get(unit, port, index,
                                                   &header_node,
                                                   &header_subnode,
                                                   &header_payload_node));
    entry->rsvd4_header_node            = header_node;
    entry->rsvd4_header_subnode         = header_subnode;
    entry->rsvd4_control_payload_node   = header_payload_node;
    return BCM_E_NONE;
#else
    return BCM_E_UNAVAIL;
#endif
...
int bcm_esw_cpri_port_rsvd4_tx_cm_hdr_lkup_entry_get(
    int unit,
    int port,
    int flow_id,
    int *header_index)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_rsvd4_tx_control_flow_header_index_get(unit, port, flow_id, &index));
    *header_index = index;
    ...
    bcm_cpri_slow_hdlc_config_info_t *config)
{
    ...
    portmod_config.crc_init_val     = (portmod_cpri_hdlc_crc_init_val_t)
                                        config->crc_init_val;
    ...
    portmod_config.fcs_size         = (portmod_cpri_hdlc_fcs_size_t)
                                        config->hdlc_fcs_size;
    ...
    portmod_config.priority         = config->priority;
    portmod_config.queue_size       = config->queue_size;
    portmod_config.cycle_size       = 1; /* Fixed to 1 for control flow. */
    ...
    bcm_cpri_slow_hdlc_config_info_t *config)
{
    ...
    config->crc_init_val        = (bcm_cpri_crc_init_value_t)
                                    portmod_config.crc_init_val;
    ...
    config->hdlc_fcs_size       = (bcm_cpri_hdlc_fcs_size_t)
                                    portmod_config.fcs_size;
...
int bcm_esw_cpri_encap_control_queue_tag_to_flow_id_map_get(
    int unit,
    int port,
    uint32 tag_id,
    uint32 *flow_id)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_encap_queue_control_tag_to_flow_map_get(unit, port, tag_id, flow_id));
    return (BCM_E_NONE);
#else
    return (BCM_E_UNAVAIL);
#endif
...
int bcm_esw_cpri_port_rx_tag_config_get(
    int unit,
    int port,
    bcm_cpri_control_tag_config_t *config)
{
#ifdef CPRIMOD_SUPPORT
    uint32 default_tag = 0;
    uint32 no_match_tag = 0;

    BCM_IF_ERROR_RETURN(_bcm_esw_port_gport_validate(unit, port, &port));

    BCM_IF_ERROR_RETURN
        (portmod_rx_tag_config_get(unit, port, &default_tag, &no_match_tag));
    config->default_tag     = default_tag;
    config->no_match_tag    = no_match_tag;
    ...
    bcm_cpri_1588_capture_config_t *config)
{
    ...
    cfg.mode = (cprimod_1588_capture_mode_t) config->mode;
    ...
    bcm_cpri_1588_capture_config_t *config)
{
    ...
    config->mode = (bcm_cpri_1588_capture_mode_t) cfg.mode;
...
/* Set RoE frame format for the port. */
int bcm_esw_cpri_port_frame_config_get(
    int unit,
    bcm_port_t port,
    bcm_cpri_port_frame_config_t *config) {
    ...
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_frame_config_get(unit, port, &pm_config));

    config->port_frame_format   = pm_config.frame_type;
    config->decap_primary_key   = pm_config.decap_primary_key;
    config->decap_secondary_key = pm_config.decap_secondary_key;
...
/* Set Rx 8b10b descrambler enable */
int bcm_esw_cpri_port_rx_8b10b_descrambler_enable_set(
    int unit,
    bcm_gport_t port,
    int enable)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_esw_port_gport_validate(unit, port, &port));
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_rx_frame_optional_config_set( unit, port, 
                                                         PORTMOD_RX_CONFIG_8B10B_DESCRAMBLER_EN,
                                                         enable? 1:0));
    return SOC_E_NONE;
#else
    return BCM_E_UNAVAIL;
...
/* Get Rx 8b10b descrambler enable */
int bcm_esw_cpri_port_rx_8b10b_descrambler_enable_get(
    int unit,
    bcm_gport_t port,
    int *enable)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_esw_port_gport_validate(unit, port, &port));
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_rx_frame_optional_config_get( unit, port,
                                                         PORTMOD_RX_CONFIG_8B10B_DESCRAMBLER_EN,
                                                         enable));
...
/* Set Tx 8b10b scrambler enable */
int bcm_esw_cpri_port_tx_8b10b_scrambler_enable_set(
    int unit,
    bcm_gport_t port,
    int enable)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_esw_port_gport_validate(unit, port, &port));
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_tx_frame_optional_config_set( unit, port,
                                                         PORTMOD_TX_CONFIG_CWA_SCRABMLER_EN_8B10B,
                                                         enable? 1:0));
    return SOC_E_NONE;
#else
    return BCM_E_UNAVAIL;
...
/* Get Tx 8b10b scrambler enable */
int bcm_esw_cpri_port_tx_8b10b_scrambler_enable_get(
    int unit,
    bcm_gport_t port,
    int *enable)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_esw_port_gport_validate(unit, port, &port));
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_tx_frame_optional_config_get( unit, port,
                                                         PORTMOD_TX_CONFIG_CWA_SCRABMLER_EN_8B10B,
                                                         &scrambler));
    *enable = scrambler ? 1:0;
    return SOC_E_NONE;
#else
    return BCM_E_UNAVAIL;
#endif
...
/* Set Tx 8b10b scrambler seed value */
int bcm_esw_cpri_port_tx_8b10b_scrambler_seed_set(
    int unit,
    bcm_gport_t port,
    uint32 seed_value)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_tx_frame_optional_config_set( unit, port,
                                                         PORTMOD_TX_CONFIG_SCRAMBLER_SEED_8B10B,
                                                         seed_value));
    return SOC_E_NONE;
#else
    return BCM_E_UNAVAIL;
...
/* Get Tx 8b10b scrambler seed value */
int bcm_esw_cpri_port_tx_8b10b_scrambler_seed_get(
    int unit,
    bcm_gport_t port,
    uint32 *seed_value)
{
