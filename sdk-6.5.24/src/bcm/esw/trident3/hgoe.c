                                ...
                                bcm_port_encap_config_t *encap_cfg)
{
    ...
    if (pkt->_higig[7] & BCM_TX_EXT_HG_HDR) {
        /* Packet has 4 additional bytes of HG extension header.*/
        hg_extn_offset = 4;
    }

    /* Create a dma'able block of memory */
    dma_buffer = sal_dma_alloc(42 + hg_extn_offset, "HgoeHeader");
    if (!dma_buffer) {
        return BCM_E_MEMORY;
    }
    ...
    tmp_pkt_blk_arr[0].len = 42 + hg_extn_offset;

    for (blk_idx = 0 ; blk_idx < pkt->blk_count; blk_idx++ ) {
        tmp_pkt_blk_arr[blk_idx + 1] = pkt->pkt_data[blk_idx];
    }

    /* Create and copy all bytes of HGoE HDR (DA, SA, EtherType, HG, DA, SA) */
    ...
    if (hg_extn_offset) {
       /* Copy the  4 additional bytes of HG extension header.*/
       sal_memcpy(dma_buffer + 30, BCM_PKT_EXT_HG_HDR(pkt), 4);
    }
    if (!BCM_PKT_TX_HG_READY(pkt)) {
        /* Rewrite the SOP as HGoE requires the SOP to be 0xFB */
        dma_buffer[14] = SOC_HIGIG_START;
    }
    sal_memcpy(dma_buffer + 30 + hg_extn_offset, pkt->pkt_data[0].data, 12);
...
bcm_td3_port_hgoe_mode_set(int unit, bcm_port_t port)
{
    ...
    if (SOC_IS_HURRICANE4(unit) && IS_EGPHY_PORT(unit, port)) {
        /* HGoE is not supported on GPhy ports */
    ...
    soc_mem_field32_set(unit, mem, &egr_entry,  EDIT_CTRL_IDf, edit_ctrl_id);
    soc_mem_field32_set(unit, mem, &egr_entry, PORT_TYPEf, 1);
    soc_mem_field32_set(unit, mem, &egr_entry, EGR_PORT_CTRL_IDf, 8);
    SOC_IF_ERROR_RETURN(soc_mem_write(unit, mem, MEM_BLOCK_ALL, port, &egr_entry));
    return BCM_E_NONE;
}
...
int bcm_td3_port_hgoe_mode_get(int unit, bcm_port_t port, uint8 *is_hgoe) {
    ...
    *is_hgoe = 0;
    ...
    if (hdr_type == BCM_TD3_HGOE_HDR_TYPE && mode == BCM_PORT_ENCAP_IEEE &&
