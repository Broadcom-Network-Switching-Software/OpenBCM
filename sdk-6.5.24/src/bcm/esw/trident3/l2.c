...
#define _BCM_EXLATE_CHANGE_L2_FIELDS_KEY_TYPE(_u_) \
     ((SOC_IS_HELIX5X(_u_)) ? (0x13) : (0x11))

#define _BCM_EXLATE_CHANGE_L2_FIELDS_DATA_TYPE(_u_) \
        ((SOC_IS_HELIX5(_u_)) ? (25) : \
        ((SOC_IS_HURRICANE4(_u_) | SOC_IS_FIREBOLT6(_u_)) ? \
        (23) : (22)))
...
int
_bcm_td3_flex_l2_replace_data_mask_setup(int unit, bcm_l2_addr_t *l2addr, _bcm_l2_replace_t *rep)
{
    bcm_mac_t mac_mask;
    bcm_mac_t mac_data;
    int field_len;
    ...
    if ((rep->flags & BCM_L2_REPLACE_DELETE) &&
            (process_flex_key == 1)) {
        field_len = soc_mem_field_length(unit, view_id, KEY_0f);
        soc_mem_field32_set(unit, view_id, &rep->match_mask, KEY_0f,
                (1 << field_len) - 1);

        /*Initialize the logical KEY fields with value */
        BCM_IF_ERROR_RETURN(soc_flow_db_mem_view_field_list_get(
                    unit,
                    view_id,
                    SOC_FLOW_DB_FIELD_TYPE_LOGICAL_KEY,
                    size,
                    fields,
                    &field_count));
        if (field_count != l2addr->num_of_fields) {
            return BCM_E_INTERNAL;
        }
        for (i = 0; i < field_count; i++) {
            BCM_IF_ERROR_RETURN(soc_flow_db_mem_view_field_info_get(
                        unit,
                        view_id,
                        fields[i],
                        &field_info));
            if (fields[i] != l2addr->logical_fields[i].id) {
                return BCM_E_INTERNAL;
            }
        ...
        int *tpid_index)
{
    ...
    _bcm_td3_sd_tag_action_profile_get(unit, action_present,
            action_not_present, &profile_index, 0, 0);
