             ...
             void *cookie)
{
    ...
    else if (SOC_IS_TRIDENT3X(unit) && soc_feature(unit, soc_feature_higig3)) {
        return bcm_td3_tx_hg3_array(unit, pkt, count, all_done_cb, cookie);
    }
#endif
...
bcm_esw_tx_list(int unit, bcm_pkt_t *pkt, bcm_pkt_cb_f all_done_cb, void *cookie)
{
    ...
    if (SOC_IS_TRIDENT3X(unit)
        ...
        return bcm_td3_tx_hg3_list(unit, pkt, all_done_cb, cookie);
    }
#endif
