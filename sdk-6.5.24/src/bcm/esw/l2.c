...
/****************************************************************
 *
 * Definitions for L2 MAC move monitoring(dampening) feature
 */
static sal_sem_t _l2_mmm_cb_done[SOC_MAX_NUM_DEVICES];

typedef struct _l2_mac_move_mon_cb_entry_s {
    bcm_l2_mac_move_mon_cb_t	fn;
    void                        *fn_data;
} _l2_mac_move_mon_cb_entry_t;

typedef struct _l2_mmm_data_s {
    _l2_mac_move_mon_cb_entry_t cb;
    sal_mutex_t                 mmm_cb_mutex;
} _l2_mmm_data_t;

static _l2_mmm_data_t l2_mmm_data[SOC_MAX_NUM_DEVICES];

/* Macros for MAC move monitoring (dampening) feature */
#define L2_MMM_LOCK(unit) \
    sal_mutex_take(l2_mmm_data[unit].mmm_cb_mutex, sal_mutex_FOREVER)

#define L2_MMM_UNLOCK(unit) sal_mutex_give(l2_mmm_data[unit].mmm_cb_mutex)

#define L2_MMM_MUTEX(unit) if (l2_mmm_data[unit].mmm_cb_mutex == NULL && \
                               (l2_mmm_data[unit].mmm_cb_mutex = \
                               sal_mutex_create("_bcm_l2_mmm_lock")) == NULL) \
                                  return BCM_E_MEMORY
...
_bcm_esw_l2_traverse_mem(int unit, soc_mem_t mem, _bcm_l2_traverse_t *trav_st)
{
...
#if defined(BCM_TRIDENT2_SUPPORT)
    /* Whether the entry is first valid one of the chunk */
    int             first_valid_entry = TRUE;
#endif /* BCM_TRIDENT2_SUPPORT */
    ...
    for (chnk_idx = soc_mem_index_min(unit, mem); 
        ...
        if (SOC_IS_TRIDENT2(unit)
            || SOC_IS_TITAN2(unit) || SOC_IS_TD2P_TT2P(unit)) {
            first_valid_entry = TRUE;
        }
#endif /* BCM_TRIDENT2_SUPPORT */
...
#if defined(BCM_TRIDENT2_SUPPORT)
            if ((first_valid_entry) && (SOC_IS_TRIDENT2(unit)
                || SOC_IS_TITAN2(unit) || SOC_IS_TD2P_TT2P(unit))) {
    ...
    if (soc_mem_field_valid(unit, mem, PENDINGf)) {
        if (soc_mem_field32_get(unit, mem, entry, PENDINGf)) {
            if (!(rep_st->flags & BCM_L2_REPLACE_PENDING ||
                  rep_st->flags & BCM_L2_REPLACE_IGNORE_PENDING)) {
                return (BCM_E_NONE);
            }
        } else {
            if (rep_st->flags & BCM_L2_REPLACE_PENDING) {
    ...
    * non-static entries */
    ...
    if (SOC_IS_TRIDENT2PLUS(unit) || SOC_IS_TRIDENT3X(unit)) {
        ...
        if (rep->flags & BCM_L2_REPLACE_MATCH_INT_PRI) {
            ...
            soc_mem_field32_set(unit, L2Xm, &rep->match_mask, PRIf, 0xf);
            ...
            if (rep->new_dest.vp != -1) {
                ...
                if (soc_feature(unit, soc_feature_trunk_group_overlay)) {
                    if (soc_feature(unit, soc_feature_generic_dest)) {
                        ...
                        } else if (rep->new_dest.module != -1 && rep->new_dest.port != -1) {
                            soc_mem_field32_dest_set(unit, L2Xm, &rep->new_data,
                                DESTINATIONf, SOC_MEM_FIF_DEST_DGPP,
                    ...
                    bcm_l2_traverse_cb trav_fn, void *user_data)
{
    ...
    if (soc_feature(unit, soc_feature_flex_flow) &&
        ...
        rv = soc_flow_db_ffo_to_mem_view_id_get(unit,
                              ...
                              &rep_st.view_id);
        BCM_IF_ERROR_RETURN(rv);
        BCM_IF_ERROR_RETURN(_bcm_td3_flex_l2_replace_data_mask_setup(unit, match_addr, &rep_st));
    }
    else
#endif
    {
                                           ...
                                           bcm_gport_t *gport)
{
    ...
    if ((flags & ~(BCM_L2_REPLACE_DELETE | BCM_L2_REPLACE_NO_CALLBACKS |
                   ...
                   BCM_L2_REPLACE_IGNORE_PENDING)) != 0) {
        return BCM_E_NONE;
    }
    ...
    if (flags & BCM_L2_REPLACE_DELETE) {
        ...
        if (flags & BCM_L2_REPLACE_NO_CALLBACKS) {
            ...
            if (SOC_CONTROL(unit)->l2x_mode == L2MODE_FIFO &&
                ...
                /* When the 'bcm_esw_l2_addr_freeze' is called above, the
                 * L2Xm lock taken, but when we have to stop the L2 thread,
                 * it is possible it is still running and looking for the
                 * same lock, we hence relinquish the lock, give the thread
                 * to gracefully stop and then retake the lock. In the
                 * 'soc_l2x_stop', there is also a timeout to track and
    ...
    bcm_l2_change_fields_t *l2_info)
{
    ...
    if (soc_feature(unit, soc_feature_vfi_switched_l2_change_fields)) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_td3_sd_tag_action_get(unit,
            profile_index, &action_present, &action_not_present, NULL, NULL));
...
/*
 * Function:
 * 	_bcm_l2_mac_move_mon_appl_callback
 * Description:
 *  This function invokes application-registered callback function
 * Parameters:
 *  unit - StrataSwitch unit number (driver internal).
 *  info - Information about MAC for which threshold exceeded condition was
 *         reported
 * Returns:
 *  No value
 * Notes:
 */
STATIC void
_bcm_l2_mac_move_mon_appl_callback(int unit,
                                   bcm_l2_mac_move_mon_cb_info_t *info)
{
    _l2_mmm_data_t *mmm_data = &l2_mmm_data[unit];

    if (mmm_data->mmm_cb_mutex == NULL) {
        return;
    }

    /* Call application-registered cb function */
    L2_MMM_LOCK(unit);

    if(mmm_data->cb.fn) {
        mmm_data->cb.fn(unit, info, mmm_data->cb.fn_data);
    }
...
/*
 * Function:
 *      _bcm_l2_mac_move_mon_register_callback
 * Description:
 *      Internal callback function to handle callbacks from soc layer
 * Parameters:
 *      unit    - StrataSwitch unit number.
 *      info    - MAC entry details passed by SOC layer are in 'move_info'
 *      fn_data - Not used currently
 * Notes:
 *      Only one callback per unit from the soc layer to the bcm layer
 *      is supported
 */
void
_bcm_l2_mac_move_mon_register_callback(int unit,
    soc_l2_mac_move_mon_entry_info_t *move_info,
    void *fn_data)
{
    bcm_l2_mac_move_mon_cb_info_t info;

    if (_l2_mmm_cb_done[unit] != NULL) {
        sal_sem_take(_l2_mmm_cb_done[unit], sal_sem_FOREVER);
    }

    sal_memset(&info, 0x0, sizeof(info));

    /* Copy a MAC and it's move information to send to the application */
    sal_memcpy(info.mac, move_info->mac, sizeof(bcm_mac_t));
    info.vlan = move_info->vlan;
    info.dst_type_cur = move_info->dst_type_cur;
    info.port_tgid_cur = move_info->portnum_or_trunk_id_cur;
    info.modid_cur = move_info->modid_cur;
    info.dst_type_prev = move_info->dst_type_prev;
    info.port_tgid_prev = move_info->portnum_or_trunk_id_prev;
    info.modid_prev = move_info->modid_prev;

    /* Call application-registered callback function. If a callback is not
     * registered, this function simply returns. Config sequence is defined such
     * that a callback function must be registered before starting the feature,
     * and the issuing thread should be stopped before unregistering callbacks.
     * So there is no unnecessary function call overhead
     */
    _bcm_l2_mac_move_mon_appl_callback(unit, &info);

    if (_l2_mmm_cb_done[unit] != NULL) {
        sal_sem_give(_l2_mmm_cb_done[unit]);
    }
}

int
bcm_esw_l2_mac_move_mon_init(int unit)
{
    int rv = BCM_E_NONE;
    ...
    rv = soc_l2_mac_move_mon_resource_init(unit);

    if (rv != SOC_E_NONE) {
        LOG_WARN(BSL_LS_BCM_L2,
                 (BSL_META_U(unit,
                  "Error initializing mac move mon feature: %s(%d))\n"),
                  bcm_errmsg(rv), rv));
        return rv;
    }

    rv = soc_l2_mac_move_mon_resource_alloc(unit);

    if (rv != SOC_E_NONE) {
        LOG_WARN(BSL_LS_BCM_L2,
                 (BSL_META_U(unit,
                  "Error in resource allocation for mac move mon feature:"
                  " %s(%d))\n"),
                  bcm_errmsg(rv), rv));
        return rv;
    }

    /* Register internal callback function for soc layer to call */
    rv = soc_l2_mac_move_mon_cb_register(unit,
                                         _bcm_l2_mac_move_mon_register_callback,
                                         NULL);
    ...
    if (rv == BCM_E_NONE) {
        if (_l2_mmm_cb_done[unit] == NULL) {
            ...
            _l2_mmm_cb_done[unit] = sal_sem_create("l2 mmm cb done",
                                                   sal_sem_BINARY, 1);
            if (_l2_mmm_cb_done[unit] == NULL) {
                rv = BCM_E_MEMORY;
            }
        ...
        LOG_WARN(BSL_LS_BCM_L2,
                 (BSL_META_U(unit,
                  "Error: Soc layer cb registration failed: %s(%d))\n"),
                  bcm_errmsg(rv), rv));
    }

    if (rv == BCM_E_NONE) {
        /* Set default values for threshold and duration */
        rv = soc_l2_mac_move_mon_params_set(unit,
                 BCM_L2_MAC_MOVE_MON_STNMOVE_THRESHOLD_DEFAULT,
                 BCM_L2_MAC_MOVE_MON_STNMOVE_DURATION_DEFAULT);

        if (rv == BCM_E_NONE) {
            /* Set feature initialization flag */
            rv = soc_l2_mac_move_mon_feature_init_set(unit);
        }
 ...
 *      Only one callback per unit to the bcm layer is supported here.
 */
int bcm_esw_l2_mac_move_mon_stop(int unit)
{
    int rv = BCM_E_NONE;
    ...
    if (soc_l2_mac_move_mon_thread_running(unit)) {
        ...
        if (rv != BCM_E_NONE) {
            LOG_INFO(BSL_LS_BCM_L2,
                     (BSL_META_U(unit,
                      "Error stopping mac move mon thread %s(%d)."
                      " Resources not freed)\n"),
                      bcm_errmsg(rv), rv));
            return rv;
        }
    ...
    rv = soc_l2_mac_move_mon_resource_destroy(unit, TRUE);

    if (rv == BCM_E_NONE) {
        if (_l2_mmm_cb_done[unit] != NULL) {
            sal_sem_destroy(_l2_mmm_cb_done[unit]);
            _l2_mmm_cb_done[unit] = NULL;
        }
    } else {
        LOG_INFO(BSL_LS_BCM_L2,
                 (BSL_META_U(unit,
                 "Error freeing resources of mac move mon thread %s(%d))\n"),
                 bcm_errmsg(rv), rv));
    }

    /* Clear callback function info, destroy mutex */
    if (l2_mmm_data[unit].mmm_cb_mutex != NULL) {
        L2_MMM_LOCK(unit);
        l2_mmm_data[unit].cb.fn = NULL;
        l2_mmm_data[unit].cb.fn_data = NULL;
        L2_MMM_UNLOCK(unit);

        sal_mutex_destroy(l2_mmm_data[unit].mmm_cb_mutex);
        l2_mmm_data[unit].mmm_cb_mutex = NULL;
    }
...
/*
 * Function:
 *	bcm_esw_l2_mac_move_mon_cb_register
 * Description:
 *	This function registers a callback function to invoke for every MAC whenever
 *	the application configured station move threshold is exceeded
 * Parameters:
 *  unit -    (IN) BCM device number
 *	fn -      (IN) Callback function to register
 *	fn_data - (IN) Application specific value passed to callback function
 * Returns:
 *	BCM_E_NONE Success
 *	BCM_E_XXX for all errors
 * Notes:
 *  Currently only one callback per unit is supported, as there is no
 *  requirement to support multiple callbacks per unit
 */
int
bcm_esw_l2_mac_move_mon_cb_register(int unit,
                                    ...
                                    void *fn_data)
{
    ...
    /* Create mutex if it wasn't already created */
    L2_MMM_MUTEX(unit);

    L2_MMM_LOCK(unit);

    if (mmm_data->cb.fn == cb_fn && mmm_data->cb.fn_data == fn_data) {
        L2_MMM_UNLOCK(unit);
        return BCM_E_NONE;
    }

    mmm_data->cb.fn = cb_fn;
    mmm_data->cb.fn_data = fn_data;
...
/*
 * Function:
 *	bcm_esw_l2_mac_move_mon_cb_unregister
 * Description:
 *	This function unregisters a previously registered callback function
 * Parameters:
 *  unit -    (IN) BCM device number
 *	fn -      (IN) Callback function to unregister
 *	fn_data - (IN) Application specific value passed to callback function
 * Returns:
 *	BCM_E_NONE Success
 *	BCM_E_XXX for all errors
 ...
 */
int
bcm_esw_l2_mac_move_mon_cb_unregister(int unit,
                                      bcm_l2_mac_move_mon_cb_t cb_fn,
                                      void *fn_data)
{
    _l2_mmm_data_t *ad = &l2_mmm_data[unit];
    ...
    /* Match the cb function and data passed by the application */
    if((ad->cb.fn == cb_fn) && (ad->cb.fn_data == fn_data)) {
        ad->cb.fn = NULL;
        ad->cb.fn_data = NULL;
    } else {
        return BCM_E_NOT_FOUND;
...
/*
 * Function:
 *	bcm_esw_l2_mac_move_mon_config_set
 * Description:
 *	This function sets configuration parameters passed by the application
 * Parameters:
 *  unit - (IN) BCM device number
 *	cfg  - (IN) Application supplied threshold and duration parameters
 * Returns:
 *	BCM_E_NONE Success
 *	BCM_E_XXX for errors
 * Notes:
 */
int bcm_esw_l2_mac_move_mon_config_set(int unit,
                                       bcm_l2_mac_move_mon_params_t *cfg)
{
    ...
    /* Check if feature was initialized prior to calling this API */
    if ((soc_l2_mac_move_mon_feature_state_get(unit, &state) == BCM_E_INIT) ||
        (!(state & SOC_L2_MAC_MOVE_MON_STATE_INITED))) {
        return BCM_E_CONFIG;
    }

    /* Validation of args */
    if ((cfg->threshold < BCM_L2_MAC_MOVE_MON_STNMOVE_THRESHOLD_MIN) ||
        (cfg->threshold > BCM_L2_MAC_MOVE_MON_STNMOVE_THRESHOLD_MAX)) {
        return BCM_E_PARAM;
    }

    if ((cfg->duration < BCM_L2_MAC_MOVE_MON_STNMOVE_DURATION_MIN) ||
        (cfg->duration > BCM_L2_MAC_MOVE_MON_STNMOVE_DURATION_MAX)) {
        return BCM_E_PARAM;
    }

    rv = soc_l2_mac_move_mon_params_set(unit, cfg->threshold, cfg->duration);
...
/*
 * Function:
 *	bcm_esw_l2_mac_move_mon_config_get
 * Description:
 *	This function gets configuration parameters
 * Parameters:
 *  unit - (IN) BCM device number
 *	cfg  - (OUT) Threshold and duration(in seconds) returned to the application
 * Returns:
 *	BCM_E_NONE Success
 *	BCM_E_XXX for errors
 * Notes:
 */
int bcm_esw_l2_mac_move_mon_config_get(int unit,
                                       bcm_l2_mac_move_mon_params_t *cfg)
{
    int rv = BCM_E_NONE;
    uint32 duration, threshold;
    uint32 state;
    ...
    rv = soc_l2_mac_move_mon_params_get(unit, &threshold, &duration);
...
/*
 * Function:
 *	bcm_esw_l2_mac_move_mon_enable_set
 * Description:
 *	This function starts or stops the thread used for mac move monitoring
 *	(dampening) feature. If interval is non-zero, the thread is
 *	started/restarted. If the interval is 0, the thread is stopped (if it was
 *	runnning)
 * Parameters:
 *  unit -          (IN) BCM device number
 *	interval_usec - (IN) Interval in micro seconds
 * Returns:
 *	BCM_E_NONE Success
 *	BCM_E_XXX for errors
 * Notes:
 */
int bcm_esw_l2_mac_move_mon_enable_set(int unit, uint32 interval_usec)
{
    int rv = BCM_E_NONE;
    sal_usecs_t interval;
    uint32 state;
    ...
    /* Stop thread if the interval of 0 was passed */
    if (interval_usec == 0) {
        /* Stop the monitoring thread if it was running */
        if (soc_l2_mac_move_mon_thread_running(unit)) {
            soc_l2_mac_move_mon_thread_stop(unit);
            /* Reset the start flag to indicate the feature has been stopped */
            rv = soc_l2_mac_move_mon_feature_start_clear(unit);
        }

        return rv;
    }

    rv = soc_l2_mac_move_mon_thread_intvl_get(unit, &interval);

    if (rv == BCM_E_NONE) {
        bcm_l2_mac_move_mon_params_t cfg;
        int rv1;

        /* If the thread interval exceeds currently configured duration,
         * it is not possible to measure threshold, so we return error
         */
        rv1 = bcm_esw_l2_mac_move_mon_config_get(unit, &cfg);

        if ((rv1 == BCM_E_NONE) &&
            (interval_usec > (cfg.duration * SECOND_USEC))) {
            LOG_WARN(BSL_LS_BCM_L2,
                (BSL_META_U(unit,
                 "Error: Thread intvl %u exceeds monitoring duration %u)\n"),
                 interval_usec, (cfg.duration * SECOND_USEC)));
            return BCM_E_CONFIG;
        } else {
            if (rv1 != BCM_E_NONE) {
        ...
        if (interval != interval_usec) {
            /* Restart the thread with new interval */
            soc_l2_mac_move_mon_thread_stop(unit);

            rv = soc_l2_mac_move_mon_thread_intvl_set(unit,
                     (sal_usecs_t)interval_usec);

            if (rv == BCM_E_NONE) {
                rv = soc_l2_mac_move_mon_thread_start(unit);
            }
        } else {
            /* Intervals are same, do nothing */
            return rv;
        }

        if (rv == BCM_E_NONE) {
            /* Set the start flag to indicate the feature is operational now */
            rv = soc_l2_mac_move_mon_feature_start_set(unit);
        } else {
            /* Reset start flag to indicate the feature is not operational */
            rv = soc_l2_mac_move_mon_feature_start_clear(unit);
        }
    }
...
/*
 * Function:
 *	bcm_esw_l2_mac_move_mon_enable_get
 * Description:
 *	This function returns the state of the thread used for mac move monitoring
 *	(dampening) feature. If interval is non-zero, the thread is
 *	running. If the interval is 0, the thread is stopped/not running
 * Parameters:
 *  unit -          (IN) BCM device number
 *	interval_usec - (OUT) Interval in micro seconds
 * Returns:
 *	BCM_E_NONE Success
 *	BCM_E_XXX for errors
 * Notes:
 */
int bcm_esw_l2_mac_move_mon_enable_get(int unit, uint32 *interval_usec)
{
    sal_usecs_t interval;
    uint32 init = 0;
    int rv = BCM_E_NONE;
    ...
    /* If the feature is not initialized, return 0 */
    if (soc_l2_mac_move_mon_feature_state_get(unit, &init) == SOC_E_INIT) {
        *interval_usec = 0;
        return rv;
    }

    /* If the monitoring thead is not running, return 0 */
    if (!soc_l2_mac_move_mon_thread_running(unit)) {
        *interval_usec = 0;
        return rv;
    }
...
/*
 * Function:
 *	_bcm_esw_l2_mac_move_mon_info_sync
 * Description:
 *	This function is called by the warmboot sync function of the devices
 *	supporting mac move monitoring(dampening) feature to store
 *	application-supplied parameters in the scache
 * Parameters:
 *  unit       - (IN) BCM device number
 *	scache_ptr - (IN/OUT) Starting location of scache where the feature
 ...
 */
int
_bcm_esw_l2_mac_move_mon_info_sync(int unit, uint8 **scache_ptr)
{
    int sz;
    int rv = BCM_E_NONE;
    uint32 state = 0;
    sal_usecs_t interval = 0;
    uint32 threshold = 0, duration = 0;
    ...
    sz = sizeof(uint32);
    if (soc_l2_mac_move_mon_feature_state_get(unit, &state) != BCM_E_INIT) {
        SOC_SCACHE_DUMP_DATA_BEGIN("mac_move_mon_feature_state");
        /* Save feature's state */
        sal_memcpy(*scache_ptr, &state, sz);
        SOC_SCACHE_DUMP_DATA_INT32x_V(*scache_ptr);
        *scache_ptr += sz;
        SOC_SCACHE_DUMP_DATA_END();

        rv = soc_l2_mac_move_mon_thread_intvl_get(unit, (sal_usecs_t *)&interval);
        if (rv == BCM_E_NONE) {
            sz = sizeof(sal_usecs_t);
            SOC_SCACHE_DUMP_DATA_BEGIN("thread_interval");
            /* Save thread interval */
            sal_memcpy(*scache_ptr, (uint8 *)(&interval), sz);
            ...
            *scache_ptr += sz;
            ...
            if (rv == BCM_E_NONE) {
                sz = sizeof(uint32);
                SOC_SCACHE_DUMP_DATA_BEGIN("duration");

                /* Save duration */
                sal_memcpy(*scache_ptr, &duration, sz);
                SOC_SCACHE_DUMP_DATA_INT32d_V(*scache_ptr);

                *scache_ptr += sz;
                SOC_SCACHE_DUMP_DATA_END();

                SOC_SCACHE_DUMP_DATA_BEGIN("threshold");
                /* Save threshold */
                sal_memcpy(*scache_ptr, &threshold, sz);
                SOC_SCACHE_DUMP_DATA_INT32d_V(*scache_ptr);
        ...
        SOC_SCACHE_DUMP_DATA_BEGIN("mac_move_mon_feature_state");
        /* Save 0's for feature's state, and all other items */
...
/*
 * Function:
 *	_bcm_esw_l2_mac_move_mon_info_restore
 * Description:
 *	This function is called by the warmboot recover/restore function of the
 *	devices supporting mac move monitoring(dampening) feature to restore
 *	application-supplied parameters from the scache in to the feature's
 *	internal db
 * Parameters:
 *  unit       - (IN) BCM device number
 *	scache_ptr - (IN) Starting location of scache from where the feature
 ...
 */
int
_bcm_esw_l2_mac_move_mon_info_restore(int unit, uint8 **scache_ptr)
{
    int rv = BCM_E_NONE;
    int sz;
    uint32 state;
    uint8 *scache_item_ptr = *scache_ptr;
    sal_usecs_t interval = 0;
    uint32 threshold = 0, duration = 0;
    bcm_l2_mac_move_mon_params_t cfg;
    SOC_SCACHE_DUMP_DECL;
    ...
    sz = sizeof(uint32);
    SOC_SCACHE_DUMP_DATA_BEGIN("mac_move_mon_feature_state");
    /* Read feature's operational state (that was before wb) */
    sal_memcpy(&state, scache_item_ptr, sz);
    SOC_SCACHE_DUMP_DATA_INT32x_V(scache_item_ptr);
    scache_item_ptr += sz;
    SOC_SCACHE_DUMP_DATA_END();

    /* If the state is 0, it means the feature was completely stopped before wb,
     * or never used
     */
    if (state == 0x0) {
        /* Increment scache ptr by appropriate offset */
        sz = sizeof(sal_usecs_t) + 2 * sizeof(uint32);

        SOC_SCACHE_DUMP_DATA_BEGIN("thread_interval");
        SOC_SCACHE_DUMP_DATA_INT32d_V(scache_item_ptr);
        SOC_SCACHE_DUMP_DATA_END();

        SOC_SCACHE_DUMP_DATA_BEGIN("duration");
        SOC_SCACHE_DUMP_DATA_INT32d_V((scache_item_ptr + sizeof(sal_usecs_t)));
        SOC_SCACHE_DUMP_DATA_END();
        ...
        scache_item_ptr += sz;
        *scache_ptr = scache_item_ptr;
        SOC_SCACHE_DUMP_STOP();
        return rv;
    }

    if (state & SOC_L2_MAC_MOVE_MON_STATE_INITED) {
        /* Allocate resources */
        rv = bcm_esw_l2_mac_move_mon_init(unit);
    ...
    sz = sizeof(sal_usecs_t);
    SOC_SCACHE_DUMP_DATA_BEGIN("thread_interval");
    sal_memcpy(&interval, scache_item_ptr, sz);
    SOC_SCACHE_DUMP_DATA_INT32d_V(scache_item_ptr);
    scache_item_ptr += sz;
    SOC_SCACHE_DUMP_DATA_END();

    /* Extract duration and threshold values */
    sz = sizeof(uint32);

    SOC_SCACHE_DUMP_DATA_BEGIN("duration");
    sal_memcpy(&duration, scache_item_ptr, sz);
    SOC_SCACHE_DUMP_DATA_INT32d_V(scache_item_ptr);
    scache_item_ptr += sz;
    SOC_SCACHE_DUMP_DATA_END();

    SOC_SCACHE_DUMP_DATA_BEGIN("threshold");
    sal_memcpy(&threshold, scache_item_ptr, sz);
    SOC_SCACHE_DUMP_DATA_INT32d_V(scache_item_ptr);
    scache_item_ptr += sz;
    SOC_SCACHE_DUMP_DATA_END();
    SOC_SCACHE_DUMP_STOP();

    bcm_l2_mac_move_mon_params_t_init(&cfg);
    cfg.duration = duration;
    cfg.threshold = threshold;
    /* Note, we do not check the return value below because the warmboot may
     * have started in between the 'init' and 'start' states (that is, when
     * threshold, duration were not set by the application, before wb started)
     */
    (void)bcm_esw_l2_mac_move_mon_config_set(unit, &cfg);

    /* If interval is non-zero, start the monitoring thread */
    if ((state & SOC_L2_MAC_MOVE_MON_STATE_STARTED) && (interval > 0)) {
        rv = bcm_esw_l2_mac_move_mon_enable_set(unit, interval);
    }
...
_bcm_esw_l2_wb_alloc(int unit)
{
    ...
    if (sw_aging_support ||
        ...
        /* Ver 1_2 introduces MAC move monitoring feature.
         * The settings for this feature and its related information is stored
         * in scache
         */
        if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit)) {
            /* Feature state flags (enabled/disabled, started/stopped) */
            sz += sizeof(uint32);

            /* Thread interval configured by the application */
            sz += sizeof(sal_usecs_t);
...
_bcm_esw_l2_wb_sync(int unit)
{
    ...
    if (sw_aging_support ||
        ...
        if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit)) {
            ...
            /* Ver 1_2: Store state of mac move monitoring(dampening) feature,
             * thread interval, duration and threshold values in the scache
             * Note this function modifies scache_ptr
             */
            BCM_IF_ERROR_RETURN(_bcm_esw_l2_mac_move_mon_info_sync(unit,
                                                                &scache_ptr));
        }
    }
        ...
        if (scache_ptr != NULL) {
            ...
            if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit)) {
                if (recovered_ver >= BCM_WB_VERSION_1_2) {
                    /* Restore mac move monitoring information from scache
                     * contents.
                     * Note scache_ptr is modified by the function below
                     */
                    BCM_IF_ERROR_RETURN(
                        _bcm_esw_l2_mac_move_mon_info_restore(unit,
                                                              &scache_ptr));
                } else {
                      int sz;
