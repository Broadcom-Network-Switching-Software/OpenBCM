    ...
    for (bnk = 0; bnk < ACB_BNK_PER_BKT(acb, vrf_id); bnk++) {
        ...
        for (eid = 0; eid < ACB_FMT_ENT_MAX(acb, vrf_id, fmt); eid++) {
            ...
            if (ACB_HAS_RTE(acb, vrf_id)) {
                ...
                /* Restore route stat counter related internal variables
                   and re-enable counter pool */
                if (adata1.defip_flex_ctr_base_id != 0 ||
                    adata1.defip_flex_ctr_mode != 0) {
                    (void)_bcm_esw_stat_flex_l3_route_restore(u,
                                          bcmStatObjectIngL3Route,
                                          adata1.defip_flex_ctr_mode,
                                          adata1.defip_flex_ctr_pool,
                                          adata1.defip_flex_ctr_base_id);
                }
        ...
        for (; idx < idx_end; idx++) {
            ...
            for (sub_idx = 0; sub_idx < ALPM_TCAM_SUBCNT; sub_idx += step_size) {
                ...
                if (!use_alpm) {
                    ...
                    /* Re-enable direct route attached flex stat counter pool */
                    if (adata0.defip_flex_ctr_base_id != 0 ||
                        adata0.defip_flex_ctr_mode != 0) {
                        (void)_bcm_esw_stat_flex_l3_route_restore(u,
                                              bcmStatObjectIngL3Route,
                                              adata0.defip_flex_ctr_mode,
                                              adata0.defip_flex_ctr_pool,
                                              adata0.defip_flex_ctr_base_id);
                    }
