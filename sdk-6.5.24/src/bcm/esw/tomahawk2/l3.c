                               ...
                               bcm_l3_egress_t *egr, bcm_l3_egress_t *old_egr)
{
    ...
    if (sum_valid == 3) {
        ...
        if (egr->flags & BCM_L3_TGID) {
            ...
            if (soc_feature(unit, soc_feature_td3_dlb)){
                BCM_IF_ERROR_RETURN
                    (bcm_esw_trunk_get(unit, egr->trunk, &trunk_info,
                              BCM_SWITCH_TRUNK_MAX_PORTCNT, member_array, &mcount));
                egr_mod = egr->module;
                egr->flags &= ~BCM_L3_TGID;
                for (j = 0; j < mcount; ++j) {
                    BCM_IF_ERROR_RETURN
                     (_bcm_esw_gport_resolve(unit, member_array[j].gport,
                         &(egr->module), &(egr->port), &member_tgid, &member_id));

                    BCM_IF_ERROR_RETURN
                        (bcm_th2_l3_egress_dlb_attr_set(unit, nh_index, egr, old_egr));
                }
                egr->flags |= BCM_L3_TGID;
                egr->module = egr_mod;
                return BCM_E_NONE;
...
#ifdef BCM_TRIDENT3_SUPPORT
            if (soc_feature(unit, soc_feature_td3_dlb)){
                rv = bcm_esw_trunk_get(unit, egr->trunk, &trunk_info,
                              BCM_SWITCH_TRUNK_MAX_PORTCNT, member_array, &mcount);
                if (BCM_FAILURE(rv)) {
                    if (rv == BCM_E_NOT_FOUND) {
                        /* Trunk already deleted.
                         * Return success, so that egress object
                         * can be deleted by the caller.
                         */
                        return BCM_E_NONE;
                    }
                    return rv;
                ...
                for (j = 0; j < mcount; ++j) {
                    ...
                    BCM_IF_ERROR_RETURN
                        (bcm_th2_l3_egress_dlb_attr_destroy(unit, nh_index, egr));
                }
                egr->flags |= BCM_L3_TGID;
                egr->module = egr_mod;
            }
...
#if defined(BCM_TOMAHAWK3_SUPPORT)
    if (soc_feature(unit, soc_feature_th3_style_dlb)) {
        if (_bcm_th3_mpath_validate(unit, ecmp->ecmp_intf) == BCM_E_PARAM) {
            return BCM_E_NONE;
        }
...
/*
 * Function:
 *      _bcm_td3_ecmp_dlb_port_array_get
 * Purpose:
 ...
 *      exp_intf_array      - (OUT) Expanded array of Egress forwarding objects.
 *      if_count            - (OUT) Number of elements in expanded intf_array.
 ...
 */
STATIC int
_bcm_td3_ecmp_dlb_port_array_get(int unit, bcm_l3_egress_ecmp_t *ecmp,
                                 int dlb_id, int primary_count, int intf_count,
                                 bcm_if_t *intf_array,
                                 SHR_BITDCL *ecmp_member_bitmap,
                                 bcm_if_t **exp_intf_array,
                                 int *if_count, int *port_array,
                                 SHR_BITDCL *portmap_first_use)
{
    ...
    uint32 dt, dv;
    int is_lag, tid = 0, intf_num = 0, j = 0, mcount = 0;
    bcm_trunk_member_t member_array[BCM_SWITCH_TRUNK_MAX_PORTCNT];
    int num_intfs = intf_count;
    bcm_trunk_info_t trunk_info;
    int lport_arr[BCM_SWITCH_TRUNK_MAX_PORTCNT];
    ...
    for (i = 0; i < intf_count; i++) {
        ...
        if (dt == SOC_MEM_FIF_DEST_LAG) {
            /* DGM Next hop should correspond to physical port. */
            if (ecmp->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_DGM) {
                return BCM_E_CONFIG;
            }
            tid = dv & SOC_MEM_FIF_DGPP_TGID_MASK;
            BCM_IF_ERROR_RETURN
                (bcm_esw_trunk_get(unit, tid, &trunk_info,
                          BCM_SWITCH_TRUNK_MAX_PORTCNT, member_array, &mcount));
            num_intfs += mcount - 1;
        }
    }

    *exp_intf_array = sal_alloc(sizeof(bcm_if_t) * (num_intfs),
                           "ECMP DLB Intfs");
    for (i = 0; i < intf_count; i++) {
        ...
        if (!is_lag) {
            ...
            for (j = 0; j < mcount; ++j) {
                BCM_IF_ERROR_RETURN
                    (bcm_esw_port_local_get(unit, member_array[j].gport,
                     &lport_arr[j]));
            }
        }

        for (j = 0; j < mcount; ++j) {
            port_array[intf_num] = port = lport_arr[j];
            (*exp_intf_array)[intf_num] = intf_array[i];
            intf_num++;
                      ...
                      int *old_id, SHR_BITDCL *ecmp_member_bitmap)
{
...
#ifdef BCM_TRIDENT3_SUPPORT
    ecmp_count_entry_t ecmp_count_entry;
    bcm_if_t *expanded_intf_array = NULL;
    int mcount = 0;
#endif
    dlb_ecmp_group_port_to_member_entry_t *entry_arr = NULL;
    ...
    if (SOC_IS_TRIDENT3X(unit) &&
        ...
        rv = _bcm_td3_ecmp_dlb_port_array_get(unit, ecmp, dlb_id,
                                              primary_count, intf_count,
                                              intf_array,
                                              ecmp_member_bitmap,
                                              &expanded_intf_array,
                                              &intf_count, port_array,
                                              port_map_first_use);
        intf_array = expanded_intf_array;
    } else
#endif
    {
...
#endif
    {
        ...
        for (i = 0; i < intf_count; i++) {
            if (soc_feature(unit, soc_feature_td3_dlb) &&
                (ecmp->dynamic_mode != BCM_L3_ECMP_DYNAMIC_MODE_DGM)){
                /* If LAG members, intf remains same */
                if (i > 0) {
                    if ((intf_array[i] == intf_array[i - 1]) &&
                        port_array[i] && port_array[i-1] &&
                        (port_array[i] != port_array[i-1])) {
                    } else {
                        mindex++;
                    }
                }
                if (!SHR_BITGET(ecmp_member_bitmap, mindex + location_offset)) {
                    continue;
                }
            } else {
                mindex = i;
            ...
            member_offset[j++] = mindex;
            /* Construct port_map */
            port = port_array[i];
            if (ecmp->dynamic_mode != BCM_L3_ECMP_DYNAMIC_MODE_DGM || i < primary_size) {
                SHR_BITSET(port_map, port);
                /* Construct member bitmap */
                SHR_BITSET(&member_bitmap[member_bitmap_bitdclsize * port], mindex);
            } else {
                SHR_BITSET(port_map2, port);
                /* Construct member bitmap */
    ...
    if (SOC_IS_TRIDENT3X(unit) &&
        (ecmp->dynamic_mode != BCM_L3_ECMP_DYNAMIC_MODE_DGM) &&
        (NULL != expanded_intf_array)) {
        sal_free(expanded_intf_array);
    }
#endif
    return BCM_E_NONE;
...
#if defined(BCM_TOMAHAWK3_SUPPORT)
    if (SOC_IS_TOMAHAWK3(unit)) {
        if (_bcm_th3_mpath_validate(unit, mpintf) == BCM_E_PARAM) {
            return BCM_E_NONE;
        }
    }
#endif
...
/*
 * Function:
 *      _bcm_td3_l3_egress_ecmp_port_status_set
 * Purpose:
 *      Set ECMP DLB member status.
 * Parameters:
 *      unit     - (IN) SOC unit number.
 *      nh_index - (IN) NH index.
 *      port     - (IN) port.
 ...
 */
STATIC int
_bcm_td3_l3_egress_ecmp_port_status_set(int unit, int nh_index, int port, int status)
{
    _th2_ecmp_dlb_port_membership_t *current_ptr;
    int old_status;
...
bcm_th2_l3_egress_ecmp_member_status_set(int unit, bcm_if_t intf, int status)
{
    ...
    if (soc_feature(unit, soc_feature_generic_dest)) {
        ...
        if (dt == SOC_MEM_FIF_DEST_LAG) {
            ...
            /* Next hop correspond to Trunk group */
            tid = dv & SOC_MEM_FIF_DGPP_TGID_MASK;
            rv = bcm_esw_trunk_get(unit, tid, &trunk_info,
            ...
            for (j = 0; j < mcount; ++j) {
                rv = bcm_esw_port_local_get(unit, member_array[j].gport, &lport_arr[j]);
                if (BCM_SUCCESS(rv)) {
                    rv = _bcm_td3_l3_egress_ecmp_port_status_set(unit, nh_index,
                                                          lport_arr[j], status);
                } else {
                    break;
                }
...
bcm_th2_l3_egress_ecmp_member_status_get(int unit, bcm_if_t intf, int *status)
{
    ...
    if (soc_feature(unit, soc_feature_generic_dest)) {
        ...
        if (dt == SOC_MEM_FIF_DEST_DGPP) {
            port = dv & SOC_MEM_FIF_DGPP_PORT_MASK;
        } else if ((SOC_IS_TRIDENT3X(unit)) && (dt == SOC_MEM_FIF_DEST_LAG)) {
            tid = dv & SOC_MEM_FIF_DGPP_TGID_MASK;
            rv = bcm_esw_trunk_get(unit, tid, &trunk_info,
                                   BCM_SWITCH_TRUNK_MAX_PORTCNT,
                                   member_array, &mcount);
            if (BCM_FAILURE(rv) || (mcount <= 0)) {
                return rv;
            }
            rv = bcm_esw_port_local_get(unit, member_array[0].gport, &port);
            if (BCM_FAILURE(rv)) {
                return rv;
            }
        } else {
...
/*
 * Function:
 *      bcm_td3_l3_ecmp_dlb_port_add
 * Purpose:
 *      Add a member to an DLB ECMP forwarding object.
 * Parameters:
 *      unit          - (IN) bcm device.
 ...
 */
int
bcm_td3_l3_ecmp_dlb_port_add(int unit,
                             ...
                             bcm_port_t port)
{
    ...
    bcm_l3_egress_t egr;
    int nh_index = -1;
    int dynamic_scaling_factor = -1;
    int dynamic_load_weight = -1;
    int dynamic_qsize_weight = -1;
    _th2_ecmp_dlb_port_membership_t *membership = NULL;

    bcm_l3_egress_ecmp_t_init(&ecmp);
    ecmp.ecmp_intf = ecmp_group_id + BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
    ...
    member_bitmap = sal_alloc(SHR_BITALLOCSIZE(member_bitmap_width),
                              "DLB ECMP member bitmap");
    rv = bcm_esw_trunk_get(unit, tid, &trunk_info, BCM_SWITCH_TRUNK_MAX_PORTCNT,
                           member_array, &mcount);
    if (BCM_FAILURE(rv)) {
        if (rv == BCM_E_NOT_FOUND) {
            rv = BCM_E_PARAM;
    ...
    for (j = 0; j < mcount; ++j) {
        ...
        if (lport_arr[j] == port) {
            found = 1;
        } else if (ECMP_DLB_PORT_INFO(unit, port).nh_count == 0) {
            BCM_IF_ERROR_RETURN
                (_bcm_th2_ecmp_dlb_port_attr_get(unit,
                                                 lport_arr[j],
                                                 &dynamic_scaling_factor,
    ...
    group_member_ptr =
        BCM_XGS3_L3_ECMP_DLB_MEMBER_GROUP_PTR(unit, ecmp_group_id);
    sal_memcpy(member_bitmap, group_member_ptr,
               SHR_BITALLOCSIZE(member_bitmap_width));
    ...
    for (j = 0; j < dlb_intf_count; ++j) {
        rv = bcm_xgs3_l3_egress_get(unit, dlb_intf_array[j], &egr);
        if ((!BCM_FAILURE(rv)) && (egr.flags & BCM_L3_TGID) && (egr.trunk == tid)) {
            found = 1;
            nh_index = dlb_intf_array[j] - BCM_XGS3_EGRESS_IDX_MIN(unit);
            break;
        }
    }
    if (!found) {
    ...
    if (BCM_E_NONE == _bcm_th2_ecmp_dlb_id_alloc(
                                  unit, &ecmp, &dlb_id)) {
        rv = _bcm_th2_ecmp_dlb_set(unit, &ecmp, dlb_intf_count, dlb_intf_array,
                                   dlb_intf_count, dlb_id,
                                   &old_id, member_bitmap);
        if (BCM_FAILURE(rv)) {
            goto clean_up;
        }
        rv = _bcm_th2_ecmp_dlb_free_resource(unit, ecmp_group_id,
                                             old_id, TRUE);
        if (BCM_FAILURE(rv)) {
            goto clean_up;
        }
...
/*
 * Function:
 *      bcm_td3_l3_ecmp_dlb_port_delete
 * Purpose:
 ...
 */
int
bcm_td3_l3_ecmp_dlb_port_delete(int unit,
                                bcm_if_t ecmp_group_id,
                                bcm_trunk_t tid,
                                bcm_port_t port)
{
    ...
    bcm_l3_egress_t egr;
    int nh_index = -1;
    _th2_ecmp_dlb_port_membership_t *prev_ptr = NULL;
    _th2_ecmp_dlb_port_membership_t *current_ptr = NULL, *record_ptr = NULL, *current_next;
...
#if defined(BCM_TOMAHAWK3_SUPPORT)
    if (SOC_IS_TOMAHAWK3(unit)) {
        if (_bcm_th3_mpath_validate(unit, ecmp_info->ecmp_intf) == BCM_E_PARAM) {
            goto clean_up;
        }
    }
#endif
    ecmp_group_idx = ecmp_info->ecmp_intf -
                 BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
    group_bitmap_ptr =
        BCM_XGS3_L3_ECMP_DLB_MEMBER_GROUP_PTR(unit, ecmp_group_idx);
 ...
 */
int
bcm_th2_ecmp_dlb_member_sync(int unit, uint8 **scache_ptr, int offset)
{
    int         port;
    ...
    SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_dlb_member_nh_bmp");
    ...
    SHR_BIT_ITER(bitmap, BCM_XGS3_L3_NH_TBL_SIZE(unit), nh_index) {
        ...
        if (SOC_IS_TRIDENT3X(unit)) {
            ...
            if (dest_type == SOC_MEM_FIF_DEST_DGPP) {
                ...
                /* Next hop correspond to Trunk group */
                tid = port & SOC_MEM_FIF_DGPP_TGID_MASK;
                rv = bcm_esw_trunk_get(unit, tid, &trunk_info,
                          BCM_SWITCH_TRUNK_MAX_PORTCNT, member_array, &mcount);
                if (BCM_SUCCESS(rv)) {
                    for (j = 0; j < mcount; ++j) {
                                              ...
                                              int offset)
{
    ...
    SOC_SCACHE_DUMP_START(unit, bcm_module_name(unit, BCM_MODULE_L3),
                          *scache_ptr - offset);
    SOC_SCACHE_DUMP_DATA_BEGIN("th3_l3_ecmp_dlb_member_nh_bmp");
    /* Save the standalone NHOPs from the DLP port based list */
    for (port=0 ;port < ECMP_DLB_INFO(unit)->ecmp_dlb_port_info_size ;port++) {
        current_ptr = ECMP_DLB_PORT_LIST(unit, port);
        while (current_ptr) {
    ...
    SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_dlb_group_member_bmp");
    SOC_SCACHE_DUMP_META_L((*scache_ptr), bytes);
    SOC_SCACHE_DUMP_DATA_END();
 ...
 */
int
bcm_th2_ecmp_dlb_sync(int unit, uint8 **scache_ptr, int offset)
{
    int value;
    int num_elements;
    int i;
    ...
    SOC_SCACHE_DUMP_DATA_BEGIN("ecmp_dlb_sample_rate");
    ...
    sal_memcpy((*scache_ptr), &value, sizeof(int));
    SOC_SCACHE_DUMP_DATA_BEGIN("ecmp_dlb_tx_load_min_th");
    SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
    SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
    SOC_SCACHE_DUMP_DATA_TAIL();
    SOC_SCACHE_DUMP_DATA_END();
    ...
    sal_memcpy((*scache_ptr), &value, sizeof(int));
    SOC_SCACHE_DUMP_DATA_BEGIN("ecmp_dlb_tx_load_max_th");
    SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
    SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
    SOC_SCACHE_DUMP_DATA_TAIL();
    SOC_SCACHE_DUMP_DATA_END();
    ...
    if (!soc_feature(unit, soc_feature_td3_dlb)) {
        ...
        sal_memcpy((*scache_ptr), &value, sizeof(int));
        SOC_SCACHE_DUMP_DATA_BEGIN("ecmp_dlb_qsize_min_th");
        SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
        SOC_SCACHE_DUMP_DATA_TAIL();
        SOC_SCACHE_DUMP_DATA_END();
        ...
        sal_memcpy((*scache_ptr), &value, sizeof(int));
        SOC_SCACHE_DUMP_DATA_BEGIN("ecmp_dlb_qsize_max_th");
        SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
        SOC_SCACHE_DUMP_DATA_TAIL();
        SOC_SCACHE_DUMP_DATA_END();
    ...
    sal_memcpy((*scache_ptr), &value, sizeof(int));
    SOC_SCACHE_DUMP_DATA_BEGIN("ecmp_dlb_physical_qsize_min_th");
    SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
    SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
    SOC_SCACHE_DUMP_DATA_TAIL();
    SOC_SCACHE_DUMP_DATA_END();
    ...
    sal_memcpy((*scache_ptr), &value, sizeof(int));
    SOC_SCACHE_DUMP_DATA_BEGIN("ecmp_dlb_physical_qsize_max_th");
    SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
    SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
    SOC_SCACHE_DUMP_DATA_TAIL();
    SOC_SCACHE_DUMP_DATA_END();
    ...
    SOC_SCACHE_DUMP_DATA_BEGIN("idx;ecmp_dlb_load_weight;ecmp_dlb_qsize_weight");
    ...
    for (i = 0; i < num_elements; i++) {
        ...
        (*scache_ptr) += sizeof(uint8);
        SOC_SCACHE_DUMP_DATA_TVVd(scache_ptr_tmp, i, uint8, uint8);
    }
    SOC_SCACHE_DUMP_DATA_END();

    SOC_SCACHE_DUMP_DATA_BEGIN("ecmp_dlb_id_dgm_bitmap");
    num_elements = soc_mem_index_count(unit, DLB_ECMP_GROUP_CONTROLm);
    num_elements = SHR_BITALLOCSIZE(num_elements);
    sal_memcpy((*scache_ptr), _th2_ecmp_dlb_bk[unit]->ecmp_dlb_id_dgm_bitmap,
               num_elements);
    SOC_SCACHE_DUMP_META_L((*scache_ptr), num_elements);
    ...
    if (soc_feature(unit, soc_feature_th3_style_dlb)) {
        ...
        _bcm_th3_ecmp_dlb_member_sync(unit, scache_ptr,
            SOC_SCACHE_DUMP_DATA_OFFSET((*scache_ptr)));
    }
#endif /* BCM_TOMAHAWK3_SUPPORT */
...
bcm_th2_dgm_sw_dump(int unit)
{
    ...
    if (soc_mem_is_valid(unit, DLB_ECMP_GROUP_CONTROLm) &&
        soc_property_get(unit, spn_DLB_ENABLE, 1)) {
