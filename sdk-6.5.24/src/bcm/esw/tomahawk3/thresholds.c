...
_bcm_th3_adjust_delta_hdrm_service_pools(int unit, int itm, int hdrm_pool, int
    ...
    soc_reg_t reg;
    uint32 cur_sp_limit[_TH3_ITMS_PER_DEV] = {0};
    uint32 old_hp_limit[_TH3_ITMS_PER_DEV] = {0},
           new_sp_limit[_TH3_ITMS_PER_DEV] = {0};
    int itm_shd_limit[_TH3_ITMS_PER_DEV] = {0};
    int shd_increase = 0;
    ...
    reg = MMU_THDI_HDRM_BUFFER_CELL_LIMIT_HPr;
    BCM_IF_ERROR_RETURN
        (soc_tomahawk3_itm_reg32_get(unit, reg, itm, itm, hdrm_pool,
                                     &old_hp_limit[itm]));
    /* Retrieve current value of service pool limit*/
    reg = MMU_THDI_BUFFER_CELL_LIMIT_SPr;
    BCM_IF_ERROR_RETURN
        (soc_tomahawk3_itm_reg32_get(unit, reg, itm, itm, service_pool,
                                     &cur_sp_limit[itm]));
    if (old_hp_limit[itm] != new_hp_limit) {
        delta[itm] = new_hp_limit - old_hp_limit[itm];
        /* Delta of headroom pool limit */
        BCM_IF_ERROR_RETURN(_bcm_th3_mmu_get_shared_size(unit, itm,
                                                         &itm_shd_limit[itm]));
        if (old_hp_limit[itm] > new_hp_limit) {
            /* Hdrm pool decrease; shared space increase */
            shd_increase = 1;
        ...
        if (shd_increase == 1) {
            ...
            BCM_IF_ERROR_RETURN
                (_soc_th3_mmu_config_check_limit_counter(unit,
                                                         MMU_THDI_HDRM_POOL_COUNT_HPr,
                                                         INVALIDm,
                                                         TOTAL_BUFFER_COUNTf,
                                                         hdrm_pool, itm,
                                                         new_hp_limit));
            /* Update the Service pool shared limit for corresponding service
             * pool */
            BCM_IF_ERROR_RETURN
        ...
        itm_shd_limit[itm] -= delta[itm];
        BCM_IF_ERROR_RETURN
            (_bcm_th3_mmu_set_shared_size(unit, itm, itm_shd_limit[itm]));
    }
    return BCM_E_NONE;
}

/* Get sum of service pool limits for a given ITM */
int
_bcm_th3_get_pool_total (int unit, int itm, int *sum_pool_limit)
{
    int pool;
    soc_reg_t reg = MMU_THDI_BUFFER_CELL_LIMIT_SPr;
    soc_field_t field = LIMITf;
    uint32 rval;
   ...
   for (pool = 0; pool < _TH3_MMU_NUM_POOL; pool++) {
        ...
        *sum_pool_limit += soc_reg_field_get(unit, reg, rval, field);
   }
                               ...
                               bcm_cosq_control_t type, int arg)
{
    ...
    /* Determine the ITM mapped to the ingress port */
    itm = 0;
    for (idx = 0; idx < NUM_ITM(unit); idx++) {
        if (SOC_INFO(unit).ipipe_itm_map[pipe] & (1 << idx)) {
            itm = idx;
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_th3_cosq_ing_res_limit_get(unit, gport, cosq, buffer,
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_th3_cosq_ing_pool_get(unit, gport, cosq,
                                    bcmCosqControlIngressPool,
                                    &pool));
    if (type == bcmCosqControlIngressPoolResetOffsetLimitBytes) {
        max_value = pool_limit; /* Pool reset limit cannot exceed pool limit */
    } else if (type == bcmCosqControlIngressPoolLimitBytes) {
    ...
    new_limit = (arg + _TH3_MMU_BYTES_PER_CELL - 1) / _TH3_MMU_BYTES_PER_CELL;
    if (new_limit > max_value) {
       LOG_WARN(BSL_LS_BCM_COSQ,
           (BSL_META_U(unit, "Warning: Request of %d bytes exceeds maximum of %d bytes\n"),
            new_limit * _TH3_MMU_BYTES_PER_CELL,
           max_value * _TH3_MMU_BYTES_PER_CELL));
        return BCM_E_PARAM;
    }
    pool_limit = (pool_limit + _TH3_MMU_BYTES_PER_CELL - 1) / _TH3_MMU_BYTES_PER_CELL;
    headroom_limit = (headroom_limit + _TH3_MMU_BYTES_PER_CELL - 1)
                      / _TH3_MMU_BYTES_PER_CELL;
    if (type == bcmCosqControlIngressPoolResetOffsetLimitBytes) {
        reg = MMU_THDI_CELL_RESET_LIMIT_OFFSET_SPr;
        fld_limit = OFFSETf;
        soc_reg_field_set(unit, reg, &rval, fld_limit, new_limit);

        SOC_IF_ERROR_RETURN(
            soc_tomahawk3_itm_reg32_set(unit, reg,
                                       itm, itm, pool, rval));
    } else if (type == bcmCosqControlIngressPoolLimitBytes) {
        delta[itm] = pool_limit - new_limit;
        if (delta[itm] == 0) {
            return BCM_E_NONE;
        }
        if (delta[itm] > 0) {
        ...
        delta[itm] = new_limit - headroom_limit; /* Change to service pool is
                                                    inverse of change to
                                                    headroom pool */
        if (delta[itm] > 0) {
            shd_increase = 0;
        } else {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_th3_cosq_ing_pool_get(unit, gport, cosq,
                                       ...
                                       &hdrm_pool));
        BCM_IF_ERROR_RETURN
            (_bcm_th3_adjust_delta_hdrm_service_pools(unit, itm, hdrm_pool,
                                                      pool, new_limit));
    }
...
#define _TH3_MMU_DEF_HDRM_CABLE_LEN 100
    ...
    for (itm = 0; itm < NUM_ITM(unit); itm ++) {
        if (itm_map & (1 << itm)) {
            SOC_IF_ERROR_RETURN
                (_bcm_th3_adjust_delta_hdrm_service_pools(unit, itm,
                                                          hdrm_pool, hdrm_pool,
                                                          fld_val));
        }
    }
