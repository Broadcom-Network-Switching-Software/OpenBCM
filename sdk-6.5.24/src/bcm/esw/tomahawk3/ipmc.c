...
static int
_bcm_th3_update_repl_group_entry(int unit, int repl_group,
                                 soc_pbmp_t member_bitmap, int head_index)
{
    ...
    sal_memset(&repl_group_entry, 0, sizeof(repl_group_entry));
    ...
    soc_mem_field32_set(unit, repl_group_table, &repl_group_entry,
            head_index_f, head_index);
    /* Write to repl_group 0*/
    SOC_IF_ERROR_RETURN
    ...
    rv = soc_mem_read(unit, L3_IPMCm, MEM_BLOCK_ANY, repl_group, entry);
    if (rv != BCM_E_NONE) {
    ...
    rv = soc_mem_write(unit, L3_IPMCm, MEM_BLOCK_ALL, repl_group, &entry);
    soc_mem_unlock(unit, L3_IPMCm);
    if (rv != BCM_E_NONE) {
        goto reset_group0;
    }
 ...
 reset_group0:
    /* Reset repl group 0 to point to head index 0 */
    for (i = 0; i < SOC_PBMP_WORD_MAX; i++) {
        fldbuf[i] = 0x0;
    }
    soc_mem_field_set(unit, repl_group_table, (uint32 *)&repl_group_entry,
                ...
                int start_ptr, int intf_count)
{
    ...
    if (add_member) {
        ...
        if (rev_id != BCM56980_A0_REV_ID) {
            BCM_IF_ERROR_RETURN
                (_bcm_th3_update_repl_group_entry(unit, repl_group,
                                                  new_member_bitmap,
                                                  new_head_index));
        } else {
            for (i = 0; i < SOC_PBMP_WORD_MAX; i++) {
