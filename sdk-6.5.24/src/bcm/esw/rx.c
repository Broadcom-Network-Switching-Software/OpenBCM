...
STATIC sal_mutex_t bcmi_rx_sync_mutex[BCM_MAX_NUM_UNITS];
STATIC _bcmi_rx_sync_ctrl_t bcmi_rx_sync_ctrl[BCM_MAX_NUM_UNITS] = {{0}};
STATIC int bcmi_rx_sync_init_done[BCM_MAX_NUM_UNITS] = {0};

#define RX_SYNC_LOCK(unit)      \
    sal_mutex_take(bcmi_rx_sync_mutex[unit], sal_mutex_FOREVER)

#define RX_SYNC_UNLOCK(unit)    \
    sal_mutex_give(bcmi_rx_sync_mutex[unit])
...
bcm_esw_rx_init(int unit)
{
    ...
    if (!bcmi_rx_sync_init_done[unit]) {
        bcmi_rx_sync_mutex[unit] = sal_mutex_create("rx_sync_mutex");
        if (bcmi_rx_sync_mutex[unit] == NULL) {
            return BCM_E_MEMORY;
        }

        bcmi_rx_sync_ctrl[unit].sync_mode =
            sal_sem_create("rx_sync_mode", sal_sem_BINARY, 1);
        if (bcmi_rx_sync_ctrl[unit].sync_mode == NULL) {
            sal_mutex_destroy(bcmi_rx_sync_mutex[unit]);
            bcmi_rx_sync_mutex[unit] = NULL;

            return BCM_E_MEMORY;
        }

        bcmi_rx_sync_ctrl[unit].sync_wait =
            sal_sem_create("rx_sync_wait", sal_sem_BINARY, 0);
        if (bcmi_rx_sync_ctrl[unit].sync_wait == NULL) {
            sal_sem_destroy(bcmi_rx_sync_ctrl[unit].sync_mode);
            bcmi_rx_sync_ctrl[unit].sync_mode = NULL;
            sal_mutex_destroy(bcmi_rx_sync_mutex[unit]);
            bcmi_rx_sync_mutex[unit] = NULL;
        ...
        bcmi_rx_sync_init_done[unit] = TRUE;
    }
...
int bcm_esw_rx_deinit(int unit)
{
    ...
    if (bcmi_rx_sync_init_done[unit]) {
        /* Release packets in sync RX queue if any */
        bcm_esw_rx_sync_stop(unit, bcmi_rx_sync_ctrl[unit].sn);

        if (bcmi_rx_sync_ctrl[unit].sync_mode) {
            sal_sem_destroy(bcmi_rx_sync_ctrl[unit].sync_mode);
            bcmi_rx_sync_ctrl[unit].sync_mode = NULL;
        }

        if (bcmi_rx_sync_ctrl[unit].sync_wait) {
            sal_sem_destroy(bcmi_rx_sync_ctrl[unit].sync_wait);
            bcmi_rx_sync_ctrl[unit].sync_wait = NULL;
        }

        if (bcmi_rx_sync_mutex[unit]) {
            sal_mutex_destroy(bcmi_rx_sync_mutex[unit]);
            bcmi_rx_sync_mutex[unit] = NULL;
        }

        bcmi_rx_sync_init_done[unit] = FALSE;
    }
...
#if defined(BCM_PACKET_RATE_LIMIT_HW_ONLY_SUPPORT)
    /*
     * Don't set global SW rate limiting (RX_PPS) when
     * Packet-based rate limiting to CPU for HW only is supported.
     */
    if (soc_feature(unit, soc_feature_packet_rate_limit_hw_only)) {
        if (soc_feature(unit, soc_feature_packet_rate_limit)) {
            if (SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit) ||
                SOC_IS_HURRICANE3(unit) || SOC_IS_GREYHOUND2(unit)) {
                /* Set HW global rate limiting when cosq == BCM_RX_COS_ALL. */
                return bcm_hr2_cosq_port_pps_set(unit, CMIC_PORT(unit),
                                                 BCM_RX_COS_ALL, pps);
            }
        }
        return BCM_E_UNAVAIL;
    } else
#endif /* BCM_PACKET_RATE_LIMIT_HW_ONLY_SUPPORT */
    {
        RX_PPS(unit) = pps;
    }
...
#if defined(BCM_PACKET_RATE_LIMIT_HW_ONLY_SUPPORT)
    /*
     * Get global rate limiting from HW when
     * Packet-based rate limiting to CPU for HW only is supported.
     */
    if (soc_feature(unit, soc_feature_packet_rate_limit_hw_only)) {
        if (soc_feature(unit, soc_feature_packet_rate_limit)) {
            if (SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit) ||
                SOC_IS_HURRICANE3(unit) || SOC_IS_GREYHOUND2(unit)) {
                /* Get HW global rate limiting when cosq == BCM_RX_COS_ALL. */
                return bcm_hr2_cosq_port_pps_get(unit, CMIC_PORT(unit),
                                                 BCM_RX_COS_ALL, pps);
            }
        }
        return BCM_E_UNAVAIL;
...
bcm_esw_rx_burst_set(int unit, int burst)
{
    ...
    if (soc_feature(unit, soc_feature_packet_rate_limit)) {
        if (SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit) ||
            SOC_IS_HURRICANE3(unit) || SOC_IS_GREYHOUND2(unit)) {
            return bcm_hr2_cosq_port_burst_set(unit, CMIC_PORT(unit),
                                               BCM_RX_COS_ALL, burst);
        }
    }
#endif /* BCM_HURRICANE2_SUPPORT */
...
bcm_esw_rx_cos_rate_set(int unit, int cos, int pps)
{
    ...
    /*
     * Don't set per queue SW rate limiting (RX_COS_PPS) when
     * Packet-based rate limiting to CPU for HW only is supported.
     */
    if (!soc_feature(unit, soc_feature_packet_rate_limit_hw_only)) {
...
#endif
#ifdef BCM_PACKET_RATE_LIMIT_HW_ONLY_SUPPORT
            if (soc_feature(unit, soc_feature_packet_rate_limit_hw_only)) {
                if (SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit) ||
                    SOC_IS_HURRICANE3(unit) || SOC_IS_GREYHOUND2(unit)) {
                    if (cos == BCM_RX_COS_ALL) {
                        for (i = 0; i <= RX_QUEUE_MAX(unit); i++) {
                            BCM_IF_ERROR_RETURN
                                (bcm_hr2_cosq_port_pps_set(unit,
                                                           CMIC_PORT(unit), i,
                                                           pps));
                        }
                    } else {
...
#ifdef BCM_PACKET_RATE_LIMIT_HW_ONLY_SUPPORT
    /*
     * Get per queue rate limiting from HW when
     * Packet-based rate limiting to CPU for HW only is supported.
     */
    if (soc_feature(unit, soc_feature_packet_rate_limit_hw_only)) {
        if (soc_feature(unit, soc_feature_packet_rate_limit)) {
...
static bcm_rx_t
_bcm_rx_sync_cb(int unit, bcm_pkt_t *pkt, void *cookie)
{
    _bcmi_rx_sync_ctrl_t *rsc = &bcmi_rx_sync_ctrl[unit];
    bcm_pkt_t *sync_pkt;

    sync_pkt = sal_alloc(sizeof(bcm_pkt_t), "rx_sync_pkt");
    if (sync_pkt == NULL) {
        return BCM_RX_NOT_HANDLED;
    }

    sal_memcpy(sync_pkt, pkt, sizeof(bcm_pkt_t));
    if (pkt->pkt_data == &pkt->_pkt_data) {
        /* Update pkt_data if necessary */
        sync_pkt->pkt_data = &sync_pkt->_pkt_data;
    }
    sync_pkt->next = NULL;
    ...
    if (!rsc->head) {
        ...
        rsc->tail->next = (void *)sync_pkt;
        rsc->tail = sync_pkt;
    }

    sal_sem_give(rsc->sync_wait);
...
/*
 * Function:
 *      bcm_esw_rx_sync_start
 * Purpose:
 *      Start sync rx mode.
 * Parameters:
 *      unit              - (IN) device id.
 *      flags             - (IN) Flags for sync RX mode.
 *      sn                - (OUT) Serial number for this sync RX.
 * Returns:
 *      BCM_E_*
 */

int
bcm_esw_rx_sync_start(int unit, uint32 flags, int *sn)
{
    _bcmi_rx_sync_ctrl_t *rsc = &bcmi_rx_sync_ctrl[unit];
    int rv = BCM_E_NONE;
    ...
    rv = bcm_esw_rx_register(unit, "sync_rx", _bcm_rx_sync_cb,
                             BCM_RX_PRIO_MAX, NULL,
                             BCM_RCO_F_ALL_COS | BCM_RCO_F_INTR);
    if (BCM_FAILURE(rv)) {
        return rv;
    }

    sal_sem_take(rsc->sync_mode, sal_sem_FOREVER);

    RX_SYNC_LOCK(unit);

    rsc->sync_flags = flags;
    rsc->sn = sal_thread_id_get();
...
/*
 * Function:
 *      bcm_esw_rx_sync_stop
 * Purpose:
 *      Stop sync rx mode.
 * Parameters:
 *      unit              - (IN) device id.
 *      sn                - (IN) Serial number for this sync RX.
 * Returns:
 *      BCM_E_*
 */

int
bcm_esw_rx_sync_stop(int unit, int sn)
{
    _bcmi_rx_sync_ctrl_t *rsc = &bcmi_rx_sync_ctrl[unit];
    bcm_pkt_t *sync_pkt;
    int rv = BCM_E_NONE;
    ...
    rv = bcm_esw_rx_unregister(unit, _bcm_rx_sync_cb, BCM_RX_PRIO_MAX);
    if (BCM_FAILURE(rv)) {
        return rv;
    }
    ...
    if (rsc->curr) {
        sync_pkt = rsc->curr;
        bcm_esw_rx_free(unit, sync_pkt->alloc_ptr);
        sal_free(sync_pkt);
        rsc->curr = NULL;
    }
    ...
    while (rsc->head) {
        sync_pkt = rsc->head;
        rsc->head = (bcm_pkt_t *)rsc->head->next;
        bcm_esw_rx_free(unit, sync_pkt->alloc_ptr);
        sal_free(sync_pkt);
    }
 ...
 *      bcm_esw_rx_sync
 * Purpose:
 *      Receive a packet in sync RX mode.
 * Parameters:
 *      unit              - (IN) device id.
 *      sn                - (IN) Serial number for this sync RX.
 *      pkt               - (OUT) Packet handle.
 *      usec              - (IN) Maximum number of microseconds to wait.
 * Returns:
 *      BCM_E_*
 */

int
bcm_esw_rx_sync(int unit, int sn, bcm_pkt_t **pkt, int usec)
{
    _bcmi_rx_sync_ctrl_t *rsc = &bcmi_rx_sync_ctrl[unit];
    int rv = BCM_E_NONE;
    ...
    if (rsc->curr) {
        bcm_esw_rx_free(unit, rsc->curr->alloc_ptr);
        sal_free(rsc->curr);
        rsc->curr = NULL;
    }
    ...
    do {
        ...
        rv = sal_sem_take(rsc->sync_wait, usec);
        if (BCM_FAILURE(rv)) {
            *pkt = NULL;
            return rv;
        }
