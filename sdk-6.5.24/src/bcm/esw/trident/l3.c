         ...
         if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
             ...
             if (SOC_IS_TRIDENT3X(unit)) {
                 ...
                 if (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                     ...
                     if (ecmp_idx < (BCM_XGS3_L3_ECMP_TBL_SIZE(unit) / 2)) {
                         int ref_count =
                            BCM_XGS3_L3_ENT_REF_CNT(BCM_XGS3_L3_TBL_PTR(unit,
                                            ecmp_grp), nh_idx[idx]) - 1;
                         if (ref_count >= 0) {

                             if (nh_idx[idx] < BCMI_L3_ECMP_OVERLAY_ENTRIES(unit)) {
                                 nh_idx[idx] += BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
                             }
                             nh_idx[idx] += BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
                         }
                     }
    ...
    if ((soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) &&
        ...
        grp_mem = ECMP_GROUP_HIERARCHICALm;
               ...
               (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                                         ecmp_mode_hierarchical) &&
               hw_grp_id >= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit)) {
        hw_grp_id -= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
    }
#endif
    /* Cast input buffer. */
...
#ifdef BCM_TOMAHAWK3_SUPPORT
                if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
                    (nh_idx[nh_cycle_idx] >= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit))) {
                    nh_idx[nh_cycle_idx] -= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
                }
#endif
                if (!soc_feature(unit, soc_feature_post_ifp_single_stage_ecmp)) {
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (!soc_feature(unit, soc_feature_post_ifp_single_stage_ecmp)) {
            rv = soc_mem_write(unit, INITIAL_L3_ECMP_GROUPm, MEM_BLOCK_ALL,
                    hw_grp_id, &initial_l3_ecmp_group_entry);
        ...
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash)) {
            ...
            SOC_IF_ERROR_RETURN(READ_L3_ECMP_COUNTm(unit, MEM_BLOCK_ANY,
                        hw_grp_id, &old_ecmp_count_entry));
...
_bcm_td_l3_ecmp_grp_del (int unit, int ecmp_grp, void *info)
{
    ...
    char *buffer;    /* Buffer to clear the table. */
    int alloc_sz = 0;
    int max_grp_size = 0;  /* Maximum ecmp group size.*/
    ...
    if (max_grp_size > L3_ECMP_MEM_DMA_THRESHOLD) {
        alloc_sz = max_grp_size * sizeof(ecmp_entry_t);
        /* Allocate memory buffer. */
        buffer = soc_cm_salloc(unit, alloc_sz, "ECMP entries");
        if (buffer == NULL) {
            return (BCM_E_MEMORY);
        }
        /* Zero out the whole memory range */
        sal_memset(buffer, 0x0, alloc_sz);

        rv = soc_mem_write_range(unit, L3_ECMPm, MEM_BLOCK_ALL, ecmp_idx,
                                 ecmp_idx + max_grp_size - 1, buffer);
        ...
        /* Write buffer to hw INITIAL_L3_ECMPm table. */
        if (!soc_feature(unit, soc_feature_post_ifp_single_stage_ecmp)) {
            alloc_sz = max_grp_size * sizeof(initial_l3_ecmp_entry_t);
            /* Allocate memory buffer. */
            buffer = soc_cm_salloc(unit, alloc_sz, "ECMP entries");
            if (buffer == NULL) {
                return (BCM_E_MEMORY);
            ...
            /* Zero out the whole memory range */
            sal_memset(buffer, 0x0, alloc_sz);
            rv = soc_mem_write_range(unit, INITIAL_L3_ECMPm, MEM_BLOCK_ALL, ecmp_idx,
                                     ecmp_idx + max_grp_size - 1, buffer);
            soc_cm_sfree(unit, buffer);
            if (BCM_FAILURE(rv)) {
                return rv;
    ...
    if ((soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) &&
        ...
        } else if (ecmp_idx >= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit)) {
            ecmp_idx -= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
        }
    }
#endif
