    ...
    if (SOC_MEM_IS_VALID(unit, mem[0])) {
        ...
        if (SOC_WARM_BOOT(unit)) {
            ...
            for (i = 0; i < soc_mem_index_count(unit, ing_mem); i++) {
                ...
                } else if (key_type == VLXLT_HASH_KEY_TYPE_VLAN_MAC_PORT) {
                        if (soc_mem_field32_get(unit, ing_mem, xlate_entry,
                                    MAC_PORT__VLAN_ACTION_VALIDf)) {
                            idx = soc_mem_field32_get(unit, ing_mem,
                                    xlate_entry, MAC_PORT__TAG_ACTION_PROFILE_PTRf);
                        }
#endif
                } else {
                    idx = soc_mem_field32_get(unit, ing_mem, xlate_entry,
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) && defined(INCLUDE_L3)
                if (soc_mem_field_valid(unit, egr_mem, type_fld)) {
                                  ...
                                  bcm_vlan_action_set_t *action)
{
    ...
    if (action->forwarding_domain) {
        if (_BCM_VPN_IS_VPLS(action->forwarding_domain)) {
            int vfi_index;

            _BCM_VPN_GET(vfi_index, _BCM_VPN_TYPE_VFI, action->forwarding_domain);
            if (soc_mem_field_valid(unit, mem, XLATE__VFIf)) {
                soc_mem_field32_set(unit, mem, vent, XLATE__VFIf, vfi_index);
            }
        }
    }
#if defined(BCM_TRIUMPH3_SUPPORT)
                                            ...
                                            bcm_vlan_translate_key_t key_type)
{
    ...
    if ((rv == SOC_E_NONE) &&
        ...
        if (use_ipri_mapping_table && ipri_mapping_profile) {
            BCM_IF_ERROR_RETURN
                (_bcm_tr2_qos_egr_mpls_external_ref_del(unit,ipri_mapping_profile));
        }
        if (use_opri_mapping_table && opri_mapping_profile)
        {
            BCM_IF_ERROR_RETURN
                             ...
                             _bcm_vlan_translate_traverse_t *trvs_info)
{
    ...
    soc_field_t type_field = ENTRY_TYPEf;
#endif
    int egr_key_type = bcmVlanTranslateKeyPortDouble;
#if defined(BCM_TRIDENT_SUPPORT) && defined(INCLUDE_L3)
    soc_mem_t ing_mem = VLAN_XLATEm;
    if (SOC_MEM_IS_VALID(unit, VLAN_XLATE_1_DOUBLEm)) {
        ing_mem = VLAN_XLATE_1_DOUBLEm;
    ...
    if (mem == egr_mem || mem == egr_mem2) {
...
#endif /* BCM_TRIDENT3_SUPPORT && INCLUDE_L3 */
        {
            ...
            if (soc_feature(unit, soc_feature_egr_vxlate_supports_dgpp)) {
                int sys_port_type_dgpp = 0;
                int val = 0, rv = 0;

                if (SOC_MEM_FIELD_VALID(unit, egr_mem, PORT_TYPE_DGPPf)) {
                    if (soc_mem_field32_get(unit, mem, vent, PORT_TYPE_DGPPf)) {
                        sys_port_type_dgpp = 1;
                    }
                } else {
                    rv = bcm_esw_switch_control_get(unit, bcmSwitchSystemPortOverride, &val);
                    BCM_IF_ERROR_RETURN(rv);
                    if (val == 1) {
                        sys_port_type_dgpp = 1;
                    }
                }
                if (sys_port_type_dgpp) {
                    bcm_module_t modid =
        ...
        trvs_info->key_type = egr_key_type;
    } else {
        _bcm_trx_vlan_action_profile_entry_get(unit, trvs_info->action, 
                                               profile_idx);
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)) {
        return _bcm_td3_vlan_match_action_add(unit, options,
                port, match_info, action_set);
    }
#endif
    if (!SOC_MEM_IS_VALID(unit, VLAN_MACm)) {
        return BCM_E_UNAVAIL;
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)) {
        return _bcm_td3_vlan_match_action_get(unit, port, match_info, action_set);
    }
#endif
    if (!SOC_MEM_IS_VALID(unit, VLAN_MACm)) {
        return BCM_E_UNAVAIL;
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)) {
        return  _bcm_td3_vlan_match_action_delete(unit, port, vlan_match_info);
    }
#endif
    sal_memset(&vment, 0, sizeof(vlan_mac_entry_t));
    sal_memset(&res_vment, 0, sizeof(vlan_mac_entry_t));
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)) {
        return  _bcm_td3_vlan_match_action_delete_all(unit, match, port);
    }
#endif
    if (!SOC_MEM_IS_VALID(unit, VLAN_MACm)) {
        return BCM_E_UNAVAIL;
