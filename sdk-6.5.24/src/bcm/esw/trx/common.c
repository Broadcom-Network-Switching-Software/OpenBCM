...
#ifdef BCM_TRIDENT3_SUPPORT
    int egr_tunnel_ecn_encap_created; /* Flag to check if EGR_TUNNEL_ECN_ENCAP profile creation */
...
#endif /* BCM_TOMAHAWK2_SUPPORT or BCM_TOMAHAWK3_SUPPORT */
#if defined(BCM_TRIDENT3_SUPPORT)
    soc_profile_mem_t *egr_tunnel_ecn_encap; /*cache of EGR_TUNNEL_ECN_ENCAP profile */
#endif /* BCM_TRIDENT3_SUPPORT */
...
#endif /* BCM_TOMAHAWK_SUPPORT */
#ifdef BCM_TRIDENT3_SUPPORT
#define EGR_TUNNEL_ECN_ENCAP(_unit_) \
                    (_bcm_common_bk_info[_unit_].egr_tunnel_ecn_encap)
#endif /* BCM_TRIDENT3_SUPPORT */
#if defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT) || \
    defined(BCM_TRIDENT3_SUPPORT)
...
_bcm_common_free_resource(int unit, _bcm_common_bookkeeping_t *info)
{
    ...
    if (soc_feature(unit, soc_feature_miml) || 
        ...
        if (soc_feature(unit, soc_feature_tunnel_init_ecn)) {
            if (info->egr_tunnel_ecn_encap) {
                if (info->egr_tunnel_ecn_encap_created) {
                    (void) soc_profile_mem_destroy(unit, info->egr_tunnel_ecn_encap);
                }
                sal_free(info->egr_tunnel_ecn_encap);
                info->egr_tunnel_ecn_encap = NULL;
            }
        }
#endif /* BCM_TRIDENT3_SUPPORT */
#if defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT) || \
...
#endif /* BCM_TOMAHAWK_SUPPORT */
#ifdef BCM_TRIDENT3_SUPPORT
    info->egr_tunnel_ecn_encap_created = FALSE;
#endif /* BCM_TRIDENT3_SUPPORT */
#if defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT) || \
    defined(BCM_TRIDENT3_SUPPORT)
 ...
 */
        ...
        if (SOC_MEM_IS_VALID(unit, mem)) {
            if (NULL == info->misc_port_profile) {
                ...
                mems[mems_cnt] = mem;
                if (SOC_IS_TRIDENT3X(unit)) {
                    mem_words[mems_cnt] = sizeof(ifp_i2e_classid_select_entry_t) / sizeof(uint32);
                } else {
                ...
                if (SOC_MEM_IS_VALID(unit, mem)) {
                    mems[mems_cnt] = mem;
                    if (SOC_IS_TRIDENT3X(unit)) {
                        mem_words[mems_cnt] = sizeof(ifp_hg_classid_select_entry_t) /
                                              sizeof(uint32);
                    } else {
                ...
                if (SOC_IS_TRIDENT3X(unit)) {
                    sal_memset(&ifp_ent, 0, sizeof(ifp_i2e_classid_select_entry_t));
                    sal_memset(&ifp_hg_ent, 0, sizeof(ifp_hg_classid_select_entry_t));
                    entries[0] = &ifp_ent;
                    entries[1] = &ifp_hg_ent;
                } else {
                    sal_memset(&fp_ent, 0, sizeof(fp_i2e_classid_select_entry_t));
                    sal_memset(&fp_hg_ent, 0, sizeof(fp_hg_classid_select_entry_t));
                    entries[0] = &fp_ent;
                    entries[1] = &fp_hg_ent;
                }
                BCM_IF_ERROR_RETURN
        ...
        if (SOC_MEM_IS_VALID(unit, mem)) {
            ...
            if (NULL == info->egr_dscp_table) {
                ...
                if (soc_property_get(unit,
                    ...
                    info->egr_dscp_table->flags |= SOC_PROFILE_MEM_F_NO_SHARE;
                }
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (soc_feature(unit, soc_feature_tunnel_init_ecn)) {
        /* The memory high index bits 6,5 along with index bits 1,0 are
         * profile entry indexes. Index bits 4,3,2 is pointing to 8 profiles.
         */
        mem = EGR_TUNNEL_ECN_ENCAPm;
        if (SOC_MEM_IS_VALID(unit, mem)) {
            int i;
            int profile_tbl_size;

            profile_tbl_size = soc_mem_index_count(unit,
                                EGR_TUNNEL_ECN_ENCAPm) /
                                _BCM_TD3_TUNNEL_ECN_ENCAP_PROFILE_TABLE_NUM;
            if (NULL == info->egr_tunnel_ecn_encap) {
                info->egr_tunnel_ecn_encap =
                         sal_alloc(sizeof(soc_profile_mem_t),
                                   "EGR_TUNNEL_ECN_ENCAP Profile Mem");
                if (info->egr_tunnel_ecn_encap == NULL) {
                    _bcm_common_free_resource(unit, info);
                    return BCM_E_MEMORY;
                }
                soc_profile_mem_t_init(info->egr_tunnel_ecn_encap);
                info->egr_tunnel_ecn_encap->flags = SOC_PROFILE_MEM_F_NO_SHARE;
                mem_words[0] =
                    sizeof(egr_tunnel_ecn_encap_entry_t) / sizeof(uint32);
                mem_words[1] = mem_words[0];
                mem_words[2] = mem_words[0];
                mem_words[3] = mem_words[0];
                for (i = 0; i < _BCM_TD3_TUNNEL_ECN_ENCAP_PROFILE_TABLE_NUM; i++) {
                    index_min[i] = i * profile_tbl_size;
                    index_max[i] = i * profile_tbl_size +
                                   (profile_tbl_size - 1);
                    mems[i] = EGR_TUNNEL_ECN_ENCAPm;
                }
                ...
                rv = soc_profile_mem_index_create(unit, mems, mem_words,
                                   ...
                                   _BCM_TD3_TUNNEL_ECN_ENCAP_PROFILE_TABLE_NUM,
                                   info->egr_tunnel_ecn_encap);
                info->egr_tunnel_ecn_encap_created = TRUE;
                if (rv < 0) {
                    _bcm_common_free_resource(unit, info);
                    return rv;
                }
                              ...
                              int entries_per_set, uint32 *index, uint32 flags)
{
    ...
    if (soc_property_get(unit, spn_QOS_MAP_EGR_DSCP_NONSHARABLE_MODE, 0)) {
        EGR_DSCP_TABLE(unit)->flags |= SOC_PROFILE_MEM_F_NO_SHARE;
    }
...
/*
 * Function:
 *      _bcm_egr_dscp_table_entry_write_mode_get
 * Purpose:
 *      Internal function to get write mode of EGR_DSCP_TABLE table
 * Parameters:
 *      unit    -  (IN) Device number.
 *      index   -  (IN) Base index to get
 *      mode    -  (OUT) table write mode
 ...
 */
int
_bcm_egr_dscp_table_entry_write_mode_get(int unit,
        uint32 index, int *mode)
{
    int rv;
    COMMON_LOCK(unit);
    rv =
       soc_profile_mem_write_mode_get(unit, EGR_DSCP_TABLE(unit), index, mode);
    COMMON_UNLOCK(unit);
    return rv;
}
...
/*
 * Function:
 *      _bcm_ecn_pkt_to_tnl_ecn_entry_add
 * Purpose:
 *      Add profile entries to the EGR_TUNNEL_ECN_ENCAP table
 * Parameters:
 *      unit    -  (IN) Device number.
 *      entries -  (IN) Array of pointer to table entries set
 *      entries_per_set - (IN)  Number of entries in the set
 ...
 */
int
_bcm_ecn_pkt_to_tnl_ecn_entry_add(int unit, void **entries,
                              int entries_per_set, uint32 *index)
{
    int rv;
    rv = soc_profile_mem_add(unit, EGR_TUNNEL_ECN_ENCAP(unit), entries,
                             entries_per_set, index);
    return rv;
}
...
/*
 * Function:
 *      _bcm_ecn_pkt_to_tnl_ecn_entry_delete
 * Purpose:
 *      Delete a profile from the EGR_TUNNEL_ECN_ENCAP table
 * Parameters:
 *      unit    -  (IN) Device number.
 *      index   -  (IN) Base index for the profile to be deleted
 * Returns:
 *      BCM_X_XXX
 */
int
_bcm_ecn_pkt_to_tnl_ecn_entry_delete(int unit, int index)
{
    int rv;
    rv = soc_profile_mem_delete(unit, EGR_TUNNEL_ECN_ENCAP(unit), index);
    return rv;
}

/*
 * Function:
 *      _bcm_ecn_pkt_to_tnl_ecn_entry_get
 * Purpose:
 *      Retrieve profile entries from EGR_TUNNEL_ECN_ENCAP table
 * Parameters:
 *      unit    -  (IN) Device number.
 *      index   -  (IN) Base index for the entires allocated in HW
 *      count   -  (IN) Number of entries in the set
...
int
_bcm_ecn_pkt_to_tnl_ecn_entry_get(int unit, int index, int count, void **entries)
{
    int rv;
    rv = soc_profile_mem_get(unit, EGR_TUNNEL_ECN_ENCAP(unit), index, count,
                             entries);
    return rv;
}
...
/*
 * Function:
 *      _bcm_ecn_pkt_to_tnl_ecn_entry_reference
 * Purpose:
 *      Increase the reference count of the profile entries
 * Parameters:
 *      unit    -  (IN) Device number
 *      index   -  (IN) Base index for the entry
 *      entries_per_set - (IN) number of entries
 *
 * Returns:
 *      BCM_X_XXX
 */
...
int
_bcm_ecn_pkt_to_tnl_ecn_entry_reference(int unit, int index,
                                     int entries_per_set)
{
    int rv;
    rv = soc_profile_mem_reference(unit, EGR_TUNNEL_ECN_ENCAP(unit), index,
                                   entries_per_set);
    return rv;
}
#endif /* BCM_TRIDENT3_SUPPORT */
    ...
    for (i = min_stat_types; i < max_stat_types; i++) {
        for (j = min_dirs; j < max_dirs; j++) {
            ...
            for (k = min_index ; k < max; k++) {
                /*
                 * COVERITY
                 * Caller will assure min_index >= 0.
                 */
