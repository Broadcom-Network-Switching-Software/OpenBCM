                             ...
                             bcm_stat_val_t type, uint64 *val)
{
    ...
    case snmpBcmCustomReceive9:
        if (SOC_REG_IS_VALID(unit, RDBGC9_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, RDBGC9_64r, &count);
        }
        break;
    case snmpBcmCustomReceive10:
        if (SOC_REG_IS_VALID(unit, RDBGC10_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, RDBGC10_64r, &count);
        }
    break;
    case snmpBcmCustomReceive11:
        if (SOC_REG_IS_VALID(unit, RDBGC11_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, RDBGC11_64r, &count);
        }
    break;
    case snmpBcmCustomReceive12:
        if (SOC_REG_IS_VALID(unit, RDBGC12_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, RDBGC12_64r, &count);
        }
    break;
    case snmpBcmCustomReceive13:
        if (SOC_REG_IS_VALID(unit, RDBGC13_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, RDBGC13_64r, &count);
        }
    break;
    case snmpBcmCustomReceive14:
        if (SOC_REG_IS_VALID(unit, RDBGC14_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, RDBGC14_64r, &count);
        }
    break;
    case snmpBcmCustomReceive15:
        if (SOC_REG_IS_VALID(unit, RDBGC15_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, RDBGC15_64r, &count);
        }
    break;
#endif
    ...
    case snmpBcmCustomTransmit14:
        ...
        break;
    case snmpBcmCustomTransmit15:
        if (SOC_REG_IS_VALID(unit, TDBGC15_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, TDBGC15_64r, &count);
        }
        break;
    default:
        return BCM_E_PARAM;
...
#define _DBG_CNTR_TD3_IS_VALID(unit, type)                                 \
    ((type >= snmpBcmCustomReceive0 && type <= snmpBcmCustomReceive8)   || \
     (type >= snmpBcmCustomReceive9 && type <= snmpBcmCustomReceive15)  || \
     (type >= snmpBcmCustomTransmit0 && type <=snmpBcmCustomTransmit14) || \
     (type == snmpBcmCustomTransmit15))
...
#ifdef BCM_TRIDENT3_SUPPORT
bcm_dbg_cntr_t bcm_dbg_cntr_td3_rx[] = {
    { snmpBcmCustomReceive0,   RDBGC0_64r,  RDBGC0_SELECTr  },
    { snmpBcmCustomReceive1,   RDBGC1_64r,  RDBGC1_SELECTr  },
    { snmpBcmCustomReceive2,   RDBGC2_64r,  RDBGC2_SELECTr  },
    { snmpBcmCustomReceive3,   RDBGC3_64r,  RDBGC3_SELECTr  },
    { snmpBcmCustomReceive4,   RDBGC4_64r,  RDBGC4_SELECTr  },
    { snmpBcmCustomReceive5,   RDBGC5_64r,  RDBGC5_SELECTr  },
    { snmpBcmCustomReceive6,   RDBGC6_64r,  RDBGC6_SELECTr  },
    { snmpBcmCustomReceive7,   RDBGC7_64r,  RDBGC7_SELECTr  },
    { snmpBcmCustomReceive8,   RDBGC8_64r,  RDBGC8_SELECTr  },
    { snmpBcmCustomReceive9,   RDBGC9_64r,  RDBGC9_SELECTr  },
    { snmpBcmCustomReceive10,  RDBGC10_64r, RDBGC10_SELECTr },
    { snmpBcmCustomReceive11,  RDBGC11_64r, RDBGC11_SELECTr },
    { snmpBcmCustomReceive12,  RDBGC12_64r, RDBGC12_SELECTr },
    { snmpBcmCustomReceive13,  RDBGC13_64r, RDBGC13_SELECTr },
    { snmpBcmCustomReceive14,  RDBGC14_64r, RDBGC14_SELECTr },
    { snmpBcmCustomReceive15,  RDBGC15_64r, RDBGC15_SELECTr }
};

bcm_dbg_cntr_t bcm_dbg_cntr_td3_tx[] = {
    { snmpBcmCustomTransmit0,  TDBGC0_64r,  TDBGC0_SELECTr  },
    { snmpBcmCustomTransmit1,  TDBGC1_64r,  TDBGC1_SELECTr  },
    { snmpBcmCustomTransmit2,  TDBGC2_64r,  TDBGC2_SELECTr  },
    { snmpBcmCustomTransmit3,  TDBGC3_64r,  TDBGC3_SELECTr  },
    { snmpBcmCustomTransmit4,  TDBGC4_64r,  TDBGC4_SELECTr  },
    { snmpBcmCustomTransmit5,  TDBGC5_64r,  TDBGC5_SELECTr  },
    { snmpBcmCustomTransmit6,  TDBGC6_64r,  TDBGC6_SELECTr  },
    { snmpBcmCustomTransmit7,  TDBGC7_64r,  TDBGC7_SELECTr  },
    { snmpBcmCustomTransmit8,  TDBGC8_64r,  TDBGC8_SELECTr  },
    { snmpBcmCustomTransmit9,  TDBGC9_64r,  TDBGC9_SELECTr  },
    { snmpBcmCustomTransmit10, TDBGC10_64r, TDBGC10_SELECTr },
    { snmpBcmCustomTransmit11, TDBGC11_64r, TDBGC11_SELECTr },
    { snmpBcmCustomTransmit12, TDBGC12_64r, TDBGC12_SELECTr },
    { snmpBcmCustomTransmit13, TDBGC13_64r, TDBGC13_SELECTr },
    { snmpBcmCustomTransmit14, TDBGC14_64r, TDBGC14_SELECTr },
    { snmpBcmCustomTransmit15, TDBGC15_64r, TDBGC15_SELECTr }
};
		    ...
		    bcm_stat_val_t type, uint32 flags)
{
    ...
    int count_rx = 0;
    int count_tx = 0;
    bcm_dbg_cntr_t bcm_dbg_cntr_rx_local[16];
    bcm_dbg_cntr_t bcm_dbg_cntr_tx_local[16];
    ...
    if (SOC_IS_TRIDENT3X(unit)) {
        if (!_DBG_CNTR_TD3_IS_VALID(unit, type)) {
            return BCM_E_PARAM;
        }
    } else {
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit)) {
        count_rx = COUNTOF(bcm_dbg_cntr_td3_rx);
        sal_memcpy(bcm_dbg_cntr_rx_local,
                 bcm_dbg_cntr_td3_rx, sizeof (bcm_dbg_cntr_td3_rx));

        count_tx = COUNTOF(bcm_dbg_cntr_td3_tx);
        sal_memcpy(bcm_dbg_cntr_tx_local,
                 bcm_dbg_cntr_td3_tx, sizeof (bcm_dbg_cntr_td3_tx));
    } else
#endif /* BCM_TRIDENT3_SUPPORT */
    {
        count_rx = COUNTOF(bcm_dbg_cntr_rx);
        sal_memcpy(bcm_dbg_cntr_rx_local,
                 bcm_dbg_cntr_rx, sizeof (bcm_dbg_cntr_rx));

        count_tx = COUNTOF(bcm_dbg_cntr_tx);
        sal_memcpy(bcm_dbg_cntr_tx_local,
                 bcm_dbg_cntr_tx, sizeof (bcm_dbg_cntr_tx));
    }

    for (i = 0; i < count_rx; i++) {
        if (bcm_dbg_cntr_rx_local[i].counter == type) {
            if (SOC_IS_FBX(unit)) {
                SOC_IF_ERROR_RETURN(soc_reg32_get(unit,  bcm_dbg_cntr_rx_local[i].select,
                ...
                if (SOC_IS_FBX(unit)) {
                    ...
                    soc_reg_field_set(unit, bcm_dbg_cntr_rx_local[i].select,
                                      &ctr_sel, BITMAPf, flags & mask);
                }
                SOC_IF_ERROR_RETURN(soc_reg32_set(unit, bcm_dbg_cntr_rx_local[i].select,
                                                  REG_PORT_ANY, 0, ctr_sel));
            }
        }
    }
    ...
    for (i = 0; i < count_tx; i++) {
        if (bcm_dbg_cntr_tx_local[i].counter == type) {
            if (SOC_IS_FBX(unit)) {
                uint32 mask;

                SOC_IF_ERROR_RETURN(soc_reg32_get(unit, bcm_dbg_cntr_tx_local[i].select,
                                                  REG_PORT_ANY, 0, &ctr_sel));
                ...
                soc_reg_field_set(unit, bcm_dbg_cntr_tx_local[i].select, &ctr_sel,
                                  BITMAPf, flags & mask);

                SOC_IF_ERROR_RETURN(soc_reg32_set(unit, bcm_dbg_cntr_tx_local[i].select,
                                                  REG_PORT_ANY, 0, ctr_sel));
            }
            else {
                soc_counter_set32_by_reg(unit, bcm_dbg_cntr_tx_local[i].reg, 0, 0);
            }
            break;
        }
    }
        ...
        if (bcm_dbg_cntr_rx_local[i].counter == type) {
            if (SOC_IS_FBX(unit)) {
                SOC_IF_ERROR_RETURN(soc_reg32_get(unit, bcm_dbg_cntr_rx_local[i].select,
                                                  REG_PORT_ANY, 0, &ctr_sel));
                if (SOC_IS_FBX(unit)) {
                    *flags = soc_reg_field_get(unit, bcm_dbg_cntr_rx_local[i].select,
                                               ctr_sel, BITMAPf);
                }
            }
            break;
    ...
    for (i = 0; i < count_tx; i++) {
        if (bcm_dbg_cntr_tx_local[i].counter == type) {
            if (SOC_IS_FBX(unit)) {
                SOC_IF_ERROR_RETURN(soc_reg32_get(unit, bcm_dbg_cntr_tx_local[i].select,
                                                  REG_PORT_ANY, 0, &ctr_sel));
                    *flags = soc_reg_field_get(unit, bcm_dbg_cntr_tx_local[i].select,
                                               ctr_sel, BITMAPf);
	    }
        break;
        }
                        ...
                        uint32 *result)
{
    ...
    case bcmDbgCntMCIDXE:
        ...
        _BCM_TH3_BIT_SET(unit, result, 24); /* MULTICAST_INDEX_ERROR */
    ...
    case bcmDbgCntRxMplsControlWordInvalid:
        ...
        _BCM_TH3_BIT_SET(unit, result, 52); /* MPLS_INVALID_CW */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntRxMplsPayloadInvalid:
        ...
        _BCM_TH3_BIT_SET(unit, result, 53); /* MPLS_INVALID_PAYLOAD */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntRxMplsLabelLookupMiss:
        ...
        _BCM_TH3_BIT_SET(unit, result, 54); /* MPLS_LABEL_MISS */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    case bcmDbgCntRxMplsActionInvalid:
        _BCM_TD3_BIT_SET(unit, result, 25);
        _BCM_TH3_BIT_SET(unit, result, 51); /* MPLS_INVALID_ACTION */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntRxTunnelDecapEcnError:
        ...
        _BCM_TH3_BIT_SET(unit, result, 49); /* TUNNEL_DECAP_ECN */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    case bcmDbgCntRxL3SrcDiscardDrop:
        _BCM_TD3_BIT_SET(unit, result, 71);
        _BCM_TH3_BIT_SET(unit, result, 18); /* L3SRC_DISCARD */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntRxL3TtlError:
        ...
        _BCM_TH3_BIT_SET(unit, result, 21); /* L3_TTL_ERROR */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    case bcmDbgCntRxL3HeaderError:
        _BCM_TD3_BIT_SET(unit, result, 74);
        _BCM_TH3_BIT_SET(unit, result, 20); /* L3_HDR_ERROR */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntRxL3DstLookupDrop:
        ...
        _BCM_TH3_BIT_SET(unit, result, 19); /* L3_DST_LOOKUP_MISS */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntRxTagUntagDiscardDrop:
        ...
        _BCM_TH3_BIT_SET(unit, result, 47); /* TAG_UNTAG_DISCARD */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntRxFieldChangeL2NoRedirectDrop:
        ...
        _BCM_TH3_BIT_SET(unit, result, 34); /* FP_CHANGE_L2_FIELDS_NO_REDIRECT_DROP */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    case bcmDbgCntRxNextHopDrop:
        _BCM_TD3_BIT_SET(unit, result, 89);
        _BCM_TH3_BIT_SET(unit, result, 36); /* NEXT_HOP_DROP */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntIngressProtectionDataDrop:
        ...
        _BCM_TH3_BIT_SET(unit, result, 37); /* PROTECTION_DATA_DROP */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    case bcmDbgCntRxSrcKnockoutDrop:
        _BCM_TD3_BIT_SET(unit, result, 119);
        _BCM_TH3_BIT_SET(unit, result, 95); /* SRC_PORT_KNOCKOUT_DROP */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    ...
    case bcmDbgCntRxBlockMaskDrop:
        ...
        _BCM_TH3_BIT_SET(unit, result, 75); /* BLOCK_MASK_DROP */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    case bcmDbgCntRxDlbRhResolutionError:
        ...
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
    case bcmDbgCntRxFieldDelayDrop:
        _BCM_TH3_BIT_SET(unit, result, 98); /* IFP_DELAYED_DROP */
        _BCM_DEFAULT_BIT_SET_E_PARAM (unit, result);
        break;
                        ...
                        bcm_custom_stat_trigger_t trigger, int value)
{
    ...
    SOC_IF_ERROR_RETURN(soc_reg32_get(unit, bcm_dbg_cntr_tx_local[sel_idx].select,
        REG_PORT_ANY, 0, &ctr_sel));
    sel_bmp = soc_reg_field_get(unit, bcm_dbg_cntr_tx_local[sel_idx].select,
        ctr_sel, BITMAPf);

    sel_bmp = (value) ? (sel_bmp | ctrl_bmp[0]) :
        (sel_bmp & (~ctrl_bmp[0]));

    soc_reg_field_set(unit, bcm_dbg_cntr_tx_local[sel_idx].select,
        &ctr_sel, BITMAPf, sel_bmp);
    SOC_IF_ERROR_RETURN(soc_reg32_set(unit, bcm_dbg_cntr_tx_local[sel_idx].select,
        REG_PORT_ANY, 0, ctr_sel));
    ...
    for (i = 0; i < count_rx; i++) {
        if (bcm_dbg_cntr_rx_local[i].counter == type) {
           ...
           if (result < 32) {
               ...
               ctrl_bmp = soc_reg_field_get(unit, bcm_dbg_cntr_rx_local[i].select,
                                            ctr_sel, BITMAPf);
               ...
               soc_reg_field_set(unit, bcm_dbg_cntr_rx_local[i].select,
                                 &ctr_sel, BITMAPf, ctrl_bmp);
               return soc_reg32_set(unit, bcm_dbg_cntr_rx_local[i].select,
                                    REG_PORT_ANY, 0, ctr_sel);
           } else if (result < 64) {
    ...
    for (i = 0; i < count_tx; i++) {
        if (bcm_dbg_cntr_tx_local[i].counter == type) {
            ...
            if (result < 32) {
                SOC_IF_ERROR_RETURN(soc_reg32_get(unit, bcm_dbg_cntr_tx_local[i].select,
                                                  REG_PORT_ANY, 0, &ctr_sel));
                ctrl_bmp = soc_reg_field_get(unit, bcm_dbg_cntr_tx_local[i].select,
                                             ctr_sel, BITMAPf);
                ...
                soc_reg_field_set(unit, bcm_dbg_cntr_tx_local[i].select,
                                  &ctr_sel, BITMAPf, ctrl_bmp);
                return soc_reg32_set(unit, bcm_dbg_cntr_tx_local[i].select,
                                     REG_PORT_ANY, 0, ctr_sel);
            } else if (result < 64) {
...
#ifdef BCM_TRIDENT3_SUPPORT
/*
 * Internal function to enable the L2 source port pruning packet drop
 * debug counter for VP flows.
 */
int
_bcm_esw_stat_l2_src_port_prune_drop_debug_ctr_set(int unit, int value)
{
    uint32 ctr_sel = 0, i = 0, ctr_bmp = 0;
    uint32 count_tx = 0;
    /* SDK reserved counter for tx dropped packets */
    bcm_stat_val_t type = snmpBcmCustomTransmit3;

    count_tx = COUNTOF(bcm_dbg_cntr_td3_tx);

    /* Defined in EP_EGR_DROP_VECTOR:SOURCE_VIRTUAL_PORT_REMOVAL */
    ctr_bmp = 0x100;

    for (i = 0; i < count_tx; i++) {
        if (bcm_dbg_cntr_td3_tx[i].counter == type) {
            SOC_IF_ERROR_RETURN(soc_reg32_get(unit, bcm_dbg_cntr_td3_tx[i].select,
                                              REG_PORT_ANY, 0, &ctr_sel));


            ctr_sel = (value) ? (ctr_sel & (~ctr_bmp)) :
                  (ctr_sel | ctr_bmp);

            soc_reg_field_set(unit, bcm_dbg_cntr_td3_tx[i].select, &ctr_sel,
                              BITMAPf, ctr_sel);

            SOC_IF_ERROR_RETURN(soc_reg32_set(unit, bcm_dbg_cntr_td3_tx[i].select,
                                                  REG_PORT_ANY, 0, ctr_sel));
            break;
        }
    }
...
int
_bcm_esw_stat_l2_src_port_prune_drop_debug_ctr_get(int unit, int *value)
{
    uint32 ctr_sel = 0, i = 0, flags = 0;
    uint32 count_tx = 0;
    int idx = 8;
    /* SDK reserved counter for tx dropped packets */
    bcm_stat_val_t type = snmpBcmCustomTransmit3;
    ...
    for (i = 0; i < count_tx; i++) {
        if (bcm_dbg_cntr_td3_tx[i].counter == type) {
            SOC_IF_ERROR_RETURN(soc_reg32_get(unit, bcm_dbg_cntr_td3_tx[i].select,
                                              REG_PORT_ANY, 0, &ctr_sel));
            flags = soc_reg_field_get(unit, bcm_dbg_cntr_td3_tx[i].select,
                                           ctr_sel, BITMAPf);
            /* coverity[ptr_arith] */
            if (SHR_BITGET(&flags, idx)) {
                *value = 0;
            } else {
                *value = 1;
            }
...
/*
 * Internal function to enable the multicast packet drop debug counter
 * at the egress.
 */
int
_bcm_esw_stat_egr_mcast_drop_debug_ctr_set(int unit, int value)
{
    uint32 ctr_sel = 0, sel_bmp = 0, i = 0, ctr_bmp = 0;
    uint32 count_tx = 0;
    /* SDK reserved counter for tx dropped packets */
    bcm_stat_val_t type = snmpBcmCustomTransmit3;
    ...
    /* Defined in EGR_STATS_FORMAT:MC_DROP */
    ctr_bmp = 0x80;

    for (i = 0; i < count_tx; i++) {
        if (bcm_dbg_cntr_td3_tx[i].counter == type) {
            BCM_IF_ERROR_RETURN(READ_TDBGC_SELECT_2r(unit, i, &ctr_sel));
            sel_bmp = soc_reg_field_get(unit, TDBGC_SELECT_2r, ctr_sel,
                                        BITMAPf);

            sel_bmp = (value) ? (sel_bmp & (~ctr_bmp)) :
                        (sel_bmp | ctr_bmp);

            soc_reg_field_set(unit, TDBGC_SELECT_2r, &ctr_sel, BITMAPf,
                              sel_bmp);

            BCM_IF_ERROR_RETURN(WRITE_TDBGC_SELECT_2r(unit, i, ctr_sel));
            break;
        }
    }
    return BCM_E_NONE;
...
int
_bcm_esw_stat_egr_mcast_drop_debug_ctr_get(int unit, int *value)
{
    uint32 ctr_sel = 0, i = 0, sel_bmp = 0;
    uint32 count_tx = 0;
    int idx = 7;
    /* SDK reserved counter for tx dropped packets */
    bcm_stat_val_t type = snmpBcmCustomTransmit3;
