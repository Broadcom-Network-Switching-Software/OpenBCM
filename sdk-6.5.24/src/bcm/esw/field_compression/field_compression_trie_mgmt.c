/*! \file field_compression_trie_mgmt.c
 *
...
STATIC int
_bcm_field_compare_bits(SHR_BITDCL *key1, SHR_BITDCL *key2,
                        uint32_t key1_pfx, uint32_t key2_pfx, uint8_t max_pfx)
{
 ...
 */
STATIC void
_bcm_field_compress_key_insert_child(_bcm_field_shr_trie_md_t *trie,
                                     _bcm_field_compress_key_t *curr_node,
                                     _bcm_field_compress_key_t *parent_node)
{
    _bcm_field_compress_key_t *temp_node;
    int index;
 ...
 */
STATIC void
_bcm_field_compress_key_delete_child(_bcm_field_shr_trie_md_t *trie,
                                     _bcm_field_compress_key_t *curr_node,
                                     _bcm_field_compress_key_t *curr_node_prev_sib,
                                     _bcm_field_compress_key_t *parent_node)
{
    int index;
...
STATIC int
_bcm_field_compress_key_node_alloc(int unit,
                                   _bcm_field_basic_info_key_t *node_info,
                                   _bcm_field_compress_key_t **node,
                                   _bcm_field_shr_trie_md_t *trie)
{
    ...
    if (trie->ckey_acquire) {
        ...
        _BCM_FIELD_COMP_ALLOC(*node, sizeof(_bcm_field_compress_key_t),
                         "_bcm_fieldCompressionNode");
    }

    (*node)->prefix = node_info->prefix;
    if (node_info->flags & _BCM_FIELD_TRIE_NODE_WIDTH_COMPRESSED) {
    ...
    if (node_info->flags) {
        ...
        sal_memcpy(&(*node)->cid, &node_info->cid, sizeof(_bcm_field_cid_t));
        sal_memcpy(&(*node)->cid_mask, &node_info->cid_mask,
                   sizeof(_bcm_field_cid_t));
    }

    if (node_info->prefix < trie->max_prefix_mask) {
        _BCM_FIELD_COMP_ALLOC((*node)->child, sizeof(_bcm_field_compress_key_t *) *
                         (trie->max_prefix_mask - node_info->prefix),
                         "_bcm_fieldCompressionNodeChildren");
    }

    sal_memcpy(&(*node)->key, &node_info->key, sizeof(_bcm_field_key_t));
...
STATIC void
_bcm_field_compress_key_node_free(int unit, _bcm_field_compress_key_t *node,
                                  _bcm_field_shr_trie_md_t *trie)
{
    /* Free node */
    if (node) {
        if (node->child) {
            _BCM_FIELD_COMP_FREE(node->child);
        }
        if (trie->ckey_release && node != trie->root) {
            (void)trie->ckey_release(unit, trie->stage_id, trie->trie_type, node);
        } else {
            _BCM_FIELD_COMP_FREE(node);
        }
    }
}
                                ...
                                _bcm_field_shr_trie_md_t *trie)
{
    ...
    _bcm_field_compress_key_t *parent_node;
    _bcm_field_compress_key_t *curr_node, *curr_node_prev_sib;

    _BCM_FIELD_COMP_NULL_CHECK(trie);
    _BCM_FIELD_COMP_NULL_CHECK(trie->root);
    if (in_node == NULL) {
        rv = (_bcm_field_compress_key_node_alloc(unit, in_node_info, &in_node, trie));
        if (SHR_FAILURE(rv)) {
            return (rv);
        }
        in_node->flags = in_node_info->flags;
        sal_memcpy(&in_node->cid, &in_node_info->cid, sizeof(_bcm_field_cid_t));
        sal_memcpy(&in_node->cid_mask, &in_node_info->cid_mask, sizeof(_bcm_field_cid_t));
    }

    if (in_node->prefix > trie->max_prefix_mask) {
        _bcm_field_compress_key_node_free(unit, in_node, trie);
        return (SHR_E_PARAM);
    }

    /* If node is list compressed */
    if (in_node->flags & _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED) {
        curr_node = trie->root;
    ...
    for (prefix = 1; prefix <= trie->max_prefix_mask; prefix++) {
        ...
        while (curr_node != NULL) {
            ...
            rv = _bcm_field_compare_bits(in_node->key.w, curr_node->key.w,
                                         in_node->prefix, curr_node->prefix,
            ...
            if (rv) {
                ...
                /* Current node sub-trie need to be moved under incoming node */
                    ...
                    _bcm_field_compress_key_delete_child(trie, curr_node,
                                                         curr_node_prev_sib,
                                                         parent_node);

                    if (!in_node_linked) {
                        /* Link the incoming node to the parent node as a child */
                        _bcm_field_compress_key_insert_child(trie, in_node,
                                                             parent_node);
                        in_node_linked = 1;
                    }

                    /* Link the current node to incoming node as a child */
                    _bcm_field_compress_key_insert_child(trie, curr_node, in_node);
    ...
    if (!in_node_linked) {
        ...
        _bcm_field_compress_key_insert_child(trie, in_node, parent_node);
    }
...
{
    int rv = SHR_E_PARAM, in_node_linked = 0, prefix, index;
    _bcm_field_compress_key_t *parent_node, *in_node = NULL;
    _bcm_field_compress_key_t *curr_node, *curr_node_prev_sib;
    _bcm_field_basic_info_key_t node_info;

    _BCM_FIELD_COMP_NULL_CHECK(trie);
    _BCM_FIELD_COMP_NULL_CHECK(trie->root);
    _BCM_FIELD_COMP_NULL_CHECK(in_node_info);
    ...
    if (in_node_info->prefix == 0) {
        ...
        sal_memcpy(&in_node_info->cid, &trie->root->cid, sizeof(_bcm_field_cid_t));
        sal_memcpy(&in_node_info->cid_mask, &trie->root->cid_mask,
                   sizeof(_bcm_field_cid_t));
        return (SHR_E_NONE);
    }

    if (in_node_info->flags & _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED) {
    ...
    if (!(in_node_info->flags & _BCM_FIELD_TRIE_NODE_WIDTH_COMPRESSED)) {
        ...
        while (curr_node) {
            rv = _bcm_field_compare_bits(in_node_info->key.w, curr_node->key.w,
                                         in_node_info->prefix, curr_node->prefix,
                                         trie->max_prefix_mask);
            if (rv) {
                ...
                sal_memset(&node_info, 0, sizeof(_bcm_field_basic_info_key_t));
                sal_memcpy(node_info.key.w, &curr_node->key,
                           sizeof(_bcm_field_key_t));
                sal_memcpy(node_info.cid.w, &curr_node->cid,
                           sizeof(_bcm_field_cid_t));
                ...
                node_info.flags = _BCM_FIELD_TRIE_NODE_WIDTH_COMPRESSED;

                /* Unlink current node from list compressed nodes list */
                curr_node_prev_sib->sibling = curr_node->sibling;
                /* Free current node */
                _bcm_field_compress_key_node_free(unit, curr_node, trie);
                curr_node = curr_node_prev_sib;

                /* Add current node in trie */
                rv = _bcm_field_compress_key_insert(unit, &node_info, trie,
                                                    user_data);

                if (SHR_FAILURE(rv)) {
                    if (rv == SHR_E_RESOURCE) {
                        /* Add the node back in list compression list */
                        node_info.flags = _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED;
                        (void)(_bcm_field_compress_key_insert(unit, &node_info,
                                                              trie, user_data));
                    }
                    return (rv);
                }
                ...
                if (!in_node_linked) {
                    ...
                    in_node_info->flags = _BCM_FIELD_TRIE_NODE_WIDTH_COMPRESSED;
                    in_node_info->ref_count = 1;
                    sal_memset(in_node_info->cid.w, 0, sizeof(_bcm_field_cid_t));
                    sal_memset(in_node_info->cid_mask.w, 0,
                               sizeof(_bcm_field_cid_t));
                    rv = (_bcm_field_compress_key_insert(unit, in_node_info,
                                                         trie, user_data));
                    if (SHR_FAILURE(rv)) {
                        return (rv);
                    }
        ...
        while (curr_node != NULL) {
            ...
            if (rv) {
                if (in_node_info->flags & _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED) {
                    ...
                    in_node->flags &= ~_BCM_FIELD_TRIE_NODE_LIST_COMPRESSED;
                    in_node_info->flags &= ~_BCM_FIELD_TRIE_NODE_LIST_COMPRESSED;
                    sal_memset(in_node->cid.w, 0, sizeof(_bcm_field_cid_t));
                    sal_memset(in_node_info->cid.w, 0, sizeof(_bcm_field_cid_t));
                }
    ...
    /* Update CID/CID Mask */
    rv = _bcm_field_compress_id_update(unit, trie, parent_node, user_data);

    if (SHR_FAILURE(rv)) {
        if (rv == SHR_E_RESOURCE) {
            /* Delete the newly inserted node if we ran out of cid bits */
            (void)(_bcm_field_compress_key_delete(unit, in_node_info, trie,
                                                  user_data));
        }
        return (rv);
    }
    ...
    in_node_info->ref_count = in_node->ref_count;
    sal_memcpy(&in_node_info->cid, &in_node->cid, sizeof(_bcm_field_cid_t));
...
_bcm_field_compress_key_lookup(int unit,
                               ...
                               _bcm_field_shr_trie_md_t *trie)
{
    _bcm_field_compress_key_t *parent_node, *curr_node;
    ...
    if (in_node_info->prefix == 0) {
        ...
        sal_memcpy(&in_node_info->cid, &parent_node->cid,
                   sizeof(_bcm_field_cid_t));
    ...
    if (found) {
        /* Copy cid/cid_mask to in_node_info */
        sal_memcpy(&in_node_info->cid, &curr_node->cid, sizeof(_bcm_field_cid_t));
        sal_memcpy(&in_node_info->cid_mask, &curr_node->cid_mask,
                   sizeof(_bcm_field_cid_t));
        in_node_info->ref_count = curr_node->ref_count;
        rv = SHR_E_NONE;
 ...
 */
STATIC int
_bcm_field_compress_key_traverse_trie_children(int unit, uint32_t *oindex,
                                               _bcm_field_compress_key_t *in_node,
                                               _bcm_field_shr_trie_md_t *trie,
                                               _bcm_field_basic_info_key_t *out)
{
    ...
    uint32_t num_child;
    _bcm_field_compress_key_t *curr_node;
    _bcm_field_compress_key_t *list_cmp_node;
    ...
    for (prefix = in_node->prefix + 1;
        ...
        while (curr_node) {
            ...
            rv = _bcm_field_compare_bits(in_node->cid_mask.w, curr_node->cid_mask.w,
            ...
            rv = (_bcm_field_compress_key_traverse_trie_children(unit, oindex,
                                                                 curr_node,
                                                                 trie, out));
            if (SHR_FAILURE(rv)) {
                return rv;
            ...
            while (list_cmp_node) {
                ...
                rv = _bcm_field_compare_bits(list_cmp_node->key.w,
                                             curr_node->key.w,
                                             curr_node->prefix,
                                             list_cmp_node->prefix,
                                             trie->max_prefix_mask);
                if (rv != 0) {
                    return (SHR_E_INTERNAL);
                }
 ...
 */
int
_bcm_field_compress_key_delete(int unit, _bcm_field_basic_info_key_t *in_node_info,
                               _bcm_field_shr_trie_md_t *trie, void *user_data)
{
    _bcm_field_compress_key_t *parent_node, *curr_node, *curr_node_prev_sib;
    _bcm_field_compress_key_t *lc_node = NULL;
    _bcm_field_basic_info_key_t node_info;
            ...
            if (rv) {
                ...
                /* Incoming key has same prefix as current nodes prefix */
                    ...
                    for (prefix = curr_node->prefix + 1;
                        ...
                        _bcm_field_compress_key_insert_child(trie,
    ...
    if (lc_node) {
        ...
        rv = _bcm_field_compress_key_insert(unit, &node_info, trie, user_data);
        if (SHR_FAILURE(rv)) {
            if (rv == SHR_E_RESOURCE) {
                /* Add the node back in list compression list */
                node_info.flags = _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED;
                (void)(_bcm_field_compress_key_insert(unit, &node_info, trie,
                                                      user_data));
                ...
                in_node_info->ref_count = 1;
                sal_memset(in_node_info->cid.w, 0, sizeof(_bcm_field_cid_t));
                sal_memset(in_node_info->cid_mask.w, 0, sizeof(_bcm_field_cid_t));
                (void)(_bcm_field_compress_key_insert(unit, in_node_info, trie,
                                                      user_data));
            }
            return (rv);
        }
        ...
        _bcm_field_compress_key_node_free(unit, lc_node, trie);
        lc_node = NULL;
    } else {
        /* Update CID/CID Mask */
        rv = (_bcm_field_compress_id_update(unit, trie, parent_node, user_data));
...
_bcm_field_compress_trie_create(int unit, uint8_t max_prefix_mask,
                                uint8_t max_cid_mask, uint8_t trie_type,
                                _bcm_field_fn_cid_update_t cid_update_cb,
                                _bcm_field_shr_trie_md_t **trie_new)
{
    _bcm_field_shr_trie_md_t *trie = NULL;
    _bcm_field_basic_info_key_t root_info;
    int rv = SHR_E_MEMORY;

    _BCM_FIELD_COMP_ALLOC(trie, sizeof(_bcm_field_shr_trie_md_t),
                          "_bcm_fieldTrieManagementDataStructure");
    *trie_new = trie;
    ...
    sal_memset(&root_info, 0, sizeof(_bcm_field_basic_info_key_t));

    rv = (_bcm_field_compress_key_node_alloc(unit, &root_info, &trie->root, trie));
    if (SHR_FAILURE(rv)) {
        return (rv);
    }
...
STATIC void
_bcm_field_compress_key_nodes_delete(int unit, _bcm_field_shr_trie_md_t *trie,
                                     _bcm_field_compress_key_t *in_node)
{
    int prefix;
    _bcm_field_compress_key_t *curr_node = NULL;
    _bcm_field_compress_key_t *curr_node_sib = NULL;
    ...
    for (prefix = in_node->prefix + 1;
        ...
        while (curr_node) {
            ...
            _bcm_field_compress_key_nodes_delete(unit, trie, curr_node);
...
{
    _bcm_field_compress_key_t *curr_node = NULL;
    _bcm_field_compress_key_t *curr_node_tmp = NULL;
    if (trie) {
        if (trie->root) {
            curr_node = trie->root->sibling;
            while (curr_node) {
                ...
                curr_node = curr_node->sibling;
                _bcm_field_compress_key_node_free(unit, curr_node_tmp, trie);
                curr_node_tmp = NULL;
            }
            /* Delete all nodes in the trie. */
            _bcm_field_compress_key_nodes_delete(unit, trie, trie->root);
        }
        _BCM_FIELD_COMP_FREE(trie);
        trie = NULL;
    }
    return SHR_E_NONE;
}
