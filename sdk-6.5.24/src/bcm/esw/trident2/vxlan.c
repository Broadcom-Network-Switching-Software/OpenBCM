...
STATIC int
_bcm_td3_vxlan_phb_dscp_tunnel_term_init(int unit, int map_id, int32* phb_ctrl_id);
#endif
...
#if defined(BCM_TRIDENT3_SUPPORT)
STATIC int
_bcm_td3_phb_ctrl_id_alloc(int unit, SHR_BITDCL *bitmap, int map_size)
{
    int i;
...
STATIC int
_bcm_td3_vxlan_phb_dscp_tunnel_term_init(int unit, int32 qos_map_id, int32* ctrl_id){
    ...
    if (!SHR_BITGET(td3_vxlan_info->phb_map_bitmap, hw_map_idx)) {
        ...
        phb_ctrl_id = _bcm_td3_phb_ctrl_id_alloc(unit,
                   td3_vxlan_info->phb_sel_used, max_ctrl_id);
        if (phb_ctrl_id < 0) {
            LOG_ERROR(BSL_LS_BCM_VXLAN,
                 (BSL_META_U(unit,
        ...
        if ((tnl_info->qos_map_id >> _BCM_QOS_MAP_SHIFT) ==
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_td3_vxlan_phb_dscp_tunnel_term_init(unit,
                         tnl_info->qos_map_id, &phb_ctrl_id));
