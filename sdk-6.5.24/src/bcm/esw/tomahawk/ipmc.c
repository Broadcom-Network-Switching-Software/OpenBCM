...
_bcm_th_repl_head_info_init(int unit)
{
    ...
    int repl_head_tbl_size_adjust = 1;
#ifdef BCM_HURRICANE4_SUPPORT
    if (soc_feature(unit, soc_feature_td3x1_reduced_ipmc)) {
        /* Replication Head is reduced from 32K to 16K in TD3X1 */
        repl_head_tbl_size_adjust = 2;
    }
#endif
    ...
    for (pipe = 0; pipe < NUM_PIPES(unit); pipe++) {
        ...
        per_pipe_repl_head_tbl_size =
            (per_pipe_repl_head_tbl_size / repl_head_tbl_size_adjust);
...
_bcm_th_repl_list_entry_info_init(int unit)
{
    ...
    int pipe;
    soc_mem_t repl_list_table;
    int repl_list_tbl_size_adjust = 1;
#ifdef BCM_HURRICANE4_SUPPORT
    if (soc_feature(unit, soc_feature_td3x1_reduced_ipmc)) {
        /* Replication List is reduced from 16K to 8K in TD3X1 */
        repl_list_tbl_size_adjust = 2;
    }
#endif
    ...
    for (pipe = 0; pipe < NUM_PIPES(unit); pipe++) {
        ...
        _th_repl_list_entry_info[unit][pipe].num_entries =
            (_th_repl_list_entry_info[unit][pipe].num_entries /
                                    repl_list_tbl_size_adjust);
    ...
    if (SOC_IS_MAVERICK2(unit)) {
        if (slice == 0) {
            if (IS_LB_PORT(unit, port)) {
                ...
                soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                        &initial_copy_count_entry, ICC_BIT_CPUf, copy_count);
            } else if (phy_port >= 1 && phy_port <= 80) {
                soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field_get(unit,
                        &initial_copy_count_entry, ICC_BIT_FP_PXf, copy_count_buf);
                count_mask = (1 << count_width) - 1;
                count_shift = (phy_port - 1) * 2 % 32;
    ...
    if (SOC_IS_HELIX5(unit) || SOC_IS_HURRICANE4(unit)) {
        if (slice == 0) {
            if (IS_LB_PORT(unit, port)) {
                ...
                soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                        &initial_copy_count_entry, ICC_BIT_CPUf, copy_count);
            } else if (phy_port >= 1 && phy_port <= (SOC_IS_HELIX5(unit) ? 76 : 68)) {
                soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field_get(unit,
                        &initial_copy_count_entry, ICC_BIT_FP_PXf, copy_count_buf);
        ...
        if (!BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, if_array[if_num])) {
            ...
            if (is_l3) { /* L3 interface is being added to IPMC group */
                if (REPL_L3_INTF_NEXT_HOP_IPMC(unit, if_array[if_num]) == -1) {
                    ...
                    if (soc_feature(unit, soc_feature_l3_intf_mcast_nhi_auto_del)) {
                        if (!BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, if_array[if_num])) {
                            nh_index = REPL_L3_INTF_NEXT_HOP_IPMC(unit, if_array[if_num]);
                            bcm_xgs3_nh_inc_ref_count(unit, nh_index);
                        }
                    }
                }
                SHR_BITSET(intf_vec,
                               ...
                               int if_max, int encap_id, int is_l3)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (match) {
            ...
            /* delete the NHI */
            if (soc_feature(unit, soc_feature_l3_intf_mcast_nhi_auto_del)) {
                if (!BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, if_array[if_count])) {
                    if (REPL_L3_INTF_NEXT_HOP_IPMC(unit, if_array[if_count]) != -1) {
                        int ref_count = -1;
                        int nh_index = REPL_L3_INTF_NEXT_HOP_IPMC(unit, if_array[if_count]);
                        bcm_xgs3_nh_ref_count_get(unit, nh_index, &ref_count);
                        if (ref_count == 1) {
                            rv = bcm_xgs3_nh_del(unit, 0, nh_index);
                            if (BCM_SUCCESS(rv)) {
                                REPL_L3_INTF_NEXT_HOP_IPMC(unit, if_array[if_count]) = -1;
                            }
                        } else {
                            bcm_xgs3_nh_dec_ref_count(unit, nh_index);
                        }
                    }
                }
            }
                         ...
                         int repl_group, int intf_cnt)
{
    ...
    uint32 regval;
    bcm_pbmp_t            pbmp_all;
    int num_ports = SOC_PBMP_PORT_MAX;
