                           ...
                           int qual, uint32 *data, uint32 *mask)
{
    ...
    if (qual == bcmFieldQualifyExactMatchHitStatus) {
        if (BCM_FIELD_QSET_TEST(fg->qset,
            ...
            qual_id = (bcm_field_qualify_t)_bcmFieldQualifyExactMatchHitStatusLookup0;
        ...
        if (BCM_FIELD_QSET_TEST(fg->qset,
            ...
            qual_id = (bcm_field_qualify_t)_bcmFieldQualifyExactMatchHitStatusLookup1;
            rv = _bcm_field_th_qualify_set(unit, entry, qual_id,
                                                     data, mask,
                                                     _FP_QUALIFIER_DEL);
            BCM_IF_ERROR_RETURN(rv);
    ...
    } else if (qual == (bcm_field_qualify_t)bcmFieldQualifyExactMatchActionClassId) {
        if (BCM_FIELD_QSET_TEST(fg->qset,
                     _bcmFieldQualifyExactMatchActionClassIdLookup0)) {
            qual_id = (bcm_field_qualify_t)_bcmFieldQualifyExactMatchActionClassIdLookup0;
            rv = _bcm_field_th_qualify_set(unit, entry, qual_id,
                                                     data, mask,
                                                     _FP_QUALIFIER_DEL);
            BCM_IF_ERROR_RETURN(rv);
        ...
        if (BCM_FIELD_QSET_TEST(fg->qset,
            ...
            qual_id = (bcm_field_qualify_t)_bcmFieldQualifyExactMatchActionClassIdLookup1;
            rv = _bcm_field_th_qualify_set(unit, entry, qual_id,
                                                     data, mask,
                                                     _FP_QUALIFIER_DEL);
            BCM_IF_ERROR_RETURN(rv);
        ...
        if (BCM_FIELD_QSET_TEST(fg->qset,
            ...
            qual_id = (bcm_field_qualify_t)_bcmFieldQualifyExactMatchGroupClassIdLookup0;
            rv = _bcm_field_th_qualify_set(unit, entry, qual_id,
                                                     data, mask,
                                                     _FP_QUALIFIER_DEL);
            BCM_IF_ERROR_RETURN(rv);
        ...
        if (BCM_FIELD_QSET_TEST(fg->qset,
            ...
            qual_id = (bcm_field_qualify_t)_bcmFieldQualifyExactMatchGroupClassIdLookup1;
            rv = _bcm_field_th_qualify_set(unit, entry, qual_id,
                                                     data, mask,
                                                     _FP_QUALIFIER_DEL);
            BCM_IF_ERROR_RETURN(rv);
        ...
        if (_FP_INVALID_INDEX != f_st->hw_index) {
...
#endif
            {
                ...
                rv = _bcm_field_th_val_set(buf, &hw_ctr_base_idx, 13,
                        (soc_feature(unit, soc_feature_td3_style_fp) ? 13 :
                        (SOC_IS_TOMAHAWK2(unit) ? 13 : 12)));
                BCM_IF_ERROR_RETURN(rv);
            }
            /*
             * Not detaching the stats if there is no change in it during entry modification (re-install).
                                   ...
                                   uint32 *abuf)
{
    ...
    _field_group_t *fg = NULL;        /* Field Group Control.         */
    _field_action_t *fa = NULL;       /* Field Action Structure.      */
    void *act_prof_entry = NULL;               /* Profile table buffer pointer */
    ...
    _FP_XGS3_ALLOC(act_prof_entry, (sizeof(uint8) * hw_bitmap_entries),
                               "action profile entry");

    if (NULL == act_prof_entry) {
        return(BCM_E_MEMORY);
    }
        ...
        if (_FP_ACTION_VALID & fa->flags) {
            ...
            if ((0 == field_action_set) ||
                ...
                if(soc_feature(unit, soc_feature_ifp_action_profiling)) {
                   ...
                   if (BCM_FAILURE(rv)) {
                       LOG_ERROR(BSL_LS_BCM_FP, (BSL_META("TD3 Action profile"
                                                 "data set failed.\n")));
                       goto error;
                   }
                }
    ...
    if (counter_present) {
        ...
        rv = _field_th_em_flex_stat_action_set(unit, f_ent, policy_mem,
                                               acts->data);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_FP, (BSL_META("Flex stat Action set failed.\n")));
            goto error;
        }
        acts->valid = 1;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_FP, (BSL_META("TH Ing policer act set failed.\n")));
        goto error;
    }
    if(soc_feature(unit, soc_feature_ifp_action_profiling)) {
        meter_base = 0;
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_FP, (BSL_META("Profile mem get failed.\n")));
        goto error;
    }
    ...
    if(soc_feature(unit, soc_feature_td3_style_fp)) {
        ...
        for (idx = 0; idx < hw_bitmap_entries; idx++)
        {
            rv = _bcm_field_th_val_get(action_set_bitmap, &act_valid, idx, 1);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_FP, (BSL_META("TH Action Get failed.\n")));
                goto error;
            }
            if (act_valid == 1) {
                /*idx Bit is set in the action profile */
...
error:
    if(act_prof_entry != NULL) {
        sal_free(act_prof_entry);
    }
    return rv;
}
