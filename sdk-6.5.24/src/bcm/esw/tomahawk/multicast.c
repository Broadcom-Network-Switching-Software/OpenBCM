...
#define _TH_NUM_TRUNKS(unit) \
        (SOC_IS_TOMAHAWK3(unit) ? 144 : \
         SOC_IS_HURRICANE4(unit) ? 127 : \
         SOC_IS_TRIDENT3X(unit) ? 2048 : 1024)
...
#define _TH_MAX_NUM_TRUNKS 2048
...
bcm_th_set_free_aggid(int unit, int pipe, int aggid)
{
    ...
    aggid_bmp = th_aggid_used_bmp[unit][pipe].pipe_aggid_bmp;
    if ((aggid >= _TH_NUM_AGGID_PER_PIPE(unit)) || (aggid < 0)) {
        return BCM_E_PARAM;
    }
...
bcm_th_port_last_member_check(int unit, bcm_port_t port, int *val)
{
    ...
    for (i = 0; i < total_num_ports; i++) {
        ...
        port_aggid_info = &(th_port_aggid[unit][i]);
        /* local port to match the pipe. */
        local_port = i;
#if defined(BCM_CHANNELIZED_SWITCHING_SUPPORT)
        if (soc_feature(unit, soc_feature_channelized_switching)) {

            if (i > FB6_NUM_PORT) {
                /* In case of subport, get the local port from subport. */
                BCM_IF_ERROR_RETURN(
                    _bcm_coe_subtag_subport_num_local_port_get(
                        unit, i, &local_port));
            }
        }
#endif
...
#endif
        if ((port_aggid_info->trunk_id == tgid) &&
            (SOC_INFO(unit).port_pipe[local_port] == pipe)) {
            member_cnt++;
        }
    }
                               ...
                               uint16 recovered_ver)
{
    ...
    /* Up until 1_5, the num_trunks were as below, which has
     * changed with 1_6, hence until 1_5 we will only recover
     * what would have been cached.
     */
    if (recovered_ver >= BCM_IPMC_WB_VERSION_1_2 &&
        recovered_ver <= BCM_IPMC_WB_VERSION_1_5) {
        num_trunks = SOC_IS_TOMAHAWK3(unit) ? 144 : 1024;
    } else {
        num_trunks = _TH_NUM_TRUNKS(unit);
    }

    trunk_aggid_size = num_trunks * sizeof(_bcm_th_trunk_aggid_info_t);
    for (i = 0; i < num_pipes; i++) {
