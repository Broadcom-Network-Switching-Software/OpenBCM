...
STATIC void
speed_scan_link_cb(int unit, bcm_port_t port, bcm_cpri_interrupt_data_t *data, void *userdata)
{
    spd_scan_control_t *spd_scan  = speed_scan_control[unit];

    /*
     * If there is change in link status, speed scan will start
     * processing the link change immediately.
     */

    if (SPD_SCAN_F_SEMA_GET(spd_scan->flags)) {
        sal_sem_give(spd_scan->spd_scan_sema);
    }
    SPD_SCAN_F_LINK_SET(spd_scan->flags);
}

STATIC int
speed_scan_link_interrupt_register(int unit)
{
    static uint32 idata = 0;

    /* if interrupt mode */
    BCM_IF_ERROR_RETURN(
        bcm_esw_cpri_port_interrupt_callback_register(unit,
                                                  BCM_CPRI_INTR_RX_PCS_LINK_STATUS_LH,
                                                  speed_scan_link_cb, &idata));
    return BCM_E_NONE;
}

STATIC int
speed_scan_port_link_interrupt_enable(int unit, bcm_port_t port, int enable)
{
    BCM_IF_ERROR_RETURN(
        bcm_esw_cpri_port_interrupt_enable_set(unit, port,
                                           BCM_CPRI_INTR_RX_PCS_LINK_STATUS_LH,
                                           0, enable));
...
_speed_scan_speed_set(int unit, bcm_port_t port, bcm_cpri_port_speed_t speed_id)
{
    ...
    BCM_IF_ERROR_RETURN
        (speed_scan_port_link_interrupt_enable(unit, port, 0));
    ...
    BCM_IF_ERROR_RETURN
        (speed_scan_port_link_interrupt_enable(unit, port, 1));
...
speed_scan_thread (int unit)
{
    ...
    speed_scan_link_interrupt_register(0);

    while (SPD_SCAN_F_ENABLE_GET(spd_scan->flags)) {
        SPD_SCAN_F_LINK_CLR(spd_scan->flags);
        BCM_PBMP_ITER(spd_scan->pbmp_active, port) {
            /*
             * Linke changed detected.  Get port link status
             * and process immediately.
             */
            if (SPD_SCAN_F_LINK_GET(spd_scan->flags)) {
               LOG_DEBUG(BSL_LS_BCM_CPRI,
                         (BSL_META("%u: Link Status Chagne Detected.\n"), sal_time_usecs()));
                break;
            }
        ...
        /*
         *  If there is link status change, skip the waiting interval and 
         *  process the link status, otherwise wait here for 
         *  wait interval.
         */
        if (!SPD_SCAN_F_LINK_GET(spd_scan->flags)) {
            SPD_SCAN_F_SEMA_SET(spd_scan->flags);
            interval = spd_scan->interval_us;
            sal_sem_take(spd_scan->spd_scan_sema, interval);
            SPD_SCAN_F_SEMA_CLR(spd_scan->flags);
        }
