                            ...
                            uint32 kbits_sec, uint32 kbits_burst, uint32 mode)
{
    ...
    if (SOC_IS_GREYHOUND(unit) || SOC_IS_HURRICANE3(unit) ||
        SOC_IS_GREYHOUND2(unit) || SOC_IS_HURRICANE(unit) ||
...
_bcm_fb2_port_tpid_get(int unit, bcm_port_t port, uint16 *tpid)
{
    ...
    if (SOC_IS_TOMAHAWK3(unit)) {
        /* For TH3, default the tpid to 0, fetch tpid_enable_get. If it's
         * disabled, skip fetching tpid and return 0 to caller. All other
         * devices will fetch tpid as previously done
         */
        *tpid = 0;
        BCM_IF_ERROR_RETURN(
            _bcm_fb2_port_tpid_enable_get(unit, port, &tpid_enable));
        BCM_IF_ERROR_RETURN(
            _bcm_fb2_egr_vlan_control_field_get(
            unit, port, OUTER_TPID_INDEXf, (uint32*)&index));

        /* If the tpid_index is enabled, fetch it */
...
_bcm_fb2_port_tpid_delete(int unit, bcm_port_t port, uint16 tpid)
{
    ...
    if (index == outer_tpid_idx) {
        ...
        /* Check the soc property that allows TPID to be disabled. By default
         * the default TPID is added back to the port */
        if (soc_property_port_get(unit, port,
                    spn_PORT_ALLOW_TPID_DISABLE, 0) == 0) {
