...
STATIC uint16 *_xgs3_mod_port_to_tid_map[BCM_MAX_NUM_UNITS];
                                 ...
                                 bcm_port_t port, bcm_trunk_t trunk_id)  
{
    ...
    if (((soc_feature(unit, soc_feature_fastlag)) && (trunk_id >= 0xffff)) ||
        ((!soc_feature(unit, soc_feature_fastlag)) && (trunk_id >= 0xff))) {
...
_bcm_xgs3_trunk_mod_port_map_init(int unit) 
{
    ...
    mem_size =  _BCM_XGS3_TRUNK_MOD_PORT_MAP_IDX_COUNT(unit) * sizeof(uint16);
...
_bcm_xgs3_trunk_hwfailover_init(int unit)
{
    ...
    if (_xgs3_trunk_hwfail[unit]->hw_tinfo == NULL) {
        _xgs3_trunk_hwfail[unit]->hw_tinfo =
            sal_alloc(sizeof(_xgs3_hw_tinfo_t) * trunk_num_groups,
                    "_xgs3_trunk_hwfail_hw_tinfo");
        if (_xgs3_trunk_hwfail[unit]->hw_tinfo == NULL) {
            if (_xgs3_trunk_hwfail[unit] != NULL) {
    ...
    sal_memset(_xgs3_trunk_hwfail[unit]->hw_tinfo, 0,
               sizeof(_xgs3_hw_tinfo_t) * trunk_num_groups);
...
_bcm_xgs3_trunk_hwfailover_detach(int unit)
{
    if (_xgs3_trunk_hwfail[unit] != NULL) {
        if (_xgs3_trunk_hwfail[unit]->hw_tinfo != NULL) {
            sal_free(_xgs3_trunk_hwfail[unit]->hw_tinfo);
            _xgs3_trunk_hwfail[unit]->hw_tinfo = NULL;
        }
                               ...
                               bcm_port_t *ftp, bcm_module_t *ftm)
{
    ...
    int fp_trunk_num_groups = _BCM_FP_TRUNK_NUM_MAX;

#ifdef BCM_TOMAHAWK_SUPPORT
    if (soc_feature(unit, soc_feature_fastlag)) {
        fp_trunk_num_groups = soc_mem_index_count(unit, FAST_TRUNK_GROUPm);
    }
#endif
    ...
    tid_ix = tid + (hg_trunk ? fp_trunk_num_groups : 0);
...
_bcm_xgs3_trunk_swfailover_init(int unit)
{
    ...
    if (_xgs3_trunk_swfail[unit]->tinfo == NULL) {
        _xgs3_trunk_swfail[unit]->tinfo =
            sal_alloc(sizeof(_xgs3_tinfo_t) * trunk_num_groups,
                    "_xgs3_trunk_swfail_tinfo");
        if (_xgs3_trunk_swfail[unit]->tinfo == NULL) {
            if (_xgs3_trunk_swfail[unit] != NULL) {
    ...
    sal_memset(_xgs3_trunk_swfail[unit]->tinfo, 0,
               sizeof(_xgs3_tinfo_t) * trunk_num_groups);
...
_bcm_xgs3_trunk_swfailover_detach(int unit)
{
    if (_xgs3_trunk_swfail[unit] != NULL) {
        if (_xgs3_trunk_swfail[unit]->tinfo != NULL) {
            sal_free(_xgs3_trunk_swfail[unit]->tinfo);
            _xgs3_trunk_swfail[unit]->tinfo = NULL;
        }
        sal_free(_xgs3_trunk_swfail[unit]);
        _xgs3_trunk_swfail[unit] = NULL;
    }
                                      ...
                                      bcm_pbmp_t ports)
{
    ...
    swf = _xgs3_trunk_swfail[unit];
    swft = &swf->tinfo[hgtid + fp_trunk_num_groups];
                                          ...
                                          _xgs3_trunk_swfail_t *swf)
{
    ...
    if (SOC_IS_FB_FX_HX(unit) || SOC_IS_RAVEN(unit)) {
        ...
        for (k = j = 0; k < 2; k++) {
             swft = &swf->tinfo[fp_trunk_num_groups + k];
             nmp = swft->num_ports;
                                   ...
                                   bcm_pbmp_t ports_status)
{
    ...
    soc_mem_t            trunk_mem = TRUNK_GROUPm;
    int fp_trunk_num_groups = _BCM_FP_TRUNK_NUM_MAX;
    int hg_trunk_num_groups = _BCM_FABRIC_TRUNK_NUM_MAX;
    ...
    if (soc_feature(unit, soc_feature_fastlag)) {
        fp_trunk_num_groups = soc_mem_index_count(unit, FAST_TRUNK_GROUPm);
        hg_trunk_num_groups = soc_mem_index_count(unit, HG_TRUNK_GROUPm);
    }
#endif
    ...
    PBMP_ITER(ports_active, port) {
        ...
        if (tid >= 1 && tid < fp_trunk_num_groups + 1) {
            uint32               tg_entry[SOC_MAX_MEM_WORDS];
            uint16               modport;
            int                  i, j, nmp;
            int                  uc_nmp;
                          ...
                          trunk_private_t *trunk_info)
{
    ...
    for (tid = min_tid; tid <= max_tid; tid++, trunk_info++) {
        ...
        for (i = 0; i < num_ports; i++) {
            MEMBER_INFO(unit, (hgtid + fp_trunk_num_groups)).modport[i] = tp[i];
        }
        MEMBER_INFO(unit, (hgtid + fp_trunk_num_groups)).num_ports = num_ports;
