...
int
_bcm_coe_subtag_subport_num_local_port_get(int unit,
                                 int subport_num,
                                 bcm_port_t *port)
{
    int subport_group = -1;
    ...
    if (subport_num > si->max_subport_coe_ports) {
        return BCM_E_PARAM;
    }
    ...
    if (port) {
        ...
        /* Extract the associated group */
        subport_group =
            _BCM_SUBPORT_COE_SUBTAG_PORT_INFO_GROUP_GET(unit, subport_num);
