...
typedef struct _bcm_xgs5_ecn_bookkeeping_s {
    ...
    SHR_BITDCL  *egr_tunnel_ecn_encap_bitmap;  /* EGR_TUNNEL_ECN_ENCAP map usage bitmap */
    uint32      *egr_tunnel_ecn_encap_hw_idx;  /* Actual profile number used */
...
/*
 * Egress PKT_ECN->tunnel ECN + INT_CN usage bitmap operations
 */
#define _BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_GET(_u_, _map_) \
        SHR_BITGET(ECN_INFO(_u_)->egr_tunnel_ecn_encap_bitmap, (_map_))
#define _BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_SET(_u_, _map_) \
        SHR_BITSET(ECN_INFO((_u_))->egr_tunnel_ecn_encap_bitmap, (_map_))
#define _BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_CLR(_u_, _map_) \
        SHR_BITCLR(ECN_INFO((_u_))->egr_tunnel_ecn_encap_bitmap, (_map_))
#define _BCM_ECN_MAP_LEN_EGR_TUNNEL_ECN_ENCAP(_u_)                \
     (soc_mem_index_count((_u_), EGR_TUNNEL_ECN_ENCAPm) / \
	(_BCM_TD3_TUNNEL_ECN_ENCAP_PROFILE_TABLE_NUM * _BCM_ECN_ENCAP_ENTRIES))
...
#ifdef BCM_TRIDENT3_SUPPORT
STATIC int
_bcm_td3_pkt_ecn_to_tnl_ecn_get(int unit, int ecn_map_id, bcm_ecn_map_t *ecn_map)
{
    int num_ecn_map;
    soc_mem_t mem;
    uint32  index;
    int     table_num;
    uint32 flag_mask;
    void    *entries[_BCM_TD3_TUNNEL_ECN_ENCAP_PROFILE_TABLE_NUM];
    int     rv = BCM_E_NONE;
    int     offset;
    egr_tunnel_ecn_encap_entry_t table[_BCM_ECN_ENCAP_ENTRIES];
    egr_tunnel_ecn_encap_entry_t table1[_BCM_ECN_ENCAP_ENTRIES];
    egr_tunnel_ecn_encap_entry_t table2[_BCM_ECN_ENCAP_ENTRIES];
    egr_tunnel_ecn_encap_entry_t table3[_BCM_ECN_ENCAP_ENTRIES];
    egr_tunnel_ecn_encap_entry_t *entry;
    ...
    num_ecn_map = soc_mem_index_count(unit, mem) /
                  _BCM_ECN_ENCAP_ENTRIES;
    if ((ecn_map == NULL) || (table_num >= num_ecn_map)) {
        return BCM_E_PARAM;
    }

    if (!_BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_GET(unit, table_num)) {
        return BCM_E_NOT_FOUND;
    }

    /* Get the base index for this ECN map */
    index = ECN_INFO(unit)->egr_tunnel_ecn_encap_hw_idx[table_num] *
            _BCM_ECN_ENCAP_ENTRIES;
    sal_memset(table, 0, sizeof(table));
    sal_memset(table1, 0, sizeof(table1));
    sal_memset(table2, 0, sizeof(table2));
    sal_memset(table3, 0, sizeof(table3));
    entries[0] = table;
    entries[1] = table1;
    entries[2] = table2;
    entries[3] = table3;
    rv = _bcm_ecn_pkt_to_tnl_ecn_entry_get(unit, index,
                                              _BCM_ECN_ENCAP_ENTRIES,
                                              entries);
    BCM_IF_ERROR_RETURN(rv);
    offset = ecn_map->ecn;
    flag_mask = BCM_ECN_TRAFFIC_ACTION_RESPONSIVE |
                     BCM_ECN_TRAFFIC_ACTION_FORWARD_HEADER_ECN_PRESENT;
    if ((ecn_map->action_flags & flag_mask) == flag_mask) {
        entry  = &table3[offset];
    } else if ((ecn_map->action_flags & flag_mask) == BCM_ECN_TRAFFIC_ACTION_RESPONSIVE) {
        entry  = &table2[offset];
    } else if ((ecn_map->action_flags & flag_mask) ==
                   BCM_ECN_TRAFFIC_ACTION_FORWARD_HEADER_ECN_PRESENT) {
        entry  = &table1[offset];
    } else {
        entry  = &table[offset];
    }
    ecn_map->new_ecn = soc_mem_field32_get(unit, mem, entry, OUTER_ECNf);
    ecn_map->new_int_cn = soc_mem_field32_get(unit, mem, entry, INT_CNf);

    return BCM_E_NONE;
}

STATIC int
_bcm_td3_pkt_ecn_to_tnl_ecn_destroy(int unit, int ecn_map_id)
{
    int num_ecn_map, table_num;
    int base_idx, rv = BCM_E_NONE;

    table_num = ecn_map_id & _BCM_XGS5_ECN_MAP_NUM_MASK;
    num_ecn_map = soc_mem_index_count(unit, EGR_TUNNEL_ECN_ENCAPm) /
                  _BCM_ECN_ENCAP_ENTRIES;
    num_ecn_map /= _BCM_TD3_TUNNEL_ECN_ENCAP_PROFILE_TABLE_NUM;
    ...
    base_idx = ECN_INFO(unit)->egr_tunnel_ecn_encap_hw_idx[table_num] * _BCM_ECN_ENCAP_ENTRIES;
    BCM_IF_ERROR_RETURN(_bcm_ecn_pkt_to_tnl_ecn_entry_delete(unit, base_idx));
    _BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_CLR(unit, table_num);
    return rv;
}

STATIC int
_bcm_td3_pkt_ecn_to_tnl_ecn_update(int unit,int ecn_map_id, bcm_ecn_map_t *ecn_map)
{
    void    *entries[_BCM_TD3_TUNNEL_ECN_ENCAP_PROFILE_TABLE_NUM];
    int     rv = BCM_E_NONE;
    uint32  index;
    ...
    rv = _bcm_ecn_pkt_to_tnl_ecn_entry_get(unit, index,
                                              ...
                                              entries);
    if (BCM_SUCCESS(rv)) {
        rv = _bcm_ecn_pkt_to_tnl_ecn_entry_delete(unit, (int)index);
        BCM_IF_ERROR_RETURN(rv);
    }
    offset = ecn_map->ecn;
    flag_mask = BCM_ECN_TRAFFIC_ACTION_RESPONSIVE |
    ...
    soc_EGR_TUNNEL_ECN_ENCAPm_field32_set(unit, entry,
                OUTER_ECNf, ecn_map->new_ecn);
    soc_EGR_TUNNEL_ECN_ENCAPm_field32_set(unit, entry,
                INT_CNf, ecn_map->new_int_cn);

    rv = _bcm_ecn_pkt_to_tnl_ecn_entry_add(unit, entries,
                    _BCM_ECN_ENCAP_ENTRIES, &index);
    BCM_IF_ERROR_RETURN(rv);

    ECN_INFO(unit)->egr_tunnel_ecn_encap_hw_idx[table_num] =
            index / _BCM_ECN_ENCAP_ENTRIES;
    return rv;
}

STATIC int
_bcm_td3_egr_tunnel_ecn_encap_create(int unit, uint32 flags, int *ecn_map_id)
{
    int table_num;
    int num_ecn_map;
    egr_tunnel_ecn_encap_entry_t table[_BCM_ECN_ENCAP_ENTRIES];
    ...
    if (flags & BCM_ECN_MAP_WITH_ID) {
        ...
        table_num = (*ecn_map_id & _BCM_XGS5_ECN_MAP_NUM_MASK);
        ecn_map_type = (*ecn_map_id & _BCM_XGS5_ECN_MAP_TYPE_MASK);
        if (ecn_map_type != _BCM_XGS5_ECN_MAP_TYPE_TUNNEL_INIT) {
            return BCM_E_PARAM;
        }
        ...
        _BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_SET(unit, table_num);
    } else {
        for (table_num = 0; table_num < num_ecn_map; table_num++) {
            if (!_BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_GET(unit, table_num)) {
                _BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_SET(unit, table_num);
                *ecn_map_id = (table_num | _BCM_XGS5_ECN_MAP_TYPE_TUNNEL_INIT);
                break;
            }
        }
        if (table_num == num_ecn_map) {
    ...
    rv = _bcm_ecn_pkt_to_tnl_ecn_entry_add(unit, entries,
                                              ...
                                              &index);
    if (BCM_SUCCESS(rv)) {
        ecn_info->egr_tunnel_ecn_encap_hw_idx[table_num] =
            index / _BCM_ECN_ENCAP_ENTRIES;
    }
    return rv;
}
...
STATIC int
_bcm_td3_pkt_ecn_to_tnl_ecn_set(int unit, int ecn_map_id, bcm_ecn_map_t *ecn_map)
{
    int table_num;
    /* Check validity of exp_map_id */
    if (ecn_map_id < 0) {
    ...
    if ((ecn_map->ecn > 3) || (ecn_map->new_ecn > 3) ||
        (ecn_map->new_int_cn > 3)) {
        return BCM_E_PARAM;
    }
    return _bcm_td3_pkt_ecn_to_tnl_ecn_update(unit, ecn_map_id, ecn_map);
}

STATIC int
_bcm_xgs5_tunnel_init_ecn_vp_map_get(int unit, int vp, int *ecn_map_id)
{
    ...
    SOC_IF_ERROR_RETURN(soc_mem_read(unit, mem, MEM_BLOCK_ANY, vp, &dvp_entry));
    ...
    if (soc_feature(unit, soc_feature_flex_flow)) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, mem, DATA_TYPEf)) {
            data_type = soc_mem_field32_get(unit, mem, &dvp_entry, DATA_TYPEf);
        }
        ...
        if (SOC_SUCCESS(rv)) {
            fixed_view = 0;
            if (SOC_MEM_FIELD_VALID(unit, mem_view_id, ECN_ACTION_SETf)) {
                *ecn_map_id = soc_mem_field32_get(unit, mem_view_id, &dvp_entry,
                       ECN_ACTION_SETf);
                rv = BCM_E_NONE;
            }
            else {
    ...
    if (fixed_view) {
        if (SOC_MEM_FIELD_VALID(unit, mem, VXLAN__ECN_ENCAP_MAPPING_PTRf)) {
            *ecn_map_id = soc_mem_field32_get(unit, mem, &dvp_entry,
                                VXLAN__ECN_ENCAP_MAPPING_PTRf);
            rv = BCM_E_NONE;
        }
    }
    return rv;
...
STATIC int
_bcm_xgs5_tunnel_init_ecn_vp_map_set(int unit, int vp, int ecn_map_id)
{
    ...
    if (soc_feature(unit, soc_feature_flex_flow)) {
        ...
        if (SOC_SUCCESS(rv)) {
            if (SOC_MEM_FIELD_VALID(unit,mem_view_id,ECN_ACTION_SETf)) {
                soc_mem_field32_set(unit, mem_view_id, &dvp_entry,
                       ECN_ACTION_SETf, ecn_map_id);
            }
        } else if (rv != SOC_E_NOT_FOUND) {
            return rv;
        }
    ...
    if (rv == SOC_E_NOT_FOUND) {
        if (SOC_MEM_FIELD_VALID(unit, mem, VXLAN__ECN_ENCAP_MAPPING_PTRf)) {
            soc_mem_field32_set(unit, mem, &dvp_entry,
                           VXLAN__ECN_ENCAP_MAPPING_PTRf, ecn_map_id);
        }
    }
    SOC_IF_ERROR_RETURN(soc_mem_write(unit, mem, MEM_BLOCK_ALL, vp,
                                      &dvp_entry));
    rv = BCM_E_NONE;
    return rv;
}
...
bcmi_xgs5_ecn_map_create(int unit, uint32 flags, int *ecn_map_id)
{
    ...
    if (flags & BCM_ECN_MAP_INGRESS) {
        ...
        if (flags & BCM_ECN_MAP_TUNNEL_INIT) {
            rv = _bcm_td3_egr_tunnel_ecn_encap_create(unit, flags, ecn_map_id);
        }
    }
#endif /* BCM_TRIDENT3_SUPPORT */
    ECN_UNLOCK(unit);
...
#ifdef BCM_TRIDENT3_SUPPORT
    else if ((ecn_map_id & _BCM_XGS5_ECN_MAP_TYPE_MASK) ==
               _BCM_XGS5_ECN_MAP_TYPE_TUNNEL_INIT) {
        rv = _bcm_td3_pkt_ecn_to_tnl_ecn_destroy(unit, ecn_map_id);
    }
#endif /* BCM_TRIDENT3_SUPPORT */
    ECN_UNLOCK(unit);
    return rv;
...
bcmi_xgs5_ecn_map_set(int unit, int ecn_map_id, bcm_ecn_map_t *ecn_map) 
{
    ...
    if ((ecn_map_id & _BCM_XGS5_ECN_MAP_TYPE_MASK) == 
        ...
        rv = bcmi_xgs5_tunnel_term_ecn_map_set(unit, ecn_map_id, ecn_map);
               ...
               _BCM_XGS5_ECN_MAP_TYPE_TUNNEL_INIT) {
        rv = _bcm_td3_pkt_ecn_to_tnl_ecn_set(unit, ecn_map_id, ecn_map);
    }
#endif /* BCM_TRIDENT3_SUPPORT */
    ECN_UNLOCK(unit);
    return rv;
...
bcmi_xgs5_ecn_map_get(int unit, int ecn_map_id, bcm_ecn_map_t *ecn_map) 
{
    ...
    if ((ecn_map_id & _BCM_XGS5_ECN_MAP_TYPE_MASK) == 
        ...
        rv = bcmi_xgs5_tunnel_term_ecn_map_get(unit, ecn_map_id, ecn_map);
               ...
               _BCM_XGS5_ECN_MAP_TYPE_TUNNEL_INIT) {
        rv = _bcm_td3_pkt_ecn_to_tnl_ecn_get(unit, ecn_map_id, ecn_map);
    }
#endif /* BCM_TRIDENT3_SUPPORT */
    ECN_UNLOCK(unit);
    return rv;    
...
bcmi_xgs5_ecn_port_map_set(int unit, bcm_gport_t port, bcm_ecn_port_map_t *ecn_map)
{
    ...
    if (ecn_map->flags & BCM_ECN_INGRESS_PORT_TUNNEL_TERM_MAP) {
...
#ifdef BCM_TRIDENT3_SUPPORT
    else if (ecn_map->flags & BCM_ECN_EGRESS_PORT_TUNNEL_INIT_MAP) {
        int ecn_map_index;
        int ecn_map_type;
        int ecn_map_num;
        int ecn_map_hw_idx;
        ...
        ecn_map_index = ecn_map->ecn_map_id &
                        _BCM_XGS5_ECN_MAP_NUM_MASK;
        ecn_map_num =
            soc_mem_index_count(unit, EGR_TUNNEL_ECN_ENCAPm) /
            _BCM_ECN_ENCAP_ENTRIES;
        ecn_map_num = ecn_map_num / _BCM_TD3_TUNNEL_ECN_ENCAP_PROFILE_TABLE_NUM;
        if (ecn_map_type != _BCM_XGS5_ECN_MAP_TYPE_TUNNEL_INIT) {
            goto clean_up;
        }
        if (ecn_map_index >= ecn_map_num) {
        ...
        rv =  _bcm_xgs5_tunnel_init_ecn_vp_map_set(unit, vp, ecn_map_hw_idx);
        if (BCM_FAILURE(rv)) {
            goto clean_up;
        }
    }
...
bcmi_xgs5_ecn_port_map_get(int unit, bcm_gport_t port, bcm_ecn_port_map_t *ecn_map)
{
    ...
    if (ecn_map->flags & BCM_ECN_INGRESS_PORT_TUNNEL_TERM_MAP) {
        ...
        rv =  _bcm_xgs5_tunnel_init_ecn_vp_map_get(unit, vp, &ecn_map_hw_idx);
        if (BCM_FAILURE(rv)) {
            goto clean_up;
        }
        rv = bcmi_ecn_map_hw_idx2id(
                unit, ecn_map_hw_idx, _BCM_XGS5_ECN_MAP_TYPE_TUNNEL_INIT,
                &ecn_map_index);
        if (BCM_FAILURE(rv)) {
            goto clean_up;
        }
        ecn_map->ecn_map_id = ecn_map_index;
        ecn_map->flags |= BCM_ECN_EGRESS_PORT_TUNNEL_INIT_MAP;
    }
#endif /* BCM_TRIDENT3_SUPPORT */
clean_up:    
    ECN_UNLOCK(unit);
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_tunnel_init_ecn)) {
        int i;
        int base_idx;

        if (ecn_info->egr_tunnel_ecn_encap_bitmap) {
            /* clear the profiles */
            for (i = 0; i < _BCM_ECN_MAP_LEN_EGR_TUNNEL_ECN_ENCAP(unit); i++) {
                if (_BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_GET(unit, i)) {
                    base_idx = ecn_info->egr_tunnel_ecn_encap_hw_idx[i] *
                                   _BCM_ECN_ENCAP_ENTRIES;
                    (void)_bcm_ecn_pkt_to_tnl_ecn_entry_delete(unit, base_idx);
                }
            }
            sal_free(ecn_info->egr_tunnel_ecn_encap_bitmap);
            ecn_info->egr_tunnel_ecn_encap_bitmap = NULL;
        }
        if (ecn_info->egr_tunnel_ecn_encap_hw_idx) {
            sal_free(ecn_info->egr_tunnel_ecn_encap_hw_idx);
            ecn_info->egr_tunnel_ecn_encap_hw_idx = NULL;
        }
    }
#endif /* BCM_TRIDENT3_SUPPORT */
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_tunnel_init_ecn)) {
        int num_egr_tunnel_ecn_encap = 0;

        num_egr_tunnel_ecn_encap = _BCM_ECN_MAP_LEN_EGR_TUNNEL_ECN_ENCAP(unit);
        ecn_info->egr_tunnel_ecn_encap_bitmap =
                sal_alloc(SHR_BITALLOCSIZE(num_egr_tunnel_ecn_encap),
                          "egr_tunnel_ecn_encap_bitmap");
        if (ecn_info->egr_tunnel_ecn_encap_bitmap == NULL) {
            bcmi_xgs5_ecn_free_resource(unit);
            return BCM_E_MEMORY;
        }

        ecn_info->egr_tunnel_ecn_encap_hw_idx =
            sal_alloc(sizeof(uint32) * num_egr_tunnel_ecn_encap,
                      "egr_tunnel_ecn_encap_hw_idx");
        if (ecn_info->egr_tunnel_ecn_encap_hw_idx == NULL) {
            bcmi_xgs5_ecn_free_resource(unit);
            return BCM_E_MEMORY;
        }
        sal_memset(ecn_info->egr_tunnel_ecn_encap_bitmap, 0,
                   SHR_BITALLOCSIZE(num_egr_tunnel_ecn_encap));
        sal_memset(ecn_info->egr_tunnel_ecn_encap_hw_idx, 0,
                   num_egr_tunnel_ecn_encap * sizeof(uint32));
    }
#endif /* BCM_TRIDENT3_SUPPORT */
...
#if defined(BCM_TRIDENT3_SUPPORT)
int
bcmi_td3_ecn_scache_v13_size_get(int unit, uint32 *scache_size)
{
    if (scache_size == NULL) {
        return BCM_E_PARAM;
    }
    *scache_size = 0;
    *scache_size += SHR_BITALLOCSIZE(_BCM_ECN_MAP_LEN_EGR_TUNNEL_ECN_ENCAP(unit));
    *scache_size += _BCM_ECN_MAP_LEN_EGR_TUNNEL_ECN_ENCAP(unit) * sizeof(uint32);
    return BCM_E_NONE;
}
#endif /* BCM_TRIDENT3_SUPPORT */
...
bcmi_ecn_map_id2hw_idx (int unit,int ecn_map_id,int *hw_idx)
{
    ...
    if ((ecn_map_id & _BCM_XGS5_ECN_MAP_TYPE_MASK) == 
        ...
        *hw_idx = ECN_INFO(unit)->egr_tunnel_ecn_encap_hw_idx[table_num];
        rv = BCM_E_NONE;
    }
    ECN_UNLOCK(unit);
    return rv;
...
#if defined(BCM_TRIDENT3_SUPPORT)
    case _BCM_XGS5_ECN_MAP_TYPE_TUNNEL_INIT:
        num_map = _BCM_ECN_MAP_LEN_EGR_TUNNEL_ECN_ENCAP(unit);
        for (id = 0; id < num_map; id++) {
            if (_BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_GET(unit, id)) {
                if (ECN_INFO(unit)->egr_tunnel_ecn_encap_hw_idx[id] == hw_idx) {
                    *ecn_map_id = type | id;
                    rv = BCM_E_NONE;
                    break;
                }
...
#if defined(BCM_TRIDENT3_SUPPORT)
int
bcmi_td3_ecn_v13_sync(int unit, uint8 **scache_ptr)
{
    int num_egr_tunnel_ecn_encap = 0;
    _bcm_xgs5_ecn_bookkeeping_t *ecn_info = ECN_INFO(unit);
    ...
    if (soc_feature(unit, soc_feature_tunnel_init_ecn)) {
        num_egr_tunnel_ecn_encap = _BCM_ECN_MAP_LEN_EGR_TUNNEL_ECN_ENCAP(unit);
        SHR_BITCOPY_RANGE((SHR_BITDCL *)(*scache_ptr), 0,
                          ecn_info->egr_tunnel_ecn_encap_bitmap, 0,
                          num_egr_tunnel_ecn_encap);
        *scache_ptr += SHR_BITALLOCSIZE(num_egr_tunnel_ecn_encap);
        sal_memcpy(*scache_ptr, ecn_info->egr_tunnel_ecn_encap_hw_idx,
                   (num_egr_tunnel_ecn_encap * sizeof(uint32)));
        *scache_ptr += (num_egr_tunnel_ecn_encap * sizeof(uint32));
    }
    return BCM_E_NONE;
}
#endif /* BCM_TRIDENT3_SUPPORT */
...
#if defined(BCM_TRIDENT3_SUPPORT)
int
bcmi_td3_ecn_v13_reinit(int unit, uint8 **scache_ptr)
{
    int                 i = 0;
    int                 rv;
    int num_egr_tunnel_ecn_encap = 0;
    ...
    if (soc_feature(unit, soc_feature_tunnel_init_ecn)) {
        num_egr_tunnel_ecn_encap = _BCM_ECN_MAP_LEN_EGR_TUNNEL_ECN_ENCAP(unit);
        SHR_BITCOPY_RANGE(ecn_info->egr_tunnel_ecn_encap_bitmap, 0,
                      (SHR_BITDCL *)(*scache_ptr), 0, num_egr_tunnel_ecn_encap);
        *scache_ptr += SHR_BITALLOCSIZE(num_egr_tunnel_ecn_encap);
        sal_memcpy(ecn_info->egr_tunnel_ecn_encap_hw_idx, *scache_ptr,
                   (num_egr_tunnel_ecn_encap * sizeof(uint32)));

        *scache_ptr += (num_egr_tunnel_ecn_encap * sizeof(uint32));

        for (i = 0; i < num_egr_tunnel_ecn_encap; i++) {
            if (_BCM_EGR_PKT_ECN_TO_TNL_ECN_USED_GET(unit, i)) {
                rv = _bcm_ecn_pkt_to_tnl_ecn_entry_reference(unit,
                       ((ecn_info->egr_tunnel_ecn_encap_hw_idx[i]) *
                               _BCM_ECN_ENCAP_ENTRIES),
                       _BCM_ECN_ENCAP_ENTRIES);
                if (BCM_FAILURE(rv)) {
                    return rv;
