                       ...
                       shr_ft_msg_ctrl_collector_create_t *collector_create_msg)
{
    ...
    collector_create_msg->num_records = export_profile_info.num_records;
    collector_create_msg->domain_id = collector_info.ipfix.observation_domain_id;

    if (export_profile_info.flags & BCM_COLLECTOR_EXPORT_PROFILE_FLAGS_USE_NUM_RECORDS) {
        collector_create_msg->flags |= SHR_FT_MSG_COLLECTOR_FLAGS_USE_NUM_RECORDS;
    }
                                           ...
                                           bcm_flowtracker_flow_data_t *flow_data)
{
    ...
    sal_memcpy(send_msg.src_ipv6_addr, flow_key->src_ip.addr.ipv6_addr,
        sizeof(send_msg.src_ipv6_addr));
    sal_memcpy(send_msg.dst_ipv6_addr, flow_key->dst_ip.addr.ipv6_addr,
        sizeof(send_msg.dst_ipv6_addr));
    sal_memcpy(send_msg.src_mac_addr, flow_key->src_mac,
                sizeof(send_msg.src_mac_addr));
    sal_memcpy(send_msg.dst_mac_addr, flow_key->dst_mac,
                sizeof(send_msg.dst_mac_addr));
    send_msg.outer_vlan        = flow_key->outer_vlan_tag;
    send_msg.ether_type        = flow_key->ether_type;
    buffer_req = ft_info->dma_buffer;
...
void _bcm_xgs5_flowtracker_dump_stats_export(int unit)
{
    ...
    LOG_CLI((BSL_META("Export packet fails   : %u \n"), msg.num_pkt_export_fails));
    LOG_CLI((BSL_META("\n")));
...
/*
 * Function:
 *     _bcm_xgs5_qcm_num_tm_block_get
 * Purpose:
 ...
 *     unit           - (IN) BCM device number
 * Returns:
 *     Number of Traffic manager blocks present.
 */
static int _bcm_xgs5_qcm_num_tm_block_get(int unit)
{
    if (SOC_IS_TOMAHAWK3(unit)) {
        return 2; /* Number of ITMs */
    } else if (SOC_IS_TOMAHAWK(unit)) {
        return 4; /* Number of XPEs */
    } else if (SOC_IS_TRIDENT3(unit)) {
        return 1; /* Number of XPEs */
    }
    return 0;
}
...
static int _bcm_xgs5_qcm_global_flow_view_size_get(int unit)
{
    ...
    max_tm_per_pipe = _bcm_xgs5_qcm_num_tm_block_get(unit);
        ...
        uint32 usr_mem_size, uint8 *usr_mem_addr)
{
    ...
    for (idx = 0; idx < host_buf_msg.num_flow_views; ++idx) {
        ...
        if ((usr_mem_size >= (host_buf_msg.num_samples * _bcm_xgs5_qcm_num_tm_block_get(unit))) &&
            (host_mem_size >= (SHR_QCM_MAX_NUM_SAMPLE * _bcm_xgs5_qcm_num_tm_block_get(unit)))) {
            sal_memcpy(usr_mem_addr, host_ptr, (host_buf_msg.num_samples * _bcm_xgs5_qcm_num_tm_block_get(unit)));
        } else {
            return BCM_E_NONE;
        }
        usr_mem_size -= (host_buf_msg.num_samples * _bcm_xgs5_qcm_num_tm_block_get(unit));
        host_mem_size -= (SHR_QCM_MAX_NUM_SAMPLE * _bcm_xgs5_qcm_num_tm_block_get(unit));
        usr_mem_addr += (host_buf_msg.num_samples * _bcm_xgs5_qcm_num_tm_block_get(unit));
        host_ptr += (SHR_QCM_MAX_NUM_SAMPLE * _bcm_xgs5_qcm_num_tm_block_get(unit));
                                              ...
                                              int view_id, uint32 flags, int *num_flows)
{
    ...
    if ((mem_size >= (host_buf_msg.num_samples * _bcm_xgs5_qcm_num_tm_block_get(unit))) &&
        (host_mem_size >= (SHR_QCM_MAX_NUM_SAMPLE * _bcm_xgs5_qcm_num_tm_block_get(unit)))) {
        sal_memcpy(mem_addr, host_ptr, (host_buf_msg.num_samples * _bcm_xgs5_qcm_num_tm_block_get(unit)));
    } else {
        return BCM_E_NONE;
    }
    mem_size -= (host_buf_msg.num_samples * _bcm_xgs5_qcm_num_tm_block_get(unit));
    host_mem_size -= (SHR_QCM_MAX_NUM_SAMPLE * _bcm_xgs5_qcm_num_tm_block_get(unit));
    mem_addr += (host_buf_msg.num_samples * _bcm_xgs5_qcm_num_tm_block_get(unit));
    host_ptr += (SHR_QCM_MAX_NUM_SAMPLE * _bcm_xgs5_qcm_num_tm_block_get(unit));
...
int _bcm_xgs5_ft_qcm_eapp_prio_map_change_notify(int unit)
{
    uint16 reply_len = 0;
    _bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    int uc = 0, status = 0;
    ...
    if ((rv == SOC_E_NONE) && (status != 0)) {
        ...
        BCM_IF_ERROR_RETURN(
            _bcm_xgs5_ft_msg_send_receive(unit,
                                          MOS_MSG_SUBCLASS_FT_QCM_UPDATE_PRI_QUE_MAP,
                                          0, 0,
                                          MOS_MSG_SUBCLASS_FT_QCM_UPDATE_PRI_QUE_MAP_REPLY,
                                          &reply_len,
                                          SHR_FT_MAX_UC_MSG_TIMEOUT));
    }
