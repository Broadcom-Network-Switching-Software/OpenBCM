                                 ...
                                 uint8 *encap_data)
{
    ...
    BCM_IF_ERROR_RETURN(
            (bcm_esw_vxlan_tunnel_initiator_get(unit, &tnl_info)));
    ...
    if ((endpoint_info.type == bcmBFDTunnelTypeVxlanIPv6InIPv4) ||
        ...
        ip_tos = ((tnl_info.dscp & 0x3F << 2) | (tnl_info.ecn & 0x3));
        ipv4.dscp = ip_tos;
        ipv4.ttl = tnl_info.ttl;
        ...
        ipv4.src = tnl_info.sip;
        ipv4.dst = tnl_info.dip;
        ptr_tmp = cur_ptr;
        cur_ptr = bcmi_xgs5_bfd_ipv4_header_pack(cur_ptr, &ipv4);
        ...
        sal_memcpy(ipv6.src, tnl_info.sip6, sizeof(BCM_IP6_ADDRLEN));
        sal_memcpy(ipv6.dst, tnl_info.dip6, sizeof(BCM_IP6_ADDRLEN));
        cur_ptr = bcmi_xgs5_bfd_ipv6_header_pack(cur_ptr, &ipv6);
    }
    ...
    ptr_tmp = cur_ptr;
    sal_memset(&udp, 0, sizeof(udp));
    udp.src = tnl_info.udp_src_port;
    udp.dst = tnl_info.udp_dst_port;
    udp.length = udp_length;
    cur_ptr = bcmi_xgs5_bfd_udp_header_pack(cur_ptr, &udp);

    udp.sum = _shr_ip_chksum(SHR_BFD_UDP_HEADER_LENGTH, ptr_tmp);
    cur_ptr = bcmi_xgs5_bfd_udp_header_pack(ptr_tmp, &udp);
    ...
    if ((endpoint_info.type == bcmBFDTunnelTypeVxlanIPv4InIPv6) ||
        ...
        sal_memcpy(ipv6.src, endpoint_info.src_ip6_addr, sizeof(BCM_IP6_ADDRLEN));
        sal_memcpy(ipv6.dst, endpoint_info.dst_ip6_addr, sizeof(BCM_IP6_ADDRLEN));
        cur_ptr = bcmi_xgs5_bfd_ipv6_header_pack(cur_ptr, &ipv6);
    }
