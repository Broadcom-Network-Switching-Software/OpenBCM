...
_bcm_tr3_l3_esm_add(int unit, _bcm_l3_cfg_t *l3cfg, int nh_idx)
{
    ...
    if (tbl_free_entry <= soc_mem_index_max(unit, mem)) {
        ...
        /* If this a 'replace' action, we are done now */
        if (l3cfg->l3c_flags & BCM_L3_REPLACE) {
            return BCM_E_NONE;
        }
        ...
        bcm_l3_egress_t *l3_egress) 
{
    ...
    */
    ...
    if (SOC_IS_HURRICANE4(unit) &&
        ...
        egr_mac_da_profile_entry_t macda;
        void *entries[1] = { NULL };
        if (l3_egress->flags3 & BCM_L3_FLAGS3_L2_TAG_ONLY) {
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh, DATA_TYPEf, 2);
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                SD_TAG__HG_MC_DST_MODIDf, l3_egress->module);
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                SD_TAG__HG_MC_DST_PORT_NUMf, l3_egress->port);
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
