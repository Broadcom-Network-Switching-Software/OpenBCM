...
static int _bcm_th3_pend_learn_en[BCM_MAX_NUM_UNITS];
...
bcm_th3_l2_init(int unit)
{
    ...
    _bcm_th3_pend_learn_en[unit] = soc_property_get(unit,
    ...
    if (_bcm_th3_pend_learn_en[unit]) {
        (void)soc_l2_pending_learn_cb_register(unit,
                   ...
                   int key_only)
{
    ...
    /* TH3 does not support MESH bit in HW, but SW needs to save this
     * info in L2X entry to use it later on in AGING thread. Hence it
     * uses spare bit 79 in L2X entry which is marked as RESERVED in HW
     */
    if (l2addr->flags2 & BCM_L2_FLAGS2_MESH) {
        soc_L2Xm_field32_set(unit, l2x_entry, RESERVEDf, 1);
    }
...
_bcm_th3_l2_from_l2x(int unit, bcm_l2_addr_t *l2addr, l2x_entry_t *l2x_entry)
{
    ...
    /* TH3 does not support MESH bit. But SW needs to save this info,
     * so it uses bit 79 in L2X entry which is marked as RESERVED in HW.
     */
    if (SOC_MEM_FIELD_VALID(unit, L2Xm, RESERVEDf)) {
...
bcm_th3_l2_addr_delete(int unit, bcm_mac_t mac, bcm_vlan_t vid)
{
    ...
    if (_bcm_th3_pend_learn_en[unit]) {
        uint32 del_flags = 0x0;
        int rv1;

        sal_mutex_take(SOC_CONTROL(unit)->l2x_lrn_shadow_mutex,
                           sal_mutex_FOREVER);

        rv1 = soc_th3_lrn_shadow_entry_lookup(unit, mac, vid, FALSE, &flags);
        if (rv1 != SOC_E_NONE) {
            sal_mutex_give(SOC_CONTROL(unit)->l2x_lrn_shadow_mutex);
            return rv1;
        }

        if (flags == (SOC_TH3_L2_LRN_SHADOW_TBL_ENTRY_FOUND |
                      SOC_TH3_L2_LRN_SHADOW_TBL_PENDING)) {
            /* Application decided not to learn the entry, so we update the
             * shadow table and return
             */
            rv1 = soc_th3_lrn_shadow_entry_delete(unit, mac, vid, FALSE,
                      &del_flags);
            sal_mutex_give(SOC_CONTROL(unit)->l2x_lrn_shadow_mutex);
            return rv1;
        }
        ...
        /* For ports with pending learn not enabled, we proceed with deleting
         * the entry from the hardware L2 table
         */
    }
    ...
    static_ent = soc_L2Xm_field32_get(unit, &l2x_lookup, STATIC_BITf);
    ...
    SOC_L2_DEL_SYNC_UNLOCK(soc);
    if (static_ent && (rv >= 0)) {
        soc_th3_lrn_shadow_delete(unit, &l2x_entry, TRUE);
    }

    if ((_bcm_th3_pend_learn_en[unit]) &&
        (flags == (SOC_TH3_L2_LRN_SHADOW_TBL_ENTRY_FOUND |
                   SOC_TH3_L2_LRN_SHADOW_TBL_PENDING_ENTRY_IN_HW))) {
        uint32 del_flags = 0x0;

        (void)soc_th3_lrn_shadow_entry_delete(unit, mac, vid, FALSE,
            &del_flags);
    }
    sal_mutex_give(SOC_CONTROL(unit)->l2x_lrn_shadow_mutex);
    soc_mem_unlock(unit, L2Xm);
...
bcm_th3_l2_addr_add(int unit, bcm_l2_addr_t *l2addr)
{
    ...
    /*
     * If pending flag is passed by application and pending learn property is
     * not set, return error
     */
    if ((l2addr->flags & BCM_L2_PENDING) && !_bcm_th3_pend_learn_en[unit]) {
        return BCM_E_UNAVAIL;
    }

    if ((l2addr->flags & BCM_L2_PENDING) && _bcm_th3_pend_learn_en[unit]) {

        /* If there is an attempt to add a static, trunk or multicast entry
         * with a pending flag, return error
         */
        if ((l2addr->flags & BCM_L2_STATIC) ||
            (l2addr->flags & BCM_L2_MCAST)) {
            return BCM_E_CONFIG;
        ...
        if (BCM_GPORT_IS_SET(l2addr->port)) {
            ...
            BCM_IF_ERROR_RETURN(_bcm_th3_l2_gport_parse(unit, l2addr,
                &g_params));
        ...
        SOC_IF_ERROR_RETURN(soc_th3_lrn_shadow_entry_lookup(unit, l2addr->mac,
            l2addr->vid, TRUE, &entry_flags));

        if (!(entry_flags & SOC_TH3_L2_LRN_SHADOW_TBL_ENTRY_FOUND)) {

            /* If the entry is not found in the learn shadow table, the
             * application is attempting to add a L2 entry with pending bit
             * set; this is supported on legacy devices, not on TH3. So we
             * return unavailable error here
             */
            return BCM_E_UNAVAIL;
        }

        if ((entry_flags & SOC_TH3_L2_LRN_SHADOW_TBL_ENTRY_FOUND) &&
            (!(entry_flags & SOC_TH3_L2_LRN_SHADOW_TBL_PENDING))) {
            /* If pending flag is set, and for a existing(found) entry, if
             * pending bit is not set, it is not a valid case; return error
             */
            return BCM_E_UNAVAIL;
        }
    }

    /* Determines if this entry needs to go in to the learn shadow table */
    add_static_entry = (l2addr->flags & BCM_L2_STATIC) &&
                       !(l2addr->flags & BCM_L2_MCAST) &&
                       !(MAC_IS_MCAST(l2addr->mac));
    ...
    if ((l2addr->flags2 & BCM_L2_FLAGS2_MESH) && (BCM_E_NONE == rv) &&
        soc_mem_field32_get(unit, L2Xm, &l2x_lookup, STATIC_BITf)) {
        return BCM_E_EXISTS;
    }
    ...
    if (rv == BCM_E_NONE) {
        if (add_static_entry) {
            soc_th3_lrn_shadow_insert(unit, &l2x_entry, TRUE);
        }

        if ((l2addr->flags & BCM_L2_PENDING) && _bcm_th3_pend_learn_en[unit]) {
            soc_l2_pending_learn_info_t info;

            /* Update learn shadow table */

            /* Initialize 'info' structure */
            sal_memset(&info, 0x0, sizeof(info));
            sal_memcpy(info.mac, l2addr->mac, sizeof(info.mac));
            info.vlan = l2addr->vid;

            if (tgid != BCM_TRUNK_INVALID) {
                info.dst_type = 1;
                info.portnum_or_trunk_id = tgid;
            } else if (port != -1) {
                /* Note port, modid have been checked in _bcm_th3_l2_to_l2x
                 * above
                 */
                info.modid = modid;
                info.portnum_or_trunk_id = port;
                info.dst_type = 0;
            } else if (gport_id != -1) {
                info.modid = 0;
                info.portnum_or_trunk_id = gport_id;
                info.dst_type = 0;
            }

            /* Note, for pending entry, the SOC_TH3_L2_ENTRY_LEARN_PENDING flag
             * in the shadow db will be implcitly cleared in the call below.
             * The call below updates the existing learn shadow entry.
             */
            SOC_IF_ERROR_RETURN(soc_th3_lrn_shadow_pending_entry_set(unit,
                &info, TRUE, SOC_TH3_L2_LRN_SHADOW_TBL_SET_IN_HW));
        }
