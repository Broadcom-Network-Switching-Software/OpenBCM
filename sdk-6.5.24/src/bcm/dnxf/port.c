    ...
    int value)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fec_control_verify(unit, port, type));
    ...
    int is_init_sequence)
{
    ...
    max_link = dnxf_data_device.blocks.nof_links_in_rtp_get(unit) * nof_cores;
    /*
    ...
    BCM_PBMP_ITER(pbmp, port)
    {
        ...
        if (port >= max_link)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Fabric link %d is out of range\n", port);
        }
    ...
    pbmp_t * okay_pbmp)
{
    ...
    BCM_PBMP_ITER(pbmp, port)
    {
        SOC_INFO(unit).port_speed_max[port] = dnxf_data_port.general.speed_max_get(unit);
    }
    ...
    if (!is_init_sequence)
    {
        ...
        SHR_IF_ERR_EXIT(soc_counter_start(unit, flags, usec, PBMP_SFI_ALL(unit)));
        /*
    ...
    pbmp_t * detached)
{
    ...
    /*
     * Check if the ports beening detached are: 1. Whitin a valid range 2. Member of the SFI enabled PBMP
     */
    BCM_PBMP_ITER(pbmp, port)
    {
        DNXF_LINK_INPUT_CHECK(unit, port);
    ...
    int unit)
{
     ...
     */
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_serdes_power_disable, (unit, pbmp_disabled)));
    ...
    uint32 *phy_data)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_phy_get, (unit, port, flags, phy_reg_addr, phy_data)));
    ...
    uint32 phy_data)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_phy_set, (unit, port, flags, phy_reg_addr, phy_data)));
    ...
    uint32 phy_mask)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_port_phy_modify, (unit, port, flags, phy_reg_addr, phy_data, phy_mask)));
    ...
    uint32 burst)
{
     ...
     * config shaper
     */
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_rate_egress_ppt_set, (unit, port, burst, nof_tiks)));
    ...
    uint32 *burst)
{
     ...
     * get shaper info
     */
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_rate_egress_ppt_get, (unit, port, burst, &nof_tiks)));
    ...
    bcm_port_lane_to_serdes_map_t * serdes_map)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_lane_map_db_map_set, (unit, map_size, lane2serdes, 0)));
    ...
    bcm_port_lane_to_serdes_map_t * serdes_map)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_lane_map_db_map_get, (unit, map_size, lane2serdes)));
