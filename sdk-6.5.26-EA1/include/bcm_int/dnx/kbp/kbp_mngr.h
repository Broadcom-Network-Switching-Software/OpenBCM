...
#ifndef DNX_KBP_NOF_DBS_PER_CORE                  /** Defined in KBP swstate */
#define DNX_KBP_NOF_DBS_PER_CORE                1 /** Used as array initializer */
#endif
/* } */
...
#define DNX_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY (DNX_FIELD_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY)
...
/** KBP forwarding and RPF result sizes */
#define DNX_KBP_RESULT_SIZE_FWD                 64
#define DNX_KBP_RESULT_SIZE_RPF                 32

/** this assumes that there are 48 contexts that are reserved to FWD and 16 reserved for ACL. the first ACL ID context is
 *  48. this will allow support ISSU (we keeping some context for FWD and some for ACLs */
#define DNX_KBP_FWD_CONTEXT_LAST                (48)
/** NOF FWD2 contexts*/
#define DNX_KBP_NOF_FWD_CONTEXTS                (64)
/*
 *  This assumes that there are 48 opcodes that are reserved to FWD (static) and 16 reserved for ACL (dynamic).
 *  the first dynamic opcode is 32. this will allow support ISSU (we keeping some opcodes for FWD and some for ACLs)
 */
#define DNX_KBP_MAX_STATIC_OPCODES_NOF          (48)
 ...
 */
/**
 * Check if KBP status means that device lock has occurred.
 */
...
#define DNX_KBP_FIELD_OMMITTED_FROM_LARGE_OPTIMIZED_RESULT(_field_id_) \
...
#define DNX_KBP_FIELD_OMMITTED_FROM_OPTIMIZED_RESULT(_field_id_) \
    ((DNX_KBP_FIELD_OMMITTED_FROM_LARGE_OPTIMIZED_RESULT(_field_id_) || (_field_id_ == DBAL_FIELD_KBP_FORWARD_PMF_GENERIC_DATA)) ? 1 : 0)

#define DNX_KBP_OPTIMIZED_RESULT_TYPE_HW_VALUE 0

#define DNX_KBP_LARGE_OPTIMIZED_RESULT_SIZE_DEFAULT_CAPACITY (48 * 1024)
 ...
 * {
 */
/** global parameters related to KBP currently implemented exactly as KAPS */
#if defined(BCM_DNX2_SUPPORT) && defined(INCLUDE_KBP)
/* { */
typedef struct
{
...
typedef struct
{
    ...
    uint8 acl_contexts[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_ACL_CONTEXT_NUM];
...
*                                      Contains DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_CONTEXT_NUM_FOR_ONE_APPTYPE elements.
*  \return
*    \retval errors if unexpected behavior. See \ref shr_error_e
*  \remark
*    None
...
*          optional values are 0 - non algo, 1 - power ctrl (partitions of the TCAM), 2 - power ctrl advance, 3 - net
*          ACL (improve capacity and power) to use default value set (DNX_KBP_DB_PROPERTIES_ALGORITHMIC_MODE_DEFAULT_INDICATION).
*   \param [out]  acl_db_as_lpm - enable LPM bechavior for ACL DB.
*  \return
*    \retval errors if unexpected behavior. See \ref shr_error_e
*  \remark
*    None
...
/**
 * \brief
 *   Gets the db_id of a DBAL table for a specific core.
 * \param [in] unit - Relevant unit.
 * \param [in] dbal_table_id - The KBP DBAL table ID.
 * \param [in] core - The core for whihc to get the DB_ID. expected BCM_CORE_ALL if the table is not per_core.
 * \param [out] db_id - The returned db_id for the core.
  \return
 *    \retval errors if unexpected behavior. See \ref shr_error_e
 *  \remark
 *    None
 ...
 *    shr_error_e
 */
shr_error_e kbp_mngr_table_db_id_get(
    int unit,
    dbal_tables_e dbal_table_id,
    bcm_core_t core,
    uint32 *db_id);
...
/**
 * \brief
 *   Gets the KBP table XL DB property according to the DBAL table ID
 * \param [in] unit - Relevant unit.
 * \param [in] dbal_table_id - The KBP DBAL table ID.
 * \param [out] xl_db - The returned XL DB property value.
  \return
 *    \retval errors if unexpected behavior. See \ref shr_error_e
 *  \remark
 *    None
 ...
 *    shr_error_e
 */
shr_error_e kbp_mngr_table_property_xl_db_get(
    int unit,
    dbal_tables_e dbal_table_id,
    int *xl_db);
...
/**
 * \brief
 *   Sets the KBP table XL DB property according to the DBAL table ID
 * \param [in] unit - Relevant unit.
 * \param [in] dbal_table_id - The KBP DBAL table ID.
 * \param [in] xl_db - The XL DB property value to be set.
  \return
 *    \retval errors if unexpected behavior. See \ref shr_error_e
 *  \remark
 *    None
 ...
 *    shr_error_e
 */
shr_error_e kbp_mngr_table_property_xl_db_set(
    int unit,
    dbal_tables_e dbal_table_id,
    int xl_db);
...
/**
 * \brief - Indicates if DBs per core can be used.
 * \param [in] unit - Relevant unit.
 * \param [in] core_mode - Relevant core mode.
  \return
 *   TRUE when alowed, FALSE otherwise
 *
 * \remark
 *   * None
 * \see
 *   * None
 */
uint8 kbp_mngr_use_different_dbs_per_core(
    int unit,
