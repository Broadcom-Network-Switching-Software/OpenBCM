...
/**
 * This enum contains state table key types.
 */
typedef enum
{
    DNX_FIELD_CONTEXT_STATE_TABLE_KEY_TYPE_ADDRESS = 0,
    DNX_FIELD_CONTEXT_STATE_TABLE_KEY_TYPE_DATA = 1,
    DNX_FIELD_CONTEXT_STATE_TABLE_KEY_TYPE_OPCODE = 2,
} dnx_field_context_state_table_key_type_e;

/**
 * This enum contains different types of external TCAMs.
 */
typedef enum dnx_field_external_tcam_type_e
{
    DNX_FIELD_EXTERNAL_TCAM_TYPE_NONE = 0,
    DNX_FIELD_EXTERNAL_TCAM_TYPE_KBP = 1,
} dnx_field_external_tcam_type_t;
    ...
    { \
        ...
        {    \
            ...
            {    \
                 ...
                 */    \
                SHR_IF_ERR_EXIT(dnx_field_group_action_length_to_bit_size(_unit,DNX_FIELD_STAGE_INVALID,    \
                                                                          _action_length_type,&_block_size));    \
                break;    \
            }    \
            case (DNX_FIELD_GROUP_TYPE_EXEM):    \
            {    \
                dnx_field_stage_e _fg_field_stage; \
                SHR_IF_ERR_EXIT(dnx_field_group_field_stage_get(_unit,_fg_id,&_fg_field_stage)); \
                _block_size = dnx_data_field.stage.stage_info_get(unit,_fg_field_stage)->exem_max_result_size; \
                break;    \
            }    \
...
{                                                                                                                 \
    ...
    _nof_fem_id_in_stage = dnx_data_field.stage.stage_info_get(_unit, _field_stage)->nof_fem_id;                    \
...
{ \
    uint8 _is_enum_illegal; \
    SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_FIELD_KEY, _key_id, &_is_enum_illegal)); \
    if (_is_enum_illegal)  \
    {  \
        SHR_ERR_EXIT(_SHR_E_PARAM,  \
                     "\r\n" \
                     "key_id (%d) not of a legal value.\r\n",  \
                     _key_id);  \
    } \
}
/**
...
#define DNX_FIELD_COMPARE_PAIR_VERIFY(_unit,_pair_id)                                                   \
        if ((unsigned int) (_pair_id) >= (unsigned int) (dnx_data_field.compare.nof_compare_pairs_get(_unit))) \
        { \
            SHR_ERR_EXIT(_SHR_E_PARAM, \
                         "\r\n" \
                         "Compare pair ID (%d) is out of range (Should be %d or %d)\r\n", \
                         _pair_id, 0, (dnx_data_field.compare.nof_compare_pairs_get(_unit) - 1)); \
        }
/**
 ...
 * If user doesn't want to configure preselectors he can use default context for grou_context_attach
 ...
 * direct exraction and external TCAM, is that we cannot disable them if we have a miss.
 ...
 * This does not include the 2 bits for EFES 2MSB
 */
#define DNX_FIELD_EXEM_MIN_VMV_SIZE_NOT_INCLUDING_2MSB (2)
/**
 * The maximum number of different mappings per qualifier.
 */
#define DNX_FIELD_QAUL_MAX_NOF_MAPPINGS (DNX_DATA_MAX_FIELD_QUAL_SPECIAL_METADATA_QUAL_MAX_PARTS)
...
/**
 * Invalid indication EFES key select.
 */
#define DNX_FIELD_FES_KEY_SELECT_INVALID      ((dnx_field_fes_key_select_t)(-1))
