...
/**
 * \brief - return the internal 'input CGM port' mapped the port's reassembly contexts
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit -  Unit-ID
 *   \param [in] port -  port id
 *   \param [out] nof_entries - number of entries
 *   \param [out] in_cgm_port - input CGM port
 *
 * \par INDIRECT INPUT:
 *   * None
 * \par DIRECT OUTPUT:
 ...
 *   * None
 */
shr_error_e dnx_port_ingr_reassembly_context_in_cgm_port_get(
    int unit,
    bcm_port_t port,
    int *nof_entries,
    uint32 *in_cgm_port);
