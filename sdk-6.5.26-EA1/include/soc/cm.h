...
#define CM_MAIN2SUB_DEV(unit, sub_dev_index) (cm_composite_devices[CMDEV(unit).dev.composite_index].sub_devs[sub_dev_index])
...
/* get a sub-device of a main device, or -1 if the input is not a composite unit and a valid sub unit index */
#define CM_MAIN2SUB_DEV_SAFE(unit, sub_dev_index) \
  ...
  sub_dev_index < ((soc_cm_comp_device_info_t*)CMDEV(unit).dev.info)->nof_sub_devs ? \
  CM_MAIN2SUB_DEV(unit, (sub_dev_index)) : -1)
