...
/* The following macros and functions are used for TH3, to control issuing of
 * callbacks for the first cycle during wb recovery, when the sync thread is
 * re-building its internal shadow table
 */

/* Current/default operation. Sync thread will not notify requestors when
 * the shadow table rebuild is completed (that is, the first cycle of the sync
 * thread) (example, anytime when the sync thread is 're'-started,
 * and other than wb recover stage)
 */
#define SOC_L2X_WB_RECOVERY_DB_REBUILD_ISSUE_CALLBACKS   (1U << 0) /* bit 0 */

/*
 * BCM layer indicates not to issue any callbacks during wb recovery
 */
#define SOC_L2X_WB_RECOVERY_DB_REBUILD_PAUSE_CALLBACKS   (1U << 1) /* bit 1 */

/*
 * Sync thread sets this bit to indicate that rebuild of shadow table is
 * completed (the 'first' rebuild); it will resume issuing callbacks.
 * Valid only during wb recovery
 */
#define SOC_L2X_WB_RECOVERY_DB_REBUILD_RESUME_CALLBACKS  (1U << 2) /* bit 2*/

/* Set macro below to the highest bit position amongst the above definitions */
/* Note: Should be <= 31 for correct operation */
#define SOC_L2X_WB_RECOVERY_DB_REBUILD_MAX_BIT 2

/* Upper layer controlled bits */
#define SOC_L2X_WB_RECOVERY_DB_REBUILD_VALID_APPL_BITS_MASK \
            (SOC_L2X_WB_RECOVERY_DB_REBUILD_ISSUE_CALLBACKS | \
             SOC_L2X_WB_RECOVERY_DB_REBUILD_PAUSE_CALLBACKS)

/* General mask for checking validity of value passed by any caller */
#define SOC_L2X_WB_RECOVERY_DB_REBUILD_VALID_ALL_BITS_MASK \
            (SOC_L2X_WB_RECOVERY_DB_REBUILD_ISSUE_CALLBACKS | \
             SOC_L2X_WB_RECOVERY_DB_REBUILD_PAUSE_CALLBACKS | \
             SOC_L2X_WB_RECOVERY_DB_REBUILD_RESUME_CALLBACKS)

/* Max value to check validity of input parameters */
#define SOC_L2X_WB_RECOVERY_DB_REBUILD_MAX_VALUE \
            (1U << SOC_L2X_WB_RECOVERY_DB_REBUILD_MAX_BIT)

extern int soc_l2x_wb_recovery_rebuild_cb_action_set(int unit, int action);
extern int soc_l2x_wb_recovery_rebuild_cb_action_get(int unit, int *state);
extern void soc_l2x_wb_recovery_rebuild_cb_mutex_destroy(int unit);
extern int soc_l2x_wb_recovery_rebuild_cb_mutex_create(int unit);
