...
#define DNX_SW_STATE_ALLOC_BITMAP(unit, node_id, location, nof_bits_to_alloc, flags, dbg_string)\
    do {\
...
#define DNX_SW_STATE_BIT_RANGE_EQ(_unit, _node_id, _flags, _sw_state_bmp, _sw_state_bmp_first, __count, _input_bmp, _bool_result)\
    do {\
        SHR_IF_ERR_EXIT(dnxc_sw_state_bitmap_range_eq(_unit, _node_id, _sw_state_bmp, _sw_state_bmp_first, input_bmp, __count, _bool_result, _flags, NULL));\
    } while(0)
