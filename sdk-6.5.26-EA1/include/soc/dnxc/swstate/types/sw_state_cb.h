...
#define SW_STATE_CB_DB_REGISTER_CB(node_id, cb_db, name, get_cb_function_cb) \
    do { \
...
#define SW_STATE_CB_DB_UNREGISTER_CB(node_id, cb) \
    do { \
        sal_memset(cb.function_name, 0, SW_STATE_CB_DB_NAME_STR_SIZE-1);\
    } while(0)

#define SW_STATE_CB_DB_GET_CB(node_id, cb_db, cb, get_cb_function_cb) \
    do { \
