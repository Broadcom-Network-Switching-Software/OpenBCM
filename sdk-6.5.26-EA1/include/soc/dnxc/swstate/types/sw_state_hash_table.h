...
#define SW_STATE_HTB_CREATE(node_id, htb, init_info, key_type, value_type, is_index, nof_elements, alloc_flags) \
    SHR_IF_ERR_EXIT(sw_state_hash_table_create(unit, node_id, &htb, init_info, sizeof(key_type), sizeof(value_type), is_index, nof_elements, alloc_flags))

#define SW_STATE_HTB_CREATE_WITH_SIZE(node_id, htb, init_info, key_size, value_size, is_index, nof_elements, alloc_flags) \
    SHR_IF_ERR_EXIT(sw_state_hash_table_create(unit, node_id, &htb, init_info, key_size, value_size, is_index, nof_elements, alloc_flags))
...
#define SW_STATE_HTB_INSERT(node_id, htb, key, data, success) \
    do { \
        if (htb->is_index) { \
            SHR_IF_ERR_EXIT(sw_state_hash_table_entry_add(unit, node_id, htb, (SW_STATE_HASH_TABLE_KEY*)key, NULL, (uint32*)data, success)); \
        } else { \
            SHR_IF_ERR_EXIT(sw_state_hash_table_entry_add(unit, node_id, htb, (SW_STATE_HASH_TABLE_KEY*)key, (SW_STATE_HASH_TABLE_DATA*)data, NULL, success)); \
        } \
    } while (0)
...
#define SW_STATE_HTB_FIND(node_id, htb, key, data, found) \
    do { \
        if (htb->is_index) { \
            SHR_IF_ERR_EXIT(sw_state_hash_table_entry_lookup(unit, node_id, htb, (SW_STATE_HASH_TABLE_KEY*)key, NULL, (uint32*)data, found)); \
        } else { \
            SHR_IF_ERR_EXIT(sw_state_hash_table_entry_lookup(unit, node_id, htb, (SW_STATE_HASH_TABLE_KEY*)key, (SW_STATE_HASH_TABLE_DATA*)data, NULL, found)); \
        } \
    } while(0)
...
#define SW_STATE_HTB_GET_NEXT(node_id, htb, iter, key, data) \
    do { \
        if (htb->is_index) { \
            SHR_IF_ERR_EXIT(sw_state_hash_table_get_next(unit, node_id, htb, iter, (SW_STATE_HASH_TABLE_KEY*)key, NULL, (uint32*)data)); \
        } else { \
            SHR_IF_ERR_EXIT(sw_state_hash_table_get_next(unit, node_id, htb, iter, (SW_STATE_HASH_TABLE_KEY*)key, (SW_STATE_HASH_TABLE_DATA*)data, NULL)); \
        } \
    } while(0)
...
#define SW_STATE_HTB_INSERT_AT_INDEX(node_id, htb, key, data_indx, success) \
    SHR_IF_ERR_EXIT(sw_state_hash_table_entry_add_at_index(unit, node_id, htb, (SW_STATE_HASH_TABLE_KEY*)key, data_indx, success))

#define SW_STATE_HTB_DELETE_BY_INDEX(node_id, htb, data_indx) \
    SHR_IF_ERR_EXIT(sw_state_hash_table_entry_remove_by_index(unit, node_id, htb, data_indx))

#define SW_STATE_HTB_GET_BY_INDEX(node_id, htb, data_indx, key, found) \
    SHR_IF_ERR_EXIT(sw_state_hash_table_get_by_index(unit, node_id, htb, data_indx, (SW_STATE_HASH_TABLE_KEY*)key, found))
