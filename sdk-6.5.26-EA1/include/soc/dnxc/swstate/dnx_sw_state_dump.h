...
#define DNX_SW_STATE_PRINT(unit, ...) \
        do { \
            ...
            sal_snprintf(str, sizeof(str), __VA_ARGS__); \
...
#define DNX_SW_STATE_PRINT_ENUM(unit, e_get_name_cb) \
        do { \
            const char* enum_name = e_get_name_cb; \
            DNX_SW_STATE_PRINT(unit, "%s\n", enum_name); \
        } while (0)
...
#define DNX_SW_STATE_PRINT_MONITOR(unit, short_str, full_str, ...) \
    do { \
        if (sal_strcmp(dnx_sw_state_dump_directory_get(unit),"") == 0) { \
            ...
            sal_snprintf(prefix, sizeof(prefix), STRING_TO_PRINT(short_str, full_str), unit); \
            sal_snprintf(prefix + sal_strnlen(prefix, sizeof(prefix)), sizeof(prefix), __VA_ARGS__); \
