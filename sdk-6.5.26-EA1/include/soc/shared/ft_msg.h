...
/* NOTE : Version 4 and 5 are used for SDKLT based devices */

/*
 * Ehanced shr_ft_msg_ctrl_collector_create_t to add:
 * - domain_id
 */
#define FT_MSG_VERSION_6 6

/*
 * Enhanced shr_ft_msg_ctrl_collector_create_t, shr_ft_msg_ctrl_collector_get_t
 * - num_records
 * Enhanced shr_ft_msg_ctrl_stats_export_t to add:
 * - num_pkt_export_fails
 */
#define FT_MSG_VERSION_7 7
 ...
 * - ether_type
 */
#define FT_MSG_VERSION_8 8

#define FT_MSG_VERSION  FT_MSG_VERSION_8
...
/*
 *  * Use num_records instead of mtu to determine how many records to pack in a
 *   * single packet.
 *    */
#define SHR_FT_MSG_COLLECTOR_FLAGS_USE_NUM_RECORDS (1 << 1)
...
typedef struct __COMPILER_ATTR__ shr_ft_msg_ctrl_group_flow_data_get_s {
    ...
    /* Outer Source IPv6 */
    uint8 src_ipv6_addr[16];

    /* Outer Destination IPv6 */
    uint8 dst_ipv6_addr[16];

    /* Source Mac */
    uint8 src_mac_addr[6];
...
typedef struct __COMPILER_ATTR__ shr_ft_msg_ctrl_stats_export_s {
    ...
    /* Number of packet records fail to export */
    uint32 num_pkt_export_fails;
