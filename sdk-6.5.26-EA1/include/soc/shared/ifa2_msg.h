...
typedef enum shr_ifa2_msg_ctrl_config_update_param_e {
    ...
    SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_VXLAN_FLOW_AGING_INTERVAL = 2,
    SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_DEVICE_ID = 3,
...
/*
 * \brief IFA2 int_pri to cosq map message
 *
 * Update int_pri to cosq mapping.
 */
typedef struct shr_ifa2_msg_ctrl_cosq_map_s {
    uint8 cosq_map[SHR_IFA_UC_MAX_NUM_PORTS][SHR_IFA_UC_MAX_NUM_INT_PRI];
}shr_ifa2_msg_ctrl_cosq_map_t;


typedef struct shr_ifa2_msg_ctrl_config_get_s {
    shr_ifa2_msg_ctrl_config_update_t config;
    uint8 cosq_map[SHR_IFA_UC_MAX_NUM_PORTS][SHR_IFA_UC_MAX_NUM_INT_PRI];
} shr_ifa2_msg_ctrl_config_get_t;

#if defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_MAVERICK2_SUPPORT)

#define IFA2_MAX_MATCH_ID_FIELDS    12

typedef struct shr_ifa2_msg_ctrl_match_id_field_info_s {
    uint32 offset;
    uint32 width;
    uint32 value;
} shr_ifa2_msg_ctrl_match_id_field_info_t;

typedef struct shr_ifa2_msg_ctrl_match_id_map_s {
    uint32                              match_id_types;
    uint32                              match_id_length;
    shr_ifa2_msg_ctrl_match_id_field_info_t   match_id_fields[IFA2_MAX_MATCH_ID_FIELDS];
} shr_ifa2_msg_ctrl_match_id_map_t;
#endif
...
typedef struct shr_ifa2_msg_ctrl_stats_set_s {
    ...
    /* Number of VXLAN terminated IFA
     * packets which had lookup miss - upper 32 bits. */
    uint32 vxlan_term_lookup_miss_cnt_hi;

    /* Number of VXLAN terminated IFA
     * packets which had lookup miss - lower 32 bits. */
    uint32 vxlan_term_lookup_miss_cnt_lo;

    /* Number of VXLAN terminated data
     * packets which could not be inserted to flow table
     * - upper 32 bits. */
    uint32 vxlan_data_flow_table_insert_fail_cnt_hi;

    /* Number of VXLAN terminated data
     * packets which could not be inserted to flow table
     * - lower 32 bits. */
    uint32 vxlan_data_flow_table_insert_fail_cnt_lo;
...
typedef union shr_ifa2_msg_ctrl_s {
    ...
    /* Cosq map message. */
    shr_ifa2_msg_ctrl_cosq_map_t cosq_map;

    /* Config get. */
    shr_ifa2_msg_ctrl_config_get_t config_get;
} shr_ifa2_msg_ctrl_t;
