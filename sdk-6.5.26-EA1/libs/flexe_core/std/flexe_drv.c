    ...
    int speed)
{
    ...
    for (index = 0; index < nof_instances; ++index)
    {
        ...
        FLEXE_RX_LF_SEL_SET(&reg_val, bcm_port + index, (SHR_BITDCL *) & val);
    }
    ...
    int unit)
{
    ...
    soc_reg_field_set(unit, FSCL_FLEXE_OAM_RX_CONFIGr, &reg_val, BASE_LOS_RPL_ENf, 1);
...
shr_error_e
soc_flexe_deskew_alarm_trigger_lf_set(
    int unit,
    int group_index,
    int val)
{
    uint32 reg_val = 0, trigger_en[1];

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(READ_FSCL_FLEXE_GRP_DSW_ENr(unit, &reg_val));
    trigger_en[0] = soc_reg_field_get(unit, FSCL_FLEXE_GRP_DSW_ENr, reg_val, DESKEW_ALM_EN_2f);
    SHR_BITWRITE(trigger_en, group_index, val);
    soc_reg_field_set(unit, FSCL_FLEXE_GRP_DSW_ENr, &reg_val, DESKEW_ALM_EN_2f, trigger_en[0]);
    SHR_IF_ERR_EXIT(WRITE_FSCL_FLEXE_GRP_DSW_ENr(unit, reg_val));
...
shr_error_e
soc_flexe_deskew_alarm_trigger_lf_get(
    int unit,
    int group_index,
    int *val)
{
    ...
    SHR_IF_ERR_EXIT(READ_FSCL_FLEXE_GRP_DSW_ENr(unit, &reg_val));
    trigger_en[0] = soc_reg_field_get(unit, FSCL_FLEXE_GRP_DSW_ENr, reg_val, DESKEW_ALM_EN_2f);
    *val = SHR_BITGET(trigger_en, group_index) ? 1 : 0;
