                                   ...
                                   phymod_phy_pll_state_t* new_pll_state)
{
    ...
    *new_pll_state = *old_pll_state;
    ...
    if (speed_config->linkTraining) {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p1_phy_cl72_set(phy, speed_config->linkTraining));
    } else {
...
int blackhawk7_v1l8p1_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    /* check pll lock if not power up */
    if (!pll_power_down) {
...
int blackhawk7_v1l8p1_phy_rx_pmd_lock_status_get(const phymod_phy_access_t* phy,
                                                 uint32_t* pmd_lock,
                                                 uint32_t* pmd_lock_change)
{
    ...
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_rx_pmd_lock_status_get(&pm_phy_copy.access,
                                          pmd_lock,
                                          pmd_lock_change));
    return PHYMOD_E_NONE;
}
...
{
    info->ucode_ptr = blackhawk7_v1l8p1_ucode_get();
    info->ucode_len = BLACKHAWK7_V1L8P1_UCODE_IMAGE_SIZE;
