...
struct blackhawk7_v1l8p1_prbs_chk_hw_timer_ctrl_st {
...
/** Setup the measurement of BER calculation using HardWare timers in the blackhawk7_v1l8p1.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] time_ms is the amount of time to delay for BER calculation.
...
/** Poll the Hardware based PRBS timer for expiration of time_ms in a BER calculation previously launched with blackhawk7_v1l8p1_BER_setup_measurement().
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[out] ber_data is a struct that will get populated with number of errors and number of bits as elements.
 * @param[in] prbs_chk_hw_timer_ctrl is a structure to allow use of the PRBS error Hardware.
 * @return Error code, if generated (returns ERR_CODE_NONE if no errors).
...
typedef struct {
  ...
  uint32_t pp_field2;
  /** Number of lanes used for aggregation. Example: Set to 1 for 50GE if using BHK, 1 for 100GE if using . This is computed in the API and does not need to be programmed by user unless CUSTOM mode is used. */
 ...
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
err_code_t blackhawk7_v1l8p1_prbs_error_analyzer_extrapolate_err_count(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t error_count_index);
