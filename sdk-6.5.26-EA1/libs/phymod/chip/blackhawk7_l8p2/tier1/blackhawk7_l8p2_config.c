...
err_code_t blackhawk7_l8p2_ucode_crc_verify(srds_access_t *sa__, uint32_t ucode_len, uint16_t expected_crc_value) {
    ...
    if (status_byte & 0x20) {
       ...
       return (blackhawk7_l8p2_error(sa__, ERR_CODE_UCODE_IP_DOES_NOT_MATCH_CORE));
    }
...
err_code_t blackhawk7_l8p2_get_force_lane_cdr_mode(srds_access_t *sa__, enum blackhawk7_l8p2_force_cdr_mode_enum *mode) {
    INIT_SRDS_ERR_CODE
    uint8_t os_mode = 0;
    uint8_t br_mode = 0;
    ...
    if(os_mode && br_mode) {
        return blackhawk7_l8p2_error(sa__, ERR_CODE_INVALID_MODE);
    }

    if(os_mode) {
        *mode = BLACKHAWK7_L8P2_OSCDR_FORCE_ENABLE;
    }
    else if(br_mode) {
        *mode = BLACKHAWK7_L8P2_BRCDR_FORCE_ENABLE;
    }
    else {
        *mode = BLACKHAWK7_L8P2_CDR_FORCE_DISABLE;
    }
    return ERR_CODE_NONE;
}

err_code_t blackhawk7_l8p2_force_lane_cdr_mode(srds_access_t *sa__, enum blackhawk7_l8p2_force_cdr_mode_enum mode) {
    INIT_SRDS_ERR_CODE
    uint16_t misc_ctrl = 0;
    switch(mode) {
        case BLACKHAWK7_L8P2_OSCDR_FORCE_ENABLE:
            ESTM(misc_ctrl = rdv_usr_misc_ctrl_word());
            EFUN(wrv_usr_misc_ctrl_word(misc_ctrl | (0x1 << SRDS_FORCE_OSCDR_BIT))); /*  (force_os_on) */
            EFUN(blackhawk7_l8p2_force_lane_cdr_mode(sa__, BLACKHAWK7_L8P2_BRCDR_FORCE_DISABLE));
            break;
        case BLACKHAWK7_L8P2_OSCDR_FORCE_DISABLE:
            ESTM(misc_ctrl = rdv_usr_misc_ctrl_word());
            EFUN(wrv_usr_misc_ctrl_word(misc_ctrl & (uint16_t)~(0x1 << SRDS_FORCE_OSCDR_BIT)));
            break;
        case BLACKHAWK7_L8P2_BRCDR_FORCE_ENABLE:
            EFUN(wr_lane_uc_config_force_brdfe_en(0x1));
            EFUN(blackhawk7_l8p2_force_lane_cdr_mode(sa__, BLACKHAWK7_L8P2_OSCDR_FORCE_DISABLE));
            break;
        case BLACKHAWK7_L8P2_BRCDR_FORCE_DISABLE:
            EFUN(wr_lane_uc_config_force_brdfe_en(0x0));
            break;
        case BLACKHAWK7_L8P2_CDR_FORCE_DISABLE:
            EFUN(blackhawk7_l8p2_force_lane_cdr_mode(sa__, BLACKHAWK7_L8P2_OSCDR_FORCE_DISABLE));
            EFUN(blackhawk7_l8p2_force_lane_cdr_mode(sa__, BLACKHAWK7_L8P2_BRCDR_FORCE_DISABLE));
            break;
        default:
...
err_code_t blackhawk7_l8p2_reset_tx_lane_to_default(srds_access_t *sa__) {
    ...
    BLACKHAWK7_L8P2_UAPI_INIT_AMS_A0      /* if BLACKHAWK7_V1LXP2 */
    EFUN(_blackhawk7_l8p2_rst_tx_ln_to_dfs(sa__));
    BLACKHAWK7_L8P2_UAPI_NOT_AMS_B0_ERR   /* if not BLACKHAWK7_V2LXP2 and not BLACKHAWK7_V3LXP2, print error */
    return ERR_CODE_NONE;
}
...
err_code_t blackhawk7_l8p2_set_use_osr_mode_pins_only(srds_access_t *sa__, uint8_t use_pins_only) {
    INIT_SRDS_ERR_CODE

    EFUN(blackhawk7_l8p2_set_use_tx_osr_mode_pins_only(sa__, use_pins_only));
    EFUN(blackhawk7_l8p2_set_use_rx_osr_mode_pins_only(sa__, use_pins_only));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_get_use_osr_mode_pins_only(srds_access_t *sa__, uint8_t *tx_use_pins_only, uint8_t *rx_use_pins_only) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(blackhawk7_l8p2_get_use_tx_osr_mode_pins_only(sa__, tx_use_pins_only));
    EFUN(blackhawk7_l8p2_get_use_rx_osr_mode_pins_only(sa__, rx_use_pins_only));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_set_use_rx_osr_mode_pins_only(srds_access_t *sa__, uint8_t rx_use_pins_only) {
    INIT_SRDS_ERR_CODE
    uint8_t reset_state;
    ESTM(reset_state = rd_rx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l8p2_set_use_rx_osr_mode_pins_only(..) called without ln_dp_s_rstb=0\n"));
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
    if(rx_use_pins_only) {
        EFUN(wr_rx_osr_mode_frc(0x0));
        EFUN(wr_rx_osr_mode_frc_val(BLACKHAWK7_L8P2_OSR_UNINITIALIZED));
    }

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_get_use_rx_osr_mode_pins_only(srds_access_t *sa__, uint8_t *rx_use_pins_only) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l8p2_set_use_tx_osr_mode_pins_only(srds_access_t *sa__, uint8_t tx_use_pins_only) {
    INIT_SRDS_ERR_CODE
    uint8_t reset_state;
    ESTM(reset_state = rd_tx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l8p2_set_use_tx_osr_mode_pins_only(..) called without ln_dp_s_rstb=0\n"));
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
    if(tx_use_pins_only) {
        EFUN(wr_tx_osr_mode_frc(0x0));
        EFUN(wr_tx_osr_mode_frc_val(BLACKHAWK7_L8P2_OSR_UNINITIALIZED));
    }

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_get_use_tx_osr_mode_pins_only(srds_access_t *sa__, uint8_t *tx_use_pins_only) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l8p2_set_osr_mode(srds_access_t *sa__, enum blackhawk7_l8p2_osr_mode_enum osr_mode) {
    INIT_SRDS_ERR_CODE

    EFUN(blackhawk7_l8p2_set_tx_osr_mode(sa__, osr_mode));
    EFUN(blackhawk7_l8p2_set_rx_osr_mode(sa__, osr_mode));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_get_osr_mode(srds_access_t *sa__, enum blackhawk7_l8p2_osr_mode_enum *tx_osr_mode,  enum blackhawk7_l8p2_osr_mode_enum *rx_osr_mode) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(blackhawk7_l8p2_get_tx_osr_mode(sa__, tx_osr_mode));
    EFUN(blackhawk7_l8p2_get_rx_osr_mode(sa__, rx_osr_mode));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_set_rx_osr_mode(srds_access_t *sa__, enum blackhawk7_l8p2_osr_mode_enum rx_osr_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t reset_state;
    ESTM(reset_state = rd_rx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l8p2_set_rx_osr_mode(..) called without ln_dp_s_rstb=0\n"));
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
...
err_code_t blackhawk7_l8p2_get_rx_osr_mode(srds_access_t *sa__, enum blackhawk7_l8p2_osr_mode_enum *rx_osr_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t rx_osr_ctrl;
    ...
    if(rx_osr_ctrl) {
        ESTM(*rx_osr_mode = (enum blackhawk7_l8p2_osr_mode_enum)(rd_rx_osr_mode_frc_val()));
    }
    else {
        ESTM(*rx_osr_mode = (enum blackhawk7_l8p2_osr_mode_enum)rd_rx_osr_mode_pin());
    }

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l8p2_set_tx_osr_mode(srds_access_t *sa__, enum blackhawk7_l8p2_osr_mode_enum tx_osr_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t reset_state;
    ESTM(reset_state = rd_tx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l8p2_set_tx_osr_mode(..) called without ln_dp_s_rstb=0\n"));
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
...
err_code_t blackhawk7_l8p2_get_tx_osr_mode(srds_access_t *sa__, enum blackhawk7_l8p2_osr_mode_enum *tx_osr_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t tx_osr_ctrl;
    ...
    if(tx_osr_ctrl) {
        ESTM(*tx_osr_mode = (enum blackhawk7_l8p2_osr_mode_enum)(rd_tx_osr_mode_frc_val()));
    }
    else {
        ESTM(*tx_osr_mode = (enum blackhawk7_l8p2_osr_mode_enum)rd_tx_osr_mode_pin());
    }
...
err_code_t blackhawk7_l8p2_tx_disable(srds_access_t *sa__, uint8_t enable) {
    ...
    if (enable) {
        ...
        BLACKHAWK7_L8P2_UAPI_ELSE_AMS_B0   /* else if BLACKHAWK7_V2LXP2 || BLACKHAWK7_V3LXP2 */
        EFUN(wr_ams_tx_test_data(0x0));
...
err_code_t blackhawk7_l8p2_apply_txfir_cfg(srds_access_t *sa__, enum blackhawk7_l8p2_txfir_tap_enable_enum enable_taps, int16_t pre2, int16_t pre1, int16_t main, int16_t post1, int16_t post2, int16_t post3) {
    ...
    blackhawk7_l8p2_txfir_st txfir;
#if !defined(SMALL_FOOTPRINT) 
    struct blackhawk7_l8p2_event_st api_event;
#endif
    ...
    api_event.data2 = 0;
    EFUN(blackhawk7_l8p2_log_api_event(sa__, api_event));
#endif
    {
...
err_code_t blackhawk7_l8p2_core_reset(srds_access_t *sa__, ucode_info_t ucode_info) {
    INIT_SRDS_ERR_CODE
    uint8_t i, num_lanes;
    uint8_t orig_pll = blackhawk7_l8p2_acc_get_pll_idx(sa__);
    uint8_t orig_lane = blackhawk7_l8p2_acc_get_lane(sa__);
    ...
    for(i = 0; i < num_lanes; i++) {
        EFUN(blackhawk7_l8p2_acc_set_lane(sa__, i));
        EFUN(blackhawk7_l8p2_ln_dp_reset(sa__, 1));
    }
    EFUN(USR_DELAY_US(1));
    for (i = 0; i < NUM_PLLS; i++) {
        EFUN(blackhawk7_l8p2_acc_set_pll_idx(sa__, i));
    ...
    EFUN(blackhawk7_l8p2_acc_set_lane(sa__, orig_lane));
    EFUN(blackhawk7_l8p2_acc_set_pll_idx(sa__, orig_pll));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_init_blackhawk7_l8p2_info(srds_access_t *sa__) {
    ...
    blackhawk7_l8p2_info_ptr->signature = SRDS_INFO_SIGNATURE;
    blackhawk7_l8p2_info_ptr->info_table_version = info_table_version;
    {
        uint8_t afe_version;
...
err_code_t blackhawk7_l8p2_configure_20P625G_pll_refclk(srds_access_t *sa__,
                                        enum blackhawk7_l8p2_pll_refclk_enum refclk) {
    return blackhawk7_l8p2_INTERNAL_configure_20P625_pll(sa__, refclk, BLACKHAWK7_L8P2_PLL_DIV_UNKNOWN, BLACKHAWK7_L8P2_PLL_OPTION_NONE);
}

err_code_t blackhawk7_l8p2_configure_20P625G_pll_div(srds_access_t *sa__,
                                        enum blackhawk7_l8p2_pll_div_enum srds_div) {
    return blackhawk7_l8p2_INTERNAL_configure_20P625_pll(sa__, BLACKHAWK7_L8P2_PLL_REFCLK_UNKNOWN, srds_div, BLACKHAWK7_L8P2_PLL_OPTION_NONE);
}
...
err_code_t blackhawk7_l8p2_configure_pll_refclk_div_refclk_2x(srds_access_t *sa__,
                                           enum blackhawk7_l8p2_pll_refclk_enum refclk,
                                           enum blackhawk7_l8p2_pll_div_enum srds_div) {
    return blackhawk7_l8p2_INTERNAL_configure_pll(sa__, refclk, srds_div, 0, BLACKHAWK7_L8P2_PLL_OPTION_REFCLK_DOUBLER_EN);
}

err_code_t blackhawk7_l8p2_configure_pll_refclk_vco_refclk_2x(srds_access_t *sa__,
                                           enum blackhawk7_l8p2_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return blackhawk7_l8p2_INTERNAL_configure_pll(sa__, refclk, BLACKHAWK7_L8P2_PLL_DIV_UNKNOWN, vco_freq_khz, BLACKHAWK7_L8P2_PLL_OPTION_REFCLK_DOUBLER_EN);
}

err_code_t blackhawk7_l8p2_configure_pll_div_vco_refclk_2x(srds_access_t *sa__,
                                        enum blackhawk7_l8p2_pll_div_enum srds_div,
                                        uint32_t vco_freq_khz) {
    return blackhawk7_l8p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_L8P2_PLL_REFCLK_UNKNOWN, srds_div, vco_freq_khz, BLACKHAWK7_L8P2_PLL_OPTION_REFCLK_DOUBLER_EN);
}
...
/*  Fast Link Recovery  */
/************************/
err_code_t blackhawk7_l8p2_configure_fast_link_recovery(srds_access_t *sa__, uint8_t enable, uint8_t enable_timeout, enum blackhawk7_l8p2_flr_timeout_enum timeout_time) {
    INIT_SRDS_ERR_CODE
    ...
    ESTM(reset_state = rd_rx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l8p2_configure_fast_link_recovery(..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",blackhawk7_l8p2_get_lane(sa__),reset_state));
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
    EFUN(blackhawk7_l8p2_INTERNAL_is_flr_supported(sa__, &flr_support));
    if(!flr_support) {
        EFUN_PRINTF(("ERROR: blackhawk7_l8p2_configure_fast_link_recovery(..) unsupported with currently loaded microcode (Core=%d Lane=%d).\n", blackhawk7_l8p2_acc_get_core(sa__), blackhawk7_l8p2_acc_get_lane(sa__)));
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }

    if (timeout_time_val > 5) {
        ESTM_PRINTF(("timeout_time value for blackhawk7_l8p2_configure_fast_link_recovery() is too large:  %d\n", timeout_time_val));
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    {
...
err_code_t blackhawk7_l8p2_get_configure_fast_link_recovery(srds_access_t *sa__, uint8_t *enable, uint8_t *enable_timeout, enum blackhawk7_l8p2_flr_timeout_enum *timeout_time) {
    INIT_SRDS_ERR_CODE
    uint8_t flr_byte = 0;
    uint8_t flr_support = 0;
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_is_flr_supported(sa__, &flr_support));
    if(!flr_support) {
        EFUN_PRINTF(("ERROR: blackhawk7_l8p2_get_configure_fast_link_recovery(..) unsupported with currently loaded microcode (Core=%d Lane=%d).\n", blackhawk7_l8p2_acc_get_core(sa__), blackhawk7_l8p2_acc_get_lane(sa__)));
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    ...
    *timeout_time = (enum blackhawk7_l8p2_flr_timeout_enum)((flr_byte >> 2) & 0x7);
    return (ERR_CODE_NONE);
}
