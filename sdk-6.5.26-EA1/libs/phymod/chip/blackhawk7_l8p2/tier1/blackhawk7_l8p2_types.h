...
#define SIZE_OF_DBGFB_STATS_STRUCT 296  /* Change the size if blackhawk7_l8p2_dbgfb_stats_st size changes */
/*! Used for maintaining correctness of SIZE_OF_DBGFB_STATS_STRUCT
 *
 */
typedef struct {
    uint8_t dbgfb_tmp[sizeof(blackhawk7_l8p2_dbgfb_stats_st)==SIZE_OF_DBGFB_STATS_STRUCT ? 1: -1];
} blackhawk7_l8p2_dbgfb_stats_st_size_check;

/*! Used for debug feedback APIs
 *
 */
typedef struct blackhawk7_l8p2_dbgfb_cfg_s {
    uint8_t y;
    uint8_t x;
    uint32_t time_in_us;
    int32_t data1;
 ...
 *
 */
struct blackhawk7_l8p2_ber_data_st {
    uint64_t num_errs;
    uint64_t num_bits;
    uint8_t lcklost;
    uint8_t prbs_chk_en;
 ...
 *
 */
struct blackhawk7_l8p2_check_platform_info_st {
    char c;
    uint32_t d;
};
...
struct blackhawk7_l8p2_detailed_lane_status_st {
    ...
    blackhawk7_l8p2_dbgfb_stats_st dbgfb_stats;
    ...
    /* =========== 8-bit variables end ============== */
};  /* struct blackhawk7_l8p2_detailed_lane_status_st  for BLACKHAWK7 */
