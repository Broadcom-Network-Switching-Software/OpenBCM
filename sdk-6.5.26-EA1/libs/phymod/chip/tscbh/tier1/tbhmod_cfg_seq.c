...
#define TSCBH_FE_GEN2        0x2f /* 0x9008 Main0_serdesID - Serdes ID Register */
          ...
          case 25000:
              if (an_ability->an_mode == phymod_AN_MODE_CL73) {
                  ...
                  if (an_ability->channel == phymod_channel_short) {
                      ...
                      if ((an_ability->fec == phymod_fec_None) || (an_ability->fec == phymod_fec_CL74)) {
                          AN_X4_LD_BASE_ABIL1r_BASE_25G_CRS_KRSf_SET(AN_X4_ABILITIES_LD_BASE_ABILITIES_1r_reg, 1);
                      }
                  }
              } else {
...
int tbhmod_autoneg_ability_get(PHYMOD_ST* pc, phymod_autoneg_advert_abilities_t* autoneg_abilities)
{
  ...
  if (AN_X4_CL73_CFGr_MSA_FEC_MAPPINGf_GET(AN_X4_CL73_CFGr_reg)) {
      ...
      if (((model_num == TSCBH_GEN2) ||
           (model_num == TSCBH_FLEXE) ||
           (model_num == TSCBH_FE_GEN2)) && AN_X4_LD_UP1_ABIL_1_GEN2r_MSA_LFRf_GET(AN_X4_LD_UP1_ABIL1r_reg)) {
...
int tbhmod_set_an_timers(PHYMOD_ST* pc, phymod_ref_clk_t refclk, uint32_t *pam4_an_timer_value)    /* SET_AN_TIMERS */
{
    ...
    if (pam4_an_timer_value == NULL) {
        ...
        AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_SET(AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_CL72_PAM4r_reg, 0x3415);
...
int tbhmod_an_link_fail_inhibit_timer_set(PHYMOD_ST* pc, tbhmod_an_timer_t timer_type, uint32_t period)
{
    AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_t AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_CL72_PAM4r_reg;
    uint16_t tick_count;

    switch (timer_type) {
        case TBHMOD_AN_FAIL_INHIBIT_TIMER_LT_PAM4:
...
int tbhmod_an_link_fail_inhibit_timer_get(PHYMOD_ST* pc, tbhmod_an_timer_t timer_type, uint32_t* period)
{
    AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_t AN_X1_TIMERS_LINK_FAIL_INHIBIT_TIMER_CL72_PAM4r_reg;
    uint16_t tick_count;
...
int tbhmod_pcs_1588_ts_offset_get(PHYMOD_ST *pc, uint32_t *ns_offset, uint32_t *sub_ns_offset)
{
    PMD_X1_PM_TMR_OFFSr_t PMD_X1_PM_TMR_OFFSr_reg;
...
int tbhmod_pcs_rx_ts_update_en(PHYMOD_ST* pc, int en)
{
  RX_X4_RX_TS_CTLr_t RX_X4_RX_TS_CTLr_reg;
...
int tbhmod_pcs_mod_rx_1588_tbl_val(PHYMOD_ST* pc, int bit_mux_mode, uint32_t vco, int os_mode, int pam4, int tbl_ln, uint32_t *table) {
    ...
    if ((psll_val_max - psll_val_min) > 0xF4C0) {
        /*
        ...
        for(i=0; i<virtual_lanes; i++) {
            ...
            */
            if((psll_val_max - psll_update[i]) > 0x0F4C0) psll_update[i] |= 0x10000 ;
            if (psll_update[i] < psll_roval_min)  psll_roval_min = psll_update[i];
            if (psll_update[i] > psll_roval_max)  psll_roval_max = psll_update[i];
        }

        /*
         * Check for Fake AM.  If max to min is above 180ns, it is FAKE AM.
         * 180ns is 0xB4. First 4 bits are subns.
         */
        if ((psll_roval_max-psll_roval_min) > 0xB40) {
            /* ** DEBUG Print **
            printf(" #####  am roll over adjustment out of range.\n");
            for(i=0; i<virtual_lanes; i++) {
                printf("psll_update %d 0x%x \n",i,psll_update[i]);
            }
            */
        ...
        if ((psll_val_max-psll_val_min) > 0xB40) {
            /* ** DEBUG Print **
            printf(" #####  am adjustment out of range.\n");
            for( i = 0; i < virtual_lanes; i++) {
                printf("psll_update %d 0x%x \n",i,psll_update[i]);
            }
            */
...
int tbhmod_fec_cl74_correctable_counter_get (PHYMOD_ST* pc, uint32_t* count)
{
    RX_X4_FEC_CORRBLKSL0r_t countl_0;
    RX_X4_FEC_CORRBLKSH0r_t counth_0;

    PHYMOD_IF_ERR_RETURN
        (READ_RX_X4_FEC_CORRBLKSL0r(pc, &countl_0));
    PHYMOD_IF_ERR_RETURN
        (READ_RX_X4_FEC_CORRBLKSH0r(pc, &counth_0));

    *count = (RX_X4_FEC_CORRBLKSH0r_GET(counth_0) << 16) | (RX_X4_FEC_CORRBLKSL0r_GET(countl_0) & 0xFFFF);
    return PHYMOD_E_NONE;
}

int tbhmod_fec_cl74_uncorrectable_counter_get (PHYMOD_ST* pc, uint32_t* count)
{
    RX_X4_FEC_UNCORRBLKSL0r_t countl_0;
    RX_X4_FEC_UNCORRBLKSH0r_t counth_0;

    PHYMOD_IF_ERR_RETURN
        (READ_RX_X4_FEC_UNCORRBLKSL0r(pc, &countl_0));
    PHYMOD_IF_ERR_RETURN
        (READ_RX_X4_FEC_UNCORRBLKSH0r(pc, &counth_0));

    *count = (RX_X4_FEC_UNCORRBLKSH0r_GET(counth_0) << 16) | (RX_X4_FEC_UNCORRBLKSL0r_GET(countl_0) & 0xFFFF);
    return PHYMOD_E_NONE;
}
...
int tbhmod_400g_autoneg_timer_disable(PHYMOD_ST* pc, int timer_disable)
{
    ...
    if (timer_disable) {
        ...
        AN_X1_LNK_FAIL_INHBT_TMR_CL72r_SET(link_fail_cl72_timer_reg, 0x208e);
        PHYMOD_IF_ERR_RETURN(WRITE_AN_X1_LNK_FAIL_INHBT_TMR_CL72r(&phy_copy, link_fail_cl72_timer_reg));
        /* 0x9256 160.98 ms */
        AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72r_CLR(link_fail_not_cl72_timer_reg);
        /*AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72r_SET(link_fail_not_cl72_timer_reg, 0x29ab);*/
        AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72r_SET(link_fail_not_cl72_timer_reg, 0xa6c);
        PHYMOD_IF_ERR_RETURN(WRITE_AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72r(&phy_copy, link_fail_not_cl72_timer_reg));

        /*0x925a 3199.9 ms */
        AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_CLR(link_fail_cl72_pam4_timer_reg);
        /*AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_SET(link_fail_cl72_pam4_timer_reg, 0xc238);*/
        AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r_SET(link_fail_cl72_pam4_timer_reg, 0x3415);
        PHYMOD_IF_ERR_RETURN(WRITE_AN_X1_LNK_FAIL_INHBT_TMR_CL72_PAM4r(&phy_copy, link_fail_cl72_pam4_timer_reg));
        /*0x925b 1000.08 ms */
        AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72_PAM4r_CLR(link_fail_not_cl72_pam4_timer_reg);
        /*AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72_PAM4r_SET(link_fail_not_cl72_pam4_timer_reg, 0x41aa);*/
        AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72_PAM4r_SET(link_fail_not_cl72_pam4_timer_reg, 0x106a);
        PHYMOD_IF_ERR_RETURN(WRITE_AN_X1_LNK_FAIL_INHBT_TMR_NOT_CL72_PAM4r(&phy_copy, link_fail_not_cl72_pam4_timer_reg));
...
int tbhmod_vco_to_pll_lkup(uint32_t vco, tbhmod_refclk_t refclk, uint32_t* pll_div)
{
    switch(refclk) {
        case TBHMOD_REF_CLK_156P25MHZ:
            ...
            } else if (vco == TBHMOD_VCO_25P00G) {
                *pll_div = TBHMOD_PLL_MODE_DIV_160;
...
int tbhmod_ecc_error_intr_enable_set(PHYMOD_ST *pc, phymod_interrupt_type_t intr_type, uint32_t enable)
{
    ...
    switch (intr_type) {
        ...
        case phymodIntrEccRx1588Mpp1:
            ...
            break;
        case phymodIntrEccRsFECRs400gMpp0:
            MAIN0_ECC_1B_ERR_INTR_ENr_ECC_1B_ERR_INTR_EN_RSFEC_RS400G_MPPf_SET(reg_1b, enable);
            PHYMOD_IF_ERR_RETURN
                (MODIFY_MAIN0_ECC_1B_ERR_INTR_ENr(&pc_copy, reg_1b));
            MAIN0_ECC_2B_ERR_INTR_ENr_ECC_2B_ERR_INTR_EN_RSFEC_RS400G_MPPf_SET(reg_2b, enable);
            PHYMOD_IF_ERR_RETURN
                (MODIFY_MAIN0_ECC_2B_ERR_INTR_ENr(&pc_copy, reg_2b));
            break;
...
int tbhmod_bip_error_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
...
int tbhmod_cl49_ber_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
...
int tbhmod_cl82_ber_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
...
/*The unit of tick_time is us*/
int tbhmod_tick_time_get(PHYMOD_ST* pc, uint32_t *tick_time)
{
    ...
    uint32_t tick_override, tick_numerator_upper, tick_numerator_lower, tick_denominator;
    ...
    ref_clk = MAIN0_SETUPr_REFCLK_SELf_GET(MAIN0_SETUPr_reg);
    tick_override = MAIN0_TICK_CTL1r_TICK_OVERRIDEf_GET(MAIN0_TICK_CONTROL_1r_reg);
    tick_numerator_upper = MAIN0_TICK_CTL1r_TICK_NUMERATOR_UPPERf_GET(MAIN0_TICK_CONTROL_1r_reg);
    tick_numerator_lower = MAIN0_TICK_CTL0r_TICK_NUMERATOR_LOWERf_GET(MAIN0_TICK_CONTROL_0r_reg);
    tick_denominator = MAIN0_TICK_CTL0r_TICK_DENOMINATORf_GET(MAIN0_TICK_CONTROL_0r_reg);

    if (tick_override == 0) {
        /*the default time count is 15us*/
        *tick_time = 15;
    }

    if (tick_override) {
        if (ref_clk == TBHMOD_REF_CLK_156P25MHZ) {
            *tick_time = (((tick_numerator_upper << 4) + tick_numerator_lower) * 100) / (tick_denominator * 15625);
        } else {
            *tick_time = (((tick_numerator_upper << 4) + tick_numerator_lower) *10) / (tick_denominator * 3125);
        }
    }
...
/*
 * @brief Get fec align live state
 * @param pc handle to current TSCBH context (#tbhmod_st)
 * @returns The value PHYMOD_E_NONE upon successful completion
 */
int tbhmod_rs_fec_align_live_state_get(PHYMOD_ST* pc, uint32_t *state)
{
    /* 0xc351 */
    RX_X4_RS_FEC_RXP_STSr_t fec_rxp_sts;

    RX_X4_RS_FEC_RXP_STSr_CLR(fec_rxp_sts);
    PHYMOD_IF_ERR_RETURN(READ_RX_X4_RS_FEC_RXP_STSr(pc, &fec_rxp_sts));

    *state = RX_X4_RS_FEC_RXP_STSr_FEC_ALIGN_STATUS_LIVEf_GET(fec_rxp_sts);

    return PHYMOD_E_NONE;
}

/*
 * @brief Get RS FEC amps lock state
 * @param pc handle to current TSCBH context (#tbhmod_st)
 * @returns The value PHYMOD_E_NONE upon successful completion
 */
int tbhmod_rs_fec_amps_lock_state_get(PHYMOD_ST* pc, int nums_fec_lane, tbhmod_rx_rsfec_sync_status_t* sync_status)
{
    /* 0xc17a */
    RX_X4_RS_FEC_SYNC_STSr_t rx_fec_sync_sts;
    RX_X4_RS_FEC_SYNC_STSr_CLR(rx_fec_sync_sts);
    PHYMOD_IF_ERR_RETURN(READ_RX_X4_RS_FEC_SYNC_STSr(pc, &rx_fec_sync_sts));
    /* Check fec lane (zero). */
    sync_status->fec_amps_lock_live_0 = RX_X4_RS_FEC_SYNC_STSr_RS_FEC_AMPS_LOCK_LIVE_0f_GET(rx_fec_sync_sts);
    /* Check fec lane (one). */
    if (nums_fec_lane > 1) {
        sync_status->fec_amps_lock_live_1 = RX_X4_RS_FEC_SYNC_STSr_RS_FEC_AMPS_LOCK_LIVE_1f_GET(rx_fec_sync_sts);
    }

    return PHYMOD_E_NONE;
}

int tbhmod_fec_error_bits_counter_get (PHYMOD_ST* pc, uint32_t* count)
{
    int start_lane, num_lane;
    ...
    if (num_lane != 8) {
        ...
        *count = (RX_X4_FEC_BIT_ERR_CTR1r_GET(count_high) << 16) |
