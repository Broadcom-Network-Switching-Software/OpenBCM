    ...
    int unit)
{
    ...
    if (is_standard_image)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_tables_field_predefine_value_get
                        (unit, DBAL_TABLE_EEDB_IPV4_TUNNEL, DBAL_FIELD_OUT_LIF_PROFILE, FALSE,
                         result_type, 0, DBAL_PREDEF_VAL_MIN_VALUE, &ip_orientation_min_val));
    }
    /*
     * for bare metal 
     */
    ...
    extra_info.resource_create_info.max_tag_value[0] = 3;
    extra_info.resource_create_info.grain_size[0] = 1;
    ...
    exchange_tag_info.nof_refs = max_nof_pp_ports;
    ...
    exchange_tag_info.nof_refs = max_nof_out_lifs;
    SHR_IF_ERR_EXIT(algo_lif_profile_db.etpp_out_lif_profile.exchange
                    (unit, 0, &etpp_out_lif_profile_template_data, DNX_OUT_LIF_PROFILE_DEFAULT,
                     &exchange_tag_info, &new_etpp_out_lif_profile, &is_first, &is_last));
