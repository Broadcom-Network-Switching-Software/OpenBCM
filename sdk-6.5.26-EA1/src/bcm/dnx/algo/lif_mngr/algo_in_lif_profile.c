    ...
    int unit)
{
    ...
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles))
    {
        /** The number of profiles is equal to the number of profiles per range (according to nof_ingress_profile_bits) multiplied by the number of possible PMF profiles */
        data.nof_profiles = (1 << nof_ingress_profile_bits) * (1 << IN_LIF_PROFILE_PMF_RESERVED_NOF_BITS);
    }
    else
    {
