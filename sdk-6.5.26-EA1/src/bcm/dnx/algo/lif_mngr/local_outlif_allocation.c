    ...
    uint32 alloc_flags)
{
    ...
    if (extra_info.assigned == DNX_ALGO_OUTLIF_BANK_ASSIGNED_TO_EEDB)
    {
        ...
        extra_create_info.grain_size[outlifs_grain_idx] = nof_eedb_banks_per_outlif_bank;
        extra_create_info.max_tag_value[outlifs_grain_idx] =
            utilex_msb_bit_on(extra_create_info.grain_size[outlifs_grain_idx]);
    }
    else
    {

        extra_create_info.grain_size[outlifs_grain_idx] = nof_outlifs_per_entry;
        extra_create_info.max_tag_value[outlifs_grain_idx] =
            utilex_msb_bit_on(extra_create_info.grain_size[outlifs_grain_idx]);
        if (extra_create_info.grain_size[outlifs_grain_idx] == 1)
        {

            extra_create_info.grain_size[outlifs_grain_idx] = res_tag_bitmap_create_info.nof_elements;
            extra_create_info.max_tag_value[outlifs_grain_idx] = 0;
        }

        if (dnx_data_lif.out_lif.feature_get(unit, dnx_data_lif_out_lif_data_entries_supported))
        {
            extra_create_info.grain_size[0] = dnx_data_lif.out_lif.data_entries_size_get(unit);
            extra_create_info.max_tag_value[0] = 1;
    ...
    if (_SHR_IS_FLAG_SET(create_data->flags, RESOURCE_TAG_BITMAP_CREATE_ALLOW_FORCING_TAG))
    {
        ...
        extra_create_info.max_tag_value[outlifs_grain_idx] = OUTLIF_RESOURCE_ILLEGAL_TAG;
    }
    ...
    if (create_data->first_element == 1)
    {
        ...
        alloc_info.count = 1;
        element = 0;
        SHR_IF_ERR_EXIT(resource_tag_bitmap_alloc(unit, node_id, *res_tag_bitmap, &alloc_info, &element));
    }
    ...
    int *element)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_REPLACE))
    {
        ...
        SHR_IF_ERR_EXIT(resource_tag_bitmap_advanced_free(unit, node_id, res_tag_bitmap, &free_info, *element));
    }
    ...
    void *extra_arguments)
{
    ...
    SHR_IF_ERR_EXIT(resource_tag_bitmap_free_several
                    (unit, node_id, res_tag_bitmap, nof_elements, element, &last_reference));
