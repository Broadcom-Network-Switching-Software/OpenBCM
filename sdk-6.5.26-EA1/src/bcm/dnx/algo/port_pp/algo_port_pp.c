 ...
 * }
 */
#define DNX_ESEM_CMD_VAR_LENGTH_MAX 100

#define SW_STATE_ESEM_COMMAND_PRINT(if_index, var_name) \
    sal_strncpy(str_name, var_name, DNX_ESEM_CMD_VAR_LENGTH_MAX - 1);\
    sal_strncat_s(str_name, " Valid", sizeof(str_name));\
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT8,\
                                   str_name, esem_cmd_data->esem[if_index].valid, NULL, NULL); \
    if (esem_cmd_data->esem[if_index].valid) \
    { \
        sal_strncpy(str_name, var_name, DNX_ESEM_CMD_VAR_LENGTH_MAX - 1);\
        sal_strncat_s(str_name, " app_db_id", sizeof(str_name));\
        SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT8,\
                                       str_name, esem_cmd_data->esem[if_index].app_db_id, NULL, NULL); \
        sal_strncpy(str_name, var_name, DNX_ESEM_CMD_VAR_LENGTH_MAX - 1);\
        sal_strncat_s(str_name, " designated_offset", sizeof(str_name));\
        SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT8,\
                                       str_name, esem_cmd_data->esem[if_index].designated_offset, NULL, NULL);  \
        sal_strncpy(str_name, var_name, DNX_ESEM_CMD_VAR_LENGTH_MAX - 1);\
        sal_strncat_s(str_name, " default_result_profile", sizeof(str_name));\
        SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT8,\
                                       str_name, esem_cmd_data->esem[if_index].default_result_profile, NULL, NULL); \
    }
    ...
    const void *data)
{
    ...
    char str_name[DNX_ESEM_CMD_VAR_LENGTH_MAX] = "";

    SW_STATE_PRETTY_PRINT_INIT_VARIABLES();
    SW_STATE_ESEM_COMMAND_PRINT(0, "access_1");
    SW_STATE_ESEM_COMMAND_PRINT(1, "access_2");
    SW_STATE_ESEM_COMMAND_PRINT(2, "access_3");
    ...
    int unit)
{
    ...
    sal_strncpy(template_data.name, DNX_ALGO_TEMPLATE_ESEM_ACCESS_CMD, DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    {
        ...
        extra_info.resource_create_info.grain_size[0] =
            dnx_data_esem.access_cmd.nof_cmds_get(unit) / DNX_ALGO_PP_PORT_ESEM_CMD_RES_NOF_TAGS;
        extra_info.resource_create_info.max_tag_value[0] = extra_info.resource_create_info.grain_size[0];
