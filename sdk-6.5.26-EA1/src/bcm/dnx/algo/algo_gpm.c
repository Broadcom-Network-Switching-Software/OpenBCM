    ...
    if ((_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS)) &&
        ((BCM_GPORT_IS_TUNNEL(gport) ||
          (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
           && BCM_GPORT_IS_FLOW_LIF(gport))) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_POINTED(gport)))
    {
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    if ((BCM_GPORT_IS_VLAN_PORT(gport) ||
         (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
          && BCM_GPORT_IS_FLOW_LIF(gport))) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS_NATIVE(gport))
    {
        /*
        ...
        DNXCMN_CORES_ITER(unit, core_index_iterate, core_idx)
        {
            ...
            else
            {
                ...
                rv = dnx_flow_ingress_virtual_local_lif_get(unit,
                                                            BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE_GET(gport),
                                                            core_idx,
                                                            &(gport_hw_resources->
    ...
    bcm_gport_t * flow_gport)
{
    ...
    else
    {
        if (BCM_GPORT_IS_VLAN_PORT(in_gport))
        {
            ...
            {
                *flow_gport = BCM_GPORT_SUB_TYPE_L3_VLAN_TRANSLATION_GET(in_gport);
            }
        }
    ...
    else if (out_gport_type == BCM_GPORT_VLAN_PORT)
    {
        ...
        else if (gport_subtype == BCM_GPORT_SUB_TYPE_OUT_LIF)
        {
            ...
            BCM_GPORT_SUB_TYPE_LIF_SET(*out_gport, exc_bits, global_lif_or_virtual);
                 ...
                 (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE))
        {
