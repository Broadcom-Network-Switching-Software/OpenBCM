    ...
    uint32 alloc_flags)
{
    ...
    init_info.expected_nof_elements = create_data->nof_profiles;
    ...
    DNX_SW_STATE_MEMSET(unit, node_id, (*multi_set)->head_ids, 0, 0x00,
                        create_data->nof_profiles * (sizeof(uint32)), 0, "sw_state multi_set head id map");
    ...
    uint8 *first_appear)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_hash_table_entry_lookup
                    (unit, node_id, multi_set->hash_table, (SW_STATE_HASH_TABLE_KEY *) data, NULL, &ll_head_id,
                     &exist));
    if (exist != TRUE)
    {
        /*
    ...
    *last_appear = (ref_counter == 0) ? TRUE : FALSE;
    ...
    if (*last_appear)
    {
        ...
        if (is_last_node == TRUE)
        {
             ...
             */
            SHR_IF_ERR_EXIT(sw_state_hash_table_entry_remove_by_index
    ...
    void *data)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_hash_table_get_by_index
                    (unit, node_id, multi_set->hash_table, ll_head_id, (SW_STATE_HASH_TABLE_KEY *) data, &found));
