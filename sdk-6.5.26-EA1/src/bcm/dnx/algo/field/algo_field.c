    ...
    int *cs_prodfile_id_p)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.
                    external_tcam_opcode_cs_profile_id.allocate_single(unit, alloc_flags, NULL, cs_prodfile_id_p));
...
/**see H file: algo_field.h */
shr_error_e
dnx_algo_field_external_tcam_opcode_cs_profile_id_deallocate(
    int unit,
    int cs_prodfile_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.external_tcam_opcode_cs_profile_id.free_single(unit, cs_prodfile_id, NULL));
...
*/
static shr_error_e
dnx_algo_field_res_mngr_external_tcam_opcode_cs_profile_id_init(
    int unit)
{
    dnx_algo_res_create_data_t data;
    SHR_FUNC_INIT_VARS(unit);
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_EXTERNAL_TCAM_OPCODE_CS_PROFILE_ID, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH);

    SHR_IF_ERR_EXIT(algo_field_info_sw.external_tcam_opcode_cs_profile_id.create(unit, &data, NULL));
    ...
    int unit)
{
    ...
    sal_memset(&data, 0, sizeof(data));
    data.first_element = 0;
    data.nof_elements = dnx_data_field.exem_learn_flush_machine.nof_flush_profiles_get(unit);
    data.flags = 0;
...
/**
* \brief
*  Init simple allocation algorithm for APP_DB_ID for all EXEM physical databases.
* \param [in] unit  - Device Id
* \return
*   shr_error_e - Error Type
* \remark
...
*/
static shr_error_e
dnx_algo_field_res_mngr_exem_init(
    int unit)
{
    dnx_field_stage_e field_stage_iter;
    uint8 physical_database_set[DBAL_NOF_PHYSICAL_TABLES] = { 0 };

    SHR_FUNC_INIT_VARS(unit);

    DNX_FIELD_STAGE_ITERATOR(field_stage_iter)
    {
        dbal_physical_tables_e exem_pdb = dnx_data_field.stage.stage_info_get(unit, field_stage_iter)->exem_pdb;
        if (((int) exem_pdb < 0) || (exem_pdb >= DBAL_NOF_PHYSICAL_TABLES))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "exem_pdb %d must be 0-%d.\r\n", exem_pdb, DBAL_NOF_PHYSICAL_TABLES - 1);
        }
        if (exem_pdb != DBAL_PHYSICAL_TABLE_NONE)
        {
            if (physical_database_set[exem_pdb])
            {
                /*
                 * Do not init for the same PDB twice.
                 */
                continue;
            }
            else
            {
                physical_database_set[exem_pdb] = TRUE;
            }
        }
    ...
    int unit)
{
    ...
    /**EXEM APP_DB_ID*/
    SHR_IF_ERR_EXIT(dnx_algo_field_res_mngr_exem_init(unit));
    ...
    /**EXTERNAL_TCAM_CS profile*/
    SHR_IF_ERR_EXIT(dnx_algo_field_res_mngr_external_tcam_opcode_cs_profile_id_init(unit));
    /**iPMF2 Flush profile*/
...
*/
shr_error_e
dnx_algo_field_external_tcam_opcode_cs_profile_id_is_allocated(
    int unit,
    int cs_prodfile_id,
    uint8 *is_allocated_p)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.
                    external_tcam_opcode_cs_profile_id.is_allocated(unit, cs_prodfile_id, is_allocated_p));
