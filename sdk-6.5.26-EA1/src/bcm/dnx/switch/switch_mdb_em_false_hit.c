    ...
    int unit)
{
    ...
    {
        if (mdb_em_age_supported_internal(unit, mdb_physical_table_id_iter) == TRUE)
        {
            uint32 entry_handle_table_22;

            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                            (unit,
                             dnx_data_mdb.em.mdb_emp_tables_mapping_get(unit, mdb_physical_table_id_iter)->mdb_22_table,
    ...
    for (dh_iter = 0; dh_iter < nof_macros; dh_iter++)
    {
        int dh_iter_inner;
        int dh_skip = FALSE;
        uint32 active_dhs_bitmap = 0;
        mdb_physical_table_e mdb_physical_table_id =
            dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dh_overwrite_dbal_table_ids[dh_iter])->logical_to_physical;
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, dnx_data_mdb.direct.mdb_26_info_get(unit, mdb_physical_table_id)->dbal_table,
                         &entry_handle_id));

        for (dh_iter_inner = 0; dh_iter_inner < nof_macros; dh_iter_inner++)
        {
            if (dh_overwrite_dbal_table_ids[dh_iter_inner] == dh_overwrite_dbal_table_ids[dh_iter])
            {
        ...
        mdb_db_type = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dh_overwrite_dbal_table_ids[dh_iter])->db_type;
        ...
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                            (unit, dnx_data_mdb.direct.mdb_11_info_get(unit, mdb_physical_table_id)->dbal_table,
                             &entry_handle_id));
    ...
    for (global_macro_idx = 0; global_macro_idx < nof_macros; global_macro_idx++)
    {
        int cluster_idx;
        mdb_macro_types_e macro_type;
        int macro_local_idx;
        mdb_physical_table_e mdb_physical_table_id = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit,
                                                                                         dh_overwrite_dbal_table_ids
                                                                                         [global_macro_idx])->
            logical_to_physical;
    ...
    for (dh_iter = 0; dh_iter < nof_macros; dh_iter++)
    {
        ...
        dbal_entry_key_field32_set(unit, entry_handle_mdb_table_abk, DBAL_FIELD_MDB_KEY_0, dh_iter);

        cluster_interface_id = dnx_data_mdb.dh.dh_info_get(unit, mdb_physical_table_id)->macro_mapping[dh_iter];
        ...
        for (cluster_iter = 0; cluster_iter < max_nof_macro_clusters; cluster_iter++)
        {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_mdb_table_abk, DBAL_FIELD_MDB_ITEM_0, cluster_iter, 0x0);
    ...
    int unit)
{
    ...
    for (ii = 0; ii < DBAL_NOF_ENUM_VT2_CONTEXT_ID_VALUES; ii++)
    {
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_VT2_CONTEXT_ID, ii, &is_illegal_value));
        if (is_illegal_value)
        {
            continue;
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VT2_CONTEXT_ID, ii);
    ...
    for (ii = 0; ii < DBAL_NOF_ENUM_VT3_CONTEXT_ID_VALUES; ii++)
    {
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_VT3_CONTEXT_ID, ii, &is_illegal_value));
        if (is_illegal_value)
        {
            continue;
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VT3_CONTEXT_ID, ii);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LIF_ARR_RESOLUTION_ENABLE, INST_SINGLE, TRUE);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    for (ii = 0; ii < DBAL_NOF_ENUM_VT4_CONTEXT_ID_VALUES; ii++)
    {
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_VT4_CONTEXT_ID, ii, &is_illegal_value));
        if (is_illegal_value)
        {
            continue;
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VT4_CONTEXT_ID, ii);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LIF_ARR_RESOLUTION_ENABLE, INST_SINGLE, TRUE);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }

    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_INGRESS_IRPP_VT5_CONTEXT_PROPERTIES, entry_handle_id));
    for (ii = 0; ii < DBAL_NOF_ENUM_VT5_CONTEXT_ID_VALUES; ii++)
    {
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_VT5_CONTEXT_ID, ii, &is_illegal_value));
        if (is_illegal_value)
        {
            continue;
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VT5_CONTEXT_ID, ii);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LIF_ARR_RESOLUTION_ENABLE, INST_SINGLE, TRUE);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    for (ii = 0; ii < DBAL_NOF_ENUM_FWD1_CONTEXT_ID_VALUES; ii++)
    {
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_FWD1_CONTEXT_ID, ii, &is_illegal_value));
        if (is_illegal_value)
        {
            continue;
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FWD1_CONTEXT_ID, ii);

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARNING_KEY_CONSTRUCTION_ENABLE, INST_SINGLE,
                                     0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARNING_FIXED_KEY_ENABLE, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS_LAYER_LB_KEY_CALCULATIONS_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOAD_BALANCING_KEY_CONSTRUCTION_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHERNET_SA_MC_TRAP_ENABLE, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHERNET_SA_EQUAL_DA_TRAP_ENABLE, INST_SINGLE,
                                     0);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_UNSPECIFIED_DESTINATION_TRAP_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_LOOPBACK_ADDRESS_TRAP_ENABLE, INST_SINGLE,
                                     0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_MULTICAST_SOURCE_TRAP_ENABLE, INST_SINGLE,
                                     0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_NEXT_HEADER_NULL_TRAP_ENABLE, INST_SINGLE,
                                     0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_UNSPECIFIED_SOURCE_TRAP_ENABLE, INST_SINGLE,
                                     0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_LOCAL_LINK_DESTINATION_TRAP_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_LOCAL_SITE_DESTINATION_TRAP_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_LOCAL_LINK_SOURCE_TRAP_DEST_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_LOCAL_SITE_SOURCE_TRAP_ENABLE, INST_SINGLE,
                                     0);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_IPV4_MAPPED_DESTINATION_TRAP_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_MULTICAST_DESTINATION_TRAP_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TCP_SN_FLAGS_ZERO_TRAP_ENABLE, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TCP_SN_ZERO_FLAGS_SET_TRAP_ENABLE, INST_SINGLE,
                                     0);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TCP_FRAGMENT_INCOMPLETE_HEADER_TRAP_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TCP_FRAGMENT_OFFSET_LT_8_TRAP_ENABLE,
                                     INST_SINGLE, 0);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FCOE_FABRIC_PROVIDED_SECURITY_ENABLE,
                                     INST_SINGLE, 0);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COMPATIBLE_MC_BRIDGE_FALLBACK_ENABLE,
                                     INST_SINGLE, 0);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IEEE_1588_TCAM_IDENTIFICATION_ENABLE,
                                     INST_SINGLE, 0);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHERNET_OAM_IDENTIFICATION_ENABLE, INST_SINGLE,
                                     0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS_TP_OR_BFD_IDENTIFICATION_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MULTI_HOP_BFD_IDENTIFICATION_ENABLE, INST_SINGLE,
                                     0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ONE_HOP_BFD_IDENTIFICATION_ENABLE, INST_SINGLE,
                                     0);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FORWARDING_LOOKUP_STATISTICS_ENABLE, INST_SINGLE,
                                     0);
    ...
    for (ii = 0; ii < DBAL_NOF_ENUM_FWD2_CONTEXT_ID_VALUES; ii++)
    {
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_FWD2_CONTEXT_ID, ii, &is_illegal_value));
        if (is_illegal_value)
        {
            continue;
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FWD2_CONTEXT_ID, ii);
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_FWD2_CONTEXT_ID, DBAL_RANGE_ALL,
                                         DBAL_RANGE_ALL);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FWD_CASCADED_PRIORITY_DECODER_ENABLE,
                                     INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RPF_CASCADED_PRIORITY_DECODER_ENABLE,
                                     INST_SINGLE, 0);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }

    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_INGRESS_IRPP_VT1_CONTEXT_PROPERTIES, entry_handle_id));
    for (ii = 0; ii < DBAL_NOF_ENUM_VT1_CONTEXT_ID_VALUES; ii++)
    {
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_VT1_CONTEXT_ID, ii, &is_illegal_value));
        if (is_illegal_value)
        {
            continue;
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VT1_CONTEXT_ID, ii);
