...
#define FLEXE_STD_GEN2_MAX_MC_CH                                     (4)
/* } */
    ...
    soc_pbmp_t flexe_core_port_bmp)
{
    ...
    if (flags & BCM_PORT_FLEXE_RX)
    {
         ...
         */
        FLEXE_STD_GEN2_IF_ERR_EXIT(j2x_sal_framer_subsys_flexe_adapter_flexe_group_cfg
                                   (unit, group_index, instance_bmp[0], instance_num_array, instance_mode, 0, 0, 0));
    }
    if (flags & BCM_PORT_FLEXE_TX)
    {
        /*
         ...
         */
        FLEXE_STD_GEN2_IF_ERR_EXIT(j2x_sal_framer_subsys_flexe_adapter_flexe_group_cfg
                                   (unit, group_index, instance_bmp[0], instance_num_array, instance_mode, 0, 1, 0));
        /*
         * Configure overhead
         */
        SHR_BIT_ITER(instance_bmp, dnx_data_nif.flexe.nof_flexe_instances_get(unit), instance_id)
...
{
    int i;
    int dst_block[FLEXE_STD_GEN2_MAX_MC_CH];
    int dst_channel[FLEXE_STD_GEN2_MAX_MC_CH];

    SHR_FUNC_INIT_VARS(unit);

    FLEXE_STD_GEN2_IF_ERR_EXIT(j2x_xx_api_cross_info_query
                               (unit, src_info->block_id, src_info->channel, (J2X_U32 *) dst_block,
                                (J2X_U32 *) dst_channel));

    *nof_dest = 0;
    for (i = 0; i < FLEXE_STD_GEN2_MAX_MC_CH; ++i)
    {
