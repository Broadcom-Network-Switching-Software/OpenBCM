    ...
    uint32 flags)
{
    ...
    if (DNX_ALGO_PORT_FLEXE_TX_GET(flags))
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, TRUE, TRUE))
        {
            ...
            if (!has_tx_speed)
            {
                SHR_IF_ERR_EXIT(flexe_core_mac_client_delete(unit, client_channel, tx_speed));
            }
            else
            {
                SHR_IF_ERR_EXIT(flexe_core_mac_client_add(unit, client_channel, tx_speed));
            }
        }
        ...
        {
            /*
             * Configure rate adpt for L1 ETH ports
             */
    ...
    dnx_flexe_cal_info_t * cal_info)
{
     ...
     */
    if (DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info) || DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, TRUE))
    {
        /*
    ...
    for (i = 0; i < nof_pcs; i++)
    {
        ...
        if (i > 0)
        {
            ...
            SHR_VAL_VERIFY(phy_info[i].slot_mode, phy_info[0].slot_mode, _SHR_E_PARAM,
                           "All the physical port slot_mode should be the same.\n");
        }
        /*
    ...
    dnx_algo_flexe_flow_type_e * flow_type)
{
        ...
        DNX_ALGO_PORT_TYPE_IS_FLEXE_CLIENT(unit, dst_port_info))
    {
        *flow_type = DNX_ALGO_FLEXE_FLOW_CLIENT2CLENT;
             ...
             DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, dst_port_info, TRUE, FALSE))
    {
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, src_port_info, TRUE, FALSE) &&
             DNX_ALGO_PORT_TYPE_IS_FLEXE_CLIENT(unit, dst_port_info))
    {
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, src_port_info, FALSE, FALSE) &&
             ...
             DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC_L1(unit, dst_port_info) && (DNX_FLEXE_MODE_DISTRIBUTED == flexe_mode))
    {
        *flow_type = DNX_ALGO_FLEXE_FLOW_SAR2MAC;
             ...
             DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, dst_port_info, FALSE, TRUE) &&
    ...
    switch (*flow_type)
    {
        ...
        {
            max_channel =
                dnx_data_nif.sar_client.feature_get(unit,
                                                    dnx_data_nif_sar_client_connect_to_66bswitch) ? dnx_data_nif.
                framer.b66switch_backup_channels_get(unit) : 0;
            break;
        }
        default:
            max_channel = 0;
    ...
    int is_add)
{
    ...
    src_info.block_id = src_block;
    dst_info.block_id = dst_block;
    SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, src_port, &src_info.channel));
    SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, dst_port, &dst_info.channel));
    /*
    ...
    int is_add)
{
    ...
    dst_info.block_id = dst_block;
    /*
     * Get direction, the Rx and Tx are from switch perspective,
     * not SAR perspective.
     */
    ...
    {
        /*
         * Get the virtual FlexE channel
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_sar_virtual_flexe_channel_get(unit, dst_port, &dst_info.channel));
        /*
        ...
        if (flow_type != DNX_ALGO_FLEXE_FLOW_SAR2SAR)
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, src_port, &src_info.channel));
        }
    }
        ...
        (flow_type == DNX_ALGO_FLEXE_FLOW_SAR2ETH) || (flow_type == DNX_ALGO_FLEXE_FLOW_SAR2SAR))
    {
         ...
         * Get the virtual FlexE channel
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_sar_virtual_flexe_channel_get(unit, src_port, &src_info.channel));
        /*
         * Configure channel map
         */
    ...
    int is_add)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /*
     * Get src and dst block ID
     */
    src_block = dnx_data_nif.framer.b66switch_block_id_get(unit, flow_type, channel)->src_block_id;
    dst_block = dnx_data_nif.framer.b66switch_block_id_get(unit, flow_type, channel)->dst_block_id;
    ...
    switch (flow_type)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_port_flexe_gen2_simple_66bswitch_flow_set
                            (unit, src_port, dst_port, src_block, dst_block, is_add));
            break;
        }
        case DNX_ALGO_FLEXE_FLOW_CLIENT2SAR:
        case DNX_ALGO_FLEXE_FLOW_MAC2SAR:
        ...
        case DNX_ALGO_FLEXE_FLOW_SAR2SAR:
        {
            SHR_IF_ERR_EXIT(dnx_port_flexe_gen2_sar_66bswitch_flow_set
                            (unit, src_port, dst_port, src_block, dst_block, flow_type, is_add));
            break;
        }
        default:
        {
