    ...
    uint32 threshold)
{
    ...
    uint32 rmc_fifo_size, rmc_fifo_resolution, max_threshold, priority_group;
    ...
    SHR_IF_ERR_EXIT(dnx_port_link_list_db_double_priority_nof_sections_allocated_get
                    (unit, DNX_PORT_NIF_OFR_LINK_LIST, nif_interface_id, priority_group, &rmc_fifo_size));
    ...
    uint8 *is_single_port_on_profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF,
                     DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_ELK | DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FLEXE_PHY, &nif_pbmp));
    ...
    uint32 priority_group)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, port, 0, &nif_phy_port));

    SHR_IF_ERR_EXIT(dnx_ofr_prd_port_counter_hw_set(unit, counter, nif_phy_port, priority_group));
