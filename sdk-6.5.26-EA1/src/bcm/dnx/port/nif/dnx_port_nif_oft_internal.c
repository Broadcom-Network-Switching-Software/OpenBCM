    ...
    bcm_port_t logical_port)
{
    ...
    dnx_port_map_destination_e port_destination;
     ...
     */
    SHR_IF_ERR_EXIT(dnx_port_arb_map_destination_get(unit, logical_port, &port_destination));
     ...
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_BYPASS, INST_SINGLE,
                                 port_destination == DNX_PORT_ARB_MAP_DESTINATION_BYPASS);
...
shr_error_e
dnx_port_oft_enable_dynamic_memories_set(
    int unit,
    int enable)
{
    uint32 entry_handle_id;
     ...
     * alloc DBAL table handle
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_OFT_ENABLE_MEMORIES, &entry_handle_id));
