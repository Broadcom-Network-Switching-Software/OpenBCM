    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        {
            for (client_channel = 0; client_channel < dnx_data_nif.sar_client.nof_clients_get(unit); ++client_channel)
            {
    ...
    const uint64 *counter_val)
{
    ...
    SHR_IF_ERR_EXIT(imb_framer_internal_mac_access_control_set_er_journal_insert(unit, port));
 ...
 */
static int
imb_framer_port_mac_tx_thr_set(
    int unit,
    bcm_port_t port,
    int tx_thr)
{
    ...
    bcm_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(imb_framer_port_mac_tx_thr_set(unit, port, tx_thr));
    /*
     * Enable padding
     */
...
imb_framer_port_mac_speed_set(
    ...
    int speed)
{
    int speed_group = 0, num_credit = 0, tx_thr = 0;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(imb_framer_utils_mac_tx_speed_config_get(unit, speed, &speed_group, &num_credit, &tx_thr));
    SHR_IF_ERR_EXIT(imb_framer_mac_set(unit, port, DBAL_TABLE_TINY_MAC_CTRL, DBAL_FIELD_SPEED_GROUP, speed_group));
    SHR_IF_ERR_EXIT(imb_framer_mac_set(unit, port, DBAL_TABLE_TINY_MAC_CTRL, DBAL_FIELD_NUMB_CREDIT, num_credit));
    SHR_IF_ERR_EXIT(imb_framer_port_mac_tx_thr_set(unit, port, tx_thr));
exit:
    SHR_FUNC_EXIT;
}
    ...
    const portmod_speed_config_t * speed_config)
{
    ...
    {
        /*
         *de-asserting RX_EN and TX_EN and
         *asserting DISCARD and SOFT_RESET
         */
        SHR_IF_ERR_EXIT(imb_framer_port_mac_tx_enable_set(unit, port, 0));
        SHR_IF_ERR_EXIT(imb_framer_port_mac_rx_enable_set(unit, port, 0));
        SHR_IF_ERR_EXIT(imb_framer_port_mac_tx_discard_set(unit, port, 1));
        SHR_IF_ERR_EXIT(imb_framer_port_mac_ctrl_soft_reset_set(unit, port, 1));
        ...
        /*
         * De-assert the per-port SOFT_RESET,
         * and set RX_EN and TX_EN
         */
        SHR_IF_ERR_EXIT(imb_framer_port_mac_tx_discard_set(unit, port, 0));
        SHR_IF_ERR_EXIT(imb_framer_port_mac_ctrl_soft_reset_set(unit, port, 0));
        SHR_IF_ERR_EXIT(imb_framer_port_mac_tx_enable_set(unit, port, 1));
        SHR_IF_ERR_EXIT(imb_framer_port_mac_rx_enable_set(unit, port, 1));
    }
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_flexe_general_cal_build(unit, slots_per_client, max_nof_clients, max_cal_len, calendar));

    SHR_IF_ERR_EXIT(imb_framer_internal_calendar_config(unit, IMB_FRAMER_SB_RX_CALENDAR, max_cal_len, calendar));
exit:
    ...
    int unit)
{
    ...
    max_cal_len =
        (nof_special_clients >
         0) ? dnx_data_nif.mac_client.nof_normal_clients_get(unit) : dnx_data_nif.mac_client.nof_sb_cal_slots_get(unit);

    SHR_IF_ERR_EXIT(dnx_algo_flexe_general_cal_build(unit, slots_per_client, nof_flexe_clients, max_cal_len, calendar));
    /*
    ...
    SHR_IF_ERR_EXIT(imb_framer_internal_calendar_config(unit, IMB_FRAMER_SB_TX_CALENDAR, max_cal_len, calendar));
    /*
    ...
    uint32 flags)
{
    ...
    if (IMB_FLEXE_DIRECTION_RX_ONLY_GET(flags))
    {
         ...
         * SB RX calendar
         */
        SHR_IF_ERR_EXIT(imb_framer_sb_rx_calendar_set(unit));
    }
    ...
    if (IMB_FLEXE_DIRECTION_TX_ONLY_GET(flags))
    {
         ...
         * SB Tx calendar
         */
        SHR_IF_ERR_EXIT(imb_framer_sb_tx_calendar_set(unit));
    }
    /*
     * FEU calendar
     */
    if (IMB_FRAMER_FEU_IS_VALID(unit))
    {
        SHR_IF_ERR_EXIT(imb_feu_calendar_set(unit, port, flags));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    dnx_algo_flexe_link_list_fifo_type_e fifo_type)
{
    ...
    flags = (fifo_type == DNX_ALGO_FLEXE_FIFO_SB_RX) ? DNX_ALGO_PORT_SPEED_F_RX : DNX_ALGO_PORT_SPEED_F_TX;
    ...
    if (dnx_data_nif.mac_client.nof_special_clients_get(unit) > 0)
    {
        if (!DNX_ALGO_FLEXE_MAC_CHANNEL_IS_OAM(unit, client_channel))
        {
            ...
            {
                oam_speed = dnx_data_nif.mac_client.oam_channel_max_bandwidth_get(unit);
                SHR_IF_ERR_EXIT(imb_framer_internal_port_fifo_link_list_config_set
    ...
    uint32 flags)
{
    ...
    if (IMB_FLEXE_DIRECTION_RX_ONLY_GET(flags))
    {
        SHR_IF_ERR_EXIT(imb_framer_port_sb_fifo_resource_set(unit, port, DNX_ALGO_FLEXE_FIFO_SB_RX));
    }
    if (IMB_FLEXE_DIRECTION_TX_ONLY_GET(flags)
        && dnx_data_nif.mac_client.feature_get(unit, dnx_data_nif_mac_client_sb_tx_fifo_supported))
    {
        SHR_IF_ERR_EXIT(imb_framer_port_sb_fifo_resource_set(unit, port, DNX_ALGO_FLEXE_FIFO_SB_TX));
    }
    if (IMB_FRAMER_FEU_IS_VALID(unit))
    {
    ...
    uint32 val)
{
    ...
    DNX_ERR_RECOVERY_EXPR_SUPPRESS_IF_ERR_EXIT(imb_framer_internal_mac_access_control_set(unit, port));
    SHR_IF_ERR_EXIT(imb_framer_internal_mac_set(unit, table, field, val));
    SHR_IF_ERR_EXIT(imb_framer_internal_mac_access_control_set_er_journal_insert(unit, port));
exit:
    SHR_IF_ERR_CONT(dnx_algo_flexe_tiny_mac_access_lock_give(unit));
    SHR_FUNC_EXIT;
    ...
    if (IMB_FLEXE_DIRECTION_RX_ONLY_GET(flags))
    {
        ...
        if ((dnx_data_nif.mac_client.nof_special_clients_get(unit) > 0))
        {
            if ((nof_ports == 1) && !DNX_ALGO_FLEXE_MAC_CHANNEL_IS_OAM(unit, client_channel))
            {
                 ...
                 */
                speed = dnx_data_nif.mac_client.oam_channel_max_bandwidth_get(unit);
                oam_channel = dnx_data_nif.mac_client.oam_client_channel_get(unit);
                SHR_IF_ERR_EXIT(imb_framer_internal_port_fifo_link_list_config_set
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, TRUE, TRUE))
    {
        ...
        if (IMB_FLEXE_DIRECTION_TX_ONLY_GET(flags))
        {
            DNX_ERR_RECOVERY_EXPR_SUPPRESS_IF_ERR_EXIT(flexe_core_mac_client_delete(unit, client_channel, 0));
        }
        /*
    ...
    else
    {
        ...
        /*
         * Clear virtual FlexE channel
         */
        if (!dnx_data_nif.sar_client.feature_get(unit, dnx_data_nif_sar_client_connect_to_66bswitch))
        {
            SHR_IF_ERR_EXIT(dnx_algo_flexe_shared_channel_free(unit, bcmPortFramerModuleVirtualFlexe, client_channel));
        }
    }
    ...
    int unit)
{
    ...
    granularity = dnx_data_nif.mac_client.speed_granularity_get(unit);
