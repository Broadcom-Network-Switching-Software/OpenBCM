 ...
 */
static shr_error_e
imb_otnu_phy_port_portmod_port_add_er_journal_insert(
    int unit,
    int port)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_entry_new(unit,
                                                        ...
                                                        sizeof(int),
                                                        (uint8 *) &port,
                                                        sizeof(int), &imb_otnu_phy_port_portmod_port_add_er_rollback,
                                                        FALSE));
exit:
    SHR_FUNC_EXIT;
}
 ...
 */
static shr_error_e
imb_otnu_phy_port_portmod_port_remove_er_journal_insert(
    int unit,
    int port)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    int pam4_lane_order)
{
    ...
    if (flags & IMB_COMMON_DIRECTION_RX)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PAM4_ENABLE, INST_SINGLE, pam4_lane_order);
    ...
    bcm_port_t port)
{
    ...
    int i, nof_phys, mlane_enabled = 0;
    ...
    SHR_IF_ERR_EXIT(imb_otnu_phy_port_mlane_group_enable_set(unit, port, nof_phys, phy_lanes, 1, &mlane_enabled));
...
exit:
    if (mlane_enabled)
    {
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_mlane_group_enable_set(unit, port, nof_phys, phy_lanes, 0, &mlane_enabled));
    }
    SHR_FUNC_EXIT;
}
...
imb_otnu_phy_port_reset_set(
    ...
    int in_reset)
{
    int pm_id, pm_lane, i, nof_lanes, mlane_enabled = 0;
    ...
    SHR_IF_ERR_EXIT(imb_otnu_phy_port_mlane_group_enable_set(unit, port, nof_lanes, lanes, 1, &mlane_enabled));
    for (i = 0; i < nof_lanes; i++)
    {
...
exit:
    if (mlane_enabled)
    {
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_mlane_group_enable_set(unit, port, nof_lanes, lanes, 0, &mlane_enabled));
    }
    SHR_FUNC_EXIT;
}
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(imb_otnu_phy_port_portmod_port_add_er_journal_insert(unit, port));
    ...
    bcm_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(imb_otnu_phy_port_portmod_port_remove_er_journal_insert(unit, port));
