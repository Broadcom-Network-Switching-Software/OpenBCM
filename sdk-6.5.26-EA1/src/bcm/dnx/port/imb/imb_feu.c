    ...
    int unit)
{
    ...
    _SHR_PBMP_ITER(logical_ports, logical_port)
    {
        ...
        else
        {
            ...
            slots_per_client[client_channel] = speed / dnx_data_nif.mac_client.speed_granularity_get(unit);
        }
    }
                ...
                else
                {
                        ...
                        && (rmc.sch_priority != bcmPortNifSchedulerLow))
                    {
                        ...
                        slots_per_hi_or_esb_rmc[rmc.rmc_id] =
                            speed / dnx_data_nif.mac_client.speed_granularity_get(unit);
                    }
                    else
                    {
                        /*
                         ...
                         * If rmc_cal_per_priority is disabled, it is for all the RMC slot
                         */
                        slots_per_rmc[rmc.rmc_id] = speed / dnx_data_nif.mac_client.speed_granularity_get(unit);
                    }
                }
            }
        }
    ...
    const bcm_port_prio_config_t * priority_config)
{
    ...
    if (has_speed)
    {
        ...
        new_nof_entries = speed / dnx_data_nif.mac_client.speed_granularity_get(unit);
        /*
