    ...
    int unit)
{
    ...
    bcm_port_t port;
    uint32 index, imb_id, nof_types = 0, nof_ethus = 0, nof_ilkn_units = 0, nof_fabric_pms = 0, nof_imbs = 0;
    uint32 nof_framer_imbs = 0, nof_feu_phy_imbs = 0, nof_otnu_phy_imbs = 0;
    ...
    {
        nof_framer_imbs = dnx_data_nif.framer.nof_units_get(unit);
        nof_feu_phy_imbs = dnx_data_nif.framer.nof_units_get(unit);
    }
    if (dnx_data_nif.otn.feature_get(unit, dnx_data_nif_otn_is_supported))
    {
        nof_otnu_phy_imbs = dnx_data_nif.otn.nof_units_get(unit);
    }
    nof_imbs = nof_ethus + nof_ilkn_units + nof_fabric_pms + nof_framer_imbs + nof_feu_phy_imbs + nof_otnu_phy_imbs;
    ...
    if (!dnx_data_dev_init.general.access_only_get(unit) || dnx_data_dev_init.general.heat_up_get(unit))
    {
        ...
        /*
         * Init all Framer units
         */
        for (index = 0; index < nof_framer_imbs; ++index)
        {
            SHR_IF_ERR_EXIT(imbm.imb.get(unit, imb_id, &imb));
            imb.type = imbDispatchTypeImb_framer;
            imb.inst_id = index;
            SHR_IF_ERR_EXIT(imb_init(unit, &imb, &imb.imb_specific_info));
            SHR_IF_ERR_EXIT(imbm.imb.set(unit, imb_id, imb));
            ++imb_id;
         ...
         * Init all OTNU units
         */
        for (index = 0; index < nof_otnu_phy_imbs; ++index)
        {
            SHR_IF_ERR_EXIT(imbm.imb.get(unit, imb_id, &imb));
            imb.type = imbDispatchTypeImb_otnu_phy;
            imb.inst_id = index;
            SHR_IF_ERR_EXIT(imb_init(unit, &imb, &imb.imb_specific_info));
            SHR_IF_ERR_EXIT(imbm.imb.set(unit, imb_id, imb));
            ++imb_id;
...
imb_deinit_all(
    int unit)
{
    int index, nof_ethus = 0, nof_ilkn_units = 0, nof_imbs, nof_fabric_pms = 0;
    int nof_framer_imbs = 0, nof_feu_phy_imbs = 0, nof_otnu_phy_imbs = 0;
    imb_create_info_t imb;
    uint8 is_init = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    uint32 flags)
{
    ...
    if (IMB_PORT_ADD_F_SKIP_SETTINGS_GET(flags) == FALSE)
    {
        ...
        {
            imb_type = imbDispatchTypeImb_otnu_phy;
        }
        else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_CLIENT(unit, port_info))
        {
            imb_type = imbDispatchTypeImb_framer;
        }
#endif
    ...
    uint32 enable)
{
    ...
    {
        SHR_IF_ERR_EXIT(imbm.imb.get(unit, index, &imb));
        if (__imb__dispatch__[imb.type]->f_imb_pfc_mask_on_link_fail_enable_set != NULL)
        {
            SHR_IF_ERR_EXIT(__imb__dispatch__[imb.type]->f_imb_pfc_mask_on_link_fail_enable_set(unit, &imb, enable));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    uint8 is_continuous)
{
    ...
    {
        SHR_IF_ERR_EXIT(imbm.imb.get(unit, imb_id, &imb));
        if (__imb__dispatch__[imb.type]->f_imb_instru_synced_counters_config_set != NULL)
        {
            SHR_IF_ERR_EXIT(__imb__dispatch__[imb.type]->f_imb_instru_synced_counters_config_set
                            (unit, imb_id, nof_intervals, is_continuous));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int bring_back_up)
{
    ...
    for (imb_id = 0; imb_id < nof_imbs; ++imb_id)
    {
        ...
        {
            SHR_IF_ERR_EXIT(__imb__dispatch__[imb.type]->f_imb_instru_counters_reset(unit, imb_id, bring_back_up));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(imbm.imb.get(unit, index, &imb));
        if (__imb__dispatch__[imb.type]->f_imb_instru_traffic_counters_reset != NULL)
        {
            SHR_IF_ERR_EXIT(__imb__dispatch__[imb.type]->f_imb_instru_traffic_counters_reset(unit, &imb));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(imbm.imb.get(unit, index, &imb));
        if (__imb__dispatch__[imb.type]->f_imb_instru_pfc_deadlock_counters_reset != NULL)
        {
            SHR_IF_ERR_EXIT(__imb__dispatch__[imb.type]->f_imb_instru_pfc_deadlock_counters_reset(unit, &imb));
        }
    }
exit:
    SHR_FUNC_EXIT;
