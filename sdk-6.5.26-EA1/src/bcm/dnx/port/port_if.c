    ...
    bcm_port_resource_t * resource)
{
    ...
    nof_logical_lanes = DNXC_PORT_PHY_SPEED_IS_PAM4(resource->speed) ? nof_phys * 2 : nof_phys;
    ...
    bcm_port_resource_t * resource)
{
    ...
    int num_of_lanes, data_rate, mgmt_ports_counter = 0;
    dnx_algo_port_info_s port_info;
    bcm_pbmp_t all_nif_ports;
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF,
                     DNX_ALGO_PORT_LOGICALS_F_MASTER_ONLY, &all_nif_ports));
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, L1)))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_ilkn_serdes_rate_to_data_rate_conv
                            (unit, logical_port, resource[resource_index].speed, num_of_lanes, &data_rate));
            current_set_speeds += data_rate;
        }
        else if (!DNX_ALGO_PORT_TYPE_IS_FABRIC(unit, port_info))
        {
            current_set_speeds += resource[resource_index].speed;
        }

        _SHR_PBMP_PORT_REMOVE(all_nif_ports, logical_port);
    }

    /*
     * Summing the speed of all the other ports which reconfigure in resource_set.
     */
    _SHR_PBMP_ITER(all_nif_ports, logical_port)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, logical_port, &port_info));
         ...
         */
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_MGMT(logical_port, port_info))
        {
            ++mgmt_ports_counter;
            if (mgmt_ports_counter > dnx_data_nif.global.max_nof_mgmt_ports_get(unit) ||
                speed > dnx_data_nif.global.single_mgmt_port_capacity_get(unit))
            {
    ...
    if (current_set_speeds + aggregated_capacity > dnx_data_nif.global.total_data_capacity_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Exceeded speed limit by %dG. Current session tries to set %dG",
                     (current_set_speeds + aggregated_capacity -
                      dnx_data_nif.global.total_data_capacity_get(unit)) /1000, current_set_speeds / 1000);
    }
    ...
    bcm_port_resource_t * resource)
{
    ...
    int num_of_lanes, is_flr_fw_support;
    uint64 u64_calc;
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_IMB(unit, port_info, FALSE))
        {
             ...
             */
            if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, MGMT, OTN_PHY))
            ...
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, L1, MGMT))
                || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info) || DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
            {
        ...
        else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_CLIENT(unit, port_info))
        {
        ...
        if (is_flexe_enabled && DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, MGMT)))
        {
        ...
        if (is_flexe_enabled && DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, TRUE, FALSE))
        {
    ...
    dnx_port_resource_port_info_t * ports_info)
{
    ...
    /**
     * this loop should contain only per interface code,
     * Configure the OTN lane map.
     */
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        if (DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, ports_info[resource_index].port_info))
        {
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index].port_info,
            ...
            || DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, ports_info[resource_index].port_info, TRUE, FALSE)
    ...
    dnx_port_resource_port_info_t * ports_info)
{
    ...
    if (!dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        ...
        for (resource_index = 0; resource_index < nport; ++resource_index)
        {
            if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, ports_info[resource_index].port_info,
 ...
 *   to make sure we are not going ober the max limit of total egress interface credits
 */
static shr_error_e
    ...
    bcm_port_resource_t * resource)
{
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, resource[resource_index].port, &core));
            SHR_IF_ERR_EXIT(dnx_algo_port_interface_offset_get(unit, resource[resource_index].port, &interface_offset));
    ...
    /**
     * The below code is anly relevant for RCY interfaces. It is used to determine if RCY ports previously existed on
            ...
            || dnx_data_nif.oft.feature_get(unit, dnx_data_nif_oft_is_supported))
        {
            if (ports_info[resource_index].is_tx_speed_change &&
                (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index].port_info, 0)
                 || DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, ports_info[resource_index].port_info, 0, 0)))
            {
...
/**
 * \brief - verify parameters for dnx_port_l1_eth_transcode_set
 */
static shr_error_e
dnx_port_l1_eth_transcode_verify(
    int unit,
    int port,
    bcm_port_control_otn_map_type_t map_type)
{
    ...
    /** Verify map_type range */
    SHR_RANGE_VERIFY(map_type, 0, bcmPortControlOtnMapCount - 1, _SHR_E_PARAM, "map type is out of range.\n");
    ...
    if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, TRUE) &&
        !dnx_data_nif.framer.feature_get(unit, dnx_data_nif_framer_l1_eth_supported))
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "Wrong port type, only L1 ETH port type is supported!\r\n");
    }

    /** There is no GFP for 200/400G */
    SHR_IF_ERR_EXIT(dnx_algo_port_speed_get(unit, port, 0, &speed));
    if ((map_type == bcmPortControlOtnMapGfp) && (speed >= 2000000))
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "There is no GFP if speed is great than 200G!\r\n");
    }
exit:
    SHR_FUNC_EXIT;
}
/**
 * \brief - Configure L1 ETH transcode
 */
static shr_error_e
dnx_port_l1_eth_transcode_set(
    int unit,
    int port,
    bcm_port_control_t type,
    bcm_port_control_otn_map_type_t map_type)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_l1_eth_transcode_verify(unit, port, map_type));

    is_rx = (type == bcmPortControlL1EthToOtnMapTranscodeType) ? 1 : 0;
    /** call imb layer  */
    SHR_IF_ERR_EXIT(imb_port_l1_eth_transcode_set(unit, port, is_rx, map_type, 1));
    ...
    int is_set)
{
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, 1, 1))
    {
    ...
    int *start_tx)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    if (!DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, 1, 0))
    {
    ...
    int value)
{
    ...
    switch (type)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_port_l1_eth_transcode_set(unit, port, type, value));
            break;
        }
 ...
 * \param [in] is_set - Shows if verify function is called from set or get
 *
 * \return
 *   See shr_error_r
 * \remark
...
dnx_port_phy_tx_verify(
    ...
    int is_set)
{
    uint32 serdes_tx_tap_pre3_supported = dnx_data_port.static_add.feature_get(unit,
                                                                               dnx_data_port_static_add_serdes_tx_taps_pre3_supported_instead_post3);
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (is_set)
    {
        if (serdes_tx_tap_pre3_supported && tx->post3 != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "port %d: TX FIR parameter post3 is not supported but is given a value of %d", port,
                         tx->post3);
        }
        else if (!serdes_tx_tap_pre3_supported && tx->pre3 != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    bcm_port_phy_tx_t * tx)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_port_phy_tx_verify(unit, port, tx, 1));
    /** get logical port*/
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get(unit,
                                                    port,
                                                    DNX_ALGO_GPM_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info));
    ...
    bcm_port_phy_tx_t * tx)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_port_phy_tx_verify(unit, port, tx, 0));
    /** get logical port*/
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
                    (unit, port, DNX_ALGO_GPM_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info));
    ...
    int is_set)
{
    ...
    if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, MGMT))
        && !DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info))
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "FLR is only supported on ETH ports and FlexE physical interface.\r\n");
    }
    if (is_set)
    {
        /*
