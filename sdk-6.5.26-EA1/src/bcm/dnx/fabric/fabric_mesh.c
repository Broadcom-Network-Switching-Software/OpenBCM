    ...
    int enable)
{
    ...
    if (min_dest_modid > min_local_modid)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FABRIC_MESH_TOPOLOGY_LINKS, &fabric_links_entry_handle_id));
        for (i_link = 0; i_link < links_count; ++i_link)
        {
            int fabric_link;
            SHR_IF_ERR_EXIT(dnx_algo_port_fabric_link_get(unit, links_array[i_link], &fabric_link));
            ...
            dbal_entry_value_field32_set(unit, fabric_links_entry_handle_id, DBAL_FIELD_ENABLE, INST_SINGLE,
                                         enable ? 0 : 1);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, fabric_links_entry_handle_id, DBAL_COMMIT));
        }

        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FABRIC_MESH_TOPOLOGY_LINKS, fabric_links_entry_handle_id));
        is_empty = 1;
        for (i_link = 0; i_link < nof_links; ++i_link)
        {
            uint32 is_enabled = 0;
            ...
            dbal_value_field32_request(unit, fabric_links_entry_handle_id, DBAL_FIELD_ENABLE, INST_SINGLE, &is_enabled);
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FABRIC_MESH_TOPOLOGY_INIT, fabric_links_entry_handle_id));
 ...
 * \param [in] destination - The destination id.
 * \param [in] links_count - Number of links in links_array.
 * \param [in] links_array - Array of logical ports or fabric gports to be mapped to local_dest_id.
 * \return
 *   See \ref shr_error_e
 */
int
    ...
    bcm_module_t * destid_array)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_fabric_multicast_set_verify(unit, mc_id, flags, destid_count, destid_array));
    /*
    ...
    bcm_module_t * destid_array)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_fabric_multicast_get_verify(unit, mc_id, flags, destid_count_max, destid_count,
                                                           destid_array));
    /*
