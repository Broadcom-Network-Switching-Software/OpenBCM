...
/**
 * \brief
 *   Put relevant FMAC4s of a given FSRD in reset.
 * \param [in] unit -
 *   The unit number.
 * \param [in] fsrd_block -
 *   FSRD id.
 ...
 */
static shr_error_e
dnx_fabric_if_fmacs_in_fsrd_reset(
    int unit,
    int fsrd_block,
    int in_reset)
{
    int fmac_block_idx, fsrd_first_fmac_block;

    SHR_FUNC_INIT_VARS(unit);

    fsrd_first_fmac_block = fsrd_block * dnx_data_fabric.blocks.nof_fmacs_in_fsrd_get(unit);

    for (fmac_block_idx = fsrd_first_fmac_block;
         fmac_block_idx < fsrd_first_fmac_block + dnx_data_fabric.blocks.nof_fmacs_in_fsrd_get(unit); fmac_block_idx++)
    {
        SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_reset(unit, fmac_block_idx, in_reset));
    }
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief
 *   Enable/disable a FSRD and the coressponding FMAC blocks.
 ...
 *   The unit number.
 * \param [in] fsrd_block -
 *   FSRD block id. FMACs are disabled in a resolution in 8 lanes also (FMAC8) 
 *   so that is why we access per FSRD id
 * \param [in] enable -
 *   1:enable, 0:disable
 * \return
 *   See \ref shr_error_e
 ...
 */
shr_error_e
dnx_fabric_if_fmac8_fsrd_block_enable_set(
    int unit,
    int fsrd_block,
    int enable)
{
    ...
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_blocks_reset_required_before_power_down))
    {
        if (!enable)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmacs_in_fsrd_reset(unit, fsrd_block, 1));
        }
    }
    ...
    *block_valid_p = enable ? 1 : 0;
    ...
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_blocks_reset_required_before_power_down))
    {
        if (enable)
        {
            /** Put FMACs out of reset after power up */
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmacs_in_fsrd_reset(unit, fsrd_block, 0));
        }
    }
    ...
    int unit)
{
    ...
    for (fsrd_block = 0; fsrd_block < nof_fsrd; ++fsrd_block)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_fabric_if_fmac8_fsrd_block_enable_set(unit, fsrd_block, 0));

        if (_SHR_PBMP_MEMBER(supported_fsrds, fsrd_block))
        {
            /** Set FSRD state according to supported FSRD bitmap (supported_links) */
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmac8_fsrd_block_enable_set(unit, fsrd_block, 1));
        }
    }
