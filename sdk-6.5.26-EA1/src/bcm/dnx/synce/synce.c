...
/**
 * \brief
 *   Get SyncE nif clock enable status
 * \param [in] unit -
 *   The unit number.
 * \param [in] core -
 *   core ID
...
static int
dnx_nif_synce_ref_clk_nif_clk_enable_get(
    int unit,
    int core,
    int is_mgu,
    int synce_index,
    uint32 *enable)
{
     ...
     */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CLOCK_ENABLE, INST_SINGLE, enable);
    /*
     * commit value
     */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    DNXC_SERDES_REF_CLOCK_TYPE ref_clock_out)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_nif_synce_ref_clk_nif_clk_enable_set(unit, core_id, is_mgu, synce_index, 1));
    }
    /*
     * Enable synce
     */
    ...
    bcm_port_t * port)
{
    ...
    else
    {
        ...
        {
            /*
             * Next check MGU is enabled or not
             */
            if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_nif_clk_en_cfg))
            {
                SHR_IF_ERR_EXIT(dnx_nif_synce_ref_clk_nif_clk_enable_get(unit, -1, 1, synce_index, &synce_enable));
                if (synce_enable)
                {
                    synce_clk_blk_id = dnx_data_nif.eth.nbu_nof_get(unit);
                }
                else
                {
