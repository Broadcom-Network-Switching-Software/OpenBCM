 ...
 */
int
dnx_bier_mcdb_copies_per_entry_get(
    int unit)
{
    int copies_per_entry = 0;

    /*
     * if there is no ingress replication in the system optimization can be made to save only 2 copies per BFT
     * if there is no egress replication in the system optimization can be made to save only 1 copy per BFT
     */
    if (dnx_data_bier.params.feature_get(unit, dnx_data_bier_params_ingress_only))
    {
        copies_per_entry = 1;
    }
    else if (dnx_data_bier.params.feature_get(unit, dnx_data_bier_params_egress_only))
    {
        copies_per_entry = 2;
    }
    else
...
dnx_bier_mcdb_size_get(
    int unit)
{
    int copies_per_entry;
    uint32 bundle_bfr_entries;

    copies_per_entry = dnx_bier_mcdb_copies_per_entry_get(unit);
