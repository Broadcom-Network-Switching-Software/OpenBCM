...
#endif
#define BSL_LOG_MODULE BSL_LS_BCMDNX_DRAMTEMP
    ...
    else
    {
        SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_iter)
        {
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT(dnx_dram_db.temperature_monitoring_last_read.set(unit, hbm_iter, this_hbm_temp_read));
            }
    ...
    uint8 *hbm_is_empty)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_hbmc_traffic_is_empty_verify(unit, hbm_is_empty));
...
{
    int nof_pvt_mon =
        dnx_data_pvt.general.nof_pvt_monitors_get(unit) + dnx_data_dram.general_info.max_nof_drams_get(unit);
    ...
    void *user_data)
{
    ...
    SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_iter)
    {
        SHR_IF_ERR_EXIT_WITH_LOG(dnx_hbmc_temp_get(unit, hbm_iter, &temp),
                                 "HBM temperature monitoring failed, temp read from HBM%d was not successful %s%s\n",
                                 hbm_iter, EMPTY, EMPTY);
        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "HBM%d temperature: %d C\n"), hbm_iter, temp));

        min_temp = UTILEX_MIN(min_temp, temp);
        max_temp = UTILEX_MAX(max_temp, temp);
    }
    ...
    if (max_temp >= dnx_data_dram.hbm.power_down_temp_threshold_get(unit))
    {
        ...
        power_down_temperature_threshold_detected = 1;
             ...
             && (max_temp >= dnx_data_dram.hbm.stop_traffic_temp_threshold_get(unit)))
    {
        ...
        stop_traffic_temperature_threshold_detected = 1;
             ...
             && (max_temp <= dnx_data_dram.hbm.restore_traffic_temp_threshold_get(unit)))
    {
        LOG_INFO(BSL_LOG_MODULE,
                 (BSL_META_U
                  (unit,
    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_low_temperature_dram_protction))
    {
        ...
        if (min_temp <= dnx_data_dram.hbm.power_down_low_temp_threshold_get(unit))
        {
            ...
            power_down_temperature_threshold_detected = 1;
                 ...
                 && (min_temp <= dnx_data_dram.hbm.stop_traffic_low_temp_threshold_get(unit)))
        {
            ...
            stop_traffic_temperature_threshold_detected = 1;
                 ...
                 && (min_temp >= dnx_data_dram.hbm.restore_traffic_low_temp_threshold_get(unit)))
        {
            LOG_INFO(BSL_LOG_MODULE,
                     (BSL_META_U
                      (unit,
    ...
    if (power_down_temperature_threshold_detected)
    {
        ...
        SHR_IF_ERR_EXIT(shr_thread_manager_stop(temp_monitor_handler, 1));
    ...
    uint32 *temp_arr)
{
    ...
    {
        SHR_ALLOC_ERR_EXIT(temp_element, sizeof(dnx_hbmc_temp_stub_t),
                           "dnx_hbm_tem_stub", "%s%d%s", "sal_alloc returned NULL pointer for index=", i, EMPTY);
