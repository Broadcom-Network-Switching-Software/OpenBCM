    ...
    int ignore_vendor)
{
    ...
    const dnx_data_dram_general_info_timing_params_t *timing_params =
        dnx_data_dram.general_info.timing_params_get(unit);
    uint32 trtw_explicit = dnx_data_dram.general_info.tertw_get(unit);
    uint32 trtw_indirect = timing_params->trtw;
    uint32 trtw_final;
    ...
    if ((int) trtw_explicit != -1)
    {
        trtw_final = trtw_explicit;
    }
    else if ((int) trtw_indirect != -1)
    {
        trtw_final = trtw_indirect + BURST_LENGTH_DIV_4 -
            (dnx_data_dram.general_info.write_latency_get(unit) + write_latency_vendor_specific_variation) +
            dnx_data_dram.general_info.read_latency_get(unit);
    }
    else
    {
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_NOT_FOUND,
                                 "User must define either ext_ram_t_rtw or ext_ram_t_e_rtw for the DRAM to work properly%s%s%s\n",
                                 EMPTY, EMPTY, EMPTY);
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_TWTRS, INST_SINGLE,
                                 ...
                                 (dnx_data_dram.general_info.write_latency_get(unit) +
                                  write_latency_vendor_specific_variation));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_TRTW, INST_SINGLE, trtw_final);
