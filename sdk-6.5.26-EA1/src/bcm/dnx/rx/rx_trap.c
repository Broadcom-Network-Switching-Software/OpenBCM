    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        /** Set action attribute to clear bytes_to_remove value, i.e bytes_to_remove=0 */
        dnx_rx_trap_ingress_action_with_flag_hw_set(unit, entry_handle_id, config_p->flags2,
                                                    BCM_RX_TRAP_FLAGS2_START_PACKET_STRIP_CLEAR,
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    /** Check whether bytes to remove overwrite was set as action*/
    if (bytes_to_rm_ow == TRUE)
    {
        config_p->flags2 |= BCM_RX_TRAP_FLAGS2_START_PACKET_STRIP_CLEAR;
    }
    ...
    int unit)
{
    ...
    const dnxc_data_table_info_t *fwd1_trap_info = dnx_data_mpls.general.fwd1_trap_destination_info_get(unit);
    const dnx_data_mpls_general_fwd1_trap_destination_t *fwd1_trap_table;
    ...
    for (idx = 0; idx < fwd1_trap_info->key_size[0]; idx++)
    {
        fwd1_trap_table = dnx_data_mpls.general.fwd1_trap_destination_get(unit, idx);
        if (fwd1_trap_table->context != DBAL_FIELD_EMPTY)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, fwd1_trap_table->context, INST_SINGLE,
    ...
    if (protocol_trap_mode == DNX_RX_TRAP_PROTOCOL_MODE_IN_LIF)
    {
        ...
        if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_cs_in_lif_profile_based_on_stages))
        {
            SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                            (unit, port, _SHR_GPORT_TYPE_FLOW_LIF,
