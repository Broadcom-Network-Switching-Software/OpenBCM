    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(sat_pkt_header_info.init(unit));
    SHR_IF_ERR_EXIT(sat_pkt_header_info.pkt_header_base.alloc(unit));
    ...
    if (dnx_data_sat.general.feature_get(unit, dnx_data_sat_general_skip_event_handling) == 0)
    {
    ...
    if (dnx_data_sat.general.accum_count_not_collect_during_unavaliable_status_get(unit))
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_WINDOW_SIZE, INST_SINGLE, 0);
    ...
    int unit)
{
    ...
    if (dnx_data_sat.general.feature_get(unit, dnx_data_sat_general_skip_event_handling) == 1)
    {
        SHR_EXIT();
    }
    ...
    bcm_sat_config_t * conf)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_config_get_verify(unit, conf));
    ...
    bcm_sat_config_t * conf)
{
    ...
    if (dnx_data_sat.tx_flow_param.feature_get(unit, dnx_data_sat_tx_flow_param_seq_number_wrap_around))
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_SAT_SEQ_NUM_WRAP_AROUND, entry_handle_id));
    ...
    bcm_sat_config_t * conf)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_config_set_verify(unit, conf));
    ...
    SHR_IF_ERR_EXIT(dnx_sat_gen_cfg_set(unit, conf));
    ...
    void *user_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_event_register_verify(unit, user_data));
    ...
    bcm_sat_event_cb cb)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_sat_event_unregister_verify(unit, event_type));
