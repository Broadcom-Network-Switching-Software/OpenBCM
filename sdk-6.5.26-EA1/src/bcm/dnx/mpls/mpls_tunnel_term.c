...
#include <bcm_int/dnx/auto_generated/dnx_failover_dispatch.h>
#include "mpls_evpn.h"
    ...
    bcm_mpls_special_label_t * label_info_p)
{
     ...
     */
    SHR_IF_ERR_EXIT(dbal_entry_attribute_set(unit, entry_handle_id, DBAL_ENTRY_ATTR_PRIORITY, 1));
...
{
    uint32 entry_handle_id;
    uint32 dbal_tables[] = { DBAL_TABLE_MPLS_SKIP_TCAM_DB };
    uint32 nof_tables;
    uint32 table_id;
    int is_end;
    uint32 field_val;
    ...
    dnx_mpls_termination_profile_t * term_profile_info)
{
    ...
    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_EXPECT_BOS))
    {
        term_profile_info->check_bos = 1;
        term_profile_info->expect_bos = 1;
    }
    else if (_SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_EXPECT_NON_BOS))
    {
        term_profile_info->check_bos = 1;
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    {
        info->flags2 |= BCM_MPLS_SWITCH2_EXPECT_NON_BOS;
    }
...
/**
* \brief
* Verify info struct according to MPLS termination requirements.
* Invoked from dnx_mpls_tunnel_switch_term_add().
* \par DIRECT INPUT
*    \param [in] unit -
*     Relevant unit.
*     \param [in] info -
*     A pointer to a struct containing relevant information for termination configuration.
* \par INDIRECT INPUT:
*
* \par DIRECT OUTPUT:
...
*   * None.
* \see
*   * \ref dnx_mpls_tunnel_switch_term_add
*/
shr_error_e
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (DNX_MPLS_SWITCH_IS_EVPN(info) && _SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_EXTENDED_TERMINATION))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot use BCM_MPLS_SWITCH2_EXTENDED_TERMINATION with EVPN tennel\n");
    }

    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_EXPECT_BOS)
        && _SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_EXPECT_NON_BOS))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot expect BOS and Non_BoS for one particual label\n");
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *local_in_lif)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_is_table_active
                    (unit, DBAL_TABLE_MPLS_TERMINATION_SINGLE_LABEL_DB_2ND_PASS, &table_active));
    if (!table_active)
    {
        table_id = DBAL_TABLE_MPLS_TERMINATION_SINGLE_LABEL_DB;
    }
    ...
    switch (match_info->match_criteria)
    {
        ...
        case TERM_MATCH_CRITERIA_LABEL_2ND_PASS_DB:
            ...
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VMPI, match_info->vmpi);
            }
            break;
        case TERM_MATCH_CRITERIA_LABEL_BOS:
    ...
    lif_table_mngr_inlif_info_t * lif_info)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dbal_tables_field_max_value_get
                        (unit, table_id, DBAL_FIELD_PROTECTION_POINTER, 0, 0, INST_SINGLE, &protection_ptr_max_value));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PROTECTION_POINTER, INST_SINGLE,
                                     protection_ptr_max_value);
    }
    if (stat_enable)
    {
    ...
    int *in_lif_profile)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_PROPAGATION_PROFILE, INST_SINGLE, propagation_profile));
    }
    ...
    if (protection_pointer != NULL)
    {
        ...
        if (!field_exists || (*protection_pointer == protection_ptr_max_value))
        {
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    int old_in_global_lif, sys_in_lif;
    dnx_mpls_termination_profile_t term_profile_info;
    ...
    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_REPLACE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_manager_info_to_mpls_term
                        (unit, old_entry_handle_id, &lif_info, &old_in_global_lif, (uint32 *) &old_term_profile,
                         &old_propag_profile, NULL, NULL, NULL, NULL, NULL, &old_in_lif_profile));
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_allocate
                        (unit, lif_flags, DNX_ALGO_LIF_INGRESS, NULL, &global_lif_info.global_lif));
    ...
    {
        BCM_FAILOVER_ID_GET(failover_id[0], info->failover_id);
    ...
    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_REPLACE))
    {
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data
                        (unit, old_in_lif_profile, &in_lif_profile_info, DBAL_TABLE_IN_LIF_FORMAT_LSP, LIF));
    }
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange
                    (unit, &in_lif_profile_info, old_in_lif_profile, &new_in_lif_profile, LIF,
                     DBAL_TABLE_IN_LIF_FORMAT_LSP, &last_profile));
    ...
    {
        sys_in_lif = global_lif_info.global_lif;
    }
     ...
     */
    lif_info.global_lif = global_lif_info.global_lif;
    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_REPLACE))
    {
    ...
    if (!_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_WITH_ID))
    {
        BCM_GPORT_TUNNEL_ID_SET(info->tunnel_id, global_lif_info.global_lif);
    }
    ...
    if (_SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_EXTENDED_TERMINATION))
    {
        SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_write
                        (unit, global_lif_info.global_lif, DBAL_CORE_ALL, local_inlif, FALSE));
    }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (info->failover_id != dnx_data_failover.path_select.ing_no_protection_get(unit))
    {
        ...
        BCM_FAILOVER_SET(info->failover_id, info->failover_id, BCM_FAILOVER_TYPE_INGRESS);
    ...
    SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_get(unit, in_global_lif, DBAL_CORE_DEFAULT, &ext_term_found));
    if (ext_term_found)
    {
        info->flags2 |= BCM_MPLS_SWITCH2_EXTENDED_TERMINATION;
    }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (_SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_EXTENDED_TERMINATION))
    {
        SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_delete(unit, in_global_lif, DBAL_CORE_ALL));
    }
        ...
        while (!is_end)
        {
            ...
            switch (dbal_tables[table])
            {
                ...
                case DBAL_TABLE_MPLS_TERMINATION_SINGLE_LABEL_DB_2ND_PASS:
                    ...
                    {
                        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                                        (unit, entry_handle_id, DBAL_FIELD_VMPI, field_value));
                        info.class_id = field_value[0];
                    }
                    break;
