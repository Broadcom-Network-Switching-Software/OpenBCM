...
bcm_dnx_time_pll_init(int unit)
{
    ...
    SOC_REG_ABOVE_64_CLEAR(field_above_64_val);
    soc_reg_above_64_field_set(unit, ECI_BS_PLL_CONFIGURATIONSr, reg_above_64_val, MISC_PLL_4_RESET_Bf, field_above_64_val);
    SOC_IF_ERROR_RETURN(WRITE_ECI_BS_PLL_CONFIGURATIONSr(unit, reg_above_64_val));

    SOC_IF_ERROR_RETURN(READ_ECI_BS_PLL_CONFIGURATIONSr(unit, reg_above_64_val));
    SOC_REG_ABOVE_64_CLEAR(field_above_64_val);
    soc_reg_above_64_field_set(unit, ECI_BS_PLL_CONFIGURATIONSr, reg_above_64_val, MISC_PLL_4_POST_RESET_Bf, field_above_64_val);
    SOC_IF_ERROR_RETURN(WRITE_ECI_BS_PLL_CONFIGURATIONSr(unit, reg_above_64_val));
    ...
    /* On J2P, the BS-PLL output seems to be 40MHz. Hence use the post divider to get 20MHz output */
    if (soc_is(unit, J2P_DEVICE)) {
        SOC_REG_ABOVE_64_CLEAR(field_above_64_val);
        field_above_64_val[0] = 2;
        soc_reg_above_64_field_set(unit, ECI_BS_PLL_CONFIGURATIONSr, reg_above_64_val, MISC_PLL_4_PDIVf, field_above_64_val);
    }
    ...
    soc_reg_above_64_field_set(unit, ECI_BS_PLL_CONFIGURATIONSr, reg_above_64_val, MISC_PLL_4_RESET_Bf, field_above_64_val);
    SOC_IF_ERROR_RETURN(WRITE_ECI_BS_PLL_CONFIGURATIONSr(unit, reg_above_64_val));
    sal_usleep(100);    /* For the PLL to lock ?? */
...
typedef struct bcm_dnx_time_ts_capture_info_s {
    ...
    shr_thread_ctrl_t *thread_id_time_capture_ts;
        ...
        void *user_data)
{
    ...
    if (cb_overflow.cb == NULL && cb_fill.cb == NULL && cb_all.cb == NULL){
        ...
        if(SOC_IS_J2P(unit) || SOC_IS_J2C(unit) || SOC_IS_Q2A(unit)) {
            WRITE_NS_TS_INT_ENABLEr(unit, 0x3);
        } else {
            WRITE_CMIC_TIMESYNC_INTERRUPT_ENABLEr(unit, 0);
        }
...
#if defined(BCM_TIMESYNC_TIME_CAPTURE_SUPPORT)
    if(SOC_IS_J2P(unit) || SOC_IS_J2C(unit) || SOC_IS_Q2A(unit)) {
        WRITE_NS_TS_INT_ENABLEr(unit, 0);
    } else {
        WRITE_CMIC_TIMESYNC_INTERRUPT_ENABLEr(unit, 0);
    }
#endif
...
void
_bcm_dnx_time_capture_ts_cb_thread(shr_thread_ctrl_t * th, void *unit_vp)
{
    int num_entries;
