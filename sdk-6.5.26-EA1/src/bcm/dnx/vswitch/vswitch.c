...
#include <src/bcm/dnx/flow/porting/vswitch_flow.h>
    ...
    int *is_p2p)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_vswitch_is_p2p_flow(unit, gport, is_p2p));
        SHR_EXIT();
    }
    ...
    bcm_gport_t gport2)
{
    ...
    if (gport2 != 0)
    {
        ...
        /*
         * Verify that if gport2 is LIF, it has a valid forward information.
         */
        if ((_SHR_GPORT_IS_VLAN_PORT(gport2)) || (_SHR_GPORT_IS_MPLS_PORT(gport2)) ||
            (_SHR_GPORT_IS_EXTENDER_PORT(gport2)))
        {
            dnx_algo_gpm_forward_info_t forward_info2;
            shr_error_e rv;
            bcm_gport_t gport = gport2;


            rv = algo_gpm_gport_l2_forward_info_get(unit, gport, &forward_info2);
            SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
            if (rv == _SHR_E_NOT_FOUND)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Wrong setting. port2 = 0x%08X is LIF but has no forward info! Please use port2 + encap2 instead!\n",
                             gport2);
            }
        }
    }
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    SHR_IF_ERR_EXIT_NO_MSG(dnx_vswitch_cross_connect_get_forward_info
                           (unit, entry_handle_id, forward_info, TRUE, FALSE));
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_nof_res_type_get(unit, DBAL_TABLE_L2_GPORT_TO_FORWARDING_SW_INFO, &nof_res_type));
    current_forward_info.fwd_info_result_type = nof_res_type;
    /** Taking a new handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, gport_hw_resources->inlif_dbal_table_id, &entry_handle_id));
    ...
    else
    {
        ...
        else
        {
            dbal_table_field_info_t field_info;
            rv = dbal_tables_field_info_get_no_err(unit, gport_hw_resources->inlif_dbal_table_id,
                                                   DBAL_FIELD_GLOB_OUT_LIF, FALSE,
                                                   gport_hw_resources->inlif_dbal_result_type, INST_SINGLE,
                                                   &field_info);
            if (rv == _SHR_E_NONE)
            {
            ...
            rv = dbal_tables_field_info_get_no_err(unit, gport_hw_resources->inlif_dbal_table_id, DBAL_FIELD_EEI,
                                                   FALSE, gport_hw_resources->inlif_dbal_result_type, INST_SINGLE,
                                                   &field_info);
            if (rv == _SHR_E_NONE)
            {
                dbal_entry_value_field32_set(unit, new_entry_handle_id, DBAL_FIELD_EEI, INST_SINGLE, 0);
            }
            else
            {
                SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
    ...
    void *user_data)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_is_table_active(unit, table_id, &is_table_active));
    ...
    if (table_id == DBAL_TABLE_IN_AC_INFO_DB)
    {
        ...
        if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_p2p_or_mp_based_on_cs_in_lif_profile))
        {
            ...
            SHR_IF_ERR_EXIT(dbal_iterator_key_field_arr32_rule_add
                            (unit, entry_handle_id, DBAL_FIELD_IN_LIF, DBAL_CONDITION_NOT_EQUAL_TO,
                             &recycle_default_inlif, NULL));
        }
    }
    ...
    bcm_vlan_t * vsi)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_create_verify(unit, vsi));
    ...
    bcm_vlan_t vsi)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_create_with_id_verify(unit, vsi));
    allocated_vsi = vsi;
    /*
     * DNX SW Algorithm, allocate VSI.
     */
...
/**
 * \brief -
 *  Verify function for bcm_dnx_vswitch_cross_connect_add for IN_LIF_IPvX_TUNNELS
 *
 * \param [in] unit - Relevant unit
 * \param [in] gports - pointer to structure type bcm_vswitch_cross_connect_t which holds the
 *        information about the two ports attached to P2P
 *
 * \return
 *   shr_error_e
 *
 ...
 */
shr_error_e static
dnx_vswitch_cross_connect_in_lif_ipvx_tunnels_verify(
    int unit,
    bcm_vswitch_cross_connect_t * gports)
{
    int rv = _SHR_E_NONE;
    ...
    rv = algo_gpm_encode_destination_field_from_gport
        (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, gports->port2, &destination);

    if (rv != _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Wrong value of gports.port2=0x%08X. For IPvX tunnels it can be set to any kind of port, FEC or ECMP group only!\n",
                     gports->port2);
    }
    if (gports->encap2 != BCM_FORWARD_ENCAP_ID_INVALID)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Wrong value of gports.encap2=0x%08X. It should be set to BCM_FORWARD_ENCAP_ID_INVALID for IPvX tunnels!\n",
                     gports->encap2);
    }
    ...
    bcm_vswitch_cross_connect_t * gports)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_cross_connect_verify(unit, gports));

    /** Verify port1 ==> port2 */
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_cross_connect_port_verify(unit, gports->port1, gports->port2));

    if ((_SHR_IS_FLAG_SET(gports->flags, BCM_VSWITCH_CROSS_CONNECT_DIRECTIONAL) == FALSE))
    {
        /** Verify port2 ==> port1 */
        SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_cross_connect_port_verify(unit, gports->port2, gports->port1));
    }

    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_add_flow(unit, gports));
        SHR_EXIT();
    }
    ...
    if (_SHR_IS_FLAG_SET(gports->flags, BCM_VSWITCH_CROSS_CONNECT_DIRECTIONAL)
        && (gport_hw_resources1.inlif_dbal_table_id == DBAL_TABLE_IN_LIF_IPvX_TUNNELS))
    {
        SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_cross_connect_in_lif_ipvx_tunnels_verify(unit, gports));
    }
    /*
     * gport2: Resolve forward information:
     */
    gport = gports->port2;
    SHR_IF_ERR_EXIT(algo_gpm_is_gport_valid_destination(unit, gports->port2, &is_valid_dest));
    /** convert to flow_id if it is not valid_dest, which means it is probably LIF */
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
        && (is_valid_dest == FALSE))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, gports->port2, _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
                         &gport));
    }
    SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_to_forward_information
                    (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, gport, gports->encap2, &forward_info2));
    ...
    if ((_SHR_IS_FLAG_SET(gports->flags, BCM_VSWITCH_CROSS_CONNECT_DIRECTIONAL) == FALSE))
    {
         ...
         */
        gport = gports->port1;
        SHR_IF_ERR_EXIT(algo_gpm_is_gport_valid_destination(unit, gports->port1, &is_valid_dest));
        /** convert to flow_id if it is not valid_dest, which means it is probably LIF */
        if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
            && (is_valid_dest == FALSE))
        {
            SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                            (unit, gports->port1, _SHR_GPORT_TYPE_FLOW_LIF,
                             DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR, &gport));
        }

        SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_to_forward_information
                        (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, gport, gports->encap1, &forward_info1));
    ...
    bcm_vswitch_cross_connect_t * gports)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_cross_connect_port_verify(unit, gports->port1, 0));

    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_get_flow(unit, gports));
        SHR_EXIT();
    }
    /*
     * GPORT: Get local In-LIF using DNX Algo GPORT Managment:
    ...
    bcm_vswitch_cross_connect_t * gports)
{
    ...
    if ((_SHR_IS_FLAG_SET(gports->flags, BCM_VSWITCH_CROSS_CONNECT_DIRECTIONAL) == FALSE))
    {
         ...
         * Verifying gport is valid
         */
        SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_cross_connect_port_verify(unit, gports->port2, 0));
    ...
    void *user_data)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_flow(unit, cb, user_data));
        SHR_EXIT();
    }
    ...
    SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_apply(unit, DBAL_TABLE_IN_LIF_FORMAT_EVPN, cb, user_data));
    ...
    int unit)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_delete_flow(unit));
        SHR_EXIT();
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_apply
                        (unit, DBAL_TABLE_IN_AC_INFO_DB, dnx_vswitch_cross_connect_delete_all_cb, &user_data));
    }

    SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_apply
                    (unit, DBAL_TABLE_IN_LIF_FORMAT_PWE, dnx_vswitch_cross_connect_delete_all_cb, &user_data));
    SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_apply
                    (unit, DBAL_TABLE_INNER_ETH_VLAN_EDIT_CLASSIFICATION, dnx_vswitch_cross_connect_delete_all_cb,
                     &user_data));
    SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_apply
                    (unit, DBAL_TABLE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_VLAN_EDIT,
                     dnx_vswitch_cross_connect_delete_all_cb, &user_data));
    SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_apply
                    (unit, DBAL_TABLE_IN_LIF_IPvX_TUNNELS, dnx_vswitch_cross_connect_delete_all_cb, &user_data));
    SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_traverse_apply
                    (unit, DBAL_TABLE_IN_LIF_FORMAT_EVPN, dnx_vswitch_cross_connect_delete_all_cb, &user_data));
    ...
    bcm_vlan_t vsi)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_destroy_verify(unit, vsi));
...
/**
 * \brief - Verify function for bcm_dnx_vswitch_flexible_connect_add/get/delete
 */
static shr_error_e
dnx_vswitch_flexible_connect_match_verify(
    int unit,
    uint8 is_add,
    uint32 flags,
    ...
    bcm_vswitch_flexible_connect_fwd_t * port_fwd)
{
    ...
    SHR_NULL_CHECK(port_match, _SHR_E_PARAM, "A valid pointer for port_match is needed!");
    SHR_NULL_CHECK(port_fwd, _SHR_E_PARAM, "A valid pointer for port_fwd is needed!");

    /** Verify the match information*/
    if ((port_match->match != BCM_PORT_MATCH_PORT_VLAN_STACKED) &&
        (port_match->match != BCM_PORT_MATCH_PORT_VLAN) && (port_match->match != BCM_PORT_MATCH_PORT))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Not supported match %d. Only BCM_PORT_MATCH_PORT/_PORT_VLAN/_PORT_VLAN_STACKED are available!\n",
                     port_match->match);
    }

    if (!BCM_GPORT_IS_MPLS_PORT(port_match->match_port) ||
        (BCM_GPORT_SUB_TYPE_LIF_EXC_GET(port_match->match_port) == BCM_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The match port(0x%08X) must be an mpls port with not only egress information!\n",
                     port_match->match_port);
    }

    BCM_DNX_VLAN_CHK_ID(unit, port_match->match_vlan);
    BCM_DNX_VLAN_CHK_ID(unit, port_match->match_inner_vlan);

    if (_SHR_IS_FLAG_SET(flags, BCM_VSWITCH_FLEXIBLE_CONNECT_UPDATE) && !is_add)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_VSWITCH_FLEXIBLE_CONNECT_UPDATE can be used in _add operation!\n");
    }
...
/**
 * \brief - Verify function for bcm_dnx_vswitch_flexible_connect_add
 */
static shr_error_e
dnx_vswitch_flexible_connect_add_verify(
    int unit,
    uint32 flags,
    bcm_vswitch_flexible_connect_match_t * port_match,
    bcm_vswitch_flexible_connect_fwd_t * port_fwd)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_flexible_connect_match_verify(unit, TRUE, flags, port_match, port_fwd));

    /** Verify the entry existence if neccessary.*/
    if (_SHR_IS_FLAG_SET(flags, BCM_VSWITCH_FLEXIBLE_CONNECT_UPDATE))
    {
        bcm_vswitch_flexible_connect_fwd_t port_fwd_orig;
        uint32 flags_i = flags & (~BCM_VSWITCH_FLEXIBLE_CONNECT_UPDATE);
        SHR_IF_ERR_EXIT(bcm_vswitch_flexible_connect_get(unit, flags_i, port_match, &port_fwd_orig));
    }

    /** Verify the port_fwd if neccessary: _add, in _get/_delete case, it should be NULL.*/
    if (port_fwd->forward_encap != BCM_FORWARD_ENCAP_ID_INVALID &&
        !BCM_FORWARD_ENCAP_ID_IS_OUTLIF(port_fwd->forward_encap) &&
        !BCM_FORWARD_ENCAP_ID_IS_EEI(port_fwd->forward_encap))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "port_fwd->forward_encap (0x%08X) can be an outLIF or EEI only!\n", port_fwd->forward_encap);
    }

    if (BCM_FORWARD_ENCAP_ID_IS_EEI(port_fwd->forward_encap) &&
        (BCM_FORWARD_ENCAP_ID_EEI_USAGE_GET(port_fwd->forward_encap) == BCM_FORWARD_ENCAP_ID_EEI_USAGE_MPLS_PORT))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "port_fwd->forward_encap (0x%08X) can be an EEI for MPLS push only in JR1 mode!\n",
                         port_fwd->forward_encap);
        }
    }
    ...
    if (BCM_FORWARD_ENCAP_ID_IS_EEI(port_fwd->forward_encap) &&
        (BCM_FORWARD_ENCAP_ID_EEI_USAGE_GET(port_fwd->forward_encap) != BCM_FORWARD_ENCAP_ID_EEI_USAGE_MPLS_PORT) &&
        (BCM_FORWARD_ENCAP_ID_EEI_USAGE_GET(port_fwd->forward_encap) != BCM_FORWARD_ENCAP_ID_EEI_USAGE_ENCAP_POINTER))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "If port_fwd->forward_encap (0x%08X) is EEI, it must be encoded with either "
                     "BCM_FORWARD_ENCAP_ID_EEI_USAGE_MPLS_PORT or BCM_FORWARD_ENCAP_ID_EEI_USAGE_ENCAP_POINTER!\n",
                     port_fwd->forward_encap);
    }
...
/**
 * \brief - Set the key for bcm_dnx_vswitch_flexible_connect_add/get/delete
 */
static shr_error_e
dnx_vswitch_flexible_connect_match_set(
    int unit,
    uint32 flags,
    bcm_vswitch_flexible_connect_match_t * port_match,
    uint32 entry_handle_id)
{
    ...
    lif_flags = DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS;
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, port_match->match_port, lif_flags, &gport_hw_resources));

    /** Set the key field in handle. */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOB_IN_LIF, gport_hw_resources.global_in_lif);
    if (port_match->match == BCM_PORT_MATCH_PORT_VLAN_STACKED)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_S_VID_1, port_match->match_vlan);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_S_VID_2, port_match->match_inner_vlan);
    }
    else if (port_match->match == BCM_PORT_MATCH_PORT_VLAN)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_S_VID_1, port_match->match_vlan);
    }
...
/**
 * \brief - Set the payload for bcm_dnx_vswitch_flexible_connect_add
 */
static shr_error_e
dnx_vswitch_flexible_connect_payload_set(
    int unit,
    uint32 flags,
    bcm_vswitch_flexible_connect_fwd_t * port_fwd,
    uint32 entry_handle_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_encode_destination_field_from_gport
                    (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, port_fwd->forward_port, &forward_info.destination));

    /** The forward_encap is outlif. */
    if (BCM_FORWARD_ENCAP_ID_IS_OUTLIF(port_fwd->forward_encap))
    {
        result_type = DBAL_RESULT_TYPE_EXTENDED_P2P_2TAG_FWD_DEST_OUTLIF;
        forward_info.outlif = BCM_FORWARD_ENCAP_ID_VAL_GET(port_fwd->forward_encap);
    }
    /** The forward_encap is EEI. It can be a pointer or push command and used in JR1 mode only.*/
    else if (BCM_FORWARD_ENCAP_ID_IS_EEI(port_fwd->forward_encap))
    {
        result_type = DBAL_RESULT_TYPE_EXTENDED_P2P_2TAG_FWD_DEST_EEI;
        if (BCM_FORWARD_ENCAP_ID_EEI_USAGE_GET(port_fwd->forward_encap) == BCM_FORWARD_ENCAP_ID_EEI_USAGE_ENCAP_POINTER)
        {
            SHR_IF_ERR_EXIT(dnx_mpls_eei_enc_pointer_information_fill
                            (unit, BCM_FORWARD_ENCAP_ID_VAL_GET(port_fwd->forward_encap), 0, &forward_info.eei));
        }
        else if (BCM_FORWARD_ENCAP_ID_EEI_USAGE_GET(port_fwd->forward_encap) ==
                 BCM_FORWARD_ENCAP_ID_EEI_USAGE_MPLS_PORT)
        {
            uint32 vc_label, push_profile;
            vc_label = BCM_FORWARD_ENCAP_ID_EEI_USAGE_MPLS_PORT_VC_GET(port_fwd->forward_encap);
            push_profile = BCM_FORWARD_ENCAP_ID_EEI_USAGE_MPLS_PORT_PUSH_PROFILE_GET(port_fwd->forward_encap);

            SHR_IF_ERR_EXIT(dnx_mpls_eei_push_information_fill(unit, vc_label, push_profile, &forward_info.eei));
        }
    }
    else
    {
        result_type = DBAL_RESULT_TYPE_EXTENDED_P2P_2TAG_FWD_DEST;
    }

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, result_type);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE, forward_info.destination);
    if (result_type == DBAL_RESULT_TYPE_EXTENDED_P2P_2TAG_FWD_DEST_OUTLIF)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOB_OUT_LIF, INST_SINGLE, forward_info.outlif);
    }
    else if (result_type == DBAL_RESULT_TYPE_EXTENDED_P2P_2TAG_FWD_DEST_EEI)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EEI, INST_SINGLE, forward_info.eei);
    }
...
/**
 * \brief - Get the payload for bcm_dnx_vswitch_flexible_connect_get
 */
static shr_error_e
dnx_vswitch_flexible_connect_payload_get(
    int unit,
    uint32 entry_handle_id,
    bcm_vswitch_flexible_connect_fwd_t * port_fwd)
{
    ...
    else if (result_type == DBAL_RESULT_TYPE_EXTENDED_P2P_2TAG_FWD_DEST_EEI)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_EEI, INST_SINGLE,
                                                            &forward_info.eei));
    }
    else
    {
    ...
    /** Get the gport and encap_id.*/
    SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_from_forward_information
                    (unit, &port_fwd->forward_port, (uint32 *) &port_fwd->forward_encap, &forward_info, 1));

    /** Set the encap_id with invalid in case it's not part of the payload. */
    if (forward_info.fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY)
    {
        port_fwd->forward_encap = BCM_FORWARD_ENCAP_ID_INVALID;
    }
...
/**
 * \brief - Get a proper dbal table for the flexible connect
 */
static shr_error_e
dnx_vswitch_flexible_connect_table_get(
    int unit,
    bcm_vswitch_flexible_connect_match_t * port_match,
    dbal_tables_e * dbal_table_id)
{
    ...
    SHR_NULL_CHECK(port_match, _SHR_E_INTERNAL, "NULL pointer is used!");
    SHR_NULL_CHECK(dbal_table_id, _SHR_E_INTERNAL, "NULL pointer is used!");

    if (port_match->match == BCM_PORT_MATCH_PORT_VLAN_STACKED)
    {
        *dbal_table_id = DBAL_TABLE_EXTENDED_P2P_2TAG;
    }
    else if (port_match->match == BCM_PORT_MATCH_PORT_VLAN)
    {
        *dbal_table_id = DBAL_TABLE_EXTENDED_P2P_1TAG;
    }
    else
    {
        *dbal_table_id = DBAL_TABLE_EXTENDED_P2P_0TAG;
    }
...
/**
 * \brief -
 * BCM API: Add a flexible connection.
 * \param [in] unit - relevant unit.
 * \param [in] flags - relevant flags.
 * \param [in] port_match - Match information for the flexible connection.
 * \param [in] port_fwd - Destination and possible encapsulation information for the
 *             flexible connection.
 *
 * \return
 *   shr_error_e
...
{
    uint32 entry_handle_id;
    dbal_tables_e dbal_table_id = DBAL_TABLE_EXTENDED_P2P_2TAG;
    ...
    /** Verify the inputs. */
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_flexible_connect_add_verify(unit, flags, port_match, port_fwd));

    /** Get the proper dbal table. */
    SHR_IF_ERR_EXIT(dnx_vswitch_flexible_connect_table_get(unit, port_match, &dbal_table_id));

    /** Alloc a handle. */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, dbal_table_id, &entry_handle_id));

    /** Set the match key. */
    SHR_IF_ERR_EXIT(dnx_vswitch_flexible_connect_match_set(unit, flags, port_match, entry_handle_id));

    /** Set the payload. */
    SHR_IF_ERR_EXIT(dnx_vswitch_flexible_connect_payload_set(unit, flags, port_fwd, entry_handle_id));
 ...
 * \param [in] flags - relevant flags.
 * \param [in] port_match - Match information for the flexible connection.
 * \param [out] port_fwd - Destination and possible encapsulation information for the
 *              flexible connection.
 *
 * \return
 *   shr_error_e
    ...
    bcm_vswitch_flexible_connect_fwd_t * port_fwd)
{
    ...
    /** Verify the inputs. */
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_flexible_connect_match_verify(unit, FALSE, flags, port_match, port_fwd));
    ...
    /** Get the match payload. */
    SHR_IF_ERR_EXIT(dnx_vswitch_flexible_connect_payload_get(unit, entry_handle_id, port_fwd));
    ...
    bcm_vswitch_flexible_connect_match_t * port_match)
{
    ...
    /** Verify the inputs. */
    SHR_INVOKE_VERIFY_DNXC(dnx_vswitch_flexible_connect_match_verify(unit, FALSE, 0, port_match, &port_fwd));
    ...
    /** Set the match key. */
    SHR_IF_ERR_EXIT(dnx_vswitch_flexible_connect_match_set(unit, 0, port_match, entry_handle_id));
