    ...
    bcm_l3_vpbr_entry_t * entry)
{
    ...
    {
        if (entry->gport_wide_data != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "illegal value for entry->gport_wide_data [0x%x], gport_wide_data is not supported for VPBR tunnel entries\n",
                         entry->gport_wide_data);
        }
    }
    else
    {
...
/**
 * \brief
 * Get result from the L3 vPBR database (IPv4 / IPv6).
 *     \param [in] unit - Relevant unit.
    ...
    bcm_l3_vpbr_entry_t * entry)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_vpbr_entry_key_set(unit, entry, entry_handle_id));
    SHR_IF_ERR_EXIT(dnx_l3_vpbr_entry_result_get(unit, entry, entry_handle_id));
