    ...
    bcm_l3_egress_t * egr_prev)
{
        ...
        && _SHR_IS_FLAG_SET(egr->flags, BCM_L3_NATIVE_ENCAP))
    {
        if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
        {
            ...
            if ((egr->encap_access != bcmEncapAccessRif) && (egr->encap_access != bcmEncapAccessInvalid)
                && (egr->encap_access != bcmEncapAccessArp))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "encap_access should be bcmEncapAccessRif, bcmEncapAccessArp or bcmEncapAccessInvalid when the flags BCM_L3_NATIVE_ENCAP and BCM_L3_FLAGS2_VLAN_TRANSLATION are set.\r\n");
            }
            if ((egr->encap_access == bcmEncapAccessRif) && (egr->vlan == 0))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Encap access cannot be Rif when vlan = 0 and flags BCM_L3_NATIVE_ENCAP and BCM_L3_FLAGS2_VLAN_TRANSLATION are set.\n");
            }
            if ((egr->encap_access == bcmEncapAccessArp) && (egr->vlan != 0))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Encap access cannot be Arp when vlan != 0 and flags BCM_L3_NATIVE_ENCAP and BCM_L3_FLAGS2_VLAN_TRANSLATION are set.\n");
            }
        }
    }
    /*
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        if ((egr->encap_access != bcmEncapAccessAc) && (egr->encap_access != bcmEncapAccessInvalid)
            && (egr->encap_access != bcmEncapAccessArp) && (egr->encap_access != bcmEncapAccessRif))
        {
    ...
    int qos_index)
{
    ...
    outlif_info.logical_phase = dnx_l3_egress_arp_outlif_phase_get(egr->flags, egr->flags2, egr->encap_access);
