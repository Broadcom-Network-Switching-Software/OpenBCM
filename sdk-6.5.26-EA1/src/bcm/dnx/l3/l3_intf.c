    ...
    bcm_if_t * intf_id)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
        && _SHR_IS_FLAG_SET(ing_intf->flags,
                            ~(L3_INTF_INGRESS_FLOW_SUPPORTED_FLAGS | BCM_L3_INGRESS_WITH_ID | BCM_L3_INGRESS_REPLACE)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
...
*   \param [in] my_mac_prefix_profile - MyMac Prefix profile
    ...
    uint32 ecn_mapping_profile)
{
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MY_MAC_PREFIX, INST_SINGLE,
                                     L3_INTF_MY_MAC_ETH_PREFIX(my_mac_prefix_profile));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MY_MAC, INST_SINGLE,
                                     SOURCE_ADDRESS_GET_MY_MAC_LSB(intf->l3a_mac_addr));
    }
    ...
    bcm_l3_intf_t * intf)
{
    ...
    uint32 mymac_prefix_profile = 0, old_mymac_prefix_profile;
    ...
    if (is_replace || is_vsi_allocated)
    {
        ...
        rv = dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_MY_MAC_PREFIX,
                                               FALSE, table_result_type, INST_SINGLE, &field_info);
        if (rv == BCM_E_NONE)
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange
                    (unit, &in_lif_profile_info, old_in_lif_profile, &new_in_lif_profile, ETH_RIF, table_id,
                     &last_profile));
    new_in_lif_profile = CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, new_in_lif_profile);

    /** MyMAC configuration */
    SHR_IF_ERR_EXIT(dnx_l3_intf_add_my_mac(unit, intf, &mymac_prefix_profile, old_mymac_prefix_profile));
    ...
    SHR_IF_ERR_EXIT(dnx_l3_intf_vsi_hw_set
                    (unit, intf, table_result_type, new_in_lif_profile, propag_profile, mymac_prefix_profile,
                     ecn_mapping_profile));
...
static shr_error_e
dnx_terminator_update_remove_unchangeable_fields(
    int unit,
    uint32 flow_id,
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (!dnx_data_tunnel.feature.has_flow_gport_to_tunnel_type_get(unit))
    {
        SHR_EXIT();
    }
    SHR_EXIT();
    ...
    bcm_if_t * intf_id)
{
    ...
    else if (BCM_L3_ITF_TYPE_IS_LIF(*intf_id))
    {
        ...
        if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
        {
            ...
            SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, gport, _SHR_GPORT_TYPE_FLOW_LIF,
            ...
            /*
             * Remove unchangeable field in terminator create 
             */
            dnx_terminator_update_remove_unchangeable_fields(unit, flow_handle_info.flow_handle, &special_fields);
