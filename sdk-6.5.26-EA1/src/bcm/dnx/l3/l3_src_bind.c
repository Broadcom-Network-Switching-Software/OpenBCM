    ...
    uint32 mac_compress_id)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV4, src_bind_info->sip);
    ...
    uint32 ip6_msb32_compress_id)
{
    ...
    dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_IPV6, src_bind_info->sip6);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    in_lif_profile_info_t * in_lif_profile_info)
{
    ...
    if ((!is_native) || is_native_non_virtual)
    {
        ...
        if (is_p2p)
        {
            if (inlif_dbal_table_id == DBAL_TABLE_PON_DTC_CLASSIFICATION)
            {
