    ...
    int unit)
{
    ...
    if (dnx_data_field.tests.epmf_debug_rely_on_tcam0_get(unit))
    {
        ...
        /*
         * Set occupied of access profile 63 to TRUE
         */
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_profile_sw.access_profiles.occupied.set(unit, app_db_id, TRUE));

        for (ctx_id = 0; ctx_id < nof_contexts; ctx_id++)
        {
            SHR_IF_ERR_EXIT(dnx_field_key_kbr_init(unit, DNX_FIELD_STAGE_EPMF, ctx_id, key_id));
        }
    }
    ...
    int unit)
{
    ...
    if (dnx_data_field.features.tcam_cs_inlif_profile_supported_get(unit))
    {
