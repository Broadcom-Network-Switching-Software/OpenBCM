...
extern const dnx_field_header_qual_info_t dnx_header_qual_info[DNX_FIELD_HEADER_QUAL_NOF];
...
static const dnx_field_qual_name_t dnx_qual_name[DNX_FIELD_QUAL_NOF] = {
        ...
        [DNX_FIELD_QUAL_STATE_ADDRESS]                                  = {"Q_STATE_ADDRESS"},
        [DNX_FIELD_QUAL_STATE_DATA]                                     = {"Q_STATE_DATA"},
        [DNX_FIELD_QUAL_STATE_DATA_WRITE]                               = {"Q_STATE_DATA_WRITE"},
    ...
    dnx_field_qual_info_t * dnx_qual_info_p)
{
    ...
    switch (dnx_qual_class)
    {
        ...
        {
            const dnx_field_header_qual_info_t *header_qual_info_p;
            header_qual_info_p = &dnx_header_qual_info[dnx_qual_id];

            if (header_qual_info_p->field_id == 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Header Qualifier:%s(%d) Stage:%s is not supported\n",
                             header_qual_info_p->name, dnx_qual_id, dnx_field_stage_text(unit, stage));
            }
            ...
            dnx_qual_info_p->field_id = header_qual_info_p->field_id;
            dnx_qual_info_p->sizes[0] = header_qual_info_p->size;
            dnx_qual_info_p->offsets[0] = header_qual_info_p->offset;
    ...
    dnx_field_qual_t dnx_qual)
{
    ...
    header_qual_info_p = &dnx_header_qual_info[qual_id];
    if (header_qual_info_p->field_id == 0)
        return NULL;

    return header_qual_info_p->name;
}
    ...
    int *dnx_qual_num_p)
{
    ...
    if (dnx_data_field.stage.stage_info_get(unit, stage)->pbus_header_length > 0)
    {
        ...
        {
            header_qual_info_p = &dnx_header_qual_info[i_info];
            if (ISEMPTY(header_qual_info_p->name))
                continue;
    ...
    int unit)
{
    ...
    {
        header_qual_info_p = &dnx_header_qual_info[i_id];
        DNX_FIELD_QUAL_SIZE_VERIFY(DNX_FIELD_QUAL_CLASS_HEADER, i_id, header_qual_info_p->size);
    }
    /*
