    ...
    uint32 *second_additional_header)
{
    ...
    if (DNX_TUNNEL_TERM_SRV6_IS_STRICT_SRV6_TYPE(tcam_table_key->type))
    {
    ...
    uint8 *use_flow_app)
{
    ...
    *use_flow_app = FALSE;
    ...
    if (info->type == bcmTunnelTypeGreAnyIn4)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_term_create_flow(unit, info));
            *use_flow_app = TRUE;
        }
    }
    ...
    *use_flow_app = FALSE;
    ...
    if (info->type == bcmTunnelTypeGreAnyIn4)
    {
        if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
            || (dnx_data_flow.ipv4.use_flow_lif_term_get(unit)))
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_term_get_flow(unit, info));
            *use_flow_app = TRUE;
        }
    }
    ...
    *use_flow_app = FALSE;
    ...
    if (info->type == bcmTunnelTypeGreAnyIn4)
    {
        if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
            || (dnx_data_flow.ipv4.use_flow_lif_term_get(unit)))
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_term_delete_flow(unit, info));
            *use_flow_app = TRUE;
        }
    }
