    ...
    uint32 *local_outlif)
{
    ...
    if (_SHR_IS_FLAG_SET(tunnel->flags, BCM_TUNNEL_REPLACE))
    {
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_get_outlif_info
                        (unit, *local_outlif, entry_handle_id, &lif_table_mngr_outlif_info));
        lif_table_mngr_outlif_info.flags |= LIF_TABLE_MNGR_LIF_INFO_REPLACE;

        /** Get ESEM cmd */
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE,
                                            &new_esem_cmd_profile);
    }
    else
    {
        /** set outlif phase */
