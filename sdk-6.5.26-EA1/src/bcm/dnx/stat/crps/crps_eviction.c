    ...
    uint32 *curr_entry)
{
    ...
    /** 2. Once conditional eviction will be implemented in the HW, we can remove it, */
    ...
    if (is_zero == FALSE)
    {
        ...
        /** get how many stat-obj-id's in one counter shadow */
        ...
        /** Validate counter index in range*/
    ...
    void *user_data)
{
    ...
    if (DNX_CRPS_FIFO_DEPTH_CRITICAL(unit) < depth)
    {
        ...
        SHR_IF_ERR_EXIT(shr_thread_manager_interval_set(crps_eviction_handler, interval));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_crps_eviction_db.bg_thread_data.crps_eviction_thread_handler.set(unit, crps_eviction_handler));
        ...
        if (is_active == FALSE)
        {
            ...
            if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_application_v1))
            {
                SHR_IF_ERR_EXIT(shr_thread_manager_interval_set
                                (crps_eviction_handler, DNX_CRPS_BACKGROUND_THREAD_ITER_DEFAULT));
