    ...
    int unit)
{
    ...
    *controlled_counters_p_p = dnx_controlled_counters_array;
    ...
    if (sw_state_is_warm_boot(unit))
    {
        ...
        BCM_PBMP_AND(flexe_mac_pbmp, stat_pbmp->pbmp);
        ...
        BCM_PBMP_OR(*counter_pbmp_p, flexe_mac_pbmp);
    }
    {
        /*
...
dnx_init_mib_thread_start(
    int unit)
{
    uint32 interval, counter_flags = 0;
    SHR_FUNC_INIT_VARS(unit);
    /*
     * start counter
     */
    ...
    switch (type)
    {
        case snmpIfOutErrors:
        {
            ...
            if (!SAL_BOOT_PLISIM)
            {
                ...
                {
                    skip_pad = dnx_data_nif.mac_client.feature_get(unit, dnx_data_nif_mac_client_is_tx_pading_supported)
                        ? 0 : 1;
                }
                if (!skip_pad)
                {
        ...
        case dnx_mib_counter_rx_rs_fec_bit_error:
            if (!DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, L1)))
            {
                ...
                if (port_speed_config.fec > PORTMOD_PORT_PHY_FEC_NONE)
                {
                    ...
                    if (phy.type == phymodDispatchTypeTscbh)
                    {
