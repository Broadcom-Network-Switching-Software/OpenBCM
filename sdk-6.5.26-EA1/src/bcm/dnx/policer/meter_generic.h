...
/**
* \brief
*     poll credits amount for meter, if not empty return error
* \param [in] unit -unit id
* \param [in] core_id -core id
* \param [in] time_out -time out for pulling
* \param [in] min_polls - min polls number
* \param [in] is_ingress -indicates ingress or egress
* \param [in] is_dual_bucket - is dual bucket
* \param [in] is_eir -is eir
* \param [in] database_id -database id
* \param [in] meter_idx -meter  index
* \return
*   \retval Non-zero (!= _SHR_E_NONE) in case of an error
...
*   NONE
*/
shr_error_e dnx_meter_generic_buckets_empty_poll(
    int unit,
    int core_id,
