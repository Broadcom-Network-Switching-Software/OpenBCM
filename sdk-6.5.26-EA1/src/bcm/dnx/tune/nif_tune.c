    ...
    bcm_port_t port)
{
    ...
    if ((DNX_ALGO_PORT_TYPE_IS_NIF_ETH
         (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, MGMT))
         || (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, FALSE, FALSE)
             && dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))) && is_master_port
        && DNX_ALGO_PORT_TYPE_IS_ING_TM(unit, port_info))
    {
        ...
        /*
         * All ports are mapped to default profile 0 till later assined to profile 
         */
        if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
            if (if_tdm_mode != DNX_ALGO_PORT_IF_TDM_ONLY)
            {
                SHR_IF_ERR_EXIT(bcm_dnx_cosq_control_set
                                (unit, port, 0, bcmCosqControlIngressPortDropPortProfileMap,
                                 DNX_PORT_NIF_OFR_DEFAULT_PRD_PROFILE));
            }
        }

        SHR_IF_ERR_EXIT(bcm_dnx_port_priority_config_set(unit, port, &priority_config));
    }
    else if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, FALSE) && is_master_port)
    {
        /*
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1)) && is_master_port)
    {
        int master_port;
