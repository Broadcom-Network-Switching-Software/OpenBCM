...
#include <soc/dnx/swstate/dnx_sw_state_example.h>
#include <soc/dnx/swstate/dnx_sw_state_example_temp_mngr.h>
#include <soc/dnx/swstate/dnx_sw_state_example_res_mngr.h>
#include <soc/dnx/dnx_err_recovery_manager.h>
#include <soc/dnx/arr/auto_generated/dnx2_arr_output.h>
#include <src/appl/diag/dnx/pp/kleap/auto_generated/diag_dnx2_ikleap_layers_info.h>
...
/**
 * \brief
 * steps to never skip during backwards pass (deinit)
 */
#define NEVER_SKIP_DEINIT         DNX_INIT_STEP_F_NEVER_SKIP_DEINIT
/**
 * \brief
 * Steps will use DMA for writing to memories or registers
 */
#define USE_DMA                   DNX_INIT_STEP_F_USE_DMA
...
/** callback function which skips MACSec_IPSec module */
static shr_error_e
sec_skip(
    int unit,
    int *dynamic_flags)
...
bier_skip(
    ...
    int *dynamic_flags)
{
    if (dnx_data_bier.params.feature_get(unit, dnx_data_bier_params_is_supported))
    {
        *dynamic_flags = 0;
    }
    else
...
/** Skip step when HBM PHY Firmware is not supported */
static shr_error_e
hbm_apd_phy_skip(
    int unit,
    int *dynamic_flags)
    ...
    int *dynamic_flags)
{
    *dynamic_flags = 0;
    if ((dnx_data_nif.flexe.flexe_mode_get(unit) == DNX_FLEXE_MODE_DISABLED) ||
        (!dnx_data_nif.framer.feature_get(unit, dnx_data_nif_framer_is_supported)) ||
        (!dnx_drv_soc_feature(unit, soc_feature_sw_linkscan_flr_support)))
    {
        *dynamic_flags = UTILEX_SEQ_STEP_F_SKIP;
    }
...
/**
 * Skip the initiallization of Sat module when Oamp is disabled
 */
static shr_error_e
sat_skip(
    int unit,
    ...
    int *dynamic_flags)
{
    ...
    *dynamic_flags = 0;
    if (dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_not_supported) == 1)
    {
        *dynamic_flags = DNX_INIT_STEP_F_SKIP;
    }
...
#ifdef INCLUDE_CTEST
/**
 * Callback function which skips the step if we are not running tests of example module
 */
static shr_error_e
dnx_sw_state_example_flag_cb(
    int unit,
    int *flags)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    *flags = 0;
    if (!dnx_data_infra_tests.sw_state.feature_get(unit, dnx_data_infra_tests_sw_state_init_enable))
    {
        *flags = DNX_INIT_STEP_F_SKIP;
    }
...
/**
 * Callback function which skips the step if we are not running tests of example resource manger module
 */
static shr_error_e
dnx_sw_state_example_res_mngr_flag_cb(
    int unit,
    int *flags)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    *flags = 0;
    if (!dnx_data_infra_tests.res_mngr_example.feature_get(unit, dnx_data_infra_tests_res_mngr_example_init_enable))
    {
        *flags = DNX_INIT_STEP_F_SKIP;
    }
...
/**
 * Callback function which skips the step if we are not running tests of example template manager module
 */
static shr_error_e
dnx_sw_state_example_temp_mngr_flag_cb(
    int unit,
    int *flags)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    *flags = 0;
    if (!dnx_data_infra_tests.temp_mngr_example.feature_get(unit, dnx_data_infra_tests_temp_mngr_example_init_enable))
    {
        *flags = DNX_INIT_STEP_F_SKIP;
    }
...
static const  dnx_init_step_t dnx_init_deinit_field_seq[] = {
  ...
  {DNX_INIT_STEP_FLD_GROUP,       "FP Group",    dnx_field_group_init,           dnx_field_group_deinit,         NULL,             WB_SKIP,                      NULL},
...
static const  dnx_init_step_t dnx_init_deinit_common_seq[] = {
  ...
  {DNX_INIT_STEP_RX,              "RX",              dnx_rx_init,                  dnx_rx_deinit,                  NULL,             MEM_TEST_SKIP,                                 NULL},
...
static const  dnx_init_step_t dnx_init_deinit_tm_seq[] = {
  ...
  {DNX_INIT_STEP_SEC_MODULE,                    "MACSec_IPSec",         dnx_sec_init,                   NULL,                       sec_skip,       WB_SKIP,                    NULL},
  ...
  {DNX_INIT_STEP_COSQ_AQM_MODULE,               "COSQ AQM",             dnx_cosq_aqm_init,              NULL,                       NULL,           WB_SKIP,                    NULL},
  {DNX_INIT_STEP_COSQ_LATENCY_MODULE,           "COSQ Latency",         dnx_cosq_latency_init,          NULL,                       NULL,           WB_SKIP,                    NULL},
  ...
  {DNX_INIT_STEP_SAT_MODULE,                    "SAT",                  dnx_sat_init,                   dnx_sat_deinit,             sat_skip,       WB_SKIP,                    NULL},
  {DNX_INIT_STEP_LINKSCAN,                      "LINKSCAN",             dnx_linkscan_init,              dnx_linkscan_deinit,        NULL,           MEM_TEST_SKIP | NEVER_SKIP_DEINIT,              NULL},
  {DNX_INIT_STEP_FLEXE,                         "FlexE",                dnx_flexe_init,                 dnx_flexe_deinit,           flexe_skip,     0,                          NULL},
  {DNX_INIT_STEP_LAST_STEP,                     "Last Step",            NULL,                           NULL,                       NULL,           0,                          NULL}
};
...
static const dnx_init_step_t dnx_init_deinit_pp_seq[] = {
  ...
  {DNX_INIT_STEP_ARR,                   "ARR",                    dnx2_arr_output_init,                     NULL,                                 NULL,            WB_SKIP,                 NULL},
  {DNX_INIT_STEP_PP_GENERAL,            "PP General",             dnx_general_pp_init,                      NULL,                                 NULL,            WB_SKIP | TIME_PER_DEV,  NULL},
  {DNX_INIT_KLEAP_INFO,                 "iKleap info",            dnx2_kleap_layers_info_init,              NULL,                                 NULL,            0,                       NULL},
  ...
  {DNX_INIT_STEP_PTP,                   "Ptp",                    dnx_ptp_init,                             dnx_ptp_deinit,                       NULL,            0,                       NULL},
  {DNX_INIT_STEP_OAM,                   "OAM",                    dnx_oam_init,                             dnx_oam_deinit,                       oam_skip,       WB_SKIP | TIME_PER_DEV,   NULL},
  {DNX_INIT_STEP_BFD,                   "BFD",                    dnx_bfd_init,                             NULL,                                 oam_skip,       WB_SKIP,                  NULL},
  ...
  {DNX_INIT_STEP_FLOW,                  "flow",                   dnx_flow_init,                            dnx_flow_deinit,                      NULL,            0,                       NULL},
  {DNX_INIT_STEP_LAST_STEP,             "Last Step",              NULL,                                     NULL,                                 NULL,            0,                       NULL}
};
...
static const  dnx_init_step_t dnx_init_deinit_access_seq[] = {
  /*STEP_ID,                      STEP_NAME,                    STEP_INIT,                              STEP_DEINIT,                            FLAG_CB             STEP_FLAGS                               SUB_STEP_LIST_PTR */
  {DNX_INIT_STEP_IPROC,           "IRPOC_PAXB",                 dnx_drv_soc_dnx_iproc_config,           NULL,                                   NULL,               ACCESS | STANDALONE_EN,                          NULL},
  ...
  {DNX_INIT_STEP_EDK,             "EDK",                        dnx_edk_state_init,                     dnx_edk_state_deinit,                   NULL,               0,                                             NULL},
  {DNX_INIT_STEP_SBUS,            "SBUS",                       NULL,                                   NULL,                                   NULL,               ACCESS | STANDALONE_EN,                          dnx_init_deinit_sbus_seq},
#ifdef ADAPTER_SERVER_MODE
  {DNX_INIT_STEP_ADAPTER_REG_ACC,  "Adapter",                   dnxc_init_adapter_reg_access_init,       dnxc_init_adapter_reg_access_deinit,     NULL,               ACCESS | DISABLE_WB_CHECK,                       NULL},
#endif
  ...
  /** DMA step frees memory allocated in RX (APPL) step, thus should be ignored in memory tests */
  {DNX_INIT_STEP_DMA,             "DMA",                        dnx_drv_soc_dnx_dma_init,               dnx_drv_soc_dnx_dma_deinit,             NULL,               ACCESS | STANDALONE_EN | MEM_TEST_SKIP,          NULL},
  {DNX_INIT_STEP_LAST_STEP,       "Last Step",                  NULL,                                   NULL,                                   NULL,               0,                                               NULL}
};
...
static const  dnx_init_step_t dnx_init_deinit_algo_seq[] = {
  ...
  {DNX_INIT_STEP_ALGO_SAT,              "Algo SAT",                         dnx_algo_sat_init,                          NULL,                                           sat_skip,           WB_SKIP,                 NULL},
  {DNX_INIT_STEP_ALGO_BFD,              "Algo BFD",                         dnx_algo_bfd_init,                          NULL,                                           oam_skip,          WB_SKIP,                 NULL},
  {DNX_INIT_STEP_ALGO_OAM,              "Algo OAM",                         dnx_algo_oam_init,                          NULL,                                           oam_skip,          WB_SKIP,                 NULL},
  {DNX_INIT_STEP_ALGO_OAMP,             "Algo OAMP",                        dnx_algo_oamp_v1_init,                      NULL,                                           oam_skip,          WB_SKIP,                 NULL},
...
/**
 * \brief Init-Deinit swstate seq.
 *
 * For specific info on steps look for each function description
 */
static const dnx_init_step_t dnx_init_deinit_swstate_seq[] = {
  /*STEP_ID,                                 STEP_NAME,                            STEP_INIT,                             STEP_DEINIT,                 FLAG_CB                                  STEP_FLAGS                SUB_STEP_LIST_PTR */
  {DNX_INIT_STEP_SW_STATE_GENERAL,           "SW State General",                   dnx_sw_state_init,                     dnx_sw_state_deinit,         NULL,                                    ACCESS | TIME_PER_DEV,    NULL},
#ifdef INCLUDE_CTEST
  {DNX_INIT_STEP_SW_STATE_EXAMPLE,           "SW State Example",                   dnx_sw_state_example_init,             NULL,                        dnx_sw_state_example_flag_cb,            0,                        NULL},
  {DNX_INIT_STEP_SW_STATE_EXAMPLE_RES_MNGR,  "SW State Resource Manager Example",  dnx_sw_state_example_res_mngr_init,    NULL,                        dnx_sw_state_example_res_mngr_flag_cb,   0,                        NULL},
  {DNX_INIT_STEP_SW_STATE_EXAMPLE_TEMP_MNGR, "SW State Template Manager Example",  dnx_sw_state_example_temp_mngr_init,   NULL,                        dnx_sw_state_example_temp_mngr_flag_cb,  0,                        NULL},
#endif
  {DNX_INIT_STEP_LAST_STEP,                  "Last Step",                          NULL,                                  NULL,                        NULL,                                    0,                        NULL}
};
...
static const  dnx_init_step_t dnx_init_deinit_pre_soft_init_seq[] = {
  ...
  {DNX_INIT_STEP_BLOCKS_ENABLE,       "Blocks enable",      dnx_drv_soc_block_enable_set,               NULL,                         NULL,             ACCESS,                                 NULL},
  {DNX_INIT_STEP_DRAM_INITIAL_CONFIG, "DRAM initial config",dnx_dram_initial_config,                    NULL,                         NULL,             ACCESS | STANDALONE_EN | TIME_PER_DEV,  NULL},
  {DNX_INIT_STEP_BIST_TESTS,          "BIST tests",         dnx_drv_soc_dnx_perform_bist_tests,         NULL,                         NULL,             0,                                      NULL},
  {DNX_INIT_STEP_DRAM_PHY_BIST,       "DRAM PHY BIST",      dnx_hbm_phy_bist,                           NULL,                         hbm_apd_phy_skip, TIME_PER_DEV | WB_SKIP | PARALLEL,      NULL},
  ...
  {DNX_INIT_STEP_SEC_MEM_INIT,        "Sec Memories",       dnx_sec_mem_init,                           NULL,                         sec_skip,         WB_SKIP,                                NULL},
  {DNX_INIT_STEP_MEM_DEFAULTS,        "Memories",           dnx_init_mem_default_init,                  NULL,                         NULL,             WB_SKIP | TIME_PER_DEV,                 NULL},
  {DNX_INIT_STEP_CLOCK_POWER_DOWN,    "Clock power down",   dnx_init_clock_power_down,                  NULL,                         NULL,             WB_SKIP,                                NULL},
...
static const  dnx_init_step_t dnx_init_deinit_thread_seq[] = {
    ...
    {DNX_INIT_STEP_L2_CPU_LEARN_THREAD,         "L2 CPU learn thread",          dnx_l2_learn_mact_fifodma_cpu_learn_init,  dnx_l2_learn_mact_fifodma_cpu_learn_cleanup, bare_metal_skip,    0,                  NULL},
    /** PortMod thread is created in portmod_create */
    {DNX_INIT_STEP_PORTMOD_THREAD,              "PortMod thread",               NULL,                                      dnx_portmod_thread_stop,                     0,                  NEVER_SKIP_DEINIT,  NULL},
    ...
    {DNX_INIT_STEP_OAMP_EVENT_FIFODMA_THREAD,   "Oamp event fifodma thread",    dnx_oam_oamp_event_fifodma_init,           dnx_oam_oamp_event_fifodma_deinit,           0,                  0,                  NULL},
    {DNX_INIT_STEP_FLR_SW_LINKSCAN_THREAD,      "FLR SW linkscan thread",       dnx_flr_sw_linkscan_thread_init,           dnx_flr_sw_linkscan_thread_deinit,           sw_flr_thread_skip, 0,                  NULL},
    {DNX_INIT_STEP_LAST_STEP,                   "Last Step",                    NULL,                                      NULL,                                        NULL,               0,                  NULL}
};
...
const dnx_init_step_t dnx_init_deinit_seq[] = {
  ...
  {DNX_INIT_STEP_SW_STATE,        "SW State",                     NULL,                             NULL,                            NULL,               ACCESS | TIME_PER_DEV,                      dnx_init_deinit_swstate_seq},
  ...
  {DNX_INIT_STEP_SOFT_INIT,       "Soft Init",                    dnx_drv_soc_dnx_soft_init,        NULL,                            NULL,               ACCESS|WB_SKIP|STANDALONE_EN|TIME_PER_DEV|JOIN,  NULL},
