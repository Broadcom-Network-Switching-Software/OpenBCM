    ...
    int *nif_in_core)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, nif_port, nif_in_core));
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_local_to_global_phy_get(unit, core, nif_port, global_nif));
...
static shr_error_e
dnx_fc_nof_nif_for_core_get(
    int unit,
    bcm_core_t core,
    int *nof_nif)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_nof_phys_per_core_get(unit, core, nof_nif));
    ...
    if (bitmap_index_old != bitmap_index_new)
    {
        DNXCMN_CORES_ITER(unit, _SHR_CORE_ALL, core_iter)
        {
            ...
            for (qpair = 0; qpair < dnx_data_egr_queuing.params.nof_q_pairs_get(unit); qpair++)
            {
                ...
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_EGR_QUEUE, qpair);
    ...
    int *bitmap_index_new)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_fc_nif_in_core_get(unit, src_port, &nif_port));
    ...
    if ((map_sel == DNX_REC_PFC_MAP_SEL_GENERIC_PFC_BMP)
        || (map_sel == dnx_data_fc.calendar_common.rx_dest_pfc_generic_bmp_get(unit)))
    {
    ...
    int *category)
{
    ...
    {
        *src_sel = dnx_data_fc.calendar_common.tx_src_vsq_d_get(unit);
        *fc_index = BCM_COSQ_GPORT_VSQ_PP_GET(vsq_gport);
    }
    else
    {
        *src_sel = dnx_data_fc.calendar_common.tx_src_vsq_a_b_c_get(unit);
    ...
    int cosq_max)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, port, &core));
    SHR_IF_ERR_EXIT(dnx_algo_port_out_channel_get(unit, port, &channel));
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    else
    {
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_llfc_handled_in_cfc))
        {
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    else
    {
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_pfc_rec_cdu_shift))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_nof_phys_per_core_get(unit, core, &nof_phys_in_core));
            first_affected_nif_port = nof_phys_in_core - (2 * dnx_data_nif.eth.nof_lanes_in_cdu_get(unit));
            if ((nif_port >= first_affected_nif_port) && (source->cosq == 0))
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_PORT,
                                           ((nif_port ==
                                             (nof_phys_in_core - 1)) ? first_affected_nif_port : (nif_port + 1)));
            }
            else
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_PORT, nif_port);
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, MGMT)))
    {
        ...
        map_val = (is_delete ? 0 : bitmap_index_new);
        dest_sel =
            (is_delete ? dnx_data_fc.calendar_common.rx_dest_empty_get(unit) : dnx_data_fc.
             calendar_common.rx_dest_pfc_generic_bmp_get(unit));
    ...
    else
    {
        ...
        dest_sel =
            (is_delete ? dnx_data_fc.calendar_common.rx_dest_empty_get(unit) : dnx_data_fc.
             calendar_common.rx_dest_nif_llfc_get(unit));
    }
        ...
        if (target->cosq == -1)
        {
            ...
            switch (num_priorities)
            {
                case 1:
                    dest_sel =
                        (is_delete ? dnx_data_fc.calendar_common.rx_dest_empty_get(unit) : dnx_data_fc.
                         calendar_common.rx_dest_pfc_get(unit));
                    break;
                case 2:
                    dest_sel =
                        (is_delete ? dnx_data_fc.calendar_common.rx_dest_empty_get(unit) : dnx_data_fc.
                         calendar_common.rx_dest_port_2_prio_get(unit));
                    break;
                case 4:
                    dest_sel =
                        (is_delete ? dnx_data_fc.calendar_common.rx_dest_empty_get(unit) : dnx_data_fc.
                         calendar_common.rx_dest_port_4_prio_get(unit));
                    break;
                case 8:
                    dest_sel =
                        (is_delete ? dnx_data_fc.calendar_common.rx_dest_empty_get(unit) : dnx_data_fc.
                         calendar_common.rx_dest_port_8_prio_get(unit));
                    break;
                default:
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fc_nif_in_core_get(unit, port, &nif_port));
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FC_SRC_SEL,
                                 INST_SINGLE,
                                 (is_delete ? dnx_data_fc.calendar_common.
                                  tx_src_empty_get(unit) : dnx_data_fc.calendar_common.tx_src_glb_rsc_get(unit)));
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "Unsupported port (0x%x), must be NIF", source->port);
    }
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FC_SRC_SEL,
                                 INST_SINGLE,
                                 (is_delete ? dnx_data_fc.calendar_common.
                                  tx_src_empty_get(unit) : dnx_data_fc.calendar_common.tx_src_llfc_vsq_get(unit)));
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_first_phy_per_core_get(unit, core_id, &first_phy_in_core));
    fc_index = pg_base_vsq_id + (first_phy_in_core * BCM_COS_COUNT) + source->cosq;
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FC_SRC_SEL,
                                 INST_SINGLE,
                                 (is_delete ? dnx_data_fc.calendar_common.
                                  tx_src_empty_get(unit) : dnx_data_fc.calendar_common.tx_src_pfc_vsq_get(unit)));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FC_INDEX, INST_SINGLE, (is_delete ? 0 : fc_index));
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FC_SRC_SEL,
                                 INST_SINGLE,
                                 (is_delete ? dnx_data_fc.calendar_common.tx_src_empty_get(unit) : src_sel));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FC_INDEX, INST_SINGLE, (is_delete ? 0 : fc_index));
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    /** Link-Level FC (from the NIF-RX-MLF) that can be mapped to calendar are NIF ports + CMIC ports or ILKN ports depending on device.
      ...
      * Calendar LLFC index is equal to the ID of the NIF port, as the CMIC/ILKN ports are the last. However for ports on other cores
      * a calculation must be made. */
    llfc_index = nif_port + core * dnx_data_fc.general.nof_cal_llfcs_get(unit);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FC_SRC_SEL,
                                 INST_SINGLE,
                                 (is_delete ? dnx_data_fc.calendar_common.
                                  tx_src_empty_get(unit) : dnx_data_fc.calendar_common.tx_src_nif_llfc_get(unit)));
    ...
    bcm_cosq_fc_endpoint_t * endpoint)
{
    ...
    /** Verify endpoint calendar type */
    if ((BCM_GPORT_IS_CONGESTION(endpoint->port) && !BCM_GPORT_IS_CONGESTION_COE(endpoint->port) &&
         !dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_oob_support))
        || (BCM_GPORT_IS_CONGESTION_COE(endpoint->port)
            && !dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_support))
        || (!BCM_GPORT_IS_CONGESTION(endpoint->port)
            && !dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_ilkn_inband_support)))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Invalid Calendar type 0x%x for device!", endpoint->port);
    }
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    /** Verift port type - mapping PFC to Interface is not supported for CPU port */
    port =
        BCM_GPORT_IS_LOCAL(source->port) ? BCM_GPORT_LOCAL_GET(source->
                                                               port) : BCM_GPORT_LOCAL_INTERFACE_GET(source->port);
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
...
{
    uint32 entry_handle_id;
    uint32 entry_handle_id_oob_en = 0;
    uint32 src_sel_get;
    bcm_port_t port;
    bcm_port_t nif_port, src_port;
    uint32 fc_index_get;
    uint32 fc_index;
    uint32 enabled_oob = 0, enabler_valid = 0;
    ...
    if (src_sel == dnx_data_fc.calendar_common.tx_src_glb_rsc_get(unit))
    {
    ...
    else if (src_sel == dnx_data_fc.calendar_common.tx_src_pfc_vsq_get(unit))
    {
        pg_base_vsq_id = BCM_COSQ_GPORT_VSQ_PG_GET(source->port);
        core_id = BCM_COSQ_GPORT_VSQ_CORE_ID_GET(source->port);
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_first_phy_per_core_get(unit, core_id, &first_phy_in_core));
    ...
    else if (src_sel == dnx_data_fc.calendar_common.tx_src_llfc_vsq_get(unit))
    {
        src_port_vsq_id = BCM_COSQ_GPORT_VSQ_SRC_PORT_GET(source->port);
        core_id = BCM_COSQ_GPORT_VSQ_CORE_ID_GET(source->port);
        fc_index = src_port_vsq_id + dnx_data_ingr_congestion.vsq.vsq_e_nof_get(unit) * core_id;
    }
    else if (src_sel == dnx_data_fc.calendar_common.tx_src_nif_llfc_get(unit))
    {
        src_port =
            BCM_GPORT_IS_LOCAL(source->
                               port) ? BCM_GPORT_LOCAL_GET(source->port) : BCM_GPORT_LOCAL_INTERFACE_GET(source->port);
        ...
        /** Link-Level FC (from the NIF-RX-MLF) that can be mapped to OOB are NIF ports + 4 CMIC ports.
          * This API allows mapping of the NIF ports, but the correct offset needs to be calculated. For ports on core 0 the
          * OOB LLFC index is equal to the ID of the NIF port, as the CMIC ports are the last 4. However for ports on other cores
          * a calculation must be made. */
        fc_index = nif_port + core_id * dnx_data_fc.general.nof_cal_llfcs_get(unit);
    }
    else
    {
        /** Determine the VSQ properties, FC index and source select from the VSQ Gport */
    ...
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_oob_support))
    {
        ...
        for (cal_if_cur = 0; cal_if_cur < dnx_data_fc.oob.nof_oob_ids_get(unit); cal_if_cur++)
        {
            ...
            {
                if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_glb_rsc_oob_en)
                    && (src_sel == dnx_data_fc.calendar_common.tx_src_glb_rsc_get(unit)))
                {
                    ...
                    dbal_value_field32_request(unit, entry_handle_id_oob_en, DBAL_FIELD_ENABLE, INST_SINGLE,
                                               &enabled_oob);
                ...
                if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_cat2_oob_en)
                    && (src_sel == dnx_data_fc.calendar_common.tx_src_vsq_a_b_c_get(unit) && (category == 2)
                        && (traffic_class != -1)))
                {
                    dbal_entry_key_field32_set(unit, entry_handle_id_oob_en, DBAL_FIELD_OOB_IF, cal_if_cur);
                    dbal_entry_key_field32_set(unit, entry_handle_id_oob_en, DBAL_FIELD_CAL_IDX, cal_index);
    ...
    int *target_count)
{
    ...
    else
    {
        ...
        {
            /** Port is not CPU, get the NIF port that is mapped to it */
            SHR_IF_ERR_EXIT(dnx_fc_nif_in_core_get(unit, src_port, &nif_port));
    ...
    int *target_count)
{
    ...
    if (dest_sel == dnx_data_fc.calendar_common.rx_dest_pfc_generic_bmp_get(unit))
    {
        /*
    ...
    else if ((dest_sel == dnx_data_fc.calendar_common.rx_dest_pfc_get(unit))
             || (dest_sel == dnx_data_fc.calendar_common.rx_dest_port_2_prio_get(unit))
             || (dest_sel == dnx_data_fc.calendar_common.rx_dest_port_4_prio_get(unit))
             || (dest_sel == dnx_data_fc.calendar_common.rx_dest_port_8_prio_get(unit)))
    {
        /*
         * Reaction point is Port
         */
        ...
        BCM_GPORT_LOCAL_SET(target[*target_count].port, port);
        if (dest_sel != dnx_data_fc.calendar_common.rx_dest_pfc_get(unit))
        {
            target[*target_count].cosq = -1;
        }
        *target_count += 1;
    }
    else if (dest_sel == dnx_data_fc.calendar_common.rx_dest_nif_llfc_get(unit))
    {
    ...
    int *target_count)
{
   ...
   /** Determine the FC index , resource type and priority according to the source */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_fc_nof_nif_for_core_get(unit, core, &nof_nifs_for_core));
        for (if_id = 0; if_id < nof_nifs_for_core; if_id++)
        {
            ...
            if (enabled_llfc)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_fc_global_nif_get(unit, core, if_id, &nif_port));
    ...
    SHR_IF_ERR_EXIT(dnx_fc_gen_to_cal_targets_get
                    (unit, dnx_data_fc.calendar_common.tx_src_glb_rsc_get(unit), source, target_max, target,
                     target_count));
    ...
    int *target_count)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fc_gen_to_cal_targets_get
                    (unit, dnx_data_fc.calendar_common.tx_src_llfc_vsq_get(unit), source, target_max, target,
                     target_count));
    ...
    int *target_count)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fc_gen_to_cal_targets_get
                    (unit, dnx_data_fc.calendar_common.tx_src_pfc_vsq_get(unit), source, target_max, target,
                     target_count));
    ...
    int *target_count)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fc_gen_to_cal_targets_get
                    (unit, dnx_data_fc.calendar_common.tx_src_vsq_a_b_c_get(unit), source, target_max, target,
                     target_count));
    ...
    int *target_count)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fc_gen_to_cal_targets_get
                    (unit, dnx_data_fc.calendar_common.tx_src_nif_llfc_get(unit), source, target_max, target,
                     target_count));
    ...
    uint8 is_set)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_eth_port_threshold_set_get_verify(unit, gport, threshold, is_set));
    ...
    uint8 is_set)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_vsq_gl_total_fc_threshold_verify(unit, core_id, index_type,
    ...
    uint8 is_set)
{
    ...
    /** Verify API arguments */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_vsq_gl_per_pool_fc_threshold_verify(unit, core_id, pool_id,
                                                                        priority, thresh_info, &threshold_internal,
                                                                        is_set));
...
void
dnx_qlmf_thresholds_profile_template_print_cb(
    int unit,
    const void *data)
{
#ifdef DNX_SW_STATE_DIAGNOSTIC
    dnx_rmc_thresholds_t *rmc_thresholds = (dnx_rmc_thresholds_t *) data;
#endif
    SW_STATE_PRETTY_PRINT_INIT_VARIABLES();
    SW_STATE_PRETTY_PRINT_SUB_STRUCT_START();
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "PFC Set Threshold",
                                   rmc_thresholds->pfc_set_thresh, NULL, "0x%x");
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "PFC Clear Threshold",
                                   rmc_thresholds->pfc_clr_thresh, NULL, "0x%x");
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "LLFC Set Threshold",
                                   rmc_thresholds->llfc_set_thresh, NULL, "0x%x");
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "LLFC Clear Threshold",
                                   rmc_thresholds->llfc_clr_thresh, NULL, "0x%x");
    SW_STATE_PRETTY_PRINT_SUB_STRUCT_END();
    SW_STATE_PRETTY_PRINT_FINISH();
}
    ...
    int unit)
{
    ...
    dnx_rmc_thresholds_t default_thresholds;
    dnx_algo_template_create_data_t rmc_thresholds;
    uint32 max_ref = 0;
    ...
    /**
     * Calculate the max references for the template - theoretically it is possible that the all calendars
     * (all interfaces) and all PFCs will be mapped to only one bitmap, so max_ref must be the size of the calendars + all PFCs.
     */
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_oob_support))
    {
    ...
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_ilkn_inband_support))
    {
        for (interface_number = 0; interface_number < dnx_data_fc.inband.nof_inband_intlkn_cal_instances_get(unit);
             interface_number++)
        {
            max_ref += dnx_data_fc.inband.calender_length_get(unit, interface_number)->rx;
        }
    }
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_support))
    {
        for (interface_number = 0; interface_number < dnx_data_fc.coe.nof_coe_cal_instances_get(unit);
             interface_number++)
        {
            max_ref += dnx_data_fc.coe.calender_length_get(unit, interface_number)->len;
        }
    }
    {
    ...
    /** Create template manager insance - handle: TEMPLATE_PFC_GENERIC_BMP for the index of generic PFC bitmaps */
    sal_memset(&default_data, 0, sizeof(dnx_pfc_generic_bmp_t));
    ...
    /**
     * For some devices the allocation does not start from 0. This is because bitmap 0 is reserved,
     * meaning it will not be available to user because it is used for for empty entries in the calendars,
     * in order to avoid accidental
     */
    fc_generic_bmp.first_profile = dnx_data_fc.general.gen_bmp_first_id_get(unit);
    /*
     * We have X in total Generic Bitmaps in HW (0 to (X-1)).
     * Number of profiles is X+1 and not X because the profile with index X+1 is used as a default profile and does not correspond
     ...
     */
    fc_generic_bmp.nof_profiles =
        (dnx_data_fc.general.nof_gen_bmps_get(unit) - dnx_data_fc.general.gen_bmp_first_id_get(unit)) + 1;
    fc_generic_bmp.default_profile = dnx_data_fc.general.nof_gen_bmps_get(unit);
    ...
    sal_strncpy(fc_generic_bmp.name, TEMPLATE_PFC_GENERIC_BMP, DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    ...
    {
        sal_memset(&default_thresholds, 0, sizeof(dnx_rmc_thresholds_t));
        sal_memset(&rmc_thresholds, 0, sizeof(dnx_algo_template_create_data_t));
        default_thresholds.pfc_set_thresh = dnx_data_fc.general.rmc_pfc_set_threshold_default_get(unit);
        default_thresholds.pfc_clr_thresh = dnx_data_fc.general.rmc_pfc_clear_threshold_default_get(unit);
        default_thresholds.llfc_set_thresh = dnx_data_fc.general.rmc_llfc_set_threshold_default_get(unit);
        default_thresholds.llfc_clr_thresh = dnx_data_fc.general.rmc_llfc_clear_threshold_default_get(unit);
        rmc_thresholds.flags = DNX_ALGO_TEMPLATE_CREATE_USE_DEFAULT_PROFILE;
        rmc_thresholds.first_profile = 0;
        rmc_thresholds.nof_profiles = dnx_data_fc.general.nof_rmc_thresholds_profiles_get(unit);
        rmc_thresholds.default_profile = 0;
        rmc_thresholds.data_size = sizeof(dnx_rmc_thresholds_t);
        rmc_thresholds.default_data = &default_thresholds;
        rmc_thresholds.max_references =
            dnx_data_nif.phys.nof_phys_get(unit) * dnx_data_nif.eth.priority_groups_nof_get(unit);
        sal_strncpy(rmc_thresholds.name, "Template for QMLF FC Thresholds", DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
        SHR_IF_ERR_EXIT(flow_control_db.rmc_thresholds.create(unit, &rmc_thresholds, NULL));
    }

    {
        ...
        /** Disable PFC mask on link failure for all ETHUs */
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_disable_pfc_mask_on_link_fail))
        {
            SHR_IF_ERR_EXIT(imb_pfc_mask_on_link_fail_enable_all(unit, FALSE));
        }
    ...
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_additional_enablers))
    {
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_oob_support))
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_OOB_RX_TO_NIF_FAST_LLFC_EN, INST_SINGLE,
                                         1);
        }
    }
    ...
    /** START OF ILKN-INBAND FC CONFIGURATIONS */
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_ilkn_inband_support))
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FC_SRC_SEL, INST_SINGLE,
                                     dnx_data_fc.calendar_common.tx_src_empty_get(unit));
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        ...
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CAL_IF, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CAL_IDX, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DEST_SEL, INST_SINGLE,
                                     dnx_data_fc.calendar_common.rx_dest_empty_get(unit));
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    /** END OF ILKN-INBAND FC CONFIGURATIONS */

    /** START OF OOB FC CONFIGURATIONS */
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_oob_support))
    {
        ...
        for (interface_number = 0; interface_number < dnx_data_fc.oob.nof_oob_ids_get(unit); interface_number++)
        {
                ...
                (dnx_data_fc.oob.port_get(unit, interface_number)->mode == DNX_FC_CAL_MODE_TX_RX_ENABLE))
            {
                ...
                if (dnx_data_fc.oob.port_get(unit, interface_number)->type == DNX_OOB_TYPE_ILKN)
                {
                    ...
                    {
                        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_OOB_RX_FC_POLARITY,
                                                     INST_SINGLE, 1);
                    }
                ...
                if (dnx_data_fc.oob.port_get(unit, interface_number)->type == DNX_OOB_TYPE_SPI)
                {
                    ...
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SPI_OOB_RX_CAL_LEN, INST_SINGLE,
                                                 (dnx_data_fc.oob.calender_length_get(unit, interface_number)->rx));
                ...
                (dnx_data_fc.oob.port_get(unit, interface_number)->mode == DNX_FC_CAL_MODE_TX_RX_ENABLE))
            {
                ...
                if (dnx_data_fc.oob.port_get(unit, interface_number)->type == DNX_OOB_TYPE_ILKN)
                {
                    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_ilkn_polarity_set))
                    {
                        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_OOB_TX_FC_POLARITY,
                                                     INST_SINGLE, 1);
                    }
                ...
                if (dnx_data_fc.oob.port_get(unit, interface_number)->type == DNX_OOB_TYPE_SPI)
                {
                    ...
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SPI_OOB_TX_CAL_LEN, INST_SINGLE,
                                                 (dnx_data_fc.oob.calender_length_get(unit, interface_number)->tx));
    ...
    /** END OF OOB FC CONFIGURATIONS */

    /** START OF COE FC CONFIGURATIONS */
    if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_support))
    {
             ...
             interface_number++)
        {
            ...
            {
                /** COE Channel ID is based on PTC+VID mapping - configure parameters to get VID from the packet header */
                ...
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COE_PTC_MASK, INST_SINGLE,
                                             dnx_data_fc.coe.coe_ptc_mask_get(unit));
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COE_PTC_SHIFT_LEFT, INST_SINGLE,
                                             dnx_data_fc.coe.coe_ptc_left_shift_get(unit));
            }
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }
