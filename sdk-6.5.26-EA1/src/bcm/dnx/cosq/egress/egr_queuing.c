    ...
    uint32 kbits_sec_speed)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ofp_rates_calculate_credits_from_rate(unit, 1, 1, 1, FALSE, kbits_sec_speed, &internal_rate));
    if (is_mirror)
    {
    ...
    uint32 credit_size)
{
    ...
    {
        /*
         * at the moment only mode="init credit with value" is supported. When new modes will be supported enum should
         * be used.
         */
        SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_INTERFACE_REG_ATTRIBUTES,
                                         2, 1,
                                         GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, (uint32) core,
                                         GEN_DBAL_FIELD32, DBAL_FIELD_EGQ_IF, (uint32) egr_if,
                                         GEN_DBAL_FIELD32, DBAL_FIELD_CREDIT_MODE, INST_SINGLE, (uint32) 1,
                                         GEN_DBAL_FIELD_LAST_MARK));
        SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_INTERFACE_REG_ATTRIBUTES,
                                         2, 1,
                                         GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, (uint32) core,
                                         GEN_DBAL_FIELD32, DBAL_FIELD_EGQ_IF, (uint32) egr_if,
                                         GEN_DBAL_FIELD32, DBAL_FIELD_CREDIT_MODE, INST_SINGLE, (uint32) 0,
                                         GEN_DBAL_FIELD_LAST_MARK));
    }
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_port_t port)
{
    ...
    else
    {
        ...
        /*
         * for NIF ports lp txq needs to be configured as well 
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    ...
    DNXCMN_CORES_ITER(unit, core, core_id)
    {
        ...
        for (cal_type = 0; cal_type < NUM_DNX_EGR_CAL_TYPE; cal_type++)
        {
            ...
            for (ii = 0; ii < nif_cal_len; ii++)
            {
                ...
                /*
                 * For devices supporting calendar windows the number of windows (64 slots) need to be configured to HW.
                 * No need to farther configuring the calendar after that.
                 */
                if (dnx_data_egr_queuing.
                    params.feature_get(unit, dnx_data_egr_queuing_params_cal_windows_is_supported) == TRUE)
                {
                    if ((ii / 64) >= UTILEX_DIV_ROUND_UP(cal_len_optimal, 64))
                    {
                        SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_GLOBAL_CONFIGURATION,
                                                         1, 1,
                                                         GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, core_id,
                                                         GEN_DBAL_FIELD32, DBAL_FIELD_CAL_NOF_WINDOWS, INST_SINGLE,
                                                         UTILEX_DIV_ROUND_UP(ii, 64), GEN_DBAL_FIELD_LAST_MARK));
                        break;
                    }
                }
            }
    ...
    bcm_port_t logical_port)
{
    ...
    queue_priority = ((is_tdm == TRUE) || (is_flexe_dedicated == TRUE)) ? BCM_COSQ_SP0 : BCM_COSQ_SP1;
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_am_template_egress_queue_mapping_create(unit, profile, mapping_info));
    {
        ...
        if (dnx_data_egr_queuing.params.feature_get(unit, dnx_data_egr_queuing_params_cal_read_en_is_supported) == 1)
        {
            SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_GLOBAL_CONFIGURATION, 1, 1,
                                             GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, (uint32) core,
        ...
        if (dnx_data_egr_queuing.reassembly.feature_get(unit, dnx_data_egr_queuing_reassembly_prs))
        {
            ...
            if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE && is_tdm_supported == 1)
            {
                if (dnx_data_tdm.params.mode_get(unit) != TDM_MODE_NONE)
                {
                    SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_GLOBAL_CONFIGURATION, 1, 3,
                                                     GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, (uint32) core,
                                                     GEN_DBAL_FIELD32, DBAL_FIELD_PRS_TIMEOUT_RESOLUTION_A, INST_SINGLE,
                                                     0xb, GEN_DBAL_FIELD32, DBAL_FIELD_PRS_TIMEOUT_CFG_A, INST_SINGLE,
                                                     0x2, GEN_DBAL_FIELD32, DBAL_FIELD_PRS_TIMEOUT_EN, INST_SINGLE, 1,
                                                     GEN_DBAL_FIELD_LAST_MARK));
                }
            }
        }
    ...
    if (dnx_data_mdb.feature.feature_get(unit, dnx_data_mdb_feature_bubble_pulse_width_too_short))
    {
        ...
        if (dnx_data_egr_queuing.params.feature_get(unit, dnx_data_egr_queuing_params_const_rqp_fqp_bubbles_disabled))
        {
            /** Disable RQP and FQP constant bubble generation and configure the ECGM bubble generation */
            SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_GLOBAL_CONFIGURATION, 1, 5,
                                             GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, BCM_CORE_ALL,
                                             GEN_DBAL_FIELD32, DBAL_FIELD_RQP_BUBBLE_REQ_EN, INST_SINGLE, FALSE,
                                             GEN_DBAL_FIELD32, DBAL_FIELD_RQP_CONST_BUBBLE_EN, INST_SINGLE, FALSE,
                                             GEN_DBAL_FIELD32, DBAL_FIELD_FQP_CONST_BUBBLE_EN, INST_SINGLE, FALSE,
                                             GEN_DBAL_FIELD32, DBAL_FIELD_ECGM_FORCE_BUBBLE_PULSE_WIDTH, INST_SINGLE,
                                             0x4, GEN_DBAL_FIELD32, DBAL_FIELD_ECGM_BUBBLE_REQ_EN, INST_SINGLE, TRUE,
                                             GEN_DBAL_FIELD_LAST_MARK));
        }
        else
        {
