...
/** return error if the field is too small for the value - See header file for more details */
shr_error_e
dnx_cosq_dbal_2_complement_field_value_check(
    int unit,
    dbal_tables_e table_id,
    ...
    int value)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /** The value in HW is in two's compliment, so we need the size in order to convert to signed number. */
    SHR_IF_ERR_EXIT(dbal_tables_field_size_get(unit, table_id, field_id, FALSE, 0, 0, &field_size));
    max_value = (1 << (field_size - 1)) - 1;
    if (utilex_abs(value) > max_value)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "value %d not compatible to field size (%d)", value, field_size);
    }
