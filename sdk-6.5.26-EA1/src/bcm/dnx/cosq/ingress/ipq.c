    ...
    uint32 alloc_flags)
{
    ...
    extra_create_info.grain_size[0] = region_nof_quartets * DNX_COSQ_IPQ_BITMAP_ELEMS_PER_QUEUE_BUNDLE(unit);

    /*
     * Tag is actually the queue type. so max tag value is [nof_queue_types - 1]
     */
    extra_create_info.max_tag_value[0] = NUM_DNX_ALGO_IPQ_QUEUE_TYPE - 1;
