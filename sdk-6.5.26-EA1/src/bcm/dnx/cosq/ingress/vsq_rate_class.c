    ...
    dnx_ingress_congestion_vsq_f_resource_alloc_params_t * resource_alloc_params)
{
    ...
    sal_memset(&resource_alloc_hw_params, 0x0, sizeof(dnx_ingress_congestion_vsq_f_resource_alloc_params_t));
    ...
    int is_clear)
{
                                 ...
                                 dnx_data_ingr_congestion.vsq.pool_nof_get(unit) : 1); pool_id++)
    {
        ...
        for (dp_index = 0; dp_index < DNX_COSQ_NOF_DP; dp_index++)
        {
            ...
            if (is_clear == TRUE)
            {
                SHR_IF_ERR_EXIT(dnx_rate_class_common_wred_clear(unit, wred_table_id, &wred_key));
            }
            else
            {
    ...
    if (wred_exp_weight_ptr == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "wred_exp_weight_ptr is NULL. Illegal.\r\n");
    }
    ...
    dnx_ingress_congestion_vsq_rate_class_info_t * data_rate_class)
{
    ...
    if (is_allocated)
    {
        SHR_IF_ERR_EXIT(dnx_vsq_rate_class_hw_set(unit, core_id, new_rate_class, vsq_type, data_rate_class, FALSE));
    }
    ...
    if (is_last && new_rate_class != old_rate_class)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_vsq_rate_class_hw_set(unit, core_id, old_rate_class, vsq_type, &cleared_data_rate, TRUE));
    }
