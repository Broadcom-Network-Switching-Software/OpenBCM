...
#define UDP_PORTS_ARE_0(_udp_ports) (((_udp_ports).src_port == 0) && ((_udp_ports).dst_port == 0))
...
static shr_error_e
flow_ipvx_tunnel_initiator_udp_profiles_free_and_hw_update(
    int unit,
    udp_ports_t * udp_ports)
{
    uint8 is_last_reference = FALSE;
    int udp_profile = 0;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(algo_tunnel_db.udp_ports_profile.profile_get(unit, udp_ports, &udp_profile));
    SHR_IF_ERR_EXIT(algo_tunnel_db.udp_ports_profile.free_single(unit, udp_profile, &is_last_reference));
    if (is_last_reference)
    {
        SHR_IF_ERR_EXIT(dnx_egress_additional_header_map_tables_configuration_udp_ports_clear(unit, udp_profile));
    }
exit:
    SHR_FUNC_EXIT;
}
...
int
flow_ipvx_tunnel_intiator_is_valid(
    int unit)
{
...
dbal_fields_e
dbal_field_lif_ahp_gre(
    int unit)
{
    return DBAL_FIELD_LIF_AHP_GRE_GENEVE;
}

int
dbal_enum_fval_lif_ahp_gre8_w_key(
    int unit)
{
    return DBAL_ENUM_FVAL_LIF_AHP_GRE_GENEVE_LIF_AHP_GRE8_W_KEY;
}

int
dbal_enum_fval_lif_ahp_gre8_w_sn(
    int unit)
{
    return DBAL_ENUM_FVAL_LIF_AHP_GRE_GENEVE_LIF_AHP_GRE8_W_SN;
}

int
dbal_enum_fval_lif_ahp_gre_only(
    int unit)
{
    return DBAL_ENUM_FVAL_LIF_AHP_GRE_GENEVE_LIF_AHP_NONE;
}

static void
flow_ipvx_tunnel_initiator_lif_ahp_gre_field_and_value_calculate(
    int unit,
    uint32 gre_flag,
    dbal_fields_e * lif_ahp_field,
    int *lif_ahp_data)
{
    *lif_ahp_field = dbal_field_lif_ahp_gre(unit);
    if (gre_flag == BCM_TUNNEL_INIT_GRE_KEY_USE_LB)
    {
        *lif_ahp_data = dbal_enum_fval_lif_ahp_gre8_w_key(unit);
    }
    else if (gre_flag == BCM_TUNNEL_INIT_GRE_WITH_SN)
    {
        *lif_ahp_data = dbal_enum_fval_lif_ahp_gre8_w_sn(unit);
    }
    else
    {
        *lif_ahp_data = dbal_enum_fval_lif_ahp_gre_only(unit);
    }
}
    ...
    int *lif_ahp_data)
{
    ...
    switch (tunnel_type)
    {
        ...
        case bcmTunnelTypeGreAnyIn6:
            flow_ipvx_tunnel_initiator_lif_ahp_gre_field_and_value_calculate(unit, gre_flag, lif_ahp_field,
                                                                             lif_ahp_data);
            break;
...
static shr_error_e
flow_ipvx_tunnel_initiator_special_fields_udp_ports_get(
    int unit,
    bcm_flow_special_fields_t * special_fields,
    udp_ports_t * udp_ports)
{
...
static shr_error_e
flow_ipvx_tunnel_initiator_udp_ports_set(
    int unit,
    bcm_tunnel_type_t tunnel_type,
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_special_fields_udp_ports_get(unit, special_fields, &udp_ports));
    if (UDP_PORTS_ARE_0(udp_ports))
    {
        SHR_EXIT();
    }
...
shr_error_e
flow_ipvx_tunnel_initiator_tunnel_type_and_derivatives_set(
    int unit,
    uint32 entry_handle_id,
    bcm_tunnel_type_t tunnel_type,
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_udp_ports_set(unit, tunnel_type, special_fields));
...
shr_error_e
flow_ipvx_tunnel_initiator_flow_tunnel_type_and_lif_ahp_get(
    int unit,
    uint32 entry_handle_id,
    ...
    uint32 *lif_ahp)
{
    ...
    /** tunnel_type bcmTunnelTypeGreAnyIn4 type */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, dbal_field_lif_ahp_gre(unit), &is_field_valid, lif_ahp));

    if (is_field_valid)
    {
        *flow_ipvx_tunnel_type = TUNNEL_TYPE_GRE;
        SHR_EXIT();
    }

    /** tunnel_type bcmTunnelTypeUdp type */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_LIF_AHP_UDP, &is_field_valid, lif_ahp));
    if (is_field_valid)
    {
        *flow_ipvx_tunnel_type = TUNNEL_TYPE_UDP;
        SHR_EXIT();
    }

    /** tunnel_type bcmTunnelTypeIpAnyIn4 type */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_LIF_AHP_IP, &is_field_valid, lif_ahp));
    if (is_field_valid)
    {
        *flow_ipvx_tunnel_type = TUNNEL_TYPE_RAW;
        SHR_EXIT();
    }

    /** tunnel_type vxlan (2 options) */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_LIF_AHP_VXLAN_VXLAN_GPE, &is_field_valid, lif_ahp));
    if (is_field_valid)
    {

        if (*lif_ahp == DBAL_ENUM_FVAL_LIF_AHP_VXLAN_VXLAN_GPE_LIF_AHP_VXLAN_GPE)
        {
            *flow_ipvx_tunnel_type = TUNNEL_TYPE_VXLAN_GPE;
        }
        else
        {
            *flow_ipvx_tunnel_type = TUNNEL_TYPE_VXLAN;
        }
        SHR_EXIT();
    }

    SHR_ERR_EXIT(_SHR_E_UNAVAIL, "can't find tunnel type. Didn't find any field which can be used for lif ahp. ");
    ...
    bcm_tunnel_type_t tunnel_type)
{
            ...
            (tunnel_type == bcmTunnelTypeVxlan6) || (tunnel_type == bcmTunnelTypeVxlan6Gpe))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_init_esem_command_alloc_and_set_hw(unit, 0, 0, TUNNEL_INIT_ESEM_ACCESS_TYPE_VNI,
                                                                      dnx_data_esem.access_cmd.no_action_get(unit),
                                                                      &esem_cmd_hw_value, (uint8 *) &dummy));
...
flow_ipvx_tunnel_initiator_udp_port_profile_replace(
    int unit,
    bcm_flow_special_fields_t * new_special_fields,
    bcm_flow_special_fields_t * prev_special_fields,
    uint32 prev_entry_handle_id,
    udp_ports_t * new_udp_ports)
{
    udp_ports_t prev_udp_ports;
    uint8 has_prev_udp_ports = FALSE;
    uint8 has_new_udp_ports = FALSE;

    SHR_FUNC_INIT_VARS(unit);

    sal_memset(&prev_udp_ports, 0, sizeof(udp_ports_t));

    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_special_fields_udp_ports_get
                    (unit, prev_special_fields, &prev_udp_ports));
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_special_fields_udp_ports_get(unit, new_special_fields, new_udp_ports));

    has_prev_udp_ports = !UDP_PORTS_ARE_0(prev_udp_ports);
    has_new_udp_ports = !UDP_PORTS_ARE_0(*new_udp_ports);

    if (!has_prev_udp_ports && has_new_udp_ports)
    {
        SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_udp_profile_allocate_and_hw_set(unit, new_udp_ports));
    }
    else if (has_prev_udp_ports && !has_new_udp_ports)
    {
        SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_udp_profiles_free_and_hw_update(unit, &prev_udp_ports));
    }
    else if (has_prev_udp_ports && has_new_udp_ports)
    {
        SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_udp_profile_replace_and_hw_update
                        (unit, new_udp_ports, prev_entry_handle_id));
    }
    else
    {
        /*
         * prev and new haven't any udp profile, nothing to do 
         */
    }
exit:
    SHR_FUNC_EXIT;
...
shr_error_e
flow_ipvx_tunnel_initiator_tunnel_type_and_derivatives_replace(
    int unit,
...
flow_ipvx_tunnel_initiator_special_field_gre_flags_add(
    ...
    bcm_flow_special_fields_t * special_fields)
{
    if (lif_ahp == DBAL_ENUM_FVAL_LIF_AHP_GRE_GENEVE_LIF_AHP_GRE8_W_KEY)
    {
        SPECIAL_FIELD_ENABLER_ADD(special_fields, FLOW_S_F_IPVX_INIT_GRE_KEY_USE_LB);
    }
    else if (lif_ahp == DBAL_ENUM_FVAL_LIF_AHP_GRE_GENEVE_LIF_AHP_GRE8_W_SN)
    {
...
shr_error_e
flow_ipvx_tunnel_initiator_special_fields_tunnel_type_derivatives_add(
    int unit,
    flow_ipvx_tunnel_type_t flow_ipvx_tunnel_type,
    uint32 lif_ahp,
    bcm_flow_special_fields_t * special_fields)
{
    SHR_FUNC_INIT_VARS(unit);
    if (flow_ipvx_tunnel_type == TUNNEL_TYPE_GRE)
    {
        flow_ipvx_tunnel_initiator_special_field_gre_flags_add(lif_ahp, special_fields);
    }

    if (flow_ipvx_tunnel_type == TUNNEL_TYPE_UDP)
    {
        SHR_IF_ERR_EXIT(flow_ipvx_tunnel_initiator_special_field_udp_ports_add(unit, lif_ahp, special_fields));
    }
exit:
    SHR_FUNC_EXIT;
}
