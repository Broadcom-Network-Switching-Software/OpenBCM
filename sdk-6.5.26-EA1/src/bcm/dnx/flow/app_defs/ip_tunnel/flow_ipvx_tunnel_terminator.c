    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    if (is_fec)
    {
         ...
         */
        BCM_L3_ITF_FEC_TO_GPORT_FORWARD_GROUP(gport_forward_port, dest_port);
...
static shr_error_e
flow_ipvx_tunnel_terminator_in_lif_profile_init(
    int unit,
    uint32 current_entry_handle_id,
    bcm_flow_handle_info_t * flow_handle_info,
    void *app_entry_data)
{
    uint32 common_fields_bitmap = ((bcm_flow_terminator_info_t *) app_entry_data)->valid_elements_set;
    uint32 ingress_info_bitmap = (BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID |
                                  BCM_FLOW_TERMINATOR_ELEMENT_L3_INGRESS_INFO_VALID);
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(flow_ipvx_tunnel_terminator_in_lif_profile_init
                    (unit, current_entry_handle_id, flow_handle_info, app_entry_data));
