    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    uint8 is_field_valid, is_replace = 0;
    uint8 bitstr_field_exist = 0;
    uint32 orig_bit_str[4] = { 0 };
    ...
    if (!is_replace || bitstr_field_exist)
    {
    ...
    {
        dbal_entry_handle_value_field_arr32_get(unit, prev_entry_handle_id,
                                                DBAL_FIELD_BFR_BIT_STR, INST_SINGLE, orig_bit_str);
        dbal_entry_value_field_arr32_set(unit, entry_handle_id, DBAL_FIELD_BFR_BIT_STR, INST_SINGLE, orig_bit_str);
    }
    ...
    SHR_IF_ERR_EXIT(dnx_lif_lib_encap_access_to_logical_phase_convert
                    (unit, next_entry_encap_access[init_info->encap_access], &outlif_info.logical_phase));
    ...
    if (is_replace)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                        (unit, prev_entry_handle_id, DBAL_FIELD_NEXT_OUTLIF_POINTER, &is_field_valid, &field_val));
        local_outlif = field_val;
    }
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (is_field_valid)
    {
        ...
        if (next_pointer != 0)
        {
            ...
            {
                BCM_GPORT_FLOW_LIF_TO_L3_ITF_LIF(init_info->l3_intf_id, next_gport);
                init_info->valid_elements_set |= BCM_FLOW_INITIATOR_ELEMENT_L3_INTF_ID_VALID;
            }
        }
    }
