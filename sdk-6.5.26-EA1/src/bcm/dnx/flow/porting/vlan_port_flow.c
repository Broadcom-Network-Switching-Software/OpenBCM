    ...
    bcm_vlan_port_t * vlan_port)
{
     ...
     * For Native:
     *    - zero the dest port
     * Need to handle native symmetric
     */
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
    {
        ...
        {
            vlan_port_flow_term->l2_learn_info.dest_port = 0;
            destination = 0;
        }
        else
        {
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    /*
     *
     * What about BCM_FLOW_TERMINATOR_ELEMENT_QOS_INGRESS_MODEL_VALID?
     *     -    DBAL_FIELD_PROPAGATION_PROFILE:
     *        for ingress native = DNX_QOS_INGRESS_PROPAGATION_PROFILE_UNIFORM
     *        for ingress non-native = DNX_QOS_INGRESS_PROPAGATION_PROFILE_PIPE
     *        for control LIF =  (phb == uniform)? UNIFORM:PIPE
     */
    ...
    if (vlan_port->match_ethertype == 0)
    {
            ...
            (vlan_port->criteria != BCM_VLAN_PORT_MATCH_NONE))
        {
            ...
            SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                            (unit, vlan_port->port, _SHR_GPORT_TYPE_FLOW_LIF,
    ...
    if ((_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE)) && (vlan_port->tunnel_id != 0))
    {
        ...
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, vlan_port->tunnel_id,
                         _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR, &flow_id));
    ...
    bcm_vlan_port_t * old_vlan_port)
{
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_WITH_ID))
    {
        ...
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, vlan_port->vlan_port_id, _SHR_GPORT_TYPE_FLOW_LIF,
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE) && (!is_replace))
    {
        ...
        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_ALLOC_SYMMETRIC))
        {
            ...
            SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                         "Error! Currently Egress Native symmetric is not supported! flags = 0x%08X\n",
                         vlan_port->flags);
        }
    }

    SHR_IF_ERR_EXIT(dnx_vlan_port_flow_initiator_common_fields_set(unit, &vlan_port_flow_init, vlan_port));
    SHR_IF_ERR_EXIT(dnx_vlan_port_flow_initiator_special_fields_set(unit, &special_fields, vlan_port));
    ...
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_CREATE_EGRESS_ONLY) && !is_replace)
    {
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_update_symmetric_indication
                        (unit, _SHR_GPORT_FLOW_LIF_VAL_GET(flow_handle_info.flow_id), TRUE));
    }
    ...
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_WITH_ID))
    {
        ...
        /** Need to handle virtual Out-LIFs */
        vlan_port->encap_id = _SHR_GPORT_FLOW_LIF_VAL_GET(flow_handle_info.flow_id);
    }
    ...
    bcm_flow_handle_t * flow_handle)
{
    ...
    else if (BCM_GPORT_SUB_TYPE_LIF_EXC_GET(vlan_port_id) == BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY)
    {
        SHR_IF_ERR_EXIT(bcm_dnx_flow_handle_get(unit, FLOW_APP_NAME_VLAN_PORT_AC_NATIVE_TERMINATOR, flow_handle));
             ...
             (BCM_GPORT_SUB_TYPE_LIF_EXC_GET(vlan_port_id) == 0))
    {
    ...
    bcm_vlan_port_t * old_vlan_port)
{
    ...
    if (is_with_id)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_vlan_port_terminator_flow_handle_from_gport_get
                        (unit, vlan_port->vlan_port_id, &flow_handle));
    }
    else
    {
    ...
    if (is_native && (!is_replace))
    {
        ...
        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_ALLOC_SYMMETRIC))
        {
            flow_handle_info.flags |= BCM_FLOW_HANDLE_INFO_SYMMETRIC_ALLOC;
            SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                         "Error! Currently Ingress Native symmetric is not supported! flags = 0x%08X\n",
                         vlan_port->flags);
        }
    }
    ...
    bcm_gport_t vlan_port_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, vlan_port_id, _SHR_GPORT_TYPE_FLOW_LIF,
                     DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR, &flow_handle_info.flow_id));
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_update_symmetric_indication
                        (unit, _SHR_GPORT_FLOW_LIF_VAL_GET(flow_handle_info.flow_id), FALSE));
    }
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, vlan_port_translation->gport, _SHR_GPORT_TYPE_FLOW_LIF,
                     DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR, &flow_handle_info.flow_id));
    ...
    void *data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_lif_id_is_symmetric
                    (unit, _SHR_GPORT_FLOW_LIF_VAL_GET(flow_handle_info->flow_id), &lif_id_is_symmetric));
    ...
    bcm_gport_t port)
{
    ...
    /** Encode VLAN-PORT gport back to FLOW gport */
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, port, _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
