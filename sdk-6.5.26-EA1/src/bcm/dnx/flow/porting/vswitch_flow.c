    ...
    bcm_flow_handle_t * flow_handle)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, src_port, _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR, &flow_id));
    ...
    int *is_p2p)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, src_port, _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
    ...
    bcm_vswitch_cross_connect_t * gports)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, gports->port1, _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
                     &flow_handle_info.flow_id));
    ...
    if (vswitch_flow_term.valid_elements_set & BCM_FLOW_TERMINATOR_ELEMENT_FLOW_DEST_INFO_VALID)
    {
        if (BCM_GPORT_IS_FLOW_LIF(vswitch_flow_term.dest_info.dest_port))
        {
            ...
            else
            {
                ...
                /** THe dest_port can be ingress or egress - should be extructed from flow_id once supported */
    ...
    bcm_gport_t gport)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(gport))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_vlan_port_flow_terminator_l2_learn_info_set(unit, &vswitch_flow_term, &vlan_port));
 ...
 */
static shr_error_e
dnx_vswitch_cross_connect_traverse_flow_cb_evpn(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    bcm_flow_terminator_info_t * terminator_info,
    bcm_flow_special_fields_t * special_fields,
    ...
    void *data)
{
    ...
    flow_handle_info.flow_handle = flow_handle;
    SHR_IF_ERR_EXIT(bcm_dnx_flow_terminator_info_traverse
                    (unit, &flow_handle_info, dnx_vswitch_cross_connect_traverse_flow_cb_evpn, &user_data));
...
static shr_error_e
dnx_vswitch_cross_connect_delete_cb_evpn(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    bcm_flow_terminator_info_t * terminator_info,
    bcm_flow_special_fields_t * special_fields,
    ...
    int unit)
{
    ...
    flow_handle_info.flow_handle = flow_handle;
    SHR_IF_ERR_EXIT(bcm_dnx_flow_terminator_info_traverse
                    (unit, &flow_handle_info, dnx_vswitch_cross_connect_delete_cb_evpn, &data));
