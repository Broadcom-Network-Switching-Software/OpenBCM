    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    if (!is_esem_default)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, FLOW_LIF_DBAL_TABLE_GET(flow_app_info), &dbal_handle_id));

        SHR_IF_ERR_EXIT(dnx_flow_egress_virtual_key_fields_set(unit, flow_handle_info->flow_id, dbal_handle_id));

        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, dbal_handle_id, DBAL_FIELD_FODO, &vsi));
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, dbal_handle_id, DBAL_FIELD_ESEM_NAME_SPACE, &port));
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, dbal_handle_id, DBAL_FIELD_C_VID, &c_vid));
    ...
    bcm_flow_handle_t * flow_handle)
{
    ...
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "feature not supported\n");
        }
    }
    ...
    bcm_gport_t gport)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, gport, _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR, &flow_gport));
