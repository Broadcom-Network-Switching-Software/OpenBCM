    ...
    bcm_flow_special_fields_t * special_fields)
{
        ...
        (!(_SHR_IS_FLAG_SET(flow_handle_info->flags, BCM_FLOW_HANDLE_INFO_REPLACE))))
    {
        ...
        if (!global_lif)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error, Flow Gport is not given when flag BCM_FLOW_HANDLE_INFO_WITH_ID is set");
        }
        ...
        if (!(FLOW_APP_IS_MATCH(flow_app_info->flow_app_type)))
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Error, Virtual flag usage (%d) doesn't match the flow-id subtype for 0x%x",
        ...
        if (is_allocated)
        {
            SHR_ERR_EXIT(_SHR_E_EXISTS,
                         "Flag BCM_FLOW_HANDLE_INFO_WITH_ID with an already allocated Flow gport (flow_id=0x%08x)\n",
                         flow_handle_info->flow_id);
        }
    }
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (!is_virtual)
    {
        inlif_info.global_lif = _SHR_GPORT_FLOW_LIF_VAL_GET_ID(flow_handle_info->flow_id);
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (!is_virtual)
    {
        lif_table_mngr_inlif_info.global_lif = _SHR_GPORT_FLOW_LIF_VAL_GET_ID(flow_handle_info->flow_id);
        global_lif_id = lif_table_mngr_inlif_info.global_lif;
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_terminator_info_create(unit, flow_handle_info, entry_handle_id,
                                                             &lif_table_mngr_inlif_info, selectable_result_types));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    lif_table_mngr_outlif_info.global_lif = _SHR_GPORT_FLOW_LIF_VAL_GET_ID(flow_handle_info->flow_id);
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_initiator_info_create
                    (unit, flow_handle_info, entry_handle_id, &lif_table_mngr_outlif_info, selectable_result_types));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_initiator_info_replace
                    (unit, flow_handle_info, &gport_hw_resources, entry_handle_id,
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (flow_app_info->app_specific_operations_cb)
    {
        SHR_IF_ERR_EXIT(flow_app_info->app_specific_operations_cb
                        (unit, entry_handle_id, FLOW_ENTRY_HANDLE_ID_INVALID, flow_handle_info, flow_cmd_control,
                         &gport_hw_resources, initiator_info, special_fields, NULL, NULL));
        if (flow_cmd_control->flow_command != FLOW_COMMAND_CONTINUE)
        {
            SHR_EXIT();
        }
    ...
    bcm_flow_handle_info_t * flow_handle_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_init_dedicated_logic_qos_destroy(unit, flow_handle_info, flow_app_info,
                                                              entry_handle_id, &gport_hw_resources));
