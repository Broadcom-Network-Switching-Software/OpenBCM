         ...
         field_idx_to_ignore < table->multi_res_info[table->nof_result_types].nof_result_fields; field_idx_to_ignore++)
    {
        ...
        if (rv == _SHR_E_NONE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_select_result_type
                            (unit, flow_handle_info, selectable_result_types,
...
dnx_flow_lif_mgmt_set_hw_replace_result_type(
    ...
    int local_lif_id)
{
    int is_core_set_needed = (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc) && is_ingress
    ...
    if (is_ingress)
    {
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_reserve_new_lif
                        (unit, core_id, dbal_table_id, (lif_table_mngr_inlif_info_t *) lif_info, result_type,
                         current_result_type, local_lif_id, TRUE));
    }
    else
    {
        if (is_local_lif)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_lif_table_mngr_replace_result_type_outlif
                            (unit, dbal_table_id, (lif_table_mngr_outlif_info_t *) lif_info,
                             result_type, current_result_type, next_pointer_required_by_user, local_lif_id, TRUE));
        }
    }
...
{
    uint32 lif_table_entry_handle_id, set_result_type;
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_key_fields_set
                    (unit, core_id, is_ingress, dbal_table_id, set_result_type, local_lif_id,
    ...
    if (is_replace_result_type)
    {
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_set_hw_replace_result_type
                        (unit, dbal_table_id, is_ingress, core_id, TRUE, lif_table_entry_handle_id, entry_handle_id,
                         current_result_type, result_type, lif_info, local_lif_id));
    }
    ...
    uint32 selectable_result_types)
{
    ...
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_journal_update
                        (unit, local_lif_id, 1, 0, physical_table_id, is_new_local_lif));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_journal_update
        ...
        && is_new_local_lif)
    {
        ...
        {
            lif_mapping_local_lif_payload_t local_lif_payload = {.local_lifs_array = {{0}
                                                                                      }
    ...
    uint32 selectable_result_types)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_terminator_info_symmetric_replace
                    (unit, flow_handle_info, gport_hw_resources, entry_handle_id, lif_info, selectable_result_types));
    ...
    uint32 selectable_result_types)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_inlif_allocate
                    (unit, _SHR_CORE_ALL, dbal_table_id, selected_result_type, lif_info->entry_size_to_allocate,
                     &local_lifs_array));
    ...
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_journal_update(unit, local_lif_id, 1, 0, physical_table_id, TRUE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_journal_update
    ...
    if (!_SHR_IS_FLAG_SET(lif_info->flags, LIF_TABLE_MNGR_LIF_INFO_DONT_UPDATE_GLOBAL_TO_LOCAL_MAPPING))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_flow_ingress_virtual_mapping_add(unit, BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE_GET
    ...
    uint32 selectable_result_types)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_terminator_info_symmetric_create
                    (unit, flow_handle_info, entry_handle_id, lif_info, selectable_result_types));
 ...
 *   \param [in] no_local_lif_gport - The gport of the retrieved entry that has no local LIF
    ...
    uint32 entry_handle_id)
{
    ...
    uint32 retrieve_entry_handle_id, result_type = gport_hw_resources->outlif_dbal_result_type;
    int field_idx;
    uint8 found;
    dbal_fields_e field_id;
    ...
    SHR_IF_ERR_EXIT(algo_flow_lif.
                    valid_fields_per_no_local_lif_egress_htb.find(unit, &no_local_lif_gport, &entry_lif_info, &found));
    if (!found)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
                     "Error: Failed to retrieve entry info for no-local-LIF gport 0x%08X, found - %d",
                     no_local_lif_gport, found);
    }
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    /** for SET need to update CORE_ID to ALL */
    if ((dnx_data_esem.feature.feature_get(unit, dnx_data_esem_feature_esem_is_dpc)) &&
        (dbal_table_id != DBAL_TABLE_ESEM_DEFAULT_RESULT_TABLE))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, DBAL_CORE_ALL);
    }
    /** Remove the no-local-LIF entry from DBAL */
    SHR_IF_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id, DBAL_COMMIT));

    /** Delete the valid sw state entry */
    SHR_IF_ERR_EXIT(algo_flow_lif.valid_fields_per_no_local_lif_egress_htb.delete(unit, &no_local_lif_gport));
    ...
    if (is_replace_result_type)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_set_hw_replace_result_type
                        (unit, dbal_table_id, FALSE, _SHR_CORE_ALL, FALSE, entry_handle_id_for_hw_set, entry_handle_id,
                         existing_result_type, selected_result_type, NULL, -1));
    }
    ...
    uint32 selected_result_type)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_print_debug_info(unit, dbal_table_id, FALSE, selected_result_type, 0));
    ...
    uint32 selectable_result_types)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get(unit, curr_local_lif_id, NULL, NULL, NULL,
         ...
         (existing_result_type_has_ll != next_pointer_required_by_user)))
    {
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_outlif_allocate
                        (unit, dbal_table_id, selected_result_type, existing_result_type,
                         next_pointer_required_by_user, TRUE, lif_info->logical_phase, &curr_local_lif_id,
                         &entry_reuse));
        if (entry_reuse)
        {
            if (existing_result_type != selected_result_type)
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_journal_update(unit, curr_local_lif_id, 0, 0, 0, is_new_local_lif));
    FLOW_TIMER_STOP(flow_timer_idx++);
    ...
    else
    {
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_set_hw
                        (unit, _SHR_CORE_ALL, FALSE, (void *) lif_info, dbal_table_id, flow_handle_info,
                         entry_handle_id, selected_result_type, existing_result_type, curr_local_lif_id,
                         (replace_mode == FLOW_LIF_MGMT_REPLACE_MODE_REUSE_LIF)));
    }
    FLOW_TIMER_STOP(flow_timer_idx++);
    ...
    uint32 selectable_result_types)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_outlif_allocate
                    (unit, dbal_table_id, selected_result_type, 0,
                     next_pointer_required_by_user, FALSE, lif_info->logical_phase, &local_lif_id, &entry_reuse));
    FLOW_TIMER_STOP(flow_timer_idx++);
    ...
    FLOW_TIMER_SET("dnx_lif_table_mngr_journal_update", flow_timer_idx);
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_journal_update(unit, local_lif_id, 0, 0, 0, TRUE));
    FLOW_TIMER_STOP(flow_timer_idx++);
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_set_hw
                    (unit, _SHR_CORE_ALL, FALSE, (void *) lif_info, dbal_table_id, flow_handle_info,
                     entry_handle_id, selected_result_type, 0, local_lif_id, 0));
    ...
    dnx_flow_app_config_t * flow_app_info)
{
    ...
    if (dnx_flow_app_is_ind_set(unit, flow_app_info, FLOW_APP_INIT_IND_NO_LOCAL_LIF))
    {
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_initiator_no_local_lif_clear
                        (unit, flow_handle_info->flow_id, gport_hw_resources));
        SHR_EXIT();
    }
    ...
    uint32 entry_handle_id)
{
    ...
    if (dnx_flow_app_is_ind_set(unit, flow_app_info, FLOW_APP_INIT_IND_NO_LOCAL_LIF))
    {
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_initiator_no_local_lif_get
                        (unit, gport_hw_resources, flow_handle_info->flow_id, entry_handle_id));
        SHR_EXIT();
    }
