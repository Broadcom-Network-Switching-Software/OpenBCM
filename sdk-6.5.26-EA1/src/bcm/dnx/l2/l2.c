 ...
 * {
 */
/** Iterate between the lem DBs that are used by the device */
#define L2_LEM_DBS_ITER(unit, db_id) \
    for(db_id = 0; db_id < dnx_data_l2.general.lem_nof_dbs_get(unit); db_id++)
 /*
  * }
  */
    ...
    int *nof_vsis)
{
    ...
    if (!is_std_1)
    {
        *nof_vsis = DNX_PP_NON_STD_1_DEFAULT_NOF_ELEMENTS;
    }
    else
    {
        SHR_IF_ERR_EXIT(dbal_tables_capacity_get(unit, dnx_data_l2.vsi.vsi_table_get(unit), nof_vsis));
    }
    ...
    int unit)
{
    uint32 entry_handle_id;
    uint8 vmv_size = 0;
    uint8 long_ivl_vmv_size = 0;
    uint8 vmv_value = 0;
    uint8 long_ivl_vmv_value = 0;
    ...
    /** MDB_EM_ENTRY_ENCODING_HALF is a good format for all the MACT payloads except IVL with outlif in the payload. */
    ...
    /** vmv details of IVL entries with out LIF in the payload */
    SHR_IF_ERR_EXIT(mdb_em_get_vmv_size_value
                    (unit, dnx_data_mdb.em.mact_dbal_id_get(unit), MDB_EM_ENTRY_ENCODING_ONE, &long_ivl_vmv_size,
                     &long_ivl_vmv_value));
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MDB_VMV, INST_SINGLE, long_ivl_vmv_value);
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MDB_VMV, long_ivl_vmv_value);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FORMAT_INDEX, INST_SINGLE,
                                 DBAL_DEFINE_MACT_LEARN_FORMAT_IDX_ONE);
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_indication_set(unit, DBAL_TABLE_FWD_MACT_IVL, DBAL_TABLE_IND_IS_LEARNING_EN, TRUE));
    ...
    /** IVL  */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEM_APP_DB, app_db_id_ivl);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_MACT, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_LARGE_EM, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX, INST_SINGLE,
    ...
    if (dnx_data_l2.feature.feature_get(unit, (dnx_data_l2_feature_feature_e) dnx_data_l2_general_SLLB_v1_support))
    {
        /** PCC  */
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX,
                               DBAL_ENUM_FVAL_MACT_APP_DB_INDEX_IVL);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEM_APP_DB, INST_SINGLE, app_db_id_ivl);
...
{
    uint32 entry_handle_id;
    uint32 size, ivl_offset, svl_offset, wa_offset, pcc_offset;
    ...
    switch (learn_limit_mode)
    {
        case LEARN_LIMIT_MODE_VLAN:
             ...
             */
            pcc_offset = ivl_offset = svl_offset = MACT_ENTRY_VSI_OFFSET;
        ...
        case LEARN_LIMIT_MODE_VLAN_PORT:
            ...
            {
                pcc_offset = ivl_offset = svl_offset = MACT_ENTRY_OUTLIF_OFFSET_LEARN_RES_OPT;
            }
            else
            {
                pcc_offset = ivl_offset = svl_offset = MACT_ENTRY_OUTLIF_OFFSET;
            }
    ...
    /** IVL without outlif in the payload*/
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX,
                               DBAL_ENUM_FVAL_MACT_APP_DB_INDEX_IVL);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FORMAT_INDEX,
                               DBAL_DEFINE_MACT_LEARN_FORMAT_IDX_HALF);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_POINTER_OFFSET, INST_SINGLE, ivl_offset);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_POINTER_SIZE, INST_SINGLE, size);

    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    /** IVL with outlif in the payload */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX,
                               DBAL_ENUM_FVAL_MACT_APP_DB_INDEX_IVL);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FORMAT_INDEX,
                               DBAL_DEFINE_MACT_LEARN_FORMAT_IDX_ONE);
    ...
    int unit)
{
...

    {
         ...
         */
        if (dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_ignore_limit_check))
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DYNAMIC_STRENGTH_THRESHOLD, INST_SINGLE,
                                         STATIC_MACT_ENTRY_STRENGTH);
        }
        else
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DYNAMIC_STRENGTH_THRESHOLD, INST_SINGLE,
                                         DYNAMIC_MACT_ENTRY_STRENGTH);
        }
    }
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_DB_ID_MACT_MASK, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_DB_ID_BMACT_MASK, INST_SINGLE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_DB_ID_PROG_MASK, INST_SINGLE, 0);
    }

    /** Reject opportunistic learn lookup request when management access request and opportunistic learn lookup request are co-existed */
    if (dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_opportunistic_rejected))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OPPORTUNISTIC_REJECT_ENABLE, INST_SINGLE, 1);
    }
    /** Flush drop messages will be sent regardless of stamp_valid */
    if (dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_flush_drop_stuck))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ALWAYS_SEND_FLUSH_DROP_MESSAGE, INST_SINGLE, 1);
    }
    ...
    /** Set DBAL commit flags for static transplant-able entries */
    dnx_l2_static_transplant_commit_flags_set(unit, DBAL_COMMIT);
    ...
    int *limit)
{
    ...
    /*
     * If the field CHECK_MACT_DB_LIMIT is unset, then set the limit to the max value of entries.
     */
    if (check_mact_db_limit == 0)
    {
        *limit = -1;
    }
    ...
    int unit)
{
    ...
    /** Same configuration for dynamic non-transplantable entries */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, DBAL_ENUM_FVAL_MAC_STRENGTH_3);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/**
 * \brief - 
 * Get the max value of LEARN_LIMIT field.
 *
 * \param [in] unit - unit id
 * \param [out] predef_max_limit - max value of LEARN_LIMIT field.
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
dnx_limit_predef_max_limit_get(
    int unit,
    uint32 *predef_max_limit)
{
    uint32 entry_handle_id;
    ...
    bcm_l2_learn_limit_t * limit)
{
    ...
    if (_SHR_IS_FLAG_SET(limit->flags, BCM_L2_LEARN_LIMIT_PORT))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
                        (unit, limit->port, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS, &gport_hw_resources));

        SHR_IF_ERR_EXIT(dnx_learn_limit_per_vsi_get(unit, gport_hw_resources.global_out_lif, &limit->limit));
    }

    /*
     * If the limit is the maximum value, then set the limit to no-limit setting value(-1).
     */
    SHR_IF_ERR_EXIT(dnx_limit_predef_max_limit_get(unit, &predef_max_limit));
    if (limit->limit == predef_max_limit)
    {
        limit->limit = -1;
    }
    ...
    int aging_profile_id)
{
    ...
    /** Same configuration for strength 3 (dynamic non transplantable) */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, DBAL_ENUM_FVAL_MAC_STRENGTH_3);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    dnx_aging_profile_info_t * aging_profile_info)
{
    ...
    if (first_reference || (allocate_failed != _SHR_E_NONE))
    {
        SHR_IF_ERR_EXIT(dnx_aging_profile_hw_set(unit, new_aging_profile, aging_profile_info));
    }
    ...
    int limit)
{
     ...
     *  Use this value on vsi learning profile template exchange and hw update.
     */
    if (limit == -1)
    {
        SHR_IF_ERR_EXIT(dnx_limit_predef_max_limit_get(unit, &predef_max_limit));
        limit = predef_max_limit;
    }
    ...
    dnx_learn_mode_t learn_mode)
{
    ...
    sal_memset(&aging_profile_info, 0, sizeof(aging_profile_info));
    switch (learn_mode)
    {
    ...
    /** Update the HW with the new aging profile properties */
    SHR_IF_ERR_EXIT(dnx_aging_profile_hw_set(unit, DNX_VSI_AGING_PROFILE_DEFAULT_PROFILE_ID, &aging_profile_info));
    ...
    switch (nof_meta_cycles_for_ageout)
    {
        ...
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "nof meta cycles must be one of the following: 0, 2, 3, 4,..., 8, 10, 12, 14, 16, 32, 64, 128 .\n");
    }
    ...
    int *meta_cycle_time)
{
    ...
    uint32 entry_handle_id;
    dnxc_time_t time;
    dbal_tables_e table_id = DBAL_TABLE_MDB_22_LEM;
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_l2_age_timer_meta_cycle_get_verify(unit, meta_cycle_time));
    ...
    SHR_IF_ERR_EXIT(dnxc_time_clock_cycles_to_time_get
                    (unit, nof_100_clocks_between_metacycles, DNXC_TIME_UNIT_MILISEC, &time));
    ...
    int unit)
{
    ...
    L2_LEM_DBS_ITER(unit, db_id)
    {
        /** Get the number of clocks between two meta cycles */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, (dbal_tables_e) dnx_data_l2.general.scan_data_bases_info_get(unit, db_id)->scan_cycles,
                         &entry_handle_id));
    ...
    L2_LEM_DBS_ITER(unit, db_id)
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR
                        (unit, (dbal_tables_e) dnx_data_l2.general.scan_data_bases_info_get(unit, db_id)->age_scan,
                         entry_handle_id));
    ...
    int meta_cycle_time)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_age_timer_set_verify(unit, meta_cycle_time));
     ...
     */
    time_for_100_clocks.time_units = DNXC_TIME_UNIT_MILISEC;
    /*
     * Implement:
    ...
    L2_LEM_DBS_ITER(unit, db_id)
    {
        ...
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_MDB_ITEM_1, INST_SINGLE, machine_auto_enable);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    int unit)
{
    ...
    L2_LEM_DBS_ITER(unit, db_id)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, (dbal_tables_e) dnx_data_l2.general.scan_data_bases_info_get(unit, db_id)->scan_cycles,
                         &entry_handle_id));
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MDB_ITEM_1, INST_SINGLE, &machine_auto_enable);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    L2_LEM_DBS_ITER(unit, db_id)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR
                        (unit, (dbal_tables_e) dnx_data_l2.general.scan_data_bases_info_get(unit, db_id)->age_scan,
                         entry_handle_id));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MDB_ITEM_1, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int *is_enabled)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_learn_limit_to_cpu_get_verify(unit, is_enabled));
...
/**
 * \brief - Verify function for dnx_l2_learn_limit_check_static_set/get.
 *
 * \param [in] unit - Relevant unit
 *
 * \return
 ...
 */
static shr_error_e
dnx_l2_learn_limit_check_static_verify(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcmSwitchL2LearnLimitCheckStatic is not supported! \n");
    }
 ...
 */
shr_error_e
dnx_l2_learn_limit_check_static_set(
    int unit,
    int enable)
{
    uint32 entry_handle_id;
    uint32 exceed_limit_allowed;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_l2_learn_limit_check_static_verify(unit));

    exceed_limit_allowed = enable ? 0 : 1;
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_MACT_LIMIT_EXCEED_CONFIGURATION, &entry_handle_id));
    ...
    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_IS_STATIC, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ALLOW_IF_EXCEED_MACT_LIMIT, INST_SINGLE,
                                 exceed_limit_allowed);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ALLOW_IF_EXCEED_VSI_LIMIT, INST_SINGLE,
                                 exceed_limit_allowed);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_IS_HOST_REQUEST, 1);
    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_IS_STATIC, 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
shr_error_e
dnx_l2_learn_limit_check_static_get(
    int unit,
    int *enable)
{
    uint32 exceed_limit_allowed;
    ...
    dbal_entry_key_field8_range_set(unit, entry_handle_id, DBAL_FIELD_IS_HOST_REQUEST, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_IS_STATIC, 1);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_ALLOW_IF_EXCEED_MACT_LIMIT, INST_SINGLE,
                               &exceed_limit_allowed);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    *enable = exceed_limit_allowed ? 0 : 1;
    ...
    bcm_l2_gport_forward_info_t * frwrd_info)
{
    ...
    bcm_gport_t gport = gport_id;
    uint8 is_valid_dest;
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);

    SHR_IF_ERR_EXIT(algo_gpm_is_gport_valid_destination(unit, gport_id, &is_valid_dest));
        ...
        && (is_valid_dest == FALSE))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, gport_id, _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
                         &gport));
    }
    /*
    ...
    bcm_l2_gport_control_info_t * control_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_gport_control_info_verify(unit, control_info));
    /*
     * Fetch OUT_LIF from gport
     */
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
