    ...
    bcm_vlan_control_vlan_t * control)
{
    ...
    /**check if the multicaset group id is valid*/
    group_id = uc;
    ...
    if (_SHR_IS_FLAG_SET(control->flags2, BCM_VLAN_FLAGS2_UNKNOWN_DEST))
    {
        ...
        if (no_trap_for_unknown_destination && BCM_GPORT_IS_TRAP(control->unknown_dest))
        {
    ...
    bcm_vlan_control_vlan_t * control)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_control_vlan_get_verify(unit, vsi, control));
