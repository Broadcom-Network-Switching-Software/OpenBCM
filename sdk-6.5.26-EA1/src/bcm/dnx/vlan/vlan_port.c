    ...
    int is_virtual_ac)
{
    ...
    supported_flags2 = BCM_VLAN_PORT_FLAGS2_OAM_TRUNK_MEMBER;
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, vlan_port->vlan_port_id, _SHR_GPORT_TYPE_FLOW_LIF,
                         DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR, &local_gport));
    }
    ...
    int is_replace)
{
    ...
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, vlan_port_id, _SHR_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
                         &local_gport));
    }
    /*
     * For Native Virtual:
    ...
    int is_native)
{
     ...
     */
    if (learn_context != DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF0__ANY_DEST__16_GROUP__2L_STRENGTH__SVL)
    {
        if (learn_strength)
        {
    ...
    if (is_native == FALSE)
    {
        ...
        if (is_opt_learn_context)
        {
            ...
            {
                full_dbal_struct_data |= (ingress_ac_table_info->flush_group << 1);
            }
        }

        if (learn_info_set)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, learn_info_field_id, INST_SINGLE,
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    if (is_native_ac || is_pon_dtc_ac || dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
    {
        ...
        if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
        {
            SHR_IF_ERR_REPLACE_AND_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS), _SHR_E_INTERNAL,
                                        _SHR_E_NOT_FOUND);
        }
        else
        {
    ...
    dnx_ingress_ac_table_info_t * ingress_ac_table_info)
{
    ...
    if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO2_MODE)
    {
            ...
            || (forward_info->fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY))
        {
            ...
            {
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__TMF_OR_MC_DEST__16_GROUP__2L_STRENGTH__SVL:
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__TMF_OR_MC_DEST__16_GROUP__2L_STRENGTH__IVL:
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__TMF_OR_MC_DEST__16_GROUP__2L_STRENGTH__BOTH;
                    break;
                }
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__FEC_OR_SPA_OR_TRAP_DEST__16_GROUP__2L_STRENGTH__SVL:
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__FEC_OR_SPA_OR_TRAP_DEST__16_GROUP__2L_STRENGTH__IVL:
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__FEC_OR_SPA_OR_TRAP_DEST__16_GROUP__2L_STRENGTH__BOTH;
                    break;
                }
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__SYMMETRIC_DEST__16_GROUP__2L_STRENGTH__SVL:
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__SYMMETRIC_DEST__16_GROUP__2L_STRENGTH__IVL:
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__SYMMETRIC_DEST__16_GROUP__2L_STRENGTH__BOTH;
                    break;
