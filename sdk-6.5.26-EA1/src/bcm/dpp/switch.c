                                    ...
                                    int arg)
{
    ...
    case bcmSwitchHashIP6Field0:
        ...
        if (arg & BCM_HASH_FIELD_FLOWLABEL_LO) {
            info.mask &= ~SOC_PPC_HASH_MASKS_IPV6_LABLE_LO; 
        }
        else {
            info.mask |= SOC_PPC_HASH_MASKS_IPV6_LABLE_LO;
        }

        if (arg & BCM_HASH_FIELD_FLOWLABEL_HI) {
            info.mask &= ~SOC_PPC_HASH_MASKS_IPV6_LABLE_HI; 
        }
        else {
            info.mask |= SOC_PPC_HASH_MASKS_IPV6_LABLE_HI;
        }
...
/************************************************************************/
    ...
    /* CR Switch Crash Recovery Mode is not supported */
        rv = BCM_E_UNAVAIL;
        break;
    ...
    /* CR Swtich CR last transaction status is not supported */
        rv = BCM_E_UNAVAIL;
        break;

    case bcmSwitchCrCouldNotRecover:
    /* CR Switch CR could not recover is not supported */
        rv = BCM_E_UNAVAIL;
        break;
...
/************************************************************************/
    ...
    case bcmSwitchPFCStopInterfaceBitmap:
        if (!SOC_IS_JERICHO(unit)) { /* supported only for Jer/QMX/Jer+/QMX+/QAX/QUX */
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcmSwitchPFCStopInterfaceBitmap supported only for Jericho and above.")));
        }
                            ...
                            int arg)
{
    ...
    case bcmSwitchCrashRecoveryMode:
    /* CR Switch CR Mode is not supported */
        rv = BCM_E_UNAVAIL;
        break;

    case bcmSwitchCrTransactionStart:
    /* CR Switch CR transaction start is not supported */
        rv = BCM_E_UNAVAIL;
        break;

    case bcmSwitchCrCommit:
    /* CR Switch CR commit is not supported */
        rv = BCM_E_UNAVAIL;
        break;

    case bcmSwitchCrAbort:
    /* CR Switch CR abort is not supported */
        rv = BCM_E_UNAVAIL;
        break;
...
/************************************************************************/
  ...
  case bcmSwitchIngressRateLimitMpps:
    ...
    case bcmSwitchPFCStopInterfaceBitmap:
        if (!SOC_IS_JERICHO(unit)) { /* supported only for Jer/QMX/Jer+/QMX+/QAX/QUX */
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcmSwitchPFCStopInterfaceBitmapsupported only for Jericho and above, besides QUX.")));
        }
        if ((arg < 0) || (arg > 0xFF)){
            ...
            if (SOC_PORT_TYPE(unit, port) == SOC_BLK_XLP) {
                ...
                BCMDNX_IF_ERR_EXIT(
                    soc_reg_field32_modify(unit, GPORT_MODE_REGr, port,
                                           EGR_1588_TIMESTAMPING_MODEf,
                                           (arg == bcmTimesyncTimestampingMode48bit) ? 0: 1));
            } else if (SOC_PORT_TYPE(unit, port) == SOC_BLK_CLP) {
                BCMDNX_IF_ERR_EXIT(
        ...
        if (SOC_IS_JERICHO_PLUS(unit)) {
            ...
            switch (key.index) {
                ...
                case bcmL3LpmHitbitTableIPv6MC:
                    table_id_1 = SOC_DPP_DBAL_SW_TABLE_ID_IPV6MC;
                    if (SOC_DPP_L3_IPV6_COMPRESSED_SIP_ENABLE(unit)) {
                        table_id_2 = SOC_DPP_DBAL_SW_TABLE_ID_IPV6MC_SSM_KAPS;
                    }
                    break;
