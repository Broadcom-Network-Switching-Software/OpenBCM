    ...
    bcm_stat_counter_config_t *config)
{
    ...
    if (config->source.engine_source == bcmStatCounterSourceIngressVoq)
    {
        q_set_size = config->source.num_voqs_per_entry;
        BCMDNX_IF_ERR_EXIT(bcm_dpp_counter_voq_base_val_get(unit, config->source.core_id, range.start, &command_base_offset));

        if (range.start < command_base_offset) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("range.start %d is less than VOQ cnt base Q %d.\n"
                                                           "rage.start of the first counter engine configured for VOQ is used as base Q.\n"
                                                           "Any subsequent configured counter engine for VOQ should have a range.start greater than the first one.\n"),
                                              range.start, command_base_offset));
        }

        if (((range.start - command_base_offset) % counter_sets_per_counter_processor) != 0) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Counter source %s VOQ-Counting range start is %d, cannot be resolved to the same cnt base Q %d.\n"), 
                                              SOC_TMC_CNT_SRC_TYPE_to_string(src_type), range.start, command_base_offset));
        }

        we_val = (range.start - command_base_offset) / counter_sets_per_counter_processor;

        rv = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_max_we_val_get, (unit, src_type, &max_we_val));
        BCMDNX_IF_ERR_EXIT(rv);

        if (we_val > max_we_val) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Counter source %s, engine %d misconfiguration, MSB-pattern %d."),
                SOC_TMC_CNT_SRC_TYPE_to_string(src_type), engine->engine_id, we_val));
        }
    }
    else
    {
