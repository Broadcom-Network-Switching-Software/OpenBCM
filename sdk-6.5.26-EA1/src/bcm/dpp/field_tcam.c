    ...
    uint8 *entry_hit_core_bmp_p)
{
    ...
    *entry_hit_core_bmp_p = FALSE;
    ...
    tcam_db_id = ARAD_PP_FP_DB_ID_TO_TCAM_DB(entryGroup);
    arad_tcam_db_entry_id_to_location_entry_get(unit, tcam_db_id, hwHandle, FALSE, &entry_abs_location, &found);

    res = sw_state_access[unit].dpp.soc.arad.tm.tcam.tcam_db.entry_size.get(unit,tcam_db_id, &entry_size);
    if (res != BCM_E_NONE) {
        LOG_CLI((BSL_META_U(unit, "The given entry %d (0x%08X) get entry size failed.\n"), entry_handle, entry_handle));
        BCM_EXIT;
    }
    
    /*
     * calc the number of tcam entry(granularity 80bits)
     */
    if (entry_size == SOC_TMC_TCAM_BANK_ENTRY_SIZE_80_BITS) {
        nof_80b_per_entry = 1;
    } else if (entry_size == SOC_TMC_TCAM_BANK_ENTRY_SIZE_160_BITS) {
        nof_80b_per_entry = 2;
    } else if (entry_size == SOC_TMC_TCAM_BANK_ENTRY_SIZE_320_BITS) {
        nof_80b_per_entry = 4;
    }

    if (!nof_80b_per_entry) {
        LOG_CLI((BSL_META_U(unit, "The given entry has wrong number of entry size(%d)\n"), nof_80b_per_entry));
        BCM_EXIT;
    }
    ...
    /** Max number of entries in two SRAMs of one 160b TCAM line
     *
     *      for key=80b, each 160b contain 2 entries, one point to SRAM0, the other to SRAM1
     *      for key=160b/320b, each 160b only service as(parts of) 1 entry. point to SRAM0+SRAM1
     * Each SRAM line contain 8bits.
     */
    if (nof_80b_per_entry == 1) {
        ...
        sram_line_entry_id = (entry_abs_location.entry % max_nof_entries_in_two_srams) / 2;
    } else {
        max_nof_entries_in_two_srams = 8;
        bank_sram_id = bank_id * max_nof_srams_per_tcam_bank;
        sram_line_entry_id = entry_abs_location.entry % max_nof_entries_in_two_srams;
    }
    ...
    if ((bank_id < SOC_DPP_DEFS_GET(unit, nof_tcam_big_banks)))
    {
        ...
        bank_line_id = (nof_80b_per_entry > 1) ?
                        ((entry_abs_location.entry % max_nof_big_bank_lines) / max_nof_entries_in_two_srams): 
                        ((entry_abs_location.entry % (max_nof_big_bank_lines*2)) / max_nof_entries_in_two_srams);
    }
    else
    {
        if (SOC_IS_QUX(unit)) {
        ...
        bank_line_id = (nof_80b_per_entry > 1) ?
                        ((entry_abs_location.entry % max_nof_small_bank_lines) / max_nof_entries_in_two_srams): 
                        ((entry_abs_location.entry % (max_nof_small_bank_lines*2)) / max_nof_entries_in_two_srams);
    }
...
_bcm_dpp_field_tcam_entry_hit_per_location_flush(
    int unit,
    ARAD_TCAM_BANK_ENTRY_SIZE entry_size,
    ARAD_TCAM_ACTION_SIZE action_bitmaps,
    ARAD_TCAM_LOCATION entry_abs_location)
{
    uint32 mem_addr;
    uint8 at;
    ...
    /*
     * cacl the number of SRAM for each entry: for each entry size, depends on action size.
     */
    if (action_bitmaps & SOC_TMC_TCAM_ACTION_SIZE_FORTH_20_BITS) {
        nof_used_sram_id = 4;
    } else if (action_bitmaps & SOC_TMC_TCAM_ACTION_SIZE_THIRD_20_BITS) {
        nof_used_sram_id = 3;
    } else if (action_bitmaps & SOC_TMC_TCAM_ACTION_SIZE_SECOND_20_BITS) {
        nof_used_sram_id = 2;
    } else if (action_bitmaps & SOC_TMC_TCAM_ACTION_SIZE_FIRST_20_BITS) {
        nof_used_sram_id = 1;
    }
    /*
     * calc the number of tcam entry(granularity 80bits)
    ...
    if ((nof_used_sram_id > nof_80b_per_entry) || !nof_80b_per_entry || !nof_used_sram_id) {
        LOG_CLI((BSL_META_U(unit, "The given entry has wrong number of entry size(%d) or SRAM size(%d)\n"), nof_80b_per_entry, nof_used_sram_id));
        BCM_EXIT;
    }    
    ...
    if (entry_abs_location.entry != BCM_FIELD_ENTRY_INVALID)
    {
         ...
         *
         *      for key=80b, each 160b contain 2 entries, one point to SRAM0, the other to SRAM1
         *          ent0 in tcam_entry.80lsb, ent1 in tcam_entry.80msb. ent0.hit in SRAM0, ent1.hit in SRAM1
         *      for key=160b/320b, each 160b only service as(parts of) 1 entry. point to SRAM0+SRAM1
         * Each SRAM line contain 8bits.
         */
        if (nof_80b_per_entry == 1) {
    ...
    for (mem_index = 0; mem_index < nof_tcam_hit_indication_memories; mem_index++)
    {
        ...
        if (SOC_MEM_IS_ARRAY(unit, mem))
        {
             ...
             */
            if (entry_abs_location.entry != BCM_FIELD_ENTRY_INVALID) {
                array_index_min = bank_sram_id;
                array_index_max = array_index_min + nof_used_sram_id - 1;
            }
        }
        else
        {
    ...
    uint32 entry_handle)
{
    ...
    uint32 tcam_db_id;
    ARAD_TCAM_BANK_ENTRY_SIZE entry_size;
    ARAD_TCAM_ACTION_SIZE action_bmps;
    ...
    if (flags & BCM_FIELD_ENTRY_HIT_FLUSH_ALL)
    {
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_field_tcam_entry_hit_per_location_flush(unit, 0, 0, entry_abs_location));
    }
    else
    {
        /** Check if the given entry_handle is valid. */
        ...
        res = arad_pp_fp_action_bitmap_ndx_get_unsafe(unit, entryGroup, &action_bmps);
        if (res != BCM_E_NONE) {
            LOG_CLI((BSL_META_U(unit, "The given entry %d (0x%08X) get action size failed.\n"), entry_handle, entry_handle));
            BCM_EXIT;
        }    

        /** Clear hit info for the given entry entry. */
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_field_tcam_entry_hit_per_location_flush(unit, entry_size, action_bmps, entry_abs_location));
    }
