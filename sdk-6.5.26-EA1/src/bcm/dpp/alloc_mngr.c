...
#define _DPP_AM_TEMPLATE_BFD_REQ_INTERVAL_MAX_ENTITIES (2*SOC_PPC_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit))
...
#define _DPP_AM_TEMPLATE_TRAP_EGRESS_COUNT (_BCM_PPD_TRAP_MGMT_EG_ACTION_PROFILE_MAX+1) 
...
_bcm_dpp_pp_resource_setup(int unit, int res_id, int core_id, int pool_id) {
    ...
    break;
        ...
        case dpp_am_res_eep_trill:
                BCM_EXIT;
            break;
        case dpp_am_res_cosq_end_to_end:
            p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
            p_info.start = 0;
            p_info.count = SOC_DPP_CONFIG(unit)->tm.nof_flow_profile;
            p_info.max_elements_per_allocation = 1;
            desc_str = "TM flow profile";
            break;
...
int
bcm_dpp_am_tm_flow_profile_alloc(
        int unit,
        uint32 flags,
        int *profile_id)
{
    ...
    int elem = *profile_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_cosq_end_to_end,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
...
int
bcm_dpp_am_tm_flow_profile_dealloc(
        int unit,
        uint32 flags,
        int profile_id)
{
    ...
    int elem = profile_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_cosq_end_to_end,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
...
int
bcm_dpp_am_tm_flow_profile_is_alloced(
        int unit,
        int profile_id)
{
    int elem = profile_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_cosq_end_to_end,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_template_setup(int unit, int template_id, int core_id, int pool_id) {
    ...
    case dpp_am_template_vsq_pg_tc_mapping:
        ...
        case dpp_am_template_lif_term_profile:
            ...
            t_info.count = _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_COUNT * ( 1 + soc_property_get(unit, spn_MPLS_LSP_DESTINATION_DISABLE, 0));
...
/*
 * If spn_MPLS_LSP_DESTINATION_DISABLE is set, only one template is initialized with profile range 0 - 7
 * for both PWE and LSP with or without CW.
 */
int _bcm_dpp_am_template_lif_term_init(int unit,int template_init_id,const SOC_PPC_MPLS_TERM_PROFILE_INFO* term_profile)
{
    ...
    if (soc_property_get(unit, spn_MPLS_LSP_DESTINATION_DISABLE, 0)) {
        /* In this case, the available term profiles are 0 - 7, the same one template is used.*/
 ...
 */
static int
_bcm_dpp_am_template_term_profile_data_get(int unit, int template, SOC_PPC_MPLS_TERM_PROFILE_INFO *term_profile_info)
{
    int rc = BCM_E_NONE;
    ...
    } else if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_EVPN_ENABLE, 0)) {
    ...
    BCMDNX_IF_ERR_EXIT_MSG(rc, (_BSL_BCM_MSG("Failed to get term profile info for profile %d"), template));
...
int _bcm_dpp_am_template_lif_term_data_get(int unit, SOC_PPC_LIF_ID lif_id, SOC_PPC_MPLS_TERM_PROFILE_INFO *term_profile_info)
{
    ...
    rc = _bcm_dpp_am_template_term_profile_data_get(unit, template, term_profile_info);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (is_pwe_lif) {
        ...
        if (rc == BCM_E_NONE) {
            ...
            if (old_profile < _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_COUNT) {
                ...
                /* Allocate it in template1.*/
                rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_lif_term_pw_with_cw_profile,
                                              0, term_profile_info, is_allocated, &new_profile);
                ...
                /* De-Allocate the profile from template2.*/
                rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_lif_term_profile, old_profile, &tmp_is_last);
            ...
            /* Proper profile doesn't exist, allocate in template2.*/
            rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_lif_term_profile,
                                          0, term_profile_info, is_allocated, &new_profile);
            if (rc == BCM_E_MEMORY) {
                /* If failed in template2, also try to allocate in template1.*/
                rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_lif_term_pw_with_cw_profile,
                                              0, term_profile_info, is_allocated, &new_profile);
                BCMDNX_IF_ERR_EXIT_MSG(rc, (_BSL_BCM_MSG("Failed to exchange in dpp_am_template_lif_term_pw_with_cw_profile for lif %d"), lif_id));
            } else {
                BCMDNX_IF_ERR_EXIT_MSG(rc, (_BSL_BCM_MSG("Failed to allocate in dpp_am_template_lif_term_profile for lif %d"), lif_id));
                /* exchanged in template2, range is 4 - 7.*/
                new_profile += _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_COUNT;
            }

            /* De-allocate the old profile from the proper template*/
            if (old_profile >= _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_COUNT) {
                old_profile -= _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_COUNT;
                rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_lif_term_profile, old_profile, &tmp_is_last);
                BCMDNX_IF_ERR_EXIT_MSG(rc, (_BSL_BCM_MSG("Failed to de-allocate in dpp_am_template_lif_term_profile for lif %d"), lif_id));
            } else {
                rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_lif_term_pw_with_cw_profile, old_profile, &tmp_is_last);
                BCMDNX_IF_ERR_EXIT_MSG(rc, (_BSL_BCM_MSG("Failed to de-allocate in dpp_am_template_lif_term_pw_with_cw_profile for lif %d"), lif_id));
            }
        } else {
            BCMDNX_IF_ERR_EXIT(rc);
        }
...
int _bcm_dpp_am_template_lif_term_profile_exchange(int unit, SOC_PPC_LIF_ID lif_id, int is_pwe_lif, SOC_PPC_MPLS_TERM_PROFILE_INFO *term_profile_info,  int *old_template, int *is_last, int *new_term_profile,int *is_allocated)
{
    ...
    if (soc_property_get(unit, spn_MPLS_LSP_DESTINATION_DISABLE, 0)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,
            dpp_am_template_lif_term_profile,0,term_profile_info,tmp_old_template,is_last,new_term_profile,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);

        /* In this case, all profiles are init in the same template*/
        BCM_EXIT;
    }
...
_bcm_dpp_am_template_l2_flooding_mapping_get(int unit, SOC_PPC_LIF_ID lif_index,int *profile)
{
    ...
    if ((lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_AC) && (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP) &&
        ...
        (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_IP_TUNNEL_RIF)) {
    ...
    if (lif_entry_info->type == SOC_PPC_LIF_ENTRY_TYPE_PWE) {
        ...
        *profile = lif_entry_info->value.ip_term_info.default_forward_profile;
    } else {
...
int _bcm_dpp_am_template_ip_tunnel_exchange_test(int unit, int tunnel_id, int is_new_ip_tunnel, SOC_PPC_EG_ENCAP_IP_TUNNEL_SRC_IP *sip, int ttl, int tos, int *sip_template, int *ttl_template, int *tos_template,int *is_allocated)
{
    ...
    /*default old_profile_tos is 8 for new ip tunnel */
    if(SOC_DPP_CONFIG(unit)->pp.ip_tunnel_defrag_set && !is_new_ip_tunnel){
