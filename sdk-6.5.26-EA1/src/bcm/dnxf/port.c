    ...
    int value)
{
    ...
    switch (type)
    {
        ...
        case bcmPortControlPadToSize:
            SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fmac_tx_padding_set(unit, port, value));
            break;
    ...
    int *value)
{
    ...
    switch (type)
    {
        ...
        case bcmPortControlPadToSize:
            SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fmac_tx_padding_get(unit, port, value));
            break;
...
/**
 * \brief - Verify parameters for bcm_dnxf_port_phy_tx_set and
 *       bcm_dnx_port_phy_tx_get API.
 *
 * \param [in] unit - Unit ID
 * \param [in] port - logical port
 * \param [in] tx - tx parameters
 * \param [in] is_set - if verifier is called for set or get function.
 *
 * \return
 *   See shr_error_r
 * \remark
 ...
 */
static shr_error_e
dnxf_port_phy_tx_verify(
    int unit,
    bcm_port_t port,
    bcm_port_phy_tx_t * tx,
    int is_set)
{
    uint32 serdes_tx_tap_pre3_supported = dnxf_data_port.static_add.feature_get(unit,
    ...
    bcm_port_phy_tx_t * tx)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnxf_port_phy_tx_verify(unit, port, tx, 1));
    ...
    bcm_port_phy_tx_t * tx)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnxf_port_phy_tx_verify(unit, port, tx, 0));
