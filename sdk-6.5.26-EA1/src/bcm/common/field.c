    ...
    bcm_field_context_info_t * context_info)
{
    ...
    if (context_info != NULL)
    {
        ...
        context_info->state_collection_engine_enabled = FALSE;
    ...
    bcm_field_group_info_t * fg_info)
{
    ...
    if (fg_info != NULL)
    {
        ...
        for (ii = 0; ii < BCM_FIELD_NUMBER_OF_TCAM_BANKS; ii++)
        {
            fg_info->tcam_info.core_ids[ii] = BCM_CORE_ALL;
        }
    }
...
/**
 * \brief
 *  Initialize the bcm_field_range_qual_info_t
 * \param [out] range_qual_info  - Pointer to input structure of bcm_field_range_qual_info_t that needs to be initialized
 * \return
 *  void
 * \remark
 *  This function should be called prior of using bcm_field_range_qual_info_t struct
 * \see
 *  Reference to:
 *    - bcm_field_range_type_config_set
 *    - bcm_field_range_type_config_get
 */
extern void bcm_field_range_qualifier_info_t_init(
    bcm_field_range_qual_info_t *range_qual_info)
{
    if (range_qual_info != NULL) {
        sal_memset (range_qual_info, 0, sizeof (bcm_field_range_qual_info_t));
        range_qual_info->qual_type = bcmFieldQualifyCount;
        bcm_field_qual_attach_info_t_init(&(range_qual_info->qual_info));
    }
    return;
}
...
/**
 * \brief
 *  Initialize the bcm_field_entry_state_table_info_t
 * \param [out] entry_state_table_info  -
 *  Pointer to input structure of bcm_field_entry_state_table_info_t
 *  that needs to be initialized
 * \return
 *  void
 * \remark
 *  This function should be called prior of using bcm_field_entry_state_table_info_t struct
 * \see
 *  Reference to:
 *    - bcm_field_entry_state_table_add
 *    - bcm_field_entry_state_table_info_get
 */
extern void bcm_field_entry_state_table_info_t_init(
    bcm_field_entry_state_table_info_t *entry_state_table_info)
{
    int ii;
    if (entry_state_table_info != NULL) {
        sal_memset (entry_state_table_info, 0, sizeof (bcm_field_entry_state_table_info_t));
        entry_state_table_info->nof_address_quals = BCM_FIELD_INVALID;
        entry_state_table_info->nof_data_quals = BCM_FIELD_INVALID;
        for (ii = 0; ii < BCM_FIELD_NUMBER_OF_QUALS_PER_GROUP; ii++)
        {
            entry_state_table_info->address_qual[ii].type = bcmFieldQualifyCount;
            entry_state_table_info->data_qual[ii].type = bcmFieldQualifyCount;
        }
        entry_state_table_info->core = BCM_CORE_ALL;
    }
    return;
}
...
/**
 * \brief
 *  Initialize the bcm_field_context_state_table_info_t
 * \param [out] state_table_info  -
 *  Pointer to input structure of bcm_field_context_state_table_info_t
 *  that needs to be initialized
 * \return
 *  void
 * \remark
 *  This function should be called prior of using bcm_field_context_state_table_info_t struct
 * \see
 *  Reference to:
 *    - bcm_field_context_state_table_create
 */
 extern void bcm_field_context_state_table_info_t_init(
    bcm_field_context_state_table_info_t *state_table_info)
{
    int ii;
    if (state_table_info != NULL)
    {
        sal_memset (state_table_info, 0, sizeof (bcm_field_context_state_table_info_t));
        for (ii = 0; ii < BCM_FIELD_NUMBER_OF_QUALS_PER_GROUP; ii++)
        {

            state_table_info->data.qual_types[ii] = bcmFieldQualifyCount;
            state_table_info->data.qual_info[ii].input_type = BCM_FIELD_INVALID;
            state_table_info->data.qual_info[ii].input_arg = BCM_FIELD_INVALID;
            state_table_info->data.qual_info[ii].offset = BCM_FIELD_INVALID;

            state_table_info->address.qual_types[ii] = bcmFieldQualifyCount;
            state_table_info->address.qual_info[ii].input_type = BCM_FIELD_INVALID;
            state_table_info->address.qual_info[ii].input_arg = BCM_FIELD_INVALID;
            state_table_info->address.qual_info[ii].offset = BCM_FIELD_INVALID;

            state_table_info->opcode_based_qual.qual_types[ii] = bcmFieldQualifyCount;
            state_table_info->opcode_based_qual.qual_info[ii].input_type = BCM_FIELD_INVALID;
            state_table_info->opcode_based_qual.qual_info[ii].input_arg = BCM_FIELD_INVALID;
            state_table_info->opcode_based_qual.qual_info[ii].offset = BCM_FIELD_INVALID;
        }

        for (ii = 0; ii < BCM_FIELD_NUMBER_OF_ACTIONS_PER_GROUP; ii++)
        {
            state_table_info->current_state.action_types[ii] = bcmFieldActionCount;
            state_table_info->current_state.action_info[ii].priority = BCM_FIELD_ACTION_DONT_CARE;

            state_table_info->next_state.action_types[ii] = bcmFieldActionCount;
            state_table_info->next_state.action_info[ii].priority = BCM_FIELD_ACTION_DONT_CARE;
        }

        state_table_info->opcode = bcmFieldStateOpcodeQual;
    }
    return;
}
    ...
    bcm_field_tcam_bank_info_t * bank_info)
{
    ...
    if (bank_info != NULL)
    {
        ...
        for (bank_iter = 0; bank_iter < BCM_FIELD_NUMBER_OF_TCAM_BANKS; bank_iter++)
        {
            ...
            bank_info->core_ids[bank_iter] = BCM_CORE_ALL;
        }
    }
...
/*
 * Function:
 *      bcm_field_policer_action_config_t_init
 * Purpose:
 *      Initialize the Field policer action structure.
 * Parameters:
 *      policer_cfg - Pointer to the policer action structure.
 * Returns:
 *      NONE
 */
void
bcm_field_policer_action_config_t_init(bcm_field_policer_action_config_t *policer_cfg)
{
    if (policer_cfg != NULL) {
        sal_memset(policer_cfg, 0, sizeof(bcm_field_policer_action_config_t));
    }
