...
#define k_FLG_MASTER_PDV_THRESHOLD_EXCEED_SET    (1u)
#define k_FLG_MASTER_PDV_THRESHOLD_EXCEED_CLR    (2u)
...
int _bcm_ptp_ext_servo_master_table_entry_get( bcm_ptp_port_identity_t *master_portId,
                                               uint16                  *inst_num)
{
    uint16 i=0;
...
void _bcm_ptp_ext_servo_qual_timer_exp_evt_send(int unit,
                                                bcm_ptp_stack_id_t ptp_id,
                                                int clock_num,
                                                uint16 inst_num)
{
    ...
    _bcm_ptp_uint16_write(payload+len, _bcm_ptp_qual_timer_exp_event);
    len+=2;

    payload[len] = master_table[inst_num].master_portaddr.addr_type;
    len++;

    if(master_table[inst_num].master_portaddr.addr_type == bcmPTPUDPIPv4) {
        sal_memcpy(payload+len, master_table[inst_num].master_portaddr.address, PTP_IPV4_ADDR_SIZE_BYTES);
        len+=PTP_IPV4_ADDR_SIZE_BYTES;
    } else if(master_table[inst_num].master_portaddr.addr_type == bcmPTPUDPIPv6) {
        sal_memcpy(payload+len, master_table[inst_num].master_portaddr.address, PTP_IPV6_ADDR_SIZE_BYTES);
        len+=PTP_IPV6_ADDR_SIZE_BYTES;
    } else if(master_table[inst_num].master_portaddr.addr_type == bcmPTPIEEE8023) {
        sal_memcpy(payload+len, master_table[inst_num].master_portaddr.address, PTP_MAC_ADDR_SIZE_BYTES);
        len+=PTP_MAC_ADDR_SIZE_BYTES;
    }

    _bcm_ptp_send_event_cb(unit, ptp_id, clock_num, payload, len);

}

void _bcm_ptp_ext_servo_pdv_thres_exceed_evt_send(int unit,
                                                  bcm_ptp_stack_id_t ptp_id,
                                                  int clock_num,
                                                  uint16 inst_num,
                                                  int set)
{
    ...
    /* Notify the application of pdv threshold exceed event(set/clear) for master */
    _bcm_ptp_uint16_write(payload+len, _bcm_ptp_pdv_thres_exceed_event);
    len+=2;
    ...
    int                                  clock_num)
{
    ...
    for (i=0; i<BCM_PTP_MAX_UNICAST_MASTER_TABLE_ENTRIES; i++) {
        ...
        if (GET_BIT(master_table[i].flag, k_FLG_MASTER_SET_LOCK_OUT)) {

            LOG_INFO(BSL_LS_BCM_PTP,
                    (BSL_META_U(0, "Servo_def_evt_hdlr : lock mstr[%d] \n"), i));

            _bcm_ptp_ext_servo_qual_timer_exp_evt_send(unit, ptp_id, clock_num, i);

            /* Clear the flag */
            CLEAR_BIT(master_table[i].flag, k_FLG_MASTER_SET_LOCK_OUT);
        }

        if (GET_BIT(master_table[i].flag, k_FLG_MASTER_PDV_THRESHOLD_EXCEED_SET)) {

            LOG_INFO(BSL_LS_BCM_PTP,
                    (BSL_META_U(0, "Servo_def_evt_hdlr : pdv thres exceed set for mstr[%d] \n"), i));

            _bcm_ptp_ext_servo_pdv_thres_exceed_evt_send(unit, ptp_id, clock_num, i, 1);

            /* Clear the flag */
            CLEAR_BIT(master_table[i].flag, k_FLG_MASTER_PDV_THRESHOLD_EXCEED_SET);
        }

        if (GET_BIT(master_table[i].flag, k_FLG_MASTER_PDV_THRESHOLD_EXCEED_CLR)) {

            LOG_INFO(BSL_LS_BCM_PTP,
                    (BSL_META_U(0, "Servo_def_evt_hdlr : pdv thres exceed clear for mstr[%d] \n"), i));

            _bcm_ptp_ext_servo_pdv_thres_exceed_evt_send(unit, ptp_id, clock_num, i, 0);

            /* Clear the flag */
            CLEAR_BIT(master_table[i].flag, k_FLG_MASTER_PDV_THRESHOLD_EXCEED_CLR);
        }
                                                 ...
                                                 T_osBool set )
{
    ...
    if (PTP_CLOCK_IS_G8275P2_PROF(clock_info)) {
        ...
        rv = _bcm_ptp_ext_servo_master_table_entry_get(&master_portId, &inst_num);
        if (rv == BCM_E_NONE) {
            if (set) {
                SET_BIT(master_table[inst_num].flag, k_FLG_MASTER_PDV_THRESHOLD_EXCEED_SET);
            } else {
                SET_BIT(master_table[inst_num].flag, k_FLG_MASTER_PDV_THRESHOLD_EXCEED_CLR);
            }
        } else {
            LOG_ERROR(BSL_LS_BCM_COMMON,
                    (BSL_META_U(0, " PDV threshold exceed notification received for invalid master. \n")));
        }
    }
    ...
    uint32                         holdoverInSpecSec)
{
  ...
  cmnReadConfigurationFile("idtServoConfig.json",  &configParam, NULL);
  ...
  configParam.externalConfig.holdoverConfig.holdoverTimeoutSeconds = holdoverInSpecSec;
  ...
  bsl_printf("\n desiredPrecisionSeconds : %Lf", configParam.externalConfig.referenceTrackerConfig[0].desiredPrecisionSeconds);
  bsl_printf("\n timeLockThresholdNanoseconds: %Lf", configParam.externalConfig.referenceTrackerConfig[0].timeLockThresholdNanoseconds);
  bsl_printf("\n phaseSnapThresholdSeconds: %Lf", configParam.externalConfig.referenceTrackerConfig[0].phaseSnapThresholdSeconds);
  bsl_printf("\n qualificationTimeoutSeconds: %d", (int)configParam.externalConfig.qualificationTimeoutSeconds);
  bsl_printf("\n minExpProportionForMinTracking: %Lf", configParam.externalConfig.referenceTrackerConfig[0].minExpProportionForMinTracking);
  bsl_printf("\n frequencyLockThresholdPpb: %Lf\n", configParam.externalConfig.referenceTrackerConfig[0].frequencyLockThresholdPpb);
  for(i = 0; i < IDT_CNFG_MAX_NUM_STACK_INSTANCES; i++) {
      bsl_printf("\n minExpProportionForMinTracking[%d]: %Lf\n", i, configParam.externalConfig.referenceTrackerConfig[i].minExpProportionForMinTracking);
  }
  ...
  /* Reset the copy of best master maintained in sdk when resetting servo. */
    ...
    unsigned char                  clockClass)
{
    ...
    if(PTP_CLOCK_IS_G8275P1_PROF(clock_info) && g_apts_enabled != 1) {
        /* There is a special case where the profile is G.8275.1 but Ext servo is used only for GPS filtering.
         * Such configuration is only supported in the case where ALL PTP ports are MASTER_ONLY
         * i.e., it is a GM configuration with EXT servo being used for filtering the GPS reference.
         * For such a configuration we will NOT return an error */
    ...
    bcm_ptp_clock_port_address_t * master_portaddr)
{
    ...
    if (inst_num == PTP_EXT_SERVO_MSTR_INST_UNKNOWN) {
        ...
        if(BCM_SUCCESS(rv = _bcm_ptp_ext_servo_master_table_entry_get(master_portId, &inst_num))) {
    ...
    /*
     * In case the master_portId is same as the current bestmaster_portId
     * then send the new best_master as 0 to IDT. And let the master_change_event
     * update the new bestmaster_portId and also update the same to IDT.
     */
    if (!(_bcm_ptp_peer_portid_compare(master_portId, &bestmaster_portId))) {
        sal_memcpy(&best_master.clockIdentity, bestmaster_portId.clock_identity, sizeof(bcm_ptp_clock_identity_t));
        best_master.portNumber = bestmaster_portId.port_number;
    }
