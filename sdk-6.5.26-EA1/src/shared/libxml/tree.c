...
xmlStaticCopyNode(xmlNodePtr node, xmlDocPtr doc, xmlNodePtr parent,
	...
	/* The function xmlAddChild doesn't free the second parameter on all cases, so probably it's ok */
    ...
    do {
	    ...
	    */
        ...
        if (xmlStrlen(buffer) + sizeof(nametemp) + 20 > buf_len) {
            ...
            temp = (xmlChar *) xmlRealloc(buffer, (buf_len - (xmlStrlen(buffer) + sizeof(nametemp) + 20))/2, buf_len);
            if (temp == NULL) {
		xmlTreeErrMemory("getting node path");
                xmlFree(buf);
                xmlFree(buffer);
            ...
            buffer = temp;
            temp = (xmlChar *) xmlRealloc(buf, (buf_len - (xmlStrlen(buffer) + sizeof(nametemp) + 20))/2, buf_len);
            if (temp == NULL) {
		xmlTreeErrMemory("getting node path");
                xmlFree(buf);
                xmlFree(buffer);
        ...
        if (node->type == XML_ELEMENT_NODE) {
			...
			xmlTreeErrMemory("getting namespace list");
                ...
                if (i >= nbns) {
                    if (nbns >= maxns) {
                        maxns *= 2;
                        ret = (xmlNsPtr *) xmlRealloc(ret, (maxns/2) * sizeof(xmlNsPtr),
                                                      (maxns +
                                                       1) *
                                                      sizeof(xmlNsPtr));
                        if (ret == NULL) {
	...
	if (node->ns != NULL) {
		...
		if (n != NULL) { /* :-( what if else ??? */
		    ...
		    if (sizeCache <= nbCache) {
		        sizeCache *= 2;
			oldNs = (xmlNsPtr *) xmlRealloc(oldNs, (sizeCache/2) * sizeof(xmlNsPtr),sizeCache *
			                               sizeof(xmlNsPtr));
		        if (oldNs == NULL) {
			    xmlTreeErrMemory("fixing namespaces");
			    xmlFree(newNs);
			...
			newNs = (xmlNsPtr *) xmlRealloc(newNs, (sizeCache/2) * sizeof(xmlNsPtr), sizeCache *
			                               sizeof(xmlNsPtr));
		        if (newNs == NULL) {
			    xmlTreeErrMemory("fixing namespaces");
			    xmlFree(oldNs);
	...
	if (node->type == XML_ELEMENT_NODE) {
		...
		if (attr->ns != NULL) {
			...
			if (n != NULL) { /* :-( what if else ??? */
			    ...
			    if (sizeCache <= nbCache) {
				sizeCache *= 2;
				oldNs = (xmlNsPtr *) xmlRealloc(oldNs, (sizeCache/2) * sizeof(xmlNsPtr),
				...
				newNs = (xmlNsPtr *) xmlRealloc(newNs, (sizeCache/2) * sizeof(xmlNsPtr),
				           sizeCache * sizeof(xmlNsPtr));
				if (newNs == NULL) {
				    xmlTreeErrMemory("fixing namespaces");
				    xmlFree(oldNs);
...
xmlBufferGrow(xmlBufferPtr buf, unsigned int len) {
	...
	newbuf = (xmlChar *) xmlRealloc(buf->contentIO, buf->size, start_buf + size);
	if (newbuf == NULL) {
	...
	newbuf = (xmlChar *) xmlRealloc(buf->content, buf->size, size);
	if (newbuf == NULL) {
	    xmlTreeErrMemory("growing buffer");
	    return(-1);
	}
...
xmlBufferResize(xmlBufferPtr buf, unsigned int size)
{
    ...
    if ((buf->alloc == XML_BUFFER_ALLOC_IO) && (buf->contentIO != NULL)) {
	    ...
	    rebuf = (xmlChar *) xmlRealloc(buf->contentIO, buf->size, start_buf + newSize);
	    if (rebuf == NULL) {
		xmlTreeErrMemory("growing buffer");
		return 0;
	    }
	...
	if (buf->content == NULL) {
	    ...
	    rebuf = (xmlChar *) xmlRealloc(buf->content, buf->size, newSize);
        } else {
	    /*
 ...
 */
int
xmlBufferAddRv(xmlBufferPtr buf, const xmlChar *str, int len) {
...
xmlBufferCat(xmlBufferPtr buf, const xmlChar *str) {
    ...
    if (str == NULL) return;
    /* The return type of the function is void - there is no rv to check */
    /* coverity[check_return:FALSE] */
...
xmlBufferWriteCHAR(xmlBufferPtr buf, const xmlChar *string) {
    if (buf == NULL)
        return;
    if (buf->alloc == XML_BUFFER_ALLOC_IMMUTABLE) return;
    /* xmlBufferCat is a void function */
    /* coverity[check_return:FALSE] */
