    ...
    int unit)
{
    ...
    return ((SAL_BOOT_PLISIM && (is_std_1) && (rv == 0) &&
             dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_is_kbp_supported))
            || (!SAL_BOOT_PLISIM && dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_is_kbp_supported)
                && ((dnx_data_elk.general.device_type_get(unit) == DNX_KBP_DEV_TYPE_BCM52311) ||
                    ...
                    (dnx_data_elk.general.device_type_get(unit) == DNX_KBP_DEV_TYPE_BCM5235)))) ? TRUE : FALSE;
}
