...
do {                                                                                   \
    SHR_ALLOC_ERR_EXIT((_ptr),(_size), (_str), "%s%s%s",EMPTY,EMPTY,EMPTY) ;           \
    ...
    dnx_rollback_journal_handle_t * handle)
{
    ...
    for (er_thread_id = 0; er_thread_id < DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION; er_thread_id++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_rollback_journal_clear(unit, er_thread_id, *handle));
        SHR_FREE(dnx_rollback_journal_pool[unit][er_thread_id].entries[*handle]);
    ...
    dnx_rollback_journal_make_distinct_cb make_distinct_cb)
{
    ...
    while (entry != NULL)
    {
        ...
        if (!DNX_ROLLBACK_JOURNAL_ENTRY_STATE_IS_VALID(entry->state))
        {
            ...
            if (entry->state == DNX_ROLLBACK_JORUNAL_ENTRY_STATE_CONDITIONALLY_INVALID)
            {
                entry->state = DNX_ROLLBACK_JOURNAL_ENTRY_STATE_INVALID;
            }
            entry = entry->next;
        ...
        while (trav_entry != NULL)
        {
            if (!DNX_ROLLBACK_JOURNAL_ENTRY_STATE_IS_VALID(trav_entry->state)
                && (trav_entry->state != DNX_ROLLBACK_JORUNAL_ENTRY_STATE_CONDITIONALLY_INVALID))
            {
