...
{
    if (((int) pp_stage < 0) || (pp_stage >= dnx_data_pp.stages.params_info_get(unit)->key_size[0]))
    {
    ...
    dnx_pp_stage_e * pp_stage_id)
{
    ...
    if (dnx_pp_stage_string_to_id_no_error(unit, pp_stage_name, pp_stage_id) != _SHR_E_NONE)
    {
...
shr_error_e
dnx_pp_stage_string_to_id_no_error(
    int unit,
    char *pp_stage_name,
    dnx_pp_stage_e * pp_stage_id)
{
    ...
    {
        if (sal_strncasecmp
            (dnx_data_pp.stages.params_get(unit, pp_stage_index)->name, pp_stage_name,
    ...
    int *pema_id_p)
{
    ...
    pema_id = dnx_data_pp.stages.params_get(unit, pp_stage_id)->chuck_block_id;
...
shr_error_e
dnx_pp_block_string_to_id_no_error(
    int unit,
    char *pp_block_name,
    dnx_pp_block_e * pp_block_id_p)
{
    ...
    {
        if (sal_strncasecmp
            (pp_block_info[pp_block_id].name, pp_block_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dnx_pp_block_e * pp_block_id_p)
{
    ...
    if (dnx_pp_block_string_to_id_no_error(unit, pp_block_name, pp_block_id_p) != _SHR_E_NONE)
    {
...
#define KBR_MAX_NUM     MAX_NOF_PP_KBR
    ...
    char *stage_n_out)
{
    ...
    {
        if (!sal_strncasecmp
            (tokens[0], pp_klear_kbr_info[i_kbr].kbr_connect_n, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            break;
    }
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        if (select > 1)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Wrong select value:%d for kbr:%s, kbr_id:%d \n",
                         select, pp_klear_kbr_info[i_kbr].kbr_connect_n, kbr_id);
        }
        ...
        sw_select = select;
        SHR_IF_ERR_EXIT(pp_db.pp_stage_kbr_select.set(unit, i_kbr, kbr_id, sw_select));
    }
    else
    {
        SHR_IF_ERR_EXIT(pp_db.pp_stage_kbr_select.get(unit, i_kbr, kbr_id, &sw_select));
    }

    if (sw_select > 1)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Wrong select value:%d for kbr:%s, kbr_id:%d \n",
                     sw_select, pp_klear_kbr_info[i_kbr].kbr_connect_n, kbr_id);
    }

    sal_strncpy(stage_n_out, pp_klear_kbr_info[i_kbr].stage_n[sw_select], RHNAME_MAX_SIZE - 1);
