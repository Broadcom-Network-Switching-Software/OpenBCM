    ...
    int unit)
{
    ...
    rv = dnxc_sw_state_journal_destroy(unit, TRUE);
    if (rv != _SHR_E_NONE)
    {
        SHR_SET_CURRENT_ERR(rv);
    }

    rv = dnx_dbal_journal_destroy(unit, TRUE);
    if (rv != _SHR_E_NONE)
    {
        SHR_SET_CURRENT_ERR(rv);
    }

    rv = dnx_generic_state_journal_destroy(unit, TRUE);
    if (rv != _SHR_E_NONE)
    {
        SHR_SET_CURRENT_ERR(rv);
    }
...
exit:
    if (should_clean)
    {
        dnxc_sw_state_journal_clear(unit, er_thread_id, TRUE);
        dnx_generic_state_journal_clear(unit, er_thread_id, TRUE);
        dnx_dbal_journal_clear(unit, er_thread_id, TRUE);
