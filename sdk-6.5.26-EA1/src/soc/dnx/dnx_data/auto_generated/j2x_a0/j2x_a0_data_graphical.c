    ...
    int unit)
{
    ...
    if (dnx_graphical_nif_rx < table->keys[0].size && 0 < table->keys[1].size)
    {
        ...
        data->regs[1] = ARB_RX_PM_TOTAL_DROPPED_PKT_BYTE_COUNTERr;
        SHR_RANGE_VERIFY(2, 0, DNX_DIAG_COUNTER_REG_NOF_REGS, _SHR_E_INTERNAL, "out of bound access to array")
        data->flds[0] = ILU_RX_NUM_TOTAL_DROPPED_EOPSf;
        data->flds[1] = RX_PM_N_TOTAL_DROPPED_PKT_COUNTERf;
        data->fld_num = 2;
    ...
    if (dnx_graphical_nif_rx < table->keys[0].size && 2 < table->keys[1].size)
    {
        ...
        data->flds[1] = RX_PM_N_TOTAL_INCOMING_BYTE_COUNTERf;
        data->flds[2] = RX_TMAC_TOTAL_INCOMING_BYTE_COUNTERf;
        data->fld_num = 3;
