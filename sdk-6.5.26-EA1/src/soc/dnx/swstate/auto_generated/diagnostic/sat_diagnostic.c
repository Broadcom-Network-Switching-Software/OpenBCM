...
sat_pkt_header_info_pkt_header_base_dump(int  unit,  int  pkt_header_base_idx_0,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    else
    {
        ...
        if(i0 >= dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_SAT_PKT_HEADER_INFO__PKT_HEADER_BASE, ((sat_pkt_header_info_t*)sw_state_roots_array[unit][SAT_MODULE_ID])->pkt_header_base))
        {
        ...
        if(dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_SAT_PKT_HEADER_INFO__PKT_HEADER_BASE, ((sat_pkt_header_info_t*)sw_state_roots_array[unit][SAT_MODULE_ID])->pkt_header_base) == 0)
        {
            SHR_EXIT();
        }
