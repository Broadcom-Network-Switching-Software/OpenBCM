...
void __test_setup_method(){
    ht_setup(&__test_address_to_data, sizeof(u_long), sizeof(unsigned int*), 100);
    ht_setup(&__test_expected_core_values, sizeof(u_long), sizeof(unsigned int*), 100);
}
...
void __test_init_method(bool test_all_cores, int test_core_id, bool test_verify_read, bool test_verify_read_from_cache, bool test_verify_all_cores_write, bool test_debug_mode) {
    __test_all_cores = test_all_cores;
    __test_core_id = test_core_id;
    __test_verify_read = test_verify_read;
    __test_verify_read_from_cache = test_verify_read_from_cache;
    __test_verify_all_cores_write = test_verify_all_cores_write;
    __test_debug_mode = test_debug_mode;

    if (test_verify_all_cores_write){
        for (int i=0; i<PEM_CFG_API_NOF_CORES; ++i){
            start_block_id[i] = -1;
        }
    }
    ...
    if (test_all_cores){
        for (int i=0; i<PEM_CFG_API_NOF_CORES; ++i) {
            nof_reads_per_core[i]=0;
            nof_writes_per_core[i] = 0;
        }
    }
}

void __test_teardown_method() {
    ht_clear(&__test_address_to_data);
    ht_clear(&__test_expected_core_values);

    ht_destroy(&__test_address_to_data);
    ht_destroy(&__test_expected_core_values);
}
...
STATIC int dnx_write_all_physical_data_from_list(
    int unit,
...
STATIC void dnx_pemladrv_get_reg_name(const int unit, const int reg_id, char** target_name){
  ...
  dnx_pemladrv_get_reg_or_reg_field_name(api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_info_arr[i].reg_field_mapping_per_core_arr[core_id].reg_field_mapping_arr[0].register_name, 1, target_name);
}
...
int pemladrv_direct_write_inner(
    const int unit,
    const int core_id, 
    table_id_t db_id,
    uint32 row_index,
    pemladrv_mem_t * data)
{
  ...
  dnx_build_virtual_db_mask_and_data_from_result_fields(api_info[unit].db_direct_container.db_direct_info_arr[db_id].db_direct_info_per_core_arr[core_id].total_result_width, api_info[unit].db_direct_container.db_direct_info_arr[db_id].db_direct_info_per_core_arr[core_id].field_bit_range_arr, data, 1, &virtual_result_mask, &virtual_result_data);
  ...
  target_result_physical_memory_entry_data_list = dnx_run_over_all_chunks_read_physical_data_and_update_physical_memory_entry_data_list(unit,
                                                                                                                                        ...
                                                                                                                                        virtual_result_mask,
                                                                                                                                        virtual_result_data);
  if(dnx_write_all_physical_data_from_list(unit, target_result_physical_memory_entry_data_list) != 0)
  {
    error++;
  }
...
static int dnx_pem_direct_access_wrapper(
    int unit,
    int sub_unit_idx,
    int is_read,
    int is_mem,
    ...
    uint32 *entry_data)
{
    ...
    if (SOC_IS_NEW_ACCESS_INITIALIZED(unit))
    {
        ...
        return access_direct_regmem(unit, flags, block_identifier, address, BITS2WORDS(size_in_bits), entry_data);
    }
    else
#endif
    {
#ifdef ADAPTER_SERVER_MODE
        return adapter_memreg_access
            (unit, sub_unit_idx, block_identifier, address, BITS2BYTES(size_in_bits), is_mem, is_read, 0, entry_data);
#else
        if (is_read)
        {
            return (soc_direct_memreg_get(unit, block_identifier, address, BITS2WORDS(size_in_bits), is_mem, entry_data));
        }
        else
        {
            return (soc_direct_memreg_set(unit, block_identifier, address, BITS2WORDS(size_in_bits), is_mem, entry_data));
        }
#endif
    }
...
int pemladrv_direct_write(
    ...
    if (core_id == -1){
        for (int core_i=0; core_i<PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_i){
            if(pemladrv_direct_write_inner(unit, core_i, db_id, row_index, data) != 0)
            {
                error++;
            }
        }
        ...
        error = pemladrv_direct_write_inner(unit, core_id, db_id, row_index, data);
    }
    return error;
}
...
int pemladrv_direct_read(
    ...
    virtual_db_data_array = (unsigned int *) pemladrv_calloc(virtual_db_data_array,
    ...
    dnx_set_pem_mem_accesss_fldbuf_from_physical_sram_data_array(virtual_db_data_array,
                                                                 api_info[unit].db_direct_container.db_direct_info_arr[db_id].db_direct_info_per_core_arr[core_id].field_bit_range_arr,
                                                                 result);
    sal_free(virtual_db_data_array);
    return 0;
}
    ...
    pemladrv_mem_t * data)
{
    ...
    if (core_id == -1) {
        for (int core_i=0; core_i<PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_i) {
            result += dnx_logical_cam_based_db_write(&api_info[unit].db_tcam_container.db_tcam_info_arr[tcam_id], unit,
...
int dnx_logical_cam_based_db_write(
  ...
  dnx_build_virtual_db_mask_and_data_from_key_fields(tcam_key_width, 2 * tcam_key_width + 1, cam_info->db_tcam_per_core_arr[core_id].key_field_bit_range_arr, key, mask, valid, &virtual_key_mask, &virtual_key_data);
  dnx_build_virtual_db_mask_and_data_from_result_fields(cam_info->db_tcam_per_core_arr[core_id].total_result_width, cam_info->db_tcam_per_core_arr[core_id].result_field_bit_range_arr, data, 1, &virtual_result_mask, &virtual_result_data);
  ...
  if(dnx_write_all_physical_data_from_list(unit, target_key_physical_memory_entry_data_list) != 0)
  {
    error++;
  }
  if(dnx_write_all_physical_data_from_list(unit, target_result_physical_memory_entry_data_list) != 0)
...
int pemladrv_tcam_read(
    ...
    pemladrv_mem_t * valid,
    pemladrv_mem_t * result) {
    return dnx_logical_cam_based_db_read(&api_info[unit].db_tcam_container.db_tcam_info_arr[tcam_id], unit, core_id,
                                         row_index, key,
                                         mask, valid, result);
}
...
int dnx_pemladrv_full_reg_write(const int unit, const int core_id, const int reg_id, const int size, const unsigned int* data){
  ...
  rv |= pemladrv_reg_write(unit, core_id, reg_id, reg_p);
...
unsigned int dnx_pemladrv_reg_info_allocate(const int unit, pemladrv_reg_info** arr, int* size) {
    ...
    *arr = (pemladrv_reg_info*)pemladrv_calloc(*arr, api_info[unit].reg_container.nof_registers, sizeof(pemladrv_reg_info));
    ...
    for (reg_index = 0; reg_index < api_info[unit].reg_container.nof_registers; ++reg_index)
    {
        ...
        (*arr)[mapped_regs_size].ptr = (pemladrv_reg_field_info *)pemladrv_calloc((*arr)[mapped_regs_size].ptr, (*arr)[mapped_regs_size].nof_fields, sizeof(pemladrv_reg_field_info));
        ...
        for (field_index = 0; field_index < api_info[unit].reg_container.reg_info_arr[reg_index].nof_fields; ++field_index)
        {
            ...
            if (reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].nof_mappings > 0)
            {
              ...
              (*arr)[mapped_regs_size].ptr[mapped_fields_index].addresses = (pemladrv_reg_mapping_info*)sal_calloc((*arr)[mapped_regs_size].ptr[mapped_fields_index].addresses, reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].nof_mappings, sizeof(pemladrv_reg_mapping_info));
...
unsigned int dnx_pemladrv_reg_info_de_allocate(const int unit, pemladrv_reg_info* ptr){
  ...
  for (unsigned int i = 0; i < api_info[unit].reg_container.nof_registers; ++i){
    ...
    for(unsigned int field_idx = 0; field_idx < temp.nof_fields; field_idx++){
      if (temp.ptr[field_idx].field_name == NULL)
        continue;
      sal_free(temp.ptr[field_idx].field_name);
      sal_free(temp.ptr[field_idx].addresses);
    }
    sal_free(temp.ptr);
    sal_free(temp.reg_name);
  }
  sal_free(ptr);
  return 0;
}
...
unsigned int dnx_pemladrv_direct_info_allocate( const int unit, pemladrv_db_info** arr, int* size) {
  ...
  *arr = (pemladrv_db_info*)pemladrv_calloc(*arr, api_info[unit].db_direct_container.nof_direct_dbs, sizeof(pemladrv_db_info));
  ...
  for (direct_index = 0; direct_index < api_info[unit].db_direct_container.nof_direct_dbs; ++direct_index)
  {
    ...
    (*arr)[mapped_diret_size].ptr = (pemladrv_db_field_info*)pemladrv_calloc((*arr)[mapped_diret_size].ptr, (*arr)[mapped_diret_size].nof_fields, sizeof(pemladrv_db_field_info));
...
unsigned int dnx_pemladrv_tcam_info_allocate( const int unit, pemladrv_db_info** arr, int* size) {
    int mapped_tcam_size = 0;
    int name_len;
    unsigned int tcam_index, field_index;
    const int core_id = 0;

    *arr = (pemladrv_db_info*)pemladrv_calloc(*arr, api_info[unit].db_tcam_container.nof_tcam_dbs, sizeof(pemladrv_db_info));
    ...
    for (tcam_index = 0; tcam_index < api_info[unit].db_tcam_container.nof_tcam_dbs; ++tcam_index)
    {
        ...
        int nof_key_fields = api_info[unit].db_tcam_container.db_tcam_info_arr[tcam_index].db_tcam_per_core_arr[core_id].nof_fields_in_key;
        int nof_fields = nof_key_fields;
        ...
        (*arr)[mapped_tcam_size].db_name = (char*)sal_alloc(name_len + 1, "");
        ...
        (*arr)[mapped_tcam_size].ptr = (pemladrv_db_field_info*)pemladrv_calloc((*arr)[mapped_tcam_size].ptr, (*arr)[mapped_tcam_size].nof_fields, sizeof(pemladrv_db_field_info));

        for (field_index = 0; field_index < nof_fields; ++field_index)
        {
            FieldBitRange *tcam_field_key_info_arr = &tcam_info_arr->db_tcam_per_core_arr[core_id].key_field_bit_range_arr[field_index];
            FieldBitRange *tcam_field_result_info_arr = &tcam_info_arr->db_tcam_per_core_arr[core_id].result_field_bit_range_arr[field_index];
...
unsigned int dnx_pemladrv_tcam_info_de_allocate(const int unit, pemladrv_db_info* ptr){
    pemladrv_db_info temp;
...
unsigned int dnx_pemladrv_em_info_allocate( const int unit, pemladrv_db_info** arr, int* size) {
    ...
    *arr = (pemladrv_db_info*)pemladrv_calloc(*arr, api_info[unit].db_em_container.nof_em_dbs, sizeof(pemladrv_db_info));
    ...
    for (em_index = 0; em_index < api_info[unit].db_em_container.nof_em_dbs; ++em_index)
    {
        ...
        int nof_key_fields = api_info[unit].db_em_container.db_em_info_arr[em_index].logical_em_info.db_tcam_per_core_arr[core_id].nof_fields_in_key;
        int nof_fields = nof_key_fields;
        ...
        (*arr)[mapped_em_size].db_name = (char*)sal_alloc(name_len + 1, "");
        ...
        (*arr)[mapped_em_size].ptr = (pemladrv_db_field_info*)pemladrv_calloc((*arr)[mapped_em_size].ptr, (*arr)[mapped_em_size].nof_fields, sizeof(pemladrv_db_field_info));

        for (field_index = 0; field_index < nof_fields; ++field_index) {
            FieldBitRange *em_field_key_info_arr = &em_info_arr->logical_em_info.db_tcam_per_core_arr[core_id].key_field_bit_range_arr[field_index];
            FieldBitRange *em_field_result_info_arr = &em_info_arr->logical_em_info.db_tcam_per_core_arr[core_id].result_field_bit_range_arr[field_index];
...
unsigned int dnx_pemladrv_lpm_info_allocate( const int unit, pemladrv_db_info** arr, int* size) {
    ...
    int name_len;
    unsigned int lpm_index, field_index;
    const int core_id = 0;

    *arr = (pemladrv_db_info*)pemladrv_calloc(*arr, api_info[unit].db_lpm_container.nof_lpm_dbs, sizeof(pemladrv_db_info));
    ...
    for (lpm_index = 0; lpm_index < api_info[unit].db_lpm_container.nof_lpm_dbs; ++lpm_index)
    {
        ...
        int nof_key_fields = api_info[unit].db_lpm_container.db_lpm_info_arr[lpm_index].logical_lpm_info.db_tcam_per_core_arr[core_id].nof_fields_in_key;
        int nof_fields = nof_key_fields;
        ...
        (*arr)[mapped_lpm_size].db_name = (char*)sal_alloc(name_len + 1, "");
        ...
        (*arr)[mapped_lpm_size].ptr = (pemladrv_db_field_info*)pemladrv_calloc((*arr)[mapped_lpm_size].ptr, (*arr)[mapped_lpm_size].nof_fields, sizeof(pemladrv_db_field_info));

        for (field_index = 0; field_index < nof_fields; ++field_index) {
            FieldBitRange *lpm_field_key_info_arr = &lpm_info_arr->logical_lpm_info.db_tcam_per_core_arr[core_id].key_field_bit_range_arr[field_index];
            FieldBitRange *lpm_field_result_info_arr = &lpm_info_arr->logical_lpm_info.db_tcam_per_core_arr[core_id].result_field_bit_range_arr[field_index];
...
int dnx_logical_cam_based_db_read(
    ...
    virtual_db_key_data_array = (unsigned int *) pemladrv_calloc(virtual_db_key_data_array,
                                                            (size_t) pemladrv_ceil(total_key_width_in_bits,
                                                                                   UINT_WIDTH_IN_BITS), sizeof(uint32));
    virtual_db_result_data_array = (unsigned int *) pemladrv_calloc(virtual_db_result_data_array,
                                                               (size_t) pemladrv_ceil(total_result_width_in_bits,
                                                                                      UINT_WIDTH_IN_BITS),
                                                               sizeof(uint32));

    dnx_build_physical_db_key_data_array(unit, cam_info->db_tcam_per_core_arr[core_id].key_chunk_mapper_matrix_arr,
    ...
    dnx_set_pem_mem_accesss_fldbuf_from_physical_tcam_data_array(virtual_db_key_data_array,
                                                                 cam_info->db_tcam_per_core_arr[core_id].total_key_width,
    ...
    pemladrv_mem_t * data)
{
  ...
  if(dnx_write_all_physical_data_from_list(unit, target_reg_physical_memory_entry_data_list) != 0)
  {
    error++;
  }
        ...
        pemladrv_mem_t * data)
{
    ...
    if (core_id == -1) {
        for (int core_i = 0; core_i < PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_i) {
            if(pemladrv_reg_write_inner(unit, core_i, reg_id, data) != 0)
            {
                error++;
            }
        }
        ...
        error = pemladrv_reg_write_inner(unit, core_id, reg_id, data);
    }
    return error;
}
    ...
    pemladrv_mem_t * result)
{
  ...
  dnx_pemladrv_reg_read_and_retreive_read_bits_mask(unit, core_id, reg_id, result, result_read_bits_mask);
    ...
    int *index)
{
  ...
  unsigned int *virtual_key_data = (unsigned int*)pemladrv_calloc(virtual_key_data, (size_t)pemladrv_ceil(total_key_width_in_bits, UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  *index = next_free_index;
  return pemladrv_em_entry_set_by_id(unit, core_id, table_id, next_free_index, key, result);
}
...
int pemladrv_em_insert(
    ...
    if (core_id == -1 ){
        for (int core_i=0; core_i<PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_i){
            res += pemladrv_em_insert_inner(unit, core_i, table_id, key, result, index);
        }
    }
    else{
        res = pemladrv_em_insert_inner(unit, core_id, table_id, key, result, index);
    }
    ...
    int *index)
{
  ...
  *index = dnx_pemladrv_find_physical_entry_index_by_key(unit, core_id, &api_info[unit].db_em_container.db_em_info_arr[table_id].logical_em_info, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache[core_id].physical_entry_occupation, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache[core_id].em_key_entry_arr, key);
  if (*index == -1) {
    
    return 1; 
  }
  return pemladrv_em_remove_by_index(unit, core_id, table_id, *index);
}
    ...
    int *index)
{
  *index = dnx_pemladrv_find_physical_entry_index_by_key(unit, core_id, &api_info[unit].db_em_container.db_em_info_arr[table_id].logical_em_info, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache[core_id].physical_entry_occupation, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache[core_id].em_key_entry_arr, key);
  ...
  return dnx_pemladrv_get_em_data_by_entry(unit, core_id, table_id, *index, key, result);
}
    ...
    pemladrv_mem_t * result)
{
  ...
  virtual_key_data    = (unsigned int*)pemladrv_calloc(virtual_key_data, (size_t)pemladrv_ceil(total_key_width_in_bits, UINT_WIDTH_IN_BITS), sizeof(uint32));
  virtual_result_data = (unsigned int*)pemladrv_calloc(virtual_result_data, (size_t)pemladrv_ceil(total_result_width_in_bits,  UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  error = dnx_logical_cam_based_db_write(&api_info[unit].db_em_container.db_em_info_arr[table_id].logical_em_info, unit, core_id, index, key, mask, valid, result);
...
int pemladrv_em_remove_all_inner(
        const int unit,
        const int core_id,
        table_id_t table_id)
{
    ...
    for(index = 0; index < nof_entries; ++index) {
        if(api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache[core_id].physical_entry_occupation[index] == 1)
            pemladrv_em_remove_by_index(unit, core_id, table_id, index);
    }
    return 0;
}
    ...
    table_id_t table_id)
{
  ...
  if (core_id == -1 ){
      ...
      for (int core_i=0; core_i<PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_i){
          result += pemladrv_em_remove_all_inner(unit, core_i, table_id);
      }
  }
  else{
	result = pemladrv_em_remove_all_inner(unit, core_id, table_id);
  }
...
int pemladrv_em_remove_by_index(
    ...
    int index)
{
  dnx_pemladrv_set_valid_bit_to_zero(unit, core_id, index, &api_info[unit].db_em_container.db_em_info_arr[table_id].logical_em_info);
...
int pemladrv_lpm_insert_inner(
    ...
    unsigned int *virtual_key_data = (unsigned int *) pemladrv_calloc(virtual_key_data,
                                                                 (size_t) pemladrv_ceil(total_key_width_in_bits,
                                                                                        UINT_WIDTH_IN_BITS),
                                                                 sizeof(uint32));
    ...
    *index = dnx_pemladrv_does_key_exists_in_physical_lpm_db(
            unit,
    ...
    if (*index != -1) {
        ...
        sal_free(virtual_key_data);
        return dnx_pem_lpm_set_by_id(unit, core_id, table_id, *index, key, &length_in_bits,
                                     result);
    }
    ...
    *index = dnx_pemladrv_find_lpm_index_to_insert(unit,
    ...
    if (api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id].physical_entry_occupation[*index] == 0) {
        dnx_pem_lpm_set_by_id(unit, core_id, table_id, *index, key, &length_in_bits, result);
        sal_free(virtual_key_data);
        return 0;
	}
    ...
    dnx_pemladrv_shift_physical_to_clear_entry(unit, core_id, shift_direction, index, table_id);
    dnx_pem_lpm_set_by_id(unit, core_id, table_id, *index, key, &length_in_bits, result);
    sal_free(virtual_key_data);
	return 0;
}
...
int pemladrv_lpm_insert(
    ...
    if (core_id==-1 ){
        for (int core_i=0; core_i<PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_i){
            res += pemladrv_lpm_insert_inner(unit, core_i, table_id, key, length_in_bits, result, index);
        }
    }
    else{
        res = pemladrv_lpm_insert_inner(unit, core_id, table_id, key, length_in_bits, result, index);
    }
...
int pemladrv_lpm_remove(
    ...
    int *index)
{
  *index = dnx_pemladrv_find_physical_entry_index_by_key(unit, core_id, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id].physical_entry_occupation, api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id].lpm_key_entry_arr, key);
  if (*index == -1) {
    
    return 1; 
  }
  return pemladrv_lpm_remove_by_index(unit, core_id, table_id, *index);
}
...
int pemladrv_lpm_lookup(
    ...
    int *index)
{
  *index = dnx_pemladrv_find_physical_lpm_entry_index_by_key(unit, core_id, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id], key);
  if (*index == -1) {
    
    return 1; 
  }
  return dnx_pemladrv_get_lpm_data_by_entry(unit, core_id, table_id, *index, key, result);
}
    ...
    pemladrv_mem_t * result)
{
  ...
  error = dnx_logical_cam_based_db_write(&api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, unit, core_id, index, key, mask, valid, result);
...
int pemladrv_lpm_remove_all_inner(
    const int unit,
    const int core_id,
    table_id_t table_id)
{
  ...
  unsigned int index;
      for (index = 0; index < nof_entries; ++index) {
          if (api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id].physical_entry_occupation[index] == 1) {
              pemladrv_lpm_remove_by_index(unit, core_id, table_id, index);
          }
      }
...
int pemladrv_lpm_remove_all(
    ...
    if (core_id == -1){
        for (int core_i=0; core_i<PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_i){
            result += pemladrv_lpm_remove_all_inner(unit, core_i, table_id);
        }
    }
    else{
        result = pemladrv_lpm_remove_all_inner(unit, core_id, table_id);
    }
...
int pemladrv_lpm_remove_by_index(
    const int unit,
    const int core_id,
    table_id_t table_id,
    int index) {

    dnx_pemladrv_set_valid_bit_to_zero(unit, core_id, index,
                                       &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info);
    ...
    unsigned int **virtual_key_data)
{
  ...
  *virtual_key_mask = (unsigned int*)pemladrv_calloc((*virtual_key_mask), (size_t)pemladrv_ceil(total_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  *virtual_key_data = (unsigned int*)pemladrv_calloc((*virtual_key_data), (size_t)pemladrv_ceil(total_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
    ...
    unsigned int **virtual_result_data)
{
  ...
  *virtual_result_mask = (unsigned int*)pemladrv_calloc((*virtual_result_mask), (size_t)pemladrv_ceil(total_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  *virtual_result_data = (unsigned int*)pemladrv_calloc((*virtual_result_data), (size_t)pemladrv_ceil(total_width,  UINT_WIDTH_IN_BITS), sizeof(uint32));
    ...
    unsigned int *physical_memory_entry_data)
{
  ...
  if (flag == TCAM) {
    ...
    one_chunk_virt_data = (unsigned int*)pemladrv_calloc(one_chunk_virt_data, 1 + (size_t)pemladrv_ceil(chunk_mapping_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
    one_chunk_virt_mask = (unsigned int*)pemladrv_calloc(one_chunk_virt_mask, 1 + (size_t)pemladrv_ceil(chunk_mapping_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
    ...
    one_chunk_virt_mask = (unsigned int*)pemladrv_calloc(one_chunk_virt_mask, (1 + (size_t)pemladrv_ceil((chunk_physical_mem_width_offset + chunk_mapping_width),  UINT_WIDTH_IN_BITS)), sizeof(uint32));
    one_chunk_virt_data = (unsigned int*)pemladrv_calloc(one_chunk_virt_data, (1 + (size_t)pemladrv_ceil((chunk_physical_mem_width_offset + chunk_mapping_width),  UINT_WIDTH_IN_BITS)), sizeof(uint32));
    ...
    PhysicalWriteInfo ** target_physical_memory_entry_data_list)
{
  ...
  target_physical_memory_entry_data_new_element = (PhysicalWriteInfo*)pemladrv_calloc(target_physical_memory_entry_data_new_element, 1, sizeof(PhysicalWriteInfo));
  entry_data_arr_nof_entries = dnx_pemladrv_calc_nof_arr_entries_from_width(chunk_phy_mem_width);
  entry_data_arr_nof_entries += 1; 
  target_physical_memory_entry_data_new_element->entry_data = (unsigned int*)pemladrv_calloc(target_physical_memory_entry_data_new_element->entry_data, entry_data_arr_nof_entries, sizeof(unsigned int));
    ...
    PhysicalWriteInfo * target_physical_memory_entry_data_curr)
{
    ...
    while(target_physical_memory_entry_data_curr) {
      ret_val = pem_write(unit, &target_physical_memory_entry_data_curr->mem, !(target_physical_memory_entry_data_curr->mem.is_reg), target_physical_memory_entry_data_curr->entry_data);
      if(ret_val)
        break;
...
dnx_produce_valid_field_bit_range(){
  FieldBitRange *valid_bit_array = (FieldBitRange *)pemladrv_calloc(valid_bit_array, 1, sizeof(FieldBitRange));
    ...
    unsigned int *virtual_db_data_arr)
{
  ...
  read_data = (unsigned int*)pemladrv_calloc(read_data, 2, sizeof(unsigned int));
  phy_mem   = (phy_mem_t*)pemladrv_calloc(phy_mem, 1, sizeof(phy_mem_t));
    ...
    unsigned int *tcam_data)
{
  ...
  key_arr = (unsigned int*)pemladrv_calloc(key_arr, (size_t)pemladrv_ceil(key_width_in_bits, UINT_WIDTH_IN_BITS), sizeof(uint32));
    ...
    const unsigned int *tcam_data)
{
  ...
  mask_arr = (unsigned int*)pemladrv_calloc(mask_arr, (size_t)pemladrv_ceil(total_key_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
...
void dnx_pemladrv_update_lpm_cache_from_physical(const int unit) {
      ...
      for (index = 0; index < api_info[unit].db_lpm_container.nof_lpm_dbs; ++index) {
...
#ifdef BCM_DNX_SUPPORT
          tcam_data          = (unsigned int *) pemladrv_calloc(tcam_data, UTILEX_DIV_ROUND_UP(2 * total_key_width + 1, SAL_UINT32_NOF_BITS), sizeof(uint32));
#else
          tcam_data = (unsigned int *) calloc((size_t) ceil((2.0f * total_key_width + 1) / UINT_WIDTH_IN_BITS),
                                              sizeof(uint32)); 
#endif 
          ...
          for (entry = 0; entry < PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES; ++entry) {
...
#ifdef BCM_DNX_SUPPORT
              result_arr = (unsigned int *) pemladrv_calloc(result_arr, UTILEX_DIV_ROUND_UP(total_result_width, SAL_UINT32_NOF_BITS), sizeof(uint32));
#else
              result_arr = (unsigned int *) calloc((size_t) ceil((1.0f * total_result_width) / UINT_WIDTH_IN_BITS),
                                                   sizeof(uint32));
#endif 
...
#else
              valid = (tcam_data[(size_t) ceil((2.0f * total_key_width + 1) / UINT_WIDTH_IN_BITS) - 1])
                      >> ((2 * total_key_width + 1) % UINT_WIDTH_IN_BITS - 1);
#endif 
              ...
              if (!valid) {
                  ...
                  dnx_pemladrv_update_lpm_cache(core_id, entry, prefix_length, key_arr, result_arr,
                                                &api_info[unit].db_lpm_container.db_lpm_info_arr[index]);
              }
          }
          sal_free(tcam_data);
    ...
    pemladrv_mem_t * mem_access)
{
  ...
  for (field_index = 0; field_index < mem_access->nof_fields; ++field_index) {
    ...
    if (__test_verify_read_from_cache){
        const unsigned int* in_buff = virtual_db_data_array;
        int in_buf_offset = field_bit_range_arr[field_id].lsb + offset;
        unsigned int *out_buff = mem_access->fields[field_index]->fldbuf;
        if (__test_debug_mode) {
            sal_printf("__READ__ from cache: data=");
        }
        for(int field_i=0; field_i<ceil(field_width/32.0); ++field_i){
            sal_printf("[%d]%08X ", field_i, in_buff[field_i]);

            if (in_buff[field_i] != out_buff[field_i]){
                sal_printf("\n invalid data during read field=%d ; expected=%08X ; actual=%08X  \n",
                       field_i, in_buff[field_i], out_buff[field_i]);
                assert(false);
            }
        }
        sal_printf("\n");
    ...
    const unsigned int *key)
{
  ...
  unsigned int *temp_key_data = (unsigned int*)pemladrv_calloc(temp_key_data, (size_t)pemladrv_ceil(key_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
    ...
    const pemladrv_mem_t * key)
{
  ...
  pemladrv_mem_t* mask = (pemladrv_mem_t*)pemladrv_calloc(mask, 1, sizeof(pemladrv_mem_t));

  mask->nof_fields = key->nof_fields;
  mask->flags   = key->flags;
  mask->fields  = (pemladrv_field_t**)pemladrv_calloc(mask->fields, mask->nof_fields, sizeof(pemladrv_field_t*));
  for (field_index = 0; field_index < mask->nof_fields; ++field_index) {
    mask->fields[field_index] = (pemladrv_field_t*)pemladrv_calloc(mask->fields[field_index], 1, sizeof(pemladrv_field_t));
    ...
    mask->fields[field_index]->fldbuf = (unsigned int*)pemladrv_calloc(mask->fields[field_index]->fldbuf, num_of_chunks_in_fldbuf, sizeof(unsigned int));
  }
  return mask;
}
    ...
    pemladrv_mem_t * key)
{
  ...
  virtual_mask = (unsigned int*)pemladrv_calloc(virtual_mask, num_of_uint_chunks_in_mask, sizeof(unsigned int));
...
pemladrv_mem_t * dnx_pemladrv_build_tcam_valid(){
  ...
  pemladrv_mem_t* valid       = (pemladrv_mem_t*)pemladrv_calloc(valid, 1, sizeof(pemladrv_mem_t));
  valid->fields               = (pemladrv_field_t**)pemladrv_calloc(valid->fields, 1, sizeof(pemladrv_field_t*));
  valid->fields[0]            = (pemladrv_field_t*)pemladrv_calloc(valid->fields[0], 1, sizeof(pemladrv_field_t));
  valid->flags = 0;
  valid->nof_fields = 1;
  valid->fields[0]->flags        = 0;
  valid->fields[0]->field_id     = 0;
  valid->fields[0]->fldbuf       = (unsigned int*)pemladrv_calloc(valid->fields[0]->fldbuf, 1, sizeof(unsigned int));
...
int dnx_pemladrv_find_physical_entry_index_by_key(
  ...
  unsigned int   *virtual_key_data = (unsigned int*)pemladrv_calloc(virtual_key_data, (size_t)pemladrv_ceil(total_key_width, UINT_WIDTH_IN_BITS), sizeof(uint32));

  dnx_build_virtual_db_data(0, key_field_bit_rang_arr, key, virtual_key_data);
  ret = dnx_pemladrv_does_key_exists_in_physical_db(unit, tcam_info, valids_arr, key_field_arr, virtual_key_data);
  sal_free(virtual_key_data);
  return ret;
}
...
int dnx_pemladrv_find_physical_lpm_entry_index_by_key(
  ...
  virtual_key_data = (unsigned int*)pemladrv_calloc(virtual_key_data, (size_t)pemladrv_ceil(total_key_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
  dnx_build_virtual_db_data(0, key_field_bit_rang_arr, key, virtual_key_data);
...
void dnx_pemladrv_move_physical_lpm_entry(
    ...
    pemladrv_lpm_remove_by_index(unit, core_id, table_id, old_index);
    dnx_pem_lpm_set_by_id(unit, core_id, table_id, new_index, key, &length_in_bits, result);
    ...
    if (shift_direction == UP) {
        *index = *index - 1;                   
        ...
        next_free_entry = dnx_pemladrv_find_next_lpm_free_entry(unit, core_id, *index, UP,
                                                                api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id].physical_entry_occupation);
        for (core_id_idx = 0; core_id_idx < PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_id_idx) {
            for (index_to_write_to = next_free_entry; index_to_write_to != *index; ++index_to_write_to) {
                dnx_pemladrv_move_physical_lpm_entry(unit, core_id_idx, index_to_write_to + 1, index_to_write_to,
                                                     table_id);
            }
        }
        return 0;
    ...
    pemladrv_mem_t * pem_mem_access)
{
  ...
  for (index = 0; index < nof_fields; ++index) {
    if (bit_range_array[index].is_field_mapped) {
      ...
      pemladrv_mem_alloc_field(pem_mem_access->fields[mapped_field_index], field_width_in_bits);
...
void dnx_pemladrv_set_valid_bit_to_zero(
    const int unit,
    const int core_id,
    ...
    for (int core_id = 0; core_id < PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_id) {
        for (i = 0; i < nof_dbs; ++i) {
            if (NULL ==
                ...
                continue;
            for (entry_index = 0; entry_index < PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES; ++entry_index)
                dnx_pemladrv_set_valid_bit_to_zero(unit, core_id, entry_index,
...
int get_core_from_memory_address(const int unit, unsigned long long address) {
...
static int dnx_pemladrv_create_pemladrv_mem_struct_db_direct(
    pemladrv_mem_t ** mem_access,
    LogicalDirectInfo * db_info) {
    const int core_id = 0; 
    return dnx_pemladrv_allocate_pemladrv_mem_struct(mem_access,
                                                     db_info->db_direct_info_per_core_arr[core_id].nof_fields,
                                                     db_info->db_direct_info_per_core_arr[core_id].field_bit_range_arr);
}
static void dnx_pemladrv_set_mbist_value_to_field_db_direct(
    pemladrv_mem_t * mem_access,
    LogicalDirectInfo * db_info,
    unsigned int field_val) {
    const int core_id = 0; 
...
static int dnx_pemladrv_compare_pemladrv_mem_structs_db_direct(
    pemladrv_mem_t * mem_access_a,
    pemladrv_mem_t * mem_access_b,
    LogicalDirectInfo * db_info) {
    const int core_id = 0; 
...
dnx_pemladrv_pem_mem_direct_db_virtual_bist(int unit)
{
  ...
  for (db_ndx = 0; db_ndx < nof_direct_dbs; ++db_ndx){
    ...
    for (entry_ndx = 0; entry_ndx < nof_entries; ++entry_ndx){
      ...
      if(pemladrv_direct_write(unit, core_id, db_ndx, entry_ndx, data_to_write_to_design) != 0)
      {
          error=1;
      }
      pemladrv_direct_read(unit, core_id,  db_ndx, entry_ndx, data_read_from_design);
...
static int dnx_pemladrv_create_pemladrv_mem_struct_db_tcam(
    pemladrv_mem_t ** key,
    pemladrv_mem_t ** mask,
    pemladrv_mem_t ** valid,
    pemladrv_mem_t ** result,
    ...
    ret_val |= dnx_pemladrv_allocate_pemladrv_mem_struct(key, nof_fields_in_key,
                                                         db_info->db_tcam_per_core_arr[core_id].key_field_bit_range_arr);
    ret_val |= dnx_pemladrv_allocate_pemladrv_mem_struct(mask, nof_fields_in_key,
                                                         db_info->db_tcam_per_core_arr[core_id].key_field_bit_range_arr);
    ret_val |= dnx_pemladrv_allocate_pemladrv_mem_struct(result, nof_fields_in_result,
...
static void dnx_pemladrv_set_mbist_value_to_field_db_tcam(
    pemladrv_mem_t * key,
    pemladrv_mem_t * mask,
    pemladrv_mem_t * valid,
    pemladrv_mem_t * result,
...
static int dnx_pemladrv_compare_pemladrv_mem_structs_db_tcam(
...
static void dnx_pemladrv_free_pemladrv_mem_structs_db_tcam(
    pemladrv_mem_t ** key,
    pemladrv_mem_t ** mask,
    pemladrv_mem_t ** valid,
    pemladrv_mem_t ** result)
...
static int dnx_pemladrv_pem_mem_tcam_db_virtual_bist(int unit){
  ...
  for (db_ndx = 0; db_ndx < nof_dbs; ++db_ndx){
    ...
    for (entry_ndx = 0; entry_ndx < nof_entries; ++entry_ndx){
      ...
      pemladrv_tcam_write(unit, core_id, db_ndx, entry_ndx, key_write, mask_write, valid_write, result_write);
      pemladrv_tcam_read(unit, core_id, db_ndx, entry_ndx, key_read, mask_read, valid_read, result_read);
...
static void dnx_pemladrv_set_mbist_value_to_field_db_em_lpm(
    pemladrv_mem_t * key,
    pemladrv_mem_t * result,
    LogicalTcamInfo * db_info,
    unsigned int field_val) {
...
static int dnx_pemladrv_compare_pemladrv_mem_structs_db_em_lpm(
    pemladrv_mem_t * key_write,
    pemladrv_mem_t * key_read,
    pemladrv_mem_t * result_write,
    pemladrv_mem_t * result_read,
...
static void dnx_pemladrv_free_pemladrv_mem_structs_db_em_lpm(
    pemladrv_mem_t ** key,
    pemladrv_mem_t ** result)
{
  dnx_pemladrv_free_pemladrv_mem_struct(key);
...
static int dnx_pemladrv_pem_mem_em_db_virtual_bist(int unit){
  ...
  for (db_ndx = 0; db_ndx < nof_dbs; ++db_ndx){
    ...
    for (entry_ndx = 0; entry_ndx < nof_entries; ++entry_ndx){
      ...
      if(pemladrv_em_entry_set_by_id(unit, core_id, db_ndx, entry_ndx, key_write, result_write) != 0)
        error++;
      pemladrv_em_entry_get_by_id(unit, core_id, db_ndx, entry_ndx, key_read, result_read);
      if (dnx_pemladrv_compare_pemladrv_mem_structs_db_em_lpm(key_write, key_read, result_write, result_read, db_info))
        error = 1;

      pemladrv_em_remove_by_index(unit, core_id, db_ndx, entry_ndx);
      }
    }
    ...
    for (db_ndx = 0; db_ndx < nof_dbs; ++db_ndx) {
        ...
        for (entry_ndx = 0; entry_ndx < nof_entries; ++entry_ndx) {
            ...
            for (mbist_val_ndx = 0; mbist_val_ndx < nof_mbist_values; ++mbist_val_ndx) {
                ...
                dnx_pem_lpm_set_by_id(unit, core_id, db_ndx, entry_ndx, key_write, &length_in_bits_write, result_write);
                dnx_pem_lpm_get_by_id(unit, core_id, db_ndx, entry_ndx, key_read, &length_in_bits_read, result_read);
                if (dnx_pemladrv_compare_pemladrv_mem_structs_db_em_lpm(key_write, key_read, result_write, result_read,
                                                                        db_info))
                    error = 1;

                pemladrv_lpm_remove_by_index(unit, core_id, db_ndx, entry_ndx);
            }
        }
        dnx_pemladrv_free_pemladrv_mem_structs_db_em_lpm(&key_write, &result_write);
        dnx_pemladrv_free_pemladrv_mem_structs_db_em_lpm(&key_read, &result_read);
  ...
  for (reg_ndx = 0; reg_ndx < nof_regs; ++reg_ndx){
      for (int core_id=0; core_id<PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_id) {
          ...
          for (mbist_val_ndx = 0; mbist_val_ndx < nof_mbist_values; ++mbist_val_ndx) {
              ...
              pemladrv_reg_write(unit, core_id, reg_ndx, data_write);
              dnx_pemladrv_reg_read_and_retreive_read_bits_mask(unit, core_id, reg_ndx, data_read, mask);
    ...
    void* entry_data)
{
    ...
    if(!dnx_data_field.features.tcam_cs_is_tcam_direct_access_get(unit))
    {
        ...
        rv =  dnx_pem_direct_access_wrapper(unit, 0, FALSE, TRUE, (int)mem->block_identifier, tcam_mask_line_address,
        ...
        rv = dnx_pem_direct_access_wrapper(unit, 0, TRUE, TRUE, (int)mem->block_identifier, tcam_mask_line_address,
            mem->mem_width_in_bits, data);
        if(rv)
        {
            return rv;
        ...
        rv = utilex_bitstream_get_any_field(data, 0, 2, &valid_bits);
        if(rv)
        {
            return rv;
        }
        rv = utilex_bitstream_get_any_field(data, 2, 200, field_data);
        if(rv)
        {
            return rv;
        }
        ...
        rv = dnx_pem_direct_access_wrapper(unit, 0, FALSE, TRUE, (int)mem->block_identifier, tcam_key_line_address,
            mem->mem_width_in_bits, data);
        if(rv)
        {
            return rv;
        ...
        rv = dnx_pem_direct_access_wrapper(unit, 0, TRUE, TRUE, (int)mem->block_identifier, tcam_key_line_address,
            mem->mem_width_in_bits, data);
        if(rv)
        {
            return rv;
        ...
        rv = utilex_bitstream_get_any_field(data, 2, 200, field_data1);
        if(rv)
        {
            return rv;
        }
        ...
        rv = utilex_bitstream_set_any_field(field_data1, 5, 200, entry_data_as_uint);
        if(rv)
        {
            return rv;
        }
        ...
        rv = utilex_bitstream_set_any_field(field_data, 205, 200, entry_data_as_uint);
        if(rv)
        {
            return rv;
        }
  ...
  int entry_in_cache = -1;
    ...
    if (!currently_writing_applets)
    {
        ...
        if(mem->is_industrial_tcam)
        {
            ...
            return pem_cs_tcam_read_entry(unit, mem, entry_data);
        }
        else
        {
            
            return dnx_pem_direct_access_wrapper(unit, 0, TRUE, is_mem, (int)mem->block_identifier, mem->mem_address,
...
#ifdef TEST_CFG_API
      unsigned int * ui_entry_data = (unsigned int *)entry_data;
      core = get_core_from_block_id(mem->block_identifier);

      ulong addr_blk_and_mem = mem->block_identifier;
      addr_blk_and_mem <<= 32;
      addr_blk_and_mem |= mem->mem_address;

      if(__test_all_cores){
          nof_reads_per_core[core]++;
      }

      if (__test_verify_read){
          if (ht_contains(&__test_address_to_data, &addr_blk_and_mem) == HT_NOT_FOUND){
              sal_printf("\n read from uninitialized entry: core=%d , block_id=%04X , mem_address=%08X . \n",
                     core, mem->block_identifier, mem->mem_address);
              assert(false);
          }

          unsigned int* expected_data = HT_LOOKUP_AS(unsigned int*, &__test_address_to_data, &addr_blk_and_mem);
          unsigned int* actual_data = ui_entry_data;

          int nof_fields = ceil(mem->mem_width_in_bits/32.0);

          for (int field=0, nof_bits=mem->mem_width_in_bits; field<nof_fields; ++field, nof_bits-=32){
              unsigned int actual_field_data = (nof_bits>32) ? actual_data[field] : actual_data[field] & ((unsigned int)pow(2,nof_bits)-1);
              unsigned int expected_field_data = (nof_bits>32) ? expected_data[field] : expected_data[field] & ((unsigned int)pow(2,nof_bits)-1);
              if (actual_field_data!=expected_field_data) {
                  sal_printf("invalid data during read: core_id=%d , block_id=%04X , mem_address=%08X , field=%d , expected_data=%08X . actual_data=%08X \n",
                         core, mem->block_identifier, mem->mem_address, field, expected_field_data, actual_field_data);
                  assert(false);
              }
          }
      }
      if (__test_debug_mode) {
          sal_printf("__READ__ core_id=%d ; block_id=%04X ; mem_address=%08X ; \n", core, mem->block_identifier, mem->mem_address);
      }
      if (core != __test_core_id && !__test_all_cores){
          sal_printf("\n invalid core during read: core=%d , block_id=%04X , mem_address=%08X . expected_core=%d \n",
                 core, mem->block_identifier , mem->mem_address, __test_core_id);
          assert(false);
      }
...
int pem_write(
  ...
  int applet_mem_size_in_bits = api_info[unit].applet_info.applet_mem_info[0][0].length_in_bits;
    ...
    {   
        ...
        return dnx_pem_direct_access_wrapper(unit, 0, FALSE, is_mem, (int)mem->block_identifier, mem->mem_address,
            mem->mem_width_in_bits, entry_data);
    }
#else
  if (currently_writing_applets == 0) {  
