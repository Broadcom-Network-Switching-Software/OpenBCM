    ...
    switch (arr_history.decode_type)
    {
        ...
        case ARR_DECODE_IPV6_o_SRV6:
            if ((arr_history.nof_data_entries == 1) && (arr_history.opaque == 1))
            {
                ...
                SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", "ENC2", "", "out_pp_port", &out_pp_port_sig, 1));
                SHR_IF_ERR_EXIT(dnx_algo_port_pp_to_logical_get(unit, core, out_pp_port_sig, &logical_port));
                SHR_IF_ERR_EXIT(dnx_port_pp_recycle_app_get(unit, logical_port, &recycle_app_type));
                if (recycle_app_type == bcmPortControlRecycleAppExtendedEncap)
                {
    ...
    char to_n[RHNAME_MAX_SIZE])
{
    ...
    {
        data_type = 0;
        SHR_IF_ERR_EXIT(dnx_etps_arr_table_id_and_data_type_get(unit, core, arr_history, &table_id, &result_type));
    }
    else
    {
