...
#define DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_FORCE(_entry_handle, _entry_handle_id) \
    SHR_ALLOC_ERR_EXIT((_entry_handle).user_output_info, \
        sizeof(dbal_user_output_info_t) * DBAL_TABLE_MAX_NUM_OF_RESULT_FIELDS_PER_RES_TYPE, \
        (_entry_handle).table->table_name, "%s%s%s",EMPTY,EMPTY,EMPTY) ;

#define DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_CHECK(_entry_handle, _entry_handle_id) \
do \
{ \
    if ((_entry_handle).user_output_info == NULL) \
    { \
        SHR_ERR_EXIT(_SHR_E_MEMORY, "Failed to allocate memory for user_output_info for DBAL handle %d\n", (_entry_handle_id)); \
    } \
} while (0)

#define DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_CHECK_VOID(_entry_handle, _entry_handle_id) \
do \
{ \
    if ((_entry_handle).user_output_info == NULL) \
    { \
        LOG_ERROR(BSL_LOG_MODULE,(BSL_META_U(unit,"Failed to allocate memory for user_output_info for DBAL handle %d\n"), (_entry_handle_id))); \
        (_entry_handle).error_info.error_exists = _SHR_E_MEMORY; \
    } \
} while (0)

#define DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC(_entry_handle, _entry_handle_id) \
do \
{ \
    if ((_entry_handle).user_output_info == NULL) \
    { \
        DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_FORCE((_entry_handle), (_entry_handle_id)); \
        DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_CHECK((_entry_handle), (_entry_handle_id)); \
        sal_memset((_entry_handle).user_output_info, 0x0, \
                   sizeof(dbal_user_output_info_t) * DBAL_TABLE_MAX_NUM_OF_RESULT_FIELDS_PER_RES_TYPE); \
    } \
} while (0)

#define DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_VOID(_entry_handle, _entry_handle_id) \
do \
{ \
    if ((_entry_handle).user_output_info == NULL) \
    { \
        DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_FORCE((_entry_handle), (_entry_handle_id)); \
        DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_CHECK_VOID((_entry_handle), (_entry_handle_id)); \
        sal_memset((_entry_handle).user_output_info, 0x0, \
                   sizeof(dbal_user_output_info_t) * DBAL_TABLE_MAX_NUM_OF_RESULT_FIELDS_PER_RES_TYPE); \
    } \
} while (0)

#define DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_COPY(_src_entry_handle, _dst_entry_handle, _dst_entry_handle_id) \
do \
{ \
    ...
    { \
        ...
        { \
            DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_FORCE((_dst_entry_handle), (_dst_entry_handle_id)); \
            DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_CHECK((_dst_entry_handle), (_dst_entry_handle_id)); \
        } \
