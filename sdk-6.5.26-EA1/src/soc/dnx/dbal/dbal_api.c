    ...
    int unit)
{
    ...
    if (!dbal_is_initiated(unit))
    {
        ...
        if (!sw_state_is_warm_boot(unit))
        {
            ...
            sal_memset(&init_info, 0, sizeof(sw_state_htbl_init_info_t));
    ...
    uint32 attr_val)
{
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_NO_PAYLOAD)
    {
        if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_ZERO_SIZE_PAYLOAD_SUPPORT))
        {
            if (attr_val)
            {
                entry_handle->phy_entry.attr_flags |= DBAL_ATTR_FLAG_NO_PAYLOAD;
            }
        }
        else
        {
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_INVALID)
    {
        if (DBAL_TABLE_IS_TCAM(entry_handle->table))
        {
            if (attr_val)
            {
                entry_handle->phy_entry.attr_flags |= DBAL_ATTR_FLAG_INVALID;
            }
        }
        else
        {
    ...
    uint32 *attr_val)
{
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_INVALID)
    {
        if (DBAL_TABLE_IS_TCAM(entry_handle->table))
        {
            (*attr_val) = (entry_handle->phy_entry.attr_flags & DBAL_ATTR_FLAG_INVALID) ? TRUE : FALSE;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Attribute Invalid is supported for TCAM tables only\n");
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_NO_PAYLOAD)
    {
        ...
        if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_ZERO_SIZE_PAYLOAD_SUPPORT))
        {
            if (entry_handle->phy_entry.attr_flags & DBAL_ATTR_FLAG_NO_PAYLOAD)
            {
                (*attr_val) = TRUE;
            }
        }
    ...
    dbal_entry_action_flags_e flags)
{
        ...
        !SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_DEFAULT_NON_STANDARD))
    {
        ...
        {
            rv = dbal_actions_access_entry_get(unit, entry_handle, NULL);
        }
    }
    else
    {
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    {
        SHR_IF_ERR_EXIT_NO_MSG(dbal_actions_access_entry_clear(unit, entry_handle));
    }
