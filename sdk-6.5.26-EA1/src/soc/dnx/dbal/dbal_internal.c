 ...
 *   Activation of access procedures (dispatcher) which is
...
#define DBAL_VALIDATE_HANDLE_STATUS_BEFORE_ACTION                                                               \
...
extern dbal_field_types_defs_e dnx2_dbal_field_id_to_type[DBAL_NOF_FIELDS];
...
dbal_is_initiated(
    int unit)
{
    return UTILEX_NUM2BOOL(dbal_mngr[unit] && dbal_mngr[unit]->status > DBAL_STATUS_NOT_INITIALIZED);
}
    ...
    int unit)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(dbal_mngr[unit], sizeof(dbal_mngr_info_t), "dbal_mngr_info_t",
                                "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    handles_size = sizeof(dbal_entry_handle_t) * (DBAL_SW_NOF_ENTRY_HANDLES);

    SHR_ALLOC_SET_ZERO_ERR_EXIT(dbal_mngr[unit]->entry_handles_pool, handles_size,
...
static void
dbal_global_var_init(
    void)
{

    sal_memset(G_dbal_field_full_mask, 0xff, sizeof(G_dbal_field_full_mask));
}
    ...
    for (ii = 0; ii < DBAL_RES_INFO.nof_result_fields; ii++)
    {
        ...
        if (entry_handle->error_info.error_exists)
        {
            ...
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Error when setting init/const value for result field %s. table %s\n",
                         dbal_field_encoded_with_inst_to_string(unit,
                                                                entry_handle->error_info.field_and_instance,
                                                                field_string), entry_handle->table->table_name);
        }
    }
...
void
dbal_entry_handle_status_performed_set(
    dbal_entry_handle_t * entry_handle)
{
    ...
    uint8 operation)
{
    ...
    {
        entry_handles_pool_ptr->core_id = DBAL_CORE_NOT_INITIALIZED;
    }

    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_KEY_CONST_VALUE_EXISTS))
    {
        SHR_IF_ERR_EXIT(dbal_handle_take_update_init_key_value(unit, handle_index));
    }

    if (!SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE) &&
        SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_RESULT_CONST_VALUE_EXISTS))
    {
        SHR_IF_ERR_EXIT(dbal_handle_take_update_init_result_value(unit, entry_handles_pool_ptr));
    }

    entry_handles_pool_ptr->er_flags = 0x0;

    if (dnx_dbal_journal_is_done_init(unit))
    {
        if (dnx_dbal_journal_is_on(unit, DNX_DBAL_JOURNAL_ROLLBACK))
        {
            entry_handles_pool_ptr->er_flags |= DNX_DBAL_JOURNAL_ROLLBACK_FLAG;
        }

        if (dnx_dbal_journal_is_on(unit, DNX_DBAL_JOURNAL_COMPARISON))
        {
...
exit:
    if (mutex_taken)
    {
        SHR_SET_CURRENT_ERR(sal_mutex_give(dbal_mngr[unit]->handle_mutex));
    }
    ...
    uint32 *dst_handle_id)
{
    ...
    *dst_handle_id = handle_index;
    ...
    DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_COPY((*src_handles_pool_ptr), (*dst_handles_pool_ptr), (*dst_handle_id));
...
shr_error_e
dbal_entry_key_as_buffer_set(
    int unit,
    uint32 entry_handle_id,
    uint32 *key_buffer,
    int size_in_bits)
{
    int curr_field_index;
    ...
    if (size_in_bits < entry_handle->table->key_size)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "size_in_bits %d to small, expected value bigger than %d\n", size_in_bits,
                     entry_handle->table->key_size);
    }

    entry_handle->phy_entry.key_size = 0;

    for (curr_field_index = 0; curr_field_index < entry_handle->table->nof_key_fields; curr_field_index++)
    {
        table_field = &(entry_handle->table->keys_info[curr_field_index]);
        entry_handle->phy_entry.key_size += table_field->field_nof_bits;
        entry_handle->key_field_ids[curr_field_index] = table_field->field_id;
    }

    entry_handle->nof_key_fields = entry_handle->table->nof_key_fields;

    SHR_IF_ERR_EXIT(utilex_bitstream_set_bit_range(entry_handle->phy_entry.k_mask, 0, entry_handle->table->key_size));
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    DBAL_VALIDATE_HANDLE_STATUS_BEFORE_ACTION;

    DBAL_FIELD_ERR_HANDLE(dbal_mngr[unit]->iterators_pool[entry_handle_id].is_init,
                          "Key fields cannot be changed on handle with valid iterator", 0);
    ...
    for (curr_field_index = 0; curr_field_index < table->nof_key_fields; curr_field_index++)
    {
        ...
        if (SHR_BITGET(table->keys_info[curr_field_index].field_indication_bm, DBAL_FIELD_IND_IS_PARENT_FIELD))
        {
            DBAL_FIELD_ERR_HANDLE(dbal_fields_sub_field_match(unit, table->keys_info[curr_field_index].field_id,
                                                              field_id, &is_sub_field_found), "error sub field", 0);
    ...
    if (field_id == DBAL_FIELD_CORE_ID)
    {
        ...
        {
            DBAL_FIELD_ERR_HANDLE(TRUE, "Invalid value set for key field CORE_ID", 0);
        }
        ...
        && !SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ADDRESS))
    {
        ...
        if (is_sub_field)
        {
            DBAL_FIELD_ERR_HANDLE(dbal_fields_max_value_get(unit, field_id, &max_value),
                                  "Cannot get the field max value", 0);
        }
        ...
        if (!(SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM)))
        {
            ...
            DBAL_FIELD_ERR_HANDLE((is_illegal || ret_val), "Illegal value for field, in illegal list", 0);
        }
    }
    ...
    if (!(SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM)))
    {
        ...
        DBAL_FIELD_ERR_HANDLE(ret_val, "Copy from offset error for field", 0);
    }
    else
    {
    ...
    if ((SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENCODED)) || is_sub_field)
    {
        if (is_sub_field)
        {
            dbal_field_types_basic_info_t *child_field_info;
            ret_val = dbal_fields_field_types_info_get(unit, field_id, &child_field_info);
            if (child_field_info->encode_info.encode_mode != DBAL_VALUE_FIELD_ENCODE_NONE)
            {
                ret_val = dbal_fields_field_encode(unit, field_id, DBAL_FIELD_EMPTY, table_field->field_nof_bits,
                ...
                if (ret_val != _SHR_E_NONE)
                {
                    ...
                    if (!dbal_image_name_is_std_1(unit) && dbal_mngr[unit]->status != DBAL_STATUS_DEVICE_INIT_DONE)
                    {
                        ...
                        entry_handle->error_info.field_and_instance = field_id;
                        entry_handle->error_info.error_exists = _SHR_E_INTERNAL;
                    }
                    else
                    {
                        DBAL_FIELD_ERR_HANDLE(ret_val, "encoding error for field", 0);
                    }
                }
                copied_field_value[0] = encoded_field_value[0];
            }
        }
    ...
    DBAL_FIELD_ERR_HANDLE(ret_val, "utilex_bitstream_set_any_field from offset error for field", 0);

    if (field_mask && DBAL_TABLE_IS_KEY_MASK_REQUIRED(table))
    {
        uint32 *field_mask_ptr = field_mask;
        uint32 copied_field_value_mask[DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS] = { 0 };
        if (table_field->offset_in_logical_field != 0)
        {

            ret_val = utilex_bitstream_get_any_field(field_mask, table_field->offset_in_logical_field,
                                                     table_field->field_nof_bits, copied_field_value_mask);

            DBAL_FIELD_ERR_HANDLE(ret_val, "Copy from offset error for field mask", 0);
            field_mask_ptr = copied_field_value_mask;
        }
        ret_val =
            dbal_entry_key_field_mask_set(unit, field_id, table_field, entry_handle, is_sub_field, parent_field_id,
                                          field_mask_ptr);
        DBAL_FIELD_ERR_HANDLE(ret_val, "dbal_entry_key_field_mask_set issue", 0);
    }
    ...
    else
    {
        ...
        {
            if ((entry_handle->error_info.field_and_instance == field_id) && !dbal_image_name_is_std_1(unit))
            {
    ...
    uint32 max_val)
{
    ...
    DBAL_FIELD_ERR_HANDLE(((entry_handle->table->access_method != DBAL_ACCESS_METHOD_KBP) &&
                          ...
                          "Range actions for TCAM are supported only for KBP", 0);

    DBAL_FIELD_ERR_HANDLE(((min_val != DBAL_RANGE_ALL)
                           && (min_val > max_val)), "Min range is bigger than max range", 0);
    ...
    DBAL_FIELD_ERR_HANDLE((ii == entry_handle->table->nof_key_fields), "field not found for range", 0);
    DBAL_FIELD_ERR_HANDLE(((entry_handle->table->access_method == DBAL_ACCESS_METHOD_KBP) &&
                           (!SHR_BITGET(field_info->field_indication_bm, DBAL_FIELD_IND_IS_RANGED))),
                          "The field is not ranged", 0);
    ...
    if (min_val == DBAL_RANGE_ALL)
    {
        ...
        DBAL_FIELD_ERR_HANDLE(((entry_handle->table->access_method == DBAL_ACCESS_METHOD_KBP) &&
                              ...
                              "Min value for the field is bigger than the max supported value for KBP ranges", 0);
    }
    ...
    if (!dbal_mngr[unit]->is_std_1)
    {
        ...
        if (field_type_info->encode_info.encode_mode == DBAL_VALUE_FIELD_ENCODE_ENUM)
        {
            if ((min_val < field_type_info->nof_enum_values) && field_type_info->enum_val_info[min_val].is_invalid)
            {
                ...
                if (jj == field_type_info->nof_enum_values)
                {
    ...
    int res_type)
{
    ...
    DBAL_FIELD_ERR_HANDLE((field_id != DBAL_FIELD_RESULT_TYPE), "for table with multiple result types need"
                          " to configure entry type before setting any value field", inst_id);
    ...
    DBAL_FIELD_ERR_HANDLE(iterator_info->is_init,
                          "RESULT_TYPE field cannot be changed on handle with valid iterator", inst_id);

    DBAL_FIELD_ERR_HANDLE(((res_type > table->nof_result_types)
                           && (res_type != DBAL_SUPERSET_RESULT_TYPE)), "illegal result type value", inst_id);
    ...
    if (res_type != DBAL_SUPERSET_RESULT_TYPE)
    {
        ...
        DBAL_FIELD_ERR_HANDLE(ret_val, "mul res utilex_bitstream_set_any_field from offset error for field", inst_id);
        ...
        DBAL_FIELD_ERR_HANDLE(ret_val, "mul res utilex_bitstream_set_any_field from offset error for field", inst_id);
        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_RESULT_CONST_VALUE_EXISTS))
        {
    ...
    uint32 *field_mask)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
    {
        ...
        {
            DBAL_FIELD_ERR_HANDLE((inst_id != 0), "result type cannot be set with not default instance idx", inst_id);
            dbal_entry_mul_result_handle(unit, entry_handle, field_id, inst_id, (int) field_val[0]);
            return;
        }
        else
        {
            ...
            DBAL_FIELD_ERR_HANDLE((field_id == DBAL_FIELD_RESULT_TYPE),
                                  "for table with multiple result types cannot re-set the result type", inst_id);
        }
    }

    DBAL_FIELD_ERR_HANDLE(dbal_tables_field_info_get_internal(unit, entry_handle->table_id, field_id, 0,
                                                              entry_handle->cur_res_type, inst_id,
                                                              &table_field, &field_index_in_table, &parent_field_id),
                          "VALUE field not found in table", inst_id);
    ...
    if (!SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_VALID_INDICATION))
    {
        ...
        DBAL_FIELD_ERR_HANDLE(((dbal_field_legal_sizes[pointer_type]) &&
                               (table_field->field_nof_bits > dbal_field_legal_sizes[pointer_type])),
                              "field size too big for pointer type", inst_id);
    }
        ...
        && !SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ADDRESS))
    {
        ...
        {
            DBAL_FIELD_ERR_HANDLE(dbal_fields_max_value_get(unit, field_id, &max_value),
                                  "Cannot get the field max value", inst_id);
        }
            ...
            && !(SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM)))
        {
            ...
            DBAL_FIELD_ERR_HANDLE(ret_val, "dbal_field_arr_prefix_encode", inst_id);
        }
        else
        {
        ...
        {
            ret_val = dbal_fields_is_illegal_value(unit, field_id, field_val[0], &is_illegal);
            DBAL_FIELD_ERR_HANDLE((is_illegal || ret_val), "Illegal value for field, in illegals list", inst_id);
        }
    }
    ...
    if (!(SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM)))
    {
        ...
        DBAL_FIELD_ERR_HANDLE(ret_val, "Copy from offset error for field", inst_id);
        curr_field_value_ptr = copied_field_value;
    }
    else
    {
    ...
    if ((SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENCODED)) || is_sub_field)
    {
        if (is_sub_field)
        {
            ...
            if (child_field_info->encode_info.encode_mode != DBAL_VALUE_FIELD_ENCODE_NONE)
            {
                ...
                if (ret_val != _SHR_E_NONE)
                {
                    ...
                    DBAL_FIELD_ERR_HANDLE(ret_val, "encoding error for field", inst_id);
                    return;
                }
                copied_field_value[0] = encoded_field_value[0];
            }
        ...
        if (SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM))
        {
            ...
            if (table_field->arr_prefix_size != 0)
            {
                ret_val =
                    dbal_field_arr_prefix_decode(unit, table_field, curr_field_value_ptr[0], curr_field_value_ptr,
                                                 TRUE);
                DBAL_FIELD_ERR_HANDLE(ret_val, "dbal_field_arr_prefix_decode", inst_id);
            }
            if ((enum_max_value > 0) && (curr_field_value_ptr[0] > enum_max_value))
            {
                ...
                DBAL_FIELD_ERR_HANDLE(TRUE, "enum max value error", inst_id);
            }
        }
    }
    ...
    DBAL_FIELD_ERR_HANDLE(ret_val, "bitstream issue", inst_id);
    entry_handle->value_field_ids[field_index_in_table] = field_id;
    ...
    DBAL_FIELD_ERR_HANDLE(ret_val, "bitstream issue mask", inst_id);
    ...
    int inst_id)
{
    ...
    int instance_id = inst_id;
    dbal_table_field_info_t *table_field = NULL;

    DBAL_VALIDATE_HANDLE_STATUS_BEFORE_ACTION;

    if ((inst_id == INST_SINGLE) || (inst_id == INST_ALL))
    {
        instance_id = 0;
    }
    ...
    DBAL_FIELD_ERR_HANDLE(ret_val, "bitstream issue mask", instance_id);
    ...
    void *field_mask)
{
    ...
    DBAL_FIELD_ERR_HANDLE((table_field->permission == DBAL_PERMISSION_WRITEONLY),
                          "Cannot get value from write only field", inst_id);
    ...
    DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_ALLOC_VOID(*entry_handle, entry_handle_id);
    ...
    uint32 *field_val)
{
    ...
    if (entry_handle->core_id != DBAL_CORE_NOT_INITIALIZED)
    {
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "no available fields, need to perform get to receive fields res type %s\n",
    ...
    else
    {
        ...
        rv = dbal_field_from_buffer_get(unit, table_field_info, field_id, entry_handle->phy_entry.payload, field_val);
    ...
    uint8 format)
{
    ...
    if (DBAL_TABLE_IS_KEY_MASK_REQUIRED(table))
    {
        if ((table->physical_db_id[DBAL_PHY_DB_DEFAULT_INDEX] == DBAL_PHYSICAL_TABLE_TCAM) ||
            (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, table->physical_db_id[DBAL_PHY_DB_DEFAULT_INDEX])->db_type ==
    ...
    if ((log_enabled) || (dbal_mngr[unit]->logger_info.dbal_file != NULL))
    {
        ...
        for (iter = 0; iter < table->nof_key_fields; iter++)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dbal_field_in_table_printable_string_get(unit, key_field, 0,
        ...
        if ((entry_handle->nof_result_fields) || (entry_handle->get_all_fields))
        {
            ...
            {
                field_info = &table->multi_res_info[entry_handle->cur_res_type].results_info[iter];
                inst_idx = field_info->inst_idx;
                value_field = entry_handle->value_field_ids[iter];
                if (entry_handle->value_field_ids[iter] == DBAL_FIELD_EMPTY)
                {
                    if (entry_handle->get_all_fields)
                    {
                        value_field = field_info->field_id;
                    }
                    else
                    {
                        continue;
                ...
                SHR_IF_ERR_EXIT(dbal_field_in_table_printable_string_get(unit, value_field, inst_idx,
                                                                         entry_handle->table_id,
                                                                         entry_handle->phy_entry.payload, NULL,
                                                                         entry_handle->cur_res_type, FALSE, TRUE,
                                                                         buffer));
                DBAL_DUMP(dbal_field_with_inst_to_string(unit,
                                                         field_info->field_id,
                                                         field_info->inst_idx,
                                                         field_str),
        ...
        if (entry_handle->attribute_info[attr_index] != NULL)
        {
            ...
            switch (attr_index)
            {
                ...
                case DBAL_ENTRY_ATTR_RES_HIT:
                    ...
                    break;
                case DBAL_ENTRY_ATTR_RES_NO_PAYLOAD:
                    (*attr_val_ptr) = (entry_handle->phy_entry.attr_flags & DBAL_ATTR_FLAG_NO_PAYLOAD);
                    break;
                case DBAL_ENTRY_ATTR_RES_INVALID:
                    (*attr_val_ptr) = (entry_handle->phy_entry.attr_flags & DBAL_ATTR_FLAG_INVALID);
                    break;
            }
        }
    }
    ...
    uint32 *attr_val)
{
    ...
    if (attr_type == DBAL_ENTRY_ATTR_PRIORITY)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "priority get supported only for TCAM table\n");
        }
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_CACHED)
    {
        if (DBAL_TABLE_IS_TCAM(entry_handle->table) && ((entry_handle->table->access_method != DBAL_ACCESS_METHOD_MDB)))
        {
            entry_handle->attribute_info[DBAL_ENTRY_ATTR_RES_CACHED] = attr_val;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Cached is not supported this table %s\n", entry_handle->table->table_name);
        }
    }
    else if (attr_type == DBAL_ENTRY_ATTR_INVALID)
    {
        if (DBAL_TABLE_IS_TCAM(entry_handle->table) && (entry_handle->table->access_method == DBAL_ACCESS_METHOD_MDB))
        {
            entry_handle->attribute_info[DBAL_ENTRY_ATTR_RES_INVALID] = attr_val;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Invalid entry is not supported this tables %s\n",
                         entry_handle->table->table_name);
        }
    }
    else if (attr_type == DBAL_ENTRY_ATTR_NO_PAYLOAD)
    {
        if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_ZERO_SIZE_PAYLOAD_SUPPORT))
        {
            entry_handle->attribute_info[DBAL_ENTRY_ATTR_RES_NO_PAYLOAD] = attr_val;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "no_payload entry is not supported for table %s\n",
                         entry_handle->table->table_name);
        }
    }
    else
            ...
            if (is_sub_field)
            {
                ...
                if (sal_memcmp(verify_field_val, field_value, BITS2BYTES(field_nof_bits)) != 0)
                {
                    LOG_INFO_EX(BSL_LOG_MODULE,
                                "Subfield encoding issue: requested subfield %s from field %s but value %d didn't match%s\n",
                                dbal_field_to_string(unit, field_id),
                                dbal_field_to_string(unit, table_field_info->field_id), field_value[0], EMPTY);
                ...
                {
                    SHR_IF_ERR_EXIT(dbal_fields_field_decode
                                    (unit, field_id, DBAL_FIELD_EMPTY, field_nof_bits, curr_field_value_ptr,
                                     verify_field_val));
                    *curr_field_value_ptr = *verify_field_val;
                }
            }
        }
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    for (ii = nof_struct_fields - 1; ii >= 0; ii--)
    {
        ...
        if (field_type_info->struct_field_info[ii].struct_field_id != DBAL_FIELD_RESERVED)
        {
            ...
            buffer_to_print += sal_strnlen(buffer_to_print, DBAL_MAX_PRINTABLE_BUFFER_SIZE);
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    if (sub_field_id != DBAL_FIELD_EMPTY)
    {
        ...
        if (dbal_fields_is_field_encoded(unit, sub_field_id) || (sub_field_info->nof_struct_fields > 0))
        {

            return dbal_field_printable_string_get(unit, sub_field_id, sub_field_val, NULL, 1, DBAL_FIELD_EMPTY,
...
static shr_error_e
dbal_internal_format_uint32_array_as_hex(
    int unit,
    char *buffer_dst,
    uint32 *buffer_src,
    char *prefix_str,
...
{
    int iter;
    int end_idx = BITS2WORDS(size);
    int cur_char_count = 0;
    SHR_FUNC_INIT_VARS(unit);

    if (buffer_dst == NULL || buffer_src == NULL || prefix_str == NULL || suffix_str == NULL || separator_str == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "null parameters\n");
    }

    cur_char_count = sal_snprintf(buffer_dst, DBAL_MAX_STRING_LENGTH, prefix_str);
    buffer_dst += cur_char_count;
    *total_char_count += cur_char_count;

    if (sal_strncmp(separator_str, "", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        cur_char_count = sal_snprintf(buffer_dst, DBAL_MAX_STRING_LENGTH, "0x");
        buffer_dst += cur_char_count;
        *total_char_count += cur_char_count;
    }
    ...
    for (iter = 0; iter < end_idx; iter++)
    {
        ...
        cur_char_count = sal_snprintf(buffer_dst, DBAL_MAX_STRING_LENGTH, "%08x", buffer_src[iter]);
        buffer_dst += cur_char_count;
        *total_char_count += cur_char_count;
        cur_char_count = sal_snprintf(buffer_dst, DBAL_MAX_STRING_LENGTH, separator_str);
        buffer_dst += cur_char_count;
        *total_char_count += cur_char_count;
    }
    if (sal_strncmp(separator_str, "", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
    ...
    cur_char_count = sal_snprintf(buffer_dst, DBAL_MAX_STRING_LENGTH, suffix_str);
    buffer_dst += cur_char_count;
    *total_char_count += cur_char_count;
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    dbal_field_types_basic_info_t *field_type_info;
    dbal_field_print_type_e curr_print_type;
    dbal_logger_print_type_e logger_print_type;
    ...
    curr_print_type = field_type_info->print_type;
    SHR_IF_ERR_EXIT(dbal_logger_print_type_get_internal(unit, &logger_print_type));

    if ((logger_print_type == DBAL_LOGGER_PRINT_TYPE_AS_HEX) && ((curr_print_type == DBAL_FIELD_PRINT_TYPE_UINT32)))
    {
        curr_print_type = DBAL_FIELD_PRINT_TYPE_HEX;
    }

    switch (curr_print_type)
    {
        ...
        case DBAL_FIELD_PRINT_TYPE_UINT32:
            ...
            {
                char_count =
                    sal_snprintf(buffer_to_print, DBAL_MAX_PRINTABLE_BUFFER_SIZE, "%u (%u)", decoded_field_val[0],
                                 field_val[0]);
                buffer_to_print += char_count;
            ...
            {
                char_count = sal_snprintf(buffer_to_print, DBAL_MAX_PRINTABLE_BUFFER_SIZE, "%u", field_val[0]);
                buffer_to_print += char_count;
            }
            break;
        ...
        case DBAL_FIELD_PRINT_TYPE_IPV6:
        {
            ...
            if (logger_print_type == DBAL_LOGGER_PRINT_TYPE_DEFAULT)
            {
            ...
            else if (logger_print_type == DBAL_LOGGER_PRINT_TYPE_AS_HEX)
            {
                SHR_IF_ERR_EXIT(dbal_internal_format_uint32_array_as_hex(unit, buffer_to_print, field_val, "", "",
                                                                         "", size, &char_count));
                buffer_to_print += char_count;
            }
            break;
        ...
        case DBAL_FIELD_PRINT_TYPE_ARRAY8:
        {
            ...
            else if (logger_print_type == DBAL_LOGGER_PRINT_TYPE_AS_HEX)
            {
                ...
                if (is_field_encoded)
                {
                    SHR_IF_ERR_EXIT(dbal_internal_format_uint32_array_as_hex
                                    (unit, buffer_to_print, decoded_field_val, "(", ")", "", size, &char_count));
                    buffer_to_print += char_count;
                }
            }
            break;
        ...
        case DBAL_FIELD_PRINT_TYPE_ENUM:
        {
            ...
            if (enum_val == UTILEX_U32_MAX || enum_val >= field_type_info->nof_enum_values)
            {
                char *err_msg = "Unknown HW enum value: ";
                char_count =
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    dbal_value_field_encode_types_e field_encode_mode;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dbal_fields_encode_type_get(unit, field_id, &field_encode_mode));
    if ((is_full_buffer == 0) && (field_encode_mode == DBAL_VALUE_FIELD_ENCODE_ENUM))
    {
    ...
    else
    {
        SHR_IF_ERR_EXIT(dbal_tables_field_info_get
                        (unit, table_id, field_id, is_key, result_type_idx, inst_idx, &table_field_info));
    ...
    char_count = sal_strnlen(buffer_to_print, DBAL_MAX_PRINTABLE_BUFFER_SIZE);
    buffer_to_print += char_count;
    ...
    uint8 *field_mask)
{
    ...
    GET_FIELD_TYPE(unit, field_id, &field_type);
    if (field_type == DBAL_FIELD_TYPE_DEF_L2_MAC)
    {
    ...
    uint32 *field_mask_u32)
{
    ...
    {
        int size = sal_strnlen((char *) (field_val), SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
            ...
            else
            {
                ...
                *is_passed = 0;
                ...
                res = dbal_tables_field_info_get_no_err(unit, entry_handle->table_id, condition.field_id,
                                                        FALSE, entry_handle->cur_res_type, condition.inst_idx,
                                                        &field_info);
                ...
                res = dbal_entry_value_field_get(unit, entry_handle, condition.field_id, condition.inst_idx,
                                                 compared_value, NULL, DBAL_POINTER_TYPE_ARR_UINT32);

                SHR_IF_ERR_EXIT_EXCEPT_IF(res, _SHR_E_CONFIG);
                if (res == _SHR_E_CONFIG)
                {
                    *is_passed = FALSE;
                    SHR_EXIT();
                }
    ...
    dbal_key_value_validity_e * is_valid)
{
    ...
    *is_valid = DBAL_KEY_IS_VALID;
    ...
    if ((entry_handle->table->access_method == DBAL_ACCESS_METHOD_HARD_LOGIC) &&
        (entry_handle->table->table_type == DBAL_TABLE_TYPE_DIRECT) &&
        SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_ASYMMETRIC))
    {
        int rv = _SHR_E_NONE;
        int result_type = 0;
        ...
        do
        {
            rv = dbal_hl_unmapped_access_handle(unit, entry_handle, result_type);
            if (rv != _SHR_E_ACCESS)
            {
                SHR_SET_CURRENT_ERR(rv);
                SHR_EXIT();
        ...
        while (result_type < entry_handle->table->nof_result_types);

        if (result_type == entry_handle->table->nof_result_types)
        {
            *is_valid = DBAL_KEY_IS_INVALID;
            SHR_EXIT();
        }
...
shr_error_e
dbal_logger_print_type_set_internal(
    int unit,
    dbal_logger_print_type_e print_type)
{
    SHR_FUNC_INIT_VARS(unit);

    if (print_type >= DBAL_LOGGER_NOF_PRINT_TYPES)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "print type %d is out of range\n", print_type);
    }

    dbal_mngr[unit]->logger_info.print_type = print_type;
...
shr_error_e
dbal_logger_print_type_get_internal(
    int unit,
    dbal_logger_print_type_e * print_type)
{
    SHR_FUNC_INIT_VARS(unit);

    *print_type = dbal_mngr[unit]->logger_info.print_type;
    ...
    uint32 *field_mask)
{
    ...
    if (iterator->nof_key_rules == 0)
    {
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT(iterator->key_rules_info, 1 * sizeof(dbal_access_condition_info_t),
    ...
    else
    {
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT(temp_cond_rule_ptr,
    ...
    uint32 *field_mask)
{
    ...
    if (iterator_info->nof_val_rules == 0)
    {
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT(iterator_info->val_rules_info, 1 * sizeof(dbal_access_condition_info_t),
    ...
    iterator_info->val_rules_info[iterator_info->nof_val_rules].inst_idx = inst_idx;
    ...
    uint32 *field_mask)
{
    ...
    if (iterator_info->nof_actions == 0)
    {
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT(iterator_info->actions_info, 1 * sizeof(dbal_iterator_actions_info_t),
    ...
    else
    {
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT(temp_action_ptr,
    ...
    if (action_type == DBAL_ITER_ACTION_UPDATE)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Action update cannot be set with field EMPTY or RESULT TYPE\n");
        }
        else
        {
            SHR_IF_ERR_EXIT(dbal_fields_max_size_get(unit, field_id, &field_max_size));
    ...
    char *top_node_name)
{
    ...
    while ((dirp = sal_readdir(dir)) != NULL)
    {
        ...
        if ((sal_strncmp(dirp->d_name, ".", sizeof(dirp->d_name)) == 0)
            || ((sal_strncmp(dirp->d_name, "..", sizeof(dirp->d_name)) == 0)))
        {
            continue;
        }
        SHR_FREE(path);
        SHR_ALLOC_ERR_EXIT(path,
                           (sal_strnlen(parent_dir, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) +
                            sal_strnlen("/", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) +
                            sal_strnlen(dirp->d_name, sizeof(dirp->d_name)) + 100),
                           "dir path", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
        ...
        if (sal_strncmp
            (path + (sal_strnlen(path, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) - 4), ".xml",
...
/* *INDENT-OFF* */
#define DBAL_KBP_DUMMY_PROTOTYPE(_unit_) \
SHR_FUNC_INIT_VARS(_unit_);SHR_ERR_EXIT(_SHR_E_INTERNAL,"KBP SDK is not compiled\n");exit:SHR_FUNC_EXIT

shr_error_e dbal_kbp_entry_get(int unit, dbal_entry_handle_t * entry_handle){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_entry_add(int unit, dbal_entry_handle_t * entry_handle){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_entry_delete(int unit, dbal_entry_handle_t * entry_handle) {DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_table_clear(int unit, dbal_entry_handle_t * entry_handle){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_table_iterator_init(int unit, dbal_entry_handle_t * entry_handle){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_table_entry_get_next(int unit, dbal_entry_handle_t * entry_handle){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_table_iterator_deinit(int unit, dbal_entry_handle_t * entry_handle){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_access_id_by_key_get(int unit, dbal_entry_handle_t * entry_handle){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_fwd_caching_enabled_get(int unit, int *enabled){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_fwd_caching_enabled_set(int unit, int enabled){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_fwd_cache_commit(int unit){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
shr_error_e dbal_kbp_res_type_resolution(int unit, dbal_entry_handle_t * entry_handle, uint8 payload_size){DBAL_KBP_DUMMY_PROTOTYPE(unit);}


#endif 

dbal_maturity_level_e
dbal_convert_to_basic_maturity_level(
    dbal_maturity_level_e maturity_level)
{
    switch (maturity_level)
    {
        case DBAL_MATURITY_LOW:
        case DBAL_MATURITY_PARTIALLY_FUNCTIONAL:
        case DBAL_MATURITY_HIGH:
            return maturity_level;
        case DBAL_MATURITY_HIGH_SKIP_ADAPTER:
            return DBAL_CONVERT_HIGH_SKIP_ADAPTER_TO_BASIC_MATURITY_LEVEL();
        case DBAL_MATURITY_HIGH_SKIP_DEVICE:
            return DBAL_CONVERT_HIGH_SKIP_DEVICE_TO_BASIC_MATURITY_LEVEL();
        default:
            return maturity_level;
    }
}
...
uint8
dbal_maturity_level_is_high(
    dbal_maturity_level_e maturity_level)
{
    dbal_maturity_level_e basic_maturity_level = dbal_convert_to_basic_maturity_level(maturity_level);
    return UTILEX_NUM2BOOL(basic_maturity_level == DBAL_MATURITY_HIGH);
}

dbal_field_types_defs_e
dbal_field_id_to_field_type(
    int unit,
    dbal_fields_e dbal_field)
{
    return dnx2_dbal_field_id_to_type[dbal_field];
}
