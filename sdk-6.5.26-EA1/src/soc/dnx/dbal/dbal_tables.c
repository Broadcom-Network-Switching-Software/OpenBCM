    ...
    dbal_logical_table_t * table)
{
    ...
    for (iter = 0; iter < nof_result_fields; iter++)
    {
        ...
        {
            table->multi_res_info[result_type_index].results_info[iter].max_value =
                UTILEX_MIN(table->multi_res_info[result_type_index].results_info[iter].max_value,
                           results_info[iter].max_value);
        }
    ...
    int inst_idx)
{
    ...
    dbal_table_field_info_t *field_info_p;
    dbal_logical_table_t *table = NULL;
    dbal_table_field_info_t *temp_res_info = NULL;
    int is_active;
    ...
    if (is_key)
    {
        dbal_table_field_info_t temp_keys_info[DBAL_TABLE_MAX_NUM_OF_KEY_FIELDS] = { {0}
        };

        if (field_index_in_table != table->nof_key_fields - 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "API allow only to remove the last field %s, requested %s\n",
                         dbal_field_to_string(unit, table->keys_info[table->nof_key_fields - 1].field_id),
                         dbal_field_to_string(unit, field_id));
        }
        sal_memcpy(&temp_keys_info[0], table->keys_info, sizeof(dbal_table_field_info_t) * table->nof_key_fields);
        SHR_FREE(table->keys_info);
        table->nof_key_fields = table->nof_key_fields - 1;

        SHR_ALLOC_SET_ZERO(table->keys_info, table->nof_key_fields * sizeof(dbal_table_field_info_t),
                           "key fields info allocation", "%s%s%s\r\n", table->table_name, EMPTY, EMPTY);

        sal_memcpy(table->keys_info, &temp_keys_info[0], sizeof(dbal_table_field_info_t) * table->nof_key_fields);
    ...
    {
        int jj, res_type;
        multi_res_info_t *multi_res_info = &table->multi_res_info[result_type_idx];
        dbal_hard_logic_access_types_e hl_access_type;

        SHR_ALLOC_SET_ZERO(temp_res_info, multi_res_info->nof_result_fields * sizeof(dbal_table_field_info_t),
                           "key fields info allocation", "%s%s%s\r\n", table->table_name, EMPTY, EMPTY);

        if (field_index_in_table != multi_res_info->nof_result_fields - 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "API allow only to remove the last field %s, requested %s\n",
                         dbal_field_to_string(unit,
                                              multi_res_info->results_info[multi_res_info->nof_result_fields -
                                                                           1].field_id), dbal_field_to_string(unit,
                                                                                                              field_id));
        }
        sal_memcpy(&temp_res_info[0], multi_res_info->results_info,
                   sizeof(dbal_table_field_info_t) * multi_res_info->nof_result_fields);
        SHR_FREE(multi_res_info->results_info);
        table->multi_res_info[result_type_idx].nof_result_fields -= 1;

        SHR_ALLOC_SET_ZERO(multi_res_info->results_info,
                           multi_res_info->nof_result_fields * sizeof(dbal_table_field_info_t),
                           "key fields info allocation", "%s%s%s\r\n", table->table_name, EMPTY, EMPTY);

        sal_memcpy(multi_res_info->results_info, &temp_res_info[0],
                   sizeof(dbal_table_field_info_t) * multi_res_info->nof_result_fields);
        ...
        for (hl_access_type = 0; hl_access_type < DBAL_NOF_HL_ACCESS_TYPES; hl_access_type++)
        {
            ...
            {
                for (jj = 0;
                     jj < table->hl_mapping_multi_res[res_type].l2p_hl_info[hl_access_type].num_of_access_fields; jj++)
                {
                    dbal_hl_l2p_field_info_t *curr_l2p_field =
                        &table->hl_mapping_multi_res[res_type].l2p_hl_info[hl_access_type].l2p_fields_info[jj];
                    if (curr_l2p_field->field_id == field_id)
                    {

                        SHR_ERR_EXIT(_SHR_E_INTERNAL, "value field NOT supported yet\n");
                    }
                }
            }
        }
    ...
    uint8 is_add_operation)
{
    ...
    if ((dbal_tables_is_non_direct(unit, entry_handle->table_id) == FALSE) &&
        ...
        (!dbal_table_is_in_lif(table) && !dbal_table_is_out_lif(table)))
    {
        SHR_EXIT();
    }
