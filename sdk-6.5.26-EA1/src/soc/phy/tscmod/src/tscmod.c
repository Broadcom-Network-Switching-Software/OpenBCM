...
_init_tscmod_st(int unit, soc_port_t port, tscmod_st *tsc)
{
   int i ;
      ...
      /* tsc->aer_bcst_ofs_strap = 0x1ff; */ /* good for TR3 */
      /*
   ...
   case 21000:
      if(tsc->port_type   == TSCMOD_SINGLE_PORT ) {
         ...
         if (DEV_CFG_PTR(pc)->line_intf & TSCMOD_IF_KR2) {
             spd_intf = TSCMOD_SPD_21G_HI_MLD_DXGXS;
             asp_id   = TSCMOD_ASPD_20G_KR2;
         } else if (DEV_CFG_PTR(pc)->line_intf & TSCMOD_IF_CR2) {
             spd_intf = TSCMOD_SPD_21G_HI_MLD_DXGXS;
             asp_id   = TSCMOD_ASPD_20G_CR2;
         } else {
...
_phy_tscmod_an_set(int unit, soc_port_t port, int an)
{
   ...
   if( an ) {
      ...
      */
      ...
      /* Disable remote fault reporting, because RF1 and RF2 are swapped */
      if(an_type == TSCMOD_CL37) {
          tsc->per_lane_control = TSCMOD_AN_SET_RF_DISABLE;
          tscmod_tier1_selector("AUTONEG_SET", tsc, &rv);
      }
...
_phy_tscmod_control_prbs_enable_set(tscmod_st *tsc, uint32 enable, int plane)
{
   ...
   */
   ...
   if(enable ||
      (!enable && ((tsc->spd_intf==TSCMOD_SPD_40G_MLD)||(tsc->spd_intf==TSCMOD_SPD_40G_X4) ||
       (tsc->spd_intf==TSCMOD_SPD_42G_MLD)||(tsc->spd_intf==TSCMOD_SPD_42G_X4)))) {
      if(tsc->port_type== TSCMOD_MULTI_PORT) {
