...
adapter_access_info_t *adapter_info[BCM_LOCAL_UNITS_MAX][DNX_DATA_MAX_DEVICE_GENERAL_NOF_DIES] = { {0} };
    ...
    int unit)
{
    if (SOC_IS_DNX(unit))
    {
        ...
        return dnx_data_device.general.nof_dies_get(unit);
    }
    else
    {
    ...
    uint32 *data)
{
    ...
    adapter_context_db.params.adapter_mem_reg_mutex.take(unit, sub_unit_index, sal_mutex_FOREVER);
...
exit:
    if (is_created == TRUE)
    {
        adapter_context_db.params.adapter_mem_reg_mutex.give(unit, sub_unit_index);
    }
    SHR_FUNC_EXIT;
}
    ...
    soc_reg_above_64_val_t data)
{
    ...
    for (block = 0; block < access_info.num_blks && rv == SOC_E_NONE; ++block)
    {
        ...
        rv = adapter_memreg_access
            (unit, 0, cmic_block, access_info.offset, reg_size, 0, is_get, is_clear_on_read, (uint32 *) data);
        if (is_get)
        {
            break;
        }
    ...
    void *entry_data)
{
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk)
    {
        ...
        rv = adapter_memreg_access(unit, 0, cmic_blk, addr, data_byte_len, 1, is_read, is_clear_on_read,
                                   (uint32 *) entry_data);
        SHR_IF_ERR_EXIT(rv);
    ...
    int sub_unit_id)
{
    ...
    if (SOC_IS_NEW_ACCESS_INITIALIZED(unit))
    {
        ...
        SHR_IF_ERR_EXIT(dnxc_adapter_regmem_access_handle(unit, 0, rECI_RESERVED_SPARE_0, sub_unit_id, 0, 0, &reg_val));
        if (reg_val != DNXC_ADAPTER_UINT32_ALL_ONES)
        {
            SHR_IF_ERR_EXIT(dnxc_adapter_regmem_access_handle
                            (unit, FLAG_ACCESS_IS_WRITE, rDMU_CRU_RESET, sub_unit_id, 0, 0, &reg_val));
        }
        reg_val = 0;
        SHR_IF_ERR_EXIT(dnxc_adapter_regmem_access_handle
                        (unit, FLAG_ACCESS_IS_WRITE, rECI_RESERVED_SPARE_0, sub_unit_id, 0, 0, &reg_val));
    }
    else
#endif
    {

        adapter_reg32_get(unit, ECI_RESERVED_SPARE_0r, REG_PORT_ANY, 0, &reg_val);
        if (reg_val != DNXC_ADAPTER_UINT32_ALL_ONES)
        {
            adapter_reg32_set(unit, DMU_CRU_RESETr, REG_PORT_ANY, 0, 1);
        }
        adapter_reg32_set(unit, ECI_RESERVED_SPARE_0r, REG_PORT_ANY, 0, 0);
    }
#ifdef BCM_ACCESS_SUPPORT
exit:
#endif
    ...
    int sub_unit_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(adapter_reg_access_reset_handle(unit, sub_unit_id));
    }
...
/* *INDENT-OFF* */
                ...
                SHR_IF_ERR_EXIT(dnx_algo_port_egr_if_get(unit, src_port, &egress_if));
                nif_or_cpu_port = egress_if - port_info;
            }
            else
            {
    ...
    uint32 *entry_data)
{
    ...
    *out_mem_index = in_mem_index;
    ...
    {
        special_swap_info = dnx_data_adapter.reg_mem_access.special_swap_core_table_get(unit, regmem);
        switch (special_swap_info->type)
        {
            ...
            case SPECIAL_SWAP_TYPE_ENTRY_DATA:
                SHR_IF_ERR_EXIT(dnxc_adapter_regmem_entry_data_swap(unit, regmem, special_swap_info->arg, entry_data));
                SHR_EXIT();
                break;
            default:
    ...
    uint32 *entry_data)
{
    ...
    access_runtime_info_t *runtime_info = SOC_CONTROL(unit)->device_access_runtime_info;        
    ...
    else
    {
        SHR_IF_ERR_EXIT(access_get_regmem_model_access_info
                        (unit, flags, regmem, swap_inst, array_index, swap_mem_index, &sbus_access_info));
    ...
    for (int operation_idx = 0; operation_idx < sbus_access_info.nof_operations; operation_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(adapter_memreg_access(unit,
                                              ...
                                              sbus_access_info.address, data_byte_len, is_mem, is_read, FALSE,
                                              (uint32 *) entry_data));
    ...
    access_regmem_sbus_info_t * sbus_access_info)
{
    ...
    for (block_idx = 0; block_idx < device_info->nof_block_instances; block_idx++)
    {
        if (device_info->block_instances[block_idx].sbus_info.sbus_block_id == in_block_id)
        {
            ...
            if ((is_pem == FALSE) &&
                (local_block->nof_instances != dnx_data_device.general.nof_cores_per_die_get(unit)))
            {
                SHR_EXIT();
            }
            
            for (int inst_idx = 0; inst_idx < local_block->nof_instances; inst_idx++)
            {
                if (device_info->block_instances[local_block->instances_start_index + inst_idx].sbus_info.sbus_block_id
                    == in_block_id)
                {
                    in_inst_idx = inst_idx;
                    break;
                }
            }
            break;
    ...
    if (in_inst_idx == -1)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "could not find the instance index of sbus_access_info.block_id=%d\n",
                     in_block_id);
    }
    if (is_pem == TRUE)
    {
    ...
    uint32 *entry_data)
{
    ...
    for (int operation_idx = 0; operation_idx < sbus_access_info.nof_operations; operation_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(adapter_memreg_access(unit,
                                              ACCESS_SBUS_BLOCK_GET_SUBDEV(sbus_access_info.block_ids[operation_idx]),
                                              ACCESS_SBUS_BLOCK_GET_INSTID(sbus_access_info.block_ids[operation_idx]),
                                              sbus_access_info.address, data_byte_len, is_mem,
