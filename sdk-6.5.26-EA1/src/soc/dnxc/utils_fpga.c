...
#include <soc/cm.h>
#include <soc/dnxc/utils_fpga.h>
#include <soc/sand/shrextend/shrextend_debug.h>

#include <sal/core/alloc.h>

#if (defined(BCM_PETRA_SUPPORT)) && (defined(LINUX) || defined(UNIX)) && (defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__))
...
static void
soc_dnx_fpga_altera_download_error_report(
    int error_code)
{
    ...
    if (error_code == 5)
    {
        LOG_INFO(BSL_LS_SOCDNX_COMMON, (BSL_META("     Error: no base address for download buffer\r\n")));
    }
...
int
soc_dnx_fpga_load_indication_run(
    int current_work_load)
{
    int err = 0;
    int percent = 0;
...
void
soc_dnx_fpga_load_indication_start(
    int total_work_load)
{
...
int
soc_dnx_fpga_altera_download(
    int unit,
    char *fpga_buffer_file,
    int file_size)
{
    ...
    LOG_VERBOSE(BSL_LS_SOCDNX_COMMON,
                (BSL_META_U(unit,
                            "[%s %d],%s: start: gpout_reg_value=0x%08x, gpin_reg_value=0x%08x\n"), __FILE__, __LINE__,
                 FUNCTION_NAME(), gpout_reg_value, gpin_reg_value));
    ...
    {
        ex = 10;
        LOG_ERROR(BSL_LS_SOCDNX_COMMON, (BSL_META_U(unit, "error [%s %d]:%s\n"), __FILE__, __LINE__, FUNCTION_NAME()));
        goto exit;
    }
    ...
    LOG_VERBOSE(BSL_LS_SOCDNX_COMMON,
                (BSL_META_U(unit,
                 ...
                 __LINE__, FUNCTION_NAME(), gpout_reg_value, gpin_reg_value));

    if (((gpin_reg_value & SOC_DNX_GPINDR_CONF_DONE_BIT) != 0)
        || ((gpin_reg_value & SOC_DNX_GPINDR_NSTATUS_BIT) == 0x0))
    {
        ex = 10;
        LOG_ERROR(BSL_LS_SOCDNX_COMMON, (BSL_META_U(unit, "error [%s %d]:%s\n"), __FILE__, __LINE__, FUNCTION_NAME()));
        LOG_ERROR(BSL_LS_SOCDNX_COMMON,
                  (BSL_META_U(unit,
        ...
        if ((gpin_reg_value & SOC_DNX_GPINDR_NSTATUS_BIT) == 0)
        {
            ...
            LOG_ERROR(BSL_LS_SOCDNX_COMMON,
                      (BSL_META_U(unit,
                       ...
                       __FILE__, __LINE__, FUNCTION_NAME(), i, SOC_DNX_GPINDR_NSTATUS_BIT, gpout_reg_value,
                       gpin_reg_value));
            goto exit;
        }
    ...
    LOG_INFO(BSL_LS_SOCDNX_COMMON, (BSL_META_U(unit, "\n")));
    ...
    int *p_size)
{
    ...
    if (p_buff == NULL)
    {
        LOG_ERROR(BSL_LS_SOCDNX_COMMON, (BSL_META("utils_fpga.c, soc_dnx_fpga_file2buff(): Buffer pointer is NULL\n")));
        ret = -1;
        goto exit;
    }
    ...
    char *file_name)
{
    ...
    LOG_ERROR(BSL_LS_SOCDNX_COMMON,
              (BSL_META_U(unit, "%s(): download and burn file: %s/%s\n"), FUNCTION_NAME(), the_path, file_name));

    if (file_name == NULL)
    {
        LOG_ERROR(BSL_LS_SOCDNX_COMMON, (BSL_META_U(unit, "%s(): file_name is NULL\n"), FUNCTION_NAME()));
        ret = -1;
        goto exit;
    }
    ...
    {
        LOG_ERROR(BSL_LS_SOCDNX_COMMON, (BSL_META_U(unit, "%s(): failde to allocate fpga buffer\n"), FUNCTION_NAME()));
        ret = -1;
        goto exit;
    }
