    ...
    int *count_p)
{
    ...
    switch (int_id)
    {
        ...
        case MACSEC_BLOCK_INT:
        {
            *count_p = COUNTOF(SOC_INFO(unit).macsec_blocks);
            break;
        }
        default:
        {
    ...
    int unit)
{
    ...
    return ((SOC_FLAGS_GET(unit) & SOC_F_ATTACHED) != 0);
}
...
shr_error_e
dnx_drv_soc_dnx_schan_deinit(
    int unit)
{
    shr_error_e ret;

    ret = soc_dnx_schan_deinit(unit);
    return (ret);
}
    ...
    int unit)
{
    ...
    if (SOC_IS_NEW_ACCESS_INITIALIZED(unit))
    {
        ret = access_dnx_sbus_broadcast_config(unit);
    }
    else
#endif
    {
