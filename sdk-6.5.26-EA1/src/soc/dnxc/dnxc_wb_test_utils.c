    ...
    int unit)
{
    ...
    {
#if defined (INCLUDE_CTEST) && defined(BCM_DNX_SUPPORT)
        warm_boot_exp_param = dnx_data_infra_tests.sw_state.warmboot_backoff_rate_get(unit);
#endif
    }
    ...
    int unit)
{
    ...
    {
#if defined (INCLUDE_CTEST) && defined(BCM_DNX_SUPPORT)
        nof_guaranteed_reboots = dnx_data_infra_tests.sw_state.nof_guaranteed_reboots_get(unit);
#endif
    }
    else if (soc_is(unit, DNXF_DEVICE))
    {
