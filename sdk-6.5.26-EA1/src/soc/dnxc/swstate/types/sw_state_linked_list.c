...
#define SW_STATE_LL_SET_NEXT(index, value) \
    if (IDX_IS_SERVICE_NODE(index)) { \
        DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, &(ll->service_nexts[TRANSLATE_LL_SERVICE_IDX(index)]), &value, sizeof(sw_state_ll_node_t), 0x0, "service nexts") \
    } else { \
...
#define SW_STATE_LL_SET_PREV(index, value) \
    if (IDX_IS_SERVICE_NODE(index)) { \
        DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, &(ll->service_prevs[TRANSLATE_LL_SERVICE_IDX(index)]), &value, sizeof(sw_state_ll_node_t), 0x0, "service prevs")\
    } else { \
...
#define SW_STATE_LL_SET_SERVICE_DOWN(index, value) \
    DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, &(ll->service_downs[TRANSLATE_LL_SERVICE_IDX(index)]), &value, sizeof(sw_state_ll_node_t), 0x0, "service down")
    ...
    *created_node = SW_STATE_LL_GET_HEAD(0, is_service_node);
    ...
    if(!is_service_node) {
        if(value == &sw_state_ll_head_defult_val) {
            DNX_SW_STATE_MEMSET(unit, node_id, VALUE_AT_IDX(*created_node), 0, 0, ll->value_size, 0x0, "sw state ll node's value")
        }
        else {
            DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, VALUE_AT_IDX(*created_node), value, ll->value_size, 0x0, "sw state ll node's value");
        }
        
        if (key != NULL) {
            if (key == &sw_state_ll_head_defult_val) {
                DNX_SW_STATE_MEMSET(unit, node_id, KEY_AT_IDX(*created_node), 0, 0, ll->key_size, 0x0, "sw state ll node's key")
            }
            else {
                DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, KEY_AT_IDX(*created_node), key, ll->key_size, 0x0, "sw state ll node's key");
            }
        }
    }
    
    else {
            DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, &(ll->service_pointers_keys[TRANSLATE_LL_SERVICE_IDX(*created_node)]), &key_pointer, sizeof(sw_state_ll_node_t), 0x0, "key_pointer")
    }
...
static void sw_state_ll_find_nodes_before_node(int unit, uint32 node_id, sw_state_ll_t ll, const void *key, sw_state_ll_node_t *update){
...
STATIC int sw_state_ll_add_internal(int unit, uint32 node_id, sw_state_ll_t ll, uint32 ll_index, sw_state_ll_node_t node_to_add_after, uint8 is_first, const void* key, const void* value) {
    ...
    if (ll->is_sorted) {
        sw_state_ll_find_nodes_before_node(unit, node_id, ll, key, nodes_to_add_after);
        ...
        sw_state_ll_create_new_node(unit, node_id, ll, ll_index, &created_temp_node, key, value, nodes_to_add_after[0], next, SW_STATE_LL_INVALID, SW_STATE_LL_INVALID, !SERVICE_NODE);
        next = created_temp_node;
        key_pointer = created_temp_node;

        for (i=1; i<level; i++){
            next = SW_STATE_LL_GET_NEXT(nodes_to_add_after[i]);
            sw_state_ll_create_new_node(unit, node_id, ll, ll_index, &created_node, key, value, nodes_to_add_after[i], next, created_temp_node, key_pointer, SERVICE_NODE);
...
int sw_state_ll_create_empty(int unit, uint32 node_id, sw_state_ll_init_info_t *init_info, uint32 flags, uint32 key_size,
    ...
    if (sw_state_is_flag_on(flags, SW_STATE_LL_SORTED))
    {
        ...
        DNX_SW_STATE_ALLOC(unit, node_id, ll->service_prevs, sw_state_ll_node_t, elements_to_alloc, el_alloc_flags, "ll service prevs");
        DNX_SW_STATE_ALLOC(unit, node_id, ll->service_nexts, sw_state_ll_node_t, elements_to_alloc, el_alloc_flags, "ll service nexts");
        DNX_SW_STATE_ALLOC(unit, node_id, ll->service_pointers_keys, sw_state_ll_node_t, elements_to_alloc, el_alloc_flags, "ll service pointers keys");
    ...
    if (ll->is_sorted) {
        ...
        sw_state_ll_create_new_node(unit, node_id, ll, 1, &created_temp_node, &sw_state_ll_head_defult_val, &sw_state_ll_head_defult_val, sw_state_ll_invalid, sw_state_ll_invalid, sw_state_ll_invalid, key_pointer, SERVICE_NODE);
        SW_STATE_LL_SET_HEAD(1, created_temp_node);
        for (i = 0; i < ll->max_level - 2 ; i++) {
            sw_state_ll_create_new_node(unit, node_id, ll, 1, &created_node, &sw_state_ll_head_defult_val, &sw_state_ll_head_defult_val, sw_state_ll_invalid, sw_state_ll_invalid, sw_state_ll_invalid, key_pointer, SERVICE_NODE);
            SW_STATE_LL_SET_SERVICE_DOWN(created_temp_node, created_node);
            created_temp_node = created_node;
        }
        sw_state_ll_create_new_node(unit, node_id, ll, 1, &created_node, &sw_state_ll_head_defult_val, &sw_state_ll_head_defult_val, sw_state_ll_invalid, sw_state_ll_invalid, sw_state_ll_invalid, sw_state_ll_invalid, !SERVICE_NODE);
        SW_STATE_LL_SET_SERVICE_DOWN(created_temp_node, created_node);
...
static int sw_state_ll_sorted_ll_remove_node(int unit, uint32 node_id, sw_state_ll_t ll, uint32 ll_index, sw_state_ll_node_t node) {
    ...
    sw_state_ll_find_nodes_before_node(unit, node_id, ll, KEY_AT_IDX(node), prevs);
    ...
    for(i=0; i<nodes_to_remove_num; i++){
        ...
        if (i==0) {
            DNX_SW_STATE_MEMSET(unit, node_id, KEY_AT_IDX(nodes_to_remove[i]), 0, -1, ll->key_size, 0x0, "sw state ll node's key");
            DNX_SW_STATE_MEMSET(unit, node_id, VALUE_AT_IDX(nodes_to_remove[i]), 0, -1, ll->value_size, 0x0, "sw state ll node's value");
        }
...
int sw_state_ll_remove_node(int unit, uint32 node_id, sw_state_ll_t ll, uint32 ll_index, sw_state_ll_node_t node) {
    ...
    if (ll->is_sorted) {
        ...
        DNX_SW_STATE_MEMSET(unit, node_id, VALUE_AT_IDX(node), 0, -1, ll->value_size, 0x0, "sw state ll node's value");
    }
...
int sw_state_ll_node_update(int unit, uint32 node_id, sw_state_ll_t ll, sw_state_ll_node_t node, const void *value) {
    ...
    DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, VALUE_AT_IDX(node), value, ll->value_size, 0x0, "sw state ll node's value");
...
int sw_state_ll_find(int unit, uint32 node_id, sw_state_ll_t ll, uint32 ll_index, sw_state_ll_node_t * node, const void *key, uint8* found) {
    ...
    sw_state_ll_find_nodes_before_node(unit, node_id, ll, key, prevs);
