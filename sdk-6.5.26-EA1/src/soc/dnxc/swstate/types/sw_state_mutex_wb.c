...
STATIC int dnxc_sw_state_mutex_create_wb_internal_verify(int unit, uint32 node_id, sw_state_mutex_t *ptr_mtx, char *desc)
{
...
int dnxc_sw_state_mutex_create_wb(int unit, uint32 node_id, sw_state_mutex_t *ptr_mtx, char *desc, uint32 flags) {
    ...
    DNXC_VERIFY_INVOKE(SHR_IF_ERR_EXIT(dnxc_sw_state_mutex_create_wb_internal_verify(unit, node_id, ptr_mtx, desc)));
    ...
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_mutex_create(unit, node_id, ptr_mtx));
    }
#endif 
...
int dnxc_sw_state_mutex_destroy_wb(int unit, uint32 node_id, sw_state_mutex_t *ptr_mtx, uint32 flags)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(dnxc_sw_state_free_wb(unit, node_id, (uint8**)ptr_mtx, flags,"sw_state_mutex_destroy"),
        "unit:%d sw_state mutex destroy failed\n%s%s", unit, EMPTY, EMPTY);
    
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_mutex_destroy(unit, node_id, ptr_mtx));
    }
#endif 
...
int dnxc_sw_state_mutex_take_wb(int unit, uint32 node_id, sw_state_mutex_t *ptr_mtx, int usec, uint32 flags)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_mutex_take(unit, node_id, ptr_mtx, usec));
    }
#endif 
...
int dnxc_sw_state_mutex_give_wb(int unit, uint32 node_id, sw_state_mutex_t *ptr_mtx, uint32 flags)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnxc_sw_state_journal_mutex_give(unit, node_id, ptr_mtx));
    }
#endif 
