    ...
    soc_port_t * links)
{
    ...
    for (blk_id = 0; blk_id < dnxf_data_device.blocks.nof_instances_dch_get(unit); blk_id++)
    {
        SHR_IF_ERR_EXIT(soc_reg32_get
                        (unit, LCM_LINK_TH_PROFILES_MAPPINGr, blk_id, 0,
                         &regs_values_link_th_profiles_mapping[blk_id]));
        SHR_IF_ERR_EXIT(soc_reg32_get
                        (unit, LCM_DTL_TH_PROFILES_MAPPINGr, blk_id, 0, &regs_values_dtl_th_profiles_mapping[blk_id]));
        SHR_IF_ERR_EXIT(soc_reg32_get(unit, LCM_DTL_LINK_TYPEr, blk_id, 0, &regs_values_dtl_link_type[blk_id]));
        SHR_IF_ERR_EXIT(soc_reg32_get
                        (unit, LCM_DTL_LCLRT_LINK_TYPEr, blk_id, 0, &regs_values_dtl_lclrt_link_type[blk_id]));
        SHR_IF_ERR_EXIT(soc_reg32_get
                        (unit, DCH_LINK_LEVEL_FLOW_CONTROL_BMPr, blk_id, 0, &regs_values_llfc_bmp[blk_id]));
    }
    ...
    for (ii = 0; ii < links_count; ii++)
    {
        ...
        SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                        (unit, mbcm_dnxf_drv_link_to_block_mapping, (unit, link, &blk_id, &inner_link, SOC_BLK_DCH)));
    ...
    for (blk_id = 0; blk_id < dnxf_data_device.blocks.nof_instances_dch_get(unit); blk_id++)
    {
        SHR_IF_ERR_EXIT(soc_reg32_set
                        (unit, LCM_LINK_TH_PROFILES_MAPPINGr, blk_id, 0, regs_values_link_th_profiles_mapping[blk_id]));
        SHR_IF_ERR_EXIT(soc_reg32_set
                        (unit, LCM_DTL_TH_PROFILES_MAPPINGr, blk_id, 0, regs_values_dtl_th_profiles_mapping[blk_id]));
