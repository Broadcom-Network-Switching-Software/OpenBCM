    ...
    int unit)
{
    ...
    for (port_i = 0; port_i < dnxf_data_port.general.nof_links_get(unit); ++port_i)
    {
        ...
        {
            if (dnxf_data_port.pll.info_get(unit, lcpll)->fabric_ref_clk_out == DNXC_SERDES_REF_CLOCK_BYPASS)
            {
                SOC_INFO(unit).port_refclk_int[port_i] = dnxf_data_port.pll.info_get(unit, lcpll)->fabric_ref_clk_in;
            }
            else
            {
                SOC_INFO(unit).port_refclk_int[port_i] = dnxf_data_port.pll.info_get(unit, lcpll)->fabric_ref_clk_out;
            }
        }
        else
        {
            ...
            switch (lcpll)
            {
                ...
                case 1:
                    ...
                    SHR_BITCOPY_RANGE(&puc_lcpll_out_sel, 0, puc, SOC_RAMON_PORT_PUC_LCPLL01_OUT_SEL,
                                      SOC_RAMON_PORT_PUC_LCPLL01_OUT_SEL_NOF_BITS);
                ...
                case 3:
                    ...
                    SHR_BITCOPY_RANGE(&puc_lcpll_out_sel, 0, puc, SOC_RAMON_PORT_PUC_LCPLL23_OUT_SEL,
                                      SOC_RAMON_PORT_PUC_LCPLL23_OUT_SEL_NOF_BITS);
            ...
            {
                SOC_INFO(unit).port_refclk_int[port_i] = DNXC_SERDES_REF_CLOCK_312_5;
            }
            else if (puc_lcpll_out_sel == SOC_RAMON_PORT_PUC_LCPLL_OUT_156_25_MHZ)
            {
                SOC_INFO(unit).port_refclk_int[port_i] = DNXC_SERDES_REF_CLOCK_156_25;
            }
            else if (puc_lcpll_out_sel == SOC_RAMON_PORT_PUC_LCPLL_OUT_125_MHZ)
            {
                SOC_INFO(unit).port_refclk_int[port_i] = DNXC_SERDES_REF_CLOCK_125;
            }
            else
            {
    ...
    int port)
{
    ...
    SHR_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg_val2));

    if (is_master)
    {

        soc_reg_field_set(unit, ECI_SELECT_OUTPUT_OF_SYNCHRONOUS_ETHERNET_PADSr, &reg_val, FABRIC_MASTER_SYNC_E_SELf,
                          fsrd_block);

        soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val2, MASTER_CLK_SELf,
                            port % dnxf_data_device.blocks.nof_links_in_fsrd_get(unit));

    }
    else
    {

        soc_reg_field_set(unit, ECI_SELECT_OUTPUT_OF_SYNCHRONOUS_ETHERNET_PADSr, &reg_val, FABRIC_SLAVE_SYNC_E_SELf,
                          fsrd_block);

        soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val2, SLAVE_CLK_SELf,
                            port % dnxf_data_device.blocks.nof_links_in_fsrd_get(unit));
    }
    SHR_IF_ERR_EXIT(WRITE_ECI_SELECT_OUTPUT_OF_SYNCHRONOUS_ETHERNET_PADSr(unit, reg_val));
    SHR_IF_ERR_EXIT(soc_reg_set(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, reg_val2));
    ...
    int *port)
{
    ...
    {
        fsrd_block =
            soc_reg_field_get(unit, ECI_SELECT_OUTPUT_OF_SYNCHRONOUS_ETHERNET_PADSr, reg_val,
                              FABRIC_MASTER_SYNC_E_SELf);
        SHR_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg_val2));
        inner_fsrd_port = soc_reg64_field_get(unit, FSRD_SYNC_E_SELECTr, reg_val2, MASTER_CLK_SELf);
    }
    else
    {
        fsrd_block =
            soc_reg_field_get(unit, ECI_SELECT_OUTPUT_OF_SYNCHRONOUS_ETHERNET_PADSr, reg_val, FABRIC_SLAVE_SYNC_E_SELf);
        SHR_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg_val2));
        inner_fsrd_port = soc_reg64_field_get(unit, FSRD_SYNC_E_SELECTr, reg_val2, SLAVE_CLK_SELf);
    }
    ...
    int *divider)
{
    ...
    SHR_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg_val2));
    *divider = soc_reg64_field_get(unit, FSRD_SYNC_E_SELECTr, reg_val2, SYNC_E_CLK_DIV_PHASE_1f) + 1;
    ...
    int divider)
{
    ...
    if (is_enabled)
    {
        SHR_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg_val));
        soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val, SYNC_E_CLK_DIV_PHASE_1f, phase1_divider);
        soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val, SYNC_E_CLK_DIV_PHASE_0f, phase1_divider / 2);
        SHR_IF_ERR_EXIT(soc_reg_set(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, reg_val));
...
STATIC shr_error_e
soc_ramon_blocks_enable_get(
    int unit,
    int_in_soc_info_e block_info,
    int block,
    int *enable)
{
    int blk;
    ...
    SHR_IF_ERR_EXIT(soc_info_int_address_get(unit, block_info, &block_p));
    block_p = &(block_p[block]);
    blk = *block_p;
    ...
    int unit)
{
    ...
    int fsrd_sbus_chain[DNXF_DATA_MAX_DEVICE_BLOCKS_NOF_INSTANCES_FSRD];
    int fmac_sbus_chain[DNXF_DATA_MAX_DEVICE_BLOCKS_NOF_INSTANCES_FMAC];
    int enable = 0;
    uint32 nof_fsrds = dnxf_data_device.blocks.nof_instances_fsrd_get(unit);
    uint32 nof_fmacs = dnxf_data_device.blocks.nof_instances_fmac_get(unit);
    uint32 fsrds = 0;
    uint32 fmacs = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        index_in_chain = dnxf_data_device.blocks.fsrd_sbus_chain_get(unit, fsrd)->index;

        SHR_IF_ERR_EXIT(soc_ramon_blocks_enable_get(unit, FSRD_BLOCK_INT, fsrd, &enable));
        if (enable)
        {
            SHR_IF_ERR_EXIT(WRITE_FSRD_SBUS_LAST_IN_CHAINr(unit, fsrd, 0));

            fsrd_sbus_chain[index_in_chain] = fsrd;
        }
        ++fsrds;
    }

    for (blk = fsrds - 1; blk >= 0; --blk)
    {
        if (fsrd_sbus_chain[blk] != -1)
        {
            fsrd_last_in_chain = fsrd_sbus_chain[blk];
            break;
        }
    }
    ...
    {
        index_in_chain = dnxf_data_device.blocks.fmac_sbus_chain_get(unit, fmac)->index;
        SHR_IF_ERR_EXIT(soc_ramon_blocks_enable_get(unit, FMAC_BLOCK_INT, fmac, &enable));
        if (enable)
        {
            SHR_IF_ERR_EXIT(WRITE_FMAC_SBUS_LAST_IN_CHAINr(unit, fmac, 0));

            fmac_sbus_chain[index_in_chain] = fmac;
        }
        ++fmacs;
    }

    for (blk = fmacs - 1; blk >= 0; --blk)
    {
        if (fmac_sbus_chain[blk] != -1)
        {
            fmac_last_in_chain = fmac_sbus_chain[blk];
            break;
        }
    }
    ...
    int is_init_sequence)
{
    ...
    {
        rc = soc_ramon_lane_map_db_is_lane_mapped_get(unit, port, &is_lane_mapped);
        SHR_IF_ERR_EXIT(rc);
        if (!is_lane_mapped)
        {
    ...
    SOC_PBMP_ITER(pbmp, port)
    {
        ...
        SHR_IF_ERR_EXIT(soc_ramon_lane_map_db_pm_map_get
                        (unit, pm_index, DNXC_PORT_FABRIC_LANES_PER_CORE, port_config.lane2serdes));
    ...
    SOC_PBMP_ITER(pbmp, port)
    {
        ...
        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                                   (unit, port, SOC_DNXF_PBMP_PORT_ADD, soc_dnxf_port_update_type_sfi));
        SHR_IF_ERR_EXIT(rc);

        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                                   (unit, port, SOC_DNXF_PBMP_PORT_ADD, soc_dnxf_port_update_type_port));
        SHR_IF_ERR_EXIT(rc);

        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                                   (unit, port, SOC_DNXF_PBMP_PORT_ADD, soc_dnxf_port_update_type_all));
        SHR_IF_ERR_EXIT(rc);

        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                                   (unit, port, SOC_DNXF_PBMP_PORT_REMOVE, soc_dnxf_port_update_type_sfi_disabled));
        SHR_IF_ERR_EXIT(rc);

        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                                   (unit, port, SOC_DNXF_PBMP_PORT_REMOVE, soc_dnxf_port_update_type_port_disabled));
        SHR_IF_ERR_EXIT(rc);

        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                                   (unit, port, SOC_DNXF_PBMP_PORT_REMOVE, soc_dnxf_port_update_type_all_disabled));
        SHR_IF_ERR_EXIT(rc);
    }
...
shr_error_e
soc_ramon_port_fsrd_block_power_set(
    int unit,
    soc_port_t port,
    int power_up)
{
    ...
    uint32 field_val;
    uint64 reg_val64;
    const soc_field_t fields[] =
        { BLOCKS_POWER_DOWN_0f, BLOCKS_POWER_DOWN_1f, BLOCKS_POWER_DOWN_2f, BLOCKS_POWER_DOWN_3f, BLOCKS_POWER_DOWN_4f,
        BLOCKS_POWER_DOWN_5f,
        BLOCKS_POWER_DOWN_6f, BLOCKS_POWER_DOWN_7f, BLOCKS_POWER_DOWN_8f, BLOCKS_POWER_DOWN_9f, BLOCKS_POWER_DOWN_10f,
        BLOCKS_POWER_DOWN_11f,
        BLOCKS_POWER_DOWN_12f, BLOCKS_POWER_DOWN_13f, BLOCKS_POWER_DOWN_14f, BLOCKS_POWER_DOWN_15f,
        BLOCKS_POWER_DOWN_16f, BLOCKS_POWER_DOWN_17f,
        BLOCKS_POWER_DOWN_18f, BLOCKS_POWER_DOWN_19f, BLOCKS_POWER_DOWN_20f, BLOCKS_POWER_DOWN_21f,
        BLOCKS_POWER_DOWN_22f, BLOCKS_POWER_DOWN_23f
    };
    SHR_FUNC_INIT_VARS(unit);
    ...
    for (fmac_block_idx = fmac_block; fmac_block_idx < fmac_block + dnxf_data_device.blocks.nof_fmacs_in_fsrd_get(unit);
         fmac_block_idx++)
    {
...
shr_error_e
soc_ramon_port_first_link_in_fsrd_get(
    int unit,
    soc_port_t port,
    int *is_first_link)
{
    ...
    for (i = fsrd_block * dnxf_data_device.blocks.nof_links_in_fsrd_get(unit);
         i <
         fsrd_block * dnxf_data_device.blocks.nof_links_in_fsrd_get(unit) +
         dnxf_data_device.blocks.nof_links_in_fsrd_get(unit); i++)
    {
    ...
    int unit)
{
    ...
    BCM_PBMP_ITER(dnxf_data_port.general.supported_lanes_get(unit)->pbmp, port)
    {
    ...
    int unit)
{
    ...
    portmod_pm_instances_t pm_types_and_instances[] =
        { {portmodDispatchTypePm8x50_fabric, dnxf_data_port.general.nof_pms_get(unit)} };
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        SHR_IF_ERR_EXIT(soc_ramon_lane_map_db_map_set(unit, nof_links, lane2serdes, 1));
    }
    ...
    soc_port_t port)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                               (unit, port, SOC_DNXF_PBMP_PORT_REMOVE, soc_dnxf_port_update_type_sfi));
    SHR_IF_ERR_EXIT(rc);

    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                               (unit, port, SOC_DNXF_PBMP_PORT_REMOVE, soc_dnxf_port_update_type_port));
    SHR_IF_ERR_EXIT(rc);

    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                               (unit, port, SOC_DNXF_PBMP_PORT_REMOVE, soc_dnxf_port_update_type_all));
    SHR_IF_ERR_EXIT(rc);

    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                               (unit, port, SOC_DNXF_PBMP_PORT_ADD, soc_dnxf_port_update_type_sfi_disabled));
    SHR_IF_ERR_EXIT(rc);

    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                               (unit, port, SOC_DNXF_PBMP_PORT_ADD, soc_dnxf_port_update_type_port_disabled));
    SHR_IF_ERR_EXIT(rc);

    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_dynamic_port_update,
                               (unit, port, SOC_DNXF_PBMP_PORT_ADD, soc_dnxf_port_update_type_all_disabled));
    SHR_IF_ERR_EXIT(rc);
    ...
    portmod_loopback_mode_t loopback)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &fmac_block, &fmac_inner_link, SOC_BLK_FMAC)));

    SHR_IF_ERR_EXIT(soc_reg32_get
                    (unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, fmac_block, fmac_inner_link, &reg32_val));
    if (loopback == portmodLoopbackMacOuter || loopback == portmodLoopbackMacAsyncFifo)
    {
        soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_TX_PERIODf,
                          SOC_RAMON_PORT_COMMA_BURST_PERIOD_MAC_LOOPBACK);
        soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_BRST_SIZEf,
                          SOC_RAMON_PORT_COMMA_BURST_SIZE_MAC_LOOPBACK);
    }
    else
    {
        SHR_IF_ERR_EXIT(soc_dnxf_fabric_link_device_stage_get(unit, port, 0, &link_mode));

        soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_TX_BYTE_MODEf, 0x1);
        switch (link_mode)
        {
            case soc_dnxf_fabric_device_stage_fe1:
                soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_TX_PERIODf,
                                  SOC_RAMON_PORT_COMMA_BURST_PERIOD_FE1);
                soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_BRST_SIZEf,
                                  SOC_RAMON_PORT_COMMA_BURST_SIZE_FE1);
                break;
            case soc_dnxf_fabric_device_stage_fe3:
                soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_TX_PERIODf,
                                  SOC_RAMON_PORT_COMMA_BURST_PERIOD_FE3);
                soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_BRST_SIZEf,
                                  SOC_RAMON_PORT_COMMA_BURST_SIZE_FE3);
                break;
            case soc_dnxf_fabric_device_stage_fe2:
                soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_TX_PERIODf,
                                  SOC_RAMON_PORT_COMMA_BURST_PERIOD_FE2);
                soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_BRST_SIZEf,
                                  SOC_RAMON_PORT_COMMA_BURST_SIZE_FE2);
                break;
            case soc_dnxf_fabric_device_stage_repeater:
                soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_TX_PERIODf,
                                  SOC_RAMON_PORT_COMMA_BURST_PERIOD_FE3);
                soc_reg_field_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, &reg32_val, FMAL_N_CM_BRST_SIZEf,
                                  SOC_RAMON_PORT_COMMA_BURST_SIZE_FE3);
    ...
    soc_pbmp_t disable_pbmp)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        SHR_IF_ERR_EXIT(soc_ramon_brdc_blk_id_set(unit));
    }
...
shr_error_e
soc_ramon_port_update_valid_block_database(
    int unit,
    int block_type,
    int block_number,
    int enable)
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping, (unit, port, &dcml_block, &inner_link, SOC_BLK_DCML)));
    ...
    if (SOC_FLAGS_GET(unit) & SOC_F_ALL_MODULES_INITED)
    {
        SHR_IF_ERR_EXIT(soc_ramon_fabric_links_aldwp_config(unit, port));
    }

    SHR_IF_ERR_EXIT(soc_ramon_port_pump_enable_set(unit, port, enable));
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_port_disable_and_save_enable_state(unit, port, 0, &port_original_state));

    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping, (unit, port, &blk, &inner_link, SOC_BLK_FMAC)));

    SHR_IF_ERR_EXIT(READ_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr(unit, blk, inner_link, &reg64_val));
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_GENERAL_CONFIGURATIONr, &reg64_val, FMAL_N_TX_PUMP_WHEN_LB_DNf,
                          enable ? 1 : 0);
    SHR_IF_ERR_EXIT(WRITE_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr(unit, blk, inner_link, reg64_val));

    SHR_IF_ERR_EXIT(soc_dnxf_port_restore_enable_state(unit, port, 0, port_original_state));
...
shr_error_e
soc_ramon_port_dynamic_soc_init(
    int unit,
    soc_port_t port,
    int is_first_link)
{
    ...
    if (is_first_link)
    {
        for (blk = fmac_first_block; blk < fmac_first_block + dnxf_data_device.blocks.nof_fmacs_in_fsrd_get(unit);
             blk++)
        {
            SHR_IF_ERR_EXIT(READ_FMAC_LEAKY_BUCKET_CONTROL_REGISTERr(unit, blk, &reg32_val));
            soc_reg_field_set(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, &reg32_val, BKT_FILL_RATE_Nf,
    ...
    SHR_IF_ERR_EXIT(soc_reg_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, fmac_block, 0, &reg64_val));
    *field = soc_reg64_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg64_val, LNK_LVL_FC_TX_ENf);
    SHR_BITSET(field, fmac_inner_link);
    soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg64_val, LNK_LVL_FC_TX_ENf, *field);
    *field = soc_reg64_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg64_val, LNK_LVL_FC_RX_ENf);
    SHR_BITSET(field, fmac_inner_link);
    soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg64_val, LNK_LVL_FC_RX_ENf, *field);
    SHR_IF_ERR_EXIT(soc_reg_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, fmac_block, 0, reg64_val));
    ...
    soc_dnxf_port_update_type_t type)
{
    ...
    if (!(PBMP_MEMBER(dnxf_data_port.general.supported_lanes_get(unit)->pbmp, port)))
    {
