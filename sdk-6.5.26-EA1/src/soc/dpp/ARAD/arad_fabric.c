  ...
  )
{
  ...
  for (link_ndx = 0; link_ndx < SOC_DPP_DEFS_GET(unit, nof_fabric_links); ++link_ndx)
  {
    ...
    if (tx_enable)
    {
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1000, exit, soc_reg_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,fmac_index,0,&reg_val));
      tx_field = soc_reg64_field_get(unit,FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,reg_val,LNK_LVL_FC_TX_ENf);
      SOC_SAND_SET_BIT(tx_field,fld_val,inner_link);
      soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg_val,LNK_LVL_FC_TX_ENf, tx_field);
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1010, exit, soc_reg_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,fmac_index,0,reg_val));
    }

    if (rx_enable)
    {
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1020, exit, soc_reg_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,fmac_index,0,&reg_val));
      rx_field = soc_reg64_field_get(unit,FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,reg_val,LNK_LVL_FC_RX_ENf);
      SOC_SAND_SET_BIT(rx_field,fld_val,inner_link);
      soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg_val,LNK_LVL_FC_RX_ENf, rx_field);
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1030, exit, soc_reg_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,fmac_index,0,reg_val));
    }
  ...
  )
{
  ...
  for (link_idx = 0; link_idx < SOC_DPP_DEFS_GET(unit, nof_fabric_links); ++link_idx)
  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1130, exit, soc_reg_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,fmac_index,0,&reg_val));
    *rx_field = soc_reg64_field_get(unit,FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,reg_val,LNK_LVL_FC_RX_ENf);
    *tx_field = soc_reg64_field_get(unit,FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr,reg_val,LNK_LVL_FC_TX_ENf);
...
arad_fabric_links_llf_control_source_get(int unit, soc_port_t link, soc_dcmn_fabric_control_source_t* val) 
{
  ...
  SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, blk_id, 0, &reg64_val));
  *field_val_rx = soc_reg64_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg64_val, LNK_LVL_FC_RX_ENf);
  *field_val_tx = soc_reg64_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg64_val, LNK_LVL_FC_TX_ENf);
