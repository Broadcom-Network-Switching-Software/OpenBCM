...
static int
    arad_egr_prge_mgmt_fast_copy_shadow(
       SOC_SAND_IN    int                             unit,
       lfem_maps_shadow_t lfem_maps_src,
       lfem_maps_shadow_t lfem_maps_dst)
{
    uint32 rv = SOC_SAND_OK;
    uint8 lfem_table_ndx;

    SOCDNX_INIT_FUNC_DEFS;

    for (lfem_table_ndx = 0; lfem_table_ndx < ARAD_EGR_PRGE_MAX_NOF_LFEM_TABLES; lfem_table_ndx++) {
        sal_memcpy(lfem_maps_dst[lfem_table_ndx], lfem_maps_src[lfem_table_ndx], ARAD_EGR_PRGE_MAX_NOF_LFEMS * sizeof(lfem_maps_src[0][0]));
    }
    SOCDNX_IF_ERR_EXIT(rv);
exit:
    SOCDNX_FUNC_RETURN;
  ...
  )
{
    ...
    rv = arad_egr_prge_mgmt_fast_copy_shadow(unit, lfem_maps_shadow, lfem_maps_shadow_dummy);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    for (permutation = 0; permutation < all_program_permutations[current_program].valid_permutation_number; permutation++) {
        ...
        if (rv != SOC_SAND_NO_ERR) {
            ...
            if (rv != SOC_SAND_NO_ERR) {
                ...
                rv = arad_egr_prge_mgmt_fast_copy_shadow(unit, lfem_maps_shadow_dummy, lfem_maps_shadow);
                SOCDNX_IF_ERR_EXIT(rv);
