...
uint32
   arad_pp_flp_prog_sel_cam_pppoe_ipv6mc(
     int unit,
     int32 *prog_id
   )
{
    ...
    res = arad_pp_flp_set_app_id_and_get_cam_sel(unit, PROG_FLP_PPPOE_IPV6MC, FALSE, TRUE, &cam_sel_id, prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_prog_sel_cam_pppoe_ipv6mc", 0, 0);
}
...
uint32
   arad_pp_flp_process_pppoe_ipv6mc(
     int unit,
     int prog_id
   )
{
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_process_pppoe_ipv6mc", 0, 0);
}
...
uint32
   arad_pp_flp_prog_sel_cam_pppoe_ipv4mc(
     int unit,
     int32 *prog_id
   )
{
    ...
    res = arad_pp_flp_set_app_id_and_get_cam_sel(unit, PROG_FLP_PPPOE_IPV4COMPMC_WITH_RPF, FALSE, TRUE, &cam_sel_id, prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_prog_sel_cam_pppoe_ipv4compmc_with_rpf", 0, 0);
}

uint32
   arad_pp_flp_process_pppoe_ipv4mc(
     int unit,
     int prog_id
   )
{
   ...
   )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.ipmc_ivl || SOC_DPP_L3_IPV6_COMPRESSED_SIP_ENABLE(unit)) {
      prog_selection_cam_tbl.lsb_rif_profile_bit_mask = 0x1; 
  }
...
STATIC uint32 arad_pp_flp_program_get_namespace(int unit, uint8 app_id, int *name_space_idx)
{
    ...
    switch (app_id) {
        ...
        case PROG_FLP_PPPOE_IPV4COMPMC_WITH_RPF:
   ...
   )
{
  ...
  if (SOC_DPP_PPPOE_IS_ENABLE(unit)) {
      if (SOC_DPP_L3_SRC_BIND_IPV4_ENABLE(unit)) {
          ...
          res = arad_pp_flp_prog_sel_cam_pppoe_ipv4mc(unit, &prog_id);
          SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
          res = arad_pp_flp_dbal_pppoe_ipv4mc_program_tables_init(unit, prog_id);
          SOC_SAND_CHECK_FUNC_RESULT(res, 44, exit);
          res = arad_pp_flp_process_pppoe_ipv4mc(unit, prog_id);
          SOC_SAND_CHECK_FUNC_RESULT(res, 45, exit);
      }
      ...
      if (SOC_DPP_L3_SRC_BIND_IPV6_ENABLE(unit)) {
          ...
          res = arad_pp_flp_prog_sel_cam_pppoe_ipv6mc(unit, &prog_id);
          SOC_SAND_CHECK_FUNC_RESULT(res, 71, exit);
          res = arad_pp_flp_dbal_pppoe_ipv6mc_program_tables_init(unit, prog_id);
          SOC_SAND_CHECK_FUNC_RESULT(res, 72, exit);
          res = arad_pp_flp_process_pppoe_ipv6mc(unit, prog_id);
  ...
  if(is_routing_enabled)
  {
...
#endif
     {
      ...
      if(JER_KAPS_ENABLE(unit)
#if defined(INCLUDE_KBP)
        || (ARAD_KBP_ENABLE_IPV4_RPF && SOC_IS_JERICHO(unit))
#endif
   ) {
      ...
      if(JER_KAPS_ENABLE(unit)
#if defined(INCLUDE_KBP)
        || (ARAD_KBP_ENABLE_IPV4_UC && SOC_IS_JERICHO(unit))
#endif
     ){
    ...
    if (SOC_IS_ARADPLUS_A0(unit) && SOC_DPP_CONFIG(unit)->pp.oam_statistics) {
      ...
      if (SOC_DPP_CONFIG(unit)->l3.ipmc_vpn_lookup_enable && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mim_spb_enable", 0)){
          ...
          if(JER_KAPS_ENABLE(unit)
#if defined(INCLUDE_KBP)
            || (ARAD_KBP_ENABLE_IPV4_MC && SOC_IS_JERICHO(unit))
#endif
            ){
   ...
   )
{
    ...
    flp_key_construction_tbl.instruction_1_16b        = (SOC_IS_JERICHO(unit)? ARAD_PP_FLP_16B_INST_N_ZEROS(1): ARAD_PP_FLP_16B_INST_N_ZEROS(4)); 
    flp_key_construction_tbl.instruction_4_32b        = ARAD_PP_FLP_32B_INST_N_ZEROS(32); 
    flp_key_construction_tbl.instruction_5_32b        = rif_ce_inst; 
    flp_key_construction_tbl.key_a_inst_0_to_5_valid    = 0x33 ;
    flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 ;
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 ;
...
)
{
    ...
    case PROG_FLP_PPPOE_IPV4COMPMC_WITH_RPF:
       return "PROG_FLP_PPPOE_IPV4COMPMC_WITH_RPF";
    case PROG_FLP_PPPOE_IPV6MC:
       return "PROG_FLP_PPPOE_IPV6MC";
    default:
