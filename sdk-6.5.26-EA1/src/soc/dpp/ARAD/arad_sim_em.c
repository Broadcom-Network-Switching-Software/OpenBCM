             ...
             )
{
  ...
  for (blk_ndx = 0; blk_ndx < block_cnt; blk_ndx++) {
      ...
      if (table_type == ARAD_EM_TABLE_LEM_CR) {
          continue;
      }
                   ...
                   int                  is_lem_shadow_supported)
{
  ...
  case SOC_SAND_DEV_ARAD:
    ...
    if (!SOC_WARM_BOOT(unit)) {
        rv = chip_sim_em_init_chip_specifics_arad(unit, is_lem_shadow_supported);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 10, exit);
        rv = chip_sim_em_malloc_arad(unit, block_cnt, is_lem_shadow_supported);
