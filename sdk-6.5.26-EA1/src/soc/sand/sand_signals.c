    ...
    int show_value)
{
    ...
    {
        char field_enum_str[DBAL_MAX_STRING_LENGTH];
        res = dnx_dbal_fields_string_form_hw_value_get(unit, cur_sigparam->dbal_n, value, field_enum_str);
        if(res != _SHR_E_NONE)
        {
        ...
        {
            sal_snprintf(value_n, (2 * DBAL_MAX_STRING_LENGTH) + 1, "%s_%s", cur_sigparam->dbal_n, field_enum_str);
            if (show_value == 1)
            {
    ...
    char to_n[RHNAME_MAX_SIZE])
{
    ...
    sal_strncat_s(context_parse_sig->print_value, "_", sizeof(context_parse_sig->print_value));
    sal_strncat_s(context_parse_sig->print_value, context_parse_string, sizeof(context_parse_sig->print_value));
    sal_strncat_s(context_parse_sig->print_value, "(0x", sizeof(context_parse_sig->print_value));
    sal_itoa(context_value_string, data[0], 16, 0, 1);
    sal_strncat_s(context_parse_sig->print_value, context_value_string, sizeof(context_parse_sig->print_value));
    sal_strncat_s(context_parse_sig->print_value, ")", sizeof(context_parse_sig->print_value));

exit:
    sand_signal_output_free(nasid_sig);
    SHR_FUNC_EXIT;
}
    ...
    char to_n[RHNAME_MAX_SIZE])
{
    ...
    SHR_IF_ERR_EXIT(dnx_pp_stage_string_to_id(unit, from_n, &current_stage_id));

    if(current_stage_id == DNX_PP_STAGE_IPMF1)
    {
        field_stage = DNX_FIELD_STAGE_IPMF2;
    }
    else if(current_stage_id == DNX_PP_STAGE_IPMF3)
    {
        field_stage = DNX_FIELD_STAGE_IPMF3;
    }
    else if(current_stage_id == DNX_PP_STAGE_EPMF)
    {
        field_stage = DNX_FIELD_STAGE_EPMF;
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Signal is not from a PMF context stage. stage is %s,"
                " and expected stages are: IPMF1, IPMF3, EPMF.", from_n);
    }

    
    SHR_IF_ERR_EXIT(dnx_field_context_mode_get(unit, field_stage, *data, &context_mode));

    
    if (NULL == *parsed_info)
    {
        if ((*parsed_info = utilex_rhlist_create("pmf context parsed info", sizeof(signal_output_t), 0)) == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_RESOURCE, "Could not allocate parsed information list to parse the required buffer.\n");
        }
    }
    
    SHR_IF_ERR_EXIT(utilex_rhlist_entry_add_tail(*parsed_info, "PMF_NAME", RHID_TO_BE_GENERATED, &rhhandle));
    new_sig = rhhandle;
    new_sig->size = CONTEXT_SIG_SIZE;
    sal_strncpy(new_sig->print_value, context_mode.name, DSIG_MAX_SIZE_STR);
    new_sig->value[0] = *data;
        ...
        RHSAFE_ITERATOR(field_output, field_list)
        {
            ...
            {   
                ...
                if((flags & SIGNALS_MATCH_LEAF) && (target_signal_output_p != NULL))
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT(utilex_rhlist_entry_remove(field_list, field_output));
                                     ...
                                     match_t * match_p)
{
    ...
    RHITERATOR(translated_signal, device->translated_list)
    {
        ...
        if (!ISEMPTY(RHNAME(translated_signal)) && !sal_strcasecmp(RHNAME(translated_signal), signal_n))
        {
            ...
            match_p->name = !ISEMPTY(translated_signal->translate_signal_n) ?
                    translated_signal->translate_signal_n : match_p->name;
            match_p->block = !ISEMPTY(translated_signal->translate_block_n) ?
                    translated_signal->translate_block_n : match_p->block;
            match_p->from = !ISEMPTY(translated_signal->translate_from_n) ?
                    translated_signal->translate_from_n : match_p->from;
            match_p->to = !ISEMPTY(translated_signal->translate_to_n) ?
                    translated_signal->translate_to_n : match_p->to;
            SHR_EXIT();
       }
    }
    ...
    signal_output_t **signal_output_p)
{
...
#ifdef ADAPTER_SERVER_MODE
    int is_db_signal = 0;
    dbal_physical_tables_e phy_table_id = (dbal_physical_tables_e)(-1);
#endif
    ...
    if (match_p == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "match_p is NULL. Illegal.\r\n");
    }
    ...
    {
        uint8 replace_from = 0;
        uint8 replace_to = 0;
        uint8 replace_block = 0;
        if((match_p->from != NULL) && (sal_strncasecmp(match_p->from,"Fabric",15) == 0))
        {
            replace_from = 1;
            if((match_p->block != NULL) && (sal_strncasecmp(match_p->block,"ERPP",15) == 0))
            {
                replace_block = 1;
            }
            if((match_p->to != NULL) && ((sal_strncasecmp(match_p->to,"ERParser",15) == 0) || (sal_strncasecmp(match_p->to,"PHL",15) == 0)))
            {
                replace_to = 1;
            }
        }
        ...
        if(replace_from)
        {
            ...
            {
                match_p->from = "ERParser";
            }
            else
            {
                match_p->from = "ETParser";
            }
        }
        if(replace_to)
        {
    ...
    if((match_p->from != NULL) && (sal_strncasecmp(match_p->from,"IPMF1",15) == 0) && (match_p->to != NULL) && (sal_strncasecmp(match_p->to,"FER",15) == 0))
    {
        match_p->to = "IPMF2";
    }
#endif
    ...
    for (i_bl = 0; i_bl < device->block_num; i_bl++)
    {
        ...
        if(!(match_p->flags & SIGNALS_MATCH_ASIC) && !(match_p->flags & SIGNALS_MATCH_PEM))
        {
            ...
            for (i_st = 0; i_st < cur_pp_block->stage_num; i_st++)
            {
                ...
                is_db_signal = 0;
                if ( dbal_physical_table_string_to_id_no_error(unit, match_p->from, &phy_table_id) == _SHR_E_NONE  ||
                   dbal_physical_table_string_to_id_no_error(unit, match_p->to, &phy_table_id) == _SHR_E_NONE )
                {
                    is_db_signal = 1;
                }
                ...
                if(!(match_p->flags & SIGNALS_MATCH_NOVALUE) && (cur_pp_stage->stage_signal_num == 0))
                {
                    int rv;
                    rv = sand_adapter_get_stage_signals(device, unit, core, cur_pp_block->name, cur_pp_stage,
                                                                                                    match_p->flags);
                    if(rv == _SHR_E_EMPTY)
                    {
                        SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
                ...
                for (i_sig = 0; i_sig < cur_pp_stage->stage_signal_num; i_sig++)
                {
                    ...
                    if ( is_db_signal && sand_signal_filter_stage(debug_signal, NULL, NULL, match_p) != _SHR_E_NONE )
                    {
                        continue;
                    }
    ...
    if (expected_value_a != NULL)
    {
        ...
        if (sal_memcmp(expected_value_a, signal_output->value, WORDS2BYTES(BITS2WORDS(signal_output->size))) != 0)
        {
            ...
            else
            {
                ...
                sal_strncpy_s(result_str, signal_output->print_value, DSIG_MAX_SIZE_STR - 1);
            }
