    ...
    *nof_signals = 0;
    ...
    for(vw_idx = 0; vw_idx < virtual_wires->nof_virtual_wires; vw_idx++)
    {
        ...
        for(stage_idx = start_stage; stage_idx <= end_stage; stage_idx++)
        {
            ...
            if((pp_stage != DNX_PP_STAGE_INVALID) && (dnx_data_pp.stages.params_get(unit, pp_stage)->is_pem))
            {
            ...
            if((pp_stage != DNX_PP_STAGE_INVALID) && (dnx_data_pp.stages.params_get(unit, pp_stage)->is_pem))
            {
                ...
                if(sdk_block_id == pemla_block)
                {
                    ...
                    for(vw_mappping_idx = 0; vw_mappping_idx < virtual_wires->vw_info_arr[vw_idx].nof_mappings; vw_mappping_idx++)
                    {
                        ...
                        debug_signal->address[vw_mappping_idx].low = pp_stage;
                    }
                    if(phy_wire_found)
                    {
...
#ifdef BCM_DNX_SUPPORT
                    if (debug_signal->block_id == DBAL_ENUM_FVAL_PP_ASIC_BLOCK_TO_ENGINE)
                    {
                        SHR_IF_ERR_EXIT(dnx_pp_stage_string_to_id(unit, debug_signal->from, (dnx_pp_stage_e *)&debug_signal->address[0].low));
                    }
                    else if (debug_signal->block_id == DBAL_ENUM_FVAL_PP_ASIC_BLOCK_FROM_ENGINE)
                    {
                        SHR_IF_ERR_EXIT(dnx_pp_stage_string_to_id(unit, debug_signal->to, (dnx_pp_stage_e *)&debug_signal->address[0].low));
                    }
#endif
                }
