...
soc_link_mask2_set(int unit, pbmp_t mask)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) &&
        ...
        /* To support, switching from cascaded port to phy-port at runtime,
         * additional logic is needed: If, the port in question:
         *      1. Is designated to be used as a subport (linkphy_pp_port_pbm)
         *      2. Is currently not 'created' as subport (enabled_linkphy_pp_port_pbm).
         *          (Governed by the API: bcm_subport_port_add)
         *      3. Is currently not setup to be used as a subport (linkphy_pbm).
         *           (Governed by the run time port property: bcmPortControlLinkPhyEnable)
         *  If all these criteria are met, add the port to 'link_mask2', this might be a case
         *  where user is going from COE -> PHY-PORT at runtime.
         */
        SOC_PBMP_ITER(mask, pp_port_id) {
            if (SOC_PBMP_MEMBER(SOC_INFO(unit).linkphy_pp_port_pbm, pp_port_id) &&
                !SOC_PBMP_MEMBER(SOC_INFO(unit).enabled_linkphy_pp_port_pbm, pp_port_id)) {
                   if (!SOC_PBMP_MEMBER(SOC_INFO(unit).linkphy_pbm, pp_port_id)) {
                       SOC_PBMP_PORT_ADD(SOC_CONTROL(unit)->link_mask2, pp_port_id);
                   }
            }
        }
    ...
    if (soc_feature(unit, soc_feature_subtag_coe) &&
        ...
        /* To support, switching from cascaded port to phy-port at runtime,
         * additional logic is needed: If, the port in question:
         *      1. Is designated to be used as a subport (subtag_pp_port_pbm)
         *      2. Is currently not 'created' as subport (enabled_subtag_pp_port_pbm).
         *          (Governed by the API: bcm_subport_port_add)
         *      3. Is currently not setup to be used as a subport (subtag_pbm).
         *           (Governed by the run time port property: bcmPortControlSuntagEnable)
         *  If all these criteria are met, add the port to 'link_mask2', this might be a case
         *  where user is going from COE -> PHY-PORT at runtime.
         */
        SOC_PBMP_ITER(mask, pp_port_id) {
            if (SOC_PBMP_MEMBER(SOC_INFO(unit).subtag_pp_port_pbm, pp_port_id) &&
                !SOC_PBMP_MEMBER(SOC_INFO(unit).enabled_subtag_pp_port_pbm, pp_port_id)) {
                   if (!SOC_PBMP_MEMBER(SOC_INFO(unit).subtag_pbm, pp_port_id)) {
                       SOC_PBMP_PORT_ADD(SOC_CONTROL(unit)->link_mask2, pp_port_id);
                   }
            }
        }
...
{
    SOC_LINKSCAN_DRIVER(unit) = drv;
    return SOC_E_NONE;
}

int
soc_linkscan_flr_timeout_value_get(int unit,
                                   ...
                                   uint32 *value)
{
    ...
    switch (timeout) {
        /* Timeout value 50ms  */
        case socLinkscanFlrTimeout50ms:
            *value = 50;
            break;
        /* Timeout value 75ms  */
        case socLinkscanFlrTimeout75ms:
            *value = 75;
            break;
        /* Timeout value 100ms */
        case socLinkscanFlrTimeout100ms:
            *value = 100;
            break;
        /* Timeout value 150ms */
        case socLinkscanFlrTimeout150ms:
            *value = 150;
            break;
        /* Timeout value 250ms */
        case socLinkscanFlrTimeout250ms:
            *value = 250;
            break;
        /* Timeout value 500ms */
        case socLinkscanFlrTimeout500ms:
            *value = 500;
            break;
        default:
            return SOC_E_PARAM;
...
int
soc_linkscan_flr_support_set(int unit, soc_pbmp_t support_ppbm)
{
    if (SOC_LINKSCAN_DRIVER(unit) == NULL) {
        return SOC_E_INIT;
    }

    SOC_PBMP_ASSIGN(LINKSCAN_FLR_SUPPORT_PPBM(unit), support_ppbm);
...
int
soc_linkscan_flr_enable_set(int unit, int port, int enable,
                            soc_linkscan_flr_timeout_t timeout)
{
    uint32 value = 0;
    int pport;
    ...
    if (pport < 0 ||
        LINKSCAN_FLR_SUPPORT(unit, pport) == 0) {
        return SOC_E_PARAM;
    }


    /* Parameters check. */
    if ((enable && timeout == socLinkscanFlrTimeoutInvalid) ||
         timeout >= socLinkscanFlrTimeoutCount)
    {
        return SOC_E_PARAM;
    }
    ...
    if (enable) {
        ...
        SOC_IF_ERROR_RETURN
            (soc_linkscan_flr_timeout_value_get(unit, timeout, &value));

        SOC_IF_ERROR_RETURN
            (SOC_LINKSCAN_FLR_TIMEOUT_SET(unit, pport, value));

    }

    /* Enable or Disbale FLR. */
    SOC_IF_ERROR_RETURN
        (SOC_LINKSCAN_FLR_ENABLE_SET(unit, pport, enable));

    /* Save setting for warm boot. */
    if (enable) {
        SOC_PBMP_PORT_ADD(SOC_PERSIST(unit)->lc_pbm_flr_enable, port);
        SOC_PERSIST(unit)->lc_flr_timeout[port] = value;
    } else {
        SOC_PBMP_PORT_REMOVE(SOC_PERSIST(unit)->lc_pbm_flr_enable, port);
    }
...
int
soc_linkscan_flr_linkup_count_get(int unit, int port, int *count)
{
    uint32 val;
    int pport;
    soc_info_t *si;
    ...
    /* Get event count. */
    SOC_IF_ERROR_RETURN
        (SOC_LINKSCAN_FLR_LINKUP_CNT_GET(unit, pport, &val));
...
int
soc_linkscan_flr_linkup_callback_set(int unit,
                                     soc_linkscan_flr_linkup_callback_f cb)
{
    int int_en;

    SOC_CONTROL(unit)->hw_linkscan_flr_linkup_cb = cb;
    ...
    return SOC_LINKSCAN_FLR_LINKUP_CALLBACK_EN_SET(unit, int_en);
}
...
soc_linkscan_flr_wb_restore(int unit)
{
    ...
    SOC_PBMP_ITER(LINKSCAN_FLR_SUPPORT_PPBM(unit), pport) {

        lport = si->port_p2l_mapping[pport];
        if (lport < 0) continue;

        value = SOC_PERSIST(unit)->lc_flr_timeout[lport];

        if (PBMP_MEMBER(SOC_PERSIST(unit)->lc_pbm_flr_enable, lport)) {

            SOC_IF_ERROR_RETURN
                (SOC_LINKSCAN_FLR_TIMEOUT_SET(unit, pport, value));

            SOC_IF_ERROR_RETURN
                (SOC_LINKSCAN_FLR_ENABLE_SET(unit, pport, 1));
        }
    }
...
{
    SOC_IF_ERROR_RETURN
        (soc_linkscan_drv_set(unit, NULL));

#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        SOC_IF_ERROR_RETURN
            (soc_cmicx_linkscan_driver_init(unit));
    }
#endif
    return SOC_E_NONE;
}
