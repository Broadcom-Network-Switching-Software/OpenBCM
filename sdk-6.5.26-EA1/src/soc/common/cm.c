 ...
 *              sub_devs - An Array of the existing sub-devices of the composite device.
...
int
soc_cm_device_create_composite(uint32 composite_dev_type, int *sub_devs, void *cookie, int comp_unit)
{
    cm_device_t null_device;
    ...
    CMDEV(comp_unit).dev.composite_index = soc_cm_nof_composite_devices++;
...
const soc_cm_composite_dev_t* soc_cm_get_composite_dev_info(int unit)
{
    ...
    if (((unsigned)unit) < SOC_MAX_NUM_DEVICES && cm_dev->dev == -1 &&
        ...
        assert(ret->dev_type->nof_sub_devs > 1 &&
               soc_cm_device[ret->sub_devs[0]].dev.composite_index == unit &&
               soc_cm_device[ret->sub_devs[1]].dev.composite_index == unit);
    }
    return ret;
}
...
soc_cm_device_destroy(int unit)
{
    ...
    if (info->dev_type & SOC_SWITCH_DEV_TYPE) { /* remove the unit from the loop over units */
    ...
    if (bde_dev >= 0 && bde_dev < SOC_MAX_NUM_DEVICES) { /* Remove the mapping from BDE device to the unit */
        if (cm_bde_dev2unit[bde_dev] == unit) {
            ...
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META("WARNING: Destroyed unit %d has BDE device %d which is mapped to a different unit %d"), unit, bde_dev, cm_bde_dev2unit[bde_dev]));
        }
    }
