...
int soc_is_miim_12r(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_iproc_17) || SOC_IS_TOMAHAWK3(unit) || SOC_IS_HELIX5(unit) ||
...
soc_cmicx_miim_divider_set_all(int unit, int int_divider, int ext_divider, int out_delay)
{
    ...
    for (ring_idx = CMICX_MIIM_RING_INDEX_START; ring_idx <= ring_idx_end; ring_idx++) {
        value = soc_property_suffix_num_get(unit, ring_idx,
                    spn_RATE_EXT_MDIO_DIVISOR, "", RATE_EXT_MDIO_DIVISOR_DEF(unit));
        delay = soc_property_suffix_num_get(unit, ring_idx,
                    spn_MDIO_OUTPUT_DELAY, "", -1);
        soc_cmicx_miim_divider_set_ring(unit, ring_idx, int_divider, value, delay);
    }
...
soc_cmicx_miim_operation(int unit, int is_write, int clause, uint32 phy_id, uint32 phy_reg_addr, uint16 *phy_data)
{
    ...
    if (SOC_IS_DNX(unit)) {
        if (is_write && !DNX_ERR_RECOVERY_HW_ACCESS_IS_VALID(unit))
        {
            LOG_WARN(BSL_LS_SOC_MIIM,
                (BSL_META_U(unit,
                                "soc_cmicx_miim_operation: ERROR, Invalid error recovery hw access Attempting write outside of dbal access region\n")));
            _SOC_IF_ERR_EXIT(SOC_E_FAIL);
        }
    }
#endif
    ...
    */
    ...
    if (clause45){
        if (is_write && is_broadcast) {
            LOG_VERBOSE(BSL_LS_SOC_MIIM,
                    (BSL_META_U(unit,
                                "soc_cmicx_miim_operation cl45 broadcast write: "
                                "(id=0x%02x readr=0x%x dtype=0x%x data=0x%04x clause45=%d, real_phy_id=0x%x, mdio_buses=0x%x)\n"),
                    phy_id, SOC_PHY_CLAUSE45_REGAD(phy_reg_addr), SOC_PHY_CLAUSE45_DEVAD(phy_reg_addr),
                    *phy_data, clause45, real_phy_id, mdio_buses));
        } else {
            LOG_VERBOSE(BSL_LS_SOC_MIIM,
                    (BSL_META_U(unit,
                                "soc_cmicx_miim_operation cl45 %s: "
                                "(id=0x%02x readr=0x%x dtype=0x%x data=0x%04x clause45=%d, real_phy_id=0x%x, real_bus_id=0x%x)\n"),
                    is_write ? "write" : "read ", phy_id, SOC_PHY_CLAUSE45_REGAD(phy_reg_addr),
        ...
        if (is_write && is_broadcast) {
            LOG_VERBOSE(BSL_LS_SOC_MIIM,
                    (BSL_META_U(unit,
                                "soc_cmicx_miim_operation cl22 broadcast write: "
                                "(id=0x%02x addr=0x%02x data=0x%04x clause45=%d, real_phy_id=0x%x, mdio_buses=0x%x)\n"),
                    phy_id, phy_reg_addr, *phy_data, clause45, real_phy_id, mdio_buses));
        } else {
            LOG_VERBOSE(BSL_LS_SOC_MIIM,
                    (BSL_META_U(unit,
                                "soc_cmicx_miim_operation cl22 %s: "
                                "(id=0x%02x addr=0x%02x data=0x%04x clause45=%d, real_phy_id=0x%x, mdio_buses=0x%x)\n"),
    ...
    soc_reg_field_set(unit, MIIM_CH0_PARAMSr, &reg_val, RING_MAPf, mdio_buses);
