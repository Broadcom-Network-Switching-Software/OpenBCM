...
static uint32 _soc_iproc_mbox_get_devid_val(int unit)
{
    uint32 rc = CMDEV(unit).dev.info->dev_id_driver;
#if (defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT))
    if (SOC_IS_JERICHO2(unit))
    {
        rc = BCM88690_DEVICE_ID;
    }
    else if (SOC_IS_J2C(unit))
    {
        /* include sku BCM88800_DEVICE_ID & BCM88820_DEVICE_ID */
        rc = BCM88800_DEVICE_ID;
    }
    else if (SOC_IS_Q2A(unit))
    {
        /* include Q2A_DEVICE_ID/Q2U_DEVICE_ID/Q2N_DEVICE_ID */
        rc = Q2A_DEVICE_ID;
    }
    else if (SOC_IS_J2P(unit))
    {
        /* include BCM88850_DEVICE_ID & BCM88840_DEVICE_ID */
        rc = BCM88850_DEVICE_ID;
    }
    else if (SOC_IS_J2X(unit))
    {
        rc = BCM88830_DEVICE_ID;
    }
    else if (SOC_IS_RAMON(unit))
    {
        rc = BCM88790_DEVICE_ID;
    }
#endif
...
int soc_iproc_mbox_init(int unit)
{
    ...
    for (ucnum = 0; ucnum < max_ucores; ucnum++) {
        ...
        if (!(SAL_BOOT_PLISIM || SAL_BOOT_BCMSIM)) {
            devid_val = _soc_iproc_mbox_get_devid_val(unit);
            soc_m0ssq_tcam_write32(unit, ucnum, DEVID_ID_OFFSET, devid_val);
        }
