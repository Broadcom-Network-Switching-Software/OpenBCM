                      ...
                      soc_field_t field, uint32 *fldbuf)
{
    ...
    if (NULL == fieldinfo) {
        ...
        assert(fieldinfo);
        /*
         * Since the above 'assert' does not always end with 'exit',
         * We need to add 'return' here (for the sake of coverity).
         */
        return;
    }
                     ...
                     soc_field_t field, const ip6_addr_t ip6, int flags)
{
    ...
    if (flags == SOC_MEM_IP6_UPPER_ONLY) {
        ...
        ip6_field[0] = ((ip6[12] << 24)| (ip6[13] << 16) |
                        (ip6[14] << 8) | (ip6[15] << 0));
        soc_mem_field_set(unit, mem, entry, field, ip6_field);
    } else if (flags == SOC_MEM_IP6_LOWER_100BIT) {
        ip6_field[3] = ip6[3] & 0xf;
        ip6_field[2] = ((ip6[4] << 24) | (ip6[5] << 16) |
                        (ip6[6] << 8)  | (ip6[7] << 0));
        ip6_field[1] = ((ip6[8] << 24) | (ip6[9] << 16) |
                        (ip6[10] << 8) | (ip6[11] << 0));
        ip6_field[0] = ((ip6[12] << 24)| (ip6[13] << 16) |
                        (ip6[14] << 8) | (ip6[15] << 0));
        soc_mem_field_set(unit, mem, entry, field, ip6_field);
    } else if (flags == SOC_MEM_IP6_UPPER_28BIT) {
        ip6_field[0] = ((ip6[0] << 20) | (ip6[1] << 12) |
                        (ip6[2] << 4)  | (ip6[3] >> 4));
        soc_mem_field32_set(unit, mem, entry, field, ip6_field[0]);
    } else {
                     ...
                     soc_field_t field, ip6_addr_t ip6, int flags)
{
    ...
    if (flags == SOC_MEM_IP6_UPPER_ONLY) {
        ...
        ip6_field[0] = soc_mem_field32_get(unit, mem, entry, field);
        ip6[3] |= (uint8) ((ip6_field[0] << 4) & 0xff);
        ip6[2] = (uint8) ((ip6_field[0] >> 4) & 0xff);
        ip6[1] = (uint8) ((ip6_field[0] >> 12) & 0xff);
        ip6[0] = (uint8) ((ip6_field[0] >> 20) & 0xff);
    } else {
        soc_mem_field_get(unit, mem, entry, field, ip6_field);
