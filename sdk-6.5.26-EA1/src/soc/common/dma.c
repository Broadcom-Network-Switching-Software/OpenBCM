...
#ifdef BCM_CMICM_SUPPORT
            /* Either DMA is stuck or RX packet rate is normal */
            if (SOC_FEATURE_CMICD(unit)) {
                int detected;
                int rv;
                /* check if rx pkt processing stopped */
                if (stat->dma_rpkt == dma_rpkt) {
                    /* Check if address decode err is set. */
                    cmic_drv[unit].chan_poll(unit, vchan,
                                     SOC_DMA_POLL_ADDR_DECODE_ERR, &detected);
                    if (detected) {
                        uint32 soc_dma_lock;
                        sdc_t *sc;

                        soc->stat.err_dma_addr_decode++;
                        sc = &soc->soc_channels[vchan];

                        /* Notify the application about address decode error */
                        soc_event_generate(unit, SOC_SWITCH_EVENT_IO_ERROR,
                                SOC_SWITCH_EVENT_IO_ERROR_PKTDMA_ADDR_DECODE_ERR,
                                0, 0);

                        /* Restart the dma channel */
                        SOC_DMA_LOCK(soc_dma_lock);
                        rv = cmic_drv[unit].chan_restart(unit, sc);
                        SOC_DMA_UNLOCK(soc_dma_lock);
                        if (SOC_FAILURE(rv)) {
                            LOG_ERROR(BSL_LS_SOC_PACKETDMA,
                                (BSL_META_U(unit,
                                 "%s:  DMA Address decode error mitigation failed\n"),
                                 __func__));
                            /*
                            ...
                            soc_event_generate(unit, SOC_SWITCH_EVENT_IO_ERROR,
                                SOC_SWITCH_EVENT_IO_ERROR_PKTDMA_TIMEOUT,
                                SOC_SWITCH_PCI_RESET_COLD_BOOT, 0);
                        }
                    }
                } else {
                    dma_rpkt = stat->dma_rpkt;
                }
                sal_sem_take(sdc_mtg->thread_sem, DMA_CHAIN_DONE_TIMEOUT_USEC * 250);
                continue;
            }
            else
#endif
...
soc_dma_dump_dv(int unit, char *pfx, dv_t *dv_chain)
{
    ...
    for (dv = dv_chain; dv; dv = dv->dv_chain) {
        ...
        if (dv_chain->dv_flags & DV_F_NOTIFY_DSC) {
            sal_strlcpy(tmps + len_tmps , "notify-dsc ", (128 - len_tmps));
        }
        if (dv_chain->dv_flags & DV_F_NOTIFY_CHN) {
            sal_strlcpy(tmps + len_tmps, "notify-chn ", (128 - len_tmps));
        }
        if (dv_chain->dv_done_packet) {
            sal_strlcpy(tmps + len_tmps, "notify-pkt ", (128 - len_tmps));
        }
        if (dv_chain->dv_flags & DV_F_COMBINE_DCB) {
            sal_strlcpy(tmps + len_tmps, "combine-dcb ", (128 - len_tmps));
        }
...
soc_dma_start_dv(int unit, sdc_t *sc, dv_t *dv_chain)
{
     ...
     */
    if (SOC_KCHAN(unit, dv_chain->dv_channel)) {
...
soc_dma_process_done_desc(int unit, dv_t *dv_chain, dv_t *dv)
{
    ...
    while (dv != NULL) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_PACKETDMA,
                    (BSL_META_U(unit,
                                "soc_dma_process_done_desc "
                                "dv_chain=%p dv_active=%p, chan=%d\n"),
                     (void *)dv_chain, (void *)dv, dv_chain->dv_channel));
        ...
        for (i = dv->dv_dcnt; i < dv->dv_vcnt; i++) {
            ...
            LOG_VERBOSE(BSL_LS_SOC_PACKETDMA, /* VERBOSE */
                        (BSL_META_U(unit,
                             "soc_dma_process_done_desc "
                             "at %p:0x%lx flags=0x%x dcnt = %d vcnt = %d\n"),
                             (void *)dcb, (long unsigned int)soc_cm_l2p(unit, dcb),
                             (uint32)flags, dv->dv_dcnt, dv->dv_vcnt));
 ...
 * Purpose:
 *      Process PKTDMA PCI timeout error.
 * Parameters:
 *      unit number
 *        cmc
 * Returns:
...
static void
dma_dpc_restart(void *p_unit, void *p_tx, void *p_vchan,
                void *p4, void *p5)
{
    int unit = PTR_TO_INT(p_unit);
    int tx =   PTR_TO_INT(p_tx);
    dv_t *dv_chain = (dv_t *)p4;
    dv_t *dv_active = (dv_t *)p5;
    int vchan =  PTR_TO_INT(p_vchan);
    ...
    LOG_ERROR(BSL_LS_SOC_PACKETDMA,
        (BSL_META_U(unit,
         "%s dma chain dump.......\n"), (tx) ? "TX": "RX"));
    soc_dma_dump_dv(unit, "RX dma chain: ", dv_chain);

    if (dv_active != dv_chain) {
        LOG_ERROR(BSL_LS_SOC_PACKETDMA,
            (BSL_META_U(unit,
             "%s dma chain dump.......\n"), (tx) ? "TX": "RX"));
        soc_dma_dump_dv(unit, "RX dma active: ", dv_active);
    }

    /* Log all types of DMA that are currently active. */
    if (soc_feature(unit, soc_feature_cmicm)) {
        int cmc = vchan / N_DMA_CHAN;
        (void)soc_cmicm_dma_active(unit, cmc);
    }

    etime = sal_time_usecs();
    LOG_ERROR(BSL_LS_SOC_PACKETDMA,
        (BSL_META_U(unit,
         "%s:  DV/DCB dump is done in  %d usec\n"),
         __func__, SAL_USECS_SUB(etime, stime)));

    if ((!tx) && (soc->soc_dcb_error_mitigate_enable)) {
        soc_event_generate(unit, SOC_SWITCH_EVENT_IO_ERROR,
                           SOC_SWITCH_EVENT_IO_ERROR_PKTDMA_NO_DATA,
                           SOC_SWITCH_PKTDMA_RX_RESTART, 0);
    } else {
...
#endif
static void
dma_done_desc_check(int unit, dv_t *dv_chain, dv_t *dv_active, sdc_t *sc )
{
#ifdef BCM_TOMAHAWK_SUPPORT
    int retries = 3;
    soc_control_t *soc = SOC_CONTROL(unit);
    ...
    tx = (dv_chain->dv_op == DV_TX);
    if (dv_chain->dv_dcnt != dv_chain->dv_vcnt) {
        /* Only apply mitigation for Tomahawk/Tomahawk2 */
        if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)) {
            soc->soc_dv_rx_dv_done_unmatch_cnt++;
            stime = sal_time_usecs();
            /* Retry in case of RX to sync up DCB */
            if (!tx) {
                while((dv_chain->dv_dcnt != dv_chain->dv_vcnt) && (retries > 0)) {
            ...
            /* Check and assert */
            if (!retries) {
                /* Dump dma and paxb registers */
                if (cmic_drv[unit].chan_cur_op_datails) {
                    cmic_drv[unit].chan_cur_op_datails(unit, sc);
                }

                etime = sal_time_usecs();
                LOG_ERROR(BSL_LS_SOC_PACKETDMA,
                    (BSL_META_U(unit,
                    "%s:  Retry and DMA reg dump is done in  %d usec\n"),
                        __func__, SAL_USECS_SUB(etime, stime)));
                sal_dpc(dma_dpc_restart, INT_TO_PTR(unit),
                        INT_TO_PTR(tx), INT_TO_PTR(sc->sc_channel),
                        dv_chain, dv_active);
            }
        } else {
            assert(dv_chain->dv_dcnt == dv_chain->dv_vcnt);
...
soc_dma_init(int unit)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        for (chan = 2; chan < rx_chans; chan++) {
            SOC_IF_ERROR_RETURN
                (soc_dma_chan_config(unit, chan, DV_RX, flags));
        }
    }
    else
    {
        for (chan = 2; chan < rx_chans; chan++) {
            SOC_IF_ERROR_RETURN
                (soc_dma_chan_config(unit, chan, DV_NONE, SOC_DMA_F_MBM));
        }
    }
    ...
    /* Enable retry and DMA restart incase of DCB count mismatch */
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)) {
        soc_control_t *soc = SOC_CONTROL(unit);
        soc->soc_dcb_error_mitigate_enable =
             soc_property_get(unit, spn_CMICM_DCB_ERROR_MITIGATE_ENABLE, 0);
    }
...
soc_dma_attach(int unit, int reset)
{
...
#ifdef INCLUDE_KNET
    if ((rv = soc_knet_init(unit)) == SOC_E_NONE) {
        ...
        LOG_ERROR(BSL_LS_SOC_PACKETDMA,
                  (BSL_META_U(unit, "Failed to initialize KNET: rv=%d\n"), rv));
...
#ifdef BCM_CMICX_SUPPORT
        if (soc_feature(unit, soc_feature_cmicx)) {
            soc->soc_unet_channels  = soc_property_suffix_num_get(unit, -1,
                                        spn_CUSTOM_FEATURE, "unet_channels", 0);
            soc->soc_unet_channels &= (0x1 << soc->soc_dma_channels) - 1;
            if (soc->soc_unet_channels & 0x3) {
                LOG_WARN(BSL_LS_SOC_COMMON,
                          (BSL_META("Channel 0 and 1 are always used for Knet TX and RX.\n")));
                soc->soc_unet_channels &= ~0x3;
            }
        } else
#endif
        {
            soc->soc_unet_channels  = 0;
        }
    } else
#endif
    {
    ...
    if (soc_property_get(unit, spn_DCB_INTR_MITIGATE_ENABLE, 0) &&
        ...
        for (i = 0; i < soc->soc_dma_channels; i++) {
            ...
            if (soc_dma_intr_mitigation[unit].enabled == 0) {
