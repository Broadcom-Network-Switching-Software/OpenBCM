...
#ifdef BCM_CMICX_SUPPORT
uint32 sw_prog_intr[SOC_MAX_NUM_DEVICES] = {SW_PROG_INTR};
#endif
...
_soc_cmic_uc_msg_thread(void *unit_vp) {
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        soc_cmic_intr_enable(unit, sw_prog_intr[unit]);
    }
#endif
...
#if defined(BCM_CMICM_SUPPORT) && defined(UC_MSG_DEFER_SWI_ENABLE)
                if (soc_feature(unit, soc_feature_cmicm)) {
                    (void)soc_cmicm_intr0_enable(unit, IRQ_CMCx_SW_INTR(uC_intr));
                }
#endif
            }
...
#ifdef BCM_CMICM_SUPPORT
    if (soc_feature(unit, soc_feature_cmicm)) {
#ifdef UC_MSG_DEFER_SWI_ENABLE
        (void)soc_cmicm_intr0_disable(unit, IRQ_CMCx_SW_INTR(rupt_num));
#else
...
#if defined(BCM_DNX_SUPPORT)
    sw_prog_intr[unit] = SW_PROG_INTR;
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
        sw_prog_intr[unit] = IPROC17_SW_PROG_INTR;

        /* Enable the SW interrupts from all 4 cores */
        WRITE_PAXB_0_SW_PROG_INTR_ENABLEr(unit, 0xF<<5);
    }
#endif
    handle->num = sw_prog_intr[unit];
