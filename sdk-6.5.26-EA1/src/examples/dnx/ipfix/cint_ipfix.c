...
cint_ipfix_egress_create(int unit, int in_port, int out_port, int eventor_port, bcm_gport_t *ipfix_destination)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in bcm_instru_ipfix_encap_create\n", proc_name, rv);
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in bcm_instru_ipfix_interface_add\n", proc_name, rv);
        return rv;
    }
    ...
    /* Enable IPFIX on eventor port */
    rv = bcm_port_control_set(unit, eventor_port,bcmPortControlIpFixRxEnable,1);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in bcm_port_control_set\n", proc_name, rv);
        return rv;
    }
    ...
    *ipfix_destination = ipfix_encap_info.ipfix_encap_id;
    ...
    rv = cint_ipfix_field_egress(unit, &ipfix_encap_info);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in cint_ipfix_field_egress\n", proc_name, rv);
        return rv;
    }
...
cint_ipfix_ingress_create(int unit, int in_port, int ipfix_out_port, bcm_gport_t ipfix_destination)
{
     ...
     */
    rv = cint_ipfix_mirror_create(unit, ipfix_out_port, ipfix_destination,
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), cint_ipfix_mirror_create \n", proc_name, rv);
        return rv;
    }
    ...
    rv = cint_ipfix_field_entry_add(unit, bcmFieldLayerTypeEth, bcmFieldLayerTypeIp4, TRUE, cint_ipfix_basic_info.ipfix_mirror_destination,
                field_entry_handle);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in cint_ipfix_field_entry_add\n", proc_name, rv);
        return rv;
    }
...
cint_ipfix_registers_init(int unit, int eventor_id)
{
    ...
    proc_name = "cint_ipfix_registers_init";

    /*
     * Set the global IPFIX System UP time.
     */
    rv = bcm_instru_control_set(unit, 0, bcmInstruControlIpFixSystemUpTime, cint_ipfix_basic_info.uptime);
    if (rv != BCM_E_NONE)
    {
     ...
     */
    rv = bcm_instru_control_set(unit, 0, bcmInstruControlIpFixEventorId, eventor_id);
    if (rv != BCM_E_NONE)
    {
         printf("%s: Error (%d), in bcm_instru_control_set\n", proc_name, rv);
        return rv;
     ...
     */
    rv = bcm_instru_control_set(unit, 0, bcmInstruControlIpFixTemplateId, 0x14);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in bcm_instru_control_set\n", proc_name, rv);
        return rv;
     ...
     */
    rv = bcm_instru_control_set(unit, 0, bcmInstruControlIpFixTxTime, 0x230614);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in bcm_instru_control_set\n", proc_name, rv);
        return rv;
     ...
     * For the purpose of the example cint, we'll use a random Time.
     */
    rv = bcm_instru_control_set(unit, 0, bcmInstruControlIpFixObservationDomain, cint_ipfix_basic_info.observation_domain);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in bcm_instru_control_set\n", proc_name, rv);
        return rv;
...
uint8 header_data_udp_tunnel[128] = {
            ...
            0x00, 0x0A, 0x01, 0x70, /* IPFIX_VERSION | LENGTH */
...
cint_ipfix_general_init(int unit, int ipfix_out_port, int eventor_port, int eventor_id, int nof_samples)
{
	 ...
	 */
    rv = set_counter_resource(unit, eventor_port, 0, 0, &cint_ipfix_basic_info.ipfix_sequence_counter_id);
    if (rv != BCM_E_NONE)
    {
     ...
     */
    rv = cint_ipfix_field_main(unit, context_id);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in cint_ipfix_field_main\n", proc_name, rv);
        return rv;
    }
     ...
     */
    rv = cint_ipfix_registers_init(unit, eventor_id);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in cint_ipfix_registers_init\n", proc_name, rv);
        return rv;
    }

    rv = cint_ipfix_eventor_set(unit, eventor_id, nof_samples, eventor_port, ipfix_out_port);
    if (rv != BCM_E_NONE)
    {
       printf("%s: Error (%d), in cint_ipfix_eventor_set\n", proc_name, rv);
       return rv;
    }
...
int cint_ipfix_main(int unit, int in_port, int route_out_port, int ipfix_out_port, int eventor_port, int eventor_id)
{
     ...
     */
    rv = dnx_basic_example(unit, in_port, route_out_port, fec_id);
     ...
     */
    rv = cint_ipfix_egress_create(unit, in_port, route_out_port, eventor_port, &ipfix_destination);

    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in cint_ipfix_utils_egress_create\n", proc_name, rv);
        return rv;
    }
     ...
     */
    rv = cint_ipfix_ingress_create(unit, in_port, eventor_port, ipfix_destination);
    if (rv != BCM_E_NONE)
    {
        printf("%s: Error (%d), in cint_ipfix_ingress_create\n", proc_name, rv);
        return rv;
    }
    ...
    */
    rv = cint_ipfix_export_route(unit, eventor_port, ipfix_out_port, fec_id +1);
    if (rv != BCM_E_NONE)
    {
       printf("%s: Error (%d), in cint_ipfix_utils_route_ipfix\n", proc_name, rv);
       return rv;
   }
   ...
   */
   ...
   rv = cint_ipfix_field_entry_add_eventor_port(unit, eventor_port, cint_ipfix_field_fg_id, &field_entry_handle);
   if (rv != BCM_E_NONE)
   {
	  printf("%s: Error (%d), in cint_ipfix_field_utils_entry_add_eventor_port\n", proc_name, rv);
	  return rv;
   }
   ...
   */
   rv = cint_ipfix_field_export_packet(unit, eventor_port, ipfix_out_port, context_id);
   if (rv != BCM_E_NONE)
   {
	  printf("%s: Error (%d), in cint_ipfix_field_export_packet\n", proc_name, rv);
	  return rv;
   }
