    ...
    int virtual_machine_port)
{
     ...
     */
    rv = vxlan_port_add(unit, core_port, cint_vxlan_tunnel_basic_info.tunnel_term_id, &vxlan_port_id);
    if (rv != BCM_E_NONE)
    {
    ...
    int virtual_machine_port)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_vxlan_tunnel_basic_info.fec_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = intf_eth_rif_create(unit, cint_vxlan_tunnel_basic_info.virtual_machine_vlan,
                             cint_vxlan_tunnel_basic_info.core_mac_address);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_eth_rif_create eth_rif_intf_core \n", proc_name);
    ...
    rv = l3__egress_only_fec__create(unit, cint_vxlan_tunnel_basic_info.fec_id, encap_tunnel_intf, 0, virtual_machine_port, 0, fwd_flag);
    if (rv != BCM_E_NONE)
    {
    ...
    int virtual_machine_port)
{
    ...
    /* configure SA for ETH-RIF configured in tunnel's arp */
    rv = intf_eth_rif_create(unit, cint_vxlan_tunnel_basic_info.virtual_machine_vlan_2,
                             cint_vxlan_tunnel_basic_info.core_mac_address);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_eth_rif_create eth_rif_intf_core \n", proc_name);
    ...
    rv = l3__egress_only_fec__create(unit, cint_vxlan_tunnel_basic_info.fec_id, cint_vxlan_tunnel_basic_info.eth_rif_intf_virtual_machine,
                                     cint_vxlan_tunnel_basic_info.arp_id, virtual_machine_port, 0, fwd_flag);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, l3__egress_only_fec__create fec_id=0x%x\n", proc_name,
               cint_vxlan_tunnel_basic_info.tunnel_fec_id);
    ...
    rv = l3__egress_only_fec__create(unit, cint_vxlan_tunnel_basic_info.fec_id_2, encap_tunnel_id,
                                     0, virtual_machine_port, 0, fwd_flag);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, l3__egress_only_fec__create fec_id=0x%x\n", proc_name,
               cint_vxlan_tunnel_basic_info.tunnel_fec_id);
    ...
    int virtual_machine_port)
{
    ...
    rv = vxlan6_tunnel_terminator_create(unit, &term_tunnel_id, FALSE);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in function vxlan_tunnel_terminator_create \n", proc_name);
        return rv;
    ...
    if (cint_vxlan_tunnel_basic_info.is_public_l3_routing)
    {
    	vxlan_tunnel_in_lif_enable_public_routing(unit);
    }
