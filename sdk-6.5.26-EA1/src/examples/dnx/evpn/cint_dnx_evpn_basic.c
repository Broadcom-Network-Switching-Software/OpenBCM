    ...
    int unit)
{
    ...
    /*
     * Remove the usage of it once the related feature is ready
     */
    evpn_global_info.not_ready = *dnxc_data_get(unit, "flow", "general", "is_flow_enabled_for_legacy_applications", NULL);
    ...
    bcm_mpls_egress_label_t * plabel)
{
    ...
    if (iml_info->add_to_flooding && !evpn_global_info.not_ready)
    {
    ...
    if (evpn_global_info.use_split_horizon && !evpn_global_info.not_ready)
    {
