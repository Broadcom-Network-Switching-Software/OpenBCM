        ...
        int eir_weight)
{
    ...
    if ((rv = bcm_cosq_scheduler_gport_add(unit, 0, params, &cir_gport)) != BCM_E_NONE) {
        ...
        for (l4_index = 0; l4_index < L4_NUM_ELEMENTS; l4_index++) {
            ...
            for (l3_index = 0; l3_index < L3_NUM_ELEMENTS; l3_index++) {
                for (se_elem = 0; se_elem < L2_NUM_ELEMENTS; se_elem++) {
                    ...
                    if (rv != BCM_E_NONE) {
                        printf("bcm_cosq_voq_connector_gport_add failed $rv: vc_group=0x%x\n",
                                vc_group);
                        return rv;
                    }
                    ...
                    rv = bcm_cosq_ingress_queue_bundle_gport_add(unit, &conf, &gvoq_base);
                    if (rv != BCM_E_NONE) {
