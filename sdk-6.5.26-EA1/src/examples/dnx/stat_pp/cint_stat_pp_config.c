...
struct cint_stat_pp_config_info_s
{
    ...
    int engine_pp_profile;   /* engine encoded profile of the counter object*/
};
   ...
   int core_id)
{
    ...
    case bcmStatCounterInterfaceEgressPp:
        ...
        break;
    default:
        int es = engine_source; /** W.A. to print the engine source ID */
        printf("engine source not found %d\n", es);
        return 1;
        break;
    }
    if (rv != BCM_E_NONE)
    ...
    if (*dnxc_data_get(unit, "crps", "eviction", "conditional_action_is_supported", NULL))
    {
        engine.core_id = core_id;
        engine.engine_id = cint_stat_pp_config_info.crps_engine_id;
        rv = bcm_stat_counter_engine_control_set(unit, flags, &engine, bcmStatCounterSequentialSamplingInterval, 1);
        if (rv != BCM_E_NONE)
        {
            printf("bcm_stat_counter_engine_control_set failed\n");
        }
    }
    ...
    int *pp_profile_id)
{
    ...
    *pp_profile_id = cint_stat_pp_config_info.pp_profile;
    ...
    cint_stat_pp_config_info.engine_pp_profile = *pp_profile_id;
