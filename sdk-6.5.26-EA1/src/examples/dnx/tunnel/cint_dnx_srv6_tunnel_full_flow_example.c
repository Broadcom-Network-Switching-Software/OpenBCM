                            ...
                            uint8 lif_cs_profile)     /** if not 0, terminated LIFs are set with this CS Profile */
{
    ...
    if (cross_connect)
    {
        rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[EGRESS_FEC_ID], cint_srv6_tunnel_info.eth_rif_intf_out[EGRESS] , cint_srv6_tunnel_info.tunnel_arp_id[EGRESS], gport, 0, 0, 0, 0,&egress_encoded_fec1);
    }
    else
    {
        rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[EGRESS_FEC_ID], 0, cint_srv6_tunnel_info.tunnel_arp_id[EGRESS], gport, 0, fec_flags2, 0, 0,&egress_encoded_fec1);
    }
    if (rv != BCM_E_NONE)
    {
    ...
    else
    {
        ...
        int vrf = 1;
        bcm_ip6_t ip6_mask = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
        rv = add_route_ipv6(unit, cint_srv6_tunnel_info.route_ipv6_dip, ip6_mask, cint_srv6_tunnel_info.vrf_in[INGRESS], cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_FIRST_SID_FEC_ID]);
        if (rv != BCM_E_NONE)
        {
    ...
    if (*dnxc_data_get(unit, "dev_init", "general", "application_v1", NULL))
    {
        /*
