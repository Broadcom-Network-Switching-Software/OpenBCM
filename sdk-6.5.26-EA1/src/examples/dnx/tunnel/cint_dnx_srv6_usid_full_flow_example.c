                    ...
                    int srh_present)         /** If set, SRH header is present in uSID packet */
{
    ...
    rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_IPV6_TUNNEL_FEC_ID], 0 , srv6_basic_lif_encode, gport, 0, 0,
                                           0, 0,&encoded_fec1);
    ...
    rv = l3__egress_only_fec__create(unit, cint_srv6_tunnel_info.tunnel_fec_id[T_INSERT_FEC_ID], srv6_basic_lif_encode, 0, gport, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf
     ...
     */
    {
        ...
        /** End-Point-5: Add MyDIP of current End-Point, getting same VRF for all End-Points */
        srv6_tunnel_termination(unit, END_POINT_TO_T_INSERT, 0, srh_present, cint_srv6_tunnel_info.vrf_in[END_POINT_1], tunnel_type_endpoint);
