    ...
    int is_bridge_fallback)
{
    ...
    rv = bcm_multicast_create(unit, (flags | BCM_MULTICAST_WITH_ID), &cint_ipmc_info.mc_group);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_multicast_create \n");
        return rv;
    }
    rv = bcm_multicast_add(unit, cint_ipmc_info.mc_group, flags, 2, replications);
    if (rv != BCM_E_NONE)
    {
