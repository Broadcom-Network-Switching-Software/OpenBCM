    ...
    int out_port2)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &fec_id_fwd_pass);
    if (rv != BCM_E_NONE)
    {
    ...
    {
        rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 1, &fec_id_sip);
        if (rv != BCM_E_NONE)
        {
            printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
            return rv;
    ...
    for (iter = 0; iter < NOF_OF_MEMBERS_ECMP_GROUP; iter++)
    {
         ...
         */
        rv = create_arp_plus_ac_type(unit, 0, arp_next_hop_mac, &encap_id, 0);
        if (rv != BCM_E_NONE)
        {
     ...
     */
    rv = l3__egress_only_fec__create_inner(unit, fec_id_sip, intf_in, 0, gport, 0, fec_rpf_flags2, 0, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create egress object FEC only\n");
        return rv;
    ...
    {
        rv = add_route_ipv4_rpf(unit, sip, 0xffffffff, vrf, fec_id_sip);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_route_ipv4_rpf(), \n");
            return rv;
        }
    }
    ...
    int vsi_none)
{
    ...
    if(*dnxc_data_get(unit, "l3", "feature", "separate_fwd_rpf_dbs", NULL))
    {
        rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 1, &fec_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
            return rv;
        ...
        rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &fec_id_fwd[0]);
        if (rv != BCM_E_NONE)
        {
            printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
            return rv;
    ...
    rv = l3__egress_only_fec__create(unit, fec_id, intf_out, encap_id, out_port, 0, fec_rpf_flags2);
    if (rv != BCM_E_NONE)
    {
    ...
    for (iter = 0; iter < NOF_OF_MEMBERS_ECMP_GROUP; iter++)
    {
         ...
         * 7.2 Create FEC and set its properties
         */
        rv = l3__egress_only_fec__create(unit, fec_id_ecmp, intf_out, encap_index, out_port, 0, fec_rpf_flags2);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object FEC only, intf_out\n");
            return rv;
    ...
    for (mid = 0; mid < nof_rpf_modes; mid++)
    {
        ...
        rv = l3__egress_only_fec__create(unit, fec_id_fwd[mid], intf_out, encap_id, out_port, 0, fec_fwd_flags2);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object FEC only, intf_out\n");
            return rv;
        ...
        if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
        {
             ...
             */
            rv = add_host_ipv4(unit, dip[mid], vrfs[mid], fec_id_fwd[mid], 0, 0);
            if (rv != BCM_E_NONE)
            {
        ...
        else
        {
             ...
             */
            rv = add_route_ipv4(unit, dip[mid], 0xffffffff, vrfs[mid], fec_id_fwd[mid]);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in function add_route_ipv4_rpf(), \n");
                return rv;
             ...
             * 8.4. Create the Source host entry - same SIP, but different VRF based on RPF type.
             */
            rv = add_route_ipv4_rpf(unit, sip, 0xffffffff, vrfs[mid], ecmp.ecmp_intf);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in function add_route_ipv4_rpf(), \n");
                return rv;
    ...
    else
    {
        ...
        sip++;
        rv = add_route_ipv4_rpf(unit, sip, 0xffffffff, vrfs[0], fec_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_host, dip, vrfs[mid]\n");
            return rv;
        ...
        sip++;
        rv = add_route_ipv4_rpf(unit, sip, 0xffffffff, vrfs[2], ecmp.ecmp_intf - 1);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_host, dip, vrfs[mid]\n");
            return rv;
