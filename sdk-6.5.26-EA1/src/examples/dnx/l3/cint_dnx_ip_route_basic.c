...
int default_vlan_port_id_out[4] = {0, 0, 0, 0};;
    ...
    int out_port)
{
    ...
    rc = bcm_port_match_add(unit, default_vlan_port_id_out[unit], &match_info);
    if (rc != BCM_E_NONE)
    {
    ...
    int kaps_result)
{
    ...
    rv = l3__egress_only_fec__create_inner(unit, fec, intf_out, encap_id, gport, 0, fec_flags2, 0, 0, &encoded_fec);
    if (rv != BCM_E_NONE)
    {
    ...
    int kaps_result)
{
    ...
    rv = l3__egress_only_fec__create_inner(unit, fec, (cint_ip_route_info.is_arp_rif_mode ? 0 : intf_out), encap_id,
                                           gport, 0, fec_flags2, 0, failover_is_primary, &encoded_fec);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC only\n", proc_name);
        return rv;
    ...
    int kaps_result)
{
    ...
    rv = dnx_basic_example_inner(unit, in_port, out_port, 0, kaps_result);
    printf("%s(): Exit\r\n",proc_name);
    return rv;
}
    ...
    int kaps_result)
{
    ...
    rv = l3__egress_only_fec__create_inner(unit, fec, (cint_ip_route_info.is_arp_rif_mode ? 0 : intf_out), encap_id, gport, 0,
                                           fec_flags2, 0, failover_is_primary,&encoded_fec);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC only\n",proc_name);
        return rv;
    ...
    int *fec_id)
{
    ...
    rv = get_fec_range(unit, hierarchy, direction, fec_id, &nof_fecs);
    ...
    int destination)
{
    ...
    if (!*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
    {
        rc = add_route_ipv4(unit, addr, 0xffffffff, vrf, intf);
        return rc; 
    }
    ...
    int intf)
{
    ...
    if (!*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
    {
        rc = add_route_ipv6(unit, addr, full_mask, vrf, intf);
        if (rc != BCM_E_NONE)
        {
            printf("add_route_ipv6 failed: %x \n", rc);
        }
    }
    else
    {
        ...
        int vlan)
{
    ...
    rv = l3__egress_only_encap__create_inner(unit, 0, encap_id, arp_next_hop_mac,
    ...
    int arp_plus_ac_type)
{
     ...
     * 5. Create ARP and set its properties
     */
    rv = create_arp_plus_ac_type(unit, arp_plus_ac_type, arp_next_hop_mac, &encap_id, vsi);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create_arp_plus_ac_type\n", proc_name);
        return rv;
    ...
    int arp_plus_ac_type)
{
     ...
     * 3. Create ETH-RIF and set its properties
     */
    rv = intf_eth_rif_create(unit, intf_in, cint_ip_route_info.intf_in_mac_address);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_eth_rif_create intf_in = %d, err_id = %d\n", proc_name, intf_in, rv);
        return rv;
    }
    rv = intf_eth_rif_create(unit, intf_out, cint_ip_route_info.intf_out_mac_address);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_eth_rif_create intf_out = %d, err_id = %d\n", proc_name, intf_out, rv);
        return rv;
    ...
    rv = l3__egress_only_fec__create(unit, fec, intf_out, encap_id, gport, 0, fec_flags2);
    ...
    int arp_plus_ac_type)
{
    ...
    char *proc_name;
    char *err_proc_name;
    proc_name = "basic_example_ipv6_no_rif";
    l3_ingress_intf ingress_itf;
    l3_ingress_intf_init(&ingress_itf);
    ...
    rv = l3__egress_only_fec__create(unit, fec, 0, encap_id_itf, gport, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf
            ("%s(): Error, create egress object FEC only: fec = %d, intf_out = %d, encap_id = %d, out_port = %d, err_id = %d\n",
     ...
     * 6. Create ARP and set its properties
     */
    rv = l3__egress_only_encap__create(unit, BCM_L3_FLAGS2_VLAN_TRANSLATION, encap_id, arp_next_hop_mac, intf_out);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object ARP only: encap_id = %d, vsi = %d, err_id = %d\n", proc_name,
               encap_id, vsi, rv);
...
get_default_fec_id(
    ...
    int *fec_id)
{
    int rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, fec_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
    }
    ...
    int out_port)
{
    ...
    char *proc_name;
    char *err_proc_name;
    proc_name = "basic_example_ipv6_multidevice";
    l3_ingress_intf ingress_itf;
    l3_ingress_intf_init(&ingress_itf);
    bcm_l3_intf_t l3if;

    rv = get_first_fec_in_range_which_not_in_ecmp_range(ingress_unit, 0, 0, &fec);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
        return rv;
     ...
     * 1. Set In-Port to In ETh-RIF
     */
    rv = in_port_intf_set(ingress_unit, in_port, intf_in);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in_port_intf_set in_port = %d, intf_in = %d, err_id = %d\n", proc_name, in_port, intf_in,
               rv);
     ...
     * 3. Create ETH-RIF and set its properties
     */
    rv = intf_eth_rif_create(ingress_unit, intf_in, intf_in_mac_address);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_eth_rif_create intf_in = %d, err_id = %d\n", proc_name, intf_in, rv);
        return rv;
    }
    rv = intf_eth_rif_create(egress_unit, intf_out, intf_out_mac_address);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_eth_rif_create intf_out = %d, err_id = %d\n", proc_name, intf_out, rv);
        return rv;
     ...
     * 5. Create FEC and set its properties
     */
    rv = l3__egress_only_fec__create(ingress_unit, fec, intf_out, encap_id, out_port, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf
            ("%s(): Error, create egress object FEC only: fec = %d, intf_out = %d, encap_id = %d, out_port = %d, err_id = %d\n",
     ...
     * 6. Create ARP and set its properties
     */
    rv = l3__egress_only_encap__create(egress_unit, 0, encap_id, arp_next_hop_mac, vsi);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object ARP only: encap_id = %d, vsi = %d, err_id = %d\n", proc_name,
               encap_id, vsi, rv);
    ...
    rv = add_route_ipv6(ingress_unit, cint_ip_route_basic_ipv6_route, cint_ip_route_basic_ipv6_mask, vrf, fec);
    err_proc_name = "add_route_ipv6";
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in function %s: vrf = %d, fec = %d, err_id = %d \n", proc_name, err_proc_name, vrf, fec, rv);
     ...
     * 7. Add host entry
     */
    rv = add_host_ipv6(ingress_unit, cint_ip_route_basic_ipv6_host, vrf, fec);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in function add_host_ipv6: vrf = %d, fec = %d, err_id = %d\n", proc_name, vrf, fec, rv);
        return rv;
    ...
    int kaps_result)
{
    ...
    rv = dnx_basic_example_inner(unit, in_port, out_port, arp_plus_ac_type, kaps_result);
    printf("%s(): Exit\r\n", proc_name);
    return rv;
}
