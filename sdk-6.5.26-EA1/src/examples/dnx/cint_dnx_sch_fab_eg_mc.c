...
BCM> cint $SDK/src/examples/dnx/cint_dnx_sch_fab_eg_mc.c
    ...
    int out_sys_port)
{
    ...
    rv = add_ipv4_ipmc(unit, cint_ipmc_route_info.mc_group_ip, 0xFFFFFFFF, cint_ipmc_route_info.host_ip, 0xFFFFFFFF,
                       cint_ipmc_route_info.in_rif, cint_ipmc_route_info.vrf, cint_ipmc_route_info.mc_group, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    int out_sys_port)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, multi_dev_ipmc_init\n");
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, multi_dev_ipmc_l3_intf_configuration\n");
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, sch_fabric_egress_mc_ipmc_forward_entry_configuration\n");
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, multi_dev_ipmc_encapsulation_configuration\n");
        return rv;
    }
    return rv;
}
