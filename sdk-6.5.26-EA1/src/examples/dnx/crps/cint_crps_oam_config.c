    ...
    int core_id)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        rv = crps_oam_database_set(unit, core_id, oam_source[i], command_id[i], nof_engines[i], &engine_array[i][0], counter_base[i], database_id[i], &total_nof_counters_get);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = crps_oam_database_engines_add(unit, core_id, database_id[i], nof_new_engines, &new_engines[i][0], &total_nof_counters_get);
        if (rv != BCM_E_NONE)
        {
