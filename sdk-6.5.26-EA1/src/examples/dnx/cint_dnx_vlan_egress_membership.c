 ...
 * CINT files:
 * cd ../../../../src/examples/dnx/
 * utility/cint_dnx_utils_l2.c
 * utility/cint_dnx_utils_vlan_translate.c
 * cint_dnx_vlan_egress_membership.c
 *
...
int
dnx_vlan_egress_membership_init(int unit, dnx_vlan_egress_membership_s *test_data)
{
    if (test_data != NULL) {
...
dnx_vlan_egress_membership__start_run(int unit, dnx_vlan_egress_membership_s *test_data)
{
    ...
    rv = dnx_vlan_egress_membership_init(unit, test_data);
    if (rv != BCM_E_NONE) {
        printf("Error, dnx_vlan_egress_membership_init, rv - %d\n", rv);
        return rv;
    }

    /* Create VSI */
    rv = bcm_vlan_create(unit, g_dnx_vlan_egress_membership.vsi);
    if (rv != BCM_E_NONE) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        rv = vlan_translate_tag_classification_set(unit, g_dnx_vlan_egress_membership.ports[i], /*tag_format*/ 4, 0x8100, 0xFFFFFFFF);
        if (rv != BCM_E_NONE) {
            printf("Error, vlan_translate_tag_classification_set for port - %d, rv - %d\n", g_dnx_vlan_egress_membership.ports[i], rv);
            return rv;
        }
        ...
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_vlan_port_create for port - %d, vlan - %d, rv - %d\n",
                   g_dnx_vlan_egress_membership.ports[i], g_dnx_vlan_egress_membership.vlan[0], rv);
            return rv;
        }
        rv = bcm_vlan_gport_add(unit, g_dnx_vlan_egress_membership.vlan[0], g_dnx_vlan_egress_membership.ports[i], 0);
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_vlan_gport_add for port - %d, vlan - %d, rv - %d\n",
                   g_dnx_vlan_egress_membership.ports[i], g_dnx_vlan_egress_membership.vlan[0], rv);
            return rv;
        }
        rv = bcm_vlan_gport_add(unit, g_dnx_vlan_egress_membership.vlan[1], g_dnx_vlan_egress_membership.ports[i], 0);
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_vlan_gport_add for port - %d, vlan - %d, rv - %d\n",
                   g_dnx_vlan_egress_membership.ports[i], g_dnx_vlan_egress_membership.vlan[1], rv);
            return rv;
        }
        ...
        rv = bcm_vswitch_port_add(unit, g_dnx_vlan_egress_membership.vsi, vlan_port.vlan_port_id);
        if (rv != BCM_E_NONE) {
            printf("Error in bcm_vswitch_port_add, vsi - %d, vlan_port_id - %d, rv - %d\n",
                   g_dnx_vlan_egress_membership.vsi, g_dnx_vlan_egress_membership.gport[i][0], rv);
            return rv;
        }
        ...
        if (rv != BCM_E_NONE) {
            printf("Error in bcm_vswitch_port_add, vsi - %d, vlan_port_id - %d, rv - %d\n",
                   g_dnx_vlan_egress_membership.vsi, g_dnx_vlan_egress_membership.gport[i][1], rv);
            return rv;
        }
    }
...
dnx_vlan_egress_membership__start_run_with_ports(int unit, int port1, int port2, bcm_vlan_t vsi, bcm_vlan_t vlan1, bcm_vlan_t vlan2)
{
    ...
    rv = dnx_vlan_egress_membership__start_run(unit, &temp_s);
