 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_sand_utils_mpls.c Purpose: utility for VPLS.
 */
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    if (is_egress)
    {
        ...
        mpls_port_egr.flags2 |= BCM_MPLS_PORT2_EGRESS_ONLY | BCM_MPLS_PORT2_ALLOC_SYMMETRIC;
    ...
    if (is_ingress)
    {
        ...
        if (!dnx_utils_is_jr2_system_header_mode(unit) && BCM_L3_ITF_TYPE_IS_FEC(mpls_port->egress_tunnel_if) && (mpls_port->failover_mc_group == 0)) {
...
mpls__add_switch_entry(int unit, mpls__ingress_tunnel_utils_s *mpls_tunnel_properties)
{
    ...
    if ((entry.vpn == 0) && (entry.flags & (BCM_MPLS_SWITCH_NEXT_HEADER_L2 | BCM_MPLS_SWITCH_EVPN_IML))) {
