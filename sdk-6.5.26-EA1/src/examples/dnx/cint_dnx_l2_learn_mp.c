...
int dnx_l2_learn_mp__create_ac(int unit, bcm_vlan_t vsi, ac_info_s *ac_param) {
    ...
    if (ac_param->use_fec)
    {
        ...
        rv = bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY, &l3_egress, &ac_param->fec_id);
        if (rv != BCM_E_NONE) 
        {
            printf("%s(): Error, failed during bcm_l3_egress_create for port %d, rv - %d\n",
    ...
    if (rv != BCM_E_NONE) {
        printf("%s(): Error, failed during bcm_vlan_port_find for port %d, rv - %d\n",
    ...
    if (vlan_port_get.learn_strength != vlan_port.learn_strength)
    {
        ...
        printf("%s(): Error, learn_strength of vlan port creating:%d isn't equel to vlan port getting:%d, rv - %d\n",
               proc_name, vlan_port.flags2, vlan_port_get.flags2, rv);
        return rv;
    }
...
int dnx_l2_learn_mp__cleanup(int unit) {
    ...
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vswitch_port_delete gport: 0x%08x\n", g_dnx_l2_learn_mp.in_ac_1.gport);
        return rv;
    }

    rv = bcm_vswitch_port_delete(unit, g_dnx_l2_learn_mp.vsi, g_dnx_l2_learn_mp.in_ac_2.gport);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vswitch_port_delete gport: 0x%08x\n", g_dnx_l2_learn_mp.in_ac_2.gport);
        return rv;
    }

    rv = bcm_vswitch_port_delete(unit, g_dnx_l2_learn_mp.vsi, g_dnx_l2_learn_mp.out_ac.gport);
    if (rv != BCM_E_NONE) {
    ...
    {
        rv = bcm_l3_egress_destroy(unit, g_dnx_l2_learn_mp.in_ac_1.fec_id);
        if (rv != BCM_E_NONE) 
        {
            printf("%s(): Error, failed to bcm_l3_egress_destroy fec:%d, rv - %d\n",
                   g_dnx_l2_learn_mp.in_ac_1.fec_id, rv);
            return rv;
        }
    }
    ...
    {
        rv = bcm_l3_egress_destroy(unit, g_dnx_l2_learn_mp.in_ac_2.fec_id);
        if (rv != BCM_E_NONE) 
        {
            printf("%s(): Error, failed to bcm_l3_egress_destroy fec:%d, rv - %d\n",
                   g_dnx_l2_learn_mp.in_ac_1.fec_id, rv);
    ...
    {
        rv = bcm_l3_egress_destroy(unit, g_dnx_l2_learn_mp.out_ac.fec_id);
        if (rv != BCM_E_NONE) 
        {
            printf("%s(): Error, failed to bcm_l3_egress_destroy fec:%d, rv - %d\n",
                   g_dnx_l2_learn_mp.in_ac_1.fec_id, rv);
    ...
    rv = bcm_multicast_destroy(unit, g_dnx_l2_learn_mp.vsi);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_multicast_destroy mc: 0x%08x\n", g_dnx_l2_learn_mp.vsi);
        return rv;
    }

    rv = bcm_vswitch_destroy(unit, g_dnx_l2_learn_mp.vsi);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vswitch_destroy mc: vsi%08x\n", g_dnx_l2_learn_mp.vsi);
        return rv;
    }
...
int dnx_l2_vlan_control_default_dest_main(int unit, int port1, int port2, int port3, int port4)
{
    ...
    for (index = 0; index < nof_vsis; index++) {
        ...
        if (rv != BCM_E_NONE) {
            if (!(no_trap_for_unknown_destination && BCM_GPORT_IS_TRAP(gports[index]) && rv == BCM_E_PARAM))
            {
