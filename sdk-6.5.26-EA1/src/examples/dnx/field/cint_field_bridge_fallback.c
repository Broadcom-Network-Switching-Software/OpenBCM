...
int cint_field_bridge_fallback_select_unknown_dest_and_l3_mc_packets_and_vis_profile_update_lif_and_port(int unit)
{
    ...
    bcm_field_context_param_set(unit, 0, bcmFieldStageIngressPMF2, bridge_fallback_context_id, &param_info);
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &bridge_fallback_fg_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
...
int cint_field_bridge_fallback_entry_add(int unit, int nof_cores,uint32 rcy_port_core_0, uint32 rcy_port_core_1, int vsi_profile)
{
    ...
    for (core_id = 0; core_id < nof_cores; core_id ++) {
        ...
        rv = bcm_field_entry_add(unit, 0, bridge_fallback_fg_id, &ent_info, &cint_bridge_fallback_entry_id[core_id]);
        if (rv != BCM_E_NONE)
        {
...
int cint_field_bridge_fallback_entry_destroy(int unit, int nof_cores, int vsi_profile)
{
    ...
    for (core_id = 0;core_id < nof_cores;core_id++) {
        ...
        rv = bcm_field_entry_delete(unit, bridge_fallback_fg_id, NULL, cint_bridge_fallback_entry_id[core_id]);
        if (rv != BCM_E_NONE)
        {
