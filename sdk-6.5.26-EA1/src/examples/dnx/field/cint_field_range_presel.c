    ...
    bcm_field_stage_t stage)
{
    ...
    */
    ...
    rv = bcm_field_context_create(unit, 0, stage, &context_info, &CINT_FIELD_RANGE_PRESEL_CONTEXT_ID);
    if (rv != BCM_E_NONE)
    {
    ...
    bcm_field_stage_t stage)
{
     ...
     * Add the FG.
     */
    rv = bcm_field_group_add(unit, 0, &fg_info, &CINT_FIELD_RANGE_PRESEL_FG_ID);
    if (rv != BCM_E_NONE)
    {
    ...
    int range_configuration)
{
     ...
     */
    rv = cint_field_range_presel_context_create_presel_set(unit, stage);
    if (rv != BCM_E_NONE)
    {
