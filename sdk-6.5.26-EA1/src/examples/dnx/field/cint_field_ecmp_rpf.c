 ...
 */
    ...
    int out_port)
 {
     ...
     */
    rv = l3__egress_only_fec__create(unit, fec_fwd, cint_field_ecmp_rpf_pre_pmf_info.intf_out, cint_field_ecmp_rpf_pre_pmf_info.encap_id_fwd, gport_out, 0,0);
    if (rv != BCM_E_NONE)
    {
 ...
 */
 int field_ecmp_rpf_pmf(int unit, bcm_field_context_t context_id, uint8 is_mc)
 {
     ...
     rv = bcm_field_group_add(unit, 0, &fg_info, &cint_field_ecmp_rpf_pmf_info.ecmp_lookup_fg_id);
     if(rv != BCM_E_NONE)
     {
     ...
     rv = bcm_field_group_add(unit, 0, &fg_info, &cint_field_ecmp_rpf_pmf_info.trap_fg_id);
     if(rv != BCM_E_NONE)
     {
         printf("Error (%d) in bcm_field_group_add\n", rv);
         return rv;
