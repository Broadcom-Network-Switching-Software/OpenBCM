    ...
    int num_of_qualifiers)
{
    ...
    while (fg_config_iter < Cint_field_data_qualifier_create_nof_fg)
    {
        ...
        for (fg_iter = 0; fg_iter < Cint_field_data_qualifier_create_nof_fg / 2; fg_iter++)
        {
            ...
            {
                rv = bcm_field_group_add(unit, 0, &fg_info, &Cint_field_data_qualifier_create_fg_ids[fg_config_iter + fg_iter]);
                if (rv != BCM_E_NONE)
                {
            ...
            {
                rv = bcm_field_group_add(unit, 0, &fg_info, &Cint_field_data_qualifier_create_fg_ids[fg_iter]);
                if (rv != BCM_E_NONE)
                {
