 ...
 */
bcm_field_context_t Cint_field_fes_valid_bit_context_id;
/**
 * The number of FGs
 */
    ...
    int unit)
{
    ...
    */
    ...
    rv = bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF1, &context_info, &Cint_field_fes_valid_bit_context_id);
    if (rv != BCM_E_NONE)
    {
    ...
    cint_field_fes_valid_bit_fg_info_t * fes_fg_info_p)
{
     ...
     * Add the FG.
     */
    rv = bcm_field_group_add(unit, 0, &fg_info, &fes_fg_info_p->fg_id);
    if (rv != BCM_E_NONE)
    {
