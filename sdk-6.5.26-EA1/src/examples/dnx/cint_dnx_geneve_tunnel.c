    ...
    int virtual_machine_port)
{
     ...
     */
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, &cint_geneve_tunnel_term_info.fec_id);
    if (rv != BCM_E_NONE)
    {
    ...
    int  provider_port)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, &cint_geneve_tunnel_init_info.tunnel_fec_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
        return rv;
    ...
    int vm_port)
{
    ...
    rv = l3__egress_only_fec__create(unit, cint_geneve_tunnel_init_info.tunnel_fec_id,
                                     tunnel_out_rif_intf, 0, core_port, 0);
    if (rv != BCM_E_NONE)
    {
