    ...
    uint32 flags)
{
    ...
    /** Input argument validation: This cint is relevant only for NIF ports (excluding ELK and STIF) and flexe clients */
    port_is_nif_dynamic = is_port_nif_dynamic (unit, port, interface_info.interface, flags_get);
    ...
    {
        printf("Error, cint_dyn_port_remove_port_full_example supports only NIF ports (excluding ELK / STIF), FlexE clients and Recycle ports. port=%d\n", port);
        return BCM_E_PORT;
    }
    ...
    if (!port_is_rcy_dynamic)
    {
        ...
        if (is_last_in_if)
        {
            ...
            if (pp_tm_context_en)
            {
                rv  = bcm_mirror_port_to_rcy_port_map_get(unit, flags_mirror_port, port, &rcy_map_info);
                if (rv != BCM_E_NONE)
                {
                    printf("Error in bcm_mirror_port_to_rcy_port_map_get: unit=%d, port=%d\n", unit, port);
                    return rv;
                }
                if (rcy_map_info.rcy_mirror_port != BCM_PORT_INVALID)
                {
                    rv = bcm_port_remove(unit, rcy_map_info.rcy_mirror_port, 0);
                    if (rv != BCM_E_NONE)
                    {
                        printf("Error, in bcm_port_remove for the mapped rcy_mirror port=%d of forward port %d\n",
    ...
    if (!(flags_get & BCM_PORT_ADD_FLEXE_PHY) && !(flags_get & BCM_PORT_ADD_CROSS_CONNECT) &&
        !(flags_get & BCM_PORT_ADD_FLEXE_SAR) && (interface_info.interface != BCM_PORT_IF_FLEXE_CLIENT) &&
    ...
    {    /*
         ...
         */
        if (*(dnxc_data_get(unit, "egr_queuing_v1", "general", "v1_supported", NULL)) == 0)
        {
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_port_remove, unit=%d, port=%d\n", unit, port);
        return rv;
    }
    return BCM_E_NONE;
}
    ...
    uint32 flags)
{
    ...
    rv = appl_dnx_sys_device_params_get(unit, &sys_params);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in appl_dnx_sys_device_params_get\n");
        return rv;
    }
    ...
    int pp_set_header_only)
{
    ...
    for (port_index = 0; port_index < nof_ports; ++port_index)
    {
        ...
        if (!port_is_local)
        {
         ...
         if (!port_is_nif_dynamic && !port_is_flexe_dynamic && !port_is_rcy_dynamic)
         {
             printf("Error, cint_dyn_port_add_ports_full_example supports only NIF ports (excluding ELK / STIF), FlexE clients and Recycle ports. port=%d\n", port_list[port_index]);
             return BCM_E_PORT;
         }
         ...
         * For FlexE clients, no need to set the resource
         */
        if (interface_has_single_channel && (interface_info[port_index].interface != BCM_PORT_IF_FLEXE_CLIENT))
        {
            ...
            if ((interface_info[port_index].interface != BCM_PORT_IF_VIRTUAL_FLEXE_CLIENT) && (interface_info[port_index].interface != BCM_PORT_IF_RCY))
            {
                if (*(dnxc_data_get(unit, "nif", "global", "not_implemented", NULL)) == 0)
                {
    ...
    for (port_index = 0; port_index < nof_ports; ++port_index)
    {
        /** check if port is local to this device */
        port_is_local = (sys_params->unit_to_device_index[unit] == device_id_list[port_index]);
        /** Check if SAR is tdm port */
        sar_is_non_tdm_port = 0;
        if (flags[port_index] & BCM_PORT_ADD_FLEXE_SAR)
        {
            sar_is_non_tdm_port = !(*(dnxc_data_get(unit, "nif", "sar_client", "is_tdm_port", NULL)));
        }
        if (!(flags[port_index] & BCM_PORT_ADD_FLEXE_PHY) && !(flags[port_index] & BCM_PORT_ADD_CROSS_CONNECT) &&
            (interface_info[port_index].interface != BCM_PORT_IF_FLEXE_CLIENT) &&
            (interface_info[port_index].interface != BCM_PORT_IF_OTN) && !sar_is_non_tdm_port)
        {
            if (port_is_local)
            {
                /** stage 3 - configure egr and sch shapers */
                switch (interface_info[port_index].interface)
                {
                    ...
                    case BCM_PORT_IF_ILKN:
                        rv = bcm_port_control_get(unit, port_list[port_index], bcmPortControlDataRate, &ilkn_data_rate);
                        if (rv != BCM_E_NONE)
                        {
                            printf("Error, bcm_port_control_get, unit=%d, port=%d\n", unit, port_list[port_index]);
                            return rv;
                        }
                ...
                if (!(flags[port_index] & BCM_PORT_ADD_FLEXE_SAR))
                {
                    ...
                    rv = cint_dyn_port_add_packet_processing_properties_flexible_set(unit, port_list[port_index], header_type_in[port_index],
                                                                                              header_type_out[port_index], pp_set_header_only);
                    if (rv != BCM_E_NONE)
                    {
                        printf("Error, bcm_dnx_port_enable_set: unit=%d, port=%d\n", unit, port_list[port_index]);
                        return rv;
            ...
            rv = appl_sand_device_to_sysport_convert(unit, sys_port_map.device_index, APPL_DNX_NOF_SYSPORTS_PER_DEVICE,
                                                                                sys_port_map.local_port, &sys_port_map.system_port_id);
            if (rv != BCM_E_NONE)
            {
                printf("Error, appl_sand_device_to_sysport_convert\n");
                return rv;
            }
            /** Port might not be local to this FAP */
            appl_flags = APPL_DNX_DYN_PORT_USE_SYS_PORT_MAP;
            rv = appl_dnx_dynamic_port_create(unit, appl_flags, &sys_port_map, port_list[port_index]);
            if (rv != BCM_E_NONE)
            {
                printf("Error, bcm_dnx_port_enable_set: unit=%d, port=%d\n", unit, port_list[port_index]);
                return rv;
        ...
        else
        {
            ...
            {
                rv = bcm_linkscan_mode_set(unit, port_list[port_index], BCM_LINKSCAN_MODE_SW);
                if (rv != BCM_E_NONE)
                {
        ...
        if(port_is_local)
        {
            ...
            if ((interface_has_single_channel) && (interface_info[port_index].interface != BCM_PORT_IF_RCY))
            {
                if (*(dnxc_data_get(unit, "nif", "global", "not_implemented", NULL)) == 0)
                {
                    rv = bcm_port_enable_set(unit,  port_list[port_index], enable_port_at_last);
                    if (rv != BCM_E_NONE)
                    {
                        printf("Error, bcm_dnx_port_enable_set: unit=%d, port=%d\n", unit, port_list[port_index]);
                        return rv;
    ...
    uint32 flags)
{
    ...
    if ((flags & BCM_PORT_ADD_FLEXE_MAC) && !(flags & BCM_PORT_ADD_CROSS_CONNECT))
    {
        mapping_info.tm_port = port;
        mapping_info.num_priorities = 2;
     ...
     */
    rv = cint_dyn_port_add_port_full_example(unit, port, mapping_info, interface_info, resource,
                                             0, 0, header_type_in, header_type_out, flags);
    if (rv != BCM_E_NONE)
    {
        printf("Error, cint_dyn_port_add_port_full_example, unit: %d, logical_port: %d\n", unit, port);
        return rv;
...
)
{
    ...
    BCM_PBMP_ITER(pbmp, logical_port) {
        ...
        rv = bcm_port_get(unit, logical_port, &flags, &interface_info, &mapping_info);
        if (BCM_FAILURE(rv)) {
            printf("bcm_port_get %d: failed. Error:%d (%s) \n", logical_port, rv, bcm_errmsg(rv));
            return rv;
        }
        switch (free_resource_type_requested) {
            case logical_ports:
