    ...
    int egress_unit)
{
    ...
    rv = sand_utils_vxlan_open_vpn(egress_unit, cint_vxlan_roo_mc_info.pe2_native_eth_rif, 0, cint_vxlan_roo_mc_info.pe2_vni_id);
    if (rv != BCM_E_NONE)
    {
    ...
    /*   
     * 8. update tunnel init orientation. It's not done by vxlan_port_add since we configure it with tunnel term lif and FEC.
     */
    rv = bcm_port_class_set(ingress_unit, cint_vxlan_roo_mc_info.pe1_vxlan_tunnel_id, bcmPortClassForwardEgress, cint_vxlan_roo_mc_info.vxlan_network_group_id);
    if (rv != BCM_E_NONE) 
    {
    ...
    rv = bcm_port_class_set(egress_unit, cint_vxlan_roo_mc_info.pe2_vxlan_tunnel_id, bcmPortClassForwardEgress, cint_vxlan_roo_mc_info.vxlan_network_group_id);
    if (rv != BCM_E_NONE) 
    {
        printf("%s(): Error, bcm_port_class_set \n", proc_name);
        return rv;
    ...
    /* 
     * 9. Add VXLAN port properties and configure the tunnel terminator In-Lif and tunnel initiatior Out-Lif
     */
    rv = multi_dev_vxlan_roo_vxlan_port_add(ingress_unit, cint_vxlan_roo_mc_info.pe1_port, cint_vxlan_roo_mc_info.vxlan_tunnel_if, &vxlan_port_id);
    if (rv != BCM_E_NONE) 
    {
    ...
    rv = multi_dev_vxlan_roo_vxlan_port_add(egress_unit, cint_vxlan_roo_mc_info.pe2_port, cint_vxlan_roo_mc_info.pe2_vxlan_tunnel_if, &vxlan_port_id);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, vxlan_port_add \n", proc_name);
        return rv;
    ...
    int pe2_sys_port)
{
    ...
    if (is_device_or_above(ingress_unit, JERICHO2))
    {
        rv = bcm_port_class_set(ingress_unit,g_vxlan_roo_mc.recycle_port,bcmPortClassFieldIngressPMF3TrafficManagementPortCs,4);
        if (rv != BCM_E_NONE)
        {
