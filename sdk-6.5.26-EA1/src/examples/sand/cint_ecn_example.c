...
int ecn_basic_example(int unit, int in_port, int out_port){
    ...
    rv = ecn_extract_example(unit,0,0);
    if (rv != BCM_E_NONE) {
...
/* 
 * Enable/disable ECN on ETPP (per device)
 */
int ecn_etpp_enable(int unit, int ecn_enable)
{
    int rv;
    uint32 flags;
...
/* 
 * ECN latency profile config
 */
int ecn_latency_profile_set(int unit, int profile_id, int ecn_enable, int threshold)
{
    int rv;
    uint32 flags = BCM_COSQ_LATENCY_END_TO_END | BCM_COSQ_LATENCY_PROFILE_WITH_ID;

    /* Create latency profile */
    rv = bcm_cosq_latency_profile_create(unit, flags, &profile_id);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_cosq_latency_profile_create\n");
        return rv;
    }

    /* Enable ECN on latency profile */
    rv = bcm_cosq_control_set(unit, profile_id, 0, bcmCosqControlLatencyEcnEnable, ecn_enable);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_cosq_control_set: bcmCosqControlLatencyEcnEnable\n");
        return rv;
    }

    /* Set latency profile ECN threshold */
    rv = bcm_cosq_control_set(unit, profile_id, 0, bcmCosqControlLatencyEcnThreshold, threshold);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_cosq_control_set: bcmCosqControlLatencyEcnThreshold\n");
        return rv;
    }
...
/* 
 * Enable ECN on the device and out port, and set extraction of ECN (capable and congestion bits)
 * Add IPV4 route
 * Disable queue based ECN marking
 * Only run on Jericho_plus
 */
int latency_ecn_basic_example(int unit, int in_port, int out_port, int flags, int latency_flow_id){
    ...
    /* Set the rules to extract the ECN value (2 bits) */
    rv = ecn_extract_example(unit, flags, latency_flow_id);
    if (rv != BCM_E_NONE) {
        printf("Error, ecn_extract_example\n");
        return rv;
    }

    /* Enable/disable ECN on out port */
    rv = bcm_port_control_set(unit, out_port, bcmPortControlLatencyEcnEnable, 1);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_port_control_set type: bcmPortControlLatencyEcnEnable\n");
        return rv;
    }
    ...
    BCM_GPORT_UNICAST_QUEUE_GROUP_SET(base_q, 24 + (out_port * 8));
    /* in bytes */
    ecn_config.flags = BCM_COSQ_DISCARD_MARK_CONGESTION | BCM_COSQ_DISCARD_BYTES;
    ecn_config.ecn_thresh = 0x200000;
    rv = bcm_cosq_gport_discard_set(unit, base_q, cosq, &ecn_config);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_cosq_gport_discard_set with ecn_thresh %d\n", ecn_config.ecn_thresh);
        return rv;
    ...
    /* in BDs */
    ecn_config.flags = BCM_COSQ_DISCARD_MARK_CONGESTION | BCM_COSQ_DISCARD_BUFFER_DESC;
    ecn_config.ecn_thresh = 0x100000;
    rv = bcm_cosq_gport_discard_set(unit, base_q, cosq, &ecn_config);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_cosq_gport_discard_set with ecn_thresh %d\n", ecn_config.ecn_thresh);
        return rv;
    ...
    /* Add IPV4 route and send to out_port */
    rv = basic_example(&unit, 1, in_port, out_port);
    if (rv != BCM_E_NONE) {
        printf("Error, basic_example\n");
        return rv;
    }
