...
int l2_learning_info_update_check(int unit, int port)
{
    int rv;
    int vid = 13;
    bcm_vlan_t vsi = 2000;
    bcm_mac_t smac={0x66,0x77,0x88,0x99,0xaa,0xbb};
    bcm_mac_t dmac={0x00,0x11,0x22,0x33,0x44,0x55};
    ...
    sprintf(cmd, "tx 1 psrc=%d data=0x00112233445566778899aabb8100000d08004500002e0000000040ff7bb17fffff1d7fffff03", port);
    bshell(unit, cmd);

    rv = bcm_l2_addr_get(unit, smac, vsi, &l2addr);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_l2_addr_get for step 1\n");
        return rv;
    }
    if (BCM_GPORT_IS_MODPORT(l2addr.port)) {
        if (BCM_GPORT_MODPORT_PORT_GET(l2addr.port) != port) {
            printf("Error, learning port[%#x] is not correct!\n", l2addr.port);
            return -1;
        }
    } else {
        printf("Error, learning port type is not local for step 1!\n");
        return -1;
    }
    ...
    l3_egress.port = port; 
    rv = bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY, &l3_egress, &fec);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_l3_egress_create for step 2\n");
        return rv;
    }
    vp.flags |= BCM_VLAN_PORT_REPLACE | BCM_VLAN_PORT_WITH_ID;
    BCM_GPORT_FORWARD_PORT_SET(vp.port, fec);
    rv = bcm_vlan_port_create(unit, &vp);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vlan_port_create for step 2\n");
        return rv;
    }
    ...
    if (BCM_GPORT_IS_FORWARD_PORT(l2addr.port)) {
        if (BCM_GPORT_FORWARD_PORT_GET(l2addr.port) != BCM_L3_ITF_VAL_GET(fec)) {
            printf("Error, learning port[%#x] is not correct!\n", l2addr.port);
            return -1;
        }
    } else {
        printf("Error, learning port type is not FEC!\n");
        return -1;
    }
    ...
    rv = bcm_vlan_port_create(unit, &vp);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vlan_port_create for step 3\n");
        return rv;
    }
    ...
    if (BCM_GPORT_IS_MODPORT(l2addr.port)) {
        ...
        printf("Error, learning port type is not local for step 3!\n");
        return -1;
    }
