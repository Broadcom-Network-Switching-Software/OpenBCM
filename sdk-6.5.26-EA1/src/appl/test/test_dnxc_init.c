...
int linkscan_interval[SOC_MAX_NUM_DEVICES] = {0};
...
test_dnxc_init_test(int unit, args_t *a, void *p)
{
    ...
    for (i = 0; i < test_params->repeat; i++)
    {
        ...
        if (test_params->appl_init_params.warmboot) {
            if(dnxc_sw_state_is_warmboot_supported_get(unit) == FALSE) {
                ...
                rv = bcm_linkscan_enable_get(unit, &linkscan_interval[unit]);
                ...
                if (!test_params->appl_init_params.no_dump) {
                    ...
                    sal_strncat(pre_reboot_file_name, "/sw_state_pre_reboot", sizeof(pre_reboot_file_name) - sal_strnlen(pre_reboot_file_name, sizeof(pre_reboot_file_name)) - 1);
                    sal_strncat(post_reboot_file_name, "/sw_state_post_reboot", sizeof(post_reboot_file_name) - sal_strnlen(post_reboot_file_name, sizeof(post_reboot_file_name)) - 1);
                    timestamp = sal_time();
                    ...
                    sal_strncat(bshell_command, pre_reboot_file_name, sizeof(bshell_command) - sal_strnlen(bshell_command, sizeof(bshell_command)) - 1);
    ...
    test_params->appl_init_params.loop_id = i;
        ...
        if (test_params->appl_init_params.warmboot)
        {
            ...
            /*
             * Re-enabling linkscan with the original interval from before WB, due to the linkscan application not having WB support.
             */
            ret_val = bcm_linkscan_enable_set(unit, linkscan_interval[unit]);
            if (ret_val != 0)
            {
                LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Linkscan interval was restored unsuccessfully\n")));
                return CMD_FAIL;       /** fail */
            }
        }
        ...
        if (test_params->appl_init_params.warmboot && !test_params->appl_init_params.no_dump) {
            ...
            sal_strncpy(bshell_command, "swstate config dump file_path=", sizeof(bshell_command) - 1);
            sal_strncat(bshell_command, post_reboot_file_name, sizeof(bshell_command) - sal_strnlen(bshell_command, sizeof(bshell_command)) - 1);
            sh_process_command(unit, bshell_command);
            ...
            sal_strncpy(diff_command, "diff -q ", sizeof(diff_command));
            strncat(diff_command, pre_reboot_file_name, sizeof(diff_command) - sal_strnlen(diff_command,sizeof(diff_command)) - 1);
            strncat(diff_command, " ", sizeof(diff_command) - 1);
            strncat(diff_command, post_reboot_file_name, sizeof(diff_command) - sal_strnlen(diff_command,sizeof(diff_command)) - 1);
            ...
            if (rv != 0)
            {
                ...
                sal_strncpy(diff_command, "diff -u ", sizeof(diff_command));
                strncat(diff_command, pre_reboot_file_name, sizeof(diff_command) - sal_strnlen(diff_command, sizeof(diff_command)) - 1);
                strncat(diff_command, " ", sizeof(diff_command) - 1);
                strncat(diff_command, post_reboot_file_name, sizeof(diff_command) - sal_strnlen(diff_command, sizeof(diff_command)) - 1);
                system(diff_command);
            ...
            if(!test_params->appl_init_params.no_clean_up)
            {
                ...
                sal_strncpy(clean_command, "rm ", sizeof(clean_command));
                strncat(clean_command, pre_reboot_file_name, sizeof(clean_command) - sal_strnlen(clean_command, sizeof(clean_command)) - 1);
                system(clean_command);
                /*delete post-reboot.txt*/
                sal_memset(clean_command, 0, sizeof(clean_command));
                sal_strncpy(clean_command, "rm ", sizeof(clean_command));
                strncat(clean_command, " ", sizeof(clean_command) - 1);
                strncat(clean_command, post_reboot_file_name, sizeof(clean_command) - sal_strnlen(clean_command, sizeof(clean_command)) - 1);
                system(clean_command);
            }
