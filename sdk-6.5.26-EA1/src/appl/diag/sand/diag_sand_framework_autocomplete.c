...
void
sh_sand_cmd_autocomplete_enum_init_recursive(
    int unit,
    autocomplete_node_t * ac_node,
    sh_sand_enum_t * enum_struct)
{
    for (; enum_struct != NULL && (enum_struct->string != NULL || enum_struct->inherited_enum != NULL); enum_struct++)
    {
        char enum_text[AUTOCOMPLETE_MAX_STRING_LEN] = { 0 };

        /*
         * recursively process all inherited enums as siblings of current enum's siblings
         */
        if (enum_struct->inherited_enum != NULL)
        {
            sh_sand_cmd_autocomplete_enum_init_recursive(unit, ac_node, enum_struct->inherited_enum);
        }

        if (enum_struct->string)
        {
    ...
    for (; option != NULL && (option->keyword != NULL || option->inherited_options != NULL); option++)
    {
        ...
        if (option->keyword)
        {
            ...
            if (option->type == SAL_FIELD_TYPE_ENUM && option->ext_ptr)
            {
                ...
                sh_sand_cmd_autocomplete_enum_init_recursive(unit, enum_ac_node, (sh_sand_enum_t *) option->ext_ptr);
            }
            else
            {
...
void
sh_sand_cmd_autocomplete_init_root(
    int unit,
    sh_sand_cmd_t * root_cmds)
{
    sh_sand_cmd_autocomplete_init(unit, NULL, root_cmds);
    ...
    sh_sand_cmd_t * sh_sand_cmd_a)
{
    ...
    /*
     * If the current node is a root node and has the skip root flag, initialize each of its children as if they were at the same level.
     */
    if (parent == NULL && utilex_is_prime_flag_set(sh_sand_cmd_a->flags, SH_CMD_SKIP_ROOT))
    {
        sh_sand_cmd_autocomplete_init(unit, NULL, sh_sand_cmd_a->child_cmd_a);
    }
...
sh_sand_cmd_autocomplete_deinit(
    ...
    sh_sand_cmd_t * sh_sand_cmd_a)
{
    int flags = SH_CMD_TRAVERSE_NO_CB;
    sh_sand_cmd_t *sh_sand_cmd;
