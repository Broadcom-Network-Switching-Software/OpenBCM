...
static test_run_memory_stat_t test_run_memory_stat;
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        switch (mem)
        {
            ...
            case OAMP_RMEP_DBm:
                ...
                /*
                 * J2P memories under investigation 
                 */
        ...
        {
            if (SOC_MEM_BLOCK_MIN(unit, mem) == MDB_BLOCK(unit))
            {
                return 1;
            }
        }
    ...
    args_t * a)
{
    ...
    {
        /*
         * enable macsec blocks 
         */
        READ_MSW_MACSEC_CFGr(unit, &val);
        soc_reg_field32_modify(unit, MSW_MACSEC_CFGr, REG_PORT_ANY, MACSEC_DISABLE_STRAPf, 0);
    }
    ...
    if (SOC_IS_J2X(unit))
    {
         ...
         * enable macsec blocks 
         */
        WRITE_MSW_MACSEC_CFGr(unit, val);
    }
    return rv;
}
#endif /* dpp support */
...
#if defined(PLISIM) && defined(BCM_DNX_SUPPORT)
        if (SOC_MEM_TYPE(unit, mem) == SOC_MEM_TYPE_CUSTOM)
        {
            LOG_VERBOSE(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Skipping custom memory %s\n"), SOC_MEM_NAME(unit, mem)));
            return rv;
        }
#endif
    ...
    soc_mem_t mem)
{
    ...
    sal_memset(f_copy, 0, strlen(first) + 1);
    ...
    while ((s = ARG_GET(a)) != 0)
    {
        ...
        sal_memset(s_copy, 0, strlen(s) + 1);
        sal_strcpy(s_copy, s);
    ...
    int incr)
{
    ...
    while ((fmod = sal_strtok_r(mod, ",", &tokstr)) != 0)
    {
        ...
        if (!sal_strcasecmp(fmod, "clear"))
        {
            ...
            if (mask)
            {
                sal_memset(mask, 0xff, entry_dw * sizeof(*entry));
            }
            continue;
        }
        for (fld = &m->fields[0]; fld < &m->fields[m->nFields]; fld++)
...
/*
 * For the new access - List memory information, or fields of a memory entry
 */
char cmd_access_sand_mem_list_usage[] =
    ...
    "If MEMORY is cache-table, displays all cached memories.\n\t"
    "If MEMORY is ser-action, displays all SER action memories.\n\t"
    "If MEMORY is dynamic-table, displays all dynamic memories.\n\t"
    "If MEMORY is read-write, displays all read and write memories.\n\t"
    "If MEMORY is init-on-read, displays all init on read memories.\n\t"
    "If MEMORY is clear-bits-on-write, displays all clear bits on write memories.\n\t"
    "If MEMORY is counter, displays all counter memories.\n\t"
    "If MEMORY is alias, displays all alias memories.\n\t"
    "If MEMORY is type-1st-bar, displays all type-1st-bar memories.\n\t"
    "If MEMORY is type-2nd-bar, displays all type-2nd-bar memories.\n\t"
    "If MEMORY is wide, displays all wide memories.\n\t"
    "If MEMORY is access-per-port, displays all access-per-port memories.\n\t"
    "If MEMORY is no-protection, displays all no-protection memories.\n\t"
    "If MEMORY is has-ecc, displays all has-ecc memories.\n\t"
    "If MEMORY is has-parity, displays all has-parity memories.\n\t"
    ...
    args_t * a)
{
    ...
    if (SOC_IS_NEW_ACCESS_INITIALIZED(unit))
    {
        access_runtime_info_t *runtime_info = SOC_CONTROL(unit)->device_access_runtime_info;    /* Access information
                                                                                                 * for the device */
        return access_cmd_dnxc_mem_list(runtime_info, a);
    }
#endif /* BCM_ACCESS_SUPPORT */
    ...
    unsigned operation)         /* operation should be 1 for read and 0 for write */
{
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk)
    {
        ...
        {       /* read operation */
            ...
            sal_memcpy(entry_data, above_64_val, (indirect_size / 8));
            ...
            sal_memcpy(((uint32 *) entry_data) + (indirect_size / 32), above_64_val, (indirect_size / 8));
        }
...
char cmd_sand_mem_get_usage[] = "Usages:\n\t"
    ...
    "DUMP [options]\n"
#else
    "DUMP [File=<name>] [Append=true|false] [raw] [hex] [all] [chg] [disable_cache] [force]\n\t"
    ...
    args_t * a)
{
    ...
    for (;;)
    {
        ...
        else if (arg1 != NULL && !sal_strcasecmp(arg1, "force"))
        {
            force = 1;
            arg1 = ARG_GET(a);
        }
    ...
    if (parse_memory_name(unit, &mem, arg1, &copyno, &array_index) >= 0)
    {
        ...
        if (soc_mem_is_writeonly(unit, mem) && !force)
        {
            LOG_INFO(BSL_LS_APPL_COMMON,
    ...
    int mod)
{
    ...
    if (tab != NULL && sal_strcasecmp(tab, "force") == 0)
    {
        force = 1;
        tab = ARG_GET(a);
    }
    ...
    if (mod == 1 && soc_mem_is_writeonly(unit, mem) && !force)
    {
        LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "ERROR: Table %s is write-only\n"), SOC_MEM_UFNAME(unit, mem)));
        goto done;
    }

    if (soc_mem_is_readonly(unit, mem) && !force)
    {
    ...
    args_t * a)
{
    ...
    /** Initialization for macsec access */
    if (SOC_IS_J2P(unit))
    {
        READ_MSD_MACSEC_CLK_POWER_DOWNr(unit, &reg_val);
        READ_MSS_MACSEC_CLK_POWER_DOWNr(unit, &reg_val);
        WRITE_MSD_MACSEC_CLK_POWER_DOWNr(unit, 0x0);
        WRITE_MSS_MACSEC_CLK_POWER_DOWNr(unit, 0x0);
    }
    else if (SOC_IS_J2X(unit))
    {
        READ_MSW_MACSEC_CFGr(unit, &reg_val);
        soc_reg_field32_modify(unit, MSW_MACSEC_CFGr, REG_PORT_ANY, MACSEC_DISABLE_STRAPf, 0);
    }
    ...
    LOG_INFO(BSL_LS_APPL_TESTS,
             (BSL_META_U(unit, "Number of Tested memories: %d (Skipped %d)\n"), test_run_memory_stat.mem_test_num,
    ...
    /** Macsec access back to normal */
    if (SOC_IS_J2P(unit))
    {
        WRITE_MSD_MACSEC_CLK_POWER_DOWNr(unit, reg_val);
        WRITE_MSS_MACSEC_CLK_POWER_DOWNr(unit, reg_val);
    }
    else if (SOC_IS_J2X(unit))
    {
        WRITE_MSW_MACSEC_CFGr(unit, reg_val);
    }
    ...
    void *data)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        switch (mem)
        {
            ...
            case FLEXMAC_TINY_MAC_REGISTERSm:
                /*
                 * HBMC_HBM_PHY_REGISTER_ACCESS, HBM phy register access 
                 */
            case HBMC_HBM_PHY_REGISTER_ACCESSm:
                LOG_VERBOSE(BSL_LS_APPL_TESTS,
                            (BSL_META_U(unit, "Skipping excluded memory %s\n"), SOC_MEM_NAME(unit, mem)));
...
#endif
    test_run_memory_stat.mem_test_num++;
    LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "test memory %s\n"), SOC_MEM_NAME(unit, mem)));
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, copyno)
    {
                ...
                || SOC_BLOCK_TYPE(unit, SOC_MEM_BLOCK_MIN(unit, mem)) == SOC_BLK_CDPORT))
        {
            ...
            int cd_block = 0;
            /*
             * Get NIF logical port bitmap 
             */
            if (dnx_algo_port_logicals_get(unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF, 0, &valid_ports) < 0)
            {
                break;
            }
        ...
        /*
         * count_copyno holds the number all instances for the current memory (mem) 
         */
        count_copyno++;
    }
    /*
     * at the end of our loop (SOC_MEM_BLOCK_ITER) we check if the number of pass and fail memories is equal to 0 and
     * if no, we subtract count_copyno from the number and then add 1 to ensure that there is no negative values. All
     * this is required because at the statistic we want to print only the number of pass and fail memories, not for
     * their all instances. 
     */
    test_run_memory_stat.mem_pass_num = test_run_memory_stat.mem_pass_num == 0 ? test_run_memory_stat.mem_pass_num
        : (test_run_memory_stat.mem_pass_num - count_copyno) + 1;
    test_run_memory_stat.mem_fail_num = test_run_memory_stat.mem_fail_num == 0 ? test_run_memory_stat.mem_fail_num
        : (test_run_memory_stat.mem_fail_num - count_copyno) + 1;
