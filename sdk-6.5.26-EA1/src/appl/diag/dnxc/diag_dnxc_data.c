...
shr_error_e diag_dnxc_dump_autocomplete_cb(
    int unit,
    char *dump_substring,
    char *dump_full_list,
...
static sh_sand_option_t dnxc_data_dump_options[] = {
    ...
    {"name",     SAL_FIELD_TYPE_STR,  "[<module_name>[.<submodule_name>[.<data_name>]]]", "", NULL, NULL, SH_SAND_ARG_FREE, .ac_option_cb=diag_dnxc_dump_autocomplete_cb},
    {NULL}
};
/* *INDENT-ON* */
...
static sh_sand_man_t dnxc_data_dump_man = {
    ...
    .full     = "Prints DNX data modules/submodules/data names, types and values.\nNote that dumping modules, submodules and data can be done with or without prefacing with 'name=',\n but autocomplete is supported only if prefaced with 'name='",
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    if (module != NULL && !sal_strncasecmp("*", module, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        submodule = "*";
        data = "*";
    }
    if (submodule != NULL && !sal_strncasecmp("*", submodule, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        data = "*";
    }
...
/**
 *  The method receives a module, submodule and field names and checks whether the field is exist in the DNX Data block or not.
 *  In case the module is exist, the module contains the associated submodule and inside has a field with the given name,
 *  The method returns 1. at any other scenario, the method returns 0. 
 **/
int
diag_dnxc_data_is_field_exist(
    int unit,
    char *module,
    char *submodule,
    char *data)
{
    dnxc_data_module_t *c_module;
    dnxc_data_submodule_t *c_submodule;
    int module_index, submodule_index, field_index;
    for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
    {
        /*
         * Make sure the given module name is exist 
         */
        c_module = &_dnxc_data[unit].modules[module_index];
        if (c_module->name == NULL || sal_strncasecmp(c_module->name, module, DNXC_DATA_UTILS_STRING_MAX))
        {
            continue;
        }

        for (submodule_index = 0; submodule_index < c_module->nof_submodules; submodule_index++)
        {
            /*
             * Make sure the module contains a submodule with the given submodule name 
             */
            c_submodule = &c_module->submodules[submodule_index];
            if (c_submodule->name == NULL || sal_strncasecmp(c_submodule->name, submodule, DNXC_DATA_UTILS_STRING_MAX))
            {
                continue;
            }

            /*
             * Checking if a field of type define\numeric with the given field name is exist inside the submodule 
             */
            for (field_index = 0; field_index < c_submodule->nof_defines; field_index++)
            {
                if (c_submodule->defines[field_index].name != NULL
                    && !sal_strncasecmp(c_submodule->defines[field_index].name, data, DNXC_DATA_UTILS_STRING_MAX))
                {
                    return 1;
                }
            }
            ...
            /*
             * Checking if a feature with the given field name is exist inside the submodule 
             */
            for (field_index = 0; field_index < c_submodule->nof_features; field_index++)
            {
                if (c_submodule->features[field_index].name != NULL
                    && !sal_strncasecmp(c_submodule->features[field_index].name, data, DNXC_DATA_UTILS_STRING_MAX))
                {
                    return 1;
                }
            }
            ...
            /*
             * Checking if a table with the given field name is exist inside the submodule 
             */
            for (field_index = 0; field_index < c_submodule->nof_tables; field_index++)
            {
                if (c_submodule->tables[field_index].name != NULL
                    && !sal_strncasecmp(c_submodule->tables[field_index].name, data, DNXC_DATA_UTILS_STRING_MAX))
                {
                    return 1;
                }
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        for (define_index = 0; define_index < submodule->nof_defines; define_index++)
        {
            ...
            /** skip define match, if define is not attached */
            if (submodule->defines[define_index].name == NULL)
            {
                continue;
            }
             ...
             * Make sure at least one of the strings is NULL terminated.
             */
            if (!sal_strncasecmp("*", data, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
        ...
        for (define_index = 0; define_index < submodule->nof_defines; define_index++)
        {
            /** skip numeric match, if numeric is not attached */
            if (submodule->defines[define_index].name == NULL)
            {
                continue;
            }
        ...
        for (feature_index = 0; feature_index < submodule->nof_features; feature_index++)
        {
            /** skip feature match, if feature is not attached */
            if (submodule->features[feature_index].name == NULL)
            {
                continue;
            }
        ...
        for (table_index = 0; table_index < submodule->nof_tables; table_index++)
        {
            ...
            /** skip table match, if table is not attached */
            if (submodule->tables[table_index].name == NULL)
            {
                continue;
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        for (submodule_index = 0; submodule_index < module->nof_submodules; submodule_index++)
        {
            /** skip submodule match, if submodule is not attached */
            if (submodules[submodule_index].name == NULL)
            {
                continue;
            }
            /*
            ...
            if (!sal_strncasecmp(submodules[submodule_index].name, submodule, DNXC_DATA_UTILS_STRING_MAX) ||
                !sal_strncasecmp("*", submodule, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            {
                submodule_found = 1;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
    {
        /** skip module match, if module is not attached */
        if (modules[module_index].name == NULL)
        {
            continue;
        }
        /*
    ...
    else
    {
        ...
        for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
        {
            ...
            if (!sal_strncasecmp(modules[module_index].name, module, DNXC_DATA_UTILS_STRING_MAX) ||
                !sal_strncasecmp("*", module, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            {
                module_found = 1;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        for (submodule_index = 0; submodule_index < module->nof_submodules; submodule_index++)
        {
             ...
             * Make sure at least one of the strings is NULL terminated.
             */
            if (!sal_strncasecmp("*", submodule, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
        {
             ...
             * Make sure at least one of the strings is NULL terminated.
             */
            if (!sal_strncasecmp("*", module, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
...
shr_error_e
diag_dnxc_dump_autocomplete_cb(
    int unit,
    char *dump_substring,
    char *dump_full_list,
    int *num_matches_found)
{
    ...
    uint32 nof_tokens = 0;
    char **datatokens = NULL;
    int module_index, submodule_index, data_index, rv;
    dnxc_data_module_t *matching_module = NULL;
    dnxc_data_submodule_t *matching_submodule = NULL;
     ...
     * Expected format - <module>.<submodule>.<data>
     */
    datatokens = utilex_str_split(dump_substring, ".", 3, &nof_tokens);
    if (datatokens != NULL)
    {
        if (nof_tokens > 0)
        {
    ...
    if (nof_tokens == 1)
    {
        ...
        {
            /*
             * skip module match if module name doesn't exist
             */
            if (_dnxc_data[unit].modules[module_index].name == NULL)
            {
                continue;
            }
            /*
             * if the module provided is '*' or NULL (to match all modules) or there is at least a partial match, add it as an option (append to dump_full_list)
             */
            if ((module == NULL)
                || !sal_strncasecmp("*", module, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
                || strcaseindex(_dnxc_data[unit].modules[module_index].name, module))
            {
                (*num_matches_found)++;
                /*
                 ...
                 */
                if (sal_strlen(dump_full_list) + sal_strlen(_dnxc_data[unit].modules[module_index].name) + 2 >=
                    AUTOCOMPLETE_MAX_STRING_LEN)
                {
                    (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                    dump_full_list[0] = 0;
                    cli_out("\n");
                    rv = diag_dnxc_data_module_list(unit, NULL);
                    if (rv == _SHR_E_NONE)
                    {
                        cli_out
                ...
                {
                    sal_strncat_s(dump_full_list, " ", AUTOCOMPLETE_MAX_STRING_LEN);
                }
                /*
                 * add module to string containing all partially-matching modules in the form of 'MODULE.' (module name followed by '.')
                 */
                sal_strncat_s(dump_full_list, _dnxc_data[unit].modules[module_index].name, AUTOCOMPLETE_MAX_STRING_LEN);
                sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
            }
        }
    }
    /*
     * if nof_tokens is 2, only the module and submodule were provided
     */
    else if (nof_tokens == 2)
    {
        for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
        {
            /*
                ...
                || strcaseindex(_dnxc_data[unit].modules[module_index].name, module))
            {
                (*num_matches_found)++;
                if ((*num_matches_found) == 1)
                    matching_module = &_dnxc_data[unit].modules[module_index];
            }
        }
        /*
         * if no matching module was found or the module matches multiple modules, no autocomplete can be done
         */
        if (matching_module == NULL || (*num_matches_found) > 1)
        {
            (*num_matches_found) = 0;
            SHR_EXIT();
        }
        else
        {
            (*num_matches_found) = 0;
            /*
             * iterate through the submodules and find all those that are at least a partial match
             */
            for (submodule_index = 0; submodule_index < matching_module->nof_submodules; submodule_index++)
            {
                /*
                 * if the submodule provided is '*' or NULL (to match all submodules) or there is at least a partial match, add it as an option (append to dump_full_list)
                 */
                if ((submodule == NULL)
                    || !sal_strncasecmp("*", submodule, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
                    || strcaseindex(matching_module->submodules[submodule_index].name, submodule))
                {
                    (*num_matches_found)++;
                    /*
                     * if the returned string isn't long enough to include all partially-matching submodules, print table with all available submodules
                     */
                    if (sal_strlen(dump_full_list) + sal_strlen(matching_module->name) +
                        sal_strlen(matching_module->submodules[submodule_index].name) + 3 >=
                        AUTOCOMPLETE_MAX_STRING_LEN)
                    {
                        (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                        dump_full_list[0] = 0;
                        cli_out("\n");
                        rv = diag_dnxc_data_submodule_list(unit, matching_module, NULL);
                        if (rv == _SHR_E_NONE)
                        {
                            cli_out
                                ("\n\nThe The submodule name provided (%s) partially matches many submodules. Please choose a submodule from the table above and try again.\n",
                                 submodule);
                            SHR_EXIT();
                        }
                        SHR_IF_ERR_EXIT(rv);
                    ...
                    /*
                     * add submodule to string containing all partially-matching submodules in the form of 'MODULE.SUBMODULE.' (module name followed by submodule name followed by '.')
                     */
                    sal_strncat_s(dump_full_list, matching_module->name, AUTOCOMPLETE_MAX_STRING_LEN);
                    sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
                    sal_strncat_s(dump_full_list, matching_module->submodules[submodule_index].name,
                                  AUTOCOMPLETE_MAX_STRING_LEN);
                    sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
                }
            }
    ...
    /*
     * if nof_tokens is 3, the module, submodule and data were provided
     */
    else if (nof_tokens == 3)
    {
        for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
        {
            /*
        ...
        else
        {
            ...
            for (submodule_index = 0; submodule_index < matching_module->nof_submodules; submodule_index++)
            {
                    ...
                    || strcaseindex(matching_module->submodules[submodule_index].name, submodule))
                {
                    (*num_matches_found)++;
                    if ((*num_matches_found) == 1)
                        matching_submodule = &matching_module->submodules[submodule_index];
                }
            }
            /*
             * if no matching submodule was found or the submodule matches multiple submodules, no autocomplete can be done
             */
            if (matching_submodule == NULL || (*num_matches_found) > 1)
            {
                (*num_matches_found) = 0;
                SHR_EXIT();
            }
            else
            {
                (*num_matches_found) = 0;
                /*
                 * iterate through the defines and numerics and find all those that are at least a partial match
                 */
                for (data_index = 0; data_index < matching_submodule->nof_defines; data_index++)
                {
                    /*
                     * if the data provided is '*' or NULL (to match all data) or there is at least a partial match, add it as an option (append to dump_full_list)
                     */
                    if ((data == NULL) || !sal_strncasecmp("*", data, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
                        strcaseindex(matching_submodule->defines[data_index].name, data))
                    {
                        (*num_matches_found)++;
                        /*
                         * if the returned string isn't long enough to include all partially-matching data points, print table with all available data points
                         */
                        if (sal_strlen(dump_full_list) + sal_strlen(matching_module->name) +
                            sal_strlen(matching_submodule->name) +
                            sal_strlen(matching_submodule->defines[data_index].name) + 3 >= AUTOCOMPLETE_MAX_STRING_LEN)
                        {
                            (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                            dump_full_list[0] = 0;
                            cli_out("\n");
                            rv = diag_dnxc_data_list(unit, matching_module->name, matching_submodule, 0, NULL, NULL);
                            if (rv == _SHR_E_NONE)
                            {
                                cli_out
                                    ("\n\nThe data name provided (%s) partially matches many features/defines/numerics/tables. Please choose a data point from the table above and try again.\n",
                                     data);
                                SHR_EXIT();
                            }
                            SHR_IF_ERR_EXIT(rv);
                        ...
                        /*
                         * add defines and numerics to string containing all partially-matching data points in the form of 'MODULE.SUBMODULE.DATA' (module name followed by submodule name followed by data name with '.' in between)
                         */
                        sal_strncat_s(dump_full_list, matching_module->name, AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, matching_submodule->name, AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, matching_submodule->defines[data_index].name,
                                      AUTOCOMPLETE_MAX_STRING_LEN);
                    }
                }
                /*
                 * iterate through the features and find all those that are at least a partial match
                 */
                for (data_index = 0; data_index < matching_submodule->nof_features; data_index++)
                {
                    /*
                     * if the data provided is '*' or NULL (to match all data) or there is at least a partial match, add it as an option (append to dump_full_list)
                     */
                    if ((data == NULL) || !sal_strncasecmp("*", data, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
                        strcaseindex(matching_submodule->features[data_index].name, data))
                    {
                        (*num_matches_found)++;
                        /*
                         * if the returned string isn't long enough to include all partially-matching data points, print table with all available data points
                        ...
                        if (sal_strlen(dump_full_list) + sal_strlen(matching_module->name) +
                            sal_strlen(matching_submodule->name) +
                            sal_strlen(matching_submodule->features[data_index].name) + 3 >=
                            AUTOCOMPLETE_MAX_STRING_LEN)
                        {
                            (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                            dump_full_list[0] = 0;
                        ...
                        /*
                         * add features to string containing all partially-matching data points in the form of 'MODULE.SUBMODULE.DATA' (module name followed by submodule name followed by data name with '.' in between)
                         */
                        sal_strncat_s(dump_full_list, matching_module->name, AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, matching_submodule->name, AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, matching_submodule->features[data_index].name,
                                      AUTOCOMPLETE_MAX_STRING_LEN);
                    }
                }
                /*
                 * iterate through the tables and find all those that are at least a partial match
                 */
                for (data_index = 0; data_index < matching_submodule->nof_tables; data_index++)
                {
                    /*
                     * if the data provided is '*' or NULL (to match all data) or there is at least a partial match, add it as an option (append to dump_full_list)
                     */
                    if ((data == NULL) || !sal_strncasecmp("*", data, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
                        strcaseindex(matching_submodule->tables[data_index].name, data))
                    {
                        (*num_matches_found)++;
                        /*
                         * if the returned string isn't long enough to include all partially-matching data points, print table with all available data points
                        ...
                        if (sal_strlen(dump_full_list) + sal_strlen(matching_module->name) +
                            sal_strlen(matching_submodule->name) +
                            sal_strlen(matching_submodule->tables[data_index].name) + 3 >= AUTOCOMPLETE_MAX_STRING_LEN)
                        {
                            (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                            dump_full_list[0] = 0;
                            cli_out("\n");
                        ...
                        /*
                         * add tables to string containing all partially-matching data points in the form of 'MODULE.SUBMODULE.DATA' (module name followed by submodule name followed by data name with '.' in between)
                         */
                        sal_strncat_s(dump_full_list, matching_module->name, AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, matching_submodule->name, AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, ".", AUTOCOMPLETE_MAX_STRING_LEN);
                        sal_strncat_s(dump_full_list, matching_submodule->tables[data_index].name,
                                      AUTOCOMPLETE_MAX_STRING_LEN);
                    }
                }
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
    {
        ...
        if (!sal_strncasecmp(module->name, module_name, DNXC_DATA_UTILS_STRING_MAX) ||
            !sal_strncasecmp("*", module_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            /*
