...
#define DIAG_DNXC_DIAG_COUNTER_BRIEF_MAN            "Display the counters of the internal blocks"

#define DIAG_DNX_DIAG_COUNTER_SYNOPSYS_MAN          "[<block_name>] [graphical] [nz] [hex] [interval=<integer>] [timeunites=<time_unit>] [error]"
#define DIAG_DNXF_DIAG_COUNTER_SINGLE_CORE_SYNOPSYS_MAN         "[<block_name>] [graphical] [nz] [hex] [interval=<integer>] [error]"
#define DIAG_DNXF_DIAG_COUNTER_NOT_SINGLE_CORE_SYNOPSYS_MAN     "[<block_name>] [graphical] [vdev] [nz] [hex] [interval=<integer>] [error]"

#define DIAG_DNX_DIAG_COUNTER_EXAMPLES_MAN          ""                                          \
                                                    "CGM\n"                                     \
                                                    "CGM nz\n"                                  \
                                                    "CGM hex\n"                                 \
                                                    "CGM interval=1\n"                          \
                                                    ...
                                                    "graphical interval=1 timeunits=sec\n"      \
                                                    "graphical CoreDiSPlay\n"                     \
                                                    "graphical core=1"

#define DIAG_DNXF_DIAG_COUNTER_SHARED_EXAMPLES_MAN  ""                                       \
                                                    "RTP \n"                                 \
                                                    "DCH nz\n"                               \
                                                    "error\n"                                \
                                                    "graphical\n"                            \
                                                    "graphical hex\n"                        \
                                                    "graphical interval=10 timeunits=sec\n"

#define DIAG_DNXF_DIAG_COUNTER_NOT_SINGLE_CORE_EXAMPLES_MAN  "graphical virtualdevice=0"
...
#define DIAG_DNXC_DIAG_COUNTER_OPTIONS  \
    ...
    {"interval", SAL_FIELD_TYPE_UINT32, "interval for gtimer (in timeUnits)", "0", NULL}, \
...
#define DIAG_DNXF_NOT_SINGLE_CORE_DIAG_COUNTER_OPTIONS \
    {"VirtualDEVice", SAL_FIELD_TYPE_INT32, "display counters per Virtual Device (FE only)", "-1", NULL }

/** Maximum cores support in diag common codes */
#define DIAG_DNXC_DIAG_COUNTER_MAX_CORES (8)
...
/**
 * Enum for option time units
 */
sh_sand_enum_t diag_dnxc_diag_time_units[] = {
    {"sec", DIAG_DNXC_TIME_UNIT_SEC, "Seconds"},
    {"milisec", DIAG_DNXC_TIME_UNIT_MILISEC, "Mili Seconds"},
    {"microsec", DIAG_DNXC_TIME_UNIT_USEC, "Micro Seconds"},
    {"nanosec", DIAG_DNXC_TIME_UNIT_NSEC, "Nano Seconds"},
    {NULL}
};
/**
 * \brief
    ...
    char *buf)
{
    ...
    if (val[3] > 0)
    {
        ...
        sal_snprintf(buf + sal_strnlen(buf, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH), PRT_COLUMN_WIDTH, "%08x",
                     val[2]);
        sal_snprintf(buf + sal_strnlen(buf, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH), PRT_COLUMN_WIDTH, "%08x",
                     val[1]);
        sal_snprintf(buf + sal_strnlen(buf, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH), PRT_COLUMN_WIDTH, "%08x",
 ...
 * \param [in] fld_value - counters value for all cores
 * \param [in] nof_bit - number of bits this decimal value occupies
 * \param [in] in_hex - indicate decimal or hex string is required
 * \param [out] buf - buffer to store the string
...
sum_value_to_str(
    ...
    int *overflow)
{
    uint64 val_64_cores[DIAG_DNXC_DIAG_COUNTER_MAX_CORES], val_64_sum;
    int nof_bit_i, nof_words, nof_bit_in_mask, mask, core_id;
    ...
    if (nof_cores >= DIAG_DNXC_DIAG_COUNTER_MAX_CORES)
    {
        LOG_CLI(("Invalid parameter: nof_cores is more than %d\n", DIAG_DNXC_DIAG_COUNTER_MAX_CORES));
        return _SHR_E_PARAM;
    }
    ...
    {
        fld_value[core_id][nof_words - 1] &= mask;
    }
    /*
     * set the second word to 0 if decimal value occupy one word only
     */
    ...
    *overflow = FALSE;
    ...
    for (core_id = 0; core_id < nof_cores; core_id++)
    {
        COMPILER_64_SET(val_64_cores[core_id], fld_value[core_id][1], fld_value[core_id][0]);
        COMPILER_64_ADD_64(val_64_sum, val_64_cores[core_id]);
        if (COMPILER_64_LT(val_64_sum, val_64_cores[core_id]))
        {
            *overflow = TRUE;
        }
    }
    ...
    int *block_ids)
{
    ...
    *nof_block = 0;
    ...
    for (i_bl = 0; SOC_BLOCK_INFO(unit, i_bl).type >= 0; i_bl++)
    {
        ...
        {
            if (cmp_len != sal_strnlen(SOC_BLOCK_NAME(unit, i_bl), SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            {
                continue;
            }
        }
    ...
    diag_dnxc_time_unit_e time_units)
{
    ...
    {
#ifdef BCM_DNX_SUPPORT
        rv = sh_dnx_diag_block_gtimer_start(unit, nof_block, block_ids, interval, time_units);
#endif
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (interval)
    {
        SHR_IF_ERR_EXIT(sh_dnxc_diag_block_gtimer_start(unit, nof_block, block_ids, interval, time_units));
    }
    ...
    sh_sand_control_t * sand_control)
{
...
#ifdef BCM_DNX_SUPPORT
    int desc = FALSE, core_id = DNX_DATA_MAX_DEVICE_GENERAL_NOF_CORES;
#endif
#ifdef BCM_DNXF_SUPPORT
    int core_idx = DNXF_DRV_CORE_ALL;
#endif
    SHR_FUNC_INIT_VARS(NO_UNIT);
    ...
    if (graphical)
    {
        ...
        if (SOC_IS_DNXF(unit))
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnxf_counters_graphical_print(unit, core_idx, sand_control));
        }
#endif /* BCM_DNXF_SUPPORT */
        ...
        if (SOC_IS_DNX(unit))
        {
            ...
            if (cdsp && (core_id != BCM_CORE_ALL))
            {
                SHR_CLI_EXIT(_SHR_E_PARAM, "Option CoreDiSPlay cannot be provided together with CORE option!\n");
            }
            if ((core_id != BCM_CORE_ALL) && ((core_id >= DNX_DATA_MAX_DEVICE_GENERAL_NOF_CORES) || (core_id < 0)))
            {

                SHR_CLI_EXIT(_SHR_E_PARAM, "Invalid core id, core id in diag is less than %d\n",
                             DNX_DATA_MAX_DEVICE_GENERAL_NOF_CORES);
            }
            SHR_IF_ERR_EXIT(sh_dnx_diag_counter_graphical_print
                            (unit, cdsp, core_id, non_zero, in_hex, interval, time_units, sand_control));
        }
#endif
    }
    else
...
static sh_sand_man_t sh_dnx_diag_counters_man = {
    .brief = DIAG_DNXC_DIAG_COUNTER_BRIEF_MAN,
    .full = DIAG_DNXC_DIAG_COUNTER_FULL_MAN,
    .synopsis = DIAG_DNX_DIAG_COUNTER_SYNOPSYS_MAN,
    .examples = DIAG_DNX_DIAG_COUNTER_EXAMPLES_MAN
};
...
static sh_sand_man_t sh_dnxf_diag_counters_man = {
    .brief = DIAG_DNXC_DIAG_COUNTER_BRIEF_MAN,
    .full = DIAG_DNXC_DIAG_COUNTER_FULL_MAN,
    .synopsis = DIAG_DNXF_DIAG_COUNTER_SINGLE_CORE_SYNOPSYS_MAN,
    .examples = DIAG_DNXF_DIAG_COUNTER_SHARED_EXAMPLES_MAN
};
...
sh_sand_cmd_t sh_dnxc_diag_cmds[] = {
    ...
    {"counter", sh_dnxc_diag_counter_cmd, NULL,    sh_dnxf_diag_counters_options,                   &sh_dnxf_diag_counters_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnxf_single_core},
    {NULL}
};
