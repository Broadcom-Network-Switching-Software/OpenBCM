                        ...
                        for (section = bcmPolicerEngineSectionLow; section <= bcmPolicerEngineSectionHigh; section++)
                        {
                            ...
                            if (engine_found == 1)
                            {
                                ...
                                else
                                {
                                    ...
                                    sal_strncat_s(engine_section,
                                                  (section == bcmPolicerEngineSectionLow) ? " Low" : " High",
                                                  sizeof(engine_section));
                                    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
                                    PRT_CELL_SET("%d", core_idx);
            ...
            for (engine_id = 0; engine_id < dnx_data_crps.engine.nof_engines_get(unit); engine_id++)
            {
                ...
                if (DNX_CRPS_METER_SHMEM_IS_METER_SINGLE_BUCKET_ENGINE(unit, engine_id))
                {
                    if (database_id == 0)
                    {
                        ...
                        if (BCM_POLICER_DATABASE_HANDLE_IS_INGRESS_GET(config.policer_database_handle))
                        {
                            find_is_ingress = 1;
                        }
                        else
                        ...
                        /** check if engine meets ingress or egress condition provided as input*/
                        if (is_egress == find_is_ingress)
                            continue;
        ...
        else
        {
            ...
            for (meter_idx = bcmPolicerFwdTypeUc; meter_idx < bcmPolicerFwdTypeMax; meter_idx++)
            {
                ...
                for (group_idx = bcmPolicerFwdTypeUc; group_idx < bcmPolicerFwdTypeMax; group_idx++)
                {
                    ...
                    {
                        sal_strncat_s(type, diag_dnx_meter_global_type_enum_to_char_binding_table[group_idx].type_str,
                                      sizeof(type));
                        sal_strncat_s(type, "  ", sizeof(type));
                        valid_flag = 1;
                    }
                }
                if (valid_flag == 0)
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 max_size;
    dnxc_time_t time_given_milisec, gtimer_max_sec, gtimer_max_milisec;
    int max_ingress_database_id = dnx_data_meter.meter_db.nof_ingress_db_get(unit);
    int max_egress_database_id = dnx_data_meter.meter_db.nof_egress_db_get(unit);
    PRT_INIT_VARS;
    ...
    gtimer_max_sec.time_units = DNXC_TIME_UNIT_SEC;
    COMPILER_64_COPY(gtimer_max_sec.time, gtimer_max_period_sec);
    SHR_IF_ERR_EXIT(dnxc_time_units_convert(unit, &gtimer_max_sec, DNXC_TIME_UNIT_MILISEC, &gtimer_max_milisec));
            ...
            if (rate == TRUE)
            {
                ...
                else
                {
...
#endif
                    {
                        ...
                        time_given_milisec.time_units = DNXC_TIME_UNIT_MILISEC;
                        COMPILER_64_SET(time_given_milisec.time, 0, interval);
                        SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get
                                        (unit, &time_given_milisec, &nof_clock_cycles));
    ...
    int unit)
{
    ...
    engine_info = dnx_data_crps.engine.engines_info_get(unit, engine_id);
    nof_meters = engine_info->nof_entries * dnx_data_meter.mem_mgmt.nof_dual_buckets_per_entry_get(unit);
    engine.engine_id = engine_id;
...
sh_dnx_meter_generic_database_info_init_cb(
    int unit)
{
    int flags = 0, core = BCM_CORE_ALL, database_id = 0, engine_id, special_engine_id;
    bcm_policer_database_config_t database_config, egress_database_config;
    int database_handle, is_ingress = 1, egress_database_handle;
    bcm_policer_engine_t engine, special_engine;
    bcm_policer_database_attach_config_t attach_config, egress_attach_config;
    bcm_policer_config_t pol_cfg;
    int policer_id;
    SHR_FUNC_INIT_VARS(unit);
    ...
    bcm_policer_database_attach_config_t_init(&attach_config);
    bcm_policer_database_config_t_init(&database_config);
    bcm_policer_engine_t_init(&special_engine);
    bcm_policer_database_attach_config_t_init(&egress_attach_config);
    bcm_policer_database_config_t_init(&egress_database_config);
    ...
    egress_database_config.expansion_enable = 0;
    egress_database_config.is_single_bucket = 1;

    BCM_POLICER_DATABASE_HANDLE_SET(egress_database_handle, 0, 0, core, database_id);
    SHR_IF_ERR_EXIT(bcm_policer_database_create(unit, flags, egress_database_handle, &egress_database_config));
    SHR_IF_ERR_EXIT(bcm_policer_database_enable_set(unit, flags, egress_database_handle, TRUE));
    egress_attach_config.policer_database_handle = egress_database_handle;
    special_engine_id = dnx_data_meter.mem_mgmt.egress_single_bucket_engine_get(unit);
    special_engine.engine_id = special_engine_id;
    special_engine.section = bcmPolicerEngineSectionAll;
    special_engine.core_id = core;
    egress_attach_config.object_stat_pointer_base = 0;
    SHR_IF_ERR_EXIT(bcm_policer_engine_database_attach(unit, flags, &special_engine, &egress_attach_config));
