 ...
 * GLOBALS   *
 *************/
dnx_pp_stage_e dnx_pp_termi_ingress_stages[DNX_PP_TERMI_INGRESS_STAGES] =
    { DNX_PP_STAGE_VTT1, DNX_PP_STAGE_VTT2, DNX_PP_STAGE_VTT3, DNX_PP_STAGE_VTT4, DNX_PP_STAGE_VTT5,
    DNX_PP_STAGE_IFWD1
};
    ...
    diag_pp_termi_stage_info_t * stage_info)
{
    ...
    for (stage_idx = 0; stage_idx < DNX_PP_TERMI_INGRESS_STAGES - 1; stage_idx++)
    {
         ...
         * Skip VTT5
         */
        if (!dnx_data_pp.stages.params_get(unit, dnx_pp_termi_ingress_stages[stage_idx])->valid)
        {
            continue;
        }

        sal_strncpy(stage_info[stage_idx].name,
                    dnx_data_pp.stages.params_get(unit, dnx_pp_termi_ingress_stages[stage_idx])->name,
                    RHNAME_MAX_SIZE - 1);
     ...
     */
    sal_strncpy(stage_info[DNX_PP_TERMI_INGRESS_STAGES - 1].name, "FWD1", RHNAME_MAX_SIZE - 1);
    ...
    diag_pp_termi_output_t * outputs)
{
    ...
    int nof_mpls_term = 0;
    int nof_ext_headers = 0;
    int mpls_idx;
    int initial_protocol_index = 0;
    int initial_layer_offset = stage_info[0].layer_offsets[0];

    SHR_FUNC_INIT_VARS(unit);

    if (initial_layer_offset != 0)
    {
        initial_protocol_index++;
    }
    next_protocol_idx = initial_protocol_index;

    /*
     * handle VTT1
     */
    if ((sal_strnstr(next_protocol_array[next_protocol_idx], "ETH", sizeof(next_protocol_array[next_protocol_idx]))) &&
    ...
    /** Other terminated headers except for ETH should be handled too to exhaust the protocol array properly.*/
    else if (stage_info[1].fwd_layer_index == stage_info[0].fwd_layer_index + 1)
    {
        sal_strncpy(outputs->terminated_headers[0], next_protocol_array[next_protocol_idx], RHNAME_MAX_SIZE);
        next_protocol_idx++;
    }
    ...
    for (stage_idx = 1; stage_idx < DNX_PP_TERMI_INGRESS_STAGES - 1; stage_idx++)
    {
        ...
        nof_mpls_term =
            (stage_info[stage_idx].layer_offsets[fwd_layer_idx + 1] -
             stage_info[stage_idx - 1].layer_offsets[prev_fwd_layer_idx + 1]);
        nof_mpls_term = nof_mpls_term / 4;
             ...
             dnx_init_pp_layer_types_value_get(unit, DBAL_ENUM_FVAL_LAYER_TYPES_MPLS)) && (nof_mpls_term > 0))
        {
            for (mpls_idx = 0; mpls_idx < nof_mpls_term; mpls_idx++)
            {
                ...
                if (sal_strncmp
                    (next_protocol_array[next_protocol_idx], "MPLS",
                     sizeof(next_protocol_array[next_protocol_idx])) != 0
                    && sal_strncmp(next_protocol_array[next_protocol_idx], "CW",
                                   sizeof(next_protocol_array[next_protocol_idx])) != 0
                    && sal_strncmp(next_protocol_array[next_protocol_idx], "GACH",
                                   sizeof(next_protocol_array[next_protocol_idx])) != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Expecting MPLS header but got %s in VTT%d",
                                 next_protocol_array[next_protocol_idx], stage_idx + 1);
                }
                ...
                /** When GACH is found, GAL and GACH are both  terminated*/
                if (sal_strncmp(next_protocol_array[next_protocol_idx + 1], "GACH",
                                sizeof(next_protocol_array[next_protocol_idx + 1])) == 0)
                {
                    sal_snprintf(outputs->terminated_headers[stage_idx], RHNAME_MAX_SIZE, "%s(0x%s)",
                                 gal_name, mpls_label_array[mpls_label_idx]);
                }
                else if (sal_strncmp(next_protocol_array[next_protocol_idx], "GACH",
                                     sizeof(next_protocol_array[next_protocol_idx])) == 0)
                {
                    sal_snprintf(outputs->terminated_headers[stage_idx], RHNAME_MAX_SIZE, "%s",
                ...
                if (mpls_idx != 0)
                {
                    sal_strncat_s(outputs->terminated_headers[stage_idx], "o",
                                  sizeof(outputs->terminated_headers[stage_idx]));
                    sal_strncat_s(outputs->terminated_headers[stage_idx], previous_str,
                                  sizeof(outputs->terminated_headers[stage_idx]));
                }
                                                              ...
                                                              stage_info[stage_idx + 1].layer_offsets)))
        {
            ...
            if (sal_strnstr(next_protocol_array[next_protocol_idx - 1], "RCH", RHNAME_MAX_SIZE) &&
                sal_strnstr(next_protocol_array[next_protocol_idx], "Skipped", RHNAME_MAX_SIZE))
            {
                /** In case of RCH and following RCH Extension, skip them too.*/
                nof_ext_headers++;
            }
            else if (sal_strnstr(next_protocol_array[next_protocol_idx - 1], "IPv6", RHNAME_MAX_SIZE) &&
                     sal_strnstr(next_protocol_array[next_protocol_idx], "SRv6", RHNAME_MAX_SIZE))
            {
                /** Proceed again since SRv6 header is port of IPv6 protocol in parser.*/
                next_protocol_idx++;
            }
        }
    }
     ...
     * handle FWD
     */
    if (sal_strncmp(next_protocol_array[next_protocol_idx], "MPLS", sizeof(next_protocol_array[next_protocol_idx])) ==
        0)
    {
    ...
    {
        next_protocol_idx = stage_info[DNX_PP_TERMI_INGRESS_STAGES - 1].fwd_layer_index + initial_protocol_index;
        next_protocol_idx += (mpls_label_idx == 0 ? 0 : mpls_label_idx - 2);
        next_protocol_idx += nof_ext_headers;
    ...
    diag_pp_termi_output_t * outputs)
{
    ...
    SH_DNX_CORES_ITER(unit, inputs.core_id, core_id)
    {
        ...
        /** Check if the core is ready firstly.*/
    ...
    diag_pp_termi_output_t * outputs)
{
    ...
    SH_DNX_CORES_ITER(unit, inputs.core_id, core_id)
    {
        ...
        for (stage_idx = 0; stage_idx < DNX_PP_TERMI_INGRESS_STAGES - 1; stage_idx++)
        {
            ...
            PRT_CELL_SET("%s", dnx_data_pp.stages.params_get(unit, dnx_pp_termi_ingress_stages[stage_idx])->name);
...
/**
 * \brief - print termination information
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] core_id - core id
 *   \param [out] packet_struct_str - packet termination information string
 *
 * \par DIRECT OUTPUT:
 *   shr_error_e
 */
shr_error_e
sh_dnx_pp_termi_info(
    int unit,
    int core_id,
    char *packet_struct_str)
{
    ...
    for (stage_idx = 0; stage_idx < DNX_PP_TERMI_INGRESS_STAGES; stage_idx++)
    {
        ...
        sal_strncat_s(packet_struct_str,
                      dnx_data_pp.stages.params_get(unit, dnx_pp_termi_ingress_stages[stage_idx])->name,
                      DSIG_MAX_SIZE_STR);
        sal_strncat_s(packet_struct_str, ":", DSIG_MAX_SIZE_STR);
        sal_strncat_s(packet_struct_str, outputs_p[core_id].terminated_headers[stage_idx], DSIG_MAX_SIZE_STR);
        sal_strncat_s(packet_struct_str, "\n", DSIG_MAX_SIZE_STR);
    }
exit:
    SHR_FREE(outputs_p);
