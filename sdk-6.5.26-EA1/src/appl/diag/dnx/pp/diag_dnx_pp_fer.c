 ...
 */
#define DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN 19
/*************
 *  MACROS  *
 *************/
...
/** \brief List of signals types */
typedef enum
{
    ECMP_LB_KEYS_SIGNAL,
    IN_PORT_SIGNAL,
    ECMP_NWK_LB_KEYS_SIGNAL,
    NOF_SIGNALS
} ecmp_signal_info_t;
    ...
    char *values[NOF_COLUMNS])
{
    ...
    int nof_signals = NOF_SIGNALS;
    uint32 hierarchy;
    uint32 fec_offset;
    uint32 used_member = 0;
    uint16 ecmp_lb_keys[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES];
    char sub_titles[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES][DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN] =
    ...
    SHR_ALLOC_ERR_EXIT(signal_key,
                       (sizeof(bcm_instru_vis_signal_key_t) * nof_signals),
                       "signal_key", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_ERR_EXIT(signal_result,
                       (sizeof(bcm_instru_vis_signal_result_t) * nof_signals),
                       "signal_result", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    ...
    sal_strncpy(signal_key[ECMP_LB_KEYS_SIGNAL].block, dnx_pp_stage_block_name(unit, DNX_PP_STAGE_IFWD1),
                BCM_INSTRU_VIS_BLOCK_NAME_MAX_SIZE - 1);
    sal_strncpy(signal_key[ECMP_LB_KEYS_SIGNAL].from, dnx_pp_stage_name(unit, DNX_PP_STAGE_IFWD1),
                BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
    sal_strncpy(signal_key[ECMP_LB_KEYS_SIGNAL].signal, "Ecmp_LB_Keys", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);


    SHR_IF_ERR_EXIT(bcm_dnx_instru_vis_signal_get(unit, core_id, 0, nof_signals, signal_key, signal_result));
    if (signal_result[ECMP_LB_KEYS_SIGNAL].status != BCM_E_NONE)
    {
        LOG_CLI(("Could not retrieve signal data for core = %d\n", core_id));
        SHR_EXIT();
    }

    ecmp_lb_keys[2] = signal_result[ECMP_LB_KEYS_SIGNAL].value[1];
    ecmp_lb_keys[1] = signal_result[ECMP_LB_KEYS_SIGNAL].value[0] >> lb_key_length;
    ecmp_lb_keys[0] = signal_result[ECMP_LB_KEYS_SIGNAL].value[0] & UTILEX_BITS_MASK(lb_key_length - 1, 0);

    sal_strncpy(values[ECMP_COLUMN_ID_ECMP_HIERARCHY], sub_titles[hierarchy_id], DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    ...
    char *values[NOF_COLUMNS])
{
    ...
    sal_strncpy(values[FEC_COLUMN_ID_FEC_HIERARCHY], sub_titles[hierarchy_id], DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int nof_columns = NOF_COLUMNS;
    int iter;
    char *ecmp_values[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES][NOF_COLUMNS] = { {NULL} };
    char *fec_values[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES][NOF_COLUMNS] = { {NULL} };
    char *fec_substages[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES] = { "FEC1", "FEC2", "FEC3" };

    char ecmp_fields[NOF_COLUMNS][DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN];
    char fec_fields[NOF_COLUMNS][DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN];
    ...
    for (iter = 0; iter < NOF_COLUMNS; iter++)
    {
        sal_strncpy_s(ecmp_fields[iter], "", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
        sal_strncpy_s(fec_fields[iter], "", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    }
    /** Initialize the ecmp_fields array**/
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_ECMP_HIERARCHY], "ECMP Hierarchy", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_GROUP_ID], "Group ID", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_GROUP_SIZE], "Group size", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_PROTECTION], "Protection", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_STATEFUL], "Stateful", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_HASHING_MODE], "Hashing mode", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_LB_KEY], "LB key", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_BASE_FEC], "Base FEC", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_FEC_OFFSET], "FEC offset", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(ecmp_fields[ECMP_COLUMN_ID_SELECTED_FEC], "Selected FEC", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);

    for (iter = ECMP_COLUMN_ID_SELECTED_FEC + 1; iter < NOF_COLUMNS; iter++)
    {
        sal_strncpy_s(ecmp_fields[iter], "", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    }
       /** Initialize the fec_fields array**/
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_FEC_HIERARCHY], "FEC Hierarchy", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_FEC_ID], "FEC ID", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_STATISTICS_ID], "Statistics ID", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_STAT_CMD], "Stat Cmd", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_DESTINATION], "Destination", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_MC_RPF_MODE], "MC RPF mode", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_EEI], "EEI", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_HIER_TM_FLOW], "Hier TM flow", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_GLOB_OUT_LIF], "Glob_Out_LIF", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_GLOB_OUT_LIF_2ND], "Glob_Out_LIF_2nd", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    sal_strncpy_s(fec_fields[FEC_COLUMN_ID_PROTECTION_PATH], "Protection Path", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    for (iter = FEC_COLUMN_ID_PROTECTION_PATH + 1; iter < NOF_COLUMNS; iter++)
    {
        sal_strncpy_s(fec_fields[iter], "", DIAG_DNX_PP_VIS_FER_MAX_FIELD_LEN);
    }
    {
    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
        ...
        for (hier_id = 0; hier_id < nof_hierarchies; hier_id++)
        {
            ...
            {
                SHR_ALLOC_ERR_EXIT(ecmp_values[hier_id][idx],
                                   (sizeof(char) * DBAL_MAX_PRINTABLE_BUFFER_SIZE),
                                   "ECMP values print", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
                SHR_ALLOC_ERR_EXIT(fec_values[hier_id][idx],
                                   (sizeof(char) * DBAL_MAX_PRINTABLE_BUFFER_SIZE),
                                   "FEC values print", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
