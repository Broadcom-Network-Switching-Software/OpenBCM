 ...
 * \param [out] retrieved_rpf_lookup -Boolean for whether or not an RPF lookup was found.
    ...
    bcm_instru_vis_signal_result_t * signal_result_stack)
{
    ...
    dnx_pp_stage_e vtt_stage;
    SHR_FUNC_INIT_VARS(unit);

    vtt_stage = (dnx_data_pp.stages.params_get(unit, DNX_PP_STAGE_VTT5)->valid ? DNX_PP_STAGE_VTT5 : DNX_PP_STAGE_VTT4);
    ...
    *retrieved_rpf_lookup = TRUE;
    ...
    sal_strncpy(signal_key[5].block, dnx_pp_stage_block_name(unit, vtt_stage), BCM_INSTRU_VIS_BLOCK_NAME_MAX_SIZE - 1);
    sal_strncpy(signal_key[5].from, dnx_pp_stage_name(unit, vtt_stage), BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 fwd_act_dst;
    uint32 fec_interfaces[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES] = { 0 };
    bcm_if_t in_lif = 0;
    const int nof_hierarchies = dnx_data_mdb.direct.nof_fec_ecmp_hierarchies_get(unit);
    uint32 rpf_fec[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES];
    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
         ...
         */
        SHR_IF_ERR_EXIT(sh_dnx_pp_vis_rpf_sig
                        (unit, core_id, &retrieved_rpf_lookup, signal_result, signal_result_stack));
        if (!retrieved_rpf_lookup)
        {
            LOG_CLI(("No valid RPF lookup was found for core=%d\n", core_id));
            SHR_EXIT();
        }
        ...
        if (in_lif >= dnx_data_l3.rif.nof_rifs_get(unit))
        {
        ...
        if (rv != _SHR_E_NONE)
        {
            sal_printf("Ingress interface found in signals %d does not exists \n", in_lif);
            SHR_EXIT();
        }
        ...
        if (dnx_data_debug.general.fer_debug_signals_get(unit))
        {
            fec_interfaces[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1] = signal_result_stack[3].value[0];
            fec_interfaces[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_2] = signal_result_stack[4].value[0];
            fec_interfaces[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_3] = signal_result_stack[5].value[0];
        }
        ...
        if (is_mc == 0)
        {
            if (dnx_data_debug.general.fer_debug_signals_get(unit))
            {
                if ((fec_interfaces[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1] == 0)
                    && (fec_interfaces[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_2] == 0)
                    && (fec_interfaces[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_3] == 0))
                {
        ...
        for (hier_id = 0; hier_id < nof_hierarchies; hier_id++)
        {
            ...
            if (is_allocated == 1)
            {
                ...
                if (is_mc == TRUE)
                {
                    SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get(unit, DBAL_TABLE_SUPER_FEC_1ST_HIERARCHY,
                                                                            DBAL_FIELD_MC_RPF_MODE, 0,
                ...
                else
                {
                    ...
                    SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get(unit, DBAL_TABLE_IN_LIF_PROFILE_TABLE,
                                                                            DBAL_FIELD_RPF_MODE, 0,
                ...
                if ((dnx_data_l3.fec.fer_hw_version_get(unit) == DNX_L3_FER_HW_VERSION_1) && (failover_id != 0))
                {
            ...
            {
                sal_printf("FEC with ID 0x%04x is not allocated \n", rpf_destination);
                break;
            }
        }
        PRT_COMMITX;
