    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    cmd_id_max = dnx_data_snif.ingress.nof_profiles_get(unit) - 1;
        ...
        for (cmd_id = cmd_id_start; cmd_id <= cmd_id_end; cmd_id++)
        {
            ...
            if (is_allocated == TRUE)
            {
                ...
                DNXCMN_CORES_ITER(unit, core, core_id)
                {
                    ...
                    {
                        nof_egress_profiles = dnx_data_snif.egress.snoop_nof_profiles_get(unit);
                    }
                    else if (type == DNX_SNIF_TYPE_MIRROR)
                    {
                        nof_egress_profiles = dnx_data_snif.egress.mirror_nof_profiles_get(unit);
                    }

                    if (((ingr_egr == DNX_SNIF_EGRESS) || (ingr_egr == DNX_SNIF_COUNT))
                        && (type != DNX_SNIF_TYPE_STATISTICAL_SAMPLE) && (cmd_id < nof_egress_profiles))
                    {
                        /*
