...
typedef struct xd_s
{
    ...
    int xd_in_tm_port;          /* Speified in TM port */
    int xd_ptch_src_port;       /* Speified in PP port as PTCH src system port */
...
sh_sand_option_t sh_dnx_tx_options[] = {
    ...
    {"PORT", SAL_FIELD_TYPE_INT32, "Logical port to set IN TM PORT", "-1"},
    ...
    {"PtchSRCport", SAL_FIELD_TYPE_INT32, "Logical port to set IN PP PORT", "-1"},
    {"ItmhPRESent", SAL_FIELD_TYPE_BOOL, "Indicate ITMH header is present", "No"},
    {"FwdActStrength3LSB", SAL_FIELD_TYPE_UINT32, "Specify forward action strength", "0"},
    ...
    int unit)
{
    ...
    xd->xd_in_tm_port = BCM_PORT_INVALID;
    xd->xd_ptch_src_port = BCM_PORT_INVALID;
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint8 *packet_data)
{
    ...
    data_len = sal_strnlen(packet_string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) - data_base;
 ...
 * \param [in] xd - Transmit description structure
 * \param [in] is_no_ptch - true if ptch does not exist
 * \param [out] ibch1_header - Buffer pointer to IBCH1 header
 * \return
 * \remark
 *   * None
 * \see
    ...
    uint8 *ibch1_header)
{
    ...
    ibch1_header[0] = 0x0;
    ibch1_header[1] = 0x0;
    /** get the ptc from the psrc port if ptch does not exist. next header is user responsibility according to its header_Type */
    if (is_no_ptch)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_ptc_get(unit, xd->xd_ptch_src_port, &core_id, &ptc));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_ptc_get(unit, xd->xd_in_tm_port, &core_id, &ptc));
    }
    /** set PTC value in IBCH1 header */
    ibch1_header[2] = (uint8) ptc;
 ...
 * \param [in] xd - Transmit description structure
 * \param [in] header_type_incoming - incoming tm port header type, use JR1 format if INJECTED_2_JR1_MODE, INJECTED_2_PP_JR1_MODE
 * \param [in] is_no_ptch - inidcate no ptch and following headers, used for PON/COE ports
 * \param [in] ibch1_enable - inidcate that ibch1 header enabled
 * \return
 *   System header size
 * \remark
 *   * None
    ...
    int ibch1_enable)
{
    ...
    header_size += ibch1_enable ? SH_DNX_TX_IBCH1_LEN : 0;
    if (!is_no_ptch)
    {
    ...
    xd_t * xd)
{
    ...
    uint32 ptch_ssp = 0;
    uint32 ptch_ssp_tm = 0;
    bcm_core_t ptch_ssp_core = 0;
    uint32 in_tm_port = 0;
    uint32 in_tm_port_pp = 0;
    bcm_core_t in_tm_port_core = 0;
    uint8 is_ptch_ssp_cpu = FALSE;
    uint8 has_cpu_port = FALSE;
    uint8 is_no_ptch = FALSE;
    uint8 skip_in_port_check = FALSE;
    int header_type_incoming, header_type_incoming_valid;
    int cpu_channel;
     ...
     */
    if (xd->xd_ptch_src_port != BCM_PORT_INVALID)
    {
        ...
        ptch_ssp_tm = mapping_info.in_tm_port;
        ...
        SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get(xd->xd_tx_unit, xd->xd_ptch_src_port,
                                                        DNX_ALGO_GPM_GPORT_TO_PHY_OP_NONE, &gport_info));
        SHR_IF_ERR_EXIT(dnx_port_sit_port_flag_get(xd->xd_tx_unit, gport_info.local_port, &flags));
        /*
         * In the case of COE port
         * 1. inject packet without PTCH and following headers
         * 2. cpu_channel is force to be 0, this is done via overwriting xd_in_tm_port
         * 3. bypass in port check because in_tm_port is specified and in_pp_port is not cared
         */
        if (DNX_PORT_IS_COE_PORT(flags))
        {
            uint8 has_channel_id_0 = FALSE;
            is_no_ptch = TRUE;
            skip_in_port_check = TRUE;

            if (is_ptch_ssp_cpu == FALSE)
            {
                SHR_CLI_EXIT(_SHR_E_PARAM, "PtchSRCport is limited to CPU port when it is COE\n");
            }
            SHR_IF_ERR_EXIT(bcm_port_config_get(xd->xd_tx_unit, &config));
            BCM_PBMP_ITER(config.cpu, port_ndx)
            {
                SHR_IF_ERR_EXIT(dnx_algo_port_in_channel_get(xd->xd_tx_unit, port_ndx, &cpu_channel));
                if (cpu_channel == 0)
                {
                    has_channel_id_0 = TRUE;
                    xd->xd_in_tm_port = port_ndx;
                    break;
                }
            }
            if (has_channel_id_0 == FALSE)
            {
                SHR_CLI_EXIT(_SHR_E_PARAM,
                             "There is no channelized port whose channel id is 0 when PtchSRCport is COE\n");
            }
        }
        if (is_no_ptch)
        {
            ...
            {
                SHR_CLI_EXIT(_SHR_E_PARAM,
                             "Option PortBitMap and ItmhPRESent are not allowed when PtchSRCport is COE/PON\n");
            }
        }
    }
     ...
     */
    if (xd->xd_in_tm_port != BCM_PORT_INVALID)
    {
        ...
        if ((flags & BCM_PORT_ADD_CONFIG_CHANNELIZED) != BCM_PORT_ADD_CONFIG_CHANNELIZED)
        {
            SHR_CLI_EXIT(_SHR_E_PARAM, "TX failed because source TM port(%d,%d) is not channelized port\n",
                         xd->xd_in_tm_port, in_tm_port_core);
        }
    }

    if (skip_in_port_check == FALSE)
    {
        if (xd->xd_in_tm_port != BCM_PORT_INVALID)
        {
            if (xd->xd_ptch_src_port != BCM_PORT_INVALID)
        ...
        else
        {
            ...
            {
                /*
                 * Set in tm port per PtchSRCport when PORT is not specified,
                 */
                if (is_ptch_ssp_cpu)
                {
                ...
                {
                    /*
                     * Set in tm port per first CPU port which is on the same core with PtchSRCport
                     */
                    SHR_IF_ERR_EXIT(bcm_port_config_get(xd->xd_tx_unit, &config));
                    BCM_PBMP_ITER(config.cpu, port_ndx)
                    {
            ...
            {
                /*
                 * Set in TM port and in PP port per first CPU port when both are not specified
                 */
                SHR_IF_ERR_EXIT(bcm_port_config_get(xd->xd_tx_unit, &config));
                BCM_PBMP_ITER(config.cpu, port_ndx)
                {
                    ...
                    xd->xd_ptch_src_port = xd->xd_in_tm_port = port_ndx;
                    in_tm_port = ptch_ssp_tm;
                    in_tm_port_pp = ptch_ssp;
                    in_tm_port_core = ptch_ssp_core;
                    has_cpu_port = TRUE;
        ...
        /*
         * Check if psrc is member of trunk
         */
    ...
    else
    {
        key.index = 1;
        key.type = bcmSwitchPortHeaderType;
        SHR_IF_ERR_EXIT(bcm_switch_control_indexed_port_get(xd->xd_tx_unit, xd->xd_in_tm_port, key, &value));
        header_type_incoming = value.value;
    }

    /** get if IBCH1 enabled for xd_in_tm_port */
    SHR_IF_ERR_EXIT(bcm_port_control_get(xd->xd_tx_unit, xd->xd_in_tm_port, bcmPortControlIbch1Enable, &ibch1_enable));
    ...
    if ((header_type_incoming == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED_2_PP)
        ...
        || (header_type_incoming == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED_2_PP_JR1_MODE))
    {
        header_type_incoming_valid = TRUE;
    }
    else
    {
        header_type_incoming_valid = FALSE;
        is_no_ptch = TRUE;
    }
    if ((header_type_incoming_valid) || (ibch1_enable))
    {
        int actual_ibch1_header_size = ibch1_enable ? SH_DNX_TX_IBCH1_LEN : 0;
        header_size = sh_dnx_tx_calc_header_size(xd, header_type_incoming, is_no_ptch, ibch1_enable);
        ...
        if (ibch1_enable)
        {
            SHR_IF_ERR_EXIT(sh_dnx_tx_setup_ibch1
                            (xd->xd_unit, xd, is_no_ptch, pkt_data_with_header + DNX_MODULE_HEADER_SIZE));
        }
        if (!is_no_ptch)
        {
            sh_dnx_tx_setup_ptch(xd, (pkt_data_with_header + DNX_MODULE_HEADER_SIZE + actual_ibch1_header_size),
                                 ptch_ssp, header_type_incoming);
            if (BCM_PBMP_NOT_NULL(xd->tx_pbmp) || xd->xd_itmh_present)
            {
                sh_dnx_tx_setup_itmh(xd, (pkt_data_with_header + DNX_MODULE_HEADER_SIZE + actual_ibch1_header_size +
                                          SH_DNX_TX_PTCH_TYPE2_LEN), header_type_incoming);
            }
        }
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_in_channel_get(xd->xd_tx_unit, xd->xd_in_tm_port, &cpu_channel));
        pkt_info->_dpp_hdr[0] = cpu_channel;
#ifdef ADAPTER_SERVER_MODE
        /*
         * Logical port is accepted to adapter_send_buffer()
         * xd->xd_in_tm_port is the first local CPU port on the same core as the SSP
         * Use it as src_gport
         */
        pkt_info->src_gport = xd->xd_in_tm_port;
#endif
        ...
        for (xd->xd_cur_cnt = 0; xd->xd_cur_cnt < xd->xd_tot_cnt; xd->xd_cur_cnt++)
        {
            ...
            if (BCM_PBMP_NOT_NULL(xd->tx_pbmp))
            {
                ...
                BCM_PBMP_ITER(xd->tx_pbmp, port_ndx)
                {
                     ...
                     */
                    SHR_IF_ERR_EXIT(bcm_port_get(xd->xd_tx_unit, port_ndx, &flags, &interface_info, &mapping_info));
                    if ((interface_info.interface == BCM_PORT_IF_ILKN) || (interface_info.interface == BCM_PORT_IF_SFI))
                    {
                        continue;
                    }
                    ...
                    sh_dnx_tx_itmh_dest_port_set(xd->xd_tx_unit,
                                                 ...
                                                 actual_ibch1_header_size + SH_DNX_TX_PTCH_TYPE2_LEN,
                                                 BCM_GPORT_SYSTEM_PORT_ID_GET(sysport_gport), header_type_incoming);
...
exit:
    ...
    if (pkt_info)
    {
        bcm_pkt_free(xd->xd_tx_unit, pkt_info);
        pkt_info = NULL;
    }
    SHR_FUNC_EXIT;
}
