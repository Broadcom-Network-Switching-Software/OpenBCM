    ...
    char *lane_str)
{
    ...
    BCM_PBMP_ITER(*pbmp, lane)
    {
        ...
        if (last_added == -1)
        {
            ...
            sal_strncat_s(tmp_str2, tmp_str1, sizeof(tmp_str2));
            continue;
        }
        /*
         * Handle range.
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXC_SERDES_REF_CLOCK_TYPE ref_clk_index;
    ...
    char *ref_clk_str[] = { "125", "156.25", "25", "312.5", "BYPASS" };
    ...
    BCM_PBMP_ITER(nif_pbmp, port)
    {
        ...
        {
            sal_sprintf(port_info_str, "OTN %s", if_fmt_speed(speed_str, port_resource.speed));
        }
        else
        {
        ...
        if (port_has_speed)
        {
            ...
            lane_speed = DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, L1))
                || DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info) ? speed : (speed / phy_count);
        ...
        SHR_IF_ERR_EXIT(dnx_nif_synce_enable_get(unit, ethu_info.core, is_mgu, &synce_enable));
        ...
        if (ref_clk_index == DNXC_SERDES_REF_CLOCK_BYPASS)
        {
            /*
        ...
        if (DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
        {
            ...
            for (idx = 0; idx < dnx_data_nif.otn.supported_interfaces_info_get(unit)->key_size[0]; idx++)
            {
                if (dnx_data_nif.otn.supported_interfaces_get(unit, idx)->is_valid)
                {
                    ...
                    {
                        if (phy_count == dnx_data_nif.otn.supported_interfaces_get(unit, idx)->nof_lanes)
                        {
                            utilex_str_append(comment_str,
                                              dnx_data_nif.otn.supported_interfaces_get(unit, idx)->otn_intf_name);
                            found++;
                        }
                    }
                }
            ...
            if (!found)
            {
                utilex_str_append(comment_str, "OTN");
            }
        }
        if (is_over_fabric)
        {
