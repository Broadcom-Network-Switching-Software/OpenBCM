    ...
    int reg_index)
{
...

    {
        ...
        /** For FDR block, still use field "arr_i" for backward compatibility requirement */
        if (SOC_BLOCK_IN_LIST(unit, SOC_REG_INFO(unit, reg).block, SOC_BLK_FDR))
        {
            reg_index = 0;
            first_array_idx = 0;
            reg_numels = 1;
        ...
        if (first_array_idx)
        {
            reg_index = reg_index - first_array_idx;
        }

        if (total_instances > nof_cores)
        {
            core_id = (blk_instance_id * reg_numels + reg_index) / inst_per_core;
        }
        else
        {
            core_id = (blk_instance_id * reg_numels + reg_index) % nof_cores;
        }
    }
 ...
 * \param [in] reg_index - register index which associate the counter register
 * \param [in] reg_field - register field to read
 * \param [in] reg_overflow_field - register overflow field to read
...
sh_dnx_counter_graphic_per_core_counter_regs_read(
    ...
    soc_field_t reg_overflow_field,
    sh_dnx_graphic_counter_reg_cache_t * reg_cache,
    int field_len,
    ...
    uint32 *overflow)
{
    ...
    uint32 temp_overflow = 0;
    bcm_core_t core_id;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(sh_dnx_counter_graphic_get_reg_value(unit, reg, reg_port, reg_index, reg_cache, reg_value));
    ...
    core_id = sh_dnx_counter_graphic_counter_reg_core_id_get(unit, reg, reg_port, reg_index);
    ...
    int *row_valid)
{
    ...
    soc_field_t cnt_field, over_flow_field;
    int cnt_reg_index, core_idx;
    ...
    {
        reg = blk_group[*blk_idx].counter_table[*cnt_idx].reg_arr[reg_idx];
        cnt_field = blk_group[*blk_idx].counter_table[*cnt_idx].cnt_field[reg_idx];
        over_flow_field = blk_group[*blk_idx].counter_table[*cnt_idx].overflow_field[reg_idx];
        ...
        *field_len = soc_reg_field_length(unit, reg, cnt_field);
        ...
        {
            reg_numels = 1;
            first_array_idx = blk_group[*blk_idx].counter_table[*cnt_idx].reg_index;
        }
        ...
        for (instance_id = 0; instance_id < nof_instances; instance_id++)
        {
            for (cnt_reg_index = first_array_idx; cnt_reg_index < reg_numels + first_array_idx; cnt_reg_index++)
            {
                SHR_IF_ERR_EXIT(sh_dnx_counter_graphic_per_core_counter_regs_read
                                (unit, options.interval, reg, instance_id, cnt_reg_index, cnt_field,
                                 over_flow_field, reg_cache, *field_len, fld_value_u64, overflow));
            }
        }
    }
    ...
    uint64 *counter)
{
    ...
    for (reg_idx = 0; reg_idx < nof_regs; reg_idx++)
    {
        ...
        /** For FDR block, still use field "arr_i" for backward compatibility requirement */
        if (SOC_BLOCK_IN_LIST(unit, SOC_REG_INFO(unit, reg_entry->reg_arr[reg_idx]).block, SOC_BLK_FDR))
        {
            reg_numels = 1;
