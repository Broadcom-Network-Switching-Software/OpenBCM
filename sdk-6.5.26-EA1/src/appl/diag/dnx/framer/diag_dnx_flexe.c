...
#define DNX_DIAG_FLEXE_MAX_STR_LEN                   10

#define DNX_DIAG_FLEXE_BLOCK_FLEXE                   6
#define DNX_DIAG_FLEXE_BLOCK_FLEXE_BKUP              23
#define DNX_DIAG_FLEXE_BLOCK_VIRTUAL_FLEXE           7
#define DNX_DIAG_FLEXE_BLOCK_BUSB                    9
#define DNX_DIAG_FLEXE_BLOCK_BUSB_BKUP               26
...
/**
 * \brief FlexE flow port type
 */
typedef enum
{
    DNX_DIAG_FLEXE_FLOW_ETH_L1,
    DNX_DIAG_FLEXE_FLOW_FLEXE_CLIENT,
    DNX_DIAG_FLEXE_FLOW_FRAMER_MAC,
    DNX_DIAG_FLEXE_FLOW_FRAMER_SAR
} dnx_diag_flexe_flow_port_type_e;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_pbmp_t l1_eth_ports, framer_mac_clients;
    bcm_port_interface_info_t if_info;
    bcm_port_mapping_info_t mapping_info;
...
dnx_diag_flexe_gen2_per_port_type_dump(
    int unit,
    prt_control_t *prt_ctr,
    dnx_diag_flexe_flow_port_type_e port_type)
{
    ...
    switch (port_type)
    {
        case DNX_DIAG_FLEXE_FLOW_ETH_L1:
        {
            _SHR_PBMP_ASSIGN(src_port_pbmp, l1_eth_ports);
            src_info.block_id = DNX_DIAG_FLEXE_BLOCK_FLEXE;
            break;
        }
        case DNX_DIAG_FLEXE_FLOW_FLEXE_CLIENT:
        {
            _SHR_PBMP_ASSIGN(src_port_pbmp, flexe_clients);
            src_info.block_id = DNX_DIAG_FLEXE_BLOCK_FLEXE;
            break;
        }
        case DNX_DIAG_FLEXE_FLOW_FRAMER_MAC:
        {
            _SHR_PBMP_ASSIGN(src_port_pbmp, framer_mac_clients);
            src_info.block_id = DNX_DIAG_FLEXE_BLOCK_BUSB;
            break;
        }
        case DNX_DIAG_FLEXE_FLOW_FRAMER_SAR:
        {
            _SHR_PBMP_ASSIGN(src_port_pbmp, framer_sar_clients);
            src_info.block_id = DNX_DIAG_FLEXE_BLOCK_VIRTUAL_FLEXE;
            break;
        }
        default:
        {
    ...
    _SHR_PBMP_ITER(src_port_pbmp, src_logical_port)
    {
         ...
         */
        if (port_type == DNX_DIAG_FLEXE_FLOW_FRAMER_SAR)
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_flexe_sar_virtual_flexe_channel_get(unit, src_logical_port, &src_info.channel));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, src_logical_port, &src_info.channel));
        }
        /*
        ...
        if (nof_dest > 0)
        {
            ...
            {
                switch (dst_info[i].block_id)
                {
                    case DNX_DIAG_FLEXE_BLOCK_FLEXE:
                    {
                        _SHR_PBMP_ASSIGN(dest_port_pbmp, ports_use_flexe1);
                        break;
                    }
                    case DNX_DIAG_FLEXE_BLOCK_FLEXE_BKUP:
                    {
                        _SHR_PBMP_ASSIGN(dest_port_pbmp, ports_use_flexe1);
                        bkup_index = i;
                        break;
                    }
                    case DNX_DIAG_FLEXE_BLOCK_VIRTUAL_FLEXE:
                    {
                        _SHR_PBMP_ASSIGN(dest_port_pbmp, framer_sar_clients);
                        break;
                    }
                    case DNX_DIAG_FLEXE_BLOCK_BUSB:
                    {
                        _SHR_PBMP_ASSIGN(dest_port_pbmp, framer_mac_clients);
                        break;
                    }
                    case DNX_DIAG_FLEXE_BLOCK_BUSB_BKUP:
                    {
                        _SHR_PBMP_ASSIGN(dest_port_pbmp, framer_mac_clients);
                        bkup_index = i;
                        break;
                    ...
                    default:
                    {
                        SHR_CLI_EXIT(_SHR_E_INTERNAL, "The block type is not supported! \n");
                    }
                }
                _SHR_PBMP_ITER(dest_port_pbmp, dest_logical_port)
                {
                    if (dst_info[i].block_id == DNX_DIAG_FLEXE_BLOCK_VIRTUAL_FLEXE)
                    {
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_diag_flexe_gen2_per_port_type_dump(unit, prt_ctr, DNX_DIAG_FLEXE_FLOW_ETH_L1));
    SHR_IF_ERR_EXIT(dnx_diag_flexe_gen2_per_port_type_dump(unit, prt_ctr, DNX_DIAG_FLEXE_FLOW_FLEXE_CLIENT));
    SHR_IF_ERR_EXIT(dnx_diag_flexe_gen2_per_port_type_dump(unit, prt_ctr, DNX_DIAG_FLEXE_FLOW_FRAMER_MAC));
    SHR_IF_ERR_EXIT(dnx_diag_flexe_gen2_per_port_type_dump(unit, prt_ctr, DNX_DIAG_FLEXE_FLOW_FRAMER_SAR));
    ...
    prt_control_t *prt_ctr)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_FLEXE_PHY, 0, &flexe_phys));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_diag_flexe_groups_get(unit, &group_ports));
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_FLEXE_PHY, 0, &phy_ports));
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_FLEXE_CLIENT, 0, &client_ports));
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_MAC, 0, &client_macs));
