    ...
    for (presel_index = 0; presel_index < max_nof_cs_lines; presel_index++)
    {
        ...
        if (context_id != DNX_FIELD_CONTEXT_ID_DEFAULT(unit))
        {
            ...
            for (presel_qual_index = 0; presel_qual_index < entry_data.nof_qualifiers; presel_qual_index++)
            {
                ...
                {
                    DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(presel_quals_string, "%s", ", ");
                    sal_strncat_s(presel_quals_string, ", ", sizeof(presel_quals_string));
                }
        ...
        else
        {
            ...
            else
            {
                ...
                sal_strncpy_s(presel_quals_string, "Always Hit", sizeof(presel_quals_string));
            }
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_lookup_fwd_contexts(unit, apptype, &fwd_nof_contexts, fwd_contexts));
    SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_acl_contexts(unit, apptype, &acl_nof_contexts, acl_contexts));
