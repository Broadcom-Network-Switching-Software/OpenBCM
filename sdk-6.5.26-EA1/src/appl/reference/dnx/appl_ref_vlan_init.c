    ...
    int unit)
{
    ...
    bcm_gport_t gport;
    int system_port_id;
    bsl_severity_t orig_stk_severity;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (nof_port)
    {
        ...
        BCM_PBMP_ITER(pbmp_eth, logical_port)
        {
            if (BCM_PBMP_MEMBER(port_config.oamp, logical_port))
                continue;
            /** Suppress warning printout from sysport_get() */
            orig_stk_severity = bslenable_get(bslLayerBcmdnx, bslSourceStk);
            bslenable_set(bslLayerBcmdnx, bslSourceStk, bslSeverityError);
            bcm_stk_gport_sysport_get(unit, logical_port, &system_port_id);
            bslenable_set(bslLayerBcmdnx, bslSourceStk, orig_stk_severity);
