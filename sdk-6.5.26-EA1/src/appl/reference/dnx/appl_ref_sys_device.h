...
#define APPL_DNX_NOF_MODIDS 8192
...
typedef struct
{
    ...
    /**
     * Maximal ports per modid, used to calculate scheduling element IDs (such as voq connectors).
     * By default it is set to 256 (same as the amount of total PP_DSP's per modid), but in order to save resources
     * this attribute can be overridden by the application using - 'appl_param_nof_ports_per_modid'
     * When user chooses to override this property the total amount of ports per modid and their value (pp_dsp id) has
     * to be smaller than this value.
     *
     * \note - this property must not be overridden in case the system mixes dnx and dpp devices since dpp assumes 256
     *  modid's per device (the default amount)
     */
    ...
    /**
     * Maximal amount of cores per device in the system, will be used in order to allocate symmetric voq connectors
     * to remote cores (assuming the flow regions are configured with the same amount of remote cores)
     */
    int max_nof_cores;
 ...
 *
 */
shr_error_e appl_dnx_fap_and_pp_dsp_to_modid_convert(
    int unit,
    int fap_index,
    int core_id,
    int pp_dsp,
    int *modid);
/**
 * \brief - Convert modid to FAP index
 *
 */
 ...
 *
 */
shr_error_e appl_dnx_fap_and_pp_dsp_to_modport_convert(
    int unit,
    int fap_index,
    int core_id,
    int pp_dsp,
