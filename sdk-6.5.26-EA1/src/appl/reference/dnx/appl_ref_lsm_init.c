    ...
    xml_node qualifier_db_node)
{
    ...
    RHDATA_ITERATOR(current, qualifier_db_node, "qualifier")
    {
        ...
        for (attr_index = 0; attr_index < MAX_NOF_QUAL && attrs[attr_index].name[0] != '\0'; ++attr_index)
        {
            ...
            else if (qulifier_fields[qual_index].qualifier_type == lsm_qualifier_signal)
            {
                ...
                {
                    sal_strncpy_s(qulifier_fields[qual_index].qualifier_command.sig.signal_to, attrs[attr_index].value,
                                  LSM_SIG_TO_STRING_SIZE);
                }
            ...
            else if (qulifier_fields[qual_index].qualifier_type == lsm_qualifier_register)
            {
            ...
            for (index = 0; index < LSM_MAX_TAG_ATTR; ++index)
            {
                ...
                else
                {
                    ...
                    if (sal_strlen(attrs[index].value) > 2 && !isdigit((unsigned) attrs[index].value[0]))
                    {
                        ...
                        entry.qualifiers.list_of_qualifiers[entry.qualifiers.nof_qualifiers].qualifier_value = value;
                    }
                    /**
