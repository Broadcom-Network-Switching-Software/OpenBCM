    ...
    int port)
{
    ...
    if (in_flags & APPL_DNX_DYN_PORT_USE_SYS_PORT_MAP)
    {
        port_is_local = (in_sys_port_map->device_index == sys_params->unit_to_device_index[unit]);
        sys_port_map = *in_sys_port_map;
        system_port_id = sys_port_map.system_port_id;
        /** when dynamic port is using APPL_DNX_DYN_PORT_USE_SYS_PORT_MAP it is assumed that
         * it does not need to be filtered by APPL_DNX_FILTERED_PORTS_FLAGS, this cannot be checked
         * directly by bcm_port_get because the port is optionally not local to this FAP */
        flags = 0;
    }
    else
    {
        /** port is assumed to always be local if flag was not used */
        port_is_local = TRUE;
    ...
    if (!(flags & APPL_DNX_FILTERED_PORTS_FLAGS))
    {
        ...
        /** this is done as part of dynamic port create and not destroy because we currently maintin a DB per CPU and not per FAP. so when
