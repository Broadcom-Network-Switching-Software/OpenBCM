    ...
    int unit)
{
    ...
    if ((init_param[unit].no_deinit == 0) && (init_param[unit].no_init == 0))
    {
        ...
        /*
         * soc_cm_device_destroy() will destroy the unit to bde_dev mapping, so keep the bde dev fore recreating the
         * unit by appl_dnxc_sdk_init() 
         */
