...
int run_phy_ext_lb_prbs(int unit, int port1, int port2, bcm_port_prbs_t poly)
{
    ...
    int lb_orig, linkscan_interval_orig;
    bcm_port_config_t config;
    bcm_info_t info;
    ...
    rv = bcm_linkscan_enable_get(unit, &linkscan_interval_orig);
    if(rv != 0) {
        printf("Error, in bcm_linkscan_enable_get, unit %d, rv=%d, \n", unit, rv);
        return rv;
    }

    rv = bcm_linkscan_enable_set(unit, 0);
    if(rv != 0) {
        printf("Error, in bcm_linkscan_enable_set, unit %d, rv=%d, \n", unit, rv);
        return rv;
    }
    ...
    rv = bcm_linkscan_enable_set(unit, linkscan_interval_orig);
    if(rv != 0) {
        printf("Error, in bcm_linkscan_enable_set, unit %d, rv=%d, \n", unit, rv);
        return rv;
    }
