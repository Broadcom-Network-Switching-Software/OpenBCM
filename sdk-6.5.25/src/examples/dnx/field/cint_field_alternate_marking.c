...
int cint_field_alternate_marking_ingress_node_iPMF1(int unit)
{
    ...
    rv = bcm_field_group_add(unit, 0, &cint_field_am_ipmf1_ing_node_fg_info, &cint_field_am_ingress_fg_id);
    if (rv != BCM_E_NONE)
    {
    ...
    if (tod_append_mode ==2)
    {
         ...
         */
        rv = appl_dnx_field_flow_id_init(unit,cint_field_am_ingress_ctx_id);
        if (rv != BCM_E_NONE)
        {
...
int cint_field_alternate_marking_ingress_node_ePMF(int unit)
{
    ...
    */
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &cint_field_am_egress_fg_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
...
int cint_field_alternate_marking_inter_or_egress_node_iPMF1(int unit, int is_inter_pe)
{
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &cint_field_am_i_o_e_iPMF1_fg_id);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in bcm_field_group_add\n", rv);
       return rv;
...
int cint_field_alternate_marking_inter_or_egress_node_iPMF2_fg_add(int unit)
{
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &cint_field_am_i_o_e_iPMF2_flow_fg_id);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in bcm_field_group_add\n", rv);
       return rv;
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &cint_field_am_i_o_e_iPMF2_D_fg_id);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in bcm_field_group_add\n", rv);
       return rv;
    ...
    int unit)
{
    ...
    if (ifit_enable && cint_field_am_i_pe_fieh_ext_qual_id[0] == 0) {
        ...
        for (idx = 0; idx < nof_fieh_ext; idx ++) {
            ...
            rv = bcm_field_qualifier_create(unit, 0, &qual_info, (cint_field_am_i_pe_fieh_ext_qual_id + idx));
            if (rv != BCM_E_NONE)
            {
                printf("Error (%d), in bcm_field_qualifier_create Flow bit \n", rv);
                return rv;
