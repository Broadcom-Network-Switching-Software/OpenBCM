    ...
    bcm_field_group_info_t *fg_info_p)
{
    ...
    proc_name = "cint_field_external_entry_update";
     ...
     * Call the attach API
     */
    rv = bcm_field_entry_add(unit, BCM_FIELD_FLAG_UPDATE, external_fg_id, &entry_info, &entry_id);
    if (rv != BCM_E_NONE)
    {
        void *dest_char;
        char *fg_name;
        ...
        printf("%s Error (%d), in bcm_field_entry_add update fg_id %d fg_name %s \n", proc_name, rv, external_fg_id, fg_name);
        return rv;
    }
