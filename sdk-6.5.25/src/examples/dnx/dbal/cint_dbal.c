    ...
    int unit)
{
    ...
    dnx_dbal_entry_value_field_arr32_set(unit, entry_handle_id, "ESEM_COMMAND", inst_id, &set_val[1]);
    ...
    dnx_dbal_entry_value_field_symbol_set(unit, entry_handle_id, "OUT_PP_PORT_EGRESS_FORWARD_CODE_SELECTION_PROFILE", inst_id, "STACKING_PORT");
    ...
    dnx_dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, "ESEM_COMMAND", inst_id, &get_val[1]);
    ...
    dnx_dbal_entry_handle_value_field_symbol_get(unit, entry_handle_id, "OUT_PP_PORT_EGRESS_FORWARD_CODE_SELECTION_PROFILE", inst_id, &symbol_name[0]);
     ...
     */
    set_val[4] = 0x6;
    dnx_dbal_entry_value_field_symbol_set(unit, entry_handle_id, "RESULT_TYPE", inst_id, "ETPS_AC_STAT");
    ...
    */
    ...
    if ((get_val[0][0] != 0xa) || (get_val[1][0] != 0xb) || (get_val[2][0] != 0xc) || (get_val[3][0] != 0xd) || (get_val[4][0] != 0x6) ||
        (sal_strncmp(symbol_name, "STACKING_PORT",sizeof(symbol_name))))
    {
        printf("Compare Error: got %x,%x,%x,%x,%x,%s,%s expected 0xa,0xb,0xc,0xd,0x6,STACKING_PORT,ETPS_AC_STAT\n",
    ...
    {
        printf("Compare MATCH: got %x,%x,%x,%x,%x,%s,%s expected 0xa,0xb,0xc,0xd,0x6,STACKING_PORT,ETPS_AC_STAT\n",
                get_val[0][0],get_val[1][0],get_val[2][0],get_val[3][0],get_val[4][0],symbol_name,result_type_name);
        return rv;
    }
}
