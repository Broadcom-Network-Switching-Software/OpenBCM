    ...
    bcm_field_context_t  context_id)
{
    ...
    rv = bcm_field_group_add(unit, 0, fg_info, &cint_rx_trap_erpp_epmf_trap_action_fg_id);
    if(rv != BCM_E_NONE)
    {
    ...
    rv = bcm_field_group_context_attach(unit, 0, cint_rx_trap_erpp_epmf_trap_action_fg_id, context_id, &attach_info);
    if(rv != BCM_E_NONE)
    {
    ...
    bcm_field_context_t  context_id)
{
    ...
    rv = bcm_field_group_context_detach(unit, cint_rx_trap_erpp_epmf_trap_action_fg_id, context_id);
    if(rv != BCM_E_NONE)
    {
    ...
    bcm_field_context_t context_id)
{
    ...
    rv = cint_erpp_pmf_trap_action_pmf_configuration_create(unit, context_id);
    if(rv != BCM_E_NONE)
    {
      printf("Error in egress_pmf_trap_create_and_set \n");
      return rv;
