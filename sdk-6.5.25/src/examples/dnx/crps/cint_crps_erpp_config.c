...
int ERPP_DATA_MAPPING_ARRAY_MAX = 16;        /* defines the max number of entries for data mapping table for all examples in
                                         * this file */
    ...
    int expansion_type_flag)
{
    ...
    bcm_stat_expansion_data_mapping_t data_mapping_array[ERPP_DATA_MAPPING_ARRAY_MAX];
    int counter_set_size;
    ...
    crps_erpp_config_database_t create_db)
{
    ...
    int command_id = 1;
    bcm_stat_counter_format_type_t format_type = bcmStatCounterFormatDoublePackets; /**As configured in crps_erpp_data_base_create*/
    int nof_counters;
    uint32 *engine_size;
    ...
    nof_counters = crps_engine_nof_counters_get(unit, engine_arr[0], format_type) + crps_engine_nof_counters_get(unit, engine_arr[1], format_type);
