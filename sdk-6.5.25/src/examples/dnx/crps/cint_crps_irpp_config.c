...
int IRPP_DATA_MAPPING_ARRAY_MAX = 1024;      /* defines the max number of entries for data mapping table for all examples in
                                         * this file */
    ...
    int expansion_type_flag)
{
    ...
    bcm_stat_expansion_data_mapping_t data_mapping_array[IRPP_DATA_MAPPING_ARRAY_MAX];
    int counter_set_size;
    ...
    crps_irpp_config_database_t create_db)
{
    ...
    for (index = 0; index < 4; index++)
    {
        ...
        /** configure type 2 */
        engine_size = crps_engine_nof_counters_get(unit, engine_arr[2], format_type);
