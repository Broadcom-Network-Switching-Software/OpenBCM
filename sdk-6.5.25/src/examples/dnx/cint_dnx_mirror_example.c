...
*/
/*
 * Example of mirror into various tunnels
 * Test Scenario 
 *
 * ./bcm.user
...
/* put ERSPAN tunnel/IP tunnel/ARP into seperate outlif extensions */
int G_mirror_erspan_separate_outlifs = 0;
...
int mirror_rspan_example(int unit, int in_port, int in_vlan, int out_port, int mirror_port, int rspan_tag) {
    ...
    */
    ...
    if (mirror_info.udh_enable)
    {
        for (ii=0; ii<(sizeof(mirror_info.udh_size)/sizeof(mirror_info.udh_size[0])); ++ii)
        {
            mirror_header_info.udh[ii].size = mirror_info.udh_size[ii];
            mirror_header_info.udh[ii].data = mirror_info.udh_data[ii];
        }
    }
...
uint32 get_mirror_incoming_tag_structure(int unit, uint32 llvp_profile){
    ...
    */
    ...
    /* Get the SW-ID for this HW-ID */
    dnx_dbal_entry_handle_take(unit, "TAG_STRUCT_INGRESS_HW_ID_TO_SW_ID", &entry_handle_id);
    key_field = get_val[0][0];
    dnx_dbal_entry_key_field_arr32_set(unit, entry_handle_id, "INCOMING_TAG_STRUCTURE", &key_field);
    dnx_dbal_entry_get(unit, entry_handle_id);
    dnx_dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, "BCM_TAG_FORMAT_CLASS_ID", inst_id, &get_val[0]);
...
int mirror_erspan_tunnel_create(int unit, bcm_if_t *encap_id) {
    ...
    if (G_mirror_erspan_separate_outlifs == 0)
    {
    ...
    */
    ...
    {
        /** ARP, minimum 14 */
        erspan_tunnel.estimated_encap_size = 14;

        /**
          * IPv4/IPv6
          * If IP tunnel is not linked to ERSPAN tunnel, then it will be counted separately.
          */
        erspan_tunnel.estimated_encap_size += G_mirror_erspan_separate_outlifs ? 0 : (G_mirror_tunnel_is_ipv6 ? 40 : 20);

        /** GRE8 */
        erspan_tunnel.estimated_encap_size += 8;

        /** ERSPANv2/v3 */
        erspan_tunnel.estimated_encap_size += erspan_info.is_erspan_v3 ? 20:8;
    }
...
int mirror_erspan_destination_create(int unit, int in_port, bcm_vlan_t in_vlan, int out_port, int mirror_port, int is_mc, int mc_id, bcm_if_t encap_id) {
    ...
    if (is_mc == TRUE) {
        ...
        if (G_mirror_erspan_separate_outlifs != 0)
        {
            BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(mirror_header_info.pp.out_vport_ext[0], erspan_info.ip_tunnel_intf.l3a_intf_id);
            BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(mirror_header_info.pp.out_vport_ext[1], erspan_info.next_hop_if);
        }
    }
    ...
    if (rv = bcm_mirror_header_info_set(unit, mirror_info.flags, dest.mirror_dest_id, &mirror_header_info)) {
        printf("Error: bcm_mirror_header_info_set %d\n", rv);
        return rv;
    }
...
int mirror_egress_probabiliy_sample_fg_config(int unit)
{
    ...
    if (rv = bcm_field_group_add(unit, 0, &fg_info, &fg_id))
    {
    ...
    /**
     * Mirror code offset in signal bus.
     */
...
int mirror_egress_probabiliy_sample_clean_up(int unit)
{
 ...
 /*
  * Mirroring into a VxLAN tunnel
  *
  * - in_port : port that packets to be recieved from
  * - out_port : port that packets to be transmitted out
  * - mirror_port : mirror destination port
  * - mirror_port : mirror destination port
  * - access_port : VxLAN access port
  * - provider_port : VxLAN provider port
  ...
  cint sand/utility/cint_sand_utils_vxlan.c
  cint sand/utility/cint_sand_utils_l3.c
  cint dnx/field/cint_field_utils.c
  cint dpp/cint_port_tpid.c
  cint dpp/cint_advanced_vlan_translation_mode.c
  cint sand/cint_sand_field_fec_id_align.c
  cint sand/cint_ip_route_basic.c
  cint sand/cint_sand_ip_tunnel.c
  cint sand/cint_sand_vxlan.c
  cint dnx/dbal/cint_dbal.c
  cint dnx/cint_dnx_mirror_example.c
  cint dnx/crps/cint_crps_oam_config.c
  cint
  mirror_vxlan_example(0,200,201,202,13,14);
  exit;

  debug bcm rx
  tx 1 l=128 psrc=200 data=000000000002000000000001810000140899000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20

  Expect to receive mirror copy with VxLAN encapsulation on port 202:

  00000000 cd1d000c 00020000 8100800a 08004528 00a00000 00003c11 1ce5a00f
  1011a10f 101161e8 12b5008c 00000800 00000013 88000000 00000002 00000000
  00018100 00140899 00010203 04050607 08090a0b 0c0d0e0f 10111213 14151617
  18191a1b 1c1d1e1f 20000000 00000000 00000000 00000000 00000000 00000000
  00000000 00000000 00000000 00000000 00000000 00000000 00000000 00000000
  00000000 00000000 00000000 00000000 0000

  */
int mirror_vxlan_example(int unit, int in_port, int out_port, int mirror_port, int access_port, int provider_port) {
    bcm_mirror_header_info_t mirror_header_info;
    int rv;
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in vxlan_example\n", rv);
        return rv;
    }

    mirror_info.vlan = 20;
    rv = mirror_span_example(unit, in_port, out_port, mirror_port);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in mirror_span_example\n", rv);
        return rv;
    }

    bcm_mirror_header_info_t_init(&mirror_header_info);
    mirror_header_info.tm.src_sysport = in_port;
    mirror_header_info.tm.out_vport = ip_tunnel_basic_info.encap_tunnel_id[0];
    mirror_header_info.pp.vsi = cint_vxlan_basic_info.vpn_id;
    rv = bcm_mirror_header_info_set(unit, 0, mirror_info.mirror_dest_id, &mirror_header_info);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_mirror_header_info_set\n", rv);
        return rv;
    }
 ...
 /*
 * Clean up procedure for mirror_vxlan_example
 */
int mirror_vxlan_example_clean_up(int unit) {
    return mirror_span_example_clean_up(unit);
}
