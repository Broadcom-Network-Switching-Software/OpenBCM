    ...
    int payload)
{
    ...
    {
        rv = add_ipv6_ipmc(unit, cint_bier_info.ipv6_dip, full_mask, compressed_vrf?full_mask_zero:cint_bier_info.ipv6_sip, 
                           compressed_vrf?full_mask_zero:full_mask, in_lif, cint_bier_info.vrf, 0, 0, 
                           cint_bier_info.native_mc_id, 0, 0);
        if (rv != BCM_E_NONE)
        {
    ...
    int mode)
{
    ...
    if (node_type == BFIR)
    {
        ...
        {
            replications[0].encap1 = cint_bier_info.bfr1_bift_initator_id & 0xffffff;
        }
    ...
    else if (node_type == BFER)
    {
        ...
        rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|flag|BCM_MULTICAST_BIER_256_GROUP, &cint_bier_info.bier_mc_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_create \n");
            return rv;
        ...
        rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID|flag, &cint_bier_info.native_mc_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_create \n");
            return rv;
        ...
        /*MCDB setting*/
        rv = bcm_multicast_set(unit, cint_bier_info.native_mc_id, flag, 2, replications);
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_multicast_set\n");
            return rv;
    ...
    int payload)
{
    ...
    if (node_type == BFIR)
    {
        ...
        if (!bit_str_in_mcdb) 
        {
            ...
            flow_id = flow_handle_info.flow_id &= 0xffffff;
            BCM_L3_ITF_SET(intf_id, BCM_L3_ITF_TYPE_LIF, flow_id);
...
{    
    ...
    int flow_id = flow_handle_info.flow_id &= 0xffffff;
    BCM_L3_ITF_SET(intf_id, BCM_L3_ITF_TYPE_LIF, flow_id);
