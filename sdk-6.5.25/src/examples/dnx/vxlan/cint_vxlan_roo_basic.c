    ...
    int core_port)
{
    ...
    rv = vlan_translate_ive_eve_translation_set_with_pri_action(unit, vlan_port.vlan_port_id,        /* outlif                  */
                                                                      0x8100,                        /* outer_tpid              */
                                                                      0,                             /* inner_tpid              */
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in vlan_translate_ive_eve_translation_set_with_pri_action\n", proc_name);
        return rv;
    }
     ...
     */
    rv = vxlan_port_add(unit, core_port, term_tunnel_id, &vxlan_port_id);
    if (rv != BCM_E_NONE) 
    {
    ...
    int unit)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 1, 0, &fec_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
        return rv;
    ...
    rv = l3__egress_only_fec__create(unit, cint_vxlan_roo_basic_info.core_native_fec_id,
                                     ...
                                     cint_vxlan_roo_basic_info.core_native_arp_id, gport, flags, 0);
    ...
    rv = l3__egress_only_fec__create(unit, cint_vxlan_roo_basic_info.access_fec_id,
                                     ...
                                     cint_vxlan_roo_basic_info.access_port, flags, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    bcm_gport_t *vxlan_port_id)
{
    ...
    rv = bcm_vxlan_port_add(unit, cint_vxlan_roo_basic_info.vpn_id, &vxlan_port);
    if(rv != BCM_E_NONE) {
    ...
    *vxlan_port_id = vxlan_port.vxlan_port_id;
    ...
    rv = multicast__add_multicast_entry(unit, cint_vxlan_roo_basic_info.vpn_id, &core_port, &encap_id, 1, egress_mc);
    if (rv != BCM_E_NONE) {
            printf("Error, in multicast__add_multicast_entry mc_group_id\n");
            return rv;
    }
    ...
    int *encap_tunnel_id)
{
    ...
    *encap_tunnel_id = tunnel_init.tunnel_id;
    ...
    rv = bcm_port_class_set(unit, tunnel_init.tunnel_id, bcmPortClassForwardEgress, cint_vxlan_roo_basic_info.vxlan_network_group_id);
...
int vxlan_roo_modify_native_vlan_number(int unit, int nbr_native_vlans) {
    ...
    rv = bcm_vlan_translate_action_id_set(unit, flags, native_eve_action_id, &action);
    if (rv != BCM_E_NONE) {
    ...
    int unit)
{
    ...
    /* Delete VXLAN port */
    rv = bcm_vxlan_port_delete(unit, cint_vxlan_roo_basic_info.vpn_id, cint_vxlan_roo_basic_info.vxlan_port_id);
    if (rv != BCM_E_NONE)
    {
    ...
    /* Destroy VPN */
    rv = bcm_vxlan_vpn_destroy(unit, cint_vxlan_roo_basic_info.vpn_id);
    if (rv != BCM_E_NONE)
    {
 ...
 * is_ipv6_vxlan - indicates if VXLAN6 or VXLAN
 */
    ...
    int access_port)
{
    ...
    proc_name = "vxlan_roo_nni2nni_example";

    vxlan_roo_basic_init(access_port, core_port_in);
     ...
     */
    rv = out_port_set(unit, core_port_out);
    if (rv != BCM_E_NONE)
    {
        printf("Error, out_port_set intf_out\n");
        return rv;
    ...
    rv = intf_eth_rif_create(unit, egress_core_overlay_eth_rif, egress_core_overlay_eth_rif_mac);
    if (rv != BCM_E_NONE)
    {
        printf("Error, intf_eth_rif_create pwe_intf\n");
        return rv;
    ...
    /* Create multicast group */
    rv = multicast__open_mc_group(unit, &cint_vxlan_roo_basic_info.vpn_id, 0);
    if (rv != BCM_E_NONE) {
     ...
     */
    rv = vxlan_port_add(unit, core_port_in, term_tunnel_id, &vxlan_port_id);
    if (rv != BCM_E_NONE) 
    {
        printf("%s(): Error, vxlan_port_add \n", proc_name);
        return rv;
     ...
     * Configure native ARP entry 
     */
    rv = l3__egress_only_encap__create_inner(unit, 0, &egress_core_native_arp_id,egress_core_native_next_hop_mac,0,BCM_L3_NATIVE_ENCAP,0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create AC egress object ARP only\n");
        return rv; 
     ...
     * Configure overlay ARP entry
     */
    rv = l3__egress_only_encap__create(unit, 0, &egress_core_overlay_arp_id,egress_core_overlay_next_hop_mac,egress_core_overlay_eth_rif);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create AC egress object ARP only\n");
        return rv;
    ...
    rv = vxlan_tunnel_initiator_create(unit, is_ipv6_vxlan, egress_core_overlay_arp_id, &(cint_vxlan_roo_basic_info.core_overlay_encap_tunnel_intf), &encap_tunnel_id);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in function tunnel_initiator_configure \n", proc_name);
        return rv;
    ...
    rv = l3__egress_only_fec__create(unit, egress_core_native_fec_id,
                                     cint_vxlan_roo_basic_info.core_native_eth_rif,
                                     egress_core_native_arp_id, gport, 0, 0);
    ...
    rv = l3__egress_only_fec__create(unit, egress_core_overlay_fec_id,
                                     cint_vxlan_roo_basic_info.core_overlay_encap_tunnel_intf,
                                     0, gport, BCM_L3_CASCADED, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error in creating overlay FEC for routing into overlay\n");
        return rv;
     ...
     */
    rv = add_host_ipv4(unit, dip_v4, cint_vxlan_roo_basic_info.tunnel_termination_vrf, egress_core_native_fec_id, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in add_host_ipv42, \n");
        return rv;
     ...
     */
    rv = add_route_ipv6(unit, dip_v6, cint_vxlan_roo_basic_info.maskv6_dip_out_of_overlay, cint_vxlan_roo_basic_info.tunnel_termination_vrf, egress_core_native_fec_id);
    if (rv != BCM_E_NONE)
    {
      printf("%s(): Error, in function add_route_ipv6, err_id = %d \n", proc_name, rv);
      return rv;
    ...
    int core_port)
{
    ...
    rv = vxlan_roo_basic(unit, is_ipv6_vxlan, access_port, core_port);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, vxlan_roo_basic \n", proc_name);
        return rv;
    }

    /* Ingress L3 profile and mapping */
    rv = qos_map_l3_ingress_profile(unit, is_ipv6_fwd);
    if (rv != BCM_E_NONE) {
        printf("%s(): Error: in qos_map_l3_ingress_profile\n", proc_name);
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_ingress_rif_set ingr_itf_access \n", proc_name);
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_ingress_rif_set ingr_itf_native_overlay \n", proc_name);
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error: in qos_l2_egress_map_create\n", proc_name);
        return rv;
    }
    ...
    rv = bcm_qos_port_map_set(unit, gport, -1, l2_eg_map_id);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in bcm_qos_port_map_set\n", proc_name);
        return rv;
    }
    ...
    rv = bcm_qos_port_map_set(unit, vlan_port.vlan_port_id, -1, l2_eg_map_id);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in bcm_qos_port_map_set\n", proc_name);
        return rv;
