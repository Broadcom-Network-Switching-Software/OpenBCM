        ...
        int * rate_class_get)
{
    ...
    if(rv != BCM_E_NONE)
    {
        printf("Error in appl_dnx_e2e_scheme_logical_port_to_base_voq_get\n");
        return rv;
    }

    BCM_GPORT_UNICAST_QUEUE_GROUP_SET(&voq_gport, base_voq);
    voq_profile_params.profile_type = bcmCosqIngressQueueToRateClass;
    rv = bcm_cosq_profile_mapping_get(unit, voq_gport, 0 /*cosq*/, 0 /*flags*/, &voq_profile_params);
    if(rv != BCM_E_NONE)
    {
        printf("Error in bcm_cosq_profile_mapping_get\n");
        return rv;
    }
        ...
        int voq_size)
{
    ...
    uint32 flags;
    int dp, rv = BCM_E_NONE, cosq = 0, rate_class, base_voq;
    bcm_gport_t rate_class_gport, port_gport, voq_gport;
    bcm_cosq_gport_size_t bytes_fadt_drop_params;
    bcm_switch_profile_mapping_t voq_profile_params;

    /** get voq's rate class*/
    rv = latency_port_rate_class_get(unit, out_port, &rate_class);
    if(rv != BCM_E_NONE)
    {
        printf("Error in ingress_latency_port_rate_class_get\n");
        return rv;
    }
    ...
    /** Set queue size*/
    {
        ...
        for(dp = 0; dp < 4; dp++)
        {
            rv = bcm_cosq_gport_color_size_set(unit, rate_class_gport, cosq, dp, flags, &bytes_fadt_drop_params);

            if(rv != BCM_E_NONE)
            {
                printf("Error in bcm_cosq_gport_color_size_set\n");
                return rv;
            }
        }
    }
