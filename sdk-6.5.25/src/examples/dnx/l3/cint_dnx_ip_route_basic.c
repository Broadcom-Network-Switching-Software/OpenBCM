    ...
    int out_port)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(default_vlan_port_id_out) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(default_vlan_port_id_out)) {
        bcm_vlan_port_t_init(&vlan_port);
    ...
    if  (*dnxc_data_get(unit, "flow", "vlan_port", "flow_in_use", NULL) != 1)
    {
    ...
    int out_port)
{
     ...
     * 5. Create ARP and set its properties
     */
    rv = create_arp_plus_ac_type(unit, 0, arp_next_hop_mac, &encap_id, vsi);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create_arp_plus_ac_type\n", proc_name);
        return rv;
