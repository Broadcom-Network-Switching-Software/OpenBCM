    ...
    int unit)
{
     ...
     */
    evpn_global_info.not_ready = *dnxc_data_get(unit, "flow", "mpls", "egress_flow_in_use", NULL);
    ...
    int with_stat)
{
    ...
    /*
     * evpn_global_info.mpls_term_till_bos_en == 0: Set In-LIF profile for iPMF selection of ESI filter application
     * evpn_global_info.mpls_term_till_bos_en == 1: Not needed, since iPMF qualifies IML-range-Qual
     */
    if ((expect_esi) && (!evpn_global_info.mpls_term_till_bos_en))
    {
    ...
    int vsi)
{
    ...
    if (!evpn_global_info.mpls_term_till_bos_en) {
        /**If the feature mpls_term_till_bos is available, no need to add the 2nd IML term entry.*/
