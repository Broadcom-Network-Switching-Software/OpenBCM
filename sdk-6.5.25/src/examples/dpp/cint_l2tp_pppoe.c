...
int del_session_attr_uni(int unit, int z, bcm_port_t port, int idx)
{
	...
	if(PppoeTerminatorGport[z][idx]) 
	{
		...
		if(rc)
		{
			printf("Error-del_session_attr_uni-10: rc=%d\n", rc); return rc;
		}
		PppoeTerminatorGport[z][idx] = 0;
    }
	if(Ac[z][idx].vlan_port_id)
	{
		printf("Delete pppoe ac gport, gport=0x%x\n", Ac[z][idx].vlan_port_id);
		rc = bcm_vswitch_port_delete(unit, Vsi[z], Ac[z][idx].vlan_port_id);
		if(rc)
        {
            printf("Error bcm_vswitch_port_delete: rc=%d\n", rc);
            return rc;
        }
		rc = bcm_vlan_port_destroy(unit, Ac[z][idx].vlan_port_id);
		if(rc)
        {
            printf("Error bcm_vlan_port_destroy: rc=%d\n", rc);
            return rc;
        }
    }
...
/**l2tp terminator delete and then recreate test.*/
int l2tp_terminator_delete_and_recreate(int unit, int z, int idx)
{
    int rv;
    bcm_ppp_terminator_t pt;
    ...
    pt.ppp_terminator_id = L2tpTerminatorGport[z][idx];
    pt.type  = bcmPPPTypeL2TPv2;
    rv = bcm_ppp_terminator_delete(unit, &pt);
    if (rv != BCM_E_NONE) {
        printf("bcm_ppp_terminator_delete failed!\n");
        return rv;
    }
    L2tpTerminatorGport[z][idx] = 0;

    /**this one to occupy postion*/
    sal_memset(&pt, 0, sizeof(pt));
    pt.type = bcmPPPTypeL2TPv2;
    pt.session_id = L2tpSession[idx]+1;
    pt.l2tpv2_tunnel_id = L2tpTunnel+1;
    pt.network_domain = L2tpVrf+1;
    pt.flags =  BCM_PPP_TERM_WIDE  | BCM_PPP_TERM_CROSS_CONNECT;
    rv = bcm_ppp_terminator_create(unit, &pt);
    if (rv != BCM_E_NONE) {
        printf("bcm_ppp_terminator_create failed!\n");
        return rv;
    }

    /**this one is the expected one*/
    sal_memset(&pt, 0, sizeof(pt));
    pt.type = bcmPPPTypeL2TPv2;
    pt.session_id = L2tpSession[idx];
    pt.l2tpv2_tunnel_id = L2tpTunnel;
    ...
    if(L2tpTerminatorGport[z][idx] && PppoeFec[3][idx]) {
        rv = set_wide_lif_data_with_fec(unit, L2tpTerminatorGport[z][idx], PppoeFec[3][idx]);
        if (rv != BCM_E_NONE) {
            printf("set_wide_lif_data_with_fec failed!\n");
            return rv;
        }
    }
