...
int LATENCY_BASED_ECN_MARKING_DISABLED = 0;
int LATENCY_BASED_ECN_MARKING_ENABLE__PRGE_SOLUTION = 1;
int LATENCY_BASED_ECN_MARKING_ENABLE__ETPP_SOLUTION = 2;
...
int ecn_extract_field_group_set(/* in */ int unit,
                               /* in */ int flags,  /* Options: LATENCY_BASED_ECN_MARKING_ENABLE__PRGE_SOLUTION, LATENCY_BASED_ECN_MARKING_ENABLE__ETPP_SOLUTION, LATENCY_BASED_ECN_MARKING_DISABLED */
                                /* in */ int latency_flow_id, /* If flags=LATENCY_BASED_ECN_MARKING_ENABLE__ETPP_SOLUTION will be used as latency_flow_id */
                                ...
                                /* out */ bcm_field_group_t *group) 
{
  ...
  for (ipv6_tc_ndx = 0; ipv6_tc_ndx < 5; ipv6_tc_ndx++) {
    ...
    switch(flags) {
        case LATENCY_BASED_ECN_MARKING_ENABLE__PRGE_SOLUTION:
            BCM_FIELD_ASET_ADD(aset, bcmFieldActionOam);
        break;
        case LATENCY_BASED_ECN_MARKING_ENABLE__ETPP_SOLUTION:
  ...
  for (entry_iter = 0; entry_iter < ecn_cases_nof_entries; entry_iter++) {
      ...
      for (entry_iter_2 = 0; entry_iter_2 < nof_entries; entry_iter_2++) {
          ...
          switch(flags) {
              case LATENCY_BASED_ECN_MARKING_ENABLE__PRGE_SOLUTION:
                  /* Any packet with FTMH.ECN-Enable=1 and OAM_Subtype=7 will be processed by PRGE ECN program*/
                  /* Here we set the OAM sub type to 7 */
                  result = bcm_field_action_add(unit, ent, bcmFieldActionOam, 0x1c000, 0);
              ...
              case LATENCY_BASED_ECN_MARKING_ENABLE__ETPP_SOLUTION:
                  /* Latency OAM-TS */
                  /* use profile_id as latency_flow_id */
 ...
 */
int ecn_extract_example(int unit, int flags, int latency_flow_id) 
{
  int result;
  ...
  result = ecn_extract_field_group_set(unit, flags, latency_flow_id, group_priority, 2 /* 2b encoding */, &group);
  if (BCM_E_NONE != result) {
