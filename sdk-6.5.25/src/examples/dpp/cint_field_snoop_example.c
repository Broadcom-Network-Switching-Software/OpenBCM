...
/*
 * Cint porting from Customer case. Check TC/DP update in system header for snoop copy
 */
int pmf_snoop_update_tc_dp(int unit, bcm_port_t portsrc, bcm_port_t portdst)
{
    int rv = BCM_E_NONE;
    ...
    bcm_field_group_t fg_group = 20;
    bcm_field_entry_t entry_1;
    bcm_port_t portsrc_mask = 0xffffffff;
    int flags = 0;
    int snoop_cmd=5;
    ...
    int prio=4;
    rv = bcm_rx_snoop_create(unit, 0, &snoop_cmd);
    if (rv != BCM_E_NONE) {         
        printf("Error in bcm_rx_snoop_create\n");
        return rv;
    }
    ...
    /* Set the snoop destination to dest port */
    BCM_GPORT_LOCAL_SET(snoop_config.dest_port, portdst);
    snoop_config.prio = prio;
    snoop_config.color = 2;
    snoop_config.size = -1;
    snoop_config.probability = 100000;
    snoop_config.flags = BCM_RX_SNOOP_UPDATE_DEST | BCM_RX_SNOOP_UPDATE_PRIO | BCM_RX_SNOOP_UPDATE_COLOR;
    ...
    rv = bcm_rx_trap_type_create(unit, 0x0, bcmRxTrapUserDefine, &trap_id);
    if (rv != BCM_E_NONE) {
        printf("Error in bcm_rx_trap_type_create\n");
        return rv;
    }
    ...
    bcm_rx_trap_config_t_init(&trap_config);    
    trap_config.trap_strength = trap_dest_strength;
    BCM_GPORT_LOCAL_SET(trap_config.dest_port, portdst);
    trap_config.snoop_cmnd = snoop_cmd;
    trap_config.flags = BCM_RX_TRAP_UPDATE_PRIO | BCM_RX_TRAP_UPDATE_DEST |  BCM_RX_TRAP_TRAP;
    trap_config.prio = 7;
    ...
    printf("--- trap_create for priority --- \n");
    printf("        Trap GPORT=                          %d\n", snoop_trap_gport_id);
    printf("        Trap ID=                                %d\n", trap_id);
    printf("        trap_config.prio=                                %d\n", trap_config.prio);
    ...
    rv = bcm_field_qualify_InPort(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, portsrc, portsrc_mask);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_field_group_action_set(unit, fg.group, aset);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_field_qualify_InPort(unit,entry_1,portsrc,portsrc_mask);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_field_qualify_InPort\n");
        return rv;
    }

    rv = bcm_field_action_add(unit, entry_1, bcmFieldActionSnoop, snoop_trap_gport_id, 0);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_field_action_add\n");
        return rv;
    }
    printf("bcm_field_action_add with snoop_trap_gport_id=%d\n",snoop_trap_gport_id);
            
    rv = bcm_field_entry_install(unit, entry_1);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_field_entry_install\n");
        return rv;
    }
    rv = bcm_field_group_install(unit,fg.group);
    if (rv != BCM_E_NONE) {
