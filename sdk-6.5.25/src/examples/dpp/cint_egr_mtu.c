...
/*checking egq action profile max number*/
int egr_egq_action_profile_check(int unit)
{
    int rv = BCM_E_NONE;
    int num_cores;
    bcm_gport_t destinations[2];
    int flags = BCM_RX_TRAP_UPDATE_DEST;
    int trap_id;
    int i, j;
    bcm_rx_trap_config_t config;
    bcm_rx_trap_core_config_t arr_core[2];
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (num_cores > 1)
        {
            sal_memset(&config, 0, sizeof(config));
            config.flags = flags;
            config.trap_strength = 3;

            for (j = 0; j< num_cores; ++j)
            {
                arr_core[j].dest_port = destinations[j];
            }

            config.core_config_arr = arr_core;
            config.core_config_arr_len = num_cores;
        }
        else
        {
            sal_memset(&config, 0, sizeof(config));
        ...
        rv = bcm_rx_trap_type_create(unit, flags, bcmRxTrapEgPortNotVlanMember+i, &trap_id);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_rx_trap_type_create in loop[%d]\n", i);
            return rv;
        }

        rv = bcm_rx_trap_set(unit, trap_id, &config);
        if ((i < 4) && (rv != BCM_E_NONE)) {
            printf("Error, in bcm_rx_trap_set in loop[%d]\n", i);
            return rv;
        } else if ((i == 4) && (rv == BCM_E_NONE)) {
            printf("Error, in bcm_rx_trap_set in loop[%d]\n", i);
            return -1;
        } else if ((i == 4) && (rv != BCM_E_NONE)) {
            rv = BCM_E_NONE;
