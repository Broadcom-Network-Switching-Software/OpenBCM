/*~~~~~~~~~~~~~~~~~~~~~~~~~~ Priority Over Port Example (Arad_Plus) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
/*
 * 
 *
 ...
 * Purpose: Example for Arad_Plus application. Includes Priority Over Port Cosq Application.
 * Reference: BCM88660 TM Device Driver (Arad_Plus)
 *
 ...
 *  This cint directed to system with reduced resources (Arad_Plus).    
 ...
 * In Arad_Plus this port Interface should be TM_INTERNAL_PKT.
 ...
 *  2. Call arad_plus_appl_init() with the relevant system configuration, reference can be found in arad_plus_appl_exmaple():
       (this is not a specific example for a relevant system, but a general reference)
         ...
         arad_plus_appl_example(unit, nof_local_ports, mc_group_num, nof_isqs, cgm_thr_enable);
         quit;

 *  2a. Specific example for mgmt system can be found in arad_plus_mgmt_appl_example().
 ...
 *
         ...
         int nof_isqs = 512;
         int cgm_thr_enable = 1;
         arad_plus_mgmt_appl_example(unit, nof_local_ports, mc_group_num, nof_isqs, cgm_thr_enable);
         quit;
 ...
 * ========================
 *   The function arad_plus_appl_mc() demonstrate the use of BCM Multicast API. This function open several MC groups , according to mc_group_num. 
 ...
 *
 * Running the Application on Arad_Plus pcid:
 * ======================================
 *
 *   Application:
         cd ../../../src/examples/dpp
 ...
 *   ext_ram_present.BCM88650=0
 *   bcm886xx_ocb_enable.BCM88650=1             ==> this is the default, just validate
 *
 * Arad_Plus EMUL setup:
 * ===========
 * Enable to following SOC property:
 *
 * When running arad_plus on emulation it is advised to use the following(to save time due to the slow pace of the emulator) :
    cint cint_tm_prio_o_port_reduce_resources.c
    cint
    int unit = 0;
    int nof_local_ports = 6;
 ...
 * Arad_Plus FC:
 * =========
 * Arad_Plus recieves FCV (Flow Control Vector) with the size of 256 bit.
 * FCV vector can be mapped only to CL's 0-255.
 * During Arad_Plus init the table CFC_NIF_SHR_MAP is configured.
 *
 ...
 * Using BCM API each bit in FCV can be mapped to a d different CL (0-255). example can be found in:\
       ...
       int num_tm_ports = 28;
       int num_cos = 8;
       arad_plus_fc_shr_example(unit, num_tm_ports, num_cos);
       quit;
 *
 ...
 * ISQ configuration:
 *   isq_bw = 2500000;
 *   rv = arad_plus_shaping_configuration(unit, nof_isqs, MAX_COS, isq_bw);
 *   if (BCM_FAILURE(rv)) {
 *       printf("arad_plus_shaping_configuration: failed. Error:%d (%s) \n", rv, bcm_errmsg(rv));
 *       return rv;
 *   }
 * 
 
 * Arad_Plus GCM example:
 * ==================
 * This function responsible for configuring Arad_Plus per port thresholds according arad_plus_egq_thresholds.xls.
 ...
 * call to the application
 * rv = arad_plus_thr_example(unit, tm_internal_pkt_port, MAX_COS);
 * if (BCM_FAILURE(rv)) {
 *      printf("arad_plus_thr_example: failed. Error:%d (%s) \n", rv, bcm_errmsg(rv));
 *      return rv;
 *  }
 */
int MAX_COS = 8;
...
int appl_arad_plus_stk_init(int unit)
{
    int 
        rv = BCM_E_NONE,
        idx,
 ...
 */
int appl_arad_plus_cosq_init(int unit, int modid, int nof_units, int* unit_arr, int* modid_arr)
{
    int rv = BCM_E_NONE;
    int unit_idx;   
...
/*
 * Purpose: Initialize Arad_Plus Application.
 ...
 * - Run the Cosq Application (Scheduling scheme) (cosq_init)
 * - Add Ports to the main Arad_Plus interface (appl_arad_plus_stk_diag_init)
 * Parameters:
 *  - unit:
 *  - modid:
 *  - tm_internal_pkt_port - Main Arad_Plus interface local port number
 *  - nof_local_ports - Number of local ports map to the Main Arad_Plus port 
 *  - nof_units: Number of devices in the system. cannot be above 8
 *  - unit_arr: An array with unit ids of the rest of units controlled by this CPU. 
 *  - modid_arr: An array with mod ids of the rest of units controlled by this CPU. 
 */
int arad_plus_appl_init(int unit, bcm_port_t tm_internal_pkt_port, int nof_local_ports, int modid, int nof_units, int* unit_arr, int* modid_arr)
{
    int rv = BCM_E_NONE;
    int unit_idx;
    ...
    printf("appl_arad_plus_stk_init\n");
    rv = appl_arad_plus_stk_init(unit);
    if (BCM_FAILURE(rv)) {
        printf("appl_arad_plus_stk_init: failed. unit %d Error:%d (%s) \n", unit, rv, bcm_errmsg(rv));
        return rv;
    }
    if (g_info.cfg_2nd_unit) {
        for (unit_idx = 0; unit_idx < nof_units - 1; unit_idx++) {
            rv = appl_arad_plus_stk_init(unit_arr[unit_idx]);
            if (BCM_FAILURE(rv)) {
                printf("appl_arad_plus_stk_init: failed. Error:%d (%s) \n", unit, rv, bcm_errmsg(rv));
                return rv;
            }
        }
    }
    
    printf("appl_arad_plus_cosq_init\n");    
    rv = appl_arad_plus_cosq_init(unit, modid, nof_units, unit_arr, modid_arr);
    if (BCM_FAILURE(rv)) {
        printf("appl_arad_plus_cosq_init: failed. Error:%d (%s) \n", rv, bcm_errmsg(rv));
        return rv;
    }
    
    printf("%s(), EXIT. rv is %d\r\n",proc_name, rv);
    return rv;
}
...
int arad_plus_appl_mc(int unit, int mc_group_num, int mc_type, int mc_id, int nxt_mc_id, int mc_port_member)
{
    int 
        rv = BCM_E_NONE,
        i;
    ...
    for (i = 0; i < mc_group_num; i++) {
        ...
        printf("arad_plus_appl_mc(): Open Multicast Group, mc_id=%d, mc_type=%d\n", mc_id + i, mc_type);        
        ...
        /* Add MC members */
        printf("arad_plus_appl_mc():Add MC member. mc_port_member=0x%x, nxt_mc_id=0x%x\n", mc_port_member + i, nxt_mc_id + i);
...
/* 
 * Arad_Plus FC configuration Example 
 ...
 */
int arad_plus_fc_shr_example(int unit, int num_tm_ports, int num_cos) 
{
    int 
        rv = BCM_E_NONE,
        port,
    ...
    printf("arad_plus_fc_shr_example(): num_tm_ports=%d, num_cos=%d\n", num_tm_ports, num_cos);
    for (cos = 0; cos < num_cos; cos++) {
        for (port = 0; port < num_tm_ports; port++) {
            ...
            printf("arad_plus_fc_shr_example(): port=%d, cos=%d, source.calender_index=%d, target.port=0x%x\n", 
                   port, cos, source.calender_index, target.port);
            rv = bcm_cosq_fc_path_add(unit,bcmCosqFlowControlReception, &source, &target);
            if (rv != BCM_E_NONE) {
                printf("arad_plus_fc_shr_example(): bcm_cosq_fc_path_add() Fail !!!\n");
                return rv;
            }
        }
    }
    ...
    printf("arad_plus_fc_shr_example(): Disable FCV[255].\n");
    ...
    if (rv != BCM_E_NONE) {
        printf("arad_plus_fc_shr_example(): bcm_cosq_fc_path_delete() Fail !!!\n");
        return rv;
    }
...
/* 
 * Arad_Plus shaping configuration Example 
 ...
 */
int arad_plus_shaping_configuration(int unit, int nof_isqs, int nof_cos, int isq_bw) {
    ...
    /* printf("arad_plus_shaping_configuration. nof_isqs %d nof_cos %d isq_bw %d\n",nof_isqs, nof_cos, isq_bw); */
    ...
    /* printf("arad_plus_shaping_configuration. Exit\n"); */

    return rv;
}

/* 
 * Arad_Plus THR assiatance function 
...
/* 
 * Arad_Plus THR configuration Example 
 * This function responsible for configuring Arad_Plus per port thresholds according arad_plus_egq_thresholds.xls.
 * This function configures only the 1 port 100G/120G with 8 priority example. 
 ...
 * set_eqr_thresholds help function defined to receive these parameters and configure proper values.
 */
int arad_plus_thr_example(int unit, int local_port_id, int nof_cos) {
    int rv = BCM_E_NONE;
    int tc;
    int color;
    int is_mc;
    ...
    /* printf("arad_plus_thr_example, local_port_id=%d, nof_cos=%d\n", local_port_id, nof_cos ); */
    ...
    */ 
    ...
    /* printf("arad_plus_thr_example, Exit\n" ); */

    return rv;
}

int arad_plus_appl_example(int unit, int nof_local_ports, int mc_group_num, int nof_isqs, int cgm_thr_enable) {
    int 
        rv,
        i,
        modid, 
    ...
    proc_name = "-+-+-+-+-+-+-+-+ arad_plus_appl_example";
    
    printf("%s(), ENTER. nof_local_ports=%d, mc_group_num=%d, nof_isqs=%d, cgm_thr_enable=%d\n", proc_name, nof_local_ports, mc_group_num, nof_isqs, cgm_thr_enable );
    ...
    unit_arr[0] = 0;
    modid_arr[0] = 0;    
    rv = arad_plus_appl_init(unit, tm_internal_pkt_port, nof_local_ports, modid, nof_units, unit_arr, modid_arr);
    if (BCM_FAILURE(rv)) {
        printf("arad_plus_appl_init: failed. Error:%d (%s) \n", rv, bcm_errmsg(rv));
        return rv;
    }
    ...
    nxt_mc_id = 0;
    mc_port_member = 1;
    rv = arad_plus_appl_mc(unit, mc_group_num, mc_type, mc_id, nxt_mc_id, mc_port_member);
    if (BCM_FAILURE(rv)) {
        printf("arad_plus_appl_mc: failed. Error:%d (%s) \n", rv, bcm_errmsg(rv));
        return rv;
    }
    ...
    if (nof_isqs != 0x0) {
        ...
        /* printf("arad_plus_appl_example, Call arad_plus_shaping_configuration. isq_bw %d\n",isq_bw); */
...
/*
 * Arad_Plus on mgmt device application example.
 *
 * This function is pretty similar to arad_plus_appl_example(), only it was written specifically to work 
 ...
 *
 */
int arad_plus_mgmt_appl_example(int unit, int nof_local_ports, int mc_group_num, int nof_isqs, int cgm_thr_enable) {
    int 
        rv,
        i,
        modid, 
    ...
    /* printf("arad_plus_mgmt_appl_example, nof_local_ports=%d, mc_group_num=%d, nof_isqs=%d, cgm_thr_enable=%d\n", nof_local_ports, mc_group_num, nof_isqs, cgm_thr_enable ); */
    ...
    if (g_info.cfg_2nd_unit) {
        for (unit_idx = 0; unit_idx < nof_units - 1; ++unit_idx) {
            mc_type = 0x1; /* BCM_MULTICAST_EGRESS_GROUP = 1 */
            rv = arad_plus_appl_mc(unit_arr[unit_idx], mc_group_num, mc_type, mc_id, nxt_mc_id, mc_port_member);
            if (BCM_FAILURE(rv)) {
                printf("arad_plus_appl_mc: failed. Error:%d (%s) \n", rv, bcm_errmsg(rv));
                return rv;
            }
    ...
    if (cgm_thr_enable == 0x1) {
        ...
        if (g_info.cfg_2nd_unit) {
            for (unit_idx = 0; unit_idx < nof_units - 1; ++unit_idx) {
                rv = arad_plus_thr_example(unit_arr[unit_idx], tm_internal_pkt_port, MAX_COS);
                if (BCM_FAILURE(rv)) {
                    printf("arad_plus_thr_example: failed. Error:%d (%s) \n", rv, bcm_errmsg(rv));
                    return rv;
                }
    ...
    if (nof_isqs != 0x0) {
        ...
        /* printf("arad_plus_mgmt_appl_example, Call arad_plus_shaping_configuration. isq_bw %d\n",isq_bw); */
        ...
        if (g_info.cfg_2nd_unit) {
            for (unit_idx = 0; unit_idx < nof_units - 1; ++unit_idx) {
                rv = arad_plus_shaping_configuration(unit_arr[unit_idx], nof_isqs, MAX_COS, isq_bw);
                if (BCM_FAILURE(rv)) {
                    printf("arad_plus_shaping_configuration: failed. Error:%d (%s) \n", rv, bcm_errmsg(rv));
                    return rv;
                }
    ...
    /* printf("arad_plus_mgmt_appl_example, Exit\n"); */
