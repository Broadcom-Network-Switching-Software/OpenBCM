...
int sand_utils_l3_port_set(int unit, sand_utils_l3_port_s *l3_port_structure)
{
    ...
    if (rc != BCM_E_NONE)
    {
        printf("Error, Ingress bcm_vlan_port_create\n");
        return rc;
    }
    ...
    if (!l3_port_structure->eg_vlan_port_id)
    {
        ...
        if (rc != BCM_E_NONE)
        {
            printf("Error, Egress bcm_vlan_port_create\n");
            return rc;
        }
...
/* Convert FEC-ID to even FEC_ID */
int sand_utils_l3__fec_id_to_even_fec_id(int fec_id) {
    return ((fec_id / 2) * 2 );
}
