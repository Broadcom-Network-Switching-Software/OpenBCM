...
/*
 * The VXLAN NNI to NNI example with parameters:
 * Unit - relevant unit
 ...
 * is_vxlan_gpe - indicates vxlan or vxlan_gpe
 */
int vxlan_nni2nni_example(int unit, int core_port_in, int core_port_out,int access_port, int is_vxlan_gpe)
{
    ...
    /* tunnel initiator parameters for egress core port */
    bcm_mac_t next_hop_mac_2 = {0x00, 0x00, 0x00, 0x11, 0xcd, 0x1d};
    bcm_mac_t core_fwd_mac = {0x00, 0x00, 0x00, 0x00, 0x11, 0x22};
    bcm_ip_t tunnel_2_dip = 0xAB000012; /* 171.0.0.18 */
    bcm_ip_t tunnel_2_sip = 0xA0000002; /* 160.0.0.2 */

    proc_name = "vxlan_nni2nni_example";
    ...
    if(!device_is_jericho2)
    {
        printf("%s(): Error, example only for JR2 \n", proc_name);
        return BCM_E_UNAVAIL;
    }

    /* Get FEC id */
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, &g_vxlan.tunnel_fec_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = multicast__open_mc_group(unit, &g_vxlan.vpn_id, 0);
    if (rv != BCM_E_NONE) {
        printf("%s(): Error, multicast__open_mc_group\n", proc_name);
         return rv;
    }
    ...
    bcm_vxlan_vpn_config_t_init(&vpn_config);
    vpn_config.flags = BCM_VXLAN_VPN_WITH_ID|BCM_VXLAN_VPN_WITH_VPNID;
    vpn_config.vpn = g_vxlan.vpn_id;
    vpn_config.broadcast_group = g_vxlan.vpn_id;
    vpn_config.unknown_unicast_group = g_vxlan.vpn_id;
    vpn_config.unknown_multicast_group = g_vxlan.vpn_id;
    ...
    vxlan_vpn_config.vni = g_vxlan.vni;
    vxlan_vpn_config.vsi = g_vxlan.vpn_id;
    ...
    if(rv != BCM_E_NONE) {
        printf("%s(): error in bcm_vxlan_network_domain_config_add \n", proc_name);
        return rv;
    }

    /* Configure egress VSI to VNI mapping */
    g_vxlan.vni++; /* Use different VNI than ingress */
    bcm_vxlan_network_domain_config_t_init(&vxlan_vpn_config);
    vxlan_vpn_config.network_domain  = 0;
    vxlan_vpn_config.vni = g_vxlan.vni;
    vxlan_vpn_config.vsi = g_vxlan.vpn_id;
     ...
     *  Set In-Port to In ETh-RIF
     */
    rv = in_port_intf_set(unit, core_port_in, g_vxlan.eth_rif_intf_core);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in_port_intf_set intf_in\n", proc_name);
        return rv;
    ...
    rv = intf_eth_rif_create(unit, g_vxlan.core_vlan, g_vxlan.intf_core_mac_addr);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, intf_eth_rif_create eth_rif_intf_core \n", proc_name);
        return rv;
    ...
    rv = l3__egress_only_encap__create(unit, BCM_L3_EGRESS_ONLY, &(g_vxlan.tunnel_arp_id), g_vxlan.tunnel_next_hop_mac, g_vxlan.core_vlan);
    if(rv != BCM_E_NONE)
    {
        printf("%s(): Error, l3_egress_only_encap__create\n", proc_name);
        return rv;
     ...
     * Create the tunnel initiator
     */
    rv = tunnel_initiator_create(unit, g_vxlan.tunnel_arp_id, &tunnel_out_rif_intf);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in function tunnel_initiator_configure \n", proc_name);
        return rv;
    ...
    if (*dnxc_data_get(unit, "esem", "feature", "etps_properties_assignment", NULL)) {
        ...
        rv = bcm_port_class_set(unit,g_vxlan.tunnel_id_init, bcmPortClassEgress, network_domain);
        if (rv != BCM_E_NONE) {
            print rv;
     ...
     * Create FEC and configure its: Outlif, Destination port, Global Out-EthRIF
     */
    rv = l3__egress_only_fec__create(unit, g_vxlan.tunnel_fec_id, tunnel_out_rif_intf, 0, core_port_in, 0);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC only\n", proc_name);
    }
    ...
    /*
     * Add VXLAN port, update learn info as FEC + outlif
     */
    /* Add VXLAN port properties and configure the tunnel terminator In-Lif and tunnel initiatior Out-Lif*/
    vxlan_port_add_s vxlan_port_add;
    vxlan_port_s_clear(&vxlan_port_add);
    vxlan_port_add.vpn = g_vxlan.vpn_id;
    vxlan_port_add.in_port = core_port_in;
    vxlan_port_add.in_tunnel = g_vxlan.tunnel_id_term;
    vxlan_port_add.out_tunnel_if = tunnel_out_rif_intf;
    BCM_L3_ITF_SET(vxlan_port_add.egress_if, BCM_L3_ITF_TYPE_FEC, g_vxlan.tunnel_fec_id);
    ...
    rv = bcm_port_class_set(unit, g_vxlan.tunnel_id_init, bcmPortClassForwardEgress, g_vxlan.vxlan_network_group_id);
    {
        printf("%s(): Error, create in update orientation for tunnel outlif \n", proc_name);
    }

    /*
     * Create the tunnel termination for egress core port
     */
    g_vxlan.tunnel_dip = tunnel_2_dip;
    g_vxlan.tunnel_sip = tunnel_2_sip;
    rv = tunnel_terminator_create(unit, is_vxlan_gpe);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, tunnel_terminator_create \n", proc_name);
    ...
    /*
     * Create ARP entry for VXLAN tunnel egress core port
     */
    g_vxlan.tunnel_arp_id = 0;
    rv = l3__egress_only_encap__create(unit, BCM_L3_EGRESS_ONLY, &(g_vxlan.tunnel_arp_id), next_hop_mac_2, g_vxlan.core_vlan);
    if(rv != BCM_E_NONE)
    {
        printf("%s(): Error, l3_egress_only_encap__create\n", proc_name);
        return rv;
     ...
     */
    g_vxlan.tunnel_fec_id++;
    rv = l3__egress_only_fec__create(unit, g_vxlan.tunnel_fec_id, tunnel_out_rif_intf, 0, core_port_out, 0);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC only\n", proc_name);
    }

    printf("vxlan tunnel fec_id for egress core port: 0x%x\n", g_vxlan.tunnel_fec_id);

    /*
     * Add VXLAN port for egress core port
     */
    /* Add VXLAN port properties and configure the tunnel terminator In-Lif and tunnel initiatior Out-Lif*/
    vxlan_port_s_clear(&vxlan_port_add);
    vxlan_port_add.vpn = g_vxlan.vpn_id;
    vxlan_port_add.in_port = core_port_out;
    vxlan_port_add.in_tunnel = g_vxlan.tunnel_id_term;
    vxlan_port_add.out_tunnel_if = tunnel_out_rif_intf;
    BCM_L3_ITF_SET(vxlan_port_add.egress_if, BCM_L3_ITF_TYPE_FEC, g_vxlan.tunnel_fec_id);
    vxlan_port_add.add_to_mc_group = 1;
    ...
    rv = vlan_port_add(unit, access_port, &vlan_port_id);
    if (rv != BCM_E_NONE) {
        printf("%s(): Error, vlan_port_add\n",proc_name);
    }

    rv = vswitch_add_port(unit, g_vxlan.vpn_id, access_port, vlan_port_id);
    if (rv != BCM_E_NONE) {
        printf("%s(): Error, vswitch_add_port\n", proc_name);
        return rv;
    }
    ...
    if (*dnxc_data_get(unit, "esem", "feature", "etps_properties_assignment", NULL)) {
        /*
         * Create ESEM entry and set out lif profile for split horizon
         */
        bcm_vlan_port_t vlan_port;
        bcm_vlan_port_t_init(&vlan_port);
        vlan_port.vsi = g_vxlan.vpn_id;
        /* 
        * Use a different network domain for split horizon, 
        *  we don't want to drop nni 2 nni packet
        */
        vlan_port.match_class_id = network_domain; /* network domain, part of the key in ESEM lookup */
        ...
        vlan_port.egress_network_group_id = g_vxlan.vxlan_network_group_id;
    ...
    /*
    * MACT entry from core to access
    */
    rv = vxlan_l2_addr_add (unit, g_vxlan.vm1_da, g_vxlan.vpn_id, access_port);
    if (rv != BCM_E_NONE)
    {
    ...
    /*
     * MACT entry from ingress core port to egress core port
    */
    rv = vxlan_l2_addr_add (unit, core_fwd_mac, g_vxlan.vpn_id, vxlan_port_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error(%d), l2_addr_add\n", rv);
        return rv;
