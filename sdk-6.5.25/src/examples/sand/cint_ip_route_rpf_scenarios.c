...
int ipv4_mc_example_rpf(int unit, int in_port, int out_port,int nof_dest_ports)
{
    ...
    /* Get MRPF Explicit Mode */
    bcm_l3_egress_t l3eg_get;
    bcm_l3_egress_t_init(&l3eg_get);
    rv = bcm_l3_egress_get(unit, fec[eg_itf_index], &l3eg_get);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_l3_egress_get, unit=%d, \n", unit);
        return rv;
    }

    if (l3eg_get.flags != BCM_L3_MC_RPF_EXPLICIT)
    {
        printf("Fail to get MRPF Explicit Mode, unit=%d, \n", unit);
        return BCM_E_FAIL;
    }
...
int outlif_in_mrpf_fec = 0;
/* 
 * Example of multicast explicit rpf checking when multicast routing into a unicast destination.
 */
int dvapi_run_ipv4_mc_example_rpf_with_uc_dest(int unit, int in_port, int out_port)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_ip_t mc_ip = 0xE0E0E003; /* 224.224.224.3 */
    bcm_ip_t src_ip = 0x0; /* masked */
    ...
    /*
     * create egress object with multicast explicit RPF check
     */
    ...
    encap_id_r = l3eg.encap_id;
    outlif_in_mrpf_fec = BCM_L3_ITF_VAL_GET(encap_id_r);
    ...
    /* create egress objects for IPMC with RPF incorrect expected inRIF */
     ...
     if(print_level >= 2) {
         printf("created egress object 0x%08x with expected inRIF 0x%08x encap:0x%08x\n", fec[eg_itf_index],ing_intf_r, encap_id_r);
     }
     ++eg_itf_index;
     ...
     if(print_level >= 2) {
         printf("created egress object 0x%08x with expected inRIF 0x%08x encap:0x%08x\n", fec[eg_itf_index], ing_intf_l, encap_id_l);
     }
     ++eg_itf_index;
