...
int bcm_l3_basic_ip6_mc(int unit, int port0, int port1, int port2, int port3) {
    ...
    int ipmc_ssm_group[NOF_BASIC_MC_GROUP];
    ...
    for (i=0; i < NOF_BASIC_MC_GROUP; i++) {  /* picking random values for the mc groups */
        ...
        while (found){ /* verifying that we don't get a previous value */
            ...
            ipmc_ssm_group[i] = (sal_rand() % ipmc_index_upper_bound) + ipmc_index_offset;
            for(j=0 ; j < i ;j++) {
                if((ipmc_group[i] == ipmc_group[j]) || (ipmc_group[i] == ipmc_ssm_group[j])) {found = 1;}
                if((ipmc_ssm_group[i] == ipmc_group[j]) || (ipmc_ssm_group[i] == ipmc_ssm_group[j])) {found = 1;}
            }
        }
    }
    ...
    for (i = 0; i < NOF_BASIC_MC_GROUP; i++) {
        ...
        flags = is_device_or_above(unit, JERICHO2) ? (BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID) : (BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_TYPE_L3 | BCM_MULTICAST_WITH_ID);
        printf("Creating IPMC (vrf,*,G,InRif) group with mcID=%d(0x%x)\n", ipmc_group[i], ipmc_group[i]);
        rc = bcm_multicast_create(unit, flags, &(ipmc_group[i]));
        if (rc != BCM_E_NONE) {
            printf("Error, bcm_multicast_create \n");
            return rc;
        }
        printf("Creating IPMC (vrf,S,G,InRif) group with mcID=%d(0x%x)\n", ipmc_ssm_group[i], ipmc_ssm_group[i]);
        rc = bcm_multicast_create(unit, flags, &(ipmc_ssm_group[i]));
        if (rc != BCM_E_NONE) {
            printf("Error, bcm_multicast_create \n");
            return rc;
        }
        ...
        rc = bcm_multicast_set(unit, ipmc_ssm_group[i], flags, out_ports[i], replications);/* the same members but different group */
        if (rc) {
            printf("Error, bcm_multicast_set index %d\n", i);
            return rc;
        }
    ...
    for (i = 0; i < NOF_BASIC_MC_GROUP; i++) {
        ...
        data.flags |= (use_tcam ? BCM_IPMC_TCAM : 0);
        for (j = 0; j < 16; j++) {
        ...
        if (!is_device_or_above(unit, JERICHO2) && compressed_sip && use_tcam == 0)
        {
            data.s_ip_addr = compressed_sip;
            data.group = ipmc_ssm_group[i];/* use different mcid for SSM entries */
            rc = bcm_ipmc_add(unit, &data);
            if (rc != BCM_E_NONE) {
                printf("Error, create_forwarding_entry_dip_sip \n");
                return rc;
...
int ipv6_mc_prefix(int unit, int port0, int port1, int port2, int port3, int table_id) {
    ...
    for (i = 0; i < NOF_IPV6_MC_PREFIX_INTERFACES; i++) {
        ...
        /*create different IPMC groups */
        rc = BCM_E_EXISTS;
        /* verifying that we don't get an existing value */
        while (rc == BCM_E_EXISTS)
        {
