...
int bfd_server_example(int server_unit, int client_unit) {
    ...
    int server_tunnel_id, client_tunnel_id, next_hop_mac2;
    ...
    if (is_dnx) 
    {
        ...
        else
        {
            if(is_ipv6)
            {
                ...
                rv = basic_example_ipv6(client_unit,bfd_endpoint_over_lag_1.eg_port, bfd_endpoint_over_lag_1.trunk_gport,0,0,1);

                if (rv != BCM_E_NONE) {
                    printf("Error, in basic_example_ipv6\n");
                    return rv;
                }
                rv = basic_example_ipv6(server_unit,bfd_endpoint_over_lag_1.eg_port2, bfd_endpoint_over_lag_1.trunk_gport,0,0,1);
            ...
            else
            {
                ...
                rv = l3_interface_init(client_unit, bfd_endpoint_over_lag_1.trunk_gport, bfd_endpoint_over_lag_1.eg_port, &next_hop_mac2, &tunnel_id, 0, 0, 0);
                if (rv != BCM_E_NONE) {
                    printf("(%s) \n", bcm_errmsg(rv));
                    return rv;
                }
    ...
    if(is_device_or_above(client_unit, JERICHO2) && (bfd_echo == 1))
    {
       ...
       rv = cint_field_bfd_echo_entry_add(client_unit, 0x7fffff03, src_ipv6, bfd_local_discr & 0xFFFF, bfd_local_discr & 0xFFFF);
       if (rv != BCM_E_NONE) {
