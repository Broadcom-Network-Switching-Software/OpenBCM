 ...
 * File: cint_sat.c
 *
 * For JR2 devices please refer to cint_dnx_sat.c
 *
 * Purpose: 
 ...
 */
int sat_gtf_calculate_cbs_granularity(unsigned int max_rate, unsigned int *cbs,  unsigned int *granularity, int clock_khz) {
    if (!clock_khz){
        return BCM_E_PARAM;
    }
...
int sat_gtf_setup(int unit, bcm_sat_gtf_t *gtf_id) {
    ...
    sat_gtf_calculate_cbs_granularity(bw.rate, &bw.max_burst, &bw.granularity, 1000000);
    rv = bcm_sat_gtf_bandwidth_set(unit, *gtf_id, -1, &bw);
    if (rv != BCM_E_NONE) {
        printf("Failed($rv) in bcm_sat_gtf_bandwidth_set\n");
        return rv;
