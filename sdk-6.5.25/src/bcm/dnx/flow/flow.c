...
static shr_error_e
flow_create_init_specific_validations(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    bcm_flow_initiator_info_t * bcm_app_info,
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** Qos validations */
    SHR_IF_ERR_EXIT(dnx_flow_init_dedicated_logic_qos_validate
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    {
        SHR_IF_ERR_EXIT(flow_create_init_specific_validations
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error: Missing virtual flag BCM_FLOW_HANDLE_INFO_VIRTUAL while creating an object for the Virtual Only application %s",
                     flow_app_info->app_name);
    }
    ...
    bcm_flow_special_fields_t * key_special_fields)
{
    ...
    while ((dbal_table != DBAL_TABLE_EMPTY) && (ii < max_nof_tables))
    {
        ...
        else
        {
            ...
            if (flow_app_info->dbal_table_to_key_field_map)
            {
                /** used in traverse to map the dbal table to the key field. */
                SHR_IF_ERR_EXIT(flow_app_info->dbal_table_to_key_field_map
                                (unit, flow_handle_info, FLOW_MAP_TYPE_TABLE_TO_KEY_FIELD, key_special_fields,
                                 &dbal_table));
            }
            ...
            if (flow_app_info->app_specific_operations_cb)
            {
                ...
                SHR_IF_ERR_EXIT(flow_app_info->app_specific_operations_cb
                                (unit, entry_handle_id, FLOW_ENTRY_HANDLE_ID_INVALID, flow_handle_info,
                                 &flow_cmd_control, NULL, NULL, key_special_fields, NULL, NULL));
            }
                 ...
                 && (ii == FLOW_MAX_NOF_MATCH_DBAL_TABLES - 1))
        {
            ...
            {
                dbal_table = flow_app_info->second_pass_table;
                bud_table_used = 1;
            }
            else
            {
    ...
    uint32 gport)
{
    ...
    {
        BCM_GPORT_FLOW_LIF_ID_SET(flow_gport, gport);
    }
 ...
 */
static shr_error_e
dnx_flow_terminator_info_virtual_gport_free(
    int unit,
    bcm_gport_t virtual_lif_gport)
{
    uint32 virtual_val;
    ...
    SHR_IF_ERR_EXIT(lif_table_mngr_db.flow_ingress_virtual_gport_id.free_single(unit, virtual_val, NULL));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (!is_virtual)
    {
        inlif_info.global_lif = _SHR_GPORT_FLOW_LIF_ID_GET(flow_handle_info->flow_id);
        global_in_lif_field_val = inlif_info.global_lif;
    }
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOB_IN_LIF, INST_SINGLE, global_in_lif_field_val);
 ...
 */
static shr_error_e
dnx_flow_terminator_info_virtual_gport_allocate(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info)
{
    ...
    SHR_IF_ERR_EXIT(lif_table_mngr_db.flow_ingress_virtual_gport_id.allocate_single(unit,
                                                                                    sw_handle_alloc_flags,
                                                                                    NULL, &virtual_val));

    /** Encode the virtual_val as a flow virtual gport with a subtype */
    BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE_SET(virtual_id, virtual_val);
    ...
    global_lif_alloc_info_t * alloc_info)
{
    ...
    if (_SHR_IS_FLAG_SET(flow_handle_info->flags, BCM_FLOW_HANDLE_INFO_VIRTUAL))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_flow_terminator_info_virtual_gport_allocate(unit, flow_handle_info, flow_app_info));
        }
        SHR_EXIT();
    }
    ...
    if (_SHR_IS_FLAG_SET(flow_handle_info->flags, BCM_FLOW_HANDLE_INFO_WITH_ID))
    {
        ...
        global_lif = _SHR_GPORT_FLOW_LIF_ID_GET(flow_handle_info->flow_id);
    }
...
dnx_flow_terminator_info_create(
    ...
    bcm_flow_special_fields_t * special_fields)
{
    int local_inlif = 0, global_in_lif_field_val = 0;
    uint32 entry_handle_id;
    ...
    if (!is_virtual)
    {
        ...
        global_in_lif_field_val = lif_table_mngr_inlif_info.global_lif;
    }
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOB_IN_LIF, INST_SINGLE, global_in_lif_field_val);
    ...
    if (flow_app_info->flow_app_type == FLOW_APP_TYPE_TERM_MATCH)
    {
        if (!dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
        {
            ...
            SHR_IF_ERR_EXIT(dbal_tables_physical_table_get
                            (unit, FLOW_LIF_DBAL_TABLE_GET(payload_flow_app_info), DBAL_PHY_DB_DEFAULT_INDEX,
    ...
    dbal_tables_e * selected_dbal_table)
{
    ...
    if (_SHR_IS_FLAG_SET(flow_handle_info->flags, BCM_FLOW_HANDLE_INFO_BUD))
    {
        ...
        {
            (*selected_dbal_table) = flow_app_info->second_pass_table;
        }
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(flow_app_info->dbal_table_to_key_field_map
                            (unit, flow_handle_info, FLOW_MAP_TYPE_KEY_FIELD_TO_TABLE, key_special_fields,
                             selected_dbal_table));
        }
        else
        {
    ...
    bcm_flow_special_fields_t * key_special_fields)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_match_related_dbal_table_get
                    (unit, flow_handle_info, flow_app_info, key_special_fields, &dbal_table));
...
static shr_error_e
dnx_flow_is_match_entry_designated(
    int unit,
    int core_id,
    dnx_flow_app_config_t * flow_app_info,
    uint32 entry_handle_id,
    ...
    int *is_designated)
{
    ...
    while (flow_app_info->special_fields[idx] != FLOW_S_F_EMPTY)
    {
        if (flow_app_info->special_fields[idx] == FLOW_S_F_IN_PORT)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                            (unit, entry_handle_id, DBAL_FIELD_IN_PP_PORT, &pp_port));
    ...
    dnx_flow_traverse_operation_e traverse_operation)
{
    ...
    while (!is_end)
    {
        ...
        else
        {
            ...
            rv = dnx_flow_terminator_virtual_mapping_sw_gport_get(unit, key_field, core_id, &virtual_id);
            SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
            _SHR_GPORT_FLOW_LIF_ID_SET(flow_handle_info->flow_id, virtual_id);
            is_virtual = TRUE;
        }
        ...
        if (rv == _SHR_E_NONE)
        {
            ...
            else
            {
                ...
                if (_SHR_IS_FLAG_SET(flow_app_info->valid_flow_flags, BCM_FLOW_HANDLE_INFO_ALLOC_BY_CORE_BM) &&
                    dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc) && !is_virtual)
                {
    ...
    int bud_table_in_use)
{
    ...
    uint32 entry_handle_id;
    int is_end;
    int global_lif, local_lif, is_phy_dpc =
        ...
        if (flow_cmd_control.flow_command != FLOW_COMMAND_SKIP_TO_END)
        {
            if (traverse_operation == FLOW_TRAVERSE_OPERATION_USE_CALLBACK)
            {
                    ...
                    && (dbal_table != DBAL_TABLE_INGRESS_PP_PORT) && (is_lif_dpc))
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_flow_is_match_entry_designated
    ...
    bcm_flow_handle_info_t * flow_handle_info)
{
    ...
    if (is_virtual)
    {
        SHR_IF_ERR_EXIT(dnx_flow_terminator_info_virtual_gport_free(unit, flow_handle_info->flow_id));
    }
    else
    {
    ...
    bcm_flow_handle_info_t * flow_handle_info)
{
    ...
    /** Free the Global LIF allocation */
    if (is_virtual)
    {
    ...
    {
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_no_local_lif_entry_clear
                        (unit, flow_handle_info->flow_id, FLOW_LIF_DBAL_TABLE_GET(flow_app_info)));
    }
    ...
    bcm_flow_field_id_t * field_id)
{
    ...
    SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Field %s is not supported by the application\n", field_name);
