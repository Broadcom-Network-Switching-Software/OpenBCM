    ...
    uint32 value)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, gport_lif, _SHR_GPORT_TYPE_FLOW_LIF, &flow_handle_info.flow_id));
    SHR_IF_ERR_EXIT(flow_lif_flow_app_from_gport_get(unit, flow_handle_info.flow_id,
                                                     flow_app_type, &flow_handle_info.flow_handle));
    ...
    {
        SHR_IF_ERR_EXIT(bcm_dnx_flow_initiator_info_create(unit, &flow_handle_info, &flow_init_info, NULL));
    }
    ...
    int value)
{
    ...
    flow_term_info.l2_ingress_info.outer_policer_remark = flow_term_info_get.l2_ingress_info.outer_policer_remark;
    flow_term_info.l2_ingress_info.inner_policer_remark = flow_term_info_get.l2_ingress_info.inner_policer_remark;
