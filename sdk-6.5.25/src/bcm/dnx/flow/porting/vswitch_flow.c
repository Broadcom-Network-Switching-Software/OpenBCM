    ...
    bcm_flow_handle_t * flow_handle)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_flow_handle_get(unit, flow_app_name, flow_handle));
    ...
    bcm_vswitch_cross_connect_t * gports)
{
          ...
          (vswitch_flow_term.l2_ingress_info.service_type == bcmFlowServiceTypeCrossConnect)))
    {
         ...
         */
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Wrong setting. port1 = 0x%08X is not a P2P LIF.\n", gports->port1);
    }

    if (vswitch_flow_term.valid_elements_set & BCM_FLOW_TERMINATOR_ELEMENT_FLOW_DEST_INFO_VALID)
    {
        if (_SHR_GPORT_IS_FLOW_LIF(vswitch_flow_term.dest_info.dest_port))
        {
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif
                                (unit,
                                 ...
                                 _SHR_GPORT_FLOW_LIF_ID_GET(vswitch_flow_term.dest_info.dest_port), &(gports->port2)));
                if (gports->port2 == BCM_GPORT_INVALID)
                {
                    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif
                                    (unit, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS, _SHR_CORE_ALL,
                                     _SHR_GPORT_FLOW_LIF_ID_GET(vswitch_flow_term.dest_info.dest_port),
                                     &(gports->port2)));
                }
            }
        }
    ...
    void *data)
{
        ...
        (terminator_info->dest_info.dest_port != 0 || terminator_info->dest_info.dest_encap != 0))
    {
        ...
        {
            BCM_GPORT_TUNNEL_ID_SET(gports.port1, _SHR_GPORT_FLOW_LIF_ID_GET(flow_handle_info->flow_id));
        }

        /*
         * Process for the dst gport: gport2
         */
        if (_SHR_GPORT_IS_FLOW_LIF(terminator_info->dest_info.dest_port))
        {
            uint8 is_physical_port;
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_is_physical
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif
                                (unit,
                                 (DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS |
                                  DNX_ALGO_GPM_GPORT_HW_RESOURCES_NON_STRICT), _SHR_CORE_ALL,
                                 _SHR_GPORT_FLOW_LIF_ID_GET(terminator_info->dest_info.dest_port), &(gports.port2)));
                if (gports.port2 == BCM_GPORT_INVALID)
                {
                    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif
                                    (unit, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS, _SHR_CORE_ALL,
                                     _SHR_GPORT_FLOW_LIF_ID_GET(terminator_info->dest_info.dest_port),
                                     &(gports.port2)));
                }
            }
        }
