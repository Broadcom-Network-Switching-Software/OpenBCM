...
shr_error_e
dnx_tunnel_terminator_match_flow_app_get(
    int unit,
    ...
    bcm_flow_handle_info_t * flow_handle_info)
{
    ...
    dnx_tunnel_terminator_match_flow_app_get(unit, tunnel->type, &flow_app);
    ...
    rv = bcm_dnx_flow_logical_field_id_get(unit, flow_handle, TUNNEL_TERM_FLOW_FIELD_IPV4_TUNNEL_TYPE, &field_id);
    SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
    if (rv == _SHR_E_NONE)
    {
        special_fields.special_fields[3].field_id = field_id;
        special_fields.special_fields[3].symbol = tunnel->type;
        special_fields.actual_nof_special_fields++;
    }
    ...
    bcm_tunnel_terminator_t * tunnel)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_create_flow_common_fields_set(unit, tunnel, &flow_tunnel_term));
    SHR_IF_ERR_EXIT(bcm_dnx_flow_terminator_info_create
                    (unit, &flow_handle_info, &flow_tunnel_term, &flow_special_fields));
    BCM_GPORT_TUNNEL_ID_SET(tunnel->tunnel_id, _SHR_GPORT_FLOW_LIF_ID_GET(flow_handle_info.flow_id));
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_create_flow_match_lookup_config(unit, tunnel, &flow_handle_info));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_tunnel_terminator_t * tunnel)
{
    ...
    flow_handle_info.flow_handle = flow_handle;
    BCM_GPORT_FLOW_LIF_ID_SET(flow_handle_info.flow_id, tunnel->tunnel_id);
