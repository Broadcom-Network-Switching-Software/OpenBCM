 ...
 * \param [in] is_ibch1_enabled - Indication if IBCH1 is enabled on a specific port
 * \param [in] first_header_size_to_skip - First header size to skip value
 *
 * \return
 *   shr_error_e
    ...
    int *first_header_size_value)
{
    ...
    int switch_header_type = 0;
    int recycle_command_prt_profile = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    {
        recycle_command_prt_profile = DBAL_ENUM_FVAL_RECYCLE_CMD_PRT_PROFILE_NONE;
    }
#endif
    ...
    uint32 first_header_size_value)
{
    ...
    if (first_reference)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_prt_profiles_update
                        (unit, new_profile_index, switch_header_type, port_first_header_size_data.is_before,
                         is_ibch1_enabled, first_header_size_value));
    }
    ...
    if (last_reference)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_prt_profiles_update
                        (unit, old_profile_index, switch_header_type, port_first_header_size_data.is_before,
                         is_ibch1_enabled, 0));
    }
    ...
    uint32 first_header_size_value)
{
    ...
    if (last_reference)
    {
        /** Set user defined PRT configuration to its default values */
        SHR_IF_ERR_EXIT(dnx_port_prt_profiles_update
                        (unit, old_profile, switch_header_type, port_first_header_size_data.is_before, is_ibch1_enabled,
                         0));
    }
    ...
    uint32 phy_mask)
{
    ...
    else
    {
        SHR_IF_ERR_EXIT(dnx_port_logical_verify(unit, port));
        SHR_IF_ERR_EXIT(dnx_port_logical_get(unit, port, &logical_port));
        SHR_IF_ERR_EXIT(imb_port_phy_reg_get(unit, logical_port, flags, phy_reg_addr, &phy_rd_data));
        phy_data |= (phy_rd_data & ~phy_mask);
        SHR_IF_ERR_EXIT(imb_port_phy_reg_set(unit, logical_port, flags, phy_reg_addr, phy_data));
    }
