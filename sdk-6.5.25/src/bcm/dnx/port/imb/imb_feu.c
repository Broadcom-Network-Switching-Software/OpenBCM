...
typedef enum
{
     ...
     */
    IMB_FEU_MAC_TX_FAULT_SOURCE_RX_FAULT_SM = 0,
    /*
     * The fault is according to the external input
     */
    IMB_FEU_MAC_TX_FAULT_SOURCE_EXT_INPUT,
    /*
     * The fault is according to the SW configuration
     */
    IMB_FEU_MAC_TX_FAULT_SOURCE_SW_CFG
} imb_feu_mac_tx_fault_source_e;
    ...
    uint64 *counter_val)
{
    ...
    DNX_ERR_RECOVERY_EXPR_SUPPRESS_IF_ERR_EXIT(imb_feu_internal_mac_access_control_set(unit, port));
    ...
    const uint64 *counter_val)
{
    ...
    SHR_IF_ERR_EXIT(imb_feu_internal_mac_access_control_set_er_journal_insert(unit, port));
    ...
    bcm_port_t port)
{
    ...
    imb_feu_mac_tx_fault_source_e tx_fault_src;
     ...
     * Enable padding
     */
    pad_size = dnx_data_nif.flexe.mac_pad_size_get(unit);
    SHR_IF_ERR_EXIT(imb_feu_port_pad_size_set(unit, port, pad_size));
    /*
    ...
    else
    {
         ...
         */
        tx_fault_src =
            DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC_L1(unit,
                                               port_info) ? IMB_FEU_MAC_TX_FAULT_SOURCE_EXT_INPUT :
            IMB_FEU_MAC_TX_FAULT_SOURCE_RX_FAULT_SM;
        SHR_IF_ERR_EXIT(imb_feu_port_mac_tx_fault_src_set(unit, port, tx_fault_src));
    }
    ...
    int in_reset)
{
    ...
    {
        SHR_IF_ERR_EXIT(imb_feu_port_enable_get(unit, port_i, &is_enabled));
        if (!is_enabled)
        {
            /*
    ...
    int unit)
{
     ...
     */
    for (client_channel = 0; client_channel < dnx_data_nif.flexe.nof_clients_get(unit); ++client_channel)
    {
        if (!_SHR_PBMP_MEMBER(active_client_channel, client_channel))
        {
            SHR_IF_ERR_EXIT(imb_feu_internal_tx_credit_counter_enable_set(unit, client_channel, 0));
        }
    }
    /** Build uniform calendar */
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, port_info, TRUE))
    {
        ...
        if (IMB_FLEXE_DIRECTION_TX_ONLY_GET(flags))
        {
            ...
            SHR_IF_ERR_CONT(flexe_core_mac_client_delete(unit, client_channel, 0));
    ...
    else
    {
         ...
         */
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(flexe_core_sar_client_delete(unit, client_channel, flags, 0));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
 ...
 */
int
imb_feu_port_remote_fault_enable_set(
    int unit,
    bcm_port_t port,
    int enable)
{
 ...
 */
int
imb_feu_port_local_fault_enable_set(
    int unit,
    bcm_port_t port,
    int enable)
{
 ...
 */
int
imb_feu_port_remote_fault_enable_get(
    int unit,
    bcm_port_t port,
    int *enable)
{
 ...
 */
int
imb_feu_port_local_fault_enable_get(
    int unit,
    bcm_port_t port,
    int *enable)
{
 ...
 */
int
imb_feu_port_link_fault_force_set(
    int unit,
    bcm_port_t port,
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(imb_feu_port_link_fault_force_get(unit, port, type, &orig_enable));
    ...
    if (enable)
    {
         ...
         */
        tx_fault_src = IMB_FEU_MAC_TX_FAULT_SOURCE_SW_CFG;
    }
    else
    {
        /*
    ...
    if (enable)
    {
        ...
        SHR_IF_ERR_EXIT(imb_feu_mac_set(unit, port, DBAL_TABLE_TINY_MAC_RX_LSS_CTRL, dbal_field_to_clear, 0));
    }
    /*
     * Force Link fault
     */
    ...
    SHR_IF_ERR_EXIT(imb_feu_mac_set(unit, port, DBAL_TABLE_TINY_MAC_RX_LSS_CTRL, dbal_field, enable));
exit:
    SHR_FUNC_EXIT;
}
/**
 ...
 */
int
imb_feu_port_link_fault_force_get(
    int unit,
    bcm_port_t port,
    bcm_port_control_t type,
    int *enable)
{
    ...
    dbal_field =
        (type ==
         bcmPortControlLinkFaultLocalForce) ? DBAL_FIELD_FORCE_LOCAL_FAULT_OS : DBAL_FIELD_FORCE_REMOTE_FAULT_OS;
    SHR_IF_ERR_EXIT(imb_feu_mac_get(unit, port, DBAL_TABLE_TINY_MAC_RX_LSS_CTRL, dbal_field, (uint32 *) enable));
exit:
    SHR_FUNC_EXIT;
}
...
/** See .h file */
int
imb_feu_port_faults_status_get(
    int unit,
    bcm_port_t port,
    int *local_fault,
    int *remote_fault)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(imb_feu_port_local_fault_status_get(unit, port, local_fault));
    SHR_IF_ERR_EXIT(imb_feu_port_remote_fault_status_get(unit, port, remote_fault));
