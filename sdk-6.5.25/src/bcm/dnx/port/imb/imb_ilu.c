...
/**
 * \brief
 *   Put ILU Rx in OFR out of reset.
 */
static int
imb_ilu_rx_ofr_sch_reset_out_of_reset(
    int unit,
    bcm_port_t port)
{
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
     ...
     * Take NMG Rx HRF out of reset
     */
    SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_counter_reset_set(unit,
                                                              ilkn_access_info.ilkn_id,
                                                              tdm_mode == DNX_ALGO_PORT_TDM_MODE_BYPASS, FALSE));
...
/**
 * \brief
 *   Put ILU Rx in OFR into reset.
 */
static int
imb_ilu_rx_ofr_sch_reset_in_reset(
    int unit,
    bcm_port_t port)
{
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
    ...
    if (BCM_PBMP_IS_NULL(channels))
    {
        SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_counter_reset_set(unit, ilkn_access_info.ilkn_id, is_tdm, TRUE));
    }
    ...
    int in_reset)
{
    ...
    if (in_reset == IMB_COMMON_IN_RESET)
    {
        SHR_IF_ERR_EXIT(imb_ilu_rx_ofr_sch_reset_in_reset(unit, port));
    }
    else
    {
        SHR_IF_ERR_EXIT(imb_ilu_rx_ofr_sch_reset_out_of_reset(unit, port));
    }
    ...
    int in_reset)
{
    ...
    SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_counter_reset_set(unit,
                                                              ilkn_id,
                                                              sch_priority == bcmPortNifSchedulerTDM, in_reset));
    ...
    int enable)
{
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
    bcm_port_nif_scheduler_t sch_prio;
    int is_tdm_priority_in_port = 0;
    int is_high_priority_in_port = 0;
    ...
    if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_scheduler_context_per_port_support))
    {
        ...
        /** Check if it is the last channel */
        SHR_IF_ERR_EXIT(dnx_algo_port_channels_nof_get(unit, port, &nof_channels));
        is_tdm_priority_in_port = ((nof_channels == 1) && (enable == 0)) ? 0 : 1;
        ...
        if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
            SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_sched_request_enable_set
                            (unit, ilkn_access_info.ilkn_id, DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_HIGH,
                             is_high_priority_in_port));
            SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_sched_request_enable_set
                            (unit, ilkn_access_info.ilkn_id, DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_LOW,
                             !is_high_priority_in_port));
            SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_sched_request_enable_set
                            (unit, ilkn_access_info.ilkn_id, DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_TDM,
                             is_tdm_priority_in_port));
            SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_is_high_priority_config_set
                            (unit, ilkn_access_info.ilkn_id, is_high_priority_in_port));
        }

        else
        {
            SHR_IF_ERR_EXIT(imb_ilu_port_nmg_sch_config_hw_set
                            (unit, ilkn_access_info.ilkn_core, ilkn_access_info.port_in_core, is_tdm_priority_in_port,
                             is_high_priority_in_port, dbal_sch_weight_low));
        }
    ...
    else
    {
         ...
         */
        is_high_priority_in_port = (nof_high_sch_prio_ports > 0) ? 1 : 0;
        /*
         ...
         * For TDM scheduler priorities we set maximum weight.
         */
        is_tdm_priority_in_port = ((nof_high_sch_prio_ports + nof_low_sch_prio_ports == 0) && (enable == 0)) ? 0 : 1;
         ...
         */
        SHR_IF_ERR_EXIT(imb_ilu_core_scheduler_config_hw_set
                        (unit, ilkn_access_info.ilkn_core, is_tdm_priority_in_port, is_high_priority_in_port,
                         dbal_sch_weight_low));
    ...
    imb_ilu_sch_config_t * sch_config)
{
    ...
    if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_sched_request_enable_get
                        (unit, ilkn_access_info.ilkn_id, DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_TDM,
...
/**
 * \brief - configure rx scheduler or disable specific HRF (is_enable = FALSE)
 */
static int
    ...
    imb_ilu_sch_config_t * sch_config)
{
    ...
    if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_sched_request_enable_set(unit, ilkn_access_info.ilkn_id,
    ...
    {
        /*
         * Disable priority that matches HRF type. Otherwise - keep existing configuration 
         */
    ...
    imb_ilu_sch_config_t * current_sch_config)
{
    ...
    SHR_IF_ERR_EXIT(imb_ilu_rx_sch_config_set(unit, ilkn_access_info, &sch_config_copy));
    ...
    int is_enable)
{
    ...
    else
    {
        if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
            SHR_IF_ERR_EXIT(imb_ilu_internal_ofr_rx_counter_reset_set(unit, ilkn_access_info.ilkn_id,
                                                                      ilkn_hrf_type ==
                                                                      DBAL_ENUM_FVAL_ILKN_HRF_TYPE_TDM, is_enable));
        }

        else if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_scheduler_context_per_port_support))
        {
            SHR_IF_ERR_EXIT(imb_ilu_internal_nmg_rx_hrf_reset(unit, ilkn_access_info.ilkn_core,
                                                              ilkn_access_info.port_in_core, ilkn_hrf_type, is_enable));
        }
        else
        {
            SHR_IF_ERR_EXIT(imb_ilu_internal_nmg_rx_core_reset(unit, ilkn_access_info.ilkn_core,
                                                               hrf_sch_priority, is_enable));
        }
    }
    ...
    dbal_enum_value_field_ilkn_hrf_type_e ilkn_hrf_type)
{
    ...
    if (is_sync_with_sch_required)
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_ilu_hrf_outgoing_data_disable(unit, ilkn_access_info, ilkn_hrf_type, &sch_config));

        /*
         * Ensure scheduler will stay synced with HRF after HRF is reset
         */
        SHR_IF_ERR_EXIT(imb_ilu_hrf_sch_sync_enable_set(unit, port, ilkn_access_info, ilkn_hrf_type, hrf_sch_priority,
                                                        TRUE));
    ...
    if (is_sync_with_sch_required)
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_ilu_rx_sch_config_set(unit, ilkn_access_info, &sch_config));
    }
    ...
    bcm_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(imb_ilu_hrf_rx_segmentation_reset(unit, port,
                                                      (nof_ports_on_core == 1) ? 1 : 0, IMB_COMMON_IN_RESET));
    ...
    int enable)
{
    ...
    if (enable)
    {
        ...
        if (dnx_data_esb.general.feature_get(unit, dnx_data_esb_general_esb_support)
            && DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info) && !soc_is(unit, J2X_DEVICE))
        {
    ...
    else
    {
        ...
        _SHR_PBMP_ITER(pbmp_same_if, port_i)
        {
            SHR_IF_ERR_EXIT(dnx_egr_queuing_port_enable_and_flush_set(unit, port_i, enable, TRUE));
        }
