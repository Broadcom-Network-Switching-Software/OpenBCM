    ...
    int ethu_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_local_to_global_get(unit, core, ethu_id, &global_ethu_id));

    SHR_IF_ERR_EXIT(dnx_algo_global_ethu_id_lane_info_get(unit, global_ethu_id, &first_lane, &nof_lanes_in_ethu));
    ...
    int loopback)
{
    ...
    if (soc_is(unit, J2X_DEVICE))
    {
        ...
        if (loopback == BCM_PORT_LOOPBACK_MAC)
        {
            ...
            SHR_IF_ERR_EXIT(soc_reg32_get(unit, CDMAC_CTRLr, port, 0, &rval));
            soc_reg_field_set(unit, CDMAC_CTRLr, &rval, LOCAL_LPBKf, 1);
            SHR_IF_ERR_EXIT(soc_reg32_set(unit, CDMAC_CTRLr, port, 0, rval));

            SHR_EXIT();
        }
        else if (curr_loopback == BCM_PORT_LOOPBACK_MAC && loopback == BCM_PORT_LOOPBACK_NONE)
        {
            SHR_IF_ERR_EXIT(soc_reg32_get(unit, CDMAC_CTRLr, port, 0, &rval));
    ...
    int *loopback)
{
    ...
    *loopback = 0;
    ...
    if (soc_is(unit, J2X_DEVICE))
    {
        ...
        *loopback = (enable ? BCM_PORT_LOOPBACK_MAC : BCM_PORT_LOOPBACK_NONE);
