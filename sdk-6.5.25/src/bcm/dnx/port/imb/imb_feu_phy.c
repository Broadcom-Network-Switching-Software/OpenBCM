    ...
    int imb_id)
{
    ...
    if (!is_active)
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_feu_internal_lf_rf_li_encap_enable_set(unit, 1));
        /*
    ...
    uint32 flags)
{
    ...
    if (!is_flexe_clock_src_pm_initialized && (flexe_clock_src_pm != pm_id))
    {
        ...
        SHR_IF_ERR_CONT(portmod_core_add(unit, flexe_clock_src_pm, core_add_flags, &add_info));
    ...
    DNX_ERR_RECOVERY_SUPPRESS(unit);
    SHR_IF_ERR_CONT(portmod_core_add(unit, pm_id, core_add_flags, &add_info));
    DNX_ERR_RECOVERY_UNSUPPRESS(unit);
    if (SHR_FUNC_ERR())
    {
        SHR_EXIT();
    ...
    DNX_ERR_RECOVERY_SUPPRESS(unit);
    SHR_IF_ERR_CONT(portmod_port_add(unit, port, &add_info));
    DNX_ERR_RECOVERY_UNSUPPRESS(unit);
    if (SHR_FUNC_ERR())
    {
        SHR_EXIT();
    ...
    bcm_port_t port)
{
    ...
    DNX_ERR_RECOVERY_SUPPRESS(unit);
    SHR_IF_ERR_CONT(portmod_port_remove(unit, port));
    DNX_ERR_RECOVERY_UNSUPPRESS(unit);
    if (SHR_FUNC_ERR())
    {
        SHR_EXIT();
     ...
     */
    DNX_ERR_RECOVERY_SUPPRESS(unit);
    SHR_IF_ERR_CONT(flexe_core_channel_map_set(unit, BCM_PORT_FLEXE_RX | BCM_PORT_FLEXE_TX, flexe_core_port, 0xF));
    DNX_ERR_RECOVERY_UNSUPPRESS(unit);
    if (SHR_FUNC_ERR())
    {
        SHR_EXIT();
     ...
     */
    DNX_ERR_RECOVERY_SUPPRESS(unit);
    SHR_IF_ERR_CONT(flexe_core_phy_default_config_set(unit, flexe_core_port, speed, 0));
    DNX_ERR_RECOVERY_UNSUPPRESS(unit);
    if (SHR_FUNC_ERR())
    {
        SHR_EXIT();
    ...
    int speed)
{
    ...
    if (has_speed)
    {
        ...
        {
            /*
             * Clear FlexE Default instatnce configuration
             */
            SHR_IF_ERR_EXIT(flexe_core_phy_default_config_set(unit, flexe_core_port_tmp, speed, 0));
            /*
    ...
    if (flexe_core_port_tmp == DNX_ALGO_PORT_INVALID)
    {
         ...
         */
        SHR_IF_ERR_EXIT(flexe_core_phy_default_config_set(unit, flexe_core_port_tmp, speed, 1));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *is_required)
{
     ...
     */
    DNX_ERR_RECOVERY_SUPPRESS(unit);
    SHR_IF_ERR_CONT(portmod_port_speed_config_get(unit, logical_port, &speed_config));
    DNX_ERR_RECOVERY_UNSUPPRESS(unit);
    if (SHR_FUNC_ERR())
    {
        SHR_EXIT();
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_cal_simple_fixed_len_cal_build
                    (unit, FALSE, slots_per_phy_port, nof_phy_ports, nof_phy_ports, nof_phy_ports, calendar));
    ...
    bcm_port_timesync_tx_info_t * tx_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_flexe_core_port_get(unit, port, &core_port));
