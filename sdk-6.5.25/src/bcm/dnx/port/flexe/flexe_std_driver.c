    ...
    int *latter_lphy)
{
    ...
    *latter_lphy = dnx_data_nif.flexe.nof_flexe_lphys_get(unit) - 1;
    ...
    while (DNX_SW_STATE_LL_IS_NODE_VALID(node))
    {
        ...
        {
            *pre_lphy = shr_bitop_last_bit_get(lphy_bitmap_tmp, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS);
        }
        else if (!is_latter_lphy_found)
        {
            *latter_lphy = shr_bitop_first_bit_get(lphy_bitmap_tmp, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS);
    ...
    const flexe_core_port_info_t * port_info)
{
    ...
    SHR_BITCLR_RANGE(group_instances, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_INSTANCES);
    /*
    ...
    for (ii = 0; ii < nof_ports; ++ii)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.phy_speed.set(unit, flexe_core_ports_arr[ii], port_info->speed));
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.
        ...
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.
                        nof_instances.set(unit, flexe_core_ports_arr[ii], FLEXE_PHY_NOF_INSTANCES(port_info->speed)));
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.
                        demux_lphy_bitmap.bit_range_write(unit, flexe_core_ports_arr[ii], 0, 0,
                                                          DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS, demux_lphy_bitmap));
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.
    ...
    {
        pos = 0;
        SHR_BIT_ITER(group_instances, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_INSTANCES, instance)
        {
            SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.demux_lphy_bitmap.bit_range_read(unit, instance, 0, 0,
                                                                                         ...
                                                                                         demux_lphy_bitmap));
            SHR_BIT_ITER(demux_lphy_bitmap, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS, lphy)
            {
    ...
    if (flags & BCM_PORT_FLEXE_TX)
    {
        ...
        SHR_BIT_ITER(group_instances, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_INSTANCES, instance)
        {
            SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.mux_lphy_bitmap.bit_range_read(unit, instance, 0, 0,
                                                                                       DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS,
                                                                                       mux_lphy_bitmap));
            SHR_BIT_ITER(mux_lphy_bitmap, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS, lphy)
            {
                mux_lphys[pos++] = lphy;
            }
        }
    ...
    soc_pbmp_t * flexe_core_port_bmp)
{
    ...
    SHR_BITCLR_RANGE(group_demux_lphys, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS);
    SHR_BITCLR_RANGE(group_mux_lphys, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS);
    ...
    _SHR_PBMP_ITER(*flexe_core_port_bmp, flexe_core_port)
    {
        ...
        SHR_BITCLR_RANGE(demux_lphy_bitmap, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS);
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.demux_lphy_bitmap.bit_range_read
                        (unit, flexe_core_port, 0, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS, demux_lphy_bitmap));

        SHR_BITOR_RANGE(group_demux_lphys, demux_lphy_bitmap, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS,
                        group_demux_lphys);

        SHR_BITCLR_RANGE(mux_lphy_bitmap, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS);
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.mux_lphy_bitmap.bit_range_read
                        (unit, flexe_core_port, 0, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS, mux_lphy_bitmap));

        SHR_BITOR_RANGE(group_mux_lphys, mux_lphy_bitmap, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS, group_mux_lphys);
    }
    ...
    uint32 *global_ts_array)
{
    ...
    for (ii = 0; ii < nof_flexe_core_ports; ++ii)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.demux_lphy_bitmap.bit_range_read
                            (unit, flexe_core_port_array[ii], 0, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS,
                             lphy_bitmap));
        }
        else
        {
    ...
    int enable)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(flexe_std_66b_switch_set_verify(unit, src_channel, dest_channel, index, enable));
    ...
    int logical_phy_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(flexe_std_logical_phy_id_to_instance_id_get(unit, speed, logical_phy_id, ii, &instance_id));
    ...
    int *logical_phy_id)
{
    ...
    SHR_IF_ERR_EXIT(flexe_std_instance_id_to_logical_phy_id_get(unit, speed, instance_id, logical_phy_id));
    ...
    int enable)
{
    ...
    if (enable)
    {
        shr_bitop_range_count(mux_lphy_bitmap, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_FLEXE_LPHYS, &nof_lphys);
        /*
