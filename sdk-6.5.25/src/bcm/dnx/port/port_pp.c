    ...
    int filter_flags)
{
    ...
    if ((dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use) &&
         (BCM_GPORT_IS_VLAN_PORT(port) || _SHR_GPORT_IS_FLOW_LIF(port))) ||
        (dnx_data_flow.mpls.feature_get(unit, dnx_data_flow_mpls_ingress_flow_in_use) &&
         (BCM_GPORT_IS_MPLS_PORT(port) || _SHR_GPORT_IS_FLOW_LIF(port))))
    {
    ...
    int in_lif_same_interface_mode)
{
    ...
    /** Flow-lif procedures */
    if ((dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use) &&
         (BCM_GPORT_IS_VLAN_PORT(gport) || _SHR_GPORT_IS_FLOW_LIF(gport))) ||
        (dnx_data_flow.mpls.feature_get(unit, dnx_data_flow_mpls_ingress_flow_in_use) &&
         (BCM_GPORT_IS_MPLS_PORT(gport) || _SHR_GPORT_IS_FLOW_LIF(gport))))
    {
    ...
    int preserve_dscp_mode)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    dnx_in_lif_profile_last_info_t_init(&last_profile);
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_in_lif_profile_preserve_dscp_set_verify(unit, gport, preserve_dscp_mode));
    ...
    int *preserve_dscp_mode)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_in_lif_profile_preserve_dscp_get_verify(unit, gport, preserve_dscp_mode));
    ...
    bcm_core_t * core)
{
     ...
     */
    if ((flags & DNX_ALGO_PORT_APP_FLAG_COE) && (port_info.port_type != DNX_ALGO_PORT_TYPE_RCY_MIRROR))
    {
    ...
    int recycle_app_type)
{
    ...
    switch (recycle_app_type)
    {
        ...
        {
            /*
             * when working with IBCH1 the same port can be used for several RCY applications 
             */
            if (!dnx_data_headers.
                system_headers.feature_get(unit, dnx_data_headers_system_headers_system_headers_ibch1_supported))
            {
    ...
    uint32 vlan_domain)
{
    ...
    if (!BCM_GPORT_IS_TUNNEL(gport_lif) && !BCM_GPORT_IS_MPLS_PORT(gport_lif) && !_SHR_GPORT_IS_FLOW_LIF(gport_lif))
    {
    ...
    uint32 vlan_domain)
{
    ...
    /** Flow-lif procedures */
    if ((dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use) &&
         (BCM_GPORT_IS_VLAN_PORT(gport_lif) || _SHR_GPORT_IS_FLOW_LIF(gport_lif))) ||
        (dnx_data_flow.mpls.feature_get(unit, dnx_data_flow_mpls_ingress_flow_in_use) &&
         (BCM_GPORT_IS_MPLS_PORT(gport_lif) || _SHR_GPORT_IS_FLOW_LIF(gport_lif))))
    {
    ...
    uint32 vlan_domain)
{
    ...
    if ((dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use) &&
         (BCM_GPORT_IS_VLAN_PORT(gport_lif) || _SHR_GPORT_IS_FLOW_LIF(gport_lif))) ||
        (dnx_data_flow.mpls.feature_get(unit, dnx_data_flow_mpls_egress_flow_in_use) &&
         (BCM_GPORT_IS_MPLS_PORT(gport_lif) || _SHR_GPORT_IS_FLOW_LIF(gport_lif))))
    {
        SHR_IF_ERR_EXIT(dnx_port_class_flow_lif_property_set(unit, gport_lif, pclass, vlan_domain));
        SHR_EXIT();
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_egress_estimated_bta_btr_config))
        {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESTIMATE_BTR_TERMINATE_SYSTEM_HEADERS,
                                         INST_SINGLE,
                                         egress_port_info->estimated_btr_system_headers_termination_enable);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESTIMATE_BTR_TERMINATE_NETWORK_HEADERS,
                                         INST_SINGLE,
                                         egress_port_info->estimated_btr_network_headers_termination_enable);
        }
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    uint64 data)
{
    ...
    if (dnx_data_flow.wide_data.feature_get(unit, dnx_data_flow_wide_data_flow_in_use))
    {
    ...
    dnx_default_frwrd_dst_t * default_frwrd_dst)
{
         ...
         da_type < DBAL_NOF_ENUM_DESTINATION_DISTRIBUTION_TYPE_VALUES; da_type++)
    {
        ...
        else if (BCM_GPORT_IS_TRAP(defaut_dst_offset[da_type]) || BCM_GPORT_IS_FORWARD_PORT(defaut_dst_offset[da_type]))
        {
            default_frwrd_dst[da_type].add_vsi_dst = FALSE;
            default_frwrd_dst[da_type].offset = 0;
            SHR_IF_ERR_EXIT(algo_gpm_encode_destination_field_from_gport
    ...
    bcm_port_flood_group_t * flood_groups)
{
         ...
         da_type < DBAL_NOF_ENUM_DESTINATION_DISTRIBUTION_TYPE_VALUES; da_type++)
    {
        ...
        if ((default_frwrd_dst[da_type].add_vsi_dst == FALSE) &&
            ((defaut_dst_offset[da_type] != BCM_GPORT_BLACK_HOLE) &&
             (!BCM_GPORT_IS_TRAP(defaut_dst_offset[da_type])) &&
             (!BCM_GPORT_IS_FORWARD_PORT(defaut_dst_offset[da_type]))))
        {
