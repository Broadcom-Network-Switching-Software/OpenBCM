    ...
    int value)
{
    ...
    virtual_tcam_app_db_id = PRT_TCAM_APP_DB_COE;
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
                    (unit, port, DNX_ALGO_GPM_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info));
    SHR_IF_ERR_EXIT(dnx_algo_port_pp_port_get(unit, port, &core_id, &pp_port));
    ...
    int value)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_port_sit_tag_swap_set(unit, port, enable, DBAL_ENUM_FVAL_SIT_TAG_SWAP_MODE_GLOBAL_TRIGGER));
    }
    ...
    if (!enable)
    {
        BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
        {
            ...
            if (rv == _SHR_E_NONE)
            {
                ...
                /** clear TCAM SW table */
                SHR_IF_ERR_EXIT(dnx_port_prt_tcam_virtual_port_tcam_sw_set
                                (unit, core_id, DBAL_FIELD_PRTTCAM_KEY_VP_PON, prt_tcam_key, access_id, 0, 0, 0,
                                 FC_CHAN_ID_INVALID, 1));
            }
        }
