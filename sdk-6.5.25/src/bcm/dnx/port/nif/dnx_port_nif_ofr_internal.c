    ...
    int *rmc_id)
{
    ...
    {
        *rmc_id =
            (DNX_PORT_NIF_OFR_PRIORITY_IS_TDM(priority_config->priority_groups[priority_group])) ? nif_interface_id :
            nif_interface_id + dnx_data_nif.ofr.nof_rmc_per_priority_group_get(unit);
    }
    else
    {
        *rmc_id =
            (DNX_PORT_NIF_OFR_PRIORITY_IS_HIGH(priority_config->priority_groups[priority_group])) ? nif_interface_id :
            nif_interface_id + dnx_data_nif.ofr.nof_rmc_per_priority_group_get(unit);
    }
    ...
    const bcm_port_prio_config_t * priority_config)
{
    ...
    for (priority_group = 0; priority_group < priority_config->nof_priority_groups; ++priority_group)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_nif_db.ofr_link_list.nof_sections_allocated.get(unit, priority_index, nif_interface_id,
    ...
    int in_reset)
{
    ...
    if ((DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, L1))) ||
        (DNX_ALGO_PORT_TYPE_IS_FLEXE(unit, port_info, TRUE /** include mac */ , FALSE /** include phy */ )))
    {
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_SAR(unit, port_info))
    {
        /*
         * alloc DBAL table handle
         */
...
dnx_port_ofr_rx_sch_context_quantum_value_set(
    ...
    bcm_port_t port)
{
    int nif_interface_id, core, speed;
    uint32 entry_handle_id;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
     ...
     * set value field
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_QUANTUM_VALUE, INST_SINGLE,
                                 (speed / dnx_data_nif.ofr.rx_sch_granularity_get(unit)));
        ...
        switch (priority_config->priority_groups[priority_group].sch_priority)
        {
            ...
            case bcmPortNifSchedulerHigh:
                    ...
                    DNX_ALGO_PORT_TYPE_IS_FLEXE_CLIENT(unit, port_info))
                {
                     ...
                     */
                    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                                    (unit, DBAL_TABLE_NIF_OFR_RX_SCHEDULER_REQUEST_ENABLE_CONFIG, &entry_handle_id));
                    /*
                     * set key fields
                     */
                ...
                else if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1)))
                {
                     ...
                     */
                    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                                    (unit, DBAL_TABLE_NIF_OFR_ILKN_RX_SCHEDULER_REQUEST_ENABLE, &entry_handle_id));
                    /*
                     * set key fields
                     */
                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_ID, ilkn_access_info.ilkn_id);
                    SHR_IF_ERR_EXIT(imb_common_dbal_sch_prio_from_bcm_sch_prio_get(unit,
                                                                                   priority_config->priority_groups
                                                                                   [priority_group].sch_priority,
                                                                                   &dbal_sch_prio));
                    ...
                    /*
                     * clear DBAL table handle
                     */
                    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_NIF_OFR_ILKN_SCHEDULER_CONFIG, entry_handle_id));
                    /*
                     * set key fields
                     */
                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_ID, ilkn_access_info.ilkn_id);
                    ...
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_ILKN_HIGH, INST_SINGLE,
                                                 DNX_PORT_NIF_OFR_PRIORITY_IS_HIGH(priority_config->priority_groups
                                                                                   [priority_group]));
            ...
            case bcmPortNifSchedulerTDM:
                    ...
                    DNX_ALGO_PORT_TYPE_IS_FLEXE_CLIENT(unit, port_info))
                {
                     ...
                     */
                    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                                    (unit, DBAL_TABLE_NIF_OFR_TDM_ETH_RX_SCHEDULER_REQUEST_ENABLE, &entry_handle_id));
                    /*
                     * set key fields
                     */
                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_ETH_PORT_ID, nif_interface_id);
