    ...
    for (blk_index = blk_index_min; blk_index <= blk_index_max; blk_index++)
    {
        for (array_index = array_index_min; array_index <= array_index_max; array_index++)
        {
            for (index = index_min; index <= index_max; index++)
            {
                ...
                {
                    SHR_IF_ERR_EXIT(WRITE_TCAM_INDIRECT_COMMANDr(unit, _SHR_CORE_ALL, 0xE000003));
                }
 ...
 * \param [in] reset_value - Init value (only LSB) - used for testing - should be zero for standard initialziation.
 * \return
 *   See shr_error_e
 * \remark
 *   * None
...
/**
* \brief - Set all valid writeable memories which are not included in mem_exclude_bitmap using indirect commands.
*
* \param [in] unit - unit #
* \param [in] mem_exclude_bitmap - Each bit represent the memories that should be excluded (will be skipped)
* \return
    ...
    for (cmd_id = 0; cmd_id < DNX_INIT_MEM_INDIRECT_CMDS_PER_BLOCK_MAX; cmd_id++)
    {
        for (blk_index = 0; blk_index < SOC_MAX_NUM_BLKS; blk_index++)
        {
            if (nof_cmds[blk_index] > cmd_id)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_access_init_handle_error_skipping
                                (unit,
                                 sand_mem_indirect_reset_write(unit, cmd->mem, cmd->blk_id, mem_addr,
                                                               cmd->nof_entries, entry)));
    ...
    int unit)
{
    ...
    const soc_reg_t *dynamic_mem_enable_regs = NULL;
    const soc_reg_t *dynamic_mem_disable_regs = NULL;
    ...
    if (dnx_data_device.general.device_id_get(unit) == 0x8830)
    {
        dynamic_mem_enable_regs = j2x_dynamic_mem_enable_regs;
        dynamic_mem_disable_regs = j2x_dynamic_mem_disable_regs;
    }
    else if (dnx_data_device.general.device_id_get(unit) == 0x8850)
    {
        dynamic_mem_enable_regs = j2p_dynamic_mem_enable_regs;
        dynamic_mem_disable_regs = j2p_dynamic_mem_disable_regs;
    }
    else if (dnx_data_device.general.device_id_get(unit) == 0x8800)
    {
        dynamic_mem_enable_regs = j2c_dynamic_mem_enable_regs;
        dynamic_mem_disable_regs = j2c_dynamic_mem_disable_regs;
    }
    else if (dnx_data_device.general.device_id_get(unit) == 0x8480)
    {
        dynamic_mem_enable_regs = q2a_dynamic_mem_enable_regs;
        dynamic_mem_disable_regs = q2a_dynamic_mem_disable_regs;
    }
    else
    {
        dynamic_mem_enable_regs = jr2_dynamic_mem_enable_regs;
        dynamic_mem_disable_regs = jr2_dynamic_mem_disable_regs;
    }

    SHR_IF_ERR_EXIT(soc_sand_control_dynamic_mem_writes(unit, dynamic_mem_enable_regs, 1));     /* enable * * * dynamic
                                                                                                 * * * * * memory * * *
                                                                                                 * * * * * * * * * * * *
                                                                                                 * * * * * * * * * * * *
                                                                                                 * * * * * * * * * * * *
    ...
    SHR_IF_ERR_EXIT(soc_sand_control_dynamic_mem_writes(unit, dynamic_mem_disable_regs, 0)); /** disable dynamic memory writes */
    ...
    int unit)
{
        ...
        switch (default_mem->mode)
        {
            ...
            case dnx_init_mem_default_mode_zero:
            {
                ...
                if (!SAL_BOOT_PLISIM)
                {
                    SHR_IF_ERR_EXIT(dnx_access_init_handle_error_skipping
                                    (unit,
                                     sand_fill_table_with_entry(unit, default_mem->mem, dnx_drv_mem_block_all(),
                                                                entry)));
            ...
            case dnx_init_mem_default_mode_all_ones:
            {
                ...
                if (soc_sand_is_emulation_system(unit))
                {
                    ...
                    else
                    {
                        ...
                        SHR_IF_ERR_EXIT(dnx_access_init_handle_error_skipping
                                        (unit,
                                         sand_fill_table_with_entry(unit, default_mem->mem,
    ...
    SHR_IF_ERR_EXIT(soc_sand_control_dynamic_mem_writes(unit, dynamic_mem_disable_regs, 0));
