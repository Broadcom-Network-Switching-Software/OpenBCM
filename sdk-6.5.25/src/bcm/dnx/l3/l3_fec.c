    ...
    l3_fec_entry_info_t * fec_entry_info)
{
    ...
    dbal_result_type_t super_fec_type = 0;
    ...
    if (core_mode == DBAL_CORE_MODE_DPC)
    {
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fecz.
                        direction.get(unit, fec_entry_info->fec_id, &fec_entry_info->direction));
        SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fecz.
                        hierarchy.get(unit, fec_entry_info->fec_id, &fec_entry_info->hierarchy));
    }
    ...
    if (!dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_fec_hit_bit))
    {
            ...
            || (_SHR_IS_FLAG_SET(fec_entry_info->flags, L3_FEC_ENTRY_INFO_GET_CLEAR_HIT_BIT)))
        {
             ...
             * Set the attribute
             */
            SHR_IF_ERR_EXIT(dbal_entry_attribute_request(unit, entry_handle_id, attr_type, &attr_val));
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, &super_fec_type));

    fec_type_in_super_fec = dnx_data_l3.fec.super_fec_result_types_map_get(unit, super_fec_type)->fec_type_in_super_fec;

    /*
     * Retrieve the FEC resource type based on super FEC type
     */
    fec_resource_type = dnx_data_l3.fec.super_fec_result_types_map_get(unit, super_fec_type)->fec_resource_type;

    if (dnx_data_l3.fec.fec_resource_info_get(unit, fec_resource_type)->is_statistic_type)
    {
        stat_instance =
            dnx_data_l3.fec.fec_resource_info_get(unit, fec_resource_type)->is_protection_type ? 0 : instance;
    ...
    * hierarchical tm flow resolution
    ...
    if (dnx_data_l3.fec.fec_resource_info_get(unit, fec_resource_type)->is_protection_type)
    {
    ...
    dnx_l3_fec_entry_actions_e entry_action)
{
    ...
    /** Result fields */
    dbal_result_type_t super_fec_type;
     ...
     *   * hierarchy
     */
    SHR_IF_ERR_EXIT(dnx_algo_l3_fec_super_fec_type_get(unit, fec_entry_info->fec_id, (uint32 *) &super_fec_type));
    /*
     * Get FEC physical DB based on FEC ID
     */
    SHR_IF_ERR_EXIT(algo_l3_fec_db_stage_map_get(unit, fec_entry_info->fec_id, &fec_db));
    ...
    instance = L3_FEC_ID_TO_FEC_INSTANCE(fec_entry_info->fec_id);
    result_type_map = dnx_data_l3.fec.super_fec_result_types_map_get(unit, super_fec_type);
    /*
     ...
     * Retrieve the FEC resource type based on super FEC type
     */
    fec_resource_info = dnx_data_l3.fec.fec_resource_info_get(unit, result_type_map->fec_resource_type);

    if (fec_resource_info->is_statistic_type)
    {
        stat_instance = fec_resource_info->is_protection_type ? 0 : instance;
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fecz.
                            direction.set(unit, fec_entry_info->fec_id, fec_entry_info->direction));

            SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fecz.
                            hierarchy.set(unit, fec_entry_info->fec_id, fec_entry_info->hierarchy));
        }
    }
    ...
    bcm_if_t * if_id)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_WITH_ID))
    {
        ...
        if (_SHR_IS_FLAG_SET(flags, BCM_L3_REPLACE))
        {
            ...
            uint32 super_fec_res_type = 0;
            uint32 super_fec_new_type;
            uint8 is_protected;
            uint8 is_stat_fec;
            ...
            SHR_IF_ERR_EXIT(dnx_algo_l3_fec_super_fec_type_get(unit, BCM_L3_ITF_VAL_GET(*if_id), &super_fec_new_type));
    ...
    bcm_if_t * if_id)
{
    ...
    if (is_alloc_required == TRUE)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_l3_fec_allocate(unit, &fec_index, alloc_flags, fec_db, super_fec_res_type));
    }
    /*
    ...
    bcm_l3_egress_t * egr)
{
    ...
    if (dnx_data_l3.fec.feature_get(unit, dnx_data_l3_fec_separate_fwd_rpf_dbs))
    {
    ...
    bcm_if_t * if_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_l3_fec_allocation_get(unit, nof_members, db, super_fec_type, if_id));
