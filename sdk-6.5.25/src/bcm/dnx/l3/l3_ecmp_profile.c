...
/** Get the Hw value of the RPF mode based on input bcm_l3_ecmp_urpf_mode_t value */
#define DNX_L3_ECMP_RPF_MODE_GET(rpf_mode)  ((rpf_mode == bcmL3EcmpUrpfLoose) ? DBAL_ENUM_FVAL_ECMP_RPF_MODE_LOOSE : \
    ...
    bcm_if_t * intf_array)
{
    ...
    ecmp_profile->rpf_mode = DNX_L3_ECMP_RPF_MODE_GET(ecmp->rpf_mode);
    ...
    bcm_if_t * intf_array)
{
    ...
    if (ecmp_profile_data.rpf_mode == DBAL_ENUM_FVAL_ECMP_RPF_MODE_LOOSE)
    {
        ecmp->rpf_mode = bcmL3EcmpUrpfLoose;
    }
    else if (ecmp_profile_data.rpf_mode == DBAL_ENUM_FVAL_ECMP_RPF_MODE_STRICT_EM)
    {
    ...
    bcm_if_t * intf_array)
{
    ...
    if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_WITH_ID))
    {
        ...
        if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_REPLACE))
        {
            ...
            if (ecmp_profile_data.rpf_mode != DNX_L3_ECMP_RPF_MODE_GET(ecmp->rpf_mode))
            {
    ...
    bcm_if_t * intf_array)
{
    ...
    dnx_l3_egress_ecmp_user_profile_build_struct(unit, ecmp, &ecmp_profile, intf_count, intf_array);
