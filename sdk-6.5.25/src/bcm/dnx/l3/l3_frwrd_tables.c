    ...
    uint32 *dbal_table)
{
    ...
    if (!is_kbp_entry)
    {
        ...
        if (dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_public_routing_support) &&
            ((info->l3a_vrf == 0) || _SHR_IS_FLAG_SET(info->l3a_flags2, BCM_L3_FLAGS2_SCALE_ROUTE)))
        {
    ...
    uint8 is_kbp_split_rpf_in_use)
{
    ...
    else
    {
        ...
        if (dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_separate_fwd_rpf_dbs))
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "One of the flags BCM_L3_FLAGS2_FWD_ONLY or BCM_L3_FLAGS2_RPF_ONLY must be set\n");
            }
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Only one of the flags BCM_L3_FLAGS2_FWD_ONLY or BCM_L3_FLAGS2_RPF_ONLY must be set\n");
            }

            if (_SHR_IS_FLAG_SET(info->l3a_flags2, BCM_L3_FLAGS2_NO_PAYLOAD)
                && !_SHR_IS_FLAG_SET(info->l3a_flags2, BCM_L3_FLAGS2_RPF_ONLY))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Route flag BCM_L3_FLAGS2_NO_PAYLOAD is only supported for RPF tables\n");
            }
        }
    }
 ...
 * \param [in] is_old_entry_without_payload - Indication whether RPF entry with no payload was configured
...
dnx_l3_route_payload_verify(
    ...
    uint8 is_kbp_split_rpf_in_use,
    uint8 is_replace,
    uint8 is_old_entry_without_payload,
    uint32 dbal_table,
    ...
    else
    {
                     ...
                     DBAL_RESULT_TYPE_KBP_IPV6_UNICAST_PRIVATE_LPM_FORWARD_KBP_FORWARD_DEST_EEI_W_DEFAULT))
        {
            ...
            if (sub_field_id == DBAL_FIELD_EEI_MPLS_MULTI_SWAP)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                                (unit, DBAL_FIELD_EEI_MPLS_MULTI_SWAP, DBAL_FIELD_MPLS_LABEL, &mpls_val,
                                 &sub_field_val));
    ...
    bcm_l3_route_t * info)
{
    ...
    uint32 is_old_entry_without_payload = 0;
    uint32 entry_handle_id;
    uint32 l3_intf;
    uint32 dbal_table;
    uint8 is_kbp_entry;
    ...
    is_replace = (_SHR_IS_FLAG_SET(info->l3a_flags, BCM_L3_REPLACE) ? TRUE : FALSE);
    ...
    /** Verify payload */
    SHR_IF_ERR_EXIT(dnx_l3_route_payload_verify(unit, info, is_kbp_entry, is_kbp_split_rpf_in_use, is_replace,
                                                (is_old_entry_without_payload ? TRUE : FALSE), dbal_table, &dest_type));
    ...
    bcm_l3_route_t * info)
{
    ...
    rv = dnx_l3_route_payload_get(unit, info, is_kbp_entry, is_kbp_split_rpf_in_use, dbal_table, entry_handle_id);
    ...
    void *user_data)
{
    ...
    for (table = 0; table < nof_tables; table++)
    {
         ...
         */
        is_kbp_table = L3_TABLE_IS_KBP(dbal_tables[table]) ? TRUE : FALSE;
        is_l3_table_public = L3_TABLE_IS_PUBLIC(dbal_tables[table]);

        while (!is_end)
        {
            bcm_l3_route_t_init(&dbal_entry);
            result = is_kbp_table ? DBAL_FIELD_DESTINATION : DBAL_FIELD_KAPS_RESULT;
            ...
            if (is_kbp_table)
            {
                if (L3_TABLE_IS_KBP_RPF(dbal_tables[table]))
                {
                    ...
                    if (dnx_stat_pp_result_type_verify(unit, dbal_tables[table], result_type) == _SHR_E_NONE)
                    {
                        ...
                        if (stat_pp_profile != STAT_PP_PROFILE_INVALID)
                        {
                            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit,
                                                                                ...
                                                                                INST_SINGLE, &dbal_entry.stat_id));
            ...
            if (result == DBAL_FIELD_KAPS_RESULT)
            {
                 ...
                 */
                if (DNX_L3_ENTRY_IS_DEFAULT(unit, (&dbal_entry)))
                {
                    result = DBAL_FIELD_FEC_DEFAULT;
                }
                else
                {
                    result = DBAL_FIELD_FEC;
                }
            }
...

            {
                if (L3_TABLE_IS_KBP_RPF(dbal_tables[table]))
                {
                    ...
                    if (kbp_mngr_split_rpf_in_use(unit))
                    {
                        ...
                        dbal_entry_handle_key_field_arr8_get(unit, entry_handle_id, DBAL_FIELD_IS_DEFAULT,
                                                             &is_kbp_default);
