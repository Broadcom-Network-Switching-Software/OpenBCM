    ...
    bcm_l3_intf_t * intf)
{
    ...
    {
        /*
         * The result type will be determined by the LIF manager
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                     DBAL_SUPERSET_RESULT_TYPE);
    }
    ...
    int old_mymac_prefix_profile)
{
    ...
    /** In case the MAC address is all 0s, use the default MyMac Prefix profile*/
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.source_address_table.exchange
                    (unit, 0, &source_address_entry, old_mymac_prefix_profile, NULL, (int *) my_mac_prefix_index,
...
{
    uint32 entry_handle_id = 0, my_mac_prefix_index = 0;
    ...
    if (dbal_tables_field_info_get_no_err(unit, dbal_table_id, DBAL_FIELD_VRF,
                                          FALSE, table_result_type, INST_SINGLE, &field_info) == _SHR_E_NONE)
    {
    ...
    if (!is_replace)
    {
        ...
        vsi_id = intf->l3a_vid;
        SHR_IF_ERR_EXIT(algo_l3_db.rif_alloc.
                        intf_rif_res_mngr.allocate_single(unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, NULL, &vsi_id));
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
dnx_l3_intf_eth_rif_create_ingress(
    ...
    bcm_l3_intf_t * intf)
{
    uint32 table_result_type, entry_handle_id = 0, old_vsi_profile;
    int propag_profile = 0, org_propag_profile;
    uint32 ecn_mapping_profile;
    ...
    if (is_replace || is_vsi_allocated)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data
                        (unit, old_in_lif_profile, &in_lif_profile_info, dnx_data_l2.vsi.vsi_table_get(unit), ETH_RIF));
        /*
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_vsi_table_initial_result_type_get
                        (unit, vsi_value, VSI_TYPE_ROUTE_ONLY, &table_result_type));
    }
    ...
    if (propag_profile > L3_INTF_QOS_PROPAGATION_PROFILE_MAX)
    {
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange
                    (unit, &in_lif_profile_info, old_in_lif_profile, &new_in_lif_profile, ETH_RIF,
                     dnx_data_l2.vsi.vsi_table_get(unit), &last_profile));
    ...
    /** save VSI info */
    SHR_IF_ERR_EXIT(dnx_l3_intf_vsi_hw_set
...
/**
 * \brief
 *   Retrieve the old LIF profile and the hw resources for the LIF.
 * \param [in] unit - relevant unit
 * \param [in] intf_id - LIF ID
 * \param [in] ing_intf - Input interface data
 * \param [out] old_in_lif_profile - old in lif profile
 * \param [out] gport_hw_resources - HW resources data
 * \return
 *   Negative in case of an error. See \ref shr_error_e
 */
static shr_error_e
dnx_l3_ingress_lif_entry_get(
    int unit,
    bcm_if_t * intf_id,
    bcm_l3_ingress_t * ing_intf,
    ...
    bcm_if_t * intf_id)
{
    ...
    if (BCM_L3_ITF_TYPE_IS_RIF(*intf_id))
    {
        ...
        {
            in_lif_profile_info.ingress_fields.lif_gport = BCM_L3_ITF_VAL_GET(*intf_id);
            SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data
    ...
    {
        BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(gport, *intf_id);
        if (dnx_data_flow.in_lif.feature_get(unit, dnx_data_flow_in_lif_flow_in_use))
        {
            ...
            flow_handle_info.flow_id = dnx_flow_gport_to_flow_id(unit, gport);
            ...
            if (ing_intf->flags != 0 && !_SHR_IS_FLAG_SET(ing_intf->flags, BCM_L3_INGRESS_WITH_ID))
            {
                terminator_info.l3_ingress_info.l3_ingress_flags = ing_intf->flags;
                terminator_info.valid_elements_set |= BCM_FLOW_TERMINATOR_ELEMENT_L3_INGRESS_INFO_VALID;
            }
        ...
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_l3_ingress_lif_entry_get
                            (unit, intf_id, ing_intf, &old_in_lif_profile, &gport_hw_resources));
            if (old_in_lif_profile != 0)
            {
                /*
    ...
    uint8 print_error_if_not_found)
{
    ...
    /** Clear mymac prefix profile from sw algo template. In case of last_ref - clear it from source address eth and l3 mymac prefixes tables */
    L3_INTF_MY_MAC_ETH_PREFIX_RESTORE(old_prefix_profile);
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.source_address_table.exchange
                    (unit, 0, &source_address_entry, old_prefix_profile, NULL, &new_mymac_prefix_profile, &is_first,
                     &is_last));
    /*
    ...
    if (is_vsi_allocated)
    {
        ...
        SHR_IF_ERR_EXIT(in_lif_profile_info_t_init(unit, &new_in_lif_profile_info));
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data
                        (unit, vsi_profile, &in_lif_profile_info, dnx_data_l2.vsi.vsi_table_get(unit), ETH_RIF));
        ...
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange
                        (unit, &new_in_lif_profile_info, vsi_profile, &new_in_lif_profile, ETH_RIF,
                         dnx_data_l2.vsi.vsi_table_get(unit), &last_profile));
        /*
    ...
    bcm_l3_ingress_t * ing_intf)
{
    ...
    if ((dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_cs_in_lif_profile_based_on_stages))
        && (intf_type == ETH_RIF))
    {
        in_lif_profile_info.ingress_fields.lif_gport = BCM_L3_ITF_VAL_GET(intf);
    }
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data
