    ...
    bcm_l3_vpbr_entry_t * entry)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "illegal value for entry->gport_wide_data [%x], valid values are from 0 to %d\n",
    ...
    bcm_l3_vpbr_entry_t * entry)
{
    ...
    VPBR_ENTRY_FIELD32_MASKED_SET_IF_VALID(unit, entry_handle_id, DBAL_FIELD_LIF_WIDE_DATA_21_TO_18,
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ZERO_PADDING, INST_SINGLE, 0);
    ...
    bcm_l3_vpbr_entry_t * entry)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_vpbr_entry_verify(unit, entry));
    ...
    void *user_data)
{
    ...
    while (!is_end)
    {
        ...
        vpbr_entry.gport_wide_data = field_value[0];
        ...
        vpbr_entry.stat_id = field_value[0];

        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_STAT_OBJECT_CMD, INST_SINGLE, field_value));
        vpbr_entry.stat_pp_profile = field_value[0];
    ...
    void *user_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_vpbr_traverse_verify(unit, flags));
