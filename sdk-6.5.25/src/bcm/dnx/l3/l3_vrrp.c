...
/* Calculate the layer protocol in bits */
#define DNX_L3_VRRP_FFC_LAYER_PROTOCOL_WIDTH         utilex_log2_round_up(dnx_data_device.general.max_nof_layer_protocols_get(unit))
...
/*
 * Returns whether the MAC addresse's MSB is one or zero. MAC addr of type bcm_mac_t
 */
#define DNX_L3_VRRP_IS_MAC_ADDRESS_MSB_1(mac_addr) (mac_addr[0] & 0x80)
    ...
    uint32 *internal_flags)
{
    ...
    sal_memset(internal_flags, 0, sizeof(internal_flags[0]) * DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32);
    ...
    uint32 *members_bmp)
{
    ...
    dbal_entry_value_field_arr32_set(unit, entry_handle_id, DBAL_FIELD_L3_PROTOCOLS_BITMAP, INST_SINGLE, members_bmp);
...
{
    uint32 entry_handle_id;
    uint32 temp_members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] = { 0 };
    int protocol_group, array_index;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /** Get protocol group dbal SW table */
    sal_memset(members_bmp, 0, sizeof(members_bmp[0]) * DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32);

    /** create handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_L3_VRRP_PROTOCOL_GROUP_SW, &entry_handle_id));

    /** Read all groups from SW database. If the group matches the mask, add its members to the return value. */
    for (protocol_group = 0; protocol_group < dnx_data_l3.vrrp.nof_protocol_groups_get(unit); protocol_group++)
    {
        if ((protocol_group) == (group_id & group_mask))
        {

            /** set key - group id*/
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_GROUP_ID, protocol_group);
            /** Get value - protocol members bitmap */
            dbal_value_field_arr32_request(unit, entry_handle_id, DBAL_FIELD_L3_PROTOCOLS_BITMAP, INST_SINGLE,
                                           temp_members);

            /** commit */
            SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

            for (array_index = 0; array_index < DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32; array_index++)
            {
                /** add current group protocol members to the total members */
                members_bmp[array_index] |= temp_members[array_index];
            }
        }
    }
    ...
    int group_id)
{
    ...
    rc = dbal_fields_enum_value_get(unit, DBAL_FIELD_LAYER_TYPES, protocol_layer, &protocol_layer_enum);

    /** Get protocol group dbal SW table */
    if (!rc)
    {
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_LAYER_TYPES, protocol_layer_enum, &is_illegal));
        if (!is_illegal)
        {
            /** create handle */
            ...
            /** set key - protocol */
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LAYER_TYPES, protocol_layer_enum);
            /** Get value - protocol members bitmap */
...
dnx_l3_vrrp_protocol_group_members_set(
    ...
    uint32 *members_bmp)
{
    int protocol_group, array_element, member = 0;
    int bitmap_size = 0;
    uint32 old_group_members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] =
        { 0 }, members_bmp_arr[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] =
    {
    0};
    uint32 curr_group_members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] = { 0 };

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    bitmap_size = dnx_data_device.general.max_nof_layer_protocols_get(unit);

    /** Copy the members_bmp array*/
    sal_memcpy(members_bmp_arr, members_bmp, sizeof(members_bmp_arr[0]) * DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32);
    ...
    for (protocol_group = 0; protocol_group < dnx_data_l3.vrrp.nof_protocol_groups_get(unit); protocol_group++)
    {
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_protocol_group_members_sw_get
                        (unit, protocol_group, DNX_L3_VRRP_PROTOCOL_GROUP_MASK_FULL, curr_group_members));

        for (array_element = 0; array_element < DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32; array_element++)
        {
            /** Remove the new group members from all groups. */
            curr_group_members[array_element] &= ~members_bmp_arr[array_element];

            if (protocol_group == 0)
            {
                /** Set all old members of the group that are not in the group anymore to group 0. */
                curr_group_members[array_element] |=
                    (old_group_members[array_element] & ~members_bmp_arr[array_element]);
            }

            if (protocol_group == group_id)
            {
                curr_group_members[array_element] = members_bmp[array_element];
            }
        }
        /** Set SW - group new members */
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_protocol_group_members_sw_set(unit, protocol_group, curr_group_members));
    }

    /** Write to HW  */
    {
        /** first, iterate over all the provided group protocols and clear them from the group (map to group 0)*/
        SHR_BIT_ITER(old_group_members, bitmap_size, member)
        {
            SHR_IF_ERR_EXIT(dnx_l3_vrrp_protocol_layer_to_group_id_hw_set(unit, member, 0));
        }

        /** second, map the new members to the group */
        SHR_BIT_ITER(members_bmp_arr, bitmap_size, member)
        {
...
{
    int protocol_group;
    uint32 valid_members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] = { 0 };
    uint32 default_value[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] = { 0 };
    SHR_FUNC_INIT_VARS(unit);
    ...
    for (protocol_group = DNX_L3_VRRP_L3_PROTOCOL_GROUP_ID_LOWEST_VALID;
         protocol_group < dnx_data_l3.vrrp.nof_protocol_groups_get(unit); protocol_group++)
    {
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_protocol_group_members_sw_set(unit, protocol_group, default_value));
    }
 ...
 * remark-
 *  Even though the lookup should be configure for either sexem1 or sexem3 depending on the stage,
 *  the configuration here is uses sexem1 only. This is because the tables are identical and
 *  they share the same app_db_id.
 */
static shr_error_e
dnx_l3_vrrp_exem_based_multi_my_mac_da_profiles_init(
    int unit)
{
    uint32 entry_handle_id = DBAL_SW_NOF_ENTRY_HANDLES, app_db_id, exem_ffc;
    int app_id_size, core;
    const dnx_data_l3_vrrp_vrrp_tables_t *vrrp_table;
    ...
    /** set VRID_MY_MAC (VRRP protocol) */
    {
        ...
        vrrp_table = dnx_data_l3.vrrp.vrrp_tables_get(unit, L3_VRRP_TABLE_VRID_MY_MAC_WITH_PROTOCOL);
        /** set app db */
        SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, vrrp_table->table[0], &app_db_id, &app_id_size));
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        vrrp_table = dnx_data_l3.vrrp.vrrp_tables_get(unit, L3_VRRP_TABLE_PER_PORT_MY_MAC);
        /** set app db */
        SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, vrrp_table->table[0], &app_db_id, &app_id_size));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EXEM_VRRP_APP_DB_ID, INST_SINGLE, app_db_id);

        /** Set FFC 0 - source port */
        exem_ffc =
            DNX_L3_VRRP_EXEM_FFC_BUILD(0, dnx_data_port.general.pp_port_bits_size_get(unit) - 1,
                                       DNX_L3_VRRP_FFC_IN_PORT_OFFSET(unit));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 0, exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 0, TRUE);

        /** Get the core mode of the table*/
        SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, vrrp_table->table[0], &core_mode));
        /**Core should not be included in the key for DPC tables*/
        if (dnx_data_device.general.nof_bits_for_nof_cores_get(unit) && core_mode != DBAL_CORE_MODE_DPC)
        {
            core_included_in_key = 1;
            /** Set FFC 1 - source port core (1 bit) */
            exem_ffc =
                DNX_L3_VRRP_EXEM_FFC_BUILD(dnx_data_port.general.pp_port_bits_size_get(unit), 1 - 1,
        ...
        /** Set FFC 3 - MAC address */
        exem_ffc =
            DNX_L3_VRRP_EXEM_FFC_BUILD(dnx_data_port.general.pp_port_bits_size_get(unit) + core_included_in_key,
                                       DNX_L3_VRRP_FFC_MAC_WIDTH - 1, DNX_L3_VRRP_FFC_MAC_OFFSET(unit));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 1 + core_included_in_key,
                                     exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 1 + core_included_in_key, TRUE);
    ...
    /** set PER_VSI_MULTIPLE_MY_MAC_MSB_0 (multiple my mac based on VSI and MAC address) */
    {
        ...
        /** set key */
        if (dnx_data_l3.vrrp.is_multiple_my_mac_da_table_per_msb_bit_get(unit))
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_DA_PROFILE,
                                       DBAL_ENUM_FVAL_MY_MAC_EXEM_DA_PROFILE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_0);
            vrrp_table = dnx_data_l3.vrrp.vrrp_tables_get(unit, L3_VRRP_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_0);
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, _SHR_CORE_ALL);
        ...
        /** Set FFC 0 - VSI (use only 17 bits instead of total 18) */
        exem_ffc = DNX_L3_VRRP_EXEM_FFC_BUILD(0, DNX_L3_VRRP_FFC_VSI_WIDTH - 2, DNX_L3_VRRP_FFC_VSI_OFFSET(unit));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 0, exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 0, TRUE);

        /** Set FFC 1 - MAC address - use only 47 LSB bits (instead of total 48) */
        exem_ffc =
            DNX_L3_VRRP_EXEM_FFC_BUILD(DNX_L3_VRRP_FFC_VSI_WIDTH - 1, DNX_L3_VRRP_FFC_MAC_WIDTH - 2,
                                       DNX_L3_VRRP_FFC_MAC_OFFSET(unit));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 1, exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 1, TRUE);
    ...
    if (dnx_data_l3.vrrp.is_multiple_my_mac_da_table_per_msb_bit_get(unit))
    {
        ...
        vrrp_table = dnx_data_l3.vrrp.vrrp_tables_get(unit, L3_VRRP_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_1);
        /** set app db */
        SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, vrrp_table->table[0], &app_db_id, &app_id_size));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EXEM_VRRP_APP_DB_ID, INST_SINGLE, app_db_id);
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        vrrp_table = dnx_data_l3.vrrp.vrrp_tables_get(unit, L3_VRRP_TABLE_PER_PORT_IGNORE_MAC);
        /** set app db */
        SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, vrrp_table->table[0], &app_db_id, &app_id_size));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EXEM_VRRP_APP_DB_ID, INST_SINGLE, app_db_id);
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_switch_control_set(unit, bcmSwitchL2StationExtendedMode, L2_STATION_VSI_MODE));
...
{
    int capacity = 0;
    int non_zero_capacity_count = 0, database_iter;
    const dnx_data_l3_vrrp_data_bases_t *info;

    SHR_FUNC_INIT_VARS(unit);

    info = dnx_data_l3.vrrp.data_bases_get(unit);
    /**Initialized the sw state holding the database enable indication*/
    for (database_iter = 0; database_iter < DNX_DATA_MAX_L3_VRRP_NOF_DBS; database_iter++)
    {
        SHR_IF_ERR_EXIT(algo_l3_db.vrrp_db.database_enable.set(unit, database_iter, FALSE));
    }
    ...
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, info->data_base[database_iter], &capacity));
        if (capacity > 0)
        {
            SHR_IF_ERR_EXIT(algo_l3_db.vrrp_db.database_enable.set(unit, database_iter, TRUE));
            non_zero_capacity_count++;
        }
    }
    if (non_zero_capacity_count)
    {
    ...
    int vlan)
{
    ...
    {
        uint32 entry_handle_id;
        dbal_result_type_t vsi_result_type;
        ...
        rv = dbal_tables_field_info_get(unit, dnx_data_l2.vsi.vsi_table_get(unit), DBAL_FIELD_VRID_BITMAP,
    ...
    uint32 *vrid_bitmap)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_vrrp_vsi_vrid_bitmap_verify(unit, vlan));
    ...
    uint32 vrid_bitmap)
{
    ...
    /** set vrid bitmap */
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, dnx_data_l2.vsi.vsi_table_get(unit), entry_handle_id));
...
dnx_l3_vrrp_members_to_protocol_group_get(
    ...
    uint32 *group_mask)
{
    int protocol_group, nof_bits, array_index;
    uint32 internal_flags[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] =
        { 0 }, existing_members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] =
    {
    0};
    uint32 and_groups, nand_groups, full_mask;
    ...
    *group_mask = DNX_L3_VRRP_PROTOCOL_GROUP_MASK_FULL;
    ...
    for (protocol_group = 0; protocol_group < dnx_data_l3.vrrp.nof_protocol_groups_get(unit); protocol_group++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_protocol_group_members_sw_get
                        (unit, protocol_group, DNX_L3_VRRP_PROTOCOL_GROUP_MASK_FULL, internal_flags));
        /*
        ...
        for (array_index = 0; array_index < DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32; array_index++)
        {
            if (internal_flags[array_index] & members_flags[array_index])
            {
                ...
                else
                {
                    ...
                    and_groups = *group_id & protocol_group;
                    nand_groups = ~(*group_id) & ~protocol_group;
                    *group_mask &= (and_groups | nand_groups);
                }

                /** Accumulate all members of the group in case the user wants to be warned. */
                existing_members[array_index] |= internal_flags[array_index];
            }
        }
    }
    for (array_index = 0; array_index < DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32; array_index++)
    {
        if (exact_match && (existing_members[array_index] != members_flags[array_index]))
        {
...
{
    uint32 protocol_flags[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] = { 0 };
    ...
    dbal_enum_value_field_layer_types_e protocol)
{
    uint32 entry_handle_id;
    int database_iter;
    uint8 database_enable[DNX_DATA_MAX_L3_VRRP_NOF_DBS] = { 0 };
    const dnx_data_l3_vrrp_vrrp_tables_t *vrrp_table;
    dbal_core_mode_e core_mode;
    ...
    vrrp_table = dnx_data_l3.vrrp.vrrp_tables_get(unit, L3_VRRP_TABLE_VRID_MY_MAC_WITH_PROTOCOL);
    /*
     * Go over the databases and update the HW tables
     */
    for (database_iter = 0; database_iter < DNX_DATA_MAX_L3_VRRP_NOF_DBS; database_iter++)
    {
        /**Get database enable indication */
        SHR_IF_ERR_EXIT(algo_l3_db.vrrp_db.database_enable.get(unit, database_iter, &database_enable[database_iter]));

        /** Check if the database is enabled or not.
         * If enabled, update the HW table, otherwise skip*/
        if (database_enable[database_iter] != TRUE)
        {
            continue;
        }
        /** create handle */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, vrrp_table->table[database_iter], &entry_handle_id));

        /** Get the core mode of the table*/
        SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, vrrp_table->table[database_iter], &core_mode));
    ...
    int *index)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        /*
         * Go over the databases and set the HW tables
         */
        for (database_iter = 0; database_iter < DNX_DATA_MAX_L3_VRRP_NOF_DBS; database_iter++)
        {
            /** Get database enable indication */
            SHR_IF_ERR_EXIT(algo_l3_db.vrrp_db.
                            database_enable.get(unit, database_iter, &database_enable[database_iter]));

            /** Check if the database is enabled.
             * In case it is enabled, update the HW tables, otherwise not*/
            if (database_enable[database_iter] != TRUE)
            {
                continue;
            }
            ...
            if (core_mode == DBAL_CORE_MODE_DPC)
            {
                dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, DBAL_RANGE_ALL,
                                                 DBAL_RANGE_ALL);
            }
            if (dnx_data_l3.vrrp.is_mapped_pp_port_used_get(unit))
            {
    ...
    int *index)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        for (database_iter = 0; database_iter < DNX_DATA_MAX_L3_VRRP_NOF_DBS; database_iter++)
        {
            ...
            /** In case the database is enabled update the HW tables, otherwise not*/
            if (database_enable[database_iter] != TRUE)
            {
                continue;
            }
    ...
    uint32 protocol)
{
    ...
    /** In case the table is per MSB, the addtional TCAM index should be removed(used only by the VSI mode)*/
    if (dnx_data_l3.vrrp.is_multiple_my_mac_da_table_per_msb_bit_get(unit))
    {
        uint32 tcam_index_msb;
    ...
    int *index)
{
    ...
    int database_iter;
    uint8 database_enable[DNX_DATA_MAX_L3_VRRP_NOF_DBS] = { 0 };
    const dnx_data_l3_vrrp_vrrp_tables_t *vrrp_table = NULL;
    dbal_core_mode_e core_mode;
    ...
    for (database_iter = 0; database_iter < DNX_DATA_MAX_L3_VRRP_NOF_DBS; database_iter++)
    {
        ...
        if (DNX_L3_VRRP_IS_MAC_ADDRESS_MSB_1(mac_da))
        {
            vrrp_table = dnx_data_l3.vrrp.vrrp_tables_get(unit, L3_VRRP_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_1);
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, vrrp_table->table[database_iter], &entry_handle_id));
        }
...
dnx_l3_vrrp_exem_multi_my_mac_delete(
    ...
    int index)
{
    uint32 entry_handle_id = DBAL_SW_NOF_ENTRY_HANDLES, pp_port;
    ...
    /** clear SW EXEM entry */
    {
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.vrrp_exem_l2_station_id_alloc.res_bitmap.free_single(unit, index, NULL));
    ...
    dbal_enum_value_field_layer_types_e protocol)
{
    ...
    for (database_iter = 0; database_iter < DNX_DATA_MAX_L3_VRRP_NOF_DBS; database_iter++)
    {
        ...
        /** Check if the database is enabled.
         * If enabled update the HW tables, otherwise skip*/
        if (database_enable[database_iter] != TRUE)
        {
            continue;
        }
    ...
    uint32 vrid)
{
    ...
    {
        int db_iter = 0, non_zero_capacity_count = 0, capacity = 0;
        const dnx_data_l3_vrrp_data_bases_t *info;

        info = dnx_data_l3.vrrp.data_bases_get(unit);

        for (db_iter = 0; db_iter < dnx_data_l3.vrrp.nof_dbs_get(unit); db_iter++)
        {
            SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, info->data_base[db_iter], &capacity));
            if (capacity > 0)
            {
                non_zero_capacity_count++;
            }
    ...
    uint32 vrid)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_vrrp_config_add_delete_verify(unit, flags, vlan, vrid));
    ...
    bcm_vlan_t vlan)
{
    ...
    for (database_iter = 0; database_iter < DNX_DATA_MAX_L3_VRRP_NOF_DBS; database_iter++)
    {
        ...
        /** Skip if the database is not enabled*/
        if (database_enable[database_iter] != TRUE)
        {
            continue;
        }
    ...
    bcm_vlan_t vlan)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_vrrp_config_add_delete_verify(unit, flags, vlan, 0));
    ...
    uint32 *count)
{
    ...
    *count = 0;
    ...
    /** iterate over all EXEM_VRID_MY_MAC entries with a given vlan, and retrieve them */
    {
        ...
        vrrp_table = dnx_data_l3.vrrp.vrrp_tables_get(unit, L3_VRRP_TABLE_VRID_MY_MAC_WITH_PROTOCOL);
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, vrrp_table->table[0], &entry_handle_id));
        SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
        ...
        if (core_mode == DBAL_CORE_MODE_DPC)
        {
            /** Add KEY rule for CORE ID to skip entry duplication*/
    ...
    uint32 *count)
{
    ...
    algo_dnx_l3_vrrp_tcam_key_t tcam_data;
    int tcam_index = 0;
    uint32 vrid_bitmap[1], api_flags;
    uint32 members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] = { 0 };
    uint8 is_vrrp;
    ...
    *count = 0;
    ...
    SHR_BIT_ITER(vrid_bitmap, dnx_data_l3.vrrp.nof_vsi_tcam_entries_get(unit), tcam_index)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_is_vrrp_mac(unit, tcam_data.mac_da, &is_vrrp));
        if (is_vrrp == FALSE)
        {
            continue;
        }
    ...
    uint32 *count)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_vrrp_config_get_verify(unit, flags, vlan, alloc_size, vrid_array, count));
...
{
    uint32 internal_flags[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] =
        { 0 }, requested_flags[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] =
    {
    0};
    uint32 group_mask = 0;
    ...
    /** Input validation */
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_vrrp_protocol_group_get_verify(unit, group_members, group_id));
...
{
    uint32 count;
    uint32 curr_group_members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] =
        { 0 }, internal_group_members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] =
    {
    0};
    int protocol_group, array_element;
         ...
         protocol_group < dnx_data_l3.vrrp.nof_protocol_groups_get(unit); protocol_group++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_protocol_group_count_sw_get(unit, protocol_group, &count));

        /** verify that the provided protocols are not already mapped go a group which is already in use */
        for (array_element = 0; array_element < DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32; array_element++)
        {
            if (internal_group_members[array_element] & curr_group_members[array_element])
            {
                if (count > 0)
                {
                    SHR_ERR_EXIT(_SHR_E_EXISTS, "One of the provided protocols is already mapped to a "
                                 ...
                                 "termination or vrrp entries associated with it and try again", protocol_group);
                }
            }
        }
...
dnx_l3_vrrp_protocol_group_set(
    ...
    bcm_l3_protocol_group_id_t group_id)
{
    uint32 internal_group_members[DNX_L3_VRRP_LAYER_PROTOCOLS_SIZE_IN_UINT32] = { 0 };

    SHR_FUNC_INIT_VARS(unit);

    /** verify input parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_vrrp_protocol_group_set_verify(unit, group_members, group_id));
