...

        {
            ...
            if (!soc_is(unit, J2X_DEVICE))
            {
                    ...
                    && direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_IN)
                {
                    ...
                    /*
                     * Get first channel of the port which is of Igress PP type and get its switch hreader type
                     */
                    ...
                    /*
                     * Do checks only if we found another channel of the interface with IN header type.
                     */
    ...
    int arg)
{
    ...
    while (next_field_id != DBAL_FIELD_EMPTY)
    {
        ...
        if (arg & BCM_SWITCH_CONTROL_L3_LEARN_IPV4_UC)
        {
            ...
            {
                if (sal_strnstr(field_name_tokens[2], "_UC", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != NULL)
                {
                    learn_enable = 1;
                }
            }
    ...
    int unit)
{
    ...
    if (dnx_data_switch.svtag.supported_get(unit))
    {
        ...
        if (dnx_data_switch.svtag.svtag_init_valid_get(unit))
        {
