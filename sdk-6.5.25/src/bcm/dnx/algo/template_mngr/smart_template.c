    ...
    uint8 *last_reference)
{
    ...
    if (*last_reference)
    {
        SHR_IF_ERR_EXIT(resource_tag_bitmap_free
                        (unit, module_id, multi_set_template->allocation_bitmap, profile, NULL));
    }
    ...
    uint8 *last_reference)
{
     ...
     */
    old_profile_data = sal_alloc(multi_set_template->hash_table->key_size, "Data buffer old.");
    ...
    SHR_IF_ERR_EXIT(multi_set_get_by_index
                    (unit, module_id, multi_set_template, old_profile, &ref_count_old, old_profile_data));
    ...
    if (ref_count_old != 0)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_smart_template_free
                        (unit, module_id, multi_set_template, old_profile, nof_refs, last_reference));
        /*
    ...
    {
        new_profile_old_data = sal_alloc(multi_set_template->hash_table->key_size, "Old data of new profile");
        ...
        {
            SHR_NULL_CHECK(new_profile_old_data, _SHR_E_PARAM, "new_profile_olgsd_data");

            SHR_IF_ERR_EXIT(multi_set_get_by_index
                            (unit, module_id, multi_set_template, *new_profile, &new_profile_old_ref_count,
        ...
        SHR_IF_ERR_EXIT(dnx_algo_smart_template_allocate
                        (unit, module_id, multi_set_template, flags, nof_refs,
    ...
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_smart_template_allocate
                        (unit, module_id, multi_set_template, flags, nof_refs, (void *) profile_data,
...
exit:
    if (SHR_FUNC_ERR() || !add_success)
    {
        ...
        if (old_profile_freed)
        {
            ...
            rv = dnx_algo_smart_template_allocate(unit, module_id, multi_set_template,
