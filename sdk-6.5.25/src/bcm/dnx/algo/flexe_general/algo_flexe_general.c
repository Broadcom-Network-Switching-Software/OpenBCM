...
dnx_algo_flexe_phy_resource_validate(
    ...
    bcm_port_resource_t * resource)
{
    int i, nof_locations, flexe_core_port;
    ...
    if (resource->base_flexe_instance != -1)
    {
        ...
        for (i = 0; i < nof_locations; i++)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "flexe_core_ports_array is NULL. Illegal.\r\n");
            }
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_flexe_core_port_get(unit, resource->port, &flexe_core_port));
    if (flexe_core_port != resource->base_flexe_instance)
    {
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_flexe_group_info_db_init(unit, i));
    }
    ...
    int *nof_ports)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_core_port_get(unit, logical_ports[i], &flexe_core_ports_arr[i]));
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_logical_phy_id_get(unit, logical_ports[i], &logical_phy_id_arr[i]));
    }
exit:
    SHR_FUNC_EXIT;
}
