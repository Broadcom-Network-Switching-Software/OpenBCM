    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    if (flags & DNX_ALGO_FIELD_KEY_FLAG_WITH_ID)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_FIELD_KEY, key_id_p->id[0], &is_enum_illegal));
        if (is_enum_illegal)
        {
        ...
        if (key_length_type == DNX_FIELD_KEY_LENGTH_TYPE_DOUBLE)
        {
            SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value
                            (unit, DBAL_FIELD_FIELD_KEY, key_id_p->id[1], &is_enum_illegal));
            if (is_enum_illegal)
            {
    ...
    switch (key_length_type)
    {
        case DNX_FIELD_KEY_LENGTH_TYPE_DT:
        {
            ...
            if (fg_type == DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM || fg_type == DNX_FIELD_GROUP_TYPE_TCAM)
            {
                (*nof_keys_p) = 1;
                (*lsb_first_key_p) = 0;
                (*msb_first_key_p) = (*lsb_first_key_p) + dnx_data_field.tcam.dt_max_key_size_get(unit) - 1;
            }
            else if (fg_type == DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB)
            {
                (*nof_keys_p) = 1;
                (*lsb_first_key_p) = dnx_data_field.tcam.key_size_single_get(unit) -
                    dnx_data_field.mdb_dt.key_size_get(unit);
                (*msb_first_key_p) = (*lsb_first_key_p) + dnx_data_field.mdb_dt.key_size_get(unit) - 1;
            }
            else
            {
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    dbal_enum_value_field_field_key_e key_id_iter_no_direction;
    ...
    int allocate_starting_from_first_key = TRUE;
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_field_key_alloc_generic_verify
    ...
    if (nof_available_keys_on_stage == 0)
    {
         ...
         */
        SHR_ERR_EXIT(_SHR_E_PARAM, "Uninitialized stage (%d, %s): No free keys have been assigned\r\n",
                     (unsigned int) field_stage, dnx_field_stage_e_get_name(field_stage));
    }
    /*
    ...
    if (!(dnx_field_key_alloc_instruction.structure_is_valid))
    {
         ...
         * yet been initialized.
         */
        SHR_ERR_EXIT(_SHR_E_PARAM, "Uninitialized field group type (%d, %s) under assigned stage (%d, %s)\r\n",
    ...
    else if (nof_keys_to_allocate == 2)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Starting the second key not from 0 not supported. lsb_second_key %d "
                         "stage %d fg type %d context_id %d key_length_type %d\r\n",
                         lsb_second_key, field_stage, fg_type_effective, context_id, key_length_type);
        }
        key_length_bits_on_first_key = msb_first_key - lsb_first_key + 1;
    ...
    if ((field_stage == DNX_FIELD_STAGE_EPMF) && (fg_type_effective == DNX_FIELD_GROUP_TYPE_TCAM))
    {
        allocate_starting_from_first_key = FALSE;
    }

    for (key_id_iter_no_direction = first_available_key; key_id_iter_no_direction <= last_available_key;
         key_id_iter_no_direction += nof_keys_skip)
    {
        /*
         * Check the order of looking for a key, whether starting from the first or last key.
         */
        if (allocate_starting_from_first_key)
        {
            key_id_iter = key_id_iter_no_direction;
        }
        else
        {
            key_id_iter = last_available_key - (key_id_iter_no_direction - first_available_key);
            key_id_iter -= (key_id_iter - first_available_key) % nof_keys_skip;
        }
        ...
        key_id_p->offset_on_first_key = lsb_first_key;
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_field_key_bit_range_alloc_generic_verify
    ...
    if (force_bit_selection)
    {
        ...
        bit_lsb = key_id_p->offset_on_first_key;

        rv = dnx_algo_field_key_key_occupation_bmp_allocate
            (unit, field_stage, context_id, key_length, TRUE, TRUE, key_id_p->id[0], &bit_lsb);
        if (rv != _SHR_E_NONE)
        {
            SHR_ERR_EXIT(_SHR_E_RESOURCE, "Cannot allocate bits %d-%d in key %d in stage (%d, %s), context %d.\r\n",
                         key_id_p->offset_on_first_key, key_id_p->offset_on_first_key + key_length - 1, requested_key,
    ...
    if (alloc_succeed)
    {
        key_id_p->offset_on_first_key = bit_lsb;
        key_id_p->nof_bits[0] = key_length;
        key_id_p->id[1] = DNX_FIELD_KEY_ID_INVALID;
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_RESOURCE, "Cannot allocate %d bits in stage (%d, %s), context %d.\r\n",
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_FIELD_KEY, key_id_p->id[1], &is_enum_illegal));
    if (is_enum_illegal && (key_id_p->id[1] != DNX_FIELD_KEY_ID_INVALID))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Illegal key[1] identified (%d)\r\n", key_id_p->id[1]);
    }
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    if (key_id_p->offset_on_first_key >= DNX_DATA_MAX_FIELD_TCAM_KEY_SIZE_SINGLE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "\r\n"
                     "Illegal offset within key (%d, larger than max: %d)\r\n",
                     key_id_p->offset_on_first_key, DNX_DATA_MAX_FIELD_TCAM_KEY_SIZE_SINGLE - 1);
    }
    SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_STAGE_INFO.
                    nof_available_keys_on_stage.get(unit, field_stage, &nof_available_keys_on_stage));
    if (nof_available_keys_on_stage == 0)
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_field_key_free_generic_verify(unit, fg_type_effective,
                                                                  field_stage, context_id, key_id_p));
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_field_key_bit_range_free_generic_verify
    ...
    LOG_DEBUG_EX(BSL_LOG_MODULE,
                 ...
                 dnx_field_stage_e_get_name(field_stage), context_id, key_id_p->id[0], key_id_p->offset_on_first_key);

    SHR_IF_ERR_EXIT(dnx_algo_field_key_key_occupation_bmp_deallocate
                    (unit, field_stage, context_id, key_id_p->nof_bits[0], key_id_p->id[0],
 ...
 */
shr_error_e
dnx_algo_field_key_id_first_available_key_get(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_group_type_e fg_type,
    dbal_enum_value_field_field_key_e * first_key_id_p)
{
    ...
    SHR_NULL_CHECK(first_key_id_p, _SHR_E_PARAM, "first_key_id_p");
    ...
    SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.structure_is_valid.get(unit, field_stage, fg_type_effective,
                                                                        &structure_is_valid));
    if (!(structure_is_valid))
    {
        /*
        ...
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Uninitialized field group type (%d, %s) under assigned stage (%d, %s)\r\n",
                     (unsigned int) fg_type_effective, dnx_field_group_type_e_get_name(fg_type_effective),
                     (unsigned int) field_stage, dnx_field_stage_text(unit, field_stage));
    }

    SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.nof_available_keys.get(unit, field_stage, fg_type_effective,
                                                                        &nof_available_keys));
    ...
    if (nof_available_keys <= 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "\r\n"
                     "field group type\"%s\" is not supported on stage \"%s\".\r\n",
                     dnx_field_group_type_e_get_name(fg_type_effective), dnx_field_stage_text(unit, field_stage));
    }

    SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.first_available_key.get(unit, field_stage, fg_type_effective,
                                                                         &first_available_key));

    (*first_key_id_p) = first_available_key;
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_keys_per_stage_allocation_sw.init(unit));
    {
        ...
        num_supported_stages = 0;
        {
             ...
             * these keys to be used for EXEM. This key is indicated in DNX DATA for iPMF1 by 'ipmf1_key'
             */
        ...
        }
        {
            ...
            SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.
                            structure_is_valid.set(unit, stage_index, group_type_index, TRUE));
            /*
             * DIRECT_EXTRACTION is done in loop below.
             */
            /*
             * MDB_DT
             */
        ...
        /**********************************************************************
         * Stage DNX_FIELD_STAGE_EXTERNAL is not allocated through this module.
         **********************************************************************/

        /*
         * Init direct extraction.
         */
        for (stage_index = 0; stage_index < nof_stages; stage_index++)
        {
            int nof_dir_ext_keys = dnx_data_field.stage.stage_info_get(unit, stage_index)->nof_keys_alloc_for_dir_ext;
            int first_dir_ext_key = dnx_data_field.stage.stage_info_get(unit, stage_index)->dir_ext_first_key;
            if (nof_dir_ext_keys > 0)
            {
                int first_dir_ext_key_absolute;
                if (first_dir_ext_key < 0)
                {
                    SHR_ERR_EXIT(_SHR_E_CONFIG, "Stage %d has nof_dir_ext_keys %d but first_dir_ext_key %d.\n",
                                 stage_index, nof_dir_ext_keys, first_dir_ext_key);
                }
                if (stage_index == DNX_FIELD_STAGE_IPMF2)
                {
                    first_dir_ext_key_absolute = first_dir_ext_key +
                ...
                SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.
                                nof_available_keys.set(unit, stage_index, group_type_index, nof_dir_ext_keys));
                SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.
                                first_available_key.set(unit, stage_index, group_type_index,
                                                        first_dir_ext_key_absolute));
                SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.
                                structure_is_valid.set(unit, stage_index, group_type_index, TRUE));
            }
            else
            {
                if (first_dir_ext_key >= 0)
                {
                    SHR_ERR_EXIT(_SHR_E_CONFIG, "Stage %d has first_dir_ext_key %d but nof_dir_ext_keys %d.\n",
                                 stage_index, first_dir_ext_key, nof_dir_ext_keys);
                }
            }
        }
        /*
         * Verify dir_ext_epmf feature fits direct extraction configuration.
         */
        if (dnx_data_field.features.dir_ext_epmf_get(unit))
        {
            if (dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_EPMF)->nof_keys_alloc_for_dir_ext <= 0)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "empf has dir_ext_epmf feature but nof_dir_ext_keys is %d .\n",
                             dnx_data_field.stage.stage_info_get(unit,
                                                                 DNX_FIELD_STAGE_EPMF)->nof_keys_alloc_for_dir_ext);
            }
        }
        else
        {
            if (dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_EPMF)->nof_keys_alloc_for_dir_ext > 0)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "empf doesn't have dir_ext_epmf feature but nof_dir_ext_keys is %d .\n",
                             dnx_data_field.stage.stage_info_get(unit,
                                                                 DNX_FIELD_STAGE_EPMF)->nof_keys_alloc_for_dir_ext);
            }
        }
    ...
    int initial_offset)
{
    ...
    DNX_ALGO_FIELD_KEY_ALLOC_SUB_RESOURCE_INDEX(context_id, key_id, nof_initial_keys, first_initial_key,
                                                sub_resource_id);
    SHR_IF_ERR_EXIT(algo_field_info_sw.
                    ipmf1_initial_key_occ_bmp.free_several(unit, sub_resource_id, size, initial_offset, NULL));
    ...
    uint32 bit_lsb)
{
    ...
    switch (field_stage)
    {
        ...
        {
            SHR_IF_ERR_EXIT(algo_field_info_sw.
                            ipmf1_key_occ_bmp.free_several(unit, sub_resource_id, size, bit_lsb, NULL));
            break;
        }
        case DNX_FIELD_STAGE_IPMF2:
        {
            SHR_IF_ERR_EXIT(algo_field_info_sw.
                            ipmf2_key_occ_bmp.free_several(unit, sub_resource_id, size, bit_lsb, NULL));
            break;
        }
        case DNX_FIELD_STAGE_IPMF3:
        {
            SHR_IF_ERR_EXIT(algo_field_info_sw.
                            ipmf3_key_occ_bmp.free_several(unit, sub_resource_id, size, bit_lsb, NULL));
            break;
        }
        case DNX_FIELD_STAGE_EPMF:
        {
            SHR_IF_ERR_EXIT(algo_field_info_sw.
                            epmf_key_occ_bmp.free_several(unit, sub_resource_id, size, bit_lsb, NULL));
            break;
        }
        default:
        {
    ...
    dbal_enum_value_field_field_key_e key_id)
{
    ...
    DNX_ALGO_FIELD_KEY_ALLOC_SUB_RESOURCE_INDEX(context_id, key_id, nof_keys_alloc, first_key, context_key_combo);
    SHR_IF_ERR_EXIT(algo_field_info_sw.key_lookup_occ_bmp.free_single(unit, field_stage, context_key_combo, NULL));
    ...
    int ffc_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf_1_ffc.free_single(unit, context_id, ffc_id, NULL));
    ...
    int ffc_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf_2_ffc.free_single(unit, context_id, ffc_id, NULL));
    ...
    int ffc_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf_3_ffc.free_single(unit, context_id, ffc_id, NULL));
    ...
    int ffc_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.epmf_ffc.free_single(unit, context_id, ffc_id, NULL));
    ...
    int ffc_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.ifwd2_ffc.free_single(unit, context_id, ffc_id, NULL));
    ...
    dnx_field_qual_id_t user_qual_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.user_qual_id.free_single(unit, user_qual_id, NULL));
    ...
    dnx_field_action_id_t user_action_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.user_action_id.free_single(unit, user_action_id, NULL));
