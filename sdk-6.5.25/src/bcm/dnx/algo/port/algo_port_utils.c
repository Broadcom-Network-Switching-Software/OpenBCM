    ...
    dnx_algo_port_diag_info_t * diag_info)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, diag_info->port_info) ||
        ...
        DNX_ALGO_PORT_TYPE_IS_FLEXE(unit, diag_info->port_info, 1, 0))
    {
    ...
    /** phy id */
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, diag_info->port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT)))
    {
        /** first phy for nif */
    ...
    int *ethu_id_in_core)
{
    ...
    if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_ethu_nof_per_core_in_table))
    {
    ...
    {
        *core = global_ethu_id >= dnx_data_nif.eth.ethu_nof_per_core_get(unit) ? 1 : 0;
        *ethu_id_in_core = global_ethu_id % dnx_data_nif.eth.ethu_nof_per_core_get(unit);
    }
    ...
    int *global_ethu_id)
{
    ...
    {
        *global_ethu_id = dnx_data_nif.eth.ethu_nof_per_core_get(unit) * core + ethu_id_in_core;
    }
    ...
    int *pll_index)
{
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT)))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, logical_port, 0, &first_phy));
    ...
    int *nif_interface_id)
{
    ...
    {
        ilkn_start = nof_eth_if + dnx_data_nif.flexe.nof_clients_get(unit);
    ...
    switch (port_info.port_type)
    {
        ...
        case DNX_ALGO_PORT_TYPE_FLEXE_SAR:
            ...
            *nif_interface_id = dnx_data_nif.eth.ethu_nof_get(unit) + dnx_data_nif.flexe.nof_clients_get(unit) +
...
{
    SHR_FUNC_INIT_VARS(unit);
    *pm_type = portmodDispatchTypeCount;
...
#endif
    if (*pm_type == portmodDispatchTypeCount)
    {
