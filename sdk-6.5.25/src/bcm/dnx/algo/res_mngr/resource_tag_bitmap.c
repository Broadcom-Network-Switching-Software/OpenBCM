...
/*
 * If the ALLOC CHECK ONLY flag is passed, use this macros to return error without printing it.
 */
#define _RESOURCE_TAG_BITMAP_ERROR_EXIT(flags, error, ...)                              \
    do                                                                                  \
    {                                                                                   \
...
#define _RESOURCE_TAG_BITMAP_IF_ERROR_EXIT(flags, rv)                                   \
    do                                                                                  \
    {                                                                                   \
        if(_SHR_IS_FLAG_SET(flags, RESOURCE_TAG_BITMAP_ALLOC_CHECK_ONLY))               \
        {                                                                               \
    ...
    uint32 alloc_flags)
{
    ...
    DNX_SW_STATE_ALLOC(unit, module_id, (*res_tag_bitmap)->name, char,
                       1,
                       0,
                       "(*res_tag_bitmap)->name");
    ...
    DNX_SW_STATE_MEMCPY_BASIC(unit,
                              ...
                              sizeof(res_tag_bitmap_create_info.name),
                              RESOURCE_TAG_BITMAP_NO_FLAGS, "(*res_tag_bitmap)->name");
 ...
 */
static shr_error_e
_resource_tag_bitmap_compare_tags(
    int unit,
    uint32 module_id,
    resource_tag_bitmap_t res_tag_bitmap,
    int tag_index,
    ...
    if (tag_size > 0)
    {
        ...
        while (current_elem < elemIndex + elemCount)
        {
            ...
            if (grain_tag_valid || always_check)
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(_resource_tag_bitmap_compare_tags
    ...
    int *elem)
{
    ...
    {
        _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info.flags, _SHR_E_PARAM, "Pattern length must be at least 1. \n");
    }

    if (32 < alloc_info.length)
    {
        _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info.flags, _SHR_E_PARAM, "Pattern length must be at most 32. \n");
    }

    if (0 >= alloc_info.repeats)
    {
        _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info.flags, _SHR_E_PARAM, "Pattern repeats must be at least 1. \n");
    }

    pattern_mask = (((uint32) 1 << (alloc_info.length - 1)) - 1) | ((uint32) 1 << (alloc_info.length - 1));
    if (0 == (alloc_info.pattern & pattern_mask))
    {
        _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info.flags, _SHR_E_PARAM,
                                        "pattern must contain at least one element. \n");
    }

    if (alloc_info.pattern & ~pattern_mask)
    {
        _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info.flags, _SHR_E_PARAM, "pattern must not contain unused bits. \n");
    }
    ...
    if (_SHR_IS_FLAG_SET(alloc_info.flags, RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE))
    {
        ...
        if (alloc_info.range_end > (res_tag_bitmap->low + res_tag_bitmap->count))
        {
            _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info.flags, _SHR_E_PARAM,
                                            "End ID for allocation in range %d cannot be larger than the first invalid element of the resource %d.\n",
                                            alloc_info.range_end, res_tag_bitmap->low + res_tag_bitmap->count);
        }
    }
    ...
    if (alloc_info.flags & RESOURCE_TAG_BITMAP_ALLOC_WITH_ID)
    {
        ...
        if (pattern_first_index + allocated_block_length > res_tag_bitmap->count)
        {
            _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info.flags, _SHR_E_PARAM,
                                            ...
                                            res_tag_bitmap->low + res_tag_bitmap->count - 1);
        }
        ...
        if (wrong_offset)
        {
            _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info.flags, _SHR_E_PARAM, "provided first element %d does not conform"
    ...
    int *element)
{
    ...
    _RESOURCE_TAG_BITMAP_IF_ERROR_EXIT(flags, rv);
    ...
    int *elem)
{
    ...
    {
        _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info->flags, _SHR_E_RESOURCE,
     ...
     */
    _RESOURCE_TAG_BITMAP_IF_ERROR_EXIT(alloc_info->flags,
                                       resource_tag_bitmap_alloc_verify(unit, res_tag_bitmap, *alloc_info, elem));
    ...
    if (alloc_info->flags & RESOURCE_TAG_BITMAP_ALLOC_WITH_ID)
    {
        ...
        if (!(alloc_info->flags & RESOURCE_TAG_BITMAP_ALLOC_IGNORE_TAG))
        {
            ...
            if (tag_compare_result)
            {
                 ...
                 */
                tag_index = resource_tag_bitmap_get_grain_tag_index(res_tag_bitmap, pattern_first_index);
                DNX_SW_STATE_BIT_RANGE_READ(unit, module_id, 0, res_tag_bitmap->tagData,
                                            (tag_index * (res_tag_bitmap->tagSize)), 0, res_tag_bitmap->tagSize,
                                            &exist_tag);
                /*
                ...
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             ...
                             exist_tag, allocated_block_length, *elem, alloc_info->tag);
            }
        }
        /*
    ...
    {   /* if (flags & SHR_RES_ALLOC_WITH_ID) */
        ...
        if (!(res_tag_bitmap->flags & RESOURCE_TAG_BITMAP_CREATE_PREFER_LOW_IDS))
        {
            ...
            {
                pattern_first_index =
                    UTILEX_ALIGN_UP(res_tag_bitmap->nextAlloc[tag_for_pointers],
            ...
            if (!allocation_success && !(alloc_info->flags & RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE))
            {
                ...
                pattern_first_index =
                    ...
                    alloc_info->offs[0];
                SHR_IF_ERR_EXIT(sw_state_resource_allocate_next_free_element_in_range
                                (unit, module_id, res_tag_bitmap, *alloc_info, pattern_first_index,
                ...
                if (!allocation_success)
                {
                    ...
                    SHR_IF_ERR_EXIT(sw_state_resource_allocate_next_free_element_in_range
                                    (unit, module_id, res_tag_bitmap, *alloc_info, pattern_first_index, nextAlloc,
    ...
    {
        /*
         * if here, allocation can be successful but was run in simulation mode (RESOURCE_TAG_BITMAP_ALLOC_CHECK_ONLY flag is on)
    ...
    else
    {
         ...
         */
        _RESOURCE_TAG_BITMAP_ERROR_EXIT(alloc_info->flags, _SHR_E_RESOURCE,
                                        "No free element matching required conditions. Asked for %d elements aligned to %d with offs %d \n",
