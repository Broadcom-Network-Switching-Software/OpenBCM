    ...
    dnx_algo_res_create_data_t * data)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_verify(unit, algo_res));
    ...
    uint32 alloc_flags)
{
    ...
    sal_strncpy(data->advanced_algo_name, Resource_callbacks_map[data->advanced_algorithm].algo_name,
                DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    /*
     * 0. Verify input.
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_create_verify(unit, data));
    ...
    void *extra_arguments)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_destroy_verify(unit, module_id, (*algo_res)));
    ...
    int *element)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_allocate_verify(unit, algo_res, flags, element, 1));
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        ...
        {
            LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META(", allocated element:%d"), *element));
        }
    ...
    uint8 *is_allocated)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_get_delete_verify(unit, algo_res, 1, element));
    ...
    uint8 *is_allocated)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_get_verify(unit, algo_res, element, is_allocated));
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        ...
        if (SHR_GET_CURRENT_ERR() != _SHR_E_NONE)
        {
            LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META(", is_allocated:%d"), *is_allocated));
        }
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META(") Result:%d\n"), SHR_GET_CURRENT_ERR()));
    }
    DNX_INIT_TIME_ANALYZER_RES_MNGR_STOP(unit, DNX_INIT_TIME_ANALYZER_RES_MNGR);
    ...
    void *extra_arguments)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_res_free_several(unit, module_id, algo_res, 1, element, extra_arguments));
    ...
    int *nof_free_elements)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_nof_free_elements_get_verify(unit, algo_res, nof_free_elements));
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        ...
        if (SHR_GET_CURRENT_ERR() != _SHR_E_NONE)
        {
            LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META(", nof_free_elements:%d"), *nof_free_elements));
        }
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META(") Result:%d\n"), SHR_GET_CURRENT_ERR()));
    }
    SHR_FUNC_EXIT;
    ...
    dnx_algo_res_t algo_res)
{
    ...
    else
    {
        ...
        for (; current_element < algo_res->nof_elements; current_element++)
        {
            ...
            if (is_allocated == 1)
            {
                SHR_IF_ERR_EXIT(Resource_callbacks_map[algo_res->advanced_algorithm].free_cb(unit,
                                                                                             module_id,
    ...
    int *element)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_allocate_verify(unit, algo_res, flags, element, nof_elements));
    ...
    if ((_SHR_IS_FLAG_SET(flags, DNX_ALGO_RES_ALLOCATE_SIMULATION)) && (rv != _SHR_E_NONE))
    {
    ...
    void *extra_arguments)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_get_delete_verify(unit, algo_res, nof_elements, element));
    ...
    dnx_algo_res_dump_data_t * info)
{
    ...
    sal_memcpy(info->create_data.name, algo_res->name, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    dnx_algo_res_t * algo_res)
{
    ...
    /*
     * Call the print function of advanced algorithm if exist.
     */

    if (Resource_callbacks_map[data.create_data.advanced_algorithm].print_cb != NULL)
    {
        SHR_IF_ERR_EXIT(Resource_callbacks_map[data.create_data.advanced_algorithm].print_cb
                        (unit, module_id, &((*algo_res)->res_tag_bitmap), &data));
    }
    ...
    int *nof_allocated_elements)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_res_nof_allocated_elements_in_range_get_verify
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        ...
        if (SHR_GET_CURRENT_ERR() != _SHR_E_NONE)
        {
            LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META(", nof_elements:%d"), *nof_allocated_elements));
        }
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META(") Result:%d\n"), SHR_GET_CURRENT_ERR()));
    }
    SHR_FUNC_EXIT;
