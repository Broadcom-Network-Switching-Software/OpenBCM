 ...
 * {
 */
#define DNX_FLOW_EGRESS_VIRTUAL_GPORT_ID_RESOURCE_NAME  "FLOW EGRESS VIRTUAL GPORT ID"
#define DNX_FLOW_INGRESS_VIRTUAL_GPORT_ID_RESOURCE_NAME "FLOW INGRESS VIRTUAL GPORT ID"
    ...
    int unit)
{
    ...
    for (dbal_table_id = 1; dbal_table_id < DBAL_NOF_TABLES; dbal_table_id++)
    {
        ...
        if (dbal_table_is_in_lif(table) || dbal_table_is_out_lif(table) || dbal_table_is_esem(table) ||
            (dbal_table_id == DBAL_TABLE_ESEM_DEFAULT_RESULT_TABLE))
        {
            lif_table_mngr_lif_table_result_type_info_t result_types_arr[MAX_NOF_RESULT_TYPES];

#ifdef BCM_DNX2_SUPPORT
            /** For legacy LIF-Table-Mngr support only selected ESEM tables */
            if (dnx_data_lif.
                lif_table_manager.feature_get(unit, dnx_data_lif_lif_table_manager_use_per_field_attributes))
            {
                if (dbal_table_is_esem(table) && (dbal_table_id != DBAL_TABLE_ESEM_FORWARD_DOMAIN_MAPPING_DB))
                {
                    continue;
                }
            }
            ...
            /*
             * Verify that the defined maximum number of result types isn't reached
             */
            if (table->nof_result_types >= MAX_NOF_RESULT_TYPES)
            {
                SHR_ERR_EXIT(_SHR_E_INIT,
                             "Error: Num of result types in table %s exceeds the defined max of %d",
                             dbal_logical_table_to_string(unit, dbal_table_id), MAX_NOF_RESULT_TYPES);
            }
            ...
            for (dbal_result_type = 0; dbal_result_type < table->nof_result_types; dbal_result_type++)
            {
                 ...
                 */
                max_nof_fields = UTILEX_MAX(max_nof_fields, table->multi_res_info[dbal_result_type].nof_result_fields);
            }
            /*
             * Sort the list
             */
            utilex_os_qsort(result_types_arr, table->nof_result_types,
                            sizeof(lif_table_mngr_lif_table_result_type_info_t), lif_table_mngr_result_types_compre);
            ...
            for (dbal_result_type = 0; dbal_result_type < table->nof_result_types; dbal_result_type++)
            {
                sw_state_table_info.result_type[dbal_result_type] = result_types_arr[dbal_result_type].result_type_id;
            }
...
dnx_algo_lif_table_mngr_init(
    int unit)
{
    int max_nof_out_lifs, nof_sbc_lifs, nof_dpc_lifs, max_nof_lif_tables;
    sw_state_htbl_init_info_t egress_outlif_init_info, egress_virtual_gport_init_info, dbal_id_info;
    dnx_algo_res_create_data_t ing_virtual_res_data, egr_virtual_res_data;
    uint8 is_init;
    int esem_table_capacity = 0;
    ...
    if (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit))
    {
        ...
        ing_virtual_res_data.nof_elements = nof_sbc_lifs + nof_dpc_lifs;
        ing_virtual_res_data.flags = 0;
        sal_strncpy(ing_virtual_res_data.name, DNX_FLOW_INGRESS_VIRTUAL_GPORT_ID_RESOURCE_NAME,
                    DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
        SHR_IF_ERR_EXIT(lif_table_mngr_db.flow_ingress_virtual_gport_id.create(unit, &ing_virtual_res_data, NULL));
    }

    /*
     * Create a resource allocation pool for egress virtual gport IDs 
     */
#ifdef BCM_DNX2_SUPPORT
    SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_ESEM, &esem_table_capacity));
#endif
    ...
    /** if (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit)) */
    {
        sal_memset(&egr_virtual_res_data, 0, sizeof(egr_virtual_res_data));
        egr_virtual_res_data.first_element = 1;
        egr_virtual_res_data.nof_elements = esem_table_capacity;
        egr_virtual_res_data.flags = 0;
        sal_strncpy(egr_virtual_res_data.name, DNX_FLOW_EGRESS_VIRTUAL_GPORT_ID_RESOURCE_NAME,
                    DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
        SHR_IF_ERR_EXIT(lif_table_mngr_db.flow_egress_virtual_gport_id.create(unit, &egr_virtual_res_data, NULL));
    }
    ...
    /*
     * Create a SW State for valid fields per egress virtual gports: ESEM & ESEM-Default
     */
    egress_virtual_gport_init_info.max_nof_elements =
        esem_table_capacity + dnx_data_esem.default_result_profile.nof_allocable_profiles_get(unit);
    egress_virtual_gport_init_info.expected_nof_elements =
        esem_table_capacity + dnx_data_esem.default_result_profile.nof_allocable_profiles_get(unit);
    egress_virtual_gport_init_info.hash_function = NULL;
    egress_virtual_gport_init_info.print_cb_name = "dnx_algo_lif_table_mngr_valid_fields_htb_entry_print_cb";
    SHR_IF_ERR_EXIT(lif_table_mngr_db.
                    valid_fields_per_no_local_lif_egress_htb.create(unit, &egress_virtual_gport_init_info));

    /*
     * Allocate a SW State for valid fields per local In-LIF
     */
    ...
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(lif_table_mngr_db.valid_fields_per_ingress_lif1.alloc(unit, nof_sbc_lifs));

        /*
         * Allocate and initiate a SW State for valid fields per local In-LIF in Physical tables INLIF-2/3
         */
        SHR_IF_ERR_EXIT(lif_table_mngr_db.valid_fields_per_ingress_lif2.alloc(unit, nof_dpc_lifs));
#endif
    }
    ...
    uint32 *table_specific_flags)
{
    ...
    else if (result_type == DBAL_RESULT_TYPE_EEDB_IPV4_TUNNEL_ETPS_IPV4_TUNNEL_COPY_INNER)
    {
        *table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_COPY_TUNNEL;
    }
    else if (result_type == DBAL_RESULT_TYPE_EEDB_IPV4_TUNNEL_ETPS_IPV4_TUNNEL_DOUBLE_UDP_VXLAN)
    {
        *table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_DOUBLE_UDP;
    }
    else
    {
 ...
 *   \param [in] unit - unit id
 *   \param [in] result_type - Result type of the current lif
 *   \param [in] logical_phase - Out-LIF logical phase of
 ...
 */
static shr_error_e
dnx_lif_table_mngr_table_specific_flags_fill_egress_eedb_ac(
    int unit,
    int result_type,
    uint32 *table_specific_flags)
{
    ...
    /** Allowed result types */
    if (result_type == DBAL_RESULT_TYPE_EEDB_OUT_AC_ETPS_AC_STAT_PROTECTION_NON_EM_BUSTER)
    {
        *table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_AC_EGRESS_NON_ESEM_BUSTER;
    }
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_AC_INGRESS_NON_NATIVE_P2P))
    {
         ...
         *  - DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC2EEI_WITH_PROTECTION
         *  - DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_P2P_W_1_VLAN
         *  - DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_P2P_W_VSI_W_1_VLAN
         *  - DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_P2P_W_VSI_WO_PROTECTION
         *  - DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_P2P_LARGE
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    else if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_COPY_TUNNEL))
    {
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_EEDB_IPV4_TUNNEL_ETPS_IPV4_TUNNEL_COPY_INNER);
    }
    ...
    else if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_DOUBLE_UDP))
    {
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_EEDB_IPV4_TUNNEL_ETPS_IPV4_TUNNEL_DOUBLE_UDP_VXLAN);
    }
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    /** Allowed result types */
    if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_IPV6_SCALE))
    {
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_EEDB_IPV6_TUNNEL_ETPS_IPV6_TUNNEL_SCALE);
    }
 ...
 */
static shr_error_e
dnx_lif_table_mngr_table_specific_result_type_filtering_egress_eedb_ac(
    int unit,
    uint32 table_specific_flags,
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    /** Allowed result types */
    if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_AC_EGRESS_NON_ESEM_BUSTER))
    {
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_EEDB_OUT_AC_ETPS_AC_STAT_PROTECTION_NON_EM_BUSTER);
    }
    else if (table_specific_flags == 0)
    {
        /** For EEDB_OUT_AC, no specific flags. All result types are allowed  */
        int nof_res_types;
        SHR_IF_ERR_EXIT(dbal_tables_table_nof_res_type_get(unit, DBAL_TABLE_EEDB_OUT_AC, &nof_res_types));
        SHR_BITSET_RANGE(&(table_specific_rules->allowed_result_types_bmp), 0, nof_res_types);
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    /*
     * Check if 2 labels were used 
     */
    if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_MPLS_2_LABELS))
    {
        SHR_BITCLR(&(table_specific_rules->allowed_result_types_bmp), DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_1);
        SHR_BITCLR(&(table_specific_rules->allowed_result_types_bmp),
...
/**
 * \brief - Returns table specific information that is required in order to
 *          filter result types for the table. 
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] entry_handle_id - DBAL entry handle id
 *   \param [in] table_specific_flags - User requested flag for the table
 *   \param [in] dbal_table_id - DBAL table ID
 *   \param [out] table_specific_rules - table specific result type info
 * \par DIRECT OUTPUT:
 *   shr_error_e
 * \see
 *
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    SHR_BITCLR_RANGE(&(table_specific_rules->allowed_result_types_bmp), 0, MAX_NOF_RESULT_TYPES);
    ...
    else if (dbal_table_id == DBAL_TABLE_EEDB_OUT_AC)
    {
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_table_specific_result_type_filtering_egress_eedb_ac
                        (unit, table_specific_flags, table_specific_rules));
    }
...
#ifdef BCM_DNX2_SUPPORT
    if (dnx_data_lif.lif_table_manager.feature_get(unit, dnx_data_lif_lif_table_manager_use_per_field_attributes))
    {
        lif_table_mngr_table_info_t sw_state_table_info;
        uint8 found;
    ...
    uint32 *table_specific_flags)
{
    ...
    else if (dbal_table_id == DBAL_TABLE_EEDB_IPV6_TUNNEL)
    {
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_table_specific_flags_fill_egress_ipv6_tunnel
                        (unit, result_type, logical_phase, table_specific_flags));
    }
    else if (dbal_table_id == DBAL_TABLE_EEDB_OUT_AC)
    {
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_table_specific_flags_fill_egress_eedb_ac
                        (unit, result_type, table_specific_flags));
    }
    else if (dbal_table_id == DBAL_TABLE_IN_AC_INFO_DB)
    {
         ...
         field_idx_to_ignore < table->multi_res_info[table->nof_result_types].nof_result_fields; field_idx_to_ignore++)
    {
        ...
        if (rv == _SHR_E_NONE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_lif_table_mngr_decide_result_type_internal
                            (unit, table_specific_flags, logical_phase, test_entry_handle_id, &result_type_decision,
                             TRUE));
    ...
    uint8 is_test)
{
     ...
     */
    SHR_IF_ERR_EXIT(lif_table_mngr_db.lif_table_info_htb.find(unit, (int *) &dbal_table_id,
                                                              &sw_state_table_info, &found));
    ...
    if (valid_result_type == FALSE)
    {
        ...
        if (is_test == FALSE)
        {
            SHR_IF_ERR_EXIT(dnx_algo_lif_table_mngr_no_result_type_found_debug
                            (unit, dbal_table_id, table_specific_flags, logical_phase, entry_handle_id));
    ...
    uint32 *result_type_decision)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_lif_table_mngr_decide_result_type_internal
                    (unit, table_specific_flags, logical_phase, entry_handle_id, result_type_decision, FALSE));
