    ...
    int *trap_id_p)
{
    ...
    if ((type == bcmRxTrapEgTxUserDefine) && div_to_erpp_and_etpp
        && (*trap_id_p > DNX_RX_TRAP_ETPP_FWD_ACT_PROFILE_USER_TRAP_4))
    {
    ...
    int unit)
{
    ...
    /** ETPP Trap resource manager  */
    {
        ...
        /** Number of etpp user traps: user defined profiles + visibility trap (if supported) */
    ...
    if (dnx_data_trap.etpp.feature_get(unit, dnx_data_trap_etpp_oam_traps_support))
    {
        /** First trap id in trap res mngr */
        data.first_element = 0;
