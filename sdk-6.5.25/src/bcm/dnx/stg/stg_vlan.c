    ...
    {
        {
            ...
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit,
                                                                get_entry_handle_id, DBAL_FIELD_RESULT_TYPE,
            ...
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI, vsi);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STP_TOPOLOGY_ID, INST_SINGLE,
                                         stp_topology_id);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }
    }
    ...
    bcm_vlan_t vid)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stg_vlan_add_verify(unit, stg, vid));
    ...
    bcm_vlan_t vid)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stg_vlan_remove_verify(unit, stg, vid));
    ...
    bcm_stg_t stg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stg_vlan_remove_all_verify(unit, stg));
    ...
    bcm_stg_t stg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_stg_set_verify(unit, stg, vid));
    ...
    bcm_stg_t * stg_ptr)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_stg_get_verify(unit, vid, stg_ptr));
    ...
    int stp_state)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_stp_set_verify(unit, vid, port, stp_state));
    ...
    int *stp_state)
{
     ...
     * Defined APIs were called, so no needes to verify the inputs specially.
     */
    /** SHR_INVOKE_VERIFY_DNXC(dnx_vlan_stp_get_verify(unit, vsi, port, stp_state));*/
