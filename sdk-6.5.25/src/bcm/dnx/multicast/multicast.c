    ...
    int *rep_count)
{
     ...
     */
    if (flags &
        ~(BCM_MULTICAST_WITH_ID | BCM_MULTICAST_REMOVE_ALL | BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_EGRESS_GROUP |
    ...
    if (is_sar)
    {
        ...
        if (api_indication != DNX_MULTICAST_API_GET)
        {
            ...
            local_port =
                (BCM_GPORT_IS_LOCAL(rep_array[0].port)) ? BCM_GPORT_LOCAL_GET(rep_array[0].port) : rep_array[0].port;
        ...
        if ((api_indication == DNX_MULTICAST_API_ADD) || (api_indication == DNX_MULTICAST_API_DELETE))
        {
            ...
            if ((out_rep_count > 0) && (out_rep_array.port != rep_array->port) && (!(flags & BCM_MULTICAST_REMOVE_ALL)))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    bcm_multicast_t group)
{
    ...
    if (DNX_MULTICAST_IS_INGRESS(flags))
    {
        ...
        if (is_open)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry(unit, _BCM_MULTICAST_ID_GET(group),
                                                      DNX_MULTICAST_IS_BITMAP(flags), TRUE, core_id, &entry_id));
     ...
     */
    if (DNX_MULTICAST_IS_EGRESS(flags) && (dnx_data_multicast.params.max_egr_mc_groups_get(unit) >= 0))
    {
        ...
        if (is_open)
        {
            ...
            if (!DNX_MULTICAST_SAR_SKIP_MCDB(unit, is_sar_allocated))
            {
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_multicast_id_to_entry(unit, _BCM_MULTICAST_ID_GET(group),
                                                              DNX_MULTICAST_IS_BITMAP(flags), FALSE, core_id,
                                                              &entry_id));
                    /** for bitmap cases - need to release first bitmap entry */
                    if (DNX_MULTICAST_IS_BITMAP(flags))
                    {
