    ...
    int init_stage)
{
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EXPANSION_IS_GLBL_POINTER, INST_SINGLE, FALSE);
    }
    ...
    int enable)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, curr_core)
    {
        SHR_IF_ERR_EXIT(dnx_meter_db.generic_meter_info.db_info.is_single_bucket.get
                        (unit, curr_core, is_ingress, database_id, &is_single_bucket));
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EBL_RANDOM_MODE_ENABLE, INST_SINGLE,
                                     is_single_bucket ? FALSE : enable);
