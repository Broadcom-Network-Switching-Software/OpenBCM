    ...
    dnx_field_range_info_t * range_info_p)
{
    ...
    sal_memset(range_info_p, 0x0, sizeof(*range_info_p));
    /**
 ...
 */
shr_error_e
dnx_field_range_type_qual_info_t_init(
    int unit,
    dnx_field_range_type_qual_info_t * range_type_qual_info_p)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(range_type_qual_info_p, _SHR_E_PARAM, "range_type_qual_info_p");

    sal_memset(range_type_qual_info_p, 0x0, sizeof(*range_type_qual_info_p));

    range_type_qual_info_p->dnx_qual = DNX_FIELD_QUAL_TYPE_INVALID;
    SHR_IF_ERR_EXIT(dnx_field_qual_attach_info_t_init(unit, &(range_type_qual_info_p->qual_info)));
    ...
    switch (range_info_p->range_type)
    {
        ...
        case DNX_FIELD_RANGE_TYPE_L4_DST_PORT:
            ...
            if (!dnx_data_field.features.extended_l4_ops_get(unit) || field_stage == DNX_FIELD_STAGE_EPMF)
            {
                ...
                if (!DNX_FIELD_RANGE_L4_RANGE_ID_VALID(unit, field_stage, range_id))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 " The RANGE_ID: %d is not within range for L4 Ports [0-%d]! \n",
                                 range_id, dnx_data_field.stage.stage_info_get(unit,
                                                                               field_stage)->nof_l4_ops_ranges_legacy -
                                 1);
                }
            }
            /**
            ...
            if (is_for_set == DNX_FIELD_RANGE_IS_FOR_SET)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "The range for L4 Ops Range qualifier must be [%d-%d]. Range given is [%d - %d] \n",
                                 DNX_FIELD_RANGE_QUALS_MIN_VAL, DNX_FIELD_RANGE_L4_OPS_MAX_VAL,
        ...
        case DNX_FIELD_RANGE_TYPE_OUT_LIF:
            ...
            if (field_stage == DNX_FIELD_STAGE_IPMF1 || field_stage == DNX_FIELD_STAGE_IPMF2)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "The iPMF1 out LIF RANGE_ID %d should be within [0-%d].\n",
                                 range_id, dnx_data_field.stage.stage_info_get(unit,
                                                                               DNX_FIELD_STAGE_IPMF1)->nof_out_lif_ranges
                                 - 1);
                }
                if (is_for_set == DNX_FIELD_RANGE_IS_FOR_SET)
                {
                    ...
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "The range for OUT LIFF Range qualifier must be within [%d-%d]. Range given is [%d - %d]\n",
                                     DNX_FIELD_RANGE_QUALS_MIN_VAL, DNX_FIELD_RANGE_OUT_LIF_MAX_VAL,
                                     range_info_p->min_val, range_info_p->max_val);
                    }
                }
            }
            else if (field_stage == DNX_FIELD_STAGE_IPMF3)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "The iPMF3 out LIF RANGE_ID %d should be within [%d-%d] \n",
                                 range_id, 0, dnx_data_field.stage.stage_info_get(unit,
                                                                                  DNX_FIELD_STAGE_IPMF3)->nof_out_lif_ranges
                                 - 1);
                }
                if (is_for_set == DNX_FIELD_RANGE_IS_FOR_SET)
                {
                    ...
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "The range for OUT LIF Range qualifier must be within [%d-%d]. Range given is [%d - %d]\n",
                                     DNX_FIELD_RANGE_QUALS_MIN_VAL, DNX_FIELD_RANGE_OUT_LIF_MAX_VAL,
                                     range_info_p->min_val, range_info_p->max_val);
                    }
                }
        ...
        case DNX_FIELD_RANGE_TYPE_PKT_HDR_SIZE:
            ...
            if (field_stage == DNX_FIELD_STAGE_IPMF1 || field_stage == DNX_FIELD_STAGE_IPMF2)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "The iPMF1 header packet RANGE_ID %d should be within [0-%d].\n",
                                 range_id, dnx_data_field.stage.stage_info_get(unit,
                                                                               DNX_FIELD_STAGE_IPMF1)->nof_pkt_hdr_ranges
                                 - 1);
                }
                if (is_for_set == DNX_FIELD_RANGE_IS_FOR_SET)
                {
                    ...
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "The range for PKT HDR Range qualifier must be [%d-%d]. Range given is [%d - %d]\n",
                                     DNX_FIELD_RANGE_QUALS_MIN_VAL, DNX_FIELD_RANGE_PKT_HDR_MAX_VAL,
                                     range_info_p->min_val, range_info_p->max_val);
                    }
                }
            }
        ...
        case DNX_FIELD_RANGE_TYPE_FFC2_HIGH:
            ...
            if (is_for_set == DNX_FIELD_RANGE_IS_FOR_SET)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "The range for L4 Ops Range qualifier %s must be within [%d-%d]. Range given is [%d - %d] \n",
                                 dnx_field_range_type_text(range_info_p->range_type),
                                 DNX_FIELD_RANGE_QUALS_MIN_VAL, DNX_FIELD_RANGE_L4_OPS_MAX_VAL,
                                 range_info_p->min_val, range_info_p->max_val);
                }
    ...
    dnx_field_range_info_t * range_info_p)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_field_range_verify
