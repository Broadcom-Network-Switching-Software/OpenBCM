    ...
    bcm_field_group_t * fg_id)
{
    ...
    if (fg_info->external_info.entries_per_core != FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Per core entries for external TCAM not supported.\n");
    }
    ...
    bcm_field_entry_t * entry_handle)
{
    ...
    if (fg_type != DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM && fg_type != DNX_FIELD_GROUP_TYPE_TCAM &&
        fg_type != DNX_FIELD_GROUP_TYPE_STATE_TABLE && entry_info->core != DBAL_CORE_ALL)
    {
    ...
    if (fg_type != DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM)
    {
        for (qual_iter = 0; qual_iter < entry_info->nof_entry_quals; qual_iter++)
        {
            for (i_data = 0; i_data < BCM_FIELD_QUAL_WIDTH_IN_WORDS; i_data++)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Max value selection is available only for external TCAM. "
                                 "Qualifier number %d.\n", qual_iter);
                }
            }
        }
    ...
    bcm_field_entry_info_t * entry_info)
{
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM:
        {
            SHR_IF_ERR_EXIT(dnx_field_entry_kbp_get(unit, dnx_fg_id, dnx_entry_handle, NULL, &dnx_entry_info));
        }
            break;
        case DNX_FIELD_GROUP_TYPE_EXEM:
        {
 ...
 * \param [in] unit            - Device ID
 * \param [in] flags           - TBD
 * \param [in] add_info        - Add information
 *
 * \return
 ...
 */
static shr_error_e
bcm_dnx_field_tcam_bank_add_verify(
    int unit,
    uint32 flags,
    bcm_field_tcam_bank_info_t * add_info)
{
    ...
    if (add_info->nof_banks <= 0 || add_info->nof_banks > dnx_data_field.tcam.nof_banks_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Number of banks to add/evacuate needs to be between 1-%d, value given: %d\n",
                     dnx_data_field.tcam.nof_banks_get(unit), add_info->nof_banks);
    }

    for (bank_idx = 0; bank_idx < add_info->nof_banks; bank_idx++)
    {
        if (add_info->tcam_bank_ids[bank_idx] < 0
            || add_info->tcam_bank_ids[bank_idx] >= dnx_data_field.tcam.nof_banks_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Given Bank ID %d at index %d during bank add/evacuate call is out of range\n",
                         add_info->tcam_bank_ids[bank_idx], bank_idx);
        }

        /*
         * Verify core
         */
        DNXCMN_CORE_VALIDATE(unit, add_info->core_ids[bank_idx], TRUE);

        if (add_info->pp_app != bcmFieldAppDbInvalid && add_info->core_ids[bank_idx] != BCM_CORE_ALL)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Core ID %d at index %d can specified only for field group, not for APP-DB\n",
                         add_info->core_ids[bank_idx], bank_idx);
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_field_tcam_bank_info_t * evac_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(bcm_dnx_field_tcam_bank_add_verify(unit, flags, evac_info));
    ...
    bcm_field_tcam_bank_info_t * add_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(bcm_dnx_field_tcam_bank_add_verify(unit, flags, add_info));
    ...
    bcm_field_tcam_bank_status_info_t * bank_info)
{
    ...
    for (pp_app = bcmFieldAppDbFirst; pp_app < bcmFieldAppDbCount; pp_app++)
    {
         ...
         * returned TCAM handler ID to the given banks.
         */
        SHR_IF_ERR_EXIT(dnx_field_map_pp_app_dbal_table_get(unit, pp_app, &dbal_table));
        SHR_IF_ERR_EXIT(dbal_tables_tcam_handler_id_get(unit, dbal_table, &tcam_handler_id));
