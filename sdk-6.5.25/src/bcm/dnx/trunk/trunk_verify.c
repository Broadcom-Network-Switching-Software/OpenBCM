...
/**
 * \brief - verify function to check if a the psc_info is valid
 *
 * \param [in] unit - unit number
    ...
    bcm_trunk_id_info_t * id_info)
{
    ...
    max_nof_members_in_profile =
        (max_group_is_support == TRUE) ? psc_info.max_nof_members_in_profile : psc_info.max_nof_members_in_profile + 1;
    ...
    bcm_trunk_id_info_t * id_info)
{
    ...
    switch (psc)
    {
        ...
        case BCM_TRUNK_PSC_C_LAG:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_trunk_psc_info_verify(unit, psc_info, id_info));
            break;
        }
        default:
        {
    ...
    uint32 *flags)
{
    ...
    if (*flags & ~(BCM_TRUNK_MEMBER_INGRESS_DISABLE | BCM_TRUNK_MEMBER_EGRESS_DISABLE | BCM_TRUNK_MEMBER_SPA_WITH_ID))
    {
