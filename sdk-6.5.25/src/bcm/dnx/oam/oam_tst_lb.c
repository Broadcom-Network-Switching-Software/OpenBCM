...
_dnx_ins_pph_for_tst_lb(
    ...
    const dnx_PPH_J2_base_head * pph_j2_fields,
    int byte_start_offset,
    int is_jericho_mode)
{
    int offset = byte_start_offset;
    if (!is_jericho_mode)
    {
        ...
        buffer[offset++] = (pph_j2_fields->field2 & 0xC) >> 2;
        buffer[offset++] = ((pph_j2_fields->field2 & 0x3) << 6) | ((pph_j2_fields->field3 & 0xe0) >> 5);
        buffer[offset++] = ((pph_j2_fields->field3 & 0x1f) << 3) | ((pph_j2_fields->field5 & 0x38000) >> 15);
        buffer[offset++] = ((pph_j2_fields->field5 & 0x7f80) >> 7);
        buffer[offset++] = ((pph_j2_fields->field5 & 0x7f) << 1) | ((pph_j2_fields->field4 & 0x200000) >> 21);
        buffer[offset++] = (pph_j2_fields->field4 & 0x1FE000) >> 13;
        buffer[offset++] = (pph_j2_fields->field4 & 0x1FE0) >> 5;
        buffer[offset++] = ((pph_j2_fields->field4 & 0x1F) << 3) | ((pph_j2_fields->field1 & 0xE0) >> 5);
        buffer[offset++] = ((pph_j2_fields->field1 & 0x1F) << 3);
        buffer[offset++] = 0;
        buffer[offset++] = 0;
        buffer[offset++] = (0x4 << 1) | pph_j2_fields->field6;
    }
    else
    {
    ...
    int *found)
{
    int rv;
    uint32 entry_handle_id;
    uint8 peer_mac[DNX_OAM_MAC_SIZE] = { 0 };
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    if (rv == _SHR_E_NONE)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_GTF_INFO_FLAGS, INST_SINGLE, &gtf_info->flag));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_LB_TLV_NUM, INST_SINGLE, &gtf_info->lb_tlv_num));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_LB_TST_MEP_ID, INST_SINGLE, (uint32 *) &gtf_info->mepid));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_TLV_TYPE, INST_SINGLE, &gtf_info->tlv.tlv_type));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_TLV_LENGTH, INST_SINGLE,
                         (uint32 *) &gtf_info->tlv.tlv_length));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_TLV_PATTERN, INST_SINGLE,
                         &gtf_info->tlv.four_byte_repeatable_pattern));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_PKT_PRI, INST_SINGLE, &gtf_info->pkt_pri));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_INNER_PKT_PRI, INST_SINGLE, &gtf_info->inner_pkt_pri));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_INT_PRI, INST_SINGLE, (uint32 *) &gtf_info->int_pri));

        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                        (unit, entry_handle_id, DBAL_FIELD_LB_PEER_MAC, INST_SINGLE, peer_mac));
        sal_memcpy(gtf_info->mac_address, peer_mac, sizeof(gtf_info->mac_address));
    ...
    const DNX_OAM_SAT_GTF_ENTRY * gtf_info)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_TX_FLOW_ID, gtf_id);
    sal_memcpy(peer_mac, gtf_info->mac_address, sizeof(bcm_mac_t));

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LB_TST_MEP_ID, INST_SINGLE, gtf_info->mepid);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GTF_INFO_FLAGS, INST_SINGLE, gtf_info->flag);
    ...
    dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_LB_PEER_MAC, INST_SINGLE, peer_mac);
    ...
    uint32 *extra_enc_hdr_len)
{
    ...
    dbal_fields_e destination_field;
    dbal_fields_e port_id_field;
    int is_spa = FALSE;
    dnx_PPH_base_head pph;
    dnx_PPH_J2_base_head pph_j2;
    uint8 exp, ttl;
    uint32 field_6 = 0;
    uint32 field_7 = 0;
    uint32 field_9 = 0;
    ...
    {
#ifdef BCM_DNX2_SUPPORT
        SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mep_db_ccm_endpoint_get(unit, lb_tst_info->endpoint_id, is_up, mep_db_entry));
#endif
    }
    ...
    if (next_is_itmh)
    {
        ...
        if (!(lb_tst_info->int_pri < 0))
        {
            tc = lb_tst_info->int_pri;
        }
        ...
        destination_field = is_jericho_mode ? DBAL_FIELD_DESTINATION_JR1 : DBAL_FIELD_DESTINATION;
        SHR_IF_ERR_EXIT(dnx_trunk_system_port_is_spa(unit, _sys_phy_port_id, &is_spa));
        port_id_field = is_spa ? DBAL_FIELD_LAG_ID : DBAL_FIELD_PORT_ID;
        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set(unit, destination_field,
                                                             port_id_field, &_sys_phy_port_id, &fwd_dst_info));
    ...
    if (mep_db_entry->mep_type == DBAL_ENUM_FVAL_OAMP_MEP_TYPE_ETH_OAM)
    {
        if (!_SHR_IS_FLAG_SET(mep_db_entry->flags, DNX_OAMP_OAM_CCM_MEP_IS_UPMEP)
            && (dnx_drv_soc_property_get(unit, "injection_with_user_header_enable", 0) == 1))
        {
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            ...
            if (mep_db_entry->nof_vlan_tags == 1)
            {
                ...
                else
                {
                    ...
                    vlans[0] = ((mep_db_entry->inner_pcp_dei & 0xf) << 12) + (mep_db_entry->inner_vid & 0xfff);
                }
                SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_tpid_profile_get(unit, mep_db_entry->inner_tpid_index, &tpids[0]));
            }
            else if (mep_db_entry->nof_vlan_tags == 2)
            {
                ...
                if (lb_tst_info->pkt_pri != 0xff)
                {
                    ...
                    vlans[0] = ((lb_tst_info->pkt_pri & 0xf) << 12) + (mep_db_entry->outer_vid & 0xfff);
                }
                else
                {

                    vlans[0] = ((mep_db_entry->outer_pcp_dei & 0xf) << 12) + (mep_db_entry->outer_vid & 0xfff);
                }
                SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_tpid_profile_get(unit, mep_db_entry->outer_tpid_index, &tpids[0]));

                if (lb_tst_info->inner_pkt_pri != 0xff)
                {

                    vlans[1] = ((lb_tst_info->inner_pkt_pri & 0xf) << 12) + (mep_db_entry->inner_vid & 0xfff);
                }
                else
                {

                    vlans[1] = ((mep_db_entry->inner_pcp_dei & 0xf) << 12) + (mep_db_entry->inner_vid & 0xfff);
                }
                SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_tpid_profile_get(unit, mep_db_entry->inner_tpid_index, &tpids[1]));
            }
        }
#endif
    ...
    else
    {
        ...
        if (is_jericho_mode)
        {
            ...
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_OAMP_INTERNAL_HEADER, &entry_handle_id));

            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FIELD_1, INST_SINGLE, &field_6);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FIELD_2, INST_SINGLE, &field_7);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FIELD_3, INST_SINGLE, &field_9);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FIELD_4, INST_SINGLE, &field_10);
            ...
            pph.field8 = 0;
            pph.field9 = field_9;
            pph.field10 = field_10;

            offset = _dnx_ins_pph_for_tst_lb(header_buffer, &pph, NULL, offset, is_jericho_mode);
        }
        else
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_OAMP_TX_PPH_HEADER, &entry_handle_id));

            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_NTWK_QOS, INST_SINGLE, &pph_j2.field1);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FORWARDING_LAYER_ADDITONAL_INFO, INST_SINGLE,
                                       &pph_j2.field2);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_INLIF_PROFILE, INST_SINGLE, &pph_j2.field3);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_INLIF, INST_SINGLE, &pph_j2.field4);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FORWARD_DOMAIN, INST_SINGLE, &pph_j2.field5);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FORWARDING_STRENGTH, INST_SINGLE,
                                       &pph_j2.field6);

            SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
            offset = _dnx_ins_pph_for_tst_lb(header_buffer, NULL, &pph_j2, offset, is_jericho_mode);
        }

        sal_memset(&header_buffer[offset], 0, udh_size);
        offset = offset + udh_size;
        *sys_hdr_len = offset;
        *extra_enc_hdr_len = 14;

#ifdef BCM_DNX2_SUPPORT
        if (DNX_OAMP_IS_V1(unit))
        {
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mpls_pwe_exp_ttl_profile_get(unit, mep_db_entry->push_profile, &exp, &ttl));
        }
        else
#endif
        {
        ...
        else if (mep_db_entry->mep_type == DBAL_ENUM_FVAL_OAMP_MEP_TYPE_Y1731_PWE)
        {
            ...
            SHR_IF_ERR_EXIT(bcm_dnx_oam_mpls_tp_channel_type_tx_get(unit, bcmOamMplsTpChannelY1731, &pwe_gach));
    ...
    const bcm_oam_tst_rx_t * tst_ptr)
{
        ...
        && (tst_ptr->expected_tlv.tlv_length < DNX_TST_TVL_LENGTH_MIN))
    {
        SHR_ERR_EXIT(BCM_E_PARAM,
                     " TLV length should be larger than or equal to 34 ( Make sure that packet will be larger than 64 bytes) .");
    }
    ...
    const bcm_oam_tst_tx_t * tst_ptr)
{
    ...
    {
        SHR_ERR_EXIT(BCM_E_PARAM,
                     " TLV length should be longer than or equal to 34 ( Since packet should be longer than 64 bytes) .");
    }
    ...
    bcm_oam_tst_rx_t * tst_ptr)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(_dnx_oam_tst_rx_add_verify(unit, tst_ptr));
    ...
    sal_memset(&ctf_info, 0, sizeof(ctf_info));
    ...
    if (!(tst_ptr->flags & BCM_OAM_TST_RX_UPDATE))
    {
        ...
        ctf_info.identifier_trap_id = trap_id;
    ...
    bcm_oam_tst_rx_t * tst_ptr)
{
    ...
    if (index == dnx_data_sat.collector.nof_ctf_ids_get(unit))
    {
        SHR_ERR_EXIT(BCM_E_NOT_FOUND, "endpoint id %d didn't add ctf.", tst_ptr->endpoint_id);
    }
    ...
    bcm_oam_tst_tx_t * tst_ptr)
{
    ...
    uint32 sys_hdr_len = 0;
    uint32 extra_enc_hdr_len = 0;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    SHR_INVOKE_VERIFY_DNXC(_dnx_oam_tst_tx_add_verify(unit, tst_ptr));
    ...
    if (!DNX_OAMP_IS_V2(unit))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mep_db_ccm_endpoint_get(unit, tst_ptr->endpoint_id,
    ...
    sal_memcpy(gtf_info.mac_address, tst_ptr->peer_da_mac_address, sizeof(bcm_mac_t));
    ...
    SHR_IF_ERR_EXIT(_dnx_pp_oam_oamp_lb_tst_header_set(unit, &tst_info,
                                                       ...
                                                       sat_header->pkt_data->data, &offset, &sys_hdr_len,
                                                       &extra_enc_hdr_len));
    ...
    p_config->offsets.packet_length_compensation = tst_ptr->gtf_packet_length_compensation;
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++)
    {
        ...
        for (i = 0; i < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; i++)
        {
            ...
            p_config->packet_edit[priority].packet_length[i] =
                offset - (is_test_tlv ? 1 : 0) + tst_ptr->tlv.tlv_length + 1;
            min_size = 60 - extra_enc_hdr_len + sys_hdr_len;
            if (p_config->packet_edit[priority].packet_length[i] < min_size)
            {
                SHR_ERR_EXIT(BCM_E_PARAM,
                             "packet_length(%d) should be longer than or equal to %d( Since packet should be longer than 64 bytes) .",
                             p_config->packet_edit[priority].packet_length[i], min_size);
            }
        }
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info.flags, BCM_OAM_ENDPOINT_UP_FACING))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, gport_info.local_port, &core_id));
    ...
    bcm_oam_tst_tx_t * tst_ptr)
{
    ...
    for (index = 0; index < dnx_data_sat.generator.nof_gtf_ids_get(unit); index++)
    {
        ...
        if (found && (gtf_info.flag == DNX_PP_OAM_LOOPBACK_TST_INFO_FLAGS_TST)
            && (gtf_info.mepid == tst_ptr->endpoint_id))
        {
    ...
    {
        SHR_ERR_EXIT(BCM_E_NOT_FOUND, "endpoint id %d didn't add gtf.", tst_ptr->endpoint_id);
    }
    gtf_id = index;

    SHR_IF_ERR_EXIT(bcm_dnx_sat_gtf_destroy(unit, gtf_id));
    ...
    bcm_oam_tst_tx_t * tst_ptr)
{
    ...
    for (index = 0; index < dnx_data_sat.generator.nof_gtf_ids_get(unit); index++)
    {
            ...
            && (gtf_info.mepid == tst_ptr->endpoint_id))
        {
            ...
            sal_memcpy(tst_ptr->peer_da_mac_address, gtf_info.mac_address, sizeof(bcm_mac_t));
            break;
        }
    }
    if (index == dnx_data_sat.generator.nof_gtf_ids_get(unit))
    ...
    const bcm_oam_loopback_t * lb_ptr)
{
    ...
    {
        SHR_ERR_EXIT(BCM_E_PARAM, "Trap code bcmRxTrapEgTxOamUpMEPDest2 already used by TST.");
    }
    ...
    bcm_oam_loopback_t * lb_ptr)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_INVOKE_VERIFY_DNXC(_dnx_oam_lb_rx_add_verify(unit, lb_ptr));
    ...
    {
        SHR_IF_ERR_EXIT(oam_sw_db_info.tst_trap_used_cnt.get(unit, &lb_trap_used_cnt));
    ...
    for (index = 0; index < lb_ptr->num_tlvs; index++)
    {
    ...
    bcm_oam_loopback_t * lb_ptr)
{
...
#endif
    DNX_OAM_SAT_GTF_ENTRY gtf_info;
    DNX_PP_OAM_LOOPBACK_TST_INFO lb_info;
    uint32 min_size = 0;
    uint32 gtf_seq_number_offset;
    int offset = 0;
    int tlv_data_offset = 0;
    int shift_offset = 0;
    uint8 is_test_tlv = 0;
    uint8 repeated_data[] = { 0, 0, 0, 0 };
    ...
    rv = bcm_pkt_alloc(unit, DNX_TST_LB_SAT_PKT_HDR_LENGTH, 0, &sat_header);
    SHR_IF_ERR_EXIT(rv);
    _dnx_oam_lb_tst_info_t_init(&lb_info);
    bcm_oam_endpoint_info_t_init(&endpoint_info);
    SHR_IF_ERR_EXIT(bcm_dnx_oam_endpoint_get(unit, lb_ptr->id, &endpoint_info));
#ifdef BCM_DNX2_SUPPORT
    if (!DNX_OAMP_IS_V2(unit))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mep_db_ccm_endpoint_get(unit, lb_ptr->id,
                                                                _SHR_IS_FLAG_SET(endpoint_info.flags2,
                                                                                 BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN),
                                                                &mep_db_entry));
    }
    ...
    lb_info.flags = DNX_PP_OAM_LOOPBACK_TST_INFO_FLAGS_LBM;
    lb_info.endpoint_id = lb_ptr->id;
    lb_info.pkt_pri = lb_ptr->pkt_pri;
    lb_info.inner_pkt_pri = lb_ptr->inner_pkt_pri;
    lb_info.int_pri = lb_ptr->int_pri;

    sal_memcpy(lb_info.mac_address, lb_ptr->peer_da_mac_address, sizeof(bcm_mac_t));
    sal_memcpy(gtf_info.mac_address, lb_ptr->peer_da_mac_address, sizeof(bcm_mac_t));

    SHR_IF_ERR_EXIT(_dnx_pp_oam_oamp_lb_tst_header_set(unit, &lb_info,
                                                       _SHR_IS_FLAG_SET(endpoint_info.flags,
                                                                        BCM_OAM_ENDPOINT_UP_FACING),
                                                       sat_header->pkt_data->data, &offset, &sys_hdr_len,
                                                       &extra_enc_hdr_len));
    ...
    for (i = 0; i < lb_ptr->num_tlvs; i++)
    {
        ...
        if (lb_ptr->tlvs[i].tlv_type == bcmOamTlvTypeData)
        {
            ...
            repeated_data[0] = (lb_ptr->tlvs[i].four_byte_repeatable_pattern & 0xFF000000) >> 24;
            repeated_data[1] = (lb_ptr->tlvs[i].four_byte_repeatable_pattern & 0xFF0000) >> 16;
            repeated_data[2] = (lb_ptr->tlvs[i].four_byte_repeatable_pattern & 0xFF00) >> 8;
            repeated_data[3] = (lb_ptr->tlvs[i].four_byte_repeatable_pattern & 0xFF);

            if ((tlv_data_offset % 4) != 0)
            {
                shift_offset = 4 - (tlv_data_offset % 4);
                p_config->payload.payload_pattern[0] = repeated_data[shift_offset];
                p_config->payload.payload_pattern[1] = repeated_data[(shift_offset + 1) % 4];
                p_config->payload.payload_pattern[2] = repeated_data[(shift_offset + 2) % 4];
                p_config->payload.payload_pattern[3] = repeated_data[(shift_offset + 3) % 4];
                p_config->payload.payload_pattern[4] = repeated_data[shift_offset];
                p_config->payload.payload_pattern[5] = repeated_data[(shift_offset + 1) % 4];
                p_config->payload.payload_pattern[6] = repeated_data[(shift_offset + 2) % 4];
                p_config->payload.payload_pattern[7] = repeated_data[(shift_offset + 3) % 4];
            }
        }
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++)
    {
        for (i2 = 0; i2 < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; i2++)
        {
            ...
            p_config->packet_edit[priority].packet_length[i2] =
                offset - (is_test_tlv ? 1 : 0) + lb_ptr->tlvs[i].tlv_length + 1;

            min_size =
                _SHR_IS_FLAG_SET(endpoint_info.flags,
                                 BCM_OAM_ENDPOINT_UP_FACING) ? 65 : (60 - extra_enc_hdr_len + sys_hdr_len);

            if (p_config->packet_edit[priority].packet_length[i2] < min_size)
            {
                p_config->packet_edit[priority].packet_length[i2] = min_size;
            }
        }
    ...
    p_config->offsets.packet_length_compensation = lb_ptr->gtf_packet_length_compensation;
    ...
    bcm_oam_loopback_t * lb_ptr)
{
    ...
    sal_memset(&gtf_info, 0, sizeof(DNX_OAM_SAT_GTF_ENTRY));
    ...
    sal_memcpy(lb_ptr->peer_da_mac_address, gtf_info.mac_address, sizeof(bcm_mac_t));
    lb_ptr->int_pri = gtf_info.int_pri;
    lb_ptr->pkt_pri = gtf_info.pkt_pri;
    lb_ptr->inner_pkt_pri = gtf_info.inner_pkt_pri;

    lb_ptr->num_tlvs = gtf_info.lb_tlv_num;
    ...
    bcm_oam_loopback_t * lb_ptr)
{
    ...
    if (index == dnx_data_sat.collector.nof_ctf_ids_get(unit))
    {
        SHR_ERR_EXIT(BCM_E_PARAM, "endpoint id %d didn't add ctf.", lb_ptr->id);
    }
    lb_ptr->ctf_id = index;
    ...
    if (index == dnx_data_sat.generator.nof_gtf_ids_get(unit))
    {
        SHR_ERR_EXIT(BCM_E_PARAM, "endpoint id %d didn't add gtf.", lb_ptr->id);
    }
    lb_ptr->gtf_id = index;
    ...
    rv = bcm_dnx_sat_ctf_destroy(unit, lb_ptr->ctf_id);
    SHR_IF_ERR_EXIT(rv);
    ...
    rv = _dnx_oam_sat_ctf_info_del(unit, lb_ptr->ctf_id);
    rv = _dnx_oam_sat_gtf_info_del(unit, lb_ptr->gtf_id);
