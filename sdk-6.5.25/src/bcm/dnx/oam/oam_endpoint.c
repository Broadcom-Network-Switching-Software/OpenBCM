    ...
    const bcm_oam_group_info_t * group_info)
{
    ...
    if (!DNX_OAM_IS_ENDPOINT_RFC6374(endpoint_info) && !DNX_OAM_DISSECT_IS_LMM(endpoint_info)
        ...
        !DNX_OAM_DISSECT_IS_1DM(endpoint_info) && !DNX_OAM_DISSECT_IS_DMM_ONE_WAY_STATS(endpoint_info))
    {
    ...
    const uint32 endpoint_id)
{
    ...
    use_ouflif = !USE_GLOB_IN_LIF(endpoint_info, lif_resolution);
    /*
     * Store lif info in sw_state
     */
    if ((lif_resolution->local_out_lif != DNX_OAM_INVALID_LIF) && use_ouflif)
    {
        /*
