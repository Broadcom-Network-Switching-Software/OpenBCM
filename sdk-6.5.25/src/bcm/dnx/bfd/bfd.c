            ...
            if (endpoint_info->bfd_period_cluster != existing_endpoint_info->bfd_period_cluster)
            {
                SHR_IF_ERR_EXIT(dnx_bfd_oamp_v1_existing_cluster_info_get
                                (unit, endpoint_info->id, endpoint_info->bfd_period_cluster, &cluster_profile_exists,
                                 &cluster_period, &cluster_scan_rate));
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_WITH_ID))
    {
        if (!_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_UPDATE))
        {
           /** Find endpoint in SW state table and read data   */
            rv = dnx_bfd_get_endpoint_from_sw_state_table(unit, endpoint_info->id, &sw_read_data);
            ...
            if ((rv == BCM_E_NONE)
                && _SHR_IS_FLAG_SET(sw_read_data.bfd_sw_state_flags,
                                    DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_SERVER))
            {
                is_server = 1;
            }
        }
 ...
 */
    ...
    /** BFD Echo doesn't use OAM Classifier */
    if (!_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_ECHO)
        || dnx_data_oam.general.feature_get(unit, dnx_data_oam_general_oam_hlm_support) == 0)
    {
...
#ifdef BCM_DNX2_SUPPORT /** ********************* V1 ONLY *********************** */
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
    ...
    if (_SHR_IS_FLAG_SET(sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_FROM_OAMP))
    {
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            ...
            if (_SHR_IS_FLAG_SET
                (sw_read_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_VCCV_TYPE))
            {
