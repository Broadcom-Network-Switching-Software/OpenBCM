...
/** Maximum reserved DBs per SP must be below the total DBs of the SP minus this amount */
#define NOF_DBS_BELOW_MAX 64
    ...
    dnx_ecgm_threshold_sp_action_e action)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ecgm_sp_reserved_db_threshold_modify_verify(unit, core_id, sp, reserved));
    ...
    uint32 sp_reserved)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_multicast_sp_threshold_get
                    (unit, core, sp, DBAL_FIELD_DB_DROP, &current_sp_drop));
    ...
    if (sp_reserved > (current_sp_drop - NOF_DBS_BELOW_MAX))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Invalid new reserved value - the new reserved DBs value %d for SP%d is over the limit. Must be below max-%d, where max DBs is %d!",
                     sp_reserved, sp, NOF_DBS_BELOW_MAX, current_sp_drop);
    }
exit:
    SHR_FUNC_EXIT;
}
