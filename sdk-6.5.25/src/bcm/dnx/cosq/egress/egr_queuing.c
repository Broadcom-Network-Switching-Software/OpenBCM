    ...
    if (master_logical_port == DNX_ALGO_PORT_INVALID)
    {
        ...
        switch (interface_type)
        {
            ...
            case SOC_PORT_IF_VIRTUAL_FLEXE_CLIENT:
            {
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, port, &flexe_channel_id));
    ...
    bcm_port_t port)
{
    ...
    if (dnx_data_egr_queuing.params.feature_get(unit, dnx_data_egr_queuing_params_fqp_enhanced_attribuites) == 1)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_egr_queuing_fqp_param_set
                        (unit, core, egr_if, DBAL_FIELD_ALLOW_CONS_SEL, (fqp_min_gap == 0)));
    }
    SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_INTERFACE_ATTRIBUTES,
                                     2, 1,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, (uint32) core,
            ...
            params.feature_get(unit, dnx_data_egr_queuing_params_shared_hp_tdm_config_is_supported) == FALSE)
        {
            ...
            if (if_tdm_mode != DNX_ALGO_PORT_IF_NO_TDM)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Invalid parameter: a TDM interface can't have high priority queues at this device");
            }
        }
    }
    else
