    ...
    bcm_cosq_stat_info_t * config)
{
    ...
    int cmd_id_min_value, cmd_id_max_value;
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(key, _SHR_E_PARAM, "key");
    ...
    cmd_id_min_value = itm_command_ids->base;
    cmd_id_max_value = cmd_id_min_value + (itm_command_ids->size) - 1;
    if (key->command_id < cmd_id_min_value || key->command_id > cmd_id_max_value)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "key.command_id=%d invalid. min allowed: %d, max allowed: %d", key->command_id,
                     cmd_id_min_value, cmd_id_max_value);
    }
    ...
    uint32 object_stat_id)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_stat_obj_map_set_verify(unit, flags, key, object_stat_id));
    ...
    uint32 *object_stat_id)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_stat_obj_map_get_verify(unit, flags, key, object_stat_id));
    ...
    bcm_cosq_stat_info_t * config)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_stat_info_set_verify(unit, flags, key, config));
    ...
    bcm_cosq_stat_info_t * config)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_stat_info_get_verify(unit, flags, key, config));
...
static shr_error_e
dnx_cosq_stat_discard_egress_meter_set_verify(
    int unit,
    uint32 flags)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    switch (flags)
    {
        case (BCM_COSQ_DISCARD_EGRESS | BCM_COSQ_DISCARD_ENABLE | BCM_COSQ_DISCARD_COLOR_ALL):
        case (BCM_COSQ_DISCARD_EGRESS | BCM_COSQ_DISCARD_ENABLE | BCM_COSQ_DISCARD_COLOR_BLACK | BCM_COSQ_DISCARD_COLOR_RED | BCM_COSQ_DISCARD_COLOR_YELLOW):
        case (BCM_COSQ_DISCARD_EGRESS | BCM_COSQ_DISCARD_ENABLE | BCM_COSQ_DISCARD_COLOR_BLACK | BCM_COSQ_DISCARD_COLOR_RED):
        case (BCM_COSQ_DISCARD_EGRESS | BCM_COSQ_DISCARD_ENABLE | BCM_COSQ_DISCARD_COLOR_BLACK):
        case (BCM_COSQ_DISCARD_EGRESS):
            break;
        default:
    ...
    uint32 flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_stat_discard_egress_meter_set_verify(unit, flags));
    ...
    else
    {
        ...
        for (dp_index = 0; dp_index <= field_info.max_value; dp_index++)
        {
            ...
            /** set the table value. TRUE - enable drop, FALSE - disable drop  */
            dbal_value = flags & discard_dp[dp_index] ? TRUE : FALSE;
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENABLE, INST_SINGLE, dbal_value);
