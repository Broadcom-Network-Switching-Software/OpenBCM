    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqControlServicePoolUcSize:
            if (dnx_data_ecgm.pool_db_size.feature_get(unit, dnx_data_ecgm_pool_db_size_pool_db_size) == FALSE)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "This API is not supported for this device type.\n");
            }
            ...
            SHR_RANGE_VERIFY(arg, 0, dnx_data_ecgm.pool_db_size.uc_pool_db_max_size_get(unit), _SHR_E_PARAM,
        ...
        case bcmCosqControlServicePoolMcSize:
            ...
            SHR_RANGE_VERIFY(arg, 0, dnx_data_ecgm.pool_db_size.mc_pool_db_max_size_get(unit), _SHR_E_PARAM,
                             "Pool size out of bound");
            break;
        default:
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_control_interface_verify(unit, gport, cosq, type));
    ...
    bcm_cosq_fadt_threshold_t * threshold)
{
    ...
    is_vsq_allowed = FALSE;
    if (fadt_info->thresh_type != bcmCosqFadtEgressEnqueueAdmissonDrop &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueAdmissionFcHigh &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueAdmissionFcLow &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueAdmissionFcPfcOn &&
        fadt_info->thresh_type != bcmCosqFadtEgressEnqueueAdmissionFcPfcOff)
    {
        /*
    ...
    switch (fadt_info->thresh_type)
    {
        ...
        case bcmCosqFadtEgressEnqueueAdmissonDrop:
        case bcmCosqFadtEgressEnqueueAdmissionFcHigh:
        case bcmCosqFadtEgressEnqueueAdmissionFcLow:
        case bcmCosqFadtEgressEnqueueAdmissionFcPfcOn:
        case bcmCosqFadtEgressEnqueueAdmissionFcPfcOff:
            break;
        default:
    ...
    bcm_cos_queue_t * cosq)
{
...

    {
        ...
        if ((key_array = sal_alloc(sizeof(bcm_cosq_queue_offset_mapping_key_t) * count, "key_array")) == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error allocating memory of key_array for port %d\n", port);
        }
        if ((offset_array = sal_alloc(sizeof(bcm_cosq_queue_offset_t) * count, "offset_array")) == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error allocating memory of offset_array for port %d\n", port);
        }
