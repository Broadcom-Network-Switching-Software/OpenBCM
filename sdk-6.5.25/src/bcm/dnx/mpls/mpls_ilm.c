...
/**
  *  \brief
  * Verify info struct according to ilm (mpls forwarding) table, used by dnx_mpls_tunnel_switch_ilm_add()
  * requirements.
  * \par DIRECT INPUT
  *    \param [in] unit -
  *     Relevant unit.
  *    \param [in] result_type -
  *     ilm result type to be validated, other validations are
  *     done according to its value
  *    \param [in] info -
  *     A pointer to a struct containing relevant information for ilm configuration.
  * \par INDIRECT INPUT:
  *
  * \par DIRECT OUTPUT:
  ...
  *   * None.
  * \see
  *   * \ref dnx_mpls_tunnel_switch_ilm_add
  */
static shr_error_e
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (info->egress_label.egress_qos_model.egress_qos == bcmQosEgressModelUniform)
    {
        if (info->egress_label.exp != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "exp must be 0 in case qos model is uniform");
        }
    }
    else
    {
        if (info->egress_label.exp > dnx_data_qos.qos.packet_max_exp_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "exp value %d is out of range", info->egress_label.exp);
        }
    }
    ...
    if (info->egress_label.egress_qos_model.egress_ttl == bcmQosEgressModelUniform)
    {
        if (info->egress_label.ttl != 0)
        {
        SHR_ERR_EXIT(_SHR_E_PARAM, "ttl must be 0 in case ttl model is uniform")}
    }
    ...
    if (info->action == BCM_MPLS_SWITCH_ACTION_SWAP && DNX_QOS_MAP_PROFILE_GET(info->qos_map_id) != 0)
    {
        ...
        if (!DNX_QOS_MAP_IS_EGRESS(info->qos_map_id) ||
            ...
            (DNX_QOS_MAP_PROFILE_GET(info->qos_map_id) >= DBAL_NOF_ENUM_SWAP_QOS_PROFILE_VALUES))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "info->qos_map_id %d is out of range (1~7)",
                         DNX_QOS_MAP_PROFILE_GET(info->qos_map_id));
        }
        if (DNX_QOS_MAP_PROFILE_GET(info->qos_map_id) >= DBAL_ENUM_FVAL_SWAP_QOS_PROFILE_MAX)
        {
            /** Note the ENUM is defined in range 1 - 8, and the qos_profile is in range 0 - 7.*/
            SHR_ERR_EXIT(_SHR_E_PARAM, "Only %d remark profiles are available in SWAP cases",
                         DBAL_ENUM_FVAL_SWAP_QOS_PROFILE_MAX);
        }
    }
...
/**
  *  \brief
  * Verify info struct according to ilm (mpls forwarding) table, used by dnx_mpls_tunnel_switch_ilm_delete()
  * requirements.
  * \par DIRECT INPUT
  *    \param [in] unit -
  *     Relevant unit.
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    /** Verification. */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_switch_ilm_add_verify(unit, result_type, info));
    ...
    else
    {
        ...
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VMPI, info->class_id);
        }
    }
    ...
    {
        /*
         * In JR1 mode, maybe there is an allocated push profile**
         */
        uint32 eei;
        ...
        if ((old_result_type == DBAL_RESULT_TYPE_MPLS_FWD_FWD_DEST_EEI) ||
            (old_result_type == DBAL_RESULT_TYPE_MPLS_FWD_FWD_DEST_EEI_STAT))
        {
            uint32 label;
            ...
            {
                SHR_IF_ERR_EXIT(dnx_mpls_eei_push_information_retrieve(unit, eei, &label, &old_push_cmd));
            }
        }
    }

    SHR_IF_ERR_EXIT(dbal_entry_handle_payload_clear(unit, entry_handle_id));
             ...
             || (result_type == DBAL_RESULT_TYPE_MPLS_FWD_FWD_DEST_EEI_STAT))
    {
        ...
        {
            uint8 push_cmd_replace = FALSE;

            if (old_push_cmd != 0)
            {
                push_cmd = old_push_cmd;
                push_cmd_replace = TRUE;
                /*
                 * old push cmd will be freed in push cmd exchange process, so must set to 0, to avoid double free
                 */
                old_push_cmd = 0;
            }
            SHR_IF_ERR_EXIT(dnx_mpls_tunnel_create_push_entry_from_eei(unit, &(info->egress_label), FALSE,
                                                                       push_cmd_replace, &push_cmd));
    ...
    /*
     * In case replace, force to update
     */
    ...
    /*
     * In case aciton push to swap or others, the old push cmd should be freed
     */
    if (old_push_cmd)
    {
        SHR_IF_ERR_EXIT(dnx_mpls_tunnel_delete_push_entry_from_eei(unit, old_push_cmd));
    }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    /** Verification. */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_switch_ilm_delete_verify(unit, info));
...
/**
 * \brief
 * callback for delete an ILM entry used by dnx_mpls_tunnel_switch_ilm_delete_all.
 *
 * \param [in] unit - The unit number.
 * \param [in] info   - A pointer to the tunnel switch information.
 * \param [in] user_data - user data that will be sent to the callback function
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 ...
 */
static shr_error_e
dnx_mpls_tunnel_switch_iml_delete_all_cb(
    int unit,
    bcm_mpls_tunnel_switch_t * info,
    void *user_data)
{
    ...
    int unit)
{
    ...
    /** clear tables by traverse*/
    SHR_IF_ERR_EXIT(dnx_mpls_tunnel_switch_ilm_traverse(unit, dnx_mpls_tunnel_switch_iml_delete_all_cb, NULL));
