 ...
 */
#define TUNNEL_INIT_VXLAN_VNI_ESEM_CMD_FLAG_GET(tunnel_type) ((((tunnel_type == bcmTunnelTypeVxlan) || (tunnel_type == bcmTunnelTypeVxlan6) ||\
                (tunnel_type == bcmTunnelTypeVxlanGpe) || (tunnel_type == bcmTunnelTypeVxlan6Gpe))) ? TUNNEL_INIT_ESEM_ACCESS_TYPE_VXLAN_VNI : 0)
/*
 ...
 */
#define TUNNEL_INIT_ESEM_CMD_FLAGS_GET(tunnel)  (TUNNEL_INIT_VXLAN_VNI_ESEM_CMD_FLAG_GET(tunnel->type) | TUNNEL_INIT_SVTAG_ESEM_CMD_FLAG_GET(tunnel))
/*
    ...
    uint32 *local_outlif)
{
    ...
    /** In case of SRv6 Scale Flag, set to Result of IPv6_SCALE */
    ...
    /** In case of SRv6 Scale Flag, Set DIP on the DIP_prefix field, locatedon teh main entry, and skip IPv6 entry */
    if (_SHR_IS_FLAG_SET(tunnel->flags, BCM_TUNNEL_INIT_SRV6_SCALE_VPN_SID))
    {
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_IPV6_DIP, INST_SINGLE, tunnel->dip6);
    }
    else if ((tunnel->type == bcmTunnelTypeSR6) || (tunnel->type == bcmTunnelTypeEthSR6))
    {
        if (srv6_with_dip)
        {
    ...
    bcm_tunnel_type_t tunnel_type)
{
    ...
    if (old_esem_cmd_profile != dnx_data_esem.access_cmd.no_action_get(unit))
    {
        ...
        uint32 vxlan_flags = TUNNEL_INIT_VXLAN_VNI_ESEM_CMD_FLAG_GET(tunnel_type);
        ...
        if (remove_esem_cmd_profile == TRUE)
        {
            ...
            if (_SHR_IS_FLAG_SET(vxlan_flags, TUNNEL_INIT_ESEM_ACCESS_TYPE_VXLAN_VNI))
            {
                uint32 old_lif_trap_profile = 0;
                uint32 new_lif_trap_profile = 0;
    ...
    uint32 *svtag_flag)
{
    ...
    if ((((tunnel_type == bcmTunnelTypeIpAnyIn4) || (tunnel_type == bcmTunnelTypeIpAnyIn6))
         && lif_additional_header_profile == DBAL_ENUM_FVAL_LIF_AHP_IP_LIF_AHP_IP_ESP)
        || (((tunnel_type == bcmTunnelTypeUdp) || (tunnel_type == bcmTunnelTypeUdp6))
            && lif_additional_header_profile == DBAL_ENUM_FVAL_LIF_AHP_UDP_LIF_AHP_UDP_ESP)
        || ((tunnel_type == bcmTunnelTypeVxlan || tunnel_type == bcmTunnelTypeVxlan6)
            && lif_additional_header_profile == DBAL_ENUM_FVAL_LIF_AHP_VXLAN_VXLAN_GPE_LIF_AHP_DOUBLE_UDP_ESP_VXLAN))
    {
