...
#define DNX_IPMC_GLOB_IN_LIF_LSB_GET(in_lif) (in_lif & (UTILEX_BITS_MASK(IPMC_IPV6_GLOB_IN_LIF_NOF_LSB, 0)))
    ...
    uint32 *result_type)
{
    ...
    if (IPMC_TABLE_IS_KBP(dbal_table))
    {
        ...
        if (IPMC_IS_IPV4_DEFAULT_ENTRY(data) || IPMC_IS_IPV6_DEFAULT_ENTRY(data, cmprs_vrfg))
        {
    ...
    int unit)
{
    ...
    uint8 use_kbp_ipv6 = dnx_data_elk.application.feature_get(unit, dnx_data_elk_application_ipv6) ? TRUE : FALSE;
    uint32 nof_tables;
    dbal_tables_e *dbal_mc_tables;

    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    DBAL_FUNC_INIT_VARS(unit);
    dbal_mc_tables = sal_alloc(sizeof(dbal_tables_e) * NOF_IPMC_TABLES(unit), "IPMC DBAL table array");
...
exit:
    if (dbal_mc_tables != NULL)
    {
        sal_free(dbal_mc_tables);
    }
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
    ...
    void *user_data)
{
    ...
    dbal_tables = sal_alloc(nof_tables * sizeof(dbal_tables_e), "IPMC DBAL table array");
