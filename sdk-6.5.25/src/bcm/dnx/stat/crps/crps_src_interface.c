    ...
    uint32 *offset)
{
    ...
    tmp_offset = dnx_data_crps.expansion.source_type_get(unit, src, field_type)->offset;
    ...
    for (elem_index = 0; elem_index < expansion_select->nof_elements; elem_index++)
    {
        ...
        nof_bits =
            dnx_data_crps.expansion.source_type_get(unit, src,
                                                    expansion_select->expansion_elements[elem_index].type)->nof_bits;
    ...
    int *is_selected)
{
    ...
    nof_bits = dnx_data_crps.expansion.source_type_get(unit, src, field_type)->nof_bits;
    ...
    bcm_stat_expansion_select_t * expansion_select)
{
    ...
    for (expansion_element = bcmStatExpansionTypeTmDropReason; expansion_element < bcmStatExpansionTypeMax;
         expansion_element++)
    {
        if (dnx_data_crps.expansion.source_type_get(unit, src, expansion_element)->nof_bits > 0)
        {
    ...
    uint32 *val_arr)
{
    ...
    if (interface->interface_source != bcmStatCounterInterfaceIngressTransmitPp)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_STAT_IF_OBJECT_ID, interface->command_id);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_STAT_IF_TYPE_ID, interface->type_id);
    }
    ...
    int unit)
{
    ...
    total_nof_bits = dnx_data_crps.expansion.source_type_get(unit, dynamic_metadata_src,
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_idx)
    {
        ...
        while (expansion_element != bcmStatExpansionTypeMax)
        {
            ...
            nof_bits = dnx_data_crps.expansion.source_type_get(unit, dynamic_metadata_src, expansion_element)->nof_bits;
            if (nof_bits > 0)
            {
    ...
    uint32 *next_free_shift)
{
    ...
    metadata_size = dnx_data_crps.expansion.source_type_get(unit, src, DNX_CRPS_EXPANSION_TOTAL_NOF_BITS)->nof_bits;

    for (element_type = bcmStatExpansionTypeTmDropReason; element_type < bcmStatExpansionTypeMax; element_type++)
    {
        current_element_size = dnx_data_crps.expansion.source_type_get(unit, src, element_type)->nof_bits;
    ...
    bcm_stat_expansion_select_t * expansion_select)
{
    ...
    /** get shift max value */
    metadata_size = dnx_data_crps.expansion.source_type_get(unit, interface->interface_source,
                                                            DNX_CRPS_EXPANSION_TOTAL_NOF_BITS)->nof_bits;
