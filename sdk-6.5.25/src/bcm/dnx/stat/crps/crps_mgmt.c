 ...
 */
    static int
crps_mgmt_nof_counters_in_engine_get(
    int unit,
    int core_id,
    int engine_id)
{
    ...
    uint32 *basic_offset)
{
    ...
    while (cur_engine != DNX_CRPS_MGMT_ENGINE_INVALID)
    {
        ...
        if (prev_engine != -1)
        {
            ...
            range_size += dnx_data_crps.engine.engines_info_get(unit, prev_engine)->nof_entries;
    ...
    set_config_arr[DNX_DATA_MAX_CRPS_ENGINE_NOF_ENGINES][DNX_DATA_MAX_CRPS_ENGINE_MAX_NOF_COUNTER_SETS])
{
    ...
    {
        engine_size = crps_mgmt_nof_counters_in_engine_get(unit, core_id, engine_id);
    ...
    dnx_set_config_t set_config[DNX_DATA_MAX_CRPS_ENGINE_NOF_ENGINES][DNX_DATA_MAX_CRPS_ENGINE_MAX_NOF_COUNTER_SETS])
{
    ...
    {
        nof_counters_in_engine = crps_mgmt_nof_counters_in_engine_get(unit, database->core_id, curr_engine_id);
    ...
    bcm_stat_engine_t * engine)
{
    ...
    nof_entries = dnx_data_crps.engine.engines_info_get(unit, engine->engine_id)->nof_entries;
    ...
    input_engine_range.end = input_engine_range.start + (nof_entries * nof_counters_per_entry) - 1;
    ...
    for (type_id = 0; type_id < BCM_STAT_MAX_NUMBER_OF_OBJECT_TYPES; type_id++)
    {
        ...
        if (type.valid == TRUE)
        {
            ...
            if (type_valid[type_id] == TRUE)
            {
                ...
                {
                    last_index = first_index + (nof_entries * nof_counters_per_entry) - 1;
                }
