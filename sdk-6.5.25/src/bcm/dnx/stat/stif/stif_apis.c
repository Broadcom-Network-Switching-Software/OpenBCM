    ...
    bcm_port_t port)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_stif_source_mapping_source_mapping_set_verify(unit, flags, source, port));
    ...
    bcm_port_t * port)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_stif_source_mapping_source_mapping_get_verify(unit, flags, source, port));
    ...
    for (element = 0; element < nof_elements; element++)
    {
        ...
        switch (record_format_elements[element].element_type)
        {
            ...
            case bcmStatStifRecordElementIngressIsLastCopy:
            {
                ...
                if (record_format_elements[element].element_type == bcmStatStifRecordElementIngressDispositionIsDrop)
                {
                    if (dnx_data_stif.config.feature_get(unit, dnx_data_stif_config_stif_report_mc_single_copy) &&
                        !dnx_data_stif.config.feature_get(unit,
                                                          dnx_data_stif_config_stif_report_disp_is_drop_with_mc_single_copy))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "bcmStatStifRecordElementIngressDispositionIsDrop is not supported when STIF is configured to count all ingress MC copies as one.");
                    }
                }
                break;
            }
    ...
    bcm_stat_stif_record_format_element_t * record_format_elements)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_stif_record_format_set_verify(unit, flags, nof_elements, record_format_elements));
    /** Verify API is called before traffic is enabled */
    SHR_IF_ERR_EXIT(dnx_stk_sys_traffic_enable_get(unit, &is_traffic_enabled));
    if (is_traffic_enabled == TRUE)
    {
    ...
    int *nof_elements)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_stat_stif_record_format_get_verify
