    ...
    lif_mngr_local_outlif_str_info_t * outlif_str_info)
{
    ...
    if (outlif_str_info != NULL)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_mngr_phase_string_to_id
                        (unit, outlif_str_info->outlif_phase_name, &outlif_info.logical_phase));
    ...
    SHR_IF_ERR_EXIT(dnx_lif_lib_allocate(unit, flags, global_lif_info, inlif_info_ptr, outlif_info_ptr));
