 ...
 *      bcm_common_ptp_clock_port_delay_asymmetry_get
 *      bcm_common_ptp_clock_port_delay_asymmetry_set
...
int g_apts_synce_freq_corr_ppb = 0;
int g_apts_gps_offset = 0;
int g_apts_servo_assist_gps = 1;
    ...
    bcm_ptp_foreign_master_dataset_t *data_set)
{
    ...
    while (index < num_masters) {
        ...
        sal_memcpy(&fm->address.address[0], resp + offset, BCM_PTP_MAX_NETW_ADDR_SIZE);
    ...
    bcm_ptp_trace_t *trace)
{
     ...
     *    Octet 0 : Time traceable (TTRA) and frequency traceable (FTRA) Booleans.
     *              |B7|B6|B5|B4|B3|B2|B1|B0| = |0|0|FTRA|TTRA|0|0|0|0|.
     *    Octet 1 :  B0 = NO_OVERRIDE_FREQ, B1 = NO_OVERRIDE_TIME, B2 = CHK_TRC_FLAG_ENABLE
     */
    if (trace->frequency_traceable == BCM_PTP_TRACEABILITY_NO_OVERRIDE) {
        /* Over Ride Clear Flag Set */
        payload[1] = 0x1;

    } else if ((trace->frequency_traceable == 0) || (trace->frequency_traceable == 1)) {

        payload[0] = (0x01 & trace->frequency_traceable) << 5;
    } else {
        return BCM_E_PARAM;
    }
    ...
    } else if ((trace->time_traceable == 0) || (trace->time_traceable == 1)) {
    ...
    if (trace->check_trc_flag_enable == 1){
        payload[1] |= 4;
    }
    ...
    uint32 *latency_out)
{
    ...
    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num, &portid,
            PTP_MGMTMSG_GET, PTP_MGMTMSG_ID_PORT_LATENCY, 0, 0, resp, &resp_len);
    ...
    if (rv == BCM_E_NONE) {
         ...
         *                        BCM<null><null><null>.
         *    Octet 6...9   : Inbound Latency
         *    Octet 10...13 : Outbound Latency
         */
        i = 6;
        *latency_in = _bcm_ptp_uint32_read(resp + i);
        i += sizeof(uint32);
        *latency_out = _bcm_ptp_uint32_read(resp + i);
    }
 ...
 *      bcm_common_ptp_clock_port_delay_asymmetry_set
 * Purpose:
 *      Set PTP clock port expected delay asymmetry.
 * Parameters:
 *      unit        - (IN) Unit number.
 *      ptp_id      - (IN) PTP stack ID.
 *      clock_num   - (IN) PTP clock number.
 *      clock_port  - (IN) PTP clock port number.
 *      delay_asymmetry_ns - (IN) PTP clock port delay_asymmetry (ns).
 * Returns:
 *      BCM_E_XXX
 * Notes:
 *      Function is an implementation-specific PTP management message.
    ...
    int    delay_asymmetry_ns)
{
    ...
    uint8 payload[PTP_MGMTMSG_PAYLOAD_PORT_DELAY_ASYMMETRY_SIZE_OCTETS] = {0};
    uint8 resp[PTP_MGMTMSG_RESP_MAX_SIZE_OCTETS];
    int resp_len = PTP_MGMTMSG_RESP_MAX_SIZE_OCTETS;
    uint64 delay_asymmetry;
    int i = 0;
     ...
     *                    Octet 0= 'B'; Octet 1= 'C'; Octet 2= 'M'.
     *    Octet 3...5   : Reserved.
     *    Octet 6...13  : port delay asymmetry.
     *                    Binary fixed-point value. Time (ns)= delay_asymmetry/(2^16).
     */
    i = 0;
    payload[i++] = 'B';
    payload[i++] = 'C';
    ...
    if (delay_asymmetry_ns < 0)
        COMPILER_64_SET(delay_asymmetry, 0xFFFFFFFF, delay_asymmetry_ns);
    else
        COMPILER_64_SET(delay_asymmetry, 0, delay_asymmetry_ns);
    COMPILER_64_SHL(delay_asymmetry, 16);
    _bcm_ptp_uint64_write(payload+i, delay_asymmetry);
    i += sizeof(uint64);

    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num, &portid,
            PTP_MGMTMSG_SET, PTP_MGMTMSG_ID_PORT_DELAY_ASYMMETRY,
            payload, PTP_MGMTMSG_PAYLOAD_PORT_DELAY_ASYMMETRY_SIZE_OCTETS,
            resp, &resp_len);
 ...
 *      bcm_common_ptp_clock_port_delay_asymmetry_get
 * Purpose:
 *      Get PTP clock port expected delay asymmetry
 * Parameters:
 *      unit        - (IN) Unit number.
 *      ptp_id      - (IN) PTP stack ID.
 *      clock_num   - (IN) PTP clock number.
 *      clock_port  - (IN) PTP clock port number.
 *      delay_asymmetry_ns - (OUT) PTP clock port delay_asymmetry (ns).
 * Returns:
 *      BCM_E_XXX
 * Notes:
 *      Function is an implementation-specific PTP management message.
    ...
    int   *delay_asymmetry_ns)
{
    ...
    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num, &portid,
            PTP_MGMTMSG_GET, PTP_MGMTMSG_ID_PORT_DELAY_ASYMMETRY, 0, 0, resp, &resp_len);
    ...
    if (rv == BCM_E_NONE) {
         ...
         *    Octet 0...5 :Custom management message key/identifier.
         *                        BCM<null><null><null>.
         *    Octet 6...9   : Delay asymmetry
         */
        i = 6;
        *delay_asymmetry_ns = _bcm_ptp_uint32_read(resp + i);
        i += sizeof(uint32);
    }
    ...
    bcm_ptp_clock_port_info_t *info)
{
    ...
    if (info->vport_info.onepps_in_gpio == 6) /* BS0_HB is used as 1PPS input */
    {
        _bcm_tdpll_output_clock_TS1_1ppsref_create(unit, ptp_id, clock_num, bcm_ptp_pin_bs0_hb);
        _bcm_tdpll_output_clock_TS1_1ppsref_enable( unit, ptp_id, 1);
    }
#endif
...
/*
 * Function:
 *      _bcm_ptp_packet_dump_configure
 * Purpose:
 *      Set the packet dump level for the firmware.
 * Parameters:
 *      unit       - (IN) Unit number.
 *      ptp_id     - (IN) PTP stack ID.
 *      clock_num  - (IN) PTP clock number.
 *      dump_mask  - (IN) bitmask of packet dump levels
 *      dump_count - (IN) Packet dump count
 * Returns:
 *      BCM_E_XXX
 */
int
_bcm_ptp_packet_dump_configure(int unit, bcm_ptp_stack_id_t ptp_id, int clock_num,
                    uint32 dump_mask, uint32 dump_count)
{
    int rv = BCM_E_UNAVAIL;

    uint8 payload[PTP_MGMTMSG_PAYLOAD_PACKET_DUMP_LEVEL_SIZE_OCTETS] = {0};
    uint8 *curs = &payload[0];
    uint8 resp[PTP_MGMTMSG_RESP_MAX_SIZE_OCTETS];
    int resp_len = PTP_MGMTMSG_RESP_MAX_SIZE_OCTETS;
     ...
     * Make payload.
     *    Octet 0..5:   BCM<nul><nul><nul>
     *    Octet 6..9:  new packet dump levle
     *    Octet 10..13: new packet dump count
     */
    sal_memcpy(curs, "BCM\0\0\0", 6); curs += 6;
    _bcm_ptp_uint32_write(curs, dump_mask);   curs += 4;
    _bcm_ptp_uint32_write(curs, dump_count);   curs += 4;

    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num,
                    &portid, PTP_MGMTMSG_SET, PTP_MGMTMSG_ID_PACKET_DUMP_LEVEL,
                    payload, PTP_MGMTMSG_PAYLOAD_PACKET_DUMP_LEVEL_SIZE_OCTETS,
                    resp, &resp_len);
...
void _bcm_ptp_apts_update_current_mode(int unit, int stack_id)
{
    int i;
    ...
    if (old_op_mode != g_apts_current_op_mode){
        ...
        if (((old_op_mode == bcmPtpClockAptsModePtpPtp) || (old_op_mode == bcmPtpClockAptsModeSyncEPtp)) &&
            ...
            if (!g_apts_servo_assist_gps) {
                bcm_ptp_clock_info_t *clock_info;
                bcm_ptp_port_identity_t clk_portid;
                bcm_ptp_stack_id_t ptp_id = 0;
                ...
                /* No RefTracker instance tracking GPS. */
            ...
            else if (BCM_FAILURE(rv = _bcm_ptp_servo_best_master_update(0, 0, 0, &g_gps_port_id)))
            {
    ...
    if (g_apts_usr_cfgd_source_state & bcmPtpClockAptsSourceSynceInternalDpll) {
        /* Moving from SyncePTP->PtpPTP, SynceGPS ->GpsGps mode, restoring synce frequency correction value from TDPLL */
        if (((old_op_mode == bcmPtpClockAptsModeSyncEPtp) || (old_op_mode == bcmPtpClockAptsModeSyncEGps)) &&
            ((g_apts_current_op_mode == bcmPtpClockAptsModePtpPtp) ||
             ((g_apts_current_op_mode == bcmPtpClockAptsModeGpsGps) &&
              (g_apts_servo_assist_gps)))) {
              if (BCM_FAILURE(rv = _bcm_ptp_freq_corr_get(unit, stack_id,
                  PTP_CLOCK_NUMBER_DEFAULT, &curr_ptp_freq_corr_ppt,
                  &curr_ptp_freq_offset_ppt, NULL, NULL))) {
                    PTP_ERROR_FUNC("_bcm_ptp_freq_corr_get() Error ");
              }
              diff_freq_corr_ppb = ((double)(g_apts_synce_freq_corr_ppb * 1000) + (double)curr_ptp_freq_corr_ppt)/(double)1000;
              if (g_apts_synce_freq_corr_ppb && diff_freq_corr_ppb) {
                  if (BCM_FAILURE(rv = _bcm_ptp_freq_corr_set(unit, stack_id, PTP_CLOCK_NUMBER_DEFAULT, diff_freq_corr_ppb, 0))) {
                    PTP_ERROR_FUNC("_bcm_ptp_freq_corr_set() - Error in updating frequency correction");
                  }
                  g_apts_synce_freq_corr_ppb = 0;
              }
        }
    }

    if ((!g_apts_servo_assist_gps) &&
        (old_op_mode != g_apts_current_op_mode)) {
        if ((old_op_mode != bcmPtpClockAptsModeGpsGps) &&
            (g_apts_current_op_mode == bcmPtpClockAptsModeGpsGps)) {
            _bcm_tdpll_output_clock_TS1_1ppsref_enable( unit, stack_id, 1);
        } else {
            /* disable if it was enabled. */
            _bcm_tdpll_output_clock_TS1_1ppsref_enable( unit, stack_id, 0);
        }
    }
    return;
}
    ...
    int offset_nsec)
{
    ...
    int rv = BCM_E_NONE;
#ifndef __KERNEL__
    double offset_nsec_f = (double)offset_nsec;
#else
    int64 offset_nsec_64 = (int64)offset_nsec;
#endif
    ...
    _bcm_ptp_uint32_write(payload, offset_nsec);
    payload_len =  4;

    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num, &portid,
             PTP_MGMTMSG_SET, PTP_MGMTMSG_ID_APTS_GPS_OFFSET,
             payload, payload_len, resp, &resp_len);

    if (g_apts_current_op_mode == bcmPtpClockAptsModeSyncEGps) {
#ifndef __KERNEL__
        rv = _bcm_ptp_servo_phase_offset_set(unit, ptp_id, clock_num, &offset_nsec_f);
#else
        rv = _bcm_ptp_servo_phase_offset_set(unit, ptp_id, clock_num, &offset_nsec_64);
#endif
    }
    ...
    int *offset_nsec)
{
    ...
    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num, &portid,
            PTP_MGMTMSG_GET, PTP_MGMTMSG_ID_APTS_GPS_OFFSET,
            0, 0, resp, &resp_len);

    if (rv == BCM_E_NONE) {
        *offset_nsec = _bcm_ptp_uint32_read(resp);
    }
...
/*
 * Function:
 *      bcm_common_ptp_clock_apts_input_source_set
 * Purpose:
 ...
 *      ptp_id           - (IN)  PTP stack ID.
 *      clock_num        - (IN)  PTP clock number.
 *      apts_source      - (IN)  apts source type
 *      input_source_config - (IN)  apts source config
 * Returns:
 *      BCM_E_XXX - Function status.
 * Notes:
 */
    ...
    bcm_ptp_clock_apts_input_source_config_t *input_source_config)
{
    ...
    bcm_ptp_port_identity_t portid;
    uint8 payload[PTP_MGMTMSG_PAYLOAD_APTS_INPUT_SOURCE_CONFIGS_SIZE_OCTETS] = {0};
    uint16 payload_len = 0;
    int i = 0;
    uint8 resp[PTP_MGMTMSG_RESP_MAX_SIZE_OCTETS];
    int resp_len = PTP_MGMTMSG_RESP_MAX_SIZE_OCTETS;
    ...
    if (input_source_config == NULL) {
        PTP_ERROR_FUNC("APTS input source config not found.\n");
        rv =  _SHR_E_CONFIG;
        return rv;
    }

    if (apts_source != bcmPtpClockAptsSourceGps){
        rv = BCM_E_PARAM;
        PTP_ERROR_FUNC("Invalid APTS source\n");
        return rv;
    }
     ...
     *    Octet 0...2     : Custom management message key/identifier.
     *                      Octet 0= 'B'; Octet 1= 'C'; Octet 2= 'M'.
     *    Octet 3...7     : apts source.
     *    Octet 8...11    : tod_in_source.
     *    Octet 12...15   : num_pin_configs.
     *    # num_pin_configs
     *    Octet 16...20   : signal_frequency.
     *    Octet 21...24   : ts_event_frequency.
     *    Octet 25...28   : input_pin.
     */
    ...
    _bcm_ptp_uint32_write(payload+i, apts_source);
    i += sizeof(uint32);

    _bcm_ptp_uint32_write(payload+i, input_source_config->tod_in_source);
    i += sizeof(uint32);

    _bcm_ptp_uint32_write(payload+i, input_source_config->num_pin_configs);
    i += sizeof(uint32);

    if (bcmPtpClockAptsSourceGps == apts_source) {
        if (2 == input_source_config->num_pin_configs) {
            g_apts_servo_assist_gps = 1;
        } else if (1 == input_source_config->num_pin_configs) {
            g_apts_servo_assist_gps = 0;
        }
    }

    LOG_INFO(BSL_LS_BCM_PTP,
                (BSL_META_U(unit, "\n aptsEn:%d srv_asst_gps:%d apts_src:%d\n"),
                g_apts_enabled, g_apts_servo_assist_gps, apts_source));

    for (tmp = 0; tmp < input_source_config->num_pin_configs; tmp++)
    {
        _bcm_ptp_uint32_write(payload+i, input_source_config->pin_config[tmp].signal_frequency);
        i += sizeof(uint32);
        _bcm_ptp_uint32_write(payload+i, input_source_config->pin_config[tmp].ts_event_frequency);
        i += sizeof(uint32);
        _bcm_ptp_uint32_write(payload+i, input_source_config->pin_config[tmp].input_pin);
        i += sizeof(uint32);
    }

    payload_len =  i;
    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num, &portid,
             PTP_MGMTMSG_SET, PTP_MGMTMSG_ID_APTS_SOURCE_INPUT_PIN_CONFIGS,
             payload, payload_len, resp, &resp_len);

    if ((g_apts_enabled) && (!g_apts_servo_assist_gps)) {
       _bcm_tdpll_output_clock_TS1_1ppsref_create(unit, ptp_id, clock_num,
               input_source_config->pin_config[0].input_pin);
    }
...
/*
 * Function:
 *      bcm_common_ptp_clock_apts_input_source_get
 * Purpose:
    ...
    bcm_ptp_clock_apts_input_source_config_t *input_source_config)
{
    ...
    if (input_source_config == NULL) {
        PTP_ERROR_FUNC("APTS input source config memory not found.\n");
        rv =  _SHR_E_CONFIG;
        return rv;
    }
    ...
    payload_len =  i;
    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num, &portid,
             PTP_MGMTMSG_GET, PTP_MGMTMSG_ID_APTS_SOURCE_INPUT_PIN_CONFIGS,
             payload, payload_len, resp, &resp_len);
    ...
    if (rv == BCM_E_NONE) {
         ...
         * Parse Response.
         * BCM Header.
         * Octet 0...3   : TOD in source.
         * Octet 4...7   : Num of PIN configs.
         * #num_pin_configs
         * Octet 08...11   : signal_frequency.
         * Octet 12...15   : ts_event_frequency.
         * Octet 16...19   : input_pin.
         */
         i=0;
         i += 6; /* Shipping BCM hdr */

         input_source_config->tod_in_source =  _bcm_ptp_uint32_read(resp+i);
         i += sizeof(uint32);

         input_source_config->num_pin_configs =  _bcm_ptp_uint32_read(resp+i);
         i += sizeof(uint32);

         for (tmp = 0; tmp < input_source_config->num_pin_configs; tmp++)
         {
            input_source_config->pin_config[tmp].signal_frequency =  _bcm_ptp_uint32_read(resp+i);
            i += sizeof(uint32);
            input_source_config->pin_config[tmp].ts_event_frequency = _bcm_ptp_uint32_read(resp+i);
            i += sizeof(uint32);
            input_source_config->pin_config[tmp].input_pin = _bcm_ptp_uint32_read(resp+i);
            i += sizeof(uint32);
         }
    }
...
int
_bcm_ptp_apts_apply_servo_issued_freq_corr() {
    if (!g_apts_enabled) {
        return BCM_E_NONE;
    } else if ((g_apts_current_op_mode != bcmPtpClockAptsModeGpsGps) &&
             (g_apts_current_op_mode != bcmPtpClockAptsModePtpPtp)){
        return BCM_E_DISABLED;
    } else if ((g_apts_current_op_mode == bcmPtpClockAptsModeGpsGps) &&
               (!g_apts_servo_assist_gps)) {
