...
#define REFCLOCK_L1_RECOVRD_PRI      (6)
#define REFCLOCK_L1_RECOVRD_SEC      (7)
#define REFCLOCK_1588                (8)
...
extern int _bcm_esw_gdpll_LoopFilter_set(int unit, int chan, int64 loop_filter);
#endif
...
int apts_gdpll_gps1pps_ts1 = -1;
#endif


#if defined(BCM_MONTEREY_SUPPORT) && defined(INCLUDE_GDPLL)
void gdpll_chan_bandwidth_dependent_config(int unit, bcm_gdpll_chan_t *pGdpllChan, int outClock, bcm_tdpll_dpll_bandwidth_t *bw, int reference_clock_idx)
{
    uint8 bw_index = 255;
    ...
    cli_out("outClock=%d, bw_index = %d reference_clock_idx = %d\n", outClock, bw_index, reference_clock_idx);

    if ((outClock == OUTCLOCK_BS0) ||
        ((reference_clock_idx ==  REFCLOCK_1588) && (outClock == OUTCLOCK_BS1))) {
        /* Stage 0 */
    ...
    } else if ((outClock == OUTCLOCK_BS1) && ((reference_clock_idx != REFCLOCK_1588))){
        /* Stage 0 */
    ...
    } else if ((outClock == OUTCLOCK_SYNCE) && (reference_clock_idx == REFCLOCK_1588)) {
        /* Stage 0 */
        pGdpllChan->chan_dpll_config.k1[0] = 887520630;
        pGdpllChan->chan_dpll_config.k1Shift[0] = 16;
        pGdpllChan->chan_dpll_config.k1k2[0] = 862352755;
        pGdpllChan->chan_dpll_config.k1k2Shift[0] = 7;
...
void gdpll_chan_config(int unit, bcm_gdpll_chan_t *pGdpllChan, int outClock, bcm_tdpll_dpll_bandwidth_t *bw, int reference_clock_idx)
{
    if (reference_clock_idx == REFCLOCK_1588 ) {
        bsl_printf("ERROR !! gdpll_chan_config : called with refrence clock 1588 !\n");
        return;
    }
    gdpll_chan_bandwidth_dependent_config(unit, pGdpllChan, outClock, bw, reference_clock_idx);
...
void gdpll_chan_config_with_1588_ref(int unit, bcm_gdpll_chan_t *pGdpllChan, int outClock, bcm_tdpll_dpll_bandwidth_t *bw, int reference_clock_idx)
{
    gdpll_chan_bandwidth_dependent_config(unit, pGdpllChan, outClock, bw, reference_clock_idx);

    if (reference_clock_idx != REFCLOCK_1588) {
        LOG_ERROR(BSL_LS_BCM_PTP,
                    (BSL_META_U(0, "gdpll_chan_config_with_1588_ref: Called for non 1588 ref DPLLs outClock %d reference_clock_idx %d \n"),
                    outClock, reference_clock_idx));
        return;
    }
    ...
    if (outClock == OUTCLOCK_SYNCE) {
        ...
        pGdpllChan->chan_dpll_config.dwell_count[0]=350;
        pGdpllChan->chan_dpll_config.dwell_count[1]=0;
        pGdpllChan->chan_dpll_config.dwell_count[2]=0;
        u64_H(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[0])=0;
        u64_L(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[0])=39000;
        u64_H(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[1])=0;
        u64_L(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[1])=0;
        u64_H(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[2])=0;
        u64_L(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[2])=0;
        ...
        pGdpllChan->chan_dpll_config.norm_phase_error=1;
        /* Phase error thresholds are in ns */
        pGdpllChan->chan_dpll_config.phase_error_thres0=12800000;
        pGdpllChan->chan_dpll_config.phase_error_thres1=507;
        pGdpllChan->chan_dpll_config.holdover_filter_coeff=512;
        ...
        u64_H(pGdpllChan->chan_dpll_state.loop_filter)=0;
        u64_L(pGdpllChan->chan_dpll_state.loop_filter)=0;
        pGdpllChan->chan_dpll_state.dwell_counter=350;
        pGdpllChan->chan_dpll_state.lockDetFilter=64000;
        u64_H(pGdpllChan->chan_dpll_state.offset)=0;
        u64_L(pGdpllChan->chan_dpll_state.offset)=0;
        pGdpllChan->chan_dpll_state.init_flag=0;
    ...
    } else if ((outClock == OUTCLOCK_BS0) || (outClock == OUTCLOCK_BS1)) {
        /* DPLL configs */
        pGdpllChan->chan_dpll_config.lockDetThres[0]=38000;
        pGdpllChan->chan_dpll_config.lockDetThres[1]=0;
        pGdpllChan->chan_dpll_config.lockDetThres[2]=0;
        ...
        LOG_ERROR(BSL_LS_BCM_PTP,
                    (BSL_META_U(0, "gdpll_chan_config_with_1588_ref: Unknown DPLL config req outClock %d reference_clock_idx %d \n"),
                    outClock, reference_clock_idx));
    }
    return;
}
        ...
        int reference_clock_idx)
{
    ...
    gdpll_chan_config(unit, &gdpll_chan, OUTCLOCK_SYNCE, bandwidth, reference_clock_idx);
...
int _bcm_tdpll_output_clock_synce_gdpll_create_with_1588_ref(int unit,
        int clock_index,
        bcm_port_t port,
        uint32 event_div,
        bcm_tdpll_dpll_bandwidth_t *bandwidth,
        int reference_clock_idx)
{
    ...
    /* Channel bw related config */
    gdpll_chan_config_with_1588_ref (unit, &gdpll_chan, OUTCLOCK_SYNCE, bandwidth, reference_clock_idx);
         ...
         int reference_clock_idx)
{
    ...
    if (reference_clock_idx == REFCLOCK_1588) {
        rv = _bcm_tdpll_output_clock_synce_gdpll_create_with_1588_ref(unit, clock_index, port, event_div, bandwidth, reference_clock_idx);
    }
    else {
        rv = _bcm_tdpll_output_clock_synce_gdpll_create(unit, clock_index, port, event_div, bandwidth, reference_clock_idx);
    }
    if (rv != BCM_E_NONE) {
         ...
         int reference_clock_idx)
{
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        if(!IS_CPRI_PORT(unit, port)) {
            rv = _bcm_common_tdpll_output_clock_portattach(unit, clock_index, port, bandwidth, reference_clock_idx);
            if (rv != BCM_E_NONE) {
...
int
_bcm_tdpll_output_clock_dpll_reference_get(
    int unit,
    int stack_id,
    int clock_index,
    bcm_pbmp_t pbmp,
    int *reference_clock_idx)
{
    int rv = BCM_E_NONE, num_dplls, dpll_index = 0;
    int dpll_refs[TDPLL_DPLL_INSTANCE_NUM_MAX];

    bcm_tdpll_dpll_bindings_t bindings;

    /* DPLL algo is choosen based upon refernce clock, mostly active reference
     * choosen. if active reference is not available, then configured input
     * clock based slection is done.
     */
    *reference_clock_idx = -1;

    rv = bcm_tdpll_dpll_reference_get(unit, 0, TDPLL_DPLL_INSTANCE_NUM_MAX, dpll_refs, &num_dplls);
    if (rv != BCM_E_NONE) {
        PTP_ERROR_FUNC("_bcm_tdpll_output_clock_reference_get ()");
        return rv;
    }

    /* Findout TDPLL outclock to DPLL instance bindings..*/

    for(dpll_index = 0; dpll_index < TDPLL_DPLL_INSTANCE_NUM_MAX; dpll_index++) {

        memset(&bindings, 0x0, sizeof (bcm_tdpll_dpll_bindings_t));

        rv = bcm_tdpll_dpll_bindings_get(unit, 0, dpll_index, &bindings);

        if ((bindings.output_clocks[clock_index/SHR_BITWID] & (1<<clock_index))) {
            break;
        }
    }

    if (dpll_index != TDPLL_DPLL_INSTANCE_NUM_MAX) {
        if (dpll_refs[dpll_index] >= 0 ) {
            *reference_clock_idx = dpll_refs[dpll_index];
        } else {
            if (bindings.input_clocks[0] & (1 << REFCLOCK_L1_RECOVRD_PRI) ) {
                *reference_clock_idx = REFCLOCK_L1_RECOVRD_PRI;
            } else if (bindings.input_clocks[0] & (1 << REFCLOCK_L1_RECOVRD_SEC) ) {
                *reference_clock_idx = REFCLOCK_L1_RECOVRD_SEC;
            } else if (bindings.input_clocks[0] & (1 << REFCLOCK_1588)) {
                *reference_clock_idx = REFCLOCK_1588;
            }
        }
    }
    return  *reference_clock_idx;
}
    ...
    bcm_tdpll_dpll_bandwidth_t *bandwidth)
{
    ...
    uint32 regVal=0;
    int reference_clock_idx = 0;

    SET_PTP_INFO;
    stack_p = &ptp_info_p->stack_info[stack_id];

    _bcm_tdpll_output_clock_dpll_reference_get(unit, stack_id, clock_index, pbmp, &reference_clock_idx);
    if ((reference_clock_idx < TDPLL_INPUT_CLOCK_NUM_GPIO) || (reference_clock_idx >= TDPLL_INPUT_CLOCK_NUM_MAX)) {
        LOG_ERROR(BSL_LS_BCM_PTP,
                        (BSL_META_U(unit,
                            " ### Invalid reference_clock_idx : %d \n"), reference_clock_idx));
    ...
    if (clock_index < BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC) {
        ...
        if (clock_index == 0) {
           ...
           if (reference_clock_idx ==  REFCLOCK_1588) {
                gdpll_chan_config_with_1588_ref (unit, &gdpll_chan, OUTCLOCK_BS0, bandwidth, reference_clock_idx);
           } else {
                gdpll_chan_config(unit, &gdpll_chan, OUTCLOCK_BS0, bandwidth, reference_clock_idx);
            }
        } else {
            /* BS1 DPLL */
            if (reference_clock_idx ==  REFCLOCK_1588) {
                /* lock BS1 output clock to TS1 */
               ...
               gdpll_chan_config_with_1588_ref (unit, &gdpll_chan, OUTCLOCK_BS1, bandwidth, reference_clock_idx);
            } else {
                /* BS1 output clock lock to SYNCE/CDR clock*/
                gdpll_chan.event_fb.input_event     = bcmGdpllInputEventBS1PLL;
                gdpll_chan.event_fb.ts_counter      = 0 /* TS0 */;
                ...
                gdpll_chan_config(unit, &gdpll_chan, OUTCLOCK_BS1, bandwidth, reference_clock_idx);
            }
        }

        if ((clock_index == 0) || ((reference_clock_idx ==  REFCLOCK_1588) && (clock_index == 1))
            ) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_PTP,
                    (BSL_META_U(unit,
                                " BS%d synth chan create : CurrentTS[%llu/0x%llx] phase_counter[%llu/0x%llx] start_delay[%u]\n"),
                                clock_index, (long long unsigned int)counter.ts_counter_ns, (long long unsigned int)counter.ts_counter_ns,
        ...
        _bcm_tdpll_output_clock_synce_configure(unit, clock_index, pbmp, bandwidth, reference_clock_idx);
        ...
        gdpll_chan_config(unit, &gdpll_chan, OUTCLOCK_1588, bandwidth, reference_clock_idx);
...
int
_bcm_tdpll_output_clock_TS1_1ppsref_create(
    int unit,
    int stack_id,
    int clock_index,
    bcm_ptp_clock_input_pin_t ref_input_pin)
{
#if defined(BCM_MONTEREY_SUPPORT) && defined(INCLUDE_GDPLL)
    int result=0, gdpllChan=0;
    int rv = BCM_E_NONE;
    ...
    _bcm_ptp_stack_info_t *stack_p;
    _bcm_ptp_info_t *ptp_info_p;
    bcm_gdpll_input_event_t ref_event;
    ...
    if (ref_input_pin == bcm_ptp_pin_bs0_hb) {
        ref_event = bcmGdpllInputEventBS0HB;
    } else {
        ref_event = (ref_input_pin + bcmGdpllInputEventGPIO0);
    }

    /* Reference config */
    gdpll_chan.event_fb.input_event    = ref_event;
    gdpll_chan.event_fb.event_dest     = bcmGdpllEventDestALL;
    gdpll_chan.event_fb.event_divisor  = 0;
    gdpll_chan.event_fb.ts_counter = 1; /* TS1 */
    ...
    pGdpllChan = &gdpll_chan;

    /* Stage 0 */
    pGdpllChan->chan_dpll_config.k1[0] = 2882304;
    pGdpllChan->chan_dpll_config.k1Shift[0] = 4;
    pGdpllChan->chan_dpll_config.k1k2[0] = 3689349;
    pGdpllChan->chan_dpll_config.k1k2Shift[0] = 0;

    /* Stage 1 */
    pGdpllChan->chan_dpll_config.k1[1] = 3390946;
    pGdpllChan->chan_dpll_config.k1Shift[1] = 3;
    pGdpllChan->chan_dpll_config.k1k2[1] = 9973;
    pGdpllChan->chan_dpll_config.k1k2Shift[1] = 5;
    ...
    u64_H(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[0])=0x2;
    u64_L(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[0])=0x540BE400;
    u64_H(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[1])=0x2;
    u64_L(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[1])=0x540BE400;
    u64_H(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[2])=0;
    u64_L(pGdpllChan->chan_dpll_config.phase_error_limiter_thres[2])=0;
    ...
    pGdpllChan->chan_dpll_config.lockIndicatorThresholdLo=650;
    ...
    result = bcm_gdpll_chan_create (unit, flags, &gdpll_chan, &gdpllChan);
    if (BCM_FAILURE(result)) {
        PTP_ERROR_FUNC("bcm_gdpll_chan_create() Error ");
    } else {
        apts_gdpll_gps1pps_ts1 = gdpllChan;
        LOG_INFO(BSL_LS_BCM_PTP,
                    (BSL_META_U(0, "gdpll_chan:%d created for GPS 1pps->TS1\n"),
                    gdpllChan));
    }
...
int
_bcm_tdpll_output_clock_TS1_1ppsref_enable(
    int unit,
    int stack_id,
    int enable)
{
#if defined(BCM_MONTEREY_SUPPORT) && defined(INCLUDE_GDPLL)
    int rv = BCM_E_NONE;
    int static pps_refchan_enabled = 0;
    int curr_ptp_freq_corr_ppt = 0, curr_ptp_freq_offset_ppt = 0;
    int32_t lf_hi=0, lf_lo=0;
    uint64_t curr_loop_filter=0;

    LOG_INFO(BSL_LS_BCM_PTP,
                (BSL_META_U(0, "APTS: GDPLL ch[%d] enabled[%d] enable[%d]\n"),
                apts_gdpll_gps1pps_ts1, pps_refchan_enabled, enable));

    if (apts_gdpll_gps1pps_ts1 == -1) {
        return BCM_E_INIT;
    }

    if (enable) {
        if (BCM_FAILURE(rv = _bcm_ptp_freq_corr_get(unit, stack_id,
                                 PTP_CLOCK_NUMBER_DEFAULT,
                                 &curr_ptp_freq_corr_ppt,
                                 &curr_ptp_freq_offset_ppt,
                                 &lf_hi,
                                 &lf_lo))) {
            PTP_ERROR_FUNC("_bcm_ptp_freq_corr_get() Error ");
        }
        u64_H(curr_loop_filter)=lf_hi;
        u64_L(curr_loop_filter)=lf_lo;

        LOG_INFO(BSL_LS_BCM_PTP,
                (BSL_META_U(0, "APTS: LF[0x%016llx]\n"),
                (long long unsigned int)curr_loop_filter));

        _bcm_esw_gdpll_LoopFilter_set(unit, apts_gdpll_gps1pps_ts1, curr_loop_filter);

        LOG_INFO(BSL_LS_BCM_PTP,
                (BSL_META_U(0, "APTS: Enabling PBO gdpll_chan:%d\n"),
                apts_gdpll_gps1pps_ts1));

        _bcm_esw_gdpll_pbo_set(unit, apts_gdpll_gps1pps_ts1);
    }

    rv = bcm_gdpll_chan_enable(unit, apts_gdpll_gps1pps_ts1, enable);
    if (BCM_FAILURE(rv)) {
        PTP_ERROR_FUNC("bcm_gdpll_chan_enable() Error ");
        return rv;
    }
    ...
    uint32 tsevent_frequency)
{
     ...
     */
     tdpll_frequency = BCM_TDPLL_FREQUENCY;
     if (SOC_IS_FIRELIGHT(unit) && (tdpll_frequency != 100)) {
        tdpll_frequency =  100;
     }
    if ((0 == tsevent_frequency) ||
        (!SOC_IS_MONTEREY(unit) && (tsevent_frequency % tdpll_frequency))) {
        LOG_VERBOSE(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
                                "TS EVENT frequency is not a multiple of %u Hz. fTS: %u\n"),
                     (unsigned)tdpll_frequency, (unsigned)tsevent_frequency));
        return BCM_E_PARAM;
    }
