...
bcm_tunnel_initiator_t_init(bcm_tunnel_initiator_t *tunnel_init)
{
    if (tunnel_init != NULL) {
        ...
        tunnel_init->estimated_encap_size = -1;
#endif
...
/**
 * \brief - Initialize a ppp terminator object struct.
 *
 * \par DIRECT_INPUT:
 *   \param [in] info - pointer to the ppp terminator object struct.
 *
 * \return
 *    NONE
 *
 ...
 */
void
bcm_ppp_terminator_t_init(bcm_ppp_terminator_t *info)
{
    if (info != NULL) {
...
/**
 * \brief - Initialize a ppp initiator object struct.
 *
 * \par DIRECT_INPUT:
 *   \param [in] info - pointer to the ppp initiator object struct.
 *
 * \return
 *    NONE
 *
 ...
 */
void
bcm_ppp_initiator_t_init(bcm_ppp_initiator_t *info)
{
    if (info != NULL) {
        sal_memset(info, 0, sizeof(bcm_ppp_initiator_t));
    }
    return;
}

/*!
 * \brief Initialize bcm_l3_mtu_adjustment_t structure.
 *
 * \param [in] info L3 MTU adjustment information.
 */
void
bcm_l3_mtu_adjustment_profile_t_init(bcm_l3_mtu_adjustment_profile_t *adjustment_profile)
{
    if (adjustment_profile) {
        sal_memset(adjustment_profile, 0, sizeof(bcm_l3_mtu_adjustment_profile_t));
    }
}
