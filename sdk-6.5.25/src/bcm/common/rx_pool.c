...
#if !defined(BCM_PER_UNIT_RX_POOL_SUPPORT)
/****************************************************************
 *
 * RX POOL CODE:
 *
...
#endif /* BCM_RXP_DEBUG */
#else
int bcm_rx_pool_setup(
    int pkt_count,
    int bytes_per_pkt)
{
    LOG_ERROR(BSL_LS_BCM_COMMON,
        (BSL_META("Not supported: Per unit pool allocation is defined\n")));
    return BCM_E_UNAVAIL;
}

int bcm_rx_pool_setup_done(void)
{
    LOG_ERROR(BSL_LS_BCM_COMMON,
        (BSL_META("Not supported: Per unit pool allocation is defined\n")));
    return BCM_E_UNAVAIL;
