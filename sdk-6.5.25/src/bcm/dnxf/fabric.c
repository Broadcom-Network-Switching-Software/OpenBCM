    ...
    int unit)
{
    ...
    {
        SOC_INFO(unit).port_speed_max[i] = dnxf_data_port.general.speed_max_get(unit);
    }
    ...
    uint32 *data_in)
{
...
#ifdef ADAPTER_SERVER_MODE
    {
        ...
        SHR_IF_ERR_EXIT(adapter_send_buffer
                        (unit, (uint32) ms_id, src_port, 0, WORDS2BYTES(data_in_size), (unsigned char *) packet_data,
                         nof_signals));
    }
#else
    {
