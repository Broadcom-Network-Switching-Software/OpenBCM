...
int
_bcm_dpp_in_lif_mpls_match_add(int unit, bcm_mpls_port_t *mpls_port, int lif, int allocate_outlif, int gport_is_fec)
{
    _bcm_dpp_gport_sw_resources gport_sw_resources;
...
{    
    int rv = BCM_E_NONE;
    int i,j;
    uint32 dscp_preserve_with_extend_label = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "dscp_preserve_with_extend_label", 0);
    ...
    if(SOC_IS_JERICHO_PLUS(unit)) {
        ...
        if (dscp_preserve_with_extend_label > 0) {
            i = 4;
        ...
        for (i = 1; i <= 4; i++) {
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_ADDITIONAL_LABEL,
                                                                           ...
                                                                           1 /*value*/,i/*outlif-profile*/);
            BCMDNX_IF_ERR_EXIT(rv);
            if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "critical_extended_mpls_program_match", 0))
            {
                rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_ADDITIONAL_LABEL,
                                                                               ARAD_EGR_PROG_EDITOR_PROG_MPLS_EXTENDED_JER, i+4/*entry*/,
                                                                               1 /*value*/,i/*outlif-profile*/);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
        ...
        if (dscp_preserve_with_extend_label > 0) {
            /*
             * if dscp_preserve_with_extend_label > 1 need to enable for out_lif_idx = 2/1
             */
            if (dscp_preserve_with_extend_label > 1) {
                for (i = 2; i >= 1; i--) {
                    for (j = 0; j <= 1; j++) {
                        rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_ADDITIONAL_LABEL,
                                                                                       ARAD_EGR_PROG_EDITOR_PROG_PRESERVING_DSCP_EXT_LABEL_JER, (j*2+1)/*entry*/,
                                                                                       1 /*value*/,i/*outlif-profile*/);
                        BCMDNX_IF_ERR_EXIT(rv);
                        rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_ADDITIONAL_LABEL,
                                                                                       ARAD_EGR_PROG_EDITOR_PROG_PRESERVING_DSCP_EXT_LABEL_JER_OAMTS, (j*2+1)/*entry*/,
                                                                                       1 /*value*/,i/*outlif-profile*/);
                        BCMDNX_IF_ERR_EXIT(rv);
                        rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_ADDITIONAL_LABEL,
                                                                                       ARAD_EGR_PROG_EDITOR_PROG_PRESERVING_DSCP_EXT_LABEL_IPV6_JER, (j*2+1)/*entry*/,
                                                                                       1 /*value*/,i/*outlif-profile*/);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
    ...
    int unit)
{
    ...
    if ((SOC_IS_JERICHO_AND_BELOW(unit) || soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "el_eli_jr_mode", 0)) 
        ...
        {
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_INDICATION,
                                                                              ARAD_EGR_PROG_EDITOR_PROG_MPLS_ELI_JER, 1/*entry*/,
                                                                              1 /*value*/,3/*outlif-profile3*/);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_INDICATION,
                                                                              ARAD_EGR_PROG_EDITOR_PROG_MPLS_ELI_JER, 2/*entry*/,
                                                                              1 /*value*/,4/*outlif-profile4*/);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_INDICATION,
                                                                              ARAD_EGR_PROG_EDITOR_PROG_MPLS_ELI_JER, 3/*entry*/,
                                                                              1 /*value*/,3/*outlif-profile3*/);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_INDICATION,
 ...
 *                                      In this case, if with_id is not set, a different profile might be allocated.
    ...
    uint8 is_second_mpls_label_in_entry)
{
    ...
    } else if (update_existing_profile) {
        tmp_push_profile = (int)(*push_profile);
    }
    ...
    ) /* needed only in case intf is of type EEP*/
{
    ...
    for (start_indx = num_labels; start_indx > 0; start_indx = start_indx - 1 - skip_entries) {
        ...
        if (label_action_type == _bcm_dpp_mpls_egress_action_type_swap) { /* if swap */
            ...
            if (SOC_IS_JERICHO(unit)) {
                if (is_egress_protection) {
                    DPP_FAILOVER_ID_GET(mpls_swap_info.protection_info.protection_pointer, label_array[indx].egress_failover_id);
                    mpls_swap_info.protection_info.protection_pass_value = (label_array[indx].egress_failover_if_id) ? 0 : 1;
                    mpls_swap_info.protection_info.is_protection_valid = TRUE;
                }
            }
...
/*
 * Get the entry additional info (pretection, wide-data, etc) for the initiator.
 */
STATIC
int _bcm_petra_mpls_tunnel_initiator_additional_get(
    int unit,
    int global_lif_id,
...
{
    int rv = BCM_E_NONE;
    uint8 is_egress_protection, is_wide_entry = 0, ext_type;
    int local_lif_id = 0;
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (is_egress_protection && protection_info.is_protection_valid) {

            /* Set the protection information */
            DPP_FAILOVER_TYPE_SET(label_array[label_count].egress_failover_id,
                                  protection_info.protection_pointer, DPP_FAILOVER_TYPE_ENCAP);
            label_array[label_count].egress_failover_if_id = (protection_info.protection_pass_value) ? 0 : 1;
            ...
            if (encap_entry_info[indx].entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP) {  /* mpls encap */
                ...
                for (cur_indx = 0; cur_indx < cur_count; ++cur_indx) {
                    ...
                    if (SOC_IS_JERICHO(unit)) {
                        ...
                        /* Get the initiator additional info: protection, wide-data, etc.*/
                        rv = _bcm_petra_mpls_tunnel_initiator_additional_get(unit, global_lif_id, encap_entry_info[indx].entry_val.mpls_encap_info.protection_info, label_array, *label_count);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                ...
                /* Get the initiator additional info: protection, wide-data, etc.*/
                rv = _bcm_petra_mpls_tunnel_initiator_additional_get(unit, global_lif_id, encap_entry_info[indx].entry_val.swap_info.protection_info, label_array, *label_count);
                BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    is_update = (info->flags & BCM_MPLS_SWITCH_REPLACE) ? TRUE : FALSE;
    soc_sand_rv = soc_ppd_frwrd_ilm_get(unit, &ilm_key, &ilm_val_get, &found);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    if (found && !is_update) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("The IML entry exists already, but _REPLACE flag is not set!")));
    }
    ...
    if (info->action == BCM_MPLS_SWITCH_ACTION_SWAP) {
      ...
      /* make swap */
        ...
        ilm_val.additional_info.eei.type = SOC_PPC_EEI_TYPE_MPLS;
        ilm_val.additional_info.eei.val.mpls_command.command = SOC_PPC_MPLS_COMMAND_TYPE_PUSH;
        if (is_update) {
            ilm_val.additional_info.eei.val.mpls_command.push_profile =
                ilm_val_get.additional_info.eei.val.mpls_command.push_profile;
        } else {
            ilm_val.additional_info.eei.val.mpls_command.push_profile = 0;
        }
        rv = _bcm_petra_mpls_tunnel_switch_egress_label_to_mpls_command(unit, &(info->egress_label),
                                        &(ilm_val.additional_info.eei.val.mpls_command), is_update);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    int unit)
{
    ...
    for (push_profile = 1; push_profile < NOF_MPLS_PUSH_PROFILE(unit); push_profile++) {
        int is_allocated = 0;
        int is_last = FALSE;
        if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && _BCM_DPP_MPLS_IS_FORBIDDEN_PUSH_PROFILE(push_profile)) {
            /** In case the profiles are dummy, skip them.*/
            continue;
        }
...
int _bcm_dpp_mpls_port_pwe_set(
    ...
    */
    ...
    if (with_id) {
        ...
        rv = _bcm_dpp_gport_l2_intf_calculate_port_id(unit, fec_id, global_lif_id, mpls_port, BCM_GPORT_MPLS_PORT, &gport_is_fec);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (!remote_lif && is_ingress) {
        ...
        if (_BCM_DDP_IS_MP_VPN(vpn) && !is_gal) {
          ...
          rv = _bcm_dpp_in_lif_mpls_match_add(unit, mpls_port, local_in_lif, update ? (old_mpls_port_local_out_lif != -1) : eep_required, gport_is_fec);
          BCMDNX_IF_ERR_EXIT(rv);
