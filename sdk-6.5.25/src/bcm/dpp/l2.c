...
/*****************************************************************************
* Function:  bcm_petra_init_cpu_learn_thread_info
* Purpose:   Initiate the cpu learning thread information for the current unit
* Params:
* unit:      Device Number
* remark:    The thread is supported only for Jericho devices and above
* Return:    (void)
*******************************************************************************/
STATIC void bcm_jer_init_cpu_learn_thread_info(int unit)
{
    if (SOC_IS_JERICHO(unit))
    {
        jer_mact_fifo_terminate[unit] = TRUE;
        jer_mact_fifo_sem[unit] = 0;
        jer_mact_fifo_tid[unit] = 0;
    }
}
...
* old_trap_deallocated - In old PPC_TRAP_CODE was deallocated by template manager
...
STATIC int _bcm_dpp_update_virtual_trap_if_needed(int unit,
                ...
                SOC_PPC_TRAP_SNOOP_ACTION_PROFILE_INFO *snoop_info,
                int new_trap_allocated, int old_trap_dealloced, int virtual_trap_was_set,SOC_PPC_TRAP_CODE ppc_trap_code,
                int is_for_delete,bcm_dpp_rx_virtual_traps_t *virtual_traps_p)
{
    ...
    if(virtual_trap_was_set == FALSE)
    {
        ...
        } else if (old_trap_dealloced && is_for_delete) {
            rv = _bcm_dpp_rx_trap_sw_id_to_virtual_id(unit,ppc_trap_code,&trap_id_sw);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = _bcm_dpp_am_trap_dealloc(unit,0,trap_id_sw,0);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
    ...
    /*is reserved MC and port, for reserved MC profile has to be -1 */
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) 
        {
            ...
            if(is_for_delete == TRUE)
            {
                ...
                action_info.trap_code = SOC_PPC_TRAP_CODE_RESERVED_MC_0 + (old_trap_code-1);
            } else {
                action_info.trap_code = SOC_PPC_TRAP_CODE_RESERVED_MC_0 + (new_trap_code-1);
            }
            rv = _bcm_dpp_update_virtual_trap_if_needed(unit,&_bcm_trap_info,&snoop_info,
                                                    trap_is_first,trap_is_last,is_virtual_traps_p_set,action_info.trap_code,
                  ...
                  int unit)
{
    ...
    */
    ...
    /* init l2 dma cpu learning thread info */
    bcm_jer_init_cpu_learn_thread_info(unit);
