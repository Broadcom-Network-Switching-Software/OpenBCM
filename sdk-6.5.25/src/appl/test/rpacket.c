...
bind_device(int sock, char* dev)
{
    ...
    sal_strlcpy(req.ifr_name, dev, sizeof(req.ifr_name));
...
{  
    ...
    if ((p->p_use_socket) && (p->p_ringbuf_size > 0)) {
        ...
        if (MAP_FAILED == p->p_ringbuf) {
            ...
            sal_memset(p->p_ringbuf, 0, buf_size);
        }
    }
...
knetif_clean(int unit, p_t *p) 
{
    ...
    if (p->p_sock > 0) {
        ...
        sal_strlcpy(req.ifr_name, netif.name, sizeof(req.ifr_name));
...
"  PortFromParser=<value>       : Indicate if set received port per the result of RX parser. (default=FALSE)\n"
...
rpacket_init(int u, args_t *a, void **pa)
{
    ...
    if (!p->p_init) {                   /* Init defaults first time */
       ...
       /*
        * Currently, Flag BCM_RX_F_TAKE_PORT_FROM_PARSER is dedicated for JR2 and above device
        * Set it as default when many ports are mapped to same DMA channel
        */
#if defined(BCM_DNX_SUPPORT)
       if (SOC_IS_DNX(u)) {
            ...
            p->p_rx_cfg.chan_cfg[1].flags = 0;
            rv = bcm_port_config_get(u, &port_config);
            if (rv != BCM_E_NONE) {
                return -1;
            }
            BCM_PBMP_COUNT(port_config.cpu, nof_cpu_ports);
            if (nof_cpu_ports > 7) {
                p->p_rx_cfg.chan_cfg[1].flags |= BCM_RX_F_TAKE_PORT_FROM_PARSER;
            }
       }
#endif
       p->p_rx_cfg.chan_cfg[1].cos_bmp = 0xff;
       p->rx_mode = 0;
       p->take_port_from_parser = 0;
#ifdef INCLUDE_PKTIO
       p->p_data_buf = NULL;
#endif
...
#endif
    {
        ...
        parse_table_add(&pt, "PortFromParser", PQ_DFL|PQ_INT, 0, &p->take_port_from_parser, 0);
#if (defined(INCLUDE_KNET) && defined(LINUX) && (!defined(__KERNEL__)))
