            ...
            for (k = 0; k < pkt_size; k++) {
                if (packet[k] != exp_packet[k]) {
                    test_error(unit, "\nCorrupt packet received on "
                                        ...
                                        (void *) (pktdma_p->pkt_rx_array[channel][j]));
                       ...
                       bcm_vlan_t xlate_vlan_start)
{
    ...
    if (!port_list) {
        cli_out("\nUnable to allocate memory for port list");
        return;
    }
    i = 0;
    ...
    for (channel = 0; channel < MAX_CHANNELS; channel++) {
        ...
        if (((pktdma_p->tx_bitmap_param >> channel) % 2) != 0) {
            for (i = 0; i < pktdma_p->num_pkts_param_tx; i++) {
                cli_out("%p ", (void *) pktdma_p->pkt_tx_array[channel][i]);
            }
        }
        ...
        if (((pktdma_p->rx_bitmap_param >> channel) % 2) != 0) {
            for (i = 0; i < pktdma_p->num_pkts_param_rx; i++) {
                cli_out("%p ", (void *) pktdma_p->pkt_rx_array[channel][i]);
            }
        }
        cli_out("\n");
    }
                            ...
                            if (pktdma_p->
                                ...
                                cli_out
                                    ("\nMismatch : channel = %0d, "
                                     ...
                                     i, j, k,
                                     (void *) (&pktdma_p->pkt_tx_array[pktdma_p->
                                                             source_ch[i]][j]
                                     [k]), (void *) (&pktdma_p->pkt_rx_array[i][j][k +
                                                        pktdma_p->header_offset]),
        ...
        if (!((dv_cont > 0) && (pktdma_p->cont_dma == 0))) {
            ...
            LOG_INFO(BSL_LS_APPL_TESTS,
                     (BSL_META_U(unit, "\npktdma_p->dv_tx_array[%0d] = %p"),
                      dv_cont, (void *) pktdma_p->dv_tx_array[dv_cont]));
            for (channel = 0; channel < MAX_CHANNELS; channel++) {
                if (((pktdma_p->tx_bitmap_param >> channel) % 2) != 0) {
                    LOG_INFO(BSL_LS_APPL_TESTS,
                             (BSL_META_U
                              ...
                              dv_cont, channel,
                              (void *) pktdma_p->dv_tx_array[dv_cont][channel]));
                    if (pktdma_p->chain_tx == 0) {
                        for (i = 0; i < pktdma_p->num_pkts_param_tx; i++) {
                            LOG_INFO(BSL_LS_APPL_TESTS,
                                     (BSL_META_U
                                      (unit,
                                       ...
                                       "= %p"),
                                      dv_cont, channel, i,
                                      (void *) pktdma_p->dv_tx_array[dv_cont][channel][i]));
                        }
                    } else {
                        LOG_INFO(BSL_LS_APPL_TESTS,
                                 (BSL_META_U
                                  (unit,
                                   ...
                                   "= %p"),
                                  dv_cont, channel, 0,
                                  (void *) pktdma_p->dv_tx_array[dv_cont][channel][0]));
                    }
                }
            }
            ...
            LOG_INFO(BSL_LS_APPL_TESTS,
                     (BSL_META_U(unit, "\npktdma_p->dv_rx_array[%0d] = %p"),
                      dv_cont, (void *) pktdma_p->dv_rx_array[dv_cont]));
            for (channel = 0; channel < MAX_CHANNELS; channel++) {
                if (((pktdma_p->rx_bitmap_param >> channel) % 2) != 0) {
                    LOG_INFO(BSL_LS_APPL_TESTS,
                             (BSL_META_U
                              ...
                              dv_cont, channel,
                              (void *) pktdma_p->dv_rx_array[dv_cont][channel]));
                    if (pktdma_p->chain_rx == 0) {
                        for (i = 0; i < pktdma_p->num_pkts_param_rx; i++) {
                            LOG_INFO(BSL_LS_APPL_TESTS,
                                     (BSL_META_U
                                      (unit,
                                       ...
                                       "= %p"),
                                      dv_cont, channel, i,
                                      (void *) pktdma_p->dv_rx_array[dv_cont][channel][i]));
                        }
                    } else {
                        LOG_INFO(BSL_LS_APPL_TESTS,
                                 (BSL_META_U
                                  (unit,
                                   ...
                                   "= %p"),
                                  dv_cont, channel, 0,
                                  (void *) pktdma_p->dv_rx_array[dv_cont][channel][0]));
                    }
                }
            }
        }
...
pktdma_soc_test(int unit, args_t *a, void *pa)
{
    ...
    cli_out("\npid_tx = %p, pid_rx= %p", (void *) (pktdma_p->pid_tx),
                                         (void *) (pktdma_p->pid_rx));
