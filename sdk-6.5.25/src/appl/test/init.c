...
#ifdef INCLUDE_KBP
/*
 * In case a device with KBP we should use the /tmp folder instead the shared CPU memory,
 * because the shm memory size will not be big enough.
 */
#define INIT_TEST_WARMBOOT_DATA_PATH "/tmp/warmboot_data"
#else /* INCLUDE_KBP */
#define INIT_TEST_WARMBOOT_DATA_PATH "/dev/shm/warmboot_data"
#endif /* INCLUDE_KBP */
#endif /* BCM_WARM_BOOT_SUPPORT */
...
init_deinit_test_internal(int unit, args_t *a, void *p)
{
    ...
    if (NULL != stable_filename) {
        ...
        sal_strncat_s(file_name1, INIT_TEST_WARMBOOT_DATA_PATH, sizeof(file_name1));
        sal_strncat_s(file_name2, INIT_TEST_WARMBOOT_DATA_PATH, sizeof(file_name2));
#else   /* BCM_WARM_BOOT_SUPPORT */
    ...
    if (s < 0)
    {
        cli_out("Error in init_deinit_test_internal(), socket() has failed\n"); 
        rv = BCM_E_INTERNAL; 
        goto done; 
    }
...
#if defined(BCM_PETRA_SUPPORT) && defined(BCM_88675_A0)
...
init_deinit_test(int unit, args_t *a, void *p)
{
    ...
    if (do_error_on_leak_test(init_param)) {
        ...
        /* record original no_packet_rx */
        int original_no_packet_rx = init_param->no_packet_rx;
        ...
        sal_get_alloc_counters_main_thr(&(alloc_bytes_count_on_start),&(free_bytes_count_on_start));

        /* skip RX appl init and following RX appl de-init, RX won't be started/stopped */
        init_param->no_packet_rx = 1;
        ...
        sal_get_alloc_counters_main_thr(&alloc_bytes_count,&free_bytes_count);

        /* recover origianl no_packet_rx */
        init_param->no_packet_rx = original_no_packet_rx;
