...
_cint_interpreter_include_search(const char* name, char** path_rv)
{
    ...
    for (l = __directory_list; l ; l = l->next) {
        ...
        *p = 0;
        ...
        CINT_STRNCAT(p, name, CINT_STRLEN(l->directory)+1+len_name-sal_strlen(p)); 
...
/*
 * Utility function to parse and evaluate a set of CINT expressions. This is a
 * hungry routine which keeps evaluating CINT statements until either the
 * passed buffer is exhausted (i.e. no more expressions remain to be evaluated)
 * or there is a parsing error.
 *
 * Parameters
 *   [in] expr
 *     Buffer holding CINT statements.
 *   [out] cint_err
 *     If a parsing error is encountered, returns E_INTERNAL
 *     If a run-time error is encountered, contains the CINT error code
 *     Els contains CINT_E_NONE (0).
 *
 * Returns
 *   Value of the last expression is returned as an int if there were no
 *   errors. Caller should use his value only if cint_err is CINT_E_NONE
 */
int
cint_interpreter_parse_and_eval_string(const char* expr, cint_error_t *cint_err)
{
    cint_ast_t* ast;
    cint_cparser_t* cp = cint_cparser_create();
    int parse_ok, rv = CINT_E_NONE;
    cint_variable_t *cint_result = NULL;

    cint_errno = CINT_E_NONE;
    *cint_err = cint_errno;

    cint_cparser_start_string(cp, expr);
    ...
    while (cint_errno != CINT_E_EXIT) {
        ...
        if (parse_ok && cint_cparser_error(cp)) {
            /* If parsing a file, once there's a parser error,
               continue to parse, but do not evaluate.
        ...
        if (parse_ok && ast) {
            ...
            if (!cint_result) {
                ...
                *cint_err = cint_errno;
                ...
                rv = cint_long_value(cint_result);
            }
