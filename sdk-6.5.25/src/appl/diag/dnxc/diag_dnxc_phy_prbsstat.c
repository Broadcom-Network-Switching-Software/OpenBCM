...
typedef struct dnxc_prbs_stat_cb_s
{
    ...
    shr_thread_manager_handler_t thread_info;
    sal_mutex_t lock;
} dnxc_prbs_stat_cb_t;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    thread_config_info.name = "PRBSStat";
    thread_config_info.bsl_module = BSL_LOG_MODULE;
    thread_config_info.type = SHR_THREAD_MANAGER_TYPE_EVENT;
    thread_config_info.interval = (pscb->secs * SECOND_USEC);
    thread_config_info.callback = diag_dnxc_phy_prbs_stat_thread;
    thread_config_info.user_data = INT_TO_PTR(unit);
    thread_config_info.thread_priority = 100;
    thread_config_info.stop_timeout_usec = (10 * SECOND_USEC);
    rv = shr_thread_manager_create(unit, &thread_config_info, &(pscb->thread_info));
    ...
    int is_deinit)
{
    ...
    if (DNXC_PRBS_STAT_IS_RUNNING(pscb->flags))
    {
        ...
        SHR_IF_ERR_EXIT(shr_thread_manager_destroy(&(pscb->thread_info)));
        ...
        cli_out("PRBSStat thread exited!\n");
    }
    else
    {
        /*
         * Skip this message if during the deinit phase
         */
        if (!is_deinit)
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(diag_dnxc_phy_prbs_stat_stop(unit, DIAG_DNXC_PHY_PRBSSTAT_CMD));
...
sh_sand_cmd_t sh_dnxc_phy_prbsst_cmds[] = {
    ...
    ,
    {"stop", cmd_dnxc_phy_prbs_stat_stop, NULL, NULL, &dnxc_phy_prbs_stat_stop_man, NULL, NULL,
     SH_CMD_NO_XML_VERIFY * SH_CMD_SKIP_MEMORY_CHECK}
    ,
