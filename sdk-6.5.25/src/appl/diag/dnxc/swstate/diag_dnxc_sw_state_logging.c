    ...
    char *module_name)
{
    ...
    for (module_id = INVALID_MODULE_ID; module_id < NOF_MODULE_ID; module_id++)
    {
        ...
        if (!sal_strncmp(module_name, m_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
    ...
    char *function_name)
{
    ...
    for (f_type = 0; f_type < DNX_SW_STATE_COUNT_LOGGING; f_type++)
    {
        ...
        if (!sal_strncmp(function_name, f_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            return f_type;
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!sal_strncmp(module_id, "ALL", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        _module = 0;
        _module_count = NOF_MODULE_ID;
    }
    else if (sal_strncmp(module_id, "INVALID", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
    ...
    if (!sal_strncmp(func_type, "ALL", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        _f_type = 0;
        _f_type_count = DNX_SW_STATE_COUNT_LOGGING;
    }
    else if (sal_strncmp(func_type, "INVALID", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
