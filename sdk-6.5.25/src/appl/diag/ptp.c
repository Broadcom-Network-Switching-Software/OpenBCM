...
_bcm_ptp_is_ext_servo_type_idt_servo(int unit, bcm_ptp_stack_id_t stack_id, int clock_num)
{
    ...
    if (config.servo == bcmPTPServoTypeExt)
    {
        ...
        if (PTP_CONDITIONAL_IS_KATANA(unit) || PTP_CONDITIONAL_IS_TRIUMPH3(unit) || PTP_CONDITIONAL_IS_TRIDENT2(unit) ||
...
cmd_ptp_master(int unit, args_t *a)
{
    ...
    if (!arg) {
        ...
        i += sizeof(bcm_mac_t);
        sal_memcpy(master.address.raw_l2_header + i, port_data.mac, sizeof(bcm_mac_t));
        i += sizeof(bcm_mac_t);
