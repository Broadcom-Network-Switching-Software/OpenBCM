    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "IN TM Port");
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "PP DSP");
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "OUT TM Port");
    PRT_COLUMN_ADD("In Channel");
    PRT_COLUMN_ADD("Out Channel");
    ...
    BCM_PBMP_ITER(logical_ports, logical_port)
    {
        ...
        PRT_CELL_SET("%d", diag_info.in_tm_port);
        PRT_CELL_SET("%d", diag_info.pp_dsp);
        PRT_CELL_SET("%d", diag_info.out_tm_port);
        PRT_CELL_SET("%d", diag_info.in_channel_id);
        PRT_CELL_SET("%d", diag_info.out_channel_id);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(logical_ports, logical_port)
    {
        ...
        if (diag_info.nif_info.valid)
        {
            ...
            /** ILKN info  */
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, diag_info.logical_port_info.port_info,
        ...
        {
            /** General TM port */
            PRT_CELL_SET("in tm port");
            PRT_CELL_SET("%d", diag_info.tm_pp_info.in_tm_port);
            DIAG_DNX_PORT_MGMT_DUMP_FULL_TYPE("pp dsp");
            PRT_CELL_SET("%d", diag_info.tm_pp_info.pp_dsp);
            DIAG_DNX_PORT_MGMT_DUMP_FULL_TYPE("out tm port");
            PRT_CELL_SET("%d", diag_info.tm_pp_info.out_tm_port);
            ...
            DIAG_DNX_PORT_MGMT_DUMP_FULL_TYPE("in channel");
            PRT_CELL_SET("%d", diag_info.tm_pp_info.in_channel_id);
            DIAG_DNX_PORT_MGMT_DUMP_FULL_TYPE("out channel");
            PRT_CELL_SET("%d", diag_info.tm_pp_info.out_channel_id);
            ...
            if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, diag_info.logical_port_info.port_info))
            {
                ...
                {
                    DIAG_DNX_PORT_MGMT_DUMP_FULL_TYPE("mapped rcy_mirror port");
                    PRT_CELL_SET("%d", diag_info.tm_pp_info.rcy_mirror_logical_port);
                }
            }
        }
