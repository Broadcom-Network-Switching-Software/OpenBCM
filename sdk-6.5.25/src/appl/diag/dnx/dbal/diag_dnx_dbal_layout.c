    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_CELL_SET("%s",
                 (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_NONE_DIRECT_OPTIMIZED) ? "Optimized" :
                  "Regular"));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char lart_index[DBAL_MAX_STRING_LENGTH] = "";
    ...
    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
    {
        ...
        else
        {
            ...
            {
                sal_sprintf(lart_index, ", LART index=%d", table->multi_res_info[result_idx].lart_index);
            }
            PRT_TITLE_SET("result type=%s (hw %d), payload size=%d bits%s",
                          ...
                          table->multi_res_info[result_idx].entry_payload_size, lart_index);
