...
dnx_kbp_diag_sdk_ver_cmd(
    ...
    sh_sand_control_t * sand_control)
{
    const char *ver_str_expected = "KBP SDK 1.5.16";
    ...
    if (ver_str != NULL)
    {
        ...
        /** Read the version string until invalid version symbol is found. Valid are 0-9 and '.' */
        for (version_end = ver_str_start_offset; version_end < ver_str_len; version_end++)
        {
            if ((ver_str[version_end] != '.') && !((ver_str[version_end] >= '0') && (ver_str[version_end] <= '9')))
            {
                break;
            }
        }
        /** Take the longest of the two version strings */
        /** Use version_end to dismiss all symbols appended to the version (e.g. "EA1", "_debug", etc. */
        longest_length =
            sal_strnlen(ver_str, version_end) > sal_strnlen(ver_str_expected,
                                                            SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ?
            sal_strnlen(ver_str, version_end) : sal_strnlen(ver_str_expected,
                                                            SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
        if (sal_strncmp(ver_str_expected, ver_str, longest_length))
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "kbp_sdk_ver and sw_sdk_ver do not match. Expected: %s. Current: %s.\n",
                         ver_str_expected, ver_str);
        }
        else
        {
