...
static sh_sand_man_t Field_attach_info_man = {
    ...
    .examples = "group=70 context=15"
                ...
                "\n" "group=78 context=15"
                "\n" "group=79 context=12",
    .init_cb =  diag_dnx_field_attach_init,
};
    ...
    prt_control_t * prt_ctr)
{
    ...
    else if (attach_full_info_p->attach_basic_info.qual_info[qual_iter].input_type == DNX_FIELD_INPUT_TYPE_EXTERNAL)
    {
        PRT_CELL_SET("%s", "-");
        PRT_CELL_SET("%d", fg_info_p->group_full_info.context_info[context_id].qual_attach_info[qual_iter].offset);
    }
    ...
    sh_sand_control_t * sand_control)
{
         ...
         attach_full_info->key_id.id[key_id_iter] != DNX_FIELD_KEY_ID_INVALID; key_id_iter++)
    {
        ...
        if (sal_strncmp(keys_per_fg_string, "", sizeof(keys_per_fg_string)))
        {
            DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(keys_per_fg_string, "%s", ", ");
            sal_strncat_s(keys_per_fg_string, ", ", sizeof(keys_per_fg_string));
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int nof_acl_context_per_fwd_context = 1;
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_fwd_contexts
                    (unit, apptype, &fwd_nof_contexts, fwd_contexts, acl_contexts));
    for (fwd_context_idx = 0; fwd_context_idx < fwd_nof_contexts; fwd_context_idx++)
    {
        print_new_fwd_context = TRUE;
        for (acl_context_idx = 0; acl_context_idx < nof_acl_context_per_fwd_context; acl_context_idx++)
        {
                 ...
                 DNX_FIELD_QUAL_TYPE_INVALID; qual_iter++)
            {
                     ...
                     ffc.ffc_id != DNX_FIELD_FFC_ID_INVALID; ffc_iter++)
                {
                    ...
                    if (print_new_fwd_context)
                    {
                        ...
                        PRT_CELL_SET("%d", fwd_contexts[fwd_context_idx]);
                        print_new_fwd_context = FALSE;
                    }
                    else
                    {
                    ...
                    {
                        PRT_CELL_SET("%d", acl_contexts[fwd_context_idx]);
                        print_new_acl_context = FALSE;
                    }
                    else
                    {
    ...
    sh_sand_control_t * sand_control)
{
         ...
         attach_full_info_p->attach_basic_info.dnx_quals[qual_iter] != DNX_FIELD_QUAL_TYPE_INVALID; qual_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size
                        (unit, field_stage, attach_full_info_p->attach_basic_info.dnx_quals[qual_iter],
                         NULL, NULL, &qual_size));
    ...
    sh_sand_control_t * sand_control)
{
         ...
         (attach_full_info_p->attach_basic_info.dnx_actions[action_iter] != DNX_FIELD_ACTION_INVALID); action_iter++)
    {
        ...
        {
            action_iter_non_void++;
            continue;
        }
        /** Sanity check.*/
        if (attach_full_info_p->actions_fes_info.initial_fes_quartets[action_iter_non_void].fes_id ==
            DNX_FIELD_EFES_ID_INVALID)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "action number %d, expected to be action %d with EFES but "
                         "there is no associated EFES.\n", action_iter, action_iter_non_void);
        }

        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);

        PRT_CELL_SET("FES %d", attach_full_info_p->actions_fes_info.initial_fes_quartets[action_iter_non_void].fes_id);
    ...
    /** Sanity check.*/
    if ((action_iter_non_void < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP) &&
        (attach_full_info_p->actions_fes_info.initial_fes_quartets[action_iter_non_void].fes_id !=
         DNX_FIELD_EFES_ID_INVALID))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Not all EFES printed, EFES %d.\n", action_iter_non_void);
    }
         ...
         action_iter++)
    {
        ...
        if (is_all_fes2msb_actions_same)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_action_type_to_predef_dnx_action
                            (unit, fg_info_p->group_basic_info.field_stage,
                             attach_full_info_p->actions_fes_info.added_fes_quartets[action_iter].
                             fes2msb_info[0].action_type, &dnx_action));
            SHR_IF_ERR_EXIT(dnx_field_map_action_dnx_to_bcm
                ...
                else
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_field_map_action_type_to_predef_dnx_action
                                    (unit, fg_info_p->group_basic_info.field_stage,
                                     ...
                                     added_fes_quartets[action_iter].fes2msb_info[fes2msb_index].action_type,
                                     &dnx_action));
                    SHR_IF_ERR_EXIT(dnx_field_map_action_dnx_to_bcm
                                    (unit, fg_info_p->group_basic_info.field_stage, dnx_action, &bcm_action));
                    PRT_CELL_SET("%s", dnx_field_bcm_action_text(unit, bcm_action));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        if ((context_id < dnx_data_field.external_tcam.apptype_user_1st_get(unit)) ||
