    ...
    sh_sand_control_t * sand_control)
{
    ...
    RHITERATOR(cs_dnx_dump, cs_dnx_dump_list)
    {
        ...
        rv = dbal_tables_field_size_get(unit, field_map_stage_info.cs_table_id, cs_dnx_dump->field_id[stage], TRUE, 0,
                                        0, &dnx_cs_qual_size);
        ...
        if (show_desc == TRUE)
        {
            PRT_CELL_SET("%s", bcm_qual_description[cs_dnx_dump->bcm_qual]);
        }
        PRT_CELL_SET("%s", dbal_field_to_string(unit, cs_dnx_dump->field_id[stage]));
        PRT_CELL_SET("%d", dnx_cs_qual_size);

        SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_map_get_dispatch(unit, stage, cs_dnx_dump->bcm_qual, &cs_qual_map));
        /**
         ...
         */
        if (BCM_TO_DNX_IS_BAISC_OBJ(cs_qual_map.flags))
        {
            PRT_CELL_SET("%s", dnx_field_bcm_qual_text(unit, cs_dnx_dump->bcm_qual));
        }
        else
        {
            ...
            {
                dnx_field_cs_qual_map_t cs_qual_map_raw;
                ...
                SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_map_get_dispatch
                                (unit, stage, cs_dnx_dump_raw->bcm_qual, &cs_qual_map_raw));

                if (BCM_TO_DNX_IS_BAISC_OBJ(cs_qual_map_raw.flags) &&
    ...
    prt_control_t * prt_ctr)
{
    ...
    for (bcm_qual = 0; bcm_qual < bcmFieldQualifyCount; bcm_qual++)
    {
         ...
         * Otherwise we should find the basic qualifier for current BCM qual.
         */
        if (BCM_TO_DNX_IS_BAISC_OBJ(meta_qual_map.flags))
        {
            PRT_CELL_SET("%s", dnx_field_bcm_qual_text(unit, bcm_qual));
        }
        else
        {
            ...
            for (bcm_qual_raw = 0; bcm_qual_raw < bcmFieldQualifyCount; bcm_qual_raw++)
            {
                ...
                SHR_IF_ERR_EXIT(rv);
                if (BCM_TO_DNX_IS_BAISC_OBJ(meta_qual_map.flags) && (dnx_qual == dnx_qual_raw))
                {
