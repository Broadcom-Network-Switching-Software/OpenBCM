    ...
    sh_sand_control_t * sand_control)
{
    ...
    *context_id_p = (dnx_field_context_t) context_id;
    ...
    if (DIAG_DNX_FIELD_LAST_TCAM_HIT_INDICATION_ENABLE)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get_dispatch(unit, field_stage, &field_map_stage_info));
    ...
    uint8 *is_entry_found_p)
{
    ...
    *is_entry_found_p = FALSE;
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_contexts_to_apptype_get(unit, fwd_context, acl_context, &apptype));
    SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_opcode_bcm_to_dnx(unit, apptype, &opcode_id));
    ...
    prt_control_t * prt_ctr)
{
    ...
    /** Sanity check */
    if (max_val_iter > DNX_DATA_MAX_FIELD_ENTRY_NOF_QUAL_PARAMS_PER_ENTRY)
    {
