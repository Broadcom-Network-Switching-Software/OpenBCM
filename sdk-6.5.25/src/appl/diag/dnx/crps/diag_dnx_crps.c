    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine
                            (unit, engine_id, interface.format_type, &nof_counters));
            /** interface*/
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        ...
        if (interface.source != bcmStatCounterInterfaceOamp)
        {
            ...
            if (dnx_data_crps.expansion.source_type_get(unit, interface_key.interface_source,
            ...
            for (ind = 0; ind < expansion.nof_elements; ind++)
            {
                ...
                size =
                    dnx_data_crps.expansion.source_type_get(unit, interface.source,
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id_arr[0], format_type, &nof_counters));
    end_object_stat_id_arr[0] = nof_counters / counter_set_size - 1;
    type_offset[0] = 0;

    /** configure type 1 */
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id_arr[1], format_type, &nof_counters));
    ...
    /** configure type 2 */
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id_arr[2], format_type, &nof_counters));
    ...
    /** configure type 3 */
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id_arr[3], format_type, &nof_counters));
