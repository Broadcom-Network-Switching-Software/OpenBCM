...
#define PRIMARY_FEC_INST 0
    ...
    for (fec_instance = low_instance; fec_instance <= high_instance; fec_instance++)
    {
        ...
        if (dnx_data_l3.fec.feature_get(unit, dnx_data_l3_fec_separate_fwd_rpf_dbs) && core_mode == DBAL_CORE_MODE_DPC)
        {
            /** Get FEC information*/
            SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fecz.direction.get(unit, fec_id, &fec_dir_get));
            SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fecz.hierarchy.get(unit, fec_id, &fec_hier_get));
    ...
    int unit)
{
    ...
    flags2 = (dnx_data_l3.fec.feature_get(unit, dnx_data_l3_fec_separate_fwd_rpf_dbs)) ? BCM_L3_FLAGS2_FWD_ONLY : 0;
