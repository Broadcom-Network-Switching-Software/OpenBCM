    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (ecmp_gr_id != DIAG_DNX_PP_SHOW_ALL_ECMP_GROUPS_INDICATION)
    {
        ...
        /** ECMP ID  */
        SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get(unit, DBAL_TABLE_ECMP_TABLE, DBAL_FIELD_ECMP_ID, 0,
        ...
        SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get
                        (unit, DBAL_TABLE_ECMP_GROUP_PROFILE_TABLE, DBAL_FIELD_HIERARCHY_LEVEL, 0, &hierarchy, NULL, 0,
                         TRUE, buffer));
        PRT_CELL_SET("%s", buffer);
        ...
        /** Print members table */
        {
            if (ecmp_profile.ecmp_mode != DBAL_ENUM_FVAL_ECMP_MODE_MULTIPLY_AND_DIVIDE)
            {
                ...
                for (table_index = 0; table_index < nof_tables; table_index++)
                {
                    ...
                    max_nof_members =
                        dnx_data_l3.ecmp.consistent_tables_info_get(unit, consistent_table_type)->nof_entries;
    ...
    else
    {
        ...
        while (!is_end)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_hierarchy_get(unit, ecmp_gr_id, (uint32 *) &hierarchy));
            SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get(unit, DBAL_TABLE_ECMP_GROUP_PROFILE_TABLE,
                                                                  DBAL_FIELD_HIERARCHY_LEVEL, 0, &hierarchy, NULL, 0,
                                                                  TRUE, buffer));
            PRT_CELL_SET("%s", buffer);
            ...
            SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get(unit, DBAL_TABLE_ECMP_TABLE,
                                                                  DBAL_FIELD_ECMP_GROUP_PROFILE, 0, &field_result, NULL,
                                                                  0, FALSE, buffer));
            PRT_CELL_SET("%s", buffer);
