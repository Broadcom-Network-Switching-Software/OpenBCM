    ...
    if ((fspi->fec_type == bcmPortPhyFecRs544_2xN) ||
        ...
        LOG_DEBUG(BSL_LS_APPL_PHY,
                  (BSL_META_U(unit, "Updated BER for port %d: %8.2e\n"),
                   port, ber[0]));
    } else if ((fspi->fec_type  == bcmPortPhyFecRsFec)  ||
...
{
    bcm_error_t rv = BCM_E_NONE;
    uint32 count, corrected_count, error_count[PM_MAX_LANES] = {0};
...
fec_stat_ber(int unit, bcm_pbmp_t pbmp, args_t *a)
{
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        if ((fspi->fec_type  == bcmPortPhyFecRsFec)  ||
            ...
            for (lane = 0; lane < fspi->lanes; lane++) {
                if (fspi->ber[lane]) {
                    total_port_ber += fspi->ber[lane];
                }
            }
            if (total_port_ber == 0) {
                LOG_CLI(("%-s:  N/A\n", BCM_PORT_NAME(unit, port)));
            } else {
                port_ber = total_port_ber/fspi->lanes;
                LOG_CLI(("%-s:  %4.2e\n", BCM_PORT_NAME(unit, port), port_ber));
            }
            for (lane = 0; lane < fspi->lanes; lane++) {
                if (fspi->ber[lane]) {
