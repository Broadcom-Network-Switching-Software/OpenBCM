...
xmlBufferWrite (void * context, const char * buffer, int len) {
    ...
    ret = xmlBufferAddRv((xmlBufferPtr) context, (const xmlChar *) buffer, len);
    if (ret != 0)
        return(-1);
    return(len);
}
...
#endif
	    len = xmlOutputBufferWriteRv( ctxt->doc_buff, len, buffer );
 ...
 */
int
xmlOutputBufferWriteRv(xmlOutputBufferPtr out, int len, const char *buf) {
 ...
 * Same as xmlOutputBufferWriteRv.
...
xmlOutputBufferWrite(xmlOutputBufferPtr out, int len, const char *buf) {
    /* We don't check the returned value on purpose */
    /* coverity[check_return:FALSE] */
    xmlOutputBufferWriteRv(out, len, buf);
}
...
xmlOutputBufferWriteString(xmlOutputBufferPtr out, const char *str) {
    ...
    if (len > 0)
        ...
        /* coverity[check_return:FALSE] */
        xmlOutputBufferWrite(out, len, str);
}
