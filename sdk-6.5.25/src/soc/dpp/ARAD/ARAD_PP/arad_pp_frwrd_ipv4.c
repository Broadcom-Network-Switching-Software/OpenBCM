    ...
    )
{
  ...
  key->param[1].value[0] = 0;

  key->param[2].nof_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM2_IN_BITS_FOR_IPMC_BIDIR;
  key->param[2].value[0] = rp_info->rp_id;
    ...
    )
{
  ...
  rp_info->rp_id = key->param[2].value[0];
}
    ...
    )
{
  ...
  if(JER_KAPS_ENABLE(unit)
    ...
    || ARAD_KBP_ENABLE_ANY_IPV4UC_PROGRAM
#endif
  )
  {
    uint32 read_index = 0;
  ...
  )
{
  ...
  uint32
    ...
    fld_val;
  uint32
    res = SOC_SAND_OK, res2 = SOC_SAND_OK;
#if defined(INCLUDE_KBP)
...
#endif
  {
      ...
      if (old_fld_val != fld_val) {
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res2,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, PPDB_B_LARGE_EM_DIAGNOSTICS_ACCESSED_MODEr, REG_PORT_ANY, 0, LARGE_EM_DIAGNOSTICS_ACCESSED_MODEf,  old_fld_val));
      }
...
#endif
#if defined(BCM_88675_A0)
  if (JER_KAPS_ENABLE(unit) && !(flags & SOC_PPC_FRWRD_IP_HOST_ONLY))
  {
      res = arad_pp_dbal_table_clear(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KAPS);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
