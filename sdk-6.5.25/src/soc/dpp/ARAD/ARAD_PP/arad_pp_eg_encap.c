...
#define ARAD_PP_EG_ENCAP_AC_OUTLIF_PROFILE_MAX    (7)
  ...
  )
{
    ...
    if(eep_type == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_AC) {
        ...
        if ((flags & SOC_PPC_EG_ENCAP_ENTRY_UPDATE_OUT_LIF_PROFILE) && (val > ARAD_PP_EG_ENCAP_AC_OUTLIF_PROFILE_MAX))
        {
            
            SOC_SAND_SET_ERROR_CODE(ARAD_PP_EG_ENCAP_ACCESS_ENTRY_TYPE_MISMATCH_ERR, 15, exit);
        }
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      res = arad_pp_eg_encap_protection_info_set_unsafe(
                unit, lif_eep_ndx, is_full_entry_extension, &(swap_info->protection_info));
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  }
  ...
  )
{
  ...
  if (tbl_data.mpls1_command == ARAD_PP_EG_ENCAP_MPLS1_COMMAND_SWAP_val)
  {
    ...
    SOC_PPC_EG_ENCAP_PROTECTION_INFO_clear(&(encap_info->entry_val.swap_info.protection_info));
    if (SOC_IS_JERICHO(unit)) {
        res = arad_pp_eg_encap_protection_info_get_unsafe(
                unit, eep_ndx, is_full_entry_extension, &(encap_info->entry_val.swap_info.protection_info));
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    }
  }
