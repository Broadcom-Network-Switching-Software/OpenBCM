...
void
  SOC_TMC_CNM_PETRA_PP_clear(
    SOC_SAND_OUT SOC_TMC_CNM_PETRA_PP *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);

  sal_memset(info, 0x0, sizeof(SOC_TMC_CNM_PETRA_PP));
  ...
  )
{
  ...
  info->is_cp_enabled = 0;
  SOC_TMC_CNM_PETRA_PP_clear(&(info->pp));
  ...
  )
{
  ...
  case SOC_TMC_CNM_GEN_MODE_EXT_PP:
    str = "ext_pp";
  break;
  case SOC_TMC_CNM_GEN_MODE_PETRA_PP:
    str = "dune_pp1";
  break;
...
void
  SOC_TMC_CNM_PETRA_PP_print(
    SOC_SAND_IN  SOC_TMC_CNM_PETRA_PP *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);
  ...
  )
{
  ...
  if (info->pkt_gen_mode == SOC_TMC_CNM_GEN_MODE_PETRA_PP)
  {
    SOC_TMC_CNM_PETRA_PP_print(&(info->pp));
  }
