  ...
  )
{
    ...
    SOC_PBMP_ITER(port_bm, port) {
        ...
        if (SOC_IS_QUX(unit) && SOC_DPP_CONFIG(unit)->qax->lbi_en && egr_if == SOC_QAX_EGR_IF_LBG_RESERVE) {
            field_val = 0;
        }
    ...
    SOC_SAND_IN  int                                value)
{
    ...
    switch (property)
    {
        case soc_dpp_flow_profile_ecn_enable:
        {
            int ecn_enable = value ? TRUE : FALSE;
            SOCDNX_IF_ERR_EXIT(READ_EPNI_PACKET_LATENCY_MEASUREMENT_CFGr(unit, profile, above_64_reg_val));
            soc_reg_above_64_field32_set(unit, EPNI_PACKET_LATENCY_MEASUREMENT_CFGr, above_64_reg_val, EN_LATENCY_CNI_Nf, ecn_enable);
            SOCDNX_IF_ERR_EXIT(WRITE_EPNI_PACKET_LATENCY_MEASUREMENT_CFGr(unit, profile, above_64_reg_val));
        }
        break;
        case soc_dpp_flow_profile_threshold:
        {
            uint32 field_val;

            field_val = (uint32)value;
            SOCDNX_IF_ERR_EXIT(READ_EPNI_PACKET_LATENCY_MEASUREMENT_CFGr(unit, profile, above_64_reg_val));
            soc_reg_above_64_field32_set(unit, EPNI_PACKET_LATENCY_MEASUREMENT_CFGr, above_64_reg_val, BIG_LATENCY_CNI_TH_Nf, field_val);
            SOCDNX_IF_ERR_EXIT(WRITE_EPNI_PACKET_LATENCY_MEASUREMENT_CFGr(unit, profile, above_64_reg_val));
        }
        break;
        default:
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("invalid argument property")));
            break;
    }
    ...
    SOC_SAND_OUT int                                *value)
{
    ...
    switch (property)
    {
        ...
        {
            SOCDNX_IF_ERR_EXIT(READ_EPNI_PACKET_LATENCY_MEASUREMENT_CFGr(unit, profile, above_64_reg_val));
            *value = soc_reg_above_64_field32_get(unit, EPNI_PACKET_LATENCY_MEASUREMENT_CFGr, above_64_reg_val, EN_LATENCY_CNI_Nf);
        }
        break;
        case soc_dpp_flow_profile_threshold:
        {
            ...
            SOCDNX_IF_ERR_EXIT(READ_EPNI_PACKET_LATENCY_MEASUREMENT_CFGr(unit, profile, above_64_reg_val));
            *value = soc_reg_above_64_field32_get(unit, EPNI_PACKET_LATENCY_MEASUREMENT_CFGr, above_64_reg_val, BIG_LATENCY_CNI_TH_Nf);
        }
        break;
        default:
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("invalid argument property")));
