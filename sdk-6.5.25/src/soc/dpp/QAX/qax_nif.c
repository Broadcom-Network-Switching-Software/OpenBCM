...
soc_qax_port_prd_map_get(int unit, soc_port_t port, uint32 flags, soc_dpp_prd_map_t map, uint32 key, int *priority)
{
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_prd_config);
    SOC_REG_ABOVE_64_CLEAR(prio_map);
    *priority = 0;
    if (!IS_IL_PORT(unit,port)) {
...
soc_qax_nif_sku_restrictions(int unit, soc_pbmp_t phy_pbmp, soc_port_if_t interface, uint32 protocol_offset, uint32 is_kbp){
       ...
       if (dcmn_device_block_for_feature(unit,DCMN_QAX_NIF_8_16_FEATURE)) {
           soc_pbmp_t restricted_pbmp;
           int phy;

           SOC_PBMP_CLEAR(restricted_pbmp);
           
           for (phy = 5; phy <= 8; phy++) {
               SOC_PBMP_PORT_ADD(restricted_pbmp, phy);
           }
           
           for (phy = 13; phy <= 16; phy++) {
               SOC_PBMP_PORT_ADD(restricted_pbmp, phy);
           }
           
           for (phy = 69; phy <= 120; phy++) {
               SOC_PBMP_PORT_ADD(restricted_pbmp, phy);
           }

	   SOC_PBMP_AND(restricted_pbmp, phy_pbmp);
           
           if(SOC_PBMP_NOT_NULL(restricted_pbmp)) {
               SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s can't be mapped to Quad 1/3 and Quad 8/9/10/11."),
                                                 soc_dev_name(unit)));
           }
       }
       ...
       if (dcmn_device_block_for_feature(unit,DCMN_QAX_NO_ILKN_FEATURE)) {
           if (interface == SOC_PORT_IF_ILKN) {
               SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s does not support ILKN."),
                                                  soc_dev_name(unit)));
           }
       }
