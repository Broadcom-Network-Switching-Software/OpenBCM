    ...
    bcm_port_t *src_port)
{
    ...
    SHR_IF_ERR_EXIT(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP",
                       is_header_vis_global ? "NIF" : dnx_data_pp.debug.from_signal_str_get(unit)->ptc,
                       is_header_vis_global ? "PRT" : NULL , "ptc", &ptc_sig));
    ...
    uint32 *enabled)
{
    ...
    *enabled = FALSE;
    ...
    SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", "VTT4", "VTT5", "Fwd_Layer_Index", &sig_fwd_index_vt4_p));
    fwd_layer_index_vt4 = sig_fwd_index_vt4_p->value[0];

    
    if (fwd_layer_index_vt4 <= mpls_unterm_layer_index)
    {
        SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", "VTT5", "FWD1", "In_LIFs.0", &sig_glif_p));
    }
    else
    {
        SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", "VTT4", "VTT5", "In_LIFs.0", &sig_glif_p));
    }
