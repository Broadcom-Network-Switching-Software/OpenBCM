    ...
    else
    {
        ...
        DNXCMN_CORES_ITER(unit, core, core_iter)
        {
            SHR_IF_ERR_EXIT(dnx_field_tcam_access_sw.
                            entry_location_hash.find(unit, core_iter, &(entry->id), &location, &found));
            if (found)
            {
    ...
    uint32 handler_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_profile_free(unit, handler_id));
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_sw.fg_params.prefix_value.set(unit, handler_id, 0));
    ...
    dnx_field_tcam_entry_t * entry)
{
    ...
    DNXCMN_CORES_ITER(unit, core, core_iter)
    {
        ...
        if (banks_full)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_tcam_bank_single_alloc_any(unit, handler_id, core_iter));
        }
    }
