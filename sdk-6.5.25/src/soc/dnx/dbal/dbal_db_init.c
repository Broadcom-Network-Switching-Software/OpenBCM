...
#define DBAL_DB_INIT_TABLE_UPDATE_HW_ERROR_AND_EXIT_INIT_IF_CORE_MODE_NOT_SET(_table_id, _table_entry)  \
    ...
    {                                                                                                   \
        ...
        {                                                                                               \
            SHR_IF_ERR_EXIT_NO_MSG(dbal_tables_update_hw_error(unit, _table_id));                       \
        }                                                                                               \
        SHR_EXIT();                                                                                     \
    }
...
#define DBAL_DB_INIT_TABLE_SKIP_SET_ALLOCATOR_FIELD(_table_entry)                                       \
    ...
    && _table_entry->core_mode == DBAL_CORE_MODE_DPC) ||                                                \
     (_table_entry->allocator_field_id != 0))
    ...
    int *is_reg)
{
    ...
    for (hw_entity_index = 0; hw_entity_index < group_info->group_hw_entity_num; hw_entity_index++)
    {
        ...
        {
            SHR_IF_ERR_EXIT_NO_MSG(dbal_db_init_hw_element_group_hw_entity_verify(unit, hw_entity, *is_reg));
        }
    ...
    table_db_access_params_struct_t * access_params)
{
    ...
    if (access_params->hw_entity_group_id != DBAL_HW_ENTITY_GROUP_EMPTY)
    {
        ...
        if (DBAL_DEVICE_STATE_UNDER_DEVELOPMENT == dnx_data_dbal.db_init.dbal_device_state_get(unit))
        {
            SHR_IF_ERR_EXIT_NO_MSG(dbal_db_init_hw_element_group_handle
    ...
    else
    {
        ...
        if (DBAL_DEVICE_STATE_UNDER_DEVELOPMENT == dnx_data_dbal.db_init.dbal_device_state_get(unit))
        {
            SHR_IF_ERR_EXIT_NO_MSG(dbal_db_init_reg_mem_hw_entity_validate
    ...
    int nof_valid_access_fields[])
{
    ...
    for (access_type_index = 0; access_type_index < DBAL_NOF_HL_ACCESS_TYPES; access_type_index++)
    {
        ...
        {
            SHR_ALLOC(hl_mapping->l2p_hl_info[access_type_index].l2p_fields_info,
    ...
    dbal_logical_table_t * table_info)
{
    ...
    switch (table_entry->access_method)
    {
        case DBAL_ACCESS_METHOD_HARD_LOGIC:
            ...
            {
                SHR_IF_ERR_EXIT_NO_MSG(dbal_db_init_table_add_mapping_hard_logic
                                       (unit, table_entry, table_id, table_params));
            }
            break;
    ...
    int hw_field)
{
    ...
    access_params->alias_reg_mem_hw_entity_id = INVALIDr;
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dbal_db_init_table_interface_field_indications_update(unit, field_info));
    if (!DBAL_DB_INIT_TABLE_SKIP_SET_ALLOCATOR_FIELD(table_entry))
    {
    ...
    dbal_logical_table_t * table)
{
    ...
    for (jj = 0; jj < table->nof_key_fields; jj++)
    {
        ...
        dbal_db_init_table_interface_table_allocator_update(table, field_info);
    ...
    dbal_logical_table_t * table)
{
    ...
    for (iter = 0; iter < DBAL_NOF_HL_TCAM_ACCESS_TYPES; ++iter)
    {
        ...
        memory = l2p_fields_info->memory[0];
        ...
        if (l2p_fields_info->hw_field != INVALIDf)
        {
            
            SOC_FIND_FIELD(l2p_fields_info->hw_field, meminfo->fields, meminfo->nFields, fieldinfo);
