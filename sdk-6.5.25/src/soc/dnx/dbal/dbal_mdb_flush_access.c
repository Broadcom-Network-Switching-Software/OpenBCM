    ...
    dbal_flush_shadow_info_t * rules_info)
{
    ...
    soc_mem_t flush_data_mem, flush_tcam_mem, lem_memory;
    int flush_in_work = 0;
    uint32 lem_entry_size;
    ...
    for (rule_index = 0; rule_index < nof_rules; rule_index++)
    {
        ...
        lem_memory = dnx_data_mdb.em.em_info_get(unit, lem_physical_table_id)->em_interface;
        SHR_IF_ERR_EXIT(mdb_em_prepare_entry(unit, lem_physical_table_id, app_db_id, lem_memory,
    ...
    int unit)
{
    ...
    MDB_LEM_ITER(unit, lem_iter)
    {
        ...
        flush_buffer[unit][lem_iter] = sal_alloc(host_mem_size_in_bytes, buffer_desc);
    ...
    int unit)
{
    ...
    MDB_LEM_ITER(unit, lem_iter)
    {
        if (flush_buffer[unit][lem_iter] != NULL)
        {
            sal_free(flush_buffer[unit][lem_iter]);
        }
        flush_buffer[unit][lem_iter] = NULL;
    }
}
