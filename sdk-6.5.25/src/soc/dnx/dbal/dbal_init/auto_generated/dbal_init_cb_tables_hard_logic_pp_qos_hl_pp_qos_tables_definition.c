...
{
    dbal_entry_handle_t *  eh = (dbal_entry_handle_t *) entry_handle;
    uint32 key_value__propagation_profile;
    uint32 key_value__qos_ingress_app_type;
    uint32 full_result_size = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_PROPAGATION_PROFILE, key_value__propagation_profile);
    ...
    *offset = key_value__propagation_profile*16+full_result_size*key_value__qos_ingress_app_type;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
...
uint32 * offset)
{
    ...
    *offset = key_value__propagation_profile*16+full_result_size*key_value__qos_ingress_app_type+result_size__qos_propag_strength;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
