...
dnx_ecgm_db_port_info_dump(int  unit,  int  port_info_idx_0,  int  port_info_idx_1,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    else
    {
        ...
        if(i0 >= dnx_sw_state_get_nof_elements(unit, DNX_ECGM_MODULE_ID, ((dnx_ecgm_sw_state_t*)sw_state_roots_array[unit][DNX_ECGM_MODULE_ID])->port_info))
        {
        ...
        if(dnx_sw_state_get_nof_elements(unit, DNX_ECGM_MODULE_ID, ((dnx_ecgm_sw_state_t*)sw_state_roots_array[unit][DNX_ECGM_MODULE_ID])->port_info) == 0)
        {
            SHR_EXIT();
        }
        ...
        for(; i0 < I0; i0++) {
            ...
            if(i1 >= dnx_sw_state_get_nof_elements(unit, DNX_ECGM_MODULE_ID, ((dnx_ecgm_sw_state_t*)sw_state_roots_array[unit][DNX_ECGM_MODULE_ID])->port_info[i0]))
            {
                LOG_CLI((BSL_META("dnx_ecgm_db[]->((dnx_ecgm_sw_state_t*)sw_state_roots_array[][DNX_ECGM_MODULE_ID])->port_info[][]: ")));
                LOG_CLI((BSL_META("Invalid index: %d \n"),i1));
                SHR_EXIT();
            ...
            if(dnx_sw_state_get_nof_elements(unit, DNX_ECGM_MODULE_ID, ((dnx_ecgm_sw_state_t*)sw_state_roots_array[unit][DNX_ECGM_MODULE_ID])->port_info[i0]) == 0)
            {
                SHR_EXIT();
            }
