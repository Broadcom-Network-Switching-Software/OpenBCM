    ...
    else
    {
        ...
        for(; i0 < I0; i0++) {
            ...
            DNX_SW_STATE_PRINT_ENUM(
                unit,
                dnx_field_tcam_stage_e_get_name(((dnx_field_tcam_bank_sw_t*)sw_state_roots_array[unit][DNX_FIELD_TCAM_BANK_MODULE_ID])->tcam_banks[i0].owner_stage));
...
dnx_field_tcam_bank_sw_tcam_banks_active_handlers_id_dump(int  unit,  int  tcam_banks_idx_0,  int  active_handlers_id_idx_0,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    else
    {
        ...
        for(; i0 < I0; i0++) {
            ...
            if(i1 >= dnx_sw_state_get_nof_elements(unit, DNX_FIELD_TCAM_BANK_MODULE_ID, ((dnx_field_tcam_bank_sw_t*)sw_state_roots_array[unit][DNX_FIELD_TCAM_BANK_MODULE_ID])->tcam_banks[i0].active_handlers_id))
            {
                LOG_CLI((BSL_META("dnx_field_tcam_bank_sw[]->((dnx_field_tcam_bank_sw_t*)sw_state_roots_array[][DNX_FIELD_TCAM_BANK_MODULE_ID])->tcam_banks[].active_handlers_id[]: ")));
                LOG_CLI((BSL_META("Invalid index: %d \n"),i1));
                SHR_EXIT();
            ...
            if(dnx_sw_state_get_nof_elements(unit, DNX_FIELD_TCAM_BANK_MODULE_ID, ((dnx_field_tcam_bank_sw_t*)sw_state_roots_array[unit][DNX_FIELD_TCAM_BANK_MODULE_ID])->tcam_banks[i0].active_handlers_id) == 0)
            {
                SHR_EXIT();
            }
