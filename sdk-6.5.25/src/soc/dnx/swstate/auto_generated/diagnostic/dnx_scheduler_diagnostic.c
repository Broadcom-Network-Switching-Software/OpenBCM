...
dnx_scheduler_db_virtual_flows_is_flow_virtual_dump(int  unit,  int  is_flow_virtual_idx_0,  int  is_flow_virtual_idx_1,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    else
    {
        ...
        if(i0 >= dnx_sw_state_get_nof_elements(unit, DNX_SCHEDULER_MODULE_ID, ((dnx_scheduler_db_t*)sw_state_roots_array[unit][DNX_SCHEDULER_MODULE_ID])->virtual_flows.is_flow_virtual))
        {
        ...
        if(dnx_sw_state_get_nof_elements(unit, DNX_SCHEDULER_MODULE_ID, ((dnx_scheduler_db_t*)sw_state_roots_array[unit][DNX_SCHEDULER_MODULE_ID])->virtual_flows.is_flow_virtual) == 0)
        {
            SHR_EXIT();
        }
        ...
        for(; i0 < I0; i0++) {
            ...
            if(i1 >= dnx_sw_state_get_nof_elements(unit, DNX_SCHEDULER_MODULE_ID, ((dnx_scheduler_db_t*)sw_state_roots_array[unit][DNX_SCHEDULER_MODULE_ID])->virtual_flows.is_flow_virtual[i0]))
            {
                LOG_CLI((BSL_META("dnx_scheduler_db[]->((dnx_scheduler_db_t*)sw_state_roots_array[][DNX_SCHEDULER_MODULE_ID])->virtual_flows.is_flow_virtual[][]: ")));
                LOG_CLI((BSL_META("Invalid index: %d \n"),i1));
                SHR_EXIT();
            ...
            if(dnx_sw_state_get_nof_elements(unit, DNX_SCHEDULER_MODULE_ID, ((dnx_scheduler_db_t*)sw_state_roots_array[unit][DNX_SCHEDULER_MODULE_ID])->virtual_flows.is_flow_virtual[i0]) == 0)
            {
                SHR_EXIT();
            }
