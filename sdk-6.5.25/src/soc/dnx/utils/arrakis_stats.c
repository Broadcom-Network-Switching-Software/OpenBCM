    ...
    char *stat_family_name)
{
    ...
    if (sal_strnlen(stat_family_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) >
        ARRAKIS_STATS_FAMILY_NAME_MAX_LEN - 1)
    {
    ...
    char *dimension)
{
    ...
    if (sal_strnlen(dimension, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) > ARRAKIS_STATS_DIMENSION_NAME_MAX_LEN - 1)
    {
    ...
    void *samples)
{
    ...
    for (i = 0; i < dimensions_nof; i++)
    {
        ...
        if (sal_strnlen(dimensions[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) >
    ...
    char *sample)
{
    ...
    {
        if (sal_strncmp(dimension, stat_family->dimensions[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            dimension_idx = i;
            break;
        }
    ...
    if (sal_strnlen(stat_family->samples[dimension_idx], sizeof(stat_family->samples[dimension_idx])) +
        sal_strnlen(sample, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) > ARRAKIS_STATS_SAMPLES_STR_MAX_LEN - 1)
    {
        SHR_ERR_EXIT(_SHR_E_RESOURCE, "Samples string length exceeds ARRAKIS_STATS_SAMPLES_STR_MAX_LEN");
    }
    sal_strncat_s(stat_family->samples[dimension_idx], sample, sizeof(stat_family->samples[dimension_idx]));
    ...
    char *num_str)
{
    ...
    for (i = sal_strnlen(num_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) - 1; i > 0; i--)
    {
        if (num_str[i] == '0')
        {
            num_str[i] = '\0';
    ...
    stat_family_xml_t * stat_family_xml)
{
    ...
    RHDATA_ITERATOR(stat_family_node, stats_syntax_root, "stat-family")
    {
        ...
        if (sal_strncmp(stat_family_name, stat_family_name_itr, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    stat_family_xml_t * stat_family_xml)
{
    ...
    {
        if (sal_strncmp(stat_family_xml->dimensions[idx], STATS_DATE, sizeof(stat_family_xml->dimensions[idx])) == 0)
        {
            return 1;
        }
    }
    ...
    stat_family_xml_t * stat_family_xml)
{
    ...
    for (struct_idx = 0; struct_idx < stat_family->dimensions_nof; struct_idx++)
    {
        ...
        {
            if (sal_strncmp
                (stat_family->dimensions[struct_idx], stat_family_xml->dimensions[xml_idx],
                 sizeof(stat_family->dimensions[struct_idx])) == 0)
            {
    ...
    arrakis_stat_family_t * stat_family)
{
    ...
    for (dim_idx = 0; dim_idx < stat_family->dimensions_nof; dim_idx++)
    {
        ...
        sal_strncat_s(dimension_str, stat_family->samples[dim_idx], sizeof(dimension_str));
