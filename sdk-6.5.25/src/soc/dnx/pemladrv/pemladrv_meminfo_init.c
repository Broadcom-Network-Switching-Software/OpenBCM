...
int parse_meminfo_definition_file(int unit, int restore_after_reset, uint32 use_file, const char *rel_file_path, const char *file_name)
{
    ...
    if (sal_strnstr(line, END_COMMENT, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)    { in_comment = 0; ++line_no; continue;} 
    if (sal_strnstr(line, START_COMMENT, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)  { in_comment = 1; ++line_no; continue;}
...
void dnx_pemladrv_db_info_insert(const int unit, const char* line) {
  ...
  if (sscanf( line, "%s %s %s %d %d %d %d %d %d ", key_word, db_type, db_name, &db_id, &nof_entries, &key_width, &result_width, &nof_key_fields, &nof_result_fields) != PEM_NOF_DB_TOKEN) {
    ...
    if ((sal_strncmp(db_type, KEYWORD_DB_DIRECT, sizeof(db_type)) == 0)) {
      strncpy(api_info[unit].db_direct_container.db_direct_info_arr[db_id].name, db_name, sal_strnlen(db_name, MAX_NAME_LENGTH));
      api_info[unit].db_direct_container.db_direct_info_arr[db_id].name[sal_strnlen(db_name, MAX_NAME_LENGTH)] = '\0';
    ...
    if ((sal_strncmp(db_type, KEYWORD_DB_TCAM, sizeof(db_type)) == 0)) {
      sal_strncpy(api_info[unit].db_tcam_container.db_tcam_info_arr[db_id].name, db_name, sal_strnlen(db_name, MAX_NAME_LENGTH));
      api_info[unit].db_tcam_container.db_tcam_info_arr[db_id].name[sal_strnlen(db_name, MAX_NAME_LENGTH)] = '\0';
    ...
    if ((sal_strncmp(db_type, KEYWORD_DB_EM, sizeof(db_type)) == 0)) {
      sal_strncpy(api_info[unit].db_em_container.db_em_info_arr[db_id].logical_em_info.name, db_name, sal_strnlen(db_name, MAX_NAME_LENGTH));
      api_info[unit].db_em_container.db_em_info_arr[db_id].logical_em_info.name[sal_strnlen(db_name, MAX_NAME_LENGTH)] = '\0';
    ...
    if ((sal_strncmp(db_type, KEYWORD_DB_LPM, sizeof(db_type)) == 0)) {
      sal_strncpy(api_info[unit].db_lpm_container.db_lpm_info_arr[db_id].logical_lpm_info.name, db_name, sal_strnlen(db_name, MAX_NAME_LENGTH));
      api_info[unit].db_lpm_container.db_lpm_info_arr[db_id].logical_lpm_info.name[sal_strnlen(db_name, MAX_NAME_LENGTH)] = '\0';
...
void dnx_pemladrv_db_field_info_insert(const int unit, const char* line) {
  ...
  if (sscanf( line, "%s %s %s %d %s %d %d %d", key_word, db_name_debug, db_type, &db_id, field_name_debug, &field_id, &lsb_bit, &msb_bit) != PEM_NOF_KEY_FIELD_TOKEN) {    
    ...
    if ((sal_strncmp(key_word, KEYWORD_KEY, sizeof(key_word)) == 0))
      db_key_field_info_insert(unit, db_type, db_id, field_id, lsb_bit, msb_bit);
    else if((sal_strncmp(key_word, KEYWORD_RESULT, sizeof(key_word)) == 0))
...
void dnx_pemladrv_tcam_key_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  DbChunkMapper* chunk_info_ptr = (DbChunkMapper*)pemladrv_calloc(chunk_info_ptr, 1, sizeof(DbChunkMapper));
  db_id = build_cam_key_chunk_from_ucode(unit, line, chunk_info_ptr);
  if (db_id == UINT_MAX) {
    sal_printf("failed to insert chunk.\n");
    sal_free(chunk_info_ptr);
    return;
...
void dnx_pemladrv_tcam_result_chunk_insert(const int unit, const char* line) {
  unsigned int db_id;
  DbChunkMapper* chunk_info_ptr = (DbChunkMapper*)pemladrv_calloc(chunk_info_ptr, 1, sizeof(DbChunkMapper));
  db_id = build_cam_result_chunk_from_ucode(unit,line, chunk_info_ptr);
  if (db_id == UINT_MAX) {
    sal_printf("failed to insert chunk.\n");
    sal_free(chunk_info_ptr);
    return;
...
void dnx_pemladrv_init_logical_db_chunk_mapper(const int unit, const char* line){
  ...
  if ((sal_strncmp(line + KEYWORD_DB_SINGLE_CHUNCK_INFO_SIZE + 1, KEYWORD_DB_DIRECT, KEYWORD_DB_DIRECT_SIZE) == 0)) {
    ...
    if ((sal_strncmp(line + KEYWORD_DB_SINGLE_CHUNCK_INFO_SIZE + 1, KEYWORD_DB_TCAM, KEYWORD_DB_TCAM_SIZE) == 0)) {
    ...
    else if ((sal_strncmp(line + KEYWORD_DB_SINGLE_CHUNCK_INFO_SIZE + 1, KEYWORD_DB_EM, KEYWORD_DB_EM_SIZE) == 0)) {
      ...
      init_em_cache(unit, &api_info[unit].db_em_container.db_em_info_arr[db_id].em_cache);
    ...
    else if ((sal_strncmp(line + KEYWORD_DB_SINGLE_CHUNCK_INFO_SIZE + 1, KEYWORD_DB_LPM, KEYWORD_DB_LPM_SIZE) == 0)) {
      ...
      init_lpm_cache(unit, &api_info[unit].db_lpm_container.db_lpm_info_arr[db_id].lpm_cache);
...
void dnx_pemladrv_init_reg_field_mapper(const int unit, const char* line) {
  ...
  if (sscanf( line, "%s %d %d %d %d %d", key_word, &reg_id, &field_id, &lsb_bit, &msb_bit, &nof_mappings) != VIRTUAL_REGISTER_FIELD_NOF_MAPPINGS_TOKEN) {
    ...
    api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_info_arr[field_id].reg_field_mapping_arr = (RegFieldMapper*)pemladrv_calloc(api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_info_arr[field_id].reg_field_mapping_arr, nof_mappings, sizeof(RegFieldMapper));
  }
}
    ...
    const char* write_commands_file_name)
{
  ...
  while((sal_fgets(line, sizeof(line), fp) != NULL) && (ret_val >= 0)) {
    ...
    if (sal_strncmp(line, KEYWORD_DB_DIRECT_WRITE_COMMAND, KEYWORD_DB_DIRECT_WRITE_COMMAND_SIZE) == 0)          { db_direct_write_command(unit, line);  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_TCAM_WRITE_COMMAND, KEYWORD_DB_TCAM_WRITE_COMMAND_SIZE) == 0)              { db_tcam_write_command(unit, line);  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_DB_LPM_WRITE_COMMAND, KEYWORD_DB_LPM_WRITE_COMMAND_SIZE) == 0)                { db_lpm_write_command(unit, line);  ++line_no; continue; }
    
    if (sal_strncmp(line, KEYWORD_DB_EM_WRITE_COMMAND, KEYWORD_DB_EM_WRITE_COMMAND_SIZE) == 0)                  { db_em_write_command(unit, line);  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_REG_WRITE_COMMAND, KEYWORD_REG_WRITE_COMMAND_SIZE) == 0)                      { reg_write_command(unit, line);  ++line_no; continue; }
    if (sal_strncmp(line, KEYWORD_PROGRAM_SELECT_CONFIG_WRITE_COMMAND, KEYWORD_PROGRAM_SELECT_CONFIG_WRITE_COMMAND_SIZE) == 0)  { program_select_tcam_write_command(unit, line);  ++line_no; continue; }
