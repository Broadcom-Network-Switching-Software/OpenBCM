    ...
    pemladrv_mem_t * result)
{
  ...
  virtual_db_data_array     = (unsigned int*)pemladrv_calloc(virtual_db_data_array, (size_t)pemladrv_ceil(total_result_width_in_bits, PEM_CFG_API_MAP_CHUNK_WIDTH), sizeof(unsigned int));
...
unsigned int dnx_pemladrv_direct_info_allocate( const int unit, pemladrv_db_info** arr, int* size) {
  ...
  *arr = (pemladrv_db_info*)pemladrv_calloc(*arr, api_info[unit].db_direct_container.nof_direct_dbs, sizeof(pemladrv_db_info));
  ...
  for (direct_index = 0; direct_index < api_info[unit].db_direct_container.nof_direct_dbs; ++direct_index)
  {
    ...
    name_len = sal_strnlen(api_info[unit].db_direct_container.db_direct_info_arr[direct_index].name, MAX_NAME_LENGTH);
    (*arr)[mapped_diret_size].db_name = (char*)sal_alloc(name_len + 1, "");
    sal_strncpy((*arr)[mapped_diret_size].db_name, api_info[unit].db_direct_container.db_direct_info_arr[direct_index].name, name_len);
    ...
    pemladrv_mem_t * result)
{
  ...
  virtual_db_key_data_array    = (unsigned int*)pemladrv_calloc(virtual_db_key_data_array, (size_t)pemladrv_ceil(total_key_width_in_bits, UINT_WIDTH_IN_BITS), sizeof(uint32));
  virtual_db_result_data_array = (unsigned int*)pemladrv_calloc(virtual_db_result_data_array, (size_t)pemladrv_ceil(total_result_width_in_bits , UINT_WIDTH_IN_BITS), sizeof(uint32));
    ...
    int *index)
{
  ...
  *index = dnx_pemladrv_does_key_exists_in_physical_db(unit, &api_info[unit].db_em_container.db_em_info_arr[table_id].logical_em_info, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.physical_entry_occupation, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.em_key_entry_arr, virtual_key_data);
  if (*index != -1) {
    ...
    int *index)
{
  ...
  *index = dnx_pemladrv_find_physical_entry_index_by_key(unit,&api_info[unit].db_em_container.db_em_info_arr[table_id].logical_em_info, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.physical_entry_occupation, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.em_key_entry_arr, key);
  if (*index == -1) {
    ...
    int *index)
{
  ...
  *index = dnx_pemladrv_does_key_exists_in_physical_lpm_db(unit, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, uint_len_in_bits, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache, virtual_key_data);
  if (*index != -1) {
  ...
  if (dnx_pemladrv_is_lpm_full(unit, api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info.total_nof_entries ,api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache.physical_entry_occupation)) {
    
    return 1;
  }

  
  *index = dnx_pemladrv_find_lpm_index_to_insert(unit,length_in_bits, total_key_width_in_bits, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache);
...
pemladrv_lpm_remove(
    ...
    int *index)
{
  *index = dnx_pemladrv_find_physical_entry_index_by_key(unit,&api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache.physical_entry_occupation, api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache.lpm_key_entry_arr, key);
  if (*index == -1) {
...
pemladrv_lpm_lookup(
    ...
    int *index)
{
  *index = dnx_pemladrv_find_physical_lpm_entry_index_by_key(unit, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache, key);
  if (*index == -1) {
    ...
    const int table_id)
{
  ...
  int distance_to_free_apace_above = 0;
  int distance_to_free_apace_below = 0;
  int last_index_of_same_prefix_length = dnx_pemladrv_find_last_index_with_same_prefix(unit, index, api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache.lpm_key_entry_prefix_length_arr);
    ...
    const int table_id)
{
    ...
    key        = (pemladrv_mem_t*)pemladrv_calloc(key, 1, sizeof(pemladrv_mem_t));
    result     = (pemladrv_mem_t*)pemladrv_calloc(result, 1, sizeof(pemladrv_mem_t));

    key->nof_fields              = 1;
    key->fields                  = (pemladrv_field_t**)pemladrv_calloc(key->fields, 1, sizeof(pemladrv_field_t*));
    key->fields[0]               = (pemladrv_field_t*)pemladrv_calloc(key->fields[0], 1, sizeof(pemladrv_field_t));
    key->fields[0]->field_id     = 0;
    key->fields[0]->fldbuf       = (unsigned int*)pemladrv_calloc(key->fields[0]->fldbuf, 2, sizeof(unsigned int));
    result->nof_fields           = 1;
    result->fields               = (pemladrv_field_t**)pemladrv_calloc(result->fields, 1, sizeof(pemladrv_field_t*));
    result->fields[0]            = (pemladrv_field_t*)pemladrv_calloc(result->fields[0], 1, sizeof(pemladrv_field_t));
    result->fields[0]->field_id  = 0;
    result->fields[0]->fldbuf    = (unsigned int*)pemladrv_calloc(result->fields[0]->fldbuf, 2, sizeof(unsigned int));
    ...
    const int table_id)
{
  ...
  if (shift_direction == UP) {
    *index = *index - 1;                   
    next_free_entry = dnx_pemladrv_find_next_lpm_free_entry(unit,*index, UP, api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache.physical_entry_occupation);
    ...
    next_free_entry = dnx_pemladrv_find_next_lpm_free_entry(unit,*index, DOWN, api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache.physical_entry_occupation);
    for(index_to_write_to = next_free_entry; index_to_write_to != *index; --index_to_write_to)     
      dnx_pemladrv_move_physical_lpm_entry(unit, index_to_write_to - 1, index_to_write_to, table_id);
    return 0;
  }
    ...
    const unsigned int nof_fields)
{
  unsigned int field_index;
  pemladrv_mem_t* pem_mem_access;
  pem_mem_access = (pemladrv_mem_t*)pemladrv_calloc(pem_mem_access, 1, sizeof(pemladrv_mem_t));

  pem_mem_access->nof_fields = (uint16)nof_fields;
  pem_mem_access->fields     = (pemladrv_field_t**)pemladrv_calloc(pem_mem_access->fields, nof_fields, sizeof(pemladrv_field_t*));
  for (field_index = 0; field_index < nof_fields; field_index++)
  {
    pem_mem_access->fields[field_index] = (pemladrv_field_t*)pemladrv_calloc(pem_mem_access->fields[field_index], 1, sizeof(pemladrv_field_t));
  }
...
#endif 
  pem_field_access->fldbuf = (unsigned int*)pemladrv_calloc(pem_field_access->fldbuf, nof_entries, sizeof(unsigned int));
  return;
}
    ...
    void* entry_data)
{
    ...
    if(!dnx_data_field.features.tcam_cs_is_tcam_direct_access_get(unit))
    {
        ...
        rv = adapter_memreg_access(unit, (int) mem->block_identifier, tcam_mask_line_address,
                                     BITS2BYTES(mem->mem_width_in_bits), 1, 0, 0, data);
#else
        rv = soc_direct_memreg_set(unit, (int) mem->block_identifier,
                              tcam_mask_line_address, BITS2WORDS(mem->mem_width_in_bits), 1, data);
#endif
        if(rv)
        {
            return rv;
...
#ifdef ADAPTER_SERVER_MODE
        rv = adapter_memreg_access(unit, (int) mem->block_identifier, tcam_mask_line_address,
                                     BITS2BYTES(mem->mem_width_in_bits), 1, 1, 0, data);
#else
        rv = soc_direct_memreg_get(unit, (int) mem->block_identifier,
                              tcam_mask_line_address, BITS2WORDS(mem->mem_width_in_bits), 1, data);
#endif
        if(rv)
        {
        ...
        data[0] = 2;
#ifdef ADAPTER_SERVER_MODE
        rv = adapter_memreg_access(unit, (int) mem->block_identifier, tcam_key_line_address,
                                     BITS2BYTES(mem->mem_width_in_bits), 1, 0, 0, data);
#else
        rv = soc_direct_memreg_set(unit, (int) mem->block_identifier,
                              tcam_key_line_address, BITS2WORDS(mem->mem_width_in_bits), 1, data);
#endif
        if(rv)
        {
            return rv;
    ...
    void *entry_data)
{
    ...
    if (!currently_writing_applets)
    {
        ...
        else
        {
            ...
            return adapter_memreg_access(unit, (int) mem->block_identifier, mem->mem_address,
                                         BITS2BYTES(mem->mem_width_in_bits), is_mem, 1, 0, entry_data);
#else
  ...
  if (-1 == entry_in_cache) {
#ifdef ADAPTER_SERVER_MODE
    return adapter_memreg_access(unit, (int)mem->block_identifier, mem->mem_address, BITS2BYTES(mem->mem_width_in_bits), is_mem, 1, 0, entry_data);
    ...
    void *entry_data)
{
  ...
  int applet_mem_size_in_bits = api_info[unit].applet_info.applet_mem_info[0][0].length_in_bits;
    ...
    {   
        return adapter_memreg_access(unit, (int) mem->block_identifier, mem->mem_address,
                                     BITS2BYTES(mem->mem_width_in_bits), is_mem, 0, 0, entry_data);
    }
#elif BCM_DNX_SUPPORT
    if (!currently_writing_applets)
    {   
  ...
  if (-1 == entry_in_cache) {
    assert(nof_entries_in_cache < APPLET_MEM_NOF_ENTRIES);  
        ...
        api_info[unit].applet_info.applet_mem_cache[core][is_ingress0_egress1][nof_entries_in_cache] =
          (uint32 *) pemladrv_calloc(api_info[unit].applet_info.applet_mem_cache[is_ingress0_egress1][nof_entries_in_cache], applet_mem_size_in_bits / (sizeof(uint32) + 1), sizeof(uint32) + 1);
#else
