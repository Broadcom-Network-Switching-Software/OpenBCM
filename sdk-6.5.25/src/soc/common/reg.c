...
)
{
    ...
    access_regmem_id_t regmem_id = /* get the access regmem ID of the register */
      (unsigned)reg >= ACCESS_NOF_MCM_MAPPED_REGS ? ACCESS_IVALID_REGMEM_ID : access_mcm2access_reg_ids[(unsigned)reg];
    if (regmem_id == ACCESS_IVALID_REGMEM_ID) {
        return SOC_E_PARAM;
    }
...
soc_reg64_get(int unit, soc_reg_t reg, int port, int index, uint64 *data)
{
    ...
    if (SOC_IS_NEW_ACCESS_INITIALIZED(unit)) {
        uint32 converted_data[2];
        int rv = access_regmem_wrapper(unit, 0, reg, port, index, converted_data);
        COMPILER_64_SET(*data, converted_data[0], converted_data[1]);
        return rv;
    }
#endif /* BCM_ACCESS_SUPPORT */
...
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_J2P(unit) && (SOC_REG_BLOCK_IS(unit, reg, SOC_BLK_ECI)) && (addr < 256) && (j2p_common_regs_addr_2_global_register_block_registers[addr] != NULL))
    {
        int i = 0;
        ...
        for (i = 0; j2p_common_regs_addr_2_global_register_block_registers[addr][i] != -1; i++)
        {
            soc_reg_t global_reg = j2p_common_regs_addr_2_global_register_block_registers[addr][i];
            rv = soc_reg_dnx_global_reg_set(unit, global_reg, data_above_64);
            if (rv != SOC_E_NONE) {
                LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
...
/* For global registers in devices that need it, the CPU writes to all blocks instead of ASIC */
        ...
        if (SOC_IS_J2P(unit) && (SOC_REG_BLOCK_IS(unit, reg, SOC_BLK_ECI)) && (addr < 256) && (j2p_common_regs_addr_2_global_register_block_registers[addr] != NULL))
        {
            ...
            {
                soc_reg_t global_reg = j2p_common_regs_addr_2_global_register_block_registers[addr][i];
                rv = soc_reg_dnx_global_reg_set(unit, global_reg, data);
                if (rv != SOC_E_NONE) {
                    LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                    "soc_reg_dnx_global_reg_set: failed to set global register")));
                    return rv;
