...
#define CMICM_SCHAN_OP_RETRY_AVAIL(opcode) \
            (opcode == READ_MEMORY_CMD_MSG    || \
             ...
             opcode == TABLE_INSERT_CMD_MSG   || \
             opcode == TABLE_DELETE_CMD_MSG   || \
                   ...
                   int dwc_read, uint32 flags)
{
    ...
    int i, j, rv;
    int cmc;
    int retry_times = CMICM_SCHAN_OP_RETRY_TIMES;
...
retry:
    do {
        ...
        if (msg->header.v4.opcode != opcode) {
            if (rv == SOC_E_NONE) {
                break;
            }
            msgAddr = CMIC_COMMON_CMCx_SCHAN_MESSAGEn(cmc, 0);
            resp_msg.dwords[0] = soc_pci_read(unit, msgAddr);
            msgAddr = CMIC_COMMON_CMCx_SCHAN_MESSAGEn(cmc, 1);
            resp_msg.dwords[1] = soc_pci_read(unit, msgAddr);
            if (resp_msg.header.v4.nack) {
                if (soc_feature(unit, soc_feature_new_sbus_format) &&
                    !soc_feature(unit, soc_feature_new_sbus_old_resp)) {
                    if (resp_msg.genresp_v2.response.type != SCHAN_GEN_RESP_TYPE_ERROR) {
                        rv = SOC_E_NONE;
                    }
                } else {
                    if (resp_msg.genresp.response.type != SCHAN_GEN_RESP_TYPE_ERROR) {
    ...
    if (rv == SOC_E_TIMEOUT) {
        if (retry_times-- && CMICM_SCHAN_OP_RETRY_AVAIL(msg->header.v4.opcode)) {
            soc_timeout_t to;
            uint32 schanCtrl, ctrlAddr;
            if (msg->header.v4.opcode == TABLE_INSERT_CMD_MSG) {
                msg->header.v4.opcode = TABLE_DELETE_CMD_MSG;
            }
            /* Try to abort the channel */
            _soc_cmicm_schan_reset(unit, cmc, 0);

            /* Check if channel is aborted */
            soc_timeout_init(&to, SOC_CONTROL(unit)->schanTimeout, 100);
            ctrlAddr = CMIC_COMMON_CMCx_SCHAN_CTRL_OFFSET(cmc);
            rv = SOC_E_NONE;
            while (((schanCtrl = soc_pci_read(unit, ctrlAddr)) & SC_CMCx_MSG_DONE) == 0) {
            ...
            if (rv == SOC_E_NONE) {
                LOG_WARN(BSL_LS_SOC_SCHAN,
                    (BSL_META_U(unit,
                     "The current operation (0x%x) timed out, schedule retry ...\n"), opcode));
                goto retry;
            }
        }
    ...
    } else if (retry_times != CMICM_SCHAN_OP_RETRY_TIMES) {
        if (rv == SOC_E_NONE) {
            if (msg->header.v4.opcode == TABLE_DELETE_CMD_MSG &&
                msg->header.v4.opcode != opcode) {
                msg->header.v4.opcode = TABLE_INSERT_CMD_MSG;
                goto retry;
            }
        } else {
            if (opcode == TABLE_DELETE_CMD_MSG) {
                if (msg->header.v4.nack) {
                    if (soc_feature(unit, soc_feature_new_sbus_format) &&
                        !soc_feature(unit, soc_feature_new_sbus_old_resp)) {
                        if (msg->genresp_v2.response.type == SCHAN_GEN_RESP_TYPE_NOT_FOUND) {
                            msg->genresp_v2.response.type = SCHAN_GEN_RESP_TYPE_DELETED;
                            msg->header.v4.nack = 0;
                            rv = SOC_E_NONE;
                        }
                    } else {
                        if (msg->genresp.response.type == SCHAN_GEN_RESP_TYPE_NOT_FOUND) {
                            msg->genresp.response.type = SCHAN_GEN_RESP_TYPE_DELETED;
                            msg->header.v4.nack = 0;
                            rv = SOC_E_NONE;
                        }
                    }
        ...
        if (rv == SOC_E_NONE) {
            LOG_WARN(BSL_LS_SOC_SCHAN,
                     (BSL_META_U(unit, "The operation (0x%x) done with %d time(s) retry\n"),
                      opcode, CMICM_SCHAN_OP_RETRY_TIMES - retry_times));
