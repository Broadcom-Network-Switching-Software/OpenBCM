...
/*
 * Function:
 *      soc_counter_sync_req_get
 * Purpose:
 *      Read value of 'soc_control[unit]->counter_sync_req' for specified unit.
 * Parameters:
 *      unit - Device unit #
 * Returns:
 *      int - Value of 'soc_control[unit]->counter_sync_req'. If 'unit' is illegal then
 *      return '-1'
 */
STATIC int
soc_counter_sync_req_get(int unit)
{
    soc_control_t *soc;
    int ret;
    ...
    {
        ret = soc->counter_sync_req;
    }
    return (ret);
}
/*
 * Function:
 *      soc_counter_sync_req_set
 * Purpose:
 *      Set 'value' into 'soc_control[unit]->counter_sync_req' of specified unit.
 * Parameters:
 *      unit -
 *          Device unit #
 *      value -
            Value to load into 'soc_control[unit]->counter_sync_req'
 * Returns:
 ...
 */
STATIC int
soc_counter_sync_req_set(int unit, int value)
{
    soc_control_t *soc;
    int ret;
    ...
    {
        ret = 0;
        soc->counter_sync_req = value;
    }
    return (ret);
}
#define SOC_COUNTER_SYNC_REQ_GET(unit) \
    soc_counter_sync_req_get(unit)
#define SOC_COUNTER_SYNC_REQ_SET(unit, value) \
    soc_counter_sync_req_set(unit,value)
/* } */
#else
/* { */
#define SOC_COUNTER_SYNC_REQ_GET(unit) \
    SOC_CONTROL(unit)->counter_sync_req
#define SOC_COUNTER_SYNC_REQ_SET(unit, value) \
    SOC_CONTROL(unit)->counter_sync_req = value
/* } */
#endif
                                   ...
                                   int *non_dma_entries)
{
    ...
    *non_dma2 = *non_dma0;
    ...
    non_dma0 = &soc->counter_non_dma[SOC_COUNTER_NON_DMA_QGROUP_MIN_CURRENT -
                                     SOC_COUNTER_NON_DMA_START];
    non_dma0->flags = _SOC_COUNTER_NON_DMA_VALID |
        _SOC_COUNTER_NON_DMA_DO_DMA | _SOC_COUNTER_NON_DMA_CURRENT;
    non_dma0->pbmp = PBMP_ALL(unit);
    non_dma0->base_index = non_dma_start_index + *non_dma_entries;
    non_dma0->entries_per_port = 1;
    non_dma0->num_entries =
        soc_mem_index_count(unit, MMU_THDU_XPIPE_COUNTER_QGROUPm) +
        soc_mem_index_count(unit, MMU_THDU_YPIPE_COUNTER_QGROUPm);
    non_dma0->mem = MMU_THDU_XPIPE_COUNTER_QGROUPm;
    non_dma0->reg = INVALIDr;
    non_dma0->field = MIN_COUNTf;
    non_dma0->cname = "QGROUP_MIN_CURRENT";
    non_dma0->dma_mem[0] = MMU_THDU_XPIPE_COUNTER_QGROUPm;
    non_dma0->dma_mem[1] = MMU_THDU_YPIPE_COUNTER_QGROUPm;
    non_dma0->dma_buf[0] = buf;
    non_dma0->dma_buf[1] =
        &buf[soc_mem_index_count(unit, non_dma0->dma_mem[0]) *
             soc_mem_entry_words(unit, non_dma0->dma_mem[0])];
 ...
 /*we may not need separate saber2_non_dma() function */
    ...
    if (soc->blk_ctr_desc_count) {
        ...
        for (idx = 0; idx < soc->blk_ctr_desc_count; idx++) {
            ctr = 0;
            while ((int)(soc->blk_ctr_desc[idx].desc[ctr].reg) != INVALIDr) {
...
#if defined (BCM_MONTEREY_SUPPORT)
int
_soc_counter_monterey_non_dma_pbmp_update(int unit)
{
    int id, i;
    soc_control_t *soc = SOC_CONTROL(unit);
    soc_counter_non_dma_t *non_dma;
    ...
    for (i = 0; i < sizeof(soc_counter_non_dma3)/sizeof(int); ++i) {
        id = soc_counter_non_dma3[i];
        non_dma = &soc->counter_non_dma[id - SOC_COUNTER_NON_DMA_START];
        non_dma->pbmp = PBMP_ROE_ALL(unit);
    }
...
#ifdef BCM_MONTEREY_SUPPORT
    if (SOC_IS_MONTEREY(unit)) {
        SOC_IF_ERROR_RETURN(_soc_counter_monterey_non_dma_pbmp_update(unit));
        return SOC_E_NONE;
    }
#endif /* BCM_MONTEREY_SUPPORT */
#ifdef BCM_APACHE_SUPPORT
...
soc_ctr_evict_start(int unit, uint32 flags, sal_usecs_t interval)
{
    ...
    soc->ctr_evict_enable = soc_property_get(unit, "ctr_evict_enable", 1);
    if (!soc->ctr_evict_enable) {
        LOG_CLI((BSL_META_U(unit,
                 ...
                 int ar_idx, uint64 val64)
{
    ...
    if ((SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit))&&
        ...
        if (!soc_feature(unit, soc_feature_configure_loopback_port) &&
            IS_LB_PORT(unit, port)) {
            /* Do nothing for LB port if feature not supported */
            return SOC_E_NONE;
        }
...
int
soc_controlled_counter_clear_by_counter_idx(int unit, soc_port_t port, int counter_idx)
{
    soc_control_t       *soc = SOC_CONTROL(unit);
    uint64              ctr_new;
    int                 port_base;
    ...
    ctr = &soc->controlled_counters[counter_idx];
...
soc_counter_non_dma_ready(int unit, soc_counter_non_dma_t *non_dma, int *ctr_dma_ct)
{
    ...
    if (SOC_COUNTER_SYNC_REQ_GET(unit)) {
                    ...
                    for (i = 0; i < count; i++) {
                        ...
                        if (SOC_IS_MONTEREY(unit) &&
                            ...
                            if ((port >=0) && (IS_ROE_PORT(unit, port))) {
                                ...
                                /* Non RoE port, CPRI counters are zero out. */
                                COMPILER_64_ZERO(ctr_new);
                            }
                        } else
#endif
...
#define _XLPMIBS "XLPMIBS"
    ...
    if (soc->blk_ctr_desc_count) {
        ...
        for (bindex = 0; bindex < soc->blk_ctr_desc_count; bindex++) {
            cnt = 0;
            while ((int)(soc->blk_ctr_desc[bindex].desc[cnt].reg) != INVALIDr) {
                cnt++;
            }
            if (cnt) {
                uint8 at;
        ...
        for (bindex = 0; bindex < soc->blk_ctr_desc_count; bindex++) {
            if (_blk_ctr_process[unit][bindex]) {
                ...
                if (ctr_process->blk == blk) {
                    ...
                    for (rindex = 0; (int)(reg_desc[rindex].reg) != INVALIDr;
                         rindex++) {
...
soc_counter_thread(void *unit_vp)
{
    ...
    while ((interval = soc->counter_interval) != 0) {
        ...
        if (dma && soc_feature(unit, soc_feature_sbusdma)) {
            ...
            PBMP_ITER(soc->counter_pbmp, port) {
                ...
                if (-1 == phy_port) {
                    LOG_ERROR(BSL_LS_SOC_COMMON,
                            (BSL_META_U(unit,
                                "Unmapped port in counter thread: %d, phy_port %d\n"),
                                port, phy_port));
                    continue;
                }
                blk = SOC_PORT_BLOCK(unit, phy_port);
        ...
        while (1) {
            /* coverity[double_lock:FALSE] */
            COUNTER_ATOMIC_BEGIN(s);
            if (!(_soc_counter_pending[unit] && soc->counter_interval)) {
                COUNTER_ATOMIC_END(s);
                break;
            }
            COUNTER_ATOMIC_END(s);
        ...
        if (sync_gnt) {
            SOC_COUNTER_SYNC_REQ_SET(unit,0);
            sync_gnt = 0;
        }
    }
...
soc_counter_start(int unit, uint32 flags, int interval, pbmp_t pbmp)
{
    ...
    PBMP_ITER(soc->counter_pbmp, p) {
        ...
        /* The counter thread has been told to start but it found
         * an unmapped port. Remove it from the port bitmap
         * For dnx, FlexE client is supported by mib, but this
         * soc info == -1 */
#ifndef BCM_DNX_SUPPORT
        if (-1 == SOC_INFO(unit).port_l2p_mapping[p]) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                        "Unmapped port starting counter thread, removing %d\n"), p));
            SOC_PBMP_PORT_REMOVE(soc->counter_pbmp, p);
        }
#endif
    }
...
/*
 * Function:
 *      soc_counter_port_collect_enable_get
 * Purpose:
 *      Get the state of gathering of MIB statistics on this port.
 * Parameters:
 *      unit - StrataSwitch unit #.
 *      port - Port number #.
 *      enable: port's mib gathering status.
 * Returns:
 *      SOC_E_XXX
 */
int
soc_counter_port_collect_enable_get(int unit, int port, int * enable)
{
    soc_control_t       *soc = SOC_CONTROL(unit);
    ...
    *enable = SOC_PBMP_MEMBER(soc->counter_pbmp, port) ? 1 : 0;
...
soc_counter_sync(int unit)
{
    ...
    SOC_COUNTER_SYNC_REQ_SET(unit,TRUE);
    ...
    while (SOC_COUNTER_SYNC_REQ_GET(unit)) {
        if (soc_timeout_check(&to)) {
            if (SOC_COUNTER_SYNC_REQ_GET(unit)) {
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      "soc_counter_sync: counter thread not responding\n")));
                SOC_COUNTER_SYNC_REQ_SET(unit,FALSE);
                                            ...
                                            soc_reg_t ctr_reg)
{
    ...
    for (; id < non_dma_counter_end; id++) {
        ...
        /* If signaled to exit then return  */
        if (!soc->counter_interval) {
            /* We support sync_mode when counter eviction is disabled */
            if (sync_mode == 0 || soc->ctr_evict_enable) {
                return;
            }
        }
