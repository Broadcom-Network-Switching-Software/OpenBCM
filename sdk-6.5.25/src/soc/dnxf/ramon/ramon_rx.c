    ...
    int *cpu_addresses)
{
    ...
    for (cpu_addr_index = 0; cpu_addr_index < num_of_cpu_addresses; ++cpu_addr_index)
    {
        ...
        soc_reg_field_set(unit, eci_fe_global_fe_dest_ids[reg_index], &reg_val32, fe_dest_id[cpu_addr_index],
                          cpu_addresses[cpu_addr_index]);
    ...
    for (cpu_addr_index = num_of_cpu_addresses; cpu_addr_index < dnxf_data_fabric.cell.rx_max_nof_cpu_buffers_get(unit);
         ++cpu_addr_index)
    {
        reg_index = cpu_addr_index / nof_fields_in_reg;
