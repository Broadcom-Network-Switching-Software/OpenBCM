    ...
    int *aligned_lane_id)
{
    ...
    int fmac_blk_id, inner_link, fmac_nof_links;
    SHR_FUNC_INIT_VARS(unit);
        ...
        dnxf_data_device.general.feature_get(unit, dnxf_data_device_general_is_not_all_cores_active))
    {
        ...
        if (!is_core_zero_active)
        {
            SHR_IF_ERR_EXIT(soc_dnxf_drv_fmac_lane_to_block_mapping
                            (unit, lane_id, &fmac_blk_id, &inner_link, SOC_BLK_FMAC));
            fmac_nof_links = dnxf_data_device.blocks.nof_links_in_fmac_get(unit);

            if (fmac_blk_id < dnxf_data_device.blocks.nof_instances_fmac_get(unit) / 2)
            {
                *aligned_lane_id = lane_id - fmac_nof_links;
            }
            else
            {
                *aligned_lane_id = lane_id + fmac_nof_links;
