...
/* LTM4676A an integrated power control IC */
extern i2c_driver_t _soc_i2c_ltm4676A_driver;
...
STATIC i2c_device_t i2c_56175_devices[] = {
    I2CDEV(LPT, 1, pcf8574),      /* I2C address 0x27 (board ID) */
    I2CDEVTD(NVRAM, 1, eep24c64), /* I2C address 0x54 (EEPROM) */
    I2CDEV(MUX, 3, pca9548),      /* I2C address 0x73 (mux 3) */
    I2CDEV(LTM4676A, 40, ltm4676A),
    I2CDEV(LTC2974, 63, ltc2974),
    I2CDEV(LTC2974, 64, ltc2974),
};
...
soc_i2c_dev_driver_get(int unit, char *drv_name, i2c_driver_t **drv)
{
    ...
    for (i = 0; i < COUNTOF(driver_mapping_info); i++) {
    ...
    for (i = 0; i < MAX_I2C_DEVICES; i++) {
        if (i2c_custom_devices[unit][i] == NULL) {
            ...
            sal_memcpy(i2c_custom_devices[unit][i], device,
                       sizeof(i2c_device_t));
            i2c_custom_devices[unit][i]->devname = sal_strdup(device->devname);
            if (!i2c_custom_devices[unit][i]->devname) {
                sal_free(i2c_custom_devices[unit][i]);
                i2c_custom_devices[unit][i] = NULL;
                return SOC_E_MEMORY;
            }
            rv = SOC_E_NONE;
    ...
    if (num_custom_i2c_devices[unit]) {
        ...
        switch (board_id) {
            ...
            case 0xC2: i2c_devices = i2c_56070_devices;
                       ...
                       break; /* 56070K */
            case 0xCD:
            case 0xCE: i2c_devices = i2c_56175_devices;
                       num_i2c_devices = COUNTOF(i2c_56175_devices);
                       break; /* 56175K */
