                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    */
    ...
    /* coverity[mixed_enums]: ignore */
    core_init_config->interface.interface_type = (phymod_interface_t) soc_if;
    /* 
        PHY configuration
    */
...
phy_8806x_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
    ...
    /* coverity[mixed_enums]: ignore */
    interface_config.interface_type = (phymod_interface_t) pif;
...
phy_8806x_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    /* coverity[mixed_enums]: ignore */
    *pif = (soc_port_if_t) interface_config.interface_type;
...
STATIC int
phy_8806x_pause_addr_set(int unit, soc_port_t port, bcm_mac_t mac)
{
    phy_ctrl_t                  *pc;
    soc_phymod_ctrl_t           *pmc;
    phymod_phy_access_t          pm_phy_copy;
    soc_phymod_phy_t            *phy;
    phymod_pause_addr_t          pause_addr;
    ...
    sal_memcpy(&pm_phy_copy, &phy->pm_phy, sizeof(pm_phy_copy));
    sal_memcpy(pause_addr.mac, mac, 6);
    ...
    SOC_IF_ERROR_RETURN
        (phymod_port_pause_addr_set(&pm_phy_copy, pause_addr));
...
STATIC int
phy_8806x_pause_addr_get(int unit, soc_port_t port, bcm_mac_t mac)
{
    phy_ctrl_t                *pc;
    soc_phymod_ctrl_t         *pmc;
    phymod_phy_access_t        pm_phy_copy;
    ...
    SOC_IF_ERROR_RETURN
        (phymod_port_pause_addr_get(&pm_phy_copy, &pause_addr));
    sal_memcpy(mac, pause_addr.mac, 6);
...
phy_8806x_sbus_cmd(int unit,phy_ctrl_t *pc, uint32 reg, uint32 cmd, int ring,  uint32 arg[])
{
    ...
    if ((status & (SBUS_STATUS_ERR | SBUS_STATUS_NAK)) != 0) {
        LOG_CLI((BSL_META_U(unit,
                            ...
                            "cmd=0x%04x reg=0x%08x ring=%d status=%04x "), unit, pc->port, cmd, reg, ring, status));
...
phy_driver_t phy_8806xdrv = {
    ...
    /* .pd_timesync_tx_info_get       = */ NULL,
    /* .pd_diag_stats_get             = */ NULL,
    /* .pd_pause_addr_set             = */ phy_8806x_pause_addr_set,
    /* .pd_pause_addr_get             = */ phy_8806x_pause_addr_get
};
