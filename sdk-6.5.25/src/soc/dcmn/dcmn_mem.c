                           ...
                           unsigned operation) 
{
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk) {
        ...
        if (soc_sand_indirect_mem_access_block_instance_lock(unit, blk) != SOC_E_NONE) {
            goto done;
        }
        ...
        *data32 = 0;
        ...
        while (words_left > 0)
        {
            ...
            if (operation == 1) { 
                ...
                sal_memcpy(entry_words, rd_data, (words_left > (indirect_size/32))? indirect_size/8 : words_left*4);
            }
        ...
        need_to_unlock_inst = 0;
        if (soc_sand_indirect_mem_access_block_instance_unlock(unit, blk) != SOC_E_NONE) {
            goto done;
        }
    ...
    if (need_to_unlock_inst) {
        soc_sand_indirect_mem_access_block_instance_unlock(unit, blk);
    }
    return rv;
}
