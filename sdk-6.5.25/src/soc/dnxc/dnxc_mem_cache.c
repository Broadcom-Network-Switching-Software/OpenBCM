    ...
    int unit)
{
    ...
    const dnx_data_intr_ser_dynamic_mem_t *dynamic_mem;
    int mem_index;
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        table_info = dnx_data_intr.ser.dynamic_mem_info_get(unit);

        for (mem_index = 0; mem_index < table_info->key_size[0]; mem_index++)
        {
            dynamic_mem = dnx_data_intr.ser.dynamic_mem_get(unit, mem_index);
            if (SOC_MEM_IS_VALID(unit, dynamic_mem->mem))
            {
                SOC_MEM_INFO(unit, dynamic_mem->mem).flags |= SOC_MEM_FLAG_SIGNAL;
                LOG_DEBUG(BSL_LOG_MODULE,
                          (BSL_META_U(unit, "Force %s dynamic\n"), SOC_MEM_NAME(unit, dynamic_mem->mem)));
            }
        }
    }
#endif
