...
void dnxc_sw_state_wb_module_size_update(int unit, uint32 module_id, uint32 allocated_size, uint32 flags){
    
    
    if (flags == 0)
    {
        dnxc_sw_state_data_block_header[unit]->module_sizes[module_id]-= allocated_size;
    }
    else
    {
        dnxc_sw_state_data_block_header[unit]->module_sizes[module_id]+= allocated_size;
...
uint32 dnxc_sw_state_wb_module_size_get(int unit, uint32 module_id){

    return dnxc_sw_state_data_block_header[unit]->module_sizes[module_id];
