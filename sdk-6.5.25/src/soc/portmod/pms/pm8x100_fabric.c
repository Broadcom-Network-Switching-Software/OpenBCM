...
/* WB defines */
#define PM8X100_FABRIC_LANE_IS_BYPASSED_SET(unit, pm_info, is_bypass, lane) \
    SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_lane_is_bypassed], &is_bypass, lane)
#define PM8X100_FABRIC_LANE_IS_BYPASSED_GET(unit, pm_info, is_bypass, lane) \
...
#define PM8X100_FABRIC_PLL_ACTUAL_LANES_BITMAP_SET(unit, pm_info, bitmap, pll_index) \
    SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_actual_lanes_bitmap], &bitmap, pll_index)

#define PM8X100_FABRIC_PLL_ACTUAL_LANES_BITMAP_GET(unit, pm_info, bitmap, pll_index) \
    SOC_WB_ENGINE_GET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_actual_lanes_bitmap], bitmap, pll_index)
...
#define PM8X100_FABRIC_RS_FEC_SYNC_MACHINE_AM_PERIOD    (4096)
#define PM8X100_FABRIC_RS_FEC_SYNC_MACHINE_COMPARE_CNT  (2)
#define PM8X100_FABRIC_RS_FEC_SYNC_MACHINE_NIBBLE_CNT   (9)

#define PM8X100_FABRIC_FEC_SYNC_MACHINE_N1 (10)
#define PM8X100_FABRIC_FEC_SYNC_MACHINE_N2 (26)
#define PM8X100_FABRIC_FEC_SYNC_MACHINE_N3 (100)
#define PM8X100_FABRIC_FEC_SYNC_MACHINE_K1 (1)
#define PM8X100_FABRIC_FEC_SYNC_MACHINE_K2 (2)
#define PM8X100_FABRIC_FEC_SYNC_MACHINE_K3 (2)

/* Async FIFO configurations that depend on FEC - regular mode */
#define PM8X100_FABRIC_FEC_NONE_SLOW_READ_RATE     (1)
#define PM8X100_FABRIC_FEC_NONE_FMAL_WIDTH         (2)
...
#define PM8X100_FABRIC_FEC_RS_FMAL_WIDTH           (0)

/* Async FIFO configurations that depend on FEC - retimer mode with FEC disabled */
#define PM8X100_FABRIC_FEC_NONE_FMAL_WIDTH_RETIMER_FEC_DISABLED        (0)
#define PM8X100_FABRIC_FEC_BASE_R_SLOW_READ_RATE_RETIMER_FEC_DISABLED  (1)
#define PM8X100_FABRIC_FEC_BASE_R_FMAL_WIDTH_RETIMER_FEC_DISABLED      (0)

#define PM8X100_FABRIC_MAX_NUM_PLLS (2)

#define PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)  \
    for (pll_index = end_pll - 1; pll_index >= begin_pll; --pll_index)

#define PM8X100_FABRIC_PLLS_INIT(pll_vco_rate_arr, pll_active_lanes_bitmap_arr) \
        do {\
            int pll; \
            for (pll = 0; pll < PM8X100_FABRIC_MAX_NUM_PLLS; ++pll) \
            { \
...
#define PM8X100_FABRIC_PLL_IS_EMPTY(pll_active_lanes_bitmap) \
    (pll_active_lanes_bitmap == 0)

/* is src_bitmap fully included in dst_bitmap */
#define PM8X100_FABRIC_BITMAP_IS_INCLUDED(src_bitmap, dst_bitmap) \
    (((src_bitmap) & (dst_bitmap)) == (src_bitmap))

#define PM8X100_FABRIC_PLL_INDEX_DEFAULT_START   (0)
#define PM8X100_FABRIC_PLL_INDEX_DEFAULT_END     (2)
#define PM8X100_FABRIC_PREFERRED_PLL_INDEX       (1)
...
STATIC
int pm8x100_fabric_port_retimer_fec_enable_get(int unit, int port, pm_info_t pm_info, int* enable);

/*
 * Get the VCO rates from given PCS bypassed port speed
 */
STATIC
int pm8x100_shared_pcs_bypassed_vco_get(int unit, int speed, portmod_vco_type_t* vco)
{
...
int pm8x100_fabric_fmac_blk_ids_get(int unit, int port, pm_info_t pm_info, uint32* fmac_blk_ids, int* nof_fmac_blks, int* first_fmac_index)
{
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
...
STATIC
int pm8x100_fabric_port_retimer_set(int unit, soc_port_t port, pm_info_t pm_info, int is_remote_loopback, int enable);

STATIC
int pm8x100_fabric_port_retimer_get(int unit, soc_port_t port, pm_info_t pm_info, int *is_remote_loopback, int *enable);
...
int pm8x100_fabric_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "lane_is_bypassed", wb_buffer_index, sizeof(uint32), NULL, PM8X100_FABRIC_MAX_PORTS_PER_PM, VERSION(1));
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    SOCDNX_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "pll_actual_lanes_bitmap", wb_buffer_index, sizeof(uint32), NULL, PM8X100_FABRIC_MAX_NUM_PLLS, VERSION(1));
    SOCDNX_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[wb_pll_actual_lanes_bitmap] = wb_var_id;
...
int pm8x100_fabric_pm_init(int unit, const portmod_pm_create_info_internal_t* pm_add_info, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    uint32 pll_actual_lanes_bitmap = 0; /* no lanes on the PLL */
    ...
    SOCDNX_IF_ERR_EXIT(phymod_firmware_load_method_t_validate(pm_add_info->pm_specific_info.pm8x100_fabric.fw_load_method));
    ...
    fabric_data->is_rx_ctrl_bypass_supported = pm_add_info->pm_specific_info.pm8x100_fabric.is_rx_ctrl_bypass_supported;
    fabric_data->force_single_pll0 = pm_add_info->pm_specific_info.pm8x100_fabric.force_single_pll0;
    fabric_data->force_single_pll1 = pm_add_info->pm_specific_info.pm8x100_fabric.force_single_pll1;
    fabric_data->native_single_pll = pm_add_info->pm_specific_info.pm8x100_fabric.native_single_pll;
    fabric_data->clock_buffer_disable_required = pm_add_info->pm_specific_info.pm8x100_fabric.clock_buffer_disable_required;
    fabric_data->clock_buffer_bw_update_required = pm_add_info->pm_specific_info.pm8x100_fabric.clock_buffer_bw_update_required;


    PORTMOD_PBMP_ASSIGN(fabric_data->phys, pm_add_info->phys);

    /*init wb buffer*/
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_wb_buffer_init(unit, wb_buffer_index,  pm_info));
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        PM8X100_FABRIC_PORT_BITMAP_ITER(all_lanes_in_pm_bitmap, i)
        {
            is_bypass = SHR_BITGET(&info->bypass_lanes_bitmap[0], i)? 1 : 0;
            rv = PM8X100_FABRIC_LANE_IS_BYPASSED_SET(unit, pm_info, is_bypass, i);
            SOCDNX_IF_ERR_EXIT(rv);
        }
        ...
        /* PLLs */
        PM8X100_FABRIC_PLLS_ITER(0, PM8X100_FABRIC_MAX_NUM_PLLS, i) {
            rv = PM8X100_FABRIC_PLL_ACTIVE_LANES_BITMAP_SET(unit, pm_info, pll_active_lanes_bitmap, i);
            SOCDNX_IF_ERR_EXIT(rv);
            rv = PM8X100_FABRIC_PLL_ACTUAL_LANES_BITMAP_SET(unit, pm_info, pll_actual_lanes_bitmap, i);
            SOCDNX_IF_ERR_EXIT(rv);

            rv = PM8X100_FABRIC_PLL_VCO_RATE_SET(unit, pm_info, pll_vco_rate, i);
            SOCDNX_IF_ERR_EXIT(rv);
        }
    }
    else
    {
        ...
        if (is_core_probed)
        {
            ...
            SOCDNX_IF_ERR_EXIT(portmod_common_serdes_probe(pm8x100_fabric_serdes_list, &core_access, &is_phymod_probed));
            if (!is_phymod_probed)
            {
...
int pm8x100_fabric_pm_lane_mapping_changed_get(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info, int* lane_mapping_changed)
{
    ...
    *lane_mapping_changed = 0;
    ...
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("add_info->init_config.lane_map[0].num_of_lanes != PM8X100_FABRIC_LANES_PER_CORE")));
    }

    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_core_access_get(unit, port, pm_info, 0, 1, &core_access, &cores_num, NULL));
...
int pm8x100_fabric_pm_fmac_tx_lane_mapping_set(int unit, int port, pm_info_t pm_info, const phymod_lane_map_t *lane_map)
{
    ...
    int serdes_index_in_pm, lane_index_in_pm;
    const soc_reg_t tx_lane_swap_regs[] = {FMAC_TX_LANE_SWAP_0r, FMAC_TX_LANE_SWAP_1r, FMAC_TX_LANE_SWAP_2r, FMAC_TX_LANE_SWAP_3r};
    const soc_field_t tx_lane_swap_fields[] = {TX_LANE_SWAP_0f, TX_LANE_SWAP_1f, TX_LANE_SWAP_2f, TX_LANE_SWAP_3f};
    const soc_field_t tx_quad_swap_fields[] = {TX_QUAD_SWAP_0f, TX_QUAD_SWAP_1f, TX_QUAD_SWAP_2f, TX_QUAD_SWAP_3f};
    const soc_field_t tx_lane_rev_swap_fields[] = {TX_LANE_REV_SWAP_0f, TX_LANE_REV_SWAP_1f, TX_LANE_REV_SWAP_2f, TX_LANE_REV_SWAP_3f};
    const soc_field_t tx_quad_rev_swap_fields[] = {TX_QUAD_REV_SWAP_0f, TX_QUAD_REV_SWAP_1f, TX_QUAD_REV_SWAP_2f, TX_QUAD_REV_SWAP_3f};
    ...
    PM8X100_FABRIC_PORT_BITMAP_ITER(all_lanes_in_pm_bitmap, lane_index)
    {
        if (SOC_IS_DNXF(unit))
        {
            ...
            SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_LANE_IS_BYPASSED_GET(unit, pm_info, &is_bypass, lane_index));

            /* register index is serdes id, register value is logical (FMAC) lane */
            SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, FMAC_TX_LANE_SWAPr, fmac_blk_id, serdes_index_in_fmac, &reg_val));

            /* for bypass lane, the lane-swap in FMAC is aligned to be 1:1 */
            field_val = is_bypass ? serdes_index_in_fmac : lane_index_in_fmac;
            soc_reg_field_set(unit, FMAC_TX_LANE_SWAPr, &reg_val, TX_LANE_SWAP_Nf, field_val);

            SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, FMAC_TX_LANE_SWAPr, fmac_blk_id, serdes_index_in_fmac, reg_val));

            /*
             * Reverse config
             */
            /* register index is serdes id, register value is reverese logical (FMAC) lane */
            SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, FMAC_TX_LANE_SWAPr, fmac_rev_blk_id, serdes_index_in_fmac, &reg_val));

            /* for bypass lane, the lane-swap in FMAC is aligned to be 1:1 */
            field_val = is_bypass ? lane_index_in_fmac : serdes_index_in_fmac;
            soc_reg_field_set(unit, FMAC_TX_LANE_SWAPr, &reg_val, TX_LANE_REV_SWAP_Nf, field_val);

            SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, FMAC_TX_LANE_SWAPr, fmac_rev_blk_id, serdes_index_in_fmac, reg_val));
        ...
        else
        {
            ...
            /* register index is serdes id, register value is logical (FMAC) lane */
            SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, tx_lane_swap_regs[serdes_index_in_fmac], fmac_blk_id, 0, &reg_val));

            /* for bypass lane, the lane-swap in FMAC is aligned to be 1:1 */
            field_val = is_bypass ? serdes_index_in_fmac : lane_index_in_fmac;
            soc_reg_field_set(unit, tx_lane_swap_regs[serdes_index_in_fmac], &reg_val, tx_lane_swap_fields[serdes_index_in_fmac], field_val);

            field_val = is_bypass ? 0 : is_quad_swap;
            soc_reg_field_set(unit, tx_lane_swap_regs[serdes_index_in_fmac], &reg_val, tx_quad_swap_fields[serdes_index_in_fmac], field_val);

            SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, tx_lane_swap_regs[serdes_index_in_fmac], fmac_blk_id, 0, reg_val));
             ...
             */
            /* register index is serdes id, register value is reverese logical (FMAC) lane */
            SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, tx_lane_swap_regs[lane_index_in_fmac], fmac_rev_blk_id, 0, &reg_val));

            /* for bypass lane, the lane-swap in FMAC is aligned to be 1:1 */
            field_val = is_bypass ? lane_index_in_fmac : serdes_index_in_fmac;
            soc_reg_field_set(unit, tx_lane_swap_regs[lane_index_in_fmac], &reg_val, tx_lane_rev_swap_fields[lane_index_in_fmac], field_val);

            field_val = is_bypass ? 0 : is_quad_swap;
            soc_reg_field_set(unit, tx_lane_swap_regs[lane_index_in_fmac], &reg_val, tx_quad_rev_swap_fields[lane_index_in_fmac], field_val);

            SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, tx_lane_swap_regs[lane_index_in_fmac], fmac_rev_blk_id, 0, reg_val));
#endif
        }
    }
...
{
    uint32 field_val, quad_swap, reg_val;
    uint32 fmac_blk_id;
    ...
    {
        PM8X100_FABRIC_LANE_FMAC_DATA_GET(pm_info, serdes_index, fmac_blk_id, lane_index_in_fmac);

        /** register index is serdes id, register value is logical (FMAC) lane */
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, tx_lane_swap_regs[lane_index_in_fmac], fmac_blk_id, 0, &reg_val));
        field_val = soc_reg_field_get(unit, tx_lane_swap_regs[lane_index_in_fmac], reg_val, tx_lane_swap_fields[lane_index_in_fmac]);
        if ( field_val >= PM8X100_FABRIC_LANES_PER_FMAC )
        {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("Illegal FMAC swap value")));
        }

        /** read quad_swap value */
        quad_swap = soc_reg_field_get(unit, tx_lane_swap_regs[lane_index_in_fmac], reg_val, tx_quad_swap_fields[lane_index_in_fmac]);

        /** Adjust FMAC mapping to CORE mapping by adding 4 lanes to second FMAC */
        if (serdes_index < PM8X100_FABRIC_LANES_PER_FMAC)
        {
            lane_index = field_val + (PM8X100_FABRIC_LANE_FMAC_INDEX_GET(serdes_index) + quad_swap) * PM8X100_FABRIC_LANES_PER_FMAC;
        }
        else
        {
            lane_index = field_val + (PM8X100_FABRIC_LANE_FMAC_INDEX_GET(serdes_index) - quad_swap) * PM8X100_FABRIC_LANES_PER_FMAC;
...
int pm8x100_fabric_pm_fmac_rx_lane_mapping_set(int unit, int port, pm_info_t pm_info, const phymod_lane_map_t *lane_map)
{
    ...
    PM8X100_FABRIC_PORT_BITMAP_ITER(all_lanes_in_pm_bitmap, lane_index) {
        ...
        /* register index is serdes id, register value is logical (FMAC) lane */
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, rx_lane_swap_regs[serdes_index_in_fmac], fmac_blk_id, 0, &reg_val));
        ...
        SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, rx_lane_swap_regs[serdes_index_in_fmac], fmac_blk_id, 0, reg_val));
    }
...
int pm8x100_fabric_pm_fmac_is_quad_swap_get(int unit, int fmac_blk_id, int fmac_inner_link, int *is_quad_swap)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, rx_lane_swap_regs[fmac_inner_link], fmac_blk_id, 0, &reg_val));
...
int pm8x100_fabric_pm_fmac_rx_lane_mapping_get(int unit, int port, pm_info_t pm_info, phymod_lane_map_t *lane_map)
{
    ...
    PM8X100_FABRIC_PORT_BITMAP_ITER(all_lanes_in_pm_bitmap, serdes_index)
    {
        ...
        /** read quad_swap value */
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_fmac_is_quad_swap_get(unit, fmac_blk_id, fmac_inner_link, &quad_swap));
...
int pm8x100_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if(!PORTMOD_PORT_ADD_F_INIT_PASS2_GET(add_info)) {
        ...
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_core_access_get(unit, port, pm_info, 0, 1, &core_access, &cores_num, NULL));
         ...
         SOCDNX_IF_ERR_EXIT(portmod_common_serdes_probe(pm8x100_fabric_serdes_list, &core_access, &is_probed));
         if (!is_probed)
         {
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_fmac_tx_lane_mapping_set(unit, port, pm_info, add_info->init_config.lane_map));
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_fmac_rx_lane_mapping_set(unit, port, pm_info, add_info->init_config.lane_map));
    ...
    if (add_info->init_config.lane_map[0].num_of_lanes != PM8X100_FABRIC_LANES_PER_CORE)
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("add_info->lane_map.num_of_lanes != PM8X100_FABRIC_LANES_PER_CORE")));
    }
    for(i = 0; i < add_info->init_config.lane_map[0].num_of_lanes; ++i)
    {
    ...
    /* The VCO rates of PLL0, PLL1 will be determined by the speeds of the added ports */
    core_config.pll0_div_init_value = phymod_TSCBH_PLL_DIVNONE;
    core_config.pll1_div_init_value = phymod_TSCBH_PLL_DIVNONE;
    ...
    if (SOC_IS_RELOADING(unit))
    {
                ...
                (!PORTMOD_PORT_ADD_F_INIT_PASS1_GET(add_info) && !PORTMOD_PORT_ADD_F_INIT_PASS2_GET(add_info)))
        {
            ...
            params.phyn = 0;
            SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1, &phy_access, &nof_phys, NULL));
...
STATIC
int pm8x100_fabric_core_init_phase2(int unit, int port, pm_info_t pm_info)
{
    int fsrd_schan_id;
    ...
    SOCDNX_IF_ERR_EXIT(SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_is_initialized], &is_core_initialized));
...
int pm8x100_fabric_port_disable_init(int unit, int port, pm_info_t pm_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_fmac_blk_ids_get(unit, port, pm_info, fmac_blk_ids, &nof_fmac_blks, &first_fmac_index));
    ...
    for (i = 0 ; i < sizeof(reset_fields)/sizeof(*reset_fields); ++i) {
        PM8X100_FABRIC_PORT_FMACS_ITER(fmac_blk_ids, nof_fmac_blks, first_fmac_index, fmac_index) {
            ...
            *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, reset_fields[i]);
            SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_fmac_lanes_bitwrite(unit, port, bitmap, fmac_index, field_val, 1));
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_squelch_set(unit, port, pm_info, flags, 1));
...
STATIC
int pm8x100_fabric_power_set(int unit, int port, pm_info_t pm_info, int power)
{
    portmod_access_get_params_t params;
...
/*
 * Function:
 * _pm8x100_port_clock_repeater_buffer_check
 *
 * Background Info:
 ...
 */
STATIC
int _pm8x100_fabric_port_clock_repeater_buffer_check(
    int unit,
    pm_info_t pm_info,
    int pll_index,
    uint8 lanes_bitmap,
    ...
    int *rx_clk_rptr_dis_req)
{
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_core_access_get(unit, 0, pm_info, 0, 1, &core_access, &cores_num, NULL));
...
/*
 * Function:
 * _pm8x100_fabric_pll_clock_repeater_buffer_pwrdn
 *
 * Background Info:
 *      PLL0 is driving lanes 0-3 directly, and lanes 4-7 through clock repeater.
 *      PLL1 is driving lanes 4-7 directly, and lanes 0-3 through clock repeater.
 ...
 */
STATIC
int _pm8x100_fabric_pll_clock_repeater_buffer_pwrdn(
    int unit,
    pm_info_t pm_info,
    ...
    uint8 pll_lanes_bitmap)
{
    ...
    SOCDNX_IF_ERR_EXIT(_pm8x100_fabric_port_clock_repeater_buffer_check(unit, pm_info, pll_index,
...
int pm8x100_fabric_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    SOC_PBMP_ITER(fabric_data->phys, phy){
        if(PORTMOD_PBMP_MEMBER(port_phys_in_pm, phy)){
            rv = PM8X100_FABRIC_LANE2PORT_SET(unit, pm_info, ii, port);
            SOCDNX_IF_ERR_EXIT(rv);
        }
        ii++;
    }
    ...
    if(!is_core_probed || (is_core_probed == 1 && PORTMOD_PORT_ADD_F_INIT_PASS2_GET(add_info))){
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_core_init_phase1(unit, port, pm_info, add_info));
    }
    ...
    {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_lane_mapping_changed_get(unit, port, pm_info, add_info, &lane_mapping_changed));
    ...
    if(!is_core_initialized) {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_core_init_phase2(unit, port, pm_info));
    }
    ...
    if (fabric_data->clock_buffer_disable_required) {
        begin_pll = 0;
        end_pll = 2;
        ...
        /** enable clock repeaters of both PLLs. we don't know what PLL this lane is configured at */
        PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index) {
            PHYMOD_IF_ERR_RETURN(phymod_phy_pll_clock_buffer_pwrdn(&phy_access, pll_index, 0, 0));
        }
    }

    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_power_set(unit, port, pm_info, 1));
   ...
   /* in order to avoid assuming which indexes of init_config.tx phymod will use for the lanes of the port - fill all array indexes */
    for (lane_index = 0; lane_index < PHYMOD_MAX_LANES_PER_CORE; ++lane_index) {
        /* the speed is not known at this stage. Choosing 50GPAM4 defaults */
        SOCDNX_IF_ERR_EXIT(phymod_phy_tx_taps_default_get(&phy_access, phymodSignallingMethodPAM4, &init_config.tx[lane_index]));

        init_config.tx[lane_index].sig_method = phymodSignallingMethodPAM4;
    }
    rv = PM8X100_FABRIC_LANE_IS_BYPASSED_GET(unit, pm_info, &is_bypass, phy_index);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    /*disable phy and mac*/
    /*SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_enable_set(unit, port, pm_info, flags, 0));*/
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_disable_init(unit, port, pm_info));
...
int pm8x100_fabric_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    uint32 pll_active_lanes_bitmap[PM8X100_FABRIC_MAX_NUM_PLLS];
    uint32 pll_actual_lanes_bitmap[PM8X100_FABRIC_MAX_NUM_PLLS];
    portmod_vco_type_t pll_vco_rate[PM8X100_FABRIC_MAX_NUM_PLLS];
    phymod_phy_access_t phy_access;
    portmod_access_get_params_t params;
    int nof_phys;
    int is_bypass;
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_power_set(unit, port, pm_info, 0));
    ...
    PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_PLL_ACTIVE_LANES_BITMAP_GET(unit, pm_info, &pll_active_lanes_bitmap[pll_index], pll_index));
        SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_PLL_ACTUAL_LANES_BITMAP_GET(unit, pm_info, &pll_actual_lanes_bitmap[pll_index], pll_index));
        SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_PLL_VCO_RATE_GET(unit, pm_info, &pll_vco_rate[pll_index], pll_index));
        ...
        if (SHR_BITGET(&pll_active_lanes_bitmap[pll_index], phy_index))
        {
            ...
            PM8X100_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index)
            {
                SHR_BITCLR(&pll_actual_lanes_bitmap[pll_index], lane_index);
            }
            if (PM8X100_FABRIC_PLL_IS_EMPTY(pll_active_lanes_bitmap[pll_index]))
            {
                pll_vco_rate[pll_index] = portmodVCOInvalid;
                SOCDNX_IF_ERR_EXIT(phymod_phy_pll_pwrdn(&phy_access, pll_index, 1));
            }

            SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_PLL_ACTIVE_LANES_BITMAP_SET(unit, pm_info, pll_active_lanes_bitmap[pll_index], pll_index));
            SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_PLL_ACTUAL_LANES_BITMAP_SET(unit, pm_info, pll_actual_lanes_bitmap[pll_index], pll_index));
            SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_PLL_VCO_RATE_SET(unit, pm_info, pll_vco_rate[pll_index], pll_index));
    ...
    if (fabric_data->clock_buffer_disable_required) {
        PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index) {
            SOCDNX_IF_ERR_EXIT(_pm8x100_fabric_pll_clock_repeater_buffer_pwrdn(unit, pm_info,
                                                                              &phy_access, pll_index, pll_active_lanes_bitmap[pll_index]));
        }
    }
    ...
    for( i = 0 ; i < PM8X100_FABRIC_MAX_PORTS_PER_PM; i++){
        ...
        SOCDNX_IF_ERR_EXIT(rv);
        if(tmp_port == port){
            rv = PM8X100_FABRIC_LANE2PORT_SET(unit, pm_info, i, invalid_port);
            SOCDNX_IF_ERR_EXIT(rv);
        }
    }
    if(!found){
...
int pm8x100_fabric_port_replace(int unit, int port, pm_info_t pm_info, int new_port)
{
    ...
    for (i = 0; i < PM8X100_FABRIC_MAX_PORTS_PER_PM; i++){
        ...
        SOCDNX_IF_ERR_EXIT(rv);
        if (tmp_port == port) {
            rv = PM8X100_FABRIC_LANE2PORT_SET(unit, pm_info, i, new_port);
            SOCDNX_IF_ERR_EXIT(rv);
        }
    }
...
soc_error_t pm8x100_fabric_fmac_enable_set(int unit, int port, pm_info_t pm_info, int rx_tx, int enable)
{
    ...
    PM8X100_FABRIC_PORT_FMACS_ITER(fmac_blk_ids, nof_fmac_blks, first_fmac_index, fmac_index) {
        ...
        *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, field);
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_fmac_lanes_bitwrite(unit, port, bitmap, fmac_index, field_val, (enable? 0 : 1)));
...
int pm8x100_fabric_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
             ...
             PORTMOD_PORT_ENABLE_RX_GET(flags_mask) && PORTMOD_PORT_ENABLE_TX_GET(flags_mask))
    {
        ...
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_enable_get(unit, port, pm_info, flags, &cur_enable));
        ...
        if(enable){
            SOCDNX_IF_ERR_EXIT(pm8x100_fabric_squelch_set(unit, port, pm_info, flags_mask, 0));

            /* Enable FMAC Tx*/
            SOCDNX_IF_ERR_EXIT(pm8x100_fabric_fmac_enable_set(unit, port, pm_info, PM8X100_FABRIC_FMAC_TX, enable));

            /* Enable FMAC Rx*/
            SOCDNX_IF_ERR_EXIT(pm8x100_fabric_fmac_enable_set(unit, port, pm_info, PM8X100_FABRIC_FMAC_RX, enable));
        ...
        else
        {
            ...
            SOCDNX_IF_ERR_EXIT(pm8x100_fabric_squelch_set(unit, port, pm_info, flags_mask, 1));
        }
    }
...
int
pm8x100_fabric_port_cdr_lock_get(int unit, int port, pm_info_t pm_info, int* is_locked)
{
    uint32 fsrd_schan_id;
    uint32 phy_index, bitmap;
    uint32 reg32_val;
...
/*
 * This function configures proprietary RS FEC sync machine (relevant only to RS FEC (RS304, RS545).
 ...
 */
STATIC
int pm8x100_fabric_port_fec_sync_machine_config(int unit, int port, pm_info_t pm_info, portmod_port_pcs_t encoding)
{
    pm8x100_fabric_fec_sync_machine_config_t sync_machine_config;
    uint32 phy_index, bitmap;
    uint32 fmac_blk_id;
    int lane_index, lane_index_in_fmac;
    ...
    sync_machine_config.n1 = PM8X100_FABRIC_FEC_SYNC_MACHINE_N1;
    sync_machine_config.n2 = PM8X100_FABRIC_FEC_SYNC_MACHINE_N2;
    sync_machine_config.n3 = PM8X100_FABRIC_FEC_SYNC_MACHINE_N3;
    sync_machine_config.k1 = PM8X100_FABRIC_FEC_SYNC_MACHINE_K1;
    sync_machine_config.k2 = PM8X100_FABRIC_FEC_SYNC_MACHINE_K2;
    sync_machine_config.k3 = PM8X100_FABRIC_FEC_SYNC_MACHINE_K3;
...
/*
 * This function configures standard RS FEC sync machine (relevant only to RS FEC (RS544).
 * Thus this function needs to be called when setting the encoding and when setting the speed.
 * This function is not relevant for bypass (ILKN) port
 */
STATIC
int pm8x100_fabric_port_rs_fec_sync_machine_config(int unit, int port, pm_info_t pm_info)
{
    pm8x100_fabric_rs_fec_sync_machine_config_t am_config;
    uint32 phy_index, bitmap;
    uint32 fmac_blk_id;
    int lane_index, lane_index_in_fmac;
    soc_reg_above_64_val_t reg_above64;
    ...
    am_config.am_period = PM8X100_FABRIC_RS_FEC_SYNC_MACHINE_AM_PERIOD;
    am_config.comapre_cnt = PM8X100_FABRIC_RS_FEC_SYNC_MACHINE_COMPARE_CNT;
    am_config.nibble_cnt = PM8X100_FABRIC_RS_FEC_SYNC_MACHINE_NIBBLE_CNT;
    ...
    PM8X100_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index) {
        PM8X100_FABRIC_LANE_FMAC_DATA_GET(pm_info, lane_index, fmac_blk_id, lane_index_in_fmac);
        SOCDNX_IF_ERR_EXIT(READ_FMAC_RSF_CONFIGURATIONr(unit, fmac_blk_id, lane_index_in_fmac, reg_above64));
        soc_reg_above_64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, reg_above64, AUTO_DOC_NAME_100f, am_config.am_period);
        soc_reg_above_64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, reg_above64, AUTO_DOC_NAME_101f, am_config.comapre_cnt);
        soc_reg_above_64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, reg_above64, AUTO_DOC_NAME_102f, am_config.nibble_cnt);
        soc_reg_above_64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, reg_above64, RSF_N_RX_SYNC_OUT_SYNC_CRITERIAf, 2 /** both */);
        SOCDNX_IF_ERR_EXIT(WRITE_FMAC_RSF_CONFIGURATIONr(unit, fmac_blk_id, lane_index_in_fmac, reg_above64));
    }
...
int pm8x100_fabric_port_encoding_set(int unit, int port, pm_info_t pm_info, uint32 properties, portmod_port_pcs_t encoding)
{
    ...
    case PORTMOD_PCS_64B66B:
        ...
        fmal_width = PM8X100_FABRIC_FEC_NONE_FMAL_WIDTH;
        break;
    case PORTMOD_PCS_64B66B_15T_RS_FEC:
        encoding_type = 1;
        slow_read_rate = PM8X100_FABRIC_FEC_RS_SLOW_READ_RATE;
        llfc_cig = 1;
        fmal_width = PM8X100_FABRIC_FEC_RS_FMAL_WIDTH;
        rsf_frame_type = 0;
        break;
    ...
    if(!PORTMOD_ENCODING_LOW_LATENCY_LLFC_GET(properties) && PM8X100_FABRIC_ENCODING_IS_LOW_LATECY_RS_FEC(encoding)){
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("RS_FEC pcs: low latency mismatch with encoding")));
    }
    ...
    if(PORTMOD_ENCODING_EXTRCT_CIG_FROM_LLFC_GET(properties)){
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("extract CIG indications from LLFC cells is not supported on this device")));
    }
    ...
    flags = 0;
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_enable_get(unit, port, pm_info, flags, &enabled));
    /** disable phy and mac */
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_enable_set(unit, port, pm_info, flags, 0));
    ...
    /** sync machine tuning */
    if (PM8X100_FABRIC_ENCODING_IS_PROPRIETARY_RS_FEC(encoding)) {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_fec_sync_machine_config(unit, port, pm_info, encoding));
    } else if (PM8X100_FABRIC_ENCODING_IS_STANDARD_RS_FEC(encoding)) {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_rs_fec_sync_machine_config(unit, port, pm_info));
    }

    /** restore MAC and PHY */
    flags = 0;
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_enable_set(unit, port, pm_info, flags, enabled));
...
_pm8x100_fabric_loopback_serdes_overrides_set(int unit, int port, pm_info_t pm_info)
{
    ...
    for (i = 0; i < sizeof(mac_loopbacks)/sizeof(mac_loopbacks[0]); i++) {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_loopback_get(unit, port, pm_info, mac_loopbacks[i], &lb_enable));
        if (lb_enable) {
...
int pm8x100_fabric_mac_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    case portmodLoopbackMacRloop:
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_retimer_set(unit, port, pm_info, TRUE, enable));
        break;
    default:
...
int pm8x100_fabric_port_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    case portmodLoopbackMacRloop:
        break;
    case portmodLoopbackMacPCS:
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_encoding_get(unit, port, pm_info, &encoding_properties ,&encoding));
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_loopback_get(unit, port, pm_info, loopback_type, &lb_enable));
    if(enable == lb_enable){
        SOC_EXIT;
    }

    /*loopback set*/
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_are_lanes_swapped(unit, port, pm_info, &lanes_are_swapped));
    if(is_phy_loopback){
        ...
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_mac_loopback_set(unit, port, pm_info, loopback_type, enable));
    }
    ...
    SOCDNX_IF_ERR_EXIT(rv);
    if(!is_bypass) {
        SOCDNX_IF_ERR_EXIT(_pm8x100_fabric_loopback_serdes_overrides_set(unit, port, pm_info));
    }
...
int pm8x100_fabric_port_loopback_get(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int* enable)
{
    ...
    case portmodLoopbackMacRloop:
        ...
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_retimer_get(unit, port, pm_info, &is_remote_loopback, &is_enabled));
...
#define PM8X100_FABRIC_PORT_RETIMER_ILKN_FIFO_THR(is_retimer_enabled) ((is_retimer_enabled) ? (18) : (5))
#define PM8X100_FABRIC_PORT_RETIMER_AUTO_ADJUST_MIN (0x2)
#define PM8X100_FABRIC_PORT_RETIMER_AUTO_ADJUST_MAX(is_retimer_enabled) ((is_retimer_enabled) ? (0x10) : (0xa))
#define PM8X100_FABRIC_PORT_RETIMER_AUTO_ADJUST_PERIOD_FIELD_VAL(is_retimer_enabled) ((is_retimer_enabled) ? (0xffffff) : (0x7d0))
#define PM8X100_FABRIC_PORT_RETIMER_AUTO_ADJUST_PERIOD_USEC (1000000)
#define PM8X100_FABRIC_PORT_RETIMER_BUCKET_LINK_DOWN_THR(is_retimer_enabled) ((is_retimer_enabled) ? (0x3f) : (0x10))
#define PM8X100_FABRIC_PORT_RETIMER_FABRIC_MAC_MODE(is_rs_fec_mode) ((is_rs_fec_mode) ? (2) : (0))
 ...
 */
STATIC int
pm8x100_fabric_port_retimer_set(int unit, soc_port_t port, pm_info_t pm_info, int is_remote_loopback, int enable)
{
    uint32 fmac_blk_ids[PORTMOD_NOF_FMACS_PER_PM8X100_FABRIC] = {0};
    ...
    SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_TX_DC_AUTO_ADJUSTr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg64_val));
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_DC_AUTO_ADJUSTr, &reg64_val, FMAL_N_TX_DC_AUTO_ADJUST_ENf, (enable) ? (1) : (0));
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_DC_AUTO_ADJUSTr, &reg64_val, FMAL_N_TX_DC_AUTO_ADJUST_MINf, PM8X100_FABRIC_PORT_RETIMER_AUTO_ADJUST_MIN);
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_DC_AUTO_ADJUSTr, &reg64_val, FMAL_N_TX_DC_AUTO_ADJUST_MAXf, PM8X100_FABRIC_PORT_RETIMER_AUTO_ADJUST_MAX(enable));
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_DC_AUTO_ADJUSTr, &reg64_val, FMAL_03_TX_DC_AUTO_ADJUST_FIELD_8f, PM8X100_FABRIC_PORT_RETIMER_AUTO_ADJUST_PERIOD_FIELD_VAL(enable));
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAL_TX_DC_AUTO_ADJUSTr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg64_val));

    /* Set allowed fullness level of ILKN FIFO */
    /* Fmal[n]TxIlknFifoThr */
    SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg64_val));
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_GENERAL_CONFIGURATIONr, &reg64_val, AUTO_DOC_NAME_100f, PM8X100_FABRIC_PORT_RETIMER_ILKN_FIFO_THR(enable));
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg64_val));

    /* Disable asynchronous FIFO */
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_encoding_get(unit, port, pm_info, &encoding_properties, &encoding));
    fabric_mac_mode = PM8X100_FABRIC_PORT_RETIMER_FABRIC_MAC_MODE(!enable && !PM8X100_FABRIC_ENCODING_IS_RS_FEC(encoding));
    SOCDNX_IF_ERR_EXIT(READ_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg64_val));
    soc_reg64_field32_set(unit, FMAC_ASYNC_FIFO_CONFIGURATIONr, &reg64_val, FMAL_N_TX_WIDTHf, fabric_mac_mode);
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg64_val));

    /* Set the link as down in MAC leacky bucket */
    SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg32_val));
    soc_reg_field_set(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, &reg32_val, BKT_LINK_DN_TH_Nf, PM8X100_FABRIC_PORT_RETIMER_BUCKET_LINK_DOWN_THR(enable));
    SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg32_val));

    /* async FIFO configuration */
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_retimer_fec_enable_get(unit, port, pm_info, &retimer_fec_enable));
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_speed_config_get(unit, port, pm_info, &speed_config));
    ...
    {
        fmal_width = (enable && !retimer_fec_enable)? PM8X100_FABRIC_FEC_NONE_FMAL_WIDTH_RETIMER_FEC_DISABLED : PM8X100_FABRIC_FEC_NONE_FMAL_WIDTH;
        SOCDNX_IF_ERR_EXIT(READ_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg64_val));
        soc_reg64_field32_set(unit, FMAC_ASYNC_FIFO_CONFIGURATIONr, &reg64_val, FMAL_N_RX_WIDTHf, fmal_width);
        soc_reg64_field32_set(unit, FMAC_ASYNC_FIFO_CONFIGURATIONr, &reg64_val, FMAL_N_TX_WIDTHf, fmal_width);
        SOCDNX_IF_ERR_EXIT(WRITE_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg64_val));
    ...
    {
        slow_read_rate = (enable && !retimer_fec_enable)? PM8X100_FABRIC_FEC_BASE_R_SLOW_READ_RATE_RETIMER_FEC_DISABLED : PM8X100_FABRIC_FEC_BASE_R_SLOW_READ_RATE;
        fmal_width = (enable && !retimer_fec_enable)? PM8X100_FABRIC_FEC_BASE_R_FMAL_WIDTH_RETIMER_FEC_DISABLED : PM8X100_FABRIC_FEC_BASE_R_FMAL_WIDTH;
        SOCDNX_IF_ERR_EXIT(READ_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), &reg64_val));
        soc_reg64_field32_set(unit, FMAC_ASYNC_FIFO_CONFIGURATIONr, &reg64_val, FMAL_N_RX_SLOW_READ_RATEf, slow_read_rate);
        soc_reg64_field32_set(unit, FMAC_ASYNC_FIFO_CONFIGURATIONr, &reg64_val, FMAL_N_RX_WIDTHf, fmal_width);
        soc_reg64_field32_set(unit, FMAC_ASYNC_FIFO_CONFIGURATIONr, &reg64_val, FMAL_N_TX_WIDTHf, fmal_width);
    ...
    /* Enable retimer */
    SOCDNX_IF_ERR_EXIT(READ_FMAC_RETIMERr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg_above_64_val));
    ...
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_RETIMERr(unit, fmac_blk_ids[first_fmac_index], PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index), reg_above_64_val));
 ...
 */
STATIC int
pm8x100_fabric_port_retimer_get(int unit, soc_port_t port, pm_info_t pm_info, int *is_remote_loopback, int *enable)
{
    uint32 fmac_blk_ids[PORTMOD_NOF_FMACS_PER_PM8X100_FABRIC] = {0};
    int nof_fmac_blks = 0, first_fmac_index = 0;
    uint32 phy_index = 0, bitmap = 0;
 ...
 */
int
pm8x100_fabric_port_pass_through_set(int unit, soc_port_t port, pm_info_t pm_info, int loopback_en, int enable)
{
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_retimer_set(unit, port, pm_info, loopback_en, enable));
 ...
 */
int
pm8x100_fabric_port_pass_through_get(int unit, soc_port_t port, pm_info_t pm_info, int *loopback_en, int *enable)
{
    SOCDNX_INIT_FUNC_DEFS;

    

    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_retimer_get(unit, port, pm_info, loopback_en, enable));
...
int
pm8x100_fabric_port_phase_interpolator_rx_set(int unit, soc_port_t port, pm_info_t pm_info, int enable)
{
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    int nof_phys;
    ...
    SOCDNX_IF_ERR_EXIT(phymod_phy_tx_phase_lock_rx_set(&phy_access, enable));
...
int
pm8x100_fabric_port_phase_interpolator_tx_set(int unit, soc_port_t port, pm_info_t pm_info, int enable)
{
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    int nof_phys;
    ...
    SOCDNX_IF_ERR_EXIT(phymod_phy_tx_phase_lock_tx_set(&phy_access, enable));
 ...
 */
STATIC int
pm8x100_fabric_port_retimer_fec_enable_set(int unit, int port, pm_info_t pm_info, int enable)
{
    uint32 fmac_blk_ids[PORTMOD_NOF_FMACS_PER_PM8X100_FABRIC] = {0};
    int nof_fmac_blks = 0, first_fmac_index = 0;
    uint32 phy_index = 0, bitmap = 0;
    ...
    lane_index_in_fmac = PM8X100_FABRIC_LANE_INDEX_IN_FMAC_GET(phy_index);
 ...
 */
STATIC int
pm8x100_fabric_port_retimer_fec_enable_get(int unit, int port, pm_info_t pm_info, int* enable)
{
    uint32 fmac_blk_ids[PORTMOD_NOF_FMACS_PER_PM8X100_FABRIC] = {0};
    int nof_fmac_blks = 0, first_fmac_index = 0;
    uint32 phy_index = 0, bitmap = 0;
 ...
 */
int
pm8x100_fabric_port_pass_through_fec_enable_set(int unit, int port, pm_info_t pm_info, int enable)
{
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_retimer_fec_enable_set(unit, port, pm_info, enable));
 ...
 */
int
pm8x100_fabric_port_pass_through_fec_enable_get(int unit, int port, pm_info_t pm_info, int* enable)
{
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_retimer_fec_enable_get(unit, port, pm_info, enable));
 ...
 */
int
pm8x100_fabric_port_retimer_calibrate(int unit, int port, pm_info_t pm_info)
{
    uint32 fmac_blk_ids[PORTMOD_NOF_FMACS_PER_PM8X100_FABRIC] = {0};
    int nof_fmac_blks = 0, first_fmac_index = 0;
    uint32 phy_index = 0, bitmap = 0;
    uint64 reg64_val;
    int auto_adjust_lock = 0;
    int lane_index_in_fmac;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    /* Wait for auto-adjust to complete */
    sal_usleep(PM8X100_FABRIC_PORT_RETIMER_AUTO_ADJUST_PERIOD_USEC);

    /* Check that auto-adjust is locked */
    SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_TX_DC_AUTO_ADJUSTr(unit, fmac_blk_ids[first_fmac_index], lane_index_in_fmac, &reg64_val));
    auto_adjust_lock = soc_reg64_field32_get(unit, FMAC_FMAL_TX_DC_AUTO_ADJUSTr, reg64_val, FMAL_N_TX_DC_AUTO_ADJUST_LOCKf);
    if (auto_adjust_lock == 0)
    {
 ...
 */
int
pm8x100_fabric_port_pass_through_calibrate(int unit, int port, pm_info_t pm_info)
{
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_retimer_calibrate(unit, port, pm_info));
...
int pm8x100_fabric_port_link_get(int unit, int port, pm_info_t pm_info, int flag, int* link)
{
    ...
    *link = 0;
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_enable_get(unit, port, pm_info, 0, &enable));

    if (enable) {
        /* Check if there is a MAC LB set on the lane*/
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_loopback_get(unit, port, pm_info, portmodLoopbackMacAsyncFifo, &is_mac_lb_enabled));
...
int pm8x100_fabric_port_update(int unit, int port, pm_info_t pm_info, const portmod_port_update_control_t* update_control)
{
    SOCDNX_INIT_FUNC_DEFS;
    /*Do nothing*/
    SOCDNX_FUNC_RETURN;
    ...
    int op_cmd, const void *arg)
{
    ...
    switch(op_cmd) {
        case PHY_DIAG_CTRL_DSC:
            ...
            SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1,
 ...
 */
int pm8x100_fabric_pm_speed_vco_get(int unit, int speed, int is_bypass, portmod_vco_type_t* required_vco)
{
    SOCDNX_INIT_FUNC_DEFS;
    ...
    if (is_bypass)
    {
        SOCDNX_IF_ERR_EXIT(pm8x100_shared_pcs_bypassed_vco_get(unit, speed, required_vco));
    ...
    {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_speed_config_fabric_speed_to_vco_get(unit, speed, required_vco));
    }
exit:
    SOCDNX_FUNC_RETURN;
}
 ...
 */
static int pm8x100_fabric_pm_speed_config_validate_plls_get(int unit,pm_info_t pm_info, const portmod_pbmp_t* ports, portmod_pm_vco_setting_t* vco_setting, int begin_pll, int end_pll, portmod_vco_type_t* pll_vco_rate_curr, portmod_vco_type_t* pll_vco_rate_curr_updated)
{
    int port, speed_index, err_speed_index;
    int pll_index;
    portmod_vco_type_t pll_vco_rate_requested[PM8X100_FABRIC_MAX_NUM_PLLS];
    uint32 requested_lanes_bitmap_required[1];
    uint32 pll_active_lanes_bitmap_curr[PM8X100_FABRIC_MAX_NUM_PLLS];
    ...
    PM8X100_FABRIC_PLLS_ITER(0, PM8X100_FABRIC_MAX_NUM_PLLS, pll_index)
    {
    ...
    PM8X100_FABRIC_PLLS_INIT(pll_vco_rate_curr, pll_active_lanes_bitmap_curr);
    /*
     * }
     */
    ...
    {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_speed_vco_get(unit, vco_setting->speed_config_list[speed_index].speed, vco_setting->speed_for_pcs_bypass_port[speed_index], &required_vco));
        ...
        if ((required_vco != pll_vco_rate_requested[0]) && (required_vco != pll_vco_rate_requested[1]))
        {
            ...
            /*
             * Check if provided sed of ports/speeds can be set theoretically on PM8x100.
             * NOTE! We don't take into account at this point the current VCO allocation set on the PM
             * NOTE! PM8X100_FABRIC_PLLS_ITER iterates backwards so that is why the check is also opposite
             */
    ...
    PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_PLL_VCO_RATE_GET(unit, pm_info, &pll_vco_rate_curr[pll_index], pll_index));
        SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_PLL_ACTIVE_LANES_BITMAP_GET(unit, pm_info, &pll_active_lanes_bitmap_curr[pll_index], pll_index));
    ...
    PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
                ...
                && (pll_vco_rate_curr[pll_index] != pll_vco_rate_requested[0]) && (pll_vco_rate_curr[pll_index] != pll_vco_rate_requested[1]))
        {
            ...
            if (PM8X100_FABRIC_BITMAP_IS_INCLUDED(pll_active_lanes_bitmap_curr[pll_index], requested_lanes_bitmap_required[0]))
            {
                pll_vco_rate_curr_updated[pll_index] = portmodVCOInvalid;
            }
        }
...
int pm8x100_fabric_pm_speed_config_validate(int unit, int pm_id, pm_info_t pm_info, const portmod_pbmp_t* ports, int flag, portmod_pm_vco_setting_t* vco_setting)
{
    ...
    portmod_vco_type_t pll_vco_rate_curr[PM8X100_FABRIC_MAX_NUM_PLLS];
    portmod_vco_type_t pll_vco_rate_curr_updated[PM8X100_FABRIC_MAX_NUM_PLLS];
    portmod_vco_type_t required_vco = portmodVCOInvalid;
    pm8x100_fabric_t fabric_data = pm_info->pm_data.pm8x100_fabric;
    ...
    begin_pll = PM8X100_FABRIC_PLL_INDEX_DEFAULT_START;
    end_pll = PM8X100_FABRIC_PLL_INDEX_DEFAULT_END;
    if (fabric_data->force_single_pll0 || fabric_data->native_single_pll)
    {
        begin_pll = 0;
        end_pll = 1;
     ...
     *
     */
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_speed_config_validate_plls_get(unit, pm_info, ports, vco_setting, begin_pll, end_pll, pll_vco_rate_curr, pll_vco_rate_curr_updated));
    ...
    for (speed_index = 0; speed_index < vco_setting->num_speeds; ++speed_index)
    {
        ...
        {
            preferred_pll_index = PM8X100_FABRIC_PREFERRED_PLL_INDEX;
            other_pll_index = 1 - preferred_pll_index;
...
int pm8x100_fabric_pm_vco_reconfig(int unit, int pm_id, pm_info_t pm_info, const portmod_vco_type_t* vco)
{
    ...
    portmod_vco_type_t pll_vco_rate[PM8X100_FABRIC_MAX_NUM_PLLS];
    uint32 pll_active_lanes_bitmap[PM8X100_FABRIC_MAX_NUM_PLLS];
    portmod_vco_type_t vco_aligned[PM8X100_FABRIC_MAX_NUM_PLLS];
    pm8x100_fabric_t fabric_data = pm_info->pm_data.pm8x100_fabric;
    ...
    PM8X100_FABRIC_PLLS_INIT(pll_vco_rate, pll_active_lanes_bitmap);
    ...
    PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        ...
        if (vco_aligned[pll_index] != pll_vco_rate[pll_index])
        {
            ...
            /* next get the PLL divider */
            SOCDNX_IF_ERR_EXIT(pm8x100_fabric_vco_to_pll_get(unit, fabric_data->ref_clk, vco_aligned[pll_index], &pll_div));
            ...
            if (!SOC_IS_RELOADING(unit))
            {
                if (pll_div != phymod_TSCBH_PLL_DIVNONE) {
                    SOCDNX_IF_ERR_EXIT(phymod_phy_pll_reconfig(&phy_access, pll_index, pll_div, fabric_data->ref_clk));
                } else {
                    SOCDNX_IF_ERR_EXIT(phymod_phy_pll_pwrdn(&phy_access, pll_index, 1));
                }
            }
            ...
            if (fabric_data->clock_buffer_bw_update_required)
            {
                if ((pll_index == 0) && (pll_vco_rate[pll_index] != portmodVCOInvalid)) {
                    SOCDNX_IF_ERR_EXIT(phymod_phy_pll_rx_clock_buffer_bw_update(&phy_access, 0/* pll0 */,
                                                                                pll_div, fabric_data->ref_clk));
                }
...
int pm8x100_fabric_port_speed_config_set(int unit, int port, pm_info_t pm_info, const portmod_speed_config_t* speed_config)
{
    ...
    int nof_phys;
    int is_bypass;
    int tx_disable_required[2];
    int rx_disable_required[2];
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
    SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_LANE_IS_BYPASSED_GET(unit, pm_info, &is_bypass, phy_index));

    /* to change the VCO rate of a port that is currently assigned to a PLL - remove it from the PLL */
    if (is_bypass)
    {
        SOCDNX_IF_ERR_EXIT(pm8x100_shared_pcs_bypassed_vco_get(unit, speed_config->speed, &required_vco));
        if (required_vco == portmodVCOInvalid)
        {
    ...
    {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_speed_config_fabric_speed_to_vco_get(unit, speed_config->speed, &required_vco));
    }
    PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        if (SHR_BITGET(&pll_actual_lanes_bitmap[pll_index], phy_index) && (required_vco != pll_vco_rate[pll_index]))
        {
            PM8X100_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index)
            {
                SHR_BITCLR(&pll_actual_lanes_bitmap[pll_index], lane_index);
    ...
    PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        if (pll_vco_rate[pll_index] == required_vco)
        {
            PM8X100_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index)
            {
                ...
                SHR_BITSET(&pll_actual_lanes_bitmap[pll_index], lane_index);
            }
            break;
        }
    }
    ...
    if (pll_index < begin_pll)
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("port %d: the required VCO rate %d was not configured in pm8x100_fabric_pm_vco_reconfig"), port, required_vco));
    }
    ...
    if (!SOC_IS_RELOADING(unit))
    {
        if (fabric_data->clock_buffer_disable_required) {
            PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index) {
                ...
                /* disable PLL1 clock repeater buffer, if PLL1 is not used by lanes 0-3 */
                SOCDNX_IF_ERR_EXIT(_pm8x100_fabric_port_clock_repeater_buffer_check(unit, pm_info, pll_index,
                                                                                   pll_actual_lanes_bitmap[pll_index],
                                                                                   &tx_disable_required[pll_index],
                                                                                   &rx_disable_required[pll_index]));
                if (tx_disable_required[pll_index] == 0 &&
                    rx_disable_required[pll_index] == 0) {

                    PHYMOD_IF_ERR_RETURN(phymod_phy_pll_clock_buffer_pwrdn(&phy_access, pll_index,
                                                                           tx_disable_required[pll_index],
                                                                           rx_disable_required[pll_index]));
                }
            }
    ...
    if (!SOC_IS_RELOADING(unit))
    {
        ...
        if (fabric_data->clock_buffer_disable_required) {
            uint32 all_lanes_used_by_ports[1] = {0}, lanes_used_by_ports_with_speed = 0;
            int i, tmp_port;
            for (i = 0; i < PM8X100_FABRIC_MAX_PORTS_PER_PM; i++) {
                SOCDNX_IF_ERR_EXIT(PM8X100_FABRIC_LANE2PORT_GET(unit, pm_info, i, &tmp_port));
                if(tmp_port >= 0) {
                    SHR_BITSET(all_lanes_used_by_ports, i);
                }
            }

            PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index) {
                lanes_used_by_ports_with_speed |= pll_actual_lanes_bitmap[pll_index];
            }

            if (all_lanes_used_by_ports[0] == lanes_used_by_ports_with_speed) {
                /** all ports have speed */
                PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index) {
                    /* disable PLL0 clock repeater buffer, if PLL0 is not used by lanes 4-7 */
                    /* disable PLL1 clock repeater buffer, if PLL1 is not used by lanes 0-3 */
                    if (tx_disable_required[pll_index] == 1 ||
                        rx_disable_required[pll_index] == 1) {
    ...
    if (((new_pll_state.pll0_lanes_bitmap == pll_active_lanes_bitmap[0]) && (new_pll_state.pll1_lanes_bitmap == pll_active_lanes_bitmap[1])) || SOC_IS_RELOADING(unit))
    {
        ...
        SOCDNX_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_actual_lanes_bitmap], &pll_actual_lanes_bitmap[0], 0));
        SOCDNX_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_actual_lanes_bitmap], &pll_actual_lanes_bitmap[1], 1));
        PM8X100_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
        {
    ...
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("port %d: didn't get expected PLL bitmaps from phymod actual (%x, %x), expected (%x, %x)"),
                                              port, new_pll_state.pll0_lanes_bitmap, new_pll_state.pll1_lanes_bitmap,
                                              pll_active_lanes_bitmap[0], pll_active_lanes_bitmap[1]));
    }
    ...
    {
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_speed_config_new_to_legacy_encoding_get(unit, port, pm_info, speed_config->fec, &legacy_encoding));
        ...
        SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_encoding_set(unit, port, pm_info, encoding_properties, legacy_encoding));
    }
...
int pm8x100_fabric_port_speed_config_get(int unit, int port, pm_info_t pm_info, portmod_speed_config_t* speed_config)
{
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_encoding_get(unit, port, pm_info, &properties, &legacy_encoding));
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_port_speed_config_legacy_to_new_encoding_get(unit, port, pm_info, legacy_encoding, &speed_config->fec));
...
int pm8x100_fabric_port_lane_map_set(int unit, int port, pm_info_t pm_info, uint32 flags, const phymod_lane_map_t* lane_map)
{
    ...
    if (mac_only == 0)
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("port %d: pm8x100_fabric supports MAC only lane map set"), port));
    }

    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_fmac_tx_lane_mapping_set(unit, port, pm_info, lane_map));
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_fmac_rx_lane_mapping_set(unit, port, pm_info, lane_map));
...
int pm8x100_fabric_port_lane_map_get(int unit, int port, pm_info_t pm_info, uint32 flags,phymod_lane_map_t* lane_map)
{
    ...
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_fmac_tx_lane_mapping_get(unit, port, pm_info, lane_map));
    SOCDNX_IF_ERR_EXIT(pm8x100_fabric_pm_fmac_rx_lane_mapping_get(unit, port, pm_info, lane_map));
                                   ...
                                   const portmod_port_synce_clk_ctrl_t* cfg)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_schan_id, 0, &reg_val));
    soc_reg64_field32_set(unit, FSRD_SYNC_E_SELECTr, &reg_val, SYNC_E_CLK_DIVf, cfg->sdm_val);
    SOCDNX_IF_ERR_EXIT(soc_reg_set(unit, FSRD_SYNC_E_SELECTr, fsrd_schan_id, 0, reg_val));
...
int pm8x100_fabric_port_credits_enable_set(int unit, int port, pm_info_t pm_info, int enable)
{
    pm8x100_fabric_t fabric_data = pm_info->pm_data.pm8x100_fabric;
    int fsrd_schan_id;
    uint32 reg_val;
...
int pm8x100_fabric_core_add(int unit, int pm_id, pm_info_t pm_info, int flags, const portmod_port_add_info_t* add_info)
{
