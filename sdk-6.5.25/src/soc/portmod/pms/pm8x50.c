...
int _pm8x50_port_link_recover(int unit, int port, pm_info_t pm_info, int* is_completed)
{
    ...
    uint32_t signal_detect = 1, three_bad, pmd_x4_status = 0;
...
int pm8x50_port_phy_link_up_event(int unit, int port, pm_info_t pm_info)
{
    ...
    if (PORTMOD_PORT_TIMESYNC_CONFIG_ENABLE_GET(timesync_config)) {
        ...
        if (PORTMOD_PORT_TIMESYNC_CONFIG_COMP_MODE_LATEST_LANE_GET(timesync_config)){
            am_comp_mode = phymodTimesyncCompensationModeLatestlane;
        } else {
            am_comp_mode = phymodTimesyncCompensationModeEarliestLane;
        }

        _SOC_IF_ERR_EXIT
            (phymod_timesync_adjust_set( &phy_access, timesync_adj_flags, am_comp_mode));
