...
soc_error_t
soc_fe3200_port_rate_egress_ppt_set(int unit, soc_port_t port, uint32 burst, uint32 nof_tiks)
{
    uint32 limit_max;
    int max_length;
    uint64 reg64_val;
...
soc_error_t
soc_fe3200_port_rate_egress_ppt_get(int unit, soc_port_t port, uint32 *burst, uint32 *nof_tiks)
{
    uint64 reg64_val;
    int fmac_index; 
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_fe3200_port_fault_get(int unit, soc_port_t port, uint32* flags)
{
...
soc_error_t
soc_fe3200_port_dynamic_port_update(int unit, int port, int enable, soc_dfe_port_update_type_t type)
{
    soc_info_t          *si;
    int                 quad, dis;
...
soc_error_t 
soc_fe3200_port_bucket_fill_rate_validate(int unit, uint32 bucket_fill_rate)
{
    SOCDNX_INIT_FUNC_DEFS;
    

    if(bucket_fill_rate > SOC_FE3200_MAX_BUCKET_FILL_RATE) {
...
soc_fe3200_port_sync_e_link_set(int unit, int is_master, int port)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg_val2));
    ...
    if (is_master)
    {
        ...
        soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val2, MASTER_CLK_SELf,  port % SOC_DFE_DEFS_GET(unit, nof_links_in_fsrd));
    ...
    else
    {
        ...
        soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val2, SLAVE_CLK_SELf,  port % SOC_DFE_DEFS_GET(unit, nof_links_in_fsrd));
    }
    SOCDNX_IF_ERR_EXIT(WRITE_ECI_SELECT_OUTPUT_OF_SYNCHRONOUS_ETHERNET_PADSr(unit, reg_val));
    SOCDNX_IF_ERR_EXIT(soc_reg_set(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, reg_val2));
...
soc_fe3200_port_sync_e_divider_set(int unit, int divider)
{
    ...
    for (i = 0 ; i < SOC_DFE_DEFS_GET(unit, nof_instances_mac_fsrd); i++)
    {
        ...
        if (!disabled)
        {
            SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, i, 0, &reg_val));
            ...
            SOCDNX_IF_ERR_EXIT(soc_reg_set(unit, FSRD_SYNC_E_SELECTr, i, 0, reg_val));
        }
...
soc_fe3200_port_burst_control_set(int unit, soc_port_t port, soc_dcmn_loopback_mode_t loopback)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, fmac_block, fmac_inner_link, &reg32_val));
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, fmac_block, fmac_inner_link, reg32_val));
...
soc_fe3200_port_dynamic_soc_init(int unit, soc_port_t port, int is_first_link)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, fmac_block, 0, &reg64_val));
    ...
    *field = soc_reg64_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg64_val, LNK_LVL_FC_RX_ENf);
    SHR_BITSET(field, fmac_inner_link);
    soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg64_val, LNK_LVL_FC_RX_ENf, *field);
    SOCDNX_IF_ERR_EXIT(soc_reg_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, fmac_block, 0, reg64_val));
