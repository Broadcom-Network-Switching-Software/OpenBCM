...
err_code_t blackhawk7_l2p2_ucode_crc_verify(srds_access_t *sa__, uint32_t ucode_len, uint16_t expected_crc_value) {
    ...
    if (status_byte & 0x20) {
       ...
       return (blackhawk7_l2p2_error(sa__, ERR_CODE_UCODE_IP_DOES_NOT_MATCH_CORE));
    }
...
err_code_t blackhawk7_l2p2_force_lane_cdr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_force_cdr_mode_enum mode) {
    ...
    switch(mode) {
        case BLACKHAWK7_L2P2_OSCDR_FORCE_ENABLE:
            ...
            EFUN(wrv_usr_misc_ctrl_word(misc_ctrl | (0x1 << 3))); /* set bit 3 (force_os_on) */
            /* fall through to disable BRCDR */
            /* FALLTHRU */
        ...
        case BLACKHAWK7_L2P2_BRCDR_FORCE_ENABLE:
            ...
            /* fall through to disable OSCDR */
            /* FALLTHRU */
        case BLACKHAWK7_L2P2_OSCDR_FORCE_DISABLE:
            ESTM(misc_ctrl = rdv_usr_misc_ctrl_word());
            EFUN(wrv_usr_misc_ctrl_word(misc_ctrl & (uint16_t)~(0x1 << 3)));
            break;
        default:
...
err_code_t blackhawk7_l2p2_ucode_pram_load(srds_access_t *sa__, char const * ucode_image, uint32_t ucode_len) {
    ...
    if (ucode_len > UCODE_MAX_SIZE) {                       /* uCode size should be less than UCODE_MAX_SIZE  */
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_INVALID_UCODE_LEN));
    }
...
err_code_t blackhawk7_l2p2_uc_reset(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
  ...
  if (enable) {
...

    {
      ...
      EFUN(reg_wrc_MICRO_C_COM_RAM_CONTROL2((uint16_t)(0x8000 | (stack_size)<<2)));
    }
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_poll_micro_ra_initdone(sa__, 300)); /* Poll status of data RAM initialization */
...
err_code_t blackhawk7_l2p2_init_blackhawk7_l2p2_info(srds_access_t *sa__) {
    ...
    blackhawk7_l2p2_info_ptr->signature = SRDS_INFO_SIGNATURE;
    blackhawk7_l2p2_info_ptr->info_table_version = (uint8_t)(info_table_signature >> 24);
    {
        uint8_t afe_version;
        uint8_t revid2;
        ESTM(revid2 = rdc_revid2());
        ESTM(afe_version = rdc_afe_ipversion_id());
        blackhawk7_l2p2_info_ptr->silicon_version = GET_UIP_VERSION(afe_version, revid2);
    }
#ifdef PHYMOD_SUPPORT
    }
#endif
...
err_code_t blackhawk7_l2p2_ucode_load_init(srds_access_t *sa__, uint8_t pram) {
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_poll_micro_ra_initdone(sa__, 500));/* Poll for micro_ra_initdone = 1 to indicate initialization done */
