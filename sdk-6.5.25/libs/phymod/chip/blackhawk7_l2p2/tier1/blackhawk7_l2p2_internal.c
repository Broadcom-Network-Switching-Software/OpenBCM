...
extern err_code_t blackhawk7_l2p2_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l2p2_ber_data_st *ber_data, struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl);
extern err_code_t blackhawk7_l2p2_get_BER_data_HW_mode(srds_access_t *sa__, struct blackhawk7_l2p2_ber_data_st *ber_data, struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl);
...
err_code_t blackhawk7_l2p2_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l2p2_ber_data_st *ber_data, enum blackhawk7_l2p2_prbs_chk_timer_selection_enum timer_sel) {
    ...
    if(timer_sel == USE_HW_TIMERS) {
        struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl_bak;
        err_code_t err = ERR_CODE_NONE;
        err = blackhawk7_l2p2_config_BER_HW_mode(sa__, time_ms, ber_data, &prbs_chk_hw_timer_ctrl_bak);
        if(err != ERR_CODE_NONE) {
            return err;
        }
        EFUN(blackhawk7_l2p2_get_BER_data_HW_mode(sa__, ber_data, &prbs_chk_hw_timer_ctrl_bak));
    } else
    {
        uint8_t clk_gate = 0;
        UNUSED(timer_sel);
...
uint16_t blackhawk7_l2p2_INTERNAL_eye_to_mV(srds_access_t *sa__, uint8_t var, uint8_t ladder_range) {
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_ladder_setting_to_mV(sa__, var, ladder_range, slicer, &nlmv_val));
...
err_code_t blackhawk7_l2p2_INTERNAL_read_lane_state(srds_access_t *sa__, blackhawk7_l2p2_lane_state_st *istate) {
  INIT_SRDS_ERR_CODE

  blackhawk7_l2p2_lane_state_st state;

  ENULL_MEMSET(&state, 0, sizeof(blackhawk7_l2p2_lane_state_st));

  if(!istate)
    return(blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  state.lane_idx = blackhawk7_l2p2_acc_get_lane(sa__);

  EFUN(blackhawk7_l2p2_INTERNAL_pmd_lock_status(sa__,&state.rx_lock, &state.rx_lock_chg));
  {
      err_code_t err_code = 0;
      state.stop_state = blackhawk7_l2p2_INTERNAL_stop_micro(sa__,state.rx_lock,&err_code);
      if(err_code) USR_PRINTF(("WARNING: Unable to stop microcontroller,  following data is suspect\n"));
  }
  EFUN(blackhawk7_l2p2_get_rx_tuning_status(sa__, &state.rx_tuning_done));
  ...
  if(state.osr_mode.tx_rx != 255) {
      state.osr_mode_tx_str = blackhawk7_l2p2_INTERNAL_get_osr_str(state.osr_mode.tx_rx, GET_OSR_STR_LONG);
  } else {
      state.osr_mode_tx_str = blackhawk7_l2p2_INTERNAL_get_osr_str(state.osr_mode.tx, GET_OSR_STR_SHORT);
      state.osr_mode_rx_str = blackhawk7_l2p2_INTERNAL_get_osr_str(state.osr_mode.rx, GET_OSR_STR_SHORT);
  }
  {
     enum blackhawk7_l2p2_rx_pam_mode_enum pam_mode = NRZ;
     EFUN(blackhawk7_l2p2_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));
     state.rx_pam_mode = (uint8_t)pam_mode;
     ESTM(state.tx_pam_mode = rd_tx_pam4_mode());
  }
  {
  ...
  EFUN(blackhawk7_l2p2_INTERNAL_sigdet_status(sa__,&state.sig_det, &state.sig_det_chg));
  ...
  EFUN(blackhawk7_l2p2_INTERNAL_decode_br_os_mode(sa__, &state.br_pd_en));
  EFUN(blackhawk7_l2p2_read_rx_afe(sa__, RX_AFE_PF, &state.pf_main));
  EFUN(blackhawk7_l2p2_read_rx_afe(sa__, RX_AFE_PF2, &state.pf2_ctrl));
  ESTM(state.bwr_ctrl = rdv_afe_bw_adjust());
  EFUN(blackhawk7_l2p2_read_rx_afe(sa__, RX_AFE_VGA, &state.vga));

  ESTM(state.dc_offset = rd_dc_offset_bin());
  EFUN(blackhawk7_l2p2_read_rx_afe(sa__, RX_AFE_PF3, &state.pf3_ctrl));
  ESTM(state.p1_lvl = rdv_usr_main_tap_est()/32);
  EFUN(blackhawk7_l2p2_INTERNAL_ladder_setting_to_mV(sa__, state.p1_lvl, 0, INVALID_SLICER, &state.p1_lvl));
  if (state.rx_pam_mode == NRZ) {
      ESTM(state.m1_lvl = rd_rx_phase02_status());
      EFUN(blackhawk7_l2p2_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)state.m1_lvl, 0, INVALID_SLICER, &state.m1_lvl));
  ...
  if (state.rx_pam_mode == NRZ) {
      EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe1(sa__, &state.dfe1));
  }
  EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe2(sa__, &state.dfe2));
  EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe3(sa__, &state.dfe3));
  EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe4(sa__, &state.dfe4));
  EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe5(sa__, &state.dfe5));
  EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe6(sa__, &state.dfe6));

  EFUN(blackhawk7_l2p2_INTERNAL_get_tx_ppm(sa__, &state.tx_ppm));
  ...
  ESTM(state.txfir_use_pam4_range = rd_txfir_nrz_tap_range_sel() ? 0 : 1);
  {
      uint8_t tap_num;
      for (tap_num=0; tap_num<TXFIR_ST_NUM_TAPS; ++tap_num) {
          EFUN(blackhawk7_l2p2_INTERNAL_get_tx_tap(sa__, tap_num, &state.txfir.tap[tap_num]));
      }
      ESTM(state.enable_6taps = rd_txfir_tap_en());
  ...
  EFUN(blackhawk7_l2p2_INTERNAL_get_eye_margin_est(sa__, &state.heye_left, &state.heye_right, &state.veye_upper, &state.veye_lower));
  {
      struct blackhawk7_l2p2_usr_ctrl_disable_functions_st dsu, dss;
      ...
      state.disable_eye_display = nrz_mode_en || pam4_mode_en;
  }
  EFUN(blackhawk7_l2p2_INTERNAL_get_link_time(sa__, &state.link_time));
  {
      ...
      if (state.rx_lock != rx_lock_at_end) {
          USR_PRINTF(("WARNING: Lane %d rx_lock status changed while reading lane state (rx_lock_start=%d, rx_lock_chg_start=%d, rx_lock_at_end=%d, rx_lock_chg_at_end=%d) \n",
                      blackhawk7_l2p2_acc_get_lane(sa__), state.rx_lock, state.rx_lock_chg, rx_lock_at_end, rx_lock_chg_at_end));
      }
      state.rx_lock &= rx_lock_at_end;
      state.rx_lock_chg |= rx_lock_chg_at_end;
  }
  ESTM(state.tx_pmd_dp_invert = rd_tx_pmd_dp_invert());
  ESTM(state.rx_pmd_dp_invert = rd_rx_pmd_dp_invert());
  ESTM(state.rmt_lpbk_en = rd_rmt_lpbk_en());
  ESTM(state.dig_lpbk_en = rd_dig_lpbk_en());
  ESTM(state.ilb_en = rd_ilb_en());
  ESTM(state.tx_pi_en = rd_tx_pi_en());
  ...
  EFUN(blackhawk7_l2p2_INTERNAL_get_BER_string(sa__, 100, state.ber_str, sizeof(state.ber_str)));
  {
    const char * const prepend = "    ";
    if(state.ucv_status != 0) {
      EFUN(blackhawk7_l2p2_decode_uc_sts(sa__, state.ucv_status, state.uc_sts_decoded, sizeof(state.uc_sts_decoded), prepend));
    }
    if(state.ucv_status_ext != 0) {
      EFUN(blackhawk7_l2p2_decode_uc_sts_ext(sa__, state.ucv_status_ext, state.uc_sts_ext_decoded, sizeof(state.uc_sts_ext_decoded), prepend));
    }
  }
...
err_code_t blackhawk7_l2p2_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {
  ...
  EFUN(blackhawk7_l2p2_INTERNAL_read_lane_state(sa__, &state));

  EFUN_PRINTF(("%2d ", state.lane_idx));
  if(state.osr_mode.tx_rx != 255) {
      EFUN_PRINTF(("(%s%s%2s%s, 0x%04x, 0x%02x ,", (state.tx_pmd_dp_invert ? "-" : "+"), (state.rx_pmd_dp_invert ? "-" : "+"), (state.br_pd_en) ? "BR" : "OS",
                                                    state.osr_mode_tx_str, state.ucv_config,state.ucv_status));
  } else {
      EFUN_PRINTF(("(%s%s%3s%s,%2s%s:%s, 0x%04x, 0x%02x_%04x,", (state.tx_pmd_dp_invert ? "-" : "+"), (state.rx_pmd_dp_invert ? "-" : "+"),
                    ...
                    (state.br_pd_en ? "BR" : "OS"), state.osr_mode_tx_str, state.osr_mode_rx_str, state.ucv_config, state.ucv_status, state.ucv_status_ext));
  }
  ...
  EFUN_PRINTF(("%s", state.rx_lock ? (state.rx_tuning_done ? " " : "+") : " "));
...
static err_code_t _blackhawk7_l2p2_check_vco_freq_khz(srds_access_t *sa__, uint32_t vco_freq_khz) {
...
err_code_t blackhawk7_l2p2_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    if (*vco_freq_khz != 0) {
        EFUN(_blackhawk7_l2p2_check_vco_freq_khz(sa__,*vco_freq_khz));
    }
