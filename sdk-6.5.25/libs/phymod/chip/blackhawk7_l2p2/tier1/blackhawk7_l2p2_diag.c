        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                ...
                if(i[lane] >= -i_max[lane]) {
                    ...
                    if(enable_read) {
                        local_error = blackhawk7_l2p2_read_eye_scan_stripe(sa__, &stripe[0], &status);
                        if (local_error) {
                            lane_mask &= (uint8_t)~(1<<lane);
                            _blackhawk7_l2p2_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                            continue;
                    ...
                    local_error = blackhawk7_l2p2_display_eye_scan_stripe(sa__, i[lane], &stripe[buffer_idx], usr_diag_status[lane]);
                    if (local_error) {
                        lane_mask &= (uint8_t)~(1<<lane);
                        _blackhawk7_l2p2_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                        continue;
...
err_code_t blackhawk7_l2p2_display_diag_data_core(srds_access_t *sa__, uint16_t  diag_level, uint8_t lane_select) {
    INIT_SRDS_ERR_CODE
    uint8_t rx_lock, micro_stop, lane, num_lanes=0, save_lane=0;
    uint8_t core;
    char core_type[25] = "blackhawk7_l2p2";
    uint16_t data_cnt=0;
    ...
    EFUN_PRINTF(("\n"));
    EFUN_PRINTF(("SerDes type          = %s\n",core_type));
    ESTM_PRINTF(("Silicon Version      = V%X\n", BLACKHAWK7_L2P2_UAPI_SWITCH));
    EFUN_PRINTF(("\n"));
    ...
    EFUN(blackhawk7_l2p2_pmd_lock_status(sa__,&rx_lock));
    {
        ...
        micro_stop = blackhawk7_l2p2_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
...
Exit:
    ...
    if((diag_level & SRDS_DIAG_PRBS_PROJ)
        ...
        EFUN_PRINTF(("\n********************************************************\n"));
        EFUN_PRINTF(("Running CORE %d LANE %d PRBS Error Analyzer for %d seconds\n", blackhawk7_l2p2_acc_get_core(sa__), blackhawk7_l2p2_acc_get_lane(sa__), timeout_s));
        EFUN_PRINTF(("********************************************************\n"));
        /* Display PRBS Error Analyzer for 60s */
        lane_mask_aggregate      = (uint8_t)(0x1 << (blackhawk7_l2p2_acc_get_lane(sa__)));
