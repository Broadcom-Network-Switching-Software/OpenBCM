...
err_code_t falcon16_v1l4p1_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string) {
    char string2[3];
    struct falcon16_v1l4p1_ber_data_st ber_data_local;
...
err_code_t falcon16_v1l4p1_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct falcon16_v1l4p1_ber_data_st *ber_data, enum falcon16_v1l4p1_prbs_chk_timer_selection_enum timer_sel) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    if((time_ms <= PRBS_MAX_HW_TIMER_TIMEOUT) && (timer_sel == USE_HW_TIMERS)) {
        ...
        struct falcon16_v1l4p1_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl_bak;
        USR_MEMSET(&prbs_chk_hw_timer_ctrl_bak, 0, sizeof(struct falcon16_v1l4p1_prbs_chk_hw_timer_ctrl_st));
...
#if defined(FALCON16_V1L4P1_PRBS_CHK_HARDWARE_TIMERS)
err_code_t falcon16_v1l4p1_INTERNAL_get_prbs_timeout_count_from_time(uint16_t time_ms, uint16_t * time_ms_adjusted, struct falcon16_v1l4p1_prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl ) {
...
err_code_t falcon16_v1l4p1_INTERNAL_get_vco_from_refclk_div(srds_access_t *sa__, uint32_t refclk_freq_hz, enum falcon16_v1l4p1_pll_div_enum srds_div, uint32_t *vco_freq_khz, enum falcon16_v1l4p1_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    uint64_t temp_refclk_freq_hz;
    EFUN(_falcon16_v1l4p1_get_divisor_ratio(sa__, srds_div, &numerator, &denominator));
    if (pll_option == FALCON16_V1L4P1_PLL_OPTION_REFCLK_DOUBLER_EN) refclk_freq_hz *= 2;
    if (pll_option == FALCON16_V1L4P1_PLL_OPTION_REFCLK_DIV2_EN)    refclk_freq_hz /= 2;
    if (pll_option == FALCON16_V1L4P1_PLL_OPTION_REFCLK_DIV4_EN)    refclk_freq_hz /= 4;
    /* *vco_freq_khz =(uint32_t) (((uint64_t)refclk_freq_hz * denominator + (numerator >> 1)) / numerator); */
    COMPILER_64_SET(temp_refclk_freq_hz, 0, refclk_freq_hz);
    COMPILER_64_UMUL_32(temp_refclk_freq_hz, denominator);
    COMPILER_64_ADD_32(temp_refclk_freq_hz, (numerator >> 1));
    COMPILER_64_UDIV_32(temp_refclk_freq_hz, numerator);
    *vco_freq_khz = COMPILER_64_LO(temp_refclk_freq_hz);
    return (ERR_CODE_NONE);
}
