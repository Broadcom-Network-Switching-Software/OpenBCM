...
/**************************************************************************************
 **************************************************************************************
 *  File Name     :  blackhawk7_v1l8p1_pwr_mgt.c                                             *
...
#include "blackhawk7_v1l8p1_pwr_mgt.h"
...
err_code_t blackhawk7_v1l8p1_core_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    switch(mode) {
    case PWR_ON:
        EFUN(blackhawk7_v1l8p1_INTERNAL_core_clkgate(sa__, 0));
    ...
    case PWRDN_DEEP:
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_core_clkgate(sa__, 1));
        break;
    default:
        EFUN(blackhawk7_v1l8p1_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
        break;
    }
    return ERR_CODE_NONE;
}
...
err_code_t blackhawk7_v1l8p1_lane_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    switch(mode) {
    case PWR_ON:
        EFUN(blackhawk7_v1l8p1_INTERNAL_lane_clkgate(sa__, 0));
        EFUN(blackhawk7_v1l8p1_lane_pwrdn(sa__, PWR_ON_RX));
        EFUN(blackhawk7_v1l8p1_lane_pwrdn(sa__, PWR_ON_TX));
        break;
    case PWR_ON_TX:
    ...
    case PWR_ON_RX:
        ...
        break;
    case PWRDN:
        EFUN(blackhawk7_v1l8p1_lane_pwrdn(sa__, PWRDN_TX));
        EFUN(blackhawk7_v1l8p1_lane_pwrdn(sa__, PWRDN_RX));
        break;
    case PWROFF_DEEP: /* PWROFF_DEEP is identical to PWRDN_DEEP for lane pwrdn. FALL THROUGH */
    case PWRDN_DEEP:
        EFUN(blackhawk7_v1l8p1_lane_pwrdn(sa__, PWRDN_TX_DEEP));
        EFUN(blackhawk7_v1l8p1_lane_pwrdn(sa__, PWRDN_RX_DEEP));
        EFUN(blackhawk7_v1l8p1_INTERNAL_lane_clkgate(sa__, 1));
        break;
    case PWRDN_TX:
...
err_code_t blackhawk7_v1l8p1_pwrdn_all(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    for (lane = 0; lane < lanes_per_core; ++lane) {
        ...
        EFUN(blackhawk7_v1l8p1_lane_pwrdn(sa__, mode));
    }
    EFUN(blackhawk7_v1l8p1_acc_set_lane(sa__, lane_orig));
    {
      ...
      for (pll = 0; pll < NUM_PLLS; ++pll) {
        ...
        EFUN(blackhawk7_v1l8p1_core_pwrdn(sa__, mode));
      }
