...
/**
@brief   enable the Port(Forced Speed Mode)
@param   pc handle to current TSCE context (#PHYMOD_ST)
@returns The value PHYMOD_E_NONE upon successful completion
@details Per Port PCS Init

*/
int temod16_enable_set(PHYMOD_ST* pc)              /* INIT_PCS */
{
  SC_X4_CTLr_t reg_sc_ctrl;
...
int temod16_pcs_lane_swap_get(PHYMOD_ST *pc,  uint32_t *tx_rx_swap)
{
  unsigned int lane_swap = 0;
  MAIN0_LN_SWPr_t reg_lane_swap;
  phymod_access_t pa_copy;
  TMOD_DBG_IN_FUNC_INFO(pc);

  MAIN0_LN_SWPr_CLR(reg_lane_swap);
  ...
  lane_swap = MAIN0_LN_SWPr_LOG0_TO_PHY_TX_LNSWAP_SELf_GET(reg_lane_swap);
  *tx_rx_swap |= ((lane_swap & 0x3) << 0);

  lane_swap = MAIN0_LN_SWPr_LOG1_TO_PHY_TX_LNSWAP_SELf_GET(reg_lane_swap);
  *tx_rx_swap |= ((lane_swap & 0x3) << 4);

  lane_swap = MAIN0_LN_SWPr_LOG2_TO_PHY_TX_LNSWAP_SELf_GET(reg_lane_swap);
  *tx_rx_swap |= ((lane_swap & 0x3) << 8);

  lane_swap = MAIN0_LN_SWPr_LOG3_TO_PHY_TX_LNSWAP_SELf_GET(reg_lane_swap);
  *tx_rx_swap |= ((lane_swap & 0x3) << 12);

  lane_swap = MAIN0_LN_SWPr_LOG0_TO_PHY_RX_LNSWAP_SELf_GET(reg_lane_swap);
  *tx_rx_swap |= ((lane_swap & 0x3) << 16);

  lane_swap = MAIN0_LN_SWPr_LOG1_TO_PHY_RX_LNSWAP_SELf_GET(reg_lane_swap);
  *tx_rx_swap |= ((lane_swap & 0x3) << 20);

  lane_swap = MAIN0_LN_SWPr_LOG2_TO_PHY_RX_LNSWAP_SELf_GET(reg_lane_swap);
  *tx_rx_swap |= ((lane_swap & 0x3) << 24);

  lane_swap = MAIN0_LN_SWPr_LOG3_TO_PHY_RX_LNSWAP_SELf_GET(reg_lane_swap);
  *tx_rx_swap |= ((lane_swap & 0x3) << 28);
...
int temod16_refclk_get(PHYMOD_ST* pc, phymod_ref_clk_t *ref_clk)
{
    MAIN0_SETUPr_t  reg_setup;
    uint32_t clk = 0;
    ...
    clk = MAIN0_SETUPr_REFCLK_SELf_GET(reg_setup);

    if (clk == main0_refClkSelect_clk_125MHz) {
...
STATIC
int _temod16_tx_phy_lane_get(PHYMOD_ST *pc, int tx_logical_lane, int* tx_phy_lane)
{
    MAIN0_LN_SWPr_t reg_lane_swap;
    phymod_access_t pa_copy;
    ...
    /*always use lane 0 since register is single copy */
    pa_copy.lane_mask = 0x1;
...
int temod16_port_start_lane_get(PHYMOD_ST *pc, int *port_starting_lane, int *port_num_lane)
{
    ...
    if (num_of_lane > 1) {
        ...
        *port_num_lane = num_of_lane;
        ...
        switch (port_mode_sel_reg) {
            case TEMOD16_PORT_MODE_SINGLE:
                /*single mode*/
                *port_starting_lane = 0;
                *port_num_lane = 4;
                break;
            case TEMOD16_PORT_MODE_DUAL:
                /*dual mode*/
                if (start_lane < 2) {
                    *port_starting_lane = 0;
                    *port_num_lane = 2;
...
int temod16_fec_error_inject_set(PHYMOD_ST *pc,
                                 uint16_t error_control_map,
                                 temod16_fec_error_mask_t bit_error_mask)
{
    int start_lane = 0, num_lane = 0, phy_lane = 0;
    int i = 0;
    ...
    for (i = 0; i < num_lane; i++) {
         ...
         PHYMOD_IF_ERR_RETURN
             (_temod16_tx_phy_lane_get(pc, start_lane + i, &phy_lane));
...
int temod16_fec_error_inject_get(PHYMOD_ST *pc,
                                 uint16_t* error_control_map,
                                 temod16_fec_error_mask_t* bit_error_mask)
{
    int start_lane = 0, num_lane = 0, phy_lane = 0;
...
int temod16_encode_mode_get(PHYMOD_ST* pc, temod16_encoder_type_t *mode)
{
...
int temod16_fec_correctable_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
    ...
    *count = (RX_X4_FEC_CORRBLKSHr_GET(counth) << 16) | (RX_X4_FEC_CORRBLKSLr_GET(countl) & 0xffff);

    return PHYMOD_E_NONE;
}

int temod16_fec_uncorrectable_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
    ...
    *count = (RX_X4_FEC_UNCORRBLKSHr_GET(counth) << 16) | (RX_X4_FEC_UNCORRBLKSLr_GET(countl) & 0xffff);

    return PHYMOD_E_NONE;
}

/* The number of cl82 BIP error */
int temod16_bip_error_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
...
int temod16_cl49_ber_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
    RX_X4_BERCNTr_t bercnt;
...
int temod16_cl82_ber_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
