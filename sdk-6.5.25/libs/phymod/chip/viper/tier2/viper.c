    ...
    for (i = 0; i < num_lane; i++) {
         ...
         switch (tx_control) {
            case phymodTxSquelchOn:
                PHYMOD_IF_ERR_RETURN(viper_tx_squelch_set(&phy_copy, 1));
                break;
            case phymodTxSquelchOff:
                PHYMOD_IF_ERR_RETURN(viper_tx_squelch_set(&phy_copy, 0));
                break;
            default:
                break;
        }
...
int viper_phy_tx_lane_control_get(const phymod_phy_access_t* phy, phymod_phy_tx_lane_control_t* tx_control)
{
    ...
    PHYMOD_IF_ERR_RETURN(viper_tx_squelch_get(&phy_copy, &tx_squelch));
    *tx_control = tx_squelch ? phymodTxSquelchOn : phymodTxSquelchOff;
