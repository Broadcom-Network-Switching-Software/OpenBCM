...
{        
    ...
    PHYMOD_IF_ERR_RETURN
        (tefmod_gen3_port_start_lane_get(&phy->access,
                                         &port_starting_lane,
                                         &port_num_lane));
    phy_copy.access.lane_mask = 1 << port_starting_lane;
    /* Toggle the pcs data path reset */
    PHYMOD_IF_ERR_RETURN
        (tefmod_gen3_trigger_speed_change(&phy_copy.access));
...
{
    PHYMOD_IF_ERR_RETURN
        (tefmod_gen3_1588_ts_offset_get(&core->access, ts_offset));
...
int tscf_gen3_timesync_adjust_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t ts_am_norm_mode)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (tefmod_gen3_speed_id_get(&phy_copy.access, &speed_id));
