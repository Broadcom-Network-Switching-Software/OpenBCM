       ...
       for (i = 0; i < 4; i++) {
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            default:
                ...
                if (i == 0) {
                    PHYMOD_IF_ERR_RETURN
                        (eagle_tsc_display_diag_data(&phy_copy.access, SRDS_DIAG_CORE|SRDS_DIAG_UC_CORE|SRDS_DIAG_LANE|SRDS_DIAG_REG_CORE|SRDS_DIAG_UC_LANE|SRDS_DIAG_EYE));
                } else {
                    PHYMOD_IF_ERR_RETURN
                        (eagle_tsc_display_diag_data(&phy_copy.access, SRDS_DIAG_LANE|SRDS_DIAG_REG_CORE|SRDS_DIAG_UC_LANE|SRDS_DIAG_EYE));
                }
...
int tsce_phy_fec_error_inject_set(const phymod_phy_access_t* phy,
                                  ...
                                  phymod_fec_error_mask_t bit_error_mask)
{
    ...
    temod_fec_error_mask_t fec_error_mask;
    ...
    PHYMOD_IF_ERR_RETURN(temod_fecmode_get(&phy->access, &fec_enable));
    if (!fec_enable) {
    ...
    PHYMOD_IF_ERR_RETURN(temod_fec_error_inject_set(&phy_copy.access, error_control_map, fec_error_mask));

    return PHYMOD_E_NONE;
}

int tsce_phy_fec_error_inject_get(const phymod_phy_access_t* phy,
                                       uint16_t* error_control_map,
                                       phymod_fec_error_mask_t* bit_error_mask)
{
    temod_fec_error_mask_t fec_error_mask;
    ...
    PHYMOD_IF_ERR_RETURN(temod_fec_error_inject_get(&phy->access, &control_map, &fec_error_mask));
