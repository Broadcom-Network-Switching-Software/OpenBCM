...
typedef struct temod_fec_error_mask_s {
...
} temod_fec_error_mask_t;

typedef enum {
    TEMOD_PORT_MODE_QUAD  = 0,    /* quad port mode.  */
    TEMOD_PORT_MODE_TR1,          /* lanes 2,3 aggregated   */
    TEMOD_PORT_MODE_TR2,          /* lanes 1,0 aggregated   */
    TEMOD_PORT_MODE_DUAL,         /* dual port mode.  */
    TEMOD_PORT_MODE_SINGLE,       /* single port mode. */
    TEMOD_PORT_MODE_COUNT
} temod_port_mode_type;
...
extern  int temod_diag_disp(PHYMOD_ST *pc, const char* mystr);
...
extern int temod_fec_error_inject_set(PHYMOD_ST *pc,
                                      uint16_t error_control_map,
                                      temod_fec_error_mask_t bit_error_mask);
extern int temod_fec_error_inject_get(PHYMOD_ST *pc,
                                      uint16_t* error_control_map,
                                      temod_fec_error_mask_t* bit_error_mask);
extern int temod_port_start_lane_get(PHYMOD_ST *pc,
