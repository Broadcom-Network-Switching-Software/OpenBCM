...
int temod_autoneg_control(PHYMOD_ST* pc, temod_an_control_t *an_control)
{
  ...
  switch (an_control->an_type) {
    ...
    if(an_control->enable){
     ...
     /* Disable remote fault reporting, because RF1 and RF2 are swapped in A0 */
     if(an_control->an_type == TEMOD_AN_MODE_CL37){
         PHYMOD_IF_ERR_RETURN(temod_revid_read(pc, &revid));
         if ((revid & 0xf800) == REV_A0) {
             DIG_CTL1000X2r_CLR(reg_ctrl1000x2);
             DIG_CTL1000X2r_DISABLE_REMOTE_FAULT_REPORTINGf_SET(reg_ctrl1000x2, 1);
             PHYMOD_IF_ERR_RETURN(MODIFY_DIG_CTL1000X2r(pc, reg_ctrl1000x2));
         }
     }
...
int temod_fec_error_inject_set(PHYMOD_ST *pc,
                               ...
                               temod_fec_error_mask_t bit_error_mask)
{
    ...
    for (i = 0; i < num_lane; i++) {
         ...
         PHYMOD_IF_ERR_RETURN
             (_temod_rx_phy_lane_get(pc, start_lane + i, &phy_lane));
...
int temod_fec_error_inject_get(PHYMOD_ST *pc,
                               uint16_t* error_control_map,
                               temod_fec_error_mask_t* bit_error_mask)
{
    int start_lane = 0, num_lane = 0, phy_lane = 0;
...
int temod_port_start_lane_get(PHYMOD_ST *pc,
    ...
    if (num_of_lane > 1) {
        ...
        *port_num_lane = num_of_lane;
        ...
        switch (port_mode_sel_reg) {
            case TEMOD_PORT_MODE_SINGLE:
                /*single mode*/
                *port_starting_lane = 0;
                *port_num_lane = 4;
                break;
            case TEMOD_PORT_MODE_DUAL:
                /*dual mode*/
                if (start_lane < 2) {
                    *port_starting_lane = 0;
                    *port_num_lane = 2;
