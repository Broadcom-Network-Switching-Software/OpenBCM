...
err_code_t merlin16_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(merlin16_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_VSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_HORIZ) {
        EFUN(merlin16_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_HSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_SLICE) {
        EFUN(merlin16_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_EYE_SLICE,GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}

err_code_t merlin16_INTERNAL_poll_diag_data(srds_access_t *sa__, const merlin16_info_t *merlin16_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
...
err_code_t merlin16_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    EFUN(merlin16_INTERNAL_verify_merlin16_info(merlin16_info_ptr, sa__));
        ...
        lane_diag_base = merlin16_info_ptr->diag_mem_ram_base + ((lane%merlin16_info_ptr->lane_count) * merlin16_info_ptr->diag_mem_ram_size) + 
            merlin16_info_ptr->grp_ram_size*merlin16_INTERNAL_grp_idx_from_lane(merlin16_get_physical_lane(sa__));

    EFUN(merlin16_INTERNAL_poll_diag_data(sa__, merlin16_info_ptr, status, &diag_rd_ptr, stripe_size, 400));
...
err_code_t merlin16_meas_eye_scan_done(srds_access_t *sa__) {
  EFUN(merlin16_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_DISABLE,GRACEFUL_STOP_TIME));
  return(ERR_CODE_NONE);
}
...
err_code_t merlin16_start_ber_scan_test(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin16_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_START, ber_scan_mode,GRACEFUL_STOP_TIME));
    return(ERR_CODE_NONE);
}
...
err_code_t merlin16_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin16_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,200));
...
err_code_t merlin16_display_lane_state_hdr(void) {
  EFUN_PRINTF(("LN (CDRxN  , UC_CFG,UC_STS,TX_RX_RST,STP) "));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if((osr_mode.rx > MERLIN16_OSX4) && (osr_mode.rx != MERLIN16_OSX2P5)) {
#else
    if(osr_mode.rx > MERLIN16_OSX4) {
#endif
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(merlin16_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        EFUN(merlin16_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
...
err_code_t merlin16_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    ESTM(rx_lock = rd_pmd_rx_lock());
    {
        ...
        micro_stop = merlin16_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
...
err_code_t merlin16_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin16_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + merlin16_info_ptr->lane_var_ram_base + (merlin16_get_physical_lane(sa__) * merlin16_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
    ...
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin16_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, GRACEFUL_STOP_TIME));
