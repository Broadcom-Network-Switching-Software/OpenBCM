...
err_code_t blackhawk_tsc_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    EFUN(blackhawk_tsc_INTERNAL_verify_blackhawk_tsc_info(blackhawk_tsc_info_ptr, sa__));
        ...
        lane_diag_base = blackhawk_tsc_info_ptr->diag_mem_ram_base + ((lane%blackhawk_tsc_info_ptr->lane_count) * blackhawk_tsc_info_ptr->diag_mem_ram_size) +
            (uint32_t)(blackhawk_tsc_info_ptr->grp_ram_size*blackhawk_tsc_INTERNAL_grp_idx_from_lane(blackhawk_tsc_get_physical_lane(sa__)));
    ...
    EFUN(blackhawk_tsc_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
                                              (uint16_t)blackhawk_tsc_info_ptr->diag_mem_ram_size,
                                              diag_rd_ptr,
                                              stripe_size,
                                              &arg,
                                              blackhawk_tsc_read_eye_scan_stripe_callback));

    diag_rd_ptr = (uint8_t)(((uint32_t)diag_rd_ptr + stripe_size) % blackhawk_tsc_info_ptr->diag_mem_ram_size);
...
err_code_t blackhawk_tsc_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(blackhawk_tsc_INTERNAL_poll_diag_done(sa__, &sts,(uint32_t)(timer_control*2000)));
    ...
    do {
        if(heye == 1) {
            ...
            if(intrusive) {
                ...
                margins[i] = direction*blackhawk_tsc_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], 0);
            } else {
                margins[i] = direction*blackhawk_tsc_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250);
            }
#else
            margins[i] = info_out->margins[i];
#endif
...
err_code_t blackhawk_tsc_display_core_state_hdr(void) {
    char core_type[23] = "blackhawk_tsc_b0";
...
err_code_t blackhawk_tsc_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(blackhawk_tsc_acc_rdt_reg(sa__, (uint16_t)(i+addr),&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(blackhawk_tsc_acc_rdt_reg(sa__, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(blackhawk_tsc_acc_wr_reg(sa__, addr,tmp2));
    ...
    case SRDS_EN_BREAKPOINT: /* Add feature when available */
