...
*  Revision      :  $Id: falcon_tsc_functions.c 1586 2018-11-15 21:58:26Z jg943801 $ *
*                                                                                 *
...
err_code_t falcon_tsc_display_core_state_hdr (const phymod_access_t *pa){
    ...
    EFUN_PRINTF(("CORE RST ST PLL_PWDN  UC_ATV   COM_CLK   UCODE_VER  API_VER  AFE_VER   LIVE_TEMP   AVG_TMON   RESCAL   VCO_RATE  ANA_VCO_RANGE   PLL_DIV    PLL_LOCK\n"));
	return ERR_CODE_NONE;
}
	...
	for (loop = 0; loop < 100; loop++) {
		...
		if (rddata & 0x0080) {          /* bit 7 is uc_dsc_ready_for_cmd */
			if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
                ...
                EFUN(falcon_tsc_pmd_wr_reg(pa, DSC_A_DSC_UC_CTRL,0x80));
...
err_code_t falcon_tsc_event_log_stop (const phymod_access_t *pa){
	...
	EFUN(falcon_tsc_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 1000));
...
err_code_t falcon_tsc_event_log_readmem (const phymod_access_t *pa, uint8_t *trace_mem){
	...
	do {
		/* Read Next */
		EFUN(falcon_tsc_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 1000));
	...
	EFUN(falcon_tsc_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 1000));
