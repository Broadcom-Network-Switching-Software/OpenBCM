...
#define BCM_QOS_MAP_GTP                 BCM_QOS_MAP_PACKET_INVALID 
...
/* Extract dscp_map (table index) for the use of bcm_tunnel_initiator_set */
#define BCM_QOS_MAP_EGR_DSCP_TBL_IDX_GET(map_id)               ((map_id)  & 0x3ff) 

#define BCM_QOS_MAP_TYPE_EGR_DSCP_TABLE                        (4) 

/* Construct qos_mapid by dscp_map from bcm_tunnel_initiator_get */
#define BCM_QOS_MAP_EGR_DSCP_TABLE_MAP_ID_CONSTRUCT(idx)       ((BCM_QOS_MAP_TYPE_EGR_DSCP_TABLE << 10) | ((idx) & 0x3ff)) 
...
typedef enum bcm_qos_control_type_e {
    ...
    bcmQosControlMplsPhpTtlUseMin = 9,  /* Use minimal value of popped label's
                                           TTL and above header's TTL in case
                                           MPLS PHP TTL mode is uniform */
    bcmQosControl__Count = 10           /* Always last. Not a usable value. */
