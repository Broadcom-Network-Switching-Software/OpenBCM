...
/**
 * \brief If 'expression' is 'error code', log an error message, continue and
 * return fail at the end of the function.
 *
 * The expression can be a function call or a fixed error code.
 *
...
/** exit in case of error, in case the error core is the exception error code, exit without error */
...
/**
 * \brief Memory allocation with leak check. Goto error exit on alloc fail.
 *
 * This macro is exactly the same as SHR_ALLOC but, if allocation fails,
 * control goes to 'exit' with an error code (_SHR_E_MEMORY).
 */
...
/**
 * \brief Memory allocation with leak check and error exit on alloc fail
 *
 * This macro is exactly the same as SHR_ALLOC2 but, if allocation fails,
 * control goes to 'exit' with an error code (_SHR_E_MEMORY).
 *
 */
...
/**
 * \brief Memory allocation with leak check and setting to zero
 *        the allocated memory. Goto error exit on alloc fail.
 *
 * This macro is exactly the same as SHR_ALLOC_SET_ZERO but, if allocation fails,
 * control goes to 'exit' with an error code (_SHR_E_MEMORY).
 */
