...
typedef enum
{
    ...
    TERM_LOOKUP_DELETE,
    /**
     * \brief Add the entry. If existing already, override it.
     */
    TERM_LOOKUP_FORCE_SET,
    /**
     * \brief Get the entry extra. Don't print error msg if not_found
     */
    TERM_LOOKUP_EXTRA_GET
...
/**
 * \brief
 * Create MPLS tunnels in the ingress for termination (pop).
 * Done before forwarding stage.
 * \par DIRECT INPUT
 *    \param [in] unit -
 *     Relevant unit.
 *     \param [in] info -
 *     A pointer to the struct that holds information for the ingress MPLS entry.
 *      info.flags - The requested configuration.
 *      info.label - The label to be looked up for termination.
 *      info.second_label - The second label to be looked up for
 *      termination if info.flags is set with
 *      BCM_MPLS_SWITCH_LOOKUP_SECOND_LABEL.
 *      info.tunnel_id - Will contain global_lif if 'info.flags'
 *      has WITH_ID
 * \par INDIRECT INPUT:
 ...
 * \par INDIRECT OUTPUT
 *   * Write to INGRESS MPLS Hardware.
 *      info.tunnel_id - In case WITH_ID flag is not set global_lif will be returned here.
 * \remark
 * Ingress MPLS entries are those that are matched in
 * VLAN/Tunnel termination prior to forwarding.
 */
