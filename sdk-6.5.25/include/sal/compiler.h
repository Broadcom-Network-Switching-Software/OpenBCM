...
/*
 * Divide a 64 bits number by a 32 bits number.
 * on input:
 *   'dst' is the number to divide.
 * On output:
 *   'dst' is the quotient (result)
 * 'src' is the divisor. Input only. 32bits.
 * 'remainder' is what remains after the division. Output only. 32bits.
 */
#define COMPILER_64_UDIV_32_WITH_REMAINDER(dst, src, remainder)         \
{                                                                       \
    COMPILER_UINT64 u64_dst_base;                                       \
    COMPILER_UINT64 u64_dst_tmp;                                        \
    /*                                                                  \
     * Keep the original dividend in 'u64_dst_base'                     \
     * Load 'dst' with the quotient (result).                           \
     */                                                                 \
    COMPILER_64_COPY(u64_dst_base,dst);                                 \
    COMPILER_64_UDIV_32(dst,src);                                       \
    /*                                                                  \
     * Load result into 'u64_dst_tmp' and multiply by the divisor.      \
     * This number is smaller than the original 'dst' and the           \
     * difference is the remainder.                                     \
     */                                                                 \
    COMPILER_64_COPY(u64_dst_tmp,dst);                                  \
    COMPILER_64_UMUL_32(u64_dst_tmp, src);                              \
    COMPILER_64_SUB_64(u64_dst_base,u64_dst_tmp);                       \
    remainder = COMPILER_64_LO(u64_dst_base);                           \
}
 /*
  * 64-bit power: 
  * dst = base^exp 
...
#ifndef SAL_ATTR_WEAK
#if defined(SAL_WEAK_SYMBOLS) && defined(BCM_SAL_SOURCECODE)
#define SAL_ATTR_WEAK COMPILER_ATTRIBUTE((weak))
#else
/*
 * Either SAL_WEAK_SYMBOLS is not defined
 * or we are outside of BRCM SAL source code.
 */
#define SAL_ATTR_WEAK
#endif /* SAL_WEAK_SYMBOLS build flag */
#endif /* SAL_ATTR_WEAK attribute */
