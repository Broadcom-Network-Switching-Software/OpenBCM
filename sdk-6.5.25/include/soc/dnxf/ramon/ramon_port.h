...
shr_error_e soc_ramon_port_soc_init(
    int unit);

shr_error_e soc_ramon_port_speed_max(
    int unit,
    soc_port_t port,
    int *speed);

shr_error_e soc_ramon_port_init(
    int unit);

shr_error_e soc_ramon_port_deinit(
    int unit);

shr_error_e soc_ramon_port_detach(
    int unit,
    soc_port_t port);

shr_error_e soc_ramon_port_probe(
    int unit,
    pbmp_t pbmp,
    pbmp_t * okay_pbmp,
    int is_init_sequence);

shr_error_e soc_ramon_port_burst_control_set(
    int unit,
    soc_port_t port,
    portmod_loopback_mode_t loopback);

shr_error_e soc_ramon_port_serdes_power_disable(
    int unit,
    soc_pbmp_t disable_pbmp);

shr_error_e soc_ramon_port_link_status_get(
    int unit,
    soc_port_t port,
    int *link_up);

shr_error_e soc_ramon_port_first_link_in_fsrd_get(
    int unit,
    soc_port_t port,
    int *is_first_link);

shr_error_e soc_ramon_port_fsrd_block_power_set(
    int unit,
    soc_port_t port,
    int power_up);

shr_error_e soc_ramon_port_update_valid_block_database(
    int unit,
    int block_type,
    int block_number,
    int enable);

shr_error_e soc_ramon_port_pump_enable_set(
    int unit,
    soc_port_t port,
    int enable);

shr_error_e soc_ramon_port_dynamic_soc_init(
    int unit,
    soc_port_t port,
    int is_first_link);

shr_error_e soc_ramon_port_sync_e_link_set(
    int unit,
    int is_master,
    int port);

shr_error_e soc_ramon_port_sync_e_divider_set(
    int unit,
    int is_master,
    int divider);

shr_error_e soc_ramon_port_sync_e_link_get(
    int unit,
    int is_master,
    int *port);

shr_error_e soc_ramon_port_sync_e_divider_get(
    int unit,
    int is_master,
    int *divider);

shr_error_e soc_ramon_port_fault_get(
    int unit,
    soc_port_t port,
    uint32 *flags);

shr_error_e soc_ramon_port_bucket_fill_rate_validate(
    int unit,
    uint32 bucket_fill_rate);

shr_error_e soc_ramon_port_enable_set(
    int unit,
    soc_port_t port,
    int enable);

shr_error_e soc_ramon_port_stat_gtimer_config_set(
    int unit,
    soc_port_t port,
    soc_dnxc_port_stat_gtimer_config_t * gtimer_config);

shr_error_e soc_ramon_port_stat_gtimer_config_get(
    int unit,
    soc_port_t port,
    soc_dnxc_port_stat_gtimer_config_t * gtimer_config);

shr_error_e soc_ramon_port_link_up_check(
    int unit,
    soc_pbmp_t port_bmp);

shr_error_e soc_ramon_port_enabled_fmac_blocks_get(
    int unit,
    soc_pbmp_t * active_fmacs_bmp);
