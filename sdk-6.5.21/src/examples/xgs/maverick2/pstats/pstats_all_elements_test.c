...
/*  Feature  : MV2 Packetized Statistic Test
 *
 ...
 *  Summary:
 *  ========
 * Maverick2 supports a feature packetized statistics (PSTATS) which allows software
 * to read out MMU buffer use counts over SBUS and send this data via packets into the
 * Maverick2 switch to any destination in the network in a timely manner.
 *
...
//////////////////////////////////////////////////////////////
/* MV2 COSQ related Parameters */
const int MV2_MAX_PORTS = 65;
const int MV2_NUM_CPU_Q = 48;
const int MV2_NUM_PORT_L0 = 10;
const int MV2_NUM_PORT_Q_UC = 10;
const int MV2_NUM_PORT_Q_MC = 10;

bcm_gport_t cpu_cosq_gp_l0[MV2_NUM_PORT_L0];
bcm_gport_t cpu_cosq_gp_mc[MV2_NUM_CPU_Q];

bcm_gport_t cosq_gp_l0[MV2_MAX_PORTS][MV2_NUM_PORT_L0];
bcm_gport_t cosq_gp_uc[MV2_MAX_PORTS][MV2_NUM_PORT_Q_UC];
bcm_gport_t cosq_gp_mc[MV2_MAX_PORTS][MV2_NUM_PORT_Q_MC];
...
int gport_callback(int unit, bcm_gport_t port, int numq, uint32 flags,
    ...
    else
    {
        ...
        else if (flags & BCM_COSQ_GPORT_UCAST_QUEUE_GROUP)  // 0x08
        {
            ...
            index = (index + 1) % MV2_NUM_PORT_Q_UC;
        }
        else if (flags & BCM_COSQ_GPORT_MCAST_QUEUE_GROUP)  // 0x20
        {
            ...
            index = (index + 1) % MV2_NUM_PORT_Q_MC;
        }
        else
        {
            printf("ERR: INVALID FLAG for port 0x%x(localport %d)\n",
...
int mv2_dump_gports (int port)
{
    int start = 0, end = 0, idx, g_idx, num_q = 0, n_idx, q_idx;

    if (port < -1) {
        printf ("Err. Input valid port number or -1 for all (%d) ports", MV2_MAX_PORTS);
    }

    if ((port == -1) || (port == 0)) {
    printf("\nPrinting Gport Hierarchy for CPU Port %d\n", port);
    for (n_idx = 0; n_idx < MV2_NUM_PORT_L0; n_idx++) {
            printf ("    L0.%d: 0x%x\n", n_idx, cpu_cosq_gp_l0[n_idx]);
            if (n_idx == 0) {
                for (q_idx = 0; q_idx < MV2_NUM_CPU_Q; q_idx++) {
