...
/*
 * Feature  : Vxlan Bridging , IPv4 access to network tunnel termination flow
 *
 * Usage    : BCM.0> cint ipv4_NtoA_acc_encap_ovid_ivid.c
 *
 * config   : config_td3_flex.bcm
 *
 * Log file : ipv4_NtoA_acc_encap_ovid_ivid_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
Output (access):
=======
data[0000]: {00000000bbbb} {00000000aaaa} 8100 0064
data[0010]: 8100 00c8 0800 4500 0050 0001 0000 4011
data[0020]: 7497 0202 0202 0101 0101 0035 0035 003c
data[0030]: 32f6 7465 7374 7465 7374 7465 7374 7465
data[0040]: 7374 7465 7374 7465 7374 7465 7374 7465
data[0050]: 7374 7465 7374 7465 7374 7465 7374 7465
data[0060]: 7374 7465 7374 3799 7afd
...
verify(int unit)
{
    ...
    snprintf(str, 1184, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port)
{
    ...
    /*Add OVID/IVID based on access VP*/
    ...
    print bcm_switch_tpid_delete(unit, &tpid_info);

    tpid_info.tpid_value = 0x9100;
    print bcm_switch_tpid_add(unit, 0, &tpid_info);

    bcm_vlan_action_set_t action;
    action.dt_outer = bcmVlanActionReplace;
    action.new_outer_vlan = 100;
    action.dt_inner = bcmVlanActionReplace;
    action.new_inner_vlan = 200;
    rv = bcm_vlan_translate_egress_action_add(unit, *acc_flow_port, vxlan_acc->acc_vlan, 4, &action);
    if (BCM_FAILURE(rv)) {
...
config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, bcm_flow_handle_t flow_handle)
{
    ...
    bcm_port_t                  phy_port;
    int flags   =   BCM_FLOW_PORT_NETWORK;
