...
/*
 * Feature  : Vxlan Bridging , IPv4 access to network tunnel initiation flow
 *            with network port encap criteria based on VFI+DVP
 *
 * Usage    : BCM.0> cint ipv4_AtoN_net_encap_VFI_DVP.c
 *
 * config   : config_fb6_flex.bcm
 *
 * Log file : ipv4_AtoN_net_encap_VFI_DVP_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
Ingress Hex Packet:
tx port=1 data=0x00000000CCCC00000000BBBB810000150800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374
...
Packet Dump
Packet[4]: data[0000]: {000000000002} {000000002223} 8100 0017
Packet[4]: data[0010]: 0800 4500 0082 0000 0000 3f11 a5b6 0a0a
Packet[4]: data[0020]: 0a02 c0a8 0101 ffff 2118 006e 0000 0000
Packet[4]: data[0030]: 0000 0535 3500 0000 0000 cccc 0000 0000
Packet[4]: data[0040]: bbbb 0800 4500 0050 0001 0000 4011 7497
Packet[4]: data[0050]: 0101 0101 0202 0202 0035 0035 003c 32f6
Packet[4]: data[0060]: 7465 7374 7465 7374 7465 7374 7465 7374
Packet[4]: data[0070]: 7465 7374 7465 7374 7465 7374 7465 7374
Packet[4]: data[0080]: 7465 7374 7465 7374 7465 7374 7465 7374
Packet[4]: data[0090]: 7465 7374 38ba 7e94
 */
 ...
 *   In Packet 2:
 *   ------------
 *   00000000 CCCC0000 0000BBBB 81000015
 *   08004500 00500001 00004011 74970101
 *   01010202 02020035 0035003C 32F67465
 ...
 *   Out Packet:
 *   -----------
 *   00000000 00020000 00002223 81000017
 *   08004501 00820000 00003F11 A5B50A0A
 *   0A02C0A8 0101FFFF 2118006E 00000000
 *   00000535 35000000 0000CCCC 00000000
 *   BBBB0800 45000050 00010000 40117497
 *   01010101 02020202 00350035 003C32F6
 *   74657374 74657374 74657374 74657374
 *   74657374 74657374 74657374 74657374
...
verify(int unit)
{
    ...
    printf("\nSending known unicast packet to ingress_port access side:%d\n", ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000CCCC00000000BBBB810000150800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port)
{
    ...
    rv = vxlan_create_egr_obj(unit, 0, acc_intf_id, vxlan_acc->acc_remote_mac,
                              vxlan_acc->phy_Gport, vxlan_acc->acc_vlan, &acc_egr_id);
    if (BCM_FAILURE(rv)) {
...
int
config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, uint32 same_vnid, bcm_flow_handle_t flow_handle)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_vlan_control_port_set(unit, *net_flow_port,
                bcmVlanPortTranslateEgressKey, bcmVlanTranslateEgressKeyVpnGport));
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    /*create net_flow_port_1 with vnid 0x53535*/
    printf("\nSetup Network VP on port %d\n", egress_port_2);
    rv = config_vxlan_network(unit, &vxlan_cfg, &vxlan_net_2, &net_flow_port_2, 0, flow_handle);
    if (BCM_FAILURE(rv)) {
        printf("\nError in network config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = add_to_l2_table(unit, "00:00:00:00:aa:aa", vxlan_cfg.vxlan_vpn, net_flow_port_1);
    if (BCM_FAILURE(rv)) {
    ...
    printf("Access-to-Network: Add payload-dest-mac Address => Network-flow-port (VP).\n");
    rv = add_to_l2_table(unit, "00:00:00:00:cc:cc", vxlan_cfg.vxlan_vpn, net_flow_port_2);
    if (BCM_FAILURE(rv)) {
        printf("\nError in l2 entry config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
