...
/*
 * Feature  : Vxlan Bridging,IPv4 access to network tunnel initiation flow on a shared network vp
 *
 * Usage    : BCM.0> cint ipv4_AtoN_net_vp_sharing.c
 *
 * config   : config_fb6_flex.bcm
 *
 * Log file : ipv4_AtoN_net_vp_sharing_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 *                                  |                      |   VLAN-22        Underlay-NextHop
 *   SMAC 00::bb:bb                 |                      |  egress_port       +---+
 *   DMAC 00::aa:aa                 |                      +--------------------+   |
...
ingress_port_2:
tx port=2 data=0x00000000CCCC00000000BBBB810000170800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374
...
Packet Dump:
egress packet 1:
Packet[2]: data[0000]: {000000000002} {000000002222} 8100 0016
Packet[2]: data[0010]: 0800 4500 0082 0000 0000 3f11 a5b7 0a0a
Packet[2]: data[0020]: 0a01 c0a8 0101 ffff 2118 006e 0000 0000
Packet[2]: data[0030]: 0000 0123 4500 0000 0000 aaaa 0000 0000
Packet[2]: data[0040]: bbbb 0800 4500 0050 0001 0000 4011 7497
Packet[2]: data[0050]: 0101 0101 0202 0202 0035 0035 003c 32f6
...
Packet[2]: data[0090]: 7465 7374 65ce 70b0
egress packet 2:
Packet[4]: data[0000]: {000000000002} {000000002222} 8100 0016
Packet[4]: data[0010]: 0800 4500 0082 0001 0000 3f11 a5b6 0a0a
Packet[4]: data[0020]: 0a01 c0a8 0101 ffff 2118 006e 0000 0000
...
Packet[4]: data[0090]: 7465 7374 de45 727c
...
verify(int unit)
{
    ...
    printf("\nSending known unicast packet to ingress_port access side:%d\n", ingress_port_2);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000CCCC00000000BBBB810000170800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ingress_port_2);
    bshell(unit, str);
    bshell(unit, "show c");
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    rv = add_to_l2_table(unit, "00:00:00:00:aa:aa", vxlan_cfg_1.vxlan_vpn, net_flow_port);
    if (BCM_FAILURE(rv)) {
    ...
    printf("Access-to-Network: Add payload-dest-mac Address => Network-flow-port (VP).\n");
    rv = add_to_l2_table(unit, "00:00:00:00:cc:cc", vxlan_cfg_2.vxlan_vpn, net_flow_port);
    if (BCM_FAILURE(rv)) {
        printf("\nError in l2 entry config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
