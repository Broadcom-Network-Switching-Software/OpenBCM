...
/*  Feature  : L3 Ipv6 to Ipv4 Auto Tunnel
 *
 *  Usage    : BCM.0> cint l3_tunnel_ip6to4.c
 *
 *  config   : ip_tunnel_config.bcm
 *
 *  Log file : l3_tunnel_ip6to4_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+
 ...
 *  Summary:
 *  ========
 *    The 6to4 automatic tunneling encapsulates incoming IPv6 packets with an IPv4 tunnel header and
 ...
 *
 *    2) Step2 - Configuration (Done in config_l3_tunnel_ip6to4())
 *    ============================================================
 ...
 */
bcm_error_t config_l3_tunnel_ip6to4(int unit)
{
  bcm_error_t   rv;
  ...
  bcm_ip6_t tnl_init_dip6 = {0x20, 0x02, 0xc0, 0xa8, 0x01, 0x01, 0x00, 0x00,
                             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};/*2002:192:168:1:1*/
  bcm_ip6_t tnl_term_dip6 = {0x20, 0x02, 0xc0, 0xa8, 0x02, 0x01, 0x00, 0x00,
                             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};/*2002:192:168:2:1*/
...
void verify(int unit)
{
 ...
 /*      For IP6TO4 Tunnel initiation, send the below IPv6 packet on access_port.
 *
 *       DA 0x1111
 *       SA 0x1
 *       VLAN 21
 ...
 */
  ...
  snprintf(str, 512, "tx 1 pbm=%d data=0x0000000011110000000000018100001586DD6030000000063BFF2002C0A80201000000000000000000002002C0A8010100000000000000000000000000000000D7388CF0; sleep quiet 1", access_port);
  bshell(unit, str);
  bshell(unit, "sleep 2");
  bshell(unit, "show c");
  bshell(unit, "sleep 1");
 ...
 *       0000 0000 4b3e 8e53 0000 0000
  */
  ...
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000222200000000000281000016080045000042000000004029F640C0A80101C0A802016030000000063BFF2002C0A80101000000000000000000002002C0A80201000000000000000000000000000000004B3E8E53; sleep quiet 1", network_port);
  bshell(unit, str);
  bshell(unit, "sleep 2");
  bshell(unit, "show c");
  bshell(unit, "sleep 1");
 ...
 * a)test setup
 * b)actual configuration (Done in config_l3_tunnel_ip6to4())
...
bcm_error_t execute()
{
  ...
  if (BCM_FAILURE((rv = config_l3_tunnel_ip6to4(unit)))) {
    printf("config_l3_tunnel_ip6to4() failed.\n");
    return -1;
  }
  printf("Completed configuration(i.e executing config_l3_tunnel_ip6to4()) successfully.\n");
