...
const int BSCAN_MAX_INFO_ELEMENTS = 45;
const int BSCAN_NUM_EXPORT_ELEMENTS = 45;
...
bcm_error_t test_setup(int unit)
{
	...
	if (BCM_E_NONE != egress_port_setup(unit, egress_port)) {
        ...
        /* create vlan 10 */
        snprintf(cmd_str, 512, "vlan create 10 pbm=%d,%d", ingress_port,egress_port);
        bshell(0, cmd_str);

        snprintf(cmd_str, 512, "port %d discard=all", egress_port);
        bshell(0, cmd_str);
...
verify(int unit, bcm_flowtracker_group_t flow_group_id)
{
	...
	snprintf(str, 512, "tx 1 pbm=%d 
			data=0x1011121314122021222324228100000A86DD6500000100420614000000010002000300040000000000020000000200030004000500060007000255667788000000004006E23B90020002A0A8780210002000000000010000000050122233D9EA6600000102030405060708090A0B0C0D0E0F10111213141516000103;sleep 1", ingress_port);
	bshell(unit, str);
                        ...
                        int ipprot, int vlan)
{
	...
	BCM_IF_ERROR_RETURN(bcm_field_qualify_IpType(unit, presel_eid, bcmFieldIpTypeIpv6NoExtHdr));
	...
	/* FTFP Group for IPV6 packets Identify
	 * - Unicast L2 and Unicast IPV6
	 * - Multicast L2 and Unicast IPV6
	 * - Unicast L2 and Multicast IPV6
	...
	*ftfp_group = ftfp_group_config.group;
	...
	/* Create an entry in Flow tracker FP gorup */
        ...
        BCM_IF_ERROR_RETURN(bcm_field_qualify_Ip6PktNextHeader(unit, ftfp_entry, ipprot, 0xff));
        BCM_IF_ERROR_RETURN(bcm_field_qualify_InPort(unit, ftfp_entry, ingress_port, 0xff));
        BCM_IF_ERROR_RETURN(bcm_field_qualify_OuterVlanId(unit, ftfp_entry, vlan, 0xfff));
...
fte_export_element_map_s  fte_export_element_maps[BSCAN_NUM_EXPORT_ELEMENTS] =  {
    ...
    { bcmFlowtrackerTrackingParamTypeEgrPort, bcmFlowtrackerExportElementTypeEgrPort, 2},
    { bcmFlowtrackerTrackingParamTypeMMUQueueId, bcmFlowtrackerExportElementTypeMMUQueueId, 2}, 
};
...
int execute()
{
	...
	int num_tracking_list = 9; /* Number of tracking params list */
	int tracking_param_tuple[5] =  {bcmFlowtrackerTrackingParamTypeSrcIPv6,
	...
	tracking_param_list[i].param = bcmFlowtrackerTrackingParamTypeMMUQueueId;
	i++;

        /* add eggress port */
	tracking_param_list[i].flags = 0;
	tracking_param_list[i].param = bcmFlowtrackerTrackingParamTypeEgrPort;
	i++;
	...
	/* Creating Flow Selection  based on ingress packet*/
        int ipprot = 6;
        int vlan_id = 10; /* untagged */
        rv = broadscan_flow_selection(unit, &ftfp_group, flow_group_id, ipprot, vlan_id);
	if (BCM_FAILURE(rv)) {
	...
	rv = broadscan_set_trigger(unit, flow_group_id, bcmFlowtrackerExportTriggerTimer);
	if (BCM_FAILURE(rv)) {
