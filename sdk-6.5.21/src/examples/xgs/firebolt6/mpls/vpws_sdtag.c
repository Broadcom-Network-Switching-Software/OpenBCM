...
/*
 * Feature  : VPWS with SD-TAG scenario
 *
 * Usage    : BCM.0> cint vpws_sdtag.c
 *
 * config   : mpls_config.bcm
 *
 * Log file : vpws_sdtag_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of VPWS with SD-TAG functional
 ...
 *
 *   2) Step2 - Configuration (Done in config_vpws_sdtag()):
 *   =========================================================
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000445566000000AABBCC8100600B000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F202122232425262728292A2B2C2D2E2F303132337B9BCEDD; sleep quiet 1", access_port);
    bshell(unit, str);
    bshell(unit, "show c");
    bshell(unit, "sleep 1");
    bshell(unit, "clear c");
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000BB0000000000AA8100601688470022204000333140000000AABBCC00000044556691006001000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F202122232425262728292A2B2C2D2E2F3031323300539BAB; sleep quiet 1", network_port);
    bshell(unit, str);
    bshell(unit, "show c");
    bshell(unit, "sleep 1");
    bshell(unit, "clear c");
...
config_vpws_sdtag(int unit)
{
    ...
    mpls_c_port.egress_service_vlan = customer_vid;
    mpls_c_port.service_tpid = 0x8100;
    mpls_c_port.flags = (BCM_MPLS_PORT_SERVICE_TAGGED |
    ...
    /* Add provider port to VPN */
    bcm_mpls_port_t_init (&mpls_p_port);
    mpls_p_port.flags = (BCM_MPLS_PORT_EGRESS_TUNNEL |
    ...
    mpls_p_port.egress_label.ttl   = 16;
    mpls_p_port.egress_service_vlan = 1;
    mpls_p_port.service_tpid = 0x9100;
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_vpws_sdtag())
...
execute(void)
{
    ...
    print "~~~ #2) config_vpws_sdtag(): ** start **";
    if (BCM_FAILURE((rv = config_vpws_sdtag(unit)))) {
        printf("config_vpws_sdtag() failed.\n");
        return -1;
    }
    print "~~~ #2) config_vpws_sdtag(): ** end **";
