...
/*
 * Feature  : H-VPLS with QinQ at access-layer
 *            i.e Spoke & Hub QinQ/Double tagged scenario
 *
 * Usage    : BCM.0> cint vpls_hierarchy_qiq.c
 *
 * config   : mpls_config.bcm
 *
 * Log file : vpls_hierarchy_qiq_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of H-VPLS tunnel initiation
 ...
 *
 *   2) Step2 - Configuration (Done in config_vpls_hierarchy_qiq()):
 *   ===============================================================
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000445566000000AABBCC9100000B8100000C08004500002A0000000040FF79D60000000000000000000102030405060708090A0B0C0D0E0F101112131415; sleep quiet 1", access_port);
    bshell(unit, str);
    bshell(unit, "show c");
    bshell(unit, "sleep 1");
    bshell(unit, "clear c");
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000BB0000000000AA8100001688470022204000333140000000AABBCC0000004455669100000B8100000B08004500002E0000000040FF79D20000000000000000910000038902A0010446000000000000D36D6169643438627974; sleep quiet 1", network_port);
    bshell(unit, str);
    bshell(unit, "show c");
    bshell(unit, "sleep 1");
    bshell(unit, "clear c");
...
bcm_error_t
config_vpls_hierarchy_qiq(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_vlan_control_port_set(unit, customer_port,
            bcmVlanPortTranslateKeyFirst, bcmVlanTranslateKeyPortDouble));
    ...
    mpls_c_port.criteria = BCM_MPLS_PORT_MATCH_PORT_VLAN_STACKED; /* BCM_MPLS_PORT_MATCH_PORT_VLAN */
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_vpls_hierarchy_qiq())
...
execute(void)
{
    ...
    print "~~~ #2) config_vpls_hierarchy_qiq(): ** start **";
    if (BCM_FAILURE((rv = config_vpls_hierarchy_qiq(unit)))) {
        printf("config_vpls_hierarchy_qiq() failed.\n");
        return -1;
    }
    print "~~~ #2) config_vpls_hierarchy_qiq(): ** end **";
