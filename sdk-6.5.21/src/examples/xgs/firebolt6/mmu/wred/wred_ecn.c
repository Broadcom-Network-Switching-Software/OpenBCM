 ...
 * VLAN=2,Pri=4  xe0(1)|            |xe1(2)   VLAN=2,Pri=4
 *                     | l2:mac=0x1 |
 ...
 *    ==================================
 *      a) Transmit ICMP packets with ECN=01 through Ixia connected to xe0
 *      b) Transmit IGMP packets with ECN=01 through Ixia connected to xe0
 *      c) Expected Result:
 *         ================
 *         For ICMP packets - In case of congestion we should see ECN marking 3 (ECN bits=11)
 *                            for packets egressing xe1. If no congestion, we should see
 *                            ECN marking 2 (ECN bits=10)
 *         FOR IGMP packets - In case of congestion, WRED drops should be seen on xe1.
 *                            This can be verified by WRED_PKT_GRE.xe1' in 'show c' output.
 *                            If no congestion, No wred drops and no ECN marking should be seen.
 */
...
bcm_error_t mmu_wred_ecn_setup(int unit) 
{
 ...
 discard_cfg.min_thresh = 2*256; /*Queue depth in bytes to begin dropping.(256 bytes = 1 cell in FB6)*/
 discard_cfg.max_thresh = 3*256; /*Queue depth in bytes to drop all packets(256 bytes = 1 cell in FB6)*/
...
bcm_error_t bcm_egr_int_cn_update(int unit)
{
 ...
 config.color        = bcmColorGreen;
 config.action_flags = BCM_ECN_TRAFFIC_ACTION_DEQUEUE_CONGESTION_INT_CN_UPDATE|BCM_ECN_TRAFFIC_ACTION_DEQUEUE_NON_CONGESTION_INT_CN_UPDATE;
 config.congested_int_cn     = nonresp_mapped_int_cn; /* no change, same as int_cn assigned at ingress - 0*/  
 config.non_congested_int_cn = nonresp_mapped_int_cn; /* no change, same as int_cn assigned at ingress - 0 */ 
 rv = bcm_ecn_traffic_action_config_set(unit, &config);
 if (rv != BCM_E_NONE) {
...
{
  /* Test 1: ICMP pkt (responsive) - check ECN marking in packet egressing on egress port.
   * If congestion ECN bits = 11
   * If no congestion ECN bits = 10
   *
   * Test 2: IGMP packet (non responsive)
   * If congestion - WRED drops seen in 'show c' output for egress port 
 ...
 if(check){
  ...
  printf("Sending ICMP(ip protocol =1) packet with ECN=1\n"); 
  snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000010000000000028100800208004501002E0000000040013E93141414140A0A0A0A0000B7AE00000000000102030405060708090A0B0C0D0E0F10113100DF21;sleep 1", ing_port);
  bshell(unit, str);
  printf("Executing 'show c'\n");
  bshell(unit, "show c");

  printf("Sending IGMP (ip protocol=2) packet with ECN=1\n"); 
  snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000010000000000028100800208004501001C0000000040023EA4141414140A0A0A0A1164EE9B00000000000102030405060708090A0B0C0D0E0F10116984522C;sleep 1", ing_port);
  bshell(unit, str);
  printf("Executing 'show c'\n");
  bshell(unit, "show c");
