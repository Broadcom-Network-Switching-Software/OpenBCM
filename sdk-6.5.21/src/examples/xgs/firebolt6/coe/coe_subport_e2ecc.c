...
/*
 * Feature  : COE subport - Channelized E2ECC  
 *
 * Usage    : BCM.0> cint coe_subport_e2ecc.c 
 *
 * config   : coe_flow_control_config.bcm
 *
 * Log file : coe_subport_e2ecc_log.txt
 *
 * Test Topology: |FB6(56470)|xe20<--->xe8|FL(56070)|xe0<---->xe0|Traffic Generator(TG)|
 *
 *  Summary:
 *  ========
 *  This cint example demonstrates channelized E2ECC feature
 *
 ...
 *      e) Configure Channelized e2ecc on created subport
 *    3) Step3 - Verification (Done in verify())
 *    ==========================================
 *      a) Send C-e2ecc packet from TG connected to FL
 ...
 *      d) Expected Result:
 *         ================
 *         'show c' should not show any egress on xe20. L1 node corresponding to COS should be paused 
 *          and packet egress should stop on COE port xe20 on FB6.
 *
 *      Channelized e2ecc packet format that is sent from TG to xe0 on FL:
 *      0180c200000100000100aa01880800000000000000000000000000000000000a000003030303030303030000000000000000000000000000000000000000000000000000
 *      Packet egressing xe8, FL-->FB6 with SVTAG
 *      0180c200000100000100aa01ff0008ff880800000000000000000000000000000000000a000003030303030303030000000000000000000000000000000000000000000000000000     
 */
...
/* Channelized e2ecc configuration on subport */
bcm_error_t c_e2ecc_config(int unit, bcm_gport_t subport_gport, int enable)
{
  int rv = 0;
  ...
  {
      config.status = 1;
      rv = bcm_cosq_subport_congestion_config_set(unit, subport_gport, bcmCosqSubportCongestionTypeE2ecc, &config);
      if (rv != BCM_E_NONE) {
          printf("bcm_cosq_subport_pfc_class_mapping_set returned '%s'\n", bcm_errmsg(rv));
          return rv;
      }

      rv = bcm_cosq_subport_congestion_config_get(unit, subport_gport, bcmCosqSubportCongestionTypeE2ecc, &get_config);
      if (rv != BCM_E_NONE) {
          printf("bcm_cosq_subport_pfc_class_mapping_get returned '%s'\n", bcm_errmsg(rv));
          return rv;
      }
      printf("Status = %x \n", get_config.status);
  } else {
...
void verify(int unit)
{
  ...
  bshell(unit, "show c");
  printf("Now start sending channelized-e2ecc frame from TG connected to FL\n");
  bshell(unit, "clear c");
  snprintf(str, 512, "tx 1000 pbm=%d data=0x00000000AAAA00000000BBBB8100001508004500002E0000000040FFAFCB0A000001C0000005000102030405060708090A0B0C0D0E0F10111213141516171819; sleep 1", ing_port);
  bshell(unit, str);
  printf("Executing 'show c'\n");
...
bcm_error_t execute()
{
  ...
  /* Enable Channelized e2ecc on subport */
  if (BCM_FAILURE((rv = c_e2ecc_config(unit, subport_gport,1)))) {
      printf("e2ecc_config() failed.: %s.\n", bcm_errmsg(rv));
      return rv;
  }
