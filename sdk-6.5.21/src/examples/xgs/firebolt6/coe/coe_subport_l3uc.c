...
/*
 * Feature  : COE subport - L3 UC Flow 
 *
 * Usage    : BCM.0> cint coe_subport_l3uc.c
 *
 * config   : coe_config.bcm
 *
 * Log file : coe_subport_l3uc_log.txt
 *
 ...
 *      b) Send L3 UC packet on ingress port as in verify() function
 *      c) Check if packet egressing out of egress port has 4 byte subport tag following SA MAC.
 *      c) Expected Result:
 *         ================
 *         L3 UC packet egressing out of egress channelized port should have 4 bytes Subport tag after SA MAC 
 */
...
bcm_error_t configure_nexthop(int unit, int dest_port, int vid, char* smac, char* dmac, bcm_if_t *nh_index, int flags)
{
  bcm_l3_intf_t intf;
...
void verify(int unit)
{
  ...
  bshell(unit, "clear c");
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA00000000BBBB8100001508004500002E0000000040FFAFCB0A000001C0000005000102030405060708090A0B0C0D0E0F10111213141516171819; sleep quiet 1", ing_port);
  bshell(unit, str);
  bshell(unit, "l3 defip show");
  bshell(unit, "show c");
...
/*  bshell(0, "hsp pbm=xe6 subport=1");
  bshell(0, "port xe0 lb=mac");
  bshell(0, "port xe6 lb=mac");
  bshell(0, "port xe6 discard=all");
  bshell(0, "tx 1 pbm=xe0 data=00000000AAAA00000000BBBB8100001508004500002E0000000040FFAFCB0A000001C0000005000102030405060708090A0B0C0D0E0F10111213141516171819");
  bshell(0, "sleep 5");
  bshell(0, "show c");
*/
}
