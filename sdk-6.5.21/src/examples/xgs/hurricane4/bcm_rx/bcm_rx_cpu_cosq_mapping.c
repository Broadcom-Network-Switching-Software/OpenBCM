...
/*
 * Hurricane4 Rx Queue Channel Configuration
 *
 * The mechanism for configuring Rx Queue channels on Hurricane4 
 * (BCM56275) differs from previous devices. Previous devices used
 * bcm_tx_queue_channel_set(). This API does not apply to Hurricane4. For
 * Hurricane4 devices, the user needs to use a combination of cosq attach/detach
 * operations on L0 gports. On Hurricane4, the L0 nodes are directly wired
 ...
 * 
 * bcm_error_t
 * hr4_rx_queue_channel_set(int unit,
 ...
 * Rx 0 => L0.0 => CMIC 0 (HOST CPU PCI)/Channel 1 => PCI Rx Channel 0
 * Rx 1 => L0.1 => CMIC 0 (HOST CPU PCI)/Channel 2 => PCI Rx Channel 1
 * Rx 2 => L0.2 => CMIC 0 (HOST CPU PCI)/Channel 3 => PCI Rx Channel 2
 * Rx 3 => L0.3 => CMIC 0 (HOST CPU PCI)/Channel 4 => PCI Rx Channel 3
 * Rx 4 => L0.4 => CMIC 0 (HOST CPU PCI)/Channel 5 => PCI Rx Channel 4 
 * Rx 5 => L0.5 => CMIC 0 (HOST CPU PCI)/Channel 6 => PCI Rx Channel 5
 * Rx 6 => L0.6 => CMIC 0 (HOST CPU PCI)/Channel 7 => PCI Rx Channel 6
 *
                  ...
                  bcm_gport_t gport, void *user_data)
{
    ...
    if (flags == BCM_COSQ_GPORT_SCHEDULER) {
        if (sched_cosq_idx >= 12) {
            printf("\nError in traverse_callback() with out-of-scope 'sched_cosq_idx'.\n");
            return BCM_E_INTERNAL;
        }
        int    physical_port, num_cos_levels;
        uint32 port_flags;
        rv = bcm_cosq_gport_get(unit, gport, &physical_port, &num_cos_levels, &port_flags);
        if (BCM_FAILURE(rv)) {
            printf("\nError in bcm_cosq_gport_get(): %s.\n",bcm_errmsg(rv));
            return rv;
        }
        if (num_cos_levels<48)
            return BCM_E_NONE;
 ...
 */
bcm_error_t
get_hr4_cpu_queues(int unit, cpu_queues_t * cpu_queues)
{
    int cos_idx;
    int input_idx;
    int rv = BCM_E_NONE;
 ...
 * used in place of bcm_rx_queue_channel_set() for Hurricane4 device.
 */
bcm_error_t
hr4_rx_queue_channel_set(int unit,
                              cpu_queues_t * cpu_queues,
...
int rx_cpu_cosq_setup(int unit)
{
     ...
     */
    rv = get_hr4_cpu_queues(unit, &cpu_cosq_map_db);
    if(BCM_FAILURE(rv)) {
        printf("\nError in get_hr4_cpu_queues(): %s.\n",bcm_errmsg(rv));
        return rv;
    }
 ...
 *
 * Demonstrates how to map Rx PCI channels for Hurricane4.
...
test_harness(int unit)
{
    ...
    */ 
    for (cos = 0; cos < max_rx_cos; cos++) {
        ...
        rv = hr4_rx_queue_channel_set(unit, &cpu_cosq_map_db, cos, rx_queue_channel);
        if(BCM_FAILURE(rv)) {
