...
bcm_error_t verify(int unit)
{
    ...
    /* send 6 packets */
    sprintf(command, "tx 2 pbm=%d CosQueue=0 SourceMac=00:00:00:00:10:10 DestMac=00:00:00:00:00:10 length=128", egress_port);
    bshell(unit, command);
    
    sprintf(command, "tx 2 pbm=%d CosQueue=1 SourceMac=00:00:00:00:10:10 DestMac=00:00:00:00:00:10 length=128", egress_port);
    bshell(unit, command);
    
    sprintf(command, "tx 2 pbm=%d Cosqueue=2 SourceMac=00:00:00:00:10:10 DestMac=00:00:00:00:00:10 length=128", egress_port);
    bshell(unit, command);
