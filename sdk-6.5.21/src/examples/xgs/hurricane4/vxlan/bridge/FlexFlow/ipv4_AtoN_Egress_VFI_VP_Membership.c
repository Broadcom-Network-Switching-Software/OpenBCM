...
/*
 * Feature  : HR4 Vxlan Bridging , IPv4 access to network tunnel initiation Flow, Verifying DVP membership check based on VFI (VFI,VP).
 *
 * Usage    : BCM.0> cint ipv4_AtoN_Egress_VFI_VP_Membership.c
 *
 * config   : BCM56277_A1-PORT-FLEXFLOW.bcm
 *
 * Log file : ipv4_AtoN_Egress_VFI_VP_Membership_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
	...
	printf("\n\n Enabling membership check on DVP and we will see packet get dropped on egress port \n");
	if (BCM_E_NONE != bcm_port_vlan_member_set(unit, net_vp, BCM_PORT_VLAN_MEMBER_EGRESS) ) {
	...
	printf("\n\n Now Adding DVP for VFI based membership check and packet get egress on network port\n");
        if (BCM_E_NONE != bcm_vlan_gport_add(unit, vpn, net_vp, BCM_VLAN_GPORT_ADD_EGRESS_ONLY) ) {
	    printf("bcm_vlan_gport_add() failed \n");
	    return -1;
	}
	bshell(unit, str);
	bshell(unit, "show c");
