...
/*
 * Feature  : riot multicast bud node
 *
 * Usage    : BCM.0> cint riot_decap_mc_bud.c
 *
 * config   : BCM56275_A1-RIOT-LEGACY.bcm
 *
 * Log file : riot_decap_mc_bud_log.txt
 *
 * Test Topology :
 *
                                 +-----------------+
 * access port 1(L3) 2nd pass <--| 		           |<-- Network port 1, VxLAN packet in for 1st pass
 *                               |       switch    |
 * access port 2(L2) end pass <--|                 |--> Network port 2, BUD IPMC out, and loopback for 2nd pass
 *                               +-----------------+
 *
 * Summary:
 * ========
 *   This Cint example to show configuration of riot mc bud mode
 *
 *
 ...
 *   1) Step1 - Test Setup ():
 *   =============================================
 *      a) Select 2 ports as 2 access_ports, another 2 ports as 2 network ports
 *
 *      b)
 *
 ...
 *   ========================================================================
 *     a)
 *     b) run riot_decap_mc_bud.c, to create riot VPN.
 ...
 *   ===========================================
         input pkt to network port 1
 *       tx 1 PortBitMap=(network port 1) data=01005e00010a0000000003018100000b08004500006400000000401186740a0a0a01e000000affff211800000000080000000123450001005e0010010000000002028100000c080045000032000000004011549b1e1e1e01e0000a0100000000000000000000000000000000000000000000000000000
 *
 *
 *
 *
 *       == Expected Result:
 *
 *       Packet out of network port 2  (BUD) 1st pass
 *
 ...
 *
 *       Packet out of access_port_1 2nd pass
 *
 ...
 *       21be 4d26
 *
 *       Packet out of access_port_2 2nd pass
 *
...
bcm_error_t test_setup(int unit)
{
  ...
  network_port_1 = port_list[2];
  network_port_2 = port_list[3];
...
bcm_error_t riot_decap_multicast_bud(int unit)
{

	/* ==== set var ==== */
	bcm_mac_t dummy_mac_addr    = "00:00:00:00:00:00";
	bcm_mac_t acc_ol_mac        = "00:00:00:00:01:01";
	bcm_mac_t acc_ol_nh_mac     = "00:00:00:00:01:02";
	bcm_mac_t net_ol_mac        = "00:00:00:00:02:01";
	bcm_mac_t net_ol_nh_mac     = "00:00:00:00:02:02";
	bcm_mac_t net_ul_hn_mac     = "00:00:00:00:03:01";
	bcm_mac_t net_ul_mac        = "00:00:00:00:03:02";
	bcm_mac_t net2_mac          = "00:00:00:00:04:01";
	bcm_mac_t net_ul_mc_mac     = "01:00:5e:00:01:0A";
	bcm_mac_t net_ol_mc_mac     = "01:00:5e:00:10:01";
	bcm_vlan_t acc_vid_1        = 10;
	bcm_vlan_t net_ul_vid       = 11;
	bcm_vlan_t net_ol_vid       = 12;
	bcm_vlan_t acc_vid_2        = 12;
	bcm_vlan_t net_vid_bud      = 13;
	int ttl                     = 0x20;
	bcm_vrf_t vrf               = 1;
	uint32 acc_vpn_vnid         = 0;
	uint32 net_vpn_vnid         = 0x12345;
	uint16 udp_src_port         = 0xFFFF;
	uint16 udp_dst_port         = 0x2118;
	bcm_ip_t tunnel_local_ip    = 0x14141401;  /* 20.20.20.1 */
	bcm_ip_t tunnel_remote_ip   = 0x0a0a0a01;  /* 10.10.10.1 */
	...
	BCM_IF_ERROR_RETURN(vxlan_network_port_config(unit, network_port_2));
	...
	BCM_IF_ERROR_RETURN(vlan_create_add_port(unit, net_vid_bud, network_port_2));
	...
	BCM_IF_ERROR_RETURN(vxlan_vpn_group_create(unit, &vpn_net, &mcast_net, net_vpn_vnid, flags, 0x8100, net_ol_vid));
	printf("vpn_net=0x%x\n", vpn_net);

	/*
	 * Create vxlan access vp
	 */
	...
	BCM_IF_ERROR_RETURN(l3_egress_create(unit, BCM_L3_IPMC, acc_intf_ol, acc_vp, net_ol_mc_mac, &acc_egr_obj_ol));
	...
	printf("bcm_l3_ingress_create() %s acc_ing_obj=0x%x\n", bcm_errmsg(rv), acc_ing_obj);

	printf("==== update VFI table vrf field ====\n");
	vlan_vfi_profile_update(unit, vpn_acc, vrf, acc_ing_obj);
	*/
	...
	BCM_IF_ERROR_RETURN(l3_egress_create(unit, BCM_L3_VXLAN_ONLY, net_intf_ul, network_port_1,
	                 net_ul_mc_mac, &net_egr_obj_ul));
	printf("l3_egress_create() net_egr_obj_ul=0x%x\n", net_egr_obj_ul);
	/*
	 * Create L3 ingress intf at Network UL for BUD IPMC routing
	 */
	net_ul_ing_obj = net_intf_ul;
	bcm_l3_ingress_t_init(&l3_ingress);
	l3_ingress.flags = BCM_L3_INGRESS_WITH_ID;
	l3_ingress.vrf = vrf;
	l3_ingress.ipmc_intf_id = net_intf_ul;
	BCM_IF_ERROR_RETURN(bcm_l3_ingress_create(unit, &l3_ingress, &net_ul_ing_obj));
	printf("bcm_l3_ingress_create() net_ul_ing_obj=0x%x\n",  net_ul_ing_obj);

    bcm_vlan_control_vlan_t_init(&vlan_info);
    BCM_IF_ERROR_RETURN(bcm_vlan_control_vlan_get(unit, net_ul_vid, &vlan_info));
    vlan_info.ingress_if = net_ul_ing_obj;
    BCM_IF_ERROR_RETURN(bcm_vlan_control_vlan_set(unit, net_ul_vid, vlan_info));




	printf("==== update VFI table vrf field ====\n");
	BCM_IF_ERROR_RETURN(vlan_vfi_profile_update(unit, net_ul_vid, vrf, net_ul_ing_obj));

	/* Network UC VP */
	/**********************************
	In _bcm_td2_vxlan_match_add(), there is:
	        if (vxlan_port->flags & BCM_VXLAN_PORT_MULTICAST) {
	              return BCM_E_NONE;
	        }
	this code means if BCM_VXLAN_PORT_MULTICAST is used, the tunnel_remote_ip
	will not be used to add MPLS_ENTRY::VXLAN_SIP__SIP, thus the MC SVP will not be derived.
	So we created the UC network VP here to get the MPLS_ENTRY added
 */

	printf("==== UC tunnel setup ====\n");
	...
	BCM_IF_ERROR_RETURN(vxlan_port_create(unit, vpn_net, -1 /* port */, flags,
	                  ...
	                  &net_vp));
	printf("vxlan_port_create() %s net_vp=0x%x\n", bcm_errmsg(rv), net_vp);

	BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, net_vp, bcmPortControlIP4Mcast, 1));

	/* Network MC VP */
	printf("==== MC tunnel setup ====\n");
	...
	bcm_gport_t net_vp_mc;
	flags = BCM_VXLAN_PORT_NETWORK | BCM_VXLAN_PORT_EGRESS_TUNNEL |
	        BCM_VXLAN_PORT_SERVICE_TAGGED | BCM_VXLAN_PORT_MULTICAST;
	BCM_IF_ERROR_RETURN(vxlan_port_create(unit, vpn_net, -1 /* port */, flags,
	                  net_egr_obj_ul,
	                  BCM_VXLAN_PORT_MATCH_VN_ID, BCM_VLAN_INVALID,
	                  tnl_mc_id, tnl_mc_id,
	                  net_ol_vid, 0x8100 /* egress_service_tpid */,
	                  &net_vp_mc));
	printf("vxlan_port_create() %s net_vp_mc=0x%x\n", bcm_errmsg(rv), net_vp_mc);

	BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, net_vp_mc, bcmPortControlIP4Mcast, 1));

	/*
	 * Routing interface (network side)
	 */
	printf("==== Create vxlan l3 egress - network OL ====\n");
	...
	bcm_if_t net_egr_obj_ol_mc;
	BCM_IF_ERROR_RETURN(l3_egress_create(unit, BCM_L3_IPMC, net_intf_ol, net_vp_mc, dummy_mac_addr, &net_egr_obj_ol_mc));
	printf("l3_egress_create() net_egr_obj_ol_mc=0x%x\n", net_egr_obj_ol_mc);

	/*
	 * Create L3 ingress intf at Network OL routing
	 */
	bcm_l3_ingress_t_init(&l3_ingress);
	l3_ingress.flags = BCM_L3_INGRESS_WITH_ID;
	l3_ingress.vrf = vrf;
	net_ol_ing_obj = net_intf_ol;
	BCM_IF_ERROR_RETURN(bcm_l3_ingress_create(unit, &l3_ingress, &net_ol_ing_obj));
	printf("bcm_l3_ingress_create() %s net_ol_ing_obj=0x%x\n", bcm_errmsg(rv), net_ol_ing_obj);


    bcm_vlan_control_vlan_t_init(&vlan_info);
    BCM_IF_ERROR_RETURN(bcm_vlan_control_vlan_get(unit, vpn_net, &vlan_info));
    vlan_info.ingress_if = net_ol_ing_obj;
    BCM_IF_ERROR_RETURN(bcm_vlan_control_vlan_set(unit, vpn_net, vlan_info));

	printf("==== update VFI table vrf field ====\n");
	BCM_IF_ERROR_RETURN(vlan_vfi_profile_update(unit, vpn_net, vrf, net_ol_ing_obj));

	/*
	 * OL IPMC - L3 route to accesss_port_1, L2 switch to access_port_2
	 */
	printf("Setup ipmc for OL\n");
	printf("===create a l3 mc group - OL===\n");
	BCM_IF_ERROR_RETURN(bcm_multicast_create(unit, BCM_MULTICAST_TYPE_L3, &ol_mc_group));

	printf("Get the L3 Encap for the interfaces\n");
	bcm_if_t encap_id;
	BCM_IF_ERROR_RETURN(bcm_multicast_l3_encap_get(unit, ol_mc_group, acc_vp, acc_egr_obj_ol, &encap_id));
	printf("bcm_multicast_l3_encap_get()  encap_id=0x%x\n", encap_id);
	BCM_IF_ERROR_RETURN(bcm_multicast_egress_add(unit, ol_mc_group, acc_vp, encap_id));


	BCM_IF_ERROR_RETURN(bcm_multicast_l2_encap_get(unit, ol_mc_group, access_gport_2, net_ol_vid, &encap_id));
	printf("bcm_multicast_l2_encap_get()  encap_id=0x%x\n", encap_id);
	...
	/*
	 * BUD mode IPMC, send out of network_port_2
	 */
	bcm_if_t net2_intf;
	BCM_IF_ERROR_RETURN(l3_intf_create(unit, net2_mac, net_vid_bud, &net2_intf));
	printf("l3_intf_create() net2_intf=0x%x\n", net2_intf);

	BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, network_gport_2, bcmPortControlIP4Mcast, 1));

	bcm_if_t net2_egr_obj;
	BCM_IF_ERROR_RETURN(l3_egress_create(unit, BCM_L3_IPMC, net2_intf, network_gport_2, net_ul_mc_mac, &net2_egr_obj));
	printf("l3_egress_create() net2_egr_obj=0x%x\n", net2_egr_obj);

	printf("===create a l3 mc group - UL===\n");
	BCM_IF_ERROR_RETURN(bcm_multicast_create(unit, BCM_MULTICAST_TYPE_L3, &ul_mc_group));

	printf("Get the L3 Encap for the interfaces\n");
	BCM_IF_ERROR_RETURN(bcm_multicast_egress_object_encap_get(unit, ul_mc_group, net2_egr_obj, &encap_id));
	printf("bcm_multicast_egress_object_encap_get()  encap_id=0x%x\n", encap_id);
	BCM_IF_ERROR_RETURN(bcm_multicast_egress_add(unit, ul_mc_group, network_gport_2, encap_id));

	bcm_ipmc_addr_t_init(&ipmc);
	ipmc.flags = 0;
	ipmc.mod_id = modid;
	ipmc.port_tgid = network_port_1;
	ipmc.vid = net_ul_vid;
	ipmc.vrf = vrf;
	ipmc.ing_intf = net_ul_ing_obj;
	ipmc.s_ip_addr = tunnel_remote_ip;
	ipmc.mc_ip_addr = tunnel_remote_mc;
	ipmc.group = ul_mc_group;
	print bcm_ipmc_add(unit, &ipmc);

	printf("Update mcast tunnel to BUD mode\n");
	BCM_IF_ERROR_RETURN(vxlan_tunnel_term_update(unit, tnl_mc_id, BCM_VXLAN_MULTICAST_TUNNEL_STATE_BUD_ENABLE));

	//print bcm_port_loopback_set(unit, network_port_2, BCM_PORT_LOOPBACK_MAC);
	//printf("riot decap mc bud SUCCESS \n");
 ...
 */
 void verify(int unit)
 {
	...
	snprintf(str, 512, "port %d,%d,%d,%d lb=mac", access_port_1, access_port_2,network_port_1,network_port_2);
	bshell(unit, str);

	snprintf(str, 512,"port  %d,%d,%d,%d discard=all ", access_port_1, access_port_2,network_port_1,network_port_2);
	bshell(unit, str);
	bshell(0, "sleep 1");

	snprintf(str, 512,"dmirror %d Mode=Ingress DestPort=cpu0", network_port_2);
	bshell(unit, str);
	snprintf(str, 512,"dmirror %d Mode=Ingress DestPort=cpu0", access_port_1);
	bshell(unit, str);
	snprintf(str, 512,"dmirror %d Mode=Ingress DestPort=cpu0", access_port_2);
	bshell(unit, str);
	bshell(0, "sleep 1");

	snprintf(str, 512, "port %d,%d  discard=none ", network_port_1,network_port_2);
	bshell(unit, str);

	snprintf(str, 512,"tx 1 pbm=%d data=0x01005e00010a0000000003018100000b08004500006400000000401186740a0a0a01e000000affff211800000000080000000123450001005e0010010000000002028100000c080045000032000000004011549b1e1e1e01e0000a0100000000000000000000000000000000000000000000000000000", network_port_1);
	bshell(unit, str);
	bshell(0, "sleep 1");
	bshell(0, "l2 show ");
	bshell(0, "sleep 1");
 ...
 bcm_error_t execute()
 {
   ...
   if (BCM_FAILURE((rv = riot_decap_multicast_bud(unit)))) {
	 printf("riot_decap_multicast_bud() Failed\n");
	 return -1;
   }
