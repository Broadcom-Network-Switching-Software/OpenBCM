...
/*  Feature  : TH2 Packetized Statistic Test
 *
 ...
 *  Summary:
 *  ========
 * Tomahawk3 supports a feature packetized statistics (PSTATS) which allows software
...
//////////////////////////////////////////////////////////////
// COSQ related
const int TH_MAX_PORTS = 256;// 136;
const int TH_NUM_CPU_Q = 48;
const int TH_NUM_PORT_L0 = 12; // 10
const int TH_NUM_PORT_Q_UC = 8; //10;
const int TH_NUM_PORT_Q_MC = 4;
bcm_gport_t cpu_cosq_gp_l0[TH_NUM_PORT_L0];
bcm_gport_t cpu_cosq_gp_mc[TH_NUM_CPU_Q];

bcm_gport_t cosq_gp_l0[TH_MAX_PORTS][TH_NUM_PORT_L0];
bcm_gport_t cosq_gp_uc[TH_MAX_PORTS][TH_NUM_PORT_Q_UC];
bcm_gport_t cosq_gp_mc[TH_MAX_PORTS][TH_NUM_PORT_Q_MC];
...
int gport_callback(int unit, bcm_gport_t port, int numq, uint32 flags,
    ...
    if( local_port >= TH_MAX_PORTS )
    {
        printf("Crossing the port limit %d vs %d\n", local_port, TH_MAX_PORTS);
        return 0;
    }
    //printf("    localport %d\n", local_port);
    if (local_port == 0) {
    ...
    else
    {
        ...
        else if (flags & BCM_COSQ_GPORT_UCAST_QUEUE_GROUP)  // 0x08
        {
            ...
            index = (index + 1) % TH_NUM_PORT_Q_UC;
        }
        else if (flags & BCM_COSQ_GPORT_MCAST_QUEUE_GROUP)  // 0x20
        {
            ...
            index = (index + 1) % TH_NUM_PORT_Q_MC;
        }
        else
        {
            printf("ERR: INVALID FLAG for port 0x%x(localport %d)\n",
...
int th_dump_gports (int port)
{
    int start = 0, end = 0, idx, g_idx, num_q = 0, n_idx, q_idx;

    if (port < -1) {
        printf ("Err. Input valid port number or -1 for all (%d) ports", TH_MAX_PORTS);
    }

    if ((port == -1) || (port == 0)) {
    printf("\nPrinting Gport Hierarchy for CPU Port %d\n", port);
    for (n_idx = 0; n_idx < TH_NUM_PORT_L0; n_idx++) {
            printf ("    L0.%d: 0x%x\n", n_idx, cpu_cosq_gp_l0[n_idx]);
            if (n_idx == 0) {
                for (q_idx = 0; q_idx < TH_NUM_CPU_Q; q_idx++) {
