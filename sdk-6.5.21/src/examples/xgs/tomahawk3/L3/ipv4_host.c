 ...
 *
 * Usage    : BCM.0> cint ipv4_host.c
 *
 * config   : l3_config.bcm
 *
 * Log file : ipv4_host_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of the IPv4 host entry using
 *   BCM APIs.
 *
 ...
 *
 *   2) Step2 - Configuration (Done in config_l3_ipv4_host()):
 *   =========================================================
...
configure_host(int unit , bcm_if_t *egr_if)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egress_port, &egr_gport));
               ...
               bcm_port_t ingress_port)
{
    ...
    rv = create_vlan_add_port(unit, ing_vlan, ingress_port);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring vlan : %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
/* Configuration: L3 IPv4 Entry/Host related */
bcm_error_t
config_l3_ipv4_host(int unit)
{
    bcm_error_t rv;
    ...
    rv = add_router_mac(unit, router_mac, ing_vlan, ingress_port);
    if (BCM_FAILURE(rv)) {
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_l3_ipv4_host())
...
execute(void)
{
    ...
    print "~~~ #2) config_l3_ipv4_host(): ** start **";
    if (BCM_FAILURE((rv = config_l3_ipv4_host(unit)))) {
        printf("config_l3_ipv4_host() failed.\n");
        return -1;
    }
    print "~~~ #2) config_l3_ipv4_host(): ** end **";
