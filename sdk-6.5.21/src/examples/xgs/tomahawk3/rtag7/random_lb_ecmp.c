...
bcm_error_t set_up_ecmp_packet_spray(int unit, bcm_port_t port)
{
    ...
    rv = bcm_switch_control_set(unit, bcmSwitchECMPLevel1RandomSeed, 0x1234);
    if (BCM_FAILURE (rv))
    {
    ...
    rv = bcm_switch_control_set(unit, bcmSwitchECMPLevel2RandomSeed, 0x1234);
    if (BCM_FAILURE (rv))
    {
    ...
    rv = bcm_port_control_set(unit, port,
            bcmPortControlECMPLevel2LoadBalancingRandomizer, 3);
    if (BCM_FAILURE (rv))
    {
