...
bcm_port_t egress_port3, egress_port4;
...
config_lsr_trunk(int unit)
{
    ...
    bcm_port_t eg_port3 = egress_port3, eg_port4 = egress_port4;
    bcm_gport_t eg_gport1, eg_gport2, eg_gport3, eg_gport4;
    ...
    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, eg_port4, &eg_gport4));
    ...
    trunk_ports[3] = eg_port4;
    rv = vlan_create_add_ports(unit, eg_vlan, 4, &trunk_ports);
    if (BCM_FAILURE(rv)) {
    ...
    rv = create_trunk(unit, &trunk_id, 4, &trunk_ports);
    if (BCM_FAILURE(rv)) {
        printf("\nError in configuration: %s.\n",bcm_errmsg(rv));
        return rv;
    }
