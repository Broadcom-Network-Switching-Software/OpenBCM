/*
 Feature        : EBST - Queue Based Events

 Usage          : BCM.0> cint th4_hsdk_mmu_ebst_mc_queue.c

 config         : th4_hsdk_mmu_config.yml
                  th4_hsdk_mmu_config_4g.yml

 Log file       : th4_hsdk_mmu_ebst_mc_queue_log.txt

 Test Topology  :

                       +------------+
                       |            |
 DMAC=0x000000000011   |   56990    |cd1(2) DMAC=0x000000000011
 SMAC=0x2      ------+->------------>-+     SMAC=0x2
 VLAN=3,Pri=0 cd0(1) | |            | |     VLAN=3,Pri=0
                     ^-<------------<-v
                       |            |
                       +------------+
  ...
  Summary:
  ========
  This example illustrates configuration of EBST based on Multicast queue events
    ...
    2) Step2 - EBST Configuration ebst_config()
      a. Configure threshold for selected port and MC queue
    ...
    3) Step3 - Verification (Done in test_verify())
      a. Send packet to ingress port, packet is DLF'ed and bounces between ingress
         and egress port with MC Queue 0
      b. These packets are expected to pass the BST threshold, and generate events
      c. Remove MC Queue from monitor profile before read EBST event data
...
bcm_cos_t mc_cos = 8; /* MC COS Queue */
...
bcm_error_t test_setup(int unit)
{
    ...
    /* Set ingress and egress ports to loopback, so DLF'ed traffic bounces between them */
...
bcm_error_t set_ebst_threshold_profile(int unit)
{
    ...
    /* Set EBST monitor on egress port and MC queue 0 (queue #8)  */
    object_id.port = egr_port;
    object_id.cosq = mc_cos;
    object_id.buffer = -1;
    ...
    rv = bcm_cosq_ebst_threshold_profile_set(unit, &object_id, bcmBstStatIdMcast, &profile);
    if (BCM_FAILURE(rv)) {
...
/*
 * Configure EBST Monitor on (egress port, MC queue) with 1000 entries
...
bcm_error_t set_ebst_monitor(int unit)
{
    ...
    rv = bcm_cosq_ebst_monitor_set(unit, &object_id, bcmBstStatIdMcast, &monitor);
    if (BCM_FAILURE(rv)) {
    ...
    if(check){
        bshell(0, "bsh -c 'lt TM_EBST_MC_Q traverse -l'");
    }
...
/*
 * Remove (egress port, MC Queue) from EBST monitor before FIFO Read
 */
bcm_error_t remove_object_ebst_monitor(int unit)
{
    ...
    /* Set EBST monitor on egress port MC queue */
    rv = bcm_cosq_ebst_monitor_set(unit, &object_id, bcmBstStatIdMcast, &monitor);
    if (BCM_FAILURE(rv)) {
        printf("bcm_cosq_ebst_monitor_set : %s.\n", bcm_errmsg(rv));
        return rv;
 ...
 */
bcm_error_t read_ebst_stat(int unit)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_cosq_object_id_t object_id;
    ...
    for (itm = 0; itm<2 ; itm++) {
        ...
        rv = bcm_cosq_ebst_data_get(unit, &object_id, bcmBstStatIdMcast,
...
test_verify(int unit)
{
    ...
    bshell(unit, "clear c");
    printf("\nSending one unicast packet to ingress port : %d\n", ing_port);
    printf("packet is DLF to egress port %d, and bouncing back and forth between the 2 ports\n", egr_port);
    snprintf(str, 512, "tx 1 pbm=%d DestMac=0x11 vlantag=3 length=2500; sleep quiet 1", ing_port);
    bshell(unit, str);
