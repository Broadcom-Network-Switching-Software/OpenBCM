/*
 * Feature  : Tx and Rx Pause frames
 *
 * Usage    : BCM.0> cint th4_hsdk_mmu_fc_pause.c
 *
 * Config   : th4_hsdk_mmu_config.yml
 *
 * Log file : th4_hsdk_mmu_fc_pause_log.txt
 *
 * Test Topology :
 *
 *          DMAC=0x1            +------------+         DMAC=0x1
 *     I    SMAC=0x2   +-------->   56980    +-------> SMAC=0x2  I
 *     X    VLAN=2        cd0(1)|            |cd1(2)   VLAN=2    X
 *     I                        | l2:mac=0x1 |                   I 
 *     A                        |    vlan=2  |                   A
 *               <-------+      |    port=2  |     <-------+
 ...
 *  Summary:
 *  ========
 *  This cint example demonstrates generic PAUSE feature on TH4
 *
 ...
 *
 *  2) Step2 - Configuration (Done in pause_tx_rx_config())
 *  =================================================
 *    a) Enable Tx and Rx Pause
 *
 *  3) Step3 - Verification (Done in test_verify())
 *  ==========================================
 *    a) Transmit l2 packet with dmac=0x1, vlan=1 at line rate from IXIA to ingress port cd0(Port-1)
 *
...
bcm_error_t
pause_tx_rx_config(int unit, bcm_port_t ing_port, bcm_port_t egr_port)
{
...
/*
 * This functions gets the port numbers and sets up ingress and egress ports.
 */
bcm_error_t
test_setup(int unit)
{
...
execute(void)
{
    ...
    if (BCM_FAILURE((rv = pause_tx_rx_config(unit, ing_port, egr_port)))) {
        printf("pause_tx_rx_config() failed.\n");
        return -1;
    }
