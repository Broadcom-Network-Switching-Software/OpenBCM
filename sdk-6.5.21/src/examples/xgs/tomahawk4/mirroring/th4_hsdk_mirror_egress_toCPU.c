/*
 * Feature  : Mirror packets destined for a egress port to CPU port.
 *
 * Usage    : BCM.0> cint th4_hsdk_mirror_egress_toCPU.c
 *
 * config   : th4_hsdk_mirroring_config.yml
 *
 * Log file : th4_hsdk_mirror_egress_toCPU_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *   egress_port1 to the mirror-to-port CPU (mtp_port).
 *
 ...
 *       c.2) Add mirroring destination to the egress port.
 ...
 *
 *     The packet capture shows the mirrored packet received on CPU port
...
bcm_error_t testVerify(int unit)
{
    ...
    BCM_PBMP_PORT_ADD(expected_pbmp[1], egress_port1);
    ...
    if (COMPILER_64_LO(counter_value.value[0]) != count_cpu) {
