/*
 * Feature  : Mirror packets ingress & egress on a port.
 *
 * Usage    : BCM.0> cint th4_hsdk_mirror_ing_egr_pair.c
 *
 * config   : th4_hsdk_mirroring_config.yml
 *
 * Log file : th4_hsdk_mirror_ing_egr_pair_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 *   Cint script to demonstrate mirror the traffic ingressing & egressing on
 *   port (egress_port1) to the mirror-to-port (mtp_port).
 *
 ...
 *       c.2) Add mirroring destination to the port using flags
...
unsigned char    *expected_packets[3];
bcm_pbmp_t        expected_pbmp[3];
uint32            packet_patterns = 3;
...
setup_mirror_egress(int unit)
{
    ...
    mirror_flags |= BCM_MIRROR_PORT_INGRESS | BCM_MIRROR_PORT_EGRESS;
...
bcm_error_t testVerify(int unit)
{
    ...
    expected_packets[2]  = expected_egress_packet1;
    ...
    BCM_PBMP_PORT_ADD(expected_pbmp[2], mtp_port);
    ...
    if (COMPILER_64_LO(counter_value.value[1]) != ((length_mtp+4)*count_mtp)) {
        printf("mtp_port: Packet verify failed. Expected bytes stat %d but get %d\n",
                                   ((length_mtp+4)*count_mtp),
                                   COMPILER_64_LO(counter_value.value[1]));
        flexCtr_pass = 0;
    }
