/*
 * Feature  : Mirror packets ingressing on a port using IFP action
 *            MirrorIngress.
 *
 * Usage    : BCM.0> cint th4_hsdk_mirror_ingress_ifp.c
 *
 * config   : th4_hsdk_mirroring_config.yml
 *
 * Log file : th4_hsdk_mirror_ingress_ifp_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *   ingress_port to the mirror-to-port (mtp_port) using IFP action
 *   MirrorIngress.
 *
 ...
 *       c.2) Qualify packet using FP qualifiers InPort & OuterVlan and
 *            add action MirrorIngress (mirror_dest_id).
...
setup_mirror_ingress(int unit)
{
    ...
    rv = bcm_field_action_add(unit, mirror_entry_id, bcmFieldActionMirrorIngress, 0,
                              mirror_dest_id);
    if(BCM_FAILURE(rv)) {
