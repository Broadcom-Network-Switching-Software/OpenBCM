/*
 * Feature  : Mirror packets ingressing on a por with RSPAN encapsulation.
 *
 * Usage    : BCM.0> cint th4_hsdk_mirror_rspan.c
 *
 * config   : th4_hsdk_mirroring_config.yml
 *
 * Log file : th4_hsdk_mirror_rspan_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *   ingress_port to the mirror-to-port (mtp_port) with RSPAN (Remote Switch
 *   Port Analyzer) encapsulation.
 *
 ...
 *
 *     c) Configuration (Done in setup_mirror_rspan()):
 *   =========================================================
 ...
 *
 *     The packet capture shows the egress packet transmitted on outgoing port
 *     (mtp_port) as per the mirror RSPAN setup.
 *
 *     Egress Packet 2:
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11,
...
/* Cleanup L2 switch configuration function. */
bcm_error_t
cleanup_mirror_rspan(int unit)
{
    bcm_error_t                 rv = BCM_E_NONE;
    uint32                      mirror_flags = 0;
...
test_setup(int unit)
{
    ...
    if (BCM_E_NONE != setup_mirror_rspan(unit)) {
        printf("setup_mirror_rspan() failed.\n");
        return -1;
    }
...
int test_cleanup(int unit)
{
    ...
    if (BCM_E_NONE != cleanup_mirror_rspan(unit)) {
        printf("cleanup_mirror_rspan() failed.\n");
        return -1;
    }
 ...
 *  This function does the following
 *  a) Test setup
 *  b) Actual configuration (done in setup_mirror_rspan())
