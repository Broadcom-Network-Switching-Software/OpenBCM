/*
 * Feature  : Mirror packets ingress on a port with four different
 *            encapsulations over L2 path.
 *
 * Usage    : BCM.0> cint th4_hsdk_mirror_max_copies_per_packet.c
 *
 * config   : th4_hsdk_mirroring_config.yml
 *
 * Log file : th4_hsdk_mirror_max_copies_per_packet_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *        ingress_port  |                   +------------------>
 *       -------------->+                   |
 *        VLAN 20       |                   | mtp_port2 (ERSPAN)
 *                      |                   +------------------>
 *                      |        TH4        |
 *                      |                   | mtp_port3 (SFLOW)
 *        egress_port   |                   +------------------>
 *       <--------------+                   |
 *        VLAN 20       |                   | mtp_port4 (PSAMP)
 *                      |                   +------------------>
 *                      |                   |
 *                      |                   |
 *                      +-------------------+
 *
 *       L2 entry : dst_mac=0x00:00:00:00:00:22, VLAN=20 --> PORT=egress_port
 *
 * Summary:
 * ========
 *   Cint script to demonstrate ingress mirror the traffic ingressing on
 *   ingress_port to the mirror-to-ports (mtp_port1, mtp_port2, mtp_port3,
 *   mtp_port4). Up to four mirrored copies can be sent for a single packet.
 *
 * Detailed steps done in the CINT script:
 * =======================================
 *   1) Step1 - Test Setup (Done in test_setup()):
 *   =============================================
 *     a) Selects 6 ports and configure them in Loopback mode. Out of these
 *        6 ports, one port is used as ingress port and the other five ports as
 *        egress ports.
 *
 ...
 *        in VLAN 20 to switch packet to egress port (egress_port).
 *
 *     c) Configuration (Done in setup_mirror_ingress()):
 *   =========================================================
 *       c.1) Create mirroring destination descriptors
 *              mirror_dest_id_1 - RSPAN
 *              mirror_dest_id_2 - ERSPAN
 *              mirror_dest_id_3 - SFLOW
 *              mirror_dest_id_4 - PSAMP
 *            - Descriptor contains mirrored traffic destinations (gport).
 *       c.2) Add mirroring destination to the ingress port.
 *       c.3) Validate MAX mirror port destination
 ...
 *       0xbc, 0x94, 0x97, 0x7f
 *
 *     The packet capture shows the egress packets transmitted on outgoing MTP
 *     ports as per the mirror setup.
 *
 *     Egress Packet 2: mtp_port1
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11,
 *       0x81, 0x00, 0x00, 0x0A, 0x81, 0x00, 0x00, 0x14, 0x08, 0x06, 0x00, 0x01,
 *       0x08, 0x00, 0x06, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02,
 *       0x0A, 0x0A, 0x0A, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x0A,
 *       0x0A, 0x03, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
 *       0x0a, 0x0b, 0x0c, 0x0d, 0xbc, 0x94, 0x97, 0x7f
 *
 *     Egress Packet 2: mtp_port2
 ...
 *
 *     Egress Packet 3: mtp_port3
 *       0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
 *       0x81, 0x00, 0x00, 0x0a, 0x08, 0x00, 0x45, 0x00, 0x00, 0x68, 0x00, 0x00,
 *       0x00, 0x00, 0x05, 0x11, 0xaf, 0x80, 0x02, 0x02, 0x02, 0x02, 0x01, 0x01,
 *       0x01, 0x01, 0x12, 0x34, 0x43, 0x21, 0x00, 0x54, 0x00, 0x00, 0x00, 0x00,
 *       0x00, 0x00, 0x01, 0x00, 0x02, 0x00, 0x80, 0x80, 0x00, 0x00, 0x00, 0x00,
 *       0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x81, 0x00,
 *       0x00, 0x14, 0x08, 0x06, 0x00, 0x01, 0x08, 0x00, 0x06, 0x04, 0x00, 0x01,
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x0a, 0x0a, 0x0a, 0x02, 0x00, 0x00,
 *       0x00, 0x00, 0x00, 0x00, 0x0a, 0x0a, 0x0a, 0x03, 0x00, 0x01, 0x02, 0x03,
 *       0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0xbc, 0x94,
 *       0x97, 0x7f
 *
 *     Egress Packet 4: mtp_port4
 *       0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
...
bcm_gport_t       mirror_dest_id_1, mirror_dest_id_2,
                  mirror_dest_id_3, mirror_dest_id_4, mirror_dest_id_5;
bcm_port_t        ingress_port;
bcm_port_t        egress_port;
bcm_port_t        mtp_port1, mtp_port2, mtp_port3, mtp_port4, mtp_port5;
...
unsigned char    *expected_packets[5];
bcm_pbmp_t        expected_pbmp[5];
uint32            packet_patterns = 5;
...
setup_flex_ctr(int unit)
{
    ...
    rv = bcm_port_stat_attach(unit, mtp_port3, stat_counter_id);
    if (BCM_FAILURE(rv)) {
        printf("bcm_vlan_stat_attach %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_port_stat_attach(unit, mtp_port4, stat_counter_id);
    if (BCM_FAILURE(rv)) {
        printf("bcm_vlan_stat_attach %s\n", bcm_errmsg(rv));
        return rv;
    }
...
cleanup_flex_ctr(int unit)
{
    ...
    rv = bcm_port_stat_detach_with_id(unit, mtp_port3, stat_counter_id);
    if (BCM_FAILURE(rv)) {
        printf("bcm_port_stat_detach_with_id: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_port_stat_detach_with_id(unit, mtp_port4, stat_counter_id);
    if (BCM_FAILURE(rv)) {
        printf("bcm_port_stat_detach_with_id: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
setup_mirror_ingress(int unit)
{
    ...
    bcm_gport_t                 mtp_gport1, mtp_gport2, mtp_gport3,
                                mtp_gport4, mtp_gport5;
    ...
    /* Get the GPORT ID for the MTP ports. */
    bcm_port_gport_get(unit, mtp_port1, &mtp_gport1);
    bcm_port_gport_get(unit, mtp_port2, &mtp_gport2);
    bcm_port_gport_get(unit, mtp_port3, &mtp_gport3);
    bcm_port_gport_get(unit, mtp_port4, &mtp_gport4);
    bcm_port_gport_get(unit, mtp_port5, &mtp_gport5);
    ...
    printf("RSPAN mirror destination create\n");
    bcm_mirror_destination_t_init(&mirror_dest);
    mirror_dest.flags  |= BCM_MIRROR_DEST_TUNNEL_L2;
    mirror_dest.gport   = mtp_gport1;
    ...
    mirror_dest_id_1 = mirror_dest.mirror_dest_id;
    printf("Mirror port destination add - %d\n", mirror_dest_id_1);
    ...
    mirror_dest_id_2 = mirror_dest.mirror_dest_id;
    printf("Mirror port destination add - %d\n", mirror_dest_id_2);
    mirror_flags = BCM_MIRROR_PORT_INGRESS;
    rv = bcm_mirror_port_dest_add(unit, ingress_port,
                                  mirror_flags,
    ...
    mirror_dest.truncate = 0;
    rv = bcm_mirror_destination_create(unit, &mirror_dest);
    if(BCM_FAILURE(rv)) {
        printf("\nError in mirror dest create: %s.\n",bcm_errmsg(rv));
        return rv;
    }
    mirror_dest_id_3 = mirror_dest.mirror_dest_id;
    printf("Mirror port destination add - %d\n", mirror_dest_id_3);
    ...
    mirror_dest_id_4 = mirror_dest.mirror_dest_id;
    printf("Mirror port destination add - %d\n", mirror_dest_id_4);
    mirror_flags = BCM_MIRROR_PORT_INGRESS | BCM_MIRROR_PORT_SFLOW |
    ...
    bcm_mirror_destination_t_init(&mirror_dest);
    mirror_dest.flags  |= 0;
    mirror_dest.gport   = mtp_gport5;
    rv = bcm_mirror_destination_create(unit, &mirror_dest);
    if(BCM_FAILURE(rv)) {
        printf("\nError in mirror dest create: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    mirror_dest_id_5 = mirror_dest.mirror_dest_id;
    mirror_flags = BCM_MIRROR_PORT_INGRESS;
    printf("Validate MAX mirror port destination\n");
    rv = bcm_mirror_port_dest_add(unit, ingress_port,
                                  mirror_flags,
                                  mirror_dest_id_5);
    if(BCM_SUCCESS(rv)) {
    ...
    rv = port_learn_mode_set(unit, mtp_port3, 0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_learn_mode_set: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = port_learn_mode_set(unit, mtp_port4, 0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_learn_mode_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
cleanup_mirror_ingress(int unit)
{
    ...
    rv = bcm_mirror_destination_destroy(unit, mirror_dest_id_1);
    if (BCM_FAILURE(rv)) {
    ...
    /* Remove mirror destination descriptor. */
    rv = bcm_mirror_destination_destroy(unit, mirror_dest_id_2);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_mirror_destination_destroy: %s\n",
                                        bcm_errmsg(rv));
        return rv;
    ...
    /* Remove mirror destination descriptor. */
    rv = bcm_mirror_destination_destroy(unit, mirror_dest_id_3);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_mirror_destination_destroy: %s\n",
                                        bcm_errmsg(rv));
        return rv;
    ...
    /* Remove mirror destination descriptor. */
    rv = bcm_mirror_destination_destroy(unit, mirror_dest_id_4);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_mirror_destination_destroy: %s\n",
                                        bcm_errmsg(rv));
        return rv;
    ...
    rv = port_learn_mode_set(unit, mtp_port3,
                             BCM_PORT_LEARN_ARL | BCM_PORT_LEARN_FWD);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_learn_mode_set: %s\n", bcm_errmsg(rv));
        return rv;
    ...
    rv = port_learn_mode_set(unit, mtp_port4,
                             BCM_PORT_LEARN_ARL | BCM_PORT_LEARN_FWD);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_learn_mode_set: %s\n", bcm_errmsg(rv));
        return rv;
...
test_setup(int unit)
{
    ...
    mtp_port3 = port_list[4];
    mtp_port4 = port_list[5];
    mtp_port5 = port_list[6];
    ...
    BCM_PBMP_PORT_ADD(egr_pbmp, mtp_port3);
    BCM_PBMP_PORT_ADD(egr_pbmp, mtp_port4);
    ...
    rv = add_l2_static_entry(unit, 0, dst_mac, switch_vlan, egress_port);
    if (BCM_FAILURE(rv)) {
...
unsigned char expected_egress_packet3[106] = {
        0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
        0x81, 0x00, 0x00, 0x0A, 0x08, 0x00, 0x45, 0x00, 0x00, 0x58, 0x00, 0x00,
        0x00, 0x00, 0x05, 0x2F, 0xAF, 0x72, 0x02, 0x02, 0x02, 0x02, 0x01, 0x01,
        0x01, 0x01, 0x00, 0x00, 0x88, 0xBE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
...
unsigned char expected_egress_packet4[122] = {
        0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
        0x81, 0x00, 0x00, 0x0a, 0x08, 0x00, 0x45, 0x00, 0x00, 0x68, 0x00, 0x00,
        0x00, 0x00, 0x05, 0x11, 0xaf, 0x80, 0x02, 0x02, 0x02, 0x02, 0x01, 0x01,
        0x01, 0x01, 0x12, 0x34, 0x43, 0x21, 0x00, 0x54, 0x00, 0x00, 0x00, 0x00,
...
unsigned char expected_egress_packet5[142] = {
        0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
        0x81, 0x00, 0x00, 0x0a, 0x08, 0x00, 0x45, 0x00, 0x00, 0x7c, 0x00, 0x00,
        0x00, 0x00, 0x05, 0x11, 0xaf, 0x6c, 0x02, 0x02, 0x02, 0x02, 0x01, 0x01,
        0x01, 0x01, 0x12, 0x34, 0x43, 0x21, 0x00, 0x68, 0x00, 0x00, 0x00, 0x0a,
...
bcm_error_t testVerify(int unit)
{
    ...
    BCM_PBMP_PORT_ADD(expected_pbmp[2], mtp_port2);
    BCM_PBMP_CLEAR(expected_pbmp[3]);
    BCM_PBMP_PORT_ADD(expected_pbmp[3], mtp_port3);
    BCM_PBMP_CLEAR(expected_pbmp[4]);
    BCM_PBMP_PORT_ADD(expected_pbmp[4], mtp_port4);
    ...
    if (COMPILER_64_LO(counter_value.value[1]) != (length_mtp1+4)) {
    ...
    if (COMPILER_64_LO(counter_value.value[1]) != (length_mtp2+4)) {
    ...
    /* Get counter value for mtp_port3. */
    sal_memset(&counter_value, 0, sizeof(counter_value));
    counter_index = mtp_port3;
    rv = bcm_flexctr_stat_sync_get(unit,
                                   stat_counter_id,
                                   num_entries,
                                   &counter_index,
    ...
    printf("FlexCtr Get on port_mtp_port3 : %d packets / %d bytes\n",
                                   COMPILER_64_LO(counter_value.value[0]),
                                   COMPILER_64_LO(counter_value.value[1]));
    if (COMPILER_64_LO(counter_value.value[0]) != count) {
        printf("mtp_port3: Packet verify failed. Expected packet stat %d but get %d\n",
                                   count,
                                   COMPILER_64_LO(counter_value.value[0]));
        flexCtr_pass = 0;
    }
    if (COMPILER_64_LO(counter_value.value[1]) != (length_mtp3+4)) {
        printf("mtp_port3: Packet verify failed. Expected bytes stat %d but get %d\n",
                                   (length_mtp3+4),
                                   COMPILER_64_LO(counter_value.value[1]));
        flexCtr_pass = 0;
    }

    /* Get counter value for mtp_port4. */
    sal_memset(&counter_value, 0, sizeof(counter_value));
    counter_index = mtp_port4;
    rv = bcm_flexctr_stat_sync_get(unit,
                                   stat_counter_id,
                                   num_entries,
                                   &counter_index,
    ...
    printf("FlexCtr Get on port_mtp_port4 : %d packets / %d bytes\n",
                                   COMPILER_64_LO(counter_value.value[0]),
                                   COMPILER_64_LO(counter_value.value[1]));
    if (COMPILER_64_LO(counter_value.value[0]) != count) {
        printf("mtp_port4: Packet verify failed. Expected packet stat %d but get %d\n",
                                   count,
                                   COMPILER_64_LO(counter_value.value[0]));
        flexCtr_pass = 0;
    }
    if (COMPILER_64_LO(counter_value.value[1]) != (length_mtp4+4)) {
        printf("mtp_port4: Packet verify failed. Expected bytes stat %d but get %d\n",
                                   (length_mtp4+4),
                                   COMPILER_64_LO(counter_value.value[1]));
        flexCtr_pass = 0;
    }
