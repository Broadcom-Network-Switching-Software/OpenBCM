/*
 * Feature  : Mirror packets ingressing on a port to multi destination with
 *            ERSPAN IPv4 encapsulation.
 *
 * Usage    : BCM.0> cint th4_hsdk_mirror_multi_dest.c
 *
 * config   : th4_hsdk_mirroring_config.yml
 *
 * Log file : th4_hsdk_mirror_multi_dest_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *   ingress_port to the mirror-to-port (mtp_port) with multi destination
 *   ERSPAN (Encapsulated Remote Switch Port Analyzer) IPv4 encapsulation.
 *
 ...
 *
 *     c) Configuration (Done in setup_mirror_erspan()):
 *   =========================================================
 *       c.1) Create multiple mirror destination IP group
 *       c.2) Create mirroring destination descriptor (mirror_dest_id) with
 *            multiple mirror destination IP group and ERSPAN encapsulation.
 *            - Descriptor contains mirrored traffic destination (gport) and
 *              multiple mirror destination IP group and mirrored traffic
 *              encapsulation.
 *       c.3) Add mirroring destination to the ingress port.
 *       c.4) Set hash value to choose multi destination.
 *       c.5) L2 learn mode is set up on ingress and egress ports.
 *
 *   2) Step3 - Verification (Done in verify()):
 *   ===========================================
 *     a) Transmit the packets on the ingress port (ingress_port) and loopback.
 *
 *     Ingress Packets:
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x99, 0x12, 0x34, 0x51, 0x11,
 ...
 *
 *     The packet capture shows the egress packet transmitted on outgoing port
 *     (mtp_port) as per the mirror ERSPAN setup.
 *
 *     Egress Packet 2:
 *       0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
 *       0x81, 0x00, 0x00, 0x0A, 0x08, 0x00, 0x45, 0x00, 0x00, 0x58, 0x00, 0x00,
 *       0x00, 0x00, 0x05, 0x2F, 0x7e, 0x94, 0x02, 0x02, 0x02, 0x02, 0x02, 0x30,
 *       0x30, 0xb0, 0x00, 0x00, 0x88, 0xBE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
 *       0x00, 0x99, 0x12, 0x34, 0x51, 0x11, 0x81, 0x00, 0x00, 0x14, 0x08, 0x06,
 *       0x00, 0x01, 0x08, 0x00, 0x06, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
 *       0x00, 0x02, 0x0A, 0x0A, 0x0A, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
 *       0x0A, 0x0A, 0x0A, 0x03, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
 ...
 *       0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
 *       0x81, 0x00, 0x00, 0x0A, 0x08, 0x00, 0x45, 0x00, 0x00, 0x58, 0x00, 0x00,
 *       0x00, 0x00, 0x05, 0x2F, 0x9e, 0xc4, 0x02, 0x02, 0x02, 0x02, 0x02, 0x10,
 *       0x10, 0xa0, 0x00, 0x00, 0x88, 0xBE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x12, 0x81, 0x00, 0x00, 0x14, 0x08, 0x06,
 *       0x00, 0x01, 0x08, 0x00, 0x06, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
 *       0x00, 0x02, 0x0A, 0x0A, 0x0A, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
 *       0x0A, 0x0A, 0x0A, 0x03, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
 *       0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0xbc, 0x94, 0x97, 0x7f
...
/* Mirror ingress ERSPAN configuration. */
bcm_error_t
setup_mirror_erspan(int unit)
{
    ...
    bcm_mirror_multi_dip_entry_num_t  num_entries = bcmMirrorMultiDipEntryNum2;
    bcm_ip_t                          ip_addrs[2] = {0x021010a0, 0x023030b0};

    /* Get the GPORT ID for the MTP port. */
    bcm_port_gport_get(unit, mtp_port, &mtp_gport);

    /* Create multiple mirror destination IP group. */
    print bcm_mirror_multi_dip_group_ipv4_create(unit, num_entries, ip_addrs, &multi_dip_group[0]);
    printf("Multiple DIP group %d created with size = %d.\n", multi_dip_group[0], 1 << num_entries);

    printf("ERSPAN mirror destination create\n");
    ...
    mirror_dest.ttl       = 5;
    mirror_dest.multi_dip_group = multi_dip_group[0];
    ...
    print bcm_switch_control_set(unit, bcmSwitchHashSeed0, 0x12345677);
    print bcm_switch_control_set(unit, bcmSwitchHashField0Config, BCM_HASH_FIELD_CONFIG_CRC32LO);
    print bcm_switch_control_set(unit, bcmSwitchHashField0Config1, BCM_HASH_FIELD_CONFIG_CRC32HI);
    print bcm_switch_control_port_set(unit, ingress_port, bcmSwitchEntropyHashSet0Offset, 86);
...
bcm_error_t
cleanup_mirror_erspan(int unit)
{
    bcm_error_t                 rv = BCM_E_NONE;
    uint32                      mirror_flags = 0;
    ...
    /* Destroy multiple mirror destination IP group. */
    rv = bcm_mirror_multi_dip_group_ipv4_destroy(unit, multi_dip_group[0]);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_mirror_multi_dip_group_ipv4_destroy: %s\n",
                                        bcm_errmsg(rv));
        return rv;
    }
...
test_setup(int unit)
{
    ...
    if (BCM_E_NONE != setup_mirror_erspan(unit)) {
        printf("setup_mirror_erspan() failed.\n");
        return -1;
    }
...
int test_cleanup(int unit)
{
    ...
    if (BCM_E_NONE != cleanup_mirror_erspan(unit)) {
        printf("cleanup_mirror_erspan() failed.\n");
        return -1;
    }
...
unsigned char expected_egress_packet2[106] = {
        0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
        0x81, 0x00, 0x00, 0x0A, 0x08, 0x00, 0x45, 0x00, 0x00, 0x58, 0x00, 0x00,
        0x00, 0x00, 0x05, 0x2F, 0x7e, 0x94, 0x02, 0x02, 0x02, 0x02, 0x02, 0x30,
        0x30, 0xb0, 0x00, 0x00, 0x88, 0xBE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
...
unsigned char expected_egress_packet4[106] = {
        0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
        0x81, 0x00, 0x00, 0x0A, 0x08, 0x00, 0x45, 0x00, 0x00, 0x58, 0x00, 0x00,
        0x00, 0x00, 0x05, 0x2F, 0x9e, 0xc4, 0x02, 0x02, 0x02, 0x02, 0x02, 0x10,
        0x10, 0xa0, 0x00, 0x00, 0x88, 0xBE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
...
bcm_error_t testVerify(int unit)
{
    ...
    BCM_PBMP_PORT_ADD(expected_pbmp[2], egress_port1);
    BCM_PBMP_CLEAR(expected_pbmp[3]);
    BCM_PBMP_PORT_ADD(expected_pbmp[3], mtp_port);
    ...
    snprintf(str, 512, "tx %d pbm=%d data=0x00000000002200991234511181000014080600010800060400010000000000020A0A0A020000000000000A0A0A03000102030405060708090a0b0c0dbc94977f; sleep quiet 3",1, ingress_port);
    bshell(unit, str);

    snprintf(str, 512, "tx %d pbm=%d data=0x00000000002200000000001281000014080600010800060400010000000000020A0A0A020000000000000A0A0A03000102030405060708090a0b0c0dbc94977f; sleep quiet 3",1, ingress_port);
    bshell(unit, str);
    ...
    if (COMPILER_64_LO(counter_value.value[1]) != ((length+4)*2)) {
