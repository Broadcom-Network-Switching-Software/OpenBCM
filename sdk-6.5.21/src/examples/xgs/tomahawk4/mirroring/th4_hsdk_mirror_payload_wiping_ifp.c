/*
 * Feature  : Wipe the mirror payload packet using IFP action
 *            MirrorZeroingEnable.
 *
 * Usage    : BCM.0> cint th4_hsdk_mirror_payload_wiping_ifp.c
 *
 * config   : th4_hsdk_mirroring_config.yml
 *
 * Log file : th4_hsdk_mirror_payload_wiping_ifp_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *   ingress_port to the mirror-to-port (mtp_port) with ERSPAN (Encapsulated
 *   Remote Switch Port Analyzer) IPv4 encapsulation and payload values get
 *   wiped from L2 payload packet post configured offset using IFP action
 *   MirrorZeroingEnable.
 *
 *
 ...
 *       c.3) Create mirror payload zero profile (mirror_prof_index)
 *       c.4) Create FP entry using FP qualifiers InPort & OuterVlan and
 *            add action MirrorZeroingEnable (mirror_prof_index).
 ...
 *       0x00, 0x00, 0x05, 0x2f, 0xaf, 0x72, 0x02, 0x02, 0x02, 0x02, 0x01, 0x01,
 *       0x01, 0x01, 0x00, 0x00, 0x88, 0xbe, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x81, 0x00, 0x00, 0x14, 0x08, 0x06,
 *       0x00, 0x01, 0x08, 0x00, 0x06, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
uint32            mirror_prof_index = -1;      /* Mirror payload zero profile ID */
...
{
    bcm_mirror_payload_zero_offsets_t   payload_zero_offsets;
    ...
    rv = bcm_mirror_port_dest_add(unit, ingress_port, mirror_flags,
                                  mirror_dest_id);
    if(BCM_FAILURE(rv)) {
    ...
    printf("Configure offset for payload wiping using IFP profile actions \n");
    bcm_mirror_payload_zero_offsets_t_init(&payload_zero_offsets);
    payload_zero_offsets.l2_offset = 3;
    payload_zero_offsets.l2_offset_disable = 0;
    rv = bcm_mirror_payload_zero_profile_create(unit, &payload_zero_offsets,
                                                BCM_MIRROR_PAYLOAD_ZERO_FP,
                                                &mirror_prof_index);
    print mirror_prof_index;

    /* Mirror using IFP action */
    ...
    BCM_FIELD_ASET_ADD(action, bcmFieldActionMirrorZeroingEnable);
    BCM_FIELD_ASET_ADD(action, bcmFieldActionMirrorIngress);
    ...
    rv = bcm_field_action_add(unit, mirror_entry_id,
                              bcmFieldActionMirrorZeroingEnable, mirror_prof_index,
                              0);
    if(BCM_FAILURE(rv)) {
...
cleanup_mirror_ingress(int unit)
{
    ...
    printf("Destroy mirror payload zero profile \n");
    rv = bcm_mirror_payload_zero_profile_destroy(unit,
                                                BCM_MIRROR_PAYLOAD_ZERO_FP,
                                                mirror_prof_index);
    if(BCM_FAILURE(rv)) {
        printf("\nError in bcm_mirror_payload_zero_profile_destroy: %s.\n",
                                                           bcm_errmsg(rv));
        return rv;
    }
    ...
    /* Destroy mirror destination descriptor. */
...
bcm_error_t testVerify(int unit)
{
    ...
    if (debug) {
        ...
        bshell(unit, "bsh -c 'lt MIRROR_EGR_ZERO_PAYLOAD_PROFILE lookup MIRROR_EGR_ZERO_PAYLOAD_PROFILE_ID=1'");
        bshell(unit, "fp show");
    }
