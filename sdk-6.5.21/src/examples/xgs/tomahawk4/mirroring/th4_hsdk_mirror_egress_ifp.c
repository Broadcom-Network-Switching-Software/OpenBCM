/*
 * Feature  : Mirror packets destined for a egress port using IFP action
 *            MirrorEgress.
 *
 * Usage    : BCM.0> cint th4_hsdk_mirror_egress_ifp.c
 *
 * config   : th4_hsdk_mirroring_config.yml
 *
 * Log file : th4_hsdk_mirror_egress_ifp_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 *   Cint script to demonstrate egress mirror the traffic egressing on
 *   egress_port1 to the mirror-to-port (mtp_port) using IFP action
 *   MirrorEgress.
 *
 ...
 *
 *     c) Configuration (Done in setup_mirror_egress()):
 *   =========================================================
 *       c.1) Create mirroring destination descriptor (mirror_dest_id).
 *            - Descriptor contains mirrored traffic destination (gport).
 *       c.2) Qualify packet using FP qualifiers L3DestRouteHit & OuterVlan and
 *            add action MirrorEgress (mirror_dest_id).
...
setup_mirror_egress(int unit)
{
    ...
    BCM_FIELD_QSET_ADD(qual, bcmFieldQualifyL3DestRouteHit);
    BCM_FIELD_QSET_ADD(qual, bcmFieldQualifyOuterVlan);

    BCM_FIELD_ASET_INIT(action);
    BCM_FIELD_ASET_ADD(action, bcmFieldActionMirrorEgress);
    ...
    rv = bcm_field_qualify_L3DestRouteHit(unit, mirror_entry_id, 1, 0xf);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field qualify OuterVlan : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_field_qualify_OuterVlan(unit, mirror_entry_id, vlan_in, 0xfff);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field qualify OuterVlan : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_field_action_add(unit, mirror_entry_id, bcmFieldActionMirrorEgress, 0,
                              mirror_dest_id);
    if(BCM_FAILURE(rv)) {
...
test_setup(int unit)
{
    ...
    if (BCM_E_NONE != setup_mirror_egress(unit)) {
        printf("setup_mirror_egress() failed.\n");
        return -1;
    }
 ...
 *  This function does the following
 *  a) Test setup
 *  b) Actual configuration (done in setup_mirror_egress())
