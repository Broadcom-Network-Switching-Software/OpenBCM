/*
 * Feature  :  IFP rule with SystemPort class as a qualifier
 *
 * Usage    : BCM.0> c th4_hsdk_ifp_sysportclass_group.c
 *
 * config   : th4_hsdk_ifp_sysportclass_group_config.yml
 *            with FP_ING_OPERMODE: GLOBAL_PIPE_AWARE
 *
 * Log file : th4_hsdk_ifp_sysportclass_group_log.txt
 *
 * Test Topology :
 *                              CPU
 *                                ^
 ...
 * this script demonstrates use of SystemPort class as a IFP qualifier in global
 * pipe aware IFP mode.
 *
 ...
 *   =============================================
 *     a) Select list of random ingress ports and configure them in Loopback mode.
 *     b) Assign SystemPort classids to the selected test ports.
 *     c) Start packet watcher and register additional custom Rx callback for the
 *        packet verification.
 *
 *   2) Step2 - IFP Configuration (Done in ifpClassSetup()):
 *   =========================================================
 *     a) An IFP group is created with Outer vlan and SystemPort class as qualifiers
...
ifpClassSetup(int unit) {
    ...
    /* IFP group configuration with System port bitmap qualifier */
    BCM_FIELD_QSET_INIT(group_config.qset);
    BCM_FIELD_QSET_ADD(group_config.qset, bcmFieldQualifyStageIngress);
    BCM_FIELD_QSET_ADD(group_config.qset, bcmFieldQualifyOuterVlanId);
    BCM_FIELD_QSET_ADD(group_config.qset,bcmFieldQualifySystemPortBitmap);
    ...
    /* Create FP entry to match on class[0] and class[1] SystemPorts */
    ...
    for (inst = 0; inst < num_pipe_instances; inst++) {
    ...
    rv |= bcm_field_qualify_SystemPortBitmap(unit, entry[0], in_pbmp, in_pbmp_mask);
    if (BCM_FAILURE(rv)) {
    ...
    /* Create FP entry to match on class[2] and class[1] SystemPorts */
    ...
    rv |= bcm_field_qualify_SystemPortBitmap(unit, entry[1], in_pbmp, in_pbmp_mask);
    if (BCM_FAILURE(rv)) {
...
/* Configures the port group SystemPort classids */
bcm_error_t
port_ipbm_setup(int unit) {
    int rv = 0;
    /* Set port System port bitmap for all test ports port_list[0-3] */
    rv = bcm_port_class_set(unit, port_list[0], bcmPortClassFieldIngressSystemPort, class[0]);

    rv |= bcm_port_class_set(unit, port_list[1], bcmPortClassFieldIngressSystemPort, class[1]);
    rv |= bcm_port_class_set(unit, port_list[2], bcmPortClassFieldIngressSystemPort, class[1]);

    rv |= bcm_port_class_set(unit, port_list[3], bcmPortClassFieldIngressSystemPort, class[2]);
