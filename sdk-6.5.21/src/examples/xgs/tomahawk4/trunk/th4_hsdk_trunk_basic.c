/*  Feature  : Trunk basic configuration
 *
 *  Usage    : BCM.0> cint th4_hsdk_trunk_basic.c
 *
 *  config   : th4_hsdk_trunk_config.yml
 *
 *  Log file : th4_hsdk_trunk_basic_log.txt
 *
 *  Test Topology :
 *
 *                +---------------+
 ...
 *   Summary:
 *   ========
 *     This cint example details basic trunk configurations like trunk creation,
 *     adding ports to the trunk, removing ports from the trunk, changing the
 *     selection policy, configuring RTAG7 hashing based on the L4 ports.
 *
 ...
 *     =======================================================
 *       a) Create a trunk and add ports egress_port_1 and egress_port_2
 *          as members. Set the selection criteria to BCM_TRUNK_PSC_RANDOMIZED
 ...
 *          the egress ports using "show counters".
 *
 *          Ingress Packets
 *          ===============
 *          DA 0x00000000AAAA
...
bcm_error_t trunk_basic_config(int unit)
{
    ...
    /* Initialize trunk chip information structure */
     bcm_trunk_chip_info_t_init(&ta_info);

    /* Gets the underlying SoC device's trunk information */
    rv = bcm_trunk_chip_info_get(unit, &ta_info);
    if (BCM_FAILURE (rv))
    {
        printf ("Error in bcm_trunk_chip_info_get(): %s.\n", bcm_errmsg (rv));
        return rv;
    }
    printf("Device trunk info structure :\n");
    print ta_info;
    ...
    printf("\nCreate trunk with ports %d and %d\n",egress_port1, egress_port2);

    BCM_GPORT_TRUNK_SET (trunk_gp_id, trunk_id);

    rv = trunk_add_member(unit, trunk_id, egress_port3);
    if (BCM_FAILURE (rv))
    {
    ...
    rv = trunk_delete_member(unit, trunk_id, egress_port3);
    if (BCM_FAILURE (rv))
    {
    ...
    /* Get the trunk id for given port */
    rv = bcm_trunk_find(unit, 0, egress_port1, &tid);
    if (BCM_FAILURE (rv))
    {
        printf ("Error in bcm_trunk_find(): %s.\n", bcm_errmsg (rv));
        return rv;
    }
    printf("\nPort %d trunk group id is %d\n", egress_port1, tid);

    /* Get all associated trunk member ports for a given port bitmap */
    BCM_PBMP_CLEAR(pbm);
    BCM_PBMP_PORT_ADD(pbm, egress_port2);
    rv = bcm_trunk_bitmap_expand(unit, pbm);
    if (BCM_FAILURE (rv))
    {
        printf ("Error in bcm_trunk_bitmap_expand(): %s.\n", bcm_errmsg (rv));
        return rv;
    }
    printf("\nOther member ports associated with port %d are ", egress_port2);
    print pbm;
...
bcm_error_t test_verify(int unit)
{
    ...
    /* Print counter value. */
    printf("\nFlexCtr Get on egress_port1 : %d packets / %d bytes\n",
    ...
    /* Print counter value. */
    printf("FlexCtr Get on egress_port2 : %d packets / %d bytes\n",
                                   COMPILER_64_LO(counter_value.value[0]),
                                   COMPILER_64_LO(counter_value.value[1]));
    received_rx_count += COMPILER_64_LO(counter_value.value[0]);
    received_rx_bytes += COMPILER_64_LO(counter_value.value[1]);
