/*  Feature  : RTAG7 of L3 IPv4 Route to ECMP group
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3_rtag7_ecmp.c
 *
 *  config   : bcm56990-generic-l3.config.yml
 *
 *  Log file : th4_hsdk_l3_rtag7_ecmp_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
 ...
 *  Summary:
 *  ========
 *  This cint example demonstrate L3 IPv4 route to ecmp group with RTAG7 hash
 *  configuration.
 *
 ...
 *
 *    2) Step2 - Configuration (Done in config_rtag7_l3_route_ecmp())
 *    ======================================================
 ...
 *    ==========================================
 *      a) Verify the result after send test packets, two packets with
 *      different source IP, and two packets with different L4 destination
 *      port
 *         ingress_port:
 ...
 *
 *         {000000000001} {002a10777700} 8100 000b
 *         0800 4500 003c 6762 0000 ff01 bd75 0a3a
 *         4002 0a3a 4273 0000 550a 0001 0051 6162
 *         6364 6566 6768 696a 6b6c 6d6e 6f70 7172
 *         7374 7576 7761 6263 6465 6667 6869
 *
 *         {000000000001} {002a10777700} 8100 000b
 ...
 *         egress_port[1]/egr l3 obj 2:
 *         {00000002fe02} {000000000202} 8100 0016
 *         0800 4500 003c 6762 0000 fe01 be76 0a3a
 *         4001 0a3a 4273 0000 550a 0001 0051 6162
 *         6364 6566 6768 696a 6b6c 6d6e 6f70 7172
 *         7374 7576 7761 6263 6465 6667 6869
 *         Packet[1]:   Ethernet: dst<00:00:00:02:fe:02> src<00:00:00:00:02:02> Tagged Packet
 *         ProtID<0x8100> Ctrl<0x0016> Internet Protocol (IP)
 *         Packet[1]:   IP: V(4) src<10.58.64.1> dst<10.58.66.115> hl<5> service-type<0> tl<60>
 *         id<26466> frg-off<0> ttl<254> > chk-sum<0xbe76>
 ...
 *
 *         egress_port[0]/ egr l3 obj:
 *         {00000000fe02} {000000000002} 8100 000c
 *         0800 4500 003c 6762 0000 fe01 be75 0a3a
 *         4002 0a3a 4273 0000 550a 0001 0051 6162
 *         6364 6566 6768 696a 6b6c 6d6e 6f70 7172
 *         7374 7576 7761 6263 6465 6667 6869
 *         Packet[2]:   Ethernet: dst<00:00:00:00:fe:02> src<00:00:00:00:00:02> Tagged Packet
 *         ProtID<0x8100> Ctrl<0x000c> Internet Protocol (IP)
 *         Packet[2]:   IP: V(4) src<10.58.64.2> dst<10.58.66.115> hl<5> service-type<0> tl<60>
 *         id<26466> frg-off<0> ttl<254> > chk-sum<0xbe75>
 *         Packet[2]:   ICMP: Type-0 (Echo Reply [RFC792]) Code-0
 *         Packet[2]:   Generic: Code <25088> Checksum <0x6364> hun <65 66 67 68>
 *
 *         egress_port[0]/ egr l3 obj:
 *         Packet[3]: data[0000]: {00000000fe02} {000000000002} 8100 000c
 *         Packet[3]: data[0010]: 0800 4500 003c 6762 0000 fe11 be65 0a3a
 *         Packet[3]: data[0020]: 4002 0a3a 4273 0000 550a 0028 6907 6162
 *         Packet[3]: data[0030]: 6364 6566 6768 696a 6b6c 6d6e 6f70 7172
 *         Packet[3]: data[0040]: 7374 7576 7761 6263 6465 6667 6869
 *
 *         Packet[3]:   Ethernet: dst<00:00:00:00:fe:02> src<00:00:00:00:00:02> Tagged Packet
 *         ProtID<0x8100> Ctrl<0x000c> Internet Protocol (IP)
 *         Packet[3]:   IP: V(4) src<10.58.64.2> dst<10.58.66.115> hl<5> service-type<0> tl<60>
 *         id<26466> frg-off<0> ttl<254> > chk-sum<0xbe65>
 *         Packet from data=<>, length=78
 *
 *         egress_port[1]/egr l3 obj 2:
 *         Packet[4]: data[0000]: {00000002fe02} {000000000202} 8100 0016
 *         Packet[4]: data[0010]: 0800 4500 003c 6762 0000 fe11 be65 0a3a
 *         Packet[4]: data[0020]: 4002 0a3a 4273 0000 550c 0028 6905 6162
 *         Packet[4]: data[0030]: 6364 6566 6768 696a 6b6c 6d6e 6f70 7172
 *         Packet[4]: data[0040]: 7374 7576 7761 6263 6465 6667 6869
 *
 *         Packet[4]:   Ethernet: dst<00:00:00:02:fe:02> src<00:00:00:00:02:02> Tagged Packet
 *         ProtID<0x8100> Ctrl<0x0016> Internet Protocol (IP)
 *         Packet[4]:   IP: V(4) src<10.58.64.2> dst<10.58.66.115> hl<5> service-type<0> tl<60>
 *         id<26466> frg-off<0> ttl<254> > chk-sum<0xbe65>
 *
 *
 *    4) Step4 - The l3 init ECMP with RTAG7 teardown configuration(Done in test_cleanup())
 *    ==========================================
 *      a) Mostly reverse the configuration, done in clear_rtag7_l3_route_ecmp()
 */
cint_reset();
...
pktioRxCallback(int unit, bcm_pktio_pkt_t * packet, void *cookie)
{
	...
	if ((egress_port[0] != port) && (egress_port[1] != port)){
		...
		if ((sal_memcmp(buffer, expected_packets[0], length) != 0) &&
			(sal_memcmp(buffer, expected_packets[2], length) != 0))
		{
   	        test_failed = 1;
		...
		if((sal_memcmp(buffer, expected_packets[1], length) != 0) &&
		   (sal_memcmp(buffer, expected_packets[3], length) != 0))
		{
   	        test_failed = 1;
            printf("pktioRxCallback: packet received from port %d as expected, but packet data not match\n", port);
		}
...
check_flexctr(int unit, uint32 stat_counter_id, int *flexctr_check_failed)
{
    ...
    int expected_packet_length = 82*2;
	...
	/* expected to receive 2 packets from each of egr l3 egress obj */
    if (COMPILER_64_LO(counter_value[1].value[0]) != 2) {
        printf("Flexctr packets verify failed. flexctr attached to egr obj 1 got %d packets.\n",
				COMPILER_64_LO(counter_value[1].value[0]));
        *flexctr_check_failed = 1;
        print counter_value;
        rv = BCM_E_FAIL;
    }else if(COMPILER_64_LO(counter_value[2].value[0]) != 2) {
...
bcm_error_t
config_rtag7_l3_route_ecmp(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
...
bcm_error_t clear_rtag7_l3_route_ecmp(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
...
void test_verify(int unit)
{
    ...
    bshell(unit, str);
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000001002a107777008100000B08004500003c67620000ff01bd750a3a40020a3a42730000550a000100516162636465666768696a6b6c6d6e6f7071727374757677616263646566676869; sleep quiet 1", ingress_port);
    bshell(unit, str);
    ...
    printf("\n------------------------------------------------------------------------------- \n");
    printf("    RESULT OF RTAG7 of L3 IPv4 ROUTE to ECMP GROUP with RTAG7 HASH VERIFICATION  ");
    printf("\n------------------------------------------------------------------------------- \n");
...
int test_cleanup(int unit)
{
    ...
    /* Remove the l3 route to ECMP group related configuration */
	rv = clear_rtag7_l3_route_ecmp(unit);
    if (BCM_FAILURE(rv)) {
        printf("clear_rtag7_l3_route_ecmp() failed.\n");
        return rv;
	}
 ...
 *  a) test setup
 *  b) demonstrates the deployment for l3 route to ECMP group with RTAG7 configure sequence
 *     (Done in config_rtag7_l3_route_ecmp())
 *  c) demonstrates the functionality(done in test_verify()).
 *  d) demonstrates the l3 IPv4 route to ECMP group with RTAG7 teardown configure sequence
...
execute(void)
{
    ...
    print "~~~ #2) config_rtag7_l3_route_ecmp(): ** start **";
    if (BCM_FAILURE((rv = config_rtag7_l3_route_ecmp(unit)))) {
        printf("config_rtag7_l3_route_ecmp() failed.\n");
        return rv;
    }
    print "~~~ #2) config_rtag7_l3_route_ecmp(): ** end **";
