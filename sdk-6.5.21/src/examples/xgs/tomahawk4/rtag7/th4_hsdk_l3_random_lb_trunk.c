/*  Feature  : L3 IPv4 route to trunk group with random load balance
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3_random_lb_trunk.c
 *
 *  config   : bcm56990-generic-l3.config.yml
 *
 *  Log file : th4_hsdk_l3_random_lb_trunk_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
 ...
 *
 *    2) Step2 - Configuration (Done in config_l3_random_lb_trunk())
 *    ======================================================
 ...
 *    ==========================================
 *      a) Verify the result after send 100 packet.
 ...
 *
 *      b) Expected Result:
 *         From both trunk member ports, received same packet,
 *         check the packets count of each trunk member ports withing tolerace range
 *         Packet raw data (78):
 ...
 *         7374 7576 7761 6263 6465 6667 6869
 *
 *    4) Step4 - The l3 IPv4 route to trunk group with randon load balance teardown
 *       configuration(Done in test_cleanup())
 *    ==========================================
 *      a) Mostly reverse the configuration, done in clear_l3_random_lb_trunk()
 */
cint_reset();
...
check_flexctr(int unit, uint32 stat_counter_id, int *flexctr_check_failed)
{
    ...
    if (COMPILER_64_LO(counter_value[egress_port[i]].value[0]) < (test_pkts_num/2-tolerance)) {
...
bcm_error_t
config_l3_random_lb_trunk(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
...
bcm_error_t clear_l3_random_lb_trunk(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
...
void test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx %d pbm=%d data=0x000000000001002a107777008100000B08004500003c67620000ff01bd760a3a40010a3a42730000550a000100516162636465666768696a6b6c6d6e6f7071727374757677616263646566676869; sleep 1", test_pkts_num, ingress_port);
    bshell(unit, str);
    ...
    printf("\n-------------------------------------------------------------------- \n");
    printf("    RESULT OF L3 ROUTE TO TRUNK WITH RANDOM LOAD BALANCE VERIFICATION  ");
    printf("\n-------------------------------------------------------------------- \n");
...
int test_cleanup(int unit)
{
	...
	rv = clear_l3_random_lb_trunk(unit);
    if (BCM_FAILURE(rv)) {
        printf("clear_l3_random_lb_trunk() failed.\n");
        return rv;
	}
 ...
 *  a) test setup
 *  b) demonstrates the deployment for l3 mpls configure sequence(Done in config_l3_random_lb_trunk())
...
execute(void)
{
    ...
    print "~~~ #2) config_l3_random_lb_trunk(): ** start **";
    if (BCM_FAILURE((rv = config_l3_random_lb_trunk(unit)))) {
        printf("config_l3_random_lb_trunk() failed.\n");
        return rv;
    }
    print "~~~ #2) config_l3_random_lb_trunk(): ** end **";
