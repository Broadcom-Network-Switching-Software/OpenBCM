/*  Feature  : L3 IPv4 route to trunk group with rtag7 versatile hash load balance
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3_rtag7_versatile_hash_trunk.c
 *
 *  config   : bcm56990-generic-l3.config.yml
 *
 *  Log file : th4_hsdk_l3_rtag7_versatile_hash_trunk_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
...
bcm_error_t config_trunk_rtag7_versatile_hash(int unit)
{
  int flags;
  bcm_error_t rv = BCM_E_NONE;
  ...
  rv = bcm_switch_control_set(unit, bcmSwitchHashField0Config,
      BCM_HASH_FIELD_CONFIG_VERSATILE_HASH_LO);
  if (BCM_FAILURE(rv)) {
    printf("Error executing bcm_switch_control_set(): %s.\n", bcm_errmsg(rv));
    return rv;
  }
  ...
  rv = bcm_switch_control_set(unit, bcmSwitchHashField0Config1,
      BCM_HASH_FIELD_CONFIG_VERSATILE_HASH_HI);
  if (BCM_FAILURE(rv)) {
    printf("Error executing bcm_switch_control_set(): %s.\n", bcm_errmsg(rv));
    return rv;
  }
...
config_l3_route_rtag7_trunk(int unit)
{
    ...
    rv = config_trunk_rtag7_versatile_hash(unit);
    if (BCM_FAILURE(rv)) {
        printf("\nError in config_trunk_rtag7_versatile_hash(): %s.\n",bcm_errmsg(rv));
        return rv;
    }
...
void test_verify(int unit)
{
    ...
    printf("\n---------------------------------------------------------------------------------- \n");
    printf("    RESULT OF L3 ROUTE TO TRUNK WITH RTAG7 with VERSATILE HASH ALGORITHM VERIFICATION  ");
    printf("\n---------------------------------------------------------------------------------- \n");
 ...
 *  d) demonstrates the l3 IPv4 route to tunk group teardown configure sequence
