/*  Feature  : L3 MPLS tunnel termination to trunk group with rtag7 hash
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_term_rtag7_trunk.c
 *
 *  config   : bcm56990-generic-l3.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_term_rtag7_trunk_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
 ...
 *  Summary:
 *  ========
 *  This cint example demonstrate L3 MPLS termination traffic configuration.
 *
 ...
 *
 *    2) Step2 - Configuration (Done in config_l3_mpls_term_rtag7_trunk())
 *    ======================================================
 ...
 *      a) Verify the result after send 4 packets, which UDP L4 Dst port increased step by 1
 *
 *         {000000000001} {002a10777700} 8100 000b
 *         8847 0005 00fe 0020 01fe 4500 003c 6762  pkt with 2 mpls labels 0x50, 0x200
 *         0000 fe11 be66 0a3a 4001 0a3a 4273 0000
 *         550a 0028 6908 6162 6364 6566 6768 696a  L4 UDP Dst Port 21770-11273
 *         6b6c 6d6e 6f70 7172 7374 7576 7761 6263
 *         6465 6667 6869
 *
 *      b) Expected to receive 4 L3 packets with UDP L4 Dst port increased step by 1,
 *         and the packets egressed from trunk member ports, egress_port[0] and
 *         egress_port[1] split evenly.
 *
 *         Packet raw data (78):
 *         {00000000fe02} {000000000002} 8100 000c
 *         0800 4500 003c 6762 0000 fd11 bf66 0a3a  L3 payload SIP:10.58.64.1
 *         4001 0a3a 4273 0000 550a 0028 6908 6162  DIP:10.58.66.115, UDP Dst port 11270-11273
 ...
 *    ==========================================
 *      a) Mostly reverse the configuration, done in clear_l3_mpls_term_rtag7_trunk()
 */
cint_reset();
...
bcm_error_t config_l3mpls_term_rtag7(int unit)
{
  int flags;
  bcm_error_t rv = BCM_E_NONE;

  flags = BCM_HASH_FIELD_SRCPORT|
		  ...
		  BCM_HASH_FIELD_DSTL4|
		  BCM_HASH_FIELD_SRCL4;
  rv = bcm_switch_control_set(unit, bcmSwitchHashL3MPLSField0, flags);
  if (BCM_FAILURE(rv)) {
    printf("Error executing bcm_switch_control_set(): %s.\n", bcm_errmsg(rv));
    return rv;
  }

  rv = bcm_switch_control_set(unit, bcmSwitchHashL3MPLSField1, flags);
  if (BCM_FAILURE(rv)) {
    printf("Error executing bcm_switch_control_set(): %s.\n", bcm_errmsg(rv));
    return rv;
  }
...
bcm_error_t config_l3_mpls_term_rtag7_trunk(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
    ...
    rv = config_l3mpls_term_rtag7(unit);
    if (BCM_FAILURE(rv)) {
        printf("\nError in config_l3mpls_term_rtag7(): %s.\n",bcm_errmsg(rv));
        return rv;
    }
...
bcm_error_t clear_l3_mpls_term_rtag7_trunk(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
...
check_flexctr(int unit, uint32 stat_counter_id, int *flexctr_check_failed)
{
    ...
    if (COMPILER_64_LO(counter_value[egress_port[i]].value[0]) != 2) {
...
bcm_error_t test_verify(int unit)
{
    ...
    printf("Transmiting mpls packets into ingress_port:%d\n", ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000001002A107777008100000B8847000500FE002001FE4500003C67620000FE11BE660A3A40010A3A42730000550A002869086162636465666768696A6B6C6D6E6F7071727374757677616263646566676869; sleep 1", ingress_port);
    bshell(unit, str);

    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000001002A107777008100000B8847000500FE002001FE4500003C67620000FE11BE660A3A40010A3A42730000550B002869076162636465666768696A6B6C6D6E6F7071727374757677616263646566676869; sleep 1", ingress_port);
    bshell(unit, str);

    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000001002A107777008100000B8847000500FE002001FE4500003C67620000FE11BE660A3A40010A3A42730000550C002869066162636465666768696A6B6C6D6E6F7071727374757677616263646566676869; sleep 1", ingress_port);
    bshell(unit, str);

    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000001002A107777008100000B8847000500FE002001FE4500003C67620000FE11BE660A3A40010A3A42730000550D002869056162636465666768696A6B6C6D6E6F7071727374757677616263646566676869; sleep 1", ingress_port);
    bshell(unit, str);
    ...
    printf("\n------------------------------------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL MPLS LABEL TERMINATION TO TRUNK with RTAG7 HASH VERIFICATION  ");
    printf("\n------------------------------------------------------------------------------------- \n");
...
bcm_error_t test_cleanup(int unit)
{
	...
	rv = clear_l3_mpls_term_rtag7_trunk(unit);
    if (BCM_FAILURE(rv)) {
        printf("clear_l3_mpls_term_rtag7_trunk() failed.\n");
        return rv;
	}
 ...
 *  a) test setup
 *  b) demonstrates the deployment for l3 mpls configure sequence(Done in config_l3_mpls_term_rtag7_trunk())
...
execute(void)
{
    ...
    print "~~~ #2) config_l3_mpls_term_rtag7_trunk(): ** start **";
    if (BCM_FAILURE((rv = config_l3_mpls_term_rtag7_trunk(unit)))) {
        printf("config_l3_mpls_term_rtag7_trunk() failed.\n");
        return rv;
    }
    print "~~~ #2) config_l3_mpls_term_rtag7_trunk(): ** end **";
