/*  Feature  : L2 l2 multicast
 *
 *  Usage    : BCM.0> cint th4_hsdk_l2_multicast.c
 *
 *  config   : th4_hsdk_l2_config.yml
 *
 *  Log file : th4_hsdk_l2_multicast.log
 *  
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *       (1.2) create a L2 MC group with 2 egress ports
 *       (1.3) add a L2 entry <vlan, M-MAC> pointing to the created L2 MC group
 * 
 ...
 *    ======================================================
 *      (3.1) Send a MC packet destined to <vlan, M-MAC> and make sure the packet is 
 *            flooded to the two ports configured in the L2 MC group
 *
 *      Expected Result:
 *      ================
...
int testSetup(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egress_port1, &egress_gport1));
    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egress_port2, &egress_gport2));
    BCM_IF_ERROR_RETURN(bcm_multicast_egress_add(unit, l2mc_group, egress_gport1, -1));
    BCM_IF_ERROR_RETURN(bcm_multicast_egress_add(unit, l2mc_group, egress_gport2, -1));

    /* Add a L2 multicast address on front panel ports */
...
int testCleanup(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egress_port1, &egress_gport1));
    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egress_port2, &egress_gport2));
    BCM_IF_ERROR_RETURN(bcm_multicast_egress_delete(unit, l2mc_group, egress_gport1, -1));
    BCM_IF_ERROR_RETURN(bcm_multicast_egress_delete(unit, l2mc_group, egress_gport2, -1));
...
unsigned char expected_egress_packet1[64] = {
        0x01, 0x00, 0x00, 0xbb, 0xbb, 0xbb, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x81, 0x00,
...
bcm_error_t testVerify(int unit)
{
    ...
    printf("Verify MC pkt from port %d forwarding to port %d and port %d\n", ingress_port, egress_port1, egress_port2);
    snprintf(str, 512, "tx 1 pbm=%d data=0x010000bbbbbb00000000001181000002080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 1", ingress_port);
    bshell(unit, str);
