 ...
 * Feature  : L3 routing
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_src_ip_classid_ifp.c
 *
 * config   : th4_hsdk_l3_src_ip_classid_ifp_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_src_ip_classid_ifp_log.txt
 *
 * Test Topology :
 *                      +--------------------+
 *                      |                    |
 ...
 * Summary:
 * ========
 * cint script to demonstrate L3 source IP lookup [l3_alpm_template: 6 in config
 * for parallel mode with SIP lookup]. In this script, two SIPs are assigned the
 * same class_id to be looked up in IFP. In IFP, both the packets are forwarded
 * by L3Switch action. Packets with diferrent SIPs that need the same treatement
 * can be assigned the same class id with this SIP lookup mode and IFP can be
 * used to lookup and take the needed action.
 *
 ...
 *     c) The configurations of vlan, L3_IIF, egress intf and next hop are done
 *        to forward the packet.
 *     d) Configure a basic IPv4 route for SIP lookup and assign the class id.
 *     e) Configure IFP to match on the class id assigned by SIP lookup and
 *        forward the packet to the nexthop configured in step c.
 *
 ...
 *        'l3 route show'
 *     b) Transmit the IPv4 unicast packet. The packet used matches the route
...
int num_sip_entries = 2;
bcm_vlan_t ing_vid[num_sip_entries] = {2, 10};
...
bcm_vrf_t vrf = 100;
bcm_ip_t sip[num_sip_entries] = { 0x0A0A0A2A, 0xAC100202};
...
unsigned char expected_egress_packet_2[64] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x81, 0x00, 0x00, 0x1E,
        0x08, 0x00, 0x45, 0x00, 0x00, 0x2A, 0x00, 0x00, 0x00, 0x00, 0x3F, 0xFF, 0x8c, 0x82, 0xAC, 0x10,
        0x02, 0x02, 0x20, 0x20, 0x20, 0x21, 0x00, 0x01, 0x02, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 0xFE, 0x85, 0x79
};
...
pktio_Rx_call_back(int unit, bcm_pktio_pkt_t * packet, void *cookie)
{
	...
	if ((sal_memcmp(buffer, expected_egress_packet_1, length) == 0) || (sal_memcmp(buffer, expected_egress_packet_2, length) == 0)) {
...
configure_l3_routing(int unit)
{
	...
	for (i=0; i<num_sip_entries; i++) {
		rv = create_vlan_add_port(unit, ing_vid[i], ingress_port);
		if (BCM_FAILURE(rv)) {
			printf("Error in configuring ing vlan %s\n", bcm_errmsg(rv));
			return rv;
		}
		BCM_IF_ERROR_RETURN(bcm_port_untagged_vlan_set(unit, ingress_port, ing_vid[i]));
	}
	...
	for (i=0; i<num_sip_entries; i++) {
		rv = configure_l3_iif_binding(unit, ing_vid[i], ingress_if);
		if (BCM_FAILURE(rv)) {
...
configure_sip_lookup(int unit)
{
	...
	for (i=0; i<num_sip_entries; i++) {
		...
		route.l3a_flags2 = BCM_L3_FLAGS2_SRC_ROUTE;
		route.l3a_lookup_class = class_id;
		route.l3a_subnet  = sip[i];
		route.l3a_ip_mask = sip_mask;
		route.l3a_vrf     = vrf;
...
configure_ifp(int unit)
{
    ...
    bcm_field_group_config_t_init(&group_config);
    BCM_FIELD_QSET_INIT(group_config.qset);
    BCM_FIELD_QSET_ADD(group_config.qset, bcmFieldQualifySrcClassL3);
    BCM_FIELD_QSET_ADD(group_config.qset, bcmFieldQualifyStageIngress);
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualify_SrcClassL3(unit, entry, class_id, BCM_FIELD_EXACT_MATCH_MASK));

    BCM_IF_ERROR_RETURN(bcm_field_action_add(unit, entry, bcmFieldActionL3Switch, egr_if, 0));
...
configure_flex_counter(int unit)
{
    ...
    rv = bcm_field_entry_flexctr_attach(unit, ifp_entry, &flexctr_cfg);
    if (BCM_FAILURE(rv)) {
...
test_setup(int unit)
{
    ...
    rv = configure_sip_lookup(unit);
    if (BCM_FAILURE(rv)) {
        printf("error in configure_sip_lookup %s\n", bcm_errmsg(rv));
        return rv;
    }
...
test_verify(int unit)
{
    ...
    printf("Sending %d unicast packets to ingress port %d\n", count, ingress_port);
    snprintf(str, 512, "tx %d pbm=%d data=0x0000000000020000000000018100000A08004500002A0000000040FF8B82AC100202202020210001020300000000000000000000000000000000000005fe8579c", count, ingress_port);
    bshell(unit, str);
    sal_sleep(5);
    ...
    if (COMPILER_64_LO(counter_value.value[0]) != (count+count)) {
