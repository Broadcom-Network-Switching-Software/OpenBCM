/*
 * Feature  : IP4in4 tunnel
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_ip_tunnel_ip4in4.c
 *
 * config   : th4_hsdk_l3_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_ip_tunnel_ip4in4_log.txt
 *
 * Test Topology :
 *                      +---------------------+
 *                      |                     |
 ...
 * across a network not supporting the native packet protocol.The Ipv4 tunneling
 * encapsulates incoming IPv4 or IPv6 packets with an IPv4 tunnel header and
 * routes the packets as IPv4 packets across the core. At the end of the tunnel,
 * the XGS switches removes the IPv4 header and forwards the original IP based
 * on its own header.
 *
 ...
 *     c) Configure a IP4in4 tunnel. This adds the route in L3 ALPM table
...
bcm_ip_t tnl_init_outer_sip = 0x0a0a0a0a;
bcm_ip_t tnl_init_outer_dip = 0x09090909;
                            ...
                            bcm_mac_t dst_mac, bcm_l3_intf_t *egr_l3_intf)
{
    ...
    tnl_init_info.type = type; /* Choose Ipv4-in-Ipv4 tunnel type */
                              ...
                              bcm_if_t ingress_if, bcm_port_t port)
{
    ...
    tnl_term_info.dip_mask = bcm_ip_mask_create(28);
...
/*    IP4in4 tunnel configuration function    */

int
configure_ip4in4_tunnel(int unit)
{
    ...
    rv = configure_tunnel_initiation(unit, bcmTunnelTypeIp4In4, ttl,
    ...
    rv = configure_tunnel_termination(unit, bcmTunnelTypeIp4In4, BCM_L3_VRF_DEFAULT,
...
/*
 *    Configures the IP4in4 tunnel and the required set up to verify it
 */
...
test_setup(int unit)
{
    ...
    rv = configure_ip4in4_tunnel(unit);
    if (BCM_FAILURE(rv)) {
        printf("Error in configure_ip4in4_tunnel %s\n", bcm_errmsg(rv));
        return rv;
    }
...
test_verify(int unit)
{
    ...
    printf("Sending %d unicast packets to ingress port %d\n", count, port_network);
    snprintf(str, 512, "tx %d pbm=%d data=0x000000002222000000000002810000160800450000420000000040045493090909090A0A0A0A4500002E0000000040FF73CC010101010202020200000000000000000000000000000000000000000000000000008856EEF7", count, port_network);
    bshell(unit, str);
    sal_sleep(2);
