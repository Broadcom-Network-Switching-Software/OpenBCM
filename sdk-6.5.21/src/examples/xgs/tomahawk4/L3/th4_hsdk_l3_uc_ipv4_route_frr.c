/*
 * Feature  : L3 routing with primary and failover path
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_uc_ipv4_route_frr.c
 *
 * config   : th4_hsdk_l3_uc_ipv4_frr_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_uc_ipv4_route_frr_log.txt
 *
 * Test Topology :
 *                      +--------------------+
 *                      |                    |
 ...
 * mode with fail over path configurations.
 * [l3_alpm_template: 2
 ...
 *     c) Configure a IPv4 route with failover configurations. This adds the
 * 	  route in L3 ALPM table and does the necessary configurations of vlan,
 ...
 *        'l3 route show'
 *     b) Transmit the IPv4 unicast packet. The packet used matches the
...
configure_l3_routing(int unit)
{
    ...
    rv = configure_route(unit, dip, dip_mask, egr_if_primary, vrf);
    if (BCM_FAILURE(rv)) {
...
/*
 *    Configures the IPV4 unicast route with primary and failover paths and the
 *    required set up to verify it
 */
