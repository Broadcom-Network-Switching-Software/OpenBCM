/*
 * Feature  : Hierarchical ECMP routing with host entry
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_hecmp_ipv4_host.c
 *
 * config   : th4_hsdk_l3_hecmp_ipv4_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_hecmp_ipv4_host_log.txt
 *
 * Test Topology :
 *                      +--------------------+
 *                      |                    |
 ...
 * Summary:
 * ========
 * cint script to demonstrate hierarchical ECMP forwarding in parallel mode with
 * host entry
 * [l3_alpm_template: 2
 ...
 *     c) Configure a IPv4 hierarchical ECMP host. This does the necessary
...
config_packet_spray(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, ingress_port, bcmPortControlECMPLevel1LoadBalancingRandomizer, 7));
    BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, ingress_port, bcmPortControlECMPLevel2LoadBalancingRandomizer, 9));
...
/*    Adding an ECMP host entry    */

int
configure_host(int unit, bcm_ip_t ip_addr, bcm_vrf_t vrf)
{
    bcm_error_t  rv;
    bcm_l3_host_t host;
    ...
    host.l3a_intf = ecmp_if_over_lay;
    host.l3a_vrf = vrf;
...
configure_hecmp(int unit)
{
    ...
    rv = configure_host(unit, dip, vrf);
    if (BCM_FAILURE(rv)) {
...
/*
 *    Configures the IPV4 ECMP host and the required set up to verify it
 */
