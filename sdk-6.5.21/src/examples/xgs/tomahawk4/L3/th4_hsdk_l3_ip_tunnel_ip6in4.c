/*
 * Feature  : IP6in4 tunnel
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_ip_tunnel_ip6in4.c
 *
 * config   : th4_hsdk_l3_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_ip_tunnel_ip6in4_log.txt
 *
 * Test Topology :
 *                      +-----------------------------+
 *                      |                             |
 ...
 *      src_mac-0x01    |init_dip - 2002:c0a8:0101::00|  src_mac-0x02
 *                      |term_dip - 2002:c0a8:0201::00|
 *                      +-----------------------------+
 *
 *
 * Summary:
 ...
 * encapsulates incoming IPv6 packets with an IPv4 tunnel header and routes the
 * packets as IPv4 packets across the core. At the end of the tunnel, the XGS
 * switches removes the IPv4 header and forwards the original IP6 packet based
 * on its header.
 *
 ...
 *     c) Configure a IP6in4 tunnel. This adds the route in L3 ALPM table
...
bcm_ip_t tnl_init_outer_dip = 0xc0a80101;
bcm_ip_t tnl_init_outer_sip = 0xc0a80201;
...
/*    IP6in4 tunnel configuration function    */

int
configure_ip6in4_tunnel(int unit)
{
    ...
    rv = configure_tunnel_initiation(unit, bcmTunnelTypeIp6In4, ttl,
    ...
    rv = configure_tunnel_termination(unit, bcmTunnelTypeIp6In4, vrf,
    ...
    rv = configure_route(unit, tnl_init_dip6, mask6, egr_if_network, vrf);
    if (BCM_FAILURE(rv)) {
    ...
    rv = configure_route(unit, tnl_term_dip6, mask6, egr_if_access, vrf);
    if (BCM_FAILURE(rv)) {
        printf("Error in configure_route for tunnel initiation %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
configure_flex_counter_tunnel_terminator(int unit)
{
    ...
    int tnl_term_flags = BCM_TUNNEL_TERM_USE_OUTER_DSCP |
                         ...
                         BCM_TUNNEL_TERM_KEEP_INNER_DSCP;
    tnl_term_info.flags = tnl_term_flags;
    tnl_term_info.type = bcmTunnelTypeIp6In4; /* Choose Ipv4-in-Ipv4 tunnel type */
    tnl_term_info.vrf = vrf;
...
/*
 *    Configures the IP6in4 tunnel and the required set up to verify it
 */
...
test_setup(int unit)
{
    ...
    rv = configure_ip6in4_tunnel(unit);
    if (BCM_FAILURE(rv)) {
        printf("Error in configure_ip6in4_tunnel %s\n", bcm_errmsg(rv));
        return rv;
    }
...
test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx %d pbm=%d data=0x0000000011110000000000018100001586DD6030000000063BFF2002C0A80201000000000000000000002002C0A8010100000000000000000000000000000000D7388CF0", count, port_access);
    bshell(unit, str);
    sal_sleep(2);
    ...
    printf("Sending %d unicast packets to ingress port %d\n", count, port_network);
    snprintf(str, 512, "tx %d pbm=%d data=0x00000000222200000000000281000016080045000042000000004029F640C0A80101C0A802016030000000063BFF2002C0A80101000000000000000000002002C0A80201000000000000000000000000000000004B3E8E53", count, port_network);
    bshell(unit, str);
    sal_sleep(2);
