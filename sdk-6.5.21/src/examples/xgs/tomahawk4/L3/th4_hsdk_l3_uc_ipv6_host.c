 ...
 * Feature  : L3 routing
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_uc_ipv6_host.c
 *
 * config   : th4_hsdk_l3_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_uc_ipv6_host_log.txt
 *
 * Test Topology :
 *                      +--------------------+
 *                      |                    |
 ...
 * Summary:
 * ========
 * cint script to demonstrate L3 forwarding using host APIs in parallel mode
 * [l3_alpm_template: 2 in config file]. In Tomahawk4, there is no separate host
 * table. Both routes and hosts use ALPM DB.
 *
 ...
 *     c) Configure a basic IPv6 host. This adds the host in L3 ALPM table
 ...
 *        'l3 host show'
 *     b) Transmit the IPv6 unicast packet. The packet used matches the host
...
int
configure_host(int unit, bcm_ip6_t subnet_addr, bcm_if_t egr_if, bcm_vrf_t vrf)
{
    bcm_error_t  rv;
    bcm_l3_host_t host;
...
/*    L3 host configuration function    */
...
/*
 *    Configures the IPV6 unicast host and the required set up to verify it
 */
...
test_verify(int unit)
{
    ...
    /* print counter value. */
    printf("Routed through IPv6 host : %d packets / %d bytes\n", COMPILER_64_LO(counter_value.value[0]),
