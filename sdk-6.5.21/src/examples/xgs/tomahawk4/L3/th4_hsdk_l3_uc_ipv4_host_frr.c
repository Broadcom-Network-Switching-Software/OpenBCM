/*
 * Feature  : L3 host entry with primary and failover path
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_uc_ipv4_host_frr.c
 *
 * config   : th4_hsdk_l3_uc_ipv4_frr_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_uc_ipv4_host_frr_log.txt
 *
 * Test Topology :
 *                      +--------------------+
 *                      |                    |
 ...
 *        --------->----+  Routing           +------->------
 *             vlan 2   |    ALPM            |  vlan_primary 30, vlan_failover 40
 *        dst_mac-0x02  |    vrf -100        |  dst_mac_primary-0x22, dst_mac_failover-0x44
 *                      |    dip - 0xC0A80114|  src_mac_primary-0x11, src_mac_failover-0x33
 *                      |                    |
 *                      +--------------------+
 *
 *
 * Summary:
 * ========
 * cint script to demonstrate L3 forwarding through host entry configuration in
 * parallel mode with fail over path configurations.
 * [l3_alpm_template: 2
 *  failover_fixed_nh_offset_enable : 1 in config file].
 *
 ...
 *        three ports, one port is used as ingress_port, the second one as
 *        primary egress_port and the port as failover egress_port .
 *     b) Install an IFP rule to copy egress packets to CPU and also drop them.
 *        start packet watcher.
 *     c) Configure an IPv4 host with failover configurations. This adds an host
 *	  entry in L3 ALPM table and does the necessary configurations of vlan,
 *	  L3_IIF, egress intf and next hop for both primary and backup/failover
 *	  paths.
 *
 ...
 *        'l3 host show'
 *     b) Transmit the IPv4 unicast packet. The packet used matches the host
 *        entry configured through script.
 *     c) Expected Result:
 *     ===================
 *       1. We see that dmac, smac, TTL and vlan are all changed as the packet
 *          gets routed through the egress port with primart path configurations.
 *          Now enable the failover path and observe the packet egressing through
...
bcm_port_t egress_port_primary;
bcm_port_t egress_port_failover;
bcm_port_t ingress_port;
bcm_vlan_t ing_vid = 2;
bcm_vlan_t egr_vid_primary = 30;
bcm_vlan_t egr_vid_failover = 40;
bcm_mac_t src_mac_primary = {0x00, 0x00, 0x00, 0x0, 0x00, 0x11};
bcm_mac_t dst_mac_primary = {0x00, 0x00, 0x00, 0x0, 0x00, 0x22};
bcm_mac_t src_mac_failover = {0x00, 0x00, 0x00, 0x0, 0x00, 0x33};
bcm_mac_t dst_mac_failover = {0x00, 0x00, 0x00, 0x0, 0x00, 0x44};
bcm_mac_t router_mac = {0x00, 0x00, 0x00, 0x00, 0x00, 0x02};
bcm_if_t ingress_if = 100;
bcm_vrf_t vrf = 100;
bcm_ip_t dip = 0xC0A80114;
bcm_if_t egr_if_primary;
...
unsigned char expected_egress_packet_2[64] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x44, 0x00, 0x00, 0x00, 0x00, 0x00, 0x33, 0x81, 0x00, 0x00, 0x28,
        0x08, 0x00, 0x45, 0x00, 0x00, 0x2A, 0x00, 0x00, 0x00, 0x00, 0x3F, 0xFF, 0xA4, 0xE5, 0x0A, 0x0A,
        0x0A, 0x2A, 0xC0, 0xA8, 0x01, 0x14, 0x00, 0x01, 0x02, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x4D, 0x7B, 0xD6
};
...
pktio_Rx_call_back(int unit, bcm_pktio_pkt_t * packet, void *cookie)
{
    ...
    if((egress_port_primary == src_port) || (egress_port_failover == src_port))  {
       	result_port_check = 0;
    }
...
int
create_egr_obj(int unit, bcm_if_t l3_if, bcm_mac_t mac, bcm_gport_t gport, int failover)
{
    bcm_error_t rv;
    ...
    if(!failover) {
        rv = bcm_l3_egress_create(unit, 0, &l3_egr, &egr_if_primary);
        if (BCM_FAILURE(rv)) {
	    printf("Error in configuring L3 egress object %s.\n", bcm_errmsg(rv));
	    return rv;
        }
    } else {
	rv = bcm_failover_egress_set(unit, egr_if_primary, &l3_egr);
	if (BCM_FAILURE(rv)) {
            printf("Error in bcm_failover_egress_set %s.\n", bcm_errmsg(rv));
	    return rv;
	 }
    }
...
/*    Adding host entry to ALPM table    */

int
configure_host(int unit, bcm_ip_t ip_addr, bcm_if_t egr_if, bcm_vrf_t vrf)
{
    bcm_error_t  rv;
    bcm_l3_host_t host;
...
/*    Primary path configurations    */

int
configure_primary_path(int unit)
{
    bcm_error_t rv;
    bcm_if_t egr_l3_if;
    bcm_gport_t egress_gport;

    rv = create_vlan_add_port(unit, egr_vid_primary, egress_port_primary);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring egr vlan %s\n", bcm_errmsg(rv));
        return rv;
    }

    BCM_IF_ERROR_RETURN(bcm_port_untagged_vlan_set(unit, egress_port_primary, egr_vid_primary));

    /* Create L3 egress interface */
    rv = create_l3_intf(unit, src_mac_primary, egr_vid_primary, &egr_l3_if);
    if (BCM_FAILURE(rv)) {
        printf("Error in create_l3_intf %s.\n", bcm_errmsg(rv));
        return rv;
    }

    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egress_port_primary, &egress_gport));

    /* Create L3 egress object */
    rv = create_egr_obj(unit, egr_l3_if, dst_mac_primary, egress_gport, 0);
    if (BCM_FAILURE(rv)) {
...
/*    Failover path configurations    */

int
configure_failover_path(int unit)
{
    bcm_error_t rv;
    bcm_if_t egr_l3_if;
    bcm_gport_t egress_gport;

    rv = create_vlan_add_port(unit, egr_vid_failover, egress_port_failover);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring egr vlan %s\n", bcm_errmsg(rv));
        return rv;
    }

    BCM_IF_ERROR_RETURN(bcm_port_untagged_vlan_set(unit, egress_port_failover, egr_vid_failover));

    /* Create L3 egress interface */
    rv = create_l3_intf(unit, src_mac_failover, egr_vid_failover, &egr_l3_if);
    if (BCM_FAILURE(rv)) {
        printf("Error in create_l3_intf %s.\n", bcm_errmsg(rv));
        return rv;
    }

    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egress_port_failover, &egress_gport));

    /* Create L3 egress object */
    rv = create_egr_obj(unit, egr_l3_if, dst_mac_failover, egress_gport, 1);
    if (BCM_FAILURE(rv)) {
        printf("Error in create_egr_obj %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
/*    L3 host configuration    */

int
configure_l3_host_frr(int unit)
{
    bcm_error_t rv;
    ...
    rv = configure_primary_path(unit);
    if (BCM_FAILURE(rv)) {
        printf("Error in configure_primary_path %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Configure failover path */
    rv = configure_failover_path(unit);
    if (BCM_FAILURE(rv)) {
        printf("Error in configure_failover_path %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    /* Program host in ALPM table */
    rv = configure_host(unit, dip, egr_if_primary, vrf);
    if (BCM_FAILURE(rv)) {
        printf("Error in configure_host %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
/* Create flex counter action with L3 Host table as source and user set index */
...
configure_flex_counter(int unit)
{
    ...
    action.flags = 0;
    action.source = bcmFlexctrSourceL3Host;
    ...
    host.l3a_intf = egr_if_primary;
    host.l3a_vrf = vrf;

    /* Attach counter action to host */
    rv = bcm_l3_host_stat_attach(unit, &host, stat_counter_id);
    if (BCM_FAILURE (rv)) {
        printf ("Error in bcm_l3_host_stat_attach %s\n", bcm_errmsg (rv));
        return rv;
    }

    /* Set the counter index */
    rv = bcm_l3_host_flexctr_object_set(unit, &host, flexctr_obj_val);
    if (BCM_FAILURE (rv)) {
        printf ("Error in bcm_l3_host_flexctr_object_set %s\n", bcm_errmsg (rv));
        return rv;
    }
...
/*
 *    Configures the IPV4 unicast host and the required set up to verify it
 */
...
test_setup(int unit)
{
    ...
    egress_port_primary = port_list[1];
    egress_port_failover = port_list[2];
    ...
    BCM_PBMP_CLEAR(pbmp);
    BCM_PBMP_PORT_ADD(pbmp, egress_port_primary);
    BCM_PBMP_PORT_ADD(pbmp, egress_port_failover);
...
test_verify(int unit)
{
	...
	/* Switch over from primary to failover path */
    printf("\nSwitch-over from primary-path to failover/protection-path\n");
    rv = bcm_failover_egress_status_set(unit, egr_if_primary, 1);
    if (BCM_FAILURE(rv)) {
	...
	/* Switch back to primary from failover path */
    printf("\nSwitch-over from failover/protection-path to primary-path\n");
    rv = bcm_failover_egress_status_set(unit, egr_if_primary, 0);
    if (BCM_FAILURE(rv)) {
        printf("Error executing bcm_failover_egress_status_set(): %s.\n", bcm_errmsg(rv));
        return rv;
    }
