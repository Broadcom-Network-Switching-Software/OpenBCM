/*
 * Feature  : IP6in6 tunnel
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_ip_tunnel_ip6in6.c
 *
 * config   : th4_hsdk_l3_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_ip_tunnel_ip6in6_log.txt
 *
 * Test Topology :
 *                      +---------------------+
 *                      |                     |
 ...
 *      src_mac-0x01    |init_dip - AA00::01  |  src_mac-0x02
 *                      |term_dip - BB00::01  |
 *                      +---------------------+
 *
 *
 ...
 * encapsulates incoming IPv4 or IPv6 packets with an IPv6 tunnel header and
 * routes the packets as IPv6 packets across the core. At the end of the tunnel,
 * the XGS switches removes the IPv6 header and forwards the original IP based
 * on its own header.
 *
 ...
 *     c) Configure a IP6in6 tunnel. This adds the route in L3 ALPM table
 ...
 *        'l3 route show'
 *     b) Transmit the IPv6 packet. The packet used matches the configurations
 *        done through script.
 *     c) Expected Result:
 *     ===================
...
/*    IP6in6 tunnel configuration function    */

int
configure_ip6in6_tunnel(int unit)
{
    ...
    rv = configure_tunnel_initiation(unit, bcmTunnelTypeIp6In6, ttl,
    ...
    rv = configure_tunnel_termination(unit, bcmTunnelTypeIp6In6, vrf,
...
/*
 *    Configures the IP6in6 tunnel and the required set up to verify it
 */
...
test_setup(int unit)
{
    ...
    rv = configure_ip6in6_tunnel(unit);
    if (BCM_FAILURE(rv)) {
        printf("Error in configure_ip6in6_tunnel %s\n", bcm_errmsg(rv));
        return rv;
    }
...
test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx %d pbm=%d data=0x0000000011110000000000018100001586DD6030000000063BFFBB000000000000000000000000000001AA000000000000000000000000000001000000000000114C3F26", count, port_access);
    bshell(unit, str);
    sal_sleep(2);
    ...
    printf("Sending %d unicast packets to ingress port %d\n", count, port_network);
    snprintf(str, 512, "tx %d pbm=%d data=0x0000000022220000000000028100001686DD60300000002E29FFEE000000000000000000000000000001FF0000000000000000000000000000016030000000063BFFAA000000000000000000000000000001BB00000000000000000000000000000100000000000000000000CD51FB06", count, port_network);
    bshell(unit, str);
    sal_sleep(2);
