/*
 * Feature  : L3 routing to IPGRE tunnel IPv4inIPv6
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_ip_tunnel_ipgre_4in6.c
 *
 * config   : th4_hsdk_l3_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_ip_tunnel_ipgre_4in6_log.txt
 *
 * Test Topology :
 *                      +---------------------+
 *                      |                     |
...
int
configure_ipgre4in6_tunnel(int unit)
{
    bcm_error_t rv = BCM_E_NONE;
    ...
    rv = configure_tunnel_initiation(unit, bcmTunnelTypeGre4In6, ttl,
    ...
    rv = configure_tunnel_termination(unit, bcmTunnelTypeGre4In6, BCM_L3_VRF_DEFAULT,
...
configure_flex_counter_tunnel_terminator(int unit)
{
    ...
    int tnl_term_flags = BCM_TUNNEL_TERM_USE_OUTER_DSCP |
                         ...
                         BCM_TUNNEL_TERM_KEEP_INNER_DSCP;
    tnl_term_info.flags = tnl_term_flags;
    tnl_term_info.type = bcmTunnelTypeGre4In6;
...
test_setup(int unit)
{
    ...
    rv = configure_ipgre4in6_tunnel(unit);
    if (BCM_FAILURE(rv)) {
        printf("Error in configure_ipgre4in6_tunnel %s\n", bcm_errmsg(rv));
        return rv;
    }
...
test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx %d pbm=%d data=0x0000000011110000000000018100001508004500002E0000000040FF73CC0202020201010101000102030405060708090A0B0C0D0E0F10111213141516171819FB82E0B2; sleep quiet 1", count, port_access);
    bshell(unit, str);
	sal_sleep(2);

	printf("Sending %d unicast packets to ingress port %d\n", count, port_network);
    snprintf(str, 512, "tx %d pbm=%d data=0x0000000022220000000000028100001686DD6030000000182FFFCC000000000000000000000000000001DD00000000000000000000000000000100000800450000140000000040FF73E601010101020202029D9CB928; sleep quiet 1", count, port_network);
    bshell(unit, str);
    sal_sleep(2);
    ...
    printf("\n-------------------------------------------- \n");
    printf("    RESULT OF L3 ROUTE to IPGRE 4in6 TUNNEL VERIFICATION  ");
    printf("\n-------------------------------------------- \n");
