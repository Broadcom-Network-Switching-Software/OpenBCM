/*
 * Feature  : ECMP routing using host APIs
 *
 * Usage    : BCM.0> cint th4_hsdk_l3_ecmp_ipv4_host.c
 *
 * config   : th4_hsdk_l3_64x400.config.yml
 *
 * Log file : th4_hsdk_l3_ecmp_ipv4_host_log.txt
 *
 * Test Topology :
 *                      +--------------------+
 *                      |                    |
 ...
 *     c) Configure a IPv4 ECMP host. This adds the host in L3 ALPM table
 ...
 *     b) Transmit the IPv4 packet. The packet used matches the ECMP host entry
 *        configured through script.
 *     c) Expected Result:
 *     ===================
 *       We see that dmac, smac, TTL and vlan are all changed as the packet gets
 *       forwarded through the egress ports and the number of packets sent
 *       should match the flex ctr stat get output
 */
...
/*    Adding a host entry to ALPM table    */
...
configure_ecmp(int unit)
{
    ...
    /* Program a host entry in ALPM table */
