/*  Feature  : L3 MPLS tunnel termination exp to ipecn mapping
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_term_exp2ecn.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_term_exp2ecn_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *  ========
 *  bcm_l3_route_add()
 *  bcm_ecn_map_create()/bcm_ecn_map_set()
 ...
 *         0800 4503 003c 6762 0000 fd01 bf73 0a3a   //ecn=3
...
int ingress_map_id, ingress_map_id_2, ingress_map_id_3;
...
0x08, 0x00, 0x45, 0x03, 0x00, 0x3c, 0x67, 0x62, 0x00, 0x00, 0xfd, 0x01, 0xbf, 0x73, 0x0a, 0x3a,
...
bcm_error_t config_exp2ipecn_mapping(int unit)
{
    ...
    rv = bcm_ecn_map_create(unit, BCM_ECN_MAP_INGRESS | BCM_ECN_MAP_MPLS, &ingress_map_id);
    if (BCM_FAILURE(rv)) {
        printf("bcm_ecn_map_create() %s\n", bcm_errmsg(rv));
        return rv;
    }
    print ingress_map_id;

    bcm_ecn_map_t_init(&ecn_map);
    ecn_map.action_flags = 0;
    ecn_map.ecn = 0;
    ecn_map.exp = 3;
    ecn_map.new_ecn = 2;
    rv = bcm_ecn_map_set(unit, options, ingress_map_id, &ecn_map);
    if (BCM_FAILURE(rv)) {
        printf("bcm_ecn_map_set() %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_ecn_map_create(0, BCM_ECN_MAP_INGRESS | BCM_ECN_MAP_MPLS, &ingress_map_id_2);
    if (BCM_FAILURE(rv)) {
        printf("bcm_ecn_map_create() %s\n", bcm_errmsg(rv));
        return rv;
    }
    print ingress_map_id_2;
    ...
    ecn_map.ecn = 2;
    ecn_map.new_ecn = 3;
    rv = bcm_ecn_map_set(unit, options, ingress_map_id_2, &ecn_map);


    rv = bcm_ecn_map_create(unit, BCM_ECN_MAP_INGRESS | BCM_ECN_MAP_TUNNEL_TERM, &ingress_map_id_3);
    if (BCM_FAILURE(rv)) {
        printf("bcm_ecn_map_create() %s\n", bcm_errmsg(rv));
        return rv;
    }
    print ingress_map_id_3;
    ...
    ecn_map.ecn = 3;
    ecn_map.new_ecn = 3;
    rv = bcm_ecn_map_set(unit, options, ingress_map_id_3, &ecn_map);
...
bcm_error_t config_l3_mpls_term(int unit)
{
    ...
    rv = config_exp2ipecn_mapping(unit);
    if (BCM_FAILURE(rv)) {
        printf("config_exp2ipecn_mapping() %s\n", bcm_errmsg(rv));
        return rv;
    }
	...
	switch_info.flags     |= BCM_MPLS_SWITCH_INGRESS_ECN_MAP | BCM_MPLS_SWITCH_TUNNEL_TERM_ECN_MAP;
	...
	switch_info.ecn_map_id = ingress_map_id;
	switch_info.tunnel_term_ecn_map_id = ingress_map_id_3;
    ...
    switch_info.action     = BCM_MPLS_SWITCH_ACTION_POP;
    switch_info.ingress_if = ingress_if;
	switch_info.ecn_map_id = ingress_map_id_2;
	switch_info.tunnel_term_ecn_map_id = ingress_map_id_3;
    rv = bcm_mpls_tunnel_switch_add(unit, &switch_info);
    if (BCM_FAILURE(rv)) {
       printf("Error in bcm_mpls_tunnel_switch_add(): %s.\n", bcm_errmsg(rv));
...
bcm_error_t test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000001002a107777008100000b8847000506fe002007ff4501003c67620000fe01be750a3a40010a3a42730000550a000100516162636465666768696a6b6c6d6e6f7071727374757677616263646566676869; sleep quiet 1", ingress_port);
    bshell(unit, str);
    ...
    printf("\n---------------------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL TERMINATION with EXP2ECN MAPPING VERIFICATION  ");
    printf("\n---------------------------------------------------------------------- \n");
