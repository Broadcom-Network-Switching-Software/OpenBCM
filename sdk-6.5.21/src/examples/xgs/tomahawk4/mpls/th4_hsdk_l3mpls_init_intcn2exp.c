/*  Feature  : L3 MPLS tunnel initiation int_cn to exp mapping
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_init_intcn2exp.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_init_intcn2exp_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *         8847 0020 0eff 0020 01fe 4500 003c 6762
...
0x88, 0x47, 0x00, 0x20, 0x0e, 0xff, 0x00, 0x20, 0x01, 0xfe, 0x45, 0x00, 0x00, 0x3c, 0x67, 0x62,
...
create_l3_ingress_interface(int unit, bcm_vrf_t vrf, bcm_if_t *ingress_if)
{
    ...
    l3_ingress.vrf = vrf;
    l3_ingress.flags |= BCM_L3_INGRESS_DSCP_TRUST;
	l3_ingress.qos_map_id = ing_qos_id;
...
/* L3 mpls qos ecn configuration */
bcm_error_t
config_l3_mpls_qos_ecn(int unit, int *ing_qos, int *egr_qos)
{
    ...
    bcm_error_t                     rv = BCM_E_NONE;
    uint32                          flags;
    /* Set the DSCP to internal priority mapping profile */
    int                dscp[8] = {0, 1, 2, 3, 4, 5, 6, 7};
    int                in_pri[8] = {6, 1, 2, 3, 4, 5, 6, 7};
    ...
    rv = bcm_qos_map_create(unit, BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_L3,
                            &ing_qos_id);
    if (BCM_FAILURE(rv)) {
    ...
    *ing_qos = ing_qos_id;
    ...
    for (i = 0; i < 8; i++) {
        ...
        ing_qos_map.dscp= dscp[i];
        ing_qos_map.int_pri = in_pri[i];
        ing_qos_map.color = bcmColorGreen;
        rv = bcm_qos_map_add(unit, BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_L3,
...
bcm_error_t
clear_l3_mpls_init_qos_mapping(int unit, int ing_qos, int egr_qos)
{
    bcm_error_t rv = BCM_E_NONE;
...
config_l3_mpls_init(int unit)
{
	...
	bcm_mpls_label_t tunnel_label_init = 0x200;
	uint8 ttl = 255;

    rv = config_l3_mpls_qos_ecn(unit, &ing_qos_id, &egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in config_l3_mpls_qos_ecn(): %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    /* Create L3 egress object */
    rv = create_egr_obj_with_route_label(unit, l3_intf_id, dst_mac, egress_port, tunnel_label_init, &egr_obj_id);
    if (BCM_FAILURE(rv)) {
	...
	label_array[0].flags = BCM_MPLS_EGRESS_LABEL_TTL_SET | BCM_MPLS_EGRESS_LABEL_EXP_REMARK;
	label_array[0].qos_map_id = egr_qos_id;
...
bcm_error_t clear_l3_mpls_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(clear_l3_mpls_init_qos_mapping(unit, ing_qos_id, egr_qos_id));
...
void test_verify(int unit)
{
    ...
    printf("\n------------------------------------------------------------------------------ \n");
    printf("    RESULT OF L3MPLS TUNNEL MPLS TUNNEL INITIATION INTCN2EXP MAPPING VERIFICATION  ");
    printf("\n------------------------------------------------------------------------------ \n");
