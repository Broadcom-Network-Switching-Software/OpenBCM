 ...
 *
 * Usage    : BCM.0> cint th4_hsdk_l3mpls_lsr_swap.c
 *
 * config   : bcm56990-generic-l3mpls.config.yml
 *
 * Log file : th4_hsdk_l3mpls_lsr_swap_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *   and push operation using BCM APIs.
 *
 ...
 *
 *   2) Step2 - Configuration (Done in config_lsr_swap_push()):
 *   =========================================================
 ...
 *     8847 0022 2010 0044 4001 000a a140 4500
...
bcm_mpls_label_t out_vc_label     = 0x444;  /* 1092 */

uint32_t stat_counter_id;
bcm_field_entry_t ifp_entry;
bcm_field_group_t ifp_group;
int expected_packet_length = 94;
 ...
 *  884700111002000AA140
 *  4500003C67620000FE01BE760A3A40010A3A4273
 *  0000550A000100516162636465666768696A6B6C6D6E6F7071727374757677616263646566676869
 *
 *
 *  Out Packet:
 *  -----------
...
unsigned char expected_egress_packet[90] = {
0x00, 0x00, 0x00, 0xaa, 0x22, 0x22, 0x00, 0x00, 0x00, 0xaa, 0x12, 0x12, 0x81, 0x00, 0x00, 0x16,
0x88, 0x47, 0x00, 0x22, 0x20, 0x10, 0x00, 0x44, 0x40, 0x01, 0x00, 0x0a, 0xa1, 0x40, 0x45, 0x00,
...
test_verify(int unit)
{
    ...
    printf("\n----------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL LABEL SWAP VERIFICATION  ");
    printf("\n----------------------------------------------------------- \n");
...
bcm_error_t
config_lsr_swap_push(int unit)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_if_t intf_id;
    bcm_if_t egr_obj;
	int tunnel_label_num = 1;
    ...
    /* Push new tunnel label */
    bcm_mpls_egress_label_t_init(&tunnel_label);
    tunnel_label.flags   =   BCM_MPLS_EGRESS_LABEL_TTL_SET;
    tunnel_label.label   =   out_tunnel_label;
    tunnel_label.ttl     =   ttl;
    BCM_IF_ERROR_RETURN(bcm_mpls_tunnel_initiator_set(unit, intf_id, tunnel_label_num, &tunnel_label));

    /* Create Egress object with outgoing label */
    rv = create_egr_obj_with_route_label(unit, intf_id, nh_mac, egress_port, out_vc_label, &egr_obj);
    if (BCM_FAILURE(rv)) {
...
clear_lsr_swap_push(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_mpls_tunnel_initiator_clear(unit, egr.intf));
...
bcm_error_t test_cleanup(int unit)
{
    ...
    /* Remove the l3 mpls label swap and push related configuration */
	if (BCM_FAILURE(rv = clear_lsr_swap_push(unit))){
        printf("clear_lsr_swap_push() failed.\n");
        return rv;
	}
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_lsr_swap_push())
 *  c) demonstrates the functionality(done in verify()).
 *  d) demonstrates the l3 mpls swap and push operation configure sequence(Done in test_cleanup())
 */
bcm_error_t
execute(void)
{
    ...
    print "~~~ #2) config_lsr_swap_push(): ** start **";
    if (BCM_FAILURE((rv = config_lsr_swap_push(unit)))) {
        printf("config_lsr_swap_push() failed.\n");
        return rv;
    }
    print "~~~ #2) config_lsr_swap_push(): ** end **";
