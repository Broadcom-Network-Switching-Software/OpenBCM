/*  Feature  : L3 MPLS tunnel initiation push maximum mpls labels
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_init_max_labels.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_init_max_labels_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *         8847 0123 4aff 0567 86fe 0020 01fe 4500 // 3 mpls labels:0x1234,0x5678,0x200
...
0x88, 0x47, 0x01, 0x23, 0x4a, 0xff, 0x05, 0x67, 0x86, 0xfe, 0x00, 0x20, 0x01, 0xfe, 0x45, 0x00,
...
config_l3_mpls_init(int unit)
{
	...
	bcm_mpls_label_t tunnel_label_init1 = 0x5678;
	bcm_mpls_label_t tunnel_label_init2 = 0x1234;
	bcm_mpls_label_t vc_label_init = 0x200;
	uint8 exp1 = 3, exp2 = 5;
	uint8 ttl1 = 254, ttl2 = 255;
    ...
    label_array[0].label = tunnel_label_init1;
	label_array[0].exp   = exp1;
    label_array[0].ttl   = ttl1;
	label_array[1].flags = BCM_MPLS_EGRESS_LABEL_TTL_SET | BCM_MPLS_EGRESS_LABEL_EXP_SET;
    label_array[1].label = tunnel_label_init2;
	label_array[1].exp   = exp2;
    label_array[1].ttl   = ttl2;
