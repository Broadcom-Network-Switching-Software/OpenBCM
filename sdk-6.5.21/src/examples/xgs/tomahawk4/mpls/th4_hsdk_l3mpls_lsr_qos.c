 ...
 *
 * Usage    : BCM.0> cint th4_hsdk_l3mpls_lsr_qos.c
 *
 * config   : bcm56990-generic-l3mpls.config.yml
 *
 * Log file : th4_hsdk_l3mpls_lsr_qos_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 * Summary:
 * ========
 *   This cint example to show configuration of L3 MPLS tunnel LSR QoS
 *   using BCM APIs.
 *
 ...
 *   =========================================================
 *     a) Configure a basic L3 MPLS Tunnel swap/switching functional scenario and Qos,
 ...
 *       8847 0022 2a01 000a a140 4500 003c 6762
 *       0000 fe01 be76 0a3a 4001 0a3a 4273 0000
 *       550a 0001 0051 6162 6364 6566 6768 696a
 *       6b6c 6d6e 6f70 7172 7374 7576 7761 6263
 *       6465 6667 6869
...
0x88, 0x47, 0x00, 0x22, 0x2a, 0x01, 0x00, 0x0a, 0xa1, 0x40, 0x45, 0x00, 0x00, 0x3c, 0x67, 0x62,
...
test_verify(int unit)
{
    ...
    printf("\n----------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS LSR QOS VERIFICATION  ");
    printf("\n----------------------------------------------------------- \n");
...
/* Create L3 egress object with outgoing label, egr_qos_id */
bcm_error_t
create_egr_obj(int unit, int l3_if, bcm_mac_t nh_mac, bcm_port_t port,
               bcm_mpls_label_t out_tunnel_label, int egr_qos_id, bcm_if_t *egr_obj_id)
{
    bcm_error_t rv = BCM_E_NONE;
	...
	l3_egress.mpls_flags = BCM_MPLS_EGRESS_LABEL_EXP_REMARK;
...
/* Qos related configuration, ingress/egress exp mapping */
bcm_error_t
config_l3_mpls_lsr_qos_mapping(int unit, int *ing_qos, int *egr_qos)
{
    ...
    /* Create ingress QOS mapping profile for EXP to internal pririty mapping */
    rv = bcm_qos_map_create(unit, BCM_QOS_MAP_MPLS | BCM_QOS_MAP_INGRESS, &ing_qos_id);
    if (BCM_FAILURE(rv)) {
    ...
    *ing_qos = ing_qos_id;
    ...
    for (i = 0; i < 8; i++) {
        ...
        if (BCM_FAILURE(rv)) {
            printf("\nError in ingress map bcm_qos_map_add: %s.\n", bcm_errmsg(rv));
            return rv;
        }
    }

    /* Create QoS mapping profile for intenral priority to EXP remarking */
    rv = bcm_qos_map_create(unit, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_MPLS, &egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in ingress bcm_qos_map_create: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    *egr_qos = egr_qos_id;
    ...
    for (i = 0; i < 8; i++) {
        ...
        egr_qos_map.int_pri = intpri[i];
        egr_qos_map.color = 0;
        egr_qos_map.exp = tunnel_exp[i];
        rv = bcm_qos_map_add(unit, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_MPLS,
                &egr_qos_map, egr_qos_id);
        if (BCM_FAILURE(rv)) {
            printf("\nError in egress map bcm_qos_map_add: %s.\n", bcm_errmsg(rv));
            return rv;
        }
    }
...
bcm_error_t
clear_l3_mpls_lsr_qos_mapping(int unit, int *ing_qos, int *egr_qos)
{
    bcm_error_t rv = BCM_E_NONE;
...
config_lsr_qos(int unit)
{
    ...
    rv = config_l3_mpls_lsr_qos_mapping(unit, &ing_qos_id, &egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in config_l3_mpls_lsr_qos_mapping() %s.\n",bcm_errmsg(rv));
        return rv;
    }

    /* Create Egress object with outgoing label, egr_qos_id */
    rv = create_egr_obj(unit, intf_id, nh_mac, egress_port, out_tunnel_label, egr_qos_id, &egr_obj);
    if (BCM_FAILURE(rv)) {
    ...
    tunnel_switch.exp_map = ing_qos_id;
...
clear_lsr_qos(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(clear_l3_mpls_lsr_qos_mapping(unit, ing_qos_id, egr_qos_id));
...
bcm_error_t test_cleanup(int unit)
{
	...
	if (BCM_FAILURE(rv = clear_lsr_qos(unit))){
        printf("clear_lsr_qos() failed.\n");
        return rv;
	}
