/*  Feature  : L3 MPLS tunnel init
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_init_ecn2exp_trust_port.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_init_ecn2exp_trust_port_log.txt
 *
 *  Test Topology : None
 *
 ...
 *    ======================================================
 *      a) Config ipecn to mpls exp mapping.
 *      b) Create egress objects used for unicast route destionation.
 *      c) Create a route.
 *
 ...
 *         8847 0567 8eff 0020 01fe 4500 003c 6762  // mapped exp=7
...
0x88, 0x47, 0x05, 0x67, 0x8e, 0xff, 0x00, 0x20, 0x01, 0xfe, 0x45, 0x00, 0x00, 0x3c, 0x67, 0x62,
...
bcm_error_t
config_ecn2exp_mapping(int unit, int *ecn_map_id)
{
	bcm_error_t rv = BCM_E_NONE;
	int map_id;
	...
	rv = bcm_ecn_map_create(unit, BCM_ECN_MAP_EGRESS | BCM_ECN_MAP_MPLS, &map_id);
    if (BCM_FAILURE(rv)) {
        printf("bcm_ecn_map_create()%s\n", bcm_errmsg(rv));
        return rv;
    }
	*ecn_map_id = map_id;
	print *ecn_map_id;
	...
	print bcm_ecn_map_set(unit, options, map_id, &ecn_map);

    return rv;
}

bcm_error_t
clear_ecn2exp_mapping(int unit, int ecn_map_id)
{
	bcm_error_t rv = BCM_E_NONE;

	rv = bcm_ecn_map_destroy(unit, ecn_map_id);
    if (BCM_FAILURE(rv)) {
        printf("bcm_ecn_map_destroy()%s\n", bcm_errmsg(rv));
        return rv;
    }
...
bcm_error_t config_ecn2exp_trustport(int unit, bcm_gport_t egr_gport, int ecn_map_id)
{
	bcm_error_t rv = BCM_E_NONE;
    bcm_ecn_port_map_t ecn_port_map;

    bcm_ecn_port_map_t_init(&ecn_port_map);
    ecn_port_map.ecn_map_id = ecn_map_id;
    ecn_port_map.flags = BCM_ECN_EGRESS_PORT_ECN_TO_EXP_MAP;
    rv = bcm_ecn_port_map_set(unit, egr_gport,  &ecn_port_map);
    if (BCM_FAILURE(rv)) {
        printf("bcm_ecn_port_map_set()%s\n", bcm_errmsg(rv));
        return rv;
    }
...
config_l3_mpls_init(int unit)
{
    ...
    rv = config_ecn2exp_mapping(unit, &ecn_map_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in config_ecn2exp_mapping(): %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = config_ecn2exp_trustport(unit, egress_port, ecn_map_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in config_port_ecn_to_exp_mapping(): %s.\n", bcm_errmsg(rv));
        return rv;
    }
	...
	label_array[0].flags = BCM_MPLS_EGRESS_LABEL_TTL_SET | BCM_MPLS_EGRESS_LABEL_ECN_TO_EXP_MAP | BCM_MPLS_EGRESS_LABEL_EXP_SET;
    label_array[0].label = tunnel_label_init;
	label_array[0].exp   = exp;
    label_array[0].ttl   = ttl;
	label_array[0].ecn_map_id = ecn_map_id;
	...
	config_l3_egress_flexctr(unit, egr_obj_id, &stat_counter_id);
...
bcm_error_t clear_l3_mpls_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(clear_ecn2exp_mapping(unit, ecn_map_id));
...
void test_verify(int unit)
{
    ...
    printf("\n--------------------------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL INITIATION ECN2EXP MAPPING TRUST PORT VERIFICATION  ");
    printf("\n--------------------------------------------------------------------------- \n");
