 ...
 *
 * Usage    : BCM.0> cint th4_hsdk_l3mpls_lsr_multi_segment.c
 *
 * config   : bcm56990-generic-l3mpls.config.yml
 *
 * Log file : th4_hsdk_l3mpls_lsr_multi_segment_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *
 *     b) Transmit the MPLS packet with two mpls labels.
 *       {000000001111} {000000000202} 8100 000b
 *       8847 1111 1002 2222 2140 4500 003c 6762
 ...
 *       8847 aaaa a040 bbbb b13f 4500 003c 6762
 *       0000 fe01 be76 0a3a 4001 0a3a 4273 0000
 *       550a 0001 0051 6162 6364 6566 6768 696a
 *       6b6c 6d6e 6f70 7172 7374 7576 7761 6263
 *       6465 6667 6869
...
bcm_mpls_label_t in_tunnel_label = 0x11111;
bcm_mpls_label_t in_vc_label = 0x22222;
bcm_mpls_label_t out_tunnel_label = 0xaaaaa;
bcm_mpls_label_t out_vc_label = 0xbbbbb;
...
0x88, 0x47, 0xaa, 0xaa, 0xa0, 0x40, 0xbb, 0xbb, 0xb1, 0x3f, 0x45, 0x00, 0x00, 0x3c, 0x67, 0x62,
...
test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000011110000000002028100000B884711111002222221404500003C67620000FE01BE760A3A40010A3A42730000550A000100516162636465666768696A6B6C6D6E6F7071727374757677616263646566676869; sleep quiet 1", ingress_port);
    bshell(unit, str);
    ...
    printf("\n----------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS LSR MULTI SEGMENT VERIFICATION  ");
    printf("\n----------------------------------------------------------- \n");
...
config_lsr_multi_segment(int unit)
{
    ...
    tunnel_label.ttl     =   64;
    BCM_IF_ERROR_RETURN(bcm_mpls_tunnel_initiator_set(unit, intf_id, 1, &tunnel_label));

    /* Create Egress object with outgoing label */
    rv = create_egr_obj(unit, intf_id, nh_mac, egress_port, out_vc_label, &egr_obj);
    if (BCM_FAILURE(rv)) {
    ...
    bcm_mpls_tunnel_switch_t_init (&tunnel_switch);
    tunnel_switch.label   = in_vc_label;
    tunnel_switch.port    = BCM_GPORT_INVALID;
...
bcm_error_t
clear_lsr_multi_segment(int unit)
{
    bcm_error_t rv = BCM_E_NONE;
	bcm_l3_egress_t egr;
    bcm_mpls_tunnel_switch_t tunnel_switch;
...
bcm_error_t test_cleanup(int unit)
{
    ...
    /* Remove the l3 mpls lsr multi segment related configuration */
	if (BCM_FAILURE(rv = clear_lsr_multi_segment(unit))){
        printf("clear_lsr_multi_segment() failed.\n");
        return rv;
	}
 ...
 *  d) demonstrates the teardown l3 mpls lsr multi segment configure sequence(Done in test_cleanup())
 */
bcm_error_t
execute(void)
{
