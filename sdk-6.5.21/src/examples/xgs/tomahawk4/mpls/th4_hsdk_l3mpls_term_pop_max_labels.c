/*  Feature  : L3 MPLS tunnel termination pop maximum mpls labels
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_term_pop_max_labels.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_term_pop_max_labels_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *         8847 0050 00fe 0005 00fe 0020 01fe 4500 // 3 mpls labels: 0x500, 0x50, 0x200
 *         003c 6762 0000 fe01 be76 0a3a 4001 0a3a // IPv4 DIP:10.58.66.115
...
bcm_mpls_label_t tunnel_label_term1 = 0x500;
bcm_mpls_label_t tunnel_label_term2 = 0x50;
...
bcm_error_t config_l3_mpls_term(int unit)
{
    ...
    switch_info.label      = tunnel_label_term1;
    ...
    bcm_mpls_tunnel_switch_t_init(&switch_info);
    switch_info.label      = tunnel_label_term2;
    switch_info.port       = ingress_port;
    switch_info.action     = BCM_MPLS_SWITCH_ACTION_POP;
    switch_info.ingress_if = ingress_if;
...
bcm_error_t clear_l3_mpls_term(int unit)
{
    ...
    tunnel_switch.label = tunnel_label_term1;
    ...
    bcm_mpls_tunnel_switch_t_init (&tunnel_switch);
    tunnel_switch.label = tunnel_label_term2;
    tunnel_switch.port  = ingress_port;
    BCM_IF_ERROR_RETURN(bcm_mpls_tunnel_switch_get(unit, &tunnel_switch));
	BCM_IF_ERROR_RETURN(bcm_mpls_tunnel_switch_delete(unit, &tunnel_switch));
...
bcm_error_t test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000001002a107777008100000b8847005000fe000500fe002001fe4500003c67620000fe01be760a3a40010a3a42730000550a000100516162636465666768696a6b6c6d6e6f7071727374757677616263646566676869; sleep quiet 1", ingress_port);
    bshell(unit, str);
    ...
    printf("\n--------------------------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL TERM POP MAX MPLS LABELS TERMINATION VERIFICATION  ");
    printf("\n--------------------------------------------------------------------------- \n");
