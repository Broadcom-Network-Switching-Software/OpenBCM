 ...
 *
 * Usage    : BCM.0> cint th4_hsdk_l3mpls_lsr_swap_ecmp.c
 *
 * config   : bcm56990-generic-l3mpls.config.yml
 *
 * Log file : th4_hsdk_l3mpls_lsr_swap_ecmp_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of L3 MPLS tunnel LSR to ecmp group
 *   using BCM APIs.
 *
 ...
 *        ecmp group member egress ports.
 *
 ...
 *
 *   2) Step2 - Configuration (Done in config_lsr_swap_ecmp()):
 *   =========================================================
 ...
 *
 *       ecmp member 1:
 *       {000002aa2222} {000002aa1212} 8100 0017
 *       8847 0033 3001 000a a140 4500 003c 6762 // TWO labels-0x333, 0xAA, TTL-1, TTL-64
 *       0000 fe01 be76 0a3a 4001 0a3a 4273 0000
 *       550a 0001 0051 6162 6364 6566 6768 696a
 *       6b6c 6d6e 6f70 7172 7374 7576 7761 6263
 *       6465 6667 6869
...
bcm_mpls_label_t out_tunnel_label2 = 0x333;  /* 546 */
...
0x88, 0x47, 0x00, 0x33, 0x30, 0x01, 0x00, 0x0a, 0xa1, 0x40, 0x45, 0x00, 0x00, 0x3c, 0x67, 0x62,
0x00, 0x00, 0xfe, 0x01, 0xbe, 0x76, 0x0a, 0x3a, 0x40, 0x01, 0x0a, 0x3a, 0x42, 0x73, 0x00, 0x00,
0x55, 0x0a, 0x00, 0x01, 0x00, 0x51, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a,
0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x61, 0x62, 0x63,
0x64, 0x65, 0x66, 0x67, 0x68, 0x69,
...
test_verify(int unit)
{
    ...
    printf("\n----------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL LABEL SWAP to ECMP GROUP VERIFICATION  ");
    printf("\n----------------------------------------------------------- \n");
...
config_lsr_swap_ecmp(int unit)
{
    ...
    rv = create_egr_obj(unit, intf_id, nh_mac, egress_port[0], out_tunnel_label, &egr_obj);
    if (BCM_FAILURE(rv)) {
    ...
    rv = create_egr_obj(unit, intf_id2, nh_mac2, egress_port[1], out_tunnel_label2, &egr_obj2);
    if (BCM_FAILURE(rv)) {
        printf("\nError in create_egr_obj(): %s.\n",bcm_errmsg(rv));
        return rv;
    }
...
bcm_error_t test_cleanup(int unit)
{
    ...
    /* Remove the l3 mpls label swap to ecmp group related configuration */
	if (BCM_FAILURE(rv = clear_lsr_swap_ecmp(unit))){
        printf("clear_lsr_swap_ecmp() failed.\n");
        return rv;
	}
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_lsr_swap_ecmp())
...
execute(void)
{
    ...
    print "~~~ #2) config_lsr_swap_ecmp(): ** start **";
    if (BCM_FAILURE((rv = config_lsr_swap_ecmp(unit)))) {
        printf("config_lsr_swap_ecmp() failed.\n");
        return rv;
    }
    print "~~~ #2) config_lsr_swap_ecmp(): ** end **";
