 ...
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_term_ecmp.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_term_ecmp_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
 ...
 *
 *    2) Step2 - Configuration (Done in config_l3_mpls_term_ecmp())
 *    ======================================================
 ...
 *         Packet raw data (86):
 *         ecmp member 0:
 *         {00000000fe02} {000000000002} 8100 000c // DMAC:fe02 SMAC:02 VLAN:12
 ...
 *
 *         ecmp member 1:
 *         {00000002fe02} {000000000202} 8100 0016 // DMAC:02fe02 SMAC:0202 VLAN:22
 *         0800 4500 003c 6762 0000 fd01 bf76 0a3a // l3 payload SIP:10.58.64.1
 *         4001 0a3a 4273 0000 550a 0001 0051 6162 // DIP:10.58.66.115
 *         6364 6566 6768 696a 6b6c 6d6e 6f70 7172
 *         7374 7576 7761 6263 6465 6667 6869
 ...
 *    ==========================================
 *      a) Mostly reverse the configuration, done in clear_l3_mpls_term_ecmp()
 */
cint_reset();
...
bcm_error_t config_l3_mpls_term_ecmp(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
    ...
    /* Create and add egress ports to egress VLANs */
    ...
    /* Create egress L3 interface */
    rv = create_l3_interface(unit, flags, router_mac_out2, out_vlan2, &intf_id2);
    if (BCM_FAILURE(rv)) {
        printf("\nError in create_l3_interface: %s.\n",bcm_errmsg(rv));
        return rv;
    }
    ...
    /* Create L3 egress object */
    rv = create_egr_obj(unit, intf_id, dst_mac, egress_port[0], &egr_obj_id);
    if (BCM_FAILURE(rv)) {
    ...
    /* Create L3 egress object */
    rv = create_egr_obj(unit, intf_id2, dst_mac2, egress_port[1], &egr_obj_id2);
    if (BCM_FAILURE(rv)) {
        printf("\nError in create_egr_obj(): %s.\n",bcm_errmsg(rv));
        return rv;
    }
...
bcm_error_t clear_l3_mpls_term_ecmp(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
	...
	BCM_IF_ERROR_RETURN(clear_egr_obj(unit, egr_obj_id));
	BCM_IF_ERROR_RETURN(clear_egr_obj(unit, egr_obj_id2));
...
bcm_error_t test_verify(int unit)
{
    ...
    printf("\n---------------------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL MPLS LABEL TERMINATION to ECMP GROUP VERIFICATION  ");
    printf("\n---------------------------------------------------------------------- \n");
...
bcm_error_t test_cleanup(int unit)
{
	...
	rv = clear_l3_mpls_term_ecmp(unit);
    if (BCM_FAILURE(rv)) {
        printf("clear_l3_mpls_term_ecmp() failed.\n");
        return rv;
	}
 ...
 *  a) test setup
 *  b) demonstrates the deployment for l3 mpls configure sequence(Done in config_l3_mpls_term_ecmp())
...
execute(void)
{
    ...
    print "~~~ #2) config_l3_mpls_term_ecmp(): ** start **";
    if (BCM_FAILURE((rv = config_l3_mpls_term_ecmp(unit)))) {
        printf("config_l3_mpls_term_ecmp() failed.\n");
        return rv;
    }
    print "~~~ #2) config_l3_mpls_term_ecmp(): ** end **";
