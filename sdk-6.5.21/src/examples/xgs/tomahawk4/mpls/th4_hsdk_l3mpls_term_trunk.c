 ...
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_term_trunk.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_term_trunk_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
 ...
 *
 *    2) Step2 - Configuration (Done in config_l3_mpls_term_trunk_trunk())
 *    ======================================================
 ...
 *         8847 0005 00fe 0020 01fe 4500 003c 6762 // pkt with 2 mpls labels 0x50, 0x200
 ...
 *         0800 4500 003c 6762 0000 fd01 bf76 0a3a // l3 payload SIP:10.58.64.1
 *         4001 0a3a 4273 0000 550a 0001 0051 6162 // DIP:10.58.66.115
 ...
 *    ==========================================
 *      a) Mostly reverse the configuration, done in clear_l3_mpls_term_trunk_trunk()
 */
cint_reset();
...
unsigned char expected_egress_packet[78] = {
0x00, 0x00, 0x00, 0x00, 0xfe, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x81, 0x00, 0x00, 0x0c,
0x08, 0x00, 0x45, 0x00, 0x00, 0x3c, 0x67, 0x62, 0x00, 0x00, 0xfd, 0x01, 0xbf, 0x76, 0x0a, 0x3a,
...
bcm_error_t config_l3_mpls_term_trunk(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
    ...
    rv = create_l3_interface(unit, flags, router_mac_out, out_vlan, &intf_id);
    if (BCM_FAILURE(rv)) {
    ...
    /* Create L3 egress object */
    rv = create_egr_obj(unit, intf_id, dst_mac, trunk_gp_id, &egr_obj_id);
    if (BCM_FAILURE(rv)) {
	...
	rv = config_trunk_flexctr(unit, trunk_id, &stat_counter_id);
    if (BCM_FAILURE(rv)) {
       printf("ERROR: config_trunk_flexctr() : %s.\n", bcm_errmsg(rv));
       return rv;
    }
...
bcm_error_t clear_l3_mpls_term_trunk(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
...
bcm_error_t test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx 10 pbm=%d data=0x000000000001002a107777008100000b8847000500fe002001fe4500003c67620000fe01be760a3a40010a3a42730000550a000100516162636465666768696a6b6c6d6e6f7071727374757677616263646566676869; sleep quiet 1", ingress_port);
    bshell(unit, str);
    ...
    printf("\n---------------------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL MPLS LABEL TERMINATION TO TRUNK VERIFICATION  ");
    printf("\n---------------------------------------------------------------------- \n");
...
bcm_error_t test_cleanup(int unit)
{
    ...
    rv = clear_trunk_flexctr(unit, trunk_id, stat_counter_id);
    if (BCM_FAILURE(rv)) {
	...
	rv = clear_l3_mpls_term_trunk(unit);
    if (BCM_FAILURE(rv)) {
        printf("clear_l3_mpls_term_trunk() failed.\n");
        return rv;
	}
 ...
 *  a) test setup
 *  b) demonstrates the deployment for l3 mpls configure sequence(Done in config_l3_mpls_term_trunk())
...
execute(void)
{
    ...
    print "~~~ #2) config_l3_mpls_term_trunk(): ** start **";
    if (BCM_FAILURE((rv = config_l3_mpls_term_trunk(unit)))) {
        printf("config_l3_mpls_term_trunk() failed.\n");
        return rv;
    }
    print "~~~ #2) config_l3_mpls_term_trunk(): ** end **";
