 ...
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_init_trunk.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_init_trunk_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
 ...
 *
 *    2) Step2 - Configuration (Done in config_l3_mpls_init_trunk())
 *    ======================================================
 ...
 *         8847 0020 02ff 0020 01fe 4500 003c 6762
 ...
 *    ==========================================
 *      a) Mostly reverse the configuration, done in clear_l3_mpls_init_trunk()
 */
cint_reset();
...
bcm_error_t
config_l3_mpls_init_trunk(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
...
bcm_error_t clear_l3_mpls_init_trunk(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
    bcm_l3_route_t route_info;
	bcm_l3_egress_t egr;
...
void test_verify(int unit)
{
    ...
    printf("\n-------------------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL MPLS LABEL INITIATION TO TRUNK VERIFICATION  ");
    printf("\n-------------------------------------------------------------------- \n");
...
int test_cleanup(int unit)
{
	...
	rv = clear_l3_mpls_init_trunk(unit);
    if (BCM_FAILURE(rv)) {
        printf("clear_l3_mpls_init_trunk() failed.\n");
        return rv;
	}
 ...
 *  a) test setup
 *  b) demonstrates the deployment for l3 mpls configure sequence(Done in config_l3_mpls_init_trunk())
...
execute(void)
{
    ...
    print "~~~ #2) config_l3_mpls_init_trunk(): ** start **";
    if (BCM_FAILURE((rv = config_l3_mpls_init_trunk(unit)))) {
        printf("config_l3_mpls_init_trunk() failed.\n");
        return rv;
    }
    print "~~~ #2) config_l3_mpls_init_trunk(): ** end **";
