 ...
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_term.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_term_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *      b) Create a L3 IPv4 route.
 *
 *    3) Step3 - Verification (Done in test_verify())
 *    ==========================================
 *      a) Register pktio call back funciton,
 *      b) CPU send the test packet to ingress_port which set MAC lookback.
 *         {000000000001} {002a10777700} 8100 000b
 *         8847 0005 00fe 0020 01fe 4500 003c 6762 // 2 mpls labels:0x50, 0x200
 *         0000 fe01 be76 0a3a 4001 0a3a 4273 0000 // IPv4 DIP:10.58.66.115
 *         550a 0001 0051 6162 6364 6566 6768 696a
 *         6b6c 6d6e 6f70 7172 7374 7576 7761 6263
 *         6465 6667 6869
 *
 *      c) Verify the result after send a packet.
 *         Expected Result:
 *         Packet raw data (78):
 *         {00000000fe02} {000000000002} 8100 000c // IPv4 pkt with DMAC SMAC VLAN
 *         0800 4500 003c 6762 0000 fd01 bf76 0a3a // got changed for l3 route after
 *         4001 0a3a 4273 0000 550a 0001 0051 6162 // mpls tunnel terminated
...
bcm_error_t test_verify(int unit)
{
    ...
    printf("\n----------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL MPLS LABEL TERMINATION VERIFICATION  ");
    printf("\n----------------------------------------------------------- \n");
