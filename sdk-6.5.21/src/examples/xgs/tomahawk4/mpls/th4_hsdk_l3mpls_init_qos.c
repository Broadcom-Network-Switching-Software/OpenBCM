/*  Feature  : L3 MPLS tunnel initiation QoS
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3mpls_init_qos.c
 *
 *  config   : bcm56990-generic-l3mpls.config.yml
 *
 *  Log file : th4_hsdk_l3mpls_init_qos_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *         8847 0567 8eff 0020 01fe 4500 003c 6762
...
/* Qos configuration */
bcm_error_t
config_l3_mpls_init_qos_mapping(int unit, int *ing_qos, int *egr_qos)
{
    bcm_error_t        rv = BCM_E_NONE;
...
config_l3_mpls_init(int unit)
{
    ...
    /* QOS mapping for l3 mpls tunnel init */
    rv = config_l3_mpls_init_qos_mapping(unit, &ing_qos_id, &egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("config_l3_mpls_init_qos return error : %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
void test_verify(int unit)
{
    ...
    printf("\n----------------------------------------------------------- \n");
    printf("    RESULT OF L3MPLS TUNNEL INITIATION QOS VERIFICATION  ");
    printf("\n----------------------------------------------------------- \n");
