 ...
 *            admission control are able to be mirrored to an MTP and
 *            encapsulated with an IPv4 PSAMP header. This script verifies
 *            the MOD queue which is a fixed MC-COS queue.
 *
 * Usage    : BCM.0> cint th4_hsdk_mod_mmu_cos.c
 *
 * config   : th4_hsdk_mod_config.yml
 *
 * Log file : th4_hsdk_mod_mmu_cos_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *
 *   2) Step2 - Configuration (Done in setup_mod_cosq()):
 *   =========================================================
 *       1) Configure an MOD configuration.
 *
 *       2) Enalbe MOD on the ingress port.
 *
 *       3) Cosq configurations to egress drop the traffic.
 *
 ...
 *
 *     Egress Packet 1:
 *     00 00 00 01 02 33 00 00 00 01 02 44 81 00 00 0A
 *     08 00 45 00 00 66 00 00 00 00 05 2F AF 64 02 02
 *     02 02 01 01 01 01 00 00 88 BE 00 00 00 00 00 01
 *     00 2A 10 77 77 00 81 00 00 0B 08 00 45 00 00 3C
 *     67 62 00 00 FF 01 BD 76 0A 3A 40 01 0A 3A 42 73
 *     00 00 55 0A 00 01 00 51 61 62 63 64 65 66 67 68
...
pktioRxCallback(int unit, bcm_pktio_pkt_t * packet, void *cookie)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_pktio_pmd_field_get (unit, packet,
                                                 bcmPktioPmdTypeRx,
                                                 BCM_PKTIO_RXPMD_QUEUE_NUM,
                                                 &queue_num));

    printf("pktioRxCallback: received from port 0x%x\n", port);
    printf("pktioRxCallback: received on queue_num %d\n", queue_num);
...
unsigned char expected_egress_packet1[120] = {
        0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44, 0x81, 0x00, 0x00, 0x0A,
        0x08, 0x00, 0x45, 0x00, 0x00, 0x66, 0x00, 0x00, 0x00, 0x00, 0x05, 0x2F, 0xAF, 0x64, 0x02, 0x02,
        0x02, 0x02, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x88, 0xBE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
        0x00, 0x2A, 0x10, 0x77, 0x77, 0x00, 0x81, 0x00, 0x00, 0x0B, 0x08, 0x00, 0x45, 0x00, 0x00, 0x3C,
        0x67, 0x62, 0x00, 0x00, 0xFF, 0x01, 0xBD, 0x76, 0x0A, 0x3A, 0x40, 0x01, 0x0A, 0x3A, 0x42, 0x73,
        0x00, 0x00, 0x55, 0x0A, 0x00, 0x01, 0x00, 0x51, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68,
