/*
 * Feature  : Mirror packets dropped in ingress pipeline destined
 *            to loopbackport
 *
 * Usage    : BCM.0> cint th4_hsdk_mod_ipipe_lb_port.c
 *
 * config   : th4_hsdk_mod_config.yml
 *
 * Log file : th4_hsdk_mod_ipipe_lb_port_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 *                      |                   |  egress_port1 (lb port)
 *                      |                   +----------<---->
 *                      |                   |  VLAN 20
 *        ingress_port  |                   |
 *       -------------->+       TH4         |
 ...
 *     00 6C 00 00 00 00 00 00 00 01 00 00 00 64 12 34
 *     00 5C 00 00 00 00 00 00 00 00 00 00 00 00 00 00
 *     00 13 0C 00 00 AE 00 FF 00 40 00 00 00 00 00 22
 ...
 *     08 00 45 02 00 88 00 00 40 00 02 11 44 04 02 10
 *     10 A0 11 10 10 A0 00 00 00 00 00 74 00 00 00 0A
 *     00 6C 00 00 00 00 00 00 00 02 00 00 00 64 12 34
 *     00 5C 00 00 00 00 00 00 00 00 00 00 00 00 00 00
 *     00 13 0C 00 00 AE 00 FF 00 40 00 00 00 00 00 22
...
setup_flex_ctr(int unit)
{
    ...
    rv = bcm_port_stat_attach(unit, lb_port, stat_counter_id);
    if (BCM_FAILURE(rv)) {
...
cleanup_flex_ctr(int unit)
{
    ...
    rv = bcm_port_stat_detach_with_id(unit, lb_port, stat_counter_id);
    if (BCM_FAILURE(rv)) {
...
setup_mirror_ingress(int unit)
{
    ...
    event_profile.sample_rate = 0x80000000;
...
test_setup(int unit)
{
    ...
    rv = add_l2_static_entry(unit, 0, dst_mac, switch_vlan, lb_port);
    if (BCM_FAILURE(rv)) {
...
unsigned char expected_egress_packet1[154] = {
        ...
        0x00, 0x6C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x64, 0x12, 0x34,
        0x00, 0x5C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x13, 0x0C, 0x00, 0x00, 0xAE, 0x00, 0xFF, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
...
unsigned char expected_egress_packet2[154] = {
        ...
        0x08, 0x00, 0x45, 0x02, 0x00, 0x88, 0x00, 0x00, 0x40, 0x00, 0x02, 0x11, 0x44, 0x04, 0x02, 0x10,
        0x10, 0xA0, 0x11, 0x10, 0x10, 0xA0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x74, 0x00, 0x00, 0x00, 0x0A,
        0x00, 0x6C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x64, 0x12, 0x34,
        0x00, 0x5C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x13, 0x0C, 0x00, 0x00, 0xAE, 0x00, 0xFF, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
...
bcm_error_t testVerify(int unit)
{
    ...
    expected_egress_packet1[81] = ingress_port;
    ...
    if (COMPILER_64_LO(counter_value.value[0]) != 0) {
        printf("lb_Port: Packet verify failed. Expected packet stat %d but get %d\n",
                                   count,
                                   COMPILER_64_LO(counter_value.value[0]));
        flexCtr_pass = 0;
    };
    if (COMPILER_64_LO(counter_value.value[1]) != 0) {
        printf("lb_port: Packet verify failed. Expected bytes stat 0 but get %d\n",
                                   COMPILER_64_LO(counter_value.value[1]));
        flexCtr_pass = 0;
    };
