/*
 * Feature  : Mirror packets which are sflow flex sampled to CPU in ingress pipeline
 *
 * Usage    : BCM.0> cint th4_hsdk_mod_ipipe_flex_sample.c
 *
 * config   : th4_hsdk_mod_config.yml
 *
 * Log file : th4_hsdk_mod_ipipe_flex_sample_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 *   Cint script to demonstrate ingress mirror the traffic which is
 *   sflow flex sampled to the CPU
 *
 ...
 *       2) Add mirror event source type.
 *       3) Create and set mirror profile using type bcmMirrorSampleTypeFlex.
 *       4) Attach the created sample profile id to an IFP entry
 *       5) L2 learn mode is set up on ingress and egress ports.
 *
 ...
 *     (egress_port1) as per the forwarding path setup and
 *     verify the mirror sample stat (bcm_mirror_sample_stat_type_t).
 *
 *     Egress Packet 1:
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11,
 *       0x81, 0x00, 0x00, 0x14, 0x08, 0x06, 0x00, 0x01, 0x08, 0x00, 0x06, 0x04,
 ...
 *
 *     Egress Packet 2:
 *      0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x81, 0x00, 0x00, 0x05,
 *      0x08, 0x00, 0x45, 0x00, 0x00, 0x88, 0x00, 0x00, 0x00, 0x00, 0x01, 0x11, 0x53, 0x00, 0x55, 0x55,
 *      0x55, 0x55, 0xDD, 0xDD, 0xDD, 0xDD, 0x00, 0x00, 0x00, 0x00, 0x00, 0x74, 0x00, 0x00, 0x00, 0x0A,
 *      0x00, 0x6C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x64, 0x12, 0x34,
 *      0x00, 0x5C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xDF,
 *      0x00, 0xEF, 0x0C, 0x00, 0x00, 0x01, 0x00, 0xFF, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22,
 *      0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x81, 0x00, 0x00, 0x14, 0x08, 0x06, 0x00, 0x01, 0x08, 0x00,
...
bcm_field_group_t mirror_group_id;
bcm_field_entry_t mirror_entry_id;
int               profile_id = 6;
...
setup_mirror_ingress(int unit)
{
    ...
    bcm_mirror_ingress_mod_event_t events[2] =
                    {bcmMirrorIngModEventSflowFlexSample,
                     bcmMirrorIngModEventSflowIngSample};
    ...
    mirror_dest.dst_mac = "11:12:13:14:15:16";
    mirror_dest.src_mac = "01:02:03:04:05:06";
    mirror_dest.tpid = 0x8100;
    mirror_dest.vlan_id = 5;
    mirror_dest.version = 4;
    mirror_dest.src_addr = 0x55555555;
    mirror_dest.dst_addr = 0xdddddddd;
    mirror_dest.ttl = 1;
    mirror_dest.tos = 0;
    mirror_dest.df = 0;
    mirror_dest.ipfix_version = 0xA;
    mirror_dest.initial_seq_number = 0x1;
    mirror_dest.meta_data = 0x1;
    ...
    options = BCM_MIRROR_SAMPLE_PROFILE_OPTIONS_WITH_ID;
    type = bcmMirrorSampleTypeFlex;
    print bcm_mirror_sample_profile_create(unit, options, type, &profile_id);
    print profile_id;
    ...
    profile.enable = TRUE;
    profile.rate = 1;
    profile.copy_to_cpu = TRUE;
    profile.mirror = TRUE;
    print bcm_mirror_sample_profile_set(unit, type, profile_id, &profile);

    if (debug) {
        /* Get sample profile. */
        bcm_mirror_sample_profile_t prf_get;
        bcm_mirror_sample_profile_t_init(&prf_get);
        print bcm_mirror_sample_profile_get(unit, type, profile_id, &prf_get);
        print prf_get;
    }

    /* Attach sample profile id to an IFP entry. */
    BCM_FIELD_QSET_INIT(qual);
    BCM_FIELD_QSET_ADD(qual, bcmFieldQualifyStageIngress);
    BCM_FIELD_QSET_ADD(qual, bcmFieldQualifyInPort);
    BCM_FIELD_QSET_ADD(qual, bcmFieldQualifyOuterVlan);

    BCM_FIELD_ASET_INIT(action);
    BCM_FIELD_ASET_ADD(action, bcmFieldActionIngSampleEnable);
    BCM_FIELD_ASET_ADD(action, bcmFieldActionFlexSampleProfile);

    print bcm_field_group_config_t_init(gcfg);
    gcfg.flags= BCM_FIELD_GROUP_CREATE_WITH_ASET;
    gcfg.qset= qual;
    gcfg.aset= action;
    gcfg.priority= 2;
    gcfg.group = 1;
    rv = bcm_field_group_config_create(unit, &gcfg);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field group config create : %s.\n", bcm_errmsg(rv));
        return rv;
    }
    mirror_group_id = gcfg.group;
    print mirror_group_id;

    rv = bcm_field_entry_create(unit, gcfg.group, mirror_entry_id);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field entry create : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_field_qualify_InPort(unit, mirror_entry_id, ingress_port, -1);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field qualify InPort : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_field_qualify_OuterVlan(unit, mirror_entry_id, switch_vlan, 0xfff);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field qualify OuterVlan : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_field_action_add(unit, mirror_entry_id,
                              bcmFieldActionIngSampleEnable, 0, 0);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field action add : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_field_action_add(unit, mirror_entry_id,
                              bcmFieldActionFlexSampleProfile, profile_id, 1);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field action add : %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_field_entry_install(unit, mirror_entry_id);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field entry install : %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
cleanup_mirror_ingress(int unit)
{
    ...
    /* Delete mirror FP entry and group. */
    rv = bcm_field_entry_destroy(unit, mirror_entry_id);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field entry destroy : %s.\n", bcm_errmsg(rv));
        return rv;
    }
    rv = bcm_field_group_destroy(unit, mirror_group_id);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field group destroy : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Destroy mirror sample profile */
    rv = bcm_mirror_sample_profile_destroy(unit, bcmMirrorSampleTypeFlex,
                                           profile_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_mirror_sample_profile_destroy: %s\n",
                                        bcm_errmsg(rv));
        return rv;
    }
...
bcm_error_t testVerify(int unit)
{
    ...
    /*update the Ipv4 PSAMP ingress port header field for comparison*/
    expected_egress_packet2[79] = egress_port1;
    expected_egress_packet2[81] = ingress_port;
    ...
    snprintf(str, 512, "tx %d pbm=%d data=0x00000000002200000000001181000014080600010800060400010000000000020A0A0A020000000000000A0A0A03000102030405060708090a0b0c0dbc94977f; sleep quiet 3", count, ingress_port);
    bshell(unit, str);

    if (debug) {
        bshell(unit, "bsh -c 'lt CTR_MIRROR_ING_FLEX_SFLOW lookup CTR_MIRROR_ING_FLEX_SFLOW_ID=6 PIPE=0'");
        bshell(unit, "bsh -c 'lt CTR_MIRROR_ING_FLEX_SFLOW lookup CTR_MIRROR_ING_FLEX_SFLOW_ID=6 PIPE=1'");
        bshell(unit, "bsh -c 'lt CTR_MIRROR_ING_FLEX_SFLOW lookup CTR_MIRROR_ING_FLEX_SFLOW_ID=6 PIPE=2'");
        bshell(unit, "bsh -c 'lt CTR_MIRROR_ING_FLEX_SFLOW lookup CTR_MIRROR_ING_FLEX_SFLOW_ID=6 PIPE=3'");
    }
    ...
    if (COMPILER_64_LO(counter_value.value[1]) != ((length+4))*count) {
    ...
    if (COMPILER_64_LO(counter_value.value[1]) != ((length_mtp+4)*count)) {
    ...
    /* Get mirror sample counter value. */

    sal_memset(&mirror_counter_value, 0, sizeof(mirror_counter_value));
    rv = bcm_mirror_sample_stat_get (unit, profile_id,  bcmMirrorSampleStatFlexPkts,
                                     &mirror_counter_value);
    if (BCM_FAILURE(rv)) {
        printf("bcm_mirror_sample_stat_get - Pkts %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("Mirror sample stat flex packets : %d \n",
                                   COMPILER_64_LO(mirror_counter_value));
    if (COMPILER_64_LO(mirror_counter_value) != count) {
        printf("Mirror FlexPkts : Packet verify failed. Expected packet stat %d but get %d\n",
                                   count,
                                   COMPILER_64_LO(mirror_counter_value));
        flexCtr_pass = 0;
    }
    ...
    sal_memset(&mirror_counter_value, 0, sizeof(mirror_counter_value));
    rv = bcm_mirror_sample_stat_get (unit, profile_id,  bcmMirrorSampleStatFlexSampledPkts,
                                     &mirror_counter_value);
    if (BCM_FAILURE(rv)) {
        printf("bcm_mirror_sample_stat_get - SampledPkts %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("Mirror sample stat flex sampled packets : %d \n",
                                   COMPILER_64_LO(mirror_counter_value));
    if (COMPILER_64_LO(mirror_counter_value) != count) {
        printf("Mirror FlexPkts : Packet verify failed. Expected packet stat %d but get %d\n",
                                   count,
    ...
    sal_memset(&mirror_counter_value, 0, sizeof(mirror_counter_value));
    rv = bcm_mirror_sample_stat_get (unit, profile_id,  bcmMirrorSampleStatFlexSnapshotPkts,
                                     &mirror_counter_value);
    if (BCM_FAILURE(rv)) {
        printf("bcm_mirror_sample_stat_get - FlexSnapshotPkts %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("Mirror sample stat flex snapshot packets : %d \n",
                                   COMPILER_64_LO(mirror_counter_value));
    if (COMPILER_64_LO(mirror_counter_value) != count) {
        printf("Mirror FlexPkts : Packet verify failed. Expected packet stat %d but get %d\n",
                                   count,
