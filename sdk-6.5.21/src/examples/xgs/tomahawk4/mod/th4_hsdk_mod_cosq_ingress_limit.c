 ...
 *            encapsulated with an IPv4 PSAMP header.
 *
 * Usage    : BCM.0> cint th4_hsdk_mod_cosq_ingress_limit.c
 *
 * config   : th4_hsdk_mod_config.yml
 *
 * Log file : th4_hsdk_mod_cosq_ingress_limit_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
...
cleanup_mod_cosq (int unit)
{
    ...
    rv = bcm_field_entry_destroy(unit, mod_entry_id);
    if(BCM_FAILURE(rv)) {
        printf("\nError in field entry destroy : %s.\n", bcm_errmsg(rv));
        return rv;
    }
    rv = bcm_field_group_destroy(unit, mod_group_id);
    if(BCM_FAILURE(rv)) {
