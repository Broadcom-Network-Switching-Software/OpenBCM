/*
 * Feature  : MOD (MirrorOnDrop), packets dropped in MMU due to ingress
 *            admission control are able to be mirrored to an MTP
 *            with an IPv4 PSAMP header and verify the MOD cosq stats.
 *
 * Usage    : BCM.0> cint th4_hsdk_mod_cosq_ingress_limit_stats.c
 *
 * config   : th4_hsdk_mod_config.yml
 *
 * Log file : th4_hsdk_mod_cosq_ingress_limit_stats_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 *   Cint example to demonstrate MOD configuration for IPv4 PSAMP initiation
 *   and verify the MOD cosq stats (bcm_cosq_mod_stat_t).
 *
 ...
 *
 *     c) Configuration (Done in setup_mod_cosq()):
 *   =========================================================
 *       c.1) Configure an MOD configuration.
 *
 *       c.2) Install an IFP rule to enalbe MOD on the ingress port.
 *
 *       c.3) Cosq configurations to ingress drop the traffic.
 *
 ...
 *
 *     The packet capture shows the egress packet transmitted on outgoing port
 *     (mtp_port) as per the MOD setup.
 *
 *     Egress Packet 2:
 *       0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
 *       0x81, 0x00, 0x00, 0x0a, 0x08, 0x00, 0x45, 0x00, 0x00, 0x92, 0x00, 0x00,
 *       0x00, 0x00, 0x05, 0x11, 0xaf, 0x56, 0x02, 0x02, 0x02, 0x02, 0x01, 0x01,
 *       0x01, 0x01, 0x12, 0x34, 0x43, 0x21, 0x00, 0x7e, 0x00, 0x00, 0x00, 0x0a,
 *       0x00, 0x76, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x00,
 *       0x00, 0x64, 0x12, 0x34, 0x00, 0x66, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0x00, 0x03,
 *       0x00, 0x00, 0x00, 0xff, 0x00, 0x4a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
 *       0x00, 0x2a, 0x10, 0x77, 0x77, 0x00, 0x08, 0x00, 0x45, 0x00, 0x00, 0x3c,
 *       0x67, 0x62, 0x00, 0x00, 0xff, 0x01, 0xbd, 0x76, 0x0a, 0x3a, 0x40, 0x01,
 *       0x0a, 0x3a, 0x42, 0x73, 0x00, 0x00, 0x55, 0x0a, 0x00, 0x01, 0x00, 0x51,
 *       0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c,
 *       0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x61,
 *       0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69
 *
 * Header Info :
 *      ###[ Ethernet ]###
 *        dst= 00:00:00_01:02:33
 *        src= 00:00:00_01:02:44
 *        type= 0x8100
 *      ###[ 802.1Q ]###
 *         vlan= 10
 *         type= 0x800
 ...
 *         len= 0x66
 *         Egress port of the dropped packet= 0x2
 *         Ingress port= 0x1
 *         Drop reason= 0x3 >> 1 = 1 (Ingress Limit)
 *         variable flag= 0xff
 *      ###[ Ethernet ]###
 *                     dst= 00:00:00:00:aa:01
 *                     src= 00:2a:10:77:77:00
 *                     type= 0x800
 *      ###[ IPv4 ]###
 *           version= 4
 *
 ...
 *      5 = Egress WRED drop
 *
 *     Verify the cosq mirror-on-drop statistics by invoking
 *     bcm_cosq_mod_stat_get.
 */
...
bcm_field_group_t mod_group_id;
bcm_field_entry_t mod_entry_id;
int               mod_profile_id;
 ...
 */
bcm_error_t
mod_ing_port_setup(int unit, bcm_port_t port, int profile_id)
{
    bcm_field_group_config_t    group_config;
    ...
    BCM_IF_ERROR_RETURN(bcm_field_group_config_create(unit, group_config));
    mod_group_id = group_config.group;

    BCM_IF_ERROR_RETURN(bcm_field_entry_create(unit, mod_group_id,
                                               &mod_entry_id));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_InPort(unit, mod_entry_id, port,
                                                 BCM_FIELD_EXACT_MATCH_MASK));
    BCM_IF_ERROR_RETURN(bcm_field_action_add(unit, mod_entry_id,
                                             bcmFieldActionMirrorOnDropEnable,
                                             0, 0));
    BCM_IF_ERROR_RETURN(bcm_field_action_add(unit, mod_entry_id,
                                           bcmFieldActionMirrorOnDropProfileSet,
                                           profile_id, 1));
    BCM_IF_ERROR_RETURN(bcm_field_entry_install(unit, mod_entry_id));
...
mod_init(int unit, int *profile_id)
{
    ...
    control.pool_limit = 0x9FFD8; /* 254(one cell) * 2580. */
    ...
    profile.percent_25_50 = 0xffff;
    profile.percent_50_75 = 0xffff;
    profile.percent_75_100 = 0xffff;
    profile.enable = 1;
...
setup_mod_cosq (int unit)
{
    ...
    uint32                      limit_kbits_sec = 1000;

    /* MOD config */
    rv = mod_init(unit, &mod_profile_id);
    if(BCM_FAILURE(rv)) {
        printf("\nError in mod init: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    /* Enable MOD */
    rv = bcm_port_control_set(unit, ingress_port, bcmPortControlModEnable, 1);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_port_control_set(ModEnable): %s\n",bcm_errmsg(rv));
        return rv;
    }

    /* Per port mod profile config */
    rv = bcm_port_control_set(unit, ingress_port, bcmPortControlModProfileId,
                                                              mod_profile_id);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_port_control_set(ModProfileId): %s\n",
                                                        bcm_errmsg(rv));
        return rv;
    }
    ...
    mirror_dest.initial_seq_number = 10;
    mirror_dest.flags2 = BCM_MIRROR_DEST_FLAGS2_MOD_TUNNEL_PSAMP;
    mirror_dest.flags2   |= BCM_MIRROR_DEST_FLAGS2_MOD_TUNNEL_PSAMP |
                            BCM_MIRROR_DEST_FLAGS2_MIRROR_ON_DROP;
    ...
    printf("Adding source-to-destination for MOD \n");
    ...
    /* CosQ configurations to drop the traffic */
    type = bcmCosqControlIngressHeadroomPoolLimitBytes;
    rv = bcm_cosq_control_set(unit, ingress_port, 0, type, 0);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcmCosqControlIngressHeadroomPoolLimitBytes : %s\n",
                                                        bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_cosq_control_set(unit, ingress_port, 0, type, 0);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcmCosqControlIngressPortPoolSharedLimitBytes : %s\n",
                                                        bcm_errmsg(rv));
        return rv;
    }

    /*Bandwidth limit on egress queue */
    rv = bcm_cosq_gport_bandwidth_set(unit, egress_port, 0, limit_kbits_sec,
                                      limit_kbits_sec, 0);
    if (rv != BCM_E_NONE) {
...
bcm_error_t
cleanup_mod_cosq (int unit)
{
    bcm_mirror_source_t         source;
    bcm_error_t                 rv = BCM_E_NONE;
    uint32                      mirror_flags = 0;

    /* Delete mirror source destination */
    bcm_mirror_source_t_init(&source);
    source.type = bcmMirrorSourceTypeCosqMod;
    ...
    /* Destroy cosq MOD profile */
    rv = bcm_cosq_mod_profile_destroy(unit, mod_profile_id);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_cosq_mod_profile_destroy: %s\n",bcm_errmsg(rv));
        return rv;
    }
    ...
    /* Disable MOD */
    rv = bcm_port_control_set(unit, ingress_port, bcmPortControlModEnable, 0);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_port_control_set(ModDisable): %s\n",
                                        bcm_errmsg(rv));
        return rv;
    }
...
test_setup(int unit)
{
    ...
    if (BCM_E_NONE != setup_mod_cosq(unit)) {
        printf("setup_mod_cosq() failed.\n");
        return -1;
    }
...
int test_cleanup(int unit)
{
    ...
    if (BCM_E_NONE != cleanup_mod_cosq(unit)) {
        printf("cleanup_mod_cosq() failed.\n");
        return -1;
    }
...
unsigned char expected_egress_packet1[164] = {
        0x00, 0x00, 0x00, 0x01, 0x02, 0x33, 0x00, 0x00, 0x00, 0x01, 0x02, 0x44,
        0x81, 0x00, 0x00, 0x0a, 0x08, 0x00, 0x45, 0x00, 0x00, 0x92, 0x00, 0x00,
        0x00, 0x00, 0x05, 0x11, 0xaf, 0x56, 0x02, 0x02, 0x02, 0x02, 0x01, 0x01,
        0x01, 0x01, 0x12, 0x34, 0x43, 0x21, 0x00, 0x7e, 0x00, 0x00, 0x00, 0x0a,
...
bcm_error_t testVerify(int unit)
{
    ...
    if (debug) {
        ...
        bshell(unit, "bsh -c 'lt PORT_TM_MIRROR_ON_DROP traverse -l'");
        bshell(unit, "bsh -c 'lt TM_MIRROR_ON_DROP_CONTROL traverse -l'");
        bshell(unit, "bsh -c 'lt TM_MIRROR_ON_DROP_DESTINATION traverse -l'");
        bshell(unit, "bsh -c 'lt TM_MIRROR_ON_DROP_ENCAP_PROFILE traverse -l'");
        bshell(unit, "bsh -c 'lt TM_MIRROR_ON_DROP_PROFILE traverse -l'");
        bshell(unit, "fp show");
    }
    ...
    if ((COMPILER_64_LO(counter_value.value[0]) == 0) ||
        (COMPILER_64_LO(counter_value.value[0]) == count)) {
        printf("Egress Port: Packet verify failed. Received packet count %d \n",
                                   COMPILER_64_LO(counter_value.value[0]));
        flexCtr_pass = 0;
    };
    ...
    if (COMPILER_64_LO(counter_value.value[0]) == 0) {
        printf("mtp_port: Packet verify failed. Received packet count %d \n",
                                   COMPILER_64_LO(counter_value.value[0]));
        flexCtr_pass = 0;
    }
    ...
    rv = bcm_cosq_mod_stat_get(unit,
                               &id,
                               bcmCosqModStatDroppedPackets,
                               &value1);
    if (BCM_FAILURE(rv)) {
        printf("COSQ MOD stat get : bcmCosqModStatDroppedPackets %s\n",
                                                         bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_cosq_mod_stat_get(unit,
                               &id,
                               bcmCosqModStatEnqueuedPackets,
                               &value2);
    if (BCM_FAILURE(rv)) {
        printf("COSQ MOD stat get : bcmCosqModStatEnqueuedPackets %s\n",
                                                         bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_cosq_mod_stat_get(unit,
                               &id,
                               bcmCosqModStatPoolBytesCurrent,
                               &value3);
    if (BCM_FAILURE(rv)) {
        printf("COSQ MOD stat get : bcmCosqModStatPoolBytesCurrent %s\n",
                                                         bcm_errmsg(rv));
        return rv;
    }
 ...
 *  This function does the following
 *  a) Test setup
 *  b) Actual configuration (done in setup_mod_cosq())
