/*
 * Feature  : MOD (MirrorOnDrop), packets dropped in MMU due to egress
 *            admission control are able to be mirrored to an MTP with
 *            an IPv4 PSAMP header and verify the MOD cosq stats.
 *
 * Usage    : BCM.0> cint th4_hsdk_mod_cosq_egress_limit_stats.c
 *
 * config   : th4_hsdk_mod_config.yml
 *
 * Log file : th4_hsdk_mod_cosq_egress_limit_stats_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *
 *       c.2) Enalbe MOD on the ingress port.
 *
 *       c.3) Cosq configurations to egress drop the traffic.
 *
 ...
 *       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0x00, 0x07,
 ...
 *         Ingress port= 0x1
 *         Drop reason= 0x7 >> 1 = 3 (Egress port SP Shared Limit)
 *         variable flag= 0xff
 *      ###[ Ethernet ]###
...
setup_mod_cosq (int unit)
{
    ...
    /* Enable MOD and set MOD profile using IFP */
    rv = mod_ing_port_setup (unit, ingress_port, mod_profile_id);
    if (BCM_FAILURE(rv)) {
        printf("mod_ing_port_setup() failed for port %d\n", ingress_port);
        return rv;
    }
    ...
    rv = bcm_cosq_control_set(unit, egress_port, 0, type, 0);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcmCosqControlEgressUCQueueGroupMinEnable : %s\n",
                                                        bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_cosq_control_set(unit, egress_port, 0, type, 0);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcmCosqControlEgressPortPoolSharedLimitBytes : %s\n",
                                                        bcm_errmsg(rv));
        return rv;
    }
...
bcm_error_t testVerify(int unit)
{
    ...
    if (COMPILER_64_LO(counter_value.value[0]) != 0) {
        printf("Egress Port: Packet verify failed. Expected packet stat %d but get %d\n",
                                   0,
                                   COMPILER_64_LO(counter_value.value[0]));
        flexCtr_pass = 0;
    };
    if (COMPILER_64_LO(counter_value.value[1]) != 0) {
        printf("Egress Port: Packet verify failed. Expected bytes stat %d but get %d\n",
                                   0,
                                   COMPILER_64_LO(counter_value.value[1]));
        flexCtr_pass = 0;
    };
