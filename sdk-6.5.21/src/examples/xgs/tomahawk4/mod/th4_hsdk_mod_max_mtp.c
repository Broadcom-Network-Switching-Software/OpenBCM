 ...
 *            and IFP drop events.
 *
 * Usage    : BCM.0> cint th4_hsdk_mod_max_mtp.c
 *
 * config   : th4_hsdk_mod_config.yml
 *
 * Log file : th4_hsdk_mod_max_mtp_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *
 *     Error: Max mirror session = 8: No resources for operation.
 */
...
setup_mirror_ingress(int unit)
{
    ...
    for (i=0; i < 9; i++) {
        ...
        mirror_dest[i].flags  = BCM_MIRROR_DEST_TUNNEL_WITH_SEQ;
        mirror_dest[i].flags2 = BCM_MIRROR_DEST_FLAGS2_MOD_TUNNEL_PSAMP;
        mirror_dest[i].gport  = mtp_gport;
        sal_memcpy(mirror_dest[i].dst_mac, dest_mac, 6);
        sal_memcpy(mirror_dest[i].src_mac, src_mac, 6);
        mirror_dest[i].tpid=0x8100;
        mirror_dest[i].vlan_id=vlan_id+i;
        mirror_dest[i].version=4;
        mirror_dest[i].src_addr=0x021010a0;
        mirror_dest[i].dst_addr=0x111010a0;
        mirror_dest[i].ttl=2;
        mirror_dest[i].tos=2;
        mirror_dest[i].df=1;
        mirror_dest[i].ipfix_version = 0xA;
        mirror_dest[i].initial_seq_number = 0x1;
        mirror_dest[i].meta_data = 0xae;
        mirror_dest[i].meta_data_type = bcmMirrorPsampFmt2HeaderUserMeta;
        rv = bcm_mirror_destination_create(unit, &mirror_dest[i]);
        max_mirror_dest = i;
        if(BCM_FAILURE(rv)) {
            printf("\nError: Max mirror session = %d: %s.\n",i,bcm_errmsg(rv));
            return i;
        }
    ...
    rv = bcm_mirror_source_dest_add(unit, &source, mirror_dest_id[7]);
    if (BCM_FAILURE(rv)) {
...
{
    bcm_error_t                 rv = BCM_E_NONE;
    uint32                      mirror_flags = 0, i = 0;

    if (max_mirror_dest != 0){
        for (i=0; i<max_mirror_dest; i++){
    ...
    /*Remove ingress mod*/
    rv = bcm_mirror_source_dest_delete(unit, &source, mirror_dest_id[7]);
    if (BCM_FAILURE(rv)) {
...
test_setup(int unit)
{
    ...
    rv =  setup_mirror_ingress(unit);
    if (BCM_FAILURE(rv)) {
        printf("Error in setup_mirror_ingress: %s\n", bcm_errmsg(rv));
        return rv;
    }
