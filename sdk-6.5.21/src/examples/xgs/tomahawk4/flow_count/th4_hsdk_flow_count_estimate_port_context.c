/*  Feature  : Flow count estimate with port context
 *
 *  Usage    : BCM.0> cint th4_hsdk_flow_count_estimate_port_context.c
 *
 *  config   : th4_hsdk_flow_count_estimate_config.yml
 *
 *  Log file : th4_hsdk_flow_count_estimate_port_context_log.txt
 *
 *  Test Topology :
 *
 *                      +-------------------+
 ...
 *  Summary:
 *  ========
 *    This CINT configures flow count estimate with port context using BCM APIs.
 *    Two port contexts are used and each port conext has 256 counters configured
 *    for stochastic averaging.
 *    The ingress port is assigned to port context 1 for this test.
...
int num_of_port_contexts = 2;
int ingress_port_context = 1;
...
flexctr_stat_create(int unit, uint32_t *flexctr_act_id)
{
    ...
    action.index_num = 4096*num_of_port_contexts;  /* 4096 counters for each port context */

    /* Flow count flex counters: 256 counters per port context */
    index_op = &action.index_operation;
    index_op->object[0] = bcmFlexctrObjectStaticIngFlowCountPortContext;
    /* flexCtr index = port_context(4b) << 12 || counter_hash */
...
/*
  ...
  +---------------------------------------------------------------+
             ...
             <-----    flex counter      ----->
                          index
  In this example, the ingress port is assigned to port_context 1
*/
bcm_error_t
flowcount_config(int unit, uint32_t stat_counter_id)
{
    ...
    /* Assign port context 1 to the ingress port */
    rv = bcm_port_control_set(unit, ingress_port, bcmPortControlFlowCountClassId,
                              ingress_port_context);
    if(BCM_FAILURE(rv)) {
...
/* Clean up flow count configuration */
bcm_error_t
flowcount_unconfig(int unit)
{
    bcm_error_t rv;

    rv = bcm_port_control_set(unit, ingress_port, bcmPortControlFlowCountClassId, 0);
    if(BCM_FAILURE(rv)) {
        printf("\nError in bcm_port_control_set(): %s.\n",bcm_errmsg(rv));
        return rv;
    }
...
flexctr_counter_value_get_show(int unit, uint32_t stat_counter_id)
{
    ...
    bcm_error_t rv;
    int starting_offset = 4096*ingress_port_context;
    for (i = 0; i < index_cnt; i++) {
        index[i] = i + starting_offset;
    }
