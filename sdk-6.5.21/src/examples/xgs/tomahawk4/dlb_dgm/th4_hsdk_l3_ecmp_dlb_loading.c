/*  Feature  : L3 IPv4 Route to DLB ECMP group
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3_ecmp_dlb_loading.c
 *
 *  config   : bcm56990-generic-l3.config.yml
 *
 *  Log file : th4_hsdk_l3_ecmp_dlb_loading_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
 ...
 *  ========
 *   bcm_l3_route_add()
 *   bcm_l3_ecmp_create() with flags BCM_L3_ECMP_DYNAMIC_MODE_DLB
 *
 *
 *  Summary:
 *  ========
 *  This cint example demonstrate L3 IPv4 route to DLB ECMP group with RTAG7 hash
 *  configuration.
 *
 ...
 *
 *    2) Step2 - Configuration (Done in config_rtag7_l3_ecmp_dlb())
 *    ======================================================
 *      a) Create egress objects used for unicast route destionation.
 *      b) Create a DLB ECMP grpup.
 *      c) Create a L3 route to that DLB ECMP group.
 *
 *    3) Step3 - Verification (Done in test_verify())
 *    ==========================================
 *      a) For a packet received with flow of existing assignment not valid any more(for example,
 *      egress port link status down/disabled), DLB logic will automatically choose a new assignment,
 *      this is done by TH4 Switch hardware itself, no need application software involved to reconstructure
 *      the ECMP group member table.
 *      Verify the result after send test packets into ingress_port four times, each time the
 *      packet receive callback fucntion will verify the expected received port number and received
 *      packet data, and disable the received egress_port[], let the port link status down, next
 *      time received the same packet flow, DLB logic expect to choose another valid assignment.
 *
 *         ingress_port:
 ...
 *
 *      b) Expected Result:
 *      Expect the l3 IPv4 route packets egressed from different
 ...
 *
 *
 *    4) Step4 - The L3 route to DLB ECMP group with RTAG7 hash teardown configuration(Done in test_cleanup())
 *    ==========================================
 *      a) Reverse the configuration, done in clear_rtag7_l3_ecmp_dlb()
 */
cint_reset();
...
int egress_port_num = 4;
bcm_port_t egress_port[egress_port_num];
bcm_vlan_t in_vlan = 11;
bcm_vlan_t out_vlan[egress_port_num] = {12, 22, 32, 42};
bcm_vrf_t vrf = 1;
bcm_if_t l3_intf_id[egress_port_num];
bcm_if_t egr_obj_id[egress_port_num];
...
uint32 dlb_mapping_port_quality = 6; /* range from 0 - 7, 7 as highest*/
...
void Overide_port_quality(bcm_port_t port, uint32 dlb_mapping_port_quality)
{
    char   str[512];

    snprintf(str, 512, "bsh -c 'pt DLB_ECMP_PORT_QUALITY_MAPPINGm set BCMLT_PT_INDEX=1535  ASSIGNED_QUALITY=%d'", dlb_mapping_port_quality);
    bshell(unit, str);
    snprintf(str, 512, "bsh -c 'pt DLB_ECMP_QUANTIZE_CONTROLm set BCMLT_PT_INDEX=%d PORT_QUALITY_MAPPING_PROFILE_PTR=2'", port);
    bshell(unit, str);
...
pktioRxCallback(int unit, bcm_pktio_pkt_t * packet, void *cookie)
{
	...
	/* overide port quality mapping value */
    printf("Overide port:%d quality mapping value.\n", port);
    Overide_port_quality(port, dlb_mapping_port_quality);
...
check_flexctr(int unit, uint32 stat_counter_id, int *flexctr_check_failed)
{
	...
	/* expected to receive 1 packets from each of egr l3 egress object */
	for(i=0;i<egress_port_num;i++){
    if (COMPILER_64_LO(counter_value[i+1].value[0]) != 1) {
        printf("Flexctr packets verify failed. flexctr attached to egr obj:%d got %d packets.\n",
				egr_obj_id[i], COMPILER_64_LO(counter_value[i+1].value[0]));
    ...
    if (COMPILER_64_LO(counter_value[i+1].value[1]) != expected_packet_length) {
        printf("Flexctr bytes verify failed. flexctr attached to egr obj:%d expected bytes %d but got %d\n",
				egr_obj_id[i], expected_packet_length, COMPILER_64_LO(counter_value[i+1].value[1]));
        *flexctr_check_failed = 1;
	    print counter_index;
        print counter_value;
        rv = BCM_E_FAIL;
...
bcm_error_t
config_rtag7_l3_ecmp_dlb(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
	int flags, i;
    ...
    int ecmp_loading_weight[egress_port_num] = {50, 50, 50, 50};
    int ecmp_queue_size_weight[egress_port_num] = {40, 40, 40, 40};
    int ecmp_scaling_factor[egress_port_num] = {10, 10, 10, 10};
    ...
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_l3_ecmp_dlb_port_quality_attr_set(): %s.\n", bcm_errmsg(rv));
        return rv;
    }
	}
...
bcm_error_t clear_rtag7_l3_ecmp_dlb(int unit)
{
	bcm_error_t rv = BCM_E_NONE;
    bcm_l3_route_t route_info;
	bcm_l3_egress_t egr;
...
void test_verify(int unit)
{
    ...
    printf("Transmiting l3 packet into ingress_port:%d first time\n", ingress_port);
    ...
    bshell(unit, "sleep quiet 1");
    printf("Transmiting l3 packet into ingress_port:%d second time\n", ingress_port);
    bshell(unit, str);

    bshell(unit, "sleep quiet 1");
    printf("Transmiting l3 packet into ingress_port:%d third time\n", ingress_port);
    bshell(unit, str);

    bshell(unit, "sleep quiet 1");
    printf("Transmiting l3 packet into ingress_port:%d fourth time\n", ingress_port);
    bshell(unit, str);
    ...
    printf("\n----------------------------------------------------------------------------------- \n");
    printf("    RESULT OF L3 IPv4 ROUTE to DLB ECMP GROUP with RTAG7 HASH VERIFICATION  ");
    printf("\n----------------------------------------------------------------------------------- \n");
...
int test_cleanup(int unit)
{
    ...
    /* Remove the l3 route to DLB ECMP group related configuration */
	rv = clear_rtag7_l3_ecmp_dlb(unit);
    if (BCM_FAILURE(rv)) {
        printf("clear_rtag7_l3_ecmp_dlb() failed.\n");
        return rv;
	}
 ...
 *     (Done in config_rtag7_l3_ecmp_dlb())
 *  c) demonstrates the functionality(done in test_verify()).
 *  d) demonstrates the L3 IPv4 route to DLB ECMP group with RTAG7 teardown configure sequence
...
execute(void)
{
    ...
    print "~~~ #2) config_rtag7_l3_ecmp_dlb(): ** start **";
    if (BCM_FAILURE((rv = config_rtag7_l3_ecmp_dlb(unit)))) {
        printf("config_rtag7_l3_ecmp_dlb() failed.\n");
        return rv;
    }
    print "~~~ #2) config_rtag7_l3_ecmp_dlb(): ** end **";
