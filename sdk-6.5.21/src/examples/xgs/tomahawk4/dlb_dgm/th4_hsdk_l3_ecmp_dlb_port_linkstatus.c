 ...
 *
 *  Usage    : BCM.0> cint th4_hsdk_l3_ecmp_dlb_port_linkstatus.c
 *
 *  config   : bcm56990-generic-l3.config.yml
 *
 *  Log file : th4_hsdk_l3_ecmp_dlb_port_linkstatus_log.txt
 *
 *  Test Topology :
 *                  +------------------------------+
 *                  |                              |
 ...
 *    ==========================================
 *      a) For a packet received with flow of existing assignment not valid any more
 *      (for example, egress port link status down or port disabled), DLB logic will
 *      automatically choose a new assignment, this is done by TH4 Switch hardware
 *      itself, no need application software involved to reconstructure the ECMP group
 *      member table. Verify the result after send test packets into ingress_port four
 *      times, each time the packet receive callback fucntion will verify the expected
 *      received port number and received packet data, and disable that received
 *      egress_port[], let the port link status down, next time received the same
 *      packet flow, per DLB feature, it expect to choose another valid assignment.
 *
 *         ingress_port:
...
pktioRxCallback(int unit, bcm_pktio_pkt_t * packet, void *cookie)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_enable_set(unit, port, FALSE));
