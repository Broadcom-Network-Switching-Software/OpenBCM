/*
 * Feature  : Flex counter to demonstrate usage of slim mode
 *
 * Usage    : BCM.0> cint h4_hsdk_flexctr_slim_mode.c
 *
 * Config   : th4_hsdk_flexctr_64x400.config.yml
 *
 * Log file : th4_hsdk_flexctr_slim_mode_log.txt
 *
 * Test Topology :
 *                      +----------------------+
 *                      |                      |
 ...
 * Summary:
 * ========
 * cint script to show usage of slim mode counters. In Slim mode, each counter
 * pool entry supports three 24 bit narrow width counters. Only one out of 3 can
 ...
 *        - IngPort is selected as the source and it is configured in slim mode
 *
 ...
 *   ===============================================
 *     a) Send the packet through selected ingress port
 *
 *     b) Expected Result:
 *     ===================
...
/* Configures the flex counters to count in slim mode */

int
configure_flex_counter_slim_mode(int unit)
{
...
int test_setup(int unit)
{
    ...
    /* Configure flex counters to count in slim mode */
    rv = configure_flex_counter_slim_mode(unit);
    if (BCM_FAILURE(rv)) {
...
test_verify(int unit)
{
    ...
    /*    Vlan creation and adding of ports is just for verification purpose and
     *    not a needed step
     */
    ...
    /* print counter value. */
    printf("Ingress port %d : %d packets \n", ingress_port,
