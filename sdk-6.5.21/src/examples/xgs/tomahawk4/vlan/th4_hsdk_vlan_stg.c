/*  Feature  : VLAN spanning tree group
 *
 *  Usage    : BCM.0> cint th4_hsdk_vlan_stg.c
 *
 *  config   : th4_hsdk_vlan_config.yml
 *
 *  Log file : th4_hsdk_vlan_stg_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *  Summary:
 *  ========
 *    This CINT configures per port per stg (spanning tree group) forwarding state using BCM APIs.
 *    Packet forwarding is verified after each per port per stg forwarding state change
 *
 ...
 *         a.3) set egress_port2 to BLOCKING in the stg and repeat (a.1), expect the packet to be
 *              forwarded to egress_port1 only
 *         a.4) set egress_port4 to BLOCKING in the stg and repeat (a.2), expect the packet to be
...
bcm_error_t testVerify(int unit)
{
    ...
    printf("\n----------------------------------------------------------- \n");
    printf("    RESULT OF STG VERIFICATION  ");
    printf("\n----------------------------------------------------------- \n");
    ...
    bcm_stg_stp_set(unit, stg1, egress_port2, BCM_STG_STP_BLOCK);
    ...
    bcm_stg_stp_set(unit, stg1, egress_port4, BCM_STG_STP_BLOCK);
