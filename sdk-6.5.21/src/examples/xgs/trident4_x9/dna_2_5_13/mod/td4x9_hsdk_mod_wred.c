...
/*
 * Feature  : Mirror-On-Drop, packets dropped in MMU due to WRED drop are 
 * able to be mirrored to an MTP and encapsulated with an IPv4 PSAMP header.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_mod_wred.c
 *
 * config   : BCM956780-56780_A0-mod-config.yml
 *
 * Log file : td4x9_hsdk_mod_wred_log.txt
 *
 * Test Topology :
 *
 *                                +----------------------+      
...
Header Info
  ...
  ###[ PSAMP ]###     
     ...
     Ingress port= 1
     Drop reason= 0xb >> 1 = 5 (Egress WRED drop)
     variable flag= 0xff
  ###[ Ethernet ]###
                 dst= 00:00:00:00:aa:aa
                 src= 00:00:00:00:bb:bb
...
data[0050]: 0000 0014 0001 000b 0000 00ff 005e 0000
...
config_mod(int unit)
{
    ...
    /* WRED Configurations */
    BCM_IF_ERROR_RETURN(mod_wred_setup(unit, ingress_port[4]));
