...
/*
 * Feature  : Verify ingress vlan membership check
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vlan_ingress_membership_check.c
 *
 * config   : bcm56780_a0-generic-vlan.config.yml
 *
 * Log file : td4x9_hsdk_vlan_ingress_membership_check_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate ingress vlan membership check
 *
 ...
 *
 *  Egress Packet:
 *  No packet egress seen after removing ingress port from vlan membership
 */
...
verify(int unit)
{
    ...
    printf("\nDelete the ingress port from its vlan member to check packet drop due to vlan membership check fail\n");
    rv = bcm_vlan_gport_delete(unit, 20, ingress_phy_gport);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_gport_delete: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    print "show c";
    bshell(unit, "show c");
    bcm_pkt_drop_event_t drop_type = bcmPktDropEventIngressMembershipCheckFailed;
    rv = bcm_stat_drop_event_counter_get(unit, drop_type, &val);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_stat_drop_event_counter_get: %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("\n\nReading ingress vlan membership check drop event counter(bcmPktDropEventIngressMembershipCheckFailed) :: ");
    print val;
...
setup_vlan_switching(int unit)
{
    ...
    rv = bcm_vlan_gport_add(unit, vlanId, ingress_phy_gport, BCM_VLAN_GPORT_ADD_INGRESS_ONLY);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_gport_add: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_vlan_gport_add(unit, vlanId, egress_phy_gport, BCM_VLAN_GPORT_ADD_EGRESS_ONLY);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_gport_add: %s\n", bcm_errmsg(rv));
        return rv;
    }
