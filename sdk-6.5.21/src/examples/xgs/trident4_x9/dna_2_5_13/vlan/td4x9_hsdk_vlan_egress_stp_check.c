...
/*
 * Feature  : Verify vlan based egress STP checks
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vlan_egress_stp_check.c
 *
 * config   : bcm56780_a0-generic-vlan.config.yml
 *
 * Log file : td4x9_hsdk_vlan_egress_stp_check_log.txt
 *
 * Test Topology :
 *
 *                      +-------------------+
 ...
 * Summary:
 * ========
 * cint script to verify vlan based egress STP config
 *
 ...
 *
 *   Egress Packet:
 *   No Egress packet seen as egress port is in STP block state for the vlan      
*/
...
verify(int unit)
{
    ...
    printf("\nConfiguring egress port in STP block state\n");
    rv = bcm_vlan_stp_set(unit, 20, egress_port, BCM_PORT_STP_BLOCK);
    if (BCM_FAILURE(rv)) {
    ...
    bshell(unit, "ps 1");
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000001100000000000581000014080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 2", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
    bshell(unit, "l2 show");
    ...
    uint64 val;
    bcm_pkt_drop_event_t drop_type = bcmPktDropEventEgressStgBlockDrop;
    rv = bcm_stat_drop_event_counter_get(unit, drop_type, &val);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_stat_drop_event_counter_get: %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("\n\nReading egress spanning tree check failed drop event counter(bcmPktDropEventEgressStgBlockDrop) :: ");
    print val;
...
setup_vlan_switching(int unit)
{
    ...
    rv = bcm_vlan_stp_get(unit, vlanId, ingress_port, &stp_state);
    if (BCM_FAILURE(rv)) {
        printf("Error executing bcm_vlan_stp_get(): %s.\n", bcm_errmsg(rv));
        return rv;
    }
    printf("\nPort %d in VLAN id %d had a STP state set to %d\n", ingress_port, vlanId, stp_state);

    rv = bcm_vlan_stp_get(unit, vlanId, egress_port, &stp_state);
    if (BCM_FAILURE(rv)) {
        printf("Error executing bcm_vlan_stp_get(): %s.\n", bcm_errmsg(rv));
        return rv;
    }
    printf("Port %d in VLAN id %d had a STP state set to %d\n", egress_port, vlanId, stp_state);
