...
/*
 * Feature  : L2 switching for outer tagged packets by configuring VFI from vlan range.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vlan_vfi_from_vlan_range.c
 *
 * config   : bcm56780_a0-generic-vlan.config.yml
 *
 * Log file : td4x9_hsdk_vlan_vfi_from_vlan_range_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * packets on switch using VFI configuration based on vlan range feature.
 *
 ...
 *  Ingress Packet:
 *  [bcmPWN.0]Packet[1]: data[0000]: {000000000011} {000000000002} 8100 0078
 ...
 *
 *  Ingress Packet:
 *  [bcmPWN.0]Packet[3]: data[0000]: {000000000011} {000000000003} 8100 0096
 ...
 *
 *  Egress Packet:
 *  [bcmPWN.0]Packet[4]: data[0000]: {000000000011} {000000000003} 8100 0014
...
verify(int unit)
{
    ...
    bshell(unit, "l2 show");
    bshell(unit, "clear c");
    /* sending packets outer vid in between vlan range 100 to 200 */
    printf("\nSending vlan id 120 packet to ingress port:%d\n",ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000001100000000000281000078080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 2", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
    ...
    printf("\nSending vlan id 150 packet to ingress port:%d\n",ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000001100000000000381000096080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 2", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
...
/*configure vlan xlate keys to derive the forwarding VFI for VLAN compression*/
bcm_error_t
vlan_range_xlate_setup(int unit,bcm_gport_t port,bcm_vlan_t outer_vlan_min, bcm_vlan_t outer_vlan_max, bcm_vlan_t fwd_vfi)
{
    bcm_error_t rv = BCM_E_NONE;
    ...
    rv = bcm_vlan_translate_action_add(unit, port_class_2,
                            bcmVlanTranslateKeyPortGroupOuter, outer_vlan_min, BCM_VLAN_INVALID, &action);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_translate_action_add: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_vlan_translate_action_range_add(unit, port, outer_vlan_min, outer_vlan_max, BCM_VLAN_INVALID, BCM_VLAN_INVALID, NULL);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_translate_action_range_add: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
{
    bcm_vlan_t fwd_vfi=20, pkt_outer_vlan_min=100, pkt_outer_vlan_max=200;
    bcm_error_t rv = BCM_E_NONE;
    bcm_mac_t dst_mac = "00:00:00:00:00:11";

    printf("create vlan %d\n", fwd_vfi);
    rv = bcm_vlan_create(unit, fwd_vfi);
    if (BCM_FAILURE(rv)) {
    ...
    /*Add ports to vlan*/
    rv = vlan_add_port(unit, fwd_vfi, ingress_port, 0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vlan_add_port: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = vlan_add_port(unit, fwd_vfi, egress_port, 0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vlan_add_port: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = vlan_range_xlate_setup(unit, ingress_port, pkt_outer_vlan_min, pkt_outer_vlan_max, fwd_vfi);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vlan_range_xlate_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = add_l2_static_entry(unit, 0, dst_mac, fwd_vfi,  egress_port);
    if (BCM_FAILURE(rv)) {
