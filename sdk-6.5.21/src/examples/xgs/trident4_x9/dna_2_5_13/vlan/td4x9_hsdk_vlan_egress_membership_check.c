...
/*
 * Feature  : Verify egress vlan membership check 
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vlan_egress_membership_check.c
 *
 * config   : bcm56780_a0-generic-vlan.config.yml
 *
 * Log file : td4x9_hsdk_vlan_egress_membership_check_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate egress vlan membership check
 *
...
verify(int unit)
{
    ...
    rv = bcm_vlan_gport_delete(unit, 20, egress_phy_gport);
    if (BCM_FAILURE(rv)) {
    ...
    print "show c";
    bshell(unit, "show c");
    bcm_pkt_drop_event_t drop_type = bcmPktDropEventIngressEgrMembershipCheckFailed;
    rv = bcm_stat_drop_event_counter_get(unit, drop_type, &val);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_stat_drop_event_counter_get: %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("\n\nReading egress vlan membership check drop event counter(bcmPktDropEventIngressEgrMembershipCheckFailed) :: ");
    print val;
