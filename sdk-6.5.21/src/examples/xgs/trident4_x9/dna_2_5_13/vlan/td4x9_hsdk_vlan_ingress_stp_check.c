...
/*
 * Feature  : Verify vlan based ingress STP checks
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vlan_ingress_stp_check.c
 *
 * config   : bcm56780_a0-generic-vlan.config.yml
 *
 * Log file : td4x9_hsdk_vlan_ingress_stp_check_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to verify vlan based ingress STP config
 *
 ...
 *
 *   Egress Packet:
 *   No Egress packet seen as ingress port is in STP block state for the vlan
 */
...
verify(int unit)
{
    ...
    rv = bcm_vlan_stp_set(unit, 20, ingress_port, BCM_PORT_STP_BLOCK);
    if (BCM_FAILURE(rv)) {
    ...
    uint64 val;
    bcm_pkt_drop_event_t drop_type = bcmPktDropEventIngressSpanningTreeCheckFailed;
    rv = bcm_stat_drop_event_counter_get(unit, drop_type, &val);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_stat_drop_event_counter_get: %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("\n\nReading ingress spanning tree check failed drop event counter(bcmPktDropEventIngressSpanningTreeCheckFailed) :: ");
    print val;
