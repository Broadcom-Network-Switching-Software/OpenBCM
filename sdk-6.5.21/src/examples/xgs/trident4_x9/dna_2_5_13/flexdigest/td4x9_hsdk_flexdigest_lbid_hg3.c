 ...
 *
 *  Usage    : BCM.0> cint td4x9_hsdk_flexdigest_lbid_hg3.c
 *
 *  config   : bcm56780_a0-generic-48x100-8x400_flexdigest-mc.config.yml
 *
 *  Log file : td4x9_hsdk_flexdigest_lbid_hg3_log.txt
 *
 *  Test Topology :
 *      +-----------------+            +---------+
 *      |   (hg3)-ing-port|<-----------+         |
...
portNumbersGet(int unit, int *port_list, int num_ports)
{
    ...
    for (i = 1; i < BCM_PBMP_PORT_MAX; i++) {
        if (BCM_PBMP_MEMBER(&ports_pbmp, i)) {
            ...
            rv = bcm_port_resource_speed_get(0,i,&resource);
            if (BCM_FAILURE(rv)) {
