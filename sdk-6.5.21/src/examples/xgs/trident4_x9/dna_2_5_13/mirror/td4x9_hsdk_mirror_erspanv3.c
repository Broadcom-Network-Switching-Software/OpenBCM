...
/*
 * Feature  : Mirroring over L3 tunnel(ERSPANv3)
 *                                           
 * Usage    : BCM.0> cint td4x9_hsdk_mirror_erspanv3.c
 *                                               
 * config   : bcm56780_a0-generic-48x100-8x400_mirror.config.yml
 *                                                 
 * Log file : td4x9_hsdk_mirror_erspanv3_log.txt        
 *                                                 
 * Test Topology :                                 
 #                +---------------------------+    
 #                |                           |    
 ...
 * Summary:
 * ========
 * Cint example to demonstrate ERSPANv3 Mirroing.
 *
 ...
 *
 *   2) Step2 - Configuration (Done in config_erspanv3_mirror()):
 *   =========================================================
 *     a) enable ingress mirroring on ingress_port and make cpu_port as MTP
 *     b) Set MTP.flags as "BCM_MIRROR_DEST_TUNNEL_IP_GRE"
 *     c) configure erpsanv3 header values i.e gre_protocol,sequence_num dst_mac, src_mac, src_ip, dst_ip etc
 *
 ...
 *       [bcmPWN.0]Packet[1]: data[0030]: 03ff 0000 0000 0000 83f1 1404 0002 0000
...
/* config erspanv3 mirror */
bcm_error_t
config_erspanv3_mirror(int unit)
{
    int erspan_vid=100;
    int modid;
    bcm_gport_t gport;
    ...
    uint16 gre_protocol = 0x22eb; /* GRE Protocol Type value for ERSPANv3 */
    uint32 gre_seq_num = 10; /* Sequence number value used in GRE header. If no
                                value is provided,gre_seq_number will start with
                                0. Valid only if BCM_MIRROR_DEST_TUNNEL_IP_GRE
                                is set. */
    uint8 truncated_flag = 0; /* This should be set if ERSPAN encapsulated frame
                                 exceeds the configured MTU */
    uint16 session_id = 0x3ff;  /* Id associated with each ERSPAN session */   
    uint8 hw_id = 0x3f; /* Unique Id of an ERSPAN engine within a system */
    uint8 opt_hdr = 1; /* Indicates if the optional platform-specific sub-header
                          is present */
    uint8 platform_id = 0x5; /* Platform Id to parse the sub-header. Valid only
                                if optional_hdr is set */
    uint16 switch_id = 4; /* Identifies a source switch at the receiving end.
    ...
    mtp.gre_protocol = gre_protocol;
    mtp.gre_seq_number = gre_seq_num;
    mtp.erspan_header.truncated_flag = truncated_flag;
    mtp.erspan_header.session_id = session_id;
    mtp.erspan_header.hw_id = hw_id;
    mtp.erspan_header.optional_hdr = opt_hdr;
    mtp.erspan_header.platform_id = platform_id;
    mtp.erspan_header.switch_id = switch_id;
 ...
 *  This function does the following
 *  a) test setup
 *  b) actual configuration (Done in config_erspanv3_mirror())
...
execute(void)
{
    ...
    print "~~~ #2) config_erspanv3_mirror(): ** start **";
    if (BCM_FAILURE((rv = config_erspanv3_mirror(unit)))) {
        printf("config_erspanv3_mirror() failed.\n");
        return -1;
    }
    print "~~~ #2) config_erspanv3_mirror(): ** end **";
