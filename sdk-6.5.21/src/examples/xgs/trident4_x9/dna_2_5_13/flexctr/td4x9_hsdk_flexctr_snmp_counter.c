 ...
 *
 * Usage    : BCM.0> cint td4x9_hsdk_flexctr_snmp_counter.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_flexctr.config.yml
 *
 * Log file : td4x9_hsdk_flexctr_snmp_counter_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *
 *   2) Step2 - Configuration (Done in testConfigure()):
 *   ===============================================
 *     a) Create Field group with Ethertype as part of QSET and 
 *           StatGroupWithoutCounterIndex as part of ASET
 *     b) Configure Flex counters with appropriate paremeters giving field
 *           group id as hint during flex counter action create
 *     c) Create field entries and attach flex counters to these entries
 * 
 ...
 *   ===============================================
 *     a) Transmit 1 IPv4 and 1 IPv6 packet and check the 
 *        bcm_flexctr_stat_get for snmpv4 and snmpv6 
 *     b) Expected Result:
 *     ===================
 *     The output of bcm_flexctr_stat_get should match the number of transmitted 
 *     packets
 *     SNMPV4 FlexCtr Get for port 78 : 1 packets / 78 bytes
 *     SNMPV6 FlexCtr Get for port 78 : 1 packets / 78 bytes
 */
...
int execute()
{
  ...
  if (BCM_FAILURE(rv)) {
    printf("testConfigure() failed, %s.\n", bcm_errmsg(rv));
    return rv;
  }
  printf("Completed test configure successfully.\n");
