...
/*
 * Feature  : Drop Event, mirror 'Ingress Pipeline: Ingress Membership Check Failure' 
 *            packets to MTP (Mirror-to-Port).
 *
 * Usage    : BCM.0> cint td4x9_hsdk_dropevent_ip_mirror_to_mtp.c
 *
 * Config   : bcm56780_a0-generic-48x100-8x400_event.config.yml
 *
 * Log file : td4x9_hsdk_dropevent_ip_mirror_to_mtp_log.txt
 *
 * Test Topology :
 *
 *
 ...
 * Summary:
 * ========
 * CINT example to demonstrate "Drop Event" feature by mirroring 'Ingress Membership
 * Check Failure ' packets to MTP.
 *
 ...
 *       For the second packet, it is dropped but mirrored to MTP due to 'Ingress Membership
 *       Check Failure'.
...
config_event_action(int unit)
{
    ...
    /* MIRROR-TO-MTP for Drop event 'Ingress Membership Check Failure'. */
    printf("Enable the specific drop event and mirror it to MTP.\n");
    /* Mirror Port */
    ...
    /* Setup a Mirror source about bcmPktDropEventIngressMembershipCheckFailed event */
    bcm_mirror_source_t source;
    bcm_mirror_source_t_init(&source);
    source.type = bcmMirrorSourceTypePktDropEvent;
    source.drop_event = bcmPktDropEventIngressMembershipCheckFailed;
