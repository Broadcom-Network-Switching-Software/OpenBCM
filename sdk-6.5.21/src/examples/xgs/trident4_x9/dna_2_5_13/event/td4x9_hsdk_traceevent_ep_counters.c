...
/*
 * Feature  : Trace Event, trigger the specific trace event in egress pipeline and check 
 *            it is properly counted to the related Trace Event counter.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_traceevent_ep_counters.c
 *
 * Config   : bcm56780_a0-generic-48x100-8x400_event.config.yml
 *
 * Log file : td4x9_hsdk_traceevent_ep_counters_log.txt
 *
 * Test Topology :
 *
 *
 ...
 * Summary:
 * ========
 * CINT example to demonstrate "Trace Event" feature for counting the specific trace
 * event in egress pipeline to the related Trace Event Counter.
 *
 ...
 *     configuration.
 *
 *   2) Step2 - Configuration (Done in config_ep_trace_action()):
 *   =========================================================
 *     a) Configure the "Trace Event" configuration.
 *        This does the necessary configurations to create the trace condition for
 ...
 *
 *     b) Transmit two packets with DA=00:00:00:00:02:01/00:00:00:00:02:02,
 ...
 *           the packet is discarded at egress pipeline. The Trace Event counter (EFP) is increased.
 *           Counter 'bcmPktTraceEventEgressEfp' should be 1.
...
======
  ...
  ###[ Ethernet ]###
    dst= 00:00:00:00:02:01/00:00:00:00:02:02
...
//  bshell(unit, "hm ieee");
    ...
    for(i=0; i<sizeof(pkt_data)/512; i++)
    {
        ...
        uint64 val;
        rv = bcm_stat_trace_event_counter_sync_get(unit, bcmPktTraceEventEgressEfp, &val);
        uint32 val_low = COMPILER_64_LO(val);
        if (BCM_FAILURE(rv)) {
            printf("Error in getting the event counter: %s.\n", bcm_errmsg(rv));
        }
        else {
            printf("Counter 'bcmPktTraceEventEgressEfp': %x.\n", val_low);
        }
    }
...
bcm_error_t
config_ep_trace_action(int unit)
{
    int rv, i=0;
    ...
    /* Create some static L2 MAC addresses on egress_port[1]/[2] */
    printf("Create some static L2 MAC addresses on egress_port[1]/[2].\n");
    ...
    BCM_FIELD_ASET_ADD(group_config.aset, bcmFieldActionDrop);
    BCM_FIELD_ASET_ADD(group_config.aset, bcmFieldActionPktTraceIdx);
    ...
    rv = bcm_field_action_add(unit, entry, bcmFieldActionPktTraceIdx, 0, 0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in applying EFP actions: %s.\n", bcm_errmsg(rv));
        return rv;
    }
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_ep_trace_action())
...
execute(void)
{
    ...
    print "~~~ #2) config_ep_trace_action(): ** start **";
    if (BCM_FAILURE((rv = config_ep_trace_action(unit)))) {
        printf("config_ep_trace_action() failed.\n");
        return -1;
    }
    print "~~~ #2) config_ep_trace_action(): ** end **";
