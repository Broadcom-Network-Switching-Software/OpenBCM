...
/*
 * Feature  : Drop Event, copy 'Ingress Pipeline: Ingress Membership Check Failure' packets to CPU.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_dropevent_ip_copy_to_cpu.c
 *
 * Config   : bcm56780_a0-generic-48x100-8x400_event.config.yml
 *
 * Log file : td4x9_hsdk_dropevent_ip_copy_to_cpu_log.txt
 *
 * Test Topology :
 *
 *
 ...
 * Summary:
 * ========
 * CINT example to demonstrate "Drop Event" feature by copying 'Ingress Membership
 * Check Failure' packets to CPU.
 *
 ...
 *        port is used as mirror-to port. Two egress ports and mirror-to port are
 *        also belonging to VLAN=24.
 *
 ...
 *        This does the necessary configurations of the "Drop Event" settings, VLAN,
 *        and L2 FDB.
 *
 ...
 *
 *     b) Transmit one packet with DA=00:00:00:aa:bb:cc/SA=00:00:00:00:00:01/VLAN=22 and
 *        one packet with DA=00:00:00:aa:bb:cc/SA=00:00:00:00:00:01/VLAN=24 to ingress port.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see that the first packet is flooded into other VLAN domain (egress_ports).
 *       For the second packet, it is dropped but copied to CPU due to 'Ingress Membership
 *       Check Failure'.
 *       Counter 'bcmPktDropEventIngressMembershipCheckFailed' should be 1
...
tx port=1 data=
...
verify(int unit)
{
    ...
    snprintf(pkt_data[1],512,"%s","");
...
//  bshell(unit, "hm ieee");
    ...
    uint64 val;
    rv = bcm_stat_drop_event_counter_sync_get(unit, bcmPktDropEventIngressMembershipCheckFailed, &val);
    uint32 val_low = COMPILER_64_LO(val);
    if (BCM_FAILURE(rv)) {
        printf("Error in getting the event counter: %s.\n", bcm_errmsg(rv));
    }
    else {
        printf("Counter 'bcmPktDropEventIngressMembershipCheckFailed': %x.\n", val_low);
    }
...
config_event_action(int unit)
{
    ...
    /* VLAN creation */
    printf("Create VLANs and add member ports.\n");
    rv = vlan_create_add_port(unit, vlan_1, ingress_port);
    if (BCM_FAILURE(rv)) {
        printf("\nError in VLAN create: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    for(i=1; i<VLAN_MEMBERS; i++) {
        rv = vlan_create_add_port(unit, vlan_1, egress_port[i]);
        if (BCM_FAILURE(rv)) {
            printf("\nError in VLAN create: %s.\n", bcm_errmsg(rv));
            return rv;
        }
    ...
    for(i=1; i<VLAN_MEMBERS; i++) {
        rv = vlan_create_add_port(unit, vlan_2, egress_port[i]);
        if (BCM_FAILURE(rv)) {
            printf("\nError in VLAN create: %s.\n", bcm_errmsg(rv));
            return rv;
        }
    }
    rv = vlan_create_add_port(unit, vlan_2, mirror_to_port);
    if (BCM_FAILURE(rv)) {
        printf("\nError in VLAN create: %s.\n", bcm_errmsg(rv));
        return rv;
    }
	...
	/* COPY-TO-CPU for Drop event 'Ingress Membership Check Failure'. */
    printf("Enable the specific drop event and copy to CPU.\n");
	bcm_switch_drop_event_mon_t drop_event_mon;
	bcm_switch_drop_event_mon_t_init(&drop_event_mon);
    drop_event_mon.drop_event = bcmPktDropEventIngressMembershipCheckFailed;
	drop_event_mon.actions    |= BCM_SWITCH_MON_ACTION_COPY_TO_CPU;
	rv = bcm_switch_drop_event_mon_set(unit, &drop_event_mon);
    if (BCM_FAILURE(rv)) {
        printf("\nError in Drop-Event config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
