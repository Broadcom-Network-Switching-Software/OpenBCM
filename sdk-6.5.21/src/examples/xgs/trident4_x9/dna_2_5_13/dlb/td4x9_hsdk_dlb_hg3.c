...
/*  Feature  : DLB configuration for Higig3 Trunk
 *
 *  Usage    : BCM.0> cint td4x9_hsdk_dlb_hg3.c
 *
 *  config   : bcm56780_a0-generic-48x100-8x400_dlb-mc.config.yml
 *
 *  Log file : td4x9_hsdk_dlb_hg3_log.txt
 *
 *  Test Topology :
 *      +-----------------+            +---------+
 *      |   (hg3)-ing-port|<-----------+         |
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of the DLB for Higig3
 *   LBID based UC and Non-UC trunk with FlexDigest hashing using BCM APIs.
 *
 ...
 *
 *
 *   2) Step2 - Configuration (Done in config_dlb_hg3_trunk()):
 *   =========================================================
 *     a) Configure a basic port, vlan, flexdigest with LBID based for Higig3
 *        trunk with DLB attributes functional scenario and does the necessary
 *        configurations.
 *
 ...
 *
 *     b) Transmit the few/many Non-UC packets from ingress-port(hg3). The contents of
...
portNumbersGet(int unit, int *port_list, int num_ports)
{
    ...
    for (i = 1; i < BCM_PBMP_PORT_MAX; i++) {
        if (BCM_PBMP_MEMBER(&ports_pbmp, i)) {
            /* exclude port with speed <= 10G as HG3 encap fails */ 
            rv = bcm_port_resource_speed_get(unit,i,&resource);
            if (BCM_FAILURE(rv)) {
                printf("\nError in evaluating port speed: %s.\n",bcm_errmsg(rv));
                return rv;
            } 
            if (resource.speed <= 10000) continue; 
            tempports[port] = i;
            port++;
        }
    }
...
configure_HashOutSelection(int unit, bcm_port_t port)
{
    ...
    /* Enable/Disable HiGig3 Entropy based hashing for non-unicast Trunk */
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit, bcmSwitchHashUseHigig3EntropyTrunkNonUnicast, 1));
    /* Hash offset for LBID : 112 - 127 */
    BCM_IF_ERROR_RETURN(bcm_switch_control_port_set(unit, port, bcmSwitchTrunkNonUnicastHashOffset, 112));
...
test_setup(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_encap_config_set(unit, ingress_port, &encap_config));
...
verify(int unit)
{
    ...
    printf("Verify %d UC pkts from port %d forwarding to ing-port(hg3)\n", pkt_cnt, ingress_port);
    bshell(unit, "sleep 2");
    for (i = 0; i < pkt_cnt; i++) {
    ...
    printf("Verify %d Non-UC pkts from port %d forwarding to ing-port(hg3)\n", pkt_cnt, ingress_port);
    bshell(unit, "sleep 2");
    for (i = 0; i < pkt_cnt; i++) {
        snprintf(str, 512, "tx 1 pbm=%d sm=00:00:00:cc:cc:cc dm=00:00:00:bb:bb:%02d vlan=11;sleep 1;", ingress_port, i);
        printf("%s\n", str);
        bshell(unit, str);
    }
    printf("Executing 'show c'\n");
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_dlb_hg3_trunk())
...
execute(void)
{
    ...
    print "~~~ #2) config_dlb_hg3_trunk(): ** start **";
    if (BCM_FAILURE((rv = config_dlb_hg3_trunk(unit)))) {
        printf("config_dlb_hg3_trunk() failed.\n");
        return -1;
    }
    print "~~~ #2) config_dlb_hg3_trunk(): ** end **";
