 ...
 *
 *  Usage    : BCM.0> cint td4x9_hsdk_dlb_lag.c
 *
 *  config   : bcm56780_a0-generic-48x100-8x400_dlb.config.yml
 *
 *  Log file : td4x9_hsdk_dlb_lag_log.txt
 *
 *  Test Topology :
 *      +-----------------+            +---------+
 *      |         ing-port|<-----------+         |
...
verify(int unit)
{
    ...
    for (i = 0; i < pkt_cnt; i++) {
        snprintf(str, 512, "tx 1 pbm=%d sm=00:00:00:cc:cc:%02d dm=00:00:00:dc:dd:de vlan=11;sleep 2;", ingress_port, i);
        printf("%s\n", str);
        bshell(unit, str);
    }
