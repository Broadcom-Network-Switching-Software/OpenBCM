...
/*
 * Feature  : Delete l2 entries by vlan function
 *
 * Usage    : BCM.0> cint td4x9_hsdk_l2_delete_entry_by_vlan.c
 *
 * config   : bcm56780_a0-generic-l2.config.yml
 *
 * Log file : td4x9_hsdk_l2_delete_entry_by_vlan_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 delete by vlan function for static entries present in L2 HOST TABLE
 *
 ...
 *     e) delete l2 entry by vlan and verify l2 table traverse
 ...
 *
 *   delete l2 entry from vlanId 20
 *   Show l2 entries:
 */
...
bcm_error_t
l2_delete_entry_by_vlan(int unit)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_module_t mod = 0;
    uint32 flags = BCM_L2_DELETE_STATIC;

    printf("\ndelete l2 entry from vlanId %d",vlanId);
    rv = bcm_l2_addr_delete_by_vlan(unit,vlanId,BCM_L2_DELETE_STATIC);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_l2_addr_delete_by_vlan: %s\n", bcm_errmsg(rv));
        return rv;
    }
    return BCM_E_NONE;
}
...
setup_l2_forwarding(int unit)
{
    ...
    vlan_control.forwarding_vlan = default_vlan;

    rv = bcm_vlan_control_vlan_selective_set(unit,vlanId,0,vlan_control);
    if (BCM_FAILURE(rv)) {
