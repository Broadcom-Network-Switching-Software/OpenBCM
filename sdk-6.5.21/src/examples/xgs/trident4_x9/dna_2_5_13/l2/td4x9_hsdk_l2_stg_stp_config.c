...
/*
 * Feature  : Verify vlan based STP,STG configuration
 *
 * Usage    : BCM.0> cint td4x9_hsdk_stg_stp_config.c
 *
 * config   : bcm56780_a0-generic-l2.config.yml
 *
 * Log file : td4x9_hsdk_stg_stp_config_log.txt
 *
 * Test Topology :
 *
 *
 * Summary:
 * ========
 * cint script to configure STP,STG
 *
 ...
 *
 *   Ingress Packet:
 *   1. Send unicast packet to ingress port 1 
 ...
 *   2. Configure ingress port 1 in STP block state for vlan 20 and send same packet again
 *   0000 0000 0011 0000 0000 0002 8100 0014
 *   0806 0001 0800 0604 0001 0000 0000 0002
 *   0a0a 0a02 0000 0000 0000 1414 1402 0001
 *   0203 0405 0607 0809 0a0b 0c0d bc94 977f
 *  
 *   Egress Packet:
 *   1. the same ingress packet be seen on egress port 2 which is memner of vlan 20
 *   0000 0000 0011 0000 0000 0002 8100 0014
 *   0806 0001 0800 0604 0001 0000 0000 0002
 *   0a0a 0a02 0000 0000 0000 1414 1402 0001
 *   0203 0405 0607 0809 0a0b 0c0d bc94 977f
 *   2. No packet egress out of port 2 as stp is in blocked state on ingress port 1  
 */
...
verify(int unit)
{
    ...
    rv = bcm_vlan_stp_set(unit, 20, ingress_port, BCM_STG_STP_BLOCK);
    if (BCM_FAILURE(rv)) {
    ...
    bshell(unit, "ps 1");
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000001100000000000581000014080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
    bshell(unit, "l2 show");
...
setup_l2_forwarding(int unit)
{
    ...
    printf("\nMapping vlan %d to stg group %d\n", vlanId, stg);
    rv = bcm_vlan_stg_set(unit, vlanId, stg);
    if (BCM_FAILURE(rv)) {
        printf("Error executing bcm_vlan_stg_set(): %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_vlan_stg_get(unit, vlanId, &stg);
    if (BCM_FAILURE(rv)) {
    ...
    rv = bcm_vlan_stp_set(unit, vlanId, ingress_port, BCM_STG_STP_FORWARD);
    if (BCM_FAILURE(rv)) {
        printf("Error executing bcm_vlan_stp_set(): %s.\n", bcm_errmsg(rv));
        return rv;
    }
