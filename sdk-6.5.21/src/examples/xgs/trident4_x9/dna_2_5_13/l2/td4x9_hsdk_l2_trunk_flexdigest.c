 ...
 *
 * Usage    : BCM.0> cint td4x9_hsdk_l2_trunk_flexdigest.c
 *
 * config   : bcm56780_a0-generic-l2.config.yml
 *
 * Log file : td4x9_hsdk_l2_trunk_flexdigest_log.txt
 *
 * Test Topology :
 ...
 *     b) Transmit 10 unicast packet with increment source mac on the ingress port side
 *
 *     c) Expected Result:
 *     ===================
 *     The packet capture shows the egress packet transmitted on either of the egress trunk member ports 
 */
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 10 pbm=%d DestMac=0x11 SourceMac=0x3214 SourceMacInc=1 vlan=20 len=80; sleep quiet 2", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
