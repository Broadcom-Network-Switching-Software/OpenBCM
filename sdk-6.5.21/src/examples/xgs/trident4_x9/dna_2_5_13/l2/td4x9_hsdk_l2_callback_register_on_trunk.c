 ...
 *
 * Usage    : BCM.0> cint td4x9_hsdk_l2_callback_register_on_trunk.c
 *
 * config   : bcm56780_a0-generic-l2.config.yml
 *
 * Log file : td4x9_hsdk_l2_callback_register_on_trunk_log.txt
 *
 * Test Topology :
 *
 *                      +-------------------+
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000011000000000002080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 2", ingress_port_1);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
    ...
    bshell(unit, "clear c");
    printf("\nSending unicast packet to ingress port:%d\n",ingress_port_2);
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000011000000000033080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 2", ingress_port_2);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
