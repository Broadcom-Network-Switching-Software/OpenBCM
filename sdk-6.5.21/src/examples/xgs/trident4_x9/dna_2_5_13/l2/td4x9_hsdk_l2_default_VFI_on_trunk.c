...
/*
 * Feature  : L2 switching for untagged packets on trunk port members
 *
 * Usage    : BCM.0> cint td4x9_hsdk_l2_default_VFI_on_trunk.c
 *
 * config   : bcm56780_a0-generic-l2.config.yml
 *
 * Log file : td4x9_hsdk_l2_default_VFI_on_trunk_log.txt
 *
 * Test Topology :
 *
 *                         +-----------------------+
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 forwarding for ingress untagged
 ...
 *        three ports, two ports is used as ingress_port and the one as
 *        egress_port.
 *
 ...
 *     b) Enable default vfi assignment at both ingress port
 *     c) L2 static entry is setup for unicast forwarding on egress port
 ...
 *     SMAC learning on both ingress ports will be on trunk group id.
 *
 *   Ingress Packet:
...
bcm_error_t
config_l2_trunk_rtag7(int unit, bcm_trunk_t trunk_id)
{
...
setup_l2_forwarding(int unit)
{
    ...
    rv = config_l2_trunk_rtag7(unit, trunk_id);
    if (BCM_FAILURE(rv)) {
