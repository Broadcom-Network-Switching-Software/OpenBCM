...
/*
 * Feature  : Delete l2 entries by port function
 *
 * Usage    : BCM.0> cint td4x9_hsdk_l2_delete_entry_by_port.c
 *
 * config   : bcm56780_a0-generic-l2.config.yml
 *
 * Log file : td4x9_hsdk_l2_delete_entry_by_port_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 delete by port function for static entries present in L2 HOST TABLE
 *
 ...
 *     e) delete l2 entry by port and verify l2 table traverse
...
bcm_error_t
l2_delete_entry_by_port(int unit)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_module_t mod = 0;
    uint32 flags = BCM_L2_DELETE_STATIC;

    printf("\ndelete l2 entry from port %d",egress_port);
    rv = bcm_l2_addr_delete_by_port(unit,mod,egress_port,BCM_L2_DELETE_STATIC);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_l2_addr_delete_by_port: %s\n", bcm_errmsg(rv));
        return rv;
    }
    return BCM_E_NONE;
}
