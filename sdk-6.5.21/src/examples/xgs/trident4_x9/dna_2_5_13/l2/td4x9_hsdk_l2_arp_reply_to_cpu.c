...
/*
 * Feature  : copy arp reply packets to cpu
 *
 * Usage    : BCM.0> cint td4x9_hsdk_l2_arp_reply_to_cpu.c
 *
 * config   : bcm56780_a0-generic-l2.config.yml
 *
 * Log file : td4x9_hsdk_l2_arp_reply_to_cpu_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000011000000000002080600010800060400020000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 2", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
    bshell(unit, "l2 show");
...
setup_l2_forwarding(int unit)
{
     ...
     * But on TD4.X9, station entry is not reqiuired for arp reply packets.
     */
