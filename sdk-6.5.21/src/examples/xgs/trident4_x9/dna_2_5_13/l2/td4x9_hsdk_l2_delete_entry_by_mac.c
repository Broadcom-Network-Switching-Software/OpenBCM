...
/*
 * Feature  : L2 forwarding for unknown unicast packets in vlan flooding domain
 *            when a static entry is deleted
 *
 * Usage    : BCM.0> cint td4x9_hsdk_l2_delete_entry_by_mac.c
 *
 * config   : bcm56780_a0-generic-l2.config.yml
 *
 * Log file : td4x9_hsdk_l2_delete_entry_by_mac_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |  egress_port_1
 *                      |        TD4.X9     +---------->
 *       ingress_port   |                   |  dst_mac 00:00:00:00:00:11
 *           ---------->+                   |  vfi 20
 *             vfi 20   |                   |
 *                      |                   |  egress_port_2
 *                      |                   +---------->
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 flooding for outer tagged
 * packets on switch when l2 entry is deleted
 *
 ...
 *     e) Delete the static entry and verify flooding on the vfi domain
 *
 ...
 *
 *   step: delete the l2 static entry to verify flooding in vfi domain
 *
 *   Ingress Packet: port 1
 ...
 *
 *   Egress Packet: port 2
 *   [bcmPWN.0]Packet[4]: data[0000]: {000000000011} {000000000002} 8100 0014
...
/*delete l2 static entry*/
bcm_error_t
l2_delete_entry(int unit)
{
    bcm_error_t rv = BCM_E_NONE;

    rv = bcm_l2_addr_delete_by_mac(unit,dst_mac,BCM_L2_DELETE_STATIC);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_l2_addr_delete_by_mac: %s\n", bcm_errmsg(rv));
        return rv;
    }
    return BCM_E_NONE;
}
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000001100000000000281000014080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 2", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
    ...
    /*deleting l2 static entry from l2 host table*/
    l2_delete_entry(unit);

    printf("verifying vlan flooding after deleting l2 static entry\n");
    print "l2 show";
    bshell(unit, "l2 show");
    bshell(unit, "clear c");
    printf("\nSending unknown unicast packet to ingress port:%d\n",ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000001100000000000281000014080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 2", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
...
bcm_error_t
add_l2_static_entry(int unit,uint32 flag,bcm_vpn_t vpn_id,bcm_port_t port)
{
    bcm_l2_addr_t l2_addr;
    bcm_l2_addr_t_init(&l2_addr, dst_mac, vpn_id);
...
/*configure vlan flooding domain*/
bcm_error_t
setup_l2_multicast_group(int unit,bcm_vlan_t vlanId)
{
    bcm_error_t rv = BCM_E_NONE;
    ...
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_multicast_create: %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("L2 MC group 0x%x added\n",l2mc_group);
    ...
    uint32_t flags = BCM_VLAN_CONTROL_VLAN_BROADCAST_GROUP_MASK |
    ...
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_vlan_control_vlan_selective_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
setup_l2_forwarding(int unit)
{
    ...
    /*configure vlan flooding domain*/
    rv = setup_l2_multicast_group(unit,vlanId);
    if (BCM_FAILURE(rv)) {
        printf("\nError in setup_l2_multicast_group: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    /*Configure static l2 entry for forwarding on port 1*/
    rv = add_l2_static_entry(unit,0,vlanId,egress_port_1);
    if (BCM_FAILURE(rv)) {
