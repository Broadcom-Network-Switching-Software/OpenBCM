...
/*  Feature  : L3 Ipv6 in Ipv4 Tunnel
 *
 *  Usage    : BCM.0> cint td4x9_hsdk_l3_ip_tunnel_ip6in4.c
 *
 *  config   : bcm56780_a0-generic-48x100-8x400_l3.config.yml
 *
 *  Log file : td4x9_hsdk_l3_ip_tunnel_ip6in4_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+
 ...
 *  Summary:
 *  ========
 *    This cint example demonstrates L3 Ipv6 in Ipv4 Tunnel initiation and termination
 *    using BCM APIs.
 *
 ...
 *
 *    2) Step2 - Configuration (Done in config_l3_tunnel_ip6in4())
 *    ============================================================
...
/*
 * Configure Ipv6 in Ipv4 Tunnel initiation and termination.
 */
bcm_error_t config_l3_tunnel_ip6in4(int unit)
{
  bcm_error_t        rv;
  ...
  bcm_ip6_t         tnl_init_dip6 = {0xAA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01}; /* AA00:0:0:0:0:0:0:1 */
  bcm_ip6_t         tnl_term_dip6 = {0xBB, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01}; /* BB00:0:0:0:0:0:0:1 */
  bcm_ip6_t         mask_v6 = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                               0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00};
  bcm_ip6_t       def_dip6 = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  bcm_ip6_t       def_dip6_mask = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  tnl_init_info.type = bcmTunnelTypeIp6In4; /* Choose Ipv6-in-Ipv4 tunnel type */
  ...
  tnl_term_info.type     = bcmTunnelTypeIp6In4; /* Choose Ipv6-in-Ipv4 tunnel type */
  ...
  route_d.l3a_flags   = BCM_L3_IP6;
  route_d.l3a_vrf     = vrf;
  route_d.l3a_intf    = l3_egr_def_obj_id;
  sal_memcpy(route_d.l3a_ip6_net, def_dip6, sizeof(def_dip6));
  sal_memcpy(route_d.l3a_ip6_mask, def_dip6_mask, sizeof(def_dip6_mask));
  ...
  sal_memcpy(route_2.l3a_ip6_net, tnl_term_dip6, sizeof(route_1.l3a_ip6_net));
  sal_memcpy(route_2.l3a_ip6_mask, mask_v6, sizeof(route_1.l3a_ip6_mask));
...
void verify(int unit)
{
 ...
 /*
  *  For IPv4 tunnel initiation, send the below IPv6 packet on access_port.
  *
  *    DA 0x1111
  *    SA 0x1
  *    VLAN 21
  ...
  */
  ...
  snprintf(str, 512, "tx 1 pbm=%d data=0x0000000011110000000000018100001586DD6030000000063BFFBB000000000000000000000000000001AA000000000000000000000000000001000000000000114C3F26; sleep quiet 1", access_port);
  bshell(unit, str);
  bshell(unit, "sleep 2");
  bshell(unit, "show c");
 ...
 /*
  *  4)For IPv4 Tunnel Termination, send the below packet on network_port.
  *
  *     DA 0x2222
  *     SA 0x2
  *     VLAN 22
  ...
  */
  ...
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000222200000000000281000016080045000042000000004029546E090909090A0A0A0A6030000000063BFFAA000000000000000000000000000001BB000000000000000000000000000001000000000000014B4D98; sleep quiet 1", network_port);
  bshell(unit, str);
  bshell(unit, "sleep 2");
  bshell(unit, "show c");
}
 ...
 * a)test setup
 * b)actual configuration (Done in config_l3_tunnel_ip6in4())
...
bcm_error_t execute()
{
  ...
  if (BCM_FAILURE((rv = config_l3_tunnel_ip6in4(unit)))) {
    printf("config_l3_tunnel_ip6in4() failed.\n");
    return -1;
  }
  printf("Completed configuration(i.e executing config_l3_tunnel_ip6in4()) successfully.\n");
