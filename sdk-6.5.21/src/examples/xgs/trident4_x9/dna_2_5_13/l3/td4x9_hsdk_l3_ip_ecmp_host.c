...
/*
 * Feature  : L3 IPv4 Unicast host pointing ECMP
 *
 * Usage    : BCM.0> cint td4x9_hsdk_l3_ip_ecmp_host.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_l3.config.yml
 *
 * Log file : td4x9_hsdk_l3_ip_ecmp_host_log.txt
 *
 * Test Topology :
 *
 *                   +------------------------------+
 ...
 *                   |                              |      +-----------------+
 *                   |                              |      |
 *  ingress_port cd0 |                              | ECMP |egress_port_2 cd2
 *  +----------------+          DUT                 +------+-----------------+
 *                   |                              |      |
 *                   |                              |      |egress_port_3 cd3
 *                   |                              |      +-----------------+
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of the IPv4 host entry pointing ECMP scenario
 *   using BCM APIs.
 *
 ...
 *
 *      f) Create L3 egress interface and L3 egress object for egress side_1.
 *
 *      g) Create L3 egress interface and L3 egress object for egress side_2.
 *
 *      h) Create L3 egress interface and L3 egress object for egress side_3.
 *
 *      i) Create L3 egress ECMP object with above three L3 egress objects as members.
 *
 *      j) Add host entry for DIP by pointing L3 egress ECMP object.
 *
 *      k) Configure flex digest for ECMP object hashing configuration.
 *
 ...
 *        'l3 intf show', 'l3 egress show', 'l3 host show', 'l3 ecmp show'
 *        and 'l3 host show'
 *
 ...
 *       through the L3 forwarding logic and different ECMP member is selected for
 ...
 *         5453 1a65
 *
 *     c3) After step 3.b3, verify that the below packet egress out of cd2
 *
 *         DMAC 0x33
 *         SMAC 0x03
 *         VLAN 0xd
 ...
 *         SIP 20.20.20.23
 *
 *         0000 0000 0033 0000 0000 0003 8100 000d
 *         0800 4500 002e 0000 0000 40ff 3d93 1414
 *         1417 0a0a 0a0a 0000 0000 0000 0000 0000
 *         0000 0000 0000 0000 0000 0000 0000 0000
 *         ca9a 1395
...
/* Flex digest configuration for ECMP hash selection */
...
/*
 * IP ECMP pointing host configuration.
 */
bcm_error_t config_ip_ecmp(int unit)
{
  ...
  bcm_l3_ecmp_member_t    ecmp_member_array[3];
  bcm_l3_egress_ecmp_t    ecmp_grp;
  int                     ecmp_member_count = 3;
  ...
  bcm_l3_ecmp_member_t_init(&ecmp_member_array[2]);
  ecmp_member_array[2].egress_if = egr_obj_out_3;
  ...
  /* Add host entry with ECMP object */
