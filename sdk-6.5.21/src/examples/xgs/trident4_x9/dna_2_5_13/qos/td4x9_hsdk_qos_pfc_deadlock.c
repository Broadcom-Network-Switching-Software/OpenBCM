 ...
 *
 * Usage          : BCM.0>cint td4x9_hsdk_qos_pfc_deadlock.c
 *
 * Config         : bcm56780_a0-generic-qos.config.yml
 *
 * Log file       : td4x9_hsdk_qos_pfc_deadlock_log.txt
 *
 * Test Topology  :
 *
 * DMAC=0x1            +------------+         DMAC=0x1
 * SMAC=0x2   +-------->   56780    +-------> SMAC=0x2
 * VLAN=2,Pri=0  ce0(1)|            |ce1(2)   VLAN=2,Pri=0
 *                     | l2:mac=0x1 |
 *                     |    vlan=2  |
 *      <-------+      |    port=2  |     <-------+
 ...
 *  Summary:
 *  ========
 *  This cint example demonstrates PFC Deadlock Detection feature
 *
 ...
 *    ==========================================
 *      a) send at linerate to ce0 L2 learnt packets (vlan=1 prio=2 dmac=0x1 smac=0x2)
 *      b) Along with a), send at linerate to ce1 PFC frames with pause-control class 0 enabled
 *      c) Expected Result:
 *         ================
 *         After (a) and (b)
 *         For the time interval for which PFC deadlock recovery is enabled, port ce1 should
 ...
 *         Port ce1 should not egress any packets as pfc xoff frames are being
 *         received. In 'show c' output, there should not be any Tx on ce1 (port-2).
 */
...
bcm_error_t pfc_deadlock_recovery_setup(int unit, bcm_port_t egr_port)
{
  ...
  bshell (0, "echo 'Show c to indicate packets are not drained from queue due to PFC XOFF received on ce1'");
  bshell(0, "clear c");
  bshell(0, "show c");
  ...
  bshell (0, "echo 'Show c to indicate packets drained of PFC queues due to deadlock recovery on ce1'");
  bshell(0, "show c");
  return BCM_E_NONE;
}
