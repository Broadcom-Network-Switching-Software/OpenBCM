 ...
 *
 * Usage          : BCM.0>cint td4x9_hsdk_qos_pfc.c
 *
 * Config         : bcm56780_a0-generic-qos.config.yml
 *
 * Log file       : td4x9_hsdk_qos_pfc_log.txt
 *
 * Test Topology  :
 *
 * DMAC=0x1            +------------+         DMAC=0x1
 * SMAC=0x2   +-------->   56780    +-------> SMAC=0x2
 * VLAN=2,Pri=2  ce0(1)|            |ce1(2)   VLAN=2,Pri=2
 *                     | l2:mac=0x1 |
 *                     |    vlan=2  |
 *      <-------+      |    port=2  |     <-------+
 ...
 *  Summary:
 *  ========
 *  This cint example demonstrates PFC feature
 *
 ...
 *      b) Along with a), send at linerate to ce1 PFC frames with pause-control class 2 enabled
 *      c) Expected Result:
 *         ================
 *         For test case mentioned in a)
 *         On ce0 (Port-1): verify that PFC frames should be transmitted at regular intervals as
 *         egress port is congested.
 *         In 'show c' output, PFC frame Tx should be seen on ce0 (port-1)
 *
 *         On ce1 (Port-2): packets should egress at configured rate
 *
 *         For test case mentioned in b)
 *         Port ce1 should not egress any packets as pfc pause frames are being
 *         received. In 'show c' output, there should not be any Tx on on ce1 (port-2)
 */
