...
/*
 * Feature  : DSCP+ECN remarking for L3 Ipv4 packets
 *
 * Usage    : BCM.0> cint td4x9_hsdk_qos_l3_dscp_ecn_remark.c
 *
 * config   : bcm56780_a0-generic-qos.config.yml
 *
 * Log file : td4x9_hsdk_qos_l3_dscp_ecn_remark_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 * Summary:
 * ========
 *  Cint example to show the qos configuration for L3 packets DSCP+ECN remarking
 *
 ...
 *
 *   Egress Packet:
 *   [bcmPWN.0]Packet[2]: data[0000]: {000000000022} {000000000011} 8100 200a
 *   [bcmPWN.0]Packet[2]: data[0010]: 0800 4517 002e 0000 0000 3fff 3e7f 1414
 ...
 *  0800 4517 002e 0000 0000 3fff 3e7f 1414
 *  1414 0a0a 0a0a 0000 0000 0000 0000 0000
 *  0000 0000 0000 0000 0000 0000 0000 0000
 *  f3b4 2e69
 */
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000020000000000018100000508004500002e0000000040ff3d96141414140a0a0a0a0000000000000000000000000000000000000000000000000000f3b42e69; sleep quiet 2", ingress_port);
    bshell(unit, str);
    bshell(unit, "l3 host show");
    bshell(unit, "show c");
...
bcm_error_t
dscp_ecn_remark_config_set(int unit, int *in_map, int *eg_map)
{
    ...
    ecn_map.ecn = 0;
    ecn_map.int_cn = 0;
    rv = bcm_ecn_traffic_map_set(unit, &ecn_map);
    if (BCM_FAILURE(rv)) {
    ...
    ecn_action.action_flags = BCM_ECN_TRAFFIC_ACTION_DEQUEUE_NON_CONGESTION_INT_CN_UPDATE;
    ecn_action.int_cn = 0;
    ecn_action.color = bcmColorGreen;
    ecn_action.non_congested_int_cn = 1;
    rv = bcm_ecn_traffic_action_config_set(unit, &ecn_action);
    if (BCM_FAILURE(rv)) {
    ...
    rv =  bcm_qos_map_create(unit, flags, &ing_map);
    if (BCM_FAILURE(rv)) {
    ...
    rv = bcm_qos_map_add(unit, flags, &map, ing_map);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_qos_map_add: %s\n", bcm_errmsg(rv));
        return rv;
    }

    flags = BCM_QOS_MAP_L3 | BCM_QOS_MAP_ECN | BCM_QOS_MAP_EGRESS;
    rv = bcm_qos_map_create(unit, flags, &egr_map);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_qos_map_create: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    map.pkt_ecn = 3;
    map.dscp = 5;
    rv = bcm_qos_map_add(unit, flags, &map, egr_map);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_qos_map_add: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
configure_ipuc_forwarding(int unit)
{
    ...
    /* dscp+ecn remarking */
    rv = dscp_ecn_remark_config_set(unit , &ing_map, &egr_map);
    if (BCM_FAILURE(rv)) {
