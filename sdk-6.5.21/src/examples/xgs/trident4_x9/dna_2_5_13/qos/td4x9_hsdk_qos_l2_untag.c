...
/*
 * Feature  : Qos mapping for L2 untagged packets
 *
 * Usage    : BCM.0> cint td4x9_hsdk_qos_l2_untag.c
 *
 * config   : bcm56780_a0-generic-qos.config.yml
 *
 * Log file : td4x9_hsdk_qos_l2_untag_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate dot1p, cfi remarking for L2 untagged packets.
 *
 ...
 *     d) default VFI and default pri/cfi is configured
 *
 ...
 *
 *   Egress Packet:
 *   0000 0000 0011 0000 0000 0002 8100 7014
 *   0806 0001 0800 0604 0001 0000 0000 0002
 *   0a0a 0a02 0000 0000 0000 1414 1402 0001
 *   0203 0405 0607 0809 0a0b 0c0d bc94 977f
 */
...
verify(int unit)
{
    ...
    bshell(unit, "l2 show");
    bshell(unit, "clear c");
    printf("\nSending untag unicast packet to ingress port:%d\n",ingress_port);
...
bcm_error_t
port_default_untag_config(int unit, bcm_port_t port, int vlan, uint8 priority, uint8 cfi)
{
    ...
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_port_untagged_priority_set: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_vlan_port_default_action_get(unit,port,&default_action);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_port_default_action_get: %s\n", bcm_errmsg(rv));
        return rv;
    }

    default_action.priority = priority;
    default_action.new_outer_cfi = cfi;
    default_action.forwarding_domain = vlan;
...
qos_port_setup(int unit, bcm_port_t port, unsigned int flags, uint8 internal_pri, bcm_color_t clr, uint8 priority, uint8 cfi)
{
    ...
    if(flags & BCM_QOS_MAP_INGRESS) {
        rv = bcm_qos_port_map_set(unit, port, qMapId, -1);
        if (BCM_FAILURE(rv)) {
        ...
        rv = bcm_qos_port_map_set(unit, port, -1, qMapId);
        if (BCM_FAILURE(rv)) {
            printf("\nError in bcm_qos_port_map_set: %s\n", bcm_errmsg(rv));
            return rv;
        }
...
bcm_error_t
setup_qos_remarking(int unit)
{
    bcm_vlan_t vlanId=20, default_vlan=20;
    bcm_mac_t dst_mac = "00:00:00:00:00:11";
    bcm_vlan_action_set_t default_action;
    uint8 default_priority=1, default_cfi=0;
    uint8 int_pri=12;
    uint8 new_pri=3, new_cfi=1;
    bcm_error_t rv = BCM_E_NONE;
    ...
    /*Setup port default vlan id, priority and cfi for untagged pakcets */
    rv = port_default_untag_config(unit, ingress_port, default_vlan, default_priority, default_cfi);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_default_untag_config: %s\n", bcm_errmsg(rv));
        return rv;
    }

    /* Setup L2 qos mapping */
    rv = qos_port_setup(unit, ingress_port, BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_L2, int_pri, bcmColorGreen, default_priority, default_cfi);
    if (BCM_FAILURE(rv)) {
        printf("\nError in ingress qos_port_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = qos_port_setup(unit, egress_port, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_L2, int_pri, bcmColorGreen, new_pri, new_cfi);
    if (BCM_FAILURE(rv)) {
        printf("\nError in egress qos_port_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }
    BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, 0, bcmPortControlEgressVlanPriSrc, bcmPortEgressVlanPriSrcIntPri));
 ...
 *  a) test setup
 *  b) actual configuration (Done in setup_qos_remarking())
...
execute(void)
{
    ...
    print "~~~ #2) setup_qos_remarking(): ** start **";
    if (BCM_FAILURE((rv = setup_qos_remarking(unit)))) {
        printf("setup_qos_remarking() failed.\n");
        return -1;
    }
    print "~~~ #2) setup_qos_remarking(): ** end **";
