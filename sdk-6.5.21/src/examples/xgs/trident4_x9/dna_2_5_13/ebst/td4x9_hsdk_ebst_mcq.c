...
/*
 * Feature  : EBST threshold profile configuration on a per mc queue per port basis
 *
 * Usage    : BCM.0> cint td4x9_hsdk_ebst_mcq.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_ebst.config.yml
 *
 * Log file : td4x9_hsdk_ebst_mcq_log.txt
 *
 * Test Topology :
 *
 *                  +---------------------+
 ...
 *                  |  | MCQ EBST_DATA |  +--------------+   |
 *                  |  +---------------+  |              +---+
 *                  |                     |        egress/observation port (egress rate limiting applied)
 *                  +---------------------+
 *
 * Summary:
 * ========
 * CINT example to demonstrate Even Buffer Statistics Tracking (EBST) capability (MCQ case)
 *
 ...
 *        - Enable BST counters
 *     c) Set threshold for MC Queue 8 (as well as UC Queue 0 for the ease of presentation) of the egress observation port
 *     d) Configure Monitoring on (Port,Queue)
 *     e) Set ebst monitor on egress Port (the observation port) MC Queue 8 (as well as UC Queue 0 for the ease of presentation)
 *     f) Configure the SCAN_ROUND and SCAN_MODE
 *     g) Start EBST scan
 *
 *   3) Step3 - Verification (Done in verify()):
 *   ===========================================
 *     a) CPU sends 20 1532-byte DLF packets to loopback ingress port  and
 *     b) Show port status and packet counts of ingress port and observation port on console by applying "ps" and "show c"
 *     c) Read the status of EBST_DATA buffer of the multicast queue with respect to the observation port
 *     d) Expected Result:
 *     ===================
 ...
 *     The above-mentioned information can be retrieved by reading EBST_DATA buffer of the multicast queue.
 */
...
test_setup(int unit)
{
    ...
    /* Apply egress rate limiting on the egress port */
...
void verify(int unit)
{
  ...
  /* CPU start to send DLF traffic */
  printf("\nCPU start sending DLF packet\n");
  bshell(unit, "vlan remove 1 pbm=cpu");
  snprintf(str, 512, "tx 20 pbm=%d length=1532 vlantag=0x1 sm=0x1 dm=0x2; sleep quiet 1", ingress_port);
  bshell(unit, str);
  ...
  /* STEP 1. Remove the queue from the EBST scan to read the status of EBST_DATA of the multicast queue.
             Disable ebst monitor on mc queue 0 of observation port */
  ...
  object_id.cosq = mc_cosq;
  object_id.buffer = 0;
  BCM_IF_ERROR_RETURN(bcm_cosq_ebst_data_get(unit, &object_id, bcmBstStatIdMcast, 100, stat_array, &array_count));
  printf("\nEBST_DATA Count ...\n");
  print array_count;
  bshell(unit, "sleep quiet 15");
  printf("\nEBST_DATA Status ...\n");
  print stat_array;
