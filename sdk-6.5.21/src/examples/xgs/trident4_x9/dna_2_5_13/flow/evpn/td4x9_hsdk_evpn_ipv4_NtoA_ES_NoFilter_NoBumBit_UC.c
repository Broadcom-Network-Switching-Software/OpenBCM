 ...
 *            termination flow for unicast traffic with NO BUM bit &
 *            NO ES filter (EFP).
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_NtoA_ES_NoFilter_NoBumBit_UC.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_NtoA_ES_NoFilter_NoBumBit_UC_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *
 *     b) Transmit the unicast packet with NO BUM BIT. The contents of the
 *        packet are printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see with no BUM bit, the tunnel terminated unicast packet from
 *       network port is L2 switched to access port 1 (egr_acc_port_1).
...
verify(int unit)
{
	...
	snprintf(str, 512, "tx 1 pbm=%d data=; sleep quiet 1", ing_net_port_1);
	bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
