...
/*
 * Feature  : Vxlan EVPN Bridging, IPv4 access to network tunnel
 *            initiation flow with PORT + OUTER VLAN + INNER VLAN as a flow
 *            match criteria.
 *
 * Usage    : BCM.0> cint Wtd4x9_hsdk_evpn_ipv4_AtoN_MatchPortOuterInnerVlan.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
*  Log file : td4x9_hsdk_evpn_ipv4_AtoN_MatchPortOuterInnerVlan_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *
 *
 *   In this device, ingress interface at access side is represented by L2 interface.
 *   L2_IIF at ingress direction while L2_OIF at egress direction. On the
 *   access-side incoming packet is learnt on L2_IIF.
 *   However, VFI may be assigned using any of the following match criteria
 *   while initiating a tunnel.
 *
 *   Port Group + Outer VLAN
 *       i.e BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_VLAN
 *   Port Group + Inner VLAN
 *       i.e BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_INNER_VLAN
 *   Port Group + Outer VLAN + Inner VLAN
 *       i.e BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_VLAN_STACKED
 *
 *   This example uses Port Group + Outer VLAN + Inner VLAN to assign VFI.
 *
 *   VP at network side is represented by SVP at ingress direction
 *   and DVP at egress direction.
 *
 ...
 *        enable EVPN, port learning, vlan, access and network port setting,
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see, sent packet SRC MAC is learnt on access port
 *       (BCM_PORT_LEARN_ARL),  a copy of packet to CPU (BCM_PORT_LEARN_CPU)
 *       and the tunnel packet on the egress port with the IPv4 encapsulation.
...
Ingress Hex Packet:
tx port=1 data=0x00000000AAAA00000000BBBB81000015910000050800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374
...
verify(int unit)
{
	...
	printf("\nSending known unicast packet to ingress_port access side:%d\n",
                                         ingress_port_1);
	snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA00000000BBBB81000015910000050800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ingress_port_1);
	bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
    ...
    BCM_IF_ERROR_RETURN(bcm_stat_get(unit, ingress_port_1,
    ...
    BCM_IF_ERROR_RETURN(bcm_stat_get(unit, egress_port_1,
                    ...
                    bcm_flow_handle_t flow_handle)
{
	...
	printf("VPN assignment, port_group + outer_VID + inner_VID -> VPN \n");
	bcm_flow_match_config_t_init(&match_info);
	match_info.criteria = BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_VLAN_STACKED;
	match_info.port     = vxlan_acc->port_class;
	match_info.vlan     = vxlan_acc->acc_vlan;
    match_info.inner_vlan = vxlan_acc->acc_inner_vlan;
