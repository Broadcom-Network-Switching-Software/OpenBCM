 ...
 *            initiation flow with PORT + INNER VLAN as a flow match criteria.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_AtoN_MatchPortInnerVlan.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_AtoN_MatchPortInnerVlan_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *
 *   This example uses Port Group + Inner VLAN to assign VFI.
 *
...
Ingress Hex Packet:
tx port=1 data=0x00000000AAAA00000000BBBB910000050800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374
...
verify(int unit)
{
	...
	printf("\nSending known unicast packet to ingress_port access side:%d\n",
                                         ingress_port_1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA00000000BBBB910000050800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ingress_port_1);
	bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
                    ...
                    bcm_flow_handle_t flow_handle)
{
	...
	printf("VPN assignment, port_group + inner_VID -> VPN \n");
	bcm_flow_match_config_t_init(&match_info);
	match_info.criteria = BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_INNER_VLAN;
