 ...
 *            initiation for known multicast, with PORT as a flow match
 *            criteria.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_AtoN_mc.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_AtoN_mc_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *             - access to network (encap flow) known multicast traffic
 *
 ...
 *
 *     b) Transmit the known multicast packets. The contents of the packets are
 *        printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see, sent multicast packet is flooded to network port with
 *       the IPv4 encapsulation and BUM is set in the VXLAN header.
