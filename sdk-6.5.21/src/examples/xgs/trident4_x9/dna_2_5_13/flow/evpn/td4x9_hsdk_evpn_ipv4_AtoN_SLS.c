 ...
 *            initiation flow to verify source lookup success (SLS).
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_AtoN_SLS.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_AtoN_SLS_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see, SRC HIT on the L2 static entry for the sent packet and
 *       the tunnel packet on the egress port with the IPv4 encapsulation.
...
config_vxlan_ipv4_vtep(int unit)
{
	...
	printf("Add payload-src-mac => Access-port .\n");
	rv = add_to_l2_table(unit, "00:00:00:00:bb:bb", vxlan_cfg.vxlan_vpn,
                         ingress_port_1);
	if (BCM_FAILURE(rv)) {
		printf("\nError in l2 entry config: %s.\n", bcm_errmsg(rv));
		return rv;
