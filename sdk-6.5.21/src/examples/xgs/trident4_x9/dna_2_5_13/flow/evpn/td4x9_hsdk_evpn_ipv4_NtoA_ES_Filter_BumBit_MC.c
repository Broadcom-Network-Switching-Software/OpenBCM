 ...
 *            termination flow for known multicast traffic with BUM bit &
 *            ES filter (EFP).
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_NtoA_ES_Filter_BumBit_MC.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_NtoA_ES_Filter_BumBit_MC_log.txt
 *
 * Test Topology :
 *
 *
 ...
 *             - network to access(decap flow) known multicast traffic
 *               with BUM bit & ES filter out of egr_acc_port_3 (EFP).
 *
 ...
 *
 *     b) Transmit the known multicast packet with BUM BIT. The contents of
 *        the packet are printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see with BUM bit & ES filter out of access port 3
 *       (egr_acc_port_3) using EFP, the tunnel terminated known multicast
...
Ingress Hex Packet:
tx port=1 data=
...
Packet Dump:
data[0000]: 0100 5e01 0101 0000 0000 eeee 8100 0015
...
verify(int unit)
{
    ...
    printf("MC Packet comes into network port %d, expect to foward to access port %d\n", ing_net_port_1, egr_acc_port_1);
	snprintf(str, 512, "tx 1 pbm=%d data=; sleep quiet 1", ing_net_port_1);
	bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
...
config_vxlan_ipv4_vtep(int unit)
{
	...
	rv = vxlan_evpn_EFP_pruning(unit, net_flow_port, egr_acc_port_3);
	if (BCM_FAILURE(rv)) {
