 ...
 *            initiation for unknown multicast, with PORT as a flow match
 *            criteria.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_AtoN_umc.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_AtoN_umc_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *                    Access port 2 (egr_acc_port_1)
 *                    Network port 1 (egr_net_port_1)
 ...
 *             - access to network (encap flow) unknown multicast traffic
 *
 ...
 *        enable EVPN, port learning, vlan, access port setting, network port
 *        setting, tunnel setup interface and next hop.
 *
 ...
 *
 *     b) Transmit the unknown multicast packets. The contents of the packets are
 *        printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see, sent multicast packet is flooded to network port and
 *       access port. In Netowrk port the egress packet is IPv4 encapsulation
 *       and BUM is set in the VXLAN header.
...
Ingress Hex Packet:
tx port=1 data=0x00000000AAAA00000000BBBB0800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374
...
bcm_gport_t   egr_net_gport_1;
bcm_port_t    ing_acc_port_1;
bcm_port_t    egr_acc_port_1;
bcm_port_t    egr_net_port_1;
...
test_setup(int unit)
{
	...
	if (BCM_E_NONE !=bcm_port_gport_get(unit, egr_net_port_1,
                                        &(egr_net_gport_1)))
    {
        printf("bcm_port_gport_get() failed to get gport for port %d\n",
                                                  egr_net_port_1);
    ...
    BCM_PBMP_PORT_ADD(pbmp, egr_net_port_1);
...
verify(int unit)
{
    ...
    COMPILER_64_ZERO(out_pkt);
    COMPILER_64_ZERO(out_bytes);
    BCM_IF_ERROR_RETURN(bcm_stat_get(unit, egr_net_port_1,
                                     snmpDot1dTpPortOutFrames, &out_pkt));
    BCM_IF_ERROR_RETURN(bcm_stat_get(unit, egr_net_port_1,
                                     snmpIfOutOctets, &out_bytes));
    printf("Tx : %d packets / %d bytes\n", COMPILER_64_LO(out_pkt),
