 ...
 *            initiation flow with PORT as a flow match criteria.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_AtoN_MatchPort.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_AtoN_MatchPort_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *        and vp's
 *
 *     b) Transmit the known unicast packets i.e untagged, single outer tagged,
...
verify(int unit)
{
    ...
    printf("\nSending single inner tagged known unicast packet to ingress_port access side:%d\n",
                                         ingress_port_1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA00000000BBBB9100000A0800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1",
                                         ingress_port_1);
    bshell(unit, str);
    bshell(unit, "sleep 2");
    bshell(unit, "show c");
    ...
    if (COMPILER_64_LO(out_pkt) != 4) {
        printf("Packet verify failed. Expected out packet 4 but get %d\n",
                                           COMPILER_64_LO(out_pkt));
        return -1;
    }
