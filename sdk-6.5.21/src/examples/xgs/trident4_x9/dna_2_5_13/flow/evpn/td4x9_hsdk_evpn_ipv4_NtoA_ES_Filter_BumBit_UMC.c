...
/*
 * Feature  : Vxlan EVPN Bridging, IPv4 network to access tunnel
 *            termination flow for unknown multicast traffic with BUM bit &
 *            ES filter (EFP).
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_NtoA_ES_Filter_BumBit_UMC.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_NtoA_ES_Filter_BumBit_UMC_log.txt
 *
 * Test Topology :
 *
 *
 ...
 *             - network to access(decap flow) unknown multicast traffic
 *               with BUM bit & ES filter out of egr_acc_port_2 (EFP).
 *
 ...
 *        enable EVPN, EFP ES filter, port learning, vlan, access port setting,
 *        network port setting, tunnel setup interface and next hop.
 *
 ...
 *
 *     b) Transmit the unknown multicast packet with BUM BIT. The contents of
 *        the packet are printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see with BUM bit & ES filter out of access port 2
 *       (egr_acc_port_2) using EFP, the tunnel terminated known multicast
 *       packet is multicast-ed to access port 1 (egr_acc_port_1) only.
...
Header Info
  ...
  ###[ VXLAN ]###
     ...
     reserved2= 0x0
  ###[ Ethernet ]###
                 dst= 01:00:5E:01:01:06
                 src= 00:00:00:00:EE:EE
                 type= 0x8100
  ###[ 802.1Q ]###
     vlan= 21
     type= 0x800
...
Ingress Hex Packet:
tx port=1 data=
...
Packet Dump:
data[0000]: 0100 5e01 0106 0000 0000 eeee 8100 0015
...
verify(int unit)
{
    ...
    printf("UMC Packet comes into network port %d, expect to foward to access port %d\n", ing_net_port_1, egr_acc_port_1);
	snprintf(str, 512, "tx 1 pbm=%d data=; sleep quiet 1", ing_net_port_1);
	bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
...
bcm_error_t
vxlan_evpn_EFP_pruning (int unit, bcm_gport_t net_flow_port, int egr_port)
{
    bcm_field_group_config_t group_config;
    bcm_field_entry_t        entry;
    bcm_port_t               port;

    /* ES filtering out of UC flow with ingress BUM set */
    bcm_field_group_config_t_init(&group_config);
    ...
    BCM_FIELD_QSET_ADD(group_config.qset, bcmFieldQualifyL2EgressInterface);
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_create(unit, group_config.group,
                                               &entry));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_SrcGport(unit, entry, net_flow_port));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_L2EgressInterface(unit, entry, egr_port, 0x3f));
...
config_vxlan_ipv4_vtep(int unit)
{
	...
	rv = vxlan_evpn_EFP_pruning(unit, net_flow_port, egr_acc_port_2);
	if (BCM_FAILURE(rv)) {
		printf("\nError in VXLAN EVP EFP config: %s.\n", bcm_errmsg(rv));
		return rv;
	}
