 ...
 *            initiation for known unicast, with PORT as a flow match
 *            criteria.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_AtoN_uc.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_AtoN_uc_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *             - access to network (encap flow) unicast traffic
 *
 ...
 *
 *     b) Transmit the known unicast packets. The contents of the packets are
 *        printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see, sent unicast packet is bridged to network port with
 *       the IPv4 encapsulation and BUM is not set in the VXLAN header.
