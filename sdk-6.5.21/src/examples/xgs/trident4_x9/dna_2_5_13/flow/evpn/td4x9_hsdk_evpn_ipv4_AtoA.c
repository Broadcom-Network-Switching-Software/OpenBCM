...
/*
 * Feature  : Vxlan EVPN Bridging, IPv4 access to access flow.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_evpn_ipv4_AtoA.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_evpn.config.yml
 *
 * Log file : td4x9_hsdk_evpn_ipv4_AtoA_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *        three ports, one port is used as ingress_port and the other ports as
 *        egress ports.
 *
 ...
 *        enable EVPN, port learning, vlan & access port setting.
 *
 ...
 *
 *     b) Transmit the known unicast, known multicast, unknown unicast,
 *        unknow multicast & broadcast packets. The contents of the packets are
 *        printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see, sent packets are switched / flooded to access ports.
...
verify(int unit)
{
    ...
    printf("\nSending known unicast packet to ingress_port access side:%d\n",
                                         ing_acc_port_1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA00000000BBBB810000150800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ing_acc_port_1);
    bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
    ...
    printf("\nSending known multicast packet to ingress_port access side:%d\n",
                                         ing_acc_port_1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x01005E01010100000000111181000015080045000050000040004011569901010101E101010100350035003C54F774657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ing_acc_port_1);
    bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
    ...
    printf("\nSending unknown unicast packet to ingress_port access side:%d\n",
                                         ing_acc_port_1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000CCDD00000000BBBB810000150800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ing_acc_port_1);
    bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
    ...
    printf("\nSending unknown multicast packet to ingress_port access side:%d\n",
                                         ing_acc_port_1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x01005E01010200000000111181000015080045000050000040004011569901010101E101010100350035003C54F774657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ing_acc_port_1);
    bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
    ...
    printf("\nSending broadcast packet to ingress_port access side:%d\n",
                                         ing_acc_port_1);
    snprintf(str, 512, "tx 1 pbm=%d data=0xFFFFFFFFFFFF00000000111181000015080045000050000040004011569901010101E101010100350035003C54F774657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ing_acc_port_1);
    bshell(unit, str);
	bshell(unit, "sleep 2");
	bshell(unit, "show c");
    ...
    if (COMPILER_64_LO(out_pkt) != 3) {
        printf("Packet verify failed. Expected out packet 3 but get %d\n",
                                           COMPILER_64_LO(out_pkt));
        return -1;
    }
                     ...
                     bcm_flow_handle_t flow_handle)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_multicast_encap_create(unit, &mc_encap, &encap_id));

    printf("UnKnown group: 0x%x\n", vxlan_cfg->bc_group);
    BCM_IF_ERROR_RETURN(bcm_multicast_egress_add(unit, vxlan_cfg->bc_group, vxlan_net->phy_Gport, encap_id));
    printf("Known MC group: 0x%x\n", vxlan_cfg->mc_group);
    BCM_IF_ERROR_RETURN(bcm_multicast_egress_add(unit, vxlan_cfg->mc_group, vxlan_net->phy_Gport, encap_id));
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    rv = add_l2_mc_entry(unit, "01:00:5e:01:01:01", vxlan_cfg.vxlan_vpn,
                         vxlan_cfg.mc_group);
    if (BCM_FAILURE(rv)) {
