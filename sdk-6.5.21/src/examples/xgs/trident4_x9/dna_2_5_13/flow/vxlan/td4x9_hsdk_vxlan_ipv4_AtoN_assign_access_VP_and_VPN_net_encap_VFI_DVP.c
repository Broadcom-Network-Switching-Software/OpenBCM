...
/*
 * Feature  : Vxlan Bridging, access to network IPv4 tunnel initiation
 *            with network port encap criteria based on VFI+DVP
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_ipv4_AtoN_assign_access_VP_and_VPN_net_encap_VFI_DVP.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_ipv4_AtoN_assign_access_VP_and_VPN_net_encap_VFI_DVP_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *
 *     c) Perform network-side configuration for two network Virtual ports such that
 *        each network VPs use different VNID mapping.
 *
 *     d) Add an L2 entry with destination-mac as "00:00:00:00:aa:aa" pointing to network Virtual port 1.
 *
 *     f) Add an L2 entry with destination-mac as "00:00:00:00:cc:cc" pointing to network Virtual port 2.
 *
 ...
 *        and vp's
 *
 *     b) Transmit known unicast packet with destination-mac as "00:00:00:00:aa:aa".
 *
 *     c) Transmit known unicast packet with destination-mac as "00:00:00:00:cc:cc".
 *
 *     d) Expected Result:
 *     ===================
 *       After 3.b, verify that VXLAN encapsulated packet egress out of egress_port_1
 *       mapped to network Virtual port 1.
 *
 *       After 3.c, verify that VXLAN encapsulated packet egress out of egress_port_2
 *       mapped to network Virtual port 2.
 *
 *       The packets egressing out of network Virtual port 1 and network Virtual port 2
 *       uses different VNIDs.
 */
...
test_setup(int unit)
{
  ...
  if (BCM_E_NONE !=bcm_port_gport_get(unit, egress_port_2,
                                        &(egress_phy_gport_2)))
    {
        printf("bcm_port_gport_get() failed to get gport for port %d\n",
                                                  egress_port_2);
...
verify(int unit)
{
  ...
  /* dump l2 table to see if SA learned against SVP*/
  bshell(unit, "l2 show");
  ...
  printf("\nSending known unicast packet to ingress_port access side:%d\n", ingress_port_1);
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000CCCC00000000BBBB810000150800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ingress_port_1);
  bshell(unit, str);
  bshell(unit, "sleep 2");
  bshell(unit, "show c");
  ...
  COMPILER_64_ZERO(out_pkt);
  COMPILER_64_ZERO(out_bytes);
  BCM_IF_ERROR_RETURN(bcm_stat_get(unit, egress_port_2, snmpDot1dTpPortOutFrames, &out_pkt));
  BCM_IF_ERROR_RETURN(bcm_stat_get(unit, egress_port_2, snmpIfOutOctets, &out_bytes));
  printf("Tx : %d packets / %d bytes\n", COMPILER_64_LO(out_pkt),COMPILER_64_LO(out_bytes));
  if (COMPILER_64_LO(out_pkt) != 1) {
      printf("Packet verify failed. Expected out packet 1 but get %d\n", COMPILER_64_LO(out_pkt));
      return -1;
                    ...
                    bcm_flow_handle_t flow_handle)
{
  ...
  printf("Access SVP & VPN assignment, port_group + outer_VID -> VPN + SVP\n");
  ...
  printf("Access Side Encapsulation. VP=>Access-side destination port.\n");
  bcm_flow_port_encap_t_init(&peinfo);
  peinfo.flow_handle    = flow_handle;
  peinfo.flow_port      = vxlan_acc->acc_gport;
  peinfo.dst_port      =  vxlan_acc->phy_Gport;
  peinfo.valid_elements = BCM_FLOW_PORT_ENCAP_DST_PORT_VALID;
                     ...
                     bcm_flow_handle_t flow_handle)
{
  ...
  rv = bcm_vlan_control_port_set(unit, vxlan_net->nw_gport,
                                 bcmVlanPortTranslateEgressVnidEnable, TRUE);
  if (BCM_FAILURE(rv)) {
    printf("\nError executing bcm_vlan_control_port_set(,bcmVlanPortTranslateEgressVnidEnable,): %s.\n", bcm_errmsg(rv));
    return rv;
  }
...
config_vxlan_ipv4_vtep(int unit)
{
  ...
  vxlan_net_cfg_t vxlan_net_2 = {
    egress_phy_gport_2,  /* gPort */
    23,                  /* vlan */
    "00:00:00:00:22:23", /* local mac */
    "00:00:00:00:00:02", /* remote mac */
    0xC0A80102,          /* 192.168.1.2 remote IP*/
    0x53535,             /* vnid */
    0x0a0a0a02,          /* 10.10.10.2 local IP*/
    0xffff,
    0,
    0xBC3
  };
  ...
  rv = config_vxlan_network(unit, &vxlan_cfg, &vxlan_net_1, &net_flow_port_1, 0,
                              flow_handle);
  if (BCM_FAILURE(rv)) {
    printf("\nError in network config: %s.\n", bcm_errmsg(rv));
    return rv;
  ...
  rv = config_vxlan_tunnel(unit, &vxlan_cfg, &vxlan_net_1, net_flow_port_1,
                             flow_handle);
  if (BCM_FAILURE(rv)) {
  ...
  rv = config_vxlan_network(unit, &vxlan_cfg, &vxlan_net_2, &net_flow_port_2, 0,
                              flow_handle);
  if (BCM_FAILURE(rv)) {
    printf("\nError in network config: %s.\n", bcm_errmsg(rv));
    return rv;
  ...
  rv = config_vxlan_tunnel(unit, &vxlan_cfg, &vxlan_net_2, net_flow_port_2,
                             flow_handle);
  if (BCM_FAILURE(rv)) {
    printf("\nError in tunnel config: %s.\n", bcm_errmsg(rv));
    return rv;
  ...
  printf("Access-to-Network: Add payload-dest-mac Address => Network-flow-port (NVP-1).\n");
  ...
  printf("Access-to-Network: Add payload-dest-mac Address => Network-flow-port (NVP-2).\n");
  rv = add_to_l2_table(unit, "00:00:00:00:cc:cc", vxlan_cfg.vxlan_vpn,
                         net_flow_port_2);
  if (BCM_FAILURE(rv)) {
    printf("\nError in l2 entry config: %s.\n", bcm_errmsg(rv));
    return rv;
