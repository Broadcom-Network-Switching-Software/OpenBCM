 ...
 *            and decapped packet is added with OVID
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_v4_NtoA_encap_add_ovid.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_v4_NtoA_encap_add_ovid_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *             - network to access(decap flow) with new OVID added
 *
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see the decapped packet on the egress port with the OVID 0x19 added
...
struct vxlan_acc_cfg_t {
    ...
    int           ingress_port_class;
    int           egress_port_class;
    int           new_pkt_ovid;
    int           new_pkt_pri;
};
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port);
    bshell(unit, str);
...
int
vxlan_access_port_config(int unit, bcm_port_t port_acc, int ingress_port_class, int egress_port_class)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, port_acc, bcmPortClassEgress, egress_port_class));

    /* Enable egress vlan xlate */
    BCM_IF_ERROR_RETURN(bcm_vlan_control_port_set(unit, port_acc, bcmVlanTranslateEgressEnable, 1));
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port)
{
    ...
    rv = vxlan_access_port_config(unit, phy_port, vxlan_acc->ingress_port_class, vxlan_acc->egress_port_class);
    if (BCM_FAILURE(rv)) {
    ...
    match_info.port = vxlan_acc->ingress_port_class;
    ...
    printf("Access: Egress Vlan Translation to add OVID %d PRIORITY %d.\n", vxlan_acc->new_pkt_ovid, vxlan_acc->new_pkt_pri);
    bcm_vlan_action_set_t action;
    action.outer_tag = bcmVlanActionAdd;
    action.new_outer_vlan = vxlan_acc->new_pkt_ovid;
    action.new_outer_cfi = 0;
    action.priority = vxlan_acc->new_pkt_pri;
    action.inner_tag = bcmVlanActionNone;
    rv = bcm_vlan_translate_egress_action_add(unit, vxlan_acc->egress_port_class, vxlan_cfg->vxlan_vpn, BCM_VLAN_INVALID, &action);
    if (BCM_FAILURE(rv)) {
...
config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, bcm_flow_handle_t flow_handle)
{
    ...
    rv = create_vxlan_vp(unit, vxlan_cfg->vxlan_vpn, BCM_FLOW_PORT_NETWORK | flags, net_flow_port);
    if (BCM_FAILURE(rv)) {
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    /*create net_flow_port with vxlan_cfg*/
    ...
    rv = add_to_l2_table(unit, "00:00:00:00:bb:bb", vxlan_cfg.vxlan_vpn, egress_port);
    if (BCM_FAILURE(rv)) {
