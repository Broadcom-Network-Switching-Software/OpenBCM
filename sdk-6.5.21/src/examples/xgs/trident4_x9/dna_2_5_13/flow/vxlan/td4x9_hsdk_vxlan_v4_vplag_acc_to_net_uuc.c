...
/*
 * Feature  : Vxlan Bridging , IPv4 access to network VP LAG (LAG of virtual network ports) non-unicast Flow
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_v4_vplag_acc_to_net_uuc.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan-vplag.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_v4_vplag_acc_to_net_uuc_log.txt
 *
 * Test Topology :
 *                                                                                     Underlay+NextHop
 ...
 *
 *     b) Transmit 3 non-unicast packets. The contents of the packets
 *        are printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see that all packets are forwarded to the designated egress VP port.
 *       =---------------------------------------------------------------------=
 *       [TD4-6374]
 *       In TD4 DNA VPLAG resolution using ECMP resources is only supported
 *       explicitly for UC destination cases.
 *       For any non-UC cases, the expectation is that VPLAG member selection
 *       is done by programming only one member per group in the IPMC replication
 *       lists.  This is true for both access and network VP cases.
 *       =---------------------------------------------------------------------=
...
Header Info
  ...
  ###[ Ethernet ]###
    dst= ff:ff:ff:ff:ff:ff/01:00:5E:01:02:03/00:00:00:55:AA:55
...
Ingress Hex Packet:
tx port=1 data=0xFFFFFFFFFFFF00000000BBBB8100001508004500002E00000000401174BA010101010202020200350036001AB0F8000102030405060708090A0B0C0D0E0F101133CF192C

tx port=1 data=0x01005E01020300000000BBBB8100001508004500002E00000000401174B9010101020202020200350036001AB0F7000102030405060708090A0B0C0D0E0F1011975E7DF7

tx port=1 data=0x00000055AA5500000000BBBB8100001508004500002E00000000401174b6010101050202020200350036001AB0F8000102030405060708090A0B0C0D0E0F101133CF192C
...
data[0020]: 0a02 c0a8 0102 e0d5 12b5 0050 0000 0800
data[0030]: 0000 0123 4500 ffff ffff ffff 0000 0000
...
data[0000]: {000000000002} {000000002222} 8100 0016
data[0010]: 0800 4500 0064 0000 0000 3f11 a5d3 0a0a
data[0020]: 0a02 c0a8 0102 c000 12b5 0050 0000 0800
data[0030]: 0000 0123 4500 0100 5e01 0203 0000 0000
...
data[0000]: {000000000002} {000000002222} 8100 0016
data[0010]: 0800 4500 0064 0000 0000 3f11 a5d3 0a0a
data[0020]: 0a02 c0a8 0102 cc90 12b5 0050 0000 0800
data[0030]: 0000 0123 4500 0000 0055 aa55 0000 0000
...
bcm_gport_t vplag_member_uuc_selected_net_phy_gport = 0;
bcm_if_t    vplag_member_uuc_selected_net_egr_id;
bcm_gport_t vplag_member_uuc_selected_net_flow_port;
...
verify(int unit)
{
    ...
    snprintf(pkt_data[0],512,"%s","FFFFFFFFFFFF00000000BBBB8100001508004500002E00000000401174BA010101010202020200350036001AB0F8000102030405060708090A0B0C0D0E0F101133CF192C");

    snprintf(pkt_data[1],512,"%s","01005E01020300000000BBBB8100001508004500002E00000000401174B9010101020202020200350036001AB0F7000102030405060708090A0B0C0D0E0F1011975E7DF7");

    snprintf(pkt_data[2],512,"%s","00000055AA5500000000BBBB8100001508004500002E00000000401174b6010101050202020200350036001AB0F8000102030405060708090A0B0C0D0E0F101133CF192C");
...
/* Add a member port into the multicast group of a specific VxLAN VPN */
//  Access Port          : vxlan_multicast_port_add(unit, &vxlan_cfg, 0, gport, 0, 0);
//  Network Port(Tunnel) : vxlan_multicast_port_add(unit, &vxlan_cfg, bcmMulticastEncapTypeL2Tunnel, gport, egr_obj, vp);
//  Network Port(Transit): vxlan_multicast_port_add(unit, &vxlan_cfg, bcmMulticastEncapTypeL3, gport, egr_intf, vp);
int
vxlan_multicast_port_add(int unit, vxlan_cfg_t *vxlan_cfg, bcm_multicast_encap_type_t type,
                         bcm_gport_t gport, bcm_if_t egr_obj_if, bcm_gport_t flow_port)
{
    bcm_multicast_t mc_group = vxlan_cfg->mc_group;
    bcm_if_t        encap_id = BCM_IF_INVALID;
    bcm_multicast_encap_t mc_encap;
    bcm_error_t rv = BCM_E_NONE;

    switch ( type ) {
        case bcmMulticastEncapTypeL2Tunnel: /* VxLAN L2 tunnel */
             bcm_multicast_encap_t_init(&mc_encap);
             mc_encap.encap_type   = bcmMulticastEncapTypeL2Tunnel;
             mc_encap.ul_egress_if = egr_obj_if; /* egress object */
             mc_encap.l3_intf      = 0;
             mc_encap.port_id      = flow_port;
             ...
             break;
        case bcmMulticastEncapTypeL3: /* VxLAN MC-BUD Transit */
             bcm_multicast_encap_t_init(&mc_encap);
             mc_encap.encap_type   = bcmMulticastEncapTypeL3;
             mc_encap.ul_egress_if = 0;
             mc_encap.l3_intf      = egr_obj_if; /* egress intf */
             rv = bcm_multicast_encap_create(unit, &mc_encap, &encap_id);
             if (BCM_FAILURE(rv) && (rv!=BCM_E_EXISTS)) {
                 printf("\nError in creating Mcast Encap ID: %s.\n", bcm_errmsg(rv));
                 return rv;
             ...
             break;
        default : /* L2: e.g. access port */
             break;
    }

    rv = bcm_multicast_egress_add(unit, mc_group, gport, encap_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in egress add for multicast group: %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port, bcm_flow_handle_t flow_handle)
{
    ...
    printf("Access: Add port (0x%x) to Mcast Group (0x%x).\n", vxlan_acc->phy_Gport, vxlan_cfg->mc_group);
    rv = vxlan_multicast_port_add(unit, vxlan_cfg, 0,
                                  vxlan_acc->phy_Gport, 0, 0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in adding port to Mcast Group: %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
/*  printf("Network: Add port (0x%x) to Mcast Group (0x%x).\n", *net_flow_port, vxlan_cfg->mc_group);
    rv = vxlan_multicast_port_add(unit, vxlan_cfg, bcmMulticastEncapTypeL2Tunnel,
                                  vxlan_net->phy_Gport, net_egr_id, *net_flow_port);
    if (BCM_FAILURE(rv)) {
        printf("\nError in adding port to Mcast Group: %s.\n", bcm_errmsg(rv));
        return rv;
    }
 ...
 *   lists.  This is true for both access and network VP cases.
 *  =---------------------------------------------------------------------=
 *  Here, the first VP port of VPLAG is selected as the flooding port for
 *  non-unicast traffic.
 */ if ( vplag_member_uuc_selected_net_phy_gport == 0 )
    {
         vplag_member_uuc_selected_net_phy_gport = vxlan_net->phy_Gport;
         vplag_member_uuc_selected_net_egr_id    = net_egr_id;
         vplag_member_uuc_selected_net_flow_port = *net_flow_port;
    }
...
/*  bcm_gport_t vplag_gport;
    BCM_GPORT_TRUNK_SET(vplag_gport, vplag_id);
    printf("Network: Add VPLAG (0x%x) to Mcast Group (0x%x).\n", vplag_gport, vxlan_cfg.mc_group);
    rv = vxlan_multicast_port_add(unit, vxlan_cfg, 0, vplag_gport, 0, 0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in adding port to Mcast Group: %s.\n", bcm_errmsg(rv));
        return rv;
    }
 ...
 *   lists.  This is true for both access and network VP cases.
 *  =---------------------------------------------------------------------=
 */ printf("Network: Add port (0x%x) to Mcast Group (0x%x).\n", vplag_member_uuc_selected_net_flow_port, vxlan_cfg.mc_group);
    rv = vxlan_multicast_port_add(unit, vxlan_cfg, bcmMulticastEncapTypeL2Tunnel,
                                  vplag_member_uuc_selected_net_phy_gport,
                                  vplag_member_uuc_selected_net_egr_id,
                                  vplag_member_uuc_selected_net_flow_port);
    if (BCM_FAILURE(rv)) {
        printf("\nError in adding port to Mcast Group: %s.\n", bcm_errmsg(rv));
        return rv;
    }
