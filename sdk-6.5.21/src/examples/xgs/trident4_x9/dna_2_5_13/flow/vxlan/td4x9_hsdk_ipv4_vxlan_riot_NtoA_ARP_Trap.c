...
/*
 * Feature  : Vxlan Riot, Trap ARP packets to cpu in VXLAN Riot termination
 *
 * Usage    : BCM.0> cint td4x9_hsdk_ipv4_vxlan_riot_NtoA_ARP_Trap.c
 *
 * config   : BCM956780-56780_A0-Default-config.yml
 *
 * Log file : td4x9_hsdk_ipv4_vxlan_riot_NtoA_ARP_Trap_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * ========
 * 
 * Cint example to demonstrates how to trap & drop ARP packets to cpu in VXLAN Riot termination
 *
 ...
 *        packet watcher.
 *        Ingress port does not need an IFP rule to copy packet to CPU in this case
 *
 ...
 *     ===================
 *     
 *       We can see the tunnel packet with ARP payload is trapped to CPU from ingress port. 
 *       No packet is seen from egress port
...
###[ UDP ]###
           ...
           dport= 8472
###[ VXLAN ]###
              flags= NextProtocol+Instance
              reserved0= 0
              NextProtocol= Ethernet
              vni= 0x12345
              reserved2= 0x0
###[ Ethernet ]###
...
###[ ARP ]###
                       ...
                       plen= 4
                       op= who-has
                       hwsrc= 00:00:00:00:04:01
                       psrc= 40.40.40.1
                       hwdst= 00:00:00:00:00:00
                       pdst= 30.30.30.1                   

Ingress Hex Packet:
tx port=2 data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d880800000001234500FFFFFFFFFFFF0000000004018100000108060001080006040001000000000401282828010000000000001E1E1E01

Packets received from CPU port:
...
[bcmPWN.0]Packet[1]: data[0030]: 0000 0123 4500 ffff ffff ffff 0000 0000
[bcmPWN.0]Packet[1]: data[0040]: 0401 8100 0001 0806 0001 0800 0604 0001
[bcmPWN.0]Packet[1]: data[0050]: 0000 0000 0401 2828 2801 0000 0000 0000
[bcmPWN.0]Packet[1]: data[0060]: 1e1e 1e01
[bcmPWN.0]
[bcmPWN.0][RX metadata information]
        ...
        MATCH_ID_HI                     :0xc00(3072)
        MPB_FLEX_DATA_TYPE              :0x3(3)
        MULTICAST                       :0x1(1)
        COPY_TO_CPU                     :0x1(1)
[FLEX fields]
        EVENT_TRACE_VECTOR_15_0         :0x44
...
verify(int unit)
{
	...
	snprintf(str, 512, "tx 1 pbm=%d data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d880800000001234500FFFFFFFFFFFF0000000004018100000108060001080006040001000000000401282828010000000000001E1E1E01; sleep quiet 3", ingress_port);
	bshell(unit, str);
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
    bshell(unit, "l2 show");
...
config_vxlan_ipv4_vtep(int unit)
{
	...
	/*
	 *  Trap APR packets to CPU, and drop the packets
	 */
	
	/* Switch protocol control */
	uint32 	 options = 0;
	int		 priority = 1;
	bcm_switch_pkt_protocol_match_t  protocol_control;
	bcm_switch_pkt_control_action_t	 action;
	
	bcm_switch_pkt_protocol_match_t_init(&protocol_control);
	
	/* 
	 *		tunnel_processing_results_1[0]: If set, tunnel terminated;
     *      tunnel_processing_results_1[1]: If set, L3 tunnel decapped;
     *      tunnel_processing_results_1[2]: If set, L2 tunnel decapped;
	 *      tunnel_processing_results_1[3]: If set, 2-pass processing with pkt loopback;
	*/
	protocol_control.tunnel_processing_results_1 = 0x5;
	protocol_control.tunnel_processing_results_1_mask = 0xf;
	
	/*
	 *  Inner ARP packet, Ethertype = 0x0806, operation = 0x0001
	 */
	printf (" Adding packet protocl control ... ");
	protocol_control.ethertype = 0x0806;
	protocol_control.ethertype_mask = 0xffff;
	protocol_control.arp_rarp_opcode = 0x0001;
	protocol_control.arp_rarp_opcode_mask = 0xffff;
	
	action.copy_to_cpu = 1;
	action.discard = 1;
	rv = bcm_switch_pkt_protocol_control_add(unit, options, &protocol_control, &action, priority);
	if (BCM_FAILURE(rv)) {
