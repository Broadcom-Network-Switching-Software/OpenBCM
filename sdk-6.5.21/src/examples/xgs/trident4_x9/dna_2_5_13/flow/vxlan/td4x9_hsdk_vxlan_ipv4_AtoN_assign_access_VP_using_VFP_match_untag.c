 ...
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_ipv4_AtoN_assign_access_VP_using_VFP_match_untag.c
 *
 * config   : BCM956780-56780_A0-Default-config.yml
 *
 * Log file : td4x9_hsdk_vxlan_ipv4_AtoN_assign_access_VP_using_VFP_match_untag_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *
 *   In this example, on the access-side source virtual port is assigned for an incoming
 *   untagged packet using VFP and VFI is assigned using ING_L2_IIF_TABLE.VFI.
 *
 *
 ...
 *
 *     b) Perform access-side configuration. As part of access-side configuration
 *        a VFP rule is created to assign source-vp if the incoming packet
 *        is untagged.
 *
 ...
 *        and vp's
 *
 *     b) Transmit known unicast untagged packets. The contents of the packets
 *        are printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see vxlan tunnel packets on the egress port with the IPv4 encapsulation.
 */
/* Reset C interpreter*/
cint_reset();
...
verify(int unit)
{
  ...
  snprintf(str, 512, "tx 10 pbm=%d data=0x00000000AAAA00000000BBBB0800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 3", ingress_port_1);
  bshell(unit, str);
  bshell(unit, "show c");
  ...
  if (COMPILER_64_LO(out_pkt) != 10) {
      printf("Packet verify failed. Expected out packet 10 but get %d\n", COMPILER_64_LO(out_pkt));
      return -1;
  }
                    ...
                    bcm_flow_handle_t flow_handle)
{
  ...
  print vxlan_acc->acc_gport;
  /*Create a VFP rule with below match criteria for untagged packets
   * OuterVlan == 0 &&
   * InnerVlan == 0 &&
   * OuterTpid == 0 &&
   * InnerTpid == 0
 */
  ...
  BCM_FIELD_QSET_ADD(group_config.qset, bcmFieldQualifyOuterTpid);
  BCM_FIELD_QSET_ADD(group_config.qset, bcmFieldQualifyInnerVlan);
  BCM_FIELD_QSET_ADD(group_config.qset, bcmFieldQualifyInnerTpid);

  BCM_FIELD_ASET_ADD(group_config.aset, bcmFieldActionIngressGportSet);
  ...
  BCM_IF_ERROR_RETURN(bcm_field_qualify_InPort(unit, entry, vxlan_acc->phy_Gport,
                                            BCM_FIELD_EXACT_MATCH_MASK));
  BCM_IF_ERROR_RETURN(bcm_field_qualify_OuterVlan(unit, entry, 0,
                                            BCM_FIELD_EXACT_MATCH_MASK));
  BCM_IF_ERROR_RETURN(bcm_field_qualify_InnerVlan(unit, entry, 0,
                                            BCM_FIELD_EXACT_MATCH_MASK));
  BCM_IF_ERROR_RETURN(bcm_field_qualify_OuterTpid(unit, entry, 0));
  BCM_IF_ERROR_RETURN(bcm_field_qualify_InnerTpid(unit, entry, 0));

  rv = bcm_field_action_add(unit, entry, bcmFieldActionIngressGportSet, vxlan_acc->acc_gport, 0);
  if (BCM_FAILURE(rv)) {
    printf("\nError(%s) configuring bcmFieldActionIngressGportSet vxlan_acc->acc_gport = 0x%x\n",
                                            bcm_errmsg(rv), vxlan_acc->acc_gport);
    return rv;
  }
