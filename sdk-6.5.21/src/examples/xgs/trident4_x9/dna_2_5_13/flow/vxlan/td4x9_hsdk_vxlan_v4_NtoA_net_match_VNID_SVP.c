 ...
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_v4_NtoA_net_match_VNID_SVP.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_v4_NtoA_net_match_VNID_SVP_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
vxlan_network_port_config(int unit, bcm_gport_t port_net)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, port_net, bcmPortControlVxlanTunnelbasedVnId, 1));
...
config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, uint32 same_vp, bcm_flow_handle_t flow_handle)
{
    ...
    bcm_flow_match_config_t_init(&match_info);
    match_info.flow_handle    = flow_handle;
    match_info.criteria       = BCM_FLOW_MATCH_CRITERIA_SVP_VNID;
    match_info.vpn            = vxlan_cfg->vxlan_vpn;
