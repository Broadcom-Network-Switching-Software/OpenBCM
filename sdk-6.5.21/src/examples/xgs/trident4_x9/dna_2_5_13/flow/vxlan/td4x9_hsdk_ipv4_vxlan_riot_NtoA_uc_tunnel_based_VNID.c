...
/*
 * Feature  : Vxlan Riot, IPv4 network to access tunnel termination unicast flow,
 *            VFI is derived from {SVP+VNID}
 *
 * Usage    : BCM.0> cint td4x9_hsdk_ipv4_vxlan_riot_NtoA_uc_tunnel_based_VNID.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan.config.yml
 *
 * Log file : td4x9_hsdk_ipv4_vxlan_riot_NtoA_uc_tunnel_based_VNID_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
Ingress Hex Packet:
tx port=2 data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E000040000A06E482282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A
...
Packet Dump
{000000000001} {000000222222} 8100 000a
0800 4500 002e 0000 4000 0906 e582 2828 TTL=9
2801 1e1e 1e01 0000 0000 0000 0000 0000
...
verify(int unit)
{
	...
	snprintf(str, 512, "tx 1 pbm=%d data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E000040000A06E482282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A; sleep quiet 3", ingress_port);
	bshell(unit, str);
    bshell(unit, "sleep 2");
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
                     ...
                     bcm_flow_handle_t flow_handle)
{
	...
	/* Set port learning */
    rv = bcm_port_learn_set(unit, vxlan_net->net_port, BCM_PORT_LEARN_ARL | BCM_PORT_LEARN_FWD);
    if (BCM_FAILURE(rv)) {
        printf("\nError in Network:bcm_port_learn_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_port_learn_set(unit, *net_flow_port, BCM_PORT_LEARN_FWD);
    if (BCM_FAILURE(rv)) {
        printf("\nError in Network:bcm_port_learn_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_port_control_set(unit, vxlan_net->net_port, bcmPortControlL3Ingress, ingress_if);
    if (BCM_FAILURE(rv)) {
        printf("\nError in Network:bcm_port_control_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_vlan_control_vlan_selective_set(unit,vxlan_net->net_ol_vlan, BCM_VLAN_CONTROL_VLAN_INGRESS_IF_MASK, &vlan_ctrl);
    if (BCM_FAILURE(rv)) {
        printf("\nError in Network:bcm_vlan_control_vlan_selective_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
