...
/*
 * Feature  : Vxlan Riot, Trap Overlay DIP = My router IP packet to CPU for tunnel termination flow
 *
 * Usage    : BCM.0> cint td4x9_hsdk_ipv4_vxlan_riot_NtoA_DestIP_ToCPU.c
 *
 * config   : BCM956780-56780_A0-Default-config.yml
 *
 * Log file : td4x9_hsdk_ipv4_vxlan_riot_NtoA_DestIP_ToCPU_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * ========
 * 
 * Cint example to demonstrate to trap overlay DIP = My router IP packet to CPU for tunnel termination flow
 *
 ...
 *   =============================================
 *   
 *     a) Selects ingress port for network side and CPU port is assigned as egress port on access side 
 *
 ...
 *     ===================
 *     
 *       We can see the tunnel packet with inner DIP = my router IP is trapped to CPU with the entire tunnel header.
...
Packet recieved on CPU port:

(Tunnel header is preserved)
...
test_setup(int unit)
{
	...
	*/
	...
	/* In this test case, egress port is CPU port, no need to do egress port setup */
	/*
    BCM_PBMP_CLEAR(pbmp);
                    ...
                    bcm_gport_t *acc_flow_port)
{
	...
	/* To lift the original packet to CPU, BCM_L3_COPY_TO_CPU need to be specified*/
	host.l3a_flags |= BCM_L3_COPY_TO_CPU;
