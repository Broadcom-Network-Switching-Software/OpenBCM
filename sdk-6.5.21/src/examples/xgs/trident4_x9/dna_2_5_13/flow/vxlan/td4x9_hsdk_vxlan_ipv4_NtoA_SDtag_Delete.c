...
/*
 * Feature  : Vxlan Bridging , IPv4 network to access tunnel termination flow, remove the outer vlan tag from payload while egressing on access port
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_ipv4_NtoA_SDtag_Delete.c
 *
 * config   : BCM956780-56780_A0-Default-config.yml
 *
 * Log file : td4x9_hsdk_vxlan_ipv4_NtoA_SDtag_Delete_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see the payload packet on egress port with outer vlan tag removed.
