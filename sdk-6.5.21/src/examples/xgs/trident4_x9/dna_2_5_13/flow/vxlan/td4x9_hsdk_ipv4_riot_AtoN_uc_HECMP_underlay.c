 ...
 *
 * Usage    : BCM.0> cint td4x9_hsdk_ipv4_riot_AtoN_uc_HECMP_underlay.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan.config.yml
 *
 * Log file : td4x9_hsdk_ipv4_riot_AtoN_uc_HECMP_underlay_log.txt
 *
 * Test Topology :
 *
 *            ACCESS                             NETWORK
 ...
 *                        |                | DMAC=00::02:02:02   .
 *      +------>          |                | SMAC=00::01:01:01   .
 * DMAC=00::22:22:22      |                |     VLAN=14         .
 * SMAC=00::11:11:11      |      DUT       |                     .
...
0a01 1414 1401 ce63 12b5 0050 0000 0800
0000 0123 4500 0000 0002 0202 0000 0001
0101 0800 4500 002e 0000 0000 3fff db77
...
vxlan_tunnel_term_update(int unit, bcm_gport_t tnl_id, uint32 mflags)
{
    ...
    rv = bcm_vxlan_tunnel_terminator_get(unit, &tnl_info);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_vxlan_tunnel_terminator_get: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_vxlan_tunnel_terminator_update: %s\n", bcm_errmsg(rv));
        return rv;
    }
    return rv;
}
                     ...
                     bcm_flow_handle_t flow_handle)
{
    ...
    rv = vxlan_create_ecmp_obj(unit,BCM_L3_ECMP_OVERLAY, 1, net_egr_obj_ol,&net_ecmp_obj_id_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in configuring ECMP obj: %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
/*
    printf("Create Overlay network L3 ECMP object for Overlay.\n");
    rv = vxlan_create_ecmp_obj(unit,BCM_L3_ECMP_OVERLAY, 1, net_egr_obj_ol,&net_ecmp_obj_id_ol,net_egr_obj_ul);
    if (BCM_FAILURE(rv)) {
        printf("\nError in configuring ECMP obj: %s.\n", bcm_errmsg(rv));
        return rv;
    }
