...
/*
 * Feature  : Vxlan Bridging, access to network IPv4 tunnel initiation using
 *            packet's vlan as vfi
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_ipv4_AtoN_use_pkt_vlan_as_vfi.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_ipv4_AtoN_use_pkt_vlan_as_vfi_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *   using packet's vlan as the vfi. The advantage is that there is no need to burn
 *   one entry in ing_vlan_xlate table to derive VFI.
 *
 ...
 *   ============================================================
 *     a) Creates a vxlan. For VFI <= 4095, use bcm_vlan_create() API to create the
 *        VFI and configure unknown_unicast_group, unknown_multicast_group and broadcast_group
 *        using bcm_vlan_control_vlan_selective_set() API. This is done in create_vxlan_vpn().
 *        Note: config property "vlan_flooding_l2mc_num_reserved" should be set to 0.
 *
...
create_vxlan_vpn(int unit, vxlan_cfg_t *vxlan_cfg)
{
  ...
  if (4095 <= vxlan_cfg->vxlan_vpn) {
    ...
    BCM_IF_ERROR_RETURN(bcm_vlan_create(unit, vxlan_cfg->vxlan_vpn));
    vlan_control.broadcast_group = mc_group;
    vlan_control.unknown_multicast_group = mc_group;
    vlan_control.unknown_unicast_group = mc_group;
    ...
    BCM_IF_ERROR_RETURN(bcm_vlan_control_vlan_selective_set(unit, vxlan_cfg->vxlan_vpn, flags, &vlan_control));
  }
