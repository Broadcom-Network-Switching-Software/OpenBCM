 ...
 *            with network port encap criteria based on VFI
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_ipv4_AtoN_assign_access_VP_and_VPN_net_encap_VFI.c
 *
 * config   : BCM956780-56780_A0-Default-config.yml
 *
 * Log file : td4x9_hsdk_vxlan_ipv4_AtoN_assign_access_VP_and_VPN_net_encap_VFI_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *        both the network VPs use the same VFI -> VNID mapping.
 *
 ...
 *
 *       Both the packets egressing out of network Virtual port 1 and network Virtual port 2
 *       share the same VNID(0x12345).
 */
...
config_vxlan_ipv4_vtep(int unit)
{
  ...
  rv = config_vxlan_network(unit, &vxlan_cfg, &vxlan_net_2, &net_flow_port_2, 1,
                              flow_handle);
  if (BCM_FAILURE(rv)) {
    printf("\nError in network config: %s.\n", bcm_errmsg(rv));
    return rv;
