...
/*
 * Feature  : Vxlan Riot, VFI member check
 *
 * Usage    : BCM.0> cint td4x9_hsdk_ipv4_riot_AtoN_uc_VFI_member_check.c
 *
 * config   : BCM956780-56780_A0-Default-config.yml
 *
 * Log file : td4x9_hsdk_ipv4_riot_AtoN_uc_VFI_member_check_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *
 *   3) Step3 - Verification (Done in verify(),basic VxLAN forwarding):
 *   ==================================================================
 ...
 *
 *     c) Remove port members from VFI.
 *
 *     d) Transmit the known unicast packet. The contents of the packet
 *        are printed on screen.
 *
 *     e) Ports are added to VFI again to pass membership check.
 *
 *     f) Transmit the known unicast packet. The contents of the packet
 *        are printed on screen.
 *
 *     g) Expected Result:
 *     ===================
 *       After step 3.b, We can see the tunnel packet on the egress port with the IPv4 encapsulation
 *       and 'show c' to check the Tx/Rx packet stats/counters.
 *
 *       After step 3.d, traffic is dropped in ingress pipe due to VFI check failure.
 *
 *       After step 3.f, We can see the tunnel packet on the egress port with the IPv4 encapsulation
 *       and 'show c' to check the Tx/Rx packet stats/counters.
 */
...
bcm_error_t
vxlan_vp_membership_remove(int unit, bcm_gport_t vxlan_vp, bcm_vpn_t vxlan_vpn)
{
    bcm_error_t rv = BCM_E_NONE;

    rv = bcm_vlan_gport_delete(unit, vxlan_vpn, vxlan_vp);
    if (BCM_FAILURE(rv)) {
...
verify(int unit)
{
    ...
    print "VFI member check start";
    print "remove port member ";

	vxlan_vp_membership_remove(unit, ingress_phy_gport, acc_vpn_record);
	vxlan_vp_membership_remove(unit, egress_phy_gport, acc_vpn_record);
    ...
    print "add port member";

   	vxlan_vp_membership(unit, ingress_phy_gport, acc_vpn_record);
	vxlan_vp_membership(unit, egress_phy_gport, acc_vpn_record);
                    ...
                    bcm_gport_t *acc_flow_port)
{
	...
	acc_vpn_record = vxlan_acc->acc_vpn;
