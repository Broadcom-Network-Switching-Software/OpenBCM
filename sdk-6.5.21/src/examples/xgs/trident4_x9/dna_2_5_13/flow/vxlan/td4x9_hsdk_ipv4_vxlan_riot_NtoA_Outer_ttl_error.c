...
/*
 * Feature  : Vxlan Riot, Tunnel TTL error packet processing in Vxlan termination flow
 *
 * Usage    : BCM.0> cint td4x9_hsdk_ipv4_vxlan_riot_NtoA_Outer_ttl_error.c
 *
 * config   : BCM956780-56780_A0-Default-config.yml
 *
 * Log file : td4x9_hsdk_ipv4_vxlan_riot_NtoA_Outer_ttl_error_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * ========
 * 
 * Cint example to verify the processing of Vxlan tunnel terminated packets with Tunnel TTL=0/1
 ...
 *
 *     b) Transmit the known unicast packet with outer TTL=0/1. The contents of the packet
 *        are printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *     
 *       We can see the Vxlan packet with Outer TTL=1 is decapped on the egress port.
 *		 Vxlan packet with Outer TTL=0 packet is trapped to cpu and drop.
...
Ingress Hex Packet (Outer TTL=1):
tx port=2 data=0x0000000003010000000001018100001E0800450000680000400001113D66141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E000040000A06E482282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A

Ingress Hex Packet (Outer TTL=0):
tx port=2 data=0x0000000003010000000001018100001E0800450000680000400000113E66141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E000040000A06E482282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A

Packets recieved on CPU port:

(Out TTL=1, packet is trapped to CPU by IFP from egress port)
[bcmPWN.0]Packet[1]: data[0000]: {000000000001} {000000222222} 8100 000a
[bcmPWN.0]Packet[1]: data[0010]: 0800 4500 002e 0000 4000 0906 e582 2828
[bcmPWN.0]Packet[1]: data[0020]: 2801 1e1e 1e01 8181 5151 b1b2 b3b4 a1a2
[bcmPWN.0]Packet[1]: data[0030]: a3a4 500f 01f5 3a9f 0000 1112 1314 1516
[bcmPWN.0]Packet[1]: data[0040]: 1718 191a
[bcmPWN.0]
[bcmPWN.0][RX metadata information]
        PKT_LENGTH                      :0x48(72)
        SRC_PORT_NUM                    :0x1(1)                    // egress port = 1
        SWITCH                          :0x1(1)
...
(Outer TTL=0, packet is trapped to cpu by Drop event monitor from ingress port)
[bcmPWN.0]
[bcmPWN.0]Packet[2]: data[0000]: {000000000301} {000000000101} 8100 001e
[bcmPWN.0]Packet[2]: data[0010]: 0800 4500 0068 0000 4000 0a11 3466 1414
[bcmPWN.0]Packet[2]: data[0020]: 1401 0a0a 0a01 d234 12b5 0054 7d88 0800
[bcmPWN.0]Packet[2]: data[0030]: 0000 0123 4500 0000 0000 0401 0000 0000
[bcmPWN.0]Packet[2]: data[0040]: 0201 0800 4500 002e 0000 0000 0006 2e83
[bcmPWN.0]Packet[2]: data[0050]: 2828 2801 1e1e 1e01 8181 5151 b1b2 b3b4
[bcmPWN.0]Packet[2]: data[0060]: a1a2 a3a4 500f 01f5 3a9f 0000 1112 1314
[bcmPWN.0]Packet[2]: data[0070]: 1516 1718 191a
[bcmPWN.0]
[bcmPWN.0][RX metadata information]
        PKT_LENGTH                      :0x7a(122)
        SRC_PORT_NUM                    :0x2(2)                   // ingress port = 1
        SWITCH                          :0x1(1)
        ...
        COPY_TO_CPU                     :0x1(1)
[FLEX fields]
        EVENT_TRACE_VECTOR_15_0         :0x4000
        DROP_CODE_15_0                  :0x1010
...
verify(int unit)
{
	...
	/* Sending Outer TTL=1 Packets */
    printf("\nSending Outer TTL=1 packet to ingress network side:%d\n", ingress_port);
	snprintf(str, 512, "tx 1 pbm=%d data=0x0000000003010000000001018100001E0800450000680000400001113D66141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E000040000A06E482282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A; sleep quiet 3", ingress_port);
	bshell(unit, str);
	
	/* Sending Outer TTL = 0 Packets*/
	printf("\nSending Outer TTL=0 packet to ingress network side:%d\n", ingress_port);
	snprintf(str, 512, "tx 1 pbm=%d data=0x0000000003010000000001018100001E0800450000680000400000113E66141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E000040000A06E482282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A; sleep quiet 3", ingress_port);
	bshell(unit, str);
...
config_vxlan_ipv4_vtep(int unit)
{
	...
	/* 
	 * Drop Event adding to trap tunnel_ttl error packets to CPU 
	 */
	...
	monitor.drop_event = bcmPktDropEventIngressTunnelTtlCheckFailed;
