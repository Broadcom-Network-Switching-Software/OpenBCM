...
/*
 * Feature  : Vxlan Riot, IPv4 network to access tunnel termination flow, VFI is derived from {SVP+VNID}
 *            VxLAN GBP-ID validation by GBP-DID derived through L3 IPMC entry
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_gbp_ipv4_NtoA_mc_l3.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan-gbp-em.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_gbp_ipv4_NtoA_mc_l3_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *             - GBP-DID assigned by L3 IPMC entry through class ID
 ...
 *
 *     b) Transmit the known unicast VxLAN packet with in-valid GBP-SID and IPCM payload. The contents of the packet
 *        are printed on screen.
 *        As GBP match doesn't occur, decapsulated packet should be dropped (shouldn't egress out) after hitting IPMC entry
 *
 *     c) Expected Result:
 *     ===================
 *      As the second ingress packet is dropped, there will be only one egress packet
 */
...
Ingress Hex Packet:
tx port=2 data=0000000003010000000001018100000B080045000068000040000A113466141414010A0A0A01D23412B500547D8888000FAB0123450001005E01010100000000020108004500002E000040000A0653A91E1E1E01E001010181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A
...
Ingress Hex Packet:
tx port=2 data=0000000003010000000001018100000B080045000068000040000A113466141414010A0A0A01D23412B500547D8888000DEC0123450001005E01010100000000020108004500002E000040000A0653A91E1E1E01E001010181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A
...
Packet Dump
data[0000]: {01005e010101} {000000222222} 8100 000a
data[0010]: 0800 4500 002e 0000 4000 0906 54a9 1e1e
data[0020]: 1e01 e001 0101 8181 5151 b1b2 b3b4 a1a2
...
struct vxlan_acc_cfg_t {
    ...
    bcm_if_t      acc_l3_ing_intf;  /* Output: L3 ingress interface */
    bcm_if_t      acc_l3_egr_intf;  /* Output: L3 egress interface */
};
...
struct vxlan_net_cfg_t {
    ...
    bcm_if_t      net_l3_ing_intf;  /* Output: L3 ingress interface */
    bcm_if_t      net_l3_egr_intf;  /* Output: L3 egress interface */
};
...
verify(int unit)
{
    ...
    snprintf(pkt_data[0],512,"%s","0000000003010000000001018100000B080045000068000040000A113466141414010A0A0A01D23412B500547D8888000FAB0123450001005E01010100000000020108004500002E000040000A0653A91E1E1E01E001010181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A");

    /* Packet with In-valid GBP-ID */
    snprintf(pkt_data[1],512,"%s","0000000003010000000001018100000B080045000068000040000A113466141414010A0A0A01D23412B500547D8888000DEC0123450001005E01010100000000020108004500002E000040000A0653A91E1E1E01E001010181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A");
                    ...
                    bcm_gport_t *acc_flow_port)
{
	...
	vxlan_acc->acc_l3_egr_intf = l3_intf.l3a_intf_id;
    ...
    vxlan_acc->acc_l3_ing_intf = ingress_if;
                     ...
                     bcm_flow_handle_t flow_handle)
{
	...
	BCM_IF_ERROR_RETURN(bcm_port_learn_set(unit, *net_flow_port, BCM_PORT_LEARN_FWD));
	...
	vxlan_net->net_l3_egr_intf = l3_intf.l3a_intf_id;

    /* Create L3 ingress interface */
	bcm_l3_ingress_t_init(&l3_ingress);
	l3_ingress.vrf = vxlan_cfg->vrf;
	l3_ingress.ipmc_intf_id  = vxlan_net->net_l3_egr_intf;
	rv = bcm_l3_ingress_create(unit, &l3_ingress, &ingress_if);
	if (BCM_FAILURE(rv)) {
	...
	BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, vxlan_net->net_port, bcmPortControlL3Ingress, ingress_if));

	bcm_vlan_control_vlan_t vlan_ctrl;
	bcm_vlan_control_vlan_t_init(&vlan_ctrl);
	vlan_ctrl.ingress_if = ingress_if;
	BCM_IF_ERROR_RETURN(bcm_vlan_control_vlan_selective_set(unit,vxlan_net->net_ol_vlan, BCM_VLAN_CONTROL_VLAN_INGRESS_IF_MASK, &vlan_ctrl));

    return BCM_E_NONE;
}

/* Configure IPMC entry to match Payload IPMC packet (Network to Access flow) */
bcm_error_t
config_access_ipmc_entry(int unit, vxlan_cfg_t *vxlan_cfg,vxlan_acc_cfg_t *vxlan_acc,
    ...
    ipmc_encap.l3_intf    = vxlan_acc->acc_l3_egr_intf;
    ...
    printf("Add L3 IPMC entry for RIOT\n");
    bcm_ipmc_addr_t ipmc_entry;

    bcm_ipmc_addr_t_init(&ipmc_entry);
    ipmc_entry.flags = 0;
    ipmc_entry.mc_ip_addr = vxlan_acc->acc_src_ip;
    ipmc_entry.mc_ip_mask = 0xFFFFFFFF;
    ipmc_entry.s_ip_addr  = vxlan_acc->acc_dest_ip;
    ipmc_entry.vrf        = vxlan_cfg->vrf;
    ipmc_entry.mtu        = 1500;
    ipmc_entry.ing_intf   = vxlan_net->net_l3_ing_intf ;
    ipmc_entry.group      = multicast_group;
    ipmc_entry.lookup_class = vxlan_cfg->gbp_id; /*Add GBP-ID as Destination Class (DID)*/
...
config_em_fp(int unit, uint16 gbp_sid) {
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualify_OuterL5PayloadByte3(unit, entry_id, first_byte_of_gbp_sid, 0xFF)); /* Qualify GBP ID first byte*/
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    vxlan_acc_cfg_t vxlan_acc = {
        ...
        "01:00:5E:01:01:01",     /*payload src mac*/
        0,                       /*access vpn*/
        0,                       /* dummy for Access side VPN */
        0xE0010101,              /*payload sip*/
        0x1E1E1E01,              /*payload dip*/
        0,                       /*mcast group*/
    ...
    /* Configure RIOT IPMC entry that also sets GBP-DID */
    rv = config_access_ipmc_entry(unit, &vxlan_cfg, &vxlan_acc, &vxlan_net);
    if (BCM_FAILURE(rv)) {
        printf("\nError in Configuring IPMC entry: %s\n", bcm_errmsg(rv));
        return rv;
    }
