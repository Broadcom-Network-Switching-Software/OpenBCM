...
/*
 * Feature  : Vxlan Bridging, network to access unicast IPv4 tunnel termination
 *            with Vxlan GBP-SID,DID verification based on MAC-DA+Vlan in EM-FP entry
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_gbp_ipv4_NtoA_uc_em_fp.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan-gbp-em.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_gbp_ipv4_NtoA_uc_em_fp_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    rv = add_to_l2_table(unit,vxlan_acc.acc_local_mac,vxlan_cfg.vxlan_vpn, vxlan_acc.phy_Gport);
    if (BCM_FAILURE(rv)) {
        printf("\nError in l2 entry config: %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = config_gbp_sid_verify(unit,net_flow_port,vxlan_acc.acc_local_mac,vxlan_acc.acc_vlan);
    if (BCM_FAILURE(rv)) {
