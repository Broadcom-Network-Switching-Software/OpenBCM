 ...
 *            Vxlan GBP-SID, DID verification based on MAC-DA+Vlan in IFP entry
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_gbp_ipv4_NtoA_mc_ifp.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan-gbp.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_gbp_ipv4_NtoA_mc_ifp_log.txt
 *
 * Test Topology :
 *                                  +----------------------+ known_mc <-
 ...
 *             - Match based on MAC-DA+Vlan of the payload destination
 *
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see the drop counters increment for packet GBP-SID matching condition
 */
/*
Packet:
...
data=
...
Input Hex (ce1: Network):
Broadcast case:
data=
...
/**** global constants ****/
const int   IN_PORT_NUM = 2;   // 1 or 2
...
test_setup(int unit)
{
    ...
    for (i = 0; i < IN_PORT_NUM; i++){
        ...
        if (BCM_E_NONE != ingress_port_setup(unit, ingress_port[i], group_config, &entry[i])) {
 ...
 */
bcm_error_t
config_gbp_sid_verify(int unit, bcm_gport_t flow_port,bcm_mac_t dst_mac,bcm_vlan_t vlan,int priority)
{
    bcm_field_entry_t entry;
    bcm_field_group_config_t group_config;
    int gbp_sid = 0xfab;
    bcm_mac_t mac_mask = "ff:ff:ff:ff:ff:ff";
    bcm_vlan_t mask = 0xffff;
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualify_DstMac(unit,entry,dst_mac,mac_mask));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_OuterVlanId(unit,entry,vlan,mask));
...
bcm_ip_t        net1_remote_ip  = 0x0a0a0a01;  /* 10.10.10.1  - no use for AtoN */
bcm_ip_t        net1_local_ip   = 0x0a0a0a03;  /* 10.10.10.3 */
bcm_ip_t        net1_mcast_ip   = 0xe1020202;  /* 225.2.2.2 */
int             net1_dvp_group  = 10 + 2048;
...
bcm_ip_t        net2_remote_ip  = 0x0a0a0a02;  /* 10.10.10.2  - no use for AtoN */
bcm_ip_t        net2_local_ip   = 0x0a0a0a03;  /* 10.10.10.3 */
bcm_ip_t        net2_mcast_ip   = 0xe1020101;  /* 225.2.1.1 */
int             net2_dvp_group  = 11 + 2048;
...
config_vxlan(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(vxlan_port_create(unit, my_modid, ingress_port[0],  FLAGS_PROVIDER, &provider_port1));
    BCM_IF_ERROR_RETURN(vxlan_port_create(unit, my_modid, ingress_port[1],  FLAGS_PROVIDER, &provider_port2));
    ...
    int priority=3;
    rv = config_gbp_sid_verify(unit,provider_port1.vxlan_port.flow_port.flow_port_id,
                                "01:00:5e:01:01:01",acc_vid,priority);
    if (BCM_FAILURE(rv)) {
    ...
    priority++;
    rv = config_gbp_sid_verify(unit,provider_port2.vxlan_port.flow_port.flow_port_id,
                                "ff:ff:ff:ff:ff:ff",acc_vid,priority);
    if (BCM_FAILURE(rv)) {
        printf("\nError executing config_gbp_sid_verify(): %s.\n", bcm_errmsg(rv));
        return rv;
    }
 ...
 *  0a01e102 0202ffff 12b5006e 00000000
...
verify(int unit)
{
    ...
    snprintf(str, 1184, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port[0]);
    bshell(unit, str);
    sal_sleep(2);
    bshell(unit, "show c");
    ...
    snprintf(str, 1184, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port[1]);
    bshell(unit, str);
    sal_sleep(2);
    bshell(unit, "show c");
    /* dump l2 table to see if SA learned againt SVP */
