...
/*
 * Feature  : Vxlan Riot, IPv4 access to network tunnel initiation flow, removing SD_TAG in the payload
              VxLAN GBP SID added in VxLAN header by L3 host table entry that's hit.
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_gbp_ipv4_AtoN_uc_l3.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan-gbp.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_gbp_ipv4_AtoN_uc_l3_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *             - removing sdtag in the payload
 *             - Additionally, by means of L3 Host table entry that's hit and associated FP rule
...
data[0020]: 0a01 1414 1401 c000 12b5 0050 0000 8800
data[0030]: 0fab 0123 4500 0000 0002 0202 0000 0001
...
struct vxlan_cfg_t {
    ...
    uint16 gbp_sid; /*GBP SID to be added in VxLAN header*/
};
 ...
 */
bcm_error_t
l3_host_add(int unit, bcm_ip_t ip_addr, bcm_ip_t ip_mask,
                         bcm_vrf_t vrf, bcm_if_t ol_obj, bcm_if_t ul_obj,int class_id)
{
    int rv = BCM_E_NONE;
    ...
    host.l3a_intf = ol_obj;
    host.l3a_ul_intf = ul_obj;
    host.l3a_vrf = vrf;
    host.l3a_lookup_class = class_id; /* Class ID */
...
vxlan_tunnel_term_update(int unit, bcm_gport_t tnl_id, uint32 mflags)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_vxlan_tunnel_terminator_get(unit, &tnl_info));
                    ...
                    bcm_gport_t *acc_flow_port)
{
	...
	*   0.0.0.0 /0 VRF--> DROP
	*/
	rv = l3_host_add(unit,0,0,vxlan_cfg->vrf,
                      100001, 0,0);
	if (BCM_FAILURE(rv)) {
        printf("\nError in default route l3_host_add: %s\n", bcm_errmsg(rv));
        return rv;
    }


	printf("Access: add the l3 route\n");
    rv = l3_host_add(unit,vxlan_acc->acc_src_ip,mask,vxlan_cfg->vrf,
                      acc_egr_obj_ol, 0,0);
    if (BCM_FAILURE(rv)) {
                     ...
                     bcm_flow_handle_t flow_handle)
{
	...
	/*
	 * sdtag operation: delete
	 */
	/* Disable payload tag delete action in EGR_DVP */
	rv = bcm_vlan_control_port_set(unit, vxlan_net->net_gport, bcmVlanPortPayloadTagsDelete, 1);
	if (BCM_FAILURE(rv)) {
    ...
    printf("Network: add GBP-SID as L3 host entry class ID\n");
    rv = l3_host_add(unit,vxlan_acc->acc_dest_ip,mask,vxlan_cfg->vrf,
                        net_egr_obj_ol, net_egr_obj_ul, vxlan_cfg->gbp_sid);
    if (BCM_FAILURE(rv)) {
        printf("\nError in l3_host_add: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    config_fp_vxlan_gbp_l3_classid(unit, vxlan_cfg.gbp_sid);
    if (BCM_FAILURE(rv)) {
