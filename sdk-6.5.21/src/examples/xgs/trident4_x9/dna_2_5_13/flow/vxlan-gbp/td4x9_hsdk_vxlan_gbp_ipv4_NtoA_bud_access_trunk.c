 ...
 *            Access has trunk, distribution based on incoming packet's VxLAN GBP-ID field
 *
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_gbp_ipv4_bud_access_trunk.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan-gbp.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_gbp_ipv4_bud_access_trunk_log.txt
 *
 * Test Topology :
 *
 *                                +----------------------+
...
multi_egress_port_setup(int unit, bcm_vlan_t acc_vid, bcm_vlan_t net_vid, bcm_port_t *port)
{
    ...
    for(i=0; i<NO_OF_TRUNK_MEMBERS+1; i++)
    {
        ...
        if(i == NO_OF_TRUNK_MEMBERS){ /*Network port*/
            print "";
...
vxlan_access_port_create(int unit, bcm_vpn_t vpn, vxlan_phy_port_s *vxlan_access_port)
{
    ...
    if(BCM_GPORT_IS_TRUNK(vxlan_access_port->gport_id)) {
        int i = 0;
        for(i = 0; i < NO_OF_TRUNK_MEMBERS; i++) {
            rv = bcm_vlan_gport_add(unit, vpn, egress_phy_gport[i], 0);
            if (BCM_FAILURE(rv)) {
...
vxlan_trunk_port_create(int unit, int modid, bcm_trunk_t tid, int flags, vxlan_phy_port_s * port)
{
    ...
    for(i = 0; i < NO_OF_TRUNK_MEMBERS; i++) {
        BCM_IF_ERROR_RETURN( bcm_port_control_set(unit, egress_port[i], bcmPortControlVxlanTunnelbasedVnId, 0));
        if (FLAGS_PROVIDER == flags) {
            BCM_IF_ERROR_RETURN( bcm_port_control_set(unit, egress_port[i], bcmPortControlVxlanEnable, 1));
        }
        else {
            BCM_IF_ERROR_RETURN( bcm_port_control_set(unit, egress_port[i], bcmPortControlVxlanEnable, 0));
            BCM_IF_ERROR_RETURN( bcm_vlan_control_port_set(unit, egress_port[i], bcmVlanTranslateIngressEnable, 1));

            /* Configure ING_L2_IIF_TABLE.PORT_GROUP_ID - use port id as the port group */
            BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, egress_port[i], bcmPortClassIngress, (port->gport_id & 0xff)));
        }
    }
    port->modid             = modid;
    port->port_id           = tid;
...
config_vxlan(int unit)
{
   ...
   rv = config_FlexDigest(unit);
    ...
    BCM_IF_ERROR_RETURN(vxlan_trunk_port_create(unit, my_modid, tid,  FLAGS_CUSTOMER, &customer_port));
    BCM_IF_ERROR_RETURN(vxlan_port_create(unit, my_modid, egress_port[NO_OF_TRUNK_MEMBERS],  FLAGS_PROVIDER, &provider_port2));
