...
/*
 * Feature  : Vxlan Bridging, access to network unknown multicast IPv4 tunnel initiation
 *            with Vxlan GBP-SID assignment in IFP entry
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_gbp_ipv4_AtoN_mc_ifp.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan-gbp.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_gbp_ipv4_AtoN_mc_ifp_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *     e) Match fp rule on the inport and assign gbp sid
 *
 ...
 *        and vp's
 *
 *     b) Transmit unknown multicast packets single outer tagged
...
Ingress Hex Packet:
tx port=1 data=0x01005E01010200000000111181000015080045000050000040004011569901010101E101010100350035003C54F774657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374
...
data[0030]: 0fab 0123 4500 0100 5e01 0102 0000 0000
...
verify(int unit)
{
    ...
    printf("\nSending unknown multicast packet to ingress_port access side:%d\n",
                                         ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x01005E01010200000000111181000015080045000050000040004011569901010101E101010100350035003C54F774657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "sleep 2");
    bshell(unit, "show c");
 ...
 */
bcm_error_t
config_gbp_sid_port(int unit, bcm_port_t inport,bcm_mac_t src_mac,bcm_vlan_t vlan)
{
    bcm_field_group_t group;
    bcm_field_entry_t entry;
    bcm_field_group_config_t group_config;
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualify_SrcMac(unit,entry,src_mac,mac_mask));
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    rv = config_gbp_sid_port(unit,vxlan_acc.acc_port,vxlan_acc.acc_local_mac,vxlan_acc.acc_vlan);
    if (BCM_FAILURE(rv)) {
