...
/*
 * Feature  : Vxlan Bridging Bud mode:
 *               Network to network known multicast and Access flow
 *               VxLAN GBP-ID validation in 2nd pass derived through L2 entry fail, packet drops
 *
 *
 * Usage    : BCM.0> cint td4x9_hsdk_vxlan_gbp_ipv4_NtoA_bud_pass1_ifp.c
 *
 * config   : bcm56780_a0-generic-48x100-8x400_vxlan-gbp.config.yml
 *
 * Log file : td4x9_hsdk_vxlan_gbp_ipv4_NtoA_bud_pass1_ifp.c
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *                                  |                      |  ingress_port      +---+
 *   SMAC 00::11:11                 |                      +--------------------+   |
 *            +---+ X(DROP)  VLAN-21|                      |00::22:22           +---+
 *   Host(acc)|   +-----------------+        DUT           |  Network_port      00::02
 *            +---+    Egress_port  |                      |
 *                                  |                      |
 *            +---+          VLAN-23|                      |
 *     Network|   +-----------------|                      |
 ...
 * Additionally:
 *             GBP-SID from packet extracted by Exact Match
 *             GBP-DID assigned by L2 entry
 *             Matching done by IFP
 *             Network to Network (IPMC transit) goes through anyway
 *             GBP-ID matching by IFP is "no match" and Decapsulated packet doesn't egresses access port (Drops)
 *
 ...
 *
 *     b) The GBP DID is configured as class ID in L2 host entry which the decap packet hits
 ...
 *        If class ID set by EM(GBP-SID) matches class ID set by L2 host(GBP-DID)
 ...
 *        - Network to Network Transit packet hits IPMC entry and goes out successfully
 *        - No GBP match occurs, decapsulated packet should drop.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see the 1 packet : ipmc routed packet on network (as decap packet drops at ingress)
...
Ingress Hex Packet:
tx 1 pbm=1 data=
...
data[0030]: 0dec 0123 4500 0100 5e01 0101 0000 0000
...
======
No Access egress packet as it's Dropped at ingress by IFP
...
Entry 1:
 ...
 - Matches class ID set by L2 table : GBP-DID
...
*/
bcm_error_t
config_gbp_ifp_policy(int unit, bcm_gport_t flow_port, uint16 gbp_sid, uint16 gbp_did) {
    bcm_field_group_config_t group_config;
    int entry_id;
    ...
    /* Entry-1: No OP entry */
    BCM_IF_ERROR_RETURN(bcm_field_entry_create(unit, group_config.group, &entry_id));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_SrcGport(unit, entry_id, flow_port));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_ExactMatchOpaqueObject0(unit, entry_id, gbp_sid, 0xFFFF)); /* Qualify EM Class ID */
    BCM_IF_ERROR_RETURN(bcm_field_qualify_DstClassL2(unit, entry_id, gbp_did, 0xFFFF)); /* Qualify L3 Dest class ID */
...
uint16 vxlan_gbp_id = 0xFAB; /*VxLAN GBP ID that's also used as Class ID*/
bcm_error_t
config_vxlan(int unit)
{
    ...
    rv = config_em_fp(unit,vxlan_gbp_id);
    if (BCM_FAILURE(rv)) {
    ...
    rv = config_gbp_ifp_policy(unit, provider_port1.vxlan_port[0].flow_port.flow_port_id, vxlan_gbp_id, vxlan_gbp_id);
    if (BCM_FAILURE(rv)) {
...
verify(int unit)
{
    ...
    bshell(unit, "e; fp show");
    printf("\nSending known multicast packet to ingress_port network side: port-%d\n", ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port);
