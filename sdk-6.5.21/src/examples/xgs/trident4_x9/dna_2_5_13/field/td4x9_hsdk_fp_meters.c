 ...
 *
 *  Usage : BCM.0> cint td4x9_hsdk_fp_meters.c YES [Stage=2/3]
 *           E.g.  cint td4x9_hsdk_fp_meters.c YES 2      <<<<< Stage: IFP
 *           E.g.  cint td4x9_hsdk_fp_meters.c YES 3      <<<<< Stage: EFP
 *
 *  config   : BCM956780-56780_A0-fp-config.yml
 *
 *  Log file : td4x9_hsdk_fp_meters_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+
                          ...
                          int index_num, int CountDrop, uint32 *stat_action_id /*OUT*/)
{
    ...
    if( FpStage == bcmFieldStageLookup ) {
        ...
        if( CObjId == 2 ) {
            printf("METER OBJ is OBJ2\n");
            index_op->object[0] = bcmFlexctrObjectIngIfpOpaqueObj2;
        } else {
            printf("METER OBJ MUST be OBJ2\n");
            return BCM_E_PARAM;
        }
    ...
    TestFunc()
{
    ...
    if( opt_IsIfp ) {
        ...
        hint.value = bcmFlexctrObjectIngIfpOpaqueObj2;
    ...
    if( 1 )
    {
        ...
        stat_desc[statid_used] = "Entry 1 GREEN: ";
        ...
        if( opt_IsIfp ) {
            ...
            stat_desc[statid_used] = "Entry 1 RED  : ";
            BCM_IF_ERROR_RETURN( bcm_field_entry_flexctr_attach(unit, entry,
                                            &flexctr_cfg[statid_used]) );
            statid_used++;
        }
