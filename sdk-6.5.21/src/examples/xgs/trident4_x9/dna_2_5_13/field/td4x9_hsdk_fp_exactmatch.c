 ...
 *
 *  Usage : BCM.0> cint td4x9_hsdk_fp_exactmatch.c
 *           E.g.  cint td4x9_hsdk_fp_exactmatch.c
 *
 *  config   : BCM956780-56780_A0-fp-config.yml
 *
 *  Log file : td4x9_hsdk_fp_exactmatch_log.txt
 *
                          ...
                          int index_num, int CountDrop, uint32 *stat_action_id /*OUT*/)
{
    ...
    if( FpStage == bcmFieldStageIngressExactMatch ) {
        printf(">>>> EXACTMATCH STAT...\n");
        
        if( 1 ) {
            /* X9 is user supplied index based stat, just like IFP */
            action.source = bcmFlexctrSourceExactMatch;
            action.mode = bcmFlexctrCounterModeNormal;
            action.index_num = 8192;

            /* Counter index is IfpOpaqueObj0. */
            index_op->object[0] = bcmFlexctrObjectIngEmFtOpaqueObj;
            ...
            /* For X11, which is having EM hit index based stat */
            action.mode = bcmFlexctrCounterModeSlim;
            action.source = bcmFlexctrSourceExactMatch;
            action.index_num = 262144;  // As EM is a HASH table, index is not known, so give full scale
    ...
    TestFunc(int prio)
{
    ...
    if( opt_IsEM ) {
        //BCM_FIELD_ASET_ADD(group_config.aset,
                        //bcmFieldActionStatGroupWithoutCounterIndex); // on x11 this is supported
        BCM_FIELD_ASET_ADD(group_config.aset, bcmFieldActionStatGroup); // On X9 only this is supported
    } else {
    ...
    if( 1 )
    {
        ...
        flexctr_cfg[statid_used].counter_index = 0;
        ...
        /* As x9 does not support EM hit index based stats, it needs stat index supplied
           by the user just like in the case of IFP. Hence below line is connected out for X9 */
        /* flexctr_cfg[statid_used].counter_index = entry; */ /* Set it to eid for stat get
                                                                     hit-index based */
        printf("ENTRY %d\n", entry);
        statid_used++;
    }
