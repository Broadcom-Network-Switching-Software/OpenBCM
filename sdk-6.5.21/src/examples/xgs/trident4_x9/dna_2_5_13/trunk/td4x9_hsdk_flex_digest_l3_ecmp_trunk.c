 ...
 *
 *  Usage    : BCM.0> cint td4x9_hsdk_flex_digest_l3_ecmp_trunk.c
 *
 *  config   : bcm56780_a0-generic-48x100-8x400_trunk.config.yml
 *
 *  Log file : td4x9_hsdk_flex_digest_l3_ecmp_trunk_log.txt
 *
 *  Test Topology :
 *
 *
 ...
 *    b)Expected Result:
 *      ================
 *      Packets are loadbalanced across all egress ports as can be seen fron 'show c' output
 */
...
/************/
  ...
  /*****To change as separate API */
    ...
    /*NOTE : This is optional on older XGS devices, but mandatory on td4.x9, L3 flow won't work*/
...
void verify(int unit)
{
  ...
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA0000000022228100000C08004500002E0000000040FF9B1D0A0A0A01C0A80A0143031FE5B29ECF90C1813A5699BD7802A69EFB499E95BA5ABF76C348747B; sleep quiet 5", Ingress_port);
  bshell(unit, str);
  bshell(unit, "show c");
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA0000000022228100000C08004500002E0000000040FF9AEF0A0A0A18C0A80A183A101D7B400184D9B155D1506CFE827B76834F3CF7F655508526FA5E7E81; sleep quiet 5", Ingress_port);
  bshell(unit, str);
  bshell(unit, "show c");
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA0000000022228100000C08004500002E0000000040FF9ADD0A0A0A21C0A80A21C46587431445B97663DBABC34FDF83CDBC36E5B51A205DEAE7B3B8B4B175; sleep quiet 2", Ingress_port);
  bshell(unit, str);
  bshell(unit, "show c");
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA0000000022228100000C08004500002E0000000040FF9AB90A0A0A33C0A80A33B894D7BC72CE32C92D38603C1DC9CAE99C033B7C6F2544544BA1321E695D; sleep quiet 2", Ingress_port);
  bshell(unit, str);
  bshell(unit, "show c");
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA0000000022228100000C08004500002E0000000040FF9A9B0A0A0A42C0A80A4257626DF6D18E572A9849379B542A94C364A565C9223A795568ECCD72668F; sleep quiet 2", Ingress_port);
  bshell(unit, str);
  bshell(unit, "show c");
  snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA0000000022228100000C08004500002E0000000040FF9A710A0A0A57C0A80A57EEE4EFB98ABCA134A408326993D152D3349263E08C4B38A5AFD1E51FA4DC; sleep quiet 2", Ingress_port);
  bshell(unit, str);
  bshell(unit, "show c");
