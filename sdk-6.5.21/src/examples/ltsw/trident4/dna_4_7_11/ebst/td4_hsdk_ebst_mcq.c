 ...
 * Usage    : BCM.0> cint TD4_HSDK_EBST_mcq.c
 * config   : bcm56880_a0-generic-32x400_vxlan.config.yml
 * Log file : TD4_HSDK_EBST_mcq_test.log
 *
 * Purpose: Demonstrate EBST threshold profile configuration for MC queue on a selected port
 *
 * Description: This example demonstrates the sequence to configure EBST to monitor 
 *              multicast Queue 0 on xe0 (logical port 38.) 
 ...
 * Summary:
 * ========
 * CINT example to demonstrate Even Buffer Statistics Tracking (EBST) capability on Trident4 (MCQ case)
 *
 ...
 *        - Enable BST counters
 *     c) Set threshold for MC Queue 8 (as well as UC Queue 0 for the ease of presentation) of the observation port xe0 (Port 38)
 *     d) Configure Monitoring on (Port,Queue)
 *     e) Set ebst monitor on Port 38 (the observation port) MC Queue 8 (as well as UC Queue 0 for the ease of presentation)
 ...
 *     c) Read the status of EBST_DATA buffer of the multiicast queue with respect to the observation port
 *     d) Expected Result:
 *     ===================
