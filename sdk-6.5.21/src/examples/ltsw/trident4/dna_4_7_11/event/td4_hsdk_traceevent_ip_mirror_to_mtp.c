...
/*
 * Feature  : Trace Event, mirror 'Ingress Pipeline: Station Move for Static L2 Source address' packets to MTP (Mirror-to-Port).
 *
 * Usage    : BCM.0> cint TD4_HSDK_TraceEvent_ip_mirror_to_mtp.c
 *
 * Config   : bcm56880_a0-generic-32x400_event.config.yml
 *
 * Log file : TD4_HSDK_TraceEvent_ip_mirror_to_mtp_log.txt
 *
 * Test Topology :
 *
 *
 ...
 * Summary:
 * ========
 * CINT example to demonstrate "Trace Event" feature by mirroring 'Station Move for
 * Static L2 Source address' packets to MTP.
 *
 ...
 *       For the second packet, it is dropped but mirrored to MTP due to 'Station Move for
...
[bcmPWN.0][RX metadata information]
        ...
        SRC_PORT_NUM                    :0x4(4)
        SWITCH                          :0x1(1)
        MATCH_ID_LO                     :0x88905242(-2003807678)
...
config_event_action(int unit)
{
    ...
    /* Change the default outer TPID for mirror_to_port. */
    /* This would treat any incoming packet (mirror packets via MAC loopback)
     * as untagged and assign PVID=1 as its VLAN domain to isolate the traffic.
     * Then, same packets would not be clarified as 'Station Move for Static L2
     * Source address' and trigger the trace event to mirror to MTP again.
     */
    printf("Change the default outer TPID for mirror_to_port: 0x9100.\n");
    rv = bcm_port_tpid_set(unit, mirror_to_port, 0x9100);
    if (BCM_FAILURE(rv)) {
        printf("\nError in default TPID config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
	...
	/* MIRROR-TO-MTP for Trace event 'Station Move for Static L2 Source address'. */
    printf("Enable the specific trace event and mirror it to MTP.\n");
	/* Mirror Port */
	bcm_mirror_destination_t mtp;
	bcm_mirror_destination_t_init(&mtp);
    bcm_port_gport_get(unit, mirror_to_port, &(mtp.gport));
	/* Create Mirror destination. */
	rv = bcm_mirror_destination_create(unit, &mtp);
    if (BCM_FAILURE(rv)) {
        printf("\nError in creating MTP: %s.\n", bcm_errmsg(rv));
        return rv;
    }
	/* Setup a Mirror source about bcmPktTraceEventIngressL2SrcStaticMove event */
	...
	rv = bcm_mirror_source_dest_add(unit, &source, mtp.mirror_dest_id);
    if (BCM_FAILURE(rv)) {
