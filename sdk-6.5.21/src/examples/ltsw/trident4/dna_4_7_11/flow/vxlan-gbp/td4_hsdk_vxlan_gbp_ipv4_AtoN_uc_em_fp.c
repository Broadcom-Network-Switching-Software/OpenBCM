...
/*
 * Feature  : TD4(56880) Vxlan Bridging, access to network unicast IPv4 tunnel initiation
 *            with Vxlan GBP-SID assignment in Exact Match(EM) FP entry
 *
 * Usage    : BCM.0> cint td4_hsdk_vxlan_gbp_ipv4_AtoN_uc_em_fp.c
 *
 * config   : bcm56880_a0-generic-32x400.config_em.yml
 *
 * Log file : td4_hsdk_vxlan_gbp_ipv4_AtoN_uc_em_fp_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *     e) Match EM fp rule on the inport and assign gbp sid
 *
...
data[0030]: 0fab 0123 4500 0000 0000 aaaa 0000 0000
                    ...
                    bcm_flow_handle_t flow_handle)
{
    ...
    */
    ...
    rv = bcm_port_class_set(unit, vxlan_acc->acc_port, bcmPortClassIngress,
 ...
 */
bcm_error_t
config_gbp_sid_port(int unit, bcm_port_t inport)
{
    bcm_field_group_t group;
    bcm_field_entry_t entry;
    bcm_field_group_config_t group_config;
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    rv = config_gbp_sid_port(unit,vxlan_acc.acc_port);
    if (BCM_FAILURE(rv)) {
