 ...
 *
 * Usage    : BCM.0> cint TD4_HSDK_VxLAN_v4_net_to_net_ecmp_using_entropy.c
 *
 * config   : bcm56880_a0-generic-32x400_vxlan.config.yml
 *
 * Log file : TD4_HSDK_VxLAN_v4_net_to_net_ecmp_using_entropy_log.txt
 *
 * Test Topology :
 *                                                                                       Underlay+NextHop
...
config_vxlan_ecmp_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, bcm_flow_handle_t flow_handle)
{
    ...
    bcm_if_t      net_egr_id_ol[NO_OF_ECMP_MEMBERS];
    ...
    for(i=0; i<NO_OF_ECMP_MEMBERS; i++)
    {
        ...
        rv = vxlan_create_egr_obj(unit, BCM_L3_FLOW_ONLY, net_intf_id_ol, vxlan_net->net_remote_mac[i],
                        egress_phy_gport[i], vxlan_net->net_vlan, &net_egr_id_ol[i]);
        if (BCM_FAILURE(rv)) {
            printf("\nError in configuring egress obj (OL): %s.\n", bcm_errmsg(rv));
            return rv;
        }
    }
    printf("Create network L3 ECMP object (OL).\n");
    rv = vxlan_create_ecmp_obj(unit, 0, net_egr_id_ol, &net_ecmp_obj_id_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in configuring ECMP obj (OL): %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = add_to_l3_route_table(unit, vxlan_net->remote_ip, vxlan_net->vrf, net_ecmp_obj_id_ol, 0);
    if (BCM_FAILURE(rv)) {
...
config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, bcm_flow_handle_t flow_handle)
{
    ...
    printf("Create network L3 Egress object.\n");
    rv = vxlan_create_egr_obj(unit, BCM_L3_FLOW_ONLY|BCM_L3_FLAGS2_UNDERLAY, net_intf_id, vxlan_net->net_remote_mac[0],
...
config_FlexDigest(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_flexdigest_action_add(unit, entry, bcmFlexDigestActionExtractBinSetBCmds14, BCM_FLEXDIGEST_FIELD_OUTER_L4_DST_PORT, 0xffff));
    BCM_IF_ERROR_RETURN(bcm_flexdigest_action_add(unit, entry, bcmFlexDigestActionExtractBinSetBCmds15, BCM_FLEXDIGEST_FIELD_OUTER_L4_SRC_PORT, 0xffff));
