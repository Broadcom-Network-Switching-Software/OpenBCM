...
/*
 * Feature  : TD4(56880) Vxlan Bridging ,  network to network tunnel termination and initiate to the same VNID
 *
 * Usage    : BCM.0> cint TD4_HSDK_VXLAN_v4_net_to_net_UC_Same_VNID.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_VXLAN_v4_net_to_net_UC_Same_VNID_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * Summary:
 * ========
 * Cint example to demonstrate VxLAN Bridging configuration for tunnel Termination and initiate to the same VNID -
 *             - network to network (decap & ecnap flow)
 *
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see the network packet on the egress port with the same VNID in the tunnel header
...
Ingress Hex Packet:
tx 1 pbm=1 data=
0x00000000222200000000000281000016
080045000082000040003F1165B7C0A8010
10A0A0A01D90312b5006E790B0800000001
23450000000000BBBB00000000AAAA08004
50000500001000040117497020202020101
010100350035003C32F6746573747465737
47465737474657374746573747465737474
65737474657374746573747465737474657
3747465737474657374
...
data[0010]: 0800 4500 0082 0000 0000 3f11 a4b6 0a0a
data[0020]: 0a01 c0a8 0202 c000 12b5 006e 0000 0800
data[0030]: 0000 0123 4500 0000 0000 bbbb 0000 0000
data[0040]: aaaa 0800 4500 0050 0001 0000 4011 7497
data[0050]: 0202 0202 0101 0101 0035 0035 003c 32f6
...
vxlan_phy_port_s    provider_port1, provider_port2;
...
config_vxlan(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(vxlan_port_create(unit, my_modid, egress_port[0], FLAGS_PROVIDER, &provider_port2));
    ...
    provider_port1.vxlan_port[0].vnid       = net1_vnid;
    ...
    provider_port2.vxlan_port[0].vnid       = net2_vnid;
    ...
    provider_port2.vxlan_port[0].remote_ip  = net2_remote_ip;
    provider_port2.vxlan_port[0].mc_tnl_flag = 0;
    ...
    BCM_IF_ERROR_RETURN(vxlan_l2_add(unit, "00:00:00:00:bb:bb", vxlan_vpn, 
                           provider_port2.vxlan_port[0].flow_port.flow_port_id));

    /* Network-to-Access: Add dest-mac Address => VxLAN termination. */
    BCM_IF_ERROR_RETURN(vxlan_l2_station_add(unit, net1_local_mac));
...
verify(int unit)
{
    ...
    snprintf(str, 1184, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port);
    bshell(unit, str);
    sal_sleep(2);
    bshell(unit, "show c");
