...
/*
 * Feature  : Vxlan Riot, Inner TTL error packet processing in termination flow
 *
 * Usage    : BCM.0> cint TD4_HSDK_ipv4_vxlan_riot_NtoA_Inner_ttl_error.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_ipv4_vxlan_riot_NtoA_Inner_ttl_error_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * Summary:
 * 
 * Cint example to verify the processing of Vxlan tunnel terminated packets with inner TTL=0/1
 *             - packet trace is added to copy TTL error packets to CPU
 *
 ...
 * 			IFP is not applied to ingress packet in this case, since packet trace will be used to capture TTL error packets to CPU
 *
 ...
 *
 *     b) Transmit the known unicast packet with inner TTL=0/1. The contents of the packet
 *        are printed on screen.
 *
 *     c) Expected Result:
 *     
 *       We can see the inner TTL error (TTL=0/1) tunnel packets are copied to CPU and dropped.
 *		 No packet is seen from the egress port
...
Ingress Hex Packet (Inner TTL=1):
tx port=2 data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E0000000001062d83282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A

Ingress Hex Packet (Inner TTL=0):
tx port=2 data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E0000000000062e83282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A

Packets reseived on CPU port:
...
[bcmPWN.0]Packet[1]: data[0040]: 0201 0800 4500 002e 0000 0000 0106 2d83
...
verify(int unit)
{
	...
	/* Sending TTL=1 Packets */
    printf("\nSending TTL=1 packet to ingress network side:%d\n", ingress_port);
	snprintf(str, 512, "tx 1 pbm=%d data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E0000000001062d83282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A; sleep quiet 3", ingress_port);
	bshell(unit, str);
	
	/* Sending TTL = 0 Packets*/
	printf("\nSending TTL=0 packet to ingress network side:%d\n", ingress_port);
	snprintf(str, 512, "tx 1 pbm=%d data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d88080000000123450000000000040100000000020108004500002E0000000000062e83282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A; sleep quiet 3", ingress_port);
	bshell(unit, str);
...
config_vxlan_ipv4_vtep(int unit)
{
	...
	/* 
	 * Trace Event adding to trap ttl error packets to CPU 
	 */
	bcm_switch_trace_event_mon_t monitor;
	bcm_switch_trace_event_mon_t_init (&monitor);
	monitor.trace_event = bcmPktTraceEventIngressL3TtlError;
	monitor.actions |= BCM_SWITCH_MON_ACTION_COPY_TO_CPU;
	rv = bcm_switch_trace_event_mon_set(unit, &monitor);
