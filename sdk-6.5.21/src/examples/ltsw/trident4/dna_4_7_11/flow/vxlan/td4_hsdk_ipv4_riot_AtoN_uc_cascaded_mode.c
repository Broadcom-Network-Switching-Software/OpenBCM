...
/*
 * Feature  : Vxlan Riot, IPv4 access to network tunnel initiation flow using
 *            cascaded mode.
 *
 * Usage    : BCM.0> cint TD4_HSDK_ipv4_riot_AtoN_uc_cascaded_mode.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_ipv4_riot_AtoN_uc_cascaded_mode_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * Summary:
 * ========
 * Cint example to demonstrate VxLAN Riot configuration for tunnel Initiation using
 * cascaded egress object for tunnel encapsulation.
 *
 * In TD4 DNA, L3 forwarding table provides both overlay next-hop and underlay
 * next-hop. In some usecases, cascaded mode is preferred to save convergence
 * time for any changes in underlay network. In cascaded mode, L3 forwarding
 * table provides an index to ECMP member0 table which inturn provides both
 * overlay next-hop and underlay next-hop.
 *
 ...
 *        Setup network port for routing into vxlan tunnel.Use "BCM_FLOW_PORT_ENCAP_FLAG_EGRESS_CASCADED"
 *        while setting network port's encapsulation(i.e bcm_flow_port_encap_set()).
 *        This also does the necessary configurations of vxlan global settings, vlan,
...
{
    char str[512];
    print "Executing \"vlan show\"";
    bshell(unit, "vlan show");
    print "Executing \"l2 show\"";
    bshell(unit, "l2 show");
    print "Executing \"l3 route show\"";
    bshell(unit, "l3 route show");
    print "Executing \"l3 egress show\"";
    bshell(unit, "l3 egress show");
    print "Executing \"l3 intf show\"";
                     ...
                     bcm_flow_handle_t flow_handle)
{
    ...
    rv = bcm_vlan_control_port_set(unit, vxlan_net->net_gport,
    ...
    einfo.vpn  = vxlan_net->net_vpn;
	  einfo.dvp_group = vxlan_net->net_vp_group;
    ...
    peinfo.dvp_group = vxlan_net->net_vp_group;
    peinfo.flags = BCM_FLOW_PORT_ENCAP_FLAG_EGRESS_CASCADED;
    ...
    /*Need not specify underlay egress object as
     * overlay egress object is in cascaded mode.*/
    rv = l3_route_add(unit,vxlan_acc->acc_dest_ip,mask,vxlan_cfg->vrf,
                        net_egr_obj_ol, 0/*net_egr_obj_ul*/);
    if (BCM_FAILURE(rv)) {
        printf("\nError in l3_route_add: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    vxlan_net_cfg_t vxlan_net = {
		...
		(11 + 2048)       		 /* Input: network vp group ID */
    };
