 ...
 *            with default network SVP assigned 
 *
 * Usage    : BCM.0> cint TD4_HSDK_VXLAN_v4_NtoA_net_Default_SVP.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_VXLAN_v4_NtoA_net_Default_SVP_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
int
config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, int default_svp, bcm_flow_handle_t flow_handle)
{
    ...
    if(default_svp == 1) {
        printf("Setting up default SVP\n");
        flags |= BCM_FLOW_PORT_DEFAULT;
    }
    ...
    printf("Do not configure SIP=>SVP, Rely on Default SVP\n");
    /*
    *    We are assigning the default SVP and do not need to setup the SIP lookup to get SVP
