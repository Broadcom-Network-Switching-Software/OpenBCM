 ...
 *
 * Usage    : BCM.0> cint TD4_HSDK_ipv4_riot_AtoN_uc_HECMP_overlay.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_ipv4_riot_AtoN_uc_HECMP_overlay_logs.txt
 *
 *            ACCESS                             NETWORK
 *
 *                        +----------------+
...
bcm_error_t
vxlan_create_ecmp_obj(int unit, uint32 flags, int no_of_intfs, bcm_if_t *egr_obj_array, bcm_if_t *ecmp_obj_id,bcm_if_t egr_obj_un)
{
    ...
    for(i=0; i<no_of_intfs; i++)
    {
        ...
        {
            ecmp_group[i].egress_if_2 = egr_obj_un;/*new change in TD4*/
        }
                     ...
                     bcm_flow_handle_t flow_handle)
{
    ...
    printf("Network: create UNDERLAY UL L3 interface\n");
    ...
    printf("Network: create UNDERLAY L3 egress object\n");
    rv = vxlan_create_egr_obj(unit,BCM_L3_FLOW_ONLY | BCM_L3_FLAGS2_UNDERLAY,net_intf_ul,vxlan_net->net_ul_nh_mac,
    ...
    rv = vxlan_create_ecmp_obj(unit,BCM_L3_ECMP_OVERLAY, NO_OF_OVERLAY_ECMP_MEMBERS, net_egr_obj_ol,&net_ecmp_obj_id_ol,net_egr_obj_ul);
    if (BCM_FAILURE(rv)) {
