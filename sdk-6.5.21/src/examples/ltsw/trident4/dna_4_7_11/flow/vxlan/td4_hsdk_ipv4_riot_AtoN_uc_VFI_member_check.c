 ...
 *
 * Usage    : BCM.0> cint TD4_HSDK_ipv4_riot_AtoN_uc_VFI_member_check.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_ipv4_riot_AtoN_uc_VFI_member_check_logs.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *   4) Step4 - Verification (VFI member check):
 *   ===========================================
 *     a) Remove port members from VFI
 *
 *     b) Transmit the known unicast packet. The contents of the packet
 *        are printed on screen. 
 *
 *     c) Expected Result:
 *     ===================
 *       Traffic is dropped in ingress pipe due to VFI check failure.
 *
 *     d) Ports are added to VFI again to pass membership check
 *
 *     e) Expected Result:
 *     ===================
 *       We can see the tunnel packet on the egress port with the IPv4 encapsulation
