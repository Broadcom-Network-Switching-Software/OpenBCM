 ...
 *
 * Usage    : BCM.0> cint TD4_HSDK_ipv4_riot_AtoN_uc_DSCP_cos_update.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_ipv4_riot_AtoN_uc_DSCP_cos_update_logs.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see the tunnel packet on the egress port(queue 6) with the IPv4 encapsulation
...
Packet Dump
data[0000]: {00000000aaaa} {00000000bbbb} 8100 c00b
data[0010]: 0800 4500 0064 0000 0000 3f11 3f6a 0a0a 
data[0020]: 0a01 1414 1401 c000 12b5 0050 0000 0800 
...
XLMIB_T127.cd1                        :                     1                  +1
XLMIB_R127.cd1                        :                     1                  +1
UC_PERQ_PKT(6).cd1                    :                     1                  +1
UC_PERQ_BYTE(6).cd1                   :                   122                +122
Send to queue 6
 */
...
/* Create DSCP PROFILE to send packet to queue 6 */
bcm_error_t
vxlan_create_dscp_profile(int unit)
{
    uint32 flags;
    bcm_error_t rv=0;
	bcm_qos_map_t map;
	...
	if (BCM_FAILURE(rv)) {
        printf("\nError in ing_map: bcm_qos_map_create: %s\n", bcm_errmsg(rv));
        return rv;
    }
	print"ing_map";
    print ing_map;
	...
	if (BCM_FAILURE(rv)) {
        printf("\nError in ing_map: bcm_qos_map_add: %s\n", bcm_errmsg(rv));
        return rv;
    }
	...
	if (BCM_FAILURE(rv)) {
        printf("\nError in egr_map: bcm_qos_map_create: %s\n", bcm_errmsg(rv));
        return rv;
    }
	...
	if (BCM_FAILURE(rv)) {
        printf("\nError in egr_map: bcm_qos_map_add: %s\n", bcm_errmsg(rv));
        return rv;
    }
                        ...
                        bcm_if_t ol_intf_id,bcm_if_t *ing_obj_id)
{
    ...
    l3_ingress.flags = flag|BCM_L3_INGRESS_DSCP_TRUST;
    l3_ingress.vrf = vrf;
	l3_ingress.qos_map_id = ing_map;
                    ...
                    bcm_gport_t *acc_flow_port)
{
	...
	rv = vxlan_create_dscp_profile(unit);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vxlan_create_dscp_profile: %s\n", bcm_errmsg(rv));
        return rv;
    }
