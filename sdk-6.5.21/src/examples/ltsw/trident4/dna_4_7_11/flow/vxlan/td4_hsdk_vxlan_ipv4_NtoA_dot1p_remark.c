...
/*
 * Feature  : TD4(56880) Vxlan Bridging, IPv4 network to access tunnel
 *            termination flow for unicast traffic with dot1p remarking.
 *
 * Usage    : BCM.0> cint td4_hsdk_vxlan_ipv4_NtoA_dot1p_remark.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_vxlan_ipv4_NtoA_dot1p_remark.c.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * Summary:
 * ========
 * Cint example to demonstrate VxLAN  Bridging configuration
 * for tunnel termination with dot1p remarking :-
 *             - network to access(decap flow) with dot1p remarking 
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see the tunnel terminated packet on the access egress port,
...
Ingress Hex Packet:
tx port=1 data=
...
data[0000]: 0000 0000 bbbb 0000 0000 aaaa 8100 a015
...
/* added to disable CPU port based remarking when copy packets to cpu */ 
    if (BCM_E_NONE != bcm_port_control_set(0, 0, bcmPortControlEgressVlanPriSrc, bcmPortEgressVlanPriSrcIntPri))
    {
        printf("bcmPortControlEgressVlanPriUsesPktPri failed for CPU port \n");
        return -1;
    }
...
verify(int unit)
{
    ...
    printf("UC Packet comes into network port %d, expect to foward to access port with remarking based on payload vlan%d\n", ing_net_port_1, egr_acc_port_1);
	snprintf(str, 512, "tx 1 pbm=%d data=; sleep quiet 1", ing_net_port_1);
	bshell(unit, str);
	bshell(unit, "show c");
    
    printf("UC Packet comes into network port %d, expect to foward to access port with no remarking %d\n", ing_net_port_1, egr_acc_port_1);
	snprintf(str, 512, "tx 1 pbm=%d data=; sleep quiet 1", ing_net_port_1);
	bshell(unit, str);
	bshell(unit, "show c");
...
vxlan_access_port_config(int unit, int port_acc, bcm_gport_t gport_acc)
{
	...
	rv=create_qos_dot1p_map(unit,BCM_QOS_MAP_L2|BCM_QOS_MAP_EGRESS,&egr_map_id,5,0,5,bcmColorGreen);
	if (BCM_FAILURE(rv)) {
...
create_vxlan_vp(int unit, bcm_vpn_t vpn, uint32 flags, bcm_gport_t *vp)
{
	...
	if(flags & BCM_FLOW_PORT_NETWORK){
		...
		rv=create_qos_dot1p_map(unit ,BCM_QOS_MAP_L2 | BCM_QOS_MAP_INGRESS,&ing_map_id,0,0,5,bcmColorGreen);
		if (BCM_FAILURE(rv)) {
	...
	flow_port.flags = flags;
        flow_port.vlan_pri_map_id = ing_map_id;
	...
	/* Required for Qos remarking : Set for both access & nvp */
...
bcm_error_t
  create_qos_dot1p_map(int unit ,uint32 qos_flags,int *qos_map_id,uint8 qos_pktpri,uint8 qos_pktcfi,
                                                     int qos_intpri,bcm_color_t qos_color)
{
	/*Configure QOS MAP */
		bcm_qos_map_t map;
		int rv;
		...
		map.pkt_pri = qos_pktpri;
		map.pkt_cfi = qos_pktcfi;
		map.int_pri = qos_intpri;
		map.color = qos_color;
