...
/*
 * Feature  : TD4(56880) Vxlan Bridging , IPv4 network to access tunnel termination flow, add a new tag to the 
 *            payload while egressing on access port. Also, attaching flex counters for VPN and Flow port
 *
 * Usage    : BCM.0> cint TD4_HSDK_VxLAN_ipv4_NtoA_Flex_Ctr.c
 *
 * config   : bcm56880_a0-generic-32x400_vxlan.config.yml
 *
 * Log file : TD4_HSDK_VxLAN_ipv4_NtoA_Flex_Ctr_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
void verify(int unit)
{
    ...
    flexctr_flow_stat_get(unit, app_vxlan_net_port.flow_gport, app_vxlan_net_port.ing_flow_stat_id, "Ingress Flow");
    flexctr_flow_stat_get(unit, app_vxlan_acc_port.flow_gport, app_vxlan_acc_port.egr_flow_stat_id, "Egress Flow");    
