...
/*
 * Feature  : Vxlan Bridging, IPv4 access to access and network tunnel initiation flow
 *            and assigning network group using IFP.
 *
 * Usage    : BCM.0> cint td4_hsdk_vxlan_ipv4_assign_network_group_using_IFP.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_vxlan_ipv4_assign_network_group_using_IFP_log.txt
 *
 * Test Topology :
 *                                  +----------------------+
 ...
 *                           VLAN-24|                      |  egress_port[1]    +---+
 *         (acc)  +-----------------+                      +--------------------+   |
 *                   egress_port[2] |                      |01:00:5e:02:02:02   +---+
 *                                  |                      |                    00::02
 *                                  |                      |
 *                                  +----------------------+
 *
 ...
 * Summary:
 * ========
 * Cint example to demonstrate assigning network group using IFP.
 *
 ...
 *        four ports, one port is used as ingress port and the others are used as
 *        egress ports.
 *
 ...
 *   =========================================================
 *     a) Configure a VxLAN Bridging configuration with two access ports and two network ports.
 *        While configuring network ports, assign network group id for the network ports.
 *
 *     b) Add all the four ports to broadcast group for the VPN.
 *
 *   3) Step3 - Verification (Done in verify()):
 *   ===========================================
 *     a) Send unknown unicast packet on ingress port.
 *
 *     b) Install an IFP rule to assign source network group id which is equal to the one
 *        configured on network ports.
 *
 *     c) Send unknown unicast packet again on ingress port.
 *
 *     d) Expected Result:
 *     ===================
 *       After 3.a, We can see the tunnel packets on both the network ports and non-tunnel packet on
 *       access port.
 *
 *       After 3.c, We can see that non-tunnel packet on access port and the packets are dropped
 *       on network ports with drop reason(SPLIT_HORIZON_DROP)
 */
...
/**** global constants ****/
const int   IN_PORT_NUM = 1;
const int   OUT_PORT_NUM = 3;

bcm_port_t ingress_port;
bcm_port_t egress_port[OUT_PORT_NUM];

int network_group_id = 1;
...
vxlan_network_port_create(int unit, bcm_vpn_t vpn, vxlan_phy_port_s *vxlan_network_port)
{
    ...
    flow_port.network_group_id = v_port->network_group_id;
    ...
    peinfo.network_group    = v_port->network_group_id;
...
vxlan_tunnel_create(int unit, vxlan_port_s * v_port)
{
    ...
    tiinfo.valid_elements   = BCM_FLOW_TUNNEL_INIT_TTL_VALID |  BCM_FLOW_TUNNEL_INIT_UDP_DPORT_VALID;
...
vxlan_phy_port_s    customer_port_1, customer_port_2, provider_port1, provider_port2;

/* Access Port 1 Coinfigurations */
bcm_vlan_t      acc_vid_1 = 21;
bcm_vlan_t      acc_vid_2 = 24;
...
config_vxlan(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_stk_modid_get(unit, &my_modid));
    BCM_IF_ERROR_RETURN(vxlan_port_create(unit, my_modid, ingress_port,    FLAGS_CUSTOMER, &customer_port_1));
    ...
    BCM_IF_ERROR_RETURN(vxlan_port_create(unit, my_modid, egress_port[2],  FLAGS_CUSTOMER, &customer_port_2));
    ...
    customer_port_1.vxlan_port[0].vid = acc_vid_1;
    BCM_IF_ERROR_RETURN(vxlan_access_port_create(unit, vxlan_vpn, &customer_port_1));

    customer_port_2.vxlan_port[0].vid = acc_vid_2;
    BCM_IF_ERROR_RETURN(vxlan_access_port_create(unit, vxlan_vpn, &customer_port_2));
    ...
    provider_port1.vxlan_port[0].network_group_id = network_group_id;
    ...
    provider_port2.vxlan_port[0].network_group_id = network_group_id;
...
verify(int unit)
{
    ...
    BCM_FIELD_ASET_ADD(group_config.aset, bcmFieldActionSrcNetworkGroup);
    BCM_IF_ERROR_RETURN(bcm_field_group_config_create(unit, &group_config));
    BCM_IF_ERROR_RETURN(bcm_field_entry_create(unit, group_config.group,
                                             &entry));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_InPort(unit, entry, ingress_port,
                                              BCM_FIELD_EXACT_MATCH_MASK));
    rv = bcm_field_action_add(unit, entry, bcmFieldActionSrcNetworkGroup,
                                       network_group_id, network_group_id);
    if (BCM_FAILURE(rv)) {
      printf("\nError(%s) configuring bcmFieldActionSrcNetworkGroup \n", bcm_errmsg(rv));
      return rv;
    }
    ...
    bshell(unit, str);
    sal_sleep(2);
    bshell(unit, "show c; sleep 2");

    bshell(unit, "bsh -c 'lt traverse -l CTR_EGR_DROP_EVENT'");
...
execute(void)
{
    ...
    print "dump sw config; attach; version";
    bshell(unit, "dump sw config; a ; version");
