...
/* 
 *  Feature  : ExactMatch stage qualifications in IFP Stage.
 *
 *  Usage : BCM.0> cint fp_exactmatch.c 
 *           E.g.  cint fp_exactmatch.c 
 *
 *  config   : bcm56880_a0-generic-128x100.config.yml
 *
 *  Log file : fp_exactmatch_log.txt
 *
 * YOUR YAML FILE MUST HAVE BELOW CONFIGS FOR EXACTMATCH TO WORK:
 * Description:-
 * l3_alpm_template = 3 {Combined + FP Comp}
 * uft_mode         = 7 {2 Levels DIP - 2 Levels DIP & SIP, Large FP_EM Capacity}
 *
 * CONFIG YAML FILE CONTENT SNIPPET:-
########################################
...
---
bcm_device:
 ...
 *  Summary:
 *  ========
 *      This CINT illustrates how to use ExactMatch in IFP stage in TD4.
 *      There is a setting that needs to be done in the YAML Config file before launching
 *      the SDK. These settings are given in the comment section above.
 *
    ...
    TestFunc(int prio)
{
    ...
    if( opt_IsVfp || opt_IsIfp || opt_IsEM )
    {
