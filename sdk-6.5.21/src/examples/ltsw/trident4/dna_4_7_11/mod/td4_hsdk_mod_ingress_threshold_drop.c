...
/*
 * Feature  : Mirror-On-Drop, packets dropped in MMU due to ingress admission control are 
 * able to be mirrored to an MTP and encapsulated with an IPv4 PSAMP header.
 *
 * Usage    : BCM.0> cint TD4_HSDK_MOD_ingress_threshold_drop.c
 *
 * config   : bcm56880_a0-generic-32x400_lossless.config.yml
 *
 * Log file : TD4_HSDK_MOD_ingress_threshold_drop_log.txt
 *
 * Test Topology :
 *
 *                                +----------------------+      
 ...
 *
 *     c) Disable Flow Control to drop packets by ingress threholds.
 *
 ...
 *
 *     (d) Start PacketWatcher and log some packets. Dump one packet to see whether the IPv4
 *         PSMAP header is encasuplated or not.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see the tunnel packet on the egress port with the IPv4 PSAMP encapsulation.
...
data[0010]: 9100 00c8 0800 4500 00a6 0000 0000 1011
data[0020]: 8c2a 0606 0606 0909 0909 1234 4321 0092
data[0030]: 0000 000a 008a 0000 0000 0000 0047 0000
data[0040]: 0064 1234 007a 0000 0000 0000 0000 0000
data[0050]: 0000 0014 0001 0003 0000 00ff 005e 0000
data[0060]: 0000 aaaa 0000 0000 bbbb 0800 4500 0050
data[0070]: 0001 0000 4011 7497 0101 0101 0202 0202
data[0080]: 0035 0035 003c 32f6 7465 7374 7465 7374
data[0090]: 7465 7374 7465 7374 7465 7374 7465 7374
data[00a0]: 7465 7374 7465 7374 7465 7374 7465 7374
data[00b0]: 7465 7374 7465 7374 7465 7374
...
config_mod(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(mod_mirror_destination_create(unit, FLAGS_MOD_ENCAP_IPV4, mtp_gport, &mirror_dest_id));

    BCM_IF_ERROR_RETURN(mod_mirror_destination_add(unit, mirror_dest_id));

    /* Disable PAUSE */
    BCM_IF_ERROR_RETURN(bcm_cosq_port_priority_group_property_set(unit, ingress_port[0], 0, bcmCosqPauseEnable, 0));
...
verify(int unit)
{
    ...
    printf("\nLT Table Dump (pkt drops on ingress port):\n");
    snprintf(str, 512, "bsh -c 'lt CTR_ING_TM_PORT_UC_DROP lookup port_id=%d'", ingress_port[0]);    
    bshell(unit, str);
