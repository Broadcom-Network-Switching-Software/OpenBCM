...
/*
 * Feature  : TD4(56880) VPLS (L2 MPLS) tunnel termination flow with LABEL and
 *            Port Group as match criteria(BCM_MPLS_PORT_MATCH_LABEL_PORT_GROUP)
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpls_NtoA_MatchLabelPortGroup.c
 *
 * config   : bcm56880_a0-generic-32x400.mpls-config.yml
 *
 * Log file : td4_hsdk_mpls_vpls_NtoA_MatchLabelPortGroup_log.txt
 *
 * Test Topology :
 *
 *                    +-------------------+
 ...
 * Summary:
 * ========
 *   Cint script to demonstrate VPLS (L2 MPLS) tunnel termination flow with
 *   LABEL + Port_Group as match criteria.
 *
 ...
 *
 *   2) Step2 - Configuration (Done in setup_vpls()):
 *   ================================================
 ...
 *      c) Expected Result:
 *      ===================
 *         We can see that VPLS tunnel packet out on network 2
 *         and customer (payload) packet out on access port 1 & 2.
 *         'show c' & bcm_stat_get used to check the Tx/Rx packet
 *         stats/counters.
 *
 *   Ingress Packet : ing_net_port_3
 *   0000 0000 00bb 0000 0000 00aa 8100 0016
 *   8847 0045 6010 0033 3110 0000 00ff 0000
 ...
 *   8847 0055 5016 0088 8116 0000 0000 0011
...
verify(int unit)
{
    ...
    printf("\nSending MPLS packet to ingress port:%d\n", ing_net_port_3);
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000bb0000000000aa8100001688470045601000333110000000ff0000000000110000000000228100000b0800450000360000000040ff65ac0a0000140a00000a000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f2021239412f0; sleep quiet 3", ing_net_port_3);
    bshell(unit, str);
                     ...
                     l2vpn_acc_cfg_t *l2vpn_acc_cfg)
{
    ...
    rv = bcm_vlan_port_default_action_get(unit, l2vpn_acc_cfg->acc_gport,
                                          &vlan_action);
    if (BCM_FAILURE(rv)) {
    ...
    vlan_action.outer_tag = bcmVlanActionNone;  /* Preserve outer tag */
    vlan_action.inner_tag = bcmVlanActionNone;  /* Preserve inner tag */
    vlan_action.forwarding_domain = l2vpn_cfg->mpls_vpn;
    rv = bcm_vlan_port_default_action_set(unit, l2vpn_acc_cfg->acc_gport,
                                          vlan_action);
    if (BCM_FAILURE(rv)) {
    ...
    rv = bcm_port_control_set(unit, l2vpn_acc_cfg->acc_gport,
    ...
    /* BCM_MPLS_PORT_MATCH_PORT is not supported for VPLS.
     * It's not necessary to create the VPLS PORT-based access port.
     * Use the physical port for the VPLS PORT-based access port.
     */
                      ...
                      l2vpn_net_cfg_t *l2vpn_net_cfg)
{
    ...
    net_mpls_port.port_group = l2vpn_net_cfg->port_class;
...
setup_vpls(int unit)
{
    ...
    l2vpn_net_cfg_t l2vpn_net_1 = {
        ...
        BCM_MPLS_PORT_MATCH_LABEL_PORT_GROUP, /* Network match criteria */
        0x444,                     /* VC init label */
        0x333,                     /* VC term label */
        0x111,                     /* Tunnel init label */
        0x456,                     /* Tunnel term label */
        0x0,                       /* Tunnel ID */
    ...
    /* Enable port independent mpls label range */
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit,
                        bcmSwitchMplsPortIndependentLowerRange1, 0x400));
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit,
                        bcmSwitchMplsPortIndependentLowerRange2, 0x400));

    /* Set port classification ID */
    rv = bcm_port_class_set(unit, l2vpn_net_1.net_port,bcmPortClassIngress,
                            l2vpn_net_1.port_class);
    if (BCM_FAILURE(rv)) {
