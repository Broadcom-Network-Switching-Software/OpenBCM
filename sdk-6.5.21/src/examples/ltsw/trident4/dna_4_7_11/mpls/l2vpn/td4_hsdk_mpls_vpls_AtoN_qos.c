...
/*
 * Feature  : TD4(56880) VPLS (L2 MPLS) QOS mapping in tunnel initiation flow
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpls_AtoN_qos.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_mpls_vpls_AtoN_qos_log.txt
 *
 * Test Topology :
 *
 *                    +-------------------+
 ...
 *                    |                   |  egr_net_port_2
 *                    |                   +---------------->
 *      VLAN-11       |                   |  Egress with MPLS tunnel header
 *   ing_acc_port_1   |                   |
 *   ---------------->+     Trident-4     |
 *   Ingress packet   |                   |     VLAN-23
 *                    |                   |  egr_net_port_3
 *                    |                   +---------------->
 ...
 *                Network port 1 - (egr_net_port_2)
 *                Network port 2 - (egr_net_port_3)
 *
 * Summary:
 * ========
 *   Cint script to demonstrate VPLS (L2 MPLS) tunnel initiation with
 *   QOS using BCM APIs.
 *
 ...
 *         packet on network port 1 & network port 2 with EXP value.
 *         'show c' & bcm_stat_get used to check the Tx/Rx packet
 *         stats/counters.
 *
 *   Ingress Packets : ing_acc_port_1
 *   0000 0000 0022 0000 0000 0011 8100 800b
 ...
 *
 *   Egress Packet : egr_net_port_2
 *   0000 0000 00aa 0000 0000 00bb 8100 0016
 *   8847 0011 1816 0044 4116 0000 0000 0022
 ...
 *   0000 0000 00cc 0000 0000 00dd 8100 8017
 *   8847 0055 5816 0088 8116 0000 0000 0022
 *   0000 0000 0011 8100 800b 0800 4500 0036
 *   0000 0000 40ff 65ac 0a00 0014 0a00 000a
 *   0001 0203 0405 0607 0809 0a0b 0c0d 0e0f
 *   1011 1213 1415 1617 1819 1a1b 1c1d 1e1f
 *   2021 2394 12f0
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000220000000000118100800b0800450000360000000040FF65AC0A0000140A00000A000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F2021239412F0; sleep quiet 3", ing_acc_port_1);
    bshell(unit, str);
...
config_l2vpn_qos (int unit, int *ing_qos, int *egr_qos)
{
    ...
    *egr_qos = egr_qos_id;
    ...
    for (i = 0; i < 8; i++) {
        ...
        egr_qos_map.int_pri = int_pri[i];    /* internal priority */
        egr_qos_map.color = bcmColorGreen;
        egr_qos_map.tunnel_exp[0] = pkt_exp[i];
        egr_qos_map.tunnel_exp[1] = pkt_exp[i];
        egr_qos_map.tunnel_exp[2] = pkt_exp[i];
        egr_qos_map.tunnel_exp[3] = pkt_exp[i];
        egr_qos_map.tunnel_exp[4] = pkt_exp[i];
        egr_qos_map.tunnel_exp[5] = pkt_exp[i];
        egr_qos_map.tunnel_exp[6] = pkt_exp[i];
...
config_l2vpn_access (int unit, l2vpn_cfg_t *l2vpn_cfg,
                     l2vpn_acc_cfg_t *l2vpn_acc_cfg, int ing_qos_id)
{
    ...
    rv = bcm_qos_port_map_set(unit, l2vpn_acc_cfg->acc_gport, ing_qos_id, -1);
    if (BCM_FAILURE(rv)) {
...
config_l2vpn_network (int unit, l2vpn_cfg_t *l2vpn_cfg,
                      l2vpn_net_cfg_t *l2vpn_net_cfg, int egr_qos_id)
{
    bcm_vlan_action_set_t  vlan_action;
    ...
    rv = mpls_tunnel_initiator(unit, l2vpn_net_cfg->tunnel_label_init,
                               ...
                               &l2vpn_net_cfg->tunnel_id, egr_qos_id);
    if (BCM_FAILURE(rv)) {
...
setup_vpls(int unit)
{
	...
	rv = config_l2vpn_access(unit, &l2vpn_cfg, &l2vpn_acc_1, ing_qos_id);
	if (BCM_FAILURE(rv)) {
	...
	rv = config_l2vpn_network(unit, &l2vpn_cfg, &l2vpn_net_1, egr_qos_id);
	if (BCM_FAILURE(rv)) {
	...
	rv = config_l2vpn_network(unit, &l2vpn_cfg, &l2vpn_net_2, egr_qos_id);
	if (BCM_FAILURE(rv)) {
		printf("\nError in network 1 config: %s.\n", bcm_errmsg(rv));
		return rv;
	}
