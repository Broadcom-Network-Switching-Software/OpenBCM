...
/*
 * Feature  : VPWS (L2 MPLS) tunnel termination flow with LABEL+PortGroup
 *            as match criteria(BCM_MPLS_PORT_MATCH_LABEL_PORT_GROUP)
 *
 * Usage    : BCM.0> cinttd4_hsdk_mpls_vpws_NtoA_MatchLabelPortGroup.c
 *
 * config   : bcm56880_a0-generic-32x400.mpls-config.yml
 *
 * Log file : td4_hsdk_mpls_vpws_NtoA_MatchLabelPortGroup_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *           <----------+       TD4         +<----------
 *      Egress packet   |                   |  Ingress mpls packet
 *                      |                   |
 *                      |                   |
 *                      |                   |
 *                      +-------------------+
 ...
 * Summary:
 * ========
 * Cint script to demonstrate VPWS (L2 MPLS) tunnel termination flow with
 * label and port group as match criteria.
 *
 ...
 *        'l3 egress show'.
 *
 *     b) Transmit the mpls packet on provider_port. The contents of the packet
 *        are printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *     We can see that VPWS tunnel decap packet out on egress access port.
 *
 *   Ingress Packet:
 *   0000 0000 00bb 0000 0000 00aa 8100 0016
 *   8847 0055 5010 0033 3110 0000 00ff 0000
...
test_setup(int unit)
{
    ...
    ingress_port = provider_port;
    egress_port = customer_port;
...
verify(int unit)
{
    ...
    bshell(unit, "clear c");
    /* In TD4, L2 mpls decap support expect control word to be present by default */
    printf("\nSending mpls packet to ingress port:%d\n", ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000bb0000000000aa8100001688470055501000333110000000ff0000000000110000000000228100000b0800450000360000000040ff65ac0a0000140a00000a000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f2021239412f0; sleep quiet 3", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
...
bcm_error_t
mpls_customer_port_create(int unit, bcm_vpn_t vpn, uint32 vid,uint32 port_group)
{
...
mpls_provider_port_create(int unit, bcm_vpn_t vpn, uint32 vc_label_init,
                          ...
                          bcm_if_t tunnel_id, uint32 port_group)
{
    ...
    net_mpls_port.port_group = port_group;
    net_mpls_port.criteria = BCM_MPLS_PORT_MATCH_LABEL_PORT_GROUP;
...
setup_vpws(int unit)
{
    ...
    uint32 tunnel_label_term = 0x555;
    uint32 vc_label_term     = 0x333;
    uint32 vc_label_init     = 0x112;
    uint8 ttl = 16;
    bcm_mpls_vpn_config_t vpn_info;
    ...
    bcm_vpn_t   vpn;
    bcm_gport_t tunnel_id;
    uint32 port_class = 10, port_class1 = 20;
    bcm_error_t rv = BCM_E_NONE;
    ...
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(0,
    ...
    rv = bcm_port_class_set(unit, customer_port,bcmPortClassIngress,port_class);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_port_class_set: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    /* Create mpls access port and bind it to vpn */
    rv = mpls_customer_port_create(unit, vpn, customer_vid, port_class);
    if (BCM_FAILURE(rv)) {
    ...
    /* Configure ingress port group */
     rv=bcm_port_class_set(unit,provider_port,bcmPortClassIngress,port_class1);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_port_class_set: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = mpls_provider_port_create(unit, vpn, vc_label_init, vc_label_term,
                                   ttl, egr_obj, tunnel_id, port_class1);
    if (BCM_FAILURE(rv)) {
