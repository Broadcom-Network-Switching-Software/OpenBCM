...
/*
 * Feature  : TD4(56880) VPLS (L2 MPLS) tunnel initiation flow, removing
 *            SDi_TAG in the payload.
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpls_AtoN_SDTag_Delete.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_mpls_vpls_AtoN_SDTag_Delete_log.txt
 *
 * Test Topology :
 *
 *                    +-------------------+
 ...
 * Summary:
 * ========
 *   Cint script to demonstrate VPLS (L2 MPLS) tunnel initiation flow,
 ...
 *         packet on network port without payload VLAN.
                      ...
                      l2vpn_net_cfg_t *l2vpn_net_cfg)
{
    ...
    /* Disable payload tag delete action */
    rv = bcm_vlan_control_port_set(unit, l2vpn_net_cfg->net_vport,
                                   bcmVlanPortPayloadTagsDelete, 1);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_control_port_set: %s.\n", bcm_errmsg(rv));
        return rv;
    }
