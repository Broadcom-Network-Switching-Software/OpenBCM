...
/*
 * Feature  : L3 MPLS ECN mapping in tunnel initiation
 *
 * Usage    : BCM.0> cint td4_hsdk_l3mpls_init_ecn.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_l3mpls_init_ecn_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *   QOS/ECN using BCM APIs. in TD4 ECN remarking is moved to the QoS module.
 *
 ...
 *   =========================================================
 *     a) Configure a basic L3 MPLS tunnel initiation with QOS/ECN functional
 ...
 *       MPLS tunnel Label. Also run the 'show c' to check the TX/RX packet
 *       stats/counters.
 */
...
bcm_error_t
config_l3_mpls_access (int unit, l3vpn_acc_cfg_t *l3vpn_acc_cfg, int ing_qos_id)
{
    ...
    rv = bcm_qos_port_map_set(unit, l3vpn_acc_cfg->acc_gport, ing_qos_id, -1);
    if (BCM_FAILURE(rv)) {
...
config_l3_mpls_network (int unit, l3vpn_acc_cfg_t *l3vpn_acc_cfg,
                        l3vpn_net_cfg_t *l3vpn_net_cfg, int egr_qos_id)
{
    ...
    tunnel_encap.qos_map_id = egr_qos_id;
...
/* Qos configuration */
bcm_error_t
config_l3_mpls_qos (int unit, int *ing_qos, int *egr_qos)
{
    ...
    bcm_error_t                     rv = BCM_E_NONE;
    uint32                          flags;
    /* Set the 802.1P to internal priority mapping profile */
    ...
    /* Config ECNTOINTCN table */
    bcm_ecn_traffic_map_info_t_init(&ecn_map);
    ecn_map.flags = BCM_ECN_TRAFFIC_MAP_RESPONSIVE;
    ecn_map.ecn = 1;
    ecn_map.int_cn = 1;
    ecn_map.tunnel_ecn = 1;
    ecn_map.tunnel_type = bcmEcnTrafficMapNonTunnel;
    print bcm_ecn_traffic_map_set(unit, &ecn_map);
    ...
    /* Config INTCN UPDATE table */
    bcm_ecn_traffic_action_config_t_init(&ecn_action);
    ecn_action.action_type = BCM_ECN_TRAFFIC_ACTION_TYPE_DEQUEUE;
    ecn_action.action_flags =
                    BCM_ECN_TRAFFIC_ACTION_DEQUEUE_NON_CONGESTION_INT_CN_UPDATE;
    ecn_action.color = bcmColorGreen;
    ecn_action.int_cn = 1;
    ecn_action.non_congested_int_cn = 2;
    ecn_action.responsive = 1;
    print bcm_ecn_traffic_action_config_set(unit, &ecn_action);
    ...
    ecn_action.color = bcmColorGreen;
    ecn_action.int_cn = 0;
    ecn_action.non_congested_int_cn = 1;
    ecn_action.responsive = 0;
    print bcm_ecn_traffic_action_config_set(unit, &ecn_action);

    /* Create ingress QOS mapping profile */
    rv = bcm_qos_map_create(unit, BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_L2,
                            &ing_qos_id);
    if (BCM_FAILURE(rv)) {
    ...
    *ing_qos = ing_qos_id;
    ...
    for (i = 0; i < 8; i++) {
        ...
        ing_qos_map.pkt_pri = pkt_pri[i];
        ing_qos_map.pkt_cfi = 0;
        ing_qos_map.int_pri = in_pri[i];
        ing_qos_map.color = bcmColorGreen;
        rv = bcm_qos_map_add(unit, BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_L2,
    ...
    *egr_qos = egr_qos_id;
    ...
    /* Config INTCNTOEXP table */
    for (i = 0; i < 8; i++) {
        bcm_qos_map_t_init(&egr_qos_map);
        egr_qos_map.int_pri = int_pri[i];
        egr_qos_map.int_cn = 2;
        egr_qos_map.color = bcmColorGreen;
        egr_qos_map.responsive = 1;
        egr_qos_map.tunnel_exp[0] = 0;
        egr_qos_map.tunnel_exp[1] = 0;
        egr_qos_map.tunnel_exp[2] = 0;
        egr_qos_map.tunnel_exp[3] = 0;
        egr_qos_map.tunnel_exp[4] = 0;
        egr_qos_map.tunnel_exp[5] = 0;
        egr_qos_map.tunnel_exp[6] = 1;
        rv = bcm_qos_map_add(unit, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_MPLS |
                             BCM_QOS_MAP_ENCAP, &egr_qos_map, egr_qos_id);
        if (BCM_FAILURE(rv)) {
    ...
    for (i = 0; i < 8; i++) {
        /* For non-responsive flow, the int_cn is not used for remarking */
        bcm_qos_map_t_init(&egr_qos_map);
        egr_qos_map.int_pri = int_pri[i];
        // egr_qos_map.int_cn = 1;
        egr_qos_map.color = bcmColorGreen;
        egr_qos_map.responsive = 0;
        egr_qos_map.tunnel_exp[0] = 0;
        egr_qos_map.tunnel_exp[1] = 0;
        egr_qos_map.tunnel_exp[2] = 0;
        egr_qos_map.tunnel_exp[3] = 0;
...
config_l3_mpls(int unit)
{
    ...
    rv = config_l3_mpls_qos (unit, &ing_qos_id, &egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("config_l3_mpls_qos return error : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = config_l3_mpls_access (unit, &l3vpn_acc, ing_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("config_l3_mpls_access return error : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = config_l3_mpls_network (unit, &l3vpn_acc, &l3vpn_net, egr_qos_id);
    if (BCM_FAILURE(rv)) {
