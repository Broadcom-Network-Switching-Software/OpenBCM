...
/*  Feature  : FlexDigest for Higig3 LBID based hashing for UC and Non-UC trunk
 *
 *  config   : td4_hsdk_flexdigest.config.yml
 *             (Usage: rack#./bcm.user -y ./td4_hsdk_flexdigest.config.yml)
 *
 *  Usage    : BCM.0> cint td4_hsdk_flexdigest_lbid_hg3.c
 *
 *  Log file : td4_hsdk_flexdigest_lbid_hg3_log.txt
 *
 *  Test Topology :
 *      +-----------------+            +---------+
 *      |   (hg3)-ing-port|<-----------+         |
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of the FlexDigest for Higig3 UC
 *   and Non-UC trunk application using BCM APIs.
 *
 ...
 *        egress_ports.
 *
 *   2) Step2 - Configuration (Done in config_flexdigest_lbid_hg3()):
 *   =========================================================
 *     a) Configure a basic port, vlan, flexdigest for LBID based UC and Non-UC
 *        trunk functional scenario and does the necessary configurations.
 *
 *   3) Step3 - Verification (Done in verify()):
 *   ===========================================
 *     a) Check the configurations by 'vlan show', 'l2 show', and 'trunk show'.
 *
 *     b) Transmit the few/many UC & Non-UC packets from ingress-port(hg3).
 *        The contents of the packet are printed on screen.
 *
 *     c) Expected Result:
 *     ===================
 *       We can see that the packets/traffic is splits in a way that the
 *       traffic load is balanced across multiple egress-ports ( i.e trunk ports)
...
bcm_error_t
config_flexdigest_lbid_hg3(int unit)
{
...
verify(int unit)
{
    ...
    printf("Verify %d UC pkts from port %d forwarding to ing-port(hg)\n", pkt_cnt, ingress_port);
    bshell(unit, "sleep 2");
    for (i = 0; i < pkt_cnt; i++) {
    ...
    printf("Verify %d Non-UC pkts from port %d forwarding to ing-port(hg)\n", pkt_cnt, ingress_port);
    bshell(unit, "sleep 2");
    for (i = 0; i < pkt_cnt; i++) {
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_flexdigest_lbid_hg3())
...
execute(void)
{
    ...
    print "~~~ #2) config_flexdigest_lbid_hg3(): ** start **";
    if (BCM_FAILURE((rv = config_flexdigest_lbid_hg3(unit)))) {
        printf("config_flexdigest_lbid_hg3() failed.\n");
        return -1;
    }
    print "~~~ #2) config_flexdigest_lbid_hg3(): ** end **";
