...
/*
 * Feature  : Ingress Mirroring (sFLOW)
 *                                           
 * Usage    : BCM.0> cint td4_hsdk_mirror_sflow.c
 *                                               
 * config   : bcm56880_a0-generic-32x400_config.yml
 *                                                 
 * Log file : td4_hsdk_mirror_sflow_log.txt        
 *                                                 
 * Test Topology :                                 
 #                +---------------------------+    
 #                |                           |    
 ...
 * Summary:
 * ========
 * Cint example to demonstrate Ingress sampling using SFLOW.
 *
 ...
 *
 *   2) Step2 - Configuration (Done in config_sflow_mirror()):
 *   =========================================================
 *     a) enable ingress mirroring on ingress_port and make egress_port_2 as MTP port.
 *     b) Set MTP.flags as BCM_MIRROR_DEST_TUNNEL_SFLOW | BCM_MIRROR_DEST_TUNNEL_WITH_SEQ
 *     c) Create a sample profile and attach it to ingress_port
 * 
 ...
 *       [bcmPWN.0]Packet[2]: data[0010]: 0800 4500 0070 0000 0000 0511 8b54 0a0a
 *       [bcmPWN.0]Packet[2]: data[0020]: 0a0a 0b0b 0b0b 5566 7788 005c 0000 0000
 *       [bcmPWN.0]Packet[2]: data[0030]: 0001 0002 003d 8080 1234 ffee ffee 00bb
 *       [bcmPWN.0]Packet[2]: data[0040]: bbbb bbbb 0011 2233 4455 8100 0002 0800
 *       [bcmPWN.0]Packet[2]: data[0050]: 4500 001c 0000 0000 4002 f95d c0a8 001e
 *       [bcmPWN.0]Packet[2]: data[0060]: c0a8 0014 1164 ee9b 0000 0000 0001 0203
 *       [bcmPWN.0]Packet[2]: data[0070]: 0405 0607 0809 0a0b 0c0d 0e0f 1011 e544
 *       [bcmPWN.0]Packet[2]: data[0080]: d79b
*/
...
config_sflow_mirror(int unit)
{
    ...
    mtp.flags = BCM_MIRROR_DEST_TUNNEL_SFLOW | BCM_MIRROR_DEST_TUNNEL_WITH_SEQ;  
    ...
    profile.enable = TRUE;
    profile.rate = 0x1; /* Indicate MAX_THRESHOLD */
 ...
 *  This function does the following
 *  a) test setup
 *  b) actual configuration (Done in config_sflow_mirror())
...
execute(void)
{
    ...
    print "~~~ #2) config_sflow_mirror(): ** start **";
    if (BCM_FAILURE((rv = config_sflow_mirror(unit)))) {
        printf("config_sflow_mirror() failed.\n");
        return -1;
    }
    print "~~~ #2) config_sflow_mirror(): ** end **";
