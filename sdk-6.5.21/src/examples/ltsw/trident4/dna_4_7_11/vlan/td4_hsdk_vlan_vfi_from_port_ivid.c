 ...
 *            using vlan translate table(ING_VLAN_XLATE_2_TABLE)
 *
 * Usage    : BCM.0> cint td4_hsdk_vlan_vfi_from_port_ivid.c
 *
 * config   : bcm56880_a0-generic-vlan.config.yml
 *
 * Log file : td4_hsdk_vlan_vfi_from_port_ivid_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * packets on switch using VFI configuration from vlan_xlate_2 entry.
 *
 ...
 *     b) Assign VFI from port+outer vlan key lookup vlan_xlate table
 ...
 *
 *   Ingress Packet:
 *   0000 0000 0011 0000 0000 0002 8100 000a
 ...
 *   8100 000a 0806 0001 0800 0604 0001 0000
 *   0000 0002 0a0a 0a02 0000 0000 0000 1414
 *   1402 0001 0203 0405 0607 0809 0a0b 0c0d
 *   bc94 977f
 */
...
vlan_xlate_entry_setup(int unit,bcm_gport_t port,bcm_vlan_t outer_vlan, bcm_vlan_t fwd_vfi)
{
    ...
    action.outer_tag = bcmVlanActionDelete; /* don't preserve outer tag */
    action.inner_tag = bcmVlanActionNone;  /* preserve inner tag */
    action.forwarding_domain = fwd_vfi;
    rv = bcm_vlan_translate_action_add(unit,port_class_2,
                            bcmVlanTranslateKeyPortGroupInner, BCM_VLAN_INVALID, outer_vlan, &action);
    if (BCM_FAILURE(rv)) {
...
setup_vlan_switching(int unit)
{
    ...
    rv = bcm_port_tpid_set(unit, ingress_port, 0x9100);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_port_tpid_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
