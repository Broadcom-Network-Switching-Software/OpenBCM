...
/*
 * Feature  : Default port based vlan action set for ingress packets
 *
 * Usage    : BCM.0> cint td4_hsdk_vlan_ing_port_actions.c
 *
 * config   : bcm56880_a0-generic-vlan.config.yml
 *
 * Log file : td4_hsdk_vlan_ing_port_actions_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate default port based vlan action set for ingress packets
 *
 ...
 *     d) default VFI and default port vlan actions is configured
 *
 ...
 *
 *   Egress Packet:
 *   000000000011 000000000002 8100 2014
...
verify(int unit)
{
    ...
    bshell(unit, "l2 show");
    bshell(unit, "clear c");
    printf("\nSending an untag packet to ingress port:%d\n",ingress_port);
...
setup_vlan_switching(int unit)
{
    ...
    default_action.new_outer_cfi = 0;
    default_action.forwarding_domain = default_vlan;
    /*do not preserve outer tag*/
    default_action.outer_tag = bcmVlanActionDelete;
    /*preserve outer tag*/
    default_action.inner_tag = bcmVlanActionNone;
    ...
    /*configure default egr_vfi actions*/
    bcm_vlan_control_vlan_t vlan_control;
    bcm_vlan_control_vlan_t_init(&vlan_control);
    vlan_control.forwarding_vlan = default_vlan;
    vlan_control.egress_action.outer_tag = bcmVlanActionNone;
    vlan_control.egress_action.inner_tag = bcmVlanActionNone;

    rv = bcm_vlan_control_vlan_selective_set(unit,vlanId,
                    BCM_VLAN_CONTROL_VLAN_EGRESS_ACTION_MASK |
                    BCM_VLAN_CONTROL_VLAN_FORWARDING_VLAN_MASK,vlan_control);
    if (BCM_FAILURE(rv)) {
