...
/*
 * Feature  : Egress OTAG and ITAG actions based on egress vlan translation
 *
 * Usage    : BCM.0> cint td4_hsdk_vlan_egr_xlate_tag_actions.c
 *
 * config   : bcm56880_a0-generic-vlan.config.yml
 *
 * Log file : td4_hsdk_vlan_egr_xlate_tag_actions_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * egress vlan translation for untag/ single tag/ double tagged packets
 *
...
bcm_error_t
egr_vlan_xlate_setup(int unit,bcm_gport_t port,bcm_vlan_t outer_vlan,
                            bcm_vlan_t inner_vlan)
{
    ...
    rv = bcm_vlan_control_port_set(unit,port,bcmVlanTranslateEgressEnable,1);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_control_port_set: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_port_class_set(unit,port,bcmPortClassEgress,port_class_2);
    if (BCM_FAILURE(rv)) {
    ...
    action.new_outer_vlan = outer_vlan;
    action.new_outer_cfi = 0;
    action.priority = 1;
    action.new_inner_vlan = inner_vlan;
    action.new_inner_cfi = 0;
    action.new_inner_pkt_prio = 1;
    rv = bcm_vlan_translate_egress_action_add(unit,port_class_2,
                                              outer_vlan,BCM_VLAN_INVALID,&action);
    if (BCM_FAILURE(rv)) {
...
setup_vlan_switching(int unit)
{
    ...
    /*setup egress vlan xlate actions*/
    printf("egress vlan xlate actions setup\n");
    rv = egr_vlan_xlate_setup(unit,egress_port, default_vlan, pkt_inner_vlan);
    if (BCM_FAILURE(rv)) {
        printf("\nError in egr_vlan_xlate_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }
