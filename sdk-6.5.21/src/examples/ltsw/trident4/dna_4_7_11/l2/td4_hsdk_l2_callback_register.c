...
/*
 * Feature  : L2 callback function
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_callback_register.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_callback_register_log.txt
 *
 * Test Topology :
 *
 *                      +-------------------+
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 callback function with setup forwarding for ingress untagged
...
verify(int unit)
{
    ...
    printf("\nverify l2 delete callback");
    del_cb_done = 0;
    print bcm_l2_addr_delete_by_mac(unit,"00:00:00:00:00:02",0);
    while (del_cb_done == 0 && time < 10) {
        time++;
        sal_sleep(1);
    }
