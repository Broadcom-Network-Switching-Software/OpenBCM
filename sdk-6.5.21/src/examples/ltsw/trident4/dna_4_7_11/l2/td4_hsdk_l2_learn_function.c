 ...
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_learn_function.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_learn_function_log.txt
 *
 * Test Topology :
 *
 *                      +-------------------+
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 learning configuration on port with setup
 ...
 *     e) ingress_port1 is enabled with smac learning
 *     f) ingress_port1 is disabled for smac learning
 ...
 *
 *   verify l2 learning, port 2 learning is disabled:
...
verify(int unit)
{
    ...
    printf("\nverify l2 learning, port %d learning is disabled:\n",ingress_port_2);
    /*l2 traverse routine*/
...
setup_l2_forwarding(int unit)
{
    ...
    /*Learn mode port config*/
    printf("source mac learning disabled on port %d\n",ingress_port_2);
    rv = port_learn_mode_set(unit, ingress_port_2, BCM_PORT_LEARN_FWD);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_learn_mode_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
