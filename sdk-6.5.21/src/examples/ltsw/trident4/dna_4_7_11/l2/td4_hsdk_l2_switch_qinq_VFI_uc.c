 ...
 * Usage    : BCM.0> cint td4_hsdk_l2_switch_qinq_VFI_uc.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_switch_qinq_VFI_uc_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * packets on switch using VFI configuration based on port+outer vlan+inner vlan
 *
 ...
 *     b) Assign VFI from port+outer vlan+inner vlan key lookup in vlan_xlate table
...
bcm_error_t
vlan_xlate_entry_setup(int unit,bcm_gport_t port,bcm_vlan_t outer_vlan,
                            bcm_vlan_t inner_vlan,bcm_vlan_t fwd_vfi)
{
    ...
    rv = bcm_vlan_translate_action_add(unit,port_class_2,
                            bcmVlanTranslateKeyPortGroupDouble,outer_vlan,inner_vlan,&action);
    if (BCM_FAILURE(rv)) {
...
setup_l2_forwarding(int unit)
{
    ...
    /*configure vlan xlate keys for lookup*/
    rv = vlan_xlate_entry_setup(unit,ingress_port,pkt_outer_vlan,pkt_inner_vlan,default_vlan);
    if (BCM_FAILURE(rv)) {
