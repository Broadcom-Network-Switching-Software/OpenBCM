...
/*
 * Feature  : L2 hitbit functionality
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_setup_hitbit.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_setup_hitbit_log.txt
 *
 * Test Topology :
 *
 *                      +-------------------+
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 hitbit functionality with setup forwarding for ingress untagged
 * packets on switch using default VFI configuration
 * Users who want to use L2 hitbit functionality can set HSDK config property
 * "l2_hitbit_enable=1" to enable L2 hitbit collection
 *
 ...
 *     e) setup l2 age timer of 2 seconds
 ...
 *
 *   L2_EVENT! MAC=00:00:00:00:00:02  VID=20  PORT=1  Operation=BCM_L2_CALLBACK_AGE_EVENT
 */
...
verify(int unit)
{
    ...
    printf("\nverify l2 learning hitbit:\n");
    bshell(unit, "l2 show");
    /*l2 traverse routine*/
    show_l2_entries(unit);

    bshell(unit, "sleep 2");
    printf("\nverify l2 learning hitbit clear:\n");
    bshell(unit, "l2 show");
    /*l2 traverse routine*/
    show_l2_entries(unit);
...
setup_l2_forwarding(int unit)
{
    ...
    /*l2 age timer function*/
    printf("setup l2 age timer of %d seconds\n", age);
    rv = bcm_l2_age_timer_set(unit, age);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_l2_age_timer_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
