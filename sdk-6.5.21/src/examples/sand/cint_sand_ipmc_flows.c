    ...
    int outP3)
{
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        ...
        ipmc_index_lower_bound = 2; /** Skip using MC group 0 and 1. */
    }
    ...
    for (i=0; i <= 5; i++) {  /* picking random values for the mc groups */
        ipmc_flows_info1.ipmc_mc_groups[i] = (sal_rand() % (ipmc_index_upper_bound - ipmc_index_lower_bound)) + ipmc_index_lower_bound;
