/*
 *   cint_qos_l3_remark.c,v 2.0 2017/03/20 08:21:02 Rongwei Exp $
 ...
 *
   ...
   cint cint_qos_l3_remark.c
   cint
 */

/* Map IPV4 ingress as follows
 *   Ingress      INTERNAL       INTERNAL        INGRESS
 *   IPv4.TOS        TC            DP            NWK-QOS
 *
 *    0~255    (Ipv4.tos+1)%8  (IPv4.tos+1)%2    Ipv4.tos+1
 */

/* Map IPv6 ingress as follows
 *   Ingress       INTERNAL        INTERNAL       INGRESS
 *   IPv6.TC          TC             DP           NWK-QOS
 *
 *   tc(0~255)   (IPv6.tc+1)%8   (Ipv6.tc+1)%2    Ipv6.tc+1
 */

/* Map L2 PCP&DEI ingress as follows
 *   Ingress          INTERNAL       INTERNAL        INGRESS
 *   PCP_DEI            TC             DP            NWK-QOS
 *
 *   (pcp<<1)+dei  (pcp_dei+1)%8   (pcp_dei+1)%2    pcp_dei+1
 */

/* Map egress L2 packet  as follows
 *  EGRESS                     Egress                      Egress
 * NWK-QOS/DP                   PCP                          DEI
 *
 *  0~255/nwk_qos%2        ((nwk_qos-2) >> 1) & 0x7     (nwk_qos-2) & 0x1
 *  0~255/others                   0                        0
 */

/* Map egress IPv4 packet  as follows
 *  EGRESS                     Egress
 * NWK-QOS/DP                 IPv4.tos
 *
 *  0~255/nwk_qos%2        (nwk_qos-2)&0xff
 */

/* Map egress Ipv6 packet  as follows
 *  EGRESS                     Egress
 * NWK-QOS/DP                  IPv6.tc
 *
 *  0~255/nwk_qos%2        (nwk_qos-2)& 0xff
...
int
qos_map_egress_remark_map_entry_resolve(int unit, uint32 flags, int qos_value, int dp, int nof_qos, bcm_qos_map_t *qos_map)
{
    /* 
 ...
 */
int
qos_map_egress_remark_set(int unit, uint32 remark_flags, uint32 fwd_hrd_flags)
{
    ...
    for (qos = 0; qos < nof_qos; qos++) {
        for (dp = 0; dp < 4; dp++) {
             ...
             *          map-result: remark_int_pri
             */
            rv = qos_map_egress_remark_map_entry_resolve(unit, remark_map_flags, qos, dp, nof_qos, &l3_eg_map);
            if (rv != BCM_E_NONE) {
                printf("error (%d) in qos_map_egress_remark_map_entry_resolve\n", rv);
                printf("(%s)\n", bcm_errmsg(rv));
                return rv;
            }
                       ...
                       int egr_qos_profile)
{
    ...
    rv = qos_map_egress_remark_set(unit, egr_remark_flags, egr_remark_flags);
    if (rv) {
...
int
qos_map_egress_remark_clear(int unit, uint32 remark_flags, uint32 fwd_hrd_flags)
{
    bcm_qos_map_t l3_eg_map;
    int flags = 0;
    int qos;
                         ...
                         int egr_qos_profile)
{
    ...
    rv = qos_map_egress_remark_clear(unit, egr_remark_flags, egr_remark_flags);
    if (rv) {
 ...
 * Note, used in JR2 mode only
 */
int dnx_qos_l3_dscp_preserve_fai_convert(int unit, bcm_field_layer_type_t forwarding_type, int recycle_port)
{
    int is_jericho2_mode = *(dnxc_data_get(unit,  "headers", "system_headers", "system_headers_mode", NULL));
	...
	presel_entry_data.qual_data[1].qual_type = bcmFieldQualifyForwardingType;
    presel_entry_data.qual_data[1].qual_arg = 0;  
    presel_entry_data.qual_data[1].qual_value = bcmFieldLayerTypeIp4;
    presel_entry_data.qual_data[1].qual_mask = 0x1F;
    rv = bcm_field_presel_set(unit, 0, &presel_entry_id, &presel_entry_data);
    if(rv != BCM_E_NONE) {
        printf("cint_field_evpn_context_create Error in bcm_field_presel_set  \n");
