...
int multicast__delete_multicast_entry(int unit, int ipmc_index, int *ports, int *cud, int nof_mc_entries, int is_egress) {
    ...
    else
    {
        ...
        {
            if (is_egress) {
                rv = bcm_multicast_egress_delete(unit,ipmc_index,ports[ii],cud[ii]);
                if (rv != BCM_E_NONE) {
                    printf("Error, bcm_multicast_egress_delete: port %d encap_id: %d \n", ports[ii], cud[ii]);
                    return rv;
                }
            } 
            else {
                rv = bcm_multicast_ingress_delete(unit,ipmc_index,ports[ii],cud[ii]);
                if (rv != BCM_E_NONE) {
                    printf("Error, bcm_multicast_ingress_delete: port %d encap_id: %d \n", ports[ii], cud[ii]);
                    return rv;
                }
            }
        }
