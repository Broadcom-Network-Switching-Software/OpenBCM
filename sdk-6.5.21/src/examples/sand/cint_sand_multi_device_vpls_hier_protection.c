      ...
      int is_access_side)
{
 ...
 multi_dev_vpls_hier_protection_add_access_port(int unit){
    ...
    rv = fec_id_alignment_field_add(unit, cint_hvpls_info.ac_local_port, BCM_L3_ITF_VAL_GET(cint_hvpls_info.primary_pwe_fec_id));
    if (rv != BCM_E_NONE)
    {
      ...
      int unit)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, multi_dev_vpls_hier_protection_pwe_port_term\n");
        return rv;
    }
