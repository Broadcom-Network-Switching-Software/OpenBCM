...
int tunnels_create(int unit, int encap_mode, int next_pointer, int * last_tunnel_id)
{
    ...
    if (!is_device_or_above(unit, JERICHO2)) {
        ...
        *last_tunnel_id = (encap_mode > 0) ? additional_labels_info.B_intf : additional_labels_info.C_intf;
    } else {
        ...
        */
        ...
        } else if (encap_mode == 2 || encap_mode == 4) {
            /** Create B and C tunnel*/
            label_index = 0;
            nof_labels = (encap_mode == 4) ? 4 : 5;
        } else if (encap_mode == 1 || encap_mode == 3) {
            /** Create B and C tunnel*/
            label_index = 1;
            nof_labels = (encap_mode == 3) ? 3 : 4;
...
int example_ip_to_tunnel_ext_label(int *units_ids, int nof_units, int in_sysport, int out_sysport, int encap_mode){
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        if (is_device_or_above(unit, JERICHO2)) {
            ...
            route_qos_lif = (encap_mode > 0) ? additional_labels_info.B_intf : additional_labels_info.C_intf;
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        fec_structure.tunnel_gport = (encap_mode > 0) ? additional_labels_info.B_intf : additional_labels_info.C_intf;
