    ...
    int outP)
{
    ...
    encap1 = egress_intf[0].encap_id;
    encap2 = egress_intf[1].encap_id;
...
/** Extra copies for the group(special case for QAX/QUX) to verify the ingress double replication format mcdb*/
int extra_reps_en = 0;
int extra_reps = 3; 
    ...
    uint8 change_vlan)
{
    ...
    bcm_multicast_replication_t replications[IP_ROUTE_NOF_REPLICATIONS + extra_reps];
    ...
    /** Special cases to example the usage the double replication format of ingress MCDB*/
    if (extra_reps_en) {
        int extra_ports[extra_reps] = {13, 14, 15};
        for(port_indx = 0; port_indx < extra_reps; ++port_indx) {
            replications[nof_reps].encap1 = replications[port_indx].encap1;
            replications[nof_reps].port = extra_ports[port_indx];
            nof_reps ++;
        }
    }
