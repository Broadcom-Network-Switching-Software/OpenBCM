...
int mirror_erspan_tunnel_create(int unit, bcm_if_t *encap_id) {
    ...
    */
    ...
    if(bta_required)
    {
        /** since all out lifs are pointed from the erspan tunnel, need to estimate the entire encap size */
        /** ARP = min 14, IP = 20 for Ipv4 or 40 for Ipv6 */
        erspan_tunnel.estimated_encap_size = 14 + (G_mirror_tunnel_is_ipv6 ? 40 : 20);
    }
