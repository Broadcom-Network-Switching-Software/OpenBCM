...
int dnx_qos_php_into_mpls(int unit, int in_port, int out_port, int is_pipe, int mode)
{
    ...
    switch (mode)
    {
        case 0:
            /*PHP use API bcm_mpls_tunnel_switch_create*/
            rv = dnx_qos_php_into_mpls_fec_create(unit, out_port, 0, 0, is_pipe);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in dnx_qos_php_fec_create\n");
                return rv;
        ...
        case 1:
            /*PHP use FEC*/
            rv = dnx_qos_php_into_mpls_fec_create(unit, out_port, 1, 0, is_pipe);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in dnx_qos_php_fec_create\n");
                return rv;
        ...
        case 2:
            ...
            rv = mpls_create_initiator_tunnels(unit, mpls_php, 1);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in mpls_create_initiator_tunnels\n");
                return rv;
