 ...
 */
    ...
    if (rv != BCM_E_NONE)
    {   
        printf("%s(): Error, multicast__open_mc_group, vpn=%d, \n", proc_name, cint_vxlan_roo_basic_info.vpn_id);
        return rv; 
    }   
    ...
    rv = vxlan_port_add(unit, core_port1, term_tunnel_id, cint_vxlan_roo_basic_info.core_overlay_fec_id1, &vxlan_port_id);
    if (rv != BCM_E_NONE)
    {
    ...
    bcm_gport_t * vxlan_port_id)
{
    ...
    BCM_L3_ITF_SET(vxlan_port_add.egress_if, BCM_L3_ITF_TYPE_FEC, fec_id);
    ...
    int *term_tunnel_id)
{
    ...
    BCM_GPORT_TUNNEL_ID_SET(tunnel_term.tunnel_id, 0x6000);
