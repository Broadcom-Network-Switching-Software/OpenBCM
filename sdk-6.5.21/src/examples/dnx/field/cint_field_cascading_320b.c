...
int cint_field_cascading_320b_main(int unit, bcm_field_context_t context_id)
{
    ...
    rv = cint_field_cascading_320b_fg_attach(unit, cint_field_cascading_320b_ipmf1_fg_id, context_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error while trying to attach 320b FG (%d) in iPMF1 stage on context %d: %d\n",
                cint_field_cascading_320b_ipmf1_fg_id, context_id, rv);
        return rv;
    }
    ...
    rv = cint_field_cascading_320b_ipmf2_failing_attach(unit, cint_field_cascading_320b_ipmf2_fg_id, context_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error when fail-attaching iPMF2 stage FG (%d) on context (%d): %d\n",
                cint_field_cascading_320b_ipmf2_fg_id, context_id, rv);
        return rv;
    }

    rv = cint_field_cascading_320b_ipmf2_attach(unit, cint_field_cascading_320b_ipmf2_fg_id, context_id);
    if (rv != BCM_E_NONE)
    {
...
int cint_field_cascading_320b_destroy(int unit, bcm_field_context_t context_id)
{
    ...
    rv = bcm_field_group_context_detach(unit, cint_field_cascading_320b_ipmf2_fg_id, context_id);
    if (rv != BCM_E_NONE)
    {
        printf ("Error detaching iPMF2 stage FG (%d) from context %d: %d\n", cint_field_cascading_320b_ipmf2_fg_id, context_id, rv);
    }

    rv = bcm_field_group_context_detach(unit, cint_field_cascading_320b_ipmf1_fg_id, context_id);
    if (rv != BCM_E_NONE)
    {
