...
int field_contexts_cascading_main(int unit, uint32 presel_id, bcm_field_context_t context_id_ipmf1)
{
    ...
    rv = bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF2, &context_info, &context_id_ipmf2);
    if (rv != BCM_E_NONE)
    {
...
int field_contexts_cascading_destroy(int unit)
{
    ...
    rv = bcm_field_context_destroy(unit, bcmFieldStageIngressPMF2, context_id_ipmf2);
    if (rv != BCM_E_NONE)
    {
