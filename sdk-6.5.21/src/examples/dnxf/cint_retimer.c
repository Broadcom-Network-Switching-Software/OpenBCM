    ...
    bcm_port_t *fe_to_fap1_links_array)
{
     ...
     */
    for (link_idx = 0; link_idx < fe_to_fap_nof_links; link_idx++) {
        rv = bcm_fabric_link_control_set(fe_unit, fe_to_fap0_links_array[link_idx], bcmFabricLinkRetimerFecEnable, 1);
        if (BCM_FAILURE(rv))
        {
            printf("bcmFabricLinkRetimerFecEnable failed for link %d.\n", fe_to_fap0_links_array[link_idx]);
            return rv;
        }

        rv = bcm_fabric_link_control_set(fe_unit, fe_to_fap1_links_array[link_idx], bcmFabricLinkRetimerFecEnable, 1);
        if (BCM_FAILURE(rv))
        {
            printf("bcmFabricLinkRetimerFecEnable failed for link %d.\n", fe_to_fap1_links_array[link_idx]);
            return rv;
