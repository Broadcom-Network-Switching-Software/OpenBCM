...
int mpls_nop_action_info_run(int unit) {
    ...
    mpls_encap_id_swap = label_stack.tunnel_id;
    ...
    /*
     * ARAD+ does not support next 
     * pointer from nop mpls tunnel
     */
    ...
    if (is_device_or_above(unit,JERICHO)) 
    {
        label_stack.l3_intf_id = mpls_encap_id_swap;
    }
    rv = bcm_mpls_tunnel_initiator_create(unit, 0, 1, &label_stack);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_mpls_tunnel_initiator_create\n");
    ...
    if (!is_device_or_above(unit,JERICHO)) {
        ...
        eei_value = (mpls_encap_id_swap & 0xFFFFF)+ 0xF00000 /*Valid bits and type*/ ;
