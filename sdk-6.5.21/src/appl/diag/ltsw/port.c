...
cmd_ltsw_port(int unit, args_t *arg)
{
    ...
    int port, dport;
    parse_table_t pt;
    bcm_pbmp_t pbmp, pbmp_op, pbmp_mask;
    char pfmt[_SHR_PBMP_FMT_LEN];
    ...
    if ((_ARG_CUR(arg) == NULL) ||
        (sal_strcasecmp(_ARG_CUR(arg), "StatId") != 0)) {
        BCM_PBMP_AND(pbmp, cfg.port);
    } else {
        BCM_PBMP_ASSIGN(pbmp_mask, cfg.port);
        BCM_PBMP_OR(pbmp_mask, cfg.cpu);
        BCM_PBMP_AND(pbmp, pbmp_mask);
    }
    ...
    if (ARG_CNT(arg) == 0) {
        ...
        if (!sal_strcasecmp(_ARG_CUR(arg), "detach")) {
            ...
            return CMD_OK;
        } else if (!sal_strcasecmp(_ARG_CUR(arg), "StatId")) {
            ARG_GET(arg);
            if ((c = ARG_GET(arg)) == NULL) {
                return CMD_USAGE;
            }
            stat_counter_id = sal_ctoi(c, 0);

            if ((c = ARG_GET(arg)) == NULL) {
                return CMD_USAGE;
            }
            BCM_PBMP_ITER(pbmp, port) {
                if (!sal_strcasecmp(c, "attach")) {
                    rv = bcm_port_stat_attach(unit, port, stat_counter_id);
                    if (BCM_FAILURE(rv)) {
                        cli_out("Stat attach %s: errorNo%d\n",
                                bcmi_ltsw_port_name(unit, port), rv);
                    }
                } else if (!sal_strcasecmp(c, "detach")) {
                    rv = bcm_port_stat_detach_with_id(unit, port, stat_counter_id);
                    if (BCM_FAILURE(rv)) {
                        cli_out("Stat detach %s: errorNo%d\n",
                                bcmi_ltsw_port_name(unit, port), rv);
                    }
                } else {
                    return CMD_USAGE;
...
if_ltsw_asf(int unit, args_t *a)
{
    ...
    if (arg && !sal_strcasecmp(arg, "show")) {
        ...
        BCMI_LTSW_DPORT_PBMP_ITER(unit, pbm, dp, p) {
...
static ltsw_pm_mode_t pm8x50_th4_mode[] = {
    {
        "None",
        0,
    },
    ...
    /* example of mixed speed */
    {
        ...
        3,
        {
            [0] = {
                ...
                .lanes = 4,
                .encap = BCM_PORT_ENCAP_IEEE,
                .fec_type = bcmPortPhyFecRs272,
                .phy_lane_config = -1,
                .link_training = 0,
            },
            /* 50G ports start from 1 */
            [1] = {
                .speed = 50000,
                .lanes = 1,
                .encap = BCM_PORT_ENCAP_IEEE,
...
typedef struct ltsw_flexport_traffic_info_s {
    /* Info get from cli input*/
    char *traffic_str;
} ltsw_flexport_traffic_info_t;

typedef struct ltsw_flexport_info_s {
    ltsw_flexport_pm_info_t pm[MAX_PM_NUM];
    ltsw_flexport_traffic_info_t traffic;
    int add_cnt;
    int del_cnt;
} ltsw_flexport_info_t;

typedef struct ltsw_flexport_stat_s {
    uint64_t byte;
    uint32_t time;
} ltsw_flexport_stat_t;
...
ltsw_flexport_port_assign(int unit, ltsw_flexport_info_t *f)
{
    ...
    (void)bcmi_ltsw_port_bitmap(unit, BCMI_LTSW_PORT_TYPE_ALL, &cur_pbmp);
...
extern int bshell(int, char*);
static cmd_result_t
ltsw_flexport_traffic_enable(int unit, ltsw_flexport_info_t *f, int enable)
{
    int port, port_tx = 0, pm, i, lb, lnk, rv;
    char cmd[128];
    ltsw_dev_info_t *dev;

    dev = ltsw_dev_lookup(unit);
    lb = enable ? BCM_PORT_LOOPBACK_PHY : BCM_PORT_LOOPBACK_NONE;
    lnk = enable ? BCM_LINKSCAN_MODE_SW : BCM_LINKSCAN_MODE_NONE;

    for (pm = 0; pm < dev->num_pm; pm++) {
        for (i = 0; i < f->pm[pm].port_cnt; i ++) {
            port = f->pm[pm].pr[i].port;
            if (port_tx == 0) {
                port_tx = port;
            }
            ...
            if (BCM_FAILURE(rv)) {
                cli_out("Port[%d] enable[%d] failed(%d).\n", port, enable, rv);
                return CMD_FAIL;
            }
            rv = bcm_linkscan_mode_set(unit, port, lnk);
            if (BCM_FAILURE(rv)) {
                cli_out("Port[%d] link[%d] failed(%d).\n", port, lnk, rv);
                return CMD_FAIL;
            }
            rv = bcm_port_loopback_set(unit, port, lb);
            if (BCM_FAILURE(rv)) {
                cli_out("Port[%d] loopback[%d] failed(%d).\n", port, lb, rv);
                return CMD_FAIL;
            }
            rv = bcm_port_control_set(unit, port, bcmPortControlBridge, enable);
            if (BCM_FAILURE(rv)) {
                cli_out("Port[%d] ing bridge[%d] failed(%d).\n", port, enable, rv);
                return CMD_FAIL;
            }
            rv = bcm_port_control_set(unit, port, bcmPortControlBridgeEgress, enable);
            if (BCM_FAILURE(rv) && rv != BCM_E_UNAVAIL) {
                cli_out("Port[%d] egr bridge[%d] failed(%d).\n", port, enable, rv);
                return CMD_FAIL;
            }
        }
    }
    ...
    if (enable) {
        sal_sprintf(cmd, "tx 100 pbm=%d vl=1 l=1200", port_tx);
        bshell(unit, cmd);
        sal_sleep(2);
    }
...
static cmd_result_t
ltsw_flexport_traffic_rate(int unit, ltsw_flexport_info_t *f)
{
    int port, pm, time = 5, time_us, i, size, rate, rv;
    ltsw_flexport_stat_t *stat_start = NULL, *stat_end = NULL;
    uint64_t u64;
    ltsw_dev_info_t *dev;
    cmd_result_t retCode = CMD_OK;
    ...
    size = MAX_PM_NUM * MAX_PORTS_PER_PM * sizeof(ltsw_flexport_stat_t);
    stat_start = sal_alloc(size, "stat start");
    if (stat_start == NULL) {
        cli_out("Insufficient memory.\n");
        retCode = CMD_FAIL;
        goto exit;
    }
    sal_memset(stat_start, 0, size);
    stat_end = sal_alloc(size, "stat end");
    if (stat_end == NULL) {
        cli_out("Insufficient memory.\n");
        retCode = CMD_FAIL;
        goto exit;
    }
    sal_memset(stat_end, 0, size);

    for (pm = 0; pm < dev->num_pm; pm++) {
        for (i = 0; i < f->pm[pm].port_cnt; i ++) {
            port = f->pm[pm].pr[i].port;

            stat_start[port].time = sal_time_usecs();
            rv = bcm_stat_sync_get(unit, port, snmpIfInOctets, &stat_start[port].byte);
            if (BCM_FAILURE(rv)) {
                cli_out("Port[%d] snmpIfInOctets failed(%d).\n", port, rv);
                retCode = CMD_FAIL;
                goto exit;
            }
            rv = bcm_stat_sync_get(unit, port, snmpIfInUcastPkts, &u64);
            if (BCM_FAILURE(rv)) {
                cli_out("Port[%d] snmpIfInUcastPkts failed(%d).\n", port, rv);
                retCode = CMD_FAIL;
                goto exit;
            }
            stat_start[port].byte += (u64 * 20);
        }
    }
    ...
    for (pm = 0; pm < dev->num_pm; pm++) {
        for (i = 0; i < f->pm[pm].port_cnt; i ++) {
            ...
            stat_end[port].time = sal_time_usecs();
            rv = bcm_stat_sync_get(unit, port, snmpIfInOctets, &stat_end[port].byte);
            if (BCM_FAILURE(rv)) {
                cli_out("Port[%d] snmpIfInOctets failed(%d).\n", port, rv);
                retCode = CMD_FAIL;
                goto exit;
            ...
            stat_end[port].byte += (u64 * 20);
        }
    }

    cli_out("  PM  Port  PHY  Rate (Mbps)\r\n");
    cli_out("-------------------------\r\n");
    for (pm = 0; pm < dev->num_pm; pm++) {
        for (i = 0; i < f->pm[pm].port_cnt; i ++) {
            port = f->pm[pm].pr[i].port;

            time_us = stat_end[port].time - stat_start[port].time;
            rate = (stat_end[port].byte - stat_start[port].byte) * 8 / time_us;
            cli_out("  %2d  %3d   %3d   %4d.%03d  ", pm, port, f->pm[pm].pr[i].physical_port, rate / 1000, rate % 1000);
            cli_out("%s\r\n", (rate < f->pm[pm].pr[i].speed * 0.9) ? "FAIL" : "");
        }
    }
...
static cmd_result_t
ltsw_flexport_traffic_test(int unit, ltsw_flexport_info_t *f, int traffic, int allmode, int allpm)
{
    int pm, pm_groups, i, auto_assign;
    ltsw_dev_info_t *dev;
    ltsw_pm_mode_t *mode;
    cmd_result_t retCode = CMD_OK;
    ...
    if (allmode) {
        for (mode = dev->pm_mode + 1; mode->pm_str; mode ++) {
            cli_out("\r\nTest %s:\r\n", mode->pm_str);

            if (allpm) {
                /* some modes need too many ports, cannot test all PMs once */
                pm_groups = sal_ceil_func((dev->num_pm / dev->num_pipe),
                    (dev->num_port / dev->num_pipe / mode->num_port));
            } else {
                /* non-allpm mode doesn't consider this problem */
                pm_groups = 1;
            }

            for (i = 0; i < pm_groups; i ++) {
                for (pm = 0; pm < dev->num_pm; pm++) {
                    if (pm % pm_groups == i) {
                        if (allpm || (f->pm[pm].flags & FLXP_F_SEEN)) {
                            f->pm[pm].pm_str = mode->pm_str;
                            f->pm[pm].flags &= ~FLXP_F_PORT_ASSIGNED;
                        }
                    } else {
                        /* not tested PMs set to none */
                        f->pm[pm].pm_str = "None";
                        f->pm[pm].flags &= ~FLXP_F_PORT_ASSIGNED;
                    }
                }
                if (retCode == CMD_OK) {
                    retCode = ltsw_flexport_traffic_test(unit, f, traffic, 0, 0);
                }
            }
        }
    } else {
        auto_assign = 0;
        retCode = ltsw_flexport_parse_portlist(unit, f->pm);
        if (retCode == CMD_OK) {
            retCode = ltsw_flexport_parse_pm_mode(unit, f, &auto_assign);
        }
        if (retCode == CMD_OK && auto_assign) {
            retCode = ltsw_flexport_port_assign(unit, f);
        }
        if (retCode == CMD_OK) {
            retCode = ltsw_flexport_diag(unit, f);
        }
        if (traffic) {
            if (retCode == CMD_OK) {
                retCode = ltsw_flexport_traffic_enable(unit, f, 1);
            }
            if (retCode == CMD_OK) {
                retCode = ltsw_flexport_traffic_rate(unit, f);
            }
            if (retCode == CMD_OK) {
                retCode = ltsw_flexport_traffic_enable(unit, f, 0);
            }
            sal_sleep(1);
        }
    }
...
cmd_ltsw_flexport(int unit, args_t *a)
{
    ...
    cmd_result_t retCode = CMD_OK;
    int i;
    uint32 pm_seen, pm_parsed, traffic;
    char *pm_str[] = {
    ...
    parse_table_add(&pt, "Traffic", PQ_STRING|PQ_DFL|PQ_NO_EQ_OPT, 0,
                    &flexinfo->traffic.traffic_str, NULL);
    ...
    if (pm_seen && pm_parsed) {
        cli_out("%s: Cannot get and set port resources in one command\n",
                ARG_CMD(a));
        retCode = CMD_FAIL;
    } else if (pm_seen) {
        retCode = ltsw_flexport_show(unit, flexinfo);
        if ((retCode == CMD_OK) && traffic) {
            retCode = ltsw_flexport_traffic_test(unit, flexinfo, 1, 1, 0);
        }
    } else if (pm_parsed) {
        retCode = ltsw_flexport_traffic_test(unit, flexinfo, traffic, 0, 0);
    } else if (traffic) {
        retCode = ltsw_flexport_traffic_test(unit, flexinfo, 1, 1, 1);
    }
