...
underlay L3 next hop and interface indices do not comply with configured allocation bitmap"}
};
...
counter_control_get(int unit, ltsw_counter_t *cfg)
{
    ...
    port_ptr = sal_alloc(BCM_PBMP_PORT_MAX * sizeof(uint64_t),
                         "bcmDiagLtswCounterPortPtr");
    if (port_ptr == NULL) {
        rv = SHR_E_MEMORY;
        goto error;
    }
    sal_memset(port_ptr, 0, BCM_PBMP_PORT_MAX * sizeof(uint64_t));

    rv = bcmlt_entry_field_array_get(entry_hdl, PORT_IDs, 0, port_ptr,
    ...
    for (port = 0; port < num_array; port++) {
        if (port_ptr[port] > 0) {
...
counter_control_update(int unit, ltsw_counter_t *cfg)
{
    ...
    rv = bcmlt_entry_field_array_add(entry_hdl, PORT_IDs, 0, port_ptr,
                                     field_def.elements);
    if (SHR_FAILURE(rv)) {
        goto error;
    }
...
ltsw_resync_counters(int unit, bcm_pbmp_t pbmp)
{
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        rv = ltsw_mac_counter_resync(unit, CTR_ING_PFCs, port);
        if (SHR_FAILURE(rv)) {
            cli_out("Error: Resync ingress PFC counters for port=%d failed(%s).\n",
                    port, shr_errmsg(rv));
        }
...
ltsw_show_counters_proc(int unit, bcm_pbmp_t pbmp, int flags)
{
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        rv = ltsw_mac_counter_show(unit, CTR_ING_PFCs, port, flags);
        if (SHR_FAILURE(rv)) {
            cli_out("Error: Show ingress PFC counters for port=%d failed(%s).\n",
                    port, shr_errmsg(rv));
        }
...
ltsw_clear_counters_proc(int unit, bcm_pbmp_t pbmp)
{
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        /* Clear ingress PFC counters. */
        rv = ltsw_mac_counter_clear(unit, CTR_ING_PFCs, port);
        if (SHR_FAILURE(rv)) {
            cli_out("Error: Clear ingress PFC counter for port=%d failed(%s).\n",
                    port, shr_errmsg(rv));
        }

        /* Clear the software ingress PFC counters. */
        ltsw_counter_val_set(unit, port, ltsw_ctr_ctrl.ing_pfc_entries_perport,
                             ltsw_ctr_ctrl.ing_pfc_base_index, -1, 0);
