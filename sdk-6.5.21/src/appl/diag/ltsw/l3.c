...
l3_cmd_host_print(int unit, bcm_l3_host_t *info)
{
    ...
    hit = info->l3a_flags & BCM_L3_HIT ? 1 : 0;
    if (info->l3a_flags & BCM_L3_IP6) {
        format_ip6addr(ip_str, info->l3a_ip6_addr);
        cli_out("%-4d %-19s %-4d %-4d %-4d", info->l3a_vrf, ip_str,
                info->l3a_intf, info->l3a_ul_intf, hit);
    } else {
        format_ipaddr(ip_str, info->l3a_ip_addr);
        cli_out("%-4d %-19s %-4d %-4d %-4d", info->l3a_vrf, ip_str, info->l3a_intf,
                info->l3a_ul_intf, hit);
    }
    cli_out("\n");
}
...
l3_cmd_route_print(int unit, bcm_l3_route_t *info)
{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        cli_out("%-8s %-15s/%d %-4d %-4d %-4d", vrf_str, ip_str, masklen,
                info->l3a_intf, info->l3a_ul_intf, hit);
    } else {
        format_ipaddr_mask(ip_str, info->l3a_subnet, info->l3a_ip_mask);
        cli_out("%-8s %-19s %-4d %-4d %-4d", vrf_str, ip_str, info->l3a_intf,
                info->l3a_ul_intf, hit);
    }
    cli_out("\n");
}
...
route_perf_test_setup(int unit)
{
    ...
    if ((rv < 0) && (rv != BCM_E_UNAVAIL)) {
        cli_out("UT Route: Could not get mod id %d: %s\n", rv, bcm_errmsg(rv));
        return rv;
 ...
 * \param [in] data_mode Data mode (0-VRF data mode; 1-reduced; 2-full) .
...
route_perf_v4_op_test(int unit, bcmi_ltsw_l3_route_perf_opcode_t op, int use_api,
                      int set_idx, route_perf_data_t *rpd, int vrf,
                      int data_mode, int *route_cnt, bool forte)
{
    int rv = BCM_E_NONE;
    ...
    bcm_l3_vrf_route_data_mode_t data_type = 0;

    *route_cnt = 0;

    if (data_mode == 1) {
        data_type = bcmL3VrfRouteDataModeReduced;
        flags3 = BCM_L3_FLAGS3_DATA_MODE_REDUCED;
    } else if (data_mode == 2) {
        data_type = bcmL3VrfRouteDataModeFull;
        flags3 = BCM_L3_FLAGS3_DATA_MODE_FULL;
    } else {
        bcm_l3_vrf_route_data_mode_get(unit, vrf, 0, &data_type);
    }

    if (forte == 1) {
        flags3 = BCM_L3_FLAGS3_FORTE_ROUTE;
    }
    ...
    for (i = 0; i < loop; i++) {
        if (use_api) {
            ...
            rv = bcmi_l3_route_perf_v4_lt_op(unit, op, vrf, ip, ip_mask,
                                             intf, data_type, forte);
        }
        if (BCM_FAILURE(rv)) {
            end = sal_time_usecs();
            duration = end - start;
 ...
 */
static int
route_perf_v4_test(int unit, int ops, int loops, int nsets, int use_api,
                   int vrf, int data_mode, bool forte)
{
    int run, i, sz, op;
    ...
    for (run = 0; run < loops; run++) {
        ...
        for (op = 0; op < BCMI_LTSW_RP_OPCODE_NUM; op++) {
            ...
            for (i = 0; i < nsets; i++) {
                rv = route_perf_v4_op_test(unit, op, use_api, i, rpd[op], vrf,
                                           data_mode, &route_cnt, forte);
                      ...
                      int data_mode, int *route_cnt, bool forte)
{
    ...
    if (data_mode == 1) {
        ...
        bcm_l3_vrf_route_data_mode_get(unit, vrf, 1, &data_type);
    }
    ...
    *route_cnt = 0;
    ...
    for (i = 0; i < loop; i++) {
        if (use_api) {
            ...
            rv = bcmi_l3_route_perf_v6_lt_op(unit, op, vrf, ip, ip_mask,
                                             intf, data_type, forte);
        }
        if (BCM_FAILURE(rv)) {
            end = sal_time_usecs();
 ...
 */
static int
route_perf_v6_test(int unit, int ops, int loops, int nsets, int use_api,
                   int vrf, int data_mode, bool forte)
{
    int run, i, sz, op;
    int route_add = 0, set_size = 0, route_cnt = 0, tot_route = 0;
    ...
    for (run = 0; run < loops; run++) {
        ...
        for (op = 0; op < BCMI_LTSW_RP_OPCODE_NUM; op++) {
            ...
            for (i = 0; i < nsets; i++) {
                rv = route_perf_v6_op_test(unit, op, use_api, i, rpd[op], vrf,
                                           data_mode, &route_cnt, forte);
                if (op == BCMI_LTSW_RP_OPCODE_ADD) {
                    route_add += route_cnt;
                    if (i == 0) {
...
ltsw_l3_route_perf(int unit, int ip_type, int ops, int loops, int nsets,
                   int use_api, int vrf_bmp, int data_mode, bool forte)
{
    ...
    if ((ip_type == 1) || (ip_type == 3)) { /* IPv4 first */
        ...
        for (i = 0; i < COUNTOF(vrf_arr); i ++) {
            ...
            rv = route_perf_v4_test(unit, ops, loops, nsets, use_api,
                                    vrf_arr[i], data_mode, forte);
    ...
    if (ip_type != 1) { /* IPv6 */
        ...
        for (i = 0; i < COUNTOF(vrf_arr); i ++) {
            ...
            rv = route_perf_v6_test(unit, ops, loops, nsets, use_api,
                                    vrf_arr[i], data_mode, forte);
            if (BCM_FAILURE(rv) && rv != BCM_E_RESOURCE) {
                goto exit;
            }
...
{
    int ip_type, loops, nsets, op_bitmap, vrf_bmp, use_api, rv, data_mode;
    ...
    parse_table_add(&pt, "DataMode", PQ_DFL | PQ_INT, 0, (void *)&data_mode, 0);
    parse_table_add(&pt, "FORTE", PQ_DFL | PQ_INT, 0, (void *)&forte, 0);
    ...
    rv = ltsw_l3_route_perf(unit, ip_type, op_bitmap, loops, nsets, use_api,
                            vrf_bmp, data_mode, forte);
    if (BCM_FAILURE(rv)) {
...
cmd_ipmc_entry_print(int unit, bcm_ipmc_addr_t *info, void *cookie)
{
    ...
    if (info->flags & BCM_IPMC_IP6) {
        ...
        sip_masklen = bcm_ip6_mask_length(info->s_ip6_mask);
        masklen = bcm_ip6_mask_length(info->mc_ip6_mask);
        cli_out("SRC IP ADDRESS : %s\n", s_ip_str);
        cli_out("SRC IP MASK LEN : %d\n", sip_masklen);
        ...
        sip_masklen = bcm_ip_mask_length(info->s_ip_mask);
...
alpm_res_usage_print(int unit, bcm_l3_alpm_resource_t *res)
{
    ...
    for (grp = 0; grp < bcmL3RouteGroupCounter; grp++) {
        ...
        if (!ltsw_feature(unit, LTSW_FT_MTOP)) {
            if (grp >= bcmL3RouteGroupFortePrivateV4 &&
...
#define IPMC_SET 5
    ...
    if (sal_strcasecmp(table, "table") == 0) {
        ...
        case IPMC_ADD:
            ...
            parse_table_add(&pt, "Src_IP", PQ_DFL|PQ_IP, 0,
                    (void *)&s_ip_addr, 0);
            parse_table_add(&pt, "Src_IPMaskLen", PQ_DFL|PQ_INT, 0,
                    (void *)&s_ip_mask_len, 0);
            ...
            ipmc_data.s_ip_mask = bcm_ip_mask_create(s_ip_mask_len);
    ...
    if (sal_strcasecmp(table, "ip6table") == 0) {
        ...
        case IPMC_ADD:
            ...
            bcm_ip6_mask_create(ipmc_data.s_ip6_mask, s_ip6_mask_len);
            ipmc_data.cos = cos;
            ipmc_data.vid = vlan;
