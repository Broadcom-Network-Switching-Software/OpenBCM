...
do_dump_sw(int unit, args_t *a)
{
    ...
    if (dump_all || !sal_strcasecmp(c, "id_port")) {
        bcmi_ltsw_id_port_dump_sw(unit);
    }

    if (dump_all || !sal_strcasecmp(c, "id_l2_if")) {
        bcmi_ltsw_id_l2_if_dump_sw(unit);
    }
    ...
    if (dump_all || !sal_strcasecmp(c, "l3")) {
        bcmi_ltsw_l3_fib_sw_dump(unit);
    }

    if (dump_all || !sal_strcasecmp(c, "link")) {
        bcmi_ltsw_linkscan_dump_sw(unit);
    }
