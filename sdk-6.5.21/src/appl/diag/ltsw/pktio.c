...
static bcm_pktio_trace_data_t pkt_td[BCM_MAX_NUM_UNITS];
...
trace_tx(int unit, args_t *a)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_TRACE_DOP)) {
        ...
        rv = bcm_pktio_trace_data_collect(unit, &trace_pkt, &pkt_td[unit]);
    }
    if (BCM_FAILURE(rv)) {
        goto error;
    }
...
trace_parse(int unit, args_t *a)
{
    ...
    if (buf_data) {
        ...
        if (ltsw_feature(unit, LTSW_FT_TRACE_DOP)) {
            ...
            if (pkt_td[unit].len) {
                trace_data.len = pkt_td[unit].len;
                sal_memcpy(&trace_data.buf, pkt_td[unit].buf, pkt_td[unit].len);
            } else {
                goto error;
            }
        }
    ...
    /* Parse trace buffer */
    if (ltsw_feature(unit, LTSW_FT_TRACE_DOP)) {
        (void)bcmi_ltsw_pktio_trace_dop_data_dump(unit, dop, &trace_data);
    } else {
        (void)bcmi_ltsw_pktio_trace_fixed_data_dump(unit, &trace_data);
    }
