...
static char **obj_list_pruned = NULL;
 ...
 */
static int
get_flexctr_obj_from_index(int object_idx) {
    if (object_idx > bcmFlexctrObjectStaticCount) {
        return (object_idx +
          bcmFlexctrObjectIngSystemSource - (bcmFlexctrObjectStaticCount + 1));
    } else {
        return object_idx;
    }
}
...
static void
create_flexctr_obj_list(void) {
    int idx;
    int obj_count = 0;
    int max = COUNTOF(obj_list);

    /* Find out how many valid strings exist in the list */
    ...
    /* Allocate memory to store only valid ENUM member names */
    obj_list_pruned =
        (char **)sal_alloc((obj_count+1)*sizeof(char *), "flexctrlist");

    if (obj_list_pruned) {
        obj_count=0;
        /* Store valid ENUM member name string addresses to the new list */
        for (idx=0 ; idx<max ; idx++) {
            if (obj_list[idx][0]) {
                obj_list_pruned[obj_count++] = obj_list[idx];
            }
        }
        /* Store NULL to the last location as we need NULL terminated array */
        obj_list_pruned[obj_count] = NULL;
    }
    return;
}
...
cmd_flexctr_quantization_create(int unit, args_t *a)
{
    ...
    parse_table_add(&pt, "Object",
                    PQ_DFL|PQ_MULTI, 0, &object, obj_list_pruned);
    ...
    */
    ...
    quant.object = (bcm_flexctr_object_t) get_flexctr_obj_from_index(object);
                      ...
                      bcm_flexctr_index_operation_t *index_op)
{
    ...
    cli_out("\tindex=[%d %s 0]|[%d %s 0]\n",
            bit1, obj_list[obj1], bit0, obj_list[obj0]);
}
...
cmd_flexctr_index_op_setup(int unit, args_t *a)
{
    ...
    parse_table_init(unit, &pt);
    parse_table_add(&pt, "Object0", PQ_DFL|PQ_MULTI, 0, &obj0, obj_list_pruned);
    parse_table_add(&pt, "Object1", PQ_DFL|PQ_MULTI, 0, &obj1, obj_list_pruned);
    ...
    if (flags & 0x1) {
        index_op->object[0] =
            (bcm_flexctr_object_t) get_flexctr_obj_from_index(obj0);
    }

    if (flags & 0x2) {
        index_op->object[1] =
            (bcm_flexctr_object_t) get_flexctr_obj_from_index(obj1);
    }
...
cmd_flexctr_trigger_setup(int unit, args_t *a)
{
    ...
    if (flags & 0x8) {
        trigger->object =
            (bcm_flexctr_object_t) get_flexctr_obj_from_index(object);
...
static cmd_result_t
cmd_flexctr_object_show(int unit, args_t *a)
{
    parse_table_t pt;
    int all = TRUE, rv = CMD_OK, i;
    ...
    if (ARG_CNT(a)) {
        ...
        parse_table_init(unit, &pt);
        parse_table_add(&pt, "object",
                        PQ_DFL|PQ_MULTI, 0, &object, obj_list);
        if (parse_arg_eq(a, &pt) < 0 || ARG_CNT(a) > 0) {
            cli_out("%s: ERROR: Unknown option: %s\n", ARG_CMD(a), ARG_CUR(a));
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
    ...
    cli_out("\t%50s %10s %10s \n",
            "object", "offset", "width");
    if (all) {
        for (i = bcmFlexctrObjectConstZero;
             i < bcmFlexctrObjectStaticCount; i++) {
            bcm_flexctr_object_info_t_init(&info);
            rv = bcm_flexctr_object_info_get(unit, i, &info);
            if (BCM_SUCCESS(rv)) {
                cli_out("\t%50s %10d %10d \n",
                obj_list[i], info.offset, info.width);
            }
        }
        for (i = bcmFlexctrObjectIngSystemSource;
             i < bcmFlexctrObjectCount; i++) {
            bcm_flexctr_object_info_t_init(&info);
            rv = bcm_flexctr_object_info_get(unit, i, &info);
            if (BCM_SUCCESS(rv)) {
                cli_out("\t%50s %10d %10d \n",
        ...
        bcm_flexctr_object_info_t_init(&info);
        rv = bcm_flexctr_object_info_get(unit, object, &info);
        if (BCM_SUCCESS(rv)) {
                cli_out("\t%50s %10d %10d \n",
                obj_list[object], info.offset, info.width);
        } else {
            cli_out("%s: Not supported: %s\n", ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
    }
...
cmd_ltsw_flexctr(int unit, args_t *a)
{
    ...
    if ((sal_strcasecmp(table, "quantization") == 0) ||
        (sal_strcasecmp(table, "index") == 0) ||
        (sal_strcasecmp(table, "valueA") == 0) ||
        (sal_strcasecmp(table, "valueB") == 0) ||
        (sal_strcasecmp(table, "trigger") == 0)) {
        if (NULL == obj_list_pruned) {
            create_flexctr_obj_list();
            if (NULL == obj_list_pruned) {
                cli_out("%s: ERROR: Memory alloc failure!\n", ARG_CMD(a));
                return CMD_FAIL;
            }
        }
    }
    ...
    if (sal_strcasecmp(table, "object") == 0) {
        switch (sub_command) {
            case FLEXCTR_SHOW:
                return cmd_flexctr_object_show(unit, a);
            default:
                return CMD_USAGE;
        }
    }
