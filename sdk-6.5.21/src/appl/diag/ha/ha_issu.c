...
static issu_info_t issu_info = {
    {false, SDK_VER_START, SDK_VER},
    {false, SDKLT_VER_START, SDKLT_VER}
};
 ...
 */
static int
ha_issu_version_set(const char *version, char *sdk_ver, char *sdklt_ver)
{
    char *separator = NULL;
...
{
    return ha_issu_version_set(start_ver,
...
{
    return ha_issu_version_set(current_ver,
...
appl_ha_issu_upgrade_start(void)
{
    ...
    if (!skip_sdk) {
        rv = soc_cm_issu_upgrade_start(sdk->start_ver, sdk->current_ver);
        if (rv < 0) {
            return rv;
        }
        sdk->actived = true;
...
appl_ha_issu_upgrade_done(void)
{
    ...
    if (sdk->actived) {
        rv = soc_cm_issu_upgrade_done();
        if (rv < 0) {
            return rv;
        }
        sdk->actived = false;
    }
    appl_ha_issu_start_version_set(VERSION_UNKNOWN);
