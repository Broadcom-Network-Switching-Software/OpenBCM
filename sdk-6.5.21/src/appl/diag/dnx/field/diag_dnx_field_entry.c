    ...
    int unit)
{
    ...
    entry_handle = DNX_FIELD_ENTRY_TCAM_INTERNAL(200, DNX_FIELD_TCAM_CORE_ALL);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (core_index = 0; core_index < nof_cores; core_index++)
    {
        ...
        if (dnx_field_group_tcam_entry_cache[fg_id].nof_valid_entries > 0)
        {
            char entry_ids_string[DIAG_DNX_FIELD_UTILS_STR_SIZE] = { 0 };
            ...
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            for (entry_iter = 0; entry_iter < dnx_field_group_tcam_entry_cache[fg_id].nof_valid_entries; entry_iter++)
            {
                char ent_id_buff[DIAG_DNX_FIELD_UTILS_STR_SIZE];
                /** Check if the current entry belongs to the current core. */
                if (dnx_field_group_tcam_entry_cache[fg_id].tcam_entries[entry_iter].entry_info.core != core_index
                    && dnx_field_group_tcam_entry_cache[fg_id].tcam_entries[entry_iter].entry_info.core !=
                ...
                DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(NULL, "0x%06X, ",
                                                      dnx_field_group_tcam_entry_cache[fg_id].
                                                      tcam_entries[entry_iter].entry_handle);
                sal_snprintf(ent_id_buff, sizeof(ent_id_buff), "0x%06X, ",
                             dnx_field_group_tcam_entry_cache[fg_id].tcam_entries[entry_iter].entry_handle);
                entry_ids_string_length = sal_strlen(entry_ids_string);
                tmp_buff_length = sal_strlen(ent_id_buff);
                if ((entry_ids_string_length + tmp_buff_length) < RHSTRING_MAX_SIZE)
                {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (DNX_KBP_STATUS_IS_LOCKED(kbp_mngr_status))
    {
        ...
        while (!is_end)
        {
            ...
            DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(NULL, "0x%08X, ", DNX_FIELD_ENTRY_TCAM_EXTERNAL(access_id));
            sal_snprintf(ent_id_buff, sizeof(ent_id_buff), "0x%08X, ", DNX_FIELD_ENTRY_TCAM_EXTERNAL(access_id));
            entry_ids_string_length = sal_strlen(entry_ids_string);
            tmp_buff_length = sal_strlen(ent_id_buff);
            if ((entry_ids_string_length + tmp_buff_length) < RHSTRING_MAX_SIZE)
            {
        ...
        for (bank_id_iter = 0; bank_id_iter < DNX_DATA_MAX_FIELD_TCAM_NOF_BANKS; bank_id_iter++)
        {
            ...
            if (SHR_BITGET(banks_bmp, bank_id_iter))
            {
                ...
                for (entry_iter = 0; entry_iter < max_nof_dt_entries; entry_iter++)
                {
                    ...
                    entry_handle = DNX_FIELD_ENTRY_TCAM_DT(entry_iter, fg_id, core_index);
    ...
    int is_cached)
{
    ...
    if (fg_type == DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM)
    {
        core_id = DNX_FIELD_ENTRY_TCAM_DT_CORE_ID(entry_id);
    }
    else
    {
        core_id = DNX_FIELD_ENTRY_TCAM_INTERNAL_CORE_ID(entry_id);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Get information about the given Entry. */
    SHR_IF_ERR_EXIT(dnx_field_entry_t_init(unit, &entry_info));
    SHR_SET_CURRENT_ERR(dnx_field_entry_kbp_get(unit, fg_id, entry_id, &is_entry_cached, &entry_info));
     ...
     * FG id, entry id.
     */
    SHR_IF_ERR_EXIT(diag_dnx_field_entry_info_external_print(unit, fg_id, entry_id, is_entry_cached, sand_control));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (fg_info->group_basic_info.fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_TCAM:
        {
            ...
            entry_core = DNX_FIELD_ENTRY_TCAM_INTERNAL_DT_CORE_ID(entry_id);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (entry_iter = 0; entry_iter < dnx_field_group_tcam_entry_cache[fg_id].nof_valid_entries; entry_iter++)
    {
        SHR_IF_ERR_EXIT(diag_dnx_field_entry_info_key_and_payload_print_row
                        (unit, fg_id, fg_info,
                         dnx_field_group_tcam_entry_cache[fg_id].tcam_entries[entry_iter].entry_handle,
                         &(dnx_field_group_tcam_entry_cache[fg_id].tcam_entries[entry_iter].entry_info),
                         NULL, NULL, prt_ctr));
    }
