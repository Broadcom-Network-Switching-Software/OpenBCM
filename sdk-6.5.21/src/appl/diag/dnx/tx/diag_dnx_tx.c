    ...
    xd_t * xd)
{
        ...
        if (DNX_PORT_IS_COE_PORT(flags))
        {
            ...
            BCM_PBMP_ITER(config.cpu, port_ndx)
            {
                ...
                if (cpu_channel == 0)
                {
                    ...
                    /*
                     * Overwrite PtchSRCport with the port whose channel id is 0
                     * cpu_channel is gotten from PtchSRCport on Cmodel
                     */
                    xd->xd_ptch_src_port = port_ndx;
#endif
                    break;
                }
            }
        ...
        || (header_type_incoming == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED_2_PP_JR1_MODE))
    {
         ...
         */
        pkt_info->src_gport = xd->xd_ptch_src_port;
#endif
