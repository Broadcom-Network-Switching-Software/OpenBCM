    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (dram_index = dram_start; dram_index <= dram_end; ++dram_index)
    {
        ...
        {
            /**
             * Mask cattrip indication.
             * This test doesn't gate the cattrip indication, so we need to mask it.
             * Otherwise it might be falsly asserted.
             */
...
#ifdef INCLUDE_AAPL
/** 'firmware aapl' man page */
static sh_sand_man_t sh_dnx_firmware_aapl_man = {
    .brief = "HBM PHY firmware AAPL",
    .full = "HBM PHY firmware AAPL\n"
            "Use command=\"hbm\" to see all hbm options.\n"
            "By default the command runs only on HBM0.\n"
            "Add \"-addr :<hbm_index>:\" to run on a specific HBM (use -1 to run on all HBMs).",
    .examples = "command=\"hbm -hbm-diag\"\n"
                "command=\"hbm -hbm-diag -addr :1:\""
};

/** Params that can be used for the for the 'firmware aapl' diag */
static sh_sand_option_t dnx_firmware_aapl_option_list[] = {
    /*
     * Option name | Option Type            | Option Description      | Option Default | Option Extension                               | Valid Range
     */
    {"CoMmanD",         SAL_FIELD_TYPE_STR,   "AAPL command",         "",         NULL,           NULL},
    {NULL}
};

/** see man section */
static shr_error_e
sh_dnx_dram_firmware_aapl_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    char *command = NULL;
    const char *mask_cattrip_command = "hbm -hbm-diag";
     ...
     * cause falsly cattrip indication, so we need to mask the cattrip indication
     * when running this specific tests package.
     */
    if (0 == sal_strncmp(mask_cattrip_command, command, sal_strlen(mask_cattrip_command)))
    {
    ...
    SHR_IF_ERR_EXIT(dnx_hbmc_firmware_aapl_cmd(unit, command));
...
exit:
    if (is_cattrip_masked)
    {
        SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_index)
        {
            SHR_IF_ERR_CONT(dnx_hbmc_cattrip_indication_mask_set(unit, hbm_index, 0));
        }
    }
    SHR_FUNC_EXIT;
}
...
#ifdef INCLUDE_AAPL
    {"AAPL",        sh_dnx_dram_firmware_aapl_cmd,        NULL,                                  dnx_firmware_aapl_option_list,         &sh_dnx_firmware_aapl_man},
#endif
    {NULL}
};
