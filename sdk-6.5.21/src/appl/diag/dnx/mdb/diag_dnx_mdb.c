    ...
    sh_sand_control_t * sand_control)
{
         ...
         dbal_physical_table_id_iter++)
    {
         ...
         */
        if (!MDB_DB_TYPE_IS_VALID(dnx_data_mdb.pdbs.pdb_info_get(unit, dbal_physical_table_id_iter)->db_type))
        {
            continue;
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else if (ISEMPTY(table_name))
    {
        ...
        for (ii = 1; ii < DBAL_NOF_PHYSICAL_TABLES; ii++)
        {
             ...
             * Only print info for MDB tables
             */
            if (!MDB_DB_TYPE_IS_VALID(dnx_data_mdb.pdbs.pdb_info_get(unit, ii)->db_type))
            {
                continue;
            }
                         ...
                         cluster_idx++)
                    {
                        ...
                        if (mdb_physical_table_id != MDB_NOF_PHYSICAL_TABLES)
                        {
                            dbal_physical_table_id =
                                dnx_data_mdb.pdbs.pdbs_info_get(unit, mdb_physical_table_id)->physical_to_logical;
    ...
    else
    {
        ...
        db_type = dnx_data_mdb.pdbs.pdb_info_get(unit, dbal_physical_table_id)->db_type;
...
mdb_diag_verify_kbpsdk(
    int unit)
{
    const char *kbp_sdk_ver_expected = "KBP SDK 1.5.14";
