...
#include <appl/diag/ha/ha_issu.h>
...
sysconf_ltsw_init(void)
{
    ...
    rv = bslmgmt_redir_hook_set(bcma_sys_conf_hybrid_sdk_cli_redir_bsl);
    if (rv < 0) {
...
sysconf_ltsw_ha_init(void)
{
    ...
    for (i = 0; i < SOC_MAX_NUM_DEVICES; i++) {
        ...
        rv = appl_ha_unit_open(i, DEFAULT_HA_FILE_SIZE,
                               enable, warmboot);
        if (SHR_FAILURE(rv)) {
...
sysconf_ltsw_ha_deinit(void)
{
    int i, rv;
    int enable = 0;
    bool has_ltsw_devices = false;

    /* If has ltsw devices, ignore sysconf_ltsw_probe_done status. */
    if (sysconf_ltsw_probe_done <= 0) {
        for (i = 0; i < SOC_MAX_NUM_DEVICES; i++) {
            if (cmdev2ltdev[i] == -1) {
                continue;
            }
            has_ltsw_devices = true;
        }
        if (!has_ltsw_devices) {
            return 0;
        }
    ...
    for (i = 0; i < SOC_MAX_NUM_DEVICES; i++) {
        ...
        rv = appl_ha_unit_close(i, enable, warmexit);
        if (SHR_FAILURE(rv)) {
...
sysconf_ltsw_ha_state_dump(void)
{
    ...
    if (ha_check && warmexit) {
        for (i = 0; i < SOC_MAX_NUM_DEVICES; i++) {
            ...
            rv = appl_ha_unit_state_dump(i, ha_dump_file);
            if (SHR_FAILURE(rv)) {
...
{
    return ha_check ? ha_dump_file : NULL;
}
...
sysconf_ltsw_ha_state_comp(void)
{
    ...
    if (ha_check && warmboot) {
        for (i = 0; i < SOC_MAX_NUM_DEVICES; i++) {
            ...
            rv = appl_ha_unit_state_comp(i, ha_dump_file);
            if (SHR_FAILURE(rv)) {
...
int
sysconf_ltsw_ha_start_version_set(const char *start_ver)
{
    return appl_ha_issu_start_version_set(start_ver);
}

int
sysconf_ltsw_ha_current_version_set(const char *current_ver)
{
    return appl_ha_issu_current_version_set(current_ver);
}

int
sysconf_ltsw_ha_issu_start(void)
{
    int rv = 0;
    ...
    if (warmboot) {
        rv = appl_ha_issu_upgrade_start();
    }
    return rv;
}

int
sysconf_ltsw_ha_issu_done(void)
{
    int rv = 0;
...
sysconf_ltsw_core_start(void)
{
    ...
    if (bcma_sys_conf_hybrid_sdk_init(BCMA_INIT_STAGE_SYSM) < 0) {
...
sysconf_ltsw_detach(int unit)
{
    ...
    if (!sysconf_ltsw_probe_done) {
        bcma_sys_conf_hybrid_sdk_cleanup(BCMA_INIT_STAGE_SYSM);
...
{
    return bcma_sys_probe_override(cmdev2ltdev[unit], dev_name);
}
...
sysconf_ltsw_ha_check_set(int val, const char *valstr)
{
    ...
    if (ha_check) {
        ...
        return bcma_sys_conf_hybrid_sdk_option_set(BCMA_SYS_CONF_OPT_HA_CHECK,
                                                   val,
                                                   valstr);
    }
    return 0;
...
void
sysconf_ltsw_ha_check_get(int *val, char **valstr)
{
    if (val) {
        *val = ha_check;
    }
...
int
sysconf_ltsw_ha_no_del_set(int val)
{
    return bcma_sys_conf_hybrid_sdk_option_set(BCMA_SYS_CONF_HA_NO_DEL,
                                               val,
                                               NULL);
}
