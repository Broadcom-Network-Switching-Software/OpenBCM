    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        if (2 == dnx_data_nif.phys.pm8x50_gen_get(unit))
        {
            BCM_PBMP_ITER(pbmp, port)
            {
                SHR_IF_ERR_EXIT(bcm_dnx_port_resource_get(unit, port, &resource));
                if (10000 == resource.speed)
                {
                    SHR_CLI_EXIT(_SHR_E_PARAM, "Eyescan for 10G port is not supported on this device! \n");
                }
            }
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(logical_ports, port)
    {
        ...
        SHR_IF_ERR_EXIT(phymod_phy_linkcat(phy_access, linkcat_mode));
    }
exit:
    if ((_func_rv = dnxc_phy_diag_linkscan_recover(unit, interval)) < 0)
    {
