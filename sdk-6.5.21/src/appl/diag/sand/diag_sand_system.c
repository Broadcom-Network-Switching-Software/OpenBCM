    ...
    char *arguments_string)
{
    ...
    for (option_index = 0; option_index < option_count; option_index++)
    {
        ...
        for (index = 0; index < example_count; index++)
        {
            if (option_index != 0)
            {
                sal_strncat(example_string[index], " ", sal_strnlen(arguments_string, SH_SAND_MAX_TOKEN_SIZE));
            }

            sal_strncat(example_string[index], examples_range[option_index].keyword,
            ...
            {
                sal_strncat(example_string[index], examples_range[option_index].start,
                            sal_strnlen(arguments_string, SH_SAND_MAX_TOKEN_SIZE));
            }
            else
            {
                ...
                sal_strncat(example_string[index], buf, sal_strnlen(arguments_string, SH_SAND_MAX_TOKEN_SIZE));
            }
        }
    }
    ...
    if ((option = sh_sand_cmd->options) != NULL)
    {
        ...
        RHITERATOR(example, examples_list)
        {
            ...
            if(sal_strstr(examples_list_string, example->params) != NULL)
            {
                ...
                sal_strncat(examples_list_string, example->params, sal_strnlen(example->params, SH_SAND_MAX_TOKEN_SIZE));
            }
        }
    }
    ...
    int flags)
{
    ...
    /** get memory allocation after ctest run */
        ...
        if (!(sh_sand_cmd->flags & SH_CMD_SKIP_MEMORY_CHECK))
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sand_control->sh_sand_cmd->flags & SH_CMD_SKIP_MEMORY_CHECK)
        flags |= SH_CMD_SKIP_MEMORY_CHECK;
