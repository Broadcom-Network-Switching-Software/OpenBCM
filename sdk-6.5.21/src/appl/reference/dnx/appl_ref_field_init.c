    ...
    int *dynamic_flags)
{
    ...
    if (dnx_data_headers.feature.feature_get(unit, dnx_data_headers_feature_flow_id_ext_enable))
    {
        *dynamic_flags = 0;
    }
    else
...
* \param [in] unit        - Device id
* \param [in] context id  - context id
* \return
*   int - Error Type
* \remark
*   * None
    ...
    bcm_field_context_t context_id)
{
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_egress_estimated_bta_btr_config))
    {
        ...
        action_info.prefix_size = action_info.size - 1;
        action_info.size = 1;
        action_info.prefix_value = 26 / 2;
        ...
        bcm_field_qualifier_info_create_t_init(&qual_info);
        dest_char = &(qual_info.name[0]);
        sal_strncpy_s(dest_char, "IP_MC_FB_EBTR_QUAL", sizeof(qual_info.name));
        qual_info.size = 1;
        SHR_IF_ERR_EXIT(bcm_field_qualifier_create(unit, 0, &qual_info, &qual_id));
        ...
        fg_info.action_types[0] = action_id;
        dest_char = &(fg_info.name[0]);
        sal_strncpy_s(dest_char, "IP_MC_FB_EBTR_FG", sizeof(fg_info.name));
        SHR_IF_ERR_EXIT(bcm_field_group_add(unit, 0, &fg_info, &fg_id));
    ...
    int unit)
{
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_egress_estimated_bta_btr_config))
    {
        ...
        bcm_field_context_info_t_init(&context_info);
        dest_char = &(context_info.name[0]);
        sal_strncpy_s(dest_char, "EthFwdLayerEbtrCtx", sizeof(context_info.name));
        SHR_IF_ERR_EXIT(bcm_field_context_create(unit, 0, bcmFieldStageEgress, &context_info, &context_id));
 ...
 */
        ...
        fg_info.stage = bcmFieldStageEgress;
        dest_char = &(fg_info.name[0]);
        sal_strncpy_s(dest_char, "eve_ebtr_epmf", sizeof(fg_info.name));
        fg_info.nof_quals = 0;
        fg_info.nof_actions = 1;
        attach_info.key_info.nof_quals = fg_info.nof_quals;
        attach_info.payload_info.nof_actions = fg_info.nof_actions;
        ...
        /** handle also ip mc fallback to bridge in the same context, with higher priority */
        SHR_IF_ERR_EXIT(appl_dnx_field_ip_mc_fallback_to_bridge_ebtr(unit, context_id));
    }
