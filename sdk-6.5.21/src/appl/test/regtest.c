        ...
        for (i = 0; i < brdc_count; i++) {
            ...
            for (reg = 0; reg < NUM_SOC_REG; reg++) {
                ...
                /* Check the size of  the register by counting the fields and storing the number in reg_size */
                    ...
                    if (SOC_IS_J2P(unit) && (blk == SOC_BLOCK_BROADCAST_MEMBER(unit, brdc_block[i-1], 0)))
                    {
                        rv = soc_direct_reg_set(unit,SOC_BLOCK2SCH(unit, brdc_block[i-1]), addr, reg_size_word, reg_above_64);
                        if (rv < 0)
                        {
                            LOG_ERROR(BSL_LS_APPL_TESTS,
                                        (BSL_META_U(unit, "brdc_blk_test: ERROR ADDR previous instance set: unit %d, register %d : %s\n"), unit, reg, soc_errmsg(rv)));
                            result = BCM_E_FAIL;
                        }
                    }
