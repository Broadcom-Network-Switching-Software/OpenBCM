...
_fb_l3ip6ucast_test_hash(int unit, args_t *a, void *p)
{
    ...
    sal_memcpy(dst_ip, ad->opt_base_ip6, sizeof(ip6_addr_t));
...
#endif
    if (ad->opt_key_src_ip) {
        sal_memcpy(src_ip, ad->opt_src_ip6, sizeof(ip6_addr_t));
...
_fb_l3ip4ucast_test_ov(int unit, args_t *a, void *p)
{
    ...
    while (iterations--) {
        ...
        for (i = 0; i < num_ent_per_bucket; i++) {
            sal_memcpy(entry_tmp[i], entry, sizeof(entry));
            if (ad->opt_verbose) {
                cli_out("Inserting ");
...
_td2_l3ip4ucast_test_ov(int unit, args_t *a, void *p)
{
    ...
    defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_TRIDENT3_SUPPORT)
            ...
            sal_memcpy(&entry_array[offset], entry, sizeof(entry));
            if (ad->opt_verbose) {
                cli_out("Inserting ");
