...
dnx_scheduler_db_port_shaper_rate_dump(int  unit,  int  port_shaper_idx_0,  int  port_shaper_idx_1,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    else
    {
        ...
        if(i0 >= dnx_sw_state_get_nof_elements(unit, DNX_SCHEDULER_MODULE_ID, ((dnx_scheduler_db_t*)sw_state_roots_array[unit][DNX_SCHEDULER_MODULE_ID])->port_shaper))
        {
        ...
        if(dnx_sw_state_get_nof_elements(unit, DNX_SCHEDULER_MODULE_ID, ((dnx_scheduler_db_t*)sw_state_roots_array[unit][DNX_SCHEDULER_MODULE_ID])->port_shaper) == 0)
        {
            SHR_EXIT();
        }
        ...
        for(; i0 < I0; i0++) {
            ...
            if(i1 >= dnx_sw_state_get_nof_elements(unit, DNX_SCHEDULER_MODULE_ID, ((dnx_scheduler_db_t*)sw_state_roots_array[unit][DNX_SCHEDULER_MODULE_ID])->port_shaper[i0]))
            {
                LOG_CLI((BSL_META("dnx_scheduler_db[]->((dnx_scheduler_db_t*)sw_state_roots_array[][DNX_SCHEDULER_MODULE_ID])->port_shaper[][].rate: ")));
                LOG_CLI((BSL_META("Invalid index: %d \n"),i1));
                SHR_EXIT();
            ...
            if(dnx_sw_state_get_nof_elements(unit, DNX_SCHEDULER_MODULE_ID, ((dnx_scheduler_db_t*)sw_state_roots_array[unit][DNX_SCHEDULER_MODULE_ID])->port_shaper[i0]) == 0)
            {
                SHR_EXIT();
            }
