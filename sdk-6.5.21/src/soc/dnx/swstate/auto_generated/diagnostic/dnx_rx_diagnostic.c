...
dnx_rx_parser_info_ftmh_pp_dsp_to_cpu_port_dump(int  unit,  int  ftmh_pp_dsp_to_cpu_port_idx_0,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    else
    {
        ...
        if(i0 >= dnx_sw_state_get_nof_elements(unit, DNX_RX_MODULE_ID, ((rx_paser_info_t*)sw_state_roots_array[unit][DNX_RX_MODULE_ID])->ftmh_pp_dsp_to_cpu_port))
        {
        ...
        if(dnx_sw_state_get_nof_elements(unit, DNX_RX_MODULE_ID, ((rx_paser_info_t*)sw_state_roots_array[unit][DNX_RX_MODULE_ID])->ftmh_pp_dsp_to_cpu_port) == 0)
        {
            SHR_EXIT();
        }
