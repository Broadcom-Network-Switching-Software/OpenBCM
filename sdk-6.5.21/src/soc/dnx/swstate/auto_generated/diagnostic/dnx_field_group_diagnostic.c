...
dnx_field_group_sw_fg_info_context_info_key_id_dump(int  unit,  int  fg_info_idx_0,  int  context_info_idx_0,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_sw_fg_info_context_info_key_id_lsb_dump(unit, fg_info_idx_0, context_info_idx_0, -1, filters));
...
int
dnx_field_group_sw_fg_info_context_info_key_id_lsb_dump(int  unit,  int  fg_info_idx_0,  int  context_info_idx_0,  int  lsb_idx_0,  dnx_sw_state_dump_filters_t  filters)
{
    int i0 = fg_info_idx_0, I0 = fg_info_idx_0 + 1;
    int i1 = context_info_idx_0, I1 = context_info_idx_0 + 1, org_i1 = context_info_idx_0;
    int i2 = lsb_idx_0, I2 = lsb_idx_0 + 1, org_i2 = lsb_idx_0;
    char *s0 = "", *s1 = "", *s2 = "";
    DNXC_SW_STATE_INIT_FUNC_DEFS;
    ...
    if (dnx_sw_state_compare(filters.namefilter, "dnx_field_group fg_info context_info key_id lsb") != TRUE)
    {
         SHR_EXIT();
    }
    ...
    if (filters.nocontent)
    {
         DNX_SW_STATE_PRINT(unit, "swstate dnx_field_group fg_info context_info key_id lsb\n");
    }
    else
    {
        dnx_sw_state_dump_attach_file(
            unit,
            "dnx_field_group_sw/fg_info/context_info/key_id/lsb.txt",
            "dnx_field_group_sw[%d]->","((dnx_field_group_sw_t*)sw_state_roots_array[%d][DNX_FIELD_GROUP_MODULE_ID])->","fg_info[].context_info[].key_id.lsb[]: ");
        ...
        if(i0 >= dnx_sw_state_get_nof_elements(unit, DNX_FIELD_GROUP_MODULE_ID, ((dnx_field_group_sw_t*)sw_state_roots_array[unit][DNX_FIELD_GROUP_MODULE_ID])->fg_info))
        {
            LOG_CLI((BSL_META("dnx_field_group_sw[]->((dnx_field_group_sw_t*)sw_state_roots_array[][DNX_FIELD_GROUP_MODULE_ID])->fg_info[].context_info[].key_id.lsb[]: ")));
            LOG_CLI((BSL_META("Invalid index: %d \n"),i0));
            SHR_EXIT();
        }
        ...
        for(; i0 < I0; i0++) {
            ...
            for(; i1 < I1; i1++) {
                if (i2 == -1) {
                    I2 = DNX_DATA_MAX_FIELD_GROUP_NOF_KEYS_PER_FG_MAX;
                    i2 = dnx_sw_state_dump_check_all_the_same(unit,
                        ((dnx_field_group_sw_t*)sw_state_roots_array[unit][DNX_FIELD_GROUP_MODULE_ID])->fg_info[i0].context_info[i1].key_id.lsb
                        , sizeof(*((dnx_field_group_sw_t*)sw_state_roots_array[unit][DNX_FIELD_GROUP_MODULE_ID])->fg_info[i0].context_info[i1].key_id.lsb), I2, &s2) ? I2 - 1 : 0;
                }
                ...
                for(; i2 < I2; i2++) {
                    ...
                    DNX_SW_STATE_PRINT_MONITOR(
                        unit,
                        "dnx_field_group_sw[%d]->","((dnx_field_group_sw_t*)sw_state_roots_array[%d][DNX_FIELD_GROUP_MODULE_ID])->","fg_info[%s%d].context_info[%s%d].key_id.lsb[%s%d]: ", s0, i0, s1, i1, s2, i2);
                    ...
                    dnx_sw_state_print_uint8(
                        unit,
                        &((dnx_field_group_sw_t*)sw_state_roots_array[unit][DNX_FIELD_GROUP_MODULE_ID])->fg_info[i0].context_info[i1].key_id.lsb[i2]);
...
const char* dnx_field_group_sw_layout_str[DNX_FIELD_GROUP_SW_INFO_NOF_ENTRIES] = {
    ...
    "DNX_FIELD_GROUP_SW~FG_INFO~CONTEXT_INFO~KEY_ID~LSB~",
