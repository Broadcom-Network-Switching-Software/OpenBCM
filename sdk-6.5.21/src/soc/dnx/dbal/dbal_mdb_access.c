...
static dbal_physical_table_def_t physical_table_mngr_mat[DBAL_NOF_ENUM_MDB_DB_TYPE_VALUES][DBAL_NOF_CORE_MODE_TYPES -
                                                                                           1] = {
    {
     {
    ...
    if (bsl_severity >= bslSeverityInfo)
    {
        if (dbal_logger_is_enable(unit, entry_handle->table_id))
        {
            ...
            LOG_CLI((BSL_META("Entry %s Physical Table: %s\n"), action,
                     dnx_data_mdb.pdbs.pdb_info_get(unit, physical_table_id)->name));
    ...
    int unit)
{
    ...
    for (table_idx = 0; table_idx < DBAL_NOF_PHYSICAL_TABLES; table_idx++)
    {
        ...
        if ((table_idx == DBAL_PHYSICAL_TABLE_TCAM) || (table_idx == DBAL_PHYSICAL_TABLE_KBP)
            || (dnx_data_mdb.pdbs.pdb_info_get(unit, table_idx)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_NONE))
        {
        ...
        SHR_IF_ERR_EXIT(dbal_physical_table_get(unit, table_idx, &physical_table));
        if (physical_table->table_actions->table_init)
        {
            SHR_IF_ERR_EXIT(physical_table->table_actions->table_init(unit));
        }
    }
    ...
    int unit)
{
    ...
    for (table_idx = 0; table_idx < DBAL_NOF_PHYSICAL_TABLES; table_idx++)
    {
        ...
        if (physical_table->table_actions->table_deinit)
        {
            SHR_IF_ERR_EXIT(physical_table->table_actions->table_deinit(unit));
        }
    }
    ...
    dbal_physical_table_def_t ** physical_table)
{
    ...
    dbal_core_mode_e core_mode_shifted;
    ...
    db_type = dnx_data_mdb.pdbs.pdb_info_get(unit, physical_table_id)->db_type;
    core_mode_shifted = dnx_data_mdb.pdbs.pdb_info_get(unit, physical_table_id)->core_mode - 1;

    *physical_table = &(physical_table_mngr_mat[db_type][core_mode_shifted]);
...
dbal_logical_table_is_mact(
    ...
    dbal_tables_e table_id)
{
    return (((table_id == DBAL_TABLE_FWD_MACT) || (table_id == DBAL_TABLE_FWD_MACT_IVL))
    ...
    else
    {
        ...
        if (PhysicalTable->table_actions->iterator_init == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "ERROR missing implementation for %s\n",
                         dnx_data_mdb.pdbs.pdb_info_get(unit, physical_db_id)->name);
        }
    ...
    for (ii = 0; ii < iterator_info->nof_val_rules; ii++)
    {
        if (iterator_info->val_rules_info[ii].field_id == DBAL_FIELD_RESULT_TYPE)
        {
            if (has_rt_rule == FALSE)
            {
                ...
                {
                    *nof_flush_rules = 1;
                    *result_types_in_flush_bitmap = (1 << ii);
                    mdb_iterator_info->result_types_in_non_flush_bitmap = 0;
                }
                else
                {
    ...
    dbal_iterator_info_t * iterator_info)
{
    ...
    uint32 result_types_in_flush_bitmap[1] = { 0 };
...
static shr_error_e
dbal_mdb_sbc_table_hitbit_get_over_physical_tables(
    int unit,
    dbal_logical_table_t * table,
    dbal_iterator_info_t * iterator_info,
    dbal_physical_entry_t * phy_entry,
    uint32 app_id)
{
        ...
        && (iterator_info->mdb_iterator.hit_bit_flags & DBAL_PHYSICAL_KEY_HITBIT_GET))
    {
        dbal_physical_table_def_t *PhysicalTable = NULL;
        while (!
               ((phy_entry->hitbit & DBAL_PHYSICAL_KEY_PRIMARY_AND_SECONDARY_HITBIT_ACCESSED) ==
               ...
               && (iterator_info->physical_db_index < (table->nof_physical_tables - 1)))
        {
            ...
            SHR_IF_ERR_EXIT(dbal_physical_table_get
                            (unit, table->physical_db_id[iterator_info->physical_db_index], &PhysicalTable));
            if (PhysicalTable->table_actions->entry_get == NULL)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "missing get implementation %s\n",
                             dnx_data_mdb.pdbs.pdb_info_get(unit,
                                                            table->physical_db_id[iterator_info->
                                                                                  physical_db_index])->name);
            }

            SHR_IF_ERR_EXIT(PhysicalTable->table_actions->entry_get(unit,
                                                                    table->physical_db_id
                                                                    [iterator_info->physical_db_index], app_id,
                                                                    phy_entry, 0));
        }
    }
    ...
    if (!iterator_info->is_end)
    {
        ...
        *continue_calling_mdb_get_next = FALSE;
            ...
            && (iterator_info->mdb_iterator.hit_bit_flags & DBAL_PHYSICAL_KEY_HITBIT_GET))
        {
            ...
            SHR_IF_ERR_EXIT(dbal_mdb_sbc_table_hitbit_get_over_physical_tables
                            (unit, table, iterator_info, phy_entry, app_id));
        }
    }
    ...
    uint8 is_eedb_ll)
{
...

    {
        ...
        if (skip_shadow && entry_handle->table->table_type != DBAL_TABLE_TYPE_LPM)
        {
    ...
    uint8 is_eedb_ll)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "missing implementation %s\n",
                     dnx_data_mdb.pdbs.pdb_info_get(unit, physical_db_ids[0])->name);
    }
