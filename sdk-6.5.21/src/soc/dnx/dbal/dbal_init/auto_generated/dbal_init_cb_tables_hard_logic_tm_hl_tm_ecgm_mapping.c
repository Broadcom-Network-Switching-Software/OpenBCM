...
uint32 * offset)
{
    ...
    *offset = key_value__is_mc*2*dnx_data_egr_queuing.params.first_lp_egr_interface_get(unit)+key_value__ecgm_priority*dnx_data_egr_queuing.params.first_lp_egr_interface_get(unit)+key_value__egq_if;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
...
uint32 * offset)
{
    ...
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_EGQ_IF, key_value__egq_if);
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_ECGM_PRIORITY, key_value__ecgm_priority);
    *offset = (key_value__egq_if+key_value__ecgm_priority*dnx_data_egr_queuing.params.first_lp_egr_interface_get(unit))%dnx_data_ecgm.info.fc_egr_if_vector_nof_presented_get(unit);
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
...
uint32 * offset)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_QPAIR, key_value__qpair);
    *offset = key_value__qpair%dnx_data_ecgm.info.fc_q_pair_vector_nof_presented_get(unit);
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
