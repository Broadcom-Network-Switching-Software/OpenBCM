...
void
soc_higig3_field_set(int unit, soc_higig3_hdr_t *hg3,
                      soc_higig_field_t field, uint32 val)
{
    COMPILER_REFERENCE(unit);
    ...
    case HG_start:
    case HG_ether_type:
        hg3->hg3_overlay.ethertype_high = (val >> 2) & 0xff;
        hg3->hg3_overlay.ethertype_low = (val) & 0x3;
        break;
    case HG_class_id:
        hg3->hg3_overlay.class_id_high = (val >> 8) & 0x3;
        hg3->hg3_overlay.class_id_low = (val) & 0xff;
        break;
    case HG_cn:
        hg3->hg3_overlay.cn = val;
        break;
    case HG_l3:
        hg3->hg3_overlay.l3_routed = val;
        break;
    case HG_hgi:
        hg3->hg3_overlay.ethertype_ver = val;
        break;
    case HG_mcst:
        hg3->hg3_overlay.system_dest_type = val;
        break;
    case HG_cos:
    case HG_tc:
        hg3->hg3_overlay.tc = val;
        break;
    /* mod[14:8] 7bits | port[0:7] 8 bits */
    /* system_dest_high 8bits | system_dest_low 7bits */
    case HG_dst_mod:
        hg3->hg3_overlay.system_dest_high = val;
        break;
    case HG_dst_port:
        hg3->hg3_overlay.system_dest_low = val;
        break;
    case HG_mgid:
        hg3->hg3_overlay.system_dest_high = val << 7;
        hg3->hg3_overlay.system_dest_low  = val & 0xff;
	    break;
    case HG_src_mod:
        hg3->hg3_overlay.system_source_high = val;
        break;
    case HG_src_port:
    case HG_tgid:
        hg3->hg3_overlay.system_source_low = val;
        break;
    case HG_lbid:
        hg3->hg3_overlay.entropy = val;
        break;
    case HG_cng:
    case HG_dp:
        hg3->hg3_overlay.cng = val;
        break;
    case HG_ehv:
        /* extension header version, only 2 is supported */
        assert(val == 2);
        hg3->hg3_overlay.ethertype_exthdr = val;
        break;
    case HG_mirror:
        hg3->hg3_overlay.ethertype_mirror_copy = val;
        break;
    case HG_multipoint:
    case HG_dst_t:
        hg3->hg3_overlay.system_dest_type = val;
    ...
    case HG_vni:
    case HG_vlan_tag: 
        hg3->hg3_overlay.forwarding_domain_high = (val >> 8) & 0xff;
        hg3->hg3_overlay.forwarding_domain_low  = (val) & 0xff;
        break;
    case HG_dst_vp:
    case HG_replication_id:
    case HG_l2mc_ptr:
        hg3->hg3_overlay.dvp_or_l3_iif_high = (val >> 8) & 0xff;
        hg3->hg3_overlay.dvp_or_l3_iif_low  = (val) & 0xff;
        break;
    case HG_src_vp:
        hg3->hg3_overlay.svp_high = (val >> 8) & 0xff;
        hg3->hg3_overlay.svp_low  = (val) & 0xff;
        break;
    case HG_sw_gen:
        /* Flags bit0 */
        hg3->hg3_overlay.flags = (hg3->hg3_overlay.flags & 0x3e) | (val & 0x1);
        break;
    case HG_donot_learn:
        /* Flags bit2 */
        hg3->hg3_overlay.flags = (hg3->hg3_overlay.flags & 0x3b) | ((val & 0x1) << 2);
        break;
    case HG_lag_failover:
        /* Flags bit1 */
        hg3->hg3_overlay.flags = (hg3->hg3_overlay.flags & 0x3d) | ((val & 0x1) << 1);
        break;
    case HG_opcode:
        /* Flags bit5:bit3 */
        hg3->hg3_overlay.flags = (hg3->hg3_overlay.flags & 0x7) | ((val & 0x7) << 3);
        break;
    case HG_fwd_type:
    case HG_donot_modify:
    case HG_protection_status:
    ...
    case HG_src_type:
    	LOG_WARN(BSL_LS_SOC_COMMON,
            (BSL_META_U(unit,
            "hg3_set: unit %d: Unsupported higig3 field=%d val=0x%x\n"),
            unit, field, val));
        break;
    case HG_ppd_type:
    	LOG_WARN(BSL_LS_SOC_COMMON,
            (BSL_META_U(unit,
            "hg3_set: unit %d: Not applicable field, it's for HG2 only, field=%d val=0x%x\n"),
            unit, field, val));
        break;
    case HG_hdr_format:
    case HG_ingress_tagged:
    ...
    default:
    	LOG_WARN(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "hg3_set: unit %d: Unknown higig3 field=%d val=0x%x\n"),
                      unit, field, val));
	    break;
    }
}
...
uint32
soc_higig3_field_get(int unit, soc_higig3_hdr_t *hg3,
                      soc_higig_field_t field)
{
    COMPILER_REFERENCE(unit);
    ...
    case HG_start:
    case HG_ether_type:
        return hg3->hg3_overlay.ethertype_high << 2 | hg3->hg3_overlay.ethertype_low;
    case HG_class_id:
        return hg3->hg3_overlay.class_id_high << 8 | hg3->hg3_overlay.class_id_low;
    case HG_cn:
        return hg3->hg3_overlay.cn;
    case HG_l3:
        return hg3->hg3_overlay.l3_routed;
    case HG_hgi:
        return hg3->hg3_overlay.ethertype_ver;
    case HG_mcst:
        return hg3->hg3_overlay.system_dest_type;
    case HG_cos:
    case HG_tc:
        return hg3->hg3_overlay.tc;
    /* mod[14:8] 7bits | port[0:7] 8 bits */
    /* system_dest_high 8bits | system_dest_low 7bits */
    case HG_dst_mod:
        return hg3->hg3_overlay.system_dest_high;
    case HG_dst_port:
        return hg3->hg3_overlay.system_dest_low;
    case HG_mgid:
        return hg3->hg3_overlay.system_dest_high << 7 | hg3->hg3_overlay.system_dest_low;
    case HG_src_mod:
        return hg3->hg3_overlay.system_source_high;
    case HG_src_port:
    case HG_tgid:
        return hg3->hg3_overlay.system_source_low;
    case HG_lbid:
        return hg3->hg3_overlay.entropy;
    case HG_cng:
    case HG_dp:
        return hg3->hg3_overlay.cng;
    case HG_ehv:
        /* extension header version, only 2 is supported */
        return hg3->hg3_overlay.ethertype_exthdr;
    case HG_mirror:
        return hg3->hg3_overlay.ethertype_mirror_copy;
    case HG_multipoint:
    case HG_dst_t:
        return hg3->hg3_overlay.system_dest_type;
    case HG_vni:
    case HG_vlan_tag: 
        return hg3->hg3_overlay.forwarding_domain_high << 8 | hg3->hg3_overlay.forwarding_domain_low;
    case HG_dst_vp:
    case HG_replication_id:
    case HG_l2mc_ptr:
        return hg3->hg3_overlay.dvp_or_l3_iif_high << 8 | hg3->hg3_overlay.dvp_or_l3_iif_low;
    case HG_src_vp:
        return hg3->hg3_overlay.svp_high << 8 | hg3->hg3_overlay.svp_low;
    case HG_sw_gen:
        /* Flags bit0 */
        return hg3->hg3_overlay.flags & 0x1;
    case HG_donot_learn:
        /* Flags bit2 */
        return (hg3->hg3_overlay.flags >> 2) & 0x1;
    case HG_lag_failover:
        /* Flags bit1 */
        return (hg3->hg3_overlay.flags >> 1) & 0x1;
    case HG_opcode:
        /* Flags bit5:bit3 */
        return (hg3->hg3_overlay.flags >> 3) & 0x7;
    case HG_fwd_type:
    case HG_donot_modify:
    case HG_protection_status:
    case HG_preserve_dscp:
    ...
    case HG_src_type:
    	LOG_WARN(BSL_LS_SOC_COMMON,
            (BSL_META_U(unit,
            "hg3_get: unit %d: Unsupported higig3 field=%d\n"),
            unit, field));
        return 0;
    case HG_ppd_type:
    	LOG_WARN(BSL_LS_SOC_COMMON,
            (BSL_META_U(unit,
            "hg3_get: unit %d: Not applicable field, it's for HG2 only, field=%d\n"),
            unit, field));
        return 0;
    case HG_hdr_format:
    case HG_ingress_tagged:
    ...
    default:
    	LOG_WARN(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "hg3_get: unit %d: Unknown higig3 field=%d\n"),
                      unit, field));
        return 0;
    }
}
...
void
soc_higig3_dump(int unit, char *pfx, soc_higig3_hdr_t *hg3)
{
    LOG_CLI((BSL_META_U(unit,
             "%s0x%02x%02x%02x%02x "
             "<ETHERTYPE=0x%x VER=%d EHV=%d MIRROR=%d\n"),
             pfx,
             hg3->overlay0.bytes[0],
             hg3->overlay0.bytes[1],
             hg3->overlay0.bytes[2],
             hg3->overlay0.bytes[3],
             soc_higig3_field_get(unit, hg3, HG_start),
             soc_higig3_field_get(unit, hg3, HG_hgi),
             soc_higig3_field_get(unit, hg3, HG_ehv),
             soc_higig3_field_get(unit, hg3, HG_mirror)));
    LOG_CLI((BSL_META_U(unit,
             "%s            MULTIPOINT|DST_T=%d DST_MOD=%d DST_PORT=%d>\n"),
             pfx,
             soc_higig3_field_get(unit, hg3, HG_dst_t),
             soc_higig3_field_get(unit, hg3, HG_dst_mod),
             soc_higig3_field_get(unit, hg3, HG_dst_port)));
    LOG_CLI((BSL_META_U(unit,
                        "%s0x%02x%02x%02x%02x "
             "<L3_ROUTED=%d SRC_MOD=%d SRC_PORT=%d\n"),
             pfx,
             hg3->overlay0.bytes[4],
             hg3->overlay0.bytes[5],
             hg3->overlay0.bytes[6],
             hg3->overlay0.bytes[7],
             soc_higig3_field_get(unit, hg3, HG_l3),
             soc_higig3_field_get(unit, hg3, HG_src_mod),
             soc_higig3_field_get(unit, hg3, HG_src_port)));
    LOG_CLI((BSL_META_U(unit,
                        "%s            ENTROPY|LBID=%d CNG=%d CN=%d TC=%d>\n"),
             pfx,
             soc_higig3_field_get(unit, hg3, HG_lbid),
             soc_higig3_field_get(unit, hg3, HG_cng),
             soc_higig3_field_get(unit, hg3, HG_cn),
             soc_higig3_field_get(unit, hg3, HG_tc)));

    LOG_CLI((BSL_META_U(unit,
                        "%s0x%02x%02x%02x%02x "
             "<OPCODE=%d DONOT_LEARN=%d LAG_FAILOVER=%d SW_GEN=%d\n"),
             pfx,
             hg3->overlay0.bytes[8],
             hg3->overlay0.bytes[9],
             hg3->overlay0.bytes[10],
             hg3->overlay0.bytes[11],
             soc_higig3_field_get(unit, hg3, HG_opcode),
             soc_higig3_field_get(unit, hg3, HG_donot_learn),
             soc_higig3_field_get(unit, hg3, HG_lag_failover),
             soc_higig3_field_get(unit, hg3, HG_sw_gen)));
    LOG_CLI((BSL_META_U(unit,
             "%s            CLASS_ID=%d FORWARDING_DOMAIN=%d>\n"),
             pfx,
             soc_higig3_field_get(unit, hg3, HG_class_id),
             soc_higig3_field_get(unit, hg3, HG_vni)));

    LOG_CLI((BSL_META_U(unit,
                        "%s0x%02x%02x%02x%02x "
             "<SRC_VP=0x%x DVP_OR_L3_IIF=0x%x>\n"),
             pfx,
             hg3->overlay0.bytes[12],
             hg3->overlay0.bytes[13],
             hg3->overlay0.bytes[14],
             hg3->overlay0.bytes[15],
             soc_higig3_field_get(unit, hg3, HG_src_vp),
             soc_higig3_field_get(unit, hg3, HG_dst_vp)));
}
