...
typedef struct _soc_th3_asf_ctrl_256_s {
    uint8                     init;
    pbmp_256_t                asf_ports;
    pbmp_256_t                pause_restore;
} _soc_th3_asf_ctrl_256_t;
...
/* Legacy ASF Warmboot */
typedef struct _soc_th3_asf_wb_256_s {
    int unit;
    _soc_th3_asf_ctrl_256_t asf_256_ctrl;
} _soc_th3_asf_wb_256_t;
    ...
    uint16            scache_ver)
{
    ...
    *mem_sz = 0;
    ...
    if (scache_ver <= BCM_WB_VERSION_1_13 ) {
        ...
        *mem_sz += sizeof(_soc_th3_asf_wb_256_t);
    } else {
        ...
        *mem_sz += sizeof(int);     /* _soc_th3_asf_wb_t.unit */
        *mem_sz += sizeof(uint8);   /* _soc_th3_asf_wb_t.asf_ctrl.init */
        *mem_sz += pbmp_size_bytes; /* _soc_th3_asf_wb_t.asf_ctrl.asf_ports */
        *mem_sz += pbmp_size_bytes; /* _soc_th3_asf_wb_t.asf_ctrl.pause_restore */
    }
    ...
    IN_OUT uint8* const wb_data)
{
    ...
    scache += sizeof(int);
    sal_memcpy(scache, &_soc_th3_asf_ctrl[unit]->init,
                    sizeof(_soc_th3_asf_ctrl[unit]->init));
    scache += sizeof(_soc_th3_asf_ctrl[unit]->init);
    sal_memcpy(scache, &_soc_th3_asf_ctrl[unit]->asf_ports,
                    sizeof(_soc_th3_asf_ctrl[unit]->asf_ports));
    scache += sizeof(_soc_th3_asf_ctrl[unit]->asf_ports);
    sal_memcpy(scache, &_soc_th3_asf_ctrl[unit]->pause_restore,
                    sizeof(_soc_th3_asf_ctrl[unit]->pause_restore));
    ...
    uint16       scache_ver)
{
    ...
    if (scache_ver <= BCM_WB_VERSION_1_13 ) {
        ...
        _soc_th3_asf_wb_256_t *asf_wb_256 = (_soc_th3_asf_wb_256_t *)wb_data;
        ...
        if (recovered_unit == unit) {
            ...
            sal_memcpy(&(_soc_th3_asf_ctrl[unit]->init),
                       scache, sizeof(_soc_th3_asf_ctrl[unit]->init));
            scache += sizeof(_soc_th3_asf_ctrl[unit]->init);
            sal_memcpy(&(_soc_th3_asf_ctrl[unit]->asf_ports), scache,
                       SOC_SCACHE_SIZE_MIN(pbmp_size_bytes, sizeof(pbmp_t)));
            scache += pbmp_size_bytes;
            sal_memcpy(&(_soc_th3_asf_ctrl[unit]->pause_restore), scache,
                       SOC_SCACHE_SIZE_MIN(pbmp_size_bytes, sizeof(pbmp_t)));
        }
    }
