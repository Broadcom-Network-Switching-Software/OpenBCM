...
/*! @fn void _soc_fb6_tdm_print_port_map(
        int unit,
 ...
 */
void
_soc_fb6_tdm_print_port_map(int unit, soc_port_map_type_t *port_map)
{
    int port;
    int is_hg2, is_ovs, is_mgm, is_coe;
    int speed, phy_port, idb_port, mmu_port, num_lanes, pblk;
    int phy_bmap, p2l_map, m2p_map;
    int lc_map, num_subports;

    for (port = 0; port < FB6_NUM_EXT_PORTS; port++) {
        is_mgm = SOC_PBMP_MEMBER(port_map->management_pbm, port) ? 1 : 0;
        if ((is_mgm == 1) || (port_map->log_port_speed[port] > 0)) {
            lc_map   = port_map->port_lc_mapping[port];
            num_subports = port_map->num_subports[port];
            is_coe    = SOC_PBMP_MEMBER(port_map->coe_port_pbm, port) ? 1 : 0;
            ...
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                "lc_map=%0d num_sub_ports=%0d log_port=%3d phy_port=%3d speed=%3dG "
                                "%s %s idb_port=%3d mmu_port=%3d "
                                "num_lanes=%1d pblk=%3d phy_bmap=%1d "
                                ),
                                 lc_map, num_subports, port, phy_port, speed,
                                (is_hg2==1) ? "HG2" : "ETH",
                                (is_ovs==1) ? "OVS" : " LR",
                                idb_port, mmu_port, num_lanes, pblk,
                                phy_bmap ));
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "%s p2l_map=%3d   m2p_map=%3d %s\n"),
                                (is_coe==1) ? "COE" : "NON_COE", p2l_map, m2p_map, (is_mgm==1) ? "MGM" : "" ));
        }
    }
...
/*! @fn void _soc_fb6_tdm_print_tdm_info(
        int unit,
 ...
 */
void
_soc_fb6_tdm_print_tdm_info(int unit, soc_tdm_schedule_t *tdm_info,
                            int hpipe, int is_ing)
{
    ...
    if (hpipe == 0) {
        for (cal_len = tdm_info->cal_len; cal_len > 0; cal_len--) {
            if (tdm_info->linerate_schedule[cal_len - 1] != FB6_NUM_EXT_PORTS) {
                break;
            }
        }
    ...
    for (j=0; j< tdm_info->num_ovs_groups; j++) {
        ...
        for (m=0; m< tdm_info->ovs_group_len; m++) {
            if (tdm_info->oversub_schedule[j][m] != FB6_NUM_EXT_PORTS) {
    ...
    if (is_ing == 1) {
        ...
        for (j=0; j<tdm_info->pkt_sch_or_ovs_space_len; j++) {
            ...
            if (tdm_info->pkt_sch_or_ovs_space[0][j] != FB6_NUM_EXT_PORTS) {
                LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "\t%d"),
...
/*! @fn void _soc_fb6_tdm_print_port_resource(
        int unit,
 ...
 */
void
_soc_fb6_tdm_print_port_resource(int unit,
                                 soc_port_resource_t *port_info,
                                 int entry_num)
{
    int is_hg2, lane;
...
/*! @fn void _soc_fb6_tdm_print_schedule_state(
        int unit,
 ...
 */
void
_soc_fb6_tdm_print_schedule_state(int unit,
                                  soc_port_schedule_state_t *sch_info)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                        "\nPRINTING soc_port_schedule_state_t"
                        "::[in_port_map]\n")));
    _soc_fb6_tdm_print_port_map(unit, &sch_info->in_port_map);

    if (sch_info->is_flexport == 1) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "\nPRINTING soc_port_schedule_state_t"
                            "::[out_port_map]\n")));
        _soc_fb6_tdm_print_port_map(unit, &sch_info->out_port_map);
    }
    ...
    if (print_tdm == 1) {
        for (pipe = 0; pipe< NUM_PIPE(unit); pipe++) {
            ...
            for (hpipe = 0; hpipe< MAX_SCH_SLICES; hpipe++) {
                ...
                _soc_fb6_tdm_print_tdm_info(unit, tdm_schedule, hpipe, 1);
            }
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
            ...
            for (hpipe = 0; hpipe< MAX_SCH_SLICES; hpipe++) {
                tdm_schedule = &(sch_info->tdm_egress_schedule_pipe[
                                 pipe].tdm_schedule_slice[hpipe]);
                _soc_fb6_tdm_print_tdm_info(unit, tdm_schedule, hpipe, 0);
            }
        }
    }
    ...
    if ((sch_info->is_flexport == 1) && (sch_info->nport > 0)) {
        ...
        for (i = 0; i < sch_info->nport; i++) {
            _soc_fb6_tdm_print_port_resource(unit, &sch_info->resource[i], i);
        }
    }
}


/*! @fn void soc_fb6_tdm_get_port_ratio(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int pm_num,
        int *pm_mode,
...
/*
 * #1   single: 100  -   x  -   x  -   x  SOC_FB6_PORT_RATIO_SINGLE
 * #2   single:  40  -   x  -   x  -   x  SOC_FB6_PORT_RATIO_TRI_023_2_1_1
 * #3   dual:    50  -   x  -  50  -   x  SOC_FB6_PORT_RATIO_DUAL_1_1
 * #4   dual:    40  -   x  -  40  -   x  SOC_FB6_PORT_RATIO_DUAL_1_1
 * #5   dual:    20  -   x  -  20  -   x  SOC_FB6_PORT_RATIO_DUAL_1_1
 * #6   dual:    40  -   x  -  20  -   x  SOC_FB6_PORT_RATIO_DUAL_2_1
 * #7   dual:    20  -   x  -  40  -   x  SOC_FB6_PORT_RATIO_DUAL_1_2
 * #8   tri:     50  -   x  - 25/x - 25/x SOC_FB6_PORT_RATIO_TRI_023_2_1_1
 * #9   tri:     20  -   x  - 10/x - 10/x SOC_FB6_PORT_RATIO_TRI_023_2_1_1
 * #10  tri:     40  -   x  - 10/x - 10/x SOC_FB6_PORT_RATIO_TRI_023_4_1_1
 * #11  tri:    25/x - 25/x -  50  -   x  SOC_FB6_PORT_RATIO_TRI_012_1_1_2
 * #12  tri:    10/x - 10/x -  20  -   x  SOC_FB6_PORT_RATIO_TRI_012_1_1_2
 * #13  tri:    10/x - 10/x -  40  -   x  SOC_FB6_PORT_RATIO_TRI_012_1_1_4
 * #14  quad:   25/x - 25/x - 25/x - 25/x SOC_FB6_PORT_RATIO_QUAD
 * #15  quad:   10/x - 10/x - 10/x - 10/x SOC_FB6_PORT_RATIO_QUAD
 */
void
soc_fb6_tdm_get_port_ratio(int unit,
                            soc_port_schedule_state_t *sch_info,
                            int pm_num, int *pm_mode, int prev_or_new)
{
    int port, phy_port_base, lane, mode = 0;
    int num_lanes[FIREBOLT6_TDM_PORTS_PER_PBLK];
    int speed_max[FIREBOLT6_TDM_PORTS_PER_PBLK];
    soc_port_map_type_t *port_map;

    sal_memset(num_lanes, 0, FIREBOLT6_TDM_PORTS_PER_PBLK*sizeof(int));
    sal_memset(speed_max, 0, FIREBOLT6_TDM_PORTS_PER_PBLK*sizeof(int));
    ...
    phy_port_base = 1 + (pm_num * FIREBOLT6_TDM_PORTS_PER_PBLK);
    /*phy_port_base = _soc_fb6_tdm_get_phy_port_base(pm_num);*/
    for (lane = 0; lane < FIREBOLT6_TDM_PORTS_PER_PBLK; lane += 2) {
    ...
    if (num_lanes[0] == 4) {
        mode = SOC_FB6_PORT_RATIO_SINGLE;
    } else if (num_lanes[0] == 2 && num_lanes[2] == 2) {
        if (speed_max[0] == speed_max[2]) {
            mode = SOC_FB6_PORT_RATIO_DUAL_1_1;
        } else if (speed_max[0] > speed_max[2]) {
            mode = SOC_FB6_PORT_RATIO_DUAL_2_1;
        } else {
            mode = SOC_FB6_PORT_RATIO_DUAL_1_2;
        }
...
/*! @fn void _soc_fb6_tdm_get_pipe_map(
        int unit,
        soc_port_schedule_state_t *sch_info,
        uint32 *pipe_map)
    @param unit Chip unit number.
 ...
 */
void
_soc_fb6_tdm_get_pipe_map(int unit,
                          soc_port_schedule_state_t *sch_info,
                          uint32 *pipe_map)
{
    ...
    if (sch_info->is_flexport == 0) { /* Init */
        for (port = 0; port < FIREBOLT6_TDM_PHY_PORTS_PER_DEV; port++) {
            if (sch_info->out_port_map.log_port_speed[port] > 0 ) {
                pipe = port / FIREBOLT6_TDM_PHY_PORTS_PER_PIPE;
                pipe_bmap |= 1 << pipe;
            }
        }
    } else { /* FlexPort */
    ...
    *pipe_map = pipe_bmap;
    /*printf(" check on pipe bit map %d, FIREBOLT6_TDM_PHY_PORTS_PER_PIPE %d \n ",pipe_bmap,FIREBOLT6_TDM_PHY_PORTS_PER_PIPE);*/
}

/*! @fn int _soc_fb6_tdm_get_mport_2_hp_num(
        uint32 phy_num
        )
    @param phy_num
 ...
 */
int
 _soc_fb6_tdm_get_mport_2_hp_num(uint32 mmu_port_num
                          )
{
    if(mmu_port_num<32)
      return(mmu_port_num);
    else if (mmu_port_num >=32 && mmu_port_num<=63)
...
/*! @fn int _soc_fb6_tdm_get_cal_id(
        uint32 phy_num
        )
    @param phy_num
    @brief Returns PM ID.Needed correction as 76/4 results 19 pm instead 10
...
/*
int
_soc_fb6_tdm_get_cal_id(uint32 phy_num
                          )
{

    if (phy_num <= 48)
        return((phy_num/16));
    else if(phy_num <= FIREBOLT6_TDM_PHY_PORTS_PER_PIPE) {
        return((phy_num/4)-9);
        }
    else
...
*/
/*! @fn int _soc_fb6_tdm_get_phy_port_base (
        uint32 pm_num
        )
    @param phy_num
    @brief Returns PM ID.Needed correction as 76/4 results 19 pm instead 10
 ...
 */
int
_soc_fb6_tdm_get_phy_port_base (uint32 pm_num)
{
    if (pm_num < 3)
        return((pm_num*16)+1);
...
/*! @fn void _soc_fb6_tdm_get_ovs_pipe_map(
        int unit,
        soc_port_schedule_state_t *sch_info,
        uint32 *ovs_pipe_map)
    @param unit Chip unit number.
 ...
 */
void
_soc_fb6_tdm_get_ovs_pipe_map(int unit,
                              soc_port_schedule_state_t *sch_info,
                              uint32 *ovs_pipe_map)
{
...
/*! @fn int _soc_fb6_tdm_get_speed_ovs_class(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
int
_soc_fb6_tdm_get_speed_ovs_class(int unit, int speed)
{
    int speed_class = 0;
   /*uint16 dev_id = 0;*/
   /*uint8  rev_id = 0;*/
    ...
    if (speed >= 40000) {
        ...
        if (speed >= 25000) {
            ...
            /* take 1G speed as 10G in FB6 for ovsb port */
            speed_class = 1;
        }
    }
...
/* if (((dev_id & TDM_FB6_DEV_ID_MASK) ==*/
/*     (TDM_FB6_DEV_ID_BCM56370 & TDM_FB6_DEV_ID_MASK)) &&*/
/*     (speed_class == 1)) {*/
/*     speed_class = 3;*/
/* }*/
/**/
...
/*! @fn int _soc_fb6_tdm_check_is_hsp_port_e(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
STATIC int
_soc_fb6_tdm_check_is_hsp_port_e(int unit, int speed)
{
    int speed_class, port_is_hsp = 0;

    speed_class = _soc_fb6_tdm_get_speed_ovs_class(unit, speed);
    switch (speed_class) {
        case 4: /* 40G */
        case 5: /* 50G */
        case 6: /* 100G */
...
/*! @fn int _soc_fb6_tdm_check_mgmt_enable(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit   Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
 ...
 */
/*STATIC int
_soc_fb6_tdm_check_mgmt_enable(int unit,
                               soc_port_schedule_state_t *sch_info)
{
    int port;
    int mgmt_port_cnt = 0;
...
/*! @fn int _soc_fb6_tdm_check_2nd_mgmt_enable(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit   Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
...
/*
STATIC int
_soc_fb6_tdm_check_2nd_mgmt_enable(int unit,
                                   soc_port_schedule_state_t *sch_info)
{
    int port;
    int mgmt_port_cnt = 0;
...
/*! @fn int _soc_fb6_tdm_get_same_port_spacing_e(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
int
_soc_fb6_tdm_get_same_port_spacing_e(int unit, int speed)
{
    int port_same_spacing = 0;

    /*if (_soc_fb6_tdm_check_is_hsp_port_e(unit, speed) == 1) {*/
        port_same_spacing = 6;
    /*} else {
...
/*! @fn int _soc_fb6_tdm_get_sis_port_spacing_e(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
int
_soc_fb6_tdm_get_sis_port_spacing_e(int unit, int phy_port){
...
/*! @fn int _soc_fb6_tdm_calculation(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
 ...
 */
int
_soc_fb6_tdm_calculation(int unit, soc_port_schedule_state_t *sch_info)
{
    int i, j, pm_num, phy_port, lgc_port, port_lanes, phy_port_tmp;
    int pipe, hpipe;
    int lane, group, index;
    int mgmt_pm_hg = 0, mgmt_mode[FIREBOLT6_TDM_PHY_PORTS_PER_DEV];
    ...
    tdm_soc_t _chip_pkg;
    tdm_mod_t *_tdm_pkg;
    uint32 port_speeds[FIREBOLT6_TDM_PHY_PORTS_PER_DEV];
    uint32 port_states[FIREBOLT6_TDM_PHY_PORTS_PER_DEV];


  /*  soc_cm_get_id(unit, &dev_id, &rev_id); */
  /*  LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "fb6_tdm_dev_id %0d\n"), dev_id)); */

    sal_memset(port_speeds, 0, FIREBOLT6_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));
    sal_memset(port_states, 0, FIREBOLT6_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));


    /* Remap input speeds to ETH bitrates */
    soc_firebolt6_port_schedule_speed_remap(unit, sch_info);

    for(i=0; i<FIREBOLT6_TDM_PHY_PORTS_PER_DEV; i++) {
      mgmt_mode[i] = 0;
    }
    for (lgc_port = 0; lgc_port < FIREBOLT6_TDM_PHY_PORTS_PER_DEV; lgc_port++) {
        phy_port = sch_info->in_port_map.port_l2p_mapping[lgc_port];
        if (SOC_PBMP_MEMBER(sch_info->in_port_map.management_pbm, lgc_port)) {
           mgmt_mode[(phy_port-1)] = 1;
        }
        else {
           mgmt_mode[(phy_port-1)] = 0;
        }
    }
    /* Set port states and speeds */
    for (lgc_port = 0; lgc_port < FIREBOLT6_TDM_PHY_PORTS_PER_DEV; lgc_port++) {
        phy_port = sch_info->in_port_map.port_l2p_mapping[lgc_port];
        if (phy_port < FIREBOLT6_TDM_PHY_PORTS_PER_DEV) {
            if (sch_info->in_port_map.log_port_speed[lgc_port] > 0 ) {
                ...
                for (lane = 1; lane < port_lanes; lane++) {
                    ...
                    if (phy_port_tmp < FIREBOLT6_TDM_PHY_PORTS_PER_DEV) {
    ...
    /*if (_soc_fb6_tdm_check_mgmt_enable(unit, sch_info) == 1) {
    ...
    if (LOG_CHECK(BSL_LS_SOC_TDM | BSL_INFO)) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "%8s%8s%8s\n"),
                            "port", "speed", "state"));
        for (phy_port = 0; phy_port < FIREBOLT6_TDM_PHY_PORTS_PER_DEV;
             phy_port++) {
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                "%8d%8d%8d\n"),
   ...
   /* _chip_pkg.dev_id        = dev_id; */
    _chip_pkg.num_ext_ports = FB6_NUM_EXT_PORTS;
    ...
    for (index = 0; index < _chip_pkg.num_ext_ports &&
                    index < FIREBOLT6_TDM_PHY_PORTS_PER_DEV; index++) {
    ...
    _chip_pkg.soc_vars.fb6.mgmt_pm_hg= mgmt_pm_hg;
    for (index = 0; index < _chip_pkg.num_ext_ports &&
                    index < FIREBOLT6_TDM_PHY_PORTS_PER_DEV; index++) {
      _chip_pkg.soc_vars.fb6.mgmt_mode[index] = mgmt_mode[index];
    }
    ...
    if (_tdm_pkg == NULL) {
        ...
        for (pipe = 0; pipe < FIREBOLT6_TDM_PIPES_PER_DEV; pipe++) {
            ...
            if (tdm_pipe_ovs_group == NULL ||
                ...
                for (i = 0; i < tdm_pipe_grp_num; i++) {
                    ...
                    hpipe = i / FIREBOLT6_TDM_OVS_GROUPS_PER_HPIPE;
                    group = i % FIREBOLT6_TDM_OVS_GROUPS_PER_HPIPE;
                ...
                for (hpipe = 0; hpipe < 2; hpipe++) {
                    for (j = 0; j < FB6_SHAPING_GRP_LEN; j++) {
                        ...
                        tdm_pipe_ovs_group[hpipe+FB6_SHAPING_GRP_IDX_0][j];
                    }
                }
            }
        }
        ...
        for (pm_num = 0; pm_num < FB6_NUM_PHY_PM; pm_num++) {
            for (lane = 0; lane < FIREBOLT6_TDM_PORTS_PER_PBLK; lane++) {
                sch_info->out_port_map.port_p2PBLK_inst_mapping[
                (pm_num * FIREBOLT6_TDM_PORTS_PER_PBLK) + 1 + lane] =
                _tdm_pkg->_chip_data.soc_pkg.soc_vars.fb6.pm_num_to_pblk[pm_num];
            }
        }
        /* Free the memory allocated in TDM algorithm code */
        tdm_fb6_main_free(_tdm_pkg); /* This function frees all pointers allocated during _soc_set_tdm_tbl call */
        sal_free(_tdm_pkg);
    }
    ...
    if (LOG_CHECK(BSL_LS_SOC_TDM | BSL_INFO)) {
        for (pipe = 0; pipe < FIREBOLT6_TDM_PIPES_PER_DEV; pipe++) {
            ...
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "\n")));
            /* Oversub groups */
            for (hpipe = 0; hpipe < FIREBOLT6_TDM_HPIPES_PER_PIPE; hpipe++) {
                LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "Pipe %d Half-Pipe %d [IDB TDM Oversub Groups]\n"),
                            pipe, hpipe));
                for (group = 0; group < FIREBOLT6_TDM_OVS_GROUPS_PER_HPIPE;
                     group++) {
                    LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                "oversub group %d: "), group));
                    for (index = 0; index < FIREBOLT6_TDM_OVS_GROUP_LENGTH;
                         index++) {
                        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, " %3d"),
                                    sch_info->tdm_ingress_schedule_pipe[pipe].
            ...
            for (hpipe = 0; hpipe < FIREBOLT6_TDM_HPIPES_PER_PIPE; hpipe++) {
                LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            ...
                            pipe, hpipe));
                for (index = 0; index < FB6_SHAPING_GRP_LEN; index++) {
                    LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, " %3d"),
                    sch_info->tdm_ingress_schedule_pipe[pipe].
    ...
    if (sch_info->is_flexport == 0) {
        for (i= 0; i< SOC_MAX_NUM_PORTS;i++) {
            ...
            sch_info->out_port_map.port_lc_mapping[i] =
                sch_info->in_port_map.port_lc_mapping[i];
            sch_info->out_port_map.num_subports[i] =
                sch_info->in_port_map.num_subports[i];
        }
        ...
        for (i = 0; i < _SHR_PBMP_WORD_MAX; i++) {
            ...
            sch_info->out_port_map.coe_port_pbm.pbits[i]    =
                sch_info->in_port_map.coe_port_pbm.pbits[i];
        }
    }
    /*
     * else {
    ...
    */
    ...
    _soc_fb6_tdm_print_schedule_state(unit, sch_info);
    return SOC_E_NONE;
}


/*! @fn int _soc_fb6_tdm_set_idb_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the FB6 IDB main calendar.
 */
int
_soc_fb6_tdm_set_idb_calendar(int unit, soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    uint32 cal_len;
    soc_mem_t mem;
    ...
    uint32 mem_indx;
    static const soc_mem_t idb_tdm_cal_mem[FIREBOLT6_TDM_PIPES_PER_DEV][2] = {
        {IS_TDM_CALENDAR0_PIPE0m, IS_TDM_CALENDAR1_PIPE0m}/* ,
        {IS_TDM_CALENDAR0_PIPE1m, IS_TDM_CALENDAR1_PIPE1m} */
    };
    static const soc_reg_t idb_tdm_cfg_reg[FIREBOLT6_TDM_PIPES_PER_DEV] = {
    ...
    _soc_fb6_tdm_get_pipe_map(unit, sch_info, &pipe_map);
    ...
    for (pipe = 0; pipe < FIREBOLT6_TDM_PIPES_PER_DEV; pipe++) {
        ...
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == FB6_OVSB_TOKEN) {
                idb_port = FIREBOLT6_TDM_IDB_OVSB_TOKEN;
            } else if (phy_port == FB6_IDL1_TOKEN) {
                idb_port = FIREBOLT6_TDM_IDB_SBUS_TOKEN;
            } else if (phy_port == FB6_IDL2_TOKEN) {
                idb_port = FIREBOLT6_TDM_IDB_OPT2_TOKEN;
            } else if (phy_port == FB6_NULL_TOKEN) {
                idb_port = FIREBOLT6_TDM_IDB_NULL_TOKEN;
            } else if (phy_port == FB6_FAE_TOKEN) {
                idb_port = FIREBOLT6_TDM_IDB_FAE_TOKEN;
            } else if (phy_port >= FB6_NUM_EXT_PORTS) {
                idb_port = FIREBOLT6_TDM_IDB_UNUSED_TOKEN;
            }  else if (phy_port == FB6_MGM1_TOKEN) {
                /* TDM code always allocates slots for management port.
                 * If mgmt ports are omitted from config, this mapping
                 * is not set up.
                 */
                idb_port = FIREBOLT6_TDM_IDB_MGMT1_TOKEN;
            }  else if (phy_port == FB6_MGM2_TOKEN) {
                 idb_port = FIREBOLT6_TDM_IDB_MGMT2_TOKEN;
            }  else if (phy_port == FB6_MGM3_TOKEN) {
                 idb_port = FIREBOLT6_TDM_IDB_MGMT3_TOKEN;
            }  else if (phy_port == FB6_MGM4_TOKEN) {
                 idb_port = FIREBOLT6_TDM_IDB_MGMT4_TOKEN;
            }
	    else {
                port = sch_info->out_port_map.port_p2l_mapping[phy_port];
                idb_port = sch_info->out_port_map.port_l2i_mapping[port] & 0x7f;
            }
            if (idb_port > 67) { /* Not a general front panel port */
                id = 0xf;
            } else {
                id = (phy_port - 1) / FIREBOLT6_TDM_PORTS_PER_PBLK;
	        /*id = _soc_fb6_tdm_get_cal_id((phy_port-1));*/
            }
	    /*printf(" even phy_port %d , idb_port %d\n ",phy_port,idb_port);*/
	    ...
	    /*printf(" odd phy_port %d , idb_port %d\n ",phy_port,idb_port);*/
            memfld = idb_port & 0x7f; /* [0,69] && 127 */
   ...
   *      if (_soc_fb6_tdm_check_2nd_mgmt_enable(unit, sch_info) == 1 &&
   *         pipe == (FIREBOLT6_TDM_PIPES_PER_DEV - 1)) {
...
/*! @fn int _soc_fb6_tdm_set_idb_hsp(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the FB6 IDB HSP.
 */
int
_soc_fb6_tdm_set_idb_hsp(int unit, soc_port_schedule_state_t *sch_info)
{
    soc_reg_t reg;
    uint32 pipe_map;
    ...
    for (port=0; port< 70 ; port++) { /* mmu logical ports */
        ...
        if (port_speed > 0) {
            if (_soc_fb6_tdm_check_is_hsp_port_e(unit, port_speed) == 1) {
                phy_port = sch_info->out_port_map.port_l2p_mapping[port];
                if ((phy_port >= 1) &&
                    (phy_port <= FIREBOLT6_TDM_GPORTS_PER_DEV)) {
...
/*! @fn int _soc_fb6_tdm_set_idb_ppe_credit(
        int unit,
    ...
    @brief API to initialize the FB6 IDB PPE Credit Config register.
           Regs/Mems configured:PPE_CREDIT_CONFIG_PIPE<pipe>
 */
int
_soc_fb6_tdm_set_idb_ppe_credit(int unit,
                                soc_port_schedule_state_t *sch_info,
                                int full_credit_threshold_0,
                                int opp_credit_threshold_0,
                                int full_credit_threshold_1,
                                int opp_credit_threshold_1 )
{
    ...
    int pipe;
    uint32 rval = 0;
    static const soc_reg_t idb_ppe_credit[FIREBOLT6_TDM_PIPES_PER_DEV] = {
...
/*! @fn int _soc_fb6_tdm_set_idb_opportunistic(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int cpu_op_en,
        int lb_opp_en,
    ...
    @brief API to initialize the FB6 IDB Opportunistic related registers
           Regs/Mems configured: IS_CPU_LB_OPP_CFG_PIPE<pipe>
                                 IS_OPP_SCHED_CFG_PIPE<pipe>
 */
int
_soc_fb6_tdm_set_idb_opportunistic(int unit,
                                   soc_port_schedule_state_t *sch_info,
                                   int cpu_op_en,
                                   int lb_opp_en,
                                   int opp1_port_en,
                                   ...
                                   int opp_ovr_sub_en)
{
    ...
    int pipe;
    uint32 rval = 0;
    static const soc_reg_t idb_cpu_lb_opp_cfg[FIREBOLT6_TDM_PIPES_PER_DEV] = {
        IS_CPU_LB_OPP_CFG_PIPE0r/* , IS_CPU_LB_OPP_CFG_PIPE1r */
    };
    static const soc_reg_t idb_opp_sched_cfg[FIREBOLT6_TDM_PIPES_PER_DEV] = {
...
/*! @fn int soc_fb6_tdm_set_idb_dpp_ctrl(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int credits)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @param credits
    @brief API to initialize the FB6 IDB_DPP_CTRL register.
 */
int
soc_fb6_tdm_set_idb_dpp_ctrl(int unit,
                             soc_port_schedule_state_t *sch_info,
                             int credits)
{
    soc_reg_t reg;
    ...
    int pipe;
    uint32 rval = 0;
    static const soc_reg_t idb_dpp_ctrl[FIREBOLT6_TDM_PIPES_PER_DEV] = {
...
/*! @fn int _soc_fb6_tdm_set_idb_pkt_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the FB6 IDB Packet Shaper calendar.
           Regs/Mems configured: IDB PKT_SCH_CALENDAR
 */
int
_soc_fb6_tdm_set_idb_pkt_calendar(int unit,
                                  soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    soc_mem_t mem;
    ...
    uint32 memfld;
    uint32 entry[SOC_MAX_MEM_WORDS];
    static const soc_mem_t idb_pkt_shaper_mems[FIREBOLT6_TDM_PIPES_PER_DEV][2] = {
    ...
    for (pipe = 0; pipe < FIREBOLT6_TDM_PIPES_PER_DEV; pipe++) {
        ...
        for (hpipe = 0; hpipe < FIREBOLT6_TDM_HPIPES_PER_PIPE; hpipe++) {
            ...
            for (slot = 0; slot < FB6_SHAPING_GRP_LEN; slot ++) {
                phy_port = sch_info->tdm_ingress_schedule_pipe[pipe].
                tdm_schedule_slice[hpipe].pkt_sch_or_ovs_space[0][slot];
                if ((phy_port >= FB6_NUM_EXT_PORTS) || (phy_port <= 0)) {
                    idb_port = FIREBOLT6_TDM_IDB_UNUSED_TOKEN;
                } else {
...
/*! @fn int _soc_fb6_tdm_set_ovs_group_sel(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int set_idb,
        int set_mmu)
    ...
    @brief API to initialize the FB6 MMU &/or IDB OVS tables.
            Regs/Mems configured: IDB HPIPE0/1_OVR_SUB_GRP_CFG,
                                  HPIPE0/1_OVR_SUB_GRP_CFG
                                  PBLK<n>_CALENDAR
 */
int
_soc_fb6_tdm_set_ovs_group_sel(int unit,
                               soc_port_schedule_state_t *sch_info,
                               int set_idb, int set_mmu)
{
    ...
    int pblks[FIREBOLT6_TDM_PIPES_PER_DEV][2][FIREBOLT6_TDM_PBLKS_PER_HPIPE];
    int pm_num, pblk_cal_idx;
    int pblk_valid, pblk_spacing;
    int port_speed_class, port_sister_spacing;

    static const soc_reg_t idb_grp_cfg_regs[FIREBOLT6_TDM_PIPES_PER_DEV][2] = {
    ...
    static const soc_reg_t idb_grp_ovs_regs[FIREBOLT6_TDM_PIPES_PER_DEV][2][6] = {
        {
    ...
    static const soc_reg_t idb_pblk_cal_regs[FIREBOLT6_TDM_PIPES_PER_DEV][2][10] = {
        {
    ...
    static int pblk_slots[SOC_FB6_PORT_RATIO_COUNT][7] = {
        { 0, -1,  0,  0, -1,  0, -1 }, /* SOC_FB6_PORT_RATIO_SINGLE */
        { 0, -1,  2,  0, -1,  2, -1 }, /* SOC_FB6_PORT_RATIO_DUAL_1_1 */
        { 0,  0,  2,  0,  0,  2, -1 }, /* SOC_FB6_PORT_RATIO_DUAL_2_1 */
        { 0,  2,  2,  0,  2,  2, -1 }, /* SOC_FB6_PORT_RATIO_DUAL_1_2 */
        { 0, -1,  2,  0, -1,  3, -1 }, /* SOC_FB6_PORT_RATIO_TRI_023_2_1_1 */
        { 0,  0,  2,  0,  0,  3, -1 }, /* SOC_FB6_PORT_RATIO_TRI_023_4_1_1 */
        { 0, -1,  2,  1, -1,  2, -1 }, /* SOC_FB6_PORT_RATIO_TRI_012_1_1_2 */
        { 0,  2,  2,  1,  2,  2, -1 }, /* SOC_FB6_PORT_RATIO_TRI_012_1_1_4 */
        { 0, -1,  2,  1, -1,  3, -1 }  /* SOC_FB6_PORT_RATIO_QUAD */
    };

    _soc_fb6_tdm_get_pipe_map(unit, sch_info, &pipe_map);
    _soc_fb6_tdm_get_ovs_pipe_map(unit, sch_info, &ovs_pipe_map);
        ...
        for (hpipe = 0; hpipe < FIREBOLT6_TDM_HPIPES_PER_PIPE; hpipe++) {
            for (group = 0; group < FIREBOLT6_TDM_OVS_GROUPS_PER_HPIPE; group++) {
                ...
                for (slot = 0; slot < FIREBOLT6_TDM_OVS_GROUP_LENGTH; slot++) {
                     ...
                     */
                    if (phy_port > FB6_NUM_PHY_PORTS+4) {
                        idb_port = 0x3f;
                        mmu_port = 0x3f;
                        id = 0x7; /* 0x7 */
                        /*printf("port_map check phy ports  greater than total ports phy_port %d\n", phy_port);*/
                    } else {
                        port = sch_info->in_port_map.port_p2l_mapping[phy_port];
                        idb_port = sch_info->in_port_map.port_l2i_mapping[
                                   port] & 0x7f;
			idb_port = _soc_fb6_tdm_get_mport_2_hp_num(idb_port) ;
                        mmu_port = sch_info->in_port_map.port_p2m_mapping[
                                   phy_port] & 0x7f ;
			mmu_port = _soc_fb6_tdm_get_mport_2_hp_num(mmu_port) ;
                        id = sch_info->out_port_map.port_p2PBLK_inst_mapping[
                            phy_port] & 0xf;
	        	 /* id = _soc_fb6_tdm_get_cal_id((phy_port-1)) & 0xf; */
        		/*printf(" port_map check l_port %d phy_port %d idb_port %d mmu_port %d id %d  pipe %d group %d \n ",port,phy_port,idb_port,mmu_port,id,hpipe,group );*/
                    ...
                    if (set_idb == 1) {
        		     ...
        		     /*printf(" IDB OVSB : port_map check l_port %d phy_port %d idb_port %d mmu_port %d id %d  pipe %d group %d  ",port,phy_port,idb_port,mmu_port,id,hpipe,group );*/
                        soc_reg_field_set(unit, reg, &rval, PHY_PORT_IDf, id);
        		/*printf(" OVSB reg value group %d \n ",rval);*/
                    ...
                    if (set_mmu == 1) {
        		     ...
        		     /*printf(" MMU OVSB : port_map check l_port %d phy_port %d idb_port %d mmu_port %d id %d  pipe %d group %d ",port,phy_port,idb_port,mmu_port,id,hpipe,group );*/
                        soc_reg_field_set(unit, reg, &rval, PHY_PORT_IDf, id);
        		/*printf(" OVSB reg value group %d \n ",rval);*/
                ...
                for (slot = 0; slot < FIREBOLT6_TDM_OVS_GROUP_LENGTH; slot++) {
                    phy_port = sch_info->tdm_ingress_schedule_pipe[pipe].
                                         tdm_schedule_slice[hpipe].
                                         oversub_schedule[group][slot];
                    if (phy_port < FB6_NUM_EXT_PORTS) {
                        break;
                    }
                }

                /* No valid ports found, skip group */
                if (FIREBOLT6_TDM_OVS_GROUP_LENGTH == slot) {
                    port_speed_class = 0;
                   /* port_same_spacing = 0; */
                    ...
                    speed_max = 25000 * sch_info->in_port_map.port_num_lanes[port];
        		        /*  printf(" Speed_max %d \n Port_num_lanes %d\n port_speed %d\n port %d\n",speed_max,sch_info->in_port_map.port_num_lanes[port],sch_info->in_port_map.log_port_speed[port],port); */
                    if (speed_max > sch_info->in_port_map.log_port_speed[port]) {
                        speed_max = sch_info->in_port_map.log_port_speed[port];
                    }
                    port_speed_class =
                        _soc_fb6_tdm_get_speed_ovs_class(unit, speed_max);
        		     /*printf(" MMU OVSB : port_map speed check phy_port %d speed %d speed_class %d \n ",port,speed_max,port_speed_class );*/
                   /* port_same_spacing =
                    *    _soc_fb6_tdm_get_same_port_spacing_e(unit, speed_max);
										*/
                    port_sister_spacing =
                        _soc_fb6_tdm_get_sis_port_spacing_e(unit, port);
                }
                if (set_idb == 1) {
                    rval = 0;
    ...
    sal_memset(pblks, -1, sizeof(int) * FIREBOLT6_TDM_PIPES_PER_DEV * 2 *
               FIREBOLT6_TDM_PBLKS_PER_HPIPE);
    for (pipe = 0; pipe < FIREBOLT6_TDM_PIPES_PER_DEV; pipe++) {
        if (!(ovs_pipe_map & (1 << pipe))) {
            continue;
        }
            ...
            for (group = 0; group < FIREBOLT6_TDM_OVS_GROUPS_PER_HPIPE; group++) {
                for (slot = 0; slot < FIREBOLT6_TDM_OVS_GROUP_LENGTH; slot++) {
                    phy_port = sch_info->tdm_ingress_schedule_pipe[pipe].
                                         tdm_schedule_slice[hpipe].
                                         oversub_schedule[group][slot];
        		     /*printf(" MMU OVSB : port_map check hpipe %d group %d slot %d phy_port %0d\n ",hpipe,group,slot,phy_port );*/
                    if (phy_port < (FB6_NUM_EXT_PORTS-3)) {
                       pm_num = (phy_port - 1) / FIREBOLT6_TDM_PORTS_PER_PBLK;
                       /*printf(" MMU OVSB pm_num %d phy_port %d FB6_NUM_EXT_PORTS %d \n", pm_num, phy_port, FB6_NUM_EXT_PORTS);*/
		           /*pm_num = _soc_fb6_tdm_get_cal_id((phy_port - 1)); */
                        pblk_cal_idx = sch_info->out_port_map.
                                       port_p2PBLK_inst_mapping[phy_port];
        		          /*printf(" pblk_cal_idx %d \n ",pblk_cal_idx);*/
                        if (pblk_cal_idx >= 0 &&
                            pblk_cal_idx < FIREBOLT6_TDM_PBLKS_PER_HPIPE) {
    ...
    for (pipe = 0; pipe < FIREBOLT6_TDM_PIPES_PER_DEV; pipe++) {
        ...
        inst = pipe | SOC_REG_ADDR_INSTANCE_MASK;
        for (hpipe = 0; hpipe < FIREBOLT6_TDM_HPIPES_PER_PIPE; hpipe++) {
            for (pblk_cal_idx = 0; pblk_cal_idx < FIREBOLT6_TDM_PBLKS_PER_HPIPE;
                    pblk_cal_idx++) {
                pm_num = pblks[pipe][hpipe][pblk_cal_idx];
                phy_port_base = 1 + (pm_num * FIREBOLT6_TDM_PORTS_PER_PBLK);
		/*phy_port_base = _soc_fb6_tdm_get_phy_port_base(pm_num); */
                /* Get port ratio for this Port Macro */
                soc_fb6_tdm_get_port_ratio(unit, sch_info, pm_num, &mode, 1);
                reg  = idb_pblk_cal_regs[pipe][hpipe][pblk_cal_idx];
                reg1 = mmu_pblk_cal_regs[hpipe][pblk_cal_idx];
        		      /*printf(" Config PBlk_cal_id :%d\n hpipe %d\n pm_num:%d\n",pblk_cal_idx,hpipe,pm_num);*/
                /* Write PBLK regs
                 *      Only first 6 out of 7 slots could be valid
                 */
                if ((pm_num < 0) || (pm_num > FIREBOLT6_TDM_PBLKS_PER_DEV)) {
        		      /*printf(" Config PBlk :%d\n ",pm_num);*/
                    ...
                    for (slot = 0; slot < 6; slot++) {
                        ...
                        if (lane == -1) {
                            ...
                            port = sch_info->in_port_map.port_p2l_mapping[
												phy_port_base + lane];
	    /*printf(" Before Reg port %d , phy_port_base %d lane %d pm_num %d\n ",port,phy_port_base, lane, pm_num);*/
                            idb_port = sch_info->in_port_map.port_l2i_mapping[
												port] & 0x7f;
	    /*printf(" Before Reg port %d , idb_port %d\n ",port,idb_port);*/
			     idb_port = _soc_fb6_tdm_get_mport_2_hp_num(idb_port) ;
                            mmu_port = sch_info->in_port_map.port_p2m_mapping[
												(phy_port_base + lane)] & 0x7f;
	    /*printf(" Before Reg port %d , mmu_port %d\n ",port,mmu_port);*/
			    mmu_port = _soc_fb6_tdm_get_mport_2_hp_num(mmu_port) ;
	    /*printf(" Reg port %d , idb_port %d\n ",port,idb_port);*/
	    /*printf(" Reg port %d , mmu_port %d\n ",port,mmu_port);*/
                            ...
                            port_speed_class =
                                _soc_fb6_tdm_get_speed_ovs_class(unit,
                                                                 speed_max);
                            pblk_spacing =
                                _soc_fb6_tdm_get_same_port_spacing_e(unit,
                                                                     speed_max);
                        }
...
/*! @fn int _soc_fb6_tdm_set_ovs_group(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the FB6 MMU & IDB OVS tables.
            Regs/Mems configured: IDB HPIPE0/1_OVR_SUB_GRP_CFG
                                  HPIPE0/1_OVR_SUB_GRP_CFG
                                  PBLK<n>_CALENDAR
 */
int
_soc_fb6_tdm_set_ovs_group(int unit, soc_port_schedule_state_t *sch_info)
{
    return (_soc_fb6_tdm_set_ovs_group_sel(unit, sch_info, 1, 1));
}


/*! @fn int _soc_fb6_tdm_set_mmu_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the FB6 MMU main calendar.
            Regs/Mems configured: MMU TDM_CALENDAR
                                  TDM_CONFIG
 */
int
_soc_fb6_tdm_set_mmu_calendar(int unit, soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    uint32 cal_len;
    soc_mem_t mem;
    ...
    for (pipe = 0; pipe < FIREBOLT6_TDM_PIPES_PER_DEV; pipe++) {
        ...
        mem = calendar_mems[(calendar_id * FIREBOLT6_TDM_PIPES_PER_DEV) + pipe];
        cal_end_field = calendar_end_fields[calendar_id];
        ...
        for (length = cal_len; length > 0; length--) {
            if (sch_info->tdm_egress_schedule_pipe[pipe].
                          ...
                          linerate_schedule[length - 1] != FB6_NUM_EXT_PORTS) {
                break;
            }
        }
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == FB6_OVSB_TOKEN) {
                mmu_port = FIREBOLT6_TDM_MMU_OVSB_TOKEN;
            } else if (phy_port == FB6_IDL1_TOKEN) {
                mmu_port = FIREBOLT6_TDM_MMU_PURG_TOKEN;
            } else if (phy_port == FB6_IDL2_TOKEN) {
                mmu_port = FIREBOLT6_TDM_MMU_UNUSED_TOKEN;
            } else if (phy_port == FB6_NULL_TOKEN) {
                mmu_port = FIREBOLT6_TDM_MMU_NULL_TOKEN;
            } else if (phy_port >= FB6_NUM_EXT_PORTS) {
                mmu_port = FIREBOLT6_TDM_MMU_UNUSED_TOKEN;
            }
             else {
...
/*! @fn int _soc_fb6_tdm_set_mmu_hsp(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the FB6 MMU HSP.
            Regs/Mems configured: TDM_HSP_PIPE
 */
int
_soc_fb6_tdm_set_mmu_hsp(int unit,
                         soc_port_schedule_state_t *sch_info)
{
    soc_reg_t reg;
    uint32 pipe_map;
...
/*! @fn int _soc_fb6_tdm_set_mmu_opportunistic(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int cpu_op_en,
        int lb_opp_en,
    ...
    @param opp2_port_en
    @param opp_ovr_sub_en
    @brief API to initialize the FB6 MMU Opportunistic related registers
            Regs/Mems configured: CPU_LB_OPP_CFG
                                  OPP_SCHED_CFG
 */
int
_soc_fb6_tdm_set_mmu_opportunistic(int unit,
                                   soc_port_schedule_state_t *sch_info,
                                   int cpu_op_en,
                                   int lb_opp_en,
                                   int opp1_port_en,
 ...
 */
int soc_fb6_tdm_init(int unit, soc_port_schedule_state_t *sch_info)
{
    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_calculation(unit, sch_info));

    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_idb_calendar(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_idb_hsp(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_idb_pkt_calendar(unit, sch_info));
   SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_idb_ppe_credit(unit, sch_info, 16, 8, 16, 8));
    /* SOC_IF_ERROR_RETURN(soc_fb6_tdm_set_idb_dpp_ctrl(unit, sch_info, 32)); */
    /* IDB opp1_port_en should always be 1 to enable sbus access to the rest of IPIPE*/
    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_idb_opportunistic(unit, sch_info,
                                                           1, 1, 1, 1, 1));

    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_ovs_group(unit, sch_info));

    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_mmu_calendar(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_mmu_hsp(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_fb6_tdm_set_mmu_opportunistic(unit, sch_info,
                                                           1, 1, 1, 1, 1));

    return SOC_E_NONE;
}

/*! @fn int soc_firebolt6_port_speed_higig2eth(int speed)
 *  @param int speed
 *  @brief Map port speed to ETH bitrates
 */
static int
soc_firebolt6_port_speed_higig2eth(int speed)
{
    switch (speed) {
    case 11000:
        /* 10G */
...
/*! @fn void soc_firebolt6_port_schedule_speed_remap(int unit,
 ...
 */
void
soc_firebolt6_port_schedule_speed_remap(
    int  unit,
    soc_port_schedule_state_t *port_schedule_state)
{
    int i, port;
    ...
    for (port=0; port < SOC_MAX_NUM_PORTS; port++) {
        port_schedule_state->in_port_map.log_port_speed[port] =
            soc_firebolt6_port_speed_higig2eth(
                port_schedule_state->in_port_map.log_port_speed[port]);
    }
