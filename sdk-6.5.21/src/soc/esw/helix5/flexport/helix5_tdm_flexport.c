...
/**
@name: soc_helix5_set_tdm_soc_pkg
 ...
 */
STATIC int 
soc_helix5_set_tdm_soc_pkg(int unit, soc_port_schedule_state_t 
			*port_schedule_state, tdm_soc_t *soc_pkg, int new_or_prev)
{
    int port, phy_port, lane, index, pm_num;
    uint32 speed[HELIX5_TDM_PHY_PORTS_PER_DEV];
    uint32 port_state[HELIX5_TDM_PHY_PORTS_PER_DEV]; /* enum port_state_e */
    ...
    soc_pkg->unit = unit;
    soc_pkg->num_ext_ports = HX5_NUM_EXT_PORTS;
    ...
    sal_memset(speed,      0, HELIX5_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));
    sal_memset(port_state, 0, HELIX5_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));

    for (port = 0; port < HELIX5_DEV_PORTS_PER_DEV; port++) {
    ...
    if (1 == port_schedule_state->is_flexport) {
        ...
        for (pm_num = 0; pm_num < HX5_NUM_PHY_PM; pm_num++) { /* HELIX5_PBLKS_PER_PIPE * HELIX5_PIPES_PER_DEV */
            soc_pkg->soc_vars.hx5.pm_num_to_pblk[pm_num] = 
                  port_schedule_state->in_port_map.port_p2PBLK_inst_mapping[
									(pm_num * HELIX5_PORTS_PER_PBLK) + 1];
        }
    }
    ...
    for (index = 0; index < HX5_NUM_EXT_PORTS; index++) {
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
...
/**
@name: soc_helix5_tdm_copy_prev_tables
@param: int unit, soc_port_schedule_state_t *port_schedule_state
        tdm_soc_t *soc_pkg, int new_or_prev
 * Description:
 *      Helper function to allocate & populate state and speed in a tdm_soc_t structure
 ...
 */
STATIC int 
soc_helix5_tdm_copy_prev_tables(int unit, 
			soc_port_schedule_state_t *port_schedule_state, tdm_mod_t *_tdm_pkg)
{
    ...
    /* OVS tables */
    for (grp_num = 0; grp_num < HELIX5_OVS_GROUP_COUNT_PER_PIPE; grp_num ++) {
        hpipe = grp_num / HELIX5_OVS_GROUP_COUNT_PER_HPIPE;
        hpipe_grp = grp_num % HELIX5_OVS_GROUP_COUNT_PER_HPIPE;
        sal_memcpy(_tdm_pkg->_prev_chip_data.cal_0.cal_grp[grp_num], 
                   port_schedule_state->tdm_ingress_schedule_pipe[
					0].tdm_schedule_slice[hpipe].oversub_schedule[hpipe_grp],
                   sizeof(int) * HELIX5_OVS_GROUP_TDM_LENGTH);
    ...
    for (hpipe = 0; hpipe < 2; hpipe ++) {
        sal_memcpy(_tdm_pkg->_prev_chip_data.cal_0.cal_grp[
							HX5_SHAPING_GRP_IDX_0 + hpipe],
                   port_schedule_state->tdm_ingress_schedule_pipe[
					0].tdm_schedule_slice[hpipe].pkt_sch_or_ovs_space[0],
                   sizeof(int) * HX5_SHAPING_GRP_LEN);
...
/**
@name: soc_helix5_tdm_set_out_map
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 */
STATIC int
soc_helix5_tdm_set_out_map(int unit, 
				soc_port_schedule_state_t *port_schedule_state)
{
    ...
    sal_memcpy(port_schedule_state->out_port_map.port_l2p_mapping, 
               ...
               sizeof(int)*HELIX5_DEV_PORTS_PER_DEV);
    ...
    sal_memcpy(port_schedule_state->out_port_map.port_m2p_mapping, 
               ...
               sizeof(int)*HELIX5_MMU_PORTS_PER_DEV);
    ...
    soc_helix5_port_schedule_speed_remap(unit, port_schedule_state);
...
/**
@name: soc_helix5_tdm_flexport_ovs_add_rem_port
 ...
 */
int
soc_helix5_tdm_flexport_ovs_add_rem_port(int unit, 
	soc_port_schedule_state_t *port_schedule_state, 
    int logical_port,
    int rem_add_cmd,
    int set_idb,
    int set_mmu )
{
    ...
    static const soc_reg_t idb_grp_cfg_regs[HELIX5_PIPES_PER_DEV][2] = {
    ...
    static const soc_reg_t idb_grp_ovs_regs[HELIX5_PIPES_PER_DEV][2][6] = {
        {
    ...
    static const soc_reg_t idb_pblk_cal_regs[HELIX5_PIPES_PER_DEV][2][8] = {
        {
    ...
    static const soc_mem_t idb_pkt_shaper_mems[HELIX5_PIPES_PER_DEV][2] = {
    ...
    switch(rem_add_cmd) {
        ...
        case 0: port_map = &port_schedule_state->in_port_map;
                ...
                idb_port = HELIX5_TDM_IDB_UNUSED_TOKEN & 0x3f;
                mmu_port = HELIX5_TDM_MMU_UNUSED_TOKEN & 0x3f;
                id = 0x7;
    ...
    for (hpipe_i = 0; hpipe_i < HELIX5_OVS_HPIPE_COUNT_PER_PIPE; hpipe_i++) {
        for (group_i = 0; group_i < HELIX5_OVS_GROUP_COUNT_PER_HPIPE; group_i++) {
            for (slot_i = 0; slot_i < HELIX5_OVS_GROUP_TDM_LENGTH; slot_i++) {
                if (phy_port == 
    ...
    switch(rem_add_cmd) {
        ...
        case 2: 
                ...
                curr_tdm_schedule_pipe->tdm_schedule_slice[
					hpipe].oversub_schedule[group][slot] = HX5_NUM_EXT_PORTS;
                /* If group gets empty due to remove then configure ovs_class to 0; otherwise skip*/
                cfg_en_OVR_SUB_GRP_CFG = 1;
                for (i = 0; i < HELIX5_OVS_GROUP_TDM_LENGTH; i++) {
                    if (HX5_NUM_EXT_PORTS > 
						curr_tdm_schedule_pipe->tdm_schedule_slice[
        ...
        case 3: 
                ...
                if (cfg_en_OVR_SUB_GRP_CFG == 1)  {
                    ovs_class = _soc_hx5_tdm_get_speed_ovs_class(unit, 
     ...
     */
    if (cfg_en_OVR_SUB_GRP_CFG == 1) {
        ovs_sister_spacing = (ovs_class > 0) ? (phy_port > 64) ? 2 : 4 : 0; 
        ovs_same_spacing = _soc_hx5_tdm_get_same_port_spacing_e(unit, 
									port_map->log_port_speed[logical_port]);
        ...
        if (set_idb == 1) {
            ...
            soc_reg_field_set(unit, reg, &rval, SAME_SPACINGf, 
						(ovs_class == 0) ? 0 : ovs_same_spacing); /* on ingress, all ports have HSP same spacing */
    ...
    if (cfg_en_PBLK_CALENDAR == 1) {
        ...
        subport = (phy_port - 1) % HELIX5_PORTS_PER_PBLK;
...
/**
@name: soc_helix5_tdm_flexport_remove_ports_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
int
soc_helix5_tdm_flexport_remove_ports_sel(int unit, 
	soc_port_schedule_state_t *port_schedule_state, 
    int set_idb,
    int set_mmu )
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if ((-1 == physical_port) && (1 == is_port_oversub)) { /* that is, OVS port going DOWN */
            soc_helix5_tdm_flexport_ovs_add_rem_port(unit, 
                                                        port_schedule_state, 
                                                        logical_port,
                                                        rem_add_cmd,
                                                        set_idb,
...
/**
@name: soc_helix5_tdm_flexport_add_ports_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
int
soc_helix5_tdm_flexport_add_ports_sel(int unit, 
	soc_port_schedule_state_t *port_schedule_state, 
    int set_idb,
    int set_mmu )
{
...
/**
@name: soc_helix5_tdm_flexport_ovs_consolidate_per_speed
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
/* int
 *soc_helix5_tdm_flexport_ovs_consolidate_per_speed(int unit, 
 *  soc_port_schedule_state_t *port_schedule_state, 
 *  int pipe,
 *  int hpipe,
 *  int ovs_class,
 ...
 *  int ovs_class_i;
 *  int grp_bmp[HELIX5_OVS_GROUP_COUNT_PER_HPIPE];
 *  int num_of_ovs_grps;
 *  
 *  int port_moved;
 ...
 *  for (group = 0; group < HELIX5_OVS_GROUP_COUNT_PER_HPIPE; group++) {
 *      for (slot = 0; slot < HELIX5_OVS_GROUP_TDM_LENGTH; slot++) {
 *          physical_port = 
 *             curr_tdm_schedule_hpipe->oversub_schedule[group][slot];
 *          if (physical_port < HX5_NUM_EXT_PORTS) {
 *              logical_port = 
 *      			port_schedule_state->out_port_map.port_p2l_mapping[physical_port];
 *              ovs_class_i = _soc_hx5_tdm_get_speed_ovs_class(unit, 
 *              port_schedule_state->out_port_map.log_port_speed[logical_port]);
 ...
 *  LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, 
 *      		"START soc_helix5_tdm_flexport_ovs_consolidate_per_speed"
 ...
 *                      found = 0;
 *                      for (i = 0; i < HELIX5_OVS_GROUP_COUNT_PER_HPIPE; i++) {
 *                          for (j = 0; j < HELIX5_OVS_GROUP_TDM_LENGTH; j++) {
 *                              if (physical_port == 
 ...
 *      							dst_group][dst_slot] < HX5_NUM_EXT_PORTS) {
 *                          continue;
 *                      } else {
 *                          logical_port = 
 *      							port_schedule_state->out_port_map.port_p2l_mapping[
...
/**
@name: soc_helix5_tdm_flexport_ovs_consolidate_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
/* int
* soc_helix5_tdm_flexport_ovs_consolidate_sel(int unit, 
* 	soc_port_schedule_state_t *port_schedule_state, 
*     int set_idb,
*     int set_mmu )
* {
...
*      * 5 : 100G
*      *
*     int prev_num_grp[HELIX5_PIPES_PER_DEV][HELIX5_OVS_HPIPE_COUNT_PER_PIPE][7];
*     int  new_num_grp[HELIX5_PIPES_PER_DEV][HELIX5_OVS_HPIPE_COUNT_PER_PIPE][7];
...
* 												pipe][hpipe][ovs_class]) {
*                      soc_helix5_tdm_flexport_ovs_consolidate_per_speed(unit,
* 								port_schedule_state,
*                                 pipe,
*                                 hpipe,
*                                 ovs_class,
...
/*! @fn int soc_helix5_tdm_calculate_ovs_per_pipe(int unit,
 *              soc_port_schedule_state_t *port_schedule_state,
 ...
 */
int
soc_helix5_tdm_calculate_ovs_per_pipe(
    ...
    int pipe, int prev_or_new)
{
    ...
    int pipe_ovs_slots;
    int hpipe_ovs_slots[HELIX5_OVS_HPIPE_COUNT_PER_PIPE];
    soc_port_map_type_t *port_map;
    int pipe_ovs_ratio; /* per PIPE ovs_ratio */
    int hpipe_ovs_ratio[HELIX5_OVS_HPIPE_COUNT_PER_PIPE]; /* per Half PIPE ovs_ratio */
 ...
 */
int
soc_helix5_tdm_calculate_ovs(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        prev_or_new)
{
    int pipe;
    for (pipe = 0; pipe < HELIX5_PIPES_PER_DEV; pipe++) {
        soc_helix5_tdm_calculate_ovs_per_pipe(unit, port_schedule_state,
                                                 pipe,
                                                 prev_or_new);
    }
    return SOC_E_NONE;
...
/**
@name: tdm_hx5_free_prev_chip_data
@param:
...
tdm_hx5_free_prev_chip_data( tdm_mod_t *_tdm_pkg )
{
    ...
    TDM_FREE(_tdm_pkg->_prev_chip_data.cal_1.cal_grp[HX5_SHAPING_GRP_IDX_0]);
    TDM_FREE(_tdm_pkg->_prev_chip_data.cal_1.cal_grp[HX5_SHAPING_GRP_IDX_1]);
...
/**
@name: soc_helix5_tdm_calculation_flexport
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 *      SOC_E_*
 */
int soc_helix5_tdm_calculation_flexport(int unit, 
	soc_port_schedule_state_t *port_schedule_state)
{
    int port, pm_num;
    int pipe, hpipe;
    ...
    soc_helix5_tdm_set_out_map(unit, port_schedule_state);

  	/* Compute OVS ratios before FlexPort*/
    soc_helix5_tdm_calculate_ovs(unit, port_schedule_state, 0);
    ...
    soc_helix5_set_tdm_soc_pkg(unit, port_schedule_state, &soc_pkg, 1);
    ...
    for (cal_id = 0; cal_id < HELIX5_PIPES_PER_DEV; cal_id++) {
        ...
        sal_memcpy(port_schedule_state->tdm_ingress_schedule_pipe[
                   ...
                   tdm_idb_pipe_main, sizeof(int)*HELIX5_TDM_LENGTH);

        /* MMU TDM main calendar */
        sal_memcpy(port_schedule_state->tdm_egress_schedule_pipe[
					cal_id].tdm_schedule_slice[0].linerate_schedule, 
                   tdm_mmu_pipe_main, sizeof(int)*HELIX5_TDM_LENGTH);

        /* IDB and MMU OverSub Groups */
        for (index = 0; index < HELIX5_OVS_GROUP_COUNT_PER_PIPE; index++) {
            /* Group 0:5 -> hpipe 0, Group 6:11 -> hpipe1 */
            hpipe = index / HELIX5_OVS_GROUP_COUNT_PER_HPIPE;
            cal_grp = index % HELIX5_OVS_GROUP_COUNT_PER_HPIPE;
            sal_memcpy(port_schedule_state->tdm_egress_schedule_pipe[
        ...
        for (hpipe = 0; hpipe < 2; hpipe ++) {
            sal_memcpy(port_schedule_state->tdm_ingress_schedule_pipe[
                    ...
                    tdm_pipe_group[hpipe+HX5_SHAPING_GRP_IDX_0], sizeof(int) 
					* HX5_SHAPING_GRP_LEN);
        }
    }
    /* 
    ...
    for (pm_num = 0; pm_num < HX5_NUM_PHY_PM; pm_num++) { /* HELIX5_PBLKS_PER_PIPE * HELIX5_PIPES_PER_DEV */
        for (port = 0; port < HELIX5_PORTS_PER_PBLK; port++) {
            port_schedule_state->out_port_map.port_p2PBLK_inst_mapping[
				(pm_num * HELIX5_PORTS_PER_PBLK) + 1 + port] = 
    ...
    soc_helix5_tdm_calculate_ovs(unit, port_schedule_state, 1);


    for (pipe = 0; pipe < HELIX5_PIPES_PER_DEV; pipe++) {
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "Pipe %d idb_tdm:"), pipe));
        for (index = 0; index < HELIX5_TDM_LENGTH; index++) {
            if (index % 16 == 0) {
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                                    "\n    ")));
            }
        ...
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "\n")));
        for (hpipe = 0; hpipe < HELIX5_OVS_HPIPE_COUNT_PER_PIPE; hpipe++) {
            for (group = 0; group < HELIX5_OVS_GROUP_COUNT_PER_HPIPE;
                    group++) {
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "Pipe %d hpipe %d group %d ovs_tdm"),
                            pipe, hpipe, group));
                for (index = 0; index < HELIX5_OVS_GROUP_TDM_LENGTH;
                        index++) {
                    if (index == 0) {
                        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                                            "\n    ")));
	...
	_soc_hx5_tdm_print_schedule_state(unit, port_schedule_state);
...
/**
@name: soc_helix5_tdm_flexport_remove_ports
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
int
soc_helix5_tdm_flexport_remove_ports(int unit, 
	soc_port_schedule_state_t *port_schedule_state)
{
    return (soc_helix5_tdm_flexport_remove_ports_sel(
...
/**
@name: soc_helix5_tdm_flexport_ovs_consolidate
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 */
int
soc_helix5_tdm_flexport_ovs_consolidate(int unit, 
	soc_port_schedule_state_t *port_schedule_state)
{
   /* return (soc_helix5_tdm_flexport_ovs_consolidate_sel(
    *				unit, port_schedule_state, 1, 1));
    */
    return SOC_E_NONE; /* nothing to do hx5 consolidation not required HWHELIX5-2182 */
}


/**
@name: soc_helix5_tdm_flexport_add_ports
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 */
int
soc_helix5_tdm_flexport_add_ports(int unit, 
	soc_port_schedule_state_t *port_schedule_state)
{
    return (soc_helix5_tdm_flexport_add_ports_sel(unit, 
				port_schedule_state, 1, 1));
}

/*! @fn int soc_helix5_tdm_flexport_idb(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_helix5_tdm_flexport_idb(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    SOC_IF_ERROR_RETURN(soc_helix5_tdm_flexport_remove_ports_sel(unit,
                                                   port_schedule_state, 1, 0));
    /* SOC_IF_ERROR_RETURN(soc_helix5_tdm_flexport_ovs_consolidate_sel(unit,
     *                                                port_schedule_state, 1, 0));
     */
    /* nothing to do hx5 consolidation not required HWHELIX5-2182 */
    SOC_IF_ERROR_RETURN(soc_helix5_tdm_flexport_add_ports_sel(unit,
                                                   port_schedule_state, 1, 0));
    return SOC_E_NONE;
}

/*! @fn int soc_helix5_tdm_flexport_mmu(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_helix5_tdm_flexport_mmu(int                        unit,
                               soc_port_schedule_state_t *port_schedule_state)
{
    SOC_IF_ERROR_RETURN(soc_helix5_tdm_flexport_remove_ports_sel(unit,
...
/**
@name: tdm_hx5_alloc_prev_chip_data
@param:
...
tdm_hx5_alloc_prev_chip_data( tdm_mod_t *_tdm_pkg )
{
    ...
    _tdm_pkg->_prev_chip_data.cal_0.cal_main=(int *) TDM_ALLOC(
				(HX5_LR_VBS_LEN)*sizeof(int), "OLD TDM inst 0 main calendar");	
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp=(int **) TDM_ALLOC(
				(HX5_OS_VBS_GRP_NUM+2)*sizeof(int *), "OLD TDM inst 0 groups");
    for (index=0; index<(HX5_OS_VBS_GRP_NUM); index++) {
        _tdm_pkg->_prev_chip_data.cal_0.cal_grp[index]=
			(int *) TDM_ALLOC((HX5_OS_VBS_GRP_LEN)*sizeof(int), 
				"OLD TDM inst 0 group calendars");
    }
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp[HX5_SHAPING_GRP_IDX_0]=
			(int *) TDM_ALLOC((HX5_SHAPING_GRP_LEN)*sizeof(int), 
			"OLD TDM inst 0 shaping calendars");
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp[HX5_SHAPING_GRP_IDX_1]=
			(int *) TDM_ALLOC((HX5_SHAPING_GRP_LEN)*sizeof(int), 
			"OLD TDM inst 0 shaping calendars");
    /* Pipe 1 calendar group */
    _tdm_pkg->_prev_chip_data.cal_1.cal_main=
			(int *) TDM_ALLOC((HX5_LR_VBS_LEN)*sizeof(int), 
			"OLD TDM inst 1 main calendar");	
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp=
			(int **) TDM_ALLOC((HX5_OS_VBS_GRP_NUM+2)*sizeof(int *), 
    ...
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp[HX5_SHAPING_GRP_IDX_0]=
			(int *) TDM_ALLOC((HX5_SHAPING_GRP_LEN)*sizeof(int), 
			"OLD TDM inst 1 shaping calendars");
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp[HX5_SHAPING_GRP_IDX_1]=(
			int *) TDM_ALLOC((HX5_SHAPING_GRP_LEN)*sizeof(int), 
			"OLD TDM inst 1 shaping calendars");
