...
#if defined(BCM_HELIX5_SUPPORT) && defined(INCLUDE_L3)
 ...
 */
int
soc_hx5_lpm_flex_ip4entry0_to_1(int u, void *src, void *dst, uint32 key_type, int copy_hit)
{
    uint32      ipv4a;
    soc_mem_t   view_id;
 ...
 */
STATIC int
soc_hx5_lpm_flex_vrf_get(int unit, void *lpm_entry, uint32 view_id, int *vrf)
{
    int vrf_id;
 ...
 * Purpose:
 *      soc_hx5_lpm_flex_prefix_length_get (Extract vrf weighted  prefix lenght from the 
 ...
 */
int
soc_hx5_lpm_flex_prefix_length_get(int u, void *entry, int *ipv6, int *vrf_id, int *pfx, uint32 key_type)
{
    int         rv;
    uint32      ipv4a;
    soc_mem_t   view_id;
    ...
    *ipv6 = soc_mem_field32_get(u, view_id, entry, KEY_MODE0f);
    ...
    SOC_IF_ERROR_RETURN(soc_hx5_lpm_flex_vrf_get(u, entry, view_id, vrf_id));
