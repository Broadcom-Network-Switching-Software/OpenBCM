...
soc_hurricane4_bond_info_init(int unit)
{
    ...
    int feature_enables[][3] = {
        ...
        {355, 1, socBondInfoFeatureEfp256HalfSlice},  /* EFP_SLICE_DEPTH */
        {380, 1, socBondInfoFeatureVfp256HalfSlice},  /* VFP_SLICE_DEPTH */
    ...
    /* IPMC Groups */
    SOC_BOND_OPTIONS_COPY(val, uint32, bond_info_regval, 381, 382);
    if (val == 0) {
        SOC_BOND_INFO_FEATURE_SET(unit, socBondInfoFeatureIpmcDisabled);
    } else if (val == 1) {
        SOC_BOND_INFO_FEATURE_SET(unit, socBondInfoFeatureIpmc2k);
    } else if (val == 2) {
