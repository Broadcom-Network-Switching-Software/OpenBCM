...
soc_hr4_utt_init(int u)
{
    ...
    if (soc_feature(u, soc_feature_td3x1_reduced_utt)) {
        ...
        SOC_UTT_AGENT_MAX_TCAMS(u, aid)        = HR4_REDUCED_UTT_LPM_MAX_TCAMS;
        SOC_UTT_AGENT_DEF_NUM_LOOKUPS(u, aid)  = 4;
...
soc_hurricane4_mem_config(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_td3x1_novrf)) {
        sop->memState[VRF_ATTRS_2m].index_max = 0;
    }
...
_soc_hr4_gphy_war_enable(int unit)
{
    ...
    if (gpio < 32) {
        ...
        /* Enable M0.0 interrupt. */
        SOC_IF_ERROR_RETURN(READ_U0_M0SS_INTR_MASK_63_32r(unit, &rval));
        rval |= (1 << (46 - 32));
        SOC_IF_ERROR_RETURN(WRITE_U0_M0SS_INTR_MASK_63_32r(unit, rval));
    };
