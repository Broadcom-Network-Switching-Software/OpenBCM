...
tdm_mv2_ovsb_pkt_shaper_per_pm(tdm_mod_t *_tdm, int shift_step)
{
    ...
    switch (param_speeds[port_phy]) {
        case SPEED_10G:
            /* 10G combination: p0_p1_p2_p3, where pi=0 or pi=10G, i=0,1,2,3 */
            for (lane = 0; lane < MV2_NUM_PM_LNS; lane++) {
                int pport = pm_port_lo + lane;
                if (param_speeds[pport] == SPEED_10G) {
                    switch (lane) {
                        case 0:
                            pm_slot_tbl[0 % MV2_NUM_PKT_SLOTS_PER_PM] = pport;
                            pm_slot_tbl[8 % MV2_NUM_PKT_SLOTS_PER_PM] = pport;
                            break;
                        case 1:
                            pm_slot_tbl[10 % MV2_NUM_PKT_SLOTS_PER_PM] = pport;
                            pm_slot_tbl[18 % MV2_NUM_PKT_SLOTS_PER_PM] = pport;
                            break;
                        case 2:
                            pm_slot_tbl[5 % MV2_NUM_PKT_SLOTS_PER_PM] = pport;
                            pm_slot_tbl[13 % MV2_NUM_PKT_SLOTS_PER_PM] = pport;
                            break;
                        default:
                            pm_slot_tbl[3 % MV2_NUM_PKT_SLOTS_PER_PM] = pport;
                            pm_slot_tbl[15 % MV2_NUM_PKT_SLOTS_PER_PM] = pport;
                            break;
                    }
                }
            }
        ...
        case SPEED_40G:
            ...
            break;
        case SPEED_25G:
            /* 25G combination: p0_p1_p2_p3, where pi=0 or pi=25G, i=0,1,2,3 */
            for (lane = 0; lane < MV2_NUM_PM_LNS; lane++) {
                if (param_speeds[pm_port_lo + lane] == SPEED_25G) {
                    for (i = 0; i < pkt_sched_repetitions; i++) {
                        switch (lane) {
                            case 0: tbl_offset = (i * 4 + 0); break;
                            case 1: tbl_offset = (i * 4 + 2); break;
                            case 2: tbl_offset = (i * 4 + 1); break;
                            default:tbl_offset = (i * 4 + 3); break;
                        }
                        pm_slot_tbl[tbl_offset % MV2_NUM_PKT_SLOTS_PER_PM] =
                            pm_port_lo + lane;
                    }
                }
            }
