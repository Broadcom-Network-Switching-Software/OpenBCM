...
tdm_fb6_ovsb_map_pm_num_to_pblk(tdm_mod_t *_tdm)
{
    ...
    tdm_fb6_cmn_get_half_pipe_map_en(_tdm, half_pipe_map_en);
    tdm_fb6_cmn_get_mgmt_pm(_tdm, &mgmt_pm);
    for (pm_num=pipe_id*pms_per_pipe; pm_num<(pipe_id+1)*pms_per_pipe; pm_num++) {
        ...
        if(pm_num != mgmt_pm) {
          ...
          if ((half_pipe_num==0) || (half_pipe_num==1)){
            ...
            /*Added this logic to resolve flexing unused pm
            While flexing unused PM we are seeing BW issues on non flexed ports
            due to PBLK numbering/Half Pipe Oversub Table issues (same as MV2)*/

              if(half_pipe_map_en[pm_num] == 1) {
                 half_pipe_num = 0;
              }
              else {
                 half_pipe_num = 1;
          ...
          TDM_PRINT4("OVSB Mapping pm_num %d half_pipe_num %d pblk_indx[half_pipe_num] %d pm_num_to_pblk %d\n", pm_num, half_pipe_num, pblk_indx[half_pipe_num], _tdm->_chip_data.soc_pkg.soc_vars.fb6.pm_num_to_pblk[pm_num]);
        }
    }
