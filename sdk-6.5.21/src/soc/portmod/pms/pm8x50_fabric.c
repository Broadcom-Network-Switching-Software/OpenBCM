...
#define PM8X50_FABRIC_PLL_ACTUAL_LANES_BITMAP_SET(unit, pm_info, bitmap, pll_index) \
    SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_actual_lanes_bitmap], &bitmap, pll_index)

#define PM8X50_FABRIC_PLL_ACTUAL_LANES_BITMAP_GET(unit, pm_info, bitmap, pll_index) \
...
int pm8x50_fabric_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "pll_actual_lanes_bitmap", wb_buffer_index, sizeof(uint32), NULL, PM8X50_FABRIC_MAX_NUM_PLLS, VERSION(1));
    SOCDNX_IF_ERR_EXIT(rv);
...
{   
    ...
    fabric_data->clock_buffer_bw_update_required = pm_add_info->pm_specific_info.pm8x50_fabric.clock_buffer_bw_update_required;
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        PM8X50_FABRIC_PLLS_ITER(0, PM8X50_FABRIC_MAX_NUM_PLLS, i) {
            ...
            SOCDNX_IF_ERR_EXIT(rv);
            rv = PM8X50_FABRIC_PLL_ACTUAL_LANES_BITMAP_SET(unit, pm_info, pll_actual_lanes_bitmap, i);
            SOCDNX_IF_ERR_EXIT(rv);
...
int pm8x50_fabric_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    uint32 pll_actual_lanes_bitmap[PM8X50_FABRIC_MAX_NUM_PLLS];
    ...
    PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        ...
        SOCDNX_IF_ERR_EXIT(PM8X50_FABRIC_PLL_ACTUAL_LANES_BITMAP_GET(unit, pm_info, &pll_actual_lanes_bitmap[pll_index], pll_index));
        ...
        if (SHR_BITGET(&pll_active_lanes_bitmap[pll_index], phy_index))
        {
            ...
            SOCDNX_IF_ERR_EXIT(PM8X50_FABRIC_PLL_ACTUAL_LANES_BITMAP_SET(unit, pm_info, pll_actual_lanes_bitmap[pll_index], pll_index));
    ...
    if (fabric_data->clock_buffer_disable_required) {
        PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index) {
            SOCDNX_IF_ERR_EXIT(_pm8x50_fabric_pll_clock_repeater_buffer_pwrdn(unit, pm_info,
...
#define PM8X50_FABRIC_PORT_RETIMER_AUTO_ADJUST_MIN (0x2)
#define PM8X50_FABRIC_PORT_RETIMER_AUTO_ADJUST_MAX(is_retimer_enabled) ((is_retimer_enabled) ? (0x10) : (0xa))
#define PM8X50_FABRIC_PORT_RETIMER_AUTO_ADJUST_PERIOD_FIELD_VAL(is_retimer_enabled) ((is_retimer_enabled) ? (0xffffff) : (0x7d0))
#define PM8X50_FABRIC_PORT_RETIMER_AUTO_ADJUST_PERIOD_USEC (1000000)
...
pm8x50_fabric_port_retimer_set(int unit, soc_port_t port, pm_info_t pm_info, int is_remote_loopback, int enable)
{
    ...
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_DC_AUTO_ADJUSTr, &reg64_val, FMAL_N_TX_DC_AUTO_ADJUST_MINf, PM8X50_FABRIC_PORT_RETIMER_AUTO_ADJUST_MIN);
...
int 
pm8x50_fabric_port_phase_interpolator_rx_set(int unit, soc_port_t port, pm_info_t pm_info, int enable)
{
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    int nof_phys;
...
int 
pm8x50_fabric_port_phase_interpolator_tx_set(int unit, soc_port_t port, pm_info_t pm_info, int enable)
{
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    int nof_phys;
...
static
int pm8x50_fabric_vco_to_pll_get(
    int unit, 
...
int pm8x50_fabric_pm_vco_reconfig(int unit, int pm_id, pm_info_t pm_info, const portmod_vco_type_t* vco)
{
    ...
    PM8X50_FABRIC_PLLS_ITER(begin_pll, end_pll, pll_index)
    {
        ...
        if (vco_aligned[pll_index] != pll_vco_rate[pll_index])
        {
            ...
            SOCDNX_IF_ERR_EXIT(pm8x50_fabric_vco_to_pll_get(unit, fabric_data->ref_clk, vco_aligned[pll_index], &pll_div));
 ...
 */
    ...
    if (!SOC_IS_RELOADING(unit))
    {
        ...
        if (fabric_data->clock_buffer_disable_required) {
            ...
            int i, tmp_port;
            for (i = 0; i < PM8X50_FABRIC_MAX_PORTS_PER_PM; i++) {
                SOCDNX_IF_ERR_EXIT(PM8X50_FABRIC_LANE2PORT_GET(unit, pm_info, i, &tmp_port));
                if(tmp_port >= 0) {
