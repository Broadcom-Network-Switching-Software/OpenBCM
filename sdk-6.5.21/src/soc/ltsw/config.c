...
/*!
 * \brief Delete a configuration field.
 *
 * \param [in] unit         Unit number.
 * \param [in] name         Field name string.
 *
 ...
 */
static int
config_field_delete(int unit,
                    const char *name)
{
    config_db_t *cd = &(config_info.config_db[unit]);
    config_fld_t *found_fld = NULL;
    config_fld_t *pre_fld = NULL;
    int hash = config_field_hash(name);
    ...
    /* Delete the found field. */
    if (found_fld != NULL) {

        if (pre_fld == NULL) {
            cd->fld[hash] = found_fld->next;
        } else {
            pre_fld->next = found_fld->next;
        }

        SHR_FREE(found_fld->name);
        SHR_FREE(found_fld->value);
        SHR_FREE(found_fld);
...
int
soc_ltsw_config_str_delete(int unit, const char *name)
{
    SHR_FUNC_ENTER(unit);
...
int
soc_ltsw_config_port_str_delete(int unit,
                                int port,
                                const char *name)
{
    bool asterisk;
    ...
    SHR_IF_ERR_EXIT
        (config_field_delete(unit, cfg_name));
