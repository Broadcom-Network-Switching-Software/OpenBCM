...
#define PHY_IS_BCM53540(_pc) (PHY_MODEL_CHECK((_pc), \
                              PHY_BCM53540_OUI, \
                              PHY_BCM53540_MODEL))
STATIC int
...
static int
_phy53540_ieee_compliance_setup(int unit, phy_ctrl_t *pc)
{
    /* Turn off AOF. AFE_TX_CONFIG_0 */
    SOC_IF_ERROR_RETURN
        (phy_egphy28_rdb_reg_write(unit, pc, 0x1E5, 0x0000));
    /* 1g AB symmetry Iq. AFE_TX_CONFIG_1 */
    SOC_IF_ERROR_RETURN
        (phy_egphy28_rdb_reg_write(unit, pc, 0x1EA, 0x0BCC));
    /* LPF BW. AFE_TX_IQ_RX_LP */
    SOC_IF_ERROR_RETURN
        (phy_egphy28_rdb_reg_write(unit, pc, 0x1E4, 0x233F));
    /* RCAL +6LSB to make impedance from 112 to 100ohm. AFE_TEMPSEN_OTHERS */
    SOC_IF_ERROR_RETURN
        (phy_egphy28_rdb_reg_write(unit, pc, 0x1EC, 0xC600));
    /* Enable DSP register access */
    SOC_IF_ERROR_RETURN
        (phy_egphy28_rdb_reg_write(unit, pc, 0x028, 0x4C30));
    /* Make master current -4. DSP_TAP10 */
    SOC_IF_ERROR_RETURN
        (phy_egphy28_rdb_reg_write(unit, pc, 0x0125, 0x191B));
    /* Disable DSP register access */
    SOC_IF_ERROR_RETURN
        (phy_egphy28_rdb_reg_write(unit, pc, 0x028, 0x4430));
    return SOC_E_NONE;
}
...
_phy_egphy28_ieee_compliance_setup(int unit, soc_port_t port)
{
    ...
    if (PHY_IS_BCM53540(pc)) {
        return _phy53540_ieee_compliance_setup(unit, pc);
    }
    if (!PHY_IS_BCM56160(pc)) {
        return SOC_E_NONE;
    }

    if (pc->phy_rev == BCM56160_A0_REV_ID) {
        ...
        SOC_IF_ERROR_RETURN
            (phy_egphy28_rdb_reg_write(unit, pc, 0x1E4, 0x0000));
        ...
        SOC_IF_ERROR_RETURN
            (phy_egphy28_rdb_reg_write(unit, pc, 0x1EA, 0x2900));

        /* Set AFE_VDAC_ICTRL_0(0x1E5) to 0xA7AB */
        SOC_IF_ERROR_RETURN
            (phy_egphy28_rdb_reg_write(unit, pc, 0x1E5, 0xA7AB));

        /* Set AFE_HPF_TRIM_OTHERS(0x1E8) to 0x00C3 */
        SOC_IF_ERROR_RETURN
            (phy_egphy28_rdb_reg_write(unit, pc, 0x1E8, 0x00C3));

        /* Set AFE_TX_CONFIG_1(0x1E9) to 0x00CC */
        SOC_IF_ERROR_RETURN
            (phy_egphy28_rdb_reg_write(unit, pc, 0x1E9, 0x00CC));
        ...
        SOC_IF_ERROR_RETURN
            (phy_egphy28_rdb_reg_write(unit, pc, 0x125, 0x191B));

        /* Set CORE_SHD18_000(0x28) to 0x4430 (disable DSP_CLK_EN) */
        SOC_IF_ERROR_RETURN
            (phy_egphy28_rdb_reg_write(unit, pc, 0x28, 0x4430));
...
typedef int _phy_egphy28_not_empty; /* Make ISO compilers happy. */
