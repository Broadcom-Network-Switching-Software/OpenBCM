...
soc_ramon_sr_cell_format(int unit, const dnxc_vsc256_sr_cell_t* cell, int buf_size_bytes, uint32* buf)
{
    ...
    rc = soc_dnxc_vsc256_sr_cell_build_header(unit, cell, buf_size_bytes - (RAMON_SR_DATA_CELL_PAYLOAD_LENGTH/8), (uint32*)(((uint8*)buf) + (RAMON_SR_DATA_CELL_PAYLOAD_LENGTH/8) +dnxf_data_fabric.cell.source_routed_cells_header_offset_get(unit)/8));
    SHR_IF_ERR_EXIT(rc);
...
shr_error_e
soc_ramon_dest_routed_cell_send(int unit, dnxc_vsc256_dest_routed_cell_t* cell)
{
    ...
    SHR_IF_ERR_EXIT(soc_ramon_fabric_reachability_status_get(unit, cell->header.dest_modid, dnxf_data_port.general.nof_links_get(unit), links_array, &links_count));
    if(links_count == 0)
    {
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_vsc256_dest_routed_cell_build_header(unit, cell, DNXC_VSC256_DESTINATION_ROUTED_CELL_HEADER_SIZE_UINT32, header_buf ));
    SOC_REG_ABOVE_64_CLEAR(cell_header_reg);
    SOC_REG_ABOVE_64_RANGE_COPY(cell_header_reg, 0, header_buf, 0, DNXC_VSC256_DESTINATION_ROUTED_CELL_HEADER_SIZE_BITS);
    ...
    soc_reg_field_set(unit, DCH_TX_CPU_CELL_INFOr, &reg_val32, TX_CPU_CELL_TWO_PIPESf,   dnxf_data_fabric.pipes.nof_pipes_get(unit) == 2 ? 1 : 0);
    soc_reg_field_set(unit, DCH_TX_CPU_CELL_INFOr, &reg_val32, TX_CPU_CELL_THREE_PIPESf, dnxf_data_fabric.pipes.nof_pipes_get(unit) == 3 ? 1 : 0);
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_val);
    max_reg_arr_index = BYTES2BITS(DNXC_VSC256_DESTINATION_ROUTED_CELL_PAYLOAD_SIZE_BYTES) / DNXF_RAMON_DATA_CELL_DATA_REG_FIELD_SIZE_BITS - 1;
    for (reg_arr_index = 0; reg_arr_index <= max_reg_arr_index; reg_arr_index++)
    {
        ...
        {
            reg_bit_offset = DNXF_RAMON_DATA_CELL_DATA_REG_FIELD_SIZE_BITS - (word_index * WORDS2BITS(1)) - WORDS2BITS(1);
            bytes_offset = reg_arr_index * BITS2BYTES(DNXF_RAMON_DATA_CELL_DATA_REG_FIELD_SIZE_BITS) +   (word_index      * WORDS2BYTES(1));
            bytes_written = reg_arr_index * BITS2BYTES(DNXF_RAMON_DATA_CELL_DATA_REG_FIELD_SIZE_BITS) + ((word_index + 1) * WORDS2BYTES(1));
        ...
        SHR_IF_ERR_EXIT(soc_reg_above_64_set(unit, DCH_TX_CPU_CELL_DATAr, dch_block, max_reg_arr_index - reg_arr_index, reg_val));
