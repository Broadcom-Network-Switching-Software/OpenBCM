...
soc_ramon_drv_sbus_broadcast_config(int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(WRITE_FMAC_SBUS_BROADCAST_IDr(unit, block_idx, SOC_BLOCK_INFO(unit,BRDC_FMAC_BLOCK(unit, 0)).schan));
    }
    ...
    for (block_idx=0; block_idx<dnxf_data_device.blocks.nof_instances_fsrd_get(unit); block_idx++)
    {
        ...
        if (is_enabled)
        {
            SHR_IF_ERR_EXIT(WRITE_FSRD_SBUS_BROADCAST_IDr(unit, block_idx, SOC_BLOCK_INFO(unit,BRDC_FSRD_BLOCK(unit, 0)).schan));
        }
    }
