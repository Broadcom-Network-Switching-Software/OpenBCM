  ...
  )
{
    ...
    if (SOC_IS_FLAIR(unit)) {
        SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, ECI_RESCAL_STATUSr, REG_PORT_ANY, 0, &reg64));
        field_val = soc_reg64_field_get(unit, ECI_RESCAL_STATUSr, reg64, PONf);
...
soc_error_t
  soc_jer_fabric_pcp_mode_init(
    SOC_SAND_IN  int          unit
  )
{
    int modid_max, modid;
    int force_no_packing;
    ...
    force_no_packing = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pcp_force_no_packing", 0);
    ...
    if (force_no_packing) {
        ...
        for (modid = 0; modid < modid_max; ++modid) {
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_fabric_pcp_dest_mode_config_set,
                                                             (unit, 0, modid, bcmFabricPcpModeNoPacking)));
        }
    }
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_fabric_pcp_mode_init(unit));
    ...
    SOC_SAND_OUT int             *divider)
{
    ...
    if (!SOC_IS_QAX_WITH_FABRIC_ENABLED(unit)) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg64_val));
        *divider = soc_reg64_field_get(unit, FSRD_SYNC_E_SELECTr, reg64_val, SYNC_E_CLK_DIV_PHASE_1f) + 1; 
    } else {
    ...
    SOC_SAND_IN  int             divider)
{
    ...
    if (!SOC_IS_QAX_WITH_FABRIC_ENABLED(unit)) {
        ...
        for (i = 0; i < SOC_DPP_DEFS_GET(unit, nof_instances_fsrd); ++i){
            SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, i, 0, &reg64_val));
            soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg64_val, SYNC_E_CLK_DIV_PHASE_1f, phase1_divider);
            soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg64_val, SYNC_E_CLK_DIV_PHASE_0f, phase1_divider/2);
            SOCDNX_IF_ERR_EXIT(soc_reg_set(unit, FSRD_SYNC_E_SELECTr, i, 0, reg64_val));
        }
    } else {
  ...
  )
{
    ...
    if (is_master) {
        ...
        if (!SOC_IS_QAX_WITH_FABRIC_ENABLED(unit)) {
            ...
            soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg64_val, MASTER_CLK_SELf, inner_fsrd_port);
        } else {
        ...
        if (!SOC_IS_QAX_WITH_FABRIC_ENABLED(unit)) {
            ...
            soc_reg64_field_set(unit, FSRD_SYNC_E_SELECTr, &reg64_val, SLAVE_CLK_SELf, inner_fsrd_port);
        } else {
    ...
    if (!SOC_IS_QAX_WITH_FABRIC_ENABLED(unit)) {
        SOCDNX_IF_ERR_EXIT(soc_reg_set(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, reg64_val));
    }
  ...
  )
{
    ...
    if (!SOC_IS_QAX_WITH_FABRIC_ENABLED(unit)) {
        if (is_master) {
            ...
            SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg64_val));
            inner_fsrd_port = soc_reg64_field_get(unit, FSRD_SYNC_E_SELECTr, reg64_val, MASTER_CLK_SELf);
        } else {
            SOCDNX_IF_ERR_EXIT(READ_ECI_SYNCE_SLAVE_PLL_CONFIGr(unit, reg_above64_val));
            field = SOC_IS_JERICHO_PLUS_ONLY(unit)? SYNCE_SLAVE_PLL_CFG_REFRENCE_FABRIC_SEL_SLAVEf : SYNCE_SLAVE_PLL_CFG_REFRENCE_FABRIC_SELf;
            ...
            SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg64_val));
            inner_fsrd_port = soc_reg64_field_get(unit, FSRD_SYNC_E_SELECTr, reg64_val, SLAVE_CLK_SELf);
        }
