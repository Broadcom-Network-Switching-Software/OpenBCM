...
soc_jer_ports_config(int unit) 
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "gpon_application_enable", 0) && (SOC_DPP_CONFIG(unit)->pp.pon_port_ptc_size > 5)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("pon_port_ptc_size must not larger than 5.")));
    }
