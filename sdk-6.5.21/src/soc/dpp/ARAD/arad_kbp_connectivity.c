...
#if ((defined(BCM_88650_A0) || defined(BCM_88660_A0)) && defined(INCLUDE_KBP))
    ...
    uint32 core)
{
    ...
    if (!SAL_BOOT_PLISIM) {
        ...
        if (ARAD_KBP_OP_IS_DUAL_MODE) {
            kbp_config = &AradAppData[unit-1]->device_config[core];
        } else {
...
uint32 arad_kbp_init_gpio_config(
    int unit)
{
    int res = SOC_SAND_OK, kbp_gpio_address, kbp_gpio_mask;
    int i2c_access_disable = 0;
    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
