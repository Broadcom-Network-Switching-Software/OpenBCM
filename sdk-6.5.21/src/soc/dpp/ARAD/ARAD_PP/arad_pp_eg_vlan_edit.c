  ...
  )
{
  ...
  uint32
      ...
      pcp_profile_ndx, up_ndx, cfi_ndx, tc_ndx, dp_ndx, packet_is_tagged_after_eve;
  uint64 reg_val;
  ...
  for (tc_ndx = 0; tc_ndx <= SOC_SAND_PP_TC_MAX; ++tc_ndx) 
  {
    ...
    {
      cfi_ndx = dp_ndx & 0x1;
      res = arad_pp_eg_vlan_edit_pcp_map_untagged_set_unsafe(unit, pcp_profile_ndx, tc_ndx, dp_ndx, up_ndx, cfi_ndx);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
