...
soc_port_sw_db_print(int unit, uint32 flags)
{
    ...
    char **logical_ports_str = NULL;
    soc_phy_to_logical_ports_str_t *logical_ports = NULL;
    int *logical_port_count = NULL;
    ...
    logical_port_count = sal_alloc(sizeof(int)*SOC_MAX_NUM_PORTS, "soc_port_sw_db_print.logical_port_count");
    if (!logical_port_count) {
        LOG_CLI((BSL_META_U(unit,
                            "Memory allocation failure\n")));
        return SOC_E_MEMORY;
    }

    logical_ports_str = sal_alloc(sizeof(char*)*SOC_MAX_NUM_PORTS, "soc_port_sw_db_print.logical_ports_str");
    if (!logical_ports_str) {
        LOG_CLI((BSL_META_U(unit,
                            "Memory allocation failure\n")));
        sal_free(logical_port_count);
        return SOC_E_MEMORY;
    }
