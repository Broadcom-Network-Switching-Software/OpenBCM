    ...
    dnxc_core_address_get_f address_get_func)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        pm_info.pm_specific_info.pm8x50_fabric.fw_load_method =
            dnx_data_port.static_add.fabric_fw_load_method_get(unit);
#endif
    }
    else
    {
#ifdef BCM_DNXF_SUPPORT
        pm_info.pm_specific_info.pm8x50_fabric.fw_load_method = dnxf_data_port.general.fw_load_method_get(unit);
#endif
    }
    ...
    if (bw_update)
    {
        pm_info.pm_specific_info.pm8x50_fabric.clock_buffer_bw_update_required = TRUE;
    }

    pm_info.pm_specific_info.pm8x50_fabric.clock_buffer_disable_required = clock_buffer_disable_required;
    ...
    int enable)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        if (dnx_data_fabric.links.feature_get(unit, dnx_data_fabric_links_isolation_needed_before_disable))
        {
