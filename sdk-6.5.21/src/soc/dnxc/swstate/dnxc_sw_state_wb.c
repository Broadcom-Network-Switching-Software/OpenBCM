...
static void
dnxc_sw_state_parallel_memset(shr_thread_ctrl_t * th, void* input);
...
void dnxc_sw_state_parallel_memset(shr_thread_ctrl_t * th, void *input)
{
...
void dnxc_sw_state_parallel_data_block_zero_init(uint32 data_size, uint8* data_ptr)
{
    ...
    shr_thread_ctrl_t                   *shr_thread_control[DNXC_SW_STATE_THREADS_NUM];
    ...
    for (thread_idx = 0; thread_idx <DNXC_SW_STATE_THREADS_NUM; thread_idx++)
    {
        ...
        shr_thread_control[thread_idx] = shr_thread_start("mem_init_threads", 1, dnxc_sw_state_parallel_memset, &(mem_init[thread_idx]));
    }

    for (thread_idx = 0; thread_idx <DNXC_SW_STATE_THREADS_NUM; thread_idx++)
    {
        
        shr_thread_stop(shr_thread_control[thread_idx], SHR_THREAD_FOREVER);
    }
}
