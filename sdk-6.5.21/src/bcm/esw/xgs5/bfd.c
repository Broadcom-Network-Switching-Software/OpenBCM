                                 ...
                                 uint8 *encap_data)
{
    ...
    if(endpoint_info.vlan_tpid) {
        ...
        inner_l2.outer_vlan_tag.tci.vid  = endpoint_info.pkt_vlan_id;
        inner_l2.outer_vlan_tag.tci.prio =  endpoint_info.vlan_pri;
        num_vlans++;
    }

    if (endpoint_info.inner_vlan_tpid) {
        inner_l2.inner_vlan_tag.tpid = endpoint_info.inner_vlan_tpid;
        inner_l2.inner_vlan_tag.tci.vid  = endpoint_info.pkt_inner_vlan_id;
        inner_l2.inner_vlan_tag.tci.prio =  endpoint_info.inner_vlan_pri;
        num_vlans++;
    }
    ...
    if ((endpoint_info.type == bcmBFDTunnelTypeVxlanIPv4InIPv6) ||
        ...
        ipv4.ttl = 1;
        ipv4.protocol = SHR_BFD_IP_PROTOCOL_UDP;
        ipv4.src = endpoint_info.inner_src_ip_addr;
        ipv4.dst = endpoint_info.inner_dst_ip_addr;
        ...
        sal_memset(&ipv6, 0, sizeof(ipv6));
        ipv6.version = SHR_BFD_IPV6_VERSION;
        ipv6.t_class = endpoint_info.inner_ip_tos;
        ipv6.f_label = 0;
        ipv6.next_header = SHR_BFD_IP_PROTOCOL_UDP;
        ipv6.hop_limit = 1;
        ipv6.p_length = bfd_pkt_length + SHR_BFD_UDP_HEADER_LENGTH;
        sal_memcpy(ipv6.src, endpoint_info.inner_src_ip6_addr, sizeof(BCM_IP6_ADDRLEN));
        sal_memcpy(ipv6.dst, endpoint_info.inner_dst_ip6_addr, sizeof(BCM_IP6_ADDRLEN));
        cur_ptr = bcmi_xgs5_bfd_ipv6_header_pack(cur_ptr, &ipv6);
    }
                              ...
                              bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (endpoint_info->vrf_id) {
        session_flags |= SHR_BFD_SESS_SET_F_VRF_BASED_SESS;
    }
