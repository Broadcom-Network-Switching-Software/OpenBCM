 ...
 */
STATIC int
bcmi_xgs5_range_wb_reinit_1_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;range_oper_mode}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;1;%d}\n"), (*scache_pos),
              (uint32)RANGE_CTRL(unit)->range_oper_mode));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                            "S$END:range\n")));
    scache_p++;
    (*scache_pos)++;
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;num_ranges}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;1;%d}\n"), (*scache_pos),
              RANGE_CTRL(unit)->num_ranges));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                            "S$END:range\n")));
    scache_p++;
    (*scache_pos)++;
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;range_used_by_module}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;1;%d}\n"), (*scache_pos),
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;" \
            "pipe_instance;rid;udf_id;offset;width}\n")));
    if (RANGE_CTRL(unit)->range_used_by_module ==
        ...
        for (idx = index_min; idx <= index_max; idx ++) {
            ...
            if (RANGE_CTRL(unit)->range_oper_mode ==
                    bcmRangeOperModePipeLocal) {
                ...
                /* In Perpipe mode, only that particular
                 * pipe's PBMP will be used
                 */
            ...
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                ";%u"), (uint32)(range_p->udf_id)));
            scache_p += sizeof(bcm_udf_id_t);
            (*scache_pos) += sizeof(bcm_udf_id_t);

            /* Get the UDF offset */
            range_p->offset = *scache_p;
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                ";%d"), (uint8)(range_p->offset)));
            scache_p++;
            (*scache_pos)++;
            ...
            if ((SOC_MEM_FIELD_VALID(unit, mem, FIELD_SELECTf))) {
                soc_mem_field_get(unit, mem, tbl_entry,
                    FIELD_SELECTf, (uint32 *)&(range_p->rtype));
            } else {
                soc_mem_field_get(unit, mem, tbl_entry,
 ...
 */
STATIC int
bcmi_xgs5_range_wb_reinit_1_1(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    uint8       *scache_p;  /* Scache pointer */
    int max_num_pipes;
    int      temp_idx = 0;

    max_num_pipes = soc_scache_dictionary_entry_get(unit,
                                                    ssden_SOC_MAX_NUM_PIPES,
                                                    SOC_MAX_NUM_PIPES);
    scache_p = (uint8 *)(*scache_ptr);
    sal_memcpy(&(RANGE_CTRL(unit)->range_group_used_bitmap),
               scache_p,
               SOC_SCACHE_SIZE_MIN(sizeof(SHR_BITDCL) * max_num_pipes,
                                   sizeof(SHR_BITDCL) * SOC_MAX_NUM_PIPES));

    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;range_group_used_bitmap}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;%d;0x"), (*scache_pos),
             (int) (sizeof(SHR_BITDCL) * SOC_MAX_NUM_PIPES)));
    temp_idx = (int)(sizeof(SHR_BITDCL) * max_num_pipes);
    while (temp_idx > 0 ) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "%02x"), (unsigned char)(scache_p[temp_idx-1])));
        temp_idx--;
    }
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0, "}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0, "S$END:range\n")));
    (*scache_pos) += (sizeof(SHR_BITDCL) * SOC_MAX_NUM_PIPES);

    scache_p += sizeof(SHR_BITDCL) * max_num_pipes;
    /* fast forward the parameter pointer to where this function finished */
    *scache_ptr = scache_p;
 ...
 */
STATIC int
bcmi_xgs5_range_wb_sync_1_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    uint8       *scache_p;  /* Scache pointer */
    _range_t    *range_p;   /* Pointer to a range */
 ...
 */
STATIC int
bcmi_xgs5_range_wb_sync_1_1(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    uint8       *scache_p;  /* Scache pointer */
    int      temp_idx = 0;
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;%d;0x"), (*scache_pos),
             (int) (sizeof(SHR_BITDCL) * SOC_MAX_NUM_PIPES)));
    temp_idx = (sizeof(SHR_BITDCL) * SOC_MAX_NUM_PIPES);
    while (temp_idx > 0 ) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "%02x"), (unsigned char)(scache_p[temp_idx-1])));
        temp_idx--;
    ...
    scache_p += sizeof(SHR_BITDCL) * SOC_MAX_NUM_PIPES;
...
bcmi_xgs5_range_wb_sync(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_sync_1_0(unit, &scache_ptr, &scache_pos));
    BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_sync_1_1(unit, &scache_ptr, &scache_pos));
    return BCM_E_NONE;
}
...
bcmi_xgs5_range_reinit(int unit)
{
    ...
    if (BCM_E_NOT_FOUND == rv) {
        ...
        if (recovered_ver >= BCM_WB_VERSION_1_0) {
            BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_reinit_1_0(unit, &scache_ptr, &scache_pos));
        }
        if (recovered_ver >= BCM_WB_VERSION_1_1) {
            BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_reinit_1_1(unit, &scache_ptr, &scache_pos));
        }
    }
