 ...
 *
 * File:        port.c
 * Purpose:     Port Management for HURRICANE 4
 *
 *              The purpose is to set port management implementation specified
 *              to HURRICANE 4 in this file.
 */
...
static bcmi_xgs5_dev_info_t  *bcmi_hr4_dev_info[SOC_MAX_NUM_DEVICES];

/* HW Definitions */
static bcmi_xgs5_port_drv_t    bcmi_hr4_port_drv;


/*
 * Function:
 *      bcmi_hr4_port_dev_info_init
 * Purpose:
 ...
 */
int
bcmi_hr4_port_dev_info_init(int unit)
{
    ...
    /* Free bcmi_hr4_dev_info[unit] if already allocated */
    if (bcmi_hr4_dev_info[unit]) {
        sal_free(bcmi_hr4_dev_info[unit]);
        bcmi_hr4_dev_info[unit] = NULL;
    }

    /* Allocate memory for main SW data structure */
    alloc_size = sizeof(bcmi_xgs5_dev_info_t) +
                 (sizeof(bcmi_xgs5_phy_port_t) * (HURRICANE4_PHY_PORTS_PER_DEV));
    dev_info = sal_alloc(alloc_size, "bcmi_hr4_dev_info");
    if (dev_info == NULL) {
        return BCM_E_MEMORY;
    }
    sal_memset(dev_info, 0, alloc_size);
    dev_info->phy_port_info = (bcmi_xgs5_phy_port_t *)&dev_info[1];
    bcmi_hr4_dev_info[unit] = dev_info;

    /* Clear data structure */

    /* Device information init */
    dev_info = bcmi_hr4_dev_info[unit];
    dev_info->phy_ports_max = HURRICANE4_PHY_PORTS_PER_DEV;
    ...
    dev_info->ports_pipe_max[pipe] = HURRICANE4_PHY_PORTS_PER_PIPE;
     ...
     */
    dev_info->speed_class.speed_class_num = _HR4_TDM_SPEED_CLASS_MAX;
    ...
    /* dev_info->pipe_lr_bw = 560000; in HX5 */
    dev_info->pipe_lr_bw = 331000;

    /* Set valid speeds for 1/2 Lanes */
    ...
    for (phy_port = 1; phy_port < HURRICANE4_PHY_PORTS_PER_DEV; phy_port++) {
        ...
        for (i = 0; i < SOC_DRIVER(unit)->port_num_blktype; i++) {
            ...
            if ((SOC_BLOCK_TYPE(unit, blk) == SOC_BLK_CLPORT) ||
                ...
                (SOC_BLOCK_TYPE(unit, blk) == SOC_BLK_GXFPPORT) ||
                ...
                /* Single lane support on GPHY and PMQ instance 1 */
                if ((phy_port > 24) &&
        ...
        if (phy_port <= 24) {
            /* GPHY */
            dev_info->phy_port_info[phy_port].max_lane_speed = 1000;
        } else if (phy_port <= 56) {
            /* PMQ-0 & 1 */
            dev_info->phy_port_info[phy_port].max_lane_speed = 11000;
        } else if (phy_port <= 60) {
            /* FALCON */
...
/*
 * Function:
 *      bcmi_hr4_port_soc_resource_init
 * Purpose:
 ...
 */
STATIC int
bcmi_hr4_port_soc_resource_init(int unit, int nport,
                                ...
                                soc_port_resource_t *soc_resource)
{
    ...
    for (i = 0; i < nport; i++) {
        ...
        pipe = 0;
        /* Port Mapping is fixed */
        if ((phy_port > 0) && (phy_port <= 64) &&
            !(IS_OVERSUB_PORT(unit, si->port_p2l_mapping[phy_port])) &&
            !(resource[i].flags & SOC_PORT_RESOURCE_SPEED)) {
            idb_port = phy_port - 1;
            if (phy_port >= 57 && phy_port <= 64) {
                mmu_port = phy_port - 57;
            } else {
                mmu_port = phy_port + 7;
            }
        } else if (phy_port > 0) {
            ...
            mmu_port = -1;
            if (si->port_l2p_mapping[logic_port] != -1) {
                mmu_port = si->port_p2m_mapping[si->port_l2p_mapping[logic_port]];
            }
        }
...
/* Function:
 *      bcmi_hr4_port_resource_validate
 * Purpose:
 ...
 */
STATIC int
bcmi_hr4_port_resource_validate(int unit,
 ...
 */
STATIC int
bcmi_hr4_pre_flexport_tdm(int unit,
                            soc_port_schedule_state_t *port_schedule_state)
{
    port_schedule_state->is_flexport = 1;

    BCM_IF_ERROR_RETURN
        (soc_hr4_port_schedule_tdm_init(unit, port_schedule_state));
    return BCM_E_NONE;
}

/*
 * Function:
 *      bcmi_hr4_post_flexport_tdm
 * Purpose:
 ...
 */
STATIC int
bcmi_hr4_post_flexport_tdm(int unit,
                            soc_port_schedule_state_t *port_schedule_state)
{
    int mmu_port, port;
    soc_reg_t reg;
    ...
    BCM_IF_ERROR_RETURN
        (soc_hr4_soc_tdm_update(unit, port_schedule_state));
 ...
 */
STATIC int
bcmi_hr4_port_enable(int unit, soc_port_schedule_state_t *port_schedule_state)
{
    if (!soc_feature(unit, soc_feature_port_leverage)) {
        return BCM_E_NONE;
    }
    return soc_hurricane4_flex_top_port_up(unit, port_schedule_state);
}

/*
 * Function:
 *      bcmi_hr4_port_disable
 * Purpose:
 *      Disable port.
 * Parameters:
 *      unit                 - (IN)  Unit number.
 ...
 */
STATIC int
bcmi_hr4_port_disable(int unit, soc_port_schedule_state_t *port_schedule_state)
{
    if (!soc_feature(unit, soc_feature_port_leverage)) {
        return BCM_E_NONE;
    }
    return soc_hurricane4_flex_top_port_down(unit, port_schedule_state);
}

/*
 * Function:
 *      bcmi_hr4_speed_ability_get
 * Purpose:
 ...
 */
STATIC int
bcmi_hr4_speed_ability_get(int unit, bcm_port_t port, bcm_port_abil_t *mask)
{
    BCM_IF_ERROR_RETURN(soc_hr4_support_speeds(unit, port, mask));
    return BCM_E_NONE;
}

STATIC int
bcmi_hr4_port_attach_mmu(int unit, int port)
{
    ...
    BCM_IF_ERROR_RETURN
        (soc_hr4_mmu_config_init_port(unit, port));
 ...
 */
STATIC int
bcmi_hr4_port_detach_asf(int unit, bcm_port_t port)
{
    return BCM_E_NONE;
}
...
static bcm_esw_port_drv_t bcm_hr4_port_drv = {
    /* fn_drv_init                   */ bcmi_hr4_port_fn_drv_init,
...
static bcmi_xgs5_port_func_t bcmi_hr4_port_calls = {
    /* reconfigure_ports        */ soc_hurricane4_reconfigure_ports,
    /* soc_resource_init        */ bcmi_hr4_port_soc_resource_init,
    /* port_resource_validate   */ bcmi_hr4_port_resource_validate,
    /* pre_flexport_tdm         */ bcmi_hr4_pre_flexport_tdm,
    /* post_flexport_tdm        */ bcmi_hr4_post_flexport_tdm,
    /* port_macro_update        */ NULL,
    /* port_enable              */ bcmi_hr4_port_enable,
    /* port_disable             */ bcmi_hr4_port_disable,
    /* no_tdm_speed_update      */ bcmi_td3_port_speed_1g_update,
    /* speed_ability_get        */ bcmi_hr4_speed_ability_get,
    /* speed_mix_validate       */ NULL,
    /* cos_map_update           */ bcmi_hr4_port_cosmap_update,
};

static
int (*hr4_port_attach_exec[PORT_ATTACH_EXEC__MAX])(int unit, bcm_port_t port) = {
    /* MMU config init */
    &bcmi_hr4_port_attach_mmu,
...
static
int (*hr4_port_detach_exec[PORT_DETACH_EXEC__MAX])(int unit, bcm_port_t port) = {
    /* Other modules related to port */
#if defined(INCLUDE_L3)
 ...
 */
int
bcmi_hr4_port_fn_drv_init(int unit)
{
    int phase_pos, present;

    BCM_IF_ERROR_RETURN
        (bcmi_hr4_port_dev_info_init(unit));

    /* BCMI Driver init */
    if (!soc_feature(unit, soc_feature_port_leverage)) {
        bcmi_hr4_port_calls.reconfigure_ports = NULL;
    }

    bcmi_hr4_port_drv.port_calls = &bcmi_hr4_port_calls;
    bcmi_hr4_port_drv.dev_info[unit] = bcmi_hr4_dev_info[unit];


    sal_memcpy(bcmi_hr4_port_drv.port_calls->port_attach_exec,
               hr4_port_attach_exec,
               sizeof(bcmi_hr4_port_drv.port_calls->port_attach_exec));

    sal_memcpy(bcmi_hr4_port_drv.port_calls->port_detach_exec,
               hr4_port_detach_exec,
               sizeof(bcmi_hr4_port_drv.port_calls->port_detach_exec));
    ...
    if (soc_feature(unit, soc_feature_port_leverage)) {
        BCM_IF_ERROR_RETURN(soc_check_flex_phase(unit, hr4_flexport_phases,
                                                 soc_hurricane4_flex_top_port_down,
                                                 &present));
        /* Detach port enable/disable in DV Flexport */
        if (present) {
            BCM_IF_ERROR_RETURN
                (soc_detach_flex_phase(unit, hr4_flexport_phases,
                                       soc_hurricane4_flex_top_port_down,
                                       &phase_pos));
        }
        BCM_IF_ERROR_RETURN(soc_check_flex_phase(unit, hr4_flexport_phases,
