...
STATIC soc_field_t dip_31_23_f[3] = {
    LS_NAT_L3UC_DNAT__INNER_IPV4_DIP_31_23f,
    LS_NAT_L3UC_SNAT__INNER_IPV4_DIP_31_23f,
    LS_NAT_L3MC__INNER_IPV4_DIP_31_23f};

STATIC soc_field_t dip_22_0_f[3] = {
    LS_NAT_L3UC_DNAT__INNER_IPV4_DIP_22_0f,
    LS_NAT_L3UC_SNAT__INNER_IPV4_DIP_22_0f,
    LS_NAT_L3MC__INNER_IPV4_DIP_22_0f};
    ...
    uint32 *entry)
{
    ...
    if (nat_info->flags & BCM_L3_LARGE_NAT_EGRESS_MULTICAST)  {
        ...
        key_type = BCM_TD3_L3MC_NAT_KEY_TYPE(unit);
    }
        ...
        key_type = BCM_TD3_L3UC_SNAT_KEY_TYPE(unit);
    }
    else {
        nat_lookup = BCM_TD3_L3UC_DNAT_FIELDS;
        nat_info->flags |= BCM_L3_LARGE_NAT_EGRESS_DNAT;
        key_type = BCM_TD3_L3UC_DNAT_KEY_TYPE(unit);
    }
...
#if defined (BCM_FIREBOLT6_SUPPORT)
    /* IPV4 DIP is split between policy_data_0 and
     * policy_data_1 in the table. For FB6, another field
     * EDIT_CTRL_ID is 8 bit wide instead of 2 bits as in TD3/HX5/MV2.
     * As a result the bit positions and consequently name of
     * IPv4 DIP changes in the view definition in patch regsfile
     */
    if (SOC_IS_FIREBOLT6(unit)) {
        soc_mem_field32_set(unit, mem, entry, dip_22_0_f[nat_key],
            nat_info->dip_addr & 0x7fffff);

        soc_mem_field32_set(unit, mem, entry, dip_31_23_f[nat_key],
            nat_info->dip_addr>>23);
    } else
#endif
    {
    ...
    bcm_l3_large_nat_egress_t *nat_info)
{
    ...
    if (SOC_IS_FIREBOLT6(unit)) {
        nat_info->dip_addr = soc_mem_field32_get(unit, mem, entry,
            dip_22_0_f[nat_lookup]);

        nat_info->dip_addr |= soc_mem_field32_get(unit, mem, entry,
            dip_31_23_f[nat_lookup])<<23;
    } else
#endif
    {
        nat_info->dip_addr = soc_mem_field32_get(unit, mem, entry,
    ...
    bcm_l3_large_nat_egress_t *nat_info)
{
    ...
    if (key_type == BCM_TD3_L3MC_NAT_KEY_TYPE(unit)) {
    ...
    else if (key_type == BCM_TD3_L3UC_SNAT_KEY_TYPE(unit)) {
    ...
    else if (key_type == BCM_TD3_L3UC_DNAT_KEY_TYPE(unit)) {
        nat_info->flags |= BCM_L3_LARGE_NAT_EGRESS_DNAT;
        nat_lookup = BCM_TD3_L3UC_DNAT_FIELDS;
    }
    else {
