...
static sal_sem_t bst_detach_notify[BCM_MAX_NUM_UNITS];
...
STATIC int
soc_td2_hw_intr_cb_unregister(int unit)
{
#if defined(BCM_MONTEREY_SUPPORT)
    if (SOC_IS_MONTEREY(unit)) {
        BCM_IF_ERROR_RETURN(soc_monterey_set_bst_callback(unit, NULL));
        return SOC_E_NONE;
    }
#endif
#if defined(BCM_APACHE_SUPPORT)
    if (SOC_IS_APACHE(unit)) {
        BCM_IF_ERROR_RETURN(soc_apache_set_bst_callback(unit, NULL));
        return SOC_E_NONE;
    }
#endif
    BCM_IF_ERROR_RETURN(soc_td2_set_bst_callback(unit, NULL));
    return SOC_E_NONE;
}
...
_bcm_bst_td2_init(int unit)
{
    ...
    bst_info->hw_intr_cb_unregister = soc_td2_hw_intr_cb_unregister;
    bst_info->intr_enable_set = _bst_intr_enable_set;
...
void
_bcm_bst_hw_event_cb_unregister(void *unit_vp, void *d1, void *d2, void *d3,
                                void *d4)
{
    ...
    if (bst_info) {
        if (bst_info->hw_intr_cb_unregister) {
            bst_info->hw_intr_cb_unregister(unit);
        }
    }

    if (bst_detach_notify[unit] != NULL) {
        sal_sem_give(bst_detach_notify[unit]);
    }
}
...
int _bcm_bst_attach(int unit, _bcm_bst_device_handlers_t *cbs)
{
    ...
    if (bst_detach_notify[unit] == NULL) {
        bst_detach_notify[unit] = sal_sem_create("bst detach notify", sal_sem_BINARY, 0);
        if (bst_detach_notify[unit] == NULL) {
            (void)_bcm_bst_detach(unit);
            return BCM_E_MEMORY;
        }
...
int _bcm_bst_detach(int unit)
{
    ...
    defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_APACHE_SUPPORT)
    if (bst_detach_notify[unit] != NULL) {
        sal_dpc(_bcm_bst_hw_event_cb_unregister, INT_TO_PTR(unit), 0, 0, 0, 0);
        sal_sem_take(bst_detach_notify[unit], sal_sem_FOREVER);
    }
#endif
    ...
    if (bst_detach_notify[unit] != NULL) {
        sal_sem_destroy(bst_detach_notify[unit]);
        bst_detach_notify[unit] = NULL;
    }
