...
#define BSL_LSS_SCACHE_VERBOSE (BSL_LS_SHARED_SCACHE | BSL_VERBOSE)
...
char *elem_name[] = {
    ...
    "_bcmFieldInternalGroupDropMaskProfileAction",
...
static _field_type_map_t static_type_map[] = {
    ...
    {_bcmFieldInternalEndStageAEFTFP,       4, _FP_WB_TLV_NO_INGRESS
                                               |_FP_WB_TLV_NO_VALUE},
    {_bcmFieldInternalGroupDropMaskProfileAction,   4, _FP_WB_TLV_NO_TYPE
                                               | _FP_WB_TLV_LEN_PRESENT},
};
...
_tlv_print_array(int unit, void *value, int length, int chunk_sz)
{
    ...
    int scache_log_reqd = 0;

    if ((LOG_CHECK(BSL_LSS_SCACHE_VERBOSE)) && (wb_write_disable != 1)) {
        scache_log_reqd = 1;
...
tlv_write(int unit, _field_tlv_t *tlv, uint8 *ptr, uint32 *position)
{
    ...
    if ((LOG_CHECK(BSL_LSS_SCACHE_VERBOSE)) && (wb_write_disable != 1)) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:field:{offset;length;tlv_enum")));
        if (!(static_type_map[elem].flags & _FP_WB_TLV_NO_TYPE)) {
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ";tlv_enum_val")));
            len += sizeof(uint32);
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
             "    {0x%x;%d;%s"), (*position), len, elem_name[tlv->type]));
        if (!(static_type_map[elem].flags & _FP_WB_TLV_NO_TYPE)) {
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ";%d"), data));
        }
...
tlv_read(int unit, _field_tlv_t *tlv, uint8 *ptr, uint32 *position)
{
    uint32 pos;
    uint32 data = 0;
    uint32 data_temp = 0;  /* local variables for scache_pos and data */
    int read_size    = 0;     /* size used for value field */
    uint8 is_type_present = 0;
    int len = 0;
    ...
    if (!(recovery_type_map[unit][tlv->type].flags
        ...
        if (recovery_type_map[unit][tlv->type].flags
             & _FP_WB_TLV_LEN_ENCODED) {
              data_temp = (((tlv->length) & _FP_WB_TLV_TYPES_MASK) >>
                       _FP_WB_TLV_TYPE_SHIFT);
              read_size = data_temp * sizeof(uint32);
        }
    }

    if (LOG_CHECK(BSL_LSS_SCACHE_VERBOSE)) {
        /* write the header first */
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:field:{offset;length;tlv_enum")));
        if (!(is_type_present)) {
...
_field_presel_entry_sync(int unit)
{
    ...
    _field_tlv_t *tlv       = NULL;       /* tlv data */
    _field_control_t        *fc;          /* Field control structure.       */
    _field_action_t         *f_act;       /* Field action structure.       */
    ...
    uint32 endmarkerpresel  = _FIELD_WB_EM_PRESEL;
     ...
     int type_presel[13] =  {             /* types in _field_presel_entry_t */
    (int)((_bcmFieldInternalVariable << _FP_WB_TLV_BASIC_TYPE_SHIFT)
           | _bcmFieldInternalPreselId),
    (int)((_bcmFieldInternalVariable << _FP_WB_TLV_BASIC_TYPE_SHIFT)
           | _bcmFieldInternalPreselFlags),
    ...
    /* In the encoded length below , first ten bits is value 13 [no of types],
    ...
    TLV_CREATE(_bcmFieldInternalControlPreselInfo, _bcmFieldInternalVariable,
                (presel_ent_count | (13 << _FP_WB_TLV_TYPE_SHIFT)), &tlv);
    ...
    for (idx = 0; idx < BCM_FIELD_PRESEL_SEL_MAX ; idx++) {
        ...
        if ((NULL != f_pr) && (f_pr->presel_id != -1)
            ...
            if (f_pr->actions != NULL) {
               for (f_act = f_pr->actions;
                    (f_act) && (bcmFieldActionGroupClassSet != f_act->action);
                    f_act = f_act->next) {
                   /* Dont do anything. Loop exits when action is found */
               }
               if (f_act) {
            ...
            if (f_pr->actions != NULL) {
               for (f_act = f_pr->actions;
                    (f_act) && (bcmFieldActionDropMaskControlProfile != f_act->action);
                    f_act = f_act->next) {
                   /* Dont do anything. Loop exits when action is found */
               }
               if (f_act) {
                   TLV_CREATE(_bcmFieldInternalGroupDropMaskProfileAction,
                           _bcmFieldInternalArray,1, &tlv);
                   tlv->value = &(f_pr->actions->param[0]);
               } else {
                   TLV_CREATE(_bcmFieldInternalGroupDropMaskProfileAction,
        ...
        if ((stable_size > 0) && !SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit)) {
             ...
             if (scache_size == 0) {
                  ...
                  */
                  ...
                  /* scache size is incremented to sync the max_num_pipes
                   * for efp and vfp stages of th devices
                   */
                  scache_size += (2*4);
             }
            ...
            if (SOC_WARM_BOOT(unit)) {
                ...
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                    "S$BEGIN:field:{offset;length;version}\n")));
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                    "    {0x%x;2;%d}\n"), fc->scache_pos,  (uint16)(fc->wb_recovered_version)));
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                    "S$END:field\n")));
            }
                            ...
                            _field_group_t *new_fg, int total_qual_count)
{
    ...
    for (i = 0; i < num_instances; i++) {
        ...
        while  ((tlv2.type != _bcmFieldInternalEndStructPreselInfo)
            ...
            switch (tlv2.type) {
                ...
                case _bcmFieldInternalPreselClassAction:
                    ...
                    break;
                case _bcmFieldInternalGroupDropMaskProfileAction:
                    if (tlv2.length == 1) {
                        _field_action_t *fa;
                        ...
                        action->action = bcmFieldActionDropMaskControlProfile;
                        action->param[0] = *(uint32 *)tlv2.value;
                        fa = f_pr->actions;
                        if (fa == NULL) {
                            f_pr->actions = action;
        ...
        if (f_pr->group !=NULL)
        {
            ...
            BCM_IF_ERROR_CLEANUP(_bcm_field_th_lt_entry_tcam_parts_count(unit,
                                                                 fg->stage_id,
                                                                 fg->flags,
                                                                 &parts_count));
            ...
            for (idx = 0; (idx < parts_count && f_presel_p != NULL); idx++) {
                ...
                BCM_IF_ERROR_CLEANUP (_bcm_field_th_presel_mem_data_get(
                                                       unit, curr_stage_fc,
                                                       fg->instance,
                                                       f_presel_p,
                                                       &f_presel_p->lt_data));
    ...
    while (tlv.type != _bcmFieldInternalEndStructControl) {
        ...
        switch (tlv.type)
        {
            ...
            case _bcmFieldInternalControlMeterInUse:
                 num_pipes_rec = (tlv.length)/
                     ((recovery_type_map[unit][tlv.type].size) *
                       (_FIELD_MAX_METER_POOLS));
                 pipe_to_recover = ((num_pipes_rec < _FP_MAX_NUM_PIPES) ?
                           num_pipes_rec : _FP_MAX_NUM_PIPES); 
                 sal_memcpy(fc->ifp_em_meter_in_use, tlv.value,
                      ((recovery_type_map[unit][tlv.type].size) *
                           ( pipe_to_recover* _FIELD_MAX_METER_POOLS)));
                 break;
                               ...
                               uint8 action_param_type)
{
    ...
    switch (fa->action) {
            ...
            case bcmFieldActionGpTimeStampToCpu:
                 ...
                 break;
            case bcmFieldActionEgressFlowControlEnable:
                 if (bcmFieldEgressFlowControlCount == act->param[0]) {
                     act->param[0] = val_array[position++];
                 }
                 break;
            case bcmFieldActionEgressFlowEncapEnable:
                 if (bcmFieldEgressFlowEncapCount == act->param[0]) {
                     act->param[0] = val_array[position++];
                 }
                 break;
            case bcmFieldActionDstMacNew:
            ...
            if ((curr_action->action == bcmFieldActionRedirectBcastPbmp) ||
                ...
                if ((curr_action->action == bcmFieldActionEgressMask) &&
                    (soc_feature(unit, soc_feature_th3_style_fp))) {
                     BCM_IF_ERROR_CLEANUP(_bcm_field_th3_redirect_profile_get(unit,
                                                           &redirect_profile));

                } else
#endif
                {
                     BCM_IF_ERROR_CLEANUP(_field_trx_redirect_profile_get(unit,
                                                           &redirect_profile));
                }
    ...
    while (tlv.type != _bcmFieldInternalEndStructEntry) {
        ...
        switch (tlv.type)
        {
            ...
            case _bcmFieldInternalEntryPbmpFullData:
                 pbmp_max_words = (tlv.length)/
                     ((recovery_type_map[unit][tlv.type].size));
                 pbmp_max_words = ((pbmp_max_words < _SHR_PBMP_WORD_MAX) ?
                           pbmp_max_words : _SHR_PBMP_WORD_MAX);
        ...
        _field_stage_t *stage_fc)
{
    ...
    BCM_IF_ERROR_CLEANUP(_field_scache_stage_hdr_chk(fc, _FIELD_CLASS_DATA_END, 0));
cleanup:
    TLV_INIT(tlv);
                                         ...
                                         _field_action_bmp_t *act_bmp)
{
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        if (0 == valid) {
           rv1 = soc_cancun_ceh_obj_field_get(unit, "EXTRACTION_CTRL_ID",
                                              "INHERIT_VXLAN_GBP_TO_ANOTHER_TUNNEL",
                                              &ceh_info1);
           rv2 = soc_cancun_ceh_obj_field_get(unit, "EXTRACTION_CTRL_ID",
                                              "INHERIT_VXLAN_GBP_TO_ANOTHER_TUNNEL_MASK",
                                              &ceh_info2);
           ...
           if (act_val == ext_ctrl_id) {
              action = bcmFieldActionEgressFlowControlEnable;
              param0 = bcmFieldEgressFlowControlVxlanGbpSrcIdInherit;
              valid = 1;
           }
        }
        ...
        if (_BCM_FIELD_STAGE_INGRESS == f_ent->group->stage_id) {
            if ((NULL != act_bmp) && (NULL != act_bmp->w)) {
                if (SHR_BITGET(act_bmp->w, bcmFieldActionEgressFlowControlEnable)) {
                    action = bcmFieldActionEgressFlowControlEnable;
                    param0 = bcmFieldEgressFlowControlCount;
                    valid = 1;
                }
            }
        }
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        if (_BCM_FIELD_STAGE_INGRESS == f_ent->group->stage_id) {
            if ((NULL != act_bmp) && (NULL != act_bmp->w)) {
                if (SHR_BITGET(act_bmp->w, bcmFieldActionEgressFlowEncapEnable)) {
                    action = bcmFieldActionEgressFlowEncapEnable;
                    param0 = bcmFieldEgressFlowEncapCount;
                    valid = 1;
                }
            }
        }
                                    ...
                                    _field_stage_t *stage_fc)
{
    ...
    WB_FIELD_CONTROL_GET_WITH_LOCK(fc, ptr, pos);
    BCM_IF_ERROR_CLEANUP(_field_scache_stage_hdr_chk(fc, _FIELD_EM_DATA_START, 1));
    ...
    rv = _field_scache_stage_hdr_chk(fc, _FIELD_EM_DATA_END, 0);
    BCM_IF_ERROR_CLEANUP(rv);
                                   ...
                                   _field_stage_t   *stage_fc)
{
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "S$BEGIN:field:{offset;length;section_end_mark}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "    {0x%x;4;0x%x}\n"), ((fc->scache_pos)-4),
        (uint32)(temp)));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ...
                if (SHR_BITGET(bmp, i)) {
                    switch (action_arr[action_idx]) {
                        ...
                        case bcmFieldActionLoopbackType:
                            ...
                            if (act_val == a_offset.value[0]) {
                                param[0] = bcmFieldLoopbackTypeIfaVxLanDecapPayload;
                                break;
                            }
                            break;
                        case bcmFieldActionNatEgressOverride:
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    WB_FIELD_CONTROL_GET_WITH_LOCK(fc, scache_ptr, pos);
    BCM_IF_ERROR_CLEANUP(_field_scache_stage_hdr_chk(fc, _FIELD_FT_DATA_START, 1));
    ...
    rv = _field_scache_stage_hdr_chk(fc, _FIELD_FT_DATA_END, 0);
    BCM_IF_ERROR_CLEANUP(rv);
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    WB_FIELD_CONTROL_GET_WITH_LOCK(fc, scache_ptr, pos);
    BCM_IF_ERROR_CLEANUP(_field_scache_stage_hdr_chk(fc, _FIELD_AMFTFP_DATA_START, 1));
    ...
    rv = _field_scache_stage_hdr_chk(fc, _FIELD_AMFTFP_DATA_END, 0);
    BCM_IF_ERROR_CLEANUP(rv);
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    WB_FIELD_CONTROL_GET_WITH_LOCK(fc, scache_ptr, pos);
    BCM_IF_ERROR_CLEANUP(_field_scache_stage_hdr_chk(fc, _FIELD_AEFTFP_DATA_START, 1));
    ...
    rv = _field_scache_stage_hdr_chk(fc, _FIELD_AEFTFP_DATA_END, 0);
    BCM_IF_ERROR_CLEANUP(rv);
