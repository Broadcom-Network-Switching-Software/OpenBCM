    ...
    bcm_if_t * intf_id)
{
    ...
    if (BCM_L3_ITF_TYPE_IS_RIF(*intf_id))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data(unit, old_in_lif_profile, &in_lif_profile_info, intf_kind));
        }
    ...
    else if (BCM_L3_ITF_TYPE_IS_LIF(*intf_id))
    {
        ...
        if (rv == _SHR_E_NONE)
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit,
                                                                entry_handle_id, DBAL_FIELD_FODO, INST_SINGLE, &fodo));
    ...
    uint8 print_error_if_not_found)
{
    ...
    bcm_mac_t prefix;
    uint8 is_ingress_mymac_prefix_last, is_propag_profile_last;
    uint8 intf_exists = 0;
    uint8 is_vsi_allocated;
    int vsi;
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_l3_intf_get_delete_verify(unit, intf, &intf_exists));
    ...
    if (old_prefix_profile)
    {
        ...
        {
            SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ingress_mymac_prefix_table.free_single
                            (unit, old_prefix_profile, &is_ingress_mymac_prefix_last));
        }
    }
    ...
    if (is_vsi_allocated)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data(unit, vsi_profile, &in_lif_profile_info, ETH_RIF));
     ...
     */
    if (is_ingress_mymac_prefix_last)
    {
