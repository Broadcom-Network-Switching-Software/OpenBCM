    ...
    int unit)
{
    ...
    if (dnx_data_fabric.cell.pcp_force_no_packing_get(unit) == 1)
    {
    ...
    int unit)
{
    ...
    if (dnx_data_fabric.general.connect_mode_get(unit) == DNX_FABRIC_CONNECT_MODE_FE)
    {
        ...
        if (dnx_data_fabric.
            links.feature_get(unit, dnx_data_fabric_links_load_balancing_context_0_reserved_for_cpu_cells))
        {
            /*
             * By default the RR Load Balancing mechanism is using pointers from context 0.
             * In this case context 0 will be reserved for CPU cells and context 1 will be used for all others.
             */
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FABRIC_LOAD_BALANCING_CONTEXT_MAPPING, entry_handle_id));
            dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_FAP_ID, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOAD_BALANCING_CONTEXT, INST_SINGLE, 1);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }
        /*
        ...
        {
            /*
             * Disable MAC groups load balancing. By defaut it is enabled.
             */
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FABRIC_MAC_GROUPS_LOAD_BALANCING_CFG, entry_handle_id));
    ...
    switch (type)
    {
        ...
        case bcmFabricKeepAliveCellEnable:
        {
            if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_blocks_exist))
            {
                if (arg == TRUE)
                {
                    SHR_IF_ERR_EXIT(dnx_fabric_delay_test_ctrl_cells_enable_set(unit, 0));
                }
                else
                {
    ...
    bcm_fabric_pcp_mode_config_t * pcp_config)
{
    ...
    {
        SHR_INVOKE_VERIFY_DNX(dnx_fabric_pcp_dest_mode_config_set_verify(unit, flags, modid, pcp_config));
    ...
    bcm_fabric_pcp_mode_config_t * pcp_config)
{
    ...
    if (dnx_data_fabric.cell.pcp_enable_get(unit) == 1)
    {
        SHR_INVOKE_VERIFY_DNX(dnx_fabric_pcp_dest_mode_config_get_verify(unit, flags, modid, pcp_config));
