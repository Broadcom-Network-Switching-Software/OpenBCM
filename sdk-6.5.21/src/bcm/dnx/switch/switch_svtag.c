    ...
    uint32 entry_handle_id)
{
    ...
    else
    {
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OUT_TM_PORT, gport);
    }
    ...
    bcm_gport_t gport)
{
    ...
    else
    {
        ...
        if (!SWITCH_SVTAG_SUPPORTED_LIF_TYPES(gport) && !is_physical_port)
        {
    ...
    uint8 enable)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_switch_svtag_local_port_enable(unit, BCM_GPORT_LOCAL_GET(gport), enable));
    }
