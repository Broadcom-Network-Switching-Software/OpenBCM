    ...
    int *group_lsb_p)
{
    ...
    *group_lsb_p = context_info.key_id.lsb[0];
    ...
    dbal_enum_value_field_field_io_e * field_io_p)
{
    ...
    /** Sanity check. */
    if ((key_id_p->id[0] == DBAL_NOF_ENUM_FIELD_KEY_VALUES) || (key_id_p->id[1] == DBAL_NOF_ENUM_FIELD_KEY_VALUES))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal key ID.\r\n");
    }
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    if (key_id_p->id[1] == DBAL_NOF_ENUM_FIELD_KEY_VALUES)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal second key.\r\n");
    }
    /*
    ...
    /** Verify that if we do have double key, the second key is allocated starting from the first bit> */
    if ((key_id_p->id[1] != DNX_FIELD_KEY_ID_INVALID) && (key_id_p->lsb[1] != 0))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Second key begins with bit %d, must begin at zero.\r\n", key_id_p->lsb[1]);
    }
    /** Sanity checks */
    ...
    bit_range_offset_within_key = key_id_p->lsb[0];
            ...
            else
            {
                     ...
                     dnx_data_field.stage.stage_info_get(unit, field_stage)->fem_key_select_resolution_in_bits) != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 ...
                                 fg_id, context_id, context_info.key_id.lsb[0],
                                 dnx_data_field.stage.stage_info_get(unit,
                                                                     field_stage)->fem_key_select_resolution_in_bits);
                }
                actions_lsb = context_info.key_id.lsb[0] + relative_actions_lsb;
            }
        }
    ...
    dnx_field_context_t context_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_key_detach(unit, context_id, &key_in_info, &(context_info.key_id)));
    SHR_IF_ERR_EXIT(dnx_field_group_fes_unconfig(unit, key_in_info.field_stage, fg_id, context_id));
    /**Remove the Program Id from SW of the given Field Group*/
    SHR_IF_ERR_EXIT(dnx_field_attach_context_info_t_init(unit, &context_info));
    SHR_IF_ERR_EXIT(dnx_field_group_context_info_set(unit, fg_id, context_id, &context_info));
    SHR_IF_ERR_EXIT(dnx_field_group_fems_context_detach(unit, fg_id, context_id));
    ...
    dnx_field_group_cache_mode_e mode)
{
    ...
    switch (mode)
    {
        case DNX_FIELD_GROUP_CACHE_MODE_START:
             ...
             */
            dnx_field_group_tcam_entry_cache[fg_id].cache_size = DNX_FIELD_TCAM_INITIAL_CACHE_SIZE;
            dnx_field_group_tcam_entry_cache[fg_id].cache_mode = DNX_FIELD_GROUP_CACHE_MODE_START;
            dnx_field_group_tcam_entry_cache[fg_id].tcam_entries = NULL;
            SHR_ALLOC_SET_ZERO(dnx_field_group_tcam_entry_cache[fg_id].tcam_entries,
                               sizeof(*(dnx_field_group_tcam_entry_cache[fg_id].tcam_entries)) *
        ...
        case DNX_FIELD_GROUP_CACHE_MODE_INSTALL:
             ...
             */
            qsort(dnx_field_group_tcam_entry_cache[fg_id].tcam_entries,
            ...
            {
                /*
                 * WITH_ID flag is on, since entry_handler war already allocated
                 */
                SHR_IF_ERR_EXIT(dnx_field_entry_tcam_install
                                (unit, DNX_FIELD_ENTRY_ADD_FLAG_WITH_ID, fg_id,
                                 &(dnx_field_group_tcam_entry_cache[fg_id].tcam_entries[entry_idx].entry_info),
             ...
             */
            dnx_field_group_tcam_entry_cache[fg_id].cache_mode = DNX_FIELD_GROUP_CACHE_MODE_CLEAR;
            SHR_FREE(dnx_field_group_tcam_entry_cache[fg_id].tcam_entries);
            dnx_field_group_tcam_entry_cache[fg_id].tcam_entries = NULL;
            dnx_field_group_tcam_entry_cache[fg_id].cache_size = 0;
            dnx_field_group_tcam_entry_cache[fg_id].nof_valid_entries = 0;
            break;
        case DNX_FIELD_GROUP_CACHE_MODE_CLEAR:
            for (entry_idx = 0; entry_idx < dnx_field_group_tcam_entry_cache[fg_id].nof_valid_entries; entry_idx++)
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(dnx_field_entry_access_id_deallocate
                                (unit, dnx_field_group_tcam_entry_cache[fg_id].tcam_entries[entry_idx].entry_handle));
            }
            /*
             * Set cache mode to Clear and free the cache tcam_entries
             */
    ...
    dnx_field_group_cache_mode_e mode)
{
    ...
    switch (mode)
    {
        ...
        case DNX_FIELD_GROUP_CACHE_MODE_CLEAR:
        {
            uint32 nof_entries_to_clear = dnx_field_group_tcam_entry_cache[fg_id].nof_valid_entries;
            /** Set cache mode to Clear and set the cached entries to 0 */
            dnx_field_group_tcam_entry_cache[fg_id].cache_mode = DNX_FIELD_GROUP_CACHE_MODE_CLEAR;
            dnx_field_group_tcam_entry_cache[fg_id].nof_valid_entries = 0;
            /** Clear cached entries */
            SHR_IF_ERR_EXIT(dnx_kbp_cache_db_clear(unit, dbal_table_id, nof_entries_to_clear));
            break;
        }
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Cache mode: %d not supported\r\n", mode);
    ...
    dnx_field_group_cache_mode_e mode)
{
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_KBP:
        {
            SHR_IF_ERR_EXIT(dnx_field_group_kbp_cache(unit, flags, fg_id, mode));
            break;
        }
#endif
        default:
    ...
    dnx_field_group_t fg_id)
{
    ...
    if (fg_type == DNX_FIELD_GROUP_TYPE_TCAM || fg_type == DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Before deleting Field Group=%d need to Clear all entries on cache\n", fg_id);
        }
        SHR_IF_ERR_EXIT(dnx_field_group_tcam_handler_id_get(unit, fg_id, &tcam_handler_id));
    ...
    int unit)
{
    ...
    uint32 user_opcodes[DNX_DATA_MAX_FIELD_KBP_APPTYPE_USER_NOF];
    int opcode_idx;
    ...
    uint32 opcode_that_uses_zero_padding = DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_INVALID;
    kbp_mngr_key_segment_t ms_key_segments[DNX_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY];
    uint32 nof_segments;
    ...
    int unit)
{
    ...
    {
        dnx_field_group_tcam_entry_cache[fg_idx].cache_mode = 0;
        dnx_field_group_tcam_entry_cache[fg_idx].cache_size = 0;
        dnx_field_group_tcam_entry_cache[fg_idx].nof_valid_entries = 0;
        dnx_field_group_tcam_entry_cache[fg_idx].tcam_entries = NULL;
    }
    SHR_FUNC_EXIT;
}
