    ...
    uint8 is_set)
{
    ...
    UTILEX_U64 time64;
    dnxcmn_time_t time_user;
    dnxcmn_time_t time_max;
    uint32 log2_val = 0;
    int granularity;
    ...
    if (is_set == TRUE)
    {
        switch (type)
        {
            case bcmCosqPFCDeadlockDetectionTimer:
                ...
                COMPILER_64_SET(time_user.time, time64.arr[1], time64.arr[0]);
                time_user.time_units = DNXCMN_TIME_UNIT_MILISEC;
                /**
                 ...
                 */
                SHR_IF_ERR_EXIT(dnxcmn_clock_cycles_to_time_get(unit, 0x7FFFFFFF, DNXCMN_TIME_UNIT_MILISEC, &time_max));
                 ...
                 */
                SHR_IF_ERR_EXIT(dnxcmn_time_to_clock_cycles_get(unit, &time_user, &interval_in_clocks));
                ...
                SHR_IF_ERR_EXIT(dnxcmn_clock_cycles_to_time_get
                                (unit, closest_interval_in_clocks, DNXCMN_TIME_UNIT_NSEC, &time_user));
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_cosq_pfc_deadlock_control_verify(unit, port, pfc_priority, type, arg, TRUE));
    ...
    int *arg)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_cosq_pfc_deadlock_control_verify(unit, port, pfc_priority, type, *arg, FALSE));
