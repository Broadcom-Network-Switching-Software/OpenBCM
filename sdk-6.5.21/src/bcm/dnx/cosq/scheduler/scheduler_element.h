...
#define DNX_SCHEDULER_ELEMENT_IS_CL(unit, se_id) \
    (dnx_data_sch.se.quartet_offset_to_cl_num_get(unit, DNX_SCH_CONVERT_SE_ID_TO_QUARTET_OFFSET(unit, se_id))->cl_num != -1)

#define DNX_SCHEDULER_ELEMENT_CL_ID_GET(unit, se_id) \
    (dnx_data_sch.se.quartet_offset_to_cl_num_get(unit, DNX_SCH_CONVERT_SE_ID_TO_QUARTET_OFFSET(unit, se_id))->cl_num \
