    ...
    uint32 *fqp_min_gap)
{
    ...
    for (idx = 0; idx < dnx_data_egr_queuing.params.if_speed_params_info_get(unit)->key_size[0]; idx++)
    {
        if (dnx_data_egr_queuing.params.if_speed_params_get(unit, idx, clk_idx)->if_id == if_id)
        {
            ...
            {
                /** If the corredponfing rate is set to -1, 
                  * then rate is not relevgant to the amount of credits allocated to this interface */
                break;
            }

            if (if_speed <= dnx_data_egr_queuing.params.if_speed_params_get(unit, idx, clk_idx)->speed)
            {
                /** If the corredponfing rate is not set to -1,
    ...
    int speed)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info))
    {
        ...
        {
            /** Check if the RCY master ports already were set and have speed */
            ...
            if (has_speed)
            {
                ...
                /**
                 * On each RCY interface there is only one master port. Sum the old speed of the master ports
                 * in order to determine how many (if any) credits need to be removed from the total 
                 * Always remove the credits for bothe RCY interfaces, because function expects that 
                 * the input speed will also be a total for bothe RCY interfaces 
                 */
                prev_if_speed += if_speed_i;
            }
        }
    }
    else
    ...
    if (next_master_port == DNX_ALGO_PORT_INVALID)
    {
        if (dnx_data_esb.general.feature_get(unit, dnx_data_esb_general_esb_support) == 1)
        {
            if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info))
            {
                ...
                BCM_PBMP_ITER(all_rcy_master_ports, rcy_port_i)
                {
                    /** Check if the other RCY master port has speed. If it does, the remaining total speed 
                      * of the commom RCY TXI will not be 0 */
                    if (port != rcy_port_i)
                    {
                ...
                /** The below function is called in order to update the SW state for total egress IF credits 
                  * For RCY interfaces, this function must be called with the total speed for bothe RCY interfaces */
