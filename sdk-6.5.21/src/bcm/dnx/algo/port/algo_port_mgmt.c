    ...
    int unit)
{
    ...
    int nof_out_tm_ports, nof_pp_dsp, nof_ptc;
    int nof_cores;
    int nof_ilkn_if;
    int phy;
    ...
    else
    {
         ...
         */
        {
             ...
             */
            nof_cores = dnx_data_device.general.nof_cores_get(unit);
            nof_ptc = dnx_data_port.general.nof_ptc_get(unit);
            SHR_IF_ERR_EXIT(dnx_algo_port_db.in_tm.alloc(unit, nof_cores, nof_ptc));
    ...
    dnx_algo_port_if_add_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_db_interface_id_get
                    (unit, port_info, info->core, info->interface_offset, &info->phys, info->common_info.channel,
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_channel_free_verify(unit, logical_port, interface_id, info->common_info.channel));
    }
    ...
    dnx_algo_port_db_2d_handle_t interface_id)
{
    ...
    {
        int if_master_port, is_channelized, is_ingress_interleave;
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_db.interface.is_ingress_interleave.get(unit,
                                                                             interface_id.h0, interface_id.h1,
        ...
        if (!DNX_ALGO_PORT_TYPE_IS_L1(unit, port_info) && DNX_ALGO_PORT_TYPE_IS_L1(unit, master_port_info))
        {
         ...
         /** TDM interface mode */
        SHR_IF_ERR_EXIT(dnx_algo_port_db.interface.tdm_mode.get(unit, interface_id.h0, interface_id.h1, &if_tdm_mode));
    ...
    dnx_algo_port_db_2d_handle_t interface_id)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_ING_TM(unit, port_info))
    {
        ...
        {
            /** mark as valid */
            SHR_IF_ERR_EXIT(dnx_algo_port_db.in_tm.ref_count.set(unit, in_tm_handle.h0, in_tm_handle.h1, 1));
            /** Set IN TM port */
            SHR_IF_ERR_EXIT(dnx_algo_port_db.in_tm.in_tm_port.set(unit, in_tm_handle.h0, in_tm_handle.h1,
                                                                  info->in_tm_port));
            /** Set core */
            SHR_IF_ERR_EXIT(dnx_algo_port_db.in_tm.core.set(unit, in_tm_handle.h0, in_tm_handle.h1, core));
            /** set master port (of IN TM DB!)*/
            SHR_IF_ERR_EXIT(dnx_algo_port_db.in_tm.master_logical_port.set(unit, in_tm_handle.h0, in_tm_handle.h1,
                                                                           logical_port));
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_db.in_tm.ref_count.inc(unit, in_tm_handle.h0, in_tm_handle.h1, 1));
        }
    ...
    if (DNX_ALGO_PORT_TYPE_IS_PP(unit, port_info, 0 /* not in lag */ ))
    {
         ...
         */
        /** is is not allowed to have 2 ports with the same PP DSP */
        ...
        if (DNX_ALGO_PORT_TYPE_IS_CPU(unit, port_info))
        {
            ...
            DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_port_channel_free_verify
                                (unit, logical_port, cpu_interface_id, info->common_info.channel));
            }
        }
    ...
    int num_priorities)
{
     ...
     * Set data
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.out_tm.priorities.set(unit, out_tm_handle.h0, out_tm_handle.h1, num_priorities));
