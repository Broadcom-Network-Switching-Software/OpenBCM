 ...
 */
shr_error_e
dnx_algo_port_out_tm_valid_verify(
    int unit,
    bcm_core_t core,
    bcm_port_t out_tm_port)
{
    ...
    if (!valid)
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "OUT TM port expected to be valid (core %d, OUT TM port %d) .\n", core, out_tm_port);
    }
    ...
    bcm_port_t in_tm_port)
{
    ...
    if (valid)
    {
        ...
        else
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PORT, "IN TM port is in use by non RCY MIRROR port %d (core %d, in tm port %d) .\n",
