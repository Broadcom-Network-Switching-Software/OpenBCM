    ...
    dnx_algo_local_outlif_logical_phase_e * logical_phase)
{
    ...
    strlen_input = sal_strlen(logical_phase_str);
    strlen_logical = sal_strlen(DNX_ALGO_LIF_OUTLIF_LOGICAL_STAGE_STRING);
    ...
    dnx_algo_local_outlif_hw_info_t * outlif_hw_info)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_lif_mngr_lif_allocate_verify
                          (unit, flags, &(global_lif_info->global_lif), inlif_info, outlif_info, outlif_hw_info));
    ...
    {
        SHR_NULL_CHECK(global_lif_info, _SHR_E_INTERNAL, "global_lif");
    }
 ...
 */
    ...
    if (is_ingress && allocate_global_lif)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_lif_mapping_create(unit, DNX_ALGO_LIF_INGRESS, global_lif_tmp, &local_lif_info, TRUE));
    }
    ...
    if (is_egress && allocate_global_lif)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_lif_mapping_create(unit, DNX_ALGO_LIF_EGRESS, global_lif_tmp, &local_lif_info, TRUE));
    }
