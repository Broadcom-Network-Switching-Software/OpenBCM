...
shr_error_e
dnx_algo_global_out_lif_bta_hw_get(
    int unit,
    int global_lif,
    int *bta)
{
    uint32 entry_handle_id;
    int field_size, global_lif_size;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    *bta = 0;
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_egress_estimated_bta_btr_config))
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGRESS_GLOB_OUT_LIF_BTA_SOP_TABLE, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOB_OUT_LIF_MSB,
                                   (global_lif / (1 << (global_lif_size - field_size))));
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_BTA_SOP, INST_SINGLE, (uint32 *) bta);
    ...
    void *tag_info)
{
    ...
    int nof_grains =
        resource_tag_bitmap_get_nof_grains(res_tag_bitmap->count, res_tag_bitmap->grainSize, res_tag_bitmap->low,
