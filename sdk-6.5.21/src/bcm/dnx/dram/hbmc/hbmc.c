    ...
    dnx_hbmc_mode_registers_t * mrs)
{
    ...
    uint32 read_latency_hw_val = dnx_data_dram.general_info.read_latency_get(unit) - 2;

    uint32 write_latency_hw_val = dnx_data_dram.general_info.write_latency_get(unit) - 1;

    uint32 tras = dnx_data_dram.general_info.timing_params_get(unit)->tras;
    ...
    int hbm_index)
{
    ...
    dnx_hbmc_mode_registers_default_values_get(unit, &mrs);
