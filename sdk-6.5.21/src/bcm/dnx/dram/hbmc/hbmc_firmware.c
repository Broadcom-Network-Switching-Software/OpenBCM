...
dnx_hbmc_firmware_aapl_sbus_access(
    ...
    uint * sbus_data)
{
    return (_SHR_E_FAILURE(dnx_hbmc_firmware_aapl_sbus_access_internal(aapl, addr, reg_addr, command, sbus_data))) ?
        FALSE : TRUE;
}

shr_error_e
dnx_hbmc_firmware_aapl_cmd(
    int unit,
    char *command)
{
    int is_mutex_locked = FALSE;
    ...
    SHR_IF_ERR_EXIT(aapl->return_code);

    aapl_main_str(aapl, command);
    SHR_IF_ERR_EXIT(aapl->return_code);
