    ...
    else
    {
        ...
        *label_count = encap_info.labels_nof;
        ...
        if (gport_hw_resources.global_out_lif != DNX_ALGO_GPM_LIF_INVALID)
        {
            ...
            {
                /** value -1 means that estimated_encap_size will be determine internally by sdk, */
                /** therefore, no need to return it back */
                if (label_array[0].estimated_encap_size != -1)
                {
                    SHR_IF_ERR_EXIT(dnx_algo_global_out_lif_bta_hw_get
                                    (unit, gport_hw_resources.global_out_lif, &label_array[0].estimated_encap_size));
                }
            }
            else
            {
                label_array[0].estimated_encap_size = -1;
            }
        }
        else
        {
