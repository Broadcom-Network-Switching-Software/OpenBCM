    ...
    int *trap_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, type, &trap_type_info));
    ...
    int *nof_traps_p)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, i_trap, &trap_type_info));
    ...
    char *trap_name)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, trap_type, &trap_type_info));
...
{
    uint8 keep_original;
    uint32 keep_original_hw_context_id;
    int trap_code_loop_limit, trap_code_start, trap_code_idx;

    int trap_code_and_context[3][2] = {
        {DBAL_ENUM_FVAL_EGRESS_FWD_ACTION_PROFILE_USER_TRAP_1, DBAL_ENUM_FVAL_TRAP_CONTEXT_ID_GENERAL___USER_TRAP_1},
        {DBAL_ENUM_FVAL_EGRESS_FWD_ACTION_PROFILE_USER_TRAP_2, DBAL_ENUM_FVAL_TRAP_CONTEXT_ID_GENERAL___USER_TRAP_2},
        {DBAL_ENUM_FVAL_EGRESS_FWD_ACTION_PROFILE_USER_TRAP_3, DBAL_ENUM_FVAL_TRAP_CONTEXT_ID_GENERAL___USER_TRAP_3}
    };
    ...
    if (trap_code == DBAL_NOF_ENUM_EGRESS_FWD_ACTION_PROFILE_VALUES)
    {
        keep_original = FALSE;
        trap_code_start = 0;
        trap_code_loop_limit = 3;
    }
    else
    {
        keep_original = TRUE;
        trap_code_start = trap_code - DBAL_ENUM_FVAL_EGRESS_FWD_ACTION_PROFILE_USER_TRAP_1;
        trap_code_loop_limit = trap_code_start + 1;
    }
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_TRAP_CONTEXT_ID,
                                                  ...
                                                  &keep_original_hw_context_id));

    for (trap_code_idx = trap_code_start; trap_code_idx < trap_code_loop_limit; trap_code_idx++)
    {
        uint32 user_trap_val = 0, user_trap_cotext_val = 0;
        uint32 data[SOC_REG_ABOVE_64_MAX_SIZE_U32] = { 0 };
        int cs_line = 0;

        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_EGRESS_FWD_ACTION_PROFILE, trap_code_and_context[trap_code_idx][0],
                         &user_trap_val));
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_TRAP_CONTEXT_ID, trap_code_and_context[trap_code_idx][1],
                         &user_trap_cotext_val));

        for (cs_line = 0; cs_line < 48; cs_line++)
        {
            int field_0 = 0, field_1 = 0;
            SHR_IF_ERR_EXIT(soc_mem_read(unit, ETPPB_TRAP_CONTEXT_SELECTION_CAMm, -1, cs_line, data));
            field_0 = soc_mem_field32_get(unit, ETPPB_TRAP_CONTEXT_SELECTION_CAMm, data, FORWARD_ACTION_PROFILEf);
            field_1 = soc_mem_field32_get(unit, ETPPB_TRAP_CONTEXT_SELECTION_CAMm, data, OAM_SUB_TYPEf);

            if ((field_0 == user_trap_val) && (field_1 == 0))
            {
                if (keep_original)
                {
                    soc_mem_field32_set(unit, ETPPB_TRAP_CONTEXT_SELECTION_CAMm, data, PROGRAMf,
                                        ((keep_original_hw_context_id << 1) | 1));
                }
                else
                {
                    soc_mem_field32_set(unit, ETPPB_TRAP_CONTEXT_SELECTION_CAMm, data, PROGRAMf,
                                        ((user_trap_cotext_val << 1) | 1));
                }
                SHR_IF_ERR_EXIT(soc_mem_write(unit, ETPPB_TRAP_CONTEXT_SELECTION_CAMm, -1, cs_line, data));
            }
        }
    }
    ...
    int trap_id)
{
    ...
    if (is_oam_punt_or_protection == FALSE)
    {
        ...
        if (trap_type_info.trap_class != DNX_RX_TRAP_CLASS_USER_DEFINED)
        {
    ...
    int *is_strengthless)
{
    ...
    *is_strengthless = TRUE;
    ...
    if (ingress_ud_info.valid == FALSE)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, trap_type_idx, &predefined_trap_info));
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    switch (trap_class)
    {
        case DNX_RX_TRAP_CLASS_USER_DEFINED:
            ...
            {
                SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_drop_hw_set(unit, &etpp_config_p->action));
            }
            else
            {
        ...
        case DNX_RX_TRAP_CLASS_USER_DEFINED:
            switch (trap_type)
            {
                ...
                case DNX_RX_TRAP_ID_TYPE_GET(BCM_RX_TRAP_EG_TX_TRAP_ID_DROP):
                    ...
                    SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_drop_hw_get(unit, &(etpp_config_p->action)));
                    break;
                default:
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    erpp_config.core_config_arr = config_p->core_config_arr;
    erpp_config.core_config_arr_len = config_p->core_config_arr_len;
    dnx_rx_trap_etpp_config_t_init(unit, &etpp_config);
    ...
    uint8 *is_legal)
{
    ...
    *is_legal = FALSE;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, oam_etpp_traps[trap_type], &trap_type_info));
        if (trap_id == DNX_RX_TRAP_ID_SET(DNX_RX_TRAP_BLOCK_ETPP, DNX_RX_TRAP_CLASS_OAM, trap_type_info.trap_id))
        {
            *is_legal = TRUE;
        }
    }
    ...
    int unit)
{
    ...
    for (erpp_snif_profile = 0; erpp_snif_profile < nof_erpp_snif_profiles; ++erpp_snif_profile)
    {
        ...
        etpp_snif_profile = DNX_MIRROR_EGRESS_PROFILE_GET(erpp_snif_profile);
...
*   \param [out] new_in_lif_profile_p  -  the new inlif profile allocated
*               \b As \b input - \n
*               Pointer contains the new inlif profile
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e - Error type
    ...
    int *new_in_lif_profile_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange(unit, &in_lif_profile_info, old_in_lif_profile,
                                                new_in_lif_profile_p, intf_type, inlif_dbal_table_id));
    SHR_IF_ERR_EXIT(dnx_in_lif_vsi_profile_set(unit, gport, *new_in_lif_profile_p, result_type, intf_type));
    ...
    bcm_rx_trap_protocol_profiles_t * protocol_profiles_p)
{
    ...
    if (protocol_trap_mode == DNX_RX_TRAP_PROTOCOL_MODE_IN_LIF)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_rx_trap_protocol_in_lif_profile_set
                        (unit, port, LIF, gport_hw_resources.inlif_dbal_table_id, *protocol_profiles_p,
                         &new_in_lif_profile));
