    ...
    uint32 val)
{
         ...
         FLEXE_CORE_1588_SRC_MAC_LO))
    {
        ...
        if (flags & (FLEXE_CORE_1588_DEST_MAC_HI | FLEXE_CORE_1588_SRC_MAC_HI))
        {
            new_mac_hi = (val >> 8) & 0xFFFF;
            new_mac_lo = ((new_mac_lo & 0xFFFFFF) | ((val & 0xFF) << 24));
        }
        else if (flags & (FLEXE_CORE_1588_DEST_MAC_LO | FLEXE_CORE_1588_SRC_MAC_LO))
        {
