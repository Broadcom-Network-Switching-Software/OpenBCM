    ...
    uint32 *is_copy_inner)
{
    if ((tunnel->type == bcmTunnelTypeIpAnyIn4) && (tunnel->dip == 0) && (tunnel->sip == 0) &&
        (tunnel->l3_intf_id != 0) && (tunnel->l3_intf_id != BCM_IF_INVALID)
    ...
    uint32 *local_outlif)
{
    ...
    /** Set TTL model */
    if (tunnel->egress_qos_model.egress_ttl == bcmQosEgressModelUniform || tunnel_is_copy_inner)
    {
        /** TTL UNIFORM model */
    ...
    if (((tunnel->egress_qos_model.egress_qos == bcmQosEgressModelPipeNextNameSpace) ||
         (tunnel->egress_qos_model.egress_qos == bcmQosEgressModelPipeMyNameSpace)) && !tunnel_is_copy_inner)
    {
