    ...
    uint32 curr_state)
{
    ...
    if (dnx_data_sat.general.accum_count_not_collect_during_unavaliable_status_get(unit))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PKT_LOST_THRESHOLD, INST_SINGLE, lost_threshold);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CURR_STATE, INST_SINGLE, curr_state);
    }
    ...
    uint32 *curr_state)
{
    ...
    if (dnx_data_sat.general.accum_count_not_collect_during_unavaliable_status_get(unit))
    {
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_PKT_LOST_THRESHOLD, INST_SINGLE, lost_threshold);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CURR_STATE, INST_SINGLE, curr_state);
    }
    ...
    bcm_sat_ctf_stat_t * stat)
{
    ...
    dnx_sat_ctf_stat_per_flow_config_get(unit, ctf_id, &window_size, &update_cntrs_always, &lost_threshold,
                                         &current_state);
    ...
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    dnx_sat_ctf_stat_per_flow_config_set(unit, ctf_id, window_size, update_cntrs_always, lost_threshold, current_state);
