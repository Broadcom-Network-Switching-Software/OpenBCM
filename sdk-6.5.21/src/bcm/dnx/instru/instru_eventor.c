...
int dnx_instru_eventor_write_dma_descriptors(
    int unit,
...
static inline int
dnx_instru_eventor_do_buffers_collide(
    uint32 buf1_start,
    uint32 buf1_end,
    uint32 buf2_start,
    uint32 buf2_end)
    ...
    uint8 *enable)
{
    ...
    if (nof_sources >= bcmInstruSyncedCountersTypeCount)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "nof sources %d exceed the number of available sources %d.\n", nof_sources,
                     bcmInstruSyncedCountersTypeCount);
    }
    ...
    for (source_index = 0; source_index < nof_sources; source_index++)
    {
        switch (sources[source_index])
        {
            case bcmInstruSyncedCountersTypeIcgm:
                dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_CGM_ENABLE, INST_SINGLE,
                                            enable[source_index]);
                break;
            case bcmInstruSyncedCountersTypeNif:
                dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_CDU_ENABLE, INST_SINGLE,
                                            enable[source_index]);
                break;
            case bcmInstruSyncedCountersTypeCrps:
                dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_CRPS_ENABLE, INST_SINGLE,
                                            enable[source_index]);
                break;
            default:
        ...
        switch (sources[source_index])
        {
            case bcmInstruSyncedCountersTypeIcgm:
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                                (unit, entry_handle_id, DBAL_FIELD_CGM_ENABLE, INST_SINGLE, &enable[source_index]));
                break;
            case bcmInstruSyncedCountersTypeNif:
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                                (unit, entry_handle_id, DBAL_FIELD_CDU_ENABLE, INST_SINGLE, &enable[source_index]));
                break;
            case bcmInstruSyncedCountersTypeCrps:
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                                (unit, entry_handle_id, DBAL_FIELD_CRPS_ENABLE, INST_SINGLE, &enable[source_index]));
                break;
            default:
    ...
    bcm_instru_eventor_event_id_config_t * config)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field16_get
                    (unit, entry_handle_id, DBAL_FIELD_EVENTOR_CONTEXT_ID, INST_SINGLE, &config->context));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
    ...
    switch (context_type)
    {
        case bcmEventorContextTypeRx:
            ...
            for (context_i = 0; context_i < DNX_EVENTOR_NOF_CONTEXTS(unit); context_i++)
            {
                ...
                {
                    SHR_IF_ERR_EXIT(instru.eventor_info.rx_contexts_info.
                                    buffer0_offset.get(unit, context_i, &buf_i1_start));
                    SHR_IF_ERR_EXIT(instru.eventor_info.rx_contexts_info.
                                    buffer1_offset.get(unit, context_i, &buf_i2_start));

                    buf_i1_bank = buf_i1_start & DNX_EVENTOR_SRAM_BANK_MASK(unit);
                    buf_i2_bank = buf_i2_start & DNX_EVENTOR_SRAM_BANK_MASK(unit);

                    SHR_IF_ERR_EXIT(instru.eventor_info.
                                    tx_sram_banks_bitmap.bit_get(unit, buf_i1_bank, &bank_1_is_used_in_tx));
                    SHR_IF_ERR_EXIT(instru.eventor_info.
                                    tx_sram_banks_bitmap.bit_get(unit, buf_i2_bank, &bank_2_is_used_in_tx));
                    if (bank_1_is_used_in_tx || bank_2_is_used_in_tx)
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "May not use same SRAM bank for RX and TX contexts: bank1: %u used by tx: %u bank2: %u used by tx: %u.\n",
                    ...
                    if (conf->bank1 == buf_i1_bank
                        && dnx_instru_eventor_do_buffers_collide(conf->buffer1_start, buffer1_end, buf_i1_start,
                                                                 buf_i1_end))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "context %u 1st buffer bank %u word offset %u collides with specified 1st buffer.\n",
                                     context_i, buf_i1_bank, buf_i1_start);
                    }
                    else if (conf->bank2 == buf_i1_bank
                             && dnx_instru_eventor_do_buffers_collide(conf->buffer2_start, buffer2_end, buf_i1_start,
                                                                      buf_i1_end))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "context %u 1st buffer bank %u word offset %u collides with specified 2nd buffer.\n",
                                     context_i, buf_i1_bank, buf_i1_start);
                    }
                    else if (conf->bank1 == buf_i2_bank
                             && dnx_instru_eventor_do_buffers_collide(conf->buffer1_start, buffer1_end, buf_i2_start,
                                                                      buf_i2_end))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "context %u 2nd buffer bank %u word offset %u collides with specified 1st buffer.\n",
                                     context_i, buf_i2_bank, buf_i2_start);
                    }
                    else if (conf->bank2 == buf_i2_bank
                             && dnx_instru_eventor_do_buffers_collide(conf->buffer2_start, buffer2_end, buf_i2_start,
                                                                      buf_i2_end))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "context %u 2nd buffer bank %u word offset %u collides with specified 2nd buffer.\n",
                                     context_i, buf_i2_bank, buf_i2_start);
                    }
                }
            }
        ...
        case bcmEventorContextTypeTx:
            ...
            for (context_i = 0; context_i < DNX_EVENTOR_NOF_CONTEXTS(unit); context_i++)
            {
                ...
                if ((buffer_size_i > 0) && (context != context_i))
                {
                    SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                                    buffer0_offset.get(unit, context_i, &buf_i1_start));
                    SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                                    buffer1_offset.get(unit, context_i, &buf_i2_start));
                    ...
                    SHR_IF_ERR_EXIT(instru.eventor_info.
                                    rx_sram_banks_bitmap.bit_get(unit, buf_i1_bank, &bank_1_is_used_in_rx));
                    SHR_IF_ERR_EXIT(instru.eventor_info.
                                    rx_sram_banks_bitmap.bit_get(unit, buf_i2_bank, &bank_2_is_used_in_rx));
                    if (bank_1_is_used_in_rx || bank_2_is_used_in_rx)
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "May not use same SRAM bank for RX and TX contexts: bank1: %u used by rx: %u bank2: %u used by rx: %u.\n",
                                     buf_i1_bank, bank_1_is_used_in_rx, buf_i1_bank, bank_2_is_used_in_rx);
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "SRAM bank is not TX: bank1 %u is_rx_bank %u bank2 %u is_rx_bank %u.\n",
                                     buf_i1_bank, bank_1_is_used_in_rx, buf_i1_bank, bank_2_is_used_in_rx);
                    }
...
int
bcm_dnx_instru_eventor_context_set(
    int unit,
    uint32 flags,
    bcm_eventor_context_id_t context,
    bcm_eventor_context_type_t context_type,
    ...
    switch (context_type)
    {
        case bcmEventorContextTypeRx:
            ...
            SHR_IF_ERR_EXIT(instru.eventor_info.rx_contexts_info.buffer0_offset.set(unit, context, conf->bank1 |
                                                                                    (conf->buffer1_start <<
                                                                                     DNX_EVENTOR_SRAM_BANK_BITS
                                                                                     (unit))));
            SHR_IF_ERR_EXIT(instru.eventor_info.rx_contexts_info.buffer1_offset.set(unit, context,
                                                                                    conf->bank2 | (conf->
                                                                                                   buffer2_start <<
                                                                                                   DNX_EVENTOR_SRAM_BANK_BITS
                                                                                                   (unit))));
        ...
        case bcmEventorContextTypeTx:
            ...
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.buffer0_offset.set(unit, context, conf->bank1 |
                                                                                    (conf->buffer1_start <<
                                                                                     DNX_EVENTOR_SRAM_BANK_BITS
                                                                                     (unit))));
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.buffer1_offset.set(unit, context,
                                                                                    conf->bank2 | (conf->
                                                                                                   buffer2_start <<
                                                                                                   DNX_EVENTOR_SRAM_BANK_BITS
                                                                                                   (unit))));
            ...
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                            program0_offset.set(unit, context, conf->program1_start));
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                            program1_offset.set(unit, context, conf->program2_start));
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                            nof_operations.set(unit, context, conf->nof_operations));
...
int
bcm_dnx_instru_eventor_context_get(
    int unit,
    uint32 flags,
    bcm_eventor_context_id_t context,
    bcm_eventor_context_type_t context_type,
    bcm_instru_eventor_context_conf_t * out_conf)
{
    ...
    else if (is_tx_context && (context_type == bcmEventorContextTypeTx || context_type == bcmEventorContextTypeAny))
    {
        ...
        SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                        program0_offset.get(unit, context, &out_conf->program1_start));
        SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                        program1_offset.get(unit, context, &out_conf->program2_start));
        SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                        nof_operations.get(unit, context, &out_conf->nof_operations));
        SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                        nof_const_values.get(unit, context, &out_conf->nof_const_values));
    }
    else
    {
        ...
        for (context_i = 0; context_i < DNX_EVENTOR_NOF_CONTEXTS(unit); context_i++)
        {
            ...
            if (is_tx_context)
            {
                ...
                if (builders & (0x1 << builder))
                {
                    context_found = TRUE;
                    SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                                    buffer_size.get(unit, context_i, &buffer_size));
                    if (conf->thresh_size <= 0 || (conf->thresh_size % buffer_size))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "thresh_size given %u is not a positive multiple of the buffer size %u.\n",
                                     conf->thresh_size, buffer_size);
                    }
                    else if (conf->thresh_size > DNX_EVENTOR_BUILDER_MAX_BUFFER_SIZE_WORDS(unit))
                    {
...
static shr_error_e
dnx_instru_eventor_builder_tx_header_set(
    int unit,
    bcm_eventor_builder_id_t builder,
    bcm_instru_eventor_builder_conf_t * conf)
{
    ...
    dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_TRANSMIT_HEADER_STACK_DATA_HIGH, INST_SINGLE,
                                    header_data_high);
    dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_TRANSMIT_HEADER_STACK_DATA_LOW, INST_SINGLE,
                                    header_data_low);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EVPCK_HEADER_STACK_SIZE, INST_SINGLE,
                                 DNX_EVENTOR_EVPCK_DEFAULT_HEADER_STACK_SIZE);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EVPCK_HEADER_STACK_EN, INST_SINGLE, 0);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EVPCK_HEADER_STACK_NOF_SAMPLES, INST_SINGLE,
                                 nof_samples);
...
int
bcm_dnx_instru_eventor_builder_set(
    int unit,
    uint32 flags,
    bcm_eventor_builder_id_t builder,
    bcm_instru_eventor_builder_conf_t * conf)
...
static shr_error_e
dnx_instru_eventor_builder_tx_header_get(
    int unit,
    bcm_eventor_builder_id_t builder,
    bcm_instru_eventor_builder_conf_t * conf)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_TRANSMIT_HEADER_STACK_SIZE, INST_SINGLE, &conf->header_length));
    ...
    if (conf->header_length > 0)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                        (unit, entry_handle_id, DBAL_FIELD_TRANSMIT_HEADER_STACK_DATA_HIGH, INST_SINGLE,
                         header_data_high));
        ...
        length =
            (conf->header_length >
             (DNX_EVENTOR_TRANSMIT_HEADER_MAX_SIZE / 2)) ? (DNX_EVENTOR_TRANSMIT_HEADER_MAX_SIZE /
                                                            2) : conf->header_length;
        ...
        if (length > 0)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                            (unit, entry_handle_id, DBAL_FIELD_TRANSMIT_HEADER_STACK_DATA_LOW, INST_SINGLE,
                             header_data_low));
...
int
bcm_dnx_instru_eventor_builder_get(
    int unit,
    uint32 flags,
    bcm_eventor_builder_id_t builder,
    bcm_instru_eventor_builder_conf_t * out_conf)
    ...
    int active)
{
    ...
    for (context_i = 0; context_i < DNX_EVENTOR_NOF_CONTEXTS(unit); context_i++)
    {
        ...
        if (is_rx_context && buffer_size_i)
        {
            ...
            SHR_IF_ERR_EXIT(instru.eventor_info.
                            builder_configured_bitmap.bit_get(unit, context_i, &is_builder_configured));

            if (is_builder_configured == 0)
            {
                LOG_ERROR(BSL_LOG_MODULE,
                          ("Context %u configured as RX, and the same builder was not configured.\n", context_i));
                error = 1;
            }
            else
            {
                ...
                dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CONTEXT_QUEUE_MAX_SIZE, INST_SINGLE,
                                           &thresh_size);
                SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
                ...
                {
                    LOG_ERROR(BSL_LOG_MODULE,
                              ("RX context %u has a builder thresh_size %u which is not a positive multiple of the rx buffer size %u.\n",
    ...
    uint8 is_tx_context)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BUFFER_END_ADDR, 0,
                                 buff0_start_address + buffer_size);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BUFFER_START_ADDR, 1, buff1_start_address);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BUFFER_END_ADDR, 1,
                                 buff1_start_address + buffer_size);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
int
bcm_dnx_instru_eventor_active_set(
    int unit,
    uint32 flags,
    int active)
{
    ...
    uint32 context_i, bank_i, buff0_start_address, buff1_start_address, sw_state_flags, buffer_size_i, program0_offset,
        program1_offset, builders;
    uint32 index;
    ...
    for (context_i = 0; context_i < DNX_EVENTOR_NOF_CONTEXTS(unit); context_i++)
    {
        ...
        if (is_rx_context)
        {
            SHR_IF_ERR_EXIT(instru.eventor_info.rx_contexts_info.buffer_size.get(unit, context_i, &buffer_size_i));
            SHR_IF_ERR_EXIT(instru.eventor_info.rx_contexts_info.
                            buffer0_offset.get(unit, context_i, &buff0_start_address));
            SHR_IF_ERR_EXIT(instru.eventor_info.rx_contexts_info.
                            buffer1_offset.get(unit, context_i, &buff1_start_address));
            program0_offset = 0;
            program1_offset = 0;
            ...
            if (buffer_size_i)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_instru_eventor_active_context_set
                                (unit, context_i, buffer_size_i, buff0_start_address, buff1_start_address,
                                 program0_offset, program1_offset, FALSE));
            }
        }
    }
    ...
    for (context_i = 0; context_i < DNX_EVENTOR_NOF_CONTEXTS(unit); context_i++)
    {
        ...
        if (is_tx_context)
        {
            ...
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                            buffer1_offset.get(unit, context_i, &buff1_start_address));
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                            program0_offset.get(unit, context_i, &program0_offset));
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.
                            program1_offset.get(unit, context_i, &program1_offset));
            SHR_IF_ERR_EXIT(instru.eventor_info.tx_contexts_info.builders.get(unit, context_i, &builders));
            ...
            if (buffer_size_i)
            {
                SHR_IF_ERR_EXIT(dnx_instru_eventor_active_context_set
                                (unit, context_i, buffer_size_i, buff0_start_address, buff1_start_address,
                                 program0_offset, program1_offset, TRUE));
    ...
    nof_sbus_dma_channel_values =
        soc_property_get_csv(unit, spn_EVENTOR_SBUS_DMA_CHANNELS, MAX_NOF_EVENTOR_SBUS_DMA_CHANNELS,
                             sbus_dma_channel_values);
    ...
    for (context_i = 0; context_i < DNX_EVENTOR_NOF_CONTEXTS(unit); context_i++)
    {
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_EVENTOR_CONTEXT_ID, context_i);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_STANDARD_QUEUE_TYPE, INST_SINGLE,
                                     (queue_type ? 1 : 0));
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CMIC_FIFO_DMA_CFG_WR_STROBE, INST_SINGLE,
                                 0xffffffff);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    bcm_instru_access_info_t * access_info)
{
    ...
    if (soc_reg_xaddr_get(unit, reg, instance, array_index,
                          ...
                          &reg_access_info) != SOC_E_NONE || reg_access_info.num_blks != 1)
    {
    ...
    access_info->entry_size =
        SOC_REG_IS_ABOVE_64(unit, reg) ? SOC_REG_ABOVE_64_INFO(unit, reg).size : (SOC_REG_IS_64(unit, reg) ? 2 : 1);
    ...
    const bcm_instru_eventor_context_conf_t * context_conf)
{
    ...
    eventor_chain_address[0] =
        DNX_EVENTOR_AXI_SRAM_OFFSET(unit) + DNX_EVENTOR_SRAM_BANKS_FULL_SIZE(unit) * context_conf->bank1 +
        context_conf->program1_start * 4;
    eventor_chain_address[1] =
        DNX_EVENTOR_AXI_SRAM_OFFSET(unit) + DNX_EVENTOR_SRAM_BANKS_FULL_SIZE(unit) * context_conf->bank2 +
        context_conf->program2_start * 4;
    eventor_buffer_address[0] =
        DNX_EVENTOR_AXI_SRAM_OFFSET(unit) + DNX_EVENTOR_SRAM_BANKS_FULL_SIZE(unit) * context_conf->bank1 +
        context_conf->buffer1_start * 4;
    eventor_buffer_address[1] =
        DNX_EVENTOR_AXI_SRAM_OFFSET(unit) + DNX_EVENTOR_SRAM_BANKS_FULL_SIZE(unit) * context_conf->bank2 +
        context_conf->buffer2_start * 4;
    ...
    for (buffer_index = 0; buffer_index < DNX_EVENTOR_NOF_BUFFERS_IN_DOUBLE_BUFFER; buffer_index++)
    {
        for (desc_index = 0; desc_index < context_conf->nof_operations; desc_index++)
        {
            ...
            desc_cfg_ptr->flags =
                (operation_ptr->access_info.flags & BCM_INSTRU_ACCESS_FLAG_IS_WRITE) ? SOC_SBUSDMA_WRITE_CMD_MSG : 0;
        ...
        desc_ptr =
            (void *) (sal_vaddr_t) EVENTOR_SRAM_DIRECT_MEM_ACCESS(unit,
                                                                  (eventor_chain_address[buffer_index] & 0xfffff));
