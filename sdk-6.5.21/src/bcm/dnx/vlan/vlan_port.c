    ...
    int is_virtual_ac)
{
    ...
    supported_flags =
        ...
        BCM_VLAN_PORT_RECYCLE | BCM_VLAN_PORT_VLAN_TRANSLATION_TWO_VLAN_TAGS | BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE |
        BCM_VLAN_PORT_EXTENDED;
    ...
    if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
    {
        ...
        if (vlan_port->group == RING_PROTECTION_MACT_GROUP_IN_JR1_MODE)
        {
            ...
            if (vlan_port->failover_port_id == 0)
            {
    ...
    bcm_vlan_port_t * old_vlan_port)
{
    ...
    supported_flags =
        ...
        BCM_VLAN_PORT_VLAN_TRANSLATION_TWO_VLAN_TAGS | BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE | BCM_VLAN_PORT_EXTENDED;
    ...
    if ((new_vlan_port->flags & ~supported_flags) != ((old_vlan_port->flags & ~supported_flags)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error! VLAN-Port REPLACE: Cannot change flags!\n"
                     ...
                     "or BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE(0x%08X)!\n",
                     ...
                     BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE);
    }
    ...
    int is_native)
{
    ...
    if (is_native == FALSE)
    {
        ...
        {
            if ((learn_context == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_OPTIMIZED) ||
    ...
    int *is_p2p)
{
    ...
    *is_p2p = FALSE;
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_INGRESS_WIDE))
    {
        ...
        /*
         * Set LIF_GENERIC_DATA_1 to support 28bits, otherwise only 8bits are supported
    ...
    dnx_ingress_ac_table_info_t * ingress_ac_table_info)
{
    ...
    if (is_native == FALSE)
    {
        ...
        else
        {
            ...
            {
                uint32 spa_type_shift, spa_mask;
            ...
            || (forward_info->fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY))
        {
            ...
            else if (ingress_ac_table_info->learn_context == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC)
            {
 ...
 * The function sets HW for ESEM CMD if necesary
 * \param [in] unit - Unit #
    ...
    if (!is_replace)
    {
        ...
        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vlan_port_esem_cmd_set
                            (unit, TRUE, old_esem_cmd_profile, &is_esem_cmd_last_reference,
                             &(egress_ac_table_info->esem_cmd)));
        }
    ...
    else
    {
        ...
        if (gport_hw_resources->outlif_dbal_table_id == DBAL_TABLE_EEDB_OUT_AC)
        {
            ...
            if (is_esem_cmd_field_on_handle)
            {
                ...
                if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE))
                {
                    SHR_IF_ERR_EXIT(dnx_vlan_port_esem_cmd_set(unit, TRUE, old_esem_cmd_profile,
                ...
                else
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT(dnx_vlan_port_esem_cmd_set(unit, FALSE, old_esem_cmd_profile,
                                                                   &is_esem_cmd_last_reference,
                                                                   &(egress_ac_table_info->esem_cmd)));
                    }
                }
                ...
                if (is_virtual_ac == TRUE)
                {
                    ...
                    if (!is_replace)
                    {
                        SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress_match_esem_sw_state
                                        (unit, vlan_port, DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_AC_STAT));
                    }
                }
            }
            break;
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress_match_esem_sw_state
                    (unit, &esem_default_vlan_port, DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_AC_STAT));
    ...
    dnx_ingress_ac_fields_table_info_get_t * ingress_ac_fields_info)
{
         ...
         DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                        (unit, entry_handle_id, DBAL_FIELD_LEARN_PAYLOAD_CONTEXT, &is_field_on_handle,
                         &dbal_struct_data));
        if (dbal_struct_data == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC)
        {
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    if (rv != _SHR_E_NOT_FOUND)
    {
        SHR_IF_ERR_EXIT(dnx_switch_svtag_is_present_in_esem_cmd(unit, esem_cmd_profile, &svtag_present));
        if (svtag_present)
        {
