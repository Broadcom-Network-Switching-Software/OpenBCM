    ...
    uint32 *result_type)
{
    ...
    else
    {
        if (dbal_table_type == DBAL_TABLE_TYPE_EM)
        {
            ...
            {
                /*
                 * If VID value is valid, then DESTINATION_ONLY result type will be defined for the IVL table.
    ...
    uint32 *dbal_table)
{
    ...
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        ...
        else if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
        {
            ...
            if (data->mod_id > 0)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "LPM IVL table for Bridge IPV6 is not yet supported. \n");
            }
            else
            {
