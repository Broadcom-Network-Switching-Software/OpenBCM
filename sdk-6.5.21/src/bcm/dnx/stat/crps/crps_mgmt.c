...
static shr_error_e
dnx_crps_mgmt_crps_ire_scan_cfg_init(
    int unit)
{
    ...
    /** Timeout send a packet that is full but didn't reach to the desired packet size [cycles] =
     * 255 << ire_packing_watchdog_cfg */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IRE_PACKING_WATCHDOG_CFG, INST_SINGLE, 3);
    ...
    int unit)
{
    ...
    if (dnx_data_crps.engine.feature_get(unit, dnx_data_crps_engine_slim_counters_is_supported))
    {
        SHR_IF_ERR_EXIT(dnx_crps_mgmt_crps_ire_scan_cfg_init(unit));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int arg)
{
    ...
    field_val = utilex_log2_round_up(arg / 3);
