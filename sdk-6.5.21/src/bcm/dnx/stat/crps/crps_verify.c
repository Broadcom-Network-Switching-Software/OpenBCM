    ...
    bcm_stat_counter_database_t * database)
{
    ...
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        ...
        while (current_engine_id != DNX_CRPS_MGMT_ENGINE_INVALID)
        {
            ...
            if (engine_info->nof_counters != C_TYPE_ENGINE_SIZE(unit))
            {
...
static shr_error_e
dnx_crps_mgmt_counter_interface_set_slim_format_verify(
    int unit,
    bcm_stat_counter_database_t * database)
{
    ...
    /*
     * Valid the device support slim counters
     */
    if (!dnx_data_crps.engine.feature_get(unit, dnx_data_crps_engine_slim_counters_is_supported))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "format_type=%d\n is not supported", bcmStatCounterFormatSlimPacketsAndBytes);
    }

    /*
     * valid all the engines attached to the database are from type C
     */
    SHR_INVOKE_VERIFY_DNX(dnx_crps_mgmt_all_c_engines_verify(unit, database));

    /*
     * valid there are less than 8 slim databases
     */
    SHR_INVOKE_VERIFY_DNX(dnx_crps_mgmt_slim_database_avilable_verify(unit, database));
    ...
    bcm_stat_counter_format_type_t format_type)
{
    ...
    {
        SHR_INVOKE_VERIFY_DNX(dnx_crps_mgmt_counter_interface_set_slim_format_verify(unit, database));
    }
    ...
    bcm_stat_counter_interface_t * config)
{
    ...
    if ((flags & BCM_STAT_COUNTER_MODIFY_RANGE_ONLY) == FALSE)
    {
        SHR_INVOKE_VERIFY_DNX(dnx_crps_mgmt_counter_interface_set_conter_format_verify(unit,
                                                                                       database, config->format_type));
    }
    ...
    bcm_stat_eviction_t * eviction)
{
    ...
    /** eviction type range */
    if (eviction->type < bcmStatEvictionDestinationLocalHost || eviction->type >= bcmStatEvictionDestinationSize)
    {
    ...
    /** LFSR is not supported for 68b counter format */
    ...
    else                                                                                         /** Thus parameters are not relevant for network eviction type */
    {
        /** verify dma fifo */
