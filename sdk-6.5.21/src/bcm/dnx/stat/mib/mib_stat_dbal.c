...
static uint32 *ctrs_saved_fmac_symb_error[SOC_MAX_NUM_DEVICES];
/*
 * Define the bits definition in SW
 * - Bits 0-14 stand for 1,2 ... ..., 15 errors
 * - Bit 15 stand for uncorrectable error
 */
#define DNX_STAT_FEC_SYMB_ERROR_PER_FRAME_NOF_BITS               16
/*
 * Define uncorrectable error indication bit in DBAL for 15T and 5T FEC
 */
#define DNX_15T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL   15
#define DNX_5T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL    5
    ...
    int unit)
{
    ...
    if (ctrs_saved_fmac_symb_error[unit] == NULL &&
        dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_is_symb_errors_per_frame_supported))
    {
        number_of_entries = dnx_data_fabric.links.nof_links_get(unit);
        ctrs_saved_fmac_symb_error[unit] = sal_alloc(number_of_entries * sizeof(uint32), "ctrs_saved_fmac_symb_error");
        sal_memset(ctrs_saved_fmac_symb_error[unit], 0, number_of_entries * sizeof(uint32));
    }
}
    ...
    int unit)
{
    ...
    {
        SHR_FREE(ctrs_saved_fmac_symb_error[unit]);
    }
}
 ...
 */
shr_error_e
dnx_mib_fmac_symb_error_dbal_get(
    int unit,
    bcm_port_t port,
    int counter_id,
    uint64 *val)
{
    uint32 entry_handle_id;
    int link_id, symb_err_indication_bit, error_indication;
    uint32 symb_error_val;
    ...
    if (counter_data->offset_in_row == 0)
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FMAC_SYMB_ERRORS_PER_FRAME,
                                   INST_SINGLE, &ctrs_saved_fmac_symb_error[unit][link_id]);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
        /*
        ...
        {
            error_indication =
                SHR_BITGET(&ctrs_saved_fmac_symb_error[unit][link_id],
                           DNX_5T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL);
            if (error_indication)
            {
                SHR_BITCLR(&ctrs_saved_fmac_symb_error[unit][link_id],
                           DNX_5T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL);
                SHR_BITSET(&ctrs_saved_fmac_symb_error[unit][link_id], DNX_STAT_FEC_SYMB_ERROR_PER_FRAME_NOF_BITS - 1);
            }
        }
    }
    /*
     * Get counter value from SW DB that stored the request values
     * Only the MSB is relevant for this counter
     */
    symb_err_indication_bit =
        shr_bitop_last_bit_get(&ctrs_saved_fmac_symb_error[unit][link_id], DNX_STAT_FEC_SYMB_ERROR_PER_FRAME_NOF_BITS);
    if (symb_err_indication_bit == counter_data->offset_in_row)
    {
        symb_error_val = 1;
    }
    else
    ...
    COMPILER_64_SET(*val, 0, symb_error_val);
