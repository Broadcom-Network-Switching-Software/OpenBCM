    ...
    uint32 *clear_on_read)
{
    ...
    if (counter_id >= dnx_mib_counter_rx_fec_1_symbol_error_per_frame &&
        counter_id <= dnx_mib_counter_rx_fec_uncorrrectable_errors_per_frame)
    {

        SHR_IF_ERR_EXIT(dnx_mib_fmac_symb_error_dbal_get(unit, port, counter_id, val));
        SHR_EXIT();
    }
