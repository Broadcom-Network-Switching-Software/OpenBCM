...
ipmc_hitbit_flexctr_cleanup(int unit)
{
    ...
    info->hit_flexctr_enabled = false;
...
int
bcmint_ipmc_l2_tbl_get(int unit,
                       uint32_t tbl_id,
                       const bcmint_ipmc_l2_tbl_t **tbl_info)
{
    bcmint_ipmc_l2_tbl_db_t tbl_db = {0};

    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_EXIT
        (mbcm_ltsw_ipmc_l2_tbl_db_get(unit, &tbl_db));

    if (!(tbl_db.tbl_bmp & (1ULL << tbl_id))) {
                              ...
                              bcm_ipmc_addr_t *ipmc)
{
    ...
    ipmc->s_ip_mask   = l3_fib->sip_addr_mask;
    ...
    sal_memcpy(ipmc->s_ip6_mask, l3_fib->sip6_addr_mask, BCM_IP6_ADDRLEN);
    ...
    } else if (ltsw_feature(unit, LTSW_FT_ROUTE_SIMULATE_HOST)) {
        ipmc->ing_intf = l3_fib->l3_iif ? l3_fib->l3_iif : BCM_IF_INVALID;
    }
...
bcm_ltsw_ipmc_add(int unit, bcm_ipmc_addr_t *data)
{
    ...
    if (data->flags & BCM_IPMC_L2) {
        ...
        SHR_IF_ERR_EXIT
            (mbcm_ltsw_ipmc_l2_add(unit, data));
        SHR_EXIT();
    }
    ...
    if (!subnet || ltsw_feature(unit, LTSW_FT_ROUTE_SIMULATE_HOST)) {
...
bcm_ltsw_ipmc_remove(int unit, bcm_ipmc_addr_t *data)
{
    ...
    if (data->flags & BCM_IPMC_L2) {
        ...
        SHR_IF_ERR_EXIT
            (mbcm_ltsw_ipmc_l2_remove(unit, data));
        SHR_EXIT();
    }
...
bcm_ltsw_ipmc_remove_all(int unit)
{
    ...
    rv = mbcm_ltsw_ipmc_l2_delete_all(unit);
...
bcm_ltsw_ipmc_find(int unit, bcm_ipmc_addr_t *data)
{
    ...
    if (data->flags & BCM_IPMC_L2) {
        ...
        SHR_IF_ERR_EXIT
            (mbcm_ltsw_ipmc_l2_find(unit, data));
        SHR_EXIT();
    }
                       ...
                       bcm_ipmc_traverse_cb cb, void *user_data)
{
    ...
    rv = mbcm_ltsw_ipmc_l2_traverse(unit, flags, cb, user_data);
    SHR_IF_ERR_EXIT_EXCEPT_IF(rv, SHR_E_UNAVAIL);
                              ...
                              int *untag, bcm_vlan_t *vid, int *ttl_threshold)
{
    ...
    SHR_NULL_CHECK(untag, SHR_E_PARAM);
    SHR_NULL_CHECK(vid, SHR_E_PARAM);
    SHR_NULL_CHECK(ttl_threshold, SHR_E_PARAM);
                          ...
                          uint32 stat_counter_id)
{
    ...
    if (info->flags & BCM_IPMC_L2) {
        SHR_IF_ERR_EXIT
            (mbcm_ltsw_ipmc_l2_flexctr_info_get(unit, info, &pre_ci));
    } else {
        SHR_IF_ERR_EXIT
    ...
    if (info->flags & BCM_IPMC_L2) {
        SHR_IF_ERR_CONT
            (mbcm_ltsw_ipmc_l2_flexctr_attach(unit, info, &ci));
    } else {
        SHR_IF_ERR_CONT
...
bcm_ltsw_ipmc_stat_detach(int unit, bcm_ipmc_addr_t *info)
{
    ...
    if (info->flags & BCM_IPMC_L2) {
        SHR_IF_ERR_EXIT
            (mbcm_ltsw_ipmc_l2_flexctr_info_get(unit, info, &ci));
    } else {
        SHR_IF_ERR_EXIT
            (ipmc_addr_to_l3_fib_key(unit, info, &l3_fib));
    ...
    if (info->flags & BCM_IPMC_L2) {
        SHR_IF_ERR_CONT
            (mbcm_ltsw_ipmc_l2_flexctr_detach(unit, info));
    } else {
        SHR_IF_ERR_CONT
                                 ...
                                 uint32_t value)
{
    ...
    if (info->flags & BCM_IPMC_L2) {
        SHR_IF_ERR_EXIT
            (mbcm_ltsw_ipmc_l2_flexctr_object_set(unit, info, value));
    } else {
        SHR_IF_ERR_EXIT
            (ipmc_addr_to_l3_fib_key(unit, info, &l3_fib));

        SHR_IF_ERR_EXIT
            (bcmi_ltsw_l3_fib_mc_flexctr_object_set(unit, &l3_fib, value));
    }
                                  ...
                                  uint32_t *value)
{
    ...
    if (info->flags & BCM_IPMC_L2) {
        SHR_IF_ERR_EXIT
            (mbcm_ltsw_ipmc_l2_flexctr_object_get(unit, info, value));
    } else {
        SHR_IF_ERR_EXIT
            (ipmc_addr_to_l3_fib_key(unit, info, &l3_fib));

        SHR_IF_ERR_EXIT
            (bcmi_ltsw_l3_fib_mc_flexctr_object_get(unit, &l3_fib, value));
    }
...
/*!
 * \brief Get IPMC counters of hardware.
 *
 * \param [in] unit Unit Number.
 * \param [in] port port to get information.
 * \param [out] counters IPMC entry information.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
    ...
    bcm_ipmc_counters_t *counters)
{
    ...
    SHR_NULL_CHECK(counters, SHR_E_PARAM);

    SHR_IF_ERR_EXIT
        (bcm_ltsw_stat_get(unit, port, snmpIfHCInMulticastPkts,
                           &counters->rmca));
    SHR_IF_ERR_EXIT
        (bcm_ltsw_stat_get(unit, port, snmpIfHCOutMulticastPkts,
                           &counters->tmca));
    SHR_IF_ERR_EXIT
        (bcm_ltsw_stat_get(unit, port, snmpBcmIPMCBridgedPckts,
                           &counters->imbp));
    SHR_IF_ERR_EXIT
        (bcm_ltsw_stat_get(unit, port, snmpBcmIPMCRoutedPckts,
                           &counters->imrp));
    SHR_IF_ERR_EXIT
        (bcm_ltsw_stat_get(unit, port, snmpBcmIPMCInDroppedPckts,
                           &counters->rimdr));
    SHR_IF_ERR_EXIT
        (bcm_ltsw_stat_get(unit, port, snmpBcmIPMCOutDroppedPckts,
                           &counters->timdr));
