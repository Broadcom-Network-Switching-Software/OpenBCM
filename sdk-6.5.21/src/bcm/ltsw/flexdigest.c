...
/*! Software entry is reserved in hw. */
#define FD_ENTRY_RESERVED                   (1 << 3)
    ...
    bcm_flexdigest_entry_t entry)
{
    ...
    if (ent->flags & FD_ENTRY_RESERVED) {
 ...
 */
static int
fd_lkup_entry_create_reseved_id(
    int unit,
    bcm_flexdigest_group_t group,
    bcm_flexdigest_entry_t entry)
{
    ...
    bool keep_reserved)
{
    ...
    if (keep_reserved && ent->flags & FD_ENTRY_RESERVED) {
        SHR_ERR_EXIT(SHR_E_BADID);
    }
    ...
    ent->id = entry_id_offset;
    ent->pri = entry_id_offset;
    ent->flags &= ~(FD_ENTRY_USED | FD_ENTRY_RESERVED);
    ...
    int group)
{
    ...
    for (i = 0; i < grp->entry_id_count; i++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (fd_lkup_entry_destroy(unit,
                                   grp->entry_id_base + ent->id,
                                   true));
    }
    ...
    bcm_flexdigest_group_t group)
{
    ...
    /* Reserve a catch-all entry in prio zero group. */
    if (grp->pri == 0) {
        SHR_IF_ERR_VERBOSE_EXIT
            (fd_lkup_entry_create_reseved_id(unit, group, grp->entry_id_base));
    }
    ...
    bcm_flexdigest_group_t group)
{
    ...
    for (i = 0; i < grp->entry_id_count; i++) {
        ent = &grp_res->entry_pool[i];
        /* Destroy the resovled entries. */
        if (ent->flags & FD_ENTRY_RESERVED) {
            SHR_IF_ERR_VERBOSE_EXIT
                (fd_lkup_entry_destroy(unit, i + grp->entry_id_base, false));
        }
        if (ent->flags != FD_ENTRY_UNUSED) {
            SHR_ERR_EXIT(SHR_E_BUSY);
        }
    ...
    bcm_flexdigest_entry_t entry)
{
    ...
    if (ent->flags & FD_ENTRY_RESERVED) {
        LOG_CLI((BSL_META_U(unit,
                            "    EID %d [%d:%d] Reserved\n"),
                            entry, ent->id, ent->pri));
    } else if (ent->flags & FD_ENTRY_USED) {
        LOG_CLI((BSL_META_U(unit,
    ...
    bcm_flexdigest_entry_t entry)
{
    ...
    SHR_IF_ERR_CONT
        (fd_lkup_entry_destroy(unit, entry, true));
