...
mc_l3_grp_create(int unit, uint32_t flags, bcm_multicast_t *group)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_multicast_l3_grp_create(unit, type, mc_index));
                                ...
                                bcm_if_t *encap_id)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_NO_TNL_MC_NH_REMAP)) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
...
int
bcmi_ltsw_multicast_l3_grp_validate(
    int unit,
    bcm_multicast_t group,
    int *valid)
{
    uint32_t mc_index, grp_type, type_flags;
    int rv = 0;
    ...
    grp_type = _BCM_MULTICAST_TYPE_GET(group);
    type_flags = mc_grp_type_to_flags(grp_type);
    rv = mc_type_validate(unit, type_flags);
    if (SHR_FAILURE(rv)){
        *valid = 0;
        SHR_EXIT();
    }
    ...
    mc_index = _BCM_MULTICAST_ID_GET(group);
    SHR_IF_ERR_VERBOSE_EXIT
        (mc_l3_grp_id_get(unit, mc_index, &mc));
    if (mc != group) {
        *valid = 0;
        SHR_EXIT();
    }
