                    ...
                    bcm_bfd_endpoint_info_t *ep_info)
{
    ...
    if (!(BFD_ENCAP_TYPE_TNL_MPLS(ep_info->type) ||
          BFD_ENCAP_TYPE_TNL_VXLAN(ep_info->type))) {
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    if (!BFD_ENCAP_TYPE_TNL_VXLAN(ep_info->type)) {
        SHR_IF_ERR_EXIT
...
int
bcm_ltsw_bfd_tnl_vxlan_endpoint_entry_set(int unit,
        bool update,
        bcm_bfd_endpoint_info_t *ep_info)
{
    int dunit;
    ...
    uint8 ip_tos;
    uint32 flow_handle;
    bcm_flow_tunnel_initiator_t tunnel_info;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_allocate(dunit, OAM_BFD_TNL_L2_VXLAN_ENDPOINTs, &entry_hdl));
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(entry_hdl, OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDs,
                               ep_info->id));

    if (ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4) {
        symbol = UNDERLAY_IPV4_OVERLAY_IPV4s;
    } else if (ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv6) {
        symbol = UNDERLAY_IPV4_OVERLAY_IPV6s;
    } else if (ep_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4) {
        symbol = UNDERLAY_IPV6_OVERLAY_IPV4s;
    } else {
        symbol = UNDERLAY_IPV6_OVERLAY_IPV6s;
    }

    if (ep_info->ip_ttl == 0) {
        ep_info->ip_ttl = 1;
    } else if (ep_info->ip_ttl != 1) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    /* VXLAN tunnel configuration */
    bcm_flow_tunnel_initiator_t_init(&tunnel_info);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm_ltsw_flow_handle_get(unit, "CLASSIC_VXLAN", &flow_handle));
    tunnel_info.flow_handle = flow_handle;
    tunnel_info.tunnel_id = ep_info->gport;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm_ltsw_flow_tunnel_initiator_get(dunit, &tunnel_info, 0, NULL));
    ...
    bcmi_ltsw_util_mac_to_uint64(&val, tunnel_info.dmac);
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, UNDERLAY_DST_MACs, val));

    bcmi_ltsw_util_mac_to_uint64(&val, tunnel_info.smac);
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, UNDERLAY_SRC_MACs, val));

    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, UNDERLAY_VLAN_IDs,
                               tunnel_info.vlan));

    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, UNDERLAY_VLAN_PRIs,
                               tunnel_info.pkt_pri));

    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, UNDERLAY_TPIDs,
                               tunnel_info.tpid));
    num_vlan = 1;

    symbol = bcm_ltsw_bfd_vlan_tag_type_symbol_get(num_vlan);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_symbol_add(entry_hdl, UNDERLAY_TAG_TYPEs, symbol));

    if ((ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4) ||
        (ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv6)) {

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_SRC_IPV4s, tunnel_info.sip));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_DST_IPV4s, tunnel_info.dip));

        ip_tos = ((tunnel_info.dscp & 0x3F << 2) | (tunnel_info.ecn & 0x3));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_TOSs, ip_tos));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_TTLs, tunnel_info.ttl));
    }

    if ((ep_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4) ||
        (ep_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv6)) {
        sal_memcpy(&ipv6_lower, tunnel_info.sip6, 8);
        sal_memcpy(&ipv6_upper, &(tunnel_info.sip6[8]), 8);

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_SRC_IPV6_LOWERs, ipv6_lower));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_SRC_IPV6_UPPERs, ipv6_upper));

        sal_memcpy(&ipv6_lower, tunnel_info.dip6, 8);
        sal_memcpy(&ipv6_upper, &(tunnel_info.dip6[8]), 8);

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_DST_IPV6_LOWERs, ipv6_lower));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_DST_IPV6_UPPERs, ipv6_upper));

        ip_tos = ((tunnel_info.dscp & 0x3F << 2) | (tunnel_info.ecn & 0x3));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_TRAFFIC_CLASSs, ip_tos));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, UNDERLAY_HOP_LIMITs,
                                   tunnel_info.ttl));

    }

    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, UNDERLAY_UDP_SRC_PORTs, tunnel_info.udp_src_port));

    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, UNDERLAY_UDP_DST_PORTs, tunnel_info.udp_dst_port));

    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, VNIDs, ep_info->vxlan_vnid));

    bcmi_ltsw_util_mac_to_uint64(&val, ep_info->dst_mac);
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, OVERLAY_DST_MACs, val));

    bcmi_ltsw_util_mac_to_uint64(&val, ep_info->src_mac);
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, OVERLAY_SRC_MACs, val));

    num_vlan = 1;

    if (ep_info->vlan_tpid) {
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_VLAN_IDs, ep_info->pkt_vlan_id));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_VLAN_PRIs, ep_info->vlan_pri));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_TPIDs, ep_info->vlan_tpid));
        num_vlan = 1;
    }

    if (ep_info->inner_vlan_tpid) {
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_INNER_VLAN_IDs, ep_info->pkt_inner_vlan_id));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_INNER_VLAN_PRIs, ep_info->inner_vlan_pri));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_INNER_TPIDs, ep_info->inner_vlan_tpid));
        num_vlan = 2;
    }

    symbol = bcm_ltsw_bfd_vlan_tag_type_symbol_get(num_vlan);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_symbol_add(entry_hdl, OVERLAY_TAG_TYPEs, symbol));

    if ((ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4) ||
        (ep_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4)) {
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_SRC_IPV4s,
                                    ep_info->inner_src_ip_addr));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_DST_IPV4s,
                                    ep_info->inner_dst_ip_addr));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_TOSs,
                                    ep_info->ip_tos));
    }

    if ((ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv6) ||
        (ep_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv6)) {
        sal_memcpy(&ipv6_lower, ep_info->inner_src_ip6_addr, 8);
        sal_memcpy(&ipv6_upper, &(ep_info->inner_src_ip6_addr[8]),
                8);
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_SRC_IPV6_LOWERs,
                                   ipv6_lower));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_SRC_IPV6_UPPERs,
                                   ipv6_upper));

        sal_memcpy(&ipv6_lower, ep_info->inner_dst_ip6_addr, 8);
        sal_memcpy(&ipv6_upper, &(ep_info->inner_dst_ip6_addr[8]), 8);
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_DST_IPV6_LOWERs,
                                   ipv6_lower));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_DST_IPV6_UPPERs,
                                   ipv6_upper));

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry_hdl, OVERLAY_TRAFFIC_CLASSs,
                            ep_info->inner_ip_tos));
    }

    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry_hdl, OVERLAY_UDP_SRC_PORTs,
                               ep_info->udp_src_port));
...
int
bcm_ltsw_bfd_tnl_vxlan_endpoint_entry_get(int unit,
        bcm_bfd_endpoint_info_t *ep_info)
{
    int dunit;
    bcmlt_entry_handle_t entry_hdl = BCMLT_INVALID_HDL;
    ...
    val = ep_info->id;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(entry_hdl, OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDs, val));
    ...
    if ((ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4) ||
        ...
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, UNDERLAY_TOSs, &val));
        ep_info->ip_tos = val;
    ...
    if ((ep_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4) ||
        ...
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, UNDERLAY_TRAFFIC_CLASSs, &val));
        ep_info->ip_tos = val;

    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, VNIDs, &val));
    ep_info->vxlan_vnid = val;

    ep_info->inner_ip_ttl = 1;

    if ((ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4) ||
        (ep_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4)) {

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, OVERLAY_SRC_IPV4s, &val));
        ep_info->inner_src_ip_addr = val;

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, OVERLAY_DST_IPV4s, &val));
        ep_info->inner_dst_ip_addr = val;

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, OVERLAY_TOSs, &val));
        ep_info->inner_ip_tos = val;
    }
    ...
    if ((ep_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv6) ||
        ...
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, OVERLAY_SRC_IPV6_LOWERs, &ipv6_lower));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, OVERLAY_SRC_IPV6_UPPERs, &ipv6_upper));
        sal_memcpy(&ep_info->inner_src_ip6_addr, &ipv6_lower, 8);
        sal_memcpy(&(ep_info->inner_src_ip6_addr[8]),&ipv6_upper, 8);

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, OVERLAY_DST_IPV6_LOWERs, &ipv6_lower));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, OVERLAY_DST_IPV6_UPPERs, &ipv6_upper));
        sal_memcpy(&ep_info->inner_dst_ip6_addr, &ipv6_lower, 8);
        sal_memcpy(&(ep_info->inner_dst_ip6_addr[8]),&ipv6_upper, 8);

        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get(entry_hdl, OVERLAY_TRAFFIC_CLASSs, &val));
        ep_info->inner_ip_tos = val;
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_UDP_SRC_PORTs, &val));
    ep_info->udp_src_port = val;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_DST_MACs, &val));
    bcmi_ltsw_util_uint64_to_mac(ep_info->dst_mac, &val);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_SRC_MACs, &val));
    bcmi_ltsw_util_uint64_to_mac(ep_info->src_mac, &val);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_TPIDs, &val));
    ep_info->vlan_tpid = val;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_VLAN_IDs, &val));
    ep_info->pkt_vlan_id = val;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_VLAN_PRIs, &val));
    ep_info->vlan_pri = val;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_INNER_TPIDs, &val));
    ep_info->inner_vlan_tpid = val;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_INNER_VLAN_IDs, &val));
    ep_info->pkt_inner_vlan_id = val;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(entry_hdl, OVERLAY_INNER_VLAN_PRIs, &val));
    ep_info->inner_vlan_pri = val;
...
int bcm_ltsw_bfd_endpoint_entry_delete(int unit, bcm_bfd_endpoint_t glb_id)
{
    ...
    } else if (BFD_ENCAP_TYPE_TNL_VXLAN(encap_type)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_allocate(dunit, OAM_BFD_TNL_L2_VXLAN_ENDPOINTs, &entry_hdl));
        ...
        bcm_bfd_endpoint_stat_t *ctr_info)
{
    ...
    if (encap_type == (bcmBFDTunnelTypeUdp | BCM_LTSW_BFD_ENDPOINT_IPV6)) {
        ...
        table_name = OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATSs;
        key_name = OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDs;
    } else {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    if (!(BFD_ENCAP_TYPE_TNL_MPLS(encap_type) ||
          BFD_ENCAP_TYPE_TNL_VXLAN(encap_type))) {
        SHR_IF_ERR_VERBOSE_EXIT
...
bcm_ltsw_bfd_event_entry_get(int unit, bcmlt_entry_handle_t entry_hdl,
    ...
    *event = bcm_ltsw_bfd_event_enum_get(str);
    ...
    if(!sal_strcmp(str, TNL_L2_VXLANs)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(entry_hdl, OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDs, &val));
        *ep_id = val;
    }
        ...
        bcm_bfd_endpoint_info_t *ep_info)
{
    ...
    if (encap_type == (bcmBFDTunnelTypeUdp | BCM_LTSW_BFD_ENDPOINT_IPV6)) {
        ...
        table_name = OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSs;
        key_name = OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDs;
    } else {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
