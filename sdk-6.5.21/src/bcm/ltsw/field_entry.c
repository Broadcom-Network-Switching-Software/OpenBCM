    ...
    bcmi_field_ha_entry_oper_t **entry_oper)
{
    ...
    oper->policer_pool_id = -1;
    oper->color_pool_id = -1;
                           ...
                           bcmi_field_ha_entry_oper_t *entry_oper)
{
    ...
    if (entry_oper->entry_flags & BCMINT_FIELD_ENTRY_DEFAULT_ONLY) {
        ...
        if(tbls_info->def_sbr_key_fid != NULL) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(grp_template,
        ...
        if (stage_info->tbls_info->grp_ltid_oper_mode_fid) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_remove(grp_template,
                    stage_info->tbls_info->grp_ltid_oper_mode_fid));
        }
    ...
    if (stage_info->flags & BCMINT_FIELD_STAGE_FLEXSTAT_SLICE_INFO) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_group_stage_based_id_get(unit,
                                                   ...
                                                   &group_id));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_group_active_entries_get(unit,
                                                   stage_info,
                                                   group_id,
        ...
        if (active_entries_count == 1)  {
            ...
            if (!(entry_oper->entry_flags &
                ...
                int eid;
                bcmi_field_ha_entry_oper_t *eid_oper;

                /* Install slice_reserve_entry  during active entry deletion */
                eid = stage_info->entry_info->eid_max;
                for (; eid > 0; eid--) {
                    if (SHR_BITGET(stage_info->entry_info->eid_bmp.w, eid)) {
                        continue;
                    }
                    break;
                }
                ...
                eid = stage_info->stage_base_id | eid;

                bcm_field_entry_config_t_init(&entry_config);
                entry_config.group_id = stage_info->stage_base_id | group_id;
                entry_config.entry_id = eid;
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmint_field_entry_oper_lookup(unit,
                                                    ...
                                                    stage_info,
                                                    &eid_oper));
                eid_oper->entry_flags |= BCMINT_FIELD_ENTRY_TEMP_FOR_SLICE_INFO;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmint_field_entry_install(unit, eid));
            }
        }
    }
    ...
    if (entry_oper->policer_id) {
        if (group_oper->group_flags & BCMINT_FIELD_GROUP_ENT_LEVEL_POLICER_POOL) {
            ...
            policer_cfg.policer_id = entry_oper->policer_id;
            policer_cfg.pool_id = entry_oper->policer_pool_id;
            policer_cfg.index = entry_oper->color_pool_id;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_entry_policer_extended_detach(unit, entry,
                                                            &policer_cfg));
        } else {
            SHR_IF_ERR_VERBOSE_EXIT
    ...
    /* Update flexctr action */
    if ((stage_info->flags & BCMINT_FIELD_STAGE_FLEXSTAT_SLICE_INFO) &&
        (stage_info->flags & BCMINT_FIELD_STAGE_KEY_TYPE_FIXED) &&
        ((active_entries_count % stage_info->slice_entry_count) == 0)) {
        int pipe = -1;
        bcmi_field_ha_group_oper_t *group_oper = NULL;
                                 ...
                                 bcm_field_entry_config_t *entry_config)
{
    ...
    for (i = 0; i < entry_oper->num_actions; i++) {
        if (i < bcmFieldActionCount) {
            /* Update entry info OUT param */
            BCM_FIELD_ASET_ADD(entry_config->aset, i);
        }
    }
                           ...
                           bcm_field_entry_t entry)
{
    ...
    if (entry_oper->entry_flags &
        BCMINT_FIELD_ENTRY_DEFAULT_ONLY) {
        if ((NULL == stage_info->tbls_info->def_pdd_key_fid) ||
            (NULL == stage_info->tbls_info->def_policy_key_fid)) {
        ...
        if (stage_info->tbls_info->def_sbr_key_fid != NULL) {
             SHR_IF_ERR_VERBOSE_EXIT
                 (bcmlt_entry_field_add(grp_template,
         ...
         /*
          * UPDATE group template. Update only operation.
          * Entry install should not insert Group Template.
          * So bcmi_lt_entry_set_commit is not used here.
          */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_commit(grp_template, BCMLT_OPCODE_UPDATE,
                BCMLT_PRIORITY_NORMAL));
    }
    ...
    if ((group_oper->group_flags & BCMINT_FIELD_GROUP_AVOID_HW_INSTALL) &&
        (stage_info->tbls_info->sw_flowtracker_fid != 0)) {
        sw_flowtracker = true;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(entry_template,
                                   stage_info->tbls_info->sw_flowtracker_fid,
                                   sw_flowtracker));
    }

    /* Disable temporary reserved entry */
    ...
    if (entry_oper->entry_flags & BCMINT_FIELD_ENTRY_COLOR_INDEPENDENT) {
        uint16_t a;

        for (a = 0; a < entry_oper->num_actions; a++) {
            if (entry_oper->action_arr[a] == bcmFieldActionColorIndependent) {
                break;
            }
        }

        if (a == entry_oper->num_actions) {
            bcmlt_entry_handle_t policy_template = BCMLT_INVALID_HDL;
    ...
    if (stage_info->flags & BCMINT_FIELD_STAGE_FLEXSTAT_SLICE_INFO) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_group_stage_based_id_get(unit,
                                                   ...
                                                   entry_oper->group_id,
                                                   &group_id));
        rv = bcmint_field_group_active_entries_get(unit,
                                                   stage_info,
                                                   group_id,
                                                   &active_entries_count);
                              ...
                              bcm_field_entry_t entry, int *prio)
{
    ...
    SHR_NULL_CHECK(prio, SHR_E_PARAM);
                              ...
                              int *enable_flag)
{
    ...
    SHR_NULL_CHECK(enable_flag, SHR_E_PARAM);
                              ...
                              int enable)
{
    ...
    if ((stage_info->flags & BCMINT_FIELD_STAGE_FLEXSTAT_SLICE_INFO) &&
        ...
        bcmi_field_ha_entry_oper_t *eid_oper = NULL;
    ...
    uint32_t *color_ct)
{
    ...
    bcm_field_group_t group_id = 0, hgid = 0;
    uint32_t slice_id = 0;
    int e, sel;
    int profile_idx;
    bcmi_ltsw_flexctr_packet_attribute_info_t *info = NULL;
    ...
    if (ctr_info.act_cfg.hint_type == bcmFlexctrHintFieldGroupId) {
        ...
        if ((hgid & ~BCM_FIELD_ID_MASK) != stage_info->stage_base_id) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
         ...
         */
    } else if (ctr_info.act_cfg.hint_type == bcmFlexctrHintSharedBlock) {
        /* validate the flexctr_action_id */
        if (group_oper->group_flags & BCMINT_FIELD_GROUP_FLEXCTR_VMAP_ENABLED) {
            int idx;

            for (idx = 0; idx < BCMI_FIELD_HA_VSLICE_MAX; idx++) {
                if (group_oper->ctr_action_vmap_arr[idx] == ctr_info.action_index) {
                    break;
                }
            }

            if (idx == BCMI_FIELD_HA_VSLICE_MAX) {
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
            }
        } else {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    ...
    *opq_obj_val = counter_idx;
    ...
    if ((bcmFlexctrObjectIngIfpOpaqueObj0 == object) ||
        ...
        (bcmFlexctrObjectStaticIngPktAttribute == object) ||
        ...
        switch (object) {
            ...
            case bcmFlexctrObjectStaticIngPktAttribute:
            case bcmFlexctrObjectIngIfpOpaqueObj0:
                ...
                if (stage_info->flags & BCMINT_FIELD_STAGE_FLEXCTR_CONT_OBJ_SUPPORT) {
                    if ((object == bcmFlexctrObjectStaticIngPktAttribute) &&
                        (object1 != bcmFlexctrObjectStaticIngFieldStageIngress)) {
                        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                    }
                    ...
                    if (ctr_info.act_cfg.hint_type == bcmFlexctrHintFieldGroupId) {
                        SHR_IF_ERR_VERBOSE_EXIT
                            (bcmint_field_group_primary_slice_id_get(unit,
                                                               hgid,
                                                               &slice_id));
                        *flexctr_cont_id = slice_id;
                    } else {
                        if (object == bcmFlexctrObjectStaticIngPktAttribute) {
                            *flexctr_cont_id = (uint64_t)ctr_info.act_cfg.index_operation.object_id[1];
                        } else {
        ...
        if ((color != NULL) &&
            ((object == bcmFlexctrObjectStaticIngPktAttribute) ||
             (object1 == bcmFlexctrObjectStaticIngPktAttribute))) {
            *color_ct = 0;
            if (object1 == bcmFlexctrObjectStaticIngPktAttribute) {
                profile_idx = ctr_info.act_cfg.index_operation.object_id[1];
            } else {
                profile_idx = ctr_info.act_cfg.index_operation.object_id[0];
            }

            BCMINT_FIELD_MEM_ALLOC
                (info,
                sizeof(bcmi_ltsw_flexctr_packet_attribute_info_t),
                "Flex counter info");
            ...
            for (e = 0; e < info->obj_inst_max; e++) {
                ...
                for (sel = 0; sel < obj_inst_info->num_entries; sel++) {
                    if (obj_inst_info->map_entry_type[sel] ==
                        ...
                        color[*color_ct] = obj_inst_info->map_entry_value[sel];
                        *color_ct += 1;
                    }
                }
            }
        ...
        switch (object) {
            ...
            default:
                ...
                (bcmFlexctrObjectIngEmFtOpaqueObj == object))) {
    ...
    if (stat_without_obj == false) {
        ...
        if (!(stage_info->flags & BCMINT_FIELD_STAGE_HIT_INDEX_CTR_SUPPORT) &&
            ((object == bcmFlexctrObjectIngEmFtHitIndex0) ||
             (object == bcmFlexctrObjectIngEmFtHitIndex1))) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
    }
                                  ...
                                  bcm_field_flexctr_config_t *flexctr_cfg)
{
    ...
    bool stat_action_found = false;
    int act_indx = 0;
    bool is_action_conflict = false;

    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(flexctr_cfg, SHR_E_PARAM);
    ...
    /* check if flex stat action conflict with configured action */
    FP_HA_GROUP_OPER_ASET_TEST(group_oper,
                               bcmFieldActionStatGroup,
                               stat_action_found);
    for (act_indx = 0; act_indx < entry_oper->num_actions; act_indx++) {
        if ((entry_oper->action_arr[act_indx] != bcmFieldActionStatGroup) &&
             stat_action_found) {
            /* Below will check only for the opaque_object Lt name
             * to see if it conflicts with the configured action
             */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_is_action_conflict(unit, entry,
                    bcmFieldActionStatGroup, entry_oper->action_arr[act_indx],
                    opaque_object,
                    &is_action_conflict));
                                  ...
                                  bcm_policer_t policer_id)
{
    ...
    if (group_oper->policer_pool_id == -1) {
        ...
        /*
         * Check if user has assigned a pool. If yes, use
         * user specified pool_id.
         */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_policer_pool_status_get(unit,
                                               policer_id,
                                               &pool_assigned));

        if (pool_assigned == false) {
            /*
    ...
    if (group_oper->policer_pool_id != -1) {
        ...
        pool_bmp = group_oper->policer_pool_bmp;
    }

    /* Notify policer about FP entry attach. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_policer_attach
         (unit, stage_info->stage_id, policer_id, pipe, pool, pool_bmp, false, NULL));
                                  ...
                                  int level)
{
    ...
    if ((0 == (stage_info->flags & BCMINT_FIELD_STAGE_POLICER_ACTION_SUPPORT)) &&
...
int
bcmint_field_entry_policer_extended_attach(int unit,
                                           bcm_field_entry_t entry,
                                           bcm_field_policer_config_t *policer_cfg)
{
    int entry_id = 0, group_id = 0;
    ...
    policer_id = policer_cfg->policer_id;
    if ((policer_id & ~(BCM_FIELD_ID_MASK)) == BCM_FIELD_EFP_ID_BASE) {
        policer_stage = bcmiPolicerStageEgress;
    } else {
        policer_stage = bcmiPolicerStageIngress;
    ...
    pool = policer_cfg->pool_id;
    if (pool != bcmPolicerPoolAny) {
        /*
         * If the pool specified is not present among the hints,
         * throw error.
         */
        if ((pool != group_oper->policer_pool_id) &&
            (!SHR_BITGET(&group_oper->policer_pool_bmp, pool))) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
        fixed_pool = true;
        pool_bmp = 0;
    ...
    /* Check if the specified container is valid. */
    if (policer_cfg->index >= color_tbl_info->num_pools) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_policer_attach
         (unit, stage_info->stage_id, policer_id,
          pipe, pool, pool_bmp, fixed_pool, &ret_pool_id));

    if (pipe != -1) {
        pipe_val = pipe;
    }

    entry_oper->policer_pool_id = ret_pool_id;
    /*
     * If the color table referenced  by this container is full,
     * select another color table pool referenced by the
     * same container.
     */
    if (!SHR_BITGET(&stage_info->color_pool_full_bmp[pipe_val],
                    policer_cfg->index)) {
        entry_oper->color_pool_id = policer_cfg->index;
    } else {
        for (iter = 0; iter < color_tbl_info->num_pools; iter++) {
            if (iter == policer_cfg->index) {
                continue;
            }
            if (color_tbl_info->cont_id[iter] ==
                color_tbl_info->cont_id[policer_cfg->index]) {
                entry_oper->color_pool_id = iter;
            }
        }
    }
...
int
bcmint_field_entry_policer_extended_detach(int unit,
                                           bcm_field_entry_t entry,
                                           bcm_field_policer_config_t *policer_cfg)
{
    int a = 0;
                        ...
                        uint32 flags)
{
    ...
    SHR_NULL_CHECK(dst_entry, SHR_E_PARAM);
    ...
    *dst_entry = entry_config.entry_id;
    ...
    if (entry_oper->policer_id) {
        if (group_oper->group_flags & BCMINT_FIELD_GROUP_ENT_LVL_ACTION_RESOLVE) {
            ...
            policer_cfg.index = entry_oper->color_pool_id;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_entry_policer_extended_attach(unit, *dst_entry,
                                                            &policer_cfg));
        } else {
            SHR_IF_ERR_VERBOSE_EXIT
