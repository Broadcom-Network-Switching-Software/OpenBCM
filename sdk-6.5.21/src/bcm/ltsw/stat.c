...
static bcmint_stat_ctr_map_info_t ltsw_stat_mac_ctr_map_tab[] = {
    ...
    {snmpIfOutUcastPkts, CTR_MAC_ERRs, TX_OVER_SIZE_PKTs, FALSE},
 ...
 */
    ...
    {snmpBcmRxPFCFrameXonPriority0, CTR_ING_PFCs, RX_PFC_OFF_PKT_PRI0s, FALSE},
    /* snmpBcmRxPFCFrameXonPriority1 = 183*/
    {snmpBcmRxPFCFrameXonPriority1, CTR_ING_PFCs, RX_PFC_OFF_PKT_PRI1s, FALSE},
    /* snmpBcmRxPFCFrameXonPriority2 = 184 */
    {snmpBcmRxPFCFrameXonPriority2, CTR_ING_PFCs, RX_PFC_OFF_PKT_PRI2s, FALSE},
    /* snmpBcmRxPFCFrameXonPriority3 = 185*/
    {snmpBcmRxPFCFrameXonPriority3, CTR_ING_PFCs, RX_PFC_OFF_PKT_PRI3s, FALSE},
    /* snmpBcmRxPFCFrameXonPriority4 = 186*/
    {snmpBcmRxPFCFrameXonPriority4, CTR_ING_PFCs, RX_PFC_OFF_PKT_PRI4s, FALSE},
    /* snmpBcmRxPFCFrameXonPriority5 = 187*/
    {snmpBcmRxPFCFrameXonPriority5, CTR_ING_PFCs, RX_PFC_OFF_PKT_PRI5s, FALSE},
    /* snmpBcmRxPFCFrameXonPriority6 = 188*/
    {snmpBcmRxPFCFrameXonPriority6, CTR_ING_PFCs, RX_PFC_OFF_PKT_PRI6s, FALSE},
    /* snmpBcmRxPFCFrameXonPriority7 = 189*/
    {snmpBcmRxPFCFrameXonPriority7, CTR_ING_PFCs, RX_PFC_OFF_PKT_PRI7s, FALSE},
    /* snmpBcmRxPFCFramePriority0 = 190 */
    {snmpBcmRxPFCFramePriority0, CTR_ING_PFCs, RX_PFC_PKT_PRI0s, FALSE},
    /* snmpBcmRxPFCFramePriority1 = 191 */
    {snmpBcmRxPFCFramePriority1, CTR_ING_PFCs, RX_PFC_PKT_PRI1s, FALSE},
    /* snmpBcmRxPFCFramePriority2 = 192 */
    {snmpBcmRxPFCFramePriority2, CTR_ING_PFCs, RX_PFC_PKT_PRI2s, FALSE},
    /* snmpBcmRxPFCFramePriority3 = 193 */
    {snmpBcmRxPFCFramePriority3, CTR_ING_PFCs, RX_PFC_PKT_PRI3s, FALSE},
    /* snmpBcmRxPFCFramePriority4 = 194 */
    {snmpBcmRxPFCFramePriority4, CTR_ING_PFCs, RX_PFC_PKT_PRI4s, FALSE},
    /* snmpBcmRxPFCFramePriority5 = 195 */
    {snmpBcmRxPFCFramePriority5, CTR_ING_PFCs, RX_PFC_PKT_PRI5s, FALSE},
    /* snmpBcmRxPFCFramePriority6 = 196 */
    {snmpBcmRxPFCFramePriority6, CTR_ING_PFCs, RX_PFC_PKT_PRI6s, FALSE},
    /* snmpBcmRxPFCFramePriority7 = 197 */
    {snmpBcmRxPFCFramePriority7, CTR_ING_PFCs, RX_PFC_PKT_PRI7s, FALSE},
...
ltsw_stat_lt_detach(int unit)
{
    ...
    /* Remove CTR_ING_PFC LT entries. */
    SHR_IF_ERR_CONT
        (bcmi_lt_clear(unit, CTR_ING_PFCs));
    ...
    const char *table_name)
{
    ...
    BCM_PBMP_ITER(pbmp, port) {
        fields[0].u.val = port;
        rv = bcmi_lt_entry_insert(unit, table_name, &lt_entry, NULL);
        if (rv == SHR_E_EXISTS) {
            break;
        }
 ...
 * \param [out] state Counter state.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_stat_sync_state_get(int unit, bcmi_ltsw_stat_counter_type_t type,
                         const char **state)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    bcmlt_entry_handle_t entry_hdl = BCMLT_INVALID_HDL;
    const char *ctr_type_str[BCMI_LTSW_COUNTER_TYPE_COUNT] =
        BCMI_LTSW_STAT_COUNTER_TYPE_STR;
    ...
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_symbol_add(entry_hdl, CTR_EVENT_SYNC_STATE_IDs,
                                      ctr_type_str[type]));
    SHR_IF_ERR_EXIT
 ...
 */
static int
ltsw_stat_sync_on(int unit, bcmi_ltsw_stat_counter_type_t type)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    bcmlt_entry_handle_t entry_hdl = BCMLT_INVALID_HDL;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_stat_sync_state_get(unit, type, &state));
 ...
 */
static int
ltsw_stat_sync_off(int unit, bcmi_ltsw_stat_counter_type_t type)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    bcmlt_entry_handle_t entry_hdl = BCMLT_INVALID_HDL;
    shr_timeout_t timeout;
    ...
    bool enable)
{
    ...
    uint32_t num_array, count = COUNTOF(data_arr);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_commit(unit, entry_hdl, BCMLT_OPCODE_LOOKUP,
                              BCMLT_PRIORITY_NORMAL));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_array_get(entry_hdl, PORT_IDs, port,
                                     &data, 1, &num_array));
    if ((enable ^ (int)data) == 0) {
        /* Already enabled, don't need to update. */
        SHR_EXIT();
    }
    data_arr[0] = enable;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_array_add(entry_hdl, PORT_IDs, port,
                                     data_arr, count));
    SHR_IF_ERR_VERBOSE_EXIT
 ...
 */
static int
ltsw_stat_init(int unit, int warm)
{
    SHR_FUNC_ENTER(unit);

    if (warm) {
        /*
         * This is a new LT, it does not exist 6.5.20, however it exists 6.5.21.
         * If warmboot from 6.5.20 to 6.5.21, this LT needs to be initialized.
         */
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_stat_mac_counter_init(unit, CTR_ING_PFCs));
    } else {
        /* Insert CTR_MAC LT entries. */
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_stat_mac_counter_init(unit, CTR_MACs));

        /* Insert CTR_ING_PFC LT entries. */
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_stat_mac_counter_init(unit, CTR_ING_PFCs));
    ...
    uint64_t *value_arr)
{
    ...
    entry_hdl = BCMLT_INVALID_HDL;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_allocate(dunit, CTR_ING_PFCs, &entry_hdl));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(entry_hdl, PORT_IDs, port));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_attrib_set(entry_hdl, entry_attr));
    ...
    for (i = 0; i < nstat; i++) {
        ...
        if (stat_arr[i] == snmpIfOutQLen) {
            ...
            for (index = stat_mac_index[unit][stat_arr[i]];
                ...
                } else if (sal_strcmp(table_name, CTR_ING_PFCs) == 0) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_transaction_entry_num_get(trans_hdl, 2, &entry_info));
                } else {
...
bcmi_ltsw_stat_port_attach(int unit, int port)
{
    ...
    /* Insert CTR_ING_PFC LT entry. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_stat_lt_counter_insert(unit, CTR_ING_PFCs, port));
...
bcmi_ltsw_stat_port_detach(int unit, int port)
{
    ...
    /* Delete CTR_ING_PFC LT entry. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_stat_lt_counter_delete(unit, CTR_ING_PFCs, port));
...
bcm_ltsw_stat_clear(int unit, bcm_port_t port)
{
    ...
    if (bcmi_ltsw_port_is_type(unit, port, BCMI_LTSW_PORT_TYPE_PORT) ||
        ...
        /* Clear CTR_ING_PFCs LT entry fields. */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_stat_lt_counter_clear(unit, CTR_ING_PFCs, port));
...
/*!
 * \brief Synchronize software counters with hardware with counter type.
 *
 * \param [in] unit Unit Number.
 * \param [in] type Counter type.
 *
 ...
 */
int
bcmi_ltsw_stat_sync_with_type(int unit, bcmi_ltsw_stat_counter_type_t type)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    bcmlt_entry_handle_t entry_hdl = BCMLT_INVALID_HDL;
    bool locked = FALSE;
    ...
    if (type >= BCMI_LTSW_COUNTER_TYPE_COUNT) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    /* Trigger stat sync. */
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_stat_sync_on(unit, type));

    /* Turn off stat sync. */
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_stat_sync_off(unit, type));

exit:
    if (SHR_FUNC_ERR()) {
        (void)ltsw_stat_sync_off(unit, type);
    }
    if (locked) {
...
bcm_ltsw_stat_sync(int unit)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_stat_sync_with_type(unit, BCMI_LTSW_COUNTER_TYPE_ALL));
 ...
 */
int
bcm_ltsw_stat_direct_get(int unit, bcm_port_t port, bcm_stat_val_t type,
                         uint64_t *val)
{
    SHR_FUNC_ENTER(unit);
 ...
 */
int
bcm_ltsw_stat_direct_multi_get(int unit, bcm_port_t port, int nstat,
                               bcm_stat_val_t *stat_arr, uint64_t *value_arr)
{
    int stix;
    ...
    for (stix = 0; stix < nstat; stix++) {
        if (!((stat_arr[stix] >= snmpBcmRxPFCFrameXonPriority0) &&
              (stat_arr[stix] <= snmpBcmRxPFCFramePriority7))) {
            SHR_IF_ERR_VERBOSE_EXIT(BCM_E_UNAVAIL);
        }
    }
