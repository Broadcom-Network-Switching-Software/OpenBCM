...
/*!
 * \brief Defines software database for flow related LT information.
 */
typedef struct ltsw_virtual_lt_db_s {
    /* Start tunnel index. */
    uint32_t vt_start;
...
/*!
 * \Flow software database of LT information.
 */
static ltsw_flow_lt_db_t ltsw_flow_lt_db[BCM_MAX_NUM_UNITS];
...
{
   ltsw_flow_bookkeeping_t *flow_info = &ltsw_flow_bk_info[unit];
   uint32_t num_vp = ltsw_flow_lt_db[unit].num_vp;
   ...
   int warm = bcmi_warmboot_get(unit);
    ...
    ha_req_size = num_vp * sizeof(uint32_t);
...
flow_port_delete_all(int unit, bcm_vpn_t vpn)
{
    ...
    for (vp_idx = 0; vp_idx < ltsw_flow_lt_db[unit].num_vp; vp_idx++) {
...
flow_port_match_info_reset(int unit, bcm_flow_match_config_t *info)
{
    ...
    for (vp = 0; vp < ltsw_flow_lt_db[unit].num_vp; vp++) {
        match = &(ltsw_flow_bk_info[unit].vp_match[vp]);
        if (match->valid == 0) {
            continue;
        }
...
/*!
 * \brief Initialize LT table software database.
 *
 * \param [in] unit Unit Number.
 *
 * \retval SHR_E_NONE No errors.
 ...
 */
int
bcmi_ltsw_flow_init_lt_db(int unit)
{
    uint32_t start, end, num_vp;
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_virtual_tnl_range_get(unit, &start, &end));
    ltsw_flow_lt_db[unit].vt_start = start;
    ltsw_flow_lt_db[unit].vt_end = end;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_virtual_vp_range_get(unit, &start, &end));
    ltsw_flow_lt_db[unit].vp_start = start;
    ltsw_flow_lt_db[unit].vp_end = end;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_virtual_vp_num_get(unit, &num_vp));
    ltsw_flow_lt_db[unit].num_vp = num_vp;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_virtual_vfi_range_get(unit, &start, &end));
    ltsw_flow_lt_db[unit].vfi_start = start;
    ltsw_flow_lt_db[unit].vfi_end = end;
...
bcm_ltsw_flow_vpn_destroy(int unit, bcm_vpn_t vpn)
{
    ...
    rv = bcm_ltsw_flow_vpn_stat_id_get(unit, vpn, bcmFlexctrDirectionIngress,
                                       &stat_counter_id);
    if (SHR_SUCCESS(rv)) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_BUSY);
    }

    rv = bcm_ltsw_flow_vpn_stat_id_get(unit, vpn, bcmFlexctrDirectionEgress,
                                       &stat_counter_id);
    if (SHR_SUCCESS(rv)) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_BUSY);
    }
    ...
    if (vfi_index >= ltsw_flow_lt_db[unit].vfi_start) {
        (void)bcmi_ltsw_virtual_vfi_free(unit, BCMI_LTSW_VFI_TYPE_FLOW,
                                         vfi_index);
    }
exit:
...
bcm_ltsw_flow_vpn_destroy_all(int unit)
{
    ...
    for (vfi_index = ltsw_flow_lt_db[unit].vfi_start;
         vfi_index <= ltsw_flow_lt_db[unit].vfi_end;
         vfi_index++) {
...
bcm_ltsw_flow_port_create(int unit, bcm_vpn_t vpn, bcm_flow_port_t *flow_port)
{
    ...
    if (flow_port->flags & BCM_FLOW_PORT_REPLACE) {
        ...
        if (!BCM_GPORT_IS_FLOW_PORT(flow_port->flow_port_id)) {
         ...
         if (vp >= ltsw_flow_lt_db[unit].num_vp) {
             SHR_ERR_EXIT(SHR_E_BADID);
         }

         if (vp < ltsw_flow_lt_db[unit].vp_start ||
             vp > ltsw_flow_lt_db[unit].vp_end) {
             SHR_ERR_EXIT(SHR_E_BADID);
         }
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_virtual_vp_alloc(unit, ltsw_flow_lt_db[unit].vp_start,
                                        (int)ltsw_flow_lt_db[unit].vp_end,
                                        1, vp_info, &vp));
...
bcm_ltsw_flow_port_destroy(int unit, bcm_vpn_t vpn, bcm_gport_t flow_port_id)
{
    ...
    rv = bcm_ltsw_flow_port_stat_id_get(unit, flow_port_id,
                                        bcmFlexctrDirectionIngress,
                                        &stat_counter_id);
    if (SHR_SUCCESS(rv)) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_BUSY);
    }
                        ...
                        bcm_flow_logical_field_t *field)
{
    int locked = false;
    int vp = 0;
    bcm_flow_match_config_t old_info;
    ...
    if (info->options & BCM_FLOW_MATCH_OPTION_REPLACE) {
        bcm_flow_match_config_t_init(&old_info);
        sal_memcpy(&old_info, info, sizeof(bcm_flow_match_config_t));
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_flow_match_hw_opt(unit, HW_OPT_GET, &old_info));
    }
    ...
    if (flow_match_assigned_to_flow_port(unit, info)) {
        if (info->options & BCM_FLOW_MATCH_OPTION_REPLACE) {
            flow_port_match_info_reset(unit, &old_info);
        }
                           ...
                           bcm_flow_logical_field_t *field)
{
    ...
    rv = bcm_ltsw_flow_match_stat_id_get(unit, info, num_of_fields, field,
                                         &stat_counter_id);
    if (SHR_SUCCESS(rv)) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_BUSY);
    }
                           ...
                           bcm_flow_logical_field_t *field)
{
    ...
    rv = bcm_ltsw_flow_encap_stat_id_get(unit, info, num_of_fields, field,
                                         &stat_counter_id);
    if (SHR_SUCCESS(rv)) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_BUSY);
    }
                             ...
                             bcm_flow_logical_field_t *field)
{
    ...
    if(SHR_SUCCESS(rv)) {
        ...
        if ((info->valid_elements & BCM_FLOW_PORT_ENCAP_OL_EGRESS_IF_VALID) &&
            (info->ol_egress_if != cur_info.ol_egress_if)) {
            egress_if_updated |= 1;
        }
    }
...
bcm_ltsw_flow_tunnel_initiator_destroy(int unit, bcm_gport_t flow_tunnel_id)
{
    ...
    rv = bcm_ltsw_flow_tunnel_initiator_stat_id_get(unit, flow_tunnel_id,
                                                    &stat_counter_id);
    if (SHR_SUCCESS(rv)) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_BUSY);
    }
                           ...
                           uint32_t *tnl_idx)
{
    ...
    *tnl_idx = -1;
    ...
    if ((soft_tnl_idx < ltsw_flow_lt_db[unit].vt_start) ||
        (soft_tnl_idx > ltsw_flow_lt_db[unit].vt_end)) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
                              ...
                              uint32_t *soft_tnl_idx)
{
    ...
    for (tmp_soft_tnl_idx = ltsw_flow_lt_db[unit].vt_start;
         tmp_soft_tnl_idx < ltsw_flow_lt_db[unit].vt_end;
         tmp_soft_tnl_idx++) {
    ...
    if (tmp_soft_tnl_idx >= ltsw_flow_lt_db[unit].vt_end) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    } else {
                                            ...
                                            bcm_flow_logical_field_t *field)
{
    ...
    rv = bcm_ltsw_flow_tunnel_terminator_stat_id_get(unit, info, num_of_fields,
...
int
bcmi_ltsw_flow_match_flow_port_find(
    int unit,
    bcm_flow_match_criteria_t criteria,
    ...
    int *vp_id)
{
    ...
    for (vp = 0; vp < ltsw_flow_lt_db[unit].num_vp; vp++) {
        ...
        if (match->criteria != criteria) {
            continue;
        }

        if (match->criteria == BCM_FLOW_MATCH_CRITERIA_PORT
            && match->port == match_port) {
            *vp_id = vp;
            break;
        }
    }
...
int
bcmi_ltsw_flow_match_trunk_member_delete(
    int unit,
    ...
    bcm_port_t *trunk_member_array)
{
    ...
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_flow_match_trunk_member_delete(
            unit,
            trunk_id,
            trunk_member_count,
            trunk_member_array));
...
int
bcmi_ltsw_flow_port_encap_l3_ecmp_notify(
    int unit,
    ...
    int add)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_port_encap_l3_ecmp_notify(
            unit,
            ecmp_group_id,
            ecmp_member_count,
            ecmp_member_array,
    ...
    bcm_gport_t flow_port_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_es_filter_stat_detach(unit, flow_port_id));
...
int bcm_ltsw_flow_vpn_stat_detach_with_id(
    int unit,
    bcm_vpn_t vpn,
    uint32_t stat_counter_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_vpn_stat_detach_with_id(unit, vpn, stat_counter_id));
    ...
    bcm_vpn_t vpn)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_vpn_stat_detach(unit, vpn));
...
int bcm_ltsw_flow_port_stat_detach_with_id(
    int unit,
    bcm_gport_t flow_port_id,
    uint32_t stat_counter_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_port_stat_detach_with_id(unit, flow_port_id, stat_counter_id));
    ...
    bcm_gport_t flow_port_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_port_stat_detach(unit, flow_port_id));
    ...
    bcm_gport_t tunnel_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_tunnel_initiator_stat_detach(unit, tunnel_id));
...
int bcm_ltsw_flow_tunnel_initiator_flexctr_object_get(
    int unit,
    bcm_gport_t tunnel_id,
    uint32_t *value)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_tunnel_initiator_flexctr_object_get(unit, tunnel_id, value));
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_encap_stat_detach(unit, info, num_of_fields, field));
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_match_stat_detach(unit, info, num_of_fields, field));
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flow_tunnel_terminator_stat_detach(unit, info, num_of_fields, field));
