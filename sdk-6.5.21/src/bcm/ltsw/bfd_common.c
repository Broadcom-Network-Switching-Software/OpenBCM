...
bcm_ltsw_bfd_clear_tables(int unit)
{
    ...
    const char *ltname[] = {
        ...
        OAM_BFD_TNL_L2_VXLAN_ENDPOINTs,
    };
...
static int bcm_ltsw_int_bfd_endpoint_create(int unit,
    ...
    } else if (BFD_ENCAP_TYPE_TNL_VXLAN(endpoint_info->type)) {
                SHR_IF_ERR_EXIT
                    (bcm_ltsw_bfd_tnl_vxlan_endpoint_entry_set(unit, false, endpoint_info));
                ep_created = true;

                BFD_EP_ENCAP_TYPE(unit, glb_id) = endpoint_info->type;

                SHR_IF_ERR_EXIT
                    (bcm_ltsw_bfd_tnl_vxlan_endpoint_entry_get(unit, endpoint_info));
    } else {
        ...
        bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (encap_type == bcmBFDTunnelTypeUdp) {
        ...
        SHR_IF_ERR_EXIT
            (bcm_ltsw_bfd_tnl_vxlan_endpoint_entry_set(unit, true, endpoint_info));
    }
   ...
   if (encap_type == bcmBFDTunnelTypeUdp) {
        ...
        rv = bcm_ltsw_bfd_tnl_vxlan_endpoint_entry_get(unit, endpoint_info);
    }
   ...
   if (SHR_FAILURE(rv)) {
       if (encap_type == bcmBFDTunnelTypeUdp) {
           ...
           SHR_IF_ERR_EXIT
               (bcm_ltsw_bfd_tnl_vxlan_endpoint_entry_set(unit, true, &cur_info));
       }
    }
exit:
    SHR_FUNC_EXIT();
