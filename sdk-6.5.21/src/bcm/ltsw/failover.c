 ...
 */
int
failover_prot_offset_get(int unit, uint16_t *prot_offset)
{
 ...
 */
/*!
 * \brief Get protection nexthop offset.
 *
 * \param [in]  unit            Unit number.
 * \param [out] prot_offset     Protection NH offset.
 *
...
bcmi_ltsw_failover_prot_offset_get(int unit, uint16_t *prot_offset)
{
    ...
    SHR_NULL_CHECK(prot_offset, SHR_E_PARAM);

    if (!LTSW_FAILOVER_INITIALIZED(unit)) {
        *prot_offset = 0;
    } else {
        *prot_offset = ltsw_failover_bk_info[unit].prot_offset;
    }
...
bcm_ltsw_failover_cleanup(int unit)
{
    ...
    ltsw_failover_bk_info[unit].prot_offset = 0;
...
bcm_ltsw_failover_init(int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (failover_prot_offset_get(unit, &prot_offset));
    ltsw_failover_bk_info[unit].prot_offset = prot_offset;
...
bcm_ltsw_failover_egress_set(int unit, bcm_if_t intf, bcm_l3_egress_t *egr)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_FAILOVER_UNDERLAY_NH_ONLY)) {
