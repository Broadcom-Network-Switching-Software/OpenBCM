...
lt_egr_l3_tnl_cmp(int unit, int index, void *entry, int *result)
{
    ...
    *result = 1;
    ...
    if (num_labels > XFS_ENCAP_LABEL_CNT_MAX) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }

    for (hdr_idx = 0; hdr_idx < num_labels; hdr_idx++) {
 ...
 *
 * \return SHR_E_NONE Set the match info successfully.
 * \return !SHR_E_NONE Fail setting the match info.
 */
static int
mpls_vp_info_set(
 ...
 * \param [in] vp_info    VP information.
 *
 * \return SHR_E_NONE  Get the match info successfully.
 * \return !SHR_E_NONE Fail getting the match info.
 */
static int
    ...
    int vc_label_idx, int tnl_idx)
{
    ...
    if (!(mpls_port->flags & BCM_MPLS_PORT_NETWORK)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_to_l2_if(unit, mpls_port->port, &l2_if));
...
mpls_port_match_delete(int unit, int vp)
{
    ...
    if (criteria == BCM_MPLS_PORT_MATCH_PORT) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_to_l2_if(unit, vp_info.port, &l2_if));
        SHR_IF_ERR_VERBOSE_EXIT
