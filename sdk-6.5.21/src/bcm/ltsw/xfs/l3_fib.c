...
#define L3_FIB_IPV6_MC_DEFIP_128  (1 << 1)
    ...
    /* LARGE_VRF enabled, default is disabled. */
    bool large_vrf_enabled;
} l3_fib_info_t;
...
#define L3_FIB_LARGE_VRF_ENABLED(_u) (l3_fib_info[_u].large_vrf_enabled)
...
/* If WCEMP member weighted is supported.
 *      misc_ctrl_0[0:0]: If set, entry is valid;
 *      misc_ctrl_0[2:1]: Opaque info;
 *      misc_ctrl_0[3:3]: ECMP member valid;
 */
#define L3_MISC_CTRL_0_OPQ_CTRL_ID_WITH_WECMP_MEMBER_MAX (0x3)

/* L3 ALPM trie type. */
typedef enum trie_type_e {
    /* V4 ROUTE table. */
    BCMINT_LTSW_L3_V4_GLO = 0,

    /* V4 ROUTE_VRF table. */
    BCMINT_LTSW_L3_V4_VRF = 1,

    /* V4 ROUTE_OVERRIDE table. */
    BCMINT_LTSW_L3_V4_GHI = 2,

    /* V6 ROUTE table. */
    BCMINT_LTSW_L3_V6_GLO = 3,

    /* V6 ROUTE_VRF table. */
    BCMINT_LTSW_L3_V6_VRF = 4,

    /* V6 ROUTE_OVERRIDE table. */
    BCMINT_LTSW_L3_V6_GHI = 5,

    /* Count type, always last. */
    BCMINT_LTSW_L3_TRIE_TYPE_COUNT = 6,
} trie_type_t;

/* ALPM trie location, MTOP or MAIN. */
typedef struct l3_alpm_trie_type_s {
    /* Indicating the trie is in MTOP table. */
    bool mtop;

    /* Indicating the trie is in MAIN table. */
    bool main;

    /* Indicating the VRF trie is in MTOP table. */
    SHR_BITDCL *mtop_vrf;

    /* Indicating the VRF trie is in MAIN table. */
    SHR_BITDCL *main_vrf;
} l3_alpm_table_type_t;

l3_alpm_table_type_t l3_alpm_table_info[BCMINT_LTSW_L3_TRIE_TYPE_COUNT] = {{0}};

/* Get used ALPM VRF table. */
#define VRF_BMP_GET(bmp, vrf_id) (SHR_BITGET(bmp, vrf_id))


/* Set used ALPM VRF table. */
#define VRF_BMP_SET(bmp, vrf_id) (SHR_BITSET(bmp, vrf_id))

/* Delete used ALPM VRF table. */
#define VRF_BMP_DELETE(bmp, vrf_id) (SHR_BITCLR(bmp, vrf_id))


/* MAP ID that is enabled during init stage. */
#define PRIVATE_TRIE_TYPE(type) (((type) == BCMINT_LTSW_L3_V4_VRF) || \
    ((type) == BCMINT_LTSW_L3_V6_VRF))
 ...
 */
static int
l3_mtop_tables_clear(int unit)
{
    int i, num;
    bcmint_ltsw_l3_fib_tbl_id_t lt[] = {
    ...
    for (i = 0; i < num; i++) {
        SHR_IF_ERR_EXIT
            (bcmint_ltsw_l3_fib_tbl_get(unit, lt[i], &ti));
        SHR_IF_ERR_CONT
            (bcmi_lt_clear(unit, ti->name));
    }
           ...
           l3_cfg_t *cfg)
{
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (fi[i].symbol) {
            rv = fi[i].scalar_to_symbol(unit, fv[i], &sym_val);
            if (SHR_SUCCESS(rv)) {
            ...
            if (fi[i].elements) {
                ...
                rv = bcmlt_entry_field_add_by_id(eh, fi[i].fld_id, fv[i]);
            }
        }
    }
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (fi[i].symbol) {
            rv = bcmlt_entry_field_symbol_get_by_id(eh, fi[i].fld_id,
                                                    &sym_val);
            if (SHR_SUCCESS(rv)) {
                rv = fi[i].symbol_to_scalar(unit, sym_val, &fv[i]);
            }
        } else {
            if (fi[i].elements) {
                       ...
                       l3_filter_cmd_t *flt_cmd)
{
    ...
    if (ecmp_member_dest) {
        ...
        rv = bcmi_ltsw_ecmp_member_dest_get(unit,
        ...
        rv = bcmi_ltsw_l3_egress_obj_id_construct(unit, ol_nhidx, ol_type,
                                                  &ol_intf);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
        ...
        if (flt_crtr->ul_intf && ul_nhidx) {
            rv = bcmi_ltsw_l3_egress_obj_id_construct(unit, ul_nhidx, ul_type,
                                                      &ul_intf);
            if (SHR_FAILURE(rv)) {
                return rv;
            }
             ...
             l3_cfg_t *l3_cfg)
{
    ...
    return lt_eh_parse(unit, eh, ti, l3_cfg);
}
                 ...
                 bcmi_ltsw_l3_fib_t *fib)
{
    ...
    if(fib->ul_intf) {
        rv = bcmi_ltsw_l3_egress_obj_id_resolve(unit, fib->ul_intf,
                                                &nh_ecmp_idx, &type);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
    ...
    if (fib->ol_intf) {
        rv = bcmi_ltsw_l3_egress_obj_id_resolve(unit, fib->ol_intf,
                                                &nh_ecmp_idx, &type);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
                 ...
                 bcmi_ltsw_l3_fib_t *fib, l3_uc_cfg_t *cfg)
{
    ...
    int dest, dest_type, nh_ecmp_idx, ecmp_and_prot, rv;
    ...
    if (fib->flags2 & BCM_L3_FLAGS2_FAILOVER_UNDERLAY) {
        if (SHR_FAILURE(bcmi_ltsw_failover_prot_offset_get(unit, &prot_en))) {
            prot_en = 0;
        }
        if (!prot_en) {
            LOG_ERROR(BSL_LOG_MODULE,
    ...
    if (ltsw_feature(unit, LTSW_FT_ECMP_MEMBER_WEIGHTED)) {
        if (fib->opaque_ctrl_id >
            L3_MISC_CTRL_0_OPQ_CTRL_ID_WITH_WECMP_MEMBER_MAX) {
            return SHR_E_PARAM;
        }
    }

    rv = bcmi_ltsw_l3_egress_obj_id_resolve(unit, fib->ol_intf, &nh_ecmp_idx,
                                            &type);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    ...
    if ((ti->fld_bmp & (1 << BCMINT_LTSW_L3_FIB_TBL_UC_DATA_TYPE)) &&
        ...
        if (L3_REDUCED_ROUTE_MAX_ECMP_GRP(unit)) {
            (void)bcmi_ltsw_l3_egress_id_range_get(unit,
                                        BCMI_LTSW_L3_EGR_OBJ_T_OL,
                                        NULL, &max_nhop_intf);
            (void)bcmi_ltsw_l3_egress_obj_id_construct(unit,
    ...
    if (type == BCMI_LTSW_L3_EGR_OBJ_T_OL) {
        rv = bcmi_ltsw_l3_egress_obj_info_get(unit, fib->ol_intf,
                                              &egr_obj_info);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
        if (egr_obj_info.flags & BCMI_LTSW_L3_EGR_OBJ_INFO_F_CASCADED) {
            ...
            rv = bcmi_ltsw_ecmp_member_dest_alloc(unit, &dest_info, &dest);
            if (SHR_FAILURE(rv)) {
                return rv;
            }
    ...
    if ((!(fib->flags & BCM_L3_MULTIPATH) && is_ecmp) ||
        ((fib->flags & BCM_L3_MULTIPATH) && !is_ecmp)) {
        return SHR_E_PARAM;
    }
                  ...
                  bcmlt_opcode_t op, l3_cfg_t *cfg)
{
    ...
    if (op != BCMLT_OPCODE_INSERT) {
        ...
        rv = lt_eh_fill(unit, eh, ti, &pre_cfg);
        if (SHR_FAILURE(rv)) {
            goto exit;
        }
        ...
        if (op == BCMLT_OPCODE_LOOKUP) {
            ...
            rv = lt_eh_parse(unit, eh, ti, &pre_cfg);
            if (SHR_FAILURE(rv)) {
                goto exit;
            }
        }
        rv = bcmlt_entry_clear(eh);
        if (SHR_FAILURE(rv)) {
            goto exit;
        }
    }
...
static int
l3_alpm_tbl_info_to_trie(int unit, const bcmint_ltsw_l3_fib_tbl_t *ti,
                           trie_type_t *trie_type, bool *mtop)
{
    if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_V6) {
        if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GL) {
            if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) {
                *trie_type = BCMINT_LTSW_L3_V6_GLO;
                *mtop = 1;
            } else {
                *trie_type = BCMINT_LTSW_L3_V6_GLO;
                *mtop = 0;
            }
        } else if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_VRF) {
            if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) {
                *trie_type = BCMINT_LTSW_L3_V6_VRF;
                *mtop = 1;
            } else {
                *trie_type = BCMINT_LTSW_L3_V6_VRF;
                *mtop = 0;
            ...
            if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) {
                *trie_type = BCMINT_LTSW_L3_V6_GHI;
                *mtop = 1;
            } else {
                *trie_type = BCMINT_LTSW_L3_V6_GHI;
                *mtop = 0;
    ...
    } else if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_V4){
        if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GL) {
            if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) {
                *trie_type = BCMINT_LTSW_L3_V4_GLO;
                *mtop = 1;
            } else {
                *trie_type = BCMINT_LTSW_L3_V4_GLO;
                *mtop = 0;
            ...
            if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) {
                *trie_type = BCMINT_LTSW_L3_V4_VRF;
                *mtop = 1;
            } else {
                *trie_type = BCMINT_LTSW_L3_V4_VRF;
                *mtop = 0;
            ...
            if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) {
                *trie_type = BCMINT_LTSW_L3_V4_GHI;
                *mtop = 1;
            } else {
                *trie_type = BCMINT_LTSW_L3_V4_GHI;
                *mtop = 0;
...
static int
l3_alpm_tbl_to_trie(int unit, bcmint_ltsw_l3_fib_tbl_id_t tbl_id,
                    trie_type_t *trie_type, bool *mtop)
{

    if (trie_type == NULL || mtop == NULL) {
        return SHR_E_PARAM;
    }

    switch(tbl_id) {
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_IPV4_UC_GL:
        *trie_type = BCMINT_LTSW_L3_V4_GLO;
        *mtop = 0;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_IPV4_UC_VRF:
        *trie_type = BCMINT_LTSW_L3_V4_VRF;
        *mtop = 0;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_IPV4_UC_GH:
        *trie_type = BCMINT_LTSW_L3_V4_GHI;
        *mtop = 0;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_IPV6_UC_GL:
        *trie_type = BCMINT_LTSW_L3_V6_GLO;
        *mtop = 0;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_IPV6_UC_VRF:
        *trie_type = BCMINT_LTSW_L3_V6_VRF;
        *mtop = 0;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_IPV6_UC_GH:
        *trie_type = BCMINT_LTSW_L3_V6_GHI;
        *mtop = 0;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GL:
        *trie_type = BCMINT_LTSW_L3_V4_GLO;
        *mtop = 1;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_VRF:
        *trie_type = BCMINT_LTSW_L3_V4_VRF;
        *mtop = 1;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GH:
        *trie_type = BCMINT_LTSW_L3_V4_GHI;
        *mtop = 1;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GL:
        *trie_type = BCMINT_LTSW_L3_V6_GLO;
        *mtop = 1;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_VRF:
        *trie_type = BCMINT_LTSW_L3_V6_VRF;
        *mtop = 1;
        break;
    case BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GH:
        *trie_type = BCMINT_LTSW_L3_V6_GHI;
        *mtop = 1;
        break;
    default:
...
/*!
 * \brief Protection mechanism when MTOP ALPM enable.
 *
 * \param [in] unit Unit number
 * \param [in] tbl_id LT table.
 * \param [in] vrf VRF_ID.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
l3_alpm_mtop_protection_check(int unit, const bcmint_ltsw_l3_fib_tbl_t *ti,
                              bcmint_ltsw_l3_fib_tbl_id_t tbl_id, int vrf)
{
    ...
    if (ti != NULL) {
        SHR_IF_ERR_EXIT
            (l3_alpm_tbl_info_to_trie(unit, ti, &trie_type, &mtop));
    } else {
        SHR_IF_ERR_EXIT
            (l3_alpm_tbl_to_trie(unit, tbl_id, &trie_type, &mtop));
    }

    if (PRIVATE_TRIE_TYPE(trie_type) && (!L3_FIB_LARGE_VRF_ENABLED(unit))) {
        if (mtop) {
            if (VRF_BMP_GET(l3_alpm_table_info[trie_type].main_vrf, vrf)) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "This Entry type already located in "
                                      "Main table "
                                      "cannot allocated in FORTE table.\n ")));
                SHR_IF_ERR_EXIT(SHR_E_CONFIG);
            }
        } else {
            if (VRF_BMP_GET(l3_alpm_table_info[trie_type].mtop_vrf, vrf)) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "This Entry type already located in "
                                      "FORTE table "
                                      "cannot allocated in MAIN table.\n ")));
                SHR_IF_ERR_EXIT(SHR_E_CONFIG);
            }
        }
    } else {
        if (mtop) {
            if (l3_alpm_table_info[trie_type].main == 1) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "This Entry type already located in "
                                      "Main table "
            ...
            if (l3_alpm_table_info[trie_type].mtop == 1) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "This Entry type already located in "
                                      "FORTE table "
 ...
 */
static int
l3_alpm_mtop_protection_add(int unit, const bcmint_ltsw_l3_fib_tbl_t *ti,
                            bcmint_ltsw_l3_fib_tbl_id_t tbl_id, int vrf)
{
    trie_type_t trie_type;
    bool mtop;
    ...
    if (PRIVATE_TRIE_TYPE(trie_type) && (!L3_FIB_LARGE_VRF_ENABLED(unit))) {
        if (mtop) {
            if (VRF_BMP_GET(l3_alpm_table_info[trie_type].main_vrf, vrf)) {
                ...
                VRF_BMP_SET(l3_alpm_table_info[trie_type].mtop_vrf, vrf);
            }
        } else {
            if (VRF_BMP_GET(l3_alpm_table_info[trie_type].mtop_vrf, vrf)) {
                SHR_IF_ERR_EXIT(SHR_E_CONFIG);
            } else {
                VRF_BMP_SET(l3_alpm_table_info[trie_type].main_vrf, vrf);
            }
        }
    } else {
        if (mtop) {
            if (l3_alpm_table_info[trie_type].main == 1) {
                ...
                l3_alpm_table_info[trie_type].mtop = 1;
            }
        } else {
            if (l3_alpm_table_info[trie_type].mtop == 1) {
                SHR_IF_ERR_EXIT(SHR_E_CONFIG);
            } else {
                l3_alpm_table_info[trie_type].main = 1;
            }
        }
    }
 ...
 * \param [in] tbl_id LT table.
 * \param [in] vrf VRF_ID.
 * \param [in] ipt IP type.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
l3_alpm_mtop_protection_delete(int unit, const bcmint_ltsw_l3_fib_tbl_t *ti,
    ...
    if (PRIVATE_TRIE_TYPE(trie_type) && (!L3_FIB_LARGE_VRF_ENABLED(unit))) {
        if (mtop) {
            if (VRF_BMP_GET(l3_alpm_table_info[trie_type].main_vrf, vrf)) {
                ...
                if(VRF_ALPM_ROUTE_CNT(unit, vrf, ipt) == 0) {
                    VRF_BMP_DELETE(l3_alpm_table_info[trie_type].mtop_vrf, vrf);
                }
            }
        } else {
            if (VRF_BMP_GET(l3_alpm_table_info[trie_type].mtop_vrf, vrf)) {
                ...
                if(VRF_ALPM_ROUTE_CNT(unit, vrf, ipt) == 0) {
                    VRF_BMP_DELETE(l3_alpm_table_info[trie_type].main_vrf, vrf);
                }
            }
        }
    } else {
        if (mtop) {
            if (l3_alpm_table_info[trie_type].main == 1) {
                ...
                if(VRF_ALPM_ROUTE_CNT(unit, vrf, ipt) == 0) {
                    l3_alpm_table_info[trie_type].mtop = 0;
                }
            }
        } else {
            if (l3_alpm_table_info[trie_type].mtop == 1) {
                ...
                if(VRF_ALPM_ROUTE_CNT(unit, vrf, ipt) == 0) {
                    l3_alpm_table_info[trie_type].main = 0;
                }
            }
        }
    }
                  ...
                  bcmlt_opcode_t op, bool lpm, l3_cfg_t *cfg, bool *new_added)
{
    ...
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    l3_cfg_t pre_cfg;
    uint64_t fld_bmp = cfg->cfg.uc.fld_bmp;
    uint16_t vrf_id;
    int dest_type, dest;
    ...
    if (op != BCMLT_OPCODE_INSERT) {
        /* Get the previous route entry info. */
        /* Only fill key fields for LOOKUP opertion. */
        cfg->cfg.uc.fld_bmp = 0;
        rv = lt_eh_fill(unit, eh, ti, cfg);
        if (SHR_FAILURE(rv)) {
            goto exit;
        }
        ...
        if ((op == BCMLT_OPCODE_LOOKUP) && lpm) {
            rv = bcmlt_entry_attrib_set(eh, BCMLT_ENT_ATTR_LPM_LOOKUP);
            if (SHR_FAILURE(rv)) {
                goto exit;
            }
        }
        ...
        /* Restore the field bitmap to be operated. */
        cfg->cfg.uc.fld_bmp = fld_bmp;
        ...
        if (op == BCMLT_OPCODE_LOOKUP) {
            ...
            /* Resolve the DIP route info for UPDATE/DELETE operation. */
            sal_memset(&pre_cfg, 0, sizeof(l3_cfg_t));
            pre_cfg.cfg.uc.fld_bmp =
     ...
     * miss.
     */
    if (!L3_FIB_LARGE_VRF_ENABLED(unit)) {
        rv = l3_alpm_def_check(unit, ti->attr, cfg->cfg.uc.flds, op);
        if (SHR_FAILURE(rv)) {
            goto exit;
        }
    }
...
exit:
    ...
    if (SHR_FAILURE(rv)) {
        ...
        ipt = (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_V6) ? IP_TYPE_V6 :
                                                            IP_TYPE_V4;
        ...
        if (op == BCMLT_OPCODE_INSERT) {
            ...
            rv = l3_alpm_mtop_protection_add(unit, ti, 0, vrf_id);
        } else if (op == BCMLT_OPCODE_DELETE) {
            VRF_ALPM_ROUTE_DEC(unit, vrf_id, ipt);
            rv = l3_alpm_mtop_protection_delete(unit, ti, 0, vrf_id, ipt);
        }
    }
    return rv;
}
                      ...
                      l3_cfg_t *cfg)
{
    ...
    if ((rv == SHR_E_NOT_FOUND) && (op == BCMLT_OPCODE_UPDATE)) {
        /* BCM_L3_REPLACE: replace existing entry if it exists,
         * othereise, just insert the entry. */
        rv = bcmlt_entry_commit(eh, BCMLT_OPCODE_INSERT,
                       ...
                       const bcmint_ltsw_l3_fib_tbl_t *ti, l3_cfg_t *l3_cfg)
{
    ...
    rv = lt_eh_parse(unit, eh, ti, l3_cfg);
    if (SHR_FAILURE(rv)) {
        return rv;
    }

    rv = bcmi_ltsw_ipmc_flexctr_hit_get(unit,
                        ...
                        const bcmint_ltsw_l3_fib_tbl_t *ti, l3_cfg_t *l3_cfg)
{
    ...
    rv = bcmi_ltsw_l3_mtu_recover(unit,
                 ...
                 bcmi_ltsw_l3_fib_t *fib)
{
    ...
    if (fv[BCMINT_LTSW_L3_FIB_TBL_MC_DEST_TYPE] ==
        ...
        rv = bcmi_ltsw_multicast_l3_grp_id_get(unit,
    ...
    rv = bcmi_ltsw_l3_mtu_get(unit, &mtu_cfg);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    fib->mtu = mtu_cfg.mtu;
                 ...
                 bcmi_ltsw_l3_fib_t *fib, l3_mc_cfg_t *cfg)
{
    ...
    rv = bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type, &sbr_index);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    ...
    rv = bcmi_ltsw_l3_mtu_set(unit, &mtu_cfg);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
...
{
    if (cfg->uc) {
        return l3_uc_cfg_to_fib(unit, ti, &cfg->cfg.uc, fib);
    } else {
        return l3_mc_cfg_to_fib(unit, ti, &cfg->cfg.mc, fib);
    }
}
...
l3_uc_refcnt_decr(int unit, const bcmint_ltsw_l3_fib_tbl_t *ti, l3_cfg_t *cfg)
{
    ...
    uint16_t vrf_id = 0;
    int dest_type, dest, ipt, rv = SHR_E_NONE;
    ...
    if (dest_type == BCMI_XFS_DEST_TYPE_ECMP_MEMBER) {
        rv = bcmi_ltsw_ecmp_member_dest_free(unit, dest);
        if (SHR_FAILURE(rv)) {
            LOG_WARN(BSL_LOG_MODULE,
                     (BSL_META_U(unit,
                                 "Failed to free the ECMP_MEMBER0 resource "
                                 "(%d).\n"), dest));
        }
    }
...
l3_mc_refcnt_decr(int unit, const bcmint_ltsw_l3_fib_tbl_t *ti, l3_cfg_t *cfg)
{
    ...
    rv = bcmi_ltsw_l3_mtu_delete(unit, &mtu_cfg);
    if (SHR_FAILURE(rv)) {
        LOG_WARN(BSL_LOG_MODULE,
                 (BSL_META_U(unit,
                             "Failed to free the MTU profile index (%d).\n"),
                  mtu_cfg.index));
    }
    ...
    if ((trav_info->op == L3_TRAV_OP_DEL) ||
        ...
        if ((tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_UC) &&
            ...
            if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_V6) {
                if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) {
                    if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GL) {
                        src_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV6_UC_GL;
                    } else if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GH) {
                        src_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV6_UC_GH;
                    } else {
                        src_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV6_UC_VRF;
                    }
                } else {
                    if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GL) {
                ...
                if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) {
                    if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GL) {
                        src_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV4_UC_GL;
                    } else if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GH) {
                        src_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV4_UC_GH;
                    } else {
                        src_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV4_UC_VRF;
                    }
                } else {
                    if (tbl_info->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GL) {
...
l3_alpm_strength_set(int unit)
{
    ...
    if (!ltsw_feature(unit, LTSW_FT_MTOP)) {
        ...
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(eh, IPV4_UC_STRENGTH_PROFILE_INDEXs,
                                   sbr_index));
        ...
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(eh, IPV6_UC_STRENGTH_PROFILE_INDEXs,
                                   sbr_index));
        ...
        /*
         * When MTOP ALPM co-exist with Legacy ALPM,
         * strength value should be GH > VRF > GL.
         * In order to prevent packet mis-match.
         */
        pth = BCMI_LTSW_SBR_PTH_L3_IPV4_ALPM;

        ent_type = BCMI_LTSW_SBR_PET_ALPM_GL;
        SHR_IF_ERR_EXIT
            (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                                 &sbr_index));
        SHR_IF_ERR_EXIT
        ...
        ent_type = BCMI_LTSW_SBR_PET_ALPM_VRF;
        SHR_IF_ERR_EXIT
            (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                                 &sbr_index));
        SHR_IF_ERR_EXIT
        ...
        ent_type = BCMI_LTSW_SBR_PET_ALPM_GH;
        SHR_IF_ERR_EXIT
            (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                                 &sbr_index));
        SHR_IF_ERR_EXIT
...
/*!
 * \brief Set MTOP ALPM strength information.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
 ...
 */
static int
l3_alpm_mtop_strength_set(int unit)
{
    int dunit = 0;
    int sbr_index = 0;
    bcmi_ltsw_sbr_profile_tbl_hdl_t pth;
    ...
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    const bcmint_ltsw_l3_fib_tbl_t *ti;
    const bcmint_ltsw_l3_fib_fld_t *field;
    uint64_t val;
    ...
    pth = BCMI_LTSW_SBR_PTH_L3_IPV4_ALPM_MTOP;

    ent_type = BCMI_LTSW_SBR_PET_ALPM_GL;
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                             &sbr_index));
    field = &ti->flds[BCMINT_LTSW_L3_ALPM_CTRL_TBL_IPV4_UC_GL_STRENGTH_PROF_IDX];
    val = sbr_index;
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add_by_id(eh, field->fld_id, val));

    ent_type = BCMI_LTSW_SBR_PET_ALPM_VRF;
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                             &sbr_index));
    field = &ti->flds[BCMINT_LTSW_L3_ALPM_CTRL_TBL_IPV4_UC_VRF_STRENGTH_PROF_IDX];
    val = sbr_index;
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add_by_id(eh, field->fld_id, val));
    ...
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                             &sbr_index));
    field = &ti->flds[BCMINT_LTSW_L3_ALPM_CTRL_TBL_IPV4_UC_GH_STRENGTH_PROF_IDX];
    val = sbr_index;
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add_by_id(eh, field->fld_id, val));
    ...
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                             &sbr_index));
    field = &ti->flds[BCMINT_LTSW_L3_ALPM_CTRL_TBL_IPV6_UC_GL_STRENGTH_PROF_IDX];
    val = sbr_index;
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add_by_id(eh, field->fld_id, val));
    ...
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                             &sbr_index));
    field = &ti->flds[BCMINT_LTSW_L3_ALPM_CTRL_TBL_IPV6_UC_VRF_STRENGTH_PROF_IDX];
    val = sbr_index;
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add_by_id(eh, field->fld_id, val));
    ...
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                             &sbr_index));
    field = &ti->flds[BCMINT_LTSW_L3_ALPM_CTRL_TBL_IPV6_UC_GH_STRENGTH_PROF_IDX];
    val = sbr_index;
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add_by_id(eh, field->fld_id, val));
                          ...
                          const bcmint_ltsw_l3_fib_tbl_t *ti, l3_cfg_t *l3_cfg)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_MTOP)) {
        ...
        if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_VRF) {
            if (VRF_ALPM_ROUTE_CNT(unit, fv[BCMINT_LTSW_L3_FIB_TBL_UC_VRF], ipt) == 1) {
                SHR_IF_ERR_EXIT
                    (l3_alpm_mtop_protection_check(unit, ti, 0,
                                                   fv[BCMINT_LTSW_L3_FIB_TBL_UC_VRF]));
                SHR_IF_ERR_EXIT
                    (l3_alpm_mtop_protection_add(unit, ti, 0,
                                                 fv[BCMINT_LTSW_L3_FIB_TBL_UC_VRF]));
            }
        } else if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GL) {
            if (VRF_ALPM_ROUTE_CNT(unit, L3_FIB_VRF_GLO(unit), ipt) == 1) {
                SHR_IF_ERR_EXIT
                    (l3_alpm_mtop_protection_check(unit, ti, 0, L3_FIB_VRF_GLO(unit)));
                SHR_IF_ERR_EXIT
                    (l3_alpm_mtop_protection_add(unit, ti, 0, L3_FIB_VRF_GLO(unit)));
            }
        } else if (ti->attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_GH) {
            if (VRF_ALPM_ROUTE_CNT(unit, L3_FIB_VRF_GHI(unit), ipt) == 1) {
                SHR_IF_ERR_EXIT
                    (l3_alpm_mtop_protection_check(unit, ti, 0, L3_FIB_VRF_GHI(unit)));
                SHR_IF_ERR_EXIT
                    (l3_alpm_mtop_protection_add(unit, ti, 0, L3_FIB_VRF_GHI(unit)));
            }
        }
    }
...
/*!
 * \brief Check L3_ALPM_CONTROL_MTOP valid or not.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
 ...
 */
static int
l3_mtop_alpm_control_check(int unit)
{
    int dunit;
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    const bcmint_ltsw_l3_fib_tbl_t *ti;
    ...
    if (ltsw_feature(unit, LTSW_FT_MTOP)) {
        SHR_IF_ERR_EXIT
            (bcmint_ltsw_l3_fib_tbl_get(
                unit, BCMINT_LTSW_L3_FIB_TBL_ALPM_CONTROL_MTOP, &ti));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_allocate_by_id(dunit, ti->tbl_id,
                                        ti->fld_cnt, &eh));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_commit(eh, BCMLT_OPCODE_LOOKUP,
                                BCMLT_PRIORITY_NORMAL));
        field = &ti->flds[BCMINT_LTSW_L3_ALPM_CTRL_TBL_OP_STATE];
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_symbol_get_by_id(eh, field->fld_id, &val));

        if (sal_strcasecmp(val, VALIDs)) {
            SHR_IF_ERR_EXIT(SHR_E_CONFIG);
        }
    }
...
l3_alpm_recover(int unit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_MTOP)) {
        SHR_IF_ERR_EXIT
            (l3_traverse(unit, BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_VRF,
                         &trav_info));

        SHR_IF_ERR_EXIT
            (l3_traverse(unit, BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_VRF,
                         &trav_info));

        SHR_IF_ERR_EXIT
            (l3_traverse(unit, BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GH,
                         &trav_info));

        SHR_IF_ERR_EXIT
            (l3_traverse(unit, BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GH,
                         &trav_info));

        SHR_IF_ERR_EXIT
            (l3_traverse(unit, BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GL,
                         &trav_info));

        SHR_IF_ERR_EXIT
            (l3_traverse(unit, BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GL,
                         &trav_info));
    }
...
l3_large_vrf_init(int unit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_LARGE_VRF)) {
        dunit = bcmi_ltsw_dev_dunit(unit);
        SHR_IF_ERR_EXIT
            (bcmint_ltsw_l3_fib_tbl_get(unit,
        ...
        SHR_IF_ERR_EXIT
            (bcmlt_entry_commit(eh, BCMLT_OPCODE_LOOKUP,
                                BCMLT_PRIORITY_NORMAL));
        field = &ti->flds[BCMINT_LTSW_L3_ALPM_CTRL_TBL_LARGE_VRF];
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_get_by_id(eh, field->fld_id, &val));
        fi->large_vrf_enabled = val;
        /*  MTOP and Main ALPM should have same LARGE_VRF setting. */
        if (ltsw_feature(unit, LTSW_FT_MTOP)) {
            if (eh != BCMLT_INVALID_HDL) {
                SHR_IF_ERR_EXIT
                    (bcmlt_entry_free(eh));
            ...
            if (fi->large_vrf_enabled != val) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "LARGE_VRF should configure same value "
                                      "between FORTE and Main ALPM.\n")));
...
l3_alpm_info_init(int unit, int alpm_temp)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_LARGE_VRF)) {
        SHR_IF_ERR_EXIT
            (l3_large_vrf_init(unit));
    } else {
        fi->large_vrf_enabled = false;
    }
    ...
    if (ltsw_feature(unit, LTSW_FT_MTOP)) {
        SHR_IF_ERR_EXIT
                (l3_mtop_alpm_control_check(unit));

        sz = SHR_BITALLOCSIZE(L3_FIB_VRF_MAX(unit) + 1);
        for (i = 0; i < BCMINT_LTSW_L3_TRIE_TYPE_COUNT; i++) {

            if (!PRIVATE_TRIE_TYPE(i)) {
                /* Only Private Trie will have VRF_ID. */
                continue;
            }

            l3_alpm_table_info[i].main_vrf = sal_alloc(sz, "bcmLtswAlpmVrfTable");
            sal_memset(l3_alpm_table_info[i].main_vrf, 0, sz);
            l3_alpm_table_info[i].mtop_vrf = sal_alloc(sz, "bcmLtswAlpmVrfMtopTable");
            sal_memset(l3_alpm_table_info[i].mtop_vrf, 0, sz);
        }
    }
...
l3_alpm_init(int unit)
{
    ...
    if (!bcmi_warmboot_get(unit)) {
        ...
        if (ltsw_feature(unit, LTSW_FT_MTOP)) {
            SHR_IF_ERR_EXIT
                (l3_alpm_mtop_strength_set(unit));
        }
    }
...
l3_alpm_deinit(int unit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_MTOP)) {
        for (i = 0; i < BCMINT_LTSW_L3_TRIE_TYPE_COUNT; i++) {
            ...
            SHR_FREE(l3_alpm_table_info[i].main_vrf);
            SHR_FREE(l3_alpm_table_info[i].mtop_vrf);
        }
    }
...
int
xfs_ltsw_l3_host_ipv4_mc_flexctr_object_set(int unit, bcmi_ltsw_l3_fib_t *fib,
                                            uint32_t val)
{
    bcmint_ltsw_l3_fib_tbl_id_t tbl_id;
    const bcmint_ltsw_l3_fib_tbl_t *tbl_info = NULL;
    ...
    if (!(tbl_info->fld_bmp & (1ULL << BCMINT_LTSW_L3_FIB_TBL_MC_OPQ_OBJ))) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }

    mc->fld_bmp = (1ULL << BCMINT_LTSW_L3_FIB_TBL_MC_OPQ_OBJ);
    mc->flds[BCMINT_LTSW_L3_FIB_TBL_MC_OPQ_OBJ] = val;
...
int
xfs_ltsw_l3_host_ipv4_mc_flexctr_object_get(int unit, bcmi_ltsw_l3_fib_t *fib,
                                            uint32_t *val)
{
    bcmint_ltsw_l3_fib_tbl_id_t tbl_id;
    const bcmint_ltsw_l3_fib_tbl_t *tbl_info = NULL;
    ...
    *val = mc->flds[BCMINT_LTSW_L3_FIB_TBL_MC_OPQ_OBJ];
...
int
xfs_ltsw_l3_host_ipv6_mc_flexctr_object_set(int unit, bcmi_ltsw_l3_fib_t *fib,
                                            uint32_t val)
{
    bcmint_ltsw_l3_fib_tbl_id_t tbl_id;
    const bcmint_ltsw_l3_fib_tbl_t *tbl_info = NULL;
...
int
xfs_ltsw_l3_host_ipv6_mc_flexctr_object_get(int unit, bcmi_ltsw_l3_fib_t *fib,
                                            uint32_t *val)
{
    bcmint_ltsw_l3_fib_tbl_id_t tbl_id;
    const bcmint_ltsw_l3_fib_tbl_t *tbl_info = NULL;
...
xfs_ltsw_l3_route_ipv4_uc_add(int unit, bcmi_ltsw_l3_fib_t *fib)
{
    ...
    if (fib->flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) {
        if (fib->vrf == BCM_L3_VRF_GLOBAL) {
            tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GL;
        } else if (fib->vrf == BCM_L3_VRF_OVERRIDE) {
            tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GH;
        } else {
            tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_VRF;
        }
    } else {
        if (fib->vrf == BCM_L3_VRF_GLOBAL) {
    ...
    SHR_IF_ERR_EXIT
        (l3_alpm_mtop_protection_check(unit, NULL, tbl_id, fib->vrf));
    ...
    if (L3_FIB_URPF_ENABLED(unit) && ltsw_feature(unit, LTSW_FT_URPF)) {
        ...
        if (fib->flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) {
            if (fib->vrf == BCM_L3_VRF_GLOBAL) {
                tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV4_UC_GL;
            } else if (fib->vrf == BCM_L3_VRF_OVERRIDE) {
                tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV4_UC_GH;
            } else {
                tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV4_UC_VRF;
            }
        } else {
            if (fib->vrf == BCM_L3_VRF_GLOBAL) {
...
int
xfs_ltsw_l3_route_ipv4_mc_flexctr_object_set(int unit, bcmi_ltsw_l3_fib_t *fib,
                                             uint32_t val)
{
    const bcmint_ltsw_l3_fib_tbl_t *tbl_info = NULL;
    l3_cfg_t cfg;
...
int
xfs_ltsw_l3_route_ipv4_mc_flexctr_object_get(int unit, bcmi_ltsw_l3_fib_t *fib,
                                             uint32_t *val)
{
    const bcmint_ltsw_l3_fib_tbl_t *tbl_info = NULL;
    l3_cfg_t cfg;
...
xfs_ltsw_l3_route_ipv6_uc_add(int unit, bcmi_ltsw_l3_fib_t *fib)
{
    ...
    if (fib->flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) {
        if (fib->vrf == BCM_L3_VRF_GLOBAL) {
            tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GL;
        } else if (fib->vrf == BCM_L3_VRF_OVERRIDE) {
            tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GH;
        } else {
            tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_VRF;
        }
    } else {
        if (fib->vrf == BCM_L3_VRF_GLOBAL) {
    ...
    if (ltsw_feature(unit, LTSW_FT_URPF) && L3_FIB_URPF_ENABLED(unit)) {
        ...
        if (fib->flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) {
            if (fib->vrf == BCM_L3_VRF_GLOBAL) {
                tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV6_UC_GL;
            } else if (fib->vrf == BCM_L3_VRF_OVERRIDE) {
                tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV6_UC_GH;
            } else {
                tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_SRC_IPV6_UC_VRF;
            }
        } else {
            if (fib->vrf == BCM_L3_VRF_GLOBAL) {
...
int
xfs_ltsw_l3_route_ipv6_mc_flexctr_object_set(int unit, bcmi_ltsw_l3_fib_t *fib,
                                             uint32_t val)
{
    bcmint_ltsw_l3_fib_tbl_id_t tbl_id;
    const bcmint_ltsw_l3_fib_tbl_t *tbl_info = NULL;
    ...
    /* Keys. */
    l3_fib_to_mc_cfg_key_route_v6(unit, fib, mc, &tbl_id);
...
int
xfs_ltsw_l3_route_ipv6_mc_flexctr_object_get(int unit, bcmi_ltsw_l3_fib_t *fib,
                                             uint32_t *val)
{
    bcmint_ltsw_l3_fib_tbl_id_t tbl_id;
    const bcmint_ltsw_l3_fib_tbl_t *tbl_info = NULL;
    ...
    mc->fld_bmp = (1 << BCMINT_LTSW_L3_FIB_TBL_MC_OPQ_OBJ);
...
xfs_ltsw_l3_route_del_by_intf(int unit, bcmi_ltsw_l3_fib_t *fib, int negate)
{
    ...
    if (fib->flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) {
        tbl_id = ipv6 ? BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GH :
                        BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GH;
        SHR_IF_ERR_EXIT
            (l3_traverse(unit, tbl_id, &trav_info));

        tbl_id = ipv6 ? BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_VRF :
                        BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_VRF;
        SHR_IF_ERR_EXIT
            (l3_traverse(unit, tbl_id, &trav_info));

        tbl_id = ipv6 ? BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GL :
                        BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GL;
        SHR_IF_ERR_EXIT
            (l3_traverse(unit, tbl_id, &trav_info));
...
int
xfs_ltsw_l3_route_perf_v4_lt_op(int unit, bcmi_ltsw_l3_route_perf_opcode_t op,
                                int vrf, uint32_t ip, uint32_t ip_mask,
                                int intf, int data_type, bool mtop)
{
    int dunit;
...
int
xfs_ltsw_l3_route_perf_v6_lt_op(int unit, bcmi_ltsw_l3_route_perf_opcode_t op,
                                int vrf, uint64_t *v6, uint64_t *v6_mask,
                                int intf, int data_type, bool mtop)
{
    int dunit;
    const char *symbol = NULL;
