...
typedef struct ecmp_level_cfg_s {
    ...
    /* Fields array. */
    uint64_t flds[XFS_LTSW_L3_ECMP_TBL_FLD_CNT];
...
typedef struct ecmp_src_cfg_s {
    ...
    /* Filed array. */
    uint64_t flds[XFS_LTSW_L3_SECMP_TBL_FLD_CNT];
} ecmp_src_cfg_t;
 ...
 */
static int
ecmp_member_arr_fill(int unit, xfs_ltsw_l3_ecmp_fld_t fld, uint32_t num,
                     bcmi_ltsw_l3_ecmp_member_info_t *member_info,
                     uint64_t *arr)
{
    int sbr_idx_def, sbr_idx_null, i, rv;

    switch (fld) {
        case XFS_LTSW_L3_ECMP_TBL_NHOP_1:
        case XFS_LTSW_L3_ECMP_TBL_WEIGHTED_NHOP_INDEX:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].nhop_1;
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_DVP:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].dvp;
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_NHOP_2_OR_ECMP_1:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].nhop_2_or_ecmp_grp_1;
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_ECMP_AND_PROT_SWT_PROF_IDX:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].prot_swt_prfl_idx;
                if (member_info[i].ul_ecmp) {
                    arr[i] |= (1 << 2);
                }
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_STRENGTH_PROF_IDX:
            ...
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].nhop_1 ? sbr_idx_def : sbr_idx_null;
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_WEIGHT:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].weight;
            }
            break;
        default:
            return SHR_E_PARAM;
 ...
 */
static int
ecmp_member_arr_parse(int unit, xfs_ltsw_l3_ecmp_fld_t fld, uint32_t num,
                      uint64_t *arr,
                      bcmi_ltsw_l3_ecmp_member_info_t *member_info)
{
    int i;
    ...
    switch (fld) {
        ...
        case XFS_LTSW_L3_ECMP_TBL_WEIGHTED_NHOP_INDEX:
            for (i = 0; i < num; i++) {
                member_info[i].nhop_1 = arr[i];
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_DVP:
            for (i = 0; i < num; i++) {
                member_info[i].dvp = arr[i];
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_NHOP_2_OR_ECMP_1:
            for (i = 0; i < num; i++) {
                member_info[i].nhop_2_or_ecmp_grp_1 = arr[i];
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_ECMP_AND_PROT_SWT_PROF_IDX:
            for (i = 0; i < num; i++) {
                member_info[i].ul_ecmp = (arr[i] & (1 << 2)) ? true : false;
                member_info[i].prot_swt_prfl_idx = arr[i] & 0x3;
            }
            break;
        case XFS_LTSW_L3_ECMP_TBL_WEIGHT:
            for (i = 0; i < num; i++) {
                ...
                ecmp_level_cfg_t *cfg)
{
    ...
    uint64_t *data = NULL;
    const char *sym_val;
    uint32_t i, num_of_elem, max_paths, sz = 0;
    int rv = SHR_E_NONE;
    ...
    if (num_of_elem) {
        ...
        data = sal_alloc(sz, "bcmLtswXfsEcmpMemberData");
        if (!data) {
            return SHR_E_MEMORY;
        }
    }

    for (i = 0; i < XFS_LTSW_L3_ECMP_TBL_FLD_CNT; i++) {
                 ...
                 ecmp_level_cfg_t *cfg)
{
    ...
    if (cfg->num_of_set) {
        ...
        data = sal_alloc(data_sz, "bcmLtswXfsEcmpMemberData");
        if (!data) {
            return SHR_E_MEMORY;
        }
    }
...
/*!
 * \brief Fill SRC ECMP LT entry handle.
 *
 * \param [in] unit Unit number
 * \param [in] eh LT entry handle.
 * \param [in] ti LT info.
 * \param [in] cfg Data of ECMP source group.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
lt_eh_secmp_fill(int unit, bcmlt_entry_handle_t eh,
                const bcmint_ltsw_l3_egress_tbl_t *ti,
                ecmp_src_cfg_t *cfg)
{
    const bcmint_ltsw_l3_egress_fld_t *fi = ti->flds;
    ...
    for (i = 0; i < XFS_LTSW_L3_SECMP_TBL_FLD_CNT; i++) {
        /* Skip fields that are invalid for the given LT. */
        if (!(ti->fld_bmp & (1 << i))) {
            continue;
        }
        ...
        */
        ...
        if (fi[i].symbol) {
            ...
            rv = bcmlt_entry_field_add_by_id(eh, fi[i].fld_id, cfg->flds[i]);
        }
    }
...
/*!
 * \brief Parse entry handle of ECMP source group.
 *
 * \param [in] unit Unit number
 * \param [in] eh LT entry handle.
 * \param [in] ti LT info.
 * \param [out] cfg Data of ECMP source group.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
lt_eh_secmp_parse(int unit, bcmlt_entry_handle_t eh,
                  const bcmint_ltsw_l3_egress_tbl_t *ti,
                  ecmp_src_cfg_t *cfg)
{
    const bcmint_ltsw_l3_egress_fld_t *fi = ti->flds;
    ...
    for (i = 0; i < XFS_LTSW_L3_SECMP_TBL_FLD_CNT; i++) {
        ...
        if (fi[i].symbol) {
            ...
            rv = bcmlt_entry_field_get_by_id(eh, fi[i].fld_id, &cfg->flds[i]);
        }
    }
 ...
 */
int
snh_ing_l3_oif_get(int unit, int nhidx, uint32_t *l3_oif)
{
    snh_cfg_t snh;
    int dunit;
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
...
olnh_ing_set(int unit, int nhidx, bcm_l3_egress_t *egr)
{
    ...
    if (egr->flags2 & BCM_L3_FLAGS2_MC_GROUP) {
        ...
        if (egr->flags & BCM_L3_DST_DISCARD) {
            ...
            SHR_IF_ERR_EXIT
                (bcmi_ltsw_id_port_to_l2_if(unit, BCM_GPORT_BLACK_HOLE,
                                         &l2_if));
            inh.dest = l2_if;
            ...
            SHR_IF_ERR_EXIT
                (bcmi_ltsw_id_port_to_l2_if(unit, gport, &l2_if));
...
olnh_ing_get(int unit, int nhidx, bcm_l3_egress_t *egr)
{
    ...
    if (inh.data_type == INH1_VIEW_DVP) {
        ...
        if (inh.dest_valid & DEST_VALID_L2OIF) {
            SHR_IF_ERR_EXIT
                (bcmi_ltsw_id_l2_if_to_port(unit, inh.dest, &gport));
 ...
 */
static int
secmp_lt_op(int unit, bcmlt_opcode_t op, ecmp_src_cfg_t *cfg)
{
    int dunit, i;
    ...
    for (i = 0; i < 2; i++) {
        SHR_IF_ERR_EXIT
            (bcmint_ltsw_l3_ecmp_tbl_get(unit, tbl_id[i], &tbl_info));
        ...
        SHR_IF_ERR_EXIT
            (lt_eh_secmp_fill(unit, eh, tbl_info, cfg));
        ...
        if (op == BCMLT_OPCODE_LOOKUP) {
            cfg->fld_bmp = fbmp;
            SHR_IF_ERR_EXIT
                (lt_eh_secmp_parse(unit, eh, tbl_info, cfg));
        }
 ...
 */
static int
secmp_set(int unit, int idx, bcm_l3_egress_ecmp_t *ecmp_info,
          bcmi_ltsw_l3_ecmp_member_info_t *member, int count)
{
    ...
    uint32_t l3_oif;
    bcmlt_opcode_t op;
    int oif_fid[ECMP_SRC_GROUP_MAX_MEMBER] = {
    ...
    /* Full field bitmap. */
    secmp.fld_bmp = (1 << XFS_LTSW_L3_SECMP_TBL_FLD_CNT) -1;
    secmp.flds[XFS_LTSW_L3_SECMP_TBL_ECMP_ID] = idx;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (cnt >= ECMP_SRC_GROUP_MAX_MEMBER) {
            secmp.flds[XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT_GT8] = 1;
            break;
        }

        SHR_IF_ERR_EXIT
            (snh_ing_l3_oif_get(unit, member[i].nhop_1, &l3_oif));
        secmp.flds[oif_fid[cnt]] = l3_oif;
        cnt ++;
    }

    /* COUNT width 3 bits. */
    secmp.flds[XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT] = cnt ? (cnt - 1) : 0;
...
secmp_del(int unit, int idx)
{
    ...
    SHR_IF_ERR_EXIT
        (secmp_lt_op(unit, BCMLT_OPCODE_DELETE, &secmp));
 ...
 */
static int
secmp_member_add(int unit, int idx, bcmi_ltsw_l3_ecmp_member_info_t *member)
{
    int cnt = 0;
    uint32_t l3_oif;
    ecmp_src_cfg_t secmp;
    ...
    SHR_IF_ERR_EXIT
        (secmp_lt_op(unit, BCMLT_OPCODE_LOOKUP, &secmp));

    if (secmp.flds[XFS_LTSW_L3_SECMP_TBL_L3_OIF0_VAL_TYPE] &&
        secmp.flds[XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT]) {
        cnt = secmp.flds[XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT] + 1;
    }

    if (cnt < ECMP_SRC_GROUP_MAX_MEMBER) {
        /* Update count and l3_oif. */
        secmp.fld_bmp = (1 << XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT) |
                        (1 << oif_fid[cnt]);
        secmp.flds[XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT] ++;
        SHR_IF_ERR_EXIT
            (snh_ing_l3_oif_get(unit, member->nhop_1, &l3_oif));
        secmp.flds[oif_fid[cnt]] = l3_oif;
    } else {
        if (secmp.flds[XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT_GT8] == 0) {
            /* Update gt8 flag. */
            secmp.fld_bmp = (1 << XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT_GT8);
            secmp.flds[XFS_LTSW_L3_SECMP_TBL_L3_OIF_CNT_GT8] = 1;
        } else {
    ...
    SHR_IF_ERR_EXIT
        (secmp_lt_op(unit, BCMLT_OPCODE_UPDATE, &secmp));
 ...
 */
static int
secmp_member_del(int unit, int idx, bcmi_ltsw_l3_ecmp_member_info_t *member,
                 int count, bcmi_ltsw_l3_ecmp_member_info_t *del_member)
{
    int i, cnt;
    uint32_t l3_oif;
    bool del = false;
 ...
 */
static int
secmp_member_del_all(int unit, int idx)
{
    ecmp_src_cfg_t secmp;
...
/*!
 * \brief Construct ECMP group info from ecmp_level_cfg_t.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
ecmp_cfg_to_info(int unit, const bcmint_ltsw_l3_egress_tbl_t *ti,
                 ecmp_level_cfg_t *cfg, bcm_l3_egress_ecmp_t *ecmp_info)
{
    uint64_t *fv = cfg->flds;

    if (cfg->weighted) {
        ecmp_info->max_paths =
            1 << (fv[XFS_LTSW_L3_ECMP_TBL_WEIGHTED_SIZE] + 8);
    } else if (cfg->rh) {
        ecmp_info->dynamic_size =
            1 << (fv[XFS_LTSW_L3_ECMP_TBL_WEIGHTED_SIZE] + 8);
        ecmp_info->dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT;
    } else {
        ecmp_info->max_paths = fv[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS];
        if (fv[XFS_LTSW_L3_ECMP_TBL_LB_MODE] ==
            XFS_LTSW_L3_ECMP_LB_MODE_REGULAR) {
            ecmp_info->dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_DISABLED;
        } else if (fv[XFS_LTSW_L3_ECMP_TBL_LB_MODE] ==
                   XFS_LTSW_L3_ECMP_LB_MODE_RANDOM) {
...
/*!
 * \brief Construct ecmp_level_cfg_t from ECMP group info.
 *
 * \param [in] unit Unit number.
 * \param [in] idx ECMP index.
 * \param [in] ecmp_info ECMP group info.
 * \param [in] count ECMP member count.
 * \param [out] cfg LT entry info.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
ecmp_info_to_cfg(int unit, int idx, bcm_l3_egress_ecmp_t *ecmp_info, int count,
                 ecmp_level_cfg_t *cfg)
{
    uint64_t *fv = cfg->flds;
    int lb_mode, weighted_size;

    fv[XFS_LTSW_L3_ECMP_TBL_ECMP_ID] = idx;
    if ((ecmp_info->ecmp_group_flags & BCM_L3_ECMP_WEIGHTED) ||
        (ecmp_info->ecmp_group_flags & BCM_L3_ECMP_MEMBER_WEIGHTED)) {
        cfg->weighted = true;
        fv[XFS_LTSW_L3_ECMP_TBL_NUM_PATHS] = count;
        fv[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS] = ecmp_info->max_paths;
        if (ecmp_info->ecmp_group_flags & BCM_L3_ECMP_MEMBER_WEIGHTED) {
            fv[XFS_LTSW_L3_ECMP_TBL_WEIGHTED_MODE] =
                                        XFS_LTSW_L3_ECMP_MEMBER_WEIGHTED;
        }
    } else if (ecmp_info->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT) {
        cfg->rh = true;
        fv[XFS_LTSW_L3_ECMP_TBL_RH_MODE] = 1;
        /* No API for random seed. Set to 0 temporarily. */
        fv[XFS_LTSW_L3_ECMP_TBL_RH_RANDOM_SEED] = 0;
        fv[XFS_LTSW_L3_ECMP_TBL_RH_NUM_PATHS] = count;
        fv[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS] = ecmp_info->dynamic_size;
    } else {
        fv[XFS_LTSW_L3_ECMP_TBL_NUM_PATHS] = count;
        fv[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS] = ecmp_info->max_paths;

        if (ecmp_info->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_DISABLED) {
            lb_mode = XFS_LTSW_L3_ECMP_LB_MODE_REGULAR;
        } else if (ecmp_info->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RANDOM) {
            lb_mode = XFS_LTSW_L3_ECMP_LB_MODE_RANDOM;
        ...
        fv[XFS_LTSW_L3_ECMP_TBL_LB_MODE] = lb_mode;
    }

    if (cfg->weighted || cfg->rh) {
        switch (fv[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS]) {
        case 256:
            weighted_size = XFS_LTSW_L3_ECMP_WGT_SZ_256;
            break;
        case 512:
            weighted_size = XFS_LTSW_L3_ECMP_WGT_SZ_512;
            break;
        case 1024:
            weighted_size = XFS_LTSW_L3_ECMP_WGT_SZ_1K;
            break;
        case 2048:
            weighted_size = XFS_LTSW_L3_ECMP_WGT_SZ_2K;
            break;
        case 4096:
            weighted_size = XFS_LTSW_L3_ECMP_WGT_SZ_4K;
            break;
        case 8192:
            weighted_size = XFS_LTSW_L3_ECMP_WGT_SZ_8K;
            break;
        case 16384:
            weighted_size = XFS_LTSW_L3_ECMP_WGT_SZ_16K;
            break;
        default:
            return SHR_E_PARAM;
        }
        fv[XFS_LTSW_L3_ECMP_TBL_WEIGHTED_SIZE] = weighted_size;
    }
...
/*!
 * \brief Operate ECMP LT.
 *
 * \param [in] unit Unit number.
 * \param [in] ti Table inf.
 * \param [in] op LT operation code.
 * \param [in] cfg Data of ECMP.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
ecmp_lt_op(int unit, const bcmint_ltsw_l3_egress_tbl_t *ti, bcmlt_opcode_t op,
           ecmp_level_cfg_t *cfg)
{
    int dunit;
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    ...
    SHR_IF_ERR_EXIT
        (lt_eh_ecmp_fill(unit, eh, ti, cfg));
    ...
    if (op == BCMLT_OPCODE_LOOKUP) {
        ...
        cfg->fld_bmp = fbmp;
        SHR_IF_ERR_EXIT
            (lt_eh_ecmp_parse(unit, eh, ti, cfg));
    }
           ...
           bcm_l3_ecmp_member_t *ecmp_member_array)
{
    ...
    /* ECMP group info. */
    SHR_IF_ERR_EXIT
        (ecmp_info_to_cfg(unit, idx, ecmp_info, ecmp_member_count, &olecmp));
    max_paths = olecmp.flds[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS];
    num_paths = olecmp.rh ? olecmp.flds[XFS_LTSW_L3_ECMP_TBL_RH_NUM_PATHS] :
                            olecmp.flds[XFS_LTSW_L3_ECMP_TBL_NUM_PATHS];
    ...
    if (ecmp_info->flags & BCM_L3_REPLACE) {
        op = BCMLT_OPCODE_UPDATE;
        /* Set the unused member elements to 0. */
    ...
    olecmp.fld_bmp = tbl_info->fld_bmp;
    olecmp.start_idx = 0;
    olecmp.num_of_set = ecmp_member_count;
    olecmp.member_info = member;

    SHR_IF_ERR_EXIT
        (ecmp_lt_op(unit, tbl_info, op, &olecmp));

    if (ltsw_feature(unit, LTSW_FT_URPF)) {
        rv = secmp_set(unit, idx, ecmp_info, olecmp.member_info, num_paths);
        if (SHR_FAILURE(rv) && op == BCMLT_OPCODE_INSERT) {
            /* Roll back the inserted ECMP group. */
            olecmp.fld_bmp = 0;
            (void)ecmp_lt_op(unit, tbl_info, BCMLT_OPCODE_DELETE, &olecmp);
            SHR_ERR_EXIT(rv);
        }
    }
...
olecmp_del(int unit, int idx)
{
    ...
    sal_memset(&olecmp, 0, sizeof(ecmp_level_cfg_t));
    olecmp.flds[XFS_LTSW_L3_ECMP_TBL_ECMP_ID] = idx;

    SHR_IF_ERR_EXIT
        (ecmp_lt_op(unit, tbl_info, BCMLT_OPCODE_DELETE, &olecmp));
           ...
           int *ecmp_member_count)
{
    ...
    olecmp.flds[XFS_LTSW_L3_ECMP_TBL_ECMP_ID] = idx;
    SHR_IF_ERR_EXIT
        (lt_eh_ecmp_fill(unit, eh, tbl_info, &olecmp));

    SHR_IF_ERR_EXIT
        (bcmi_lt_entry_commit(unit, eh, BCMLT_OPCODE_LOOKUP,
                              BCMLT_PRIORITY_NORMAL));

    /* Parse ECMP group info. */
    olecmp.fld_bmp = tbl_info->fld_bmp;
    SHR_IF_ERR_EXIT
        (lt_eh_ecmp_parse(unit, eh, tbl_info, &olecmp));

    SHR_IF_ERR_EXIT
        (ecmp_cfg_to_info(unit, tbl_info, &olecmp, ecmp_info));
    ...
    if (ecmp_member_size > 0) {
        *ecmp_member_count = ecmp_member_size < num_paths ?
        ...
        olecmp.fld_bmp = (1 << XFS_LTSW_L3_ECMP_TBL_NHOP_1) |
                         (1 << XFS_LTSW_L3_ECMP_TBL_NHOP_2_OR_ECMP_1) |
                         (1 << XFS_LTSW_L3_ECMP_TBL_ECMP_AND_PROT_SWT_PROF_IDX)|
                         (1 << XFS_LTSW_L3_ECMP_TBL_DVP);
...
{
    ecmp_level_cfg_t olecmp;
    uint32_t sz, num_paths, num_paths_fid, max_paths;
    ...
    if (L3_OL_ECMP_GRP_FLAGS(unit, idx) & BCM_L3_ECMP_WEIGHTED) {
        ...
        olecmp.rh = true;
        tbl_id = BCMINT_LTSW_L3_ECMP_TBL_WEIGHTED_OL;
        num_paths_fid = XFS_LTSW_L3_ECMP_TBL_RH_NUM_PATHS;
    } else {
        tbl_id = BCMINT_LTSW_L3_ECMP_TBL_OL;
        num_paths_fid = XFS_LTSW_L3_ECMP_TBL_NUM_PATHS;
    }
    ...
    num_paths = olecmp.flds[num_paths_fid];
    max_paths = olecmp.rh ?
                (1 << (olecmp.flds[XFS_LTSW_L3_ECMP_TBL_WEIGHTED_SIZE] + 8)) :
                olecmp.flds[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS];
    ...
    member_fbmp = (1 << XFS_LTSW_L3_ECMP_TBL_NHOP_1) |
                  (1 << XFS_LTSW_L3_ECMP_TBL_NHOP_2_OR_ECMP_1) |
                  (1 << XFS_LTSW_L3_ECMP_TBL_ECMP_AND_PROT_SWT_PROF_IDX) |
                  (1 << XFS_LTSW_L3_ECMP_TBL_DVP);
    ...
    if ((L3_OL_ECMP_GRP_FLAGS(unit, idx) & BCM_L3_ECMP_PATH_NO_SORTING) ||
        ...
        olecmp.fld_bmp = member_fbmp;

        SHR_IF_ERR_EXIT
            (lt_eh_ecmp_parse(unit, eh, tbl_info, &olecmp));

        sal_memcpy(&arr[num_paths], member_info,
                   sizeof(bcmi_ltsw_l3_ecmp_member_info_t));
        num_paths ++;
        SHR_IF_ERR_EXIT
    ...
    olecmp.fld_bmp = member_fbmp | (1 << num_paths_fid);
    olecmp.flds[num_paths_fid] = num_paths;
    ...
    SHR_IF_ERR_EXIT
        (lt_eh_ecmp_fill(unit, eh_new, tbl_info, &olecmp));
    ...
    if (ltsw_feature(unit, LTSW_FT_URPF)) {
        rv = secmp_member_add(unit, idx, member_info);
        if (SHR_FAILURE(rv)) {
...
olecmp_member_del(int unit, int idx, bcm_l3_ecmp_member_t *ecmp_member)
{
    ecmp_level_cfg_t olecmp;
    int dunit, i, sz;
    uint32_t num_paths, num_paths_fid, max_paths;
    ...
    /* Parse ECMP group and member info. */
    ...
    if (ltsw_feature(unit, LTSW_FT_URPF)) {
        SHR_IF_ERR_EXIT
            (secmp_member_del(unit, idx, arr, num_paths, member_info));
    }
...
{
    ecmp_level_cfg_t olecmp;
    bcmint_ltsw_l3_egress_tbl_id_t tid;
    const bcmint_ltsw_l3_egress_tbl_t *ti = NULL;
    int dunit, num_paths_fid;
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    uint32_t num_paths;
    ...
    if (L3_OL_ECMP_GRP_FLAGS(unit, idx) & BCM_L3_ECMP_WEIGHTED) {
        ...
        olecmp.rh = true;
        tid = BCMINT_LTSW_L3_ECMP_TBL_WEIGHTED_OL;
        num_paths_fid = XFS_LTSW_L3_ECMP_TBL_RH_NUM_PATHS;
    } else {
        tid = BCMINT_LTSW_L3_ECMP_TBL_OL;
        num_paths_fid = XFS_LTSW_L3_ECMP_TBL_NUM_PATHS;
    }

    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_ecmp_tbl_get(unit, tid, &ti));
    ...
    olecmp.flds[XFS_LTSW_L3_ECMP_TBL_ECMP_ID] = idx;
    SHR_IF_ERR_EXIT
        (lt_eh_ecmp_fill(unit, eh, ti, &olecmp));

    SHR_IF_ERR_EXIT
        (bcmi_lt_entry_commit(unit, eh, BCMLT_OPCODE_LOOKUP,
                              BCMLT_PRIORITY_NORMAL));

    olecmp.fld_bmp = (1 << num_paths_fid);

    SHR_IF_ERR_EXIT
        (lt_eh_ecmp_parse(unit, eh, ti, &olecmp));
 ...
 */
static int
olecmp_find(int unit, int ecmp_member_count,
            bcmi_ltsw_l3_ecmp_member_info_t *ecmp_member,
            bcm_l3_egress_ecmp_t *ecmp_info)
{
    ...
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    ecmp_level_cfg_t cfg;
    uint32_t num_paths, num_paths_fid;
    bcmint_ltsw_l3_egress_tbl_id_t tid;
    const bcmint_ltsw_l3_egress_tbl_t *ti = NULL;

    SHR_FUNC_ENTER(unit);

    sz = L3_ECMP_WEIGHTED_MAX_PATHS(unit) *
         sizeof(bcmi_ltsw_l3_ecmp_member_info_t);
    SHR_ALLOC(emi, sz, "bcmLtswXfsEcmpMemberInfo");
    SHR_NULL_CHECK(emi, SHR_E_MEMORY);
    ...
    for (ltidx = L3_OL_ECMP_GRP_MIN(unit); ltidx <= L3_OL_ECMP_GRP_MAX(unit);
        ...
        tid = (wgt_i || rh_i) ? BCMINT_LTSW_L3_ECMP_TBL_WEIGHTED_OL :
                                BCMINT_LTSW_L3_ECMP_TBL_OL;

        num_paths_fid = rh_i ? XFS_LTSW_L3_ECMP_TBL_RH_NUM_PATHS :
        ...
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add_by_id(eh,
                                 ti->flds[XFS_LTSW_L3_ECMP_TBL_ECMP_ID].fld_id,
                                 ltidx));
        ...
        cfg.fld_bmp = (1 << num_paths_fid);

        SHR_IF_ERR_EXIT
            (lt_eh_ecmp_parse(unit, eh, ti, &cfg));

        num_paths = cfg.flds[num_paths_fid];
        ...
        SHR_IF_ERR_EXIT
            (ecmp_cfg_to_info(unit, ti, &cfg, ecmp_info));
                  ...
                  int count, bcmi_ltsw_l3_ecmp_member_info_t* member)
{
    ...
    /* ECMP group info. */
    SHR_IF_ERR_EXIT
        (ecmp_info_to_cfg(unit, idx, ecmp_info, count, &ulecmp));

    if (ulecmp.weighted || ulecmp.rh) {
        tid = BCMINT_LTSW_L3_ECMP_TBL_WEIGHTED_UL;
    } else {
        tid = BCMINT_LTSW_L3_ECMP_TBL_UL;
    }

    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_ecmp_tbl_get(unit, tid, &ti));

    ulecmp.fld_bmp = ti->fld_bmp;
    if (ecmp_info->ecmp_group_flags & BCM_L3_ECMP_MEMBER_WEIGHTED) {
        ulecmp.fld_bmp &= ~(1 << XFS_LTSW_L3_ECMP_TBL_NHOP_1);
    } else {
        ulecmp.fld_bmp &= ~(1 << XFS_LTSW_L3_ECMP_TBL_WEIGHTED_NHOP_INDEX);
    }
    ...
    if (ecmp_info->flags & BCM_L3_REPLACE) {
        ...
        ulecmp.num_of_unset = ulecmp.flds[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS] -
                              count;
    } else {
        op = BCMLT_OPCODE_INSERT;
    }
...
ulecmp_config_del(int unit, int idx)
{
    ...
    if ((L3_UL_ECMP_GRP_FLAGS(unit, idx) &
         (BCM_L3_ECMP_WEIGHTED | BCM_L3_ECMP_MEMBER_WEIGHTED)) ||
    ...
    sal_memset(&ulecmp, 0, sizeof(ecmp_level_cfg_t));
    ulecmp.flds[XFS_LTSW_L3_ECMP_TBL_ECMP_ID] = idx;

    SHR_IF_ERR_EXIT
        (ecmp_lt_op(unit, ti, BCMLT_OPCODE_DELETE, &ulecmp));
 ...
 */
static int
ulecmp_config_member_add(int unit, int idx,
                         bcmi_ltsw_l3_ecmp_member_info_t *member,
                         bool alt, int pri_cnt,
                         bcmi_ltsw_l3_ecmp_member_info_t *dlb_member)
{
    ecmp_level_cfg_t ulecmp;
    int dunit, alt_cnt, i;
    uint32_t sz, num_paths, num_paths_fid, max_paths;
    ...
    ulecmp.fld_bmp = ti->fld_bmp;
    SHR_IF_ERR_EXIT
        (lt_eh_ecmp_parse(unit, eh, ti, &ulecmp));

    num_paths = ulecmp.flds[num_paths_fid];
    if (ulecmp.rh) {
        max_paths = 1 << (ulecmp.flds[XFS_LTSW_L3_ECMP_TBL_WEIGHTED_SIZE] + 8);
    } else {
        max_paths = ulecmp.flds[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS];
    }
    ...
    member_fbmp = (1 << XFS_LTSW_L3_ECMP_TBL_NHOP_1) |
                  (1 << XFS_LTSW_L3_ECMP_TBL_NHOP_2_OR_ECMP_1) |
                  (1 << XFS_LTSW_L3_ECMP_TBL_STRENGTH_PROF_IDX);
    ...
    ulecmp.flds[num_paths_fid] = num_paths + 1;
    ulecmp.fld_bmp = member_fbmp | (1 << num_paths_fid);

    SHR_IF_ERR_EXIT
        (bcmlt_entry_clear(eh));

    SHR_IF_ERR_EXIT
        (lt_eh_ecmp_fill(unit, eh, ti, &ulecmp));

    SHR_IF_ERR_EXIT
        (bcmi_lt_entry_commit(unit, eh, BCMLT_OPCODE_UPDATE,
                              BCMLT_PRIORITY_NORMAL));
    if (dlb_member) {
        sal_memcpy(dlb_member, arr,
                   (num_paths + 1) * sizeof(bcmi_ltsw_l3_ecmp_member_info_t));
    }
...
ulecmp_config_member_del_all(int unit, int idx)
{
    ...
    ulecmp.fld_bmp = 1 << num_paths_fid;
           ...
           bcm_l3_ecmp_member_t *ecmp_member_array, bool dlb_en)
{
    ...
    wgt = (ecmp_info->ecmp_group_flags &
          (BCM_L3_ECMP_WEIGHTED | BCM_L3_ECMP_MEMBER_WEIGHTED)) ? true : false;
           ...
           int *ecmp_member_count)
{
    ...
    bool dlb_en = false;
    int dunit, sz, i, j, ltidx, num_paths, num_paths_fid;
    ...
    /* Parse ECMP group info. */
    ulecmp.fld_bmp = ti->fld_bmp;
    if (L3_UL_ECMP_GRP_FLAGS(unit, idx) & BCM_L3_ECMP_MEMBER_WEIGHTED) {
        ulecmp.fld_bmp &= ~(1 << XFS_LTSW_L3_ECMP_TBL_NHOP_1);
    } else {
        ulecmp.fld_bmp &= ~(1 << XFS_LTSW_L3_ECMP_TBL_WEIGHTED_NHOP_INDEX);
    }
    ...
    if (dlb_en) {
        ...
        ecmp_info->max_paths = ulecmp.flds[XFS_LTSW_L3_ECMP_TBL_MAX_PATHS];
        ...
        SHR_IF_ERR_EXIT
            (ecmp_cfg_to_info(unit, ti, &ulecmp, ecmp_info));
    }
    ...
    if (ecmp_member_size > 0) {
        *ecmp_member_count = ecmp_member_size < num_paths ?
        ...
        for (i = 0; i < *ecmp_member_count; i++) {
            ...
            ecmp_member_array[i].egress_if_2 = (j == 2) ? intfs[1] : 0;
            if (L3_UL_ECMP_GRP_FLAGS(unit, idx) & BCM_L3_ECMP_MEMBER_WEIGHTED) {
                ecmp_member_array[i].weight = ulecmp.member_info[i].weight;
            }
...
ulecmp_member_add(int unit, int idx, bcm_l3_ecmp_member_t *ecmp_member)
{
    ...
    SHR_IF_ERR_EXIT
        (ecmp_member_info_array_alloc(unit, 1, ecmp_member, &member,
                                      &pri_cnt, &egr_types, &prot_ul, false));
            ...
            int pri_cnt, bcm_l3_egress_ecmp_t *ecmp_info)
{
    ...
    for (ltidx = L3_UL_ECMP_GRP_MIN(unit); ltidx <= L3_UL_ECMP_GRP_MAX(unit);
        ...
        if (L3_ECMP_DLB_GRP_CONFIGURED(unit, ltidx)) {
            ...
            wgt_i = (L3_UL_ECMP_GRP_FLAGS(unit, ltidx) &
                    (BCM_L3_ECMP_WEIGHTED | BCM_L3_ECMP_MEMBER_WEIGHTED)) ?
                    true : false;
            rh_i = (L3_OL_ECMP_GRP_DYNA_MODE(unit, ltidx) == XFS_ECMP_DM_RESILIENT) ?
                   true : false;
        ...
        /* Fetch group and member info from ECMP_LEVEL1 */
        tid = (wgt_i || rh_i) ? BCMINT_LTSW_L3_ECMP_TBL_WEIGHTED_UL :
        ...
        ulecmp.fld_bmp = ti->fld_bmp;
        if (L3_UL_ECMP_GRP_FLAGS(unit, ltidx) & BCM_L3_ECMP_MEMBER_WEIGHTED) {
            ulecmp.fld_bmp &= ~(1 << XFS_LTSW_L3_ECMP_TBL_NHOP_1);
        } else {
            ulecmp.fld_bmp &= ~(1 << XFS_LTSW_L3_ECMP_TBL_WEIGHTED_NHOP_INDEX);
        }
...
ecmp_member_dest_table_init(int unit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_ECMP_MEMBER1_NO_DEFAULT_INIT)) {
        SHR_EXIT();
    }
...
l3_egress_tables_clear(int unit)
{
    ...
    for (i = 0; i < num; i++) {
        if (ltsw_feature(unit, LTSW_FT_ECMP_MEMBER1_NO_DEFAULT_INIT)) {
            if (!sal_strcmp(ltname[i], ECMP_MEMBER1s)) {
                continue;
            }
        }
    ...
    for (i = 0; i < BCMINT_LTSW_L3_ECMP_TBL_CNT; i++) {
        if (SHR_SUCCESS(bcmint_ltsw_l3_ecmp_tbl_get(unit, i, &tbl_info))) {
            (void)bcmi_lt_clear(unit, tbl_info->name);
        }
    }
...
exit:
    if (SHR_FUNC_ERR()) {
        if (nh_add && !(egr->flags & BCM_L3_REPLACE)) {
            if (ul) {
...
xfs_ltsw_l3_egress_find(int unit, bcm_l3_egress_t *egr, int *idx)
{
    ...
    for (nhidx = min; nhidx <= max; nhidx ++) {
        ...
        /* Only compare intf/port/mac for now. */
...
exit:
    if (SHR_FUNC_ERR()) {
        if (nh_add && !(egr_ifa->flags & BCM_L3_REPLACE)) {
                            ...
                            bcm_l3_ecmp_member_t *ecmp_member)
{
    ...
    if (type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_UL) {
        ...
        if (L3_UL_ECMP_GRP_FLAGS(unit, ltidx) & BCM_L3_ECMP_MEMBER_WEIGHTED) {
            /* Can't add member to weighted ECMP group in member weighted mode. */
            SHR_IF_ERR_EXIT(SHR_E_FULL);
        }
        ...
        if (L3_OL_ECMP_GRP_FLAGS(unit, ltidx) & BCM_L3_ECMP_MEMBER_WEIGHTED) {
            /* Can't add member to weighted ECMP group in member weighted mode. */
            SHR_IF_ERR_EXIT(SHR_E_FULL);
        }
