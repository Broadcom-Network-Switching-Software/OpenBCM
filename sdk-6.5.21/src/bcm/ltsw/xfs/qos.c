...
qos_port_egr_l2_map_set(int unit, bcm_port_t port, int src)
{
    ...
    /* Get L2 oif. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_to_l2_if(unit, port, &l2_oif));
...
qos_egr_policy_table_init(int unit)
{
    ...
    for (i = 0; i < ent_num; i++) {
         ...
         */
        for (j = 0; j < field_num; j++) {
            if (tbl_cfg->field_array[j] && tbl_ent->field_flags[j] != 0) {
    ...
    for (i = 0; i < ent_num; i++) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(entry_hdl,
                                   ...
                                   i + index));
        for (j = 0; j < field_num; j++) {
            if (tbl_cfg->field_array[j] && tbl_cfg->one_to_one) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl,
                                           tbl_cfg->field_array[j],
                                           i));
            } else if (tbl_cfg->field_array[j]) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl,
                                           tbl_cfg->field_array[j],
                                           tbl_cfg->ent[i].field[j]));
            }
        }
