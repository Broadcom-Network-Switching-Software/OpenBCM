...
ltsw_xfs_dlb_control_info_value_validate(
    ...
    uint64_t value)
{
    bcmint_dlb_ctrl_info_t *cinfo = ctrl_info[dlb_type];
    uint64_t min, max;
    const char *lt_name = DLB_QUANTIZATION_THRESHOLDs;

    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(cinfo, SHR_E_MEMORY);
    ...
    uint64_t value)
{
    ...
    SHR_NULL_CHECK(ctrl_info, SHR_E_MEMORY);
...
ltsw_xfs_dlb_control_info_set(
    ...
    uint64_t value)
{
    bcmint_dlb_ctrl_info_t cinfo, *ci = ctrl_info[dlb_type];
    int rv;
    ...
    ci = ctrl_info[bcmi_dlb_type_fp_trunk];
    if (ci) {
        SHR_IF_ERR_VERBOSE_EXIT
