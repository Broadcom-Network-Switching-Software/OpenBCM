...
#define VP_SAME_VP_CHK_EN_SFT VP_SHC_EN_SFT
...
/*! Field definition of QOS_REMARK_CTRL in EGR_L3_TUNNEL_0. */
#define QOS_REMARK_CTRL_DISABLE_REMARKING_SFT 0
#define QOS_REMARK_CTRL_DISABLE_REMARKING_MSK 0x1
#define QOS_REMARK_CTRL_COPY_DOT1P_SFT        1
#define QOS_REMARK_CTRL_COPY_DOT1P_MSK        0x1
#define QOS_REMARK_CTRL_COPY_TOS_SFT          2
#define QOS_REMARK_CTRL_COPY_TOS_MSK          0x1
                                        ...
                                        bcm_flow_tunnel_terminator_t *info)
{
    ...
    if (!sal_strncmp(lt_name, L2_TUNNEL_IPV6_DIP_TABLEs, lt_len)) {
        info->valid_elements = BCM_FLOW_TUNNEL_TERM_DIP6_VALID;
        info->flow_handle = BCMINT_FLOW_ID_VXLAN;
        info->type = bcmTunnelTypeVxlan6;
    } else if (!sal_strncmp(lt_name, L2_TUNNEL_IPV4_DIP_TABLEs, lt_len)) {
                              ...
                              bcm_flow_tunnel_initiator_t *info)
{
    ...
    uint32_t act_cnt = 0;
    int evpn_ptr;
    const bcmint_flow_lt_t *egr_tnl_0_lt = NULL;
    ...
    rv = bcmint_flow_lt_get(unit, BCMINT_LTSW_FLOW_LT_EGR_L3_TUNNEL_0,
                            &egr_tnl_0_lt);
    SHR_IF_ERR_EXIT_EXCEPT_IF(rv, SHR_E_UNAVAIL);

    if (egr_tnl_0_lt != NULL) {
        fid  = (int)BCMINT_LTSW_FLOW_FLD_EGR_L3_TUNNEL_0_QOS_REMARK_CTRL;
        if (egr_tnl_0_lt->fld_bmp & (1 << fid)) {
            flds = egr_tnl_0_lt->flds;
            fld_name = flds[fid].name;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_get(entry_hdl1, fld_name, &get_val));

            if ((get_val >> QOS_REMARK_CTRL_DISABLE_REMARKING_SFT) &
                QOS_REMARK_CTRL_DISABLE_REMARKING_MSK) {
                info->flags |= BCM_TUNNEL_INIT_DISABLE_QOS_REMARK;
            }

            if ((get_val >> QOS_REMARK_CTRL_COPY_DOT1P_SFT) &
                QOS_REMARK_CTRL_COPY_DOT1P_MSK) {
                info->pri_sel = bcmTunnelPriPayload;
            } else {
                info->pri_sel = bcmTunnelPriNone;
            }

            if ((get_val >> QOS_REMARK_CTRL_COPY_TOS_SFT) &
                QOS_REMARK_CTRL_COPY_TOS_MSK) {
                info->dscp_ecn_sel = bcmTunnelDscpEcnPayloadDscp;
            }
        }
    }
                                           ...
                                           bcm_flow_tunnel_initiator_t *info)
{
    ...
    int rv = SHR_E_NONE;
    int qos_remark_ctrl_supported  = 0;
    const bcmint_flow_lt_t *egr_tnl_0_lt = NULL;
    ...
    if (opt == HW_OPT_SET) {
        ...
        if (info->flow_handle == BCMINT_FLOW_ID_VXLAN) {
            supported_items = BCM_FLOW_TUNNEL_INIT_TTL_VALID |
                              ...
                              BCM_FLOW_TUNNEL_INIT_PRI_SEL_VALID |
        ...
        if (egr_tnl_0_lt != NULL) {
            if (egr_tnl_0_lt->fld_bmp &
                (1 << BCMINT_LTSW_FLOW_FLD_EGR_L3_TUNNEL_0_QOS_REMARK_CTRL)) {
                qos_remark_ctrl_supported = 1;
            }
        }

        if (!qos_remark_ctrl_supported) {
            if (info->flags & BCM_TUNNEL_INIT_DISABLE_QOS_REMARK) {
                SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                    (BSL_META_U(unit,
                                     "Bad flag: DISABLE_QOS_REMARK!\n")));
            }

            if (info->valid_elements & BCM_FLOW_TUNNEL_INIT_PRI_SEL_VALID) {
                SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                    (BSL_META_U(unit,
                                     "Invalid element: PRI_CFI_VALID!\n")));
            }
        ...
        if (info->valid_elements & BCM_FLOW_TUNNEL_INIT_PRI_SEL_VALID) {
            if ((info->pri_sel < bcmTunnelPriNone) ||
                (info->pri_sel >= bcmTunnelPriCount)) {
                SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                    (BSL_META_U(unit, "Bad dscp_ecn_sel:%d!\n"),
                                                info->dscp_ecn_sel));
            }
                                 ...
                                 void *entry)
{
    ...
    uint64 get_val;
    int evpn_enable = 0;
    uint64_t qos_remark_ctrl_val = 0;
    int qos_remark_ctrl_valid = 0;
    const bcmint_flow_lt_t *egr_tnl_0_lt = NULL;
    const bcmint_flow_fld_t *flds;
    int fid;
    const char *fld_name;
    ...
    if (info->flags & BCM_TUNNEL_REPLACE) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(entry_hdl2, TUNNEL_WORD_0s,
                                   &tunnel_word_0_val));
        if (qos_remark_ctrl_valid) {
            flds = egr_tnl_0_lt->flds;
            fid  = (int)BCMINT_LTSW_FLOW_FLD_EGR_L3_TUNNEL_0_QOS_REMARK_CTRL;
            fld_name = flds[fid].name;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_get(entry_hdl1, fld_name,
                                       &qos_remark_ctrl_val));
        }
    ...
    if (qos_remark_ctrl_valid) {
        if (info->flags & BCM_TUNNEL_INIT_DISABLE_QOS_REMARK) {
            qos_remark_ctrl_val |= (QOS_REMARK_CTRL_DISABLE_REMARKING_MSK <<
                                   QOS_REMARK_CTRL_DISABLE_REMARKING_SFT);
        } else {
            qos_remark_ctrl_val &= ~(QOS_REMARK_CTRL_DISABLE_REMARKING_MSK <<
                                   QOS_REMARK_CTRL_DISABLE_REMARKING_SFT);
        }
        ...
        if (info->valid_elements & BCM_FLOW_TUNNEL_INIT_PRI_SEL_VALID) {
            if (info->pri_sel == bcmTunnelPriPayload) {
                qos_remark_ctrl_val |= (QOS_REMARK_CTRL_COPY_DOT1P_MSK <<
                                       QOS_REMARK_CTRL_COPY_DOT1P_SFT);
            } else if (info->pri_sel == bcmTunnelPriNone) {
                qos_remark_ctrl_val &= ~(QOS_REMARK_CTRL_COPY_DOT1P_MSK <<
                                       QOS_REMARK_CTRL_COPY_DOT1P_SFT);
            }
        }
    ...
    if (info->valid_elements & BCM_FLOW_TUNNEL_INIT_DSCP_ECN_VALID) {
        ...
        if (qos_remark_ctrl_valid) {
            qos_remark_ctrl_val &= ~(QOS_REMARK_CTRL_COPY_TOS_MSK <<
                                   QOS_REMARK_CTRL_COPY_TOS_SFT);
        }

        if ((info->dscp_ecn_sel == bcmTunnelDscpEcnPayloadDscp) &&
            (qos_remark_ctrl_valid != 0)) {
                qos_remark_ctrl_val |= (QOS_REMARK_CTRL_COPY_TOS_MSK <<
                                       QOS_REMARK_CTRL_COPY_TOS_SFT);
        } else if (info->dscp_ecn_sel == bcmTunnelDscpEcnMap) {
    ...
    if (qos_remark_ctrl_valid) {
        ...
        fld_name = flds[fid].name;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(entry_hdl1, fld_name, qos_remark_ctrl_val));
    }
    ...
    if (qos_remark_ctrl_valid && (qos_remark_ctrl_val == 0)) {
        /* Do not remark qos ctrl. */
                         ...
                         bcm_flow_match_config_t *info)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_to_l2_if(unit, info->port, &l2_iif));
                                  ...
                                  bcm_flow_match_config_t *info)
{
    ...
    if (!sal_strncmp(lt_name, ING_VLAN_XLATE_1_TABLEs, lt_len)) {
        info->criteria = BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_VLAN;
        info->valid_elements = BCM_FLOW_MATCH_PORT_VALID |
                               BCM_FLOW_MATCH_VLAN_VALID;
    } else if (!sal_strncmp(lt_name, ING_VLAN_XLATE_2_TABLEs, lt_len)) {
        info->criteria = BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_INNER_VLAN;
        info->valid_elements = BCM_FLOW_MATCH_PORT_VALID |
                               BCM_FLOW_MATCH_INNER_VLAN_VALID;
    } else if (!sal_strncmp(lt_name, ING_VLAN_XLATE_3_TABLEs, lt_len)) {
        info->criteria = BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_VLAN_STACKED;
        info->valid_elements = BCM_FLOW_MATCH_PORT_VALID |
                               BCM_FLOW_MATCH_VLAN_VALID |
                               BCM_FLOW_MATCH_INNER_VLAN_VALID;
    } else if (!sal_strncmp(lt_name, L2_TUNNEL_VN_ID_TABLEs, lt_len)) {
    ...
    } else if (!sal_strncmp(lt_name, L2_TUNNEL_IPV4_SIP_TABLEs, lt_len)) {
        info->criteria = BCM_FLOW_MATCH_CRITERIA_SIP;
        info->valid_elements = BCM_FLOW_MATCH_SIP_VALID;
    } else if (!sal_strncmp(lt_name, L2_TUNNEL_IPV6_SIP_TABLEs, lt_len)) {
        info->criteria = BCM_FLOW_MATCH_CRITERIA_SIP;
                                    ...
                                    const char *lt_name)
{
    ...
    while ((rv = bcmi_lt_entry_commit(dunit, entry_hdl, BCMLT_OPCODE_TRAVERSE,
        ...
        if (vp != -1) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_l2_if_to_port(unit, l2_iif, &info.port));
...
xfs_flow_match_criteria_adjust(int unit,
                               ...
                               bcm_flow_match_config_t *info)
{
    if (!sal_strncmp(lt_name, L2_TUNNEL_VN_ID_TABLEs, sal_strlen(lt_name))) {
                        ...
                        bcmlt_entry_handle_t entry_hd)
{
    ...
    if (info->valid_elements & BCM_FLOW_ENCAP_VNID_VALID) {
        ...
        if (!sal_strncmp(lt_name, EGR_VLAN_TRANSLATION_2s, lt_len)) {
    ...
    bcm_flow_port_t *flow_port)
{
    ...
    if (opt == HW_OPT_SET) {
        ...
        if (flow_port->flags & BCM_FLOW_PORT_NETWORK) {
            ...
            /* Enable same VP check. */
            field_val = 1 << VP_SAME_VP_CHK_EN_SFT;
        }
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    if (opt == HW_OPT_GET) {
        ...
        if (!(vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT)) {
            ...
            if (value == DEST_TYPE_L2_OIF) {
                l2_if = (int)dest;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_id_l2_if_to_port(unit, l2_if, &info->dst_port));
            }
        }
        ...
        if (info->options & BCM_FLOW_PORT_ENCAP_OPTION_CLEAR) {
            ...
            if (info->valid_elements & BCM_FLOW_PORT_ENCAP_DST_PORT_VALID) {
                if (!(vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT)) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmi_ltsw_id_port_to_l2_if(unit, info->dst_port, &l2_if));
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_entry_field_add(ent_hd_ing_dvp,
                                               DESTINATIONs, l2_if));
                    SHR_IF_ERR_VERBOSE_EXIT
