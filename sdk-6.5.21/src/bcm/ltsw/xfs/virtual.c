...
xfs_ltsw_virtual_vfi_init(int unit, int vfi)
{
    ...
    ent_type = ltsw_feature(unit, LTSW_FT_DVP_TAG_CTRL) ?
               BCMI_LTSW_SBR_PET_NO_CLASSID : BCMI_LTSW_SBR_PET_NONE;
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_sbr_profile_ent_index_get(unit, pth, ent_type,
                                             &profile_idx));
                                    ...
                                    int *arg)
{
    ...
    if ((type == bcmVlanPortUseOuterPri) ||
        ...
        (type == bcmVlanPortTranslateEgressKey)) {
        process_egr_dvp = 1;
    }
    ...
    case bcmVlanPortTranslateEgressKey:
        ...
        /* Key selection is available when bcmVlanTranslateEgressEnable = 1. */
        fld_mask = 1 << BCMI_XFS_VP_PROCESS_CTRL_EVXLT1_DISABLE_SFT;
        if (get_val & fld_mask) {
            SHR_ERR_EXIT(SHR_E_UNAVAIL);
        }

        fld_mask = 1 << BCMI_XFS_VP_PROCESS_CTRL_EVXLT1_SELECT_SFT;
        *arg = (get_val & fld_mask)? bcmVlanTranslateEgressKeyVpnGportGroup:
                                     bcmVlanTranslateEgressKeyVpnPortGroup;
        break;
                                    ...
                                    int arg)
{
    ...
    case bcmVlanTranslateEgressEnable:
        ...
        break;
    case bcmVlanPortTranslateEgressKey:
        if ((arg != bcmVlanTranslateEgressKeyVpnGportGroup) &&
            (arg != bcmVlanTranslateEgressKeyVpnPortGroup)) {
            SHR_ERR_EXIT(BCM_E_UNAVAIL);
        }

        /*
         * Select key only when bcmVlanTranslateEgressEnable = 1,
         * otherwise, the key selection might be overwritten.
         */
        if (process_ctrl_val &
            (1 << BCMI_XFS_VP_PROCESS_CTRL_EVXLT1_DISABLE_SFT)) {
            SHR_ERR_EXIT(SHR_E_UNAVAIL);
        }
        process_ctrl_val &= ~(BCMI_XFS_VP_PROCESS_CTRL_EVXLT1_SELECT_MSK <<
                              BCMI_XFS_VP_PROCESS_CTRL_EVXLT1_SELECT_SFT);
        if (arg == bcmVlanTranslateEgressKeyVpnGportGroup) {
            process_ctrl_val |=
                (0x1 & BCMI_XFS_VP_PROCESS_CTRL_EVXLT1_SELECT_MSK) <<
                 BCMI_XFS_VP_PROCESS_CTRL_EVXLT1_SELECT_SFT;
        }
        break;
    default:
...
int
xfs_ltsw_virtual_port_egress_adapt_key_sync_status_get(
    int unit,
    int *sync_required)
{
    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(sync_required, SHR_E_PARAM);
    *sync_required = 0x0;
