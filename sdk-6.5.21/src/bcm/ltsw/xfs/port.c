...
static port_control_map_t port_control_port_map[] = {
    ...
    {bcmPortControlFabricSourceKnockout, BCMI_PT_HG3_BRIDGE,
         xfs_pc_invert, xfs_pc_invert},
    /* PM controls */
...
xfs_port_default_prio_cfi_get(int unit, int port, int *prio, int *cfi)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_DEFAULT_PRI, &prio_cfi));
...
xfs_port_default_prio_cfi_set(int unit, int port, int prio, int cfi)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_DEFAULT_PRI, prio_cfi));
...
{
    return bcmint_ltsw_port_tab_bit_set
            (unit, port, BCMI_PT_EGR_L2_PROCESS_CTRL, 2, enable ? 1 : 0);
}
...
{
    return bcmint_ltsw_port_tab_bit_get
            (unit, port, BCMI_PT_EGR_L2_PROCESS_CTRL, 2, enable);
}
...
{
    return bcmint_ltsw_port_tab_bit_set
            (unit, port, BCMI_PT_EGR_L2_PROCESS_CTRL, 1, enable ? 0 : 1);
}
...
xfs_port_egr_bridge_get(int unit, bcm_port_t port, int *enable)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_bit_get
            (unit, port, BCMI_PT_EGR_L2_PROCESS_CTRL, 1, enable));
...
xfs_port_egr_vlan_pri_src_set(int unit, bcm_port_t port, int src)
{
    ...
    if (xfs_port_db[unit]->feature->egr_vlan_pri_src) {
        ...
        SHR_IF_ERR_EXIT
            (bcmint_ltsw_port_tab_bit_set
                (unit, port, BCMI_PT_EGR_L2_PROCESS_CTRL, 3, en));
    } else {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
...
xfs_port_trust_vlan_set(int unit, bcm_port_t port, int enable)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_L2_DEST_TYPE, &type));
    if (type == DEST_TYPE_VP) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_VLAN_MAP_TO_VFI, enable));
...
xfs_port_trust_vlan_get(int unit, bcm_port_t port, int *enable)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_VLAN_MAP_TO_VFI, enable));
    ...
    for (idx = 0; idx < COUNTOF(port_control_port_map); idx++) {
        if (port_control_port_map[idx].type == type) {
            if (port_control_port_map[idx].flags & PORT_CTRL_PM) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmint_ltsw_port_tab_get(unit, port, fld, value));
    ...
    for (idx = 0; idx < COUNTOF(port_control_port_map); idx++) {
        if (port_control_port_map[idx].type == type) {
            if (port_control_port_map[idx].flags & PORT_CTRL_PM) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmint_ltsw_port_tab_set(unit, port, fld, value));
            }
                      ...
                      uint16_t tpid)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, pt, &tpid_bmp));
    ...
    if (tpid_bmp & (1 << tpid_index)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_set(unit, port, pt, tpid_bmp));
    }
                      ...
                      uint16_t tpid)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, pt, 1 << tpid_index));
                             ...
                             uint16_t tpid_default)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, pt, (1 << tpid_default_index)));
...
{
    if (type == OUTER_TPID) {
        return bcmint_ltsw_port_tab_get
...
xfs_ifp_ing_egr_block_init(int unit)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set
            (unit, BLOCK_PROFILE_IFP, BCMI_PT_ING_EGR_VLAN_MEMBERSHIP_CHECK, 1));
    ...
    PORT_FP_ITER(unit, port) {
        SHR_IF_ERR_VERBOSE_EXIT
            (xfs_ltsw_port_l3_mtu_set
                (unit, port, ltsw_port_info[unit]->max_frame_size));
    }
...
xfs_port_group_set(int unit, bcm_port_t port, int dir, int port_group)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
       (bcmint_ltsw_port_tab_set(unit, port, pt, port_group));
...
xfs_ltsw_port_info_init(int unit)
{
    ...
    if (!bcmi_warmboot_get(unit)) {
        rv = bcmi_ltsw_issu_struct_info_get
                (unit, BCMINT_PORT_XFS_HA_INFO_T_ID, BCMI_HA_COMP_ID_PORT,
                 BCMINT_PORT_SUB_COMP_ID_XFS_HA, &offset);
        if (rv == SHR_E_NOT_FOUND) {
            SHR_IF_ERR_EXIT
                (bcmi_ltsw_issu_struct_info_report
                    (unit, BCMI_HA_COMP_ID_PORT,
                     BCMINT_PORT_SUB_COMP_ID_XFS_HA,
...
xfs_ltsw_port_profile_deinit_per_port(int unit, int port)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_EGR_MASK_ID, &pid_old));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_EGR_MASK_ID, pid_def));
...
xfs_ltsw_port_gih_cpu_init(int unit)
{
    ...
    /* port type */
    types[0] = BCMI_PT_GIH_CPU_PORT_TYPE;
    data[0] = PORT_TYPE_ETHERNET;
    /* parser type */
    types[1] = BCMI_PT_GIH_CPU_PARSER_TYPE;
    data[1] = PARSER_TYPE_LOOPBACK;
    /* system port */
    types[2] = BCMI_PT_GIH_CPU_SYS_PORT;
    SHR_IF_ERR_VERBOSE_EXIT
        (xfs_ltsw_port_to_sys_port(unit, 0, &sys_port));
    data[2] = sys_port;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_set(unit, 0, types, data, COUNTOF(types)));
                            ...
                            uint32_t op)
{
    ...
    for (i = 0, fld = fields; i < count; i ++, fld ++) {
        ...
        if ((fld->op & op) == PORT_CFG) {
            ...
            if (fld->index == BCMI_L2_IF_INVALID) {
                fld->op &= ~TRUNK_CFG;
            }
        }
    }
...
xfs_ltsw_port_l2_if_add(int unit, int port, int l2_if)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_map_port(unit, l2_if, port));

    SHR_IF_ERR_VERBOSE_EXIT
        (xfs_ltsw_l2_if_add(unit, l2_if));

    /* l2_oif for mc */
    types[0] = BCMI_PT_EGR_MC_L2_OIF;
    data[0] = l2_if;
    ...
    types[1] = BCMI_PT_L2_OIF_STR_PROFILE_ID;
    data[1] = id;
...
xfs_ltsw_port_l2_if_del(int unit, int port)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_to_l2_if(unit, port, &l2_if));
...
xfs_ltsw_gport_to_trunk_l2_if(int unit, int gport, int *l2_if)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_lag_get(unit, gport, &lag));

    if (lag == BCM_TRUNK_INVALID) {
        *l2_if = BCMI_L2_IF_INVALID;
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_to_l2_if(unit, lag, l2_if));
    }
...
xfs_ltsw_l2_if_to_pipes(int unit, int l2_if, uint32_t *pipe_bmp)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_to_port(unit, l2_if, &gport));
...
xfs_ltsw_port_sys_port_add(int unit, int port, int sys_port)
{
    ...
    types[0] = BCMI_PT_ING_SYS_PORT;
    data[0] = sys_port;

    /* ing sys port -> ing pp port */
    types[1] = BCMI_PT_ING_PP_PORT;
    data[1] = port;
    ...
    types[2] = BCMI_PT_SYS_PORT_STR_PROFILE_ID;
    data[2] = id;
    ...
    types[3] = BCMI_PT_SYS_DEST_STR_PROFILE_ID;
    data[3] = id;
    ...
    types[4] = BCMI_PT_ING_SYS_DEST_L2_OIF_STR;
    data[4] = strength;
    ...
    types[5] = BCMI_PT_L2MC_L3MC_L2_OIF_SYS_DST_VALID_STR;
    data[5] = strength;

    /* egr sys port isn't system trunk */
    types[6] = BCMI_PT_DEST_SYS_PORT_IS_TRUNK;
    data[6] = 0;

    /* egr sys port -> egr local port */
    types[7] = BCMI_PT_DEST_SYS_PORT_LPORT;
    data[7] = port;
...
xfs_ltsw_port_sys_port_move(int unit, int port, int sys_port)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_ING_SYS_PORT, sys_port));
...
xfs_ltsw_sys_port_map_l2_if(int unit, int sys_port, int l2_if)
{
    ...
    /* ing sys port -> l2_iif */
    types[0] = BCMI_PT_ING_L2_IIF;
    data[0] = l2_if;
    index[0] = sys_port;

    /* dest sys port -> l2_oif */
    types[1] = BCMI_PT_ING_SYS_DEST_L2_OIF;
    data[1] = l2_if;
    index[1] = sys_port;


    /* is l2_oif a trunk */
    types[2] = BCMI_PT_L2_OIF_IS_TRUNK;
    data[2] = 0;
    index[2] = l2_if;

    /* l2_oif -> egr sys port */
    types[3] = BCMI_PT_L2_OIF_SYS_PORT;
    data[3] = sys_port;
    index[3] = l2_if;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_raw_multi_set(unit, 0, index, types, data, COUNTOF(types)));
...
xfs_ltsw_port_learn_set(int unit, bcm_port_t port, uint32_t flags)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_CML_NEW, cml));
...
xfs_ltsw_port_learn_get(int unit, bcm_port_t port, uint32_t *flags)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_CML_NEW, &cml));
...
xfs_ltsw_port_l3_mtu_set(int unit, bcm_port_t port, int size)
{
    ...
    if (xfs_port_db[unit]->feature->l2_mtu) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_bit_set
                (unit, port, BCMI_PT_EGR_L2_PROCESS_CTRL, 3, 1));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_L2_MTU, size));
    }
...
xfs_ltsw_port_tpid_set(int unit, bcm_port_t port, int type, uint16_t tpid)
{
    ...
    if (type & OUTER_TPID) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_EGR_OTPID, tpid));
    }
...
xfs_ltsw_port_tpid_get(int unit, bcm_port_t port, int type, uint16_t *tpid)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_EGR_OTPID, &tpid_egr));
...
xfs_ltsw_port_inner_tpid_set(int unit, bcm_port_t port, uint16_t tpid)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_EGR_ITPID, tpid));
...
xfs_ltsw_port_untagged_vlan_get(int unit, bcm_port_t port, bcm_vlan_t *vid)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_DEFAULT_VFI, &vfi));
...
xfs_ltsw_port_untagged_vlan_set(int unit, bcm_port_t port, bcm_vlan_t vid)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_DEFAULT_VFI, vid));
                             ...
                             int pkt_prio, uint32 flags)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_ING_PVLAN_EN, en));
                             ...
                             int *pkt_prio, uint32 *flags)
{
    ...
    *flags = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_ING_PVLAN_EN, &en));
    if (en) {
        *flags |= BCM_PORT_FORCE_VLAN_ENABLE;
                                  ...
                                  int action_profile_index)
{
    ...
    types[0] = BCMI_PT_DEFAULT_VFI;
    data[0] = vfi;

    /* default outer priority */
    types[1] = BCMI_PT_DEFAULT_PRI;
    data[1] = (action->priority << 1) | (action->new_outer_cfi & 0x1);

    /* default inner priority */
    types[2] = BCMI_PT_DEFAULT_INNER_PRI;
    data[2] = (action->new_inner_pkt_prio << 1) | (action->new_inner_cfi & 0x1);
                                  ...
                                  int *action_profile_index)
{
    ...
    type_num = COUNTOF(types);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_get(unit, port, types, data, type_num));

    /* default vfi */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_virtual_vpnid_get(unit, data[0], &vlan));
    action->forwarding_domain = vlan;
    /* default outer priority */
    action->priority = (data[1] >> 1) & 0x7;
    action->new_outer_cfi = data[1] & 0x1;
    /* default inner priority */
    action->new_inner_pkt_prio = (data[2] >> 1) & 0x7;
    action->new_inner_cfi = data[2] & 0x1;
    /* vlan tag preserve */
    action->outer_tag = (data[3] & 0x1) ? bcmVlanActionNone : bcmVlanActionDelete;
    action->inner_tag = (data[3] & 0x2) ? bcmVlanActionNone : bcmVlanActionDelete;
...
{
    return bcmint_ltsw_port_tab_bit_set
            (unit, port, BCMI_PT_EGR_L2_PROCESS_CTRL, 0, enable ? 1 : 0);
}
...
{
    return bcmint_ltsw_port_tab_bit_get
            (unit, port, BCMI_PT_EGR_L2_PROCESS_CTRL, 0, enable);
}
...
xfs_ltsw_port_hg3_enable_set(int unit, bcm_port_t port, int enable)
{
    ...
    port_type = (enable ? PORT_TYPE_SYSTEM : PORT_TYPE_ETHERNET);

    types[0] = BCMI_PT_ING_PORT_TYPE;
    data[0] = (enable ? PORT_TYPE_SYSTEM : PORT_TYPE_ETHERNET);
    types[1] = BCMI_PT_EGR_ING_PORT_TYPE;
    data[1] = (enable ? PORT_TYPE_SYSTEM : PORT_TYPE_ETHERNET);
    types[2] = BCMI_PT_PORT_PARSER_TYPE;
    data[2] = (enable ? PARSER_TYPE_SYSTEM : PARSER_TYPE_ETHERNET);
    types[3] = BCMI_PT_SHAPER_IFG_ADJ;
    data[3] = (enable ? 12 : 0);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xfs_port_egr_port_ctrl_set(unit, port, port_type, -1, -1));
...
xfs_ltsw_port_hg3_enable_get(int unit, bcm_port_t port, int *enable)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_ING_PORT_TYPE, &type));
...
xfs_ltsw_port_hg3_eth_type_set(int unit, int type)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, 0, BCMI_PT_ING_HG3_ETYPE, type));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, 0, BCMI_PT_EGR_HG3_ETYPE, type));
...
xfs_ltsw_port_hg3_eth_type_get(int unit, int *type)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, 0, BCMI_PT_ING_HG3_ETYPE, type));
...
xfs_ltsw_port_ifp_egr_vlan_check_enable_set(int unit, uint32_t enable)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set
            (unit, BLOCK_PROFILE_IFP, BCMI_PT_ING_EGR_VLAN_MEMBERSHIP_CHECK,
             !enable));
...
xfs_ltsw_port_dump_sw(int unit)
{
    ...
    PORT_ALL_ITER(unit, port) {
        ...
        LOG_CLI((BSL_META_U(unit, "  %-5s %3d   %4d     %4d    %5d     %4d/%-4d\n"),
                 ...
                 pInfo->sys_port, l2_if, pInfo->pvlan.vfi,
