    ...
    bcm_time_tod_t *tod)
{
    ...
    bool hw_update_supported = false, op_state_supported = false;
    ...
    if (SHR_SUCCESS(bcmi_lt_field_validate(unit, tbl_name, HW_UPDATEs))) {
        hw_update_supported = true;
    }

    if (SHR_SUCCESS(bcmi_lt_field_validate(unit, tbl_name, OPERATIONAL_STATEs))) {
        op_state_supported = true;
    }
    ...
    if (hw_update_supported) {
        ...
        SHR_IF_ERR_EXIT
            (bcmi_lt_entry_field_get(unit, &lt_entry, HW_UPDATEs,
                                     &auto_update));
        if (auto_update && op_state_supported) {
            SHR_IF_ERR_EXIT
                (bcmi_lt_entry_field_symbol_get(unit, &lt_entry,
                                                OPERATIONAL_STATEs, &st));
            if (sal_memcmp(st, SUCCESSs, sal_strlen(SUCCESSs))) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                                        "ToD auto-update is not "
                                        "working properly.\n")));
    ...
    int *val)
{
    ...
    const char *tbl_name = DEVICE_TS_CONTROLs;

    SHR_FUNC_ENTER(unit);

    /* Check if it's supported. */
    if (SHR_FAILURE(bcmi_lt_field_validate(unit, tbl_name, fld_name))) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
    ...
    if (!sal_strcmp(fld_name, TIMESTAMPING_MODEs)) {
        ...
        *val = COMPILER_64_LO(field.u.val);
    }
    ...
    int val)
{
    ...
    if (!sal_strcmp(fld_name, TIMESTAMPING_MODEs)) {
        ...
        COMPILER_64_SET(field.u.val, 0, val);
    }
...
/*!
 * \brief Clear all entries in timesync config profile.
 *
 * \param [in]  unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
 ...
 */
static int
ltsw_xfs_timesync_config_profile_clear(int unit)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_clear(unit, DEVICE_TS_PTP_PROFILEs));
...
/*!
 * \brief Get entry in timesync config profile.
 *
 * \param [in]  unit Unit number.
 * \param [in]  profile_idx Index to the timesync config profile.
 *
 * \param [out] profile Timesync config profile.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
ltsw_xfs_timesync_config_profile_entry_get(
    int unit,
    int profile_idx,
    bcm_time_timesync_config_profile_t *profile)
{
    ...
    bcmi_lt_field_t field[] =
    {
        /* 0*/ {DEVICE_TS_PTP_PROFILE_IDs,   BCMI_LT_FIELD_F_SET, 0, {0}},
        /* 1*/ {TIMESTAMP_VALIDs,            BCMI_LT_FIELD_F_GET, 0, {0}},
        /* 2*/ {DO_NOT_MODIFYs,              BCMI_LT_FIELD_F_GET, 0, {0}},
        /* 3*/ {ING_UPDATE_DONEs,            BCMI_LT_FIELD_F_GET, 0, {0}},
    };

    SHR_FUNC_ENTER(unit);

    lt_entry.fields = field;
    lt_entry.nfields = COUNTOF(field);
    lt_entry.attr = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_get(unit,
                           DEVICE_TS_PTP_PROFILEs,
                           &lt_entry,
                           NULL,
                           NULL));
    profile->flags = 0;
    if (field[1].u.val == 0) {
        profile->flags |= BCM_TIME_TIMESYNC_ONE_STEP_TIMESTAMP_DISABLE;
    }
    if (field[2].u.val == 1) {
        profile->flags |= BCM_TIME_TIMESYNC_PTP_DO_NOT_MODIFY;
    }
    if (field[3].u.val == 1) {
        profile->flags |= BCM_TIME_TIMESYNC_PTP_ING_UPDATE_DONE;
    }
...
/*!
 * \brief Create entry in DEVICE_TS_PTP_PROFILE table.
 *
 * \param [in] unit Unit Number.
 * \param [in/out] profile Timesync config profile.
 *
 * \param [OUT] profile_idx Profile index created.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
ltsw_xfs_timesync_config_profile_create(
    int unit,
    bcm_time_timesync_config_profile_t *profile,
    int *profile_idx)
{
    ...
    {
        /* 0*/ {DEVICE_TS_PTP_PROFILE_IDs,   BCMI_LT_FIELD_F_SET, 0, {0}},
        /* 1*/ {TIMESTAMP_VALIDs,            BCMI_LT_FIELD_F_SET, 0, {1}},
        /* 2*/ {DO_NOT_MODIFYs,              BCMI_LT_FIELD_F_SET, 0, {0}},
        /* 3*/ {ING_UPDATE_DONEs,            BCMI_LT_FIELD_F_SET, 0, {0}},
    };

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(profile, SHR_E_PARAM);
    SHR_NULL_CHECK(profile_idx, SHR_E_PARAM);

    profile_hdl = BCMI_LTSW_PROFILE_DEVICE_TS_PTP;
    ...
    *profile_idx = index;
    ...
    if (profile->flags & BCM_TIME_TIMESYNC_ONE_STEP_TIMESTAMP_DISABLE) {
        field[1].u.val = 0;
    }
    if (profile->flags & BCM_TIME_TIMESYNC_PTP_DO_NOT_MODIFY) {
        field[2].u.val = 1;
    }
    if (profile->flags & BCM_TIME_TIMESYNC_PTP_ING_UPDATE_DONE) {
        field[3].u.val = 1;
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_set(unit, DEVICE_TS_PTP_PROFILEs, &lt_entry, NULL));
...
/*!
 * \brief Delete entry in timesync config profile.
 *
 * \param [in] unit Unit Number.
...
static int
ltsw_xfs_timesync_config_profile_destroy(
    int unit,
    int profile_index)
{
    int rv;
    ...
    bcmi_lt_field_t field[] =
    {
        /* 0*/ {DEVICE_TS_PTP_PROFILE_IDs,   0, 0, {0}},
    };

    SHR_FUNC_ENTER(unit);

    rv = bcmi_ltsw_profile_index_free(unit,
                                      BCMI_LTSW_PROFILE_DEVICE_TS_PTP,
                                      (int)profile_index);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_delete(unit, DEVICE_TS_PTP_PROFILEs, &lt_entry, NULL));
...
/*!
 * \brief Calculate hash signature for Timesync profile LT.
 *
 * \param [in]  unit Unit Number.
 * \param [in]  entries Timesync config profile entry.
 ...
 */
static int
ltsw_xfs_timesync_config_profile_hash_cb(
    int unit, void *entries,
    int entries_per_set, uint32_t *hash)
{
    int size = entries_per_set * sizeof(bcm_time_timesync_config_profile_t);
    *hash = shr_crc32(0, entries, size);
    return SHR_E_NONE;
}

/*!
 * \brief Compare profile set of timesync config profile LT.
 *
 * \param [in]  unit Unit Number.
 * \param [in]  entries Timesync config profile entry.
 * \param [in]  entries_per_set Number of entries in the profile set.
 ...
 */
static int
ltsw_xfs_timesync_config_profile_cmp_cb(
    int unit, void *entries,
    int entries_per_set, int index, int *cmp)
{
    ...
    sal_memset(&profile, 0, sizeof(bcm_time_timesync_config_profile_t));

    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_xfs_timesync_config_profile_entry_get(unit, index, &profile));

    *cmp = sal_memcmp(entries, (void *)&profile,
                      sizeof(bcm_time_timesync_config_profile_t));
...
/*!
 * \brief Recover timesync config profile information at the given index.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
ltsw_xfs_timesync_config_profile_entry_recover(int unit, int profile_idx)
{
    bcmi_ltsw_profile_hdl_t profile_hdl = BCMI_LTSW_PROFILE_DEVICE_TS_PTP;
    bcm_time_timesync_config_profile_t profile;
    ...
    if (ref_count == 0) {
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_xfs_timesync_config_profile_entry_get(unit,
                                                        profile_idx,
                                                        &profile));
...
/*!
 * \brief Recover timesync config profile.
 *
 * \param [in]  unit Unit Number.
 *
 * \retval SHR_E_NONE No errors.
 ...
 */
static int
ltsw_xfs_timesync_config_profile_recover(int unit)
{
    ...
    while ((rv = bcmi_lt_entry_commit(unit, entry_hdl,
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(entry_hdl,
                                   DEVICE_TS_PTP_PROFILE_IDs,
                                   &data));

        profile_idx = data;

        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_xfs_timesync_config_profile_entry_recover
                (unit, profile_idx));
    }
...
xfs_ltsw_time_init(int unit)
{
    ...
    /* Time egress timesync config profile table */
    profile_hdl = BCMI_LTSW_PROFILE_DEVICE_TS_PTP;
    ...
    if (warm) {
        SHR_IF_ERR_EXIT
            (ltsw_xfs_timesync_config_profile_recover(unit));
    } else {
        int idx;
        bcmi_lt_entry_t lt_entry;
        bcmi_lt_field_t field[] =
        ...
        SHR_IF_ERR_EXIT
            (ltsw_xfs_timesync_config_profile_clear(unit));

        /* Default enable TIMESTAMP_VALIDs. */
        lt_entry.fields = field;
        lt_entry.nfields = COUNTOF(field);
        lt_entry.attr = 0;
        for (idx = idx_min; idx <= idx_max ; idx++) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_lt_entry_set
                    (unit, DEVICE_TS_PTP_PROFILEs, &lt_entry, NULL));
        }
...
xfs_ltsw_time_deinit(int unit)
{
    ...
    rv = bcmi_ltsw_profile_unregister(unit, BCMI_LTSW_PROFILE_DEVICE_TS_PTP);
    ...
    int arg)
{
    ...
    switch (type) {
        ...
        case bcmSwitchTimesyncIngressVersion:
            ...
            break;
        case bcmSwitchEgressRawResidenceTimeDelta:
            SHR_IF_ERR_EXIT
                (ltsw_xfs_device_ts_control_field_set
                    (unit, EGR_OBJ_LATENCY_ADJUSTs, arg));
            break;
        case bcmSwitchEgressRawResidenceTimeRightShift:
            SHR_IF_ERR_EXIT
                (ltsw_xfs_device_ts_control_field_set
                    (unit, EGR_OBJ_LATENCY_SHIFTs, arg));
            break;
...
int
xfs_ltsw_time_timesync_config_profile_create(
    int unit,
    bcm_time_timesync_config_profile_t *profile,
    int *profile_id)
{
    return ltsw_xfs_timesync_config_profile_create(unit, profile, profile_id);
}

int
xfs_ltsw_time_timesync_config_profile_destroy(
    int unit,
    int profile_id)
{
    return ltsw_xfs_timesync_config_profile_destroy(unit, profile_id);
}

int
xfs_ltsw_time_timesync_config_profile_get(
    int unit,
    int profile_id,
    bcm_time_timesync_config_profile_t *profile)
{
    return ltsw_xfs_timesync_config_profile_entry_get(unit,
                                                      profile_id, profile);
}
