    ...
    bcmlt_entry_handle_t ent_hdl)
{
    ...
    const bcmint_field_destination_lt_t *variant_lt = NULL;
    const char *fld_name;
    bcmint_field_destination_lt_id_t lt_id;
    int fid;
    int rv = 0;
    int drop_code = 0, drop_code_mask = 0;

    SHR_FUNC_ENTER(unit);

    lt_id = BCMINT_LTSW_FIELD_DEST_LT_DESTINATION_FP_TABLE;
    SHR_IF_ERR_EXIT_EXCEPT_IF(
        bcmint_field_destination_lt_get(unit, lt_id, &variant_lt),
        SHR_E_UNAVAIL);

    if (match->gport_mask != 0) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_to_l2_if(unit, match->gport, &l2_if));
    ...
    if (variant_lt != NULL) {
        rv = bcmint_field_destination_drop_reason_to_code(unit,
                                                          match->drop_reason,
                                                          &drop_code);
        if (SHR_SUCCESS(rv)) {
            drop_code_mask = match->drop_reason_mask;
        ...
        fid = BCMINT_LTSW_FIELD_DEST_FLD_DFP_TBL_DROP_CODE;
        if (variant_lt->fld_bmp & (1 << fid)) {
            fld_name = variant_lt->flds[fid].name;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(ent_hdl, fld_name, drop_code));
        }

        fid = BCMINT_LTSW_FIELD_DEST_FLD_DFP_TBL_DROP_CODE_MASK;
        if (variant_lt->fld_bmp & (1 << fid)) {
            fld_name = variant_lt->flds[fid].name;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(ent_hdl, fld_name, drop_code_mask));
        }
    }
    ...
    bcmlt_entry_handle_t ent_hdl)
{
    ...
    if (action->flags & BCM_FIELD_DESTINATION_ACTION_REDIRECT) {
        ...
        if ((dst_type == bcmFieldRedirectDestinationPort) ||
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_port_to_l2_if(unit, action->gport, &l2_if));
    ...
    bcm_field_destination_match_t *match)
{
    ...
    if (value != 0) {
        ...
        if (value == DEST_TYPE_L2_OIF) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_l2_if_to_port(unit, (int)dest, &match->gport));
    ...
    } else if (value == DEVICE_PORT_TYPE_HIGIG3) {
    ...
    if (variant_lt != NULL) {
        ...
        if (variant_lt->fld_bmp & (1 << fid)) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_get(ent_hdl, fld_name, &value));
            drop_code_mask= (int)value;
        }

        rv = bcmint_field_destination_drop_code_to_reason(unit,
                                                          drop_code,
                                                          &match->drop_reason);
        if (SHR_SUCCESS(rv)) {
            match->drop_reason_mask = drop_code_mask;
        } else if (rv == SHR_E_UNAVAIL) {
            rv = SHR_E_NONE;
        }
        SHR_IF_ERR_VERBOSE_EXIT(rv);
    ...
    bcm_field_destination_action_t *action)
{
    ...
    if ((i == REDIRECT_ONLY) || (i == CNG_REDIRECT)) {
        ...
        if (value == DEST_TYPE_L2_OIF) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_l2_if_to_port(unit, (int)dest, &action->gport));
    ...
    bcm_gport_t match_port)
{
    ...
    if (match_port == BCM_GPORT_BLACK_HOLE) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_to_l2_if(unit, match_port, &l2_if));
        key_dest_msk = 0xffff;
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    if (variant_lt != NULL) {
        ...
        if (variant_lt->fld_bmp & (1 << fid)) {
            fld_name = variant_lt->flds[fid].name;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(entry_hdl, fld_name, 0));
        }
        ...
        rv = bcmlt_field_desc_get(dunit, DESTINATION_FP_TABLEs, "DROP_CODE", &desc);
        if (SHR_SUCCESS(rv)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(entry_hdl, "DROP_CODE", 0));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(entry_hdl, "DROP_CODE_MASK",0));
        }
    }
