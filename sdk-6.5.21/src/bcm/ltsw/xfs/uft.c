...
/*!
 * \brief Get the active state of the tile for a bank.
 *
 * \param [in] unit Unit number.
 * \param [in] bank_str Bank string.
 * \param [out] active Active state of the tile.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_xfs_uft_tile_active_status_get(int unit, const char *bank_str, bool *active)
{
    int dunit = bcmi_ltsw_dev_dunit(unit), rv;
    ...
    *active = TRUE;
    ...
    SHR_IF_ERR_EXIT
        (bcmlt_entry_allocate(dunit, DEVICE_EM_TILE_INFOs, &entry_hdl));
    ...
    rv = bcmlt_entry_field_get(entry_hdl, ACTIVEs, &field_val);
    if (rv == SHR_E_NONE) {
        if (field_val == 0) {
            *active = FALSE;
        }
...
ltsw_xfs_uft_tile_mode_init(int unit)
{
    ...
    /* Tile mode initialization for tiles with fixed mode. */
    mode_db = uft_db.fixed_tile_mode_db;
    if (mode_db != NULL ) {
        SHR_IF_ERR_EXIT
    ...
    for (tbl_idx = 0; tbl_idx < mode_db->table_cnt; tbl_idx++) {
        ...
        for (ent_idx = 0; ent_idx < table_db->entry_cnt; ent_idx++) {
            ...
            if (sal_strcmp(entry_db->field_db[0].symbol, tile_id) == 0) {
                ...
                tile_info->entry_attr2 = entry_db->entry_attr2;
                tile_info->bank_number = entry_db->bank_number;
                if (tile_info->entry_attr == NULL) {
                    SHR_ERR_EXIT(SHR_E_INTERNAL);
                }
                tile_info->hw_learn = entry_db->hw_learn;

                /* MTOP ALPM Level 1 atributes. */
                if (ltsw_feature(unit, LTSW_FT_MTOP)) {
                    tile_info->mtop_attr.mtop_entry_attr_1 =
                        entry_db->mtop_attr.mtop_entry_attr_1;
                    tile_info->mtop_attr.bank_number_1 =
                        entry_db->mtop_attr.bank_number_1;
                    tile_info->mtop_attr.mtop_entry_attr_2 =
                        entry_db->mtop_attr.mtop_entry_attr_2;
                    tile_info->mtop_attr.bank_number_2 =
                        entry_db->mtop_attr.bank_number_2;
                    tile_info->mtop_attr.mtop_entry_attr_3 =
                        entry_db->mtop_attr.mtop_entry_attr_3;
                    tile_info->mtop_attr.bank_number_3 =
                        entry_db->mtop_attr.bank_number_3;
                    tile_info->mtop_attr.mtop_entry_attr_4 =
                        entry_db->mtop_attr.mtop_entry_attr_4;
                    tile_info->mtop_attr.bank_number_4 =
                        entry_db->mtop_attr.bank_number_4;
                }
                SHR_EXIT();
            }
        }
...
ltsw_xfs_uft_var_bank_info_init(int unit)
{
    ...
    uft_bank_info_t *bank_info = NULL;
    uft_group_info_t *group_info = NULL;
    int group_id = 0, bank_id = 0, mtop_l1_bank0_id = 0, l1_id;
    int i = 0, j = 0;
    const char *tile_id = NULL;
    const char *tile_mode = NULL;
    ...
    bcmint_uft_entry_info_t tile_info = {
        ...
        .bank_number = 0,
        .hw_learn = FALSE,
        .mtop_attr.mtop_entry_attr_1 = NULL,
        .mtop_attr.bank_number_1 = 0,
        .mtop_attr.mtop_entry_attr_2 = NULL,
        .mtop_attr.bank_number_2 = 0,
        .mtop_attr.mtop_entry_attr_3 = NULL,
        .mtop_attr.bank_number_3 = 0,
        .mtop_attr.mtop_entry_attr_4 = NULL,
        .mtop_attr.bank_number_4 = 0,
    };
    bcmint_uft_db_t uft_db = {
        .tile_bank_cnt = 0,
        .tile_bank_map = 0,
    ...
    for (i = 0; i < dev_info->max_bank; i++) {
        ...
        for (j = 0; j < bank_info->group_cnt; j++) {
            ...
            /*
             * MTOP_ALPM_LEVEL1 TILE has total 16 banks.
             * Currently 4 groups (L3UC_DST/SRC & FP_DST/SRC)
             * Can see all the 16 banks.
             */
            if (sal_strcmp(tile_mode, UFT_MTOP_ALPM_LEVEL1_MODE0s) == 0) {

                if (!ltsw_feature(unit, LTSW_FT_MTOP)) {
                    continue;
                }
                /*
                 * As UFT_MTOP_ALPM_LEVEL1_BANK sequence is not continuous through
                 * DEVICE_EM_BANK_INFO traverse. Which will result in bank allocated
                 * not continuous for group, result in ALPM init fail. Thus use bank_id to
                 * allocate continuous bank for group.
                 */
                if (sal_strcmp(bank_info->bank_str, UFT_MTOP_ALPM_LEVEL1_BANK0s) == 0) {
                    mtop_l1_bank0_id = bank_id;
                }
                l1_id = bank_id - mtop_l1_bank0_id;
                if (tile_info.mtop_attr.mtop_entry_attr_1 != NULL &&
                    (sal_strcmp(group_info->attr,
                       tile_info.mtop_attr.mtop_entry_attr_1) == 0)) {
                    if (l1_id < 0 || l1_id >=
                        tile_info.mtop_attr.bank_number_1) {
                        continue;
                    }
                } else if (tile_info.mtop_attr.mtop_entry_attr_2 != NULL &&
                           (sal_strcmp(group_info->attr,
                            tile_info.mtop_attr.mtop_entry_attr_2) == 0)) {
                    if (l1_id < tile_info.mtop_attr.bank_number_1 ||
                        l1_id >= (tile_info.mtop_attr.bank_number_1 +
                                  tile_info.mtop_attr.bank_number_2)) {
                        continue;
                    }
                } else if (tile_info.mtop_attr.mtop_entry_attr_3 != NULL &&
                           (sal_strcmp(group_info->attr,
                            tile_info.mtop_attr.mtop_entry_attr_3) == 0)) {
                    if (l1_id < (tile_info.mtop_attr.bank_number_1 +
                                 tile_info.mtop_attr.bank_number_2) ||
                        l1_id >= (tile_info.mtop_attr.bank_number_1 +
                                  tile_info.mtop_attr.bank_number_2 +
                ...
                } else if (tile_info.mtop_attr.mtop_entry_attr_4 != NULL &&
                           (sal_strcmp(group_info->attr,
                            tile_info.mtop_attr.mtop_entry_attr_4) == 0)) {
                    if (l1_id < (tile_info.mtop_attr.bank_number_1 +
                                 tile_info.mtop_attr.bank_number_2 +
                                 tile_info.mtop_attr.bank_number_3) ||
                        l1_id >= (tile_info.mtop_attr.bank_number_1 +
                 ...
                 */
                if ((sal_strcmp(tile_info.entry_attr,
                                ALPM_LEVEL1_LEVEL2_ONLYs) == 0) ||
                    (sal_strcmp(tile_info.entry_attr,
                                ALPM_COMPRESSION_LEVEL1_LEVEL2_ONLYs) == 0)) {
                    if (sal_strcmp(group_info->attr,
                                   ALPM_LEVEL_3s) == 0) {
                        continue;
                    }
                } else if ((sal_strcmp(NONEs, tile_info.entry_attr) != 0) &&
                    (sal_strcmp(group_info->attr,
                                tile_info.entry_attr)) != 0) {
                    if (tile_info.entry_attr2 != NULL) {
                        if (sal_strcmp(group_info->attr,
                                       tile_info.entry_attr2) != 0) {
                            continue;
                        }
                    } else {
                        continue;
                ...
                if (sal_strcmp(group_info->attr, DYNAMIC_FT_FPs) == 0) {
                    if (hw_learn_enable == 0) {
                         ...
                         */
                        if (sal_strcmp(DT_EM_FT_ENTRYs,
                                       group_info->lkup0_lt[0]) == 0) {
                            continue;
                        }
                    } else {
                        /*
                ...
                if ((sal_strcmp(group_info->lkup0_lt[0],
                                L3_IPV4_UNICAST_DEFIP_TABLEs) == 0) ||
                    (sal_strcmp(group_info->lkup0_lt[0],
                                L3_IPV6_UNICAST_DEFIP_128_TABLEs) == 0) ||
                    (sal_strcmp(group_info->lkup1_lt[0],
                                L3_IPV4_UNICAST_DEFIP_TABLEs) == 0) ||
                    (sal_strcmp(group_info->lkup1_lt[0],
                                L3_IPV6_UNICAST_DEFIP_128_TABLEs) == 0)) {
                    continue;
                }
                /* For 1 tile assgin for 2 PDD type. */
                if (tile_info.bank_number != 0) {
                    if (group_info->var_cnt == tile_info.bank_number) {
                        continue;
                    }
                }
                group_info->var_bank[group_info->var_cnt] = bank_id;
...
/*!
 * \brief Init DEVICE_EM_BANK_PAIR LT, update PAIRED_DEVICE_EM_BANK_ID field.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
...
ltsw_xfs_uft_pair_bank_init(int unit)
{
    ...
    for (i = 0; i < dev_info->max_bank; i++) {
        ...
        SHR_IF_ERR_EXIT
            (ltsw_xfs_uft_tile_active_status_get(unit, bank_info->bank_str, &active));
        if (!active) {
            /* Skip inactive tile. */
            continue;
        }
...
ltsw_xfs_uft_em_group_init(int unit)
{
    ...
    for (i = 0; i < dev_info->max_group; i++) {
        ...
        /* Skip bank assignment if the tile is inactive. */
        if (group_info->var_cnt > 0) {
            SHR_IF_ERR_EXIT
                (ltsw_xfs_uft_bank_info_get(unit, group_info->var_bank[0],
                                            &bank_info));
            SHR_IF_ERR_EXIT
...
ltsw_xfs_uft_table_max_entries_init(int unit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_FLEXCTR_L2_IPMC)) {
        l2mc_wide_view = bcmi_ltsw_property_get(unit,
                                                BCMI_CPN_L2_IPMC_FLEXCTR_ENABLE,
                                                0);
        if (l2mc_wide_view) {
            SHR_IF_ERR_EXIT
                (ltsw_xfs_uft_table_max_entries_set(unit, L2_IPV4_MULTICAST_TABLEs,
                                                    0));
        } else {
            SHR_IF_ERR_EXIT
                (ltsw_xfs_uft_table_max_entries_set(unit, L2_IPV4_MULTICAST_WIDE_TABLEs,
                                                    0));
        }
    }
    ...
    for (i = 0; i < uft_db.property_cnt; i++) {
        ...
        num_entries = bcmi_ltsw_property_get(unit, property, -1);
        if (num_entries < 0) {
            continue;
        }

        if (sal_strcmp(property, BCMI_CPN_IP4_L2MC_TABLE_ENTRIES_MAX) == 0) {
            if (l2mc_wide_view) {
                table_id = L2_IPV4_MULTICAST_WIDE_TABLEs;
            }
        }
...
/*!
 * \brief Remove default table entries on a tile that the specific bank belongs to.
 *
 * \param [in] unit Unit number.
    ...
    bcmlt_entry_handle_t *entry_hdl_array)
{
    ...
    int *bank_id_in_tile = NULL;
    int alloc_size;
    int bank_cnt_in_tile = 0;
    int j = 0;
    bcmint_uft_db_t uft_db = {
        .bank_id_cnt = 0,
        .bank_id_map = NULL,
    ...
    alloc_size = sizeof(int) * uft_db.tile_bank_cnt;
    SHR_ALLOC(bank_id_in_tile, alloc_size, "bcmLtswHashTileBank");
    SHR_NULL_CHECK(bank_id_in_tile, SHR_E_MEMORY);
    sal_memset(bank_id_in_tile, 0, alloc_size);

    /* Get the Tile name*/
    tile_id = ltsw_xfs_uft_id2name(uft_db.tile_bank_map, uft_db.tile_bank_cnt,
                                   bank_num);

    /* Get all banks belong to the tile */
    for (i = 0; i < uft_db.tile_bank_cnt; i++) {
        if (sal_strcasecmp(uft_db.tile_bank_map[i].name, tile_id) == 0) {
            bank_id_in_tile[bank_cnt_in_tile] = uft_db.tile_bank_map[i].val;
            bank_cnt_in_tile++;
        }
    }
    ...
    for (i = 0; i < hdl_cnt; i++) {
        ...
        /* Find default LT entries on all banks that belong to the tile. If found, delete it. */
        for (j = 0; j < bank_cnt_in_tile; j++) {
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_xfs_uft_bank_table_is_mapped(unit, bank_id_in_tile[j],
                                                   table_id, &is_mapped));
            if (is_mapped) {
                break;
            }
