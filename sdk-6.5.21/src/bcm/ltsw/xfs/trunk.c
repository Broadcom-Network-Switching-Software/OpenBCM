...
/* L2IF information database. */
static bcmint_trunk_xfs_l2if_info_t *l2if_info[BCM_MAX_NUM_UNITS];

/*! L2IF information database. */
#define L2IF_INFO_DB(unit)  (l2if_info[unit])

/*! L2IF information. */
#define L2IF_INFO(unit, _l2if_)  (&(L2IF_INFO_DB(unit)[_l2if_]))

/*! Trunk ID of L2_EIF. */
#define L2EIF_2_TID(unit, _l2eif_)  (L2IF_INFO(unit, _l2eif_)->tid)
    ...
    bcm_gport_t *gport)
{
    ...
    if (BCM_GPORT_IS_FLOW_PORT(cap_gport)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_l2_if_to_port(unit, l2_iif, &cap_gport));
    ...
    int *psc)
{
    ...
    } else if (sal_strcmp(local_lb_mode, RESILIENTs) == 0) {
...
/*!
 * \brief Dynamic size decode to HW resilient hash size.
 *
 * \param [in] unit Unit Number.
 * \param [in] size Dynamic size.
 * \param [out] rh_size Resilient hash size string.
 *
 * \retval SHE_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
ltsw_xfs_trunk_rh_size_decode(
    int unit,
    int size,
    const char **rh_size)
{
    const char *local_rh_size;

    SHR_FUNC_ENTER(unit);

    switch (size) {
        case 64:
            local_rh_size = RH_SIZE_64s;
            break;
        case 128:
            local_rh_size = RH_SIZE_128s;
            break;
        default:
            SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
 ...
 */
static int
ltsw_xfs_trunk_fabric_rh_size_decode(
    int unit,
    int size,
    const char **rh_size)
{
...
/*!
 * \brief Set DLB Id by L2 font panel trunk Id.
 *
 * \param [in] unit Device unit number.
 * \param [in] t_id The front panel trunk ID.
 * \param [in] t_priv Trunk private information.
 ...
 */
static int
ltsw_xfs_trunk_dlb_set_dlb_id_by_tid(
    int unit,
    bcmint_trunk_id_t t_id,
    bcmint_trunk_priv_info_t *t_priv,
    int l2_eif,
    int dlb_id)
{
    ...
    /*
     * When the trunk is a redirect destination, the trunk DLB ID should be set
     * to all the LT entries for the TRUNK groups redirected to the trunk.
     */
    while ((rv = bcmi_lt_entry_commit(unit, lte_hdl,
                                      BCMLT_OPCODE_TRAVERSE,
                                      BCMLT_PRIORITY_NORMAL)) == SHR_E_NONE) {
        ...
        if (t_id != (bcmint_trunk_id_t)value) {
            continue;
        }
    ...
    int dlb_id)
{
    ...
    switch (t_type) {
        case bcmintTrunkTypeFrontPanel:
            ...
            if (!t_info->redirected) {
            /*
             * When the trunk is rediected, the trunk DLB ID is not allowed to
             * set to the LT entry as the entry is taken over by the redirect
             * destination TRUNK.
             */
                SHR_IF_ERR_VERBOSE_EXIT
                    (ltsw_xfs_trunk_dlb_set_dlb_id_by_l2_eif(unit,
                                                             l2_eif, dlb_id));
            }

            if (t_info->redirect_cnt) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (ltsw_xfs_trunk_dlb_set_dlb_id_by_tid(unit, t_id, t_priv,
                                                          l2_eif, dlb_id));
            }
            break;
...
ltsw_xfs_trunk_init(int unit)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_capacity_get(unit, L2_EIF_SYSTEM_DESTINATIONs, &entry_num));

    ha_array_size = entry_num;
    ha_instance_size = sizeof(bcmint_trunk_xfs_l2if_info_t);
    ha_req_size = ha_array_size * ha_instance_size;
    ha_alloc_size = ha_req_size;
    ha_ptr = bcmi_ltsw_ha_mem_alloc(unit,
                                    BCMI_HA_COMP_ID_TRUNK,
                                    BCMINT_TRUNK_L2_EIF_2_TID_SUB_COMP_ID,
                                    "bcmTrunkL2eifToTid",
                                    &ha_alloc_size);
    ...
    if (warm) {
        rv = bcmi_ltsw_issu_struct_info_get(unit,
                                            BCMINT_TRUNK_XFS_L2IF_INFO_T_ID,
                                            BCMI_HA_COMP_ID_TRUNK,
                                            BCMINT_TRUNK_L2_EIF_2_TID_SUB_COMP_ID,
                                            &offset);
        if (rv == SHR_E_NOT_FOUND) {
            ...
            ha_ptr = (void *)((uint8_t *)ha_ptr + offset);
        }
    } else {
        rv = bcmi_ltsw_issu_struct_info_report(unit,
                                               BCMI_HA_COMP_ID_TRUNK,
    ...
    bcmint_trunk_priv_info_t *t_priv)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_alloc(unit, &l2_eif));

    BCM_GPORT_TRUNK_SET(gport,t_id);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_reserve(unit, gport));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_map_port(unit, l2_eif, gport));
    ...
    bcm_trunk_member_t *member)
{
    ...
    if (ltsw_xfs_trunk_dlb_is_configured(unit, trunk_info->psc)) {
        ...
        SHR_IF_ERR_EXIT
            (bcmi_ltsw_dlb_feature_get(unit, &dlb_feature));
        if (!(dlb_feature & BCMI_LTSW_DLB_F_FP_TRUNK)) {
    ...
    bcm_trunk_member_t *member)
{
    ...
    if (ltsw_xfs_trunk_dlb_is_configured(unit, trunk_info->psc)) {
        ...
        SHR_IF_ERR_EXIT
            (bcmi_ltsw_dlb_feature_get(unit, &dlb_feature));
        if (!(dlb_feature & BCMI_LTSW_DLB_F_FABRIC_TRUNK)) {
            SHR_IF_ERR_EXIT(SHR_E_UNAVAIL);
        }
    }
    ...
    int *member_count)
{
    ...
    if (member_max > 0) {
        ...
        if (SHR_SUCCESS(rv)) {
            ...
            if (member_max < weighted_size) {
                member_cnt = member_max;
            } else {
                member_cnt = weighted_size;
            }
...
exit:
    if (dlb_id_is_created && SHR_FUNC_ERR()) {
        (void)bcmi_ltsw_dlb_config_set(unit, dlb_type, dlb_id, NULL);
...
/*!
 * \brief Update the front panel trunk group resilient hash configuration.
 *
 * \param [in] unit Unit Number.
 * \param [in] t_id Type-based Trunk Identifier.
 * \param [in] trunk_info Trunk Information.
 ...
 */
static int
ltsw_xfs_trunk_update_fp_group_rh(
    int unit,
    bcmint_trunk_id_t t_id,
    bcm_trunk_info_t *trunk_info,
    int member_count,
    ...
    bcmint_trunk_update_info_t *t_update)
{
    bcmlt_entry_handle_t lte_hdl = BCMLT_INVALID_HDL;
    int i, dunit;
    const char *lb_mode = NULL, *rh_size = NULL;
    uint64_t value;
    int member_cnt;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_xfs_trunk_rh_size_decode(unit,trunk_info->dynamic_size,&rh_size));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_symbol_add(lte_hdl, RH_SIZEs, rh_size));
    ...
    for (i = 0; i < member_count; i++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_array_add(lte_hdl, RH_UC_EGR_BLOCKs,
                                         member_cnt, &value, 1));
        ...
        value = system_port;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_array_add(lte_hdl, RH_UC_MEMBER_PORT_SYSTEMs,
                                         member_cnt, &value, 1));
        if (ltsw_xfs_trunk_gport_is_channelized_access_port(unit,
                                                       member_array[i].gport)) {
            value = l2_eif;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_array_add(lte_hdl, RH_MEMBER_L2_EIFs,
                                             member_cnt, &value, 1));
        }
        member_cnt++;
    }
    ...
    value = member_cnt;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(lte_hdl, RH_UC_MEMBER_CNTs, value));
    ...
    bcmint_trunk_update_info_t *t_update)
{
    ...
    if (trunk_info->psc == BCM_TRUNK_PSC_DYNAMIC_RESILIENT) {
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_xfs_trunk_update_fp_group_rh(unit, t_id, trunk_info,
 ...
 *
 * \param [in] unit Unit Number.
 * \param [in] system_port System port Id.
 * \param [in] l2_iif L2 ingress interface.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
    ...
    int l2_iif)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(lte_hdl, L2_IIFs, &value));
    l2_iif_get = value & 0xffffffff;
    if (l2_iif_get != l2_iif) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(lte_hdl, L2_IIFs, l2_iif));
        SHR_IF_ERR_VERBOSE_EXIT
...
/*!
 * \brief Update the front panel trunk port egress attributes.
 *
 * \param [in] unit Unit Number.
 ...
 */
static int
ltsw_xfs_trunk_update_fp_port_egr_attr(
    int unit,
    int port,
    int l2_eif)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_port_tab_set(unit, port, BCMI_PT_EGR_MC_L2_OIF, l2_eif));
...
ltsw_xfs_trunk_update_fp_member(
    ...
    bcmint_trunk_update_info_t *t_update)
{
    int i, system_port, modid, port, modid_is_local;
    int l2_eif, l2_eif_port, l2_eif_trunk, gport;
    ...
    for (i = 0; i < member_count; i++) {
        if (member_array[i].flags & BCM_TRUNK_MEMBER_INGRESS_DISABLE ||
            member_array[i].flags & BCM_TRUNK_MEMBER_EGRESS_DISABLE) {
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_xfs_trunk_gport_resolve(unit, member_array[i].gport,
                                              &modid, &port, &system_port,
                                              &l2_eif_port));
        } else {
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_xfs_trunk_gport_resolve(unit, member_array[i].gport,
        ...
        t_info = t_priv->t_info;
        l2_eif_trunk = t_info->l2_dest;

        l2_eif = member_array[i].flags & BCM_TRUNK_MEMBER_INGRESS_DISABLE ?
                 l2_eif_port : l2_eif_trunk;
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_xfs_trunk_update_fp_port_attr(unit, system_port, l2_eif));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_modid_is_local(unit, modid, &modid_is_local));
        if (modid_is_local) {
            l2_eif = member_array[i].flags & BCM_TRUNK_MEMBER_EGRESS_DISABLE ?
                     l2_eif_port : l2_eif_trunk;
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_xfs_trunk_update_fp_port_egr_attr(unit, port, l2_eif));
        }

        BCM_GPORT_TRUNK_SET(gport,t_id);
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_lag_set(unit, member_array[i].gport, gport));
    }
    ...
    for (i = 0; i < t_update->num_leaving; i++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_lag_set
                (unit, t_update->leaving_members[i], BCM_TRUNK_INVALID));
    }
...
/*!
 * \brief Update the fabric trunk group resilient hash configuration.
 *
 * \param [in] unit Unit Number.
 * \param [in] t_id Type-based Trunk Identifier.
 * \param [in] trunk_info Trunk Information.
 ...
 */
static int
ltsw_xfs_trunk_update_fabric_group_rh(
    int unit,
    bcmint_trunk_id_t t_id,
    bcm_trunk_info_t *trunk_info,
    int member_count,
    ...
    bcmint_trunk_update_info_t *t_update)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_xfs_trunk_fabric_rh_size_decode(unit, trunk_info->dynamic_size,
                                              &rh_size));
    ...
    for (i = 0; i < member_count; i++) {
        ...
        value = port;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_array_add(lte_hdl, RH_MEMBER_PORT_IDs,
                                         member_cnt, &value, 1));
        member_cnt++;
    }
    ...
    value = member_cnt;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(lte_hdl, RH_MEMBER_CNTs, value));
    ...
    bcmint_trunk_update_info_t *t_update)
{
    ...
    if (trunk_info->psc == BCM_TRUNK_PSC_DYNAMIC_RESILIENT) {
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_xfs_trunk_update_fabric_group_rh(unit, t_id, trunk_info,
                                                   member_count, member_array,
                                                   t_priv, t_update));
        SHR_EXIT();
    }
    ...
    bcmint_trunk_priv_info_t *t_priv)
{
    ...
    for (i = 0; i < member_count; i++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_lag_set
                (unit, member_array[i].gport, BCM_TRUNK_INVALID));
    }
    ...
    bcmint_trunk_priv_info_t *t_priv)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_demap_port(unit, t_info->l2_dest));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_free(unit, t_info->l2_dest));
...
ltsw_xfs_trunk_clear_port_attribute(int unit)
{
    ...
    int dunit, rv;
    uint64_t value;
    int l2_eif, is_trunk, system_port, t_id, gport;
    ...
    while ((rv = bcmi_lt_entry_commit(unit, lte_hdl,
        ...
        if (is_trunk) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_l2_if_demap_port(unit, l2_eif));

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_l2_if_free(unit, l2_eif));
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_xfs_trunk_update_fp_port_attr(unit, system_port, l2_eif));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_l2_if_to_port(unit, l2_eif, &gport));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_port_lag_set(unit, gport, BCM_TRUNK_INVALID));
        }
    }
exit:
    if (lte_hdl != BCMLT_INVALID_HDL) {
...
/*!
 * \brief Clear L2_EIF_ID from trunk port to normal port.
 *
 * \param [in] unit Unit Number.
 *
 * \retval SHR_E_NONE No errors.
 ...
 */
static int
ltsw_xfs_trunk_clear_port_egr_attribute(int unit)
{
    bcmlt_entry_handle_t lte_hdl = BCMLT_INVALID_HDL;
    int dunit, rv;
    uint64_t value;
    int l2_eif, l2_eif_port, port;
    bcm_pbmp_t rdb_pbmp;
    ...
    while ((rv = bcmi_lt_entry_commit(unit, lte_hdl,
        ...
        if (BCM_PBMP_MEMBER(rdb_pbmp, port)) {
            continue;
        }

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_to_l2_if(unit, port, &l2_eif_port));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(lte_hdl, L2_EIF_IDs, &value));
        l2_eif = value & 0xffffffff;

        if (l2_eif != l2_eif_port) {
            value = l2_eif_port;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(lte_hdl, L2_EIF_IDs, value));
            SHR_IF_ERR_CONT
...
ltsw_xfs_trunk_clear_fp(int unit)
{
    ...
    /* Clear PORT table. */
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_xfs_trunk_clear_port_egr_attribute(unit));
    ...
    bcmint_trunk_info_t *dst_info)
{
    ...
    value = dst_info->dlb_id < 0 ? 0 : 1;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(lte_hdl, DLB_ID_VALIDs, value));

    value = dst_info->dlb_id < 0 ? 0 : dst_info->dlb_id;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(lte_hdl, DLB_IDs, value));
