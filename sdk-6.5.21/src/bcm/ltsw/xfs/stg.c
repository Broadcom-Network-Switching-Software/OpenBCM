 ...
 */
static int
stg_vfi_reset(int unit, int ingress,
              bcmi_ltsw_vlan_info_t *info, void *user_data)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_vlan_stg_set(unit, info->vid, ingress, stg));
...
{
    int uidx = 0;
    stg_map_recover_info_t *stg_info = NULL;

    stg_info = (stg_map_recover_info_t *)user_data;

    if (SHR_BITGET(stg_info->stg_bmp, info->stg)) {
        uidx = bcmint_stg_user_idx_get(unit, info->vid);
    ...
    int *uidx_next)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_vlan_traverse(unit, 1, stg_map_recover, &info));
    ...
    bcm_stg_t stg)
{
    ...
    if (!bcmi_warmboot_get(unit)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_vlan_traverse(unit, 1, stg_vfi_reset, &stg));

        /* Reset EGR_VFI with default STG. */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_vlan_traverse(unit, 0, stg_vfi_reset, &stg));
