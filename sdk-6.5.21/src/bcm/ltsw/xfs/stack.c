                           ...
                           bcm_port_t dest_port)
{
    ...
    SYSTEM_PORT_SET(unit, sys_port, modid, dest_port);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_alloc(unit, &l2_if));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_l2_if_add(unit, l2_if));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_reserve(unit, modport));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_map_port(unit, l2_if, modport));
                             ...
                             bcm_port_t dest_port)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_demap_port(unit, l2_if));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_l2_if_del(unit, l2_if));
    /* Free the allocasted l2_if */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_l2_if_free(unit, l2_if));
    /* Delete the system table that map with L2_IF. */
    SYSTEM_PORT_SET(unit, sys_port, modid, dest_port);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_sys_port_del(unit, sys_port, BCMI_LTSW_PORT_ING));
    /* Free gport id */
    BCM_GPORT_MODPORT_SET(gport, modid, dest_port);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_free(unit, gport));
...
{
    int sys_port, port, modid, l2_if, modport;
    ltsw_stack_info_t *si;
    SHR_FUNC_ENTER(unit);
    ...
    *sys_source_ifa_probe_copy = sys_port;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
       (bcmi_ltsw_port_tab_raw_set(unit, sys_port,
                                   BCMI_PT_ING_OTPID_ENABLE, 0xf));

    SHR_IF_ERR_VERBOSE_EXIT
       (bcmi_ltsw_port_tab_raw_set(unit, sys_port,
                                   BCMI_PT_ING_PAYLOAD_OTPID_ENABLE, 0xf));
    ...
    /* Preserve both inner & outer tag. */
    SHR_IF_ERR_VERBOSE_EXIT
       (bcmi_ltsw_port_tab_raw_set(unit, l2_if,
                                   BCMI_PT_VLAN_PRESERVE, 0x3));
...
{
    int sys_port, port, modid, l2_if, gport;
    ltsw_stack_info_t *si;
    SHR_FUNC_ENTER(unit);
