...
typedef struct l2_ctrl_s {
    ...
    /* PVLAN info. */
    bcmint_l2_pvlan_info_t *pvlan_info;
    ...
    /* Flex ctr extended info. */
    bcmint_l2_flexctr_ext_info_t *flexctr_ext_info;
    ...
    /* L2 ctrl info. */
    bcmint_l2_ctrl_info_t ctrl_info;
} l2_ctrl_t;
...
#define L2_HOST_MISC_CTRL_0_ECMP_MEMBER0_CTRL  (3)
...
/* Maximum times to get hit bit. */
#define L2_HITBIT_GET_MAX_TIMES 100
                     ...
                     uint32_t flags)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_to_l2_if(unit, port, &l2_iif));
 ...
 */
static int
l2_pvlan_info_get(int unit, bcm_port_t port, bcmint_l2_pvlan_info_t *info)
{
    l2_ctrl_t *l2c = &l2_ctrl[unit];
    int l2_iif, rv;
    bcm_port_t port_out;
    ...
    rv = bcmi_ltsw_id_port_to_l2_if(unit, port, &l2_iif);
    if (SHR_SUCCESS(rv)) {
...
l2_pvlan_set(int unit, bcmlt_entry_handle_t eh, bcm_port_t port)
{
    ...
    sal_memset(&info, 0, sizeof(bcmint_l2_pvlan_info_t));
    SHR_IF_ERR_VERBOSE_EXIT
                         ...
                         int *dst_hit, int *src_hit)
{
    ...
    bcmint_l2_flexctr_ext_info_t *flexctr_ext_info = NULL;
    const bcmint_l2_id_map_t *id_map = NULL;
    uint64_t data;
    ...
    if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
        flexctr_ext_info = l2c->flexctr_ext_info;
    ...
    if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
        counter_id[0] = flexctr_ext_info->dst_id;
        counter_id[1] = flexctr_ext_info->src_id;
    } else {
        /*
                         ...
                         bool dst, bool src, int dst_val, int src_val)
{
    ...
    if (dst) {
        if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
            counter_id = flexctr_ext_info->dst_id;
        } else {
    ...
    if (src) {
        if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
            counter_id = flexctr_ext_info->src_id;
        } else {
                                ...
                                int *dst_hit, int *src_hit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
        counter_id[0] = flexctr_ext_info->narrow_dst_id;
        counter_id[1] = flexctr_ext_info->narrow_src_id;
    } else {
        counter_id[0] = flexctr_info->uft_dst_id;
        counter_id[1] = flexctr_info->uft_src_id;
    }
                                ...
                                bool dst, bool src, int dst_val, int src_val)
{
    ...
    if (dst) {
        if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
            counter_id = flexctr_ext_info->narrow_dst_id;
        } else {
            counter_id = flexctr_info->uft_dst_id;
        }
    ...
    if (src) {
        if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
            counter_id = flexctr_ext_info->narrow_src_id;
        } else {
            counter_id = flexctr_info->uft_src_id;
        }
                               ...
                               bcm_l2_addr_t *l2addr, uint32_t fld_bmp)
{
    ...
    bool l3mc = false, vp_lag = false, ecmp_member_index_is_valid = false;
    ...
    if (fld_bmp & (L2_HOST_FLD_DST | L2_HOST_FLD_DST_TYPE)) {
        if (l2addr->flags2 & BCM_L2_FLAGS2_LOOKUP_MISS) {
            /* Set destination_type as NO_OP. */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_symbol_add_by_id(eh,
                     id_map[BCMINT_L2_HOST_FLD_DESTINATION_TYPE].val,
                     str_map[BCMINT_L2_DEST_TYPE_NO_OP].str));
                    ...
                    if (vp_info.flags & BCMI_LTSW_VIRTUAL_VP_CASCADED) {
                        ...
                        SHR_IF_ERR_VERBOSE_EXIT
                            (bcmlt_entry_field_add_by_id(eh,
                                 id_map[BCMINT_L2_HOST_FLD_DESTINATION].val,
                                 port_out));
                        ecmp_member_index_is_valid = true;
    ...
    if (fld_bmp & L2_HOST_FLD_FLEX_CTR_ACTION) {
        if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
            if (l2c->flexctr_ext_info) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_flexctr_counter_id_info_get(unit,
                         l2c->flexctr_ext_info->dst_id,
                          &counter_info));
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add_by_id(eh,
    ...
    if (fld_bmp & L2_HOST_FLD_MISC_CTRL_0) {
        ...
        if (ecmp_member_index_is_valid &&
            l2c->ctrl_info.misc_ctrl_0_ecmp_member_is_valid) {
            data |= 1 << L2_HOST_MISC_CTRL_0_ECMP_MEMBER0_CTRL;
        }
                               ...
                               uint32_t fld_bmp, bcm_l2_addr_t *l2addr)
{
    ...
    if (fld_bmp & L2_HOST_FLD_MISC_CTRL_0) {
        ...
        if (!(data & (1 << L2_HOST_MISC_CTRL_0_VALID))) {
            l2addr->flags2 |= BCM_L2_FLAGS2_LOOKUP_MISS;
        }
    ...
    if (fld_bmp & L2_HOST_FLD_FLEX_CTR_ACTION) {
        if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
            if (l2c->flexctr_ext_info) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_flexctr_counter_id_info_get(unit,
                         l2c->flexctr_ext_info->narrow_dst_id,
                         &counter_info));
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add_by_id(eh,
                          ...
                          bcm_l2_station_t *station, bool key_only)
{
    ...
    if (station->src_port_mask == 0) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_to_l2_if(unit, station->src_port, &l2iif));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_to_l2_if(unit,
                                     station->src_port_mask,
                                     &l2iif_mask));
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
                       ...
                       bcm_l2_station_t *station, bool no_key)
{
    ...
    if (!no_key) {
        ...
        if (BCM_GPORT_IS_SET(station->src_port)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_l2_if_to_port(unit,
                                         l2_if_mask,
                                         &station->src_port_mask));
        } else {
            SHR_IF_ERR_VERBOSE_EXIT
                              ...
                              bcm_l2_addr_t *l2addr)
{
    ...
    if (cache[6] == BCMI_XFS_DEST_TYPE_L2_OIF) {
        ...
        rv = bcmi_ltsw_id_l2_if_to_port(unit, l2_if, &gport);
        if (SHR_FAILURE(rv)) {
            LOG_WARN(BSL_LOG_MODULE,
...
/*!
 * \brief Get count of L2 host table inuse entry.
 * Based on L2_HOST_NARROW_TABLE.
 *
 * \param [in] unit Unit number.
 * \param [out] unit count.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
l2_entry_inuse_cnt_get(int unit, int *count)
{
 ...
 */
static void
l2_host_table_usage_rate_get(int unit, int *ratio)
{
    int count = 0;
    /* static */
    ...
    if (l2_host_table_size[unit] == 0) {
        (void)xfs_ltsw_l2_size_get(unit, &l2_host_table_size[unit]);
        l2_host_table_size[unit] = l2_host_table_size[unit] * 2;
    }
    *ratio = 0;
    if (l2_host_table_size[unit]) {
        (void)l2_entry_inuse_cnt_get(unit, &count);
               ...
               void *user_data)
{
    ...
    sal_usecs_t time, schedule_time = 0, time_spent = 0;
    const bcmint_l2_id_map_t *id_map = NULL;
    ...
    l2_host_table_usage_rate_get(unit, &ratio);
        ...
        if (ratio > L2_HOST_TAB_LOW_WATERMARK) {
            ...
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
                         ...
                         ratio, slow_poll_mode[unit] ? "slow" : "fast"));
        } else {
...
l2_learn_enable(int unit, bool enable)
{
    ...
    if (enable) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_capacity_get(unit, LEARN_CACHE_DATAs,
...
/*!
 * \brief Restore L2 host entries for a VLAN.
 *
 * \param [in] unit Unit number.
 * \param [in] info L2 table view vlan info.
 *
 ...
 */
static int
l2_view_table_restore(int unit, bcmi_ltsw_l2_view_vlan_info_t *info)
{
    bool wrong_view;
    bcmi_ltsw_l2_view_vlan_info_t correct_info;
    bcmint_l2_traverse_t trav_st;
    const bcmint_l2_id_map_t *id_map = NULL;
    ...
    wrong_view = info->l2_view;
    id_map = L2_MAP_ID_INFO(unit, wrong_view);
    SHR_NULL_CHECK(id_map, SHR_E_INIT);

    correct_info.l2_view = !(wrong_view);
    correct_info.vlan = info->vlan;
    /*
     * Traverse wrong table view and move entries from wrong view to
     * correct view.
     */
    sal_memset(&trav_st, 0, sizeof(bcmint_l2_traverse_t));
    trav_st.tid = wrong_view ? id_map[BCMINT_L2_HOST_NARROW_TBL].val :
                               id_map[BCMINT_L2_HOST_TBL].val;
    trav_st.fld_cnt = wrong_view ?
        id_map[BCMINT_L2_HOST_NARROW_FLD_ELEMENT_CNT].val :
        id_map[BCMINT_L2_HOST_FLD_ELEMENT_CNT].val;
    trav_st.user_data = (void *)&correct_info;
    trav_st.int_cb = l2_view_table_move_int_cb;
    SHR_IF_ERR_VERBOSE_EXIT
        (l2_traverse(unit, &trav_st));
...
l2_flexctr_info_dump(int unit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
        ...
        LOG_CLI((BSL_META_U(unit,
                            "           dst_id: %#x\n"),
                            flexctr_ext_info->dst_id));
        LOG_CLI((BSL_META_U(unit,
                            "           src_id: %#x\n"),
                            flexctr_ext_info->src_id));
        LOG_CLI((BSL_META_U(unit,
                            "           narrow_dst_id: %#x\n"),
                            flexctr_ext_info->narrow_dst_id));
        LOG_CLI((BSL_META_U(unit,
                            "           narrow_src_id: %#x\n"),
                            flexctr_ext_info->narrow_src_id));
    } else {
        flexctr_info = l2c->flexctr_info;
...
l2_hitbit_detach(int unit)
{
    l2_ctrl_t *l2c = &l2_ctrl[unit];
    bcmint_l2_flexctr_info_t *flexctr_info = NULL;
    bcmint_l2_flexctr_ext_info_t *flexctr_ext = NULL;

    SHR_FUNC_ENTER(unit);

    if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
        flexctr_ext = l2c->flexctr_ext_info;
        SHR_NULL_CHECK(flexctr_ext, SHR_E_INIT);

        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_l2_hitbit_detach(unit, flexctr_ext));
    } else {
        flexctr_info = l2c->flexctr_info;
...
l2_hitbit_init(int unit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_l2_hitbit_init(unit, flexctr_ext));
    } else {
        flexctr_info = l2c->flexctr_info;
        SHR_NULL_CHECK(flexctr_info, SHR_E_INIT);
 ...
 */
static int
l2_ctrl_info_init(int unit)
{
    l2_ctrl_t *l2c = &l2_ctrl[unit];
    bcmint_l2_ctrl_info_t l2_ctrl_info;

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(l2c, SHR_E_MEMORY);

    sal_memset(&l2_ctrl_info, 0x0, sizeof(bcmint_l2_ctrl_info_t));
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_l2_ctrl_info_init(unit, &l2_ctrl_info));
    sal_memcpy(&l2c->ctrl_info,
               &l2_ctrl_info,
               sizeof(bcmint_l2_ctrl_info_t));
 ...
 */
static int
l2_ctrl_info_deinit(int unit)
{
    bcmint_l2_ctrl_info_t *l2_ctrl_info = &l2_ctrl[unit].ctrl_info;

    if (l2_ctrl_info) {
        sal_memset(l2_ctrl_info, 0, sizeof(bcmint_l2_ctrl_info_t));
    }
...
xfs_ltsw_l2_init(int unit)
{
    ...
    uint32_t vfi_num = 0, l2_iif_num = 0, offset = 0;
    ...
    ha_instance_size = sizeof(bcmint_l2_pvlan_info_t);
    ha_array_size = l2_iif_num;
    req_size = sizeof(bcmint_l2_pvlan_info_t) * l2_iif_num;
    alloc_size = req_size;
    ...
    if (enable && !l2c->host_disable) {
        if (ltsw_feature(unit, LTSW_FT_L2_HITBIT_EXT)) {
            /* Allocate HA memory for L2 flexctr ext info. */
            req_size = sizeof(bcmint_l2_flexctr_ext_info_t);
            alloc_size = req_size;
            ptr_flexctr = bcmi_ltsw_ha_mem_alloc(
                            unit,
                            BCMI_HA_COMP_ID_L2,
                            BCMINT_L2_SUB_COMP_ID_L2_FLEXCTR_EXT_INFO,
                            "bcmL2FlexExtCtrInfo",
                            &alloc_size);
            ...
            rv = bcmi_ltsw_issu_struct_info_report(
                    unit,
                    BCMI_HA_COMP_ID_L2,
                    BCMINT_L2_SUB_COMP_ID_L2_FLEXCTR_EXT_INFO,
                    0, req_size, 1,
                    BCMINT_L2_FLEXCTR_EXT_INFO_T_ID);
            if (rv != SHR_E_EXISTS) {
                SHR_IF_ERR_VERBOSE_EXIT(rv);
            }

            l2c->flexctr_ext_info = ptr_flexctr;
        } else {
    ...
    /* Init L2 control info. */
    SHR_IF_ERR_VERBOSE_EXIT
        (l2_ctrl_info_init(unit));
    ...
    bcm_l2_addr_t *l2addr)
{
    ...
    int retry = 0;
    int src_hit = 0, dst_hit = 0, src = 0, dst = 0;
    ...
    if (fld_bmp & (L2_HOST_FLD_DST_ENTRY_IDX | L2_HOST_FLD_SRC_ENTRY_IDX)) {
        ...
        while (retry < L2_HITBIT_GET_MAX_TIMES) {
            rv = l2_hitbit_get(unit, eh, single, &dst_hit, &src_hit);
            if (rv < 0) {
                LOG_WARN(BSL_LOG_MODULE,
                         (BSL_META_U(unit, "l2_hitbit_get fails rv(%d)!\n"),
                          rv));
            ...
            /*
             * Skip waiting in the following cases:
             * 1) No race condition during update since CCI does not set hit bits.
             * 2) Both dst and src hit bits are set by CCI during insertion
             */
            if (update || (dst_hit && src_hit)) {
                break;
            }

            /* Hitbits not set, sleep. */
            sal_usleep(10);
            retry++;
        }

        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Retry to get hitbit %d times!"
                                "src_hit=%d dst_hit=%d\n"),
                                retry, src_hit, dst_hit));

        /* If hit bit is already set and API hit flag is present, skip. */
        src = src_hit ^ ((l2addr->flags & (BCM_L2_HIT | BCM_L2_SRC_HIT)) != 0);
        dst = dst_hit ^ ((l2addr->flags & (BCM_L2_HIT | BCM_L2_DES_HIT)) != 0);

        rv = l2_hitbit_set(unit, eh, dst, src, l2addr, single);
        if (SHR_FAILURE(rv)) {
            /*
             * Because hitbit is a debugging functionality, even though
             * hitbit set/get fails, it shouldn't stop here so that
    ...
    bcm_vlan_t vid)
{
    ...
    if (l2addr_del.flags & BCM_L2_HIT) {
        int src = 0, dst = 0;

        src = (l2addr_del.flags & BCM_L2_SRC_HIT) ? true : false;
        dst = (l2addr_del.flags & BCM_L2_DES_HIT) ? true : false;
        rv = l2_hitbit_set(unit, eh, dst, src, &l2addr, single);
        if (SHR_FAILURE(rv)) {
            /*
             * Because hitbit is a debugging functionality, even though
    ...
    bcmi_ltsw_l2_view_vlan_info_t *info)
{
    ...
    rv = l2_view_table_update(unit, info);

    /*
     * During l2_view_table_update, some L2 table entries might have been moved
     * to new table view before error return, so need to restore those entries.
     */
    if (rv != SHR_E_NONE) {
        SHR_IF_ERR_VERBOSE_EXIT
            (l2_view_table_restore(unit, info));
        SHR_ERR_EXIT(rv);
    }
