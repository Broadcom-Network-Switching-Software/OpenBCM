...
typedef struct ltsw_flexctr_quant_info_s {
    ...
    uint32_t quant_global_max[FC_MAX_STAGE];
...
/*!
 * \brief Flex counter block address.
 */
typedef struct ltsw_flexctr_block_addr_s {

    /*! The first counter pool. */
    uint32_t start_pool_idx;

    /*! The first counter section in the first pool. */
    uint32_t start_section_idx;

    /*! The last counter pool. */
    uint32_t end_pool_idx;

    /*! The last counter section in the last pool. */
    uint32_t end_section_idx;

} ltsw_flexctr_block_addr_t;

/*!
 * \brief Flex counter block info.
 */
typedef struct ltsw_flexctr_block_info_s {

    /*! Min block id. */
    uint32_t blk_id_min[FC_MAX_STAGE];

    /*! Max block id. */
    uint32_t blk_id_max[FC_MAX_STAGE];

    /*! Block array. */
    ltsw_flexctr_block_t *blks[FC_MAX_STAGE];

    /*! Protection mutex. */
    sal_mutex_t mutex;

} ltsw_flexctr_block_info_t;

/*! Per chip flex counter pool information. */
static
ltsw_flexctr_block_info_t *flexctr_block_info[BCM_MAX_NUM_UNITS] = {0};

/*! Flex counter block operation lock. */
#define FLEXCTR_BLOCK_LOCK(_u)                                           \
    sal_mutex_take(flexctr_block_info[_u]->mutex, SAL_MUTEX_FOREVER)

/*! Flex counter block operation unlock. */
#define FLEXCTR_BLOCK_UNLOCK(_u)                                         \
    sal_mutex_give(flexctr_block_info[_u]->mutex)
...
/*! Maximum right shift number. */
#define FLEXCTR_OP_SHIFT_RIGHT_MAX  16

/*! Maximum left shift number. */
#define FLEXCTR_OP_SHIFT_LEFT_MAX   32
...
static ltsw_flexctr_stage_info_t st_info[FC_MAX_STAGE] = {
    {
        ...
        .ha_blk_id = BCMINT_FLEXCTR_ING_BLOCK_SUB_COMP_ID,
        .ph = {
        ...
        .obj_info = CTR_ING_EFLEX_OBJ_INFOs,
    },
    {
        ...
        .ha_blk_id = BCMINT_FLEXCTR_EGR_BLOCK_SUB_COMP_ID,
        .ph = {
        ...
        .obj_info = CTR_EGR_EFLEX_OBJ_INFOs,
    },
};
                                 ...
                                 bool quant_32bit_mode)
{
    ...
    if ((key->id >= max_quant_id) ||
        (quant_cfg->flags & BCM_FLEXCTR_QUANTIZATION_FLAGS_COVER_ALL_RANGE)) {
...
ltsw_flexctr_quantization_db_init(int unit)
{
    ...
    int rv = SHR_E_NONE;
    const bcmint_flexctr_enum_map_t *quant_map;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexctr_quant_object_enum_map_get(unit, &quant_map,
                                                     &quant_count));
    ...
    for (stage = FC_MIN_STAGE; stage < FC_MAX_STAGE; stage++) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_field_value_range_get(unit,
                                           st_info[stage].range_chk_profile,
                                           OBJ_SELECTs, &min, &max));
                                          ...
                                          bool *quant_32bit_mode)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_FLEXCTR_QUANT_32BIT)) {
        if (quant_cfg->flags & BCM_FLEXCTR_QUANTIZATION_FLAGS_32BIT_MODE) {
            if (quant_cfg->flags &
                BCM_FLEXCTR_QUANTIZATION_FLAGS_COVER_ALL_RANGE) {
                LOG_ERROR(BSL_LOG_MODULE,
    ...
    if (quant_cfg->flags & BCM_FLEXCTR_QUANTIZATION_FLAGS_COVER_ALL_RANGE) {
...
/*
 * \brief Initialize the flex counter block database.
 *
 * This function will allocate resources for the block information.
 *
 * It assumes that the caller will invoke ltsw_flexctr_block_db_cleanup() to
 * clean the resources when any errors retured by this function.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_block_db_init(int unit)
{
    ltsw_flexctr_block_info_t *bi = flexctr_block_info[unit];
    ltsw_flexctr_pool_info_t *pi = flexctr_pool_info[unit];
    int rv, stage, num_pools, num_pipes;
    uint32_t ha_alloc_size, ha_req_size;
    uint32_t ha_instance_size, ha_array_size;
    void *alloc_ptr;
    int warm = bcmi_warmboot_get(unit);

    SHR_FUNC_ENTER(unit);

    /* Do nothing if the database is already initialized */
    if (bi) {
        SHR_EXIT();
    }

    SHR_ALLOC(bi, sizeof(*bi), "ltswFlexctrBlockInfo");
    SHR_NULL_CHECK(bi, SHR_E_PARAM);
    sal_memset(bi, 0, sizeof(*bi));
    flexctr_block_info[unit] = bi;

    /* Create mutex. */
    bi->mutex = sal_mutex_create("ltswFlexctrBlockInfoLock");
    SHR_NULL_CHECK(bi->mutex, SHR_E_MEMORY);

    /* Alloc block buffer from HA. */
    for (stage = FC_MIN_STAGE; stage < FC_MAX_STAGE; stage++) {
        num_pipes = flexctr_pipe_max[unit][stage];
        num_pools = pi->npools[stage];

        ha_instance_size = sizeof(ltsw_flexctr_block_t);
        ha_array_size = num_pools * num_pipes;
        ha_req_size = ha_instance_size * ha_array_size;
        ha_alloc_size = ha_req_size;
        alloc_ptr = bcmi_ltsw_ha_mem_alloc(unit,
                                           BCMI_HA_COMP_ID_FLEXCTR,
                                           st_info[stage].ha_blk_id,
                                           "bcmFlexCtrBlockInfo",
                                           &ha_alloc_size);
        SHR_NULL_CHECK(alloc_ptr, SHR_E_MEMORY);
        ...
        rv = bcmi_ltsw_issu_struct_info_report(unit,
                                               ...
                                               ha_instance_size,
                                               ha_array_size,
                                               LTSW_FLEXCTR_BLOCK_T_ID);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, SHR_E_EXISTS);

        bi->blks[stage] = alloc_ptr;
        bi->blk_id_min[stage] = 0;
        bi->blk_id_max[stage] = ha_array_size - 1;
    }

exit:
    if (!warm && SHR_FUNC_ERR()) {
        for (stage = FC_MIN_STAGE; stage < FC_MAX_STAGE; stage++) {
            if (bi->blks[stage]) {
                (void)bcmi_ltsw_ha_mem_free(unit, bi->blks[stage]);
            }
        }
    }
...
/*!
 * \brief Cleanup flex counter block software database.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_block_db_cleanup(int unit)
{
    ltsw_flexctr_block_info_t *bi = flexctr_block_info[unit];
    ...
    if (bi->mutex) {
        sal_mutex_destroy(bi->mutex);
    }

    SHR_FREE(bi);
    flexctr_block_info[unit] = NULL;
...
/*!
 * \brief Flex counter block id to block function.
 *
 * This function is used to get the block data of a block ID.
 *
 * \param [in] unit Unit number.
 * \param [in] blk_id Block ID.
 * \param [out] blk Block data.
 * \param [out] key Output key identification.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_block_id_to_block(int unit,
                               uint32_t blk_id,
                               ltsw_flexctr_block_t **blk,
                               ltsw_flexctr_key_t *key)
{
    ltsw_flexctr_block_info_t *bi = flexctr_block_info[unit];
    ltsw_flexctr_block_t *blk_tmp = NULL;

    SHR_FUNC_ENTER(unit);

    FLEXCTR_ID_DECOMPOSE(blk_id, key);

    if (key->type != FLEXCTR_BLOCK) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Invalid type %x from block id %x.\n"),
                   key->type, blk_id));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    if (key->stage < FC_MIN_STAGE || key->stage >= FC_MAX_STAGE) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Invalid stage %x from block id %x.\n"),
                   key->stage, blk_id));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    if (key->pipe != 0) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Invalid pipe %x from block id %x.\n"),
                   key->pipe, blk_id));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (key->id < bi->blk_id_min[key->stage] &&
        key->id > bi->blk_id_max[key->stage]) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Invalid id %x from block id %x.\n"),
                   key->id, blk_id));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    blk_tmp = &(bi->blks[key->stage][key->id]);
    if (!blk_tmp->used) {
        LOG_INFO(BSL_LOG_MODULE,
                 (BSL_META_U(unit, "Block id %x not found.\n"), blk_id));
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }
...
/*!
 * \brief Flex counter block creation function.
 *
 * This function is used to create a flex counter block.
 *
 * \param [in] unit Unit number.
 * \param [in] act_cfg Flex counter block data structure.
 * \param [out] blk_id Block ID.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_block_create(int unit,
                          bcm_flexctr_block_t *blk_cfg,
                          uint32 *blk_id)
{
    ltsw_flexctr_pool_info_t *pi = flexctr_pool_info[unit];
    ltsw_flexctr_block_info_t *bi = flexctr_block_info[unit];
    ltsw_flexctr_pool_t *pool = NULL;
    ltsw_flexctr_block_t *blk = NULL;
    bool is_pipe_unique;
    int num_pools, num_ctrs_pool, num_pools_req, num_pools_free;
    uint32_t idx, start_pidx, end_pidx, blk_id_min, blk_id_max;
    int pipe, num_pipes;
    ltsw_flexctr_key_t key = { 0 };

    SHR_FUNC_ENTER(unit);

    FLEXCTR_BLOCK_LOCK(unit);
    FLEXCTR_POOL_LOCK(unit);

    SHR_NULL_CHECK(blk_cfg, SHR_E_PARAM);
    SHR_NULL_CHECK(blk_id, SHR_E_PARAM);

    key.type = FLEXCTR_BLOCK;
    key.pipe = 0;

    /* Get stage */
    if (blk_cfg->stage == bcmFlexctrDirectionIngress) {
        key.stage = BCMI_LTSW_FLEXCTR_STAGE_ING_CTR;
    } else if (blk_cfg->stage == bcmFlexctrDirectionEgress) {
        key.stage = BCMI_LTSW_FLEXCTR_STAGE_EGR_CTR;
    } else {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    /* Get a free block ID */
    blk_id_min = bi->blk_id_min[key.stage];
    blk_id_max = bi->blk_id_max[key.stage];
    for (idx = blk_id_min; idx <= blk_id_max; idx++) {
        if (!bi->blks[key.stage][idx].used) {
            blk = &bi->blks[key.stage][idx];
            break;
        }
    }
    if (!blk) {
    ...
    /* Get pipes */
    num_pipes = flexctr_pipe_max[unit][key.stage];
    is_pipe_unique = flexctr_pipe_unique[unit][key.stage];
    if (BCM_PBMP_IS_NULL(blk_cfg->ports)) {
        /* Reserve all pipes */
        SHR_BITSET_RANGE(blk->pipe, 0, num_pipes);
    } else {
        if (is_pipe_unique) {
            /* Only reserve one pipe per request */
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_flexctr_config_port_to_pipe(unit, blk_cfg->ports, &pipe));
            SHR_BITSET(blk->pipe, pipe);
        } else {
            /* For global mode, ports must be empty */
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    }

    /* Search the free pools */
    if (blk_cfg->num_ctrs == 0) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    num_pools = pi->npools[key.stage];
    num_ctrs_pool = pi->nctrs_in_pool[key.stage];
    num_pools_req = (blk_cfg->num_ctrs + num_ctrs_pool - 1) / num_ctrs_pool;
    num_pools_free = 0;
    for (idx = 0; idx < num_pools; idx++) {
        bool pool_free = true;

        /* Check if the pool is free across all requested pipes */
        SHR_BIT_ITER(blk->pipe, num_pipes, pipe) {
            pool = &(pi->pools[key.stage][pipe][idx]);

            /* Check if the pool is already used by other blocks or actions. */
            if (pool->shared || pool->used_count) {
                pool_free = false;
                break;
            }
        }
        ...
        /* Stop searching when we have enoguh free and adjacent pools */
        num_pools_free++;
        if (num_pools_free == num_pools_req) {
            break;
        }
    }
    if (num_pools_free == num_pools_req) {
        end_pidx = idx;
        start_pidx = end_pidx - num_pools_req + 1;
    } else {
        SHR_ERR_EXIT(SHR_E_RESOURCE);
    }

    /* Compose block id. */
    FLEXCTR_ID_COMPOSE(&key, *blk_id);

    /* Update pool information to reserve the pools */
    SHR_BIT_ITER(blk->pipe, num_pipes, pipe) {
        for (idx = start_pidx; idx <= end_pidx; idx++) {
            pool = &(pi->pools[key.stage][pipe][idx]);
            pool->shared = true;
            pool->hint = *blk_id;
        }
    ...
    blk->num_ctrs = blk_cfg->num_ctrs;
    blk->start_pool_idx = start_pidx;
    blk->end_pool_idx = end_pidx;
    blk->ref_count = 0;
    blk->used = true;

exit:
    FLEXCTR_POOL_UNLOCK(unit);
    FLEXCTR_BLOCK_UNLOCK(unit);

    SHR_FUNC_EXIT();
}

/*!
 * \breif Flex counter block destroy function.
 *
 * This function is used to destroy a flex counter block.
 *
 * \param [in] unit Unit number.
 * \param [in] blk_id Flex counter Block ID.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_block_destroy(int unit, uint32_t blk_id)
{
    ltsw_flexctr_pool_info_t *pi = flexctr_pool_info[unit];
    ltsw_flexctr_pool_t *pool = NULL;
    ltsw_flexctr_block_t *blk = NULL;
    ltsw_flexctr_key_t key = { 0 };
    uint32_t num_pipes, pipe, idx;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_block_id_to_block(unit, blk_id, &blk, &key));

    if (blk->ref_count != 0) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Counter block %x is still in use.\n"),
                     blk_id));
        SHR_ERR_EXIT(SHR_E_BUSY);
    }

    /* Update pool information to free the pools */
    FLEXCTR_POOL_LOCK(unit);
    num_pipes = flexctr_pipe_max[unit][key.stage];
    SHR_BIT_ITER(blk->pipe, num_pipes, pipe) {
        for (idx = blk->start_pool_idx; idx <= blk->end_pool_idx; idx++) {
            pool = &(pi->pools[key.stage][pipe][idx]);
            pool->shared = false;
            pool->hint = 0;
        }
...
/*!
 * \breif Flex counter block get function.
 *
 * This function is used to get a flex counter block.
 *
 * \param [in] unit Unit number.
 * \param [in] blk_id Flex counter block ID.
 * \param [out] blk_cfg Flex counter block configuration.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_block_get(int unit, uint32_t blk_id, bcm_flexctr_block_t *blk_cfg)
{
    ltsw_flexctr_block_t *blk = NULL;
    ltsw_flexctr_key_t key = { 0 };
    bcm_port_config_t pcfg;
    int num_pipes, pipe_cnt, pipe;
    ...
    blk_cfg->num_ctrs = blk->num_ctrs;
    blk_cfg->stage = key.stage;

    num_pipes = flexctr_pipe_max[unit][key.stage];
    SHR_BITCOUNT_RANGE(blk->pipe, pipe_cnt, 0, num_pipes);
    if (pipe_cnt == num_pipes) {
        BCM_PBMP_CLEAR(blk_cfg->ports);
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm_ltsw_port_config_get(unit, &pcfg));
        SHR_BIT_ITER(blk->pipe, num_pipes, pipe) {
            BCM_PBMP_ASSIGN(blk_cfg->ports, pcfg.per_pp_pipe[pipe]);
        }
    }
 ...
 * \breif Flex counter block get function.
 *
 * This function is used to traverse the flex counter blocks.
 *
 * \param [in] unit Unit number.
 * \param [in] blk_id Flex counter block ID.
 * \param [out] blk_cfg Flex counter block configuration.
 ...
 */
static int
ltsw_flexctr_block_traverse(int unit,
                            ...
                            void *user_data)
{
    ...
    ltsw_flexctr_block_t *blk = NULL;
    ltsw_flexctr_key_t key = { 0 };
    bcm_flexctr_block_t blk_cfg = { 0 };
    int rv, blk_id, blk_id_min, blk_id_max;
    ...
    for (key.stage = FC_MIN_STAGE; key.stage < FC_MAX_STAGE; key.stage++) {
        ...
        for (key.id = blk_id_min; key.id <= blk_id_max; key.id++) {
            blk = &(bi->blks[key.stage][key.id]);
            ...
            FLEXCTR_ID_COMPOSE(&key, blk_id);
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_flexctr_block_get(unit, blk_id, &blk_cfg));

            rv = trav_fn(unit, blk_id, &blk_cfg, user_data);
            if (SHR_FAILURE(rv)) {
                break;
            }
        }
...
/*!
 * \brief Attach a block id to a action.
 *
 * \param [in] unit Unit number.
 * \param [in] blk_id Block ID.
 * \param [in] ctr_offset Counter offset.
 * \param [in] akey Action key.
 * \param [in] ctr_mode Counter mode. FC_POOL_NONE or FC_POOL_SLIM.
 * \param [in] num_ctrs_req Number of required counters by the action.
 * \param [out] baddr Block address.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_block_attach(int unit,
                          uint32_t blk_id,
                          uint32_t ctr_offset,
                          ltsw_flexctr_key_t *akey,
                          int ctr_mode,
                          uint32_t num_ctrs_req,
                          ltsw_flexctr_block_addr_t *baddr)
{
    ltsw_flexctr_pool_info_t *pi = flexctr_pool_info[unit];
    ltsw_flexctr_block_t *blk = NULL;
    ltsw_flexctr_key_t bkey = { 0 };
    int num_sects_pool, num_ctrs_pool, num_ctrs_sect;
    int sect_offset;
    ...
    num_sects_pool = pi->nsects_in_pool[akey->stage];
    num_ctrs_sect = pi->nctrs_in_sect[akey->stage];
    num_ctrs_pool = pi->nctrs_in_pool[akey->stage];

    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_block_id_to_block(unit, blk_id, &blk, &bkey));

    if (bkey.stage != akey->stage) {
        LOG_ERROR(BSL_LOG_MODULE,
             (BSL_META_U(unit, "Block %x is not used for stage %x.\n"),
              blk_id, akey->stage));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (!SHR_BITGET(blk->pipe, akey->pipe)) {
        LOG_ERROR(BSL_LOG_MODULE,
             (BSL_META_U(unit, "Block %x is not used for pipe %x.\n"),
              blk_id, akey->pipe));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    /* Check if the block size is sufficient */
    if (num_ctrs_req + ctr_offset > blk->num_ctrs) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Block size is not sufficient.\n")));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    /* Get the section offset within the block */
    sect_offset = 0;
    if (ctr_offset) {
        if (ctr_mode == FC_POOL_SLIM) {
            if (ctr_offset % num_ctrs_pool) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "Counter offset must align with %d for "
                                      "slim mode.\n"),
                           num_ctrs_pool));
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
        } else {
            if (ctr_offset % num_ctrs_sect) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "Counter offset must align with %d.\n"),
                           num_ctrs_sect));
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
        }

        sect_offset = ctr_offset / num_ctrs_sect;
    }

    baddr->start_pool_idx = blk->start_pool_idx + sect_offset / num_sects_pool;
    baddr->start_section_idx = sect_offset % num_sects_pool;
    baddr->end_pool_idx = blk->end_pool_idx;
    baddr->end_section_idx = num_sects_pool - 1;
...
/*!
 * \brief Detach a block id from a action.
 *
 * \param [in] unit Unit number.
 * \param [in] blk_id Block ID.
 * \param [in] akey Action key.
 ...
 */
static int
ltsw_flexctr_block_detach(int unit,
                          uint32_t blk_id,
                          ltsw_flexctr_key_t *akey)
...
/*! Break the current loop when alloc with pool id. */
#define FLEXCTR_POOL_ALLOC_BREAK(with_pool_id) \
        if (with_pool_id) {                    \
            break;                             \
        }
                        ...
                        ltsw_flexctr_action_t *action)
{
    ...
    ltsw_flexctr_block_addr_t baddr;
    bool with_pool_id = false;
    int start_section_idx = 0;

    SHR_FUNC_ENTER(unit);

    sal_memset(&baddr, 0, sizeof(baddr));
    num_pools = pi->npools[key->stage];
    ...
    global = (act_cfg->flags & BCM_FLEXCTR_ACTION_FLAGS_GLOBAL) ?
             true : false;
    with_pool_id = (act_cfg->flags & BCM_FLEXCTR_ACTION_FLAGS_POOL_ID) ?
                    true : false;

    if (flexctr_mode == bcmiFlexctrModePipeUnique) {
        if (global) {
            baddr.start_pool_idx = num_pools - pi->rsvd_num[key->stage];
        } else {
            num_pools -= pi->rsvd_num[key->stage];
        }
    }
    ...
    if ((act_cfg->source == bcmFlexctrSourceIfp ||
        ...
        phint = act_cfg->hint;
    } else if (act_cfg->hint_type == bcmFlexctrHintSharedBlock) {
        /* Use the pre-allocated block */
        SHR_IF_ERR_EXIT
            (ltsw_flexctr_block_attach(unit, act_cfg->hint, act_cfg->hint_ext,
                                       key, mode, num_ctr_req, &baddr));
        SHR_EXIT();
    }

    /* Search from pool ID and section ID given by user. */
    if (with_pool_id) {
        baddr.start_pool_idx = action->start_pool_idx;
        start_section_idx = action->start_section_idx;
    }

    FLEXCTR_POOL_LOCK(unit);

    /* Find free counter resources */
    while (baddr.start_pool_idx < num_pools) {
        /* Reset. */
        cur_pool_idx = baddr.start_pool_idx;
        cur_section_idx = 0;
        section_cnt = 0;
        section_cnt_rem = 0;
        done = FALSE;
        ...
        if (pool->shared) {
            /* Pool is reserved by a block, try the next pool. */
            baddr.start_pool_idx = cur_pool_idx + 1;

            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit, "Pool %d is occupied, next try %d\n"),
                         cur_pool_idx, baddr.start_pool_idx));
            FLEXCTR_POOL_ALLOC_BREAK(with_pool_id);
            continue;
        }
        ...
        if (!SHR_BITNULL_RANGE(pool->used_by_tbls, 0, FLEXCTR_NUM_TBLS_MAX)) {
            ...
            if (l2_hit_alloc && phint == 1) {
                if (SHR_BITGET(pool->used_by_tbls, tbl_bit)) {
                    ...
                    baddr.start_pool_idx = cur_pool_idx;
                    baddr.end_pool_idx = cur_pool_idx + 1;
                    baddr.start_section_idx = 16;
                    baddr.end_section_idx = num_sects_pool - 1;
                    done = TRUE;
                    break;
                } else {
            ...
            /* When create with pool id, it is allowed to share the pool. */
            if ((pool->mode != mode) || (((pool->hint != phint) ||
                !SHR_BITGET(pool->used_by_tbls, tbl_bit)) && !with_pool_id)) {
        ...
        for (cur_section_idx = start_section_idx;
            ...
            if (section_cnt == num_section_req) {
                baddr.end_pool_idx = cur_pool_idx;
                baddr.start_section_idx = cur_section_idx + 1 - num_section_req;
                baddr.end_section_idx = cur_section_idx;
                done = TRUE;
                break;
            }
        }
        ...
        if (!done) {
            ...
            baddr.start_section_idx = num_sects_pool - section_cnt;
        ...
        while (!done) {
            ...
            if (cur_pool_idx >= num_pools) {
                baddr.start_pool_idx = num_pools;

                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit, "Max pool reached %d\n"),
                             baddr.start_pool_idx));
                break;
            }
        ...
        /* Check the starting pool ID and section index. */
        if (with_pool_id) {
            if (baddr.start_pool_idx != action->start_pool_idx ||
                baddr.start_section_idx != action->start_section_idx) {
                done = false;
                break;
            }
        }
        ...
        if (done) {
            ...
            if (baddr.start_pool_idx == baddr.end_pool_idx) {
                /* Set bits in current pool. */
                start_bit = baddr.start_section_idx;
                ...
                /* Not update hint when create with pool id. */
                if (!with_pool_id) {
                ...
                pool->used_count++;
                pool->ref_count[tbl_bit]++;
            } else {
                /* Counters are spread across multiple pools */
                cur_pool_idx = baddr.start_pool_idx;

                while (cur_pool_idx <= baddr.end_pool_idx) {
                    pidx = cur_pool_idx;
                    pool = &(pi->pools[key->stage][key->pipe][pidx]);

                    /* Calculate start bit and number of bits in the pool */
                    if (cur_pool_idx == baddr.start_pool_idx) {
                        start_bit = baddr.start_section_idx;
                        num_bits = num_sects_pool - baddr.start_section_idx;
                    } else if (cur_pool_idx == baddr.end_pool_idx) {
                        start_bit = 0;
                        num_bits = baddr.end_section_idx + 1;
                    } else {
                        start_bit = 0;
...
exit:
    if (!SHR_FUNC_ERR()) {
        action->start_pool_idx = baddr.start_pool_idx;
        action->end_pool_idx = baddr.end_pool_idx;
        action->start_section_idx = baddr.start_section_idx;
        action->end_section_idx = baddr.end_section_idx;
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Pool assignment success %s\n"
                     ...
                     si->table, mode, act_cfg->hint,
                     baddr.start_pool_idx, baddr.end_pool_idx,
                     baddr.start_section_idx, baddr.end_section_idx));
    }
    SHR_FUNC_EXIT();
}
                       ...
                       ltsw_flexctr_action_t *action)
{
    ...
    bcm_flexctr_hint_type_t hint_type;
    ...
    if (hint_type == bcmFlexctrHintSharedBlock) {
        SHR_IF_ERR_EXIT
            (ltsw_flexctr_block_detach(unit, action->hint, key));
        SHR_EXIT();
    }
                              ...
                              ltsw_flexctr_action_t *action)
{
    ...
    for (pool_id = action->start_pool_idx;
        ...
        seed_val = ((uint64_t)(pool_id + 1) << 16);
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    for (i = 0; i < ai->op_obj_max[key.stage]; i++) {
        ...
        if (((oi->obj_type == FlexctrObjectTypeMenu) ||
            ...
            if (oi->obj_type == FlexctrObjectTypeQuant) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (ltsw_flexctr_enum_encode(quant_map, quant_count,
                                              id, &object));
            }
...
/*!
 * \brief Insert or update an entry from EFLEX_ACTION_PROFILE LT.
 * This function is used to insert or update an entry from
 ...
 */
static int
ltsw_flexctr_action_set(int unit,
                        ltsw_flexctr_key_t *key,
                        ltsw_flexctr_action_t *action,
                        bcmlt_opcode_t opcode)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_lt_field_validation(unit, st->act_profile,
                                          SKIP_COLLECTIONs,
                                          &lt_field_valid));
    if (lt_field_valid) {
        data = action->flags & BCM_FLEXCTR_ACTION_FLAGS_SKIP_COLLECTION ?
               true : false;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(eh, SKIP_COLLECTIONs, data));
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_commit(unit, eh, opcode,
                              BCMLT_PRIORITY_NORMAL));
                                   ...
                                   bcm_flexctr_value_operation_t *value_op)
{
    ...
    if (value_op->select == bcmFlexctrValueSelectCounterValue) {
        ...
        for (i = 0; i < ai->op_obj_max[stage]; i++) {
            ...
            /*
             * If (i % 2) == 0 (even for right shift 0/2/4):
             * - FLEXCTR_OP_SHIFT_RIGHT_MAX
             * If (i % 2) == 1 (odd for left shift 1/3):
             * - FLEXCTR_OP_SHIFT_LEFT_MAX
             */
            if ((i % 2) == 0) {
                if (value_op->shift[i] > FLEXCTR_OP_SHIFT_RIGHT_MAX) {
                    LOG_ERROR(BSL_LOG_MODULE,
                              (BSL_META_U(unit, "Invalid right shift[%d] %u "
                                                "for value\n"),
                               i, value_op->shift[i]));
                    SHR_ERR_EXIT(SHR_E_PARAM);
                }
            } else {
                if (value_op->shift[i] > FLEXCTR_OP_SHIFT_LEFT_MAX) {
                    LOG_ERROR(BSL_LOG_MODULE,
                              (BSL_META_U(unit, "Invalid left shift[%d] %u "
                                                "for value\n"),
                               i, value_op->shift[i]));
                    SHR_ERR_EXIT(SHR_E_PARAM);
                }
        ...
        for (j = i; j < ai->op_shift_max[stage]; j++) {
            ...
            /*
             * If (j % 2) == 0 (even for right shift 0/2/4):
             * - FLEXCTR_OP_SHIFT_RIGHT_MAX
             * If (j % 2) == 1 (odd for left shift 1/3):
             * - FLEXCTR_OP_SHIFT_LEFT_MAX
             */
            if ((j % 2) == 0) {
                if (value_op->shift[j] > FLEXCTR_OP_SHIFT_RIGHT_MAX) {
                    LOG_ERROR(BSL_LOG_MODULE,
                              (BSL_META_U(unit, "Invalid right shift[%d] %u "
                                                "for value\n"),
                               j, value_op->shift[j]));
                ...
                if (value_op->shift[j] > FLEXCTR_OP_SHIFT_LEFT_MAX) {
                    LOG_ERROR(BSL_LOG_MODULE,
                              (BSL_META_U(unit, "Invalid left shift[%d] %u "
                                                "for value\n"),
                               j, value_op->shift[j]));
                                    ...
                                    ltsw_flexctr_key_t *key)
{
    ...
    for (i = 0; i < BCM_FLEXCTR_OPERATION_OBJECT_SIZE; i++) {
        if (i >= ai->op_obj_max[stage]) {
            if (index_op->object[i] != bcmFlexctrObjectConstZero) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit, "Invalid object %d for "
    ...
    for (i = 0; i < ai->op_obj_max[stage]; i++) {
         ...
         */
        if ((i % 2) == 0) {
            if (index_op->shift[i] > FLEXCTR_OP_SHIFT_RIGHT_MAX) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit, "Invalid right shift[%d] %u "
                                            "for index operand\n"),
                           i, index_op->shift[i]));
            ...
            if (index_op->shift[i] > FLEXCTR_OP_SHIFT_LEFT_MAX) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit, "Invalid left shift[%d] %u "
                                            "for index operand\n"),
                           i, index_op->shift[i]));
    ...
    for (j = i; j < ai->op_shift_max[stage]; j++) {
         ...
         */
        if ((j % 2) == 0) {
            if (index_op->shift[j] > FLEXCTR_OP_SHIFT_RIGHT_MAX) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "Invalid right shift[%d] %u "
                                      "for index operand\n"),
                           j, index_op->shift[j]));
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
        } else {
            if (index_op->shift[j] > FLEXCTR_OP_SHIFT_LEFT_MAX) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "Invalid left shift[%d] %u "
                                      "for index operand\n"),
                           j, index_op->shift[j]));
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
                                  ...
                                  uint32_t *stat_counter_id)
{
    ...
    action->hint_ext = act_cfg->hint_ext;
    ...
    action->hint_type = act_cfg->hint_type;
    if (action->flags & BCM_FLEXCTR_ACTION_FLAGS_POOL_ID) {
        action->start_pool_idx = act_cfg->base_pool_id;
        action->start_section_idx = act_cfg->base_index /
            pi->nctrs_in_sect[nkey.stage];
    }
    ...
    if (flexctr_eviction_enabled[unit]) {
        ...
        if (act_cfg->source == (bcm_flexctr_source_t)bcmiFlexctrSourceL2Dst ||
            act_cfg->source == (bcm_flexctr_source_t)bcmiFlexctrSourceL2Src ||
            act_cfg->source == bcmFlexctrSourceExactMatchFlowtracker ||
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_action_set(unit, &nkey, action, BCMLT_OPCODE_INSERT));
 ...
 */
static int
ltsw_flexctr_action_get_by_id(int unit,
                              uint32_t stat_counter_id,
                              bcm_flexctr_action_t *act_cfg)
{
    ltsw_flexctr_key_t key = {0};
    ...
    act_cfg->base_pool_id = action->start_pool_idx;
    act_cfg->base_index = action->start_section_idx *
        pi->nctrs_in_sect[key.stage];
                        ...
                        bcm_flexctr_action_t *act_cfg)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_action_get_by_id(unit, stat_counter_id, act_cfg));
exit:
 ...
 * \brief Flex counter action parameter validation function.
 *
 * This function is used to validate action parameters during update op.
 *
 * \param [in] unit Unit number.
 * \param [in] old_action Old flex counter action data structure.
 * \param [in] stat_counter_id Flex counter ID.
 * \param [in] act_cfg New flex counter action data structure.
 * \param [in] nkey Flex counter key identification.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_action_update_params_validate(int unit,
                                           ltsw_flexctr_action_t *old_action,
                                           uint32_t stat_counter_id,
                                           bcm_flexctr_action_t *act_cfg,
                                           ltsw_flexctr_key_t *nkey)
{
    int old_pipe;
    SHR_FUNC_ENTER(unit);

    if (old_action->source != act_cfg->source) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Not allow to update source from %d "
                     "to %d!\n"), old_action->source, act_cfg->source));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    old_pipe= FLEXCTR_ID_PIPE(stat_counter_id);
    if (old_pipe != nkey->pipe) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Not allow to update pipe from %d "
                     "to %d!\n"), old_pipe, nkey->pipe));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (old_action->mode != act_cfg->mode) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Not allow to update mode from %d "
                     "to %d!\n"), old_action->mode, act_cfg->mode));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
 ...
 *
 * This function is used to update a flex counter action.
 *
 * \param [in] unit Unit number.
 * \param [in] options BCM_FLEXCTR_OPTIONS_XXX options.
 * \param [in] act_cfg Flex counter action data structure.
 ...
 */
static int
ltsw_flexctr_action_single_update(int unit, int options,
                                  bcm_flexctr_action_t *act_cfg,
                                  bool global,
                                  int pipe,
                                  uint32_t stat_counter_id)
{
    ltsw_flexctr_action_t *action = NULL, *old_action = NULL;
    bcm_flexctr_trigger_t *trigger = NULL;
    bcm_flexctr_index_operation_t *index_op = NULL;
    bcm_flexctr_value_operation_t *value_op = NULL;
    ltsw_flexctr_key_t nkey = {0};
    bcm_flexctr_action_t *old_act_cfg = NULL;
    int i;
    bool realloc = false, rollback = false;
    int action_type = FC_ACTION_TYPE_SIMPLE;
    bool evict_ena = false;
    ltsw_flexctr_pool_info_t *pi = flexctr_pool_info[unit];
    bool index_op_updated = false, value_a_updated = false;
    bool value_b_updated = false;

    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(act_cfg, SHR_E_PARAM);
    SHR_ALLOC(old_action, sizeof(ltsw_flexctr_action_t), "ltswFlexctrAction");
    SHR_NULL_CHECK(old_action, SHR_E_PARAM);
    SHR_ALLOC(old_act_cfg, sizeof(bcm_flexctr_action_t), "FlexctrAction");
    SHR_NULL_CHECK(old_act_cfg, SHR_E_PARAM);
    ...
    /* Fetch the old counter action from flex counter ID. */
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_counter_id_to_action(unit, stat_counter_id,
                                           &action, NULL));
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_action_get_by_id(unit, stat_counter_id, old_act_cfg));
    sal_memcpy(old_action, action, sizeof(ltsw_flexctr_action_t));
    nkey.id = FLEXCTR_ID_INDEX(stat_counter_id);

    /* Validate action parameters for update op. */
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_action_update_params_validate(unit, old_action,
                                                    stat_counter_id,
                                                    act_cfg, &nkey));
    rollback = true;

    /* Save new action information. */
    action->options = options;
    action->flags = act_cfg->flags;
    action->source = act_cfg->source;
    action->hint_type = act_cfg->hint_type;
    ...
    if (act_cfg->mode == bcmFlexctrCounterModeNormal) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_flexctr_action_value_operation_process(unit, &nkey,
                                                         FC_VALUE_B_TRUE,
                                                         value_op, action));
        value_b_updated = true;
    }

    /* Check if need to reallocate counter resource. */
    if (old_action->index_num != action->index_num) {
        realloc = true;
    } else if (action->flags & BCM_FLEXCTR_ACTION_FLAGS_POOL_ID) {
        if (old_action->start_pool_idx != action->start_pool_idx ||
            old_action->start_section_idx != action->start_section_idx) {
            realloc = true;
        }
    }

    if (realloc) {
        /* Free old action's counter pool first. */
        (void)ltsw_flexctr_pool_free(unit, &nkey, old_action);
    ...
    /* Update flex counter action LT. */
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_action_set(unit, &nkey, action, BCMLT_OPCODE_UPDATE));
...
exit:
    if (SHR_FUNC_ERR() && rollback) { /* Rollback. */
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Start rollback\n")));
        /* Process action trigger. */
        if (old_act_cfg->flags & BCM_FLEXCTR_ACTION_FLAGS_TRIGGER) {
            trigger = &old_act_cfg->trigger;
            (void)ltsw_flexctr_action_trigger_process(unit, &nkey, trigger,
                                                      old_action);
        }
        ...
        if (index_op_updated) {
            (void)ltsw_flexctr_op_profile_entry_remove(unit,
                                                       action->index_profile);
            index_op = &old_act_cfg->index_operation;
            (void)ltsw_flexctr_action_index_operation_process(unit, &nkey,
                                                              index_op,
                                                              old_action);
        }
        ...
        if (value_a_updated) {
            (void)ltsw_flexctr_op_profile_entry_remove(unit,
                                                       action->value_a_profile);
            value_op = &old_act_cfg->operation_a;
            (void)ltsw_flexctr_action_value_operation_process(unit, &nkey,
                                                              FC_VALUE_A_TRUE,
                                                              value_op,
                                                              old_action);
        }
        ...
        if (value_b_updated) {
            (void)ltsw_flexctr_op_profile_entry_remove(unit,
                                                       action->value_b_profile);
            value_op = &old_act_cfg->operation_b;
            (void)ltsw_flexctr_action_value_operation_process(unit, &nkey,
                                                              FC_VALUE_B_TRUE,
                                                              value_op,
                                                              old_action);
        ...
        if (realloc) {
            (void)ltsw_flexctr_pool_free(unit, &nkey, action);
            (void)ltsw_flexctr_pool_alloc(unit, &nkey, old_act_cfg, old_action);
        }

        if (evict_ena) {
            /* Set pool control LT. */
            (void)ltsw_flexctr_pool_control_set(unit, &nkey, old_action);
        }
        ...
        /* Update flex counter action LT. */
        (void)ltsw_flexctr_action_set(unit, &nkey, old_action,
                                      BCMLT_OPCODE_UPDATE);

        sal_memcpy(action, old_action, sizeof(ltsw_flexctr_action_t));
    } else if (!SHR_FUNC_ERR()) { /* Success. */
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Clean up old action resources!\n")));

        (void)ltsw_flexctr_op_profile_entry_remove(unit,
                                                   old_action->index_profile);

        (void)ltsw_flexctr_op_profile_entry_remove(unit,
                                                   old_action->value_a_profile);

        (void)ltsw_flexctr_op_profile_entry_remove(unit,
                                                   old_action->value_b_profile);

        /* Last update old quantization reference count. */
        for (i = 0; i < FC_ACTION_OPOBJ_COUNT; i++) {
            ltsw_flexctr_quant_t *quant = NULL;

            if (old_action->quant_id[i] == FC_ID_INVALID) {
                continue;
            }

            (void)ltsw_flexctr_quant_id_to_quant(unit, old_action->quant_id[i],
                                                 &quant, NULL, NULL);
            quant->ref_count--;
        }

        /* Last update old packet attribute profile reference count. */
        for (i = 0; i < FC_ACTION_OBJ_COUNT; i++) {
            if ((old_action->objects[i] ==
                 bcmFlexctrObjectStaticIngPktAttribute) ||
                (old_action->objects[i] ==
                 bcmFlexctrObjectStaticEgrPktAttribute)) {
                ltsw_flexctr_pkt_attr_t *pkt_attr = NULL;

                (void)ltsw_flexctr_profile_id_to_pkt_attr(unit,
                         old_action->objects_id[i],
                         &pkt_attr, NULL);
                pkt_attr->ref_count--;
            }
        }
...
/*!
 * \brief Flex counter global action update function.
 *
 * This function is used to update a flex counter global action in pipe
 * unique mode, and the action on each pipe shares the same configuration.
 *
 * \param [in] unit Unit number.
 * \param [in] options BCM_FLEXCTR_OPTIONS_XXX options.
 ...
 */
static int
ltsw_flexctr_action_global_update(int unit, int options,
                                  bcm_flexctr_action_t *act_cfg,
                                  uint32_t stat_counter_id)
{
    int i, pipe_num, stage;
    bcmint_flexctr_source_info_t *si = NULL;
    uint32_t counter_id_tmp;
    ltsw_flexctr_key_t key = {0};
    ...
    for (i = 0; i < pipe_num; i++) {
        key.pipe = i;
        FLEXCTR_ID_COMPOSE(&key, counter_id_tmp);

        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_flexctr_action_single_update(unit, options, act_cfg,
                                               true, i, counter_id_tmp));
    }
                           ...
                           uint32_t *stat_counter_id)
{
    ...
    if (act_cfg->flags & BCM_FLEXCTR_ACTION_FLAGS_GLOBAL) {
        if ((options & BCM_FLEXCTR_OPTIONS_REPLACE) &&
            (options & BCM_FLEXCTR_OPTIONS_WITH_ID)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_flexctr_action_global_update(unit,
                                                   options,
                                                   act_cfg,
                                                   *stat_counter_id));
        } else {
        ...
        if ((options & BCM_FLEXCTR_OPTIONS_REPLACE) &&
            (options & BCM_FLEXCTR_OPTIONS_WITH_ID)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_flexctr_action_single_update(unit,
                                                   options,
                                                   act_cfg,
                                                   false,
                                                   0,
...
ltsw_flexctr_stats_force_sync(int unit)
{
     ...
     */
    rv = bcmi_ltsw_stat_sync_with_type(unit, BCMI_LTSW_COUNTER_TYPE_EVICT);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
...
/*
 * Bulk counter sync using CTR_EVENT_SYNC_STATE_CONTROL, this uses SBUSDMA.
 * But LT op takes time too, so its not easy to decide when to use bulk sync.
 * This is an experimential value.
 */
#define FLEXCTR_EXPLICIT_SYNC_THRESHOLD 4000
                           ...
                           bcm_flexctr_counter_value_t *counter_values)
{
    ...
    if (LOG_CHECK(BSL_LOG_MODULE | BSL_INFO)) {
        time[0] = sal_time_usecs();
    }
    ...
    if (action->source == bcmFlexctrSourceExactMatch) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm_ltsw_flexctr_action_get(unit,
                                         stat_counter_id,
                                         &act_cfg));
        if (act_cfg.index_operation.object[0] ==
    ...
    if ((operation == FLEXCTR_OPERATION_GET) && sync) {
        if (action->evict_ena) {
            /* Eviction pool doesn't need explicit sync since CCI does it already. */
            hw_sync = false;
        } else if (num_entries >= FLEXCTR_EXPLICIT_SYNC_THRESHOLD) {
            /* Try a LT sync for bulk op, but do not want it block API calling. */
            rv = ltsw_flexctr_stats_force_sync(unit);
            if (SHR_SUCCESS(rv)) {
                hw_sync = false;
            }
        }
        ...
        switch (operation) {
            ...
            case FLEXCTR_OPERATION_SET:
                if (action->global_rsvd) {
                    ...
                    SHR_IF_ERR_VERBOSE_EXIT
                        (ltsw_flexctr_stats_update(unit, &key, index,
                                                   action->mode,
                                                   value));
                }
                break;
    ...
    if (LOG_CHECK(BSL_LOG_MODULE | BSL_INFO)) {
        time[1] = sal_time_usecs();
        LOG_INFO(BSL_LOG_MODULE,
                 (BSL_META_U(unit, "ltsw_flexctr_stats_get_set time:%d us\n"),
                  time[1] - time[0]));
    }
...
ltsw_flexctr_packet_attribute_db_init(int unit)
{
    ...
    uint32_t ha_alloc_size = 0, ha_req_size = 0, pipe_req_size = 0;
    uint32_t offset = 0;
    uint32_t issu_struct_size = 0, issu_array_size = 0;
    void *alloc_ptr = NULL;
    int warm = bcmi_warmboot_get(unit), stage, max_pipe, i, rv;
    ...
    for (stage = FC_MIN_STAGE; stage < FC_MAX_STAGE; stage++) {
        ...
        issu_struct_size = sizeof(ltsw_flexctr_pkt_attr_t);
        issu_array_size = pai->pkt_attr_max[stage] * max_pipe;
        if (warm) {
            rv = bcmi_ltsw_issu_struct_info_get(unit,
                                                LTSW_FLEXCTR_PKT_ATTR_T_ID,
                                                BCMI_HA_COMP_ID_FLEXCTR,
                                                st_info[stage].ha_pkt_attr_id,
                                                &offset);
           if (rv == SHR_E_NOT_FOUND) {
               rv = bcmi_ltsw_issu_struct_info_report(unit,
                                         BCMI_HA_COMP_ID_FLEXCTR,
                                         st_info[stage].ha_pkt_attr_id,
                                         0, issu_struct_size, issu_array_size,
                                         LTSW_FLEXCTR_PKT_ATTR_T_ID);
                SHR_IF_ERR_VERBOSE_EXIT(rv);
            } else {
                alloc_ptr = (void *)((uint8_t *)alloc_ptr + offset);
            }
    ...
    ltsw_flexctr_key_t *key)
{
    ...
    if (num_selectors <= 0 ||
        num_selectors > BCMINT_LTSW_FLEXCTR_PKT_ATTR_SELECTOR_NUM_MAX) {
        LOG_ERROR(BSL_LOG_MODULE,
    ...
    int *profile_id)
{
    ltsw_flexctr_pkt_attr_info_t *pai = flexctr_pkt_attr_info[unit];
    ltsw_flexctr_pkt_attr_t *pkt_attr = NULL;
    bcmint_ltsw_flexctr_pkt_attr_obj_inst_t *obj_inst = NULL;
    ltsw_flexctr_key_t nkey = {0}, okey = {0};
    uint32_t min_id, max_id;
    int i;
...
ltsw_flexctr_gen2_detach(int unit)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_block_db_cleanup(unit));
...
ltsw_flexctr_gen2_init(int unit)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_block_db_init(unit));
...
/*!
 * \brief Flex counter hit index compact control function.
 *
 * This function is used to update hit control LT.
 *
 * \param [in] unit Unit number.
 * \param [in] ctrl Flex counter hit index compact control info.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_hitidx_compact_ctrl(
    int unit,
    bcmi_ltsw_flexctr_hitidx_compact_ctrl_t *ctrl)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    int rv, i;
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    const char *profile_id[] = {L2_SRC_LOOKUP_COMPACT_HIT_INDEXs,
                                L2_DST_LOOKUP_COMPACT_HIT_INDEXs,
                                EM_FT_COMPACT_HIT_INDEXs};
    const char *mux_sel[] = {
            MUX0_SEL_0s, MUX0_SEL_1s, MUX0_SEL_2s, MUX0_SEL_3s,
            MUX0_SEL_4s, MUX0_SEL_5s, MUX0_SEL_6s, MUX0_SEL_7s,
            MUX0_SEL_8s, MUX0_SEL_9s, MUX0_SEL_10s, MUX0_SEL_11s,
            MUX0_SEL_12s, MUX0_SEL_13s, MUX0_SEL_14s, MUX0_SEL_15s,
            MUX0_SEL_16s, MUX0_SEL_17s, MUX0_SEL_18s, MUX0_SEL_19s,
            MUX0_SEL_20s, MUX0_SEL_21s, MUX0_SEL_22s, MUX0_SEL_23s,
            MUX0_SEL_24s, MUX0_SEL_25s, MUX0_SEL_26s, MUX0_SEL_27s,
            MUX0_SEL_28s, MUX0_SEL_29s, MUX0_SEL_30s, MUX0_SEL_31s};
    uint64_t val;
    ...
    if (ctrl->op < 0 || ctrl->op >= bcmiFlexCtrHitIdxOpCount) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (ctrl->hit_obj < 0 || ctrl->hit_obj >= bcmiFlexCtrHitIdxObjCount) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_allocate(dunit,
                              CTR_EFLEX_HIT_INDEX_COMPACT_PROFILEs, &eh));

    /* Key fields */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_symbol_add(eh,
                                      CTR_EFLEX_HIT_INDEX_COMPACT_PROFILE_IDs,
                                      profile_id[ctrl->hit_obj]));

    if (ctrl->op == bcmiFlexCtrHitIdxOpGet) {
        ...
        for (i = 0; i < BCMI_LTSW_FLEXCTR_HITIDX_BIT_MAP_MAX; i++) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_get(eh, mux_sel[i], &val));
            ctrl->bit_map[i] = (uint8_t)val;
        }
        SHR_EXIT();
    }
    if (ctrl->op == bcmiFlexCtrHitIdxOpSet) {
        for (i = 0; i < BCMI_LTSW_FLEXCTR_HITIDX_BIT_MAP_MAX; i++) {
            val = ctrl->bit_map[i];
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(eh, mux_sel[i], val));
        }
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_entry_set_commit(unit, eh, BCMLT_PRIORITY_NORMAL));
    }
    if (ctrl->op == bcmiFlexCtrHitIdxOpDelete) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_entry_commit(unit, eh, BCMLT_OPCODE_DELETE,
                                  BCMLT_PRIORITY_NORMAL));
    }
...
/*!
 * \brief Update flexctr action index object_id.
 *
 * This function is used to update flexctr action index object_id.
 *
 * \param [in] unit             Unit number.
 * \param [in] stat_counter_id  Flex counter ID.
 * \param [in] idx_op           Index operation.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
ltsw_flexctr_action_object_id_single_update(
    int unit,
    uint32_t stat_counter_id,
    bcm_flexctr_index_operation_t *idx_op)
{
    ltsw_flexctr_key_t key = {0};
    ltsw_flexctr_action_t *action = NULL;
    ltsw_flexctr_action_info_t *ai = flexctr_action_info[unit];
    int i, j, k = 0, updated = 0;
    int dunit = bcmi_ltsw_dev_dunit(unit);
    ltsw_flexctr_stage_info_t *st;
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    uint64_t data;
    const char *symbol = NULL;
    bcmint_flexctr_object_info_t *oi = NULL;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_counter_id_to_action(unit, stat_counter_id, &action, &key));
    st = &(st_info[key.stage]);

    /* Flexctr action profile is not managed in profile in current hsdk logic.
     * So update object_id will only affect itself.
     * mask/shift/quant_id/object can not change for it affects op profile.
     */
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit, "Counter action lt update %d %d %u\n"),
              key.stage, key.pipe, key.id));
    ...
    for (i = 0; i < ai->op_obj_max[key.stage]; i++) {
        j = FC_INDEX_OP * ai->op_obj_max[key.stage] + i;
        /*Object is not allowed to update.*/
        if (idx_op->object[i] != action->object[j]) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
        if (idx_op->object[i] != FLEXCTR_OBJ_INVALID) {
            SHR_IF_ERR_VERBOSE_EXIT
                (mbcm_ltsw_flexctr_object_info_get(unit,
                                                   idx_op->object[i], &oi));
            /* Only update of MenuId type is supported.*/
            if (oi->obj_type == FlexctrObjectTypeMenuId) {
                /* Update sw info.*/
                action->object_id[j] = idx_op->object_id[i];
                for (; k < ai->op_obj_max[key.stage]; k++) {
                    if (idx_op->object[i] == action->objects[k]) {
                        action->objects_id[k] = idx_op->object_id[i];
                        /* Update HW Field.*/
                        SHR_IF_ERR_VERBOSE_EXIT
                            (mbcm_ltsw_flexctr_object_enum_get
                                             (unit, action->objects[k],
                                              action->objects_id[k], &symbol));
                        SHR_IF_ERR_VERBOSE_EXIT
                            (bcmlt_entry_field_array_symbol_add(eh, OBJs,
                                                                k, &symbol, 1));
                        k++;
                        updated = 1;
                        break;
 ...
 */
static int
ltsw_flexctr_action_object_id_update(
    int unit,
    uint32_t stat_counter_id,
    bcm_flexctr_index_operation_t *idx_op)

{
    ...
    /* Flexctr action profile is not managed in profile in current hsdk logic.
     * This function udpates object_id only.
     * mask/shift/quant_id/object will not change for it affects op profile.
     */
     FLEXCTR_ACTION_LOCK(unit);

    /* For group type, update each member as they may not share same idx_op.*/
    if (FLEXCTR_ID_TYPE(stat_counter_id) != FLEXCTR_ACTION) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    if (info.single_pipe) {
         SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_flexctr_action_object_id_single_update(unit,
                                                         stat_counter_id,
                                                         idx_op));
    } else {
        pipe_num = flexctr_pipe_max[unit][info.stage];
        FLEXCTR_ID_DECOMPOSE(stat_counter_id, &key);
        for (i = 0; i < pipe_num; i++) {
            key.pipe = i;
            FLEXCTR_ID_COMPOSE(&key, counter_id_tmp);
    ...
    for (stage = FC_MIN_STAGE; stage < FC_MAX_STAGE; stage++) {
        ...
        for (pipe = 0; pipe < max_pipe; pipe++) {
            ...
            for (pidx = 0; pidx < pi->npools[stage]; pidx++) {
                ...
                LOG_CLI((BSL_META_U(unit, "\t\tPool %2d: "),
                        pidx));
                if (pool->shared) {
                    LOG_CLI((BSL_META_U(unit,
                                        "Reserved by shared block 0x%x\n"),
                            pool->hint));
                } else {
                    LOG_CLI((BSL_META_U(unit, "Mode: %d"
                                        " Hint: %d Use_count: %u\n"),
                            pool->mode, pool->hint,
                    ...
                    SHR_BIT_ITER(pool->used_by_tbls, FLEXCTR_NUM_TBLS_MAX, i) {
                        LOG_CLI((BSL_META_U(unit, "%2d(ref_cnt=%2d) "),
                                 i, pool->ref_count[i]));
                    }
                    LOG_CLI((BSL_META_U(unit, "\n")));
...
/*!
 * \brief Get flex counter IDs by given pool info.
 *
 * \param [in] unit Unit number.
 * \param [in] pool_info Flex counter pool info.
 * \param [in] array_size Number of elements in stat counter ID array, 0 to query.
 * \param [out] counter_id_array Stat counter ID array.
 * \param [out] count The actual number of stat counter ID.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
ltsw_flexctr_pool_counter_id_multi_get(int unit,
                                       bcm_flexctr_pool_info_t *pool_info,
                                       int array_size,
                                       uint32_t *counter_id_array,
                                       int *count)
{
    ltsw_flexctr_action_info_t *ai = flexctr_action_info[unit];
    ltsw_flexctr_action_t *action = NULL;
    ltsw_flexctr_pool_info_t *pi = flexctr_pool_info[unit];
    ltsw_flexctr_key_t key = {0};
    int flex_stage, pipe = 0;
    uint32_t min_id, max_id;
    int max_pipe, stat_counter_id, cnt = 0;
    SHR_FUNC_ENTER(unit);

    FLEXCTR_ACTION_LOCK(unit);

    SHR_NULL_CHECK(count, SHR_E_PARAM);
    SHR_NULL_CHECK(pool_info, SHR_E_PARAM);
    if (array_size != 0) {
        /* Zero is to query the array size and array is set to NULL. */
        SHR_NULL_CHECK(counter_id_array, SHR_E_PARAM);
    } else if (array_size < 0) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Incorrect array size %d!\n"),
                     array_size));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    /* Get stage. */
    if (pool_info->stage == bcmFlexctrDirectionIngress) {
        flex_stage = BCMI_LTSW_FLEXCTR_STAGE_ING_CTR;
    } else if (pool_info->stage == bcmFlexctrDirectionEgress) {
        flex_stage = BCMI_LTSW_FLEXCTR_STAGE_EGR_CTR;
    } else {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Incorrect stage %d!\n"),
                     pool_info->stage));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    /* Get pipe. */
    if (flexctr_pipe_unique[unit][flex_stage]) {
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_flexctr_config_port_to_pipe(unit, pool_info->ports, &pipe));
        max_pipe = flexctr_pipe_max[unit][flex_stage];

        if (pipe >= max_pipe) {
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit, "Incorrect pipe %d!\n"), pipe));
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    } else {
        if (BCM_PBMP_NOT_NULL(pool_info->ports)) {
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit, "Incorrect ports!\n")));
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    }

    /* Validate pool id. */
    if (pool_info->pool_id >= pi->npools[flex_stage]) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Incorrect pool id %d!\n"),
                     pool_info->pool_id));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    key.type = FLEXCTR_ACTION;
    key.stage = flex_stage;
    key.pipe = pipe;
    min_id = ai->action_min[flex_stage];
    max_id = ai->action_max[flex_stage];
    ...
    for (key.id = min_id; key.id < max_id; key.id++) {
        ...
        if (pool_info->pool_id >= action->start_pool_idx &&
            pool_info->pool_id <= action->end_pool_idx) {
            if (counter_id_array && (cnt < array_size)) {
                counter_id_array[cnt] = stat_counter_id;
            }
            cnt++;
        }
    }
...
static int
ltsw_flexctr_object_info_get(int unit,
                                bcm_flexctr_object_t object,
                                bcm_flexctr_object_info_t *info)
{
    ...
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    int dunit = bcmi_ltsw_dev_dunit(unit);
    uint64_t min_bit, max_bit;
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexctr_object_info_get(unit, object, &oi));
    if (oi->stage >= BCMI_LTSW_FLEXCTR_STAGE_COUNT) {
        SHR_ERR_EXIT(SHR_E_DISABLED);
    }
    st = &(st_info[oi->stage]);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_allocate(dunit, st->obj_info, &eh));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_symbol_add(eh, OBJs, oi->symbol));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_commit(unit, eh, BCMLT_OPCODE_LOOKUP,
                              BCMLT_PRIORITY_NORMAL));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(eh, MIN_BITs, &min_bit));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(eh, MAX_BITs, &max_bit));

    info->offset = COMPILER_64_LO(min_bit);
    info->width = COMPILER_64_LO(max_bit) - COMPILER_64_LO(min_bit) + 1;
...
int
bcmi_ltsw_flexctr_flow_count_set(int unit, int val)
{
    if (ltsw_feature(unit, LTSW_FT_FLEXCTR_GEN2)) {
        ...
        return mbcm_ltsw_flexctr_flow_count_set(unit, val);
    } else {
        return SHR_E_UNAVAIL;
    }
}
...
int
bcmi_ltsw_flexctr_flow_count_get(int unit, int *val)
{
    if (ltsw_feature(unit, LTSW_FT_FLEXCTR_GEN2)) {
        ...
        return mbcm_ltsw_flexctr_flow_count_get(unit, val);
    } else {
        return SHR_E_UNAVAIL;
    }
}
...
int
bcmi_ltsw_flexctr_hitidx_compact_ctrl(
    int unit,
    bcmi_ltsw_flexctr_hitidx_compact_ctrl_t *ctrl)
{
    SHR_FUNC_ENTER(unit);
    ...
    if (ltsw_feature(unit, LTSW_FT_FLEXCTR_GEN2)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_flexctr_hitidx_compact_ctrl(unit, ctrl));
    } else {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
...
int
bcm_ltsw_flexctr_block_create(int unit,
                              ...
                              uint32 *block_id)
{
    ...
    if (!flexctr_initialized[unit]) {
        SHR_ERR_EXIT(SHR_E_INIT);
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_block_create(unit, block, block_id));
...
int
bcm_ltsw_flexctr_block_destroy(int unit,
                               uint32 block_id)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_block_destroy(unit, block_id));
...
int
bcm_ltsw_flexctr_block_get(int unit,
                           uint32 block_id,
                           bcm_flexctr_block_t *block)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_block_get(unit, block_id, block));
...
int
bcm_ltsw_flexctr_block_traverse(int unit,
                                bcm_flexctr_block_traverse_cb trav_fn,
                                void *user_data)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_flexctr_block_traverse(unit, trav_fn, user_data));
    ...
    bcm_flexctr_index_operation_t *idx_op)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_FLEXCTR_GEN2)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_flexctr_action_object_id_update(unit,
                                                  stat_counter_id, idx_op));
    } else {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
...
int
bcm_ltsw_flexctr_pool_counter_id_multi_get(int unit,
                                           bcm_flexctr_pool_info_t *pool_info,
                                           int array_size,
                                           ...
                                           int *count)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_FLEXCTR_GEN2)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_flexctr_pool_counter_id_multi_get(unit,
                                                    pool_info,
                                                    array_size,
                                                    counter_id_array,
                                                    count));
...
int
bcm_ltsw_flexctr_object_info_get(int unit,
                                      bcm_flexctr_object_t object,
                                      bcm_flexctr_object_info_t *info)
{
    SHR_FUNC_ENTER(unit);
    ...
    if (ltsw_feature(unit, LTSW_FT_FLEXCTR_GEN2)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (ltsw_flexctr_object_info_get(unit, object, info));
    } else {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
