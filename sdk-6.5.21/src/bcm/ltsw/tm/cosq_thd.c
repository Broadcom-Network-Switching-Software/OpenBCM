                                      ...
                                      int buffer_id, int spid)
{
    ...
    } else if (sal_strcasecmp(op_state, INCORRECT_SHARED_LIMITs) == 0) {
        LOG_ERROR(BSL_LOG_MODULE,
            (BSL_META_U(unit,
                "ITM %d EGR service pool %d threshold update error: %s\n"),
                buffer_id, spid, op_state));
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_RESOURCE);
    } else if ((sal_strcasecmp(op_state, INCORRECT_RESUME_LIMITs) == 0) ||
               (sal_strcasecmp(op_state, INCORRECT_COLOR_LIMITs) == 0)) {
        LOG_ERROR(BSL_LOG_MODULE,
            (BSL_META_U(unit,
                "ITM %d EGR service pool %d threshold update error: %s\n"),
                buffer_id, spid, op_state));
        ...
        case COSQ_THD_INDEX_STYLE_EGR_POOL:
            if (hierarchical) {
                ...
                if (is_cpu_port(unit, resolved_port)) {
                    ...
                    if (startq >= num_uc) {
                        resolved_index = startq - num_uc;
                    } else {
                        resolved_index = startq;
                    }
                }

                if (is_cpu_port(unit, resolved_port) || (startq >= num_uc)) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (cosq_thd_port_mc_spid_get(unit, resolved_port,
                                                          resolved_index,
                                                          &resolved_id, NULL));
        ...
        case COSQ_THD_INDEX_STYLE_MCAST_QUEUE:
            if (hierarchical) {
                ...
                if (is_cpu_port(unit, resolved_port)) {
                    ...
                    if ((startq < num_uc) || (startq >= numq)) {
                        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                    }
                    resolved_id = startq - num_uc;
                }
            }
            break;
        default:
 ...
 */
static int
cosq_thd_table_state_check(int unit, const char *lt_val,
                           const char *key, int key_value,
                           const char *key2, int key2_value,
                           const char *op_state)
{
    SHR_FUNC_ENTER(unit);
    ...
    if (sal_strcasecmp(op_state, VALIDs) == 0) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_NONE);
    } else if ((sal_strcasecmp(op_state, INCORRECT_MIN_GUARANTEEs) == 0) ||
               (sal_strcasecmp(op_state, INCORRECT_HEADROOM_LIMITs) == 0) ||
               (sal_strcasecmp(op_state, INCORRECT_SHARED_LIMITs) == 0)) {
        LOG_ERROR(BSL_LOG_MODULE,
            (BSL_META_U(unit,
                   "lt %s update %s=%d %s=%d update error OPERATIONAL_STATE=%s\n"),
                    lt_val,
                    (key != NULL) ? key : " ", key_value,
                    (key2 != NULL) ? key2 : " ", key2_value,
                    op_state));
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_RESOURCE);
    } else if ((sal_strcasecmp(op_state, INCORRECT_RESUME_LIMITs) == 0) ||
               (sal_strcasecmp(op_state, INCORRECT_COLOR_LIMITs) == 0)) {
                   ...
                   bool check_state)
{
    ...
    if (!check_state) {
        ...
        if (rv != SHR_E_NOT_FOUND) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (cosq_thd_table_state_check(unit, lt_val, key_val, key_value,
                                            key2_val, key2_value, op_state));
        }
    }
    ...
    int *index)
{
    ...
    if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport)) {
        ...
        *flags = MC;
        ...
        if (is_cpu_port(unit, lport) || (cosq >= device_info.num_ucq)) {
            SHR_IF_ERR_VERBOSE_EXIT
