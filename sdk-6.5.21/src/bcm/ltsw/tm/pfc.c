...
/* PFC deadlock state. */
typedef enum pfc_deadlock_recovery_state_e {
    /* Normal state. */
    PFC_STATE_IDLE = 0,

    /* Recovery state. */
    PFC_STATE_RECOVERY = 1,

    /* Deadlock state. */
    PFC_STATE_DEADLOCK = 2,
} pfc_deadlock_recovery_state_t;
...
typedef struct tm_pfc_deadlock_recovery_s {
    /* recover enable. */
    int recover_enable;
...
static int
pfc_deadlock_recovery_init(tm_pfc_deadlock_recovery_t *recovery)
{
    recovery->recover_enable = -1;
    recovery->detect_timer = -1;
    recovery->timer_gran = -1;
    recovery->recover_timer = -1;
    recovery->manual_recovery = -1;
    recovery->recovery_mode = -1;
...
/*
 * \brief TM_PFC_DEADLOCK_RECOVERY_STATE_CONTROLs set.
 *
 * \param [in]  unit        Unit Number.
 ...
 */
static int
pfc_deadlock_recovery_state_control_set(int unit, int lport, int pfc_pri,
                                        int state)
{
    ...
    bcmi_lt_field_t fields[] =
    {
        ...
        /* 2 */ {STATEs,                 BCMI_LT_FIELD_F_SET |
                                         BCMI_LT_FIELD_F_SYMBOL, 0, {0}},
    };

    SHR_FUNC_ENTER(unit);

    if (state != PFC_STATE_IDLE) {
    ...
    fields[2].u.sym_val = IDLEs;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_set(unit, TM_PFC_DEADLOCK_RECOVERY_STATE_CONTROLs,
                           &lt_entry, NULL));
 ...
 */
static int
pfc_deadlock_recovery_manual_state_control_set(int unit, int lport,
                                               int pfc_pri, int state)
{
    bcmi_lt_entry_t lt_entry;
    bcmi_lt_field_t fields[] =
    {
    ...
    fields[0].u.val = lport;
    fields[1].u.val = pfc_pri;
    fields[2].u.sym_val = (state == 1) ? RECOVERYs : IDLEs;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_set(unit, TM_PFC_DEADLOCK_RECOVERY_MANUAL_STATE_CONTROLs,
                           &lt_entry, NULL));
 ...
 */
static int
pfc_deadlock_recovery_set(int unit, int lport, int pfc_pri,
                          tm_pfc_deadlock_recovery_t *recovery)
{
    bcmi_lt_entry_t lt_entry;
    bcmi_lt_field_t fields[] =
    {
        ...
        /*6 */ {MANUAL_RECOVERYs,             0, 0, {0}},
        /*7 */ {RECOVERY_MODEs,               0, 0, {0}},
    };
    ...
    if (recovery->recover_enable != -1) {
        fields[2].flags = BCMI_LT_FIELD_F_SET;
        fields[2].u.val = recovery->recover_enable ? true : false;
    }

    if (recovery->detect_timer != -1) {
        fields[3].flags = BCMI_LT_FIELD_F_SET;
        fields[3].u.val = recovery->detect_timer;
    }

    if (recovery->timer_gran != -1) {
        fields[4].flags = BCMI_LT_FIELD_F_SET | BCMI_LT_FIELD_F_SYMBOL;
        timer_gran = recovery->timer_gran;
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    if (recovery->recover_timer != -1) {
        fields[5].flags = BCMI_LT_FIELD_F_SET;
        fields[5].u.val = recovery->recover_timer;
    }

    if (recovery->manual_recovery != -1) {
        fields[6].flags = BCMI_LT_FIELD_F_SET;
        fields[6].u.val = recovery->manual_recovery;
    }

    if (recovery->recovery_mode != -1) {
        fields[7].flags = BCMI_LT_FIELD_F_SET | BCMI_LT_FIELD_F_SYMBOL;
        fields[7].u.sym_val = (recovery->recovery_mode == 1) ?
                               PFC_STATE_MODEs : TIMER_MODEs;
    }
    ...
    LOG_INFO(BSL_LOG_MODULE,
        (BSL_META_U(unit,
            "lt %s update %s=%d %s=%d %s=%d %s=%d %s=%s %s=%d %s=%d %s=%d\n"),
            TM_PFC_DEADLOCK_RECOVERYs,
 ...
 */
static int
pfc_deadlock_recovery_get(int unit, int lport, int pfc_pri,
                          tm_pfc_deadlock_recovery_t *recovery)
{
    bcmi_lt_entry_t lt_entry;
    bcmi_lt_field_t fields[] =
    {
        ...
        /*6 */ {MANUAL_RECOVERY_OPERs,        BCMI_LT_FIELD_F_GET, 0, {0}},
        /*7 */ {RECOVERY_MODEs,               BCMI_LT_FIELD_F_GET, 0, {0}},
    };
    tm_ltsw_pfc_chip_driver_t *drv = NULL;
    int timer_gran;
    ...
    LOG_INFO(BSL_LOG_MODULE,
        (BSL_META_U(unit,
            "lt %s lookup %s=%d %s=%d %s=%d %s=%d %s=%s %s=%d %s=%d %s=%d\n"),
            TM_PFC_DEADLOCK_RECOVERYs,
            fields[0].fld_name, (int)fields[0].u.val,
            fields[1].fld_name, (int)fields[1].u.val,
            fields[2].fld_name, (int)fields[2].u.val,
    ...
    if (recovery->recover_enable != -1) {
        recovery->recover_enable = fields[2].u.val;
    }

    if (recovery->detect_timer != -1) {
        recovery->detect_timer = fields[3].u.val;
    }
    ...
    if (recovery->timer_gran != -1) {
        ...
        recovery->timer_gran = timer_gran;
    }

    if (recovery->recover_timer != -1) {
        recovery->recover_timer = fields[5].u.val;
    }

    if (recovery->manual_recovery != -1) {
        recovery->manual_recovery = fields[6].u.val;
    }

    if (recovery->recovery_mode != -1) {
        recovery->recovery_mode = fields[7].u.val;
    }
 ...
 */
static int
pfc_deadlock_recovery_count_get(int unit, int port, int prc_pri, uint32_t *cnt)
{
    bcmi_lt_entry_t lt_entry;
    bcmi_lt_field_t fields[] =
    {
        /*0 */ {PORT_IDs,                        BCMI_LT_FIELD_F_SET, 0, {0}},
        /*1 */ {PFC_PRIs,                        BCMI_LT_FIELD_F_SET, 0, {0}},
        /*2 */ {RECOVERY_CNTs,                   BCMI_LT_FIELD_F_GET, 0, {0}},
    };
...
/*
 * \brief Get LT TM_PFC_DEADLOCK_RECOVERY_STATEs.
 *
 * \param [in] unit          Unit Number.
 * \param [in] lport         local port.
 * \param [in] prc_pri       prc pri.
                                 ...
                                 int *recovery)
{
    ...
    bcmi_lt_field_t fields[] =
    {
        /*0 */ {PORT_IDs,                 BCMI_LT_FIELD_F_SET, 0, {0}},
        /*1 */ {PFC_PRIs,                 BCMI_LT_FIELD_F_SET, 0, {0}},
        /*2 */ {STATEs,                   BCMI_LT_FIELD_F_GET |
                                          BCMI_LT_FIELD_F_SYMBOL, 0, {0}},
    };

    tm_pfc_deadlock_recovery_t dd_recovery = {0};

    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_EXIT_EXCEPT_IF
        (pfc_deadlock_recovery_get(unit, port, prc_pri, &dd_recovery),
                                   SHR_E_NOT_FOUND);

    state_table = (dd_recovery.manual_recovery == 0) ? TM_PFC_DEADLOCK_RECOVERY_STATEs :
                                                       TM_PFC_DEADLOCK_RECOVERY_MANUAL_STATEs;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_get(unit, state_table, &lt_entry, NULL, NULL));
    ...
    if (recovery != NULL) {
        if (sal_strcasecmp(fields[2].u.sym_val, DEADLOCKs) == 0) {
            *recovery = 2;
        } else if (sal_strcasecmp(fields[2].u.sym_val, RECOVERYs) == 0) {
            *recovery = 1;
        } else if (sal_strcasecmp(fields[2].u.sym_val, IDLEs) == 0) {
            *recovery = 0;
        }
    }
...
pfc_deadlock_timer_gran_get(
    ...
    int *timer_gran)
{
    tm_pfc_deadlock_recovery_t recovery = {0};
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (pfc_deadlock_recovery_get(unit, port, pfc_priority, &recovery),
                                   SHR_E_NOT_FOUND);
...
static int
pfc_deadlock_event_log(
    int unit,
    pfc_deadlock_event_info_t *event,
    int recovery)
{
    tm_pfc_deadlock_recovery_t dd_recovery = {0};
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "Port %d PFC Priority %d Recovery %d\n"),
              event->port, event->pfc_pri, recovery));

    SHR_IF_ERR_EXIT_EXCEPT_IF
        (pfc_deadlock_recovery_get(unit, event->port, event->pfc_pri, &dd_recovery),
                                   SHR_E_NOT_FOUND);
    if (dd_recovery.manual_recovery == 0) {
        if (recovery) {
        ...
        if (recovery == 2) {
            event->recovery_state = recovery;
            LOG_INFO(BSL_LOG_MODULE,
                     (BSL_META_U(unit,
                                 "Port %d PFC Priority %d Deadlock state.\n"),
                      event->port, event->pfc_pri));
        } else {
            if (recovery == 1) {
                event->recovery_state = bcmCosqPfcDeadlockRecoveryEventBegin;
            } else {
            ...
            LOG_INFO(BSL_LOG_MODULE,
                     (BSL_META_U(unit,
                                 "Port %d PFC Priority %d Manual Recovery %s\n"),
                      event->port, event->pfc_pri, recovery ? "BEGIN" : "END"));
        }
    }
                         ...
                         bcmi_ltsw_event_status_t *status)
{
    ...
    if (sal_strcasecmp(fval.sym_val, DEADLOCKs) == 0) {
        recovery = 2;
    } else if (sal_strcasecmp(fval.sym_val, RECOVERYs) == 0) {
        recovery = 1;
    } else if (sal_strcasecmp(fval.sym_val, IDLEs) == 0) {
        recovery = 0;
    }

    rv = pfc_deadlock_event_log(unit, event, recovery);
}
...
pfc_deadlock_monitor_enable(int unit, bool enable)
{
    ...
    if (enable) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_parser_set(unit, TM_PFC_DEADLOCK_RECOVERY_STATEs,
                                      pfc_deadlock_event_parse,
                                      sizeof(pfc_deadlock_event_info_t)));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_attrib_set(unit, TM_PFC_DEADLOCK_RECOVERY_STATEs,
                                      BCMI_LTSW_EVENT_HIGH_PRIORITY));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_subscribe(unit, TM_PFC_DEADLOCK_RECOVERY_STATEs,
                                     pfc_deadlock_notification_cb, NULL));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_parser_set(unit, TM_PFC_DEADLOCK_RECOVERY_MANUAL_STATEs,
                                      pfc_deadlock_event_parse,
                                      sizeof(pfc_deadlock_event_info_t)));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_attrib_set(unit, TM_PFC_DEADLOCK_RECOVERY_MANUAL_STATEs,
                                      BCMI_LTSW_EVENT_HIGH_PRIORITY));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_subscribe(unit, TM_PFC_DEADLOCK_RECOVERY_MANUAL_STATEs,
                                     pfc_deadlock_notification_cb, NULL));
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_unsubscribe(unit, TM_PFC_DEADLOCK_RECOVERY_STATEs,
                                       pfc_deadlock_notification_cb));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_unsubscribe(unit, TM_PFC_DEADLOCK_RECOVERY_MANUAL_STATEs,
                                       pfc_deadlock_notification_cb));
    }
...
tm_ltsw_pfc_init(int unit)
{
    ...
    alloc_size = sizeof(pfc_deadlock_cb_t);

    SHR_ALLOC(pfc_deadlock_cb[unit], alloc_size, "pfcDeadlockCb");

    SHR_NULL_CHECK(pfc_deadlock_cb[unit], SHR_E_MEMORY);

    sal_memset(pfc_deadlock_cb[unit], 0, alloc_size);
...
tm_ltsw_pfc_detach(int unit)
{
    ...
    if (!warm) {
        ...
        SHR_IF_ERR_CONT
            (bcmi_lt_clear(unit, TM_PFC_DEADLOCK_RECOVERY_STATE_CONTROLs));

        SHR_IF_ERR_CONT
            (bcmi_lt_clear(unit, TM_PFC_DEADLOCK_RECOVERY_MANUAL_STATE_CONTROLs));
    }
    ...
    int *arg)
{
    ...
    if (type == bcmCosqPFCDeadlockRecoveryOccurrences) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pfc_deadlock_recovery_count_get(unit, lport, pfc_priority, &cnt));
    ...
    switch (type) {
        ...
        case bcmCosqPFCDeadlockDetectionTimer:
            ...
            gran = pfc_timer_tick_encoding[timer_tick];
            *arg = recovery.detect_timer * gran;
            break;
        case bcmCosqPFCDeadlockRecoveryTimer:
            timer_tick = bcmCosqPFCDeadlockTimerInterval100MiliSecond;
            gran = pfc_timer_tick_encoding[timer_tick];
            *arg = recovery.recover_timer * gran;
            break;
        case bcmCosqPFCDeadlockTimerGranularity:
            *arg = recovery.timer_gran;
            break;
        default:
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }
    ...
    LOG_DEBUG(BSL_LOG_MODULE,
        (BSL_META_U(unit,
                    ...
                    lport, pfc_priority, recovery.recover_enable,
                    recovery.detect_timer, recovery.recover_timer,
                    recovery.timer_gran));
    ...
    int arg)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pfc_deadlock_recovery_init(&recovery));
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pfc_deadlock_recovery_set(unit, lport, pfc_priority, &recovery));
    ...
    bcm_cosq_pfc_deadlock_info_t *pfc_deadlock_info)
{
    ...
    BCM_PBMP_ITER(pbmp, port) {
        dd_recovery.recover_enable = 0;

        SHR_IF_ERR_EXIT_EXCEPT_IF
            (pfc_deadlock_recovery_get(unit, port, pfc_priority, &dd_recovery),
                                       SHR_E_NOT_FOUND);

        if(dd_recovery.recover_enable) {
            BCM_PBMP_PORT_ADD(pfc_deadlock_info->enabled_pbmp, port);
            LOG_INFO(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "port=%d recover_enable\n"),
                            port));
        }
        ...
        if(recovery) {
            LOG_INFO(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "port=%d pfc_priority=%d recover state\n"),
                            port, pfc_priority));
...
int
tm_ltsw_pfc_deadlock_recovery_exit(
    int unit,
    bcm_port_t port,
    int pfc_priority)
{
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (pfc_deadlock_recovery_get(unit, port, 0, &recovery),
                                   SHR_E_NOT_FOUND);
    if (recovery.manual_recovery == 0) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pfc_deadlock_recovery_state_control_set(unit, port, pfc_priority,
                                                     PFC_STATE_IDLE));
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
            (pfc_deadlock_recovery_manual_state_control_set(unit, port, pfc_priority,
                                                            PFC_STATE_IDLE));
    }
...
int
tm_ltsw_pfc_deadlock_recovery_start(
    int unit,
    bcm_port_t port,
    int pfc_priority)
{
...
int
tm_ltsw_pfc_dd_recovery_mode_get(
    int unit,
    bcm_port_t port,
    bcm_cosq_pfc_dd_recovery_mode_t *recovery_mode)
{
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (pfc_deadlock_recovery_get(unit, port, 0, &recovery),
                                   SHR_E_NOT_FOUND);
    if (recovery.recovery_mode == 0) {
        *recovery_mode = bcmCosqPfcDdRecoveryModeTimer;
    } else if (recovery.recovery_mode == 1) {
...
int
tm_ltsw_pfc_dd_recovery_mode_set(
    int unit,
    bcm_port_t port,
    bcm_cosq_pfc_dd_recovery_mode_t recovery_mode)
{
    ...
    for (pfc_pri = 0; pfc_pri < PFC_PRIORITY_NUM; pfc_pri++) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pfc_deadlock_recovery_set(unit, port, pfc_pri, &recovery));
    }
