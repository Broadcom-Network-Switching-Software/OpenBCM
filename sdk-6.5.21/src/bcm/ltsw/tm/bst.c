...
/*
 * \brief CTR_EGR_TM_BST_UC_Q and CTR_EGR_TM_BST_MC_Q are mapped to MMU_THDO_BST_TOTAL_QUEUE.
 *        When working at high water mark mode, CPU write zeros to these LTs
 *        should be avoided and replaced by reading with "Clear on Read"
 *
 * \param [in]  unit                 Unit Number.
 * \param [in]  res_info             cosq_bst_handler_t.
 ...
 */
static int
cosq_bst_stat_clear_war(int unit, cosq_bst_handler_t *res_info,
                        int key0_val, int key1_val, int key2_val, int *war_stat)
{
    bcmi_lt_field_t field;
    const char *tbl_name;
    ...
    if ((sal_strcasecmp(tbl_name, CTR_EGR_TM_BST_UC_Qs) != 0) &&
        (sal_strcasecmp(tbl_name, CTR_EGR_TM_BST_MC_Qs) != 0)) {
        SHR_EXIT();
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (cosq_bst_control_get(unit, res_info->bst_module, &hwm, &cor, NULL));
    if (hwm == 1) {
        if (cor == 0) {
            cor = 1;
            SHR_IF_ERR_VERBOSE_EXIT
                (cosq_bst_control_set(unit, res_info->bst_module, -1, cor, -1));
            SHR_IF_ERR_VERBOSE_EXIT
                (cosq_bst_table_get(unit, tbl_name, key0, key0_val,
                                    key1, key1_val, key2, key2_val, &field));
            LOG_INFO(BSL_LOG_MODULE,
            (BSL_META_U(unit, "lt %s lookup %s=%d %s=%d %s=%d %s=%d\n"),
                               tbl_name, key0 ? key0 : "invalid", key0_val,
                               key1 ? key1 : "invalid", key1_val,
                               key2 ? key2 : "invalid", key2_val,
                               field.fld_name, (int)field.u.val));
            cor = 0;
            SHR_IF_ERR_VERBOSE_EXIT
                (cosq_bst_control_set(unit, res_info->bst_module, -1, cor, -1));
                       ...
                       int flags, uint32_t *value)
{
    ...
    if (flags == LTSW_BST_OP_SET) {
        if (drv->enable_cor_to_clear_stat == 1) {
            SHR_IF_ERR_VERBOSE_EXIT
                (cosq_bst_stat_clear_war(unit, res_info, key0_val,
                                         key1_val, key2_val, &war_stat_clear));
        }
        if (war_stat_clear == 0) {
            field.u.val = *value;
            SHR_IF_ERR_VERBOSE_EXIT
 ...
 */
static int
cosq_bst_tracking_enable_set(int unit, int enable)
{
    int itm, from_itm, to_itm, i;
    bcmi_ltsw_cosq_device_info_t device_info;
    ...
    for (i = 0; i < COUNTOF(module); i++) {
        for (itm = from_itm; itm <= to_itm; itm++ ) {
            SHR_IF_ERR_VERBOSE_EXIT
                (cosq_bst_event_state_control_set(unit, itm, module[i], enable));
        }
    }
                      ...
                      bcm_bst_stat_id_t bid)
{
    ...
    for (port = 0; port < max_port; port++) {
        ...
        if ((bid == bcmBstStatIdMcast) && (!(is_cpu_port(unit, port)))) {
            max_index = device_info.num_mcq - 1; /* Maximum number of TM_MC_Q_ID. */
        }
        ...
        case bcmBstStatIdMcast:
            ...
            *gport = port;
            if (is_cpu_port(unit, port)) {
                ...
                *cosq = index + device_info.num_ucq;
            }
            break;
        default:
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                                    ...
                                    bcm_bst_stat_id_t bid)
{
    ...
    if (BCM_GPORT_IS_MCAST_QUEUE_GROUP(gport)) {
        ...
        if (is_cpu_port(unit, port)) {
            ...
            port_num_ucq = device_info.num_ucq;
    ...
    int arg)
{
    ...
    if (bst_info->snapshot_mode) {
        if (enable) {
            SHR_IF_ERR_VERBOSE_EXIT
                (cosq_bst_tracking_enable_set(unit, 0));
        }
    }
    SHR_IF_ERR_VERBOSE_EXIT
        (cosq_bst_tracking_enable_set(unit, enable));
