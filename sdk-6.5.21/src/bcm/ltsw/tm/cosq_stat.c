 ...
 */
typedef struct cosq_stat_lt_field_s {
    /*! Field ID. */
    uint32_t        fid;
 ...
 */
typedef struct cosq_stat_lt_entry_s {
    /*! Fields information array. */
    cosq_stat_lt_field_t   *fields;
...
/*!
 * \brief Retrieve the specified fields of an entry using ID based BCMLT API.
 *
 * \param [in]  unit          Unit number.
 * \param [in]  tid           Logical table ID.
 * \param [inout] entry_info  LT entry information.
 *
 * \retval SHR_E_NONE         No errors.
 * \retval !SHR_E_NONE        Failure.
 */
static int
cosq_stat_lt_entry_get_by_id(
    int unit,
    uint32_t tid,
    cosq_stat_lt_entry_t *entry_info)
{
    ...
    rv = bcmlt_entry_allocate_by_id(dunit, tid, entry_info->nfields, &entry_hdl);
    if (SHR_FAILURE(rv)) {
    ...
    for (i = 0; i < entry_info->nfields; i++) {
        if (!(entry_info->fields[i].flags & BCMI_LT_FIELD_F_SET)) {
            continue;
        }
        rv = bcmlt_entry_field_add_by_id(entry_hdl, entry_info->fields[i].fid,
                                         entry_info->fields[i].u.val);
        if (SHR_FAILURE(rv)) {
            if (BCMLT_INVALID_HDL != entry_hdl) {
                (void)bcmlt_entry_free(entry_hdl);
            }
    ...
    rv = bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL);
    if (SHR_FAILURE(rv)) {
        if (BCMLT_INVALID_HDL != entry_hdl) {
            (void)bcmlt_entry_free(entry_hdl);
        }
    ...
    for (i = 0; i < entry_info->nfields; i++) {
        if (!( entry_info->fields[i].flags & BCMI_LT_FIELD_F_GET)) {
            continue;
        }
        if ( entry_info->fields[i].flags & BCMI_LT_FIELD_F_SYMBOL) {
            continue;
        }
        rv = bcmlt_entry_field_get_by_id(entry_hdl,  entry_info->fields[i].fid,
                                         &entry_info->fields[i].u.val);
        if (SHR_FAILURE(rv)) {
            if (BCMLT_INVALID_HDL != entry_hdl) {
                (void)bcmlt_entry_free(entry_hdl);
...
cosq_stat_port_queue_wred_drop_get(int unit, int local_port, uint64_t *value, int sync)
{
    ...
    int num_uc = -1;
    uint32_t table_id;
    cosq_stat_lt_entry_t lt_entry;
    cosq_stat_lt_field_t fields[] =
    {
         /* 0: PORT_ID */       {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 1: BUFFER_POOL*/    {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: TM_UC_Q_ID*/     {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 3: WRED_PKT */      {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: BYTE */          {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 5: PKT */           {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 6: OP_STATE */      {0, BCMI_LT_FIELD_F_GET | BCMI_LT_FIELD_F_SYMBOL, {0}},
    };
    bcmi_ltsw_cosq_device_info_t device_info;
    int rv;
    ...
    fields[0].fid = CTR_TM_UC_Q_DROPt_PORT_IDf;
    fields[1].fid = CTR_TM_UC_Q_DROPt_BUFFER_POOLf;
    fields[2].fid = CTR_TM_UC_Q_DROPt_TM_UC_Q_IDf;
    fields[3].fid = CTR_TM_UC_Q_DROPt_WRED_PKTf;
    fields[4].fid = CTR_TM_UC_Q_DROPt_BYTEf;
    fields[5].fid = CTR_TM_UC_Q_DROPt_PKTf;
    for (itm = 0; itm < device_info.num_itm; itm++) {
        fields[1].u.val = itm;
        for (ci = 0; ci < num_uc; ci++) {
            fields[2].u.val = ci;
            rv = cosq_stat_lt_entry_get_by_id(unit, table_id, &lt_entry);
            if (SHR_FAILURE(rv)) {
                return rv;
            }
    ...
    bcm_cosq_debug_stat_match_t *match)
{
    ...
    if ((match->match_flags & BCM_COSQ_DEBUG_STAT_MATCH_DST_PORT) ||
        ...
        if (match_valid) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_cosq_device_info_get(unit, &device_info));
            if (is_cpu_port(unit, match->dst_port)) {
                num_uc = 0;
            } else {
                num_uc = device_info.num_ucq;
            }
    ...
    bcm_cosq_debug_stat_match_t *match)
{
    ...
    if ((match->match_flags & BCM_COSQ_DEBUG_STAT_MATCH_DST_PORT) ||
        ...
        if (match->match_flags & BCM_COSQ_DEBUG_STAT_MATCH_QUEUE) {
            ...
            if (is_cpu_port(unit, match->dst_port)) {
                num_uc = 0;
                num_mc = device_info.num_cpu_queue;
            } else {
                num_uc = device_info.num_ucq;
                num_mc = device_info.num_mcq;
            }

            if ((match->cosq >= 0) && (match->cosq < num_uc)) {
                SHR_IF_ERR_VERBOSE_EXIT
            ...
            } else if ((match->cosq < (num_uc + num_mc))){
                SHR_IF_ERR_VERBOSE_EXIT
...
tm_ltsw_cosq_stat_rqe_drop_counter_get(int unit, bcm_gport_t port, bcm_cos_queue_t cosq,
                                       uint64_t *pkt, uint64_t *byte, uint64_t *red_pkt,
                                       uint64_t *yellow_pkt, int sync)
{
    int rv;
    int itm;
    cosq_stat_lt_entry_t lt_entry;
    cosq_stat_lt_field_t fields[] =
    {
         /* 0: BUFFER_POOL */  {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 1: REPL_Q_NUM */   {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: PKT */          {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 3: BYTE */         {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: RED_PKT */      {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 5: YELLOW_PKT */   {0, BCMI_LT_FIELD_F_GET | BCMI_LT_FIELD_F_SYMBOL, {0}},
    };
    ...
    fields[0].fid = CTR_TM_REPL_Q_DROPt_BUFFER_POOLf;
    fields[1].fid = CTR_TM_REPL_Q_DROPt_REPL_Q_NUMf;
    fields[2].fid = CTR_TM_REPL_Q_DROPt_PKTf;
    fields[3].fid = CTR_TM_REPL_Q_DROPt_BYTEf;
    fields[4].fid = CTR_TM_REPL_Q_DROPt_RED_PKTf;
    fields[5].fid = CTR_TM_REPL_Q_DROPt_YELLOW_PKTf;

    fields[1].u.val = cosq;
    LTSW_COMPILER_64_ZERO(*pkt);
    LTSW_COMPILER_64_ZERO(*byte);
    LTSW_COMPILER_64_ZERO(*red_pkt);
    LTSW_COMPILER_64_ZERO(*yellow_pkt);
    ...
    for (itm = 0; itm < device_info.num_itm; itm++) {
        fields[0].u.val = itm;
        rv = cosq_stat_lt_entry_get_by_id(unit, CTR_TM_REPL_Q_DROPt, &lt_entry);
        if (SHR_FAILURE(rv)) {
            return rv;
        }

        LTSW_COMPILER_64_ADD_64(*pkt, fields[2].u.val);
        LTSW_COMPILER_64_ADD_64(*byte, fields[3].u.val);
        LTSW_COMPILER_64_ADD_64(*red_pkt, fields[4].u.val);
        LTSW_COMPILER_64_ADD_64(*yellow_pkt, fields[5].u.val);
    }
    ...
    int sync)
{
    ...
    int itm;
    cosq_stat_lt_entry_t lt_entry;
    uint32_t table_id = CTR_TM_REPL_Qt;
    cosq_stat_lt_field_t fields[] =
    {
         /* 0: BUFFER_POOL */        {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 1: REPL_Q_NUM*/          {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: SHARED_USAGE_CELLS */ {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 3: MIN_USAGE_CELLS */    {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: TOTAL_USAGE_CELLS */  {0, BCMI_LT_FIELD_F_GET, {0}},
    };
    bcmi_ltsw_cosq_device_info_t device_info;
    ...
    fields[0].fid = CTR_TM_REPL_Qt_BUFFER_POOLf;
    fields[1].fid = CTR_TM_REPL_Qt_REPL_Q_NUMf;
    fields[2].fid = CTR_TM_REPL_Qt_SHARED_USAGE_CELLSf;
    fields[3].fid = CTR_TM_REPL_Qt_MIN_USAGE_CELLSf;
    fields[4].fid = CTR_TM_REPL_Qt_TOTAL_USAGE_CELLSf;
    fields[1].u.val = cosq;
    LTSW_COMPILER_64_ZERO(*shared);
    LTSW_COMPILER_64_ZERO(*min);
    LTSW_COMPILER_64_ZERO(*total);
    ...
    for (itm = 0; itm < device_info.num_itm; itm++) {
        ...
        LTSW_COMPILER_64_ADD_64(*shared, fields[2].u.val);
        LTSW_COMPILER_64_ADD_64(*min, fields[3].u.val);
        LTSW_COMPILER_64_ADD_64(*total, fields[4].u.val);
    }
    ...
    int sync)
{
    ...
    int itm;
    cosq_stat_lt_entry_t lt_entry;
    uint32_t table_id = CTR_EGR_TM_PORTt;
    cosq_stat_lt_field_t fields[] =
    {
         /* 0: PORT_ID */       {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 1: BUFFER_POOL*/    {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: UC_PKT */        {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 3: MC_PKT */        {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: OP_STATE */      {0, BCMI_LT_FIELD_F_GET | BCMI_LT_FIELD_F_SYMBOL, {0}},
    };
    bcmi_ltsw_cosq_device_info_t device_info;
    ...
    fields[0].fid = CTR_EGR_TM_PORTt_PORT_IDf;
    fields[1].fid = CTR_EGR_TM_PORTt_BUFFER_POOLf;
    fields[2].fid = CTR_EGR_TM_PORTt_UC_PKTf;
    fields[3].fid = CTR_EGR_TM_PORTt_MC_PKTf;

    fields[0].u.val = local_port;
    LTSW_COMPILER_64_ZERO(*uc_pkt);
    LTSW_COMPILER_64_ZERO(*mc_pkt);
    ...
    for (itm = 0; itm < device_info.num_itm; itm++) {
        ...
        LTSW_COMPILER_64_ADD_64(*uc_pkt, fields[2].u.val);
        LTSW_COMPILER_64_ADD_64(*mc_pkt, fields[3].u.val);
    }
...
tm_ltsw_cosq_stat_port_queue_counter_get(int unit, bcm_gport_t gport, bcm_cos_queue_t cosq,
                                         bcm_cosq_stat_t stat, uint64_t *packet, uint64_t *byte, int sync)
{
    int rv;
    int local_port;
    ...
    {
         /* 0: PORT_ID */  {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 1: QUEUE*/     {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: PKT */      {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 3: BYTES */    {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: OP_STATE */ {0, BCMI_LT_FIELD_F_GET | BCMI_LT_FIELD_F_SYMBOL, {0}},
    };
    ...
    if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport)) {
        rv = bcmi_ltsw_cosq_gport_port_resolve(unit, gport, &local_port, &startq);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
        fields[0].fid = CTR_EGR_UC_Qt_PORT_IDf;
        fields[1].fid = CTR_EGR_UC_Qt_UC_Qf;
        fields[2].fid = CTR_EGR_UC_Qt_PKTf;
        fields[3].fid = CTR_EGR_UC_Qt_BYTESf;
        fields[1].u.val = startq;
        table_id = CTR_EGR_UC_Qt;
        rv = cosq_stat_lt_entry_get_by_id(unit, table_id, &lt_entry);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
        ...
        fields[0].fid = CTR_EGR_MC_Qt_PORT_IDf;
        fields[1].fid = CTR_EGR_MC_Qt_MC_Qf;
        fields[2].fid = CTR_EGR_MC_Qt_PKTf;
        fields[3].fid = CTR_EGR_MC_Qt_BYTESf;
        fields[1].u.val = startq;
        table_id = CTR_EGR_MC_Qt;
        rv = cosq_stat_lt_entry_get_by_id(unit, table_id, &lt_entry);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
        ...
        rv = bcmi_ltsw_cosq_sched_node_child_get(unit, gport, &num_uc, uc_q,
                                                 &num_mc, mc_q);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
    ...
    LTSW_COMPILER_64_COPY(*packet, sum);
    LTSW_COMPILER_64_COPY(*byte, byte_sum);
    return SHR_E_NONE;
}
...
tm_ltsw_cosq_stat_port_queue_color_drop_get(int unit, bcm_gport_t gport, bcm_cos_queue_t cosq,
                                            uint64_t *uc_red_pkt, uint64_t *uc_yellow_pkt,
                                            uint64_t *mc_red_pkt, uint64_t *mc_yellow_pkt,
                                            uint64_t *wred_red_pkt, uint64_t *wred_yellow_pkt,
                                            uint64_t *wred_pkt, int sync)
{
    int local_port;
    int itm;
    uint32_t table_id;
    ...
    cosq_stat_lt_field_t fields[] =
    {
         /* 0: PORT_ID */         {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 1: BUFFER_POOL*/      {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: UC_RED_PKT */      {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 3: UC_YELLOW_PKT */   {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: MC_RED_PKT */      {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 5: MC_YELLOW_PKT */   {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 6: WRED_RED_PKT */    {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 7: WRED_YELLOW_PKT */ {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 8: OP_STATE */        {0, BCMI_LT_FIELD_F_GET | BCMI_LT_FIELD_F_SYMBOL, {0}},
    };
    bcmi_ltsw_cosq_device_info_t device_info;
    ...
    table_id = CTR_EGR_TM_PORT_DROPt;
    fields[0].fid = CTR_EGR_TM_PORT_DROPt_PORT_IDf;
    fields[1].fid = CTR_EGR_TM_PORT_DROPt_BUFFER_POOLf;
    fields[2].fid = CTR_EGR_TM_PORT_DROPt_UC_RED_PKTf;
    fields[3].fid = CTR_EGR_TM_PORT_DROPt_UC_YELLOW_PKTf;
    fields[4].fid = CTR_EGR_TM_PORT_DROPt_MC_RED_PKTf;
    fields[5].fid = CTR_EGR_TM_PORT_DROPt_MC_YELLOW_PKTf;
    fields[6].fid = CTR_EGR_TM_PORT_DROPt_WRED_RED_PKTf;
    fields[7].fid = CTR_EGR_TM_PORT_DROPt_WRED_YELLOW_PKTf;

    fields[0].u.val = local_port;
    LTSW_COMPILER_64_ZERO(*uc_red_pkt);
    LTSW_COMPILER_64_ZERO(*uc_yellow_pkt);
    LTSW_COMPILER_64_ZERO(*mc_red_pkt);
    LTSW_COMPILER_64_ZERO(*mc_yellow_pkt);
    LTSW_COMPILER_64_ZERO(*wred_red_pkt);
    LTSW_COMPILER_64_ZERO(*wred_yellow_pkt);
    ...
    for (itm = 0; itm < device_info.num_itm; itm++) {
        ...
        LTSW_COMPILER_64_ADD_64(*uc_red_pkt, fields[2].u.val);
        LTSW_COMPILER_64_ADD_64(*uc_yellow_pkt, fields[3].u.val);
        LTSW_COMPILER_64_ADD_64(*mc_red_pkt, fields[4].u.val);
        LTSW_COMPILER_64_ADD_64(*mc_yellow_pkt, fields[5].u.val);
        LTSW_COMPILER_64_ADD_64(*wred_red_pkt, fields[6].u.val);
        LTSW_COMPILER_64_ADD_64(*wred_yellow_pkt, fields[7].u.val);
    }

    rv = cosq_stat_port_queue_wred_drop_get(unit, local_port, wred_pkt, sync);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
...
tm_ltsw_cosq_stat_port_queue_drop_counter_get(int unit, bcm_gport_t gport, bcm_cos_queue_t cosq,
                                              bcm_cosq_stat_t stat, uint64_t *packet, uint64_t *byte,
                                              uint64_t *wred_pkt, int sync)
{
    int local_port, itm;
    int numq, startq, loop_idx, ci;
    ...
    cosq_stat_lt_field_t fields[] =
    {
         ...
         /* 1: BUFFER_POOL*/ {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: TM_UC_Q_ID */ {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 3: PKT */        {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: BYTES */      {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 5: WRED_PKT */   {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 6: OP_STATE */   {0, BCMI_LT_FIELD_F_GET | BCMI_LT_FIELD_F_SYMBOL, {0}},
    };
    int rv;
    ...
    if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport)) {
        ...
        fields[1].fid = CTR_TM_UC_Q_DROPt_BUFFER_POOLf;
        fields[2].fid = CTR_TM_UC_Q_DROPt_TM_UC_Q_IDf;
        fields[3].fid = CTR_TM_UC_Q_DROPt_PKTf;
        fields[4].fid = CTR_TM_UC_Q_DROPt_BYTEf;
        fields[5].fid = CTR_TM_UC_Q_DROPt_WRED_PKTf;
        ...
        fields[0].fid = CTR_TM_MC_Q_DROPt_PORT_IDf;
        fields[1].fid = CTR_TM_MC_Q_DROPt_BUFFER_POOLf;
        fields[2].fid = CTR_TM_MC_Q_DROPt_TM_MC_Q_IDf;
        fields[3].fid = CTR_TM_MC_Q_DROPt_PKTf;
        fields[4].fid = CTR_TM_MC_Q_DROPt_BYTEf;
        fields[2].u.val = startq;
        fields[5].flags = 0;
        table_id = CTR_TM_MC_Q_DROPt;
        for (itm = 0; itm < device_info.num_itm; itm++) {
            fields[1].u.val = itm;
            rv = cosq_stat_lt_entry_get_by_id(unit, table_id, &lt_entry);
            if (SHR_FAILURE(rv)) {
    ...
    LTSW_COMPILER_64_COPY(*packet, sum);
    LTSW_COMPILER_64_COPY(*byte, byte_sum);
    LTSW_COMPILER_64_COPY(*wred_pkt, wred_sum);
                                    ...
                                    bcm_cosq_stat_t stat, uint64_t *value, int sync)
{
    ...
    cosq_stat_lt_field_t fields[] =
    {
         ...
         /* 1: BUFFER_POOL*/          {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: TM_UC_Q_ID*/           {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 3: SHARED_USAGE_CELLS */  {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: MIN_USAGE_CELLS */     {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 5: TOTAL_USAGE_CELLS */   {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 6: OP_STATE */            {0, BCMI_LT_FIELD_F_GET | BCMI_LT_FIELD_F_SYMBOL, {0}},
    };
    bcmi_ltsw_cosq_device_info_t device_info;
    ...
    switch (stat) {
        ...
        case bcmCosqStatEgressUCQueueBytesCurrent:
            ...
            if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport)) {
                rv = bcmi_ltsw_cosq_gport_port_resolve(unit, gport,
                                                       &local_port,
                                                       &startq);
                if (SHR_FAILURE(rv)) {
                    return rv;
            ...
            table_id = CTR_TM_THD_UC_Qt;
            fields[0].fid = CTR_TM_THD_UC_Qt_PORT_IDf;
            fields[1].fid = CTR_TM_THD_UC_Qt_BUFFER_POOLf;
            fields[2].fid = CTR_TM_THD_UC_Qt_TM_UC_Q_IDf;
            fields[3].fid = CTR_TM_THD_UC_Qt_SHARED_USAGE_CELLSf;
            fields[4].fid = CTR_TM_THD_UC_Qt_MIN_USAGE_CELLSf;
            fields[5].fid = CTR_TM_THD_UC_Qt_TOTAL_USAGE_CELLSf;
            break;
        ...
        case bcmCosqStatEgressMCQueueBytesCurrent:
            ...
            table_id = CTR_TM_THD_MC_Qt;
            fields[0].fid = CTR_TM_THD_MC_Qt_PORT_IDf;
            fields[1].fid = CTR_TM_THD_MC_Qt_BUFFER_POOLf;
            fields[2].fid = CTR_TM_THD_MC_Qt_TM_MC_Q_IDf;
            fields[3].fid = CTR_TM_THD_MC_Qt_SHARED_USAGE_CELLSf;
            fields[4].fid = CTR_TM_THD_MC_Qt_MIN_USAGE_CELLSf;
            fields[5].fid = CTR_TM_THD_MC_Qt_TOTAL_USAGE_CELLSf;
            break;
        default:
            return SHR_E_PARAM;
    }
...
tm_ltsw_cosq_stat_pg_current_usage_get(int unit, bcm_gport_t port, bcm_cos_queue_t cosq,
                                       uint64_t *min, uint64_t *shared,
                                       uint64_t *headroom, int sync)
{
    int rv;
    int local_port;
    cosq_stat_lt_entry_t lt_entry;
    ...
    {
         /* 0: PORT_ID */               {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 1: TM_PRI_GRP_ID */         {0, BCMI_LT_FIELD_F_SET, {0}},
         /* 2: MIN_USAGE_CELLS */       {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 3: SHARED_USAGE_CELLS */    {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 4: HEADROOM_USAGE_CELLS */  {0, BCMI_LT_FIELD_F_GET, {0}},
         /* 5: OP_STATE */              {0, BCMI_LT_FIELD_F_GET | BCMI_LT_FIELD_F_SYMBOL, {0}},
    };
    ...
    rv = bcmi_ltsw_cosq_localport_resolve(unit, port, &local_port);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    ...
    fields[0].fid = CTR_ING_TM_PORT_PRI_GRPt_PORT_IDf;
    fields[1].fid = CTR_ING_TM_PORT_PRI_GRPt_TM_PRI_GRP_IDf;
    fields[2].fid = CTR_ING_TM_PORT_PRI_GRPt_MIN_USAGE_CELLSf;
    fields[3].fid = CTR_ING_TM_PORT_PRI_GRPt_SHARED_USAGE_CELLSf;
    fields[4].fid = CTR_ING_TM_PORT_PRI_GRPt_HEADROOM_USAGE_CELLSf;

    fields[0].u.val = local_port;
    fields[1].u.val = cosq;

    rv = cosq_stat_lt_entry_get_by_id(unit, CTR_ING_TM_PORT_PRI_GRPt, &lt_entry);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    LTSW_COMPILER_64_COPY(*min, fields[2].u.val);
    LTSW_COMPILER_64_COPY(*shared, fields[3].u.val);
    LTSW_COMPILER_64_COPY(*headroom, fields[4].u.val);
