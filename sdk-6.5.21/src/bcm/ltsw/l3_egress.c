                                        ...
                                        int member_cnt, bcm_port_t *member_arr)
{
    ...
    if (!bcmi_ltsw_property_get(unit, BCMI_CPN_DEST_SYSPORT_TO_UL_NH, 0) ||
        !ltsw_feature(unit, LTSW_FT_DEST_SYSPORT_TO_UL_NH)) {
        SHR_EXIT();
    }
                          ...
                          bcm_if_t *intf)
{
    ...
    /* Notify virtual module the changes. */
    if (ltsw_feature(unit, LTSW_FT_FLEX_FLOW)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_virtual_port_egress_info_notify(unit, &egr_int, *intf));
    }
                               ...
                               uint32_t stat_counter_id)
{
    ...
    if (ci.source != bcmFlexctrSourceL3Egress) {
        if (((type == BCMI_LTSW_L3_EGR_OBJ_T_OL) &&
                        ...
                        bcm_l3_ecmp_member_t *ecmp_member_array)
{
    ...
    /* Notify flow module the changes. */
    if (ltsw_feature(unit, LTSW_FT_FLEX_FLOW)) {
        /*
         * If ECMP created for the first time, no sync needed since the
         * binding has not happened.
         */
...
bcm_ltsw_l3_ecmp_member_delete_all(int unit, bcm_if_t ecmp_group_id)
{
    ...
    /* Get memberships before delete. */
    if (ltsw_feature(unit, LTSW_FT_FLEX_FLOW)) {
        max_paths = ei->ecmp_info->max_paths;
        array_size = max_paths * sizeof(bcm_l3_ecmp_member_t);
        SHR_ALLOC(ecmp_member_array, array_size, "EcmpMemberArray");
        SHR_NULL_CHECK(ecmp_member_array, SHR_E_MEMORY);
        sal_memset(ecmp_member_array, 0, array_size);

        ecmp_member_size = max_paths;
    ...
    uint32_t stat_counter_id)
{
    ...
    if (ci.source != bcmFlexctrSourceEcmp) {
        if ((type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_UL) &&
            (ci.source != bcmFlexctrSourceEcmpUnderlay)) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }

        if ((type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_OL) &&
            (ci.source != bcmFlexctrSourceEcmpOverlay)) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
    }
