...
#define LTSW_TNL_DEFAULT_TERM_MASK    0
#define LTSW_TNL_DEFAULT_TERM_EM      1
...
#define TUNNEL_DEFAULT_TERM(unit) \
    ltsw_tunnel_info[unit].tunnel_default_term
 ...
 */
static int
ltsw_tunnel_initiator_flexctr_object_get(
    int unit,
    bcm_if_t intf,
    uint32_t *value)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_tunnel_initiator_flexctr_object_get(unit, intf, value));
...
ltsw_tunnel_terminator_validate(int unit, bcm_tunnel_terminator_t *tnl_term_info)
{
    ...
    if (TUNNEL_DEFAULT_TERM(unit) == LTSW_TNL_DEFAULT_TERM_MASK) {
        if (tnl_term_info->flags & BCM_TUNNEL_TERM_MASK) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    } else if (TUNNEL_DEFAULT_TERM(unit) == LTSW_TNL_DEFAULT_TERM_EM) {
    ...
    if (((TUNNEL_DEFAULT_TERM(unit) == LTSW_TNL_DEFAULT_TERM_MASK) &&
        ...
        ((TUNNEL_DEFAULT_TERM(unit) == LTSW_TNL_DEFAULT_TERM_EM) &&
...
{
    bcmint_tunnel_tbl_db_t tbl_db = {0};

    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_tunnel_tbl_db_get(unit, &tbl_db));
...
bcmi_ltsw_tunnel_init_lt_db(int unit)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_tunnel_init_lt_db(unit));
    ...
    uint32 stat_counter_id)
{
    ...
    if (((TUNNEL_DEFAULT_TERM(unit) == LTSW_TNL_DEFAULT_TERM_MASK) &&
         (terminator->flags & BCM_TUNNEL_TERM_EM)) ||
        ((TUNNEL_DEFAULT_TERM(unit) == LTSW_TNL_DEFAULT_TERM_EM) &&
         (!(terminator->flags & BCM_TUNNEL_TERM_MASK)))) {
 ...
 */
int
bcm_ltsw_tunnel_initiator_flexctr_object_get(
    int unit,
    bcm_if_t intf,
    uint32_t *value)
{
    ...
    SHR_IF_ERR_CONT
        (ltsw_tunnel_initiator_flexctr_object_get(unit, intf, value));
