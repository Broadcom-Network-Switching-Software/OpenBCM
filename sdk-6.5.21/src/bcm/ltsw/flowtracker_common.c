...
/* User callback function for learn notifications */
bcm_flowtracker_flow_learn_notify_cb ft_learn_user_cb[BCM_MAX_NUM_UNITS] = {NULL};

/* User callback function cookie for learn notifications */
void *ft_learn_user_cb_data[BCM_MAX_NUM_UNITS] = {NULL};

/* User callback function for learn failure notifications */
bcm_flowtracker_flow_learn_fail_notify_cb ft_learn_fail_user_cb[BCM_MAX_NUM_UNITS] = {NULL};

/* User callback function cookie for learn failure notifications */
void *ft_learn_fail_user_cb_data[BCM_MAX_NUM_UNITS] = {NULL};

/* EM group information */
bcm_int_flowtracker_em_group_info_t *ft_em_group_info[BCM_MAX_NUM_UNITS] = {NULL};
...
static bcmi_ltsw_collector_transport_type_t
bcmi_ltsw_collector_int_transport_type_get(bcm_collector_transport_type_t type) {
    switch(type) {
        case bcmCollectorTransportTypeIpv4Udp:
            return bcmiCollectorTransportTypeIpv4Udp;
        case bcmCollectorTransportTypeIpv6Udp:
            return bcmiCollectorTransportTypeIpv6Udp;
        default:
            return bcmiCollectorTransportTypeCount;
    }
}
 ...
 * 1) Call alloc to get the old version's ptr.
 * 2) copy from the old version to new version inside ft_info.
 * 3) Delete/free the old version.
 */

static int
bcm_ltsw_flowtracker_lists_info_old_format_list_handle(
                                        int unit, uint32_t *new_mem_ptr,
                                        int sub_comp_id, int max_ids)
{
    const char *blk_ids[] = FT_HA_SUB_COMP_ID_STRINGS;
    SHR_BITDCL *old_mem_ptr = NULL;
    uint32_t alloc_sz = SHR_BITALLOCSIZE(max_ids);

    SHR_FUNC_ENTER(unit);
    old_mem_ptr = bcmi_ltsw_ha_mem_alloc(
            unit,
            BCMI_HA_COMP_ID_FLOWTRACKER,
            sub_comp_id,
            blk_ids[sub_comp_id],
            &alloc_sz);
    SHR_NULL_CHECK(old_mem_ptr, SHR_E_MEMORY);

    /* Copy the old to new. */
    sal_memcpy(new_mem_ptr, old_mem_ptr, alloc_sz);

    /* Delete the old one */
    SHR_IF_ERR_VERBOSE_EXIT(bcmi_ltsw_ha_mem_free(unit, old_mem_ptr));
exit:
    SHR_FUNC_EXIT();
}

/*
 * This routine should only be called when it is warmbooting from old
 * version which supports the pool IDs outside the global info to a version
 * which supports it.
 * *NOTE*: For the number of IDs, we are using numbers rather than macros
 * because those macros could change in the future, but the older versions
 * would have been allocated for the original value.
 */
static int
bcm_ltsw_flowtracker_lists_info_old_format_handle(int unit)
{
    ...
    /* FT old group pool info */
    sub_comp_id = FT_SUB_COMP_ID_FLOW_GROUP_POOL;
    max_ids = 256;
    SHR_IF_ERR_VERBOSE_EXIT(
        bcm_ltsw_flowtracker_lists_info_old_format_list_handle(unit,
                                                    ft_info->flow_group_pool,
                                                    sub_comp_id,
                                                    max_ids));
    /* Export template old group pool info */
    sub_comp_id = FT_SUB_COMP_ID_EXPORT_TEMPLATE_POOL;
    max_ids = 5;
    SHR_IF_ERR_VERBOSE_EXIT(
        bcm_ltsw_flowtracker_lists_info_old_format_list_handle(unit,
                                                    ft_info->export_template_pool,
                                                    sub_comp_id,
                                                    max_ids));

    /* Export template old group pool info */
    sub_comp_id = FT_SUB_COMP_ID_ELEPHANT_PROFILE_POOL;
    max_ids = 5;
    SHR_IF_ERR_VERBOSE_EXIT(
        bcm_ltsw_flowtracker_lists_info_old_format_list_handle(unit,
                                                    ft_info->elph_profile_pool,
                                                    sub_comp_id,
                                                    max_ids));
exit:
    SHR_FUNC_EXIT();
...
bcm_ltsw_flowtracker_init_index_lists(int unit, bool warm)
{
    ...
    SHR_NULL_CHECK(ft_info, SHR_E_INIT);

    /*
     * If this is warmboot and we are coming from an old version where
     * the various pool IDs are not stored in the global info structure,
     * 1) copy them to the global_info structure.
     * 2) Free up the older HA memory blocks.
     */
    if ((warm) && (ft_info->pools_info_in_glb_info == false)) {
        SHR_IF_ERR_VERBOSE_EXIT(
                bcm_ltsw_flowtracker_lists_info_old_format_handle(unit));
    }
    ...
    if (!warm) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT(
                bcm_ltsw_flowtracker_list_id_reserve
                            (unit, ft_info->flow_group_pool, reserved_id));

        /* Reserve index 0 since that is invalid export template id */
        SHR_IF_ERR_VERBOSE_EXIT(
                bcm_ltsw_flowtracker_list_id_reserve(
                    unit, ft_info->export_template_pool, reserved_id));

        /* Reserving zeroth index to use it as invaild id */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm_ltsw_flowtracker_list_id_reserve(
                 unit, ft_info->elph_profile_pool, reserved_id));
    }

    /*
     * From this version onwards, the pool information are all inside
     * the global info structure. Set the appropriate indication.
     */
    ft_info->pools_info_in_glb_info = true;
exit:
    SHR_FUNC_EXIT();
}

static int
bcm_ltsw_flowtracker_free_em_group_info (int unit)
{
    SHR_FUNC_ENTER(unit);

    if (ft_em_group_info[unit] == NULL) {
        SHR_ERR_EXIT(SHR_E_INIT);
    }

    SHR_IF_ERR_VERBOSE_EXIT(bcmi_ltsw_ha_mem_free(unit,
                                            ft_em_group_info[unit]));
    ft_em_group_info[unit] = NULL;
exit:
    SHR_FUNC_EXIT();
}

static int
bcm_ltsw_flowtracker_alloc_em_group_info (int unit, bool warm)
{
    const char *blk_ids[] = FT_HA_SUB_COMP_ID_STRINGS;
    int comp_id = BCMI_HA_COMP_ID_FLOWTRACKER;
    int sub_comp_id = FT_SUB_COMP_ID_EM_GROUP_INFO;
    uint32_t alloc_size = 0;
    uint32_t num_em_groups = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT(
            mbcm_ltsw_flowtracker_num_em_groups_get(unit, &num_em_groups));

    if (num_em_groups == 0) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }

    /* We are allocating an array of structures. */
    data_size = sizeof(bcm_int_flowtracker_em_group_info_t);
    num_instances = num_em_groups;
    alloc_size = data_size * num_instances;

    ft_em_group_info[unit] = bcmi_ltsw_ha_mem_alloc
                                            (unit,
                                            comp_id,
                                            sub_comp_id,
                                            blk_ids[sub_comp_id],
    ...
    SHR_NULL_CHECK(ft_em_group_info[unit], SHR_E_MEMORY);

    if (!warm) {
        sal_memset(ft_em_group_info[unit], 0, alloc_size);
    }

    rv = bcmi_ltsw_issu_struct_info_report(unit,
            comp_id,
            sub_comp_id,
            0, data_size, num_instances,
            BCM_INT_FLOWTRACKER_EM_GROUP_INFO_T_ID);
    if (rv != SHR_E_EXISTS) {
        ...
        bool *id_alloced)
{
    ...
    if ((options & BCM_FLOWTRACKER_GROUP_REPLACE) &&
        (options & BCM_FLOWTRACKER_GROUP_WITH_ID)) {
        if (!bcm_ltsw_flowtracker_list_id_present(unit, ft_info->flow_group_pool, *id)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT(
                mbcm_ltsw_flowtracker_per_pipe_em_mode_validate(unit, ft_info->num_pipes, flow_group_info));
    ...
    if (options & BCM_FLOWTRACKER_GROUP_WITH_ID) {
        if (!(options & BCM_FLOWTRACKER_GROUP_REPLACE)) {
            ...
            rv = bcm_ltsw_flowtracker_list_id_reserve(unit, ft_info->flow_group_pool,
                                         *id);
            if (SHR_FAILURE(rv)) {
        ...
        rv = bcm_ltsw_flowtracker_list_id_alloc(unit, ft_info->flow_group_pool,
        ...
        )
{
    ...
    if (!bcm_ltsw_flowtracker_list_id_present(unit, ft_info->flow_group_pool,
                                              id)) {
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }

    flow_group_int_info.flow_group_id = id;
    flow_group_int_info.flags = 0;
    SHR_IF_ERR_VERBOSE_EXIT(
        ...
        bcm_int_ft_flow_group_info_t *flow_group_int_info)
{
    ...
    /* Check if setting of flow limit on this group ID is allowed. */
    SHR_IF_ERR_VERBOSE_EXIT(
        mbcm_ltsw_flowtracker_flow_limit_config_supported(unit, id));
exit:
    SHR_FUNC_EXIT();
}
...
/* Checks if the action list contains mirror action and if yes,
 * returns the mirror flags and either mirror_index or mirror_dest_gport
 * within the mirror_dest_gport_or_index (depends on how the action_list was got
 * from LT).
 */
static bool
bcm_ltsw_flowtracker_group_actions_contains_mirror(int num_actions,
                                                   bcm_int_ft_group_action_info_t *action_list,
                                                   int *mirror_flags,
                                                   int *mirror_dest_gport_or_index)
{
    bool action_found = false;
    int i;
    for (i = 0; i < num_actions; i++) {
        if (action_list[i].action == bcmFlowtrackerGroupActionMirrorIndex) {
            action_found = true;
            *mirror_flags = action_list[i].params[0] |
                    (BCMI_LTSW_MIRROR_EM_FT | BCMI_LTSW_MIRROR_HW_LEARN_ENTRY);
            *mirror_dest_gport_or_index = action_list[i].params[1];
        }
    }
    return action_found;
}
                           ...
                           bcm_int_ft_flow_group_info_t *flow_group_int_info)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    int mirror_flags = 0;
    bcm_gport_t mirror_dest_gport = BCM_GPORT_INVALID;
    SHR_FUNC_ENTER(unit);
    ...
    /*
     * If HW is learning the flows and there is no embedded app to keep
     * track of the flow table and actions associated with each flow,
     * then there is no way to change/modify the actions of the flows
     * learnt as part of that group already. So we reject any updates on
     * the flowtracker group actions.
     */
    if (ft_info->hw_learn_en == FT_HW_LEARN_ENABLE_WO_EAPP) {
        /* Group already has some action. So reject any updates except delete */
        if ((flow_group_int_info->num_actions != 0) &&
            (num_actions != 0)) {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                    "FT(unit %d) Error: Cannot update group actions when flowtracker_hardware_learn_enable is 2\n"),
                     unit));
            SHR_ERR_EXIT(SHR_E_CONFIG);
        }

        if (num_actions == 0) /* Delete case */ {
            /*
             * Check if one of the actions is mirror.
             * If yes, delete the mirror dest association to flowtracker.
             */
            if (bcm_ltsw_flowtracker_group_actions_contains_mirror(
                        flow_group_int_info->num_actions,
                        flow_group_int_info->action_list,
                        &mirror_flags, &mirror_dest_gport)) {
                /*
                 * Call delete only if mirror gport is not 0
                 * which is mirror cancel case
                 */
                if (mirror_dest_gport != 0) {
                    SHR_IF_ERR_VERBOSE_EXIT(
                            bcmi_ltsw_mirror_field_dest_delete(unit, id,
                                mirror_flags,
                                mirror_dest_gport));
                }
            }
...
bcm_ltsw_flowtracker_clear_tables(int unit)
{
    int i, num;
    const char *ltname[] = {
        MON_FLOWTRACKER_HW_LEARN_FLOW_ACTION_CONTROLs,
        MON_FLOWTRACKER_HW_LEARN_FLOW_ACTION_STATEs,
        MON_FLOWTRACKER_LEARN_EVENT_CONTROLs,
        MON_FLOWTRACKER_LEARN_EVENT_LOGs,
        MON_FLOWTRACKER_LEARN_EVENT_LOG_STATUSs,
        MON_FLOWTRACKER_LEARN_EVENT_STATSs,
        ...
        bcm_collector_info_t *collector_info)
{
    ...
    if (export_profile_info.wire_format == bcmCollectorWireFormatIpfix) {
        if (!bcm_ltsw_flowtracker_list_id_present(unit, ft_info->export_template_pool,
                                                  template_id)) {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
    ...
    if (export_profile_info.wire_format == bcmCollectorWireFormatIpfix) {
        if (bcmi_ltsw_collector_check_user_is_other(unit, collector_id,
                bcmiCollectorExportAppFlowtrackerIpfix)) {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
        ...
        if (bcmi_ltsw_collector_check_user_is_other
                (unit, collector_id, bcmiCollectorExportAppFlowtrackerProtoBuf)) {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "FT(unit %d) Error: Collector (ID:%d) is associated to another wire format.\n"),
                     unit, collector_id));
...
static int
bcm_ltsw_ft_flexctr_pkt_byte_reserve_wo_eapp(uint32_t unit,
                                bcm_int_ft_info_t *ft_info)
{
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_VERBOSE_EXIT(
            mbcm_ltsw_flowtracker_flex_ctr_pkt_byte_reserve_wo_eapp(
                unit,
                ft_info->max_flows[0],
...
bcm_ltsw_flowtracker_info_init(int unit, bcm_int_ft_info_t *ft_info)
{
    ...
    ft_info->flex_ctr_alloc_enable = bcmi_ltsw_property_get(unit,
                                BCMI_CPN_FLOWTRACKER_FLEXCTR_ALLOC_ENABLE, 0);
...
bcm_ltsw_flowtracker_init_common(int unit)
{
    ...
    if (ft_info == NULL) {
        const char *blk_ids[] = FT_HA_SUB_COMP_ID_STRINGS;
        int comp_id = BCMI_HA_COMP_ID_FLOWTRACKER;
        int sub_comp_id = FT_SUB_COMP_ID_GLOBAL;
        uint32_t num_instances = 0;
        uint32_t data_size = 0;
        data_size = sizeof(bcm_int_ft_info_t);
        num_instances = 1;
        alloc_size = data_size * num_instances;
    ...
    if (!warmboot) {
        ...
        } else if (ft_info->flex_ctr_alloc_enable) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm_ltsw_ft_flexctr_pkt_byte_reserve_wo_eapp(unit, ft_info));
        }
        ...
        if (ft_info->hw_learn_en) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT(
                    bcm_ltsw_flowtracker_learn_event_stats_dummy_entries_set(unit));
            SHR_IF_ERR_EXIT_EXCEPT_IF(
                    mbcm_ltsw_flowtracker_hit_index_compaction_ctrl_set(unit),
                    SHR_E_UNAVAIL);
        }
    ...
    if ((warmboot) && (ft_info->host_mem)) {
        ...
        LOG_WARN(BSL_LOG_MODULE, (BSL_META_U(unit,
          "FT(unit %d) Warning: Flowtracker app using host memory. Warmboot not supported.\n"),
                 unit));
    }
    ...
    if (ft_info->hw_learn_en == FT_HW_LEARN_ENABLE_WO_EAPP) {
        SHR_IF_ERR_VERBOSE_EXIT(
            bcm_ltsw_flowtracker_alloc_em_group_info(unit, warmboot));
        /* Subscribe for the flow learn failure notifications from SDKLT */
        SHR_IF_ERR_EXIT(
                bcm_ltsw_flowtracker_learn_fail_event_state_subscribe_set(unit, true));
    }
 ...
 *      SHR_E_XXX   - BCM error code.
 * Note:
 *      All the internal APIs to destroy HW information has been
 *      modified to ignore E_NOT_FOUND since this can happen during
 *      "ltsw" command which wipes out all LTs without going through
...
bcm_ltsw_flowtracker_detach_common(int unit)
{
    ...
    if ((warmboot) && (ft_info->host_mem)) {
        ...
        LOG_WARN(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                 "FT(unit %d) Warn: Warmboot not supported when host memory is enabled.\n"),
                 unit));
    ...
    if (ft_info->hw_learn_en == FT_HW_LEARN_ENABLE_WO_EAPP) {
        /* Un-subscribe from the flow learn notifications from SDKLT */
        SHR_IF_ERR_EXIT(
                bcm_ltsw_flowtracker_learn_fail_event_state_subscribe_set(unit, false));
    }
    ...
    if (!warmboot) {
        ...
        if (ft_info->hw_learn_en == FT_HW_LEARN_ENABLE_WO_EAPP) {
            SHR_IF_ERR_VERBOSE_EXIT(
                    bcm_ltsw_flowtracker_free_em_group_info(unit));
        }
        ...
        for (pipe = 0; pipe < ft_info->num_pipes; pipe++) {
            if (ft_info->flex_ctr_stat_id[pipe]) {
                SHR_IF_ERR_EXIT_EXCEPT_IF(
                        bcm_ltsw_flexctr_action_destroy(unit,
                                    ft_info->flex_ctr_stat_id[pipe]),
                        SHR_E_NOT_FOUND);
            }
        }
        ...
        if (hw_learn_en) {
            SHR_IF_ERR_EXIT_EXCEPT_IF(
                    ...
                    SHR_E_NOT_FOUND);
            rv = mbcm_ltsw_flowtracker_hit_index_compaction_ctrl_delete(unit);
        ...
        /*
         * SQA does warmboot from within the bcm.user without re-loading it.
         * In such a case, this being non NULL will make us access it during
         * reload_ltsw thinking the module was in initialized state.
         */
        ft_glb_info[unit] = NULL;
    }
exit:
        ...
        bcm_flowtracker_group_info_t *flow_group_info)
{
    ...
    if (ft_info->hw_learn_en != FT_HW_LEARN_ENABLE_WO_EAPP) {
        ...
        /*
         * Using the field group of one pipe (0) is enough since this is
         * for fetching the qualifier info which is same across all pipes.
         */
        em_group_id = flow_group_info->field_group[0] & BCM_FIELD_ID_MASK;
        /* User app based FT - Fetch and keep the em group info ready */
        SHR_IF_ERR_VERBOSE_EXIT(bcm_ltsw_flowtracker_field_em_group_info_get(
                                        unit, em_group_id,
                                        &(ft_em_group_info[unit][em_group_id])));
        ft_info->ft_grp_to_em_grp_map[*flow_group_id] = em_group_id;
    }
...
exit:
    if (SHR_FUNC_ERR()) {
        ...
        if (!(options & BCM_FLOWTRACKER_GROUP_REPLACE)) {
            if (id_alloced == true) {
                (void)bcm_ltsw_flowtracker_list_id_clr(unit, ft_info->flow_group_pool,
                            *flow_group_id);
            }
        ...
        bcm_flowtracker_template_transmit_config_t *config)
{
    ...
    bcm_collector_info_t collector_info;
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    bcmi_ltsw_collector_transport_type_t coll_type;
    ...
    export_template.collector_type = bcmi_ltsw_collector_int_transport_type_get(collector_info.transport_type);
        ...
        bcm_flowtracker_elephant_profile_t profile_id)
{
    ...
    if (!bcm_ltsw_flowtracker_list_id_present(unit, ft_info->elph_profile_pool, profile_id)) {
        LOG_ERROR(BSL_LOG_MODULE,
                (BSL_META_U(unit,
    ...
    if (!bcm_ltsw_flowtracker_list_id_present(unit, ft_info->flow_group_pool, flow_group_id)) {
        LOG_ERROR(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "FT(unit %d) Error: Group (ID:%d) does not exist.\n"),
                 unit, flow_group_id));
        ...
        int *list_size)
{
    ...
    *list_size = 0;
    for (group = 1; group < FT_MAX_FLOW_GROUPS; group++) {
        if (bcm_ltsw_flowtracker_list_id_present(unit, ft_info->flow_group_pool, group)) {
            if (max_size != 0) {
        ...
        bcm_flowtracker_export_template_t template_id)
{
    ...
    entry.collector_type = bcmi_ltsw_collector_int_transport_type_get(collector_info.transport_type);
    ...
    if (export_profile_info.wire_format == bcmCollectorWireFormatIpfix) {
        SHR_IF_ERR_EXIT(bcmi_ltsw_collector_user_update(unit,
                    collector_id, bcmiCollectorExportAppFlowtrackerIpfix));
    } else {
        SHR_IF_ERR_EXIT
                                  ...
                                  bcm_flowtracker_export_template_t template_id)
{
    ...
    /* Verify if template exists */
    if (export_profile_info.wire_format == bcmCollectorWireFormatIpfix) {
        if (!bcm_ltsw_flowtracker_list_id_present(unit, ft_info->export_template_pool, template_id)) {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "FT(unit %d) Error: Template (ID:%d) does not exist.\n"),
                     unit, template_id));
        ...
        bcm_flowtracker_elephant_profile_t *profile_id)
{
    ...
    if (options & BCM_FLOWTRACKER_ELEPHANT_PROFILE_WITH_ID) {
        ...
        rv = bcm_ltsw_flowtracker_list_id_reserve(unit, ft_info->elph_profile_pool,
                                                  *profile_id);
        id = *profile_id;
        if (SHR_FAILURE(rv)) {
            SHR_ERR_EXIT ((rv == SHR_E_RESOURCE) ? SHR_E_EXISTS : rv);
        ...
        SHR_IF_ERR_EXIT
            (bcm_ltsw_flowtracker_list_id_alloc(unit,
                                                ft_info->elph_profile_pool,FT_MAX_ELEPHANT_PROFILES+1, &id));
        *profile_id = id;
    }
    id_alloc = true;
...
exit:
    if (SHR_FAILURE(_func_rv) && (id_alloc == true)) {
        (void)bcm_ltsw_flowtracker_list_id_clr(unit, ft_info->elph_profile_pool, id);
    }
    SHR_FUNC_EXIT();
}
        ...
        bcm_flowtracker_elephant_profile_t profile_id)
{
    ...
    (void)bcm_ltsw_flowtracker_list_id_clr(unit, ft_info->elph_profile_pool,
            profile_id);
        ...
        int *count)
{
    ...
    *count       = 0;
    ...
    for (i = 1; i <= FT_MAX_ELEPHANT_PROFILES; i++) {
        if (bcm_ltsw_flowtracker_list_id_present(unit, ft_info->elph_profile_pool, i)) {
            /* Profile exists */
            num_profiles++;
            if (*count < max) {
        ...
        bcm_flowtracker_export_element_info_t *list_of_export_elements)
{
    ...
    if (options & BCM_FLOWTRACKER_EXPORT_TEMPLATE_WITH_ID) {
        /* Reserve the requested Id */
        rv = bcm_ltsw_flowtracker_list_id_reserve(unit, ft_info->export_template_pool,
                *id);
        profile_id = *id;
        if (SHR_FAILURE(rv)) {
            SHR_ERR_EXIT ((rv == SHR_E_RESOURCE) ? SHR_E_EXISTS : rv);
        ...
        SHR_IF_ERR_EXIT
            (bcm_ltsw_flowtracker_list_id_alloc(unit,
             ft_info->export_template_pool, FT_MAX_EXPORT_TEMPLATES+1, &profile_id));
        if (SHR_FAILURE(rv)) {
            SHR_ERR_EXIT(rv);
        }
    }
...
exit:
    if (SHR_FAILURE(_func_rv) && (id_alloc == true)) {
        (void)bcm_ltsw_flowtracker_list_id_clr(unit, ft_info->export_template_pool, profile_id);
    }
    SHR_FUNC_EXIT();
}
        ...
        bcm_flowtracker_export_template_t id)
{
    ...
    if (!bcm_ltsw_flowtracker_list_id_present(unit, ft_info->export_template_pool, id)) {
        LOG_ERROR(BSL_LOG_MODULE,
                (BSL_META_U(unit,
    ...
    (void)bcm_ltsw_flowtracker_list_id_clr(unit, ft_info->export_template_pool, id);
        ...
        int *list_size)
{
    ...
    if (entry.collector_type == bcmiCollectorTransportTypeIpv4Udp) {
        collector_id = entry.collector_ipv4_id;
    } else if (entry.collector_type == bcmiCollectorTransportTypeIpv6Udp) {
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_notify_config_set_common
 * Purpose:
 *      Set the flow learn notification configuration.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 *      pipe                    - (IN)  Port pipe number
 *      config                  - (IN)  Flow learn notification configuration.
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_notify_config_set_common(
    int unit,
    int pipe,
    bcm_flowtracker_flow_learn_notify_config_t  *config)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    bcm_int_ft_learn_evt_ctrl_t ctrl_entry;
    bcm_int_ft_learn_fail_evt_ctrl_t fail_ctrl_entry;
    ...
    if ((config->fail_enable) && (config->fail_threshold == 0)) {
        /*
         * 0 is not a valid fail threshold if you are
         * enabling failure notification
         */
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (config->fail_threshold == 0) { /* Special case */
        /* LT would not accept 0. So set 1 which is the default value */
        fail_threshold = 1;
        /*
         * Set configured = false to ensure we do not return value 1
         * which will be returned by LT in get API.
         */
        ft_info->fail_threshold_configured[pipe] = false;
    } else {
        fail_threshold = config->fail_threshold; 
        ft_info->fail_threshold_configured[pipe] = true;
    }
    sal_memset(&ctrl_entry, 0, sizeof(ctrl_entry));
    ctrl_entry.pipe = pipe;
    ctrl_entry.notif_learn = config->enable;
    ctrl_entry.flags = config->flags;
    ctrl_entry.fail_threshold = fail_threshold;
    SHR_IF_ERR_EXIT(bcm_ltsw_flowtracker_learn_event_control_entry_set(unit, 
                &ctrl_entry));
    sal_memset(&fail_ctrl_entry, 0, sizeof(fail_ctrl_entry));
    fail_ctrl_entry.pipe = pipe;
    fail_ctrl_entry.intr_enable = config->fail_enable;
    SHR_IF_ERR_EXIT(bcm_ltsw_flowtracker_learn_fail_event_control_entry_set(
                unit, &fail_ctrl_entry));
exit:
    SHR_FUNC_EXIT();
}
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_notify_config_get_common
 * Purpose:
 *      Get the flow learn notification configuration.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 *      pipe                    - (IN)  Port pipe number
 *      config                  - (OUT) Flow learn notification configuration.
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_notify_config_get_common(
    int unit,
    int pipe,
    bcm_flowtracker_flow_learn_notify_config_t  *config)
{
    ...
    sal_memset(&ctrl_entry, 0, sizeof(ctrl_entry));
    ctrl_entry.pipe = pipe;
    SHR_IF_ERR_EXIT(bcm_ltsw_flowtracker_learn_event_control_entry_get(unit, 
                &ctrl_entry));
    config->enable = ctrl_entry.notif_learn;
    config->flags = ctrl_entry.flags;
    if(ft_info->fail_threshold_configured[pipe]) {
        config->fail_threshold = ctrl_entry.fail_threshold;
    } else {
        config->fail_threshold = 0;
    }
    sal_memset(&fail_ctrl_entry, 0, sizeof(fail_ctrl_entry));
    fail_ctrl_entry.pipe = pipe;
    SHR_IF_ERR_EXIT(bcm_ltsw_flowtracker_learn_fail_event_control_entry_get(unit, 
                &fail_ctrl_entry));
    config->fail_enable = fail_ctrl_entry.intr_enable;
exit:
    SHR_FUNC_EXIT();
}
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_notify_register_common
 * Purpose:
 *      Register a callback for flow learn notifications.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_notify_register_common(
    int unit,
    ...
    void *user_data)
{
    ...
    ft_learn_user_cb[unit] = cb;
    ft_learn_user_cb_data[unit] = user_data;

    /* Subscribe for the flow learn notifications from SDKLT */
    SHR_IF_ERR_EXIT(
        bcm_ltsw_flowtracker_learn_event_log_subscribe_set(unit, true));
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_notify_unregister_common
 * Purpose:
 *      Unregister the callback for flow learn notifications.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_notify_unregister_common(int unit)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    SHR_FUNC_ENTER(unit);
    ...
    if (ft_learn_user_cb[unit] == NULL) {
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }

    /* Un-subscribe from the flow learn notifications from SDKLT */
    SHR_IF_ERR_EXIT(
        bcm_ltsw_flowtracker_learn_event_log_subscribe_set(unit, false));

    ft_learn_user_cb[unit] = NULL;
    ft_learn_user_cb_data[unit] = NULL;
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_fail_count_get_common
 * Purpose:
 *      Get the number of flows failed to learn.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 *      pipe                    - (IN)  Port pipe number
 *      count                   - (OUT) Flow learn failure count.
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_fail_count_get_common(
    int unit,
    int pipe,
    uint32 *count)
{
    ...
    entry.pipe = pipe;
    SHR_IF_ERR_EXIT(
        bcm_ltsw_flowtracker_learn_event_stats_get(unit, &entry));
    *count = entry.num_learn_fail;
exit:
    SHR_FUNC_EXIT();
}
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flexctr_action_info_get_common
 * Purpose:
 *      Get the flex ctr action allocated by the flowtracker module.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 *      flexctr_action_info     - (OUT) Flex ctr action information.
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flexctr_action_info_get_common(
    int unit,
    bcm_flowtracker_flexctr_action_info_t *flexctr_action_info)
{
    ...
    SHR_NULL_CHECK(flexctr_action_info, SHR_E_PARAM);
    SHR_NULL_CHECK(ft_info, SHR_E_INIT);

    /* This API is valid only if flex ctr allocation is done in Flowtracker */
    if (!(ft_info->flex_ctr_alloc_enable)) {
        SHR_ERR_EXIT(SHR_E_CONFIG);
    }

    for (pipe = 0; pipe < ft_info->num_pipes; pipe++) {
        flexctr_action_info->pkt_byte_flexctr_action_id[pipe] =
                                    ft_info->flex_ctr_stat_id[pipe];
    }
exit:
    SHR_FUNC_EXIT();
...
/*
 * Function:
 *      bcmi_ltsw_flowtracker_mirror_index_get
 * Purpose:
 *      Internal function to get the mirror index used by the flowtracker group.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 *      flow_group_id           - (IN)  Flowtracker group id
 *      mirror_index            - (OUT) Mirror index used by that group
 *                                      if mirror action is present.
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcmi_ltsw_flowtracker_mirror_index_get(int unit,
                                           bcm_flowtracker_group_t flow_group_id,
                                           int *mirror_index)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    ...
    sal_memset(&flow_group_int_info, 0, sizeof(flow_group_int_info));
    flow_group_int_info.flow_group_id = flow_group_id;
    /* Set this flag to indicate that we need only mirror index and not Gport */
    flow_group_int_info.flags |= FT_GRP_ENTRY_GET_MIRROR_IDX_ONLY;
    /* Get the flow group info for this group id */
    SHR_IF_ERR_EXIT(bcm_ltsw_flowtracker_group_entry_get(unit,
                &flow_group_int_info));
    /*
     * Check if one of the actions is mirror.
     * If yes, get the mirror index used by the group.
     */
    if (bcm_ltsw_flowtracker_group_actions_contains_mirror(
                flow_group_int_info.num_actions,
                flow_group_int_info.action_list,
                &mirror_flags, mirror_index)) {
        if (mirror_index == 0) {
            /*
             * This is mirror cancel case. No gport or mirror destination
             * is associated. Do not return this
             */
            SHR_ERR_EXIT(SHR_E_NOT_FOUND);
        }
        SHR_EXIT();
    } else {
        /* If mirror action was not found, return not found */
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }
exit:
    SHR_FUNC_EXIT();
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_fail_notify_register_common
 * Purpose:
 *      Register a callback for flow learn failure notifications.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 *      cb                      - (IN)  Callback function.
 *      user_data               - (IN)  Cookie.
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_fail_notify_register_common(
    int unit,
    ...
    void *user_data)
{
    ...
    ft_learn_fail_user_cb[unit] = cb;
    ft_learn_fail_user_cb_data[unit] = user_data;
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_fail_notify_unregister_common
 * Purpose:
 *      Unregister the callback for flow learn failure notifications.
 * Parameters:
 *      unit                    - (IN)  BCM device number
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_fail_notify_unregister_common(int unit)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(ft_info, SHR_E_INIT);
    ...
    if (ft_learn_fail_user_cb[unit] == NULL) {
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }

    ft_learn_fail_user_cb[unit] = NULL;
    ft_learn_fail_user_cb_data[unit] = NULL;
