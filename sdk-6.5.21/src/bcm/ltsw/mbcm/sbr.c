/*
 * DO NOT EDIT THIS FILE!
 * This file is auto-generated.
 * Edits to this file will be lost when it is regenerated.
 *
 * This license is set out in https://raw.githubusercontent.com/Broadcom-Network-Switching-Software/OpenBCM/master/Legal/LICENSE file.
 * 
 * Copyright 2007-2020 Broadcom Inc. All rights reserved.
 */

#include <bcm/types.h>

#include <bcm_int/ltsw/mbcm/sbr.h>
#include <bcm_int/ltsw/mbcm/mbcm.h>

#include <shr/shr_debug.h>

/******************************************************************************
 * Local definitions
 */

#define BSL_LOG_MODULE BSL_LS_BCM_COMMON

/*!
 * \brief The Sbr mbcm drivers.
 */
static mbcm_ltsw_sbr_drv_t mbcm_ltsw_sbr_drv[BCM_MAX_NUM_UNITS];

/******************************************************************************
 * Private functions
 */

/******************************************************************************
 * Public functions
 */

int
mbcm_ltsw_sbr_drv_set(
    int unit,
    mbcm_ltsw_sbr_drv_t *drv)
{
    mbcm_ltsw_sbr_drv_t *local = &mbcm_ltsw_sbr_drv[unit];

    SHR_FUNC_ENTER(unit);


    mbcm_ltsw_drv_init((mbcm_ltsw_drv_t *)local,
                       (mbcm_ltsw_drv_t *)drv,
                       sizeof(*local)/MBCM_LTSW_FNPTR_SZ);

    SHR_EXIT();

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_sbr_db_get(
    int unit,
    bcmint_sbr_db_t *sbr_db)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_sbr_drv[unit].sbr_db_get(unit, sbr_db));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_sbr_ent_status_get(
    int unit,
    bcmi_ltsw_sbr_profile_tbl_hdl_t tbl_hdl,
    bcmi_ltsw_sbr_profile_ent_type_t ent_type,
    uint32_t *status)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_sbr_drv[unit].sbr_ent_status_get(unit, tbl_hdl, ent_type, status));

exit:
    SHR_FUNC_EXIT();
}

