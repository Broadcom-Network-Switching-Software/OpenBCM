/*
 * DO NOT EDIT THIS FILE!
 * This file is auto-generated.
 * Edits to this file will be lost when it is regenerated.
 *
 * This license is set out in https://raw.githubusercontent.com/Broadcom-Network-Switching-Software/OpenBCM/master/Legal/LICENSE file.
 * 
 * Copyright 2007-2020 Broadcom Inc. All rights reserved.
 */

#include <bcm/types.h>

#include <bcm_int/ltsw/mbcm/flexstate.h>
#include <bcm_int/ltsw/mbcm/mbcm.h>

#include <shr/shr_debug.h>

/******************************************************************************
 * Local definitions
 */

#define BSL_LOG_MODULE BSL_LS_BCM_FLEXSTATE

/*!
 * \brief The Flexstate mbcm drivers.
 */
static mbcm_ltsw_flexstate_drv_t mbcm_ltsw_flexstate_drv[BCM_MAX_NUM_UNITS];

/******************************************************************************
 * Private functions
 */

/******************************************************************************
 * Public functions
 */

int
mbcm_ltsw_flexstate_drv_set(
    int unit,
    mbcm_ltsw_flexstate_drv_t *drv)
{
    mbcm_ltsw_flexstate_drv_t *local = &mbcm_ltsw_flexstate_drv[unit];

    SHR_FUNC_ENTER(unit);


    mbcm_ltsw_drv_init((mbcm_ltsw_drv_t *)local,
                       (mbcm_ltsw_drv_t *)drv,
                       sizeof(*local)/MBCM_LTSW_FNPTR_SZ);

    SHR_EXIT();

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_source_info_get(
    int unit,
    int source,
    bcmint_flexstate_source_info_t **source_info)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_source_info_get(unit, source, source_info));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_object_info_get(
    int unit,
    int object,
    bcmint_flexstate_object_info_t **object_info)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_object_info_get(unit, object, object_info));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_stage_info_get(
    int unit,
    bcmint_flexstate_stage_info_t **stage_info)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_stage_info_get(unit, stage_info));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_action_mode_validate(
    int unit,
    int val)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_action_mode_validate(unit, val));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_action_mode_enum_map_get(
    int unit,
    const bcmint_flexstate_enum_map_t **map,
    uint32_t *count)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_action_mode_enum_map_get(unit, map, count));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_action_source_validate(
    int unit,
    int val)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_action_source_validate(unit, val));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_action_source_enum_map_get(
    int unit,
    const bcmint_flexstate_enum_map_t **map,
    uint32_t *count)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_action_source_enum_map_get(unit, map, count));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_trigger_interval_validate(
    int unit,
    int val)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_trigger_interval_validate(unit, val));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_action_scale_enum_map_get(
    int unit,
    const bcmint_flexstate_enum_map_t **map,
    uint32_t *count)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_action_scale_enum_map_get(unit, map, count));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_quant_object_enum_map_get(
    int unit,
    const bcmint_flexstate_enum_map_t **map,
    uint32_t *count)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_quant_object_enum_map_get(unit, map, count));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_update_comp_validate(
    int unit,
    int val)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_update_comp_validate(unit, val));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_update_comp_enum_map_get(
    int unit,
    const bcmint_flexstate_enum_map_t **map,
    uint32_t *count)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_update_comp_enum_map_get(unit, map, count));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_update_mode_validate(
    int unit,
    int val)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_update_mode_validate(unit, val));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_update_mode_enum_map_get(
    int unit,
    const bcmint_flexstate_enum_map_t **map,
    uint32_t *count)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_update_mode_enum_map_get(unit, map, count));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_update_source_validate(
    int unit,
    int val)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_update_source_validate(unit, val));

exit:
    SHR_FUNC_EXIT();
}

int
mbcm_ltsw_flexstate_update_source_enum_map_get(
    int unit,
    const bcmint_flexstate_enum_map_t **map,
    uint32_t *count)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_flexstate_drv[unit].flexstate_update_source_enum_map_get(unit, map, count));

exit:
    SHR_FUNC_EXIT();
}

