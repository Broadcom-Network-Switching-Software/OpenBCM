...
extern int _bd##_ltsw_collector_drv_attach(int unit); \
...
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
    {{   \
        ...
        _bd##_ltsw_collector_drv_attach \
     }},
static struct {
...
{
    (void)mbcm_ltsw_feature_drv_set(unit, NULL);

    (void)mbcm_ltsw_dev_drv_set(unit, NULL);

    (void)mbcm_ltsw_dev_misc_drv_set(unit, NULL);

    (void)mbcm_ltsw_l2_drv_set(unit, NULL);

    (void)mbcm_ltsw_l3_drv_set(unit, NULL);

    (void)mbcm_ltsw_l3_intf_drv_set(unit, NULL);

    (void)mbcm_ltsw_l3_egress_drv_set(unit, NULL);

    (void)mbcm_ltsw_l3_fib_drv_set(unit, NULL);

    (void)mbcm_ltsw_port_drv_set(unit, NULL);

    (void)mbcm_ltsw_stg_drv_set(unit, NULL);

    (void)mbcm_ltsw_vlan_drv_set(unit, NULL);

    (void)mbcm_ltsw_rate_drv_set(unit, NULL);

    (void)mbcm_ltsw_trunk_drv_set(unit, NULL);

    (void)mbcm_ltsw_flow_drv_set(unit, NULL);

    (void)mbcm_ltsw_stat_drv_set(unit, NULL);

    (void)mbcm_ltsw_virtual_drv_set(unit, NULL);

    (void)mbcm_ltsw_flexctr_drv_set(unit, NULL);

    (void)mbcm_ltsw_sc_drv_set(unit, NULL);

    (void)mbcm_ltsw_time_drv_set(unit, NULL);

    (void)mbcm_ltsw_cosq_drv_set(unit, NULL);

    (void)mbcm_ltsw_cosq_thd_drv_set(unit, NULL);

    (void)mbcm_ltsw_cosq_stat_drv_set(unit, NULL);

    (void)mbcm_ltsw_cosq_bst_drv_set(unit, NULL);

    (void)mbcm_ltsw_cosq_ebst_drv_set(unit, NULL);

    (void)mbcm_ltsw_tunnel_drv_set(unit, NULL);

    (void)mbcm_ltsw_failover_drv_set(unit, NULL);

    (void)mbcm_ltsw_mirror_drv_set(unit, NULL);

    (void)mbcm_ltsw_multicast_drv_set(unit, NULL);

    (void)mbcm_ltsw_mpls_drv_set(unit, NULL);

    (void)mbcm_ltsw_field_drv_set(unit, NULL);

    (void)mbcm_ltsw_range_drv_set(unit, NULL);

    (void)mbcm_ltsw_stack_drv_set(unit, NULL);

    (void)mbcm_ltsw_dlb_drv_set(unit, NULL);

    (void)mbcm_ltsw_ipmc_drv_set(unit, NULL);

    (void)mbcm_ltsw_sbr_drv_set(unit, NULL);

    (void)mbcm_ltsw_pfc_drv_set(unit, NULL);

    (void)mbcm_ltsw_obm_drv_set(unit, NULL);

    (void)mbcm_ltsw_qos_drv_set(unit, NULL);

    (void)mbcm_ltsw_nat_drv_set(unit, NULL);

    (void)mbcm_ltsw_flowtracker_drv_set(unit, NULL);

    (void)mbcm_ltsw_oob_drv_set(unit, NULL);

    (void)mbcm_ltsw_flexdigest_drv_set(unit, NULL);

    (void)mbcm_ltsw_cosq_mod_drv_set(unit, NULL);

    (void)mbcm_ltsw_uft_drv_set(unit, NULL);

    (void)mbcm_ltsw_rx_drv_set(unit, NULL);

    (void)mbcm_ltsw_hos_drv_set(unit, NULL);

    (void)mbcm_ltsw_udf_drv_set(unit, NULL);

    (void)mbcm_ltsw_lb_hash_drv_set(unit, NULL);

    (void)mbcm_ltsw_policer_drv_set(unit, NULL);

    (void)mbcm_ltsw_flexstate_drv_set(unit, NULL);

    (void)mbcm_ltsw_pktio_drv_set(unit, NULL);

    (void)mbcm_ltsw_ldh_drv_set(unit, NULL);

    (void)mbcm_ltsw_uft_drv_set(unit, NULL);

    (void)mbcm_ltsw_ifa_drv_set(unit, NULL);

#if defined(INCLUDE_INT)
    (void)mbcm_ltsw_int_drv_set(unit, NULL);
#endif

    (void)mbcm_ltsw_l3_aacl_drv_set(unit, NULL);

    (void)mbcm_ltsw_collector_drv_set(unit, NULL);
