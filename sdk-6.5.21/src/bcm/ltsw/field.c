    ...
    int unit)
{
    ...
    rv = bcmi_ltsw_issu_struct_info_get(unit,
    ...
    if (rv != SHR_E_NOT_FOUND) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_issu_struct_info_clear(unit,
                                              BCMI_FIELD_HA_BLK_HDR_T_ID,
                                              BCMI_HA_COMP_ID_FIELD,
                                              blk_id));
    }
...
bcmint_field_db_detach(int unit, bool warm, bool init)
{
    ...
    for (stage_id = bcmiFieldStageIngress;
        ...
        (void)(mbcm_ltsw_field_stage_cleanup(unit, stage_info));
    }
                              ...
                              uint32_t width)
{
    ...
    uint64_t mask = 0, clr_mask = 0, grp_mask = 0, grp_mask2 = 0;
    uint8_t qual_part_num = 0;

    obuf_ptr = ((uint64_t *)op_buf + (dst_offset / 64));
    dst_bit_offset = (dst_offset % 64);
    mask = (mask64 >> (64 - width));
    if (grp_qual_mask != NULL) {
        qual_part_num = dst_offset / 64;
        if (qual_part_num) {
            grp_mask = grp_qual_mask[qual_part_num];
        } else {
            grp_mask = *(uint64_t *)grp_qual_mask;
        }
    } else {
        grp_mask = mask64;
    }
    ...
    case BCM_QUAL_DATA_TYPE_UINT8:
        if (width > 8) {
            ...
            *obuf_ptr &= clr_mask;
            *obuf_ptr |=
                (((*(uint64_t *)ibuf_ptr >> src_bit_offset) & mask) << dst_bit_offset);
        } else {
            ibuf_ptr = ((uint8_t *)inp_buf + (src_offset / 8));
            src_bit_offset = (src_offset % 8);
            *obuf_ptr &= clr_mask;
            *obuf_ptr |=
                (((*(uint8_t *)ibuf_ptr >> src_bit_offset) & mask) << dst_bit_offset);
        }
        *obuf_ptr &= grp_mask;
        break;
    ...
    case BCM_QUAL_DATA_TYPE_IP6:
        if ((dst_offset + width) > 64 && (dst_offset < 64)) {
            ...
            if (grp_qual_mask != NULL) {
                grp_mask2 = grp_qual_mask[1];
            } else {
                grp_mask2 = mask64;
            }
            clr_mask = ~(mask);
            obuf_ptr[1] &= clr_mask;
            obuf_ptr[1] |= (tmp64[0] & mask);
            obuf_ptr[1] &= grp_mask2;
            ...
            clr_mask = ~(mask << dst_bit_offset);
            obuf_ptr[0] &= clr_mask;
            obuf_ptr[0] |= ((tmp64[0] & mask) << dst_bit_offset);
            obuf_ptr[0] &= grp_mask;
        } else {
            ...
            clr_mask = ~(mask << dst_bit_offset);
            *obuf_ptr &= clr_mask;
            *obuf_ptr |= ((tmp64[0] & mask) << dst_bit_offset);
            obuf_ptr[0] &= grp_mask;
        }
        break;
    default:
                              ...
                              uint32_t width)
{
    ...
    case BCM_QUAL_DATA_TYPE_UINT8:
        if (width > 8) {
            ...
            dst_bit_offset = (dst_offset % 64);
            *(uint64_t *)obuf_ptr |=
                     (((*ibuf_ptr >> src_bit_offset) & mask) << dst_bit_offset);
        } else {
    ...
    case BCM_QUAL_DATA_TYPE_IP6:
        if ((src_offset + width) > 64 && (src_offset < 64)) {
            ...
            obuf_ptr = ((uint8_t *)op_buf + (dst_offset / 8));
            mask = (mask64 >> (64 - width));
            tmp64[0] = ((ibuf_ptr[0] >> src_bit_offset) & mask);
            ...
            obuf_ptr = ((uint8_t *)op_buf + (dst_offset / 8));
            mask = (mask64 >> (64 - width));
            tmp64[0] = ((*ibuf_ptr >> src_bit_offset) & mask);
            bcmi_ltsw_util_uint64_to_generic(obuf_ptr, tmp64, width);
        }
        break;
    default:
                                   ...
                                   uint32_t *qual_mask_value)
{
    ...
    if (map->width[0] == 0) {
        ...
        if ((fld_defs->dtag == BCMLT_FIELD_DATA_TAG_BOOL)
            ...
            dst_offset = 0;
            if (qual_info->flags & BCM_FIELD_F_ARRAY &&
                qual_info->data_type == BCM_QUAL_DATA_TYPE_UINT8) {
                /* Check input qual length. copy only till input length. */
                if (*src_ofst_current >= qual_info->depth * 8) {
                    /* Full depth is already written. */
                    return;
                } else if ((*src_ofst_current + width) > qual_info->depth * 8) {
                    /*
                     * Width of current element is not utilized by user fully.
                     * extract the partial mask and value set by user
                     */
                    width = (qual_info->depth * 8) - *src_ofst_current;
                }
            }
                                ...
                                bcm_qual_field_t *qual_info)
{
    switch(qual_info->data_type) {
        ...
        case BCM_QUAL_DATA_TYPE_UINT8:
            ...
            } else if (qual_info->flags & BCM_FIELD_F_ARRAY)  {
                if ((qual_info->depth * 8) >
                    (width)) {
                    return SHR_E_PARAM;
                }
            } else {
                                 ...
                                 bcmlt_entry_handle_t rule_template)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_field_compression_set(unit, stage_info, e_oper,
                                         &qset, 0x0,
                                         rule_template, &c_flags));
    if (e_oper) {
        e_oper->entry_flags |= c_flags;
    }
...
int
bcmi_ltsw_field_qualifier_macro_validate(int unit,
                                         ...
                                         bcm_qual_field_values_t *qual_info_values)
{
    int entry_id = 0;
    int i = 0;
    uint32_t macro_flag = 0;
    uint32_t flags_supported = 0;
    shr_error_t rv = SHR_E_NONE;
    bcm_field_hintid_t hintid = 0;
    bcmint_field_stage_info_t *stage_info = NULL;
    ...
    if (BCMINT_FIELD_IS_PRESEL_ENTRY(entry)) {
        ...
        hintid = group_oper->hintid;
    }

    /* Get all flags supported */
    for (i = 0; i < qual_info_values->num; i++) {
        flags_supported |= qual_info_values->macro_flags[i];
    }

    /* Validate incoming data with supported flags */
    if ((*(uint32 *)qual_info->qual_data) & (~(flags_supported))) {
        SHR_ERR_EXIT(BCM_E_PARAM);
    }
    if ((*(uint32 *)qual_info->qual_mask) & (~(flags_supported))) {
        SHR_ERR_EXIT(BCM_E_PARAM);
    }
    ...
    if (hintid != 0) {
        ...
        hint.qual = qual_info->sdk6_qual_enum;
        rv = bcmint_field_hints_get(unit, hintid, &hint);
        if (SHR_SUCCESS(rv) &&
            (hint.flags & BCM_FIELD_HINT_QUALIFIER_MACROS_SELECT)) {
            for (i = 0; i < qual_info_values->num; i++) {
                macro_flag = qual_info_values->macro_flags[i];
                if (((*(uint32 *)qual_info->qual_mask) & macro_flag) ||
                    ((*(uint32 *)qual_info->qual_data) & macro_flag)) {
                    if (!(hint.value & macro_flag)) {
                              ...
                              bcm_qual_field_t *qual_info)
{
    ...
    int pbm_pipe = -1;
    shr_error_t rv = SHR_E_NONE;
    int dunit = 0, idx = 0, iter = 0, grp_iter = 0;
    uint8_t qual_value_inp_buf_size = 0;
    bcmint_field_stage_info_t *stage_info = NULL;
    bcm_field_qual_map_t *lt_map_ref = NULL;
    bcm_field_qual_map_t *grp_lt_map_ref = NULL;
    ...
    bcmlt_entry_handle_t grp_template_hdl = BCMLT_INVALID_HDL;
    ...
    uint16 byte;
    int r_offset;
    uint64_t *grp_qual_buf = NULL;
    int grp_map_count = 0;
    bool extract_group_qual_mask = false;
    bool ip6_qual = false;
    ...
    BCMINT_FIELD_MEM_ALLOC
        (grp_fld_defs_ref, sizeof(bcmlt_field_def_t),
    ...
    if (BCMINT_FIELD_IS_PRESEL_ENTRY(entry)) {
        ...
        grp_template_name = stage_info->tbls_info->group_sid;
        grp_template_id_name = stage_info->tbls_info->group_key_fid;
        ...
        if (hintid != 0) {
            /* Entry handle allocate for grp template */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_allocate(dunit,
                                      grp_template_name,
                                      &grp_template_hdl));

            /* Add grp_template_id field to grp LT. */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(grp_template_hdl,
                                       grp_template_id_name,
                                       entry_oper->group_id));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_attrib_set(grp_template_hdl,
                            BCMLT_ENT_ATTR_FILTER_DEFAULTS));

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_qual_map_find(unit,
                                            qual_info->sdk6_qual_enum,
                                            stage_info, 0, 1, &grp_map_count,
                                            &grp_lt_map_ref));

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_lt_field_def_get(unit,
                        grp_template_name,
                        (grp_lt_map_ref->map[grp_iter]).lt_field_name,
                        grp_fld_defs_ref));
            /*
             * For bool type fields, hints are not applied and hence
             * no need of sending explicit group qual mask
             */
            if (!(grp_fld_defs_ref->dtag ==
                    BCMLT_FIELD_DATA_TAG_BOOL)) {
                extract_group_qual_mask = true;
                /* Lookup for the grp template */
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_lt_entry_commit(unit, grp_template_hdl,
                                      BCMLT_OPCODE_LOOKUP,
                                      BCMLT_PRIORITY_NORMAL));
            }
        }
    ...
    switch(qual_info->data_type) {
        ...
        case BCM_QUAL_DATA_TYPE_UINT8:
            if (qual_info->flags & BCM_FIELD_F_ARRAY) {
                if (0 == qual_info->depth) {
                    SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
                }
                qual_value_inp_buf_size = sizeof(uint8_t) * qual_info->depth;
            } else {
            ...
            if (qual_info->flags & BCM_FIELD_F_ENUM) {
                ...
                sal_memcpy(qual_value_inp_buf,
                           *(uint8_t **)qual_info->qual_data,
                           qual_value_inp_buf_size);
                sal_memcpy(qual_mask_inp_buf,
                            *(uint8_t **)qual_info->qual_mask,
                            qual_value_inp_buf_size);
                sal_memset(&ref_data, 0, sizeof(ref_data));
                sal_memset(&ref_mask, 0, sizeof(ref_mask));

                r_idx =  (qual_info->depth * 8)/ 32;
                r_offset = (qual_info->depth * 8) % 32;
                ...
                for (byte = 0; byte < qual_info->depth; byte++) {
                    ref_data[r_idx] |= qual_value_inp_buf[byte] << shift;
                    ref_mask[r_idx] |= qual_mask_inp_buf[byte] << shift;
                    if (shift < 8) {
                        shift = 24;
                        r_idx--;
                    }
        ...
        case BCM_QUAL_DATA_TYPE_MPLS_LABEL:
            ...
            tmp_qual_val_inp_buf = *(uint32_t *)qual_value_inp_buf;
            break;
        ...
        case BCM_QUAL_DATA_TYPE_PBMP:
            ...
            /* group_oper cannot be NULL. It should not reach
             * here for presel groups as presel group does not
             * have any qualifier with pbmp as input data type */
            if (group_oper == NULL) {
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
            }
            SHR_IF_ERR_VERBOSE_EXIT
                (mbcm_ltsw_field_pbmp_index_get(unit,
                                         ...
                                         (bcm_pbmp_t *)qual_info->qual_mask,
                                          qual_value_inp_buf,
                                          qual_mask_inp_buf));
            /* Count the number of bits(ports) in the key. */
            SHR_BITCOUNT_RANGE((uint32_t *)qual_value_inp_buf,
                               pbmp_bits,
                               0,
                               (qual_value_inp_buf_size * 8));
            break;
    ...
    if ((hintid != 0) &&
        ...
        bool pbmp_hint_extracted = false;
        int l_port = 0;

        sal_memset(&hint_pbmp, 0, sizeof(hint_pbmp));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_hints_group_pbmp_value_update(unit,
                                                        hintid,
                                                        &pbmp_hint_extracted,
                                                        &hint_pbmp));
        if (pbmp_hint_extracted == true) {
            /*
             * When pbmp hint is present, InPorts qualifier cannot qualify
             * on more than one port.
             */
            if (pbmp_bits > 1) {
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
            }
            /* Check if pbmp mask is subset of hint pbmp. */
            BCM_PBMP_ITER(*(bcm_pbmp_t *)qual_info->qual_mask, l_port) {
                if (!(BCM_PBMP_MEMBER(hint_pbmp, l_port))) {
                    SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                }
            }
        }
    ...
    for (iter = 0; iter < lt_map_ref->num_maps; iter++) {
        ...
        if (fld_defs_iter[iter].elements > 1) {
            ...
            if (fld_defs_iter[iter].symbol) {
                ...
                if (extract_group_qual_mask == true &&
                    ...
                    if (ip6_qual &&
                        (stage_info->stage_id == bcmiFieldStageIngress) &&
                        grp_map_count > 1) {
                        /* Get the values in reverse order */
                        grp_iter = grp_map_count -1 - iter;
                    } else if (iter > (grp_map_count - 1)) {
                      /* More than 1 map's in rule template but 1 in grp_map */
                        grp_iter = 0;
                    } else {
                        grp_iter = iter;
                    }
                    ...
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmi_lt_field_def_get(unit,
                                ...
                                grp_fld_defs_ref));
                    BCMINT_FIELD_MEM_ALLOC
                        (grp_qual_buf, sizeof(uint64_t) * num_element,
                    ...
                    if (SHR_FAILURE(rv) &&
                        (rv != SHR_E_NOT_FOUND)) {
                        BCMINT_FIELD_MEM_FREE(grp_qual_buf);
                        SHR_IF_ERR_VERBOSE_EXIT(rv);
                    }
                }
            ...
            if (fld_defs_iter[iter].symbol) {
                ...
                if ((lt_map_ref)->data_type == bcmFieldDataTypeEnumtoLtField) {
                    ...
                    if (tmp_qual_val_inp_buf == (lt_map_ref->map[iter]).value) {
                        *(uint32_t *)qual_value_inp_buf = 1;
                        *(uint32_t *)qual_mask_inp_buf = 1;
                    } else {
                        *(uint32_t *)qual_value_inp_buf = 0;
                        *(uint32_t *)qual_mask_inp_buf = 0;
                    }
                    src_ofset_current = 0;
                ...
                if ((stage_oper_mode == bcmiFieldOperModePipeUnique) &&
                    ...
                    (pipe != -1)) {
                    /*
                     * Skip LT fields belonging to different pipes.
                     * Keeping this code here instead of the beginning of
                     * the for loop so that the offset "src_ofset_current"
                     * is incremented properly because the source buffer
                     * is setup to have values for all LT fields.
                     */
                    pbm_pipe = (lt_map_ref->map[iter]).value;
                    if (pbm_pipe != pipe) {
...
exit:
    ...
    if (BCMLT_INVALID_HDL != grp_template_hdl) {
        (void) bcmlt_entry_free(grp_template_hdl);
    }
    BCMINT_FIELD_MEM_FREE(qual_field);
    BCMINT_FIELD_MEM_FREE(fld_defs_ref);
    BCMINT_FIELD_MEM_FREE(grp_fld_defs_ref);
                         ...
                         bcm_qual_field_t *qual_info)
{
    ...
    bcm_field_snap_header_t snap_mask;
    uint16_t lt_map_qual_width = 0, lt_map_field_width = 0;
    uint16_t valid_bytes = 0, start_byte = 0;
    uint8  shift;
    uint8  r_idx;
    int16 byte;
    int r_offset;
    ...
    switch(qual_info->data_type) {
        ...
        case BCM_QUAL_DATA_TYPE_UINT8:
            if (qual_info->flags & BCM_FIELD_F_ARRAY)  {
                qual_value_inp_buf_size = qual_info->depth * sizeof(uint8_t);
                if (lt_map_qual_width < qual_info->depth * 8) {
                    /*
                     * If user passes count more than supported,
                     * API will return BCM_E_PARAM.
                     */
                    SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                }
            } else {
            ...
            if (fld_defs_iter[iter].symbol) {
                ...
                if ((lt_map_ref)->data_type == bcmFieldDataTypeEnumtoLtField) {
                    ...
                    if (*qual_value_op_buf == 1 && *qual_mask_op_buf == 1) {
                        *(uint32_t *)qual_value_inp_buf = (lt_map_ref->map[iter]).value;
                        *(uint32_t *)qual_mask_inp_buf = (lt_map_ref->map[iter]).value;
                        break;
                    } else {
                        BCMINT_FIELD_MEM_FREE(qual_field);
                        BCMINT_FIELD_MEM_FREE(qual_mask_field);
                     ...
                     * the Macro value and get only that LT field.
                     */
                    if (*qual_value_op_buf == 0 && *qual_mask_op_buf == 1) {
                        *qual_mask_op_buf = 0;
                    }
                }
    ...
    switch(qual_info->data_type) {
        ...
        case BCM_QUAL_DATA_TYPE_UINT8:
            if (qual_info->flags & BCM_FIELD_F_ENUM) {
                ...
                sal_memset(&ref_mask, 0, sizeof(ref_mask));
                start_byte = 0;
                for (byte = qual_info->depth - 1; byte >= 0 ; byte--) {
                    if (qual_mask_inp_buf[byte] == 0) {
                        start_byte++;
                    } else {
                        break;
                    }
                }
                valid_bytes = qual_info->depth - start_byte;
                if (valid_bytes != 0) {
                    sal_memcpy(ref_data,
                               qual_value_inp_buf,
                               qual_value_inp_buf_size);
                    sal_memcpy(ref_mask,
                               qual_mask_inp_buf,
                               qual_value_inp_buf_size);
                    sal_memset(qual_value_inp_buf, 0, qual_value_inp_buf_size);
                    sal_memset(qual_mask_inp_buf, 0, qual_value_inp_buf_size);

                    r_idx =  (valid_bytes * 8)/ 32;
                    r_offset = (valid_bytes * 8) % 32;
                    ...
                    for (byte = 0; byte < valid_bytes; byte++) {
                        qual_value_inp_buf[byte] =
                            (uint8)(ref_data[r_idx] >> shift);
                        qual_mask_inp_buf[byte] =
                            (uint8)(ref_mask[r_idx] >> shift);
                        if (shift < 8) {
                            shift = 24;
                            r_idx--;
                        }
 ...
 * \param [in] unit    - BCM device number.
 * \param [in] hint_id - Hint identifier.
 * \param [in] hint_type - Hint type.
 * \param [in] hint_size - Max hint size.
 * \param [in/out] hint_arr  - Hint entry.
 * \param [out] hints_count - Number of hints of given type
 *
 * Returns:
 *     SHR_E_INIT      - BCM unit not initialized
 *     SHR_E_NOT_FOUND - provided hintid does not exist in system
 ...
 */
int
bcm_ltsw_field_hints_multi_get(int unit,
                               bcm_field_hintid_t hint_id,
                               bcm_field_hint_type_t hint_type,
                               int hints_size,
                               bcm_field_hint_t *hints_arr,
                               int *hints_count)
{
    SHR_FUNC_ENTER(unit);
    ...
    FP_LOCK(unit);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_hints_multi_get(unit,
                                      hint_id,
                                      hint_type,
                                      hints_size,
                                      hints_arr,
    ...
    bcm_field_presel_group_config_t *group_config)
{
    ...
    FP_LOCK(unit);
    SHR_IF_ERR_EXIT
        (bcmint_field_presel_group_config_get(unit, group_config));
    ...
    bcm_field_qset_t *qset)
{
    ...
    SHR_NULL_CHECK(qset, SHR_E_PARAM);
    ...
    uint16 *map_val_arr)
{
    ...
    switch (type) {
        ...
        case bcmFieldCompressionDirectMapTypeL4SrcPort:
            sid = fp_control->comp_tbls_info->direct_mapped_l4srcport_sid;
            val_fid = fp_control->comp_tbls_info->direct_val_l4srcport_fid;
            map_fid = fp_control->comp_tbls_info->direct_map_val_l4srcport_fid;
            pipe_fid = fp_control->comp_tbls_info->pipe_fid;
            break;

        case bcmFieldCompressionDirectMapTypeL4DstPort:
            sid = fp_control->comp_tbls_info->direct_mapped_l4dstport_sid;
            val_fid = fp_control->comp_tbls_info->direct_val_l4dstport_fid;
            map_fid = fp_control->comp_tbls_info->direct_map_val_l4dstport_fid;
            pipe_fid = fp_control->comp_tbls_info->pipe_fid;
            break;
    ...
    uint16 *map_val)
{
    ...
    SHR_NULL_CHECK(map_val, SHR_E_PARAM);
                            ...
                            uint32_t *num_counters_in_use)
{
    ...
    SHR_NULL_CHECK(stage_info, SHR_E_INTERNAL);
    SHR_NULL_CHECK(num_counters_total, SHR_E_PARAM);
    SHR_NULL_CHECK(num_counters_free, SHR_E_PARAM);
    SHR_NULL_CHECK(num_counters_in_use, SHR_E_PARAM);
                            ...
                            uint32_t *num_meters_free)
{
    ...
    SHR_NULL_CHECK(stage_info, SHR_E_INTERNAL);
    SHR_NULL_CHECK(num_meters_total, SHR_E_PARAM);
    SHR_NULL_CHECK(num_meters_free, SHR_E_PARAM);
    ...
    *num_meters_total = COMPILER_64_LO(total_meters);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_policer_num_meters_in_use_get(unit,
                                                 stage_info->stage_id,
                                                 pipe,
                                                 &total_used_meters));
...
{
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(udf, SHR_E_PARAM);
    SHR_NULL_CHECK(cont, SHR_E_PARAM);

    if ((sal_strncmp(lt_field_name, "CONTAINER_2_BYTE", 16) == 0) ||
         (sal_strncmp(lt_field_name, "CONTAINER_1_BYTE", 16) == 0)) {
        *udf = TRUE;
        if (sal_strncmp(lt_field_name, "CONTAINER_2_BYTE", 16) == 0) {
    ...
    } else if ((sal_strcmp(lt_field_name, "QUAL_UDF_CHUNKS") == 0) ||
               (sal_strcmp(lt_field_name, "QUAL_UDF_CHUNKS_BITMAP") == 0)) {
        *udf = TRUE;
        *cont = qual_idx - bcmiFieldQualifyB2Chunk0;
    }
exit:
...
/* This function is used to check if a qualifier is configured */
static int
bcmint_field_is_qual_print_require(
    int unit,
    bcmlt_entry_handle_t *templ,
    bcmint_field_stage_info_t *stage_info,
    bcm_field_qset_t *grp_qset,
    bcm_field_qual_map_t *lt_qual_map,
    bcmlt_field_def_t *fld_defs_ref,
    char * sid,
    shr_pb_t *shr_pb_data_str,
    shr_pb_t *shr_pb_mask_str,
    uint32_t qual_idx,
    bool *is_print_reqd)
{
    int iter = 0;
    ...
    char *qual_field = NULL;
    uint64_t *qual_value_op_mask_buf = NULL;
    uint64_t *qual_value_set_buf = NULL;
    uint64_t *qual_value_op_data_buf = NULL;
    uint32_t num_element = 0;
    uint32_t num_element_temp = 0;
    uint8_t id = 0;
    uint32_t part_width = 0;
    uint64_t mask = 0;
    uint64_t mask64 = 0xFFFFFFFFFFFFFFFFULL;
    uint64_t *dst_ptr = NULL;
    uint32_t *dst_ptr32 = NULL;
    int idx = 0;
    bcmlt_field_def_t *fld_defs_iter = NULL;
    uint32 value = 0;
    ...
    *is_print_reqd = false;
    fld_defs_iter = fld_defs_ref;
    /* loop through all the qual_maps of a qualifier
     * for each supported map, store the data nad mask values
     */
    for (iter = (lt_qual_map->num_maps - 1); iter >=0 ; iter--) {
        ...
        if ((0 == num_element) ||
            ((num_element > 1) && (fld_defs_iter->symbol))) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
        }
        /* in case the map is a symbol, check if its default value */
        if (fld_defs_iter->symbol) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_symbol_get(*templ,
                    lt_field_name,
                    (const char **)&lt_sym_str_op));
                if (sal_strcmp(lt_sym_str_op,
                    fld_defs_iter->sym_def) != 0) {
                    *is_print_reqd = true;
                }
                shr_pb_printf(shr_pb_data_str, "%s ", lt_sym_str_op);
                fld_defs_iter++;
                continue;
        }

        BCMINT_FIELD_MEM_ALLOC
            (qual_value_op_data_buf,
            (sizeof(uint64_t) * num_element),
            "qualifier op data value");
        BCMINT_FIELD_MEM_ALLOC
            (qual_value_set_buf,
            (sizeof(uint64_t) * num_element),
            "qualifier set value");
        /* FP/EM stages doesnot require mask */
        if (!(stage_info->flags & BCMINT_FIELD_STAGE_DEFAULT_ENTRY_SUPPORT)) {
            BCMINT_FIELD_MEM_ALLOC
                (qual_value_op_mask_buf,
                (sizeof(uint64_t) * num_element),
                "qualifier op value");
            BCMINT_FIELD_MEM_ALLOC
                (qual_field,
                (strlen(lt_field_name) + 6),
                    "fp qual mask field");
            SHR_IF_ERR_VERBOSE_EXIT
                (mbcm_ltsw_field_qual_mask_lt_field_get(unit,
                     lt_field_name,
                     &qual_field));
        }

        /* retrieve the value of the lt field */
        if (num_element > 1) {
            SHR_IF_ERR_VERBOSE_EXIT
            ...
            if (!(stage_info->flags &
                     BCMINT_FIELD_STAGE_DEFAULT_ENTRY_SUPPORT)) {
                /* Retrieve the data field*/
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_array_get(*templ,
                        lt_field_name, cont, qual_value_op_data_buf,
                        num_element, &num_element_temp));
                /* Retreive from qualifier value */
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_array_get(*templ,
                        qual_field, cont, qual_value_op_mask_buf,
                         num_element, &num_element));
            } else {
                /* Retrieve the data field*/
                SHR_IF_ERR_VERBOSE_EXIT
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_get(*templ,
                    lt_field_name, qual_value_op_data_buf));
            if (!(stage_info->flags &
                     BCMINT_FIELD_STAGE_DEFAULT_ENTRY_SUPPORT)) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_get(*templ,
                        qual_field, qual_value_op_mask_buf));
            }
        }
        /* Go through all the offsets of the qual_map
         * and set the corresponding bits in qual_value_set_buf
         */
        if (num_element > 1) {
            for (idx = 0; idx < num_element; idx++) {
                offset = (idx * 64);
                dst_ptr = (qual_value_set_buf + (offset / 64));
                mask = (mask64 >> (64 - width));
            ...
            for (id = 0; id < lt_qual_map->map[iter].num_offsets;
                ...
                if ((lt_qual_map->map[iter].num_offsets == 1) &&
                             (lt_qual_map->map[iter].width[0] == 0) ) {
                    width =  fld_defs_iter->width;
                } else {
                    width = (lt_qual_map->map[iter].width[id]);
                }
                if ((offset < 64) && ((offset + width) > 64)) {
                    dst_ptr = (uint64_t *)qual_value_set_buf;
                    part_width = ((offset + width) - 64);
                    mask = (mask64 >> (64 - part_width));
                    dst_ptr[1] |= (mask);
                    width -= part_width;
                    ...
                    dst_ptr = ((uint64_t *)qual_value_set_buf + (offset / 64));
                    mask = (mask64 >> (64 - width));
                    *dst_ptr |= (mask << (offset % 64));
                }
            }
        }
        /* AND the retrieved LT field value with the qual_value_set_buf
         * the resultant is stored in qual_value_set_buf will
         * specify the configured bits of the lt map
         */
        SHR_BITAND_RANGE((uint32_t *)qual_value_set_buf,
            (uint32_t *)qual_value_op_data_buf, 0,
            ((sizeof(uint64_t) * num_element) *8),
            (uint32_t *)qual_value_op_data_buf);
        if (!(stage_info->flags &
                     BCMINT_FIELD_STAGE_DEFAULT_ENTRY_SUPPORT)) {
            SHR_BITAND_RANGE((uint32_t *)qual_value_set_buf,
                (uint32_t *)qual_value_op_mask_buf, 0,
                ((sizeof(uint64_t) * num_element) *8),
                (uint32_t *)qual_value_set_buf);

            for (idx = 0; idx < num_element*2; idx++) {
                dst_ptr32 = (uint32_t *)qual_value_set_buf;
                if (dst_ptr32[idx]) {
                    *is_print_reqd = true;
                }
            }
        }
        /* store the data and mask values in shr_pb_data_str
         *  and shr_pb_mask_str
         */
        if ((fld_defs_iter->dtag == BCMLT_FIELD_DATA_TAG_BOOL)
            && (fld_defs_iter->width == 1)
            && (num_element > 1)) {
            BCM_PBMP_CLEAR(pbmp);
            BCM_PBMP_CLEAR(pbmp_mask);
            for (idx = 0; idx < num_element; idx++) {
                if (COMPILER_64_LO(qual_value_set_buf[idx])) {
                    BCM_PBMP_PORT_ADD(pbmp_mask, idx);
                }
                if (COMPILER_64_LO(qual_value_op_data_buf[idx])) {
                    BCM_PBMP_PORT_ADD(pbmp, idx);
                }
            }
            if (BCM_PBMP_NOT_NULL(pbmp)) {
                shr_pb_printf(shr_pb_data_str, "%s ",
                              _SHR_PBMP_FMT(pbmp, buf));
            }
            for (idx = 0 ; idx < _SHR_PBMP_FMT_LEN; idx++) {
                buf[idx] = 0;
            }
            if (BCM_PBMP_NOT_NULL(pbmp_mask)) {
                shr_pb_printf(shr_pb_mask_str, "%s ",
                              _SHR_PBMP_FMT(pbmp_mask, buf));
            }
        } else {
            for (idx = num_element -1; idx >= 0; idx--) {
                if (fld_defs_iter->width > 32) {
                    value = COMPILER_64_HI(qual_value_set_buf[idx]);
                    shr_pb_printf(shr_pb_mask_str, "%08x ", value);
                    value = COMPILER_64_HI(qual_value_op_data_buf[idx]);
                    shr_pb_printf(shr_pb_data_str, "%08x ", value);
                }
                value = COMPILER_64_LO(qual_value_set_buf[idx]);
                shr_pb_printf(shr_pb_mask_str, "%08x ", value);
                value = COMPILER_64_LO(qual_value_op_data_buf[idx]);
                shr_pb_printf(shr_pb_data_str, "%08x ", value);
            }
        }
        BCMINT_FIELD_MEM_FREE(qual_field);
        BCMINT_FIELD_MEM_FREE(qual_value_op_mask_buf);
        BCMINT_FIELD_MEM_FREE(qual_value_op_data_buf);
        BCMINT_FIELD_MEM_FREE(qual_value_set_buf);
        fld_defs_iter++;
    }
    ...
    bcm_field_entry_t entry)
{
    ...
    bcmlt_entry_handle_t entry_info_template = BCMLT_INVALID_HDL;
    int ct = 0;
    bcmi_field_ha_group_oper_t *group_oper = NULL;
    ...
    uint32_t multi_param[4] = {0};
    int multi_act = 0;
    bool is_print_reqd  = false;
    shr_pb_t *shr_pb_data_str = NULL; /* to hold data of qual */
    shr_pb_t *shr_pb_mask_str = NULL; /* to hold mask of qual */
    bool is_src_compressed = false;
    bool is_dst_compressed = false;
    uint64_t list_compress = 0;
    uint8_t cont = 0;
    bool udf = FALSE;
    ...
    if (entry_oper->entry_flags & BCMINT_FIELD_ENTRY_TEMP_FOR_SLICE_INFO) {
        SHR_EXIT();
    }
    ...
    for (ct = 0; ct < group_oper->qual_cnt; ct++) {
       ...
       /* Find lt map from db for given qualifier in group qset */
        ...
        fld_defs_iter = fld_defs_ref;
        is_print_reqd = false;
        /* is_print_reqd is set to true if qualifier is configured
         * or stage is EM/FT.
         * the qualifier is configured or not is decided by checking
         * if its mask is set
         * For EM/ft, there is no mask, hence all quals of group qset
         * are considered as configured
         * The variable is_print_reqd is used to decide whether to
         * print the qual info.
         */
        shr_pb_data_str = shr_pb_create();
        shr_pb_mask_str = shr_pb_create();
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_is_qual_print_require(unit, &rule_templ,
                 stage_info, &grp_qset, lt_qual_map,
                 fld_defs_iter, stage_info->tbls_info->rule_sid,
                  shr_pb_data_str, shr_pb_mask_str,
                  group_oper->qset_arr[ct],
                  &is_print_reqd));
        /* for EM/FP stages, print the qual info for all quals */
        if (stage_info->flags & BCMINT_FIELD_STAGE_DEFAULT_ENTRY_SUPPORT) {
            is_print_reqd = true;
        }
        ...
        if (is_print_reqd) {
            ...
            for (iter = (lt_qual_map->num_maps - 1); iter >=0 ; iter--) {
                ...
                if ((stage_info->flags & BCMINT_FIELD_STAGE_KEY_TYPE_FIXED) &&
                       ((sal_strncmp(lt_qual_map->map[iter].lt_field_name,
                ...
                /* Get Field Qualifier name for group template */
                SHR_IF_ERR_VERBOSE_EXIT
                    (mbcm_ltsw_field_lt_qual_name_update(unit, 0, 1,
                       stage_info, (const bcm_field_qset_t *) &grp_qset,
                       &(lt_qual_map->map[iter]),
                       &lt_field_name));
                ...
                for (part_id = 1; part_id <= num_parts; part_id++) {
                    ...
                    SHR_IF_ERR_VERBOSE_EXIT
                        (mbcm_ltsw_field_qual_info_lt_name_get(
                            unit, lt_field_name, num_offset_field_name,
                            offset_field_name, width_field_name));
                    ...
                    if (fld_defs_qual_info->elements > 1) {
                        ...
                        SHR_IF_ERR_VERBOSE_EXIT
                            (bcmlt_entry_field_get(
                                grp_info_qual_templ[part_id-1],
                                num_offset_field_name, &num_offsets));
                    }
        ...
        if (is_print_reqd) {
            LOG_CLI((BSL_META_U(unit,
                "\n     DATA=")));
            /* for stages other tham EM/FT, the qual data is
             *  aready stored in shr_pb_data_str
             */
            LOG_CLI((BSL_META_U(unit, "%s"), shr_pb_str(shr_pb_data_str)));
            ...
            if (!(fld_defs_ref->symbol)) {
                ...
                LOG_CLI((BSL_META_U(unit, "%s"),
                                 shr_pb_str(shr_pb_mask_str)));
            }
        }
        BCMINT_FIELD_MEM_FREE(fld_defs_ref);
        if (shr_pb_data_str != NULL) {
            shr_pb_destroy(shr_pb_data_str);
            shr_pb_data_str = NULL;
        }
        if (shr_pb_mask_str != NULL) {
            shr_pb_destroy(shr_pb_mask_str);
            shr_pb_mask_str = NULL;
        }
        ...
        for (indx = 0; indx < class_id_count; indx++) {
            if ((class_id_arr[indx] != -1) &&
                ...
                LOG_CLI((BSL_META_U(unit,
                         ...
                         class_id_arr[indx]));
                if ((aacl_qual_arr[indx] == bcmFieldQualifySrcIp) ||
                    (aacl_qual_arr[indx] == bcmFieldQualifySrcIp6)) {
                    is_src_compressed = true;
                } else if ((aacl_qual_arr[indx] == bcmFieldQualifyDstIp) ||
                    (aacl_qual_arr[indx] == bcmFieldQualifyDstIp6)) {
                    is_dst_compressed = true;
                }
    ...
    if ((group_oper->group_flags & BCMINT_FIELD_GROUP_COMPRESSION_ENABLED) &&
            (entry_oper->entry_flags & BCMINT_FIELD_ENTRY_INSTALLED)) {
        LOG_CLI((BSL_META_U(unit,
            "AACL Compression Information :\n")));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_allocate(dunit,
                                  "FP_ING_ENTRY_INFO",
                                  &entry_info_template));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(entry_info_template,
                                   "FP_ING_ENTRY_ID",entry_id));
	    SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_entry_commit(unit, entry_info_template,
                                  BCMLT_OPCODE_LOOKUP,
                                  BCMLT_PRIORITY_NORMAL));

        /* Lookup entry_info template using handle. */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_entry_commit(unit, entry_info_template,
                                  BCMLT_OPCODE_LOOKUP,
                                  BCMLT_PRIORITY_NORMAL));
        ...
        /* Retreive operation mode field from fetched entry. */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(entry_info_template,
                                   "LIST_COMPRESSED",
                                   &list_compress));
        ...
        if (list_compress) {
            LOG_CLI((BSL_META_U(unit,
                    "    List compressed : Yes\n")));
            LOG_CLI((BSL_META_U(unit,
                    "    HW entries Consumed in IFP table : One\n")));
        } else {
            LOG_CLI((BSL_META_U(unit,
                        "    List compressed : No\n")));
            LOG_CLI((BSL_META_U(unit,
                        "    HW entries Consumed in IFP table : None\n")));
        }

        if (is_src_compressed || is_dst_compressed) {
            LOG_CLI((BSL_META_U(unit,
                        "    Width compressed : Yes\n")));
            if (is_src_compressed && is_dst_compressed) {
                LOG_CLI((BSL_META_U(unit,
                        "    HW entries Consumed in LPM table : Two\n")));
            } else {
                LOG_CLI((BSL_META_U(unit,
                        "    HW entries Consumed in LPM table : One\n")));
            }
        } else {
            LOG_CLI((BSL_META_U(unit,
                        "    Width compressed : No\n")));
            LOG_CLI((BSL_META_U(unit,
                        "    HW entries Consumed in LPM table : None\n")));
        }
    }
    LOG_CLI((BSL_META_U(unit,
                        "\n")));
    ...
    if (stage_info->flags & BCMINT_FIELD_STAGE_POLICY_TYPE_PDD) {
        max_actions = group_oper->action_cnt;
    } else {
        max_actions = bcmFieldActionCount;
    }
    /* loop through all actions and see if action is configured in entry*/
    for (ct = 0; ct < max_actions; ct++) {
        if (stage_info->flags & BCMINT_FIELD_STAGE_POLICY_TYPE_PDD) {
        ...
        for (iter = 0; iter < multi_act; iter++) {
            LOG_CLI((BSL_META_U(unit,
                        "         action = ")));
            LOG_CLI((BSL_META("{act=%s, "
                              ...
                              (multi_act > 1) ? multi_param[iter]: params.param1 ,
                              (multi_act > 1) ? multi_param[iter]: params.param1 ,
...
exit:
    ...
    if (BCMLT_INVALID_HDL != entry_info_template) {
        (void) bcmlt_entry_free(entry_info_template);
    }
...
static int
bcmint_field_group_entry_count_slice_sharing_groups(
    int unit,
    bcm_field_group_t group,
    uint64_t *entries_created_shared_group)
{
    bcmint_field_stage_info_t *stage_info = NULL;
    int gid = 0;
    int index = 0;
    ...
    bool reserved_entry_present = FALSE;
    bool is_resource_error = FALSE;
    uint64_t entries_created = 0;
    bcm_field_group_t group_temp = 0;
    ...
    for (index = 1;
        ...
        if (SHR_BITGET(stage_info->group_info->grpid_bmp.w, index)) {
            ...
            if ((group_oper->priority == oper_temp->priority)) {
                BCMINT_FIELD_STAGE_ENTRY_ID_GET(stage_info->stage_id,
                                                index,
                                                group_temp);
                 SHR_IF_ERR_VERBOSE_EXIT
                     (bcmi_field_group_is_empty(unit, group_temp,
                                                &entry_present,
                                                &reserved_entry_present));
                 SHR_IF_ERR_VERBOSE_EXIT
                     (bcmint_field_group_info_get(unit, group_temp,
                                                entry_present,
                                                &is_resource_error,
                                                &grp_info_templ));
                 SHR_IF_ERR_VERBOSE_EXIT
                     (bcmlt_entry_field_get(grp_info_templ,
                         ...
                         &entries_created));
                (*entries_created_shared_group) += entries_created;
             }
        }
    }
exit:
    ...
    bool brief)
{
    ...
    bool reserved_entry_present = FALSE;
    uint64_t entries_created_shared_group = 0;
    ...
    /* If group has no entries and entry install failed due to resource error,
     * then perform below
     */
    if ((!entry_present) && (!is_resource_error)) {
    ...
    /* Reserved internal entry in SDKLT */
    if (entries_tentative != 0) {
        entries_tentative = entries_tentative - 1;
    }
    ...
    if (entries_tentative != 0) {
        ...
        value = COMPILER_64_LO(entries_created);
    }

    /* Consider entries from slice sharing groups */
    SHR_IF_ERR_VERBOSE_EXIT(
        bcmint_field_group_entry_count_slice_sharing_groups(
            unit,
            group,
            &entries_created_shared_group));

    value += COMPILER_64_LO(entries_created_shared_group);
    ...
    const char *pfx)
{
    ...
    SHR_NULL_CHECK(pfx, SHR_E_PARAM);
    ...
    bcm_field_qset_t *pqset)
{
    ...
    SHR_NULL_CHECK(pqset, SHR_E_PARAM);
    ...
    for (qual_idx = 0; qual_idx < BCM_FIELD_QUALIFY_MAX; qual_idx++) {
        ...
        for (iter = (lt_map_ref->num_maps - 1); iter >= 0; iter--) {
            ...
            if (presel_grp_info) {
                ...
                if (fld_defs_ref->symbol) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_entry_field_symbol_get(presel_templ,
                            lt_field_name,
                            (const char **)&lt_sym_str_op));
                    if (sal_strcmp(lt_sym_str_op,
                        fld_defs_ref->sym_def) != 0) {
    ...
    int presel_id)
{
    ...
    if (stage_info->flags & BCMINT_FIELD_STAGE_PRESEL_KEY_DYNAMIC) {
        ...
        rv = (bcmint_field_presel_qset_get(unit,
                                          ...
                                          1,
                                          &presel_qset));
        presel_sid = stage_info->tbls_info->presel_group_sid;
        ...
        rv = (bcmint_field_presel_qset_get(unit,
                                          ...
                                          0,
                                          &presel_qset));
        presel_sid = stage_info->tbls_info->presel_entry_sid;
    }
    if (rv == SHR_E_PARAM) {
        LOG_CLI((BSL_META_U(unit,"\n")));
        SHR_EXIT();
    ...
    for (qual_idx = 0; qual_idx < BCM_FIELD_QUALIFY_MAX; qual_idx++) {
        ...
        BCMINT_FIELD_MEM_ALLOC
            (fld_defs_ref,
             sizeof(bcmlt_field_def_t)  * (lt_map_ref->num_maps),
             "field definitions");
        ...
        shr_pb_mask_str = shr_pb_create();
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_is_qual_print_require(unit, &presel_templ,
                 stage_info, NULL, lt_map_ref,
                 fld_defs_iter, presel_sid, shr_pb_data_str,
                 shr_pb_mask_str, qual_idx, &is_print_reqd));
        /* for EM/FP stages, print the qual info for all quals */
        if (stage_info->flags & BCMINT_FIELD_STAGE_DEFAULT_ENTRY_SUPPORT) {
            is_print_reqd = true;
        }
                               ...
                               bcm_field_group_status_t *status)
{
    ...
    status->entries_total += COMPILER_64_LO(entries_created_shared_group);
    ...
    for (idx = 0; idx < stage_info->entry_info->hash_size; idx++) {
        ...
        do {
            if ((entry_oper->group_id == gid) && (!reserved_entry_present)) {
                ...
                if ((stage_info->flags & BCMINT_FIELD_STAGE_POLICER_SUPPORT) ||
                    ...
                    if (entry_oper->policer_id != 0) {
                        pid = (entry_oper->policer_id & BCM_FIELD_ID_MASK);
                        /*
                         * Several entries can share a policer.
                         * Skip if this policer has already been counted.
                         */
                        if (!SHR_BITGET(policer_bmp, pid)) {
                            /* Get the number of meter entries for this policer. */
                            SHR_IF_ERR_VERBOSE_EXIT
                                (bcmi_ltsw_num_policer_entries_get(
                                        unit,
                                        entry_oper->policer_id,
                                        &num_meters));
                            status->meter_count += num_meters;
                            SHR_BITSET(policer_bmp, pid);
                        }
                    }
                }
            }
    ...
    if ((reserved_entry_present) && (stage_info->tbls_info->entry_priority_fid != NULL)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_field_def_get(unit,
                stage_info->tbls_info->entry_sid,
                stage_info->tbls_info->entry_priority_fid, &fld_defs));
        status->prio_min = fld_defs.min;
        status->prio_max = fld_defs.max;
    }
...
exit:
    BCMINT_FIELD_MEM_FREE(policer_bmp);
    if (BCMLT_INVALID_HDL != grp_info_templ) {
        (void) bcmlt_entry_free(grp_info_templ);
    }
    FP_UNLOCK(unit);
    ...
    bcm_field_stage_slice_info_t *slice_info)
{
    ...
    slice_info->flags = 0;
    slice_info->slice_gid_ct = 0;
    for (id = 0; id < BCM_FIELD_MAX_GROUPS_PER_SLICE; id++) {
          slice_info->slice_gid_list[id] = 0;
    }

    /* Slice related group info. */
    for (gid = 1;
         gid <= ((stage_info->group_info->gid_max)-
         BCMINT_FIELD_GROUP_RESERVE_GID_COUNT); gid++) {
        if (!SHR_BITGET(stage_info->group_info->grpid_bmp.w, gid)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_group_mode_get(unit, group, &mode));

        switch (mode) {
            case bcmFieldGroupModeTriple:
                slice_info->flags |= BCM_FIELD_SLICE_MODE_TRIPLE;
                break;
            case bcmFieldGroupModeDouble:
                slice_info->flags |= BCM_FIELD_SLICE_MODE_DOUBLE;
                break;
            case bcmFieldGroupModeIntraSliceDouble:
                slice_info->flags |= BCM_FIELD_SLICE_MODE_INTRADOUBLE;
                break;
            case bcmFieldGroupModeSingle:
                slice_info->flags |= BCM_FIELD_SLICE_MODE_SINGLE;
                break;
            default:
                break;
        }
        ...
        for (id = 1; id <= num_parts; id++) {
            ...
            for (slice_id = 0; slice_id < num_slices; slice_id++) {
                if (slice[slice_id] == slice_no) {
                    slice_info->slice_gid_list[slice_info->slice_gid_ct] = group;
                    slice_info->slice_gid_ct += 1;
                    if (id == 1) {
                        if (slice_id == 0) {
                            slice_info->flags |= BCM_FIELD_SLICE_PART_PRIMARY;
                        } else {
                            slice_info->flags |= BCM_FIELD_SLICE_PART_PRIMARY_EXPANDED;
                        }
                    } else if (id == 2) {
                        if (slice_id == 0) {
                            slice_info->flags |= BCM_FIELD_SLICE_PART_SECONDARY;
                        } else {
                            slice_info->flags |= BCM_FIELD_SLICE_PART_SECONDARY_EXPANDED;
                        }
                    } else if (id == 3) {
                        if (slice_id == 0) {
                            slice_info->flags |= BCM_FIELD_SLICE_PART_TERTIARY;
                        } else {
                            slice_info->flags |= BCM_FIELD_SLICE_PART_TERTIARY_EXPANDED;
                        }
                    }
                }
            }
    ...
    if (slice_info->slice_gid_ct > 0) {
        slice_info->flags |= BCM_FIELD_SLICE_ALLOCATED;
    }
    ...
    bcm_field_qset_t* qset)
{
    ...
    for (key_mode = 0; key_mode < BCMINT_FIELD_EFP_KEYS; key_mode++) {
        if ((stage_info->efp_key_qual_map[key_mode].mode_type == mode_type) &&
            ...
            (packet_type == bcmFieldGroupPacketTypeDefault)))) {
...
int
bcm_ltsw_field_entry_policer_extended_attach(int unit,
                                             bcm_field_entry_t entry,
                                             bcm_field_policer_config_t *policer_cfg)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_entry_policer_extended_attach(unit,
                                                    entry,
                                                    policer_cfg));
...
int
bcm_ltsw_field_entry_policer_extended_detach(int unit,
                                             bcm_field_entry_t entry,
                                             bcm_field_policer_config_t *policer_cfg)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_entry_policer_extended_detach(unit,
                                                    entry,
                                                    policer_cfg));
exit:
    FP_UNLOCK(unit);
...
int
bcm_int_field_group_to_ltid_get(int unit,
                             bcm_field_group_t group,
                             uint32_t *ltid) {
    bcmlt_entry_handle_t grp_info_templ = BCMLT_INVALID_HDL;
    bool is_resource_error = FALSE;
    ...
    if (NULL != stage_info->tbls_info->group_info_act_res_fid) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(grp_info_templ,
                                   stage_info->tbls_info->group_info_act_res_fid,
                                   &lt_val));
    }
    *ltid = COMPILER_64_LO(lt_val);
...
int
bcm_int_field_ltid_to_group_get(int unit,
                             uint32_t ltid,
                             bcm_field_group_t *group) {
    bcmlt_entry_handle_t grp_info_templ = BCMLT_INVALID_HDL;
    bool is_resource_error = FALSE;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_stage_info_get(unit, bcmiFieldStageExactMatch,
                                     &stage_info));
    for (gid = 1;
         gid <= ((stage_info->group_info->gid_max)-
         BCMINT_FIELD_GROUP_RESERVE_GID_COUNT); gid++) {
        ...
        BCMINT_FIELD_STAGE_ENTRY_ID_GET(stage_info->stage_id,
                                        ...
                                        group_id);
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_field_group_is_empty(unit, group_id,
                                       &entry_present,
                                       &reserved_entry_present));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_group_info_get(unit, group_id,
                                       entry_present,
                                       &is_resource_error,
                                       &grp_info_templ));
...
/*
 * Function:
 *     bcmint_field_hint_name
 * Purpose:
 *     Translate a Hint enum value to a text string.
 * Parameters:
 *     Enum value from bcm_field_hint_type_t.
 * Returns:
 *     Text name of indicated hint enum value.
 */
char *
bcmint_field_hint_name(bcm_field_hint_type_t hid)
{
    /* Text names of the enumerated hint IDs. */
    static char *hint_text[bcmFieldHintTypeCount] = BCM_FIELD_HINT_TYPE_STRINGS;

    if ((int)hid < bcmFieldHintTypeCount) {
        return hint_text[hid];
    }
    return "UnknownHint";
 }
...
/*
 * Function:
 *     bcmint_field_flexctr_object_name
 * Purpose:
 *     Translate a flexctr_object enum value to a text string.
 * Parameters:
 *     Enum value from bcm_flexctr_object_type_t.
 * Returns:
 *     Text name of indicated flexctr_object enum value.
 */
char *
bcmint_field_flexctr_object_name(bcm_flexctr_object_t fid)
{
    /* Text names of the enumerated flexctr_object IDs. */
    static char *flexctr_object_text[bcmFlexctrObjectCount+1] = BCM_FLEXCTR_OBJECT_NAME_INITIALIZER;

    if ((int)fid < bcmFlexctrObjectCount) {
        return flexctr_object_text[fid];
    }
    return "UnknownObject";
 }

int
bcmi_field_hint_dump(
    int unit,
    int hint_id)
{
    uint32_t index = 0;
    ...
    for (index = 0 ; index < _SHR_PBMP_FMT_LEN; index++) {
        buf[index] = 0;
    }
    BCM_PBMP_CLEAR(hint_pbmp);

    if (hint_id == -1) {
        min = 0;
        max = BCMI_FIELD_HA_HINT_ID_MAX;
    } else {
        min = hint_id;
        max = hint_id;
    }

    for (hint = min ; hint <= max; hint++) {
        index = (hint & BCMINT_FP_HASH_INDEX_MASK);
        f_ht = &fp_control->hints_hash[index];
        ...
        if ((f_ht != NULL) && (f_ht->hintid != 0)) {
            ...
            while (hint_entry != NULL) {
                ...
                LOG_CLI((BSL_META_U(unit,
                "Hint_Type=%s\n\r"),
                bcmint_field_hint_name(hint_entry->hint_type)));
                LOG_CLI((BSL_META_U(unit,"flags=%#x\n\r"), hint_entry->flags));
                ...
                switch (hint_entry->hint_type) {
                    ...
                    case bcmFieldHintTypeExtraction:
                        LOG_CLI((BSL_META_U(unit,
                        "qualifier=%s\n\r"),
                        bcmint_field_qual_name(hint_entry->qual)));

                        if (hint_entry->qual == bcmFieldQualifyUdf) {
                            LOG_CLI((BSL_META_U(unit,
                            "start_bit=%d\n\r"), hint_entry->start_bit));
                            LOG_CLI((BSL_META_U(unit,
                            "end_bit=%d\n\r"), hint_entry->end_bit));
                            LOG_CLI((BSL_META_U(unit,
                            "udf_id=%d\n\r"), hint_entry->udf_id));
                            LOG_CLI((BSL_META_U(unit,
                            "value=%d\n\r"), hint_entry->value));

                        } else if (hint_entry->qual == bcmFieldQualifyInPorts) {
                            sal_memcpy(hint_entry->pbmp, &hint_pbmp, sizeof(bcm_pbmp_t));
                            LOG_CLI((BSL_META_U(unit, "pbmp=%s,\n\r"),
                                _SHR_PBMP_FMT(hint_pbmp, buf)));
                            LOG_CLI((BSL_META_U(unit, "value=%d\n\r"), hint_entry->value));
                        } else {
                            LOG_CLI((BSL_META_U(unit,
                            "max_values=%d\n\r"),
                            hint_entry->max_values));
                    ...
                    case bcmFieldHintTypeGroupAutoExpansion:
                        LOG_CLI((BSL_META_U(unit,
                        "max_group_size=%d\n\r"), hint_entry->max_group_size));
                        break;
                    case bcmFieldHintTypeFlexCtrOpaqueObjectSel:
                        LOG_CLI((BSL_META_U(unit,
                        "value=%s\n\r"),
                        bcmint_field_flexctr_object_name(hint_entry->value)));
                        break;
