...
#include <bcm_int/ltsw/generated/virtual_ha.h>
...
/*!
 * \brief Defines software database for virtual related LT information.
 */
typedef struct ltsw_virtual_lt_db_s {
    /*! Tunnel table min index */
    uint32_t tnl_min;

    /*! Tunnel table max index */
    uint32_t tnl_max;
...
typedef struct ltsw_virtual_info_s {
    ...
    /*! VP info */
    bcmint_virtual_vp_info_t *vp_info;
    ...
    /*! Overlay egress object mapping of VP. */
    bcm_if_t       *ol_egress_if_map;
} ltsw_virtual_info_t;

/*!
 * \Virtual software database of LT information.
 */
static ltsw_virtual_lt_db_t ltsw_virtual_lt_db[BCM_MAX_NUM_UNITS];
...
typedef enum virtual_ha_subcomponent_id_e {
    ...
    VIRTUAL_SUB_COMP_ID_OL_EGRESS_IF_MAP   = 7,  /* overlay egress object mapping of VP. */
    VIRTUAL_SUB_COMP_ID_COUNT           = 8
...
virtual_vp_manage_init(int unit)
{
    ...
    bcmint_virtual_vp_info_t *vp_info = vi->vp_info;
    uint32_t i, alloc_size, req_size;
    uint16_t *list_ptr = NULL, *head_ptr = NULL;
    uint16_t *ptr = NULL, *ptr_2 = NULL;
    int warm = bcmi_warmboot_get(unit);
    int user_size[BCMI_LTSW_VIRTUAL_VP_USER_CNT] ={(ltsw_virtual_lt_db[unit].l2_iif_max + 1),
                                                   (ltsw_virtual_lt_db[unit].vp_max + 1)};
    ...
    vp_num = ltsw_virtual_lt_db[unit].vp_max + 1;
    for (i = 0; i < BCMI_LTSW_VIRTUAL_VP_USER_CNT; i++) {
        /* Keep this check for future more users.*/
 ...
 */
int
bcmi_ltsw_virtual_init_lt_db(int unit)
{
    int start, end;
    int vfi_min, vfi_max, vp_min, vp_max, l2_iif_min, l2_iif_max;

    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_EXIT
        (mbcm_virtual_tnl_index_get(unit, &start, &end));
    ltsw_virtual_lt_db[unit].tnl_min = start;
    ltsw_virtual_lt_db[unit].tnl_max = end;

    SHR_IF_ERR_EXIT
        (mbcm_virtual_index_get(unit,
                                &vfi_min, &vfi_max,
                                &vp_min, &vp_max,
                                &l2_iif_min, &l2_iif_max));
    ltsw_virtual_lt_db[unit].vfi_min = vfi_min;
    ltsw_virtual_lt_db[unit].vfi_max = vfi_max;
    ltsw_virtual_lt_db[unit].vp_min = vp_min;
    ltsw_virtual_lt_db[unit].vp_max = vp_max;
    ltsw_virtual_lt_db[unit].l2_iif_min = l2_iif_min;
    ltsw_virtual_lt_db[unit].l2_iif_max = l2_iif_max;
...
bcmi_ltsw_virtual_init(int unit)
{
    ...
    num_vfi = ltsw_virtual_lt_db[unit].vfi_max + 1;
    num_vp = ltsw_virtual_lt_db[unit].vp_max + 1;
    ...
    if (sizeof(bcmint_virtual_vp_info_t) != sizeof(bcmi_ltsw_virtual_vp_info_t)) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
    }
    /* Allocate HA memory for VP info. */
    ha_instance_size = sizeof(bcmint_virtual_vp_info_t);
    ha_array_size = num_vp;
    req_size = sizeof(bcmint_virtual_vp_info_t) * num_vp;
    alloc_size = req_size;
    ...
    if (warm) {
        ...
        if (rv == SHR_E_NOT_FOUND) {
            ...
            ptr_tmp = (void *)((uint8_t *)ptr_tmp + offset);
        }
    } else {
        rv = bcmi_ltsw_issu_struct_info_report(unit,
                                       BCMI_HA_COMP_ID_VIRTUAL,
        ...
        for (i = 0; i < ltsw_virtual_lt_db[unit].vfi_min; i++) {
    ...
    /* Allocate HA memory for overlay egress mapping info. */
    req_size = num_vp * sizeof(bcm_if_t);
    alloc_size = req_size;
    ptr = bcmi_ltsw_ha_mem_alloc(unit,
                                 BCMI_HA_COMP_ID_VIRTUAL,
                                 VIRTUAL_SUB_COMP_ID_OL_EGRESS_IF_MAP,
                                 "bcmVirtualOverlayEgrIfMap",
                                 &alloc_size);
    SHR_NULL_CHECK(ptr, SHR_E_MEMORY);
    SHR_IF_ERR_VERBOSE_EXIT
        ((alloc_size < req_size) ? SHR_E_MEMORY : SHR_E_NONE);
...
exit:
    if (SHR_FUNC_ERR()) {
        ...
        if (!warm) {
            ...
            if (vi->ol_egress_if_map != NULL) {
                (void)bcmi_ltsw_ha_mem_free(unit, vi->ol_egress_if_map);
                vi->ol_egress_if_map = NULL;
            }
        }
    }
    SHR_FUNC_EXIT();
                           ...
                           bcmi_ltsw_virtual_vp_info_t vp_info, int *base_vp)
{
    ...
    if (start > end) {
        reverse = 1;
        end = (end > ltsw_virtual_lt_db[unit].vp_min ) ? end : ltsw_virtual_lt_db[unit].vp_min;
    } else {
        start = (start > ltsw_virtual_lt_db[unit].vp_min) ? start : ltsw_virtual_lt_db[unit].vp_min;
    }
                              ...
                              bcmi_ltsw_virtual_vp_info_t vp_info)
{
    ...
    if (vp > ltsw_virtual_lt_db[unit].vp_max) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    vi->vp_info[vp].flags = vp_info.flags;
    vi->vp_info[vp].vpn = vp_info.vpn;
    vi->vp_info[vp].l2_oif = vp_info.l2_oif;
    vi->vp_info[vp].dvp_pg = vp_info.dvp_pg;
    vi->vp_info[vp].flags |= BCMI_LTSW_VIRTUAL_VP_VALID;
    VIRTUAL_UNLOCK(unit);
                              ...
                              bcmi_ltsw_virtual_vp_info_t *vp_info)
{
    ...
    vp_info->flags = vi->vp_info[vp].flags;
    vp_info->vpn = vi->vp_info[vp].vpn;
    vp_info->l2_oif = vi->vp_info[vp].l2_oif;
    vp_info->dvp_pg = vi->vp_info[vp].dvp_pg;
                    ...
                    uint32_t leave_pbmp, uint32_t stay_pbmp, uint32_t join_pbmp)
{
    ...
    alloc_size = (ltsw_virtual_lt_db[unit].vp_max + 1) * sizeof(uint16_t);
                            ...
                            int *vfi)
{
    ...
    for (i = ltsw_virtual_lt_db[unit].vfi_min;
         i <= ltsw_virtual_lt_db[unit].vfi_max;
         i++) {
        if (!VIRTUAL_VFI_USED_GET(unit, i)) {
            break;
        }
    }
    if (i > ltsw_virtual_lt_db[unit].vfi_max) {
        VIRTUAL_UNLOCK(unit);
        SHR_ERR_EXIT(SHR_E_RESOURCE);
    }
                                    ...
                                    int vfi)
{
    ...
    if (vfi < ltsw_virtual_lt_db[unit].vfi_min ||
        vfi > ltsw_virtual_lt_db[unit].vfi_max ) {
        VIRTUAL_UNLOCK(unit);
        SHR_ERR_EXIT(SHR_E_RESOURCE);
    }
...
bcmi_ltsw_virtual_vfi_idx_get(int unit, bcm_vpn_t vpn, uint32_t *vfi)
{
    ...
    if ((vi->initialized) && BCMI_LTSW_VIRTUAL_VPN_IS_SET(vpn)) {
        ...
        if (*vfi > ltsw_virtual_lt_db[unit].vfi_max) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    } else {
        if (vpn > BCM_VLAN_MAX) {
...
bcmi_ltsw_virtual_vfi_num_get(int unit, uint32_t *vfi_num)
{
    ...
    SHR_NULL_CHECK(vfi_num, SHR_E_PARAM);

    *vfi_num = ltsw_virtual_lt_db[unit].vfi_max + 1;
...
bcmi_ltsw_virtual_vp_num_get(int unit, uint32_t *vp_num)
{
    ...
    SHR_NULL_CHECK(vp_num, SHR_E_PARAM);
...
bcmi_ltsw_virtual_vfi_range_get(int unit, uint32_t *start, uint32_t *end)
{
    ...
    SHR_NULL_CHECK(end, SHR_E_PARAM);

    *start = ltsw_virtual_lt_db[unit].vfi_min;
    *end = ltsw_virtual_lt_db[unit].vfi_max;
...
bcmi_ltsw_virtual_vp_range_get(int unit, uint32_t *start, uint32_t *end)
{
    ...
    *start = ltsw_virtual_lt_db[unit].vp_min;
    *end = ltsw_virtual_lt_db[unit].vp_max;
...
bcmi_ltsw_virtual_l2_iif_range_get(int unit, uint32_t *start, uint32_t *end)
{
    ...
    *start = ltsw_virtual_lt_db[unit].l2_iif_min;
    *end = ltsw_virtual_lt_db[unit].l2_iif_max;
...
bcmi_ltsw_virtual_l2_iif_num_get(int unit, uint32_t *l2_iif_num)
{
    ...
    SHR_NULL_CHECK(l2_iif_num, SHR_E_PARAM);

    *l2_iif_num = ltsw_virtual_lt_db[unit].l2_iif_max + 1;
 ...
 * \param [in]  unit        Unit number.
 * \param [in]  egress_if   Egress object.
 * \param [out] bind_vp     bind vp.
 *
 * \retval SHR_E_NONE    No errors.
 * \retval !SHR_E_NONE   Failure.
 */
int
bcmi_ltsw_virtual_port_egress_obj_used(int unit, bcm_if_t egress_if, int *bind_vp)
{
    ltsw_virtual_info_t *vi = VIRTUAL_INFO(unit);
    uint32_t vp = 0, start = 0, end = 0;
    int nh_idx;
    bcmi_ltsw_l3_egr_obj_type_t nh_type, vp_nh_type;
    ...
    *bind_vp= 0;
    ...
    for (vp = start; vp < end; vp++) {
        ...
        if (nh_type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_UL) {
            ...
            ecmp_member_count = 1;
            ecmp_member_array[0].egress_if = vi->egress_if_map[vp];
        } else { /* Underlay next hop case. */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_l3_egress_obj_id_resolve(unit,
                                                    vi->egress_if_map[vp],
                                                    &nh_idx,
                                                    &vp_nh_type));
            if (vp_nh_type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_UL) {
                    ecmp_member_size = max_paths;
                    ecmp_info.ecmp_intf = vi->egress_if_map[vp];
                    SHR_IF_ERR_VERBOSE_EXIT
                         (bcm_ltsw_l3_ecmp_get(unit, &ecmp_info,
                                               ecmp_member_size,
                                               ecmp_member_array,
                                               &ecmp_member_count));
            } else {
        ...
        for (ecmp_member_idx = 0;
             ecmp_member_idx < ecmp_member_count;
             ecmp_member_idx++) {
            if (ecmp_member_array[ecmp_member_idx].egress_if == egress_if) {
                *bind_vp = vp;
                SHR_EXIT();
            }
        }
...
/*!
 * \brief Set overlay egress object to port.
 *
 * \param [in] unit        Unit number.
 * \param [in] port        Gport number.
 * \param [in] egress_if   Egress object.
 ...
 */
int
bcmi_ltsw_virtual_port_overlay_egress_obj_set(int unit, bcm_gport_t port,
                                              bcm_if_t egress_if)
{
    ltsw_virtual_info_t *vi = VIRTUAL_INFO(unit);
    int vp;
    ...
    VIRTUAL_LOCK(unit);
    vi->ol_egress_if_map[vp] = egress_if;
    VIRTUAL_UNLOCK(unit);
...
/*!
 * \brief Get overlay egress object from port.
 *
 * \param [in]  unit        Unit number.
 * \param [in]  port        Gport number.
 * \param [out] egress_if   Egress object.
 ...
 */
int
bcmi_ltsw_virtual_port_overlay_egress_obj_get(int unit, bcm_gport_t port,
                                              bcm_if_t *egress_if)
{
    ltsw_virtual_info_t *vi = VIRTUAL_INFO(unit);
    int vp;
    ...
    VIRTUAL_LOCK(unit);
    *egress_if = vi->ol_egress_if_map[vp];
    VIRTUAL_UNLOCK(unit);
...
/*!
 * \brief Check bind status of overlay egress object.
 *
 * \param [in]  unit        Unit number.
 * \param [in]  egress_if   Egress object.
 * \param [out] bind_vp     bind vp.
 ...
 */
int
bcmi_ltsw_virtual_port_overlay_egress_obj_used(int unit, bcm_if_t egress_if,
                                               int *bind_vp)
{
    ltsw_virtual_info_t *vi = VIRTUAL_INFO(unit);
    uint32_t vp = 0, start = 0, end = 0;
    ...
    *bind_vp= 0;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_virtual_vp_range_get(unit, &start, &end));
    for (vp = start; vp < end; vp++) {
        if (vi->ol_egress_if_map[vp] == egress_if) {
            *bind_vp = vp;
            SHR_EXIT();
        }
    }
...
/*!
 * \brief Get forwarding egress object from port.
 *
 * \param [in]  unit        Unit number.
 * \param [in]  port        Gport number.
 * \param [out] egress_if   Egress object.
 ...
 */
int
bcmi_ltsw_virtual_port_fwd_egress_obj_get(int unit, bcm_gport_t port,
                                          bcm_if_t *egress_if)
{
    ltsw_virtual_info_t *vi = VIRTUAL_INFO(unit);
    int vp;
    ...
    /*
     * If a VP is mapped to underlay egress object only, the underlay egress
     * object should be the one configured in L2 forwarding tables.
     * If a VP is mapped to overlay egress object and underlay nexthop, the
     * overlay egress object should be the one configured in L2 forwarding
     * tables. The mapping between VP and underlay egress object should be
     * keeped since it's used during overlay nexthop egress object creation.
    */
    VIRTUAL_LOCK(unit);
    if (vi->ol_egress_if_map[vp]) {
        *egress_if = vi->ol_egress_if_map[vp];
    } else {
        *egress_if = vi->egress_if_map[vp];
    }
...
bcmi_ltsw_virtual_vp_group_pipe_get(int unit, int pg, uint32_t *pipe_bmp)
{
    ...
    if (pg > ltsw_virtual_lt_db[unit].vp_max) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    VIRTUAL_VP_USER_LOCK(unit);
    is_locked = 1;
...
bcmi_ltsw_virtual_vp_user_trunk_cleanup(int unit)
{
    ...
    for (i = 0; i <= ltsw_virtual_lt_db[unit].l2_iif_max; i++) {
    ...
    bcm_if_t egress_if)
{
    ...
    bcm_if_t tmp_egress_if = BCM_IF_INVALID;
    ...
    if (BCM_IF_INVALID != egress_if) {
        ...
        while (ecmp_member_idx < ecmp_member_count) {
            if (local_rv == SHR_E_NONE) {
                tmp_egress_if = ecmp_member_array[ecmp_member_idx].egress_if;
                bcm_l3_egress_t_init(&egr);
                local_rv = bcm_ltsw_l3_egress_get(unit, tmp_egress_if, &egr);
                if ((local_rv == SHR_E_NONE) && egr.intf != BCM_IF_INVALID) {
                    SHR_IF_ERR_VERBOSE_EXIT
...
bcmi_ltsw_virtual_tnl_range_get(int unit, uint32_t *start, uint32_t *end)
{
    ...
    *start = ltsw_virtual_lt_db[unit].tnl_min;
    *end = ltsw_virtual_lt_db[unit].tnl_max;
 ...
 */
int
bcmi_ltsw_virtual_qos_port_map_set(
    int unit,
    bcm_gport_t port,
    int ing_map,
    int egr_map)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_virtual_qos_port_map_set(unit, port, ing_map, egr_map));
 ...
 */
int
bcmi_ltsw_virtual_qos_port_map_type_get(
    int unit,
    bcm_gport_t port,
    uint32_t flags,
    int *map_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_virtual_qos_port_map_type_get(unit, port, flags, map_id));
...
/*!
 * \brief Retrieve the adapt key sync status.
 *
 * \param [in] unit Unit Number.
 * \param [out] sync_required Sync status.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
...
int
bcmi_ltsw_virtual_port_egress_adapt_key_sync_status_get(
    int unit,
    int *sync_required)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_virtual_port_egress_adapt_key_sync_status_get(unit, sync_required));
...
/*!
 * \brief Notify the egress if changes.
 *
 * \param [in] unit Unit number.
 * \param [in] egr Egress forwarding destination.
 * \param [in] intf L3 interface id pointing to Egress object.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
int
bcmi_ltsw_virtual_port_egress_info_notify(
    int unit,
    bcm_l3_egress_t *egr,
    bcm_if_t intf)
{
    ...
    /* Only TH4G/B0 require the update. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_virtual_port_egress_adapt_key_sync_status_get(unit, &sync_required));
    if (sync_required == 0) {
        SHR_EXIT();
    }

    /* MC nexthop bind VP during egress object creation. */
    if (egr->flags & BCM_L3_IPMC) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_virtual_vp_range_get(unit, &min, &max));
        if ((egr->encap_id >= min) && (egr->encap_id <= max)) {
            BCM_GPORT_FLOW_PORT_ID_SET(vp, egr->encap_id);
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_virtual_port_egress_adapt_key_sync(unit, vp, intf));
        }
    }

    if (bcmi_ltsw_virtual_vp_is_vfi_type(unit, egr->port)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_virtual_port_egress_adapt_key_sync(unit, egr->port, intf));
    }
