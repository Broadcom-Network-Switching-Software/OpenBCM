 ...
 */
static int
bcm_ltsw_init_lt_db(int unit)
{
    SHR_FUNC_ENTER(unit);

    /*
     * This routine is for caching unchanged LT information
     * like table size, depth, index range ...etc during initization
     * to avoid accessing SDKLT APIs which is time-consuming.
     */
    if (ltsw_feature(unit, LTSW_FT_FLEX_FLOW) ||
        (ltsw_feature(unit, LTSW_FT_MPLS) &&
         !ltsw_feature(unit, LTSW_FT_NO_L2_MPLS))) {
        SHR_IF_ERR_CONT
            (bcmi_ltsw_virtual_init_lt_db(unit));
    }

    if (ltsw_feature(unit, LTSW_FT_FLEX_FLOW)) {
        /* Flow must after virtual's init. */
        SHR_IF_ERR_CONT
            (bcmi_ltsw_flow_init_lt_db(unit));
    }

    SHR_IF_ERR_CONT
        (bcmi_ltsw_tunnel_init_lt_db(unit));
...
bcm_ltsw_init_common(int unit)
{
    ...
    /*
     * Must after UFT init because it will determine some LT table sizes
     * like L2, L3, ALPM ...etc
     */
    SHR_IF_ERR_CONT
        (bcm_ltsw_init_lt_db(unit));
...
bcm_ltsw_init(int unit)
{
...
#ifdef BCM_PKTIO_EARLY_CUSTOM_INIT_F
        /*
         * Call custom pktio early init function during WB.
         * This can let custom Rx callback get registered ASAP.
         * Also custom Tx can be notified to start just now.
         */
        {
            extern int BCM_PKTIO_EARLY_CUSTOM_INIT_F(int unit);
            BCM_PKTIO_EARLY_CUSTOM_INIT_F(unit);
        }
#endif /* BCM_PKTIO_EARLY_CUSTOM_INIT_F */
    }
    ...
    if (ltsw_feature(unit, LTSW_FT_BFD)) {
...
bcm_ltsw_init_selective(int unit, uint32 module_number)
{
    ...
    case BCM_MODULE_PORT:
        ...
        break;
    case BCM_MODULE_L2:
        if (!ltsw_feature(unit, LTSW_FT_NO_L2)) {
            SHR_IF_ERR_EXIT
...
_bcm_ltsw_detach(int unit)
{
    ...
    SHR_IF_ERR_CONT
        (bcmi_ltsw_issu_detach(unit));

    SHR_IF_ERR_CONT
        (bcmi_ltsw_ha_cleanup(unit));
