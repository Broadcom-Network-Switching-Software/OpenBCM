...
/* MTOP route type. */
#define MTOP_ROUTE_TYPE(type) (((type) == bcmL3RouteGroupFortePrivateV4) || \
    ((type) == bcmL3RouteGroupForteGlobalV4) || \
    ((type) == bcmL3RouteGroupForteOverrideV4) || \
    ((type) == bcmL3RouteGroupFortePrivateV6) || \
    ((type) == bcmL3RouteGroupForteGlobalV6) || \
    ((type) == bcmL3RouteGroupForteOverrideV6))
...
l3_fib_object_capacity_get(int unit, uint32_t attr, uint32_t flags, int *entries)
{
    ...
    for (i = 0; i < BCMINT_LTSW_L3_FIB_TBL_CNT; i++) {
        if ((!(tbl_db.tbl_bmp & (1ULL << i))) ||
            (tbl_db.tbls[i].attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_SRC)) {
            continue;
        }

        if ((tbl_db.tbls[i].attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_MTOP) &&
            !(ltsw_feature(unit, LTSW_FT_MTOP))) {
            continue;
        }

        if ((tbl_db.tbls[i].attr & BCMINT_LTSW_L3_FIB_TBL_ATTR_HOST) &&
            (ltsw_feature(unit, LTSW_FT_ROUTE_SIMULATE_HOST))) {
            continue;
        }
 ...
 */
static int
l3_route_grp_entry_cnt_get(int unit, bcm_l3_route_group_t grp,
                           uint32_t *cnt, uint32_t *max_cnt)
{
    ...
    if (MTOP_ROUTE_TYPE(grp) && !ltsw_feature(unit, LTSW_FT_MTOP)) {
    ...
    switch (grp) {
        ...
        case bcmL3RouteGroupOverrideV6:
            ...
            break;
        case bcmL3RouteGroupFortePrivateV4:
            fib_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_VRF;
            break;
        case bcmL3RouteGroupForteGlobalV4:
            fib_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GL;
            break;
        case bcmL3RouteGroupForteOverrideV4:
            fib_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV4_UC_GH;
            break;
        case bcmL3RouteGroupFortePrivateV6:
            fib_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_VRF;
            break;
        case bcmL3RouteGroupForteGlobalV6:
            fib_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GL;
            break;
        case bcmL3RouteGroupForteOverrideV6:
            fib_tbl_id = BCMINT_LTSW_L3_FIB_TBL_ROUTE_MTOP_IPV6_UC_GH;
            break;
        default:
    ...
    if (max_cnt) {
        SHR_IF_ERR_EXIT
            (bcmi_lt_capacity_get(unit, ti->name, max_cnt));
    }
 ...
 */
static int
l3_alpm_level_n_usage_get(int unit, int tbl_id, bcm_l3_route_group_t grp,
                          int *max_bkt, int *inuse_bkt)
{
    int dunit, bk_cnt, i, j, fld;
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    const bcmint_ltsw_l3_fib_tbl_t *ti;
    const bcmint_ltsw_l3_fib_fld_t *fi;
    const char *sym_val;
    uint64_t val, max = 0, inuse = 0;
    bcmint_ltsw_l2_alpm_db_t db;
    int db_cnt = 0, db_arr[BCMINT_LTSW_L3_ALPM_DB_CNT] = {0};
    int bk_db[] = {
    ...
    if (MTOP_ROUTE_TYPE(grp)) {
        SHR_IF_ERR_EXIT
            (bcmint_ltsw_l3_fib_tbl_get(unit, BCMINT_LTSW_L3_FIB_TBL_ALPM_CONTROL_MTOP,
                                        &ti));
    } else {
        SHR_IF_ERR_EXIT
    ...
    if (db_cnt == 0) {
        ...
        /*
         * When ALPM COMBINED mode, only a single database (DB_0) is used for
         * global override, global and VRF routes.
         */
        db = db_arr[0];
    } else if (db_cnt == 2) {
        /*
         * When ALPM PARALLEL mode, two databases (DB_0 and DB_1) are used.
         * The lower numbered Database is for private routes.
         * The higher numbered Databse is for global routes.
         */
        if ((grp == bcmL3RouteGroupPrivateV4) ||
            (grp == bcmL3RouteGroupPrivateV6) ||
            (grp == bcmL3RouteGroupFortePrivateV4) ||
            (grp == bcmL3RouteGroupFortePrivateV6)) {
            db = db_arr[0] < db_arr[1] ? db_arr[0] : db_arr[1];
        } else {
            db = db_arr[0] > db_arr[1] ? db_arr[0] : db_arr[1];
        }
    } else {
    ...
    SHR_IF_ERR_EXIT
        (fi->scalar_to_symbol(unit, db, &sym_val));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_symbol_add_by_id(eh, fi->fld_id, sym_val));
    SHR_IF_ERR_EXIT
                             ...
                             int *prj_cnt, int *max_usage_lvl, int *max_usage)
{
    ...
    uint32_t grp_route_cnt[bcmL3RouteGroupCounter] = {0};
    ...
    if ((alpm_temp == 1) || (alpm_temp == 3) || (alpm_temp == 5) ||
        ...
        (ltsw_feature(unit, LTSW_FT_LPM_L3MC) && alpm_temp == 8)) {
    ...
    for (i = 0; i < bcmL3RouteGroupCounter; i++)  {
        if (MTOP_ROUTE_TYPE(i) && !ltsw_feature(unit, LTSW_FT_MTOP)) {
            continue;
        }

        SHR_IF_ERR_EXIT
            (l3_route_grp_entry_cnt_get(unit, i, &grp_route_cnt[i], NULL));
    }
    ...
    if (alpm_mode_combined) {
        ...
        } else if (grp >= bcmL3RouteGroupPrivateV6 &&
                   grp < bcmL3RouteGroupFortePrivateV4) {
            route_cnt = grp_route_cnt[bcmL3RouteGroupPrivateV6] +
                        grp_route_cnt[bcmL3RouteGroupGlobalV6];
        } else if (grp >= bcmL3RouteGroupFortePrivateV4 &&
                   grp < bcmL3RouteGroupFortePrivateV6) {
            route_cnt = grp_route_cnt[bcmL3RouteGroupFortePrivateV4] +
                        grp_route_cnt[bcmL3RouteGroupForteGlobalV4];
        } else {
            route_cnt = grp_route_cnt[bcmL3RouteGroupFortePrivateV6] +
                        grp_route_cnt[bcmL3RouteGroupForteGlobalV6];
        }
    } else {
    ...
    if ((((max_usage100 + 50)/ 100) < ALPM_PRJ_MIN_USAGE) ||
        ((max_lvl == 1) &&
        ((grp_route_cnt[bcmL3RouteGroupForteOverrideV4] +
          grp_route_cnt[bcmL3RouteGroupForteOverrideV6]) > 2))) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
                         ...
                         int *max_usage_lvl, int *max_usage)
{
    ...
    switch (grp) {
        ...
        case bcmL3RouteGroupGlobalV6:
            ...
            break;
        case bcmL3RouteGroupFortePrivateV4:
            grp_opp = bcmL3RouteGroupFortePrivateV6;
            ip6 = false;
            break;
        case bcmL3RouteGroupForteGlobalV4:
            grp_opp = bcmL3RouteGroupForteGlobalV6;
            ip6 = false;
            break;
        case bcmL3RouteGroupFortePrivateV6:
            grp_opp = bcmL3RouteGroupFortePrivateV4;
            ip6 = true;
            break;
        case bcmL3RouteGroupForteGlobalV6:
            grp_opp = bcmL3RouteGroupForteGlobalV4;
            ip6 = true;
                break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
...
bcmi_ltsw_l3_vrf_max_get(int unit, uint32_t *max_vrf)
{
    ...
    SHR_NULL_CHECK(max_vrf, SHR_E_PARAM);
    if (!L3_FIB_INITED(unit)) {
        SHR_ERR_EXIT(SHR_E_INIT);
    }

    *max_vrf = L3_VRF_MAX(unit);
...
bcmi_ltsw_l3_fib_mc_add(int unit, bcmi_ltsw_l3_fib_t *fib)
{
    ...
    if (fib->network || ltsw_feature(unit, LTSW_FT_ROUTE_SIMULATE_HOST)) {
...
int
bcmi_ltsw_l3_fib_mc_flexctr_object_set(int unit, bcmi_ltsw_l3_fib_t *fib,
                                       uint32_t val)
{
    SHR_FUNC_ENTER(unit);
    ...
    if (fib->network || ltsw_feature(unit, LTSW_FT_ROUTE_SIMULATE_HOST)) {
        if (fib->flags & BCM_L3_IP6) {
            SHR_IF_ERR_EXIT
                (mbcm_ltsw_l3_route_ipv6_mc_flexctr_object_set(unit, fib, val));
        } else {
            SHR_IF_ERR_EXIT
                (mbcm_ltsw_l3_route_ipv4_mc_flexctr_object_set(unit, fib, val));
        }
    } else {
        if (fib->flags & BCM_L3_IP6) {
            SHR_IF_ERR_EXIT
                (mbcm_ltsw_l3_host_ipv6_mc_flexctr_object_set(unit, fib, val));
        } else {
            SHR_IF_ERR_EXIT
                (mbcm_ltsw_l3_host_ipv4_mc_flexctr_object_set(unit, fib, val));
        }
    }
...
int
bcmi_ltsw_l3_fib_mc_flexctr_object_get(int unit, bcmi_ltsw_l3_fib_t *fib,
                                       uint32_t *val)
{
    SHR_FUNC_ENTER(unit);
    ...
    if (fib->network || ltsw_feature(unit, LTSW_FT_ROUTE_SIMULATE_HOST)) {
        if (fib->flags & BCM_L3_IP6) {
            SHR_IF_ERR_EXIT
                (mbcm_ltsw_l3_route_ipv6_mc_flexctr_object_get(unit, fib, val));
        } else {
            SHR_IF_ERR_EXIT
                (mbcm_ltsw_l3_route_ipv4_mc_flexctr_object_get(unit, fib, val));
        }
    } else {
        if (fib->flags & BCM_L3_IP6) {
            SHR_IF_ERR_EXIT
                (mbcm_ltsw_l3_host_ipv6_mc_flexctr_object_get(unit, fib, val));
        } else {
            SHR_IF_ERR_EXIT
                (mbcm_ltsw_l3_host_ipv4_mc_flexctr_object_get(unit, fib, val));
        }
    }
...
int
bcmi_ltsw_l3_fib_info_get(int unit, bcmi_ltsw_l3_fib_info_t *info)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_l3_fib_info_get(unit, info));
                                ...
                                uint32_t flags, int *entries)
{
    ...
    switch (object) {
        ...
        case BCMI_LTSW_L3_FIB_OBJ_HOST_USED:
            if (ltsw_feature(unit, LTSW_FT_ROUTE_SIMULATE_HOST)) {
                *entries = info.host_v4_used + 2 * info.host_v6_used;
                SHR_EXIT();
            }
    ...
    if (cap) {
        ...
        if (ltsw_feature(unit, LTSW_FT_ROUTE_SIMULATE_HOST)) {
            if (object == BCMI_LTSW_L3_FIB_OBJ_ROUTE_V4_USED) {
                *entries = *entries - info.host_v4_used;
            }
            if (object == BCMI_LTSW_L3_FIB_OBJ_ROUTE_V6_128_USED) {
                *entries = *entries - info.host_v6_used;
            }
            if (object == BCMI_LTSW_L3_FIB_OBJ_ROUTE_USED) {
                *entries = *entries - info.host_v4_used - 4 * info.host_v6_used;
            }
        }
    }
                                  ...
                                  int *max_usage)
{
    ...
    if ((grp == bcmL3RouteGroupOverrideV4) ||
        ...
        (grp == bcmL3RouteGroupForteOverrideV4) ||
        (grp == bcmL3RouteGroupForteOverrideV6)) {
        uint32_t prj_max = 0;
        ...
        SHR_IF_ERR_EXIT
            (l3_route_grp_entry_cnt_get(unit, grp, NULL, &prj_max));
                              ...
                              int *max_usage)
{
    ...
    if ((grp == bcmL3RouteGroupOverrideV4) ||
        ...
        SHR_IF_ERR_EXIT
            (l3_route_grp_entry_cnt_get(unit, grp, NULL, &uni_prj_max));

        if (*uni_prj_cnt > uni_prj_max) {
            *uni_prj_cnt = uni_prj_max;
        }
...
int
bcmi_ltsw_l3_fib_sw_dump(int unit)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_EXIT
        (mbcm_ltsw_l3_fib_sw_dump(unit));
...
bcm_ltsw_l3_host_delete_by_network(int unit, bcm_l3_route_t *net_addr)
{
    ...
    if ((net_addr->l3a_flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) &&
        (!ltsw_feature(unit, LTSW_FT_MTOP))) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
 ...
 *
 * \param [in] unit Unit number.
 * \param [in] flags L3 host entry flags.
 ...
 */
int
bcm_ltsw_l3_host_age(int unit, uint32 flags, bcm_l3_host_traverse_cb age_out,
                     void *user_data)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_l3_host_age(unit, age_out, user_data));
...
bcm_ltsw_l3_route_add(int unit, bcm_l3_route_t *info)
{
    ...
    if ((info->l3a_flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) &&
        (!ltsw_feature(unit, LTSW_FT_MTOP))) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
bcm_ltsw_l3_route_find(int unit, bcm_l3_host_t *host, bcm_l3_route_t *route)
{
    ...
    if ((route->l3a_flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) &&
        (!ltsw_feature(unit, LTSW_FT_MTOP))) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
                              ...
                              bcm_l3_route_t *route)
{
    ...
    if ((input->l3a_flags3 & BCM_L3_FLAGS3_FORTE_ROUTE) &&
        (!ltsw_feature(unit, LTSW_FT_MTOP))) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
                              ...
                              bcm_l3_alpm_resource_t *resource)
{
    ...
    SHR_IF_ERR_EXIT
        (l3_route_grp_entry_cnt_get(unit, grp, &route_cnt, NULL));
...
/*!
 * \brief Attach counter entries to the given L3 host entry.
 *
 * \param [in] unit Unit number.
 * \param [in] info L3 host info.
 * \param [in] stat_counter_id Stat Counter ID.
 *
 * \retval SHR_E_NONE No errors.
 */
    ...
    uint32_t stat_counter_id)
{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        fib.ip_addr = info->l3a_ip_addr;
    }

    SHR_IF_ERR_EXIT
        (bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id, &ci));

    if (ci.source != bcmFlexctrSourceL3Host) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_l3_host_flexctr_info_get(unit, &fib, &pre_ci));
    ...
    SHR_IF_ERR_CONT
        (mbcm_ltsw_l3_host_flexctr_attach(unit, &fib, &ci));
...
/*!
 * \brief Detach counter entries of the given L3 host entry.
 *
 * \param [in] unit Unit number.
 * \param [in] info L3 host info.
 *
 ...
 */
int
bcm_ltsw_l3_host_stat_detach(int unit, bcm_l3_host_t *info)
{
    bcmi_ltsw_flexctr_counter_info_t ci;
    bool locked = false;
    bcmi_ltsw_l3_fib_t fib;
    ...
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_l3_host_flexctr_info_get(unit, &fib, &ci));
    ...
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_l3_host_flexctr_detach(unit, &fib));
...
/*!
 * \brief Get stat counter id associated with the given L3 host entry.
 *
 * \param [in] unit Unit number.
 * \param [in] info L3 host info.
 * \param [in] stat Type of the counter to retrieve.
 ...
 */
int
bcm_ltsw_l3_host_flexctr_object_set(
    int unit,
    bcm_l3_host_t *info,
    uint32_t value)
{
    ...
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_l3_host_flexctr_object_set(unit, &fib,
                                              value));
 ...
 */
int
bcm_ltsw_l3_host_flexctr_object_get(
    int unit,
    bcm_l3_host_t *info,
    uint32_t *value)
{
    ...
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_l3_host_flexctr_object_get(unit, &fib, value));
