                             ...
                             bcmi_ltsw_vlan_egr_xlate_cfg_t *cfg)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_vlan_egr_xlate_add(unit, port_class,
                                      outer_vlan, inner_vlan, cfg));
exit:
    SHR_FUNC_EXIT();
}
                             ...
                             bcmi_ltsw_vlan_egr_xlate_cfg_t *cfg)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_vlan_egr_xlate_get(unit, port_class,
                                      outer_vlan, inner_vlan, cfg));
exit:
    SHR_FUNC_EXIT();
}
...

{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_vlan_egr_xlate_delete(unit, port_class,
                                         outer_vlan, inner_vlan, cfg));
                                  ...
                                  void *user_data)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_vlan_egr_xlate_traverse(unit, cb, user_data));
exit:
    SHR_FUNC_EXIT();
}
...
bcmi_ltsw_vlan_port_attach(int unit, bcm_port_t port)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (vlan_port_params_validate(unit, *(vi->def_vlan), pbmp, pbmp));

    VLAN_LOCK(unit);
    SHR_IF_ERR_CONT
        (vlan_port_add(unit, *(vi->def_vlan), pbmp, pbmp, pbmp, 0));
    VLAN_UNLOCK(unit);
...
bcmi_ltsw_vlan_port_detach(int unit, bcm_port_t port)
{
    ...
    VLAN_LOCK(unit);
    SHR_IF_ERR_CONT
        (mbcm_ltsw_vlan_port_remove(unit, *(vi->def_vlan), pbmp, pbmp, pbmp));
    VLAN_UNLOCK(unit);
...
bcmi_ltsw_vlan_sw_dump(int unit)
{
    ...
    /* Print payload TPID list sorted. */
    if (VLAN_PAYLOAD_OTPID_INFO(unit)->initialized) {
        LOG_CLI((BSL_META_U(unit,
                            "  Payload TPID Count : %4d\n"),
                            VLAN_PAYLOAD_OTPID_MAX_NUM));
        LOG_CLI((BSL_META_U(unit,
                            "  List               :")));
        for (i = 0; i < VLAN_PAYLOAD_OTPID_MAX_NUM; i++) {
            LOG_CLI((BSL_META_U(unit,
                                " 0x%4x (ref_count = %4d)"),
                                VLAN_PAYLOAD_OTPID_ENTRY(unit, i),
                                VLAN_PAYLOAD_OTPID_REF_COUNT(unit, i)));
            if (i < VLAN_PAYLOAD_OTPID_MAX_NUM - 1) {
                LOG_CLI((BSL_META_U(unit, "\n                      ")));
            }
        }
    }
                        ...
                        bcmi_ltsw_vlan_traverse_cb cb, void *user_data)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_vlan_traverse(unit, ingress, cb, user_data));
exit:
    SHR_FUNC_EXIT();
}
