...
#include <bcm_int/ltsw/generated/pktio_ha.h>
...
#define BCMINT_PKTIO_SUB_COMP_ID_INFO   0

#define PKTIO_INFO_HA_INSTANCE_NUM      1
...
#define PKTIO_F_HIGIG2  0x8
    ...
    /*! Sync Rx serial number. */
    uint64_t sn;

    /*! Head of packet list for sync Rx. */
    bcm_pktio_pkt_t *head;

    /*! Tail of packet list for sync Rx. */
    bcm_pktio_pkt_t *tail;

    /*! Current packet in sync Rx. */
    bcm_pktio_pkt_t *curr;
 ...
 */
static int
ltsw_pktio_pkt_data_dump(int unit, bcm_pktio_pkt_t *pkt)
{
    uint8_t *data = NULL;
    shr_pb_t *pb = NULL;
    uint32_t len, idx;
    ...
    rv = bcm_ltsw_pktio_pkt_data_get(unit, pkt, (void *)&data, &len);
    if (BCM_FAILURE(rv)) {
    ...
    shr_pb_printf(pb, "\r                        \n");
    shr_pb_printf(pb, "[RX packet data]\n");
...
pktio_rx_handler(int dunit, int netif_id, bcmpkt_packet_t *packet, void *cookie)
{
    ...
    if (pi->sync_rx) {
        rv = bcm_ltsw_pktio_claim(unit, (bcm_pktio_pkt_t *)packet, &pkt);
        if (SHR_FAILURE(rv)) {
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META_U(unit, "Sync-Rx: Failed to claim rx packet.\n")));
        ...
        if (pi->sync_flags & BCM_PKTIO_SYNC_RX_PKT_DUMP) {
            ltsw_pktio_pkt_data_dump(unit, pkt);
            bcmi_ltsw_pktio_rx_dump(unit, pkt, 1);
        }

        handled = TRUE;
        sal_sem_give(pi->sync_wait);
    } else {
                               ...
                               bcm_pktio_filter_t *dst)
{
    ...
    if (pi->flags & PKTIO_F_RXFLEX) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpkt_rxpmd_flex_reason_max_get(pi->variant, &reason_count));

        /* Convert SDKLT reason to HSDK reason. */
        SHR_BIT_ITER(filter->m_flex_reason.pbits, reason_count, reason) {
            hdr_info.pmd_type = bcmPktioPmdTypeRxFlex;
            hdr_info.fid = reason;
...
ltsw_pktio_init(int unit)
{
    ...
    /* Create mutex. */
    pi->mutex = sal_mutex_create("ltswPktioMutex");
    SHR_NULL_CHECK(pi->mutex, SHR_E_MEMORY);

    pi->sync_mode = sal_sem_create("ltswPktioSyncMode", 1, 1);
    SHR_NULL_CHECK(pi->sync_mode, SHR_E_MEMORY);

    pi->sync_wait = sal_sem_create("ltswPktioSyncWait", 1, 0);
    SHR_NULL_CHECK(pi->sync_wait, SHR_E_MEMORY);

    ha_instance_size = sizeof(ltsw_pktio_info_ha_t);
    ha_array_size = PKTIO_INFO_HA_INSTANCE_NUM;
    ha_req_size = ha_instance_size * ha_array_size;
    ha_alloc_size = ha_req_size;
    alloc_ptr = bcmi_ltsw_ha_mem_alloc(unit,
                                       BCMI_HA_COMP_ID_PKTIO,
                                       BCMINT_PKTIO_SUB_COMP_ID_INFO,
                                       "bcmPktIOInfo",
                                       &ha_alloc_size);
    ...
    rv = bcmi_ltsw_issu_struct_info_report(unit,
            ...
            LTSW_PKTIO_INFO_HA_T_ID);
    if (rv != SHR_E_EXISTS) {
       SHR_IF_ERR_VERBOSE_EXIT(rv);
    }

    hi = (ltsw_pktio_info_ha_t *)alloc_ptr;
    ...
    /* RXPMD FLEX */
    rv = bcmpkt_flexhdr_header_id_get(variant, "RXPMD_FLEX_T", &hid);
    if (SHR_SUCCESS(rv)) {
        hdr_info.pmd_type = bcmPktioPmdTypeRxFlex;
    ...
    /* GENERIC LOOPBACK */
    rv = bcmpkt_flexhdr_header_id_get(variant, "generic_loopback_t", &hid);
    if (SHR_SUCCESS(rv)) {
    ...
    /* HG3 */
    rv = bcmpkt_flexhdr_header_id_get(variant, "hg3_base_t", &hid);
    if (SHR_SUCCESS(rv)) {
    ...
    if (!(pi->flags & PKTIO_F_HIGIG3)) {
        hdr_info.pmd_type = bcmPktioPmdTypeHigig2;

        rv = mbcm_ltsw_pktio_fid_support_get(unit, &hdr_info,
...
exit:
    if (SHR_FUNC_ERR()) {
        ...
        if (pi->sync_mode) {
            sal_sem_destroy(pi->sync_mode);
            pi->sync_mode = NULL;
        }

        if (pi->sync_wait) {
            sal_sem_destroy(pi->sync_wait);
            pi->sync_wait = NULL;
        }
    }
...
ltsw_pktio_cleanup(int unit)
{
    ...
    bcm_ltsw_pktio_sync_rx_stop(unit, pi->sn);
...
static const struct {
    ...
    /*! Field ID. */
    int fid;
} ltsw_pktio_hg2_field_info[BCM_PKTIO_HG2_FID_COUNT] = {
    {"START",                          BCM_PKTIO_HG2_START},
    {"MCST",                           BCM_PKTIO_HG2_MCST},
    {"TC",                             BCM_PKTIO_HG2_TC},
    {"DST_MODID_MGIDH",                BCM_PKTIO_HG2_DST_MODID_MGIDH},
    {"DST_PORT_MGIDL",                 BCM_PKTIO_HG2_DST_PORT_MGIDL},
    {"SRC_MODID",                      BCM_PKTIO_HG2_SRC_MODID},
    {"SRC_PORT",                       BCM_PKTIO_HG2_SRC_PORT},
    {"LBID",                           BCM_PKTIO_HG2_LBID},
    {"DP",                             BCM_PKTIO_HG2_DP},
    {"EHV",                            BCM_PKTIO_HG2_EHV},
    {"CC",                             BCM_PKTIO_HG2_CC},
    {"PPD_TYPE",                       BCM_PKTIO_HG2_PPD_TYPE},
    {"PPD0_DST_T",                     BCM_PKTIO_HG2_PPD0_DST_T},
    {"PPD0_DST_TGID",                  BCM_PKTIO_HG2_PPD0_DST_TGID},
    {"PPD0_INGRESS_TAGGED",            BCM_PKTIO_HG2_PPD0_INGRESS_TAGGED},
    {"PPD0_MIRROR_ONLY",               BCM_PKTIO_HG2_PPD0_MIRROR_ONLY},
    {"PPD0_MIRROR_DONE",               BCM_PKTIO_HG2_PPD0_MIRROR_DONE},
    {"PPD0_MIRROR",                    BCM_PKTIO_HG2_PPD0_MIRROR},
    {"PPD0_LABEL_OVERLAY_TYPE",        BCM_PKTIO_HG2_PPD0_LABEL_OVERLAY_TYPE},
    {"PPD0_L3",                        BCM_PKTIO_HG2_PPD0_L3},
    {"PPD0_LABEL_PRESENT",             BCM_PKTIO_HG2_PPD0_LABEL_PRESENT},
    {"PPD0_VC_LABEL_19_16",            BCM_PKTIO_HG2_PPD0_VC_LABEL_19_16},
    {"PPD0_VC_LABEL",                  BCM_PKTIO_HG2_PPD0_VC_LABEL},
    {"PPD0_REP_ID",                    BCM_PKTIO_HG2_PPD0_REP_ID},
    {"PPD0_ORIG_SRC_MODID",            BCM_PKTIO_HG2_PPD0_ORIG_SRC_MODID},
    {"PPD0_ORIG_SRC_PORT",             BCM_PKTIO_HG2_PPD0_ORIG_SRC_PORT},
    {"PPD0_MIRROR_CLASSIFICATION_TAG", BCM_PKTIO_HG2_PPD0_MIRROR_CLASSIFICATION_TAG},
    {"PPD0_VID_HIGH",                  BCM_PKTIO_HG2_PPD0_VID_HIGH},
    {"PPD0_VID_LOW",                   BCM_PKTIO_HG2_PPD0_VID_LOW},
    {"PPD0_PFM",                       BCM_PKTIO_HG2_PPD0_PFM},
    {"PPD0_SRC_T",                     BCM_PKTIO_HG2_PPD0_SRC_T},
    {"PPD0_PRESERVE_DSCP",             BCM_PKTIO_HG2_PPD0_PRESERVE_DSCP},
    {"PPD0_PRESERVE_DOT1P",            BCM_PKTIO_HG2_PPD0_PRESERVE_DOT1P},
    {"PPD0_OPCODE",                    BCM_PKTIO_HG2_PPD0_OPCODE},
    {"PPD0_HDR_EXT_LEN",               BCM_PKTIO_HG2_PPD0_HDR_EXT_LEN},
    {"PPD1_CLASSIFICATION_TAG",        BCM_PKTIO_HG2_PPD1_CLASSIFICATION_TAG},
    {"PPD1_VID_HIGH",                  BCM_PKTIO_HG2_PPD1_VID_HIGH},
    {"PPD1_VID_LOW",                   BCM_PKTIO_HG2_PPD1_VID_LOW},
    {"PPD1_PFM",                       BCM_PKTIO_HG2_PPD1_PFM},
    {"PPD1_SRC_T",                     BCM_PKTIO_HG2_PPD1_SRC_T},
    {"PPD1_OPCODE",                    BCM_PKTIO_HG2_PPD1_OPCODE},
    {"PPD1_HDR_EXT_LEN",               BCM_PKTIO_HG2_PPD1_HDR_EXT_LEN},
    {"PPD2_MULTIPOINT",                BCM_PKTIO_HG2_PPD2_MULTIPOINT},
    {"PPD2_FWD_TYPE",                  BCM_PKTIO_HG2_PPD2_FWD_TYPE},
    {"PPD2_VNI_MID",                   BCM_PKTIO_HG2_PPD2_VNI_MID},
    {"PPD2_VNI_LOW",                   BCM_PKTIO_HG2_PPD2_VNI_LOW},
    {"PPD2_DEST_VP",                   BCM_PKTIO_HG2_PPD2_DEST_VP},
    {"PPD2_REP_ID_15_0",               BCM_PKTIO_HG2_PPD2_REP_ID_15_0},
    {"PPD2_SOURCE_VP",                 BCM_PKTIO_HG2_PPD2_SOURCE_VP},
    {"PPD2_MIRROR",                    BCM_PKTIO_HG2_PPD2_MIRROR},
    {"PPD2_DO_NOT_MODIFY",             BCM_PKTIO_HG2_PPD2_DO_NOT_MODIFY},
    {"PPD2_DO_NOT_LEARN",              BCM_PKTIO_HG2_PPD2_DO_NOT_LEARN},
    {"PPD2_LAG_FAILOVER",              BCM_PKTIO_HG2_PPD2_LAG_FAILOVER},
    {"PPD2_OPCODE",                    BCM_PKTIO_HG2_PPD2_OPCODE},
    {"PPD2_REP_ID_17_16",              BCM_PKTIO_HG2_PPD2_REP_ID_17_16},
    {"PPD2_VNI_HIGH",                  BCM_PKTIO_HG2_PPD2_VNI_HIGH},
    {"PPD2_PRESERVE_DSCP",             BCM_PKTIO_HG2_PPD2_PRESERVE_DSCP},
    {"PPD2_PRESERVE_DOT1P",            BCM_PKTIO_HG2_PPD2_PRESERVE_DOT1P},
    {"PPD2_DEST_TYPE",                 BCM_PKTIO_HG2_PPD2_DEST_TYPE},
    {"PPD2_SOURCE_TYPE",               BCM_PKTIO_HG2_PPD2_SOURCE_TYPE},
    {"PPD3_DATA_CONTAINER",            BCM_PKTIO_HG2_PPD3_DATA_CONTAINER},
    {"PPD3_SOURCE_INFO",               BCM_PKTIO_HG2_PPD3_SOURCE_INFO},
    {"PPD3_PRESERVE_DSCP",             BCM_PKTIO_HG2_PPD3_PRESERVE_DSCP},
    {"PPD3_PRESERVE_DOT1P",            BCM_PKTIO_HG2_PPD3_PRESERVE_DOT1P},
    {"PPD3_DO_NOT_LEARN",              BCM_PKTIO_HG2_PPD3_DO_NOT_LEARN},
    {"PPD3_SOURCE_TYPE",               BCM_PKTIO_HG2_PPD3_SOURCE_TYPE},
    {"PPD3_OPCODE",                    BCM_PKTIO_HG2_PPD3_OPCODE},
    {"PPD3_CONTAINER_TYPE",            BCM_PKTIO_HG2_PPD3_CONTAINER_TYPE}
};

static int
ltsw_pktio_hg2_dump(int unit, bcm_pktio_pkt_t *packet, shr_pb_t *pb)
{
    ltsw_pktio_info_t *pi = pktio_info[unit];
    uint32_t val;
    int fid, last = BCM_PKTIO_HG2_FID_COUNT;

    SHR_FUNC_ENTER(unit);

    for (fid = BCM_PKTIO_HG2_FID_START; fid < last; fid++) {
        if (BCM_PKTIO_FID_SUPPORT_GET(pi->support.higig2_support, fid)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm_ltsw_pktio_pmd_field_get(unit, packet, bcmPktioPmdTypeHigig2,
                                              fid, &val));
            if (val || fid == BCM_PKTIO_HG2_PPD_TYPE) {
                shr_pb_printf(pb, "\t%s=", ltsw_pktio_hg2_field_info[fid].name);
                shr_pb_format_uint32(pb, NULL, &val, 1, 0);
                shr_pb_printf(pb, "\n");
            }
            if (fid == BCM_PKTIO_HG2_PPD_TYPE) {
                switch (val) {
                case 0:
                    last = BCM_PKTIO_HG2_PPD1_CLASSIFICATION_TAG;
                    break;
                case 1:
                    fid = BCM_PKTIO_HG2_PPD1_CLASSIFICATION_TAG - 1;
                    last = BCM_PKTIO_HG2_PPD2_MULTIPOINT;
                    break;
                case 2:
                    fid = BCM_PKTIO_HG2_PPD2_MULTIPOINT - 1;
                    last = BCM_PKTIO_HG2_PPD3_DATA_CONTAINER;
                    break;
                case 3:
                    fid = BCM_PKTIO_HG2_PPD3_DATA_CONTAINER - 1;
                    break;
                default:
                             ...
                             int fid, uint32_t val)
{
    ...
    switch (type) {
        ...
        case bcmPktioPmdTypeGih:
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpkt_flexhdr_header_id_get(pi->variant, "generic_loopback_t",
                                              &hid));

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpkt_flexhdr_field_set(pi->variant, hid, lbhdr,
        ...
        case bcmPktioPmdTypeHigig3:
            ...
            if (fid >= BCM_PKTIO_HG3_EXT0_FID_OFFSET) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmpkt_flexhdr_header_id_get(pi->variant,
                                                  "hg3_extension_0_t",
                                                  &hid));

                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmpkt_flexhdr_field_set(pi->variant, hid, hghdr,
                                              BCMPKT_FLEXHDR_PROFILE_NONE,
                                              fid, val));
            } else {
                fid = fid_info->lt_fid;
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmpkt_flexhdr_header_id_get(pi->variant, "hg3_base_t",
                                                  &hid));
        ...
        case bcmPktioPmdTypeHigig2:
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (mbcm_ltsw_pktio_hg2_field_set(unit, hghdr, fid, val));
                             ...
                             int fid, uint32_t *val)
{
    ...
    switch (type) {
        ...
        case bcmPktioPmdTypeGih:
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpkt_flexhdr_field_get(pi->variant, hid, lbhdr,
                                          BCMPKT_FLEXHDR_PROFILE_NONE,
                                          fid, val));
        ...
        case bcmPktioPmdTypeHigig3:
            ...
            if (fid >= BCM_PKTIO_HG3_EXT0_FID_OFFSET) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmpkt_flexhdr_field_get(pi->variant, hid, hghdr,
                                              BCMPKT_FLEXHDR_PROFILE_NONE,
                                              fid, val));
            } else {
                fid = fid_info->lt_fid;
        ...
        case bcmPktioPmdTypeHigig2:
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (mbcm_ltsw_pktio_hg2_field_get(unit, hghdr, fid, val));
        ...
        case bcmPktioPmdTypeRxFlex:
            if (fid >= BCM_PKTIO_RXPMD_FLEX_FID_OFFSET) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmpkt_flexhdr_header_id_get(pi->variant, "RXPMD_FLEX_T",
                                                  &hid));

                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmpkt_flexhdr_field_get(pi->variant, hid, flexpmd,
    ...
    switch (type) {
        ...
        case bcmPktioPmdTypeRxFlex:
            if (fid >= BCM_PKTIO_RXPMD_FLEX_FID_OFFSET) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmpkt_flexhdr_field_set(pi->variant, hid, flexpmd,
                                              flex_profile, fid, val));
            } else {
                rxpmd = pmd->rxpmd;
                                 ...
                                 int fid, uint32_t *val)
{
    ...
    switch (type) {
        ...
        case bcmPktioPmdTypeHigig2:
            ...
            /* Get from Module Header if Higig buffer in packet->pmd is NULL */
            hghdr = pmd->higig[0] ? pmd->higig : pmd->rxpmd;
                               ...
                               bcm_pktio_reasons_t *reasons)
{
    ...
    if (pi->flags & PKTIO_F_RXFLEX) {
        ...
        /* Convert SDKLT reason to HSDK reason. */
        SHR_BIT_ITER(flex_reasons.pbits, reason_count, reason) {
            hdr_info.pmd_type = bcmPktioPmdTypeRxFlex;
            hdr_info.fid = reason;
                                   ...
                                   bcm_pktio_reasons_t *reasons)
{
    ...
    bcmpkt_bitmap_t flex_reasons = {{0}};
    bcmint_pktio_hdr_info_t hdr_info = {0};
    bcmint_pktio_fid_info_t *fid_info;
...
int
bcm_ltsw_pktio_sync_rx_start(int unit, uint32_t flags, uint64_t *sn)
{
    ltsw_pktio_info_t *pi = pktio_info[unit];
    ...
    SHR_NULL_CHECK(sn, SHR_E_PARAM);

    sal_sem_take(pi->sync_mode, SAL_SEM_FOREVER);

    LTSW_PKTIO_LOCK(unit);

    pi->sync_rx = 1;
    pi->sync_flags = flags;
    pi->sn = (unsigned long)sal_thread_self();
    *sn = pi->sn;
...
int
bcm_ltsw_pktio_sync_rx_stop(int unit, uint64_t sn)
{
    ltsw_pktio_info_t *pi = pktio_info[unit];
    bcm_pktio_pkt_t *pkt;
    ...
    if (!pi->sync_rx || sn != pi->sn) {
        LTSW_PKTIO_UNLOCK(unit);
        SHR_ERR_EXIT(SHR_E_BADID);
    }

    if (pi->curr) {
        bcm_ltsw_pktio_free(unit, pi->curr);
        pi->curr = NULL;
    }
...
int
bcm_ltsw_pktio_sync_rx(int unit, uint64_t sn,
                       bcm_pktio_pkt_t **packet, int usec)
{
    ltsw_pktio_info_t *pi = pktio_info[unit];
    int rv;
    ...
    do {
        ...
        LTSW_PKTIO_UNLOCK(unit);
        rv = sal_sem_take(pi->sync_wait, usec);
        if (SHR_FAILURE(rv)) {
            *packet = NULL;
            SHR_ERR_EXIT(rv);
        }
...
int
bcm_ltsw_pktio_rx_rate_limit_set(int unit, int rate)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpkt_rx_rate_limit_set(dunit, rate));
...
int
bcm_ltsw_pktio_rx_rate_limit_get(int unit, int *rate)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpkt_rx_rate_limit_get(dunit, rate));
...
int
bcm_ltsw_pktio_trace_type_get(int unit, bcm_pktio_trace_type_t *trace_type)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_NULL_CHECK(trace_type, SHR_E_PARAM);

    if (ltsw_feature(unit, LTSW_FT_TRACE_DOP)) {
        *trace_type = bcmPktioTraceTypeDop;
    } else {
        *trace_type = bcmPktioTraceTypeFixed;
    }
...
bcmi_ltsw_pktio_rx_dump(int unit, bcm_pktio_pkt_t *packet, int flags)
{
    ...
    if (pi->flags & PKTIO_F_HIGIG2) {
        shr_pb_printf(pb, "[Higig2 fields]\n");
        ltsw_pktio_hg2_dump(unit, packet, pb);
    }
...
int
bcmi_ltsw_pktio_trace_dop_data_dump(int unit, uint32_t dop_id,
                                    bcm_pktio_trace_data_t *trace_data)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    uint32_t idx;
...
int
bcmi_ltsw_pktio_trace_fixed_data_dump(int unit, bcm_pktio_trace_data_t *trace_data)
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    shr_pb_t *pb = NULL;
    int idx;
    ...
    if (!trace_data->len) {
        LOG_CLI((BSL_META_U(unit, "Invalid trace data\n")));
        SHR_EXIT();
    }

    LOG_CLI((BSL_META_U(unit, "DATA:\n")));
    for (idx = 0; idx < trace_data->len; idx++) {
        if ((idx & 0xf) == 0) {
            LOG_CLI((BSL_META_U(unit, "[%04x]: "), idx));
        }
    ...
    shr_pb_printf(pb, "FIELDS:\n");

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpkt_trace_fields_dump(dunit, trace_data->buf, pb));
