...
/*! Count of being rediected to the trunk. */
#define TRUNK_REDIRECT_CNT(unit, tid) \
    (TRUNK_INFO(unit, tid)->redirect_cnt)

/*! Redirect destination trunk ID. */
#define TRUNK_IS_REDIRECTED(unit, tid) \
    (TRUNK_INFO(unit, tid)->redirected)
    ...
    bcm_trunk_member_t *member)
{
    ...
    if (0 == member_count) {
        ...
        SHR_IF_ERR_CONT
            (bcmi_ltsw_flow_match_trunk_member_delete(unit,
                                                      tid,
                                                      1,
                                                      &member->gport));
    }
    ...
    bcm_trunk_t tid)
{
    ...
    int member_count;
    bcm_trunk_member_t *member_array = NULL;
    bcm_port_t *trunk_member_array = NULL;
    int alloc_sz, i;
    ...
    if (member_count > 0) {
        ...
        SHR_ALLOC(trunk_member_array,
                  sizeof(bcm_port_t) * member_count,
                  "bcmTrunkPortArray");
        if (trunk_member_array == NULL) {
            SHR_ERR_EXIT(SHR_E_MEMORY);
        }
        sal_memset(trunk_member_array, 0, sizeof(bcm_port_t) * member_count);
        ...
        for (i=0; i < member_count; i++) {
            trunk_member_array[i] = member_array[i].gport;
        }
...
int bcmi_ltsw_trunk_redirect_config_set(
    int unit,
    bcm_gport_t trunk_gport,
    bcm_port_redirect_config_t *redirect_config)
{
    bcm_trunk_t src_tid, dst_tid, old_dst_tid;
    bcm_gport_t dst_trunk = redirect_config->destination;
    bcmint_trunk_info_t *src_info = NULL;
    bcmint_trunk_info_t *dst_info = NULL;
    int locked = 0;
    bcmint_trunk_type_t t_type;
    ...
    if (!BCM_GPORT_IS_TRUNK(dst_trunk)) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }

    src_tid = BCM_GPORT_TRUNK_GET(trunk_gport);
    dst_tid = BCM_GPORT_TRUNK_GET(dst_trunk);

    TRUNK_CHECK(unit, src_tid);
    TRUNK_CHECK(unit, dst_tid);

    src_info = TRUNK_INFO(unit, src_tid);
    dst_info = TRUNK_INFO(unit, dst_tid);

    TRUNK_LOCK(unit);
    locked = 1;

    if (src_info->tid == BCM_TRUNK_INVALID) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_NOT_FOUND);
    }

    if (dst_info->tid == BCM_TRUNK_INVALID) {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_NOT_FOUND);
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_trunk_id2type_id(unit, src_tid, &t_type, NULL));
    if (t_type != bcmintTrunkTypeFrontPanel) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (ltsw_trunk_id2type_id(unit, dst_tid, &t_type, NULL));
    if (t_type != bcmintTrunkTypeFrontPanel) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_trunk_redirect_config_get(unit, src_tid,
                                             src_info, &old_dst_tid));
    if (old_dst_tid == dst_tid) {
        SHR_EXIT();
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_trunk_redirect_config_set(unit, src_tid, src_info,
                                             dst_tid, dst_info));

    if (old_dst_tid != src_tid) {
        TRUNK_REDIRECT_CNT(unit, old_dst_tid)--;
    }
    TRUNK_REDIRECT_CNT(unit, dst_tid)++;

    if (src_tid == dst_tid) {
        TRUNK_IS_REDIRECTED(unit, src_tid) = 0;
    } else {
        TRUNK_IS_REDIRECTED(unit, src_tid) = 1;
    }
...
int bcmi_ltsw_trunk_redirect_config_get(
    int unit,
    bcm_gport_t trunk_gport,
    bcm_port_redirect_config_t *redirect_config)
{
    bcm_trunk_t src_tid, dst_tid;
    bcmint_trunk_info_t *src_info = NULL;
    int locked = 0;
    bcmint_trunk_type_t t_type;
    ...
    if (!TRUNK_IS_REDIRECTED(unit, src_tid)) {
        SHR_EXIT();
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_trunk_redirect_config_get(unit, src_tid, src_info, &dst_tid));

    BCM_GPORT_TRUNK_SET(redirect_config->destination, dst_tid);
