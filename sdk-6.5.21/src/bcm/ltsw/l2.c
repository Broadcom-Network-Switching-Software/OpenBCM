...
#define L2_INIT_CHECK(u)                                \
    do {                                                \
        ...
        if (!ltsw_feature(u, LTSW_FT_NO_L2)) {          \
            if (l2i->inited == false) {                 \
...
#define HSDK_L2_STATION_OVERLAY(u, station) \
    ((((station)->flags & BCM_L2_STATION_UNDERLAY) == 0) && \
     (HYBRID_STATION_SIZE(u) > 0))
...
l2_info_init(int unit)
{
    ...
    uint32_t ha_instance_size = 0, ha_array_size = 0, i;
    ...
    if (tbl_sz != 0) {
        ...
        l2i->sc_installed = false;
        if (!warm) {
            sal_memset(l2i->sc, 0, ha_alloc_size);
        } else {
            for (i = 0; i < tbl_sz; i++) {
                if (l2i->sc[i].installed) {
                    l2i->sc_installed = true;
                    break;
                }
            }
        }
...
l2_station_id_alloc(int unit, bool overlay, int *sid)
{
    ...
    if (overlay) {
        ...
        offset = 0;
        tbl_sz = UNDERLAY_STATION_SIZE(unit);
    }
 ...
 */
static int
l2_station_search(int unit, bool overlay, bcm_l2_station_t *l2station, bool *tnl, int *sid)
{
    bcmint_l2_station_control_t *sc = STATION_INFO(unit);
...
l2_station_add(int unit, int sid, bcm_l2_station_t *l2station, bool tnl)
{
    ...
    underlay = ((l2station->flags & BCM_L2_STATION_UNDERLAY) == 0) ? 0 : 1;
    ...
    sc[sid].use_underlay_flag = underlay;
    L2_INFO(unit)->sc_installed = true;
    return SHR_E_NONE;
}
                    ...
                    void *user_data)
{
    ...
    for (i = 0; i < tbl_sz; i++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (l2_station_get(unit, i, &station, &int_flags));
                           ...
                           bcm_l2_station_t *l2station)
{
    ...
    if (l2station->flags & BCM_L2_STATION_WITH_ID) {
        overlay = HSDK_L2_STATION_OVERLAY(unit, l2station);
        if (!overlay) {
            if (*station_id < 0 ||
                *station_id >= UNDERLAY_STATION_SIZE(unit)) {
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
        } else {
...
l2_station_update_by_vlan(int unit, int vid)
{
    ...
    if (L2_INFO(unit)->sc_installed == false) {
        SHR_EXIT();
    }
...
{
    int rv, sid;
    bool overlay = HSDK_L2_STATION_OVERLAY(unit, l2station);

    L2_SC_LOCK(unit);

    rv = l2_station_search(unit, overlay, l2station, tnl, &sid);
...
bcm_ltsw_l2_station_add(int unit, int *station_id, bcm_l2_station_t *station)
{
    ...
    locked = true;
    overlay = HSDK_L2_STATION_OVERLAY(unit, station);
    /* Search database */
    if (SHR_SUCCESS(l2_station_search(unit, overlay, station, &tnl, &sid))) {
...
bcm_ltsw_l2_station_get(int unit, int station_id, bcm_l2_station_t *station)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (l2_station_get(unit, station_id, station, &int_flags));
...
bcm_ltsw_l2_station_size_get(int unit, int *size)
{
    ...
    *size =  UNDERLAY_STATION_SIZE(unit);
...
{
    int sid = -1, int_flags = BCMINT_L2_STATION_TUNNEL;
    bcm_l2_station_t station;
    bool alloc = false, locked = false, tnl = false;
    ...
    /* Search database */
    if (SHR_SUCCESS(l2_station_search(unit, 0, &station, &tnl, &sid))) {
        if (tnl) {
            SHR_ERR_EXIT(SHR_E_EXISTS);
        } else {
...
bcm_ltsw_l2_tunnel_delete(int unit, bcm_mac_t mac, bcm_vlan_t vlan)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (l2_station_search(unit, 0, &station, &tnl, &sid));
...
bcmi_ltsw_l2_sw_dump(int unit)
{
    ...
    LOG_CLI((BSL_META_U(unit,
                        "  Global station install status : %8d\n"), l2i->sc_installed));
    LOG_CLI((BSL_META_U(unit,
                        "\n")));
...
bcm_ltsw_l2_port_native(int unit, int modid, int port)
{
    ...
    if ((SHR_FAILURE(rv) && (rv != SHR_E_UNAVAIL)) ||
        (flags & (BCM_STK_ENABLE | BCM_STK_INACTIVE))) {
        return FALSE;
    }
