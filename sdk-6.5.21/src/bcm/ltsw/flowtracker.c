...
/* HW ETRAP feature check. */
#define FT_HW_ETRAP_FEATURE_CHECK(_u_)                      \
    do {                                            \
        if (!ltsw_feature(_u_, LTSW_FT_ETRAP_HW)) {   \
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_UNAVAIL); \
        }                                           \
    } while(0)

char *bcm_ltsw_ft_template_element_str[bcmFlowtrackerExportElementTypeCount] =
{
    "SrcIPv4",
    "DstIPv4",
    "SrcIPv6",
...
int ltsw_flowtracker_resource_free(int unit)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_ETRAP_HW)) {   \
        SHR_IF_ERR_EXIT
...
{
    int lock = 0;
    bool is_ft_with_ex_match_supp = false;
    ...
    /* Check if flowtracker with exact match functionality is supported */
    SHR_IF_ERR_EXIT
        (mbcm_ltsw_flowtracker_with_exact_match_supported(unit,
                                                    &is_ft_with_ex_match_supp));

    if (is_ft_with_ex_match_supp == true) {
        /*
         * Call common detach routine only if the usual FT functionality
         * with EM is supported.
         */
...
bcm_ltsw_flowtracker_init(int unit)
{
    ...
    if (is_ft_with_ex_match_supp == true) {
        /*
         * Call common init routine only if the usual FT functionality
         * with EM is supported.
         */
        SHR_IF_ERR_EXIT
    ...
    bcm_flowtracker_elephant_match_data_t *match_data)
{
    ...
    FT_HW_ETRAP_FEATURE_CHECK(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm_ltsw_flowtracker_group_collector_attach_get_all_common
         (unit, flow_group_id, max_list_size, list_of_collectors,
          list_of_templates, list_of_export_profile_ids, list_size));
exit:
    if (flowtracker_lock == 1) {
        FLOWTRACKER_UNLOCK(unit);
    }
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_notify_config_set
 * Purpose:
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_notify_config_set(
    int unit,
    int pipe,
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_notify_config_get
 * Purpose:
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_notify_config_get(
    int unit,
    int pipe,
    bcm_flowtracker_flow_learn_notify_config_t  *config)
{
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_notify_register
 * Purpose:
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_notify_register(
    int unit,
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_notify_unregister
 * Purpose:
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_notify_unregister(int unit)
{
    int flowtracker_lock = 0;
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_fail_count_get
 * Purpose:
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_fail_count_get(
    int unit,
    int pipe,
    uint32 *count)
{
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flexctr_action_info_get
 * Purpose:
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flexctr_action_info_get(
    int unit,
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_fail_notify_register
 * Purpose:
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_fail_notify_register(
    int unit,
...
/*
 * Function:
 *      bcm_ltsw_flowtracker_flow_learn_fail_notify_unregister
 * Purpose:
 ...
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_ltsw_flowtracker_flow_learn_fail_notify_unregister(int unit)
{
    int flowtracker_lock = 0;
...
/*!
 * \brief Dump flowtracker sw state.
 *
 * \param [in]   unit         Unit number.
 *
 * \retval SHR_E_NONE    No errors.
 * \retval !SHR_E_NONE   Failure.
 */
int bcmi_ltsw_flowtracker_sw_dump(
        int unit)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    int group, i = 0;
    bcm_flowtracker_group_info_t info;
    uint32 flow_limit, aging_interval_ms;
    int max_list_size =2,list_size = 0, max_size = 19;
    bcm_flowtracker_collector_t list_of_collectors[2];
    int list_of_export_profile_ids[2];
    bcm_flowtracker_export_template_t list_of_templates[2];
    uint16 set_id = 0;
    bcm_flowtracker_export_element_info_t info_elements_arr1[19];
    int max_actions = 3;
    bcm_flowtracker_group_action_info_t action_list[3];
    int num_actions = 0;
    bcm_flowtracker_elephant_profile_t profile_id;
    bcm_flowtracker_flow_learn_notify_config_t config;
    SHR_FUNC_ENTER(unit);
    ...
    for (group = 1; group < FT_MAX_FLOW_GROUPS; group++) {
        if(SHR_BITGET(ft_info->flow_group_pool, group)) {
            SHR_IF_ERR_VERBOSE_EXIT(
                    bcm_ltsw_flowtracker_group_get(unit, group, &info));
            SHR_IF_ERR_VERBOSE_EXIT(
                    bcm_ltsw_flowtracker_group_flow_limit_get(unit, group, &flow_limit));
            SHR_IF_ERR_VERBOSE_EXIT(
                    bcm_ltsw_flowtracker_group_age_timer_get(unit, group, &aging_interval_ms));
            SHR_IF_ERR_VERBOSE_EXIT(
                    bcm_ltsw_flowtracker_group_collector_attach_get_all(unit, group, max_list_size,
                        list_of_collectors, list_of_export_profile_ids, list_of_templates, &list_size));
            if (list_size) {
                SHR_IF_ERR_VERBOSE_EXIT(
                        bcm_ltsw_flowtracker_export_template_get(unit,list_of_templates[0], &set_id, max_size, info_elements_arr1, &list_size));
            }
            SHR_IF_ERR_VERBOSE_EXIT(
                    bcm_ltsw_flowtracker_group_actions_get_common(unit, group, 0, max_actions, action_list, &num_actions));
                    bcm_ltsw_flowtracker_group_elephant_profile_attach_get_common(unit, group, &profile_id);


            cli_out("Flow group ID = %u \r\n",group);
            cli_out("EM group ID : %d \r\n",info.field_group[0]);
            cli_out("\tObservation domain ID = %u\r\n",info.observation_domain_id);
            cli_out("\tAging interval(ms) = %u\r\n",aging_interval_ms);
            cli_out("\tFlow limit = %u\r\n",flow_limit);
            if (num_actions) {
                cli_out("\tActions : \r\n");
                for (i=0; i < num_actions; i++) {
                    cli_out("\tAction[%d]= %s\r\n",i, (action_list[i].action == 9) ? "OpaqueObject" : "");
                }
            }

            cli_out("Elephant Profile configured : %s \r\n", profile_id ? "Yes" : "No");
            cli_out("\tCollector ID = %u\r\n",list_of_collectors[0]);
            cli_out("Template ID = %u\r\n",list_of_templates[0]);
            cli_out("\tSet ID = %u\r\n",set_id);
            cli_out("\tNum of export elements = %u\r\n",list_size);
            for (i=0; i < list_size; i++) {
                cli_out("\t\tElement[%d]= %s\r\n",i, bcm_ltsw_ft_template_element_str[info_elements_arr1[i].info_elem_id]);
            }
        }
    }
    ...
    if (ft_info->hw_learn_en == FT_HW_LEARN_ENABLE_WO_EAPP) {
        for(i=0; i< ft_info->num_pipes; i ++) {
            cli_out("Pipe : %d\n", i);
            SHR_IF_ERR_VERBOSE_EXIT(
            bcm_ltsw_flowtracker_flow_learn_notify_config_get_common(unit, i, &config));
            cli_out("\tFlow learn  notification configured : %s \r\n", (config.enable == 1) ? "Yes" : "No");
            cli_out("\tFlow learn failure notification configured : %s \r\n", (config.fail_enable == 1) ? "Yes" : "No");

        }
        cli_out("Flow learn notification callback registered : %s \r\n", (ft_learn_user_cb[unit]) ? "Yes" : "No");
    }
