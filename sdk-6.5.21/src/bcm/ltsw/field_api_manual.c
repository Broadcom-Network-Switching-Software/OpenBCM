...
{
    int i = 0, iter = 0, shift_val = 0;
    int off_idx = 0;
    int map_count = 0;
    bool is_presel_grp = false, group_map = false;
    ...
    for (iter = 0; iter < lt_map_ref->num_maps; iter++) {
        for (off_idx = 0; off_idx < (lt_map_ref->map[iter]).num_offsets; off_idx++) {
            qual_info_values.macro_flags[i] = (lt_map_ref->map[iter]).macro_value[off_idx];
    ...
    qual_info_values.num = i;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_field_qualifier_macro_validate(unit,
                                                  entry,
                                                  qual_info,
                                                  &qual_info_values));
        ...
        void *param0, void *param1)
{
    ...
    switch (qual) {
        case bcmFieldQualifyRangeCheck:
            ...
            break;
        case bcmFieldQualifyRangeCheckGroup:
            range_group_config.range_group_id = *(uint8 *)param0;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm_ltsw_range_group_get(unit, &range_group_config));
            break;
        case bcmFieldQualifyLoopBackPacketProcessingPort:
        case bcmFieldQualifyLoopBackSrcGport:
            if (0 == BCM_GPORT_IS_MODPORT(*(bcm_gport_t *)param0)) {
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
            }

            if (*(int *)param1 != BCM_FIELD_EXACT_MATCH_MASK) {
                if (0 == BCM_GPORT_IS_MODPORT(*(int *)param1)) {
                    SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                }
            }
            break;
...
static int
bcmint_ltsw_field_qualify_SrcTrunk_get(
    int unit,
    bcm_field_entry_t entry,
    ...
    bcm_trunk_t *mask)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_field_qualify_SrcTrunk_get(unit, entry, qual, is_trunk,
                                              data, mask));
exit:
    FP_UNLOCK(unit);
    SHR_FUNC_EXIT();
...
static int
bcmint_ltsw_field_qualify_SrcTrunk(
    int unit,
    bcm_field_entry_t entry,
    bcm_field_qualify_t qual,
    bcm_trunk_t data,
    bcm_trunk_t mask)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_field_qualify_SrcTrunk(unit, entry, qual,
                                              data, mask));
exit:
    FP_UNLOCK(unit);
    SHR_FUNC_EXIT();
...
{
    bcm_module_t data_modid = 0, mask_modid = 0;
    bcm_port_t data_port = 0, mask_port = 0;
    bcmi_ltsw_gport_info_t gport_info;

    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(port, SHR_E_PARAM);
    SHR_NULL_CHECK(port_mask, SHR_E_PARAM);
    ...
    gport_info.port = data_port;
    gport_info.modid = data_modid;
    gport_info.gport_type = _SHR_GPORT_TYPE_MODPORT;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_port_gport_construct(unit, &gport_info, port));
    gport_info.port = mask_port;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_port_gport_construct(unit, &gport_info, port_mask));
exit:
    SHR_FUNC_EXIT()
}
        ...
        bcm_gport_t port_mask)
{
    ...
    data_modid = BCM_GPORT_MODPORT_MODID_GET(port);
    data_port = BCM_GPORT_MODPORT_PORT_GET(port);
    mask_modid = BCM_FIELD_EXACT_MATCH_MASK;

    if (port_mask != BCM_FIELD_EXACT_MATCH_MASK) {
        mask_port = BCM_GPORT_MODPORT_PORT_GET(port);
    } else {
        ...
        int invert)
{
    ...
    *mask |= (1ULL << range_id);
    *data = invert ? (data_l &= (~(1ULL << range_id))) : (data_l |=  (1ULL << range_id));
        ...
        int max_count, bcm_field_range_t* range, int* invert, int* count)
{
    ...
    for (idx = 0; (idx < (sizeof(data_l) * 8)) ; idx++) {
        if (mask_l & (1ULL << idx)) {
            if (ct < max_count) {
                range[ct] = idx;
                invert[ct] = (!(data_l & (1ULL << idx)));
            }
            /* Give the actual count present on the device. */
            ct++;
        }
    }
...
{
    bcm_qual_field_t qual_info;
    uint16 data_local_val = 0, mask_local_val = 0;

    SHR_FUNC_ENTER(unit);
    FP_LOCK(unit);
    if ((data > 0xf) || (mask > 0xf)){
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }
    sal_memset(&qual_info, 0, sizeof(bcm_qual_field_t));
    ...
    data_local_val = 1 << data;
    mask_local_val = 0xffff;
    if (mask != 0xf) {
        mask_local_val = 1 << mask;
    } else {
        mask_local_val = 0xffff;
    }
    qual_info.qual_data = (void *) &data_local_val;
...
bcm_ltsw_field_qualify_RangeCheckGroup_get(int unit, bcm_field_entry_t entry, uint8 *data, uint8 *mask)
{
    bcm_qual_field_t qual_info;
    uint16 data_local_val = 0, mask_local_val = 0;
    uint16 *tmp_data_val = NULL, *tmp_mask_val = NULL;
    ...
    *data = 0;
    if (data_local_val) {
        while ((data_local_val & 0x1) == 0) {
            *data = *data + 1;
            data_local_val = data_local_val >> 1;
        }
    }
    *mask = 0;
    if (mask_local_val != 0xffff) {
        if (mask_local_val) {
            while ((mask_local_val & 0x1) == 0) {
                *mask = *mask + 1;
                mask_local_val = mask_local_val >> 1;
            }
        }
    } else {
        *mask = 0xf;
                                ...
                                bcm_gport_t src_port)
{
    ...
    bcmint_field_stage_info_t *stage_info = NULL;
    bcmi_field_ha_entry_oper_t *entry_oper = NULL;
    bcm_trunk_t data_trunk = 0, mask_trunk = 0;
    ...
    if (BCM_GPORT_IS_TRUNK(src_port)) {
        data_trunk = BCM_GPORT_TRUNK_GET(src_port);
        mask_trunk = BCM_FIELD_EXACT_MATCH_MASK;

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_field_qualify_SrcTrunk(unit, entry,
                                                bcmFieldQualifySrcTrunk,
                                                data_trunk, mask_trunk));
        entry_oper->entry_flags |= BCMINT_FIELD_ENTRY_GPORT_TRUNK;
        ...
        } else if (BCM_GPORT_IS_FLOW_PORT(src_port)) {
                                    ...
                                    bcm_gport_t *src_port)
{
    ...
    if (entry_oper->entry_flags & BCMINT_FIELD_ENTRY_GPORT_TRUNK) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_field_qualify_SrcTrunk_get(unit, entry, bcmFieldQualifySrcTrunk,
                                                         NULL, &data_trunk, &mask_trunk));
        gport_info.tgid = data_trunk;
                                    ...
                                    bcm_gport_t *dst_port)
{
    ...
    if (entry_oper->entry_flags & BCMINT_FIELD_ENTRY_GPORT_MODPORT) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_l2_if_to_port(unit, gport_info.port, dst_port));
        if (!BCM_GPORT_IS_SET(*dst_port)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm_ltsw_port_gport_get(unit, *dst_port, dst_port));
        }
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
                                ...
                                bcm_gport_t *dst_port_mask)
{
    ...
    if (entry_oper->entry_flags & BCMINT_FIELD_ENTRY_GPORT_MODPORT) {
        ...
        if (mask_val != -1) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_id_l2_if_to_port(unit, gport_mask_info.port,
                                         dst_port_mask));
            if (!BCM_GPORT_IS_SET(*dst_port_mask)) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcm_ltsw_port_gport_get(unit, *dst_port_mask,
                                                    dst_port_mask));
            }
        }
    } else {
...
int
bcm_ltsw_field_qualify_DropReasonIngressCopy(int unit,
                                             bcm_field_entry_t entry,
                                             bcm_field_drop_reason_t type)
{
    bcm_qual_field_t qual_info;
    ...
    qual_info.sdk6_qual_enum = bcmFieldQualifyDropReasonIngressCopy;
    qual_info.flags = BCM_FIELD_F_ENUM;
    qual_info.depth = 1;
    qual_info.data_type = BCM_QUAL_DATA_TYPE_UINT32;
    qual_info.qual_data = (void *) &type;
    ...
    SHR_IF_ERR_VERBOSE_EXIT(
            mbcm_ltsw_field_manual_enum_to_lt_enum_value_get(unit,
                entry,
                bcmFieldQualifyDropReasonIngressCopy,
                (uint32)type,
                &qual_info));
...
int
bcm_ltsw_field_qualify_DropReasonIngressCopy_get(int unit,
                                                 bcm_field_entry_t entry,
                                                 bcm_field_drop_reason_t *type)
{
    bcm_qual_field_t qual_info;
...
int
bcm_ltsw_field_qualify_FirstDropReasonIngressCopy(int unit,
                                                  bcm_field_entry_t entry,
                                                  bcm_field_drop_reason_t type)
{
    bcm_qual_field_t qual_info;
    ...
    qual_info.sdk6_qual_enum = bcmFieldQualifyFirstDropReasonIngressCopy;
    qual_info.flags = BCM_FIELD_F_ENUM;
    qual_info.depth = 1;
    qual_info.data_type = BCM_QUAL_DATA_TYPE_UINT32;
    qual_info.qual_data = (void *) &type;
    ...
    SHR_IF_ERR_VERBOSE_EXIT(
            mbcm_ltsw_field_manual_enum_to_lt_enum_value_get(unit,
                entry,
                bcmFieldQualifyFirstDropReasonIngressCopy,
                (uint32)type,
                &qual_info));
...
int
bcm_ltsw_field_qualify_FirstDropReasonIngressCopy_get(int unit,
                                                      bcm_field_entry_t entry,
                                                      bcm_field_drop_reason_t *type)
{
    bcm_qual_field_t qual_info;
...
int
bcm_ltsw_field_qualify_OverlayOuterTpid(int unit,
                                        bcm_field_entry_t entry,
                                        uint16 tpid)
{
    uint16 tpid_mask = 0;
    ...
    qual_info.sdk6_qual_enum = bcmFieldQualifyOverlayOuterTpid;
    qual_info.flags = 0;
...
int
bcm_ltsw_field_qualify_OverlayOuterTpid_get(int unit,
                                            bcm_field_entry_t entry,
                                            uint16 *tpid)
{
    uint16 tpid_mask = 0;
    ...
    uint32 mask)
{
    ...
    qual_info.sdk6_qual_enum = bcmFieldQualifyExactMatchActionClassId;
    qual_info.depth = 1;
    qual_info.data_type = BCM_QUAL_DATA_TYPE_UINT16;
    qual_info.flags = 0;
    qual_info.qual_data = (void *) &data;
    ...
    uint8 mask)
{
    ...
    qual_info.sdk6_qual_enum = bcmFieldQualifyExactMatchHitStatus;
    qual_info.depth = 1;
    qual_info.data_type = BCM_QUAL_DATA_TYPE_UINT8;
    qual_info.flags = 0;
    qual_info.qual_data = (void *) &data;
    ...
    bcm_field_group_t group)
{
    ...
    qual_info.sdk6_qual_enum = bcmFieldQualifyExactMatchGroupClassId;
    qual_info.depth = 1;
    qual_info.data_type = BCM_QUAL_DATA_TYPE_UINT16;
    qual_info.flags = 0;

    SHR_IF_ERR_VERBOSE_EXIT(
        bcm_int_field_group_to_ltid_get(unit, group, &data));
    qual_info.qual_data = (void *) &data;
    qual_info.qual_mask = (void *) &mask;
    ...
    bcm_field_group_t *group)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT(
            bcmi_ltsw_field_qualifier_get(unit, entry, &qual_info));
    SHR_IF_ERR_VERBOSE_EXIT(
        bcm_int_field_ltid_to_group_get(unit, *data, group));
...
bcm_ltsw_field_qualify_SrcTrunk(int unit, bcm_field_entry_t entry, bcm_trunk_t data, bcm_trunk_t mask)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_field_qualify_SrcTrunk(unit, entry, bcmFieldQualifySrcTrunk,
                                                data, mask));
exit:
    FP_UNLOCK(unit);
    SHR_FUNC_EXIT();
...
bcm_ltsw_field_qualify_SrcTrunk_get(int unit, bcm_field_entry_t entry, bcm_trunk_t *data, bcm_trunk_t *mask)
{
    ...
    if (false == is_trunk) {
        gport_info.port = *data;
        gport_info.gport_type = _SHR_GPORT_TYPE_MODPORT;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_port_gport_construct(unit, &gport_info, data));
        gport_info.port = *mask;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_port_gport_construct(unit, &gport_info, mask));
    }
...
int
bcm_ltsw_field_qualify_LoopBackPacketProcessingPort(
        int unit,
        bcm_field_entry_t entry,
        bcm_gport_t port_id,
        bcm_gport_t port_mask)
...
int
bcm_ltsw_field_qualify_LoopBackPacketProcessingPort_get(
        int unit,
        bcm_field_entry_t entry,
        bcm_gport_t *port_id,
        bcm_gport_t *port_mask)
{
    ...
    gport_info.port = *port_id;
    gport_info.gport_type = _SHR_GPORT_TYPE_MODPORT;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_port_gport_construct(unit, &gport_info, port_id));

    gport_info.port = *port_mask;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_port_gport_construct(unit, &gport_info, port_mask));
exit:
    FP_UNLOCK(unit);
...
int
bcm_ltsw_field_qualify_LoopBackSrcGport(
        int unit,
        bcm_field_entry_t entry,
        bcm_gport_t port_id,
        bcm_gport_t port_mask)
...
int
bcm_ltsw_field_qualify_LoopBackSrcGport_get(
        int unit,
        bcm_field_entry_t entry,
        bcm_gport_t *port_id,
        bcm_gport_t *port_mask)
...
int
bcm_ltsw_field_qualify_StpState(int unit,
                                bcm_field_entry_t entry,
                                uint8 stpstate)
{
    uint8 stpstate_mask = 0;
    bcm_qual_field_t qual_info;
    ...
    qual_info.sdk6_qual_enum = bcmFieldQualifyStpState;
    qual_info.flags = BCM_FIELD_F_ENUM;

    qual_info.qual_data = (void *) &stpstate;
    qual_info.qual_mask = (void *) &stpstate_mask;
...
int
bcm_ltsw_field_qualify_StpState_get(int unit,
                                    bcm_field_entry_t entry,
                                    uint8 *stpstate)
{
    uint8 stpstate_mask = 0;
    bcm_qual_field_t qual_info;
    uint8 *stpstate_mask_get = &stpstate_mask;
    ...
    qual_info.qual_data = (void *) &stpstate;
    qual_info.qual_mask = (void *) &stpstate_mask_get;
