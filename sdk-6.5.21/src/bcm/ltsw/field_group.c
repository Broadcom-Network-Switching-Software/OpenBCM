...
static int
bcmint_field_group_policer_pool_bmp_get(int unit,
                                        bcm_field_group_t group,
                                        bcm_field_hintid_t hintid,
                                        uint32_t *pool_bmp)
{
    ...
    for (i = bcmPolicerPool0; i < bcmPolicerPoolCount; i++) {
        hint.value = i;
        rv = bcmint_field_hints_get(unit, hintid, &hint);
        if (SHR_SUCCESS(rv)) {
            SHR_BITSET(pool_bmp, i);
        }
    }
    ...
    if (map->width[0] == 0) {
        ...
        if ((fld_defs->dtag == BCMLT_FIELD_DATA_TAG_BOOL)
            ...
            for (idx = 0; idx < fld_defs->elements; idx++) {
                offset = (idx * 64);
                dst_ptr = (qual_value + (offset / 64));
                mask = (mask64 >> (64 - width));
                if (qual_hint_extracted) {
                    src_ptr = (qual_value_inp_buf + (*src_ofst_current / 64));
                    *dst_ptr |= (((*src_ptr >> (*src_ofst_current % 64)) & mask) << (offset % 64));
                } else {
            ...
            if (width > 64) {
                ...
                offset = (*src_ofst_current + 64);
                if (qual_hint_extracted) {
                    src_ptr = (qual_value_inp_buf + (offset / 64));
                    dst_ptr[1] |= ((*src_ptr >> (offset % 64)) & mask);
                } else {
                    dst_ptr[1] |= (mask);
                }
                width -= part_width;
            ...
            if (qual_hint_extracted) {
                src_ptr = (qual_value_inp_buf + (*src_ofst_current / 64));
                dst_ptr[0] |= ((*src_ptr >> (*src_ofst_current % 64)) & mask);
            } else {
                dst_ptr[0] |= (mask);
            }
            *src_ofst_current += (width + part_width);
        }
    } else {
        ...
        for (idx = 0; idx < map->num_offsets; idx++) {
            ...
            if ((offset < 64) && ((offset + width) > 64)) {
                ...
                if (qual_hint_extracted) {
                    src_ptr = (qual_value_inp_buf + (*src_ofst_current / 64));
                    dst_ptr[0] |= (((*src_ptr >> (*src_ofst_current % 64)) & mask) << offset);
                } else {
                                ...
                                bcmlt_entry_handle_t *group_template)
{
    ...
    uint16_t qual_pbmp_inp_buf_size = 0;
    uint8_t *qual_pbmp_inp_buf = NULL;
    ...
    for (qual_idx = 0; qual_idx < BCM_FIELD_QUALIFY_MAX; qual_idx++) {
        ...
        if (SHR_BITGET(qset->w, qual_idx)) {
            ...
            if ((hintid != 0) && (qual == bcmFieldQualifyInPorts)) {
                ...
                if (pbmp_hint_extracted == true) {
                    /* InPorts hint supported only in PIPEUNIQUE mode. */
                    if (pipe == -1) {
                        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                    }
                    qual_pbmp_inp_buf_size = sizeof(bcm_pbmp_t);
                    BCMINT_FIELD_MEM_ALLOC
                        (qual_pbmp_inp_buf, qual_pbmp_inp_buf_size, "ip buf");
                    SHR_IF_ERR_VERBOSE_EXIT
                        (mbcm_ltsw_field_pbmp_mask_get(unit,
                                                       pipe,
                                                       stage_info,
                                                       &hint_pbmp,
                                                       qual_pbmp_inp_buf));
            ...
            for (iter = 0; iter < lt_map_ref->num_maps; iter++) {
                ...
                /*
                 * Skip setting LT fields corresponding to different pipes.
                 */
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (mbcm_ltsw_field_lt_qual_name_update(unit, 1, 0,
                ...
                for(ofst_idx = 0;
                    ...
                    if ((lt_map_ref->map[iter]).macro_value[ofst_idx] != 0) {
                        macro_qual = true;
                    }
                    lt_map_field_width +=
                                    (lt_map_ref->map[iter]).width[ofst_idx];
                ...
                if (set_qual_bitmap) {
                    if (hintid != 0) {
                        if (pbmp_hint_extracted == true) {
                            SHR_BITCOPY_RANGE((uint32 *)qual_value_inp_buf,
                                              0,
                                              (uint32 *)qual_pbmp_inp_buf,
                                              map_src_offset,
                                              lt_map_field_width);
                            qual_hint_extracted = true;
                        } else if (macro_qual == true) {
                            for(ofst_idx = 0;
                                ofst_idx < (lt_map_ref->map[iter]).num_offsets;
                                ofst_idx++) {
                                SHR_IF_ERR_VERBOSE_EXIT(
                                    bcmint_field_hints_group_qual_macro_get
                                    (unit, hintid, &qual_hint_extracted,
                                     qual_idx,
                                     (lt_map_ref->map[iter]).macro_value[ofst_idx],
                                     (src_ofset_current + ofst_idx),
                                     qual_value_inp_buf));
                            }
                        } else {
                            SHR_IF_ERR_VERBOSE_EXIT(
            ...
            BCMINT_FIELD_MEM_FREE(qual_pbmp_inp_buf);
        }
    }
                                ...
                                int *color_pdd_idx)
{
    ...
    if ((stage_info->flags & BCMINT_FIELD_STAGE_ENTRY_COLOR_STAT_SUPPORT) &&
        ...
        (BCM_FIELD_ASET_TEST(*aset, bcmFieldActionStatGroup)) &&
        !(BCM_FIELD_ASET_TEST(*aset, bcmFieldActionGpStatGroup)) &&
        !(BCM_FIELD_ASET_TEST(*aset, bcmFieldActionYpStatGroup)) &&
        !(BCM_FIELD_ASET_TEST(*aset, bcmFieldActionRpStatGroup))) {
        BCM_FIELD_ASET_ADD(valid_aset, bcmiFieldActionGpStatGroup);
        BCM_FIELD_ASET_ADD(valid_aset, bcmiFieldActionYpStatGroup);
        BCM_FIELD_ASET_ADD(valid_aset, bcmiFieldActionRpStatGroup);
    }

    /* Loop trough all actions. */
    for (idx = 0; idx < bcmiFieldActionCount; idx++) {
        if (SHR_BITGET(valid_aset.w, idx)) {
            ...
            for (iter = 0; iter < lt_map->num_maps; iter++) {
                ...
                if ((lt_map->map[iter]).policer == TRUE) {
                    ...
                    if ((lt_map->map[iter]).policer_pdd == TRUE) {
                        if (((idx == bcmFieldActionGpStatGroup) ||
                             (idx == bcmFieldActionYpStatGroup) ||
                             (idx == bcmFieldActionRpStatGroup)) &&
                            (hintid > 0)) {
                            ...
                            SHR_IF_ERR_VERBOSE_EXIT
                                (mbcm_ltsw_field_action_stat_group_lt_field_validate(
                                        ...
                                        (lt_map->map[iter]).lt_pdd_field_name,
                                        &is_valid));
                            if (!is_valid) {
                                continue;
                            }
                        ...
                        if ((lt_map->map[iter]).num_internal_maps > 0) {
                            for (int_iter = 0;
                                ...
                                (bcmlt_entry_field_add(
                                        color_pdd_template,
                                        ((lt_map->map[iter]).internal_map[int_iter]).lt_pdd_field_name,
                                        pdd_value));
                            }
                        }
    ...
    if ((stage_info->flags & BCMINT_FIELD_STAGE_COLOR_INDEPENDENT) &&
            (!(stage_info->flags & BCMINT_FIELD_STAGE_ACTION_RESOLUTION_SBR)) &&
            (stage_info->flags & BCMINT_FIELD_STAGE_DEFAULT_ENTRY_SUPPORT)) {
        if (stage_info->tbls_info->action_color_independent_fid == NULL) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
        }
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(*pdd_template,
                                   stage_info->tbls_info->action_color_independent_fid,
                                   1));
    }
    ...
    bcmi_field_ha_group_oper_t **group_oper)
{
    ...
    if (group_config->flags & BCM_FIELD_GROUP_ENTRY_AVOID_HW_INSTALL) {
        oper->group_flags |= BCMINT_FIELD_GROUP_AVOID_HW_INSTALL;
    }

    if (group_config->flags & BCM_FIELD_GROUP_CREATE_SHARED_POLICER_POOL) {
        oper->group_flags |= BCMINT_FIELD_GROUP_SHARED_POLICER_POOL;
    }

    if (group_config->flags & BCM_FIELD_GROUP_CREATE_ENT_LEVEL_POLICER_POOL) {
        oper->group_flags |= BCMINT_FIELD_GROUP_ENT_LEVEL_POLICER_POOL;
    }
    *group_oper = oper;
exit:
    SHR_FUNC_EXIT();
...
int
bcmint_field_hint_flexctr_action_update(
        int unit,
        bcmint_field_stage_info_t *stage_info,
        bcmlt_entry_handle_t grp_template,
        bcmint_field_tbls_info_t *tbls_info,
        bcm_field_group_config_t *group_config,
        uint8_t max_arr_size,
        uint32_t *flexctr_action_arr,
        uint8_t *valid_cnt,
        int *group_flags)
{
    uint32_t count = 0;
    uint64_t hint_values[BCMI_FIELD_HA_HINT_ID_MAX] = {0};
    uint32_t index = 0;
    ...
    bcm_field_hintid_t hint_id = group_config->hintid;
    ...
    index = (hint_id & BCMINT_FP_HASH_INDEX_MASK);
    ...
    *valid_cnt = 0;
    if ((f_ht != NULL) && (f_ht->hintid != 0)) {
        ...
        while (hint_entry != NULL) {
            if (bcmFieldHintTypeGroupExpandFlexCtrAction == hint_entry->hint_type) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_flexctr_counter_id_info_get(unit,
                                                           hint_entry->value,
                                                           &ctr_info));
                if (bcmFlexctrHintSharedBlock != ctr_info.act_cfg.hint_type) {
                    SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                }
                if (bcmFlexctrObjectStaticEgrFieldStageEgress ==
                                ctr_info.act_cfg.index_operation.object[0]) {
                    index = ctr_info.act_cfg.index_operation.object_id[0];
                    if (index > stage_info->num_slices) {
                        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                    }
                    ...
                    hint_values[index] = (uint64_t)ctr_info.action_index;
                    flexctr_action_arr[index] = ctr_info.action_index;
                    count++;
                }
                if (bcmFlexctrObjectStaticEgrFieldStageEgress ==
                                ctr_info.act_cfg.index_operation.object[1]) {
                    index = ctr_info.act_cfg.index_operation.object_id[1];
                    if (index > stage_info->num_slices) {
                        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
                    }
    ...
    if (count) {
        ...
        SHR_NULL_CHECK(tbls_info->flexctr_action_count_fid, SHR_E_UNAVAIL);
        SHR_NULL_CHECK(tbls_info->flexctr_action_fid, SHR_E_UNAVAIL);

        if (group_config->flags & BCM_FIELD_GROUP_CREATE_AUTO_EXPANSION_DISABLE) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_CONFIG);
        }

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(grp_template,
                              tbls_info->flexctr_action_count_fid,
                              count));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_array_add(grp_template,
                                   tbls_info->flexctr_action_fid,
                                   0, hint_values, count));
        *valid_cnt = count;
        *group_flags = BCMINT_FIELD_GROUP_FLEXCTR_VMAP_ENABLED;
    }
...
STATIC int
bcmint_field_presel_traverse_in_groups(int unit,
        bcmint_field_stage_info_t *stage_info,
        int presel_id,
        bool *used)
{
    ...
    SHR_NULL_CHECK(stage_info, SHR_E_INTERNAL);
    SHR_NULL_CHECK(used, SHR_E_INTERNAL);
    ...
    for (gid = 1;
        ...
        if (SHR_BITGET(stage_info->group_info->grpid_bmp.w, gid)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_group_oper_lookup(unit, gid,
                                                stage_info,
                                                &oper));
            for (index = 0;
                    (index < BCMI_FIELD_HA_PRESEL_MAX) &&
                       (presel_id != oper->presel_arr[index]); index++) {
                /* Do Nothing. If presel ID is found, loop will end. */
            }
            if (BCMI_FIELD_HA_PRESEL_MAX != index) {
                *used = true;
                break;
            }
        }
...
STATIC int
bcmint_presel_conflict_set_create(int unit,
                                  bcmint_field_stage_info_t *stage_info,
                                  ...
                                  uint32_t *conflict_id)
{
    ...
    int dunit = 0;
    int map_count = 0;
    bool conflict_id_found = false;
    uint32_t hint_val = 0;
    uint64_t conflict_id_val = 0;
    uint64_t conflict_val = 0;
    uint64_t *conflict_val_arr = NULL;
    bcmlt_field_def_t fld_def;
    bcm_field_qual_map_t *lt_map_ref = NULL;
    bcmlt_entry_handle_t conflict_template = BCMLT_INVALID_HDL;

    SHR_FUNC_ENTER(unit);

    if (stage_info->tbls_info->conflict_sid == NULL) {
    ...
    SHR_IF_ERR_VERBOSE_EXIT
       (bcmlt_entry_allocate(dunit,
                             stage_info->tbls_info->conflict_sid,
                             &conflict_template));
    for (idx = 0; idx < presel_oper->qual_cnt; idx++) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_hints_group_conflict_val_get(
                    unit,
                    hint_id,
                    presel_oper->qset_arr[idx],
                    &hint_val));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_qual_map_find(
                    unit,
                    presel_oper->qset_arr[idx],
                    stage_info, 0, 1,
                    &map_count,
        ...
        for (iter = 0; iter < lt_map_ref->num_maps; iter++) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_lt_field_def_get(unit,
                                       ...
                                       &fld_def));
            if (fld_def.depth > 1) {
                BCMINT_FIELD_MEM_ALLOC(conflict_val_arr,
                                       (sizeof(uint64_t) * fld_def.depth),
                                       "Conflict id array");
                for (i = 0; i < fld_def.depth; i++) {
                    conflict_val_arr[i] = hint_val;
                }
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_array_add(conflict_template,
                                                 (lt_map_ref->map[iter]).lt_field_name,
                                                 0,
                                                 conflict_val_arr,
                                                 fld_def.depth));
                BCMINT_FIELD_MEM_FREE(conflict_val_arr);
            } else {
                conflict_val = hint_val;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(conflict_template,
                                           (lt_map_ref->map[iter]).lt_field_name,
                                           conflict_val));
            }
        }
    ...
    /* Allocate a conflict ID. */
    conflict_id_val = (stage_info->presel_info->last_allocated_conflict_id + 1);
    for (iter = 1; iter < stage_info->presel_info->conflict_id_max; iter++) {
        if (conflict_id_val > stage_info->presel_info->conflict_id_max) {
            conflict_id_val = 1;
        }

        if (!SHR_BITGET(stage_info->presel_info->conflict_id_bmp.w, conflict_id_val)) {
            conflict_id_found = TRUE;
            break;
        }
        conflict_id_val++;
    }

    if ((conflict_id_found == FALSE) &&
        (idx > stage_info->presel_info->conflict_id_max)) {
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "Error: All coonflict IDs are in use\n")));
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_RESOURCE);
    }

    stage_info->presel_info->last_allocated_conflict_id = conflict_id_val;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(conflict_template,
                               stage_info->tbls_info->conflict_key_fid,
                               conflict_id_val));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_commit(unit, conflict_template,
                              BCMLT_OPCODE_INSERT,
                              BCMLT_PRIORITY_NORMAL));

    SHR_BITSET(stage_info->presel_info->conflict_id_bmp.w, conflict_id_val);
    *conflict_id = conflict_id_val;

exit:
    if (BCMLT_INVALID_HDL != conflict_template) {
        (void) bcmlt_entry_free(conflict_template);
    }
    BCMINT_FIELD_MEM_FREE(conflict_val_arr);
    SHR_FUNC_EXIT();
}
...
STATIC int
bcmint_presel_conflict_set_destroy(int unit,
                                   bcmint_field_stage_info_t *stage_info,
                                   uint16_t conflict_count,
                                   uint16_t *conflict_id)
{
    int idx = 0;
    int dunit = 0;
    uint64_t conflict_val = 0;
    ...
    dunit = bcmi_ltsw_dev_dunit(unit);
    for (idx = 0; idx < conflict_count; idx++) {
        conflict_val = conflict_id[idx];
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_allocate(dunit,
                                  stage_info->tbls_info->conflict_sid,
                                  &conflict_template));
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_entry_commit(unit, conflict_template,
                                  ...
                                  BCMLT_PRIORITY_NORMAL));
        (void) bcmlt_entry_free(conflict_template);
        SHR_BITCLR(stage_info->presel_info->conflict_id_bmp.w, conflict_id[idx]);
    }
                                 ...
                                 uint32 flags)
{
    ...
    uint32_t pool_bmp = 0;
    bool pool_sharing = false;
    bool ent_lvl_policer = false;
    uint64_t presel_id[32] = {0};
    uint64_t val64_arr[32] = {0};
    uint16_t conflict_arr[32] = {0};
    ...
    uint32_t flexctr_action_arr[BCMI_FIELD_HA_VSLICE_MAX] = {0};
    ...
    if ((group_config->flags & BCM_FIELD_GROUP_ENTRY_AVOID_HW_INSTALL) &&
        (tbls_info->sw_flowtracker_fid == 0)) {
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(unit, "Error: This stage does not support"
                              " BCM_FIELD_GROUP_ENTRY_AVOID_HW_INSTALL flag.\n")));
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }
        ...
        if (stage_info->flags & BCMINT_FIELD_STAGE_POLICER_POOL_HINT_SUPPORT) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_group_policer_pool_get(unit,
                                                     ...
                                                     &policer_pool_id));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_group_policer_pool_bmp_get(unit,
                                                         group_id,
                                                         group_config->hintid,
                                                         &pool_bmp));

            if ((pool_bmp != 0) && (policer_pool_id == -1)) {
                /*
                 * bcmFieldHintTypePolicerPoolExpand for pool expansion should
                 * be specified only if bcmFieldHintTypePolicerPoolSel is specified
                 * for primary pool id.
                 */
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
            }
        }
        rv = bcmint_field_group_policer_pool_alloc(unit,
                                                   stage_info,
                                                   pool_sharing,
                                                   grp_pipe,
                                                   group_config->priority,
        ...
        for (i = 1; i < BCM_FIELD_PRESEL_SEL_MAX; i++) {
            if (SHR_BITGET(group_config->preselset.w, i)) {
                ...
                if (SHR_BITGET(
                    ...
                    /*
                     * For Single Lookup stages,
                     * same presel cant be used across groups.
                     */
                    if (stage_info->flags & BCMINT_FIELD_STAGE_SINGLE_LOOKUP_ONLY) {
                        presel_found = false;
                        SHR_IF_ERR_VERBOSE_EXIT
                            (bcmint_field_presel_traverse_in_groups(unit,
                                                                   stage_info,
                                                                   i,
                                                                   &presel_found));
                        if (presel_found) {
                    ...
                    if ((group_config->flags &
                        ...
                        if (presel_oper->qual_cnt != 0) {
                            SHR_IF_ERR_VERBOSE_EXIT
                                (bcmint_presel_conflict_set_create(unit,
                                                                   stage_info,
                                                                   group_config->hintid,
                                                                   presel_oper,
                                                                   &conflict_id));
                            conflict_arr[conflict_count] = conflict_id;
                            val64_arr[conflict_count] = conflict_id;
                            conflict_count++;
                        }
                    }
        ...
        if (presel_count > 0) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(grp_template,
                                       ...
                                       presel_count));
            if (conflict_count != 0) {
                if (conflict_count != presel_count) {
                    LOG_DEBUG(BSL_LS_BCM_FP,
                              (BSL_META_U(unit,
                                          "Error: Not all presels have"
                                          " conflict qsets configured.%d \n"), i));
                    SHR_IF_ERR_VERBOSE_EXIT(SHR_E_CONFIG);
                }
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_array_add(grp_template,
                                                 tbls_info->conflict_key_fid,
                                                 0, val64_arr, conflict_count));
            }
        }
    }
    ...
    if ((stage_info->flags & BCMINT_FIELD_STAGE_KEY_TYPE_FIXED) &&
        ...
        uint8_t valid_action_cnt = 0;

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_field_hint_flexctr_action_update(unit,
                                                     stage_info,
                                                     grp_template,
                                                     tbls_info,
                                                     group_config,
    ...
    if ((reserved_group == FALSE) &&
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_field_vslice_group_alloc(
                  unit, stage_info,
                  grp_pipe,
                  group_config->priority,
    ...
    if (conflict_count) {
        for (index = 0; index < conflict_count; index++) {
            group_oper->conflict_arr[index] = conflict_arr[index];
        }
        group_oper->conflict_cnt = conflict_count;
    }
    if ((stage_info->tbls_info->virtual_slice_grp_fid) &&
        (group_config->action_res_id == BCM_FIELD_GROUP_ACTION_RES_ID_DEFAULT)) {
        group_oper->group_flags |= BCMINT_FIELD_GROUP_DEFAULT_VSLICE_GRP_SET;
    }
    ...
    group_oper->policer_pool_bmp = pool_bmp;
    ...
    if (!(flags & BCMINT_FIELD_GROUP_CREATE_WITH_RESERVE_ID)) {
        ...
        /* Update flexctr group flags */
        if ((clear == FALSE) &&
            (stage_info->flags & BCMINT_FIELD_STAGE_KEY_TYPE_FIXED)) {
            if (flexctr_grp_flags) {
                group_oper->group_flags |= flexctr_grp_flags;
                sal_memcpy(group_oper->ctr_action_vmap_arr,
                        flexctr_action_arr,
                        sizeof(uint32_t) * BCMI_FIELD_HA_VSLICE_MAX);
            }

            SHR_IF_ERR_VERBOSE_EXIT
                (mbcm_ltsw_field_flexctr_action_update(unit,
                                                       stage_info, grp_pipe));
        }
    }
...
exit:
    if (SHR_FUNC_ERR()) {
        if (group_created == TRUE) {
            ...
            if (conflict_count != 0) {
                SHR_IF_ERR_VERBOSE_EXIT(
                    bcmint_presel_conflict_set_destroy(
                        unit,
                        stage_info,
                        conflict_count,
                        conflict_arr));
            ...
            if (stage_info && (stage_info->flags & BCMINT_FIELD_STAGE_POLICY_TYPE_PDD)
                    && (policer_pool_id != -1)) {
                (void)(bcmint_field_group_policer_pool_free(
                    unit, stage_info, grp_pipe,
                    policer_pool_id, pool_bmp));
            }

            if (stage_info && (stage_info->tbls_info->virtual_slice_grp_fid) &&
                (action_res_id != -1) &&
                (group_config->action_res_id ==
                 BCM_FIELD_GROUP_ACTION_RES_ID_DEFAULT)) {
                (void)(mbcm_ltsw_field_vslice_group_dealloc(
                            unit, stage_info,
                            grp_pipe,
                            group_config->priority));
            }
...
/*
 * This function will retreive qset->udf_map
 * from provided qualifiers set in qset->w
 */
int
bcmint_field_group_udf_map_get(int unit,
                               bcm_field_qset_t *qset)
{
    int c = 0;
    ...
    /* Set udf_map for current and new qset */
    for (c = 0; c < BCMI_FIELD_UDF_CHUNK_QUALIFIER_NUM; c++) {
        if (SHR_BITGET(qset->w, (c + bcmiFieldQualifyB4Chunk0))) {
            SHR_BITSET(qset->udf_map, c);
        }
        if (SHR_BITGET(qset->w, (c + bcmiFieldQualifyB2Chunk0))) {
            SHR_BITSET(qset->udf_map, (c + max_chunks));
        }
        if (SHR_BITGET(qset->w, (c + bcmiFieldQualifyB1Chunk0))) {
            SHR_BITSET(qset->udf_map, (c + (max_chunks * 2)));
        }
    }
                           ...
                           bcm_field_group_t group)
{
    ...
    if ((stage_info->tbls_info->virtual_slice_grp_fid) &&
        (group_oper->group_flags & BCMINT_FIELD_GROUP_DEFAULT_VSLICE_GRP_SET)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_field_vslice_group_dealloc(
                  unit, stage_info,
                  pipe,
                  group_oper->priority));
    }
    ...
    if (stage_info->flags & BCMINT_FIELD_STAGE_PRESEL_KEY_DYNAMIC) {
        ...
        if (group_oper->group_flags & BCMINT_FIELD_GROUP_CREATED_WITH_PRESEL) {
            ...
            if (group_oper->conflict_cnt != 0) {
                SHR_IF_ERR_VERBOSE_EXIT(
                    bcmint_presel_conflict_set_destroy(
                        unit,
                        stage_info,
                        group_oper->conflict_cnt,
                        group_oper->conflict_arr));
            }
        }
    }
 ...
 *
 * Remove all field entries from HW for group.
 *
 * \param [in] unit     - BCM device number.
 * \param [in] group_id - Group Identifier.
 *
    ...
    for (entry_id = 1;
        ...
        if (SHR_BITGET(stage_info->entry_info->eid_bmp.w, entry_id)) {
            ...
            if (entry_oper->group_id == group_id) {
                entry = stage_info->stage_base_id | entry_id;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmint_field_entry_enable_set(unit, entry, 2));
            }
        }
    }
                              ...
                              bcm_field_presel_set_t *presel)
{
    ...
    SHR_NULL_CHECK(presel, SHR_E_PARAM);
    dunit = bcmi_ltsw_dev_dunit(unit);
                              ...
                              bcm_field_presel_set_t *presel)
{
        ...
        if (group_oper->conflict_cnt != 0) {
            SHR_IF_ERR_VERBOSE_EXIT(
                bcmint_presel_conflict_set_destroy(
                    ...
                    group_oper->conflict_cnt,
                    group_oper->conflict_arr));
            for (i = 0; i < group_oper->conflict_cnt; i++) {
                group_oper->conflict_arr[i] = 0;
                conflict_arr[i] = 0;
            }
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_array_add(
    ...
    if (conflict_count) {
        for (i = 0; i < conflict_count; i++) {
            group_oper->conflict_arr[i] = conflict_arr[i];
        }
        group_oper->conflict_cnt = conflict_count;
    }
                            ...
                            bcm_field_qset_t *qset)
{
    ...
    /* Get udf_map for incoming set */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_group_udf_map_get(unit, qset));
                            ...
                            bcm_field_qset_t qset)
{
    ...
    /* Get udf_map for current and new qset */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_group_udf_map_get(unit, &curr_qset));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_group_udf_map_get(unit, &new_qset));

    /* Retreive pipe from group oper state */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_group_pipe_get(unit, stage_info, group_oper, &pipe));
    ...
    SHR_IF_ERR_VERBOSE_EXIT(bcmint_field_group_validate_qset_set(
        unit, group, stage_info, group_oper, pipe, &curr_qset, &new_qset));
                            ...
                            bcm_field_aset_t *aset)
{
    ...
    SHR_NULL_CHECK(aset, SHR_E_PARAM);
    dunit = bcmi_ltsw_dev_dunit(unit);
                            ...
                            bcm_field_aset_t aset)
{
    ...
    if ((stage_info->flags & BCMINT_FIELD_STAGE_POLICER_ACTION_SUPPORT) &&
        ...
        /* Group pipe info get */
        if (stage_info->flags & BCMINT_FIELD_STAGE_POLICER_POOL_HINT_SUPPORT) {
            if (group_oper->group_flags & BCMINT_FIELD_GROUP_SHARED_POLICER_POOL) {
                pool_sharing = true;
            }
            if (group_oper->group_flags & BCMINT_FIELD_GROUP_ENT_LEVEL_POLICER_POOL) {
                ent_lvl_policer = true;
            }
    ...
    if (stage_info->flags & BCMINT_FIELD_STAGE_POLICY_TYPE_PDD) {
        if (group_oper->group_flags & BCMINT_FIELD_GROUP_ENT_LVL_ACTION_RESOLVE) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_entry_multi_get(unit, group, 0, true,
                                              NULL, &entry_count));
            if (entry_count) {
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_BUSY);
            }
                                ...
                                int prio)
{
    ...
    if ((stage_info->tbls_info->virtual_slice_grp_fid) &&
        (group_oper->group_flags & BCMINT_FIELD_GROUP_DEFAULT_VSLICE_GRP_SET)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_field_vslice_group_dealloc(unit, stage_info,
                                                  pipe, curr_prio));
        action_res_id_set = true;
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_field_vslice_group_alloc(unit, stage_info, pipe,
                                                prio, -1, &action_res_id));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(group_handle,
                                   stage_info->tbls_info->virtual_slice_grp_fid,
                                   action_res_id));
    ...
    if ((stage_info->flags & BCMINT_FIELD_STAGE_FLEXSTAT_SLICE_INFO) &&
        (stage_info->flags & BCMINT_FIELD_STAGE_KEY_TYPE_FIXED)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_field_flexctr_action_update(unit, stage_info, pipe));
    } else if (stage_info->flags & BCMINT_FIELD_STAGE_FLEX_CTR_SUPPORT) {
                                 ...
                                 bcm_field_group_oper_mode_t *mode)
{
    ...
    } else if (oper_mode == bcmiFieldOperModeGlobalPipeAware) {
        *mode = bcmFieldGroupOperModeGlobalPipeAware;
    } else {
        ...
        uint32_t *slice_id)
{
    ...
    if (stage_info->tbls_info->group_info_virtual_slice_fid != NULL) {
        slice_fid = stage_info->tbls_info->group_info_virtual_slice_fid;
    } else {
        slice_fid = stage_info->tbls_info->group_info_slice_fid;
    }
    ...
    for (; base_ptr != NULL; base_ptr = base_ptr->next) {
        ...
        for (action = 0; action < bcmiFieldActionCount; action++) {
            ...
            if ((bcm_field_action_t)action == bcmFieldActionStatGroup ||
                (bcm_field_action_t)action == bcmFieldActionGpStatGroup ||
                (bcm_field_action_t)action == bcmFieldActionYpStatGroup ||
                (bcm_field_action_t)action == bcmFieldActionRpStatGroup) {
                continue;
            }
            ...
            for (iter = 0; iter < lt_map_ref->num_maps; iter++) {
                ...
                sbr_fid_name = (lt_map_ref->map[iter]).lt_field_name;
                if ((lt_map_ref->map[iter]).policer == TRUE) {
                    sbr_fid_name = (lt_map_ref->map[iter]).lt_sbr_field_name;
                }
                ...
                if ((lt_map_ref->map[iter]).num_internal_maps > 0) {
                    ...
                    for (iter1 = 0;
                         ...
                         sbr_fid_name = lt_intr_map[iter1].lt_field_name;
                         if (lt_intr_map[iter1].policer == TRUE) {
                             sbr_fid_name = lt_intr_map[iter1].lt_sbr_field_name;
                         }
                         SHR_IF_ERR_VERBOSE_EXIT
                             (bcmlt_entry_field_add(
                                        sbr_template,
...
exit:
    ...
    if (grp_prio != NULL) {
        BCMINT_FIELD_MEM_FREE(grp_prio);
    }
    SHR_FUNC_EXIT();
}
    ...
    int *color_sbr_idx)
{
    ...
    bcmi_field_aset_t valid_aset;
    bcmi_field_aset_t color_sbr_aset;
    bcm_field_action_t stat_action[3] = {
    ...
    for (i = 0; i < 3; i++) {
        if (BCM_FIELD_ASET_TEST(*aset, stat_action[i])) {
            int a, action_stat_cnt = 0;
            bcmi_field_action_t action_stat_arr[10];

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_color_flexctr_valid_action_get(
                        unit, stage_info,
                        stat_action[i],
                        hintid, &valid_aset,
                        10, action_stat_arr,
    ...
    for (iter = 0; iter < ltmap_ref->num_maps; iter++) {
        ...
        for (idx = 0; idx < stage_info->action_db_count; idx++) {
            ...
            for (iter1 = 0; iter1 < ltmap_tmp->num_maps; iter1++) {
                if (sal_strcmp((ltmap_ref->map[iter]).lt_field_name,
...
bcmint_field_color_flexctr_valid_action_get(
    int unit,
    bcmint_field_stage_info_t *stage_info,
    bcm_field_action_t stat_action,
    int hintid,
    bcmi_field_aset_t *aset,
    int max_action_cnt,
    bcmi_field_action_t *action_arr,
    int *action_cnt)
{
    ...
    *action_cnt = 0;
    if (!BCM_FIELD_ASET_TEST(*aset, stat_action)) {
        SHR_EXIT();
    } else {
        BCM_FIELD_ASET_REMOVE(*aset, stat_action);
    }
    ...
    for (iter = 0; iter < ltmap_ref->num_maps; iter++) {
        /* include the field based on opaque used used by group */
        field_name = (ltmap_ref->map[iter]).lt_field_name;
        if (ltmap_ref->map[iter].policer == true) {
            field_name = (ltmap_ref->map[iter]).lt_pdd_field_name;
        }
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_field_action_stat_group_lt_field_validate(
                unit, stage_info, hintid,
...
int
bcmint_field_group_active_entries_get(
    int unit,
    bcmint_field_stage_info_t *stage_info,
    bcm_field_group_t group,
...
int
bcmint_field_stage_group_num_get(
    int unit,
    bcmint_field_stage_info_t *stage_info,
    int pipe,
    int grp_prio,
    ...
    *num_grps = 0;
    for (idx = 0; idx < stage_info->group_info->hash_size; idx++) {
        ...
        do {
            ...
            if ((grp_prio != -1) && (grp_oper->priority != grp_prio)) {
                FP_HA_GRP_OPER_NEXT(unit, grp_oper, grp_oper);
                continue;
            }

            if ((group_flags != 0) &&
                !(grp_oper->group_flags & group_flags)) {
                FP_HA_GRP_OPER_NEXT(unit, grp_oper, grp_oper);
                continue;
            }
...
int
bcmint_field_stage_first_group_get(
    int unit,
    bcmint_field_stage_info_t *stage_info,
    int pipe,
    int grp_prio,
