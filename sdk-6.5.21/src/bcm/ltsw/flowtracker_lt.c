        ...
        if(!sal_strcmp(actions_type_symbol, EM_FT_OPAQUE_OBJ0s)) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_array_get(
                                             ...
                                             &r_val_cnt));
            action_list[action_idx].action =
                bcmFlowtrackerGroupActionMirrorIndex;
            action_list[action_idx].params[0] = 0;
            action_list[action_idx].params[1] = 0;
            if (actions_val != 0) {
                sal_memset(&info, 0, sizeof(info));
                info.mirror_index = actions_val;
                flags = 0;
                mirror_dest = 0;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_mirror_dest_id_get(unit, &info,
                                                  &mirror_dest,
                                                  &flags));
                if (flags & BCM_MIRROR_SOURCE_INGRESS) {
                    action_list[action_idx].params[0] = BCM_MIRROR_SOURCE_INGRESS;
                } else if (flags & BCM_MIRROR_SOURCE_EGRESS) {
                    action_list[action_idx].params[0] = BCM_MIRROR_SOURCE_EGRESS;
                }
                if (get_flags & FT_GRP_ENTRY_GET_MIRROR_IDX_ONLY) {
                    action_list[action_idx].params[1] = actions_val;
                } else {
                    action_list[action_idx].params[1] = mirror_dest;
                }
            }
            action_idx++;
        }
                            ...
                            int *action_idx)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_array_add(
                                     entry_hdl,
                                     ACTIONS_PKT_FLOW_ELIGIBILITY_VALs,
                                     *action_idx,
                                     &actions_val,
                                     1));
    (*action_idx)++;
    ...
    for (i = 0; i < num_actions; i++) {
        ...
        switch(action_list[i].action) {
            ...
            case bcmFlowtrackerGroupActionMirrorIndex:
                ...
                actions_val = action_list[i].params[1];
                /* Param1 = 0 is special case. considered mirror cancel */
                if (actions_val != 0) {
                    flags = action_list[i].params[0];
                    if (flags != BCM_MIRROR_SOURCE_INGRESS &&
                        flags != BCM_MIRROR_SOURCE_EGRESS) {
                        SHR_ERR_EXIT(SHR_E_PARAM);
                    }
                    flags |=
                     (BCMI_LTSW_MIRROR_EM_FT | BCMI_LTSW_MIRROR_HW_LEARN_ENTRY);
                    mirror_gport = action_list[i].params[1];
                    if (!BCM_GPORT_IS_MIRROR(mirror_gport)) {
                        SHR_ERR_EXIT(SHR_E_PARAM);
                    }
                    sal_memset(&mirror_info, 0, sizeof(mirror_info));
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmi_ltsw_mirror_field_dest_add(
                                                         ...
                                                         mirror_gport,
                                                         &mirror_info));
                    actions_val = mirror_info.mirror_index;
                }
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_array_symbol_add(
                                                        entry_hdl,
                                     ...
                                     bcm_int_ft_flow_group_info_t *entry)
{
    ...
    for (i = 0; i < ft_info->num_pipes; i++) {
        /* Or it with base value */
        entry->field_group[i] = (val | BCM_FIELD_FT_ID_BASE);
    }
        ...
        bcm_int_ft_group_col_map_t *entry)
{
    ...
    if (!sal_strcmp(str_value, "IPV4")) {
        entry->collector_type = bcmiCollectorTransportTypeIpv4Udp;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(entry_hdl, MON_COLLECTOR_IPV4_IDs, &val));
        entry->collector_ipv4_id = val;
    } else if (!sal_strcmp(str_value, "IPV6")) {
        entry->collector_type = bcmiCollectorTransportTypeIpv6Udp;
        SHR_IF_ERR_VERBOSE_EXIT
...
/*
 * The caller is expected to have allocated enough memory
 * for flow_entry_data.
 */
static
int bcm_ltsw_flowtracker_learn_flow_entry_get (int unit,
                                int pipe,
                                const char *mode_sym,
                                uint32_t exact_match_idx,
                                uint8_t *num_entry_words,
                                uint32_t *flow_entry_data)
{
    bcmi_lt_entry_t entry_info;
    const char *lt_name = MON_FLOWTRACKER_LEARN_FLOW_ENTRYs;
    int i;
    bcmi_lt_field_t fields_arr[] = {
       /* 0 */ {PIPEs, BCMI_LT_FIELD_F_SET, 0, {0}},
       /* 1 */ {MODEs, BCMI_LT_FIELD_F_SET|BCMI_LT_FIELD_F_SYMBOL, 0, {0}},
       /* 2 */ {EXACT_MATCH_INDEXs, BCMI_LT_FIELD_F_SET, 0, {0}},
       /* 3 */ {NUM_ENTRY_DATAs, BCMI_LT_FIELD_F_GET, 0, {0}},
       /* 4 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 0, {0}},
       /* 5 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 1, {0}},
       /* 6 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 2, {0}},
       /* 7 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 3, {0}},
       /* 8 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 4, {0}},
       /* 9 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 5, {0}},
       /* 10 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 6, {0}},
       /* 11 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 7, {0}},
       /* 12 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 8, {0}},
       /* 13 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 9, {0}},
       /* 14 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 10, {0}},
       /* 15 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 11, {0}},
       /* 16 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 12, {0}},
       /* 17 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 13, {0}},
       /* 18 */ {ENTRY_DATAs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_ARRAY, 14, {0}}
    };
    SHR_FUNC_ENTER(unit);


    /* Update values within the fields */
    fields_arr[0].u.val = pipe;
    fields_arr[1].u.sym_val = mode_sym;
    fields_arr[2].u.val = exact_match_idx;

    entry_info.fields = fields_arr;
    entry_info.nfields = sizeof(fields_arr)/sizeof(bcmi_lt_field_t);
    entry_info.attr = 0;
    SHR_IF_ERR_EXIT(bcmi_lt_entry_get(unit, lt_name, &entry_info, NULL, NULL));
    *num_entry_words = fields_arr[3].u.val;
    for (i = 0; i < *num_entry_words; i++) {
       flow_entry_data[i] = fields_arr[4+i].u.val;
    }
exit:
    SHR_FUNC_EXIT();
}

static void ft_learn_evt_log_parser(int unit,
                                    const char *lt_name,
                                    uint32_t entry_hdl,
                                    void *notif_info,
                                    bcmi_ltsw_event_status_t *status)
{
    bcm_int_ft_learn_notify_t *learn_notify = (bcm_int_ft_learn_notify_t *)notif_info;
    bcm_flowtracker_flow_entry_t *flow_entry = &(learn_notify->flow_entry);
    int rv;
    bcmlt_unified_opcode_t op;
    uint64_t val = 0;
    const char* mode_sym;
    uint32_t flow_entry_data[FLOWTRACKER_ENTRY_DATA_WORDS_MAX] = {0};
    uint8_t num_data_words = 0;
    bcm_int_flowtracker_em_group_info_t *em_grp_info = NULL;
    int em_group_id = 0;
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);

#define FT_LEARN_EVT_LOG_PARSE_ERR_HANDLE(rv, fail_str)\
    if (SHR_FAILURE(rv)) {\
    ...
    /*
     * Set the notification initially to dismiss. If it goes through the parse
     * logic fully, it will be changed to pass notification.
     */
    *status = bcmiLtswEventStatusDismissNotif;
    ...
    rv = bcmlt_entry_oper_get(entry_hdl, &op);
    FT_LEARN_EVT_LOG_PARSE_ERR_HANDLE(rv, "opcode");

    /* Ignore non update opcodes */
    if (op.opcode.lt != BCMLT_OPCODE_UPDATE) {
        return;
    }

    /* Fetch PIPE */
    rv = bcmlt_entry_field_get(entry_hdl, PIPEs, &val);
    FT_LEARN_EVT_LOG_PARSE_ERR_HANDLE(rv, "pipe");
    flow_entry->pipe = val;

    /* Fetch EM INDEX mode */
    rv = bcmlt_entry_field_symbol_get(entry_hdl, MODEs, &mode_sym);
    FT_LEARN_EVT_LOG_PARSE_ERR_HANDLE(rv, "EM idx mode");
    if (!strcmp(mode_sym, SINGLEs)) {
        flow_entry->mode = bcmFlowtrackerExactMatchIdxModeSingle;
    } else if (!strcmp(mode_sym, DOUBLEs)) {
        flow_entry->mode = bcmFlowtrackerExactMatchIdxModeDouble;
    } else if (!strcmp(mode_sym, QUADs)) {
        flow_entry->mode = bcmFlowtrackerExactMatchIdxModeQuad;
    }

    /* Fetch EM index */
    rv = bcmlt_entry_field_get(entry_hdl, EXACT_MATCH_INDEXs, &val);
    FT_LEARN_EVT_LOG_PARSE_ERR_HANDLE(rv, "EM idx");
    flow_entry->exact_match_idx = val;

    /* Fetch FT group ID */
    rv = bcmlt_entry_field_get(entry_hdl, MON_FLOWTRACKER_GROUP_IDs, &val);
    FT_LEARN_EVT_LOG_PARSE_ERR_HANDLE(rv, "Group ID");
    flow_entry->group_id = val;

    /* Fetch EM group ID from FT group ID */
    em_group_id = ft_info->ft_grp_to_em_grp_map[flow_entry->group_id];

    /* Fetch EM group info from EM group ID */
    em_grp_info = &(ft_em_group_info[unit][em_group_id]);
    if (em_grp_info == NULL || em_grp_info->em_group_info_valid == false) {
        FT_LEARN_EVT_LOG_PARSE_ERR_HANDLE(SHR_E_INTERNAL, "EM group");
    }

    /* Fetch the HW entry by looking up MON_FLOWTRACKER_LEARN_FLOW_ENTRY LT */
    rv = bcm_ltsw_flowtracker_learn_flow_entry_get(unit,
                                                   flow_entry->pipe,
                                                   mode_sym,
                                                   flow_entry->exact_match_idx,
                                                   &num_data_words,
                                                   flow_entry_data);
    FT_LEARN_EVT_LOG_PARSE_ERR_HANDLE(rv, "Flow entry data");

    /*
     * Extract the num_tuples, tuple_qualifiers, tuple_values from
     * flow entry data using EM group information.
     */
    bcm_ltsw_flowtracker_flow_tuple_extract(unit,
                                            em_grp_info,
                                            num_data_words,
                                            flow_entry_data,
                                            flow_entry);

    /* Parsing logic succeeded. Set status to pass the notification. */
    *status = bcmiLtswEventStatusPassNotif;
}

/*!
 * \brief The callback function to handle FT learn notification.
 *
 * This function is responsible for providing FT learn notifications to
 * user app which would have subscribed with a callback of their own.
 *
 * \param [in]   unit         Unit number.
 * \param [in]   event        Event string.
 * \param [in]   notif_info   FT learn event info.
 * \param [in]   user_data    User data.
 */
static void
ft_learn_evt_log_cb(int unit, const char *event, void *notif_info,
               void *user_data)
{
    bcm_int_ft_learn_notify_t *learn_notify;

    if (notif_info == NULL) {
        return;
    }
    learn_notify = (bcm_int_ft_learn_notify_t *)notif_info;

    if (ft_learn_user_cb[unit] != NULL) {
        ft_learn_user_cb[unit](unit, &(learn_notify->flow_entry),
                                 ft_learn_user_cb_data[unit]);
    }
}

int bcm_ltsw_flowtracker_learn_event_log_subscribe_set(int unit, bool enable)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    const char *lt_name = MON_FLOWTRACKER_LEARN_EVENT_LOGs;
    SHR_FUNC_ENTER(unit);
    ...
    if (enable) {
        SHR_IF_ERR_EXIT
            (bcmi_lt_table_parser_set(unit, lt_name,
                                      ft_learn_evt_log_parser,
                                      sizeof(bcm_int_ft_learn_notify_t)));
        SHR_IF_ERR_EXIT
            (bcmi_lt_table_attrib_set(unit, lt_name,
                                      (BCMI_LTSW_EVENT_HIGH_PRIORITY |
                                       BCMI_LTSW_EVENT_APPL_CALLBACK)));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_subscribe(unit, lt_name, ft_learn_evt_log_cb, NULL));
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_unsubscribe(unit, lt_name, ft_learn_evt_log_cb));
    }
exit:
    SHR_FUNC_EXIT();
}

int bcm_ltsw_flowtracker_learn_event_control_entry_set (int unit,
                                bcm_int_ft_learn_evt_ctrl_t *entry)
{
    bcmi_lt_entry_t entry_info;
    const char *lt_name = MON_FLOWTRACKER_LEARN_EVENT_CONTROLs;
    bcmi_lt_field_t fields_arr[] = {
       /* 0 */ {PIPEs, BCMI_LT_FIELD_F_SET, 0, {0}},
       /* 1 */ {NOTIFY_LEARN_SUCCESSs, BCMI_LT_FIELD_F_SET, 0, {0}},
       /* 2 */ {STOP_LEARN_ON_NOTIFY_FULLs, BCMI_LT_FIELD_F_SET, 0, {0}},
       /* 3 */ {FLOW_LEARN_FAIL_THRESHOLDs, BCMI_LT_FIELD_F_SET, 0, {0}},
    };
    SHR_FUNC_ENTER(unit);


    /* Update values within the fields */
    fields_arr[0].u.val = entry->pipe;
    fields_arr[1].u.val = entry->notif_learn;
    if (entry->flags & BCM_FLOWTRACKER_FLOW_LEARN_STOP_ON_FIFO_FULL) {
        fields_arr[2].u.val = 1;
    } else {
        fields_arr[2].u.val = 0;
    }
    fields_arr[3].u.val = entry->fail_threshold;

    entry_info.fields = fields_arr;
    entry_info.nfields = sizeof(fields_arr)/sizeof(bcmi_lt_field_t);
    entry_info.attr = 0;
    SHR_IF_ERR_EXIT(bcmi_lt_entry_update(unit, lt_name, &entry_info, NULL));
exit:
    SHR_FUNC_EXIT();
}

int bcm_ltsw_flowtracker_learn_event_control_entry_get (int unit,
                                bcm_int_ft_learn_evt_ctrl_t *entry)
{
    bcmi_lt_entry_t entry_info;
    const char *lt_name = MON_FLOWTRACKER_LEARN_EVENT_CONTROLs;
    bcmi_lt_field_t fields_arr[] = {
        /* 0 */ {PIPEs, BCMI_LT_FIELD_F_SET/*Key */, 0, {0}},
        /* 1 */ {NOTIFY_LEARN_SUCCESSs, BCMI_LT_FIELD_F_GET, 0, {0}},
        /* 2 */ {STOP_LEARN_ON_NOTIFY_FULLs, BCMI_LT_FIELD_F_GET, 0, {0}},
        /* 3 */ {FLOW_LEARN_FAIL_THRESHOLDs, BCMI_LT_FIELD_F_GET, 0, {0}},
    };
    SHR_FUNC_ENTER(unit);
    ...
    entry_info.attr = 0;
    SHR_IF_ERR_EXIT(bcmi_lt_entry_get(unit, lt_name, &entry_info, NULL, NULL));
    entry->notif_learn = fields_arr[1].u.val;
    if (fields_arr[2].u.val) {
        entry->flags |= BCM_FLOWTRACKER_FLOW_LEARN_STOP_ON_FIFO_FULL;
    }
    entry->fail_threshold = fields_arr[3].u.val;
exit:
    SHR_FUNC_EXIT();
}


int bcm_ltsw_flowtracker_learn_fail_event_control_entry_set (int unit,
                                bcm_int_ft_learn_fail_evt_ctrl_t *entry)
{
    bcmi_lt_entry_t entry_info;
    const char *lt_name = MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLs;
    bcmi_lt_field_t fields_arr[] = {
       /* 0 */ {PIPEs, BCMI_LT_FIELD_F_SET, 0, {0}},
       /* 1 */ {STATEs, BCMI_LT_FIELD_F_SET|BCMI_LT_FIELD_F_SYMBOL, 0, {0}},
    };
    SHR_FUNC_ENTER(unit);
    ...
    /* Update values within the fields */
    fields_arr[0].u.val = entry->pipe;
    if (entry->intr_enable) {
        fields_arr[1].u.sym_val = ARMEDs;
    } else {
        fields_arr[1].u.sym_val = OFFs;
    }
...
int bcm_ltsw_flowtracker_learn_fail_event_control_entry_get (int unit,
                                bcm_int_ft_learn_fail_evt_ctrl_t *entry)
{
    bcmi_lt_entry_t entry_info;
    const char *lt_name = MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLs;
    bcmi_lt_field_t fields_arr[] = {
        /* 0 */ {PIPEs, BCMI_LT_FIELD_F_SET/*Key */, 0, {0}},
        /* 1 */ {STATEs, BCMI_LT_FIELD_F_GET|BCMI_LT_FIELD_F_SYMBOL, 0, {0}},
    };
    SHR_FUNC_ENTER(unit);
    ...
    if (!sal_strcmp(fields_arr[1].u.sym_val, ARMEDs)) {
        entry->intr_enable = 1;
    } else {
        entry->intr_enable = 0;
    }
...
static void ft_learn_fail_evt_state_parser(int unit,
                                    const char *lt_name,
                                    uint32_t entry_hdl,
                                    void *notif_info,
                                    bcmi_ltsw_event_status_t *status)
{
    int *pipe = (int *)notif_info;
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    const char *state_sym = NULL;
    int rv;
    bcmlt_unified_opcode_t op;
    uint64_t val = 0;

#define FT_LEARN_FAIL_EVT_STATE_PARSE_ERR_HANDLE(rv, fail_str)\
    if (SHR_FAILURE(rv)) {\
        LOG_WARN(BSL_LOG_MODULE,\
                (BSL_META_U(unit,\
                            "Failed to get %s %s(%d).\n"),\
    ...
    *status = bcmiLtswEventStatusDismissNotif;
    ...
    rv = bcmlt_entry_oper_get(entry_hdl, &op);
    FT_LEARN_FAIL_EVT_STATE_PARSE_ERR_HANDLE(rv, "opcode");
    ...
    /* Fetch STATE */
    rv = bcmlt_entry_field_symbol_get(entry_hdl, STATEs, &state_sym);
    FT_LEARN_FAIL_EVT_STATE_PARSE_ERR_HANDLE(rv, "state");
    if (strcmp(state_sym, TRIGGEREDs) != 0) {
        /* If the state is other than triggered, ignore the notification */
        return;
    }

    /* Fetch PIPE */
    rv = bcmlt_entry_field_get(entry_hdl, PIPEs, &val);
    FT_LEARN_FAIL_EVT_STATE_PARSE_ERR_HANDLE(rv, "pipe");
    *pipe = val;
...
/*!
 * \brief The callback function to handle FT learn failure notification.
 *
 * This function is responsible for providing FT learn failure notifications to
 * user app which would have subscribed with a callback of their own.
 *
 * \param [in]   unit         Unit number.
 * \param [in]   event        Event string.
 * \param [in]   notif_info   FT learn failure event info.
 * \param [in]   user_data    User data.
 */
static void
ft_learn_fail_evt_state_cb(int unit, const char *event, void *notif_info,
               void *user_data)
{
    int pipe = 0;
    bcm_int_ft_learn_fail_evt_ctrl_t ctrl_entry;
    int rv = SHR_E_NONE;
    ...
    if (ft_learn_fail_user_cb[unit] != NULL) {
        ft_learn_fail_user_cb[unit](unit, pipe,
                                 ft_learn_fail_user_cb_data[unit]);
    }
    /*
     * Callback has reurned - Reset the event control state to ARMED again.
     * But first do a get to see the latest control state. If it is OFF,
     * do not set it to ARMED. User app might have disabled the interrupt
     * but not necessarily un-registered the callback.
     */
    ctrl_entry.pipe = pipe;
    ctrl_entry.intr_enable = 0;
    rv = bcm_ltsw_flowtracker_learn_fail_event_control_entry_get(unit,
            &ctrl_entry);
    if (rv != SHR_E_NONE) {
        /* Unexpected. */
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit,
                        "Getting the failure interrupt control state failed for pipe %d\n"),
                    pipe));
    }
    if (ctrl_entry.intr_enable == 0) {
        /* Do not bother re-enabling since user has disabled it */
        return;
    }
    ctrl_entry.intr_enable = 1;
    rv = bcm_ltsw_flowtracker_learn_fail_event_control_entry_set(unit,
            &ctrl_entry);
    if (rv != SHR_E_NONE) {
        /* Unexpected. */
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit,
                        "Re-arming the failure interrupt failed for pipe %d\n"),
                    pipe));
    }
}

int bcm_ltsw_flowtracker_learn_fail_event_state_subscribe_set(
                                                    int unit, bool enable)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    const char *lt_name = MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATEs;
    SHR_FUNC_ENTER(unit);
    ...
    if (enable) {
        SHR_IF_ERR_EXIT
            (bcmi_lt_table_parser_set(unit, lt_name,
                                      ft_learn_fail_evt_state_parser,
                                      /*
                                       * Notification info is just PIPE.
                                       * So size = size of int
                                       */
                                      sizeof(int)));
        SHR_IF_ERR_EXIT
            (bcmi_lt_table_attrib_set(unit, lt_name,
                                      (BCMI_LTSW_EVENT_HIGH_PRIORITY |
                                       BCMI_LTSW_EVENT_APPL_CALLBACK)));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_subscribe(unit, lt_name, ft_learn_fail_evt_state_cb,
                                    NULL));
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_table_unsubscribe(unit, lt_name, ft_learn_fail_evt_state_cb));
    }
exit:
    SHR_FUNC_EXIT();
}

int bcm_ltsw_flowtracker_learn_event_stats_get (int unit,
                                bcm_int_ft_learn_evt_stats_t *entry)
{
    bcmi_lt_entry_t entry_info;
    const char *lt_name = MON_FLOWTRACKER_LEARN_EVENT_STATSs;
    bcmi_lt_field_t fields_arr[] = {
        /* 0 */ {PIPEs, BCMI_LT_FIELD_F_SET/*Key */, 0, {0}},
        /* 1 */ {NUM_LEARN_FAILs, BCMI_LT_FIELD_F_GET, 0, {0}},
    };
    bool is_supp = false;
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT(
        mbcm_ltsw_flowtracker_ft_learn_fail_count_supported(unit, &is_supp));

    /* Simply return for unsupported platforms */
    if (is_supp == false) {
        SHR_EXIT();
    }
    ...
    entry_info.attr = 0;
    SHR_IF_ERR_EXIT(bcmi_lt_entry_get(unit, lt_name, &entry_info, NULL, NULL));
    entry->num_learn_fail = fields_arr[1].u.val;
exit:
    SHR_FUNC_EXIT();
}

int bcm_ltsw_flowtracker_learn_event_stats_set (int unit, bool update,
                                bcm_int_ft_learn_evt_stats_t *entry)
{
    bcmi_lt_entry_t entry_info;
    const char *lt_name = MON_FLOWTRACKER_LEARN_EVENT_STATSs;
    bcmi_lt_field_t fields_arr[] = {
        /* 0 */ {PIPEs, BCMI_LT_FIELD_F_SET/*Key */, 0, {0}},
        /* 1 */ {NUM_LEARN_FAILs, BCMI_LT_FIELD_F_SET, 0, {0}},
    };
    bool is_supp = false;
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT(
    ...
    /* Update values within the fields */
    fields_arr[0].u.val = entry->pipe;
    fields_arr[1].u.val = entry->num_learn_fail;
    ...
    if (update) {
        ...
        SHR_IF_ERR_EXIT(bcmi_lt_entry_insert(unit, lt_name, &entry_info, NULL));
    }
exit:
    SHR_FUNC_EXIT();
}

int bcm_ltsw_flowtracker_learn_event_stats_dummy_entries_set(int unit)
{
    bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    bcm_int_ft_learn_evt_stats_t entry;
    int i;
    ...
    for (i = 0; i < ft_info->num_pipes; i++) {
        entry.pipe = i;
        entry.num_learn_fail = 0;
        SHR_IF_ERR_EXIT(
            bcm_ltsw_flowtracker_learn_event_stats_set(unit, false, &entry));
    }
exit:
    SHR_FUNC_EXIT();
}
