...
{
    bcm_pbmp_t pbmp_all, pbm_info_skip;
    int port, rv;
    ...
    rv = bcmi_ltsw_port_bitmap(unit, BCMI_LTSW_PORT_TYPE_PORT, &pbmp_all);
    if (rv != BCM_E_NONE) {
        LOG_CLI((BSL_META_U(unit,
                           "Error: Could not get port bitmap.\n")));
        return;
    }

    LOG_CLI((BSL_META_U(unit,
                        "Linkscan callbacks for unit %d.\n"), unit));
    for (item = lc->lc_cbs; item != NULL; item = item->next) {
        LOG_CLI((BSL_META_U(unit,
                            "\tFn %p.\n"), (void *)item->cb));
    }
    LOG_CLI((BSL_META_U(unit,
                        "Pbmp of Linkscan port control %s.\n"),
            _SHR_PBMP_FMT(lc->lc_pbm_pc, pfmt)));

    BCM_PBMP_CLEAR(pbm_info_skip);

    BCM_PBMP_ITER(pbmp_all, port) {
        if (SHR_BITGET(lc->lc_pbm_info_skip, port)) {
            BCM_PBMP_PORT_ADD(pbm_info_skip, port);
        }
    }
    LOG_CLI((BSL_META_U(unit,
                        "Pbmp of skip reporting info during linkdown %s.\n\n"),
            _SHR_PBMP_FMT(pbm_info_skip, pfmt)));
}
...
/*!
 * \brief Update link state for the ports.
 *
 * \param [in] unit Unit number.
 * \param [in] pbm Bit map of ports to update link state.
 *
 * \retval SHR_E_NONE All the port link states are updated.
 * \retval SHR_E_UNIT Invalid unit.
 * \retval SHR_E_MEMORY No memory resources.
 */
int
bcm_ltsw_linkscan_update(int unit, bcm_pbmp_t pbm)
{
    bcm_port_t port, link;
    ls_port_ctrl_t *pc = NULL;
    ls_port_ctrl_t ctrl = {0};
    ...
    pc = sal_alloc(sizeof(ls_port_ctrl_t) * BCM_PBMP_PORT_MAX, "ltswlinkscanupdate");
    SHR_NULL_CHECK(pc, SHR_E_MEMORY);
    sal_memset(pc, 0, sizeof(ls_port_ctrl_t) * BCM_PBMP_PORT_MAX);

    BCM_PBMP_ITER(pbm, port) {
        /* Store the current linkscan mode */
        pc[port].port = port;
        pc[port].fld_flags = FLD_MODE | FLD_NO_SYNC;
        SHR_IF_ERR_EXIT
            (lt_port_control_get(unit, &pc[port]));

        /* Set to NO_SCAN mode */
        if (pc[port].mode != LS_NO_SCAN) {
            ctrl.port = port;
            ctrl.fld_flags = FLD_MODE;
            ctrl.mode = LS_NO_SCAN;
            SHR_IF_ERR_EXIT
                (lt_port_control_set(unit, BCMLT_OPCODE_UPDATE, &ctrl));
        }
    ...
    BCM_PBMP_ITER(pbm, port) {
        /* Eliminate latched state if HW linkscan mode is enabled */
        if (pc[port].mode == LS_HARDWARE) {
            ctrl.port = port;
            ctrl.fld_flags = FLD_NO_SYNC;
            ctrl.no_sync = 1;
            SHR_IF_ERR_EXIT
                (lt_port_control_set(unit, BCMLT_OPCODE_UPDATE, &ctrl));
            bcmi_ltsw_link_state_get(unit, port, &link);
            bcmi_ltsw_link_state_get(unit, port, &link);
        }
    }

    BCM_PBMP_ITER(pbm, port) {
        /* Force to sync link state */
        ctrl.port = port;
        ctrl.fld_flags = FLD_NO_SYNC;
        ctrl.no_sync = 0;
        SHR_IF_ERR_EXIT
    ...
    BCM_PBMP_ITER(pbm, port) {
        /* Restore the previous linkscan mode */
        SHR_IF_ERR_EXIT
            (lt_port_control_set(unit, BCMLT_OPCODE_UPDATE, &pc[port]));
    }
