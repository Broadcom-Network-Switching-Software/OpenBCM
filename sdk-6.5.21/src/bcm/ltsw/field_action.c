        ...
        *action_value = class_index;
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_field_action_profile_add(unit,
                                                ...
                                                match_config,
                                                action_value));
    } else if (bcmFieldActionVpnSet == action) {
        if (BCMI_LTSW_VIRTUAL_VPN_IS_SET((bcm_vpn_t)params->param0)) {
            BCMI_LTSW_VIRTUAL_VPN_GET(*action_value, BCMI_LTSW_VIRTUAL_VPN_TYPE_VFI,
                                      (bcm_vpn_t)params->param0);
            entry_oper->action_flags |= BCMINT_FIELD_ENTRY_ACTION_VPN_VFI_SET;
        } else {
        ...
        *action_value = class_index;
        ...
        if (params->param0 != 0) {
            ...
            if ((action == bcmFieldActionDrop) ||
                ...
                (action == bcmFieldActionPrioInputCopyIntPrio) ||
                ...
                *action_value = ((uint64_t)1 << lt_map_action_width) - 1;
                              ...
                              uint64_t *action_value)
{
    ...
    if ((action == bcmFieldActionCosMapNew) ||
        ...
        if (entry_oper->action_flags & BCMINT_FIELD_ENTRY_ACTION_VPN_VFI_SET) {
            BCMI_LTSW_VIRTUAL_VPN_SET(params->param0, BCMI_LTSW_VIRTUAL_VPN_TYPE_VFI,
                                  *action_value);
        } else {
    ...
    uint64_t *param)
{
    ...
    if (entry_oper->color_pool_id != -1) {
        pool = entry_oper->color_pool_id;
    } else {
    ...
    for (iter = 0; iter <  lt_map->num_maps; iter++) {
        if ((lt_map->map[iter]).policer == TRUE) {
            /* If LT field name is present, look for match. */
            if ((field_name != NULL) &&
                (sal_strcmp(field_name, (lt_map->map[iter]).lt_field_name)) &&
                (sal_strcmp(field_name, (lt_map->map[iter]).lt_pdd_field_name))) {
                continue;
            }
            SHR_IF_ERR_VERBOSE_EXIT
...
static int
bcmint_field_meter_profile_index_update(
    int unit,
    bcmint_field_stage_info_t *stage_info,
    bcmi_field_ha_entry_oper_t *entry_oper,
    int new_pool)
{
    ...
    for (idx = 0; idx < lt_map->num_maps; idx++) {
       if ((lt_map->map[idx]).color_index != new_pool) {
           continue;
       }
       if ((lt_map->map[idx]).param_valid == true) {
...
static int
bcmint_field_lt_entry_copy(
    int unit,
    bcmlt_entry_handle_t src_hdl,
    bcmlt_entry_handle_t dst_hdl)
{
    int rv = 0;
    const char *fname;
    ...
    while ((rv = bcmlt_entry_field_traverse(src_hdl,
                                            &fname,
                                            &finfo,
                                            &fhdl)) == SHR_E_NONE) {
        if (finfo.array) {
            if (finfo.symbol) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_array_symbol_add(dst_hdl,
                                                        fname,
                                                        finfo.idx,
                                                        &finfo.sym_val,
                                                        1));
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_array_add(dst_hdl,
                                                 fname,
                                                 finfo.idx,
                                                 &finfo.val,
                                                 1));
            ...
            if (finfo.symbol) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_symbol_add(dst_hdl,
                                                  fname,
                                                  finfo.sym_val));
            } else {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(dst_hdl,
                                           fname,
                                           finfo.val));
            }
        }
    ...
    bcm_field_action_params_t *params)
{
    ...
    bcmlt_field_def_t fdef;
    bcmlt_field_def_t int_fdef;
    bcmi_field_ha_profile_info_t *profile_ptr = NULL;
    const bcm_field_action_map_t *lt_map = NULL;
    bcmint_field_tbls_info_t *tbls_info = NULL;
    bcmint_field_color_action_tbls_info_t *color_tbl_info = NULL;
    bcmlt_entry_handle_t color_action_template = BCMLT_INVALID_HDL;
    bcmlt_entry_handle_t new_action_template = BCMLT_INVALID_HDL;
    ...
    /*
     * If group supports entry level policer, colored actions can be
     * added only after attaching policer.
     */
    ...
    if (entry_oper->color_pool_id != -1) {
        curr_pool = entry_oper->color_pool_id;
    } else {
        curr_pool = group_oper->policer_pool_id;
    }

    /* Handle for METER_ACTION_TEMPLATE table. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_allocate(dunit,
                              color_tbl_info->tbl_sid[curr_pool],
                              &color_action_template));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(color_action_template,
    ...
    if (entry_oper->color_action_id != -1) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(color_action_template,
                                   color_tbl_info->tbl_key_fid[curr_pool],
    ...
    for (iter = 0; iter <  lt_map->num_maps; iter++) {
        ...
        if ((lt_map->map[iter]).policer == TRUE) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_lt_field_def_get(
                        unit, color_tbl_info->tbl_sid[curr_pool],
                        (lt_map->map[iter]).lt_field_name,
                        &fdef));
    ...
    rv = bcmint_field_color_profile_id_alloc(
        unit,
        color_tbl_info->tbl_sid[curr_pool],
        color_tbl_info->tbl_key_fid[curr_pool],
        color_tbl_info->pipe_fid,
    ...
    if (SHR_FAILURE(rv)) {
        if (rv == SHR_E_RESOURCE) {
            SHR_BITSET(&stage_info->color_pool_full_bmp[pipe_val], curr_pool);
            for (iter = 0; iter < color_tbl_info->num_pools; iter++) {
                if (iter == curr_pool) {
                    continue;
                }
                /* Find another color table pool which uses same container. */
                if (color_tbl_info->cont_id[iter] ==
                    color_tbl_info->cont_id[curr_pool]) {
                    rv = bcmint_field_color_profile_id_alloc(
                        unit,
                        color_tbl_info->tbl_sid[iter],
                        color_tbl_info->tbl_key_fid[iter],
                        color_tbl_info->pipe_fid,
                        color_action_template,
                        pipe,
                        iter,
                    ...
                    if (SHR_SUCCESS(rv)) {
                        ...
                        SHR_BITSET(&stage_info->color_pool_full_bmp[pipe_val],
                                   iter);
                    }
                }
            }
    ...
    if (insert == true) {
        if (curr_pool != new_pool) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_allocate(dunit,
                                      color_tbl_info->tbl_sid[new_pool],
                                      &new_action_template));

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_lt_entry_copy(unit,
                                            color_action_template,
                                            new_action_template));

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(new_action_template,
                                       color_tbl_info->tbl_key_fid[new_pool],
                                       entry_oper->color_action_id));
    ...
    if (curr_pool != new_pool) {
        entry_oper->color_pool_id = new_pool;
    }
...
exit:
    ...
    if (BCMLT_INVALID_HDL != new_action_template) {
        (void) bcmlt_entry_free(new_action_template);
    }
    SHR_FUNC_EXIT();
}
        ...
        if (delete == true) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_lt_entry_commit(unit,
                                      ...
                                      BCMLT_OPCODE_DELETE,
                                      BCMLT_PRIORITY_NORMAL));
            SHR_BITCLR(&stage_info->color_pool_full_bmp[pipe_val], pool);
        }
        entry_oper->color_action_id = -1;
        /*
         * All colored actions removed from this entry and policer container
         * not provided by policer_extended_attach() API.
         */
        if (!(group_oper->group_flags & BCMINT_FIELD_GROUP_ENT_LEVEL_POLICER_POOL)) {
            entry_oper->color_pool_id = -1;
        }
        SHR_EXIT();
    }
...
static void
ltsw_field_is_offset_conflict(
    int unit,
    int offset1,
    int width1,
    int offset2,
    int width2,
    bool *is_action_conflict) {
        ...
        *is_action_conflict = true;
        ...
        if (offset1 == offset2) {
            *is_action_conflict = true;
            ...
            if ( offset2+width2 > offset1) {
                *is_action_conflict = true;
            }
        } else {
            if (offset2 < (offset1+width1)) {
                *is_action_conflict = true;
            }
        }
    }
...
/* To check if the given action's (action1) supported bits of LT names
 * conflict with supported bits of the LT fields of
 * configured action (action 2)
 * opaque_obj_map_act1 is set to check conflict of only the Lt field
 * against the LT fields of configured action. If null, all the Lt fields
 * of action will be checked agsin action 2
 */

int
bcmint_field_is_action_conflict(
    int unit,
    bcm_field_entry_t entry,
    bcm_field_action_t action1,
    bcm_field_action_t action2,
    char *opaque_obj_map_act1,
    bool *is_action_conflict)
{
    int entry_id = 0;
    bcmint_field_stage_info_t *stage_info = NULL;
    ...
    const bcm_field_action_map_t *lt_map_ref1 = NULL;
    const bcm_field_action_map_t *lt_map_ref2 = NULL;
    int offset1 =0, offset2 = 0, width1=0, width2 = 0;
    int iter1 = 0, iter2 = 0, idx1 = 0, idx2 = 0;
    int intr_map =0, intr_map2 =0, idx3 = 0, idx4 = 0;
    char *opaque_object1 = NULL;
    char *opaque_object2 = NULL;
    char *flexctr_cont_obj = NULL;
    uint64_t flex_ctr_action = 0;
    uint64_t flexctr_cont_id = 0;
    uint32_t opaque_obj_val = 0;
    uint32_t flexctr_color_valid = 0;
    uint8_t num_offset1, num_offset2;
    ...
    *is_action_conflict = false;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_group_oper_lookup(unit,
                                        ...
                                        stage_info,
                                        &group_oper));
    /* only non colored actions are validated for conflicts */
    if (stage_info->flags & BCMINT_FIELD_STAGE_ACTION_RESOLUTION_SBR) {
        if ((BCMINT_FIELD_IS_COLORED_ACTION(action1)) ||
            (BCMINT_FIELD_IS_COLORED_ACTION(action2))) {
            goto exit;
        }
        /* if action 1 is statGroup, get the opaque object field
         * to be checked against the given action (action 1) fields
         */

        if (action1 == bcmFieldActionStatGroup) {
            if (opaque_obj_map_act1 != NULL) {
                opaque_object1 = opaque_obj_map_act1;
            }
            /* if the entry is already configured, get the
             * opaque oject from profile
             */
            if (entry_oper->flexctr_action_id) {
                SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_entry_flexctr_action_prof_get(
                    unit, stage_info,
                    group_oper,
        ...
        /* if action2 is statGroup, then the read the opaque object
         * from action profile
         */
        if ((action2 == bcmFieldActionStatGroup)) {
            if (entry_oper->flexctr_action_id) {
                SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_entry_flexctr_action_prof_get(
                    unit, stage_info,
    ...
    /* do not consider MirrorActions to check conflicting*/
    if (((action1 == bcmFieldActionMirrorIngress) &&
             (action2 == bcmFieldActionMirrorEgress)) ||
        ((action1 == bcmFieldActionMirrorEgress) &&
             (action2 == bcmFieldActionMirrorIngress))) {
        goto exit;
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_action_map_find(unit, action1,
                                      stage_info, &lt_map_ref1));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_field_action_map_find(unit, action2,
                                      stage_info, &lt_map_ref2));
    /* Iterate through all lt maps */
    for (iter1 = 0; iter1 < (lt_map_ref1->num_maps); iter1++) {
        if ((opaque_object1 != NULL) &&
            (0 != (sal_strcmp((lt_map_ref1->map[iter1]).lt_field_name,
                        opaque_object1)))) {
            continue;
        }
        for (iter2 = 0; iter2 < (lt_map_ref2->num_maps); iter2++) {
            /* for each offset in map1,
             *    check if fieldname matches any map of act 2
             *    check if fieldname matches any internal map of act2
             *
             * for each offset in internal map1,
             *    check if fieldname matches any map of act 2
             *    check if fieldname matches any internal map of act2
             */
            if ((opaque_object2 != NULL) &&
                (0 != (sal_strcmp((lt_map_ref2->map[iter2]).lt_field_name,
                          opaque_object2)))) {
                continue;
            }
            /* if LT field name of both actions is same, go through all the
             * offsets of action1 and check if they conflict with action2
             */
            if (0 == (sal_strcmp((lt_map_ref1->map[iter1]).\
                                       lt_field_name,
                                 ((lt_map_ref2->map[iter2]).\
                                       lt_field_name)))) {
                /* go through each def and check if its offsets conflict */
                for (idx1 = 0;
                     idx1 < (lt_map_ref1->map[iter1]).num_offsets; idx1++) {
                    offset1 = (lt_map_ref1->map[iter1]).offset[idx1];
                    width1 =  (lt_map_ref1->map[iter1]).width[idx1];
                    if (width1 == 0) {
                        *is_action_conflict = true;
                        goto exit;
                    } else {
                        for (idx2 = 0;
                             idx2 < (lt_map_ref2->map[iter2]).num_offsets;
                              idx2++) {
                            offset2 = (lt_map_ref2->map[iter2]).offset[idx2];
                            width2 =  (lt_map_ref2->map[iter2]).width[idx2];
                            ltsw_field_is_offset_conflict(unit,
                                offset1,width1,offset2,width2, is_action_conflict);
            ...
            /* for each Lt map of action1, check if they conflict with
             * internal LT fields of action 2*/
            for (idx1 = 0;
                     idx1 < (lt_map_ref1->map[iter1]).num_offsets; idx1++) {
                    offset1 = (lt_map_ref1->map[iter1]).offset[idx1];
                    width1 =  (lt_map_ref1->map[iter1]).width[idx1];
                for (intr_map=0;
                     intr_map <
                         lt_map_ref2->map[iter2].num_internal_maps;
                     intr_map++) {
                    /* if internal map is matching,
                     * go through each of the offset
                     */
                    if (0 == (sal_strcmp((lt_map_ref1->map[iter1]).\
                                              lt_field_name,
                                 ((lt_map_ref2->map[iter2]).\
                                       internal_map[intr_map]).lt_field_name))) {
                        for (idx3 = 0;
                             idx3 < ((lt_map_ref2->map[iter2].\
                                          internal_map[intr_map])).num_offsets;
                             idx3++) {
                            offset2 =
                                ((lt_map_ref2->map[iter2].\
                                      internal_map[intr_map])).offset[idx3];
                            width2 =
                                ((lt_map_ref2->map[iter2].\
                                              internal_map[intr_map])).width[idx3];
                            ltsw_field_is_offset_conflict(unit,
            ...
            /* for ech internal map of actino1, check if they conflict
             * with Lt fields of action 2
             */
            for (intr_map=0;
                 intr_map <
                     lt_map_ref1->map[iter1].num_internal_maps;
                 intr_map++) {
                if (0 == sal_strcmp(
                             ((lt_map_ref1->map[iter1]).\
                                 internal_map[intr_map]).lt_field_name,
                             ((lt_map_ref2->map[iter2]).lt_field_name))) {
                    /* go through each def and check if its offsets conflict */
                    for (idx1 = 0;
                         idx1 < ((lt_map_ref1->map[iter1]).\
                                  internal_map[intr_map]).num_offsets;
                         idx1++) {
                        offset1 = ((lt_map_ref1->map[iter1]).\
                                    internal_map[intr_map]).offset[idx1];
                        width1 =  ((lt_map_ref1->map[iter1]).\
                                    internal_map[intr_map]).width[idx1];
                        if (width1 == 0) {
                            *is_action_conflict = true;
                            goto exit;
                        } else {
                            for (idx2 = 0;
                                 idx2 < (lt_map_ref2->map[iter2]).num_offsets;
                                 idx2++) {
                                offset2 = (lt_map_ref2->map[iter2]).\
                                           offset[idx2];
                                width2 =  (lt_map_ref2->map[iter2]).\
                                           width[idx2];
                                ltsw_field_is_offset_conflict(unit,
                                    offset1,width1,offset2,width2,
                                    is_action_conflict);
            ...
            /* for ech internal map of actino1, check if they conflict
             * with internal Lt fields of action 2
             */
            for (intr_map=0;
                 intr_map <
                     lt_map_ref1->map[iter1].num_internal_maps;
                 intr_map++) {
                /* go through the internal maps of act 2 */
                for (intr_map2=0;
                     intr_map2 <
                        lt_map_ref2->map[iter2].num_internal_maps;
                     intr_map2++) {
                    if (0 == (sal_strcmp(
                              ((lt_map_ref1->map[iter1]).\
                                   internal_map[intr_map]).lt_field_name,
                              ((lt_map_ref2->map[iter2]).\
                                   internal_map[intr_map2]).lt_field_name))) {
                        value1 =  ((lt_map_ref1->map[iter1]).\
                                    internal_map[intr_map]).value;
                        num_offset1 = (((lt_map_ref1->map[iter1].\
                                         internal_map[intr_map])).num_offsets);
                        for (idx3 = 0;
                             idx3 < (((lt_map_ref1->map[iter1].\
                                       internal_map[intr_map])).num_offsets);
                             idx3++) {
                            offset1 = ((lt_map_ref1->map[iter1]).\
                                        internal_map[intr_map]).offset[idx3];
                            width1 =  ((lt_map_ref1->map[iter1]).\
                                        internal_map[intr_map]).width[idx3];
                            value2 = ((lt_map_ref2->map[iter2].\
                                       internal_map[intr_map2])).value;
                            num_offset2 = (((lt_map_ref2->map[iter2].\
                                             internal_map[intr_map2])).\
                                           num_offsets);
                            for (idx4 = 0;
                                 idx4 < (((lt_map_ref2->map[iter2].\
                                           internal_map[intr_map2])).\
                                             num_offsets);
                                 idx4++) {
                                offset2 =
                                ...
                                if (*is_action_conflict) {
                                    /*
                                     * Some actions have the same internal map
                                     * LT field and need to be configured with
                                     * same value. Such actions shouldn't be
                                     * considered as conflicting.
                                     */
                                    if ((offset1 == offset2) &&
                                        (width1 == width2) &&
                                        (num_offset1 == num_offset2) &&
                                        (value1 == value2)) {
                                        *is_action_conflict = false;
                                    } else {
                                        goto exit;
                                    ...
                                    bcm_field_action_match_config_t *match_config)
{
    ...
    bcmlt_entry_handle_t entry_template_lkup_hdl = BCMLT_INVALID_HDL;
    bool is_action_conflict = false;
    int indx = 0;
    ...
    BCMINT_FIELD_MEM_ALLOC
        (fld_defs_ref, sizeof(bcmlt_field_def_t) * lt_map_ref->num_maps,
                                                    "field definitions");
    /* check if the action is conflicting with other configured actions */

    for (indx = 0; indx < entry_oper->num_actions; indx++) {
        if (entry_oper->action_arr[indx] != action) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_is_action_conflict(unit, entry,
                     action, entry_oper->action_arr[indx],
                     NULL, &is_action_conflict));
            if (is_action_conflict) {
    ...
    if (entry_oper->flexctr_action_id) {
        if (stage_info->flags & BCMINT_FIELD_STAGE_ACTION_RESOLUTION_SBR) {
            ...
            /*  place holder for adding conflict check for colored actions
             */
            if (entry_oper->counter_color == 0) {
                SHR_IF_ERR_VERBOSE_EXIT
                   (bcmint_field_is_action_conflict(unit, entry,
    ...
    /* Check action value is not greater than width of action programmed
     * in config.yml */
    if (lt_map_action_width < 64) {
        if (*act_value_inp_buf > (((uint64_t)1 << lt_map_action_width) - 1) ) {
            LOG_DEBUG(BSL_LS_BCM_FP,
               (BSL_META_U(unit, "Error: action param value width %d exceeds"
                   " max, check param value passed.\n"),lt_map_action_width));
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
    } else {
        if (*act_value_inp_buf > ((uint64_t)-1)) {
            LOG_DEBUG(BSL_LS_BCM_FP,
               (BSL_META_U(unit, "Error: action param value width exceeds max"
                                " 64b allowed, check param value passed.\n")));
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
    }
    ...
    if (old_action_profile_idx != 0) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (mbcm_ltsw_field_action_profile_del(unit,
                                                stage_info->stage_id,
                                                action,
                                                &old_action_prof_val));
    }
    ...
    if (entry_oper->entry_flags & BCMINT_FIELD_ENTRY_INSTALLED) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_allocate(dunit,
                                  ...
                                  &entry_template_lkup_hdl));

        /* Add entry_id field to Entry LT. */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(entry_template_lkup_hdl,
         ...
         */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_entry_commit(unit, entry_template_lkup_hdl,
                                  BCMLT_OPCODE_LOOKUP,
                                  BCMLT_PRIORITY_NORMAL));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_symbol_get(entry_template_lkup_hdl,
                                          "OPERATIONAL_STATE",
                                          &oper_stat));
...
exit:
    ...
    if (BCMLT_INVALID_HDL != entry_template_lkup_hdl) {
        (void) bcmlt_entry_free(entry_template_lkup_hdl);
    }
    ...
    if ((stage_info->flags & BCMINT_FIELD_STAGE_ENTRY_COLOR_STAT_SUPPORT) &&
        ...
        for (a = 0; a < entry_oper->num_actions; a++) {
            if ((entry_oper->action_arr[a] == bcmFieldActionGpStatGroup) ||
                (entry_oper->action_arr[a] == bcmFieldActionYpStatGroup) ||
                (entry_oper->action_arr[a] == bcmFieldActionRpStatGroup)) {
                color_set +=1;
            }
        }
    ...
    *action_cnt = 0;
    ...
    for (idx = 0; idx <= action_db_cnt; idx++) {
        ...
        for (iter = 0; iter < ltmap_tmp->num_maps; iter++) {
            sbr_fid_name = ltmap_tmp->map[iter].lt_field_name;
            if (ltmap_tmp->map[iter].policer == true) {
                sbr_fid_name = ltmap_tmp->map[iter].lt_sbr_field_name;
            }
            if (sal_strcmp(lt_field_name, sbr_fid_name) == 0) {
    ...
    uint8_t *action_size)
{
    ...
    SHR_NULL_CHECK(cont_id_arr, SHR_E_PARAM);
    SHR_NULL_CHECK(cont_size_arr, SHR_E_PARAM);
    SHR_NULL_CHECK(actual_ct, SHR_E_PARAM);
    SHR_NULL_CHECK(action_size, SHR_E_PARAM);
...
bcmint_field_action_flexctr_internal_color_set(
        int unit,
        bcmlt_entry_handle_t *policy_template,
        bcmint_field_stage_info_t *stage_info,
        bcmi_field_ha_entry_oper_t *entry_oper,
        bcmi_field_action_t action,
        uint32_t val)
{
    ...
    uint64_t val64 = val;
    const bcm_field_action_map_t *lt_map_ref = NULL;
    bcmlt_entry_handle_t policy_handle = BCMLT_INVALID_HDL;
    SHR_FUNC_ENTER(unit);
    ...
    for (iter = 0; iter < lt_map_ref->num_maps; iter++) {
        if (policy_template) {
            /* Add color field to Policy LT. */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(*policy_template,
                                       (lt_map_ref->map[iter]).lt_field_name,
                                       val64));
        } else {
            /* Set Color fields in the policy table */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_field_lt_entry_commit(unit, &policy_handle,
                                              BCMLT_OPCODE_UPDATE,
                                              BCMLT_PRIORITY_NORMAL,
                                              stage_info->tbls_info->policy_sid,
                                              stage_info->tbls_info->policy_key_fid,
                                              0, entry_oper->entry_id, NULL,
                                              (char *)(lt_map_ref->map[iter]).lt_field_name,
                                              val64,
                                              0, 0,
                                              0));
        }
...
exit:
    if (policy_handle != BCMLT_INVALID_HDL) {
        (void) bcmlt_entry_free(policy_handle);
    }
