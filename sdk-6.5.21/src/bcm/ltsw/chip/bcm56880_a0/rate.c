...
SHR_BITDCLNAME(midx_bmp[RATE_PIPE_ID_MAX], RATE_SC_METER_ID_MAX);
    ...
    uint32_t *sc_id)
{
    ...
    for (i = RATE_SC_METER_ID_ALL_PORT + 1;
        ...
        if (SHR_BITGET(midx_bmp[pipe], i)) continue;
        SHR_BITSET(midx_bmp[pipe], i);
        *sc_id = i;
        return SHR_E_NONE;
    }
...
bcm56880_a0_ltsw_rate_init(int unit)
{
    ...
    for (pipe = 0; pipe < RATE_PIPE_ID_MAX; pipe++) {
        for (sc_id = 0; sc_id < RATE_SC_METER_ID_MAX; sc_id++) {
            if (sc_id == RATE_SC_METER_ID_DEFAULT ||
                sc_id == RATE_SC_METER_ID_ALL_PORT) {
                SHR_BITSET(midx_bmp[pipe], sc_id);
            } else {
                SHR_BITCLR(midx_bmp[pipe], sc_id);
            }
        }
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (xfs_ltsw_rate_ing_pipeunique_get(unit, &unique));
    for (port = 0; port < RATE_PORT_ID_MAX; port++) {
        if (bcmi_ltsw_port_gport_validate(unit, port, &gport) == SHR_E_NONE) {
            ...
            pipe = unique ? bcmi_ltsw_dev_logic_port_pp_pipe(unit, port) : 0;
            SHR_BITSET(midx_bmp[pipe], sc_id);
        }
    }
exit:
    ...
    bcmint_rate_info_t *rate_info)
{
    ...
    if (rate_info->all_port) {
        ...
        if (sc_id != RATE_SC_METER_ID_DEFAULT &&
            ...
            bcm56880_a0_ltsw_rate_free_midx(unit, pipe, sc_id);
        }
        ...
        if (sc_id == RATE_SC_METER_ID_DEFAULT ||
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm56880_a0_ltsw_rate_alloc_midx(unit, pipe, &sc_id));
        }
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (xfs_ltsw_rate_meter_set(unit, pipe, sc_id, rate_info));

    /* Get enable status of the storm ctrl id. */
    SHR_IF_ERR_VERBOSE_EXIT
        (xfs_ltsw_rate_status(unit, pipe, sc_id, &tmp_hw_info));
...
bcm56880_a0_ltsw_rate_get(
    ...
    bcmint_rate_info_t *rate_info)
{
    uint32_t pipe, sc_id = RATE_SC_METER_ID_DEFAULT;
    int unique;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xfs_ltsw_rate_meter_get(unit, pipe, sc_id, rate_info));
