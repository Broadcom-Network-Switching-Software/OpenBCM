...
#define EP_RECIRC_PROFILE_MOD_RESERVED_ID_1 1

/*! 2nd reserved EP Recirculate profile ID for MOD. */
#define EP_RECIRC_PROFILE_MOD_RESERVED_ID_2 2
    ...
    bcm_switch_ep_recirc_profile_t *recirc_profile)
{
    ...
    if (recirc_profile->flags & BCM_SWITCH_EP_RECIRC_PROFILE_WITH_ID) {
        ...
        /* Check profile id. Profile 0 is reserved for no EP recirculate.*/
    ...
    if (recirc_profile->nih_cfg.opaque_ctrl_id_c < 0 ||
        recirc_profile->nih_cfg.opaque_ctrl_id_c > 15) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Range of opaque_ctrl_id_c is 0-15.")));
        SHR_ERR_EXIT(BCM_E_PARAM);
    }
    ...
    int *profile_id)
{
     ...
     */
    if (recirc_profile->flags & BCM_SWITCH_EP_RECIRC_PROFILE_WITH_ID) {
        *profile_id = recirc_profile->recirc_profile_id;
        SHR_EXIT();
    }
    ...
    int profile_id)
{
    ...
    bcmi_lt_field_t field[] =
    {
        ...
        /*38*/ {NIH_HEADER_OPAQUE_PROPERTYs, BCMI_LT_FIELD_F_SET, 0, {0}},
    };
    ...
    field[38].u.val = recirc_profile->nih_cfg.opaque_ctrl_id_c;
    ...
    int mirror_instance_id)
{
    ...
    {
        /*0*/ {MON_REDIRECT_PROFILE_IDs, BCMI_LT_FIELD_F_SET, 0, {0}},
        /*1*/ {NIH_HEADER_OPAQUE_PROPERTYs, BCMI_LT_FIELD_F_SET, 0, {0}},
    };
...
/*!
 * \brief Get the mirror instance id from a MON_REDIRECT_PROFILE LT entry.
 *
 * \param [in] unit Unit Number.
 * \param [in] recirc_profile_id EP Recirculate profile id.
    ...
    int *mirror_instance_id)
{
    ...
    SHR_NULL_CHECK(mirror_instance_id, SHR_E_PARAM);
    ...
    *mirror_instance_id = field[1].u.val;
...
/*!
 * \brief Get the mirror instance ids from MON_REDIRECT_PROFILE LT entries.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
ep_recirc_profile_table_mirror_instance_get_all(
    int unit,
    int *mirror_inst_id_array)
{
    ...
    SHR_NULL_CHECK(mirror_inst_id_array, SHR_E_PARAM);
    ...
    for (i = 0; i < entry_num; i++) {
        ...
        mirror_inst_id_array[i] = field[1].u.val;
    }
 ...
 */
static int
ep_recirc_profile_num_get(int unit, int *num)
{
    uint32_t entry_num;
    ...
    bcm_switch_ep_recirc_profile_t *recirc_profile)
{
    ...
    bcmi_lt_field_t field[] =
    {
        ...
        /*38*/ {NIH_HEADER_OPAQUE_PROPERTYs, BCMI_LT_FIELD_F_GET, 0, {0}},
    };
    ...
    recirc_profile->recirc_profile_id = recirc_profile_id;
    if (field[1].u.val == 1) {
    ...
    recirc_profile->nih_cfg.opaque_ctrl_id_c = field[38].u.val;
    ...
    bcm_switch_ep_recirc_drop_event_control_t *drop_event_ctrl)
{
    ...
    rv = bcmint_xfs_switch_ep_recirc_drop_event_to_symbol
             (unit, drop_event_ctrl->threshold_event, &symbol);

    if (rv == SHR_E_NOT_FOUND) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "Drop event(%d) not supported.\n"),
                                drop_event_ctrl->threshold_event));
...
ep_recirc_drop_event_control_table_get(
    ...
    bcm_switch_ep_recirc_drop_event_control_t *drop_event_ctrl)
{
    bcm_pkt_drop_event_t threshold_event;
    bcmi_lt_entry_t lt_entry;
    bcmi_lt_field_t field[] =
    {
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_xfs_switch_ep_recirc_symbol_to_drop_event(unit,
                                                          field[1].u.sym_val,
                                                          &threshold_event));
    drop_event_ctrl->threshold_event = threshold_event;
    ...
    int *recirc_profile_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ep_recirc_profile_id_allocate(unit, recirc_profile, &profile_id_tmp));

    SHR_IF_ERR_VERBOSE_EXIT
        (ep_recirc_profile_table_set(unit, recirc_profile, profile_id_tmp));

    *recirc_profile_id = profile_id_tmp;
...
static int
bcm56780_a0_switch_ep_recirc_profile_init(int unit)
{
    int mod_enable = 0, ifa_enable = 0, profile_id = 0;
    bcm_switch_ep_recirc_profile_t recirc_profile;

    SHR_FUNC_ENTER(unit);

    mod_enable = bcmi_ltsw_property_get(unit,
                                        BCMI_CPN_FLOWTRACKER_DROP_MONITOR_ENABLE,
                                        0);

    ifa_enable = bcmi_ltsw_property_get(unit, BCMI_CPN_IFA_ENABLE, 0);

    /* Initialize default profile entries for MOD. */
    if (mod_enable) {
        /* Profile entry for IPIPE and MMU drops. */
        bcm_switch_ep_recirc_profile_t_init(&recirc_profile);
        recirc_profile.recirc_profile_id = EP_RECIRC_PROFILE_MOD_RESERVED_ID_1;
        ...
        recirc_profile.nih_cfg.destination_type =
            bcmSwitchEpRecircNihDestTypeFromOpaqueA;

        recirc_profile.nih_cfg.timestamp_select =
            bcmSwitchEpRecircTimestampSelectIngress;

        recirc_profile.nih_cfg.res_time_field_cfg.nih_drop_code_en = 0;
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm56780_a0_switch_ep_recirc_profile_create(unit, &recirc_profile,
                                                         &profile_id));

        /* Profile entry for EPIPE drops. */
        bcm_switch_ep_recirc_profile_t_init(&recirc_profile);
        recirc_profile.recirc_profile_id = EP_RECIRC_PROFILE_MOD_RESERVED_ID_2;
        ...
        recirc_profile.nih_cfg.res_time_field_cfg.nih_drop_code_en = 1;
    ...
    /* Initialize default profile entry for IFA. */
    if (ifa_enable) {
        bcm_switch_ep_recirc_profile_t_init(&recirc_profile);
        recirc_profile.recirc_profile_id = EP_RECIRC_PROFILE_IFA_RESERVED_ID_1;
...
static int
bcm56780_a0_switch_ep_recirc_profile_mirror_instance_get_all(
    int unit,
    int *mirror_inst_id_array)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ep_recirc_profile_table_mirror_instance_get_all(unit,
...
static int
bcm56780_a0_switch_ep_recirc_profile_num_get(
    int unit,
    int *num)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (ep_recirc_profile_num_get(unit, num));
...
static int
bcm56780_a0_switch_ep_recirc_drop_event_min_get(
    int unit,
    bcm_pkt_drop_event_t *drop_event_array,
    int array_size,
    bcm_pkt_drop_event_t *min_drop_event)
{
    int i, drop_code, min_drop_code = -1;

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(min_drop_event, SHR_E_PARAM);
    SHR_NULL_CHECK(drop_event_array, SHR_E_PARAM);

    if ((array_size < 1) ||
        (array_size > bcmPktDropEventCount)) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    for (i = 0; i < array_size; i++) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_xfs_switch_ep_recirc_drop_event_to_drop_code
                 (unit, drop_event_array[i], &drop_code));

        if (min_drop_code == -1 || drop_code < min_drop_code) {
            min_drop_code = drop_code;
            *min_drop_event = drop_event_array[i];
        }
    }
...
static mbcm_ltsw_sc_drv_t bcm56780_a0_sc_drv = {
    ...
    .switch_ep_recirc_profile_init = bcm56780_a0_switch_ep_recirc_profile_init,
    ...
    .switch_ep_recirc_profile_mirror_instance_get_all = bcm56780_a0_switch_ep_recirc_profile_mirror_instance_get_all,
    .switch_ep_recirc_profile_num_get = bcm56780_a0_switch_ep_recirc_profile_num_get,
    ...
    .switch_ep_recirc_drop_event_min_get = bcm56780_a0_switch_ep_recirc_drop_event_min_get,
