...
#define L2_IF_SIZE          2048                /* From NPL ing_l2_iif_table */
#define L2_IF_BLACK_HOLE    L2_IF_SIZE - 1

#define MAX_PORT_GROUP      L2_IF_SIZE - 1
#define MAX_DVP_GROUP       16383               /* Max entry of ING_DVP_TABLE */
...
#define MAX_NUM_PORT        80
#define MAX_NUM_FP_PM       20                  /* 20 PM8x50 */
#define MAX_NUM_PHY_PER_PM  8
#define MAX_NUM_FP_PHY      160                 /* 20 PM8x50 x 8 ports */

#define BCM56780_A0_MAX_AN_ABILITY  20


static const ltsw_port_map_t bcm56780_fixed_port_map[] = {
    /* CPU */
    {0,     0, 0,   BCMI_LTSW_PORT_TYPE_CPU},
    /* LB */
    {39,    1, 163, BCMI_LTSW_PORT_TYPE_LB},
    {79,    3, 164, BCMI_LTSW_PORT_TYPE_LB},
    /* MGMT */
    {38,    1, 161, BCMI_LTSW_PORT_TYPE_MA | BCMI_LTSW_PORT_TYPE_XL, FLEXIBLE | NO_REMAP},
    {78,    3, 162, BCMI_LTSW_PORT_TYPE_MA | BCMI_LTSW_PORT_TYPE_XL, FLEXIBLE | NO_REMAP},
    /* RDB */
    {19,    0, 165, BCMI_LTSW_PORT_TYPE_RDB},
    {59,    2, 166, BCMI_LTSW_PORT_TYPE_RDB},
    /* spare */
    {58,    2, -1,  BCMI_LTSW_PORT_TYPE_SPARE},
    {0},
};

static const bcmi_ltsw_port_resource_spec_t bcm56780_port_resource_spec[BCMI_PORT_RESOURCE_CNT] = {
    [BCMI_PORT_GROUP_ING]   = {0, L2_IF_SIZE - 1},
    [BCMI_PORT_GROUP_EGR]   = {0, MAX_DVP_GROUP},
    [BCMI_PORT_GROUP_L2_IF] = {0, L2_IF_SIZE - 1},
    [BCMI_PORT_GROUP_DVP]   = {L2_IF_SIZE, MAX_DVP_GROUP},
};
...
static const chip_spec_t bcm56780_chip_spec = {
    bcm56780_port_resource_spec,
    bcm56780_fixed_port_map,
    0x0,
    0,
};
...
const ltsw_port_fs_ability_table_entry_t_t bcm56780_a0_fs_ability_table[] =
{
 ...
 * and their VCO rates. */
const ltsw_port_an_ability_table_entry_t bcm56780_a0_an_ability_table[] =
{
...
static int
bcm56780_a0_fs_ability_table_vco_get(
    uint32 speed,
    uint32 num_lanes,
    ...
    ltsw_port_vco_type_t* vco)
{
    ...
    entry_num = sizeof(bcm56780_a0_fs_ability_table) /
                sizeof(ltsw_port_fs_ability_table_entry_t_t);

    *vco = ltswPortVCOInvalid;
    for (i = 0; i < entry_num; i++) {
        if (bcm56780_a0_fs_ability_table[i].speed == speed &&
            bcm56780_a0_fs_ability_table[i].num_lanes == num_lanes &&
            bcm56780_a0_fs_ability_table[i].fec_type == fec_type) {
            *vco = bcm56780_a0_fs_ability_table[i].vco;
            break;
        }
    }
...
static int
bcm56780_a0_an_ability_table_vco_get(
    uint32 speed,
    uint32 num_lanes,
    bcm_port_phy_fec_t fec_type,
    ...
    ltsw_port_vco_type_t* vco)
{
    ...
    entry_num = sizeof(bcm56780_a0_an_ability_table) /
                sizeof(ltsw_port_an_ability_table_entry_t);

    *vco = ltswPortVCOInvalid;
    for (i = 0; i < entry_num; i++) {
        if (bcm56780_a0_an_ability_table[i].speed == speed &&
            bcm56780_a0_an_ability_table[i].num_lanes == num_lanes &&
            bcm56780_a0_an_ability_table[i].fec_type == fec_type &&
            bcm56780_a0_an_ability_table[i].an_mode == an_mode) {
            *vco = bcm56780_a0_an_ability_table[i].vco;
            break;
        }
    }
...
static int
bcm56780_a0_manage_port_vco_get(
    int unit,
    int port,
 ...
 */
static int
bcm56780_a0_ltsw_flexport_pm_operation_adjust(
    int unit,
    int nport,
    ...
    ltsw_flexport_port_validation_resolve_t *validate)
{
    ...
    bcm_port_speed_ability_t an_ability[BCM56780_A0_MAX_AN_ABILITY];
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_vco_get(unit, port_cfg_array[0]->pmid, &pll_num,
     ...
     * If port is set as FLEXPORT_OP_DETACH in flexport operation,
     * always tell Port module to delete it.
     * FLEXPORT_OP_DETACH indicats the port need to be deleted or
     * to be modified.
     * FLEXPORT_FLG_NO_REMAP indicats only speed/FEC change,
     * not flexport operation.
    ...
    if (is_flexport_operation) {
        ...
        if (current_ovco == ltswPortVCONone) {
            ...
            if (required_vco_num == 1) {
                ...
                if ((required_vco[0] == current_tvco) ||
                    (required_vco[1] == current_tvco)) {
             ...
             */
            if (required_vco_num == 1) {
                /* Only requires one VCO, check OVCO if need to change. */
                tvco_need_change = 0;
                if ((required_vco[0] == current_tvco) ||
                    (required_vco[0] == current_ovco)) {
                ...
                /*
                 * There are only three potential VCO values (20/25/26) for
                 * PLL0 and PLL1. So there must be one required VCO which
                 * equals one of current VCOs in this case.
                 * When one of required VCOs doesn't equal OVCO, the other
                 * required VCO must equal TVCO and vice versa.
                 */
                if((required_vco[0] != current_ovco) &&
                   (required_vco[1] != current_ovco)){
                    /* One of required VCOs doesn't equal OVCO. */
                    tvco_need_change = 0;
                    ovco_need_change = 1;
                } else if ((required_vco[0] != current_tvco) &&
                           (required_vco[1] != current_tvco)) {
                    ...
                    /*
                     * One of required VCOs equals OVCO,
                     * and the other equals TVCO.
                     */
                    tvco_need_change = 0;
                    ovco_need_change = 0;
                }
        ...
        for (i = 0; i < nport; i++) {
            if (tvco_need_change) {
                /*
                 * If tvco need to change, delete all ports during
                 * speed/FEC change
                 */
                port_cfg_array[i]->op |= FLEXPORT_OP_PC_RESET;
            } else if (ovco_need_change) {
                /*
                 * If ovco need to change, delete ports on OVCO
                 * during speed/FEC change.
                 */
                if (port_cfg_array[i]->op & FLEXPORT_OP_DETACH) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcm56780_a0_fs_ability_table_vco_get(
                            port_cfg_array[i]->speed, port_cfg_array[i]->lanes,
                            port_cfg_array[i]->fec, &vco));
                    if ((port_cfg_array[i]->speed == 40000 && port_cfg_array[i]->lanes == 4)
                        || (port_cfg_array[i]->speed == 10000 && port_cfg_array[i]->lanes == 1)) {
         ...
         * Delete the old ports which FEC type is RS272 and FEC type of other
         * ports in one MPP are RS544, or old port FEC type is RS544 and
         * FEC type of other port in one MPP are RS272.
         */
        BCM_PBMP_CLEAR(reset_ports);
        for (i = 0 ; i < nport; i++) {
            if (port_cfg_array[i]->op & FLEXPORT_OP_DETACH) {
                ...
                if (an) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmint_ltsw_port_autoneg_ability_advert_get(
                            unit, port, BCM56780_A0_MAX_AN_ABILITY,
                    ...
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmint_ltsw_port_resource_get(unit,
                                                       port,
                                                       &speed_resource));
                    /* Set up fec bmp. */
                ...
                if (((current_rs272_bmp & 0xf) &&
                     ((new_rs544_bmp & (~lane_mask)) & 0xf)) ||
                    ((current_rs272_bmp & 0xf0) &&
                     ((new_rs544_bmp & (~lane_mask)) & 0xf0)) ||
                    ((current_rs544_bmp & 0xf) &&
                     ((new_rs272_bmp & (~lane_mask)) & 0xf)) ||
                    ((current_rs544_bmp & 0xf0) &&
                     ((new_rs272_bmp & (~lane_mask)) & 0xf0))) {
 ...
 */
static int
bcm56780_a0_ltsw_flexport_pm_resource_validate(
    int unit,
    bcm_pbmp_t *current_ports,
    int nport,
    ltsw_flexport_port_cfg_t **port_cfg_array,
    ltsw_flexport_port_validation_resolve_t *validate)
{
    ...
    ltsw_port_vco_type_t required_vco[2] = {ltswPortVCOInvalid,
                                            ltswPortVCOInvalid};
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_vco_get(unit, pm_id, &pll_num,
                                  &current_tvco, &current_ovco));
    if (pll_num != 2) {
        LOG_ERROR(BSL_LS_BCM_PORT,
            (BSL_META_U(unit, "PLL get fail.\n")));
    ...
    if (pm_type == ltswPmTypePm4x10 &&
        !(port_cfg_array[0]->flags & FLEXPORT_FLG_NO_REMAP)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
    ...
    for (i = 0; i < nport; i++) {
        ...
        if(port_cfg_array[i]->op & FLEXPORT_OP_DETACH) {
            ...
            lane_mask = 0;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_ltsw_pport_lane_mask_get(
                    unit, port_cfg_array[i]->pport,
            ...
            if (vco == ltswPortVCOInvalid) {
                LOG_ERROR(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit, "port %d: speed[%d] lane[%d] fec[%d]is "
                                      "not supported.\n"),
                     port_cfg_array[i]->lport, port_cfg_array[i]->speed,
                     port_cfg_array[i]->lanes, port_cfg_array[i]->fec));
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_CONFIG);
            }

            /* Record the port index for 1x20/2x20. */
            if (vco == ltswPortVCO20P625G) {
                if (!((port_cfg_array[i]->speed == 10000 &&
                       port_cfg_array[i]->lanes == 1) ||
                      (port_cfg_array[i]->speed == 40000 &&
                       port_cfg_array[i]->lanes == 4))) {
                    if (vco_20g_index == -1) {
                        vco_20g_index = i;
                    }
                }
    ...
    if ((remain_ports_lane_bmp & new_ports_lane_bmp) != 0) {
        LOG_ERROR(BSL_LS_BCM_PORT,
            (BSL_META_U(unit, "The new ports take over the lane of remaining "
                              "ports overlap lane: 0x%x\n"),
    ...
    BCM_PBMP_ITER(remain_ports, port) {
        ...
        if (an) {
            ...
            for (i = 0; i < an_ability_num; i++) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcm56780_a0_an_ability_table_vco_get(
                        an_ability[i].speed, an_ability[i].resolved_num_lanes,
                        an_ability[i].fec_type, an_ability[i].an_mode, &vco));
                if ((an_ability[i].speed == 10000 &&
                     an_ability[i].resolved_num_lanes == 1) ||
                    (an_ability[i].speed == 40000 &&
                     an_ability[i].resolved_num_lanes == 4)) {
                    if (current_ovco != ltswPortVCO20P625G) {
                        vco = ltswPortVCO25P781G;
                    }
                }
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm56780_a0_fs_ability_table_vco_get(
                    speed_resource.speed, speed_resource.lanes,
    ...
    if ((required_vco_num == 1) && tvco_is_free &&
        (required_vco[0] == ltswPortVCO20P625G)) {
        SHR_EXIT();
    }
     ...
     * If there are 4x10/1x10 and 1x20/2x20 ports, the 1x20/2x20 port is
     * inserted before 4x10/1x10.
     */
    if ((required_vco[0] == ltswPortVCO20P625G) &&
    ...
    for (i = 0; i < required_vco_num; i++) {
        /*
         * If the TVCO is in use, check if the required VCO
         * is equal to the current TVCO.
         */
        if (!tvco_is_free) {
        ...
        /*
         * If the OVCO is in use, check if the required VCO
         * is equal to the current OVCO.
         */
        if (!ovco_is_free) {
        ...
        /*
         * Check if the required VCOs can be accommodated
         * when the TVCO or OVCO is free.
         */
 ...
 */
static int
bcm56780_a0_ltsw_flexport_pm_validate(
    int unit,
    bcm_pbmp_t *current_ports,
    int nport,
    ltsw_flexport_port_cfg_t **port_cfg_array)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT(
        bcm56780_a0_ltsw_flexport_pm_resource_validate(
            unit, current_ports, nport, port_cfg_array, &validate));

    SHR_IF_ERR_VERBOSE_EXIT(
        bcm56780_a0_ltsw_flexport_pm_operation_adjust(
            unit, nport, port_cfg_array, &validate));
 ...
 */
static int
bcm56780_a0_ltsw_port_speed_config_validate(
    int unit,
    bcm_port_resource_t *resource,
    bcm_pbmp_t *affect_ports)
{
    ...
    if (num_lane != resource->lanes) {
        LOG_ERROR(BSL_LS_BCM_PORT,
            (BSL_META_U(unit, "Invalid lane number request on exising "
                              "logical port.\n")));
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }
    ...
    if (pm_type == ltswPmTypePm4x10) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm56780_a0_manage_port_vco_get(unit, resource->port,
                                             resource, &vco));
        if (vco != current_tvco) {
            ...
            if (BCM_PBMP_IS_NULL(port_pbmp)) {
                ...
                LOG_ERROR(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit, "VCO change is required for management "
                                      "port.\n")));
    ...
    /* VCO check. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_fs_ability_table_vco_get(resource->speed, resource->lanes,
                                              resource->fec_type, &vco));
    if (vco == ltswPortVCOInvalid) {
        LOG_ERROR(BSL_LS_BCM_PORT,
            (BSL_META_U(unit, "port %d: speed[%d] lane[%d] fec[%d]is not "
                              "supported.\n"),
                        resource->port, resource->speed,resource->lanes,
                        resource->fec_type));
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_CONFIG);
    }
...
/*!
 * \brief Init BCM56780 specific information for a port.
 *
 * \param [in] unit Unit number.
 *
 * \return SHR_E_NONE on success and error code otherwise.
 */
static int
bcm56780_a0_ltsw_port_info_init_per_port(int unit, int port)
{
    int id;
...
/*!
 * \brief Init BCM56780 specific information.
 *
 * \param [in] unit Unit number.
 *
 * \return SHR_E_NONE on success and error code otherwise.
 */
static int
bcm56780_a0_ltsw_port_info_init(int unit)
{
    ...
    chip_spec[unit] = &bcm56780_chip_spec;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_mgmt_init(unit, L2_IF_SIZE));

    if (!bcmi_warmboot_get(unit)) {
        /* reserve for local ports and entry 0 */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_l2_if_reserve(unit, 0, MAX_NUM_PORT + 1));
        /* reserve for black hole */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_port_reserve(unit, BCM_GPORT_BLACK_HOLE));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_l2_if_reserve(unit, L2_IF_BLACK_HOLE, 1));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_id_l2_if_map_port
                (unit, L2_IF_BLACK_HOLE, BCM_GPORT_BLACK_HOLE));
    }
...
/*!
 * \brief Deinit BCM56780 specific information.
 *
 * \param [in] unit Unit number.
 *
 * \return SHR_E_NONE on success and error code otherwise.
 */
static int
bcm56780_a0_ltsw_port_info_deinit(int unit)
{
    SHR_FUNC_ENTER(unit);
...
/*!
 * \brief Recover BCM56780 specific information.
 *
 * \param [in] unit Unit number.
 *
 * \return SHR_E_NONE on success and error code otherwise.
 */
static int
bcm56780_a0_ltsw_port_info_recover(int unit)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_mgmt_recover(unit));

    SHR_IF_ERR_VERBOSE_EXIT
        (xfs_ltsw_port_info_recover(unit));
...
static int
bcm56780_a0_ltsw_port_profile_init_per_port(int unit, int port)
{
    SHR_FUNC_ENTER(unit);
...
static int
bcm56780_a0_ltsw_port_profile_deinit_per_port(int unit, int port)
{
    SHR_FUNC_ENTER(unit);
...
static int
bcm56780_a0_ltsw_port_profile_init(int unit)
{
    SHR_FUNC_ENTER(unit);
...
static int
bcm56780_a0_ltsw_port_profile_deinit(int unit)
{
    SHR_FUNC_ENTER(unit);
 ...
 */
static int
bcm56780_a0_ltsw_port_service_init_per_port(int unit, int port)
{
    int fields_num_max, fields_num;
    int modid, l2_if, sys_port, pp_pipe, pp_port, strength, rv;
    bcm_vlan_t def_vlan;
    uint64_t u64;
    uint32_t *data = NULL;
    bcmi_ltsw_port_tab_t *types = NULL;
    ...
    if (IS_CPU_PORT(unit, port)) {
        /*
         * Entry 0 is not suggested to use, so system port and
         * L2_IIF/OIF of CPU are changed from 0 to port_max
         * (The minimal value that above all non-CPU ports)
         */
        sys_port = (modid << 8) | PORT_NUM_MAX(unit);
        l2_if = PORT_NUM_MAX(unit);
    } else {
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_reserve(unit, port));
    SHR_IF_ERR_VERBOSE_EXIT
    ...
    fields_num = 0;
    SHR_ALLOC(types,
        sizeof(bcmi_ltsw_port_tab_t) * fields_num_max, "port_service_init_type");
    SHR_NULL_CHECK(types, SHR_E_MEMORY);
    sal_memset(types, 0, sizeof(bcmi_ltsw_port_tab_t) * fields_num_max);

    SHR_ALLOC(data,
        sizeof(uint32_t) * fields_num_max, "port_service_init_data");
    SHR_NULL_CHECK(data, SHR_E_MEMORY);
    sal_memset(data, 0, sizeof(uint32_t) * fields_num_max);

    /* port type */
    types[fields_num] = BCMI_PT_ING_PORT_TYPE;
    data[fields_num] = PORT_TYPE_ETHERNET;
    fields_num ++;
    types[fields_num] = BCMI_PT_EGR_PORT_TYPE;
    data[fields_num] =
        IS_CPU_PORT(unit, port) ? PORT_TYPE_CPU :
        (IS_LB_PORT(unit, port) ? PORT_TYPE_LOOPBACK : PORT_TYPE_ETHERNET);
    fields_num ++;
    types[fields_num] = BCMI_PT_EGR_ING_PORT_TYPE;
    data[fields_num] =
        IS_CPU_PORT(unit, port) ? PORT_TYPE_CPU : PORT_TYPE_ETHERNET;
    fields_num ++;

    /* port parser type */
    types[fields_num] = BCMI_PT_PORT_PARSER_TYPE;
    data[fields_num] =
        IS_LB_PORT(unit, port) ? PARSER_TYPE_LOOPBACK : PARSER_TYPE_ETHERNET;
    fields_num ++;

    /* default VLAN */
    rv = bcm_ltsw_vlan_default_get(unit, &def_vlan);
    if (SHR_FAILURE(rv)) {
        def_vlan = BCM_VLAN_DEFAULT;
    }
    types[fields_num] = BCMI_PT_DEFAULT_VFI;
    data[fields_num] = def_vlan;
    fields_num ++;

    /* VLAN map to VFI */
    types[fields_num] = BCMI_PT_VLAN_MAP_TO_VFI;
    data[fields_num] = 1;
    fields_num ++;

    /* only preserve itag */
    types[fields_num] = BCMI_PT_VLAN_PRESERVE;
    data[fields_num] = 0x2;
    fields_num ++;

    /* learn and forward */
    types[fields_num] = BCMI_PT_CML_NEW;
    data[fields_num] = 0x4;
    fields_num ++;

    types[fields_num] = BCMI_PT_CML_MOVE;
    data[fields_num] = 0x4;
    fields_num ++;

    /* drop static move */
    types[fields_num] = BCMI_PT_STATIC_MOVE_DROP;
    data[fields_num] = 1;
    fields_num ++;

    /* egress pruning */
    types[fields_num] = BCMI_PT_EGR_L2_PROCESS_CTRL;
    data[fields_num] = 0x2;
    fields_num ++;
    ...
    /* ifp pbmp index */
    types[fields_num] = BCMI_PT_IFP_PBM_INDEX;
    data[fields_num] = pp_port;
    fields_num ++;

    /* flex stats for LB-ECN */
    types[fields_num] = BCMI_PT_EGR_OPAQUE_OBJ0;
    data[fields_num] = (pp_pipe << 6) | (pp_port & 0x3F);
    fields_num ++;

    /* VLAN PHB strength */
    types[fields_num] = BCMI_PT_PHB_VLAN_STR;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_sbr_fld_value_get
            (unit, BCMI_LTSW_SBR_TH_EGR_L2_OIF,
             BCMI_LTSW_SBR_FT_DOT1P_PTR, &strength));
    data[fields_num] = strength;
    fields_num ++;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_set(unit, port, types, data, fields_num));

    /* internal loopback */
    u64 = IS_LB_PORT(unit, port) ? 1 : 0;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_array_set(unit, port, BCMI_PT_LOOPBACK_EN,
                                        &u64, port, 1));
    /* cpu */
    u64 = IS_CPU_PORT(unit, port) ? 1 : 0;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_array_set(unit, port, BCMI_PT_CPU_EN,
                                        &u64, port, 1));
 ...
 */
static int
bcm56780_a0_ltsw_port_service_deinit_per_port(int unit, int port)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_id_port_free(unit, port));
 ...
 */
static int
bcm56780_a0_ltsw_port_service_init(int unit)
{
    int port;
    ...
    PORT_ALL_ITER(unit, port) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm56780_a0_ltsw_port_service_init_per_port(unit, port));
    }
 ...
 */
static int
bcm56780_a0_ltsw_port_service_deinit(int unit)
{
    int port;
    ...
    PORT_ALL_ITER(unit, port) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm56780_a0_ltsw_port_service_deinit_per_port(unit, port));
    }
 ...
 */
static int
bcm56780_a0_ltsw_port_misc_init(int unit)
{
    SHR_FUNC_ENTER(unit);
 ...
 */
static int
bcm56780_a0_ltsw_port_egr_shaper_validate(
 ...
 */
static int
bcm56780_a0_ltsw_port_resource_spec_get(
    int unit,
 ...
 */
static int
bcm56780_a0_ltsw_port_class_validate(int unit, bcm_port_t *port,
 ...
 */
static int
bcm56780_a0_ltsw_port_group_range_get(int unit, bcm_port_group_type_t type,
 ...
 */
static int
bcm56780_a0_ltsw_flexport_lane_validate(
    int unit,
 ...
 */
static int
bcm56780_a0_ltsw_flexport_speed_validate(
    int unit,
    ltsw_flexport_resource_t *resource)
{
 ...
 */
static int
bcm56780_a0_ltsw_flexport_encap_validate(
    int unit,
    ltsw_flexport_resource_t *resource)
{
...
static int
bcm56780_a0_ltsw_pm_port_rearrange(
    int unit,
    int nport,
    ltsw_flexport_port_cfg_t **port_cfg_array)
{
 ...
 */
static int
bcm56780_a0_ltsw_flexport_rearrange(
    int unit,
    ltsw_flexport_resource_t *resource)
{
    ...
    for (i = 0, pm = resource->pm; i < num_pm; i ++, pm ++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm56780_a0_ltsw_pm_port_rearrange(unit, pm->num_op,
                                                pm->operation));
    }
 ...
 */
static int
bcm56780_a0_ltsw_flexport_input_validate(
    int unit,
    ltsw_flexport_resource_t *resource)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_ltsw_flexport_lane_validate(unit, resource));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_ltsw_flexport_speed_validate(unit, resource));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_ltsw_flexport_encap_validate(unit, resource));
 ...
 */
static int
bcm56780_a0_ltsw_flexport_resource_validate(
    int unit,
    ltsw_flexport_resource_t *resource)
{
    int i;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_ltsw_flexport_rearrange(unit, resource));
 ...
 */
static int
bcm56780_a0_ltsw_flexport_resource_attach(
    int unit,
    ltsw_flexport_resource_t *resource)
{
    ...
    for (i = 0, po = resource->operation; i < num_op; i ++, po ++) {
        ...
        if (po->op & (FLEXPORT_OP_ADD | FLEXPORT_OP_REMAP)) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm56780_a0_ltsw_port_service_init_per_port(unit, port));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm56780_a0_ltsw_port_profile_init_per_port(unit, port));
            SHR_IF_ERR_VERBOSE_EXIT
 ...
 */
static int
bcm56780_a0_ltsw_flexport_resource_detach(
    int unit,
    ltsw_flexport_resource_t *resource)
{
    int i, num_op = resource->num_op, port;
    ...
    for (i = 0, po = resource->operation; i < num_op; i ++, po ++) {
        ...
        if (po->op & (FLEXPORT_OP_DEL | FLEXPORT_OP_REMAP)) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm56780_a0_ltsw_port_profile_deinit_per_port(unit, port));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm56780_a0_ltsw_port_service_deinit_per_port(unit, port));
        }
 ...
 */
static int
bcm56780_a0_ltsw_flexport_resource_status_update(
    int unit,
    ltsw_flexport_resource_t *resource)
{
    ...
    rv = bcmi_ltsw_field_flex_port_update(unit, del_pbm, add_pbm);
    if (rv == SHR_E_NONE) {
        SHR_EXIT();
    } else {
        SHR_ERR_EXIT(rv);
...
static int
bcm56780_a0_ltsw_port_subsidiary_ports_get(
    int unit, bcm_port_t port,
    bcm_pbmp_t *pbmp)
{
...
static int
bcm56780_a0_ltsw_original_tvco_rearrange(
    int unit,
    int nport,
    ltsw_flexport_port_cfg_t **port_cfg_array,
    int original_tvco)
{
    ...
    for (i = 0; i < nport; i++) {
        ...
        if (po->op & (FLEXPORT_OP_ATTACH_FIRST | FLEXPORT_OP_ATTACH_SECOND)) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcm56780_a0_fs_ability_table_vco_get(po->speed, po->lanes, po->fec, &vco));
            if (vco == ltswPortVCOInvalid) {
                continue;
            }
    ...
    psrc = NULL;
    /*
     * If 25G is original tvco and 20G vco is existed, the 20G vco port should
     * be inserted in PM first, which is a default rule for PM8x50 core.
     */
    if (vco_20g_index != -1) {
...
static int
bcm56780_a0_ltsw_pm_port_init_adjust(
    int unit,
    int nport,
    ...
    int original_tvco)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_ltsw_flexport_pm_validate(unit, &current_pbm,
                                               nport, operation));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_ltsw_pm_port_rearrange(unit, nport, operation));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_ltsw_original_tvco_rearrange(unit, nport, operation,
                                                  original_tvco));
...
static mbcm_ltsw_port_drv_t bcm56780_a0_ltsw_port_drv = {
    .port_info_init                 = bcm56780_a0_ltsw_port_info_init,
    .port_info_init_per_port        = bcm56780_a0_ltsw_port_info_init_per_port,
    .port_info_deinit               = bcm56780_a0_ltsw_port_info_deinit,
    .port_info_recover              = bcm56780_a0_ltsw_port_info_recover,
    .port_profile_init              = bcm56780_a0_ltsw_port_profile_init,
    .port_profile_deinit            = bcm56780_a0_ltsw_port_profile_deinit,
    .port_service_init              = bcm56780_a0_ltsw_port_service_init,
    .port_service_deinit            = bcm56780_a0_ltsw_port_service_deinit,
    .port_misc_init                 = bcm56780_a0_ltsw_port_misc_init,
    ...
    .port_resource_spec_get         = bcm56780_a0_ltsw_port_resource_spec_get,
    .port_class_validate            = bcm56780_a0_ltsw_port_class_validate,
    .port_group_range_get           = bcm56780_a0_ltsw_port_group_range_get,
    ...
    .port_egr_shaper_validate       = bcm56780_a0_ltsw_port_egr_shaper_validate,
    .port_queue_count_get           = xfs_ltsw_port_queue_count_get,
    .flexport_input_validate        = bcm56780_a0_ltsw_flexport_input_validate,
    .flexport_resource_validate     = bcm56780_a0_ltsw_flexport_resource_validate,
    .port_speed_config_validate     = bcm56780_a0_ltsw_port_speed_config_validate,
    .flexport_resource_attach       = bcm56780_a0_ltsw_flexport_resource_attach,
    .flexport_resource_detach       = bcm56780_a0_ltsw_flexport_resource_detach,
    .flexport_resource_status_update= bcm56780_a0_ltsw_flexport_resource_status_update,
    .pm_port_init_adjust            = bcm56780_a0_ltsw_pm_port_init_adjust,
    ...
    .port_subsidiary_ports_get      = bcm56780_a0_ltsw_port_subsidiary_ports_get,
...
int
bcm56780_a0_ltsw_port_drv_attach(int unit)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_port_drv_set(unit, &bcm56780_a0_ltsw_port_drv));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_ltsw_variant_drv_attach(unit, BCM56780_A0_LTSW_VARIANT_PORT));
