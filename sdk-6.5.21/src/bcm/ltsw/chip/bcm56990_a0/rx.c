...
/*
 * \brief Calculate strength value.
 *
 ...
 */
static int
rx_cosq_mapping_strength_calc(int unit,
                              int index,
                              int *strength)
{
    int entry_num = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_capacity_get(unit, TM_COS_Q_CPU_MAPs, (uint32_t *)&entry_num));

    LOG_INFO(BSL_LOG_MODULE,
         (BSL_META_U(unit,
                     "rx cosq map entries %d.\n"),
          entry_num));
    ...
    *strength = entry_num - index;

    LOG_INFO(BSL_LOG_MODULE,
         (BSL_META_U(unit,
                     "rx cosq map index %d strength %d.\n"),
          index, *strength));
 ...
 */
static int
rx_mirror_control_set(int unit, bcm_rx_cosq_mapping_t *mapping)
{
    bcmi_lt_entry_t lt_entry;
    bcmi_lt_field_t fields[] =
    {
        /* 0 */ {CPU_Q_HI_PRIs,                0, 0, {0}},
        /* 1 */ {TRUNCATE_CPU_COPYs,           0, 0, {0}},
        /* 2 */ {CPU_COSs,                     0, 0, {0}},
        /* 3 */ {CPU_COS_STRENGTHs,            0, 0, {0}},
        /* 4 */ {SFLOW_EGR_CPU_Q_HI_PRIs,      0, 0, {0}},
        /* 5 */ {SFLOW_EGR_TRUNCATE_CPU_COPYs, 0, 0, {0}},
        /* 6 */ {SFLOW_EGR_CPU_COSs,           0, 0, {0}},
        /* 7 */ {SFLOW_EGR_CPU_COS_STRENGTHs,  0, 0, {0}},
    };
    int strength = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (rx_cosq_mapping_strength_calc(unit, mapping->index, &strength));
                               ...
                               bool key_only)
{
    ...
    if (!key_only) {
        ...
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(eh, STRENGTHs, strength));
    }
    ...
    bcm_rx_cosq_mapping_t *rx_cosq_mapping)
{
    ...
    /* Update mirror control */
    if ((rx_cosq_mapping->packet_type_mask & BCM_RX_COSQ_PACKET_TYPE_MIRROR) ||
        (BCM_RX_REASON_GET(rx_cosq_mapping->reasons, bcmRxReasonSampleDest))) {
        SHR_IF_ERR_VERBOSE_EXIT
            (rx_mirror_control_set(unit, rx_cosq_mapping));
    }
...
static int
bcm56990_a0_ltsw_rx_cosq_mapping_size_get(
    int unit,
    int *size)
{
...
static int
bcm56990_a0_ltsw_rx_cosq_mapping_reasons_get(
    int unit,
    bcm_rx_reasons_t *reasons)
{
    bcm_rx_reason_t reason_enum;
    int key_idx, key_table_len;
    ...
    for (key_idx = 0; key_idx < key_table_len; key_idx++) {
        reason_enum = rx_cos_reason_map[key_idx].reason;
        if (reason_enum != (bcm_rx_reason_t)bcmRxReasonInvalid) {
            BCM_RX_REASON_SET(*reasons, reason_enum);
        }
    }
...
static mbcm_ltsw_rx_drv_t bcm56990_a0_ltsw_rx_drv = {
    ...
    .rx_cosq_mapping_size_get = bcm56990_a0_ltsw_rx_cosq_mapping_size_get,
    .rx_cosq_mapping_reasons_get = bcm56990_a0_ltsw_rx_cosq_mapping_reasons_get,
