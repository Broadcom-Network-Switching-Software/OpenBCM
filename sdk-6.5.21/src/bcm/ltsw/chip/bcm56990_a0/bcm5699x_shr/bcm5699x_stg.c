    ...
    bcmint_stg_lt_db_t *lt_db)
{
    ...
    /* Bitmap of LTs that valid on bcm5699x. */
    lt_db->lt_bmp = 1 << BCMINT_LTSW_STG_VLAN_VFI_STG;
    lt_db->lts = bcm5699x_stg_lt;
...
/*!
 * \brief Stg sub driver functions for bcm5699x.
 */
static mbcm_ltsw_stg_drv_t bcm5699x_stg_sub_drv = {
    .stg_lt_db_get = bcm5699x_ltsw_stg_lt_db_get,
};
...
int
bcm5699x_stg_sub_drv_attach(int unit, void *hdl)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_stg_drv_set(unit, &bcm5699x_stg_sub_drv));
