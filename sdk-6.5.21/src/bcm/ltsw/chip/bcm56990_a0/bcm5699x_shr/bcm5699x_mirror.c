 ...
 */
/*!
 * \brief Get ingress flex sample profile num.
 *
 * \param [in] unit Unit number.
 * \param [out] num Ingress flex sample profile num.
 *
 * \retval SHR_E_NONE on success and error code otherwise.
 */
static int
mirror_ing_flex_sample_profile_num_get(int unit, int *num)
{
    uint64_t min = 0;
    uint64_t max = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_field_value_range_get(unit,
                                       MIRROR_ING_FLEX_SFLOWs,
                                       MIRROR_ING_FLEX_SFLOW_IDs,
                                       &min,
                                       &max));
                                    ...
                                    bcmint_mirror_ctrl_info_t *mirror_ctrl)
{
    ...
    */
    ...
    rv = mirror_ing_flex_sample_profile_num_get(unit, &num);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    mirror_ctrl->sample_prf_info.prf_num = num;
    /* Profile id 0 is not reserved. */
    mirror_ctrl->sample_prf_info.rsvd_prf_id_zero = false;
    mirror_ctrl->sample_prf_info.ing_sample_prf_unsupported = true;
