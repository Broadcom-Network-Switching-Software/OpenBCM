...
{
    int fields_num_max, fields_num, rv;
    bcm_vlan_t def_vlan;
    uint32_t *data = NULL;
    ...
    /* PORT_TYPE */
    types[fields_num] = BCMI_PT_PORT_TYPE;
    if (IS_ETH_PORT(unit, port)) {
        data[fields_num] = XGS_PORT_TYPE_ETHERNET;
    } else if (IS_LB_PORT(unit, port)) {
        data[fields_num] = XGS_PORT_TYPE_LOOPBACK;
    } else {
        data[fields_num] = XGS_PORT_TYPE_ETHERNET;
    }
    fields_num++;
    ...
    /* EN_EFILTER */
    types[fields_num] = BCMI_PT_EGR_VLAN_MEMBERSHIP_CHECK;
    data[fields_num] = 1;
    fields_num++;

    /* TRUST_INCOMING_VID */
    types[fields_num] = BCMI_PT_TRUST_INCOMING_VID;
    data[fields_num] = 1;
    fields_num++;

    /* Set the CML to PVP_CML_SWITCH by default (hw learn and forward) */
    types[fields_num] = BCMI_PT_MAC_LEARN;
    data[fields_num] = 1;
    fields_num++;

    types[fields_num] = BCMI_PT_MAC_MOVE;
    data[fields_num] = 1;
    fields_num++;

    types[fields_num] = BCMI_PT_V4L3;
    data[fields_num] = 1;
    fields_num++;

    types[fields_num] = BCMI_PT_V6L3;
    data[fields_num] = 1;
    fields_num++;

    types[fields_num] = BCMI_PT_V4IPMC;
    data[fields_num] = 1;
    fields_num++;

    types[fields_num] = BCMI_PT_V6IPMC;
    data[fields_num] = 1;
    fields_num++;

    types[fields_num] = BCMI_PT_IPMC_USE_L3_IIF;
    data[fields_num] = 1;
    fields_num++;

    types[fields_num] = BCMI_PT_MPLS;
    data[fields_num] = 1;
    fields_num++;

    types[fields_num] = BCMI_PT_TRUST_PHB_EGR_PORT_L2_OTAG;
    data[fields_num] = 1;
    fields_num++;
...
bcm56990_a0_ltsw_flexport_pm_validate(int unit, bcm_pbmp_t *current_ports, int nport, ltsw_flexport_port_cfg_t **port_cfg_array)
{
     ...
     * For example, in the case that modify 2x200G(4-lane) to 2x100G(4-lane) in one PM core,
     * there is no lane re-mapping or logical port change, however, the VCO need to be changed.
     * FLEXPORT_OP_PC_RESET is labelled to tell the upper layer to delete and add PC_PORT entry.
     */
    if (is_flexport_operation || vco_need_change) {
        BCM_PBMP_CLEAR(reset_ports);
        for (i = 0; i < nport; i++) {
            if (port_cfg_array[i]->op & FLEXPORT_OP_DETACH) {
                /* Delete the  port */
...
static mbcm_ltsw_port_drv_t bcm56990_a0_ltsw_port_drv = {
    ...
    .port_color_mode_set                    = xgs_ltsw_port_color_mode_set,
    .port_color_mode_get                    = xgs_ltsw_port_color_mode_get,
};
