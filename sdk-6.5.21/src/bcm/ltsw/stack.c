...
#include <bcm_int/ltsw/generated/stk_ha.h>
...
/* SW stack pbmp related information. */
bcmint_stk_pbmp_info_ha_t *ltsw_stack_pbmp_info[BCM_MAX_NUM_UNITS];
...
bcmi_ltsw_stk_info_deinit(int unit)
{
    ...
    bcmint_stk_pbmp_info_ha_t *pi;
    int warm = bcmi_warmboot_get(unit);

    SHR_FUNC_ENTER(unit);

    si = ltsw_stack_info[unit];
    pi = ltsw_stack_pbmp_info[unit];
    ...
    if (!warm && pi) {
        (void)bcmi_ltsw_ha_mem_free(unit, pi);
        pi = NULL;
    }
...
{
    uint32 alloc_size, req_size = 0;
    bcm_port_t p;
    uint64_t modid;
    ltsw_stack_info_t *si;
    bcmint_stk_pbmp_info_ha_t *pi;
    int warm = bcmi_warmboot_get(unit), rv;
    ...
    req_size = sizeof(bcmint_stk_pbmp_info_ha_t);
    alloc_size = req_size;
    ltsw_stack_pbmp_info[unit] = bcmi_ltsw_ha_mem_alloc(
                                    unit,BCMI_HA_COMP_ID_STACK,
                                    BCMINT_STACK_SUB_COMP_ID_STACK_PORTS,
                                    "bcmStackPorts", &alloc_size);
    pi = ltsw_stack_pbmp_info[unit];
    ...
    if (!warm) {
        sal_memset(pi, 0, alloc_size);
    }

    rv = bcmi_ltsw_issu_struct_info_report(unit,
             BCMI_HA_COMP_ID_STACK,
             BCMINT_STACK_SUB_COMP_ID_STACK_PORTS,
             0, alloc_size, 1,
             BCMINT_STK_PBMP_INFO_HA_T_ID);
    if (rv != SHR_E_EXISTS) {
                              ...
                              int inactiveChkReqd)
{
    ...
    /* The for loop is to implement BCM_PBMP_NEQ functionality. */
    for (i = 0; i < BCM_PBMP_PORT_MAX; i++) {
        if (pi->stack_ports_current[i] != pi->stack_ports_previous[i]) {
            neq = 1;
        }
    }

    if (neq || inactiveChkReqd ) {
                       ...
                       bcm_pbmp_t *pre_pbmp)
{
    ...
    if (curr_pbmp != NULL) {
        for (i = 0; i < BCM_PBMP_PORT_MAX; i++) {
            if (pi->stack_ports_current[i] == 1) {
                BCM_PBMP_PORT_ADD(*curr_pbmp, i);
            } else {
                BCM_PBMP_PORT_REMOVE(*curr_pbmp, i);
            }
        }
    }
    if (inac_pbmp != NULL) {
        for (i = 0; i < BCM_PBMP_PORT_MAX; i++) {
            if (pi->stack_ports_inactive[i] == 1) {
                BCM_PBMP_PORT_ADD(*inac_pbmp, i);
            } else {
                BCM_PBMP_PORT_REMOVE(*inac_pbmp, i);
            }
        }
    }
    ...
    if (pre_pbmp != NULL) {
        for (i = 0; i < BCM_PBMP_PORT_MAX; i++) {
            if (pi->stack_ports_previous[i] == 1) {
                BCM_PBMP_PORT_ADD(*pre_pbmp, i);
            } else {
                BCM_PBMP_PORT_REMOVE(*pre_pbmp, i);
            }
        }
    }
...
bcmi_ltsw_stk_port_set(int unit, bcm_port_t port, uint32 flags)
{
    ...
    pi = ltsw_stack_pbmp_info[unit];
    /* The if logic is to implement BCM_PBMP_MEMBER functionality. */
    if (pi->stack_ports_inactive[port] == 1) {
        isPrevInactiveSet = 1;
    }

    /*
     * Record old stack port states; make changes to current
     * for loop to implement BCM_PBMP_ASSIGN functionality.
     */
    for (i = 0; i < BCM_PBMP_PORT_MAX; i++) {
        pi->stack_ports_previous[i] = pi->stack_ports_current[i];
    }

    if (flags & BCM_STK_ENABLE) {
        pi->stack_ports_current[port] = 1;
        if (flags & BCM_STK_INACTIVE) {
            pi->stack_ports_inactive[port] = 1;
            if (!isPrevInactiveSet) {
                inactiveChkReqd = 1;
            }
        } else {
            pi->stack_ports_inactive[port] = 0;
            if (isPrevInactiveSet) {
                inactiveChkReqd = 1;
            }
        }
    } else {
        pi->stack_ports_current[port] = 0;
        pi->stack_ports_inactive[port] = 0;
    }
    STACK_UNLOCK(unit);
...
bcm_ltsw_stk_port_get(int unit, bcm_port_t port, uint32 *flags)
{
    ...
    *flags = 0;
    ...
    if (pi->stack_ports_current[port] == 1) {
