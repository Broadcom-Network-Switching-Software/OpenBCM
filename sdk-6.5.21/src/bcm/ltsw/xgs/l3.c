...
l3_ipv4_options_profile_init(int unit)
{
    ...
    if (!bcmi_warmboot_get(unit)) {
        sal_memset(info->v4_option_profile_fbmp, 0, alloc_sz);
        SHR_BITSET_RANGE(info->v4_option_profile_fbmp,
                         L3_IPV4_OPTION_PROFILE_MIN(unit),
                         L3_IPV4_OPTION_PROFILE_TBL_SZ(unit));
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
    if (!bcmi_warmboot_get(unit)) {
        sal_memset(info->v4_option_profile_hw_idx, 0, alloc_sz);
    }
...
xgs_ltsw_l3_info_get(int unit, bcm_l3_info_t *l3info)
{
    ...
    bcmint_ltsw_l3_egress_tbl_id_t tbl_id_2;
    ...
    tbl_id_2 = BCMINT_LTSW_L3_EGRESS_TBL_UC;
    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_egress_tbl_get(unit, tbl_id_2, &tbl_info));
    ltname = tbl_info->name;

    /* Nexthop. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_capacity_get(unit, ltname, &value));
    l3info->l3info_max_nexthop = value;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_inuse_cnt_get(unit, ltname, &value));

    tbl_id_2 = BCMINT_LTSW_L3_EGRESS_TBL_MPLS_ENCAP;
    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_egress_tbl_get(unit, tbl_id_2, &tbl_info));
    ltname = tbl_info->name;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_inuse_cnt_get(unit, ltname, &value1));

    tbl_id_2 = BCMINT_LTSW_L3_EGRESS_TBL_MPLS_TRNSIT;
    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_egress_tbl_get(unit, tbl_id_2, &tbl_info));
    ltname = tbl_info->name;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_inuse_cnt_get(unit, ltname, &value2));

    tbl_id_2 = BCMINT_LTSW_L3_EGRESS_TBL_MC;
    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_egress_tbl_get(unit, tbl_id_2, &tbl_info));
    ltname = tbl_info->name;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_inuse_cnt_get(unit, ltname, &value3));

    tbl_id_2 = BCMINT_LTSW_L3_EGRESS_TBL_IFP_ACTION;
    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_egress_tbl_get(unit, tbl_id_2, &tbl_info));
    ltname = tbl_info->name;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_inuse_cnt_get(unit, ltname, &value4));

    if (ltsw_feature(unit, LTSW_FT_FLEX_FLOW)) {
        tbl_id_2 = BCMINT_LTSW_L3_EGRESS_TBL_L2_TAG;
        SHR_IF_ERR_EXIT
            (bcmint_ltsw_l3_egress_tbl_get(unit, tbl_id_2, &tbl_info));
        ltname = tbl_info->name;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_entry_inuse_cnt_get(unit, ltname, &value5));
    }
    l3info->l3info_used_nexthop = value + value1 + value2 + value3 + value4 + value5;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_capacity_get(unit, TNL_IPV4_DECAP_EMs, &value));
    l3info->l3info_max_tunnel_term += value;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_inuse_cnt_get(unit, TNL_IPV4_DECAP_EMs, &value));
    l3info->l3info_used_tunnel_term += value;
    ...
    uint32_t value)
{
    ...
    bool locked = false;
    int i;
    uint64_t field_val[MAX_FIELD_CNT] = {0, 0, 0};
    const char *field_symbol[MAX_FIELD_CNT] = {NULL, NULL, NULL};
    ...
    uint32_t *value)
{
    ...
    int rv;
    bool locked = false;
    uint64_t field_value[MAX_FIELD_CNT] = {0, 0, 0};
    const char *field_symbol[MAX_FIELD_CNT] = {NULL, NULL, NULL};
    ...
    bcmi_ltsw_l3_port_control_t *info)
{
    ...
    if (rv == SHR_E_NOT_FOUND) {
        ...
        if (control & BCMI_LTSW_L3_PC_ATTR_IPMC_TTL_DEC) {
            ...
            info->ipmc_ttl_dec = fld_def.def ? 0 : 1;
        }
        ...
        if (control & BCMI_LTSW_L3_PC_ATTR_IPMC_TTL_CHECK) {
            ...
            info->ipmc_ttl_check = fld_def.def ? 0 : 1;
        }
        ...
        if (control & BCMI_LTSW_L3_PC_ATTR_IPMC_SA_REPLACE) {
            ...
            info->ipmc_sa_replace = fld_def.def ? 0 : 1;
        }
