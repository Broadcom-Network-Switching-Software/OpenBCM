    ...
    bcm_ecn_map_t *ecn_map)
{
    ...
    switch (xgs_ecn_type) {
        ...
        case EcnMapTypeTunnelIngress:
            ...
            rv = bcmlt_entry_allocate(dunit, ECN_TNL_DECAP_NON_IP_PAYLOADs,
                                      &entry_hdl);
            if (rv == SHR_E_NONE) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl,
                        ECN_TNL_DECAP_NON_IP_PAYLOAD_IDs, map_id));
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl, TNL_ECNs, ecn_map->ecn));
    ...
    bcm_ecn_map_profile_t *profile)
{
    ...
    if ((profile->int_pri > XGS_INT_PRI_MAX_VALUE)) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (profile->type >= bcmEcnMapProfileCount) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (profile->type == bcmEcnMapProfileIntCnPostForward) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_allocate(dunit, ECN_INT_PRI_TO_CNG_POSTs,
                                  &entry_hdl));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(entry_hdl, INT_PRIs, profile->int_pri));
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    if (profile->type == bcmEcnMapProfileIntCnPreForward) {
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    bcm_ecn_traffic_map_info_t *map)
{
    ...
    if (map->type == bcmEcnTrafficMapTypeIngressPostForward) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_allocate(dunit, ECN_IP_TO_CNG_POSTs, &entry_hdl));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(entry_hdl, ECN_IP_TO_CNG_IDs, map->profile_id););
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    bcm_ecn_traffic_action_config_t *ecn_config)
{
    ...
    if ((ecn_config->ecn > XGS_ECN_MAX_VALUE) ||
