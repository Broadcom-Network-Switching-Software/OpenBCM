    ...
    bcmint_udf_pkt_format_info_t *format_info)
{
    ...
    if (XGS_UDF_CTRL(unit)->lt_flags & BCMI_XGS_UDF_LT_OUTPUT_FLEX_HASH) {
        if (offset_info->flags & BCMI_UDF_OFFSET_INFO_FLEXHASH) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(handle, FLEX_HASHs, 1));
        }
    }
    ...
    bcmint_udf_offset_info_t *offset_info)
{
    ...
    bcmint_udf_pkt_format_info_t *format_info_array = NULL;
    bcmi_xgs_udf_lt_tbls_info_t *tbls_info = NULL;
    ...
    bcmi_xgs_udf_lt_hash_config_t *lt_hash_config)
{
    ...
    uint64_t u64mask = 0;
    const char *hash_field_str = NULL;
    bcm_udf_pkt_format_info_t format;
    bcmlt_entry_handle_t entry_hdl = BCMLT_INVALID_HDL;
    bcmint_udf_pkt_format_info_t *temp = NULL;
    bcmint_udf_pkt_format_info_t *format_info_array = NULL;
    bcmi_xgs_udf_hash_chunks_info_t *hash_chunks_info = NULL;
    bcmi_xgs_udf_lt_tbls_info_t *tbls_info = NULL;
    ...
    bcmint_udf_pkt_format_info_t **matched_format_info)
{
    ...
    sal_memset(&format_info, 0, sizeof(format_info));
    SHR_IF_ERR_VERBOSE_EXIT
    ...
    int arg)
{
    ...
    /* Save new values */
    if (action == bcmUdfPktFormatActionClassId) {
        format_info->class_id = arg;
    } else if (action == bcmUdfPktFormatActionSkipOuterL4IfaMetadata) {
        if (arg > 0) {
            format_info->flags |=
