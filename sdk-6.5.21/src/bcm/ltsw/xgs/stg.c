 ...
 */
static int
stg_map_recover(int unit, int ingress,
                bcmi_ltsw_vlan_info_t *info, void *user_data)
{
    bcm_vlan_t vid = 0;
    int uidx = 0;
 ...
 */
static int
xgs_stg_vlan_reset(int unit, int ingress,
                   bcmi_ltsw_vlan_info_t *info, void *user_data)
{
    bcm_vlan_t vid = 0;
    bcm_stg_t stg;
...
/*!
 * \brief Internal function to resolve the given port/gport to local port.
 *
 * \param [in] unit        Unit number.
 * \param [in] in_port     Port/gport.
 * \param [out]out_port    Local port.
 *
 * \retval SHR_E_NONE      No errors.
 * \retval !SHR_E_NONE     Failure.
 */
static int
stg_gport_resolve(int unit, bcm_port_t in_port, bcm_port_t *out_port)
{
    SHR_FUNC_ENTER(unit);
    ...
    bcm_stg_t stg)
{
    ...
    fid = BCMINT_LTSW_STG_FLD_VLAN_VFI_STG_STATE;
    SHR_IF_ERR_VERBOSE_EXIT
        (flds[fid].scalar_to_symbol(unit, BCM_STG_STP_DISABLE, &(symbol[0])));
    SHR_IF_ERR_VERBOSE_EXIT
        (flds[fid].scalar_to_symbol(unit, BCM_STG_STP_FORWARD, &(symbol[1])));
    ...
    BCM_PBMP_ITER(pbmp, port) {
        /*
         * Set all ports into disable in none-zero stg. But CPU ports
         * and LB ports are always in forward state.
         */
        if (stg == 0) {
            state[0] = symbol[1];
        } else {
            if (bcmi_ltsw_port_is_type(unit, port, BCMI_LTSW_PORT_TYPE_CPU) ||
                bcmi_ltsw_port_is_type(unit, port, BCMI_LTSW_PORT_TYPE_LB)) {
                state[0] = symbol[1];
            } else {
                state[0] = symbol[0];
            }
    ...
    int stp_state)
{
    ...
    SHR_IF_ERR_EXIT
        (stg_gport_resolve(unit, port, &local_port));
    ...
    int *uidx_next)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_vlan_traverse(unit, 0, stg_map_recover, &info));
    ...
    bcm_stg_t stg)
{
    ...
    stg_id = stg;
    SHR_IF_ERR_EXIT
        (bcmi_ltsw_vlan_traverse(unit, 0, xgs_stg_vlan_reset, &stg_id));
