...
/*! Max UFT L2 entries. */
#define UFT_L2_ENTRY_UFT_LIMIT 0x20000

/*! Max Dedicated L2 entries. */
#define UFT_L2_ENTRY_DEDICATED_LIMIT 0x2000

/*! Number of UFT L2 banks. */
#define UFT_L2_BANK_NUM 4

/*! Robust seed default value. */
#define UFT_ROBUST_SEED_DEFAULT 0xfffffd
...
hash_uft_em_table_config_set(int unit)
{
    ...
    num_entries = bcmi_ltsw_property_get(unit, BCMI_CPN_L2_MEM_ENTRIES,
                                         UFT_L2_ENTRY_DEDICATED_LIMIT);
    if (num_entries <= UFT_L2_ENTRY_DEDICATED_LIMIT) {
        dev_info->l2_uft_group_used = 0;
    } else if (num_entries <= UFT_L2_ENTRY_UFT_LIMIT) {
        l2_num_banks = (num_entries + (32 * 1024 - 1)) / (32 * 1024);
        if (l2_num_banks != UFT_L2_BANK_NUM) {
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
                                  "Error configuration: %s: %d. "
                                  "Must configure 4 banks for l2 table.\n"),
                       BCMI_CPN_L2_MEM_ENTRIES, num_entries));
        ...
        LOG_ERROR(BSL_LOG_MODULE,
                  ...
                  BCMI_CPN_L2_MEM_ENTRIES, num_entries, UFT_L2_ENTRY_UFT_LIMIT));
        SHR_ERR_EXIT(SHR_E_CONFIG);
    }
...
hash_uft_table_robust_init(int unit)
{
    ...
    seed = bcmi_ltsw_property_get(unit, BCMI_CPN_ROBUST_HASH_SEED_L2,
                                  UFT_ROBUST_SEED_DEFAULT);
    SHR_IF_ERR_VERBOSE_EXIT
    ...
    seed = bcmi_ltsw_property_get(unit, BCMI_CPN_ROBUST_HASH_SEED_MPLS,
                                  UFT_ROBUST_SEED_DEFAULT);
    SHR_IF_ERR_VERBOSE_EXIT
        (hash_bank_robust_enable_set(unit,
                                     bcmHashTableMPLS,
                                     true,
                                     seed));
    seed = bcmi_ltsw_property_get(unit, BCMI_CPN_ROBUST_HASH_SEED_L3_TUNNEL,
                                  UFT_ROBUST_SEED_DEFAULT);
    SHR_IF_ERR_VERBOSE_EXIT
        (hash_bank_robust_enable_set(unit,
    ...
    seed = bcmi_ltsw_property_get(unit, BCMI_CPN_ROBUST_HASH_SEED_EXACT_MATCH,
                                  UFT_ROBUST_SEED_DEFAULT);
    SHR_IF_ERR_VERBOSE_EXIT
        (hash_bank_robust_enable_set(unit,
