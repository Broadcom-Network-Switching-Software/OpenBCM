...
typedef struct xgs_mc_info_s {
    ...
    /* Reserve multicast resources for delete. */
    uint8                    repl_head_reserve;

    /* Minimum vaule of reserve multicast resources index. */
    uint32_t                 repl_head_reserve_idx_min;

    /*! L3 multicast max encap per port. */
    uint32_t                 eid_max;
...
#define REPL_HEAD_RESERVE(u)       (MC_INFO(u)->repl_head_reserve)
#define REPL_HEAD_RESERVE_IDX_MIN(u)   (MC_INFO(u)->repl_head_reserve_idx_min)
...
mc_repl_head_info_init(int unit)
{
     ...
     */
    REPL_HEAD_RESERVE(unit) =
        bcmi_ltsw_property_get(unit,
                               BCMI_CPN_RESERVE_MULTICAST_RESOURCES, 0);
    if (REPL_HEAD_RESERVE(unit)) {
        num_ports = bcmi_ltsw_dev_logic_port_num(unit);
        REPL_HEAD_RESERVE_IDX_MIN(unit) =
            agg_list_id_min + REPL_HEAD_SIZE(unit) - num_ports;
    }
    REPL_HEAD_FREE_LIST(unit, 0)->index = agg_list_id_min;
    REPL_HEAD_FREE_LIST(unit, 0)->size = REPL_HEAD_SIZE(unit) - num_ports;
    ...
    int *l3_mem_arr)
{
    ...
    int dunit = 0;
    uint64_t value = 0;
    uint32_t idx, max_ports, sz, r_cnt;
    uint64_t *data = NULL;
    ...
    max_ports = bcmi_ltsw_dev_logic_port_num(unit);
    sz = sizeof(uint64_t) * max_ports;
    SHR_ALLOC(data, sz, "bcmLtswXgsTmMcData");
    SHR_NULL_CHECK(data, SHR_E_MEMORY);

    if (l2_mem_arr != NULL) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_array_get(eh, L2_PORTs,
                                         0, data, max_ports, &r_cnt));
        for (idx = 0; idx < r_cnt; idx++) {
            l2_mem_arr[idx] = data[idx];
        }
    }
    ...
    int *l3_mem_arr)
{
    ...
    if (l2_mem_arr) {
        for (idx = 0; idx < max_ports; idx++) {
            data[idx] = l2_mem_arr[idx];
        }
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_array_add(eh, L2_PORTs, 0, data, max_ports));
    }

    if (l3_mem_arr) {
        for (idx = 0; idx < max_ports; idx++) {
            data[idx] = l3_mem_arr[idx];
        }
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_array_add(eh, L3_PORTs, 0, data, max_ports));
    }
 ...
 * \param [in]  insert          Indicates it is an insert operation.
 *
 * \retval SHR_E_NONE     No errors.
 * \retval !SHR_E_NONE    Failure.
 */
    ...
    bool insert)
{
    ...
    if (encap_id_cnt > 0) {
        ...
        sz = sizeof(uint64_t) * encap_id_cnt;
        SHR_ALLOC(data, sz, "bcmLtswXgsTmMcData");
        SHR_NULL_CHECK(data, SHR_E_MEMORY);
        for (idx = 0; idx < encap_id_cnt; idx++) {
            data[idx] = encap_id_arr[idx];
        }
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    int *encap_id_cnt)
{
    ...
    *encap_id_cnt = value;
    ...
    if (encap_id_arr != NULL) {
        sz = sizeof(uint64_t) * encap_id_sz;
        SHR_ALLOC(data, sz, "bcmLtswXgsTmMcData");
        SHR_NULL_CHECK(data, SHR_E_MEMORY);
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    int encap_id_cnt)
{
    ...
    int rh_base = 0, rh_cnt = 0, new_rh_base, new_rh_cnt = 0;
    int member_id, new_member_id;
    /* Encap id array. */
    int *tmp_eid_arr = NULL;
    int  r_cnt = 0, rv;
    uint32_t eid_max = xgs_mc_info->eid_max;
    ...
    if (encap_id_cnt == 0) {
        ...
        if (rh_cnt > 1) {
            new_rh_cnt = rh_cnt - 1;
            rv = mc_repl_head_block_alloc(unit, new_rh_cnt, &new_rh_base);
            if (SHR_FAILURE(rv)) {
                if (REPL_HEAD_RESERVE(unit) == 0) {
                    SHR_ERR_EXIT(rv);
                } else if (rv != BCM_E_RESOURCE) {
                    SHR_ERR_EXIT(rv);
                }
                /* Reserve multicast resources is 1 and
                 * delete has resource issue. */
                new_rh_base = REPL_HEAD_RESERVE_IDX_MIN(unit);
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (mc_l3_grp_info_set(unit, l3mc_id,
                                    new_rh_base, new_rh_cnt,
                                    NULL, l3_memb));
        } else {
            l3_memb[port] = 0;
            /* This port is the last member port. */
            SHR_IF_ERR_VERBOSE_EXIT
        ...
        /* Set back current member bitmap for later operation. */
        l3_memb[port] = 0;
    ...
    if (reserved_resources_used == 1) {
        ...
        rh_base = REPL_HEAD_RESERVE_IDX_MIN(unit);
        SHR_IF_ERR_VERBOSE_EXIT
            (mc_repl_head_block_alloc(unit, new_rh_cnt, &new_rh_base));
        member_id = 0;
        new_member_id = 0;
        for (idx = 0; idx < max_ports; idx++) {
            if (l3_memb[idx] == 0) {
    ...
    uint32_t l3mc_id)
{
    ...
    if (type == BCM_MULTICAST_TYPE_L3) {
        val = 1;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(eh, L3_PAYLOADs, val));
    }
