    ...
    lt_mpls_tnl_decap_entry_t *entry)
{
    ...
    lt_id = entry->is_trunk ?
            BCMINT_LTSW_MPLS_LT_DECAP_TRUNK_1 : BCMINT_LTSW_MPLS_LT_DECAP_1;
    ...
    lt_id = entry->is_trunk ?
            BCMINT_LTSW_MPLS_LT_DECAP_TRUNK_2 : BCMINT_LTSW_MPLS_LT_DECAP_2;
    ...
    lt_mpls_tnl_decap_entry_t *entry)
{
    ...
    if (entry->exp_map_action ==
        ...
        (void)bcmi_ltsw_qos_map_id_construct(unit,
                                             entry->phb_ing_mpls_exp_to_int_pri_id,
    ...
    lt_mpls_tnl_encap_entry_t *entry)
{
    ...
    entry->max_labels = num_labels;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_capacity_get(unit,
                              PHB_EGR_MPLS_INT_PRI_TO_TRANSIT_EXPs,
                              &num_mpls_map));
    num_mpls_map = num_mpls_map / 64;
    for (idx = 0; idx < num_labels; idx++) {
        if (label_array[idx].qos_map_id == 0) { /* treat it as using default */
            map_id = 0;
        } else {
            SHR_IF_ERR_VERBOSE_EXIT
        ...
        if ((label_array[idx].label > 0xfffff) ||
            (map_id < 0) ||
            (map_id >= num_mpls_map) ||
            (label_array[idx].exp > 7) || (label_array[idx].pkt_pri > 7) ||
            (label_array[idx].pkt_cfi > 1)) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
    ...
    void *user_data)
{
    ...
    lt_id = BCMINT_LTSW_MPLS_LT_DECAP_1;
    ...
    lt_id = BCMINT_LTSW_MPLS_LT_DECAP_TRUNK_1;
...
xgs_ltsw_mpls_tnl_switch_delete_all(int unit)
{
    ...
    int rv = SHR_E_NONE;
    lt_mpls_tnl_decap_entry_t entry;
    lt_mpls_tnl_decap_entry_t next_entry;
    const bcmint_mpls_lt_t *lt_db;
    bcmint_mpls_lt_id_t lt_id;
    ...
    rv = bcmi_lt_entry_commit(unit, ent_hdl, BCMLT_OPCODE_TRAVERSE,
                              BCMLT_PRIORITY_NORMAL);
    if (SHR_SUCCESS(rv)) {
        sal_memset(&next_entry, 0, sizeof(lt_mpls_tnl_decap_entry_t));
        SHR_IF_ERR_VERBOSE_EXIT
            (lt_mpls_tnl_decap_entry_from_hdl(unit, &next_entry, ent_hdl, lt_db));
    }

    while (rv == SHR_E_NONE) {
        sal_memcpy(&entry, &next_entry, sizeof(lt_mpls_tnl_decap_entry_t));
    ...
    lt_id = BCMINT_LTSW_MPLS_LT_DECAP_2;
    ...
    lt_id = BCMINT_LTSW_MPLS_LT_DECAP_TRUNK_2;
    ...
    bcm_mpls_egress_label_t *label_array)
{
    ...
    (void)bcmi_ltsw_idx_tbl_entry_ref_get(unit, tbl_id, user_id, old_tnl_idx,
                                          &ref_cnt);

    if (old_hw_idx && ref_cnt) {
        sal_memset(&entry, 0, sizeof(entry));

        entry.tnl_encap_id = old_tnl_idx;
        SHR_IF_ERR_VERBOSE_EXIT
            (mpls_tnl_encap_entry_op(unit, BCMLT_OPCODE_LOOKUP, &entry));
        /*
         * We need to check here if at least number of labels is same.
         * Changing push action/ num labels is not permitted,
         * need to clear/set. Keep the same behavior as TH3.
         */
        if (num_labels != (int)entry.num_labels) {
...
int
xgs_ltsw_mpls_tnl_initiator_flexctr_object_get(
    int unit,
    bcm_if_t intf,
    uint32_t *value)
{
    ...
    bcmi_ltsw_flexctr_counter_info_t *info)
{
    ...
    if (is_trunk) {
        ltname = TNL_MPLS_DECAP_TRUNK_1s;
    } else {
        ltname = TNL_MPLS_DECAP_1s;
    }
