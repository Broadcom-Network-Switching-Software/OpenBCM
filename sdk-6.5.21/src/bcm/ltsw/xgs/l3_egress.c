...
/* Index of black hole nhop. */
#define L3_EGR_BLACK_HOLE_NH_IDX 0
...
typedef struct ecmp_cfg_s {
    ...
    /* Fields array. */
    uint64_t flds[BCMINT_XGS_L3_ECMP_TBL_FLD_CNT];
...
#define L3_NHOP_TYPE(_u, _idx) (l3_egr_info[_u].nh_type[_idx])

#define L3_NHOP_FLAGS2(_u, _idx) (l3_egr_info[_u].nh_flags2[_idx])

#define L3_NHOP_L2_TAG_VIEW(_u, _idx) (l3_egr_info[_u].nh_l2_tag[_idx])
...
/*!
 * \brief Fill ECMP member array.
 *
 * \param [in] unit Unit number
 * \param [in] fld ECMP field.
 * \param [in] num Number of ECMP members.
 * \param [in] member_info ECMP member info.
 * \param [out] arr ECMP member array.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
ecmp_member_arr_fill(int unit, bcmint_xgs_l3_ecmp_fld_t fld, uint32_t num,
                     ecmp_member_info_t *member_info, uint64_t *arr)
{
    int i;

    switch (fld) {
        case BCMINT_XGS_L3_ECMP_TBL_NHOP_ID:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].ul_nhop_ecmp_id;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_WEIGHT:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].weight ? member_info[i].weight : 1;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_RH_NHOP_ID:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].ul_nhop_ecmp_id;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_OVERLAY_NHOP_ID:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].nhop_id;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_ECMP_UNDERLAY_ID:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].ul_ecmp ?
                             member_info[i].ul_nhop_ecmp_id : 0;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_UNDERLAY_NHOP_ID:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].ul_ecmp ?
                             0 : member_info[i].ul_nhop_ecmp_id;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_RH_OVERLAY_NHOP_ID:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].nhop_id;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_RH_ECMP_UNDERLAY_ID:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].ul_ecmp ?
                             member_info[i].ul_nhop_ecmp_id : 0;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_RH_UNDERLAY_NHOP_ID:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].ul_ecmp ?
                             0 : member_info[i].ul_nhop_ecmp_id;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_ECMP_NHOP_UNDERLAY:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].ul_ecmp;
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_RH_ECMP_NHOP_UNDERLAY:
            for (i = 0; i < num; i++) {
                arr[i] = member_info[i].ul_ecmp;
            }
            break;
...
/*!
 * \brief Parse ECMP member array.
 *
 * \param [in] unit Unit number
 * \param [in] fld ECMP field.
 * \param [in] num Number of ECMP members.
 * \param [in] arr ECMP member array.
 ...
 */
static int
ecmp_member_arr_parse(int unit, bcmint_xgs_l3_ecmp_fld_t fld, uint32_t num,
                      uint64_t *arr, ecmp_member_info_t *member_info)
{
    int i;
    ...
    switch (fld) {
        case BCMINT_XGS_L3_ECMP_TBL_NHOP_ID:
            for (i = 0; i < num; i++) {
                member_info[i].ul_nhop_ecmp_id = arr[i];
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_WEIGHT:
            for (i = 0; i < num; i++) {
                member_info[i].weight = arr[i];
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_RH_NHOP_ID:
            for (i = 0; i < num; i++) {
        ...
        case BCMINT_XGS_L3_ECMP_TBL_OVERLAY_NHOP_ID:
            for (i = 0; i < num; i++) {
                member_info[i].nhop_id = arr[i];
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_ECMP_NHOP_UNDERLAY:
            for (i = 0; i < num; i++) {
                member_info[i].ul_ecmp = arr[i];
            }
            break;
        case BCMINT_XGS_L3_ECMP_TBL_ECMP_UNDERLAY_ID:
            for (i = 0; i < num; i++) {
                if (member_info[i].ul_ecmp) {
                    member_info[i].ul_nhop_ecmp_id = arr[i];
                }
            }
            break;
...
/*!
 * \brief Fill ECMP LT entry handle.
 *
 * \param [in] unit Unit number
 * \param [in] eh LT entry handle.
 * \param [in] ti LT info.
 * \param [in] cfg Data of ECMP group.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
lt_eh_ecmp_fill(int unit, bcmlt_entry_handle_t eh,
                const bcmint_ltsw_l3_egress_tbl_t *ti,
                ecmp_cfg_t *cfg)
{
    const bcmint_ltsw_l3_egress_fld_t *fi = ti->flds;
    uint64_t *data = NULL;
    const char *sym_val;
    uint32_t i, num_of_elem, sz = 0;
    int rv = SHR_E_NONE;
    const char *str = NULL;

    num_of_elem = cfg->num_of_unset + cfg->num_of_set;
    if (num_of_elem) {
        sz = sizeof(uint64_t) * num_of_elem;
        data = sal_alloc(sz, "bcmLtswXgsEcmpMemberData");
        if (!data) {
            return SHR_E_MEMORY;
        }
    }

    for (i = 0; i < BCMINT_XGS_L3_ECMP_TBL_FLD_CNT; i++) {
        /* Skip fields that are invalid for the given LT. */
        if (!(ti->fld_bmp & (1 << i))) {
            continue;
        }
        ...
        */
        ...
        if (fi[i].symbol) {
            ...
            if (fi[i].depth) {
                ...
                rv = ecmp_member_arr_fill(unit, i, cfg->num_of_set,
                                          cfg->member_info, data);
                ...
                if (cfg->num_of_unset) {
                    str = fi[i].fld_name;
                    rv = (bcmlt_entry_field_array_unset(eh, str,
                              cfg->start_idx + cfg->num_of_set,
                              cfg->num_of_unset));
                }
            } else {
...
/*!
 * \brief Parse entry handle of ECMP group.
 *
 * \param [in] unit Unit number
 * \param [in] eh LT entry handle.
 * \param [in] ti LT info.
 * \param [out] cfg Data of ECMP group.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
lt_eh_ecmp_parse(int unit, bcmlt_entry_handle_t eh,
                 const bcmint_ltsw_l3_egress_tbl_t *ti,
                 ecmp_cfg_t *cfg)
{
    const bcmint_ltsw_l3_egress_fld_t *fi = ti->flds;
    uint64_t *data = NULL;
    uint32_t data_sz = 0;
    uint32_t i, sz, num_of_elem;
    const char *sym_val;
    int rv = SHR_E_NONE;

    if (cfg->num_of_set) {
        data_sz = sizeof(uint64_t) * cfg->num_of_set;
        data = sal_alloc(data_sz, "bcmLtswXgsEcmpMemberData");
        if (!data) {
            return SHR_E_MEMORY;
        }
    }
    ...
    if (cfg->num_of_set) {
        /* Fields which affect others should be processed first. */
        i = BCMINT_XGS_L3_ECMP_TBL_ECMP_NHOP_UNDERLAY;
        if (ti->fld_bmp & (1 << i)) {
            if (fi[i].depth) {
                sal_memset(data, 0, data_sz);
                num_of_elem = fi[i].depth < cfg->num_of_set ? fi[i].depth :
                                                              cfg->num_of_set;
                rv = bcmlt_entry_field_array_get_by_id(eh,
                                                       ...
                                                       &sz);
                if (SHR_SUCCESS(rv)) {
                    rv = ecmp_member_arr_parse(unit, i, num_of_elem, data,
                                               cfg->member_info);
                }
            }
        }

        i = BCMINT_XGS_L3_ECMP_TBL_RH_ECMP_NHOP_UNDERLAY;
        if (ti->fld_bmp & (1 << i)) {
            if (fi[i].depth) {
                sal_memset(data, 0, data_sz);
    ...
    for (i = 0; i < BCMINT_XGS_L3_ECMP_TBL_FLD_CNT; i++) {
        if (i == BCMINT_XGS_L3_ECMP_TBL_ECMP_NHOP_UNDERLAY ||
            i == BCMINT_XGS_L3_ECMP_TBL_RH_ECMP_NHOP_UNDERLAY) {
            /* Already processed. */
            continue;
        }
...
l3_egress_flexctr_op(int unit, bcm_if_t intf_id, l3_egr_flexctr_cfg_t *flexctr)
{
    ...
    switch (nhop_type) {
        ...
        case L3_MPLS_TRANSIT_NHOP_TYPE:
            ...
            flex_ctr_type = BCMINT_LTSW_MPLS_NHOP_FLEXCTR_ACTION;
            break;
        case L3_L2_TAG_NHOP_TYPE:
            tbl_id = BCMINT_LTSW_L3_EGRESS_TBL_L2_TAG;
            flex_ctr_type = BCMINT_LTSW_L3_EGRESS_FLEXCTR_ACTION_CNT;
            break;
        default:
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }

    if (L3_NHOP_L2_TAG_VIEW(unit, ltidx)) {
        tbl_id = BCMINT_LTSW_L3_EGRESS_TBL_L2_TAG;
    }
    ...
    fldname = tbl_info->flds[BCMINT_XGS_L3_EGRESS_TBL_CTR_EGR_EFLEX_ACTION_ID].fld_name;
        ...
        if (flexctr->flags & L3_EGR_FLXCTR_F_ACTION) {
            ...
            if (flexctr->action) {
                if (!L3_NHOP_L2_TAG_VIEW(unit, ltidx)) {
                    ...
                    val = (uint32_t)index;
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_entry_field_add(eh, fldname, val));
                } else {
                    val = flexctr->action;
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_entry_field_add(eh, fldname, val));
    ...
    bool ul)
{
    ...
    /* empty the profile entry */
    SHR_IF_ERR_VERBOSE_EXIT
        (ecmp_flex_action_profile_entry_del(unit, profile_idx, ul));
    ...
    l3_egr_flexctr_cfg_t *flexctr)
{
    ...
    action_name =
        tbl_info->flds[BCMINT_XGS_L3_ECMP_TBL_CTR_EFLEX_ACTION_ID].fld_name;
    object_name =
        tbl_info->flds[BCMINT_XGS_L3_ECMP_TBL_CTR_EFLEX_OBJECT].fld_name;
...
nh_cfg_set(
    ...
    bool field_view, bool l2tag_view, bool ul, nh_cfg_t *nh,
    bcm_l3_egress_t *egr)
{
    uint64_t value;
    uint32_t mpls_flags = egr->mpls_flags;
    int l3_eif;
    bcmi_ltsw_l3_egr_intf_type_t l3_intf_type = BCMI_LTSW_L3_EGR_INTF_T_UL;
    ...
    if (!ipmc_view) {
        ...
        if (egr->flags & BCM_L3_TGID) {
            ...
            if (ul) {
                ...
                /*
                 * ING_UNDERLAY_NHOP_VALID must be set to indicate that
                 * there is an underlay nhop related with this overlay nhop.
                 */
                fv[BCMINT_XGS_L3_EGRESS_TBL_ING_UNDERLAY_NHOP_VALID] = 1;
                nh->fld_bmp |= 1LL << BCMINT_XGS_L3_EGRESS_TBL_ING_UNDERLAY_NHOP_VALID;
            }
        }
    } else if (ipmc_view) {
    ...
    if (egr->intf == BCM_IF_INVALID) {
        ...
        if (!ltsw_feature(unit, LTSW_FT_L3_EGRESS_DEFAULT_UNDERLAY)) {
            if (ul && (l3_intf_type != BCMI_LTSW_L3_EGR_INTF_T_UL) && l3_eif) {
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
            }
            nh->fld_bmp |= 1LL << BCMINT_XGS_L3_EGRESS_TBL_L3_EIF_VALID;
        }
    ...
    if (!mpls_view) {
        ...
        if (mpls_flags & BCM_MPLS_EGRESS_LABEL_EXP_SET ||
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_qos_map_id_resolve(unit, egr->mpls_qos_map_id,
                                              &qos_type, &prof_id));
            if ((bcmiQosMapTypeMplsEgressL2 != qos_type) &&
                (egr->mpls_qos_map_id != 0)) {
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
            }
            fv[BCMINT_XGS_L3_EGRESS_TBL_PHB_EGR_MPLS_ID] = (uint32_t)prof_id;
            fv[BCMINT_XGS_L3_EGRESS_TBL_LABEL_EXP] = egr->mpls_exp;
            nh->fld_bmp |= (1LL << BCMINT_XGS_L3_EGRESS_TBL_EXP_MODE) |
                           (1LL << BCMINT_XGS_L3_EGRESS_TBL_PHB_EGR_MPLS_ID) |
                           (1LL << BCMINT_XGS_L3_EGRESS_TBL_LABEL_EXP);
           ...
           bcm_l3_egress_t *egr)
{
    ...
    if (!ipmc_view) {
        if (!ltsw_feature(unit, LTSW_FT_L3_EGRESS_DEFAULT_UNDERLAY) &&
            ...
            if (fv[BCMINT_XGS_L3_EGRESS_TBL_UNDERLAY_L3_EIF_VALID]) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_l3_egr_index_to_intf(unit,
                                                    (int)fv[BCMINT_XGS_L3_EGRESS_TBL_UNDERLAY_L3_EIF_ID],
                                                    BCMI_LTSW_L3_EGR_INTF_T_UL,
                                                    &egr->intf));
            }
            if (fv[BCMINT_XGS_L3_EGRESS_TBL_OVERLAY_L3_EIF_VALID]) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_l3_egr_index_to_intf(unit,
                                                    (int)fv[BCMINT_XGS_L3_EGRESS_TBL_OVERLAY_L3_EIF_ID],
                                                    BCMI_LTSW_L3_EGR_INTF_T_OL,
                                                    &egr->intf));
            }
        } else {
            if (fv[BCMINT_XGS_L3_EGRESS_TBL_L3_EIF_ID] != 0) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_l3_egr_index_to_intf(unit,
    ...
    if (!mpls_view) {
        ...
        if (value == BCMINT_LTSW_TNL_MPLS_EXP_MODE_FIXED) {
            ...
            rv = bcmi_ltsw_qos_map_id_construct(unit,
                                                ...
                                                &egr->mpls_qos_map_id);
            if (SHR_FAILURE(rv) &&
                !fv[BCMINT_XGS_L3_EGRESS_TBL_PHB_EGR_MPLS_ID]) {
                egr->mpls_qos_map_id = 0;
            }
            egr->mpls_exp = (uint8)fv[BCMINT_XGS_L3_EGRESS_TBL_LABEL_EXP];
        ...
        if ((value == BCMINT_LTSW_TNL_MPLS_LABEL_ACTION_PRESERVE) ||
            ...
            value = fv[BCMINT_XGS_L3_EGRESS_TBL_VPN_LABEL];
            if (value) {
                rv = bcmi_ltsw_ecn_map_id_construct(unit,
...
nh_set(int unit, bool ul, int nhidx, bcm_l3_egress_t *egr)
{
    ...
    if (egr->flags3 & BCM_L3_FLAGS3_L2_TAG_ONLY) {
        tbl_id = BCMINT_LTSW_L3_EGRESS_TBL_L2_TAG;
        nhop_type = L3_L2_TAG_NHOP_TYPE;
        l2tag_view = true;
        ...
        (void)bcmi_ltsw_l3_intf_tnl_init_get(unit, egr->intf, &tnl_type, &tnl_idx);
    ...
    if (L3_NHOP_L2_TAG_VIEW(unit, nhidx)) {
        tbl_id = BCMINT_LTSW_L3_EGRESS_TBL_L2_TAG;
        l2tag_view = true;
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (nh_cfg_set(unit, mpls_view, ipmc_view,
                    transit_view, field_view, l2tag_view, ul, &nh,
                    egr));
    ...
    L3_NHOP_FLAGS2(unit, nhidx) = egr->flags2;
...
nh_get(int unit, int nhidx, bcm_l3_egress_t *egr)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (nh_cfg_get(unit, mpls_view, ipmc_view, transit_view, field_view,
                    l2tag_view, ul, &nh, egr));

    egr->flags2 |= L3_NHOP_FLAGS2(unit, nhidx);
    if (nhop_type == L3_L2_TAG_NHOP_TYPE) {
        egr->flags3 |= BCM_L3_FLAGS3_L2_TAG_ONLY;
    }
...
dlb_member_port_get(int unit, int count, ecmp_member_info_t *member)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_allocate(dunit, tbl_info->name, &eh));
                  ...
                  int count, ecmp_member_info_t *member)
{
    ...
    ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID] = idx;
    ...
    if ((ecmp_info->ecmp_group_flags & BCM_L3_ECMP_MEMBER_REPLICATION_WEIGHTED) ||
        ...
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] = count;
        ecmp.member_info = member;
        ecmp.weighted = true;
        /* Assign weight mode. */
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_MODE] =
            (ecmp_info->ecmp_group_flags & BCM_L3_ECMP_MEMBER_WEIGHTED) ?
                 BCMINT_LTSW_L3_ECMP_MEMBER_WEIGHTED :
                 BCMINT_LTSW_L3_ECMP_MEMBER_REPLICATION;
        ecmp.fld_bmp |= (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_MODE] ==
                         BCMINT_LTSW_L3_ECMP_MEMBER_WEIGHTED) ?
                            (1LL << BCMINT_XGS_L3_ECMP_TBL_WEIGHT) : 0;
        switch (ecmp_info->max_paths) {
        case 128:
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE] =
                BCMINT_LTSW_L3_ECMP_WGT_SZ_128;
            break;
        case 256:
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE] =
                BCMINT_LTSW_L3_ECMP_WGT_SZ_256;
            break;
        case 512:
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE] =
                BCMINT_LTSW_L3_ECMP_WGT_SZ_512;
            break;
        case 1024:
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE] =
                BCMINT_LTSW_L3_ECMP_WGT_SZ_1K;
            break;
        case 2048:
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE] =
                BCMINT_LTSW_L3_ECMP_WGT_SZ_2K;
            break;
        case 4096:
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE] =
                BCMINT_LTSW_L3_ECMP_WGT_SZ_4K;
            break;
        default:
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
        ...
        if (ecmp_info->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT) {
            ...
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] = count;
            ecmp.res_hash = true;
            /* No API for random seed.  Set to be 0 temporarily. */
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_RANDOM_SEED] = 0;
            switch (ecmp_info->dynamic_size) {
            case 64:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_64;
                break;
            case 128:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_128;
                break;
            case 256:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_256;
                break;
            case 512:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_512;
                break;
            case 1024:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_1K;
                break;
            case 2048:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_2K;
                break;
            case 4096:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_4K;
                break;
            case 8192:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_8K;
                break;
            case 16384:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_16K;
                break;
            case 32768:
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE] =
                    BCMINT_LTSW_L3_ECMP_RES_SZ_32K;
                break;
            default:
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
            }
            ...
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] = count;
            ecmp.member_info = member;
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_MAX_PATHS] = ecmp_info->max_paths;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (lt_eh_ecmp_fill(unit, eh, tbl_info, &ecmp));
...
ulecmp_config_del(int unit, int idx)
{
    ...
    fid = tbl_info->flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID].fld_id;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add_by_id(eh, fid, idx));
                   ...
                   bcm_l3_egress_ecmp_t *ecmp_info)
{
    ...
    for (ltidx = ecmp_grp_min; ltidx <= ecmp_grp_max;
        ...
        fid = tbl_info->flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID].fld_id;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add_by_id(eh, fid, ltidx));
        ...
        ecmp.fld_bmp = ecmp.res_hash ? (1LL << BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS) :
                                       (1LL << BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS);
        SHR_IF_ERR_VERBOSE_EXIT
            (lt_eh_ecmp_parse(unit, eh, tbl_info, &ecmp));

        num_paths = ecmp.res_hash ? ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] :
                                    ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
        ...
        ecmp.fld_bmp = wgt_i ? (1LL << BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE) :
                               ((1LL << BCMINT_XGS_L3_ECMP_TBL_LB_MODE) |
                                (ecmp.res_hash ? (1LL << BCMINT_XGS_L3_ECMP_TBL_RH_SIZE) :
                                                 (1LL << BCMINT_XGS_L3_ECMP_TBL_MAX_PATHS)));
        ...
        if (dlb_en) {
            ecmp_info->max_paths = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_MAX_PATHS];
            ...
            ecmp_info->max_paths =
                wgt_sz_val[ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE]];
        } else if (ecmp.res_hash) {
            ecmp_info->dynamic_size =
                res_sz_val[ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE]];
            ecmp_info->dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT;
        } else {
            ecmp_info->max_paths = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_MAX_PATHS];
            if (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_LB_MODE] ==
                BCMINT_LTSW_L3_ECMP_LB_MODE_REGULAR) {
                ecmp_info->dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_DISABLED;
            } else if (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_LB_MODE] ==
                       BCMINT_LTSW_L3_ECMP_LB_MODE_RANDOM) {
                         ...
                         bool alt, int pri_cnt, ecmp_member_info_t *dlb_member)
{
    ...
    if (ecmp.weighted) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (lt_eh_ecmp_parse(unit, eh, tbl_info, &ecmp));
        if (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] ==
            wgt_sz_val[ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_SIZE]]) {
        ...
        if (!ecmp.res_hash && (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] ==
                ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_MAX_PATHS])) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_FULL);
        }

        if (ecmp.res_hash && (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] ==
                res_sz_val[ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_SIZE]])) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_FULL);
        }
    }
    ...
    if (ltsw_feature(unit, LTSW_FT_DLB) &&
        ...
        if (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] < pri_cnt) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
        }
    } else {
        pri_cnt = ecmp.res_hash ? ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS]
                                : ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
    }

    alt_cnt = ecmp.res_hash ?
                  0 : (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] - pri_cnt);

    if (ecmp.res_hash) {
        num_paths = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS];
    } else {
        num_paths = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
    }
    ...
    if (ecmp.res_hash) {
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] = num_paths + 1;
    } else {
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] = num_paths + 1;
    }
    ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] = num_paths + 1;
    ecmp.start_idx = 0;
                         ...
                         bool alt, int pri_cnt, int *member_idx)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_DLB) &&
        L3_ECMP_DLB_GRP_CONFIGURED(unit, idx)) {
        min = alt ? pri_cnt : 0;
        max = alt ? ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] : pri_cnt;
    } else {
        min = 0;
        max = ecmp.res_hash ? ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS]
                            : ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
    }
    for (i = min; i < max; i++) {
    ...
    if (!ecmp.res_hash) {
        ...
        ecmp.num_of_set = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] - i - 1;
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS]--;
        ecmp.num_of_unset = 1;
        ecmp.fld_bmp = (1LL << BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS) |
                       (1LL << BCMINT_XGS_L3_ECMP_TBL_NHOP_ID);
        ...
        ecmp.start_idx = i;
        ecmp.member_info = &arr[i + 1];
        ecmp.num_of_set = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] - i - 1;
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS]--;
        ecmp.num_of_unset = 1;
        ecmp.fld_bmp = (1LL << BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS) |
                       (1LL << BCMINT_XGS_L3_ECMP_TBL_RH_NHOP_ID);
    }
...
ulecmp_config_member_del_all(int unit, int idx)
{
    ...
    ecmp.start_idx = 0;
    ecmp.num_of_set = 0;
    ecmp.num_of_unset = ecmp.res_hash ?
                            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] :
                            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
    if (ecmp.res_hash) {
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] = 0;
        ecmp.fld_bmp = 1LL << BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS;
    } else {
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] = 0;
        ecmp.fld_bmp = 1LL << BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS;
    }
                  ...
                  int *ecmp_member_count)
{
    ...
    if (ecmp_member_size > 0) {
        ...
        *ecmp_member_count = ecmp.res_hash ?
                             ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] :
                             ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
    }
                         ...
                         bool alt, int pri_cnt)
{
    ...
    if (ltsw_feature(unit, LTSW_FT_DLB) &&
        ...
        pri_cnt = ecmp.res_hash ?
                      ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS] :
                      ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
    }

    alt_cnt = ecmp.res_hash ? 0 :
                  (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] - pri_cnt);
    if ((ecmp_grp_flags & BCM_L3_ECMP_PATH_NO_SORTING) &&
        (alt || !alt_cnt)) {
        /* Resilient HASH dosn`t support sorting. */
        if (ecmp.res_hash) {
            /* Add the new member to the tail. */
            ecmp.start_idx = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS];
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS]++;
            ecmp.num_of_set = 1;
            ecmp.member_info = member;
            ...
            /* Add the new member to the tail. */
            ecmp.start_idx = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
            ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS]++;
            ecmp.num_of_set = 1;
            ecmp.member_info = member;
        }
    } else {
        num = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] + 1;
        sz = sizeof(ecmp_member_info_t) * num;
        SHR_ALLOC(arr, sz, "bcmLtswXgsEcmpMemberInfo");
        SHR_NULL_CHECK(arr, SHR_E_MEMORY);
        sal_memset(arr, 0, sz);

        ecmp.num_of_set = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
        ecmp.member_info = arr;
        ...
        if (alt || !alt_cnt) {
            /* Add the new member to the tail*/
            sal_memcpy(&arr[ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS]],
                       member, sizeof(ecmp_member_info_t));
        } else {
            /* Shift the alternate paths. */
            for (i = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS]; i > pri_cnt; i--) {
                sal_memcpy(&arr[i], &arr[i - 1], sizeof(ecmp_member_info_t));
            }
            /* Add the new member before alternate paths. */
            sal_memcpy(&arr[pri_cnt], member, sizeof(ecmp_member_info_t));
        ...
        ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] = num;
        ecmp.start_idx = 0;
        ecmp.num_of_set = num;
    }
...
olecmp_config_member_del_all(int unit, int idx)
{
    ...
    ecmp.start_idx = 0;
    ecmp.num_of_set = 0;
    ecmp.num_of_unset = ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS];
    ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] = 0;
    ecmp.fld_bmp = ecmp.res_hash ? 1LL << BCMINT_XGS_L3_ECMP_TBL_RH_NUM_PATHS :
                                   1LL << BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS;
    ecmp.fld_bmp |= ecmp.res_hash ? ((1LL << BCMINT_XGS_L3_ECMP_TBL_RH_ECMP_UNDERLAY_ID) |
           ...
           int *ecmp_member_count)
{
    ...
    if (dlb_en) {
        if (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_NUM_PATHS] !=
            (dlb.num_paths + dlb.alternate_num_paths)) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
        }
    ...
    if (ecmp_member_size > 0) {
        ...
        *ecmp_member_count = ecmp_member_size < num_paths ?
        ...
        if (ecmp.flds[BCMINT_XGS_L3_ECMP_TBL_WEIGHTED_MODE] ==
            BCMINT_LTSW_L3_ECMP_MEMBER_WEIGHTED) {
...
l3_egress_tables_clear(int unit)
{
    ...
    for (i = 0; i <= BCMINT_LTSW_L3_EGRESS_TBL_MPLS_ENCAP; i++) {
        if (SHR_SUCCESS(bcmint_ltsw_l3_egress_tbl_get(unit, i, &tbl_info))) {
            SHR_IF_ERR_CONT
                (bcmi_lt_clear(unit, tbl_info->name));
        }
    }

    for (j = 0; j < BCMINT_LTSW_L3_ECMP_TBL_CNT; j++) {
        if (SHR_SUCCESS(bcmint_ltsw_l3_ecmp_tbl_get(unit, j, &tbl_info))) {
            SHR_IF_ERR_CONT
                (bcmi_lt_clear(unit, tbl_info->name));
        }
    }
...
l3_egress_recover(int unit)
{
    ...
    uint64_t value;
    bcmint_ltsw_l3_egress_tbl_id_t tbl_id;
    bcmint_ltsw_l3_ecmp_tbl_id_t tbl_id_2;
    const bcmint_ltsw_l3_egress_tbl_t *tbl_info = NULL;
    const char *ltname;
    int local_rv;
    ...
    sal_memset(&nh_cfg, 0, sizeof(nh_cfg));
    ...
    local_rv = bcmint_ltsw_l3_egress_tbl_get(unit, tbl_id, &tbl_info);
    ...
    if (local_rv == SHR_E_NONE) {
        ...
        SHR_IF_ERR_EXIT_EXCEPT_IF(local_rv, SHR_E_UNAVAIL);
    }

    sal_memset(&ecmp_cfg, 0, sizeof(ecmp_cfg));

    tbl_id_2 = BCMINT_LTSW_L3_ECMP_TBL_UL;

    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_ecmp_tbl_get(unit, tbl_id_2, &tbl_info));
    ...
    while ((rv = bcmlt_entry_commit(eh, BCMLT_OPCODE_TRAVERSE,
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (lt_eh_ecmp_parse(unit, eh, tbl_info, &ecmp_cfg));

        /* Don't recover from entry that not eligible for LT operation. */
        if ((ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID] < L3_UL_ECMP_GRP_MIN(unit)) ||
            (ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID] > L3_UL_ECMP_GRP_MAX(unit))) {
            continue;
        }
        L3_UL_ECMP_GRP_FBMP_CLR(unit, ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID]);
    }
    ...
    tbl_id_2 = BCMINT_LTSW_L3_ECMP_TBL_WEIGHTED_UL;
    ...
    while ((rv = bcmlt_entry_commit(eh, BCMLT_OPCODE_TRAVERSE,
        ...
        L3_UL_ECMP_GRP_FBMP_CLR(unit, ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID]);
        L3_UL_ECMP_GRP_WGT_FBMP_CLR(unit, ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID]);
    }
    ...
    if (!ltsw_feature(unit, LTSW_FT_L3_EGRESS_DEFAULT_UNDERLAY)) {
        sal_memset(&ecmp_cfg, 0, sizeof(ecmp_cfg));
        tbl_id_2 = BCMINT_LTSW_L3_ECMP_TBL_OL;
        ...
        while ((rv = bcmlt_entry_commit(eh, BCMLT_OPCODE_TRAVERSE,
            ...
            /* Don't recover from entry that not eligible for LT operation. */
            if ((ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID] < L3_OL_ECMP_GRP_MIN(unit)) ||
                (ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID] > L3_OL_ECMP_GRP_MAX(unit))) {
                continue;
            }
            L3_OL_ECMP_GRP_FBMP_CLR(unit, ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID]);
        }
        ...
        while ((rv = bcmlt_entry_commit(eh, BCMLT_OPCODE_TRAVERSE,
            ...
            L3_OL_ECMP_GRP_FBMP_CLR(unit, ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID]);
            L3_OL_ECMP_GRP_WGT_FBMP_CLR(unit, ecmp_cfg.flds[BCMINT_XGS_L3_ECMP_TBL_ECMP_ID]);
        }
...
l3_nexthop_table_special_init(int unit)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_insert(unit, tbl_info->name, &lt_entry, NULL));
    L3_NHOP_FBMP_CLR(unit, 0);
    L3_NHOP_TYPE(unit, 0) = L3_UC_NHOP_TYPE;
exit:
    SHR_FUNC_EXIT();
}
...
xgs_ltsw_l3_egress_init(int unit)
{
    ...
    bcmint_ltsw_l3_ecmp_tbl_id_t tbl_id;
    bcmint_ltsw_l3_ecmp_flexctr_act_tbl_id_t tbl_id_2;
    const bcmint_ltsw_l3_egress_tbl_t *tbl_info = NULL;
    ...
    /* UC NHOP. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_field_value_range_get(unit, tbl_info->name,
                                       NHOP_IDs, &min, &max));
    ...
    sz = (max + 1) * sizeof(int);
    alloc_sz = sz;
    ei->nh_type = bcmi_ltsw_ha_mem_alloc(unit,
                                         BCMI_HA_COMP_ID_L3_EGR,
                                         BCMINT_LTSW_L3_EGR_HA_NHOP_TYPE,
                                         "bcmL3NhopType",
                                         &alloc_sz);
    SHR_NULL_CHECK(ei->nh_type, SHR_E_MEMORY);
    SHR_IF_ERR_VERBOSE_EXIT
        ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
    if (!bcmi_warmboot_get(unit)) {
        sal_memset(ei->nh_type, 0, alloc_sz);
    }

    /* NHOP flags2. */
    sz = (max + 1) * sizeof(uint32_t);
    alloc_sz = sz;
    ei->nh_flags2 = bcmi_ltsw_ha_mem_alloc(unit,
                                           BCMI_HA_COMP_ID_L3_EGR,
                                           BCMINT_LTSW_L3_EGR_HA_NHOP_FLAGS2,
                                           "bcmL3NhopFlags2",
                                           &alloc_sz);
    SHR_NULL_CHECK(ei->nh_flags2, SHR_E_MEMORY);
    SHR_IF_ERR_VERBOSE_EXIT
        ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
    if (!bcmi_warmboot_get(unit)) {
        sal_memset(ei->nh_flags2, 0, alloc_sz);
    }
    ...
    alloc_sz = sz;
    ei->nh_l2_tag = bcmi_ltsw_ha_mem_alloc(unit,
                                           BCMI_HA_COMP_ID_L3_EGR,
                                           BCMINT_LTSW_L3_EGR_HA_NHOP_L2_TAG,
                                           "bcmL3NhopL2Tag",
                                           &alloc_sz);
    SHR_NULL_CHECK(ei->nh_l2_tag, SHR_E_MEMORY);
    SHR_IF_ERR_VERBOSE_EXIT
        ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
    if (!bcmi_warmboot_get(unit)) {
        sal_memset(ei->nh_l2_tag, 0, alloc_sz);
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
    if (!bcmi_warmboot_get(unit)) {
        sal_memset(ei->ulecmp_grp_flags, 0, alloc_sz);
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
    if (!bcmi_warmboot_get(unit)) {
        sal_memset(ei->ulecmp_grp_lb_mode, 0, alloc_sz);
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
    if (!bcmi_warmboot_get(unit)) {
        sal_memset(ei->ulecmp_grp_max_paths, 0, alloc_sz);
    }
    ...
    if (!ltsw_feature(unit, LTSW_FT_L3_EGRESS_DEFAULT_UNDERLAY)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
        if (!bcmi_warmboot_get(unit)) {
            sal_memset(ei->olecmp_grp_flags, 0, alloc_sz);
        }
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
        if (!bcmi_warmboot_get(unit)) {
            sal_memset(ei->olecmp_grp_lb_mode, 0, alloc_sz);
        }
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            ((alloc_sz < sz) ? SHR_E_MEMORY : SHR_E_NONE);
        if (!bcmi_warmboot_get(unit)) {
            sal_memset(ei->olecmp_grp_max_paths, 0, alloc_sz);
        }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_field_width_get(unit, tbl_info->name, NHOP_IDs, &width));
    ...
    /***************************************************************************/
    tbl_id_2 = BCMINT_LTSW_L3_ECMP_FLEXCTR_ACT_TBL_UL;
    SHR_IF_ERR_EXIT
        (bcmint_ltsw_l3_ecmp_flexctr_action_tbl_get(unit, tbl_id_2, &tbl_info));
    lt_name = tbl_info->name;
    lt_key_name = tbl_info->flds[BCMINT_XGS_L3_ECMP_FLEXCTR_ACT_TBL_ID].fld_name;
    ...
    if (!ltsw_feature(unit, LTSW_FT_L3_EGRESS_DEFAULT_UNDERLAY)) {
            tbl_id_2 = BCMINT_LTSW_L3_ECMP_FLEXCTR_ACT_TBL_OL;
            SHR_IF_ERR_EXIT
                (bcmint_ltsw_l3_ecmp_flexctr_action_tbl_get(unit, tbl_id_2, &tbl_info));
            lt_name = tbl_info->name;
            lt_key_name = tbl_info->flds[BCMINT_XGS_L3_ECMP_FLEXCTR_ACT_TBL_ID].fld_name;
...
exit:
    if (SHR_FUNC_ERR()) {
        ...
        if (!bcmi_warmboot_get(unit)) {
            if (ei->nh_type) {
                (void)bcmi_ltsw_ha_mem_free(unit, ei->nh_type);
            }
...
xgs_ltsw_l3_egress_dvp_set(int unit, bcm_if_t intf, int dvp)
{
    ...
    ltname = tbl_info->name;
    keyname = tbl_info->flds[BCMINT_XGS_L3_EGRESS_TBL_NHOP_ID].fld_name;
...
xgs_ltsw_l3_egress_create(int unit, bcm_l3_egress_t *egr, int *idx)
{
    ...
    if (egr->intf) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_l3_egr_intf_to_index(unit, egr->intf, &l3_eif,
                                            &l3_intf_type));
    }
    if (l3_intf_type == BCMI_LTSW_L3_EGR_INTF_T_UL) {
        if (bcmi_ltsw_virtual_vp_is_vfi_type(unit, egr->port)) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
        ul = true;
    } else if (l3_intf_type == BCMI_LTSW_L3_EGR_INTF_T_OL) {
        if (egr->port && !bcmi_ltsw_virtual_vp_is_vfi_type(unit, egr->port)) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
        }
    }
    ...
    max = ul ? (prot_offset ? (prot_offset - 1) :
                              (L3_UL_NHOP_ENT(unit) - 1)) :
                L3_NHOP_MAX(unit);
    ...
    if (egr->flags & BCM_L3_WITH_ID) {
        ...
        if (prot_offset > 0) {
            if (!(egr->failover_id & BCMI_LTSW_FAILOVER_FIXED_NH_OFFSET_ON) &&
                (nhidx >= prot_offset) && (nhidx < L3_UL_NHOP_ENT(unit))) {
                SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
            }
...
xgs_ltsw_l3_egress_find(int unit, bcm_l3_egress_t *egr, int *idx)
{
    ...
    for (nhidx = min; nhidx <= max; nhidx ++) {
        ...
        /* Skip black hole entry. */
        if (nhidx == L3_EGR_BLACK_HOLE_NH_IDX) {
            continue;
        }
                                    ...
                                    bcmi_ltsw_flexctr_counter_info_t *info)
{
    ...
    info->source = bcmFlexctrSourceL3Egress;
    ...
    switch (nhop_type) {
        ...
        case L3_L2_TAG_NHOP_TYPE:
            tbl_id = BCMINT_LTSW_L3_EGRESS_TBL_L2_TAG;
            action_type = BCMINT_LTSW_L3_EGRESS_FLEXCTR_ACTION_CNT;
            break;
        default:
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
    }
    ...
    if (val == 0) {
        ...
        if (!L3_NHOP_L2_TAG_VIEW(unit, ltidx)) {
            ...
            info->action_index = (uint32_t)val;
        }
    }
    ...
    bcm_l3_ecmp_dlb_mon_cfg_t *dlb_mon_cfg)
{
    ...
    if (!bcmi_ltsw_property_get(unit, BCMI_CPN_DLB_FLOW_MONITOR_EN, 0)) {
    ...
    if (ltidx < L3_ECMP_DLB_GRP_MIN(unit)) {
    ...
    bcm_l3_egress_ecmp_t_init(&ecmp_info);
    SHR_IF_ERR_VERBOSE_EXIT
        (ulecmp_get(unit, ltidx, &ecmp_info, 0,
                    NULL, &ecmp_member_count));
    if (ecmp_info.dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_OPTIMAL) {
        SHR_ERR_EXIT(SHR_E_CONFIG);
    }

    dlb_monitor.enable = dlb_mon_cfg->enable;
    dlb_monitor.sample_rate = dlb_mon_cfg->sample_rate_64;
    if (dlb_monitor.sample_rate > 0) {
    ...
    int *size)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_capacity_get(unit, tbl_info->name, &size_tmp));
    *size = size_tmp;
exit:
    SHR_FUNC_EXIT();
}
    ...
    int *count)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_symbol_add(entry_hdl, TABLE_IDs, tbl_info->name));
                                  ...
                                  bcmi_ltsw_flexctr_counter_info_t *info)
{
    ...
    if (type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_OL) {
        ...
        info->source = bcmFlexctrSourceEcmpOverlay;
    } else if (type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_UL) {
        if (L3_UL_ECMP_GRP_FBMP_GET(unit, ltidx)) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_NOT_FOUND);
        }
        info->source = bcmFlexctrSourceEcmpUnderlay;
        ul = true;
    } else {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_PARAM);
    }

    ecmp_group_flags = ul ? L3_UL_ECMP_GRP_FLAGS(unit, ltidx) :
                            L3_OL_ECMP_GRP_FLAGS(unit, ltidx);
    ...
    fldname = tbl_info->flds[BCMINT_XGS_L3_ECMP_TBL_CTR_EFLEX_ACTION_ID].fld_name;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_get(eh, fldname, &val));
    ...
    bcmi_ltsw_l3_egr_l2_tag_info_t *l2_tag_info)
{
    ...
    if (nhop_type == L3_UC_NHOP_TYPE) {
        ...
        tbl_id = BCMINT_LTSW_L3_EGRESS_TBL_MC;
    } else if (nhop_type == L3_L2_TAG_NHOP_TYPE) {
        tbl_id = BCMINT_LTSW_L3_EGRESS_TBL_L2_TAG;
    } else {
        SHR_IF_ERR_VERBOSE_EXIT(SHR_E_INTERNAL);
    }
    ...
    action_id = (int)nh.flds[BCMINT_XGS_L3_EGRESS_TBL_CTR_EGR_EFLEX_ACTION_ID];

    if (action_id && nhop_type != L3_L2_TAG_NHOP_TYPE) {
        SHR_IF_ERR_VERBOSE_EXIT
            (ecmp_flex_action_profile_get(unit, action_id, true, &action_profile));

        SHR_IF_ERR_VERBOSE_EXIT
            (ecmp_flex_action_profile_delete(unit, action_id, true));
    }
    ...
    nh.fld_bmp = tbl_info->fld_bmp;
    if (nhop_type == L3_L2_TAG_NHOP_TYPE) {
        nh.flds[BCMINT_XGS_L3_EGRESS_TBL_CTR_EGR_EFLEX_ACTION_ID] = action_id;
    } else {
        nh.flds[BCMINT_XGS_L3_EGRESS_TBL_CTR_EGR_EFLEX_ACTION_ID] = action_profile.action;
    }
    ...
    if (l2_tag_info->is_phb_map_id) {
        ...
        nh.fld_bmp &= ~ (1LL << BCMINT_XGS_L3_EGRESS_TBL_OCFI);
        nh.fld_bmp &= ~ (1LL << BCMINT_XGS_L3_EGRESS_TBL_OPRI);
    } else {
        nh.flds[BCMINT_XGS_L3_EGRESS_TBL_OCFI] = l2_tag_info->cfi;
        nh.flds[BCMINT_XGS_L3_EGRESS_TBL_OPRI] = l2_tag_info->pri;
        nh.fld_bmp &= ~ (1LL << BCMINT_XGS_L3_EGRESS_TBL_PHB_EGR_L2_INT_PRI_TO_OTAG_ID);
    }
    ...
    if (nhop_type == L3_UC_NHOP_TYPE) {
        if (L3_UL_NHOP_CHECK(unit, nhidx)) {
            nh.flds[BCMINT_XGS_L3_EGRESS_TBL_UNDERLAY_L3_EIF_VALID] = 1;
            nh.flds[BCMINT_XGS_L3_EGRESS_TBL_UNDERLAY_L3_EIF_ID] =
                nh.flds[BCMINT_XGS_L3_EGRESS_TBL_L3_EIF_ID];
        } else {
            nh.flds[BCMINT_XGS_L3_EGRESS_TBL_OVERLAY_L3_EIF_VALID] = 1;
            nh.flds[BCMINT_XGS_L3_EGRESS_TBL_OVERLAY_L3_EIF_ID] =
                nh.flds[BCMINT_XGS_L3_EGRESS_TBL_L3_EIF_ID];
        }
    }
    ...
    if (nhop_type != L3_L2_TAG_NHOP_TYPE) {
        L3_NHOP_L2_TAG_VIEW(unit, nhidx) = 1;
    }
    ...
    bcmi_ltsw_l3_egr_l2_tag_info_t *l2_tag_info)
{
    ...
    if (L3_NHOP_L2_TAG_VIEW(unit, nhidx) ||
        L3_NHOP_TYPE(unit, nhidx) == L3_L2_TAG_NHOP_TYPE) {
        tbl_id = BCMINT_LTSW_L3_EGRESS_TBL_L2_TAG;
    } else {
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_entry_commit(unit, eh, BCMLT_OPCODE_LOOKUP,
                              BCMLT_PRIORITY_NORMAL));
    nh.fld_bmp |= (1LL << BCMINT_XGS_L3_EGRESS_TBL_IS_PHB_MAP_ID);
    ...
    int nhidx)
{
    ...
    l3_egress_flex_action_profile_t action_profile;
    int action_id = 0;
    bcmi_ltsw_l3_egr_l2_tag_info_t l2_tag_info = {0};
    ...
    if (nh.flds[BCMINT_XGS_L3_EGRESS_TBL_CTR_EGR_EFLEX_ACTION_ID] &&
        nhop_type != L3_L2_TAG_NHOP_TYPE) {
        action_profile.action =
            (uint32_t)nh.flds[BCMINT_XGS_L3_EGRESS_TBL_CTR_EGR_EFLEX_ACTION_ID];
        SHR_IF_ERR_VERBOSE_EXIT
            (l3_egress_flex_action_profile_add(unit,
                                               BCMINT_LTSW_L3UC_NHOP_FLEXCTR_ACTION,
                                               &action_profile,
    ...
    nh.flds[BCMINT_XGS_L3_EGRESS_TBL_ES_IDENTIFIER] = l2_tag_info.es_id;
    if (nhop_type != L3_L2_TAG_NHOP_TYPE) {
        nh.flds[BCMINT_XGS_L3_EGRESS_TBL_CTR_EGR_EFLEX_ACTION_ID] = (uint32_t)action_id;
    }

    if (nhop_type == L3_UC_NHOP_TYPE) {
        if (nh.flds[BCMINT_XGS_L3_EGRESS_TBL_OVERLAY_L3_EIF_VALID]) {
            nh.flds[BCMINT_XGS_L3_EGRESS_TBL_L3_EIF_VALID] = 1;
            nh.flds[BCMINT_XGS_L3_EGRESS_TBL_L3_EIF_ID] =
                nh.flds[BCMINT_XGS_L3_EGRESS_TBL_OVERLAY_L3_EIF_ID];
        } else if (nh.flds[BCMINT_XGS_L3_EGRESS_TBL_UNDERLAY_L3_EIF_VALID]) {
            nh.flds[BCMINT_XGS_L3_EGRESS_TBL_L3_EIF_VALID] = 1;
            nh.flds[BCMINT_XGS_L3_EGRESS_TBL_L3_EIF_ID] =
                nh.flds[BCMINT_XGS_L3_EGRESS_TBL_UNDERLAY_L3_EIF_ID];
        }
    ...
    if (nhop_type != L3_L2_TAG_NHOP_TYPE) {
        L3_NHOP_L2_TAG_VIEW(unit, nhidx) = 0;
    }
