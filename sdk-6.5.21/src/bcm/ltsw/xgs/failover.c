...
int
xgs_ltsw_failover_clear(int unit)
{
    SHR_FUNC_ENTER(unit);
...
int
xgs_ltsw_failover_init(int unit)
{
    uint16_t prot_offset;
    uint32_t nh_tbl_size, ol_nhop_size;
    uint64_t min, max;
    ...
    if (bcmi_ltsw_property_get(unit, BCMI_FAILOVER_FIXED_NH_OFFSET_ENABLE, 0)) {
        ...
        /* Set NH offset to half size of underlay next hop */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_field_value_range_get(unit, tbl_info->name, NHOP_IDs, &min, &max));
        nh_tbl_size = max + 1;
        ol_nhop_size = bcmi_ltsw_property_get(unit, BCMI_CPN_RIOT_OVERLAY_L3_EGRESS_MEM_SIZE, 0);
        if (ol_nhop_size) {
            prot_offset = (nh_tbl_size - ol_nhop_size) / 2;
        } else {
...
int
xgs_ltsw_failover_egress_status_set(
    int unit,
    int nh_idx,
    int enable)
{
...
int
xgs_ltsw_failover_egress_status_get(
    int unit,
    int nh_idx,
    int *enable)
{
