        ...
        uint32 *profile_index)
{
    ...
    *profile_index = index;
    ...
    if (pyld_zero_offset->l2_offset == -1)  {
        field[1].u.val = 0;
    } else {
    ...
    if (pyld_zero_offset->l3_offset == -1)  {
        field[3].u.val = 0;
    } else {
    ...
    if (pyld_zero_offset->udp_offset == -1)  {
        field[5].u.val = 0;
    } else {
...
/*!
 * \brief Set the keys of ingress flex mirror sample entry.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
mirror_ing_flex_sample_entry_keys_set(int unit, int pipe,
                                      int profile_id,
                                      bcmlt_entry_handle_t *eh)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(*eh, MIRROR_ING_FLEX_SFLOW_IDs, profile_id));
...
/*!
 * \brief Set mirror ingress flex sample entry fields.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
mirror_ing_flex_sample_entry_fields_set(int unit, bcmlt_entry_handle_t eh,
                                        bcmint_mirror_sample_profile_t *entry)
{
    uint64_t threshold = 0;
    uint64_t threshold_max = 0;
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(entry, SHR_E_PARAM);

    if (entry->fld_bmp & BCMINT_MIRROR_SAMPLE_PRF_FLD_ENABLE) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(eh, SAMPLEs, entry->enable));
    }
    ...
    if (entry->fld_bmp & BCMINT_MIRROR_SAMPLE_PRF_FLD_RATE) {
        if (entry->rate > 0) {
            ...
            } else if (entry->rate <= threshold_max) {
                threshold = (threshold_max + 1) / entry->rate;
            } else {
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(eh, SAMPLE_RATEs, threshold));
    }

    if (entry->fld_bmp & BCMINT_MIRROR_SAMPLE_PRF_FLD_CPU) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(eh, SAMPLE_TO_CPUs, entry->copy_to_cpu));
    }

    if (entry->fld_bmp & BCMINT_MIRROR_SAMPLE_PRF_FLD_MIRROR) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(eh, SAMPLE_MIRROR_ING_INSTANCEs,
                                   entry->mirror));
    }
 ...
 */
static int
mirror_ing_flex_sample_entry_fields_get(int unit, bcmlt_entry_handle_t eh,
                                        bcmint_mirror_sample_profile_t *entry)
{
    uint64_t value = 0;
    uint64_t threshold_max = 0;
    ...
    if (entry->fld_bmp & BCMINT_MIRROR_SAMPLE_PRF_FLD_ENABLE) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(eh, SAMPLEs, &value));
        entry->enable = (bool)value;
    }

    if (entry->fld_bmp & BCMINT_MIRROR_SAMPLE_PRF_FLD_RATE) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(eh, SAMPLE_RATEs, &value));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_field_value_range_get(unit,
                                           MIRROR_ING_FLEX_SFLOWs,
                                           SAMPLE_RATEs,
                                           ...
                                           &threshold_max));
        if (value) {
            entry->rate = (threshold_max + 1) / value;
        } else {
            entry->rate = 0;
        }
    }
    ...
    if (entry->fld_bmp & BCMINT_MIRROR_SAMPLE_PRF_FLD_CPU) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(eh, SAMPLE_TO_CPUs, &value));
        entry->copy_to_cpu = (bool)value;
    }

    if (entry->fld_bmp & BCMINT_MIRROR_SAMPLE_PRF_FLD_MIRROR) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(eh, SAMPLE_MIRROR_ING_INSTANCEs, &value));
        entry->mirror = (bool)value;
    }
                          ...
                          bcm_mirror_sample_profile_t *profile)
{
    ...
    entry.fld_bmp |= BCMINT_MIRROR_SAMPLE_PRF_FLD_CPU;
    entry.copy_to_cpu = profile->copy_to_cpu;

    entry.fld_bmp |= BCMINT_MIRROR_SAMPLE_PRF_FLD_MIRROR;
    entry.mirror = profile->mirror;

    dunit = bcmi_ltsw_dev_dunit(unit);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_allocate(dunit, MIRROR_ING_FLEX_SFLOWs, &eh));

    num_pipe = bcmi_ltsw_dev_max_pp_pipe_num(unit);
    for (pipe = 0; pipe < num_pipe; pipe++) {
        /* Set the keys of sample entry. */
        SHR_IF_ERR_VERBOSE_EXIT
            (mirror_ing_flex_sample_entry_keys_set(unit, pipe, profile_id, &eh));

        /* Set the fields of sample entry. */
        SHR_IF_ERR_VERBOSE_EXIT
            (mirror_ing_flex_sample_entry_fields_set(unit, eh, &entry));
                          ...
                          bcm_mirror_sample_profile_t *profile)
{
    ...
    /* Set the keys of sample entry. */
    SHR_IF_ERR_VERBOSE_EXIT
        (mirror_ing_flex_sample_entry_keys_set(unit, 0, profile_id, &eh));
    ...
    entry.fld_bmp |= BCMINT_MIRROR_SAMPLE_PRF_FLD_ENABLE |
                     ...
                     BCMINT_MIRROR_SAMPLE_PRF_FLD_CPU |
                     BCMINT_MIRROR_SAMPLE_PRF_FLD_MIRROR;

    /* Get the fields of sample entry. */
    SHR_IF_ERR_VERBOSE_EXIT
        (mirror_ing_flex_sample_entry_fields_get(unit, eh, &entry));

    profile->enable = entry.enable;
    profile->rate = entry.rate;
    profile->copy_to_cpu = entry.copy_to_cpu;
    profile->mirror = entry.mirror;
...
/*!
 * \brief Set the keys of CTR_MIRROR_ING_FLEX_SFLOW LT.
 *
 * \param [in] unit Unit number.
 * \param [in] pipe Pipe number.
 * \param [in] profile_id Sample profile id.
 ...
 */
static int
mirror_stat_sample_entry_keys_set(int unit, int pipe,
                                  int profile_id,
                                  bcmlt_entry_handle_t eh)
{
    SHR_FUNC_ENTER(unit);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_field_add(eh, CTR_MIRROR_ING_FLEX_SFLOW_IDs, profile_id));
 ...
 * \param [in] unit Unit Number.
 * \param [in] profile_id Sample profile id.
 * \param [in] type Sample statistic type.
 ...
 */
static int
mirror_sample_stat_set(int unit,
                       int profile_id,
                       ...
                       uint64_t value)
{
    ...
    dunit = bcmi_ltsw_dev_dunit(unit);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_allocate(dunit, CTR_MIRROR_ING_FLEX_SFLOWs, &eh));

    num_pipe = bcmi_ltsw_dev_max_pp_pipe_num(unit);
    for (pipe = 0; pipe < num_pipe; pipe++) {
        /* Set the keys of sample entry. */
        SHR_IF_ERR_VERBOSE_EXIT
            (mirror_stat_sample_entry_keys_set(unit,
                                               pipe, profile_id,
                                               eh));
        switch (type) {
            case bcmMirrorSampleStatFlexPkts:
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(eh, SAMPLE_POOL_PKTs, value));
                break;
            case bcmMirrorSampleStatFlexSampledPkts:
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(eh, SAMPLE_PKTs, value));
                break;
            case bcmMirrorSampleStatFlexSnapshotPkts:
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(eh, SAMPLE_POOL_PKT_SNAPSHOTs,
                                           value));
                break;
            default:
                SHR_ERR_EXIT(SHR_E_PARAM);
 ...
 * \param [in] profile_id Sample profile id.
 * \param [in] type Sample type.
 * \param [out] value Returned sample statistic value.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
mirror_sample_stat_get(int unit,
                       int profile_id,
                       bcm_mirror_sample_stat_type_t type,
                       uint64_t *value)
{
    ...
    *value = 0;
    ...
    for (pipe = 0; pipe < num_pipe; pipe++) {
        ...
        switch (type) {
            case bcmMirrorSampleStatFlexPkts:
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_get(eh, SAMPLE_POOL_PKTs, &cnt));
                break;
            case bcmMirrorSampleStatFlexSampledPkts:
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_get(eh, SAMPLE_PKTs, &cnt));
                break;
            case bcmMirrorSampleStatFlexSnapshotPkts:
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_get(eh, SAMPLE_POOL_PKT_SNAPSHOTs, &cnt));
                break;
            default:
                SHR_ERR_EXIT(SHR_E_PARAM);
                break;
...
/*!
 * \brief Init CTR_MIRROR_ING_FLEX_SFLOW table.
 *
 * \param [in] unit Unit Number.
 * \param [in] type Sample type.
 * \param [in] profile_id Sample profile id.
 ...
 */
static int
mirror_sample_stat_init(int unit)
{
    bcmlt_entry_handle_t eh = BCMLT_INVALID_HDL;
    int pipe = 0, num_pipe = 0, profile_id = 0;
    int dunit = 0;
    uint64_t min = 0, max = 0;
    SHR_FUNC_ENTER(unit);

    /* Init flex sample profile table.*/
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_clear(unit, MIRROR_ING_FLEX_SFLOWs));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_clear(unit, CTR_MIRROR_ING_FLEX_SFLOWs));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_field_value_range_get(unit,
                                       CTR_MIRROR_ING_FLEX_SFLOWs,
                                       CTR_MIRROR_ING_FLEX_SFLOW_IDs,
                                       &min,
                                       &max));
    ...
    num_pipe = bcmi_ltsw_dev_max_pp_pipe_num(unit);
    for (pipe = 0; pipe < num_pipe; pipe++) {
        for (profile_id = min; profile_id <= max; profile_id++) {
        /* Set the keys of sample entry. */
        SHR_IF_ERR_VERBOSE_EXIT
            (mirror_stat_sample_entry_keys_set(unit,
                                               pipe, profile_id,
...
mirror_ingress_mod_bk_init(int unit)
{
    ...
    alloc_size = SHR_BITALLOCSIZE(MIRROR_ING_MOD_EVENT_PROF_NUM(unit));
...
{
    int i;
    SHR_BITDCL *bitmap = ING_MOD_EVENT_PROF_BMP(unit);
...
mirror_ingress_mod_event_profile_delete(int unit, int id)
{
    ...
    if (!ING_MOD_EVENT_PROF_BMP_USED_GET(unit, id)) {
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }
...
{
    int i, id, group_num;
    SHR_BITDCL *group_bmp_new_temp = NULL, *mask_bmp_new_temp = NULL;
    bcmi_lt_entry_t lt_entry;
    bcmi_lt_field_t field[] =
    {
        /*0*/  {EVENT_GROUP_0s, BCMI_LT_FIELD_F_SET, 0, {0}},
    ...
    group_num = MIRROR_ING_MOD_EVENT_GRP_NUM(unit);
    SHR_ALLOC(group_bmp_new_temp, SHR_BITALLOCSIZE(group_num), "ltswMirrorIngModGrpBmp");
    SHR_NULL_CHECK(group_bmp_new_temp, SHR_E_MEMORY);
    sal_memset(group_bmp_new_temp, 0, SHR_BITALLOCSIZE(group_num));
    *group_bmp_new_temp = group_bmp_new;

    SHR_ALLOC(mask_bmp_new_temp, SHR_BITALLOCSIZE(group_num), "ltswMirrorIngModGrpBmp");
    SHR_NULL_CHECK(mask_bmp_new_temp, SHR_E_MEMORY);
    sal_memset(mask_bmp_new_temp, 0, SHR_BITALLOCSIZE(group_num));
    *mask_bmp_new_temp = group_mask_bmp_new;
    ...
    for (i = 0; i < MIRROR_ING_MOD_EVENT_GRP_NUM(unit); i++) {
        id = i + BCMINT_MIRROR_ING_MOD_EVENT_GRP_ID_BASE;
        field[i].u.val = SHR_BITGET(group_bmp_new_temp, id) ? 1 : 0;
        field[i + MIRROR_ING_MOD_EVENT_GRP_NUM(unit)].u.val =
            SHR_BITGET(mask_bmp_new_temp, id) ? 1 : 0;
    }
...
exit:
    SHR_FREE(group_bmp_new_temp);
    SHR_FREE(mask_bmp_new_temp);
    SHR_FUNC_EXIT();
}
...
mirror_multi_dip_bk_init(int unit)
{
    ...
    bool multi_dip_alloc_success = FALSE;
    uint32_t longest_alloc_success_num = 0;
        ...
        if (MIRROR_MULTI_DIP_CTRL(unit)->group_num != 0) {
            ...
            if (!warm && MIRROR_MULTI_DIP_CTRL(unit)->multi_dip_allocator) {
                for (i = 0; i < MIRROR_MULTI_DIP_ALLOC_NUM; i++) {
                    dip_allocator_tmp = &MIRROR_MULTI_DIP_ALLOC(unit, i);
                    if (i < longest_alloc_success_num) {
                        sal_free(dip_allocator_tmp->longest);
                        dip_allocator_tmp->longest = NULL;
                    }
                }
                if (multi_dip_alloc_success) {
                    sal_free(MIRROR_MULTI_DIP_CTRL(unit)->multi_dip_allocator);
                    MIRROR_MULTI_DIP_CTRL(unit)->multi_dip_allocator = NULL;
                }
            }
        }
        MIRROR_MULTI_DIP_CTRL(unit)->inited = false;
        ...
        if (dip_group.ipv6) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (mirror_multi_dip_allocator_copy_at_offset(unit,
                                                           ...
                                                           allocator,
                                                           offset));
            /* Also update IPv4_index1 allocator. */
...
xgs_ltsw_mirror_misc_init(int unit)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_ltsw_mirror_sample_profile_init(unit));
...
int
xgs_ltsw_mirror_sample_profile_init(int unit)
{
    int warm = bcmi_warmboot_get(unit);
    SHR_FUNC_ENTER(unit);
    ...
    if (!warm) {
        SHR_IF_ERR_VERBOSE_EXIT
            (xgs_ltsw_mirror_sample_profile_deinit(unit));
    }
...
int
xgs_ltsw_mirror_sample_profile_deinit(int unit)
{
    SHR_FUNC_ENTER(unit);
...
int
xgs_ltsw_mirror_sample_profile_set(
    int unit,
...
int
xgs_ltsw_mirror_sample_profile_get(
    int unit,
    bcmint_mirror_sample_type_t type,
    int profile_id,
    bcm_mirror_sample_profile_t *profile)
...
int
xgs_ltsw_mirror_sample_stat_set(
    int unit,
    int profile_id,
    bcm_mirror_sample_stat_type_t type,
    uint64_t value)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mirror_sample_stat_set(unit, profile_id, type, value));
...
int
xgs_ltsw_mirror_sample_stat_get(
    int unit,
    int profile_id,
    bcm_mirror_sample_stat_type_t type,
    uint64_t *value)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mirror_sample_stat_get(unit, profile_id, type, value));
...
{
    int i, group_num;
    SHR_BITDCL *group_bmp_new = NULL, *mask_bmp_new = NULL;
    ...
    group_num = MIRROR_ING_MOD_EVENT_GRP_NUM(unit);
    SHR_ALLOC(group_bmp_new, SHR_BITALLOCSIZE(group_num), "ltswMirrorIngModGrpBmp");
    SHR_NULL_CHECK(group_bmp_new, SHR_E_MEMORY);
    sal_memset(group_bmp_new, 0, SHR_BITALLOCSIZE(group_num));
    *group_bmp_new = profile.group_bmp;

    SHR_ALLOC(mask_bmp_new, SHR_BITALLOCSIZE(group_num), "ltswMirrorIngModGrpBmp");
    SHR_NULL_CHECK(mask_bmp_new, SHR_E_MEMORY);
    sal_memset(mask_bmp_new, 0, SHR_BITALLOCSIZE(group_num));
...
exit:
    SHR_FREE(group_bmp_new);
    SHR_FREE(mask_bmp_new);
    SHR_FUNC_EXIT();
}
