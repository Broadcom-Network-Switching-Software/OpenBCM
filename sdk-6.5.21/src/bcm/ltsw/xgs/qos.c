...
static uint32_t qos_size[QoSMapTypeCount];
...
#define QOS_EGRESS_TABLE_ALL \
            ...
            PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAGs

/*! QoS egress table number. */
#define QOS_EGRESS_TABLE_NUM 9
...
qos_table_clear_all(int unit)
{
    ...
    for (i = 0; i < QOS_EGRESS_TABLE_NUM; i++) {
        if (!sal_strcmp(egress[i], PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAGs)) {
            rv = bcmint_qos_lt_get(
                     unit,
    ...
    switch (map->type) {
        case bcmiQosMapTypeL2Egress:
            ...
            if (SHR_SUCCESS(rv)) {
                ...
                flds = lt_info->flds;
                fld_name = flds[BCMINT_LTSW_QOS_FLD_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_ID].name;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl, fld_name, id));
                fld_name = flds[BCMINT_LTSW_QOS_FLD_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_INT_PRI].name;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl, fld_name,
                                           map_tmp->int_pri));
                int_cng = BCMI_LTSW_QOS_COLOR_ENCODING(map_tmp->color);
                fld_name = flds[BCMINT_LTSW_QOS_FLD_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_CNG].name;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl, fld_name, int_cng));
                fld_name = flds[BCMINT_LTSW_QOS_FLD_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_PRI].name;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl, fld_name,
                                           map_tmp->pkt_pri));
                fld_name = flds[BCMINT_LTSW_QOS_FLD_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_CFI].name;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl, fld_name,
                                           map_tmp->pkt_cfi));
                /* Add entry into transaction. */
...
{
    int dunit = bcmi_ltsw_dev_dunit(unit);
    int i, int_cng, id, color, int_pri, exp, rv;
    bcm_qos_map_t *map_tmp;
    int set_sz;
    bcmlt_entry_handle_t entry_hdl = BCMLT_INVALID_HDL;
    bcmlt_transaction_hdl_t trans_hdl = BCMLT_INVALID_HDL;
    ...
    switch (map->type) {
        case bcmiQosMapTypeL2Egress:
            ...
            if (SHR_SUCCESS(rv)) {
                ...
                for (i = 0; i < set_sz; i++) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_entry_allocate(dunit, lt_info->name,
                                              &entry_hdl));
                    flds = lt_info->flds;
                    fld_name = flds[BCMINT_LTSW_QOS_FLD_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_ID].name;
                    SHR_IF_ERR_VERBOSE_EXIT
                    ...
                    fld_name = flds[BCMINT_LTSW_QOS_FLD_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_INT_PRI].name;
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_entry_field_add(entry_hdl, fld_name, int_pri));
                    /* No need to do Color encode, serve as CNG index */
                    int_cng = color;
                    fld_name = flds[BCMINT_LTSW_QOS_FLD_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_CNG].name;
                    SHR_IF_ERR_VERBOSE_EXIT
...
qos_table_init(int unit)
{
    ...
    /* Use MPLS view to set default value for PT EGR_MPLS_EXP_MAPPING_2. */
    rv = bcmint_qos_lt_get(
             unit,
             BCMINT_LTSW_QOS_LT_PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG,
             &lt_info);
...
/*!
 * \brief Clear QoS L2 egress map on a port.
 *
 * \param [in] unit Unit Number.
 * \param [in] port Port/Gport id.
 *
 ...
 */
static int
qos_port_egr_l2_map_clear(int unit, bcm_gport_t port) {
    int dunit = bcmi_ltsw_dev_dunit(unit);
...
qos_port_egr_map_clear(int unit, bcm_gport_t port)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (qos_port_egr_l2_map_clear(unit, port));

    /* Set mapping ptr. */
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (bcmi_ltsw_port_tab_set(unit, port, BCMI_PT_EGR_DSCP_PHB, 0),
         SHR_E_UNAVAIL);
...
/*!
 * \brief Set QoS L2 egress map on a port.
 *
 * \param [in] unit Unit Number.
 * \param [in] port Port/Gport id.
 * \param [in] map_id QoS Map id.
 ...
 */
static int
qos_port_egr_l2_map_set(int unit, bcm_gport_t port, int map_id)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_add(entry_hdl, CNGs, int_cng));
        if (id == 0) {
            /* Use default value, keep the same behavior as legacy chips */
            if (int_pri > 7) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl, PRIs, 0));
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmlt_entry_field_add(entry_hdl, PRIs, int_pri));
                if (int_cng == BCMI_LTSW_QOS_COLOR_RED) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_entry_field_add(entry_hdl, CFIs, 1));
                } else {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmlt_entry_field_add(entry_hdl, CFIs, 0));
                }
...
qos_port_egr_map_set(int unit, bcm_gport_t port, int map_id)
{
    ...
    if (map_type == bcmiQosMapTypeL2Egress) {
        SHR_IF_ERR_VERBOSE_EXIT
            (qos_port_egr_l2_map_set(unit, port, map_id));
        SHR_EXIT();
    }
...
qos_port_map_set(int unit, bcm_gport_t port, int ing_map, int egr_map)
{
    ...
    if (BCM_GPORT_IS_SET(port) && !BCM_GPORT_IS_MODPORT(port) &&
        ...
        if (BCM_GPORT_IS_FLOW_PORT(port)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_virtual_qos_port_map_set(unit, port, ing_map,
                                                    egr_map));
        } else {
            /* Not support tunnel port. */
            SHR_ERR_EXIT(SHR_E_UNAVAIL);
        }
    } else {
        /* Validate port. */
...
/*!
 * \brief Get QoS L2 egress map on a port.
 *
 * \param [in] unit Unit Number.
 ...
 */
static int
qos_port_egr_l2_map_get(int unit, bcm_port_t port, int *map_id)
{
    qos_cfg_t *qos_db = NULL;
...
qos_port_egr_map_get(int unit, bcm_gport_t port, uint32_t flags, int *map_id)
{
    ...
    if (type == QoSMapTypeL2Egress) {
        flags_tmp &= ~BCM_QOS_MAP_L2;
        SHR_IF_ERR_VERBOSE_EXIT
            (qos_port_egr_l2_map_get(unit, port, map_id));
        /* More flags that could not be handled. */
        if (flags_tmp) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
...
qos_port_map_get(int unit, bcm_gport_t port, uint32_t flags, int *map_id)
{
    ...
    if (BCM_GPORT_IS_SET(port) && !BCM_GPORT_IS_MODPORT(port) &&
        ...
        if (BCM_GPORT_IS_FLOW_PORT(port)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_virtual_qos_port_map_type_get(unit, port, flags,
                                                         map_id));
        } else {
            /* Not support tunnel port. */
            SHR_ERR_EXIT(SHR_E_UNAVAIL);
