...
static port_control_map_t xgs_port_control_port_map[] = {
    ...
    {bcmPortControlFlowCountClassId, BCMI_PT_PORT_CONTEXT, NULL, NULL},
    {bcmPortControlDiscardEgress, BCMI_PT_FP_EGR_DROP, NULL, NULL},
...
static ltsw_port_tab_info_t port_tab_map_def[BCMI_PT_CNT] = {
    ...
    [BCMI_PT_PORT_CONTEXT]                      = {&port, PORT_CONTEXTs },
    ...
    [BCMI_PT_ING_CFI_AS_CNG]                    = {&port_policy, ING_CFI_AS_CNGs, FLD_IS_ARRAY | SYNC_TO_LPORT},
    ...
    */
    ...
    [BCMI_PT_FP_EGR_DROP]                       = {&port_fp, FP_EGR_DROPs,},
...
static ltsw_port_tab_info_t lport_tab_map_def[BCMI_PT_CNT] = {
    ...
    [BCMI_PT_ING_CFI_AS_CNG]                    = {&lport_policy, ING_CFI_AS_CNGs, FLD_IS_ARRAY},
                                   ...
                                   int index_offset, int index_num)
{
    ...
    int len = 0, i, type_num = COUNTOF(fwd_action_t);
    bool symbol = 0;
...
xgs_port_profile_data_get(int unit, const char *lt_name,
                               ...
                               profile_transfer_f transfer)
{
    uint64_t fld_val[DEPTH_ARRAY_FIELD] = { 0, 0, 0,0 };
...
{
    bcmi_ltsw_port_tab_t  types[3];
    uint32_t valid_flags, data[3];
    ...
    types[0] = BCMI_PT_MAC_MOVE_DROP;
    data[0] = (flags & BCM_PORT_LEARN_FWD) ? 0 : 1;

    types[1] = BCMI_PT_MAC_MOVE_COPY_TO_CPU;
    data[1] = (flags & BCM_PORT_LEARN_CPU) ? 1 : 0;

    types[2] = BCMI_PT_MAC_MOVE;
    data[2] = (flags & BCM_PORT_LEARN_ARL) ? 1 : 0;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_set(unit, port, types, data, COUNTOF(types)));
...
xgs_ltsw_port_move_get(int unit, bcm_port_t port, uint32_t *flags)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_get(unit, port, types, data, 3));
...
static int
xgs_port_sample_set(int unit, bcm_port_t port, bcm_port_control_t ctrl_type, int value)
{
    int fld_num = 0;
    uint32_t width = 0, data[3];
    ...
    case bcmPortControlSampleIngressDest:
        ...
        types[fld_num] = BCMI_PT_MIRROR_ING_ENCAP_SAMPLE_CPU;
        data[fld_num] = (value & BCM_PORT_CONTROL_SAMPLE_DEST_CPU) ? 1 : 0;
        fld_num++;

        types[fld_num] = BCMI_PT_MIRROR_ING_ENCAP_SAMPLE_INSTANCE;
        data[fld_num] = (value & BCM_PORT_CONTROL_SAMPLE_DEST_MIRROR) ? 1 : 0;
        fld_num++;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_multi_set(unit, port, types, data, fld_num));
        break;
    case bcmPortControlSampleIngressRate:
        ...
        types[fld_num] = BCMI_PT_MIRROR_ING_ENCAP_SAMPLE_RATE;
        if (value == 0) {
            data[fld_num] = 0;
        } else {
            data[fld_num] = (value == 1) ? field_val : ((field_val + 1) / value);
        }
        fld_num++;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_multi_set(unit, port, types, data, fld_num));
    ...
    case bcmPortControlSampleEgressRate:
        ...
        types[fld_num] = BCMI_PT_MIRROR_EGR_ENCAP_SAMPLE_RATE;
        if (value == 0) {
            data[fld_num] = 0;
        } else {
            data[fld_num] = (value == 1) ? field_val : ((field_val + 1) / value);
...
static int
xgs_port_sample_get(int unit, bcm_port_t port, bcm_port_control_t ctrl_type, int *value)
{
    uint32_t width = 0;
    uint64_t field_val = 0;
    bcmi_ltsw_port_tab_t types[2];
    uint32_t data[2];
    ...
    switch (ctrl_type) {
    case bcmPortControlSampleIngressDest:
        types[0] = BCMI_PT_MIRROR_ING_ENCAP_SAMPLE_CPU;
        types[1] = BCMI_PT_MIRROR_ING_ENCAP_SAMPLE_INSTANCE;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_multi_get(unit, port, types, data, 2));
        *value = data[0] ? BCM_PORT_CONTROL_SAMPLE_DEST_CPU : 0;
        *value |= data[1] ? BCM_PORT_CONTROL_SAMPLE_DEST_MIRROR : 0;
        break;
    case bcmPortControlSampleIngressRate:
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_field_width_get(unit, MIRROR_PORT_ENCAP_SFLOWs,
                                     SAMPLE_ING_RATEs, &width));
        types[0] = BCMI_PT_MIRROR_ING_ENCAP_SAMPLE_RATE;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_multi_get(unit, port, types, data, 1));
        if (data[0]) {
            field_val = 1;
            *value = (field_val << width) / data[0];
        } else {
            *value = 0;
        }
        break;
    case bcmPortControlSampleEgressRate:
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_field_width_get(unit, MIRROR_PORT_ENCAP_SFLOWs,
                                     SAMPLE_EGR_RATEs, &width));
        types[0] = BCMI_PT_MIRROR_EGR_ENCAP_SAMPLE_RATE;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_multi_get(unit, port, types, data, 1));
        if (data[0]) {
            field_val = 1;
...
static int
xgs_port_operating_mode_get(int unit, bcm_port_t port, bcm_port_control_t ctrl_type, int *value)
{
    int temp;
    ...
    case bcmPortControlL3Ingress:
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_PORT_SYSTEM_L3_IIF_ID, value));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_get
    ...
    case bcmPortControlVrf:
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_PORT_SYSTEM_VRF_ID, value));
...
static int
xgs_port_operating_mode_set(int unit, bcm_port_t port, bcm_port_control_t ctrl_type, int value)
{
    int fld_num = 0;
    uint32_t data[3];
    bcmi_ltsw_port_tab_t types[3];
    ...
    case bcmPortControlL3Ingress:
        if (value > 0) {
            types[fld_num] = BCMI_PT_PORT_SYSTEM_L3_IIF_ID;
            data[fld_num] = value;
            fld_num++;

            types[fld_num] = BCMI_PT_PORT_SYSTEM_OPERATING_MODE;
            data[fld_num] = OP_MODE_L3_IIF;
            fld_num++;

            types[fld_num] = BCMI_PT_OPERATING_MODE;
            data[fld_num] = OP_MODE_L3_IIF;
            fld_num++;
        } else {
            types[fld_num] = BCMI_PT_PORT_SYSTEM_L3_IIF_ID;
    ...
    case bcmPortControlVrf:
        if (value > 0) {
            types[fld_num] = BCMI_PT_PORT_SYSTEM_VRF_ID;
            data[fld_num] = value;
            fld_num++;

            types[fld_num] = BCMI_PT_PORT_SYSTEM_OPERATING_MODE;
            data[fld_num] = OP_MODE_VRF;
            fld_num++;
...
xgs_port_control_port_get(int unit, bcm_port_t port, bcm_port_control_t type, int *value)
{
    ...
    case bcmPortControlVxlanVpnAssignmentCriteria:
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_get(unit, port_out,
    ...
    for (idx = 0; idx < depth; idx++) {
        ...
        if (ctrl_port_ptr[idx].flags & PORT_CTRL_PM) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_ltsw_port_tab_get(unit, port_out, fld, value));
...
xgs_port_control_port_set(int unit, bcm_port_t port, bcm_port_control_t type, int value)
{
    ...
    case bcmPortControlVxlanVpnAssignmentCriteria:
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_set(unit, port_out,
                                      BCMI_PT_VXLAN_VFI_KEY_MODE,
                                      (value & 0x1)));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_set(unit, port_out,
    ...
    for (idx = 0; idx < depth; idx++) {
        ...
        if (ctrl_port_ptr[idx].flags & PORT_CTRL_PM) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_ltsw_port_tab_set(unit, port_out, fld, value));
        }
        hit = 1;
    }
    if (hit == 0) {
...
xgs_port_tpid_set(int unit, bcm_port_t port, int type, uint16_t tpid)
{
    ...
    if (type & OUTER_TPID) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_array_get(unit, port,
                                            BCMI_PT_PASS_ON_OUTER_TPID_MATCH,
                                            tpid_en, 0, MAX_TPID_CNT));
        /* allocate a TPID index */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_vlan_otpid_entry_add(unit, tpid, &tpid_index));
    } else {
    ...
    if (type & OUTER_TPID) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_array_set(unit, port,
                                            BCMI_PT_PASS_ON_OUTER_TPID_MATCH,
                                            tpid_en, 0, MAX_TPID_CNT));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_array_set(unit, port,
                                            ...
                                            tpid_en, 0, MAX_TPID_CNT));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_VLAN_OUTER_TPID_ID, tpid_index));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_ING_VLAN_OUTER_TPID_ID, tpid_index));
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_array_set(unit, port,
                                            BCMI_PT_PASS_ON_PAYLOAD_OUTER_TPID_MATCH,
...
xgs_port_tpid_delete(int unit, bcm_port_t port, int type, uint16_t tpid, uint16_t tpid_default)
{
    ...
    if (tpid_en[tpid_index]) {
        if (type & OUTER_TPID) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_ltsw_port_tab_get(unit, port,
        ...
        if (type & OUTER_TPID) {
            if (egr_tpid == tpid) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmint_ltsw_port_tab_set(unit, port,
                                              BCMI_PT_VLAN_OUTER_TPID_ID,
                                              default_tpid_index));
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmint_ltsw_port_tab_set(unit, port,
                       ...
                       int color_select)
{
    ...
    if (type & OUTER_TPID) {
        ...
        SHR_IF_ERR_EXIT_EXCEPT_IF
            (bcmint_ltsw_port_tab_array_get(unit, port, BCMI_PT_EGR_CFI_AS_CNG,
                                            val, 0, MAX_TPID_CNT),
             SHR_E_NOT_FOUND);
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_array_set(unit, port, BCMI_PT_EGR_CFI_AS_CNG,
                                            val, 0, MAX_TPID_CNT));

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_array_set(unit, port, BCMI_PT_ING_CFI_AS_CNG,
                                            val, 0, MAX_TPID_CNT));
    }
    if (type & PAYLOAD_OUTER_TPID) {
        SHR_IF_ERR_VERBOSE_EXIT
...
xgs_ltsw_port_tpid_get(int unit, bcm_port_t port, int type, uint16_t *tpid)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_VLAN_OUTER_TPID_ID, &tpid_index));
...
int
xgs_ltsw_port_color_mode_set(int unit, bcm_port_t port, int color_select)
{
    uint64_t val[MAX_TPID_CNT] = {0, 0, 0, 0};
    int i = 0;
    ...
    if (color_select == BCM_COLOR_INNER_CFI) {
        SHR_IF_ERR_VERBOSE_EXIT(BCM_E_UNAVAIL);
    } else if (color_select == BCM_COLOR_PRIORITY) {
        for (i = 0; i < MAX_TPID_CNT; i++) {
            val[i] = 0;
        }
    } else if (color_select == BCM_COLOR_OUTER_CFI) {
...
int
xgs_ltsw_port_color_mode_get(int unit, bcm_port_t port, int *color_select)
{
    uint64_t val[MAX_TPID_CNT] = {0, 0, 0, 0};
    ...
    if (val[0]) {
        ...
        *color_select = BCM_COLOR_PRIORITY;
    }
...
xgs_ltsw_port_system_profile_init(int unit)
{
    ...
    BCM_PBMP_CLEAR(pbmp_valid);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_port_bitmap(unit, BCMI_LTSW_PORT_TYPE_ALL, &pbmp_valid));
                                       ...
                                       int fields_cnt)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_PORT_SYSTEM_PROFILE_ID, &prof_index_old));
    ...
    for (i = 0; i < fields_cnt; i++) {
        ...
        if (fld_type == 0) {
            ...
            if (fld_ptr->data.n >= (uint64_t)(sym_list->len)) {
                fld_val_s = NULL;
            } else {
        ...
        rv = xgs_port_system_profile_transfer(field_name, (void *)(&profile), 1,
                                              fld_val, &fld_val_s, fld_start_idx,
                                              fld_idx_num);
        if (SHR_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "Field %s of LT %s is not found.\n"),
                                  field_name, tab_map[type].table->name));
            SHR_IF_ERR_VERBOSE_EXIT(rv);
        }
    }
    SHR_IF_ERR_VERBOSE_EXIT
    ...
    if (prof_index != prof_index_old) {
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "Profile is changed from %d to %d.\n"),
                                prof_index_old, prof_index));
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_PORT_SYSTEM_PROFILE_ID, prof_index));
    }
exit:
    SHR_FUNC_EXIT();
}
...
xgs_ltsw_port_learn_set(int unit, bcm_port_t port, uint32_t flags)
{
    ...
    types[0] = BCMI_PT_MAC_DROP;
    data[0] = (flags & BCM_PORT_LEARN_FWD) ? 0 : 1;
    types[1] = BCMI_PT_MAC_COPY_TO_CPU;
    data[1] = (flags & BCM_PORT_LEARN_CPU) ? 1 : 0;
    types[2] = BCMI_PT_MAC_LEARN;
    data[2] = (flags & BCM_PORT_LEARN_ARL) ? 1 : 0;
...
xgs_ltsw_port_l3_enable_set(int unit, bcm_port_t port, int enable)
{
    ...
    types[0] = BCMI_PT_V4L3;
    data[0] = enable ? 1 : 0;
    types[1] = BCMI_PT_V6L3;
    data[1] = enable ? 1 : 0;
...
xgs_ltsw_port_l3_mtu_set(int unit, bcm_port_t port, int size)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_MTU, size));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_MTU_EN, 1));
...
xgs_ltsw_port_l3_mtu_get(int unit, bcm_port_t port, int *size)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_MTU, size));
...
xgs_ltsw_port_ipmc_modify_set(int unit, bcm_port_t port, uint32_t flags)
{
    ...
    control = BCMI_LTSW_L3_PC_ATTR_IPMC_TTL_DEC;
    control |= BCMI_LTSW_L3_PC_ATTR_IPMC_SA_REPLACE;
    control |= BCMI_LTSW_L3_PC_ATTR_IPMC_TTL_CHECK;

    if (flags & BCM_PORT_IPMC_MODIFY_NO_TTL) {
        info.ipmc_ttl_dec = 0;
    } else {
        info.ipmc_ttl_dec = 1;
    }
    if (flags & BCM_PORT_IPMC_MODIFY_NO_SRCMAC) {
        info.ipmc_sa_replace = 0;
    } else {
        info.ipmc_sa_replace = 1;
    }
    if (flags & BCM_PORT_IPMC_CHECK_NO_TTL) {
        info.ipmc_ttl_check = 0;
    } else {
        info.ipmc_ttl_check = 1;
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_l3_port_control_set(unit, port, control, &info));
...
xgs_ltsw_port_ipmc_modify_get(int unit, bcm_port_t port, uint32_t *flags)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_l3_port_control_get(unit, port, control, &info));

    *flags = 0;
    *flags |= (info.ipmc_ttl_dec == 0) ? BCM_PORT_IPMC_MODIFY_NO_TTL : 0;
    *flags |= (info.ipmc_sa_replace == 0) ? BCM_PORT_IPMC_MODIFY_NO_SRCMAC : 0;
    *flags |= (info.ipmc_ttl_check == 0) ? BCM_PORT_IPMC_CHECK_NO_TTL : 0;
...
xgs_ltsw_port_bpdu_enable_set(int unit, bcm_port_t port, int enable)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_DROP_BPDU, !enable));
...
xgs_ltsw_port_bpdu_enable_get(int unit, bcm_port_t port, int *enable)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_DROP_BPDU, enable));
    *enable ^= 1;
                                  ...
                                  int action_profile_index)
{
    ...
    types[fields_num] = BCMI_PT_VLAN_ING_TAG_ACTION_PROFILE_ID;
    data[fields_num] = action_profile_index;
    fields_num ++;

    types[fields_num] = BCMI_PT_ING_OPRI;
    data[fields_num] = action->priority;
    fields_num ++;

    types[fields_num] = BCMI_PT_ING_OVID;
    data[fields_num] = action->new_outer_vlan;
    fields_num ++;

    types[fields_num] = BCMI_PT_ING_OCFI;
    data[fields_num] = action->new_outer_cfi;
    fields_num ++;
...

{
    ...
    types[0] = BCMI_PT_VLAN_ING_TAG_ACTION_PROFILE_ID;
    field_num = 1;

    /* Sometimes, user only wants to get old profile index. */
    if (action != NULL) {
        types[1] = BCMI_PT_ING_OPRI;

        types[2] = BCMI_PT_ING_OVID;

        types[3] = BCMI_PT_ING_OCFI;

        field_num = 4;

        if (ltsw_feature(unit, LTSW_FT_VFI)) {
            /* default vfi */
            types[4] = BCMI_PT_DEFAULT_VFI;
            field_num = 5;
        }
    }
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_get(unit, port, types, data, field_num));

    *action_profile_index = data[0];
    /* Sometimes, user only wants to get old profile index. */
    if (action != NULL) {
        action->priority = data[1];

        action->new_outer_vlan = data[2];

        action->new_outer_cfi = data[3];
...
xgs_ltsw_port_egr_vlan_action_set(int unit, bcm_port_t port,
                                  ...
                                  int action_profile_index)
{
    bcmi_ltsw_port_tab_t types[4];
    uint32_t data[4];

    SHR_FUNC_ENTER(unit);

    types[0] = BCMI_PT_VLAN_EGR_TAG_ACTION_PROFILE_ID;
    data[0] = action_profile_index;

    types[1] = BCMI_PT_EGR_OPRI;
    data[1] = action->priority;

    types[2] = BCMI_PT_EGR_OVID;
    data[2] = action->new_outer_vlan;

    types[3] = BCMI_PT_EGR_OCFI;
    data[3] = action->new_outer_cfi;
...

{
    ...
    types[fields_num] = BCMI_PT_VLAN_EGR_TAG_ACTION_PROFILE_ID;
    fields_num ++;
    /* Sometimes, user only wants old action profile index. */
    if (action != NULL) {
        types[fields_num] = BCMI_PT_EGR_OPRI;
        fields_num ++;

        types[fields_num] = BCMI_PT_EGR_OVID;
        fields_num ++;

        types[fields_num] = BCMI_PT_EGR_OCFI;
        fields_num ++;
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_get(unit, port, types, data, fields_num));
                             ...
                             int pkt_prio, uint32 flags)
{
    ...
    types[fields_num] = BCMI_PT_ING_PVLAN;
    data[fields_num] = (flags & BCM_PORT_FORCE_VLAN_ENABLE) ? 1 : 0;
    fields_num ++;

    types[fields_num] = BCMI_PT_EGR_PVLAN;
    data[fields_num] = (flags & BCM_PORT_FORCE_VLAN_ENABLE) ? 1 : 0;
    fields_num ++;

    if (flags & BCM_PORT_FORCE_VLAN_ENABLE) {
        types[fields_num] = BCMI_PT_PVLAN_EGR_UNTAG;
        data[fields_num] = (flags & BCM_PORT_FORCE_VLAN_UNTAG) ? 1 : 0;
        fields_num ++;

        if ((flags & BCM_PORT_FORCE_VLAN_UNTAG) == 0) {
            types[fields_num] = BCMI_PT_PVLAN_EGR_VID;
            data[fields_num] = vlan;
            fields_num ++;

            types[fields_num] = BCMI_PT_PVLAN_EGR_REPLACE_PRI;
            data[fields_num] = (pkt_prio < 0) ? 0 : 1;
            fields_num ++;

            if (pkt_prio > 0) {
                types[fields_num] = BCMI_PT_PVLAN_EGR_PRI;
                data[fields_num] = pkt_prio;
                fields_num ++;
            }
        }
    } else {
                             ...
                             int *pkt_prio, uint32 *flags)
{
    ...
    types[0] = BCMI_PT_ING_PVLAN;
    types[1] = BCMI_PT_EGR_PVLAN;
    types[2] = BCMI_PT_PVLAN_EGR_UNTAG;
    types[3] = BCMI_PT_PVLAN_EGR_REPLACE_PRI;
    types[4] = BCMI_PT_PVLAN_EGR_PRI;
    types[5] = BCMI_PT_PVLAN_EGR_VID;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_get(unit, port, types, data, 6));
...
xgs_ltsw_port_vlan_member_get(int unit, bcm_port_t port, uint32_t *flags)
{
    ...
    types[0] = BCMI_PT_ING_VLAN_MEMBERSHIP_CHECK;
    types[1] = BCMI_PT_EGR_VLAN_MEMBERSHIP_CHECK;
...
xgs_ltsw_port_vlan_member_set(int unit, bcm_port_t port, uint32_t flags)
{
    ...
    types[0] = BCMI_PT_ING_VLAN_MEMBERSHIP_CHECK;
    data[0] = (flags & BCM_PORT_VLAN_MEMBER_INGRESS) ? 1 : 0;

    types[1] = BCMI_PT_EGR_VLAN_MEMBERSHIP_CHECK;
    data[1] = (flags & BCM_PORT_VLAN_MEMBER_EGRESS) ? 1 : 0;
...
xgs_ltsw_port_untagged_priority_get(int unit, bcm_port_t port, int *priority)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_ING_OPRI, priority));
...
xgs_ltsw_port_untagged_priority_set(int unit, bcm_port_t port, int priority)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_ING_OPRI, priority));
...
xgs_ltsw_port_untagged_vlan_get(int unit, bcm_port_t port, bcm_vlan_t *vid_ptr)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_ING_OVID, &data));
...
xgs_ltsw_port_untagged_vlan_set(int unit, bcm_port_t port, bcm_vlan_t vid)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_ING_OVID, (int)vid));
...
xgs_ltsw_port_dscp_phb_map_mode_set(int unit, bcm_port_t port, int mode)
{
    ...
    if (ipv4 > -1) {
        types[port_tab_fld_num] = BCMI_PT_TRUST_PHB_ING_DSCP_V4;
        data[port_tab_fld_num] = ipv4;
        port_tab_fld_num++;
    }
    if (ipv6 > -1) {
        types[port_tab_fld_num] = BCMI_PT_TRUST_PHB_ING_DSCP_V6;
        data[port_tab_fld_num] = ipv6;
        port_tab_fld_num++;
    }
    if (port_tab_fld_num > 0) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmint_ltsw_port_tab_multi_set(unit, port, types, data,
                                            port_tab_fld_num));
    }
exit:
    SHR_FUNC_EXIT();
                        ...
                        bcm_port_class_t class, uint32 *class_id)
{
    ...
    switch (class) {
        ...
        case bcmPortClassFieldIngress:
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_ltsw_port_tab_get(unit, port, BCMI_PT_PORT_SYSTEM_CLASS_ID, &id));
            break;
                        ...
                        bcm_port_class_t class, uint32 class_id)
{
    ...
    switch (class) {
        case bcmPortClassFieldLookup:
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_PORT_SYSTEM_FP_VLAN_PORT_GRP, id));
            break;

        case bcmPortClassFieldIngress:
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmint_ltsw_port_tab_set(unit, port, BCMI_PT_PORT_SYSTEM_CLASS_ID, id));
            break;
                             ...
                             uint32_t burst, bcmi_ltsw_port_shaper_mode_t mode)
{
    ...
    if ((bandwidth == 0) || (burst == 0)) { /* disabe shaper */
        ...
        types[0] = BCMI_PT_SHAPER_RATE;
        data[0] = 0;

        /* burst is calculated from rate */
        types[1] = BCMI_PT_SHAPER_BURST_AUTO;
        data[1] = 1;

        field_num = 2;
    } else {
        /* shaper mode */
        types[0] = BCMI_PT_SHAPER_MODE;
        data[0] = mode;

        /* shaper rate */
        types[1] = BCMI_PT_SHAPER_RATE;
        data[1] = bandwidth;

        /* shaper burst */
        types[2] = BCMI_PT_SHAPER_BURST;
        data[2] = burst;

        /* burst is not calculated from rate */
        types[3] = BCMI_PT_SHAPER_BURST_AUTO;
        data[3] = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmint_ltsw_port_tab_multi_set(unit, port, types, data, field_num));
                             ...
                             uint32_t *burst, bcmi_ltsw_port_shaper_mode_t *mode)
{
    ...
    types[0] = BCMI_PT_SHAPER_MODE;
    /* real shaper rate */
    types[1] = BCMI_PT_SHAPER_RATE_REAL;
    /* real shaper burst */
    types[2] = BCMI_PT_SHAPER_BURST_REAL;
...
xgs_ltsw_port_discard_get(int unit, bcm_port_t port, int *mode)
{
    ...
    types[0] = BCMI_PT_DROP_TAG;
    types[1] = BCMI_PT_DROP_UNTAG;
