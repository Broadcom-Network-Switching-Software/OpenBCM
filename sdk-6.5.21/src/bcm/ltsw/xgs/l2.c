...
/* Number of learnt L2 MAC of current loop. */
static uint32_t l2_host_tbl_learn_num[BCM_MAX_NUM_UNITS];

/*
 * Maximum number of learnt L2 MAC per loop.
 * If l2_host_tbl_learn_num[unit] > l2_host_tbl_learn_max_num[unit],
 * the entries in LEARN_CACHE_DATA will be deleted directly.
 */
static uint32_t l2_host_tbl_learn_max_num[BCM_MAX_NUM_UNITS];

/* Depth of LT LEARN_CACHE_DATA. */
static uint32_t l2_learn_cache_data_depth[BCM_MAX_NUM_UNITS];

/* Over this watermark, L2 learning uses slow mode. */
#define L2_HOST_TAB_HIGH_WATERMARK 985

/* Under this watermark, L2 learning uses fast mode. */
#define L2_HOST_TAB_LOW_WATERMARK  975

/* Over the low watermark, L2 learning entry number should be less than 128. */
#define LEARN_MAX_NUM_PER_TARVERSE 128

/*
 * This value is from exprience.
 * L2 learnt events are reported every 0.5s by bcmcthL2Learn in SDKLT.
 * When L2 host table usage ratio is more L2_HOST_TAB_HIGH_WATERMARK,
 * bcmcthL2Learn will use slow mode to poll H/W LEARN_CACHE_CACHE table.
 * In this mode, in one cycle of bcmcthL2Learn, this thread will fall sleep
 * for 0.5s, then poll H/W LEARN_CACHE_CACHE table and report the L2 learnt events.
 * So if time spent on processing every event is more 0.3s,
 * the CPU utilization ratio should be more than 60% for L2 learning.
 */
#define TIME_SPENT_ON_L2_LEARN_EVENT 300000
...
xgs_ltsw_l2_member_populate(int unit, bcmlt_entry_handle_t eh,
                            bcmint_l2_tbl_t tid, int id_map_depth,
 ...
 * \param [in] unit Unit number
 * \param [in] eh LT entry handle.
 * \param [in] tid Global ID of LT table defined by HSDK.
                                     ...
                                     const char *fld_str_val, bool mem_val_used)
{
    ...
    for (i = 0; i < ass_fld_num; i++) {
        ...
        if (ass_index == id_map_depth) {
            ...
            rv = xgs_ltsw_l2_member_populate(unit, eh, tid,
                            ...
                            int index, void *data_struct, void *cookie)
{
    ...
    void *addr = NULL;
    uint64_t fld_val = INVALID_FLD_VAL;
    ...
    if (fld_map->sym_map != NULL) {
        SHR_IF_ERR_EXIT
            (xgs_ltsw_l2_member_ass_flds_populate(unit, eh, tid, id_map_depth,
    ...
    if (fld_map->sym_map == NULL && fld_map->flag_val == INVALID_FLG_VAL) {
        ...
        for (i = 0; i < ass_fld_num; i++) {
            ...
            if (ass_index == id_map_depth) {
                ...
                rv = xgs_ltsw_l2_member_parse(unit, eh, tid,
                                              id_map_depth, id_map_base,
                            ...
                            void *data_struct, void *cookie)
{
    ...
    for (i = 0; i < lt_map->depth; i++) {
        ...
        SHR_IF_ERR_EXIT
            (xgs_ltsw_l2_member_populate(unit, eh, lt_map->tid,
                         ...
                         void *data_struct, void *cookie)
{
    ...
    for (i = 0; i < lt_map->depth; i++) {
        ...
        SHR_IF_ERR_EXIT
            (xgs_ltsw_l2_member_parse(unit, eh, lt_map->tid,
                                      lt_map->depth, lt_map->id_map,
...
xgs_ltsw_l2_lt_info_get(int unit, bcmint_l2_tbl_t l2_lt_id)
{
    int i;

    for (i = 0; i < BCMINT_L2_MAX_NUM; i++) {
        if (l2_lt_map_info[unit][i].tid == l2_lt_id) {
            break;
        }
    }
    if (i == BCMINT_L2_MAX_NUM) {
        return NULL;
    } else {
...
l2_cache_insert(int unit, bcm_l2_cache_addr_t *l2caddr)
{
    ...
    lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_CACHE_ID);
...
l2_dst_block_entry_insert(int unit, int index, bcm_pbmp_t *profile)
{
    ...
    lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_DST_BLOCK_ID);
    SHR_NULL_CHECK(lt_info, SHR_E_UNAVAIL);
...
l2_dst_block_profile_recover(int unit)
{
    ...
    lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_HOST_TAB_ID);
    SHR_NULL_CHECK(lt_info, SHR_E_UNAVAIL);
                               ...
                               bcm_l2_addr_t *l2addr)
{
    ...
    lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_LEARN_DATA_ID);
    if (lt_info == NULL) {
        LOG_ERROR(BSL_LOG_MODULE,
...
/*!
 * \brief get L2 host table usage rate.
 *
 * \param [in] unit    Unit number.
 * \param [out] ratio   L2 host table usage rate.
 *
 * \retval None.
 */
static void
l2_host_table_usage_ratio_get(int unit, int *ratio)
{
    int count = 0;
    /* static */
    static int l2_host_table_size[BCM_MAX_NUM_UNITS];

    if (l2_host_table_size[unit] == 0) {
        (void)xgs_ltsw_l2_size_get(unit, &l2_host_table_size[unit]);
    }
    *ratio = 0;
    if (l2_host_table_size[unit]) {
        (void)xgs_ltsw_l2_count_get(unit, &count);
        *ratio = (count * 1000) / l2_host_table_size[unit];
    } else {
        LOG_WARN(BSL_LOG_MODULE,
                 (BSL_META_U(unit,
                             "Fail to get L2 host table size\n")));
    }
}
...
l2_learn_cache_slow_poll_enable(int unit, bool slow_poll)
{
    ...
    lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_LEARN_CTRL_ID);
    SHR_NULL_CHECK(lt_info, SHR_E_UNAVAIL);
...
l2_learn_cache_data_process(int unit, void *trav_st)
{
    ...
    if (l2_host_tbl_learn_num[unit]++ < l2_host_tbl_learn_max_num[unit]) {
        ...
        LOG_DEBUG(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "Number of traversed entry should"
                              " not be larger than [%d]\n"),
                  l2_host_tbl_learn_max_num[unit]));
    }
    /*
                        ...
                        void *notif_info, void *user_data)
{
    ...
    sal_usecs_t time, schedule_time = 0, time_spent;
    int rv = SHR_E_NONE, ratio = 0;
    ...
    l2_host_table_usage_ratio_get(unit, &ratio);

    if (ratio < L2_HOST_TAB_HIGH_WATERMARK) {
        if (ratio > L2_HOST_TAB_LOW_WATERMARK) {
            l2_host_tbl_learn_max_num[unit] = LEARN_MAX_NUM_PER_TARVERSE;
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
                                    "The L2 host table usage rate is"
                                    " %d, still use %s poll mode.\n"),
                         ratio, l2_learn_control[unit].slow_poll ? "slow" : "fast"));
        } else {
            l2_host_tbl_learn_max_num[unit] = l2_learn_cache_data_depth[unit];
            /* Use fast mode always. */
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
                                    "The L2 host table usage rate is"
                                    " %d < %d , so L2 thread uses "
                                    "normal poll mode always.\n"),
                         ratio, L2_HOST_TAB_LOW_WATERMARK));
            rv = l2_learn_cache_slow_poll_enable(unit, 0);
        }
    } else {
        if (l2_host_tbl_learn_max_num[unit] > LEARN_MAX_NUM_PER_TARVERSE) {
            l2_host_tbl_learn_max_num[unit] = LEARN_MAX_NUM_PER_TARVERSE;
        }
        /* Use slow mode always. */
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "The L2 host table usage rate is"
                                " %d > %d , so L2 thread uses "
                                "slow poll mode always.\n"),
                     ratio, L2_HOST_TAB_HIGH_WATERMARK));
        rv = l2_learn_cache_slow_poll_enable(unit, 1);
    }
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "Failed to enable or disable L2 learn "
                              "slow poll mode (%d).\n"), rv));
    }

    if (ratio > L2_HOST_TAB_HIGH_WATERMARK ||
        LOG_CHECK(BSL_LOG_MODULE | BSL_DEBUG)) {
    ...
    if (ratio > L2_HOST_TAB_HIGH_WATERMARK) {
        ...
        if (schedule_time < time) {
            time_spent = time - schedule_time;
        } else {
            time_spent = time + ((sal_usecs_t)(-1) - schedule_time);
        }
        /* SDKLT will call l2_learn_lt_cb every 0.5s. */
        if (time_spent >= TIME_SPENT_ON_L2_LEARN_EVENT) {
            l2_host_tbl_learn_max_num[unit] /= 2;
            l2_host_tbl_learn_max_num[unit]++;
        } else {
            if (l2_host_tbl_learn_max_num[unit] < LEARN_MAX_NUM_PER_TARVERSE) {
                l2_host_tbl_learn_max_num[unit]++;
        ...
        LOG_INFO(BSL_LOG_MODULE,
                 (BSL_META_U(unit,
                             "Maximun number [%d] of learnt MAC per loop."
                             "Spent [%d]us on learning these new MACs."
                             "Current usage ratio of L2 host table [%d].\n"),
                  l2_host_tbl_learn_max_num[unit],
                  time_spent, ratio));
    }
    ...
    if (LOG_CHECK(BSL_LOG_MODULE | BSL_DEBUG)) {
        ...
        LOG_CLI((BSL_META("LEARN_TIME(ts:0x%x): total(%d us) schedule(%d us)" \
                 ...
                 time - schedule_time, l2_host_tbl_learn_num[unit]));
...
l2_learn_enable(int unit, bool enable)
{
    ...
    if (enable) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_capacity_get(unit, lt_info->lt_name,
                                  &l2_learn_cache_data_depth[unit]));
        l2_host_tbl_learn_max_num[unit] = l2_learn_cache_data_depth[unit];
                     ...
                     bool is_trunk_flag, const char **dest_type)
{
    ...
    if (tgid_temp != BCM_TRUNK_INVALID) {
        ...
        *dest_type = "L2_MC_GRP";
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_virtual_port_fwd_egress_obj_get(unit, port, &egress_if));
...
/*!
 * \brief Confirm tid according information in bcm_l2_station_t.
 *
 * \param [in] unit Unit number
 * \param [in] station Information for L2 my station table.
 * \param [out] lt_info_p L2 map information of tid.
                       ...
                       bool overlay)
{
    ...
    if (station->src_port_mask == 0) {
        tid = (overlay == 0) ?
            BCMINT_L2_UNDERLAY_MY_STATION_ID : BCMINT_L2_MY_STATION_ID;
    } else if (BCM_GPORT_IS_TRUNK(station->src_port)) {
        tid = (overlay == 0) ?
            BCMINT_L2_UNDERLAY_MY_STATION_TRUNK_ID : BCMINT_L2_MY_STATION_TRUNK_ID;
    } else {
        tid = (overlay == 0) ?
            BCMINT_L2_UNDERLAY_MY_STATION_MODPORT_ID : BCMINT_L2_MY_STATION_MODPORT_ID;
    }
    lt_info = xgs_ltsw_l2_lt_info_get(unit, tid);
    /* No overlay my station table. */
    if (overlay == 0 && lt_info == NULL) {
        if (station->src_port_mask == 0) {
            tid = BCMINT_L2_MY_STATION_ID;
        } else if (BCM_GPORT_IS_TRUNK(station->src_port)) {
            tid = BCMINT_L2_MY_STATION_TRUNK_ID;
        } else {
            tid = BCMINT_L2_MY_STATION_MODPORT_ID;
        }
        lt_info = xgs_ltsw_l2_lt_info_get(unit, tid);
    }
    *lt_info_p = lt_info;
...
l2_virtual_info_update_int_cb(int unit, void *trav_st)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (l2_entry_populate(unit, eh_update, lt_info, &l2addr, 0));
...
xgs_ltsw_l2_dest_type_trans(int unit, bcmint_l2_trans_data_t *param, int dir)
{
    ...
    if (dir == DIR_POPULATE) {
        if (param->tid == BCMINT_L2_HOST_TAB_ID) {
        ...
        } else if (param->tid == BCMINT_L2_CACHE_ID) {
...
xgs_ltsw_l2_dest_port_trans(int unit, bcmint_l2_trans_data_t *param, int dir)
{
    ...
    if (dir == DIR_POPULATE) {
        ...
        if (SHR_FAILURE(rv)) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_virtual_port_fwd_egress_obj_get(unit, port_in, &egress_if));
            SHR_IF_ERR_VERBOSE_EXIT
    ...
    bcm_l2_addr_t *l2addr)
{
    ...
    bool update = false, notify = NOTIFY_L2_EVENT(unit), l2_lock = false;
    const bcmint_l2_map_info_t *lt_info = NULL;
    ...
    uint32_t *tbl_sz)
{
    ...
    lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_UNDERLAY_MY_STATION_ID);
    if (lt_info == NULL) {
        lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_MY_STATION_ID);
    }
    SHR_NULL_CHECK(lt_info, SHR_E_UNAVAIL);
    ...
    int flags)
{
    ...
    l2_station_lt_info_get(unit, station, &lt_info,
                           (flags & BCMINT_L2_STATION_2_TCAM) ? 1 : 0);
    SHR_NULL_CHECK(lt_info, SHR_E_UNAVAIL);
    ...
    int valid)
{
    ...
    lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_OPAQUE_TAG_ID);
    SHR_NULL_CHECK(lt_info, SHR_E_UNAVAIL);
    ...
    int valid)
{
    ...
    lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_PAYLOAD_OPAQUE_TAG_ID);
    SHR_NULL_CHECK(lt_info, SHR_E_UNAVAIL);
    ...
    int *l2_freeze_mode)
{
    ...
    if (L2_FROZEN_OVERRIDE_STATUS(unit) == 1) {
        lt_info = xgs_ltsw_l2_lt_info_get(unit, BCMINT_L2_LEARN_OVERRIDE_ID);
        SHR_NULL_CHECK(lt_info, SHR_E_UNAVAIL);
