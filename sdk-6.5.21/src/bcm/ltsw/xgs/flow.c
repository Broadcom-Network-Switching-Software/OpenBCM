...
typedef struct egr_vlan_xlate_user_data_s {
        bcm_flow_encap_traverse_cb cb;
        void *user_data;
} egr_vlan_xlate_user_data_t;

/*! Configures the outer QOS map IDt. */
#define FLOW_PORT_SET_OUTER_QOS_MAP_ID   (0x0001)
...
/*!
 * \brief Preparation operation for traverse with snapshot.
 *
 * \param [in]   unit          Unit number.
 ...
 */
static int
xgs_flow_traverse_snapshot_start(int unit, bcmlt_entry_handle_t entry_hd)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_attrib_set(entry_hd,
...
/*!
 * \brief Clear up operation for traverse with snapshot.
 *
 * \param [in]   unit          Unit number.
 * \param [in]   entry_hd      Logical table entry handle.
 *
 ...
 */
static int
xgs_flow_traverse_snapshot_done(int unit, bcmlt_entry_handle_t entry_hd)
{
    int rv = SHR_E_NONE;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_attrib_clear(entry_hd,
                                  BCMLT_ENT_ATTR_TRAVERSE_SNAPSHOT));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_attrib_set(entry_hd,
                                BCMLT_ENT_ATTR_TRAVERSE_DONE));

    rv = bcmi_lt_entry_commit(unit, entry_hd, BCMLT_OPCODE_TRAVERSE,
                                      ...
                                      bcm_flow_tunnel_terminator_t *info)
{
    ...
    /* Validate required elements. */
     ...
     /* DIP or DIP6 mask only set for TCAM. */
     if (info->flags & BCM_TUNNEL_TERM_MASK) {
     ...
     /* DIP and DIP6 mask can't be set at same time. */
     if ((info->valid_elements & BCM_FLOW_TUNNEL_TERM_DIP6_MASK_VALID) &&
      ...
      * DIP or DIP6 mask must be set for TCAM.
      */
     if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_DIP6_MASK_VALID) {
     ...
     /* VLAN mask should not be set without BCM_TUNNEL_TERM_MASK. */
     if ((!(info->flags & BCM_TUNNEL_TERM_MASK)) &&
        (info->valid_elements & BCM_FLOW_TUNNEL_TERM_VLAN_MASK_VALID)) {
         SHR_ERR_EXIT(SHR_E_PARAM);
     }
...
xgs_flow_tunnel_term_criteria_determine(int unit,
                                        ...
                                        bcm_flow_tunnel_terminator_t *info)
{
    int lt_len = sal_strlen(lt_name);

    SHR_FUNC_ENTER(unit);

    if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_OVID_IPV6s, lt_len)) {
        ...
        info->flags |= BCM_TUNNEL_TERM_MASK;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_OVID_IPV6_EMs, lt_len)) {
        info->valid_elements = BCM_FLOW_TUNNEL_TERM_VLAN_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP6_VALID |
                               BCM_FLOW_TUNNEL_TERM_SIP6_VALID;
        info->flow_handle = BCMINT_FLOW_ID_VXLAN;
        info->type = bcmTunnelTypeVxlan6;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_OVID_DST_IPV6s, lt_len)) {
        info->valid_elements = BCM_FLOW_TUNNEL_TERM_VLAN_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP6_VALID |
                               BCM_FLOW_TUNNEL_TERM_VLAN_MASK_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP6_MASK_VALID;
        ...
        info->type = bcmTunnelTypeVxlan6;
        info->flags |= BCM_TUNNEL_TERM_MASK;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_OVID_DST_IPV6_EMs, lt_len)) {
        info->valid_elements = BCM_FLOW_TUNNEL_TERM_VLAN_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP6_VALID;
        info->flow_handle = BCMINT_FLOW_ID_VXLAN;
        info->type = bcmTunnelTypeVxlan6;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_OVID_IPV4s, lt_len)) {
        info->valid_elements = BCM_FLOW_TUNNEL_TERM_VLAN_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP_VALID |
                               BCM_FLOW_TUNNEL_TERM_SIP_VALID |
                               BCM_FLOW_TUNNEL_TERM_VLAN_MASK_VALID |
        ...
        info->flags |= BCM_TUNNEL_TERM_MASK;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_OVID_IPV4_EMs, lt_len)) {
        info->valid_elements = BCM_FLOW_TUNNEL_TERM_VLAN_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP_VALID |
                               BCM_FLOW_TUNNEL_TERM_SIP_VALID;
        info->flow_handle = BCMINT_FLOW_ID_CLASSIC_VXLAN;
        info->type = bcmTunnelTypeVxlan;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_OVID_DST_IPV4s, lt_len)) {
        info->valid_elements = BCM_FLOW_TUNNEL_TERM_VLAN_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP_VALID |
                               BCM_FLOW_TUNNEL_TERM_VLAN_MASK_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP_MASK_VALID;
    ...
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_OVID_DST_IPV4_EMs, lt_len)) {
        info->valid_elements = BCM_FLOW_TUNNEL_TERM_VLAN_VALID |
                               BCM_FLOW_TUNNEL_TERM_DIP_VALID;
        info->flow_handle = BCMINT_FLOW_ID_CLASSIC_VXLAN;
        info->type = bcmTunnelTypeVxlan;
                                           ...
                                           bcm_flow_tunnel_initiator_t *info)
{
    ...
    if (opt == HW_OPT_SET) {
        ...
        if (info->flags & BCM_TUNNEL_REPLACE) {
            ...
            /* The maximum l3_intf is reserved for internal use. */
            if (info->l3_intf_id >= idx_max) {
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
...
/*!
 * \brief Recover tunnel decap port profile for FLOW.
 *
 * \param [in] unit Unit number.
 *
 ...
 */
static int
xgs_flow_tunnel_decap_port_profile_recover(int unit)
{
    bcmlt_entry_handle_t entry_hd = BCMLT_INVALID_HDL;
    const char *lt_list[] = {TNL_L2_VXLAN_DECAP_OVID_IPV6_EMs,
                             TNL_L2_VXLAN_DECAP_OVID_DST_IPV6_EMs,
    ...
    for (idx = 0; idx < lt_num; idx++) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_allocate(dunit, lt_list[idx], &entry_hd));

        while ((rv = bcmi_lt_entry_commit(dunit, entry_hd, BCMLT_OPCODE_TRAVERSE,
                                          BCMLT_PRIORITY_NORMAL)) == SHR_E_NONE) {
            SHR_IF_ERR_VERBOSE_EXIT
               (bcmlt_entry_field_get(entry_hd, TNL_DECAP_PORT_PROFILE_IDs,
                                      &get_val));
            prof_idx = (int)get_val;

            if (prof_idx == FLOW_TUNNEL_TERM_PBMP_PROFILE_DEF) {
                continue;
            }
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_tunnel_decap_port_profile_recover(unit, prof_idx));
        }
                                     ...
                                     bcm_flow_port_encap_t *info)
{
    ...
    if ((opt == HW_OPT_SET) &&
        ...
        uint32_t supported_elements = BCM_FLOW_PORT_ENCAP_NETWORK_GROUP_VALID |
                                      ...
                                      BCM_FLOW_PORT_ENCAP_OL_EGRESS_IF_VALID;
    ...
    if (opt == HW_OPT_SET) {
        if (info->valid_elements & BCM_FLOW_PORT_ENCAP_EGRESS_IF_VALID) {
            ...
            if (!(info->options & BCM_FLOW_PORT_ENCAP_OPTION_CLEAR)) {
                ...
                /* nexthop mapped to vp not same as configured one. */
                if ((bind_vp != 0) && (bind_vp != vp)) {
                    SHR_ERR_EXIT(SHR_E_PARAM);
                }
            }
        }
        ...
        if (info->valid_elements & BCM_FLOW_PORT_ENCAP_OL_EGRESS_IF_VALID) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_l3_egress_obj_id_resolve(unit,
                                                    ...
                                                    &nh_idx,
                                                    &nh_type));
            if ((nh_type != BCMI_LTSW_L3_EGR_OBJ_T_OL) &&
                (nh_type != BCMI_LTSW_L3_EGR_OBJ_T_ECMP_OL)) {
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
            if (!(info->options & BCM_FLOW_PORT_ENCAP_OPTION_CLEAR)) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_virtual_port_overlay_egress_obj_used(
                        unit, info->ol_egress_if, &bind_vp));
                /* nexthop mapped to vp not same as configured one. */
                if ((bind_vp != 0) && (bind_vp != vp)) {
                    SHR_ERR_EXIT(SHR_E_PARAM);
                }
        ...
        if (info->valid_elements & BCM_FLOW_PORT_ENCAP_ES_ID_VALID) {
             ...
             if (info->es_id >= 64) {
                 SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                     (BSL_META_U(unit, "Bad ES_ID (%d)/max(63)!\n"),
                                                 info->es_id));
            }
        }
        if (update_l2_tag && (vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT) &&
           (!(info->valid_elements & BCM_FLOW_PORT_ENCAP_OL_EGRESS_IF_VALID))) {
            SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                (BSL_META_U(unit, "L2 Tag on acc or network overlay!\n")));
        ...
        if ((info->valid_elements & BCM_FLOW_PORT_ENCAP_MTU_VALID) &&
            ...
            SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                (BSL_META_U(unit, "MTU on network VP only!\n")));

        }

        if ((info->valid_elements & BCM_FLOW_PORT_ENCAP_VLAN_PRI_MAP_ID_VALID) &&
            (vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT)) {
            SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                (BSL_META_U(unit, "Dot1q mapping on access VP only!\n")));

        }

        if (info->valid_elements & BCM_FLOW_PORT_ENCAP_VLAN_PRI_MAP_ID_VALID) {
            bcmi_ltsw_qos_map_type_t qos_type = bcmiQosMapTypeCount;
            int qos_base_ptr = 0;

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_qos_map_id_resolve(unit, info->vlan_pri_map_id,
        ...
        if ((info->valid_elements & BCM_FLOW_PORT_ENCAP_FLAGS_VALID) &&
            ...
            if (vid & (!(info->valid_elements & BCM_FLOW_PORT_ENCAP_VLAN_VALID))) {
                SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                    (BSL_META_U(unit, "VID not provided!\n")));
            }
            if (pri & (!(info->valid_elements & (BCM_FLOW_PORT_ENCAP_PKT_PRI_VALID |
                BCM_FLOW_PORT_ENCAP_VLAN_PRI_MAP_ID_VALID)))) {
                SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                    (BSL_META_U(unit, "PRI not provided!\n")));
            }
            if (tpid & (!(info->valid_elements & BCM_FLOW_PORT_ENCAP_TPID_VALID))) {
                SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                    (BSL_META_U(unit, "TPID not provided!\n")));
            }
        }

         if ((info->valid_elements & BCM_FLOW_PORT_ENCAP_VLAN_PRI_MAP_ID_VALID) &&
             (info->valid_elements & (BCM_FLOW_PORT_ENCAP_PKT_PRI_VALID |
             BCM_FLOW_PORT_ENCAP_PKT_CFI_VALID))) {
             SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                 (BSL_META_U(unit, "Conflict dot1q assignment!\n")));
         }
    }
                                ...
                                bcm_flow_port_encap_t *info)
{
    ...
    int max_paths = 0;
    int array_size = 0;
    int ecmp_member_idx = 0;
    bcm_if_t tmp_egress_if = info->egress_if;

    SHR_FUNC_ENTER(unit);

    /* If both overlay and underlay exist, the action comes from overlay. */
    if (info->ol_egress_if) {
        tmp_egress_if = info->ol_egress_if;
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_l3_ecmp_max_paths_get(unit, &max_paths));
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_l3_egress_obj_id_resolve(unit,
                                            ...
                                            &nh_idx,
                                            &nh_type));
    if ((nh_type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_UL) ||
        (nh_type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_OL)) {
        ecmp_member_size = max_paths;
        ecmp_info.ecmp_intf = tmp_egress_if;
        SHR_IF_ERR_VERBOSE_EXIT
             (bcm_ltsw_l3_ecmp_get(unit, &ecmp_info, ecmp_member_size,
                                   ecmp_member_array, &ecmp_member_count));

    } else{
        /* Reuse ecmp member struct for common handling. */
        ecmp_member_count = 1;
        ecmp_member_array[0].egress_if = tmp_egress_if;
    }

    while (ecmp_member_idx < ecmp_member_count) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_l3_egress_obj_id_resolve(unit,
                                                ecmp_member_array[ecmp_member_idx].egress_if,
                                                &nh_idx,
                                                &nh_type));
        ...
        if ((info->valid_elements & BCM_FLOW_PORT_ENCAP_FLAGS_VALID) &&
            ...
            if (info->valid_elements & BCM_FLOW_PORT_ENCAP_VLAN_PRI_MAP_ID_VALID) {
                l2_tag_info.is_phb_map_id = 1;
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_qos_map_id_resolve(unit, info->vlan_pri_map_id,
                                                  &qos_type, &qos_base_ptr));
                l2_tag_info.phb_egr_l2_int_pri_to_otag_id = qos_base_ptr;
            } else {
                l2_tag_info.is_phb_map_id = 0;
        ...
        if ((info->valid_elements & BCM_FLOW_PORT_ENCAP_FLAGS_VALID) &&
            ...
            if (info->flags & BCM_FLOW_PORT_ENCAP_FLAG_OPAQUE_TAG_ADD) {
                ...
                l2_tag_info.opaque_tag_not_present_action =
                    BCMI_LTSW_OPAQUE_TAG_NOT_PRESENT_ACTION_NONE;
            }
            if (info->flags & BCM_FLOW_PORT_ENCAP_FLAG_OPAQUE_TAG_REPLACE) {
                ...
                l2_tag_info.opaque_tag_present_action =
                    BCMI_LTSW_OPAQUE_TAG_PRESENT_ACTION_NONE;
            }
        }
                                ...
                                bcm_flow_port_encap_t *info)
{
    ...
    uint16_t tpid = 0;
    uint32_t opaque_action_present = 0;
    uint32_t opaque_action_not_present = 0;
    int qos_base_ptr = 0;
    bcm_if_t tmp_egress_if = info->egress_if;
    ...
    if (l2_tag_info.is_phb_map_id) {
        qos_base_ptr = l2_tag_info.phb_egr_l2_int_pri_to_otag_id;
        if (qos_base_ptr != 0) {
            SHR_IF_ERR_VERBOSE_EXIT
        ...
        info->valid_elements |= BCM_FLOW_PORT_ENCAP_VLAN_PRI_MAP_ID_VALID;
    } else {
    ...
    opaque_action_present = l2_tag_info.opaque_tag_present_action;
    if (opaque_action_present == BCMI_LTSW_OPAQUE_TAG_PRESENT_ACTION_REPLACE) {
        info->flags |= BCM_FLOW_PORT_ENCAP_FLAG_OPAQUE_TAG_REPLACE;
        info->flags |= BCM_FLOW_PORT_ENCAP_FLAG_OPAQUE_TAGGED;
    } else if (opaque_action_present == BCMI_LTSW_OPAQUE_TAG_PRESENT_ACTION_DELETE) {
        info->flags |= BCM_FLOW_PORT_ENCAP_FLAG_OPAQUE_TAG_DELETE;
        info->flags |= BCM_FLOW_PORT_ENCAP_FLAG_OPAQUE_TAGGED;
    } else {
        /* No need to update flags. */
    }
    opaque_action_not_present = l2_tag_info.opaque_tag_not_present_action;
    if (opaque_action_not_present == BCMI_LTSW_OPAQUE_TAG_NOT_PRESENT_ACTION_ADD) {
...
/*!
 * \brief Set the dvp of given egress interface.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
xgs_flow_port_encap_egr_obj_dvp_set(int unit, bcm_if_t egress_if, int dvp)
{
    int nh_idx;
    bcmi_ltsw_l3_egr_obj_type_t nh_type;
    bcm_l3_egress_ecmp_t ecmp_info;
    ...
    if (nh_type == BCMI_LTSW_L3_EGR_OBJ_T_ECMP_UL) {
        ...
        /* Reuse ecmp member struct for common handling. */
        ecmp_member_count = 1;
        ecmp_member_array[0].egress_if = egress_if;
    }

    while (ecmp_member_idx < ecmp_member_count) {
        SHR_IF_ERR_EXIT_EXCEPT_IF
            (bcmi_ltsw_l3_egress_dvp_set(unit,
                                         ecmp_member_array[ecmp_member_idx].egress_if,
                                         dvp), SHR_E_NOT_FOUND);
...
/*!
 * \brief Clear L2 tag info from nexthop.
 *
 * \param [in]  unit           Unit number.
 * \param [in] intf Egress object ID.
 *
 ...
 */
static int
xgs_flow_port_encap_egr_obj_clear(int unit, bcm_if_t egress_if)
{
    int nh_idx;
    bcmi_ltsw_l3_egr_obj_type_t nh_type;
    bcm_l3_egress_ecmp_t ecmp_info;
    ...
    while (ecmp_member_idx < ecmp_member_count) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_l3_egress_delete_l2tag(unit, nh_idx));
                                ...
                                bcm_flow_encap_config_t *info)
{
    ...
    if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI) {
        ...
        supported_elements = BCM_FLOW_ENCAP_VPN_VALID |
                             ...
                             BCM_FLOW_ENCAP_VLAN_PRI_MAP_ID_VALID |
    ...
    /* Validate elements */
     ...
     if (info->valid_elements & BCM_FLOW_ENCAP_VLAN_PRI_MAP_ID_VALID) {
         if (info->valid_elements &
             (BCM_FLOW_ENCAP_PKT_PRI_VALID | BCM_FLOW_ENCAP_PKT_CFI_VALID)) {
             SHR_ERR_EXIT(SHR_E_PARAM);
         }
     }
                                  ...
                                  bcm_flow_encap_config_t *info)
{
    ...
    if (!sal_strncmp(lt_name, TNL_L2_VXLAN_ADAPT_VFIs, lt_len)) {
        ...
        info->valid_elements = BCM_FLOW_ENCAP_VPN_VALID |
                               ...
                               BCM_FLOW_ENCAP_VLAN_PRI_MAP_ID_VALID |
                               BCM_FLOW_ENCAP_FLAGS_VALID;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_ADAPT_VFI_DVP_GRPs, lt_len)) {
        ...
        info->valid_elements = BCM_FLOW_ENCAP_VPN_VALID |
                               ...
                               BCM_FLOW_ENCAP_VLAN_PRI_MAP_ID_VALID |
                               BCM_FLOW_ENCAP_FLAGS_VALID;
    } else if (!sal_strncmp(lt_name, VFI_EGR_ADAPT_PORT_GRPs, lt_len)) {
                        ...
                        bcmlt_entry_handle_t entry_hd)
{
    ...
    if (info->options & BCM_FLOW_ENCAP_OPTION_REPLACE) {
        /* Only fields exist in all encap LTs can be put here. */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(entry_hd, IS_PHB_MAP_IDs, &get_val));
        is_phb_marking = (int)get_val;
        if (is_phb_marking) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_remove(entry_hd, PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_IDs));
        } else {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_remove(entry_hd, OPRIs));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_remove(entry_hd, OCFIs));
        }
    ...
    if (info->valid_elements & BCM_FLOW_ENCAP_VNID_VALID) {
        /*
         * Special handling for VNID since it doesn't exist in
         * VFI_EGR_ADAPT_PORT_GRP.
         */
        if (info->options & BCM_FLOW_ENCAP_OPTION_REPLACE) {
            SHR_IF_ERR_VERBOSE_EXIT
        ...
        if (info->valid_elements & BCM_FLOW_ENCAP_VLAN_PRI_MAP_ID_VALID) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(entry_hd,
                                       ...
                                       qos_base_ptr));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(entry_hd, IS_PHB_MAP_IDs, 0x1));
        } else {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmlt_entry_field_add(entry_hd, IS_PHB_MAP_IDs, 0x0));
        }
                        ...
                        uint8_t *valid)
{
    ...
    *valid = 0;
    ...
    if (is_phb_marking) {
        if (valid_element & BCM_FLOW_ENCAP_VLAN_PRI_MAP_ID_VALID) {
             SHR_IF_ERR_VERBOSE_EXIT
                 (bcmlt_entry_field_get(entry_hd,
                                        PHB_EGR_L2_ADAPT_INT_PRI_TO_OTAG_IDs,
                                        &get_val));
...
/*!
 * \brief Recover tunnel tpid profile for FLOW.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE Success.
 ...
 */
static int
xgs_flow_encap_tunnel_tpid_recover(int unit)
{
    bcmlt_entry_handle_t entry_hd = BCMLT_INVALID_HDL;
    const char *lt_list[] = {TNL_L2_VXLAN_ADAPT_VFIs,
    ...
    for (idx = 0; idx < lt_num; idx++) {
        ...
        while ((rv = bcmi_lt_entry_commit(dunit, entry_hd, BCMLT_OPCODE_TRAVERSE,
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_vlan_otpid_entry_ref_count_add(unit, tpid_idx, 1));
        }
...
/*!
 * \brief Determines whether encap is configured on egr xlate table.
 *
 * \param [in]  unit  Unit number.
 * \param [in]  info  Flow encap information.
 *
 ...
 */
static int
xgs_flow_encap_on_egr_xlate(int unit,
                            bcm_flow_encap_config_t *info)
{
    if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_PORT_GROUP) {
        return TRUE;
    } else {
        return FALSE;
    }
 ...
 * \param [in]   lt_name       Logical table name.
 * \param [in]   info          Flow encap information.
 * \param [out]  cfg           Configurations for egr vxlan xlate.
 *
 * \retval SHR_E_NONE    No errors.
 * \retval !SHR_E_NONE   Failure.
 */
static int
xgs_flow_encap_data_set_to_egr_xlate (int unit,
                        const char *lt_name,
                        bcm_flow_encap_config_t *info,
                        bcmi_ltsw_vlan_egr_xlate_cfg_t *cfg)
{
    ...
    if (info->valid_elements & BCM_FLOW_ENCAP_DVP_GROUP_VALID) {
        cfg->gport_group = info->dvp_group;
    }
    if (info->valid_elements & BCM_FLOW_ENCAP_FLOW_PORT_VALID) {
        cfg->gport = info->flow_port;
    }
    ...
    if ((info->valid_elements & BCM_FLOW_ENCAP_FLAGS_VALID) &&
        ...
        cfg->ut_outer          = bcmVlanActionNone;
        ...
        if (info->valid_elements & BCM_FLOW_ENCAP_VLAN_VALID) {
            cfg->outer_vlan = info->vlan;
        }
        if (info->valid_elements & BCM_FLOW_ENCAP_PKT_PRI_VALID) {
            cfg->outer_pri = info->pri;

        }
        if (info->valid_elements & BCM_FLOW_ENCAP_PKT_CFI_VALID) {
            cfg->outer_cfi = info->cfi;
        }
        if (remark_cfi) {
            cfg->ot_outer_cfi = bcmVlanActionReplace;
        } else {
            cfg->ot_outer_cfi = bcmVlanActionNone;
        }
        ...
        if (info->valid_elements & BCM_FLOW_ENCAP_VLAN_PRI_MAP_ID_VALID) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_qos_map_id_resolve(unit, info->vlan_pri_map_id,
                                              &qos_type, &qos_base_ptr));
            cfg->dot1p_map_id = qos_base_ptr;
            cfg->flags |= FLOW_PORT_SET_OUTER_QOS_MAP_ID;
        } else {
            cfg->flags &= (~FLOW_PORT_SET_OUTER_QOS_MAP_ID);
        }
        ...
        if ((info->flags & BCM_FLOW_ENCAP_FLAG_SERVICE_VLAN_ADD) ||
            ...
            (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_PRI_TPID_REPLACE)) {
            if (info->options & BCM_FLOW_ENCAP_OPTION_REPLACE) {
                int old_tpid_idx = cfg->outer_tpid_index;
    ...
    if ((info->valid_elements & BCM_FLOW_ENCAP_FLAGS_VALID) &&
        ...
        if (info->flags & BCM_FLOW_ENCAP_FLAG_OPAQUE_TAG_ADD) {
            ...
            cfg->opaque_tag_not_present = bcmVlanActionNone;
        }
 ...
 * \param [in]   info          Flow encap information.
 * \param [in]   lt_name       Logical table name.
 * \param [in]   cfg           Configurations for egr vxlan xlate.
 * \param [out]  valid         Indicate whether valid data found.
 *
 * \retval SHR_E_NONE    No errors.
 * \retval !SHR_E_NONE   Failure.
 */
static int
xgs_flow_encap_data_get_from_egr_xlate (int unit,
                        bcm_flow_encap_config_t *info,
                        const char *lt_name,
                        const bcmi_ltsw_vlan_egr_xlate_cfg_t *cfg,
                        uint8_t *valid)
{
    uint32_t valid_element = 0;
    uint32_t valid_flags = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_flow_encap_data_valid_indicator(unit, lt_name, BCMLT_INVALID_HDL,
                                             &valid_element, &valid_flags));

    *valid = 0;
    if (valid_element & BCM_FLOW_ENCAP_VLAN_VALID) {
        info->vlan = cfg->outer_vlan;
        *valid = 1;
    }

    if (cfg->flags & FLOW_PORT_SET_OUTER_QOS_MAP_ID) {
        is_phb_marking = 1;
    }
    if (is_phb_marking) {
        if (valid_element & BCM_FLOW_ENCAP_VLAN_PRI_MAP_ID_VALID) {
             qos_base_ptr = (uint32_t)(cfg->dot1p_map_id);
             if (qos_base_ptr != 0) {
                 SHR_IF_ERR_VERBOSE_EXIT
                     (bcmi_ltsw_qos_map_id_construct(unit, qos_base_ptr,
                                                     bcmiQosMapTypeL2Egress,
        ...
        if (valid_element & BCM_FLOW_ENCAP_PKT_PRI_VALID) {
            info->pri = cfg->outer_pri;
            *valid = 1;
        }

        if (valid_element & BCM_FLOW_ENCAP_PKT_CFI_VALID) {
            info->cfi = cfg->outer_cfi;
            *valid = 1;
        }
    }

    if (valid_element & BCM_FLOW_ENCAP_DVP_GROUP_VALID) {
        info->dvp_group = cfg->gport_group;
        *valid = 1;
    }

    if (valid_element & BCM_FLOW_ENCAP_FLOW_PORT_VALID) {
        info->flow_port = cfg->gport;
        *valid = 1;
    }

    if (valid_element & BCM_FLOW_ENCAP_FLAGS_VALID) {
        uint64_t action_present = ACTION_IF_PRESENT_NO_ACTION;
        uint64_t action_not_present = ACTION_IF_PRESENT_NO_ACTION;
        int tpid_idx = 0;
        uint16_t tpid = 0;
        ...
        if ((cfg->otag_action == bcmVlanActionReplace) &&
            ...
            (cfg->outer_tpid_action == bcmVlanTpidActionModify)) {
            action_present = ACTION_IF_PRESENT_REPLACE_VID_PRI_TPID;
        } else if ((cfg->otag_action == bcmVlanActionReplace) &&
                   (cfg->outer_tpid_action == bcmVlanTpidActionModify)) {
            action_present = ACTION_IF_PRESENT_REPLACE_VID_TPID;
        } else if ((cfg->otag_action == bcmVlanActionReplace) &&
                   (cfg->ot_outer_pkt_prio == bcmVlanActionReplace)) {
            action_present = ACTION_IF_PRESENT_REPLACE_VID_PRI_ONLY;
        } else if (cfg->otag_action == bcmVlanActionDelete) {
            action_present = ACTION_IF_PRESENT_DELETE;
        } else if (cfg->otag_action == bcmVlanActionReplace) {
            action_present = ACTION_IF_PRESENT_REPLACE_VID_ONLY;
        } else if (cfg->ot_outer_pkt_prio == bcmVlanActionReplace) {
            action_present = ACTION_IF_PRESENT_REPLACE_PRI_ONLY;
        } else if (cfg->outer_tpid_action == bcmVlanTpidActionModify) {
            action_present = ACTION_IF_PRESENT_REPLACE_TPID_ONLY;
        } else {
            action_present = ACTION_IF_PRESENT_NO_ACTION;
        }
        ...
        if ((info->flags & BCM_FLOW_ENCAP_FLAG_SERVICE_VLAN_ADD) ||
            ...
            (info->flags & BCM_FLOW_ENCAP_FLAG_TPID_REPLACE) ||
            (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_PRI_TPID_REPLACE)) {
            tpid_idx = (int)(cfg->outer_tpid_index);
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_vlan_otpid_entry_get(unit, tpid_idx, &tpid));
            info->tpid = tpid;
            *valid = 1;
     ...
     if (valid_element & BCM_FLOW_ENCAP_FLAGS_VALID) {
         if (cfg->opaque_tag_present == bcmVlanActionReplace) {
             info->flags |= BCM_FLOW_ENCAP_FLAG_OPAQUE_TAG_REPLACE;
             info->flags |= BCM_FLOW_ENCAP_FLAG_OPAQUE_TAGGED;
         } else if (cfg->opaque_tag_present == bcmVlanActionDelete) {
             info->flags |= BCM_FLOW_ENCAP_FLAG_OPAQUE_TAG_DELETE;
             info->flags |= BCM_FLOW_ENCAP_FLAG_OPAQUE_TAGGED;
         } else {
             /* No need to update flags. */
...
/*!
 * \brief Configuration on egr xlate table based on encap info.
 *
 * \param [in] unit   Unit number.
 * \param [in] opt    Opt type from ltsw_flow_hw_opt_type_t.
 * \param [in] info   Flow encap information.
 ...
 */
static int
xgs_flow_encap_egr_xlate_ctrl(int unit,
                              bcmint_flow_hw_opt_type_t opt,
                              bcm_flow_encap_config_t *info)
{
    const char *lt_name = VFI_EGR_ADAPT_PORT_GRPs;
    uint8_t valid = 0;
    int port_class = BCM_PORT_INVALID;
    bcm_vlan_t outer_vlan = BCM_VLAN_INVALID;
    bcm_vlan_t inner_vlan = BCM_VLAN_INVALID;
    bcmi_ltsw_vlan_egr_xlate_cfg_t egr_xlate_cfg;

    SHR_FUNC_ENTER(unit);

    sal_memset(&egr_xlate_cfg, 0, sizeof(egr_xlate_cfg));
    port_class = info->port_group;
    outer_vlan = info->vpn;
    /* Fixed as 1 for flexflow. */
    egr_xlate_cfg.vxlan_decap = 0x1;
    if (opt == HW_OPT_GET) {
        SHR_IF_ERR_VERBOSE_EXIT
             (bcmi_ltsw_vlan_egr_xlate_get(unit, port_class, outer_vlan,
                                           inner_vlan, &egr_xlate_cfg));
        SHR_IF_ERR_VERBOSE_EXIT
            (xgs_flow_encap_data_get_from_egr_xlate(unit, info, lt_name,
                                                    &egr_xlate_cfg, &valid));
    } else if (opt == HW_OPT_SET) {
        if (info->options & BCM_FLOW_ENCAP_OPTION_REPLACE) {
            SHR_IF_ERR_VERBOSE_EXIT
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (xgs_flow_encap_data_set_to_egr_xlate(unit, lt_name, info,
                                                  &egr_xlate_cfg));
        SHR_IF_ERR_VERBOSE_EXIT
             (bcmi_ltsw_vlan_egr_xlate_add(unit, port_class, outer_vlan,
                                           inner_vlan, &egr_xlate_cfg));
    } else if (opt == HW_OPT_CLR) {
        SHR_IF_ERR_VERBOSE_EXIT
             (bcmi_ltsw_vlan_egr_xlate_delete(unit, port_class, outer_vlan,
                                              inner_vlan, &egr_xlate_cfg));
    } else {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit, "invalid opt:%d!\n"), opt));
...
/*!
 * \brief Helper function for flow encap to traverse egr xlate tables.
 *
 * This function is used to parse info from egr xlate entries specified for
 *  flexflow and call flow encap callback.
 *
 * \param [in] unit              Unit number.
 * \param [in] port_class        Group ID.
 ...
 */
static int
xgs_flow_encap_egr_xlate_traverse_cb(int unit, int port_class,
                                     bcm_vlan_t outer_vlan,
                                     bcm_vlan_t inner_vlan,
                                     bcmi_ltsw_vlan_egr_xlate_cfg_t *cfg,
                                     void *user_data)
{
    egr_vlan_xlate_user_data_t *egr_xlate_user_data = user_data;
    bcm_flow_encap_config_t info;
    int rv = SHR_E_NONE;
    const char *lt_name = VFI_EGR_ADAPT_PORT_GRPs;
    uint8_t valid = 0;
    ...
    /* Skip non flexflow entry. */
    if (cfg->vxlan_decap != 1) {
        SHR_EXIT();
    }
    ...
    info.vpn = outer_vlan;
    info.port_group = port_class;
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_flow_encap_data_get_from_egr_xlate(unit, &info, lt_name, cfg,
                                                &valid));

    if (egr_xlate_user_data->cb) {
        rv = (egr_xlate_user_data->cb)(unit, &info, 0, NULL,
                                       egr_xlate_user_data->user_data);
...
/*!
 * \brief Traverse flexflow specified entries in egr xlate tables.
 *
 * This function is used to traverse flexflow specified entries and
 * call callback function.
 *
 * \param [in] unit              Unit number.
 * \param [in] cb                Callback function of flow encap traverse.
 * \param [in] user_data         User data to a call back.
 *
 * \retval SHR_E_NONE            No errors.
 * \retval !SHR_E_NONE           Failure.
 */
static int
xgs_flow_encap_egr_xlate_traverse(int unit,
                                  bcm_flow_encap_traverse_cb cb,
                                  void *user_data)
{
    egr_vlan_xlate_user_data_t egr_xlate_user_data;

    SHR_FUNC_ENTER(unit);

    egr_xlate_user_data.cb = cb;
    egr_xlate_user_data.user_data = user_data;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_vlan_egr_xlate_traverse(unit,
                                           xgs_flow_encap_egr_xlate_traverse_cb,
                                           &egr_xlate_user_data));
                         ...
                         bcm_flow_match_config_t *info)
{
    ...
    if (BCM_GPORT_IS_TRUNK(info->port)) {
        tid = BCM_GPORT_TRUNK_GET(info->port);
        SHR_IF_ERR_EXIT
            (bcm_ltsw_trunk_get(unit, tid, NULL, 0, NULL, &port_cnt));
        if (port_cnt > 0) {
            SHR_ALLOC(member, sizeof(bcm_trunk_member_t) * port_cnt,
                      "bcmFlowTrunkMemberArray");
            SHR_NULL_CHECK(member, SHR_E_MEMORY);
            sal_memset(member, 0, sizeof(bcm_trunk_member_t) * port_cnt);

            SHR_IF_ERR_EXIT
                (bcm_ltsw_trunk_get(unit, tid, NULL,
                                    port_cnt, member, &port_cnt));
        }
    } else {
        SHR_ALLOC(member, sizeof(bcm_trunk_member_t), "bcmFlowGPort");
        SHR_NULL_CHECK(member, SHR_E_MEMORY);
        sal_memset(member, 0, sizeof(bcm_trunk_member_t));
        member[0].gport = info->port;
    }
                                  ...
                                  bcm_flow_match_config_t *info)
{
    ...
    if (!sal_strncmp(lt_name, VFI_ING_ADAPT_OVID_PORT_GRPs, lt_len)) {
        info->criteria = BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_VLAN;
        info->valid_elements = BCM_FLOW_MATCH_PORT_VALID |
                               BCM_FLOW_MATCH_VLAN_VALID;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_VFI_ASSIGNMENT_OVID_VNIDs, lt_len)) {
        ...
        info->valid_elements = BCM_FLOW_MATCH_VNID_VALID |
                               BCM_FLOW_MATCH_VLAN_VALID;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_VFI_ASSIGNMENT_OVID_VNID_SRC_IPV4s, lt_len)) {
        /* Determined as BCM_FLOW_MATCH_CRITERIA_VN_ID based on SVP value */
        info->criteria = BCM_FLOW_MATCH_CRITERIA_SIP_VNID;
        info->valid_elements = BCM_FLOW_MATCH_VNID_VALID |
                               BCM_FLOW_MATCH_VLAN_VALID |
                               BCM_FLOW_MATCH_SIP_VALID;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_VFI_ASSIGNMENT_OVID_VNID_SRC_IPV6s, lt_len)) {
        /* Determined as BCM_FLOW_MATCH_CRITERIA_VN_ID based on SVP value */
        info->criteria = BCM_FLOW_MATCH_CRITERIA_SIP_VNID;
        info->valid_elements = BCM_FLOW_MATCH_VNID_VALID |
                               BCM_FLOW_MATCH_VLAN_VALID |
                               BCM_FLOW_MATCH_SIP_V6_VALID;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_SVP_ASSIGNMENT_OVID_SRC_IPV4s, lt_len)) {
        info->criteria = BCM_FLOW_MATCH_CRITERIA_SIP;
        info->valid_elements = BCM_FLOW_MATCH_SIP_VALID |
                               BCM_FLOW_MATCH_VLAN_VALID;
    } else if (!sal_strncmp(lt_name, TNL_L2_VXLAN_DECAP_SVP_ASSIGNMENT_OVID_SRC_IPV6s, lt_len)) {
        info->criteria = BCM_FLOW_MATCH_CRITERIA_SIP;
                                   ...
                                   const char *lt_name)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_flow_traverse_snapshot_start(unit, entry_hdl));
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_flow_traverse_snapshot_done(unit, entry_hdl));
   ...
   const char **field_name)
{
    ...
    } else if (type == bcmSwitchIngParseL2TunnelTermArp) {
        *field_name = DECAP_ARP_PAYLOADs;
    } else if (type == bcmSwitchIngParseL2TunnelTermRarp) {
        *field_name = DECAP_RARP_PAYLOADs;
    } else {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }
...
xgs_ltsw_flow_init(int unit)
{
    ...
    if (warm) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (xgs_flow_tunnel_decap_port_profile_recover(unit));

        SHR_IF_ERR_VERBOSE_EXIT
            (xgs_flow_encap_tunnel_tpid_recover(unit));
    }
    ...
    bcm_flow_port_t *flow_port)
{
    ...
    if (opt == HW_OPT_SET) {
        ...
        /* Clear the encap configuration. */
        bcm_flow_port_encap_t encap_info;

        bcm_flow_port_encap_t_init(&encap_info);
        encap_info.flow_port = flow_port->flow_port_id;
        encap_info.options = BCM_FLOW_PORT_ENCAP_OPTION_CLEAR;
        SHR_IF_ERR_VERBOSE_EXIT
            (xgs_ltsw_flow_port_encap_hw_opt(unit, HW_OPT_SET, &encap_info));
...
int
xgs_ltsw_flow_match_trunk_member_delete(
    int unit,
    ...
    bcm_port_t *trunk_member_array)
{
    ...
    bcm_port_t port = 0;
    int vp = 0;
    bcm_gport_t tgport = BCM_GPORT_INVALID;

    SHR_FUNC_ENTER(unit);

    BCM_GPORT_TRUNK_SET(tgport,trunk_id);
    rv = bcmi_ltsw_flow_match_flow_port_find(unit,
    ...
    if (rv == SHR_E_UNAVAIL ||
        rv == SHR_E_INIT ||
        rv == SHR_E_NOT_FOUND) {
        /* No need to update VP in the trunk memmbers */
        SHR_EXIT();
    } else {
        SHR_IF_ERR_EXIT(rv);
        LOG_DEBUG(BSL_LOG_MODULE,                       \
                  (BSL_META_U(unit, "trunk member delete, vp %d \n"), vp));
    }
    ...
    if (info->flow_handle == BCMINT_FLOW_ID_INT) {
        SHR_IF_ERR_VERBOSE_EXIT
            (xgs_flow_encap_int_ctrl(unit, opt, info,
                                     num_of_fields, field));
    } else if (xgs_flow_encap_on_egr_xlate (unit, info)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (xgs_flow_encap_egr_xlate_ctrl(unit, opt, info));
    } else {
           SHR_IF_ERR_VERBOSE_EXIT
    ...
    void *user_data)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_flow_encap_egr_xlate_traverse(unit, cb, user_data));
    ...
    bcm_flow_port_encap_t *info)
{
    ...
    if (opt == HW_OPT_GET) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_ltsw_virtual_port_overlay_egress_obj_get
                 (unit, info->flow_port, &info->ol_egress_if));
        if ((info->ol_egress_if != 0) &&
            (vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (xgs_flow_port_encap_egr_obj_get(unit, info));
        }
        ...
        if (info->options & BCM_FLOW_PORT_ENCAP_OPTION_CLEAR) {
            ...
            /* Clear underlay nexthop linked to access VP. */
            local_rv = bcmi_ltsw_virtual_port_egress_obj_get(unit,
                                                             info->flow_port,
                                                             &old_egress_if);
            if ((local_rv == SHR_E_NONE) && (old_egress_if != 0) &&
                (!(vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT))) {

                SHR_IF_ERR_EXIT_EXCEPT_IF
                    (xgs_flow_port_encap_egr_obj_clear(unit, old_egress_if), SHR_E_NOT_FOUND);
            }
            if (old_egress_if != 0) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (xgs_flow_port_encap_egr_obj_dvp_set(unit, old_egress_if, 0));
            }
            ...
            /* Clear overlay nexthop linked to network VP. */
            local_rv = bcmi_ltsw_virtual_port_overlay_egress_obj_get(unit,
                                                             info->flow_port,
                                                             &old_egress_if);
            if ((local_rv == SHR_E_NONE) && (old_egress_if != 0) &&
                (vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT)) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_virtual_port_overlay_egress_obj_set(
                    unit, info->flow_port, 0x0));
        } else {
            if (info->valid_elements & BCM_FLOW_PORT_ENCAP_FLAGS_VALID) {
            ...
            if (info->valid_elements & BCM_FLOW_PORT_ENCAP_EGRESS_IF_VALID) {
                /* reset dvp set on old nexthop. */
                local_rv = bcmi_ltsw_virtual_port_egress_obj_get(unit,
                                                                 info->flow_port,
                                                                 &old_egress_if);
                if ((local_rv == SHR_E_NONE) && (old_egress_if != 0)
                    && (info->egress_if != old_egress_if)) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (xgs_flow_port_encap_egr_obj_dvp_set(unit, old_egress_if, 0));
                }
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_virtual_port_egress_obj_set(unit, info->flow_port,
                                                           info->egress_if));
                SHR_IF_ERR_EXIT_EXCEPT_IF
                    (xgs_flow_port_encap_egr_obj_dvp_set(unit, info->egress_if, vp), SHR_E_NOT_FOUND);
                ...
                if (!(vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT)) {
                    ...
                    /*
                     * Tried to clear l2 tag information from original nexthop.
                     * If nexthop has not been converted to L2 tag view,
                     * SHR_E_NOT_FOUND is returned and expected.
                     */
                    if ((info->egress_if != old_egress_if) &&
                        (old_egress_if != 0)) {
                        SHR_IF_ERR_EXIT_EXCEPT_IF
                            (xgs_flow_port_encap_egr_obj_clear(unit, old_egress_if), SHR_E_NOT_FOUND);
                    }
            ...
            if (info->valid_elements & BCM_FLOW_PORT_ENCAP_OL_EGRESS_IF_VALID) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmi_ltsw_virtual_port_overlay_egress_obj_get(
                        unit, info->flow_port, &old_egress_if));
                /* Update tag action to nexthop linked to network VP. */
                if (vp_info.flags & BCMI_LTSW_VIRTUAL_VP_NETWORK_PORT) {
                    SHR_IF_ERR_VERBOSE_EXIT
                        (xgs_flow_port_encap_egr_obj_set(unit, info));
                    /*
                     ...
                     * SHR_E_NOT_FOUND is returned and expected.
                     */
                    if ((info->ol_egress_if != old_egress_if) &&
                        (old_egress_if != 0)) {
                        SHR_IF_ERR_EXIT_EXCEPT_IF
                            (xgs_flow_port_encap_egr_obj_clear(
                                 unit, old_egress_if), SHR_E_NOT_FOUND);
                    }
                }
            }
...
int
xgs_ltsw_flow_port_encap_l3_ecmp_notify(
    int unit,
    ...
    int add)
{
    ...
    SHR_NULL_CHECK(ecmp_member_array, SHR_E_PARAM);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_l3_egress_obj_id_resolve(unit,
                                            ...
                                            &nh_idx,
                                            &nh_type));
    /* SW only updates the underlay ECMP. */
    if (nh_type != BCMI_LTSW_L3_EGR_OBJ_T_ECMP_UL) {
        SHR_EXIT();
    }

    /* SW only updates the underlay ECMP binded to flow port . */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_virtual_port_egress_obj_used(unit,
                                                ecmp_group_id,
                                                &bind_vp));
    ...
    while (ecmp_member_idx < ecmp_member_count) {
        if (add) {
            /* Sync egress adaptation key type in case ECMP member changes. */
            BCM_GPORT_FLOW_PORT_ID_SET(flow_port, bind_vp);
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_virtual_port_egress_adapt_key_sync(
                    unit, flow_port,
                    ecmp_member_array[ecmp_member_idx].egress_if));
        } else {
            /* For DVP, No update during add but reset during delete. */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_ltsw_l3_egress_dvp_set(
                    unit,
                    ecmp_member_array[ecmp_member_idx].egress_if,
                    0x0));
        }
    ...
    int arg)
{
    ...
    switch (type) {
        ...
        case bcmSwitchIngParseL2TunnelTermArp:
        case bcmSwitchIngParseL2TunnelTermRarp:
            SHR_IF_ERR_VERBOSE_EXIT
    ...
    bcm_gport_t flow_port_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_ltsw_flow_es_filter_stat_id_get(unit, flow_port_id,
                                             &stat_counter_id));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id,
                                               &counter_info));
    ...
    bcm_vpn_t vpn)
{
    bcmi_ltsw_flexctr_counter_info_t counter_info = {0};
    uint32_t stat_counter_id = 0;
    int rv_ing = SHR_E_NONE;
    int rv_egr = SHR_E_NONE;
    ...
    /* First check the exsitence of flex counter. */
    rv_ing = xgs_ltsw_flow_vpn_stat_id_get(unit, vpn,
                                           bcmFlexctrDirectionIngress,
                                           &stat_counter_id);
    if (rv_ing == SHR_E_NONE) {
        rv_ing = bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id,
                                                       &counter_info);
    }
    if (rv_ing == SHR_E_NONE) {
        /* Detach if counter does exist. */
       rv_ing = xgs_flow_vpn_stat_update(unit, vpn, &counter_info, FALSE);
       if (rv_ing == SHR_E_NONE) {
           rv_ing = bcmi_ltsw_flexctr_detach_counter_id_status_update(
                        unit, stat_counter_id);
       }
    } else {
        rv_ing = SHR_E_NOT_FOUND;
    }

    /* First check the exsitence of flex counter. */
    rv_egr = xgs_ltsw_flow_vpn_stat_id_get(unit, vpn,
                                           bcmFlexctrDirectionEgress,
                                           &stat_counter_id);
    if (rv_egr == SHR_E_NONE) {
        rv_egr = bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id,
                                                       &counter_info);
    }
    if (rv_egr == SHR_E_NONE) {
        /* Detach if counter does exist. */
       rv_egr = xgs_flow_vpn_stat_update(unit, vpn, &counter_info, FALSE);
       if (rv_egr == SHR_E_NONE) {
           rv_egr = bcmi_ltsw_flexctr_detach_counter_id_status_update(
                        unit, stat_counter_id);
       }
    } else {
        rv_egr = SHR_E_NOT_FOUND;
    }

    if (rv_ing == SHR_E_NONE ) {
        if (rv_egr == SHR_E_NOT_FOUND) {
            SHR_ERR_EXIT(SHR_E_NONE);
        } else {
            SHR_ERR_EXIT(rv_egr);
        }
    } else if (rv_ing == SHR_E_NOT_FOUND ){
        SHR_ERR_EXIT(rv_egr);
    } else {
        SHR_ERR_EXIT(rv_ing);
    }
...
int
xgs_ltsw_flow_vpn_stat_detach_with_id(
    int unit,
    bcm_vpn_t vpn,
    uint32_t stat_counter_id)
{
    ...
    uint32_t *stat_counter_id)
{
    ...
    if (direction == bcmFlexctrDirectionEgress) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmlt_entry_field_get(entry_hd, CTR_ING_EFLEX_ACTIONs,
                                   &flex_ctr_action));
        if (flex_ctr_action == BCMI_LTSW_FLEXCTR_ACTION_INVALID) {
            SHR_IF_ERR_VERBOSE_EXIT(SHR_E_NOT_FOUND);
        }
    ...
    bcm_gport_t flow_port_id)
{
    ...
    /* First check the exsitence of flex counter. */
    rv_ing = xgs_ltsw_flow_port_stat_id_get(unit, flow_port_id,
                                            bcmFlexctrDirectionIngress,
                                            &stat_counter_id);
    if (rv_ing == SHR_E_NONE) {
        rv_ing = bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id,
    ...
    if (rv_ing == SHR_E_NONE) {
        /* Detach if counter does exist. */
       rv_ing = bcmi_ltsw_virtual_port_stat_detach_with_id(unit, flow_port_id,
                                                           stat_counter_id);
    } else {
        rv_ing = SHR_E_NOT_FOUND;
    }
    ...
    /* First check the exsitence of flex counter. */
    rv_egr = xgs_ltsw_flow_port_stat_id_get(unit, flow_port_id,
                                            bcmFlexctrDirectionEgress,
                                            &stat_counter_id);
    if (rv_egr == SHR_E_NONE) {
        rv_egr = bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id,
    ...
    if (rv_egr == SHR_E_NONE) {
        /* Detach if counter does exist. */
       rv_egr = xgs_flow_vp_stat_update(unit, flow_port_id, &counter_info,
                                        FALSE);
       if (rv_egr == SHR_E_NONE) {
           rv_egr = bcmi_ltsw_flexctr_detach_counter_id_status_update(
                        unit, stat_counter_id);
...
int
xgs_ltsw_flow_port_stat_detach_with_id(
    int unit,
    bcm_gport_t flow_port_id,
    uint32_t stat_counter_id)
{
    ...
    bcm_gport_t tunnel_id)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_ltsw_flow_tunnel_initiator_stat_id_get(unit, tunnel_id,
                                                    &stat_counter_id));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id,
                                               &counter_info));
...
int
xgs_ltsw_flow_tunnel_initiator_flexctr_object_get(
    int unit,
    bcm_gport_t tunnel_id,
    uint32_t *value)
{
    ...
    /*
    * Based on hardware implementation, encap index is not used as counter
    * object directly and should be adjusted based on tunnel type.
    */
    if (tnl_type == bcmiTunnelTypeIp6L3) {
        *value = encap_idx * 8;
    } else if (tnl_type == bcmiTunnelTypeIpL3) {
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_ltsw_flow_encap_stat_id_get(unit, info, num_of_fields, field,
                                         &stat_counter_id));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id,
                                               &counter_info));
    ...
    uint32_t stat_counter_id)
{
    ...
    if ((counter_info.source != bcmFlexctrSourceL2TunnelVpnAssignment) &&
        (counter_info.source != bcmFlexctrSourceL2TunnelSvpAssignment)  &&
    ...
    if ((counter_info.source == bcmFlexctrSourceL2TunnelVpnAssignment) &&
        ...
        (info->criteria != BCM_FLOW_MATCH_CRITERIA_SIP_VNID)) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit, "counter source (%d) not matched!\n"),
                                        counter_info.source));
    }
    if ((counter_info.source == bcmFlexctrSourceL2TunnelSvpAssignment) &&
        (info->criteria != BCM_FLOW_MATCH_CRITERIA_SIP)  &&
        (info->criteria != BCM_FLOW_MATCH_CRITERIA_SIP_VNID)) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit, "counter source (%d) not matched!\n"),
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_ltsw_flow_match_stat_id_get(unit, info, num_of_fields, field,
                                         &stat_counter_id));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_ltsw_flexctr_counter_id_info_get(unit, stat_counter_id,
                                               &counter_info));
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (xgs_ltsw_flow_tunnel_terminator_stat_id_get(unit, info, num_of_fields,
