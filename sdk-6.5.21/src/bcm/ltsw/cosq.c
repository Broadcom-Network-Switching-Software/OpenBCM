...
{
    return mbcm_ltsw_cosq_num_queue_get(unit, num_ucq, num_mcq);
}
...
{
    return mbcm_ltsw_cosq_port_map_info_get(unit, lport, info);
}
...
{
    return mbcm_ltsw_cosq_device_info_get(unit, info);
                                 ...
                                 bcm_port_t *local_port)
{
    ...
    if (!BCM_GPORT_IS_SET(gport)) {
        rv = bcmi_ltsw_port_gport_validate(unit, gport, &port);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
    ...
    rv = bcmi_ltsw_modid_is_local(unit, module, &result);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    if (result == FALSE) {
                                  ...
                                  bcm_port_t *port, int *id)
{
    ...
    return mbcm_ltsw_cosq_gport_port_resolve(unit, gport, port, id);
}
...
/*!
 * \brief Get COSQ device level information.
 *
 * \param [in]  unit          Unit number.
 * \param [out]  info         COSQ device level information.
 *
 * \retval SHE_E_NONE         No errors.
 * \retval !SHR_E_NONE        Failure.
 */
int
bcm_ltsw_cosq_device_info_get(
    int unit,
    bcm_cosq_device_info_t *info)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (mbcm_ltsw_cosq_device_info_get(unit, &device_info));

    info->max_pkt_size = device_info.max_pkt_byte;
    info->pkt_header_size = device_info.mmu_hdr_byte;
    info->jumbo_pkt_size = device_info.jumbo_pkt_size;
    info->default_mtu = device_info.default_mtu_size;
    info->cell_size = device_info.mmu_cell_size;
    info->cell_num = device_info.mmu_total_cell;
    info->buffer_num = device_info.num_itm;
    info->service_pool_num = device_info.num_service_pool;
    info->port_priority_group_num = device_info.num_pg;
    info->uc_queue_num = device_info.num_ucq;
    info->mc_queue_num = device_info.num_mcq;
    info->cpu_queue_num = device_info.num_cpu_queue;
