...
#define FP_HA_BLK_OFFSET_ADJUST(_blk_id, _blk_off, _prev_sz, _sz_diff)      \
{                                                                           \
    uint32_t _elm_off = 0;                                                  \
    uint32_t _elm_id = 0;                                                   \
    if ((_blk_id != BCM_FIELD_HA_BLK_ID_INVALID) &&                         \
        (_blk_off != BCM_FIELD_HA_BLK_OFFSET_INVALID) &&                    \
        (_blk_off > sizeof(bcmi_field_ha_blk_hdr_t))) {                     \
        _elm_off = (_blk_off - sizeof(bcmi_field_ha_blk_hdr_t));            \
        _elm_id = (_elm_off / _prev_sz[_blk_id]);                           \
        _blk_off += (_elm_id * _sz_diff[_blk_id]);                          \
    }                                                                       \
}
                                ...
                                bool warm)
{
    ...
    if (warm == TRUE) {
        ...
        max_entries = (tbl_size * num_pools);
        ha_blk_size = (sizeof(bcmi_field_ha_blk_hdr_t) +
                                ...
                                bool warm)
{
    ...
    if (warm == TRUE) {
        ...
        if (stage_info->tbls_info->conflict_sid) {
            SHR_IF_ERR_VERBOSE_EXIT
                 (bcmi_lt_field_value_range_get(
                       unit,
                       ...
                       &min_value,
                       &max_value));
            presel_info->conflict_id_max = max_value;
        }
    } else {
        ha_blk_size = sizeof(bcmi_field_ha_blk_hdr_t) +
        ...
        if (stage_info->tbls_info->conflict_sid) {
            SHR_IF_ERR_VERBOSE_EXIT
                 (bcmi_lt_field_value_range_get(
                       ...
                       &max_value));
            presel_info->conflict_id_max = max_value;
            presel_info->last_allocated_conflict_id = 0;
        }
        ...
        sal_memset(&presel_info->conflict_id_bmp, 0x0, sizeof(bcmi_field_conflict_id_bmp_t));
                         ...
                         bool warm)
{
    ...
    if (stage_info->tbls_info->virtual_slice_grp_fid) {
        SHR_IF_ERR_VERBOSE_EXIT
           (mbcm_ltsw_field_stage_reinit(unit, stage_info, warm));
    }
...
ltsw_field_warm_ha_init(int unit)
{
    ...
    uint32_t blk_size = 0;
    uint32_t ha_blk_size = 0;
    uint32_t prev_actual_size = 0;
    uint32_t prev_blk_size = 0;
    uint32_t num_ha_elements = 0;
    uint32_t element_size = 0;
    uint32_t prev_element_size = 0;
    bcmi_field_ha_blk_hdr_t *ha_blk_ptr = NULL;
    bcmi_field_ha_blk_info_t *hash_blk_info = NULL;
    ...
    bcmi_field_ha_hints_t *hint_blk_info = NULL;
    bcmi_field_ha_root_blk_t *root = NULL;
    uint8_t ha_blk_id = BCMI_FIELD_HA_BLK_ID_ROOT;
    uint32_t *prev_elm_sz = NULL;
    uint32_t *elm_sz_diff = NULL;
    uint32_t alloc_sz = 0;
    ...
    alloc_sz = (sizeof(uint32_t) * (root->max_ha_blk_id + 1));
    BCMINT_FIELD_MEM_ALLOC(prev_elm_sz,
                           alloc_sz,
                           "FP HA Info alloc.");
    BCMINT_FIELD_MEM_ALLOC(elm_sz_diff,
                           alloc_sz,
                           "FP HA Info alloc.");
    for (ha_blk_id = root->min_ha_blk_id;
        ...
        if (SHR_BITGET(root->in_use_ha_blk_id_bmp.w, ha_blk_id)) {
            ...
            /*
             * Check if HA block size has increased by
             * converting previous block size to multiple of 8.
             */
            prev_blk_size = ha_blk_ptr->blk_size;
            prev_actual_size = (((prev_blk_size + 7) >> 3) << 3);
            if (prev_actual_size != ha_blk_size) {
                ha_blk_ptr->blk_size = ha_blk_size;
            }

            if (ha_blk_id == BCMI_FIELD_HA_BLK_ID_ROOT) {
                continue;
            }
            SHR_IF_ERR_VERBOSE_EXIT
                (ltsw_field_ha_blk_type_num_pipe_get(unit,
                                                     ha_blk_ptr->blk_type,
                                                     &num_pipes));
            FP_HA_DYNAMIC_BLK_NUM_ELEMENTS(unit, num_pipes,
                                           ha_blk_ptr->blk_type,
                                           num_ha_elements);
            ...
            element_size =
                ((ha_blk_size - sizeof(bcmi_field_ha_blk_hdr_t)) /
                 num_ha_elements);
            prev_element_size =
                ((prev_blk_size - sizeof(bcmi_field_ha_blk_hdr_t)) /
                 num_ha_elements);
            if (element_size > prev_element_size) {
                struct_update = true;
                elm_sz_diff[ha_blk_id] = (element_size - prev_element_size);
            }
            prev_elm_sz[ha_blk_id] = prev_element_size;
        }
    }

    /* If no structures were upgraded. skip the below section. */
    if (struct_update == true) {
        for (ha_blk_id = root->min_ha_blk_id;
             ha_blk_id <= root->max_ha_blk_id;
             ha_blk_id++) {
            if (SHR_BITGET(root->in_use_ha_blk_id_bmp.w, ha_blk_id)) {
                ha_blk_ptr = FP_HA_BLK_HDR_PTR(unit, ha_blk_id);
                ha_blk_size = ha_blk_ptr->blk_size;
                blk_size = (ha_blk_size - sizeof(bcmi_field_ha_blk_hdr_t));
                switch (ha_blk_ptr->blk_type) {
                case BCMI_FIELD_HA_BLK_TYPE_IFP_PRESEL_HASH:
                case BCMI_FIELD_HA_BLK_TYPE_IFP_GROUP_HASH:
                case BCMI_FIELD_HA_BLK_TYPE_IFP_ENTRY_HASH:
                case BCMI_FIELD_HA_BLK_TYPE_VFP_PRESEL_HASH:
                ...
                case BCMI_FIELD_HA_BLK_TYPE_EMFT_ENTRY_HASH:
                    hash_blk_info =
                        ...
                        ((uint8_t *)ha_blk_ptr +
                         sizeof(bcmi_field_ha_blk_hdr_t));
                    element_size = (sizeof(bcmi_field_ha_blk_info_t));
                    num_ha_elements = (blk_size / element_size);
                    for (i = 0; i < num_ha_elements; i++) {
                        /* Update block offsets. */
                        FP_HA_BLK_OFFSET_ADJUST(hash_blk_info[i].blk_id,
                                                hash_blk_info[i].blk_offset,
                                                prev_elm_sz,
                                                elm_sz_diff);
                    }
                    break;
                case BCMI_FIELD_HA_BLK_TYPE_HINT_LIST:
                    hint_blk_info =
                        ...
                        ((uint8_t *)ha_blk_ptr +
                         sizeof(bcmi_field_ha_blk_hdr_t));
                    element_size = (sizeof(bcmi_field_ha_hints_t));
                    num_ha_elements = (blk_size / element_size);
                    for (i = 0; i < num_ha_elements; i++) {
                        /* Update block offsets. */
                        FP_HA_BLK_OFFSET_ADJUST(
                            hint_blk_info[i].hints.blk_id,
                            hint_blk_info[i].hints.blk_offset,
                            prev_elm_sz,
                            elm_sz_diff);
                    }
                    break;
                ...
                case BCMI_FIELD_HA_BLK_TYPE_EMFT_GROUP_OPER:
                    /*
                     * If this block type has not had any size change,
                     * skip the processing.
                     */
                    if (elm_sz_diff[ha_blk_id] == 0) {
                        continue;
                    }
                    group_oper =
                    ...
                    num_ha_elements = (blk_size / element_size);
                    FP_HA_BLK_OFFSET_ADJUST(ha_blk_id,
                                            ha_blk_ptr->free_blk_offset,
                                            prev_elm_sz,
                                            elm_sz_diff);
                    for (i = 0; i < num_ha_elements; i++) {
                        /* Update block offsets. */
                        FP_HA_BLK_OFFSET_ADJUST(group_oper[i].next.blk_id,
                                                group_oper[i].next.blk_offset,
                                                prev_elm_sz,
                                                elm_sz_diff);
                    }
                    break;
                ...
                case BCMI_FIELD_HA_BLK_TYPE_EMFT_PRESEL_OPER:
                    ...
                    for (i = 0; i < num_ha_elements; i++) {
                        /* Update block offsets. */
                        FP_HA_BLK_OFFSET_ADJUST(presel_oper[i].next.blk_id,
                                                presel_oper[i].next.blk_offset,
                                                prev_elm_sz,
                                                elm_sz_diff);
                    }
                    break;
                ...
                case BCMI_FIELD_HA_BLK_TYPE_EMFT_ENTRY_OPER:
                    ...
                    for (i = 0; i < num_ha_elements; i++) {
                        /* Update block offsets. */
                        FP_HA_BLK_OFFSET_ADJUST(entry_oper[i].next.blk_id,
                                                entry_oper[i].next.blk_offset,
                                                prev_elm_sz,
                                                elm_sz_diff);
                    }
                    break;
                case BCMI_FIELD_HA_BLK_TYPE_HINT_ENTRY:
                    ...
                    for (i = 0; i < num_ha_elements; i++) {
                        /* Update block offsets. */
                        FP_HA_BLK_OFFSET_ADJUST(hint_oper[i].next.blk_id,
                                                hint_oper[i].next.blk_offset,
                                                prev_elm_sz,
                                                elm_sz_diff);
                    }
                    break;
...
exit:
    BCMINT_FIELD_MEM_FREE(prev_elm_sz);
    BCMINT_FIELD_MEM_FREE(elm_sz_diff);
    SHR_FUNC_EXIT();
}

static int
bcmint_field_ha_blk_type_to_struct_id(int unit,
                                      bcmi_field_ha_blk_type_t blk_type,
                                      bcmissu_struct_id_t *struct_id)
{
    ...
    case BCMI_FIELD_HA_BLK_TYPE_ROOT:
        *struct_id = BCMI_FIELD_HA_ROOT_BLK_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_IFP_PRESEL_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_VFP_PRESEL_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EFP_PRESEL_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EMFP_PRESEL_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EMFT_PRESEL_INFO:
        *struct_id = BCMI_FIELD_HA_PRESEL_INFO_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_IFP_PRESEL_HASH:
    case BCMI_FIELD_HA_BLK_TYPE_IFP_GROUP_HASH:
    case BCMI_FIELD_HA_BLK_TYPE_IFP_ENTRY_HASH:
    ...
    case BCMI_FIELD_HA_BLK_TYPE_EMFT_GROUP_HASH:
    case BCMI_FIELD_HA_BLK_TYPE_EMFT_ENTRY_HASH:
        *struct_id = BCMI_FIELD_HA_BLK_INFO_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_IFP_PRESEL_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_VFP_PRESEL_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EFP_PRESEL_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EMFP_PRESEL_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EMFT_PRESEL_OPER:
        *struct_id = BCMI_FIELD_HA_PRESEL_OPER_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_IFP_GROUP_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_VFP_GROUP_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EFP_GROUP_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EMFP_GROUP_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EMFT_GROUP_INFO:
        *struct_id = BCMI_FIELD_HA_GROUP_INFO_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_IFP_GROUP_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_VFP_GROUP_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EFP_GROUP_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EMFP_GROUP_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EMFT_GROUP_OPER:
        *struct_id = BCMI_FIELD_HA_GROUP_OPER_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_IFP_ENTRY_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_VFP_ENTRY_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EFP_ENTRY_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EMFP_ENTRY_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EMFT_ENTRY_INFO:
        *struct_id = BCMI_FIELD_HA_ENTRY_INFO_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_IFP_ENTRY_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_VFP_ENTRY_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EFP_ENTRY_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EMFP_ENTRY_OPER:
    case BCMI_FIELD_HA_BLK_TYPE_EMFT_ENTRY_OPER:
        *struct_id = BCMI_FIELD_HA_ENTRY_OPER_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_IFP_COLOR_TBL_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_IFP_COLOR_TBL_PDD_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_IFP_COLOR_TBL_SBR_INFO:
    ...
    case BCMI_FIELD_HA_BLK_TYPE_EFP_COLOR_TBL_PDD_INFO:
    case BCMI_FIELD_HA_BLK_TYPE_EFP_COLOR_TBL_SBR_INFO:
        *struct_id = BCMI_FIELD_HA_PROFILE_INFO_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_HINT_LIST:
        *struct_id = BCMI_FIELD_HA_HINTS_T_ID;
        break;
    case BCMI_FIELD_HA_BLK_TYPE_HINT_ENTRY:
        *struct_id = BCMI_FIELD_HA_HINT_ENTRY_T_ID;
        break;
    default:
...
bcmint_field_ha_cleanup(int unit, bool warm)
{
    ...
    bcmissu_struct_id_t struct_id = 0;
    ...
    if (warm == FALSE) {
       for (; blk_id < BCM_FIELD_HA_BLK_ID_MAX; blk_id++) {
           ...
           SHR_IF_ERR_VERBOSE_EXIT
               (bcmint_field_ha_blk_type_to_struct_id(unit,
                                                      blk_hdr->blk_type,
                                                      &struct_id));
