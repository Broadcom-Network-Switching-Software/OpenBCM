...
bcm_petra_port_init(int unit)
{
  ...
  if (!SOC_WARM_BOOT(unit) && !SOC_DPP_PP_ENABLE(unit) && !SOC_IS_FLAIR(unit)) 
  {
        ...
        case bcmPortControlTrustCustomHeaderPri:
            if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mixed_qos_model_enable", 0)) {
                ...
                if (dscp_preserve_per_port) {
                    BCMDNX_IF_ERR_EXIT(arad_egr_prog_editor_pp_profile_set(unit, port, (value ? ARAD_PRGE_PP_SELECT_COPY_DSCP_MIXED_QOS_MODEL : ARAD_PRGE_PP_SELECT_PP_COPY_HEADER)));
                } else {
