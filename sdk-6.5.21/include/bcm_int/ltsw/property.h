...
/*!
 * This configuration property is used to enable/disable allocation of flex counters by
 * flowtracker module to attach to flows learnt by HW.
 * Valid values are 0 and 1 to disable/enable respectively.
 * Default value is 0 (disable).
 */
#define BCMI_CPN_FLOWTRACKER_FLEXCTR_ALLOC_ENABLE  "flowtracker_flexctr_alloc_enable"
 ...
 * bcm_l3_route_add. INT action and opaque data are not supported and thus
 * l3a_int_action_profile_id/l3a_int_opaque_data_profile_id are not configurable.
 * Value 1 indicates INT action, opaque data and internal priority derivation are supported.
 * INT action and opaque data are configurable through
 * l3a_int_action_profile_id/l3a_int_opaque_data_profile_id. Class ID is not supported.
 */
...
/*!
 * Create default EP Recirculate profile entry 3 for IFA clone & live mode.
 * During switch module init, this default profile will be created
 * with predetermined settings. Any change made on this profile after init
 * will cause default IFA behavior changes.
 * Note:
 * ifa_enable = 0: don't reserve.
 * ifa_enable = 1: reserve.
 * Default value is 0.
 */
#define BCMI_CPN_IFA_ENABLE  "ifa_enable"

/*!
 * Create default EP Recirculate profile entries 1 and 2 for Mirror on drop.
 * During switch module init, these two default profiles will be created
 * with predetermined settings. Any change made on these profiles after init
 * will cause default MOD behavior changes.
 * Note:
 * flowtracker_drop_monitor_enable = 0: don't reserve.
 * flowtracker_drop_monitor_enable = 1: reserve.
 * Default value is 0.
 */
#define BCMI_CPN_FLOWTRACKER_DROP_MONITOR_ENABLE  "flowtracker_drop_monitor_enable"

/*!
 * Compression key type.
 * Note:
 * compression_key_type = 0 : Compression key is L4_PORT + IP, L4_PORT_MASK could be all 1 or 0.
 * compression_key_type = 1 : Compression key is VRF + IP, VRF_MASK could be all 1 or 0.
 * compression_key_type = 2 : Compression key includes all fields, non-IP mask must be all 1.
 */
#define BCMI_CPN_COMP_KEY_TYPE  "compression_key_type"

/*!
 * This configuration property is used to specify the maximum number of
 * entries that can be inserted into the L3_IPV4_MC_ROUTE table.
 * The default value for this property is the maximum HW table capacity
 * that may not be achievable as the physical memories being shared
 * among multiple tables. The default value may also vary by device.
 */
#define BCMI_CPN_IP4_MC_ROUTE_TABLE_ENTRIES_MAX  "ip4_mc_route_table_entries_max"

/*!
 * This configuration property is used to specify the maximum number of
 * entries that can be inserted into the L3_IPV6_MC_ROUTE table.
 * The default value for this property is the maximum HW table capacity
 * that may not be achievable as the physical memories being shared
 * among multiple tables. The default value may also vary by device.
 */
#define BCMI_CPN_IP6_MC_ROUTE_TABLE_ENTRIES_MAX  "ip6_mc_route_table_entries_max"
 ...
 */
#define BCMI_CPN_RESERVE_MULTICAST_RESOURCES  "reserve_multicast_resources"

/*!
 * This property is used to indicate the default IP tunnel termination by
 * exact match or by mask.
 * Value 0 (default) indicates the default IP tunnel termination by mask.
 * Use BCM_TUNNEL_TERM_EM flag to set the IP tunnel termination by
 * exact match.
 * Value 1 indicates the default IP tunnel termination by exact match.
 * Use BCM_TUNNEL_TERM_MASK flag to set the IP tunnel termination by mask.
 */
#define BCMI_CPN_IP_TUNNEL_DEFAULT_TERM "ip_tunnel_default_term"

/*!
 * Number of l3 mem entries for IPv4.
 */
#define BCMI_CPN_L3_MEM_ENTRIES  "l3_mem_entries"


/*!
 * Enable flex counter functionality on L2 IPMC.
 * Note:
 * l2_ipmc_flexctr_enable = 0: Disable flexctr functionality on L2 IPMC.(Default)
 * l2_ipmc_flexctr_enable = 1: Enable flexctr functionality on L2 IPMC.
 */
#define BCMI_CPN_L2_IPMC_FLEXCTR_ENABLE  "l2_ipmc_flexctr_enable"

/*!
 * Set random seed to configure remapping structures for robust hash L2 table.
 */
#define BCMI_CPN_ROBUST_HASH_SEED_L2  "robust_hash_seed_l2"

/*!
 * Set random seed to configure remapping structures for robust hash MPLS
 * table.
 */
#define BCMI_CPN_ROBUST_HASH_SEED_MPLS  "robust_hash_seed_mpls"

/*!
 * Set random seed to configure remapping structures for robust hash L3 Tunnel
 * table.
 */
#define BCMI_CPN_ROBUST_HASH_SEED_L3_TUNNEL  "robust_hash_seed_l3_tunnel"

/*!
 * Set random seed to configure remapping structures for robust hash Exact
 * Match table.
 */
#define BCMI_CPN_ROBUST_HASH_SEED_EXACT_MATCH  "robust_hash_seed_exact_match"

/*!
 * Enable or disable to share mirror containers across ingress field entries
 * with different mirror destinations.
 * 0 - Disable (default).
 * 1 - Enable.
 */
#define BCMI_CPN_INGRESS_FIELD_SHARED_MIRROR_CONTAINER_ENABLE  "ingress_field_shared_mirror_container_enable"
