...
/*!
 * \brief Get symbol field.
 *
 * \param [in] unit   Unit Number.
 * \param [in] symbol LT symbol String
 * \param [out]val    Scalar value
 */
typedef int (*bcmint_vlan_symbol_get_f)(
    int unit,
    const char *symbol,
    uint64_t *val);

/*!
 * \brief Set symbol field.
 *
 * \param [in] unit   Unit Number.
 * \param [in] val    Scalar value
 * \param [out]symbol LT symbol String
 */
typedef int (*bcmint_vlan_symbol_set_f)(
    int unit,
    uint64_t val,
    const char **symbol);

/*!
 * \brief Get bitmap field.
 *
 * \param [in] unit Unit Number.
 * \param [in] bmp  LT Bitmap value
 * \param [out]val  Field Value
 */
typedef int (*bcmint_vlan_bmp_get_f)(
    int unit,
    uint64_t bmp,
    uint64_t *val);

/*!
 * \brief Set bitmap field.
 *
 * \param [in] unit Unit Number.
 * \param [in] val  Field Value
 * \param [out]bmp  LT Bitmap value
 */
typedef int (*bcmint_vlan_bmp_set_f)(
    int unit,
    uint64_t val,
    uint64_t *bmp);

/*!
 * \brief The format of TAG_ACTION.
 */
#define BCMINT_VLAN_TAG_ACTION_OTAG_ADD              (1ULL << 0)
#define BCMINT_VLAN_TAG_ACTION_ITAG_ADD              (1ULL << 1)
#define BCMINT_VLAN_TAG_ACTION_ODOT1P_PRESERVE       (1ULL << 2)
#define BCMINT_VLAN_TAG_ACTION_OTAG_STATUS_PRESERVE  (1ULL << 3)
...
typedef struct bcmint_vlan_fld_s {
    ...
    /*! Callback to get LT field value with specific format. */
    void *get_transform;

    /*! Callback to set LT field value with specific format. */
    void *set_transform;
