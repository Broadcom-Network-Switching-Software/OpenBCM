...
typedef struct bcmi_ltsw_stat_counter_control_s {
    ...
    /*! Base index for ingress PFC counters. */
    uint32_t ing_pfc_base_index;

    /*! The number of entries per port for ingress PFC counters. */
    uint32_t ing_pfc_entries_perport;
...
/*!
 * \brief Counter types. Its not allowed to modify the name of defined types.
 */
typedef enum bcmi_ltsw_stat_counter_type_s {

    /*! Port counters. */
    BCMI_LTSW_COUNTER_TYPE_PORT = 0,

    /*! Traffic management counters. */
    BCMI_LTSW_COUNTER_TYPE_TM = 1,

    /*! Egress pipeline counters. */
    BCMI_LTSW_COUNTER_TYPE_EGR_PIPE = 2,

    /*! Ingress pipeline counters. */
    BCMI_LTSW_COUNTER_TYPE_ING_PIPE = 3,

    /*! Eviction(flex) counters. */
    BCMI_LTSW_COUNTER_TYPE_EVICT = 4,

    /*! Security module counters. */
    BCMI_LTSW_COUNTER_TYPE_SEC = 5,

    /*! All counters. */
    BCMI_LTSW_COUNTER_TYPE_ALL = 6,

    /*! Count of counter types, not valid. */
    BCMI_LTSW_COUNTER_TYPE_COUNT = 7

} bcmi_ltsw_stat_counter_type_t;

#define BCMI_LTSW_STAT_COUNTER_TYPE_STR \
{ \
    "PORT", \
    "TM", \
    "EGR_PIPE", \
 ...
 */
extern int
bcmi_ltsw_stat_sync_with_type(
    int unit,
    bcmi_ltsw_stat_counter_type_t type);
