...
/*! Maximum hit index bit map number. */
#define BCMI_LTSW_FLEXCTR_HITIDX_BIT_MAP_MAX (32)
...
/*!
 * \brief Hit index objects
 */
typedef enum bcmi_ltsw_flexctr_hitidx_obj_s {

    /*! L2 Src lookup. */
    bcmiFlexCtrHitIdxObjL2Src = 0,

    /*! L2 Dst lookup. */
    bcmiFlexCtrHitIdxObjL2Dst = 1,

    /*! EM FT. */
    bcmiFlexCtrHitIdxObjEmft = 2,

    /*! Last one, not valid. */
    bcmiFlexCtrHitIdxObjCount = 3

} bcmi_ltsw_flexctr_hitidx_obj_t;

#define BCMI_LTSW_FLEXCTR_HITIDX_OBJ_STR \
{ \
    "L2Src", \
    "L2Dst", \
    "Emft" \
...
/*!
 * \brief Hit index operations
 */
typedef enum bcmi_ltsw_flexctr_hitidx_op_s {

    /*! Lookup op. */
    bcmiFlexCtrHitIdxOpGet = 0,

    /*! Insert or update op. */
    bcmiFlexCtrHitIdxOpSet = 1,

    /*! Delete op. */
    bcmiFlexCtrHitIdxOpDelete = 2,

    /*! Last one, not valid. */
    bcmiFlexCtrHitIdxOpCount = 3

} bcmi_ltsw_flexctr_hitidx_op_t;

#define BCMI_LTSW_FLEXCTR_HITIDX_OP_STR \
{ \
    "Get", \
    "Set", \
    "Delete" \
...
/*!
 * \brief Hit index compact control
 */
typedef struct bcmi_ltsw_flexctr_hitidx_compact_ctrl_s {

    /*! Hit index object. */
    bcmi_ltsw_flexctr_hitidx_obj_t hit_obj;

    /*! Operation. */
    bcmi_ltsw_flexctr_hitidx_op_t op;

    /*! Hit index bit map array. */
    uint8_t bit_map[BCMI_LTSW_FLEXCTR_HITIDX_BIT_MAP_MAX];

} bcmi_ltsw_flexctr_hitidx_compact_ctrl_t;
...
/*!
 * \brief Hit index compact ctrl.
 *
 * \param [in] unit Unit Number.
 * \param [in] ctrl Hit index compact control.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
 ...
 */
extern int
bcmi_ltsw_flexctr_flow_count_set(
    int unit,
    int val);
 ...
 */
extern int
bcmi_ltsw_flexctr_flow_count_get(
    int unit,
    int *val);
