...
/*! Truncate length profile info for non-encap mirroring. */
#define BCMINT_MIRROR_SUB_COMP_ID_MIRROR_TRUNCATE_LENGTH_NON_ENCAP_PRF 11
...
/*! Exact match ingress mirroring. */
#define BCMINT_MIRROR_ST_F_EM_FP_ING (1 << 17)

/*! Exact match egress mirroring. */
#define BCMINT_MIRROR_ST_F_EM_FP_EGR (1 << 18)

/*! Flowtracker ingress mirroring. */
#define BCMINT_MIRROR_ST_F_EM_FT_ING (1 << 19)

/*! Flowtracker egress mirroring. */
#define BCMINT_MIRROR_ST_F_EM_FT_EGR (1 << 20)

/*! Exact match ingress mirroring (hardware learn). */
#define BCMINT_MIRROR_ST_F_EM_FP_ING_HW (1 << 21)

/*! Exact match egress mirroring (hardware learn). */
#define BCMINT_MIRROR_ST_F_EM_FP_EGR_HW (1 << 22)

/*! Flowtracker ingress mirroring (hardware learn). */
#define BCMINT_MIRROR_ST_F_EM_FT_ING_HW (1 << 23)

/*! Flowtracker egress mirroring (hardware learn). */
#define BCMINT_MIRROR_ST_F_EM_FT_EGR_HW (1 << 24)
...
typedef struct bcmint_mirror_db_s {
    ...
    /*! One truncate profile applied to all none-encapsulation mirroring. */
    const bool all_non_encap_one_truncate_prf;
...
typedef struct bcmint_mirror_sample_prf_info_s {
    ...
    /*! True for unsupport ingress sample profile. */
    bool ing_sample_prf_unsupported;
...
/*! Enable/disalbe copy sampled packets to CPU. */
#define BCMINT_MIRROR_SAMPLE_PRF_FLD_CPU (1 << 8)

/*! Enable/disalbe Copy sampled packets to all mirror destinations created with BCM_MIRROR_PORT_SFLOW flag. */
#define BCMINT_MIRROR_SAMPLE_PRF_FLD_MIRROR (1 << 9)
...
typedef struct bcmint_mirror_sample_profile_s {
    ...
    /*! Copy sampled packets to CPU. */
    bool copy_to_cpu;

    /*! Copy sampled packets to all mirror destinations created with BCM_MIRROR_PORT_SFLOW flag. */
    bool mirror;
