...
LTSW_FEAT_ENTRY(ROUTE_SIMULATE_HOST), /*! Host entries implemented with route tables. */ \
...
LTSW_FEAT_ENTRY(FLEXCTR_L2_IPMC),   /*! Flexctr based stat on L2 IPMC. */\
...
LTSW_FEAT_ENTRY(EP_RECIRC),         /*! EP Recirculate feature. */ \
LTSW_FEAT_ENTRY(NO_L2),             /*! No L2 feature. */                         \
LTSW_FEAT_ENTRY(ECMP_MEMBER_WEIGHTED),   /*! ECMP Member Weighted feature. */ \
LTSW_FEAT_ENTRY(L2_HITBIT_EXT),  /* Different l2 hitbit calc for issu extension */ \
LTSW_FEAT_ENTRY(ECMP_MEMBER1_NO_DEFAULT_INIT),  /* ECMP_MEMBER1 no default init. */ \
LTSW_FEAT_ENTRY(FAILOVER_UNDERLAY_NH_ONLY),  /* Support only underlay nexthop failover. */ \
LTSW_FEAT_ENTRY(BFD),               /* Support BFD. */ \
LTSW_FEAT_ENTRY(FLEXSTATE_QUANT_32BIT), /*! Flex state quantization 32-bit mode support. */ \
LTSW_FEAT_ENTRY(ETRAP_HW),          /*! Hardware ETRAP feature. */ \
LTSW_FEAT_ENTRY(MTOP),              /* Support MTOP. */ \
LTSW_FEAT_ENTRY(DVP_TAG_CTRL),      /* Tag control per DVP. */ \
LTSW_FEAT_ENTRY(LARGE_VRF),         /* Support LARGE_VRF. */ \
LTSW_FEAT_ENTRY(LPM_L3MC),          /*! Support L3MC in LPM. */ \
LTSW_FEAT_ENTRY(NO_TNL_MC_NH_REMAP),  /*! No tunnel multicast nexthop remap. */ \
