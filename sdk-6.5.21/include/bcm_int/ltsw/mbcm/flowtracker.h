...
/*!
 * \brief Get the max number of exact match groups supported in the device
 *
 * \param [in] unit Unit Number.
 * \param [out] num_em_grps Max number of em groups supported.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_num_em_groups_get_f)(
    int unit,
    uint32_t *num_em_grps);

/*!
 * \brief Get the exact match base entry information for the device
 *
 * \param [in] unit Unit Number.
 * \param [out] base_entry_size Size of one base entry.
 * \param [out] key_space_avl Array of space available for key within a base entry. Caller needs to allocate this array for a size of FT_EM_KEY_MAX_NUM_BASE_ENTRIES.
 * \param [out] key_type_base_valid_size Array of sizes taken by base_valid and key_type fields within a base entry. Caller needs to allocate this array for a size of FT_EM_KEY_MAX_NUM_BASE_ENTRIES.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_base_entry_info_get_f)(
    int unit,
...
/*!
 * \brief Check whether FT_LEARN_FAIL_COUNTER feature is supported
 *
 * \param [in] unit Unit Number.
 * \param [out] is_supported Supported or not.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_ft_learn_fail_count_supported_f)(
    int unit,
    bool *is_supported);

/*!
 * \brief Reserve necessary flex counters for FT user app
 *
 * \param [in] unit Unit Number.
 * \param [in] max_flows Max flows enabled for learning by flowtracker
 * \param [out] flex_ctr_stat_id Output flex ctr stat ID
 ...
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_flex_ctr_pkt_byte_reserve_wo_eapp_f)(
    int unit,
    int max_flows,
    uint32_t *flex_ctr_stat_id,
    uint32_t *flex_ctr_lt_id);
...
/*!
 * \brief Set the compaction control LTs to compact the hit indexes.
 *
 * \param [in] unit Unit Number.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_hit_index_compaction_ctrl_set_f)(int unit);

/*!
 * \brief Delete the compaction control LT entry for flowtracker.
 *
 * \param [in] unit Unit Number.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_hit_index_compaction_ctrl_delete_f)(int unit);

/*!
 * \brief Check if flow limit configuration is supported.
 *
 * \param [in] unit Unit Number.
 * \param [in] id Flow group ID
 *
 ...
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_flow_limit_config_supported_f)(
    int unit,
    bcm_flowtracker_group_t id);

/*!
 * \brief Check if flowtracker functionality implemented with exact match tables is supported.
 *
 * \param [in] unit Unit Number.
 * \param [out] is_supported Supported or not.
 *
 ...
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_with_exact_match_supported_f)(
    int unit,
    bool *is_supported);

/*!
 * \brief Validate EM group id per pipe.
 *
 * \param [in] unit Unit Number.
 * \param [in] num_pipe Number of pipes
 * \param [in] flow_group_info Flow group info
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
typedef int (*flowtracker_per_pipe_em_mode_validate_f)(
    int unit,
    int num_pipe,
...
typedef struct mbcm_ltsw_flowtracker_drv_s {
    ...
    /*! Get the max number of exact match groups supported in the device */
    flowtracker_num_em_groups_get_f flowtracker_num_em_groups_get;

    /*! Get the exact match base entry information for the device */
    flowtracker_base_entry_info_get_f flowtracker_base_entry_info_get;

    /*! Check whether FT_LEARN_FAIL_COUNTER feature is supported */
    flowtracker_ft_learn_fail_count_supported_f flowtracker_ft_learn_fail_count_supported;

    /*! Reserve necessary flex counters for FT user app */
    flowtracker_flex_ctr_pkt_byte_reserve_wo_eapp_f flowtracker_flex_ctr_pkt_byte_reserve_wo_eapp;

    /*! Set the compaction control LTs to compact the hit indexes. */
    flowtracker_hit_index_compaction_ctrl_set_f flowtracker_hit_index_compaction_ctrl_set;

    /*! Delete the compaction control LT entry for flowtracker. */
    flowtracker_hit_index_compaction_ctrl_delete_f flowtracker_hit_index_compaction_ctrl_delete;

    /*! Check if flow limit configuration is supported. */
    flowtracker_flow_limit_config_supported_f flowtracker_flow_limit_config_supported;

    /*! Check if flowtracker functionality implemented with exact match tables is supported. */
    flowtracker_with_exact_match_supported_f flowtracker_with_exact_match_supported;

    /*! Validate EM group id per pipe. */
    flowtracker_per_pipe_em_mode_validate_f flowtracker_per_pipe_em_mode_validate;
