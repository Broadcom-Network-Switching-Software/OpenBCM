...
/**
 * \brief
 *  Inits the priorities range ranked-linked-list
 *
 ...
 * This function is used to update the algo algorithm about an entry with the given location/priority
 * that is being freed so that this module changes its databases accordingly to
 * stay updated.
 *
 * \param [in] unit       - Device ID
 * \param [in] core       - Core ID
 * \param [in] handler_id - The handler_id that is associated with the given entry
 * \param [in] priority   - Priority of the entry to be removed
...
/**
 * \brief
 *  This function updates Location-Algo about a single entry move.
 *  The function receives as input the handler_id of the entry along with
 *  the location from where the entry is to be moved and the location
 *  to which the entry is to be moved. It also receives the core
 *  on which this move operation is taking place.
 *
 * \param [in] unit          - Device ID
 * \param [in] core          - Core ID
 * \param [in] handler_id    - ID to the handler the entry belongs to
 * \param [in] location_from - The location from where the entry is moving
 * \param [in] location_to   - The location to which the entry is moving
...
/**
 * \brief
 *  Gets the previous given handler's entry location that is closest to the given location.
