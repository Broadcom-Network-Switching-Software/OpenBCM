...
int main(int argc, char *argv[])
{
    ...
    while (++ac < argc) {
        if (strcmp(argv[ac], "-testbios") == 0 ||
            ...
            if (sysconf_ltsw_ha_start_version_set(argv[ac]) < 0) {
            ...
            if (sysconf_ltsw_ha_current_version_set(argv[ac]) < 0) {
       ...
       } else if (strcmp(argv[ac], "-g") == 0) {
            if (++ac >= argc) {
                printf("No ID number for high-availability files specified\n");
                exit(1);
            }
            if (sysconf_ltsw_ha_no_del_set(sal_atoi(argv[ac])) < 0) {
