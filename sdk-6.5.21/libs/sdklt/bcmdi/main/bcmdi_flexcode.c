...
flc_auth_start(int unit, const bcmdi_flc_file_auth_t *auth) {
    ...
    SHR_IF_ERR_EXIT
        (bcmdi_iproc_write_words(unit, auth->valid.addr,
                                 auth->valid.data, auth->valid.data_count));
    LOG_VERBOSE(BSL_LOG_MODULE,
             (BSL_META_U(unit,
...
flc_auth_load(int unit, const bcmdi_flc_file_auth_t *auth) {
    ...
    SHR_IF_ERR_EXIT
        (bcmdi_iproc_write_words(unit, auth->aeal.addr,
                                 auth->aeal.data, auth->aeal.data_count));
    LOG_VERBOSE(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "Loading AEAL done (%d words)\n"),
                         auth->aeal.data_count));

    SHR_IF_ERR_EXIT
        (bcmdi_iproc_write_words(unit, auth->signature.addr,
                                 auth->signature.data, auth->signature.data_count));
    LOG_VERBOSE(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "Loading Flexcode signature done (%d words)\n"),
                         auth->signature.data_count));
