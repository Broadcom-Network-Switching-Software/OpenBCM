...
int
bcmdi_iproc_write_words(int unit, uint32_t addr, const uint32_t *vals, const uint32_t len)
{
    uint32_t i;
    uint32_t dst;
    uint32_t val;
    ...
    for (i = 0; i < len; i++) {
        SHR_IF_ERR_EXIT
            (bcmdrd_hal_iproc_write(unit, dst, vals[i]));
        dst += sizeof (uint32_t);
    }

    /* Verify the iProc writes */
    dst = addr;
    for (i = 0; i < len; i++) {
        SHR_IF_ERR_EXIT
            (bcmdrd_hal_iproc_read(unit, dst, &val));
        if (val != vals[i]) {
