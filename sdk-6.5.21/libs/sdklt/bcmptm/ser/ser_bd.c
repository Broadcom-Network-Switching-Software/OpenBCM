...
bcmptm_ser_reg_multi_fields_update(int unit, bcmdrd_sid_t sid,
                                   ...
                                   uint32_t *old_fld_val_list, int flags)
{
    ...
    rv = bcmptm_ser_pt_read(unit, sid, index, inst, -1, entry_data,
                            BCMDRD_MAX_PT_WSIZE,
                            (flags & BCMPTM_SCOR_REQ_FLAGS_DONT_USE_CACHE));
                             ...
                             int work_num, uint64_t buf_paddr, uint32_t work_flags)
{
    ...
    if (is_port_reg) {
        if ((sinfo.flags & BCMDRD_SYMBOL_FLAG_SOFT_PORT) &&
            (sinfo.flags & BCMDRD_SYMBOL_FLAG_MEMORY)) {
    ...
    for (reg_idx = 0; reg_idx < reg_num; reg_idx++) {
        ...
        for (i = 0; i < inst_num; i++) {
            ...
            if (regs_ctrl_info[reg_idx].ctrl_flds_val[0].rd <= 1) {
                rv = bcmptm_ser_pt_read(unit, sid, index, inst, -1,
                                        entry_data, BCMDRD_MAX_PT_WSIZE, 0);
                SHR_IF_ERR_VERBOSE_EXIT(rv);
            } else if (regs_ctrl_info[reg_idx].ctrl_flds_val[0].rd == 2) {
            ...
            for (fld_idx = 0; fld_idx < regs_ctrl_info[reg_idx].num_fld; fld_idx++) {
                if (regs_ctrl_info[reg_idx].ctrl_flds_val[fld_idx].rd != 0 &&
                    regs_ctrl_info[reg_idx].ctrl_flds_val[fld_idx].rd != 3) {
                    continue;
                }
                write_time++;
