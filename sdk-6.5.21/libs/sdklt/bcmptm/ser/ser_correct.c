...
/*! Handler to map SID reported by H/W to SID used to correct SER error for TCAM . */
static bcmptm_ser_tcam_cb_info_get_f ser_tcam_info_get_f[BCMDRD_CONFIG_MAX_UNITS];
...
{
    int rv = SHR_E_NONE, out_count = 0, i, tcam_out_count = 0;
    bcmbd_pt_dyn_info_t hw_dyn_info, out_dyn_info[ALPM_REMAP_COUNT];
    bcmdrd_sid_t sid[ALPM_REMAP_COUNT];
    bcmbd_pt_dyn_info_t tcam_out_dyn_info[TCAM_REMAP_COUNT];
    bcmdrd_sid_t tcam_sid[TCAM_REMAP_COUNT];
    ...
    if (out_count == 0 && ser_tcam_info_get_f[unit] != NULL) {
        rv = ser_tcam_info_get_f[unit](unit, spci->sid[0], hw_dyn_info,
                                       &tcam_sid[0], &tcam_out_dyn_info[0],
                                       &tcam_out_count);
        if (SHR_SUCCESS(rv)) {
            for (i = 0; i < tcam_out_count && i < TCAM_REMAP_COUNT; i++) {
                spci->sid[i] = tcam_sid[i];
                spci->index[i] = tcam_out_dyn_info[i].index;
                spci->tbl_inst[i] = tcam_out_dyn_info[i].tbl_inst;
            }
        } else {
            tcam_out_count = 0;
            rv = SHR_E_NONE;
    ...
    if (out_count == 0 && tcam_out_count == 0) {
        rv = bcmptm_ser_ipep_mem_remap(unit, spci);
        SHR_IF_ERR_VERBOSE_EXIT(rv);
    } else {
        SHR_IF_ERR_VERBOSE_EXIT(rv);
    }
...
void
bcmptm_ser_tcam_info_cb_reg(int unit, bcmptm_ser_tcam_cb_info_get_f tcam_info_get)
{
    ser_tcam_info_get_f[unit] = tcam_info_get;
}
