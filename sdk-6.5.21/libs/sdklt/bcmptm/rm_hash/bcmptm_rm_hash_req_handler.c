...
#define RM_HASH_REORDERING_BNAK_NUM_THRESHOLD                       8
...
{
    uint8_t e_size = 0, mmap, inst, grp_idx, map_idx;
    ...
    for (map_idx = 0; map_idx < lt_ctrl->num_maps; map_idx++) {
        if (glb_srch_info->map_srch_info[map_idx].num_insts > 0) {
            break;
        }
    }
    ...
    map_ei->e_attr.e_bm = glb_srch_info->map_srch_info[map_idx].inst_srch_info[0].e_bm;
    map_ei->e_attr.e_nm = glb_srch_info->map_srch_info[map_idx].inst_srch_info[0].e_nm;
                                ...
                                uint32_t *rsp_flags)
{
    ...
    sal_memset(&map_ei, 0, sizeof(map_ei));
    SHR_IF_ERR_EXIT
...
rm_hash_insert_with_multi_map_move(int unit,
                                   ...
                                   uint32_t *rsp_flags)
{
    uint8_t map_idx, idx, grp_idx, fmt_idx, e_idx, m_idx;
    ...
    uint32_t rsp_entry_index[BCMPTM_RM_HASH_RSP_MAX_INST_CNT];
    bcmdrd_sid_t rsp_entry_sid[BCMPTM_RM_HASH_RSP_MAX_INST_CNT];
    uint8_t move_depth = lt_ctrl->move_depth;

    SHR_FUNC_ENTER(unit);

    if ((lt_ctrl->num_maps >= RM_HASH_REORDERING_BNAK_NUM_THRESHOLD) &&
        (lt_ctrl->move_depth > 2)) {
        /*
         * For LTs with a large number of mapped banks (>=8), a good table
         * utilization will be achieved even with depth 2. It is unnecessary
         * to reorder the entries with the depth > 2. That will not increase
         * the table utilization apparently, but will decrease the LT insert
    ...
    sal_memset(&map_ei, 0, sizeof(map_ei));
    map_ei.e_attr.e_size = e_size;
    for (m_idx = 0; m_idx < lt_ctrl->num_maps; m_idx++) {
        if (pglb_si->map_srch_info[m_idx].num_insts > 0) {
            break;
        }
    }
    if (m_idx == lt_ctrl->num_maps) {
        SHR_ERR_EXIT(SHR_E_RESOURCE);
    }
    map_ei.e_attr.e_bm = pglb_si->map_srch_info[m_idx].inst_srch_info[0].e_bm;
    map_ei.e_attr.e_nm = pglb_si->map_srch_info[m_idx].inst_srch_info[0].e_nm;
    for (map_idx = 0; map_idx < lt_ctrl->num_maps; map_idx++) {
        map_si = &pglb_si->map_srch_info[map_idx];
        if (map_si->num_insts == 0) {
            continue;
        }
               ...
               uint32_t *rsp_flags)
{
    ...
    if (pmap_sr->e_exist == TRUE) {
        rsp_info->hash_store_val = pmap_sr->hash_store_val;
    ...
    for (map_idx = 0; map_idx < lt_ctrl->num_maps; map_idx++) {
        if (pglb_si->map_srch_info[map_idx].num_insts > 0) {
            break;
        }
    }
    if (map_idx == lt_ctrl->num_maps) {
    ...
    e_nm = pglb_si->map_srch_info[map_idx].inst_srch_info[0].e_nm;

    if ((rv == SHR_E_RESOURCE) &&
        ((e_nm == BCMPTM_RM_HASH_ENTRY_NARROW_MODE_HALF) ||
                      ...
                      rm_hash_nme_info_t *nme_info)
{
    ...
    rsp_info->hash_store_val = map_sr->hash_store_val;
    rm_hash_rsp_info_set(unit,
                         map_sr->num_insts,
                         rsp_entry_index,
                         rsp_entry_sid,
