    ...
    for (bank_idx = 0; bank_idx < pt_info->num_rbanks; bank_idx++) {
        ...
        if (rv == SHR_E_NONE) {
            ...
            for (idx = 0; idx < en_bank_cnt; idx++) {
                ...
                pt_info->vec_info[rbank].offset = banks_info.bank[idx].hash_offset;
            }
            ...
            for (idx = 0; idx < en_bank_cnt; idx++) {
                rbank = rbank_list[idx];
                pt_info->vec_info[rbank].type = vec_type;
                pt_info->vec_info[rbank].offset = def_bank_info.hash_offset[idx];
            }
        } else {
            SHR_ERR_EXIT(rv);
        }
...
int
bcmptm_rm_hash_narrow_mode_state_update(int unit, bcmdrd_sid_t sid, bool narrow_mode)
{
    rm_hash_pt_info_t *pt_info = NULL;
    bcmdrd_sid_t grp_sid;
    uint8_t rbank_cnt, rbank_list[RM_HASH_SHR_MAX_BANK_COUNT] = {0};
    uint8_t rbank, bank_idx;
    ...
    for (bank_idx = 0; bank_idx < rbank_cnt; bank_idx++) {
        rbank = rbank_list[bank_idx];
        pt_info->bank_in_narrow_mode[rbank] = narrow_mode;
    }
...
int
bcmptm_rm_hash_narrow_mode_state_init(int unit, rm_hash_pt_info_t *pt_info)
{
    uint8_t rbank;
