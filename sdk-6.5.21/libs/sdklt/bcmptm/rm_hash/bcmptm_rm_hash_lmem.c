...
#define RM_HASH_LTPR_INFO_CHUNK_SIZE   64
...
static shr_lmm_hdl_t rm_hash_ltpr_info_lmem_hdl;
...
rm_hash_ltpr_info_t *
bcmptm_rm_hash_ltpr_info_alloc(void)
{
    return shr_lmm_alloc(rm_hash_ltpr_info_lmem_hdl);
}

void
bcmptm_rm_hash_ltpr_info_free(rm_hash_ltpr_info_t *ltpr_info)
{
    shr_lmm_free(rm_hash_ltpr_info_lmem_hdl, ltpr_info);
}
...
bcmptm_rm_hash_lmem_cleanup(int unit)
{
    ...
    if (rm_hash_ltpr_info_lmem_hdl) {
        shr_lmm_delete(rm_hash_ltpr_info_lmem_hdl);
        rm_hash_ltpr_info_lmem_hdl = NULL;
    }
}
...
bcmptm_rm_hash_lmem_init(int unit)
{
    ...
    if (!rm_hash_ltpr_info_lmem_hdl) {
        LMEM_MGR_INIT(rm_hash_ltpr_info_t,
                      rm_hash_ltpr_info_lmem_hdl,
                      RM_HASH_LTPR_INFO_CHUNK_SIZE,
                      false,
                      rv);
        if (rv != SHR_E_NONE) {
            SHR_ERR_EXIT(SHR_E_MEMORY);
