...
/*!
 * \brief Compare two tile id.
 *
 * \param [in] x Pointer to bcmptm_lt_lookup_ltpr_info_t structure.
 * \param [in] y Pointer to bcmptm_lt_lookup_ltpr_info_t structure.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
rm_hash_ltpr_tile_cmp(const void *x, const void *y)
{
    bcmptm_lt_lookup_ltpr_info_t *p1 = (bcmptm_lt_lookup_ltpr_info_t *)x;
    bcmptm_lt_lookup_ltpr_info_t *p2 = (bcmptm_lt_lookup_ltpr_info_t *)y;

    return (int)p1->tile_id - (int)p2->tile_id;
}
...
bcmptm_rm_hash_lt_rbust_hash_config(int unit, bool warm)
{
    ...
    uint16_t ltid_idx;
    bcmptm_pt_banks_info_t banks_info;
    uint8_t bank_idx, rbank, idx, en_bank_cnt;
    bcmdrd_sid_t sid;
    uint8_t rbank_cnt, rbank_list[RM_HASH_SHR_MAX_BANK_COUNT] = {0};
    rm_hash_pt_info_t *pt_info;
    bool rb_en = FALSE, separate = FALSE;
    uint32_t seed = 0;
    bool bank_init[RM_HASH_MAX_SRCH_BANK_COUNT] = {FALSE};
    int rv = SHR_E_NONE;
    ...
    if (separate == FALSE) {
        for (bank_idx = 0; bank_idx < pt_info->num_rbanks; bank_idx++) {
            if (bank_init[bank_idx] == TRUE) {
                /* This bank has been analyzed, skip it. */
                continue;
            }
            ...
            if (rv == SHR_E_NONE) {
                ...
                if (en_bank_cnt == 0) {
                    /* No banks configured for this group. */
                    continue;
                }
                ...
                if (pt_info->rhash_ctrl[rbank] != NULL) {
                    /*
                     * All the banks of the SID share the common robust hash
                     * configuration. Thus they share the same control structure.
                     */
                    rb_en = banks_info.bank[0].robust_en;
                    seed  = banks_info.bank[0].seed;
...
int
bcmptm_rm_hash_narrow_mode_update(int unit)
{
    uint16_t ltid_idx, bank_idx, idx;
    bcmdrd_sid_t sid;
    bool narrow_mode = FALSE, separate = FALSE;
    bcmptm_pt_banks_info_t banks_info;
    rm_hash_lt_ctrl_t *lt_ctrl = NULL;
    rm_hash_pt_info_t *pt_info = NULL;
    ...
    if (pt_info->narrow_mode_supported == FALSE) {
        /* Narrow mode is not supported on this device. */
        SHR_EXIT();
    }
    for (ltid_idx = 0; ltid_idx < dev_lt_ctrl[unit]->curr; ltid_idx++) {
        lt_ctrl = dev_lt_ctrl[unit]->array + ltid_idx;
        SHR_IF_ERR_EXIT
            (bcmptm_cth_uft_bank_info_get_from_lt(unit, lt_ctrl->ltid, 0, &banks_info));
        for (bank_idx = 0; bank_idx < banks_info.bank_cnt; bank_idx++) {
            for (idx = 0; idx < RM_HASH_PAIRED_BANK_CNT; idx++) {
                if (idx == 0) {
                    sid = banks_info.bank[bank_idx].bank_sid;
                    narrow_mode = banks_info.bank[bank_idx].narrow_mode;
                } else {
                    sid = banks_info.paired_bank[bank_idx].bank_sid;
                    narrow_mode = banks_info.paired_bank[bank_idx].narrow_mode;
                }
                if (sid != INVALIDm) {
                    SHR_IF_ERR_EXIT
                        (bcmptm_rm_hash_narrow_mode_state_update(unit, sid,
                                                                 narrow_mode));
                }
            }
        }
...
int
bcmptm_rm_hash_ltpr_init(int unit, bool warm)
{
    bool separate = FALSE, clear_hit = FALSE;
    uint16_t ltid_idx;
    rm_hash_lt_ctrl_t *lt_ctrl = NULL;
    uint8_t lkup_idx;
    rm_hash_ltpr_info_t *ltpr_info = NULL;
    uint32_t ltpr_cnt = 0;
    uint16_t tile_idx;
    ...
    ltpr_info = bcmptm_rm_hash_ltpr_info_alloc();
    if (ltpr_info == NULL) {
        SHR_ERR_EXIT(SHR_E_MEMORY);
    }
    ...
    for (ltid_idx = 0; ltid_idx < dev_lt_ctrl[unit]->curr; ltid_idx++) {
        lt_ctrl = dev_lt_ctrl[unit]->array + ltid_idx;
        for (lkup_idx = 0; lkup_idx < RM_HASH_LOOKUP_MAX_CNT; lkup_idx++) {
            SHR_IF_ERR_EXIT
                (bcmptm_cth_uft_ltpr_get(unit, lt_ctrl->ltid, lkup_idx,
                                         ltpr_info->array,
                                         RM_HASH_MAX_RSRC_MAP_COUNT,
                                         &ltpr_cnt));
            if (ltpr_cnt == 0) {
                /* There is no physical lookup assigned for this LT lookup. */
                continue;
            }
            sal_qsort(ltpr_info->array,
                      ltpr_cnt,
                      sizeof(ltpr_info->array[0]), rm_hash_ltpr_tile_cmp);
            for (tile_idx = 0; tile_idx < ltpr_cnt; tile_idx++) {
                if (tile_idx == 0) {
                    clear_hit = TRUE;
                } else {
                    clear_hit = FALSE;
                ...
                SHR_IF_ERR_EXIT
                    (bcmptm_rm_hash_ltpr_info_config(unit, warm, TRUE,
                                                     &ltpr_info->array[tile_idx],
                                                     clear_hit,
                                                     NULL,
                                                     0,
                                                     0));
...
exit:
    if (ltpr_info != NULL) {
        bcmptm_rm_hash_ltpr_info_free(ltpr_info);
    }
    SHR_FUNC_EXIT();
...
int
bcmptm_rm_hash_ltpr_update(int unit,
                           bcmltd_sid_t ltid,
                           uint32_t trans_id,
                           bcmptm_rm_hash_table_write write_cb)
{
    ...
    uint32_t ltpr_cnt = 0;
    uint16_t tile_idx;
    bcmptm_lt_lookup_ltpr_info_t *ltpr_arr = NULL;
    ...
    for (ltid_idx = 0; ltid_idx < dev_lt_ctrl[unit]->curr; ltid_idx++) {
        ...
        for (lkup_idx = 0; lkup_idx < RM_HASH_LOOKUP_MAX_CNT; lkup_idx++) {
            ...
            if (ltpr_cnt > 1) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                                        ...
                                        lt_ctrl->ltid));
                for (tile_idx = 0; tile_idx < ltpr_cnt; tile_idx++) {
                    ltpr_arr = &ltpr_info->array[tile_idx];
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit,
                                            "tile id:[%d] \n"
                                            "ltpr table:[%s] \n"
                                            ...
                                            "ltpr lkup id:[%d] \n"),
                                            ltpr_arr->tile_id,
                                            bcmdrd_pt_sid_to_name(unit, ltpr_arr->ltpr_ptid),
                                            ltpr_arr->ltpr_index,
                                            ltpr_arr->lkp_id));
                }
            }
            for (tile_idx = 0; tile_idx < ltpr_cnt; tile_idx++) {
                ...
                SHR_IF_ERR_EXIT
                    (bcmptm_rm_hash_ltpr_info_config(unit, FALSE, FALSE,
                                                     &ltpr_info->array[tile_idx],
                                                     clear_hit,
                                                     write_cb,
                                                     ltid,
