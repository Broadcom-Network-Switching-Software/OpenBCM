...
typedef struct rm_hash_map_srch_result_s {
    ...
    /*! \brief Hash store is enabled/disabled. */
    bool hash_store_en;

    /*! \brief Hash store value. */
    uint32_t hash_store_val;
...
/*
 * ! \brief Structure to hold ltpr information.
 *
 * This structure describes the information about ltpr info for LT lookup.
 */
typedef struct rm_hash_ltpr_info_s {
    /*! \brief Buffer containing the ltpr info. */
    bcmptm_lt_lookup_ltpr_info_t array[RM_HASH_MAX_RSRC_MAP_COUNT];

    /*! \brief Pointer to next element. */
    struct rm_hash_ltpr_info_s *next;
} rm_hash_ltpr_info_t;
