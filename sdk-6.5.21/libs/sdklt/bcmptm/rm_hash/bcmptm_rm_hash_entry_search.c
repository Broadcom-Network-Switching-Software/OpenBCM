                     ...
                     rm_hash_inst_srch_info_t *inst_si)
{
    ...
    if (e_attrs == NULL) {
        ...
        if (pt_info->narrow_mode_supported == TRUE) {
            SHR_IF_ERR_EXIT
                (bcmptm_rm_hash_entry_narrow_mode_get(unit,
                                                      e_attrs->narrow_format,
                                                      &inst_si->e_nm));
        }
    }
                          ...
                          rm_hash_map_srch_result_t *map_sr)
{
    ...
    uint32_t *ent = NULL;
    uint8_t map_idx, inst, num_insts, grp_idx, fmt_idx, bank_idx;
    ...
    for (map_idx = 0; map_idx < lt_ctrl->num_maps; map_idx++) {
        if (e_attrs != NULL) {
            ...
            /* Entry_attrs provides the key format to be operated. */
            SHR_IF_ERR_EXIT
            ...
            if ((d_attrs != NULL) && (d_attrs->dbank_list != NULL)) {
                /* Users specify the banks to be operated. */
                for (bank_idx = 0; bank_idx < d_attrs->dbank_list->num_banks; bank_idx++) {
                    SHR_IF_ERR_EXIT
                        (bcmptm_rm_hash_grp_sid_get(unit,
                                                    d_attrs->dbank_list->bank_sid[bank_idx],
                                                    &grp_sid));
                    if (fmt_info->sid == grp_sid) {
                        break;
                    }
                }
                if (bank_idx == d_attrs->dbank_list->num_banks) {
                    /* This bank is not the intended bank. Skip this mapping. */
                    num_insts = 0;
                    glb_si->map_srch_info[map_idx].num_insts = 0;
                }
            }
            if (num_insts > 0) {
        ...
        if (all_insts_exist == TRUE) {
            ...
            map_sr->hash_store_en = inst_si->hash_store_en;
            if (map_sr->hash_store_en) {
                map_sr->hash_store_val = inst_si->hash_store_val;
            }
            SHR_EXIT();
        }
    }
