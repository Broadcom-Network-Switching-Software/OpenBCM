...
/*! Maximum dynamic bank number. */
#define BCMPTM_RM_HASH_MAX_DYNAMIC_BANK_COUNT    16
...
/*! The width of the narrow mode entry. */
typedef enum bcmptm_rm_hash_entry_narrow_format_e {
    /*! Not a narrow entry. */
    BCMPTM_RM_HASH_NARROW_FORMAT_NONE = 0,

    /*! Half width narrow entry. */
    BCMPTM_RM_HASH_NARROW_FORMAT_HALF,

    /*! Third width narrow entry. */
    BCMPTM_RM_HASH_NARROW_FORMAT_THIRD,

    /*! Third width narrow entry. */
    BCMPTM_RM_HASH_NARROW_FORMAT_COUNT,
} bcmptm_rm_hash_entry_narrow_format_t;
...
/*! The bank list that can be specified dynamically for an entry operation . */
typedef struct bcmptm_rm_hash_dynamic_bank_list_s {
    /*! Number of intended banks. */
    uint8_t num_banks;

    /*! SID of the intended banks. */
    bcmdrd_sid_t bank_sid[BCMPTM_RM_HASH_MAX_DYNAMIC_BANK_COUNT];
} bcmptm_rm_hash_dynamic_bank_list_t;
...
typedef struct bcmptm_rm_hash_dynamic_entry_attrs_s {
    ...
    /*! Narrow mode format for the entry. */
    bcmptm_rm_hash_entry_narrow_format_t narrow_format;

    /*! Hash store attribute. */
    bcmptm_rm_hash_store_attr_t hstore_attr;

    /*! Dynamic entry bank list. */
    bcmptm_rm_hash_dynamic_bank_list_t *dbank_list;
...
/*! \brief Hash store value for entry. */
...
typedef int (*bcmptm_rm_hash_table_write)(int unit,
                                          bcmltd_sid_t ltid,
...
/*!
 * \brief Update hash table narrow mode.
 *
 * \param [in] unit Logical device id.
 *
 * \retval SHR_E_NONE Success
...
/*!
 * \brief Indicates if the rm hash boot was warm or cold.
 *
 * \param [in] unit Logical device id.
 *
 ...
 */
extern bool
bcmptm_rm_hash_in_warmboot(int unit);

/*!
 * \brief Update ltpr table for a hash LT.
 *
 * \param [in] unit Logical device id.
 * \param [in] ltid Logical table id.
 * \param [in] trans_id Transaction id.
