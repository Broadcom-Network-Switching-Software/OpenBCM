...
/*! ALPM compression version 0. */
#define ALPM_COMP_VER_0         0
/*! ALPM compression version 1. */
#define ALPM_COMP_VER_1         1
...
typedef struct lpm_generic_key_field_s {
    ...
    /*! Set to true if key format has duplicate IP address mapping. */
    bool dup_ip_key_format;

    /*! Start bit of the duplicate part. */
    uint16_t dup_start_bit_physical;

    /*! End bit of the duplicate part. */
    uint16_t dup_end_bit_physical;

    /*! Logical start bit of the duplicate part. */
    uint16_t dup_start_bit_logical;

    /*! Logical end bit of the duplicate part. */
    uint16_t dup_end_bit_logical;
...
typedef struct alpm_dev_info_s {
    ...
    uint8_t comp_ver;               /*!< ALPM compression version */

    uint16_t max_key_bits;          /*!< Max key bits.  */
    uint16_t lpm_key_dup_bits[IP_VER_COUNT];   /*!< LPM key duplicate bits.  */
} alpm_dev_info_t;
...
/*!
 * \brief Check compression version.
 *
 * \param [in] u Device u.
 * \param [in] m Mtop
 *
