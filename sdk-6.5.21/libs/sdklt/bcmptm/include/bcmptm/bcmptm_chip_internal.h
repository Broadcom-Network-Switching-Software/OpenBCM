 ...
 * bcmptm_wo_aggr_info_t
 *
 * \retval SHR_E_NONE Table indicated by sid is wo_aggr type mem - read from HW
 * is supported by PTM (but not by HW).
 * \retval SHR_E_UNAVAIL Table indicated by sid is wo_aggr type mem - but read
 * from HW is not supported by PTM (but not by HW).
 * \retval SHR_E_NOT_FOUND Table indicated by sid is NOT wo_aggr type mem.
 */
