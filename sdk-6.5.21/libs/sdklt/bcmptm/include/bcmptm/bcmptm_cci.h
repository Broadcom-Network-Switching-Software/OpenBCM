...
#define BCMPTM_CCI_PIPE_ALL  (0xffffffff)
...
typedef struct bcmptm_cci_ctr_info_s {
    ...
    bool direct_access;           /* support to fetch raw HW counter directly */
...
bcmptm_pt_cci_ctr_evict_field_width_get(int unit,
                                      ...
                                      size_t *width);
/*!
 * \brief Get xlmib rx and tx registers and cdmib row offset.
 *
 * \param [in] unit, Unit number.
 * \param [in] xlmib_rxsid, rxpkt register sid
 * \param [in] xlmib_txpkt, txpkt register sid.
 * \param [in] txpkt_fieldnum, row num for txpkt in cdmib.
 * \param [in] rxpkt_fieldnum, row number for rxpkt in cdmib.
...
/*!
 * \brief Update link status using counter.
 *
 * \param [in] unit, Unit number.
 * \param [in] index,index in mib table
 * \param [in] instnace, instnace of mib table.
 * \param [in] rx, rx/tx.
...
extern void
bcmptm_pt_cci_link_status_update(int unit, uint32_t index, uint32_t instnace,
