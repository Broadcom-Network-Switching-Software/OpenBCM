...
/*! LT needs ser correction info */
#define BCMPTM_RM_TCAM_NEEDS_SER_CORR_INFO      (1 << 17)

/*! LT has Intraslice double */
#define BCMPTM_RM_TCAM_INTRA_SLICE_DOUBLE       (1 << 18)

/*! USE FULL TCAM for this LT without reserve entry */
#define BCMPTM_RM_TCAM_USE_FULL_CAPACITY         (1 << 19)
 ...
 * Otherwise ISSU will not work
 */
/*! FP STAGE with counters per slice */
#define BCMPTM_STAGE_COUNTERS_PER_SLICE         (1 << 16)
...
/*! \brief SID to save PDD profiles. */
    const bcmdrd_sid_t *pdd_profile_sids;

/*! \brief number of PDD SIDs. */
    uint8_t pdd_profile_sids_count;

/*! \brief Number of SBR profiles supported per slice. */
...
/*! /brief Indicate LT is using multi pt aggr view.
  */
    uint8_t is_multi_mem_aggr;
...
/*!
 * \brief Chip specific function to to compare different view sid's
 *
 * \param [in] unit BCM device number.
 * \param [in] sid Physical Table Enum
 * \param [in] shr_sid Shared Physical Table Enum
 * \param [out] neg_shr_flags Shared flags to be nullified
 * \param [out] compare
 *
 * \retval SHR_E_NONE Success
 * \retval SHR_E_UNAVAIL Stage is not supported.
 */
extern int
bcmptm_rm_tcam_prio_only_shared_sid_compare(int unit,
                                     bcmdrd_sid_t sid,
...
bcmptm_rm_tcam_supported_slices_per_mode_get(int unit,
                                  ...
                                  uint8_t *num_slices);
/*!
 * \brief Update correct flex ctr action ID in policy data
 *  for auto expandable group's entry depending on which slice ID
 *  is used for the current entry.
 *
 * \param [in] unit Logical device id.
 * \param [in] ltid_info LT id info structure
 * \param [in] ctr_action_prof_id flex ctr profile id
 * \param [out] policy_words policy data words
 *
 * Returns:
 * \retval SHR_E_NONE when no errors.
 */
extern int
bcmptm_rm_tcam_update_flex_ctr_action(int unit,
                                     bcmptm_rm_tcam_lt_info_t *ltid_info,
                                     uint8_t ctr_action_prof_id,
                                     uint32_t **policy_words);
/*!
 * \brief Retrieve chip specific tcam_only sid for a given
 *  combined/aggr view sid
 *
 * \param [in] unit Logical device id.
 * \param [in] sid Ltid's hw_entry_info.sid
 * \param [out] tcam_only_sid tcam_only view sid
 * \param [out] data_only_sid data_only view sid
 * \param [out] tcam_data_sid aggr view sid
 * \param [out] paired_sid paired view sid
 * \param [out] sid_count Number of sid's returned
 *
 * Returns:
 * \retval SHR_E_NONE when no errors.
 */
extern int
bcmptm_rm_tcam_ser_corr_sid_info_get(int unit,
                                 bcmdrd_sid_t sid,
                                 ...
                                 uint8_t *sid_count);
/*!
 * \brief Ser callback call for RM tcam.
 *
 * \param [in] unit Logical device id
 * \param [in] hw_sid Narrow view hw sid
 * \param [in] hw_dyn_info Narrow view hw_dyn_info
 * \param [out] out_sid hw_sid used as per sw state
 * \param [out] out_dyn_info dyn_info computed as per sw state
 * \param [out] out_count count of valid out sid's and out_dyn_info
 *
 * \return SHR_E_XXX.
 */
int
