...
/*! Allow CTH to issue multiple inserts, deletes for non_atomic transaction.
 * \n Meaningful only for bcmptm_ltm_mreq_keyed_lt API.
 * \n When atomic transactions are disabled, PTM will allow CTH to issue
 * multiple insert/delete operations per transaction only if this flag is
 * asserted.
 */
#define BCMPTM_REQ_FLAGS_ALLOW_MULTIPLE_OPS            0x00000020ULL << 32
...
/*! Transaction ID to be used when using modeled path APIs for initialization */
