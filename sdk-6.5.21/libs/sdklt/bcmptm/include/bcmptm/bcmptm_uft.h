...
typedef struct bcmptm_bank_attr_s {
    ...
    bool robust_en;
    /*! Enabled to indicate narrow mode. */
    bool narrow_mode;
} bcmptm_bank_attr_t;
...
/*! LTPR info per LT.LOOKUP under a certain tile mode. */
typedef struct bcmptm_lt_lookup_ltpr_info_s {
    /*! Tile ID. */
    uint32_t tile_id;

    /*!
     * Tile's physical lookup.
     * RM uses tile_id and p_lkp to sort all the ltpr information and
     * select the first ltpr_index.
     */
    uint16_t p_lkp;
    ...
    /*! Index of LTPR_PROFILE_TABLEm. */
    uint16_t ltpr_index;

    /*! Lookup ID value of tile's physical lookup. */
    uint16_t lkp_id;

} bcmptm_lt_lookup_ltpr_info_t;
...
bcmptm_cth_uft_lt_lts_tcam_get(int unit, bcmltd_sid_t ltid,
                               ...
                               uint32_t *r_cnt);
/*!
 * \brief Get LTPR info list for per LT.LOOKUP.
 * \param [in] unit Logical device id.
 * \param [in] ltid Logical table id.
 * \param [in] lookup_num 0 represents lookup0, 1 represents lookup1.
 * \param [out] ltpr_arr Returned LTPR info.
 * \param [in] ltpr_sz Size of ltpr_arr.
 * \param [out] r_ltpr_cnt Returned count of ltpr_arr.
 *
 * \retval SHR_E_NONE if success, otherwise failure.
 */
extern int
 ...
 * \param [out] narrow_mode Returned narrow_mode of a tile.
 *
 * \retval SHR_E_NONE if success, otherwise failure.
 */
extern int
