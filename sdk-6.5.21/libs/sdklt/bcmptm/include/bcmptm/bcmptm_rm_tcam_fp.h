...
#define BCMPTM_RM_TCAM_MAX_WIDTH_INSTANCES 8
...
typedef enum bcmptm_fp_flags_s {
    ...
    /*! To scope the FP tcams resource required. */
    BCMPTM_FP_FLAGS_RESOURCE_SCOPE,
...
/*!
 * FP required resources info to check the availability without
 * allocating the resources.
 */
typedef struct bcmptm_rm_tcam_fp_resources_req_s {
    /*!
     * FP group id  in which resource availability should
     * be verified.
     */
    uint16_t group_id[BCMPTM_RM_TCAM_GRP_MODE_COUNT];

    /*! Number of free entry slots required in the group. */
    uint32_t entries_req[BCMPTM_RM_TCAM_GRP_MODE_COUNT];

    /*!
     * Number of entry slots allocated for the group.
     * Caller needs to set all elements in this arry to
     * zero.
     */
    uint32_t entries_alloc[BCMPTM_RM_TCAM_GRP_MODE_COUNT];

    /*!
     * Toatal number of free entry slots in the group.
     * Caller needs to set all elements in this arry to
     * zero.
     */
    uint32_t entries_free[BCMPTM_RM_TCAM_GRP_MODE_COUNT];
    ...
    /*!
     * Group mode when group_id si from single mode.
     * Highest group mode when groups_ids from multiple
     * modes are valid.
     */
    bcmptm_rm_tcam_group_mode_t mode;
} bcmptm_rm_tcam_fp_resources_req_t;
...
typedef struct bcmptm_rm_tcam_entry_attrs_s {
       ...
       */
       ...
       /* Group slice counter profile mapping count */
       uint8_t grp_ctr_profiles_cnt;

       /* Group slice counter profile mapping info */
       uint8_t grp_ctr_profiles[BCMPTM_RM_TCAM_MAX_SLICES];

       /*! FP Resources required for verifying availability. */
       bcmptm_rm_tcam_fp_resources_req_t *req_res;

       /*! Number of elements in req_res array. */
       uint16_t num_req_res;

       /*!
        * Indicates whether the requested resources are
        * available or not.
        */
