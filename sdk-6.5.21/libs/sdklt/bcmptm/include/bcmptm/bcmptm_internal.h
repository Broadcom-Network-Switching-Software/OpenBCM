...
/*!
  \brief Cleanup interactive path
...
/*!
  \brief Configure forwarding, accumulation of HW access requests received on
  interactive path.

  \param [in] unit Logical device id
  \param [in] hw_wr_enable TRUE, FALSE implies enable, disable HW writes.
  \param [in] hw_wr_forward TRUE, FALSE implies forward HW writes to WAL/BCMBD.
  \param [in] hw_rd_enable TRUE, FALSE implies enable, disable HW reads.

  \retval SHR_E_NONE Success */

extern int
bcmptm_ireq_mode_set(int unit, bool hw_wr_enable, bool hw_wr_forward,
                     bool hw_rd_enable);
