...
typedef struct em_tile_info_s {
    ...
    /*! Count of available modes for this tile. */
    ...
    /*!
     * Tile's MISS_LTPR_PROFILE_TABLEm ID.
     * If tiles don't have this related PT, use INVALIDm.
 */
    bcmdrd_sid_t ltpr_ptid;

    /*!
     * Count of p_lkp_id array.
     * Index tiles   support up to 1 lookup.
     * E2T/T2T tiles support up to 2 physical lookups.
     * T4T tiles     support up to 4 physical lookups.
     */
    uint32_t p_lkp_id_count;

    /*! Tile physical lookup ID. */
    const uint32_t *p_lkp_id;
...
typedef struct em_lt_lookup_info_tile_mode_s {
    ...
    /*! Count of array p_lkp. */
    uint16_t p_lkp_count;

    /*!
     * Array of tile's physical lookups which support this LT.LOOKUP.
     * Index tiles   support up to 1 lookup.
     * E2T/T2T tiles support up to 2 physical lookups.
     * T4T tiles     support up to 4 physical lookups.
     */
    const uint16_t *p_lkp;

    /*! Corresponding array of LTPR index for each tile's physical lookup. */
    const uint16_t *ltpr_idx;

    /*! Indicates it needs to update LTPR_PROFILE.*/
    bool miss_ltpr_enable;
...
typedef struct em_tile_mode_info_s {
    ...
    /*! Bitmap for hash banks mapped to lane0. */
    uint8_t hash_lane0_bitmap;

    /*! Bitmap for hash banks mapped to lane1. */
    uint8_t hash_lane1_bitmap;
    ...
    /*! Narrow mode is enabled or disabled. */
    bool narrow_mode;

    /*! Enabled to indicate narrow mode can be overriden. */
    bool narrow_mode_override_capable;

    /*! Tile id to which this tile mode belongs. */
    uint32_t tile_id;
...
typedef struct uft_dev_em_grp_s {
    ...
    /*! Indicates enable/disable robust hash. */
    bool robust;
