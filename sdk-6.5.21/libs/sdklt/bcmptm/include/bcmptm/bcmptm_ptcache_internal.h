 ...
 * - BCMPTM_PTCACHE_TYPE is NO_CACHE or CCI_ONLY
 * - never written in cache */
...
/* Different SW cache categories */

/*! PT has not yet been initialized. */
#define BCMPTM_PTCACHE_TYPE_UNINIT 0

/*! PT is !valid or is not_cacheable. */
#define BCMPTM_PTCACHE_TYPE_NO_CACHE 1

/*! PT has multiple entries. */
#define BCMPTM_PTCACHE_TYPE_ME 2

/*! PT is a counter which also has cfg fields. */
#define BCMPTM_PTCACHE_TYPE_ME_CCI 3

/*! PT has single entry, not yet written by SW. */
#define BCMPTM_PTCACHE_TYPE_SE_NOT_VALID 4

/*! PT has single entry, has been written by SW. */
#define BCMPTM_PTCACHE_TYPE_SE_VALID 5

/*! PT is completely managed by CCI */
#define BCMPTM_PTCACHE_TYPE_CCI_ONLY 6

/*! Reserved for PT with 0 entries (in non-Default PT group) */
#define BCMPTM_PTCACHE_TYPE_ME_RSVD 7

/*! PT is cacheable but without cache resources */
#define BCMPTM_PTCACHE_TYPE_SE_CACHE0 8
...
/*! Pointers to vinfo, ltid_seg for PT group. */
typedef struct bcmptm_ptcache_pt_group_ptrs_s {
    /*! Pointer to vinfo array. */
    uint32_t *vinfo_bp;

    /*! Pointer to ltid_seg. */
    void *ltid_seg_base_ptr;
} bcmptm_ptcache_pt_group_ptrs_t;


/*! Controls for ISSU patches */
typedef struct bcmptm_ptcache_issu_ctl {
    /*! Skip init of vinfo, ltid segments during cwme_vinfo_init */
    bool skip_vinfo_ltid;

    /*! Skip calling of cci during sinfo_init */
    bool skip_sinfo_cci_init;
} bcmptm_ptcache_issu_ctl_t;
...
/*!
 * \brief Initialize sids info array.
 *
 * \param [in] unit Logical device id
 * \param [in] warm TRUE => warmboot, FALSE => coldboot.
 * \param [in] pt_size_change TRUE => Size of physical table has changed.
 * \param [in] all_sid_count Number of PTs in this physical table group.
 * \param [in] pt_group Identifier for a device-specific physical table group.
 *
 * \param [in] in_seds_index Current index into seds array.
 * \param [in] in_cwme_index Current index into cwme array.
 *
 * \param [out] vinfo_word_count Count of words needed in vinfo array.
 * \param [out] ltid_seg_word_count Count of words needed in ltid segment.
 * \param [out] group_seds_count Count of words needed in seds array.
 * \param [out] group_cwme_count Count of words needed in cwme array.
 *
 * \retval SHR_E_NONE Success
 */
extern int
...
/*!
 * \brief Initialize cwme array, vinfo array for a PT with multiple entries.
 *
 * \param [in] unit Logical device id
 * \param [in] warm TRUE => warmboot, FALSE => coldboot.
 * \param [in] pt_size_change TRUE => Size of physical table has changed.
 ...
 */
extern int
bcmptm_ptcache_cwme_vinfo_init_group(int unit, bool warm, bool pt_size_change,
                                     size_t all_sid_count, uint32_t pt_group);

/*!
 * \brief Provide pointers to PTcache arrays.
 *
 * \param [in] unit Logical device id
 * \param [in] o_sinfo_bp Address of var which holds ptr to sids info.
 * \param [in] o_pt_group_info_bp Address of var which holds ptr to pt_group
 * info.
 * \param [in] o_pt_group_ptrs_bp Address of var which holds ptr to pt_group
 * ptrs info.
 * \param [in] o_seds_bp Address of var which holds ptr to seds info.
 * \param [in] o_cwme_bp Address of var which holds ptr to cwme info.
 * \param [in] o_issu_ctl_p Address of var which holds ptr to issu_ctl.
 * \param [in] o_ltid_size16b Address of var which holds ltid_size16b.
 *
 * \retval SHR_E_NONE Success
 */
extern int
...
/*!
 * \brief Set pointers to (new) PTcache arrays.
 *
 * \param [in] unit Logical device id
 * \param [in] i_sinfo_bp Pointer to sids info.
 * \param [in] i_pt_group_info_bp Pointer to pt_group info.
 * \param [in] i_pt_group_ptrs_bp Pointer to pt_group_ptrs info.
 * \param [in] i_seds_bp Pointer to seds info.
 * \param [in] i_cwme_bp Pointer to cwme info.
 *
 * \retval SHR_E_NONE Success
 */
extern int
