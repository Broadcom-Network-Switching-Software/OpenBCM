...
/*!
  \brief Enable WAL to record, send ops for init_seq. Also can configure WAL to
  ignore ops that are being accumulated in PTcache.

  \param [in] unit Logical device id
  \param [in] record TRUE Record init_seq ops in WAL.
  \param [in] send TRUE Send ops recorded in WAL to HW and revert to configured
  WAL mode.
  \param [in] ignore TRUE Ignore ops (which are being accumulated in PTcache).

  \retval SHR_E_NONE Success */

extern int
bcmptm_wal_init_seq(int unit, bool record, bool send, bool ignore);
