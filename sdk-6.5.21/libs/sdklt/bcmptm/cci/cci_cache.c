...
#define CDMIB_RXPKT_ROW           2
#define CDMIB_TXPKT_ROW           10
#define CDMIB_ROW_PER_PORT        16
/*!
...
/*!
 * Led war update the link status basd on counter
 * [in] unit, current unit.
 * [in] map, Pointer to the entry in countr map Table for SID
 * [in] index, Index  in cdmib table.
 * [in] inst,  cdport instance.
 * [in] fieldnum, Counter Field number in entry for SID
 * [in] c,        Counter value.
 * [in] txpkt_fieldnum, Counter Field number for tx pkt.
 * [in] txpkt_fieldnum, Counter Field number for rxpkt.
 * [in] xlmib_rxsid, sid of rxpkt register in xlmib.
 * [in] xlmib_txsid, sid of txpkt register in xlmib.
 */

static inline void
ctr_led_war_link_status_update(int unit ,cci_ctr_cache_map_t *map, int index, int inst,
                               int fieldnum, uint64_t c,int txpkt_fieldnum, int rxpkt_fieldnum,
                               bcmdrd_sid_t xlmib_rxsid, bcmdrd_sid_t xlmib_txsid) {
    int rx = 0 ;
    if (((index % CDMIB_ROW_PER_PORT) == CDMIB_TXPKT_ROW && (fieldnum == txpkt_fieldnum)) ||
       ((index %  CDMIB_ROW_PER_PORT) == CDMIB_RXPKT_ROW && (fieldnum == rxpkt_fieldnum)) ||
       (map->sid == xlmib_rxsid) || (map->sid == xlmib_txsid))   {
        if (((index % CDMIB_ROW_PER_PORT) == CDMIB_RXPKT_ROW) || (map->sid == xlmib_rxsid)) {
            rx = 1;
        }
        bcmptm_pt_cci_link_status_update(unit, index, inst, rx, c);
    }
}
...
int
bcmptm_cci_cache_bypass_pt_field_read(int unit,
                                      cci_handle_t handle,
                                      bcmptm_cci_ctr_info_t *info,
                                      uint32_t *buf,
                                      size_t size)
{
    ...
    for (i = fld_start, lt_fld = 0; i < fld_end; i++, lt_fld++) {
        ...
        if (INDEX_VALID(map, dyn->tbl_inst, dyn->index)) {
            ...
            c[0] = (uint32_t)cache->ctr_table[index].sw_count;
            c[1] = (uint32_t)(cache->ctr_table[index].sw_count >> 32);
            sw_ctr = cache->ctr_table[index].sw_count;
            LOG_VERBOSE(BSL_LOG_MODULE,
              (BSL_META_U(unit, "Name = %s, index = %u, sw_ctr = 0x%08x_%08x, lt-minbit = %d\n"),
                          bcmdrd_pt_sid_to_name(unit, lt_map->sid), dyn->index,
                          c[1],c[0],
                          lt_map->lt_field[lt_fld].minbit));

            SHR_IF_ERR_EXIT
                (bcmptm_pt_cci_ctr_ing_pfc_field_val_get(unit,
                                                         info,
                                                         lt_map->sid,
                                                         sw_ctr,
                                                         buf,
                                                         size));
            LOG_VERBOSE(BSL_LOG_MODULE,
              (BSL_META_U(unit, "Name = %s, index = %u, hw_ctr = 0x%08x_%08x,"
                          "lt-minbit = %d direct_access = %s\n"),
                          bcmdrd_pt_sid_to_name(unit, lt_map->sid), dyn->index,
                          *(buf+1), *buf,
                          lt_map->lt_field[lt_fld].minbit,
                          info->direct_access ? "true" : "false"));
        } else {
            LOG_ERROR(BSL_LOG_MODULE,
              (BSL_META_U(unit, "Index = %d, Tbl_nst = %d\n"),
                          dyn->index, dyn->tbl_inst));
                        ...
                        size_t size)
{
    ...
    uint32_t index;
    uint64_t c;
     /* tpkt and rpkt field number in the data  row for cdmib */
    int txpkt_fieldnum = 4 ;
    int rxpkt_fieldnum = 0 ;
    bcmdrd_sid_t xlmib_rxsid = 0;
    bcmdrd_sid_t xlmib_txsid = 0;
    int led_war = 0, rv = 0;
    SHR_FUNC_ENTER(unit);
    ...
    if (map->ctrtype == CCI_CTR_TYPE_EVICT) {
        ...
        if (INDEX_VALID(map, dyn->tbl_inst, dyn->index)) {
            ...
            if ((mode != CCI_CTR_NORMAL_MODE) || (update_mode != CCI_UPDATE_INC_BY_CVALUE) ||
                ...
                /* Return from here in case of special counter mode for eviction counters */
                if (SHR_FAILURE(rv)) {
                    SHR_ERR_EXIT(rv);
                }
                SHR_EXIT();
    ...
    /*
     * Led war
     * Get rx and tx register for xlmib and row offset for cdmib
     */
    if ((map->ctrtype == CCI_CTR_TYPE_PORT)) {
         rv  = bcmptm_pt_cci_link_status_data_get(unit, &xlmib_rxsid, &xlmib_txsid, &rxpkt_fieldnum, &txpkt_fieldnum);
         if (rv == SHR_E_NONE) {
             led_war = 1;
         } else {
             led_war = 0;
    ...
    for (i = 0; i < map->field_num; i++) {
        ...
        if (INDEX_VALID(map, dyn->tbl_inst, dyn->index)) {
            ...
            if (map->clr_on_read) {
               ...
               cache->ctr_table[index].hw_count = 0;
               if (led_war) {
                   ctr_led_war_link_status_update(unit, map, dyn->index, dyn->tbl_inst, i, c,
                                                  txpkt_fieldnum, rxpkt_fieldnum,
                                                  xlmib_rxsid, xlmib_txsid);
               }
            } else {
                if (c < cache->ctr_table[index].hw_count) {
                   ...
                   cache->ctr_table[index].sw_count +=
                    ...
                    if (led_war) {
                        ctr_led_war_link_status_update(unit, map, dyn->index, dyn->tbl_inst,
                                                       i, c, txpkt_fieldnum, rxpkt_fieldnum,
                                                       xlmib_rxsid, xlmib_txsid);
                    }
                } else {
                   cache->ctr_table[index].sw_count +=
                   ...
                   if (led_war) {
                        ctr_led_war_link_status_update(unit, map, dyn->index, dyn->tbl_inst,
                                                       i, (c - cache->ctr_table[index].hw_count),
                                                       txpkt_fieldnum, rxpkt_fieldnum,
                                                       xlmib_rxsid, xlmib_txsid);
                   }
                }
                                       ...
                                       bool update_cache)
{
    ...
    if (update_cache) {
        ...
        size = (cache->ctr_map_arr->ctr_map_table[info->map_id].field_num + 1) *
               sizeof(uint64_t);
                            ...
                            uint32_t *ctr)
{
    ...
    if (INDEX_VALID(map, dyn->tbl_inst, dyn->index)) {
        ...
        cache->ctr_table[index].sw_count = (((uint64_t)ctr[1] << 32) | ctr[0]);
        LOG_VERBOSE(BSL_LOG_MODULE,
            (BSL_META_U(unit, "Name = %s, index = %u, count = 0x%08x_%08x\n"),
                        bcmdrd_pt_sid_to_name(unit, map->sid), index,
                        ctr[1], ctr[0]));
                                   ...
                                   bool update_cache)
{
    ...
    if (INDEX_VALID(map, dyn->tbl_inst, dyn->index)) {
        ...
        if (update_mode == CCI_UPDATE_INC_BY_CVALUE) {
            ...
            bcmptm_cci_buff32_field_copy(entry, field * n, ctr, 0, n);
            bcmptm_cci_buff32_field_copy(entry, (field + 1) * n, &ctr[2], 0, n);
        }
        /* Write back to the hardware */
        SHR_IF_ERR_EXIT
            (cci_pt_write(unit, info, entry));
