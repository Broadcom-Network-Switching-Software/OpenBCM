               ...
               cci_config_t *config)
{
    ...
    if (f_info[id].id == CTR_CONTROLt_PORT_IDf) {
        /*
    ...
    } else if (f_info[id].id == CTR_CONTROLt_COLLECTION_ENABLEf) {
        config->col_enable = data;
        *cmd = MSG_CMD_POL_CONFIG_COL_EN;
    } else if (f_info[id].id == CTR_CONTROLt_INTERVALf) {
        config->interval = data;
        *cmd = MSG_CMD_POL_CONFIG_INTERVAL;
    } else if (f_info[id].id == CTR_CONTROLt_MULTIPLIER_PORTf) {
        config->multiplier[CCI_CTR_TYPE_PORT] = data;
        *cmd = MSG_CMD_POL_CONFIG_MULTIPLIER_PORT;
    } else if (f_info[id].id == CTR_CONTROLt_MULTIPLIER_IPIPEf) {
        config->multiplier[CCI_CTR_TYPE_IPIPE] = data;
        *cmd = MSG_CMD_POL_CONFIG_MULTIPLIER_IPIPE;
    } else if (f_info[id].id == CTR_CONTROLt_MULTIPLIER_TMf) {
        config->multiplier[CCI_CTR_TYPE_TM] = data;
        *cmd = MSG_CMD_POL_CONFIG_MULTIPLIER_TM;
    } else if (f_info[id].id == CTR_CONTROLt_MULTIPLIER_EPIPEf) {
         config->multiplier[CCI_CTR_TYPE_EPIPE] = data;
        *cmd = MSG_CMD_POL_CONFIG_MULTIPLIER_EPIPE;
    } else if (f_info[id].id ==CTR_CONTROLt_MULTIPLIER_EVICTf) {
        config->multiplier[CCI_CTR_TYPE_EVICT] = data;
        *cmd = MSG_CMD_POL_CONFIG_MULTIPLIER_EVICT;
    } else if (f_info[id].id == CTR_CONTROLt_MULTIPLIER_SECf) {
        if (!bcmsec_is_sec_supported(unit) && data) {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                    "BCMSEC is disabled. MULTIPLIER_SEC skipped. \n")));
            config->multiplier[CCI_CTR_TYPE_SEC] = 0;
            *cmd = MSG_CMD_POL_CONFIG_MULTIPLIER_SEC;
        } else {
            config->multiplier[CCI_CTR_TYPE_SEC] = data;
            *cmd = MSG_CMD_POL_CONFIG_MULTIPLIER_SEC;
        }
    } else if (f_info[id].id == CTR_CONTROLt_EVICTION_THRESHOLDf) {
        config->eviction_threshold = data;
        *cmd = MSG_CMD_EVICTION_THRESHOLD;
    } else if (f_info[id].id == CTR_CONTROLt_SKIP_RESIDUE_COLLECTIONf) {
        config->skip_residue_col = data;
        *cmd = MSG_CMD_POL_CONFIG_SKIP_RESIDUE_COL;
    } else {
