...
static int
cci_col_poll_config_get(int unit,  cci_config_t *config)
{
    ...
    /* Try to get the cached config data */
    rv = bcmptm_cci_col_poll_config_get(unit, hpol, config);
    /* Get from imm */
    if (SHR_FAILURE(rv)) {
        SHR_IF_ERR_EXIT
...
static bool
cci_ctr_hw_sync_on_read(int unit,
                        cci_handle_t handle,
                        bcmptm_cci_ctr_info_t *info)
{
    bool cor, emul;
    ...
    rv = cci_col_poll_config_get(unit, &config);
    if (SHR_FAILURE(rv)) {
        return false;
    }

    /* Sync the cache for counters with COR */
    rv = bcmptm_cci_cache_clearon_read_get(unit, handle,
                                           info->map_id, &cor);
    if (SHR_FAILURE(rv)) {
        return false;
    }
     ...
     * Sync CCI Cache if COR is set but skip residue collection not set.
     */
    if ((info->flags & BCMLT_ENT_ATTR_GET_FROM_HW) ||
         (cor && !emul && !config.skip_residue_col)) {
        sync = true;
    }
...
* If direct_access=1, fetch counter directly on HW and bypass cache update process
*/
int
                    ...
                    size_t rsp_entry_size)
{
    ...
    /* Initial counter direct access */
    info->direct_access = false;

    /* Sync CCI Cache with HW. */
    if (cci_ctr_hw_sync_on_read(unit, handle, info)) {
        bcmptm_cci_cache_pt_param_t param;
        ...
        bcmptm_pt_cci_ctr_direct_access(unit, info);


        SHR_IF_ERR_EXIT
            (cci_hw_sync_req_read(unit, handle, info, &param));

        if (info->direct_access == true) {
            /* Fetch counter pt field value */
            SHR_IF_ERR_EXIT
                (bcmptm_cci_cache_bypass_pt_field_read(unit,
                                              handle,
                                              info,
                                              rsp_entry_words,
                                              rsp_entry_size));
...
bcmptm_cci_stop(int unit)
{
    ...
    SHR_FUNC_ENTER(unit);
    con = cci_context[unit];
    /* If there is no context yet simply exit quietly */
    SHR_NULL_CHECK(con, SHR_E_NONE);
...
bcmptm_cci_cleanup(int unit)
{
    ...
    SHR_FUNC_ENTER(unit);
    con = cci_context[unit];
    /* Ignore the case where there is no context */
    SHR_NULL_CHECK(con, SHR_E_NONE);
                       ...
                       uint32_t bit)
{
    ...
    if (pipe != BCMPTM_CCI_PIPE_ALL) {
        ...
        /* All pipes, take default pipe and check index */
        if (!bcmptm_pt_cci_index_valid(unit, sid, 0, index)) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    }
