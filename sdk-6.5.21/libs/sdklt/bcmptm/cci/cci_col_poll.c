...
typedef struct col_thread_instr_data_s {
   ...
   /*! Number of Counter eviction events. */
   uint32_t ctr_evict_events;
                    ...
                    bool to_cfg)
{
    ...
    case MSG_CMD_EVICTION_THRESHOLD:
        ...
        break;
    case MSG_CMD_POL_CONFIG_SKIP_RESIDUE_COL:
        if (to_cfg) {
            config->skip_residue_col = msg->data[0];
        } else {
            msg->data[0] = config->skip_residue_col;
        }
        break;
    default:
        break;
                                ...
                                int *num_items)
{
    ...
    for (i = 0; i < ctc->num_port_reg_blk; i++) {
        ...
        if (sport <= port_blk->tbl_inst_max) {
            dyn_info.index = 0;
            dyn_info.tbl_inst = sport;
            rv = bcmbd_pt_cmd_hdr_get(unit, sid, &dyn_info, NULL,
            ...
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
                                    "SID = %s, sport=%d\n"),
                        bcmdrd_pt_sid_to_name(unit, sid), sport));

            /* Populate the work data */
            data[items].op_code = cwords[0];
            data[items].buf_paddr = ctc->paddr_buf + buff_offset;
            data[items].start_addr = cwords[1];
                                ...
                                col_ctr_work_t *ctr_work)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmptm_pt_cci_frmt_index_from_port(unit, sid, ctr_work->tbl_inst,
                                            &index_min, &index_max));
    /* Get schan header */
    dyn_info.index = index_min;
    dyn_info.tbl_inst = bcmdrd_pt_tbl_inst_from_port(unit, sid,
                                             ctr_work->tbl_inst);
    ...
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
            (BSL_META_U(unit,
                        "ERROR: SID=%s, port=%d tbl_inst=%d\n"),
                        bcmdrd_pt_sid_to_name(unit, sid),
                        ctr_work->port, ctr_work->tbl_inst));
        /* Skip unsupported port */
        SHR_ERR_EXIT(SHR_E_PORT);
    }

    /* Populate the work data */
    data->op_code = cwords[0];                    /* Opcode beat */
    data->start_addr = cwords[1];                 /* Addr beat */
    data->data_width = ctr_work->pt_entry_wsize;  /* Num of data beats */
                          ...
                          cci_handle_t handle)
{
    ...
    dyn_info.index = sinfo.index_min;
    dyn_info.tbl_inst = 0;
    SHR_IF_ERR_EXIT
        (bcmbd_pt_cmd_hdr_get(unit, sid, &dyn_info, NULL,
                              BCMBD_PT_CMD_POP, 4, cwords, &nwords));

    evict_data->start_addr = cwords[1];                 /* Addr beat */
    evict_data->data_width = sinfo.entry_wsize;
    evict_data->op_code =  cwords[0];
...
col_poll_dump_instr_data(int unit, col_thread_ctrl_t *ctc)
{
   ...
   LOG_INFO(BSL_LOG_MODULE,(BSL_META_U(unit,
                        "Number of Counter eviction events. = %u\n"),
             instr_data->ctr_evict_events));
}
...
col_poll_ctr_dispatch(int unit, cci_handle_t handle, bool sync, cci_ctr_type_id type)
{
    ...
    if ((type == CCI_CTR_TYPE_SEC) || (type == CCI_CTR_TYPE_NUM)) {
        if (bcmsec_is_sec_supported(unit) &&
            (cci_col_time(ctc, CCI_CTR_TYPE_SEC) || sync)) {
            stime = sal_time_usecs();
            if (ctc->config.dma) {
                /* Counter collection using SBUSDMA. */
                SHR_IF_ERR_EXIT
                     ...
                     bool update_cache)
{
    ...
    bcmptm_ptcache_ctr_info_t ptcache_ctr_info;
    bcmptm_cci_ctr_info_t info = {0};
    bcmptm_cci_cache_ctr_info_t cci_cache_cr_info;
    uint32_t entry[4] = {0};
    uint32_t bit;
    uint32_t pipe, p_start, p_end;
    ...
    SHR_IF_ERR_EXIT
        (bcmptm_cci_cache_ctr_info_get(unit, handle,
         ptcache_ctr_info.cci_map_id, &cci_cache_cr_info));
    ...
    /* Check pipes */
    if (pipe == BCMPTM_CCI_PIPE_ALL) {
        p_start = 0;
        p_end = cci_cache_cr_info.tbl_inst - 1;
    } else {
       p_start = pipe;
       p_end = pipe;
    }

    /* Set bit in cci cache */
    for (pipe = p_start; pipe <= p_end; pipe++) {
        if (!IS_PIPE_ENABLE(cci_cache_cr_info.pipe_map, pipe)) {
            continue;
        }
                               ...
                               cci_config_t *config)
{
    ...
    if (pol->hthreads) {
        ctc = (col_thread_ctrl_t *)pol->hthreads[0];
        SHR_NULL_CHECK(ctc, SHR_E_INIT);
        sal_memcpy(config, &ctc->config, sizeof(cci_config_t));
    } else {
        SHR_VERBOSE_EXIT(SHR_E_INIT);
    }
