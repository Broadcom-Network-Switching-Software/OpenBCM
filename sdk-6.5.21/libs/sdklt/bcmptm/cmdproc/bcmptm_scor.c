...
static bool hw_wr_enable[BCMDRD_CONFIG_MAX_UNITS];
static bool hw_wr_forward[BCMDRD_CONFIG_MAX_UNITS];
static bool hw_rd_enable[BCMDRD_CONFIG_MAX_UNITS];
                 ...
                 uint32_t *rsp_entry_words)
{
    ...
    if (flags & BCMPTM_SCOR_REQ_FLAGS_DONT_USE_CACHE) { /* HW read requested */
        if (hw_rd_enable[unit]) {
            LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "SERC requested read from HW for sid %0d, "
                            "pt %s \n"),
             ...
             *   worked (hard-fault detection) and failure on read from HW
             *   indicates that SER correction could not fix the bad location.
             */
            SHR_IF_ERR_EXIT(
                bcmbd_pt_read(unit,
                              sid,
            ...
            if (bcmdrd_pt_attr_is_cam(unit, sid) &&
                ...
                SHR_IF_ERR_EXIT(
                     ...
                     * with km words to save on un-necessary copy from km words
                     * array back to rsp words array.
                     */
            }
        } else {
            /* Display warning, but don't return E_UNAVAIL.
             *      SHR_ERR_EXIT(SHR_E_UNAVAIL);
             * Instead, redirect read to the WAL.
             */
            LOG_WARN(BSL_LOG_MODULE,
                     (BSL_META_U(unit, "Will send SERC hw_read for sid=%0d (%s)"
                                 " to WAL\n"),
                      sid, bcmdrd_pt_sid_to_name(unit, sid)));
            SHR_IF_ERR_EXIT(
        ...
        SHR_EXIT(); /* Read from HW is complete */
    }
                  ...
                  uint32_t *entry_words)
{
     ...
     * - Needed only if writing to HW is enabled and forwarding is disabled. */
    if (hw_wr_enable[unit] && !hw_wr_forward[unit] &&
    ...
    if (hw_wr_enable[unit]) {
        if (hw_wr_forward[unit]) {
            SHR_IF_ERR_EXIT(
                bcmptm_wal_write(unit,
                                 0, /* flags */
                                 sid,
                                 pt_dyn_info,
                                 NULL, /* misc_info */
                                 entry_words, /* km_entry */
                                 BCMPTM_RM_OP_NORMAL, /* bcmptm_rm_op_t */
                                 NULL)); /* bcmptm_rm_slice_change_t */
        } else { /* can use bcmbd */
            SHR_IF_ERR_EXIT(
                bcmbd_pt_write(unit,
                               sid,
...
bcmptm_scor_init(int unit, bool warm)
{
    ...
    /* Normal mode */
    hw_wr_enable[unit] = TRUE;
    hw_wr_forward[unit] = FALSE;
    hw_rd_enable[unit] = TRUE;
exit:
    SHR_FUNC_EXIT();
}
...
int
bcmptm_scor_mode_set(int unit, bool wr_enable, bool wr_forward,
                     bool rd_enable)
{
    hw_wr_enable[unit] = wr_enable;
    hw_wr_forward[unit] = wr_enable && wr_forward;
    hw_rd_enable[unit] = rd_enable;

    /* Truth Table
     *
     * hw_wr_enable    hw_wr_forward    Note
     * -------------   -------------    ------------
     *      1                0          Normal mode.
     *                                  Issue hw writes to bcmbd and update
     *                                  PTcache.
     *
     *      1                1          Forwarding mode.
     *                                  Forward hw writes to WAL and update
     *                                  PTcache.
     *
     *      0                x          Only update PTcache.
     *
     * hw_rd_enable
     * ------------
     *      1                            Normal mode. Issue read to bcmbd.
     *
     *      0                            Return data from WAL.
     *                                   (don't return SHR_E_UNAVAIL)
     *
     * Forwarding:
     *  (hw_wr_enable, hw_wr_forward, hw_rd_enable) = (T, T, F)
     *
     * Accumulation:
     *  (hw_wr_enable, hw_wr_forward, hw_rd_enable) = (F, F, F)
     *
     * Normal mode:
     *  (hw_wr_enable, hw_wr_forward, hw_rd_enable) = (T, F, T)
     */
