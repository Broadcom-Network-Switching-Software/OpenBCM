...
#define BCMPTM_UT_WAL_MAX_WORDS_IN_MSG 352 /* 16 max size writes = 16 * 22
                                               words */
#define BCMPTM_UT_WAL_MAX_WORDS_IN_MSG_DEF 100
...
#define BCMPTM_UT_WAL_MAX_OPS_IN_MSG_SMALL 2
#define BCMPTM_UT_WAL_MAX_OPS_IN_MSG_DEF 5
...
#define BCMPTM_UT_WAL_MAX_MSGS_IN_TRANS_SMALL 4
#define BCMPTM_UT_WAL_MAX_MSGS_IN_TRANS_DEF 3
...
#define WAL_EMPTY_CHECK_USLEEP_TIME 1000 /* uSec */
#define WAL_EMPTY_CHECK_RETRY_COUNT 30
#define WAL_EMPTY_CHECK_RETRY_SLOW_POLLING_THR 9
...
/* max depth allowed for any WAL buffer: 1G = 1024M elements */
#define BCMPTM_WAL_BUF_DEPTH_MAX 0x40000000
...
static bool record_iseq[BCMDRD_CONFIG_MAX_UNITS];
static bool ignore_mode[BCMDRD_CONFIG_MAX_UNITS];
 ...
 */
static uint32_t
wal_depth_scale(int u, uint32_t cfg_wal_depth_mult, uint32_t d)
{
    uint64_t tmp_d = (uint64_t)cfg_wal_depth_mult * (uint64_t)d;
    tmp_d /= 1000;
    if (tmp_d <= BCMPTM_WAL_BUF_DEPTH_MAX) {
        return tmp_d;
    } else {
        LOG_WARN(BSL_LOG_MODULE,
            (BSL_META_U(u,
             "Truncating o_buf_depth to %0u for i_depth_mult=%0u, "
             "i_default_depth_device=%0u\n"),
             BCMPTM_WAL_BUF_DEPTH_MAX, cfg_wal_depth_mult, d));
        return BCMPTM_WAL_BUF_DEPTH_MAX;
    }
}
...
wal_bcmcfg_get(int u)
{
    ...
    /* This is the only place where cfg_scf_init_flags[] are evaluated.
     * - Above contains best possible settings for using schan_fifo. It must not
     *   be altered. In particular, if ccmdma_wr flag is not set in
     *   cfg_scf_init_flags[], it means WAL is not configured to use DMA for
     *   filling schan_fifo. This setting must not be violated.
     */
    ...
    cfg_wal_trans_max_count[u] =
        wal_depth_scale(u, cfg_wal_depth_mult, device_wal_info.trans_max_count);

    cfg_wal_msg_max_count[u] =
        wal_depth_scale(u, cfg_wal_depth_mult, device_wal_info.msg_max_count);

    cfg_wal_words_buf_max_count[u] =
        wal_depth_scale(u, cfg_wal_depth_mult,
                        device_wal_info.words_buf_max_count);

    cfg_wal_ops_info_max_count[u] =
        wal_depth_scale(u, cfg_wal_depth_mult,
                        device_wal_info.ops_info_max_count);
             ...
             size_t rsp_buf_wsize)
{
    ...
    /* flags = BCMBD_ROF_INTR; */
    /* send ops to HW */
    SHR_IF_ERR_EXIT(
        bcmbd_pt_range_read(u, sid, pt_dyn_info,
                            NULL, /* pt_ovrr_info */
                            entry_count, /* num_entries */
                            bcmptm_wal_words_buf[u],
                            bcmptm_wal_words_buf_bpa[u],
                                      /* physical 64b dma_addr */
                            flags));

    if (tmp_rv != SHR_E_NONE) {
        LOG_ERROR(BSL_LOG_MODULE,
            (BSL_META_U(u, "bcmptm_wal_dma_read.bcmbd_pt_range_read"
                        "returned rv=%0d\n"), tmp_rv));
    ...
    *alloc_p = shr_ha_mem_alloc(u, BCMMGMT_PTM_COMP_ID,
    ...
    if (warm) {
        ...
        if (alloc_size < req_size) {
             ...
             * Because, HA wal data_structs are not reported to issu, so issu
             * infra cannot take care of above changes - and WAL needs to adjust
             * the size changes by doing realloc. But, because warm-issu was
             * preceded by coldboot shutdown, WAL is empty during warm-issu -
             * and hence there is no need to copy data from old issu block to
             * new issu block. We can zero out the data to be on safe side.
             *
             * Note: This is different from PTcache warm-issu case where data
             *       needs to be preserved between old HA block and new HA
             *       block. Hence those structs were declared in bcmptm/yaml
             *       and reported to issu by calling
            ...
            *alloc_p = realloc_p;
             ...
             We can be here for warm-crash or warm-vacation:
             1. Because warm-vacation is preceded by coldboot shutdown,
                wal_free() has already done clear of WAL HA data-structs by
                calling wal_ha_mem_clear.
             2. Must not do mem clear for case of warm-crash.
        } */
    } else { /* coldboot */
...
bcmptm_wal_cfg_set(int u)
{
    ...
    /* cfg_xyz_count vars control size of WAL memory allocated - this should
     * remain consistent between coldboot, warmboot. Hence, these are based
     * solely on enable_legacy_init var.
     *
     * When new_init_seq is enabled:
     *      - During coldboot:
     *          - Recording of init_seq in WAL will be enabled.
     *
     *      - During warmboot:
     *          - Recording of init_seq will NOT be enabled - and thus WAL will
     *            not be temporarily be configured in atomic_trans_enabled mode.
     *
     *          - Sending of init_seq from WAL will not be requested.
     *
     * When legacy_init_seq is enabled:
     *      - Recording, sending  of init_seq will NOT be requested.
     *      - WAL will not be temporarily be configured in atomic_trans_enabled
     *        mode.
     */
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(u, "enable_legacy_init is %s\n"),
    ...
    case -1:
        ...
        } /* else: don't override. Small WAL will not work with new_init_seq. */
        break;
    case -2: /* No WAL bypass (two thread mode) */
        ...
        if (enable_legacy_init) {
            cfg_wal_max_words_in_msg[u] = BCMPTM_UT_WAL_MAX_WORDS_IN_MSG_DEF;
            cfg_wal_max_ops_in_msg[u] = BCMPTM_UT_WAL_MAX_OPS_IN_MSG_DEF;
            cfg_wal_max_msgs_in_trans[u] = BCMPTM_UT_WAL_MAX_MSGS_IN_TRANS_DEF;
        } /* else: don't override. Small WAL will not work with new_init_seq. */
        break;
    ...
    case 110: /* same as -10, but not auto-commit mode */
        ...
        if (enable_legacy_init) {
            cfg_wal_max_ops_in_msg[u] = BCMPTM_UT_WAL_MAX_OPS_IN_MSG_SMALL;
            cfg_wal_max_msgs_in_trans[u] = BCMPTM_UT_WAL_MAX_MSGS_IN_TRANS_SMALL;
        } /* else: don't override. Small WAL will not work with new_init_seq. */
        break;
...
{
    SHR_FUNC_ENTER(u);
    if (record_iseq[u] || ignore_mode[u] ||
        bcmptm_wal_autocommit_enabled(u)) { /* no checking */
        SHR_EXIT();
                     ...
                     void *notify_info)
{
    ...
    switch (trans_cmd) {
        case BCMPTM_TRANS_CMD_COMMIT:
            ...
            if (ignore_mode[u]) {
                /* Ignore the commit - we don't want to send any ops to HW. */
                if (notify_fn_p) {
                    notify_fn_p(in_cseq_id, SHR_E_NONE, notify_info);
                }
            } else {
...
wal_mreq_state_update(int u, uint32_t in_cseq_id, uint64_t flags)
{
    ...
    /* No setting of mreq_state_in_trans, recording of cseq_id when
     * record_iseq == T. */
exit:
    SHR_FUNC_EXIT();
}
...
wal_slam_exe(int u,
             ...
             uint32_t entry_count)
{
    uint32_t pt_entry_wsize, *w_words_p;
    size_t flags = 0;
    SHR_FUNC_ENTER(u);
    ...
    if (cfg_wal_words_buf_max_count[u] >= pt_entry_wsize) {
        ...
        /* send ops to HW */
        SHR_IF_ERR_EXIT(
            bcmbd_pt_range_write(u, sid, pt_dyn_info,
                                 NULL, /* pt_ovrr_info */
                                 entry_count, /* num_entries */
                                 w_words_p,
                                 bcmptm_wal_words_buf_bpa[u],
                 ...
                 bcmptm_rm_slice_change_t *slice_mode_change_info)
{
    ...
    /* When record_iseq == T, atomic_trans are enabled
     * - but we don't want to maintain undo_ops - to allow writing of
     * non-cacheable PTs through modeled path at init.
     */
    if (!record_iseq[u] && !wal_disable_undo[u]) {
    ...
    do {
        ...
        if ((avail_trans_count >= req_trans_count) &&
            ...
            if (bcmptm_wal_scf_num_chans[u] && !slam_en) {
                if (record_iseq[u]) { /* Add cmd_hdr on WAL writer side */
                    SHR_IF_ERR_EXIT(
                        bcmbd_pt_cmd_hdr_get(u, sid, pt_dyn_info, NULL,
                                             BCMBD_PT_CMD_WRITE,
                                             BCMPTM_MAX_BD_CWORDS,
            ...
            if (req_msg_count) { /* new msg */
                ...
                switch (op_type) {
                    ...
                    default:
                        ...
                        w_msg_p->cmd_hdr_stuffed = record_iseq[u];
                        break;
                }
 ...
 * - When record_iseq == T:
 *      - disable_undo = F
 *      - state can only be IDLE, IN_MSG, NEED_NEW_MSG
 *          - state can be BLOCKING_READ, but not when wal_empty_make is called
 *            (because init_seq caller always will issue commit after calling
 *             bcmptm_wal_read)
 *
 * - What does this do?
                  ...
                  void *notify_info)
{
    ...
    switch (wstate[u]->c_trans_state) {
        ...
        case TRANS_STATE_IN_MSG:
            c_trans_p = wal_trans[u] + wstate[u]->c_trans_idx;
            /* Concatenate all init_seq ops into one single atomic_trans */
            if (record_iseq[u]) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(u, "Ignoring commit for cseq_id=%0d, "
                                "trans_idx=%0d, wstate=%0d\n"),
        ...
        default: /* TRANS_STATE_IDLE, IDLE_AFTER_WR */
            ...
            if (record_iseq[u] ||
                mreq_state_in_trans || wal_disable_undo[u]) {
                 ...
                 *
                 * For record_iseq == T, enable_atomic_trans will be 1 and we
                 * want to ignore the commit because WAL is anyways empty. Is
                 * this possible?
                 */
    ...
    switch (wstate[u]->c_trans_state) {
        ...
        case TRANS_STATE_NEED_NEW_MSG:
            ...
            if (wal_disable_undo[u]) {
                ...
                if (record_iseq[u]) {
                    LOG_ERROR(BSL_LOG_MODULE,
                        (BSL_META_U(u, "Abort received while recording init_seq"
                                    " !! Cannot undo changes to PTcache or RM "
                                    "state\n")));
                    SHR_ERR_EXIT(SHR_E_INTERNAL);
                    /* mxef_INIT_SEQ: should we attempt to cleanup WAL state
                     * or assume that SDK will be shutdown and re-attached? */
                } else {
...
bcmptm_wal_drain(int u, bool end_check)
{
    ...
    do {
        ...
        if (retry_count > WAL_EMPTY_CHECK_RETRY_COUNT) {
            ...
            if (retry_count > WAL_EMPTY_CHECK_RETRY_SLOW_POLLING_THR) {
                sal_usleep(WAL_EMPTY_CHECK_USLEEP_TIME <<
                           (retry_count -
                            WAL_EMPTY_CHECK_RETRY_SLOW_POLLING_THR));
            } else { /* aggressive polling */
                sal_usleep(WAL_EMPTY_CHECK_USLEEP_TIME);
            }
            retry_count++;
        }
...
wal_fn_register_bypass(int u)
{
    ...
    if (bcmptm_wal_scf_num_chans[u]) {
        ...
        /* Turn off CCMDMA_WR for BYPASS mode operation. This is needed because
         * WAL may be previously working in 2 thread mode and is now being
         * switched back to operate in BYPASS mode. So, need to re-init
         * schan_fifo optimally for BYPASS mode. */
        SHR_IF_ERR_EXIT(
...
/* Register non_bypass mode related wal functions. */
static int
wal_fn_register_non_bypass(int u)
{
    SHR_FUNC_ENTER(u);

    cfg_wal_fn_ptr_all[u].fn_wal_write = &wal_write_normal;
    cfg_wal_fn_ptr_all[u].fn_wal_trans_cmd = &wal_trans_cmd_normal;
    cfg_wal_fn_ptr_all[u].fn_wal_mreq_state_update = &wal_mreq_state_update;

    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(u, "\nWill operate WAL in Two_Thread "
                    "(normal) mode\n")));

    /* In case WAL was previously in BYPASS mode and now is switched to 2 thread
     * mode, CCMDMA_WR must be enabled, if permitted, for better performance.
     * Hence, re_init schanfifo settings.
     */
    if (bcmptm_wal_scf_num_chans[u]) {
        SHR_IF_ERR_EXIT(
            bcmbd_schanfifo_init(u,
...
bcmptm_wal_init(int u, bool warm)
{
    ...
    if (cfg_scf_num_chans[u] && !bcmdrd_feature_is_sim(u)) {
        ...
        if (tmp_rv == SHR_E_NONE) {
            bcmptm_wal_scf_num_chans[u] = 0; /* default: no scf */
    ...
    /* mxef_INIT_SEQ:
     * We don't have bcmptm_ptcache_fb.c module yet.
     *
     * Pass scf related info needed to sync PTcache with HW.
     * Caution: PTcache must not change cfg_scf_init_flags !!
 */
    /*
    bcmptm_ptcache_fb_scf_info_set(u, bcmptm_wal_scf_num_chans[u],
                                   cfg_wal_max_words_in_msg[u],
                                   cfg_scf_init_max_polls[u],
                                   cfg_scf_init_flags[u]);
    */
...
int bcmptm_wal_dyn_atomic_trans(int u, bool enable)
{
    ...
    if (enable) {
        ...
        if (cfg_wal_bypass[u]) {
            ...
            SHR_IF_ERR_EXIT(wal_fn_register_non_bypass(u));
            LOG_VERBOSE(BSL_LOG_MODULE,
            ...
            /* We are already in 2 thread - but with atomic_trans disabled */
             ...
             *
             * Case1: We are here due to CTH dynamically requesting enable of
             *        atomic_trans.
             *        In this case, waiting for WAL to become empty is not
             *        needed. We can start adding more ops and let WAL reader
             *        continue with dequeue of previous ops.
             *
             * Case2: We are here for recording of init_seq in WAL during
             *        coldboot with FFB=0 or FFB=1. WAL will be empty in both
             *        cases - refer to comments in caller API.
             */
             ...
             * re-register. CCMDMA_WR setting is already set optimally. */
...
static int
init_seq_record(int u)
{
    SHR_FUNC_ENTER(u);

    /* Enable dynamic atomic_trans mode */
    if (wal_disable_undo[u]) {
        /* Atomic_trans are disabled - we may be in BYPASS or non_BYPASS
         * mode.
         *
         * - For coldboot case, WAL is empty.
         *
         * - To start FFB, user must first do normal shutdown
         *   and then indicate that FFB is needed during re-attach.
         *   So, PTM will see a coldboot sequence FFB=1 (so WAL will be
         *   empty) and behavior will remain same during comp_config phase.
         *
         * - In summary, WAL will be empty for coldboot sequence with FFB=0
         *   or FFB =1.
         */

        SHR_IF_ERR_EXIT(bcmptm_wal_dyn_atomic_trans(u, TRUE));
    } /* else {
       *    - we are already operating in atomic_trans enabled mode.
       *    - As explained before, WAL will be empty for both coldboot and
       *      fastboot cases.
       * }
       */

    /* Remember we are in init_seq */
    record_iseq[u] = TRUE;

    /* Notes
     *
     * Refer to notes under bcmptm_wal_cfg_set() API for coldboot, warmboot
     * behavior clarifications
     *
     * Behavior of standard WAL APIs when record_iseq == T.
     *
     * cseq_id_check
     *      - Is disabled.
     *
     * mreq_state_update
     *      - Setting of wstate->mreq_state_in_trans, recording of in_cseq_id
     *        in wstate->mreq_cseq_id is disabled.
     *
     * Writes
     *      - Are accumulated in WAL. Reuse wal_write_normal but with following
     *        differences:
     *          - Allow non-cacheable SIDs to be initialized through modeled
     *            path - can be done by not maintaining undo ops in PTcache.
     *            But,this takes away 'PTcache undo on abort' capability. Only
     *            way to restore PTcache to default state (reset values of SIDs)
     *            is to do normal shutdown followed by coldboot or FFB.
     *          - cmd_hdr for ops is stuffed on WAL writer side (messages
     *            created during init_seq record will have cmd_hdr_stuffed=T.
     *            This is because when capturing init_seq, device is switching
     *            traffic based on old config - and when traffic is halted - we
     *            need to send the recorded init_seq in WAL (and accumulated ops
     *            in PTcache) ASAP to HW.
     *          - Caller of bcmptm_wal_write is expected to update PTcache.
     * Reads (from HW)
     *      - Should not be allowed when capturing init_seq.
     *      - For now:
     *          - Caller can call bcmptm_wal_read, followed by
     *            bcmptm_wal_trans_cmd(commit).
     *            - bcmptm_wal_read(), wal_empty_make() APIs can be used without
     *              any changes.
     * Aborts
     *      - Can use existing bcmptm_wal_abort() as is except when we see
     *        abort in IN_MSG, NEED_NEW_MSG states. This should not happen
     *        - but can happen because WAL is full or some other reason.
     *        Irrespective of the reason, we cannot recover from this as we
     *        cannot undo PTcache changes !!! At present, we return
     *        SHR_E_INTERNAL if abort is invoked when record_iseq is T.
     *
     *      - We cannot be in IDLE_AFTER_WR state when capturing init_seq.
     *
     * Commits
     *      - if state is IN_MSG, NEED_NEW_MSG:
     *          - Will ignore commits - to concatenate ops for
     *            entire init_seq in one commit_seq. When record_iseq == T, such
     *            commits can come from:
     *              - callers from scor.c, ireq.c who issued bcmptm_wal_read
     *              - callers from bcmdi.fcload which tries to pack Flexcode
     *                write ops into msgs.
     *
     *            Must return notification if requested (bcmdi.fcload issues ops on
     *            modeled path, issues commits, and  expects callback).
     *            during init).
     *
     *      - if state is BLOCKING_READ, READ_AFTER_WRITE:
     *          - Will process the commit - because caller needs the read_data
     *            from HW. This will make WAL empty and change the state to
     *            IDLE.
     *
     *      - if state is IDLE
     *          - WAL is empty, so return notification if requested. Not sure if
     *            this can happen.
     *
     *      - if state is IDLE_AFTER_WR
     *          - we cannot be here when record_iseq == T
     *
     * Note:
     *      - How will RMs update their SW state? don't we need mreq_trans_cmd?
     *        Does bcmptm_sysm need to do this by calling mreq API?
     *        Should NOT need this - because when capturing init_seq - RMs are
     *        initializing their state and as part of this are updating HW as
     *        needed.
     *
     * mxef_INIT_SEQ
     *      - Can we have SER event during HW read and when record_iseq == T?
 */
exit:
    SHR_FUNC_EXIT();
}
...
/* mxef_INIT_SEQ:
 * Following is written assuming that init_seq_send is called after
 * init_seq_record.
 *
 * Must revisit to see if changes are needed when init_seq_send is
 * called after init_seq_ignore.
 */
static int
init_seq_send(int u)
{
    uint64_t flags = 0;
    uint32_t in_cseq_id = 0;
    SHR_FUNC_ENTER(u);

    /* When here, WAL must be in 2 thread mode, atomic_trans are enabled either
     * permanently or enabled dynamically - and wal_disable_undo[u]
     * must be False.
     */
    SHR_IF_ERR_EXIT(
        wal_disable_undo[u] || !record_iseq[u] ? SHR_E_INTERNAL : SHR_E_NONE);

    /* Normal functionality is gated by record_iseq.
     * Examples: commits are ignored.
     * Must de-assert this to be able to send the init_seq ops to HW.
     */
    record_iseq[u] = FALSE;


    /* Commit the transaction to send msgs for accumulated ops to WAL
     * reader. Must not use bcmptm_wal_trans_cmd to avoid getting cseq_id
     * mismatch errors.
     *
     * WAL reader will send accumulated ops to HW. If no schan_fifo, WAL
     * reader will use PIO to send ops to HW.
     */
    SHR_IF_ERR_EXIT(
        bcmptm_wal_commit(u, flags,
                          TRUE, /* mreq_state_in_trans */
                          in_cseq_id,
                          NULL, NULL));

    /* Wait for wal empty. */
    SHR_IF_ERR_EXIT(bcmptm_wal_drain(u, TRUE));
        /* trans_state must be IDLE when wal becomes empty. */
     ...
     * also cleared contents of trans pointed by wstate->c_trans_idx (if
     * last_msg was asserted.
     *      - wstate->a_vars are all useless when in IDLE state
     *        everything has already been committed and sent to HW
     *      - nothing to undo
     */

    /* If we had dynamically enabled atomic_trans mode to capture init_seq, we
     * need to revert to normal configured mode.
     */
    if (wal_dyn_enable_atomic_trans[u]) {
        SHR_IF_ERR_EXIT(
            (wstate[u]->mreq_state_in_trans ||
             (wstate[u]->c_trans_state != TRANS_STATE_IDLE)) ?
            SHR_E_INTERNAL : SHR_E_NONE);
        ...
        if (prev_cfg_wal_bypass[u]) {
            ...
            /* Re-register non_bypass mode related wal functions - just in case
             * we were in ignore_mode before sending the ops. */
            wal_fn_register_non_bypass(u);

            /* Revert disable_undo */
            wal_disable_undo[u] = TRUE;
        }

        /* Revert cfg_wal_bypass vars */
        cfg_wal_bypass[u] = prev_cfg_wal_bypass[u];
        wal_dyn_enable_atomic_trans[u] = FALSE;

    } else {
        /* Normal configured mode was atomic_trans enabled - both cfg_wal_bypass
         * and wal_dyn_enable_atomic_trans were already FALSE */
        SHR_IF_ERR_EXIT(wal_disable_undo[u] ? SHR_E_INTERNAL : SHR_E_NONE);
    ...
    /* Revert the any other WAL state as needed to resume normal mode. */
    ignore_mode[u] = FALSE;
exit:
    SHR_FUNC_EXIT();
}

/* Assumes: wal_init has already happened.
 */
int bcmptm_wal_init_seq(int u, bool record, bool send, bool ignore)
{
    SHR_FUNC_ENTER(u);

    SHR_IF_ERR_EXIT(!record && !send && !ignore ? SHR_E_PARAM : SHR_E_NONE);

    if (record) {
        SHR_IF_ERR_EXIT(ignore || send ? SHR_E_PARAM : SHR_E_NONE);

        /* Note: recording will not be requested by bcmptm_sysm if warm == T */
        SHR_IF_ERR_EXIT(init_seq_record(u));

    } else if (ignore) {
        SHR_IF_ERR_EXIT(record || send ? SHR_E_PARAM : SHR_E_NONE);

        /* mxef_INIT_SEQ: Not yet implemented. */
        /* record_iseq[u] = FALSE; ?? */
        SHR_ERR_EXIT(SHR_E_UNAVAIL);

    } else { /* send */
        SHR_IF_ERR_EXIT(record || ignore ? SHR_E_PARAM : SHR_E_NONE);
        SHR_IF_ERR_EXIT(init_seq_send(u));
    }
exit:
    SHR_FUNC_EXIT();
}
 ...
 *              the meantime SERC event is waiting to be serviced. Improvement.
 */
