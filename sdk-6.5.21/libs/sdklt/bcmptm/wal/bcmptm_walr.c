...
walr_stuff_cmd_hdr(int u, bcmptm_wal_msg_t *walr_msg)
{
    ...
    /* Done - if cmd_hdr is already stuffed by WAL writer */
    if (walr_msg->cmd_hdr_stuffed) {
        SHR_EXIT();
    }
...
walr_msg_cmd_slam_exe(int u, bcmptm_wal_msg_t *walr_msg)
{
    ...
    buf_paddr = bcmptm_wal_words_buf_bpa[u] + (4*(walr_msg->words_buf_idx));
    if (buf_paddr & 0xF) {
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    }
    ops_info_array = bcmptm_wal_ops_info[u] + walr_msg->ops_info_idx;
    ...
    /* send ops to HW */
    do {
        tmp_rv = bcmbd_pt_range_write(u, ops_info_array->sid,
                                      &(ops_info_array->dyn_info),
                                      NULL, /* pt_ovrr_info */
                                      walr_msg->num_ops, /* num_entries */
                                      bcmptm_wal_words_buf[u] +
                                      walr_msg->words_buf_idx,
                                      buf_paddr, /* physical 64b dma_addr */
                                      flags);
        if (tmp_rv != SHR_E_NONE) {
            LOG_ERROR(BSL_LOG_MODULE,
                (BSL_META_U(u, "bcmptm_walr.bcmbd_pt_range_write"
