...
{
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(uft_var_drv[unit], SHR_E_NONE);
...
sbr_var_drv_cleanup(int unit)
{
    ...
    SHR_NULL_CHECK(sbr_var_drv[unit], SHR_E_NONE);
                           ...
                           bcmptm_wo_aggr_info_t *wo_aggr_info)
{
    ...
    if ((my_fn = fn_ptr_all[unit]->fn_pt_wo_aggr_info_get) == NULL) {
        return SHR_E_NOT_FOUND; /* This sid is not wo_aggr */
    } else {
...
bcmptm_rm_tcam_prio_only_shared_sid_compare(int unit,
                                          ...
                                          bool *compare)
{
    fn_pt_rm_tcam_prio_only_shared_sid_compare_t my_fn;

    if ((my_fn = fn_ptr_all[unit]->fn_pt_rm_tcam_prio_only_shared_sid_compare) == NULL) {
        return SHR_E_UNAVAIL;
    } else {
        return my_fn(unit, sid, shr_sid, neg_shr_flags, compare);
    }
}
...
int
bcmptm_pt_cci_link_status_data_get(int unit,bcmdrd_sid_t *xlmib_rxsid,
                            bcmdrd_sid_t *xlmib_txsid, int *rxpkt_fieldnum,
                            int *txpkt_fieldnum)
{
    fn_pt_cci_link_status_data_get_t my_fn;

    if ((my_fn = fn_ptr_all[unit]->fn_pt_cci_link_status_data_get) == NULL) {
        return SHR_E_UNAVAIL;
    } else {
        my_fn(unit, xlmib_rxsid, xlmib_txsid, rxpkt_fieldnum, txpkt_fieldnum);
        return SHR_E_NONE;
    }
}

void
bcmptm_pt_cci_link_status_update(int unit, uint32_t index, uint32_t instance,
                           int rx, uint64_t count)
{
    fn_pt_cci_link_status_update_t my_fn;

    if ((my_fn = fn_ptr_all[unit]->fn_pt_cci_link_status_update) == NULL) {
        return;
    } else {
        my_fn(unit, index, instance, rx, count);
    }
}
...
{
    fn_pt_rm_tcam_update_flex_ctr_action_t my_fn;

    if ((my_fn = fn_ptr_all[unit]->fn_pt_rm_tcam_update_flex_ctr_action) == NULL) {
        return SHR_E_NONE;
    } else {
        return my_fn(unit, ltid_info, ctr_action_prof_id, policy_words);
    }
}

void
bcmptm_pt_cci_ctr_direct_access(int unit,
                                bcmptm_cci_ctr_info_t *ctr_info)
{
    fn_pt_cci_ctr_direct_access_t my_fn;

    if ((my_fn = fn_ptr_all[unit]->fn_pt_cci_ctr_direct_access) != NULL) {
        my_fn(unit, ctr_info);
    }
}

int
bcmptm_pt_cci_ctr_ing_pfc_field_val_get(int unit,
                                        ...
                                        size_t size_ctr)
{
    fn_pt_cci_ctr_ing_pfc_field_val_get_t my_fn;

    if ((my_fn = fn_ptr_all[unit]->fn_pt_cci_ctr_ing_pfc_field_val_get) == NULL) {
        return SHR_E_UNAVAIL;
    } else {
        return my_fn(unit, ctr_info, sid, sw_ctr, buf, size_ctr);
    }
}
...
bcmptm_rm_tcam_ser_corr_sid_info_get(int unit,
                                 ...
                                 uint8_t *sid_count)
{
    fn_pt_rm_tcam_ser_corr_sid_info_get_t my_fn;

    if ((my_fn = fn_ptr_all[unit]->fn_pt_rm_tcam_ser_corr_sid_info_get) == NULL) {
        return SHR_E_NONE;
    } else {
        return my_fn(unit,
                     sid,
