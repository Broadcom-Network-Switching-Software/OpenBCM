/*! \file bcmptm_bcm56780_a0_cci.c
 *
 * This file contains the misc utils for bcm56780_a0's CCI
 */
/*
...
#define NORMAL_DOUBLE_MODE_WIDTH   16
#define MIB_ROW_PER_PORT           16
#define MIB_MEMORY_ROW_FIELDS      8
#define MIB_MEMORY_ROW_12          12
#define LED_WAR_XLMIB_PORT0        161
#define LED_WAR_XLMIB_PORT1        162
#define BITS_PER_WORD              32
#define BYTES_PER_WORD             4
#define CDPORTS_PER_INSTANCE       8
#define LED_WAR_RX_PCI_ADDR        0x1307DE0
#define LED_WAR_TX_PCI_ADDR        0x1307DC0
#define CDMIB_TXPKT_FIELDNUM       4
#define CDMIB_RXPKT_FIELDNUM       0
...
typedef struct ctr_ing_pfc_map_s {
    bcmdrd_sid_t sid;
    uint32_t row;
    uint32_t field;
    uint32_t width;
...
static ctr_evict_t ctr_eviction[] = {
    ...
    {FLEX_CTR_ST_EGR0_COUNTER_TABLE_0m, FLEX_CTR_ST_EGR0_COUNTER_UPDATE_CONTROL_POOL_0r, 1},
    {FLEX_CTR_ST_EGR0_COUNTER_TABLE_1m, FLEX_CTR_ST_EGR0_COUNTER_UPDATE_CONTROL_POOL_1r, 1},
    {FLEX_CTR_ST_EGR0_COUNTER_TABLE_2m, FLEX_CTR_ST_EGR0_COUNTER_UPDATE_CONTROL_POOL_2r, 1},
    {FLEX_CTR_ST_EGR0_COUNTER_TABLE_3m, FLEX_CTR_ST_EGR0_COUNTER_UPDATE_CONTROL_POOL_3r, 1},
    {FLEX_CTR_ST_EGR1_COUNTER_TABLE_0m, FLEX_CTR_ST_EGR1_COUNTER_UPDATE_CONTROL_POOL_0r, 1},
    {FLEX_CTR_ST_EGR1_COUNTER_TABLE_1m, FLEX_CTR_ST_EGR1_COUNTER_UPDATE_CONTROL_POOL_1r, 1},
    {FLEX_CTR_ST_EGR1_COUNTER_TABLE_2m, FLEX_CTR_ST_EGR1_COUNTER_UPDATE_CONTROL_POOL_2r, 1},
    {FLEX_CTR_ST_EGR1_COUNTER_TABLE_3m, FLEX_CTR_ST_EGR1_COUNTER_UPDATE_CONTROL_POOL_3r, 1},
};
...
;
/* List of SIDs which can support direct access counter */
static const ctr_ing_pfc_map_t ctr_ing_pfc_map[] = {
    {XLMIB_RPFC0r, 4, 4, 40},
    {XLMIB_RPFC1r, 4, 6, 40},
    {XLMIB_RPFC2r, 5, 0, 40},
    {XLMIB_RPFC3r, 5, 2, 40},
    ...
    {XLMIB_RPFC6r, 6, 0, 40},
    {XLMIB_RPFC7r, 6, 2, 40},
    {XLMIB_RPFCOFF0r, 4, 5, 40},
    {XLMIB_RPFCOFF1r, 4, 7, 40},
    {XLMIB_RPFCOFF2r, 5, 1, 40},
    {XLMIB_RPFCOFF3r, 5, 3, 40},
    {XLMIB_RPFCOFF4r, 5, 5, 40},
    {XLMIB_RPFCOFF5r, 5, 7, 40},
    {XLMIB_RPFCOFF6r, 6, 1, 40},
    {XLMIB_RPFCOFF7r, 6, 3, 40},
};
static bool
                               ...
                               bcmdrd_sid_t sid)
{
    ...
    do {
        if (port) {
            ...
            rv = CCI_CTR_TYPE_TM;
        } else if (blktype == BLKTYPE_MACSEC) {
            rv = CCI_CTR_TYPE_SEC;
        }
    } while(0);
                               ...
                               bcmptm_cci_ctr_info_t *ctr_info)
{
    ...
    do {
        ...
        rv = bcm56780_a0_pt_cci_frmt_index_from_port(unit, ctr_info->sid,
...
/*
 * War for led processor xlmib rxpkt and txpkt mib sid is retrived
 * War for led processor fieldnum for rxpkt and txpkt is retrived for cdmib.
 */
...
bcm56780_a0_pt_cci_link_status_data_get(int unit, bcmdrd_sid_t *xlmib_rxsid,
                                        ...
                                        int *txpkt_fieldnum)
{
    *xlmib_rxsid = XLMIB_RPKTr;
    *xlmib_txsid = XLMIB_TPKTr;
    *rxpkt_fieldnum = CDMIB_RXPKT_FIELDNUM;
    *txpkt_fieldnum = CDMIB_TXPKT_FIELDNUM ;
}

/*
 * War for led processor tx and rx status is written
 * in specified memory for led processor.
 */
void
bcm56780_a0_pt_cci_link_status_update(int unit,
                                               uint32_t index,
                                               uint32_t instance,
                                               int      rx,
                                               uint64_t count)
...
{
    uint32_t rx_pci_addr = LED_WAR_RX_PCI_ADDR;
    uint32_t tx_pci_addr = LED_WAR_TX_PCI_ADDR;
    uint32_t val = 0x0;
    int port_offset = 0;

    if (instance == LED_WAR_XLMIB_PORT0 || instance == LED_WAR_XLMIB_PORT1 ) {
        port_offset = (instance - 1);
    }  else {
       port_offset =   (instance * CDPORTS_PER_INSTANCE + (index / MIB_ROW_PER_PORT));
    }
    if (rx) {
        /* Rx status */
        if (instance == LED_WAR_XLMIB_PORT0 || instance == LED_WAR_XLMIB_PORT1) {
            rx_pci_addr = rx_pci_addr + (((instance - 1) / BITS_PER_WORD) * BYTES_PER_WORD);
        } else {
            rx_pci_addr = rx_pci_addr + ((instance / BYTES_PER_WORD) * BYTES_PER_WORD);
        }
        (void)bcmdrd_hal_iproc_read(unit, rx_pci_addr, &val);
        if (count) {
            val = val | (1 << (port_offset % BITS_PER_WORD));
        } else {
            val = val & ~(1 << (port_offset % BITS_PER_WORD));
        }
        (void)bcmdrd_hal_iproc_write(unit, rx_pci_addr, val);
    } else  {
        if (instance == LED_WAR_XLMIB_PORT0 || instance == LED_WAR_XLMIB_PORT1 ) {
            tx_pci_addr = tx_pci_addr + ((instance / BITS_PER_WORD) * BYTES_PER_WORD);
       } else {
            tx_pci_addr += ((instance / BYTES_PER_WORD) * BYTES_PER_WORD);
       }
       (void)bcmdrd_hal_iproc_read(unit, tx_pci_addr, &val);
       if (count) {
           val = val | (1 << (port_offset % BITS_PER_WORD));
       } else {
           val = val & ~(1 << (port_offset % BITS_PER_WORD));
       }
       (void)bcmdrd_hal_iproc_write(unit, tx_pci_addr, val);
                                            ...
                                            size_t *width)
{
    ...
    break;
    case NORMAL_DOUBLE_MODE:
        *width = NORMAL_DOUBLE_MODE_WIDTH;
    break;
    default:
        rv = SHR_E_PARAM;
    break;
                                            ...
                                            bcmptm_cci_ctr_info_t *info)
{
    ...
    if (info->sid == EPOST_EGR_PERQ_XMT_COUNTERS_0m) {
        ...
        SHR_IF_ERR_EXIT(
            bcmdrd_pt_field_set(unit, info->sid, pool, COUNTER_B_OR_WIDE_COUNTER0f, val));
    }
    val[0] = val[1] = 0;
    ...
    if (info->sid == EPOST_EGR_PERQ_XMT_COUNTERS_0m) {
        ...
        SHR_IF_ERR_EXIT(
            bcmdrd_pt_field_set(unit, info->sid, pool, COUNTER_A_OR_WIDE_COUNTER1f, val));
    }
...
bcm56780_a0_pt_cci_ctr_direct_access(int unit,
                                     bcmptm_cci_ctr_info_t *ctr_info)
{
    ctr_info->direct_access = false;

    if (ctr_info->req_ltid == CTR_ING_PFCt) {
        ctr_info->direct_access = true;
    }
}
int
bcm56780_a0_pt_cci_ctr_ing_pfc_field_val_get(int unit,
                                             ...
                                             size_t size_ctr)
{
    size_t size = COUNTOF(ctr_ing_pfc_map);
    uint32_t i;
    bool ctr_ing_pfc_field = false;
    bcmdrd_pbmp_t pbmp;
    ...
    for (i = 0; i < size; i++) {
        /* check if matches with the list */
        if ((sid == ctr_ing_pfc_map[i].sid) &&
            (size_ctr > ctr_ing_pfc_map[i].width)) {
            ctr_ing_pfc_field = true;
            break;
        }
    }

    if (ctr_ing_pfc_field == false) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    /* Each MIB field occupies 64bits, offset = field index * 2 */
    /* Extract lower 32bits value */
    *buf = *(buf + ctr_ing_pfc_map[i].field * 2);
    /* Extract higher 32bits value */
    *(buf + 1) = *(buf + ctr_ing_pfc_map[i].field * 2 + 1);
    /*
     * In order to handle the rollover case on HW outside of counter width,
     * cache counter HI(40bits) used to be OR on HW counter value
     */
    *(buf + 1) |= ((sw_ctr >> ctr_ing_pfc_map[i].width) << 8);
