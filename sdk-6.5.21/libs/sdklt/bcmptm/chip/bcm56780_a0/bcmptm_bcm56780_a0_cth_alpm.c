...
bcm56780_fp_combined_rpf_blocks[16][4] = {  /* DB0 DB1 DB2 DB3 */
    ...
    {IPDB0, IPDB0, IPDB2, IPDB2},           /*  1   0   1   0  */
    {IPDB0, IPDB2, IPDB0, IPDB3},           /*  1   0   1   1  */
    {IPDB0, IPDB0, IPDB1, IPDB1},           /*  1   1   0   0  */
    {IPDB0, IPDB0, IPDB1, IPDB3},           /*  1   1   0   1  */
    {IPDB0, IPDB2, IPDB1, IPDB0},           /*  1   1   1   0  */
    {IPDB0, IPDB2, IPDB1, IPDB3}            /*  1   1   1   1  */
};
...
static bool
bcm56780_a0_alpm_is_regular_pdd_type(uint32_t pdd_type)
{
    switch (pdd_type) {
    case EM_TILE_ALPM_PDD_TYPE_L3UC_DST:
    case EM_TILE_ALPM_PDD_TYPE_L3UC_SRC:
    case EM_TILE_ALPM_PDD_TYPE_COMP_DST:
    case EM_TILE_ALPM_PDD_TYPE_COMP_SRC:
        return true;
...
static int
bcm56780_a0_cth_alpm_block_pdd_types(int u, int m, bool raw,
                                     uint32_t pdd_count,
                                     uint32_t *pdd_types)
{
    uint32_t i;
    uint32_t actual_count;
    uint32_t pdd_types1[BCM56780_ALPM_L3_BLOCKS];
    uint32_t actual_count1;
    SHR_FUNC_ENTER(u);
    ...
    if (actual_count == 0) {
        for (i = 0; i < BCM56780_ALPM_L3_BLOCKS; i++) {
            pdd_types1[i] = EM_TILE_ALPM_PDD_TYPE_NONE;
        }
    } else if (actual_count != BCM56780_ALPM_L3_BLOCKS) {
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }

    if (m == ALPM_0 && !raw) {
        SHR_IF_ERR_EXIT(
            bcmptm_uft_alpm_pdd_type_get(u, pdd_count, ALPM_1,
                                         &pdd_types1[0],
                                         &actual_count1));
        if (actual_count1 == 0) {
            for (i = 0; i < BCM56780_ALPM_L3_BLOCKS; i++) {
        ...
        } else if (actual_count1 != BCM56780_ALPM_L3_BLOCKS) {
            SHR_ERR_EXIT(SHR_E_NOT_FOUND);
        }
        for (i = 0; i < actual_count; i++) {
            if (pdd_types[i] == EM_TILE_ALPM_PDD_TYPE_NONE &&
                bcm56780_a0_alpm_is_regular_pdd_type(pdd_types1[i])) {
                pdd_types[i] = pdd_types1[i];
            }
        }
    }
...
static void
bcm56780_a0_cth_alpm_destination_set(int u, int m,
                                     bcmptm_cth_alpm_control_t *ctrl,
                                     bcm56780_lpm_ip_control_t *ip_ctrl)
{

    if (ip_ctrl->result_mode == FOUR_RESULTS_MODE) {
        ...
        ip_ctrl->destination[2] = ctrl->destination;
        ip_ctrl->destination_mask[2] = ctrl->destination_mask;
        ip_ctrl->destination_type_match[2] = ctrl->destination_type_match;
        ip_ctrl->destination_type_non_match[2] = ctrl->destination_type_non_match;

        ip_ctrl->destination[3] = ctrl->destination;
        ip_ctrl->destination_mask[3] = ctrl->destination_mask;
        ip_ctrl->destination_type_match[3] = ctrl->destination_type_match;
        ip_ctrl->destination_type_non_match[3] = ctrl->destination_type_non_match;
    } else if (ip_ctrl->result_mode == THREE_RESULTS_MODE) {
        ip_ctrl->destination[0] = ctrl->destination;
        ip_ctrl->destination_mask[0] = ctrl->destination_mask;
        ip_ctrl->destination_type_match[0] = ctrl->destination_type_match;
        ip_ctrl->destination_type_non_match[0] = ctrl->destination_type_non_match;
        ...
        ip_ctrl->destination_type_match[3] = ctrl->destination_type_match;
        ip_ctrl->destination_type_non_match[3] = ctrl->destination_type_non_match;
    } else if (ip_ctrl->result_mode == TWO_RESULTS_MODE) {
        ip_ctrl->destination[0] = ctrl->destination;
        ip_ctrl->destination_mask[0] = ctrl->destination_mask;
        ip_ctrl->destination_type_match[0] = ctrl->destination_type_match;
        ip_ctrl->destination_type_non_match[0] = ctrl->destination_type_non_match;
                                  ...
                                  int *rv)
{
     ...
     *  +-----+---------+
 */
    *rv = bcm56780_a0_cth_alpm_l1_parse_check(u, m, ctrl->tot.num_l1_banks,
                                              &ctrl->l1_key_input[0],
                                              &ctrl->l1_db[0],
                                              ctrl->db0_levels,
                                              ctrl->db1_levels,
    ...
    *rv = bcm56780_a0_cth_alpm_block_pdd_types(u, m, false, BCM56780_ALPM_L3_BLOCKS, &pdd_types[0]);
    if (SHR_FAILURE(*rv)) {
    ...
    bcm56780_a0_cth_alpm_destination_set(u, m, ctrl, ip_ctrl);
                                         ...
                                         cth_alpm_control_pt_t *pts)
{
    ...
    sal_memcpy(&ce, pts->entry[4], sizeof(ce));
    ...
    ce0 = ce;
    M_LPM_IP_CONTROLm_M_LANE_MODEf_SET(ce0, 0);

    /* Level 3 shared banks */
    M_E2T_00_SHARED_BANKS_CONTROLm_BANK_BYPASS_LPf_SET(sbc0,
            (ctrl->tot.l3_bank_bitmap & 0xff) ? 0xff : 0);
    bcm56780_a0_cth_alpm_block_pdd_types(u, m, true, BCM56780_ALPM_L3_BLOCKS, &pdd_types[0]);
     ...
     * bit1: e2t_03
     */
    pdd_lane_bmp = (bcm56780_a0_alpm_is_regular_pdd_type(pdd_types[0]) ||
                    bcm56780_a0_alpm_is_regular_pdd_type(pdd_types[1])) |
                   (bcm56780_a0_alpm_is_regular_pdd_type(pdd_types[2]) ||
                    bcm56780_a0_alpm_is_regular_pdd_type(pdd_types[3])) << 1;
    ...
    pts->sid[4] = M_LPM_IP_CONTROLm;
    sal_memcpy(pts->entry[0], &ce0, sizeof(ce0));
    ...
    sal_memcpy(pts->entry[4], &ce, sizeof(ce));
                                    ...
                                    cth_alpm_control_pt_t *pts)
{
    ...
    IFTA90_E2T_00_SHARED_BANKS_CONTROLm_BANK_BYPASS_LPf_SET(sbc0,
            (ctrl->tot.l3_bank_bitmap & 0xf) ? 0xf : 0);
    IFTA90_E2T_01_SHARED_BANKS_CONTROLm_BANK_BYPASS_LPf_SET(sbc1,
            (ctrl->tot.l3_bank_bitmap & 0xf0) ? 0xf : 0);
    bcm56780_a0_cth_alpm_block_pdd_types(u, m, true, BCM56780_ALPM_L3_BLOCKS, &pdd_types[0]);
    ...
    bcm56780_a0_cth_alpm_block_pdd_types(u, ALPM_1, true, BCM56780_ALPM_L3_BLOCKS, &pdd_types[0]);
    pdd_lane_bmp = (bcm56780_a0_alpm_is_regular_pdd_type(pdd_types[0]) ||
                    bcm56780_a0_alpm_is_regular_pdd_type(pdd_types[1])) |
                   (bcm56780_a0_alpm_is_regular_pdd_type(pdd_types[2]) ||
                    bcm56780_a0_alpm_is_regular_pdd_type(pdd_types[3])) << 1;
...
bcm56780_a0_key_mux_check_against_pdd_type(int u, int m, bcm56780_lpm_ip_control_t *lpm_ctrl)
{
    ...
    bcm56780_a0_cth_alpm_block_key_muxes(u, m, BCM56780_ALPM_L1_KEY_MUXES, &key_muxes[0]);
    bcm56780_a0_cth_alpm_block_pdd_types(u, m, false, BCM56780_ALPM_L3_BLOCKS, &pdd_types[0]);
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (lpm_ctrl->data_sel[i] != IPDBX) {
            for (j = 0; j < 4; j++) {
                if (pdd_types[i] == key_muxes[j]) { /* KEY MUX must match PDD. */
                    break;
                }
            }
            if (j == 4) {
                                             ...
                                             bcmptm_cth_alpm_control_t *ctrl)
{
    ...
    /*
     * The l1 parse check is assured in stage1 with considering l1 banks,
     * so we need to do same check again in stage2 which is added specifically
