...
static int bcm56780_l2p_route_fmts[] =
    {0, 13, 1, 2, 3, 4, 5, 6, 7, 8, 9, 14, 10, 11, 15, 12, 16, 17, 18, 19, 20,
     21, 22, 23, 24};

/* Mapping physical formats to logical formats. */
static int bcm56780_p2l_route_fmts[] =
    {0, 2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 13, 15, 1, 11, 14, 16, 17, 18, 19, 20,
     21, 22, 23, 24};
...
static format_info_t bcm56780_route_base_entry_info[] = {
    ...
    {.prefix_len = 111, .full = 1, .num_entries = 3},   /* 14 - 15 */
    {.prefix_len = 185, .full = 1, .num_entries = 2},   /* 15 - 12 */
...
static void
bcm56780_l1_lpm_vt_decode(int u, int m, int sub_prio, alpm_arg_t *arg)
{
    ...
    arg->lpm_large_vrf = bcmptm_rm_alpm_is_large_vrf(u, m, arg->key.vt);
}

static void
bcm56780_l1_lpm_wvrf_decode(int u, int m, int sub_prio, int vrf,
                              ...
                              L3_DEFIP_TCAM_KEY_t *kdata)
{
    ...
    if (info->lpm_ipv4->dup_ip_key_format) {
        dup_part = bcmdrd_field32_get(&ip_addr,
                                      info->lpm_ipv4->dup_start_bit_logical, /* 0 */
                                      info->lpm_ipv4->dup_end_bit_logical); /* 1 */
        bcmdrd_field_set((uint32_t *) &lpm_v4.v[0],
                         info->lpm_ipv4->dup_start_bit_physical, /* 182 */
                         info->lpm_ipv4->dup_end_bit_physical, /* 183 */
                         &dup_part);
    }
                                     ...
                                     L3_DEFIP_TCAM_KEY_t *kdata)
{
    ...
    if (info->lpm_ipv4->dup_ip_key_format) {
        bcmdrd_field_get(key,
                         138,
                         138 + info->lpm_ipv4->dup_end_bit_physical -
                         info->lpm_ipv4->dup_start_bit_physical, /* 139 */
                         &part_1[0]);
        bcmdrd_field_get(key,
                         info->lpm_ipv4->dup_end_bit_physical -
                         info->lpm_ipv4->dup_start_bit_physical + 139,  /* 140 */
                         info->lpm_ipv4->dup_end_bit_physical, /* 183 */
                         &part_2[0]);
        bcmdrd_field_set(key,
                         info->lpm_ipv4->dup_start_bit_physical, /* 182 */
                         info->lpm_ipv4->dup_end_bit_physical, /* 183 */
                         &part_1[0]);
        bcmdrd_field_set(key,
                         138,
                         info->lpm_ipv4->dup_start_bit_physical - 1, /* 181 */
                         &part_2[0]);
    }
                                   ...
                                   L3_DEFIP_TCAM_KEY_t *kdata)
{
    ...
    bcmdrd_field_set((uint32_t *) &lpm_v6.v[0],
                     ...
                     info->lpm_ipv6->ip_addr_end_bit,
                     ip_addr);
    if (info->lpm_ipv6->dup_ip_key_format) {
        dup_part = bcmdrd_field32_get(ip_addr,
                                      info->lpm_ipv6->dup_start_bit_logical, /* 96 */
                                      info->lpm_ipv6->dup_end_bit_logical); /* 105 */
        bcmdrd_field_set((uint32_t *) &lpm_v6.v[0],
                         info->lpm_ipv6->dup_start_bit_physical, /* 174 */
                         info->lpm_ipv6->dup_end_bit_physical, /* 183 */
                         &dup_part);
    }
                                          ...
                                          L3_DEFIP_TCAM_KEY_t *kdata)
{
    ...
    if (info->lpm_ipv6->dup_ip_key_format) {
        /* Move [137:128] to [183:174] */
        bcmdrd_field_get(udata,
                         137 - info->lpm_ipv6->dup_end_bit_physical +
                         info->lpm_ipv6->dup_start_bit_physical, /* 128 */
                         137,
                         &part_1[0]);

        bcmdrd_field_set(udata,
                         info->lpm_ipv6->dup_start_bit_physical, /* 174 */
                         info->lpm_ipv6->dup_end_bit_physical, /* 183 */
                         &part_1[0]);
                              ...
                              L3_DEFIP_TCAM_KEY_t *kdata)
{
    ...
    if (ip_addr) {
        *ip_addr = bcmdrd_field32_get((uint32_t *) &lpm_v4.v[0],
                                      ...
                                      info->lpm_ipv4->ip_addr_end_bit);
        if (info->lpm_ipv4->dup_ip_key_format) {
            dup_part = bcmdrd_field32_get((uint32_t *) &lpm_v4.v[0],
                                          info->lpm_ipv4->dup_start_bit_physical, /* 182 */
                                          info->lpm_ipv4->dup_end_bit_physical); /* 183 */
            bcmdrd_field32_set(ip_addr,
                               info->lpm_ipv4->dup_start_bit_logical, /* 0 */
                               info->lpm_ipv4->dup_end_bit_logical, /* 1 */
                               dup_part);
        }
                                   ...
                                   L3_DEFIP_TCAM_KEY_t *kdata)
{
    ...
    if (ip_addr) {
        bcmdrd_field_get((uint32_t *) &lpm_v6.v[0],
                         ...
                         ip_addr);
        if (info->lpm_ipv6->dup_ip_key_format) {
            dup_part = bcmdrd_field32_get((uint32_t *) &lpm_v6.v[0],
                                          info->lpm_ipv6->dup_start_bit_physical, /* 174 */
                                          info->lpm_ipv6->dup_end_bit_physical); /* 183 */
            bcmdrd_field32_set(ip_addr,
                               info->lpm_ipv6->dup_start_bit_logical, /* 96 */
                              ...
                              uint32_t *entry)
{
    int rv;
    if (format_type == FORMAT_ROUTE) {
        rv = bcm56780_ln_route_format_encode(u, m, format_value, kshift, arg, entry);
    } else {
        rv = bcm56780_ln_pivot_format_encode(u, m, format_value, kshift, arg, entry);
    }
    return rv;
}
...
bcm56780_l1_narrow_lpm_decode_n1(int u, int m, alpm_arg_t *arg, void *e)
{
    ...
    bcm56780_l1_lpm_vt_decode(u, m, sub_prio, arg);
    ...
    if (!arg->key.t.ipv) {
        if (large_vrf) {
            ...
            bcm56780_l1_key_lpm_v4_decode(u, m, &vrf_mask, NULL, NULL,
                                          &arg->key.ip_mask[0], &kmask);
    ...
    bcm56780_l1_lpm_wvrf_decode(u, m, sub_prio, vrf, vrf_mask, arg);
...
bcm56780_device_info_init(int u, int m, alpm_dev_info_t *dev)
{
    ...
    if (dev) {
        ...
        dev->max_key_bits = 184;
        if (info) {
            if (info->lpm_ipv4->dup_ip_key_format) {
                dev->lpm_key_dup_bits[IP_VER_4] =
                    info->lpm_ipv4->dup_end_bit_physical -
                    info->lpm_ipv4->dup_start_bit_physical + 1;
            }

            if (info->lpm_ipv6->dup_ip_key_format) {
                dev->lpm_key_dup_bits[IP_VER_6] =
                    info->lpm_ipv6->dup_end_bit_physical -
                    info->lpm_ipv6->dup_start_bit_physical + 1;
            }
        }
