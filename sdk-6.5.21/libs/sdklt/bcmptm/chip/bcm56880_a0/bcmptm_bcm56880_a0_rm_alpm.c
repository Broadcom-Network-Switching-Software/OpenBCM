                              ...
                              uint32_t *entry)
{
    int rv;
    if (format_type == FORMAT_ROUTE) {
        rv = bcm56880_ln_route_format_encode(u, format_value, kshift, arg, entry);
    } else {
        rv = bcm56880_ln_pivot_format_encode(u, format_value, kshift, arg, entry);
    }
    return rv;
}
...
bcm56880_device_info_init(int u, int m, alpm_dev_info_t *dev)
{
    ...
    if (dev) {
        ...
        if (rv != SHR_E_UNAVAIL) {
            ...
            /* Un-supported field DATA_TYPE in current NPL */
            rv = SHR_E_NONE;
        }
    } else {
