/*! \file bcmptm_bcm56990_a0_rm_tcam.c
 *
...
static const bcmdrd_sid_t bcm56990_a0_list[BCMPTM_RM_TCAM_SID_COUNT] = {
 ...
 */
static int
bcm56990_a0_rm_tcam_memreg_read(int unit,
                              bcmltd_sid_t ltid,
                              ...
                              bcmptm_rm_tcam_sid_info_t *sid_info)
{
    ...
    if (sid_info->oper_flags & BCMPTM_RM_TCAM_OPER_SLICE_MAP_UPDATE) {
        ...
        for (field_idx = (sid_info->num_slices - 1);
             field_idx >= 0; field_idx--) {
            /*Get the physical slice No. in entry buffer*/
            bcmdrd_field_get(entry_buffer,
                             start_bit,
                             start_bit + vg_offset - 1,
                             &field_val);
            if (sid_info[unit].slice_id == field_val) {
 ...
 */
int
bcm56990_a0_rm_tcam_memreg_oper(int unit,
                              bcmltd_sid_t ltid,
                              bcmptm_rm_tcam_sid_type_t sid_type,
                              ...
                              bcmptm_rm_tcam_sid_info_t *sid_info)
{
    ...
    if (sid_oper == BCMFP_RM_TCAM_SID_OPER_READ) {
        SHR_ERR_EXIT(bcm56990_a0_rm_tcam_memreg_read(unit,
                                                        ltid,
                                                        sid_type,
                                                        sid_info));
    }
    ...
    if (sid_info->oper_flags & BCMPTM_RM_TCAM_OPER_ACTION_PRIO_UPDATE) {
        ...
        for (part = 0; part < sid_info->parts_count; part++) {
            ...
            if (sid_type == BCMPTM_RM_TCAM_EFP_SLICE_CONTROL) {
                ...
                if (selcode->egr_key6_classid_sel >= 0) {
                    ...
                    bcmdrd_field_set(entry_buffer,
                                     (16 * slice_idx) + 9, (16 * slice_idx) + 10,
                                     &field_val);
                }
                /* KEY7 class id Sel */
                if (selcode->egr_key7_classid_sel >= 0) {
                    ...
                    bcmdrd_field_set(entry_buffer,
                                     (16 * slice_idx) + 11, (16 * slice_idx) + 13,
                                     &field_val);
                }
                /* KEY8 class id Sel */
                if (selcode->egr_key8_classid_sel >= 0) {
                    ...
                    bcmdrd_field_set(entry_buffer,
                                     (16 * slice_idx) + 14, (16 * slice_idx) + 15,
                                     &field_val);
                }
        ...
        for (part = 0; part < sid_info->parts_count; part++) {
            ...
            if (sid_type == BCMPTM_RM_TCAM_EFP_SLICE_CONTROL) {
                ...
                /* KEY1 class id Sel */
                bcmdrd_field_get(entry_buffer, 16 * pri_slice_idx,
                                 (16 * pri_slice_idx) + 1, &data);
                bcmdrd_field_set(entry_buffer, 16 * slice_idx,
                                 (16 * slice_idx) + 1, &data);
                /* KEY2 class id Sel */
                bcmdrd_field_get(entry_buffer, (16 * pri_slice_idx) + 2,
                                 (16 * pri_slice_idx) + 4, &data);
                bcmdrd_field_set(entry_buffer, (16 * slice_idx) + 2,
                                 (16 * slice_idx) + 4, &data);
                /* KEY3 class id Sel */
                bcmdrd_field_get(entry_buffer, (16 * pri_slice_idx) + 5,
                                 (16 * pri_slice_idx) + 6, &data);
                bcmdrd_field_set(entry_buffer, (16 * slice_idx) + 5,
                                 (16 * slice_idx) + 6, &data);
                /* KEY4 class id Sel */
                bcmdrd_field_get(entry_buffer, (16 * pri_slice_idx) + 7,
                                 (16 * pri_slice_idx) + 8, &data);
                bcmdrd_field_set(entry_buffer, (16 * slice_idx) + 7,
                                 (16 * slice_idx) + 8, &data);
                /* KEY6 class id Sel */
                bcmdrd_field_get(entry_buffer, (16 * pri_slice_idx) + 9,
                                 (16 * pri_slice_idx) + 10, &data);
                bcmdrd_field_set(entry_buffer, (16 * slice_idx) + 9,
                                 (16 * slice_idx) + 10, &data);
                /* KEY7 class id Sel */
                bcmdrd_field_get(entry_buffer, (16 * pri_slice_idx) + 11,
                                 (16 * pri_slice_idx) + 13, &data);
                bcmdrd_field_set(entry_buffer, (16 * slice_idx) + 11,
                                 (16 * slice_idx) + 13, &data);
                /* KEY8 class id Sel */
                bcmdrd_field_get(entry_buffer, (16 * pri_slice_idx) + 14,
                                 (16 * pri_slice_idx) + 15, &data);
                bcmdrd_field_set(entry_buffer, (16 * slice_idx) + 14,
                                 (16 * slice_idx) + 15, &data);
            } else if (sid_type == BCMPTM_RM_TCAM_EFP_KEY4_DVP_SELECTOR) {
                /* KEY4 DVP Sel */
...
int
bcm56990_a0_rm_tcam_prio_only_hw_key_info_init(int unit,
                            bcmltd_sid_t ltid,
...
int
bcm56990_a0_rm_tcam_update_flex_ctr_action(int unit,
                                     bcmptm_rm_tcam_lt_info_t *ltid_info,
                                     ...
                                     uint32_t **policy_words)
{
    ...
    SHR_NULL_CHECK(policy_words, SHR_E_PARAM);

    /* Only for EFP stage we need to update flex ctr action */
    if (((ltid_info->rm_more_info[0].fp_flags)
                        & BCMPTM_STAGE_KEY_TYPE_SELCODE) &&
                    ((ltid_info->rm_more_info[0].fp_flags)
                        & BCMPTM_STAGE_KEY_TYPE_FIXED)) {
        /* get the flex_ctr_action */
        /* Start bit and width of FLEX_CTR_ACTION in EFP_POLICY_TABLE */
        start_bit = 2;
        width = 4;
        bcmdrd_field_get(policy_words[0],
                         start_bit,
                         start_bit + width - 1,
                         &flex_ctr_action);
        if (flex_ctr_action == 0) {
            SHR_EXIT();
        }

        /* Update the Virtual Group No. in entry buffer*/
        field_val = ctr_action_prof_id;
        bcmdrd_field_set(policy_words[0],
                         start_bit,
                         start_bit + width - 1,
                         &field_val);
    }
...
int
bcm56990_a0_rm_tcam_ser_corr_sid_info_get(int unit,
                                 bcmdrd_sid_t sid,
                                 ...
                                 uint8_t *sid_count)
{
    ...
    switch (sid) {
        case L3_TUNNEL_TCAMm:
        case L3_TUNNEL_TCAM_ONLYm:
        case L3_TUNNEL_TCAM_DATA_ONLYm:
        case L3_TUNNEL_PAIR_TCAMm:
            tcam_only_sid[0] = L3_TUNNEL_TCAM_ONLYm;
            data_only_sid[0] = L3_TUNNEL_TCAM_DATA_ONLYm;
            tcam_data_sid[0] = L3_TUNNEL_TCAMm;
            paired_sid[0] = L3_TUNNEL_PAIR_TCAMm;
            *sid_count = 1;
            break;
        default:
            *sid_count = 0;
