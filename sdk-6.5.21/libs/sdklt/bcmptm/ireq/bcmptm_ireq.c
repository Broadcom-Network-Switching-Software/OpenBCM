...
/* Called by external bcmptm_ireq_read_cci and bcmptm_ireq_read APIs.
 * Caller APIs do all the error checking.
 * Caller APIs take, release lock if needed.
 *
 * return:
 *      SHR_E_NONE: Reading from PTcache was successful.
 *      SHR_E_UNAVAIL: Must read entry from HW.
 *      Other errors : Caller of this func returns these errors to it's caller.
 */
static inline int
ireq_read_cache(int unit,
                bool acc_for_ser,
                uint64_t flags,
                bcmdrd_sid_t sid,
                ...
                bcmltd_sid_t *rsp_ltid)
{
    ...
    if (!(flags & BCMLT_ENT_ATTR_GET_FROM_HW) &&
            ...
            case SHR_E_NONE:
                if (rsp_entry_cache_vbit) {
                    ...
                    *rsp_ltid = rsp_entry_ltid;
                    ...
                    /* Cache is alloc for this pt, but entry was not written by
                     * any SW module. Entry, is still valid in cache - because
                     * PTcache was written with 'default' values during PTcache
                     * init.
                     * If user bypasses PTM APIs for writes, then returned value
                     * may be incorrect - this is caller responsibility. Callers
                     * must either use / not use PTM APIs consistently.
                     */
                    *rsp_ltid = BCMPTM_LTID_RSP_HW; /* as if it came from HW */
                }
                break;
            default:
        ...
        tmp_rv = SHR_E_UNAVAIL; /* Caller must read entry from HW */
        *rsp_ltid = BCMPTM_LTID_RSP_HW;
    ...
    SHR_IF_ERR_EXIT(tmp_rv); /* use tmp_rv as return value from this function */
exit:
    SHR_FUNC_EXIT();
}
...
/* Called by external bcmptm_ireq_read_cci and bcmptm_ireq_read APIs.
 * Caller APIs do all the error checking.
 * Caller APIs take, release lock as needed.
 */
static inline int
ireq_read_hw(int unit,
             uint64_t flags,
             bcmdrd_sid_t sid,
             bcmbd_pt_dyn_info_t *pt_dyn_info,
             ...
             uint32_t *rsp_entry_words)
{
    ...
    if (hw_rd_enable[unit]) {
        do {
            ...
            if (ser_failure) {
                if (!retry_needed) {
                    LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit, "Will not retry failed ireq_read\n")));
        ...
        if (can_be_cam && bcmdrd_pt_attr_is_cam(unit, sid) &&
    ...
    } else { /* hw_rd is not enabled */
        /* Print warning, but don't return E_UNAVAIL.
         *      SHR_ERR_EXIT(SHR_E_UNAVAIL);
         * Redirect read to WAL.
         */
        LOG_WARN(BSL_LOG_MODULE,
                 (BSL_META_U(unit, "Will send interactive path hw_read for sid="
                             "%0d (%s) to WAL\n"),
                  sid, bcmdrd_pt_sid_to_name(unit, sid)));
        SHR_IF_ERR_EXIT(
              ...
              bool *have_ireq_lock_p)
{
    ...
    if (can_be_cam &&
        hw_wr_enable[unit] && !hw_wr_forward[unit] &&
        ...
        if (hw_wr_forward[unit]) {
            ...
            do {
                ...
                if (BCMPTM_SHR_FAILURE_SER(tmp_rv) && (retry_count == 0)) {
                    if (bcmptm_pt_ser_resp(unit, sid) == BCMDRD_SER_RESP_NONE) {
                        LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit, "Will not retry failed "
                                        "ireq_write\n")));
            ...
            if (BCMPTM_SHR_FAILURE_SER(tmp_rv)) {
                LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "ireq_write to sid=%d inst=%d index=%d failed "
                                "(after %d attempts) !!\n"),
                     ...
                     uint32_t *rsp_flags)
{
    ...
    /* Return for all errors except E_UNAVAIL */
    SHR_IF_ERR_EXIT_EXCEPT_IF(tmp_rv, SHR_E_UNAVAIL);

    /* Reading from PTcache succeeded. */
    if (tmp_rv == SHR_E_NONE) {
        SHR_EXIT();
    }

    /* Read from HW - as read from cache did not go through (tmp_rv is
     * SHR_E_UNAVAIL).
     *
     * Assume counter SIDs are not wo_aggr views.
     */
    SHR_IF_ERR_EXIT(
        ireq_read_hw(unit, flags,
                     sid, pt_dyn_info, in_pt_ovrr_info,
                     FALSE,          /* can_be_cam */
                     FALSE, NULL,    /* wo_aggr, *wo_aggr_info */
                     rsp_entry_wsize,
                 ...
                 uint32_t *rsp_flags)
{
    ...
    if (ptcache_ctr_info.pt_is_ctr) { /* is counter table */
        ...
        sal_memset(&cci_info, 0, sizeof(bcmptm_cci_ctr_info_t));
        ...
        /* Record PT read op for CCI case */
    ...
    /* Reading from PTcache succeeded. */
    if (tmp_rv == SHR_E_NONE) {
        /* Record PT read op for PT cache case */
        if (lt_ireq_op_cb[unit] != NULL) {
            SHR_IF_ERR_EXIT
                (lt_ireq_op_cb[unit](unit, req_ltid, 0xFFFFFFFF,
                                     sid, pt_dyn_info));
        }
                  ...
                  uint32_t *rsp_flags)
{
    ...
    *rsp_ltid = req_ltid;
    ...
    if (ptcache_ctr_info.pt_is_ctr) { /* is counter table */
        ...
        /* Record PT write op for CCI case */
        if (lt_ireq_op_cb[unit] != NULL) {
            SHR_IF_ERR_EXIT
                (lt_ireq_op_cb[unit](unit, cci_info.req_ltid, 0xFFFFFFFF,
                         cci_info.sid,
    ...
    /* Record PT write op for non-CCI case */
    if (lt_ireq_op_cb[unit] != NULL) {
        SHR_IF_ERR_EXIT
            (lt_ireq_op_cb[unit](unit, req_ltid, 0xFFFFFFFF, sid,
                                 (bcmbd_pt_dyn_info_t *)pt_dyn_info));
                    ...
                    uint32_t *rsp_flags)
{
    ...
    unsigned int i;
    bool have_ireq_lock = FALSE;
    bool *have_ireq_lock_p = &have_ireq_lock;
    const char *op_name;
    SHR_FUNC_ENTER(unit);
    ...
    do {
        switch (req_op) {
            case BCMPTM_OP_LOOKUP:
                SHR_IF_ERR_EXIT(!hw_rd_enable[unit] ? SHR_E_UNAVAIL
                                                    : SHR_E_NONE);
            ...
            case BCMPTM_OP_DELETE:
                SHR_IF_ERR_EXIT(!hw_wr_enable[unit] || hw_wr_forward[unit] ?
                ...
                bd_rv = bcmbd_pt_delete(unit,
                                        ...
                                        &bd_rsp_info,
                                        rsp_entry_data_words);
                /* Note: Entry is not deleted from cache - rsrc_mgr must be
                 *       invoked from interactive thread to keep PTcache
                 *       consistent - this may collide with rsrc_mgr running in
                 *       modeled thread context. */
                break; /* DELETE */
        ...
        if (BCMPTM_SHR_FAILURE_SER(bd_rv)) {
            SHR_ERR_EXIT(bd_rv);
        }
...
int
bcmptm_ireq_init(int unit, bool warm)
{
    SHR_FUNC_ENTER(unit);
...
int
bcmptm_ireq_mode_set(int unit, bool wr_enable, bool wr_forward,
                     bool rd_enable)
{
