 ...
 *    These LTs _must_ have a defined LTID.LOOKUP to access SBR-RM info.
 */
/*
                        ...
                        uint32_t *count)
{
    ...
    if (lt_fields->lt_entry_limit != 1) {
        /* Determine current entry limit for multi-entry SPLTs*/
        SHR_IF_ERR_EXIT
            (bcmptm_spm_user_lt_entry_count(unit, lt_fields));
    }
...
bcmptm_spm_init(int unit, bool warm)
{
    ...
    for (ix = 0; ix < spm_lt_count; ix++) {
        ...
        if (SHR_SUCCESS(rv) && map) {
            ...
            if (SHR_SUCCESS(rv)) {
                /* Constrain LT to declared LT size */
                spm_lt_fields->lt_index_max = lt_max;
            } else if (spm_lt_fields->key_num != 0) {
                /* SPLTs are auto-generated -- must have entry_limit defined */
                SHR_ERR_EXIT(SHR_E_INTERNAL);
            }

            if (spm_lt_fields->key_num == 0) {
                /* Dynamic SBR, no-key SPLT */
                spm_lt_fields->lt_entry_limit =
                    spm_lt_fields->lt_index_max = 1;
            }
                                    ...
                                    bool *blocked)
{
    ...
    const bcmlrd_hmf_tile_mode_map_t *tmm;
    const bcmlrd_hmf_memory_t *hw_mem;
    ...
    for (ltix = 0; ltix < lt_record->lt_num; ltix++) {
        ...
        if (table_data.entry_inuse != 0) {
            ...
            for (hwe_ix = 0; hwe_ix < hmf_instance->memory_count; hwe_ix++) {
                hw_mem= hmf_instance->memory[hwe_ix];
                if (hw_mem->tile_mode_map_count != 0) {
                    /* TM contains metadata */
                    /* For each (tile, tile_mode) in the list for this SPPT */
                    for (tmix = 0;
                         tmix < hw_mem->tile_mode_map_count;
                         tmix++) {
                        tmm = &(hw_mem->tile_mode_map[tmix]);
                        if ((tmm->tile == tile) &&
                            ((tmm->tile_mode == current_tile_mode) ||
                             (tmm->tile_mode == new_tile_mode))) {
                                /*
                                ...
                                *blocked = TRUE;
                                SHR_EXIT();
                        } /* Else, SPLT not involved in either tile mode. */
                    }
                } else {
                    /* Metadata contains TM */
                    metadata = &(hmf_instance->memory[hwe_ix]->metadata);

                    /* For each (tile, tile_mode) in the list for this SPPT */
                    for (tmix = 0;
                         tmix < metadata->tile_mode_list_count;
                         tmix++) {
                        tmi = &(metadata->tile_mode_list[tmix]);
                        if ((tmi->tile == tile) &&
                            ((tmi->tile_mode == current_tile_mode) ||
