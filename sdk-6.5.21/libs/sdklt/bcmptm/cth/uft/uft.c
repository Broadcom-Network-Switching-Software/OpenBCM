...
uft_dev_em_tile_info_lt_init(int unit, uft_dev_info_t *dev, bool warm)
{
    ...
    uint32_t bank;
    bool active_valid = FALSE, tile_valid = FALSE;
    ...
    /* Check if ACTIVE field is supported. */
    uft_lt_field_validate(unit, DEVICE_EM_TILE_INFOt,
                          DEVICE_EM_TILE_INFOt_ACTIVEf,
                          &active_valid);
    ...
    for (idx = 0; idx < var->em_tile_cnt; idx++) {
        ...
        if (active_valid) {
            tile_valid = FALSE;
            /* Check for banks are valid or not. */
            for (bank = 0; bank < var->em_bank_cnt; bank++) {
                if (var->tile_id[bank] == idx) {
                    if (bcmdrd_pt_is_valid(unit,
                                           var->em_bank[bank].w_ptid) == TRUE) {
                        tile_valid = TRUE;
                        break;
                    }
                }
            ...
            in_flds.field[f_cnt]->id   = DEVICE_EM_TILE_INFOt_ACTIVEf;
            in_flds.field[f_cnt]->idx  = 0;
            in_flds.field[f_cnt]->data = tile_valid;
            f_cnt++;
        }
...
uft_dev_em_tile_mode_info_lt_init(int unit, uft_dev_info_t *dev, bool warm)
{
    ...
    /* Check if NARROW_MODE field is supported. */
    uft_lt_field_validate(unit,
                          DEVICE_EM_TILE_MODE_INFOt,
                          DEVICE_EM_TILE_MODE_INFOt_NARROW_MODEf,
                          &narrow_mode_valid);
    /* Check if NUM_KEY field is supported. */
    uft_lt_field_validate(unit, DEVICE_EM_TILE_MODE_INFOt,
                          DEVICE_EM_TILE_MODE_INFOt_NUM_KEY_TYPEf,
                          &key_valid);
    ...
    for (idx = 0; idx < var->tile_mode_cnt; idx++) {
        ...
        if (narrow_mode_valid) {
            in_flds.field[f_cnt]->id   = DEVICE_EM_TILE_MODE_INFOt_NARROW_MODEf;
            in_flds.field[f_cnt]->idx  = 0;
            in_flds.field[f_cnt]->data = tm->narrow_mode;
            f_cnt++;
            in_flds.field[f_cnt]->id   =
                DEVICE_EM_TILE_MODE_INFOt_NARROW_MODE_OVERRIDE_CAPABLEf;
            in_flds.field[f_cnt]->idx  = 0;
            in_flds.field[f_cnt]->data = tm->narrow_mode_override_capable;
            f_cnt++;
        }

        if (key_valid) {
            in_flds.field[f_cnt]->id   = DEVICE_EM_TILE_MODE_INFOt_NUM_KEY_TYPEf;
            in_flds.field[f_cnt]->idx  = 0;
            in_flds.field[f_cnt]->data = tm->alpm_key_mux_type_count;
            f_cnt++;

            for (cnt = 0; cnt < tm->alpm_key_mux_type_count; cnt++) {
                in_flds.field[f_cnt]->id   = DEVICE_EM_TILE_MODE_INFOt_KEY_TYPEf;
                in_flds.field[f_cnt]->idx  = cnt;
                in_flds.field[f_cnt]->data = tm->alpm_key_mux_type[cnt];
                f_cnt++;
            }
        }
...
uft_dev_em_grp_lt_init(int unit, uft_dev_info_t *dev, bool warm)
{
    ...
    bool robust_valid = false, seed_valid = false;
    uint64_t def_robust = 0, def_seed = 0;
    ...
    rv = bcmlrd_field_default_get(unit, DEVICE_EM_GROUPt,
                                  DEVICE_EM_GROUPt_SEEDf,
                                  1, &def_seed, &num_actual);
    if (rv == SHR_E_NONE) {
        seed_valid = true;
    }
    ...
    for (idx = 0; idx < dev->max_grps; idx++) {
        ...
        if (seed_valid) {
            in_flds.field[f_cnt]->id   = DEVICE_EM_GROUPt_SEEDf;
            in_flds.field[f_cnt]->idx  = 0;
            in_flds.field[f_cnt]->data = def_seed;
            f_cnt++;
        }
...
uft_dev_em_grp_info_lt_init(int unit, uft_dev_info_t *dev, bool warm)
{
    ...
    bool attri_valid = FALSE, lookup0_lt_valid = FALSE;
    ...
    uft_lt_field_validate(unit, DEVICE_EM_GROUP_INFOt,
