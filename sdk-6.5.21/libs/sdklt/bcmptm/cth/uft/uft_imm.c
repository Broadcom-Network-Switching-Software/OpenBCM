    ...
    *chg = FALSE;
    SHR_IF_ERR_EXIT
        (bcmptm_uft_tile_cfg_get(unit, tile->tile_id,
                                 &tile_mode, &robust, &seed, &narrow_mode));
    ...
    if (SHR_BITGET(tile->fbmp, DEVICE_EM_TILEt_NARROW_MODEf)) {
        if (tile->narrow_mode != narrow_mode) {
            *chg = TRUE;
            SHR_EXIT();
        } else {
            SHR_BITCLR(tile->fbmp, DEVICE_EM_TILEt_NARROW_MODEf);
        }
    }
...
/*!
 * \brief Check whether a tile mode allows narrow mode overriden.
 *
 * \param [in] unit Unit number.
 * \param [in] tile_mode Tile mode.
 * \param [out] allowed TRUE indicates it is allowed to change narrow mode.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Fail.
 */
static int
dev_em_tile_nm_ovrd_check(int unit, uint32_t tile_mode, bool *allowed)
{
    ...
    if (tile_mode < var->tile_mode_cnt) {
        *allowed = var->tile_mode[tile_mode]->narrow_mode_override_capable;
    } else {
        *allowed = FALSE;
    }
    ...
    while (gen_field) {
        ...
        switch (fid) {
            ...
            case DEVICE_EM_GROUPt_NUM_BANKSf:
                ...
                break;
            case DEVICE_EM_GROUPt_SEEDf:
                grp->seed = fval;
                SHR_BITSET(grp->fbmp, DEVICE_EM_GROUPt_SEEDf);
                break;
            default:
                break;
        }
    ...
    while (gen_field) {
        ...
        switch (fid) {
            ...
            case DEVICE_EM_TILEt_MODEf:
                ...
                break;
            case DEVICE_EM_TILEt_NARROW_MODEf:
                tile->narrow_mode = fval;
                SHR_BITSET(tile->fbmp, DEVICE_EM_TILEt_NARROW_MODEf);
                break;
            default:
                SHR_ERR_EXIT(SHR_E_PARAM);
        }
                         ...
                         void *context)
{
    ...
    /* Narrow mode. */
    bool cur_nm = FALSE, allowed = FALSE;
    ...
    switch (event) {
        case BCMIMM_ENTRY_UPDATE:
            ...
            /*
             * Only when NARROW_MODE_OVRRIDE_CAPABLE is true, it is allowed
             * to change the NARROW_MODE field of a tile.
             */
            if (SHR_BITGET(tile.fbmp, DEVICE_EM_TILEt_NARROW_MODEf) &&
                cur_nm != tile.narrow_mode) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (dev_em_tile_nm_ovrd_check(unit, cur_mode, &allowed));
                if (!allowed) {
                    SHR_ERR_EXIT(SHR_E_CONFIG);
                }
            }
                          ...
                          uint64_t ev_data)
{
    ...
    (void)bcmptm_rm_hash_ltpr_update(unit,
                                     BCMPTM_UFT_EV_INFO_LTID_GET(ev_data),
                                     BCMPTM_UFT_EV_INFO_TRANS_ID_GET(ev_data),
                                            bcmptm_uft_lt_hw_write);
    (void)bcmptm_rm_hash_narrow_mode_update(unit);
}
