...
/*!
 * \brief Index LT tile mode hit context settings.
 *
 * This function sets the Flex Counter hit context for a tile when
 * a tile-aware index LT changes mapping.  The hit context is the order
 * number of the tile in the logical index sequence.
 *
 * \param [in] unit    Unit number.
 * \param [in] lt_map  Index LT physical table mapping info.
 * \param [in] ev_data Data associated with the event.
 ...
 */
static int
bcmptm_itx_hit_context_set(int unit,
                           bcmptm_itx_lt_map_t *lt_map,
                           uint64_t ev_data)
{
    uint32_t tix;
    ...
    for (tix = 0; tix < tr_list->tile_num; tix++) {
        ...
        /*
         * Hit context is ordered number of tile in LT index range
         * so it is the loop index here "tix".
         */
        SHR_IF_ERR_EXIT
            (bcmptm_uft_fc_hit_context_set_rm_index(unit,
                                                    lt_map->ltid,
                                                    &op_arg,
                                                    tr_info->tile_id,
                                                    tix));
                                    ...
                                    bcmptm_itx_lt_map_t *lt_map)
{
    ...
    for (ptix = 0; ptix < hw_tile_info->memory_count; ptix++) {
        ...
        rv = bcmptm_uft_tile_cfg_get(unit, memory->tile_id,
                                     &tile_mode, &robust, &seed, NULL);
        ...
        if (memory->tile_mode != tile_mode) {
            /* Not the memory map for the current tile mode. */
            continue;
        }
                              ...
                              uint64_t ev_data)
{
    ...
    for (ltix = 0; ltix < lt_map_list->lt_num; ltix++) {
        ...
        /* Do not write during init -- no commit to clear PTM state */
        if (BCMPTM_UFT_EV_INFO_TRANS_ID_GET(ev_data) !=
            ...
            /* Update hit context values for this LT */
            rv = bcmptm_itx_hit_context_set(unit, lt_map, ev_data);
            if (SHR_FAILURE(rv)) {
                continue;
            }
        ...
        if (BCMPTM_UFT_EV_INFO_TRANS_ID_GET(ev_data) !=
            ...
            if (SHR_FAILURE(rv)) {
                LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit,
                    "bcmptm_itx_tile_event_handler access mode set %s\n"),
                                           shr_errmsg(rv)));
            }
        }
    }
