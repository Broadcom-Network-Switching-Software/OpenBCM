...
/*!
 * \brief Seek the metadata for a particular SP PT.  Handle per-memory and
 *        per-tile mode metadata organization.  Return NULL if this SP PT
 ...
 *
 * \param [in] unit Unit number.
 * \param [in] memory_map SP PT details of this SP LT's map.
 * \param [out] metadata Return pointer.
 *                       If non-NULL, memory(tile-mode) metadata for this SP PT.
 *                       If NULL, no match found.
 *
 * \retval SHR_E_NONE No errors.
                 ...
                 const bcmlrd_hmf_memory_metadata_t **metadata)
{
    ...
    if (memory_map->tile_mode_map_count) {
        const bcmlrd_hmf_tile_mode_map_t *tmmi;
        /* Per tile-mode metadata format */
        for (tmix = 0; tmix < memory_map->tile_mode_map_count; tmix++) {
            tmmi = &(memory_map->tile_mode_map[tmix]);

            SHR_IF_ERR_EXIT
                (bcmptm_uft_tile_cfg_get(unit, tmmi->tile,
                                         &tile_mode, &robust, &seed, NULL));

            if (tmmi->tile_mode == tile_mode) {
                *metadata = &(tmmi->metadata);
                SHR_EXIT();
            }
        }
    } else {
        const bcmlrd_hmf_tile_mode_t *tmi;
        const bcmlrd_hmf_memory_metadata_t *test_metadata;

        /* Per memory metadata with tile-mode list format */
        test_metadata = &(memory_map->metadata);
        if ((test_metadata->tile_mode_list_count == 0) ||
            (test_metadata->tile_mode_list == NULL)) {
            /* No Tile mode info for this SPLT */
            *metadata = test_metadata;
            SHR_EXIT();
        }

        for (tmix = 0; tmix < test_metadata->tile_mode_list_count; tmix++) {
            tmi = &(test_metadata->tile_mode_list[tmix]);
                 ...
                 bool *is_mapped)
{
    ...
    *is_mapped = FALSE;
    ...
    for (hwe_ix = 0; hwe_ix < hmf_instance->memory_count; hwe_ix++) {
        ...
        metadata = NULL;
        SHR_IF_ERR_EXIT
            (spm_pt_is_mapped(unit, memory_map,
                              &metadata));
        if (metadata != NULL) {
            *is_mapped = TRUE;
            SHR_EXIT();
                                 ...
                                 bcmptm_spm_lt_fields_t *lt_fields)
{
    ...
    if (lt_fields->key_num > 1) {
        ...
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "Too many key fields for LT %s\n"),
                      tbl->name));
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    } else if ((lt_fields->key_num == 0) &&
               (lt_fields->ltid_lookup == BCMLTD_INVALID_LT)) {
        /* Dynamic SBR -- must have LTID.LOOKUP defined */
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "Invalid LTID.LOOKUP for no-key LT %s\n"),
                   tbl->name));
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    }
                                   ...
                                   bcmptm_spm_lt_fields_t *lt_fields)
{
    ...
    for (hwe_ix = 0; hwe_ix < hmf_instance->memory_count; hwe_ix++) {
        ...
        if (metadata->flags & BCMLRD_HMF_MEMORY_F_LOOKUP0) {
            lookup_type = BCMPTM_SBR_LOOKUP0;
        } else if (metadata->flags & BCMLRD_HMF_MEMORY_F_LOOKUP1) {
...
`        * It is likely that the LOOKUP table is inactive.
    ...
    for (hwe_ix = 0; hwe_ix < hmf_instance->memory_count; hwe_ix++) {
        ...
        if (metadata->flags & BCMLRD_HMF_MEMORY_F_OFFSET) {
            pt_index += metadata->offset;
        }
...
`        * It is likely that the LOOKUP table is inactive.
    ...
    for (lt_index = 0; lt_index < lt_fields->lt_index_max; lt_index++) {
        ...
        for (hwe_ix = 0; hwe_ix < hmf_instance->memory_count; hwe_ix++) {
            ...
            /* Only consider SBR PTs for tiles currently mapped for LOOKUP LT */
            SHR_IF_ERR_EXIT
                (spm_pt_is_mapped(unit, memory_map,
                                  &metadata));
            if (metadata == NULL) {
            ...
            if ((entry_state->usage_mode != BCMPTM_SBR_ENTRY_FREE) &&
                ...
                /* This index is not available for this LT. */
                idx_free = FALSE;
                break;
            }
        }
                                ...
                                bcmimm_entry_event_t event)
{
    ...
    for (hwe_ix = 0; hwe_ix < hmf_instance->memory_count; hwe_ix++) {
        ...
        if (metadata->flags & BCMLRD_HMF_MEMORY_F_OFFSET) {
            pt_write_index += metadata->offset;
        }
