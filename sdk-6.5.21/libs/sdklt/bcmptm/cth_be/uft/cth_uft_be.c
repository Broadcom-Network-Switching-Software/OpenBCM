...
uft_grp_cfg_get(int unit, uint32_t grp_id,
                ...
                uint32_t *r_bank_cnt,
                bool *robust, uint32_t *vec_type, uint32_t *seed)
{
    bcmltd_fields_t input = {0}, output = {0};
    size_t idx;
    uint32_t fid, fidx;
                 ...
                 bcmptm_pt_banks_info_t *banks_info)
{
    ...
    SHR_IF_ERR_EXIT
        (uft_grp_cfg_get(unit, grp_id,
                         ...
                         &v_bank_cnt,
                         &robust_en, &(banks_info->vector_type), &seed));
...
uft_em_tile_mode_check(int unit, bool *tile_mode_valid, uint32_t tile_mode_cnt)
{
    ...
    for (idx = 0; idx < var->em_tile_cnt; idx++) {
        SHR_IF_ERR_EXIT_VERBOSE_IF
            (bcmptm_uft_tile_cfg_get(unit, idx,
                                     &mode, NULL, NULL, NULL),
             SHR_E_NOT_FOUND);
    ...
    for (idx = 0; idx < banks_info->bank_cnt; idx++) {
        ...
        SHR_IF_ERR_EXIT
            (bcmptm_uft_tile_cfg_get(unit, tile_id,
                                     ...
                                     &nm_cfg));
        if (mode < var->tile_mode_cnt) {
            if (var->tile_mode[mode]->narrow_mode_override_capable) {
                bank_attr->narrow_mode = nm_cfg;
            } else {
                bank_attr->narrow_mode = var->tile_mode[mode]->narrow_mode;
            }
        } else {
           bank_attr->narrow_mode = FALSE;
        }
    }
                                    ...
                                    bcmptm_lt_lookup_banks_info_t *banks_info)
{
    ...
    for (idx = 0; idx < var->em_tile_cnt; idx++) {
        SHR_IF_ERR_EXIT
            (bcmptm_uft_tile_cfg_get(unit, idx,
                                     &w_mode, NULL, NULL, NULL));
        if (w_mode >= var->tile_mode_cnt) {
            /* Mode is disabled or invalid. */
            continue;
        }
        tile_mode_is_set[w_mode] = TRUE;
    }
                               ...
                               uint32_t *r_cnt)
{
    ...
    for (idx = 0; idx < grp_cnt; idx++) {
        ...
        bank_cnt = 0;
        SHR_IF_ERR_VERBOSE_EXIT
            (uft_grp_cfg_get(unit, grp_id, NULL, 0, &bank_cnt,
                             NULL, NULL, NULL));
        if (bank_cnt == 0) {
            continue;
        }
...
int
bcmptm_cth_uft_ltpr_get(int unit,
                        bcmltd_sid_t ltid,
                        uint32_t lookup_num,
                        bcmptm_lt_lookup_ltpr_info_t *ltpr_arr,
                        uint32_t ltpr_sz,
                        uint32_t *r_ltpr_cnt)
{
    const bcmptm_uft_var_drv_t *var = NULL;
    uint32_t size, idx, i, cnt;
    uint32_t w_mode, mode;
    bool *tile_mode_is_set = NULL;
    const em_tile_info_t *tile = NULL;
    const em_tile_mode_info_t *tm_info = NULL;
    const em_lt_lookup_info_tile_mode_t *lt_lkp = NULL;
    uint32_t tile_p_lkp;
    ...
    var = uft_be_dev_info[unit]->var;
    SHR_NULL_CHECK(var, SHR_E_PARAM);
    if ((ltpr_arr == NULL) && (ltpr_sz > 0)) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    SHR_NULL_CHECK(r_ltpr_cnt, SHR_E_PARAM);

    /* Calculates all the tile's working mode. */
    size = var->tile_mode_cnt * sizeof(bool);
    tile_mode_is_set = sal_alloc(size, "bcmptmUftTileModeIsSet");
    SHR_NULL_CHECK(tile_mode_is_set, SHR_E_MEMORY);
    sal_memset(tile_mode_is_set, 0, size);
    for (idx = 0; idx < var->em_tile_cnt; idx++) {
        SHR_IF_ERR_EXIT
            (bcmptm_uft_tile_cfg_get(unit, idx, &w_mode, NULL, NULL, NULL));
        if (w_mode >= var->tile_mode_cnt) {
            /* Mode is disabled or invalid. */
            continue;
        }
    ...
    if (lookup_num == 0) {
        for (mode = 0; mode < var->tile_mode_cnt; mode++) {
            ...
            tm_info = var->tile_mode[mode];
            for (idx = 0; idx < tm_info->lt_lookup0_count; idx++) {
                lt_lkp = &(tm_info->lt_lookup0[idx]);
                if (lt_lkp->ltid == ltid) {
                    /* No need to do LTPT programming for this LT.LOOKUP. */
                    if (!lt_lkp->miss_ltpr_enable) {
                        (*r_ltpr_cnt) = 0;
                        SHR_EXIT();
                    }
                    tile = &(var->em_tile[tm_info->tile_id]);
                    for (i = 0; i < lt_lkp->p_lkp_count; i++) {
                        if (ltpr_arr) {
                            if (cnt >= ltpr_sz) {
                                SHR_ERR_EXIT(SHR_E_PARAM);
                            }
                            tile_p_lkp = lt_lkp->p_lkp[i];
                            ltpr_arr[cnt].tile_id = tm_info->tile_id;
                            ltpr_arr[cnt].p_lkp = tile_p_lkp;
                            ltpr_arr[cnt].ltpr_ptid = tile->ltpr_ptid;
                            ltpr_arr[cnt].ltpr_index = lt_lkp->ltpr_idx[i];
                            ltpr_arr[cnt].lkp_id = tile->p_lkp_id[tile_p_lkp];
                        }
                        cnt++;
                    }
                }
        ...
        for (mode = 0; mode < var->tile_mode_cnt; mode++) {
            ...
            tm_info = var->tile_mode[mode];
            for (idx = 0; idx < tm_info->lt_lookup1_count; idx++) {
                lt_lkp = &(tm_info->lt_lookup1[idx]);
                if (lt_lkp->ltid == ltid) {
                    /* No need to do LTPT programming for this LT.LOOKUP. */
                    if (!lt_lkp->miss_ltpr_enable) {
                        (*r_ltpr_cnt) = 0;
                    ...
                    for (i = 0; i < lt_lkp->p_lkp_count; i++) {
                        if (ltpr_arr) {
                            ...
                            ltpr_arr[cnt].tile_id = tm_info->tile_id;
                            ltpr_arr[cnt].p_lkp = lt_lkp->p_lkp[i];
                            ltpr_arr[cnt].ltpr_ptid = tile->ltpr_ptid;
                            ltpr_arr[cnt].ltpr_index = lt_lkp->ltpr_idx[i];
                            ltpr_arr[cnt].lkp_id = tile->p_lkp_id[i];
                        }
                        cnt++;
                    }
                }
...
bcmptm_uft_tile_cfg_get(int unit, uint32_t tile_id,
                        uint32_t *tile_mode, bool *robust,
                        uint32_t *seed, bool *narrow_mode)
{
    uint32_t fid;
    uint64_t fval;
    bcmltd_fields_t input = {0}, output = {0};
                         ...
                         uint32_t *tile_id)
{
    ...
    if (b_sid) {
        ...
        if (em_mode_bank) {
            SHR_IF_ERR_EXIT
                (bcmptm_uft_tile_cfg_get(unit, tid,
                                         &tm, NULL, NULL, NULL));
    ...
    *r_cnt = 0;
    ...
    for (t = 0; t < var->em_tile_cnt; t++) {
        ...
        if (tile_info->alpm_pdd_type_available) {
            SHR_IF_ERR_EXIT
                (bcmptm_uft_tile_cfg_get(unit, t,
                                         &work_tm, NULL, NULL, NULL));
