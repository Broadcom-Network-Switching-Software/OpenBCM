...
static bcmptm_ptcache_issu_ctl_t issu_ctl[BCMDRD_CONFIG_MAX_UNITS];
                         ...
                         uint32_t *overlay_num_cw)
{
    ...
    if (vinfo_p) { /* caller only needs overlay_num_cw */
        oinfo_w0.entry = 0;
           ...
           uint32_t *lsw_count)
{
    ...
    *lsw_count = 0;
    ...
    if (warm) {
        ...
        case BCMPTM_PTCACHE_TYPE_UNINIT:
    ...
    if (pt_size_change) {
        SHR_IF_ERR_MSG_EXIT(
            (in_sinfo.ptcache_type == BCMPTM_PTCACHE_TYPE_UNINIT) ||
    ...
    } else if (in_sinfo.ptcache_type != BCMPTM_PTCACHE_TYPE_UNINIT) {
        *already_init = TRUE;
    ...
    if (pt_index_count == 0 || tbl_inst_count == 0) {
        if (pt_group != BCMPTM_PT_GROUP_DEFAULT) {
            if (pt_attr_is_cacheable) {
                sinfo_p->ptcache_type = BCMPTM_PTCACHE_TYPE_ME_RSVD;
                ...
                sinfo_p->ptcache_type = BCMPTM_PTCACHE_TYPE_NO_CACHE;
                *skipped = TRUE;
                SHR_EXIT(); /* done */
            }
    ...
    if (bcmdrd_pt_is_valid(u, sid) && bcmdrd_pt_is_counter(u, sid)) {
        ...
        if (!issu_ctl[u].skip_sinfo_cci_init) {
        ...
        if (pt_is_ctr_with_cfg_fields) { /* must cache this */
            ...
            sinfo_p->ptcache_type = BCMPTM_PTCACHE_TYPE_CCI_ONLY;
            *skipped = TRUE;
            LOG_VERBOSE(BSL_LOG_MODULE,
    ...
    if (bcmdrd_pt_is_valid(u, sid)
        ...
        if (overlay_info->mode == BCMPTM_OINFO_MODE_CACHE0) {
            ...
            sinfo_p->ptcache_type = BCMPTM_PTCACHE_TYPE_SE_CACHE0;
            ...
            *seds_count += 1; /* sentinel for SE data */
            ...
            if (pt_is_ctr_with_cfg_fields) {
                sinfo_p->ptcache_type = BCMPTM_PTCACHE_TYPE_ME_CCI;

                /* Should not be here for pt_size_change */
                SHR_IF_ERR_EXIT(pt_size_change ? SHR_E_INTERNAL : SHR_E_NONE);
            } else {
                sinfo_p->ptcache_type = BCMPTM_PTCACHE_TYPE_ME;
            }
 ...
 */
int
bcmptm_ptcache_sinfo_init_group(int unit, bool warm, bool pt_size_change,
                                ...
                                uint32_t *group_cwme_count)
{
    ...
    pt_group_info_p = pt_group_info_bp[unit][s] + pt_group;
    *vinfo_word_count = 0;
    ...
    *group_cwme_count = 0;
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(unit, "Running sinfo_init_group for pt_group=%0d\n"),
         pt_group));
    ...
    if (pt_group == BCMPTM_PT_GROUP_DEFAULT) {
        ...
        tmp_rv = bcmptm_pt_group_info_get(unit, pt_group, &pt_count, NULL,
                                          &pt_list);
        SHR_IF_ERR_MSG_EXIT(
            ((tmp_rv != SHR_E_NONE) || !pt_count) ? SHR_E_INTERNAL : SHR_E_NONE,
            (BSL_META_U(unit, "group_info_get returned rv = %0d, "
    ...
    for (i = 0; i < pt_count; i++) {
        ...
        SHR_IF_ERR_EXIT(
            sinfo_init(unit, warm, pt_size_change, sid, pt_group,
    ...
    *vinfo_word_count = valid_seg_word_count + data_seg_word_count +
    ...
    if (warm) {
        ...
        *ltid_seg_word_count = pt_group_info_p->ltid_seg_word_count;
        ...
        if (pt_group == BCMPTM_PT_GROUP_DEFAULT) {
            bcmptm_ptcache_sinfo_t *sinfo_bp_us = sinfo_bp[unit][s];
            *group_seds_count =
                ...
                uint32_t *ltid_seg_index) /* 1st word in lsw section for current sid */
{
    ...
    switch (sinfo_p->ptcache_type) {
        case BCMPTM_PTCACHE_TYPE_CCI_ONLY:
            ...
            break; /* BCMPTM_PTCACHE_TYPE_CCI_ONLY */

        case BCMPTM_PTCACHE_TYPE_SE_VALID: /* can happen during WB */
        ...
        case BCMPTM_PTCACHE_TYPE_SE_CACHE0:
        ...
        case BCMPTM_PTCACHE_TYPE_ME_RSVD:
        ...
        case BCMPTM_PTCACHE_TYPE_ME_CCI:
            if (sinfo_p->ptcache_type == BCMPTM_PTCACHE_TYPE_ME_CCI) {
                SHR_IF_ERR_EXIT(pt_size_change ? SHR_E_INTERNAL : SHR_E_NONE);
            ...
            /* Misc checks */
            if (sinfo_p->ptcache_type == BCMPTM_PTCACHE_TYPE_ME_RSVD) {
            ...
            *vinfo_vs_index += vs_word_count;
            ...
            if (xinfo.data_seg_en) {
                ...
                *vinfo_ds_index += 1;
                ...
                if (!issu_ctl[u].skip_vinfo_ltid &&
                    bcmdrd_pt_default_value_is_nonzero(u, sid)) {
                    SHR_IF_ERR_EXIT(
                ...
                *vinfo_ds_index += 1;
                ...
                SHR_IF_ERR_EXIT(
                    ptcache_overlay_info_set(u, sid, overlay_info,
                                             issu_ctl[u].skip_vinfo_ltid ? NULL
                                             : (vinfo_bp + (*vinfo_ds_index)),
                                             &overlay_num_cw));
            ...
            break; /* BCMPTM_PTCACHE_TYPE_ME, BCMPTM_PTCACHE_TYPE_ME_CCI */

        default: /* BCMPTM_PTCACHE_TYPE_NO_CACHE */
            SHR_IF_ERR_EXIT(
                sinfo_p->ptcache_type == BCMPTM_PTCACHE_TYPE_UNINIT ?
                SHR_E_INTERNAL : SHR_E_NONE);
            break;
    }
exit:
 ...
 */
int
bcmptm_ptcache_cwme_vinfo_init_group(int u, bool warm, bool pt_size_change,
                                     size_t all_sid_count, uint32_t pt_group)
{
    int tmp_rv;
    bcmdrd_sid_t sid;
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(u, "Running cwme_vinfo_init_group for pt_group=%0d\n"),
         pt_group));
        ...
        if ((new_banks_info.bank_cnt != pt_group_info_p->num_banks) ||
            ...
            SHR_IF_ERR_EXIT(
                bcmptm_ptcache_sinfo_init_group(
                    u, warm, pt_size_change, all_sid_count, pt_group,
                    seds_index, cwme_index,
                    /* out */
                    &vinfo_word_count, &ltid_seg_word_count,
                    &group_seds_count, &group_cwme_count));
...
bcmptm_ptcache_init(int u, bool warm)
{
    ...
    /* For both CB, WB */
    sal_memset(&issu_ctl[u], 0, sizeof(bcmptm_ptcache_issu_ctl_t));
    ...
    pt_group_ptrs_bp[u][s] = (bcmptm_ptcache_pt_group_ptrs_t *)alloc_ptr;
#ifdef ALSO_INIT_SER_PTCACHE_P
    pt_group_ptrs_bp[u][SER] = pt_group_ptrs_bp[u][s];
#endif

    SHR_IF_ERR_EXIT(bcmptm_cci_comp_config(u, warm, BCMPTM_SUB_PHASE_1));
    ...
    do {
        ...
        SHR_IF_ERR_EXIT(
            bcmptm_ptcache_sinfo_init_group(
                u, warm, pt_size_change, sid_count, pt_group,
                seds_index, cwme_index,
                /* out */
                &vinfo_word_count, &ltid_seg_word_count,
                &group_seds_count, &group_cwme_count));
...
#endif /* PTCACHE_SENTINEL_ME_ENABLE */
                break; /* BCMPTM_PTCACHE_TYPE_ME, ME_CCI */
    ...
    *rsp_entry_dfid = BCMPTM_DFID_RSP_NOCACHE;
    ...
    switch (sinfo_p->ptcache_type) {
        ...
        case BCMPTM_PTCACHE_TYPE_ME_CCI:
            ...
            if (sinfo_p->tbl_inst_en) {
                if (dyn_info->tbl_inst >= 0) {
                    ...
                    SHR_IF_ERR_EXIT(
                        (tbl_inst > cwme_p->tbl_inst_max) ?
                        SHR_E_PARAM : SHR_E_NONE);
                } /* else tbl_inst = 0; --ANY_COPY for reads */
            } /* else {
                SHR_IF_ERR_EXIT(dyn_info->tbl_inst > 0 ? SHR_E_PARAM
                                                       : SHR_E_NONE);
                * Above implies, that caller cannot read instance 5 for
                * with DUPLICATE acc_type. This may be too stringent.
                * What if SER logic attempts to read it with non-zero tbl_inst?
                * we must allow this.
            } */
            ...
            if (xinfo->overlay_mode) {
                ...
                if (w_sinfo_p->ptcache_type != BCMPTM_PTCACHE_TYPE_ME ||
        ...
        case BCMPTM_PTCACHE_TYPE_SE_NOT_VALID:
            ...
            if (sinfo_p->ptcache_type == BCMPTM_PTCACHE_TYPE_SE_VALID) {
                LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        case BCMPTM_PTCACHE_TYPE_SE_CACHE0:
             ...
             *
             * Interactive path can correctly read this table - but ONLY with
             * BCMLT_ENT_ATTR_GET_FROM_HW. Without this attribute, 0s will be
             * returned from PTcache.
             */
            SHR_IF_ERR_EXIT(entry_count != 1 ? SHR_E_PARAM : SHR_E_NONE);
            entry_wsize = bcmdrd_pt_entry_wsize(u, sid);
        ...
        default: /* BCMPTM_PTCACHE_TYPE_NO_CACHE, BCMPTM_PTCACHE_TYPE_CCI_ONLY */
            SHR_IF_ERR_EXIT(entry_count != 1 ? SHR_E_PARAM : SHR_E_NONE);
            LOG_VERBOSE(BSL_LOG_MODULE,
    ...
    switch (sinfo_p->ptcache_type) {
        ...
        case BCMPTM_PTCACHE_TYPE_ME_CCI:
            ...
            if (sinfo_p->tbl_inst_en) {
                ...
                SHR_IF_ERR_EXIT(dyn_info->tbl_inst > 0 ? SHR_E_PARAM
                                                       : SHR_E_NONE);
                /*
                 * Above implies, that caller cannot write instance 5 for
                 * with DUPLICATE acc_type - because in PTcache we
                 * have only one instance - and so cannot satisfy this request.
                 */
            }
            /* req entry size */
            entry_wsize = bcmdrd_pt_entry_wsize(u, sid);
        ...
        case BCMPTM_PTCACHE_TYPE_SE_VALID:
            ...
            sinfo_p->ptcache_type = set_cache_vbit ?
                BCMPTM_PTCACHE_TYPE_SE_VALID : BCMPTM_PTCACHE_TYPE_SE_NOT_VALID;
            break; /* BCMPTM_PTCACHE_TYPE_SE */
        ...
        default: /* BCMPTM_PTCACHE_TYPE_NO_CACHE, PTCACHE_TYPE_CCI_ONLY */
            SHR_IF_ERR_EXIT(entry_count != 1 ? SHR_E_PARAM : SHR_E_NONE);
            LOG_ERROR(BSL_LOG_MODULE,
                (BSL_META_U(u, "sid not cached ... for sid=%0d, "
                            "tbl_inst=%0d, index=%0d\n"),
                      ...
                      uint32_t *prev_tc_rdata)
{
    ...
    /* Read sinfo */
    sinfo_p = sinfo_bp[u][s] + sid;
    if (sinfo_p->ptcache_type != BCMPTM_PTCACHE_TYPE_ME) {
        SHR_VERBOSE_EXIT(SHR_E_UNAVAIL);
    }
...
int
bcmptm_ptcache_info_get(int unit,
                        bcmptm_ptcache_sinfo_t **o_sinfo_bp,
                        bcmptm_ptcache_pt_group_info_t **o_pt_group_info_bp,
                        bcmptm_ptcache_pt_group_ptrs_t **o_pt_group_ptrs_bp,
                        uint32_t **o_seds_bp,
                        bcmptm_ptcache_cwme_t **o_cwme_bp,
                        bcmptm_ptcache_issu_ctl_t **o_issu_ctl_p,
                        bool *o_ltid_size16b
                       )
{
    int s = 0;
    ...
    SHR_NULL_CHECK(o_sinfo_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(o_pt_group_info_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(o_pt_group_ptrs_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(o_seds_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(o_cwme_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(o_issu_ctl_p, SHR_E_PARAM);
    SHR_NULL_CHECK(o_ltid_size16b, SHR_E_PARAM);

    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(unit, "Getting HA pointers for PTcache....\n")));
    *o_sinfo_bp = sinfo_bp[unit][s];
    *o_pt_group_info_bp = pt_group_info_bp[unit][s];
    *o_pt_group_ptrs_bp = pt_group_ptrs_bp[unit][s];
    *o_seds_bp = seds_bp[unit][s];
    *o_cwme_bp = cwme_bp[unit][s];
    *o_issu_ctl_p = &issu_ctl[unit]; /* Note */
    *o_ltid_size16b = ltid_size16b[unit];
exit:
    SHR_FUNC_EXIT();
}

int
bcmptm_ptcache_info_set(int unit,
                        bcmptm_ptcache_sinfo_t *i_sinfo_bp,
                        bcmptm_ptcache_pt_group_info_t *i_pt_group_info_bp,
                        bcmptm_ptcache_pt_group_ptrs_t *i_pt_group_ptrs_bp,
                        uint32_t *i_seds_bp,
                        bcmptm_ptcache_cwme_t *i_cwme_bp
                       )
{
    int s = 0;
    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(i_sinfo_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(i_pt_group_info_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(i_pt_group_ptrs_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(i_seds_bp, SHR_E_PARAM);
    SHR_NULL_CHECK(i_cwme_bp, SHR_E_PARAM);

    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(unit, "Setting HA pointers for PTcache....\n")));

    sinfo_bp[unit][s] = i_sinfo_bp;
    pt_group_info_bp[unit][s] = i_pt_group_info_bp;
    pt_group_ptrs_bp[unit][s] = i_pt_group_ptrs_bp;
    seds_bp[unit][s] = i_seds_bp;
    cwme_bp[unit][s] = i_cwme_bp;
exit:
    SHR_FUNC_EXIT();
}
