...
precise_key_bits(int u, int m, int v6, uint8_t app, alpm_vrf_type_t vt)
{
    ...
    if (app == APP_LPM) {
        ...
        if (bcmptm_rm_alpm_is_large_vrf(u, m, vt)) {
            if (dev_info && dev_info->max_key_bits) {
                key_bits = dev_info->max_key_bits;
                key_bits -= dev_info->lpm_key_dup_bits[v6];
            } else {
                key_bits = 288;
            }
        }
...
bcmptm_rm_alpm_arg_init(int u, int m, alpm_arg_t *arg)
{
    ...
    if (arg) {
        ...
        arg->comp_large_port = bcmptm_rm_alpm_is_large_l4_port(u, m);
    }
}
