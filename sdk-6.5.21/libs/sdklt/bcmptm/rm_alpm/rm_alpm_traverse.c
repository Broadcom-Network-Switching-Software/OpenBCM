...
snapshot_create(int u, int m, int db, int lt_idx, int max_levels)
{
    ...
    if (app == APP_LPM && vt == VRF_PRIVATE &&
        !bcmptm_rm_alpm_is_large_vrf(u, m, vt)) {
        w_vrf_s = 0;
...
snapshot_get_next(int u, int m, bcmltd_sid_t ltid, alpm_arg_t *arg, int max_levels)
{
    ...
    do {
        ...
        match_arg->db = arg->db;
        match_arg->valid = 1;
        match_arg->lpm_large_vrf = bcmptm_rm_alpm_is_large_vrf(u, m, match_arg->key.vt);
                                         ...
                                         alpm_arg_t *arg, bool del)
{
    ...
    is_src = bcmptm_rm_alpm_db_is_src(u, m, arg->db);
    (void)route_lt_idx_encode(u, m, ltid, arg->key.t.app, arg->key.vt,
                              arg->key.t.ipv, is_src, &lt_idx);
                         ...
                         int max_levels)
{
    ...
    if (snapshot_en) {
        SHR_IF_ERR_EXIT
            (snapshot_get_first(u, m, ltid, arg, max_levels));
    } else {
        SHR_IF_ERR_EXIT
            (snapshot2_get_next(u, m, ltid, arg, max_levels, true));
    }
exit:
    SHR_FUNC_EXIT();
}
