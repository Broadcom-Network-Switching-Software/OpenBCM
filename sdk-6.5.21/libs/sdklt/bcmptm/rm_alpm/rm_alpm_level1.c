...
static int
insert_ripple(int u, int m, uint8_t ldb, int pfx, int dir, int this_pkm,
              int mul);
static int
can_insert_ripple(int u, int m, uint8_t ldb, int pfx, int dir, int this_pkm,
                  int mul);
...
unpair_insert(int u, int m, alpm_arg_t *arg, int upkm)
{
    ...
    if (bcmptm_rm_alpm_pkm_is_pair(u, m, upkm)) {
        SHR_IF_ERR_EXIT(pair_insert(u, m, arg, upkm));
        SHR_EXIT();
    }
...
static bool
comp_key_type(int u, int m, alpm_key_type_t kt)
{
    return (kt == KEY_COMP0_V4 ||
            kt == KEY_COMP0_V6 ||
            kt == KEY_COMP1_V4 ||
            kt == KEY_COMP1_V6);
}
...
pair_shift_down(int u, int m, uint8_t ldb, int pfx, int pkm, int is_last_pfx)
{
    ...
    if (pfx_is_single_half(u, m, ldb, pfx, pkm)) {
        SHR_IF_ERR_EXIT(pair_shift_sh_down(u, m, ldb, pfx, pkm, is_last_pfx));
        SHR_EXIT();
    }
...
pair_shift_up(int u, int m, uint8_t ldb, int pfx, int pkm, int is_last_pfx)
{
    ...
    if (pfx_is_single_half(u, m, ldb, pfx, pkm)) {
        SHR_IF_ERR_EXIT(pair_shift_sh_up(u, m, ldb, pfx, pkm, is_last_pfx));
        SHR_EXIT();
    }
...
pair_free_slot_alloc(int u, int m, uint8_t ldb, int pfx, int *free_slot, int pkm)
{
    ...
    if (pfx_is_single_half(u, m, ldb, pfx, pkm)) {
        SHR_IF_ERR_EXIT(pair_free_slot_alloc_sh(u, m, ldb, pfx, free_slot, pkm));
        SHR_EXIT();
    }
...
pair_free_slot_free(int u, int m, uint8_t ldb, int pfx, int del_slot, int pkm)
{
    ...
    if (pfx_is_single_half(u, m, ldb, pfx, pkm)) {
        SHR_IF_ERR_EXIT(pair_free_slot_free_sh(u, m, ldb, pfx, del_slot, pkm));
        SHR_EXIT();
    }
                  ...
                  int from_pkm, int to_pkm)
{
    ...
    if (v6) {
        /* If IPv4 pfx exists then we should move IPv4, as it's more impact */
...
l1_size(int u, int m)
{
    return L1_BLOCK_END(u, m) * (TCAM_DEPTH(u, m) * 2);
}
...
l1_info_init(int u, int m, alpm_dev_info_t *dev_info, bool recover)
{
    ...
    block_start = L1_BLOCK_START(u, m) * (TCAM_DEPTH(u, m) * 2);
    ...
    assert(block_start == l1_size(u, m));
    sz = l1_size(u, m) * 2 * sizeof(alpm_pivot_t *);
...
/*!
 * \brief Get the single-wide entry number with the given key type and packing mode.
 *
 * \param [in] u Device u.
 * \param [in] m Mtop.
 * \param [in] kt ALPM key type.
 * \param [in] pkm Packing mode.
 *
 * \return singile-wide entry count.
 */
static int
key_type_weight_single(int u, int m, alpm_key_type_t kt, int pkm)
{
    int weight;
...
/*!
 * \brief Ripple entries for insert to a given directon.
 *
 * \param [in] u Device u.
 * \param [in] m Mtop.
 * \param [in] ldb Database.
 * \param [in] in_pfx In Prefix.
 * \param [in] out_pfx Out Prefix.
 * \param [in] dir Direction.
 * \param [in] this_pkm This packing mode.
 * \param [in] next_pkm Next packing mode.
 * \param [out] rem_entry Remaining entry number.
 * \param [in] mul multiple to request entry number.
 *
 * \return SHR_E_XXX
 */
static int
insert_ripple_dir(int u, int m, uint8_t ldb, int in_pfx, int out_pfx,
                  int dir, int this_pkm, int next_pkm,
                  int *rem_entry, int mul)
{
    ...
    SHR_FUNC_ENTER(u);
    do {
        kt = pfx_to_key_type(u, m, out_pfx);
        vt = pfx_to_vrf_type(u, m, out_pfx);
        count_need = half_entry_key_type(u, m, kt, this_pkm) ? 2 : 1;

        if (pkm_free_count(u, m, ldb, kt, vt, next_pkm) >= count_need) {
            SHR_IF_ERR_EXIT(
                ripple_move(u, m, ldb, out_pfx, &rem_cnt,
                            &moved_cnt, this_pkm, next_pkm));

            *rem_entry -= key_type_weight_single(u, m, kt, this_pkm);
        } else {
            if (dir == DIR_UP &&
                half_entry_key_type(u, m, kt, this_pkm) &&
                kt == KEY_IPV6_SINGLE) {
                mul *= 2;
            }
            rv = insert_ripple(u, m, ldb, out_pfx, dir, next_pkm, mul);
            if (SHR_SUCCESS(rv)) {
                ALPM_ASSERT(pkm_free_count(u, m, ldb, kt, vt, next_pkm) >= count_need);
                SHR_IF_ERR_EXIT(
                    ripple_move(u, m, ldb, out_pfx, &rem_cnt,
                                &moved_cnt, this_pkm, next_pkm));
        ...
        if (dir == DIR_UP) {
            rv = can_pkm_move_up(u, m, ldb, in_pfx, &out_pfx, this_pkm, next_pkm);
        } else {
            rv = can_pkm_move_down(u, m, ldb, in_pfx, &out_pfx, this_pkm, next_pkm);
        }
        if (rv == 0) {
...
/*!
 * \brief Ripple entries for insert
 *
 * \param [in] u Device u.
 * \param [in] m Mtop.
 * \param [in] ldb Database.
              ...
              int mul)
{
    ...
    kt = pfx_to_key_type(u, m, pfx);
    rem_entry = key_type_weight_single(u, m, kt, this_pkm) * mul;

    if (dir & DIR_DOWN) {
        for (i = target_i - 1; i >= 0; i--) {
            if (can_pkm_move_down(u, m, ldb, pfx, &out_pfx, this_pkm, pkms[i])) {
                break;
            }
        }
        if (i >= 0) {
            SHR_IF_ERR_EXIT(
                insert_ripple_dir(u, m, ldb, pfx, out_pfx, DIR_DOWN, this_pkm,
                                  pkms[i], &rem_entry, mul));
        }
    ...
    if (rem_entry > 0 && (dir & DIR_UP)) {
        for (i = target_i + 1; i < pkms_cnt; i++) {
            if (can_pkm_move_up(u, m, ldb, pfx, &out_pfx, this_pkm, pkms[i])) {
                break;
            }
        }
        if (i < pkms_cnt) {
            SHR_IF_ERR_EXIT(
                insert_ripple_dir(u, m, ldb, pfx, out_pfx, DIR_UP, this_pkm,
                                  pkms[i], &rem_entry, mul));
        }
    }
...
l1_insert_key_type(int u, int m, alpm_arg_t *arg, int *pkm, int pkm_count)
{
    ...
    for (i = pkm_count - 1; i >= 0; i--) {
        if (pkms.v[pkm[i]]) {
            if (pfx_half_entry(u, m, ldb, pfx, pkm[i])) {
                SHR_IF_ERR_EXIT(unpair_insert(u, m, arg, pkm[i]));
                SHR_EXIT();
            }
        }
    }
    ...
    for (i = pkm_count - 1; i >= 0; i--) {
        if (rv == SHR_E_RESOURCE) {
            if (pkms.v[pkm[i]]) {
                rv = insert_ripple(u, m, ldb, pfx, DIR_BOTH, pkm[i], 1);
                if (SHR_SUCCESS(rv)) {
                    SHR_IF_ERR_EXIT(unpair_insert(u, m, arg, pkm[i]));
                    SHR_EXIT();
                }
...
/*!
 * \brief Can ripple entries for insert to a given directon.
 *
 * \param [in] u Device u.
 * \param [in] m Mtop.
 * \param [in] ldb Database.
 ...
 */
static int
can_insert_ripple_dir(int u, int m, uint8_t ldb, int in_pfx, int out_pfx,
                      int dir, int this_pkm, int next_pkm,
                      int *rem_entry, int mul)
{
    uint8_t count_need;
    ...
    do {
        kt = pfx_to_key_type(u, m, out_pfx);
        vt = pfx_to_vrf_type(u, m, out_pfx);
        count_need = (half_entry_key_type(u, m, kt, this_pkm) ? 2 : 1) * mul;
        ...
        if (pkm_free_count(u, m, ldb, kt, vt, next_pkm) >= count_need) {
            *rem_entry -= key_type_weight_single(u, m, kt, this_pkm);
            ...
            rv = can_insert_ripple(u, m, ldb, out_pfx, dir, next_pkm, mul);
            if (SHR_SUCCESS(rv)) {
                *rem_entry -= key_type_weight_single(u, m, kt, this_pkm);
            } else {
                break;
...
/*!
 * \brief Can ripple entries for insert
 *
 * \param [in] u Device u.
 * \param [in] m Mtop.
 * \param [in] ldb Database.
                  ...
                  int mul)
{
    ...
    if (dir & DIR_DOWN) {
        ...
        if (i >= 0) {
            SHR_IF_ERR_EXIT(
                can_insert_ripple_dir(u, m, ldb, pfx, out_pfx, DIR_DOWN, this_pkm,
                                      pkms[i], &rem_entry, mul));
        }
    }
    ...
    if (rem_entry > 0 && (dir & DIR_UP)) {
        ...
        if (i < pkms_cnt) {
            SHR_IF_ERR_EXIT(
                can_insert_ripple_dir(u, m, ldb, pfx, out_pfx, DIR_UP, this_pkm,
                                      pkms[i], &rem_entry, mul));
        }
    }
...
can_insert_key_type(int u, int m, alpm_arg_t *arg, int *pkm, int pkm_count)
{
    ...
    for (i = pkm_count - 1; i >= 0; i--) {
        if (!ok) {
            if (pkms.v[pkm[i]]) {
                ok = SHR_SUCCESS(
                    can_insert_ripple(u, m, ldb, arg->key.pfx, DIR_BOTH, pkm[i], 1));
            }
        } else {
            break;
        }
...
unpair_recover(int u, int m, uint8_t ldb, int pkm)
{
    ...
    while (index < low_bound) {
        /* Clear ip mask to prevent pollution in large vrf mode. */
        if (bcmptm_rm_alpm_is_large_vrf(u, m, VRF_PRIVATE)) {
            sal_memset(arg->key.ip_mask, 0, sizeof(arg->key.ip_mask));
            sal_memset(arg2->key.ip_mask, 0, sizeof(arg->key.ip_mask));
        }
...
bcmptm_rm_alpm_l1_can_insert(int u, int m, alpm_arg_t *arg)
{
    ...
    free_count = l1_free_count(u, m, arg->db, kt, vt);
    if (free_count < 10) {
        arg->near_full = 1;
    }
    if (free_count >= 4) {
...
bcmptm_rm_alpm_l1_usage_get(int u, int m, bcmptm_cth_alpm_usage_info_t *info)
{
    ...
    } else if (comp_key_type(u, m, info->kt)) {
        if (alpm_mode & ALPM_MODE_FP_COMP) {
            if (!info->is_src) {
                chk_dbs[L1_DB2] = 1;
            } else {
