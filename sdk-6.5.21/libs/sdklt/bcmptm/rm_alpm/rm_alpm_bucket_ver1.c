...
#define MERGE_RATIO                 0.92

#define COMPRESS_WHOLE     2
#define COMPRESS_PARTIAL   1
...
typedef struct base_buckets_ver1_s {
    ...
    int8_t fetched[ALPM_MAX_BANKS];
    int8_t pending[ALPM_MAX_BANKS];
    /*
...
typedef struct bucket_merge_ctrl_s {
    ...
    int freed_rbkts;
    int max_freed_rbkts;
    uint8_t cautious_merge;
    alpm_pivot_t *skip_pivots[MAX_LEVELS];
    rm_alpm_trie_t *pivot_trie[MAX_LEVELS];
    int successful_merges;
    int abort_merges;
    int index_alloc_failures;
    int skip_pivot_failures;
    int parent_pivot_failures;
    int invalid_group_failures;
...
static int rbkt_compress = 1; /* do rbkt compress */
static int split_alloc_move = 1; /* 0: alloc move first, 1: do it second */
static int merge_alloc_move = 1; /* 0: disable, 1: enable */
static int recursive_max_tries = 100000;
static int copy_alloc_enable = 1;
                       ...
                       int len, alpm1_data_t *a1)
{
    ...
    a1->kshift = len;
    banks = RBKT_VBANKS(u, m, db, ln, group) +
            RBKT_FBANKS(u, m, db, ln, group);
    if (bkt_hdl->max_banks < banks) {
        banks = bkt_hdl->max_banks;
    }
    /*
     * It's possible to have non-contiguous banks for some
     * in-middle operations.
     */
    for (i = 0; i < banks; i++) {
        phy_bank = bkt_hdl->l2p_bank_array[log_bank];
...
static void
ver1_base_buckets_init(base_buckets_ver1_t *bbkts)
{
    sal_memset(bbkts->fetched, 0, sizeof(bbkts->fetched));
}
                              ...
                              int *map_ent_ofs)
{
    ...
    SHR_DQ_TRAVERSE(&bkt->list, elem)
        ...
        if (RBKT_FROM_INDEX(index) == srbkt_idx) {
            if (map_ent_ofs) {
                if (map_ent_ofs[ENT_OFS_FROM_IDX(index)] == -1) {
                    continue;
                }
                                 ...
                                 int diff)
{
    ...
    free_count = ALPM_MIN(BKT_HDL_MAX_FREE_COUNT(u, m, db, ln), free_count);
 ...
 * \param [in] write_hw Write dest rbkt to HW.
 * \param [in] update_fmt Update format beforehand, don't set unless necessary.
 *
 * \return SHR_E_NONE/SHR_E_XXX.
 */
               ...
               bool update_fmt)
{
    ...
    if (write_hw) {
        uint32_t shw_idx, dhw_idx;


        /*
         * If the caller says update, update.
         * If the caller does not say update,
         * we still update if the dst_rbkt is active with a different
         * format.
 */
        if (update_fmt ||
            (dst_rbkt->format != DISABLED_FORMAT &&
             dst_rbkt->format != src_rbkt->format)) {
            /* Clear dst format first, as to prevent flow issue */
            dst_rbkt->format = DISABLED_FORMAT;
            SHR_IF_ERR_EXIT(
        ...
        if (src_group == INVALID_GROUP) {
            src_bank = 0;
        } else {
            src_bank = srbkt_idx - src_group;
        }
        if (sbbkts == NULL || !sbbkts->fetched[src_bank]) {
            if (sbbkts == NULL) {
                bbkts = shr_lmm_alloc(base_buckets_lmem[u][m]);
                if (bbkts == NULL) {
                    SHR_ERR_EXIT(SHR_E_MEMORY);
                }
                ver1_base_buckets_init(bbkts);
                sbbkts = bbkts;
            }
            shw_idx = ver1_hw_idx_from_rbkt_idx(u, m, db, ln, srbkt_idx);
            SHR_IF_ERR_EXIT(
                bcmptm_rm_alpm_read_ln_raw(u, m, db, ln, shw_idx,
                                           &sbbkts->rbuf[src_bank][0],
                                           &sbbkts->dt_bitmap[src_bank]));
            sbbkts->fetched[src_bank] = 1;
            SHR_IF_ERR_EXIT(
                bcmptm_rm_alpm_write_ln_raw(u, m, db, ln, dhw_idx,
                                            &sbbkts->rbuf[src_bank][0],
                                            HIT_OP_AUTO,
                                            src_rbkt->entry_bitmap,
                                            sbbkts->dt_bitmap[src_bank]));
        } else {
            ALPM_ASSERT(sbbkts->fetched[src_bank]);
            SHR_IF_ERR_EXIT(
                bcmptm_rm_alpm_write_ln_raw(u, m, db, ln, dhw_idx,
                                            &sbbkts->rbuf[src_bank][0],
                                            HIT_OP_AUTO,
 ...
 */
static int
ver1_rbkt_compress(int u, int m, int db, int ln,
                   rbkt_idx_t srbkt_idx,
                   rbkt_idx_t drbkt_idx,
                   rbkt_group_t group,
                   bool same_fmt_only,
...
/*    ALPM_ASSERT(srbkt_idx > drbkt_idx); */
    ...
    if (same_fmt_only) {
        if (src_rbkt->format == dst_rbkt->format) {
            src_total = dst_total;
            ok_to_compress = true;
        }
    } else {
    ...
    sbuf = &in_bbkts->rbuf[src_bank][0];
    dbuf = &in_bbkts->rbuf[dst_bank][0];
    if (!in_bbkts->fetched[src_bank]) {
        shw_idx = ver1_hw_idx_from_rbkt_idx(u, m, db, ln, srbkt_idx);
        SHR_IF_ERR_EXIT(
            bcmptm_rm_alpm_read_ln_raw(u, m, db, ln, shw_idx, sbuf,
                                       &in_bbkts->dt_bitmap[src_bank]));
        in_bbkts->fetched[src_bank] = 1;
    }
    if (!in_bbkts->fetched[dst_bank]) {
        dhw_idx = ver1_hw_idx_from_rbkt_idx(u, m, db, ln, drbkt_idx);
        SHR_IF_ERR_EXIT(
            bcmptm_rm_alpm_read_ln_raw(u, m, db, ln, dhw_idx, dbuf,
                                       &in_bbkts->dt_bitmap[dst_bank]));
        in_bbkts->fetched[dst_bank] = 1;
    }


    for (j = 0; j < src_total; j++) {
        ent_ofs_map[j] = -1;
    ...
    for (i = 0; i < dst_total; i++) {
        ...
        while (j < src_total) {
            if (ALPM_BIT_GET(src_rbkt->entry_bitmap, j)) {
                ...
                rv = bcmptm_rm_alpm_ln_base_entry_move(u, m, format_type,
                                                       src_rbkt->format,
                                                       j,
                                                       &pvt->key.t,
                                                       sbuf,
                ...
                ent_ofs_map[j] = i;
                j++;
                break; /* This dst entry is done, advance to next */
            }
            j++;
        }
    }
    ...
    if (src_rbkt->entry_bitmap == 0) {
        ...
        *compress = COMPRESS_WHOLE;
    } else {
        *compress = COMPRESS_PARTIAL;
    }
    ...
    SHR_IF_ERR_EXIT(
        bcmptm_rm_alpm_write_ln_raw(u, m, db, ln, dhw_idx,
                                    dbuf,
                                    HIT_OP_AUTO,
                                    in_bbkts->modify_bitmap[dst_bank],
                                    in_bbkts->dt_bitmap[dst_bank]
                                    ));
    in_bbkts->pending[dst_bank] = 0; /* Data written */
    in_bbkts->pending[src_bank] = (*compress == COMPRESS_PARTIAL); /* Bank empty if COMPRESS_WHOLE */

    bcmptm_rm_alpm_internals[u][m][db][ln].compress_write++;
                       ...
                       bool with_holes, int vbanks)
{
    ...
    for (; i < rbkt_group->valid_banks + rbkt_group->free_banks; i++) {
        rbkt = rbkt_group + i;
        assert(rbkt->format == 0);
        assert(rbkt->entry_bitmap == 0);
    }
    assert(!pivot ||
                         ...
                         base_buckets_ver1_t *sbbkts)
{
    ...
    int j, i;
    int compress;
    bool format_update_on_first_copy;
    uint32_t hw_idx;
    bool first_same_fmt_only = second_compress;
    ...
    if (rbkt_compress) {
        ...
        for (i = vbanks - 1; i > 0; i--) {
            ...
            for (j = 0; j < i; j++) {
                ...
                SHR_IF_ERR_EXIT(
                    ver1_rbkt_compress(u, m, db, ln, src_rbkt, dst_rbkt, group,
                                       first_same_fmt_only, whole_only,
                                       &compress, sbbkts));
                if (compress == COMPRESS_WHOLE) { /* src_rbkt is gone, advance to next  */
                    *format_update = true;
                    break;
                }
            }
        ...
        if (second_compress) {
            ...
            for (i = vbanks - 1; i > 0; i--) {
                ...
                for (j = 0; j < i; j++) {
                    ...
                    compress = COMPRESS_NONE;
                    if (RBKT_FORMAT(u, m, db, ln, dst_rbkt) >
                        RBKT_FORMAT(u, m, db, ln, src_rbkt)) {
                        SHR_IF_ERR_EXIT(
                            ver1_rbkt_compress(u, m, db, ln, dst_rbkt, src_rbkt, group,
                                               false, whole_only, /* whole_only */
                                               &compress, sbbkts));
                        if (compress == COMPRESS_WHOLE) { /* dst_rbkt is gone, advance to next  */
                            *format_update = true;
                            continue;
                        }
                    }
    ...
    /*
     * Needs of format update could be accumulated due to split_move and
     * rbkt_compress. The fmt update can be held off until now, because so far
     * it's just valid FMT to invalid FMT.
     * The next to do is rbkts reodering, in which the forehand pivot update
     * is determined by src and dest rbkt format, assuming the HW is reflecting
     * the same. If the accumulated format_update is true, the HW is really not
     * reflecting the same, specifically, the SW states says disable, but it's
     * not in HW.
     * So we need to force a pivot update if accumulated needs say so.
     */
    format_update_on_first_copy = *format_update;
    ...
    for (i = vbanks - 1; i > j; i--) {
        ...
        while (j < i) {
            ...
            if (!RBKT_EBMP(u, m, db, ln, dst_rbkt)) {
                ...
                SHR_IF_ERR_EXIT(
                    ver1_rbkt_copy(u, m, db, ln, src_rbkt, dst_rbkt, sbbkts,
                                   group, true, true,
                                   format_update_on_first_copy));
                if (sbbkts) {
                    sbbkts->pending[dst_rbkt - group] = 0; /* Bucket written */
                    sbbkts->pending[src_rbkt - group] = 0; /* Bucket is cleared */
                }

                bcmptm_rm_alpm_internals[u][m][db][ln].compress_write++;

                j++;
                format_update_on_first_copy = false;
...
static int
ver1_rbkt_group_traverse_compress(int u, int m, int db, int ln,
                                  bool second_compress,
                                  bool whole_only,
                                  int stop_count, int *freed_rbkts)
{
    int free_count;
    shr_dq_t *group_list;
    shr_dq_t *elem = NULL;
    ...
    SHR_FUNC_ENTER(u);
#ifdef ALPM_DEBUG
    bcmptm_rm_alpm_buckets_sanity_by_group(u, m, db, ln);
#endif
    *freed_rbkts = 0;
    ...
    sal_memset(bbkts->pending, 0, sizeof(bbkts->pending));

    for (free_count = 0; free_count <= BKT_HDL_MAX_FREE_COUNT(u, m, db, ln);
         free_count++) {
        ...
        SHR_DQ_TRAVERSE(group_list, elem)
            ...
            ver1_base_buckets_init(bbkts);
            SHR_IF_ERR_EXIT(
                ver1_rbkt_group_compress(u, m, db, ln, group,
                                         vbanks, second_compress,
                                         whole_only,
                                         &format_update, bbkts));

            if (format_update && pivot) {
                rbkts_compress = vbanks - RBKT_VBANKS(u, m, db, ln, group);
                ALPM_ASSERT(rbkts_compress);
                SHR_IF_ERR_EXIT(
            ...
            if (rbkts_compress) {
                for (i = RBKT_VBANKS(u, m, db, ln, group);
                     i < vbanks; i++) {
                     rbkt = curr + i;
                     rbkt->entry_bitmap = 0;
                }
            ...
            *freed_rbkts += rbkts_compress;
            if (*freed_rbkts >= stop_count) {
                break;
            }
                     ...
                     rbkt_group_t *free_group)
{
    ...
    for (i = 0; i < vbanks; i++) {
         ...
         */
        SHR_IF_ERR_EXIT(
            ver1_rbkt_copy(u, m, db, ln, src_group + i, *new_group + i,
                           NULL, INVALID_GROUP, TRUE, true, false));
    }
                      ...
                      )
{
    ...
    if (LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
        cli_out("append: %d(v=%d, f=%d) <-- %d(v=%d, f=%d)\n",
                prev_group,
                RBKT_VBANKS(u, m, db, ln, prev_group),
                RBKT_FBANKS(u, m, db, ln, prev_group),
                curr_group,
    ...
    while (drbkt_idx < curr_group && curr_group <= srbkt_idx) {
         ...
         */
        SHR_IF_ERR_EXIT(
            ver1_rbkt_copy(u, m, db, ln, srbkt_idx, drbkt_idx,
                           NULL, INVALID_GROUP, TRUE, true, false));
        srbkt_idx--;
        drbkt_idx++;
    }
...
#ifdef ALPM_DEBUG
    if (LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
        cli_out("prepend: %d(v=%d, f=%d) --> %d(v=%d, f=%d)\n",
                curr_group,
                RBKT_VBANKS(u, m, db, ln, curr_group),
                RBKT_FBANKS(u, m, db, ln, curr_group),
                new_group,
                RBKT_VBANKS(u, m, db, ln, new_group),
                RBKT_FBANKS(u, m, db, ln, new_group)
                );
    }
#endif
                          ...
                          )
{
     ...
     * - Otherwise, fails.
     */
    for (free_count = BKT_HDL_MAX_FREE_COUNT(u, m, db, ln); free_count >= 0;
         free_count--) {
        group_list = &GROUP_USAGE_LIST(u, m, db, ln, free_count);

        SHR_DQ_TRAVERSE(group_list, elem)
            curr = SHR_DQ_ELEMENT_GET(rbkt_t*, elem, usgnode);
            if (curr->index == group) { /* skip busy group */
                continue;
            }
            i = 1;
...
ver1_free_rbkt_group_first_get(int u, int m, int db, int ln, int size)
{
    ...
    for (free_count = BKT_HDL_MAX_FREE_COUNT(u, m, db, ln); free_count >= size;
         free_count--) {
        group_list = &GROUP_USAGE_LIST(u, m, db, ln, free_count);
                      ...
                      rbkt_group_t *new_group)
{
     ...
     */
    for (free_count = max_banks * 2; free_count >= size; free_count--) {
                                  ...
                                  rbkt_group_t *new_group)
{
    ...
    while (rv == SHR_E_RESOURCE) {
        rv = ver1_rbkt_group_alloc(u, m, db, ln, group, best_size, FALSE, new_group);
        if (best_size <= least_size) {
            if (rv == SHR_E_RESOURCE) {
                rv = ver1_rbkt_group_alloc(u, m, db, ln, group,
                                           shuffle_size,
                                           TRUE,
                                           new_group);
...

{
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        entry_arg->lpm_large_vrf = arg->lpm_large_vrf;
                     ...
                     bool *format_update)
{
    ...
    if (rbkt->entry_bitmap == 0) {
        ...
        if (compress && last_bkt != remove_bkt) {
             ...
             */
            SHR_IF_ERR_EXIT(
                ver1_rbkt_copy(u, m, db, ln, last_bkt, remove_bkt,
                               NULL, INVALID_GROUP, TRUE, true, false));
            SHR_IF_ERR_EXIT(
 ...
 * \param [in] max_len Max IP length of requesting entry.
 ...
 */
static int
ver1_base_entry_best_format(int u, int m, int db, int ln,
                            int entry_len,
                            int max_len,
                            int entry_full,
                            int *format_value)
                      ...
                      int *format_value)
{
    ...
    int best_format;
    int vbanks;
    uint32_t useable_free_banks;

    SHR_FUNC_ENTER(u);

    vbanks = RBKT_VBANKS(u, m, db, ln, group);

    if (RBKT_FBANKS(u, m, db, ln, group) && vbanks < BKT_HDL_MAX_BANKS(u, m, db, ln)) {
        useable_free_banks = BKT_HDL_MAX_BANKS(u, m, db, ln) - vbanks;
        if (useable_free_banks < RBKT_FBANKS(u, m, db, ln, group)) {
            useable_free_banks = RBKT_FBANKS(u, m, db, ln, group);
        }
        SHR_IF_ERR_EXIT(
            ver1_base_entry_best_format(u, m, db, ln,
                                        entry_len,
                                        max_len,
                                        entry_full,
                                        &best_format));
    } else {
        useable_free_banks = 0;
        best_format = INVALID_FORMAT;
    }
        ...
        if (rbkt->format < min_format) {
            SHR_IF_ERR_EXIT(
                bcmptm_rm_alpm_ln_base_entry_info(u, m, format_type,
                                                  ...
                                                  &full,
                                                  &number));
            ALPM_ASSERT((uint32_t)((1 << number) - 1) >= rbkt->entry_bitmap);
    ...
    /* Assign free banks
     * 1) best_format exists. AND
     * 2.1) no min_format OR
     * 2.2) min_format exists but dt not match OR
     * 2.3) min_format exists, dt matches, still a little waste and
     *      we have sufficient useable free banks (> provided threshold ).
     */
    if (best_format != INVALID_FORMAT &&
        (min_format == INVALID_FORMAT || entry_full != min_full ||
         (min_format > best_format && useable_free_banks > new_fmt_min_fbanks))) {
        rbkt_idx = group + vbanks;
        SHR_IF_ERR_EXIT(
            ver1_rbkt_expand_right(u, m, db, ln, group));

        *format_value = best_format;
        RBKT_FORMAT(u, m, db, ln, rbkt_idx) = best_format;
        ALPM_BIT_SET(RBKT_EBMP(u, m, db, ln, rbkt_idx), 0);
        *ent_idx = ALPM_INDEX_ENCODE_VER1(rbkt_idx, 0);
        if (format_update) {
            *format_update = TRUE;
...
static int
ver1_bucket_indexes_copy_alloc(int u, int m, int db, int ln,
                               rbkt_group_t src_group,
                               rbkt_group_t dst_group,
                               shr_dq_t *list
                               )
{
    ...
    SHR_DQ_BACK_TRAVERSE(list, elem)
        ...
        src_rbkt = RBKT_FROM_INDEX(bkt_elem->index);
        src_bank = src_rbkt - src_group;
        src_format = RBKT_FORMAT(u, m, db, ln, src_rbkt);

        rv = ver1_base_entry_assign(u, m, db, ln, dst_group,
                                    dst_group + src_bank,
                                    src_format, &dst_idx);
        if (SHR_FAILURE(rv)) {
            if (LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
                cli_out("ver1_base_entry_copy_alloc failed "\
                        "with db=%d ln=%d group=%d\n",
                        db, ln, dst_group);
            }
            break;
        }
                          ...
                          )
{
    ...
    SHR_DQ_BACK_TRAVERSE(list, elem)
        ...
        rv = ver1_base_entry_alloc(u, m, db, ln, dst_group,
                                   bkt_elem->key.t.len - dst_t->len,
                                   bkt_elem->key.t.max_bits - dst_t->len,
        ...
        if (SHR_FAILURE(rv)) {
            if (LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
                cli_out("ver1_base_entry_alloc failed "\
                        "with db=%d ln=%d group=%d full=%d count=%d\n",
                        db, ln, dst_group, full, count);
            }
            break;
        }
        i++;
    ...
    if (SHR_FAILURE(rv) && i) {
        ...
        SHR_DQ_TRAVERSE_END(list, elem);
        if (recursive) { /* Useful for non-mixed data type as well. */
            int trys = 0;
            ...
            if (LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
                cli_out("db=%d ln=%d dst_group %d trys %d rv %d full_1=%d\n",
                        db, ln, dst_group, trys, rv, full_1);
            }
        }
    }
    SHR_IF_ERR_EXIT(rv);
 ...
 * \param [in] dst_valid_banks 0 to reset, -1 to do nothing, else read.
 *
 * \return SHR_E_NONE/SHR_E_XXX.
 */
static int
                 ...
                 )
{
    ...
    SHR_DQ_BACK_TRAVERSE(list, elem)
        ...
        if (src_idx != INVALID_INDEX) {
            ...
            sbbkts->pending[src_bank] = 1; /* Mark rbkts need update */
        } else {
                 ...
                 )
{
    ...
    /* Write dest rbkts */
    for (i = 0; i < RBKT_VBANKS(u, m, db, ln, dst_group); i++) {
        dst_rbkt = dst_group + i;

        if (dst_rbkts[i] ||
            RBKT_FORMAT(u, m, db, ln, dst_rbkt) != DISABLED_FORMAT) {
            if (split_aux) {
                bcmptm_rm_alpm_internals[u][m][db][ln].split_move_write++;
            }

            hw_idx = ver1_hw_idx_from_rbkt_idx(u, m, db, ln, dst_rbkt);
            SHR_IF_ERR_EXIT(
                bcmptm_rm_alpm_write_ln_raw(u, m, db, ln, hw_idx,
                                            &dbbkts->rbuf[i][0],
                                            HIT_OP_AUTO,
...
ver1_bucket_merge(int u, int m, int db, int ln,
                  ...
                  int parent_valid_banks,
                  rbkt_group_t old_parent_group)
{
    ...
    ver1_base_buckets_init(dbbkts);
    if (old_parent_group != parent_group) {
        ver1_base_buckets_init(sbbkts);
        count = bcmptm_rm_alpm_trie_count(parent_pivot->bkt->trie);
        ...
        SHR_IF_ERR_VERBOSE_EXIT(
            ver1_bucket_move(u, m, db, ln,
                             ...
                             parent_group,
                             &parent_pivot->key.t,
                             &parent_pivot->bkt->list,
                             count,
                             NULL, /* No new entry */
                             NULL, /* Not split move */
                             NULL, /* Will update pivot anyway */
                             NULL,
                             sbbkts,
                             1, /* sbbkts_needs_read */
                             0, /* reset dbbkts*/
        ...
        ver1_bucket_indexes_set(u, m, db, ln, &parent_pivot->bkt->list);

        SHR_IF_ERR_EXIT(
            ver1_rbkt_group_free(u, m, db, ln, old_parent_group));

        parent_valid_banks = -1; /* to skip reset or read */
    }
    ...
    VER1_RBKT_GROUP_SANITY(u, m, db, ln, old_parent_group, false, 0);
    VER1_RBKT_GROUP_SANITY(u, m, db, ln, child_group, false, 0);

exit:
    ALPM_ASSERT(!SHR_FUNC_ERR() || SHR_FUNC_VAL_IS(SHR_E_MEMORY));
    if (sbbkts) {
        shr_lmm_free(base_buckets_lmem[u][m], sbbkts);
    }
    if (dbbkts) {
                     ...
                     rm_alpm_trie_traverse_states_e_t *state, void *info)
{
    ...
    int parent_valid_banks;
    int freed_rbkts;
    rbkt_group_t new_parent_group = INVALID_GROUP;
    enum {
        MERGE_ALLOC_AND_MOVE_NONE,
        MERGE_ALLOC_AND_MOVE_ALLOC,
        MERGE_ALLOC_AND_MOVE_INDEX_ALLOC_PARENT,
        MERGE_ALLOC_AND_MOVE_INDEX_ALLOC_CHILD
    } done = MERGE_ALLOC_AND_MOVE_NONE;
    uint8_t alloc_and_move = 0;
    ...
    *state = TRAVERSE_STATE_NONE;
    ...
    if (!parent_pivot || !child_pivot ||
        merge_ctrl->skip_pivots[prev_ln] == parent_pivot ||
        merge_ctrl->skip_pivots[prev_ln] == child_pivot
       ) {
        merge_ctrl->skip_pivot_failures++;
        SHR_EXIT();
    }
#ifdef ALPM_DEBUG
    {
    ...
    if (ln == LEVEL3) {
        ...
        if (l1_pivot->key.t.len > parent_pivot->key.t.len) {
            merge_ctrl->parent_pivot_failures++;
            SHR_EXIT();
        }
    }
    ...
    if (parent_group == INVALID_GROUP ||
        ...
        merge_ctrl->invalid_group_failures++;
        SHR_EXIT();
    }
    count = bcmptm_rm_alpm_trie_count(child_pivot->bkt->trie);
    ...
    } else if (rv == SHR_E_RESOURCE && !merge_ctrl->cautious_merge
               && merge_alloc_move) {

        int max_banks = BKT_HDL_MAX_BANKS(u, m, db, ln);
        if (RBKT_VBANKS(u, m, db, ln, parent_group) < max_banks) {
            alloc_and_move = 1;
            rv = ver1_rbkt_group_alloc_best_effort(u, m, db, ln,
                                                   INVALID_GROUP,
                                                   max_banks,
                                                   max_banks,
                                                   max_banks,
                                                   &new_parent_group);
            if (rv == SHR_E_RESOURCE) {
                /* Not a true error, no SW state changed. */
                SHR_EXIT();
            } else {
                SHR_IF_ERR_EXIT(rv);
            }
            /* group might be moved during group alloc process */
            parent_group = VER1_GROUP(parent_pivot->bkt->log_bkt);
            parent_valid_banks = RBKT_VBANKS(u, m, db, ln, parent_group);
            child_group = VER1_GROUP(child_pivot->bkt->log_bkt);

            VER1_RBKT_PIVOT(u, m, db, ln, new_parent_group) = parent_pivot;
            parent_pivot->bkt->log_bkt = VER1_LOG_BKT(new_parent_group);
            op_count = bcmptm_rm_alpm_trie_count(parent_pivot->bkt->trie);
            done = MERGE_ALLOC_AND_MOVE_ALLOC;
            ...
            done = MERGE_ALLOC_AND_MOVE_INDEX_ALLOC_PARENT;
    ...
    if (SHR_SUCCESS(rv)) {
        done = MERGE_ALLOC_AND_MOVE_INDEX_ALLOC_CHILD;
        freed_rbkts = RBKT_VBANKS(u, m, db, ln, child_group) + parent_valid_banks
                     -RBKT_VBANKS(u, m, db, ln, new_parent_group);

        LOG_V(("Merge: P:%d(%d) + C:%d(%d) -> NP:%d(%d) banks, freed banks=%d ",
               parent_group,
               parent_valid_banks,
               child_group,
        ...
        /*
         * For the cautious_merge, we won't do the first merge unless
         * the first merge alone is sufficient (>= max_freed_rbkts).
         * This is to ensure that once we do merge, we must know
         * it's going to a successful operation.
         * For the non-cautious_merge merge, as long as freed_rbkts is
         * non-zero, we can do it.
 */
        if ((!merge_ctrl->cautious_merge && freed_rbkts >= 0) ||
            (merge_ctrl->cautious_merge && freed_rbkts >= merge_ctrl->max_freed_rbkts)) {
            SHR_IF_ERR_EXIT(
                ver1_bucket_merge(u, m, db, ln, parent_pivot, child_pivot,
                                  parent_valid_banks, parent_group));
            *state = TRAVERSE_STATE_DELETED;
            merge_ctrl->freed_rbkts += freed_rbkts;
            if (merge_ctrl->freed_rbkts >= merge_ctrl->max_freed_rbkts) {
                *state = TRAVERSE_STATE_DONE;
            }
            merge_ctrl->successful_merges++;
        } else {
            if (alloc_and_move) {
                ver1_bucket_indexes_free(u, m, db, ln, new_parent_group,
                                         &child_pivot->bkt->list, count);
                ver1_bucket_indexes_free(u, m, db, ln, new_parent_group,
                                         &parent_pivot->bkt->list, op_count);
                parent_pivot->bkt->log_bkt = VER1_LOG_BKT(parent_group);
                ver1_rbkt_group_free(u, m, db, ln, new_parent_group);
            } else {
                ver1_bucket_indexes_free(u, m, db, ln, parent_group,
                                         &child_pivot->bkt->list, count);
            }
            merge_ctrl->abort_merges++;
        }
        LOG_V(("After:used=%d total=%d free=%d\n",
               BKT_HDL_RBKTS_USED(u, m, db, ln),
               BKT_HDL_RBKTS_TOTAL(u, m, db, ln),
               BKT_HDL_RBKTS_TOTAL(u, m, db, ln) -
               BKT_HDL_RBKTS_USED(u, m, db, ln)
...
exit:
    /*
     * E_MEMORY is not expected error, when that happens,
     * we really cannot properly rollback, so we just
     * don't try. Also when E_MEMORY happens, it's possible
     * that the child pivot is deleted, in which case,
     * we should not traverse bkt list to free indexes, because
     * the bkt list is also freed.
     */

    if (SHR_FAILURE(rv) || (SHR_FUNC_ERR() && !SHR_FUNC_VAL_IS(SHR_E_MEMORY))) {

        /*
         * In fact, no following roll back is not expected to happen,
         * except for done == MERGE_ALLOC_AND_MOVE_NONE, which equals to
         * nothing to roll back.
         */
        if (alloc_and_move) {
            switch (done) {
            case MERGE_ALLOC_AND_MOVE_INDEX_ALLOC_CHILD:
                ver1_bucket_indexes_free(u, m, db, ln, new_parent_group,
                                         &child_pivot->bkt->list, count);
            case MERGE_ALLOC_AND_MOVE_INDEX_ALLOC_PARENT:
                if (done == MERGE_ALLOC_AND_MOVE_INDEX_ALLOC_PARENT) {
                    merge_ctrl->index_alloc_failures++;
                }
                ver1_bucket_indexes_free(u, m, db, ln, new_parent_group,
                                         &parent_pivot->bkt->list, op_count);
            case MERGE_ALLOC_AND_MOVE_ALLOC:
                parent_pivot->bkt->log_bkt = VER1_LOG_BKT(parent_group);
                ver1_rbkt_group_free(u, m, db, ln, new_parent_group);
            case MERGE_ALLOC_AND_MOVE_NONE:
            default:
                break;
            }
            if (new_parent_group != INVALID_GROUP) {
                VER1_RBKT_GROUP_SANITY(u, m, db, ln, new_parent_group, false, 0);
            }
        } else {
            if (done == MERGE_ALLOC_AND_MOVE_INDEX_ALLOC_CHILD) {
                ver1_bucket_indexes_free(u, m, db, ln, parent_group,
                                         &child_pivot->bkt->list, count);
            }
        }
 ...
 * \param [in] db Database
 * \param [in] ln Level
 * \param [in] stop_count Max number of merges.
 * \param [in] skip_pivots Pivots need to be skipped.
 * \param [out] freed_rbkts Merge stop count.
 * \param [out] new_arg
 *
 * \return SHR_E_NONE/SHR_E_XXX.
 */
                           ...
                           bool cautious_merge)
{
    ...
    ctrl.db = db;
    ctrl.ln = ln;
    ctrl.max_freed_rbkts = stop_count;
    ctrl.cautious_merge = cautious_merge;
    if (skip_pivots) {
        sal_memcpy(ctrl.skip_pivots, skip_pivots, sizeof(ctrl.skip_pivots));
    }
        ...
        SHR_DQ_TRAVERSE(&DB_VRF_HDL_LIST(u, m, db, ipv), elem)
            ...
            SHR_IF_ERR_EXIT(
                bcmptm_rm_alpm_trie_traverse2(pivot_trie->trie,
                                              ...
                                              &ctrl,
                                              TRAVERSE_POSTORDER));
            accomplished = (ctrl.freed_rbkts >= ctrl.max_freed_rbkts);
            if (accomplished) {
                break;
            }
    ...
    if (ln != DB_LAST_LEVEL(u, m, db) && cautious_merge) {
        if (accomplished) {
            /*
             ...
             */
            l3_freed_rbkts = 0;
            next_ln = NEXT_LEVEL(ln);
            stop_count2 = BKT_HDL_RBKTS_TOTAL(u, m, db, next_ln) / 50;
            SHR_IF_ERR_EXIT(
                ver1_bucket_traverse_merge(u, m, db, next_ln,
                                           stop_count2,
                                           skip_pivots, &l3_freed_rbkts, false));
            /*
             * Further do level-2 buckets merges, so that we won't
             * kick-off level-3 merge frequently.
             */
            stop_count2 = BKT_HDL_RBKTS_TOTAL(u, m, db, ln) / 50;
            SHR_IF_ERR_EXIT(
                ver1_bucket_traverse_merge(u, m, db, ln,
                                           stop_count2,
                                           skip_pivots, freed_rbkts, false));
            if ((*freed_rbkts + ctrl.freed_rbkts) < ctrl.max_freed_rbkts) {
                ALPM_ASSERT(0);
                SHR_IF_ERR_EXIT(SHR_E_INTERNAL);
            }
        }
    ...
    if (freed_rbkts) {
        *freed_rbkts += ctrl.freed_rbkts;
    }
#ifdef ALPM_DEBUG
    if (LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        bcmptm_rm_alpm_buckets_sanity_by_trie(u, m, db, ln, sanity_vrf_traverse_merge, false);
    }
#endif
...
ver1_base_entry_encode_write(int u, int m, int ln, alpm_arg_t *arg)
{
    ...
    if (LOG_CHECK_INFO(BSL_LOG_MODULE) && !arg->encode_data_only) {
        ...
        sal_memcpy(&arg2.key.t.trie_ip,
                   ...
                   sizeof(trie_ip_t));
        arg2.lpm_large_vrf = arg->lpm_large_vrf;
        SHR_IF_ERR_EXIT(
            bcmptm_rm_alpm_ln_base_entry_decode(u, m, format_type,
                                                rbkt->format,
                                                pivot_len,
        ...
        for (i = 0; i < COUNTOF(arg->key.ip_addr); i++) {
            arg->key.ip_addr[i] &= arg->key.ip_mask[i];
        }
...
static int
ver1_bucket_level_full(int u, int m, int db, int8_t ln,
                       int extra)
{
    int free_rbkts;
    int min_free;
                                 ...
                                 int *near_full)
{
    ...
    if (free_rbkts <= min_free + BKT_HDL_MAX_BANKS(u, m, db, ln)) {
        ln_near_full = 1;
        if (near_full) {
            *near_full = 1;
        }
    ...
    LOG_D(("Check resource: level=%d free_rbkts=%d ln_near_full=%d\n",
           ln, free_rbkts, ln_near_full));
    stat = bcmptm_rm_alpm_ln_stats_get(u, m, db);
    if (free_rbkts <= min_free) {
        if (stat->c_curr < (stat->c_peak * MERGE_RATIO)) {
            cautious = true;
            defrag = true;
        }
    } else if (ln_near_full || (near_full && *near_full)) {
        if (stat->c_curr < (stat->c_peak * MERGE_RATIO)) {
            cautious = false;
            defrag = true;
        }
    ...
    if (defrag) {
        if (cautious) {
            stop_count = MERGE_STOP_COUNT(free_rbkts - min_free);
        } else {
            stop_count = BKT_HDL_RBKTS_TOTAL(u, m, db, ln) / 20;
        }
        SHR_IF_ERR_EXIT(
            ver1_bucket_traverse_merge(u, m, db, ln,
                                       stop_count,
                                       pivot,
                                       &freed_rbkts,
                                       cautious /* cautious_merge */));
        free_rbkts = BKT_HDL_RBKTS_TOTAL(u, m, db, ln) -
                     BKT_HDL_RBKTS_USED(u, m, db, ln);

        if (free_rbkts >= min_free || !cautious) {
            stop_count2 = BKT_HDL_RBKTS_TOTAL(u, m, db, ln) / 10;
            SHR_IF_ERR_EXIT(
                ver1_rbkt_group_traverse_compress(u, m, db, ln,
                                                  true,  /* compress second */
                                                  free_rbkts <= min_free, /* whole only */
                                                  stop_count2, &freed_rbkts2));

            if (BSL_LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
                cli_out("Traverse_merge: db:%d ln:%d freed_rbkts:%d stop_count:%d\n",
                        db, ln, freed_rbkts, stop_count);
                cli_out("Compress: db:%d ln:%d freed_rbkts:%d stop_count:%d\n",
                        db, ln, freed_rbkts2, stop_count2);
            }

            freed_rbkts += freed_rbkts2;
            free_rbkts = BKT_HDL_RBKTS_TOTAL(u, m, db, ln) -
                         BKT_HDL_RBKTS_USED(u, m, db, ln);
        }
    ...
    if (free_rbkts <= min_free) {
        if (freed_rbkts > 0) {
            LOG_ERROR(BSL_LOG_MODULE,
                (BSL_META_U(u, "Some merge done, but freed_rbkts %d(%d), "
                               "free_rbkts %d, min_free %d.\n"),
                            freed_rbkts, freed_rbkts2,
                            free_rbkts, min_free));
            SHR_ERR_EXIT(SHR_E_INTERNAL);
        }
...
ver1_bucket_resource_check(int u, int m, alpm_arg_t *arg, int extra)
{
    ...
    near_full = arg->near_full;
    for (ln = LEVEL2; ln < database_ptr->max_levels; ln++) {
        SHR_IF_ERR_VERBOSE_EXIT(
            ver1_bucket_resource_check_level(u, m, db, ln, &arg->pivot[0],
                                             extra, &near_full));
        if (near_full) {
            arg->near_full = 1;
        }
    ...
    while (rv == SHR_E_RESOURCE) {
        if (ln == DB_LAST_LEVEL(u, m, db)) {
            rv = ver1_bucket_resource_check(u, m, arg,
                                            ...
                                            EXTRA_NEW_INSERT);
            if (rv == SHR_E_RESOURCE) {
                /* Group should not be moved */
                ALPM_ASSERT(group == VER1_GROUP(pivot->bkt->log_bkt));
                if (RBKT_VBANKS(u, m, db, ln, group) <
                    BKT_HDL_MAX_BANKS(u, m, db, ln) &&
                    !ver1_bucket_level_full(u, m, db, ln,
                                            arg->dt_update ?
                                            EXTRA_UPDATE_DT:
                                            EXTRA_NEW_INSERT)) {
                    rv = SHR_E_NONE;
        ...
        SHR_IF_ERR_EXIT(
            ver1_base_entry_best_format(u, m, db, ln, bucket_len,
                                        arg->key.t.max_bits - pivot->key.t.len,
                                        arg->data_type, &format_value));
        RBKT_FORMAT(u, m, db, ln, new_rbkt) = format_value;
        ALPM_BIT_SET(RBKT_EBMP(u, m, db, ln, new_rbkt), 0);
        ent_idx = ALPM_INDEX_ENCODE_VER1(new_rbkt, 0);
        format_update = TRUE;
...
#ifdef ALPM_DEBUG
    if (LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        bcmptm_rm_alpm_buckets_sanity_by_trie(u, m, db, ln, sanity_vrf_insert, false);
    }
#endif
...
#ifdef ALPM_DEBUG
    if (LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        bcmptm_rm_alpm_buckets_sanity_by_trie(u, m, db, ln, sanity_vrf_delete, false);
    }
#endif
exit:
    SHR_FUNC_EXIT();
                       ...
                       )
{
    ...
    if (split_aux->epartial_reason == EPARTIAL_REASON_L3BKTAC_SPLIT) {
        SHR_IF_ERR_EXIT(
            ver1_bucket_special_split(u, m, ln, parent_pivot,
                                      split_root,
                                      pivot_key,
                                      pivot_len,
                                      split_aux));
                         ...
                         bkt_split_aux_t *split_aux)
{
    int rv;
    rbkt_group_t new_group;
    rbkt_group_t new_group2 = INVALID_GROUP;
    ...
    group_sz = BKT_HDL_MAX_BANKS(u, m, db, ln);
    if (split_alloc_move == 0) {
        /*
         * We want to allocate another group for parent, and we
         * want to do it now when split has not happend yet.
         */
        group_sz <<= 1;
    }
    /*
    ...
    if (split_alloc_move == 0)  {
        group_sz >>= 1;
        SHR_IF_ERR_EXIT(
            ver1_rbkt_group_split(u, m, db, ln, new_group, group_sz, &new_group2));
    }
    done = SPLIT_ALLOC_GROUP2;
    ...
    if (copy_alloc_enable && !new_in_parent && !new_in_split &&
        bcmptm_rm_alpm_trie_count(parent_pivot->bkt->trie) == 0) {
        rv = ver1_bucket_indexes_copy_alloc(u, m, db, ln,
                                            parent_group,
                                            new_group,
                                            &split_list
                                            );
    ...
    if (split_alloc_move == 0) {
        rv = ver1_bucket_alloc_and_move(u, m, db, ln, parent_group,
                                        new_group2,
                                        arg,
                                        sbbkts);
        if (rv == SHR_E_NONE) {
...
#ifdef ALPM_DEBUG
            if (LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
                bcmptm_rm_alpm_buckets_sanity_by_trie(u, m, db, ln, sanity_vrf_split_insert, false);
            }
#endif
    ...
    /* $ Compress the parent buckets to make them contiguous */
    SHR_IF_ERR_EXIT(
        ver1_rbkt_group_compress(u, m, db, ln, parent_group,
                                 vbanks,
                                 false, /* compress second */
                                 false, /* whole only */
                                 &format_update, sbbkts));
    ...
    if (split_alloc_move == 0) {
        SHR_VERBOSE_EXIT(rv); /* must be SHR_E_RESOURCE */
    } else {
        VER1_RBKT_GROUP_SANITY(u, m, db, ln, new_group, false, 0);
        VER1_RBKT_GROUP_SANITY(u, m, db, ln, parent_group, false, 0);
    ...
    if (SHR_FUNC_ERR()) {
        ...
        case SPLIT_TRIE_SPLIT:
            ...
            ver1_bkt_elems_connect(u, m, &parent_pivot->bkt->list,
                                   ...
                                   &new_in_parent);
            /* Fall through */
        case SPLIT_ALLOC_GROUP2:
            if (new_group2 != INVALID_GROUP) {
                ver1_rbkt_group_free(u, m, db, ln, new_group2);
            }
            /* Fall through */
        ...
        for (level = LEVEL2; level < db_max_levels[db_idx]; level++) {
            ...
            SHR_IF_ERR_EXIT(
                bcmptm_rm_alpm_uft_banks(u, m, db_idx, level,
                                         ...
                                         &bkt_hdl->bucket_bits,
                                         &bkt_hdl->bank_bitmap));
            bkt_hdl->max_free_count = bkt_hdl->max_banks * 2;
            ...
            max_count = bkt_hdl->max_free_count;
            ...
            bkt_hdl->group_usage_count = (max_count + 1);
            ...
            if (!recover) {
                ...
                SHR_DQ_INSERT_HEAD(&bkt_hdl->group_usage[max_count].groups,
                                   &rbkt->usgnode);
            }
...
static void
ver1_buckets_sanity(int u, int m, int db, int ln)
{
    int free_count;
    shr_dq_t *group_list;
    shr_dq_t *elem = NULL;
 ...
 */
static int
ver1_bucket_defrag(int u, int m, int db)
{
    ...
    while (ln > LEVEL1) {
        stop_count = BKT_HDL_RBKTS_TOTAL(u, m, db, ln) / 10;

        SHR_IF_ERR_EXIT(
            ver1_rbkt_group_traverse_compress(u, m, db, ln,
                                              true, /* compress second */
                                              false, /* whole only */
                                              stop_count, &freed_rbkts));

        stop_count2 = stop_count * 2;
        freed_rbkts2 = 0;
        SHR_IF_ERR_EXIT(
            ver1_bucket_traverse_merge(u, m, db, ln,
                                       stop_count2,
                                       NULL, /* pivot */
                                       &freed_rbkts2,
                                       false /* cautious_merge */));

        if (BSL_LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
            cli_out("Compress: db:%d ln:%d freed_rbkts:%d stop_count:%d\n",
                    db, ln, freed_rbkts, stop_count);
            cli_out("Traverse_merge: db:%d ln:%d freed_rbkts2:%d stop_count2:%d\n",
                    db, ln, freed_rbkts2, stop_count2);
        }
...
bcmptm_rm_alpm_bucket_ver1_register(int u, bucket_mgmt_t *bmgmt)
{
    ...
    if (bmgmt) {
        ...
        bmgmt->buckets_sanity = ver1_buckets_sanity;
        ...
        bmgmt->bucket_defrag = ver1_bucket_defrag;
    } else {
