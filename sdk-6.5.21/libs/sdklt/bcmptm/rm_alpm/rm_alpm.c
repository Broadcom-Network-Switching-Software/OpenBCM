...
static uint32_t alpm_sanity_step2 = 1;
static uint32_t alpm_sanity_start2 = 0xFFFFFFFF;
#endif
/*******************************************************************************
 * Typedefs
 */
        ...
        if (req_op == BCMPTM_OP_GET_TABLE_INFO) {
            ...
            if (arg->key.t.app == APP_LPM || arg->key.t.app == APP_COMP0) {
                ...
                case KEY_IPV6_DOUBLE:
                    ...
                    /* ALPM_ASSERT_ERRMSG(arg->key.ip_mask[1] == 0); some devices have more than 64 bits ip */
                    arg->key.ip_addr[0] &= arg->key.ip_mask[0];
...
int
bcmptm_rm_alpm_is_large_vrf(int u, int m, alpm_vrf_type_t vt)
{
    if (vt == VRF_PRIVATE ||
        (vt == VRF_GLOBAL && !bcmptm_rm_alpm_is_parallel(u, m))) {
...
int
bcmptm_rm_alpm_comp_version(int u, int m)
{
    alpm_dev_info_t *dev_info;
    dev_info = bcmptm_rm_alpm_device_info_get(u, m);
    if (dev_info) {
        return dev_info->comp_ver;
    }
    return ALPM_COMP_VER_0;;
}
...
bcmptm_rm_alpm_insert(int u, int m, bcmltd_sid_t ltid, alpm_arg_t *arg, int levels)
{
    ...
    if (inc > alpm_sanity_start &&
        ...
        bcmptm_rm_alpm_buckets_sanity_by_group(u, m, arg->db, levels - 1);
    }
    inc++;
#endif
...
#ifdef ALPM_DEBUG
    if (inc > alpm_sanity_start2 &&
        inc % alpm_sanity_step2 == 0) {
        assert(bcmptm_rm_alpm_sanity(u, m, BCMLTD_SID_INVALID, NULL) == 0);
        bcmptm_rm_alpm_buckets_sanity_by_group(u, m, arg->db, levels - 1);
    }
