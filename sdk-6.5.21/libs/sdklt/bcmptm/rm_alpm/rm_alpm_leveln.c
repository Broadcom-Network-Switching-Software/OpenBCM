...
static int defrag_percent[DELETE_DEFRAG_NUM] = {20, 30, 40, 50, 60, 70, 80, 90};
static int defrag_percent_gate[DELETE_DEFRAG_NUM] = {25, 35, 45, 55, 65, 75, 85, 95};
                          ...
                          bool resource_check)
{
    ...
    if (pre_pivot->key.t.len > pivot->key.t.len && resource_check) {
        rv = bcmptm_rm_alpm_bucket_resource_check(u, m, arg, EXTRA_NEW_INSERT);
...
alpm_bucket_insert(int u, int m, int ln, alpm_arg_t *arg, bkt_split_aux_t *split_aux)
{
    ...
    if (rv == SHR_E_RESOURCE && arg->resource_full) {
        arg->state_changed = 0;
        SHR_ERR_EXIT(rv);
    }
...
alpm_insert(int u, int m, int8_t ln, alpm_arg_t *arg, bkt_split_aux_t *split_aux)
{
    ...
    if (arg->pivot[prev_ln] == NULL) {
        ...
        if (prev_ln == LEVEL2) {
            arg->state_changed = 1;
            rv = alpm_delayed_pivot_handle(u, m, db, prev_ln, arg, true);
            if (rv == SHR_E_RESOURCE && arg->resource_full) {
                arg->state_changed = 0;
                SHR_ERR_EXIT(rv);
            }
...
alpm_vrf_add(int u, int m, alpm_arg_t *arg, alpm_vrf_hdl_t *vrf_hdl)
{
    ...
    root_arg->lpm_large_vrf = bcmptm_rm_alpm_is_large_vrf(u, m, root_arg->key.vt);
...
alpm_new_insert(int u, int m, alpm_arg_t *arg)
{
    ...
    stats = bcmptm_rm_alpm_ln_stats_get(u, m, db);
    SHR_IF_ERR_EXIT(
    ...
    if (vrf_hdl->in_use == FALSE) {
        arg->vrf_init = 1;
        arg->state_changed = 1;
        rv = bcmptm_rm_alpm_bucket_resource_check(u, m, arg, EXTRA_NEW_VRF);
        if (rv == SHR_E_RESOURCE) {
            arg->state_changed = 0;
            SHR_ERR_EXIT(SHR_E_RESOURCE);
        }
    ...
    if (arg->near_full ||
        (stats->c_last_peak && stats->c_curr > stats->c_last_peak)) {
        stats->c_last_peak = stats->c_curr;
    }

    if (stats->c_last_peak) {
        for (i = DELETE_DEFRAG_NUM - 1; i >= 0; i--) {
            if (stats->c_curr >= (stats->c_last_peak * defrag_percent_gate[i] / 100)
                && ln_info[u][m]->defrag_disable[db][i]) {
                ln_info[u][m]->defrag_disable[db][i] = 0;
                break;
            }
        }
    }
...
exit:
    if (SHR_FUNC_ERR()) {
        if (SHR_FUNC_VAL_IS(SHR_E_RESOURCE)) {
            stats->c_last_full = stats->c_curr;
        }
        if (vrf_add_flag) {
...
bcmptm_rm_alpm_ln_delete(int u, int m, alpm_arg_t *arg)
{
    ...
    bcmptm_rm_alpm_vrf_route_dec(u, m, db, arg->key.t.w_vrf,
                                 arg->key.t.ipv, &cnt);
    ...
    if (stats->c_last_peak) {
        for (i = 0; i < DELETE_DEFRAG_NUM; i++) {
            if (stats->c_curr == (stats->c_last_peak * defrag_percent[i] / 100)) {
                if (!ln_info[u][m]->defrag_disable[db][i]) {
                    if (LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
                        cli_out(" defrag percent %f last_peak %d last_full %d\n",
                                (double)stats->c_curr * 100 / stats->c_last_peak,
                                stats->c_last_peak,
                                stats->c_last_full);
                    }

                    /*
                     * If defrag fails (mostly due to wal full), disable
                     * defrag for this stage.
                     */
                    ln_info[u][m]->defrag_disable[db][i] = 1;
                    SHR_IF_ERR_EXIT(
                        bcmptm_rm_alpm_bucket_defrag(u, m, arg->db));
                    break;
                }
            }
        }
