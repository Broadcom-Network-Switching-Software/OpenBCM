    ...
    if (app == APP_LPM) {
        if (bcmptm_rm_alpm_is_parallel(u, m)) {
            ...
            if (vt == VRF_OVERRIDE) {
                ...
                if (bcmptm_rm_alpm_is_large_vrf(u, m, vt)) {
                    w_vrf = 0;
                } else {
                    w_vrf = decode_vrf;
                }
            ...
            if (bcmptm_rm_alpm_is_large_vrf(u, m, vt)) {
                if (vt == VRF_OVERRIDE) {
                    ...
                    w_vrf = 0; /* Private & Global (0) */
                }
            } else {
                /* Private, L3MC (+1), Global (+2), OVERRIDE (+3) */
                if (vt == VRF_OVERRIDE) {
                    w_vrf = (max_private_vrf + 3);
                } else if (vt == VRF_GLOBAL) {
        ...
        if (bcmptm_rm_alpm_is_large_l4_port(u, m)) {
            w_vrf = 0; /* just one trie */
        } else {
            if (vt == VRF_GLOBAL) {
                w_vrf = (alpm_dev_info[u][m]->max_fp_op + 1);
            } else if (vt == VRF_PRIVATE) {
                w_vrf = decode_vrf;
            }
        }
