...
#define MAX_BUCKET_COUNT 512 /* Number of entries in a logical bucket */
    ...
    SHR_DQ_TRAVERSE(blist, elem)
        if (verbose) {
            ...
            if (curr->ad) {
                shr_pb_printf(compact_pb[bkt_count].pb, PRE_STR2 "i=%d eidx=0x%-6x len=%-3d dt:%d<=%d ",
                              prefix,
                              RBKT_FROM_INDEX(curr->index),
                              curr->index,
                              curr->key.t.len,
                ...
                shr_pb_printf(compact_pb[bkt_count].pb, PRE_STR2 "i=%d eidx=0x%-6x len=%-3d dt:n/a ",
                              prefix,
                              RBKT_FROM_INDEX(curr->index),
                              curr->index,
                              curr->key.t.len
        ...
        bkt_count++;
        assert(bkt_count < MAX_BUCKET_COUNT);
...
internals_dump(int u, int m, int db, int ln, shr_pb_t *pb, const char *prefix)
{
    ...
    shr_pb_printf(pb, PRE_STR2"split_move_write    %d \n", prefix, internals->split_move_write);
    shr_pb_printf(pb, PRE_STR2"compress_write      %d \n", prefix, internals->compress_write);
...
static void
rbkt_group_dump(int u, int m, int db, int ln, rbkt_t *rbkt,
                bool verbose, shr_pb_t *pb, const char *prefix)
{
    alpm_pivot_t *pivot;
    int i;
    int vbanks;
    ...
    (void)bcmptm_rm_alpm_ln_base_entry_info(u, m, format_type,
                                            rbkt->format,
                                            &len,
                                            &full,
                                            &mes);
    ves = shr_util_popcount(rbkt->entry_bitmap);
    shr_pb_printf(pb, PRE_STR2 "g=%-6d ve=0x%-4x fmt=%-2d(len:%d) h=%d vbs=%-1d fbs=%-2d num=%-2d(%-2d) \n",
                  prefix,
                  rbkt->index,
                  rbkt->entry_bitmap,
                  rbkt->format,
    ...
    for (i = 1; i < vbanks; i++) {
        ...
        (void)bcmptm_rm_alpm_ln_base_entry_info(u, m, format_type,
                                                ...
                                                &full,
                                                &mes);
        ves = shr_util_popcount(rbkt2->entry_bitmap);
        shr_pb_printf(pb, PRE_STR2 "i=%-6d ve=0x%-4x fmt=%-2d(len:%d) h=%d vbs=%-1d fbs=%-2d num=%-2d(%-2d) \n",
                      prefix,
                      rbkt2->index,
                      rbkt2->entry_bitmap,
                      rbkt2->format,
    ...
    if (bkt_mes) {
        shr_pb_printf(pb, PRE_STR2 "ves:mes = %d:%d = %d%%\n", prefix, bkt_ves, bkt_mes, bkt_ves*100/ bkt_mes);
    }
...
static void
rbkt_group_list_dump(int u, int m, int db, int ln, bkt_hdl_t *bkt_hdl,
                     bool verbose, shr_pb_t *pb, const char *prefix)
{
    shr_dq_t *elem;
    rbkt_t *curr;
    ...
    SHR_DQ_TRAVERSE(&bkt_hdl->global_group_list, elem)
        ...
        rbkt_group_dump(u, m, db, ln, curr, verbose, pb, prefix);
...
static void
rbkt_group_usg_list_dump(int u, int m, int db, int ln, shr_dq_t *glist,
                         bool verbose, shr_pb_t *pb,
                         const char *prefix)
{
    shr_dq_t *elem;
    rbkt_t *curr;
...
bcmptm_rm_alpm_sanity(int u, int m, bcmltd_sid_t ltid, int *num_fails)
{
    ...
    rv = bcmptm_rm_alpm_trans_cmd(u, 0, 0, BCMPTM_TRANS_CMD_IDLE_CHECK);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(u,
                              "Sanity abort. ALPM is not in IDLE. \n")));
        return -1;
    }

    info = bcmptm_rm_alpm_info_get(u, m);
    if (info) {
        info->cmd_ctrl.req_flags = 0;
    }
                           ...
                           bool verbose, shr_pb_t *pb)
{
    ...
    DEBUG_PB_ENTER(pb);
    bcmptm_rm_alpm_bkt_hdl_get(u, m, db, ln, &bkt_hdl);
    if (bkt_hdl->rbkt_array[group].is_head) {
        rbkt_group_dump(u, m, db, ln, &bkt_hdl->rbkt_array[group], verbose, pb, "--");
    } else {
        shr_pb_printf(pb, "%d is not a group \n", group);
    }
    DEBUG_PB_EXIT(pb);
}
...
bcmptm_rm_alpm_buckets_dump(int u, int m, int db, int ln, shr_pb_t *pb, bool verbose)
{
    ...
    bcmptm_rm_alpm_bkt_hdl_get(u, m, db, ln, &bkt_hdl);
    if (bkt_hdl) {
        rbkt_group_list_dump(u, m, db, ln, bkt_hdl, verbose, pb, "");
    }
    DEBUG_PB_EXIT(pb);
}
...
bcmptm_rm_alpm_buckets_dump2(int u, int m, int db, int ln, shr_pb_t *pb, bool verbose)
{
    ...
    for (i = 0; i < bkt_hdl->group_usage_count; i++) {
        ...
        rbkt_group_usg_list_dump(u, m, db, ln, &bkt_hdl->group_usage[i].groups,
                                 verbose, pb, "");
    }
    DEBUG_PB_EXIT(pb);
}
        ...
        for (i = LAYOUT_D; i >= la_end; i--) {
            ...
            while (st && pfx >= 0) {
                ...
                if (vent > 0 && start >= 0 && end >= 0  && start <= end) {
                    ...
                    arg->key.t.ipv = ipv;
                    arg->key.t.len = len;
                    arg->lpm_large_vrf = bcmptm_rm_alpm_is_large_vrf(u, m, vt);
