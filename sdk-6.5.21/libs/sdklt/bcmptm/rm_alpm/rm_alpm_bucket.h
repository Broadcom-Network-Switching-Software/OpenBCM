...
#define BKT_HDL_MAX_FREE_COUNT(u, m, _d, _l)    (BKT_HDL(u, m, _d, _l)->max_free_count)
...
typedef void (*buckets_sanity_f)(int u, int m, int db, int ln);
...
typedef int (*bucket_defrag_f)(int u, int m, int db);
...
typedef struct bucket_mgmt_s {
    ...
    buckets_sanity_f buckets_sanity;
...
extern int
bcmptm_rm_alpm_bucket_defrag(int u, int m, int db);
...
extern void
bcmptm_rm_alpm_buckets_sanity_by_group(int u, int m, int db, int ln);

extern void
bcmptm_rm_alpm_buckets_sanity_by_trie(int u, int m, int db, int ln, int w_vrf, bool merge);
