...
bcmptm_rm_tcam_prio_only_iomd_update(int unit,
                                     bcmptm_rm_tcam_entry_iomd_t *iomd);
/*!
 * \brief Check entry info for target LTID,
 * \n whether target_index location is used for reserve entry
 *
 * \param [in] unit Logical device id
 * \param [in] ltid Logical Table enum that is accessing the table.
 * \param [in] ltid_info LRD information for the ltid.
 * \param [in] pipe_id Pipe Number
 * \param [in] target_ltid Logical Table enum that is accessing the table.
 * \param [in] target_ltid_info LRD information for the ltid.
 * \param [in] index entry index
 * \param [out] is_reserve_index set to true if index is reserve entry
 *
 * \retval SHR_E_NONE Entry info is consistent
 * \retval SHR_E_INTERNAL Bad entry info found
 */
extern int
bcmptm_rm_tcam_check_reserve_entry(int unit,
                                   bcmltd_sid_t ltid,
                                   bcmptm_rm_tcam_lt_info_t *ltid_info,
                                   int pipe_id,
