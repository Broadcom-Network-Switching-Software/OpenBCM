 ...
 */
STATIC int
rm_tcam_prio_only_shared_sid_compare(int unit,
                                     bcmdrd_sid_t sid,
                                     ...
                                     bool *compare)
{
    ...
    SHR_NULL_CHECK(shr_ltid_info, SHR_E_PARAM);

    *compare = FALSE;

    if (sid == shr_sid) {
        *compare = TRUE;
    } else if ((ltid_info->rm_more_info->is_multi_mem_aggr == 0 &&
                shr_ltid_info->rm_more_info->is_multi_mem_aggr == 1) ||
               (ltid_info->rm_more_info->is_multi_mem_aggr == 1 &&
                shr_ltid_info->rm_more_info->is_multi_mem_aggr == 0)) {
        /* one of the LTID uses base view and another uses multi mem view. */
        SHR_IF_ERR_EXIT_EXCEPT_IF
            (bcmptm_rm_tcam_prio_only_shared_sid_compare
             (unit, sid, shr_sid, neg_shr_flags, compare), SHR_E_UNAVAIL);
    }
    ...
    for (idx = 0; idx < ltid_info->rm_more_info->shared_lt_count; idx++) {
        ...
        if (ltid_info->hw_entry_info[0].view_type) {
            ...
            for (depth = 0; depth < num_depth_inst; depth++) {
                ...
                for (shr_depth = 0; shr_depth <shr_num_depth_inst; shr_depth++) {
                    shr_sid = shr_ltid_info.hw_entry_info[0].sid[shr_depth][0];
                    ...
                    if (shr_ltid_info.rm_more_info->flags & BCMPTM_RM_TCAM_HALF_MODE) {
                        shr_end_index += shr_slice_size;
                        shr_depth++;
                        shr_slice_size += shr_slice_size;
                    }
                    if (shr_ltid_info.rm_more_info->flags &
                        BCMPTM_RM_TCAM_F_DEPTH_EXPANSION) {
                        shr_end_index += shr_slice_size;
                        shr_slice_size += shr_slice_size;
                    }
                    if (ltid_info->rm_more_info->flags
                        & BCMPTM_RM_TCAM_INTRA_SLICE_DOUBLE) {
                        if (!(shr_ltid_info.rm_more_info->flags
                            & BCMPTM_RM_TCAM_INTRA_SLICE_DOUBLE)) {
                            ...
                            shr_info->shr_end_index = shr_end_index - shr_slice_size / 2;
                            shr_info++;
                            (*shr_info_count)++;
                            shr_start_index += shr_slice_size / 2;
                        }
                    } else if (shr_ltid_info.rm_more_info->flags &
                                 BCMPTM_RM_TCAM_INTRA_SLICE_DOUBLE) {
                        if (!(ltid_info->rm_more_info->flags
                               & BCMPTM_RM_TCAM_INTRA_SLICE_DOUBLE)) {
                            shr_info->shr_ltid = shr_ltid;
                            shr_info->start_index = start_index;
                            shr_info->end_index = end_index - slice_size / 2;
                            shr_info->shr_start_index = shr_start_index;
                            shr_info->shr_end_index = shr_end_index;
                            shr_info++;
                            (*shr_info_count)++;
                            start_index += slice_size / 2;
                        }
                    }
                    shr_info->shr_ltid = shr_ltid;
                    shr_info->start_index = start_index;
        ...
        if (shared == TRUE) {
            if (sid_differ == TRUE && depth_differ == TRUE) {
                ...
                /*
                 * Dynamic LT's don't have common entry info.
                 * Each LT has its own entry info.
                 */
                if (!(ltid_info->rm_more_info->flags &
                      BCMPTM_RM_TCAM_DYNAMIC_MANAGEMENT)) {
                    continue;
                }
            }
        }
                    ...
                    do {
                        for (shr_depth = 0;
                            ...
                            for (shr_key_row = 0;
                                ...
                                neg_shr_flags = 0;
                                SHR_IF_ERR_EXIT
                                    (rm_tcam_prio_only_shared_sid_compare(unit,
                                                                 sid,
                                                                 shr_sid,
                                                                 ltid_info,
                                                                 &shr_ltid_info,
                                ...
                                if (compare == TRUE) {
                                    ...
                                    shr_info->shr_flags &= ~neg_shr_flags;
                                    (*shr_info_count)++;
                                    shr_info++;
                                    break;
                                }
