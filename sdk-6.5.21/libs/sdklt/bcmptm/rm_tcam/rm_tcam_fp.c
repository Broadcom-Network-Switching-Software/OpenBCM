                                   ...
                                   uint8_t sub_component_id)
{
    ...
    trans_info->issu_ctrls.group_seg_offsets_relative = 1;
...
STATIC int
bcmptm_rm_tcam_fp_update_grp_entry_seg_offsets(int unit,
                  bcmptm_rm_tcam_fp_stage_t *stage_ptr)
{
    bcmptm_rm_tcam_fp_group_t *group_ptr = NULL;
    uint8_t seg_id = 0;
    ...
    /*
     * Loop through the group segment to correct the
     * entry segment offset's to relative one's
     */
    for (idx = 0; idx < stage_ptr->num_groups; idx++) {
        if (group_ptr->valid == 1) {
            if (!(stage_ptr->flags & BCMPTM_STAGE_ENTRY_TYPE_HASH)) {
                for (seg_id = 0; seg_id < stage_ptr->num_slices; seg_id++) {
                    if (SHR_BITGET(group_ptr->entry_seg_bmp.w,seg_id)) {
                        break;
                    }
                }
                group_ptr->entry_info_seg =
                        (((stage_ptr->max_entries_per_slice * seg_id))
                           * sizeof(bcmptm_rm_tcam_prio_eid_entry_info_t));
                group_ptr->entry_hash_seg =
                        (((group_ptr->entry_hash_size * seg_id)) * sizeof(int));
            }
            group_ptr->presel_entry_info_seg =
                        (((stage_ptr->max_presel_entries_per_slice
                           * group_ptr->primary_slice_id))
                           * sizeof(bcmptm_rm_tcam_prio_eid_entry_info_t));
            group_ptr->presel_entry_hash_seg =
                        (((group_ptr->presel_entry_hash_size
                           * group_ptr->primary_slice_id)) * sizeof(int));
        }
                                   ...
                                   uint8_t sub_component_id)
{
    ...
    if (trans_info->issu_ctrls.group_seg_offsets_relative == 0) {
        /*
         * In cold boot relative offsets are not used.
         * For all groups in this stage change entry seg,
         * entry hash seg, presel entry seg, presel hash seg,
         * to relative offset.
         */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmptm_rm_tcam_fp_update_grp_entry_seg_offsets(unit,
                                                stage_ptr));
    }
exit:
    SHR_FUNC_EXIT();
