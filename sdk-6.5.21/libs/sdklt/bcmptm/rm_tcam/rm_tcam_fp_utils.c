                                 ...
                                 bcmptm_rm_tcam_prio_eid_entry_info_t **entry_info)
{
    ...
    *entry_info = NULL;
    ...
    if (SHR_BITGET(entry_attrs->flags.w,BCMPTM_FP_FLAGS_PRESEL_ENTRY)) {
        offset = stage_ptr->presel_entry_info_seg +
                    group_ptr->presel_entry_info_seg;
    } else {
        offset = stage_ptr->entry_info_seg + group_ptr->entry_info_seg;
    }
                                      ...
                                      bcmptm_rm_tcam_entry_iomd_t *iomd)
{
    ...
    if (SHR_BITGET(entry_attrs->flags.w,BCMPTM_FP_FLAGS_PRESEL_ENTRY)) {
        offset = stage_ptr->presel_entry_hash_seg +
                        group_ptr->presel_entry_hash_seg;
    } else {
        offset = stage_ptr->entry_hash_seg +
