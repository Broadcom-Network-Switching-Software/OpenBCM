...
/*!
 * Structure for all sid's which needs ser correction call back.
 * Ser correction needs sid/view and correspnding hw_index
 * with which the hw is programmed. PTRM-TCAM
 * should check the sw entry state and return correct sid and
 * hw index depending on which type of entry is written
 * in hw at the requested index.
 */
typedef struct bcmptm_rm_tcam_ser_correction_info_s {

    /*! Logical table ID */
    bcmltd_sid_t ltid;

    /*! Narrow view tcam_only sid */
    bcmdrd_sid_t **hw_sid_tcam_only_arr;

    /*! Narrow view tcam_only sid */
    bcmdrd_sid_t **hw_sid_data_only_arr;

    /*! combined view sid */
    bcmdrd_sid_t **hw_sid_tcam_data_arr;

    /*! Wide/Paired combined view sid */
    bcmdrd_sid_t **hw_sid_paired_arr;

    /*! Narrow view sid size*/
    uint32_t hw_sid_size;

    /*! Narrow view sid size*/
    uint32_t paired_sid_size;

    /*! Narrow view data sid size*/
    uint32_t data_sid_size;

    /* Num depth instances of views */
    uint8_t num_depth_inst;

    /* Num width instances of views */
    uint8_t num_width_inst;

    /* Number of shared Lt's */
    uint8_t shared_lt_count;

    /* Shared Lt's entry type array*/
    uint8_t *entry_type_arr;

    /* Shared Lt's entry type array*/
    bcmltd_sid_t *shr_ltid_arr;

    /*! Pointer to next ser correction info. */
    struct bcmptm_rm_tcam_ser_correction_info_s *next;
} bcmptm_rm_tcam_ser_correction_info_t;
...
/*!
 *  \brief Construct ser correction info for the given LT.
 *
 *  \param [in] unit Logical device id
 *  \param [in] ltid Logical Table enum that is accessing the table.
 *  \param [in] ltid_info LRD information for the ltid.
 ...
 */
extern int
bcmptm_rm_tcam_create_ser_correction_info(int unit,
                      bcmltd_sid_t ltid,
                      bcmptm_rm_tcam_lt_info_t *ltid_info);

/*!
 *  \brief Get the start pointer of Ser correction info.
 *
 *  \param [in] unit Logical device ID
 *  \param [out] ser_corr_info ser correction info global ptr
 *
 *  \retval SHR_E_NONE Success
 */
extern int
...
/*!
 *  \brief Update new ser corr info in global Ser correction info.
 *
 *  \param [in] unit Logical device ID
 *  \param [in] ser_corr_info ser correction info ptr
 *
 *  \retval SHR_E_NONE Success
 */
extern int
...
/*!
 *  \brief Clean up ser correction info
 *
 *  \param [in] unit Logical device id
 *
 *  \retval SHR_E_NONE Success
...
/*!
 *  \brief Remove ser corr info from global Ser correction info.
 *
 *  \param [in] unit Logical device ID
 *  \param [in] ser_corr_info ser correction info ptr
 *
