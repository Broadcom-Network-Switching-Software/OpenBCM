...
{
    size_t size = 0;
    uint32_t *presel_group_ptr = NULL;

    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(stage_ptr, SHR_E_PARAM);

    size = stage_ptr->num_presel_groups * sizeof(uint32_t);
    if (stage_ptr->num_presel_groups) {
        presel_group_ptr = (uint32_t *)((uint8_t *)stage_ptr +
                           stage_ptr->presel_group_ref_count_seg);
        sal_memset(presel_group_ptr, 0, size);
    }
exit:
    SHR_FUNC_EXIT();
}
                  ...
                  uint32_t **ref_count_ptr)
{
    uint8_t idx = 0;
    uint8_t tile_id = 0;
    uint16_t presel_groups_total = 0;
    const bcmptm_rm_tcam_more_info_t *rm_more_info = NULL;
    ...
    for (idx = 0; idx < tile_id; idx++) {
        presel_groups_total +=
        rm_more_info->tile_info[idx].presel_group_count;
    }

    /* Fetch the PDD profiles segment start pointer of a slice. */
    *ref_count_ptr = (uint32_t *)
                     (((uint8_t *)stage_fc) +
                     stage_fc->presel_group_ref_count_seg +
                     (presel_groups_total *
                     sizeof(uint32_t)));
exit:
    SHR_FUNC_EXIT();
