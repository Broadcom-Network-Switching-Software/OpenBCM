...
static int
bcmptm_rm_tcam_update_flex_ctr_profiles(int unit,
                                          bcmptm_rm_tcam_fp_group_t *fg,
                                          bcmptm_rm_tcam_fp_stage_t *stage_fc,
                                          bcmptm_rm_tcam_slice_t *slice_ptr,
                                          int8_t *old_virtual_slice_mapping,
                                          uint8_t num_slices_per_pipe)
{
    bcmptm_rm_tcam_fp_group_t *grp_ptr = NULL;
    uint8_t tmp_ctr_profiles[BCMPTM_RM_TCAM_MAX_SLICES] = {0};
    uint8_t used_profiles[BCMPTM_RM_TCAM_MAX_SLICES] = {0};
    int8_t virtual_slice_id = 0;
    int8_t old_virtual_slice_id = 0;
    uint8_t i = 0;
    uint8_t group_id = 0;
    uint8_t slice_id = 0;
    ...
    SHR_NULL_CHECK(old_virtual_slice_mapping, SHR_E_PARAM);

    /*
     *1. Retain ctr profile id at virtual slice ID location if
     *      old_virtual_slice_id == new_virtual_slice_id
     *      old_virtual_slice_id != -1
     *      new_virtual_slice_id != -1
     *2. Copy old_virtual_slice_id ctr profile id to new virtual slice id
     *   profile position if
     *      old_virtual_slice_id != new_virtual_slice_id and
     *      old_virtual_slice_id != -1
     *      new_virtual_slice_id != -1
    ...
    for (group_id = 0; group_id < stage_fc->num_groups; group_id++) {
        /* For a new group, valid is not set when this function is called */
        if ((grp_ptr->valid == 1 && grp_ptr->priority <= fg->priority)
             || grp_ptr->group_id == fg->group_id) {
            /* Check if multiple counter profiles are mapped to this group */
            if (grp_ptr->num_counter_profiles != 0) {

                sal_memset(tmp_ctr_profiles, 0,
                           sizeof(uint8_t) * BCMPTM_RM_TCAM_MAX_SLICES);
                sal_memset(used_profiles, 0,
                           sizeof(uint8_t) * BCMPTM_RM_TCAM_MAX_SLICES);
                for (slice_id = 0; slice_id < num_slices_per_pipe;
                     slice_id++) {
                    if (old_virtual_slice_mapping[slice_id] !=
                            slice_ptr[slice_id].virtual_slice_id) {
                        /* New group or auto expansion to new slice case */
                        if ((old_virtual_slice_mapping[slice_id] == -1 &&
                            slice_ptr[slice_id].virtual_slice_id != -1)) {
                            virtual_slice_id =
                                slice_ptr[slice_id].virtual_slice_id;
                            if (used_profiles[virtual_slice_id] == 0) {
                                tmp_ctr_profiles[virtual_slice_id] =
                                    grp_ptr->counter_profiles[virtual_slice_id];
                                used_profiles[virtual_slice_id] = 1;
                            }
                        /* Group deletion or auto reduction case */
                        } else if (old_virtual_slice_mapping[slice_id] != -1
                            && slice_ptr[slice_id].virtual_slice_id == -1) {
                            /*
                             * Do not update anything in the
                             * tmp_ctr_profiles. Update this depeneding on
                             * what is left out from used prof ID's
                             */
                        } else {
                            old_virtual_slice_id =
                                old_virtual_slice_mapping[slice_id];
                            virtual_slice_id =
                                slice_ptr[slice_id].virtual_slice_id;
                            tmp_ctr_profiles[virtual_slice_id] =
                                grp_ptr->counter_profiles[old_virtual_slice_id];
                            if (used_profiles[old_virtual_slice_id] == 1)  {
                                /* reset the corresponding tmp_ctr */
                                tmp_ctr_profiles[old_virtual_slice_id] = 0;
                            }
                            used_profiles[old_virtual_slice_id] = 1;
                        }
                    } else {
                        if (slice_ptr[slice_id].virtual_slice_id != -1) {
                            virtual_slice_id =
                                slice_ptr[slice_id].virtual_slice_id;
                            tmp_ctr_profiles[virtual_slice_id] =
                                grp_ptr->counter_profiles[virtual_slice_id];
                ...
                for (virtual_slice_id = 0;
                     virtual_slice_id < num_slices_per_pipe;
                     virtual_slice_id++) {
                    if (tmp_ctr_profiles[virtual_slice_id] == 0) {
                        /*check if this is used already in grp_ptr */
                        if (used_profiles[virtual_slice_id] == 0) {
                            tmp_ctr_profiles[virtual_slice_id] =
                                grp_ptr->counter_profiles[virtual_slice_id];
                            used_profiles[virtual_slice_id] = 1;
                            ...
                            for (i = 0;i <num_slices_per_pipe; i++) {
                                if (used_profiles[i] == 0) {
                                    tmp_ctr_profiles[virtual_slice_id] =
                                        grp_ptr->counter_profiles[i];
                                    used_profiles[i] = 1;
                                    break;
                                }
                            }
                ...
                sal_memcpy(grp_ptr->counter_profiles, tmp_ctr_profiles,
                                          ...
                                          bcmptm_rm_tcam_fp_stage_id_t stage_id)
{
    ...
    bool slice_mapping_changed = false;
    int8_t *old_virtual_slice_mapping = NULL;
    ...
    * <= num_slices_per_pipe and in order of increasing priority*/
    ...
    for (virtual_slice_count = num_slices_per_pipe - 1;
        ...
        if (lt_info[virtual_slice_count].valid == TRUE) {
            ...
            while (slice_count-- > 0) {
                ...
                /* Copy slice priority */
                hw_lt_info[virtual_slice_no].priority =
                                     slice_prio_map[slice_count].slice_priority;
    ...
    SHR_ALLOC(old_virtual_slice_mapping,
              sizeof(uint8_t) * num_slices_per_pipe,
              "bcmptmoldvirtualsliceInfo");
    sal_memset(old_virtual_slice_mapping, -1,
              num_slices_per_pipe * sizeof(int8_t));
    ...
    for (field_idx = 0; field_idx < num_slices_per_pipe; field_idx++) {
        ...
        /* Update virtual slice id number in slice structure */
        slice_id = hw_lt_info[field_idx].lt_action_pri;
        /* Valid entry */
        if (hw_lt_info[field_idx].priority != 0) {
            if (slice_ptr[slice_id].virtual_slice_id != field_idx) {
                slice_mapping_changed = true;
            }
            /* Save the old mapping in this structure */
            old_virtual_slice_mapping[slice_id] =
                                slice_ptr[slice_id].virtual_slice_id;
            /* Update new virtual slice mapping in slice structure */
            slice_ptr[slice_id].virtual_slice_id = field_idx;
        }
    }
    ...
    /*
     * For groups having multiple flex ctr profiles,
     * update the virtual slice id accordingly.
     */
    if (slice_mapping_changed == true) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmptm_rm_tcam_update_flex_ctr_profiles(unit,
                                                fg,
                                                stage_fc,
                                                slice_ptr,
                                                old_virtual_slice_mapping,
                                                num_slices_per_pipe));
    }
exit:
    SHR_FREE(sid_info[unit].lt_action_pri);
    ...
    SHR_FREE(old_virtual_slice_mapping);
    SHR_FUNC_EXIT();
}
