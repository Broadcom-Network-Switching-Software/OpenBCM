...
#define RM_TCAM_MD(unit) rm_tcam_global_info[unit].rm_tcam_md

#define RM_TCAM_INFO_GLOBAL_TRANS(unit) rm_tcam_global_info[unit].trans_info

#define RM_TCAM_HA_HEADER_PREVIOUS(unit) rm_tcam_global_info[unit].rm_tcam_header_previous

#define RM_TCAM_DYN_PTRM_TBL(unit) rm_tcam_global_info[unit].rm_tcam_dyn_ptrm_table

#define RM_TCAM_SUB_ID(unit, sub_id) rm_tcam_global_info[unit].sub_component_id[sub_id]

#define RM_TCAM_SER_CORR_INFO(unit) rm_tcam_global_info[unit].rm_tcam_ser_correction_info
/*******************************************************************************
 * Global variables
 */
...
bcmptm_rm_tcam_sub_component_allocate(int unit, uint8_t *sub_id) {
    ...
    for (iter = 0; iter < BCMPTM_RM_TCAM_HA_SUBID_MAX; iter++) {
        if (RM_TCAM_SUB_ID(unit, iter) == FALSE) {
            *sub_id = iter;
            RM_TCAM_SUB_ID(unit, iter) = TRUE;
            return SHR_E_NONE;
        }
    }
...
bcmptm_rm_tcam_sub_component_free(int unit, uint8_t sub_id) {
    ...
    if (RM_TCAM_SUB_ID(unit, sub_id) == TRUE) {
        RM_TCAM_SUB_ID(unit, sub_id) = FALSE;
        return SHR_E_NONE;
    } else {
        return SHR_E_INTERNAL;
    }
...
bcmptm_rm_tcam_sub_component_set(int unit, uint8_t sub_id) {
    if (RM_TCAM_SUB_ID(unit, sub_id) == FALSE) {
        RM_TCAM_SUB_ID(unit, sub_id) = TRUE;
        return SHR_E_NONE;
    } else {
        return SHR_E_INTERNAL;
    }
...
bcmptm_rm_tcam_sub_component_init(int unit) {
    int16_t iter;
    for (iter = 0; iter < BCMPTM_RM_TCAM_HA_SUBID_MAX; iter++) {
        RM_TCAM_SUB_ID(unit, iter) = FALSE;
    }
    return SHR_E_NONE;
}
...
bcmptm_rm_tcam_trans_state_init(int unit)
{
    ...
    if (SHR_FAILURE(rv) ||
        ...
        RM_TCAM_INFO_GLOBAL(unit).rm_tcam_static_atomic_trans = FALSE;
        /*
         * Record whether RM is in dynamic atomic transaction mode.
         * Default value is FALSE.
         */
        RM_TCAM_INFO_GLOBAL(unit).rm_tcam_dynamic_atomic_trans = FALSE;
    } else {
        RM_TCAM_INFO_GLOBAL(unit).rm_tcam_static_atomic_trans = TRUE;
    }
...
bcmptm_rm_tcam_undo_uc_state_get(int unit, bool *undo_uc_state)
{
    ...
    *undo_uc_state = (RM_TCAM_INFO_GLOBAL(unit).rm_tcam_dynamic_atomic_trans ||
                      RM_TCAM_INFO_GLOBAL(unit).rm_tcam_static_atomic_trans);
exit:
    SHR_FUNC_EXIT();
}
...
bcmptm_rm_tcam_dynamic_trans_set(int unit, bool dynamic_trans_state)
{
    ...
    RM_TCAM_INFO_GLOBAL(unit).rm_tcam_dynamic_atomic_trans
                 = dynamic_trans_state;
                               ...
                               uint8_t *ltid_info_updated)
{
    ...
    *ltid_info_updated = 0;
    ...
    if (*ltid_info_updated == 1 &&
        ltid_info.rm_more_info->flags
                & BCMPTM_RM_TCAM_NEEDS_SER_CORR_INFO) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmptm_rm_tcam_create_ser_correction_info(unit,
                                          ltid,
                                          &ltid_info));
    }
...
int
bcmptm_rm_tcam_ser_correction_info_cleanup(int unit)
{
    ...
    ser_info = RM_TCAM_SER_CORR_INFO(unit);
    while (ser_info != NULL) {

        for (i = 0; i < ser_info->num_depth_inst; i++) {
            SHR_FREE(ser_info->hw_sid_tcam_only_arr[i]);
            SHR_FREE(ser_info->hw_sid_data_only_arr[i]);
            SHR_FREE(ser_info->hw_sid_tcam_data_arr[i]);
            SHR_FREE(ser_info->hw_sid_paired_arr[i]);
        }
        SHR_FREE(ser_info->hw_sid_tcam_only_arr);
        SHR_FREE(ser_info->hw_sid_data_only_arr);
        SHR_FREE(ser_info->hw_sid_tcam_data_arr);
        SHR_FREE(ser_info->hw_sid_paired_arr);
        SHR_FREE(ser_info->entry_type_arr);
        SHR_FREE(ser_info->shr_ltid_arr);
        RM_TCAM_SER_CORR_INFO(unit) = ser_info->next;

        SHR_FREE(ser_info);
        ser_info = RM_TCAM_SER_CORR_INFO(unit);
    }
    RM_TCAM_SER_CORR_INFO(unit) = NULL;
...
bcmptm_rm_tcam_cleanup(int unit)
{
    ...
    /* Unregister SER correction callback */
    bcmptm_ser_tcam_info_cb_reg(unit, NULL);
    ...
    /* Set all elements of global structure to null */
    RM_TCAM_MD(unit) = NULL;
    RM_TCAM_INFO_GLOBAL_TRANS(unit) = NULL;
    RM_TCAM_HA_HEADER_PREVIOUS(unit) = NULL;

    SHR_IF_ERR_VERBOSE_EXIT(
        bcmptm_rm_tcam_ser_correction_info_cleanup(unit));
                              ...
                              bcmptm_sub_phase_t phase)
{
    ...
    if (phase == BCMPTM_SUB_PHASE_1) {
        SHR_IF_ERR_EXIT
            (bcmptm_rm_tcam_sub_component_allocate(unit,&sub_id));
        req_size = size = sizeof(bcmptm_rm_tcam_trans_info_t);
        RM_TCAM_INFO_GLOBAL_TRANS(unit) =
                shr_ha_mem_alloc(unit,
                                 BCMMGMT_RM_TCAM_COMP_ID,
                                 sub_id,
                                 "bcmptmRmTcamGlobTransInfo",
                                 &size);
        SHR_NULL_CHECK(RM_TCAM_INFO_GLOBAL_TRANS(unit), SHR_E_MEMORY);
        SHR_IF_ERR_VERBOSE_EXIT
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmissu_struct_info_report(unit,
                                        ...
                                        BCMPTM_RM_TCAM_TRANS_INFO_T_ID));
        trans_info = (RM_TCAM_INFO_GLOBAL_TRANS(unit));
...
int
bcmptm_rm_tcam_ser_correction_info_global_get(int unit,
                        bcmptm_rm_tcam_ser_correction_info_t **ser_corr_info)
{

    *ser_corr_info = RM_TCAM_SER_CORR_INFO(unit);
    return SHR_E_NONE;
}
...
int
bcmptm_rm_tcam_ser_corr_info_global_update(int unit,
                           bcmptm_rm_tcam_ser_correction_info_t **ser_info)
{
    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(*ser_info, SHR_E_PARAM);

    (*ser_info)->next = RM_TCAM_SER_CORR_INFO(unit);
    RM_TCAM_SER_CORR_INFO(unit) = *ser_info;
...
int
bcmptm_rm_tcam_ser_corr_info_global_remove(int unit,
                           bcmptm_rm_tcam_ser_correction_info_t *ser_ptr)
{
    bcmptm_rm_tcam_ser_correction_info_t *ser_info = NULL;
    int i = 0;
    ...
    SHR_NULL_CHECK(ser_ptr, SHR_E_PARAM);

    for (i = 0; i < ser_ptr->num_depth_inst; i++) {
        SHR_FREE(ser_ptr->hw_sid_tcam_only_arr[i]);
        SHR_FREE(ser_ptr->hw_sid_data_only_arr[i]);
        SHR_FREE(ser_ptr->hw_sid_tcam_data_arr[i]);
        SHR_FREE(ser_ptr->hw_sid_paired_arr[i]);
    }
    SHR_FREE(ser_ptr->hw_sid_tcam_only_arr);
    SHR_FREE(ser_ptr->hw_sid_data_only_arr);
    SHR_FREE(ser_ptr->hw_sid_tcam_data_arr);
    SHR_FREE(ser_ptr->hw_sid_paired_arr);
    SHR_FREE(ser_ptr->entry_type_arr);
    SHR_FREE(ser_ptr->shr_ltid_arr);
    ...
    if (ser_info == ser_ptr) {
        ...
        while (ser_info != NULL) {
            if (ser_info->next == ser_ptr) {
                ser_info->next = ser_ptr->next;
                break;
            } else {
                ser_info = ser_info->next;
            }
        }
    }
               ...
               bcmptm_rm_tcam_trans_info_t **trans_info)
{
    ...
    if (RM_TCAM_INFO_GLOBAL_TRANS(unit) == NULL) {
        *trans_info = NULL;
        SHR_FUNC_EXIT();
    }
