...
int bcmptm_rm_tcam_check_reserve_entry(int unit,
                                       bcmltd_sid_t ltid,
                                       ...
                                       bool *is_reserve_index)
{
    ...
    SHR_NULL_CHECK(target_ltid_info, SHR_E_PARAM);
    SHR_NULL_CHECK(is_reserve_index, SHR_E_PARAM);
    ...
    for (idx = 0; idx < shared_info_count; idx++) {
        ...
        if (target_ltid != shared_info[idx].shr_ltid) {
            continue;
        }
        ...
        if (shared_info[idx].start_index <= index &&
            ...
            if (target_entry_type !=
                ...
                /* Something went wrong in index computation. */
                SHR_IF_ERR_EXIT(SHR_E_INTERNAL);
            } else {
                /* Check prio */
                if (target_entry_info[target_index].entry_pri ==
                        BCMPTM_RM_TCAM_PRIO_INVALID) {
                    *is_reserve_index = true;
                }
            }
