                                  ...
                                  bcmptm_rm_tcam_entry_iomd_t *iomd)
{
    ...
    uint8_t num_slices_width_wise = 0;
    ...
    do {
           ...
           slice_id = pri_slice_id + right_slice_id_offset[attrs->group_mode];
           pri_slice_id = slice_id;
           idx1++;
           ...
           if ((stage_fc->flags & BCMPTM_STAGE_INTRASLICE_CAPABLE) &&
               (attrs->group_mode == BCMPTM_RM_TCAM_GRP_MODE_DBLINTRA ||
                attrs->group_mode == BCMPTM_RM_TCAM_GRP_MODE_QUAD)) {
                ...
                if (right_slice_id_offset[attrs->group_mode] != 0) {
                    idx1++;
                }
           }
    ...
    num_slices_width_wise = idx1;

    for (idx1 = 0; idx1 < num_slices_width_wise; idx1++) {
        for (idx2 = 0; idx2 < hw_info->num_slices_depth_wise; idx2++) {
             sid_info.slice_id = hw_info->slice[idx1][idx2];
                                       ...
                                       int segment_id)
{
    ...
    grp->entry_info_seg = ((num_entries_per_slice * segment_id)
                           * sizeof(bcmptm_rm_tcam_prio_eid_entry_info_t));
    grp->entry_hash_seg = ((num_entries_per_slice * segment_id)
                           * sizeof(int));
exit:
    SHR_FUNC_EXIT();
    ...
    if (SHR_BITGET(entry_attr->flags.w,BCMPTM_FP_FLAGS_PRESEL_ENTRY)) {
        ...
        if (rv == SHR_E_FULL &&
            ...
            if (rv == SHR_E_RESOURCE) {
                 ...
                 * space for the new one.
                 */
                rv = bcmptm_rm_tcam_fp_group_compress(unit, iomd);
                if (rv == SHR_E_NONE) {
                    /* Try the slice expand again after compression */
                    rv = bcmptm_rm_tcam_fp_slice_expand(unit,
                                                               &new_slice_id,
                                                               iomd);
                }
            }
                                 ...
                                 bcmptm_rm_tcam_entry_iomd_t *iomd)
{
    ...
    } else if (SHR_BITGET(attrs->flags.w, BCMPTM_FP_FLAGS_RESOURCE_SCOPE)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmptm_rm_tcam_fp_req_resources_scope(unit, iomd));
    } else {
