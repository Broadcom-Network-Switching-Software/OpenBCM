...

{
    ...
    bcmptm_rm_tcam_lt_info_t *ltid_info = NULL;
    uint8_t slice_id = 0;
    int8_t virtual_slice_id = -1;
    uint8_t action_prof_id = 0;
            ...
            if (ltid_info->pt_type == LT_PT_TYPE_FP &&
                !(SHR_BITGET(entry_attrs->flags.w,BCMPTM_FP_FLAGS_PRESEL_ENTRY))
                 && ((ltid_info->rm_more_info[0].fp_flags)
                    & BCMPTM_STAGE_COUNTERS_PER_SLICE)
                 && (rm_flags & BCMPTM_RM_TCAM_F_DATA_ONLY_VIEW)) {
                /*
                 * Call chip specific function to update entry words
                 * with correct flex counter ID as per hw slice used
                 */
                ...
                if (((ltid_info->rm_more_info[0].fp_flags)
                    ...
                    /* Get the virtual slice id from slice Id strcuture */
                    virtual_slice_id = slice_fc[slice_id].virtual_slice_id;

                    if (virtual_slice_id == -1) {
                        /* Error scenario */
                        SHR_IF_ERR_EXIT(SHR_E_INTERNAL);
                    }

                    /* Get the flex ctr action id mapped to virtual slice */
                    action_prof_id = fg->counter_profiles[virtual_slice_id];

                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcmptm_rm_tcam_update_flex_ctr_action(unit,
                                                      ltid_info,
                                                      action_prof_id,
                                                      entry_words));
                }
