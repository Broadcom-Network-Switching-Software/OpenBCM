...
/*
 * This function examines the track index range of a shared resource
 * variable-width index LT table for existence, and permits an LT entry
 * insertion if it does not.
 */
int
                                            ...
                                            void *node_cookie)
{
    ...
    if (idx_info == NULL) {
        /* "No-key" LTs should not use this node function. */
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    } else {
        track_index = ltm_buffer[idx_info->index_offset];
        ti_bits = idx_info->track_index_bits;
        if (ti_bits < 2) {
            /*
             * Single-bit track index is handled by the simple
             * global bitmap functions.
             */
            SHR_ERR_EXIT(SHR_E_INTERNAL);
        }
    }
    ...
    if ((track_index + ti_bits)  > global_lt_state->bitmap_size) {
        /* Track index beyond the bounds of the global LT. */
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    }
    ...
    for (tix = track_index; tix < (track_index + ti_bits); tix++) {
        if (SHR_BITGET(global_bitmap, tix)) {
            /* Entry already used by shared LT.  Insert not permitted */
            SHR_ERR_EXIT(SHR_E_RESOURCE);
        }
    }
...
/*
 * This function examines the track index range of a shared resource
 * variable-width index LT table for existence, and removes the range of
 * in-use markers if so.
 */
int
