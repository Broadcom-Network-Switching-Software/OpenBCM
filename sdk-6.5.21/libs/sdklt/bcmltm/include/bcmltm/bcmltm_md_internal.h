...
/*! Logical table which supports only LOOKUP & TRAVERSE. */
#define BCMLTM_LT_FLAGS_READ_ONLY      0x10
...
/*!
 * \brief Field flag - Logical field mapped by subfields
 */
#define BCMLTM_FIELD_MUX_DEMUX     0x4

/*!
 * \brief Field flag - Last subfield for read
 */
#define BCMLTM_FIELD_LAST_SUBFIELD 0x8
 ...
 *
 * Some L2P field maps may transfer only a subset of the full
 * logical field to the physical location.  If the
 * BCMLTM_FIELD_MUX_DEMUX flag is set, the mux_[min,max]bit values
 * indicate the bit boundaries of the subfield mapped to
 * the WBC.
 */
typedef struct bcmltm_field_map_s {
    ...
    /*!
     * The mask to extract this subfield of the 64-bit logical
     * field value.  Equal to ((1 << (maxbit - minbit + 1)) - 1).
     * Only valid if BCMLTM_FIELD_MUX_DEMUX set in the flags member.
     */
    uint64_t mux_mask;

    /*!
     * The minimum bit position of this subfield of the 64-bit logical
     * field value.  Used to shift logical field value right.
     * Only valid if BCMLTM_FIELD_MUX_DEMUX set in the flags member.
     */
    uint8_t mux_minbit;
...
typedef struct bcmltm_ee_index_info_s {
    ...
    /*!
     * The number of bits per Track Index for this LT.
     * Used for the global bitmap record of the wider view of
     * variable-width PT entry maps.
     */
    uint32_t track_index_bits;
