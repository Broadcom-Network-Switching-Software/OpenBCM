...
/*!
 * \brief An EE tree node to validate and update the track indexrecord of a
 * shared resource variable-width simple index table on an INSERT operation.
 *
 * An EE tree node which verifies whether the range of track indexes for a
 * variable-width index table is available for a new modeled LT entry
 * to be placed into a shared resource index managed table.
 * The insert is rejected if any track indexes in the range are
 * already in-use in any of the LTs sharing this
 * device resource.
 *
 * This node updates the global resource in-use bitmap maintained by
 * the master LT of the resource group.
 ...
 */
extern int
bcmltm_ee_node_variable_global_index_insert(int unit,
                                            bcmltm_lt_md_t *lt_md,
                                            bcmltm_lt_state_t *lt_state,
                                            bcmltm_entry_t *lt_entry,
                                            uint32_t *ltm_buffer,
...
/*!
 * \brief An EE tree node to validate and update the track indexrecord of a
 * shared resource variable-width simple index table on an DELETE operation.
 *
 * An EE tree node which verifies whether the range of track indexes for a
 * variable-width index table is available for a new modeled LT entry
 * to be placed into a shared resource index managed table.
 * The delete fails if any track indexes in the range are not recorded as
 * in-use.
 *
 * This node updates the global resource in-use bitmap maintained by
 * the master LT of the resource group.
 ...
 */
extern int
bcmltm_ee_node_variable_global_index_delete(int unit,
                                            bcmltm_lt_md_t *lt_md,
                                            bcmltm_lt_state_t *lt_state,
                                            bcmltm_entry_t *lt_entry,
                                            uint32_t *ltm_buffer,
