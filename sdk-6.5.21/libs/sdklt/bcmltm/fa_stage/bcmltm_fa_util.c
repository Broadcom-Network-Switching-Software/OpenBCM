                             ...
                             uint32_t *ltm_buffer)
{
    ...
    if ((field_map->flags & BCMLTM_FIELD_MUX_DEMUX) != 0) {
        subfield = field_data->data >> field_map->mux_minbit;
        subfield &= field_map->mux_mask;
        shr_uint64_to_uint32_array(subfield, field_val);
    } else {
                             ...
                             uint32_t *ltm_buffer)
{
    ...
    bcmltm_field_value_from_wbc(&(field_map->wbc),
                                field_val, ltm_buffer);
    if ((field_map->flags & BCMLTM_FIELD_MUX_DEMUX) != 0) {
        shr_uint32_array_to_uint64(field_val, &subfield);
        subfield &= field_map->mux_mask;
        field_data->data &=
            ~(field_map->mux_mask << field_map->mux_minbit);
        field_data->data |= (subfield << field_map->mux_minbit);
    } else {
