                                   ...
                                   void *node_cookie)
{
    ...
    if (output) {
        ...
        cur_lta_field = (bcmltd_field_t *)(lta_fields_ptrs + max_fields);
                                 ...
                                 void *node_cookie)
{
    ...
    bool pt_suppress = FALSE, filter_defaults = FALSE;
    ...
    /* May not filter defaults when the map list doesn't exist */
    filter_defaults =
        ((lt_md->params->lt_flags & BCMLTM_LT_FLAGS_READ_ONLY) == 0) ?
        (lt_entry->attrib & BCMLT_ENT_ATTR_FILTER_DEFAULTS) != 0 :
        FALSE;
