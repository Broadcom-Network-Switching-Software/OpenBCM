                                ...
                                bcmltm_fields_required_t fields_required)
{
    ...
    for (fix = 0; fix < field_count; fix++) {
        ...
        if ((fields_required == BCMLTM_FIELDS_REQUIRED_UNSET) &&
            ...
            if ((cur_field_map->flags & BCMLTM_FIELD_MUX_DEMUX) != 0) {
                subfield = api_field_data->data >> cur_field_map->mux_minbit;
                subfield &= cur_field_map->mux_mask;
 ...
 * the all_required parameter.
 *
 * \param [in] base_field_map Pointer to start of this field's list
 *             of LTM map metadata.
 * \param [in] ltm_buffer Pointer to the working buffer for this entry
 * \param [out] field_val64 Return pointer for API field value.
 *
 * \retval None.
 */
static void
bcmltm_fa_node_wb_to_api_value(bcmltm_field_map_t *base_field_map,
                               uint32_t *ltm_buffer,
                               uint64_t *field_val64)
{
    bcmltm_field_map_t *cur_field_map = base_field_map;
    uint32_t field_val[BCMDRD_MAX_PT_WSIZE];
    uint64_t subfield;
    ...
    if ((cur_field_map->flags & BCMLTM_FIELD_MUX_DEMUX) != 0) {
        /*
         * Requirement
         * The linked list of field maps must have the read elements
         * needed to assemble the fully demuxed API field at the
         * start of the list.  Each is marked with the
         * BCMLTM_FIELD_MUX_DEMUX flag.  The last subfield is
         * marked with the BCMLTM_FIELD_LAST_SUBFIELD flag,
         * indicating that the API field is now complete.
         */
        *field_val64 = 0;
        while ((cur_field_map->flags & BCMLTM_FIELD_MUX_DEMUX) != 0) {
            sal_memset(field_val, 0, sizeof(field_val));
            ...
            subfield &= cur_field_map->mux_mask;
            *field_val64 |= (subfield << cur_field_map->mux_minbit);

            if ((cur_field_map->flags & BCMLTM_FIELD_LAST_SUBFIELD) != 0) {
                break;
            }

            if (cur_field_map->next_map != NULL) {
                cur_field_map = cur_field_map->next_map;
            } else {
                /*
                 * Inconsistent metadata:
                 * The current map should be marked as last subfield.
                 */
                return;
            }
        }
                                ...
                                void *node_cookie)
{
    ...
    for (fix = 0; fix < field_count; fix++) {
        ...
        if (pt_suppress) {
            ...
            bcmltm_fa_node_wb_to_api_value(cur_field_map,
                                           ltm_buffer, &field_val64);

            if (lt_entry->attrib & BCMLT_ENT_ATTR_FILTER_DEFAULTS) {
                /*
                 * Compare to default specified by the start of this
                 * field's map chain.
                 */
