                                 ...
                                 bcmltm_valid_bitmap_info_t *vbi)
{
    ...
    for (key_value = cur_key_bound->current_key_min;
        ...
        if (recurse_end) {
            ...
            if (track_index >= vbi->lt_state->bitmap_size) {
                /* Invalid track index.  Perhaps from transform? */
                const char *table_name;

                table_name = bcmltm_lt_table_sid_to_name(unit,
                                                         vbi->lt_state->ltid);
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "LTM: Invalid track index (%d) "
                                      "beyond maximum (%d) for %s(ltid=%d)\n"),
                           track_index, vbi->lt_state->bitmap_size - 1,
                           table_name, vbi->lt_state->ltid));
                SHR_ERR_EXIT(SHR_E_INTERNAL);
            }
                      ...
                      bcmltm_lt_params_t **params_ptr)
{
    ...
    if (table_attr->flag & BCMLTM_TABLE_FLAG_READ_ONLY) {
        params->lt_flags |= BCMLTM_LT_FLAGS_READ_ONLY;
    }
    if (num_fid_idx == 0) {
...
int
bcmltm_md_logical_state_data_size_get(int unit, bcmlrd_sid_t sid,
                                      bcmltm_state_type_t data_type,
                                      uint32_t *data_size)
{
    const bcmltm_table_attr_t *table_attr;
