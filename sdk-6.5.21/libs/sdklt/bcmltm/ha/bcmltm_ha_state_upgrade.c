 ...
 * \param [in] lt_state_data_hap HA pointer to the start of state data list.
 ...
 *  - Compare sizes of LT state data.
 ...
 */
static int
bcmltm_state_data_compress(int unit,
                           ...
                           bcmltm_ha_block_header_t *state_data_blk_hdr)
{
    ...
    uint32_t prev_state_data_size, type_state_data_size,
        new_state_data_size, state_data_move;
    ...
    /*
     * This boundary is (state)->allocated_size because the LT state
     * pointers are already processed.
     */
    max_lt_state =
    ...
    /*
     * This boundary is (state)->block_size because the LT state data
     * pointers are processed here.  Must use the maximum HA block
     * as the boundary until (state)->allocated_size is updated
     * at the end of this function.
     */
    state_data_end = state_data_base + state_data_blk_hdr->block_size;

    while ((state_data_from + sizeof(bcmltm_lt_state_data_t)) <=
           state_data_end) {
        cur_state_data = (bcmltm_lt_state_data_t *) state_data_from;
        prev_state_data_size = sizeof(bcmltm_lt_state_data_t);
        prev_state_data_size += cur_state_data->state_data_size;

        if ((state_data_from + prev_state_data_size) > state_data_end) {
            /* Inconsistent previous HA records */
            return SHR_E_FAIL;
        }

        if (cur_state_data->ltid == BCMLTM_LTID_INVALID) {
            state_data_from += prev_state_data_size;
            last_ltid = BCMLTM_LTID_INVALID;
            continue;
        }

        rv = bcmltm_md_logical_state_data_size_get(unit,
                                                   cur_state_data->ltid,
                                                   cur_state_data->state_data_type,
                                                   &type_state_data_size);
        if (SHR_FAILURE(rv)) {
            return rv;
        }

        new_state_data_size = 0;
        if (type_state_data_size > 0) {
            new_state_data_size +=
                BCMLTM_WORDSIZEBYTES(sizeof(bcmltm_lt_state_data_t));

            /* Size of state data */
            new_state_data_size += BCMLTM_WORDSIZEBYTES(type_state_data_size);
        }

        if (prev_state_data_size < new_state_data_size) {
            /* State has grown, need new area */
            return SHR_E_INTERNAL;
        } else if (prev_state_data_size > new_state_data_size) {
            /* State shrank.  What is still valid? */
            state_data_move = new_state_data_size;
        } else {
            /* Sizes are equal */
            state_data_move = prev_state_data_size;
        }

        if (state_data_to != state_data_from) {
            /* Some LT has been skipped or state data shrunk */
            ...
            sal_memmove(state_data_to, state_data_from, state_data_move);
                    ...
                    bcmltm_ha_block_header_t *state_data_blk_hdr)
{
    ...
    /*
     * Three passes:
     * 1. Update all the LTIDs
     * 2. Sort all the entries based on their new LTID
     * 3. Insert all the LTIDs that did not exist before
     */
    ...
    for (idx = 0, it = state_ha_ptr; idx < num_sid; idx++) {
        sid = sid_list[idx];
        lt_state = (bcmltm_lt_state_t *)it;
        if (recovered_ltids == 0) {
            /*
             * The recovered_ltids count above indicates when all prior LTs
             * are processed.  Any LTs after the count is exhausted must
             * be new LTs.  The lt_state is invalid at the end of that list --
            ...
            lt_state->ltid = sid;
            /* Other elements will be filled out during metadata init. */
        } else if (sid < lt_state->ltid) {
            /* New LT */
            ...
            /*
             * With the abandoned LTIDs sorted to the end of the list
             * and the math above, this case should not occur.
             */
            return SHR_E_INTERNAL;
        } else {
            /* Matching sid indicates this is an existing LT */
            recovered_ltids--;
        }
        it += inc_steps;
    }
     ...
     * Or some state data may have decreased in size.
     */
    rv = bcmltm_state_data_compress(unit,
                                    state_ha_ptr,
                                    state_blk_hdr,
                                    state_data_ha_ptr,
                                    state_data_blk_hdr);
...
bcmltm_unit_state_upgrade(int unit, bcmltm_ha_state_sizes_t *ha_state_sizes)
{
    ...
    if (block_size < sizeof(bcmltm_ha_block_header_t)) {
        /* Recovered block is too small for coherent record */
