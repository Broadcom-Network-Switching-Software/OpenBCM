                 ...
                 shr_pb_t *pb)
{
    ...
    shr_pb_printf(pb, "%s",
                  (flags & BCMLTM_FIELD_MUX_DEMUX) ? " mux-demux" : "");
    shr_pb_printf(pb, "%s",
                  (flags & BCMLTM_FIELD_LAST_SUBFIELD) ? " last-subfield" : "");
    shr_pb_printf(pb, "}\n");
        ...
        for (i = 0; i < field_list->num_fields; i++) {
            ...
            while (field != NULL) {
                ...
                shr_pb_printf(pb, "%sapic_ofs=%-5d\n",
                              field_prefix3,
                              field->apic_offset);
                shr_pb_printf(pb, "%smux_mask=0x%" PRIx64 " mux_minbit=%d\n",
                              field_prefix3,
                              field->mux_mask, field->mux_minbit);
