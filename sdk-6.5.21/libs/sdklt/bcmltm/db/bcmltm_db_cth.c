...
typedef struct cth_map_entries_s {
    ...
    /*! Maximum number of field elements required for this LTA interface. */
    uint32_t max_fields;
 ...
 *
 * This supports table definitions with only 1 custom table handler
 ...
 * \param [in] read_only TRUE if only LOOKUP & TRAVERSE opcodes supported.
 ...
 */
static int
cth_map_entries_parse(int unit, bcmlrd_sid_t sid, bool read_only,
                      cth_map_entries_t *lt_map)
{
    ...
    uint32_t max_fields = 0;  /* LTA interface field size */
    unsigned int size;
    uint32_t i;
    uint32_t fid;
    uint32_t idx;
    ...
    for (i = 0; i < fid_list_count; i++) {
        ...
        for (idx = 0; idx < idx_count; idx++) {
            ...
            if (field_def.key) {
                ...
                /* Only record key fields for read-only CTH */
                continue;
            }
                  ...
                  bcmltm_lta_field_list_t **flist_ptr)
{
    ...
    flist->max_fields = lt_map->max_fields;
...
int
bcmltm_db_cth_info_create(int unit, bcmlrd_sid_t sid, bool read_only,
                          bcmltm_db_cth_info_t **info_ptr)
{
    ...
    SHR_IF_ERR_EXIT
        (cth_map_entries_parse(unit, sid, read_only, &map_entries));
    ...
    SHR_IF_ERR_EXIT
        (bcmltm_wb_block_lta_cth_add(unit, map_entries.max_fields,
