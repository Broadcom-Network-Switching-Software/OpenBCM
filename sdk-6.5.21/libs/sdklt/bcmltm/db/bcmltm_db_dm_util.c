...
/*!
 * \brief Find field map for a given field ID and index.
 *
 * This routine searches the given field list for a field
 * map that matches the provided field ID and index.
 *
 * The returning field map is the array element (or base).
 *
 * \param [in] flist Field list to search.
 * \param [in] fid Field ID to find.
 * \param [in] idx Field index to find.
 *
 * \retval Pointer to matching field map, if found.
 * \retval NULL, if not found.
 */
static bcmltm_map_field_mapped_t *
field_map_find(bcmltm_map_field_mapped_list_t *flist,
               uint32_t fid,
               uint32_t idx)
{
    ...
    /* Find field element in list */
    for (i = 0; i < flist->num; i++) {
        if ((flist->field_maps[i].src_field_id == fid) &&
            (flist->field_maps[i].src_field_idx == idx)) {
...
/*!
 * \brief Initialize field maps for a new field.
 *
 * This routine adds as many field maps as necessary to cover
 * all the indices required by the new logical field.  The new
 * field maps are assigned per field (fid,idx) in an ascending
 * sequential order, as required later by the LTM metadata design.
 *
 * In addition, the new field maps are initialized
 * with core information, which are properties of the field rather
 * than the field mapping, such as id, idx, defaults.
 *
 * If the field is already present in the list, nothing is performed.
 *
 * \param [in] unit Unit number.
 * \param [in] sid Table ID.
 * \param [in] fid Logical field ID.
 * \param [in, out] flist Field list to add initial new field maps to.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
field_maps_init(int unit,
                bcmlrd_sid_t sid,
                uint32_t fid,
                bcmltm_map_field_mapped_list_t *flist)
{
    ...
    field_map = field_map_find(flist, fid, 0);
    if (field_map != NULL) {
        /* Found, nothing to do */
        SHR_EXIT();
    }
    ...
    /* Check array size */
    if ((flist->num + idx_count) > flist->max) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "Field map array too small: "
                              "expected(minimum)=%d actual=%d\n"),
                   (flist->num + idx_count), flist->max));
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    }
    ...
    /*
     * Add field maps for all field indices required by the logical field.
     *
     * The field maps required for all the indices of a field
     * need to be placed in ascending sequential order.
     */
    for (idx = 0; idx < idx_count; idx++) {
        field_map = &flist->field_maps[flist->num++];

        /* Set initial information */
        field_map->src_field_id = fid;
        ...
        /*
         * A NULL ptm_wb_block_block indicates that this field map
         * is not yet used by any field map entry.
         */
        field_map->ptm_wb_block = NULL;
...
/*!
 * \brief Add a field map for given field ID, index.
 *
 * Add a field map object for the specified field ID and index
 * in the given field map list.
 *
 * The returning new field map is initialized with core information,
 * which are properties of the field rather than the field mapping,
 * such as id, idx, defaults.
 *
 * This routine places the new field map in the location as expected
 * by the corresponding node cookies design:
 * - The field map list consists of array elements with unique field ID, index.
 * - Field maps with the same field ID and index are placed
 *   across the link list of its field array element.
 *
 * \param [in] unit Unit number.
 * \param [in] sid Table ID.
 * \param [in] fid Logical field ID.
              ...
              bcmltm_map_field_mapped_t **field_map_ptr)
{
    ...
    /* Initialize field maps for a given field (if needed) */
    SHR_IF_ERR_EXIT
        (field_maps_init(unit, sid, fid, flist));

    field_map = field_map_find(flist, fid, idx);
    if (field_map == NULL) {
        /* The init function should have added the field map */
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "Field map not found in list array: "
                              "sid=%d fid=%d idx=%d\n"),
                   sid, fid, idx));
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    }
    ...
    /*
     * Check if map array element (base) is used by a field map entry:
     *   If unused, use the map array element.
     *   Otherwise, allocate a new field map and add it to the link list.
     */
    if (field_map->ptm_wb_block == NULL) {
        *field_map_ptr = field_map;
        SHR_EXIT();
    }
    ...
    /* Copy basic information from base field map */
    new_field_map->src_field_id  = field_map->src_field_id;
    new_field_map->src_field_idx = field_map->src_field_idx;
    new_field_map->default_value = field_map->default_value;
    new_field_map->minimum_value = field_map->minimum_value;
    new_field_map->maximum_value = field_map->maximum_value;
    new_field_map->flags = field_map->flags;
    new_field_map->wbc_minbit = BCMLTM_FIELD_BIT_POSITION_INVALID;
    new_field_map->wbc_maxbit = BCMLTM_FIELD_BIT_POSITION_INVALID;
                       ...
                       bcmltm_db_dm_map_entries_t *lt_map)
{
    ...
    for (idx = 0; idx < idx_count; idx++) {
        ...
        SHR_IF_ERR_EXIT
            (field_map_add(unit, sid, lfid, idx, flist, &field_map));
