...
/* Arbitrary high table field length, to accommodate for array depth */
#define TABLE_OP_DOP_CONTROL_MAX_FIELD_LEN 256

static shr_famm_hdl_t table_op_dop_info_arr_hdl[BCMDRD_CONFIG_MAX_UNITS];
static shr_famm_hdl_t table_op_dop_control_arr_hdl[BCMDRD_CONFIG_MAX_UNITS];
...
static int
bcmltm_table_op_dop_control_update_data(int unit, uint32_t lt_id, int *port)
{
    ...
    in_flds.count = 1;
    in_flds.field = shr_famm_alloc(table_op_dop_control_arr_hdl[unit], 1);

    if (in_flds.field == NULL) {
        SHR_ERR_EXIT(SHR_E_MEMORY);
    }

    out_flds.count = TABLE_OP_DOP_CONTROL_MAX_FIELD_LEN;
    out_flds.field = shr_famm_alloc(table_op_dop_control_arr_hdl[unit],
                                    TABLE_OP_DOP_CONTROL_MAX_FIELD_LEN);
    ...
    fld = TABLE_OP_DOP_CONTROLt_TABLE_IDf;
    in_flds.field[0]->id = fld;
    in_flds.field[0]->data = lt_id;

    /* Lookup the entry. Update the entry if exists. */
    rv = bcmimm_entry_lookup(unit, TABLE_OP_DOP_CONTROLt, &in_flds, &out_flds);
    if (rv == SHR_E_NONE) {
        /* Extract the PORT ID */
        for (fld = 0; fld < out_flds.count; fld++) {
            if (out_flds.field[fld]->id == TABLE_OP_DOP_CONTROLt_PORT_IDf) {
                *port = out_flds.field[fld]->data;
                break;
            }
        }
    } else if (rv == SHR_E_NOT_FOUND) {
       SHR_IF_ERR_EXIT(bcmimm_entry_insert(unit, TABLE_OP_DOP_CONTROLt, &in_flds));
    }

exit:
    if (in_flds.field) {
        shr_famm_free(table_op_dop_control_arr_hdl[unit], in_flds.field, in_flds.count);
    }
    if (out_flds.field) {
        shr_famm_free(table_op_dop_control_arr_hdl[unit], out_flds.field,
                      TABLE_OP_DOP_CONTROL_MAX_FIELD_LEN);
    }
                                     ...
                                     bcmltm_table_op_dop_info_t *ptrm_status)
{
    ...
    bcmltm_table_op_dop_control_update_data(unit, ptrm_status->lt_id, &port);
...
bcmltm_table_op_dop_control_init(int unit)
{
    ...
    if (!table_op_dop_control_arr_hdl[unit]) {
        SHR_IF_ERR_EXIT
            (shr_famm_hdl_init(TABLE_OP_DOP_CONTROL_MAX_FIELD_LEN,
                               &table_op_dop_control_arr_hdl[unit]));
    }
...
{
    if (table_op_dop_control_arr_hdl[unit]) {
        shr_famm_hdl_delete(table_op_dop_control_arr_hdl[unit]);
        table_op_dop_control_arr_hdl[unit] = 0;
    }
