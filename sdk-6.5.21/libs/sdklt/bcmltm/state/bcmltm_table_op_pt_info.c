...
/* Marks whether an interactive op is in progress on an LT */
static SHR_BITDCL ireq_op_status[BCMDRD_CONFIG_MAX_UNITS][SHRi_BITDCLSIZE(BCMLTD_TABLE_COUNT)];
...
static int
bcmltm_table_op_pt_info_trans_in_progress(int unit, bcmltd_sid_t lt_id,
                                  uint32_t trans_id, uint8_t is_ireq)
{
    if (is_ireq) {

        if (SHR_BITGET(ireq_op_status[unit], lt_id)) {
            return TRUE;
        }
    } else {

        if ((trans_id != 0xFFFFFFFF) &&
            (trans_id == last_trans_id[unit][lt_id])) {
            return TRUE;
        }
    }
                                 ...
                                 uint8_t is_ireq)
{
    ...
    if (bcmimm_entry_lookup(unit, TABLE_OP_PT_INFOt,
        ...
        if (bcmltm_table_op_pt_info_trans_in_progress(unit, ptrm_status->lt_id,
                                     ptrm_status->lt_trans_id, is_ireq))
        {
    ...
    if (!is_ireq) {
        ...
        SHR_BITSET(ireq_op_status[unit], ptrm_status->lt_id);
    }
exit:
    if (in_flds.field) {
...
bcmltm_table_op_pt_info_init(int unit)
{
    ...
    sal_memset(last_trans_id[unit], 0xFF, sizeof(last_trans_id[unit]));
    /* Initialize all bits to 0 */
    sal_memset(ireq_op_status[unit], 0x0, sizeof(ireq_op_status[unit]));
    }
                               ...
                               bcmptm_keyed_lt_mrsp_info_t *mrsp_info)
{
    ...
    SHR_IF_ERR_EXIT(bcmltm_table_op_pt_info_update_data(unit, &ptrm_status, 0));
                         ...
                         bcmbd_pt_dyn_info_t *pt_dyn_info)
{
    ...
    ptrm_status.lt_id = ltid;
    /* lt_trans_id not used in case of interactive op, set to default */
    ptrm_status.lt_trans_id = 0xFFFFFFFF;
    ptrm_status.pt_sid_cnt = 1;
    ptrm_status.pt_sid[0] = ptid;
    ptrm_status.pt_index_cnt = 1;
    ptrm_status.pt_index[0] = pt_dyn_info->index;
    ...
    SHR_IF_ERR_EXIT(bcmltm_table_op_pt_info_update_data(unit, &ptrm_status, 1));
...
{
    SHR_BITCLR(ireq_op_status[unit], ltid);
}
