...
/*!
 * \brief Forward commit/abort to xform and CTH handlers.
 *
 * CTH and some LTA transforms track internal state during LT ops.
 * HA requires notification of commit/abort status of the overall operation
 * either per-op or at the end of an atomic transaction.
 *
 * \param [in] unit Unit number.
 * \param [in] trans_id Transaction ID.
 * \param [in] intf Handler interface type to call TABLE_COMMIT_INTF_.
 * \param [in] tc_list Table Commit handler list.
 * \param [in] cth_handler Pointer to the CTH handler for this LT.
 *
 * \retval SHR_E_NONE No errors.
 */
static int
bcmltm_lta_status_notify(int unit,
                         uint32_t trans_id,
                         table_commit_intf_t intf,
                         const bcmltm_table_commit_list_t *tc_list,
                         const bcmltd_table_handler_t *cth_handler)
{
    SHR_FUNC_ENTER(unit);
    ...
    if (cth_handler != NULL) {
        if (cth_handler->ha != NULL) {
            if (intf == TABLE_COMMIT_INTF_ABORT) {
                SHR_IF_ERR_VERBOSE_EXIT
...
ltm_entry_operation(bcmltm_entry_t *ltm_entry, bool passthru)
{
    ...
    if (BCMLTM_HA_PTR_IS_INVALID(lt_state_hap)) {
        ...
        if (lt_state->max_entry_count == 0) {
            /* LT is disabled.  No ops supported. */
            SHR_ERR_EXIT(SHR_E_UNAVAIL);
        }
    }
    ...
    if (lt_md != NULL) {
        ...
        if (table_catg == BCMLTM_TABLE_CATG_LOGICAL) {
            ...
            if ((bcmltm_trans_status[unit] == NULL) &&
                ...
                /*
                 * This ERR macro doesn't jump to exit, but does
                 * log a message.  We must use this since we're already
                 * in the exit code.
                 */
                SHR_IF_ERR_CONT
                    (bcmltm_lta_status_notify(unit, ltm_entry->trans_id,
                                              intf,
                                              lt_md->tc_list,
                                              lt_md->cth));
            } else if ((lt_md->params->lt_flags &
                        BCMLTM_LT_FLAGS_MODELED) == 0) {
                /* Notify PT op status of end of interactive op */
                bcmltm_pt_status_ireq_op_complete(unit, table_id);
            }
        }
              ...
              void *context)
{
    ...
    if ((bcmltm_trans_status[unit] != NULL) &&
        ...
        for (ltix = 0; ltix < trans_status->lt_num; ltix++) {
            ...
            /* Forward commit to LTA handlers. */
            SHR_IF_ERR_CONT
                (bcmltm_lta_status_notify(unit, trans_id,
                                          TABLE_COMMIT_INTF_COMMIT,
                                          lt_md->tc_list,
                                          lt_md->cth));
