...
/* Value range 1st element set. */
static bool value_range_1st_elem_set = false;
                                 ...
                                 bool *found)
{
    ...
    *found = false;
    for (unit = 0; unit < BCMDRD_CONFIG_MAX_UNITS; unit++) {
        ...
        if (!sal_strncasecmp(bcmdrd_dev_name(unit), str, len)) {
            SHR_IF_ERR_EXIT
                (bcmcfg_read_device_add_unit(tbl_info, unit));
            *found = true;
        }
                                        ...
                                        const char *str)
{
    ...
    if (!sal_strcmp(str, "")) {
        /*
         * Handle an empty scalar value. The only context where this
         * is known to occur is after a complex key that has no
         * associated values, where empty scalar serves to mark the
         * end of the complex key clause. There are no further actions
         * needed for this particular case, so the empty scalar may be
         * otherwise safely ignored.
         */
       SHR_EXIT();
    } else if (field->flags & BCMLTD_FIELD_F_ENUM) {
        /* check for enum */
        SHR_IF_ERR_EXIT
                          ...
                          void *user_data)
{
    ...
    case VALUE_RANGE:
        /*
         * Range specification is allowed for value field.
         * But only 1st element is allowed, 2nd element has to be rejected.
         */
        if (!value_range_1st_elem_set) {
            SHR_IF_ERR_EXIT
                (bcmcfg_read_device_handle_field(user,
                                                 info,
                                                 tbl_info,
                                                 info->state,
                                                 value));
            value_range_1st_elem_set = true;
        } else {
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
                                  "%s:%d:%d: Too many elements specified"
                                  " for value field.\n"),
                       info->locus,
                       info->line,
                       info->column));
            ...
            case BCMCFG_READ_STATE_MAP_V:
                if (tbl_info->state == FIELD) {
                    ...
                    if (array) {
                        ...
                        SHR_IF_ERR_EXIT
                            (bcmcfg_read_device_is_key_field(tbl_info, &key));
                        if (key) {
                            tbl_info->state = KEY_RANGE;
                        } else {
                            tbl_info->state = VALUE_RANGE;
                            value_range_1st_elem_set = false;
                        }
                    }
                }
                break;
