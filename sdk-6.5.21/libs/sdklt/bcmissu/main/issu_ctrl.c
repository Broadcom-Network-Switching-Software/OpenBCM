...
static int issu_warm_init(int unit, uint32_t blk_size)
{
    ...
    while (blk && blk->blk && !blk->blk->last &&
        ...
        blk->next = NULL;
        if (unit == BCMISSU_NO_UNIT) {
            blk->blk = bcmha_mem_alloc(BCMHA_GEN_UNIT,
...
exit:
    if (SHR_FUNC_ERR()) {
        ...
        if (blk->blk) {
            if (unit == BCMISSU_NO_UNIT) {
                bcmha_mem_free(BCMHA_GEN_UNIT, blk->blk);
                issu_gen_sub_comp_ids--;
            } else {
...
static int handle_general_checks(shr_ha_mod_id comp_id, bool warm)
{
    ...
    issu_general_ctrl = bcmha_mem_alloc(BCMHA_GEN_UNIT,
                                        comp_id, SHR_HA_SUB_IDS-1,
                                        (warm ? NULL : ha_id),
                                        &blk_size);
    SHR_NULL_CHECK(issu_general_ctrl,  SHR_E_MEMORY);
    if (blk_size < sizeof(*issu_general_ctrl)) {
        issu_general_ctrl = bcmha_mem_realloc(BCMHA_GEN_UNIT,
                                              issu_general_ctrl,
    ...
    if (!warm || bcmissu_is_active()) {
        if (len >= BCMISSU_SW_SIGNATURE_MAX_LEN - 1) {
            SHR_ERR_EXIT(SHR_E_INTERNAL);
        }
        sal_memcpy(issu_general_ctrl->software_signature,
                   BCMISSU_LIBSUM_STR,
                   len);
...
int bcmissu_struct_init(int unit, shr_ha_mod_id comp_id, bool warm)
{
    ...
    if (unit == BCMISSU_NO_UNIT) {
        ...
        blk = bcmha_mem_alloc(BCMHA_GEN_UNIT,
                              comp_id,
                              issu_gen_sub_comp_ids++,
                              (warm ? NULL : ha_id),
                              &blk_size);
                            ...
                            bcmissu_ha_blk_inc_t *ha_blk_info)
{
    ...
    /* Resize the HA block */
    if (unit == BCMISSU_NO_UNIT) {
        ha_blk = bcmha_mem_realloc(BCMHA_GEN_UNIT, ha_blk, new_blk_size);
        block = issu_gen_comp_s_ctrl;
    } else {
    ...
    /* Clear the content of the new HA block */
    sal_memset(ha_blk, 0, new_blk_size);
    ...
    for (blk_s_info_it = blk_s_info_chain; blk_s_info_it;) {
        ...
        /*
         * Check if the new structure starts after the previous structure
         * finished. If it does, copy the content of the gap.
         */
        if (s) {
            if (s < src) {
                sal_memcpy(dest, s, src - s);
            }
        } else if (s_info->offset > 0) {
            sal_memcpy(ha_blk, blk_cpy, s_info->offset);
        }
        ...
        s = src; /* We copied the source up to 'src', save it */
    ...
    /* If there is more memory at the top to be copied */
    if (s && s < blk_cpy + ha_blk_info->orig_size) {
        sal_memcpy(dest, s, blk_cpy + ha_blk_info->orig_size - s);
    }
                               ...
                               bcmissu_struct_id_t id)
{
    ...
    /* Find the last block */
    while (block->blk->num_of_elements == block->blk->block_size &&
           block->next) {
        block = block->next;
    }
    blk = block->blk;
    if (blk->num_of_elements == blk->block_size) {
        ...
        shr_ha_str_to_id("issu_track",
                         ...
                         ha_id);
        if (unit == BCMISSU_NO_UNIT) {
            block->blk = bcmha_mem_alloc(BCMHA_GEN_UNIT,
                                         BCMMGMT_ISSU_COMP_ID,
                                         issu_gen_sub_comp_ids++,
                                         ha_id,
                                         &blk_size);
        ...
        if (blk_size < desired_size) {
            if (unit == BCMISSU_NO_UNIT) {
                bcmha_mem_free(BCMHA_GEN_UNIT, block->blk);
                issu_gen_sub_comp_ids--;
            } else {
                            ...
                            uint32_t *offset)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT(
        bcmissu_struct_info_ext_get(unit, id, comp_id,
                                    sub_id, NULL, offset));
exit:
    SHR_FUNC_EXIT();
}

int bcmissu_struct_info_ext_get(int unit,
                                bcmissu_struct_id_t id,
                                shr_ha_mod_id comp_id,
                                shr_ha_sub_id sub_id,
                                size_t *instances,
                                uint32_t *offset)
{
    ...
    if (struct_info) {
        *offset = struct_info->offset;
        if (instances) {
            *instances = struct_info->instances;
        }
    } else {
        SHR_ERR_EXIT(SHR_E_NOT_FOUND);
    }
