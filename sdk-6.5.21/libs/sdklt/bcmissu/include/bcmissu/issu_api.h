...
/*!
 * \brief Retrieve the location and instances of an ISSU data structure.
 *
 * This function is an extension of \ref bcmissu_struct_info_get() where
 * in addition to the structure offset location the function also returns the
 * number of instances of the structure within this location.
 *
 * \param [in] unit The unit associated with the HA memory.
 * \param [in] id Unique generated structure ID.
 * \param [in] comp_id Is the component ID that allocated the HA block.
 * \param [in] sub_id Is the sub module ID that together with the \c comp_id
 * identifies the HA block.
 * \param [out] instances The number of structure instances in this location.
 ...
 * \retval SHR_E_NOT_FOUND The structure ID was not found.
 */
extern int bcmissu_struct_info_ext_get(int unit,
                                       bcmissu_struct_id_t id,
                                       shr_ha_mod_id comp_id,
                                       shr_ha_sub_id sub_id,
                                       size_t *instances,
