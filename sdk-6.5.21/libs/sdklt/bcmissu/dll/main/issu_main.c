...
typedef struct traced_struct_s {
    bcmissu_struct_id_t id;
    struct traced_struct_s *next;
} traced_struct_t;

static const char *start_from_ver;
static const char *current_ver;
static bool native_env;
static uint32_t start_from_ver_idx;
static uint32_t current_ver_idx;
...
/*!
 * Find the DB index of a given version.
 *
 * \param [in] ver The version to search for.
 * \param [out] last_idx The index of the target version in the ISSU DB.
 *
 * \return SHR_E_NONE on success
 * \return SHR_E_FAIL If the version was not found.
 */
static int retrieve_version_idx(const char *ver, uint32_t *ver_idx)
{
    const issu_version_list_t *issu_struct_dict = bcmissu_struct_db_get();
    int len = sal_strlen(ver);
    uint32_t idx;

    for (idx = 0; issu_struct_dict[idx].ver_db != NULL; idx++) {
        if (sal_strncmp(ver,
                        issu_struct_dict[idx].version,
                        len) == 0) {
            *ver_idx = idx;
            return SHR_E_NONE;
        }
...
bool bcmissu_version_in_range(const char *ver)
{
    uint32_t idx;
    const issu_version_list_t *issu_struct_dict = bcmissu_struct_db_get();
    size_t s_len = strlen(ver);

    for (idx = start_from_ver_idx; idx <= current_ver_idx; idx++) {
        if (strlen(issu_struct_dict[idx].version) == s_len &&
            strcmp(issu_struct_dict[idx].version, ver) == 0) {
            return true;
        }
    }
    return false;
                           ...
                           bool native)
{
     ...
     */
    if (retrieve_version_idx(from_ver, &idx) != SHR_E_NONE) {
        return SHR_E_PARAM;
    }
    start_from_ver_idx = idx;
    if (retrieve_version_idx(to_ver, &idx) != SHR_E_NONE) {
        return SHR_E_PARAM;
    }
    current_ver_idx = idx;
...
int bcmissu_data_resize(int unit)
{
    ...
    traced_struct_t *visited_struct = NULL;
    traced_struct_t *struct_it;
    const issu_version_list_t *issu_struct_dict = bcmissu_struct_db_get();
    ...
    while (rv == SHR_E_NONE) {
        ...
        for (j = 0; from[j].struct_info != NULL; j++) {
            ...
            if (!struct_it) {
                ...
                bcmissu_size_update(unit, &from[j], start_idx,
                                    current_ver_idx, &ha_blk_list);
            }
        }
        ver = issu_struct_dict[next_idx].version;
        rv = retrieve_versions_struct(ver,  &start_idx,  &next_idx);
...
int bcmissu_data_upgrade(int unit)
{
    ...
    while (rv == SHR_E_NONE) {
        ...
        for (j = 0; from[j].struct_info != NULL; j++) {
            /*
             * We want to visit every structure in the DB only once so we
             * track all the visited structure IDs.
             */
            for (struct_it = visited_struct;
                 struct_it && struct_it->id != from[j].id;
                 struct_it = struct_it->next);
            ...
            if (!struct_it) {
                /* Structure was not handled before */
                SHR_ALLOC(struct_it, sizeof(*struct_it), "bcmissuStTrack");
                SHR_NULL_CHECK(struct_it, SHR_E_MEMORY);
                struct_it->next = visited_struct;
                struct_it->id = from[j].id;
                visited_struct = struct_it;
                SHR_IF_ERR_EXIT
                    (bcmissu_struct_upgrade(unit, &from[j], start_idx,
                                            current_ver_idx));
            }
        }
        ver = issu_struct_dict[next_idx].version;
