                              ...
                              patch_record_t *ha_rec)
{
    ...
    /* First check if the patch device matches the current device */
    if (patch->device[0] != '*' && dev_type_name && dev_type_name[0] != '\0') {
        if (sal_strncasecmp(patch->device, dev_type_name,
                            strlen(dev_type_name)) != 0) {
            return false;
        }
    }
    /* Second check if the patch variant matches the current variant */
    if (patch->variant[0] != '*' && var_name && var_name[0] != '\0') {
        if (sal_strncasecmp(patch->variant, var_name, strlen(var_name)) != 0) {
            return false;
        }
    }
    /* Third check if the patch had been already applied. */
