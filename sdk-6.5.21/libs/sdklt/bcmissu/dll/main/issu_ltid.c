...
static const char *find_old_fld_name(int unit,
                                     const char *lt_name,
                                     const char *fname)
{
    const bcmissu_lt_renamed_fields_t  *fld_dict_it;
    size_t lt_len = strlen(lt_name);
    size_t fname_len;
    const char *dev_type_name = bcmdrd_dev_type_str(unit);
    const char *var_name = bcmlrd_variant_name(unit);

    fld_dict_it = bcmissu_renamed_fields_dict_get();
    while (fld_dict_it && fld_dict_it->lt) {
        if (strlen(fld_dict_it->lt) == lt_len &&
            strcmp(lt_name, fld_dict_it->lt) == 0) {
            fname_len = strlen(fname);
            if ((strlen(fld_dict_it->old_fname) == fname_len) &&
                (strcmp(fld_dict_it->old_fname, fname) == 0) &&
                ((strcmp(fld_dict_it->device, "all") == 0) ||
                 (sal_strncasecmp(fld_dict_it->device, dev_type_name,
                                  strlen(dev_type_name)) == 0)) &&
                ((strcmp(fld_dict_it->variant, "all") == 0) ||
                 (sal_strncasecmp(fld_dict_it->variant, var_name,
                                  strlen(var_name)) == 0)) &&
                 bcmissu_version_in_range(fld_dict_it->ver)) {
                return fld_dict_it->new_fname; /* We found the entry */
            }
        }
        fld_dict_it++;
    }
                             ...
                             uint32_t *fid)
{
    ...
    do {
        ...
        *fid = BCMLTD_SID_INVALID;
        fname = fld_ent->f_name;
        while (1) {
            inp_len = strlen(fname);
            for (j = 0; j < num_fid; j++) {
                if ((inp_len == strlen(f_info[j].name)) &&
                    sal_strcmp(fname, f_info[j].name) == 0)  {
                    *fid = f_info[j].id;
                    break;
                }
            }
            if (j == num_fid && !old_name_tried) {
                fname = find_old_fld_name(unit, tbl_ent->lt_name, fname);
                old_name_tried = true;
                if (!fname) {
                    break;
                }
