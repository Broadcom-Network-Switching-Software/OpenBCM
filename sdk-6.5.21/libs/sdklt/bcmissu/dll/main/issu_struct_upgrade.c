 ...
 * \param [in] last_idx The DB index associated with the target version.
                                 ...
                                 uint64_t fld_cnt)
{
    ...
    /* Search if this structure was previously changed */
    for (idx = from_db_idx; idx < last_idx; idx++) {
        rv = bcmissu_struct_def_find(fld->struct_id, idx, &from);
        if (SHR_SUCCESS(rv)) {
            break;
        }
    }
    if (idx >= last_idx) {
        /* The data structure definition didn't change */
     ...
     */
    rv = bcmissu_struct_def_find(from->id, last_idx, &to);
    /*
    ...
    update_fields(from_db_idx, last_idx, from, to,
                  src, fld_cnt & 0xFFFFFFFF, dest);
}
                          ...
                          uint8_t *blk)
{
    ...
    for (l = 0; l < instances; l++) {
        ...
        for (j = 0, fld = from->struct_info->fields;
            ...
            /* Now find the field count (array or scalar) */
            ...
            if (fld->struct_id != ISSU_INVALID_STRUCT_ID &&
                (!fld->is_local_enum)) {
                update_nested_struct(fld, from_db_idx, last_idx,
                                     src, dest, fld_cnt);
            } else {
                /*
                         ...
                         bcmissu_comp_s_info_t *s_info)
{
    ...
    for (j = 0; j < s_info->instances; j++) {
        ...
        update_fields(db_idx, last_idx, from, to, s_copy, 1, blk);
        /*
 ...
 */
int bcmissu_struct_def_find(bcmissu_struct_id_t id,
                            uint32_t idx,
                           ...
                           uint32_t last_idx)
{
    ...
    rv = bcmissu_struct_def_find(from->id, last_idx, &next_db);
    if (rv != SHR_E_NONE) {
        return rv;
    }
    ...
    while (s_info) {
        update_struct(unit, idx, last_idx, from, next_db, s_info);
