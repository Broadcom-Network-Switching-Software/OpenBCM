...
bcmlm_drv_detach(int unit)
{
    ...
    /* Nothing to do if there is no driver */
    SHR_NULL_CHECK(bcmlm_drv[unit], SHR_E_NONE);
...
bcmlm_port_update(int unit, shr_port_t port, int link)
{
    ...
    rv = bcmpc_pmgr_port_link_change(unit, pport, link);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_UP(unit, port,
                               "Failed in bcmpc_pmgr_port_link_change call: "
                               "port %d link %d\n"),
                   port, link));
        SHR_ERR_EXIT(rv);
    }
