...
static void
id_str_check(const char *blk_id)
{
    ...
    while (section && j < this->msec_free_idx) {
        ...
        while (blk_hdr < section->mem_end) {
            if (blk_hdr->signature == HA_MEM_SIGNATURE &&
                ...
                if (write_f(ctx, blk_hdr, blk_hdr->length) !=
                        (int)blk_hdr->length) {
                    sal_mutex_give(this->mutex);
                    return SHR_E_IO;
                }
               ...
               void *ctx)
{
    ...
    rv = read_f(ctx, &blk_hdr_in, sizeof(blk_hdr_in));
