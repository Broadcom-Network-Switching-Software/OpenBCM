                           ...
                           shr_itbm_list_params_t iparams)
{
    return ((lparams.unit == iparams.unit
             ...
             && lparams.reverse == iparams.reverse)
            ? 1 : 0);
}
                          ...
                          bool shutdown)
{
    ...
    if (shr_itbm_list_params_match(lparams, params)) {
         ...
         * if they are in-use and shutdown is not set return SHR_E_BUSY
         * error.
         */
        if (alist->num_allocs != 0 && shutdown == 0) {
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
    ...
    while (alist->next) {
        ...
        if (shr_itbm_list_params_match(lparams, params)) {
             ...
             * error.
             */
            if ((alist->next)->num_allocs !=0 && shutdown == 0) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
    ...
    if (!ctrl[unit]->lcount) {
        ...
        if (found) {
            ...
            if (mlinfo->shutdown) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                                        "LID=%u shared list shutdown(=%u) in"
                                        "progress.\n"),
                             mlinfo->list_id,
                             mlinfo->shutdown));
                SHR_VERBOSE_EXIT(SHR_E_BUSY);
            ...
            if (rv != SHR_E_NONE) {
                ...
                shr_itbm_list_attr_delete(unit, params, nlinfo, 0);
                /* Free the handle. */
                SHR_FREE(*hdl);
                /* Free the new list resources. */
                shr_itbm_list_free(unit, nlinfo);
                              ...
                              shr_itbm_list_op_t *op)
{
    ...
    while (alist) {
        if (alist->aid == hdl->aid) {
            ...
            /* Check and return error if Stat value is out of range. */
            if (((int)alist->stat.in_use_count < 0
                    || alist->stat.in_use_count > alist->tot_elems)
                || ((int)alist->stat.free_count < 0
                    || alist->stat.free_count > alist->tot_elems)) {
                SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                                    (BSL_META_U(unit,
                                                "List(AID=%u): in_use_count=%d "
                                                "or free_count=%d is invalid "
                                                "list tot_elems=%u - rv=%s.\n"),
                                     alist->aid,
                                     (int)alist->stat.in_use_count,
                                     (int)alist->stat.free_count,
                                     alist->tot_elems,
    ...
    for (i = 0; i < op->num_elem; i++) {
        if (alloc) {
            if (elem_arr[(f + i)].eref_cnt == 0) {
                /* Update number of elements/members per physical entry. */
                elem_arr[(f + i)].max_entries = op->linfo->max_ent_per_bkt;
                /*
    ...
    for (eb = l; eb >= f && eb <= l; eb--) {
        ...
        if (BSL_LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
            shr_pb_printf(pb,
                          ...
                          op->params.reverse ? "T" : "F" , rbkt_cnt, epbkt,
                          (int)eb, ebl, eb_off, num_elem);
        }
                                  ...
                                  shr_itbm_list_op_t *op)
{
    ...
    for (bb = f; bb <= l; bb++) {
        ...
        if (BSL_LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
            shr_pb_printf(pb,
                          "Reqbkts-(rev=%s): rbkt_cnt=%u - (epbkt=%u bb=0x%x "
                          "bbf=%u ne=%u).\n",
                          op->params.reverse ? "T" : "F", rbkt_cnt, epbkt, bb,
                          bbf, num_elem);
        }
                             ...
                             shr_itbm_list_op_t *op)
{
    ...
    if (BSL_LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
        LOG_DEBUG(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "[loper=0x%x]: (U=%-2u T=%-5u comp_id=%-2u "
                              "f=%-5u l=%-5u zoff=%-5u epbkt=%-2u bb=%-5u "
...
static int
shr_itbm_list_my_inuse_blks_free(int unit,
                                 shr_itbm_list_hdl_t hdl)
{
    uint32_t f, l; /* First and last element numbers of the list. */
    uint32_t b = 0, e = 0; /* Bucket and element index iterator variable. */
    uint32_t zoff = 0;     /* Zero based index offset for edata array. */
    int rv = SHR_E_INTERNAL; /* Return value. */
    shr_itbm_list_edata_t *edata; /* Pointer to list elements array. */
    shr_itbm_list_op_t op; /* List operation structure instance. */
    ...
    if (hdl->params.buckets) {
        for (b = f; b <= l; b++) {
            /*
             * Check and free in use bucket elements that have been allocated
             * using the current list handle, comp_id and entries_per_bucket
             * must match.
             */
            if (shr_itbm_list_elem_ref_cnt_get(edata, b)) {
                for (e = 0; e < edata[b].max_entries; e++) {
                    if (edata[b].eprop != NULL
                        && SHR_BITGET(edata[b].eprop, e)
                        && edata[b].comp_id[e] == hdl->params.comp_id
                        && edata[b].max_entries
                                == hdl->params.entries_per_bucket) {
                        /* List elements free operation. */
                        shr_itbm_list_op_t_init(&op);
                        op.linfo = hdl->linfo;
                        SHR_BITSET(op.loper, SHR_ITBM_LIST_OPER_FREE);
                        op.params = hdl->params;
        ...
        for (b = f; b <= l;) {
            /*
             * Check and free in use elements allocated from the list using the
             * current list handle.
             */
            if ((shr_itbm_list_elem_ref_cnt_get(edata, b)
                 && hdl->params.block_stat && edata[b].eprop != NULL
                 && SHR_BITGET(edata[b].eprop, SHR_ITBM_LIST_BLK_FIRST_ELEM)
                 && edata[b].comp_id[SHR_ITBM_LIST_BKT_NONE]
                        == hdl->params.comp_id)
                || (shr_itbm_list_elem_ref_cnt_get(edata, b)
                    && !hdl->params.block_stat
                    && edata[b].comp_id[SHR_ITBM_LIST_BKT_NONE]
                            == hdl->params.comp_id)) {
                /* List elements free operation. */
                ...
                op.base_idx = (b + zoff);
                /*
                 * If the list is a block_stat enable list, blk_sz elements
                 * count must be freed. For non-block_stat lists only one
                 * element must be freed, which has been initialized in
                 * max_ent_per_bkt list param variable.
                 */
                op.num_elem = hdl->params.block_stat
                                ? edata[b].blk_sz[SHR_ITBM_LIST_BKT_NONE]
                                    : hdl->linfo->max_ent_per_bkt;
                op.offset = zoff;
                                 ...
                                 shr_itbm_defrag_blk_mseq_t **dfg_bmseq)
{
    ...
    if (BSL_LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        ...
        shr_pb_printf(mvpb, "gap@start   : (epbkt=%-1u b=%-5u f=%-5u ecnt=%-5u "
                            ...
                            gap.ecount,
                            gap.comp_id);
        shr_pb_printf(mvpb, "dblks_moved@start : (%-5d).\n",
                            linfo->dblks_moved);
        shr_pb_printf(mvpb, "-------------------------------------------\n");
    }
    ...
    if (BSL_LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        ...
        shr_pb_printf(mvpb, "gap@end         : (b=%-5d f=%-5u ecnt=%-5u c=%-2d "
                            ...
                            gap.comp_id,
                            gap.entries_per_bucket);
        shr_pb_printf(mvpb, "dblks_moved@end : (%-5d).\n", linfo->dblks_moved);
        shr_pb_printf(mvpb, "-------------------------------------------\n");
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "%s"), shr_pb_str(mvpb)));
        shr_pb_destroy(mvpb);
                            ...
                            shr_itbm_blk_t *largest)
{
    ...
    if (fblks_cnt && dfgl_fblk_arr) {
        ...
        if (BSL_LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
                                    "No free blocks (fblks_cnt=%u reverse=%u "
                                    " aid=%u) - rv=%s\n"),
                         fblks_cnt, hdl->params.reverse, hdl->aid,
                         shr_errmsg(SHR_E_NOT_FOUND)));
        }
                     ...
                     shr_itbm_list_hdl_t *hdl)
{
    ...
    *hdl = NULL;
    ...
    if (SHR_FAILURE(rv)) {
        ...
        (void)shr_itbm_list_delete(unit, *hdl, 0);
        SHR_ERR_EXIT(rv);
    }
                      ...
                      bool shutdown)
{
    ...
    /* Check and return error if both optional params are set. */
    if (free_inuse && shutdown) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "Both free_inuse(%u) and shutdown(%u) "
                                        "options cannot be set - rv=%s.\n"),
                             free_inuse,
                             shutdown,
                             shr_errmsg(SHR_E_PARAM)));
    }
    ...
    /*
     * Check and return error if shutdown option is not set in the list
     * destroy call for shared lists that have already been shutdown.
     */
    if (hdl->linfo->shutdown && shutdown == 0) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "Requires shutdown(=%u) to be set for "
                                        "destroying this shared list LID=%u "
                                        "- rv=%s.\n"),
                             hdl->linfo->shutdown,
                             hdl->linfo->list_id,
                             shr_errmsg(SHR_E_PARAM)));
    ...
    SHR_NULL_CHECK(lattr, SHR_E_INTERNAL);
    /*
     * Check and return E_BUSY error when the list is being destroyed when
     * elements are in use and the optional free_inuse and shutdown
     * parameters have not been set.
     */
    if (!free_inuse && !shutdown && lattr->num_allocs) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_BUSY,
                            (BSL_META_U(unit,
                                        "LID=%u: elements inuse_ecount=%u "
                                        "free_count=%u num_allocs=%u - "
                                        "rv=%s.\n"),
                             hdl->linfo->list_id,
                             lattr->stat.in_use_count,
    ...
    /* Check and set shutdown in-progress flag for shared lists. */
    if (hdl->linfo->lref_cnt > 1 && shutdown && hdl->linfo->shutdown == 0 ) {
        hdl->linfo->shutdown = TRUE;
    }

    if (BSL_LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "LID=%u (free_inuse=%u shutdown=%u): "
                                "lref_cnt=%u lshutdown=%u.\n"),
                     list_id, free_inuse, shutdown, hdl->linfo->lref_cnt,
                     hdl->linfo->shutdown));
    }

    /*
     * If blocks still in use and free_inuse option has been set, then check
     * and free in use blocks back to the list.
     */
    if (free_inuse && lattr->num_allocs) {
        if (BSL_LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
            LOG_DEBUG(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
                                  "list_stat(free_inuse=%u, num_allocs=%u): "
                                  "in_use_count=%u free_count=%u current.\n"),
                       free_inuse,
                       lattr->num_allocs,
                       lattr->stat.in_use_count,
                       lattr->stat.free_count));
        }
        /* Free in use list blocks. */
        rv = shr_itbm_list_my_inuse_blks_free(unit, hdl);
        if (SHR_FAILURE(rv)) {
            sal_mutex_give(ctrl[unit]->glock);
            SHR_ERR_EXIT(rv);
        }
        if (BSL_LOG_CHECK_DEBUG(BSL_LOG_MODULE)) {
            LOG_DEBUG(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
                                  "list_stat-post_free_oper: (free_inuse=%u, "
                                  "num_allocs=%u): in_use_count=%u "
                                  "free_count=%u.\n"),
                       free_inuse,
                       lattr->num_allocs,
                       lattr->stat.in_use_count,
     ...
     */
    rv = shr_itbm_list_delete(unit, hdl, shutdown);
    if (rv != SHR_E_NONE) {
        sal_mutex_give(ctrl[unit]->glock);
        SHR_IF_ERR_MSG_EXIT(rv,
                            (BSL_META_U(unit,
                          ...
                          uint32_t *first)
{
    ...
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
                             hdl->linfo->lref_cnt,
                             shr_errmsg(SHR_E_PARAM)));
    }

    /* Check if shared list shutdown state is enabled. */
    if (hdl->linfo->shutdown) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_BUSY,
                            (BSL_META_U(unit,
                                        "LID=%u shared list shutdown(=%u) "
                                        "in-progress - rv=%s.\n"),
                             hdl->linfo->list_id,
                             hdl->linfo->shutdown,
                             shr_errmsg(SHR_E_BUSY)));
                         ...
                         uint32_t first)
{
    ...
    /* Verify the list is valid before trying to free elements to the list. */
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
    ...
    if (rv != SHR_E_NONE) {
        ...
        SHR_IF_ERR_MSG_EXIT(rv,
                            (BSL_META_U(unit,
                                        "bucket=%d first=%d count=%u - rv=%s."
                                        "\n"),
                             (int)bucket, (int)first, count, shr_errmsg(rv)));
    }
                           ...
                           uint32_t new_count)
{
    ...
    /* Verify the list is valid before trying to resize the block. */
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
                             ...
                             uint32_t *comp_id)
{
    ...
    *comp_id = SHR_ITBM_INVALID;
    ...
    /* Verify the list is valid before trying to get element state. */
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
                       ...
                       shr_itbm_list_stat_t *stat)
{
    ...
    /* Verify the list is valid before trying to get list statistics. */
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
                             ...
                             shr_itbm_blk_t **free_blk)
{
    ...
    *free_blk = NULL;
    ...
    /* Verify the list is valid before trying to get list block statistics. */
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
                           ...
                           shr_itbm_blk_t *largest_free_blk)
{
    ...
    *blk_mov_seq = NULL;
    ...
    /* Verify the list is valid before trying to defragment the list. */
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
                         ...
                         bool success)
{
    ...
    /* Verify the list is valid before trying to end defragmentation. */
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
                                    ...
                                    shr_itbm_blk_t *largest_rev)
{
    ...
    /*
     * Verify the list is valid before trying to get the largest free blocks
    ...
    if (BSL_LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "shr_itbm_list_defrag_free_block_get"
                                "(LID=%u/%u)\n"),
                     hdl->linfo->list_id, hdl_rev->linfo->list_id));
    }
    /* Take the list lock. */
    ...
    rv = itbm_ibkt_list_free_blk_get(unit, hdl_rev, largest_rev);
    if (SHR_SUCCESS(rv) && BSL_LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "Done: shr_itbm_list_defrag_free_block_get"
                                "largest - fwd/rev: epbkt=%u/%u b=%d/%d f=%d/%d"
                                " c=%d/%d ecnt=%u/%u )\n"),
                     largest->entries_per_bucket,
                     largest_rev->entries_per_bucket,
                     (int)largest->bucket,
                     (int)largest_rev->bucket,
                     (int)largest->first_elem,
                     (int)largest_rev->first_elem,
                     (int)largest->comp_id,
                     (int)largest_rev->comp_id,
                     largest->ecount,
                     largest_rev->ecount));
    }
    SHR_VERBOSE_EXIT(rv);
                     ...
                     uint32_t new_last)
{
    ...
    /* Verify the list is valid before trying to resize the list. */
    if (!hdl->linfo->llock || !hdl->linfo->lref_cnt) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_PARAM,
                            (BSL_META_U(unit,
                                        "List invalid: lref_cnt=%u - rv=%s.\n"),
    ...
    SHR_NULL_CHECK(lattr, SHR_E_INTERNAL);
    /*
     * Verify and return E_BUSY error if elements are in use in the new lower
     * index range of the list partition.
     */
    if (hdl->params.block_stat && hdl->params.first != new_first) {
        rstart = (hdl->params.first < new_first)
                    ? hdl->params.first : new_first;
        rend = ((hdl->params.first < new_first)
                    ? (new_first - 1): (hdl->params.first - 1));
    ...
    /*
     * Verify and return E_BUSY error if elements are in use in the new upper
     * index range of the list partition.
     */
    if (hdl->params.block_stat && hdl->params.last != new_last) {
        rstart = ((hdl->params.last < new_last)
                    ? (hdl->params.last + 1) : (new_last + 1));
    ...
    if (BSL_LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "Done: [List resize LID=%u new_first=%u "
                                "new_last=%u].\n"),
