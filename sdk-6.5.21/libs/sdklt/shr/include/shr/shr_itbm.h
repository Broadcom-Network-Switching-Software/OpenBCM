 ...
 * \retval SHR_E_BUSY When the new list \c entries_per_bucket config
 *                    parameter value is greater than its shared partner
 *                    list value and the elements of the shared partner list
 *                    are already in use and the shared list maximum entries
 *                    per bucket config cannot be modified when elements are
 *                    in use. Also returned for shared list, when the
 *                    \c shutdown process has already been initiated for this
 *                    shared list.
...
/*!
 * \brief Destroy an indexed list.
 *
 * This API can be called to destroy a list that was previously created
 * using the list create \ref shr_itbm_list_create API. If the list pointed
 * by the handle \c hdl is determined as a shared indexed list, then only
 * the resources allocated for maintaining this specific list instance is
 * freed as part of this destroy call, the shared list elements are not
 * destroyed.
 *
 * By default the application must free all in use elements allocated from
 * the list, prior to destroying the list else \c SHR_E_BUSY error will be
 * returned by this API. The application can alternately call this API
 * with \c free_inuse parameter set to free all in use elements and also
 * destroy the list handle/instance as part of the destroy API call. The
 * \c free_inuse option is typically used when the partner shared list is
 * expected to exist and continue to operate after the current list is
 * destroyed. The elements from the shared list can still be allocated or
 * freed using the partner list handle after this list is destroyed.
 *
 * To skip in use resource checks and destroy the list as part of the device
 * shutdown sequence, the application can call this API with \c shutdown
 * parameter set. After a shared list is shut down, elements cannot be
 * allocated or freed from the shared list using partner list handles. All
 * the list handles of this shared list must be destroyed by calling this
 * API with \c shutdown parameter set.
 *
 * Note: Application must not call this API with both \c free_inuse and
 *       \c shutdown options set, invalid parameter error will be returned
 *       by this API.
 *
 * \param [in] hdl Handle to the list to be destroyed.
 * \param [in] free_inuse When enabled, free all in use elements back to the
 *                        list before destroying the list instance/handle.
 * \param [in] shutdown Skip list in use resource checks and shutdown the
 *                      list.
 *
 * \retval SHR_E_NONE List destroyed successfully.
 * \retval SHR_E_PARAM Invalid input parameter, also returned when both
 *                     \c free_inuse and \c shutdown options are set.
 * \retval SHR_E_BUSY When a list undergoing defragmentation is destroyed.
 *                    Also returned when the list is destroyed while the
 *                    list elements are still in use and \c free_inuse or
 *                    \c shutdown parameter is not set in the destroy API
 *                    call.
 ...
 * \retval SHR_E_BUSY List defragmentation or shutdown in-progress.
 */
extern int
 ...
 *                     disabled list.
 * \retval SHR_E_BUSY List defragmentation already started or shutdown
 *                    in-progress.
 */
extern int
 ...
 * \retval SHR_E_NONE Operation successful.
 * \retval SHR_E_NOT_FOUND The given list is not found.
 * \retval SHR_E_BUSY List shutdown in-progress.
