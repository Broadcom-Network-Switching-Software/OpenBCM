...
/*!
 * \brief Add callback functions for backward compatibility.
 *
 * This function is used to install callback functions for backward
 * compatibility in case the HA memory managers are instantiated from
 * a higher layer with compile-time knowledge about the number of
 * support units.
 *
 * \param [in] cb_unit_max Maximum number of units supported by callbacks.
 * \param [in] alloc_cb Callback for HA memory allocation.
 * \param [in] realloc_cb Callback for HA memory reallocation.
 * \param [in] free_cb Callback for freeing HA memory.
 *
 * \return Nothing.
 */
extern void shr_ha_mem_cb_set(int cb_unit_max,
                              void *(*alloc_cb)(int,
 ...
 * \param [in] ctx Caller context. This enables the caller to use the
 * write function in parallel to serve multiple units simultaneously.
 *
 * \retval SHR_E_NONE success.
 ...
 * \param [in] ctx Caller context. This enables the caller to use the
 * read function in parallel to serve multiple units simultaneously.
 *
 * \retval SHR_E_NONE success.
 * \retval SHR_E_IO The I/O function failed.
 ...
 *
 * \return Handle to the HA memory instance or NULL if error.
 */
extern void *
 ...
 */
extern void
shr_ha_mm_destroy(void *private);
 ...
 * \param [in] ctx Caller context. This opaque parameter will be passed to the
 * \c write_f function.
 *
 * \retval SHR_E_NONE No errors.
