/*! \file bcm56780_a0_fp_em_ext_db_quad.c
 *
                                   ...
                                   bcmfp_stage_t *stage)
{
    ...
    stage->kgn_ext_cfg_db_arr[BCMFP_KEYGEN_MODE_QUAD] = ext_cfg_db;
    ...
    ext_num = 4;
    for (idx = 0; idx < 6; idx++) {
        offset_one = 164 + (gran * idx);
        BCMFP_KEYGEN_EXT_CFG_INIT(unit);
        if (idx == 5) {
            _ext_cfg_.valid_bitmap = 0x7FFFFF;
        }
        BCMFP_KEYGEN_EXT_PLGE_ADD(unit, part, level, gran, ext_num + idx);
        BCMFP_KEYGEN_EXT_SECTION_ADD
        ...
        BCMFP_KEYGEN_EXT_SECTION_ADD
            (unit, 0, 0, BCMFP_KEYGEN_EXT_SECTION_FK, offset_one);
        if (idx == 5) {
            SHR_BITSET(ext_attrs.w, BCMFP_KEYGEN_EXT_ATTR_PARTIAL_GRAN);
        }
        BCMFP_KEYGEN_EXT_CFG_INSERT(unit, ext_cfg_db, &ext_attrs);
        SHR_BITCLR(ext_attrs.w, BCMFP_KEYGEN_EXT_ATTR_PARTIAL_GRAN);
    }

    ext_num = 0;
    for (idx = 0; idx < 2; idx++) {
        offset_one = 80 + (gran * idx);
        offset_two = 104 + (gran * idx);
        BCMFP_KEYGEN_EXT_CFG_INIT(unit);
        BCMFP_KEYGEN_EXT_PLGE_ADD(unit, part, level, gran, ext_num + idx);
        BCMFP_KEYGEN_EXT_SECTION_ADD
            (unit, 1, 0, BCMFP_KEYGEN_EXT_SECTION_L1E32, 0);
