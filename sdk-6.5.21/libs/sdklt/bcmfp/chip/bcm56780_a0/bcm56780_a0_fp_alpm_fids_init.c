/*! \file bcm56780_a0_fp_alpm_fids_init.c
 *
...
int
bcm56780_a0_fp_alpm_fids_init(int unit,
                              bcmfp_stage_t *stage,
                              bcmptm_cth_alpm_control_t *alpm_ctrl)
{
    ...
    if (stage->compression_key_type[0] == BCMFP_COMP_KEY_IP) {
        ...
        compress_alpm_tbl->mask_fids = mask_fids;
        compress_alpm_tbl->num_mask_fids = num_mask_fids;
        size = (num_mask_fids * sizeof(bool));
        BCMFP_ALLOC(maskable, size, "bcmfpL3AlpmIPv4SrcTableMaskableFids");
        compress_alpm_tbl->maskable = maskable;
        SHR_IF_ERR_EXIT
            (bcmlrd_field_name_to_idx(unit, ltid, "IPV4", &field));
        key_fids[0] = field;
        SHR_IF_ERR_EXIT
    ...
    if (stage->compression_key_type[1] == BCMFP_COMP_KEY_IP) {
        ...
        compress_alpm_tbl->num_mask_fids = num_mask_fids;
        size = (num_mask_fids * sizeof(bool));
        BCMFP_ALLOC(maskable, size, "bcmfpL3AlpmIPv4DstTableMaskableFids");
        compress_alpm_tbl->maskable = maskable;
        SHR_IF_ERR_EXIT
            (bcmlrd_field_name_to_idx(unit, ltid, "IPV4", &field));
        key_fids[0] = field;
    ...
    if (stage->compression_key_type[2] == BCMFP_COMP_KEY_IP) {
        ...
        compress_alpm_tbl->num_mask_fids = num_mask_fids;
        size = (num_mask_fids * sizeof(bool));
        BCMFP_ALLOC(maskable, size, "bcmfpL3AlpmIPv6SrcTableMaskableFids");
        compress_alpm_tbl->maskable = maskable;
        SHR_IF_ERR_EXIT
            (bcmlrd_field_name_to_idx(unit, ltid, "IPV6_LOWER", &field));
        key_fids[0] = field;
    ...
    if (stage->compression_key_type[3] == BCMFP_COMP_KEY_IP) {
        ...
        compress_alpm_tbl->num_mask_fids = num_mask_fids;
        size = (num_mask_fids * sizeof(bool));
        BCMFP_ALLOC(maskable, size, "bcmfpL3AlpmIPv6DstTableMaskableFids");
        compress_alpm_tbl->maskable = maskable;
        SHR_IF_ERR_EXIT
            (bcmlrd_field_name_to_idx(unit, ltid, "IPV6_LOWER", &field));
        key_fids[0] = field;
