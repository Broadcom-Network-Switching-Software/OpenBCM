                               ...
                               bcmfp_stage_t *stage)
{
    ...
    uint8_t gran = 0;
    bool section_16_done = FALSE;
    bool section_32_done = FALSE;
    bool section_4_done = FALSE;
    uint16_t idx = 0, idx1 = 0;
    uint16_t section_16_inst = 0;
    uint16_t section_32_inst = 0;
    bool *section_32_tracker = 0;
    size_t size = 0;
    ...
    for (qual_idx = 0; qual_idx < pcm_info->field_count; qual_idx++) {
        ...
        for (inst_idx = 0; inst_idx < qual_cont_info->instances; inst_idx++) {
            ...
            cont_info = &(qual_cont_info->info[index]);
            SHR_IF_ERR_VERBOSE_EXIT
            ...
            if (gran == 16) {
                ...
                section_width = 16;
                if (section_16_done == TRUE) {
                    continue;
                } else {
                    section_16_inst = inst_idx;
                    section_16_done = TRUE;
                }
            } else if (gran == 4) {
                section_id = BCMFP_KEYGEN_EXT_SECTION_L1E4;
                section_width = 4;
                if (section_4_done == TRUE) {
                    continue;
                } else {
                    section_4_done = TRUE;
                }
                ...
                section_width = 32;
                if (section_32_done == TRUE) {
                    continue;
                } else {
                    section_32_inst = inst_idx;
                    section_32_done = TRUE;
                }
            } else {
        ...
        /*
         * When a qualifier is part of both 32bit and 16 bit section,
         * genreate qualifiers configuration with chunks coming from
         * both 32 and 16 bit sections. This provides more options to
         * try in group create and usefull when the QSET is dense.
         * Note: Tooling generates qualifier configs with all chunks
         * coming from same section(either 32bit, 16bit, or 8bit).
         */
        if (!(array_depth == 1 &&
            section_16_done == TRUE &&
            section_32_done == TRUE &&
            qual_cont_info->count != 1)) {
            section_16_done = FALSE;
            section_32_done = FALSE;
            section_4_done = FALSE;
            continue;
        }

        SHR_FREE(section_32_tracker);
        size = sizeof(bool) * qual_cont_info->count;
        BCMFP_ALLOC(section_32_tracker, size, "bcmfpSection32Tracker");

        for (idx = 0; idx < qual_cont_info->count; idx++) {
            section_32_tracker[idx] = TRUE;
        }
        for (idx = 0; idx < qual_cont_info->count-1; idx++) {
            section_32_tracker[qual_cont_info->count - 1 - idx] = FALSE;
            chunk_idx = 0;
            BCMFP_KEYGEN_QUAL_CFG_INIT(unit);
            for (idx1 = 0; idx1 < qual_cont_info->count; idx1++) {
                if (section_32_tracker[idx1] == TRUE) {
                    inst_idx = section_32_inst;
                    section_id = BCMFP_KEYGEN_EXT_SECTION_L1E32;
                    section_width = 32;
                } else {
                    inst_idx = section_16_inst;
                    section_id = BCMFP_KEYGEN_EXT_SECTION_L1E16;
                    section_width = 16;
                }

                cont_idx = (inst_idx * qual_cont_info->count) + idx1;
