                     ...
                     uint32_t **edw)
{
    ...
    /* Perform device-specific software setup */
    SHR_IF_ERR_VERBOSE_EXIT
        (fp_presel_edw_get[dev_type].presel_edw_get(unit,
                                                    stage_id,
                                                    group_id,
                                                    cqset_id,
                                                    opinfo,
...
typedef int (*bcmfp_alpm_fids_init_f)(int unit,
                                      bcmfp_stage_t *stage,
                                      bcmptm_cth_alpm_control_t *alpm_ctrl);

#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
    { _bc##_fp_alpm_fids_init },
static struct {
    bcmfp_alpm_fids_init_f alpm_fids_init;
} fp_alpm_fids_init[] = {
    { 0 }, /* dummy entry for type "none" */
#include <bcmdrd/bcmdrd_devlist.h>
    { 0 } /* end-of-list */
                     ...
                     bcmptm_cth_alpm_control_t *alpm_ctrl)
{
    ...
    /* Perform device-specific software setup */
    SHR_IF_ERR_VERBOSE_EXIT
        (fp_alpm_fids_init[dev_type].alpm_fids_init(unit,
                                                    stage,
                                                    alpm_ctrl));
exit:
   SHR_FUNC_EXIT();
