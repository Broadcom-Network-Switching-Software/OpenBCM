/*! \file bcm56880_a0_fp_ing_presel_edw.c
 *
                                  ...
                                  uint32_t **edw)
{
    ...
    for (part = 0; part < num_parts; part++) {
        if (cqset_id == 0) {
            ...
            ext_codes->pdd_prof_index =
                       opinfo->ext_codes[part].pdd_prof_index;
            ext_codes->sbr_prof_index =
                       opinfo->ext_codes[part].sbr_prof_index;
        }
        ...
        /* Set the valid bit */
        fv = opinfo->enable;
        bcmdrd_field_set(ebuff, 437, 437, &fv);

        /* Copy 437 bits from ebuff to edw[part][0] */
        bcmdrd_field_get(ebuff, 0, 437, &(edw[part][0]));
    }
exit:
    SHR_FUNC_EXIT();
}
