/*! \file bcm56990_b0_fp_ing_presel_edw.c
 *
 * IFP action configuration DB initialization
 * function for Tomahawk-4 B0(56990_B0).
 */
/*
                                  ...
                                  uint32_t **edw)
{
    ...
    bcmfp_ext_codes_t *conflict_ext_codes = NULL;
    ...
    if (cqset_id != 0) {
        SHR_IF_ERR_EXIT
            (bcmfp_conflict_qset_ext_codes_get(unit,
                                               stage_id,
                                               group_id,
                                               cqset_id,
                                               &conflict_ext_codes));
    ...
    for (part = 0; part < num_parts; part++) {
        if (cqset_id == 0) {
            ...
            ext_codes = &conflict_ext_codes[part];
        }
        ...
        if (ext_codes->class_id_cont_c_sel != BCMFP_EXT_SELCODE_DONT_CARE) {
            ...
            bcmdrd_field_set(ebuff, 17, 20, &fv);
        }
        ...
        if (ext_codes->ltid != BCMFP_EXT_SELCODE_DONT_CARE) {
            fv = ext_codes->ltid;
            bcmdrd_field_set(ebuff, 44, 47, &fv);
        } else {
            SHR_ERR_EXIT(SHR_E_INTERNAL);
        }
        ...
        bcmdrd_field_set(ebuff, 48, 50, &fv);
