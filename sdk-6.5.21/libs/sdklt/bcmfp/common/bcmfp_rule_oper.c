/*! \file bcmfp_rule_oper.c
 *
 * BCMFP rule operational information get/set APIs.
 */
/*
 ...
 *  holding the mapping between rule template id and entry id(s)
 *  associated to it.
 *
 * \param [in] unit Logical device id
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [out] rule_entry_map Pointer to memory segment.
 ...
 */
STATIC int
bcmfp_rule_entry_map_get(int unit,
                         ...
                         uint32_t *map_size)
{
    ...
    SHR_NULL_CHECK(rule_entry_map, SHR_E_PARAM);
    ...
    stage_oper_info = (bcmfp_stage_oper_info_t *)
                          ((uint8_t *)ha_mem + size);
    offset = stage_oper_info->rule_entry_map_seg;
    *rule_entry_map = (void *)((uint8_t *)ha_mem + offset);
    *map_size = stage_oper_info->rule_entry_map_size;
exit:
    SHR_FUNC_EXIT();
}
                              ...
                              void *user_data)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_rule_entry_map_get(unit,
                                  stage_id,
                                  &rule_entry_map,
                                  &map_size));
    map_type = BCMFP_IDP_MAP_TYPE_RULE_TO_ENTRY;
    SHR_IF_ERR_VERBOSE_EXIT
