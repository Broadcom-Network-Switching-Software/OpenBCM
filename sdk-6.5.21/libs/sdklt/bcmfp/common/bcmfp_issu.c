                             ...
                             bcmfp_ha_struct_offset_t *issu_offset)
{
    ...
    dst_ptr = dst_ptr + size;
    /*
     * num_action_prof is updated wrongly in cold boot. So taking the
     * num_action_prof value from the stage info - issu_offset.
     * This can be changed like other profiles in future.
     */
    size = (stage_oper_info->num_keygen_prof * pipe_count) +
           (issu_offset->num_action_prof * pipe_count) +
           ...
           (issu_offset->num_psg_profiles * pipe_count);
    ...
    /*
     * Leave the gap for conflict QSET ID to group ID mapping
     * if it was not present before otherwise copy. Otherwise
     * copy from source to destintaion.
     */
    if (stage_oper_info->conflict_qset_group_map_size) {
        dst_ptr = dst_ptr + size;
        src_ptr = src_ptr + size;
        size = stage_oper_info->conflict_qset_group_map_size;
        size = size * sizeof(bcmfp_ha_blk_info_t);
        sal_memcpy(dst_ptr, src_ptr, size);
        dst_ptr = dst_ptr + size;
        src_ptr = src_ptr + size;
        ...
        dst_ptr = dst_ptr + size;
        src_ptr = src_ptr + size;
        size = issu_offset->conflict_qset_group_map_size;
        size = size * sizeof(bcmfp_ha_blk_info_t);
        dst_ptr = dst_ptr + size;
    }

    /* Copy group operational information. */
    if (issu_offset->num_groups > stage_oper_info->num_groups)  {
        size = stage_oper_info->num_groups;
    } else {
        size = issu_offset->num_groups;
    }
    LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit,
             "Stage_ID=%d Old group count =%d New group count %d\n"),
             stage_id,
             stage_oper_info->num_groups,
             issu_offset->num_groups));
    size = size * sizeof(bcmfp_group_oper_info_t);
    sal_memcpy(dst_ptr, src_ptr, size);

    /* Copy presel group operational information. */
    /* Number of groups in current version. */
    size = issu_offset->num_groups;
    size = size * sizeof(bcmfp_group_oper_info_t);
    dst_ptr = dst_ptr + size;
    /* Number of groups in previous version. */
    size = stage_oper_info->num_groups;
    size = size * sizeof(bcmfp_group_oper_info_t);
    src_ptr = src_ptr + size;
    ...
    /* Copy pdd operational information. */
    src_ptr = src_ptr + size;
    dst_ptr = dst_ptr + size;
