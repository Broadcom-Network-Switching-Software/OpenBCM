                                        ...
                                        bcmfp_ha_struct_offset_t *issu_offset)
{
    ...
    if (BCMFP_STAGE_FLAGS_TEST(stage, BCMFP_STAGE_CONFLICT_QSETS_SUPPORT)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_stage_max_conflict_qset_id_get(unit,
                                                  stage_id,
                                                  &max_cqset_id));
        if (max_cqset_id > div_factor) {
            max_cqset_id = (max_cqset_id / div_factor);
        }
        if (max_cqset_id != 0)  {
            if (issu_offset->ha_blk_info_offset == 0) {
                issu_offset->ha_blk_info_offset= *size;
            }
            issu_offset->ha_blk_info_array_size += (max_cqset_id + 1);
            issu_offset->conflict_qset_group_map_size = (max_cqset_id + 1);
            issu_offset->conflict_qset_group_map_offset = *size;
            *size += (sizeof(bcmfp_ha_blk_info_t) * (max_cqset_id + 1));
        }
    }
exit:
    SHR_FUNC_EXIT();
