                     ...
                     uint16_t *fidx)
{
    ...
    for (idx = 0;
        ...
        if (actions_fid_info[idx].array_actions_count) {
            for (idx1 = 0;
                idx1 < actions_fid_info[idx].array_actions_count;
                idx1++) {
                if (actions_fid_info[idx].array_actions[idx1] == action) {
                    *fid = idx;
                    *fidx = idx1;
                    SHR_EXIT();
                }
                        ...
                        bcmfp_action_oper_info_t *a_offset)
{
    ...
    if (fid_offset_info_found == FALSE) {
        ...
        fid_offset_info->fidx = fidx;
        fid_offset_info->offset_info.blk_id =
                         action_offset_blk_id;
        fid_offset_info->offset_info.blk_offset =
                         action_offset_blk_offset;
                           ...
                           bcmfp_action_oper_info_t *a_offset)
{
    ...
    while (blk_id != BCMFP_HA_BLK_ID_INVALID &&
        ...
        if (fid == fid_offset_info->fid &&
            fidx == fid_offset_info->fidx) {
            hit_status = TRUE;
            LOG_VERBOSE(BSL_LOG_MODULE,(BSL_META_U(unit,
...
int
bcmfp_pdd_group_action_offset_info_get(int unit,
                                       bcmfp_stage_id_t stage_id,
                                       bcmfp_pdd_id_t pdd_id,
                                       bcmfp_part_id_t part_id,
                                       bcmfp_group_id_t group_id,
                                       bcmfp_action_t action_id,
                                       bcmfp_pdd_oper_type_t oper_type,
                                       bcmfp_action_oper_info_t *a_offset)
{
    bcmfp_stage_t *stage = NULL;
    bcmfp_action_oper_info_t pdd_act_offset;
    bcmfp_group_oper_info_t *opinfo = NULL;
    uint32_t policy_width = 0;
    uint32_t data_start_bit = 0;
    uint32_t data_end_bit = 0;
    uint8_t num_offsets = 0;
    uint8_t idx = 0;
    uint16_t offset = 0;
    uint32_t base_entry_data_size = 0;
    bcmfp_group_mode_t group_mode = BCMFP_GROUP_MODE_NONE;
    int8_t base_entry = 0;
    ...
    /* get the pdd action offsets */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_pdd_action_offset_info_get(unit,
                                          stage->stage_id,
                                          pdd_id,
                                          part_id,
                                          action_id,
    ...
    /*
     * for non-EM stages the action_offsets
     * are independent of group mode
     */
    if (!(BCMFP_STAGE_FLAGS_TEST(stage,
                                   BCMFP_STAGE_HASH_FLEX_KEY_DATA))) {
        sal_memcpy(a_offset,&pdd_act_offset,
                   sizeof(bcmfp_action_oper_info_t));
        SHR_EXIT();
    }

    /*
     * Extract the offsets for the given action based
     * on the group mode.
     */
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_pdd_policy_width_get(unit,
                                    ...
                                    oper_type,
                                    &policy_width));
    num_offsets = pdd_act_offset.num_offsets;
    a_offset->num_offsets = num_offsets;

    for (idx = 0; idx < num_offsets; idx++) {
        /* get the action's offset from MSB in policy_data */
        offset = policy_width - pdd_act_offset.offset[idx];
        for (base_entry = BCMFP_EM_MAX_BASE_ENTRIES -1;
                base_entry >= 0; base_entry--) {
            data_start_bit =
              stage->key_data_size_info->data_start_bit[group_mode][base_entry];
            data_end_bit =
                stage->key_data_size_info->data_end_bit[group_mode][base_entry];

            base_entry_data_size = data_end_bit - data_start_bit + 1;
            ...
            if (base_entry_data_size > 0) {
                if (base_entry_data_size >= offset) {
                    /* action is inserted in this base entry */
                    a_offset->offset[idx] = data_end_bit - offset + 1;
                    a_offset->width[idx] = pdd_act_offset.width[idx];
                    break;

                } else {
                    /*
                     * action is inserted in next base entries.
                     * obtain the action's offset from the
                     * MSB of next base_entry
                     */
                    offset = offset - base_entry_data_size;
                }
            }
        }
    }
...
int
bcmfp_pdd_group_map_cond_traverse(int unit,
                                  bcmfp_stage_id_t stage_id,
                                  uint32_t pdd_id,
