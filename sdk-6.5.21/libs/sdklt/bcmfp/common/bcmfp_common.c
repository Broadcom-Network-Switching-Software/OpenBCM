...
int bcmfp_stage_misc_info_cleanup(int unit, bcmfp_stage_t *stage)
{
    ...
    SHR_FREE(stage->compress_md.entry_id_bmp);
    SHR_FREE(stage->compress_md.group_id_bmp);
    SHR_FREE(stage->compress_md.entry_count_in_group);
...
int
bcmfp_stage_max_entry_id_get(int unit,
                             bcmfp_stage_id_t stage_id,
                             uint32_t *max_entry_id)
{
    ...
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(max_entry_id, SHR_E_PARAM);
    ...
    *max_entry_id = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_fid_max_value_get(unit, ltid, fid, max_entry_id));
exit:
    SHR_FUNC_EXIT();
}
...
int
bcmfp_stage_max_conflict_qset_id_get(int unit,
                                     bcmfp_stage_id_t stage_id,
                                     uint32_t *max_conflict_qset_id)
{
    bcmlrd_sid_t ltid = BCMLTD_SID_INVALID;
    bcmlrd_fid_t fid = 0;
    bcmfp_stage_t *stage = NULL;
    ...
    SHR_NULL_CHECK(max_conflict_qset_id, SHR_E_PARAM);

    *max_conflict_qset_id = 0;
    SHR_IF_ERR_EXIT(bcmfp_stage_get(unit, stage_id, &stage));

    if (stage->tbls.conflict_qset_tbl != NULL) {
        ltid = stage->tbls.conflict_qset_tbl->sid;
        fid = stage->tbls.conflict_qset_tbl->key_fid;
    }

    SHR_IF_ERR_EXIT
        (bcmfp_fid_max_value_get(unit,
                                 ltid,
                                 fid,
                                 max_conflict_qset_id));
    *max_conflict_qset_id = 1024;
exit:
    SHR_FUNC_EXIT();
}
...
bcmfp_misc_init(int unit)
{
    ...
    for (stage_id = 0; stage_id < BCMFP_STAGE_ID_COUNT; stage_id++) {
        ...
        stage->compress_md.entry_id_bmp_size = ((max_entries + 31) / 32);
        size = (((max_entries + 31) / 32) * sizeof(uint32_t));
        BCMFP_ALLOC(stage->compress_md.entry_id_bmp,
                    size,
                    "bcmfpStageCompressDbEntryIdBmp");

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_stage_max_group_id_get(unit, stage_id, &max_groups));
        stage->compress_md.group_id_bmp_size = ((max_groups + 31) / 32);
        size = (((max_groups + 31) / 32) * sizeof(uint32_t));
        BCMFP_ALLOC(stage->compress_md.group_id_bmp,
                    size,
                    "bcmfpStageCompressDbGroupIdBmp");

        size = (max_groups * sizeof(uint32_t));
        stage->compress_md.entry_count_in_group_size = max_groups;
        BCMFP_ALLOC(stage->compress_md.entry_count_in_group,
                    size,
                    "bcmfpStageCompressDbEntryCountInGroup");
...
bcmfp_fid_maps_adjust(int unit)
{
    ...
    bcmfp_tbl_qualifier_fid_info_t *grpaction_qualifiers_fid_info = NULL;
    ...
    uint32_t grpaction_total_array_fids = 0;
    uint16_t idx = 0;
    ...
    for (stage_id = 0; stage_id < BCMFP_STAGE_ID_COUNT; stage_id++) {
        ...
        for (fid = 0; fid < fid_count; fid++) {
            ...
            if ((fid == stage->tbls.group_tbl->qual_inports_fid) ||
                ...
                (fid == stage->tbls.group_tbl->qual_svp_ports_fid) ||
        ...
        for (fid = 0; fid < fid_count; fid++) {
            ...
            if ((fid == stage->tbls.rule_tbl->inports_key_fid) ||
                ...
                (fid == stage->tbls.rule_tbl->svp_ports_key_fid) ||
                (fid == stage->tbls.rule_tbl->svp_ports_mask_fid) ||
        ...
        if (stage->tbls.group_template_action_tbl != NULL) {
            sid = stage->tbls.group_template_action_tbl->sid;
            fid_count = stage->tbls.group_template_action_tbl->fid_count;
            grpaction_qualifiers_fid_info =
                stage->tbls.group_template_action_tbl->qualifiers_fid_info;
            grpaction_total_array_fids = 0;
            tbl = bcmltd_table_get(sid);
            if (tbl == NULL) {
                SHR_IF_ERR_EXIT(SHR_E_INTERNAL);
            }
            ...
            for (fid = 0; fid < fid_count; fid++) {
                ...
                /* Non array qualifier FIDs. */
                if ((fid ==
                     stage->tbls.group_template_action_tbl->qual_inports_fid) ||
                    (fid ==
                     stage->tbls.group_template_action_tbl->qual_system_ports_fid) ||
                    (fid ==
                     stage->tbls.group_template_action_tbl->qual_svp_ports_fid) ||
                    (fid ==
                     stage->tbls.group_template_action_tbl->qual_device_ports_fid)) {
                    continue;
                }
                if (field_def.depth == 0) {
                    continue;
                ...
                /* Non qualifier FIDs. */
                if (grpaction_qualifiers_fid_info[fid].qualifier == 0) {
                    continue;
                }
                for (idx = 0; idx < gtotal_array_fids; idx++) {
                    if (grpaction_qualifiers_fid_info[fid].qualifier ==
                        garray_fids[idx]) {
                        break;
                    }
                }
                ...
                grpaction_qualifiers_fid_info[fid].fid = garray_fids[idx];
                grpaction_qualifiers_fid_info[fid].qualifier =
                    gqualifiers_fid_info[garray_fids[idx]].qualifier;
                grpaction_total_array_fids++;
                if (grpaction_total_array_fids  == gtotal_array_fids) {
                    break;
                }
            }
        }
...
int
bcmfp_ltd_out_buffers_init(int unit,
                           bcmltd_fields_t *out)
{
    uint32_t idx = 0;
    bcmltd_field_t *temp = NULL;
    ...
    size = out->count * sizeof(bcmltd_field_t *);
    BCMFP_ALLOC(out->field,size,"bcmfptempDataFieldsArrayPtr");

    size = out->count * sizeof(bcmltd_field_t);

    if (size != 0) {
        BCMFP_ALLOC(temp,size,"bcmfptempDataFieldsArray");
        out->field[0] = temp;
        for (idx = 1; idx < out->count; idx++) {
            out->field[idx] = &(temp[idx]);
        }
        temp = NULL;
    }
exit:
...
int
bcmfp_ltd_out_buffers_cleanup(int unit,
                             bcmltd_fields_t *out) {
    SHR_FUNC_ENTER(unit);

    if (out->field != NULL) {
        if (out->field[0] != NULL) {
            SHR_FREE(out->field[0]);
        }
        SHR_FREE(out->field);
    }
    SHR_EXIT();
                                     ...
                                     bcmfp_stage_t *stage)
{
    ...
    for (idx1 = 0; idx1 < BCMFP_COMPRESS_TYPES_MAX; idx1++) {
        ...
        SHR_FREE(compress_qualifier_info->maskable);
                                  ...
                                  bcmfp_stage_t *stage)
{
    ...
    for (idx1 = 0; idx1 < BCMFP_COMPRESS_TYPES_MAX; idx1++) {
        ...
        size = (sizeof(bool) * num_remote_map_fids);
        BCMFP_ALLOC(compress_qualifier_info->maskable,
                    size,
                    "bcmfpCompressQualifierMaskableFids");
        ...
        for (idx2 = 0; idx2 < compress_alpm_tbl->num_key_fids; idx2++) {
            ...
            for (idx4 = 0; idx4 < fid_remote_maps->num_remote_map; idx4++) {
                ...
                if (compress_alpm_tbl->maskable != NULL) {
                    if (compress_alpm_tbl->maskable[idx2] == TRUE) {
                        compress_qualifier_info->maskable[num_remote_map_fids]
                                            = TRUE;
                    }
                }
                num_remote_map_fids++;
                         ...
                         bcmfp_stage_t *stage)
{
    ...
    for (idx = 0; idx < BCMFP_COMPRESS_TYPES_MAX; idx++) {
        ...
        SHR_FREE(compress_alpm_tbl->maskable);
                    ...
                    bcmptm_cth_alpm_control_t *alpm_ctrl)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_alpm_fids_init(unit, stage, alpm_ctrl));
