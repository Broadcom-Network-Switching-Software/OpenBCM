                                      ...
                                      uint32_t *map_size)
{
    ...
    /* Operational info might not be created yet. */
    if (blk_id == BCMFP_HA_BLK_ID_INVALID) {
        *child_entry_map = NULL;
        *map_size = 0;
        SHR_EXIT();
    }
                                  ...
                                  void *user_data)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_entry_list_compress_child_entry_map_get(unit,
                                      ...
                                      &map_size));
    if (entry_list_compress_child_entry_map == NULL) {
        SHR_EXIT();
    }
    map_type = BCMFP_IDP_MAP_TYPE_ENTRY_TO_LC_ENTRY;
    SHR_IF_ERR_VERBOSE_EXIT
...
static int
bcmfp_entry_list_compress_first_child_entry_fill(int unit,
                                bcmfp_stage_id_t stage_id,
                                bcmfp_entry_id_t child_entry_id,
                                bcmfp_entry_id_t parent_entry_id,
                                void *user_data)
                                ...
                                bcmfp_entry_id_t *child_entry_id)
{
    ...
    sal_memset(&idp_info, 0, sizeof(bcmfp_idp_info_t));
    cb = bcmfp_entry_list_compress_first_child_entry_fill;
    user_data = &idp_info;
    ...
    *child_entry_id = idp_info.list_compress_child_entry_id;
exit:
    SHR_FUNC_EXIT();
}
