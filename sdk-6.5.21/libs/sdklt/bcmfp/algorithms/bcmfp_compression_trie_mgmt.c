                                ...
                                void **user_data)
{
    ...
    for (trie_type = 0; trie_type < num_tries; trie_type++) {
        rv = bcmfp_compress_key_insert(unit,
                                       in_node_info[trie_type],
                                       trie[trie_type],
                                       user_data[trie_type]);
        if (SHR_FAILURE(rv)) {
            return (rv);
                          ...
                          void *user_data)
{
    ...
    if (in_node_linked) {
        /* Update CID/CID Mask for nodes moved to trie */
        rv = bcmfp_compress_id_update(unit, trie, trie->root, user_data);
        if (SHR_FAILURE(rv)) {
            return (rv);
        }
        rv = bcmfp_compress_key_lookup(unit, in_node_info, trie);
        return (rv);
    }
                                ...
                                void **user_data)
{
    ...
    for (trie_type = 0; trie_type < num_tries; trie_type++) {
        rv = bcmfp_compress_key_delete(unit,
                                       in_node_info[trie_type],
                                       trie[trie_type],
                                       user_data[trie_type]);
        if (SHR_FAILURE(rv)) {
    ...
    if (lc_node) {
        ...
        if (SHR_FAILURE(rv)) {
            if (rv == SHR_E_RESOURCE) {
                ...
                (void)(bcmfp_compress_id_update(unit, trie, trie->root, user_data));
            }
            return (rv);
        }
