                             ...
                             bcmfp_keygen_oper_t *keygen_oper)
{
    ...
    for (part = 0; part < keygen_cfg->num_parts; part++) {
        ...
        for (pmux_idx = 0; pmux_idx < BCMFP_KEYGEN_EXT_PMUX_NUM; pmux_idx++) {
        ...
        for (ext_idx = 0;
            ...
            if (ext_cfg->in_use == TRUE) {
                ...
                if (SHR_BITGET(ext_cfg->ext_attrs.w,
                    ...
                    ext_ctrl_info[idx].ctrl_sel_val = ext_cfg->ctrl_sel_val;
                } else {
                    ext_ctrl_info[idx].ctrl_sel_val = finfo->sec_val;
                    for (idx2 = 0; idx2 < finfo->qual_info_count; idx2++) {
                         ext_ctrl_info[idx].qual_id[idx2] =
                                            finfo->qual_info[idx2].qual_id;
                    }
                    ext_ctrl_info[idx].qual_id_count = finfo->qual_info_count;
                }
                               ...
                               bcmfp_keygen_md_t *keygen_md)
{
    ...
    for (idx = 0; idx < BCMFP_KEYGEN_EXT_FIELDS_MAX; idx++) {
       ...
       if (keygen_cfg->flags & BCMFP_KEYGEN_CFG_FLAGS_SINGLE_PRI_SEL) {
           ext_codes = &(keygen_md->ext_codes[0]);
       } else {
           ext_codes = &(keygen_md->ext_codes[part]);
       }
       switch (finfo->pri_ctrl_sel) {
           case BCMFP_KEYGEN_EXT_CTRL_SEL_AUX_TAG_A:
               if (ext_codes->aux_tag_a_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->aux_tag_a_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->aux_tag_a_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_AUX_TAG_B:
               if (ext_codes->aux_tag_b_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->aux_tag_b_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->aux_tag_b_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_AUX_TAG_C:
               if (ext_codes->aux_tag_c_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->aux_tag_c_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->aux_tag_c_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_AUX_TAG_D:
               if (ext_codes->aux_tag_d_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->aux_tag_d_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->aux_tag_d_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_CLASS_ID_A:
               if (ext_codes->class_id_cont_a_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->class_id_cont_a_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->class_id_cont_a_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_CLASS_ID_B:
               if (ext_codes->class_id_cont_b_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->class_id_cont_b_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->class_id_cont_b_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_CLASS_ID_C:
               if (ext_codes->class_id_cont_c_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->class_id_cont_c_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->class_id_cont_c_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_CLASS_ID_D:
               if (ext_codes->class_id_cont_d_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->class_id_cont_d_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->class_id_cont_d_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_TCP_FN:
               if (ext_codes->tcp_fn_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->tcp_fn_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->tcp_fn_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_TTL_FN:
               if (ext_codes->ttl_fn_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->ttl_fn_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->ttl_fn_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_TOS_FN:
               if (ext_codes->tos_fn_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->tos_fn_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->tos_fn_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_SRC_A:
               if (ext_codes->src_cont_a_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->src_cont_a_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->src_cont_a_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_SRC_B:
               if (ext_codes->src_cont_b_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->src_cont_b_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->src_cont_b_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_DST_A:
               if (ext_codes->dst_cont_a_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->dst_cont_a_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->dst_cont_a_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_DST_B:
               if (ext_codes->dst_cont_b_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->dst_cont_b_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->dst_cont_b_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_SRC_DST_0:
               if (ext_codes->src_dest_cont_0_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->src_dest_cont_0_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->src_dest_cont_0_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_SRC_DST_1:
               if (ext_codes->src_dest_cont_1_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->src_dest_cont_1_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->src_dest_cont_1_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_IPBM_PRESENT:
               if (ext_codes->ipbm_source ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->ipbm_source =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->ipbm_source !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_L3_L4_NORM:
               if (ext_codes->normalize_l3_l4_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->normalize_l3_l4_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->normalize_l3_l4_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_MAC_NORM:
               if (ext_codes->normalize_mac_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->normalize_mac_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->normalize_mac_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_UDF:
               if (ext_codes->udf_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->udf_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->udf_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
           case BCMFP_KEYGEN_EXT_CTRL_SEL_ALT_TTL_FN:
               if (ext_codes->alt_ttl_fn_sel ==
                   BCMFP_KEYGEN_EXT_SELCODE_DONT_CARE) {
                   ext_codes->alt_ttl_fn_sel =
                                  finfo->pri_ctrl_sel_val;
               } else if (ext_codes->alt_ttl_fn_sel !=
                          finfo->pri_ctrl_sel_val) {
                   SHR_ERR_EXIT(SHR_E_RESOURCE);
               }
               break;
