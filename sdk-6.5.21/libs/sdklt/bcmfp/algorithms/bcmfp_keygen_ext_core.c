                               ...
                               bcmfp_keygen_md_t *keygen_md)
{
    ...
    LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
              "Creating finfo's corresponding to QSET\n")));
    ...
    for (idx = 0; idx < keygen_cfg->qual_info_count; idx++) {
        ...
        for (f_idx = 0; f_idx < qual_cfg->num_chunks; f_idx++) {
             ...
             */
            if (finfo->qual_info_count >= BCMFP_KEYGEN_MAX_QUAL_PER_EXT) {
                SHR_ERR_EXIT(SHR_E_INTERNAL);
            }
            /* update qual_info */
    ...
    for (i = 0; i < field_count; i++) {
       LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                 "Section=%d, SecValue=%d, req_bmp=%X\n"),
                                 ...
                                 bcmfp_keygen_md_t *keygen_md)
{
    ...
    for (temp_level = 1; temp_level <= level; temp_level++) {
        for (ext_idx = 0;
            ...
            if (!equal) {
                LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                          "Validation failed for finfo -- "
                          "Section:%d,SectionValue:%d "
                          "req_bmp:0x%X, ext_bmp:0x%X\n"),
                          temp_finfo1->section,
                          temp_finfo1->sec_val,
                          temp_finfo1->req_bmp[out_sec_idx][0],
                          temp_finfo1->ext_bmp[out_sec_idx][0]));
                            ...
                            bcmfp_keygen_md_t *keygen_md)
{
    ...
    for (temp_level = 1; temp_level <= level; temp_level++) {
        for (ext_idx = 0;
            ...
            LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                      "finfo processed -- Section:%d,SectionValue:%d "
                      "req_bmp:0x%X, ext_bmp:0x%X\n"),
                      temp_finfo1->section,
                      temp_finfo1->sec_val,
                      temp_finfo1->req_bmp[out_sec_idx][0],
    ...
    while (temp_finfo3 != NULL) {
        ...
        if (idx == num_ext) {
            LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                  "finfo's generated(%d) in this section are "
                  "more than extractors available\n"), idx));
            break;
        }
    }
                                         ...
                                         bcmfp_keygen_md_t *keygen_md)
{
    ...
    for (idx = 0; idx < gran_count; idx++) {
        ...
        LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                  "Allocating extractors with granularity:%d "
                  "Number of extractors:%d in section:%d\n"),
                  gran, num_ext, section));
        /*
                                   ...
                                   bcmfp_keygen_md_t *keygen_md)
{
    ...
    if ((sec_flags & BCMFP_KEYGEN_EXT_SECTION_WITH_GRAN_1) &&
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_keygen_section_fields_create_type1(unit,
                                                      part,
                                                      level,
                                                      section,
                                                      keygen_cfg,
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_keygen_section_fields_create_onehot(unit,
                                                       part,
                                                       level,
                                                       section,
                                                       keygen_cfg,
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_keygen_section_fields_create_type2(unit,
                                                      part,
                                                      level,
                                                      section,
                                                      keygen_cfg,
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_keygen_section_fields_create_type3(unit,
                                                      part,
                                                      level,
                                                      section,
                                                      keygen_cfg,
                              ...
                              bcmfp_keygen_md_t *keygen_md)
{
    ...
    LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
              "Creating finfo's in Part:%d Level:%d\n"),
              part, (level+1)));
    ...
    for (idx = 0; idx < BCMFP_KEYGEN_EXT_MAX_OUT_SECTIOINS; idx++) {
        ...
        if (idx) {
            SHR_BITXOR_RANGE(finfo->req_bmp[idx - 1],
                             ...
                             finfo->req_bmp[idx]);
            LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                      "Required bitmap of previous section is "
                      "updated to 0x%X\n"),
                      finfo->req_bmp[idx-1][0]));
        ...
        LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                  "Required bitmap 0x%X Extracted Bitmap 0x%X "
        ...
        if (ext_cfg->conflict_ext_id[idx]) {
            ...
            if (conflict_ext_cfg->in_use == TRUE &&
                ...
                conflict_ext_id = conflict_ext_cfg->ext_id;
                /*
                 * the extractor cant be used only if the out_sec of ext
                 * matches with the out_sec of conflicting extractor
                 */
                LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                          "finfo cannot be pushed to this out "
                          "section because the conflicting extractor "
                          "is already in use\n")));
                LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                          "Conflicting extractor id:%d, part:%d, "
                          "gran:%d, level:%d \n"),
                          BCMFP_KEYGEN_EXT_ID_NUM_GET(conflict_ext_id),
                          BCMFP_KEYGEN_EXT_ID_PART_GET(conflict_ext_id),
                          BCMFP_KEYGEN_EXT_ID_GRAN_GET(conflict_ext_id),
                          BCMFP_KEYGEN_EXT_ID_LEVEL_GET(conflict_ext_id)));
                rv = SHR_E_RESOURCE;
                continue;
            }
        }
                      ...
                      uint16_t sticky_head_finfo_ext_num)
{
    ...
    LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
              "---------->Allocating extractors for finfo "
              "[section:%d, SecionValue:%d] in part:%d "
              "level:%d\n"),
              finfo->section, finfo->sec_val, part, level));
    ...
    for (ext_idx = 0;
        ...
        LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                  "---------->Trying Extractor with id=%d, "
                  "gran=%d, part=%d, level=%d\n"),
                  BCMFP_KEYGEN_EXT_ID_NUM_GET(ext_cfg->ext_id),
                  g, p , l));
        if (p != part) {
            LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                      "This extractor cannot be used because "
                      "(finfo part:%d, extractor part:%d)\n"),
                      part, p));
           continue;
        }
        ...
        if (l != (level + 1)) {
            LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                      "This extractor cannot be used beacuse "
                      "(finfo level:%d, extractor level:%d)\n"),
                      level, l));
            continue;
        }
        ...
        if (section != ext_cfg->in_sec) {
            LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                      "This extractor cannot be used beacuse "
                      "(finfo section:%d, extractor in section:%d)\n"),
        ...
        if (keygen_md->ext_cfg_db->sec_cfg[finfo->section]) {
            sec_cfg = keygen_md->ext_cfg_db->sec_cfg[finfo->section];
            if (sec_cfg->size) {
                if ((uint32_t)((finfo->sec_val * g) + g) > sec_cfg->size) {
                      LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                          "This extractor cannot be used beacuse "
                          "part of finfo chunk is going beyond the"
                          "section size.\n")));
                    continue;
                }
            }
        ...
        if (SHR_BITGET(ext_cfg->ext_attrs.w,
            ...
            if (ext_cfg->valid_bitmap < finfo->req_bmp[0][0]) {
                LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                          "This extractor cannot be used beacuse"
                          "(finfo req_bmp:%d, extractor valid_bmp:%d)\n"),
                          finfo->req_bmp[0][0], ext_cfg->valid_bitmap));
                continue;
            }
        } else {
            if (g != finfo->size) {
        ...
        if (rv == SHR_E_CONFIG) {
            LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                      "This extractor cannot be used beacuse "
                      "validation failed in the function "
                      "bcmfp_keygen_ext_validate\n")));
            continue;
        }
        SHR_IF_ERR_EXIT(rv);
         ...
         */
        rv = bcmfp_keygen_ext_sections_validate(unit,
                                                part,
                                                ext_cfg,
                                                finfo,
                                                keygen_cfg,
        ...
        if (rv == SHR_E_RESOURCE) {
            LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                      "This extractor cannot be used beacuse "
                      "no more free space in any of the out "
                      "sections of this extractor\n")));
            continue;
        }
        SHR_IF_ERR_EXIT(rv);

        LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                  "<----------This extractor is used to extract "
                  "the finfo\n")));
                       ...
                       bcmfp_keygen_ext_cfg_t *ext_cfg)
{
    ...
    if (SHR_BITGET(keygen_md->acl_attrs[p].w,
        BCMFP_KEYGEN_ACL_ATTR_PMUX_OVERLAY_DVP) &&
        SHR_BITGET(ext_cfg->ext_attrs.w,
        BCMFP_KEYGEN_EXT_ATTR_NOT_WITH_PMUX_OVERLAY_DVP)) {
        SHR_ERR_EXIT(SHR_E_CONFIG);
    }
    if (SHR_BITGET(keygen_md->acl_attrs[p].w,
                             ...
                             bcmfp_keygen_md_t *keygen_md)
{
     ...
     */
    LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
              "---------->All finfos must fit in %d parts\n"),
    ...
    if (keygen_cfg->num_parts == 1) {
        ...
        for (idx = 0; idx < BCMFP_KEYGEN_EXT_FIELDS_MAX; idx++) {
            ...
            for (part = 0; part < keygen_cfg->num_parts; part++) {
                ...
                LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
                          "---------->Trying to push [Section=%d, "
                          "SecValue=%d] in part=%d\n"),
    ...
    LOG_DEBUG(BSL_LOG_MODULE, (BSL_META_U(unit,
              "---------->Allocating extractors once all the "
              "finfos are equally distributed to all parts\n")));
    /*
