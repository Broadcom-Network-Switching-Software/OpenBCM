...
bcmfp_keygen_qual_cfgs_overlap_check(int unit,
                                     ...
                                     bool *qual_overlap)
{
    uint16_t qual_cfg_id1 = 0;
    uint16_t qual_cfg_id2 = 0;
    uint8_t qual_cfg_chunk_id1 = 0;
    uint8_t qual_cfg_chunk_id2 = 0;
    bcmfp_keygen_qual_cfg_t *qual_cfg1 = NULL;
    bcmfp_keygen_qual_cfg_t *qual_cfg2 = NULL;
    bcmfp_keygen_qual_cfg_info_t *qual_cfg_info1 = NULL;
    bcmfp_keygen_qual_cfg_info_t *qual_cfg_info2 = NULL;
    bcmfp_keygen_ext_section_t section1 = BCMFP_KEYGEN_EXT_SECTION_DISABLE;
    bcmfp_keygen_ext_section_t section2 = BCMFP_KEYGEN_EXT_SECTION_DISABLE;
    uint16_t sec_offset1 = 0;
    uint16_t sec_offset2 = 0;
    uint16_t width1 = 0;
    uint16_t width2 = 0;
    bool match_chunk_found = FALSE;

    SHR_FUNC_ENTER(unit);

    qual_cfg_info1 = keygen_cfg->qual_cfg_info_db->qual_cfg_info[qual_id];
    qual_cfg_info2 = keygen_cfg->qual_cfg_info_db->qual_cfg_info[qual_id2];

    /*
     * if the number of qual_cfgs donot mathc =>
     * non-overlapping qualifiers
     */
    if (qual_cfg_info1->num_qual_cfg != qual_cfg_info2->num_qual_cfg) {
        *qual_overlap = FALSE;
        SHR_EXIT();
    }

    for (qual_cfg_id1 = 0;
         qual_cfg_id1 < qual_cfg_info1->num_qual_cfg;
         qual_cfg_id1++) {

        qual_cfg1 = &(qual_cfg_info1->qual_cfg_arr[qual_cfg_id1]);

        /*
         * qualifiers are considered to be overlapping if
         * all qual_cfg's of both the qualifiers match.
         */

        for (qual_cfg_id2 = 0;
                 qual_cfg_id2 < qual_cfg_info2->num_qual_cfg;
                 qual_cfg_id2++) {

                qual_cfg2 = &(qual_cfg_info2->qual_cfg_arr[qual_cfg_id2]);
                if (qual_cfg1->num_chunks != qual_cfg2->num_chunks) {
                    continue;
                }

                for (qual_cfg_chunk_id1 = 0;
                     qual_cfg_chunk_id1 < qual_cfg1->num_chunks;
                     qual_cfg_chunk_id1++) {

                    section1 =
                        qual_cfg1->e_params[qual_cfg_chunk_id1].section;
                    sec_offset1 =
                        qual_cfg1->e_params[qual_cfg_chunk_id1].offset;
                    width1 =
                        qual_cfg1->e_params[qual_cfg_chunk_id1].width;
                    match_chunk_found = FALSE;

                    for (qual_cfg_chunk_id2 = 0;
                         qual_cfg_chunk_id2 < qual_cfg2->num_chunks;
                         qual_cfg_chunk_id2++) {
                        section2 =
                            qual_cfg2->e_params[qual_cfg_chunk_id2].section;
                        sec_offset2 =
                            qual_cfg2->e_params[qual_cfg_chunk_id2].offset;
                        width2 =
                            qual_cfg2->e_params[qual_cfg_chunk_id2].width;

                        if (section1 == section2 &&
                            sec_offset1 == sec_offset2 &&
                            width1 == width2) {
                            match_chunk_found = TRUE;
                            break;
                        }

                    }
                    if (match_chunk_found == FALSE) {
                        break;
                    }
        ...
        /*
         * If none of qual_cfg's of qual_id2
         * matches with qual_cfg1 =>
         * non overlapping qualifier configs
         */
        if (match_chunk_found == FALSE) {
            *qual_overlap = FALSE;
            SHR_EXIT();
                           ...
                           uint16_t *rkey_size)
{
    ...
    *rkey_size = 0;
    for (idx = 0; idx < keygen_cfg->qual_info_count; idx++) {
        ...
        /*
         * check if the qual_cfg overlaps with any
         * other qualifiers
         */
        for (idx2 = 0; idx2 < idx; idx2++) {
            ...
            qual_id2 =  keygen_cfg->qual_info_arr[idx2].qual_id;
            if (qual_cfg_info1[qual_id2] == NULL) {
                continue;
            }
            qual_overlap = FALSE;
            /* check if any overlap exists between qual_id and qual_id2 */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmfp_keygen_qual_cfgs_overlap_check(unit,
                                                      keygen_cfg,
                                                      qual_id,
                                                      qual_id2,
                                                      &qual_overlap));
            /*
             * If qual_id has overlapped config
             * with some qualifier before it,
             * then the qualifier config/size is already
             * accounted for. Skip adding the qualifier size
             * to overall key_size
             */
            if (qual_overlap == TRUE) {
                break;
            }
        }

        /* the qualifier config is already accounted for */
        if (qual_overlap == TRUE) {
            continue;
        }
