...
typedef enum bcmfp_stage_flag_s {
    ...
    BCMFP_STAGE_FLEX_CTR_PER_SLICE_SUPPORT,

    /*!
     * To skip ISDW mode for a group if the
     * group mode is AUTO during group creation.
     */
    BCMFP_STAGE_DISABLE_ISDW_IN_MODE_AUTO,

    /*!
     * Allows specifying conflicting qualifier sets
     * in groups QSET.
     */
    BCMFP_STAGE_CONFLICT_QSETS_SUPPORT,
...
typedef struct bcmfp_qset_aset_size_info_s {
  ...
  uint32_t data_end_bit[BCMFP_GROUP_MODE_COUNT][4];

  uint32_t data_start_bit[BCMFP_GROUP_MODE_COUNT][4];
  ...
  /* Max key size of presel stage */
  uint32_t presel_key_size_max;
...
typedef struct bcmfp_stage_misc_info_s {
    ...
    /* Maximum counter profiles that can be attached to a group */
    uint8_t max_counter_profiles_per_group;
...
/*!
 * AACL compression metadata which is generated and used at
 * when a new ALPM key is inserted or existing ALPM key is
 * deeleted.
 */
typedef struct bcmfp_compress_md_info_s {
    /*! All entry IDs that needs update. */
    uint32_t *entry_id_bmp;

    /*! Size of the entry_id_bmp in uint32_t words. */
    uint16_t entry_id_bmp_size;

    /*! Group Ids  in which entries that needs update. */
    uint32_t *group_id_bmp;

    /*! Size of the group_id_bmp in uint32_t words. */
    uint16_t group_id_bmp_size;

    /*! Number of entries in each group that needs updated. */
    uint32_t *entry_count_in_group;

    /*! Size of entry_count_in_group in uint32_t words. */
    uint32_t entry_count_in_group_size;

    /*!
     * Total number of entries across all groups that
     * needs update.
     */
    uint32_t req_slots;

    /*! Total number of groups that needs update. */
    uint16_t num_groups;
} bcmfp_compress_md_info_t;

typedef enum bcmfp_comression_key_type_s {
    /*! Compression key is L4_PORT + IP */
    BCMFP_COMP_KEY_IP_L4_PORT,
    /*! Compression key is VRF + IP */
    BCMFP_COMP_KEY_IP_VRF,
    /*! Compression key includes all fields */
    BCMFP_COMP_KEY_FULL,
    /*! Compression key includes IP addr alone */
    BCMFP_COMP_KEY_IP,
    /*! Last in the list */
    BCMFP_COMP_KEY_COUNT,
} bcmfp_compression_key_type_t;
...
typedef struct bcmfp_stage_s {
    ...
    /*! CID update back info from trie algorithim. */
    bcmfp_cid_update_list_t *cid_update_info[BCMFP_COMPRESS_TYPES_MAX];

    /*! User data in CID update call back from trie algorithim. */
    bcmfp_cid_update_cb_info_t *cb_info[BCMFP_COMPRESS_TYPES_MAX];

    /*! Metadata generated using CID update call back info. */
    bcmfp_compress_md_info_t compress_md;
    ...
    /*! Compression KeyType. */
    bcmfp_compression_key_type_t compression_key_type[BCMFP_COMPRESS_TYPES_MAX];
