...
/* Max number of Mux's per type. */
#define BCMFP_MAX_PDD_MUX                    (32)
...
typedef struct bcmfp_pdd_hw_section_info_s {
    ...
    /* Number of 4 bit Mux's for full mux style. */
    uint8_t  num_mux4;

    /* 4Bit Mux is MSB aligned. */
    bool     is_mux4_msb;
    ...
    /* Number of 8 bit Mux's for full mux style. */
    uint8_t  num_mux8;

    /* 8Bit Mux is MSB aligned. */
    bool     is_mux8_msb;
    ...
    /* Number of 16 bit Mux's for full mux style. */
    uint8_t  num_mux16;

    /* 16Bit Mux is MSB aligned. */
    bool     is_mux16_msb;

    /* 16Bit Mux section id. */
    uint8_t  mux16_sec_id;

    /* 16Bit Mux multiplier. */
    uint8_t  mux16_mul;

    /* Per entry SBR/PDD is MSB aligned. */
    bool     is_fixed_sbr_pdd_msb;

    /* AUX menu offset in 8bit mux data. */
    uint8_t  aux_byte_offset;
...
typedef struct bcmfp_pdd_hw_info_s {
    ...
    /*
     * Number of PDD profile SIDs used to save
     * priorities of all actions in a group/entry.
     */
    uint8_t pdd_profile_sid_columns;
 ...
 */
    ...
    /*
     * This bit indicates PDD data is full mux format.
     */
    bool pdd_data_full_mux_format;
...
/* Should be same as HA structure bcmfp_pdd_mux_oper_info_t */
typedef struct bcmfp_pdd_mux_info_s {
    /* MUX4 sel values. */
    int16_t  s0_mux4_sel[BCMFP_MAX_PDD_MUX];

    /* MUX8 sel values. */
    int16_t  s1_mux8_sel[BCMFP_MAX_PDD_MUX];

    /* MUX16 sel values. */
    int16_t  s1_mux16_sel[BCMFP_MAX_PDD_MUX];

    /* MUX4 align value. */
    uint8_t  s0_mux4_align;

    /* MUX8 align value. */
    uint8_t  s1_mux8_align;

    /* MUX16 align value. */
    uint8_t  s1_mux16_align;
} bcmfp_pdd_mux_info_t;

typedef struct bcmfp_pdd_mux_cont_info_s {
    /* container id */
    uint16_t cont_id;

    /* Bits required in the container */
    uint32_t bitmap;

    /* Flex Data offset for the container */
    uint16_t  offset;

    /*
     * Flex Data offset adjust for the container
     * to handle higher part of container.
     */
    uint16_t  offset_adjust;

    /* Container is spread in 2 8bit Mux's */
    bool     two_8bit_mux;

    /* Next container */
    struct bcmfp_pdd_mux_cont_info_s *next;
} bcmfp_pdd_mux_cont_info_t;
...
/*! Action related to Class Id. */
#define BCMFP_ACTION_CLASS_ID (0x1 << 6)
