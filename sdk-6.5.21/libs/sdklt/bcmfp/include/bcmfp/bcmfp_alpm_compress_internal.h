...
/*!
 * \brief Insert incoming compress key, mask into trie that returns
 *  the CID and CID_MASK.
 *
 * \param [in]  unit          Logical device id
 * \param [in]  list_compress List compress the compress keys.
 * \param [in]  ckeys         List of compress keys.
 * \param [in]  user_data     Data to be passed back to component using
 *                            compression algorithm if there are changes
 *                            in CID/CID_MASK Of existing ALPM keys.
 * \param [out] list_compressed TRUE = List compressed the compress keys.
...
bcmfp_alpm_ckeys_insert_to_trie(int unit,
                                ...
                                bool *list_compressed);
/*!
 * \brief Insert the ALPM LT entry words, using incoming compress key,
 *  mask and CID passed, into the hardware.
 *
 * \param [in]  unit          Logical device id
 * \param [in]  op_arg        Operation arguments.
 * \param [in]  stage_id      BCMFP stage ID
...
bcmfp_alpm_ckeys_delete_from_trie(int unit,
                                  ...
                                  void *user_data);
/*!
 * \brief Delete the ALPM LT entry words, using incoming compress key
 * and mask, from the hardware.
 *
 * \param [in]  unit          Logical device id
 * \param [in]  op_arg        Operation arguments.
 * \param [in]  stage_id      BCMFP stage ID
...
/*!
 * \brief Process BCMFP ALPM CONTROL LT events.
 *
 * \param [in] unit     Logical device id.
