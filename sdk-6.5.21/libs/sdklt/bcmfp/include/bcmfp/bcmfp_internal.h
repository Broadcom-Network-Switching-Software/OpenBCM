...
/*!
 * \brief Get the maximum value of conflict qset ID that can be used in
 *        FP_XXX_CONFLICT_QSET LT in a stage.
 *
 * \param [in] unit Logical device ID
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [out] max_conflict_qset_id maximum conflict qset ID
 *                                   in FP_XXX_CONFLICT_QSET LT.
 *
 * \retval SHR_E_NONE Success
...
bcmfp_stage_max_group_id_get(int unit,
                             ...
                             uint32_t *max_group_id);
/*!
 * \brief Get the maximum value of entry id that can be used in
 *        FP_XXX_ENTRY LT in a stage.
 *
 * \param [in] unit Logical device id
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [out] max_entry_id maximum entry id in FP_XXX_ENTRY LT.
 *
 * \retval SHR_E_NONE Success
 * \retval SHR_E_PARAM max_entry_id is NULL
 * \retval SHR_E_INTERNAL Unsupported stage_id
 */
extern int
...
bcmfp_policy_entry_map_check(int unit,
                             ...
                             bool *not_mapped);
/*!
 * \brief Add group id to given conflict qset id mapping.
 *
 * \param [in] unit Logical device id
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [in] conflict_qset_id Index to FP_XXX_CONFLICT_QSET LT.
...
/*!
 * \brief Delete group id from given conflict qset id mapping.
 *
 * \param [in] unit Logical device id
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [in] conflict_qset_id Index to FP_XXX_CONFLICT_QSET LT.
...
/*!
 * \brief Check whether conflict qset id is associated to any
 *  group id(s).
 *
 * \param [in] unit Logical device id
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [in] conflict_qset_id Index to FP_XXX_CONFLICT_QSET LT.
 * \param [in] not_mapped Flag to indicate map status.
 *             TRUE = conflict qset id not mapped to group id(s).
 *             FLASE = conflict qset id mapped to group id(s).
 *
 * \retval SHR_E_NONE Success
 * \retval SHR_E_INTERNAL Unsupported stage_id
 */
 ...
 * \param [in] cqset_id Conflict QSET id.
 ...
 * \param [in] alpm control updated data from event manager
 *
 * \retval SHR_E_NONE Success
 ...
 * \param [in] alpm_ctrl  Data from ALPM_CONTROL LT
 *
 * \retval SHR_E_NONE     Success
 * \retval SHR_E_PARAM    Invalid parameters
 */
...
/*!
 * \brief Loop through all groups ids associated to a given
 * pdd template id till a specific condition is met.
 *
 * \param [in] unit Logical device id
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [in] pdd_id Index to FP_XXX_PRESEL_TEMPLATE LT.
 * \param [in] cb User callback function
 * \param [in] user_data Data to be passed to traverse callback fucntion.
 * \param [in] cond_met status which indicates if looping
 *             can be stopped.
 *
 * \retval SHR_E_NONE Success
 * \retval SHR_E_INTERNAL Unsupported stage_id
...
/*!
 * \brief Get the first list compressed entry_id associated to the
 *  given parent entry ID.
 *
 * \param [in] unit Logical device id
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [in] parent_entry_id Parent FP entry ID
 * \param [out] child_entry_id First list compressed child eid.
 *
 * \retval SHR_E_NONE Success
 * \retval SHR_E_INTERNAL Unsupported stage_id
 */
extern int
bcmfp_entry_list_compress_first_child_entry_get(int unit,
                                 bcmfp_stage_id_t stage_id,
                                 bcmfp_entry_id_t parent_entry_id,
                                 bcmfp_entry_id_t *child_entry_id);
/*!
...
/*!
 * \brief Allocate the LTD buffers to be used in IMM calls.
 *
 * \param [in] unit Logical device id
 * \param [in] OUT field ptr.
 *
 * \retval SHR_E_NONE Success
 */
extern int
