...
bcmfp_pdd_config_delete(int unit,
                        ...
                        bcmltd_fields_t *output_fields);
/*!
 * \brief Check if the group_id enquired by user
 * is mapped to pdd_id.
 * the group_id which is mapped to pdd is checked against
 * the user enquired group_id if they match.
 *
 * \param [in] unit         Logical device id.
 * \param [in] stage_id     FP Stage id.
 * \param [in] group        FP Group id.
 * \param [in] template_id  PDD id.
 * \param [in] user_data    generic user data
 * \param [in] cond_met     which is set to true
 *                          if the given group_id is mapped to pdd
 */
extern int
bcmfp_pdd_group_map_id_check(int unit,
                             bcmfp_stage_id_t stage_id,
