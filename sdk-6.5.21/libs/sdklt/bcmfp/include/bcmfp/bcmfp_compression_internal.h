...
typedef struct bcmfp_cid_update_cb_info_s {
    ...
    /*!
     * IMM event(INSERT/DELETE/UPDATE) that triggers this
     * callback.
     */
    bcmimm_entry_event_t event;

    /*! Pointer to the next cid update cb_info. */
    struct bcmfp_cid_update_cb_info_s *next;
...
/*!
 * \brief Create local copies of cb_info and cid_update_info
 * from trie algo call backs and save them in stage structure.
 *
 * \param [in] unit Logical device id
...
/*!
 * \brief Free the memory created for cb_info and cid_update_info
 *  in stage structure.
 *
 * \param [in] unit Logical device id
...
bcmfp_ckey_entry_map_traverse(int unit,
                              ...
                              void *user_data);
/*!
 * \brief Check if any entry_ids are mapped to given compression key.
 *
 * \param [in] unit Logical device id
 * \param [in] stage_id stage id(BCMFP_STAGE_ID_XXX).
 * \param [in] ctype Tree to which compression key belongs to.
 * \param [in] key Compression key.
 * \param [in] prefix Prefix of compression key.
 * \param [out] not_mapped Flag to indiacte whether compression key
 *                         is mapped to at least one entry or not.
 *
 * \retval SHR_E_NONE Success
 * \retval SHR_E_PARAM Wrong parameters
 */
extern int
bcmfp_ckey_entry_map_check(int unit,
                           bcmfp_stage_id_t stage_id,
                           uint8_t ctype,
                           uint32_t *key,
                           uint32_t prefix,
...
bcmfp_compress_oper_info_create(int unit,
                                bcmfp_stage_id_t stage_id);
/*!
 * \brief compile all the data in stage->compress_md and generate
 *  data in bcmptm_rm_tcam_fp_resources_req_t format and send the
 *  request to RM-TCAM for validation. This is to validate if
 *  enough resources are present in FP TCAMs. This is needed when
 *  a new compression enable FP entry is inserted and that causes
 *  some existing compression keys to move from listo to trie.
 *
 * \param [in] unit     Logical device id
 * \param [in] op_arg   Operational arguments.
 * \param [in] stage_id FP stage Id.
 * \param [in] group_id FP group in which current FP entry is inserted.
 *
 * \retval SHR_E_NONE Success
 * \retval SHR_E_PARAM Wrong parameters
 */
