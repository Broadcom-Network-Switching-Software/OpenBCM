...
/*
 * Max number of qualifiers extracted using a
 * single multiplexer.
 */
#define BCMFP_KEYGEN_MAX_QUAL_PER_EXT 64
...
typedef enum bcmfp_keygen_ext_attr_s {
    ...
    /*!
     * Extractor cannot be used if post mux qualifier,
     * OverlAY DVP, is present in the QSET.
     */
...
typedef enum bcmfp_keygen_ext_section_e {
    ...
    /*!
     * Section for post Mux qualifier "Overlay DVP"
     * in the final key.
     */
    BCMFP_KEYGEN_EXT_SECTION_OVERLAY_DVP,
...
typedef struct bcmfp_keygen_ext_ctrl_sel_info_s {
    ...
    /*! Array of qualifiers extracted using this extractor. */
    bcmfp_qualifier_t qual_id[BCMFP_KEYGEN_MAX_QUAL_PER_EXT];

    /*! Number of valid elements in qual_id array. */
    uint16_t qual_id_count;
...
typedef struct bcmfp_keygen_ext_cfg_s {
  ...
  /*!
    * Used internally in cases of group qualifier updates.
    * The ctrl_sel_val of the extractors already in use
    * are set here.
    */
   uint16_t ctrl_sel_val;
...
typedef struct bcmfp_keygen_ext_section_cfg_s {
    ...
    /*!
     * Section size in bits. This is same as drain_bits for
     * all sections except level 1 sections. Size will be set
     * for level 1 sections during extractors initialization.
     */
    uint32_t size;
...
/*!
 * Used to set the keygen_oper->ext_ctrl_sel_info
 */
#define BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(_unit_, _keygen_cfg_,            \
                                           _part_, _gran_, _level_,         \
                                           _ext_num_, _ctrl_sel_,           \
                                           _ctrl_sel_val_, _ext_code_idx)   \
           keygen_cfg->ext_ctrl_sel_info[ext_code_idx].part = part;         \
           keygen_cfg->ext_ctrl_sel_info[ext_code_idx].gran = gran;         \
           keygen_cfg->ext_ctrl_sel_info[ext_code_idx].level = level;       \
           keygen_cfg->ext_ctrl_sel_info[ext_code_idx].ext_num = ext_num;   \
           keygen_cfg->ext_ctrl_sel_info[ext_code_idx].ctrl_sel = ctrl_sel; \
           keygen_cfg->ext_ctrl_sel_info[ext_code_idx].ctrl_sel_val =       \
                                                         ctrl_sel_val;
/*!
 * Set the section size in bits. This is required for sections in level1
 *  if some extractors cannot be used to extract chunks at the edge of the
 *  bus. For example if section size is 84 bits and has both 8 and 4 bit
 *  extractors connected to it, 8 bit extractors should not be used to
 *  extract bits 80 to 83.
 */
#define BCMFP_KEYGEN_EXT_SECTION_SIZE_SET(_unit_, _ext_cfg_db_,          \
                                           _sec_, _size_)                \
            do {                                                         \
                _rv_ = bcmfp_keygen_ext_sec_size_set((_unit_), (_sec_),  \
                                        (_size_), (_ext_cfg_db_));       \
                if (SHR_FAILURE(_rv_)) {                                 \
                    SHR_ERR_EXIT(_rv_);                                  \
                }                                                               \
...
/*!
 * If set, the fp entry can have only single keygen profile -
 * pri_sel_val for the qualifier.
 */
#define BCMFP_KEYGEN_CFG_FLAGS_SINGLE_PRI_SEL (1<<5)
 ...
 */
extern int
bcmfp_keygen_qset_size_get(int unit,
                           bcmfp_keygen_cfg_t *keygen_cfg,
                           uint16_t *rkey_size);
...
/*!
 * \brief Set the size for a given section.
 *
 * This function is used to set the size in bits
 * of the given sec_cfg.
 *
 * \param [in] unit Unit Number
 * \param [in] sec section-id enum
 * \param [in] size section size in bits
 * \param [in] ext_cfg_db Extractor Config DB
 *
 * \return SHR_E_NONE on success and error code on failure.
 */
extern int
bcmfp_keygen_ext_sec_size_set(int unit,
                              bcmfp_keygen_ext_section_t sec,
                              uint32_t size,
                              bcmfp_keygen_ext_cfg_db_t *ext_cfg_db);
...
/*!
 * \brief Check if qualifier cfg overlaps between 2 qualifiers.
 *
 * This function is used to check if there exists any overlap
 * between the 2 qualifiers' containers.
 *
 * \param [in] unit Unit number.
 * \param [in] keygen_cfg keygen config DB.
 * \param [in] qual_id Qualifier ID.
 * \param [in] qual_id2 Qualifier ID.
 * \param [out] qual_overlap set to TRUE is the
 *              qualifier cfg's of qual_id qual_id2
 *              overlap.
 *
 * \return SHR_E_NONE on success and error code on failure.
 */
extern int
bcmfp_keygen_qual_cfgs_overlap_check(int unit,
                                     bcmfp_keygen_cfg_t *keygen_cfg,
                                     bcmfp_qualifier_t qual_id,
