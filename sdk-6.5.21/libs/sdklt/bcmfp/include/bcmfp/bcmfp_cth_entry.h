...
typedef enum bcmfp_entry_state_e {
    ...
    /*!
     * For an auto expandable group with per slice counters,
     * counter profiles should be allocated for all slices.
     */
    BCMFP_ENTRY_GROUP_CTR_PROFILES_MISSING = 43,

    /*!
     * Used when an auto expandable group try to share resources
     * with non auto expandable group.
     */
    BCMFP_ENTRY_SHARED_GROUP_AUTOEXPAND_FLAG_MISMATCH = 44,

    /*! Conflict QSET ID is associated to group but not yet created. */
    BCMFP_ENTRY_CONFLICT_QUALIFIER_SET_NOT_CREATED = 45,

    /*!
     * Number of conflict QSET IDs associated to group are
     *  not same as number of presels.
     */
    BCMFP_ENTRY_CONFLICT_QUALIFIER_SETS_LESS_THAN_PRESELS = 46,

    /*!
     * One of the conflict QSET ID associated to the group is
     * invalid due to one of the following two reasons.
     * 1. Same number is assigned to more than one qualifier
     * 2. Qualifier mentioned in conflict QSET is not present
     *    in main QSET.
     */
    BCMFP_ENTRY_CONFLICT_QUALIFIER_SET_INVALID = 47,
