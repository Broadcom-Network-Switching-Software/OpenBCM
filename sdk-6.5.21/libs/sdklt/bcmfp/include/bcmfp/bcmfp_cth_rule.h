...
typedef struct bcmfp_rule_compress_key_s {
    ...
    /*! Reference count filled by trie algorithm. */
    uint32_t ref_count;

    /*! ALPM LT to which this key belongs to. */
    bcmlrd_sid_t req_ltid;

    /*!
     * Flag to indicate whether the key is inserted to
     * ALPM DB or not.
     */
    bool inserted;
    ...
    /*!
     * Compress mask consisting of all compress
     * fields masks together when unmaskable is set to TRUE..
     */
    uint32_t umask[BCMFP_MAX_WSIZE];
