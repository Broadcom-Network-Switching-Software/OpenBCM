                                             ...
                                             bcmfp_filter_t *filter)
{
    ...
    while (action_data1 != NULL) {
        ...
        if (is_default == FALSE) {
            SHR_NULL_CHECK(filter->pdd_config, SHR_E_PARAM);
            action_data2 =
                filter->pdd_config->action_data;
        } else {
            SHR_NULL_CHECK(filter->dpdd_config, SHR_E_PARAM);
            action_data2 =
                               ...
                               bcmfp_filter_t *filter)
{
    ...
    if (BCMFP_STAGE_FLAGS_TEST(stage,
        ...
        for (pipe_id = pipe_min; pipe_id <= pipe_max; pipe_id++) {
            ...
            /*
             * Find the number of bits(ports) enabled in the ipbm key.
             * If it is only one bit(port), XOR need not be done
            ...
            for (part = 0; part < stage->tbls.rule_tbl->num_ipbm_parts;
                ...
                while (qual_data != NULL) {
                    if (qual_data->qual_id == ports_bmp_qualifier) {
                        ...
                        /* Get the number of bits set in each part. */
                        SHR_BITCOUNT_RANGE(ipbm_key, tmp_count, 0, SHR_BITWID);
                        ipbm_bit_count += tmp_count;
                    }
                    qual_data = qual_data->next;
                }
            }
            ...
            for (part = 0; part < stage->tbls.rule_tbl->num_ipbm_parts;
                 ...
                 if ((ports_bmp_present == TRUE) && (qual_data != NULL)) {
                     qual_bitmap = filter->group_config->qual_bitmap;
                     while (qual_bitmap != NULL) {
                         if (qual_bitmap->qual == qual_data->qual_id) {
                             break;
                         }
                         qual_bitmap = qual_bitmap->next;
                     }
                     if ((qual_bitmap != NULL) && (qual_bitmap->partial)) {
                 ...
                 if (ports_bmp_present == TRUE) {
                     if (BCMFP_STAGE_FLAGS_TEST(stage,
                         ...
                         /*
                          * Do XOR to generate the mask only if
                          * the key has more than 1 bit(port).
                          */
                         if (ipbm_bit_count != 1) {
        ...
        if (filter->rule_config != NULL &&
            ...
            switch (filter->rule_config->pbmp_type)  {
                ...
                case BCMFP_DEVICE_PORTS:
                    ...
                    break;
                case BCMFP_SVP_PORTS:
                    ports_bmp_fid = stage->tbls.rule_tbl->svp_ports_key_fid;
                    break;
                default:
                    SHR_ERR_EXIT(SHR_E_INTERNAL);
                    break;
                    ...
                    bcmfp_filter_t *filter)
{
    ...
    if (is_presel == FALSE) {
        flowtracker_en = filter->entry_config->flowtracker_en;
        if ((flowtracker_en == TRUE) ||
                       ...
                       bcmfp_filter_t *filter)
{
    ...
    if ((op_info->ref_count > 1) || (filter->flowtracker_entry_delete)) {
        if (filter->entry_config->flowtracker_en == 0) {
            SHR_IF_ERR_EXIT
    ...
    if (!BCMFP_STAGE_FLAGS_TEST(stage, BCMFP_STAGE_HW_FLOW_TRACKER) &&
        (filter->entry_config->flowtracker_en == 0)) {
        flags = BCMFP_ENTRY_LAST_IN_GROUP;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_ptm_entry_delete(unit,
                                    flags,
                                ...
                                bcmfp_filter_t *filter)
{
    ...
    bcmfp_action_data_t *action_data = NULL;
    bcmfp_action_data_t *action_data_part[BCMFP_ACTION_MAX_PARTS] = {NULL};
    uint32_t **edw = NULL;
    uint32_t **qos = NULL;
    uint8_t part_id = 0;
...
static int
bcmfp_filter_conflict_qsets_process(int unit,
                             const bcmltd_op_arg_t *op_arg,
                             bcmltd_sid_t tbl_id,
                             bcmfp_stage_t *stage,
                             uint8_t cqset_idx,
                             bcmfp_filter_t *filter)
{
    ...
    if (filter->group_config->conflict_qset_ids == NULL) {
        SHR_EXIT();
    }
    ...
    group_id =
        filter->group_config->group_id;
    cqset_id = filter->group_config->conflict_qset_ids[cqset_idx];
                               ...
                               bcmfp_filter_t *filter)
{
    ...
    if (opinfo->pdd_type == BCMFP_PDD_OPER_TYPE_PDD_GROUP_SBR_ENTRY) {
         ...
         * In E2T tiles both PDD and SBR should be per entry or
         * per group.
         */
        if (BCMFP_STAGE_FLAGS_TEST(stage,
                        BCMFP_STAGE_ENTRY_TYPE_HASH)) {
            ...
            start_bit = (end_bit + 1);
            end_bit = (start_bit + stage->misc_info->pdd_index_size - 1);
            pdd_profile_index = opinfo->ext_codes[0].pdd_prof_index;
            bcmdrd_field_set(filter->buffers.edw[0],
                             start_bit,
                             end_bit,
                             &pdd_profile_index);
        ...
        for (idx = 0; idx < filter->num_pse_configs; idx++) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmfp_filter_pse_config_process(unit,
                                              ...
                                              filter));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmfp_filter_conflict_qsets_process(unit,
                                              op_arg,
                                              tbl_id,
                                              stage,
                                              idx,
...
static int
bcmfp_filter_group_sharing_validate(int unit,
                                int tbl_inst,
                                bcmfp_stage_id_t stage_id,
    ...
    for (idx = 0; idx < stage->misc_info->num_groups; idx++)  {
        ...
        if (group_oper_info[idx].tbl_inst == tbl_inst) {
            if (group_oper_info[idx].group_prio == gconfig->priority) {
              /*
               * All shared groups should either auto expandable groups or
               * non auto exxpandable groups.
               */
                if (((group_oper_info[idx].flags & BCMFP_GROUP_WITH_AUTO_EXPAND)
                      && (gconfig->auto_expand == 0)) ||
                   (!(group_oper_info[idx].flags & BCMFP_GROUP_WITH_AUTO_EXPAND)
                      && (gconfig->auto_expand == 1))) {
                    LOG_VERBOSE(BSL_LOG_MODULE,
                       (BSL_META_U(unit,
                       "Group ID = %d and Group ID = %d has same group"
                       " priority, but one of the group is auto expandable"
                       " and other is non auto expandable.\n"),
                       gconfig->group_id, group_oper_info[idx].group_id));
                    filter->entry_state =
                              BCMFP_ENTRY_SHARED_GROUP_AUTOEXPAND_FLAG_MISMATCH;
                    SHR_ERR_EXIT(SHR_E_CONFIG);
               }
                 ...
                 */
                   ...
                   if ((group_oper_info[idx].group_mode == BCMFP_GROUP_MODE_HALF
                       ...
                       LOG_VERBOSE(BSL_LOG_MODULE,
                          ...
                          gconfig->group_id, group_oper_info[idx].group_id));
                       filter->entry_state =
                                    BCMFP_ENTRY_GROUP_MULTIMODE_CHECK_FAILED;
                       SHR_ERR_EXIT(SHR_E_CONFIG);
                   }
                    ...
                    */
                   if ((group_oper_info[idx].group_mode ==
                                ...
                                bcmfp_filter_t *filter)
{
    ...
    if (opinfo->state == BCMFP_GROUP_STATE_CONFLICT_QSETS_LESS) {
        filter->entry_state =
                BCMFP_ENTRY_CONFLICT_QUALIFIER_SETS_LESS_THAN_PRESELS;
        SHR_ERR_EXIT(SHR_E_CONFIG);
    }
    if (opinfo->state == BCMFP_GROUP_STATE_CONFLICT_QSET_NOT_CREATED) {
        filter->entry_state =
                BCMFP_ENTRY_CONFLICT_QUALIFIER_SET_NOT_CREATED;
        SHR_ERR_EXIT(SHR_E_CONFIG);
    }
    if (opinfo->state == BCMFP_GROUP_STATE_CONFLICT_QSET_INVALID) {
        filter->entry_state =
                BCMFP_ENTRY_CONFLICT_QUALIFIER_SET_INVALID;
        SHR_ERR_EXIT(SHR_E_CONFIG);
    }

    if (opinfo->state == BCMFP_GROUP_STATE_NON_COMPRESSIBLE) {
        filter->entry_state =
                BCMFP_ENTRY_PARTIAL_COMPRESS_QUAL_BITMAP_IN_GROUP;
        SHR_ERR_EXIT(SHR_E_CONFIG);
    }
    ...
    if (!(BCMFP_STAGE_FLAGS_TEST(stage,
                                BCMFP_STAGE_ENTRY_TYPE_HASH))) {
        gconfig = filter->group_config;
        rv = bcmfp_filter_group_sharing_validate(unit,
                                             gconfig->pipe_id,
                                             stage_id,
                                             opinfo,
                                             gconfig,
    ...
    /* Validate Flex Ctr profiles mapped to the group */
    if (BCMFP_STAGE_FLAGS_TEST(stage, BCMFP_STAGE_FLEX_CTR_PER_SLICE_SUPPORT)) {
        gconfig = filter->group_config;
        /*
         * For an auto expandable group, flex counter
         * profile Id should be given for all physical slices.
         */
        if (gconfig->auto_expand == true &&
            gconfig->num_ctr_profiles != 0) {

            /* Count of proiles should be equal to stage physical slices */
            if (gconfig->num_ctr_profiles !=
                    stage->misc_info->max_counter_profiles_per_group) {
                filter->entry_state = BCMFP_ENTRY_GROUP_CTR_PROFILES_MISSING;
                SHR_ERR_EXIT(SHR_E_CONFIG);
            } else {

                /* Count of profiles is correct. Check the Id's are non-zero */
                for (i = 0; i < gconfig->num_ctr_profiles; i++) {
                    if (gconfig->ctr_profile_ids[i] == 0) {
                        break;
                    }
                }
                if ( i < gconfig->num_ctr_profiles) {
                    filter->entry_state =
                        BCMFP_ENTRY_GROUP_CTR_PROFILES_MISSING;
                    SHR_ERR_EXIT(SHR_E_CONFIG);
                }
            }
        }
                               ...
                               bcmfp_filter_t *filter)
{
    ...
    rv = bcmfp_filter_rule_compress_qual_validate(unit,
                                                  stage_id,
                                                  filter);
    if (rv == SHR_E_CONFIG) {
        filter->entry_state = BCMFP_ENTRY_NON_LPM_COMPRESS_MASK_IN_RULE;
    }
    SHR_IF_ERR_EXIT(rv);
                                    ...
                                    bcmfp_filter_t *filter)
{
    ...
    /* Validate Flex Ctr profiles mapped to the group */
    if (filter->entry_config->flex_ctr_action != 0 &&
        BCMFP_STAGE_FLAGS_TEST(stage, BCMFP_STAGE_FLEX_CTR_PER_SLICE_SUPPORT)) {
        gconfig = filter->group_config;
        /*
         * For an auto expandable group, flex counter
         * profile Id should be given for all physical slices.
                    ...
                    bcmfp_filter_t *filter)
{
    ...
    /*
     * If the filter has compression keys then, compress the
     * rule in the filter.
     */
    if (filter->compression_keys != NULL) {
                    ...
                    bcmfp_filter_t *filter_curr)
{
    ...
    bcmfp_rule_id_t rule_id_prev = 0;
    ...
    if (filter_prev != NULL) {
       ...
       rule_id_prev = filter_prev->entry_config->rule_id;
    } else {
    ...
    if (filter_prev != NULL) {
        if ((prev_state == BCMFP_ENTRY_SUCCESS) &&
            ...
            (group_id_prev == group_id_curr) &&
            (filter_prev->entry_config->flowtracker_en == 0) &&
            (filter_curr->entry_config->flowtracker_en == 1)) {
            if (filter_prev->group_config == NULL) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmfp_group_remote_config_get(unit,
                                                   op_arg,
                                                   stage->stage_id,
                                                   group_id_prev,
                                                   &group_config));
                filter_prev->group_config = group_config;
            }
            if (filter_prev->rule_config == NULL && rule_id_prev !=0) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmfp_rule_remote_config_get(unit,
                                                  op_arg,
                                                  stage_id,
                                                  rule_id_prev,
                                                  &rule_config));
                filter_prev->rule_config = rule_config;
            }

            filter_prev->flowtracker_entry_delete = 1;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmfp_filter_buffers_allocate(unit,
                                               BCMFP_BUFFERS_CREATE_EKW,
                                               tbl_id,
                                               stage,
                                               &(filter_prev->buffers)));
...
exit:
    ...
    bcmfp_filter_configs_free(unit, filter_prev);
    if (filter_prev != NULL) {
        bcmfp_filter_buffers_free(unit, &(filter_prev->buffers));
    }
...
int
bcmfp_filter_default_presel_update(int unit,
                                   const bcmltd_op_arg_t *op_arg,
                                   bcmfp_stage_id_t stage_id,
                                   bcmfp_filter_t *filter)
{
