/*! \file bcmfp_ptm_tcam.c
 *
 * FP interfacing APIs to RM-TCAM.
 */
/*
...

{
    ...
    rv = bcmfp_ptm_ltm_mreq_keyed_lt(unit,
                                     req_flags,
                                     req_ltid,
                                     pt_dyn_info,
                                     NULL,
    ...
    if (!(BCMFP_STAGE_FLAGS_TEST(stage, BCMFP_STAGE_ENTRY_TYPE_HASH))
       ...
       if (ent_prof != NULL && ent_prof->pdd != NULL &&
           ...
           if ((flags & BCMFP_ENTRY_FIRST_IN_GROUP &&
               ...
               uint8_t pdd_sid_columns = 0;
               pdd_sid_columns = stage->pdd_hw_info->pdd_profile_sid_columns;
               for (part = 0; part < (parts_count * pdd_sid_columns); part++) {
        ...
        if (BCMFP_STAGE_FLAGS_TEST(stage, BCMFP_STAGE_KEY_TYPE_SELCODE) &&
           (flags & BCMFP_ENTRY_FIRST_IN_GROUP)) {
            ...
            if (BCMFP_STAGE_FLAGS_TEST(stage,
                ...
                if (group_config->auto_expand == true &&
                    group_config->num_ctr_profiles != 0) {
                    attrs->grp_ctr_profiles_cnt =
                        group_config->num_ctr_profiles;
                    for (i = 0; i < group_config->num_ctr_profiles; i++) {
                        attrs->grp_ctr_profiles[i] =
                            group_config->ctr_profile_ids[i];
                    }
                }
            }
        }
