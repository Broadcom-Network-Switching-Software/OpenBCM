    ...
    if (stage->tbls.group_part_tbl->qual_fid_info) {
        for (idx = 0; idx < stage->tbls.group_tbl->fid_count; idx++) {
            ...
            if ((field_def.depth != 0) &&
                (idx != stage->tbls.group_tbl->qual_inports_fid) &&
                (idx != stage->tbls.group_tbl->qual_system_ports_fid) &&
                (idx != stage->tbls.group_tbl->qual_svp_ports_fid)
                && (idx != stage->tbls.group_tbl->qual_device_ports_fid)) {
                SHR_IF_ERR_VERBOSE_EXIT
            ...
            if (SHR_SUCCESS(rv)) {
               ...
               /*
                * Fill empty info for count array fields
                * which doesnt need array processing.
                */
                      ...
                      bcmltd_fields_t *out)
{
    ...
    for  (idx = 0; idx < in->count; idx++) {
          if (in->field[idx]->id
              == stage->tbls.pdd_info_tbl->group_key_fid) {
          SHR_IF_ERR_VERBOSE_EXIT(bcmfp_fid_value_u32_get(unit,
                                                in->field[idx],
                       stage->tbls.pdd_info_tbl->group_key_fid,
                                                   &group_id));
    ...
    if (pdd_id > max_pdd_id) {
     ...
     /* check if the given group_id mapped to pdd_id */
    sal_memset(&group_idp_info, 0, sizeof(bcmfp_idp_info_t));
    group_idp_info.group_id = group_id;

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_pdd_group_map_cond_traverse(unit,
                                           stage->stage_id,
                                           pdd_id,
                                           bcmfp_pdd_group_map_id_check,
                                           (void *)(&group_idp_info),
                                           &cond_met));
    ...
    for (idx = 0; idx < stage->tbls.pdd_tbl->fid_count; idx++) {
         ...
         rv = bcmfp_pdd_group_action_offset_info_get(unit,
                                                     stage->stage_id,
                                                     pdd_id,
                                                     (part_idx -1),
                                                     group_id,
                      ...
                      bcmltd_fields_t *out)
{
    ...
    for (idx = 0; idx < BCMFP_QUALIFIERS_COUNT; idx++) {
         ...
         if (NULL == stage->presel_qual_cfg_info_db->qual_cfg_info[idx]) {
             continue;
         }
         qfi = &stage->tbls.pse_info_tbl->qual_fid_info[qualifier];
         ...
         q_cfg = &stage->presel_qual_cfg_info_db->qual_cfg_info[idx]->qual_cfg_arr[0];
         if (q_cfg->num_chunks > FP_INFO_MAX) {
            SHR_ERR_EXIT(SHR_E_INTERNAL);
         }
         ...
         for (offset_idx = 0; offset_idx < q_cfg->num_chunks;
              offset_idx++) {
             SHR_IF_ERR_VERBOSE_EXIT
                 (bcmfp_cth_out_fields_set(unit, &out_idx, out,
                                           qfi->offset_fid,
                                           q_cfg->e_params[offset_idx].bus_offset,
                                           offset_idx));
         }
         ...
         for (offset_idx = 0; offset_idx < q_cfg->num_chunks;
             ...
             SHR_IF_ERR_VERBOSE_EXIT
                 (bcmfp_cth_out_fields_set(unit, &out_idx, out,
                                           qfi->width_fid,
                                           q_cfg->e_params[offset_idx].width,
                                           offset_idx));
         }
                         ...
                         const bcmltd_generic_arg_t *arg)
{
    ...
    sal_memset(&insert, 0, sizeof(bcmltd_fields_t));
    BCMFP_ALLOC(insert.field, 3 * sizeof(bcmltd_field_t *),
    ...
    /*
     * find the first operational group_id
     * which is mapped to a pdd_id
     */
    for (group_idx = 1; group_idx <= max_group_id; group_idx++) {
        opinfo = NULL;
        SHR_IF_ERR_EXIT
        (bcmfp_group_oper_info_get(unit,
                                   stage->stage_id,
        ...
        if ((opinfo->pdd_type != BCMFP_PDD_OPER_TYPE_NONE) &&
    ...
    if (found) {
        /*
         * Find the first operational PDD ID
         * with the given group_id
         */
        sal_memset(&group_idp_info, 0, sizeof(bcmfp_idp_info_t));

        /*
         * This function traverses the group_ids mapped to the pdd_id
         * and returns the first operational group_id
         */
        ...
        insert.field[2] = shr_fmm_alloc();
        if (insert.field[2] == NULL) {
            LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "No memory for "
                "KEY field in BCMFP IMM update\n")));
            SHR_ERR_EXIT(SHR_E_MEMORY);
            ...
            insert.count += 1;
            sal_memset(insert.field[2], 0, size);
            insert.field[2]->id = stage->tbls.pdd_info_tbl->group_key_fid;
            insert.field[2]->data = group_id;
        }
                        ...
                        const bcmltd_generic_arg_t *arg)
{
    ...
    uint32_t idx = 0;
    uint32_t group_idx = 0;
    uint32_t curr_group_id = 0;
    uint32_t final_group_id = 0;
    bcmfp_stage_t *stage = NULL;
    bcmltd_fields_t insert;
    size_t size = 0;
    ...
    if (part_idx < pdd_num_parts) {
        final_part = part_idx + 1;
        final_pdd  = pdd_id;
        final_group_id = curr_group_id;
        found = TRUE;
    } else {
        sal_memset(&group_idp_info, 0, sizeof(bcmfp_idp_info_t));
        /* find the next valid pdd_id mapped to the given curr_group_id */
        group_idp_info.group_id = curr_group_id;
    ...
    /*
     * if all the pdd_ids mapped to the curr_group_id are exhausted
     * find the next valid pdd_id and group_id
     */
    if (found == FALSE) {
        for (group_idx = curr_group_id + 1; group_idx <= max_group_id;
                group_idx++) {
            opinfo = NULL;
            SHR_IF_ERR_EXIT
                (bcmfp_group_oper_info_get(unit,
        ...
        if (found) {
            /* find the first pdd_id mapped to the group */
            sal_memset(&group_idp_info, 0, sizeof(bcmfp_idp_info_t));
            group_idp_info.group_id = final_group_id;
            for (idx = 1; idx <= max_pdd_id; idx++) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmfp_pdd_group_map_cond_traverse(unit,
                                                   stage_id,
    ...
    if (found) {
        ...
        if (insert.field[2] == NULL) {
            ...
            sal_memset(insert.field[2], 0, size);
            insert.field[2]->id = stage->tbls.pdd_info_tbl->group_key_fid;
            insert.field[2]->data = final_group_id;
        }
                         ...
                         uint64_t ev_data)
{
    ...
    for (stage_id = 0;
        ...
        if (stage->tbls.pse_info_tbl != NULL) {
            if (stage->tbls.pse_info_tbl->sid == info_sid) {
                pse_info_get = TRUE;
                break;
            }
        }
                              ...
                              const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_stage_max_entry_id_get(unit,
                                      stage_id,
                                      &max_entry_id));

    /* find first valid entry */
    for (idx = 1; idx <= max_entry_id; idx++) {
        entry_config = NULL;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_entry_remote_config_get(unit,
                                           op_arg,
                             ...
                             const bcmltd_generic_arg_t *arg)
{
    ...
    bcmltd_fields_t insert;
    size_t size = 0;
    uint32_t curr_entry_id = 0;
    uint32_t next_entry_id = 0;
    uint32_t max_entry_id = 0;
    uint32_t idx = 0;
    bcmfp_entry_config_t *entry_config = NULL;
    bool found = FALSE;
    ...
    for (idx = curr_entry_id +1 ; idx <= max_entry_id; idx++) {
        entry_config = NULL;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_entry_remote_config_get(unit,
                                           op_arg,
    ...
    if (found == TRUE) {
        ...
        if (insert.field[0] == NULL) {
            ...
            sal_memset(insert.field[0], 0, size);
            insert.field[0]->id = 0;
            insert.field[0]->data = next_entry_id;
        }
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_entry_info_lookup(unit,
                                     op_arg,
