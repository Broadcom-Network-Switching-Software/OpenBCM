                                 ...
                                 bcmfp_rule_config_t *config)
{
    ...
    bcmlrd_fid_t svp_ports_key_fid = 0;
    bcmlrd_fid_t svp_ports_mask_fid = 0;
    ...
    *qd = NULL;
    ...
    svp_ports_key_fid = stage->tbls.rule_tbl->svp_ports_key_fid;
    svp_ports_mask_fid = stage->tbls.rule_tbl->svp_ports_mask_fid;
    while (buffer != NULL) {
        qual_fid_info =
        ...
        if ((buffer->id == inports_key_fid) ||
            ...
            } else if (buffer->id == svp_ports_key_fid) {
                config->pbmp_type = BCMFP_SVP_PORTS;
                config->pbmp_qual_id =
                        stage->tbls.group_tbl->qual_svp_ports_fid;
            }
        ...
        if ((buffer->id == inports_mask_fid) ||
            ...
            if (buffer->id == inports_mask_fid)  {
                ...
                config->pbmp_qual_id =
                        stage->tbls.group_tbl->qual_device_ports_fid;
            } else if (buffer->id == svp_ports_mask_fid) {
                config->pbmp_type = BCMFP_SVP_PORTS;
                config->pbmp_qual_id =
                        stage->tbls.group_tbl->qual_svp_ports_fid;
            }
                             ...
                             bcmfp_rule_compress_key_t **ckeys)
{
    ...
    for (idx1 = 0; idx1 < BCMFP_COMPRESS_TYPES_MAX; idx1++) {
        ...
        for (idx2 = 0; idx2 < cfid_info->num_fids; idx2++) {
            ...
            if (qual_data != NULL) {
                ...
                SHR_BITCOPY_RANGE(ckey->mask,
                                  ...
                                  src_sbit,
                                  width);
                SHR_BITCOPY_RANGE(ckey->umask,
                                  dst_sbit,
                                  qual_data->mask,
                                  src_sbit,
                                  width);
            } else {
                if (cfid_info->maskable != NULL) {
                    if (cfid_info->maskable[idx2] == TRUE) {
                        SHR_BITCLR_RANGE(ckey->key,
                                         dst_sbit,
                                         width);
                        SHR_BITSET_RANGE(ckey->mask,
                                         dst_sbit,
                                         width);
                        SHR_BITCLR_RANGE(ckey->umask,
                                         dst_sbit,
                                         width);
                    }
                }
        ...
        ckey->req_ltid = stage->tbls.compress_alpm_tbl[idx1]->sid;
