...
static int
bcmfp_group_conflict_qset_ids_get(int unit,
                        bcmfp_stage_t *stage,
                        bcmltd_field_t *buffer,
                        uint8_t num_presels,
                        uint32_t **conflict_qset_ids)
{
    uint32_t conflict_qset_ids_fid = 0;
    uint64_t conflict_qset_id_u64 = 0;
    uint32_t *conflict_qset_ids_u32 = NULL;
    uint8_t index = 0;
    ...
    SHR_NULL_CHECK(stage, SHR_E_PARAM);
    SHR_NULL_CHECK(buffer, SHR_E_PARAM);
    SHR_NULL_CHECK(conflict_qset_ids, SHR_E_PARAM);

    conflict_qset_ids_fid = stage->tbls.group_tbl->conflict_qset_ids_fid;
    *conflict_qset_ids = NULL;

    if (num_presels == 0) {
        SHR_EXIT();
    }

    if (conflict_qset_ids_fid != 0) {
        BCMFP_ALLOC(conflict_qset_ids_u32,
                    sizeof(uint32_t) * num_presels,
                    "bcmfpGroupConflictQsetIdArray");

        for (index = 0; index < num_presels; index++) {
            conflict_qset_id_u64 = 0;
            SHR_IF_ERR_EXIT_EXCEPT_IF
                (bcmfp_fid_array_value_get(unit,
                                           conflict_qset_ids_fid,
                                           index,
                                           (void *)buffer,
                                           &conflict_qset_id_u64),
                                           SHR_E_NOT_FOUND);
            conflict_qset_ids_u32[index] = conflict_qset_id_u64 & 0xFFFFFFFF;
       }
       for (index = 0; index < num_presels; index++) {
           if (conflict_qset_ids_u32[index] != 0) {
               *conflict_qset_ids = conflict_qset_ids_u32;
               break;
           }
       }

       /* Free the memory created if there are no conflict qset IDs. */
       if (*conflict_qset_ids == NULL) {
           SHR_FREE(conflict_qset_ids_u32);
       }
    }
...
static int
bcmfp_group_ctr_profile_params_get(int unit,
                        bcmfp_stage_t *stage,
                        bcmltd_field_t *buffer,
                        uint8_t *num_ctr_profiles,
                        uint32_t **ctr_profile_ids)
{
    uint32_t num_ctr_prof_fid = 0;
    uint64_t num_ctr_prof_u64 = 0;
    uint32_t ctr_prof_ids_fid = 0;
    uint64_t ctr_prof_id_u64 = 0;
    uint32_t *ctr_prof_ids_u32 = NULL;
    uint8_t index = 0;
    ...
    SHR_NULL_CHECK(stage, SHR_E_PARAM);
    SHR_NULL_CHECK(buffer, SHR_E_PARAM);
    SHR_NULL_CHECK(num_ctr_profiles, SHR_E_PARAM);
    SHR_NULL_CHECK(ctr_profile_ids, SHR_E_PARAM);

    num_ctr_prof_fid = stage->tbls.group_tbl->ctr_prof_ids_count_fid;
    ctr_prof_ids_fid = stage->tbls.group_tbl->ctr_prof_ids_fid;

    if ((num_ctr_prof_fid != 0) && (ctr_prof_ids_fid != 0)){
        SHR_IF_ERR_EXIT_EXCEPT_IF
            (bcmfp_fid_value_get(unit,
                                 num_ctr_prof_fid,
                                 (void *)buffer,
                                 &num_ctr_prof_u64),
             SHR_E_NOT_FOUND);
        *num_ctr_profiles =  num_ctr_prof_u64 & 0xFFFFFFFF;
        if (num_ctr_prof_u64) {
            BCMFP_ALLOC(ctr_prof_ids_u32,
                    sizeof(uint32_t) * num_ctr_prof_u64,
                    "bcmfpGroupCtrProfileIdArray");
        }

        for (index = 0; index < num_ctr_prof_u64; index++) {
            ctr_prof_id_u64 = 0;
            SHR_IF_ERR_EXIT_EXCEPT_IF
                (bcmfp_fid_array_value_get(unit,
                                           ctr_prof_ids_fid,
                                           ...
                                           &ctr_prof_id_u64),
                    SHR_E_NOT_FOUND);
            ctr_prof_ids_u32[index] = ctr_prof_id_u64 & 0xFFFFFFFF;
       }
       *ctr_profile_ids = ctr_prof_ids_u32;
   }

exit:
    if (SHR_FUNC_ERR()) {
        SHR_FREE(ctr_prof_ids_u32);
    }
    SHR_FUNC_EXIT();
}
                        ...
                        bcmfp_qual_bitmap_t **qb)
{
    ...
    uint32_t qual_inports_fid = 0;
    uint32_t qual_system_ports_fid = 0;
    uint32_t qual_svp_ports_fid = 0;
    uint32_t qual_device_ports_fid = 0;
    ...
    if (tbl_id == stage->tbls.group_tbl->sid) {
        qualifiers_fid_info =
            stage->tbls.group_tbl->qualifiers_fid_info;
        qual_inports_fid = stage->tbls.group_tbl->qual_inports_fid;
        qual_system_ports_fid = stage->tbls.group_tbl->qual_system_ports_fid;
        qual_svp_ports_fid = stage->tbls.group_tbl->qual_svp_ports_fid;
        qual_device_ports_fid = stage->tbls.group_tbl->qual_device_ports_fid;
    } else {
        qualifiers_fid_info =
            stage->tbls.group_template_action_tbl->qualifiers_fid_info;
        qual_inports_fid =
            stage->tbls.group_template_action_tbl->qual_inports_fid;
        qual_system_ports_fid =
            stage->tbls.group_template_action_tbl->qual_system_ports_fid;
        qual_svp_ports_fid =
            stage->tbls.group_template_action_tbl->qual_svp_ports_fid;
        qual_device_ports_fid =
    ...
    while (buffer != NULL) {
        ...
        SHR_IF_ERR_EXIT
            (bcmlrd_table_field_def_get(unit,
                                        ...
                                        buffer->id,
                                        &field_def));
        if ((buffer->id == qual_inports_fid) ||
            (buffer->id == qual_system_ports_fid) ||
            (buffer->id == qual_svp_ports_fid) ||
            (buffer->id == qual_device_ports_fid)) {
            pbmp_type_qual = TRUE;
        } else {
                       ...
                       bcmfp_group_config_t **config)
{
    ...
    uint32_t *conflict_qset_ids = NULL;
    ...
    /* Default enable */
    bool auto_expand = 1;
    uint32_t *ctr_prof_ids = NULL;
    uint8_t num_ctr_profs = 0;
    ...
    if (stage->tbls.group_tbl == NULL ||
        !(stage->tbls.group_tbl->sid == tbl_id ||
        stage->tbls.group_template_action_tbl->sid == tbl_id)) {
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    }
    ...
    *config = group_config;
    ...
    if (stage->tbls.group_tbl->sid == tbl_id) {
       ...
       /* Get conflict QSET IDs. */
       if (stage->tbls.group_tbl->conflict_qset_ids_fid != 0) {
           buffer = (bcmltd_field_t *)data;
           SHR_IF_ERR_EXIT
               (bcmfp_group_conflict_qset_ids_get(unit,
                                              stage,
                                              buffer,
                                              num_presels,
                                              &conflict_qset_ids));
           group_config->conflict_qset_ids = conflict_qset_ids;
       }
       ...
       /* Get attached counter profiles count and IDs. */
       if (stage->tbls.group_tbl->ctr_prof_ids_count_fid != 0 &&
           stage->tbls.group_tbl->ctr_prof_ids_fid != 0) {
           buffer = (bcmltd_field_t *)data;
           SHR_IF_ERR_EXIT
               (bcmfp_group_ctr_profile_params_get(unit,
                                              stage,
                                              buffer,
                                              &num_ctr_profs,
                                              &ctr_prof_ids));
           group_config->num_ctr_profiles = num_ctr_profs;
           group_config->ctr_profile_ids = ctr_prof_ids;
       }
                        ...
                        bcmfp_group_config_t *group_config)
{
    ...
    if (group_config->conflict_qset_ids) {
        SHR_FREE(group_config->conflict_qset_ids);
    }
    if (group_config->ctr_profile_ids) {
        SHR_FREE(group_config->ctr_profile_ids);
    }
...
static int
bcmfp_group_keygen_grpmode_check(int unit,
                                 bcmfp_stage_id_t stage_id,
                                 bcmfp_group_config_t *gc,
                                 bcmfp_group_oper_info_t *opinfo,
                                 bcmfp_group_mode_t group_mode,
                                 bool *group_mode_valid)
{
    ...
    if (group_mode == BCMFP_GROUP_MODE_AUTO) {
        *group_mode_valid = FALSE;
        SHR_EXIT();
    }

    /*
     * Disable verifying DBLINTRA if the group mode
     * is auto, if the stage has DISABLE_ISDW_IN_MODE_AUTO
     * set.
     */
    if (BCMFP_STAGE_FLAGS_TEST(stage,
        BCMFP_STAGE_DISABLE_ISDW_IN_MODE_AUTO) &&
        group_mode == BCMFP_GROUP_MODE_DBLINTRA &&
        gc->mode == BCMFP_GROUP_MODE_AUTO) {
        *group_mode_valid = FALSE;
        SHR_EXIT();
    }
    /*
    ...
    if ((pdd_group_mode == BCMFP_GROUP_MODE_DBLINTER) &&
        ((group_mode == BCMFP_GROUP_MODE_SINGLE_ASET_NARROW) ||
...
static int
bcmfp_group_ext_ctrl_info_set_level1(int unit,
                                     bcmfp_keygen_cfg_t *keygen_cfg,
                                     bcmfp_ext_codes_t *ext_codes,
                                     uint16_t *num_ext_codes)
{
    uint8_t part = 0;
    uint8_t gran = 0;
    uint8_t level = 0;
    ...
    for (part = 0; part < BCMFP_ENTRY_PARTS_MAX; part++) {
        grp_ext_code = &(ext_codes[part]);
        level = 1;
        ctrl_sel = BCMFP_KEYGEN_EXT_CTRL_SEL_MULTIPLEXER;
        for (ext_num = 0; ext_num < BCMFP_MAX_EXTRACTORS_PER_LEVEL; ext_num++) {
            gran = 32;
            if (grp_ext_code->l1_e32_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l1_e32_sel[ext_num];
                BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                                   keygen_cfg,
                                                   part,
                                                   gran,
                                                   level,
            ...
            gran = 16;
            if (grp_ext_code->l1_e16_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l1_e16_sel[ext_num];
            ...
            gran = 8;
            if (grp_ext_code->l1_e8_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l1_e8_sel[ext_num];
            ...
            gran = 4;
            if (grp_ext_code->l1_e4_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l1_e4_sel[ext_num];
            ...
            gran = 2;
            if (grp_ext_code->l1_e2_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l1_e2_sel[ext_num];
...
static int
bcmfp_group_ext_ctrl_info_set_level2(int unit,
                                     bcmfp_keygen_cfg_t *keygen_cfg,
                                     bcmfp_ext_codes_t *ext_codes,
                                     uint16_t *num_ext_codes)
{
    ...
    for (part = 0; part < BCMFP_ENTRY_PARTS_MAX; part++) {
        ...
        for (ext_num = 0; ext_num < BCMFP_MAX_EXTRACTORS_PER_LEVEL; ext_num++) {
            gran = 16;
            if (grp_ext_code->l2_e16_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l2_e16_sel[ext_num];
            ...
            gran = 4;
            if (grp_ext_code->l2_e4_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l2_e4_sel[ext_num];
            ...
            gran = 2;
            if (grp_ext_code->l2_e2_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l2_e2_sel[ext_num];
            ...
            gran = 1;
            if (grp_ext_code->l2_e1_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l2_e1_sel[ext_num];
...
static int
bcmfp_group_ext_ctrl_info_set_level3(int unit,
                                     bcmfp_keygen_cfg_t *keygen_cfg,
                                     bcmfp_ext_codes_t *ext_codes,
                                     uint16_t *num_ext_codes)
{
    ...
    for (part = 0; part < BCMFP_ENTRY_PARTS_MAX; part++) {
        ...
        for (ext_num = 0; ext_num < BCMFP_MAX_EXTRACTORS_PER_LEVEL; ext_num++) {
            gran = 4;
            if (grp_ext_code->l3_e4_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l3_e4_sel[ext_num];
            ...
            gran = 2;
            if (grp_ext_code->l3_e2_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l3_e2_sel[ext_num];
            ...
            gran = 1;
            if (grp_ext_code->l3_e1_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                ctrl_sel_val = grp_ext_code->l3_e1_sel[ext_num];
...
static int
bcmfp_group_ext_ctrl_grp_sel_set(int unit,
                                 bcmfp_keygen_cfg_t *keygen_cfg,
                                 bcmfp_ext_codes_t *ext_codes,
                                 uint16_t *num_ext_codes)
{
    ...
    for (part = 0; part < BCMFP_ENTRY_PARTS_MAX; part++) {
        grp_ext_code = &(ext_codes[part]);
        if (grp_ext_code->aux_tag_a_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_AUX_TAG_A,
                                               grp_ext_code->aux_tag_a_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->aux_tag_b_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_AUX_TAG_B,
                                               grp_ext_code->aux_tag_b_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->aux_tag_c_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_AUX_TAG_C,
                                               grp_ext_code->aux_tag_c_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->aux_tag_d_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_AUX_TAG_D,
                                               grp_ext_code->aux_tag_d_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->tcp_fn_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_TCP_FN,
                                               grp_ext_code->tcp_fn_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->tos_fn_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_TOS_FN,
                                               grp_ext_code->tos_fn_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->ttl_fn_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_TTL_FN,
                                               grp_ext_code->ttl_fn_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->class_id_cont_a_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_CLASS_ID_A,
                                               grp_ext_code->class_id_cont_a_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->class_id_cont_b_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_CLASS_ID_B,
                                               grp_ext_code->class_id_cont_b_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->class_id_cont_c_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_CLASS_ID_C,
                                               grp_ext_code->class_id_cont_c_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->class_id_cont_d_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_CLASS_ID_D,
                                               grp_ext_code->class_id_cont_d_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->src_cont_a_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_SRC_A,
                                               grp_ext_code->src_cont_a_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->src_cont_b_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_SRC_B,
                                               grp_ext_code->src_cont_b_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->dst_cont_a_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_DST_A,
                                               grp_ext_code->dst_cont_a_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->dst_cont_b_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_DST_B,
                                               grp_ext_code->dst_cont_b_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->src_dest_cont_0_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_SRC_DST_0,
                                               grp_ext_code->src_dest_cont_0_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->src_dest_cont_1_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_SRC_DST_1,
                                               grp_ext_code->src_dest_cont_1_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->ipbm_present == 1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               grp_ext_code->ipbm_source,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->normalize_l3_l4_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_L3_L4_NORM,
                                               grp_ext_code->normalize_l3_l4_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->normalize_mac_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_MAC_NORM,
                                               grp_ext_code->normalize_mac_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->udf_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_UDF,
                                               grp_ext_code->udf_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }
        if (grp_ext_code->alt_ttl_fn_sel != -1) {
            ext_code_idx = *num_ext_codes;
            BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                               keygen_cfg,
                                               part,
                                               ...
                                               level,
                                               ext_num,
                                               BCMFP_KEYGEN_EXT_CTRL_SEL_ALT_TTL_FN,
                                               grp_ext_code->alt_ttl_fn_sel,
                                               ext_code_idx);
            *num_ext_codes+=1;
        }

        for (ext_num = 0; ext_num < BCMFP_MAX_EXTRACTORS_PER_LEVEL;
                ext_num++) {
            if (grp_ext_code->pmux_sel[ext_num] != -1) {
                ext_code_idx = *num_ext_codes;
                BCMFP_KEYGEN_EXT_CTRL_SEL_INFO_SET(unit,
                                                keygen_cfg,
                                                part,
...
static int
bcmfp_group_ext_ctrl_info_set(int unit,
                              bcmfp_keygen_cfg_t *keygen_cfg,
                              bcmfp_ext_codes_t *ext_codes)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_ext_ctrl_info_set_level1(unit,
                                              keygen_cfg,
                                              ext_codes,
                                              &num_ext_codes));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_ext_ctrl_info_set_level2(unit,
                                              keygen_cfg,
                                              ext_codes,
                                              &num_ext_codes));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_ext_ctrl_info_set_level3(unit,
                                               keygen_cfg,
                                               ext_codes,
                                               &num_ext_codes));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_ext_ctrl_grp_sel_set(unit,
                                          keygen_cfg,
                                          ext_codes,
                                          &num_ext_codes));
    if (keygen_cfg->ext_ctrl_sel_info_count != num_ext_codes) {
        SHR_ERR_EXIT(SHR_E_INTERNAL);
    }
exit:
    SHR_FUNC_EXIT();
...
static int
bcmfp_group_ext_ctrl_count_get(int unit,
                               bcmfp_keygen_cfg_t *keygen_cfg,
                               bcmfp_ext_codes_t *ext_codes)
{
    uint8_t ext_num = 0;
    ...
    /* Fetch the level1 ext_codes count */
    for (part = 0; part < BCMFP_ENTRY_PARTS_MAX; part++) {
        grp_ext_code = &(ext_codes[part]);
        for (ext_num = 0; ext_num < BCMFP_MAX_EXTRACTORS_PER_LEVEL; ext_num++) {
            if (grp_ext_code->l1_e32_sel[ext_num] != -1) {
    ...
    keygen_cfg->ext_ctrl_sel_info_count = num_ext_codes;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT();
}
...
static int
bcmfp_group_keygen_cfg_build(int unit,
                             bcmfp_stage_id_t stage_id,
                             bcmfp_group_config_t *gc,
                             bcmfp_group_mode_t group_mode,
                             bcmfp_group_oper_info_t *opinfo,
                             uint32_t keygen_flags,
                             ...
                             bcmfp_keygen_cfg_t *keygen_cfg)
{
    ...
    if (group_mode == BCMFP_GROUP_MODE_DBLINTRA ) {
            keygen_cfg->flags = keygen_cfg->flags |
                               BCMFP_KEYGEN_CFG_FLAGS_SINGLE_PRI_SEL;
    } else {
            keygen_cfg->flags = keygen_cfg->flags &
                               ~BCMFP_KEYGEN_CFG_FLAGS_SINGLE_PRI_SEL;
    }
    ...
    if (keygen_cfg->flags & BCMFP_KEYGEN_CFG_FLAGS_QSET_UPDATE) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_group_ext_ctrl_count_get(unit,
                                            keygen_cfg,
                                            opinfo->ext_codes));
        num_ext_codes = keygen_cfg->ext_ctrl_sel_info_count;
        if (num_ext_codes) {
            BCMFP_ALLOC(keygen_cfg->ext_ctrl_sel_info,
                    sizeof(bcmfp_keygen_ext_ctrl_sel_info_t) * num_ext_codes,
                    "bcmfpKeyGenCfgExtCtrlSelInfo");
        }
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_group_ext_ctrl_info_set(unit,
                                           keygen_cfg,
                                           opinfo->ext_codes));
    }
...
static int
bcmfp_group_keygen_cfg_process(int unit,
                               const bcmltd_op_arg_t *op_arg,
                               bcmfp_stage_id_t stage_id,
                               bcmfp_group_config_t *gc,
                               bcmfp_group_oper_info_t *opinfo,
                               ...
                               uint32_t *group_flags)
{
    ...
    for (group_mode = min_mode;
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_group_keygen_grpmode_check(unit,
                                              stage_id,
                                              gc,
                                              opinfo,
                                              group_mode,
                                              &group_mode_valid));
        if (group_mode_valid == FALSE) {
            continue;
        }
        ...
        *group_flags = 0;
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_group_keygen_cfg_build(unit,
                                          stage_id,
                                          gc,
                                          group_mode,
                                          opinfo,
        ...
        if (pdd_parts > keygen_cfg->num_parts) {
            rv = SHR_E_CONFIG;
            continue;
        }

        SHR_IF_ERR_EXIT
            (bcmfp_keygen_qset_size_get(unit,
                                        keygen_cfg,
                                        &qset_size));
...
static int
bcmfp_keygen_oper_save(int unit,
                       bcmfp_group_t *fg,
                       uint8_t num_parts,
                       bcmfp_keygen_oper_t *keygen_oper,
                       bool update_opinfo)
{
    ...
    if (update_opinfo == FALSE) {
        SHR_IF_ERR_EXIT
            (bcmfp_group_oper_info_set(unit, fg));
    } else {
        SHR_IF_ERR_EXIT
            (bcmfp_group_oper_info_update(unit, fg));
    }
exit:
    SHR_FUNC_EXIT();
...
/*
 * This is similar to bcmfp_group_keygen_process_v1 except
 * in following things.
 * 1. keygen_cfg, keygen_oper and fg are passed as parameters.
 *    The memory for these variables are created by calling
 *    function.
 * Note: This is used when the calling function the info in
 * keygen_oper and fg.
 */
static int
bcmfp_group_pqset_keygen_process(int unit,
                                 const bcmltd_op_arg_t *op_arg,
                                 bcmltd_sid_t tbl_id,
                                 bcmfp_stage_id_t stage_id,
                                 bcmfp_group_config_t *gc,
                                 ...
                                 bcmfp_keygen_oper_t *keygen_oper)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_keygen_cfg_process(unit,
                                        op_arg,
                                        stage_id,
                                        gc,
                                        opinfo,
     ...
     */
    if (pdd_parts != 0 &&
        pdd_parts > keygen_cfg.num_parts &&
        pdd_config) {
        opinfo->aset_error = TRUE;
    }
...
static int
bcmfp_group_keygen_insert_process(int unit,
                                  const bcmltd_op_arg_t *op_arg,
                                  bcmltd_sid_t tbl_id,
                                  bcmfp_stage_id_t stage_id,
                                  bcmfp_group_config_t *gc,
...
static int
bcmfp_group_selcode_insert_process(int unit,
                                   const bcmltd_op_arg_t *op_arg,
                                   bcmltd_sid_t tbl_id,
                                   bcmfp_stage_id_t stage_id,
                                   bcmfp_group_config_t *gc)
...
static int
bcmfp_group_ext_codes_unmark_unused(int unit,
                                    bcmfp_stage_id_t stage_id,
                                    bcmfp_group_oper_info_t *opinfo,
                                    bcmfp_ext_codes_t *ext_codes)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmfp_group_keygen_modes_get(unit,
                                      ...
                                      &kg_mode,
                                      &kge_mode));
    ext_cfg_db = stage->kgn_ext_cfg_db_arr[kge_mode];

    /* Mark all extractors in use in the final level of all parts. */
    for (level = 0; level < BCMFP_KEYGEN_EXT_LEVEL_COUNT; level++) {
        if (ext_cfg_db->conf_size[level] == 0) {
            continue;
        }
        for (idx1 = 0; idx1 < ext_cfg_db->conf_size[level]; idx1++) {
            ext_cfg = ext_cfg_db->ext_cfg[level] + idx1;
            if (!(ext_cfg->out_sec[0] == BCMFP_KEYGEN_EXT_SECTION_FK ||
                  ext_cfg->out_sec[0] == BCMFP_KEYGEN_EXT_SECTION_FKA ||
                  ext_cfg->out_sec[0] == BCMFP_KEYGEN_EXT_SECTION_FKB ||
                  ext_cfg->out_sec[0] == BCMFP_KEYGEN_EXT_SECTION_FKC)) {
               continue;
            }
            BCMFP_KEYGEN_EXT_ID_PARSE(ext_cfg->ext_id, p, l, g, e_num);
            if (l == 1) {
                switch (g) {
                    case 32:
                        if (ext_codes[p].l1_e32_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l1_e32_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 16:
                        if (ext_codes[p].l1_e16_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l1_e16_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 8:
                        if (ext_codes[p].l1_e8_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l1_e8_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 4:
                        if (ext_codes[p].l1_e4_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l1_e4_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 2:
                        if (ext_codes[p].l1_e2_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l1_e2_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    default:
                ...
                switch (g) {
                    case 16:
                        if (ext_codes[p].l2_e16_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l2_e16_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 4:
                        if (ext_codes[p].l2_e4_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l2_e4_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 2:
                        if (ext_codes[p].l2_e2_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l2_e2_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 1:
                        if (ext_codes[p].l2_e1_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l2_e1_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    default:
                ...
                switch (g) {
                    case 4:
                        if (ext_codes[p].l3_e4_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l3_e4_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 2:
                        if (ext_codes[p].l3_e2_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l3_e2_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    case 1:
                        if (ext_codes[p].l3_e1_sel[e_num] ==
                            BCMFP_GROUP_EXT_IN_USE) {
                            ext_codes[p].l3_e1_sel[e_num] =
                                         BCMFP_GROUP_EXT_NOT_IN_USE;
                        }
                        break;
                    default:
...
static int
bcmfp_group_ext_codes_mark_unused(int unit,
                                  bcmfp_stage_id_t stage_id,
                                  bcmfp_group_oper_info_t *opinfo,
                                  bcmfp_ext_codes_t *ext_codes)
{
...
static int
bcmfp_group_conflict_qual_ext_codes_update(int unit,
                                    bcmfp_qualifier_t qual_id1,
                                    bcmfp_keygen_oper_t *keygen_oper,
                                    bcmfp_ext_codes_t *ext_codes)
{
    bcmfp_keygen_ext_ctrl_sel_info_t *ext_ctrl = NULL;
    uint16_t idx1 = 0;
    uint16_t idx2 = 0;
    uint8_t p = 0, l = 0, g = 0, e_num = 0;
    ...
    /*
     * Clear all the extrators in all levels of all parts used
     * by qual_id1.
     */
    for (idx1 = 0; idx1 < keygen_oper->ext_ctrl_sel_info_count; idx1++) {
        ext_ctrl = &(keygen_oper->ext_ctrl_sel_info[idx1]);
        for (idx2 = 0; idx2 < ext_ctrl->qual_id_count; idx2++) {
            if (ext_ctrl->qual_id[idx2] == qual_id1) {
                p = ext_ctrl->part;
                l = ext_ctrl->level;
                g = ext_ctrl->gran;
                e_num = ext_ctrl->ext_num;
                if (l == 1) {
                    switch (g) {
                        case 32:
                            ext_codes[p].l1_e32_sel[e_num] =
...
static int
bcmfp_group_keygen_update_process(int unit,
                                  const bcmltd_op_arg_t *op_arg,
                                  bcmltd_sid_t tbl_id,
                                  bcmfp_stage_id_t stage_id,
                                  bcmfp_group_config_t *gc,
                                  bcmfp_group_oper_info_t *opinfo,
                                  bool qset_update_staging)
{
    bcmfp_stage_t *stage = NULL;
    bcmfp_keygen_cfg_t keygen_cfg;
    bcmfp_keygen_oper_t keygen_oper;
    ...
    keygen_flags = BCMFP_KEYGEN_CFG_FLAGS_QSET_UPDATE;
    ...
    if (qset_update_staging == TRUE) {
        ...
        SHR_IF_ERR_EXIT
            (bcmfp_group_oper_get(unit, gc, &keygen_oper, fg));
        fg->ltid = tbl_id;
        fg->action_res_id = opinfo->action_res_id;
        for (part = 0; part < BCMFP_ENTRY_PARTS_MAX; part++) {
            fg->ext_codes[part].ltid = opinfo->action_res_id;
            fg->ext_codes[part].pdd_prof_index =
                            opinfo->ext_codes[part].pdd_prof_index;
            fg->ext_codes[part].sbr_prof_index =
                            opinfo->ext_codes[part].sbr_prof_index;
            fg->ext_codes[part].psg_prof_index =
                            opinfo->ext_codes[part].psg_prof_index;
            fg->ext_codes[part].qos_prof_index =
                            opinfo->ext_codes[part].qos_prof_index;
            fg->ext_codes[part].default_sbr_prof_index =
                            opinfo->ext_codes[part].default_sbr_prof_index;
            fg->ext_codes[part].default_pdd_prof_index =
                            opinfo->ext_codes[part].default_pdd_prof_index;
            fg->ext_codes[part].default_qos_prof_index =
                            opinfo->ext_codes[part].default_qos_prof_index;
            fg->ext_codes[part].default_policy_enable =
                            opinfo->ext_codes[part].default_policy_enable;
        }
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_keygen_oper_save(unit,
                                    fg,
         ...
         * mode is more than qset mode provided in group template.
         */
        if (pdd_parts != 0 && pdd_parts > keygen_cfg.num_parts && pdd_config) {
            opinfo->aset_error = TRUE;
        }
    }
exit:
...
static int
bcmfp_group_qset_update_process(int unit,
                                const bcmltd_op_arg_t *op_arg,
                                bcmltd_sid_t tbl_id,
                                bcmfp_stage_id_t stage_id,
                                bcmfp_group_config_t *gc,
                                ...
                                bool qset_update_staging)
{
    ...
    if (BCMFP_STAGE_FLAGS_TEST(stage,
        ...
        SHR_IF_ERR_EXIT
            (bcmfp_group_keygen_update_process(unit,
                                               op_arg,
                                               tbl_id,
                                               stage_id,
                                               gc,
...
static int
bcmfp_group_conflict_qsets_process(int unit,
                           const bcmltd_op_arg_t *op_arg,
                           bcmltd_sid_t tbl_id,
                           bcmfp_stage_id_t stage_id,
                           bcmfp_group_config_t *gc,
                           bcmfp_group_oper_info_t *opinfo)
{
    ...
    bcmfp_conflict_qset_config_t **cqset_configs = NULL;
    bcmfp_conflict_qset_config_t *cqset_config = NULL;
    bcmfp_keygen_cfg_t keygen_cfg;
    bcmfp_keygen_oper_t *keygen_oper_pri = NULL;
    bcmfp_keygen_oper_t *keygen_oper_con = NULL;
    bcmfp_group_config_t *group_config = NULL;
    bcmfp_qualifier_t qual_id1;
    bcmfp_qualifier_t qual_id2;
    bcmfp_qual_bitmap_t *qual_bitmap1 = NULL;
    bcmfp_qual_bitmap_t *qual_bitmap2 = NULL;
    bool dont_include = FALSE;
    uint8_t num_parts = 0;
    bcmfp_stage_t *stage = NULL;
    bcmfp_keygen_mode_t kg_mode;
    bcmfp_keygen_mode_t kge_mode;
    ...
    /*
     * Validate the number of valid elements in presel_ids
     * and conflict_qset_ids are same.
     */
    for (idx1 = 0; idx1 < gc->num_presels; idx1++) {
        if (gc->presel_ids[idx1] != 0 &&
            gc->conflict_qset_ids[idx1] != 0) {
            continue;
        } else {
            opinfo->state = BCMFP_GROUP_STATE_CONFLICT_QSETS_LESS;
            SHR_EXIT();
        }
    }

    size = (sizeof(bcmfp_conflict_qset_config_t *) *
            gc->num_presels);

    BCMFP_ALLOC(cqset_configs, size, "bcmfpConflictQssetConfigArray");

    /*
     * Validate each qualifier in each conflicting QSET has
     * different number.
     */
    for (idx1 = 0; idx1 < gc->num_presels; idx1++) {
        cqset_id = gc->conflict_qset_ids[idx1];
        cqset_config = NULL;
        SHR_IF_ERR_EXIT_EXCEPT_IF
            (bcmfp_conflict_qset_remote_config_get(unit,
                                               op_arg,
                                               stage_id,
                                               cqset_id,
                                               &cqset_config),
                                               SHR_E_NOT_FOUND);
        if (cqset_config == NULL) {
            opinfo->state = BCMFP_GROUP_STATE_CONFLICT_QSET_NOT_CREATED;
            SHR_EXIT();
        }
        cqset_configs[idx1] = cqset_config;
        for (idx2 = 0; idx2 < cqset_config->num_quals; idx2++) {
            /*
             * Check no other qualifier in the conflict QSET has
             * same number assigned as the current qualifier.
             */
            for (idx3 = (idx2 + 1);
                 idx3 < cqset_config->num_quals;
                 idx3++) {
                if (cqset_config->qual_number[idx2] ==
                    cqset_config->qual_number[idx3]) {
                    opinfo->state = BCMFP_GROUP_STATE_CONFLICT_QSET_INVALID;
                    SHR_EXIT();
                }
            }
            /*
             * Check whether the qualifier present in conflict QSET is
             * also present in groups QSET.
             */
            qual_bitmap1 = gc->qual_bitmap;
            while (qual_bitmap1 != NULL) {
                if (qual_bitmap1->qual == cqset_config->qual_id[idx2]) {
                    break;
                }
                qual_bitmap1 = qual_bitmap1->next;
            }
            if (qual_bitmap1 == NULL) {
                opinfo->state = BCMFP_GROUP_STATE_CONFLICT_QSET_INVALID;
            }
        }
        cqset_config = NULL;
    ...
    /*
     * Build a new group_config with following qualifiers.
     * 1. All qualifiers which are part of group template but
     *    not part of any one of the conflict QSETs.
     * 2. Qualifiers from first conflict QSET.
     */
    size = sizeof(bcmfp_group_config_t);
    BCMFP_ALLOC(group_config, size, "bcmfpGroupConfigConflictQset1");
    sal_memcpy(group_config, gc, size);
    group_config->qual_bitmap = NULL;

    qual_bitmap1 = gc->qual_bitmap;
    size = sizeof(bcmfp_qual_bitmap_t);
    while (qual_bitmap1 != NULL)  {
        for (idx1 = 0; idx1 < gc->num_presels; idx1++) {
            cqset_config = cqset_configs[idx1];
            for (idx2 = 0; idx2 < cqset_config->num_quals; idx2++) {
                if (cqset_config->qual_id[idx2] ==
                    qual_bitmap1->qual) {
                    dont_include = TRUE;
                    break;
                }
        ...
        if (dont_include == FALSE) {
            BCMFP_ALLOC(qual_bitmap2, size, "bcmfpQualBitmap");
            sal_memcpy(qual_bitmap2, qual_bitmap1, size);
            qual_bitmap2->next = group_config->qual_bitmap;
            group_config->qual_bitmap = qual_bitmap2;
        }
        dont_include = FALSE;
        qual_bitmap2 = NULL;
        qual_bitmap1 = qual_bitmap1->next;
    ...
    size = sizeof(bcmfp_keygen_oper_t);
    BCMFP_ALLOC(keygen_oper_pri, size, "bcmfpKeygenOperPrimaryQset");
    BCMFP_ALLOC(keygen_oper_con, size, "bcmfpKeygenOperConflictQset");
    size = sizeof(bcmfp_group_t);
    BCMFP_ALLOC(fg, size, "bcmfpGroupOperInfo");
    /*
     * Call keygen process primary QSET of the group.
     */
    SHR_IF_ERR_EXIT
        (bcmfp_group_pqset_keygen_process(unit,
                                          op_arg,
                                          tbl_id,
                                          stage_id,
                                          group_config,
    ...
    /*
     * Prepare keygen_cfg for calls to keygen process for
     * conflict QSETS.
     */
    SHR_IF_ERR_EXIT
        (bcmfp_group_parts_count(unit,
    ...
    fg->flags = opinfo->flags;
    fg->group_mode = opinfo->group_mode;
    fg->group_id = opinfo->group_id;
    ...
    size = sizeof(bcmfp_qual_bitmap_t);
    BCMFP_ALLOC(group_config->qual_bitmap, size, "bcmfpConflictQualBitmap");
    for (idx1 = 1; idx1 < gc->num_presels; idx1++) {
        cqset_config = cqset_configs[idx1];
        size = (sizeof(bcmfp_ext_codes_t) * BCMFP_ENTRY_PARTS_MAX);
        sal_memcpy(fg->ext_codes, opinfo->ext_codes, size);
        SHR_IF_ERR_EXIT
            (bcmfp_group_ext_codes_mark_unused(unit,
                                               stage_id,
                                               opinfo,
                                               fg->ext_codes));
        for (idx2 = 0; idx2 < cqset_config->num_quals; idx2++) {
            qual_id2 = cqset_config->qual_id[idx2];
            for (idx3 = 0; idx3 < cqset_configs[0]->num_quals; idx3++) {
                qual_id1 = cqset_configs[0]->qual_id[idx3];
                if (cqset_config->qual_number[idx2] ==
                    cqset_configs[0]->qual_number[idx3]) {
                    break;
                }
            }
            if (idx3 == cqset_configs[0]->num_quals) {
                 SHR_ERR_EXIT(SHR_E_INTERNAL);
            }
            ...
            size = sizeof(bcmfp_qual_bitmap_t);
            while (qual_bitmap1 != NULL)  {
                if (qual_bitmap1->qual == qual_id2) {
                    sal_memcpy(group_config->qual_bitmap,
                               qual_bitmap1,
                               size);
                    group_config->qual_bitmap->next = NULL;
                    break;
                }
                qual_bitmap1 = qual_bitmap1->next;
            }
            ...
            /*
             * Clear the multiplexers used for conflicting qualifier and
             * set in use flag to all unused multiplexers in the final
             * level.
             */
            SHR_IF_ERR_EXIT
                (bcmfp_group_conflict_qual_ext_codes_update(unit,
                                                            qual_id1,
                                                            keygen_oper_pri,
                                                            fg->ext_codes));
            /*
             * Convert the multiplexer details in ext_codes to different
             * that is understandable by the keygen algorithm.
             */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmfp_group_ext_ctrl_count_get(unit,
                                                &keygen_cfg,
                                                fg->ext_codes));
            num_ext_codes = keygen_cfg.ext_ctrl_sel_info_count;
            size = (sizeof(bcmfp_keygen_ext_ctrl_sel_info_t) * num_ext_codes);
            BCMFP_ALLOC(keygen_cfg.ext_ctrl_sel_info,
                        size,
                        "bcmfpKeyGenCfgExtCtrlSelInfo");
            SHR_IF_ERR_VERBOSE_EXIT
            ...
            /* Build keygen_cfg to allocate muxers for qual_id2 */
            SHR_IF_ERR_EXIT
                (bcmfp_group_keygen_qual_info_alloc(unit,
                                                    group_config,
                                                    &keygen_cfg));

            /* Call keygen config process with UPDATE flag. */
            SHR_IF_ERR_EXIT
                (bcmfp_keygen_cfg_process(unit,
                                          &keygen_cfg,
                                          keygen_oper_con));

            /* Save the offsets of conflicting qualifiers */
            SHR_IF_ERR_EXIT
                (bcmfp_group_qual_offset_get(unit,
                                             group_config,
                                             keygen_oper_con,
                                             fg));
            SHR_IF_ERR_EXIT
                (bcmfp_group_quals_offset_info_set(unit, fg));
            ...
            keygen_cfg.qual_info_arr = NULL;
            SHR_IF_ERR_EXIT
               (bcmfp_keygen_oper_free(unit, num_parts, keygen_oper_con));
            keygen_oper_con->qual_offset_info = NULL;
            keygen_oper_con->ext_ctrl_sel_info = NULL;
            for (part = 0; part < keygen_cfg.num_parts; part++) {
                group_qual = &(fg->qual_arr[part]);
                if (group_qual->qid_arr != NULL) {
                    sal_free(group_qual->qid_arr);
        ...
        /* Clear all the unused multiplexers in the final level. */
        SHR_IF_ERR_EXIT
            (bcmfp_group_ext_codes_unmark_unused(unit,
                                                 stage_id,
                                                 opinfo,
                                                 fg->ext_codes));

        /* Save the multiplexers of conflicting QSET. */
        SHR_IF_ERR_EXIT
            (bcmfp_conflict_qset_oper_info_set(unit,
                                               stage_id,
                                               opinfo->group_id,
                                               cqset_config->conflict_qset_id,
                                               fg->ext_codes));
    }
exit:
    for (part = 0; part < keygen_cfg.num_parts; part++) {
    ...
    if (cqset_configs != NULL) {
        for (idx1 = 0; idx1 < gc->num_presels; idx1++) {
            bcmfp_conflict_qset_config_free(unit,
                                            cqset_configs[idx1]);
        }
        SHR_FREE(cqset_configs);
    }
    SHR_FREE(fg);
    bcmfp_keygen_oper_free(unit, num_parts, keygen_oper_pri);
    SHR_FREE(keygen_oper_pri);
    bcmfp_keygen_oper_free(unit, num_parts, keygen_oper_con);
    SHR_FREE(keygen_oper_con);
    if (group_config != NULL) {
        bcmfp_qual_bitmap_free(unit, group_config->qual_bitmap);
    }
    SHR_FREE(group_config);
    SHR_FREE(qual_bitmap2);
    SHR_FREE(keygen_cfg.ext_ctrl_sel_info);
    SHR_FUNC_EXIT();
}

static int
bcmfp_group_qset_insert_process(int unit,
                                const bcmltd_op_arg_t *op_arg,
                                bcmltd_sid_t tbl_id,
                                bcmfp_stage_id_t stage_id,
                                bcmfp_group_config_t *gc,
                                bcmfp_group_oper_info_t *opinfo)
{
    ...
    if (BCMFP_STAGE_FLAGS_TEST(stage,
              BCMFP_STAGE_KEY_TYPE_PRESEL) &&
        gc->conflict_qset_ids != NULL) {
        SHR_IF_ERR_EXIT
            (bcmfp_group_conflict_qsets_process(unit,
                                                op_arg,
                                                tbl_id,
                                                stage_id,
                                                gc,
                     ...
                     BCMFP_STAGE_KEY_TYPE_PRESEL)) {
        SHR_IF_ERR_EXIT
            (bcmfp_group_keygen_insert_process(unit,
                                               op_arg,
                                               tbl_id,
                                               stage_id,
                                               gc,
               ...
               BCMFP_STAGE_KEY_TYPE_FIXED)) {
        SHR_IF_ERR_EXIT
            (bcmfp_group_selcode_insert_process(unit,
                                                op_arg,
                                                tbl_id,
                                                stage_id,
                                                gc));
 ...
 */
static int
bcmfp_group_aset_insert_process(int unit,
                                const bcmltd_op_arg_t *op_arg,
                                bcmltd_sid_t tbl_id,
                                bcmfp_stage_id_t stage_id,
                                bcmfp_group_config_t *gc,
 ...
 */
static int
bcmfp_group_config_insert_process(int unit,
                                  const bcmltd_op_arg_t *op_arg,
                                  bcmltd_sid_t tbl_id,
                                  bcmfp_stage_t *stage,
                                  bcmfp_group_config_t *gc,
                                  ...
                                  bcmfp_idp_info_t *idp_info)
{
     ...
     * space.
     */
    rv = bcmfp_group_aset_insert_process(unit,
                                         op_arg,
                                         tbl_id,
                                         stage->stage_id,
                                         gc,
     ...
     * space.
     */
    rv = bcmfp_group_qset_insert_process(unit,
                                         op_arg,
                                         tbl_id,
                                         stage->stage_id,
                                         gc,
                     ...
                     bcmfp_group_config_t *gc)
{
    ...
    bcmfp_pse_id_t pse_id = 0;
    bcmfp_conflict_qset_id_t conflict_qset_id = 0;
    bcmfp_stage_t *stage = NULL;
    ...
    /* Map conflict qualifier set entries associated to group. */
    if (gc->conflict_qset_ids != NULL) {
        for (idx = 0; idx < gc->num_presels; idx++) {
            if (gc->conflict_qset_ids[idx] == 0) {
                continue;
            }
            conflict_qset_id = gc->conflict_qset_ids[idx];
            SHR_IF_ERR_EXIT
                (bcmfp_conflict_qset_group_map_add(unit,
                                           stage_id,
                                           conflict_qset_id,
                                           group_id));
        }
                     ...
                     bcmfp_group_config_t *gc)
{
    ...
    if (gc->conflict_qset_ids != NULL) {
        for (idx = 0; idx < gc->num_presels; idx++) {
            ...
            conflict_qset_id = gc->conflict_qset_ids[idx];
            SHR_IF_ERR_EXIT
                (bcmfp_conflict_qset_group_map_delete(unit,
                                           stage_id,
                                           conflict_qset_id,
                                           group_id));
        }
 ...
 */
     ...
     */
    SHR_IF_ERR_EXIT
        (bcmfp_group_config_insert_process(unit,
                                           op_arg,
                                           tbl_id,
                                           stage,
                                           group_config,
                        ...
                        void *user_data)
{
    ...
    if (idp_info->sid_type == BCMFP_SID_TYPE_CONFLICT_QSET) {
        idp_info->template_id = template_id;
        SHR_IF_ERR_EXIT
            (bcmfp_group_idp_conflict_qset_process(unit,
                                             stage_id,
                                             group_id,
                                             user_data));
    }
...
/* Delete the given group and re-insert*/
static int
bcmfp_group_delete_insert(int unit,
                          const bcmltd_op_arg_t *op_arg,
                          bcmltd_sid_t tbl_id,
                          bcmfp_stage_id_t stage_id,
                          const bcmltd_field_t *key,
...
static int
bcmfp_group_config_update_check(int unit,
                                const bcmltd_op_arg_t *op_arg,
                                bcmltd_sid_t tbl_id,
                                bcmfp_stage_id_t stage_id,
                                ...
                                bool *cond_met)
{
    ...
    /* this support is enabled on presel stages alone */
    if (BCMFP_STAGE_FLAGS_TEST(stage,
               BCMFP_STAGE_KEY_TYPE_SELCODE) ||
               BCMFP_STAGE_FLAGS_TEST(stage,
               BCMFP_STAGE_KEY_TYPE_FIXED)) {
    ...
    /* support is not enabled on EM_FP stages */
    if (BCMFP_STAGE_FLAGS_TEST(stage,
    ...
    /*
     * Check if qualifier data alone is updated
     */
    while (in_data != NULL) {
        if (qualifiers_fid_info[in_data->id].qualifier == 0) {
            *cond_met = FALSE;
            SHR_EXIT();
        }
    ...
    /*
     * Check the group_status and number of entries
    ...
    /*
     * Check if the group's qual_config update staging
     * is successful
     */
     SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_config_update_staging(unit,
                                           ...
                                           &status));
    if (status != BCMFP_GROUP_UPDATE_SUCCESS) {
        *cond_met = FALSE;
    }
exit:
   SHR_FUNC_EXIT();
...
static int
bcmfp_group_pse_update(int unit,
                       const bcmltd_op_arg_t *op_arg,
                       bcmltd_sid_t tbl_id,
                       bcmfp_stage_id_t stage_id,
                       bcmfp_group_config_t *group_config,
                       bcmfp_group_oper_info_t *curr_opinfo,
                       bcmfp_group_oper_info_t *prev_opinfo)
{
     bcmfp_stage_t *stage = NULL;
     ...
     /*
     * Install a new presel profile with the updated
     * mux configuration
     */
    ...
    num_presels = group_config->num_presels;
    filter.group_oper_info = curr_opinfo;
    filter.group_config = group_config;
        ...
        for (idx = 0; idx < num_presels; idx++) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmfp_filter_presel_update(unit,
                                            ...
                                            &filter));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmfp_pse_config_free(unit, filter.pse_config[0]));
        }
    }
    /* Delete the old presel profile */
    SHR_IF_ERR_VERBOSE_EXIT
                          ...
                          bcmltd_fields_t *output_fields)
{
    ...
    bcmfp_group_oper_info_t *opinfo = NULL;
    bcmfp_idp_info_t idp_info;
    bool qual_update = TRUE;
    bcmfp_entry_id_traverse_cb cb_func;
    void *user_data = NULL;
    bcmfp_group_oper_info_t temp_opinfo;
    ...
    /*
     * Delete and reinsert the group
     * if the following conditions are met
     * - if the group is not suceesfully inserted before
     * - if no entries are attached to the group
     * - if the group_config_update staging fails
     */
     SHR_IF_ERR_EXIT
         (bcmfp_group_config_update_check(unit,
                                          op_arg,
                                          tbl_id,
                                          stage_id,
                                          in_data,
     ...
     if (cond_met == FALSE) {
         SHR_IF_ERR_EXIT
         (bcmfp_group_delete_insert(unit,
                                    op_arg,
                                    tbl_id,
                                    stage_id,
                                    key,
         ...
         sal_memcpy(&temp_opinfo,opinfo,sizeof(bcmfp_group_oper_info_t));

         SHR_IF_ERR_EXIT
             (bcmfp_group_config_update_process(unit,
                                                op_arg,
                                                tbl_id,
                                                stage_id,
                                                updated_group_config,
                                                ...
                                                FALSE));
         /*
          * re-insert the entries whose previous entry_state !=
          * BCMFP_ENTRY_SUCCESS
          */
         sal_memset(&idp_info, 0, sizeof(bcmfp_idp_info_t));
         idp_info.op_arg = (bcmltd_op_arg_t *)op_arg;
         ...
         idp_info.output_fields = output_fields;
         idp_info.generic_data = (void *)&qual_update;
         ...
         /* Update the presel entries mapped to the group */
         SHR_IF_ERR_VERBOSE_EXIT
             (bcmfp_group_pse_update(unit,
                                     op_arg,
                                     tbl_id,
                                     stage_id,
                                     updated_group_config,
                                  ...
                                  bool qset_update_staging)
{
    ...
    rv = bcmfp_group_qset_update_process(unit,
                                         op_arg,
                                         tbl_id,
                                         stage_id,
                                         group_config,
...
{
    bcmfp_group_oper_info_t *updated_opinfo = NULL;
    bcmfp_group_id_t group_id = 0;
    bcmfp_group_oper_info_t *opinfo = NULL;
    bcmfp_group_config_t *group_config = NULL;
    bcmfp_qual_bitmap_t *qual_bitmap = NULL;
    bcmfp_qual_bitmap_t *updated_qual_bitmap = NULL;
    bool qual_found = FALSE;
    uint16_t idx = 0;
    bool qset_update_staging = TRUE;
    uint8_t part = 0;
    ...
    /*
     * Check if the group_id is inserted previously
     * without any errors
     */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_remote_config_get(unit,
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_oper_info_get(unit,
                                   ...
                                   &opinfo));
    /*
     * If the current group is not successfully inserted
     * =>qualifier offsests would change
     */
    if (opinfo == NULL ||
        opinfo->state != BCMFP_GROUP_STATE_SUCCESS) {
        *status = BCMFP_GROUP_UPDATE_QUAL_OFFSETS_CHANGE;
        SHR_EXIT();
    }

    /*
     * If the update is called for staging,identify
     * the qualifier bitmaps which are newly inserted
     * Retain only the newly inserted qualifier bitmaps
     * in updated_group_config
     */
    qual_bitmap = group_config->qual_bitmap;
    while (qual_bitmap != NULL) {
        updated_qual_bitmap = updated_group_config->qual_bitmap;
        qual_found = FALSE;
        while (updated_qual_bitmap != NULL) {
            if (updated_qual_bitmap->qual == qual_bitmap->qual &&
                updated_qual_bitmap->fidx == qual_bitmap->fidx) {
                qual_found = TRUE;
                break;
            } else {
                updated_qual_bitmap = updated_qual_bitmap->next;
            }
        }
        if (qual_found == TRUE) {
            if (updated_qual_bitmap->partial == qual_bitmap->partial &&
                updated_qual_bitmap->compress == qual_bitmap->compress) {
                for (idx = 0; idx < BCMFP_MAX_WSIZE; idx++) {
                    if (updated_qual_bitmap->bitmap[idx] !=
                            qual_bitmap->bitmap[idx]) {
                        /* Mismatch in qualifier bitmap */
                        *status = BCMFP_GROUP_UPDATE_QUAL_OFFSETS_CHANGE;
                        SHR_EXIT();

                    } else {
                        /* set the bitmap of matching qualifiers
                         * with existing group_config to 0*/
                        updated_qual_bitmap->bitmap[idx] = 0;
                    }
                }
            }
        }
    ...
    /* build the group operational info
     * and group_config to be updated
     */
    BCMFP_ALLOC(updated_opinfo, sizeof(bcmfp_group_oper_info_t),
                    "bcmfpUpdatedGroupOperInfo");
    for (part = 0; part < BCMFP_ENTRY_PARTS_MAX; part++) {
        updated_opinfo->ext_codes[part] = opinfo->ext_codes[part];
    }
    updated_opinfo->valid = FALSE;
    updated_opinfo->state = BCMFP_GROUP_STATE_SUCCESS;
    updated_opinfo->pdd_type = opinfo->pdd_type;
    updated_group_config->psg_id = group_config->psg_id;
    updated_group_config->mode = opinfo->group_mode;
    updated_group_config->pdd_id = group_config->pdd_id;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_group_config_update_process(unit,
                                           ...
                                           updated_opinfo,
                                           qset_update_staging));
    if (updated_opinfo->qset_error == TRUE) {
        *status = BCMFP_GROUP_UPDATE_QUAL_OFFSETS_CHANGE;
    }
exit:
    bcmfp_group_config_free(unit, group_config);
    SHR_FREE(updated_opinfo);
    SHR_FUNC_EXIT();
}
