                          ...
                          const bcmltd_generic_arg_t *arg)
{
    ...
    /* Fill length adjust fid data. */
    sal_memset(fid_array, 0, sizeof(fid_array));
    fid_array[0] = compression_info->ing_pkt_len_adj_0_fid;
    fid_array[1] = compression_info->ing_pkt_len_adj_1_fid;
    for (fid_idx = 0; fid_idx < array_size; fid_idx++) {
        fid = fid_array[fid_idx];
        if (fid == 0) {
            continue;
    ...
    /* Fill remap object fid data. */
    sal_memset(fid_array, 0, sizeof(fid_array));
    fid_array[0] = compression_info->remap_object_0_fid;
    fid_array[1] = compression_info->remap_object_1_fid;
    for (fid_idx = 0; fid_idx < array_size; fid_idx++) {
        fid = fid_array[fid_idx];
        if (fid == 0) {
            continue;
