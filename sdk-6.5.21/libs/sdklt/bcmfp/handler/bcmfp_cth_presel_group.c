...
/*
 * For a given presel group QSET, get the actual number of bits needs
 * to be extracted to final key.
 */
static int
bcmfp_psg_qset_size_get(int unit,
                        ...
                        uint16_t *rkey_size)
{
    ...
    *rkey_size = 0;
    size = ((BCMFP_PRESEL_INPUT_BUS_LENGTH_MAX + 31) / 32);
    BCMFP_ALLOC(input_bus, size, "bcmfpPreselInputBus");
    for (idx = 0; idx < keygen_cfg->qual_info_count; idx++) {
        ...
        qual_cfg = &(qual_cfg_info1[qual_id]->qual_cfg_arr[0]);


        bitmap = keygen_cfg->qual_info_arr[idx].bitmap;
        for (idx2 = 0; idx2 < qual_cfg->num_chunks; idx2++) {
            if ((qual_cfg->e_params[idx2].offset +
                 qual_cfg->e_params[idx2].width) >
                BCMFP_PRESEL_INPUT_BUS_LENGTH_MAX) {
                LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit,
                            "Input BUS size more than expected\n")));
                SHR_ERR_EXIT(SHR_E_INTERNAL);
            }

            for (bit = 0; bit < qual_cfg->e_params[idx2].width; bit++) {
                if (SHR_BITGET(bitmap, bit + offset) &&
                    keygen_cfg->qual_info_arr[idx].partial == TRUE) {
                    if (!SHR_BITGET(input_bus,
                                    bit + qual_cfg->e_params[idx2].offset)) {
                        SHR_BITSET(input_bus,
                                   bit + qual_cfg->e_params[idx2].offset);
                        num_bits++;
                    }
                } else {
            ...
            offset += qual_cfg->e_params[idx2].width;
        }
        offset = 0;
    }
    *rkey_size = num_bits;
                         ...
                         bcmfp_psg_config_t *psg_config)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmfp_psg_qset_size_get(unit, &keygen_cfg, &qset_size));

    if (qset_size > stage->key_data_size_info->presel_key_size_max) {
