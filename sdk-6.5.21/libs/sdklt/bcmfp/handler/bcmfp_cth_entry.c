...
static int
bcmfp_entry_flowtracker_en_get(int unit,
                               bcmfp_stage_t *stage,
                               bcmltd_field_t *buffer,
                               bool *enable)
{
    uint32_t flowtracker_fid = 0;
    uint64_t flowtracker_u64 = 0;
    ...
    flowtracker_fid = stage->tbls.entry_tbl->flowtracker_fid;
    ...
    *enable = flowtracker_u64 & 0xFFFFFFFF;
                   ...
                   bcmfp_entry_config_t *entry_config)
{
    ...
    /* Dump flowtracker entry status. */
    if (stage->tbls.entry_tbl->flowtracker_fid != 0) {
        LOG_VERBOSE(BSL_LOG_MODULE,
            (BSL_META_U(unit, "Flowtracker entry = %d\n"),
            entry_config->flowtracker_en));
    }
                       ...
                       bcmfp_entry_config_t **config)
{
    ...
    *config = entry_config;
    ...
    if (stage->tbls.entry_tbl->flowtracker_fid != 0) {
        buffer = (bcmltd_field_t *)data;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmfp_entry_flowtracker_en_get(unit,
                                            stage,
                                            buffer,
                                            &flowtracker_en));
        entry_config->flowtracker_en = flowtracker_en;
    }
                             ...
                             bcmfp_idp_info_t *idp_info)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
     ...
     if (idp_info->generic_data != NULL) {
         qual_update_alone = (bool *)idp_info->generic_data;
     }
    ...
    filter->group_oper_info = opinfo;
     ...
     /*
      * If qual_update_alone is TRUE=>only qualifier config
      * has been updated in group_config
      * Update only the entries whose prev_state is
      * not success
      */
     if (qual_update_alone != NULL &&
             *qual_update_alone == TRUE) {
         if (filter->entry_config->entry_state
                 == BCMFP_ENTRY_SUCCESS) {
             SHR_EXIT();
         }
     }
                              ...
                              void *user_data)
{
     ...
     * to multiple entries.
     */
    if (idp_info->event_reason != BCMIMM_ENTRY_DELETE) {
        SHR_IF_ERR_VERBOSE_EXIT
