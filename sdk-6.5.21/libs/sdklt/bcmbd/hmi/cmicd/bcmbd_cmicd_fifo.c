                     ...
                     uint32_t *data, size_t wsize)
{
    ...
    if (wsize > COUNTOF(schan_msg.popresp.data)) {
        return SHR_E_PARAM;
    }
    ...
    sal_memcpy(data, schan_msg.popresp.data, 4 * wsize);
                      ...
                      uint32_t *data, size_t wsize)
{
    ...
    /* Check if the requested size > S channel push data size */
    if (wsize > COUNTOF(schan_msg.pushcmd.data)) {
        return SHR_E_PARAM;
    }
    ...
    sal_memcpy(schan_msg.pushcmd.data, data, 4 * wsize);
