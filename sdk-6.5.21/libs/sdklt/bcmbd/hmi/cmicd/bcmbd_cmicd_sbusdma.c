 ...
 */
inline static int
cmicd_sbusdma_chan_get(cmicd_sbusdma_dev_t *dev, bcmbd_sbusdma_data_t *data,
                       int *cmc, int *chan)
{
    int ch;

    if (data && data->force_chan) {
        if (data->chan >= CMICD_SBUSDMA_CHAN_MAX) {
            return SHR_E_BADID;
        }
        *cmc = data->chan / CMICD_SBUSDMA_CMC_CHAN;
        *chan = data->chan % CMICD_SBUSDMA_CMC_CHAN;
        return SHR_E_NONE;
    }
 ...
 */
inline static int
cmicd_sbusdma_chan_put(cmicd_sbusdma_dev_t *dev, bcmbd_sbusdma_data_t *data,
                       int cmc, int chan)
{
    int ch;
...
cmicd_sbusdma_reg_op(cmicd_sbusdma_dev_t *dev, bcmbd_sbusdma_work_t *work)
{
    ...
    /* Allocate a SBUSDMA channel */
    rv = cmicd_sbusdma_chan_get(dev, work->data, &cmc, &ch);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    ...
    cmicd_sbusdma_chan_put(dev, work->data, cmc, ch);
...
bcmbd_cmicd_sbusdma_batch_work_create(sbusdma_ctrl_t *ctrl, bcmbd_sbusdma_work_t *work)
{
    ...
    if (work->desc == NULL) {
        ...
        da->addr = bcmdrd_hal_dma_alloc(dev->unit, da->size,
                                        "bcmbdSbusDmaDesc", &da->paddr);
        if (!da->addr) {
            sal_free(work->desc);
            work->desc = NULL;
            return SHR_E_MEMORY;
        ...
        da = work->desc;
        if (da->size < sizeof(cmicd_sbusdma_desc_t) * work->items) {
            return SHR_E_PARAM;
        }
    }
