...
#ifndef CMICX_SBUSDMA_DONE_CONFIRM_TIMES
#define CMICX_SBUSDMA_DONE_CONFIRM_TIMES  2
#endif
 ...
 */
inline static int
cmicx_sbusdma_chan_get(cmicx_sbusdma_dev_t *dev, bcmbd_sbusdma_data_t *data,
                       int *cmc, int *chan)
{
    int ch;

    if (data && data->force_chan) {
        if (data->chan >= CMICX_SBUSDMA_CHAN_MAX) {
            return SHR_E_BADID;
        }
        *cmc = data->chan / CMICX_SBUSDMA_CMC_CHAN;
        *chan = data->chan % CMICX_SBUSDMA_CMC_CHAN;
        return SHR_E_NONE;
    }
 ...
 */
inline static int
cmicx_sbusdma_chan_put(cmicx_sbusdma_dev_t *dev, bcmbd_sbusdma_data_t *data,
                       int cmc, int chan)
{
    int ch;
    ...
    if (work->flags & SBUSDMA_WF_INT_MODE) {
        ...
        while (retry--) {
            ...
            if (CMIC_CMC_SBUSDMA_STATUSr_DONEf_GET(stat)) {
                ...
                while (count < CMICX_SBUSDMA_DONE_CONFIRM_TIMES) {
                    READ_CMIC_CMC_SBUSDMA_STATUSr(unit, cmc, ch, &stat);
                    if (CMIC_CMC_SBUSDMA_STATUSr_DONEf_GET(stat)) {
                        if (CMIC_CMC_SBUSDMA_STATUSr_ERRORf_GET(stat)) {
                            cmicx_sbusdma_error_report(dev, stat);
                        ...
                        /* DONE bit is not stable. */
                        LOG_WARN(BSL_LOG_MODULE,
                                 (BSL_META_U(unit,
                                            "Sbusdma DONE is not stable on %d/%d\n"),
                                 cmc, ch));
                    }
                }
                break;
...
cmicx_sbusdma_reg_op(cmicx_sbusdma_dev_t *dev, bcmbd_sbusdma_work_t *work)
{
    ...
    /* Allocate a SBUSDMA channel */
    rv = cmicx_sbusdma_chan_get(dev, work->data, &cmc, &ch);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
    ...
    cmicx_sbusdma_chan_put(dev, work->data, cmc, ch);
...
cmicx_sbusdma_desc_op(cmicx_sbusdma_dev_t *dev, bcmbd_sbusdma_work_t *work)
{
    ...
    bcmbd_sbusdma_desc_addr_t *da = NULL;
    uint32_t paddr32;
    int cmc, ch;
    int rv;
                                      ...
                                      bcmbd_sbusdma_work_t *work)
{
    ...
    if (work->desc == NULL) {
        ...
        work->desc = sal_alloc(sizeof(bcmbd_sbusdma_desc_addr_t),
                               "bcmbdSbusDmaDesc");
        if (!work->desc) {
            return SHR_E_MEMORY;
        }
        da = work->desc;
        da->intern_alloc = true;
        ...
        da = work->desc;
        if (da->size < sizeof(cmicx_sbusdma_desc_t) * work->items) {
            return SHR_E_PARAM;
        }
    }
                                      ...
                                      bcmbd_sbusdma_work_t *work)
{
    ...
    if (work->desc && work->desc->intern_alloc) {
        da = work->desc;
        bcmdrd_hal_dma_free(dev->unit, da->size, da->addr, da->paddr);
        sal_free(work->desc);
        work->desc = NULL;
