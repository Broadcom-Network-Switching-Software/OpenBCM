...
static int
bcmbd_cmicx_hotswap_enable_set(int unit, bool enable)
{
    int ioerr = 0;
    PAXB_0_PAXB_HOTSWAP_CTRLr_t hswp_ctrl;
    PAXB_0_RESET_ENABLE_IN_PCIE_LINK_DOWNr_t rst_en_in_pcie_link_down;

    SHR_FUNC_ENTER(unit);

    ioerr += READ_PAXB_0_PAXB_HOTSWAP_CTRLr(unit, &hswp_ctrl);
    PAXB_0_PAXB_HOTSWAP_CTRLr_ENABLEf_SET(hswp_ctrl, ((enable) ? 1 : 0));
    ioerr += WRITE_PAXB_0_PAXB_HOTSWAP_CTRLr(unit, hswp_ctrl);

    /* Enable reset to propogate to paxb data path logic when pcie link is down. */
    ioerr += READ_PAXB_0_RESET_ENABLE_IN_PCIE_LINK_DOWNr(unit, &rst_en_in_pcie_link_down);
    PAXB_0_RESET_ENABLE_IN_PCIE_LINK_DOWNr_EN_PAXB_RESETf_SET(rst_en_in_pcie_link_down, 1);
    ioerr += WRITE_PAXB_0_RESET_ENABLE_IN_PCIE_LINK_DOWNr(unit, rst_en_in_pcie_link_down);
...
static int
bcmbd_cmicx_hotswap_enable_get(int unit, bool *enable)
{
    int ioerr = 0;
    PAXB_0_PAXB_HOTSWAP_CTRLr_t hswp_ctrl;
    ...
    ioerr += READ_PAXB_0_PAXB_HOTSWAP_CTRLr(unit, &hswp_ctrl);
    *enable = PAXB_0_PAXB_HOTSWAP_CTRLr_ENABLEf_GET(hswp_ctrl);
...
static bcmbd_hotswap_drv_t cmicx_hotswap_drv = {
    .hotswap_recover = bcmbd_cmicx_hotswap_recover,
    .hotswap_enable_set = bcmbd_cmicx_hotswap_enable_set,
    .hotswap_enable_get = bcmbd_cmicx_hotswap_enable_get,
};
...
bcmbd_cmicx_hotswap_init(int unit)
{
    ...
    rv = bcmbd_hotswap_drv_init(unit, &cmicx_hotswap_drv);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "CMICX hotswap init failed.\n")));
        SHR_ERR_EXIT(rv);
    }

    rv = bcmbd_cmicx_hotswap_recover(unit);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "CMICX hotswap recover failed.\n")));
        SHR_ERR_EXIT(rv);
    }
