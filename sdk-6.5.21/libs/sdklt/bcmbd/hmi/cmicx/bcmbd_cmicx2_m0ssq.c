...
/* Number of Cortex-M0. */
#define CMICX2_NUM_OF_CORTEXM0               4
...
cmicx2_m0ssq_self_reset(int unit, uint32_t uc)
{
    ...
    shr_timeout_init(&to, 500 * MSEC_USEC, 100);
...
bcmbd_cmicx2_m0ssq_reset(int unit) {
    ...
    /* Reset all M0 cores. */
    for (uc = 0; uc < CMICX2_NUM_OF_CORTEXM0; uc ++) {

         /* Don't reset PCIe hot swap firmware. */
         if (uc != CMICX2_PCIE_UC) {
             SHR_IF_ERR_EXIT
                 (cmicx2_m0ssq_uc_start_set(unit, uc, 0));
         }
    }
