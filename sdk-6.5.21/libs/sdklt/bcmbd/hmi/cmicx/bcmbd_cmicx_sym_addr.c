 ...
 */
static int
bcmbd_cmicx_sym_addr(shr_pb_t *pb, int unit, uint32_t sym_flags,
                     uint32_t adext, uint32_t addr)
{
    ...
    rv = bcmbd_cmicx_addr_decode(unit, adext, addr, sym_flags, &sid, &dyn_info);
    if (SHR_SUCCESS(rv)) {
...
{
    return bcmbd_cmicx_dump_addr_func_set(unit, bcmbd_cmicx_sym_addr);
}
