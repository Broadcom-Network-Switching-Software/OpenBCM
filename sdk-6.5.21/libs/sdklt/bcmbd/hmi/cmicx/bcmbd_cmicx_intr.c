...
/*
 * Configure interrupts in operating system.
 *
 * This function provides information to the operating system on how
 * to handle hardware interrupts. Actions may include interrupt
 * acknowledgement as well as masking of further interrupts, until the
 * user mode interrupt handling is complete.
 *
 * For KNET support, this function is also called to enable and
 * disable KNET mode in the operating system. This can be done with
 * interrupts enabled, and the operating system driver is responsible
 * for adjusting the configuration in a safe manner.
 */
static int
bcmbd_cmicx_intr_configure(int unit, int irq_num)
{
    int rv;
    uint64_t phys_addr;
    uint32_t axi_addr;
...
bcmbd_cmicx_intr_connect(int unit)
{
    ...
    rv = bcmbd_cmicx_intr_configure(unit, irq_num);
    if (SHR_FAILURE(rv)) {
        return rv;
    }
...
bcmbd_cmicx_intr_knet_enable_set(int unit, bool enable)
{
    ...
    /* Update KNET settings in kernel */
    if (intr_connected[unit]) {
        (void)bcmbd_cmicx_intr_configure(unit, irq_num);
    }
