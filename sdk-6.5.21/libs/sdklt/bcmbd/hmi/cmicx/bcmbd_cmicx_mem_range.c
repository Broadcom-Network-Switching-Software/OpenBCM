                         ...
                         uint32_t flags, uint32_t mor_clks)
{
    ...
    if (mor_clks) {
        /*
         * Calculate the number of command FIFO locations required to
         * hold a single command, given that the command FIFO is 64
         * bits wide.
         */
        if (flags & BCMBD_ROF_READ) {
            /*
             * For reads, the command length is one opcode beat and
             * one address beat.
             */
            cmdfifo_loc = DWORD_ALIGN64(1 + 1);
        } else {
            /*
             * For writes, the command length is the entry size (in
             * words) plus one opcode beat and one address beat.
             */
            cmdfifo_loc = DWORD_ALIGN64(wsize + 1 + 1);
        }
        cmdfifo_size = bcmbd_hmi_cmdbuf_size_get(unit);
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            ...
                            "mor_clks=%d burst_cmds=%d\n"),
                 op_str, adext, addr, mor_clks, data.burst_cmds));
    ...
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              ...
                              "addr=0x%04"PRIx32"%08"PRIx32": "
                              "%s (%d)\n"),
                   op_str, adext, addr, shr_errmsg(rv), rv));
    }
