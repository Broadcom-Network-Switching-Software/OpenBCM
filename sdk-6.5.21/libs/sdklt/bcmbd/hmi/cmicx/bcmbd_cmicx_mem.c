                     ...
                     uint32_t *data, size_t wsize)
{
    ...
    bcmbd_hmi_read_f simhook_read = bcmbd_simhook_read[unit];
    ...
    if (simhook_read) {
        SHR_IF_ERR_EXIT
            (simhook_read(unit, adext | BCMBD_SIM_SOC_MEM, addr,
    ...
    sal_memcpy(data, schan_msg.readresp.data, 4 * wsize);
                      ...
                      uint32_t *data, size_t wsize)
{
    ...
    int acctype, dstblk, datalen;
    schan_msg_t schan_msg;
    bcmbd_hmi_write_f simhook_write = bcmbd_simhook_write[unit];
    ...
    /* Check if the requested size > S channel write data size */
    if (wsize > COUNTOF(schan_msg.writecmd.data)) {
        SHR_IF_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    if (simhook_write) {
        SHR_IF_ERR_EXIT
            (simhook_write(unit, adext | BCMBD_SIM_SOC_MEM, addr,
                           data, BCMDRD_WORDS2BYTES(wsize)));
        /* Debug output */
        if (LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
            shr_pb_t *pb = shr_pb_create();
    ...
    sal_memcpy(schan_msg.writecmd.data, data, 4 * wsize);
