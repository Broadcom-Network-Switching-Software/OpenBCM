                    ...
                    uint64_t dma_addr, uint32_t flags)
{
    ...
    if (bd->pt_range_op == NULL) {
        uint32_t wsize, idx, *data32;
        bcmbd_pt_dyn_info_t di;
        int rv = SHR_E_NONE;

        if (addr == NULL || dyn_info == NULL) {
            return SHR_E_PARAM;
        }

        wsize = bcmdrd_pt_entry_wsize(unit, sid);
        data32 = (uint32_t *)addr;
        sal_memcpy(&di, dyn_info, sizeof(di));
        for (idx = 0; idx < num_entries; idx++) {
            rv = bcmbd_pt_read(unit, sid, &di, ovrr_info, data32, wsize);
            if (SHR_FAILURE(rv)) {
                break;
            }
            data32 += wsize;
                     ...
                     uint64_t dma_addr, uint32_t flags)
{
    ...
    if (bd->pt_range_op == NULL) {
        ...
        sal_memcpy(&di, dyn_info, sizeof(di));
        for (idx = 0; idx < num_entries; idx++) {
            rv = bcmbd_pt_write(unit, sid, &di, ovrr_info, data32);
            if (SHR_FAILURE(rv)) {
                break;
            }
            data32 += wsize;
