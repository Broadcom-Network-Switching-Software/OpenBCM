/*! \file bcm56990_a0_bd_ipep_intr.c
 *
...
ipep_intr_write_clear(int unit, int inst, unsigned int intr_num)
{
    ...
    if (intr_num >= CENTRAL_CTR_EVICTION_BIT_BASE) {
        ...
        ING_DOI_INTR_STATUSr_t idoi_st;
        READ_ING_DOI_INTR_STATUSr(unit, inst, &idoi_st);
        val = ING_DOI_INTR_STATUSr_GET(idoi_st);
        val &= ~(0x1 << (intr_num - ING_DOI_BIT_BASE));
        ING_DOI_INTR_STATUSr_SET(idoi_st, val);
        ioerr += WRITE_ING_DOI_INTR_STATUSr(unit, inst, idoi_st);
    } else if (intr_num >= ING_DII_BIT_BASE) {
        ING_DII_INTR_STATUSr_t idii_st;
        READ_ING_DII_INTR_STATUSr(unit, inst, &idii_st);
        val = ING_DII_INTR_STATUSr_EVENT_FIFO_SER_ERRf_GET(idii_st);
        val &= ~(0x1 << (intr_num - ING_DII_EVENT_FIFO0_SER_ERR));
        ING_DII_INTR_STATUSr_EVENT_FIFO_SER_ERRf_SET(idii_st, val);
        ioerr += WRITE_ING_DII_INTR_STATUSr(unit, inst, idii_st);
    } else if (intr_num >= EGR_DOI_BIT_BASE) {
        EGR_DOI_INTR_STATUSr_t edoi_st;
        READ_EGR_DOI_INTR_STATUSr(unit, inst, &edoi_st);
        val = EGR_DOI_INTR_STATUSr_GET(edoi_st);
        val &= ~(0x1 << (intr_num - EGR_DOI_BIT_BASE));
        EGR_DOI_INTR_STATUSr_SET(edoi_st, val);
        ioerr += WRITE_EGR_DOI_INTR_STATUSr(unit, inst, edoi_st);
    } else {
        EGR_DII_INTR_STATUSr_t edii_st;
        READ_EGR_DII_INTR_STATUSr(unit, inst, &edii_st);
        val = EGR_DII_INTR_STATUSr_EVENT_FIFO_SER_ERRf_GET(edii_st);
        val &= ~(0x1 << (intr_num - EGR_DII_EVENT_FIFO0_SER_ERR));
        EGR_DII_INTR_STATUSr_EVENT_FIFO_SER_ERRf_SET(edii_st, val);
        ioerr += WRITE_EGR_DII_INTR_STATUSr(unit, inst, edii_st);
    }
...
ipep_intr(int unit, uint32_t intr_idx)
{
    ...
    if (intr_idx < ipep_lp.ip_base) {
        ...
        if (IP_TO_CMIC_INTR_STATUSr_IP0f_GET(ip2c_st)) {
            ...
            val = ING_DII_INTR_STATUSr_GET(idii_st);
            for (i = 0; i < ING_DII_NUM_BITS; i++) {
                if (val & (0x1 << i)) {
                    SHR_BITSET(ipep_stat[inst].bmp, ING_DII_BIT_BASE + i);
                }
            }
        }
        ...
        if (IP_TO_CMIC_INTR_STATUSr_IP4f_GET(ip2c_st)) {
            ...
            if (IP4_INTR_STATUSr_IPOSTf_GET(ip4_st)) {
                ...
                val = ING_DOI_INTR_STATUSr_GET(idoi_st);
                for (i = 0; i < ING_DOI_NUM_BITS; i++) {
                    if (val & (0x1 << i)) {
                        SHR_BITSET(ipep_stat[inst].bmp, ING_DOI_BIT_BASE + i);
                    }
                }
            }
            if (IP4_INTR_STATUSr_ISWf_GET(ip4_st)) {
        ...
        if (EP_TO_CMIC_INTR_STATUSr_EARBf_GET(ep2c_st)) {
            ...
            val = EGR_DII_INTR_STATUSr_GET(edii_st);
            for (i = 0; i < EGR_DII_NUM_BITS; i++) {
                if (val & (0x1 << i)) {
                    SHR_BITSET(ipep_stat[inst].bmp, EGR_DII_BIT_BASE + i);
                }
            }
        }
        ...
        if (EP_TO_CMIC_INTR_STATUSr_EPOSTf_GET(ep2c_st)) {
            ...
            val = EGR_DOI_INTR_STATUSr_GET(edoi_st);
            for (i = 0; i < EGR_DOI_NUM_BITS; i++) {
                if (val & (0x1 << i)) {
                    SHR_BITSET(ipep_stat[inst].bmp, EGR_DOI_BIT_BASE + i);
                }
            }
        }
