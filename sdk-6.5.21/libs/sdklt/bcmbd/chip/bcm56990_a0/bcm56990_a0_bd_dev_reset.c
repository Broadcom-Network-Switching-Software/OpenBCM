/*! \file bcm56990_a0_bd_dev_reset.c
 *
...
#include <bcmbd/chip/bcm56990_a0_cmic_bcast.h>
...
#include <bcmbd/chip/bcm56990_a0_dev.h>
...
* Local definitions
 */
/* Bypass software sequential reset by setting timeout to 1 clock.*/
#define SEQRST_TIMEOUT_HW       1
/* Default reset stage time is 100 us. */
#define SEQ_RESET_STAGE_TIME_DEFAULT    100
/* Hardware pace time unit is 40ns. 1 usec is 25 units (1000 / 40). */
#define SEQ_RESET_UNIT_US       25
/* Convert usec to hardware units. */
#define SEQ_RESET_STAGE_TIME(_d)        ((_d) * SEQ_RESET_UNIT_US)
/* No less than: 16 (TSC) + PM top + IP + EP + MMU + TOP. */
#define SEQ_RESET_STEPS_MAX     25
/* Sequential reset mode. */
#define SEQ_RESET_T(_t)    (BCM56990_A0_LRD_DEVICE_SEQ_RESET_T_T_SEQ_RESET_##_t)
...
#define TS_CLK(mhz)      (BCM56990_A0_LRD_DEVICE_TS_PLL_REF_CLK_T_T_CLK_##mhz)
...
#define BS_CLK(mhz)      (BCM56990_A0_LRD_DEVICE_BS_PLL_REF_CLK_T_T_CLK_##mhz)
...
cdport_bcast_enable(int unit, int num_bcast, int *last_bcast_port)
{
    ...
    BCMDRD_PBMP_ITER(pbmp, port) {
        if (block_port_get(unit, port, BLKTYPE_CDPORT) == 0) {
            ...
            int blk = CDPORT_BCAST_BLK(bcast_domain);
            if (bcast_domain & 0x1) {
 ...
 * is done when TH4 is operating at a different core clock frequency
 */
static int
...
{
    int ioerr = 0;
    uint32_t srst_t, srst_d, rst_to;
    TOP_SEQ_RST_0r_t srst0;
    TOP_SEQ_RST_1r_t srst1;
    TOP_SEQ_RST_2r_t srst2;
    SEQ_RST_CONFIGr_t srst_cfg;

    srst_t = bcmbd_config_get(unit, SEQ_RESET_T(STAGE_4_PORTS),
                              "DEVICE_CONFIG", "SEQ_RESET_MODE");
    srst_d = bcmbd_config_get(unit, SEQ_RESET_STAGE_TIME_DEFAULT,
                              "DEVICE_CONFIG", "SEQ_RESET_STAGE_TIME");
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "SEQ_RESET_MODE - %d, SEQ_RESET_STAGE_TIME %d.\n"),
                 srst_t, srst_d));

    if (srst_t == SEQ_RESET_T(NONE)) {
        ...
        /* Configure TSC reset stages. */
        ioerr += READ_TOP_SEQ_RST_2r(unit, &srst2);
        switch (srst_t) {
        case SEQ_RESET_T(STAGE_ALL_PORTS):
            TOP_SEQ_RST_2r_HW_SEQ_TSC_DISABLEf_SET(srst2, 0x2);
            break;
        case SEQ_RESET_T(STAGE_8_PORTS):
            TOP_SEQ_RST_2r_HW_SEQ_TSC_DISABLEf_SET(srst2, 1);
            break;
        default:
            TOP_SEQ_RST_2r_HW_SEQ_TSC_DISABLEf_SET(srst2, 0);
        }
        ioerr += WRITE_TOP_SEQ_RST_2r(unit, srst2);

        /* Configure hardware reset space time. */
        ioerr += READ_TOP_SEQ_RST_1r(unit, &srst1);
        TOP_SEQ_RST_1r_HW_SEQ_RST_TIME_SPACEf_SET(srst1,
                                                  SEQ_RESET_STAGE_TIME(srst_d));
        ioerr += WRITE_TOP_SEQ_RST_1r(unit, srst1);

         /* Configure SW sequential reset timeout. */
        ioerr += READ_TOP_SEQ_RST_0r(unit, &srst0);
        TOP_SEQ_RST_0r_SW_SEQ_RST_TIMEOUT_DURATIONf_SET(srst0, SEQRST_TIMEOUT_HW);
        ioerr += WRITE_TOP_SEQ_RST_0r(unit, srst0);

        /*
         * Configure sequential reset timeout.
         * Total reset time should be no less than
         * SEQRST_SW_TIME + hardware stage time * steps.
         */
        rst_to = SEQRST_TIMEOUT_HW +
                 SEQ_RESET_STAGE_TIME(srst_d) * SEQ_RESET_STEPS_MAX;
        ioerr += READ_SEQ_RST_CONFIGr(unit, &srst_cfg);
        SEQ_RST_CONFIGr_TOPMGMT_RSTSEQ_TIMEOUTf_SET(srst_cfg, rst_to);
...
/* Stop traffic and set load line to init. (SDKLT-18655) */
static int
power_load_line_reset(int unit)
{
    int ioerr = 0;
    int port;
    uint32_t val;
    bcmdrd_pbmp_t pbmp;
    CDMAC_CTRLr_t cmac_ctrl;
    XLMAC_CTRLr_t xmac_ctrl;
    DMU_PCU_IPROC_CONTROLr_t iproc_ctrl;
    ...
    /* Bypass if ring-map was not configured. */
    ioerr += READ_CMIC_TOP_SBUS_RING_MAPr(unit, 0, &sbus_ring_map);
    if (!CMIC_TOP_SBUS_RING_MAPr_GET(sbus_ring_map)) {
        return ioerr ? SHR_E_IO : SHR_E_NONE;
    }

    /* Stop traffic by disabling MAC TX/TX. */
    /* Disabling CDMAC TX/RX. */
    cdport_pbmp_get(unit, &pbmp);
    BCMDRD_PBMP_ITER(pbmp, port) {
        ioerr += READ_CDMAC_CTRLr(unit, port, &cmac_ctrl);
        CDMAC_CTRLr_RX_ENf_SET(cmac_ctrl, 0);
        CDMAC_CTRLr_TX_ENf_SET(cmac_ctrl, 0);
        ioerr += WRITE_CDMAC_CTRLr(unit, port, cmac_ctrl);
    }

    /* Disabling XLPORT TX/RX. */
    xlport_pbmp_get(unit, &pbmp);
    BCMDRD_PBMP_ITER(pbmp, port) {
        ioerr += READ_XLMAC_CTRLr(unit, port, &xmac_ctrl);
        XLMAC_CTRLr_RX_ENf_SET(xmac_ctrl, 0);
        XLMAC_CTRLr_TX_ENf_SET(xmac_ctrl, 0);
        ioerr += WRITE_XLMAC_CTRLr(unit, port, xmac_ctrl);
    }

    sal_usleep(TSC_PWRUP_USEC);

    /* Set load line to init (clear bit1). */
    ioerr += READ_DMU_PCU_IPROC_CONTROLr(unit, &iproc_ctrl);
    val = DMU_PCU_IPROC_CONTROLr_IHOST_PWRCTRLI0f_GET(iproc_ctrl);
    /* bit[0] - load line feature enable; bit[1] - load line select. */
    DMU_PCU_IPROC_CONTROLr_IHOST_PWRCTRLI0f_SET(iproc_ctrl, val & 0x1);
    ioerr += WRITE_DMU_PCU_IPROC_CONTROLr(unit, iproc_ctrl);
...
bcm56990_a0_bd_dev_reset(int unit)
{
     ...
     * ring 5: PORT_IF(89)
     * ring 6: PFC(90)
     * ring 7: PM0(16)-15(31), PBST0(80)
     * ring 8: Pm31(47)-16(32), PBST1(81)
     * ring 9: PM32(48)-47(63), PBST0(82)
     * ring 10: PM63(79)-48(64), PBST0(83)
    ...
    uint32_t ring_map[] = { 0x00333210, 0x000B0000, 0x77777777, 0x77777777,
                            ...
                            0xAAAAAAAA, 0xAAAAAAAA, 0x0000A987, 0x00000654,
                            0x00B00000, 0x00000000, 0x00000000, 0x00000000 };
    ...
    ioerr += power_load_line_reset(unit);
    /* Reset Port Macros. */
     ...
     */
     ...
     AVS_HW_MNTR_SEQUENCER_MASK_PVT_MNTRr_SEQ_MASK_PVT_MNTRf_SET(avs_mntr,
                                                                 0xffffe);
     ioerr += WRITE_AVS_HW_MNTR_SEQUENCER_MASK_PVT_MNTRr(unit, avs_mntr);
