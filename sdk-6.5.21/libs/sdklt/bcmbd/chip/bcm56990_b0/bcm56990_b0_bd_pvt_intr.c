...
pvt_intr(int unit)
{
    ...
    uint32_t intr_num;
    TOP_PVTMON_INTR_STATUS_0r_t stat;
    TOP_PVTMON_INTR_MASK_0r_t mask;
    int ioerr = 0;

    ioerr += READ_TOP_PVTMON_INTR_STATUS_0r(unit, &stat);
    ioerr += READ_TOP_PVTMON_INTR_MASK_0r(unit, &mask);
    if (ioerr) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "PVTMON I/O error\n")));
    ...
    intr_stat = TOP_PVTMON_INTR_STATUS_0r_GET(stat) &
                TOP_PVTMON_INTR_MASK_0r_GET(mask);
    if (intr_stat == 0) {
