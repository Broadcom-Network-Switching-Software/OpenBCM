/*! \file bcm56990_b0_bd_mmu_intr.c
 *
...
mmu_intr_update(int unit, unsigned int intr_num, int enable)
{
    ...
    } else if (intr_num < GLB_BIT_BASE + GLB_NUM_BITS) {
        ioerr += READ_MMU_GLB_INT_ENr(unit, &misc_en);
        if (intr_num == MMU_INTR_GLB_MEM_PAR_ERR) {
            MMU_GLB_INT_ENr_MEM_PAR_ERR_INT_ENf_SET(misc_en, enable);
        }
        if (intr_num == MMU_INTR_GLB_TS_OVERFLOW) {
            MMU_GLB_INT_ENr_TS_OVERFLOW_INT_ENf_SET(misc_en, enable);
        }
        if (intr_num == MMU_INTR_GLB_TS_UNDERFLOW) {
            MMU_GLB_INT_ENr_TS_UNDERFLOW_INT_ENf_SET(misc_en, enable);
        }
        ioerr += WRITE_MMU_GLB_INT_ENr(unit, misc_en);
        enable = (misc_en._mmu_glb_int_en) ? 1 : 0;
        MMU_GLBCFG_CPU_INT_ENr_GLBCFG_INT_ENf_SET(glb_en, enable);
    }
    ...
    if (glb_eb_upd) {
        ...
        MMU_GLBCFG_CPU_INT_ENr_EBCFG8_INT_ENf_SET(glb_en, enable);
        MMU_GLBCFG_CPU_INT_ENr_EBCFG9_INT_ENf_SET(glb_en, enable);
        MMU_GLBCFG_CPU_INT_ENr_EBCFG10_INT_ENf_SET(glb_en, enable);
        MMU_GLBCFG_CPU_INT_ENr_EBCFG11_INT_ENf_SET(glb_en, enable);
        MMU_GLBCFG_CPU_INT_ENr_EBCFG12_INT_ENf_SET(glb_en, enable);
        MMU_GLBCFG_CPU_INT_ENr_EBCFG13_INT_ENf_SET(glb_en, enable);
        MMU_GLBCFG_CPU_INT_ENr_EBCFG14_INT_ENf_SET(glb_en, enable);
        MMU_GLBCFG_CPU_INT_ENr_EBCFG15_INT_ENf_SET(glb_en, enable);
    } else if (glb_itm_upd) {
        /*
...
mmu_intr_clear(int unit, unsigned int intr_num)
{
    ...
    MMU_THDR_QE_CPU_INT_SETr_t thdr_set;
    MMU_CFAP_INT_STATr_t cfap_stat;
    MMU_CFAP_INT_SETr_t cfap_set;
    MMU_THDO_CPU_INT_STATr_t thdo_stat;
    MMU_THDO_CPU_INT_SETr_t thdo_set;
    bool glb_eb_upd = false;
    bool glb_itm_upd = false;
    uint32_t intr_mask;
    uint32_t intr_inst;
    ...
    if (intr_num == MMU_INTR_EB_MEM_PAR_ERR) {
        ...
        ioerr += WRITE_MMU_ITMCFG_CPU_INT_STATr(unit, intr_inst, itm_stat);
        /* Request top-level interrupt to be cleared */
        glb_itm_upd = true;
    } else if (intr_num >= THDO_BIT_BASE) {
        /* Clear leaf interrupt instance */
        intr_mask = 1 << (intr_num - THDO_BIT_BASE);
        READ_MMU_THDO_CPU_INT_SETr(unit, intr_inst, &thdo_set);
        if (intr_num == MMU_INTR_THDO_SPID_MISMATCH) {
            MMU_THDO_CPU_INT_SETr_SPID_MISMATCHf_SET(thdo_set, 0);
        } else if (intr_num == MMU_INTR_THDO_CNT_OVERFLOW) {
            MMU_THDO_CPU_INT_SETr_COUNTER_OVERFLOWf_SET(thdo_set, 0);
        } else if (intr_num == MMU_INTR_THDO_CNT_UNDERFLOW) {
            MMU_THDO_CPU_INT_SETr_COUNTER_UNDERFLOWf_SET(thdo_set, 0);
        }
        ioerr += WRITE_MMU_THDO_CPU_INT_SETr(unit, intr_inst, thdo_set);
        MMU_THDO_CPU_INT_STATr_SET(thdo_stat, intr_mask);
        ioerr += WRITE_MMU_THDO_CPU_INT_STATr(unit, intr_inst, thdo_stat);
        ...
        MMU_CFAP_INT_SETr_CLR(cfap_set);
        ioerr += WRITE_MMU_CFAP_INT_SETr(unit, intr_inst, cfap_set);
        /* This is a 64-bit register using only the lower 32 bits */
        MMU_CFAP_INT_STATr_CLR(cfap_stat);
        MMU_CFAP_INT_STATr_SET(cfap_stat, 0, intr_mask);
        ioerr += WRITE_MMU_CFAP_INT_STATr(unit, intr_inst, cfap_stat);
        /* Then attempt to clear interrupt in parent register */
        MMU_ITMCFG_CPU_INT_STATr_CLR(itm_stat);
        ...
        READ_MMU_THDR_QE_CPU_INT_SETr(unit, intr_inst, &thdr_set);
        if (intr_num == MMU_INTR_THDR_CNT_OVERFLOW) {
            MMU_THDR_QE_CPU_INT_SETr_CNT_OVERFLOWf_SET(thdr_set, 0);
        } else {
            MMU_THDR_QE_CPU_INT_SETr_CNT_UNDERFLOWf_SET(thdr_set, 0);
        }
        ioerr += WRITE_MMU_THDR_QE_CPU_INT_SETr(unit, intr_inst, thdr_set);
        MMU_THDR_QE_CPU_INT_STATr_SET(thdr_stat, intr_mask);
        ioerr += WRITE_MMU_THDR_QE_CPU_INT_STATr(unit, intr_inst, thdr_stat);
        /* Then attempt to clear interrupt in parent register */
        MMU_ITMCFG_CPU_INT_STATr_CLR(itm_stat);
    ...
    if (intr_num < GLB_BIT_BASE + GLB_NUM_BITS) {

        MMU_GLB_INT_STATr_CLR(misc_stat);
        if (intr_num == MMU_INTR_GLB_MEM_PAR_ERR) {
            MMU_GLB_INT_STATr_MEM_PAR_ERR_INT_STATf_SET(misc_stat, 1);
        }
        if (intr_num == MMU_INTR_GLB_TS_OVERFLOW) {
            MMU_GLB_INT_STATr_TS_OVERFLOW_INT_STATf_SET(misc_stat, 1);
        }
        if (intr_num == MMU_INTR_GLB_TS_UNDERFLOW) {
            MMU_GLB_INT_STATr_TS_UNDERFLOW_INT_STATf_SET(misc_stat, 1);
        }
        ioerr += WRITE_MMU_GLB_INT_STATr(unit, misc_stat);

        MMU_GLBCFG_CPU_INT_STATr_GLB_INT_STATf_SET(glb_stat, 1);
    } else if (glb_eb_upd) {
        if (intr_inst == 0) {
            ...
            MMU_GLBCFG_CPU_INT_STATr_EBCFG8_INT_STATf_SET(glb_stat, 1);
        } else if (intr_inst == 9) {
            MMU_GLBCFG_CPU_INT_STATr_EBCFG9_INT_STATf_SET(glb_stat, 1);
        } else if (intr_inst == 10) {
            MMU_GLBCFG_CPU_INT_STATr_EBCFG10_INT_STATf_SET(glb_stat, 1);
        } else if (intr_inst == 11) {
            MMU_GLBCFG_CPU_INT_STATr_EBCFG11_INT_STATf_SET(glb_stat, 1);
        } else if (intr_inst == 12) {
            MMU_GLBCFG_CPU_INT_STATr_EBCFG12_INT_STATf_SET(glb_stat, 1);
        } else if (intr_inst == 13) {
            MMU_GLBCFG_CPU_INT_STATr_EBCFG13_INT_STATf_SET(glb_stat, 1);
        } else if (intr_inst == 14) {
            MMU_GLBCFG_CPU_INT_STATr_EBCFG14_INT_STATf_SET(glb_stat, 1);
        } else if (intr_inst == 15) {
            MMU_GLBCFG_CPU_INT_STATr_EBCFG15_INT_STATf_SET(glb_stat, 1);
        }
    } else if (glb_itm_upd) {
        if (intr_inst == 0) {
...
mmu_intr(int unit)
{
    ...
    inst = 0;
    if (MMU_GLBCFG_CPU_INT_STATr_GLB_INT_STATf_GET(glb_stat)) {
        if (READ_MMU_GLB_INT_STATr(unit, &misc_stat) != 0) {
            return;
        }
        if (MMU_GLB_INT_STATr_MEM_PAR_ERR_INT_STATf_GET(misc_stat)) {
            SHR_BITSET(mmu_stat[inst].bmp, MMU_INTR_GLB_MEM_PAR_ERR);
        }
        if (MMU_GLB_INT_STATr_TS_OVERFLOW_INT_STATf_GET(misc_stat)) {
            SHR_BITSET(mmu_stat[inst].bmp, MMU_INTR_GLB_TS_OVERFLOW);
        }
        if (MMU_GLB_INT_STATr_TS_UNDERFLOW_INT_STATf_GET(misc_stat)) {
    ...
    if (MMU_GLBCFG_CPU_INT_STATr_EBCFG8_INT_STATf_GET(glb_stat)) {
        inst_mask |= (1 << 8);
    }
    if (MMU_GLBCFG_CPU_INT_STATr_EBCFG9_INT_STATf_GET(glb_stat)) {
        inst_mask |= (1 << 9);
    }
    if (MMU_GLBCFG_CPU_INT_STATr_EBCFG10_INT_STATf_GET(glb_stat)) {
        inst_mask |= (1 << 10);
    }
    if (MMU_GLBCFG_CPU_INT_STATr_EBCFG11_INT_STATf_GET(glb_stat)) {
        inst_mask |= (1 << 11);
    }
    if (MMU_GLBCFG_CPU_INT_STATr_EBCFG12_INT_STATf_GET(glb_stat)) {
        inst_mask |= (1 << 12);
    }
    if (MMU_GLBCFG_CPU_INT_STATr_EBCFG13_INT_STATf_GET(glb_stat)) {
        inst_mask |= (1 << 13);
    }
    if (MMU_GLBCFG_CPU_INT_STATr_EBCFG14_INT_STATf_GET(glb_stat)) {
        inst_mask |= (1 << 14);
    }
    if (MMU_GLBCFG_CPU_INT_STATr_EBCFG15_INT_STATf_GET(glb_stat)) {
        inst_mask |= (1 << 15);
    }
    ...
    for (inst = 0; inst_mask && inst < NUM_ITM_INST; inst++) {
        ...
        if (READ_MMU_ITMCFG_CPU_INT_STATr(unit, inst, &itm_stat) != 0) {
            return;
        }
        ...
        if (MMU_ITMCFG_CPU_INT_STATr_CFAP_INT_STATf_GET(itm_stat)) {
            if (READ_MMU_CFAP_INT_STATr(unit, inst, &cfap_stat) != 0) {
                return;
            }
            /* This is a 64-bit register using only the lower 32 bits */
        ...
        if (MMU_ITMCFG_CPU_INT_STATr_THDR_INT_STATf_GET(itm_stat)) {
            if (READ_MMU_THDR_QE_CPU_INT_STATr(unit, inst, &thdr_stat) != 0) {
                return;
            }
        ...
        if (MMU_ITMCFG_CPU_INT_STATr_THDO_INT_STATf_GET(itm_stat)) {
            if (READ_MMU_THDO_CPU_INT_STATr(unit, inst, &thdo_stat) != 0) {
                return;
            }
...
int
bcm56990_b0_bd_mmu_intr_drv_init(int unit)
{
    int rv;
...
int
bcm56990_b0_bd_mmu_intr_drv_cleanup(int unit)
{
