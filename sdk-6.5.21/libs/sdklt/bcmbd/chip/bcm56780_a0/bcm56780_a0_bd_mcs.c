/*! \file bcm56780_a0_bd_mcs.c
 *
 * MCS routines for BCM56780_A0
 */
/*
...
#define MSG_AREAS       {0x50820000, 0x50780000, 0x51820000, 0x51780000}
...
#define APP_RESOURCES { \
    ...
    {MCS_APP_FT, (((FLOW_DB_ENTRY_SIZE + EFLOW_DB_ENTRY_SIZE) * MAX_NUM_FLOWS_HW_LEARN) + BUFFER_MEMORY),  \
                  (1024 * 1024), 0, 2, 2, 1}, \
    {MCS_APP_BS, 0, (1024 * 1024), 0, 2, 2, 0}, \
    {MCS_APP_ST, 0, (1024 * 1024), 0, 2, 2, 0}, \
    {MCS_APP_IFA, 0, (1024 * 1024), 0, 2, 2, 0}, \
    {MCS_APP_IFA2, 0, (1024 * 1024), 0, 2, 2, 0}, \
...
static const bcmbd_mcs_res_req_t
    bcm56780_a0_mcs_app_resources[MCS_APP_COUNT] = APP_RESOURCES;

static bcmbd_ukernel_config_t
    bcm56780_a0_mcs_resources_used[BCMDRD_CONFIG_MAX_UNITS][NUM_UC];

static uint32_t bcm56780_a0_mcs_cores_used[BCMDRD_CONFIG_MAX_UNITS];

/*******************************************************************************
 * Private functions
 */
static uint32_t
bcm56780_a0_mcs_to_pci_addr(int unit, int uc, uint32_t mcs_addr)
{
...
mcs_rd(int unit, int uc, uint32_t mcs_addr)
{
    ...
    pci_addr = bcm56780_a0_mcs_to_pci_addr(unit, uc, mcs_addr);
...
static void bcm56780_mcs_rm_get_all(bcmbd_ukernel_config_t *cfg, int uc)
{
    cfg->uc = -1;
...
static int
bcm56780_mcs_rm_get_free(int unit, int uc, bcmbd_ukernel_config_t *cfg,
                         uint32_t flags)
{
    int i;
    ...
    if (uc == 0) {
        other = &(bcm56780_a0_mcs_resources_used[unit][1]);
    } else if (uc == 1) {
        other = &(bcm56780_a0_mcs_resources_used[unit][0]);
    } else if (uc == 2) {
        other = &(bcm56780_a0_mcs_resources_used[unit][3]);
    } else if (uc == 3) {
        other = &(bcm56780_a0_mcs_resources_used[unit][2]);
    } else {
        return SHR_E_PARAM;
    }

    bcm56780_mcs_rm_get_all(&cfree, uc);
    ...
    if (flags & BCMBD_MCS_RM_FLAG_INCLUDE_SELF) {
        ...
        if (other->sram_size == 0) {
         ...
         for (i = 0; i < NUM_UC; i++) {
             if (i != uc) {
                 used = &(bcm56780_a0_mcs_resources_used[unit][i]);
        ...
        this = &(bcm56780_a0_mcs_resources_used[unit][uc]);
...
static int
bcm56780_mcs_find_resources(int unit, int uc,
                            ...
                            bcmbd_ukernel_config_t *cfg, uint32_t flags)
{
    ...
    (void)bcm56780_mcs_rm_get_free(unit, uc, &cfree, flags);
...
static int
bcm56780_a0_mcs_uc_reset(int unit, int uc)
{
...
static int
bcm56780_a0_mcs_uc_cfg_set(int unit, int uc, bcmbd_ukernel_config_t *cfg)
{
...
static int
bcm56780_a0_mcs_uc_cfg_get(int unit, int uc, bcmbd_ukernel_config_t *cfg)
{
    uint32_t val = 0, ver = 0;
...
static int
bcm56780_a0_mcs_uc_dbg_get(int unit, int uc, mcs_debug_extn_t *dbg)
{
    unsigned int i;
    uint32_t pci_addr = 0;
    uint32_t dbg_base;
    ...
    pci_addr = bcm56780_a0_mcs_to_pci_addr(unit, uc, dbg_base);
...
static int
bcm56780_a0_mcs_uc_start(int unit, int uc)
{
    MHOST_x_CR5_RST_CTRLr_t ctl;
...
static int
bcm56780_a0_mcs_uc_is_reset(int unit, int uc, int *isrst)
{
    MHOST_x_CR5_RST_CTRLr_t ctl;
...
static int
bcm56780_a0_mcs_intr_enable(int unit, int uc)
{
...
static int
bcm56780_a0_mcs_intr_disable(int unit, int uc)
{
    return bcmbd_sw_intr_disable(unit, (CMICX2_SWI_MHOST0 + uc));
}

static int
bcm56780_a0_mcs_intr_set(int unit, int uc)
{
...
static int
bcm56780_a0_mcs_rm_get_resources(int unit, int uc, int type, bcmbd_ukernel_config_t *cfg,
                            uint32_t flags)
{
    bcmbd_ukernel_config_t *used;

    if (type == BCMBD_MCS_RM_RESOURCES_FREE) {
        return bcm56780_mcs_rm_get_free(unit, uc, cfg, flags);
    } else if (type == BCMBD_MCS_RM_RESOURCES_USED) {
        used = &(bcm56780_a0_mcs_resources_used[unit][uc]);
        sal_memcpy(cfg, used, sizeof(bcmbd_ukernel_config_t));
        return SHR_E_NONE;
    } else if (type == BCMBD_MCS_RM_RESOURCES_ALL) {
        bcm56780_mcs_rm_get_all(cfg, uc);
    }
    return SHR_E_NONE;
}

static int
bcm56780_a0_mcs_rm_reserve(int unit, int uc, bcmbd_mcs_res_req_t *req,
                           bcmbd_ukernel_config_t *cfg, uint32_t flags)
{
    ...
    if (flags & BCMBD_MCS_RM_FLAG_AUTO_CONFIG) {
        if (req->app < MCS_APP_COUNT) {
            sal_memcpy(&res, &bcm56780_a0_mcs_app_resources[req->app],
                       sizeof(bcmbd_mcs_res_req_t));
        } else {
            sal_memcpy(&res, &bcm56780_a0_mcs_app_resources[MCS_APP_UNKNOWN],
                       sizeof(bcmbd_mcs_res_req_t));
        }
    } else {
    ...
    if (flags & BCMBD_MCS_RM_FLAG_AUTO_CORE) {
        for (i = 0; i < NUM_UC; i++) {
            if ((bcm56780_a0_mcs_cores_used[unit] & (1 << i)) == 0) {
                /* uC is free. Check for resource availability */
                rv = bcm56780_mcs_find_resources(unit, i, &res, cfg, flags);
                if (rv == SHR_E_NONE) {
                    /* Success */
                    break;
                }
        ...
        rv = bcm56780_mcs_find_resources(unit, uc, &res, cfg, flags);
    }
    ...
    used = &(bcm56780_a0_mcs_resources_used[unit][cfg->uc]);
        ...
        if (res.hostram_size > 0) {
            ...
            if (addr1 != NULL) {
                ...
                if ((eaddr >> 24) == (paddr1 >> 24)) {
                    ...
                    /* Not usable. Give it one more try */
    ...
    /* Commit the reserve */
    bcm56780_a0_mcs_cores_used[unit] |= (1 << cfg->uc);
    sal_memcpy(&bcm56780_a0_mcs_resources_used[unit][cfg->uc],
...
static int
bcm56780_a0_mcs_rm_release(int unit, int uc)
{
    int rv = SHR_E_NONE;
    bcmbd_ukernel_config_t *used;
    ...
    bcm56780_a0_mcs_cores_used[unit] &= ~(1 << uc);
...
bcm56780_a0_bd_mcs_drv_init(int unit)
{
    ...
    drv->mcs_to_pci_addr = bcm56780_a0_mcs_to_pci_addr;
    drv->mcs_uc_reset = bcm56780_a0_mcs_uc_reset;
    drv->mcs_uc_cfg_set = bcm56780_a0_mcs_uc_cfg_set;
    drv->mcs_uc_cfg_get = bcm56780_a0_mcs_uc_cfg_get;
    drv->mcs_uc_dbg_get = bcm56780_a0_mcs_uc_dbg_get;
    drv->mcs_uc_start = bcm56780_a0_mcs_uc_start;
    drv->mcs_uc_is_reset = bcm56780_a0_mcs_uc_is_reset;
    drv->mcs_intr_enable = bcm56780_a0_mcs_intr_enable;
    drv->mcs_intr_disable = bcm56780_a0_mcs_intr_disable;
    drv->mcs_intr_set = bcm56780_a0_mcs_intr_set;
    drv->mcs_rm_reserve = bcm56780_a0_mcs_rm_reserve;
    drv->mcs_rm_release = bcm56780_a0_mcs_rm_release;
    drv->mcs_rm_get_resources= bcm56780_a0_mcs_rm_get_resources;
    ...
    bcm56780_a0_mcs_cores_used[unit] = 0;
    for (uc = 0; uc < NUM_UC; uc++) {
        sal_memset(&(bcm56780_a0_mcs_resources_used[unit][uc]), 0,
