/*! \file bcm56780_a0_bd_port_intr.c
 *
...
xlport_intr(int unit)
{
    ...
    for (inst = 0; inst < XLMAC_INST; inst=inst+2) {
        if (intr_bits & LSHIFT32(1, inst)) {
    ...
    for (inst = 0; inst < XLMAC_INST; inst=inst+2) {
        port = pm_port + inst;
        /* X9 has 2 management ports 161 and 162 but they are mapped to lane 0 and lane 2 of
         * xlport core so port number needs to be adjusted
        */
        if (port == 163) {
            port = 162;
        }
    ...
    for (inst = 0; inst < XLMAC_INST; inst=inst+2) {
        ...
        port = pm_port + inst;
        /* X9 has 2 management ports 161 and 162 but they are mapped to lane0 and lane 2 of
         * xlport core so port number needs to be adjusted
        */
        if (port == 163) {
            port = 162;
