...
dpr_latency_config(int unit)
{
    ...
    uint32_t clk_freq;
    uint32_t default_clk_freq = CLK(1350MHZ);
    uint32_t null_ctr_threshold = 0;
    int pipe;
    ...
    clk_freq = bcmbd_config_get(unit, default_clk_freq,
                                "DEVICE_CONFIG",
                                "PP_CLK_FREQ");

    switch (clk_freq) {
        case CLK(1350MHZ):
            null_ctr_threshold = 0x6E;
            break;
        case CLK(1100MHZ):
            null_ctr_threshold = 0x59;
            break;
        case CLK(1025MHZ):
            null_ctr_threshold = 0x53;
            break;
        case CLK(950MHZ):
            null_ctr_threshold = 0x4D;
            break;
        default:
            break;
    }

    if (null_ctr_threshold != 0) {
        for (pipe = 0; pipe < NUM_PP_PIPES; pipe++) {
            ioerr += READ_ING_DII_DEBUG_CONFIGr(unit, pipe, &ing_dii_debug_cfg);
            ING_DII_DEBUG_CONFIGr_NULL_COUNTER_THRESHOLDf_SET(
                ing_dii_debug_cfg, null_ctr_threshold);
...
hash_init(int unit)
{
    ...
    IFTA90_E2T_00_HASH_CONTROLm_HASH_TABLE_BANK_CONFIG_LANE0f_SET(i90_hctl0, 0xf);
    IFTA90_E2T_01_HASH_CONTROLm_HASH_TABLE_BANK_CONFIG_LANE0f_SET(i90_hctl1, 0xf);
    IFTA90_E2T_00_HASH_CONTROLm_HASH_TABLE_BANK_CONFIG_LANE1f_SET(i90_hctl0, 0xf);
    IFTA90_E2T_01_HASH_CONTROLm_HASH_TABLE_BANK_CONFIG_LANE1f_SET(i90_hctl1, 0xf);
    ...
    if (bcmdrd_dev_flags_get(unit) & CHIP_FLAG_MTOP) {
        ...
        sal_usleep(1000);
        M_E2T_00_HASH_CONTROLm_HASH_TABLE_BANK_CONFIGf_SET(m_hctl0, 0x0);
        ioerr += WRITE_M_E2T_00_HASH_CONTROLm(unit, 0, m_hctl0);
    }
...
ipep_init(int unit)
{
    ...
    ING_DII_HW_RESET_CONTROL_0r_COUNTf_SET(ing_hreset_ctrl_0, 0x10000);
    ioerr += WRITE_ING_DII_HW_RESET_CONTROL_0r_ALL(unit, ing_hreset_ctrl_0);
    ...
    EGR_DII_HW_RESET_CONTROL_0r_COUNTf_SET(egr_hreset_ctrl_0, 0x10000);
...
bcm56780_a0_tm_mem_init(int unit)
{
    ...
    EDB_LPBK_RESERVED_CREDIT_COUNTr_t loopback_credit;
    int in_progress;
    int idx, pipe;
    uint32_t pipe_map;
    int pipe_init_msec;
    ...
    /* Initialize loopback port credit. */
    EDB_LPBK_RESERVED_CREDIT_COUNTr_CLR(loopback_credit);
    EDB_LPBK_RESERVED_CREDIT_COUNTr_LPBK_MIN_CREDIT_COUNTf_SET(loopback_credit, 6);
    ioerr += WRITE_EDB_LPBK_RESERVED_CREDIT_COUNTr_ALL(unit, loopback_credit);
