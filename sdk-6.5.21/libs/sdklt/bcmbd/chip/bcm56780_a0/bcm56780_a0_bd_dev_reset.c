/*! \file bcm56780_a0_bd_dev_reset.c
 *
...
#define CLK(mhz)      (BCM56780_A0_LRD_DEVICE_CLK_FREQ_T_T_CLK_##mhz)

#define TS_CLK(mhz)      (BCM56780_A0_LRD_DEVICE_TS_PLL_REF_CLK_T_T_CLK_##mhz)
#define BS_CLK(mhz)      (BCM56780_A0_LRD_DEVICE_BS_PLL_REF_CLK_T_T_CLK_##mhz)
...
#define XFLOW_MACSEC_USEC_MAX        (1000 * MSEC_USEC)
#define XFLOW_MACSEC_EMUL_USEC_MAX   (10000 * MSEC_USEC)
#define XFLOW_MACSEC_MIN_POLLS       1
...
pll_init(int unit)
{
    ...
    TOP_SOFT_RESET_REG_2r_TOP_MS_PLL_RST_Lf_SET(top_sreset_2, 1);
    ...
    TOP_SOFT_RESET_REG_2r_TOP_MS_PLL_POST_RST_Lf_SET(top_sreset_2, 1);
...
bcm56780_a0_bd_dev_reset(int unit)
{
    ...
    TOP_MACSEC_CTRL0r_t macsecreset;
    MACSEC_HW_RESET_CONTROLr_t macsec_control;
    shr_timeout_t to;
    bool emul = bcmdrd_feature_enabled(unit, BCMDRD_FT_EMUL) ;
    bool sim = bcmdrd_feature_is_sim(unit);
    int ms_ports[] = { 0, 32, 16, 88, 120, 104};
    uint32_t ring_map[] = { 0x00333210, 0x00000000, 0x44444444, 0x55555544,
                            0x00005555, 0x00000000, 0x99988800, 0x00005400,
                            0x00000000, 0x00000000, 0x00000000, 0x00000A76,
     ...
     */
    TOP_SOFT_RESET_REG_3r_SET(top_sreset_3, 0xFFFE0);
    ioerr += WRITE_TOP_SOFT_RESET_REG_3r(unit, top_sreset_3);
    sal_usleep(1000);
    TOP_SOFT_RESET_REG_3r_SET(top_sreset_3, 0xFFC0);
    ioerr += WRITE_TOP_SOFT_RESET_REG_3r(unit, top_sreset_3);
    sal_usleep(1000);
    TOP_SOFT_RESET_REG_3r_SET(top_sreset_3, 0xF800);
    ioerr += WRITE_TOP_SOFT_RESET_REG_3r(unit, top_sreset_3);
    sal_usleep(1000);
    TOP_SOFT_RESET_REG_3r_SET(top_sreset_3, 0);
    ioerr += WRITE_TOP_SOFT_RESET_REG_3r(unit, top_sreset_3);
    sal_usleep(1000);
    ...
    /*
     * SBUS ring and block number for bcm56780:
     *              block_num   ring
     *  -------------------------------
     *   IP         1           1
     *   EP         2           2
    ...
    /* Disable MACsec mux by default. Enabled later using SEC_PC_PM. */
    TOP_MACSEC_CTRL0r_SET(macsecreset, 0x1fff000);
    ioerr += WRITE_TOP_MACSEC_CTRL0r(unit, macsecreset);
    ...
    if (bcmdrd_feature_enabled(unit, BCMDRD_FT_MACSEC)) {
        /* Step 2: TOP_MACSEC_RST_L */
        ioerr += READ_TOP_SOFT_RESET_REGr(unit, &top_sreset);
        TOP_SOFT_RESET_REGr_TOP_MACSEC0_RST_Lf_SET(top_sreset, 1);
        TOP_SOFT_RESET_REGr_TOP_MACSEC1_RST_Lf_SET(top_sreset, 1);
        TOP_SOFT_RESET_REGr_TOP_MACSEC2_RST_Lf_SET(top_sreset, 1);
        TOP_SOFT_RESET_REGr_TOP_MACSEC3_RST_Lf_SET(top_sreset, 1);
        TOP_SOFT_RESET_REGr_TOP_MACSEC4_RST_Lf_SET(top_sreset, 1);
        TOP_SOFT_RESET_REGr_TOP_MACSEC5_RST_Lf_SET(top_sreset, 1);
        ioerr += WRITE_TOP_SOFT_RESET_REGr(unit, top_sreset);
    }


    /* Step 3: TOP_SOFT_RESET_REG__TOP_*#_RST_L */
    ...
    TOP_SOFT_RESET_REG_3r_SET(top_sreset_3, 0x1F);
    ioerr += WRITE_TOP_SOFT_RESET_REG_3r(unit, top_sreset_3);
    sal_usleep(1000);
    TOP_SOFT_RESET_REG_3r_SET(top_sreset_3, 0x3FF);
    ioerr += WRITE_TOP_SOFT_RESET_REG_3r(unit, top_sreset_3);
    sal_usleep(1000);
    TOP_SOFT_RESET_REG_3r_SET(top_sreset_3, 0x7FFF);
    ioerr += WRITE_TOP_SOFT_RESET_REG_3r(unit, top_sreset_3);
    sal_usleep(1000);
    TOP_SOFT_RESET_REG_3r_SET(top_sreset_3, 0xFFFFF);
    ioerr += WRITE_TOP_SOFT_RESET_REG_3r(unit, top_sreset_3);
    sal_usleep(wait_usec);

    /* Bring IP, EP, PORT_IF and MMU blocks out of reset */
    ioerr += READ_TOP_SOFT_RESET_REGr(unit, &top_sreset);
    /* RDB reset is done before EP and IP. */
    ...
    for (idx = 0; idx < 6; idx++) {
        /* Check for MACSEC */
        if (sim || !bcmdrd_feature_enabled(unit, BCMDRD_FT_MACSEC)) {
            break;
        }

        MACSEC_HW_RESET_CONTROLr_CLR(macsec_control);
        MACSEC_HW_RESET_CONTROLr_MEM_TABLE_INITf_SET(macsec_control, 1);
        MACSEC_HW_RESET_CONTROLr_MIB_INITf_SET(macsec_control, 1);
        MACSEC_HW_RESET_CONTROLr_TCAM_INITf_SET(macsec_control, 1);
        WRITE_MACSEC_HW_RESET_CONTROLr(unit, macsec_control, ms_ports[idx]);

        /* Poll for completion */
        if (emul) {
            shr_timeout_init(&to, XFLOW_MACSEC_EMUL_USEC_MAX, XFLOW_MACSEC_MIN_POLLS);
        } else {
            shr_timeout_init(&to, XFLOW_MACSEC_USEC_MAX, XFLOW_MACSEC_MIN_POLLS);
        }
        do {
            ioerr += READ_MACSEC_HW_RESET_CONTROLr(unit, &macsec_control, ms_ports[idx]);
            if (shr_timeout_check(&to)) {
                break;
            }
        } while (MACSEC_HW_RESET_CONTROLr_ISEC_MIB_SA_INIT_DONEf_GET
                 (macsec_control) == 0);

        if (MACSEC_HW_RESET_CONTROLr_ISEC_MIB_SA_INIT_DONEf_GET(macsec_control) == 0) {
            return SHR_E_TIMEOUT;
        }
    }

    /* Step 8: XLPORT_MAC_CONTROL reset. */
