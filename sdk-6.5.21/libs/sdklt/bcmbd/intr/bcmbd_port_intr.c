...
static bcmbd_intr_drv_t *port_intr_drv[BCMDRD_CONFIG_MAX_UNITS];
...
{
    bcmbd_intr_drv_t *idrv = port_intr_drv[unit];

    if (idrv == NULL) {
        return SHR_E_UNIT;
    }

    if (intr_param != PORT_INTR_PARAM_GET(intr_param)) {
