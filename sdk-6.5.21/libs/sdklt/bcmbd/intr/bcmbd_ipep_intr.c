...
static bcmbd_intr_drv_t *ipep_intr_drv[BCMDRD_CONFIG_MAX_UNITS];
...
{
    bcmbd_intr_drv_t *idrv = ipep_intr_drv[unit];

    if (idrv == NULL) {
        return SHR_E_UNIT;
    }

    if (intr_param != IPEP_INTR_PARAM_GET(intr_param)) {
