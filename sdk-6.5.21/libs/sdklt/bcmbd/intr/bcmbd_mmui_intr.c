/*! \file bcmbd_mmui_intr.c
 *
 * MMU instrument interrupt driver API.
 */
/*
...
static bcmbd_intr_drv_t *mmui_intr_drv[BCMDRD_CONFIG_MAX_UNITS];
...
{
    bcmbd_intr_drv_t *md = mmui_intr_drv[unit];

    if (md == NULL) {
        return SHR_E_UNIT;
    }

    if (intr_param != MMU_INTR_PARAM_GET(intr_param)) {
