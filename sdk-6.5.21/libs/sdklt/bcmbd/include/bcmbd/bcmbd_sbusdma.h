...
typedef struct bcmbd_sbusdma_data_s {
    ...
    /*!
     * A fixed channel will be used if \c force_chan is true, otherwise
     * an available SBUSDMA channel will be selected by the underlying driver.
     * It is the caller's responsibility to maintain the SBUSDMA channel
     * working interfaces and to avoid the resources from being conflict
     * if this value is true.
     */
    bool force_chan;

    /*! The fixed channel to be used when \c force_chan is true. */
    uint32_t chan;
 ...
 */
typedef struct bcmbd_sbusdma_desc_addr_s {
    ...
    /*!
     * This flag will be set to true if the descriptor and DMA memory are
     * allocated internally in \ref bcmbd_sbusdma_batch_work_create.
     * The allocated resources will be deleted internally as well in
     * \ref bcmbd_sbusdma_batch_work_delete if the flag is true.
     * Leave this flag to false if the descriptor and DMA memory are
     * allocated and freed externally.
     */
    bool intern_alloc;
} bcmbd_sbusdma_desc_addr_t;
