...
/*! BS Embedded Application messaging class. */
#define MCS_MSG_CLASS_BS              7

/*! BS EAPP is ready. */
#define BS_APPL_READY                       0x00

/*! Debug buffer configuration. */
#define BS_DBG_CFG                          0x01

/*! BS interface configuration. */
#define BS_INTERFACE_CFG                    0x02

/*! Configure output signal. */
#define BS_SIGNAL_SET                       0x03

/*! Get output signal. */
#define BS_SIGNAL_GET                       0x04

/*! Configure log parameters. */
#define BS_LOG_SET                          0x05

/*! Get log parameters. */
#define BS_LOG_GET                          0x06

/*! Set BS time. */
#define BS_TIME_SET                         0x07

/*! Get BS time. */
#define BS_TIME_GET                         0x08

/*! Get BS status. */
#define BS_STATE_GET                        0x09

/*! Set frequency adjustment */
#define BS_FREQ_ADJUST_SET                  0x0A

/*! Set phase adjustment */
#define BS_PHASE_ADJUST_SET                 0x0B

/*! Notify f/w about SDK warmboot entry. */
#define BS_DEINIT_ENTRY                     0x0C

/*! BS Embedded Application messaging subclass. */

/*! Message subclass indicating that BFD EApp is ready. */
#define MCS_MSG_SUBCLASS_BS_APPL_READY                                         \
    BS_APPL_READY

/*! Message subclass to intimate of the debug buffer config. */
#define MCS_MSG_SUBCLASS_BS_DBG_CFG                                            \
    BS_DBG_CFG

/*! Reply message subclass to the BS debug config message. */
#define MCS_MSG_SUBCLASS_BS_DBG_CFG_REPLY                                      \
    MCS_MSG_SUBCLASS_REPLY(BS_DBG_CFG)

/*! Message subclass to intimate of the BS interface config. */
#define MCS_MSG_SUBCLASS_BS_INTERFACE_CFG                                      \
    MCS_MSG_DMA_SET(BS_INTERFACE_CFG)

/*! Reply message subclass to the BS interface config message. */
#define MCS_MSG_SUBCLASS_BS_INTERFACE_CFG_REPLY                                \
    MCS_MSG_SUBCLASS_REPLY(BS_INTERFACE_CFG)

/*! Message subclass to configure BS signal output. */
#define MCS_MSG_SUBCLASS_BS_SIGNAL_SET                                         \
    BS_SIGNAL_SET

/*! Reply message subclass to the set BS signal message. */
#define MCS_MSG_SUBCLASS_BS_SIGNAL_SET_REPLY                                   \
    MCS_MSG_SUBCLASS_REPLY(BS_SIGNAL_SET)

/*! Reply message subclass to the get BS signal message. */
#define MCS_MSG_SUBCLASS_BS_SIGNAL_GET_REPLY                                   \
    MCS_MSG_SUBCLASS_REPLY(BS_SIGNAL_GET)

/*! Message subclass to configure BS log. */
#define MCS_MSG_SUBCLASS_BS_LOG_SET                                            \
    MCS_MSG_DMA_SET(BS_LOG_SET)

/*! Reply message subclass to the Set BS signal message. */
#define MCS_MSG_SUBCLASS_BS_LOG_SET_REPLY                                      \
    MCS_MSG_SUBCLASS_REPLY(BS_LOG_SET)

/*! Reply message subclass to the BS log message. */
#define MCS_MSG_SUBCLASS_BS_LOG_GET                                            \
    BS_LOG_GET

/*! Reply message subclass to the get BS signal message. */
#define MCS_MSG_SUBCLASS_BS_LOG_GET_REPLY                                      \
    MCS_MSG_SUBCLASS_REPLY(BS_LOG_GET)

/*! Message subclass to set BS time. */
#define MCS_MSG_SUBCLASS_BS_TIME_SET                                           \
    MCS_MSG_DMA_SET(BS_TIME_SET)

/*! Reply message subclass to the Set BS time message. */
#define MCS_MSG_SUBCLASS_BS_TIME_SET_REPLY                                     \
    MCS_MSG_SUBCLASS_REPLY(BS_TIME_SET)

/*! Message subclass to get BS time. */
#define MCS_MSG_SUBCLASS_BS_TIME_GET                                           \
    MCS_MSG_DMA_SET(BS_TIME_GET)

/*! Reply message subclass to the get BS time message. */
#define MCS_MSG_SUBCLASS_BS_TIME_GET_REPLY                                     \
    MCS_MSG_SUBCLASS_REPLY(BS_TIME_GET)

/*! Message subclass to get BS state. */
#define MCS_MSG_SUBCLASS_BS_STATE_GET                                          \
    BS_STATE_GET

/*! Reply message subclass to the get BS state message. */
#define MCS_MSG_SUBCLASS_BS_STATE_GET_REPLY                                    \
    MCS_MSG_SUBCLASS_REPLY(BS_STATE_GET)

/*! Message subclass to set frequency adjustment. */
#define MCS_MSG_SUBCLASS_BS_FREQ_ADJUST_SET                                    \
    BS_FREQ_ADJUST_SET

/*! Reply message subclass to the Set BS time message. */
#define MCS_MSG_SUBCLASS_BS_FREQ_ADJUST_SET_REPLY                              \
    MCS_MSG_SUBCLASS_REPLY(BS_FREQ_ADJUST_SET)

/*! Reply message subclass to the Set frequency adjustment message. */
#define MCS_MSG_SUBCLASS_BS_PHASE_ADJUST_SET                                   \
    MCS_MSG_DMA_SET(BS_PHASE_ADJUST_SET)

/*! Reply message subclass to the Set BS time message. */
#define MCS_MSG_SUBCLASS_BS_PHASE_ADJUST_SET_REPLY                             \
    MCS_MSG_SUBCLASS_REPLY(BS_PHASE_ADJUST_SET)

/*! Message subclass to inform firmware about SDK deinit. */
#define MCS_MSG_SUBCLASS_BS_DEINIT_ENTRY                                       \
    BS_DEINIT_ENTRY

/*! Reply message subclass to the BS  message. */
#define MCS_MSG_SUBCLASS_BS_DEINIT_ENTRY_REPLY                                 \
    MCS_MSG_SUBCLASS_REPLY(BS_DEINIT_ENTRY)

/* MAX_MCS_MSG_CLASS should == the value of the highest MCS_MSG_CLASS */

/*! Maximum number of MCS Message Classes */
#define MAX_MCS_MSG_CLASS                7
...
typedef struct {
    /*! Higher bits of Timestamp. */
    uint32_t ts_hi;
    /*! Lower bits of Timestamp. */
    uint32_t ts_lo;
} mcs_msg_hw_ts_t;

/*! Max number of TS events */
#define MCS_MSG_MAX_TS_EVENTS (19)

/*! Area for storing most recent timestamps from timestamp FIFO
 * together with system (PTP / Broadsync) time.
 */
typedef struct {
    /*! Mask for TS event capture. */
    uint32_t capture_mask;
    /*! System time seconds (upper). */
    uint32_t full_time_seconds_hi;
    /*! System time seconds (lower). */
    uint32_t full_time_seconds_low;
    /*! System time nanoseconds. */
    uint32_t full_time_nanoseconds;

    /*! TS0 Timestamp value */
    mcs_msg_hw_ts_t ts0_timestamp;
    /*! TS1 Timestamp value */
    mcs_msg_hw_ts_t ts1_timestamp;

    /*! Most recent timestamp value of the TS event */
    mcs_msg_hw_ts_t event_timestamp[MCS_MSG_MAX_TS_EVENTS];
    /*! Previous timestamp value of the TS event */
    mcs_msg_hw_ts_t prev_event_timestamp[MCS_MSG_MAX_TS_EVENTS];
} mcs_msg_timestamps_t;
