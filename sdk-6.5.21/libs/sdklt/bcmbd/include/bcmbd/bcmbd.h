...
/*!
 * \brief HMI function type for reading register/memory.
 *
 * Currently only used by simulator hooks.
 *
 * \param [in] unit Unit number
 ...
 */
typedef int (*bcmbd_hmi_read_f)(int unit, uint32_t addrx, uint32_t addr,
                                void *vptr, int size);

/*!
 * \brief HMI function type for writing register/memory.
 *
 * Currently only used by simulator hooks.
 *
 * \param [in] unit Unit number
 ...
 * \return 0 if no errors, otherwise a negative value.
 */
typedef int (*bcmbd_hmi_write_f)(int unit, uint32_t addrx, uint32_t addr,
                                 void *vptr, int size);

/*!
 * \brief Function type for S-channel operations.
 *
 * Currently only used by simulator hooks.
 *
 * \param [in] unit Unit number.
 ...
 * \return 0 if no errors, otherwise a negative value.
 */
typedef int (*bcmbd_schan_op_f)(int unit, uint32_t *schan_msg,
 ...
 *
 * Device parameters are either constant for a given device family or
 * retrieved from OTP memory.
 *
 * Used by \ref bcmbd_dev_param_get.
 */
typedef enum bcmbd_dev_param_e {

    /*! Recommended AVS voltage in mV. */
    BCMBD_DEV_PRM_AVS_VOLTAGE,

    /*! Size of HMI command buffer (e.g. XGS SBUS command buffer). */
    BCMBD_DEV_PRM_HMI_CMDBUF_SIZE,
...
/*!
 * \brief Get device parameter value.
 *
 * \param [in] unit Unit number.
 * \param [in] dev_param Type of parameter to get.
 * \param [out] val Device parameter value.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_UNAVAIL Parameter not supported for this device.
 */
typedef int (*bcmbd_dev_param_get_f)(int unit, bcmbd_dev_param_t dev_param,
                                     uint32_t *val);
...
typedef struct bcmbd_drv_s {
    ...
    bcmbd_dev_param_get_f dev_param_get;
 ...
 * \param [in] num_entries Number of (consecutive) table entries to
 *                         read.
 * \param [in] addr Logical address of data buffer to read to in case
 *                  DMA is unavailable.
 * \param [in] dma_addr Physical bus address of data buffer to read
 *                      to.
 * \param [in] flags Please refer to \ref BCMBD_ROF_xxx.
 ...
 *
 * This function is a wrapper function for \ref bcmbd_pt_param_get.
 *
 * Please see \ref BCMBD_PT_PARAM_MOR_CLKS_READ for MOR details.
 *
 * \param [in] unit Unit number.
 ...
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_INIT Device driver not initialized.
 * \retval SHR_E_UNAVAIL Parameter not supported for this device.
 */
extern int
bcmbd_dev_param_get(int unit, bcmbd_dev_param_t dev_param, uint32_t *val);

/*!
 * \brief Get HMI command buffer size.
 *
 * If multiple outstanding HMI commands are not supported, then a
 * value of zero is returned.
 *
 * The HMI command buffer is architecture-specific. For CMIC-based
 * devices, the term refers to the SBUS command buffer size for MOR
 * (Multiple Outstanding Requests) support.
 *
 * This function is a wrapper function for \ref bcmbd_dev_param_get.
 *
 * \param [in] unit Unit number.
 *
 * \return Number of entries in the HMI command buffer.
 */
extern uint32_t
