...
def write_final_table_dict(tables_dict, out_fh, variant):
    ...
    if (len(tables_dict)) > 0:
        ...
        if variant == GENERIC_NAME:
            out_fh.write('\nstatic const issu_variant_info_t bcmissu_lt_ids_variant_%s = {\n'
                         % variant)
        else:
            out_fh.write('\nconst issu_variant_info_t bcmissu_lt_ids_variant_%s = {\n'
                         % variant)
...
def update_variant_map(device, variants, ver, fh):
    # Define the external variants
    for variant in variants:
        fh.write('extern const issu_variant_info_t bcmissu_lt_ids_variant_%s;\n'
                     % variant)
    # Define the variant array
    fh.write('\nstatic const issu_variant_info_t *variant_list[] = {\n')
    for variant in variants:
        fh.write('    &bcmissu_lt_ids_variant_%s,\n' % variant)
    fh.write('    NULL\n')
    fh.write('};\n')
...
def update_dev_map(dev_list, ver, out_fh):
    ...
    out_fh.write('    .ver = \"%s\",\n' % ver)
    out_fh.write('    .gen_variant = &bcmissu_lt_ids_variant_%s,\n' % GENERIC_NAME)
        ...
        for var in variants:
            ...
            for section in variant_info:
                if section == 'TABLE':
                    ...
                    out_f_name = output_dir + LTID_PREFIX + device.lower() + '_s_' + \
                                 variant + '_' + ver + '.' + LTID_SUFFIX
                    var_fh = open(out_f_name, 'w')
                    write_out_header(var_fh)
                    process_tbl_section(variant,
                                        variant_info[section],
                                        ver, var_fh, t_dict)
                    write_final_table_dict(t_dict, var_fh, variant)
                    var_fh.close()
                elif section == 'MAP':
...
def usage():
    ...
    print('The -c followed by device name indicate that the device should be included,')
    print('unless the -x is specified which makes the device excluded from the DB.')
    print('The -c <device> combo should be repeated for every device. If a variant(s)')
    print('associated with the device is also including the input should contain a single')
    print('-c <device> followed by one or more -t <variant> pairs\n')

    print('The -t followed by variant name is used to include this specific variant,')
    print('unless the -x is specified which makes the variant excluded from the DB.')
    print('The -t <variant> combo should be repeated for every variant. Note that the -c')
    print('option should be specified before specifying the variant\n')
    ...
    print('The -n option disables dependency checking and therefore all the files')
