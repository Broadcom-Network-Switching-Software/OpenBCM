...
class issu_version_list:
    ...
    def __comp_numbers(self, x, y):
        ...
        for i in range(start, max_cnt):
            ...
            if x[i].isalpha() and y[i].isalpha():
                ...
                return self.__comp_numbers(x[i:], y[i:])
    ...
    def ver_in_range(self, ver):
        val = self.__my_comp_func(self.start_ver, ver)
        if val > 0:
            return False
        val = self.__my_comp_func(ver, self.target_ver)
        if val > 0:
            return False
        return True
