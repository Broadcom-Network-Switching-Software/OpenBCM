...
int bcmimm_unit_init(int unit, uint8_t comp_id, bool warm)
{
    ...
    SHR_IF_ERR_EXIT(init_unit_tables(unit, warm, bcmissu_is_active()));
...
int bcmimm_tbl_dyn_info_alloc(int unit,
                              table_info_t *tbl,
                              bcmimm_table_dynamic_info_t *dyn_info)
{
    size_t num_fid;
    ...
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT
        (bcmlrd_table_attributes_get(unit, dyn_info->tbl_name, &tbl_attr));
    num_fid = tbl_attr.number_of_fields;

    SHR_ALLOC(f_info, sizeof(bcmlrd_client_field_info_t) * num_fid,
              "bcmimmInit");
    SHR_NULL_CHECK(f_info, SHR_E_MEMORY);
    SHR_IF_ERR_EXIT
        (bcmlrd_table_fields_def_get(unit, dyn_info->tbl_name,
                                     num_fid, f_info, &num_fid));
    SHR_IF_ERR_EXIT(alloc_table_dynamic_info(unit, tbl));
...
void bcmimm_tbl_dyn_info_free(bcmimm_table_dynamic_info_t *dyn_info)
{
    SHR_FREE(dyn_info->data_fields);
    SHR_FREE(dyn_info->key_fields);
}
