...
/*!
 * \brief Restore the values of a back-end table with new key.
 *
 * This function restores the entries that were saved before under their new
 * transformed key. Note that the key transform can be NULL which means that
 * there is no key transform.
 *
 * \param [in] unit The associated unit.
 * \param [in] rows The number of entries to restore.
 * \param [in] entry_size Size of each entry.
 * \param [in] tbl_buf The buffer where all the entries are stored. It is a
 * matrix of \c rows entries where each entry is of size \c entry_size.
 * \param [in] be_in Assist buffer to store the input for the back-end table.
 * \param [in] key_size The combined size of the key fields (in bytes).
 * \param [in] tbl The IMM table information.
 * \param [in] enum_type_name If key transformation required use this parameter
 * to perform the transform.
 * \param [in] key_val_convert_func The transform function.
 *
 * \return SHR_E_NONE Success.
 */
static int restore_be_values(int unit,
                             size_t rows,
                             uint32_t entry_size,
                             uint8_t *tbl_buf,
                             ...
                             const char *enum_type_name,
                             uint32_t fid,
                             int (*key_val_convert_func)(int unit,
                                                    const char *enum_type_name,
                                                    uint32_t key_val,
                                                    uint32_t *new_key_val))
{
    uint32_t j;
    uint32_t *fld_cnt;
    uint8_t *data;
    uint32_t key_val;
    uint8_t new_key_buf[8];
    bcmimm_table_dynamic_info_t *dyn_info = 0;
    int rv;
    size_t num_of_fields = field_buf[unit].count;
    int idx = 0;

    /* Do not exist the function from this loop!!! */
    for (j = 0; j < rows; j++) {
        ...
        key_val = 0;
        /*
         * Convert the enum value to its new value. Note that there might be
         * other fields composing the key. We need to keep the other fields
         * in tact so only the enum portion will change.
         */
        if (key_val_convert_func) {
            if (tbl->key_fld_cnt > 1) {
                int k;
                dyn_info = &bcmimm_dynamic_info[unit][tbl->fields_idx];
                sal_memset(new_key_buf, 0, sizeof(new_key_buf));

                /*
                 * At this point the dynamic info was not allocated yet.
                 * This information is needed to extract and build the keys
                 * so we are allocating it here. Note that the memory will
                 * not be reallocated later as the key/data fields buffers
                 * will not be NULL anymore.
                 */
                bcmimm_tbl_dyn_info_alloc(unit, tbl, dyn_info);
                for (k = 0; k < tbl->key_fld_cnt; k++) {
                    bcmimm_extract_key_fld(tbl, dyn_info, (uint8_t *)fld_cnt,
                                           k, field_buf[unit].field[k]);
                    if (dyn_info->key_fields[k].fid == fid) {
                        idx = k;
                        key_val = (uint32_t)field_buf[unit].field[k]->data;
                    }
                }
            } else {
                assert (key_size <= sizeof(uint32_t));
                sal_memcpy(&key_val, fld_cnt, key_size);
            }
            rv = key_val_convert_func(unit, enum_type_name,
                                      key_val, &key_val);
            if (SHR_FAILURE(rv)) {
                continue;  /* Enum value may have been deleted */
            }
            if (tbl->key_fld_cnt > 1) {
                field_buf[unit].field[idx]->data = key_val;
                field_buf[unit].count = tbl->key_fld_cnt;
                if (bcmimm_key_build(tbl, dyn_info, &field_buf[unit],
                                     new_key_buf) != SHR_E_NONE) {
                    continue;
                }
                bcmimm_tbl_dyn_info_free(dyn_info);
            } else {
                sal_memcpy(new_key_buf, &key_val, key_size);
            }
        } else {
            sal_memcpy(new_key_buf, fld_cnt, key_size);
        }
        data = (uint8_t *)fld_cnt;
        data += key_size;
        sal_memcpy(be_in->fid, data, BCMIMM_BE_FID_SIZE * be_in->count);
        data += BCMIMM_BE_FID_SIZE * be_in->count;
        sal_memcpy(be_in->fdata, data, tbl->f_data_size * be_in->count);
        rv = bcmimm_be_entry_insert(tbl->blk_hdl, new_key_buf, be_in);
        if (SHR_FAILURE(rv)) {
            dyn_info = &bcmimm_dynamic_info[unit][tbl->fields_idx];
    ...
    field_buf[unit].count = num_of_fields;
    return SHR_E_NONE;
}
                                    ...
                                    uint8_t *data_buf)
{
    ...
    if (tbl->type == IMM_BASIC) {
        ...
        if (tbl->array_idx_size) {
            entry_size = sizeof(bcmimm_be_direct_tbl_field_array_t);
        } else {
            entry_size = sizeof(bcmimm_be_direct_tbl_field_basic_t);
        }
        entry_size *= data_field_cnt;
    }
    ...
    restore_be_values(unit, rows, entry_size, tbl_buf,
                      &be_out, key_size, tbl, enum_type_name, fid,
                      bcmissu_enum_sym_to_current);
    sal_free(tbl_buf);
 ...
 * \param [out] ent_cnt Number of entries in the back-end table.
 * \param [out] ent_size The max size of each entry.
 *
 * \return Pointer to buffer that contains the content of the previous table or
 * NULL if the content of the table was empty.
 */
static uint8_t *handle_table_handler_change(int unit,
                                            table_info_t *tbl,
                                            size_t *ent_cnt,
                                            uint32_t *ent_size)
{
    int rv;
    size_t key_size;
    uint8_t *tbl_buf = NULL;
    size_t rows;
    uint32_t entry_size;
    uint64_t in_key_buf, out_key_buf;
    bcmimm_be_fields_t be_out;
    ...
    rows = bcmimm_be_table_size_get(tbl->blk_hdl);
    /* If there is data save it into a temporary buffer */
    if (rows > 0 && SHR_SUCCESS(rv)) {
        if (tbl->type == IMM_BASIC) {
            entry_size = (BCMIMM_BE_FID_SIZE + tbl->f_data_size);
        } else {
            if (tbl->array_idx_size) {
                entry_size = sizeof(bcmimm_be_direct_tbl_field_array_t);
            } else {
        ...
        entry_size *= tbl->max_num_field;
        /* Every entry will start with the field count followed by the key value */
        entry_size += sizeof(uint32_t) + key_size;
        /* Round the entry size to multiple of 4. */
        entry_size = ((entry_size + 3) / 4) * 4;
        be_out.count = tbl->max_num_field;
        be_out.fid = sal_alloc(entry_size, "bcmimmIssu");
        if (!be_out.fid) {
            return NULL;
        }
        if (tbl->type == IMM_BASIC) {
            be_out.fdata = ((uint8_t *)be_out.fid) +
                    BCMIMM_BE_FID_SIZE * tbl->max_num_field;
        } else {
            be_out.fdata = (uint8_t *)be_out.fid;
        }
        tbl_buf = copy_be_tbl_to_local_buf(tbl, rows, entry_size, &be_out,
    ...
    for (idx = 0; idx < bcmimm_tables_list[unit]->allocated; idx++) {
        ...
        do {
            ...
            if (delete_tbl) {
                ...
                uint16_t fld_idx;
                tbl_buf = handle_table_handler_change(unit, tbl,
                                                      &ent_cnt, &ent_size);
                if (tbl_buf) {
                    sal_free(tbl_buf);
                    tbl_buf = NULL;
        ...
        if ((tbl->type == IMM_BASIC && idx_in_tbls >= imm_lts) ||
            ...
            tbl_buf = handle_table_handler_change(unit, tbl, &ent_cnt, &ent_size);
            if (tbl->key_fld_cnt == 0) {
                orig_key_size = 1;  /* Key size must be minimum 1 byte */
            } else {
                orig_key_size = BCMIMM_BITS_TO_BYTES(tbl->key_arry_size);
            }
            be_out.count = tbl->max_num_field;
        }
        ...
        if (tbl_buf) {
            be_out.fid = sal_alloc(ent_size, "bcmimmIssu");
            if (!be_out.fid) {
                return SHR_E_MEMORY;
            }
            be_out.fdata = ((uint8_t *)be_out.fid) +
                    BCMIMM_BE_FID_SIZE * be_out.count;
            /* Restore the saved data */
            restore_be_values(unit, ent_cnt, ent_size, tbl_buf,
                              &be_out, orig_key_size, tbl, NULL, 0, NULL);
