...
/*!
 * \brief Convert a pointer to memory onto uint64 value based on key length
 *
 * This function should work for big and little endians. In extract the value
 * pointed by key and has key_len bytes of length into a 64 bit integer.
 *
 * \param [in] key The value to extract.
 * \param [in] key_len The length of the value to extract in bytes.
 *
 * \return 64 bit integer.
 */
static uint64_t pointer_to_key_val(const void *key, uint32_t key_len)
{
    uint64_t val = 0;
                               ...
                               uint32_t key_len)
{
    ...
    if (key_len <= sizeof(uint64_t)) {
        key_val = pointer_to_key_val(key, key_len);
    }
    while (start <= end) {
        idx = (start + end) / 2;
        tmp_key = buf + idx * key_len;
        if (key_len <= sizeof(uint64_t)) {
            tmp_key_val = pointer_to_key_val(tmp_key, key_len);
...
static int key_comp(const void *key1, const void *key2)
{
    if (sort_key_len <= sizeof(uint64_t)) {
        uint64_t val1, val2;
        val1 = pointer_to_key_val(key1, sort_key_len);
        val2 = pointer_to_key_val(key2, sort_key_len);
        if (val1 < val2) {
            return -1;
        } else if (val1 > val2) {
            return 1;
...
int bcmimm_prep_traverse_buf(bcmimm_tbl_info_t *t_info)
{
    ...
    sal_memset(t_info->traverse_buf, 0, key_len * j);
 ...
 */
int bcmimm_be_generic_init(bool big_endian)
{
    sort_mutex = sal_mutex_create("bcmImmBeTrvrsMtx");
    if (sort_mutex == NULL) {
        return SHR_E_MEMORY;
    }
    be_big_endian = big_endian;
    return SHR_E_NONE;
}
