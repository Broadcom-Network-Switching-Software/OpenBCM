                          ...
                          bcmpkt_trace_data_t *trace_data)
{
    ...
    hid = *(hid_support + BCMPKT_GENERIC_LOOPBACK_T);
    ...
    if (hid == -1) {
        ...
        SHR_IF_ERR_EXIT(bcmpkt_flexhdr_field_id_get(variant, hid, "START_BYTE",
                                                    &fid));
        SHR_IF_ERR_EXIT
            (bcmpkt_flexhdr_field_set(variant, hid, lbhdr,
                                      BCMPKT_FLEXHDR_PROFILE_NONE, fid, 0x82));

        SHR_IF_ERR_EXIT(bcmpkt_flexhdr_field_id_get(variant, hid,
                                                    "SOURCE_SYSTEM_PORT",
                                                    &fid));
        SHR_IF_ERR_EXIT
            (bcmpkt_flexhdr_field_set(variant, hid, lbhdr,
                                      BCMPKT_FLEXHDR_PROFILE_NONE, fid, system_source));

        SHR_IF_ERR_EXIT(bcmpkt_flexhdr_field_id_get(variant, hid,
                                                    "FLAGS", &fid));
        SHR_IF_ERR_EXIT
            (bcmpkt_flexhdr_field_set(variant, hid, lbhdr,
                                      BCMPKT_FLEXHDR_PROFILE_NONE, fid, 1));
    }
