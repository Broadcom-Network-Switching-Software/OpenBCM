...
__cint_operator_Assign(cint_ast_t* ast, cint_operand_type_t optype, cint_variable_t* left, cint_variable_t* right)
{
    ...
    if(lflags && rflags) {
        ...
        switch(lflags & CINT_ATOMIC_TYPE_FLAGS_MASK)
            ...
            case CINT_ATOMIC_TYPE_F_BOOL:        cint_bool_set(left, cint_bool_value(right)); return left;
    ...
    if( (left->dt.desc.pcount == 0) &&
        ...
        if(left->dt.cap && left->dt.cap->assign) {
            ...
            cint_error_t rc1 = left->dt.basetype.ap->assign(left->data, *(char**)right->data);
            if(rc1 != CINT_E_NONE) {
                cint_ast_error(ast, rc1, "datatype '%s' assignment error", cint_datatype_format(&left->dt,0));
                return NULL;
            }
        }
        else {
...
cint_eval_operator_internal(cint_ast_t* ast, cint_operator_t op, cint_variable_t* left, cint_variable_t* right)
{
    ...
    if( (left && left->dt.desc.pcount) ||
        ...
        unsigned flags1 = cint_atomic_flags(right) | cint_atomic_flags(left);

        switch(flags1 & CINT_ATOMIC_TYPE_FLAGS_MASK)
            {
        ...
        optype += ( (flags1 & CINT_ATOMIC_TYPE_F_UNSIGNED) != 0);
    }
