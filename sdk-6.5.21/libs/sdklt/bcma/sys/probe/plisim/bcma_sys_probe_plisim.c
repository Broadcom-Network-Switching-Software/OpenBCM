...
#define PLI_PCI_ILINE(_sd)      ((_sd)->pci_dev.dev_no)
...
static int
schan_op(int unit, uint32_t *words, int dw_write, int dw_read)
{
    sys_dev_t *sd = &sys_devs[unit];

    return pli_schan(sd->pci_dev.dev_no, words, dw_write, dw_read);
}
         ...
         uint8_t pci_revision)
{
    ...
    /* PLI device handle */
    sal_memcpy(&sd->pci_dev, dev, sizeof(sd->pci_dev));
    ...
    if (use_schan_rpc) {
        bcmbd_simhook_schan_op[unit] = schan_op;
    }
    ...
    if (sd->mpool == NULL) {
        LOG_WARN(BSL_LS_SYS_PCI,
                 (BSL_META_U(unit,
                             "Unable to create DMA memory manager.\n")));
        bcmdrd_dev_destroy(unit);
        return 0;
    }
    dma_offs = (unsigned long)sd->dma_pool;
    dma_offs &= ~0xffffffffUL;
    pci_dma_offset_set(dev->dev_no, dma_offs);
...
/* Last entry must remain a null entry */
#define NUM_CFG_OPT (COUNTOF(cfg_opt) - 1)
...
bcma_sys_probe_plisim_cleanup(void)
{
    ...
    for (unit = 0; unit < BCMDRD_CONFIG_MAX_UNITS; unit++) {
        ...
        if (sd->owned) {
            ...
            bcmbd_simhook_schan_op[unit] = NULL;
        }
        if (sd->intr_lock) {
