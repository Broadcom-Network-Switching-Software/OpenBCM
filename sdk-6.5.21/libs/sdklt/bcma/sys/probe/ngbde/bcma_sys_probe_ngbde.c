...
intr_disconnect(void *devh, int irq_num)
{
    ...
    /*
     * Note that interrupts remain enabled in kernel if KNET is
     * active. This ensures that KNET-based Linux network interfaces
     * continue to work during a warm boot.
     */
 ...
 *   1. Mask further hardware interrupts until user mode processing is
 *      complete.
 *   2. Determine which interrupt sources should be handled in the
 *      kernel (KNET) and which ones should be handled in user mode.
 *   3. Ackknowledge interrupts (if required).
 *
 * \param [in] devh Device context.
 * \param [in] irq_num MSI interrupt number.
...
bcma_sys_probe_ngbde(void)
{
    ...
    if (bcmlu_ngbde_init() != 0) {
        ...
        if (geteuid() != 0) {
            LOG_WARN(BSL_LS_SYS_PCI,
                     (BSL_META("BDE access requires root privileges\n")));
        }
        return 0;
    }
    ...
    if (bcmlu_ngbde_num_dev_get(&bde_devs) < 0) {
        LOG_ERROR(BSL_LS_SYS_PCI,
                  (BSL_META("Unable to get number of BDE devices\n")));
        return 0;
    }
    ...
    for (bdx = 0; bdx < bde_devs; bdx++) {
        ...
        if (bcmlu_ngbde_dev_info_get(bdx, di) < 0) {
            LOG_ERROR(BSL_LS_SYS_PCI,
                      (BSL_META("Unable to get BDE device info\n")));
            return ndevs;
        }
        ...
        /* Overrides assume that unit numbers start at zero */
        sd = &sys_devs[ndevs];
