...
static int
cmd_compare(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    const char *arg;
    int rv;
    int unit = cli->cmd_unit;
    ...
    arg = BCMA_CLI_ARG_GET(args);
    rv = bcma_ha_mem_state_comp(unit, arg);
    if (SHR_FAILURE(rv)) {
...
static int
cmd_dump(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    const char *arg;
    int rv;
    int unit = cli->cmd_unit;
    ...
    arg = BCMA_CLI_ARG_GET(args);
    rv = bcma_ha_mem_state_dump(unit, arg);
    if (SHR_FAILURE(rv)) {
        return BCMA_CLI_CMD_FAIL;
    }
...
bcma_hacmd_ha(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    if (sal_strcasecmp(arg, "alloc") == 0) {
        ...
        return cmd_compare(cli, args);
    } else if (sal_strcasecmp(arg, "dump") == 0) {
        return cmd_dump(cli, args);
    }
