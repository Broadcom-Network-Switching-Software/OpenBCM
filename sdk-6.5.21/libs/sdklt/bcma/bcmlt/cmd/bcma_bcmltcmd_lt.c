...
{
    bcmlt_opcode_t opc;
    const bcma_cli_parse_enum_t *e = bcma_bcmlt_opcode_enum_get(false);
    ...
    if (commit_params->async) {
        ...
        if (SHR_FAILURE(rv)) {
            cli_out("%slt async=1 %s %s: "BCMA_BCMLT_ERRMSG_FMT".\n",
                    ...
                    bcma_bcmlt_opcode_format(opc, false),
    ...
    for (idx = 0, ei = ti->ei_head; idx < bcmlt_ti.num_entries && ei;
        ...
        if (verbose) {
            cli_out("  #%3d Entry: %s %s\n", idx + 1, bcmlt_ei.table_name,
                    bcma_bcmlt_opcode_format_by_entry_info(&bcmlt_ei,
                                                           NULL, NULL));
        }
        /* Check entry status */
        rv = bcmlt_ei.status;
...
lt_table_op_pre_commit_entry(bcma_bcmlt_entry_info_t *ei, bool new_entry)
{
    ...
    shr_pb_printf(pb, "  %s ", bcma_bcmlt_opcode_format(opc, false));
                         ...
                         bcmlt_entry_info_t  *entry_info, void *usr_data)
{
    ...
    LOG_VERBOSE(BSL_LS_APPL_BCMLT,
                (BSL_META("lt %s asynchronous commit notify-%s: %s event\n"),
                 ...
                 bcma_bcmlt_notification_format(entry_info->notif_opt),
                 bcma_bcmlt_notification_format(event)));
                               ...
                               void *usr_data)
{
    ...
    LOG_VERBOSE(BSL_LS_APPL_BCMLT,
                (BSL_META("lt asynchronous transaction commit notify-%s: "
                          "%s event\n"),
                 bcma_bcmlt_notification_format(trans_info->notif_opt),
                 bcma_bcmlt_notification_format(event)));
...
op_commit_traverse(bcma_bcmlt_entry_info_t *ei)
{
    ...
    if (cnt >= 0) {
        cli_out("  %d entr%s traversed", cnt, (cnt == 1) ? "y" : "ies");
        if (teci.qlfr) {
            cli_out(". %d entr%s matching",
                    teci.cnt, (teci.cnt == 1) ? "y" : "ies");
        }
        if (opcode == BCMLT_OPCODE_NOP) {
            cli_out(".\n");
        } else {
            cli_out(" for %s operation.\n",
                    bcma_bcmlt_opcode_format(opcode, false));
        }
    ...
    if (commit_params->async) {
        LOG_INFO(BSL_LS_APPL_BCMLT,
                 (BSL_META("lt %s %s: Entry commit in asynchronous mode, "
                  ...
                  ei->name, bcma_bcmlt_opcode_format(opc, false),
                  bcma_bcmlt_notification_format(commit_params->notify),
                  bcma_bcmlt_priority_format(commit_params->priority)));
        ...
        if (SHR_FAILURE(rv)) {
            ...
            cli_out("%slt %s %s: "BCMA_BCMLT_ERRMSG_FMT".\n",
                    BCMA_CLI_CONFIG_ERROR_STR, ei->name,
                    bcma_bcmlt_opcode_format(opc, false), BCMA_BCMLT_ERRMSG(rv));
            return BCMA_CLI_CMD_FAIL;
        }
        ...
        while ((opt = *opts++) != '\0') {
            if (!lt_opcode_option_check(ti, ei, ei->opcode, opt)) {
                cli_out("%sOption '%c' is not supported in %s "
                        "operation%s.\n", BCMA_CLI_CONFIG_ERROR_STR,
                        opt, bcma_bcmlt_opcode_format(ei->opcode, false),
