...
/*
 * Set the tables match number to local variable '0'
 */
static int
result_var_set(bcma_cli_t *cli, int val)
{
    char val_str[32];

    sal_snprintf(val_str, sizeof(val_str), "%d", val);
    return bcma_cli_var_set(cli, "0", val_str, TRUE);
}
...
bcmltcmd_parse_attr(bcma_cli_args_t *args)
{
    ...
    if (sal_strncasecmp(arg, cmp_str, cmp_strlen) == 0) {
        ...
        for (idx = 0; idx < tok->argc; idx++) {
            const bcma_cli_parse_enum_t *e = bcma_bcmlt_ent_attr_enum_get();
            int match = 0;
                          ...
                          bcmltcmd_params_t *cmd_params)
{
    ...
    const bcma_cli_parse_enum_t *trans_type_enum;
    const bcma_cli_parse_enum_t *commit_pri_enum;
    const bcma_cli_parse_enum_t *notify_opt_enum;

    if (!cmd_params) {
        return -1;
    }

    trans_type_enum = bcma_bcmlt_trans_type_enum_get();
    commit_pri_enum = bcma_bcmlt_commit_pri_enum_get();
    notify_opt_enum = bcma_bcmlt_notify_opt_enum_get();
