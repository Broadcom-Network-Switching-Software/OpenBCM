...
typedef  int (*ctr_ent_op_f)(void * cookie, bcmlt_entry_handle_t eh,
                             ctr_info_t *ci, int port, int offset);
...
entry_def_field_find(bcmlt_field_def_t *def_fields, int num_def_fields,
                     const char *name)
{
    int idx;
    bcmlt_field_def_t *df;

    for (idx = 0; idx < num_def_fields; idx++) {
        df = &def_fields[idx];
...
ctr_info_create(int unit, const char *ctr_lt)
{
    ...
    ci->name = ei->name;
    ci->num_def_fields = ei->num_def_fields;
    alloc_size = sizeof(bcmlt_field_def_t) * ci->num_def_fields;
    ci->def_fields = sal_alloc(alloc_size, "bcmaCtrInfoDefFields");
    if (ci->def_fields == NULL) {
        bcma_bcmlt_entry_info_destroy(ei);
        ctr_info_destroy(ci);
        return NULL;
    }
    sal_memcpy(ci->def_fields, ei->def_fields, alloc_size);
    bcma_bcmlt_entry_info_destroy(ei);
    ei = NULL;
     ...
     */
    df = entry_def_field_find(ci->def_fields, ci->num_def_fields, "PORT_ID");
    if (df && bcma_bcmlt_field_flags(df) == BCMA_BCMLT_FIELD_F_KEY) {
        ci->port_id_key = df;
        df = entry_def_field_find(ci->def_fields, ci->num_def_fields,
                                  "OPERATIONAL_STATE");
    ...
    for (idx = ci->num_def_fields - 1; idx >= 0; idx--) {
        df = &ci->def_fields[idx];
    ...
    alloc_size = ci->num_def_fields * sizeof(*ci->cache_fields_attr);
    ci->cache_fields_attr = sal_alloc(alloc_size, "bcmaCtrInfoCFAttr");
    if (ci->cache_fields_attr == NULL) {
        ctr_info_destroy(ci);
        return NULL;
    }
    for (idx = 0; idx < ci->num_def_fields; idx++) {
        df = &ci->def_fields[idx];
    ...
    cache_size = num_entries * ci->num_def_fields;

    /* Create the cache for storing the previous counter value */
    alloc_size = cache_size * sizeof(*ci->cache);
    ci->cache = sal_alloc(alloc_size, "bcmaCtrInfoCache");
    if (ci->cache == NULL) {
        ctr_info_destroy(ci);
        return NULL;
    }
    sal_memset(ci->cache, 0, alloc_size);

    /* Set the counter type if this is a pre-defined default counter */
    e = &def_counters_enum[0];
    while (e->name) {
        if (sal_strcmp(e->name, ci->name) == 0) {
            ci->types = e->val;
            break;
        }
        e++;
...
ctr_info_get(int unit, const char *ctr_lt)
{
    ...
    while (ci) {
        if (sal_strcasecmp(ci->name, ctr_lt) == 0) {
            return ci;
        }
        ci = ci->next;
    }
...
cache_entry_index(ctr_info_t *ci, int port, int offset)
{
    ...
    return offset * ci->num_def_fields;
}

/* Callback function to show counter value for each valid entry */
static int
ctr_entry_show(void *cookie, bcmlt_entry_handle_t eh,
               ctr_info_t *ci, int port, int offset)
{
    ...
    for (idx = 0; idx < ci->num_def_fields; idx++) {
        if (entry_field_is_cached(ci, idx)) {
            df = &ci->def_fields[idx];
            rv = bcmlt_entry_field_get(eh, df->name, &val);
            if (SHR_FAILURE(rv)) {
                continue;
            }
            cidx = entry_base + idx;
            ...
            if ((((flags & CTR_SHOW_CHANGED) && changed) ||
                ...
                shr_pb_printf(pb, "%s.%s", ci->name, df->name);
                for (kidx = 0; kidx < MAX_NUM_MISC_KEYS; kidx++) {
                    key = &ci->misc_keys[kidx];
                    if (key->name == NULL) {
                        break;
...
static int
ctr_entry_clear(void *cookie, bcmlt_entry_handle_t eh,
                ctr_info_t *ci, int port, int offset)
{
    ...
    for (idx = 0; idx < ci->num_def_fields; idx++) {
        df = &ci->def_fields[idx];
        if (entry_field_is_cached(ci, idx)) {
            rv = bcmlt_entry_field_add(eh, df->name, 0);
            if (SHR_FAILURE(rv)) {
                LOG_VERBOSE(BSL_LS_APPL_BCMLT,
                            (BSL_META("%s.%s: bcmlt_entry_field_add failed: "
                                      BCMA_BCMLT_ERRMSG_FMT"\n"),
                             ci->name, df->name, BCMA_BCMLT_ERRMSG(rv)));
            }
        } else if (!df->key) {
            bcmlt_entry_field_unset(eh, df->name);
        }
    }
    ...
    if (SHR_SUCCESS(rv)) {
        ...
        LOG_VERBOSE(BSL_LS_APPL_BCMLT,
                    (BSL_META("%s: update operation failed: "
                              BCMA_BCMLT_ERRMSG_FMT"\n"),
                     ci->name, BCMA_BCMLT_ERRMSG(rv)));
    }
                        ...
                        ctr_ent_op_f cb, void *cookie)
{
    ...
    rv = bcmlt_entry_allocate(unit, ci->name, &eh);
    if (SHR_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_APPL_BCMLT,
                    (BSL_META_U(unit,
                                "%s: bcmlt_entry_allocate failed: "
                                BCMA_BCMLT_ERRMSG_FMT"\n"),
                     ci->name, BCMA_BCMLT_ERRMSG(rv)));
        return -1;
    }
    ...
    if (ci->port_id_key) {
        rv = bcmlt_entry_field_add(eh, ci->port_id_key->name, port);
        if (SHR_FAILURE(rv)) {
            LOG_VERBOSE(BSL_LS_APPL_BCMLT,
                        (BSL_META_U(unit,
                                    "%s.%s: bcmlt_entry_field_add failed: "
                                    BCMA_BCMLT_ERRMSG_FMT"\n"),
                         ci->name, ci->port_id_key->name,
    ...
    for (idx = 0; idx < ci->num_misc_key_entries; idx++) {
        for (kidx = 0; kidx < MAX_NUM_MISC_KEYS; kidx++) {
            ...
            rv = bcmlt_entry_field_add(eh, key->name, key_val);
            if (SHR_FAILURE(rv)) {
                LOG_VERBOSE(BSL_LS_APPL_BCMLT,
                            (BSL_META_U(unit,
                                        "%s.%s: bcmlt_entry_field_add failed: "
                                        BCMA_BCMLT_ERRMSG_FMT"\n"),
                             ci->name, key->name, BCMA_BCMLT_ERRMSG(rv)));
            }
        }
            ...
            if (rv != SHR_E_NOT_FOUND) {
                LOG_VERBOSE(BSL_LS_APPL_BCMLT,
                            (BSL_META_U(unit,
                                        ...
                                        BCMA_BCMLT_ERRMSG_FMT"\n"),
                             ci->name, port, BCMA_BCMLT_ERRMSG(rv)));
            }
            continue;
        }
        /* Check if the entry is valid for port-based counter table */
        if (ci->port_id_key) {
            rv = bcmlt_entry_field_symbol_get(eh, "OPERATIONAL_STATE",
                                              &opstr);
...
static int
counters_iterate(int unit, bcmdrd_pbmp_t *pbmp, ctr_ent_op_f cb, void *cookie)
{
    int port;
    ctr_info_t *ci;
