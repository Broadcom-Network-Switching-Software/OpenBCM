 ...
 * If the returned qualifier is not NULL, the qualifer string will be set to
 * qfptr. Otherwise qfptr will be set to NULL.
 * The qualifier is only supported in LT traverse operations.
 */
static const bcma_cli_parse_enum_t *
trvs_qualifier(bcma_bcmlt_entry_info_t *ei, char *str, char **qfptr)
{
    ...
    *qfptr = NULL;
    ...
    while (e->name != NULL) {
        if ((*qfptr = sal_strstr(str, e->name)) != NULL) {
            break;
        }
        e++;
    }
                             ...
                             bcma_bcmlt_field_t *field, char *field_arg)
{
    ...
    for (idx = 0; idx < ei->num_def_fields; idx++) {
        ...
        if (sal_strncasecmp(field_arg, df->name, len) == 0) {
             ...
             */
            qualifier = trvs_qualifier(ei, &field_arg[len], &ptr);
            if ((qualifier && ptr == &field_arg[len]) ||
                field_arg[len] == '=' ||
    ...
    if (idx == ei->num_def_fields) {
        (void)trvs_qualifier(ei, field_arg, &ptr);
        if (ptr == NULL) {
