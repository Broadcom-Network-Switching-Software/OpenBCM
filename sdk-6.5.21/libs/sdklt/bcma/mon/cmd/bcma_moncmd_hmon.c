...
static int
bcma_moncmd_hmon_temp_get(int unit, uint32_t equation)
{
    size_t count;
    size_t sensor_count;
    size_t ix;
    ...
    if (count != 0) {
        ...
        if (equation == PVT_EQUATION_1) {
            ...
            for (ix = 0; ix < count; ix++) {
                ...
                cli_out("\t\t%.1f\t\t%.1f\t\t%.1f\t\t%s\n",
                        uptr_sec->current, uptr_sec->min, uptr_sec->max,
                        "Quadratic");
                uptr++;
                uptr_sec++;
            }
...
bcma_moncmd_hmon(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    if (bcma_cli_parse_cmp("TEMPerature", cmd, 0)) {
        /* Call bcmbd api to do actual work */
        if (detail) {
            bcma_moncmd_hmon_temp_get(unit, PVT_EQUATION_2);
        } else {
            bcma_moncmd_hmon_temp_get(unit, PVT_EQUATION_1);
        }
    } else {
        return BCMA_CLI_CMD_USAGE;
    }
