...
generic_loopback_parse_arg(const char *arg, void *data, void **option)
{
    ...
    bcmpkt_flexhdr_header_id_get(generic_loopback->variant,
                                 "GENERIC_LOOPBACK_T", &hid);

    if (bcmpkt_flexhdr_field_id_get(generic_loopback->variant, hid,
                                    "START_BYTE", &fid) ||
        bcmpkt_flexhdr_field_get(generic_loopback->variant, hid,
                                 generic_loopback->data,
                                 BCMPKT_FLEXHDR_PROFILE_NONE, fid, &val)) {
        return -1;
    }
    if (val == 0) {
    ...
    return bcmpkt_flexhdr_field_set(generic_loopback->variant, hid,
                                    generic_loopback->data,
                                    BCMPKT_FLEXHDR_PROFILE_NONE, fid, val);
}
...
generic_loopback_format_arg(const void *data, const void *option, char *buf, int bufsz)
{
    ...
    if (!bcmpkt_flexhdr_field_get(generic_loopback->variant, hid,
        generic_loopback->data, BCMPKT_FLEXHDR_PROFILE_NONE, fid, &val)) {
        len = sal_snprintf(buf, bufsz, "0x%x(%d)", val, val);
    }
...
higig3_parse_arg(const char *arg, void *data, void **option)
{
    ...
    bcmpkt_flex_field_info_t hg3_field_info;
    uint32_t val;
    int fid;
    uint32_t hid_base, hid_ext;
    ...
    bcmpkt_flexhdr_header_id_get(higig3->variant, "HG3_BASE_T", &hid_base);
    bcmpkt_flexhdr_header_id_get(higig3->variant, "HG3_EXTENSION_0_T", &hid_ext);
    bcmpkt_flexhdr_field_info_get(higig3->variant, hid_base, &hg3_field_info);

    if (bcmpkt_flexhdr_field_id_get(higig3->variant, hid_base, "RESERVED_ETYPE",
        &fid) || bcmpkt_flexhdr_field_get(higig3->variant, hid_base, higig3->data,
        BCMPKT_FLEXHDR_PROFILE_NONE, fid, &val)) {
        return -1;
    }
     ...
     */
    fid = **(int **) option;
    if (fid < hg3_field_info.num_fields) {
        return bcmpkt_flexhdr_field_set(higig3->variant, hid_base,
                                        higig3->data,
                                        BCMPKT_FLEXHDR_PROFILE_NONE, fid, val);
    } else {
        return bcmpkt_flexhdr_field_set(higig3->variant, hid_ext,
                                        higig3->data +
                                        BCMPKT_HG3_BASE_HEADER_SIZE_WORDS,
                                        BCMPKT_FLEXHDR_PROFILE_NONE,
                                        fid - hg3_field_info.num_fields, val);
    }
}
...
higig3_format_arg(const void *data, const void *option, char *buf, int bufsz)
{
    ...
    uint32_t val;
    int fid;
    uint32_t hg3_base_hid, hg3_ext_hid;
    ...
    bcmpkt_flexhdr_header_id_get(higig3->variant, "HG3_BASE_T", &hg3_base_hid);
    bcmpkt_flexhdr_header_id_get(higig3->variant, "HG3_EXTENSION_0_T",
                                 &hg3_ext_hid);
    bcmpkt_flexhdr_field_info_get(higig3->variant, hg3_base_hid, &hg3_field_info);

    /* Call base header field get or extension header field get based on fid's
     * value
     */
    if (fid < hg3_field_info.num_fields) {
        if (!bcmpkt_flexhdr_field_get(higig3->variant, hg3_base_hid,
                                      higig3->data,
                                      BCMPKT_FLEXHDR_PROFILE_NONE, fid, &val)) {
            len = sal_snprintf(buf, bufsz, "0x%x(%d)", val, val);
        }
    } else {
        if (!bcmpkt_flexhdr_field_get(higig3->variant, hg3_ext_hid,
                                      higig3->data,
                                      BCMPKT_FLEXHDR_PROFILE_NONE, fid -
                                      hg3_field_info.num_fields, &val)) {
            len = sal_snprintf(buf, bufsz, "0x%x(%d)", val, val);
        }
    }
