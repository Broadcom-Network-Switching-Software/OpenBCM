...
int
bcma_bcmpkt_test_cpu_stats_get(cpu_stats_t *cpu_stats)
{
...
void
bcma_bcmpkt_test_stats_dump(int time_diff, uint64_t pkt_count,
                            uint64_t pkt_count_2, uint32_t pkt_len,
                            cpu_stats_t *cpu_end, cpu_stats_t *cpu_start,
                            bool show_time, bool show_pkt_count_2)
{
    int bps = 0;
    int pps = 0;
    int total_diff = 0;
    ...
    cli_out("  %5"PRId32" |", pkt_len);
    if (time_diff > 0) {
        pps = (int)((1000UL * pkt_count) / time_diff);
        bps = (int)((pkt_len * pkt_count) / time_diff);
    }
    cli_out("%4"PRId32".%03"PRId32" |%4"PRId32".%03"PRId32" |%8"PRId32" |",
            (pps / 1000), (pps % 1000), (bps / 1000), (bps % 1000), (int)pkt_count);
    if (show_pkt_count_2 == true) {
        if (time_diff > 0) {
            pps = (int)((1000UL * pkt_count_2) / time_diff);
            bps = (int)((pkt_len * pkt_count_2) / time_diff);
        }
        cli_out("%4"PRId32".%03"PRId32" |%4"PRId32".%03"PRId32" |%8"PRId32" |",
            (pps / 1000), (pps % 1000), (bps / 1000), (bps % 1000), (int)pkt_count_2);
    }
    if (show_time == true) {
        cli_out("%2"PRId32".%03"PRId32" |", time_diff / 1000, time_diff % 1000);
    }
    cli_out("%7"PRId32" |%2"PRId32".%02"PRId32"|%2"PRId32".%02"PRId32"|%2"PRId32".%02"PRId32"|%2"PRId32".%02"PRId32"|%2"PRId32".%02"PRId32" |%2"PRId32".%02"PRId32"|%2"PRId32".%02"PRId32"",
            intr_diff,
...
int
bcma_bcmpkt_test_dpath_create(int unit, int port, int redirect_port, int new_pri,
                              bcma_bcmpkt_test_fp_cfg_t *ifp_cfg)
{
    ...
    SHR_IF_ERR_EXIT
        (pkttest_drv->dpath_create(unit, port, redirect_port, new_pri, ifp_cfg));
exit:
    SHR_FUNC_EXIT();
}
...
int
bcma_bcmpkt_test_dpath_update_policy(int unit, uint64_t entry_id,
                                     uint64_t policy_id)
{
    SHR_FUNC_ENTER(unit);
...
bcma_bcmpkt_test_rx_run(int unit, bcma_bcmpkt_test_rx_param_t *rx_param)
{
    ...
    for (pkt_len = rx_param->len_start; pkt_len <= rx_param->len_end;) {
        SHR_IF_ERR_EXIT
            (bcma_bcmpkt_test_dpath_update_policy(unit, ifp_cfg->entry_id,
                                                  ifp_cfg->copy_policy_id));
        /* Get the CPU usage info at the beginning of test. */
        SHR_IF_ERR_EXIT(bcma_bcmpkt_test_cpu_stats_get(&cpu_start));
        ...
        SHR_IF_ERR_EXIT(bcma_bcmpkt_test_cpu_stats_get(&cpu_end));

        time_diff = (int)(rx_param->time_end - rx_param->time_start);
        bcma_bcmpkt_test_stats_dump(time_diff, rx_param->pkt_received, 0,
                                    pkt_len, &cpu_end, &cpu_start,
                                    true, false);
...
exit:
    ...
    (void)bcmpkt_rx_unregister(unit, netif_id, packet_measure, rx_param);
    SHR_FUNC_EXIT();
}
...
bcma_bcmpkt_test_tx_run(int unit, bcma_bcmpkt_test_tx_param_t *tx_param)
{
    ...
    SHR_IF_ERR_EXIT
        (bcma_bcmpkt_test_dpath_update_policy(unit, tx_param->ifp_cfg->entry_id,
    ...
    for (pkt_len = tx_param->len_start; pkt_len <= tx_param->len_end;) {
        ...
        sal_usleep(50000);
        bcma_bcmpkt_test_stats_dump(time_diff, tx_param->send_count, 0, pkt_len,
                                    &cpu_end, &cpu_start, true, false);
