                  ...
                  watcher_data_t *wdata)
{
    ...
    if (wdata->debug_mode & WATCHER_DEBUG_SHOW_META_DATA) {
        ...
        if (flexdata_len) {
            ...
            SHR_IF_ERR_EXIT(bcmpkt_rxpmd_field_get(dev_type, rxpmd,
...
bcma_bcmpkt_generic_loopback_init(uint32_t variant, uint32_t *generic_loopback)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmpkt_flexhdr_header_id_get(variant, "GENERIC_LOOPBACK_T", &hid));
    SHR_IF_ERR_EXIT(bcmpkt_flexhdr_field_id_get(variant, hid, "START_BYTE",
                                                &fid));

    SHR_IF_ERR_EXIT
        (bcmpkt_flexhdr_field_set(variant, hid, generic_loopback,
...
bcma_bcmpkt_hg3hdr_init(uint32_t variant, uint32_t *hg3hdr)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmpkt_flexhdr_header_id_get(variant, "HG3_BASE_T", &hid));
    SHR_IF_ERR_EXIT(bcmpkt_flexhdr_field_id_get(variant, hid, "RESERVED_ETYPE",
                                                &fid));
    SHR_IF_ERR_EXIT
        (bcmpkt_flexhdr_field_set(variant, hid, hg3hdr,
                                  BCMPKT_FLEXHDR_PROFILE_NONE, fid,
                                  BCMPKT_HG3_ETHER_TYPE));
