...
rxpmd_cmds(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    if (flexdata_len) {
        (void)bcmpkt_rxpmd_field_get(dev_type, rxpmd,
...
hg3_cmds(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    base_hdr_id = *(hid_support + BCMPKT_HG3_BASE_T);
    ext0_hdr_id = *(hid_support + BCMPKT_HG3_EXTENSION_0_T);

    if ((base_hdr_id == -1) || (ext0_hdr_id == -1)) {
    ...
    pb = shr_pb_create();
    rv = bcmpkt_flexhdr_dump(variant, base_hdr_id, higig3_base,
                             BCMPKT_DUMP_F_ALL, pb);
    if (SHR_FAILURE(rv)) {
    ...
    rv = bcmpkt_flexhdr_dump(variant, ext0_hdr_id, higig3_ext0,
                             BCMPKT_DUMP_F_ALL, pb);
    if (SHR_FAILURE(rv)) {
