...
higig3_fields_list(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    hid_base = *(hid_support + BCMPKT_HG3_BASE_T);
    rv = bcmpkt_flexhdr_field_list_dump(variant, hid_base, view_name, pb_base);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
    ...
    hid_ext = *(hid_support + BCMPKT_HG3_EXTENSION_0_T);
    rv = bcmpkt_flexhdr_field_list_dump(variant, hid_ext, view_name, pb_ext);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
...
generic_loopback_fields_list(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    rv = bcmpkt_flexhdr_field_list_dump(variant, hid, view_name, pb);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Dump field list failed (%d)\n"),
                   rv));
...
tx_parser(bcma_cli_t *cli, bcma_cli_args_t *args, bcma_bcmpkt_tx_cfg_t *cfg)
{
    ...
    int glb_hid, hg3_base_hid, hg3_ext_hid;
    bcmpkt_flex_field_info_t glb_field_info;
    bcmpkt_flex_field_info_t hg3_field_info;
    bcmpkt_flex_field_info_t hg3_ext_field_info;
    ...
    if (glb_hid != -1) {
        /* Add NIH options. */
        rv = bcmpkt_flexhdr_field_info_get(generic_loopback.variant, glb_hid,
                                           &glb_field_info);
        if (SHR_FAILURE(rv)) {
            return BCMA_CLI_CMD_NOT_FOUND;
        }

        for (i = 0; i < glb_field_info.num_fields; i++) {
            char field_name[NAME_STR_LEN_MAX];
            sal_snprintf(field_name, NAME_STR_LEN_MAX, "GENERIC_LOOPBACK%s",
                    glb_field_info.field_names[i].name);
            bcma_cli_parse_table_add(&pt, field_name, "generic_loopback",
                    &generic_loopback, &glb_field_info.field_names[i].val);
        }
    } else {
        /* Add LBHDR options. */
    ...
    hg3_base_hid = *(hid_support + BCMPKT_HG3_BASE_T);
    hg3_ext_hid = *(hid_support + BCMPKT_HG3_EXTENSION_0_T);

    if (hg3_base_hid != -1) {
        rv = bcmpkt_flexhdr_field_info_get(hg3.variant, hg3_base_hid,
                                           &hg3_field_info);
        if (SHR_FAILURE(rv)) {
            return BCMA_CLI_CMD_NOT_FOUND;
        }

        for (i = 0; i < hg3_field_info.num_fields; i++) {
            char field_name[NAME_STR_LEN_MAX];
            sal_snprintf(field_name, NAME_STR_LEN_MAX, "HG3%s",
                    hg3_field_info.field_names[i].name);
            bcma_cli_parse_table_add(&pt, field_name, "higig3", &hg3,
                    &hg3_field_info.field_names[i].val);
        }
    }

    if (hg3_ext_hid != -1) {
        int ext0_fid_val[BCMPKT_HG3_EXT0_FID_MAX];
        rv = bcmpkt_flexhdr_field_info_get(hg3.variant, hg3_ext_hid,
                                           &hg3_ext_field_info);
        if (SHR_FAILURE(rv)) {
            return BCMA_CLI_CMD_NOT_FOUND;
        }

        for (i = 0; i < hg3_ext_field_info.num_fields; i++) {
            char field_name[NAME_STR_LEN_MAX];
            sal_snprintf(field_name, NAME_STR_LEN_MAX, "HG3%s",
                         hg3_ext_field_info.field_names[i].name);
            if (hg3_base_hid != -1) {
                ext0_fid_val[i] = hg3_ext_field_info.field_names[i].val +
                                  hg3_field_info.num_fields;
                bcma_cli_parse_table_add(&pt, field_name, "higig3", &hg3,
                        &ext0_fid_val[i]);
            } else {
                bcma_cli_parse_table_add(&pt, field_name, "higig3", &hg3,
                        &hg3_ext_field_info.field_names[i].val);
            }
        }
    }
