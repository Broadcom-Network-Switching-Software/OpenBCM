/*! \file bcma_bcmbdcmd_cmicd_pop.c
 *
 * CLI 'pop' command for CMICd
 */
/*
...
iter_op(const bcmdrd_symbol_t *symbol, bcmdrd_sid_t symbol_id, void *vptr)
{
    ...
    rv = bcma_bcmbd_cmicd_fifo_pop(unit, symbol, symbol_id, &sid);
    if (rv < 0) {
...
static int
cmicd_pop(bcma_cli_t *cli, bcma_cli_args_t *args,
...
int
bcma_bcmbdcmd_cmicd_pop(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    rv = cmicd_pop(cli, args, ctoks, ctoks_num);
