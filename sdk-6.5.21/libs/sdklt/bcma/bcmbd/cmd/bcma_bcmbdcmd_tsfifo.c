...
typedef struct dump_data_s {
    ...
    /*! The name of the output file for dump command if not NULL. */
    char *ofile;
...
/* Handle of the dump commands output file. */
static bcma_io_file_handle_t dp_fh = NULL;
...
/* Previous TS value for diff calculation. */
static uint64_t prev_ts0;
static uint64_t prev_ts1;
...
dump_action(int unit, bcmbd_tsfifo_capture_info_t *cap_info)
{
    ...
    ts0_diff = cap_info->ts0 - prev_ts0;
    ts1_diff = cap_info->ts1 - prev_ts1;

    prev_ts0 = cap_info->ts0;
    prev_ts1 = cap_info->ts1;
    ...
    if (dp_raw) {
        ...
        shr_pb_printf(pb, "TS1=%"PRIu64".%03"PRIu64"\n",
                      cap_info->ts1 / 1000, cap_info->ts1 % 1000);
        shr_pb_printf(pb, "TS0-diff=%"PRIu64".%03"PRIu64" ",
                      ts0_diff / 1000, ts0_diff % 1000);
        shr_pb_printf(pb, "TS1-diff=%"PRIu64".%03"PRIu64"\n",
                      ts1_diff / 1000, ts1_diff % 1000);
    }

    cli_out("%s", shr_pb_str(pb));
    if (dp_fh) {
        bcma_io_file_puts(dp_fh, shr_pb_str(pb));
    }
    shr_pb_destroy(pb);
}
 ...
 */
static int
do_dump(bcmbd_tsfifo_dump_cb_t *cb, bcma_io_file_handle_t ofh, dump_data_t *dpd)
{
...
capture_dump(dump_data_t *dpd)
{
    ...
    fh = bcma_io_file_open(dpd->lfile, "r");
    if (fh == NULL) {
        LOG_ERROR(BSL_LOG_MODULE,
    ...
    if (dpd->ofile) {
        ...
        ofh = bcma_io_file_open(dpd->ofile, "w");
        if (ofh == NULL) {
            LOG_ERROR(BSL_LOG_MODULE,
