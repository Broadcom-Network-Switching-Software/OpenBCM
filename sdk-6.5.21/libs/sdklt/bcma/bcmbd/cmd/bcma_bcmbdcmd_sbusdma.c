...
sbusdma_write(int unit, bcmdrd_sid_t sid, int pipe, int index, char *data)
{
    ...
    /* Get data size. */
    data_size = bcma_cli_parse_array_uint32(data, -1, NULL);
    if (data_size < 0) {
        SHR_ERR_EXIT(SHR_E_FAIL);
    }
    buf = bcmdrd_hal_dma_alloc(unit, data_size * 4, "bcmaBcmbdSbusdmaWrite", &paddr);
    SHR_NULL_CHECK(buf, SHR_E_RESOURCE);
    /* Parse data into buf. */
    if (bcma_cli_parse_array_uint32(data, data_size, (uint32_t *)buf) < 0) {
        SHR_ERR_EXIT(SHR_E_FAIL);
    }
    /* Dump parsing result. */
    pb = shr_pb_create();
    shr_pb_printf(pb, "Write data:\n");
    dump_data(pb, (uint32_t *)buf, data_size);
    ...
    dma_data.data_width = entry_size;
    dma_data.op_code = header[0];
    dma_data.op_count = (data_size + entry_size - 1) / entry_size;
    dma_data.buf_paddr = paddr;
...
exit:
    if (buf) {
        bcmdrd_hal_dma_free(unit, data_size * 4, buf, paddr);
        buf = NULL;
    }
    SHR_FUNC_EXIT();
}
...
sbusdma_push(int unit, bcmdrd_sid_t sid, int pipe, int index, char *data)
{
    ...
    buf = bcmdrd_hal_dma_alloc(unit, data_size * 4, "bcmaBcmbdSbusdmaPush", &paddr);
    SHR_NULL_CHECK(buf, SHR_E_RESOURCE);
    /* Parse data into buf. */
    if (bcma_cli_parse_array_uint32(data, data_size, (uint32_t *)buf) < 0) {
        SHR_ERR_EXIT(SHR_E_FAIL);
