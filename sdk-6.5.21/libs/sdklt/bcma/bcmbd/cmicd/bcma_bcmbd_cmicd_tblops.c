/*! \file bcma_bcmbd_cmicd_tblops.c
 *
...
#define CMICD_RESP_WORD_INDEX_GET(_w)  ((_w) & 0xfffff)

/*! Supported CLI BCMBD CMICd table operations. */
typedef enum cmicd_tbl_op_e {
    CMICD_TBL_INSERT,
    CMICD_TBL_DELETE,
    CMICD_TBL_LOOKUP,
} cmicd_tbl_op_t;
                ...
                cmicd_tbl_op_t tbl_op)
{
    ...
    bcma_bcmbd_cmicd_tblop_f tblop_func;
    ...
    if (tbl_op == CMICD_TBL_INSERT) {
        err_str = "Table insert failure: ";
        ok_str = "Entry is inserted or updated at ";
        tblop_func = bcmbd_cmicd_tbl_insert;
    } else if (tbl_op == CMICD_TBL_DELETE) {
        err_str = "Table delete failure: ";
        ok_str = "Entry is deleted at ";
        tblop_func = bcmbd_cmicd_tbl_delete;
    } else if (tbl_op == CMICD_TBL_LOOKUP) {
        err_str = "Table lookup failure: ";
        ok_str = "Entry is found at ";
        tblop_func = bcmbd_cmicd_tbl_lookup;
    } else {
    ...
    if (and_masks || or_masks) {
        ...
        tbl_idx = CMICD_RESP_WORD_INDEX_GET(resp_info.resp_word);

        /* Display returned entry index. */
        shr_pb_printf(pb, "%s%s[%d]\n", ok_str, sid->addr.name, tbl_idx);

        if (tbl_op != CMICD_TBL_LOOKUP) {
            return rv;
        }
    } else {
 ...
 */
static int
cmicd_tblops(int unit, const bcmdrd_symbol_t *symbol, bcma_bcmbd_id_t *sid,
             uint32_t size, uint32_t *and_masks, uint32_t *or_masks,
             cmicd_tbl_op_t tbl_op)
{
    int b, bt, t;
    ...
    for (b = sid->block.start; b <= sid->block.end; b++) {
        ...
        for (t = sid->pipe.start; t <= sid->pipe.end; t++) {
            ...
            cmicd_cmd_tblop(pb, unit, symbol, sid, b, t, size,
...
tbl_symop(int unit, const bcmdrd_symbol_t *symbol, bcma_bcmbd_id_t *sid,
          uint32_t *and_masks, uint32_t *or_masks, cmicd_tbl_op_t tbl_op)
{
    int b, wsize;
    ...
    */
    for (blktype = 0; blktype < cinfo->nblktypes; blktype++) {
        ...
        */
        ...
        cmicd_tblops(unit, symbol, &sid2, wsize, and_masks, or_masks, tbl_op);
    }
...
{
    return tbl_symop(unit, symbol, sid, and_masks, or_masks, CMICD_TBL_INSERT);
}
...
bcma_bcmbd_cmicd_tbl_delete(int unit, const bcmdrd_symbol_t *symbol,
                            ...
                            uint32_t *and_masks, uint32_t *or_masks)
{
    return tbl_symop(unit, symbol, sid, and_masks, or_masks, CMICD_TBL_DELETE);
}
...
bcma_bcmbd_cmicd_tbl_lookup(int unit, const bcmdrd_symbol_t *symbol,
                            ...
                            uint32_t *and_masks, uint32_t *or_masks)
{
    return tbl_symop(unit, symbol, sid, and_masks, or_masks, CMICD_TBL_LOOKUP);
}
