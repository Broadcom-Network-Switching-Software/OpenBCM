...
/*
 * Parse the variable information for the 'for' command.
 */
static int
parse_loop_fmt(char *arg, char **var_name, char **s_start, char **s_stop,
               char **s_step, char **s_fmt)
{
    char *s;

    *var_name = arg;
    s = sal_strchr(*var_name, '=');
    if (s == NULL || s == *var_name) {
        return BCMA_CLI_CMD_FAIL;
    }
    *s++ = '\0';
    *s_start = s;
    s = sal_strchr(*s_start, ',');
    if (s == NULL || s == *s_start) {
        return BCMA_CLI_CMD_FAIL;
    }
    *s++ = '\0';
    *s_stop = s;
    ...
    /* Continue parsing for step and string format */
    *s_step = NULL;
    *s_fmt = NULL;
    s = sal_strchr(*s_stop, ',');
    if (s != NULL) {
...
do_for_ctrlc(void *data)
{
    ...
    rv = parse_loop_fmt(BCMA_CLI_ARG_GET(args),
                        &var_name, &s_start, &s_stop, &s_step, &fmt);
    ...
    if (bcma_cli_parse_int(s_start, &start) < 0 ||
        ...
        cli_out("%s%s: start and stop must be valid integers\n",
                BCMA_CLI_CONFIG_ERROR_STR, BCMA_CLI_ARG_CMD(args));
        return BCMA_CLI_CMD_FAIL;
    }
    ...
    if (s_step) {
        if (bcma_cli_parse_int(s_step, &step) < 0 || step == 0) {
            cli_out("%s%s: step must be a non-zero integer\n",
                    BCMA_CLI_CONFIG_ERROR_STR, BCMA_CLI_ARG_CMD(args));
            return BCMA_CLI_CMD_FAIL;
        }
    } else {
