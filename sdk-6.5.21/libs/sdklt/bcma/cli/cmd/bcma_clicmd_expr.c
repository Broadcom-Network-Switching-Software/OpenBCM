...
typedef int64_t (*calculator_t)(int64_t v, int64_t v2);
...
static int
infix_push(infix_data_t *id, int64_t v)
{
...
static int
infix_pop(infix_data_t *id, int64_t *v)
{
    if (id->asp == &id->astk[0]) {
        return -1;
    }
...
static int
infix_eval(char *s, int64_t *result)
{
    ...
    while(*s != '\0') {
        if (sal_isspace(*s)) {
            ...
            s2 = bcma_cli_parse_str(s, "0123456789abcdefABCDEFxX", "_", &s);
            if (s2 == NULL) {
                return -1;
            }
            if (bcma_cli_parse_uint64(s2, &val) < 0) {
                sal_free(s2);
                return -1;
            }
            sal_free(s2);
            infix_push(id, (int64_t)val);
            op_preceding = FALSE;
        } else {
            /* Check for operator */
            op = infix_getop(&s);
            ...
            if (op->cal_fun == op_sub && op_preceding) {
                /*
                 * Check if minus sign represents negtive number but not
                 * subtraction.
                 */
                op++;
            } else if (op->cal_fun == op_add && op_preceding) {
                /*
                 * Check if plus sign represents positive number but not
                 * addition.
                 */
                op++;
            }
            ...
            if (!op->unary) {
                /*
                 * Process previous operands that has higher priority than
                 * current one, except previous operand is left parenthesis.
                 */
...
bcma_clicmd_expr(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    /* First clear the previous result (if any) */
    bcma_cli_var_set(cli, var, NULL, TRUE);
    ...
    /* Set the calculation result to local variable */
    pb = shr_pb_create();
    shr_pb_format_uint64(pb, NULL, (uint64_t *)&result, 1, 0);
    bcma_cli_var_set(cli, var, shr_pb_str(pb), TRUE);
    shr_pb_destroy(pb);
