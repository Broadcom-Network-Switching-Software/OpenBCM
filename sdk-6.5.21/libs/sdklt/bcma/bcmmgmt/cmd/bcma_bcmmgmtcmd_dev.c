...
static int
bcmmgmtcmd_dev_start(int unit, bool all_dev, bool warm)
{
    int rv = SHR_E_NONE;
    int u;
    ...
    if (all_dev) {
        ...
        rv = bcma_bcmmgmt_start(warm, false);
        if (SHR_FAILURE(rv)) {
        ...
        rv = bcmmgmt_dev_attach(unit, warm);
        if (SHR_FAILURE(rv)) {
...
static int
bcmmgmtcmd_dev_restart(int unit, bool all_dev, bool warm)
{
    int rv;

    rv = bcmmgmtcmd_dev_stop(unit, all_dev);
    if (SHR_SUCCESS(rv)) {
        rv = bcmmgmtcmd_dev_start(unit, all_dev, warm);
    }
    return rv;
}
...
bcma_bcmmgmtcmd_dev(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    if (BCMA_CLI_ARG_CNT(args) > 0) {
        ...
        bcma_cli_parse_table_add(&pt, "warm", "bool", &warm, NULL);
    ...
    if (sal_strcasecmp("stop", cmd) == 0) {
        ...
        if (SHR_FAILURE(bcmmgmtcmd_dev_start(unit, all_dev, warm))) {
            return BCMA_CLI_CMD_FAIL;
        }
    } else if (sal_strcasecmp("restart", cmd) == 0) {
        if (SHR_FAILURE(bcmmgmtcmd_dev_restart(unit, all_dev, warm))) {
            return BCMA_CLI_CMD_FAIL;
        }
