/*! \file bcma_testcase_cmicd_sbusdma.c
 *
 * Broadcom test cases for CMICD SBUSDMA access
 */
/*
...
typedef struct cmicd_sbusdma_test_param_s {
 ...
 */
static int
cmicd_sbusdma_sname_get(int unit, const char **sname)
{
...
static int
cmicd_sbusdma_select(int unit)
{
    return bcmdrd_feature_enabled(unit, BCMDRD_FT_CMICD);
}

static int
cmicd_sbusdma_parser(int unit, bcma_cli_t *cli, bcma_cli_args_t *a,
                      uint32_t flags, void **bp)
{
    bcma_cli_parse_table_t pt;
    const char *sname = NULL;
    cmicd_sbusdma_test_param_t *p = NULL;
    ...
    SHR_ALLOC(p, sizeof(*p),"bcmaTestCaseCmicdSBUSDMAPrm");
    if (p == NULL) {
    ...
    *bp = (void *)p;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (cmicd_sbusdma_sname_get(unit, &sname));
    ...
    p->acctype = BCMBD_CMICD_BLKACC2ACCTYPE(sinfo.blktypes);
    ...
    pipe_info = bcmbd_cmicd_pipe_info(unit, p->offset, p->acctype,
                                      p->blktype, -1);
    ...
    p->adext = 0;
    BCMBD_CMICD_ADEXT_BLOCK_SET(p->adext, p->blknum);
    BCMBD_CMICD_ADEXT_ACCTYPE_SET(p->adext, p->acctype);
    ...
    p->buf0 = bcmdrd_hal_dma_alloc(unit, p->msize,
                                   "bcmaTestCaseCmicdSBUSDMADataBuf0",
                                   &p->paddr0);
    if (p->buf0 == NULL) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
    ...
    if (p->write) {
        p->buf1 = bcmdrd_hal_dma_alloc(unit, p->msize,
                                       "bcmaTestCaseCmicdSBUSDMADataBuf1",
                                       &p->paddr1);
        if (p->buf1 == NULL) {
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
...
static void
cmicd_sbusdma_help(int unit, void *bp)
{
    cmicd_sbusdma_test_param_t *p = (cmicd_sbusdma_test_param_t *)bp;
    if (p == NULL) {
        return;
    }
...
static void
cmicd_sbusdma_recycle(int unit, void *bp)
{
    cmicd_sbusdma_test_param_t *p = (cmicd_sbusdma_test_param_t *)bp;
    if (p == NULL) {
        return;
 ...
 */
static int
cmicd_sbusdma_entries_cmp(char *dst, char *src, uint32_t wsize, int cnt)
{
    int i, off, len;
 ...
 */
static int
cmicd_sbusdma_test_write(int unit, void *bp, uint32_t option)
{
    cmicd_sbusdma_test_param_t *p = (cmicd_sbusdma_test_param_t *)bp;
    ...
    p->addr = p->laddr;
    SHR_IF_ERR_EXIT
        (bcmbd_cmicd_mem_range_write(unit, p->adext, p->addr, p->wsize,
                                     p->shift, p->count, p->paddr0, p->flags));
    sal_memset(p->buf1, 0, p->msize);
    SHR_IF_ERR_EXIT
        (bcmbd_cmicd_mem_range_read(unit, p->adext, p->addr, p->wsize, p->shift,
                                    p->count, p->paddr1, p->flags));
    SHR_IF_ERR_EXIT
        (cmicd_sbusdma_entries_cmp(p->buf0, p->buf1, p->wsize, p->count));
 ...
 */
static int
cmicd_sbusdma_test_read(int unit, void *bp, uint32_t option)
{
    cmicd_sbusdma_test_param_t *p = (cmicd_sbusdma_test_param_t *)bp;
...
static int
cmicd_sbusdma_test(int unit, void *bp, uint32_t option)
{
    cmicd_sbusdma_test_param_t *p = (cmicd_sbusdma_test_param_t *)bp;
    ...
    if (p->write) {
        SHR_IF_ERR_EXIT(cmicd_sbusdma_test_write(unit, bp, option));
    } else {
        SHR_IF_ERR_EXIT(cmicd_sbusdma_test_read(unit, bp, option));
    }
...
static bcma_test_proc_t cmicd_sbusdma_proc[] = {
    {
        .desc = "run SBUS DMA access test.\n",
        .cb = cmicd_sbusdma_test,
    },
};

static bcma_test_op_t cmicd_sbusdma_op = {
    .select = cmicd_sbusdma_select,
    .parser = cmicd_sbusdma_parser,
    .help = cmicd_sbusdma_help,
    .recycle = cmicd_sbusdma_recycle,
    .procs = cmicd_sbusdma_proc,
    .proc_count = COUNTOF(cmicd_sbusdma_proc),
};

bcma_test_op_t *
bcma_testcase_cmicd_sbusdma_op_get(void)
{
    return &cmicd_sbusdma_op;
}
