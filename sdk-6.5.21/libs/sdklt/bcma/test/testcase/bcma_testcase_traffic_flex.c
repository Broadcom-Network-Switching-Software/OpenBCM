...
#define TRAFFIC_FLEX_PARAM_HELP \
    ...
    "  SelfLoopBack         - Loopback mode where packet traffic swril on same port rx-tx\n"\
        ...
        traffic_flex_param_t *param, bool show)
{
    ...
    SHR_IF_ERR_EXIT
        (bcma_cli_parse_table_add(&pt, "SelfLoopBack", "int", &(param->pkt_lb_type),
                                  NULL));
    SHR_IF_ERR_EXIT
        ...
        uint32_t flags, void **bp)
{
    ...
    BCMDRD_PBMP_CLEAR(tra_param->pbmp_msall);
...
traffic_flex_vlan_set(int unit, void *bp, uint32_t option)
{
    ...
    bcmdrd_pbmp_t pbmp, ubmp, pbmp_all, ms_bmp;
    int port;
    int rv;
    ...
    BCMDRD_PBMP_ITER(pbmp_all, port) {
        ...
        if (rv == SHR_E_NOT_FOUND)
        {
            ...
            rv = (bcma_testutil_drv_flex_pipe_macsec_config_set
                                         (unit, port , vid , pid , &ms_bmp ));
    ...
    BCMDRD_PBMP_ASSIGN(tra_param->pbmp_msall, ms_bmp);
...
traffic_flex_l2_forward_clean(int unit, void *bp, uint32_t option)
{
    ...
    if ((tra_param->is_before_flex == false)) {
       BCMDRD_PBMP_ITER(tra_param->pbmp_msall, port) {
          traffic_flex_get_lport_vid_pid(unit, tra_param, port, &vid, &pid);
          rv = (bcma_testutil_drv_flex_pipe_macsec_config_delete
                                                 (unit , port , pid , vid));
          SHR_IF_ERR_EXIT_EXCEPT_IF(rv, SHR_E_UNAVAIL);
       }
    }
                ...
                SHR_IF_ERR_EXIT
                    (bcma_testutil_packet_fill
                     (packet->data_buf, tra_param->length,
                      default_src_mac, default_dst_mac, false, 0,
                      TRAFFIC_FLEX_DEFAULT_PACKET_ETHERTYPE,
                      pattern, tra_param->pattern_inc));
            }
...
traffic_flex_do_flex(int unit, void *bp, uint32_t option)
{
    ...
    BCMDRD_PBMP_REMOVE(tra_param->pbmp_all, lgc_pbmp_down);
