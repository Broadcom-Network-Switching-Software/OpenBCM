                         ...
                         traffic_pair_param_t *param, bool show)
{
    ...
    SHR_IF_ERR_EXIT
        (bcma_cli_parse_table_add(&pt, "CheckLinkTime", "int",
                                  &param->check_link_time, NULL));
    SHR_IF_ERR_EXIT
                    ...
                    uint32_t flags, void **bp)
{
    ...
    tra_param->check_link_time = BCMA_TEST_DEFAULT_CHECK_LINK_TIME;
    ...
    SHR_IF_ERR_EXIT
        (bcma_testutil_packet_dev_simple_init
            (unit, TRAFFIC_PAIR_TX_CHAN, TRAFFIC_PAIR_RX_CHAN,
             tra_param->length + 4, &netif_id, &pktdev_id));
