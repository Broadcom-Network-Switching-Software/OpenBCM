...
static void
format_time(char *prefix, uint64_t t)
{
    cli_out("%s %"PRIu64" usec", prefix, t);
}
...
static int
cmicx_sbusdma_batch_test_single(int unit, cmicx_sbusdma_batch_test_param_t *p,
                                int do_clean)
{
    ...
    int i, j;
    char *prefix = NULL;
    sal_usecs_t benchmark_timer_start = 0;
    sal_usecs_t benchmark_timer_end = 0;
    ...
    if (p->time_measure) {
        ...
        if (do_clean) {
            prefix = "Time write clean :";
        } else {
            prefix = "Time write :";
        }
        format_time(prefix, benchmark_timer_end - benchmark_timer_start);
        cli_out("\n");
    }
...
cmicx_sbusdma_batch_test(int unit, void *bp, uint32_t option)
{
    ...
    SHR_IF_ERR_EXIT
        (cmicx_sbusdma_batch_test_single(unit, p, 0));
    ...
    if (p->clean) {
        ...
        SHR_IF_ERR_EXIT
            (cmicx_sbusdma_batch_test_single(unit, p, 1));
    }
