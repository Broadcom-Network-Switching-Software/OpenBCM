...
#define MEM_MBIST_DEFAULT_RANGE_WRITE              (FALSE)
...
#define MEM_MBIST_DMA_DEFAULT_POLLING_INTERVAL     (1)  /* usec */

#define MEM_MBIST_PARAM_HELP \
    "  ID         - The testing item index                    \n"\
    "  DMAmode    - Whether to run BIST in DMA mode if support\n"\
    "  RangeWrite - Whether to write data with range access\n"\
    "\nExample:\n"\
    "    tr 515\n"\
    "    tr 515 dma=true\n"\
...
typedef struct mem_mbist_param_s {
    ...
    /* whether to write data with range access */
    int range_write;
} mem_mbist_param_t;
                      ...
                      mem_mbist_param_t *param, bool show)
{
    ...
    SHR_IF_ERR_EXIT
        (bcma_cli_parse_table_add(&pt, "DMAmode", "bool",
                                  &param->dma_mode, NULL));
    SHR_IF_ERR_EXIT
        (bcma_cli_parse_table_add(&pt, "RangeWrite", "bool",
                                  &param->range_write, NULL));
                 ...
                 uint32_t flags, void **bp)
{
    ...
    mem_param->range_write = MEM_MBIST_DEFAULT_RANGE_WRITE;
                         ...
                         int range_access)
{
    ...
    sal_usecs_t decompress_timer_start = 0, decompress_timer_end = 0;
    sal_usecs_t datadma_timer_start = 0, datadma_timer_end = 0;
    sal_usecs_t mbist_exec_timer_start = 0, mbist_exec_timer_end = 0;
    int entry_bytes;
    uint32_t i;
    bcmbd_pt_dyn_info_t dyn_info;
    uint32_t write_data;
    ...
    entry_bytes = BCMDRD_WORDS2BYTES(bcmdrd_pt_entry_wsize(unit, sid_dma_data));

    dma_size = test_info->entry_num * entry_bytes;

    if (range_access == TRUE) {
        dma_buf = bcmdrd_hal_dma_alloc
                    (unit, dma_size, "bcmaTestCaseMBISTDmaMem", &dma_addr);
    } else {
        SHR_ALLOC(dma_buf, dma_size, "bcmaTestCaseMBISTDmaMem");
    }
    ...
    /* Fill DMA data */
    decompress_timer_start = sal_time_usecs();
    dma_buf_ptr = dma_buf;
    data_idx = 0;
    ...
    decompress_timer_end = sal_time_usecs();
    ...
    dyn_info.index = 0;
    dyn_info.tbl_inst = 0;
    write_data = test_info->tap_control_value;
    SHR_IF_ERR_EXIT
        (bcmbd_pt_write(unit, sid_control, &dyn_info, NULL, &write_data));
    ...
    /* write data */
    datadma_timer_start = sal_time_usecs();
    if (range_access == TRUE) {
        SHR_IF_ERR_EXIT
            (bcma_testutil_drv_mem_range_write
                (unit, sid_dma_data, 0, test_info->entry_num - 1, 0,
                 dma_addr, false));
    } else {
        for (i = 0; i < test_info->entry_num; i++) {
            dyn_info.tbl_inst = 0;
            dyn_info.index = i;
            SHR_IF_ERR_EXIT
                (bcma_testutil_drv_pt_write
                    (unit, sid_dma_data, &dyn_info, NULL,
                     (uint32_t *)(&dma_buf[i * entry_bytes])));
        }
    }
    datadma_timer_end = sal_time_usecs();
    sal_usleep(1000);
    ...
    mbist_exec_timer_start = sal_time_usecs();
    SHR_IF_ERR_EXIT
    ...
    while (curtime < MEM_MBIST_DMA_DEFAULT_WAITING_TIME * SECOND_USEC) {
        ...
        if (testing_end == 1) {
            mbist_exec_timer_end = sal_time_usecs();
            break;
        }
    }
    if (testing_end == 0) {
    ...
    if (testing_pass == 1) {
        cli_out("pass\n");
        cli_out("data decompress = %"PRIu32" usec, "
                "data write = %"PRIu32" usec, "
                "mbist execution = %"PRId32" usec\n",
                (uint32_t)(decompress_timer_end - decompress_timer_start),
                (uint32_t)(datadma_timer_end - datadma_timer_start),
                (uint32_t)(mbist_exec_timer_end - mbist_exec_timer_start));
    } else {
...
static int
mem_mbist_lt_update(int unit, const char *table, const char *key_name, uint64_t key_val)
{
    ...
    if (rv == SHR_E_EXISTS || rv == SHR_E_NO_HANDLER) {
        SHR_IF_ERR_EXIT
...
mem_mbist_run(int unit, void *bp, uint32_t option)
{
    ...
    /*
     * stop logical table updating during testing
     *   For SER, LT SER_CONTROL would not exist if the user has disabled SER in advance
     */
    SHR_IF_ERR_EXIT
        (mem_mbist_lt_update(unit, "CTR_CONTROL", "COLLECTION_ENABLE", 0));
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (mem_mbist_lt_update(unit, "SER_CONTROL", "SRAM_SCAN", 0), SHR_E_NOT_FOUND);
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (mem_mbist_lt_update(unit, "SER_CONTROL", "TCAM_SCAN", 0), SHR_E_NOT_FOUND);
    SHR_IF_ERR_EXIT
        (mem_mbist_lt_update(unit, "LM_CONTROL", "SCAN_ENABLE", 0));
    ...
    for (test_i = test_start; test_i <= test_end; test_i++) {
        ...
        if (mem_param->dma_mode == TRUE) {
            SHR_IF_ERR_EXIT(
                mem_mbist_dma_run_single
                    (unit, &(test_array_dma[test_i]), mem_param->range_write));
        } else {
            SHR_IF_ERR_EXIT(
