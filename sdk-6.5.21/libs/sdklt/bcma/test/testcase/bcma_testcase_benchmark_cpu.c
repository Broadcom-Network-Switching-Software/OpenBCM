...
#define BENCHMARK_CPU_DEFAULT_NUM_LOOPS   (5)
...
#define BENCHMARK_CPU_PARAM_HELP \
    "  TestLoops  - Number of test loops\n"\
    "  ITERations - Test iterations in each loop\n"\
    ...
    "      tr 21\n"\
    "    Run benchmark testing with 500 loops, 1000 iterations in each loop\n"\
    "      tr 21 tl=500 iter=1000\n"\
                          ...
                          benchmark_cpu_param_t *param, bool show)
{
    ...
    SHR_IF_ERR_EXIT
        (bcma_cli_parse_table_add(&pt, "TestLoops", "int",
                                  &param->num_loops, NULL));
    SHR_IF_ERR_EXIT
        (bcma_cli_parse_table_add(&pt, "ITERations", "int",
                                  &param->iterations, NULL));
    SHR_IF_ERR_EXIT
                     ...
                     uint32_t flags, void **bp)
{
    ...
    /* init */
    bench_param->num_loops = BENCHMARK_CPU_DEFAULT_NUM_LOOPS;
    bench_param->iterations = BENCHMARK_CPU_DEFAULT_ITERATION;
...
#define BENCHMARK_CPU_TIMER_START(_bench_param_,                 \
    ...
    do {                                                         \
        ...
        int i, j;                                                \
        int loop = (_bench_param_)->num_loops;                   \
        int iter = (_bench_param_)->iterations;                  \
...
#define BENCHMARK_CPU_TIMER_STOP()                               \
        ...
        benchmark_cpu_timer_show(data_size * iter * loop,        \
    ...
    SHR_IF_ERR_EXIT
        (bcma_testutil_packet_dev_simple_init
            (unit, BENCHMARK_TRAFFIC_TX_CHAN, BENCHMARK_TRAFFIC_RX_CHAN,
             BENCHMARK_TRAFFIC_PKT_MAX_FRAME_SIZE,
             &netif_id, &pktdev_id));
...
benchmark_cpu_test(int unit, void *bp, uint32_t option)
{
    ...
    SHR_ALLOC(bench_param->time_diff, bench_param->num_loops * sizeof(int),
