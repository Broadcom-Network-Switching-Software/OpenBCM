/*! \file bcm56990_b0_testutil_max_power.c
 *
 * Chip-specific TRAFFIC data for max power.
 */
/*
...
#include <bcma/test/chip/bcm56990_b0_testutil_traffic.h>


#define BSL_LOG_MODULE BSL_LS_APPL_TEST

#define TH4B0_PWRT_DEV_PORTS_PER_DEV           272
#define TH4B0_PWRT_DEV_PORTS_PER_PIPE           17
#define TH4B0_PWRT_PIPES_PER_DEV                16


#define TH4B0_PWRT_MAX_NUM_DEVICES 1

#define TH4B0_PWRT_EN_FULL_IFP  0
#define TH4B0_PWRT_EN_EM        1
#define TH4B0_PWRT_EN_VFP       2
#define TH4B0_PWRT_EN_EFP       3
#define TH4B0_PWRT_EN_FLX_CTR   4
#define TH4B0_PWRT_EN_TRAP      5
#define TH4B0_PWRT_EN_TCAM_NOISE      6
#define TH4B0_PWRT_EN_LAG_RES      7
#define TH4B0_PWRT_EN_RND_HDR      8
#define TH4B0_PWRT_EN_RND_INITFLEXCTR      9
#define TH4B0_PWRT_DIS_CUT_THROUGH        10
#define TH4B0_PWRT_DIS_SAME_SRC_DST       11
#define TH4B0_PWRT_EN_RND_PRI             12
#define TH4B0_PWRT_EN_EXT_ELOOP           13

#define TH4B0_PWRT_MPLS_IPV6         0
#define TH4B0_PWRT_MPLS_IPV4         1
#define TH4B0_PWRT_IPV6              2
#define TH4B0_PWRT_IPV4              3
#define TH4B0_PWRT_L2_ONLY           4
#define TH4B0_PWRT_VXLAN_IPV6       5

#define TH4B0_PWRT_L3_IIF_BASE      0xfc7
#define TH4B0_PWRT_VLAN_ID_BASE     0xe00

#define TH4B0_PWRT_MAC_BASE    {0x12, 0x34, 0x56, 0x78, 0x9a, 0x00}
#define TH4B0_PWRT_IP_DA       {0xa5a5a000, 0xa5a5a5a5, 0xabcde000, 0x12345678}
#define TH4B0_PWRT_IPV4_DA     0x12345600
#define TH4B0_PWRT_OL_VLAN_ID_BASE     0x100
#define TH4B0_PWRT_UL_VLAN_ID_BASE     0xe00
#define TH4B0_PWRT_OL_VFI_BASE         0x300
#define TH4B0_PWRT_UL_VFI_BASE         0x500
#define TH4B0_PWRT_VNID_BASE        0xe00000
#define TH4B0_PWRT_SVP_BASE         0x1000
#define TH4B0_PWRT_DVP_BASE         0x0800
#define TH4B0_PWRT_MPLS_LABEL  0x000a5000
#define TH4B0_PWRT_VRF_BASE    0x1f00

/* MAC + 3xMPLS + IPv6 + CRC = 18 + 3x4 + 40 + 4 */
#define TH4B0_PWRT_MPLS_IPV6_MIN_PKT_SIZE  74
/* MAC + IPv6 + UDP + VxLAN + MAC + IPv6 + CRC
 * 18  + 40    + 8    + 8   + 18  + 40   + 4 = 136
 */
#define TH4B0_PWRT_VXLAN_IPV6_MIN_PKT_SIZE  136

#define TH4B0_PWRT_SET_FIELD(field, value) \
    memfld[0] = value; \
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, field, entry, memfld))

#define TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, id, inst)\
    do { \
...
typedef struct power_test_s {
    uint32_t dst_port[TH4B0_PWRT_DEV_PORTS_PER_DEV];
    uint32_t num_flows;
    uint32_t start_flow;
    uint32_t src_port_flow_id[TH4B0_PWRT_DEV_PORTS_PER_DEV];   /* phy_src_port = f(flow_id) */
    uint32_t dst_port_flow_id[TH4B0_PWRT_DEV_PORTS_PER_DEV];   /* phy_dst_port = f(flow_id) */
    uint32_t p2l_mapping[TH4B0_PWRT_DEV_PORTS_PER_DEV];        /* dst_port = f(flow_id) */
    uint32_t dev_enabled_ports[TH4B0_PWRT_DEV_PORTS_PER_DEV];  /* indexed by dev_port; 1 if port enabled */
    uint32_t phy_enabled_ports[TH4B0_PWRT_DEV_PORTS_PER_DEV];  /* indexed by dev_port; 1 if port enabled */
    uint32_t phy_to_flow_id[TH4B0_PWRT_DEV_PORTS_PER_DEV];     /* indexed by phy_port; retrieves flow_id */
    uint32_t alpm_level;
    uint32_t enable_map;
...
static power_test_t *power_test_parray[TH4B0_PWRT_MAX_NUM_DEVICES];

typedef struct th4b0_pwr_tsc_connect_s {
    int tsc_0;
    int tsc_1;
} th4b0_pwr_tsc_connect_t;

static const th4b0_pwr_tsc_connect_t
th4b0_pwr_tsc_connect[32] = {
    {  0,   3},
    {  1,   2},
    {  4,   6},
    {  5,   7},
    ...
    int *pair_port)
{
    ...
    if (tra_param->topology == 2) {
         ...
         */
        SHR_IF_ERR_EXIT
            (bcmtm_lport_pport_get(unit, port, &phy_port));
        tsc_in = (phy_port - 1) / 4;
        subport = (phy_port - 1) % 4;
        for (tsc = 0; tsc < 32; tsc++) {
            if (th4b0_pwr_tsc_connect[tsc].tsc_0 == tsc_in) {
                 pair_tsc = th4b0_pwr_tsc_connect[tsc].tsc_1;
                 break;
            } else if (th4b0_pwr_tsc_connect[tsc].tsc_1 == tsc_in) {
                 pair_tsc = th4b0_pwr_tsc_connect[tsc].tsc_0;
                 break;
            }
        }
...
power_test_param_init(int unit, void *bp)
{
    ...
    /* Filter out all non-active & non-FP ports */
    BCMDRD_PBMP_ITER(tra_param->pbmp_all, port) {
        if ((bcmtm_lport_is_fp(unit, port) == 1) &&
            (port < TH4B0_PWRT_DEV_PORTS_PER_DEV)) {
            if (SHR_FAILURE(bcmtm_lport_pport_get(unit, port, &phy_port))) {
    ...
    if (tra_param->enable_bmp == 0xffffffff) {
        tra_param->enable_bmp = (0x1 << TH4B0_PWRT_EN_FULL_IFP) |
                                (0x1 << TH4B0_PWRT_EN_EM) |
                                (0x1 << TH4B0_PWRT_EN_VFP) |
                                (0x1 << TH4B0_PWRT_EN_EFP) |
                                (0x1 << TH4B0_PWRT_EN_FLX_CTR) |
                                (0x1 << TH4B0_PWRT_EN_TRAP) |
                                (0x1 << TH4B0_PWRT_EN_TCAM_NOISE) | /* Not used */
                                (0x1 << TH4B0_PWRT_EN_LAG_RES) |
                                (0x1 << TH4B0_PWRT_EN_RND_HDR) | /* Not used */
                                (0x1 << TH4B0_PWRT_EN_RND_INITFLEXCTR) |
                                (0x1 << TH4B0_PWRT_DIS_CUT_THROUGH) |
                                (0x1 << TH4B0_PWRT_DIS_SAME_SRC_DST) |
                                (0x1 << TH4B0_PWRT_EN_RND_PRI) |
                                (0x1 << TH4B0_PWRT_EN_EXT_ELOOP);
    }
    ...
    if (tra_param->pkt_flow == -1) {
        power_test_p->pkt_flow = TH4B0_PWRT_MPLS_IPV6;
    } else {
        power_test_p->pkt_flow = tra_param->pkt_flow;
    }
    ...
    if (power_test_p->pkt_flow == TH4B0_PWRT_VXLAN_IPV6) {
        /* Internal/External loopback : port A =>IP/EP=> A =>int lbk  => A
         * External loopback if not eloop : port A =>IP/EP=> B =>ext cable=> A ...
         */
        if ((tra_param->loopback_mode == LB_TYPE_EXT) &&
            (((power_test_p->enable_map >> TH4B0_PWRT_EN_EXT_ELOOP) & 0x1) == 0x0)) {
            tra_param->topology = 2;
            power_test_p->same_src_dst = 0;
            power_test_p->traffic_from_all = 1;
        } else {
            power_test_p->same_src_dst = 1;
            power_test_p->traffic_from_all = 1;
        }
    } else { /* MPLS */
        /* External loopback: force topology = 2 if not eloop
        ...
        if ((tra_param->loopback_mode == LB_TYPE_EXT) &&
            ...
            if (((power_test_p->enable_map >> TH4B0_PWRT_DIS_SAME_SRC_DST) & 0x1) == 0x1) {
                power_test_p->same_src_dst = 0;
            } else {
                power_test_p->same_src_dst = 1;
            }
    ...
    switch (power_test_p->pkt_flow) {
        case TH4B0_PWRT_MPLS_IPV6 :
            min_pkt_len = TH4B0_PWRT_MPLS_IPV6_MIN_PKT_SIZE;
            break;
        case TH4B0_PWRT_VXLAN_IPV6 :
            min_pkt_len = TH4B0_PWRT_VXLAN_IPV6_MIN_PKT_SIZE;
            break;
        default :
            min_pkt_len = TH4B0_PWRT_MPLS_IPV6_MIN_PKT_SIZE;
            break;
...
{
    power_test_t *power_test_p = power_test_parray[unit];
    bcm_mac_t l_mac_addr = TH4B0_PWRT_MAC_BASE;
...
{
    power_test_t *power_test_p = power_test_parray[unit];
    uint32_t ip_addr[BCMDRD_MAX_PT_WSIZE] = TH4B0_PWRT_IP_DA;
    ...
    int is_cpu)
{
    ...
    switch (power_test_p->pkt_flow) {
        case TH4B0_PWRT_MPLS_IPV6 :
        case TH4B0_PWRT_MPLS_IPV4 :
            l_is_en = 1;
            break;
        case TH4B0_PWRT_IPV6 :
        case TH4B0_PWRT_IPV4 :
        case TH4B0_PWRT_L2_ONLY:
            l_is_en = 0;
            break;
        default :
...
static uint32_t get_ip_lport_profile_index(
    ...
    int flow_id)
{
    return (get_dev_src_port(unit, flow_id) % TH4B0_PWRT_DEV_PORTS_PER_PIPE);
}
    ...
    int flow_id)
{
    ...
    return (TH4B0_PWRT_L3_IIF_BASE + final_flow_id);
}
    ...
    int flow_id)
{
    ...
    return (TH4B0_PWRT_VLAN_ID_BASE + smaller_phy_port);
}

static uint32_t get_ol_vlan_id(
    int unit,
    int flow_id)
{
    uint32_t phy_dst_port, phy_src_port;
    ...
    return (TH4B0_PWRT_OL_VLAN_ID_BASE + smaller_phy_port);
}

static uint32_t get_ul_vlan_id(
    int unit,
    int flow_id)
{
    uint32_t phy_dst_port, phy_src_port;
    ...
    return (TH4B0_PWRT_UL_VLAN_ID_BASE + smaller_phy_port);
}

static uint32_t get_ol_vfi(
    int unit,
    int flow_id)
{
    uint32_t phy_dst_port;
    phy_dst_port = get_phy_dst_port(unit, flow_id);
    return (TH4B0_PWRT_OL_VFI_BASE + phy_dst_port);
}

static uint32_t get_ul_vfi(
    int unit,
    int flow_id)
{
    uint32_t phy_dst_port;
    phy_dst_port = get_phy_dst_port(unit, flow_id);
    return (TH4B0_PWRT_UL_VFI_BASE + phy_dst_port);
}

static uint32_t get_vxlan_vnid_ing(
    int unit,
    int flow_id)
{
    uint32_t phy_src_port;
    phy_src_port = get_phy_src_port(unit, flow_id);
    return (TH4B0_PWRT_VNID_BASE + phy_src_port);
}

static uint32_t get_vxlan_vnid_egr(
    int unit,
    int flow_id)
{
    uint32_t phy_dst_port;
    phy_dst_port = get_phy_src_port(unit, flow_id);
    return (TH4B0_PWRT_VNID_BASE + phy_dst_port);
}

static uint32_t get_allowed_port_bmp_p_ptr(
    int unit,
    int flow_id)
{
    return (flow_id % 16);
...
static uint32_t get_vxlan_svp(
    int unit,
    int flow_id)
{
    uint32_t phy_src_port;
    phy_src_port = get_phy_src_port(unit, flow_id);
    return (TH4B0_PWRT_SVP_BASE + phy_src_port);
}

static uint32_t get_dvp(
    int unit,
    int flow_id)
{
    uint32_t phy_dst_port;
    phy_dst_port = get_phy_dst_port(unit, flow_id);
    return (TH4B0_PWRT_DVP_BASE + phy_dst_port);
}

static uint32_t get_dvp_group_id(
    int unit,
    int flow_id)
{
    uint32_t phy_dst_port;
    phy_dst_port = get_phy_dst_port(unit, flow_id);
    return (0x0 + phy_dst_port);
}

static uint32_t get_ul_ttl(
    int unit,
    int flow_id)
{
    return 0xff;
...
static uint32_t get_udp_dst_port(
    int unit,
    int flow_id)
{
    return 4789;
...
static uint32_t get_udp_src_port(
    int unit,
    int flow_id)
{
    return 5555;
...
static uint32_t get_pipe_num(int dev_num) {
    return (dev_num / TH4B0_PWRT_DEV_PORTS_PER_PIPE);
}
...
static void right_shift_bit(int bit_cnt,
    uint32_t *memfld)
{
    int i;
    int wd_cnt = (bit_cnt >> 5);
    int rem = bit_cnt & 0x1f;
    for (i = 0; (i < BCMDRD_MAX_PT_WSIZE) && ((wd_cnt + 1) < BCMDRD_MAX_PT_WSIZE); i++, wd_cnt++) {
        memfld[i] = (memfld[wd_cnt] >> rem) | (memfld[wd_cnt+1] << (32 - rem));
    }
}
...
static void get_ul_mac_sa(int unit,
    int flow_id,
    bcm_mac_t mac_addr)
{
...
static void get_ul_mac_da(int unit,
    int flow_id,
    bcm_mac_t mac_addr)
{
...
static void get_ol_mac_sa(int unit,
    int flow_id,
    bcm_mac_t mac_addr)
{
    get_mac_sa(unit,flow_id,mac_addr);
...
static void get_ol_mac_da(int unit,
    int flow_id,
    bcm_mac_t mac_addr)
{
    get_mac_da(unit,flow_id,mac_addr);
...
static void get_port_bmp_pipe(int unit, int pipe, uint32_t *mem_bmp)
{
    ...
    for (l_dev_port = 0 ; l_dev_port < (2 * TH4B0_PWRT_DEV_PORTS_PER_PIPE); l_dev_port++) {
        mem_bmp_indx = l_dev_port / 32;
        mem_bmp_offset = l_dev_port % 32;
        dev_port = l_dev_port + (2 * pipe * TH4B0_PWRT_DEV_PORTS_PER_PIPE);
...
static void get_device_whole_bmp(int unit, uint32_t *mem_bmp)
{
    ...
    for (dev_port = 0; dev_port < (TH4B0_PWRT_PIPES_PER_DEV * TH4B0_PWRT_DEV_PORTS_PER_PIPE); dev_port++) {
    ...
    int use_other_port)
{
    ...
    *mpls_label = TH4B0_PWRT_MPLS_LABEL;
    ...
    int flow_id)
{
    ...
    return (TH4B0_PWRT_VRF_BASE + flow_id);
}
...
static uint32_t get_my_station_tcam_2_index(
    int unit,
    int flow_id,
    int is_cpu)
{
    ...
    int flow_id)
{
    ...
    ip_addr = TH4B0_PWRT_IPV4_DA | smaller_phy_port;
    ...
    int flow_id)
{
    ...
    ip_addr = TH4B0_PWRT_IPV4_DA | bigger_phy_port;
    ...
    uint32_t *memfld)
{
    ...
    phy_src_port = get_phy_src_port(unit, flow_id);
    phy_dst_port = get_phy_dst_port(unit, flow_id);
    /* Use DEST port for VXLAN.
     * Need to get different l3_iif from L3_TUNNEL_QUAD.
     * Different l3_iif gives different VRF.
     */
    smaller_phy_port = is_vxlan_en(unit) ? phy_dst_port :
        (phy_src_port <= phy_dst_port) ? phy_src_port : phy_dst_port;
    ...
    uint32_t *memfld)
{
    ...
    phy_src_port = get_phy_src_port(unit, flow_id);
    phy_dst_port = get_phy_dst_port(unit, flow_id);
    /* Use SRC port for VXLAN.
     * Need to get different l3_iif from L3_TUNNEL_QUAD
     *  Different l3_iif gives different VRF.
     */
    bigger_phy_port = is_vxlan_en(unit) ? phy_src_port :
        ((phy_src_port >= phy_dst_port) ? phy_src_port : phy_dst_port);
...
static void get_ul_ip_addr_sa(int unit,
    int flow_id,
    uint32_t *memfld)
{
    get_ip_addr_sa(unit,flow_id, memfld);
    memfld[0] &= 0xff00ffff;
}

static void get_ul_ip_addr_da(int unit,
    int flow_id,
    uint32_t *memfld)
{
...
static void get_ol_ip_addr_da(int unit,
    int flow_id,
    uint32_t *memfld)
{
    get_ip_addr_da(unit,flow_id, memfld);
    ...
    int slice_no)
{
    ...
    if (is_vxlan_en(unit)){
        get_ol_ip_addr_da(unit, flow_id, memfld);
    } else {
        get_ip_addr_da(unit, flow_id, memfld);
    }
...
static uint32_t get_ul_next_hop_index(
    int unit,
    int flow_id)
{
    /* 15 bits */
    return (0x1f00 + flow_id);
}

static uint32_t get_ol_next_hop_index(
    int unit,
    int flow_id)
{
    /* 15 bits */
    return (0x7100 + flow_id);
}
...
static uint32_t get_egr_ol_intf_num(
    int unit,
    int flow_id)
{
    uint32_t dev_src_port;
    ...
    /* INTF_NUM => {
            MAXBIT => 12, MINBIT => 0, */
    return (0x1100 + dev_src_port);
}

static uint32_t get_egr_ul_intf_num(
    int unit,
    int flow_id)
{
    uint32_t dev_src_port;
...
static uint32_t get_egr_tunnel_index(
    int unit,
    int flow_id)
{
    uint32_t dev_src_port;
...
static int get_ecmp_level2_base_ptr(
    int unit,
    int flow_id,
    int is_cpu)
{
...
static int get_dlb_id(
    int unit,
    int flow_id,
    int is_cpu)
{
    int dlb_id;
   /*  All pipes dlb enable will create worst power case. This will make
    * all dlb memory, logic and datapaths toggled for all pipes. I suggest
    * following dlb config for power test.
    *
    * ipipe_dual0.ip3_pipe0.Irsel : 32 ports using 32 DLB aggregation groups.
    *                             rest 32 ports, no DLB.
    * ipipe_dual0.ip3_pipe1.Irsel : 32 ports using 32 DLB aggregation groups.
    *                             rest 32 ports, no DLB.
    * ipipe_dual1.ip3_pipe0.Irsel : 32 ports using 32 DLB aggregation groups.
    *                             rest 32 ports, no DLB.
    * ipipe_dual1.ip3_pipe1.Irsel : 32 ports using 32 DLB aggregation groups.
    *                             rest 32 ports, no DLB.
    */

    dlb_id = (((flow_id & 1) << 7) | ((flow_id >> 1) & 0x7f));
    return (dlb_id);
}

static int get_flow_id_from_dlb_id(
    int unit,
    int dlb_id,
    int is_cpu)
{
    ...
    int flow_id = 0;
    for (i = 0; i < 256; i++) {
        if (get_dlb_id(unit, i, is_cpu) == dlb_id) {
            flow_id = i;
            break;
        }
    }
...
static int get_ecmp_level2_ecmp_group_ptr(
    int unit,
    int flow_id,
    int is_cpu)
{
    return (get_dlb_id_offset(unit) + get_dlb_id(unit, flow_id, is_cpu));
}
    ...
    uint32_t *pp_map)
{
    ...
    *pp_map = 0;
    for (pipe = 0; pipe < TH4B0_PWRT_PIPES_PER_DEV; pipe++) {
        if (pipe_map & (1 << pipe)) {
            pp_num = pipe / 4;
            *pp_map |= (1 << pp_num);
        }
    ...
    uint32_t *memfld)
{
    ...
    for (i = 0; i < num_words; i++) {
        ...
        memfld[i] = bcma_testutil_sal_rand32() & word_mask;
    }
}


static int
th4b0_pwrt_field_set(int unit,
                   bcmdrd_sid_t sid,
                   bcmdrd_fid_t fid,
                   uint32_t *entbuf,
                   uint32_t *fbuf)
...
static int th4b0_pwrt_pt_write(int unit,
                           bcmdrd_sid_t sid,
                           bcmltd_sid_t lt_id,
                           void *pt_dyn_info,
                           uint32_t *ltmbuf)
...
static int th4b0_pwrt_pt_insert(int unit,
                           bcmdrd_sid_t sid,
                           uint32_t *ltmbuf)
{
    int rv = SHR_E_NONE;
    ...
    if (SHR_SUCCESS(rv)) {
        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit,
                "th4b0_pwrt_pt_insert() success for %s resp_info=0x%x\n"),
                bcmdrd_pt_sid_to_name(unit, sid), resp_info.resp_word));
    } else {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit,
            "th4b0_pwrt_pt_insert() %s could not be inserted\n"),
...
static int power_test_set_key_attributes(int unit)
{
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_LSB_OFFSETf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x0);    /* L2_ENTRY_KEY_ATTRIBUTES__KEY_BASE_WIDTH_L2 */
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);   /* L2_ENTRY_KEY_ATTRIBUTES__DATA_BASE_WIDTH_L2 */
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0xf);         /* L2_ENTRY_KEY_ATTRIBUTES__KEY_WIDTH_L2 */
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);       /* L2_ENTRY_KEY_ATTRIBUTES__BUCKET_MODE_L2 */
    index = 0; /* L2_ENTRY_SINGLE__KEY_TYPE_BRIDGE */
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_LSB_OFFSETf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x0);    /* L2_ENTRY_KEY_ATTRIBUTES__KEY_BASE_WIDTH_VLAN */
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);   /* L2_ENTRY_KEY_ATTRIBUTES__DATA_BASE_WIDTH_VLAN */
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0x6);         /* L2_ENTRY_KEY_ATTRIBUTES__KEY_WIDTH_VLAN */
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);       /* L2_ENTRY_KEY_ATTRIBUTES__BUCKET_MODE_VLAN */
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_LSB_OFFSETf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0x8);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    ...
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0x12);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_LSB_OFFSETf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x3);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0x8);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_LSB_OFFSETf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 2);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 1);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 18);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    index = 6; /* L3_TUNNEL_QUAD__KEY_TYPE_VXLAN_SIP_DIP_V6 */
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0x9);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    index = 3; /* MPLS_ENTRY_SINGLE__KEY_TYPE_VXLAN_VNID */
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_LSB_OFFSETf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x1);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0x6);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    index = 7; /* MPLS_ENTRY_DOUBLE__KEY_TYPE_VXLAN_SIP_V6 */
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x1);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0x3);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    ...
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x1);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0xb);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_LSB_OFFSETf, 0x0);
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0x2);
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0x1);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 0x15);
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0x0);
    index = 2; /* EXACT_MATCH_ONLY_QUAD__KEY_TYPE_MODE320 */
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* bucket_mode=0x0 (VFI), hash_lsb_offset, key_width rand, rest 0 */
    sid = EGR_ADAPT_KEY_ATTRIBUTESm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BUCKET_MODEf, 0);
    TH4B0_PWRT_SET_FIELD(KEY_WIDTHf, 3);   /* VFI */
    TH4B0_PWRT_SET_FIELD(DATA_BASE_WIDTHf, 0);
    TH4B0_PWRT_SET_FIELD(KEY_BASE_WIDTHf, 0);
    index = 2; /* EGR_ADAPT_SINGLE__KEY_TYPE_VFI */
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
...
static int power_test_set_hash_control(int unit)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_TABLE_BANK_CONFIGf, 0x3);
    TH4B0_PWRT_SET_FIELD(ROBUST_HASH_ENf, robust_hash_en);
    TH4B0_PWRT_SET_FIELD(ROBUST_HASH_INSTANCE_SELECTf, 0x0);
    TH4B0_PWRT_SET_FIELD(HASH_OFFSET_DEDICATED_BANK_0f,
                         bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(HASH_OFFSET_DEDICATED_BANK_1f,
                         bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(HASH_TABLE_TEST_MODEf, 0x0);
    TH4B0_PWRT_SET_FIELD(L2_ENTRY_MODEf, 0x0);
    TH4B0_PWRT_SET_FIELD(LOGICAL_BANK_0_PHYSICAL_BANK_LOCATIONf, 0x0);
    TH4B0_PWRT_SET_FIELD(LOGICAL_BANK_1_PHYSICAL_BANK_LOCATIONf, 0x1);
    TH4B0_PWRT_SET_FIELD(LOGICAL_BANK_2_PHYSICAL_BANK_LOCATIONf, 0x2);
    TH4B0_PWRT_SET_FIELD(LOGICAL_BANK_3_PHYSICAL_BANK_LOCATIONf, 0x3);
    index = 0;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_TABLE_BANK_CONFIGf, 0xf);
    TH4B0_PWRT_SET_FIELD(ROBUST_HASH_ENf, robust_hash_en);
    TH4B0_PWRT_SET_FIELD(HASH_OFFSET_DEDICATED_BANK_0f,
                         bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(HASH_OFFSET_DEDICATED_BANK_1f,
                         bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(HASH_OFFSET_DEDICATED_BANK_2f,
                         bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(HASH_OFFSET_DEDICATED_BANK_3f,
                         bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(HASH_TABLE_TEST_MODEf, 0x0);
    index = 0;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(B3_HASH_OFFSETf, bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(B2_HASH_OFFSETf, bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(B1_HASH_OFFSETf, bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(B0_HASH_OFFSETf, bcma_testutil_sal_rand32() % 48);
    TH4B0_PWRT_SET_FIELD(DISABLE_XOR_WRITEf, 0x0);
    TH4B0_PWRT_SET_FIELD(FORCE_XOR_GENf, 0x0);
    index = 0;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ROBUST_HASH_ENf, 1);
    TH4B0_PWRT_SET_FIELD(HASH_TABLE_BANK_CONFIGf, 3);
    TH4B0_PWRT_SET_FIELD(HASH_OFFSET_DEDICATED_BANK_1f, 32);
    index = 0;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
...
static int power_test_set_hash_remap_and_action(int unit)
{
    ...
    for (tab = 0; tab < 8; tab++) {
        ...
        for (i = 0; i < remap_table_size; i++) {
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            TH4B0_PWRT_SET_FIELD(REMAP_DATAf,
                                 bcma_testutil_sal_rand32() & 0xfff);
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, i, 0);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
        }
    }
    ...
    for (tab = 0; tab < 8; tab++) {
        ...
        for (i = 0; i < action_table_size; i++) {
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            memfld[0] = ((bcma_testutil_sal_rand32() % 0xff) << 24) |
                        ((bcma_testutil_sal_rand32() % 0xff) << 16) |
                        ((bcma_testutil_sal_rand32() % 0xff) <<  8) |
                        ((bcma_testutil_sal_rand32() % 0xff) << 0);
            memfld[1] = ((bcma_testutil_sal_rand32() % 0xff) << 24) |
                        ((bcma_testutil_sal_rand32() % 0xff) << 16) |
                        ((bcma_testutil_sal_rand32() % 0xff) <<  8) |
                        ((bcma_testutil_sal_rand32() % 0xff) << 0);
            SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, ACTIONf, entry, memfld));
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, i, 0);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
        }
    }
...
static int setup_ing_ctrl_regs(int unit)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(UC_DISABLE_SIP_LOOKUPf, 0);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    TH4B0_PWRT_SET_FIELD(ECMP_HASH_FIELD_UPPER_BITS_COUNTf, 6);
    TH4B0_PWRT_SET_FIELD(ECMP_HASH_SELf, 3);
    TH4B0_PWRT_SET_FIELD(ECMP_HASH_USE_RTAG7f, 1);
    TH4B0_PWRT_SET_FIELD(NON_UC_TRUNK_HASH_SRC_ENABLEf, 1);
    TH4B0_PWRT_SET_FIELD(USE_TCP_UDP_PORTSf, 1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(UVLAN_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(PVLAN_VID_MISMATCH_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(PARITY_ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(MACSA_ALL_ZERO_DROPf, 1);
    TH4B0_PWRT_SET_FIELD(DOSATTACK_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(BFD_YOUR_DISCRIMINATOR_NOT_FOUND_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(BFD_UNKNOWN_VERSION_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(BFD_UNKNOWN_CONTROL_PACKET_TOCPUf, 1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ADAPT_MISS_TOCPUf, 0);
    TH4B0_PWRT_SET_FIELD(NONSTATICMOVE_TOCPUf, 0);
    TH4B0_PWRT_SET_FIELD(V6L3ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(V6L3ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(V6L3DSTMISS_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(V4L3ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(V4L3DSTMISS_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(UUCAST_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(UMC_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(TUNNEL_ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(STATICMOVE_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(SRCROUTE_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(NIP_L3ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(MC_INDEX_ERROR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(MARTIAN_ADDR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(L3_SLOWPATH_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(L3_MTU_FAIL_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(L3UC_TTL_ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(L3UC_TTL1_ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(IPMC_TTL_ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(IPMC_TTL1_ERR_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(IPMCPORTMISS_TOCPUf, 1);
    TH4B0_PWRT_SET_FIELD(IPMCERR_TOCPUf, 1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(L3_DST_ENABLEf, 1);
    /*TH4B0_PWRT_SET_FIELD(ENABLEf, 0xf);*/
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_HASHINGf, 0x1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ENABLE_BIN_12_OVERLAY_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_BIN_12_OVERLAY_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(MACRO_FLOW_HASH_BYTE_SELf, 0);
    TH4B0_PWRT_SET_FIELD(MACRO_FLOW_HASH_FUNC_SELf,
                         3 + (bcma_testutil_sal_rand32() % 13));
    TH4B0_PWRT_SET_FIELD(USE_MPLS_STACK_FOR_HASHINGf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLOW_LABEL_IPV6_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLOW_LABEL_IPV6_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_GTP_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_GTP_Bf, 0x1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_10_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_10_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_11_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_11_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_12_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_12_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_13_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_13_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_1_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_1_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_2_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_2_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_3_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_3_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_4_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_4_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_5_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_5_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_6_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_6_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_7_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_7_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_8_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_8_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_9_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_FLEX_FIELD_9_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_UDF_FIELD_1_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_UDF_FIELD_1_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_UDF_FIELD_2_Af, 0x1);
    TH4B0_PWRT_SET_FIELD(ENABLE_UDF_FIELD_2_Bf, 0x1);
    TH4B0_PWRT_SET_FIELD(HASH_A0_FUNCTION_SELECTf,
                         3 + (bcma_testutil_sal_rand32() % 13));
    TH4B0_PWRT_SET_FIELD(HASH_A1_FUNCTION_SELECTf,
                         3 + (bcma_testutil_sal_rand32() % 13));
    TH4B0_PWRT_SET_FIELD(HASH_B0_FUNCTION_SELECTf,
                         3 + (bcma_testutil_sal_rand32() % 13));
    TH4B0_PWRT_SET_FIELD(HASH_B1_FUNCTION_SELECTf,
                         3 + (bcma_testutil_sal_rand32() % 13));
    TH4B0_PWRT_SET_FIELD(HASH_PRE_PROCESSING_ENABLE_Af, 0x0);
    TH4B0_PWRT_SET_FIELD(HASH_PRE_PROCESSING_ENABLE_Bf, 0x0);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(IPV4_FIELD_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(IPV4_FIELD_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(IPV6_FIELD_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(IPV6_FIELD_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(L2_FIELD_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(L2_FIELD_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_SEED_Af, bcma_testutil_sal_rand32());
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(HASH_SEED_Bf, bcma_testutil_sal_rand32());
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (port = 0; port < TH4B0_PWRT_DEV_PORTS_PER_DEV; port++) {
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
    ...
    /* RTAG7_HASH_VH_INITIALIZE_A */
    sid = RTAG7_HASH_VH_INITIALIZE_Ar;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(VH_INIT_VAL_0f, bcma_testutil_sal_rand32());
    TH4B0_PWRT_SET_FIELD(VH_INIT_VAL_1f, bcma_testutil_sal_rand32());
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* RTAG7_HASH_VH_INITIALIZE_B */
    sid = RTAG7_HASH_VH_INITIALIZE_Br;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(VH_INIT_VAL_0f, bcma_testutil_sal_rand32());
    TH4B0_PWRT_SET_FIELD(VH_INIT_VAL_1f, bcma_testutil_sal_rand32());
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(IPV4_TCP_UDP_SRC_EQ_DST_FIELD_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(IPV4_TCP_UDP_SRC_EQ_DST_FIELD_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(IPV4_TCP_UDP_FIELD_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(IPV4_TCP_UDP_FIELD_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(IPV6_TCP_UDP_SRC_EQ_DST_FIELD_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(IPV6_TCP_UDP_SRC_EQ_DST_FIELD_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(IPV6_TCP_UDP_FIELD_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(IPV6_TCP_UDP_FIELD_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(MPLS_L3_PAYLOAD_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(MPLS_L3_PAYLOAD_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(MPLS_OUTER_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(MPLS_OUTER_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(MPLS_OUTER_BITMAP_EXTENDED_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(MPLS_OUTER_BITMAP_EXTENDED_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* RTAG7_VXLAN_PAYLOAD_L2_HASH_FIELD_BMAP */
    sid = RTAG7_VXLAN_PAYLOAD_L2_HASH_FIELD_BMAPr;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(VXLAN_PAYLOAD_L2_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(VXLAN_PAYLOAD_L2_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* RTAG7_VXLAN_PAYLOAD_L3_HASH_FIELD_BMAP */
    sid = RTAG7_VXLAN_PAYLOAD_L3_HASH_FIELD_BMAPr;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(VXLAN_PAYLOAD_L3_BITMAP_Af, 0x1fff);
    TH4B0_PWRT_SET_FIELD(VXLAN_PAYLOAD_L3_BITMAP_Bf, 0x1fff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (index = 0; index < 16; index++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(BITMAPf, 0x1fff);
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* {VXLAN_CONTROL} (reg)
     *  => udp_dest_port = pkt's udphdr.dest_prt;
     */
    sid = VXLAN_CONTROLr;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(UDP_DEST_PORTf, 4789); /* Fixed for VXLAN */
    TH4B0_PWRT_SET_FIELD(UDP_DEST_PORT_VALIDf, 1);
    TH4B0_PWRT_SET_FIELD(EXPECTED_FLAGSf, 0xaa);
    TH4B0_PWRT_SET_FIELD(EXPECTED_FLAGS_MASKf, 0xff);
    TH4B0_PWRT_SET_FIELD(FLAGS_MISMATCH_DROPf, 1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* {ING_L2_TUNNEL_PARSE_CONTROL} (reg)
     *         => parse_ipv6_payload=0x1
     *           (ENABLE parsing of IPV6 payload for VXLAN pkt)
     */
    sid = ING_L2_TUNNEL_PARSE_CONTROLr;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(PARSE_IPV6_PAYLOADf, 1);
    TH4B0_PWRT_SET_FIELD(PARSE_IPV4_PAYLOADf, 1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* {EGR_VXLAN_CONTROL} (reg)
     *       => (pkt) vxlan_flags=VXLAN_HDR.FLAGS,
     *          vxlan_reserved_1=VXLAN_HDR.RSVD1,
     *          vxlan_reserved_2=VXLAN_HDR.RSVD2
     */
    sid = EGR_VXLAN_CONTROLr;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(VXLAN_FLAGSf, 0xaa);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    if (is_vxlan_en(unit)){
        sid = L3_ECMP_BANK_CONFIGr;
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(BITMAPf, 0xc);
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

        sid = EGR_L3_INTF_BANK_CONFIGr;
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(BITMAPf, 0xf0);
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

        sid = ING_L3_NEXT_HOP_BANK_CONFIGr;
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(BITMAPf, 0xf0);/*0-3:UL, 4-7:OL*/
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

        sid = EGR_L3_NEXT_HOP_BANK_CONFIGr;
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(BITMAPf, 0xf0);
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    ...
    sid = ING_OUTER_TPID_0r;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(TPIDf, 0x8100);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = ING_OUTER_TPID_1r;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(TPIDf, 0x9100);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = ING_OUTER_TPID_2r;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(TPIDf, 0x88a8);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = ING_OUTER_TPID_3r;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(TPIDf, 0x0);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = INNER_L2_OUTER_TPID_0r;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(TPIDf, 0x8100);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
...
static int setup_ing_ctrl_tables(int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcmlt_entry_field_add(lt_entry, "LEVEL3_BANK_CONFIG_BLOCK_0", 0x0));
    SHR_IF_ERR_EXIT(bcmlt_entry_field_add(lt_entry, "LEVEL3_BANK_CONFIG_BLOCK_1", 0x0));
    ...
    SHR_IF_ERR_EXIT(bcmlt_entry_field_add(lt_entry, "LPM_IP_DATA_GEN_RESULT_MODE", 2));
    ...
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAPf, entry, memfld));
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* TH4: all ones */
    set_dev_bmp_all_one(memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, BITMAPf, entry, memfld));
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
...
static int setup_ing_nonflow_tables(int unit)
{
    ...
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SPLIT_DATA_P0f, entry, memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SPLIT_DATA_P1f, entry, memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SPLIT_DATA_P2f, entry, memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SPLIT_DATA_P3f, entry, memfld));
    ...
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SPLIT_DATA_P4f, entry, memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SPLIT_DATA_P5f, entry, memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SPLIT_DATA_P6f, entry, memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SPLIT_DATA_P7f, entry, memfld));
    /* all indexes from 0 to 63: all ones */
    for (index = 0; index < 64; index++) {
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sid = VFI_PROFILE_TABm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(OUTER_TPID_INDEXf, 0x0);
    TH4B0_PWRT_SET_FIELD(L2_PFMf, 0x1);
    TH4B0_PWRT_SET_FIELD(L3_IPV4_PFMf, 0x1);
    TH4B0_PWRT_SET_FIELD(L3_IPV6_PFMf, 0x1);
    ...
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, DATAf, entry, memfld));
    /* (configure just one entry = 0) */
    index  = get_ing_vlan_profile_ptr(0);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(IPV6L3_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPV4L3_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPMCV6_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPMCV4_ENABLEf, 0x1);
    index  = 0;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ALLOW_GLOBAL_ROUTEf, 0x1);
    index  = 0;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (index = 0; index < 8; index++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(VALIDf, 0x1);
        sal_memset(memfld, 0, sizeof(memfld));
        memfld[0] = ((index & 0x7) << 0) | (bcma_testutil_sal_rand32() << 3);
        memfld[1] = bcma_testutil_sal_rand32();
        SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, KEYf, entry, memfld));
        sal_memset(memfld, 0, sizeof(memfld));
        memfld[0] = 0x7;
        SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, MASKf, entry, memfld));
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    int flow_id, int is_cpu)
{
    ...
    if (is_cpu == 0) {
        ...
        vlan_id = is_vxlan_en(unit) ? get_ul_vlan_id(unit, flow_id) :
                                      get_vlan_id(unit, flow_id);
        lport_profile_index = get_ip_lport_profile_index(unit, flow_id);
        enable = 0x1;
    } else {
        src_dev_port = 0;
        vlan_id = is_vxlan_en(unit) ? TH4B0_PWRT_UL_VLAN_ID_BASE :
                                      TH4B0_PWRT_VLAN_ID_BASE;
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(CML_FLAGS_MOVEf, 0x4);
    TH4B0_PWRT_SET_FIELD(CML_FLAGS_NEWf, 0x4);
    TH4B0_PWRT_SET_FIELD(EN_IFILTERf, 0x1);
    TH4B0_PWRT_SET_FIELD(FILTER_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPMC_DO_VLANf, enable);
    TH4B0_PWRT_SET_FIELD(OUTER_TPID_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(INNER_L2_OUTER_TPID_ENABLEf, 0x1);
    /* Normal operation if L3 & not MPLS & CPU */
    /* In L3_ONLY flows from CPU, L3_IIF from VLAN_MPLS table is used */
    /*   *port_operation = 0x3 (VRF) => needed to force VRF getting picked up from src_trunk_map */
    if ((is_l3_en(unit, flow_id, 0) == 1)   &&
        (is_mpls_en(unit, flow_id, 0) == 0) &&
        (is_cpu == 1) ) {
        TH4B0_PWRT_SET_FIELD(PORT_OPERATIONf, 0x0); /* Normal */
    } else if (is_l2_only_en(unit, flow_id, is_cpu) == 1) {
        TH4B0_PWRT_SET_FIELD(PORT_OPERATIONf, 0x0); /* Normal */
    } else {
        TH4B0_PWRT_SET_FIELD(PORT_OPERATIONf, (enable == 0x1) ? 0x2 : 0x0); /* l3_iif */
    }
    TH4B0_PWRT_SET_FIELD(ALLOW_NON_GSHf, 0x1);
    TH4B0_PWRT_SET_FIELD(PORT_TYPEf, 0x0); /* ETHERNET */
    TH4B0_PWRT_SET_FIELD(PORT_VIDf, vlan_id);
    TH4B0_PWRT_SET_FIELD(RTAG7_PORT_LBNf, 0x5); /* used for hash in ECMP */
    TH4B0_PWRT_SET_FIELD(TAG_ACTION_PROFILE_PTRf, tag_action_profile_ptr);
    TH4B0_PWRT_SET_FIELD(V4IPMC_ENABLEf, enable);
    TH4B0_PWRT_SET_FIELD(V4L3_ENABLEf, enable);
    TH4B0_PWRT_SET_FIELD(V6IPMC_ENABLEf, enable);
    TH4B0_PWRT_SET_FIELD(V6L3_ENABLEf, enable);
    TH4B0_PWRT_SET_FIELD(VFP_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(VFP_PORT_GROUP_IDf, 0x0);
    table_id = 0x1;
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, table_id);
    TH4B0_PWRT_SET_FIELD(IFP_KEY_SEL_CLASS_IDf, get_port_tab_class_id(unit, 0));

    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_RND_PRI) & 0x1) == 0x1) {
        TH4B0_PWRT_SET_FIELD(TRUST_DOT1P_PTRf, flow_id & 0x3f);
        TH4B0_PWRT_SET_FIELD(DOT1P_REMAP_POINTERf, flow_id & 0x3f);
    }
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(L3_IIFf, get_l3_iif(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(LPORT_PROFILE_IDXf, lport_profile_index);
    if ((is_mpls_en(unit, flow_id, 0) == 1)) {
        TH4B0_PWRT_SET_FIELD(MPLS_ENABLEf, 0x1);
    }
    if ((is_vxlan_en(unit) == 1)) {
        TH4B0_PWRT_SET_FIELD(VXLAN_TERMINATION_ALLOWEDf, 0x1);
        TH4B0_PWRT_SET_FIELD(VXLAN_TERMINATION_KEY_TYPEf, 0x1); /* DIP_SIP */
    }
    index  = src_dev_port;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    pp_acc_type = pipe / 4;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, pp_acc_type);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    if ((is_cpu == 0) &&
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(SOT_OTAG_ACTIONf, 0x2);
    ...
    int is_cpu)
{
    ...
    if (is_cpu == 0) {
        vlan_id = is_vxlan_en(unit) ? get_ol_vfi(unit, flow_id) :
                                      get_vlan_id(unit, flow_id);
    } else {
        vlan_id = is_vxlan_en(unit) ? TH4B0_PWRT_OL_VFI_BASE :
                                      TH4B0_PWRT_VLAN_ID_BASE;
    }

    /* {VFI} (tbl)
     *       => l2_entry_key_type=0x0,
     *          stg=VFI_STG idx,
     *          vfi_membership_profile_ptr,
     *         port_bitmap_profile_ptr, vfi_profile_ptr
     */
    sid = VFI_TABm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    ...
    TH4B0_PWRT_SET_FIELD(VALIDf, 0x1);
    TH4B0_PWRT_SET_FIELD(STGf, get_ing_stg(flow_id));
    TH4B0_PWRT_SET_FIELD(VFI_MEMBERSHIP_PROFILE_PTRf, vlan_id);
    TH4B0_PWRT_SET_FIELD(PORT_BITMAP_PROFILE_PTRf, get_ing_port_bitmap_profile_ptr(flow_id));
    TH4B0_PWRT_SET_FIELD(L2_ENTRY_KEY_TYPEf, 0x0); /* bridging */
    TH4B0_PWRT_SET_FIELD(VFI_PROFILE_PTRf, get_ing_vlan_profile_ptr(flow_id));
    table_id = 2;
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, table_id);
    index = vlan_id;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    ...
    sid = VFI_MEMBERSHIP_PROFILE_TABm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    get_device_whole_bmp(unit, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, ING_PORT_BITMAPf, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    index = vlan_id;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    if (is_mpls_en(unit, flow_id, is_cpu) == 1) {
        ...
        TH4B0_PWRT_SET_FIELD(MPLS_ENABLEf, 0x0);
    }
    TH4B0_PWRT_SET_FIELD(L3_IIFf, get_l3_iif(unit, flow_id));
    index = vlan_id;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(MTU_SIZEf, 0x3fff);
    index = vlan_id;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    int is_cpu)
{
    ...
    vlan_id = (is_cpu == 1) ? TH4B0_PWRT_VLAN_ID_BASE :
        get_vlan_id(unit, flow_id);
    vfi = (is_cpu == 1) ? TH4B0_PWRT_OL_VFI_BASE :
        get_ol_vfi(unit, flow_id);

    /* Setup (MAC_SA, VLAN_ID) entry */
    /* {L2_ENTRY_SINGLE} (tbl)
     *       => Learn (VFI, SVP, Inner MACSA) in case of MACSA miss or station move
     *       => key={VFI,INNER_MACSA},
     *          mac_addr=INNER_SA,
     *          vfi=PortTbMplsEntryTbl.vfi,
     *          entry_type=0 (L2, from VfiTbl.entry_type)),
     *          dest_type=0 (MODPORT)
     *      => l2__destination=SRC_PORT
     */
    sid = L2_ENTRY_SINGLEm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BASE_VALIDf, 0x1); /* START_OF_KEY_BM_0 */
    TH4B0_PWRT_SET_FIELD(KEY_TYPEf, 0x0);   /* BRIDGE Key = {OVID, MAC} */
    if (is_l2_only_en(unit, flow_id, 0) == 0) {
        TH4B0_PWRT_SET_FIELD(L2v_DESTINATIONf, get_dev_src_port(unit, flow_id));
        TH4B0_PWRT_SET_FIELD(L2v_DEST_TYPEf, 0);  /* PORT */
        if (is_vxlan_en(unit)){
            TH4B0_PWRT_SET_FIELD(L2v_VFIf, vfi);
        }else{
            TH4B0_PWRT_SET_FIELD(L2v_VFIf, vlan_id);
        }
        sal_memset(memfld, 0, sizeof(memfld));
        if (is_vxlan_en(unit)){
            get_ol_mac_sa(unit, flow_id, mac_addr);
        } else {
            get_mac_sa(unit, flow_id, mac_addr);
        }
        convert_mac(mac_addr, memfld);
        SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, L2v_MAC_ADDRf, entry, memfld));
    } else {
        TH4B0_PWRT_SET_FIELD(L2v_DESTINATIONf, get_dev_dst_port(unit, flow_id));
        TH4B0_PWRT_SET_FIELD(L2v_DEST_TYPEf, 0);  /* PORT */
        TH4B0_PWRT_SET_FIELD(L2v_VFIf, vlan_id);
        sal_memset(memfld, 0, sizeof(memfld));
        get_mac_da(unit, flow_id, mac_addr);
    ...
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_insert(unit, sid, entry));
    ...
    if (is_l2_only_en(unit, flow_id, 0) == 0) {
        ...
        sal_memset(memfld, 0, sizeof(memfld));
        if (is_vxlan_en(unit)){
            get_ol_mac_da(unit, flow_id, mac_addr);
        } else {
            get_mac_da(unit, flow_id, mac_addr);
        }
        convert_mac(mac_addr, memfld);
    ...
    int is_cpu)
{
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BASE_VALIDf, 0x1); /* START_OF_KEY_BM_0 */
    TH4B0_PWRT_SET_FIELD(KEY_TYPEf, 0x0);   /* MPLS {MPLS, SGP}*/
    label_order = 3; /* BOS */
    get_mpls_label(unit, flow_id, label_order,
                   &mpls_label, &mpls_exp, &mpls_ttl, use_other_port);
    TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_LABELf, mpls_label);
    TH4B0_PWRT_SET_FIELD(MPLSv_PORT_NUMf, src_dev_port);
    TH4B0_PWRT_SET_FIELD(MPLSv_V4_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(MPLSv_V6_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_ACTION_IF_BOSf, 0x2); /* (0x2: l3_iif) */
    TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_ACTION_IF_NOT_BOSf, 0x0); /* (0x0: invalid) */
    TH4B0_PWRT_SET_FIELD(MPLSv_L3_IIFf, get_l3_iif(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(MPLSv_FLEX_CTR_ACTIONf, 0x4);
    TH4B0_PWRT_SET_FIELD(MPLSv_FLEX_CTR_OBJECTf, 0x3a54);
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_RND_PRI) & 0x1) == 0x1) {
        TH4B0_PWRT_SET_FIELD(MPLSv_DECAP_USE_EXP_FOR_PRIf, 1);
        TH4B0_PWRT_SET_FIELD(MPLSv_EXP_MAPPING_PTRf, flow_id % 32);
    }
    ...
    TH4B0_PWRT_SET_FIELD(MPLSv_V4_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(MPLSv_V6_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_ACTION_IF_BOSf, 0x0); /* (0x0: invalid) */
    TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_ACTION_IF_NOT_BOSf, 0x1); /* (0x1: POP) */
    TH4B0_PWRT_SET_FIELD(MPLSv_L3_IIFf, 0x1abc); /* don't care */
    TH4B0_PWRT_SET_FIELD(MPLSv_FLEX_CTR_ACTIONf, 0x5);
    TH4B0_PWRT_SET_FIELD(MPLSv_FLEX_CTR_OBJECTf, 0x3a55);
    ...
    TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_ACTION_IF_NOT_BOSf, 0x1); /* (0x1: POP) */
    TH4B0_PWRT_SET_FIELD(MPLSv_L3_IIFf, 0x1abc); /* don't care */
    TH4B0_PWRT_SET_FIELD(MPLSv_FLEX_CTR_ACTIONf, 0x6);
    TH4B0_PWRT_SET_FIELD(MPLSv_FLEX_CTR_OBJECTf, 0x3a56);
...
static int setup_ing_mpls_entry_for_vxlan(int unit,
    int flow_id,
    int is_cpu)
{
    bcmdrd_sid_t sid;
    ...
    if (is_cpu == 0) {
        vfi = get_ol_vfi(unit, flow_id);
    } else {
        vfi = TH4B0_PWRT_OL_VFI_BASE;
    }

    /*  {L3_TUNNEL_QUAD} (tbl)
     *   => (lookup on outer IP -> termination) key_type=SIP_DIP_V6,
     *       sip=OUTER_SIP, dip=OUTER_DIP, ovid=0,
     *       l3_iif (?configure same as vfi_2.l3_iif),
     *   => allowed_port_bitmap_profile_ptr
     *      -> lookup into ALLOWED_PORT_BITMAP_PROFILE table
     */
    sid = L3_TUNNEL_QUADm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BASE_VALID_0f, 0x1);
    TH4B0_PWRT_SET_FIELD(BASE_VALID_1f, 0x2);
    TH4B0_PWRT_SET_FIELD(BASE_VALID_2f, 0x2);
    TH4B0_PWRT_SET_FIELD(BASE_VALID_3f, 0x2);
    TH4B0_PWRT_SET_FIELD(KEY_TYPEf, 0x6);   /* VXLAN_SIP_DIP_V6 */
    get_ul_ip_addr_sa(unit, flow_id, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_DIP_V6v_O_SIP_LWRf, entry, memfld));
    right_shift_bit(55,memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_DIP_V6v_O_SIP_UPRf, entry, memfld));
    get_ul_ip_addr_da(unit, flow_id, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_DIP_V6v_O_DIP_LWRf, entry, memfld));
    right_shift_bit(90,memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_DIP_V6v_O_DIP_UPRf, entry, memfld));
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_DIP_V6v_PAYLOAD_IPV6f, 1);
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_DIP_V6v_ALLOWED_PORT_BITMAP_PROFILE_PTRf, get_allowed_port_bmp_p_ptr(unit,flow_id));
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_DIP_V6v_L3_IIFf, get_l3_iif(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_DIP_V6v_IGNORE_UDP_CHECKSUMf, 1);
    sal_memset(memfld, 0, sizeof(memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_DIP_V6v_RESERVED_KEY_PADDING_0f, entry, memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_DIP_V6v_RESERVED_KEY_PADDING_1f, entry, memfld));
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_DIP_V6v_RESERVED_KEY_PADDING_2f, entry, memfld));
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_DIP_V6v_FLEX_CTR_ACTIONf, 0x4);
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_DIP_V6v_FLEX_CTR_OBJECTf, 0x3a54);

    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_insert(unit, sid, entry));



    /*  {ALLOWED_PORT_BITMAP_PROFILE} (tbl)
     *     => ing_port_bitmap[Src Prt] = 1;
     *     This port is allowed on a VXLAN network
     */
    sid = ALLOWED_PORT_BITMAP_PROFILEm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    set_dev_bmp_all_one(memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, ING_PORT_BITMAPf, entry, memfld));
    index = get_allowed_port_bmp_p_ptr(unit,flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* {MPLS_ENTRY_DOUBLE} (tbl)
     *        => (lookup on outer SIP -> SVP}
     *            key_type=SIP_V6,
     *            sip=OUTER_SIP, ovid=0
     *        => svp -> lookup into SVP table
     */
    sid = MPLS_ENTRY_DOUBLEm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    get_ul_ip_addr_sa(unit, flow_id, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_V6v_SIP_V6_LWRf, entry, memfld));
    right_shift_bit(101,memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, VXLAN_SIP_V6v_SIP_V6_UPRf, entry, memfld));
    TH4B0_PWRT_SET_FIELD(BASE_VALID_0f, 0x1); /* START_OF_KEY_BM_0 */
    TH4B0_PWRT_SET_FIELD(BASE_VALID_1f, 0x2);
    TH4B0_PWRT_SET_FIELD(KEY_TYPEf, 0x7);   /* VXLAN_SIP_V6 */
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_V6v_OVIDf, 0);
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_V6v_SVPf, get_vxlan_svp(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_V6v_OVIDf, 0);
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_V6v_RESERVED_KEY_PADDINGf, 0);
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_V6v_FLEX_CTR_ACTIONf, 0x6);
    TH4B0_PWRT_SET_FIELD(VXLAN_SIP_V6v_FLEX_CTR_OBJECTf, 0x3a56);

    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_insert(unit, sid, entry));

    /* {MPLS_ENTRY_SINGLE} (tbl)
     *        => (lookup on VNID -> VFI)
     *           key_type=VNID, vnid=VxlanHdr.VNID, fwd_type=1 (VFI)
     *        => VFI -> lookup into VFI tables
     */
    sid = MPLS_ENTRY_SINGLEm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BASE_VALIDf, 0x1); /* START_OF_KEY_BM_0 */
    TH4B0_PWRT_SET_FIELD(KEY_TYPEf, 0x3);   /* VXLAN_VNID */
    TH4B0_PWRT_SET_FIELD(VXLAN_VNIDv_FWD_TYPEf, 1); /* VFI */
    TH4B0_PWRT_SET_FIELD(VXLAN_VNIDv_VFIf, vfi);
    TH4B0_PWRT_SET_FIELD(VXLAN_VNIDv_VNIDf, get_vxlan_vnid_ing(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(VXLAN_VNIDv_OVIDf, 0);
    TH4B0_PWRT_SET_FIELD(VXLAN_VNIDv_FLEX_CTR_ACTIONf, 0x5);
    TH4B0_PWRT_SET_FIELD(VXLAN_VNIDv_FLEX_CTR_OBJECTf, 0x3a55);
...
static int setup_ing_svp(int unit,
    int flow_id)
{
    bcmdrd_sid_t sid;
    bcmltd_sid_t ltid = -1;
    ...
    /*  {SVP} (tbl)
     *     => ipv6l3_enable=0x1
     *       IPV6 pkts allowed on this Virtual interface (VP)
     */
    sid = SVPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    index = get_vxlan_svp(unit, flow_id);
    TH4B0_PWRT_SET_FIELD(CML_FLAGS_NEWf, 4);  /* Enable Learning */
    TH4B0_PWRT_SET_FIELD(CML_FLAGS_MOVEf, 4);  /* Enable Learning */
    TH4B0_PWRT_SET_FIELD(IPV4L3_ENABLEf, 1);
    TH4B0_PWRT_SET_FIELD(IPV6L3_ENABLEf, 1);
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTION_SELf, (flow_id&1));
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = SVP_FLEX_CTR_CONTROLr;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTION_0f, 1);
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTION_1f, 2);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    int flow_id)
{
    ...
    TH4B0_PWRT_SET_FIELD(L3_IIF_PROFILE_INDEXf, get_l3_iif_profile_index(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(VRFf, get_vrf(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(IPMC_L3_IIFf, index);   /* don't care */
    TH4B0_PWRT_SET_FIELD(IPV6L3_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPV4L3_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPMCV6_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPMCV4_ENABLEf, 0x1);
    ...
    TH4B0_PWRT_SET_FIELD(ALLOW_GLOBAL_ROUTEf, 0x1);
    MMU_INT_STAT_REQ_0 = ((bcma_testutil_sal_rand32() & 0x1) == 0) ? 1 : 3;
    MMU_INT_STAT_REQ_1 = ((bcma_testutil_sal_rand32() & 0x1) == 0) ? 1 : 3;
    TH4B0_PWRT_SET_FIELD(MMU_INT_STAT_REQ_0f, MMU_INT_STAT_REQ_0);
    TH4B0_PWRT_SET_FIELD(MMU_INT_STAT_REQ_1f, MMU_INT_STAT_REQ_1);
    /* Randomize the rest */
    TH4B0_PWRT_SET_FIELD(IPMC_MISS_AS_L2MCf, bcma_testutil_sal_rand32() & 0x1);
    TH4B0_PWRT_SET_FIELD(UNKNOWN_IPV4_MC_TOCPUf,
                         bcma_testutil_sal_rand32() & 0x1);
    TH4B0_PWRT_SET_FIELD(UNRESOLVEDL3SRC_TOCPUf, 0x0);
    TH4B0_PWRT_SET_FIELD(RESERVEDf, bcma_testutil_sal_rand32() & 0x1f);
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_RND_PRI) & 0x1) == 0x1) {
    TH4B0_PWRT_SET_FIELD(TRUST_DSCP_PTRf, flow_id % 64);
    }
    ...
    uint32_t *destination)
{
    ...
    if (power_test_p->enable_ecmp_dlb != 0) {
        *dest_type = 1;  /* 0: NHI; 1: ECMP_GROUP */
        /* VXLAN uses bit 12 :: 0-underlay, 1-overlay, LSB: ECMP group pointer */
        if (is_vxlan_en(unit)){
           *destination = ((0x1 << 12) | get_ecmp_level1_ecmp_group_ptr(unit, flow_id, 0));
        } else {
    ...
    uint32_t *memfld)
{
    ...
    if (is_assoc_data_full == 1) { /* ROUTE : ASSOC_DATA_FULL */
        ...
        FLEX_CTR_OBJECT = 0;
        INT_ACTION_PROFILE_IDX = 1 + (bcma_testutil_sal_rand32() % 15);
        INT_FLOW_CLASS         = bcma_testutil_sal_rand32() & 0x1ff;
    ...
    int flow_id)
{
    ...
    if (is_vxlan_en(unit)){
        get_ol_ip_addr_da(unit, flow_id, ip_addr);
    }else{
    ...
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, TABLE_FIELDSf, entry, alpm_full_data));
    ...
    int flow_id)
{
    ...
    TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 0x0); /* regular L3 UC */
    TH4B0_PWRT_SET_FIELD(VLAN_IDf, get_vlan_id(unit, flow_id));
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_LAG_RES) & 0x1) != 0) {
        TH4B0_PWRT_SET_FIELD(Tf, 0x1);
        TH4B0_PWRT_SET_FIELD(TGIDf, flow_id);
    } else {
        TH4B0_PWRT_SET_FIELD(Tf, 0x0);
        /*TH4B0_PWRT_SET_FIELD(PORT_NUMf, get_dev_dst_port(unit, flow_id));*/
        TH4B0_PWRT_SET_FIELD(PORT_NUMf, get_final_dst_dev_port(unit, flow_id));
    }
    TH4B0_PWRT_SET_FIELD(DROPf, 0x0);
    TH4B0_PWRT_SET_FIELD(COPY_TO_CPUf, 0x0);
...
/*
 * VXLAN FLOW:: This function configures:
 * ING_L3_NEXT_HOPm
*/
static int setup_ing_next_hop_vxlan(int unit,
    int flow_id)
{
    bcmdrd_sid_t sid;
    bcmltd_sid_t ltid = -1;
    ...
    /* {ING_L3_NEXT_HOP} (tbl)
     *       => (O_NHI) entry_type=DVP_RES_INFO,
     *          do_not_cut_through=0x1,
     *          dvp_res_info=DVP,
     *          u_nhi=U_NHI,
     *          u_nhi_valid=0x1,
     *          l3_oif=0x1fff  (O_NHI)
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 0x3); /* DVP_RES_INFO */
    TH4B0_PWRT_SET_FIELD(DO_NOT_CUT_THROUGHf, 0x1);
    TH4B0_PWRT_SET_FIELD(DVP_RES_INFOf, get_dvp(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(U_NHI_VALIDf, 1);
    TH4B0_PWRT_SET_FIELD(U_NHIf, get_ul_next_hop_index(unit,flow_id));
    TH4B0_PWRT_SET_FIELD(L3_OIFf, get_ol_next_hop_index(unit,flow_id));
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_LAG_RES) & 0x1) != 0) {
        TH4B0_PWRT_SET_FIELD(Tf, 0x1);
        TH4B0_PWRT_SET_FIELD(TGIDf, flow_id);
    } else {
    ...
    TH4B0_PWRT_SET_FIELD(DROPf, 0x0);
    TH4B0_PWRT_SET_FIELD(COPY_TO_CPUf, 0x0);
    index = get_ol_next_hop_index(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* {ING_L3_NEXT_HOP} (tbl)
     *        => (U_NHI) entry_type=DVP_RES_INFO,
     *           dvp_res_info=DVP,
     *           {t,port_num}=DEST_PORT,
     *           u_nhi_valid=0x0,
     *           mtu_size=0x3fff (U_NHI -> Egress Port, TGID)
     */
    sid = ING_L3_NEXT_HOPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    ...
    TH4B0_PWRT_SET_FIELD(DO_NOT_CUT_THROUGHf, 0x1);
    TH4B0_PWRT_SET_FIELD(DVP_RES_INFOf, get_dvp(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(U_NHI_VALIDf, 0);
    TH4B0_PWRT_SET_FIELD(MTU_SIZEf, 0x3fff);
    TH4B0_PWRT_SET_FIELD(L3_OIFf, get_ol_next_hop_index(unit,flow_id));
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_LAG_RES) & 0x1) != 0) {
        TH4B0_PWRT_SET_FIELD(Tf, 0x1);
    ...
    TH4B0_PWRT_SET_FIELD(DROPf, 0x0);
    TH4B0_PWRT_SET_FIELD(COPY_TO_CPUf, 0x0);
    index = get_ul_next_hop_index(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    int flow_id)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(SELECTf, get_cos_map_sel__selectf(unit, flow_id));
    ...
    for (sel = 0; sel < 4; sel++) {
        for (pri = 0; pri < 16; pri++) {
            index  = (sel * 16) + pri;
            sal_memset(entry, 0, sizeof(entry));
            TH4B0_PWRT_SET_FIELD(COSf, get_port_cos_map__cosf(unit, flow_id, index));
            TH4B0_PWRT_SET_FIELD(RQE_Q_NUMf, get_port_cos_map__cosf(unit, flow_id, index));
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
        }
    }
    ...
    int flow_id)
{
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    for (i = 0; i < 16; i++) {
        TH4B0_PWRT_SET_FIELD(udf_base_offset[i],
                             1 + (bcma_testutil_sal_rand32() % 4)); /* range 1:4 */
        TH4B0_PWRT_SET_FIELD(udf_offset[i],
                             1 + (bcma_testutil_sal_rand32() % 5)); /* range 1:4 */
    }
    TH4B0_PWRT_SET_FIELD(HASH_MASK_1f, bcma_testutil_sal_rand32() & 0xffff);
    TH4B0_PWRT_SET_FIELD(HASH_MASK_2f, bcma_testutil_sal_rand32() & 0xffff);
    TH4B0_PWRT_SET_FIELD(ENABLE_UDF_CONDITIONAL_CHECKf, 0x1);
    /* get macda[15:0] on UDF_CONDITIONAL_CHECK_TABLE_CAM.key[15:0] */
    TH4B0_PWRT_SET_FIELD(UDF1_BASE_OFFSET_7f, 6);
    TH4B0_PWRT_SET_FIELD(UDF2_BASE_OFFSET_7f, 6);
    TH4B0_PWRT_SET_FIELD(UDF1_OFFSET7f, 2);
    TH4B0_PWRT_SET_FIELD(UDF2_OFFSET7f, 2);
    index = get_udf_index(unit, flow_id);
    ipep_acc_type = pipe / 4;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, ipep_acc_type);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    int is_cpu)
{
    ...
    src_dev_port = (is_cpu == 1) ?
        0 : get_dev_src_port(unit, flow_id);
    if (is_vxlan_en(unit)){
        vlan_id = (is_cpu == 1) ? TH4B0_PWRT_UL_VLAN_ID_BASE :
            get_ul_vlan_id(unit, flow_id);
        ol_vfi = (is_cpu == 1) ? TH4B0_PWRT_OL_VFI_BASE :
            get_ol_vfi(unit, flow_id);
    }else{
        vlan_id = (is_cpu == 1) ? TH4B0_PWRT_VLAN_ID_BASE :
            get_vlan_id(unit, flow_id);
    ...
    /* {MY_STATION} (tbl)
     *       => Lookup on OUTER MACDA -> outer L3 processing allowed
     *       => key={SRC_PRT,OUTER_DA,OVID},
     *          mac_addr=OUTER_DA,
     *          source_field=SRC_PORT,
     *          vfi=PortTbl.port_ovid,
     *       => ipv6_termination_allowed=0x1
     */
    SHR_IF_ERR_EXIT
    ...
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "ARP_RARP_TERMINATION_ALLOWED",
                               bcma_testutil_sal_rand32() & 0x1));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "IPV4_MULTICAST_TERMINATION_ALLOWED",
                               bcma_testutil_sal_rand32() & 0x1));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "IPV6_MULTICAST_TERMINATION_ALLOWED",
                               bcma_testutil_sal_rand32() & 0x1));
    /* KEY and MASK */
    SHR_IF_ERR_EXIT
    ...
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "VFI_MASK", 0xfff));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "PORT_NUM_MASK", 0));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "T_MASK", 0));

    sal_memset(memfld, 0, sizeof(memfld));
    if (is_vxlan_en(unit)){
        get_ul_mac_da(unit, flow_id, mac_addr);
    }else{
        get_mac_da(unit, flow_id, mac_addr);
    }
    convert_mac(mac_addr, memfld);
    ...
    /* {MY_STATION_2} (tbl)
     *        => Lookup on Inner MACDA -> inner L3 processing is allowed (Riot)
     *        => key={SRC_PRT,INNER_DA,VFI},
     *           mac_addr=INNER_DA,
     *           source_field=SRC_PORT,
     *           vfi=PortTbMplsEntryTbl.vfi,
     *        => inner_l2=0x1, ipv6_termination_allowed=0x1
     */
    SHR_IF_ERR_EXIT
        (bcmlt_entry_allocate(unit, "MY_STATION_2_TCAMm", &entry));

    index = get_my_station_tcam_2_index(unit, flow_id, is_cpu);
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "BCMLT_PT_INDEX", index));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "VALID", 0x3));
    ...
    if (is_vxlan_en(unit)){
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry, "VFI", ol_vfi));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry, "INNER_L2", 0x1));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry, "VFI_MASK", 0x0));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry, "INNER_L2_MASK", 0x1));
    } else {
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry, "VFI", vlan_id));
        SHR_IF_ERR_EXIT
            (bcmlt_entry_field_add(entry, "VFI_MASK", 0x1fff));
    }
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "PORT_NUM", src_dev_port));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "PORT_NUM_MASK", 0x0));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "T", 0));
    SHR_IF_ERR_EXIT
        (bcmlt_entry_field_add(entry, "T_MASK", 0));
    ...
    int is_cpu)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(MEMBER_COUNTf, 0); /* Modulo 1 is 0*/
    TH4B0_PWRT_SET_FIELD(BASE_PTRf, get_ecmp_level1_base_ptr(unit, flow_id, is_cpu));
    TH4B0_PWRT_SET_FIELD(LB_MODEf, 8); /* WECMP_0_127 */
    index = get_ecmp_level1_ecmp_group_ptr(unit, flow_id, is_cpu);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));

    sid = L3_ECMPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 1); /* WECMP */
    TH4B0_PWRT_SET_FIELD(WECMPv_NEXT_HOP_INDEX_Af, get_next_hop_index(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(WECMPv_NEXT_HOP_INDEX_Bf, get_next_hop_index(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(WECMPv_WEIGHTf, bcma_testutil_sal_rand32() & 0xfff);
...
static int setup_ing_ecmp_vxlan(
    int unit,
    int flow_id,
    int is_cpu)
{
    ...
    int index;
    uint32_t base_ptr;
    uint32_t ecmp_grp_base_ptr;

    SHR_FUNC_ENTER(unit);

   /* {L3_ECMP_GROUP_HIERARCHICAL} (tbl)
    *       => Index is ECMP GRP Ptr1
    *       => lb_mode = 1 (resilient hash),
    *          base_ptr1 (used to calculate L3_ECMP addr)
    */
    sid = L3_ECMP_GROUP_HIERARCHICALm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(LB_MODEf, 1); /* Resilient Hash */
    base_ptr = get_ecmp_level1_base_ptr(unit, flow_id, is_cpu);
    ecmp_grp_base_ptr = (base_ptr << 1);
    TH4B0_PWRT_SET_FIELD(BASE_PTRf, ecmp_grp_base_ptr);
    index = get_ecmp_level1_ecmp_group_ptr(unit, flow_id, is_cpu);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* L3_ECMP} (tbl)
     *       => entry_type=0x2 (HIERACHICAL_ECMP),
     *          hierarchical_ecmp__o_next_hop_index_0=OVERLAY_NHI
     *       => hierarchical_ecmp__u_nhi_or_ecmp_group_0=0x1 (U_ECMP_GROUP),
     *          hierarchical_ecmp__u_ecmp_group_0=ECMP_GRP Ptr2
     */
    sid = L3_ECMPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 2); /* HIERARCHICAL_ECMP */
    TH4B0_PWRT_SET_FIELD(HIERARCHICAL_ECMPv_O_NEXT_HOP_INDEX_0f, get_ol_next_hop_index(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(HIERARCHICAL_ECMPv_U_NHI_OR_ECMP_GROUP_0f, 1); /* U_ECMP_GROUP */
    TH4B0_PWRT_SET_FIELD(HIERARCHICAL_ECMPv_U_ECMP_GROUP_0f, get_ecmp_level2_ecmp_group_ptr(unit, flow_id, is_cpu));
    index = get_ecmp_level1_ecmp_index(unit, flow_id, is_cpu);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* {L3_ECMP_GROUP} (tbl)
     *       => Index is ECMP GRP Ptr2
     *       => lb_mode = 1 (resilient hash),
     *          base_ptr2 (used to calculate L3_ECMP addr)
     */
    sid = L3_ECMP_COUNTm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(MEMBER_COUNTf, 0); /* Modulo 1 is 0*/
    base_ptr = get_ecmp_level2_base_ptr(unit, flow_id, is_cpu);
    ecmp_grp_base_ptr = (base_ptr << 2) | 0x1;
    TH4B0_PWRT_SET_FIELD(BASE_PTRf, ecmp_grp_base_ptr);
    TH4B0_PWRT_SET_FIELD(LB_MODEf, 1);
    index = get_ecmp_level2_ecmp_group_ptr(unit, flow_id, is_cpu);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* {L3_ECMP} (tbl)
     *       => entry_type=0x0 (ECMP),
     *          ecmp__next_hop_index_1=UNDERLAY_NHI
     */
    sid = L3_ECMPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 0); /* ECMP */
    TH4B0_PWRT_SET_FIELD(ECMPv_NEXT_HOP_INDEX_1f, get_ul_next_hop_index(unit, flow_id));
    index = get_ecmp_level2_base_ptr(unit, flow_id, is_cpu);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
...
static int power_test_setup_dlb_ecmp_regs(
    int unit)
{
    bcmdrd_sid_t sid;
    bcmltd_sid_t ltid = -1;
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(DLB_ID_OFFSETf, get_dlb_id_offset(unit));
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = DLB_ID_0_TO_63_ENABLEr;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    for (i = 0; i < 2; i++) { memfld[i] = 0xffffffff; }
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, BITMAPf, entry, memfld));
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = DLB_ID_64_TO_127_ENABLEr;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    for (i = 0;i < 2; i++) { memfld[i] = 0xffffffff; }
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, BITMAPf, entry, memfld));
    ...
    sid = DLB_ECMP_ETHERTYPE_ELIGIBILITY_MAPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ETHERTYPEf, get_ether_type(unit,0));
    TH4B0_PWRT_SET_FIELD(VALIDf, 1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = DLB_ECMP_LINK_CONTROLm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    set_dev_bmp_all_one(memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SW_OVERRIDE_PORT_MAPf, entry, memfld));
    set_dev_bmp_all_one(memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SW_PORT_STATEf, entry, memfld));
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = DLB_ECMP_EEM_CONFIGURATIONm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(CONFIGURATIONf, 1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
...
static int power_test_setup_dlb_ecmp(
    int unit)

{
    ...
    uint32_t dlb_id;
    int flow_id;
    int src_dev_port, dst_dev_port;
    int index, i, j, last_indx;
    static const bcmdrd_sid_t dlb_ecmp_nhi_members[5] = {
        DLB_ECMP_GROUP_NHI_MEMBERS_0_TO_12m,  DLB_ECMP_GROUP_NHI_MEMBERS_13_TO_25m,
        DLB_ECMP_GROUP_NHI_MEMBERS_26_TO_38m, DLB_ECMP_GROUP_NHI_MEMBERS_39_TO_51m,
        DLB_ECMP_GROUP_NHI_MEMBERS_52_TO_63m
    };
    static const bcmdrd_sid_t dlb_ecmp_port_members[4] = {
        DLB_ECMP_GROUP_PORT_MEMBERS_0_TO_15m, DLB_ECMP_GROUP_PORT_MEMBERS_16_TO_31m,
        DLB_ECMP_GROUP_PORT_MEMBERS_32_TO_47m,
        DLB_ECMP_GROUP_PORT_MEMBERS_48_TO_63m
    };
    static const bcmdrd_fid_t member_nhi[64] = {
    ...
    static const bcmdrd_fid_t member_nhi_valid[64] = {
        MEMBER_0_NHI_VALIDf,  MEMBER_1_NHI_VALIDf,  MEMBER_2_NHI_VALIDf,
        MEMBER_3_NHI_VALIDf,  MEMBER_4_NHI_VALIDf,  MEMBER_5_NHI_VALIDf,
        MEMBER_6_NHI_VALIDf,  MEMBER_7_NHI_VALIDf,  MEMBER_8_NHI_VALIDf,
        MEMBER_9_NHI_VALIDf,  MEMBER_10_NHI_VALIDf, MEMBER_11_NHI_VALIDf,
        MEMBER_12_NHI_VALIDf, MEMBER_13_NHI_VALIDf, MEMBER_14_NHI_VALIDf,
        MEMBER_15_NHI_VALIDf, MEMBER_16_NHI_VALIDf, MEMBER_17_NHI_VALIDf,
        MEMBER_18_NHI_VALIDf, MEMBER_19_NHI_VALIDf, MEMBER_20_NHI_VALIDf,
        MEMBER_21_NHI_VALIDf, MEMBER_22_NHI_VALIDf, MEMBER_23_NHI_VALIDf,
        MEMBER_24_NHI_VALIDf, MEMBER_25_NHI_VALIDf, MEMBER_26_NHI_VALIDf,
        MEMBER_27_NHI_VALIDf, MEMBER_28_NHI_VALIDf, MEMBER_29_NHI_VALIDf,
        MEMBER_30_NHI_VALIDf, MEMBER_31_NHI_VALIDf, MEMBER_32_NHI_VALIDf,
        MEMBER_33_NHI_VALIDf, MEMBER_34_NHI_VALIDf, MEMBER_35_NHI_VALIDf,
        MEMBER_36_NHI_VALIDf, MEMBER_37_NHI_VALIDf, MEMBER_38_NHI_VALIDf,
        MEMBER_39_NHI_VALIDf, MEMBER_40_NHI_VALIDf, MEMBER_41_NHI_VALIDf,
        MEMBER_42_NHI_VALIDf, MEMBER_43_NHI_VALIDf, MEMBER_44_NHI_VALIDf,
        MEMBER_45_NHI_VALIDf, MEMBER_46_NHI_VALIDf, MEMBER_47_NHI_VALIDf,
        MEMBER_48_NHI_VALIDf, MEMBER_49_NHI_VALIDf, MEMBER_50_NHI_VALIDf,
        MEMBER_51_NHI_VALIDf, MEMBER_52_NHI_VALIDf, MEMBER_53_NHI_VALIDf,
        MEMBER_54_NHI_VALIDf, MEMBER_55_NHI_VALIDf, MEMBER_56_NHI_VALIDf,
        MEMBER_57_NHI_VALIDf, MEMBER_58_NHI_VALIDf, MEMBER_59_NHI_VALIDf,
        MEMBER_60_NHI_VALIDf, MEMBER_61_NHI_VALIDf, MEMBER_62_NHI_VALIDf,
        MEMBER_63_NHI_VALIDf
    };
    static const bcmdrd_fid_t member_port[64] = {
    ...
    static const bcmdrd_fid_t member_port_valid[64] = {
        MEMBER_0_PORT_VALIDf,  MEMBER_1_PORT_VALIDf,  MEMBER_2_PORT_VALIDf,
        MEMBER_3_PORT_VALIDf,  MEMBER_4_PORT_VALIDf,  MEMBER_5_PORT_VALIDf,
        MEMBER_6_PORT_VALIDf,  MEMBER_7_PORT_VALIDf,  MEMBER_8_PORT_VALIDf,
        MEMBER_9_PORT_VALIDf,  MEMBER_10_PORT_VALIDf, MEMBER_11_PORT_VALIDf,
        MEMBER_12_PORT_VALIDf, MEMBER_13_PORT_VALIDf, MEMBER_14_PORT_VALIDf,
        MEMBER_15_PORT_VALIDf, MEMBER_16_PORT_VALIDf, MEMBER_17_PORT_VALIDf,
        MEMBER_18_PORT_VALIDf, MEMBER_19_PORT_VALIDf, MEMBER_20_PORT_VALIDf,
        MEMBER_21_PORT_VALIDf, MEMBER_22_PORT_VALIDf, MEMBER_23_PORT_VALIDf,
        MEMBER_24_PORT_VALIDf, MEMBER_25_PORT_VALIDf, MEMBER_26_PORT_VALIDf,
        MEMBER_27_PORT_VALIDf, MEMBER_28_PORT_VALIDf, MEMBER_29_PORT_VALIDf,
        MEMBER_30_PORT_VALIDf, MEMBER_31_PORT_VALIDf, MEMBER_32_PORT_VALIDf,
        MEMBER_33_PORT_VALIDf, MEMBER_34_PORT_VALIDf, MEMBER_35_PORT_VALIDf,
        MEMBER_36_PORT_VALIDf, MEMBER_37_PORT_VALIDf, MEMBER_38_PORT_VALIDf,
        MEMBER_39_PORT_VALIDf, MEMBER_40_PORT_VALIDf, MEMBER_41_PORT_VALIDf,
        MEMBER_42_PORT_VALIDf, MEMBER_43_PORT_VALIDf, MEMBER_44_PORT_VALIDf,
        MEMBER_45_PORT_VALIDf, MEMBER_46_PORT_VALIDf, MEMBER_47_PORT_VALIDf,
        MEMBER_48_PORT_VALIDf, MEMBER_49_PORT_VALIDf, MEMBER_50_PORT_VALIDf,
        MEMBER_51_PORT_VALIDf, MEMBER_52_PORT_VALIDf, MEMBER_53_PORT_VALIDf,
        MEMBER_54_PORT_VALIDf, MEMBER_55_PORT_VALIDf, MEMBER_56_PORT_VALIDf,
        MEMBER_57_PORT_VALIDf, MEMBER_58_PORT_VALIDf, MEMBER_59_PORT_VALIDf,
        MEMBER_60_PORT_VALIDf, MEMBER_61_PORT_VALIDf, MEMBER_62_PORT_VALIDf,
        MEMBER_63_PORT_VALIDf
    };


    SHR_FUNC_ENTER(unit);

    /* Only first 128 flows use DLB.
     *  Maximum aggregate group possible = 128.
     */
    for (dlb_id = 0; dlb_id < 128; dlb_id++) {
        flow_id = get_flow_id_from_dlb_id(unit, dlb_id, 0);
        src_dev_port = get_dev_src_port(unit, flow_id);
        dst_dev_port = get_dev_dst_port(unit, flow_id);

        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit,
            "power_test_setup_dlb_ecmp() dlb_id =%d  src_dev_port=%0d dst_dev_port=%0d flow_id=%0d\n"),
            dlb_id, src_dev_port, dst_dev_port, flow_id));

        sid = DLB_ECMP_GROUP_CONTROLm;
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(FLOW_SET_SIZEf, 1); /* 256: All point to same value. */
        TH4B0_PWRT_SET_FIELD(FLOW_SET_BASEf, (dlb_id << 8));
        TH4B0_PWRT_SET_FIELD(ENABLE_OPTIMAL_CANDIDATE_UPDATEf, 0x1);
        TH4B0_PWRT_SET_FIELD(INACTIVITY_DURATIONf, 16); /* Minimum: 16 us*/
        TH4B0_PWRT_SET_FIELD(PORT_ASSIGNMENT_MODEf, 2); /* PACKET_SPRAY */
        TH4B0_PWRT_SET_FIELD(GROUP_MEMBERSHIP_POINTERf, dlb_id);
        TH4B0_PWRT_SET_FIELD(PRIMARY_GROUP_SIZEf, 64); /* $urandom_range(1,64) */
        TH4B0_PWRT_SET_FIELD(PRIMARY_PATH_THRESHOLDf, 0x1);
        TH4B0_PWRT_SET_FIELD(ALTERNATE_PATH_BIASf, 0x4);
        TH4B0_PWRT_SET_FIELD(ALTERNATE_PATH_COSTf, 0x4);
        index = dlb_id;
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));

        sid = DLB_ECMP_GROUP_MEMBERSHIPm;
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        set_mem_bit(memfld, dst_dev_port, 0x1);
        SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_MAPf, entry, memfld));
        sal_memset(memfld, 0, sizeof(memfld));
        set_mem_bit(memfld, src_dev_port, 0x1);
        SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_MAP_ALTERNATEf, entry, memfld));
        index = dlb_id;
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
        ...
        for (i = 0; i < 4; i++) {
            sid = dlb_ecmp_port_members[i];
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            for (j = (i*16); j < (i+1)*16; j++) {
                TH4B0_PWRT_SET_FIELD(member_port[j], dst_dev_port);
                TH4B0_PWRT_SET_FIELD(member_port_valid[j], 1);
            }
            index = dlb_id;
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        ...
        for (i = 0; i < 5; i++) {
            sid = dlb_ecmp_nhi_members[i];
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            last_indx = (i == 4) ? 63 : (((i + 1)*13) - 1);
            for (j = (i * 13); j <= last_indx; j++) {
                TH4B0_PWRT_SET_FIELD(member_nhi[j], get_ul_next_hop_index(unit, flow_id));
                TH4B0_PWRT_SET_FIELD(member_nhi_valid[j], 1);
            }
            index = dlb_id;
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
...
static int setup_egr_ctrl_regs(int unit)
{
    ...
    TH4B0_PWRT_SET_FIELD(MTU_ENABLEf, 1);
    TH4B0_PWRT_SET_FIELD(MTU_SIZEf, 0x3fff);
    for (dev_port = 0 ; dev_port < TH4B0_PWRT_DEV_PORTS_PER_DEV; dev_port++) {
        if (power_test_p->dev_enabled_ports[dev_port] == 1) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, dev_port);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
        }
    }
    ...
    for (i = 0; i < 8; i++) {
        ...
        TH4B0_PWRT_SET_FIELD(BITMAPf, sel_val);
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    }
    ...
    int flow_id)
{
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(VALIDf, 0x1);
    TH4B0_PWRT_SET_FIELD(STGf, get_egr_stg(flow_id)); /* one entry in vlan profile is used */
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, 0x3);
    egr_vfi_profile_ptr = flow_id;
    TH4B0_PWRT_SET_FIELD(MEMBERSHIP_PROFILE_PTRf, egr_vfi_profile_ptr);
    if (is_vxlan_en(unit)){
        index = get_ol_vfi(unit, flow_id);
    } else {
    ...
    sid = EGR_VFI_MEMBERSHIPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    get_port_bmp_pipe(unit, 0, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAP_P0f, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    get_port_bmp_pipe(unit, 1, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAP_P1f, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    get_port_bmp_pipe(unit, 2, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAP_P2f, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    get_port_bmp_pipe(unit, 3, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAP_P3f, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    get_port_bmp_pipe(unit, 4, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAP_P4f, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    get_port_bmp_pipe(unit, 5, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAP_P5f, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    get_port_bmp_pipe(unit, 6, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAP_P6f, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    get_port_bmp_pipe(unit, 7, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, PORT_BITMAP_P7f, entry, memfld));
    index = egr_vfi_profile_ptr;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    if (is_vxlan_en(unit)) {
        ...
        TH4B0_PWRT_SET_FIELD(STGf, get_egr_stg(flow_id)); /* one entry in vlan profile is used */
        TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, 0x3);
        egr_vfi_profile_ptr = flow_id + 0x400;
        TH4B0_PWRT_SET_FIELD(MEMBERSHIP_PROFILE_PTRf, egr_vfi_profile_ptr);
        index = get_ul_vfi(unit, flow_id);
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (i = 0; i < 8; i++) {
        entry[i] = bcma_testutil_sal_rand32();
        entry_b[i] = bcma_testutil_sal_rand32();
    }
    entry[i] = bcma_testutil_sal_rand32() & 0xffff;
    entry_b[i] = bcma_testutil_sal_rand32() & 0xffff;
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (dst_port < (TH4B0_PWRT_DEV_PORTS_PER_DEV / 2)) {
            bit_pos = 2 * dst_port;
            set_field_in_memfld(memfld, entry, bit_pos, 2);
        } else {
            bit_pos = 2 * (dst_port - (TH4B0_PWRT_DEV_PORTS_PER_DEV / 2));
            set_field_in_memfld(memfld, entry_b, bit_pos, 2);
        }
        if (src_port < (TH4B0_PWRT_DEV_PORTS_PER_DEV / 2)) {
            bit_pos = 2 * src_port;
            set_field_in_memfld(memfld, entry, bit_pos, 2);
        } else {
            bit_pos = 2 * (src_port - (TH4B0_PWRT_DEV_PORTS_PER_DEV / 2));
            set_field_in_memfld(memfld, entry_b, bit_pos, 2);
        }
    }
    ...
    index = get_egr_stg(flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry_b));
    ...
    int flow_id)
{
    ...
    if (is_mpls_en(unit, flow_id, 0) == 1) {
        TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 0x1);  /* (MPLS) */
        TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_LABEL_ACTIONf, 0x1); /* PUSH */
        label_order = 3; /* BOS */
        get_mpls_label(unit, flow_id, label_order,
                   &mpls_label, &mpls_exp, &mpls_ttl, 0);
        TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_LABELf, mpls_label);
        TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_EXPf, mpls_exp);
        TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_TTLf, mpls_ttl);
        TH4B0_PWRT_SET_FIELD(MPLSv_MAC_DA_PROFILE_INDEXf,
                           get_egr_mac_da_profile_index(unit, flow_id));
        TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_EXP_SELECTf, 0x0);
        TH4B0_PWRT_SET_FIELD(MPLSv_MPLS_EXP_MAPPING_PTRf, 0x0);
        TH4B0_PWRT_SET_FIELD(MPLSv_LOOPBACK_PROFILE_IDXf,
                             1 + (bcma_testutil_sal_rand32() % 3));
    } else if (is_l3_en(unit, flow_id, 0) == 1) {
        TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 0x0);  /*NORMAL L3*/
        TH4B0_PWRT_SET_FIELD(L3v_L3_UC_DA_DISABLEf, 0x0);
        TH4B0_PWRT_SET_FIELD(L3v_L3_UC_SA_DISABLEf, 0x0);
        TH4B0_PWRT_SET_FIELD(L3v_L3_UC_TTL_DISABLEf, 0x0);
        TH4B0_PWRT_SET_FIELD(L3v_L3_UC_VLAN_DISABLEf, 0x0);
        TH4B0_PWRT_SET_FIELD(L3v_CLASS_IDf, 0x0);
        TH4B0_PWRT_SET_FIELD(L3v_LOOPBACK_PROFILE_IDXf,
                             1 + (bcma_testutil_sal_rand32() % 3));
        sal_memset(memfld, 0, sizeof(memfld));
        get_mac_da(unit, flow_id, mac_addr);
        convert_mac(mac_addr, memfld);
       SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, L3v_MAC_ADDRESSf, entry, memfld));
    }
    TH4B0_PWRT_SET_FIELD(MPLSv_FLEX_CTR_ACTION_SELf, 0x1);
    TH4B0_PWRT_SET_FIELD(MPLSv_FLEX_CTR_OBJECTf, 0x7fa1);
    index = get_next_hop_index(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    sid = EGR_L3_NEXT_HOP_OBJECT_MAPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(U_INTF_NUMf, get_egr_intf_num(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(U_INTF_NUM_VALIDf, 1);
    index = get_next_hop_index(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    if (is_mpls_en(unit, flow_id, 0) == 1) {
        get_mac_sa(unit, flow_id, mac_addr);
        convert_mac(mac_addr, memfld);
        SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, MAC_ADDRESSf, entry, memfld));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(VIDf, get_vlan_id(unit, flow_id));
        TH4B0_PWRT_SET_FIELD(VFIf, get_vlan_id(unit, flow_id));
        TH4B0_PWRT_SET_FIELD(MPLS_TUNNEL_INDEXf, get_egr_mpls_tunnel_index(unit, flow_id));
        TH4B0_PWRT_SET_FIELD(TTL_THRESHOLDf, 0x1);
        TH4B0_PWRT_SET_FIELD(OPRIf, 0x1);
    } else if (is_l3_en(unit, flow_id, 0) == 1) {
        sal_memset(memfld, 0, sizeof(memfld));
        get_mac_sa(unit, flow_id, mac_addr);
        convert_mac(mac_addr, memfld);
    ...
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, 0x2);
    ...
    if (is_mpls_en(unit, flow_id, 0) == 1) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 0x3);  /* (MPLS) */
        /* "Push Two MPLS Labels - from this entry and (N+1)." */
        TH4B0_PWRT_SET_FIELD(MPLS_PUSH_ACTION_0f, 0x2);
        /* "Push ONE MPLS Label - from this entry." ,*/
        TH4B0_PWRT_SET_FIELD(MPLS_PUSH_ACTION_1f, 0x1);
        label_order = 2; /* NOT BOS - MIDDLE label */
        get_mpls_label(unit, flow_id, label_order,
                   &mpls_label, &mpls_exp, &mpls_ttl, 0);
        TH4B0_PWRT_SET_FIELD(MPLS_LABEL_0f, mpls_label);
        TH4B0_PWRT_SET_FIELD(MPLS_EXP_0f, mpls_exp);
        TH4B0_PWRT_SET_FIELD(MPLS_TTL_0f, mpls_ttl);
        TH4B0_PWRT_SET_FIELD(NEW_PRI_0f, get_priority(unit, flow_id));
        label_order = 1; /* NOT BOS - TOP label */
        get_mpls_label(unit, flow_id, label_order,
                   &mpls_label, &mpls_exp, &mpls_ttl, 0);
        TH4B0_PWRT_SET_FIELD(MPLS_LABEL_1f, mpls_label);
        TH4B0_PWRT_SET_FIELD(MPLS_EXP_1f, mpls_exp);
        TH4B0_PWRT_SET_FIELD(MPLS_TTL_1f, mpls_ttl);
        TH4B0_PWRT_SET_FIELD(NEW_PRI_1f, get_priority(unit, flow_id));
 ...
 * EGR_IP_TUNNEL_IPV6m
*/
static int setup_egr_next_hop_vxlan(int unit,
    int flow_id)
{
    bcmdrd_sid_t sid;
    bcmbd_pt_dyn_info_t pt_dyn_info;
    ...
    /* {EGR_L3_NEXT_HOP} (tbl)
     *       => (O_NHI entry) entry_type=0x0 (L3),
     *          l3__mac_address=INNER_L2_HDR.DA;
     */
    sid = EGR_L3_NEXT_HOPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    ...
    convert_mac(mac_addr, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, L3v_MAC_ADDRESSf, entry, memfld));
    TH4B0_PWRT_SET_FIELD(L3v_FLEX_CTR_ACTION_SELf, 0x1);
    TH4B0_PWRT_SET_FIELD(L3v_FLEX_CTR_OBJECTf, 0x7fa1);
    index = get_ol_next_hop_index(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* {EGR_L3_NEXT_HOP_OBJECT_MAP} (tbl)
     *       => (O_NHI entry) dvp=DVP,
     *         o_intf_num=O_L3_OIF (index to egr_l3_intf tbl),
     *          o_intf_num_valid=0x1, u_*=0x0
     */
    sid = EGR_L3_NEXT_HOP_OBJECT_MAPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(DVPf, get_dvp(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(O_INTF_NUMf, get_egr_ol_intf_num(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(O_INTF_NUM_VALIDf, 1);
    index = get_ol_next_hop_index(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* {EGR_L3_INTF} (tbl)
     *       => (O_NHI entry) adapt_lookup_key_sel=KEY_VFI,
     *         vfi=O_VFI,
     *         tunnel_index=EGR_IP_TUNNEL index
     *      => ovid=INNER_L2_VLAN.OVID,
     *         mac_address=INNER_L2_HDR.SA
     */
    sid = EGR_L3_INTFm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ADAPT_LOOKUP_KEY_SELf, 3); /* VFI */
    TH4B0_PWRT_SET_FIELD(VFIf, get_ol_vfi(unit, flow_id));
    tunnel_index = get_egr_tunnel_index(unit, flow_id);
    tunnel_index = (tunnel_index << 1);
    TH4B0_PWRT_SET_FIELD(TUNNEL_INDEXf, tunnel_index);
    TH4B0_PWRT_SET_FIELD(OVIDf, get_ol_vlan_id(unit, flow_id));
    get_ol_mac_sa(unit, flow_id, mac_addr);
    convert_mac(mac_addr, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, MAC_ADDRESSf, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    ...
    TH4B0_PWRT_SET_FIELD(OPRIf, 0x1);
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, 0x2);
    index = get_egr_ol_intf_num(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* {EGR_IP_TUNNEL_IPV6} (tbl)
     *      => entry_type=IPV6, tunnel_type=VXLAN
     *      => dip=OUTER_IP_HDR.DIP, sip=OUTER_IP_HDR.SIP,
     *         ttl=OUTER_IP_HDR.HOP_LIMIT, dscp=OUTER_IP_HDR.TOS[7:2]
     *      => l4_dest_port=OUTER_UDP_HDR.DST_PORT,
     *         l4_src_port=OUTER_UDP_HDR.SRC_PORT
     */
    sid = EGR_IP_TUNNEL_IPV6m;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 0x2);  /* (IPV6) */
    TH4B0_PWRT_SET_FIELD(TUNNEL_TYPEf, 0x7);  /* (VXLAN) */
    get_ul_ip_addr_da(unit, flow_id, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, DIPf, entry, memfld));
    get_ul_ip_addr_sa(unit, flow_id, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SIPf, entry, memfld));
    TH4B0_PWRT_SET_FIELD(TTLf, get_ul_ttl(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(L4_DEST_PORTf, get_udp_dst_port(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(L4_SRC_PORTf, get_udp_src_port(unit, flow_id));
    index = get_egr_tunnel_index(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));

    /* {EGR_DVP} (tbl)
     *      => (indexed from egr_l3_next_hop_object_map.dvp)
     *         dvp_group_id=DVP_GROUP_ID (may be used in EGR_ADAPT lkup,
     *         depending on the key; in our case, it's not)
     */
    sid = EGR_DVPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(DVP_GROUP_IDf, get_dvp_group_id(unit, flow_id));
    index = get_dvp(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(DVP_NETWORK_GROUPf, get_dvp_group_id(unit, flow_id));
    index = get_dvp(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* {EGR_ADAPT} (tbl)
     *      => key_type=VFI,
     *         vfi__vfi=O_VFI (Key), vfi__vnid=VXLAN_HDR.VN_ID)
     */
    sid = EGR_ADAPT_SINGLEm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BASE_VALIDf, 1);   /* START_OF_KEY_BM_0 */
    TH4B0_PWRT_SET_FIELD(KEY_TYPEf, 2);   /* VFI */
    TH4B0_PWRT_SET_FIELD(VFIv_VFIf, get_ol_vfi(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(VFIv_VNIDf, get_vxlan_vnid_egr(unit, flow_id));
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_insert(unit, sid, entry));

    /* {EGR_L3_NEXT_HOP} (tbl)
     *      => (U_NHI entry) entry_type=0x0 (L3),
     *         l3__mac_address=OUTER_L2_HDR.DA,
     *         l3__flex_ctr_action_sel=0x1, l3__flex_ctr_object=0x1784
     */
    sid = EGR_L3_NEXT_HOPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    ...
    /* {EGR_L3_NEXT_HOP_OBJECT_MAP} (tbl)
     *     => (U_NHI entry) u_intf_num=U_L3_OIF,
     *         u_intf_num_valid=0x1, o_*=0, dvp=0x0
     */
    sid = EGR_L3_NEXT_HOP_OBJECT_MAPm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(DVPf, 0);
    TH4B0_PWRT_SET_FIELD(U_INTF_NUMf, get_egr_ul_intf_num(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(U_INTF_NUM_VALIDf, 1);
    index = get_ul_next_hop_index(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    /* {EGR_L3_INTF} (tbl)
     *      => (U_NHI entry)
     *         vfi=U_VFI (same as O_VFI),
     *         ovid=INNER_L2_VLAN.OVID, mac_address=OUTER_L2_HDR.SA
     */
    sid = EGR_L3_INTFm;
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(VFIf, get_ul_vfi(unit, flow_id));
    TH4B0_PWRT_SET_FIELD(OVIDf, get_ul_vlan_id(unit, flow_id));
    get_ul_mac_sa(unit, flow_id, mac_addr);
    convert_mac(mac_addr, memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, MAC_ADDRESSf, entry, memfld));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(TTL_THRESHOLDf, 0x1);
    TH4B0_PWRT_SET_FIELD(OPRIf, 0x1);
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, 0x2);
    index = get_egr_ul_intf_num(unit, flow_id);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    int flow_id)
{
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(PORT_TYPEf, 0x0); /* ETHERNET */
    TH4B0_PWRT_SET_FIELD(EN_EFILTERf, 0x1);
    TH4B0_PWRT_SET_FIELD(EFP_FILTER_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(ALLOW_NON_GSHf, 0x1);
    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, 0x0);
    /* Don't care, randomize for max toggling */
    TH4B0_PWRT_SET_FIELD(EGR_PORT_GROUP_IDf,
                         bcma_testutil_sal_rand32() & 0x1ff);
    TH4B0_PWRT_SET_FIELD(LATENCY_ECN_PROFILE_PTRf, flow_id % 8);
    TH4B0_PWRT_SET_FIELD(MIRROR_ENCAP_INDEXf, bcma_testutil_sal_rand32() & 0x7);
    ...
    int num_slices)
{
    ...
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            ...
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            TH4B0_PWRT_SET_FIELD(SOURCE_CLASS_MODEf, 0);
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT
                (th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
            ...
            for (i = 0; i < ifp_num_slices; i++) {
                ...
                sal_memset(entry, 0, sizeof(entry));
                sal_memset(memfld, 0, sizeof(memfld));
                TH4B0_PWRT_SET_FIELD(IFP_SLICE_ENABLEf, 0x1);
                TH4B0_PWRT_SET_FIELD(IFP_SLICE_LOOKUP_ENABLEf, 0x1);
                TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, i, ipep_acc);
                SHR_IF_ERR_EXIT
                    (th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
            }
        }
    ...
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            ...
            sal_memset(memfld, 0, sizeof(memfld));
            for (i = 0; i < 16; i++) {
                TH4B0_PWRT_SET_FIELD(logical_table_action_priority[i], i);
            }
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit,
    ...
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            for (log_table = 0; log_table < num_log_tables; log_table++) {
                TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, log_table, ipep_acc);
                sid = IFP_LOGICAL_TABLE_CONFIGr;
                sal_memset(entry, 0, sizeof(entry));
                sal_memset(memfld, 0, sizeof(memfld));
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_MAPf, 0x1 << log_table);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_0f, 0x1);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_1f, 0x2);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_2f, 0x3);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_3f, 0x4);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_4f, 0x5);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_5f, 0x6);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_6f, 0x7);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_7f, 0x8);
                TH4B0_PWRT_SET_FIELD(LOGICAL_PARTITION_PRIORITY_8f, 0x9);
                SHR_IF_ERR_EXIT
                    (th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
            }
        }
    ...
    for (log_table = 0; log_table < num_log_tables; log_table++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(L1_C_E16_SEL_0f, 0x12);
        TH4B0_PWRT_SET_FIELD(L2_E16_SEL_0f, 0x4);
        mindex = log_table;
        for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
            if (pp_map & (1 << ipep_acc)) {
                TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, mindex, ipep_acc);
                SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit,
                    sid, ltid, &pt_dyn_info, entry));
            }
        }
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(POST_EXTRACTOR_MUX_7_SELf, 0x1);
        mindex = log_table;
        for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
            if (pp_map & (1 << ipep_acc)) {
                TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, mindex, ipep_acc);
    ...
    for (flow_id = start_flow; flow_id < (start_flow + num_flows); flow_id++) {
        ...
        for (log_table = 0; log_table < num_log_tables; log_table++) {
            ...
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            TH4B0_PWRT_SET_FIELD(TTL_OVERRIDE_SETf, 0x1);
            ...
            counter_set = (flex_ctr_action << 0) |
                          (flex_ctr_object << 5) |
                          (0x1 << 19);
            TH4B0_PWRT_SET_FIELD(COUNTER_SETf, counter_set);
            ...
            LOOPBACK_PROFILE_INDEX = bcma_testutil_sal_rand32() & 0x3;
            counter_set = (INT_ACTION_PROFILE_IDX << 2) |
                          (LOOPBACK_PROFILE_INDEX <<  24) |
                          (MMU_INT_STAT_REQ_0 << 26) |
                          (MMU_INT_STAT_REQ_1 << 29);
            TH4B0_PWRT_SET_FIELD(INBAND_TELEMETRY_SETf, counter_set);

            counter_set = (bcma_testutil_sal_rand32() & 0xfc); /* CHANGE_CPU_COS */
            TH4B0_PWRT_SET_FIELD(CHANGE_CPU_COS_SETf, counter_set);
            counter_set = (bcma_testutil_sal_rand32() & 0x1ff7f); /* DELAYED_REDIRECT_ENABLE=0 */
            TH4B0_PWRT_SET_FIELD(DELAYED_REDIRECT_SETf, counter_set);
            counter_set = (bcma_testutil_sal_rand32() & 0x7f); /* DELAYED_DROP_ENABLE = 0 */
            TH4B0_PWRT_SET_FIELD(DELAYED_DROP_SETf, counter_set);
            counter_set = (bcma_testutil_sal_rand32() & 0x3fe); /* DLB_ALTERNATE_PATH_ACTION_VALID = 0 */
            TH4B0_PWRT_SET_FIELD(DLB_ALTERNATE_PATH_CONTROL_SETf, counter_set);
            counter_set = (bcma_testutil_sal_rand32() & 0x3f); /* POLICY_MIRROR_FLOW_CLASS_ENABLE = 0 */
            TH4B0_PWRT_SET_FIELD(IFP_MULTIPLE_MIRROR_DESTINATION_SETf, counter_set);
            ...
            if (((power_test_p->enable_map >> TH4B0_PWRT_EN_RND_PRI) & 0x1) == 0x1) {
                ...
                TH4B0_PWRT_SET_FIELD(PROFILE_SET_1f, counter_set);
                /* G_CHANGE_DSCP_TOS = CHANGE_DSCP,
                 * G_CHANGE_ECN =1,
                 * G_CHANGE_PKT_PRI = 0
                 */
                counter_set = (0x3 << 0) | (0x1 << 3) | (0x5 << 4);
                TH4B0_PWRT_SET_FIELD(PROFILE_SET_2f, counter_set);
            }
...
static int setup_vfp(int unit)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(LOOKUP_ENABLE_SLICE_0f, 0x1);
    TH4B0_PWRT_SET_FIELD(LOOKUP_ENABLE_SLICE_1f, 0x1);
    TH4B0_PWRT_SET_FIELD(LOOKUP_ENABLE_SLICE_2f, 0x1);
    TH4B0_PWRT_SET_FIELD(LOOKUP_ENABLE_SLICE_3f, 0x1);
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(SLICE_0_F2f, 0x4);
    TH4B0_PWRT_SET_FIELD(SLICE_1_F2f, 0x4);
    TH4B0_PWRT_SET_FIELD(SLICE_2_F2f, 0x4);
    TH4B0_PWRT_SET_FIELD(SLICE_3_F2f, 0x4);
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(VIRTUAL_SLICE_0_PHYSICAL_SLICE_NUMBERf, 0x0);
    TH4B0_PWRT_SET_FIELD(VIRTUAL_SLICE_1_PHYSICAL_SLICE_NUMBERf, 0x1);
    TH4B0_PWRT_SET_FIELD(VIRTUAL_SLICE_2_PHYSICAL_SLICE_NUMBERf, 0x2);
    TH4B0_PWRT_SET_FIELD(VIRTUAL_SLICE_3_PHYSICAL_SLICE_NUMBERf, 0x3);
    TH4B0_PWRT_SET_FIELD(VIRTUAL_SLICE_0_VIRTUAL_SLICE_GROUPf, 0x0);
    TH4B0_PWRT_SET_FIELD(VIRTUAL_SLICE_1_VIRTUAL_SLICE_GROUPf, 0x1);
    TH4B0_PWRT_SET_FIELD(VIRTUAL_SLICE_2_VIRTUAL_SLICE_GROUPf, 0x2);
    TH4B0_PWRT_SET_FIELD(VIRTUAL_SLICE_3_VIRTUAL_SLICE_GROUPf, 0x3);
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    for (flow_id = start_flow; flow_id < (start_flow + num_flows); flow_id++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(USE_VFP_CLASS_ID_Hf, 0x1);
        TH4B0_PWRT_SET_FIELD(USE_VFP_CLASS_ID_Lf, 0x1);
        TH4B0_PWRT_SET_FIELD(VFP_CLASS_ID_Hf, 0x300 + flow_id); /* just dummy */
        TH4B0_PWRT_SET_FIELD(VFP_CLASS_ID_Lf, 0x200 + flow_id);
        TH4B0_PWRT_SET_FIELD(VFP_MATCHED_RULEf, 0x30+flow_id);
        for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
            if (pp_map & (1 << ipep_acc)) {
                for (slice = 0; slice < 8; slice++) {
                    index = (128 * slice) + (flow_id / 2);
                    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, 24 + (slice / 2));
                    TH4B0_PWRT_SET_FIELD(FLEX_CTR_OBJECTf, 0x3a80 + (slice / 2));
                    TH4B0_PWRT_SET_FIELD(HASH_FIELD_BITMAP_PTR_Af,
                                         1 + (bcma_testutil_sal_rand32() % 15));
                    TH4B0_PWRT_SET_FIELD(HASH_FIELD_BITMAP_PTR_Bf,
                                         1 + (bcma_testutil_sal_rand32() % 15));
                    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, ipep_acc);
                    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
                        unit, sid, ltid, &pt_dyn_info, entry));
                }
            }
...
static int setup_efp(int unit)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(SLICE_ENABLE_SLICE_0f, 0x1);
    TH4B0_PWRT_SET_FIELD(SLICE_ENABLE_SLICE_1f, 0x1);
    TH4B0_PWRT_SET_FIELD(SLICE_ENABLE_SLICE_2f, 0x1);
    TH4B0_PWRT_SET_FIELD(SLICE_ENABLE_SLICE_3f, 0x1);
    TH4B0_PWRT_SET_FIELD(LOOKUP_ENABLE_SLICE_0f, 0x1);
    TH4B0_PWRT_SET_FIELD(LOOKUP_ENABLE_SLICE_1f, 0x1);
    TH4B0_PWRT_SET_FIELD(LOOKUP_ENABLE_SLICE_2f, 0x1);
    TH4B0_PWRT_SET_FIELD(LOOKUP_ENABLE_SLICE_3f, 0x1);
    TH4B0_PWRT_SET_FIELD(SLICE_0_MODEf, 0x0);
    TH4B0_PWRT_SET_FIELD(SLICE_1_MODEf, 0x0);
    TH4B0_PWRT_SET_FIELD(SLICE_2_MODEf, 0x0);
    TH4B0_PWRT_SET_FIELD(SLICE_3_MODEf, 0x0);
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    for (flow_id = start_flow; flow_id < (start_flow + num_flows); flow_id++) {
        ...
        sal_memset(fld_memfld_64, 0, sizeof(fld_memfld_64));
        /* EFP_KEY4__NUMBER_OF_TAGS        204:203 */
        fld_memfld_64[0] = 2; /* NUMBER OF TAGS */
        set_field_in_memfld_64(fld_memfld_64, fld_memfld_64, 203, 2);
        sal_memset(fld_memfld_64, 0, sizeof(fld_memfld_64));
        /* EFP_KEY4__MAC_SA        201:154 */
        if (is_vxlan_en(unit)){
            get_ul_mac_sa(unit, flow_id, mac_addr);
        }else{
            get_mac_sa(unit, flow_id, mac_addr);
        }
        fld_memfld_64[0] = ((uint64_t)mac_addr[0] << 8) | ((uint64_t)mac_addr[1] << 0);
        fld_memfld_64[0] = fld_memfld_64[0] << 32;
        fld_memfld_64[0] |= ((uint64_t)mac_addr[2] << 24) | ((uint64_t)mac_addr[3] << 16) | ((uint64_t)mac_addr[4] << 8) | ((uint64_t)mac_addr[5] << 0);
        set_field_in_memfld_64(fld_memfld_64, memfld_64, 154, 48);
         ...
         * FLEX_CTR_OBJECT
         */
        TH4B0_PWRT_SET_FIELD(ENTRY_TYPEf, 0x0); /* DEFAULT */

        sal_memset(memfld, 0, sizeof(memfld));
        memfld[0] = (flow_id & 0xff) | ((flow_id & 0xff) << 8) |
                    ((flow_id & 0xff) << 16) | ((flow_id & 0xff) << 24);
        memfld[1] = 0x1f;
        SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, DEFAULTv_RESERVED_PADDINGf, entry, memfld));

        for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
            if (pp_map & (1 << ipep_acc)) {
                for (slice = 0; slice < 4; slice++) {
                    index = (128 * slice) + (flow_id / 2);
                    TH4B0_PWRT_SET_FIELD(FLEX_CTR_ACTIONf, 0x8 + slice);
                    TH4B0_PWRT_SET_FIELD(FLEX_CTR_OBJECTf, 0x1f0 + slice);
                    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, ipep_acc);
                    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
                        unit, sid, ltid, &pt_dyn_info, entry));
                }
...
static int setup_elephant_trap(int unit)
{
    ...
    sal_memset(memfld, 0, sizeof(memfld));
    set_dev_bmp_all_one(memfld);
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, ING_PORT_BMP_ENf, entry, memfld));
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ETRAP_MONITOR_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(SEEDf, 0x123);
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(INT_PRI_BMP_ENf, 0xffff);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(PKT_TYPE_OTHER_ENf, 0x1);
    TH4B0_PWRT_SET_FIELD(MPLS_ENf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPV6_UDP_ENf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPV6_TCP_ENf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPV6_OTHER_ENf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPV4_UDP_ENf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPV4_TCP_ENf, 0x1);
    TH4B0_PWRT_SET_FIELD(IPV4_OTHER_ENf, 0x1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ETRAP_ENf, 0x1);
    TH4B0_PWRT_SET_FIELD(BLOOM_FLOW_INS_ENf, 0x1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(THRESHOLD_BYTESf, 0xb);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BYTE_THRESHOLDf, 0x18);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BYTE_THRESHOLDf, 0x1b);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(CRITICAL_TIME_PERIODf, 0x5); /* 500us */
    TH4B0_PWRT_SET_FIELD(ETRAP_INTERVALf, 0x1);  /* 2ms */
    TH4B0_PWRT_SET_FIELD(RIGHT_BANK_HASH_ROTRf, 0x2);
    TH4B0_PWRT_SET_FIELD(RIGHT_BANK_HASH_SELf, 0x0);
    TH4B0_PWRT_SET_FIELD(LEFT_BANK_HASH_ROTRf, 0x2);
    TH4B0_PWRT_SET_FIELD(LEFT_BANK_HASH_SELf, 0x0);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(FILTER_0_HASH_ROTRf, 0x4);
    TH4B0_PWRT_SET_FIELD(FILTER_0_HASH_SELf, 0x0);
    TH4B0_PWRT_SET_FIELD(FILTER_1_HASH_ROTRf, 0x3);
    TH4B0_PWRT_SET_FIELD(FILTER_1_HASH_SELf, 0x1);
    TH4B0_PWRT_SET_FIELD(FILTER_2_HASH_ROTRf, 0x4);
    TH4B0_PWRT_SET_FIELD(FILTER_2_HASH_SELf, 0x3);
    TH4B0_PWRT_SET_FIELD(FILTER_3_HASH_ROTRf, 0x2);
    TH4B0_PWRT_SET_FIELD(FILTER_3_HASH_SELf, 0x2);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (i = 0; i< 2; i++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(REPORT_ELEPHANTSf, 0x1);
        TH4B0_PWRT_SET_FIELD(REPORT_EVICTIONSf, 0x1);
        for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
            if (pp_map & (1 << ipep_acc)) {
                TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
                SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
...
static int setup_exact_match(int unit)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(SOURCE_CLASS_MODEf, 0x0);
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(SEARCH_MASKf, 0xffff);
    index = em0_key_profile_index; /* matches KEY_GEN_PROGRAM_PROFILE_INDEXf */
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, ipep_acc);
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(KEY_TYPEf, 0x0); /* MODE128 */
    TH4B0_PWRT_SET_FIELD(BASE_VALID_0f, 0x1); /* START_OF_KEY_BM_0*/
    TH4B0_PWRT_SET_FIELD(BASE_VALID_1f, 0x2); /* key continues; no key_type in this entry */
    TH4B0_PWRT_SET_FIELD(DATA_TYPEf, 0x1); /* index in EXACT_MATCH_ACTION_PROFILEm */
    TH4B0_PWRT_SET_FIELD(MODE128v_KEY_0_ONLYf, get_ether_type(unit, 0));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ACTION_SET_BITMAPf, 0x8);
    TH4B0_PWRT_SET_FIELD(COUNTER_SET_ENABLEf, 0x1);
    index = 1;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(L1_C_E16_SEL_0f, 0x14);
    TH4B0_PWRT_SET_FIELD(L1_C_E32_SEL_0f, 0x3);
    TH4B0_PWRT_SET_FIELD(L2_E16_SEL_4f, 0x4);
    ...
    memfld[1] = 0xffffffff;
    memfld[2] = 0x000fffff;
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, SEARCH_MASKf, entry, memfld));
    index = em1_key_profile_index; /* matches KEY_GEN_PROGRAM_PROFILE_INDEXf */
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, ipep_acc);
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(KEY_TYPEf, 0x2); /* MODE 320 */
    TH4B0_PWRT_SET_FIELD(BASE_VALID_0f, 0x1); /* start of key */
    TH4B0_PWRT_SET_FIELD(BASE_VALID_1f, 0x2); /* key continues; no key_type in this entry */
    TH4B0_PWRT_SET_FIELD(BASE_VALID_2f, 0x2); /* key continues; no key_type in this entry */
    TH4B0_PWRT_SET_FIELD(BASE_VALID_3f, 0x7); /* key continues; no key_type in this entry */
    TH4B0_PWRT_SET_FIELD(DATA_TYPEf, 0xf); /* index in EXACT_MATCH_ACTION_PROFILEm */
    sal_memset(memfld, 0, sizeof(memfld));
    memfld[0] = 0xfff0ffff;
    memfld[1] = 0xffffffff;
    memfld[2] = 0x000fffff;
    SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, MODE320v_KEY_0_ONLYf, entry, memfld));
    index = 0;  /* This will be insert */
    index = 0x1777;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_insert(unit, sid, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(ACTION_SET_BITMAPf, 0xc);
    TH4B0_PWRT_SET_FIELD(COPY_TO_CPU_SET_ENABLEf, 0x1);
    TH4B0_PWRT_SET_FIELD(COUNTER_SET_ENABLEf, 0x1);
...
static int setup_lag_fast_trunk(int unit, int flow_id)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(TG_SIZEf, 0x0); /* always select TRUNK_MEMBER_0 */
    TH4B0_PWRT_SET_FIELD(TRUNK_MODEf, 0x0); /* RTAG7 HASH */
    index = flow_id;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    TH4B0_PWRT_SET_FIELD(TRUNK_MEMBER_0f, get_final_dst_dev_port(unit, flow_id));
    index = flow_id;
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
...
static int setup_flex_ctr(int unit)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(V6UC_FLEX_CTR_ACTION_0f, 0x7);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BITMAPf, ing_action_enable);
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    for (action = 0; action < total_actions; action++) {
        if (ing_action_enable & (1 << action)) {
            ...
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            TH4B0_PWRT_SET_FIELD(POOL_BASEf, pool_no);
            TH4B0_PWRT_SET_FIELD(B_CVALUE_1_OPf, action);
            TH4B0_PWRT_SET_FIELD(A_CVALUE_1_OPf, action);
            TH4B0_PWRT_SET_FIELD(CINDEX_OPf, action);
            TH4B0_PWRT_SET_FIELD(CINDEX_BASEf, 0x0);
            TH4B0_PWRT_SET_FIELD(SIZE0_SEL_0f, 21); /* obj1 = src port num */
            TH4B0_PWRT_SET_FIELD(SIZE0_SEL_1f, 36); /* toggling */
            TH4B0_PWRT_SET_FIELD(SIZE0_SEL_2f, 56);
            TH4B0_PWRT_SET_FIELD(SIZE0_SEL_3f, 31);
            for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
                if (pp_map & (1 << ipep_acc)) {
                    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, action, ipep_acc);
                    sid = FLEX_CTR_ING_COUNTER_ACTION_TABLE_0_INST0m;
                    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
                        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (action = 0; action < total_actions; action++) {
        if (ing_action_enable & (1 << action)) {
            ...
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            TH4B0_PWRT_SET_FIELD(B_UPDATE_FN_TRUEf, 2); /* INC_BY_CVALUE */
            TH4B0_PWRT_SET_FIELD(A_UPDATE_FN_TRUEf, 2); /* INC_BY_CVALUE */
            for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
                if (pp_map & (1 << ipep_acc)) {
                    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, action, ipep_acc);
                    sid = FLEX_CTR_ING_COUNTER_ACTION_TABLE_1_INST0m;
    ...
    for (action = 0; action < total_actions; action++) {
        if (ing_action_enable & (1 << action)) {
            ...
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            TH4B0_PWRT_SET_FIELD(SEL_1f, 0x2); /* obj1 */
            TH4B0_PWRT_SET_FIELD(MASK_SIZE_1f, 0x8);
            TH4B0_PWRT_SET_FIELD(SEL_2f, 0x0);
            for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
                if (pp_map & (1 << ipep_acc)) {
                    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, action, ipep_acc);
                    sid = FLEX_CTR_ING_COUNTER_OP_PROFILE_TABLE_INST0m;
    ...
    for (pool_no = 0; pool_no < (2 * flex_ctr_pool_num); pool_no++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(COUNTER_POOL_ENABLEf, 0x1);
        for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
            if (pp_map & (1 << ipep_acc)) {
                TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
                SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(MPLS_FLEX_CTR_ACTION_0f, 0xf);
    TH4B0_PWRT_SET_FIELD(MPLS_FLEX_CTR_ACTION_1f, 0x1);
    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
        unit, sid, ltid, &pt_dyn_info, entry));
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(BITMAPf, egr_action_enable);
    for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
        if (pp_map & (1 << ipep_acc)) {
            TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, 0, ipep_acc);
            SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    for (action = 0; action < total_actions; action++) {
        if (egr_action_enable & (1 << action)) {
            ...
            TH4B0_PWRT_SET_FIELD(CINDEX_OPf, action);
            TH4B0_PWRT_SET_FIELD(CINDEX_BASEf, 0x0);
            TH4B0_PWRT_SET_FIELD(SIZE0_SEL_0f, 5); /* obj1 = dst port num */
            TH4B0_PWRT_SET_FIELD(SIZE0_SEL_1f, 12); /* toggling */
            TH4B0_PWRT_SET_FIELD(SIZE0_SEL_2f, 2);
            TH4B0_PWRT_SET_FIELD(SIZE0_SEL_3f, 1);
            for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
                if (pp_map & (1 << ipep_acc)) {
                    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, action, ipep_acc);
                    sid = FLEX_CTR_EGR_COUNTER_ACTION_TABLE_0_INST0m;
    ...
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_RND_INITFLEXCTR) & 0x1) != 0) {
    flex_ctr_pool_num = 12;
    for (pool_no = 0; pool_no < (2 * flex_ctr_pool_num); pool_no++) {
        sid = ing_flex_ctr_table[pool_no];
        /* flex cntrs addreses will be dev_port num */
        for (index = 0; index < TH4B0_PWRT_DEV_PORTS_PER_DEV; index++) {
            sal_memset(entry, 0, sizeof(entry));
            sal_memset(memfld, 0, sizeof(memfld));
            memfld[0] = bcma_testutil_sal_rand32();
            memfld[1] = bcma_testutil_sal_rand32();
            memfld[2] = bcma_testutil_sal_rand32() & 0xff;
            SHR_IF_ERR_EXIT(th4b0_pwrt_field_set(unit, sid, WIDE_COUNTERf, entry, memfld));
            for (ipep_acc = 0; ipep_acc < 4; ipep_acc++) {
                if (pp_map & (1 << ipep_acc)) {
                    TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, ipep_acc);
                    SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
    ...
    int flow_id)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    sal_memset(memfld, 0, sizeof(memfld));
    TH4B0_PWRT_SET_FIELD(FRAGMENT_IDf, bcma_testutil_sal_rand32() & 0xffff);
    ...
    int unit)
{
    ...
    for (index = 1; index < 16; index ++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(EGR_METADATA_FIFO_RW_ENABLEf, 0x1);
        TH4B0_PWRT_SET_FIELD(MD_HDR_TYPEf, bcma_testutil_sal_rand32() & 0x7);
        TH4B0_PWRT_SET_FIELD(MD_HDR_LEN_GRANULARITYf, 0);
        TH4B0_PWRT_SET_FIELD(IFA_FLAGSf, bcma_testutil_sal_rand32() & 0xff);
        TH4B0_PWRT_SET_FIELD(MAX_OR_REMAINING_LENf,
                             bcma_testutil_sal_rand32() & 0xffff);
        TH4B0_PWRT_SET_FIELD(IP_HDR_TTLf, bcma_testutil_sal_rand32() & 0x3fff);
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
    }
    ...
    for (index = 0; index < 4; index ++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(DCN_PKT_SIZEf, bcma_testutil_sal_rand32() & 0x3);
        TH4B0_PWRT_SET_FIELD(DCN_PKT_TOSf, bcma_testutil_sal_rand32() & 0xff);
        TH4B0_PWRT_SET_FIELD(DCN_PKT_TOS_MASKf,
                             bcma_testutil_sal_rand32() & 0xff);
        TH4B0_PWRT_SET_FIELD(DCN_PKT_UPDATE_TOSf,
                             bcma_testutil_sal_rand32() & 0x1);
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (index = 0; index < 4; index ++) {
        ...
        entry[0] = bcma_testutil_sal_rand32() & 0xfff;
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
    }
    ...
    for (index = 0; index < 8; index ++) {
        ...
        sal_memset(memfld, 0, sizeof(memfld));
        /* just randomize the entry */
        entry[0] = bcma_testutil_sal_rand32();
        entry[1] = bcma_testutil_sal_rand32() & 0x1ffff;
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
    }
    ...
    for (index = 0; index < 8; index ++) {
        ...
        sal_memset(memfld, 0, sizeof(memfld));
        for (i = 0; i < 24; i++) {
            memfld[0] = ((bcma_testutil_sal_rand32() & 0x3f) << 1) | 0x1;
    ...
    for (index = 0; index < 8; index ++) {
         ...
         */
        for (i = 0; i < 12; i++) {
            memfld[0] = ((4 + (bcma_testutil_sal_rand32() % 17)) << 0) |
                        (0  << 6) |
                        (15 << 10) |
                        (0  << 15);
         ...
         */
        for (i = 0; i < 12; i++) {
            memfld[0] = ((2 + (bcma_testutil_sal_rand32() % 9)) << 0) |
                        (0  << 5) |
                        (31 << 10) |
                        (0  << 16);
    ...
    for (index = 0; index < 512; index ++) {
        ...
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(OPAQUE_FIELD_1f, bcma_testutil_sal_rand32());
        TH4B0_PWRT_SET_FIELD(OPAQUE_FIELD_2f, bcma_testutil_sal_rand32());
        TH4B0_PWRT_SET_FIELD(OPAQUE_FIELD_3f, bcma_testutil_sal_rand32());
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
    }
...
static int setup_ing_pri_dscp_cn(int unit)
{
    ...
    for (i = 0; i < 16; i++) {
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(INT_CN_PROFILE_PTRf, i & 0x1);
        index = i;
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (i = 0; i < 16; i++) {
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(INT_CNf, i & 0x3);
        index = i;
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (i = 0; i < 64; i++) {
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(NEW_CFIf, 0x0);
        TH4B0_PWRT_SET_FIELD(NEW_DOT1Pf, i & 0x7);
        index = i * 16;
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
    ...
    for (i = 0; i < 8; i++) {
        sal_memset(entry, 0, sizeof(entry));
        sal_memset(memfld, 0, sizeof(memfld));
        TH4B0_PWRT_SET_FIELD(CHANGE_INNER_ECNf,
                             bcma_testutil_sal_rand32() & 0x1);
        TH4B0_PWRT_SET_FIELD(INCREMENT_ECN_COUNTERf,
                             bcma_testutil_sal_rand32() & 0x1);
        TH4B0_PWRT_SET_FIELD(INNER_ECNf, bcma_testutil_sal_rand32() & 0x3);
        TH4B0_PWRT_SET_FIELD(COPY_TO_CPUf, 0);
        TH4B0_PWRT_SET_FIELD(DROPf, 0);
        index = i * 16;
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(
            unit, sid, ltid, &pt_dyn_info, entry));
...
static int setup_idb_ct(int unit)
{
    ...
    ct_disable = (((power_test_p->enable_map >> TH4B0_PWRT_DIS_CUT_THROUGH) & 0x1) == 0x1) ? 1 : 0;
...
static int power_test_setup_ipep(int unit)
{
    ...
    for (flow_id = start_flow; flow_id < (start_flow + num_flows); flow_id++) {
        ...
        if ((is_l2_only_en(unit, flow_id, 0) == 0) && (is_vxlan_en(unit) == 0)) {
            if ((flow_id % 2) == 0){
        ...
        if (is_vxlan_en(unit) == 1) {
            cli_out("flow_id=%0d setup_ing_mpls_entry_for_vxlan FP port \n", flow_id);
            SHR_IF_ERR_EXIT(setup_ing_mpls_entry_for_vxlan(unit, flow_id, 0));
            SHR_IF_ERR_EXIT(setup_ing_svp(unit, flow_id));
        }
        ...
        if (is_l2_only_en(unit, flow_id, 0) == 0) {
            if (is_vxlan_en(unit)) {
                SHR_IF_ERR_EXIT(setup_ing_next_hop_vxlan(unit, flow_id));
            } else {
                /*cli_out("flow_id=%0d setup_ing_next_hop\n", flow_id);*/
    ...
    for (flow_id = start_flow; flow_id < (start_flow + num_flows); flow_id++) {
        ...
        if (is_vxlan_en(unit) == 1) {
            SHR_IF_ERR_EXIT(setup_egr_next_hop_vxlan(unit, flow_id));
        }else{
    ...
    if (power_test_p->enable_ecmp_dlb != 0) {
        ...
        SHR_IF_ERR_EXIT(power_test_setup_dlb_ecmp_regs(unit));
        for (flow_id = start_flow; flow_id < (start_flow + num_flows); flow_id++) {
            if (is_vxlan_en(unit)) {
                SHR_IF_ERR_EXIT(setup_ing_ecmp_vxlan(unit, flow_id, 0));
            } else {
                SHR_IF_ERR_EXIT(setup_ing_ecmp(unit, flow_id, 0));
            }
        }
        SHR_IF_ERR_EXIT(power_test_setup_dlb_ecmp(unit));
    }

    cli_out("START power_test_setup_ipep() MISC\n");
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_FULL_IFP) & 0x1) != 0) {
        num_slices = 9;
    } else {
        num_slices = 1;
    }
    ...
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_EM) & 0x1) != 0) {
        if (power_test_p->verbose_level >= 1) {
    ...
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_VFP) & 0x1) != 0) {
        if (power_test_p->verbose_level >= 1) {
            cli_out("START setup_vfp\n");
        }
        SHR_IF_ERR_EXIT(setup_vfp(unit));
    }
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_EFP) & 0x1) != 0) {
        if (power_test_p->verbose_level >= 1) {
            cli_out("START setup_efp\n");
        }
        SHR_IF_ERR_EXIT(setup_efp(unit));
    }
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_TRAP) & 0x1) != 0) {
        if (power_test_p->verbose_level >= 1) {
            cli_out("START setup_elephant_trap\n");
        }
        SHR_IF_ERR_EXIT(setup_elephant_trap(unit));
    }
    if (((power_test_p->enable_map >> TH4B0_PWRT_EN_FLX_CTR) & 0x1) != 0) {
        if (power_test_p->verbose_level >= 1) {
    ...
    int port)
{
    ...
    if (is_vxlan_en(unit)) {
        ...
        TH4B0_PWRT_PT_DYN_INFO(pt_dyn_info, index, 0);
        SHR_IF_ERR_EXIT(bcmbd_pt_read(unit, sid, &pt_dyn_info, NULL, entry, wsize));
        TH4B0_PWRT_SET_FIELD(DROPf, 0x1);
        TH4B0_PWRT_SET_FIELD(COPY_TO_CPUf, 0x1);
        SHR_IF_ERR_EXIT(th4b0_pwrt_pt_write(unit, sid, ltid, &pt_dyn_info, entry));
    ...
    bcma_testutil_pkt_attr_t *pkt_attr)
{
    ...
    if (is_vxlan_en(unit) == 0) {
        ...
        l_pkt_attr.vlan_id = get_ol_vlan_id(unit, flow_id);
    }
    ...
    for (i = 0; i < 4; i++) {
        l_pkt_attr.ip_da[i] = ip_addr[i];
    }
    get_ip_addr_sa(unit, flow_id, ip_addr);
    for (i = 0; i < 4; i++) {
        l_pkt_attr.ip_sa[i] = ip_addr[i];
    }
    l_pkt_attr.ttl        = 0xff;
    l_pkt_attr.tos        = 0;
    ...
    /* VXLAN only */
    if (is_vxlan_en(unit) == 1) {
        l_pkt_attr.vxlan_en = 1;
        get_ul_mac_da(unit, flow_id, mac_addr);
        for (i = 0; i < BCMA_TESTUTIL_PACKET_NUM_BYTES_MAC_ADDR; i++) {
            l_pkt_attr.ul_mac_da[i] = mac_addr[i];
        }
        get_ul_mac_sa(unit, flow_id, mac_addr);
        for (i = 0; i < BCMA_TESTUTIL_PACKET_NUM_BYTES_MAC_ADDR; i++) {
            l_pkt_attr.ul_mac_sa[i] = mac_addr[i];
        }
        l_pkt_attr.ul_vlan_id = get_ul_vlan_id(unit, flow_id);
        get_ul_ip_addr_sa(unit, flow_id, ip_addr);
        for (i = 0; i < 4; i++) {
            l_pkt_attr.ul_ip_sa[i] = ip_addr[i];
        }
        get_ul_ip_addr_da(unit, flow_id, ip_addr);
        for (i = 0; i < 4; i++) {
            l_pkt_attr.ul_ip_da[i] = ip_addr[i];
        }
        l_pkt_attr.vxlan_vnid = get_vxlan_vnid_ing(unit, flow_id);
    }

    if (is_vxlan_en(unit) == 0) {  /* MPLS */
        SHR_IF_ERR_EXIT
            (bcma_testutil_mpls_packet_fill(unit, buf, &l_pkt_attr));
    } else {                       /* VXLAN */
        SHR_IF_ERR_EXIT
            (bcma_testutil_vxlan_packet_fill(unit, buf, &l_pkt_attr));
    }
...
bcm56990_b0_power_init(int unit, void *bp)
{
    ...
    LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "bcm56990_b0_power_init() START \n")));
...
#define BCM56990_B0_TRAFFIC_MAX_POWER_PARAM_HELP \
    "\n\n"\
    "  TH4B0 DevSpecific PARAM details:\n"\
    "  LpmLevel         - TH4B0 Alpm level. Admissable values: 1,2,3; default=1\n"\
    "  Topology         - TH4B0 Port traffic topology. default=0. Admissable values:\n"\
    ...
    "  PktFlow          - PktFlow type: 0 - MPLS_IPV6; 5 - VXLAN_IPV6. Default: MPLS_IPV6\n"\
    "  EnableBmp        - TH4B0 Power test flags; for debug only.\n"


int
bcm56990_b0_power_help(int unit)
{
    cli_out("%s", BCM56990_B0_TRAFFIC_MAX_POWER_PARAM_HELP);
    return SHR_E_NONE;
}
...
bcm56990_b0_power_port_set(int unit, void *bp)
{
    ...
    cli_out("bcm56990_b0_power_port_set CALLED\n");
...
bcm56990_b0_power_txrx(int unit, void *bp)
{
    ...
    cli_out("bcm56990_b0_power_txrx() CALLED \n");
    ...
    if (tra_param->config_n_send) {
        BCMDRD_PBMP_ITER(power_test_p->pbmp_traffic, port) {
            ...
            for (idx = 0; idx < tra_param->pkt_cnt[port]; idx++) {
                ...
                rv = bcm56990_b0_power_fill_pkt(unit, packet->data_buf,
                                                &pkt_attr);
                if (SHR_FAILURE(rv)) {
                    bcmpkt_free(pktdev_id, packet);
                    SHR_ERR_EXIT(rv);
                }
                rv =bcmpkt_fwd_port_set(pkt_dev_type, port, packet);
                if (SHR_FAILURE(rv)) {
                    bcmpkt_free(pktdev_id, packet);
                    SHR_ERR_EXIT(rv);
                }
                ...
                rv = bcmpkt_tx(unit, TRAFFIC_MAX_POWER_DEFAULT_NETIF_ID, packet);
                bcmpkt_free(pktdev_id, packet);
                if (SHR_FAILURE(rv)) {
                    SHR_ERR_EXIT(rv);
                }
    ...
    if (tra_param->check_n_stop) {
        ...
        cli_out("bcm56990_b0_power_txrx() run for %0d seconds \n", tra_param->runtime);
        ...
        if (tra_param->need_check_rx == TRUE) {
            BCMDRD_PBMP_ITER(power_test_p->pbmp_traffic, port) {
                ...
                if (test_result == false) {
                    cli_out("bcm56990_b0_power_txrx() PORT= %0d rx_data check CHECK_RESULT= %0d\n", port, test_result);
            ...
            SHR_IF_ERR_EXIT(bcma_testutil_traffic_check_result_disp_add(unit,
                                                     data_check_result,
    ...
    cli_out("bcm56990_b0_power_txrx() FINISHED \n");
