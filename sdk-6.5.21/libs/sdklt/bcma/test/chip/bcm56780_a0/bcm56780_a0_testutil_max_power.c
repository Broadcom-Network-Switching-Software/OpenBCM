...
#define TRAFFIC_MAX_POWER_IP_ADDR    {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x00, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0x00}
#define TRAFFIC_MAX_POWER_INVERT_IP_ADDR    {0x00, 0xff, 0xee, 0xdd, 0xcc, 0xbb, 0xaa, 0x00, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x00}
...
bcm56780_a0_l3_strength_profile_add(int unit)
{
   ...
   SHR_IF_ERR_EXIT
        (bcmlt_entry_allocate(unit, "L3_IPV6_UNICAST_DEFIP_64_STRENGTH_PROFILE", &entry));
                                     ...
                                     int ecmp_base_ptr, int ecmp_grp_cnt, bool defip_enabled)
{
    ...
    const char *tcam_banks_str[4] = {"IFTA80_T2T_01_BANK0", "IFTA80_T2T_01_BANK1", "IFTA80_T2T_01_BANK2",
                                      "IFTA80_T2T_01_BANK3"};
/*   const char *tcam_banks_str1[4] = {"IFTA80_T2T_01_BANK4", "IFTA80_T2T_01_BANK5", "IFTA80_T2T_01_BANK6",
                                      "IFTA80_T2T_01_BANK7"}; */
...
trident4_power_test_setup_ipep_wrap(int unit, void *bp)
{
    ...
    int vlan_id, port, j, port_cnt, mport[40], prof_num, prev_prof_num;
    shr_mac_t mac_addr;
    ...
    shr_ip6_t ip_addr;
    int ip_addr_new,k, speed_groups;
  ...
  if (tra_param->enable_mirror == TRUE) {
     port_cnt=0;
     speed_groups = tra_param->speed_groups;
      prof_num=0;
      if (speed_groups == 0) {
        port_cnt = 0;
        BCMDRD_PBMP_ITER(tra_param->pbmp_all, port)  {
        ...
        for ( prof=0;prof<prof_num; prof++) {
          ...
          for ( m=0; m<8; m++) {
            ...
            mc_cos[mirror_num]    = m;
            imirror   =0;
            /*cli_out ("bcm56780_a0_mirror_session_add mirror_port %d mirror_enable %d mirror_index %d mirror_num %d \n",mirror_port[mirror_num],mirror_enable,mirror_index[mirror_num],mirror_num);*/
            SHR_IF_ERR_EXIT
              (bcm56780_a0_mirror_session_add(unit, mirror_port, mirror_enable, imirror,
                                              mirror_index,mc_cos));
            mirror_value = mirror_value + 9;
            if (mirror_value>11) mirror_value = mirror_value - 12;
            port_num ++;
        ...
        for ( prof=0; prof<prof_num; prof++) {
          ...
          for ( m=0; m<8; m++) {
            ...
            mc_cos[mirror_num]         = m;
            imirror   =0;
            /*cli_out ("bcm56780_a0_mirror_control_add mport[p] %d mirror_enable %d mirror_index %d mirror_num %d \n",mport[p],mirror_enable,mirror_index[mirror_num],mirror_num);*/
            SHR_IF_ERR_EXIT
        ...
        for ( i=1; i<= speed_groups; i++){
          cli_out("mirror config for speed group %d \n",i);
          port_cnt = 0;
          if (i==1) {
            BCMDRD_PBMP_ITER(tra_param->mirror_pbmp1, port)  {
              /*cli_out(" bcm56780_a0 Mirror port 1 %d port_cnt %d\n",port, port_cnt);*/
              mport[port_cnt] = port;
              port_cnt++;
            }
          }
          else if (i==2) {
            BCMDRD_PBMP_ITER(tra_param->mirror_pbmp2, port)  {
              /*cli_out(" bcm56780_a0 Mirror port 2 %d port_cnt %d\n",port, port_cnt);*/
              mport[port_cnt] = port;
              port_cnt++;
            }
          }
          else if (i==3) {
            BCMDRD_PBMP_ITER(tra_param->mirror_pbmp3, port)  {
              /*cli_out(" bcm56780_a0 Mirror port 3 %d port_cnt %d\n",port, port_cnt);*/
              mport[port_cnt] = port;
              port_cnt++;
            }
          }
          else if (i==4) {
            BCMDRD_PBMP_ITER(tra_param->mirror_pbmp4, port)  {
              /*cli_out(" bcm56780_a0 Mirror port 4 %d port_cnt %d\n",port, port_cnt);*/
              mport[port_cnt] = port;
              port_cnt++;
            }
          }
          prev_prof_num = prof_num;
          prof_num = ((port_cnt-1)%8 == 0)? ((port_cnt-1)/8): (port_cnt-1)/8+1;
          prof_num = prev_prof_num + prof_num;
          mirror_num=0;
          mirror_value=0;
          port_num = port_cnt/2;
          for ( prof=prev_prof_num;prof<prof_num; prof++) {
            mirror_value = prof;
            for ( m=0; m<8; m++) {
              mirror_num = m;
              mirror_index[mirror_num]= mirror_value;
...
bcm56780_a0_power_txrx(int unit, void *bp)
{
    ...
    if (tra_param->need_check_rx == TRUE) {
        /* coverity[dont_call] */
        SHR_IF_ERR_EXIT
            (bcma_testutil_packet_dev_simple_init
