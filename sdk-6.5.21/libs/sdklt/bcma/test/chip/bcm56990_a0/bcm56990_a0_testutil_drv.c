...
static bcmdrd_sid_t bcm56990_a0_tr530_skip_pt_list[] = {
...
bcm56990_a0_pt_skip(int unit, bcmdrd_sid_t sid, bcma_testutil_pt_type_t type)
{
    ...
    case BCMA_TESTUTIL_PT_MEM_TR_530:
        skip_pt = bcm56990_a0_tr530_skip_pt_list;
        count = COUNTOF(bcm56990_a0_tr530_skip_pt_list);
        break;
...
static int
bcm56990_a0_mem_mbist_preconfig(int unit)
{
...
static int bcm56990_a0_pktdma_counter_get(int unit, int chan_id,
                                          bool is_tx, uint32_t *ret_count)
{
    int cmc = chan_id / CMC_CHAN_NUM_PER_CMC;
    int idx = chan_id % CMC_CHAN_NUM_PER_CMC;

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(ret_count, SHR_E_PARAM);
    ...
    if (is_tx == true) {
        CMIC_CMC_PKTDMA_PKT_COUNT_TXPKTr_t cmc_tx_count;
        READ_CMIC_CMC_PKTDMA_PKT_COUNT_TXPKTr(unit, cmc, idx, &cmc_tx_count);
        *ret_count = CMIC_CMC_PKTDMA_PKT_COUNT_TXPKTr_COUNTf_GET(cmc_tx_count);
    } else {
        CMIC_CMC_PKTDMA_PKT_COUNT_RXPKTr_t cmc_rx_count;
        READ_CMIC_CMC_PKTDMA_PKT_COUNT_RXPKTr(unit, cmc, idx, &cmc_rx_count);
        *ret_count = CMIC_CMC_PKTDMA_PKT_COUNT_RXPKTr_COUNTf_GET(cmc_rx_count);
    }
...
bcm56990_a0_testutil_drv_attach(int unit)
{
    ...
    testutil_drv->mem_mbist_preconfig =bcm56990_a0_mem_mbist_preconfig;
    ...
    testutil_drv->pktdma_counter_get = bcm56990_a0_pktdma_counter_get;
