...
#define SUFFIX_LIMIT 5
    ...
    /* we just keep number of decimals as 1 at the remainder */
    if (value < 1000 || need_suffix == false) {
        /*
         *  ex. numer = 87213, denom = 100
         *      ==> remainder = 1
         */
        remainder = ((numer * 10) / denom) % 10;
    } else {
        while (value >= 1000) {
            /*
             * ex. value = 87123
             *      ==> suffix_idx++, value = 87, remainder = 1
             */
            suffix_idx++;
            remainder = (value / 100) % 10;
            value = value / 1000;
        }
    }
    ...
    cli_out("%3"PRIu64".%01"PRIu64"%s", value, remainder, suffix[suffix_idx]);
