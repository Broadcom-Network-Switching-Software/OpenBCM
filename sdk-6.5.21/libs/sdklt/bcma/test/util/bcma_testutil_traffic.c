                                              ...
                                              bool *ret_result_pass)
{
    ...
    *ret_result_pass = true;
    ...
    while (entry_ptr != NULL) {
        ...
        time = sal_time_usecs();
        if (time > entry_ptr->time_pre) {
            diff = (int)(time - entry_ptr->time_pre);
        } else {
            diff = (int)(SAL_USECS_TIMESTAMP_ROLLOVER
                            - entry_ptr->time_pre + time);
        }
        entry_ptr->time_pre = time;

        r_rate = 8 * (rbyt_diff + (rpkt_diff * INTER_PKT_GAP))
                 * SECOND_USEC / diff;
        t_rate = 8 * (tbyt_diff + (tpkt_diff * INTER_PKT_GAP))
                 * SECOND_USEC / diff;
                                               ...
                                               bool *ret_result_pass)
{
    ...
    *ret_result_pass = true;
    ...
    while (queue_bw_ptr != NULL) {
        ...
        time = sal_time_usecs();
        if (time > queue_bw_ptr->time_pre) {
            diff = (int)(time - queue_bw_ptr->time_pre);
        } else {
            diff = (int)(SAL_USECS_TIMESTAMP_ROLLOVER
