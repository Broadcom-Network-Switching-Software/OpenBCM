                           ...
                           const uint32_t *mask, int wsize)
{
    ...
    for (i = 0; i < wsize; i++) {
        uint32_t word0 = data0[i];
        uint32_t word1 = data1[i];
        uint32_t wordm = mask[i];
        if (((word0 ^ word1) & wordm) != 0) {
            cli_out("compare word %"PRId32" failed : 0x%08"PRIx32", 0x%08"PRIx32", mask: 0x%08"PRIx32"\n",
                    i, word0, word1, wordm);
            return -1;
        }
    }
