                                  ...
                                  bool intr_mode)
{
    ...
    SHR_IF_ERR_EXIT
        (drv->mem_range_write(unit, sid, index_start, index_end, tbl_inst,
                              buf, intr_mode));
                                 ...
                                 bool intr_mode)
{
    ...
    SHR_IF_ERR_EXIT
        (drv->mem_range_read(unit, sid, index_start, index_end, tbl_inst,
                             buf, intr_mode));
...
int bcma_testutil_drv_flex_pipe_macsec_config_set (
         int unit , int port , int vid , int pid , bcmdrd_pbmp_t *ms_bmp)
{
    ...
    if (!drv->flex_pipe_macsec_config_set ) {
        SHR_VERBOSE_EXIT(SHR_E_UNAVAIL);
    }

    drv->flex_pipe_macsec_config_set
                ...
                int unit,int port , int pid , int vid)
{
    ...
    if (!drv->flex_pipe_macsec_config_delete ) {
        SHR_VERBOSE_EXIT(SHR_E_UNAVAIL);
    }

    drv->flex_pipe_macsec_config_delete
                                (unit , port , pid , vid);
                                     ...
                                     bool is_tx, uint32_t *ret_count)
{
    ...
    SHR_NULL_CHECK(drv, SHR_E_UNIT);
    if (!drv->pktdma_counter_get) {
        SHR_VERBOSE_EXIT(SHR_E_UNAVAIL);
    }

    SHR_IF_ERR_EXIT
        (drv->pktdma_counter_get(unit, chan_id, is_tx, ret_count));
