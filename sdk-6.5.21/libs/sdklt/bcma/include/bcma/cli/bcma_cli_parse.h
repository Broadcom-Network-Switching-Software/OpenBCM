 ...
 *
 * This function will return a newly allocated string consisting of
 * characters extracted from the input string.
 *
 * Characters will be extracted based on an inclusion list and an
 * exclusion list. Characters that match the inclusion list will be
 * stored in the new string, while characters that match the exclusion
 * string will be dropped. If a character does not match either list,
 * the new string will be zero terminated, and the function returns.
 *
 * If \c endptr is not NULL, the address of the first character not in
 * either list will be stored in \c *endptr.
 *
 * The returned string must be freed by the caller via \c sal_free.
 *
 * \param [in] str String to be parsed.
 * \param [in] incl String consisting of characters to include.
 * \param [in] excl String consisting of characters to exclude.
 * \param [out] endptr Address of the first non-matching character.
 *
 * \return Allocated string of extracted characters or NULL on error.
 */
extern char *
