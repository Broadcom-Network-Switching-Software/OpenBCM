...
/*! Parameter for pktdev initialization */
typedef struct bcma_testutil_packet_dev_init_param_s {
    /*! number of tx channel */
    uint32_t tx_chan_num;

    /*! array of tx channel id */
    int tx_chan[NUM_QUE_MAX];

    /*! array of tx netif id */
    int tx_netif[NUM_QUE_MAX];

    /*! number of rx channel */
    uint32_t rx_chan_num;

    /*! array of rx channel id */
    int rx_chan[NUM_QUE_MAX];

    /*! array of rx netif id */
    int rx_netif[NUM_QUE_MAX];

    /*! array of rx queue_bmp */
    SHR_BITDCL rx_queue_bmp[NUM_QUE_MAX][SHRi_BITDCLSIZE(MAX_CPU_COS)];
} bcma_testutil_packet_dev_init_param_t;
...
/*!
 * \brief Init packet device only with one pair of tx/rx channel
 *
 ...
 *
 * \param [in] unit Unit id
 * \param [in] pktdev_param The paramter to initialize packet device
 * \param [in] pkt_max_size Packet max size
 * \param [out] ret_pktdev_id Returned packet device id
 *
 * \return SHR_E_NONE on success and error code otherwise.
