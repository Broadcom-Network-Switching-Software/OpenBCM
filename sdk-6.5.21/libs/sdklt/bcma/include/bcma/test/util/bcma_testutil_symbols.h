...
/*!
 * \brief Iterate over all symbols for matching name string.
 *
 * Callback to function \c cb for symbols matching \c match_str name.
 *
 * \param [in] symbols Symbols information structure for device.
 * \param [in] match_str Sub-string for symbol matching.
 * \param [in] cb Callback function for each matched symbol.
 * \param [in] cb_data User data for callback function \c cb.
 *
 * \return Negative on errors, otherwise the number of symbols matched.
 */
extern int
