    ...
    for (memb_idx = 0;
        ...
        for (idx = 0; idx < rhg_entry->rh_entries_cnt; idx++) {
            if (rhg_entry->rh_entries_arr[idx] != BCMECMP_INVALID
                ...
                if (BCMECMP_GRP_RH_MEMB_ENTCNT_PTR(unit, ecmp_id)[memb_idx]
                    == lbound) {
                    break;
                }
            }
...
bcmecmp_grp_defrag_t_init(bcmecmp_grp_defrag_t *frag)
{
    if (frag) {
        ...
        frag->gsize = 0;
        frag->op_arg = NULL;
        frag->bucket = BCMECMP_INVALID;
        frag->first = BCMECMP_INVALID;
    }
    return;
}
                                    ...
                                    bcmecmp_lt_entry_t *lt_entry)
{
    ...
    ecmp_id = lt_entry->ecmp_id;
    if (lt_entry->lb_mode == BCMECMP_LB_MODE_RESILIENT) {
        new_max_paths = lt_entry->rh_entries_cnt;
        old_max_paths = BCMECMP_GRP_RH_ENTRIES_CNT(unit, ecmp_id);
    } else {
                               ...
                               uint32_t req_size)
{
    ...
    mpb = shr_pb_create();
    shr_pb_printf(mpb,
                  "\n\tECMP_ID\t bucket.first(s/d)\tmstart_idx(s/d)"
                  "\t SG.sz ==> DG.sz(Gap)\tgtype\tSID\n");
    shr_pb_printf(mpb,
                  "\n\t=======\t===============\t=====================\t====="
                  "\t===\n");
    ...
    for (i = 0; i < (int)itbm_blk_count; i++) {
        m_src.bucket = itbm_blk[i].bucket;
        m_src.first = itbm_blk[i].first_elem;
        ...
        for (j = 0; j < (int)(m_seq[i].mcount); j++) {
            ...
            m_dest.bucket = m_seq[i].bucket[j];
            m_dest.first = m_seq[i].first_elem[j];
            ...
            m_src.bucket = m_dest.bucket;
            m_src.first = m_dest.first;
                               ...
                               bcmecmp_lt_entry_t *lt_entry)
{
    ...
    uint32_t old_bucket = 0, old_first = 0;
    ...
    if (new_max_paths < old_max_paths) {
        ...
        if (lt_entry->lb_mode == BCMECMP_LB_MODE_RESILIENT) {
            BCMECMP_GRP_RH_ENTRIES_CNT(unit, ecmp_id) = new_max_paths;
            SHR_IF_ERR_EXIT
                (bcmecmp_group_rh_realloc(unit, ecmp_id, gtype,
                                          new_max_paths, false));
        } else {
            BCMECMP_GRP_MAX_PATHS(unit, ecmp_id) = new_max_paths;
        }
    ...
    if (new_max_paths > old_max_paths) {
        ...
        if (list_hdl) {
            uint32_t free_idx;
            rv = shr_itbm_list_block_alloc(list_hdl, new_max_paths,
                                           &bucket, &first);
            LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        if (list_hdl) {
            bcmecmp_itbm_bucket_first_get(unit, gtype, lt_entry->weighted_mode,
                                          old_mstart_idx, &old_bucket, &old_first);
        ...
        src_block.bucket = old_bucket;
        src_block.first = old_first;
        src_block.mstart_idx = old_mstart_idx;
        src_block.gsize = old_max_paths * idx_width;
        dst_block.bucket = bucket;
        dst_block.first = first;
...
static int
bcmecmp_rh_groups_info_cleanup(int unit)
{
    int ecmp_id, gstart_idx, gend_idx;
    bcmecmp_info_t *ecmp_info;
    bcmecmp_control_t *ecmp_ctrl_local;

    SHR_FUNC_ENTER(unit);

    ecmp_ctrl_local = bcmecmp_get_ecmp_control(unit);
    ecmp_info = &(ecmp_ctrl_local->ecmp_info);

    if (ecmp_info->rhg == NULL ||
        (BCMECMP_ATOMIC_TRANS(unit) && ecmp_info->rhg_bk == NULL)) {
        SHR_EXIT();
    }
    gstart_idx = 0;
    /* This end index is assigned value only for fix groups, not flex groups. */
    gend_idx = ecmp_info->grp_arr_sz;
    for (ecmp_id = gstart_idx; ecmp_id < gend_idx; ecmp_id++) {
        if (ecmp_info->rhg[ecmp_id]) {
            /*
             * Free memory allocated for storing per RH entry mapped member's
             * index information.
             */
            BCMECMP_FREE(ecmp_info->rhg[ecmp_id]->rh_mindex_arr);

            /*
             * Free memory allocated for storing RH load balanced group
             * entries.
             */
            BCMECMP_FREE(ecmp_info->rhg[ecmp_id]->rh_entries_arr);
             ...
             * count.
             */
            BCMECMP_FREE(ecmp_info->rhg[ecmp_id]->rh_memb_ecnt_arr);

            /* Free memory allocated for storing group's RH info. */
            BCMECMP_FREE(ecmp_info->rhg[ecmp_id]);
        }

        if (BCMECMP_ATOMIC_TRANS(unit) && ecmp_info->rhg_bk[ecmp_id]) {
            /*
             * Free memory allocated for storing per RH entry mapped member's
             * index information.
             */
            BCMECMP_FREE(ecmp_info->rhg_bk[ecmp_id]->rh_mindex_arr);

            /*
             * Free memory allocated for storing RH load balanced group
             * entries.
             */
            BCMECMP_FREE(ecmp_info->rhg_bk[ecmp_id]->rh_entries_arr);
             ...
             * count.
             */
            BCMECMP_FREE(ecmp_info->rhg_bk[ecmp_id]->rh_memb_ecnt_arr);

            /* Free memory allocated for storing group's RH info. */
            BCMECMP_FREE(ecmp_info->rhg_bk[ecmp_id]);
        }
    }
    ...
    if (!BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_RH_SIZE)) {
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                     ...
                     lt_entry->rh_entries_cnt,
                     lt_entry->rh_size_enc));
    } else if (lt_entry->lb_mode == BCMECMP_LB_MODE_RESILIENT &&
               lt_entry->rh_entries_cnt !=
                   BCMECMP_GRP_RH_ENTRIES_CNT(unit, ecmp_id)) {
        /*resize the ECMP group*/
        SHR_IF_ERR_EXIT(bcmecmp_group_max_paths_update(unit, lt_entry));
...
bcmecmp_groups_cleanup(int unit)
{
    ...
    ecmp_info = &(bcmecmp_get_ecmp_control(unit)->ecmp_info);
    if (ecmp_info->grp_arr_sz) {
        /* Handle xgs chips with non-zero grp_arr_sz. */
        SHR_IF_ERR_EXIT
            (bcmecmp_rh_groups_info_cleanup(unit));
        SHR_EXIT();
    }
    ...
    for (gtype = BCMECMP_GRP_TYPE_SINGLE;
        ...
        if (BCMECMP_FLEX_GRP_TYPE_IS_FLEX(gtype)) {
            bcmecmp_flex_rh_groups_cleanup(unit, gtype);
            continue;
        }
...
int
bcmecmp_group_rh_realloc(int unit,
                         bcmecmp_id_t ecmp_id,
                         bcmecmp_grp_type_t gtype,
                         ...
                         bool is_bake)
{
    ...
    SHR_NULL_CHECK(BCMECMP_GRP_RHG_PTR(unit, ecmp_id), SHR_E_INTERNAL);
    if (BCMECMP_ATOMIC_TRANS(unit) && is_bake) {
        SHR_NULL_CHECK(BCMECMP_GRP_RHG_BK_PTR(unit, ecmp_id), SHR_E_INTERNAL);
    }
    ...
    if (!is_bake) {
        ...
        BCMECMP_FREE(BCMECMP_GRP_RH_MINDEX_PTR(unit, ecmp_id));
    } else if (BCMECMP_ATOMIC_TRANS(unit)) {
        BCMECMP_FREE(ecmp_info->rhg_bk[ecmp_id]->rh_entries_arr);
        if ((bcmecmp_grp_is_overlay(gtype)&& ecmp_info->ovlay_memb_count)) {
            BCMECMP_FREE(ecmp_info->rhg_bk[ecmp_id]->rh_o_entries_arr);
            BCMECMP_FREE(ecmp_info->rhg_bk[ecmp_id]->rh_u_ecmp_nhop_arr);
    ...
    if ((bcmecmp_grp_is_overlay(gtype) && ecmp_info->ovlay_memb_count)) {
        ...
        if (!is_bake) {
            BCMECMP_ALLOC
                (ecmp_info->rhg[ecmp_id]->rh_o_entries_arr,
                 rh_entries_cnt * sizeof(int),
                 "bcmecmpRhGrpAddGrpRhOvlayEntArr");
            BCMECMP_ALLOC
                (ecmp_info->rhg[ecmp_id]->rh_u_ecmp_nhop_arr,
                 rh_entries_cnt * sizeof(bool),
                 "bcmecmpRhGrpAddGrpRhUnlayEntArr");
        } else if (BCMECMP_ATOMIC_TRANS(unit)) {
            BCMECMP_ALLOC
                (ecmp_info->rhg_bk[ecmp_id]->rh_o_entries_arr,
    ...
    /* RH-ECMP members entries usage count array has the fix size. No change. */
