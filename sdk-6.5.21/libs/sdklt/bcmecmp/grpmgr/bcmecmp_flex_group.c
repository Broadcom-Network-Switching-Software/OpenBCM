...
int
bcmecmp_flex_check_member_pool_resource(int unit, const bcmecmp_grp_type_t gtype,
                         uint32_t req_size)
{
    ...
    if (SHR_E_NONE == rv) {
            rv = shr_itbm_list_block_free(BCMECMP_TBL_ITBM_LIST
     ...
     * table.
     */
    rv = shr_itbm_list_defrag_start(BCMECMP_TBL_ITBM_LIST(
                      BCMECMP_TBL_PTR(unit, member, gtype)),
    ...
    rv = shr_itbm_list_defrag_end(BCMECMP_TBL_ITBM_LIST(
                                    ...
                                    uint32_t req_size, int mstart_idx)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (shr_itbm_list_defrag_end(BCMECMP_TBL_ITBM_LIST(
...
bcmecmp_flex_mbr_tbl_max(int unit, bcmecmp_grp_type_t gtype) {
    ...
    if (gtype == BCMECMP_GRP_TYPE_LEVEL0) {
        ...
        if (BCMECMP_TBL_DRD_SID(unit, member2, BCMECMP_GRP_TYPE_LEVEL0) &&
            (BCMECMP_TBL_DRD_SID(unit, member2, BCMECMP_GRP_TYPE_LEVEL0) !=
             INVALIDm)) {
        ...
        if (BCMECMP_TBL_DRD_SID(unit, member2, BCMECMP_GRP_TYPE_LEVEL1) &&
            (BCMECMP_TBL_DRD_SID(unit, member2, BCMECMP_GRP_TYPE_LEVEL1) !=
             INVALIDm)) {
            max_idx += BCMECMP_TBL_IDX_MAX(unit, member2,
                          ...
                          bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    /*
     * Use stored group rh_random_seed value for update operation if it has not
     * been supplied.
     */
    if (!BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_RH_RANDOM_SEED)) {
 ...
 * group ecmp info for both the group and
 * group backup data.
 *
 * \param [in] unit     Unit number.
 * \param [in] gtype    Group type.
 ...
 */
void
bcmecmp_flex_rh_groups_cleanup(
                       int unit,
                       bcmecmp_grp_type_t gtype)
{
    bcmecmp_id_t ecmp_id; /* Group identifier. */
    bcmecmp_id_t sw_ecmp_id = 0;/* DB Group identifier. */
    uint32_t     i = 0;
    ...
    if ((ecmp_info == NULL ) ||
        (ecmp_info->rhg_flex == NULL) ||
        ((BCMECMP_ATOMIC_TRANS(unit) && ecmp_info->rhg_flex_bk == NULL))) {
        return;
    }
    ...
    if BCMECMP_LT_FIELD_ATTRS_VOID_PTR(unit, group, gtype) {
        for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID(unit, group, gtype);
             ecmp_id <= BCMECMP_LT_MAX_ECMP_ID(unit, group, gtype);
             ecmp_id++) {

            if (gtype == BCMECMP_GRP_TYPE_LEVEL1_WEIGHTED) {
                sw_ecmp_id = ecmp_id + BCMECMP_TBL_SIZE(
                                                    unit,
                                                    group,
                                                    BCMECMP_GRP_TYPE_LEVEL0);
            } else if (gtype == BCMECMP_GRP_TYPE_LEVEL0_WEIGHTED) {
            ...
            if (BCMECMP_FLEX_GRP_RHG_PTR(unit, sw_ecmp_id)) {
                for (i = 0; i < BCMECMP_MEMBER_COUNT; ++i) {
                    if (BCMECMP_FLEX_GRP_RH_ENTRIES_PTR(
                                         unit,
                                         sw_ecmp_id,
                                         i)) {
                        BCMECMP_FREE(BCMECMP_FLEX_GRP_RH_ENTRIES_PTR(
                                         unit, \
                                         sw_ecmp_id,
                                         i));
                    }
                }
                if (BCMECMP_FLEX_GRP_RH_MINDEX_PTR(unit, sw_ecmp_id)) {
                    BCMECMP_FREE(BCMECMP_FLEX_GRP_RH_MINDEX_PTR(
                                         unit,
                                         sw_ecmp_id));
                }
                if (BCMECMP_FLEX_GRP_RH_MEMB_ENTCNT_PTR(unit, sw_ecmp_id)) {
                    BCMECMP_FREE(BCMECMP_FLEX_GRP_RH_MEMB_ENTCNT_PTR(
                                          unit,
                                          sw_ecmp_id));
                }
                BCMECMP_FREE(BCMECMP_FLEX_GRP_VOID_RHG_PTR(unit, sw_ecmp_id));
            }
            if (BCMECMP_ATOMIC_TRANS(unit)
                && BCMECMP_FLEX_GRP_RHG_BK_PTR(unit, sw_ecmp_id)) {
                for (i = 0; i < BCMECMP_MEMBER_COUNT; ++i) {
                    if (BCMECMP_FLEX_GRP_RH_ENTRIES_BK_PTR(
                                         unit,
                                         sw_ecmp_id, i)) {
                ...
                if (BCMECMP_FLEX_GRP_RH_MINDEX_BK_PTR(unit, sw_ecmp_id)) {
                    BCMECMP_FREE(BCMECMP_FLEX_GRP_RH_MINDEX_BK_PTR(
                                         unit,
                                         sw_ecmp_id));
                }
                if (BCMECMP_FLEX_GRP_RH_MEMB_ENTCNT_BK_PTR(unit, sw_ecmp_id)) {
                    BCMECMP_FREE(BCMECMP_FLEX_GRP_RH_MEMB_ENTCNT_BK_PTR(
                                         unit,
                                         sw_ecmp_id));
                }
                BCMECMP_FREE(BCMECMP_FLEX_GRP_VOID_RHG_BK_PTR(
                                         unit,
                                         sw_ecmp_id));
            }
