...
typedef bcmcth_ecmp_info_drv_t *(*bcmcth_ecmp_info_drv_get_f)(int unit);

#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    { _bd##_vu##_va##_cth_ecmp_src_l3_group_0_info_drv_get},
static struct {
    bcmcth_ecmp_info_drv_get_f drv_get;
} bcmecmp_src_l3_group_0_info_drv[] = {
    { 0 },
...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    { _bd##_vu##_va##_cth_ecmp_src_l3_group_1_info_drv_get},
static struct {
    bcmcth_ecmp_info_drv_get_f drv_get;
} bcmecmp_src_l3_group_1_info_drv[] = {
    { 0 },
#include <bcmlrd/chip/bcmlrd_variant.h>
    { 0 }
};
...
/*!
 * \brief Retrieve variant driver for ECMP src L3 group 0 info table.
 *
 * \param [in] unit Unit number.
 * \param [out] drv Pointer to the info data
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
static int
bcmecmp_src_l3_group_0_info_drv_get(int unit,
                                    bcmcth_ecmp_info_drv_t **drv)
{
    bcmlrd_variant_t variant;
    SHR_FUNC_ENTER(unit);

    variant = bcmlrd_variant_get(unit);
    *drv = bcmecmp_src_l3_group_0_info_drv[variant].drv_get(unit);
    if (*drv == NULL) {
        SHR_ERR_EXIT (SHR_E_INTERNAL);
    }
exit:
...
/*!
 * \brief Retrieve variant driver for ECMP src L3 group 1 info table.
 *
 * \param [in] unit Unit number.
 * \param [out] drv Pointer to the info data
 *
 ...
 */
static int
bcmecmp_src_l3_group_1_info_drv_get(int unit,
                                    bcmcth_ecmp_info_drv_t **drv)
{
    bcmlrd_variant_t variant;
    SHR_FUNC_ENTER(unit);
    ...
    variant = bcmlrd_variant_get(unit);
    *drv = bcmecmp_src_l3_group_1_info_drv[variant].drv_get(unit);
    if (*drv == NULL) {
        SHR_ERR_EXIT (SHR_E_INTERNAL);
    }
exit:
                           ...
                           bool warm)
{
    ...
    if (ids->ecmp_src_l3_group_0_info_id != (uint32_t) BCMECMP_INVALID) {
        SHR_IF_ERR_EXIT
            (bcmecmp_src_l3_group_0_info_drv_get(unit, &drv));
        if (drv && drv->get_data) {
            SHR_IF_ERR_EXIT
    ...
    if (ids->ecmp_src_l3_group_1_info_id != (uint32_t) BCMECMP_INVALID) {
        SHR_IF_ERR_EXIT
            (bcmecmp_src_l3_group_1_info_drv_get(unit, &drv));
        if (drv && drv->get_data) {
            SHR_IF_ERR_EXIT
                (drv->get_data(unit, &data));
            SHR_IF_ERR_EXIT
                                    ...
                                    const bcmecmp_drv_var_t *drv_var)
{
    ...
    /* Member2 isnot valid. Set all member2 size to 0. */
    BCMECMP_TBL_ENT_SIZE
        (unit, member2, BCMECMP_GRP_TYPE_LEVEL0) = 0;
    BCMECMP_TBL_ENT_SIZE
        (unit, member2, BCMECMP_GRP_TYPE_LEVEL1) = 0;
    BCMECMP_TBL_ENT_SIZE
        (unit, member2, BCMECMP_GRP_TYPE_LEVEL0_WEIGHTED) = 0;
    BCMECMP_TBL_ENT_SIZE
        (unit, member2, BCMECMP_GRP_TYPE_LEVEL1_WEIGHTED) = 0;
                             ...
                             bool warm)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56780_a0_ecmp_sids_init(unit, drv_var, warm));
...
bcm56780_a0_ecmp_info_init(int unit, bool warm)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56780_a0_ecmp_tables_init(unit, drv_var, warm));
...
bcm56780_a0_grp_member_fields_count_get(int unit, void *lt_entry)
{
    ...
    if (lt_ent->glt_sid == ids->ecmp_wlevel1_ltid) {
        /* include WEIGHTED_NHOP_INDEX & WEIGHT fields */
        return (drv_var->member1_info->flds_count + 2);
    } else {
