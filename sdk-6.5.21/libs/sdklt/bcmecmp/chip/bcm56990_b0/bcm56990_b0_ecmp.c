...
bcm56990_b0_ecmp_db_init(int unit, bool warm)
{
    ...
    switch (BCMECMP_MODE(unit)) {
        case BCMECMP_MODE_HIER:
            ...
            (BCMECMP_TBL_PTR
                (unit, group,
                 BCMECMP_GRP_TYPE_UNDERLAY))->flags = tbl_ptr->flags;
            break;
        ...
        switch (BCMECMP_MODE(unit)) {
            case BCMECMP_MODE_HIER:
                ...
                if (tbl_ptr->flags)  {
                    ...
                    (BCMECMP_TBL_PTR
                        (unit, group,
                         BCMECMP_GRP_TYPE_UNDERLAY))->flags = NULL;
                }
                break;
                                ...
                                bool rh_mode)
{
    ...
    if (!BCMECMP_GRP_IS_WEIGHTED(gtype)) {
        ...
        } else if (bcmecmp_grp_is_overlay(gtype)) {
...
bcm56990_b0_ecmp_grp_get(int unit, void *info)
{
    ...
    if (!BCMECMP_LT_FIELD_GET(lt_ent->fbmp, BCMECMP_LT_FIELD_ECMP_ID)) {
        /* This is for utecmp hgrps_dma case, which has no parsed IMM data. */
    ...
    if (lt_ent->num_paths == 0) {
        ...
        if (bcmecmp_grp_is_overlay(lt_ent->grp_type)) {
            sal_memset(lt_ent->uecmp_id, 0,
                       BCMECMP_LT_MAX_PATHS(unit, group, lt_ent->grp_type) *
                       sizeof(bcmecmp_id_t));
            sal_memset(lt_ent->u_ecmp_nhop, 0,
                       BCMECMP_LT_MAX_PATHS(unit, group, lt_ent->grp_type) *
                       sizeof(uint8_t));
            sal_memset(lt_ent->o_nhop_id, 0,
                       BCMECMP_LT_MAX_PATHS(unit, group, lt_ent->grp_type) *
                       sizeof(bcmecmp_nhop_id_t));
        }
        SHR_EXIT();
                                       ...
                                       void *ltentry)
{
    ...
    switch (fid) {
        ...
        case ECMP_OVERLAYt_RH_ECMP_NHOP_UNDERLAYf:
            ...
            lt_entry->rh_u_ecmp_nhop[idx] = fval;

            /* Increment parsed NHOP_IDs count. */
            lt_entry->rh_u_ecmp_nhop_count += 1;
            break;
        ...
        case ECMP_OVERLAYt_RH_ECMP_UNDERLAY_IDf:
            ...
            lt_entry->rh_uecmp_id[idx] = (int) fval;

            /* Increment parsed ECMP_UNDERLAY_IDs count. */
            lt_entry->rh_uecmp_ids_count += 1;
            break;
        ...
        case ECMP_OVERLAYt_RH_OVERLAY_NHOP_IDf:
            ...
            lt_entry->rh_o_nhop_id[idx] = (int) fval;

            /* Increment parsed NHOP_IDs count. */
            lt_entry->rh_o_nhop_ids_count += 1;
            break;
