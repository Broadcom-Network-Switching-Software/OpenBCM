...
bcm56990_a0_ecmp_itbm_lists_cleanup(int unit)
{
    ...
    rv = shr_itbm_list_destroy(list_hdl, 0, 1);
    SHR_IF_ERR_EXIT(rv);
...
bcm56990_a0_ecmp_db_init(int unit, bool warm)
{
    ...
    switch (BCMECMP_MODE(unit)) {
        case BCMECMP_MODE_FULL:
            ...
            BCMECMP_ALLOC(tbl_ptr->flags, tbl_size, "bcmecmpTblFlags");
            break;
                                     ...
                                     bool rh_mode)
{
    ...
    if (!ent_num) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "return because ent_num is zero.\n")));
        SHR_EXIT();
    }
    dma_alloc_sz = (ent_size * sizeof(uint32_t) * ent_num);
        ...
        while (ent_offset < NHOPS_PER_MEMB_ENTRY && max_paths > 0) {
            SHR_IF_ERR_EXIT
                (bcmdrd_pt_field_get(unit,
                                     ...
                                     memb_flds->ecmp_nhop_id[ent_offset],
                                     &val));
            SHR_NULL_CHECK(nhop_arr, SHR_E_INTERNAL);
            nhop_arr[nhop_arr_idx] = (bcmecmp_nhop_id_t)val;
                                ...
                                bool rh_mode)
{
    ...
    if (lt_ent->lb_mode == BCMECMP_LB_MODE_RESILIENT) {
        lt_ent->rh_size_enc = num_paths[0] ? num_paths[0] : BCMECMP_RH_SIZE_64;
        lt_ent->rh_entries_cnt = num_paths[0] ? (1 << num_paths[0]) : 0;
         ...
         */
        lt_ent->num_paths = lt_ent->num_paths ? (num_paths[0] + 1) : 0;
    }
...
bcm56990_a0_ecmp_grp_get(int unit, void *info)
{
    ...
    if (!BCMECMP_LT_FIELD_GET(lt_ent->fbmp, BCMECMP_LT_FIELD_ECMP_ID)) {
        /* This is for utecmp grps_dma case, which has no parsed IMM data. */
        lt_ent->num_paths = BCMECMP_GRP_NUM_PATHS(unit, lt_ent->ecmp_id);
    }
    ...
    if (lt_ent->num_paths == 0) {
        sal_memset(lt_ent->nhop_id, 0,
                   BCMECMP_LT_MAX_PATHS(unit, group, lt_ent->grp_type) *
...
bcm56990_a0_ecmp_rh_grp_get(int unit, void *info)
{
    ...
    if (lt_ent->rh_entries_arr == NULL) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "RH group members does not support cache-bypass"
                                "lookup. Skip reading member table.\n")));
        SHR_EXIT();
    }
