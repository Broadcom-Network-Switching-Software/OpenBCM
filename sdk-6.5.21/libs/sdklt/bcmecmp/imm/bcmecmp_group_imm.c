            ...
            if (in_use_bk &&
                ...
                SHR_IF_ERR_EXIT
                    (bcmecmp_group_rh_realloc
                        (unit, ecmp_id, BCMECMP_GRP_TYPE_SINGLE,
                         BCMECMP_GRP_RH_ENTRIES_CNT(unit, ecmp_id),
                         true));
            }
 ...
 */
static int
bcmecmp_group_lt_entry_parse(int unit,
                             bcmltd_sid_t sid,
                             bcmecmp_lt_entry_t *lt_entry,
                             bcmltd_fields_t *out)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        switch (fid) {
            ...
            case ECMPt_RH_NHOP_IDf:
                ...
                for (idx = 0;
                     idx < BCMECMP_LT_MAX_RH_NPATHS(unit, group, lt_entry->grp_type);
                     idx++) {
                    out->field[out->count]->id = fid;
                     ...
                     bcmltd_fields_t *out)
{
    ...
    if (in != NULL) {
        for (i = 0; i < in->count; i++) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmecmp_group_lt_fields_fill(unit, in->field[i], lt_entry));
        }
    }
    ...
    for (i = 0; i < out->count; i++) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmecmp_group_lt_fields_fill(unit, out->field[i], lt_entry));
    }
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmecmp_group_lt_entry_parse(unit, sid, lt_entry, out));
...
static bcmimm_lt_cb_t event_hdl = {
    ...
    .op_lookup = bcmecmp_group_lookup
};
