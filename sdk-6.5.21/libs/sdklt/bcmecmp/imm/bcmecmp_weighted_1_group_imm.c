        ...
        case BCMIMM_ENTRY_UPDATE:
             if (BCMECMP_LT_FIELD_GET
                ...
                if (lt_entry->rh_mode !=
                    ...
                    /* Release the ITBM resource for Member entries. */
                    SHR_IF_ERR_EXIT
                       ...
                       SHR_ITBM_INVALID,
                       BCMECMP_FLEX_GRP_MEMB_TBL_START_IDX(unit, lt_entry->ecmp_id)));
                    space_available = bcmecmp_flex_check_member_pool_resource(unit,
                                      lt_entry->grp_type, lt_entry->max_paths);

                    /* Restore ITBM resources for Member entries. */
                    rv = shr_itbm_list_block_alloc_id(BCMECMP_TBL_ITBM_LIST(
                        BCMECMP_TBL_PTR(unit, member, lt_entry->grp_type)),
                        BCMECMP_FLEX_GRP_MAX_PATHS(unit, lt_entry->ecmp_id),
                        SHR_ITBM_INVALID, BCMECMP_FLEX_GRP_MEMB_TBL_START_IDX(
