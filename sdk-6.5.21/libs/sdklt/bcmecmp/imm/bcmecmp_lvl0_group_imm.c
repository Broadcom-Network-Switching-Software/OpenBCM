/*! \file bcmecmp_lvl0_group_imm.c
 *
 * BCMECMP ECMP_LEVEL0t IMM handler.
 */
/*
 ...
 */
/*!
 * \brief ECMP_LEVEL0 LT entry fields parse routine.
 *
 ...
 */
static int
bcmecmp_lvl0_group_lt_fields_parse(int unit,
                                   ...
                                   bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "bcmecmp_lvl0_group_lt_fields_parse:LT_SID=%d.\n"),
                sid));
    ...
    while (gen_field) {
        SHR_IF_ERR_EXIT
          (BCMECMP_FNCALL_EXEC(
               unit, grp_ol_lt_fields_fill)(unit, gen_field, lt_entry));
 ...
 */
static int
bcmecmp_lvl0_group_validate(int unit,
                               bcmltd_sid_t sid,
                               ...
                               void *context)
{
    ...
    bcmecmp_flex_lt_entry_t *lt_entry = NULL; /* ECMP_LEVEL0 LT entry data.*/
    bcmecmp_flex_lt_entry_t *cur_lt_entry = NULL; /* current LT entry.*/
    SHR_FUNC_ENTER(unit);

    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lvl0_group_validate:SID=%d.\n"),
              sid));
    BCMECMP_INIT(unit);
    BCMECMP_LOCK(unit);
    ...
    SHR_IF_ERR_EXIT
        (bcmecmp_lvl0_group_lt_fields_parse(unit, key, sid, lt_entry));

    if (((event == BCMIMM_ENTRY_UPDATE) ||
        (event == BCMIMM_ENTRY_INSERT)) && (data != NULL)) {
        SHR_IF_ERR_EXIT
        (bcmecmp_lvl0_group_lt_fields_parse(unit, data, sid, lt_entry));
    }
    ...
    if (event == BCMIMM_ENTRY_UPDATE) {
        ...
        /* Parse the input fields and retrieve the field values. */
        SHR_IF_ERR_EXIT
            (bcmecmp_lvl0_group_lt_fields_parse(unit,
                                                 key,
                                                 sid,
                                                 cur_lt_entry));
        /* Get current fields */
 ...
 */
static int
bcmecmp_lvl0_group_stage(int unit,
                         bcmltd_sid_t sid,
                         ...
                         bcmltd_fields_t *output_fields)
{
    ...
    bcmecmp_flex_lt_entry_t *lt_entry = NULL; /*ECMP_LEVEL0 entry data. */
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lvl0_group_stage: LT_SID=%d.\n"),
              sid));
    BCMECMP_INIT(unit);
    BCMECMP_LOCK(unit);
    ...
    if (BCMECMP_TBL_ITBM_LIST(BCMECMP_TBL_PTR(unit, group,
        ...
        BCMECMP_FLEX_TBL_ENT_STAGED_SET
            (BCMECMP_TBL_PTR(unit, group, BCMECMP_GRP_TYPE_LEVEL0),
             lt_entry->ecmp_id);
...
/*!
 * \brief ECMP_LEVEL0 group imm commit call back
 *
 ...
 */
static int
bcmecmp_lvl0_group_commit(int unit,
                          bcmltd_sid_t sid,
                          uint32_t trans_id,
                          void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lvl0_group_commit: LT_SID=%d.\n"),
              sid));
    ...
    for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
         ...
         ecmp_id <= BCMECMP_LT_MAX_ECMP_ID
                        (unit, group, BCMECMP_GRP_TYPE_LEVEL0);
         ecmp_id++ ) {
        if (!BCMECMP_FLEX_TBL_ENT_STAGED_GET(BCMECMP_TBL_PTR(
                                              unit,
                                              group,
                                              BCMECMP_GRP_TYPE_LEVEL0),
                                        ecmp_id)
        ...
        BCMECMP_FLEX_TBL_ENT_STAGED_CLEAR(BCMECMP_TBL_PTR(unit, group,
                                                     BCMECMP_GRP_TYPE_LEVEL0),
                                     ecmp_id);
    }
    ...
    for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                     ...
                     BCMECMP_FLEX_TBL_BK_ENT_STAGED_GET(BCMECMP_TBL_PTR(unit,
                                                     group,
                                                     BCMECMP_GRP_TYPE_LEVEL0),
                                                     ecmp_id),
                     sid));
...
/*!
 * \brief ECMP_LEVEL0 transaction abort
 *
 * Rollback the ECMP_LEVEL0 entry from backup state
 ...
 */
static void
bcmecmp_lvl0_group_abort(int unit,
                         bcmltd_sid_t sid,
                         uint32_t trans_id,
                         void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lvl0_group_abort: LT_SID=%d.\n"),
              sid));
    ...
    for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
        ...
        if (BCMECMP_FLEX_GRP_ITBM_USE(unit, ecmp_id))  {
            SHR_IF_ERR_EXIT
                (shr_itbm_list_block_free(BCMECMP_TBL_ITBM_LIST
                    (BCMECMP_TBL_PTR(unit, member, BCMECMP_GRP_TYPE_LEVEL0)),
                    BCMECMP_FLEX_GRP_MAX_PATHS(unit, ecmp_id),
                    SHR_ITBM_INVALID,
                    BCMECMP_FLEX_GRP_MEMB_TBL_START_IDX(unit, ecmp_id)));
            SHR_IF_ERR_EXIT
     ...
     /*
      * Add all the defragmentation groups
      * itbm resources based on backup state.
      */
     for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
                        (unit, group, BCMECMP_GRP_TYPE_LEVEL0);
         ecmp_id <= BCMECMP_LT_MAX_ECMP_ID
...
/*!
 * \brief ECMP_LEVEL0 In-memory event callback structure.
 *
 * This structure contains callback functions that will be conresponding
 * to ECMP_LEVEL0 logical table entry commit stages.
 */
static bcmimm_lt_cb_t event_hdl = {

    /*! Validate function. */
    .validate = bcmecmp_lvl0_group_validate,

    /*! Extended staging function. */
    .op_stage = bcmecmp_lvl0_group_stage,

    /*! Commit function. */
    .commit = bcmecmp_lvl0_group_commit,

    /*! Abort function. */
    .abort = bcmecmp_lvl0_group_abort
};
 ...
 */
/*!
 * \brief ECMP_LEVEL0 IMM callback register
 *
 * regist ECMP_LEVEL0 LT callback in IMM module
 *
 * \param [in] unit This is device unit number.
...
bcmecmp_lvl0_group_imm_register(int unit, bcmlrd_sid_t sid)
{
    ...
    /*
     * To register callback for ECMP_LEVEL0t here.
     */
