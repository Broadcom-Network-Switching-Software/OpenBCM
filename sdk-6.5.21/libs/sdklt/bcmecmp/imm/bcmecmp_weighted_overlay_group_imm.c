/*! \file bcmecmp_weighted_overlay_group_imm.c
 *
 * BCMECMP ECMP_WEIGHTED_OVERLAYt IMM handler.
 */
/*
 ...
 */
static int
bcmecmp_weighted_overlay_group_lt_fields_parse(int unit,
                                       ...
                                       bcmecmp_lt_entry_t *lt_entry)
{
    ...
    while (gen_field) {
        SHR_IF_ERR_VERBOSE_EXIT
            (BCMECMP_FNCALL(unit, grp_ol_lt_fields_fill)(unit,
                                                         gen_field, lt_entry));
 ...
 */
static int
bcmecmp_weighted_overlay_group_validate(int unit,
                                bcmltd_sid_t sid,
                                ...
                                void *context)
{
     ...
     * resolution mode.
     */
    if (sid != BCMECMP_TBL_LTD_SID(unit, group, BCMECMP_GRP_TYPE_OVERLAY_WEIGHTED)) {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmecmp_weighted_overlay_group_lt_fields_parse
 ...
 */
static int
bcmecmp_weighted_overlay_group_stage(int unit,
                             bcmltd_sid_t sid,
                             ...
                             bcmltd_fields_t *output_fields)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_weighted_overlay_group_stage: LT_SID=%d.\n"),
              sid));
 ...
 */
static int
bcmecmp_weighted_overlay_group_commit(int unit,
                                      bcmltd_sid_t sid,
                                      uint32_t trans_id,
                                      void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_weighted_overlay_group_commit: LT_SID=%d.\n"),
              sid));
    ...
    hw_ent_attr_base = (bcmecmp_hw_entry_attr_t *)
                           (ecmp_info->group[BCMECMP_GRP_TYPE_OVERLAY_WEIGHTED].ent_arr);
    ...
    tbl_ptr = BCMECMP_TBL_PTR(unit, member, BCMECMP_GRP_TYPE_OVERLAY_WEIGHTED);
    SHR_NULL_CHECK(tbl_ptr, SHR_E_INTERNAL);

    hw_ent_attr_base = (bcmecmp_hw_entry_attr_t *)
                        (ecmp_info->group[BCMECMP_GRP_TYPE_OVERLAY_WEIGHTED].ent_bk_arr);
    for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
                        (unit, group, BCMECMP_GRP_TYPE_OVERLAY_WEIGHTED);
         ecmp_id <= BCMECMP_LT_MAX_ECMP_ID
                        (unit, group, BCMECMP_GRP_TYPE_OVERLAY_WEIGHTED);
 ...
 */
static void
bcmecmp_weighted_overlay_group_abort(int unit,
                                     bcmltd_sid_t sid,
                                     uint32_t trans_id,
                                     void *context)
{
    ...
    ecmp_info = &(bcmecmp_get_ecmp_control(unit)->ecmp_info);
    grp_list_hdl = ecmp_info->group[BCMECMP_GRP_TYPE_OVERLAY_WEIGHTED].list_hdl;
    grp_attr_base = (bcmecmp_grp_attr_t *)(ecmp_info->grp_arr);
    grp_bk_attr_base = (bcmecmp_grp_attr_t *)(ecmp_info->grp_bk_arr);
...
/*!
 * \brief Weighted overlay ECMP LT entry fields parse routine.
 *
 ...
 */
static int
bcmecmp_weighted_overlay_group_lt_entry_parse(int unit,
                                              bcmltd_sid_t sid,
                                              bcmecmp_lt_entry_t *lt_entry,
                                              bcmltd_fields_t *out)
{
    ...
    SHR_ALLOC(fid_list, table_sz, "bcmWeightOvlayEcmpFldList");
            ...
            case ECMP_WEIGHTED_OVERLAYt_ECMP_NHOP_UNDERLAYf:
                for (idx = 0;
                    ...
                    if (lt_entry->u_ecmp_nhop[idx] == (uint8_t)BCMECMP_INVALID) {
                        ...
                        out->field[out->count]->data = lt_entry->u_ecmp_nhop[idx];
                    }
                    out->field[out->count]->idx = idx;
                    out->count++;
                }
            ...
            case ECMP_WEIGHTED_OVERLAYt_ECMP_UNDERLAY_IDf:
                for (idx = 0;
                    ...
                    if (lt_entry->uecmp_id[idx] == BCMECMP_INVALID) {
                        out->field[out->count]->data = lt_fattr->min_uecmp_id;;
                    } else {
                        out->field[out->count]->data = lt_entry->uecmp_id[idx];
                    }
                    out->field[out->count]->idx = idx;
                    out->count++;
                }
            ...
            case ECMP_WEIGHTED_OVERLAYt_OVERLAY_NHOP_IDf:
                for (idx = 0;
                    ...
                    if (lt_entry->o_nhop_id[idx] == BCMECMP_INVALID) {
                        out->field[out->count]->data = lt_fattr->min_o_nhop_id;
                    } else {
                        out->field[out->count]->data = lt_entry->o_nhop_id[idx];
                    }
                    out->field[out->count]->idx = idx;
                    out->count++;
                }
...
/*!
 * \brief ECMP weighted overlay group imm lookup callback.
 *
 * \param [in]  unit            Unit number.
 ...
 */
static int
bcmecmp_weighted_overlay_group_lookup(int unit,
                                      bcmltd_sid_t sid,
                                      const bcmltd_op_arg_t *op_arg,
                                      void *context,
                                      ...
                                      bcmltd_fields_t *out)
{
    ...
    if (in != NULL) {
        for (i = 0; i < in->count; i++) {
            SHR_IF_ERR_VERBOSE_EXIT
                (BCMECMP_FNCALL_EXEC(unit, grp_ol_lt_fields_fill)(unit,
                                                                  in->field[i],
                                                                  lt_entry));
        }
    }
    ...
    if ((op_arg->attrib & BCMLT_ENT_ATTR_GET_FROM_HW)) {
        if (BCMECMP_FNCALL_EXEC(unit, grp_get)) {
            SHR_IF_ERR_EXIT_EXCEPT_IF
                (BCMECMP_FNCALL_EXEC(unit, grp_get)(unit, lt_entry),
                 SHR_E_UNAVAIL);
        }
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmecmp_weighted_overlay_group_lt_entry_parse
            (unit, sid, lt_entry, out));
...
static bcmimm_lt_cb_t event_hdl = {
    /*! Validate function. */
    .validate = bcmecmp_weighted_overlay_group_validate,

    /*! Extended staging function. */
    .op_stage = bcmecmp_weighted_overlay_group_stage,

    /*! Commit function. */
    .commit = bcmecmp_weighted_overlay_group_commit,

    /*! Abort function. */
    .abort = bcmecmp_weighted_overlay_group_abort,

    /*! Extended lookup function. */
    .op_lookup = bcmecmp_weighted_overlay_group_lookup
};
...
bcmecmp_weighted_overlay_group_imm_register(int unit, bcmlrd_sid_t sid)
{
    ...
    /*
     * To register callback for ECMP_WEIGHTED_OVERLAYt here.
     */
    ...
    if (SHR_SUCCESS(rv) && map) {
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "Registered IMM event handler for "
                                "ECMP SID:%u.\n"),
                     sid));
    }
