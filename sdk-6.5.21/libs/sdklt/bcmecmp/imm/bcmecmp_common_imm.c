                        ...
                        bcmecmp_lt_entry_t *lt_entry)
{
    ...
    if (lt_entry) {
        ...
        sal_memset(lt_entry->rh_u_ecmp_nhop, BCMECMP_INVALID,
                   sizeof(lt_entry->rh_u_ecmp_nhop));

        lt_entry->rh_u_ecmp_nhop_count = 0;
        ...
        sal_memset
            (lt_entry->rh_uecmp_id, BCMECMP_INVALID, sizeof(lt_entry->rh_uecmp_id));

        lt_entry->rh_uecmp_ids_count = 0;
        ...
        sal_memset
            (lt_entry->rh_o_nhop_id, BCMECMP_INVALID, sizeof(lt_entry->rh_o_nhop_id));

        lt_entry->rh_o_nhop_ids_count = 0;
                                 ...
                                 bcmecmp_lt_entry_t *lt_entry)
{
    ...
    uint32_t weight_mode_update;
    bcmecmp_grp_ltd_fields_t *lt_fids = NULL;
    ...
    lt_fids = (bcmecmp_grp_ltd_fields_t *)
                       ecmp_ctrl->ecmp_info.group[lt_entry->grp_type].ltd_fids;
    ...
    switch (opcode) {
        case BCMIMM_ENTRY_INSERT:
            ...
            if (ecmp_ctrl->ecmp_drv->weight_size_to_max_paths &&
                ...
                if (BCMECMP_LT_FIELD_GET
                    ...
                    SHR_IF_ERR_EXIT
                        (bcmlrd_field_value_to_symbol(unit, sid,
                                                      lt_fids->weighted_size_fid,
                                                      lt_entry->weighted_size,
                                                      &symbol));
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit,
                                            "%s cannot be set for replication"
                                            " weight mode.\n"),
                                 symbol));
                    SHR_ERR_EXIT(SHR_E_PARAM);
                }
        ...
        case BCMIMM_ENTRY_UPDATE:
            ...
            if (BCMECMP_LT_FIELD_GET(lt_entry->fbmp,
                                     BCMECMP_LT_FIELD_LB_MODE) ||
                BCMECMP_LT_FIELD_GET(lt_entry->fbmp,
            ...
            if (BCMECMP_LT_FIELD_GET(lt_entry->fbmp,
                ...
                weight_mode_update = lt_entry->weighted_mode;
            } else {
                weight_mode_update = BCMECMP_GRP_WEIGHTED_MODE
                                         (unit, lt_entry->ecmp_id);
            }
            ...
            if (BCMECMP_GRP_IS_WEIGHTED(gtype)
                ...
                && lt_entry->num_paths) {
                if (ecmp_ctrl->ecmp_drv->weight_size_to_max_paths) {
                    if (lb_mode_update != lt_fattr->min_weighted_size) {
                        if (lt_entry->num_paths != lt_entry->max_paths) {
                            LOG_VERBOSE(BSL_LOG_MODULE,
                                        (BSL_META_U(unit,
                                                    "weight_size=%u, "
                    ...
                    if (lb_mode_update != BCMECMP_WEIGHTED_SIZE_0_127) {
                        if (lt_entry->num_paths != lt_entry->max_paths) {
                            LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit,
                                            "NUM_PATHS=%u is invalid.\n"),
            ...
            if (ecmp_ctrl->ecmp_drv->weight_size_to_max_paths &&
                ...
                && BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_WEIGHTED_SIZE)
                && lt_entry->weighted_size == BCMECMP_WEIGHTED_SIZE_0_127
                && weight_mode_update != BCMECMP_WEIGHTED_MODE_MEMBER_WEIGHT) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                                        "WEIGHTED_SIZE_0_127 cannot be set for "
                                        "replication weight mode.\n")));
            ...
            if (ecmp_ctrl->ecmp_drv->weight_size_to_max_paths &&
                ...
                if (BCMECMP_LT_FIELD_GET
                    ...
                    && lb_mode_update == lt_fattr->min_weighted_size){
                    SHR_IF_ERR_EXIT
                        (bcmlrd_field_value_to_symbol(unit, sid,
                                                      lt_fids->weighted_size_fid,
                                                      lb_mode_update,
                    ...
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit,
                                            "Replication weight mode cannot"
                                            " be set with %s.\n"),
                                 symbol));
                    SHR_ERR_EXIT(SHR_E_PARAM);
                }
                ...
                && lb_mode_update == BCMECMP_WEIGHTED_SIZE_0_127) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                                        "Replication weight mode cannot be set"
                                        " with WEIGHTED_SIZE_0_127.\n")));
