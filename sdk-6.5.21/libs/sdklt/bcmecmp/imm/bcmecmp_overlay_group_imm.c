                               ...
                               void *context)
{
    ...
    if (lt_entry->lb_mode == BCMECMP_LB_MODE_RESILIENT ||
        ...
        sal_memcpy(lt_entry->o_nhop_id, lt_entry->rh_o_nhop_id,
                   BCMECMP_NHOP_ARRAY_SIZE * sizeof(int));
        lt_entry->o_nhop_ids_count = lt_entry->rh_o_nhop_ids_count;
        if (BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_RH_O_NHOP_ID)) {
            BCMECMP_LT_FIELD_SET(lt_entry->fbmp, BCMECMP_LT_FIELD_O_NHOP_ID);
        }

        sal_memcpy(lt_entry->uecmp_id, lt_entry->rh_uecmp_id,
                   BCMECMP_UNDERLAY_ECMP_ARRAY_SIZE * sizeof(int));
        lt_entry->uecmp_ids_count = lt_entry->rh_uecmp_ids_count;
        if (BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_RH_UECMP_ID)) {
            BCMECMP_LT_FIELD_SET(lt_entry->fbmp, BCMECMP_LT_FIELD_UECMP_ID);
        }

        sal_memcpy(lt_entry->u_ecmp_nhop, lt_entry->rh_u_ecmp_nhop,
                   BCMECMP_NHOP_ARRAY_SIZE * sizeof(uint8_t));
        lt_entry->u_ecmp_nhop_count = lt_entry->rh_u_ecmp_nhop_count;
        if (BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_RH_ECMP_NHOP)) {
        ...
        if (event == BCMIMM_ENTRY_UPDATE) {
            ...
            sal_memcpy(cur_lt_entry->o_nhop_id, cur_lt_entry->rh_o_nhop_id,
                       BCMECMP_NHOP_ARRAY_SIZE * sizeof(int));
            cur_lt_entry->o_nhop_ids_count = cur_lt_entry->rh_o_nhop_ids_count;

            sal_memcpy(cur_lt_entry->uecmp_id, cur_lt_entry->rh_uecmp_id,
                       BCMECMP_UNDERLAY_ECMP_ARRAY_SIZE * sizeof(int));
            cur_lt_entry->uecmp_ids_count = cur_lt_entry->rh_uecmp_ids_count;

            sal_memcpy(cur_lt_entry->u_ecmp_nhop, cur_lt_entry->rh_u_ecmp_nhop,
                       BCMECMP_NHOP_ARRAY_SIZE * sizeof(uint8_t));
            cur_lt_entry->u_ecmp_nhop_count = cur_lt_entry->rh_u_ecmp_nhop_count;
        }
    }
    ...
    for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
        ...
        if (BCMECMP_GRP_LB_MODE(unit, ecmp_id) == BCMECMP_LB_MODE_RESILIENT
            ...
            if (in_use_bk &&
                ...
                SHR_IF_ERR_EXIT
                    (bcmecmp_group_rh_realloc
                        (unit, ecmp_id, BCMECMP_GRP_TYPE_OVERLAY,
                         BCMECMP_GRP_RH_ENTRIES_CNT(unit, ecmp_id),
                         true));
            }
                                ...
                                bcmecmp_grp_attr_t *grp_bk_attr)
{
    ...
    if (BCMECMP_GRP_LB_MODE(unit, ecmp_id) == BCMECMP_LB_MODE_RESILIENT
        ...
        SHR_NULL_CHECK(BCMECMP_GRP_RH_ENTRIES_BK_PTR(unit, ecmp_id),
                                                     SHR_E_INTERNAL);
        if (BCMECMP_GRP_RH_ENTRIES_CNT_BK(unit, ecmp_id) !=
                BCMECMP_GRP_RH_ENTRIES_CNT(unit, ecmp_id)) {
            SHR_IF_ERR_EXIT
                (bcmecmp_group_rh_realloc
                    (unit, ecmp_id, BCMECMP_GRP_TYPE_OVERLAY,
...
/*!
 * \brief Overlay ECMP LT entry fields parse routine.
 *
 ...
 */
static int
bcmecmp_overlay_group_lt_entry_parse(int unit,
                                     bcmltd_sid_t sid,
                                     bcmecmp_lt_entry_t *lt_entry,
                                     bcmltd_fields_t *out)
{
    ...
    SHR_ALLOC(fid_list, table_sz, "bcmOvlayEcmpFldList");
    ...
    for (i = 0; i < count; i++) {
        ...
        switch (fid) {
            ...
            case ECMP_OVERLAYt_RH_UNDERLAY_NHOP_IDf:
                ...
                for (idx = 0;
                     idx < BCMECMP_LT_MAX_RH_NPATHS(unit, group,
                                                    lt_entry->grp_type);
                     idx++) {
                    out->field[out->count]->id = fid;
                    if (lt_entry->rh_nhop_id[idx] == BCMECMP_INVALID) {
                        out->field[out->count]->data =
                            BCMECMP_LT_MIN_NHOP_ID(unit, group,
                                                   lt_entry->grp_type);
                    } else {
                        out->field[out->count]->data = lt_entry->rh_nhop_id[idx];
                    }
                    out->field[out->count]->idx = idx;
                    out->count++;
                }
            ...
            case ECMP_OVERLAYt_RH_ECMP_NHOP_UNDERLAYf:
                ...
                for (idx = 0;
                     ...
                     idx++) {
                    out->field[out->count]->id = fid;
                    if (lt_entry->rh_u_ecmp_nhop[idx] ==
                            (uint8_t)BCMECMP_INVALID) {
                        out->field[out->count]->data = 0;
                    } else {
                        out->field[out->count]->data =
                            lt_entry->rh_u_ecmp_nhop[idx];
                    }
                    out->field[out->count]->idx = idx;
                    out->count++;
                }
            ...
            case ECMP_OVERLAYt_RH_ECMP_UNDERLAY_IDf:
                ...
                for (idx = 0;
                     ...
                     idx++) {
                    out->field[out->count]->id = fid;
                    if (lt_entry->rh_uecmp_id[idx] == BCMECMP_INVALID) {
                        out->field[out->count]->data = lt_fattr->min_uecmp_id;;
                    } else {
                        out->field[out->count]->data =
                            lt_entry->rh_uecmp_id[idx];
                    }
                    out->field[out->count]->idx = idx;
                    out->count++;
                }
            ...
            case ECMP_OVERLAYt_RH_OVERLAY_NHOP_IDf:
                ...
                for (idx = 0;
                     ...
                     idx++) {
                    out->field[out->count]->id = fid;
                    if (lt_entry->rh_o_nhop_id[idx] == BCMECMP_INVALID) {
                        out->field[out->count]->data = lt_fattr->min_o_nhop_id;
                    } else {
                        out->field[out->count]->data =
                            lt_entry->rh_o_nhop_id[idx];
                    }
                    out->field[out->count]->idx = idx;
                    out->count++;
                }
...
/*!
 * \brief ECMP overlay group imm lookup callback.
 *
 * \param [in]  unit            Unit number.
 ...
 */
static int
bcmecmp_overlay_group_lookup(int unit,
                             bcmltd_sid_t sid,
                             const bcmltd_op_arg_t *op_arg,
                             void *context,
                             ...
                             bcmltd_fields_t *out)
{
    ...
    if (BCMECMP_FNCALL_EXEC(unit, shr_grp_type_get) == NULL) {
        /*For TH and TH3, cache-bypass lookup is not supported.*/
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "cache-bypass lookup is not supported"
                                "on TH and TH3.\n")));
        SHR_EXIT();
    }
    ...
    if ((op_arg->attrib & BCMLT_ENT_ATTR_GET_FROM_HW)) {
        ...
        } else if (BCMECMP_FNCALL_EXEC(unit, rh_grp_get)) {
            SHR_IF_ERR_EXIT
                (BCMECMP_FNCALL_EXEC(unit, rh_grp_get)(unit, lt_entry));
        }
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmecmp_overlay_group_lt_entry_parse(unit, sid, lt_entry, out));
...
static bcmimm_lt_cb_t event_hdl = {
    ...
    .op_lookup = bcmecmp_overlay_group_lookup
};
