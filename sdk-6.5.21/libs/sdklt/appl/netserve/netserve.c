...
_setup_socket(int port)
{
    ...
    i = 1;
    if (setsockopt(sockfd, SOL_SOCKET,
                   SO_KEEPALIVE, (char *)&i, sizeof(i)) < 0) {
        perror("setsockopt(SO_KEEPALIVE)");
    }

    /*
     * Wait time before the first keepalive probe is sent
     * when there is no activity.
     */
    i = 600;
    if (setsockopt(sockfd, SOL_TCP,
                   TCP_KEEPIDLE, (char *)&i, sizeof(i)) < 0) {
    ...
    i = 20;
    if (setsockopt(sockfd, SOL_TCP,
                   TCP_KEEPCNT, (char *)&i, sizeof(i)) < 0) {
        perror("setsockopt(TCP_KEEPCNT)");
    }

    /* Set the time difference between each probe. */
    i = 60;
    if (setsockopt(sockfd, SOL_TCP,
                   TCP_KEEPINTVL, (char *)&i, sizeof(i)) < 0) {
