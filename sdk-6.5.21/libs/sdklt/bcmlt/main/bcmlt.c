...
static unit_state_t unit_attached_map[BCMDRD_CONFIG_MAX_UNITS];
...
int bcmlt_close(void)
{
    ...
    for (j = 0; j < BCMDRD_CONFIG_MAX_UNITS; j++) {
        if (unit_attached_map[j] != DETACHED) {
            LOG_ERROR(BSL_LOG_MODULE,
...
int bcmlt_device_attach(int unit, bool warm)
{
    ...
    if (unit_attached_map[unit] != DETACHED) {
    ...
    unit_attached_map[unit] = ATTACHING;
    ...
    unit_attached_map[unit] = ATTACHED;
exit:
    if (SHR_FUNC_ERR() && valid_unit && unit_attached_sem[unit]) {
        int rv;
        rv = shr_sysm_instance_stop(SHR_SYSM_CAT_UNIT,
                                    unit,
                                    true,
                                    unit_init_cb);
        if (SHR_SUCCESS(rv)) {
        ...
        unit_attached_map[unit] = DETACHED;
    }
    SHR_FUNC_EXIT();
}
                         ...
                         int status)
{
    if (instance_cat == SHR_SYSM_CAT_UNIT) {
        ...
        /* Unit stayed attached fs the status is not OK or Partial */
