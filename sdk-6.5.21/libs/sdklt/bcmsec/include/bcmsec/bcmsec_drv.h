...
(*bcmsec_sa_policy_get_f)(int unit, bcmltd_sid_t ltid,
                       ...
                       int encrypt);
/*!
 * Subport policy config set
 */
typedef int
(*bcmsec_subport_policy_set_f)(int unit, bcmltd_sid_t ltid,
                       bcmsec_pt_info_t *pt_dyn_info, bcmltd_field_t *in);
...
/*!
 * Check if SEC is enabled on the Port Macro.
 * PM_ID range starts from 1.
 */
typedef int
(*bcmsec_is_pm_sec_enabled_f)(int unit, int pm_id);
...
typedef struct bcmsec_drv_s {
    ...
    /*! Subport policy set. */
    bcmsec_subport_policy_set_f subport_policy_set;

    /*! Check if blktype is BLKTYPE_MACSEC. */
    bcmsec_blktype_is_sec_f     blktype_is_sec;

    /*! Check if SEC is enabled on the Port Macro. */
    bcmsec_is_pm_sec_enabled_f
                                is_pm_sec_enabled;
...
/*!
 * \brief Determine if SEC is supported on the PM_ID.
 *        Note: PM_ID range starts from 1.
 *
 * \param [in] unit   Unit Number.
 * \param [in] pm_id  Port Macro number
 *
 * \return Return TRUE if SEC is supported,
 *                FALSE otherwise.
 */
extern int
bcmsec_is_pm_sec_supported(int unit, int pm_id);
