                                     ...
                                     const bcmpc_sec_port_cfg_t *new_cfg)
{
    ...
    for (idx = 0; idx < (int)num_ports; idx++) {
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META("pport[%d] : lport [%d]\n"),
                 pport, lport));
        LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META("lport [%d] : num_lanes [%d]\n"),
                 lport, port_cfg->num_lanes));
    }
...
static int
bcmsec_esec_config_set(int unit, int pport, int port_up)
{
    int rv, lport, sec_port;
    uint32_t fval;
    ...
    SHR_IF_ERR_EXIT
        (bcmsec_port_lport_get(unit, pport, &lport));

    SHR_IF_ERR_EXIT
        (bcm56780_a0_sec_pport_to_secport(unit, pport, NULL,
                                          &sec_port, NULL));

    rv = bcmsec_egress_port_config_get(unit, lport, &fval);
...
/*
 * Disable SEC clear on read. Also enable rollover of counters.
 */
static int
bcmsec_ctr_disable_cor(int unit)
{
    uint32_t fval;
    bcmdrd_sid_t mem = MACSEC_MIB_MODEr;
    bcmsec_pt_info_t pt_info = {0};
    uint64_t flags = {0};
    bcmltd_sid_t ltid = -1; /* Requesting LT ID not available */
    uint32_t ltmbuf[2] = {0};
    ...
    for (i = 0; i < NUM_SEC_BLOCKS; i++) {
        BCMSEC_PT_DYN_INFO(pt_info, 0, i, flags);
        SHR_IF_ERR_EXIT
            (bcmsec_pt_indexed_read(unit, mem, ltid, &pt_info, ltmbuf));

        fid = MIB_CLEAR_ON_READf;
        fval = 0;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fid = MIB_SATURATEf;
...
/*
 * Configure default values for TOP_MACSEC_CTRL0r to update cache.
 */
static int
bcmsec_macsec_mux_set(int unit)
{
    bcmdrd_sid_t mem = TOP_MACSEC_CTRL0r;
    bcmsec_pt_info_t pt_info = {0};
    bcmltd_sid_t ltid = -1; /* Requesting LT ID not available */
    ...
    /* Keep in sync with bd_dev_reset. */
    ltmbuf[0] = 0x1fff000;
    SHR_IF_ERR_EXIT
        (bcmsec_pt_indexed_write(unit, mem, ltid, &pt_info, ltmbuf));
exit:
...
/*
 * Update bootup time ISEC config
 */
static int
bcmsec_isec_misc_ctrl_default(int unit)
{
    uint32_t fval;
    bcmdrd_sid_t mem = ISEC_MISC_CTRLr;
    bcmsec_pt_info_t pt_info = {0};
    uint64_t flags = {0};
    bcmltd_sid_t ltid = -1; /* Requesting LT ID not available */
    uint32_t ltmbuf[2] = {0};
    ...
    for (i = 0; i < NUM_SEC_BLOCKS; i++) {
        ...
        fid = REPLAY_WINDOW_FOR_LAPf;
        fval = 0x80000000;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fval = 0;
        fid = MACSEC_SC_MAP_ENf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));

        fval = 1;
        fid = REPLAY_DUPLICATE_EOP_CHK_ENf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fid = IPSEC_PADLEN_OUTOFRANGE_CHK_ENf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fid = IPSEC_ESP_TRAIL_ALIGNMENT_CHK_ENf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fid = IPSEC_ILLEGAL_NXTHDR_CHK_ENf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fid = ENABLE_PARSER_STRICT_IPVER_CHKf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fid = SA_SOFT_EXPIRY_ENf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fid = SA_HARD_EXPIRY_ENf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
        fid = USE_PORTID_FOR_SVTAG_SC_INDEXf;
        SHR_IF_ERR_EXIT
            (bcmsec_field_set(unit, mem, fid, ltmbuf, &fval));
...
bcm56780_a0_sec_chip_init(int unit, bool warm)
{
    ...
    if (!warm) {
        SHR_IF_ERR_EXIT
            (bcmsec_macsec_mux_set(unit));

        SHR_IF_ERR_EXIT
            (bcmsec_ctr_disable_cor(unit));

        SHR_IF_ERR_EXIT
            (bcmsec_isec_misc_ctrl_default(unit));
    }
...
bcm56780_a0_sec_port_tx_up(int unit, bcmsec_pport_t pport)
{
    ...
    /* Configure tx_threshold to default or user provided values. */
    (void) bcmsec_esec_config_set(unit, pport, 1);
exit:
    SHR_FUNC_EXIT();
}
...
bcm56780_a0_sec_port_tx_down(int unit, bcmsec_pport_t pport)
{
    ...
    /* Configure tx_threshold to default. */
    (void) bcmsec_esec_config_set(unit, pport, 0);
