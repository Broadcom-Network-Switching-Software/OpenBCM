        ...
        int *lport)
{
    ...
    *lport = pinfo->port_p2l_mapping;
    if (*lport == -1) {
        LOG_VERBOSE(BSL_LOG_MODULE,(BSL_META_U(unit,
                "Unable to get lport for physical port %d.\n"),
                pport));
        SHR_VERBOSE_EXIT(SHR_E_NOT_FOUND);
    }
exit:
...
int
bcmsec_egress_port_config_get(int unit, int lport,
                              uint32_t *tx_threshold)
{
    ...
    fid = SEC_ENCRYPT_PORTt_TX_THRESHOLD_CELLSf;
    SHR_IF_ERR_EXIT
        (bcmsec_field_list_get(unit, (bcmltd_fields_t *)&out, fid, 0, &fval));
    *tx_threshold = (uint32_t) fval;
