/*! \file bcmltx_l2_fdb_vlan_dest.c
 *
...
#include <bcmltx/bcml2/bcmltx_l2_fdb_vlan_dest.h>
...
#define LT_FLD_IDX_MODID         2
#define LT_FLD_IDX_MODPORT       3
#define LT_FLD_IDX_L2_MC_GRP_ID  4

#define PT_FLD_IDX_DEST_TYPE     0
#define PT_FLD_IDX_TGID          1
#define PT_FLD_IDX_MODULE_ID     2
#define PT_FLD_IDX_PORT_NUM      3
#define PT_FLD_IDX_L2MC_PTR      4
...
bcmltx_l2_mac_type_t bcmltx_l2_mac_type;
/******************************************************************************
* Public functions
 */
/*
                                  ...
                                  const bcmltd_transform_arg_t *arg)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "\t bcmltx_l2_fdb_vlan_dest_transform\n")));
    ...
    for (i = 0; i < in->count; i++) {
        ...
        } else if (in->field[i]->id == arg->field[LT_FLD_IDX_MODID]) {
    ...
    if (lt_dest_type_field_val == LT_PORT) {
        ...
        out->count++;
        out->field[out->count]->id = arg->rfield[PT_FLD_IDX_MODULE_ID];
                                      ...
                                      const bcmltd_transform_arg_t *arg)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                "\t bcmltx_l2_fdb_vlan_dest_rev_transform\n")));
    for (i = 0; i < in->count; i++) {
        ...
        } else if (in->field[i]->id == arg->field[PT_FLD_IDX_MODULE_ID]) {
    ...
    if (dest_type_field_val == 1) {
        ...
        out->count++;
        out->field[out->count]->id = arg->rfield[LT_FLD_IDX_MODID];
        out->field[out->count]->data = 0;
        out->count++;
