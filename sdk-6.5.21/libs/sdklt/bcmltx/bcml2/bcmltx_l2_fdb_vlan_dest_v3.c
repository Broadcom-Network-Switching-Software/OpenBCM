/*! \file bcmltx_l2_fdb_vlan_dest_v3.c
 *
...
#include <bcmltx/bcml2/bcmltx_l2_fdb_vlan_dest_v3.h>
...
#define PT_FLD_IDX_DESTINATION   1
                                     ...
                                     const bcmltd_transform_arg_t *arg)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "\t bcmltx_l2_fdb_vlan_dest_v3_transform\n")));
    ...
    if (lt_dest_type_field_val == LT_PORT) {
        ...
        out->count++;
        out->field[out->count]->id = arg->rfield[PT_FLD_IDX_DESTINATION];
                                        ...
                                        const bcmltd_transform_arg_t *arg)
{
    ...
    uint64_t destination_field_val = 0;
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                "\t bcmltx_l2_fdb_vlan_dest_v3_rev_transform\n")));

    for (i = 0; i < in->count; i++) {
        if (in->field[i]->id == arg->field[PT_FLD_IDX_DEST_TYPE]) {
            dest_type_field_val = in->field[i]->data;
        } else if (in->field[i]->id == arg->field[PT_FLD_IDX_DESTINATION]) {
           destination_field_val = in->field[i]->data;
        }else {
            rv = SHR_E_PARAM;
            SHR_ERR_EXIT(rv);
        }
    ...
    if (dest_type_field_val == 0) {
        ...
        out->field[out->count]->data = destination_field_val;
        out->count++;
