/*! \file bcmltx_port_vlan_xlate_ing_xlate_tbl_sel_first_lookup.c
 *
...
#include <bcmltx/bcmport/bcmltx_port_vlan_xlate_ing_xlate_tbl_sel_first_lookup.h>

#define BSL_LOG_MODULE BSL_LS_BCMLTX_PORT

#define LT_FLD_IDX_SEL_FIRST_LOOKUP    0
 ...
 */
int
bcmltx_port_vlan_xlate_ing_xlate_tbl_sel_first_lookup_rev_transform(int unit,
                                                                    const bcmltd_fields_t *in,
                                                                    bcmltd_fields_t *out,
                                                                    const bcmltd_transform_arg_t *arg)
{
    ...
    for (i = 0;
        ...
        if (key_type_val == (uint64_t)ing_xlate_tbl_sel_info[i].key_type &&
            port_type_val == (uint64_t)ing_xlate_tbl_sel_info[i].port_type) {
            out->field[out->count]->id = arg->rfield[LT_FLD_IDX_SEL_FIRST_LOOKUP];
