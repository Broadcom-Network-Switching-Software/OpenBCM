                ...
                uint32_t flush)
{
    ...
    ioerr += xlport_pause_get(pa, port, &pause_cfg);
    ioerr += xlport_pause_set(pa, port, &pause_disable);
    ...
    ioerr += xlport_pause_set(pa, port, &pause_cfg);
    return ioerr? ((ioerr == SHR_E_BUSY)? SHR_E_BUSY: SHR_E_ACCESS): SHR_E_NONE;
}
                           ...
                           uint32_t *en)
{
    ...
    XLMAC_CTRLr_CLR(mac_ctrl);
    ioerr += READ_XLMAC_CTRLr(pa, port, &mac_ctrl);
    *en = XLMAC_CTRLr_LAG_FAILOVER_ENf_GET(mac_ctrl);
...
static inline int
xlport_link_status_get(bcmpmac_access_t *pa, bcmpmac_pport_t port,
                       uint32_t clear_on_read, uint32_t start_lane,
                       int* link)
{
    int ioerr = 0;
    XLPORT_XGXS0_STATUS0_REGr_t ln_status;

    XLPORT_XGXS0_STATUS0_REGr_CLR(ln_status);

    ioerr += READ_XLPORT_XGXS0_STATUS0_REGr(pa, port, start_lane,
                                           &ln_status);

    *link = XLPORT_XGXS0_STATUS0_REGr_LINK_STATUSf_GET(ln_status);
