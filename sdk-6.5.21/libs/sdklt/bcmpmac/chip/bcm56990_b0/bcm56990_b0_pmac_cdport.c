/*! \file bcm56990_b0_pmac_cdport.c
 *
 * The CDPORT driver for BCM56990_B0.
 *
 * Each CDPORT on BCM56990_B0 only has 4 block ports, and its lane combination
...
#include <bcmpmac/chip/bcm56990_b0_cdport_defs.h>
...
static int
bcm56990_b0_cdport_tx_flush(bcmpmac_access_t *pa, bcmpmac_pport_t port,
                            uint32_t flush)
{
    int ioerr = 0;
    shr_timeout_t to;
    ...
    bcmpmac_pause_ctrl_t pause_cfg;
    bcmpmac_pause_ctrl_t pause_disable;
    ...
    /* Save, disable and restore Pause settings. */
    sal_memset(&pause_cfg, 0, sizeof(bcmpmac_pause_ctrl_t));
    sal_memset(&pause_disable, 0, sizeof(bcmpmac_pause_ctrl_t));
    ioerr += cdport_pause_get(pa, port, &pause_cfg);
    ioerr += cdport_pause_set(pa, port, &pause_disable);
    ...
    ioerr += cdport_pause_set(pa, port, &pause_cfg);
    return ioerr ? SHR_E_ACCESS : SHR_E_NONE;
}

static int
bcm56990_b0_cdport_op_exec(bcmpmac_access_t *pa, bcmpmac_pport_t port,
                           const char *op, uint32_t op_param)
{
    ...
    const bcmpmac_drv_op_hdl_t ops[] = {
        ...
        { "tx_flush", bcm56990_b0_cdport_tx_flush },
...
static int
bcm56990_b0_cdport_port_init(bcmpmac_access_t *pa, bcmpmac_pport_t port,
                             uint32_t init)
{
    int rv, ioerr = 0;
    CDMAC_TX_CTRLr_t tx_ctrl;
...
bcmpmac_drv_t bcm56990_b0_pmac_cdport = {
    .name = "cdport",
    ...
    .port_init = bcm56990_b0_cdport_port_init,
    ...
    .op_exec = bcm56990_b0_cdport_op_exec,
