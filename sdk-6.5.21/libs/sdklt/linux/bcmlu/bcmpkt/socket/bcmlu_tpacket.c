                            ...
                            unsigned int tp_sec, unsigned int tp_usec)
{
    ...
    SHR_NULL_CHECK(frame, SHR_E_PARAM);

    rhdr = bcmpkt_rcpu_hdr(tp->unit);
    assert(rhdr);
    ring = (bcmlu_tpacket_ring_t *)tp->priv;
    data = frame + tp_mac;
...
bcmlu_tpacket_read_v1(bcmlu_tpacket_t *tp)
{
    ...
    while (1) {
        /* Shared buffer pool may be freed and becomes dangling pointers. */
        SHR_IF_ERR_EXIT
            (bcmpkt_bpool_status_get(BCMPKT_BPOOL_SHARED_ID, &bpool_status));
        if (!bpool_status.active) {
            SHR_ERR_EXIT(SHR_E_DISABLED);
...
header_generate(int unit, bcmpkt_packet_t *packet)
{
    ...
    hg3_hid = *(hid_support + BCMPKT_HG3_BASE_T);
    glb_hid = *(hid_support + BCMPKT_GENERIC_LOOPBACK_T);

    /* If not HG/HG2, check for HG3 */
    if ((hg3_hid != -1) && (packet->pmd.higig != NULL) &&
        (hg_size == 0)) {
        uint32_t hstart;
        uint32_t ext_hdrs;
        int fid = BCMPKT_FID_INVALID;

        SHR_IF_ERR_EXIT(bcmpkt_flexhdr_field_id_get(variant, hg3_hid,
                                                    "RESERVED_ETYPE", &fid));
        SHR_IF_ERR_EXIT(bcmpkt_flexhdr_field_get(variant, hg3_hid,
                                                 packet->pmd.higig,
                                                 BCMPKT_FLEXHDR_PROFILE_NONE,
                                                 fid, &hstart));
        /* Extract 2-bit field for determining presence of ext hdr */
        SHR_IF_ERR_EXIT
            (bcmpkt_flexhdr_field_id_get(variant, hg3_hid, "EXT_HDR_PRESENT",
                                         &fid));
        SHR_IF_ERR_EXIT
            (bcmpkt_flexhdr_field_get(variant, hg3_hid, packet->pmd.higig,
                                      BCMPKT_FLEXHDR_PROFILE_NONE, fid,
                                      &ext_hdrs));
     ...
     * ethernet packet
     */
    if ((hg3_hid != -1) && (is_higig3_hdr) && (hg3_size > 0)) {
    ...
    if ((packet->pmd.txpmd != NULL) && (packet->pmd.txpmd[0] != 0)) {
        ...
        if ((packet->pmd.lbhdr != NULL) && (packet->pmd.lbhdr[0] != 0)) {
            if (glb_hid != -1) {
                int fid = BCMPKT_FID_INVALID;
                SHR_IF_ERR_EXIT
                    (bcmpkt_flexhdr_field_id_get(variant, glb_hid,
                                                 "START_BYTE", &fid));
                SHR_IF_ERR_EXIT
                    (bcmpkt_flexhdr_field_get(variant, glb_hid, packet->pmd.lbhdr,
                                              BCMPKT_FLEXHDR_PROFILE_NONE, fid,
                                              &lbstart));
            } else {
                SHR_IF_ERR_EXIT
        ...
        int fid = BCMPKT_FID_INVALID;
        SHR_IF_ERR_EXIT
            (bcmpkt_flexhdr_field_id_get(variant, glb_hid, "START_BYTE", &fid));
        SHR_IF_ERR_EXIT
            (bcmpkt_flexhdr_field_get(variant, glb_hid, packet->pmd.lbhdr,
                                      BCMPKT_FLEXHDR_PROFILE_NONE, fid,
                                      &gihstart));
