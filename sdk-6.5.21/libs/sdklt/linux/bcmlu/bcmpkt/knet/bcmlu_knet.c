                            ...
                            ngknet_filter_t *bfilter)
{
    ...
    switch (filter->dest_type) {
        case BCMPKT_DEST_T_NULL:
            bfilter->dest_type = NGKNET_FILTER_DEST_T_NULL;
            break;
        case BCMPKT_DEST_T_NETIF:
            bfilter->dest_type = NGKNET_FILTER_DEST_T_NETIF;
            break;
        default:
                            ...
                            bcmpkt_filter_t *filter)
{
    ...
    bcmpkt_bitmap_t flex_reason_mask;
    int reason_count = 0;
    ...
    switch (bfilter->dest_type) {
        case NGKNET_FILTER_DEST_T_NULL:
            filter->dest_type = BCMPKT_DEST_T_NULL;
            break;
        ...
        if (fdata_len) {
            ...
            SHR_IF_ERR_EXIT
                (bcmpkt_rxpmd_flex_reasons_get(variant, fdata_mask,
                                               &flex_reason_mask));
            SHR_IF_ERR_EXIT
                (bcmpkt_rxpmd_flex_reason_max_get(variant, &max_reason_count));
