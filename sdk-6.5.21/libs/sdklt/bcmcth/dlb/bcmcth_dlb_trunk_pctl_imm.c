...
/*!
 * \brief DLB_TRUNK_PORT_CONTROL get entry limit size.
 *
 * Get table entry limit based on hardware availability.
 *
 * \param [in] unit This is device unit number.
 * \param [in] trans_id is the transaction ID associated with this operation.
 * \param [in] sid This is the logical table ID.
 * \param [in] count Is a pointer that has existing value from caller.
 *
 * \retval None.
 */
static int
dlb_trunk_port_ctrl_entry_limit_get(int unit,
                                    uint32_t trans_id,
                                    bcmltd_sid_t sid,
                                    uint32_t *count)
{
    bcmltd_sid_t hw_sid = DLB_TRUNK_INVALID_SID;
    ...
    bcmcth_dlb_sid_get(unit, &hw_sid);

    /* Default to entry max in case no return from dispatch layer. */
    bcmlrd_map_get(unit, DLB_TRUNK_PORT_CONTROLt, &map);
    ...
    *count = 0;
    for (k = 0; k < map->table_attr->attributes; k++) {
        if (map->table_attr->attr[k].key ==
            ...
            *count = map->table_attr->attr[k].value;
            break;
        }
    }

    if (hw_sid != DLB_TRUNK_INVALID_SID) {
        if (!bcmdrd_pt_is_valid(unit, hw_sid)) {
            *count = 0;
        }
    }
...
static bcmimm_lt_cb_t bcmdlb_trunk_port_ctrl_imm_callback = {
  ...
  .ent_limit_get = dlb_trunk_port_ctrl_entry_limit_get
};
