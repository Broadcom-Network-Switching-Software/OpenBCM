...
#include <bcmcth/chip/bcm56780_a0/bcm56780_a0_cth_mon_ft_intr.h>
...
#define BCM56780_A0_EM_FT_BASE_ENTRY_SIZE 120

#define BCM56780_A0_DT_EM_FT_GRP_MODE_SINGLE 1 /* SINGLE */
#define BCM56780_A0_DT_EM_FT_GRP_MODE_DOUBLE 3 /* DOUBLE */
#define BCM56780_A0_DT_EM_FT_GRP_MODE_QUAD   5 /* QUAD */
...
ing_pdd_template_partition_info_lt = {
    ...
    .key_fid2 = "PARTITION_ID",
    .key_fid3 = "FP_EM_GRP_TEMPLATE_ID",
    .num_actions = 2,
...
ing_pdd_template_partition_info_ft_lt = {
    ...
    .key_fid2 = "PARTITION_ID",
    .key_fid3 = "FP_FT_GRP_TEMPLATE_ID",
    .num_actions = 2,
    .action_info[0].action = SHR_FT_EM_ACTION_CTR,
    .action_info[0].num_action_info_fid = "NUM_ACTION_FLEX_CTR_VALID",
    .action_info[0].action_offset_fid = "ACTION_FLEX_CTR_VALID_OFFSET",
...
static int
bcm56780_a0_cth_mon_em_ft_grp_mode_to_num_base_entries(
            int unit,
            uint32_t mode_val,
            uint8_t *num_base_entries)
{
    SHR_FUNC_ENTER(unit);
    switch (mode_val) {
        case BCM56780_A0_DT_EM_FT_GRP_MODE_SINGLE:
            *num_base_entries = 1;
            break;
        case BCM56780_A0_DT_EM_FT_GRP_MODE_DOUBLE:
            *num_base_entries = 2;
            break;
        case BCM56780_A0_DT_EM_FT_GRP_MODE_QUAD:
            *num_base_entries = 4;
            break;
        default:
...
static int
bcm56780_a0_cth_mon_em_ft_grp_mode_to_pdd_info_ref_offset(
            int unit,
            uint32_t mode_val,
            uint32_t *pdd_info_ref_offset)
{
    uint8_t num_base_entries = 0;
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT(bcm56780_a0_cth_mon_em_ft_grp_mode_to_num_base_entries(
                    unit, mode_val, &num_base_entries));
    *pdd_info_ref_offset =
        (num_base_entries * BCM56780_A0_EM_FT_BASE_ENTRY_SIZE) -1;
exit:
    SHR_FUNC_EXIT();
}
        ...
        bcmcth_mon_ft_ing_pdd_template_partition_info_lt_id_t *out)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlrd_table_field_name_to_idx(unit,
        in->ltid, in->key_fid3, &out->ltid, &out->key_fid3));
...
bcm56780_a0_cth_mon_ing_pdd_template_partition_info_lt_lookup (
    ...
    uint8_t *num_actions,
    uint32_t *total_action_data_width,
    ...
    uint32_t mode_val)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_ft_util_fields_alloc(unit, 3, &in_flds));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_ft_util_fields_alloc(unit, actual_fld_count, &out_flds));
    ...
    in_flds.field[1]->id = lt_id_info->key_fid2;
    in_flds.field[1]->data = 1;
    in_flds.field[2]->id = lt_id_info->key_fid3;
    in_flds.field[2]->data = dt_em_grp_template_id;
    ...
    *num_actions = 0;
    ...
    /*
     * Find out the smallest offset. Since its sorted, the first value
     * is the smallest value.
     */
    smallest_offset = out[0].action_offset[0];
    ...
    /*
     * To get the total action width, find the distance of smallest_offset
     * from the MSB offset of a given PDD mode
     */
    *total_action_data_width = msb_offset - smallest_offset + 1;


    /* Change all the other offsets relative to the smallest offset */
    for (i = 0; i < *num_actions; i++) {
        for (j = 0; j < out[i].num_action_info; j++) {
            out[i].action_offset[j] = out[i].action_offset[j] - smallest_offset;
        }
    }
exit:
        ...
        uint32_t data_width)
{
    ...
    for (i = 0; i < num_actions; i++) {
        ...
        /* Assumes action is not split into multiple parts */
        msg->aset_offsets[i] =
            pdd_template_partition_info[i].action_offset[0];

        msg->aset_widths[i] =
            pdd_template_partition_info[i].action_width[0];
    }
    ...
    uint32_t *total_width_in_bits)
{
    ...
    if (ft_drv) {
        ...
        SHR_IF_ERR_EXIT(bcm56780_a0_cth_mon_em_ft_grp_mode_to_num_base_entries(
                    unit, mode_val, &(msg->num_base_entries)));
    }
...
static int
bcm56780_a0_cth_mon_flowtracker_hw_flex_ctr_action_idx_get(
                                        int unit,
                                        uint32_t ctr_ing_flex_action_profile_id,
                                        uint16_t *flex_ctr_action_idx)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT(
            bcm56780_a0_cth_mon_flowtracker_flex_ctr_action_profile_get(
                unit,
                ctr_ing_flex_action_profile_id,
                &unused1,
                &unused2,
                flex_ctr_action_idx));
exit:
                           ...
                           bool *configured)
{
    ...
    /*
     * If we reached this spot it means that EM action does not match
     * any configured flowtracker group action. This could happen when
     * we implicitly assign flex counter action etc which does not come
     * through LT configuration. In such a case, set configured to false.
     */
    *configured = false;
    return 0;
}
...
static int
bcm56780_a0_cth_mon_flowtracker_hitbit_type_get_from_num_base_entries(
                    uint32_t num_base_entries)
{
    /* Currently we do not support single wide entries */
    if (num_base_entries == 2) {
        return MCS_SHR_FT_HIT_BIT_TYPE_2x;
    } else if (num_base_entries == 4) {
                                                ...
                                                uint16_t *start_offset)
{
    ...
    *start_offset = 104 - add_bits - total_width_in_bits;
                                ...
                                uint32_t data_width)
{
    ...
    uint16_t curr_offset = 0, start_offset = 0;
    bcmltd_sid_t lt_id = MON_FLOWTRACKER_GROUPt;
    bcmdrd_sid_t pt_id = FT_GROUP_TABLE_CONFIGm;
    int i;
    ...
    for (i = 0; i < msg->num_actions; i++) {
        switch (msg->aset[i]) {
            case SHR_FT_EM_ACTION_CTR:
                ...
                if (hitbit_type == MCS_SHR_FT_HIT_BIT_TYPE_2x) {
                    val =
                  (ctrl_entry->db_ewide_hb_ctr_ing_flex_grp_action) & 0x3f;
                } else if (hitbit_type == MCS_SHR_FT_HIT_BIT_TYPE_4x) {
                    val =
                  (ctrl_entry->qd_ewide_hb_ctr_ing_flex_grp_action) & 0x3f;
                }
                curr_offset = msg->aset_offsets[i];
                break;
        ...
        bcmdrd_field_set(buf, start_offset + curr_offset,
                         start_offset + curr_offset + msg->aset_widths[i] -1,
                         &val);
    }
...
static void
bcm56780_a0_cth_mon_flowtracker_grp_flex_policy_buf_action_val_set(
        uint8_t num_actions,
        uint32_t ft_grp_num_actions,
        flowtracker_action_info_t *actions,
        ing_pdd_template_partition_info_t *pdd_template_partition_info,
        uint16_t start_offset,
        uint32_t *buf)
{
    int i, j;
    uint32_t val = 0;
    uint16_t src_offset = 0, dst_offset = 0, width = 0;
    ...
    for (i = 0; i < num_actions; i++) {
        ...
        for (j = 0; j < pdd_template_partition_info[i].num_action_info; j++) {
            dst_offset =  start_offset + pdd_template_partition_info[i].action_offset[j];
            width = pdd_template_partition_info[i].action_width[j];
            shr_bitop_range_copy(buf, dst_offset, &val, src_offset, width);
            src_offset += width;
        }
    }
}
    ...
    for (pipe = 0; pipe < BCM56780_A0_FT_PIPES_MAX; pipe++) {
        ...
        SHR_IF_ERR_EXIT
            (bcmcth_mon_pt_iread_pipe(unit,
                                  ...
                                  pipe,
                                  mon_flowtracker_group_id,
                                  true,/* Since this is a HW updated value */
                                  &ft_group_table_cnt));
                                              ...
                                              bcmcth_mon_ft_control_t *entry)
{
    ...
    uint8_t e2t_00_htbl_bnk_config, e2t_01_htbl_bnk_config;
    uint8_t e2t_02_htbl_bnk_config, e2t_03_htbl_bnk_config;
    uint8_t e2t_00_hidx_entry_type, e2t_01_hidx_entry_type;
    uint8_t e2t_02_hidx_entry_type, e2t_03_hidx_entry_type;
    uint8_t e2t_00_sbus_num_bnks, e2t_01_sbus_num_bnks;
    uint8_t e2t_02_sbus_num_bnks, e2t_03_sbus_num_bnks;
    uint8_t e2t_00_ft_enable, e2t_01_ft_enable;
    uint8_t e2t_02_ft_enable, e2t_03_ft_enable;
    ...
    int index = 0;
    uint32_t max_flows = 0;
    /* In terms of number of single wide entries.*/
    const uint32_t size_of_one_tile = (32 * 1024);
    bool with_eapp = true;
    SHR_FUNC_ENTER(unit);
     ...
     * NOTE: flow threshold is in terms of single wide entries by default.
     * <=32K         -> 1 tile or 2 banks (IFTA80_E2T_00_B0/B1)
     * >32K && <=64K -> 2 tiles or 4 banks (IFTA80_E2T_00/01_B0/B1)
     * >64K && <=96K -> 3 tiles or 6 banks (IFTA80_E2T_00/01/02_B0/B1)
     * >96K && <=128K -> 4 tiles or 8 banks (IFTA80_E2T_00/01/02/03_B0/B1)
     */
    if (max_flows <= size_of_one_tile) {
        ...
        e2t_02_ft_enable = 0;
        e2t_03_ft_enable = 0;
        e2t_00_hidx_entry_type = 1;
        e2t_01_hidx_entry_type = 0;
        e2t_02_hidx_entry_type = 0;
        e2t_03_hidx_entry_type = 0;
        e2t_01_bank_bitmap_start_offset = 0;
        e2t_02_bank_bitmap_start_offset = 0;
        e2t_03_bank_bitmap_start_offset = 0;
    ...
    IFTA80_E2T_00_HASH_CONTROLm_HASH_TABLE_BANK_CONFIGf_SET(
                             ifta80_e2t_00_hash_control, e2t_00_htbl_bnk_config);
    IFTA80_E2T_00_HASH_CONTROLm_DRIVE_HT_LPM_HIT_INDEX_ENTRY_TYPEf_SET(
                             ifta80_e2t_00_hash_control, e2t_00_hidx_entry_type);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_pt_iwrite(unit, lt_id, pt_id,
                              index, &ifta80_e2t_00_hash_control));
    ...
    IFTA80_E2T_01_HASH_CONTROLm_SBUS_NUM_BANKSf_SET(
                            ifta80_e2t_01_hash_control, e2t_01_sbus_num_bnks);
    IFTA80_E2T_01_HASH_CONTROLm_DRIVE_HT_LPM_HIT_INDEX_ENTRY_TYPEf_SET(
                             ifta80_e2t_01_hash_control, e2t_01_hidx_entry_type);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_pt_iwrite(unit, lt_id, pt_id,
                              index, &ifta80_e2t_01_hash_control));
    ...
    IFTA80_E2T_02_HASH_CONTROLm_HASH_TABLE_BANK_CONFIGf_SET(
                            ifta80_e2t_02_hash_control, e2t_02_htbl_bnk_config);
    IFTA80_E2T_02_HASH_CONTROLm_DRIVE_HT_LPM_HIT_INDEX_ENTRY_TYPEf_SET(
                             ifta80_e2t_02_hash_control, e2t_02_hidx_entry_type);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_pt_iwrite(unit, lt_id, pt_id,
                              index, &ifta80_e2t_02_hash_control));
    ...
    IFTA80_E2T_03_HASH_CONTROLm_SBUS_NUM_BANKSf_SET(
                            ifta80_e2t_03_hash_control, e2t_03_sbus_num_bnks);
    IFTA80_E2T_03_HASH_CONTROLm_DRIVE_HT_LPM_HIT_INDEX_ENTRY_TYPEf_SET(
                             ifta80_e2t_03_hash_control, e2t_03_hidx_entry_type);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_pt_iwrite(unit, lt_id, pt_id,
                              index, &ifta80_e2t_03_hash_control));
                                                    ...
                                                    int *uc_fids)
{
    ...
    *num_fids = 0;
    hid_support = bcmpkt_flexhdr_support_map_get(unit);
    hid = *(hid_support + BCMPKT_RXPMD_FLEX_T);
    rv = bcmpkt_flexhdr_field_id_get(variant, hid, "I2E_CLASS_ID_15_0",
                                     &fids[*num_fids]);
    if (SHR_SUCCESS(rv)) {
                        ...
                        bcmcth_mon_ft_hw_learn_flow_action_t *entry)
{
    ...
    /*
     * Delete from HW is successful. Delete it from
     * MON_FLOWTRACKER_LEARN_FLOW_ENTRY LT also
     */
    SHR_IF_ERR_EXIT
        (bcmcth_mon_ft_learn_flow_entry_delete(unit, entry->pipe, entry->mode,
                                                entry->exact_match_idx));
exit:
    SHR_FUNC_EXIT();
}

static void
bcm56780_a0_cth_mon_flowtracker_ft_em_entry_read_mem_info(
        int unit,
        bcmltd_common_flowtracker_hw_learn_exact_match_idx_mode_t_t mode,
        uint32_t exact_match_idx,
        bcmdrd_sid_t *pt_id,
        int *index,
        uint8_t *buf_num_words)
{
    ...
    if (mode ==
            BCMLTD_COMMON_FLOWTRACKER_HW_LEARN_EXACT_MATCH_IDX_MODE_T_T_SINGLE) {
        bank_id = (exact_match_idx / num_entries_per_single_bank);
        *pt_id = pt_ids_single[bank_id];
        *index = (exact_match_idx % num_entries_per_single_bank);
        *buf_num_words = 4; /* 120 bits require 4 words */
    } else if (mode ==
            BCMLTD_COMMON_FLOWTRACKER_HW_LEARN_EXACT_MATCH_IDX_MODE_T_T_DOUBLE) {
        bank_id = (exact_match_idx / num_entries_per_double_bank);
        *pt_id = pt_ids_double[bank_id];
        *index = (exact_match_idx % num_entries_per_double_bank);
        *buf_num_words = 8; /* 240 bits require 8 words */
    } else if (mode ==
            BCMLTD_COMMON_FLOWTRACKER_HW_LEARN_EXACT_MATCH_IDX_MODE_T_T_QUAD) {
        bank_id = (exact_match_idx / num_entries_per_quad_bank);
        *pt_id = pt_ids_quad[bank_id];
        *index = (exact_match_idx % num_entries_per_quad_bank);
        *buf_num_words = 15; /* 480 bits require 15 words */
    }
}
                        ...
                        uint32_t *buf)
{
    ...
    uint8_t  data_format = FLOWTRACKER_HW_LEARN_DATA_FMT_DATA_ONLY;
    uint8_t num_actions = 0;
    uint16_t end_offset = 0, start_offset = 0;
    uint8_t unused = 0;
    uint32_t mode_val = 0;
    SHR_FUNC_ENTER(unit);
    lt_id = MON_FLOWTRACKER_HW_LEARN_FLOW_ACTION_CONTROLt;
    ...
    bcm56780_a0_cth_mon_flowtracker_ft_em_entry_read_mem_info(
            unit, entry->mode, entry->exact_match_idx, &pt_id, &index, &unused);
    /* Read the entry at given index */
    SHR_IF_ERR_EXIT
        (bcmcth_mon_pt_iread_pipe(unit, lt_id, pt_id,
                                  entry->pipe, index,
                                  /* Since this is a HW updated PT table */
                                  true,
                                  buf));
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_cth_mon_ing_pdd_template_partition_info_lt_lookup
         (unit,
          ...
          &num_actions,
          &total_width_in_bits,
          grp_entry->dt_em_grp_template_id,
          mode_val));
    if (grp_entry->oper !=
            BCMLTD_COMMON_MON_FLOWTRACKER_GROUP_STATE_T_T_SUCCESS) {
        SHR_EXIT();
    ...
    if (data_format ==
        ...
        start_offset = end_offset - 9 - total_width_in_bits;
    } else if (data_format == FLOWTRACKER_HW_LEARN_DATA_FMT_DATA_ONLY) {
        start_offset = end_offset - total_width_in_bits;
    }
...
static int
bcm56780_a0_cth_mon_ft_em_entry_lkup(
                            int unit,
                            bcmcth_mon_ft_learn_flow_entry_t *entry)
{
    ...
    bcmdrd_sid_t pt_id = 0;
    SHR_FUNC_ENTER(unit);
    lt_id = MON_FLOWTRACKER_LEARN_FLOW_ENTRYt;

    /* Get the PT ID and Index */
    bcm56780_a0_cth_mon_flowtracker_ft_em_entry_read_mem_info(
            unit, entry->mode, entry->exact_match_idx, &pt_id, &index,
            &entry->num_words_count);
...
static bcmcth_mon_flowtracker_drv_t bcm56780_a0_flowtracker_drv =
{
    ...
    .num_pipes = 2,
    .match_id_string_get = NULL,
    .ft_fifo_drv = &bcm56780_a0_cth_mon_ft_fifo_drv,
    .ft_intr_drv = &bcm56780_a0_cth_mon_ft_intr_drv,
    .ft_em_entry_lkup = &bcm56780_a0_cth_mon_ft_em_entry_lkup,
    .ft_hw_flex_ctr_action_idx_get =
            &bcm56780_a0_cth_mon_flowtracker_hw_flex_ctr_action_idx_get,
    .max_sgl_wide_entries_per_pipe = (128 * 1024),
    .max_dbl_wide_entries_per_pipe = (64 * 1024),
    .max_qd_wide_entries_per_pipe = (32 * 1024),
    .ft_hw_lt_info_init = &bcm56780_a0_cth_mon_flowtracker_hw_lt_info_init
};
...
static int bcm56780_a0_cth_mon_ft_ctr_ing_eflex_action_profile_lt_field_id_get(
                                                                       int unit)
{
...
bcm56780_a0_cth_mon_init(int unit, bool warm)
{
    ...
    if (SHR_SUCCESS(rv1) && map1) {
        ...
        if (!warm || SHR_E_NOT_FOUND ==
            bcmissu_struct_info_get(unit,
                                    BCMMON_TRACE_STATE_T_ID,
                                    BCMMGMT_MON_COMP_ID,
                                    BCMMON_TRACE_SUB_COMP_ID,
                                    &offset)) {
    ...
    if (SHR_SUCCESS(rv1) && map1) {
        ...
        if (!warm || SHR_E_NOT_FOUND ==
            bcmissu_struct_info_get(unit,
                                    BCMMON_REDIRECT_TRACE_STATE_T_ID,
                                    BCMMGMT_MON_COMP_ID,
                                    BCMMON_REDIRECT_TRACE_SUB_COMP_ID,
                                    &offset)) {
    ...
    if (SHR_SUCCESS(rv1) && map1) {
        ...
        SHR_IF_ERR_EXIT_EXCEPT_IF(
            bcm56780_a0_cth_mon_ft_ctr_ing_eflex_action_profile_lt_field_id_get(
        unit), SHR_E_UNAVAIL);
