    ...
    bool enable)
{
    ...
    if ((variant_data[unit]->ifa_1_ltid != BCMLTD_INVALID_LT) &&
        (variant_data[unit]->ifa_1_reserved_index != -1)) {
        /* Reserved IFA_1 index shouldn't be used by any other encap LT. */
        if ((lt_id != variant_data[unit]->ifa_1_ltid) &&
            (index == variant_data[unit]->ifa_1_reserved_index)) {
            LOG_DEBUG(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
                                  "Incorrect index[%d] used for LTID=%d\n"),
                                  index, lt_id));
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
        /* IFA_1 LT should use only reserved index. */
        if ((lt_id == variant_data[unit]->ifa_1_ltid) &&
            (index != variant_data[unit]->ifa_1_reserved_index)) {
            LOG_DEBUG(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
                                  "Reserved index was %d."\
                                  " Unreserved index[%d] used for LTID=%d\n"),
                                  variant_data[unit]->ifa_1_reserved_index,
                                  index, lt_id));
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    }
        ...
        case BCMCTH_MIRROR_ENCAP_BASIC:
            if (entry->encap_mode ==
                ...
                fval = 0xc2;
                       ...
                       BCM56780_A0_LRD_MIRROR_ENCAP_MODE_T_T_ENCAP_NONE) {
                /* non-encap flow. */
                fval = 0x0;
            } else {
                SHR_ERR_EXIT(SHR_E_UNAVAIL);
    ...
    bcmcth_mirror_encap_t *entry)
{
    ...
    /*
     * hdr_id (egress_pkt.sys_hdr.mirror_transport) in map.yml
     * ifa_1_hdr_id (egress_pkt.tunnel_l3_l4_hdr.ifa_1_hdr) in map.yml
     */
    if (ifa_1_probe) {
        hdr_id_list_hdr_id[0] =
            (variant_data[unit]->ifa_1_hdr_id_mirror_transport == -1 ? 54 :
             variant_data[unit]->ifa_1_hdr_id_mirror_transport);
    } else {
        hdr_id_list_hdr_id[0] = variant_data[unit]->hdr_id_mirror_transport;
    }
 ...
 */
     ...
     */
    if (variant_data[unit]->encap_data && truncate) {
        if (entry->truncate_ptr == 0) {
            fval = variant_data[unit]->default_truncate_ptr;
        } else {
            fval = entry->truncate_ptr;
        }

        if ((entry->type <= BCMCTH_MIRROR_ENCAP_RSPAN) &&
            variant_data[unit]->num_encap > 16) {
            tidx_max = variant_data[unit]->encap_data[16].edit_tcam_count;
            edit_tcam_data = variant_data[unit]->encap_data[16].edit_tcam_data;
        } else {
            tidx_max = variant_data[unit]->encap_data[index].edit_tcam_count;
        ...
        for (tidx = 0 ; tidx < tidx_max; tidx++) {

            pt_id = edit_tcam_data[tidx].pid;
            eidx = edit_tcam_data[tidx].tcam_index;
