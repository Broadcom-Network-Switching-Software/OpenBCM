...
/* Count of member fields in the TRUNK LT. */
#define TRUNK_GRP_MEMBER_FLDS_CNT 8

/* Count of member fields in the TRUNK_SYSTEM LT. */
#define TRUNK_SYS_GRP_MEMBER_FLDS_CNT 2
                          ...
                          uint32_t base_ptr)
{
    ...
    uint32_t gtype = BCMCTH_TRUNK_GRP_TYPE_REGULAR;
    uint32_t uc_cnt = 0;
    ...
    SHR_NULL_CHECK(grp, SHR_E_PARAM);
    ...
    if (gtype == BCMCTH_TRUNK_GRP_TYPE_RESILIENT) {
        /* Resilient group redistribution is per group size. */
        uc_cnt = param->uc_max_members;
    } else {
        uc_cnt = param->uc_cnt;
    }
    ...
    if (uc_cnt > 0) {
        ...
        if (gtype == BCMCTH_TRUNK_GRP_TYPE_REGULAR) {
            for (px = 0; px < uc_cnt; px++) {
            ...
            LOG_INFO(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                   "rh_member_cnt %d"), param->rh_member_cnt));

            /* Write IPOST_LAG_LAG_GROUPm. */
            IPOST_LAG_LAG_GROUPm_CLR(grp_buf);
            IPOST_LAG_LAG_GROUPm_LB_MODEf_SET(grp_buf, param->lb_mode);
            IPOST_LAG_LAG_GROUPm_LAG_SIZEf_SET(grp_buf, (uc_cnt - 1));
            IPOST_LAG_LAG_GROUPm_BASE_PTRf_SET(grp_buf, base_ptr);
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_trunk_hw_write(unit,
                                       op_arg, lt_id, IPOST_LAG_LAG_GROUPm,
            ...
            /* Write IPOST_LAG_LAG_BITMAPm. */
            for (px = 0; px < param->rh_member_cnt; px++) {
                local_port = 0;
                local_port_type = FALSE;
                if (param->unblk_rh_uc_system_port[px]) {
                    SHR_IF_ERR_VERBOSE_EXIT(bcm56780_a0_trunk_localport_get(
                                unit,
                                op_arg,
                                lt_id,
                                param->unblk_rh_uc_system_port[px],
                                &local_port, &local_port_type));
                    if (local_port_type == TRUE) {
                        BCMDRD_PBMP_PORT_ADD(new_trunk_bmp,
                                             local_port);
                             ...
                             uint32_t new_base)
{
    ...
    if (uc_cnt) {
        if (gtype == BCMCTH_TRUNK_GRP_TYPE_REGULAR) {
            ...
            LOG_INFO(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                   "rh_member_cnt %d rh_max_members %d. \n"),
                    param->rh_member_cnt, param->uc_max_members));
             ...
             * with LAG_SIZE equal to non zero uc_egr_cnt.
             */
            if (param->rh_member_cnt) {
                IPOST_LAG_LAG_GROUPm_LAG_SIZEf_SET(grp_buf, (uc_cnt - 1));
            } else {
                /*
                 * Need to delete HW info when rh_member_cnt is 0 and
                 * uc_nt is non zero.
                 * This case occurs when the member field
                 * RH_UC_MEMBER_PORT_SYSTEM are supplied with valid
                 * unicast systme ports but all the ports are
                 * disabled by setting RH_UC_EGR_BLOCK[x]=1
                 */
                IPOST_LAG_LAG_GROUPm_CLR(grp_buf);
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmcth_trunk_hw_write(unit,
    ...
    if (gtype == BCMCTH_TRUNK_GRP_TYPE_REGULAR) {
        ...
        for (px = 0; px < param->rh_member_cnt; px++) {
            ...
            local_port_type = FALSE;
            if (param->unblk_rh_uc_system_port[px]) {
                uc_port = param->unblk_rh_uc_system_port[px];
            }
            if (uc_port) {
                SHR_IF_ERR_VERBOSE_EXIT(bcm56780_a0_trunk_localport_get(
                            unit,
...
static int
bcm56780_a0_trunk_grp_lookup(int unit,
                             const bcmltd_op_arg_t *op_arg,
                             bcmltd_sid_t lt_id,
                             bcmcth_trunk_imm_grp_bk_t *grp_bk,
                             bcmcth_trunk_group_param_t *param)
{
    IPOST_LAG_LAG_GROUPm_t   grp_buf;
    uint32_t *dma_buf = NULL, *ent_buf = NULL;
    bcmdrd_sid_t pt_id = IPOST_LAG_LAG_MEMBERm;
    int  ent_sz, i;
    uint32_t dma_alloc_sz = 0;
    bcmcth_trunk_imm_group_t *grp;
    uint32_t trunk_id;
    uint32_t gtype;
    uint32_t val[1] = {0};
    ...
    gtype = param->gtype;
    param->rh_member_cnt = grp->rh_member_cnt;
    param->uc_max_members = grp->uc_max_members;
    param->rh_uc_random_seed = grp->rh_uc_random_seed;

    /* Read IPOST_LAG_LAG_GROUPm. */
    IPOST_LAG_LAG_GROUPm_CLR(grp_buf);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_trunk_hw_read(unit,
                              op_arg,
    ...
    base_ptr = IPOST_LAG_LAG_GROUPm_BASE_PTRf_GET(grp_buf);
    param->lb_mode = IPOST_LAG_LAG_GROUPm_LB_MODEf_GET(grp_buf);
    param->uc_cnt = IPOST_LAG_LAG_GROUPm_LAG_SIZEf_GET(grp_buf);
    if (param->uc_cnt != 0) {
        /* Counts from 0 - plus 1 for non zero group count. */
        param->uc_cnt += 1;
    }

    ent_sz = bcmdrd_pt_entry_wsize(unit, pt_id);
    dma_alloc_sz =  BCMDRD_WORDS2BYTES(ent_sz * param->uc_max_members);
    SHR_ALLOC(dma_buf, dma_alloc_sz, "bcmcthTrunkBcm56780GrplookupDmaBuf");
    SHR_NULL_CHECK(dma_buf, SHR_E_MEMORY);
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_trunk_hw_read_dma(unit,
                                  ...
                                  pt_id,
                                  base_ptr,
                                  param->uc_max_members,
                                  dma_buf));
    if (gtype == BCMCTH_TRUNK_GRP_TYPE_RESILIENT) {
        for (i = 0; i < param->uc_cnt; i++) {
            ent_buf = dma_buf + (i  * ent_sz);
            SHR_IF_ERR_VERBOSE_EXIT
               (bcmdrd_pt_field_get(unit, pt_id, ent_buf, SYSTEM_DEST_PORTf, val));
            param->rh_system_port[i] = val[0];
           SHR_IF_ERR_VERBOSE_EXIT
                (bcmdrd_pt_field_get(unit, pt_id, ent_buf, MEMBER_L2OIFf, val));
            param->rh_l2_oif[i] = val[0];
        }
    } else {
        for (i = 0; i < param->uc_cnt; i++) {
            ent_buf = dma_buf + (i  * ent_sz);
            SHR_IF_ERR_VERBOSE_EXIT
               (bcmdrd_pt_field_get(unit, pt_id, ent_buf, SYSTEM_DEST_PORTf, val));
            param->uc_system_port[i] = val[0];
           SHR_IF_ERR_VERBOSE_EXIT
                (bcmdrd_pt_field_get(unit, pt_id, ent_buf, MEMBER_L2OIFf, val));
            param->l2_oif[i] = val[0];
        }
    }
exit:
    if (dma_buf) {
        BCMCTH_TRUNK_FREE(dma_buf);
    }
    SHR_FUNC_EXIT();
}
...
exit:
    if ((rv != SHR_E_NONE) && (rv != SHR_E_EXISTS)) {
        if (lag_member_itbm_hdl) {
            shr_itbm_list_destroy(lag_member_itbm_hdl, 0, 0);
        }
    }
                              ...
                              uint32_t base_ptr)
{
    ...
    uint32_t gtype = BCMCTH_TRUNK_SYSTEM_GRP_TYPE_REGULAR;
    uint32_t cnt = 0;
    ...
    if (gtype == BCMCTH_TRUNK_SYSTEM_GRP_TYPE_RESILIENT) {
        /* Resilient group redistribution is per group size. */
        cnt = param->max_members;
    } else {
        cnt = param->cnt;
    }
    ...
    if (cnt > 0) {
        ...
        if (gtype == BCMCTH_TRUNK_SYSTEM_GRP_TYPE_REGULAR) {
    ...
    if (gtype == BCMCTH_TRUNK_SYSTEM_GRP_TYPE_REGULAR) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_trunk_hw_read(unit,
                                  ...
                                  trunk_id,
                                  &bitmap_buf));
        IPOST_LAG_SYSTEM_LAG_BITMAP_0m_BITMAPf_GET(
                                        bitmap_buf,
                                        new_trunk_bmp.w);
...
static int
bcm56780_a0_sys_trunk_grp_lookup(int unit,
                          const bcmltd_op_arg_t *op_arg,
                          bcmltd_sid_t lt_id,
                          bcmcth_trunk_sys_grp_bk_t *grp_bk,
                          bcmcth_trunk_sys_grp_param_t *param)
{
    IPOST_LAG_SYSTEM_LAG_GROUP_0m_t   grp_buf;
    uint32_t *dma_buf = NULL, *ent_buf = NULL;
    bcmdrd_sid_t pt_id = IPOST_LAG_SYSTEM_LAG_MEMBER_0m;
    IPOST_LAG_SYSTEM_LAG_CONFIG_0r_t cfg_buf;
    int  ent_sz, i;
    uint32_t dma_alloc_sz = 0;
    bcmcth_trunk_imm_sys_group_t *grp;
    ...
    gtype = param->gtype;
    param->max_members = grp->max_members;
    param->rh_random_seed = grp->rh_random_seed;

    /* Read IPOST_LAG_SYSTEM_LAG_GROUP_0m. */
    IPOST_LAG_SYSTEM_LAG_GROUP_0m_CLR(grp_buf);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_trunk_hw_read(unit,
                              op_arg,
    ...
    base_ptr = IPOST_LAG_SYSTEM_LAG_GROUP_0m_BASE_PTRf_GET(grp_buf);
    param->lb_mode = IPOST_LAG_SYSTEM_LAG_GROUP_0m_LB_MODEf_GET(grp_buf);
    param->cnt = IPOST_LAG_SYSTEM_LAG_GROUP_0m_SYSTEM_LAG_SIZEf_GET(grp_buf);
    if (param->cnt != 0) {
        /* Counts from 0 - plus 1 for non zero group count. */
        param->cnt += 1;
    }

    /* Lookup IPOST_LAG_SYSTEM_LAG_MEMBER_0m. */
    ent_sz = bcmdrd_pt_entry_wsize(unit, pt_id);
    dma_alloc_sz =  BCMDRD_WORDS2BYTES(ent_sz * param->max_members);
    SHR_ALLOC(dma_buf, dma_alloc_sz, "bcmcthTrunkSysBcm56780GrmaBuf");
    SHR_NULL_CHECK(dma_buf, SHR_E_MEMORY);
    ...
    if (gtype == BCMCTH_TRUNK_SYSTEM_GRP_TYPE_RESILIENT) {
        for (i = 0; i < param->cnt; i++) {
            ent_buf = dma_buf + (i  * ent_sz);
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmdrd_pt_field_get(unit, pt_id, ent_buf,
                                     LOCAL_DESTINATION_PORTf, val));
            param->rh_port[i] = val[0];
        }
    } else {
        for (i = 0; i < param->cnt; i++) {
            ent_buf = dma_buf + (i  * ent_sz);
    ...
    /* Lookup IPOST_LAG_SYSTEM_LAG_CONFIG_0r. */
    IPOST_LAG_SYSTEM_LAG_CONFIG_0r_CLR(cfg_buf);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_trunk_hw_read(unit,
                              op_arg,
    ...
    param->disable =
        IPOST_LAG_SYSTEM_LAG_CONFIG_0r_SYSTEM_LAG_RES_DISABLEf_GET(cfg_buf);
exit:
    if (dma_buf) {
        BCMCTH_TRUNK_FREE(dma_buf);
    }
...
bcm56780_a0_trunk_vp_get_hw_lb_mode(int unit, bcmcth_trunk_vp_lb_mode_t lb_mode,
    ...
    switch (lb_mode) {
        ...
        case BCMCTH_TRUNK_VP_LB_MODE_RESILIENT:
            *hw_lb_mode = 0;
            break;
    ...
    if (SHR_FUNC_ERR()) {
        ...
        if (grp_list_hdl) {
            ...
            BCMCTH_TRUNK_VP_TBL_ITBM_LIST(
                (BCMCTH_TRUNK_VP_TBL_PTR(unit, group,
                                         BCMCTH_TRUNK_VP_GRP_TYPE_WEIGHTED_0))) = NULL;
            shr_itbm_list_destroy(grp_list_hdl, 0, 0);
        }
        if (mem_list_hdl) {
            BCMCTH_TRUNK_VP_TBL_ITBM_LIST(
            ...
            BCMCTH_TRUNK_VP_TBL_ITBM_LIST(
                (BCMCTH_TRUNK_VP_TBL_PTR(unit, member,
                                         BCMCTH_TRUNK_VP_GRP_TYPE_WEIGHTED_0))) = NULL;
            shr_itbm_list_destroy(mem_list_hdl, 0, 0);
        }
    }
    SHR_FUNC_EXIT();
}
                                     ...
                                     bcmcth_trunk_vp_lt_entry_t *lt_entry)
{
    ...
    if (fid == ids->trunk_vp_trunk_vp_id) {
        ...
        /*
         * Reset the resilient hashing LB_MODE to reg_hash
         * for bcm56780_a0.
         * Reset of the LB_MODE for resilient hashing is not
         * needed when bcm56780_a0 has resilient hashing
         * load balancing mode support is added to TRUNK_VP lt.
         */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm56780_a0_trunk_vp_get_hw_lb_mode(unit,
              (uint32_t) fval, &tmp_val));
        lt_entry->lb_mode = tmp_val;
        BCMCTH_TRUNK_VP_LT_FIELD_SET(
...
/*!
 * \brief Get the member fields count for TRUNK LT.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
 */
static int
bcm56780_a0_trunk_grp_member_flds_cnt_get(int unit)
{
    return TRUNK_GRP_MEMBER_FLDS_CNT;
}

/*!
 * \brief Get the member fields count for TRUNK_SYSTEM LT.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
 */
static int
bcm56780_a0_sys_trunk_grp_member_flds_cnt_get(int unit)
{
    return TRUNK_SYS_GRP_MEMBER_FLDS_CNT;
}
...
static bcmcth_trunk_drv_t bcm56780_a0_cth_trunk_drv =  {
    ...
    .fn_imm_grp_lookup       = &bcm56780_a0_trunk_grp_lookup,
    .fn_imm_grp_member_flds_cnt
                             = &bcm56780_a0_trunk_grp_member_flds_cnt_get,
    .fn_sys_grp_add          = &bcm56780_a0_sys_trunk_grp_add,
    .fn_sys_grp_del          = &bcm56780_a0_sys_trunk_grp_del,
    .fn_sys_grp_member_flds_cnt
                             = &bcm56780_a0_sys_trunk_grp_member_flds_cnt_get,
    .fn_sys_grp_lookup       = &bcm56780_a0_sys_trunk_grp_lookup,
