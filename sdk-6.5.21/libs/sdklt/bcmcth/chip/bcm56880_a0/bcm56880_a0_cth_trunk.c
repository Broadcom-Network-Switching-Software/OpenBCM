...
static int
bcm56880_a0_trunk_grp_lookup(int unit,
                             const bcmltd_op_arg_t *op_arg,
                             bcmltd_sid_t lt_id,
                             bcmcth_trunk_imm_grp_bk_t *grp_bk,
                             bcmcth_trunk_group_param_t *param)
{
    ...
    SHR_ALLOC(dma_buf, dma_alloc_sz, "bcmcthTrunkBcm56880GrplookupDmaBuf");
    SHR_NULL_CHECK(dma_buf, SHR_E_MEMORY);
...
static int
bcm56880_a0_sys_trunk_grp_lookup(int unit,
                          const bcmltd_op_arg_t *op_arg,
                          bcmltd_sid_t lt_id,
                          bcmcth_trunk_sys_grp_bk_t *grp_bk,
                          bcmcth_trunk_sys_grp_param_t *param)
{
    ...
    SHR_ALLOC(dma_buf, dma_alloc_sz, "bcmcthTrunkSysBcm56880GrmaBuf");
    SHR_NULL_CHECK(dma_buf, SHR_E_MEMORY);
...
static int
bcm56880_a0_trunk_vp_get_hw_lb_mode(
                     int unit,
                     bcmcth_trunk_vp_lb_mode_t lb_mode,
                     uint32_t *hw_lb_mode) {
    SHR_FUNC_ENTER(unit);
    switch (lb_mode) {
...
static int
bcm56880_a0_trunk_vp_get_rh_lb_mode(
                          int unit,
                          bcmcth_trunk_vp_rh_size_t rh_size,
                          uint32_t *hw_lb_mode) {
    SHR_FUNC_ENTER(unit);
    switch (rh_size) {
                               ...
                               const bcmcth_trunk_vp_drv_var_t *vp_drv_var)
{
    ...
    /* Initialize LTD-SID values for TRUNK_VP LTs. */
                                   ...
                                   const bcmcth_trunk_drv_var_ids_t *ids)
{
    ...
    bcmcth_trunk_vp_grp_ltd_fields_t *wlt_fids = NULL;
    /* Overlay lt field attrs. */
    bcmcth_trunk_vp_lt_field_attrs_t *ogrp_attrs = NULL;
    bcmcth_trunk_vp_lt_field_attrs_t *wgrp_attrs = NULL;
    ...
    olt_fids->rh_member_cnt_fid = ids->trunk_vp_rh_member_cnt_id;
    olt_fids->rh_size_fid = ids->trunk_vp_rh_size_id;
    olt_fids->rh_rand_seed_fid = ids->trunk_vp_rh_rand_seed_id;
    ...
    */
    ...
    /*
    * Get resilient size min and max values from LRD field
    * data.
    */
    if (olt_fids->rh_size_fid != BCMCTH_TRUNK_INVALID) {
        fdata = &(map->field_data->field[olt_fids->rh_size_fid]);
        BCMCTH_TRUNK_VP_LT_MIN_RH_SIZE
            (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) = fdata->min->u32;
        BCMCTH_TRUNK_VP_LT_MAX_RH_SIZE
            (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) = fdata->max->u32;
        BCMCTH_TRUNK_VP_LT_MIN_LB_MODE
            (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) = fdata->min->u32;
        BCMCTH_TRUNK_VP_LT_MAX_LB_MODE
    ...
    /*
     * Get RH_MEMEBR_CNT logical table LRD field data pointer.
     * Initialize device member_cnt minimum and maximum values.
     */
    if (olt_fids->rh_member_cnt_fid != BCMCTH_TRUNK_INVALID) {
        fdata = &(map->field_data->field[olt_fids->rh_member_cnt_fid]);
        BCMCTH_TRUNK_VP_LT_MIN_RH_NMEMBERS
            (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) = fdata->min->u16;
        BCMCTH_TRUNK_VP_LT_MAX_RH_NMEMBERS
            (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) = fdata->max->u16;
    }
     ...
     * attribute values.
     */
    BCMCTH_TRUNK_VP_ALLOC(wgrp_attrs,
        sizeof(*wgrp_attrs), "bcmcthTrunkvpWOlayGrpAttrs");
    BCMCTH_TRUNK_VP_LT_FIELD_ATTRS_VOID_PTR
        (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_WEIGHTED_0) =
        (void *)wgrp_attrs;
    bcmcth_trunk_vp_lt_field_attrs_t_init(wgrp_attrs);

    /*
     * Initialize Group logical table field identifer values based on the
     * device TRUNK_VP mode.
     */
    BCMCTH_TRUNK_VP_ALLOC(wlt_fids,
        sizeof(*wlt_fids), "bcmcthTrunkVpWOlayGrpLtdFids");
    BCMCTH_TRUNK_VP_TBL_LTD_FIELDS
        (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_WEIGHTED_0) = (void *)wlt_fids;

    /* Initialize TRUNK_VP_WEIGHTED Group LTD fields information. */
    wlt_fids->trunk_vp_id_fid = ids->trunk_vp_weighted_trunk_vp_id;
    wlt_fids->member_cnt_fid = ids->trunk_vp_weighted_member_cnt_id;
    wlt_fids->weighted_size_fid = ids->trunk_vp_weighted_wt_size_id;

    /* Get TRUNK_VP ID min and max values from LRD field data. */
    if (wlt_fids->trunk_vp_id_fid != BCMCTH_TRUNK_INVALID) {
        fdata = &(wmap->field_data->field[wlt_fids->trunk_vp_id_fid]);
        BCMCTH_TRUNK_VP_LT_MIN_TRUNK_VP_ID
    ...
    * data.
    */
    if (wlt_fids->weighted_size_fid != BCMCTH_TRUNK_INVALID) {
        fdata = &(wmap->field_data->field[wlt_fids->weighted_size_fid]);
    ...
    /*
     * Get MEMEBR_CNT logical table LRD field data pointer.
     * Initialize device member_cnt minimum and maximum values.
     */
    if (wlt_fids->member_cnt_fid != BCMCTH_TRUNK_INVALID) {
        fdata = &(wmap->field_data->field[wlt_fids->member_cnt_fid]);
        BCMCTH_TRUNK_VP_LT_MIN_NMEMBERS
            (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_WEIGHTED_0) = fdata->min->u16;
        BCMCTH_TRUNK_VP_LT_MAX_NMEMBERS
            (unit, group, BCMCTH_TRUNK_VP_GRP_TYPE_WEIGHTED_0) = fdata->max->u16;
                                 ...
                                 const bcmcth_trunk_drv_var_ids_t *ids)
{
    ...
    const bcmlrd_map_t *wmap = NULL; /* LT map. */
    int rv = SHR_E_INTERNAL; /* Return value variable. */
    int rv1 = SHR_E_INTERNAL; /* Return value variable. */
    ...
    /* Get LRD mapping information for TRUNK_VP_WEIGHTED LT. */
...
/*!
 * \brief Initialize Trident4 device specfic TRUNK_VP tables range information.
 *
 ...
 * \param [in] vp_drv_var Device variant info.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_MEMORY Table entries memory allocation failed.
 */
static int
bcm56880_a0_trunk_vp_db_init(int unit,
                             bool warm,
                             const bcmcth_trunk_vp_drv_var_t *vp_drv_var)
{
    ...
    bcmcth_trunk_vp_grp_ha_blk_t *grp_array_ptr = NULL;
    bcmcth_trunk_vp_grp_rh_ha_blk_t *rh_grp_array_ptr = NULL;
    ...
    if (ids->trunk_vp_ltid != BCMCTH_TRUNK_VP_INVALID) {
        ...
        /*
         * Initialize resilient TRUNK_VP groups member table entries array
         * pointer.
         */
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, member,
             BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY))->ent_arr = tbl_ptr->ent_arr;
        ...
        /* Initialize resilient TRUNK_VP groups array pointer. */
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, group,
             BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY))->ent_arr = tbl_ptr->ent_arr;
        ...
        tot_ha_mem += ha_alloc_sz;
         ...
         SHR_IF_ERR_EXIT
            (bcmcth_trunk_vp_group_rh_attr_ha_alloc(
                                         ...
                                         warm,
                                         tbl_size,
                                         BCMCTH_TRUNK_VP_GRP_RH_SUB_COMP_ID,
                                         (blk_sig |
                                         BCMCTH_TRUNK_VP_GRP_RH_SUB_COMP_ID),
                                         "bcmcthTrunkVpGroupRhAttrHaInfo",
                                         &rh_grp_array_ptr,
                                         &ha_alloc_sz));
        BCMCTH_TRUNK_VP_GRP_RH_INFO_PTR(unit) = (void *) rh_grp_array_ptr->array;
        tot_ha_mem += ha_alloc_sz;

    } else {
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, member,
             BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY))->ent_arr = NULL;
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, member,
             BCMCTH_TRUNK_VP_GRP_TYPE_WEIGHTED_0))->ent_arr = NULL;
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, member,
             BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY))->ent_arr = NULL;
        (BCMCTH_TRUNK_VP_TBL_PTR
        ...
        BCMCTH_TRUNK_VP_GRP_RH_INFO_PTR(unit) = NULL;
    }
...
exit:
    if (SHR_FUNC_ERR()) {
        ...
        if (BCMCTH_TRUNK_VP_GRP_RH_INFO_PTR(unit)) {
            rh_grp_array_ptr = (bcmcth_trunk_vp_grp_rh_ha_blk_t *)
                ((uint8_t *)BCMCTH_TRUNK_VP_GRP_RH_INFO_PTR(unit)
                 - sizeof(bcmcth_trunk_vp_grp_rh_ha_blk_t));
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
                                    "\n\t free resilient grp HA addr=0x%p\n"),
 ...
 */
static int
bcm56880_a0_trunk_vp_abort_db_init(int unit,
                                   bool warm,
                                   const bcmcth_trunk_vp_drv_var_t *vp_drv_var)
{
    ...
    if (ids->trunk_vp_ltid != BCMCTH_TRUNK_VP_INVALID) {
        ...
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, member,
             BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY))->ent_bk_arr = tbl_ptr->ent_arr;
        ...
        /*
         * Allocate memory for active TRUNK_VP_RSEILIENT
         * group LT entries array elements.
         */
         SHR_IF_ERR_EXIT
            (bcmcth_trunk_vp_group_rh_attr_ha_alloc(
                                         unit,
                                         warm,
                                         tbl_size,
                                         BCMCTH_TRUNK_VP_GRP_RH_BK_SUB_COMP_ID,
                                         (blk_sig |
                                         BCMCTH_TRUNK_VP_GRP_RH_BK_SUB_COMP_ID),
                                         "bcmcthTrunkVpGroupRhAttrBkHaInfo",
                                         &rh_grp_array_ptr,
                                         &ha_alloc_sz));
        BCMCTH_TRUNK_VP_GRP_RH_INFO_BK_PTR(unit) = (void *) rh_grp_array_ptr->array;
        tot_ha_mem += ha_alloc_sz;

    } else {
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, member,
             BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY))->ent_bk_arr = NULL;
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, member,
             BCMCTH_TRUNK_VP_GRP_TYPE_WEIGHTED_0))->ent_bk_arr = NULL;
        (BCMCTH_TRUNK_VP_TBL_PTR
            (unit, group,
             BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY))->ent_bk_arr = NULL;
        (BCMCTH_TRUNK_VP_TBL_PTR
        ...
        BCMCTH_TRUNK_VP_GRP_RH_INFO_BK_PTR(unit) = NULL;
    }
...
exit:
    if (SHR_FUNC_ERR()) {
        ...
        if (BCMCTH_TRUNK_VP_GRP_RH_INFO_BK_PTR(unit)) {
            rh_grp_array_ptr = (bcmcth_trunk_vp_grp_rh_ha_blk_t *)
                ((uint8_t *)BCMCTH_TRUNK_VP_GRP_RH_INFO_BK_PTR(unit)
                 - sizeof(bcmcth_trunk_vp_grp_rh_ha_blk_t));
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
                         "\n\t free bk resilient grp HA addr=0x%p\n"),
                         (void*)rh_grp_array_ptr));
            shr_ha_mem_free(unit, rh_grp_array_ptr);
            BCMCTH_TRUNK_VP_GRP_RH_INFO_BK_PTR(unit) = NULL;
        }
 ...
 */
static int
bcm56880_a0_trunk_vp_entries_init(int unit,
                                  bool warm,
                                  const bcmcth_trunk_vp_drv_var_t *vp_drv_var)
{
    SHR_FUNC_ENTER(unit);
     ...
     */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56880_a0_trunk_vp_db_init(unit,
                                      warm,
                                      vp_drv_var));
    ...
    if (BCMCTH_TRUNK_VP_ATOMIC_TRANS(unit)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcm56880_a0_trunk_vp_abort_db_init(unit,
                                                warm,
                                                vp_drv_var));
    }
exit:
                               ...
                               const bcmcth_trunk_drv_var_t *drv_var)
{
     ...
     */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56880_a0_trunk_vp_entries_init(unit, warm, drv_var->trunk_vp_var));
...
bcm56880_a0_trunk_vp_olay_grps_flds_cleanup(int unit)
{
    ...
    /*
     * Free memory allocated for storing TRUNK_VP_WEIGHTED LT logical fields
     * attributes information.
     */
    ogrp_attrs = (bcmcth_trunk_vp_lt_field_attrs_t *)
                      BCMCTH_TRUNK_VP_LT_FIELD_ATTRS_VOID_PTR
...
bcm56880_a0_trunk_vp_itbm_groups_cleanup(int unit)
{
    ...
    for (gtype = BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY;
         gtype < BCMCTH_TRUNK_VP_GRP_TYPE_COUNT;
         gtype++) {
                                 ...
                                 const bcmcth_trunk_vp_drv_var_t *vp_drv_var)
{
    ...
    /* Get LT entry data pointer. */
    lt_ent = (bcmcth_trunk_vp_lt_entry_t *)info;
    if ((lt_ent->grp_type == BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) &&
        (BCMCTH_TRUNK_VP_GRP_IS_RH(lt_ent->lb_mode))) {
        /* Resilient member info. */
        SHR_NULL_CHECK(vp_drv_var->rmember0_info, SHR_E_INTERNAL);
        member_info = vp_drv_var->rmember0_info;
    } else {
        /* Regular hashing member info. */
        SHR_NULL_CHECK(vp_drv_var->member0_info, SHR_E_INTERNAL);
    ...
    if ((lt_ent->grp_type == BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) &&
        (BCMCTH_TRUNK_VP_GRP_IS_RH(lt_ent->lb_mode))) {
        /* In RH mode max_members is the number of unique paths */
        member_cnt = lt_ent->max_members;
    } else {
        member_cnt = lt_ent->member_cnt;
    }
    ...
    if (lt_ent->event == BCMIMM_ENTRY_UPDATE) {
        ...
        /* Initialize op_info structure member values. */
        if ((!BCMCTH_TRUNK_VP_GRP_IS_RH(lt_ent->lb_mode))
             && (member_cnt)) {
            /*
            ...
            if (overlap_mem) {
                ...
                op_info.drd_sid = BCMCTH_TRUNK_VP_TBL_DRD_SID(unit, member2, gtype);
                op_info.ecount = member_cnt - op_info.ecount;
                op_info.dyn_info.index = 0;
                SHR_IF_ERR_EXIT
    ...
    for (arr_idx = 0; arr_idx < member_cnt; arr_idx++) {
    ...
    /*
     * If member_cnt or max_members value is non-zero, decrement it by '1'
     * as hardware count field value is zero base.
     */
    if (BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY == gtype) {
        if (BCMCTH_TRUNK_VP_GRP_IS_RH(lt_ent->lb_mode)) {
            if (lt_ent->rh_member_cnt) {
                if ((lt_ent->rh_size == BCMCTH_TRUNK_VP_RH_SIZE_64) ||
                    (lt_ent->rh_size == BCMCTH_TRUNK_VP_RH_SIZE_128)) {
                    hw_npaths = (uint32_t) lt_ent->max_members - 1;
                }
            }
        } else {
                                 ...
                                 const bcmcth_trunk_vp_drv_var_t *vp_drv_var)
{
   ...
   if ((lt_ent->grp_type == BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) &&
    ...
    for (arr_idx = 0; arr_idx < member_cnt;
         arr_idx++, op_info.dyn_info.index++) {
        int i;
        sal_memset(pt_entry_buff, 0,
                   BCMCTH_TRUNK_VP_TBL_ENT_SIZE_MAX(unit) * sizeof(uint32_t));
                                     ...
                                     bcmcth_trunk_vp_lt_entry_t *lt_entry)
{
    ...
    uint32_t i;
    const bcmcth_trunk_vp_member_info_t *member_info;
    const bcmcth_trunk_vp_member_info_t *rmember_info;
    const bcmcth_trunk_drv_var_ids_t *ids;

    SHR_FUNC_ENTER(unit);

    member_info = vp_drv_var->member0_info;
    rmember_info = vp_drv_var->rmember0_info;
    ids = vp_drv_var->ids;
    ...
    SHR_NULL_CHECK(rmember_info, SHR_E_INTERNAL);
    ...
    } else if (fid == ids->trunk_vp_rh_size_id) {
        lt_entry->rh_size = (uint32_t) fval;
        BCMCTH_TRUNK_VP_LT_FIELD_SET(
        lt_entry->fbmp, BCMCTH_TRUNK_VP_LT_FIELD_RH_SIZE);
        /*
         * If the group is resilient only then rewrite the
         * max_members with the RH_SIZE to  member count,
         * if RH_SIZE is supplied in LT operation or while
         * reading the previour LT entry cur_lt_entry,
         * For resilient hashing with out the RH_SIZE value
         * supplied in LT operation, read the default value of
         * RH_SIZE and convert to the member count for max_members.
         * in lt entry initialization.
         */
        if (BCMCTH_TRUNK_VP_GRP_IS_RH(lt_entry->lb_mode)) {
            BCMCTH_TRUNK_VP_LT_FIELD_SET(
            lt_entry->fbmp, BCMCTH_TRUNK_VP_LT_FIELD_MAX_MEMBERS);
            lt_entry->max_members =
            bcmcth_trunk_vp_rh_size_convert_to_member_cnt(fval);
        }
    } else if (fid == ids->trunk_vp_rh_member_cnt_id) {
        BCMCTH_TRUNK_VP_LT_FIELD_SET
            (lt_entry->fbmp, BCMCTH_TRUNK_VP_LT_FIELD_RH_MEMBER_CNT);
        lt_entry->rh_member_cnt = (uint32_t) fval;
    } else if (fid == ids->trunk_vp_rh_rand_seed_id) {
        BCMCTH_TRUNK_VP_LT_FIELD_SET
            (lt_entry->fbmp, BCMCTH_TRUNK_VP_LT_FIELD_RH_RANDOM_SEED);
        lt_entry->rh_rand_seed = (uint32_t) fval;
    } else {
       if (BCMCTH_TRUNK_VP_GRP_IS_RH(lt_entry->lb_mode)) {
            for (i = 0 ; i < rmember_info->flds_count; i++) {
                if (fid == vp_drv_var->rflds_mem0[i].grp_fld) {
                                     ...
                                     bcmcth_trunk_vp_lt_entry_t *lt_entry)
{
    ...
    SHR_NULL_CHECK(wmember_info, SHR_E_INTERNAL);
    fid = in->id;
    fval = in->data;
    idx = in->idx;
    ids = vp_drv_var->ids;
    ...
    if (fid == ids->trunk_vp_weighted_trunk_vp_id) {
        ...
        if (match == FALSE) {
           LOG_VERBOSE(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                  "TRUNK_VP_WEIGHTED group %d not matching member fields.\n"),
...
/*!
 * \brief TRUNK_VP/TRUNK_VP_WEIGHTED LT entry
 *        fields fill routine.
 *
 * Parse TRUNK_VP logical table entry logical fields
 * and store the data in lt_entry structure.
...
/*!
 * \brief TRUNK_VP/TRUNK_VP_WEIGHTED LT entry read from hardware.
 *
 * Read the trunk vp group information from hardware
 * and store the data in lt_entry structure.
 *
 * \param [in]  unit Unit number.
 * \param [out] info LT entry.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_PARAM Invalid input parameter.
 * \retval !SHR_E_NONE Failed to convert \c in to \c lt_entry.
 */
static int
bcm56880_a0_trunk_vp_grp_dma_get(int unit,
                             void *info)
{
    bcmcth_trunk_vp_grp_type_t gtype = BCMCTH_TRUNK_VP_GRP_TYPE_COUNT; /* ECMP Group type. */
    bcmcth_trunk_vp_lt_entry_t *lt_ent = NULL;    /* LT entry data pointer. */
    bcmcth_trunk_vp_pt_op_info_t op_info;         /* Physical Table operation info. */
    uint32_t dma_alloc_sz = 0;            /* Local DMA buffer alloc size. */
    uint32_t *pt_entry_buff = NULL;       /* Local entry buffer poiner. */
    ...
    int i, overlap_mem = 0;
    const bcmcth_trunk_vp_member_info_t *member_info;
    const bcmcth_trunk_drv_var_t *drv_var = bcmcth_trunk_drv_var_get(unit);
    ...
    BCMCTH_TRUNK_VP_ALLOC(pt_entry_buff,
                  BCMCTH_TRUNK_VP_TBL_ENT_SIZE_MAX(unit) * sizeof(uint32_t),
                  "bcmcthTrunkVpBcm56880GrpDmaGetPtEntBuf");

    lt_ent->grp_type = BCMCTH_TRUNK_VP_GRP_TYPE(unit, trunk_vp_id);
    lt_ent->member_cnt = BCMCTH_TRUNK_VP_GRP_MEMBER_CNT(unit, trunk_vp_id);
    lt_ent->max_members = BCMCTH_TRUNK_VP_GRP_MAX_MEMBERS(unit, trunk_vp_id);
    lt_ent->mstart_idx = BCMCTH_TRUNK_VP_GRP_MEMB_TBL_START_IDX(unit, trunk_vp_id);
    lt_ent->lb_mode = BCMCTH_TRUNK_VP_GRP_LB_MODE(unit, trunk_vp_id);

    if ((lt_ent->grp_type == BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) &&
        (lt_ent->lb_mode == BCMCTH_TRUNK_VP_LB_MODE_RESILIENT)) {
        rh_entries_cnt = BCMCTH_TRUNK_VP_GRP_RH_ENTRIES_CNT(
                                         unit,
                                         lt_ent->trunk_vp_id);
        lt_ent->rh_size = bcmcth_trunk_vp_member_cnt_convert_to_rh_size(
                             rh_entries_cnt);
        lt_ent->rh_member_cnt = BCMCTH_TRUNK_VP_GRP_MEMBER_CNT(unit, trunk_vp_id);
        lt_ent->rh_rand_seed = BCMCTH_TRUNK_VP_GRP_RH_RAND_SEED(unit, trunk_vp_id);

    }
    lt_ent->glt_sid = BCMCTH_TRUNK_VP_GRP_LT_SID(unit, trunk_vp_id);

    dma_alloc_sz = (BCMCTH_TRUNK_VP_TBL_ENT_SIZE
                        (unit, member, gtype) * sizeof(uint32_t) *
                         lt_ent->max_members);
    BCMCTH_TRUNK_VP_ALLOC(dma_buf, dma_alloc_sz, "bcmcthTrunkVpBcm56880GrpDmaInsPtDmaBuf");
    ...
    if (lt_ent->mstart_idx > BCMCTH_TRUNK_VP_TBL_IDX_MAX(unit, member, gtype)) {
        op_info.drd_sid = BCMCTH_TRUNK_VP_TBL_DRD_SID(unit, member2, gtype);
        op_info.dyn_info.index = lt_ent->mstart_idx -
             (BCMCTH_TRUNK_VP_TBL_IDX_MAX(unit, member, gtype) + 1);
    } else if ((int)(lt_ent->mstart_idx + lt_ent->max_members - 1)
               > BCMCTH_TRUNK_VP_TBL_IDX_MAX(unit, member, gtype)) {
            op_info.ecount = BCMCTH_TRUNK_VP_TBL_IDX_MAX(unit, member, gtype)
                             - lt_ent->mstart_idx + 1;
            op_info.wsize = (BCMCTH_TRUNK_VP_TBL_ENT_SIZE
    ...
    for (arr_idx = 0; arr_idx < lt_ent->max_members; arr_idx++) {
        /* Set member field values. */
        for (i = 0; i < (member_info->flds_count); ++i) {
            minbit = member_info->flds[i].start_bit;
            maxbit = member_info->flds[i].end_bit;

            shr_bitop_range_copy((SHR_BITDCL *)val,
                                  0,
                                  &dma_buf[arr_idx * ent_size],
                                  minbit,
                                  maxbit - minbit + 1);
            if ((lt_ent->grp_type == BCMCTH_TRUNK_VP_GRP_TYPE_OVERLAY) &&
                (lt_ent->lb_mode == BCMCTH_TRUNK_VP_LB_MODE_RESILIENT)) {
 ...
 */
static int
bcm56880_a0_trunk_grp_member_flds_cnt_get(int unit)
{
 ...
 */
static int
bcm56880_a0_sys_trunk_grp_member_flds_cnt_get(int unit)
{
...
static bcmcth_trunk_drv_t bcm56880_a0_cth_trunk_drv =  {
    ...
    .fn_imm_grp_lookup       = &bcm56880_a0_trunk_grp_lookup,
    .fn_imm_grp_member_flds_cnt
                             = &bcm56880_a0_trunk_grp_member_flds_cnt_get,
    .fn_sys_grp_add          = &bcm56880_a0_sys_trunk_grp_add,
    .fn_sys_grp_del          = &bcm56880_a0_sys_trunk_grp_del,
    .fn_sys_grp_member_flds_cnt
                             = &bcm56880_a0_sys_trunk_grp_member_flds_cnt_get,
    .fn_sys_grp_lookup       = &bcm56880_a0_sys_trunk_grp_lookup,
    ...
    .grp_get                 = &bcm56880_a0_trunk_vp_grp_dma_get,
};
