        ...
        case BCMCTH_MIRROR_ENCAP_BASIC:
            if (entry->encap_mode ==
                ...
                fval = 0xc2;
                       ...
                       BCM56880_A0_LRD_MIRROR_ENCAP_MODE_T_T_ENCAP_NONE) {
                /* non-encap flow. */
                fval = 0x0;
            } else {
                SHR_ERR_EXIT(SHR_E_UNAVAIL);
...
static int
bcm56880_a0_cth_mirror_encap_0_prof_del(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
...
static int
bcm56880_a0_cth_mirror_encap_0_prof_set(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
    bool ipv6 = FALSE, truncate = FALSE;
 ...
 */
    ...
    if (len_offset) {
        ...
        if (!ipv6) {
            ...
            /*
             * ENCAP Buffer is written from MSB bit.
             * Compute iphdr_start from len_offset.
             * len_offset is byte ofset of LENGHT field in Ipv4 header.
             * Get to start of header (-2).
             */

            iphdr_end_lsb_offset = len_offset - 2;
            /*
             * IPHEADER required for checksum is 20 bytes.
             * Compute last byte required for checksum (+20 -1)
             */
            iphdr_end_lsb_offset += 20 - 1;
            /*
             * Get the corresponding LSB byte number of end bit.
             * ENCAP buffer is 80 bytes (lsb offset = 79 - msb_offset).
             */
            iphdr_end_lsb_offset = 79 - iphdr_end_lsb_offset;

            if (iphdr_end_lsb_offset < 0 || iphdr_end_lsb_offset > 60) {
                LOG_WARN(BSL_LOG_MODULE,
                         (BSL_META_U(unit,
                                     "iphdr lsb offset out of range %d \n"),
                          iphdr_end_lsb_offset));
                SHR_ERR_EXIT(SHR_E_INTERNAL);
            }

            shr_bitop_range_set(mask, iphdr_end_lsb_offset, 20);

            LOG_DEBUG(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
                                  "iphdr_end_lsb_offset=%d"
                                  "mask = %x %x %x\n"),
                       iphdr_end_lsb_offset, mask[0], mask[1], mask[2]));
    ...
    pt_fid = MIRROR_ENCAP_TRUNCATE_MODEf;
    fval = entry->truncate_action;
    SHR_IF_ERR_EXIT(
        bcmdrd_pt_field_set(unit, pt_id_3, p_profile_3_hdr,
                            pt_fid, (uint32_t *) (&fval)));

    /* mirror truncate ptr */
    pt_fid = MIRROR_ENCAP_TRUNCATE_PTRf;
    ...
    bool enable)
{
    ...
    if (enable) {
        ...
        if (basic || erspan ||sflow || vxlan || psamp || mirr_on_drop) {
            ...
            SHR_IF_ERR_EXIT
                (bcm56880_a0_cth_mirror_encap_0_prof_set(unit, entry));
        }
        ...
        SHR_IF_ERR_EXIT
            (bcm56880_a0_cth_mirror_encap_0_prof_del(unit, entry));
