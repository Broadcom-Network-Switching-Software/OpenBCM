...
cth_trunk_sysm_comp_config(shr_sysm_categories_t inst_cat,
                         ...
                         uint32_t *blocking_component)
{
    ...
    if (SHR_FAILURE(bcmcth_trunk_attach(unit, warm))) {
 ...
 */
static shr_sysm_rv_t
cth_trunk_sysm_shutdown(shr_sysm_categories_t inst_cat,
                      int unit, void *comp_data, bool warm,
                      uint32_t *blocking_component)
{
    if (SHR_FAILURE(bcmcth_trunk_detach(unit, warm))) {
        return SHR_SYSM_RV_ERROR;
    }
 ...
 */
static shr_sysm_cb_func_set_t cth_trunk_sysm_handler = {

    /*! Component configuration. */
    .comp_config = cth_trunk_sysm_comp_config,

    /*! Shutdown. */
    .shutdown    = cth_trunk_sysm_shutdown
};
