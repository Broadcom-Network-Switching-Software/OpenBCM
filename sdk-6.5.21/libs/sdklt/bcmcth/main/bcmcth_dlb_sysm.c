...
cth_dlb_sysm_comp_config(shr_sysm_categories_t inst_cat,
                         ...
                         uint32_t *blocking_component)
{
    ...
    if (SHR_FAILURE(bcmcth_dlb_attach(unit, warm))) {
 ...
 */
static shr_sysm_rv_t
cth_dlb_sysm_shutdown(shr_sysm_categories_t inst_cat,
                      int unit, void *comp_data, bool warm,
                      uint32_t *blocking_component)
{
    if (SHR_FAILURE(bcmcth_dlb_detach(unit))) {
        return SHR_SYSM_RV_ERROR;
    }
 ...
 */
static shr_sysm_cb_func_set_t cth_dlb_sysm_handler = {

    /*! Component configuration. */
    .comp_config = cth_dlb_sysm_comp_config,

    /*! Shutdown. */
    .shutdown    = cth_dlb_sysm_shutdown
};
...
bcmcth_dlb_start(void)
{
    ...
    SHR_IF_ERR_EXIT
        (shr_sysm_register(BCMMGMT_DLB_COMP_ID,
                           SHR_SYSM_CAT_UNIT,
                           &cth_dlb_sysm_handler,
                           NULL));
exit:
    SHR_FUNC_EXIT();
}
