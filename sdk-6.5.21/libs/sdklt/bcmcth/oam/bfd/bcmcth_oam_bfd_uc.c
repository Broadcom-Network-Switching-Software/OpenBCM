...
#define IP_ENCAP_TYPE(_type_) LTD_ENUM(TNL_L2_VXLAN_ENDPOINT_IP_ENCAP_TYPE, _type_)
...
#define VXLAN_HDR_VNID_OFFSET (4)
...
bfd_uc_msg_rxpmd_flex_format_set(int unit)
{
    ...
    for (profile = 0; profile < 128; profile++) {
        for (i = 0; i < num_fids; i++) {
            ...
            rv = bcmpkt_flexhdr_field_name_get(variant, hid, fids[i], &fname);
            SHR_IF_ERR_EXIT(rv);
...
static uint16_t
bfd_endpoint_ipv4_lkey_etype_get(void *type)
{
    return SHR_L2_ETYPE_IPV4;
}
...
static uint16_t
bfd_endpoint_ipv6_lkey_etype_get(void *type)
{
    return SHR_L2_ETYPE_IPV6;
}
...
static uint16_t
bfd_endpoint_tnl_ipv4_lkey_etype_get(void *type)
{
    return SHR_L2_ETYPE_IPV4;
}
...
static uint16_t
bfd_endpoint_tnl_ipv6_lkey_etype_get(void *type)
{
    return SHR_L2_ETYPE_IPV6;
}
...
static uint16_t
bfd_endpoint_tnl_mpls_lkey_etype_get(void *type)
{
    return SHR_L2_ETYPE_MPLS_UNICAST;
}

/*******************************************************************************
 * Tunnel VxLAN endpoint type specific functions.
 */
static uint16_t
bfd_endpoint_tnl_l2_vxlan_encap_len_get(void *type)
{
    bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t *ep;
    uint16_t length = 0;

    ep = (bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t *) type;

    length += SHR_MAC_ADDR_LEN * 2;

    length += SHR_VLAN_HDR_LEN * ep->overlay_eth.num_vlans;

    length += SHR_L2_ETYPE_LEN;

    if (ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV4_OVERLAY_IPV4)) {
        length += SHR_IPV4_HEADER_LENGTH;

        length += SHR_IPV4_HEADER_LENGTH;
    } else if (ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV4_OVERLAY_IPV6)) {
        length += SHR_IPV4_HEADER_LENGTH;

        length += SHR_IPV6_HEADER_LENGTH;
    } else if (ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV6_OVERLAY_IPV4)) {
        length += SHR_IPV6_HEADER_LENGTH;
    ...
    length += (SHR_UDP_HDR_LEN * 2);

    length += SHR_VXLAN_HDR_LEN;

    return length;
}

static void
bfd_endpoint_tnl_l2_vxlan_encap_pack(void *type, uint16_t udp_src_port,
                                     uint16_t *length, uint8_t *encap,
                                     uint8_t **lkey, uint16_t *lkey_length,
                                     uint16_t bfd_pkt_length)
{
    ...
    shr_util_pack_ipv6_header_t ipv6;
    shr_util_pack_udp_header_t udp;
    shr_util_pack_vxlan_header_t vxlan;
    int udp_length = 0;
    uint8_t *cur_ptr = encap;
    uint8_t *ptr_tmp = encap;
    ...
    if ((ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV4_OVERLAY_IPV4)) ||
        (ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV4_OVERLAY_IPV6))) {
        /* Outer IPv4 header. */
        sal_memset(&ipv4, 0, sizeof(ipv4));
        ipv4.version = SHR_IPV4_VERSION;
        ipv4.h_length = SHR_IPV4_HDR_WLEN;
        ipv4.dscp = ep->underlay_ipv4.tos;
        ipv4.ttl = ep->underlay_ipv4.ttl;

        if (ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV4_OVERLAY_IPV4)) {
            udp_length = SHR_IPV4_HEADER_LENGTH;
        } else {
            udp_length = SHR_IPV6_HEADER_LENGTH;
        }
        udp_length += (bfd_pkt_length + (SHR_UDP_HDR_LEN * 2) +
                       SHR_VXLAN_HDR_LEN + (SHR_MAC_ADDR_LEN * 2) +
                       SHR_VLAN_HDR_LEN * ep->overlay_eth.num_vlans +
                       SHR_L2_ETYPE_LEN + SHR_IPV4_HEADER_LENGTH);

        ipv4.length = udp_length;
        ipv4.protocol = SHR_IP_PROTO_UDP;
        ipv4.src = ep->underlay_ipv4.src_ip;
        ipv4.dst = ep->underlay_ipv4.dst_ip;
        ptr_tmp = cur_ptr;
        cur_ptr = shr_util_pack_ipv4_header_pack(cur_ptr, &ipv4);
        ...
        sal_memset(&ipv6, 0, sizeof(ipv6));
        ipv6.version = SHR_IPV6_VERSION;
        ipv6.t_class = ep->underlay_ipv6.traffic_class;
        ipv6.f_label = 0;
        ipv6.next_header = SHR_IP_PROTO_UDP;
        ipv6.hop_limit = ep->underlay_ipv6.hop_limit;
        if (ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV6_OVERLAY_IPV4)) {
            udp_length = SHR_IPV4_HEADER_LENGTH;
        } else {
            udp_length = SHR_IPV6_HEADER_LENGTH;
        }
        ...
        sal_memcpy(ipv6.src, ep->underlay_ipv6.src_ip, sizeof(shr_ip6_t));
        sal_memcpy(ipv6.dst, ep->underlay_ipv6.dst_ip, sizeof(shr_ip6_t));
        cur_ptr = shr_util_pack_ipv6_header_pack(cur_ptr, &ipv6);
    }
    ...
    ptr_tmp = cur_ptr;
    sal_memset(&udp, 0, sizeof(udp));
    udp.src = ep->underlay_udp.udp_src_port;
    udp.dst = ep->underlay_udp.udp_dst_port;
     ...
     */
    *lkey = cur_ptr + VXLAN_HDR_VNID_OFFSET;
    *lkey_length = SHR_VXLAN_VNID_LEN;
     ...
     */
    vxlan.flags = 0x8;
    vxlan.vnid  = ep->vxlan_vnid;
    cur_ptr = shr_util_pack_vxlan_header_pack(cur_ptr, &vxlan);

    /* Inner L2 header */
    sal_memset(&l2, 0, sizeof(l2));
    sal_memcpy(l2.dst_mac, ep->overlay_eth.dst_mac, sizeof(shr_mac_t));
    sal_memcpy(l2.src_mac, ep->overlay_eth.src_mac, sizeof(shr_mac_t));

    if (ep->overlay_eth.num_vlans != 0) {
        sal_memcpy(&l2.outer_vlan_tag, &ep->overlay_eth.outer_vlan,
                   sizeof(l2.outer_vlan_tag));

        if (ep->overlay_eth.num_vlans == 2) {
            sal_memcpy(&l2.inner_vlan_tag, &ep->overlay_eth.inner_vlan,
                       sizeof(l2.inner_vlan_tag));
        }
    }
    if ((ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV4_OVERLAY_IPV4)) ||
        (ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV6_OVERLAY_IPV4))) {
    ...
    if ((ep->ip_encap_type == IP_ENCAP_TYPE(UNDERLAY_IPV4_OVERLAY_IPV4)) ||
        ...
        ipv4.version = SHR_IPV4_VERSION;
        ipv4.h_length = SHR_IPV4_HDR_WLEN;
        ipv4.dscp = ep->overlay_ipv4.tos;
        ipv4.ttl = 1;
        ipv4.protocol = SHR_IP_PROTO_UDP;
        ipv4.src = ep->overlay_ipv4.src_ip;
        ipv4.dst = ep->overlay_ipv4.dst_ip;
        ipv4.length = bfd_pkt_length + SHR_UDP_HDR_LEN + SHR_IPV4_HEADER_LENGTH;
        ptr_tmp = cur_ptr;
        cur_ptr = shr_util_pack_ipv4_header_pack(cur_ptr, &ipv4);
        ipv4.sum = shr_ip_chksum(SHR_IPV4_HEADER_LENGTH, ptr_tmp);
        ...
        sal_memset(&ipv6, 0, sizeof(ipv6));
        ipv6.version = SHR_IPV6_VERSION;
        ipv6.t_class = ep->overlay_ipv6.traffic_class;
        ipv6.f_label = 0;
        ipv6.next_header = SHR_IP_PROTO_UDP;
        ipv6.hop_limit = 1;
        ipv6.p_length = bfd_pkt_length + SHR_UDP_HDR_LEN;
        sal_memcpy(ipv6.src, ep->overlay_ipv6.src_ip, sizeof(shr_ip6_t));
        sal_memcpy(ipv6.dst, ep->overlay_ipv6.dst_ip, sizeof(shr_ip6_t));
        cur_ptr = shr_util_pack_ipv6_header_pack(cur_ptr, &ipv6);
    }
...
static mcs_bfd_encap_types_t
bfd_endpoint_tnl_l2_vxlan_mcs_encap_type_get(void *type)
{
    bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t *ep;
...
static void
bfd_endpoint_tnl_l2_vxlan_msg_convert(void *type, mcs_bfd_msg_ctrl_ep_set_t *msg)
{
    bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t *ep;
...
static uint32_t
bfd_endpoint_tnl_l2_vxlan_update_flags_get(void *type1, void *type2)
{
    bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t *ep1, *ep2;
    uint32_t flags = 0;

    ep1 = (bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t *) type1;
    ep2 = (bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t *) type2;

    if (sal_memcmp(ep1, ep2, sizeof(bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t)) != 0) {
        flags |= MCS_BFD_EP_SET_UPDATE_F_ENCAP;
    }
...
static uint16_t
bfd_endpoint_tnl_l2_vxlan_lkey_etype_get(void *type)
{
    bcmcth_oam_bfd_endpoint_tnl_l2_vxlan_t *ep;
...
typedef struct bfd_ep_type_info_s {
    ...
    /* Get ethertype used by this endpoint. */
    uint16_t (*key_etype_get)(void *type);
} bfd_ep_type_info_t;
...
static bfd_ep_type_info_t g_ep_types_info[] = {
    {
        ...
        bfd_endpoint_ipv4_lkey_etype_get
    },
    {
        ...
        bfd_endpoint_ipv6_lkey_etype_get
    },
    {
        ...
        bfd_endpoint_tnl_ipv4_lkey_etype_get
    },
    {
        ...
        bfd_endpoint_tnl_ipv6_lkey_etype_get
    },
    {
        ...
        bfd_endpoint_tnl_mpls_lkey_etype_get
    },
    {
        bfd_endpoint_tnl_l2_vxlan_encap_len_get,
        bfd_endpoint_tnl_l2_vxlan_encap_pack,
        bfd_endpoint_tnl_l2_vxlan_mcs_encap_type_get,
        bfd_endpoint_tnl_l2_vxlan_msg_convert,
        bfd_endpoint_tnl_l2_vxlan_update_flags_get,
        bfd_endpoint_tnl_l2_vxlan_lkey_etype_get
    }
};
...
bfd_endpoint_loopback_header_get(int unit,
    ...
    SHR_IF_ERR_EXIT(bcmpkt_flexhdr_field_id_get(variant, hid, "RESERVED_0",
                    &fid));
    SHR_IF_ERR_EXIT
                        ...
                        mcs_bfd_msg_ctrl_ep_set_t *msg)
{
    ...
    l2.etype = ep_type_info->key_etype_get(&(entry->type));
    cur_ptr = shr_util_pack_l2_header_pack(cur_ptr, &l2);
     ...
     */
    msg->lkey_etype = ep_type_info->key_etype_get(&(entry->type));
    if (lkey != NULL) {
                         ...
                         mcs_bfd_msg_ctrl_ep_set_t *msg)
{
    ...
    msg->tx_cos = cur->cos;
    msg->encap_type = ep_type_info->mcs_encap_type_get(&cur->type);
