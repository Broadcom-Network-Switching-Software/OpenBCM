...
/*******************************************************************************
 * BFD Tunnel VxLAN endpoint functions.
 */
static int
bfd_endpoint_tnl_l2_vxlan_field_get(int unit, const bcmltd_field_t *field,
                                    bcmcth_oam_bfd_endpoint_t *entry)
{
    bcmcth_oam_bfd_drv_t *bfd_drv = NULL;
    uint32_t fid;
    ...
    switch (fid) {
        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_ROLEf:
            entry->role = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_IP_ENCAP_TYPEf:
            entry->type.tnl_vxlan.ip_encap_type = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_MODEf:
            entry->mode = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_LOCAL_DISCRIMINATORf:
            entry->local_discr = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_REMOTE_DISCRIMINATORf:
            entry->remote_discr = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_DIAG_CODEf:
            entry->diag_code = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_MIN_TX_INTERVAL_USECSf:
            entry->min_tx_interval_usecs = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_MIN_RX_INTERVAL_USECSf:
            entry->min_rx_interval_usecs = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_DETECT_MULTIPLIERf:
            entry->detect_multiplier = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_AUTH_TYPEf:
            entry->auth_type = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OAM_BFD_AUTH_SIMPLE_PASSWORD_IDf:
            entry->sp.id = fval;
            rv = bcmcth_oam_bfd_auth_sp_entry_get(unit, entry->sp.id,
                                                  &(entry->sp));
            if (rv == SHR_E_NONE) {
        ...
        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OAM_BFD_AUTH_SHA1_IDf:
            entry->sha1.id = fval;
            rv = bcmcth_oam_bfd_auth_sha1_entry_get(unit, entry->sha1.id,
                                                    &(entry->sha1));
            if (rv == SHR_E_NONE) {
        ...
        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_INITIAL_SHA1_SEQ_NUMf:
            entry->initial_sha1_seq_num = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_SHA1_SEQ_NUM_INCREMENTf:
            entry->sha1_seq_num_incr = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_POLL_SEQUENCEf:
            entry->poll_sequence = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_RX_LOOKUP_VLAN_IDf:
            entry->type.tnl_vxlan.rx_lookup_vlan_id = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_PORT_IDf:
            entry->port = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_COSf:
            entry->cos = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_DST_MACf:
            fval32[0] = (uint32_t)fval;
            fval32[1] = (uint32_t)(fval >> 32);
            BCMCTH_MAC_ADDR_FROM_UINT32(entry->dst_mac, fval32);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_SRC_MACf:
            fval32[0] = (uint32_t)fval;
            fval32[1] = (uint32_t)(fval >> 32);
            BCMCTH_MAC_ADDR_FROM_UINT32(entry->src_mac, fval32);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_TAG_TYPEf:
            SHR_IF_ERR_VERBOSE_EXIT
                (bfd_drv->num_vlans_get(fval, &(entry->num_vlans)));
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_VLAN_IDf:
            entry->outer_vlan.tci.vid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_VLAN_PRIf:
            entry->outer_vlan.tci.prio = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_TPIDf:
            entry->outer_vlan.tpid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_INNER_VLAN_IDf:
            entry->inner_vlan.tci.vid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_INNER_VLAN_PRIf:
            entry->inner_vlan.tci.prio = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_INNER_TPIDf:
            entry->inner_vlan.tpid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_SRC_IPV6u_UPPERf:
            BCMCTH_IPV6_ADDR_UPPER_SET(entry->type.tnl_vxlan.underlay_ipv6.src_ip, fval);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_SRC_IPV6u_LOWERf:
            BCMCTH_IPV6_ADDR_LOWER_SET(entry->type.tnl_vxlan.underlay_ipv6.src_ip, fval);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_DST_IPV6u_UPPERf:
            BCMCTH_IPV6_ADDR_UPPER_SET(entry->type.tnl_vxlan.underlay_ipv6.dst_ip, fval);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_DST_IPV6u_LOWERf:
            BCMCTH_IPV6_ADDR_LOWER_SET(entry->type.tnl_vxlan.underlay_ipv6.dst_ip, fval);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_TRAFFIC_CLASSf:
            entry->type.tnl_vxlan.underlay_ipv6.traffic_class = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_HOP_LIMITf:
            entry->type.tnl_vxlan.underlay_ipv6.hop_limit = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_SRC_IPV4f:
            entry->type.tnl_vxlan.underlay_ipv4.src_ip = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_DST_IPV4f:
            entry->type.tnl_vxlan.underlay_ipv4.dst_ip = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_TOSf:
            entry->type.tnl_vxlan.underlay_ipv4.tos = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_TTLf:
            entry->type.tnl_vxlan.underlay_ipv4.ttl = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_UDP_SRC_PORTf:
            entry->type.tnl_vxlan.underlay_udp.udp_src_port = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_UNDERLAY_UDP_DST_PORTf:
            entry->type.tnl_vxlan.underlay_udp.udp_dst_port = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_VNIDf:
            entry->type.tnl_vxlan.vxlan_vnid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_DST_MACf:
            fval32[0] = (uint32_t)fval;
            fval32[1] = (uint32_t)(fval >> 32);
            BCMCTH_MAC_ADDR_FROM_UINT32(entry->type.tnl_vxlan.overlay_eth.dst_mac, fval32);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_SRC_MACf:
            fval32[0] = (uint32_t)fval;
            fval32[1] = (uint32_t)(fval >> 32);
            BCMCTH_MAC_ADDR_FROM_UINT32(entry->type.tnl_vxlan.overlay_eth.src_mac, fval32);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_TAG_TYPEf:
            SHR_IF_ERR_VERBOSE_EXIT
                (bfd_drv->num_vlans_get(fval, &(entry->type.tnl_vxlan.overlay_eth.num_vlans)));
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_VLAN_IDf:
            entry->type.tnl_vxlan.overlay_eth.outer_vlan.tci.vid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_VLAN_PRIf:
            entry->type.tnl_vxlan.overlay_eth.outer_vlan.tci.prio = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_TPIDf:
            entry->type.tnl_vxlan.overlay_eth.outer_vlan.tpid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_INNER_VLAN_IDf:
            entry->type.tnl_vxlan.overlay_eth.inner_vlan.tci.vid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_INNER_VLAN_PRIf:
            entry->type.tnl_vxlan.overlay_eth.inner_vlan.tci.prio = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_INNER_TPIDf:
            entry->type.tnl_vxlan.overlay_eth.inner_vlan.tpid = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_SRC_IPV4f:
            entry->type.tnl_vxlan.overlay_ipv4.src_ip = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_DST_IPV4f:
            entry->type.tnl_vxlan.overlay_ipv4.dst_ip = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_TOSf:
            entry->type.tnl_vxlan.overlay_ipv4.tos = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_SRC_IPV6u_UPPERf:
            BCMCTH_IPV6_ADDR_UPPER_SET(entry->type.tnl_vxlan.overlay_ipv6.src_ip, fval);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_SRC_IPV6u_LOWERf:
            BCMCTH_IPV6_ADDR_LOWER_SET(entry->type.tnl_vxlan.overlay_ipv6.src_ip, fval);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_DST_IPV6u_UPPERf:
            BCMCTH_IPV6_ADDR_UPPER_SET(entry->type.tnl_vxlan.overlay_ipv6.dst_ip, fval);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_DST_IPV6u_LOWERf:
            BCMCTH_IPV6_ADDR_LOWER_SET(entry->type.tnl_vxlan.overlay_ipv6.dst_ip, fval);
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_TRAFFIC_CLASSf:
            entry->type.tnl_vxlan.overlay_ipv6.traffic_class = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_UDP_SRC_PORTf:
            entry->udp_src_port = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_TX_MODEf:
            entry->tx_mode = fval;
            break;

        case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OPERATIONAL_STATEf:
            entry->oper_state = fval;
            break;
...
static int
bfd_endpoint_tnl_l2_vxlan_defaults_get(int unit, bcmcth_oam_bfd_endpoint_t *entry)
{
    bcmcth_oam_bfd_drv_t *bfd_drv = NULL;
    bcmlrd_client_field_info_t *fields_info = NULL;
    size_t fld_num;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_oam_bfd_num_data_fields_get(unit, OAM_BFD_TNL_L2_VXLAN_ENDPOINTt,
                                            &fld_num));
    SHR_ALLOC(fields_info, sizeof(bcmlrd_client_field_info_t) * fld_num,
              "bcmcthOamBfdEndpointTnlIpv4FldInfo");
    SHR_NULL_CHECK(fields_info, SHR_E_MEMORY);
    ...
    SHR_IF_ERR_EXIT
        (bcmlrd_table_fields_def_get(unit, "OAM_BFD_TNL_L2_VXLAN_ENDPOINT",
                                     fld_num, fields_info, &actual_fld_num));
    ...
    for (idx = 0; idx < actual_fld_num; idx++) {
        switch (fields_info[idx].id) {
            ...
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_IP_ENCAP_TYPEf:
                entry->type.tnl_vxlan.ip_encap_type = fields_info[idx].def.u8;
                break;
            ...
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OVERLAY_TAG_TYPEf:
                SHR_IF_ERR_VERBOSE_EXIT
                    (bfd_drv->num_vlans_get(fields_info[idx].def.u8,
                    &(entry->type.tnl_vxlan.overlay_eth.num_vlans)));
                break;
            ...
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDf:
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_LOCAL_DISCRIMINATORf:
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_REMOTE_DISCRIMINATORf:
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OAM_BFD_AUTH_SIMPLE_PASSWORD_IDf:
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OAM_BFD_AUTH_SHA1_IDf:
...
/* Populate the output fields in OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUS LT. The fields
 * are already sorted on the basis of fid.
 */
static void
bfd_endpoint_tnl_l2_vxlan_status_populate(bcmcth_oam_bfd_endpoint_status_t *entry,
                                          bcmltd_fields_t *output_fields)
{
    uint32_t fid;
    uint64_t *data;
    ...
    for (i = 0; i < output_fields->count; i++) {
        ...
        switch (fid) {
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_LOCAL_STATEf:
                *data = entry->local_state;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_REMOTE_STATEf:
                *data = entry->remote_state;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_LOCAL_DIAG_CODEf:
                *data = entry->local_diag_code;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_REMOTE_DIAG_CODEf:
                *data = entry->remote_diag_code;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_LOCAL_AUTH_SEQ_NUMf:
                *data = entry->local_auth_seq_num;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_REMOTE_AUTH_SEQ_NUMf:
                *data = entry->remote_auth_seq_num;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_POLL_SEQUENCE_ACTIVEf:
                *data = entry->poll_sequence_active;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_REMOTE_MODEf:
                *data = entry->remote_mode;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_LOCAL_DISCRIMINATORf:
                *data = entry->local_discr;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_REMOTE_DISCRIMINATORf:
                *data = entry->remote_discr;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_REMOTE_DETECT_MULTIPLIERf:
                *data = entry->remote_detect_multiplier;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_REMOTE_MIN_TX_INTERVAL_USECSf:
                *data = entry->remote_min_tx_interval_usecs;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_REMOTE_MIN_RX_INTERVAL_USECSf:
                *data = entry->remote_min_rx_interval_usecs;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_ENDPOINT_STATEf:
                *data = entry->endpoint_state;
                break;
...
/* Populate the output fields in OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATS LT. The fields
 * are already sorted on the basis of fid.
 */
static void
bfd_endpoint_tnl_l2_vxlan_stats_populate(bcmcth_oam_bfd_endpoint_stats_t *entry,
                                         bcmltd_fields_t *output_fields)
{
    uint32_t fid;
    uint64_t *data;
    ...
    for (i = 0; i < output_fields->count; i++) {
        ...
        switch (fid) {
            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATSt_RX_PKT_CNTf:
                *data = entry->rx_pkt_cnt;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATSt_RX_PKT_DISCARD_CNTf:
                *data = entry->rx_pkt_discard_cnt;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATSt_RX_PKT_AUTH_FAILURE_DISCARD_CNTf:
                *data = entry->rx_pkt_auth_failure_discard_cnt;
                break;

            case OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATSt_TX_PKT_CNTf:
                *data = entry->tx_pkt_cnt;
                break;
...
static int
bfd_endpoint_tnl_l2_vxlan_stats_field_get(int unit, const bcmltd_field_t *field,
                                          bcmcth_oam_bfd_endpoint_stats_t *entry)
{
    uint32_t fid;
    uint64_t fval;
...
static bfd_endpoint_type_info_t ep_types_info[] = {
    ...
    {
        BCMLTD_COMMON_OAM_BFD_ENDPOINT_TYPE_T_T_TNL_L2_VXLAN,
        OAM_BFD_TNL_L2_VXLAN_ENDPOINTt,
        OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDf,
        OAM_BFD_TNL_L2_VXLAN_ENDPOINTt_OPERATIONAL_STATEf,
        OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt,
        OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATUSt_OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDf,
        OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATSt,
        OAM_BFD_TNL_L2_VXLAN_ENDPOINT_STATSt_OAM_BFD_TNL_L2_VXLAN_ENDPOINT_IDf,
        bfd_endpoint_tnl_l2_vxlan_defaults_get,
        bfd_endpoint_tnl_l2_vxlan_field_get,
        bfd_endpoint_tnl_l2_vxlan_status_populate,
        bfd_endpoint_tnl_l2_vxlan_stats_field_get,
        bfd_endpoint_tnl_l2_vxlan_stats_populate
    },
