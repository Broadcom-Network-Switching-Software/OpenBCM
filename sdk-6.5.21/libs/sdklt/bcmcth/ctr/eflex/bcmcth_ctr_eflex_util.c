...
/*! Function to check if LT is supported for given device. */
bool
bcmcth_eflex_is_lt_supported(int unit, bcmlrd_sid_t sid)
{
    int rv;
    const bcmlrd_map_t *eflex_map = NULL;

    /* Check if LT is supported for given device */
    rv = bcmlrd_map_get(unit, sid, &eflex_map);

    return (SHR_SUCCESS(rv) && eflex_map);
}
