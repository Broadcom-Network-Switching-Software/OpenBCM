...
/*! Maximum count of bucket entries */
#define BKT_ENTRY_COUNT 12
...
/*!
 * Bucket entry transform lookup array
 * when narrow mode is enabled and entry is single wide.
 */
static uint8_t single_bkt_entry_lkup[BKT_ENTRY_COUNT] = {
    0, 3, 2, 1, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY,
    INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY,
    INVALID_BKT_ENTRY
};

/*!
 * Bucket entry transform lookup array
 * when narrow mode is enabled and entry is double wide.
 */
static uint8_t double_bkt_entry_lkup[BKT_ENTRY_COUNT] = {
    0, 1, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY,
    INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY,
    INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY
};

/*!
 * Bucket entry transform lookup array
 * when narrow mode is enabled and entry is quad wide.
 */
static uint8_t quad_bkt_entry_lkup[BKT_ENTRY_COUNT] = {
    0, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY,
    INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY,
    INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY
};
...
/*!
 * Bucket entry transform lookup array
 * when narrow mode is enabled and entry is half wide.
 */
static uint8_t half_bkt_entry_lkup[BKT_ENTRY_COUNT] = {
    0, 2, 3, 5, 6, 8, 9, 11,
    INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY, INVALID_BKT_ENTRY
};
...
/*!
 * Bucket entry transform lookup array
 * when narrow mode is enabled and entry is third wide.
 */
static uint8_t third_bkt_entry_lkup[BKT_ENTRY_COUNT] = {
    ...
    ctr_eflex_hitbit_ctrl_data_t *entry)
{
    ...
    key_fields.field[count]->id = table_id_fid[comp][select];
    key_fields.field[count++]->data = entry->lt_id;
    ...
    key_fields.field[count]->id = oper_state_fid[comp][select];
    key_fields.field[count++]->data = entry->oper_state;
    ...
    ctr_eflex_group_action_data_t *grp_action_entry)
{
    ...
    /* If narrow mode is enabled, save bucket entry lookup array */
    if (hitbit_lt_info->narrow_mode) {
        switch (entry_width) {
            case BCMPTM_RM_HASH_ENTRY_WIDTH_SINGLE:
                hitbit_lt_info->bkt_entry_lkup = single_bkt_entry_lkup;
                break;
            case BCMPTM_RM_HASH_ENTRY_WIDTH_DOUBLE:
                hitbit_lt_info->bkt_entry_lkup = double_bkt_entry_lkup;
                break;
            case BCMPTM_RM_HASH_ENTRY_WIDTH_QUAD:
                hitbit_lt_info->bkt_entry_lkup = quad_bkt_entry_lkup;
                break;
            case BCMPTM_RM_HASH_ENTRY_WIDTH_HALF:
                hitbit_lt_info->bkt_entry_lkup = half_bkt_entry_lkup;
                break;
            case BCMPTM_RM_HASH_ENTRY_WIDTH_THIRD:
                hitbit_lt_info->bkt_entry_lkup = third_bkt_entry_lkup;
                break;
            default:
                hitbit_lt_info->bkt_entry_lkup = NULL;
        }
    }
    ...
    void *context)
{
    ...
    if (!bcmcth_eflex_is_lt_supported(unit,
                                      hitbit_ctrl_table_sid[comp][select])) {
        LOG_VERBOSE(BSL_LOG_MODULE,
    ...
    ctr_eflex_hitbit_lt_info_t *hitbit_lt_info)
{
    ...
    uint32_t pool_idx, pool_idx_offset;
    uint32_t ctr_idx, bit_idx;
    ...
    pool_idx_offset = ctr_bit_idx_offset / (num_hitbit_ctr * max_ctr_num);
    pool_idx += pool_idx_offset;
    bit_idx = ctr_bit_idx_offset -
              (pool_idx_offset * num_hitbit_ctr * max_ctr_num);
    ctr_idx = bit_idx / num_hitbit_ctr;
    bit_idx = bit_idx % num_hitbit_ctr;
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(unit,
            ...
            pool_idx, ctr_idx, bit_idx, hitbit_lt_info->pipe_idx));

    /* Set hitbit for all applicable pipes */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmptm_cci_hit_bit_set(unit,
                                pool_sid,
                                hitbit_lt_info->pipe_idx,
                                ctr_idx,
                                bit_idx));
...
bcmcth_ctr_eflex_imm_hitbit_ctrl_register(int unit)
{
    ...
    /* Check if LT is supported for given device */
    if (bcmcth_eflex_is_lt_supported(unit, CTR_ING_EFLEX_HITBIT_CONTROLt)) {
    ...
    /* Check if LT is supported for given device */
    if (bcmcth_eflex_is_lt_supported(unit, CTR_EGR_EFLEX_HITBIT_CONTROLt)) {
