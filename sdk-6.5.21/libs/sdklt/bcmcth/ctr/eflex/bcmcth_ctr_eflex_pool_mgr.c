                                     ...
                                     bcmptm_cci_pool_state_t state)
{
    ...
    ctr_eflex_pool_list_t *pool_action_list = NULL;
    ctr_eflex_control_t *ctrl = NULL;
    ...
    pool_action_list = entry->ingress ?
                       &ctrl->ing_pool_action_list[pool_idx][pipe_idx] :
                       &ctrl->egr_pool_action_list[pool_idx][pipe_idx];
    ...
    /* If SKIP_COLLECTION LT fid is valid, use value from LT entry. */
    if (lt_info->skip_collect_fid_valid) {
        skip_collect = entry->skip_collect;
    }
    ...
    if (!skip_collect) {
        for (inst_idx = 0; inst_idx < num_insts; inst_idx++) {
            ...
            if (state == ENABLE) {
                 ...
                 * and for flex counter if all actions are disabled in
                 * this pool.
                ...
                if (entry->comp == CTR_EFLEX) {
                    if ((SHR_BITNULL_RANGE(pool_list->inuse_bitmap,
                                           0,
                                           pool_list->max_track)) &&
                        (SHR_BITNULL_RANGE(pool_action_list->inuse_bitmap,
                                           0,
                                           pool_action_list->max_track))) {
                                  ...
                                  ctr_eflex_ctr_mode_data_t *mode_data)
{
    ...
    /* If COLLECTION LT fid is valid, use value from LT entry. */
    if (lt_info->skip_collect_fid_valid) {
        skip_collect = entry->skip_collect;
    }
    ...
    if (!skip_collect) {
        while (cur_pool_idx <= mode_data->end_pool_idx) {
            ...
            if (cur_pool_idx == mode_data->end_pool_idx) {
                index_max =
                    (mode_data->end_section_idx * num_ctr_per_section) - 1;
            } else {
            ...
            for (inst_idx = 0; inst_idx < num_insts; inst_idx++) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcmptm_cci_flex_counter_mode_set(
                        unit,
                        table_sid,
                        index_min,
                        index_max,
...
static int
bcmcth_ctr_eflex_pool_action_list_init(
    int unit,
    uint32_t num_action_id,
    uint32_t max_pipe_num,
    uint32_t max_pool_num,
    ctr_eflex_pool_list_t p_list[][MAX_PIPES_NUM])
{
    ...
    /* Allocate pool action list */
    for (pipe_idx = 0; pipe_idx < max_pipe_num; pipe_idx++) {
        for (pool_idx = 0; pool_idx < max_pool_num; pool_idx++) {
            pool_list = &p_list[pool_idx][pipe_idx];

            num_bytes = SHR_BITALLOCSIZE(num_action_id);

            /* Allocate inuse bitmap */
            pool_list->inuse_bitmap =
                sal_alloc(num_bytes, "bcmcthCtrEflexPoolActionBitmap");

            SHR_NULL_CHECK(pool_list->inuse_bitmap, SHR_E_MEMORY);

            sal_memset(pool_list->inuse_bitmap, 0, num_bytes);

            pool_list->max_track = num_action_id;
        }
    }
...
bcmcth_ctr_eflex_pool_mgr_create(int unit, eflex_comp_t comp)
{
    ...
    if (comp == CTR_EFLEX) {
        /* Create ingress pool action id list */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_ctr_eflex_pool_action_list_init(
                unit,
                num_ing_action_id,
                max_pipe_num,
                max_ing_pool_num,
                ctrl->ing_pool_action_list));

        /* Create egress pool action id list */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_ctr_eflex_pool_action_list_init(
                unit,
                num_egr_action_id,
...
bcmcth_ctr_eflex_pool_mgr_destroy(int unit, eflex_comp_t comp)
{
    ...
    /* For flex counter, destroy pool action id list */
    if (comp == CTR_EFLEX) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_ctr_eflex_pool_mgr_uninit(unit,
                                              max_pipe_num,
...
/*! Function to set\clear action id in use in the pool list */
static int
bcmcth_ctr_eflex_pool_action_list_set(int unit,
                                      bool val,
                                      bool ingress,
                                      uint32_t pipe_idx,
                                      ...
                                      uint32_t action_id)
{
    ...
    /* Set/Clear given action id in all applicable pools */
    for (pool_idx = start_pool_idx; pool_idx <= end_pool_idx; pool_idx++) {
        list = ingress ? &ctrl->ing_pool_action_list[pool_idx][pipe_idx] :
                         &ctrl->egr_pool_action_list[pool_idx][pipe_idx];

        /* Set/Clear action id in pool list */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_ctr_eflex_id_set(unit, val, action_id, list));
    }
                                    ...
                                    uint32_t *base_idx)
{
    ...
    for (pipe_idx = entry->start_pipe_idx;
        ...
        /* For flex counter, set action id for all applicable pools */
        if (entry->comp == CTR_EFLEX) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_ctr_eflex_pool_action_list_set(
                    unit,
                    true,
                    entry->ingress,
                    pipe_idx,
                    ctr_mode_data.start_pool_idx,
                    ctr_mode_data.end_pool_idx,
                    entry->action_id));
        }
                                   ...
                                   ctr_eflex_action_profile_data_t *entry)
{
    ...
    for (pipe_idx = entry->start_pipe_idx;
        ...
        /* For flex counter, clear action id for all applicable pools */
        if (entry->comp == CTR_EFLEX) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_ctr_eflex_pool_action_list_set(
                    unit,
