    ...
    ctr_eflex_op_profile_data_t *entry)
{
    ...
    for (i = 0; i < num_fid; i++) {
        ...
        } else if (f_info[i].id == shift_3_fid[comp][select]) {
            entry->shift_3 = f_info[i].def.u8;
        } else if (f_info[i].id == shift_4_fid[comp][select]) {
            entry->shift_4 = f_info[i].def.u8;
        } else if (f_info[i].id == shift_5_fid[comp][select]) {
    ...
    ctr_eflex_op_profile_data_t *cur_entry)
{
     ...
     */
    if (entry->op_profile_index != cur_entry->op_profile_index ||
        (entry->inst != cur_entry->inst) || (entry->egr_inst != cur_entry->egr_inst) ||
...
bcmcth_ctr_eflex_imm_op_profile_register(int unit)
{
    ...
    if (bcmcth_eflex_is_lt_supported(unit, CTR_ING_EFLEX_OPERAND_PROFILEt)) {
    ...
    /* Check if LT is supported for given device */
    if (bcmcth_eflex_is_lt_supported(unit, CTR_EGR_EFLEX_OPERAND_PROFILEt)) {
...
bcmcth_state_eflex_imm_op_profile_register(int unit)
{
    ...
    /* Check if LT is supported for given device */
    if (bcmcth_eflex_is_lt_supported(unit, FLEX_STATE_ING_OPERAND_PROFILEt)) {
        ...
        state_eflex_lt_cb.op_stage =
            bcmcth_state_eflex_ing_op_profile_lt_stage_cb;
    ...
    /* Check if LT is supported for given device */
    if (bcmcth_eflex_is_lt_supported(unit, FLEX_STATE_EGR_OPERAND_PROFILEt)) {
        /* Register FLEX_STATE_EGR_OPERAND_PROFILE handler */
        sal_memset(&state_eflex_lt_cb, 0, sizeof(bcmimm_lt_cb_t));
        state_eflex_lt_cb.op_stage =
            bcmcth_state_eflex_egr_op_profile_lt_stage_cb;
        state_eflex_lt_cb.ent_limit_get  =
