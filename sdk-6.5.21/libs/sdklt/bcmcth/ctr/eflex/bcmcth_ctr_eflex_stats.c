...
bcmcth_ctr_eflex_stats_idx_validate(int unit,
                                    ...
                                    ctr_eflex_action_profile_data_t *entry)
{
    uint32_t abs_ctr_idx = 0, min_ctr_idx = 0 , slim_ctr = 0;
    ...
    if (entry->ctr_mode == SLIM_MODE  && device_info->slim_ctr_row == TRUE) {
        slim_ctr = 3 ;
    } else {
        slim_ctr = 1 ;
    }
    ...
    if ((abs_ctr_idx < min_ctr_idx) ||
        (abs_ctr_idx >= (min_ctr_idx + entry->num_ctrs * slim_ctr))) {
    ...
    for (pipe_idx = entry->start_pipe_idx;
        ...
        for (inst_idx = 0; inst_idx < num_insts; inst_idx++) {
            ...
            if (entry->ctr_mode == SLIM_MODE) {
                ...
                /* Call CCI API to read  normal double  ctr */
                SHR_IF_ERR_VERBOSE_EXIT
                                 ...
                                 bcmltd_fields_t *out)
{
    ...
    /* Get LT fid info */
    lt_info = ingress ? &ctrl->ing_action_lt_info : &ctrl->egr_action_lt_info;
    ...
    /* If counter collection is skipped, return stats as 0 */
    if (skip_collect) {
        for (i = 0; i < out->count; i++) {
            out->field[i]->data = 0;
        }
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
            (BSL_META_U(unit, "Returning 0s as ctr collection is skipped\n")));
                                 ...
                                 const bcmltd_fields_t *in)
{
    ...
    /* If counter collection is skipped, do not update */
    if (skip_collect) {
        LOG_VERBOSE(BSL_LOG_MODULE,
            (BSL_META_U(unit, "No update as ctr collection is skipped\n")));
                               ...
                               bcmltd_fields_t *out)
{
    ...
    if ((stats_data.index + 1) < entry.num_ctrs) {
        ...
        if (skip_collect) {
            ...
            LOG_VERBOSE(BSL_LOG_MODULE,(BSL_META_U(unit,
                "Returning 0s as ctr collection is skipped\n")));
        }
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,(BSL_META_U(unit,
            "counter collection is %s\n"),
            entry.skip_collect ? "true" : "false"));
    }
