    ...
    ctr_eflex_action_profile_data_t *entry)
{
    ...
    entry->skip_collect = CTR_EFLEX_INVALID_VAL;
    ...
    entry->lt_id = entry->ingress ? ctrl->ing_action_lt_info.lt_sid :
                                     ...
                                     ctr_eflex_action_profile_data_t *cur_entry)
{
    ...
    if (entry->comp == CTR_EFLEX) {
        ...
        if (entry->skip_collect == CTR_EFLEX_INVALID_VAL) {
            entry->skip_collect = cur_entry->skip_collect;
        }
    ...
    if (entry->comp == STATE_EFLEX) {
        ...
        if (entry->state_ext.egr_inst == CTR_EFLEX_INVALID_VAL) {
    ...
    const bcmltd_field_t *data_fields)
{
    ...
    if (data_fields->id == lt_info->action_profile_id_fid) {
        ...
        } else if (lt_info->skip_collect_fid_valid &&
                   data_fields->id == lt_info->skip_collect_fid) {
            entry->skip_collect = data_fields->data;
        }
            ...
            if (f_info[i].id == lt_info->zone_fid) {
                ...
                for (idx = 0; idx < num_objs; idx++) {
                    entry->obj[idx] = \
                       ...
                       f_info[i].id == lt_info->skip_collect_fid) {
                entry->skip_collect = f_info[i].def.u8;
            }
        } else if (entry->comp == STATE_EFLEX) {
            /* Flex state data */
            if (entry->ingress == INGRESS &&
...
/*
 * Function to check counter reset conditions
 * in new and current entry of ACTION_PROFILE IMM LT
 */
static int
bcmcth_ctr_eflex_action_profile_ctr_reset_check(
    int unit,
    bool ingress,
    eflex_comp_t comp,
    const bcmltd_field_t *data_fields,
...
{
    int i = 0;
    const bcmlrd_fid_t *ctr_reset_fid = NULL;
    ctr_eflex_control_t *ctrl = NULL;

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(ctr_reset, SHR_E_PARAM);

    /* Get flex control struct */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_eflex_ctrl_get(unit, comp, &ctrl));

    ctr_reset_fid = ingress ? ctrl->ing_ctr_reset_fid : ctrl->egr_ctr_reset_fid;

    /* Check updating field IDs in counter reset field list */
    while (data_fields) {
        while (ctr_reset_fid[i] != CTR_EFLEX_INVALID_VAL) {
            if (data_fields->id == ctr_reset_fid[i]) {
                LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit,
                    "\n%s LT counter reset list %s field updated\n"),
                    ingress ? "ING ACTION_PROFILE":
                              "EGR ACTION_PROFILE",
                    bcmltm_lt_field_fid_to_name(unit,
                                                ingress ?
    ...
    uint32_t *base_idx)
{
    ...
    if (entry->comp == STATE_EFLEX) {
        cur_entry->ctr_update_mode[UPDATE_MODE_A_IDX] = SET_TO_CVALUE;
        cur_entry->ctr_update_mode[UPDATE_MODE_B_IDX] = SET_TO_CVALUE;
    }
    ...
    /* Allocate counters or reserve counters */
    if (ctr_reset) {
        if (entry->base_idx_auto) {
            /* Calculate the base index value & allocate counters */
            SHR_IF_ERR_VERBOSE_EXIT
...
exit:
    ...
    if (SHR_FUNC_VAL_IS(SHR_E_RESOURCE) && ctr_reset) {
                                         ...
                                         bcmltd_fields_t *out)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_ctr_eflex_action_profile_ctr_reset_check(unit,
                                                                 ingress,
                                                                 comp,
                                                                 data_fields,
                                                                 &ctr_reset));
