    ...
    ctr_eflex_action_profile_data_t *entry)
{
    ...
    if (entry->ingress && (entry->pool_idx >= MAX_ING_POOLS_NUM)) {
        LOG_VERBOSE(BSL_LOG_MODULE,
            (BSL_META_U(unit,
                "Invalid ing pool idx %d provided\n"),entry->pool_idx));
        SHR_ERR_EXIT(SHR_E_PARAM);
    } else if (!entry->ingress && (entry->pool_idx >= MAX_EGR_POOLS_NUM)) {
        LOG_VERBOSE(BSL_LOG_MODULE,
            (BSL_META_U(unit,
                "Invalid egr pool idx %d provided\n"),entry->pool_idx));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    if (entry->ctr_mode == SLIM_MODE) {
        ...
        if (device_info->slim_ctr_row == TRUE)  {
            /*
             * Slim counters are calculated row wise and hence
             * we dont need to reserve full pool
             */
    ...
    if (entry->ctr_mode == NORMAL_DOUBLE_MODE) {
        entry->num_ctrs = entry->num_ctrs % 2 ? (entry->num_ctrs / 2 + 1):
                                                 entry->num_ctrs / 2;
    }
