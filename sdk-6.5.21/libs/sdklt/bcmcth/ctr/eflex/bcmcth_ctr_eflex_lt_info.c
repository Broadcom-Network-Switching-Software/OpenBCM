                                         ...
                                         eflex_comp_t comp)
{
    ...
    bcmlrd_fid_t *ctr_reset_fid = NULL;
    ...
    if (bcmcth_eflex_is_lt_supported(unit, t_attrib.id)) {
        ...
        for (i = 0; i < num_fid; i++) {
            if (!sal_strcmp(f_info[i].name, "ACTION")) {
                ...
                }else if (!sal_strcmp(f_info[i].name, "SKIP_COLLECTION")) {
                    lt_info->skip_collect_fid = f_info[i].id;
                    lt_info->skip_collect_fid_valid = true;
                }
    ...
    /* Initialize flex counter reset fids */
    ctr_reset_fid = ingress ? ctrl->ing_ctr_reset_fid : ctrl->egr_ctr_reset_fid;

    /* Add fids which will enable counter reset on update */
    i = 0;
    ctr_reset_fid[i++] = lt_info->pipe_idx_fid;
    ctr_reset_fid[i++] = lt_info->action_fid;
    ctr_reset_fid[i++] = lt_info->pool_fid;
    ctr_reset_fid[i++] = lt_info->base_idx_auto_fid;
    ctr_reset_fid[i++] = lt_info->base_idx_fid;
    ctr_reset_fid[i++] = lt_info->ctr_mode_fid;
    ctr_reset_fid[i++] = lt_info->ctr_update_mode_a_fid;
    ctr_reset_fid[i++] = lt_info->ctr_update_mode_b_fid;
    if (lt_info->skip_collect_fid_valid) {
        ctr_reset_fid[i++] = lt_info->skip_collect_fid;
    }

    /* Set remaining reset fids to invalid value */
    for (; i < MAX_CTR_FID_NUM; i++) {
        ctr_reset_fid[i] = CTR_EFLEX_INVALID_VAL;
    }
