...
int bcmcth_mon_ft_sw_resources_alloc(int unit, bool warm)
{
    ...
    if (warm) {
        if (ft_drv->ft_hw_lt_info_init != NULL) {
            switch (info->ha->hw_learn) {
                case BCMLTD_COMMON_FLOWTRACKER_HARDWARE_LEARN_T_T_ENABLE:
                    ft_drv->ft_hw_lt_info_init(unit, true);
                    break;

                case BCMLTD_COMMON_FLOWTRACKER_HARDWARE_LEARN_T_T_ENABLE_HARDWARE_ONLY:
                    ft_drv->ft_hw_lt_info_init(unit, false);
                    break;

                default:
                    /* Nothing to do, driver assumes S/w learning by default. */
                    break;
            }
        }
    }
...
bcmcth_mon_ft_sw_resources_free(int unit)
{
    ...
    if (intr_drv != NULL && fifo_drv != NULL) {
        /* Un-initialize the flowtracker learn threads */
        rv = bcmcth_mon_ft_learn_threads_deinit(unit);
        if (SHR_FAILURE(rv)) {
            LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Unit%d:\
                                bcmcth_mon_ft_learn_threads_deinit failed\n"),
                     unit));
        }
        bcmcth_mon_ft_learn_event_log_deinit(unit);
        bcmcth_mon_ft_fifo_drv_deinit(unit);
    }
                                    ...
                                    mcs_ft_msg_ctrl_hw_learn_opt_msg_t *msg)
{
    ...
    if (ft_drv->ft_hw_learn_opt_msg_init) {
        SHR_IF_ERR_VERBOSE_EXIT(
                                ...
                                mcs_ft_msg_ctrl_rxpmd_flex_format_t *msg)
{
    ...
    SHR_BIT_ITER(prof_bmp, MCS_FT_RXPMD_FLEX_PROFILES_COUNT, profile) {
        ...
        for (i = 0; i < num_fids; i++) {
            ...
            /* Get mask value */
            rv = bcmpkt_flexhdr_field_get(variant, hid, rxpmd_flex_ref, profile,
                                          fids[i], &val);
            if (rv != SHR_E_NONE) {
                continue;
            }
            ...
            /* Set field mask */
            rv = bcmpkt_flexhdr_field_set(variant, hid, rxpmd_flex_mask,
                                          profile, fids[i], val);
            if (rv != SHR_E_NONE) {
                continue;
            }
...
/*
 * Check if flex ctr action LT ID is configured.
 * If yes, derive the HW action idx from it and store it
 * in global info.
 * NOTE: This is currently required only for flowtracker
 * without embedded app use case for programming implicit
 * flex counter action allocated by HSDK.
 */
static int
bcmcth_mon_ft_control_flex_ctr_hw_idx_update(int unit,
                               ...
                               bcmcth_mon_ft_control_oper_fields_t *oper)
{
    ...
    SHR_FUNC_ENTER(unit);
    if (FLOWTRACKER_HW_LEARN_WO_EAPP_ENABLED(entry->hw_learn)) {
        if (entry->ctr_ing_flex_action_profile_id &&
                ft_drv && ft_drv->ft_hw_flex_ctr_action_idx_get) {
            rv = (ft_drv->ft_hw_flex_ctr_action_idx_get(
                        unit, entry->ctr_ing_flex_action_profile_id,
                        &(info->ha->pkt_byte_flex_ctr_action_idx)));
            if (SHR_FAILURE(rv)) {
                entry->oper.operational_state = oper->operational_state =
                             ...
                             bcmcth_mon_ft_control_oper_fields_t *oper)
{
    ...
    if (entry.flowtracker == true) {
        ...
        if (FLOWTRACKER_HW_LEARN_ENABLED(entry.hw_learn)) {
            rv = bcmcth_mon_ft_control_flex_ctr_hw_idx_update(unit, ft_drv,
                                                              info, &entry,
                                                              oper);
            if (SHR_FAILURE(rv)) {
                /* oper state is already set in the function. Simply exit */
                SHR_EXIT();
            }
            /* Enable the HW learning */
            if (ft_drv && ft_drv->ft_hw_learn_init) {
                SHR_IF_ERR_VERBOSE_EXIT(ft_drv->ft_hw_learn_init(unit, &entry));
            }
        }
    }
    ...
    if (!FLOWTRACKER_HW_LEARN_WO_EAPP_ENABLED(old_entry.hw_learn) &&
        ...
        if (info->ha->run == true) {
            ...
            if (entry.flowtracker == true) {
                ...
                /* Enable the HW learning */
                if (ft_drv->ft_hw_learn_init) {
                    SHR_IF_ERR_VERBOSE_EXIT(ft_drv->ft_hw_learn_init(unit, &entry));
                }
            } else {
                /* De-init the HW learning */
                if (ft_drv->ft_hw_learn_deinit) {
                    ft_drv->ft_hw_learn_deinit(unit);
                }
            }
        ...
        if (entry.ctr_ing_flex_action_profile_id !=
            old_entry.ctr_ing_flex_action_profile_id) {
            rv = bcmcth_mon_ft_control_flex_ctr_hw_idx_update(unit, ft_drv,
                                                              info, &entry,
                                                              oper);
            if (SHR_FAILURE(rv)) {
        ...
        if (sal_memcmp(old_entry.max_flows, entry.max_flows,
            ...
            if (ft_drv->ft_hw_learn_update_glb_flow_threshold) {
...
static void
bcmcth_mon_ft_group_actions_flex_ctr_info_get(bcmcth_mon_ft_group_t *entry,
                                              bool *is_config,
                                              uint16_t *grp_flex_ctr_act_idx)
{
    uint32_t i;
    for (i = 0; i < entry->num_actions; i++) {
        if (entry->actions[i].type ==
            BCMLTD_COMMON_FLOWTRACKER_ACTION_TYPE_T_T_FLEX_CTR_ACTION) {
            *is_config = true;
            *grp_flex_ctr_act_idx = entry->actions[i].flex_ctr_action_val;
        }
    }
}

/*
 * If the global flex counter action is configured and
 * if either flex counter action is not configured or
 * configured but has a zero action value, then
 * add a new implicit flex counter action or
 * update the flex counter action value with the global
 * flex counter's hw idx respectively.
 * The global flex counter's hw idx should have been fetched
 * from the FLEX counter LT ID already.
 */
static void
bcmcth_mon_ft_group_actions_flex_ctr_update(int unit,
                                            uint16_t flex_ctr_action_idx,
                                            bcmcth_mon_ft_group_t *entry)
{
    bool is_config = false;
    uint16_t grp_flex_ctr_act_idx = 0;

    if (flex_ctr_action_idx) {
        /*
         * See if flex counter action is configured in group actions.
         * If yes, get the flex counter action idx configured.
         */
        bcmcth_mon_ft_group_actions_flex_ctr_info_get(entry, &is_config,
                                                      &grp_flex_ctr_act_idx);
        if (!is_config || !(grp_flex_ctr_act_idx)) {
            /* Add flex ctr implicit action as the last action. */
            entry->actions[entry->num_actions].type =
                BCMLTD_COMMON_FLOWTRACKER_ACTION_TYPE_T_T_FLEX_CTR_ACTION;
            entry->actions[entry->num_actions].flex_ctr_action_val =
                flex_ctr_action_idx;
            entry->num_actions = entry->num_actions + 1;
        }
    }
}
        ...
        bcmcth_mon_ft_group_t *insert)
{
    ...
    if (insert->num_tp && insert->dt_em_grp_template_id &&
        ...
        if (ctrl_rv == SHR_E_NONE) {
            ...
            if (FLOWTRACKER_HW_LEARN_ENABLED(info->ha->hw_learn)) {
                ...
                if (ft_drv->ft_grp_hw_learn_config) {
                    SHR_IF_ERR_VERBOSE_EXIT(
    ...
    if (FLOWTRACKER_HW_LEARN_WO_EAPP_ENABLED(info->ha->hw_learn)) {
        bcmcth_mon_ft_group_actions_flex_ctr_update(unit,
                info->ha->pkt_byte_flex_ctr_action_idx, &entry);

        /* Set FT HW learn configurations in HW */
        if (ft_drv->ft_grp_hw_learn_config_wo_eapp) {
            SHR_IF_ERR_VERBOSE_EXIT(
        ...
        bcmcth_mon_ft_group_t *update)
{
            ...
            if (em_group_id_map_status) {
                ...
                for (i = 0; i < MCS_SHR_FT_MAX_EM_GROUPS; ++i) {
                    ...
                    if (info->ha->em_group_id_map[index] == FT_INVALID_EM_GROUP_ID) {
                        ...
                        if (ft_drv->ft_grp_init) {
                            SHR_IF_ERR_VERBOSE_EXIT
                                (ft_drv->ft_grp_init(unit, &entry, &flex_msg,
                                                    &pdd_index, &data_width));
                            if (entry.oper !=
                                    BCMLTD_COMMON_MON_FLOWTRACKER_GROUP_STATE_T_T_SUCCESS) {
    ...
    if (entry.flow_limit != old_entry.flow_limit) {
        ...
        if (FLOWTRACKER_HW_LEARN_ENABLED(info->ha->hw_learn)) {
            ...
            if (ft_drv->ft_hw_learn_flow_limit_update) {
                SHR_IF_ERR_VERBOSE_EXIT(
