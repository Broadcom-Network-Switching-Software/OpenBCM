...
static int
bcmcth_mon_ft_learn_event_ctrl_lt_field_get(int unit,
        const bcmltd_field_t *gen_field,
        bcmcth_mon_ft_learn_event_control_t *entry)
{
    uint32_t fid;
    uint64_t fval;
    ...
    switch (fid) {
        case MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_PIPEf:
            entry->pipe  = fval;
            break;
        case MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_NOTIFY_LEARN_SUCCESSf:
            entry->notify_learn = fval;
            break;
        case MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_STOP_LEARN_ON_NOTIFY_FULLf:
            if (fval == 1) {
                entry->fifo_behavior |= BCMCTH_MON_FT_LEARN_FIFO_FULL_STOP_LEARNING;
            }
            break;
        case MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_FLOW_LEARN_FAIL_THRESHOLDf:
            entry->flow_learn_fail_threshold = fval;
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
static int
bcmcth_mon_ft_learn_event_ctrl_lt_fields_arr_parse(int unit,
        bcmltd_field_t **data,
        int count,
        bcmcth_mon_ft_learn_event_control_t *entry)
{
    ...
    for (idx = 0; idx < count; idx++) {
        gen_field = ((bcmltd_field_t **) data)[idx];
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_mon_ft_learn_event_ctrl_lt_field_get(unit, gen_field, entry));
    }
...
static int
bcmcth_mon_ft_learn_event_ctrl_fields_ll_parse(int unit,
        const bcmltd_field_t *key,
        const bcmltd_field_t *data,
        bcmcth_mon_ft_learn_event_control_t *entry)
{
    ...
    while (gen_field != NULL) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_mon_ft_learn_event_ctrl_lt_field_get(unit, gen_field,
                                                              entry));
        gen_field = gen_field->next;
    }
...
static int
bcmcth_mon_ft_learn_evt_control_dummy_entry_insert(int unit)
{
    uint32_t field_cnt = MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_FIELD_COUNT;
    bcmltd_fields_t lt_flds = {0};
    uint8_t pipe = 0;
    bcmcth_mon_flowtracker_drv_t *ft_drv = NULL;
    SHR_FUNC_ENTER(unit);
    ...
    /* Install entry for each pipe */
    for (pipe = 0; pipe < ft_drv->num_pipes; pipe++) {
        /* Key = Pipe */
        lt_flds.field[0]->id = MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_PIPEf;
        lt_flds.field[0]->data = pipe;

        /* field NOTIFY_LEARN_SUCCESS = default value 0 */
        lt_flds.field[1]->id = MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_NOTIFY_LEARN_SUCCESSf;
        lt_flds.field[1]->data = 0;

        /* field STOP_LEARN_ON_NOTIFY_FULL = default value 0 */
        lt_flds.field[2]->id = MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_STOP_LEARN_ON_NOTIFY_FULLf;
        lt_flds.field[2]->data = 0;

        /* field FLOW_LEARN_FAIL_THRESHOLD = default value 1 */
        lt_flds.field[3]->id = MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_FLOW_LEARN_FAIL_THRESHOLDf;
        lt_flds.field[3]->data = 1;


        SHR_IF_ERR_VERBOSE_EXIT(bcmimm_entry_insert(unit,
                    MON_FLOWTRACKER_LEARN_EVENT_CONTROLt, &lt_flds));
    }
exit:
    bcmcth_mon_ft_util_fields_free(unit, field_cnt, &lt_flds);
    SHR_FUNC_EXIT();
...
static void
bcmcth_mon_ft_learn_event_ctrl_entry_defaults_get(int unit,
                                            bcmcth_mon_ft_learn_event_control_t *entry)
{
    entry->pipe = 0;
    entry->notify_learn = 0;
    entry->fifo_behavior = 0;
    entry->flow_learn_fail_threshold = 1;
}

static int
bcmcth_mon_ft_learn_event_ctrl_entry_get(int unit, int pipe,
                                  bcmcth_mon_ft_learn_event_control_t *entry)
{
    bcmltd_fields_t in = {0};
    bcmltd_fields_t out = {0};
    uint32_t field_count = MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_FIELD_COUNT;
    ...
    in.count = 1;
    in.field[0]->id = MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_PIPEf;
    in.field[0]->data = pipe;
    entry->pipe = pipe;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmimm_entry_lookup(unit, MON_FLOWTRACKER_LEARN_EVENT_CONTROLt, &in, &out));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_ft_learn_event_ctrl_lt_fields_arr_parse(unit,
                                         out.field, out.count,
                                         entry));
...
static int
bcmcth_mon_ft_learn_event_ctrl_update(int unit,
                bcmcth_mon_ft_learn_event_control_t *old_entry,
                bcmcth_mon_ft_learn_event_control_t *entry)
{
    SHR_FUNC_ENTER(unit);
    /* Handle updates of learn notification only when it has changed. */
    if (SHR_BITGET(entry->fbmp,
        MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_NOTIFY_LEARN_SUCCESSf) &&
        (old_entry->notify_learn != entry->notify_learn)) {
        /* Learn notification off */
        if (entry->notify_learn == false) {
            /* Set learn thread to sleep forever */
            SHR_IF_ERR_EXIT(
                bcmcth_mon_ft_learn_thread_intvl_set(unit, entry->pipe,
                                                    SHR_THREAD_FOREVER));
            /* Disable the interrupt in HW. */
            SHR_IF_ERR_EXIT(
                bcmcth_mon_ft_learn_intr_enable(unit, entry->pipe, false));
        } else { /* Learn notification on */
            /* Set threshold to 1 */
            SHR_IF_ERR_EXIT(
                bcmcth_mon_ft_learn_intr_threshold_set(unit, entry->pipe, 1));
            /* Enable the interrupt in HW. */
            SHR_IF_ERR_EXIT(
                bcmcth_mon_ft_learn_intr_enable(unit, entry->pipe, true));
        }
    }

    if (SHR_BITGET(entry->fbmp,
        MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_FLOW_LEARN_FAIL_THRESHOLDf)) {
        /* Set threshold for flow learn failure */
        SHR_IF_ERR_EXIT(
                bcmcth_mon_ft_learn_fail_intr_threshold_set(unit, entry->pipe,
                    entry->flow_learn_fail_threshold));
    }

    /* Handle updates of fifo full behavior only when it has changed. */
    if (SHR_BITGET(entry->fbmp,
        MON_FLOWTRACKER_LEARN_EVENT_CONTROLt_STOP_LEARN_ON_NOTIFY_FULLf) &&
        (old_entry->fifo_behavior != entry->fifo_behavior)) {
       SHR_IF_ERR_EXIT(bcmcth_mon_ft_fifo_behavior_set(unit, entry->pipe,
                                            entry->fifo_behavior));
    }
exit:
    SHR_FUNC_EXIT();
...
static int
bcmcth_mon_ft_learn_event_ctrl_stage(
        int unit,
        bcmltd_sid_t sid,
        uint32_t trans_id,
        bcmimm_entry_event_t event,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_ft_learn_event_ctrl_fields_ll_parse(unit, key, data, &entry));
    if (output_fields) {
        output_fields->count = 0;
    }
    ...
    switch (event) {
        case BCMIMM_ENTRY_INSERT:
            ...
            break;
        case BCMIMM_ENTRY_UPDATE:
            bcmcth_mon_ft_learn_event_ctrl_entry_defaults_get(unit, &old_entry);
            /* Get the current entry for comparison */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_mon_ft_learn_event_ctrl_entry_get(unit, entry.pipe,
                                                          &old_entry));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_mon_ft_learn_event_ctrl_update(unit,
                                                       &old_entry,
                                                       &entry));
            break;
...
static int
bcmcth_mon_ft_learn_flow_entry_lt_lookup_out_fields_populate(int unit,
                                       ...
                                       bcmltd_fields_t *output_fields)
{
    ...
    /*
     * Update only data fields and do not touch key fields,
     * otherwise traverse will get broken.
     */
    for (idx = 0; idx < output_fields->count; idx++) {
        fid = output_fields->field[idx]->id;
        arr_idx = output_fields->field[idx]->idx;
        switch (fid) {
            case MON_FLOWTRACKER_LEARN_FLOW_ENTRYt_NUM_ENTRY_DATAf:
                output_fields->field[idx]->data = entry->num_words_count;
                break;
            case MON_FLOWTRACKER_LEARN_FLOW_ENTRYt_ENTRY_DATAf:
                /* Fill the appropriate array index's value */
                output_fields->field[idx]->data = entry->entry_words[arr_idx];
                break;
        }
    }
exit:
...
static int
bcmcth_mon_ft_learn_flow_entry_lt_key_fields_parse(
    int unit,
    const bcmltd_fields_t *key,
    bcmcth_mon_ft_learn_flow_entry_t *entry)
{
    ...
    for (i = 0; i < key->count; i++) {
        ...
        if (key->field[i]->id == MON_FLOWTRACKER_LEARN_FLOW_ENTRYt_PIPEf) {
            entry->pipe = key->field[i]->data;
        } else if (key->field[i]->id == MON_FLOWTRACKER_LEARN_FLOW_ENTRYt_MODEf) {
            entry->mode = key->field[i]->data;
        } else if (key->field[i]->id == MON_FLOWTRACKER_LEARN_FLOW_ENTRYt_EXACT_MATCH_INDEXf) {
            entry->exact_match_idx = key->field[i]->data;
        }
    }
exit:
    SHR_FUNC_EXIT();
...
static int
bcmcth_mon_ft_learn_flow_entry_lookup(int unit,
                 bcmltd_sid_t sid,
                 ...
                 bcmltd_fields_t *output_fields)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_ft_learn_flow_entry_lt_key_fields_parse(unit, key_fields, &entry));

    if (ft_drv && ft_drv->ft_em_entry_lkup) {
        SHR_IF_ERR_VERBOSE_EXIT(
            ft_drv->ft_em_entry_lkup(unit, &entry));
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_ft_learn_flow_entry_lt_lookup_out_fields_populate(
                                                unit, &entry,
                                                output_fields));
exit:
    SHR_FUNC_EXIT();
...
static void
bcmcth_mon_ft_learn_fail_event_ctrl_state_to_event_state(
    bcmltd_common_mon_flowtracker_learn_fail_event_control_state_t_t ctrl_state,
    bcmltd_common_mon_flowtracker_learn_fail_event_state_t_t *evt_state)
{
    switch (ctrl_state) {
        case BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL_STATE_T_T_OFF:
            *evt_state = BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATE_T_T_OFF;
            break;
        case BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL_STATE_T_T_ARMED:
            *evt_state = BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATE_T_T_ARMED;
            break;
    }
}

static int
bcmcth_mon_ft_learn_fail_event_ctrl_lt_field_get(int unit,
        const bcmltd_field_t *gen_field,
        bcmcth_mon_ft_learn_fail_event_control_t *entry)
{
    uint32_t fid;
    uint64_t fval;
    ...
    switch (fid) {
        case MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt_PIPEf:
            entry->pipe  = fval;
            break;
        case MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt_STATEf:
            entry->state = fval;
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
...
static int
bcmcth_mon_ft_learn_fail_event_ctrl_lt_fields_arr_parse(int unit,
        bcmltd_field_t **data,
        int count,
        bcmcth_mon_ft_learn_fail_event_control_t *entry)
{
    ...
    for (idx = 0; idx < count; idx++) {
        gen_field = ((bcmltd_field_t **) data)[idx];
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_mon_ft_learn_fail_event_ctrl_lt_field_get(unit, gen_field, entry));
    }
...
static int
bcmcth_mon_ft_learn_fail_event_ctrl_fields_ll_parse(int unit,
        const bcmltd_field_t *key,
        const bcmltd_field_t *data,
        bcmcth_mon_ft_learn_fail_event_control_t *entry)
{
    ...
    while (gen_field != NULL) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmcth_mon_ft_learn_fail_event_ctrl_lt_field_get(unit, gen_field,
                                                              entry));
        gen_field = gen_field->next;
    }
...
static void
bcmcth_mon_ft_learn_fail_event_ctrl_entry_defaults_get(int unit,
                               bcmcth_mon_ft_learn_fail_event_control_t *entry)
{
    entry->pipe = 0;
    entry->state = BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL_STATE_T_T_OFF;
}

static int
bcmcth_mon_ft_learn_fail_event_ctrl_entry_get(int unit, int pipe,
                                  bcmcth_mon_ft_learn_fail_event_control_t *entry)
{
    bcmltd_fields_t in = {0};
    bcmltd_fields_t out = {0};
    uint32_t field_count = MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt_FIELD_COUNT;
    ...
    in.count = 1;
    in.field[0]->id = MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt_PIPEf;
    in.field[0]->data = pipe;
    entry->pipe = pipe;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmimm_entry_lookup(unit, MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt,
                                &in, &out));

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_ft_learn_fail_event_ctrl_lt_fields_arr_parse(unit,
                                         out.field, out.count,
                                         entry));
...
static int
bcmcth_mon_ft_learn_fail_event_ctrl_update(int unit,
                bcmcth_mon_ft_learn_fail_event_control_t *old_entry,
                bcmcth_mon_ft_learn_fail_event_control_t *entry)
{
    SHR_FUNC_ENTER(unit);
    /*
     * Handle updates of learn fail notification even when it has not
     * changed to handle ARMED to ARMED case for re-enabling of interrupt
     * by user application.
     */
    if (SHR_BITGET(entry->fbmp,
        MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt_STATEf)) {
        /* Learn fail notification off */
        if (entry->state ==
                BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL_STATE_T_T_OFF) {
            /* Disable the interrupt. */
            SHR_IF_ERR_EXIT(
                bcmcth_mon_ft_learn_fail_intr_enable(unit, entry->pipe, false));
        } else if (entry->state ==
        BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL_STATE_T_T_ARMED){
            /* Learn fail notification on - Enable the interrupt. */
            SHR_IF_ERR_EXIT(
                bcmcth_mon_ft_learn_fail_intr_enable(unit, entry->pipe, true));
        }
    }
exit:
    SHR_FUNC_EXIT();
...
static int
bcmcth_mon_ft_learn_fail_event_ctrl_stage(
        int unit,
        bcmltd_sid_t sid,
        uint32_t trans_id,
        bcmimm_entry_event_t event,
        ...
        bcmltd_fields_t *output_fields)
{
    bcmcth_mon_ft_learn_fail_event_control_t entry;
    bcmcth_mon_ft_learn_fail_event_control_t old_entry;
    bcmltd_common_mon_flowtracker_learn_fail_event_state_t_t event_state =
                BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATE_T_T_OFF;
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_mon_ft_learn_fail_event_ctrl_fields_ll_parse(unit, key,
                                                            data, &entry));
    if (output_fields) {
        output_fields->count = 0;
    }
    ...
    switch (event) {
        case BCMIMM_ENTRY_INSERT:
            ...
            break;
        case BCMIMM_ENTRY_UPDATE:
            bcmcth_mon_ft_learn_fail_event_ctrl_entry_defaults_get(unit,
                                                                &old_entry);
            /* Get the current entry for comparison */
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_mon_ft_learn_fail_event_ctrl_entry_get(unit, entry.pipe,
                                                              &old_entry));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_mon_ft_learn_fail_event_ctrl_update(unit,
                                                       &old_entry,
                                                       &entry));

            /*
             * Need to update the same state to
             * MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATE entry
             */
            bcmcth_mon_ft_learn_fail_event_ctrl_state_to_event_state(
                                                entry.state,
                                                &event_state);
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_mon_ft_learn_fail_event_state_entry_update(unit,
                                                            entry.pipe,
                                                            event_state));
            break;
...
static int
bcmcth_mon_ft_learn_fail_evt_control_dummy_entries_insert(int unit)
{
    uint32_t field_cnt = MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt_FIELD_COUNT;
    bcmltd_fields_t lt_flds = {0};
    uint8_t pipe = 0;
    bcmcth_mon_flowtracker_drv_t *ft_drv = NULL;
    SHR_FUNC_ENTER(unit);
    ...
    for (pipe = 0; pipe < ft_drv->num_pipes; pipe++) {
        /* Key = Pipe */
        lt_flds.field[0]->id = MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt_PIPEf;
        lt_flds.field[0]->data = pipe;

        /* field STATE = default value OFF */
        lt_flds.field[1]->id = MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt_STATEf;
        lt_flds.field[1]->data =
            BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL_STATE_T_T_OFF;

        SHR_IF_ERR_VERBOSE_EXIT(bcmimm_entry_insert(unit,
                    MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt, &lt_flds));
    }
exit:
    bcmcth_mon_ft_util_fields_free(unit, field_cnt, &lt_flds);
    SHR_FUNC_EXIT();
...
static int
bcmcth_mon_ft_learn_fail_evt_state_dummy_entries_insert(int unit)
{
    uint32_t field_cnt = MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATEt_FIELD_COUNT;
    bcmltd_fields_t lt_flds = {0};
    uint8_t pipe = 0;
    bcmcth_mon_flowtracker_drv_t *ft_drv = NULL;
    SHR_FUNC_ENTER(unit);
    ...
    for (pipe = 0; pipe < ft_drv->num_pipes; pipe++) {
        /* Key = Pipe */
        lt_flds.field[0]->id = MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATEt_PIPEf;
        lt_flds.field[0]->data = pipe;

        /* field STATE = default value OFF */
        lt_flds.field[1]->id = MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATEt_STATEf;
        lt_flds.field[1]->data =
            BCMLTD_COMMON_MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATE_T_T_OFF;

        SHR_IF_ERR_VERBOSE_EXIT(bcmimm_entry_insert(unit,
                    MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATEt, &lt_flds));
    }
exit:
    bcmcth_mon_ft_util_fields_free(unit, field_cnt, &lt_flds);
    SHR_FUNC_EXIT();
            ...
            bcmltd_common_mon_flowtracker_learn_fail_event_state_t_t state)
{
    ...
    /* Key */
    lt_flds.field[0]->id =
    MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATEt_PIPEf;
    lt_flds.field[0]->data = pipe;

    /* Data */
    lt_flds.field[1]->id =
    MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATEt_STATEf;
    lt_flds.field[1]->data = state;
...
/*!
 * \brief MON_FLOWTRACKER_LEARN_EVENT_CONTROL In-memory event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to MON_FLOWTRACKER_LEARN_EVENT_CONTROL logical table entry commit stages.
 */
static bcmimm_lt_cb_t ft_learn_event_ctrl_hdl = {

    /*! Validate function. */
    .validate = NULL,

    /*! Staging function. */
    .stage = bcmcth_mon_ft_learn_event_ctrl_stage,
...
/*!
 * \brief MON_FLOWTRACKER_LEARN_FLOW_ENTRY In-memory event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to MON_FLOWTRACKER_LEARN_FLOW_ENTRY logical table entry commit stages.
 */
static bcmimm_lt_cb_t ft_learn_flow_entry_hdl = {
    ...
    /*! Lookup function */
    .op_lookup = bcmcth_mon_ft_learn_flow_entry_lookup
};

/*!
 * \brief MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL In-memory event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL logical table entry commit stages.
 */
static bcmimm_lt_cb_t ft_learn_fail_event_ctrl_hdl = {

    /*! Validate function. */
    .validate = NULL,

    /*! Staging function. */
    .stage = bcmcth_mon_ft_learn_fail_event_ctrl_stage,
...
static int
bcmcth_mon_ft_learn_notification_init(int unit, bool warm)
{
    ...
    /*
     * Only call the learn notification related initialization if
     * the chip supports it.
     */
    if (intr_drv != NULL && fifo_drv != NULL) {
        /* Initialize the Fifo driver */
        SHR_IF_ERR_EXIT(bcmcth_mon_ft_fifo_drv_init(unit));
        /* Initialize the MON_FLOWTRACKER_LEARN_EVENT_LOG library */
        SHR_IF_ERR_EXIT(bcmcth_mon_ft_learn_event_log_init(unit, warm));
        /* Initialize the flowtracker learn threads */
        SHR_IF_ERR_EXIT(bcmcth_mon_ft_learn_threads_init(unit));
    }
exit:
    SHR_FUNC_EXIT();
}
...
bcmcth_mon_ft_imm_register(int unit, bool warm)
{
    ...
    SHR_IF_ERR_EXIT(bcmcth_mon_ft_learn_notification_init(unit, warm));

    if (bcmcth_mon_ft_imm_mapped(unit, MON_FLOWTRACKER_LEARN_EVENT_CONTROLt)) {
        SHR_IF_ERR_EXIT
            (bcmimm_lt_event_reg(unit, MON_FLOWTRACKER_LEARN_EVENT_CONTROLt,
                                 &ft_learn_event_ctrl_hdl, NULL));
        if (!warm) {
            /* Dummy insertion of MON_FLOWTRACKER_LEARN_EVENT_CONTROL entry */
            SHR_IF_ERR_VERBOSE_EXIT(
                bcmcth_mon_ft_learn_evt_control_dummy_entry_insert(unit));
        }
    }

    if (bcmcth_mon_ft_imm_mapped(unit, MON_FLOWTRACKER_LEARN_EVENT_LOGt)) {
        if (!warm) {
            /* Dummy insertion of MON_FLOWTRACKER_LEARN_EVENT_LOG entries */
            SHR_IF_ERR_VERBOSE_EXIT(
                bcmcth_mon_ft_learn_event_log_dummy_entries_insert(unit));
        }
    }
    if (bcmcth_mon_ft_imm_mapped(unit, MON_FLOWTRACKER_LEARN_EVENT_LOG_STATUSt)) {
        if (!warm) {
            /* Dummy insertion of MON_FLOWTRACKER_LEARN_EVENT_LOG_STATUS entry */
            SHR_IF_ERR_VERBOSE_EXIT(
                bcmcth_mon_ft_learn_event_log_status_dummy_entry_insert(unit));
        }
    }
    if (bcmcth_mon_ft_imm_mapped(unit, MON_FLOWTRACKER_LEARN_FLOW_ENTRYt)) {
        SHR_IF_ERR_EXIT
            (bcmimm_lt_event_reg(unit, MON_FLOWTRACKER_LEARN_FLOW_ENTRYt,
                                 &ft_learn_flow_entry_hdl, NULL));
    }
    if (bcmcth_mon_ft_imm_mapped(unit, MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt)) {
        SHR_IF_ERR_EXIT
            (bcmimm_lt_event_reg(unit, MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROLt,
                                 &ft_learn_fail_event_ctrl_hdl, NULL));
        if (!warm) {
            /* Dummy insertion of MON_FLOWTRACKER_LEARN_FAIL_EVENT_CONTROL entries */
            SHR_IF_ERR_VERBOSE_EXIT(
                bcmcth_mon_ft_learn_fail_evt_control_dummy_entries_insert(unit));
        }
    }
    if (bcmcth_mon_ft_imm_mapped(unit, MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATEt)) {
        if (!warm) {
            /* Dummy insertion of MON_FLOWTRACKER_LEARN_FAIL_EVENT_STATE entries */
            SHR_IF_ERR_VERBOSE_EXIT(
                bcmcth_mon_ft_learn_fail_evt_state_dummy_entries_insert(unit));
        }
    }
