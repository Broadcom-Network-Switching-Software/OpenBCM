    ...
    while (gen_field) {
        ...
        switch (fid) {
            ...
            case MON_COLLECTOR_IPV4t_IPFIX_ENTERPRISE_NUMBERf:
                ...
                SHR_BITSET(entry->fbmp, fid);
                break;
             case MON_COLLECTOR_IPV4t_INITIAL_SEQUENCE_NUMBERf:
                entry->init_seq_number = fval;
                SHR_BITSET(entry->fbmp, fid);
                break;
              case MON_COLLECTOR_IPV4t_VERSIONf:
                entry->version = fval;
                SHR_BITSET(entry->fbmp, fid);
                break;
            default:
    ...
    while (gen_field) {
        ...
        switch (fid) {
            ...
            case MON_COLLECTOR_IPV6t_IPFIX_ENTERPRISE_NUMBERf:
                ...
                SHR_BITSET(entry->fbmp, fid);
                break;
            case MON_COLLECTOR_IPV6t_INITIAL_SEQUENCE_NUMBERf:
                entry->init_seq_number = fval;
                SHR_BITSET(entry->fbmp, fid);
                break;
            case MON_COLLECTOR_IPV6t_VERSIONf:
                entry->version = fval;
                SHR_BITSET(entry->fbmp, fid);
                break;
            default:
