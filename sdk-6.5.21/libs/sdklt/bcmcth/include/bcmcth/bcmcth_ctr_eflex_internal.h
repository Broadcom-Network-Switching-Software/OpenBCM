...
#define INVALID_BKT_ENTRY 0xFF

/*! Maximum number for counter FID for reset check. */
#define MAX_CTR_FID_NUM 16
...
typedef struct ctr_eflex_action_profile_data_s {
    ...
    /*! Skip counter collection. */
    uint32_t skip_collect;
...
typedef struct ctr_eflex_hitbit_lt_info_s {
    ...
    /*! Narrow mode enabled or not. */
    bool narrow_mode;

    /*! Bucket entry lookup array. */
    uint8_t *bkt_entry_lkup;
...
typedef struct ctr_eflex_control_s {
    ...
    /*! Ingress pool action id info. */
    ctr_eflex_pool_list_t
                        ing_pool_action_list[MAX_ING_POOLS_NUM][MAX_PIPES_NUM];

    /*! Egress pool action id info. */
    ctr_eflex_pool_list_t
                        egr_pool_action_list[MAX_EGR_POOLS_NUM][MAX_PIPES_NUM];

    /*! Ingress counter reset FIDs. */
    bcmlrd_fid_t ing_ctr_reset_fid[MAX_CTR_FID_NUM];

    /*! Egress counter reset FIDs. */
    bcmlrd_fid_t egr_ctr_reset_fid[MAX_CTR_FID_NUM];
 ...
 * \retval SHR_E_NONE on success and error code otherwise.
 */
extern int bcmcth_ctr_eflex_lt_info_init(int unit);
 ...
 * \retval SHR_E_NONE on success and error code otherwise.
 */
extern int bcmcth_state_eflex_lt_info_init(int unit);

/*!
 * \brief   Check if LT is supported for given device.
 *
 * \param [in] unit Unit number.
 * \param [in] sid  LT sid.
 *
 * \retval True if LT is supported otherwise False.
 */
extern bool bcmcth_eflex_is_lt_supported(int unit, bcmlrd_sid_t sid);
