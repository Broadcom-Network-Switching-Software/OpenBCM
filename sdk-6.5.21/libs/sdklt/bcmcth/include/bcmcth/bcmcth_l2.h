 ...
 * \param [in] in_intr 1: called in interrupt thread context,
 *                     0: called in other threads context.
 *
 * \retval SHR_E_NONE No errors.
 */
typedef int (*bcmcth_l2_drv_intr_enable_set_f)(int unit, bool enable, bool in_intr);
...
/*!
 * \brief Parse the learn cache entry.
 *
...
/*!
 * \brief Insert the learn cache entry.
 *
 * Insert the L2 learn cache entry.
 *
 * \param [in] unit Unit number.
 ...
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_FAIL Fail to insert the Learn cache entry.
 */
typedef int (*bcmcth_l2_drv_entry_insert_f)(int unit, uint32_t pipe,
                                            uint32_t index, void *data);

/*!
 * \brief Validate the learn cache entry.
 *
 * Validate the L2 learn cache entry.
 *
 * \param [in] unit Unit number.
 * \param [in] data Learn cache table entry data fields.
 *
 * \retval TRUE Learn cache entry is valid.
 * \retval FALSE Learn cache entry is not valid.
 */
typedef bool (*bcmcth_l2_drv_entry_valid_f)(int unit, void *data);
...
typedef struct bcmcth_l2_drv_s {
    ...
    bcmcth_l2_drv_intr_enable_set_f l2_intr_enable_set;
    ...
    /*! Parse the entry in learn cache logical table. */
    bcmcth_l2_drv_entry_parse_f entry_parse;

    /*! Insert the entry in learn cache logical table. */
    bcmcth_l2_drv_entry_insert_f entry_insert;

    /*! Validate the entry in learn cache logical table. */
    bcmcth_l2_drv_entry_valid_f entry_valid;
} bcmcth_l2_drv_t;
 ...
 */
extern int
bcmcth_l2_drv_intr_handler_set(int unit,
...
/*!
 * \brief Delete L2 entry in queue.
 *
 * \param [in] unit Unit number.
 * \param [in] pipe Pipe number.
 * \param [in] data_id Data id for the entry.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_FAIL Fail to delete entry.
 */
extern int
