...
typedef enum {
    ...
    /*!
     * Enable to count non-unicast packets dropped due to
     * packet prune configured on PORT_DVP.
     */
    CTR_EGR_DBG_SEL_NONUC_PRUNE_DROP,

    /*!
     * Enable to count packets dropped due to enabling
     * egress FP drop.
     */
    CTR_EGR_DBG_SEL_FP_DROP,

    /*!
     * Enable to count packets dropped due to adaptation lookup miss.
     */
    CTR_EGR_DBG_SEL_ADAPT_LOOKUP_MISS,

    /*!
     * Enable to count packets dropped due to overlay and underlay
     * next hop and interface indices don't comply with configured
     * allocation bitmap.
     */
    CTR_EGR_DBG_SEL_OVERLAY_UNDERLAY_RANGE_ERROR,

    /*!
     * Enable to count packets dropped due to enabling pruning of
     * packet with source virtual port equals to destination
     * virtual port.
     */
    ...
    /*!
     * Enable to count packets dropped due to enabling
     * prune packets from the specified source network group to
     * given destination network group.
     */
