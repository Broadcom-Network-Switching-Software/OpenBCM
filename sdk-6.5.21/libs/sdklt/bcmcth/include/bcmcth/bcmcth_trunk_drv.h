...
typedef struct bcmcth_trunk_drv_var_ids_s {
    ...
    /*! Resilient group size field ID. */
    const int trunk_vp_rh_size_id;

    /*! RH_MEMBER_CNT field ID for TRUNK_VP LT. */
    const int trunk_vp_rh_member_cnt_id;

    /*! RH_RAND_SEED field ID for TRUNK_VP LT. */
    const int trunk_vp_rh_rand_seed_id;
...
typedef struct bcmcth_trunk_vp_drv_var_s {
    ...
    /*! Pointer to field information for TRUNK_VP resilient LT. */
    const bcmcth_trunk_vp_member_field_info_t *rflds_mem0;
    ...
    /*! Pointer to HW field information for TRUNK_VP resilient LT. */
    const bcmcth_trunk_vp_member_info_t *rmember0_info;
...
/*!
 * \brief Lookup system trunk group.
 *
 * \param [in] unit Logical device id.
 * \param [in] op_arg Operation arguments.
 * \param [in] lt_id Logical table id.
 ...
 * \retval SHR_E_NONE Success.
 */
typedef int (*fn_sys_grp_lookup_t)(int unit,
                                   const bcmltd_op_arg_t *op_arg,
                                   bcmltd_sid_t lt_id,
                                   bcmcth_trunk_sys_grp_bk_t *grp_bk,
                                   bcmcth_trunk_sys_grp_param_t *param);
...
/*!
 * \brief Get the member fields count for system trunk group.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
 */
typedef int (*fn_sys_grp_member_flds_cnt_t)(int unit);

/*!
 * \brief Group get system trunk entry.
 *
 * \param [in] unit Logical device id.
 * \param [in] op_arg Operation arguments.
 * \param [in] lt_id Logical table id.
 ...
 * \retval SHR_E_NONE Success.
 */
typedef int (*fn_sys_grp_get_t)(int unit,
                                const bcmltd_op_arg_t *op_arg,
                                bcmltd_sid_t lt_id,
                                bcmcth_trunk_sys_grp_bk_t *grp_bk,
                                bcmcth_trunk_sys_grp_param_t *param);
 ...
 * \retval SHR_E_NONE Success.
 */
typedef int (*fn_imm_grp_lookup_t)(int unit,
                                   const bcmltd_op_arg_t *op_arg,
                                   bcmltd_sid_t lt_id,
                                   bcmcth_trunk_imm_grp_bk_t *grp_bk,
                                   bcmcth_trunk_group_param_t *param);
...
/*!
 * \brief Get the member fields count for trunk group.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
 */
typedef int (*fn_imm_grp_member_flds_cnt_t)(int unit);
...
/*! Install an TRUNK_VP group LT entry into the hardware tables. */
typedef int (*fn_vp_grp_get_t)(int unit,
                               void *info);
/*!
...
typedef struct bcmcth_trunk_drv_s {
    ...
    /*! Lookup system trunk group LT entry. */
    fn_sys_grp_lookup_t             fn_sys_grp_lookup;

    /*! Delete system trunk group LT entry. */
    fn_sys_grp_del_t                fn_sys_grp_del;

    /*! Get the member fields count for system trunk group LT entry. */
    fn_sys_grp_member_flds_cnt_t    fn_sys_grp_member_flds_cnt;

    /*! Initialize device-specific system trunk driver. */
    fn_sys_grp_init_t               fn_sys_grp_init;

    /*! Get system trunk group LT entry. */
    fn_sys_grp_get_t                fn_sys_grp_get;

    /*! Add imm trunk group LT entry. */
    fn_imm_grp_add_t                fn_imm_grp_add;

    /*! Lookup imm trunk group LT entry. */
    fn_imm_grp_lookup_t             fn_imm_grp_lookup;
    ...
    /*! Get the member fields count for LT entry. */
    fn_imm_grp_member_flds_cnt_t    fn_imm_grp_member_flds_cnt;
    ...
    /*! Get an TRUNK_VP group LT entry from the hardware tables. */
    fn_vp_grp_get_t                 grp_get;
} bcmcth_trunk_drv_t;
...
/*!
 * \brief Get system trunk group entry.
 *
 * \param [in] unit Logical device id.
 * \param [in] op_arg Operation arguments.
 * \param [in] lt_id Logical table id.
 ...
 */
extern int
bcmcth_trunk_drv_sys_grp_get(int unit,
                             const bcmltd_op_arg_t *op_arg,
                             bcmltd_sid_t lt_id,
                             bcmcth_trunk_sys_grp_bk_t *grp_bk,
                             bcmcth_trunk_sys_grp_param_t *param);
...
/*!
 * \brief Lookup trunk group info from hardware readout.
 *
 * \param [in] unit Logical device id.
 * \param [in] op_arg Operation arguments.
 * \param [in] lt_id Logical table id.
