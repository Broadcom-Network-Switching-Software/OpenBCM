...
/*! HA subcomponent ID for MON_FLOWTRACKER_LEARN_EVENT_LOG */
#define BCMMON_FLOWTRACKER_LEARN_EVENT_LOG_INFO_SUB_COMP_ID (8)
 ...
 * \param [in] index Index of memory entry to read.
 * \param [in] read_from_hw Directly read from HW or PT cache.
 * \param [out] entry_data Returned data value of entry.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
...
bcmcth_mon_pt_iread_pipe(int unit, bcmltd_sid_t lt_id,
        bcmdrd_sid_t pt_id, int pipe, int index, bool read_from_hw,
        void *entry_data);
