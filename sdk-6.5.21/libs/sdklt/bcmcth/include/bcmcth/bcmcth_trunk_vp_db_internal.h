...
#define BCMCTH_TRUNK_VP_RH_INFO_PTR_ARRAY_SIZE (4096)
...
typedef enum bcmcth_trunk_vp_lt_fields_e {
    ...
    /*!
     * Number of active TRUNK_VP paths configured for
     * the resilient hashing group field is set.
     */
    BCMCTH_TRUNK_VP_LT_FIELD_RH_MEMBER_CNT,

    /*!
     * The resilient hashing load balancing mode group
...
typedef struct bcmcth_trunk_vp_grp_ltd_fields_t_s {
    ...
    bcmcth_trunk_vp_ltd_fid_t rh_size_fid;

    /*! Resilient hashing group seed. */
    bcmcth_trunk_vp_ltd_fid_t rh_rand_seed_fid;

    /*!
     * Number of resilient hashing TRUNK_VP paths configured LTD field
     * identifier.
     */
    bcmcth_trunk_vp_ltd_fid_t rh_member_cnt_fid;
...
typedef struct bcmcth_trunk_vp_lt_field_attrs_s {
    ...
    /*!
     * Minimum value for configured resilient hashing TRUNK_VP paths allowed for
     * groups.
     */
    uint32_t min_rh_nmembers;

    /*!
     * Maximum value for configured resilient hashing TRUNK_VP paths allowed for
     * groups.
     */
    uint32_t max_rh_nmembers;

    /*! Minimum resilient TRUNK_VP group size value supported. */
    uint32_t min_rh_size;

    /*! Maximum resilient TRUNK_VP group size value supported. */
    uint32_t max_rh_size;

    /*! Resilient TRUNK_VP group size supported for the logical table. */
    uint32_t rh_size[BCMCTH_TRUNK_VP_RH_SIZE_COUNT];
} bcmcth_trunk_vp_lt_field_attrs_t;


/*!
 * Maximum TRUNK_VP fields value fields imported from
 * logical table ING_L3_TRUNK_VP_MEMBER_0_TABLE.
 */
#define BCMCTH_TRUNK_VP_MAX_MEMBER_FLDS (4)

/*!
 * \brief Structure for Resilient hashing TRUNK_VP group information.
 */
typedef struct bcmcth_trunk_vp_rh_grp_info_s {

    /*! Array of RH-TRUNK_VP group entry's mapped member's array index. */
    uint32_t *member_field[BCMCTH_TRUNK_VP_MAX_MEMBER_FLDS];
...
typedef struct bcmcth_trunk_vp_lt_entry_s {
    ...
    uint32_t member0_field[BCMCTH_TRUNK_VP_MAX_MEMBER_FLDS][BCMCTH_TRUNK_VP_NHOP_ARRAY_SIZE];

    /*! Input member fields count. */
    uint32_t member_count[BCMCTH_TRUNK_VP_MAX_MEMBER_FLDS];

    /*!
     * Resilient TRUNK_VP  group size value in
     * member entries count.
     */
    uint32_t rh_size;

    /*!
     * Numumber of paths configured for this TRUNK_VP
     * resilient hashing group field value.
     */
    uint32_t rh_member_cnt;

    /*!
     * Resilient hashing group size TRUNK_VP
     * value in member entries count.
     */
    uint32_t rh_entries_cnt;

    /*!
     * The resilient hashing load balancing mode
     * TRUNK_VP group random number
     * generator seed value.
     */
    uint32_t rh_rand_seed;

    /*!
     * Array of RH TRUNK_VP group entry's
     ...
     * by TRUNK_VP member in a
     * resilient hashing load balancing mode group.
     */
    uint32_t *rh_memb_ecnt_arr;

    /*!
     * Array of resilient hashing TRUNK_VP
     * group load balanced members.
     */
    uint32_t *rh_entries_arr[BCMCTH_TRUNK_VP_MAX_MEMBER_FLDS];
...
typedef struct bcmcth_trunk_vp_grp_info_s {
    ...
    /*!
     * Resilient hashing group size configured
     * for this group - HW encoding.
     */
    uint32_t rh_size_enc;

    /*!
     *  Maximum resilient hashing group size
     * by this group.
     */
    uint32_t rh_entries_cnt;
...
typedef struct bcmcth_trunk_vp_info_s {
    ...
    /*!
     * Resilient hashing load balancing
     * mode pseudo-random number generator.
     */
    uint32_t rh_rand_seed;

     /*! TRUNK VP resilient group information. */
    void *rh_grp_arr;

    /*! TRUNK VP resilient group entires backup array. */
    void *rh_grp_bk_arr;

    /*! Resilient hashing TRUNK VP group mode details. */
    void *rhg[BCMCTH_TRUNK_VP_RH_INFO_PTR_ARRAY_SIZE];

    /*! Resilient hashing TRUNK_VP group mode details. */
    void *rhg_bk[BCMCTH_TRUNK_VP_RH_INFO_PTR_ARRAY_SIZE];
...
/*!
 * \brief Minimum value supported for RH_MEMBER_CNT field of the logical table.
 *
 * \param [in] _u BCM unit number.
 * \param [in] _g TRUNK_VP Group table array of bcmecmp_tbl_prop_t type.
 * \param [in] _t TRUNK_VP Group type.
 *
 * \returns Minimum value supported for RH_MEMBER_CNT
 *          field of the logical table.
 */
#define BCMCTH_TRUNK_VP_LT_MIN_RH_NMEMBERS(_u, _g, _t) \
            (BCMCTH_TRUNK_VP_LT_FIELD_ATTRS_PTR(_u, _g, _t)->min_rh_nmembers)

/*!
 * \brief Maximum value supported for RH_MEMBER_CNT field of the logical table.
 *
 * \param [in] _u BCM unit number.
 * \param [in] _g TRUNK_VP Group table array of bcmecmp_tbl_prop_t type.
 * \param [in] _t TRUNK_VP Group type.
 *
 * \returns Maximum value supported for RH_MEMBER_CNT
 *          field of the logical table.
 */
#define BCMCTH_TRUNK_VP_LT_MAX_RH_NMEMBERS(_u, _g, _t) \
            (BCMCTH_TRUNK_VP_LT_FIELD_ATTRS_PTR(_u, _g, _t)->max_rh_nmembers)
...
#define BCMCTH_TRUNK_VP_TBL_ENT_DEFRAG_STAGED_SET(_tptr, _i)    \
            (((_tptr)->ent_arr[(_i)]).flags |= \
              (BCMCTH_TRUNK_VP_ENTRY_DEFRAG_STAGED))
...
/*!
 * \brief Minimum value supported for resilient TRUNK_VP group size field of the
 * logical table.
 *
 * \param [in] _u BCM unit number.
 * \param [in] _g TRUNK_VP Group table array of bcmcth_trunk_vp_tbl_prop_t type.
 ...
 * supported by the logical table.
 */
#define BCMCTH_TRUNK_VP_LT_MIN_RH_SIZE(_u, _g, _t) \
            (BCMCTH_TRUNK_VP_LT_FIELD_ATTRS_PTR(_u, _g, _t)->min_rh_size)

/*!
 * \brief Maximum value supported for resilient TRUNK_VP group size field of the
 * logical table.
 *
 * \param [in] _u BCM unit number.
 * \param [in] _g TRUNK_VP Group table array of bcmcth_trunk_vp_tbl_prop_t type.
 * \param [in] _t TRUNK_VP Group type.
 *
 * \returns Maximum resilient TRUNK_VP group size value
 * supported by the logical table.
 */
#define BCMCTH_TRUNK_VP_LT_MAX_RH_SIZE(_u, _g, _t) \
            (BCMCTH_TRUNK_VP_LT_FIELD_ATTRS_PTR(_u, _g, _t)->max_rh_size)
 ...
 */
#define BCMCTH_TRUNK_VP_GRP_RH_ENTRIES_CNT(_u, _id) \
            ((BCMCTH_TRUNK_VP_GRP_RH_INFO_ENT_PTR(_u,_id))->rh_entries_cnt)
 ...
 */
#define BCMCTH_TRUNK_VP_GRP_RHG_PTR(_u, _id) \
             ((bcmcth_trunk_vp_rh_grp_info_t *)(BCMCTH_TRUNK_VP_INFO_PTR(_u)->rhg[(_id)]))
 ...
 * \returns Pointer to resilient hashing group data.
 */
#define BCMCTH_TRUNK_VP_GRP_VOID_RHG_PTR(_u, _id) \
             (BCMCTH_TRUNK_VP_INFO_PTR(_u)->rhg[(_id)])
/*!
 ...
 */
#define BCMCTH_TRUNK_VP_GRP_RH_ENTRIES_PTR(_u, _id, _fld_idx) \
            (BCMCTH_TRUNK_VP_GRP_RHG_PTR(_u, _id)->member_field[_fld_idx])
 ...
 */
#define BCMCTH_TRUNK_VP_GRP_RH_MINDEX_PTR(_u, _id) \
            (BCMCTH_TRUNK_VP_GRP_RHG_PTR(_u, _id)->rh_mindex_arr)
 ...
 */
#define BCMCTH_TRUNK_VP_GRP_RH_MEMB_ENTCNT_PTR(_u, _id) \
            (BCMCTH_TRUNK_VP_GRP_RHG_PTR(_u, _id)->rh_memb_ecnt_arr)
 ...
 * \returns Pointer to resilient hashing group data.
 */
#define BCMCTH_TRUNK_VP_GRP_RHG_BK_PTR(_u, _id) \
            ((bcmcth_trunk_vp_rh_grp_info_t *)(BCMCTH_TRUNK_VP_INFO_PTR(_u)->rhg_bk[(_id)]))
 ...
 * \returns Pointer to resilient hashing group data.
 */
#define BCMCTH_TRUNK_VP_GRP_VOID_RHG_BK_PTR(_u, _id) \
            (BCMCTH_TRUNK_VP_INFO_PTR(_u)->rhg_bk[(_id)])
 ...
 * \returns Resilient hashing group total number of members count.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_ENTRIES_CNT_BK(_u, _id) \
            ((BCMCTH_TRUNK_VP_GRP_RH_INFO_ENT_BK_PTR(_u,_id))->rh_entries_cnt)
 ...
 * \returns Pointer to resilient hash load balanced members array.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_ENTRIES_BK_PTR(_u, _id, _idx) \
            (BCMCTH_TRUNK_VP_GRP_RHG_BK_PTR(_u, _id)->member_field[_idx])
 ...
 * \returns Number of member table entries occupied by a RH group member.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_MEMB_ENTCNT_BK_PTR(_u, _id) \
            (BCMCTH_TRUNK_VP_GRP_RHG_BK_PTR(_u, _id)->rh_memb_ecnt_arr)
 ...
 * \returns Pointer to resilient hash load balanced member indices array.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_MINDEX_BK_PTR(_u, _id) \
            (BCMCTH_TRUNK_VP_GRP_RHG_BK_PTR(_u, _id)->rh_mindex_arr)

/*!
 * \brief TRUNK_VP flex group array entry pointer.
 *
 * \param [in] _u BCM unit number.
 * \param [in] _id TRUNK_VP group identifier.
 *
 * \returns TRUNK_VP group array base pointer.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_INFO_ENT_PTR(_u,_id) \
           (((bcmcth_trunk_vp_grp_rh_attr_t *) \
            ((BCMCTH_TRUNK_VP_INFO_PTR(_u))->rh_grp_arr)) + _id)

/*!
 * \brief TRUNK_VP flex backup group array entry pointer.
 *
 * \param [in] _u BCM unit number.
 * \param [in] _id TRUNK_VP group identifier.
 *
 * \returns TRUNK_VP group array base pointer.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_INFO_ENT_BK_PTR(_u,_id) \
           (((bcmcth_trunk_vp_grp_rh_attr_t *) \
            ((BCMCTH_TRUNK_VP_INFO_PTR(_u))->rh_grp_bk_arr)) + _id)

/*!
 * \brief RH TRUNK_VP group array base pointer.
 *
 * \param [in] _u BCM unit number.
 *
 * \returns RH TRUNK_VP group array base pointer.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_INFO_PTR(_u) \
            ((BCMCTH_TRUNK_VP_INFO_PTR(_u))->rh_grp_arr)

/*!
 * \brief RH TRUNK_VP group backup array base pointer.
 *
 * \param [in] _u BCM unit number.
 *
 * \returns RH TRUNK_VP group backup array base pointer.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_INFO_BK_PTR(_u) \
            ((BCMCTH_TRUNK_VP_INFO_PTR(_u))->rh_grp_bk_arr)
 ...
 */
#define BCMCTH_TRUNK_VP_GRP_RH_RAND_SEED(_u, _id) \
            ((BCMCTH_TRUNK_VP_GRP_RH_INFO_ENT_PTR(_u,_id))->rh_rand_seed)
 ...
 * \returns Device resilient hashing pseudo-random number generator seed value.
 */
#define BCMCTH_TRUNK_VP_INIT_RH_RAND_SEED(_u, _id) \
            (BCMCTH_TRUNK_VP_LT_ENT_PTR(_u)->rh_rand_seed)
 ...
 * \returns Device resilient hashing pseudo-random number generator seed value.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_RAND_SEED_BK(_u, _id) \
            ((BCMCTH_TRUNK_VP_GRP_RH_INFO_ENT_BK_PTR(_u,_id))->rh_rand_seed)



/*!
 * \brief Get the RH mode of a given TRUNK_VP group.
 *
 * \param [in] _u BCM unit number.
 * \param [in] _id TRUNK_VP group identifier.
 *
 * \returns Load balancing mode of the TRUNK_VP group.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_MODE_BK(_u, _id) \
            ((BCMCTH_TRUNK_VP_GRP_RH_INFO_ENT_BK_PTR(_u,_id))->rh_mode)
...
static inline uint32_t bcmcth_trunk_vp_member_cnt_convert_to_rh_size
                           (uint32_t member_cnt) {
    uint32_t rh_size;

    switch (member_cnt) {
        case 64:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_64;
            break;
        case 128:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_128;
            break;
        case 256:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_256;
            break;
        case 512:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_512;
            break;
        case 1024:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_1K;
            break;
        case 2048:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_2K;
            break;
        case 4096:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_4K;
            break;
        case 8192:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_8K;
            break;
        case 16384:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_16K;
            break;
        default:
            rh_size = BCMCTH_TRUNK_VP_RH_SIZE_COUNT;
            break;
    }
...
static inline uint32_t bcmcth_trunk_vp_rh_size_convert_to_member_cnt
                           (uint32_t rh_size) {
    uint32_t member_cnt;

    switch (rh_size) {
        case BCMCTH_TRUNK_VP_RH_SIZE_64:
            member_cnt = 64;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_128:
            member_cnt = 128;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_256:
            member_cnt = 256;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_512:
            member_cnt = 512;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_1K:
            member_cnt = 1024;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_2K:
            member_cnt = 2048;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_4K:
            member_cnt = 4096;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_8K:
            member_cnt = 8192;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_16K:
            member_cnt = 16*1024;
            break;
        default:
            member_cnt = 0;
...
static inline int
bcmcth_trunk_vp_rh_size_convert_to_lb_mode(
                          int unit,
                          uint32_t rh_size) {
    uint32_t lb_mode = BCMCTH_TRUNK_VP_LB_MODE_REGULAR;

    switch (rh_size) {
        case BCMCTH_TRUNK_VP_RH_SIZE_64:
        case BCMCTH_TRUNK_VP_RH_SIZE_128:
            lb_mode = BCMCTH_TRUNK_VP_LB_MODE_REGULAR;
            break;
        case BCMCTH_TRUNK_VP_RH_SIZE_256:
        case BCMCTH_TRUNK_VP_RH_SIZE_512:
        case BCMCTH_TRUNK_VP_RH_SIZE_1K:
