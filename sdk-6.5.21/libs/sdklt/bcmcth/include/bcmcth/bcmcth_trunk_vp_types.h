...
/*!
 * HA sub-component ID for staged
 * TRUNK_VP groups configuration block.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_SUB_COMP_ID       (14)

/*!
 * HA sub-component ID for committed
 * TRUNK_VP groups configuration block.
 */
#define BCMCTH_TRUNK_VP_GRP_RH_BK_SUB_COMP_ID    (15)
...
/*!
 * \brief TRUNK_VP resilient group size.
 */
typedef enum bcmcth_trunk_vp_rh_size_e {
    BCMCTH_TRUNK_VP_RH_SIZE_64 = 0,
    BCMCTH_TRUNK_VP_RH_SIZE_128,
    BCMCTH_TRUNK_VP_RH_SIZE_256,
    BCMCTH_TRUNK_VP_RH_SIZE_512,
    BCMCTH_TRUNK_VP_RH_SIZE_1K,
    BCMCTH_TRUNK_VP_RH_SIZE_2K,
    BCMCTH_TRUNK_VP_RH_SIZE_4K,
    BCMCTH_TRUNK_VP_RH_SIZE_8K,
    BCMCTH_TRUNK_VP_RH_SIZE_16K,
    BCMCTH_TRUNK_VP_RH_SIZE_COUNT
} bcmcth_trunk_vp_rh_size_t;

/*! \brief weighted TRUNK_VP group size names. */
#define BCMCTH_TRUNK_VP_RH_SIZE_NAME {  \
            "RH_64",      \
            "RH_128",      \
            "RH_256",      \
            "RH_512",      \
 ...
 * \brief Verify if group type is weighted.
 *
 * Given a LB_MODE check if the group is weighted group.
 *
 * \param [in] _lb_mode Load balancing mode.
 *
 * \returns True if group is resilient else false.
 */
/*! Check if the group is weighted. */
#define BCMCTH_TRUNK_VP_GRP_IS_RH(_lb_mode) \
            ((_lb_mode) == BCMCTH_TRUNK_VP_LB_MODE_RESILIENT)
...
/*!
 * \brief TRUNK_VP resilient hashing group members operation type enum.
 */
typedef enum bcmcth_trunk_vp_rh_oper_e {
    /*! No operation to be performed on RH group members. */
    BCMCTH_TRUNK_VP_RH_OPER_NONE = 0,

    /*! RH group members add operation. */
    BCMCTH_TRUNK_VP_RH_OPER_ADD = 1,

    /*! RH group members delete operation. */
    BCMCTH_TRUNK_VP_RH_OPER_DELETE = 2,

    /*! RH group members replace operation. */
    BCMCTH_TRUNK_VP_RH_OPER_REPLACE = 3,

    /*! RH group members add and replace operation. */
    BCMCTH_TRUNK_VP_RH_OPER_ADD_REPLACE = 4,

    /*! RH group members delete and replace operation. */
    BCMCTH_TRUNK_VP_RH_OPER_DELETE_REPLACE = 5,

    /*! Must be last one. */
    BCMCTH_TRUNK_VP_RH_OPER_COUNT = 6
} bcmcth_trunk_vp_rh_oper_t;
