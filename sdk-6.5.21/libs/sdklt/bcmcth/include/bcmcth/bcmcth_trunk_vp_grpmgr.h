...
/*!
 * \brief Cleanup the resilient hashing group info data.
 *
 ...
 *
 * For all RH TRUNK_VP groups that have non-zero NUM_PATHS value set during
 * insert operation in cold boot mode, their load balanced members state has to
 * be reconstructed for use during update operation post warm boot. This
 * function identifies such RH TRUNK_VP groups during warm start pre_config
 * sequence.
 *
 * \param [in] unit Unit number.
 *
 ...
 * from TRUNK_VP member pool resources.
 *
 * \param [in] unit Unit number.
