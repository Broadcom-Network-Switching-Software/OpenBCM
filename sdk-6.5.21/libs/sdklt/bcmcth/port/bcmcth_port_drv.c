...
/*! Define stub functions for PORT_GIH_CPU info base variant. */
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
bcmcth_port_info_drv_t *_bc##_cth_port_gih_cpu_info_drv_get(int unit) {return NULL;}
#include <bcmdrd/bcmdrd_devlist.h>

/*! PORT_GIH_CPU info table drive get. */
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    { _bd##_vu##_va##_cth_port_gih_cpu_info_drv_get},
static struct {
    bcmcth_port_info_drv_get_f drv_get;
} port_gih_cpu_info_drv_get[] = {
    { 0 },
#include <bcmlrd/chip/bcmlrd_variant.h>
    { 0 }
};
...
static bcmcth_port_info_drv_t
*port_gih_cpu_info_drv[BCMDRD_CONFIG_MAX_UNITS];
...
bcmcth_port_drv_init(int unit, bool warm)
{
    ...
    /* Perform port_gih_cpu_info table setup */
    data.table_id = 0xFFFFFFFF;
    port_gih_cpu_info_drv[unit] =
        port_gih_cpu_info_drv_get[variant].drv_get(unit);

    if (port_gih_cpu_info_drv[unit] == NULL) {
        SHR_EXIT();
    }
    if (port_gih_cpu_info_drv[unit]->get_data) {
        SHR_IF_ERR_EXIT
            (port_gih_cpu_info_drv[unit]->get_data(unit, &data));
    } else {
        SHR_EXIT();
    }
