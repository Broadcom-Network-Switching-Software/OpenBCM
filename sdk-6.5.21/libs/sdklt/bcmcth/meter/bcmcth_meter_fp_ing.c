/*! \file bcmcth_meter_fp_ing.c
 *
 * This file contains IFP Meter custom handler functions.
 */
/*
...
typedef bcmcth_meter_fp_drv_t *(*bcmcth_meter_fp_ing_drv_get_f)(int unit);

/*! Array of device specific attach functions */
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
{ _bc##_cth_meter_fp_ing_drv_get },
static struct {
    bcmcth_meter_fp_ing_drv_get_f drv_get;
} meter_fp_ing_drv_get[] = {
...
/*! IFP meter driver. */
static bcmcth_meter_fp_drv_t *meter_fp_ing_drv[BCMDRD_CONFIG_MAX_UNITS];
...
bcmcth_meter_fp_ing_drv_init(int unit)
{
    ...
    meter_fp_ing_drv[unit] = meter_fp_ing_drv_get[dev_type].drv_get(unit);
...
int
bcmcth_meter_fp_ing_init(int unit, bool warm)
{
    bcmcth_meter_fp_sw_state_t *ifp_sw_state = NULL;
    ...
    SHR_IF_ERR_EXIT
        (bcmcth_meter_fp_ing_sw_state_get(unit, &ifp_sw_state));
    ...
    ifp_sw_state->bkp_list =
        shr_ha_mem_alloc(unit,
                         BCMMGMT_METER_COMP_ID,
                         METER_FP_ING_BKP_SUB_COMP_ID,
                         "bcmcthMeterFpIngList",
                         &ha_alloc_size);
    SHR_NULL_CHECK(ifp_sw_state->bkp_list, SHR_E_MEMORY);
    SHR_IF_ERR_EXIT((ha_alloc_size < ha_req_size) ?
                    SHR_E_MEMORY : SHR_E_NONE);

    if (!warm) {
        sal_memset(ifp_sw_state->bkp_list, 0, ha_alloc_size);

        bcmissu_struct_info_report(unit, BCMMGMT_METER_COMP_ID,
                                         METER_FP_ING_BKP_SUB_COMP_ID, 0,
                                         sizeof(bcmcth_meter_fp_entry_t),
                                         num_sw_meters,
                                         BCMCTH_METER_FP_ENTRY_T_ID);
    }
    ifp_sw_state->bkp_list[0].meter_in_use = true;

    /*
     * Allocate memory for ING meter table.
     */
    SHR_ALLOC(ifp_sw_state->list,
              ha_alloc_size,
              "bcmcthMeterFpIngSwState");
    SHR_NULL_CHECK(ifp_sw_state->list, SHR_E_MEMORY);
    sal_memset(ifp_sw_state->list, 0, ha_alloc_size);

    sal_memcpy(ifp_sw_state->list,
               ifp_sw_state->bkp_list,
               ha_alloc_size);
    ...
    ifp_sw_state->bkp_hw_idx =
        shr_ha_mem_alloc(unit,
                         BCMMGMT_METER_COMP_ID,
                         METER_FP_ING_HW_IDX_BKP_SUB_COMP_ID,
                         "bcmcthMeterFpIngHwIdx",
                         &ha_alloc_size);
    SHR_NULL_CHECK(ifp_sw_state->bkp_hw_idx, SHR_E_MEMORY);
    SHR_IF_ERR_EXIT((ha_alloc_size < ha_req_size) ?
                    SHR_E_MEMORY : SHR_E_NONE);

    if (!warm) {
        sal_memset(ifp_sw_state->bkp_hw_idx, 0, ha_alloc_size);
    }

    /*
     * Allocate memory for IFP meter table H/W index usage.
     */
    SHR_ALLOC(ifp_sw_state->hw_idx,
              ha_alloc_size,
              "bcmcthMeterFpIngHwIdxState");
    SHR_NULL_CHECK(ifp_sw_state->hw_idx, SHR_E_MEMORY);
    sal_memset(ifp_sw_state->hw_idx, 0, ha_alloc_size);

    sal_memcpy(ifp_sw_state->hw_idx,
               ifp_sw_state->bkp_hw_idx,
               ha_alloc_size);

exit:
    if (SHR_FUNC_ERR() && ifp_sw_state) {
        if (ifp_sw_state->bkp_hw_idx != NULL) {
            shr_ha_mem_free(unit, ifp_sw_state->bkp_hw_idx);
            ifp_sw_state->bkp_hw_idx = NULL;
        }
        if (ifp_sw_state->bkp_list != NULL) {
            shr_ha_mem_free(unit, ifp_sw_state->bkp_list);
            ifp_sw_state->bkp_list = NULL;
        }
        SHR_FREE(ifp_sw_state->hw_idx);
        SHR_FREE(ifp_sw_state->list);
    }
    SHR_FUNC_EXIT();
}
...
{
    bcmcth_meter_fp_sw_state_t *ifp_sw_state;
                                 ...
                                 void *context)
{
    ...
    active_ptr = ifp_sw_state->list;
    bkp_ptr = ifp_sw_state->bkp_list;
    hw_idx = ifp_sw_state->hw_idx;
    bkp_hw_idx = ifp_sw_state->bkp_hw_idx;
    ...
    table_size = (attr->num_pipes * attr->num_meters_per_pipe *
                  sizeof(bool));
                                ...
                                void *context)
{
    ...
    if (SHR_FAILURE(bcmcth_meter_fp_ing_sw_state_get(unit, &ifp_sw_state))) {
        return;
    }
                            ...
                            bcmcth_meter_fp_drv_t **drv)
{
    ...
    if (meter_fp_ing_drv[unit] != NULL) {
        *drv = meter_fp_ing_drv[unit];
    } else {
