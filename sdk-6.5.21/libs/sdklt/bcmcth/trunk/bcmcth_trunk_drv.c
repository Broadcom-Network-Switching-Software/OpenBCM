...
int
bcmcth_trunk_drv_sys_grp_lookup(int unit,
                                const bcmltd_op_arg_t *op_arg,
                                bcmltd_sid_t lt_id,
                                bcmcth_trunk_sys_grp_bk_t *grp_bk,
                                bcmcth_trunk_sys_grp_param_t *param)
{
    fn_sys_grp_lookup_t my_fn;

    if ((my_fn = trunk_drv[unit]->fn_sys_grp_lookup) == NULL) {
        return SHR_E_UNAVAIL;
    } else {
...
int
bcmcth_trunk_drv_sys_grp_member_flds_cnt_get(int unit)
{
    fn_sys_grp_member_flds_cnt_t my_fn;

    if ((my_fn = trunk_drv[unit]->fn_sys_grp_member_flds_cnt) == NULL) {
        return SHR_E_UNAVAIL;
    } else {
        return my_fn(unit);
    }
...
int
bcmcth_trunk_drv_imm_grp_lookup(int unit,
                                const bcmltd_op_arg_t *op_arg,
                                bcmltd_sid_t lt_id,
                                bcmcth_trunk_imm_grp_bk_t *grp_bk,
                                bcmcth_trunk_group_param_t *param)
{
    fn_imm_grp_lookup_t my_fn;

    if ((my_fn = trunk_drv[unit]->fn_imm_grp_lookup) == NULL) {
        return SHR_E_UNAVAIL;
    } else {
        return my_fn(unit, op_arg, lt_id, grp_bk, param);
    }
...
int
bcmcth_trunk_drv_imm_grp_member_flds_cnt_get(int unit)
{
    fn_imm_grp_member_flds_cnt_t my_fn;

    if ((my_fn = trunk_drv[unit]->fn_imm_grp_member_flds_cnt) == NULL) {
        return SHR_E_UNAVAIL;
    } else {
        return my_fn(unit);
    }
...
/*!
 * \brief Get TRUNK_VP group driver function.
 *
 * \param [in] unit BCM unit number.
 * \param [in,out] info lt entry information
 *
 ...
 */
int
bcmcth_trunk_drv_vp_grp_get(int unit, void *info)
{
    fn_vp_grp_get_t my_fn;

    if ((my_fn = trunk_drv[unit]->grp_get) == NULL) {
        return SHR_E_UNAVAIL;
    } else {
        return my_fn(unit, info);
    }
