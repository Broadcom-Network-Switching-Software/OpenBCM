                        ...
                        bcmcth_trunk_vp_lt_entry_t *lt_entry)
{
    ...
    if (lt_entry) {
        ...
        if (BCMCTH_TRUNK_VP_GRP_IS_WEIGHTED(lt_entry->grp_type)) {
             ...
             */
            lt_entry->lb_mode = BCMCTH_TRUNK_VP_LT_MIN_WEIGHTED_SIZE(
                                                     unit,
                                                     group,
                                                     lt_entry->grp_type);
            /*
            ...
            /*
             * Initialize resilient hashing group size value to device
             * minimum resilient hashing group size value.
             */
            lt_entry->rh_size = BCMCTH_TRUNK_VP_LT_MIN_RH_SIZE
                                          (unit,
                                           group,
                                           lt_entry->grp_type);
        }
        ...
        if (BCMCTH_TRUNK_VP_GRP_IS_WEIGHTED(lt_entry->grp_type)) {
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                   (bcmlrd_field_default_get(
                                     ...
                                     vp_drv_var->ids->trunk_vp_max_members_id,
                                     1,
                                     &def_val,
                                     &num));
            lt_entry->max_members = (uint16_t)def_val;
        }
        ...
        /* Initialize rh configured paths to zero. */
        lt_entry->rh_member_cnt = 0;

         /* Initialize rh max member entries count  to zero. */
        lt_entry->rh_entries_cnt = 0;
                                 ...
                                 bcmcth_trunk_vp_lt_entry_t *lt_entry)
{
    ...
    bcmcth_trunk_vp_lt_entry_t *cur_lt_entry = NULL; /* imm current entry. */
    bool in_use;
    uint32_t comp_id;
    bool is_rh_gtype = 0; /* Flag indicating if group is resilient. */
    ...
    switch (opcode) {
            ...
            if (is_rh_gtype) {
                if (lt_entry->rh_member_cnt > lt_entry->max_members) {
                    LOG_VERBOSE(
                       BSL_LOG_MODULE,
                       (BSL_META_U(unit,
                       "RH_MEMBER_CNT=%u exceeds RH_SIZE=%u.\n"),
                       lt_entry->member_cnt,
        ...
        case BCMIMM_ENTRY_UPDATE:
            ...
            /*
             * Verify specified non-zero RH_MEMBER_CNT value is a power-of-two
             * value for RESILIENT groups
             */
            if (BCMCTH_TRUNK_VP_LT_FIELD_GET(lt_entry->fbmp,
                BCMCTH_TRUNK_VP_LT_FIELD_RH_MEMBER_CNT)
                && BCMCTH_TRUNK_VP_LT_POW_TWO_PATHS(unit, group, gtype)
                && lt_entry->rh_member_cnt
                && (lt_entry->rh_member_cnt & (lt_entry->rh_member_cnt - 1))) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                             "RH_MEMBER_CNT=%u not a power-of-two.\n"),
                             lt_entry->rh_member_cnt));
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
            ...
            if (is_rh_gtype) {
                ...
                /* Verify specified RH_SIZE value is in valid range. */
                if (BCMCTH_TRUNK_VP_LT_FIELD_GET(lt_entry->fbmp,
                    BCMCTH_TRUNK_VP_LT_FIELD_RH_SIZE)
                    && (lt_entry->max_members < cur_lt_entry->rh_member_cnt)
                    && !update_member_cnt_max_members) {
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit,
                                            "RH_SIZE=%u is invalid.\n"),
                                (bcmcth_trunk_vp_member_cnt_convert_to_rh_size(
                                   lt_entry->max_members))));
                    SHR_ERR_EXIT(SHR_E_PARAM);
                }

                /*
                 * Verify RH_MEMBER_CNT value is in valid range
                 * if it's non-zero.
                 */
                if (BCMCTH_TRUNK_VP_LT_FIELD_GET
                    (lt_entry->fbmp, BCMCTH_TRUNK_VP_LT_FIELD_RH_MEMBER_CNT)
                    && lt_entry->rh_member_cnt
                    && (lt_entry->rh_member_cnt > cur_lt_entry->max_members)
                    && !update_member_cnt_max_members) {
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit,
                                            "RH_MEMBER_CNT=%u is invalid "
                                            "(Grp RH_SIZE=%u).\n"),
                                lt_entry->rh_member_cnt,
                                bcmcth_trunk_vp_member_cnt_convert_to_rh_size(
                                cur_lt_entry->max_members)));
                    SHR_ERR_EXIT(SHR_E_PARAM);
                }
                ...
                /*
                 * Verify MEMBER_CNT value is in valid range if
                 * it's non-zero.
                 */
                if (BCMCTH_TRUNK_VP_LT_FIELD_GET
                        (lt_entry->fbmp, BCMCTH_TRUNK_VP_LT_FIELD_MEMBER_CNT)
                    && lt_entry->member_cnt
                    && (lt_entry->member_cnt > cur_lt_entry->max_members)
                        && !update_member_cnt_max_members) {
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit,
                                            "MEMBER_CNT=%u is invalid "
            ...
            if (is_rh_gtype) {
               ...
               if (lt_entry->member_cnt > lt_entry->max_members) {
                    if (BCMCTH_TRUNK_VP_GRP_IS_WEIGHTED(gtype)) {
                        LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U(unit,
                         "MEMBER_CNT=%u should be propotional"\
                         " to the weighted size.\n"),
                           lt_entry->member_cnt));
                    } else {
                         LOG_VERBOSE(BSL_LOG_MODULE,
                                    (BSL_META_U(unit,
 ...
 */
int
bcmcth_trunk_vp_lt_fields_lookup(
                          int unit,
                          bcmcth_trunk_vp_lt_entry_t *lt_entry)
{
    ...
    bcmcth_trunk_vp_grp_type_t gtype = BCMCTH_TRUNK_VP_GRP_TYPE_COUNT;
    /* Get the trunk drive. */
    bcmcth_trunk_drv_var_t *drv_var = bcmcth_trunk_drv_var_get(unit);
    const bcmcth_trunk_vp_drv_var_t *vp_drv_var= NULL;
    ...
    /* Get the count member fields for the TRUNK_VP/TRUNK_VP_WEIGHTED lt. */
    if (lt_entry->glt_sid ==
        (uint32_t) vp_drv_var->ids->trunk_vp_ltid) {
        if (vp_drv_var->member0_info) {
            member_count =
                vp_drv_var->member0_info->flds_count;
        }
        if (vp_drv_var->rmember0_info) {
            member_count +=
            vp_drv_var->rmember0_info->flds_count;
        }
    } else if (lt_entry->glt_sid ==
        (uint32_t) vp_drv_var->ids->trunk_vp_weighted_ltid) {
        if (vp_drv_var->wmember0_info) {
            member_count =
            vp_drv_var->member0_info->flds_count;
        }
    } else {
    ...
    num_fields += member_count *
                  BCMCTH_TRUNK_VP_GRP_MAX_MEMBERS(unit, lt_entry->trunk_vp_id);
    ...
    if (rv == SHR_E_RESOURCE) {
        ...
        num_fields -= member_count *
                      BCMCTH_TRUNK_VP_GRP_MAX_MEMBERS(unit,
                      lt_entry->trunk_vp_id);
        /*
         * During entry lookup for TRUNK_VP LT
         * from imm if group supports
         * Resilient loaad balancing num_fields should be
         * multiplier of BCMCTH_TRUNK_VP_LT_MAX_RH_NMEMBERS
         * else BCMCTH_TRUNK_VP_LT_MAX_NMEMBERS.
         */
        if  ((lt_entry->glt_sid ==
            (uint32_t) vp_drv_var->ids->trunk_vp_ltid) &&
            (vp_drv_var->rmember0_info)) {
            num_fields += member_count *
               BCMCTH_TRUNK_VP_LT_MAX_RH_NMEMBERS(unit, group, gtype);
        } else {
            num_fields += member_count *
               BCMCTH_TRUNK_VP_LT_MAX_NMEMBERS(unit, group, gtype);
        }
