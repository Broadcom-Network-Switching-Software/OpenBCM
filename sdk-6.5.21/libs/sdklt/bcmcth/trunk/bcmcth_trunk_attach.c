...
bcmcth_trunk_vp_dev_init(int unit, bool warm)
{
     ...
     * used by the resilient TRUNK VP group.
     */
    BCMCTH_TRUNK_VP_ALLOC
        (BCMCTH_TRUNK_VP_CURRENT_LT_ENT_PTR(unit), sizeof(bcmcth_trunk_vp_lt_entry_t),
         "bcmcthTrunkVpCurLtEntInfo");
...
bcmcth_trunk_vp_dev_cleanup(int unit, bool warm)
{
    ...
    /*
     * Free memory allocated for the current LT entry buffer
     * used by the  resilient hashing group.
     */
    if (BCMCTH_TRUNK_VP_CURRENT_LT_ENT_PTR(unit)) {
        BCMCTH_TRUNK_VP_FREE(BCMCTH_TRUNK_VP_CURRENT_LT_ENT_PTR(unit));
    }
...
bcmcth_trunk_attach(int unit, bool warm)
{
    ...
    if (drv_var != NULL) {
        ...
        if (SHR_SUCCESS(rv) || SHR_SUCCESS(rv1)) {
            ...
            if (warm) {
               SHR_IF_ERR_EXIT(
                        bcmcth_trunk_vp_rh_groups_preconfig(unit));
                SHR_IF_ERR_EXIT(
...
bcmcth_trunk_detach(int unit, bool warm)
{
    ...
    if (drv_var != NULL) {
        ...
        if (SHR_SUCCESS(rv) || SHR_SUCCESS(rv1)) {
            if (vmap || wmap) {
                if (vmap) {
                    /* Cleanup the resilient VP trunk group resources. */
                    bcmcth_trunk_vp_rh_groups_cleanup(unit);
                }
                SHR_IF_ERR_VERBOSE_EXIT
