...
/*!
 * \brief TRUNK LT field array free
 *
 ...
 */
int
bcmcth_trunk_group_lt_fields_free(int unit,
                                  bcmltd_fields_t *in)
{
    uint32_t i;
...
/*!
 * \brief TRUNK LT fields allocate
 *
 * \param [in]  unit       This is device unit number.
 * \param [in]  num_fields The num of fields element.
 * \param [out] in         Ltd fields pointer.
 *
 * \retval None.
 */
int
bcmcth_trunk_group_lt_fields_allocate(int unit,
                                      size_t num_fields,
                                      bcmltd_fields_t *in)
{
    uint32_t i;
    ...
    BCMCTH_TRUNK_ALLOC(in->field, alloc_sz, "bcmctTrunkGroupEntryFields");

    in->count = num_fields;

    for (i = 0; i < num_fields; i++) {
        BCMCTH_TRUNK_ALLOC(in->field[i],
            sizeof(bcmltd_field_t), "bcmcthTrunkGroupEntryField");
    }

    SHR_FUNC_EXIT();

exit:
    bcmcth_trunk_group_lt_fields_free(unit, in);
