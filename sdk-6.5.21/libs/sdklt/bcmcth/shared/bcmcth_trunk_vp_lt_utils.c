                            ...
                            uint32_t *out_ha_alloc_sz)
{
    ...
    if ((!warm)  ||
        ...
        hw_array_ptr = (bcmcth_trunk_vp_grp_ha_blk_t *)
                        ((uint8_t *)hw_array_ptr + offset);
        SHR_IF_ERR_MSG_EXIT
            ((hw_array_ptr->signature != ref_sign
              ? SHR_E_FAIL : SHR_E_NONE),
...
/*!
 * \brief This function allocates HA memory for storing resilient group attr.
 *
 * \param [in] unit Unit number.
 * \param [in] warm Indicates cold or warm start status.
 * \param [in] array_size hw entries array size.
 ...
 */
int
bcmcth_trunk_vp_group_rh_attr_ha_alloc(
                            int unit,
                            bool warm,
                            uint32_t array_size,
                            uint8_t submod_id,
                            uint32_t ref_sign,
                            const char *ha_blk_id,
                            bcmcth_trunk_vp_grp_rh_ha_blk_t **alloc_ptr,
                            uint32_t *out_ha_alloc_sz)
{
    bcmcth_trunk_vp_grp_rh_ha_blk_t *hw_array_ptr = NULL;
    uint32_t ha_alloc_sz = 0; /* Size of allocated HA memory block. */
    uint32_t in_req_size = 0;
    uint32_t offset = 0;
    ...
    SHR_FUNC_ENTER(unit);
    /*
     * Compiler treat bcmcth_trunk_vp_grp_rh_ha_blk_t.array
     * as empty array(size=0).
     */
    in_req_size = sizeof(bcmcth_trunk_vp_grp_rh_attr_t) * array_size +
                  sizeof(bcmcth_trunk_vp_grp_rh_ha_blk_t);
    ha_alloc_sz = in_req_size;
    hw_array_ptr = shr_ha_mem_alloc(unit,
                                    BCMMGMT_TRUNK_COMP_ID,
                                    submod_id,
    ...
    if ((!warm) ||
        (bcmissu_struct_info_get(
               unit,
               BCMCTH_TRUNK_VP_GRP_RH_HA_BLK_T_ID,
               BCMMGMT_TRUNK_COMP_ID,
               submod_id,
               &offset) == SHR_E_NOT_FOUND)) {
        sal_memset(hw_array_ptr, 0, ha_alloc_sz);
        ...
        hw_array_ptr = (bcmcth_trunk_vp_grp_rh_ha_blk_t *)
                        ((uint8_t *)hw_array_ptr + offset);
        SHR_IF_ERR_MSG_EXIT
            ((hw_array_ptr->signature != ref_sign
              ? SHR_E_FAIL : SHR_E_NONE),
              (BSL_META_U(unit,
                          "\n\tWB Resilient grp attr: Signature mismatch "
                          "exp=0x%-8x act=0x%-8x\n"),
               ref_sign,
               hw_array_ptr->signature));
    }
