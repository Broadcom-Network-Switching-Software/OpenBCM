...
static const pc_phy_eyescan_t pc_phy_eyescan_map[] = {
    ...
    { BCMPC_PHY_EYESCAN_MODE_INTRUSIVE, phymodEyescanModeIntrusive }
};
                          ...
                          uint32_t flags, bcmpc_phy_prbs_cfg_t *prbs)
{
    ...
    if ((lane_idx != -1) && (lane_idx < shr_util_popcount(lport_info.lbmp))) {
        SHR_IF_ERR_VERBOSE_EXIT
            (port_lane_info_get(lport_info.lbmp,
                                &start_lane, &num_lanes));
        SHR_IF_ERR_EXIT
            (bcmpc_phymod_phy_access_t_init(unit, pport, 1 << (lane_idx + start_lane),
                                            &acc_data, pa));
    } else {
        SHR_IF_ERR_EXIT
                               ...
                               uint32_t flags, bcmpc_phy_prbs_status_t *status)
{
    ...
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpc_phy_prbs_status_get(unit, pport, lane_idx,
                                   flags, status));
    ...
    rv = phymod_phy_fast_ber_proj_get(pa, &fast_ber);

    if (rv != SHR_E_NONE) {
        /*
         * Not returning error for fast_ber_proj API failure so that
         * the other PRBS status params are returned to user.
         * Note: Mgmt PMs do not have fast ber proj phymod API implemented.
         */
        fast_ber = 0xFFFFFFFF;
    }

    status->fast_ber = fast_ber;
