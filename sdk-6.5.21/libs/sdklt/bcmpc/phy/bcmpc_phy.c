...
/* one-copy register check */
#define BCMPC_ONE_COPY_REG(reg_addr) \
        (reg_addr < 0xc000)

/* four-copy register check */
#define BCMPC_FOUR_COPY_REG(reg_addr) \
...
#define BCMPC_8_LANE_PORT_SHIFT 8
...
phy_acc_ovrr_update(phymod_access_t *acc, bcmpc_phy_acc_ovrr_t *ovrr)
{
    ...
    /* PM8x100: need to set 8-lane port flag for TSC access. */
    if (acc->lane_mask == 0xff) {
        lane_mask = 0x1 << BCMPC_8_LANE_PORT_SHIFT;
    } else {
        lane_mask = 0;
    }
                               ...
                               bcmpc_phy_acc_ovrr_t *ovrr, uint8_t *pml_sel)
{
    ...
    *reg_addr &= 0xffff;
    if (pm_info->pm_info->type == PM_TYPE_PM4X10) {
        ...
        if (BCMPC_ONE_COPY_REG(*reg_addr)) {
            /* One-copy register */
            *pml_sel = (lane >> 2) & 0x1;
            mpp_sel = (lane >> 1) & 0x1 ? BCMPC_TSC_REG_ACC_MPP1 :
                                          BCMPC_TSC_REG_ACC_MPP0;
            lane = 0;
        } else if (BCMPC_FOUR_COPY_REG(*reg_addr)) {
            /* Four-copy register */
            *pml_sel = (lane >> 3) & 0x1;
                         ...
                         bcmpc_phy_acc_ovrr_t *ovrr)
{
    ...
    if ( ovrr->lane_index != NO_LN_IDX_SPECIFIED ) {
        ...
        if ((pm_info.pm_info->type == PM_TYPE_PM8X100) &&
            BCMPC_FOUR_COPY_REG(reg_addr_tmp)) {
            num_copy = 16;
        } else {
