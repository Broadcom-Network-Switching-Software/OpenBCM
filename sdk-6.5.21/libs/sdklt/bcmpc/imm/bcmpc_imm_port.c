...
{
    pc_port_entry_t entry;
    bcmltd_fields_t *flds = NULL;
    uint8_t idx = 0, num_flds = 2;
    SHR_FUNC_ENTER(unit);
    ...
    SHR_ALLOC(flds, sizeof(bcmltd_fields_t), "bcmpcTmpTblUpdate");
    SHR_NULL_CHECK(flds, SHR_E_MEMORY);
    sal_memset(flds, 0, sizeof(bcmltd_fields_t));

    SHR_ALLOC(flds->field, sizeof(bcmltd_field_t *) * num_flds,
              "bcmpcTmpFldUpdate");

    flds->field[0] = shr_fmm_alloc();
    SHR_NULL_CHECK(flds->field[0], SHR_E_MEMORY);
    sal_memset(flds->field[0], 0, sizeof(bcmltd_field_t));
    flds->field[0]->id = PC_PORTt_PORT_IDf;
    flds->field[0]->data = lport;

    flds->field[1] = shr_fmm_alloc();
    SHR_NULL_CHECK(flds->field[1], SHR_E_MEMORY);
    sal_memset(flds->field[1], 0, sizeof(bcmltd_field_t));
    flds->field[1]->id = PC_PORTt_OPERATIONAL_STATEf;
    flds->field[1]->data = BCMPC_VALID;
    flds->count = num_flds;

    SHR_IF_ERR_EXIT
        (bcmimm_entry_update(unit, TRUE, IMM_SID, flds));
