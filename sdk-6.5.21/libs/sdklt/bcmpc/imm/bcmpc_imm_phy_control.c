                         ...
                         pc_phy_control_entry_t *entry)
{
    ...
    switch (fid) {
        ...
        case PC_PHY_CONTROLt_AUTONEG_FEC_OVERRIDE_AUTOf:
            config->phy_autoneg_fec_override_auto = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_FLD_AUTONEG_FEC_OVERRIDE_AUTO);
            break;
        case PC_PHY_CONTROLt_AUTONEG_FEC_OVERRIDEf:
            config->phy_autoneg_fec_override = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_FLD_AUTONEG_FEC_OVERRIDE);
            break;
        default:
            SHR_ERR_EXIT(SHR_E_FAIL);
    }
 ...
 * \param [in] key Key value for PC_PHY_CONTROL LT.
                                 ...
                                 bcmltd_fields_t *flds)
{
    ...
    if (data) {
        ...
        /*
         * Number of RX DFE Taps are device specific.
         * Get the number of DFE Taps supported on this device.
         */
...
/*!
 * \brief Callback function for PC_PHY_CONTROL LT lookup.
 *
 ...
 */
static int
pc_phy_ctrl_lookup_handler(int unit, bcmltd_sid_t sid,
                           ...
                           bcmltd_fields_t *out)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pc_lt_phy_control_flds_parse(unit, NULL, key_flds, &entry));
    ...
    /* Update the read-only operational values in the PC_PHY_CONTROL LT. */
...
bcmpc_imm_phy_control_register(int unit)
{
    ...
    event_hdl.lookup = pc_phy_ctrl_lookup_handler;
...
bcmpc_phy_control_prepopulate_all(int unit)
{
    ...
    /* Insert all the PC_PHY_CONTROL LTs with keys and data data. */
