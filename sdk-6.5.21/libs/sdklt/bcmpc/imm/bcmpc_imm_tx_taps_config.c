                             ...
                             pc_tx_taps_config_entry_t *entry)
{
    ...
    switch (fid) {
        ...
        case PC_TX_TAPSt_TX_PRE2_SIGNf:
            ...
            break;
        case PC_TX_TAPSt_TX_PRE3_AUTOf:
            config->pre3_auto = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_TX_TAPS_LT_FLD_TX_PRE3_AUTO);
            break;
        case PC_TX_TAPSt_TX_PRE3f:
            config->pre3 = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_TX_TAPS_LT_FLD_TX_PRE3);
            break;
        case PC_TX_TAPSt_TX_PRE3_SIGNf:
            config->pre3_sign = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_TX_TAPS_LT_FLD_TX_PRE3_SIGN);
            break;
                                 ...
                                 bcmltd_fields_t *flds)
{
    ...
    if (data) {
        ...
        if (!is_unmapped) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_TX_TAPSt_TX_PRE3_SIGNf, 0,
                 tx_taps_cfg->pre3_sign, fcnt);
        }
        SHR_IF_ERR_EXIT
            (bcmlrd_field_is_unmapped(unit, IMM_SID,
        ...
        if (!is_unmapped) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_TX_TAPSt_TX_PRE3_AUTOf, 0,
                 tx_taps_cfg->pre3_auto, fcnt);
        }
        SHR_IF_ERR_EXIT
            (bcmlrd_field_is_unmapped(unit, IMM_SID,
        ...
        if (!is_unmapped) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_TX_TAPSt_TX_PRE3f, 0,
                 tx_taps_cfg->pre3, fcnt);
        }
    }
                          ...
                          bcmltd_fields_t *out)
{
    ...
    if (pport == BCMPC_INVALID_PPORT) {
        ...
        if (SHR_FAILURE(rv) || (pcfg.num_lanes == 0) ||
            ...
            entry.data.oper_state = pcfg.oper_state;
        }
    }

    /* Update the read-only operational values in the TX_TAPS LT. */
    for (i = 0; i < out->count; i++) {
        if (out->field[i]->id != PC_PHY_CONTROLt_OPERATIONAL_STATEf) {
            continue;
        }
