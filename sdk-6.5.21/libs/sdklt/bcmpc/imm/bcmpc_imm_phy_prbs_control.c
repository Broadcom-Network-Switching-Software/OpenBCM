/*! \file bcmpc_imm_phy_prbs_control.c
 *
 * PC_PHY_PRBS_CONTROLt IMM handler.
 */
/*
...
#define IMM_SID PC_PHY_PRBS_CONTROLt

/* Data Structure for PC_PHY_PRBS_CONTROLt entry */
typedef struct pc_phy_prbs_control_entry_s {
    bcmpc_port_lane_t key;
    bcmpc_phy_prbs_control_t data;
} pc_phy_prbs_control_entry_t;
 ...
 */
 /*!
  * \brief Initialize pc_phy_prbs_control_entry_t data buffer.
  *
  ...
  */
static void
pc_phy_prbs_control_entry_t_init (pc_phy_prbs_control_entry_t *entry)
{
 ...
 */
static int
pc_phy_prbs_control_entry_set (int unit, uint32_t fid, uint32_t fidx,
                               uint64_t fval,
                               pc_phy_prbs_control_entry_t *entry)
{
    bcmpc_port_lane_t  *port_lane = &entry->key;
    bcmpc_phy_prbs_control_t *config = &entry->data;

    SHR_FUNC_ENTER(unit);

    switch (fid) {
        case PC_PHY_PRBS_CONTROLt_PORT_IDf:
            port_lane->lport = fval;
            break;
        case PC_PHY_PRBS_CONTROLt_LANE_INDEXf:
            port_lane->lane_index = fval;
            break;
        case PC_PHY_PRBS_CONTROLt_PRBS_GENERATOR_ENABLEf:
            config->generator_enable = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_FLD_GENERATOR_ENABLE);
            break;
        case PC_PHY_PRBS_CONTROLt_PRBS_CHECKER_ENABLEf:
            config->checker_enable = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_FLD_CHECKER_ENABLE);
            break;
        case PC_PHY_PRBS_CONTROLt_POLY_MODEf:
            config->poly_mode = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_FLD_POLY_MODE);
            break;
        case PC_PHY_PRBS_CONTROLt_INVERT_DATA_TXf:
            config->invert_data_tx = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_FLD_INVERT_DATA_TX);
            break;
        case PC_PHY_PRBS_CONTROLt_INVERT_DATA_RXf:
            config->invert_data_rx = fval;
            BCMPC_LT_FIELD_SET(config->fbmp, BCMPC_FLD_INVERT_DATA_RX);
            break;
        default:
 ...
 *
 * Parse the input LT fields and save the data to pc_phy_prbs_control_entry_t.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
pc_lt_phy_prbs_control_flds_parse (int unit,
                                   ...
                                   pc_phy_prbs_control_entry_t *entry)
{
    ...
    while (fld) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pc_phy_prbs_control_entry_set(unit, fid, 0, fval, entry));
    ...
    while (fld) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pc_phy_prbs_control_entry_set(unit, fid, fidx, fval, entry));
 ...
 * bcmpc_phy_prbs_control_prof_t \c data.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
pc_phy_prbs_control_entry_parse (int unit, const bcmltd_fields_t *flds,
                                 ...
                                 size_t data_size, void *data)
{
    ...
    pc_phy_prbs_control_entry_t_init(&entry);
 ...
 * bcmpc_db_imm_schema_handler_t.
 *
 * Convert bcmpc_port_lane_t \c key and a bcmpc_phy_prbs_control_t \c data to
 ...
 */
static int
pc_phy_prbs_control_entry_fill (int unit,
                                size_t key_size, void *key,
                                size_t data_size, void *data,
                                bcmltd_fields_t *flds)
{
    ...
    bcmpc_phy_prbs_control_t *config = data;
    ...
    if (key) {
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PHY_PRBS_CONTROLt_PORT_IDf, 0, port_lane->lport, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PHY_PRBS_CONTROLt_LANE_INDEXf, 0, port_lane->lane_index, fcnt);
    }

    if (data) {
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PHY_PRBS_CONTROLt_PRBS_GENERATOR_ENABLEf, 0,
             config->generator_enable, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PHY_PRBS_CONTROLt_PRBS_CHECKER_ENABLEf, 0,
             config->checker_enable, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PHY_PRBS_CONTROLt_POLY_MODEf, 0,
             config->poly_mode, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PHY_PRBS_CONTROLt_INVERT_DATA_TXf, 0,
             config->invert_data_tx, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PHY_PRBS_CONTROLt_INVERT_DATA_RXf, 0,
             config->invert_data_rx, fcnt);
    }
...
/*!
 * \brief PHY PRBS control configuration validation function.
 *
 * \param [in] unit This is device unit number.
 * \param [in] key Key value for PC_PHY_PRBS_CONTROL LT.
 ...
 */
static int
pc_phy_prbs_control_config_validate (int unit, bcmpc_port_lane_t key,
 ...
 */
static int
pc_phy_prbs_control_stage (int unit,
                           bcmltd_sid_t sid,
                           uint32_t trans_id,
                           ...
                           bcmltd_fields_t *output_fields)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pc_lt_phy_prbs_control_flds_parse(unit, key_flds, NULL, &entry));

    SHR_IF_ERR_VERBOSE_EXIT
        (pc_phy_prbs_control_config_validate(unit, entry.key, &config_valid,
                                             &config_hw_apply));
    ...
    switch (event) {
        case BCMIMM_ENTRY_INSERT:
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_phy_prbs_control_flds_parse(unit, NULL, data_flds,
                                                   &entry));

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpc_internal_phy_prbs_control_set(unit, &entry.key,
                                                     &entry.data));
            break;
        ...
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpc_internal_phy_prbs_control_delete(unit, &entry.key));
            break;
        default:
 ...
 */
int
bcmpc_imm_phy_prbs_control_register (int unit)
{
    ...
    db_hdl.entry_parse = pc_phy_prbs_control_entry_parse;
    db_hdl.entry_fill = pc_phy_prbs_control_entry_fill;

    sal_memset(&event_hdl, 0, sizeof(event_hdl));
    event_hdl.stage = pc_phy_prbs_control_stage;
