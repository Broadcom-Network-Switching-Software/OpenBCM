...
bcmpc_l2p_pport_get(int unit, bcmpc_lport_t lport)
{
    ...
    if (lport > max_lports) {
        /* Return INVALID_PPORT. */
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    SHR_IF_ERR_EXIT
        (sal_rwlock_rlock(l2p_lock[unit], SAL_RWLOCK_FOREVER));
...
exit:
    if (SHR_FUNC_ERR()) {
        LOG_INFO(BSL_LOG_MODULE,
                (BSL_META_U(unit, "Failed to get lport(%d) -> pport(%d)\n"),
                 lport, pport));
    }
    return pport;
}
                      ...
                      bool do_core_init)
{
    ...
    /* Only none PM8x50GEN2 (rev_B0) invokes this function. */
    if (!((pm_drv_info->type == PM_TYPE_PM8X50_GEN2) &&
        (dev_id.revision == BCM56880_REV_B0))) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpc_pmgr_pm_tsc_power_on_set(unit, pcfg->pport, 1));
    }
                              ...
                              bcmpc_port_cfg_t *pcfg)
{
    ...
    if (pcfg->pport != BCMPC_INVALID_PPORT && pcfg->speed != 0) {
        ...
        if ((pst.speed == 150000) ||
            (pst.speed == 300000) ||
            (pst.speed == 350000) ||
            ((pst.speed == 400000) && (pst.num_lanes == 8)) ||
            ((pst.speed == 200000) && (pst.num_lanes == 4) && (pst.fec_mode != PM_PORT_FEC_NONE))) {
                           ...
                           bcmpc_port_cfg_t *pcfg, bool do_insert)
{
    ...
    if ((sal_strcmp(pm_info.prop.name, "PC_PM_TYPE_PM8X50") == 0) ||
         ...
         (sal_strcmp(pm_info.prop.name, "PC_PM_TYPE_PM8X50_GEN3") == 0) ||
          (sal_strcmp(pm_info.prop.name, "PC_PM_TYPE_PM8X100") == 0)) {
    ...
    if ((!pcfg->autoneg) && ((cfg_old.speed != pcfg->speed) ||
        (cfg_old.fec_mode != pcfg->fec_mode) || do_insert)) {
...
{
    int pm_id, num_phys_ports = 0, pcnt;
    bool do_core_init, first_port;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpc_topo_inst_info_update(unit, pcfg));
    /*
     * Check if the logical port is the first configured port within a PM core.
     */
    ...
    /*
     * Aging support only.
     * Before the first port is configured on the PM, power cycle the PM
     * to clean up.
     */
    if (first_port &&
        pm_drv_info->pm_data.pm_ha_info->firmware_downloaded &&
        pm_drv_info->pm_data.pm_ha_info->is_pm_initialized &&
        (pm_drv_info->pm_data.pm_ha_info->serdes_core_tvco_power_state ==
                                                PM_SERDES_CORE_PLL_PWR_DOWN)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpc_pmgr_pm_tsc_power_cycle(unit, pport));

        pm_drv_info->pm_data.pm_ha_info->is_pm_initialized = 0;
        pm_drv_info->pm_data.pm_ha_info->firmware_downloaded = 0;
    }
...
pc_internal_port_update(int unit, bcmpc_lport_t lport, bcmpc_port_cfg_t *pcfg)
{
    ...
    /* Check whether RLM is supported */
    if (pcfg->rlm_mode) {
        if (!SHR_BITGET(pm_info.prop.pm_feature, BCMPC_FT_RLM)) {
            LOG_INFO(BSL_LOG_MODULE,
                     (BSL_META_U(unit,
