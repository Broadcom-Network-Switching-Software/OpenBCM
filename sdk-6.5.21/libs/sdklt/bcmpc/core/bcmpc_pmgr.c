...
typedef struct pmgr_link_state_s {
    ...
    /*! Port link up operation in progress. */
    bool link_up_in_progress;

    /*! Port link down operation in progress. */
    bool link_down_in_progress;
...
#define PMGR_PORT_UP_IN_PROGRESS(_u, _p) \
                           pmgr_link_state[_u][_p].link_up_in_progress
#define PMGR_PORT_DOWN_IN_PROGRESS(_u, _p) \
                           pmgr_link_state[_u][_p].link_down_in_progress

/*! HA link state. */
static bcmpc_ha_link_state_info_t *pmgr_ha_link_state[BCMPC_NUM_UNITS_MAX];

#define PMGR_HA_LINK_STATE_SET(_u, _p, _l)               \
    do {                                                 \
        if (pmgr_ha_link_state[_u] != NULL) {            \
            pmgr_ha_link_state[_u]->link_state[_p] = _l; \
        }                                                \
    } while (0)

/*! Structure for storing device specific link sequence execution timeouts. */
typedef struct pmgr_link_seq_exec_timeout_s {
    /*! Link Up sequence execution maximum timeout. */
    uint32_t timeout_up;

    /*! Link Down sequence execution maximum timeout. */
    uint32_t timeout_down;

} pmgr_link_seq_exec_timeout_t;

/*! device specific link sequence execution timeout. */
static pmgr_link_seq_exec_timeout_t *link_exec_timeout[BCMPC_NUM_UNITS_MAX];
...
pmgr_port_oper_done_wait(int unit, bcmpc_pport_t pport, int link, int *status)
{
    ...
    if (PMGR_PORT_UP_IN_PROGRESS(unit, pport) ||
        PMGR_PORT_DOWN_IN_PROGRESS(unit, pport)) {
        LOG_INFO(BSL_LOG_MODULE,
                 (BSL_META_U(unit, "link change in progress (%d) UP "
                                   "- %d DOWN - %d (%s) - Wait.!!!\n"),
                                     pport,
                                     PMGR_PORT_UP_IN_PROGRESS(unit, pport),
                                     PMGR_PORT_DOWN_IN_PROGRESS(unit, pport),
                                     !link? "UP":"DOWN"));
        do {
            sal_usleep(1000);
        ...
        LOG_INFO(BSL_LOG_MODULE,
                (BSL_META_U(unit, "link change done (%d) UP - %d DOWN "
                                  "- %d - link (%d).!!!\n"),
                                    pport,
                                    PMGR_PORT_UP_IN_PROGRESS(unit, pport),
                                    PMGR_PORT_DOWN_IN_PROGRESS(unit, pport),
                                    PMGR_LINK_STATE(unit, pport)));
    }
     ...
     */
    if (link) {
        timeout_in_usec = link_exec_timeout[unit]->timeout_up;
    } else {
        timeout_in_usec = link_exec_timeout[unit]->timeout_down;
    }
        ...
        if (PMGR_PORT_OPER_DONE(unit, pport)) {
            LOG_INFO(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Operation Done(%d) (%d)!!!\n"),
                                pport, timeout_count));
            *status = SHR_E_NONE;
            if ((int) PMGR_LINK_STATE(unit, pport) != link) {
                LOG_ERROR(BSL_LOG_MODULE,
                         (BSL_META_U(unit, "link state not same as expected "
                                           "(%d:%d) (%d)!!!\n"),
                                           lport, pport, link));
            }
            break;
...
bcmpc_pmgr_port_link_change(int unit, bcmpc_pport_t pport, int link)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
        (BSL_META_U(unit, "Link(%d) is changed to %s\n"), lport,
         link ? "UP" : "DOWN"));
    if (link) {
        PMGR_PORT_UP_IN_PROGRESS(unit, pport) = 1;
    } else {
        PMGR_PORT_DOWN_IN_PROGRESS(unit, pport) = 1;
    }
...
exit:
    if (!SHR_FUNC_ERR()) {
        ...
        if (PMGR_LINK_FORCED(unit, pport)) {
            LOG_INFO(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Force Link(%d) is changed to %s\n"), lport,
                                link ? "UP" : "DOWN"));
        ...
        PMGR_HA_LINK_STATE_SET(unit, pport, link);
    }

    if (link) {
        PMGR_PORT_UP_IN_PROGRESS(unit, pport) = 0;
    } else {
        PMGR_PORT_DOWN_IN_PROGRESS(unit, pport) = 0;
    }
                                   ...
                                   int is_first)
{
    ...
    bcmpc_pmgr_drv_t *pmgr_drv;
    bcmpc_drv_t *pc_drv = NULL;
    const bcmpc_dev_ability_entry_t *unabilities = NULL;
    int num_nonabil = 0;
    int idx;
    ...
    /* Don't check unsupported abilities on management port */
    if (pc_drv->dev_unsupported_abilities_get) {
        unabilities = pc_drv->dev_unsupported_abilities_get(unit, pcfg->pport, &num_nonabil);

        if (unabilities != NULL) {
            for (idx = 0; idx < num_nonabil; idx++) {
                if (pcfg->speed == unabilities[idx].speed &&
                    pcfg->num_lanes == unabilities[idx].num_lanes &&
                    pcfg->fec_mode == unabilities[idx].fec_type) {
...
int
bcmpc_pmgr_pm_tsc_power_on_set(int unit, bcmpc_pport_t pport, int enable)
{
    bcmpc_pmgr_drv_t *pmgr_drv;
    ...
    SHR_NULL_CHECK
        (pmgr_drv->pm_tsc_power_on_set, SHR_E_NO_HANDLER);

    SHR_IF_ERR_VERBOSE_EXIT
        (pmgr_drv->pm_tsc_power_on_set(unit, pport, enable));
...
int
bcmpc_pmgr_interrupt_process(int unit, bcmpc_pport_t pport,
                             ...
                             uint32_t *is_handled)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pmgr_drv->pm_port_interrupt_process(unit, pport,
...
int
bcmpc_pmgr_phy_prbs_control_set (int unit, bcmpc_port_lane_t *port_lane,
                                 bcmpc_pport_t pport, bcmpc_phy_prbs_control_t *prbs_config)
{
    bcmpc_pmgr_drv_t *pmgr_drv;
    ...
    SHR_NULL_CHECK
        (pmgr_drv->pm_phy_prbs_control_set, SHR_E_NO_HANDLER);

    SHR_IF_ERR_VERBOSE_EXIT
        (pmgr_drv->pm_phy_prbs_control_set(unit, port_lane, pport, prbs_config));
...
int
bcmpc_pmgr_phy_prbs_control_delete (int unit, bcmpc_port_lane_t *port_lane,
                                    bcmpc_pport_t pport)
{
    bcmpc_pmgr_drv_t *pmgr_drv;
    ...
    SHR_NULL_CHECK
        (pmgr_drv->pm_phy_prbs_control_delete, SHR_E_NO_HANDLER);

    SHR_IF_ERR_VERBOSE_EXIT
        (pmgr_drv->pm_phy_prbs_control_delete(unit, port_lane, pport));
...
int
bcmpc_pmgr_phy_prbs_status_get (int unit, bcmpc_port_lane_t *port_lane,
                                bcmpc_phy_prbs_status_t *pst)
{
    ...
    SHR_NULL_CHECK
        (pmgr_drv->pm_phy_prbs_status_get, SHR_E_NO_HANDLER);

    sal_memset(pst, 0, sizeof(*pst));

    /* Get the status from hardware. */
    SHR_IF_ERR_VERBOSE_EXIT
        (pmgr_drv->pm_phy_prbs_status_get(unit, port_lane, pport, pst));
...
int
bcmpc_pmgr_pm_tsc_power_cycle(int unit, bcmpc_pport_t pport)
{
    bcmpc_pmgr_drv_t *pmgr_drv;
    ...
    SHR_NULL_CHECK
        (pmgr_drv->pm_tsc_power_cycle, SHR_E_NO_HANDLER);

    SHR_IF_ERR_VERBOSE_EXIT
        (pmgr_drv->pm_tsc_power_cycle(unit, pport));
...
/*!
 * \brief Allocate memory for storing maximum timeout
 * for executing the link sequence.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
 */
int
bcmpc_pmgr_link_exec_timeout_create(int unit)
{
    bcmpc_drv_t *pc_drv = NULL;

    SHR_FUNC_ENTER(unit);

    SHR_ALLOC(link_exec_timeout[unit], sizeof(pmgr_link_seq_exec_timeout_t),
              "bcmpcLinkExecTime");
    SHR_NULL_CHECK(link_exec_timeout[unit], SHR_E_MEMORY);
    sal_memset(&link_exec_timeout[unit][0], 0,
                sizeof(pmgr_link_seq_exec_timeout_t));
    ...
    if (pc_drv && pc_drv->link_sequence_exec_timeout_get) {
        SHR_IF_ERR_EXIT
            (pc_drv->link_sequence_exec_timeout_get(unit,
                                      &link_exec_timeout[unit]->timeout_up,
                                      &link_exec_timeout[unit]->timeout_down));
    } else {
        link_exec_timeout[unit]->timeout_up = 300000;
        link_exec_timeout[unit]->timeout_down = 300000;
    }
...
/*!
 * \brief Destroy link_exec_timeout buffer.
 *
 * \param [in] unit Unit number.
 *
 * \retval Nothing.
 */
void
bcmpc_pmgr_link_exec_timeout_destroy(int unit)
{

    if (link_exec_timeout[unit]) {
        SHR_FREE(link_exec_timeout[unit]);
        link_exec_timeout[unit] = NULL;
    }
    return;
}

int
bcmpc_pmgr_port_diag_control_set (int unit, bcmpc_lport_t *lport,
                                 bcmpc_pport_t pport, bcmpc_port_diag_control_t *diag_config)
{
    bcmpc_pmgr_drv_t *pmgr_drv;
    ...
    SHR_NULL_CHECK
        (pmgr_drv->pm_port_diag_control_set, SHR_E_NO_HANDLER);

    SHR_IF_ERR_VERBOSE_EXIT
        (pmgr_drv->pm_port_diag_control_set(unit, lport, pport, diag_config));
...
int
bcmpc_pmgr_port_diag_control_delete (int unit,
                                     bcmpc_pport_t pport)
{
    bcmpc_pmgr_drv_t *pmgr_drv;
    ...
    SHR_NULL_CHECK
        (pmgr_drv->pm_port_diag_control_delete, SHR_E_NO_HANDLER);

    SHR_IF_ERR_VERBOSE_EXIT
        (pmgr_drv->pm_port_diag_control_delete(unit, pport));
