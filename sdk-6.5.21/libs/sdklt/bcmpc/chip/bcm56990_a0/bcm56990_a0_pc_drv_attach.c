    ...
    switch (intr_num) {
        case CDPORT_INTR_FDR_INTERRUPT:
            ...
            if (lport == BCMPC_INVALID_LPORT) {
                 ...
                 * interrupt hierarchy.
                 */
                rv = bcmpc_interrupt_process(unit, pport, BCMPC_INTR_FDR,
                                             &is_handled);
                if (SHR_FAILURE(rv)) {
                    LOG_WARN(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                             "Failed to clear PHYMOD FDR interrupt on pport(%d)\n"),
                              pport));
                }
                ...
                if (SHR_FAILURE(rv)) {
                    LOG_WARN(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                             "Failed to clear FDR interrupt on pport(%d)\n"), pport));
                }
            }
            break;
        default:
                               ...
                               bcmpc_fdr_port_stats_t *fdr_stats)
{
    ...
    /* The start_time timestamp is the units of 256 nano-secs. */
    fdr_stats->start_time *= 256;
    ...
    /* The end_time timestamp is the units of 256 nano-secs. */
    fdr_stats->end_time *= 256;
...
static int
bcm56990_a0_num_of_rx_dfe_tap_get(int unit, int pm_id, uint32_t *num_of_rx_taps)
{
    int rv;
    bcmpc_topo_t pm_info;
...
static bcmpc_drv_t bcm56990_a0_pc_drv = {
    ...
    .num_of_rx_dfe_tap_get = bcm56990_a0_num_of_rx_dfe_tap_get,
};
