/*! \file bcm56990_b0_phy_fw_load.c
 *
 * BCM56990_B0 PHY firmware loader.
 */
/*
...
#include <bcmbd/chip/bcm56990_b0_cmic_bcast.h>
...
#define BCM56990_CDPORT_TSC_UCMEM_DATA_SIZE 122880
                      ...
                      uint32_t fw_size, const uint8_t *fw_data)
{
    ...
    ucmem_idx_max = BCM56990_CDPORT_TSC_UCMEM_DATA_SIZE / FW_ALIGN_BYTES - 1;
...
static int
tscbh_firmware_helper_dma(const phymod_core_access_t *ca,
                          uint32_t fw_size, const uint8_t *fw_data)
{
    bcmpc_phy_access_data_t *phy_data = PHYMOD_ACC_USER_ACC(&ca->access);
    int unit = phy_data->unit;
    ...
    uint32_t ucmem_max_entries;
    uint32_t current_entry_pos = 0;
    uint32_t blkacc = BLKTYPE_CDPORT;
    uint32_t offset_tsc_mem = CDPORT_TSC_MEM_CTRLr_OFFSET;
    uint32_t access_mode;
    uint32_t dma_addr;
    uint32_t dma_adext = 0;
    size_t dma_wsize;
    size_t dma_entries;
    char   *ucode_dma_copy;
    uint64_t buf_addr;
    uint32_t blknum = 0;
    uint32_t *swapped_buffer;
    ...
    ucmem_max_entries = BCM56990_CDPORT_TSC_UCMEM_DATA_SIZE / FW_ALIGN_BYTES;

    /* Aligned firmware size */
    size = (fw_size + FW_ALIGN_MASK) & ~FW_ALIGN_MASK;
    num_entries = size/FW_ALIGN_BYTES;
    ...
    /* Getting block number from port parameter. */
    if (BCMBD_CMICX_BCAST_PORT(port)) {
        blknum = BCMBD_CMICX_BCAST_BLK(port);
    } else {
    ...
    ucode_dma_copy = bcmdrd_hal_dma_alloc(unit, size, "sbusdma data buffer0", &buf_addr);
    if (!ucode_dma_copy) {
        return SHR_E_MEMORY;
    }
    sal_memset(ucode_dma_copy, 0, size);

    /*
     * Check for endianess, if Little Endian, just copy.
     * Big Endian, swap the byte order in words.
     */

    if (bcmpc_is_little_endian()) {
        sal_memcpy(ucode_dma_copy, fw_data, fw_size);
    } else {
        /* BIG Endian. */
        swapped_buffer = (uint32_t *)ucode_dma_copy;
        for ( word_index = 0; word_index < (size/sizeof(uint32_t)); word_index++) {
            byte_index = word_index * sizeof(uint32_t);
            swapped_buffer[word_index] = fw_data[byte_index] |
                                         fw_data[byte_index+1] << 8 |
                                         fw_data[byte_index+2] << 16|
                                         fw_data[byte_index+3] << 24;
        }
    }

    /* Constructing the address and address extension to access DMA interface.
       access type is not used in PortMacro, just leave the logic
       to set ADEXT_ACCTYPE for consistency. */
    dma_addr = bcmbd_block_port_addr(unit, blknum, -1,
                                     CDPORT_TSC_UCMEM_DATAm_OFFSET, 0);
    dma_wsize =  CDPORT_TSC_UCMEM_DATAm_SIZE/sizeof(uint32_t);
    BCMBD_CMICX_ADEXT_ACCTYPE_SET(dma_adext,CDPORT_TSC_UCMEM_DATAm_BLKACC);
    BCMBD_CMICX_ADEXT_BLOCK_SET(dma_adext, blknum );

    current_entry_pos = 0;
    while (num_entries > 0) {
        dma_entries = (num_entries > ucmem_max_entries)? ucmem_max_entries:num_entries;

        bcmbd_cmicx_mem_range_write(unit, dma_adext, dma_addr,
                                    dma_wsize, 0, dma_entries,
                                    buf_addr+(current_entry_pos*CDPORT_TSC_UCMEM_DATAm_SIZE), 0);
        current_entry_pos += dma_entries;
        num_entries       -= dma_entries;
    }
    bcmdrd_hal_dma_free(unit, size, ucode_dma_copy, buf_addr);
...
pm_info_update(int unit, int pm_id, bcmpc_topo_t *pm_info, void *cookie)
{
    ...
    if ((phy_type == phymodDispatchTypeTscbh_gen3) ||
...
pm_tscbh_firmware_load(int unit, int *last_bcast_port, bcmdrd_pbmp_t *pbmp)
{
    ...
    if (fw_cfg.fw_load == PM_SERDES_FW_LOAD_SLOW) {
        /*! Slow download through MDIO. */
        BCMDRD_PBMP_ITER(pbmp[0], port) {
            BCMPC_PHY_ACCESS_DATA_SET(phy_data, unit, port);
            SHR_IF_ERR_EXIT
                (bcmpc_phymod_core_access_t_init(unit, port, phy_data, ca));
            sal_memset(cic, 0, sizeof(*cic));
            cic->firmware_loader = NULL;
            SHR_IF_ERR_EXIT
                (bcmpc_phymod_fw_load_config_set(unit, &fw_cfg, cic));
            PHYMOD_CORE_INIT_F_EXECUTE_PASS1_SET(cic);
    ...
    } else if (fw_cfg.fw_load == PM_SERDES_FW_LOAD_PRAM_NON_BCAST) {
        /*! PM by PM download by PRAM via UCMEM Non Broadcast. */
        BCMDRD_PBMP_ITER(pbmp[0], port) {
            BCMPC_PHY_ACCESS_DATA_SET(phy_data, unit, port);
            SHR_IF_ERR_EXIT
                (bcmpc_phymod_core_access_t_init(unit, port, phy_data, ca));
    ...
    } else if (fw_cfg.fw_load == PM_SERDES_FW_LOAD_PRAM_BCAST) {
        /*! Download by PRAM via UCMEM Broadcast rings.
            Transfer FW to UCMEM using DMA access. */
    ...
    } else if ((fw_cfg.fw_load == PM_SERDES_FW_LOAD_BCAST_DMA) ||
               (fw_cfg.fw_load == PM_SERDES_FW_LOAD_FAST)) {

        /*! PM_SERDES_FW_LOAD_FAST - default method of download.
            Download by RRAM, using broadcast rings. */
        ...
        for (idx = 0; idx < CDPORT_NUM_BCAST; idx++) {
            if (last_bcast_port[idx] > 0 ) {
                ...
                sal_memset(cic, 0, sizeof(*cic));
                cic->firmware_loader = tscbh_firmware_helper_dma;
                SHR_IF_ERR_EXIT
                    (bcmpc_phymod_fw_load_config_set(unit, &fw_cfg, cic));
        ...
        SHR_ERR_MSG_EXIT(SHR_E_PARAM,
                         (BSL_META_U(unit,
                                     "Non Supported Download Method %d\n"),
                                     fw_cfg.fw_load));
    }
...
cdport_last_bcast_port_get(bcmdrd_pbmp_t *pbmp, int *last_bcast_port)
{
    ...
    BCMDRD_PBMP_ITER(pbmp[0], port) {
        ...
        if (bcast_domain & 0x1) {
            ...
            /* Domain 0 and 2 are reverse sequence in the ring. */
            last_bcast_port[bcast_domain] = port;
        }
    }
}
...
int
bcm56990_b0_phy_firmware_loader(int unit, int port, uint32_t fw_loader_req,
