...
/*!
 * \brief BCMPC package level core config
 */
typedef struct bcmpc_pm_pkg_core_cfg_s {

    /*! Package level RX Lane map. */
    uint64_t rx_lane_map;

    /*! Package level TX Lane map. */
    uint64_t tx_lane_map;

    /*! Package level RX polarity flip. */
    uint16_t rx_polarity_flip;

    /*! Package level TX polarity flip. */
    uint16_t tx_polarity_flip;

} bcmpc_pm_pkg_core_cfg_t;
...
/*!
 * \brief This function returns if the lane-swap and polarity needs to be swapped
 * to accomodate the internal swap inside the chip
 *
 * \param [in] unit Unit number.
 * \param [in] pport of the PM.
 * \param [out] is_true True if internal lane swap is true.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_FAIL Failure.
 */
typedef int
(*bcmpc_does_internal_lane_swap_exist_f) (int unit, bcmpc_pport_t pport, bool *is_true);

/*!
 * \brief This function gets any package level lane-swap
 * and polarity flips needed on top of user provided core_config
 * For example, TD4-x9 has package level lane swap and polarity
 * flip compared to TD4 that is not exposed to customer.
 *
 * \param [in] unit Unit number.
 * \param [in] pm_id Port Macro instance
 *
 * \retval Package level core config structure bcmpc_pm_core_cfg_t
 */
typedef bcmpc_pm_pkg_core_cfg_t
(*bcmpc_pkg_level_core_config_get_f) (int unit, int pm_id);

/*!
 * \brief Get the number rx dfe taps.
 *
 * \param [in] unit Unit number.
 * \param [in] pm_id Port macro(PM) ID.
 * \param [out] num_of_rx_taps number of rx dfe taps.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_FAIL Failure.
 */
typedef int
(*bcmpc_num_of_rx_dfe_tap_get_f)(int unit, int pm_id, uint32_t *num_of_rx_taps);


/*!
 * \brief Get the flag for lane_mask, if applicable.
 *
 * \param [in] unit Unit number.
 * \param [in] lane_mask lane mask.
 * \param [out] flag returned flag for lane_mask.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_FAIL Failure.
 */
typedef int
(*bcmpc_pm_drv_lane_mask_flag_f)(int unit, uint32_t lane_mask, uint32_t *flag);
...
typedef struct bcmpc_drv_s {
    ...
    /*! Function to determine if internal lane swap exist. */
    bcmpc_does_internal_lane_swap_exist_f does_internal_lane_swap_exist;

    /*! Function to apply package level init config if it exists. */
    bcmpc_pkg_level_core_config_get_f pkg_level_core_config_get;

    /*! Function to get the number rx dfe taps. */
    bcmpc_num_of_rx_dfe_tap_get_f num_of_rx_dfe_tap_get;

    /*! Function to get the applicable flag for lane_mask. */
    bcmpc_pm_drv_lane_mask_flag_f pm_drv_lane_mask_flag;
} bcmpc_drv_t;
