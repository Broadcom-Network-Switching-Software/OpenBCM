...
typedef struct pm_info_s {
    ...
    /*!
     * Flag to enable/disable simulation of link UP/DOWN
     * simulation environments.
     */
    bool link_simulation;
...
/*! Phymod serdes core init lane swap and polarity swap. */
#define PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP 0x80
...
/*! Set core init lane swap flip */
#define PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP_SET(info) ((info)->flags |= \
                                            PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP)
...
/*! clear core init lane swap flip */
#define PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP_CLEAR(info) ((info)->flags &= \
                                            ~PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP)
...
/*! Get core init top device revision ID */
#define PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP_GET(info) ((info)->flags & \
                                          PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP? 1: 0)
...
typedef struct pm_port_add_info_s {
    ...
    /*!
     * Flag to enable/disable link UP/DOWN in simulation
     * environements.
     */
    bool link_simulation;
...
typedef struct pm_phy_control_s {
    ...
    /*! FEC override for CL73 100G case */
    int phy_autoneg_fec_override;
...
typedef enum pm_fw_load_method_e {
    ...
    /*! PM firmware download - Fastest Method availabe for a device. */
    PM_SERDES_FW_LOAD_FAST,

    /*! PM firmware slow download. */
    PM_SERDES_FW_LOAD_SLOW,

    /*! PM firmware SBUS download with PRAM interface
        UCMEM SBUS DMA and Broadcast download.*/
    PM_SERDES_FW_LOAD_BCAST_DMA,

    /*! PM firmware with PRAM infterface via UCMEM SBUS download. PM by PM */
    PM_SERDES_FW_LOAD_PRAM_NON_BCAST,

    /*! PM firmware download with PRAM interface and UCMEM SBUS Broadcast. */
    PM_SERDES_FW_LOAD_PRAM_BCAST
...
/*! SerDes core power mode. */
typedef enum pm_serdes_core_pll_power_e {

    /*! SerDes core initial power*/
    PM_SERDES_CORE_PLL_PWR_NONE = 0,

    /*! SerDes core is power on*/
    PM_SERDES_CORE_PLL_PWR_ON,

    /*! SerDes core is power down. */
    PM_SERDES_CORE_PLL_PWR_DOWN,

    /*! power cnt*/
    PM_SERDES_CORE_PLL_PWR_CNT

} pm_serdes_core_pll_power_t;
...
/*! Set AutoNeg FEC Override to allow CL73 100G No FEC case */
#define PM_PHY_CTRL_F_AUTONEG_FEC_OVERRIDE   (1 << 15)
...
/*! IEEE Reduced IPG mode. */
#define PM_ENCAP_IEEE_REDUCED_IPG  BCMPMAC_ENCAP_IEEE_REDUCED_IPG
