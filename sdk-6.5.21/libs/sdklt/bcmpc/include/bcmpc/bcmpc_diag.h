...
typedef enum bcmpc_phy_eyescan_mode_e {
    ...
    /*! Intrusive eyescan. */
    BCMPC_PHY_EYESCAN_MODE_INTRUSIVE,
...
typedef struct bcmpc_phy_prbs_status_s {
    ...
    /*! Fast BER calculate over 100ms duration */
    uint32_t fast_ber;
...
/*!
 * \brief PC_PHY_PRBS_CONTROL LT fields information.
 */
typedef enum bcmpc_phy_prbs_control_lt_fields_e {
    BCMPC_FLD_GENERATOR_ENABLE = 0,
    BCMPC_FLD_CHECKER_ENABLE,
    BCMPC_FLD_POLY_MODE,
    BCMPC_FLD_INVERT_DATA_TX,
    BCMPC_FLD_INVERT_DATA_RX,
} bcmpc_phy_prbs_control_lt_fields_t;

/*!
 * Number of fields in PC_PHY_PRBS_CONTROL LT.
 * Refer to \ref bcmpc_phy_prbs_control_lt_fields_t structure
 */
#define PC_PHY_PRBS_CONTROL_FIELD_ID_MAX 5

/*!
 * \brief phy prbs control configuration.
 *
 * This structure is used to configure the phy prbs control settings.
 */
typedef struct bcmpc_phy_prbs_control_s {
    /*! Bitmap of fields to be operated */
    SHR_BITDCLNAME(fbmp, PC_PHY_PRBS_CONTROL_FIELD_ID_MAX);

    /*! When set, PRBS Generator is enabled. */
    bool generator_enable;

    /*! When set, PRBS Checker is enabled. */
    bool checker_enable;

    /*! PRBS polynomial mode */
    bcmpc_phy_prbs_poly_t poly_mode;

    /*! When set, data from PRBS Generator is inverted. */
    bool invert_data_tx;

    /*! When set, data to PRBS Checker is inverted. */
    bool invert_data_rx;

} bcmpc_phy_prbs_control_t;
...
/*!
 * \brief PC_PORT_DIAG_CONTROL LT fields information.
 */
typedef enum bcmpc_port_diag_control_lt_fields_e {
    /*! Error mask bits [63:0] */
    BCMPC_FLD_ERROR_MASK_63_0 = 0,
    /*! Error mask bits [127:64] */
    BCMPC_FLD_ERROR_MASK_127_64,
    /*! Error control map */
    BCMPC_FLD_ERROR_CONTROL_MAP,
} bcmpc_port_diag_control_lt_fields_t;

/*!
 * Number of fields in PC_PORT_DIAG_CONTROL LT.
 * Refer to \ref bcmpc_port_diag_control_lt_fields_t enum
 */
#define PC_PORT_DIAG_CONTROL_FIELD_ID_MAX 3


/*!
 * \brief port diag control configuration.
 *
 * This structure is used to configure the port diag control settings.
 */
typedef struct bcmpc_port_diag_control_s {
    /*! Bitmap of fields to be operated */
    SHR_BITDCLNAME(fbmp, PC_PORT_DIAG_CONTROL_FIELD_ID_MAX);

    /*!
     * Error mask used to generate errors in bits 0-63 of a given
     * Forward Error Correction (FEC) block.
     */
    uint64_t error_mask_63_0;

    /*!
     * Error mask used to generate errors in bits 64-127 of a given
     * FEC block. Note that for CL74 FEC, only bits [64,65] are relevant
     * and for RSFEC, only bits [64,79] are relevant.
     */
    uint64_t error_mask_127_64;

    /*!
     * Error control map, has the following meanings depending on FEC
     * config on the port:
     * CL74 FEC (BASE-R FEC): Specify the lane-map of the port to inject
     *                        errors into. Note that only bits [0,3] are
     *                        valid.
     * RS FEC : Specify the bit-map of 80-bit blocks to inject the error
     *          into. For 1xN FEC mode, only bits [0,3] are valid and
     *          for 2xN FEC mode, only bits [0,7] are valid.
     */
    uint32_t error_control_map;

} bcmpc_port_diag_control_t;
...
/*!
 * \brief Get the PRBS status without FAST BER.
 *
 * \param [in] unit Unit number.
...
/*!
 * \brief Get the PRBS status along with FAST BER.
 *
 * \param [in] unit Unit number.
 * \param [in] pport Physical port number.
 * \param [in] lane_idx Logical lane index of the port.
 ...
 */
extern int
bcmpc_phy_prbs_status_info_get(int unit, bcmpc_pport_t  pport, int lane_idx,
                               uint32_t flags, bcmpc_phy_prbs_status_t *status);
