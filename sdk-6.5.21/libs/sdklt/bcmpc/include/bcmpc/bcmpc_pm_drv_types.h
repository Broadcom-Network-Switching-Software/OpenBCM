...
/*! Structure definition for diag control info */
typedef struct pm_port_diag_control_s {
    /*! Error mask used to generate errors in bits [63:0] of a given FEC block */
    uint64_t error_mask_63_0;
    /*!
     * Error mask used to generate errors in :
     * - bits [79:64] of a given RS FEC block.
     * - bits [65:64] of a given BASE-R FEC block.
     */
    uint64_t error_mask_127_64;
    /*!
     * Error control map, which is:
     * - a lane bitmap for Base-R FEC
     * - block bitmap for RS FEC
     */
    uint32_t error_control_map;
} pm_port_diag_control_t;
