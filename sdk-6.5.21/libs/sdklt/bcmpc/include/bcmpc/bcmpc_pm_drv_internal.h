...
/*!
 * \brief Get Base-R FEC uncorrectable_blk counter for a port
 *
 * \param [in] unit Unit number.
 * \param [in] pm_acc MAC and PHY access information structure.
 * \param [in] pm_info PM state structure.
 * \param [out] Base-R FEC uncorrectable_blk counter
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_PARAM Invalid input parameter.
 * \retval SHR_E_ACCESS Failed to access the register.
 */
typedef int
(*bcmpc_phy_fec_uncorrectable_blk_counter_get_f)(int unit, const pm_access_t *pm_acc,
                                                 pm_info_t *pm_info,
                                                 uint64_t *count);

/*!
 * \brief Get Base-R FEC correctable_blk counter for a port
 *
 * \param [in] unit Unit number.
 * \param [in] pm_acc MAC and PHY access information structure.
 * \param [in] pm_info PM state structure.
 * \param [out] Base-R FEC correctable_blk counter
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_PARAM Invalid input parameter.
 * \retval SHR_E_ACCESS Failed to access the register.
 */
typedef int
(*bcmpc_phy_fec_correctable_blk_counter_get_f)(int unit, const pm_access_t *pm_acc,
                                               pm_info_t *pm_info,
                                               uint64_t *count);
/*!
 * \brief PM uninit.
 ...
 */
typedef int
(*bcmpc_pm_uninit_f)(int unit, const pm_access_t *pm_acc, pm_info_t *pm_info);

/*!
 * \brief MAC string based operation execution.
 *
 * \param [in] unit Unit number.
 * \param [in] pm_acc MAC and PHY access information structure.
 * \param [out] op_st Operation status for this function call.
 ...
 */
typedef int
(*bcmpc_pm_mac_op_exec_f)(int unit, const pm_access_t *pm_acc,
                          pm_oper_status_t *op_st, pm_info_t *pm_info,
                          const char *op, uint32_t op_param);

/*!
 * \brief Set port Diag Control parameters
 *
 * \param [in] pm_acc MAC and PHY access information structure.
 * \param [in] pm_info PM state structure.
 * \param [in] port_diag_ctrl Port Diag control info
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_PARAM Invalid input parameter.
 * \retval SHR_E_ACCESS Failed to access the register.
 */
typedef int
(*bcmpc_port_diag_control_set_f)(int unit, const pm_access_t *pm_acc,
                                 pm_info_t *pm_info,
...
typedef struct bcmpc_pm_drv_s {
    ...
    /*! Port Macro FEC cl74 uncorrectable block counter get. */
    bcmpc_phy_fec_uncorrectable_blk_counter_get_f
                                     pm_phy_fec_uncorrectable_blk_counter_get;

    /*! Port Macro FEC cl74 correctable block counter get. */
    bcmpc_phy_fec_correctable_blk_counter_get_f
                                     pm_phy_fec_correctable_blk_counter_get;

    /*! PM uninit  */
    bcmpc_pm_init_f pm_uninit;

    /*! PM MAC string based operation execution. */
    bcmpc_pm_mac_op_exec_f pm_mac_op_exec;

    /*! Port diag control set. */
    bcmpc_port_diag_control_set_f pm_diag_control_set;
