...
#include "bcmpc_pm_drv_pm8x100.h"
...
#define PM8X100_NUM_LANES_PER_CORE 8

/* Number of lanes per PML in PM8x100. */
#define PM8X100_NUM_LANES_PER_PML 4

#define PM8X100_NRZ_LANE_SPEED_MAX 25000
#define PM8X100_50G_PAM4_LANE_SPEED_MAX 50000
#define PM8X100_PHY_SIGNALLING_MODE_GET(speed, num_lanes) \
        (speed/num_lanes) > PM8X100_50G_PAM4_LANE_SPEED_MAX ? \
                        phymodSignallingMethodPAM4 : \
        (speed/num_lanes) > PM8X100_NRZ_LANE_SPEED_MAX ? \
                        phymodSignallingMethod50gPAM4 : phymodSignallingMethodNRZ

#define PM8X100_OS5_SUPPORT_SPEED_MODE(speed, num_lanes) \
                                    (speed/num_lanes) == 10000 ? 1 : 0
#define PM8X100_PML_INDEX_GET(lane_mask) \
                              (lane_mask) & 0xf ? 0 : 1

#define PM8X100_PMD_FW_INFO_SIZE  128
#define PM8X100_MAX_AN_ABILITY 20
#define PM8X100_MAX_SPEED_ABILITY 60
/******************************************************************************
 * PM driver private functions.
 */
static void
pm8x100_port_index_get(const pm_access_t *pm_acc,
                       pm_info_t *pm_info, int *index)
{
    ...
    for (idx = 0; idx < PM8X100_NUM_LANES_PER_CORE; idx++) {
 ...
 *  MPP1:   Port 6, Port 7.  PML1
 */
static int
pm8x100_mpp_sister_port_index_get(int port_index, int *sister_port_index)
{
    switch (port_index) {
        case 0:
            *sister_port_index = 1;
 ...
 */
static void
pm8x100_port_valid_check(pm_info_t *pm_info, int port_index, int *is_valid)
{
...
static int
pm8x100_pll_to_vco_get(int unit, const pm_access_t *pm_acc,
...
static int
pm8x100_vco_to_pll_get(int unit, pm_oper_status_t *op_st, pm_info_t *pm_info,
...
static void
pm8x100_fec_lanebitmap_get(pm_info_t *pm_info,
                        ...
                        uint8_t *affected_lane_bitmap)
{
    ...
    pml_index = PM8X100_PML_INDEX_GET(lane_mask);
...
static int
pm8x100_pm_port_lane_map_validate(int unit, pm_oper_status_t *op_st,
...
static int
pm8x100_speed_to_vco_get(int unit, pm_oper_status_t *op_st,
                             pm_info_t *pm_info, pm_speed_config_t *speed_cfg,
                             pm_vco_t *vco)
{
    int idx;
    const pm8x100_force_speed_ability_entry_t *abil = NULL;

    SHR_FUNC_ENTER(unit);

    for (idx = 0; idx < COUNTOF(pm8x100_force_speed_ability_table); idx++) {
        abil = &(pm8x100_force_speed_ability_table[idx]);
...
static int
pm8x100_autoneg_ability_to_vco_get(int unit,
                                       pm_info_t *pm_info,
                                       pm_port_ability_t ability,
                                       pm_vco_t *vco)
{
    int idx;
    const pm8x100_autoneg_ability_table_entry_t *abil = NULL;
    SHR_FUNC_ENTER(unit);

    *vco = PM_VCO_INVALID;

    for (idx = 0; idx < COUNTOF(pm8x100_autoneg_ability_table); idx++) {
        abil = &pm8x100_autoneg_ability_table[idx];
...
static int
pm8x100_port_mode_update(int unit, const pm_access_t *pm_acc,
...
static int
pm8x100_pm_vco_reconfig(int unit, const pm_access_t *pm_acc,
                            pm_oper_status_t *op_st, pm_info_t *pm_info,
                            pm_vco_t new_vco)
{
    uint32_t pll_div;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_vco_to_pll_get(unit, op_st, pm_info, new_vco, &pll_div));
 ...
 */
static void
pm8x100_lane_mask_get(int port_index, int num_lane, uint32_t *lane_mask)
{
    int i;
...
/*
 * This function will get the auto-negotiation encap mode for the given PML.
 *
 * \param [in] pm_info PM DB data structure.
 * \param [in] pml_index PML index.
 ...
 *        PM_ENCAP_HG3: PM is in HiGig3 AN mode.
 *                                   Only HiGig3 AN can be enabled.
 */
static int
pm8x100_pml_an_encap_mode_get(int unit, const pm_access_t *pm_acc,
                              pm_oper_status_t *op_st, pm_info_t *pm_info,
                              int pml_index, int *encap)
{
    int idx, pml_start_lane;
    ...
    pml_start_lane = pml_index * PM8X100_NUM_LANES_PER_PML;
    /*
     * PM8x100 auto-negotiation(AN) only support two encap mode in AN:
     * PM_ENCAP_IEEE or PM_ENCAP_HG3.
     * Within each PML, HiGig3 and IEEE mode can not co-exist in AN.
     * If there's any AN port using HiGig3 encap mode, PML is in HiGig3 AN mode.
     * If there's any AN port using IEEE encap mode, PML is in IEEE AN mode.
     * If no AN port within the PML, user can choose AN encap mode freely.
     */

    for (idx = 0; idx < PM8X100_NUM_LANES_PER_PML; idx++) {
        /* Get lane index of the first lane of the first AN port on the PM. */
        if (!((pm_ha_info->an_enable_lane_bmap >> (idx + pml_start_lane))
                                                                       & 0x1)) {
            continue;
        }
        pm_acc_copy.pmac_blkport = idx;
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_common_port_encap_get(unit, &pm_acc_copy, op_st,
                                           pm_info, &tmp_encap));
 ...
 */
static int
pm8x100_pm_port_config_validate(int unit, const pm_access_t *pm_acc,
                               pm_oper_status_t *op_st, pm_info_t *pm_info,
                               pm_speed_config_t *spd_cfg, int is_first)
{
    ...
    /* 1. Check lane_map. */
    rv = pm8x100_pm_port_lane_map_validate(unit, op_st, pm_info,
     ...
     */
    if (SHR_FAILURE(pm8x100_speed_to_vco_get(unit, op_st, pm_info,
                                             spd_cfg, &req_vco))) {
        status = op_st->status;
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    if (rs544_req || rs272_req) {
        pm8x100_fec_lanebitmap_get(pm_info, &rs544_bm, &rs272_bm);
        ...
        rv = pm8x100_fec_validate(unit, phy_acc->access.lane_mask, pm_info,
    ...
    if (is_pll_pwrdn) {
        ...
        rv = pm8x100_pll_to_vco_get(unit, pm_acc, op_st, pll_div, &cur_vco);
        if (SHR_FAILURE(rv)) {
            status = op_st->status;
            SHR_ERR_EXIT(rv);
        }
    ...
    /* Either 41.25G or 51.5625G VCO can be used with 5 OSR support. */
    if (PM8X100_OS5_SUPPORT_SPEED_MODE(spd_cfg->speed, spd_cfg->num_lanes)) {
        if (cur_vco == PM_VCO_41P25G) {
            req_vco = PM_VCO_41P25G;
        }
    }
    ...
    if (req_vco != cur_vco) {
        if (vco_change_allowed) {
             ...
             */
            SHR_IF_ERR_VERBOSE_EXIT
                (pm8x100_pm_vco_reconfig(unit, pm_acc, op_st,
                                         pm_info, req_vco));
        } else {
...
static int
pm8x100_pm_core_probe(int unit, const pm_access_t *pm_acc,
                      pm_oper_status_t *op_st, pm_info_t *pm_info)
{
...
static int
pm8x100_pm_serdes_core_init(int unit, const pm_access_t *pm_acc,
                            pm_oper_status_t *op_st, pm_info_t *pm_info,
                            pm_port_add_info_t *add_info)
{
    ...
    pm_vco_t init_vco;
    uint32_t tx_lane_map = 0, rx_lane_map = 0, i, tmp_index;
    pm_ha_info_t *pm_ha_info = pm_info->pm_data.pm_ha_info;
    ...
    if (PM_PORT_ADD_F_INIT_PROBE_GET(add_info)) {
        rv = pm8x100_pm_core_probe(unit, pm_acc, op_st, pm_info);
    }
    if (SHR_FAILURE(rv)) {
        LOG_INFO(BSL_LOG_MODULE,
                 (BSL_META_U(unit,
    ...
    if (add_info->tvco != PM_VCO_INVALID) {
        ...
        /* Get requeseted VCO. */
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_speed_to_vco_get(unit, op_st, pm_info,
                                          &pm_spd_cfg, &init_vco));
    }

    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_vco_to_pll_get(unit, op_st, pm_info,
    ...
    if (PM_PORT_ADD_F_INIT_PASS2_GET(add_info)) {
        ...
        /* next need to check if lane swap flip is true or not */
        if (PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP_GET(add_info)) {
            /* adjust the lane map to accomodate internal lane swap */
            for (i = 0; i < core_config.lane_map.num_of_lanes; i++) {
                tmp_index = (i + 4) % 8;
                core_config.lane_map.lane_map_rx[tmp_index] = add_info->init_config.lane_map.lane_map_rx[i];
                core_config.lane_map.lane_map_tx[tmp_index] = add_info->init_config.lane_map.lane_map_tx[i];
            }
            /* adjust polarity as well */
            core_config.polarity_map.rx_polarity = 0;
            core_config.polarity_map.rx_polarity |= ((add_info->init_config.polarity.rx_polarity) & 0xf) << 4;
            core_config.polarity_map.rx_polarity |= ((add_info->init_config.polarity.rx_polarity) & 0xf0) >> 4;
            core_config.polarity_map.tx_polarity = 0;
            core_config.polarity_map.tx_polarity |= ((add_info->init_config.polarity.tx_polarity) & 0xf) << 4;
            core_config.polarity_map.tx_polarity |= ((add_info->init_config.polarity.tx_polarity) & 0xf0) >> 4;
        }
    }
...
static int
pm8x100_pm_init(int unit, const pm_access_t *pm_acc, pm_info_t *pm_info)
{
    pm_access_t pm_acc_copy;
...
static int
pm8x100_core_attach(int unit, const pm_access_t *pm_acc,
                        pm_oper_status_t *op_st, pm_info_t *pm_info,
                        pm_port_add_info_t *add_info)
{
    pm_access_t pm_acc_copy;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_pm_init(unit, pm_acc, pm_info));
    ...
    /* Populate link simulation setting. */
    if (bcmdrd_feature_enabled(unit, BCMDRD_FT_ACTIVE_SIM)) {
        pm_info->link_simulation = add_info->link_simulation;
    }

    SHR_IF_ERR_VERBOSE_EXIT
       (pm8x100_pm_serdes_core_init(unit, &pm_acc_copy, op_st,
                                        pm_info, add_info));
...
static int
pm8x100_pm_vco_rate_get(int unit, const pm_access_t *pm_acc,
                            pm_oper_status_t *op_st, pm_info_t *pm_info,
                            pm_vco_t *vco_rate)
{
    uint32_t pll_div;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_pll_to_vco_get(unit, pm_acc, op_st, pll_div, vco_rate));
...
static int
pm8x100_port_add(int unit, const pm_access_t *pm_acc,
                     pm_oper_status_t *op_st, pm_info_t *pm_info,
                     pm_port_add_info_t *add_info)
{
    int idx, num_lanes = 0;
    ...
    pm_ha_info->tvco_active_lane_bmap |= lane_mask;
 ...
 */
static int
pm8x100_port_speed_config_set(int unit, const pm_access_t *pm_acc,
                                  pm_oper_status_t *op_st, pm_info_t *pm_info,
                                  pm_speed_config_t *speed_config)
{
    ...
    pm8x100_port_index_get(pm_acc, pm_info, &port_index);
    SHR_BITCOUNT_RANGE(&(phy_acc->access.lane_mask), port_num_lanes,
                       0, PM8X100_NUM_LANES_PER_CORE);
    ...
    sig_mode = PM8X100_PHY_SIGNALLING_MODE_GET(speed_config->speed,
    ...
    if (PM_PORT_USER_F_DEFAULT_LANE_FW_CONFIG_GET(speed_config->flags)) {
        ...
        if (sig_mode != phymodSignallingMethodNRZ) {
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_port_mode_update(unit, pm_acc, op_st, pm_info,
                                  speed_config->speed));
     ...
     * set the same mode in CDMAC_MODRr.HDR_MODE.
    ...
    if (encap == BCMPMAC_ENCAP_IEEE_REDUCED_IPG) {
        SHR_IF_ERR_VERBOSE_EXIT
            (phymod_phy_codec_mode_set(phy_acc, phymodCodecReducedIPG));
    }
...
/*
 * Function:
 *   pm8x100_port_advert_abilities_validate
 * Purpose:
 ...
 */
static int
pm8x100_port_advert_abilities_validate(int unit, const pm_access_t *pm_acc,
                                           ...
                                           pm_port_fec_t *rs_fec_req)
{
    ...
    pm_ha_info_t *pm_ha_info = pm_info->pm_data.pm_ha_info;
    int pml_encap_mode, autoneg_10G_40G = 0;
    pm_encap_t port_encap;
    pm_oper_status_type_t status = PM_OPER_SUCCESS;
    int port_index, sister_port_index;
    ...
    if ((pm_ha_info->tvco_active_lane_bmap == pm_acc->phy_acc.access.lane_mask)
         && !((pm_ha_info->an_enable_lane_bmap >> port_index) & 0x1)) {
        current_vco = PM_VCO_INVALID;
    } else {
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_pm_vco_rate_get(unit, pm_acc, op_st,
                                     pm_info, &current_vco));
    }
    ...
    for (idx = 0; idx < num_abilities; idx++) {
        ...
        rv = pm8x100_autoneg_ability_to_vco_get(unit, pm_info,
                                                abilities[idx], &vco);
        if (SHR_FAILURE(rv)) {
        ...
        if (PM8X100_OS5_SUPPORT_SPEED_MODE(abilities[idx].speed,
     ...
     * since both 41G and 51G VCO will work for these two speeds
     */
    if (autoneg_10G_40G) {
        if ((current_vco != PM_VCO_51P562G) &&
            (current_vco != PM_VCO_41P25G) &&
            (current_vco != PM_VCO_INVALID)) {
            status = PM_VCO_UNAVAIL;
            SHR_ERR_EXIT(SHR_E_FAIL);
        }
    ...
    if ((port_num_lanes == 1) && (is_bam || is_msa)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_mpp_sister_port_index_get(port_index, &sister_port_index));
        pm8x100_port_valid_check(pm_info, sister_port_index, &port_is_valid);
        if (port_is_valid) {
    ...
    if (rs544_req || rs272_req) {
        ...
        rv = (pm8x100_fec_validate(unit, phy_access->access.lane_mask, pm_info,
                                   rs544_bm, rs272_bm, &affected_bm));
        if (SHR_FAILURE(rv)) {
            status = PM_ABILITY_FEC_CONFLICT;
            SHR_ERR_EXIT(rv);
    ...
    /*
     *  4. PML's encap mode can support current port's request.
     ...
     */
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_pml_an_encap_mode_get(unit, pm_acc, op_st, pm_info,
                            PM8X100_PML_INDEX_GET(phy_access->access.lane_mask),
                            &pml_encap_mode));
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_common_port_encap_get(unit, pm_acc, op_st,
                                       pm_info, &port_encap));
    if ((pml_encap_mode != -1) && (pml_encap_mode != (int)port_encap)) {
        status = PM_ABILITY_AN_CONFLICT_CFG;
        SHR_ERR_EXIT(SHR_E_FAIL);
    }
 ...
 */
static int
pm8x100_port_ability_advert_set(int unit, const pm_access_t *pm_acc,
                                    pm_oper_status_t *op_st,
                                    ...
                                    pm_port_ability_t *ability)
{
    ...
    phymod_autoneg_advert_ability_t
                                  autoneg_abilities[PM8X100_MAX_AN_ABILITY];
    ...
    sal_memset(autoneg_abilities, 0,
          PM8X100_MAX_AN_ABILITY * sizeof(phymod_autoneg_advert_ability_t));

    /* 1. Validate abilities. */
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_port_advert_abilities_validate(unit, pm_acc, op_st,
...
static int
pm8x100_port_ability_advert_get(int unit, const pm_access_t *pm_acc,
                                    pm_oper_status_t *op_st,
                                    pm_info_t *pm_info,
                                    ...
                                    pm_port_ability_t *ability)
{
    ...
    for (idx = 0; idx < PM8X100_MAX_AN_ABILITY; idx++) {
        SHR_IF_ERR_VERBOSE_EXIT
...
static int
pm8x100_port_ability_remote_get(int unit, const pm_access_t *pm_acc,
                                   pm_oper_status_t *op_st,
                                   pm_info_t *pm_info,
                                   uint32_t max_num_abilities,
                                   uint32_t *actual_num_abilities,
...
static int
pm8x100_port_autoneg_set(int unit, const pm_access_t *pm_acc,
                             pm_oper_status_t *op_st, pm_info_t *pm_info,
                             phymod_autoneg_control_t *an)
{
    ...
    pm_port_autoneg_mode_t an_mode;
    pm_port_ability_t abilities[PM8X100_MAX_AN_ABILITY];
    int port_num_lanes, port_index, pml_index;
    ...
    if (an_control.enable) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_port_ability_advert_get(unit, pm_acc, op_st, pm_info,
                                                 PM8X100_MAX_AN_ABILITY,
        ...
        if (actual_num_abilities == 0) {
            ...
            for (idx = 0; idx < actual_num_abilities; idx++) {
                if (PM8X100_OS5_SUPPORT_SPEED_MODE(abilities[idx].speed,
                     ...
                     */
                    SHR_IF_ERR_VERBOSE_EXIT
                        (pm8x100_autoneg_ability_to_vco_get(unit, pm_info,
        ...
        if ((req_vco == PM_VCO_INVALID) && is_os5_support) {
        ...
        /*
         * Encap validation is done in pm8x100_port_ability_advert_set.
         * Here just need to make sure the SPEED ID table loaded can serve
         * the given port.
         */
        pml_index = PM8X100_PML_INDEX_GET(lane_mask_backup);
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_common_port_encap_get(unit, pm_acc, op_st,
                                           pm_info, &encap));
        if (encap == BCMPMAC_ENCAP_HG3) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_port_mode_update(unit, &pm_acc_copy, op_st,
...
static int
pm8x100_port_autoneg_get(int unit, const pm_access_t *pm_acc,
                             pm_oper_status_t *op_st, pm_info_t *pm_info,
                             phymod_autoneg_control_t *an)
{
    const phymod_phy_access_t *phy_access;
...
static int
pm8x100_port_autoneg_status_get(int unit, const pm_access_t *pm_acc,
                                    pm_oper_status_t *op_st, pm_info_t *pm_info,
...
static int
pm8x100_port_link_status_get(int unit, const pm_access_t *pm_acc,
                                 pm_oper_status_t *op_st, pm_info_t *pm_info,
                                 uint32_t *link_status)
{
    ...
    bcmpmac_drv_t *pmacd;
    bcmpmac_access_t pmac_acc;
    int port_index = -1, link = 0;

    SHR_FUNC_ENTER(unit);

    if ((bcmdrd_feature_enabled(unit, BCMDRD_FT_ACTIVE_SIM)) &&
        (pm_info->link_simulation)) {
        pmacd = pm_info->pm_data.pmacd;
        sal_memcpy(&pmac_acc, &(pm_acc->pmac_acc), sizeof(bcmpmac_access_t));
        pm8x100_port_index_get(pm_acc, pm_info, &port_index);
        SHR_IF_ERR_VERBOSE_EXIT
            (pmacd->port_link_status_get(&pmac_acc, pm_acc->pmac_blkport, 1,
                                         port_index % PM8X100_NUM_LANES_PER_PML,
                                         &link));
            *link_status = (link)? 1: 0;
    } else {
        PHY_IF_ERR_EXIT
...
static int
pm8x100_port_pmd_lane_cfg_set(int unit, const pm_access_t *pm_acc,
                                  pm_oper_status_t *op_st, pm_info_t *pm_info,
 ...
 */
static int
pm8x100_port_ability_local_get(int unit, const pm_access_t *pm_acc,
                                   pm_oper_status_t *op_st, pm_info_t *pm_info,
                                   uint32_t max_num_abilities,
                                   uint32_t *actual_num_abilities,
                                   pm_port_ability_t *abilities)
{
    ...
    const phymod_phy_access_t *phy_acc = &(pm_acc->phy_acc);
    const pm8x100_force_speed_ability_entry_t *fs_abil = NULL;
    const pm8x100_autoneg_ability_table_entry_t *an_abil = NULL;

    SHR_BITCOUNT_RANGE(&(phy_acc->access.lane_mask), port_num_lanes,
                       0, PM8X100_NUM_LANES_PER_CORE);

    for (idx = 0; idx < COUNTOF(pm8x100_force_speed_ability_table); idx++) {
        fs_abil = &pm8x100_force_speed_ability_table[idx];
    ...
    for (idx = 0; idx < COUNTOF(pm8x100_autoneg_ability_table); idx++) {
        an_abil = &pm8x100_autoneg_ability_table[idx];
...
static int
pm8x100_port_abilities_get(int unit, const pm_access_t *pm_acc,
                               pm_oper_status_t *op_st, pm_info_t *pm_info,
                               ...
                               pm_port_ability_t *pabilities)
{
    ...
    if (ability_type == PM_PORT_ABILITY_LOCAL) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_port_ability_local_get(unit, pm_acc, op_st, pm_info,
                                            PM8X100_MAX_SPEED_ABILITY,
...
static int
pm8x100_port_timesync_config_set(int unit, const pm_access_t *pm_acc,
                                    pm_oper_status_t *op_st, pm_info_t *pm_info,
...
static int
pm8x100_port_detach(int unit, const pm_access_t *pm_acc,
                        pm_oper_status_t *op_st, pm_info_t *pm_info)
{
    pm_ha_info_t *pm_ha_info = pm_info->pm_data.pm_ha_info;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_port_timesync_config_set(unit, pm_acc, op_st,
                                              pm_info, &ts_config));
    ...
    if (!pm_ha_info->tvco_active_lane_bmap) {
        /* Power off PM.  */
        /*
         * Both PMLs need to be power down.
         * Here we just update the PM software state.
         * The power down logic need to be done in upper layer for both PMLs.
         */
 ...
 */
static int
pm8x100_port_phy_link_up_event(int unit, const pm_access_t *pm_acc,
                                   pm_oper_status_t *op_st, pm_info_t *pm_info)
{
    const phymod_phy_access_t *phy_acc = &(pm_acc->phy_acc);
    pm_ha_info_t *pm_ha_info = pm_info->pm_data.pm_ha_info;
    ...
    pm_port_ability_t advert_ability[PM8X100_MAX_AN_ABILITY];
    pm_port_ability_t remote_ability[PM8X100_MAX_AN_ABILITY];
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pm8x100_port_autoneg_status_get(unit, pm_acc, op_st,
                                             pm_info, &an_status));
    if ((an_status.enabled && an_status.locked)) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_port_ability_remote_get(unit, pm_acc, op_st, pm_info,
                                                 PM8X100_MAX_AN_ABILITY,
 ...
 */
static int
pm8x100_port_phy_link_down_event(int unit, const pm_access_t *pm_acc,
                                     pm_oper_status_t *op_st,
                                     pm_info_t *pm_info)
{
    const phymod_phy_access_t *phy_acc = &(pm_acc->phy_acc);
...
static int
pm8x100_port_rlm_state_update(int unit, const pm_access_t *pm_acc,
                                  pm_oper_status_t *op_st, pm_info_t *pm_info)
{
    const phymod_phy_access_t *phy_acc = &(pm_acc->phy_acc);
            ...
            if (is_initiator) {
                ...
                if (pcs_is_reconfigured) {
                   ...
                   /* read twice the pcs link status */
                    SHR_IF_ERR_VERBOSE_EXIT
                        (pm8x100_port_link_status_get(unit, pm_acc, op_st,
                                                          pm_info, &pcs_link));
                    SHR_IF_ERR_VERBOSE_EXIT
                        (pm8x100_port_link_status_get(unit, pm_acc, op_st,
                                                          pm_info, &pcs_link));
            ...
            if (!pcs_is_reconfigured) {
                ...
                if ((!prbs_enabled) || follower_prbs_resend)  {
                    for (i = 0; i < num_lane; i++) {
                        ...
                        /* if 8 lane port, need to set the 8 lane port mask */
                        if (num_lane == 8) {
                            phy_copy.access.lane_mask |= 0x1 << 8;
                        }
                        if (active_lane_map &  1 << (port_index + i)) {
...
static int
pm8x100_port_sw_state_update(int unit, const pm_access_t *pm_acc,
                                 pm_oper_status_t *op_st, pm_info_t *pm_info)
{
   ...
   if (rlm_enabled == 1) {
        ...
        if ((local_port_rlm_state != PM8X100_COMMON_PORT_RLM_DONE) &&
            (local_port_rlm_state != PM8X100_COMMON_PORT_RLM_ERROR)) {
            SHR_IF_ERR_VERBOSE_EXIT
                (pm8x100_port_rlm_state_update(unit, pm_acc,
                                                   op_st, pm_info));
        }
    }
...
static int
pm8x100_port_rlm_config_set(int unit, const pm_access_t *pm_acc,
                                pm_oper_status_t *op_st, pm_info_t *pm_info,
                                pm_port_rlm_config_t *rlm_config)
{
    ...
    if (rlm_config->rlm_enable) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pm8x100_port_speed_config_set(unit, pm_acc, op_st,
                                               pm_info, &local_speed_config));
...
static int
pm8x100_port_rlm_status_get(int unit, const pm_access_t *pm_acc,
                                pm_oper_status_t *op_st, pm_info_t *pm_info,
...
static int
pm8x100_pm_vco_calculate(int unit, pm_oper_status_t *op_st,
                             pm_info_t *pm_info, pm_vco_setting_t *vco_select)
{
    ...
    for (i = 0; i < vco_select->num_speeds; i++) {
        ...
        if (vco_select->port_starting_lane_list != NULL) {
            pm8x100_lane_mask_get(vco_select->port_starting_lane_list[i],
        ...
        /* 2. Validate force speed ability. */
        rv = pm8x100_speed_to_vco_get(unit, op_st, pm_info,
        ...
        if (PM8X100_OS5_SUPPORT_SPEED_MODE(current_speed_config->speed,
    ...
    rv = pm8x100_fec_validate(unit, lane_mask, pm_info, rs544_bm,
    ...
    if (is_os5_support) {
        if (current_vco == PM_VCO_INVALID) {
            current_vco = PM_VCO_51P562G;
        } else if ((current_vco != PM_VCO_41P25G) &&
                   (current_vco != PM_VCO_51P562G)) {
            status = PM_VCO_UNAVAIL;
            SHR_ERR_EXIT(SHR_E_UNAVAIL);
        }
...
static int
pm8x100_port_default_lane_config_get(int unit, const pm_access_t *pm_acc,
                                    pm_oper_status_t *op_st, pm_info_t *pm_info,
                                    pm_speed_config_t *speed_config,
...
pm8x100_i_phymod_intr_type_get (bcmpc_intr_type_t pc_intr_type)
{
    ...
    switch (pc_intr_type) {
        ...
        case BCMPC_INTR_ECC_2B_ERR_UM:
            ...
            break;
        case BCMPC_INTR_ECC_1B_ERR_TX_1588_MPP0:
        case BCMPC_INTR_ECC_2B_ERR_TX_1588_MPP0:
...
static int
pm8x100_port_intr_enable_get(int unit, const pm_access_t *pm_acc,
                             pm_oper_status_t *op_st, pm_info_t *pm_info,
                             ...
                             uint32_t *enable)
{
    ...
    intr_type = pm8x100_i_phymod_intr_type_get(pc_intr_type);
    if (phymodIntrNone == intr_type) {
        SHR_EXIT();
    }
...
static int
pm8x100_port_intr_enable_set(int unit, const pm_access_t *pm_acc,
                             pm_oper_status_t *op_st, pm_info_t *pm_info,
                             bcmpc_intr_type_t pc_intr_type,
                             uint32_t enable)
{
...
static int
pm8x100_port_interrupt_process(int unit, const pm_access_t *pm_acc,
                               pm_oper_status_t *op_st, pm_info_t *pm_info,
                               bcmpc_intr_type_t pc_intr_type,
                               uint32_t *intr_cleared)
{
    ...
    if (!(*intr_cleared)) {
        LOG_WARN(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "PHYMOD did not handle ECC interrupt(%d)\n"),
                 intr_type));
    }
exit:
    SHR_FUNC_EXIT();
...
bcmpc_pm_drv_t bcmpc_pm_drv_pm8x100 = {
    
    .speed_config_validate = pm8x100_speed_config_validate,
    .core_attach = pm8x100_core_attach,
    .port_detach = pm8x100_port_detach,
    .pm_init = pm8x100_pm_init,
    .pm_destroy = pm8x100_common_pm_destroy,
    .port_add = pm8x100_port_add,
    .pm_port_speed_config_set = pm8x100_port_speed_config_set,
    .pm_port_speed_config_get = pm8x100_common_port_speed_config_get,
    .pm_port_config_validate = pm8x100_pm_port_config_validate,
    .pm_port_loopback_set = pm8x100_common_port_loopback_set,
    .pm_port_loopback_get = pm8x100_common_port_loopback_get,
    .pm_port_ability_advert_set = pm8x100_port_ability_advert_set,
    .pm_port_ability_advert_get = pm8x100_port_ability_advert_get,
    .pm_port_ability_remote_get = pm8x100_port_ability_remote_get,
    .pm_port_autoneg_set = pm8x100_port_autoneg_set,
    .pm_port_autoneg_get = pm8x100_port_autoneg_get,
    .pm_port_autoneg_status_get = pm8x100_port_autoneg_status_get,
    ...
    .pm_port_link_status_get = pm8x100_port_link_status_get,
    ...
    .pm_port_pmd_lane_cfg_set = pm8x100_port_pmd_lane_cfg_set,
    ...
    .pm_port_abilities_get = pm8x100_port_abilities_get,
    .pm_vco_get = pm8x100_pm_vco_rate_get,
    .pm_port_timesync_config_set = pm8x100_port_timesync_config_set,
    ...
    .pm_port_phy_link_up_event = pm8x100_port_phy_link_up_event,
    .pm_port_phy_link_down_event = pm8x100_port_phy_link_down_event,
    .pm_port_sw_state_update = pm8x100_port_sw_state_update,
    .pm_port_rlm_config_set = pm8x100_port_rlm_config_set,
    .pm_port_link_training_status_get =
                                    pm8x100_common_port_link_training_status_get,
    .pm_port_rlm_status_get = pm8x100_port_rlm_status_get,
    .pm_vco_rate_calculate = pm8x100_pm_vco_calculate,
    .pm_default_pmd_lane_config_get = pm8x100_port_default_lane_config_get,
    ...
    .pm_phy_rsfec_uncorrectable_counter_get =
                             pm8x100_common_phy_rsfec_uncorrectable_counter_get,
    .pm_phy_rsfec_correctable_counter_get =
                             pm8x100_common_phy_rsfec_correctable_counter_get,
    .pm_phy_rsfec_symbol_error_counter_get =
                             pm8x100_common_phy_rsfec_symbol_error_counter_get,
    .port_intr_enable_get = pm8x100_port_intr_enable_get,
    .port_intr_enable_set = pm8x100_port_intr_enable_set,
    .port_interrupt_process = pm8x100_port_interrupt_process,
    .pm_diag_control_set = pm8x100_common_diag_control_set,
};
