...
/*
 * Convenient macro for checking if aging support on the device.
 * PM8x50 GEN2 (revision B0) is software aging enabled.
 */
#define PM8X50_AGING_SUPPORT(unit, pm_type, aging_support)  { \
        do { \
            bcmdrd_dev_id_t dev_id; \
            SHR_IF_ERR_EXIT(bcmdrd_dev_id_get(unit,&dev_id)); \
            aging_support = ((dev_id.revision == BCM56880_REV_B0) && \
                 (pm_type == PM_TYPE_PM8X50_GEN2)) ? 1 : 0; \
        } while (0); \
    }

/* Configured port(s) is on MPP0 */
#define MPP0_PORT(lane_mask) (lane_mask & 0xf)

/* Configured port(s) is on MPP1 */
#define MPP1_PORT(lane_mask) (lane_mask & 0xf0)

/* Lane 0 on MPP0. */
#define MPP0_LANE0(lane_mask) (lane_mask & 0x1)

/* Lane 4 on MPP1. */
#define MPP1_LANE4(lane_mask) (lane_mask & 0x10)
...
{
    int rv = 0;
    uint32_t tvco_pll = 0, ovco_pll = 0, init_tvco = 0, init_ovco = 0;
    ...
    pm_ha_info_t *pm_ha_info = pm_info->pm_data.pm_ha_info;
    int aging_support = 0, ovco_pll_idx;
    ...
    if  (pm_acc->phy_acc.access.tvco_pll_index) {
        /* Get associated PLL, tvco uses ovco_pll */
        SHR_IF_ERR_VERBOSE_EXIT
    ...
    pm_port_init_pll_get(unit, tvco_pll, &init_tvco);
    pm_port_init_pll_get(unit, ovco_pll, &init_ovco);
    core_config.pll0_div_init_value = init_tvco;
    core_config.pll1_div_init_value = init_ovco;
    ...
    pm_ha_info->rlm_original_tx_lane_swap = tx_lane_map;
     ...
     /* Check aging support. */
    PM8X50_AGING_SUPPORT(unit, pm_info->type, aging_support);
    if (aging_support && (pm_acc->phy_acc.type != phymodDispatchTypeNull)) {
        sal_memcpy(&phy_acc_copy, &pm_acc->phy_acc, sizeof(phy_acc_copy));
        ovco_pll_idx = (pm_acc->phy_acc.access.tvco_pll_index) ? 0 : 1;
        if (init_tvco != (uint32_t)phymod_TSCBH_PLL_DIVNONE) {
            pm_ha_info->serdes_core_tvco_power_state = PM_SERDES_CORE_PLL_PWR_ON;
        }
        /* Power down serdes OVCO core if it is not used. */
        if (init_ovco == (uint32_t)phymod_TSCBH_PLL_DIVNONE) {
            phy_acc_copy.access.pll_idx = ovco_pll_idx;
            SHR_IF_ERR_VERBOSE_EXIT
                (phymod_phy_serdes_power_set(&phy_acc_copy,
                                             phymodSerdesOptionCore,
                                             phymodSerdesPWRDN_DEEP));
            pm_ha_info->serdes_core_ovco_power_state = PM_SERDES_CORE_PLL_PWR_DOWN;
        } else {
            pm_ha_info->serdes_core_ovco_power_state = PM_SERDES_CORE_PLL_PWR_ON;
        }
        /*
         * lane_power_down_bmap is to track which lane(s) is explicitly to
         * call PWRDN_DEEP with phymodSerdesOptionLane on serdes.
         */
        pm_ha_info->serdes_lane_power_down_bmap = 0;
    }
...
{
    pm_ha_info_t *pm_ha_info = pm_info->pm_data.pm_ha_info;
    int idx, num_lanes = 0, rv = SHR_E_NONE, aging_support = 0;
    phymod_phy_init_config_t init_config;
    const phymod_phy_access_t *phy_acc = &(pm_acc->phy_acc);
    phymod_phy_access_t phy_acc_copy;
    uint32_t sw_war_100G2lane_mask = 0, tmp_lane_mask = 0;
    /* Local variable to store the lane_mask of phymod_access_t. */
    uint32_t phyacc_lane_mask = 0;
    ...
    if (num_lanes != 8) {
        ...
        if (!bcmdrd_feature_enabled(unit, BCMDRD_FT_PHYMOD_SIM)) {
            if (MPP0_PORT(phy_acc->access.lane_mask)) {
            ...
            if (MPP1_PORT(phy_acc->access.lane_mask)) {
                if (pm_ha_info->is_100g_2lane_sw_war_mpp1_is_done == 0) {
                    PHYMOD_PHY_INIT_F_SW_WAR_100G_2LANE_MPP1_IS_DONE_CLR(&init_config);
                    pm_ha_info->is_100g_2lane_sw_war_mpp1_is_done = 1;
                } else {
    ...
    PM8X50_AGING_SUPPORT(unit, pm_info->type, aging_support);
    if (aging_support && (pm_acc->phy_acc.type != phymodDispatchTypeNull)) {
        if (pm_ha_info->is_pm_initialized) {
            phy_acc_copy.access.lane_mask = 0;
            /* The configured port is on MPP0. */
            if (MPP0_PORT(pm_ha_info->serdes_lane_power_down_bmap) &&
                MPP0_PORT(phy_acc->access.lane_mask)) {
                /*
                 * Power on the previously powered off lane(s) if it is
                 * used in configured port.
                 */
                tmp_lane_mask = phy_acc->access.lane_mask ^
                                       pm_ha_info->serdes_lane_power_down_bmap;

                if (MPP0_PORT(tmp_lane_mask)) {
                    phyacc_lane_mask = phy_acc->access.lane_mask &
                            MPP0_PORT(pm_ha_info->serdes_lane_power_down_bmap);
                } else {
                    phyacc_lane_mask = phy_acc->access.lane_mask;
                }
                /*
                 * Power on the lane 0 on MPP0 if the lane was powered off.
                 * It is required for 100G 2 lane SW WAR in phymod_phy_init.
                 */
                if (MPP0_LANE0(pm_ha_info->serdes_lane_power_down_bmap) &&
                    !MPP0_LANE0(phyacc_lane_mask)) {
                    sw_war_100G2lane_mask = 0x1;
                }
            } else if (MPP1_PORT(pm_ha_info->serdes_lane_power_down_bmap) &&
                       MPP1_PORT(phy_acc->access.lane_mask)) {
                /*
                 * Power on the previously powered off lane(s) if it is
                 * used in configured port.
                 */
                tmp_lane_mask = phy_acc->access.lane_mask ^
                                      pm_ha_info->serdes_lane_power_down_bmap;
                if (MPP1_PORT(tmp_lane_mask)) {
                    phyacc_lane_mask = phy_acc->access.lane_mask &
                            MPP1_PORT(pm_ha_info->serdes_lane_power_down_bmap);
                } else {
                    phyacc_lane_mask = phy_acc->access.lane_mask;
                }
                /*
                 * Power on the lane 4 on MPP1 if the lane was powered off.
                 * It is required for 100G 2 lane SW WAR in phymod_phy_init.
                 */
                if (MPP1_LANE4(pm_ha_info->serdes_lane_power_down_bmap) &&
                    !MPP1_LANE4(phyacc_lane_mask)) {
                    sw_war_100G2lane_mask = 0x10;
                }
            }
            /* Copy local lane_mask to phymod_phy_access_t. */
            phy_acc_copy.access.lane_mask |= phyacc_lane_mask;
            /* Copy SW WAR lane0 (or lane4) to phymod_phy_access_t if apply. */
            phy_acc_copy.access.lane_mask |= sw_war_100G2lane_mask;
            if (phy_acc_copy.access.lane_mask) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (phymod_phy_serdes_power_set(&phy_acc_copy,
                                                 phymodSerdesOptionLane,
                                                 phymodSerdesPWR_ON));
                /* Update lane_power_down_bmap */
                tmp_lane_mask = phy_acc->access.lane_mask;
                pm_ha_info->serdes_lane_power_down_bmap &= ~tmp_lane_mask;
            }
        }
    }

    rv = phymod_phy_init(phy_acc, &init_config);

    /*
     * Power off the lane (lane 0 or lane 4) that was powered on for
     * 100G 2 lane SW WAR in phymod_phy_init.
     */
    if (aging_support && (pm_acc->phy_acc.type != phymodDispatchTypeNull)) {
        if (rv == SHR_E_NONE) {
            phy_acc_copy.access.lane_mask = sw_war_100G2lane_mask;
                   ...
                   pm_vco_t *vcos)
{
    ...
    if (aging_support &&
        (pm_ha_info->serdes_core_tvco_power_state ==
                                PM_SERDES_CORE_PLL_PWR_DOWN)) {
        return SHR_E_NONE;
    }
     ...
     */
    if ((aging_support &&
        (pm_ha_info->serdes_core_ovco_power_state !=
                                PM_SERDES_CORE_PLL_PWR_DOWN)) ||
        !aging_support) {
                             ...
                             pm_speed_config_t *speed_config)
{
    ...
    /*
     * For Higig3 port or Reduced IPG IEEE, need to programm codec mode to
...
{
    int tvco_pll_idx = 0, ovco_pll_idx = 0;
    int rv = SHR_E_NONE, aging_support = 0;
    uint32_t tvco_pll_div = 0, ovco_pll_div = 0, is_pll_pwrdn = 1;
    pm_vco_t cur_tvco = PM_VCO_INVALID, cur_ovco = PM_VCO_INVALID;
    ...
    /* TVCO is already set in core_init. */
    SHR_IF_ERR_VERBOSE_EXIT
        (phymod_phy_pll_powerdown_get(phy_acc, tvco_pll_idx, &is_pll_pwrdn));
    if (is_pll_pwrdn) {
        LOG_INFO(BSL_LOG_MODULE,
     ...
     */
    /*
     * Note: Below check is in place to remind the fact that tvco_pll_idx
     * can be 0 or 1, even though for TD4 it's recommended to be 0 and hence hardcoded to be 0.
     */
    ovco_pll_idx = (tvco_pll_idx) ? 0 : 1;
    phy_acc->access.pll_idx = ovco_pll_idx;
    SHR_IF_ERR_VERBOSE_EXIT
    ...
    if (new_vco != PM_VCO_INVALID) {
        ...
        PM8X50_AGING_SUPPORT(unit, pm_info->type, aging_support);
        if (aging_support && (pm_acc->phy_acc.type != phymodDispatchTypeNull)) {
            if (pm_ha_info->serdes_core_ovco_power_state ==
                                               PM_SERDES_CORE_PLL_PWR_DOWN) {
                phy_acc->access.pll_idx = ovco_pll_idx;
                phy_acc->access.lane_mask = pm_acc->phy_acc.access.lane_mask;
                SHR_IF_ERR_VERBOSE_EXIT
                    (phymod_phy_serdes_power_set(phy_acc,
                                                 phymodSerdesOptionCore,
                                                 phymodSerdesPWR_ON));

                pm_ha_info->serdes_core_ovco_power_state =
                                                     PM_SERDES_CORE_PLL_PWR_ON;
            }
    ...
    if ((bcmdrd_feature_enabled(unit, BCMDRD_FT_ACTIVE_SIM)) &&
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pmacd->port_link_status_get(&pmac_acc, pm_acc->pmac_blkport, 1,
                                         port_index % PM8X50_NUM_LANES_PER_CORE,
                                         &link));
                   ...
                   pm_oper_status_t *op_st, pm_info_t *pm_info)
{
    ...
    /* Power off serdes core (ovco) if no port is using. */
    if (!pm_ha_info->ovco_active_lane_bmap &&
        !pm_ha_info->ovco_advert_lane_bmap) {
        /* Power off serdes core (ovco) if no port is using. */
        if (aging_support && (pm_acc->phy_acc.type != phymodDispatchTypeNull)) {
    ...
    if (!pm_ha_info->tvco_active_lane_bmap &&
        ...
        !pm_ha_info->ovco_advert_lane_bmap) {
        /*
         * Power off serdes core (pll0 and pll1) if no port is configured
         * on it.
         */
        if (aging_support && (pm_acc->phy_acc.type != phymodDispatchTypeNull)) {
            phy_acc->access.pll_idx = tvco_pll_idx;
            SHR_IF_ERR_VERBOSE_EXIT
                (phymod_phy_serdes_power_set(phy_acc, phymodSerdesOptionCore,
                                             phymodSerdesPWRDN_DEEP));
            pm_ha_info->serdes_core_tvco_power_state =
                                             PM_SERDES_CORE_PLL_PWR_DOWN;
        } else {
        ...
        pm_ha_info->is_100g_2lane_sw_war_mpp1_is_done = 0;
    }

    /* Power off lane */
    if (aging_support && (pm_acc->phy_acc.type != phymodDispatchTypeNull)) {
        SHR_IF_ERR_VERBOSE_EXIT
            (phymod_phy_serdes_power_set(phy_acc, phymodSerdesOptionLane,
                                         phymodSerdesPWRDN_DEEP));
        pm_ha_info->serdes_lane_power_down_bmap |=
                                          pm_acc->phy_acc.access.lane_mask;
    }
                               ...
                               pm_oper_status_t *op_st, pm_info_t *pm_info)
{
    ...
    switch (an_state) {
        case PM8X50_PORT_AN_CHECK_PAGE_COMPLETE:
        {
            ...
            if (wait_link_count > TIMEOUT_COUNT_400G_AN * 2) {
...
static int
pm8x50_phy_fec_uncorrectable_blk_counter_get(int unit,
                                             ...
                                             uint64_t *count)
{
    ...
    *count = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (phymod_phy_fec_uncorrectable_counter_get(phy_acc, (uint32_t *)count));
...
static int
pm8x50_phy_fec_correctable_blk_counter_get(int unit,
                                           const pm_access_t *pm_acc,
                                           pm_info_t *pm_info,
                                           uint64_t *count)
{
    ...
    *count = 0;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (phymod_phy_fec_correctable_counter_get(phy_acc, (uint32_t *)count));
...
static int
pm8x50_pm_uninit(int unit, const pm_access_t *pm_acc, pm_info_t *pm_info)
{
    /* Check the pmd_fw info_ptr */
    if (pm_info->pm_data.pmd_fw_info != NULL) {
        SHR_FREE(pm_info->pm_data.pmd_fw_info);
...
pm8x50_i_phymod_intr_type_get (bcmpc_intr_type_t pc_intr_type)
{
    ...
    switch (pc_intr_type) {
        ...
        case BCMPC_INTR_ECC_2B_ERR_BASE_R_FEC:
            intr_type = phymodIntrEccBaseRFEC;
            break;
        ...
        case BCMPC_INTR_ECC_2B_ERR_RSFEC_MPP0:
            intr_type = phymodIntrEccRsFECRs400gMpp0;
            break;
        case BCMPC_INTR_ECC_1B_ERR_RSFEC_MPP1:
        case BCMPC_INTR_ECC_2B_ERR_RSFEC_MPP1:
            intr_type = phymodIntrEccRsFECRs400gMpp1;
            break;
...
static int
pm8x50_port_intr_enable_get(int unit, const pm_access_t *pm_acc,
                             pm_oper_status_t *op_st, pm_info_t *pm_info,
                             ...
                             uint32_t *enable)
{
    ...
    intr_type = pm8x50_i_phymod_intr_type_get(pc_intr_type);
...
static int
pm8x50_port_intr_enable_set(int unit, const pm_access_t *pm_acc,
                             pm_oper_status_t *op_st, pm_info_t *pm_info,
                             bcmpc_intr_type_t pc_intr_type,
                             uint32_t enable)
{
...
static int
pm8x50_port_interrupt_process(int unit, const pm_access_t *pm_acc,
                               pm_oper_status_t *op_st, pm_info_t *pm_info,
                               bcmpc_intr_type_t pc_intr_type,
...
bcmpc_pm_drv_t bcmpc_pm_drv_pm8x50 = {
    ...
    .pm_phy_fec_uncorrectable_blk_counter_get = pm8x50_phy_fec_uncorrectable_blk_counter_get,
    .pm_phy_fec_correctable_blk_counter_get = pm8x50_phy_fec_correctable_blk_counter_get,
    .pm_uninit = pm8x50_pm_uninit,
    .port_intr_enable_get = pm8x50_port_intr_enable_get,
    .port_intr_enable_set = pm8x50_port_intr_enable_set,
    .port_interrupt_process = pm8x50_port_interrupt_process,
    .pm_diag_control_set = pm8x50_common_diag_control_set,
