                        ...
                        uint32_t enable)
{
    ...
    if (pm_drv->port_intr_enable_set) {
        SHR_IF_ERR_VERBOSE_EXIT
...
static int
pm_phy_fec_uncorrectable_blk_counter_get(int unit, const pm_access_t *pm_acc,
                                         pm_info_t *pm_info,
                                         uint64_t *count)
{
    bcmpc_pm_drv_t *pm_drv = NULL;
    ...
    if (pm_drv->pm_phy_fec_uncorrectable_blk_counter_get) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pm_drv->pm_phy_fec_uncorrectable_blk_counter_get(unit, pm_acc,
                                                              pm_info,
                                                              count));
    } else {
        SHR_EXIT();
...
static int
pm_phy_fec_correctable_blk_counter_get(int unit, const pm_access_t *pm_acc,
                                       pm_info_t *pm_info,
                                       uint64_t *count)
{
    bcmpc_pm_drv_t *pm_drv = NULL;
    ...
    if (pm_drv->pm_phy_fec_correctable_blk_counter_get) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pm_drv->pm_phy_fec_correctable_blk_counter_get(unit, pm_acc,
                                                            pm_info,
                                                            count));
    } else {
        SHR_EXIT();
...
static int pm_uninit(int unit, const pm_access_t *pm_acc, pm_info_t *pm_info)
{
    bcmpc_pm_drv_t *pm_drv = NULL;
    ...
    if (pm_drv->pm_uninit) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pm_drv->pm_uninit(unit, pm_acc, pm_info));
    }
...
static int
pm_common_mac_op_exec(int unit, const pm_access_t *pm_acc,
                      pm_oper_status_t *op_st, pm_info_t *pm_info,
                      const char *op, uint32_t op_param)
{
    ...
    SHR_NULL_CHECK(pmacd->op_exec, SHR_E_UNAVAIL);
    SHR_IF_ERR_VERBOSE_EXIT
        (pmacd->op_exec(&pmac_acc, pm_acc->pmac_blkport, op, op_param));
...
static int
pm_diag_control_set (int unit, const pm_access_t *pm_acc,
                     pm_info_t *pm_info,
                     pm_port_diag_control_t port_diag_ctrl)
{
    ...
    if (pm_drv->pm_diag_control_set) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pm_drv->pm_diag_control_set(unit, pm_acc, pm_info,
                                         port_diag_ctrl));
    }
...
bcmpc_pm_drv_t bcmpc_pm_drv_common = {
    ...
    .pm_phy_fec_uncorrectable_blk_counter_get = pm_phy_fec_uncorrectable_blk_counter_get,
    .pm_phy_fec_correctable_blk_counter_get = pm_phy_fec_correctable_blk_counter_get,
    .pm_uninit = pm_uninit,
    .pm_mac_op_exec = pm_common_mac_op_exec,
    .pm_diag_control_set = pm_diag_control_set,
};
