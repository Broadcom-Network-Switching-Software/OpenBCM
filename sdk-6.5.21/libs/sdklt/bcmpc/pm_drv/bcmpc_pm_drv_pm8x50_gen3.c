                     ...
                     pm_port_add_info_t *add_info)
{
    ...
    if (num_lanes != 8) {
        if (!bcmdrd_feature_enabled(unit, BCMDRD_FT_PHYMOD_SIM)) {
            ...
            if (phy_acc->access.lane_mask & 0xf0) {
    ...
    if (pm_ha_info->is_pm_initialized &&
        (pm_acc->phy_acc.type != phymodDispatchTypeNull)) {
    ...
    if ((bcmdrd_feature_enabled(unit, BCMDRD_FT_ACTIVE_SIM)) &&
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pmacd->port_link_status_get(&pmac_acc, pm_acc->pmac_blkport, 1,
                               port_index % PM8X50_GEN3_NUM_PMD_LANES_PER_CORE,
                               &link));
                        ...
                        pm_oper_status_t *op_st, pm_info_t *pm_info)
{
    ...
    if (!pm_ha_info->tvco_active_lane_bmap) {
       if (pm_acc->phy_acc.type != phymodDispatchTypeNull) {
            SHR_IF_ERR_VERBOSE_EXIT
...
static int
pm8x50_gen3_pm_uninit(int unit, const pm_access_t *pm_acc, pm_info_t *pm_info)
{
    /* Check the pmd_fw info_ptr */
    if (pm_info->pm_data.pmd_fw_info != NULL) {
...
bcmpc_pm_drv_t bcmpc_pm_drv_pm8x50_gen3 = {
    ...
    .pm_uninit = pm8x50_gen3_pm_uninit,
