...
#define PHY_NUM_RX_DFE_TAPS_MAX 18
...
typedef struct pmgr_tap_mode_map_s {
    ...
    phymod_phy_signalling_method_t sig_mode;
...
static const pmgr_encap_map_t pmgr_encap_map[] = {
    ...
    { BCMPC_PORT_ENCAP_IEEE_REDUCED_IPG, BCMPMAC_ENCAP_IEEE_REDUCED_IPG }
};
...
static const pmgr_tap_mode_map_t pmgr_tap_mode_map[] = {
    /* bcmpc_phy_tap_mode_t, phymod_phy_signalling_method_t, phymod_tx_tap_mode_t */
    { BCMPC_PHY_TXFIR_TAP_DEFAULT, phymodSignallingMethodNRZ, phymodTxTapModeDefault },
    { BCMPC_PHY_TXFIR_NRZ_TAPS_6, phymodSignallingMethodNRZ, phymodTxTapMode6Tap },
    { BCMPC_PHY_TXFIR_PAM4_TAPS_6, phymodSignallingMethodPAM4, phymodTxTapMode6Tap },
    { BCMPC_PHY_TXFIR_NRZ_LP_TAPS_3, phymodSignallingMethodNRZ, phymodTxTapMode3Tap },
    { BCMPC_PHY_TXFIR_PAM4_LP_TAPS_3, phymodSignallingMethodPAM4, phymodTxTapMode3Tap },
    { BCMPC_PHY_TXFIR_PAM4_TAPS_6, phymodSignallingMethod50gPAM4, phymodTxTapMode6Tap },
    { BCMPC_PHY_TXFIR_PAM4_LP_TAPS_3, phymodSignallingMethod50gPAM4, phymodTxTapMode3Tap },
};
...
static const pmgr_sig_mode_map_t pmgr_sig_mode_map[] = {
    ...
    { BCMPC_PHY_SIG_MODE_50G_PAM4, phymodSignallingMethod50gPAM4 },
};
 ...
 * \param [in] sig_mode PHYMOD signalling mode, phymod_phy_signalling_method_t.
                       ...
                       bcmpc_phy_txfir_tap_mode_t *pmgr_mode)
{
    ...
    for (idx = 0; idx < COUNTOF(pmgr_tap_mode_map); idx++) {
        if ((phy_mode == pmgr_tap_mode_map[idx].phy_mode) &&
            (sig_mode == pmgr_tap_mode_map[idx].sig_mode)) {
                              ...
                              phymod_tx_t *phymod_tx)
{
    ...
    phymod_tx->amp = (tx_cfg->amp_sign) ? tx_cfg->amp * (-1) : tx_cfg->amp;
    phymod_tx->main = (tx_cfg->main_sign) ? tx_cfg->main * (-1) : tx_cfg->main;
    phymod_tx->pre = (tx_cfg->pre_sign) ? tx_cfg->pre * (-1) : tx_cfg->pre;
    phymod_tx->pre2 = (tx_cfg->pre2_sign) ? tx_cfg->pre2 * (-1) : tx_cfg->pre2;
    phymod_tx->pre3 = (tx_cfg->pre3_sign) ? tx_cfg->pre3 * (-1) : tx_cfg->pre3;
    phymod_tx->post = (tx_cfg->post_sign) ? tx_cfg->post * (-1) : tx_cfg->post;
    ...
    phymod_tx->rpara = (tx_cfg->rpara_sign) ? tx_cfg->rpara * (-1)
                                            : tx_cfg->rpara;
    phymod_tx->drivermode = (tx_cfg->drv_mode_sign) ? tx_cfg->drv_mode * (-1)
                                                    : tx_cfg->drv_mode;
                                   ...
                                   bcmpc_tx_taps_config_t *st_tx)
{
    ...
    if (phymod_tx->pre3 < 0) {
        st_tx->pre3 = phymod_tx->pre3 * (-1);
        st_tx->pre3_sign = TRUE;
    } else {
        st_tx->pre3 = phymod_tx->pre3;
        st_tx->pre3_sign = FALSE;
    }
    if (phymod_tx->post < 0) {
    ...
    rv = pmgr_tap_mode_from_phy(phymod_tx->tap_mode, phymod_tx->sig_method,
                                &st_tx->txfir_tap_mode);
    if (SHR_FAILURE(rv)) {
                                         ...
                                         bcmpc_pmgr_phy_status_t *pmgr_phy_st)
{
    ...
    pmgr_phy_st->phy_autoneg_fec_override = pm_phy_st->phy_autoneg_fec_override;
}
                                   ...
                                   pm_phy_control_t *pm_pctrl)
{
    ...
    pm_pctrl->phy_autoneg_fec_override = pctrl->phy_autoneg_fec_override;
