                                  ...
                                  bcmpc_tx_taps_config_t  *tx_taps_config)
{
    ...
    for (idx = 0; idx < num_lanes; idx++) {
        ...
        /*
         * If TX FIR pre3 cursor auto is set,
         *     use the default value from the driver.
         * else
         *     use the value from the DB lookup.
         */
        if (tx_taps_lookup[idx].pre3_auto) {
            tx_taps_config[idx].pre3 = default_tx_taps.pre3;
            tx_taps_config[idx].pre3_sign = default_tx_taps.pre3_sign;
        } else {
            tx_taps_config[idx].pre3 = tx_taps_lookup[idx].pre3;
            tx_taps_config[idx].pre3_sign = tx_taps_lookup[idx].pre3_sign;
        }
...
port_tx_taps_restore(int unit, bcmpc_pport_t pport, bcmpc_port_cfg_t *pcfg)
{
    ...
    /* get lane_mask flag if applicable */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpc_drv_get(unit, &pc_drv));
    if (pc_drv->pm_drv_lane_mask_flag) {
        SHR_IF_ERR_VERBOSE_EXIT
            (pc_drv->pm_drv_lane_mask_flag(unit,
            pm_acc.phy_acc.access.lane_mask, &lane_mask_flag));
    } else {
        lane_mask_flag = 0;
    }
    for (idx = 0; idx < num_lanes; idx++) {
        ...
        /* set tx taps */
        pm_acc.phy_acc.access.lane_mask = 1 << (start_lane + idx) | lane_mask_flag;
        PM_LOCK(pm_drv_info);
 ...
 */
static int
port_encap_get(int unit, bcmpc_pport_t pport, bcmpc_lport_t lport,
               bcmpc_port_encap_t *encap_mode)
{
    ...
    /*
     * Both BCMPC_PORT_ENCAP_HG3 and BCMPC_PORT_ENCAP_IEEE_REDUCED_IPG
     * set codec_mode as ReducedIPG and CDMAC_MODE.HDR_MODE as reduced
     * preamble mode.
     * When returning encap_mode, need to check PC_PORTt.encap_mode to
     * make sure PC_PORT_STATUSt.encap is synced with PC_PORTt.encap_mode.
     */
     if (*encap_mode == BCMPC_PORT_ENCAP_HG3) {
         rv = bcmpc_db_imm_entry_lookup(unit, PC_PORTt, P(&lport), P(&pcfg));
         if (rv != SHR_E_NOT_FOUND) {
             if (pcfg.encap_mode == BCMPC_PORT_ENCAP_IEEE_REDUCED_IPG) {
                 *encap_mode = BCMPC_PORT_ENCAP_IEEE_REDUCED_IPG;
             }
         }
     }
                     ...
                     bcmpc_port_cfg_t *pcfg, bool is_new)
{
        ...
        if (force_cfg) {
            ...
            if (mctrl.ifg_auto) {
                ...
                if (pcfg->encap_mode == BCMPC_PORT_ENCAP_HG3) {
                    ...
                    ifg = 12;
                } else if (pcfg->encap_mode == BCMPC_PORT_ENCAP_IEEE_REDUCED_IPG) {
                    ifg = 8;
                }
            } else {
    ...
    if (PMGR_CFG_CHANGED(0, pcfg, &old_pcfg, speed) ||
        ...
        if ((!pcfg->link_training) && (!is_new)) {
            cfg_sigmode = (pcfg->speed/pcfg->num_lanes) > 50000 ? \
                BCMPC_PHY_SIG_MODE_PAM4 : (pcfg->speed/pcfg->num_lanes) > 25000 ? \
                BCMPC_PHY_SIG_MODE_50G_PAM4 : BCMPC_PHY_SIG_MODE_NRZ;
            old_cfg_sigmode = (old_pcfg.speed/old_pcfg.num_lanes) > 50000 ? \
                BCMPC_PHY_SIG_MODE_PAM4 : (old_pcfg.speed/old_pcfg.num_lanes) > 25000 ? \
                BCMPC_PHY_SIG_MODE_50G_PAM4 : BCMPC_PHY_SIG_MODE_NRZ;
                    ...
                    bcmpc_pmgr_port_status_t *pst)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (port_encap_get(unit, pport, lport, &pst->encap_mode));
...
default_pmac_op_exec(int unit, bcmpc_pport_t pport, bcmpc_operation_t *op)
{
    ...
    rv = pm_drv->pm_mac_op_exec(unit, &pm_acc, &op_st, pm_drv_info,
                              op->name, op->param);
                         ...
                         pm_info_t *pm_drv_info)
{
    ...
    for (intr_type = 0; intr_type < BCMPC_INTR_COUNT; intr_type++) {
        switch (intr_type) {
            /*! One bit ECC errors */
            case BCMPC_INTR_ECC_1B_ERR_BASE_R_FEC:
            ...
            case BCMPC_INTR_ECC_1B_ERR_TX_1588_MPP1:
            case BCMPC_INTR_ECC_1B_ERR_TX_1588_400G:
            case BCMPC_INTR_ECC_1B_ERR_RX_1588_MPP0:
            case BCMPC_INTR_ECC_1B_ERR_RX_1588_MPP1:
            case BCMPC_INTR_ECC_1B_ERR_RX_1588_400G:

                /*! Two bit ECC errors */
            case BCMPC_INTR_ECC_2B_ERR_BASE_R_FEC:
            ...
            case BCMPC_INTR_ECC_2B_ERR_TX_1588_MPP1:
            case BCMPC_INTR_ECC_2B_ERR_TX_1588_400G:
            case BCMPC_INTR_ECC_2B_ERR_RX_1588_MPP0:
            case BCMPC_INTR_ECC_2B_ERR_RX_1588_MPP1:
            case BCMPC_INTR_ECC_2B_ERR_RX_1588_400G:
    ...
    if ((SHR_BITGET(mctrl->fbmp, BCMPC_MAC_CTRL_LT_FLD_INTER_FRAME_GAP)) ||
        ...
        if (mctrl->ifg_auto != old_mctrl.ifg_auto) {
            if (mctrl->ifg_auto == TRUE) {
                ...
                SHR_IF_ERR_VERBOSE_EXIT
                    (port_encap_get(unit, pport, lport, &encap));
                if (encap == BCMPC_PORT_ENCAP_IEEE) {
                    ifg = 0xc;
                } else if (encap == BCMPC_PORT_ENCAP_HG3) {
                    ifg = 0x8;
                } else if (encap == BCMPC_PORT_ENCAP_IEEE_REDUCED_IPG) {
                    ifg = 0x8;
                }
            } else {
                ifg = mctrl->ifg;
                    ...
                    bcmpc_phy_control_t *pctrl)
{
    ...
    int rv = SHR_E_NONE;
    pm_oper_status_t op_st;
    uint32_t phy_lane_mask = 0, i = 0, num_of_dfe_taps = 0;
    bool autoneg_fec_override_set = FALSE;
    bcmpc_drv_t *pc_drv;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpc_lt_entry_get(unit, PC_PHY_CONTROLt, P(port_lane),
                            P(&old_pctrl)));
    /* get current phy_status */
    SHR_IF_ERR_VERBOSE_EXIT
    ...
    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_RX_AFE_VGA_AUTO)) {
        ...
        phy_ctrl.rx_afe_vga_auto = old_pctrl.rx_afe_vga_auto;
        phy_ctrl.rx_afe_vga = phy_stat.rx_afe.vga.value;
    }
    ...
    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_RX_AFE_PEAKING_FILTER_AUTO)) {
        ...
        phy_ctrl.rx_afe_pk_flt_auto = old_pctrl.rx_afe_pk_flt_auto;
        phy_ctrl.rx_afe_pk_flt = phy_stat.rx_afe.peaking_filter.value;
    }
    ...
    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_RX_AFE_LO_FREQ_PEAKING_FILTER_AUTO)) {
        ...
        phy_ctrl.rx_afe_low_fre_pk_flt_auto =
                                         old_pctrl.rx_afe_low_fre_pk_flt_auto;
        phy_ctrl.rx_afe_low_fre_pk_flt =
            phy_stat.rx_afe.low_freq_peaking_filter.value;
    }
    ...
    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_RX_AFE_HI_FREQ_PEAKING_FILTER_AUTO)) {
        ...
        phy_ctrl.rx_afe_hi_fre_pk_flt =
            phy_stat.rx_afe.high_freq_peaking_filter.value;
    }
    ...
    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_RX_AFE_DFE_TAP_AUTO)) {
        ...
        phy_ctrl.rx_afe_dfe_tap_auto = old_pctrl.rx_afe_dfe_tap_auto;
        for (i = 0; i < phy_stat.rx_afe.num_of_dfe_taps; i++) {
            if (phy_stat.rx_afe.dfe[i].value >= 0) {
                phy_ctrl.rx_afe_dfe_tap[i] = phy_stat.rx_afe.dfe[i].value;
                phy_ctrl.rx_afe_dfe_tap_sign[i] = 0;
            } else {
                phy_ctrl.rx_afe_dfe_tap[i] = phy_stat.rx_afe.dfe[i].value * -1;
                phy_ctrl.rx_afe_dfe_tap_sign[i] = 1;
            }
        }
    }
    ...
    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_AUTONEG_FEC_OVERRIDE_AUTO)) {
        if (pctrl->phy_autoneg_fec_override_auto &&
            !old_pctrl.phy_autoneg_fec_override_auto) {
            /* Set default autoneg_fec_override which is 0 for all chips */
            autoneg_fec_override_set = TRUE;
            pctrl->phy_autoneg_fec_override = 0;
        } else if (!SHR_BITGET(pctrl->fbmp, BCMPC_FLD_AUTONEG_FEC_OVERRIDE) &&
                   !pctrl->phy_autoneg_fec_override_auto) {
            /* Set old autoneg_fec_override */
            autoneg_fec_override_set = TRUE;
            pctrl->phy_autoneg_fec_override = old_pctrl.phy_autoneg_fec_override;
        }
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_AUTONEG_FEC_OVERRIDE)) {
        if ((SHR_BITGET(pctrl->fbmp, BCMPC_FLD_AUTONEG_FEC_OVERRIDE_AUTO) &&
             !pctrl->phy_autoneg_fec_override_auto) ||
            (!SHR_BITGET(pctrl->fbmp, BCMPC_FLD_AUTONEG_FEC_OVERRIDE_AUTO) &&
             !old_pctrl.phy_autoneg_fec_override_auto)) {
            /* Set new autoneg_fec_override */
            autoneg_fec_override_set = TRUE;

        }
    }

    if (autoneg_fec_override_set) {
        phy_ctrl.phy_autoneg_fec_override = pctrl->phy_autoneg_fec_override;
        phy_ctrl_flags |= PM_PHY_CTRL_F_AUTONEG_FEC_OVERRIDE;
    }
    ...
    /* check which DFE TAP is to be set */
    if ((phy_ctrl_flags & PHY_CTRL_F_RX_AFE_DFE_TAP) ||
        (phy_ctrl_flags & PHY_CTRL_F_RX_AFE_VGA) ||
        (phy_ctrl_flags & PHY_CTRL_F_RX_AFE_PK_FLT) ||
        (phy_ctrl_flags & PHY_CTRL_F_RX_AFE_LOW_FRE_PK_PLT) ||
        (phy_ctrl_flags & PHY_CTRL_F_RX_AFE_HI_FRE_PK_PLT)) {
        /* get the number of supported dfe taps */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpc_drv_get(unit, &pc_drv));
        if (pc_drv->num_of_rx_dfe_tap_get) {
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_drv->num_of_rx_dfe_tap_get(unit, drv_data.pm_id, &num_of_dfe_taps));
            pm_pctrl.rx_afe.num_of_dfe_taps = num_of_dfe_taps;
            /* set enable to 1 if input tap value is different from the value in HW */
            for (i = 0; i < num_of_dfe_taps; i++) {
                if (phy_stat.rx_afe.dfe[i].value != pm_pctrl.rx_afe.dfe[i].value) {
                    pm_pctrl.rx_afe.dfe[i].enable = 1;
                } else {
                    pm_pctrl.rx_afe.dfe[i].enable = 0;
                }
            }
        } else {
                   ...
                   bcmpc_pmgr_phy_status_t *pst)
{
    ...
    sal_memset(&op_st, 0, sizeof(pm_oper_status_t));
    sal_memset(&phy_stat, 0, sizeof(phy_stat));
                       ...
                       bcmpc_tx_taps_config_t *taps_cfg)
{
    ...
    if (PMGR_CFG_CHANGED(force_cfg, taps_cfg, &old_taps_cfg, pre3_auto) ||
        PMGR_CFG_CHANGED(force_cfg, taps_cfg, &old_taps_cfg, pre3) ||
        PMGR_CFG_CHANGED(force_cfg, taps_cfg, &old_taps_cfg, pre3_sign)) {
        tx_taps_cfg_flags++;
    }
    ...
    if (!tx_taps_cfg_flags) {
        ...
        if (taps_cfg->pre3_auto) {
            taps_cfg->pre3 = default_tx_taps.pre3;
            taps_cfg->pre3_sign = default_tx_taps.pre3_sign;
        }
    ...
    pm_acc.phy_acc.access.lane_mask = 1 << lane_idx | lane_mask_flag;
                            ...
                            bcmpc_pmd_firmware_config_t *fm_cfg)
{
    ...
    for (i = 0; i < pcfg.num_lanes; i++) {
        ...
        pm_acc.phy_acc.access.lane_mask = 1 << (start_lane + i) | lane_mask_flag;
        phymod_firmware_lane_config_t_init(&local);
        SHR_IF_ERR_VERBOSE_EXIT
...
static int
pmm_tsc_power_on_set(int unit, bcmpc_pport_t pport, int enable)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (tsc_power_control(unit, pport, enable));
                ...
                bcmpc_port_cfg_t *pcfg, bcmpc_pm_core_cfg_t *ccfg)
{
    ...
    /*
     * Read SERDES_CONFIG to check if link UP/DOWN should be
     * simulated in sim environments.
     */
    if (bcmdrd_feature_enabled(unit, BCMDRD_FT_ACTIVE_SIM)) {
        SHR_IF_ERR_EXIT
            (bcmcfg_field_get(unit, PC_SERDES_CONFIGt,
                              PC_SERDES_CONFIGt_SIMULATE_LINK_EVENTSf,
                              &fval));
    }
    add_info.link_simulation = fval? 1: 0;
    ...
    if (bcmdrd_feature_enabled(unit, BCMDRD_FT_PHYMOD_SIM)) {
        ...
        if (ccfg->pm_fw_cfg.fw_load == PM_SERDES_FW_LOAD_SLOW) {
    ...
    for (init_pass = 0; init_pass < PM_CORE_INIT_PASS; init_pass++) {
         if (init_pass == 0 ){
            ...
            if (pc_drv->is_dual_pml_supported) {
                ...
                if (is_dual_pml_supported) {
                    ...
                    /* next need to check if lane swap flip eixsts */
                    if (pc_drv->does_internal_lane_swap_exist) {
                        SHR_IF_ERR_EXIT
                            (pc_drv->does_internal_lane_swap_exist(unit, original_pport,
                                                                   &lane_swap_flip));
                        if (lane_swap_flip) {
                            PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP_SET(&add_info);
                        }
                    }
       ...
       if (pc_drv->dev_revision_a0_get) {
        ...
        if (pc_drv->does_internal_lane_swap_exist) {
            SHR_IF_ERR_EXIT
                (pc_drv->does_internal_lane_swap_exist(unit, pm_acc.pport,
                                                       &lane_swap_flip));
            if (lane_swap_flip) {
                PM_PORT_ADD_F_INIT_LANE_SWAP_FLIP_SET(&add_info);
            }
                       ...
                       bcmpc_pmgr_port_diag_stats_t *diag_stats)
{
    ...
    uint64_t fec_uncorrected_blocks, fec_corrected_blocks;
    bool release_lock = FALSE;
    bcmpc_pmgr_drv_data_t drv_data;
    pm_info_t *pm_drv_info = NULL;
    bcmpc_pm_info_t pm_info;
    ...
    if (pm_drv->pm_phy_fec_uncorrectable_blk_counter_get) {
        SHR_IF_ERR_VERBOSE_EXIT(
                pm_drv->pm_phy_fec_uncorrectable_blk_counter_get(unit,
                    &pm_acc, pm_drv_info, &fec_uncorrected_blocks));
        SHR_IF_ERR_VERBOSE_EXIT(
                pm_drv->pm_phy_fec_correctable_blk_counter_get(unit,
                    &pm_acc, pm_drv_info, &fec_corrected_blocks));
    }
    ...
    if (pm_drv->pm_phy_fec_uncorrectable_blk_counter_get) {
        diag_stats->fec_uncorrected_blocks = fec_uncorrected_blocks;
        diag_stats->fec_corrected_blocks = fec_corrected_blocks;
    }
...
static int
pmm_port_interrupt_process(int unit, bcmpc_pport_t pport,
                           bcmpc_intr_type_t intr_type,
                           uint32_t *is_handled)
{
    bcmpc_pmgr_drv_data_t drv_data;
...
static int
pmm_phy_prbs_control_set (int unit, bcmpc_port_lane_t *port_lane, bcmpc_pport_t pport,
                          bcmpc_phy_prbs_control_t *pctrl)
{
    bcmpc_phy_prbs_control_t old_pctrl;
    bcmpc_phy_prbs_control_t phy_prbs_ctrl;
    bcmpc_phy_prbs_cfg_t pc_cfg;
    bool generator_enable_set = FALSE, checker_enable_set = FALSE;
    bool poly_mode_set = FALSE, invert_data_tx_set = FALSE, invert_data_rx_set = FALSE;
    int rv;

    SHR_FUNC_ENTER(unit);

    sal_memset(&old_pctrl, 0, sizeof(old_pctrl));
    sal_memset(&phy_prbs_ctrl, 0, sizeof(phy_prbs_ctrl));

    rv = bcmpc_lt_entry_get(unit, PC_PHY_PRBS_CONTROLt, P(port_lane),
                            P(&old_pctrl));

    if (rv == SHR_E_NONE) {
        /*
         * Copy the old IMM DB values to the local phy prbs control data structure
         * This structure is updated by the below logic and some of the
         * information is present in the IMM DB.
         */
        sal_memcpy(&phy_prbs_ctrl, &old_pctrl, sizeof(phy_prbs_ctrl));
    } else if (rv == SHR_E_NOT_FOUND) {
        /* Default value of poly_mode for all chips is BCMPC_PHY_PRBS_POLY_31 */
        phy_prbs_ctrl.poly_mode = BCMPC_PHY_PRBS_POLY_31;
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_GENERATOR_ENABLE)) {
        /* Set new generator_enable*/
        generator_enable_set = TRUE;
        phy_prbs_ctrl.generator_enable = pctrl->generator_enable;
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_CHECKER_ENABLE)) {
        /* Set new checker_enable*/
        checker_enable_set = TRUE;
        phy_prbs_ctrl.checker_enable = pctrl->checker_enable;
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_POLY_MODE)) {
        /* Set new poly_mode*/
        poly_mode_set = TRUE;
        phy_prbs_ctrl.poly_mode = pctrl->poly_mode;
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_INVERT_DATA_TX)) {
        /* Set new invert_data_tx*/
        invert_data_tx_set = TRUE;
        phy_prbs_ctrl.invert_data_tx = pctrl->invert_data_tx;
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_INVERT_DATA_RX)) {
        /* Set new invert_data_rx*/
        invert_data_rx_set = TRUE;
        phy_prbs_ctrl.invert_data_rx = pctrl->invert_data_rx;
    }

    if (invert_data_rx_set || poly_mode_set) {
        sal_memset(&pc_cfg, 0, sizeof(pc_cfg));
        pc_cfg.invert = phy_prbs_ctrl.invert_data_rx;
        pc_cfg.poly = phy_prbs_ctrl.poly_mode;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpc_phy_prbs_config_set(unit, pport, port_lane->lane_index,
                                       (1<<BCMPC_PHY_PRBS_CFG_F_RX), &pc_cfg));
    }

    if (checker_enable_set) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpc_phy_prbs_enable_set(unit, pport, port_lane->lane_index,
                                       (1<<BCMPC_PHY_PRBS_CFG_F_RX), phy_prbs_ctrl.checker_enable));
    }

    if (invert_data_tx_set || poly_mode_set) {
        sal_memset(&pc_cfg, 0, sizeof(pc_cfg));
        pc_cfg.invert = phy_prbs_ctrl.invert_data_tx;
        pc_cfg.poly = phy_prbs_ctrl.poly_mode;
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpc_phy_prbs_config_set(unit, pport, port_lane->lane_index,
                                       (1<<BCMPC_PHY_PRBS_CFG_F_TX), &pc_cfg));
    }

    if (generator_enable_set) {
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpc_phy_prbs_enable_set(unit, pport, port_lane->lane_index,
                                       (1<<BCMPC_PHY_PRBS_CFG_F_TX), phy_prbs_ctrl.generator_enable));
    }
...
static int
pmm_phy_prbs_control_delete (int unit, bcmpc_port_lane_t *port_lane, bcmpc_pport_t pport)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpc_phy_prbs_enable_set(unit, pport, port_lane->lane_index,
                                   BCMPC_PHY_PRBS_CFG_F_TX, 0));
...
static int
pmm_phy_prbs_status_get (int unit, bcmpc_port_lane_t *port_lane, bcmpc_pport_t pport,
                         bcmpc_phy_prbs_status_t *pst)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpc_phy_prbs_status_info_get(unit, pport, port_lane->lane_index,
                                        0, pst));
exit:
    SHR_FUNC_EXIT();
}
...
static int
pmm_pm_tsc_power_cycle(int unit, bcmpc_pport_t pport)
{
    bcmpc_pmgr_drv_data_t drv_data;
    pm_info_t *pm_drv_info = NULL;
    bcmpc_pm_info_t pm_info;
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (pm_port_drv_data_get(unit, pport, &pm_acc, &drv_data, &pm_info,
                              &pm_drv_info, &pm_drv));
    SHR_IF_ERR_EXIT
        (pmm_tsc_power_on_set(unit, pport, 0));

    PM_LOCK(pm_drv_info);
    /* Free pmd_fw info_ptr.*/
    rv = pm_drv->pm_uninit(unit, &pm_acc, pm_drv_info);
    ...
    SHR_IF_ERR_EXIT
        (pmm_tsc_power_on_set(unit, pport, 1));
...
static int
pmm_port_diag_control_set (int unit, bcmpc_lport_t *lport,
                           bcmpc_pport_t pport,
                           bcmpc_port_diag_control_t *pctrl)
{
    bcmpc_port_diag_control_t old_pctrl;
    pm_port_diag_control_t port_diag_ctrl;
    int rv;
    bcmpc_pmgr_drv_data_t drv_data;
    pm_info_t *pm_drv_info = NULL;
    bcmpc_pm_info_t pm_info;
    ...
    sal_memset(&old_pctrl, 0, sizeof(old_pctrl));
    sal_memset(&port_diag_ctrl, 0, sizeof(port_diag_ctrl));

    SHR_IF_ERR_VERBOSE_EXIT
        (pm_port_drv_data_get(unit, pport, &pm_acc, &drv_data, &pm_info,
                              &pm_drv_info, &pm_drv));

    rv = bcmpc_lt_entry_get(unit, PC_PORT_DIAG_CONTROLt, P(lport),
                            P(&old_pctrl));

    if (rv == SHR_E_NONE) {
        /*
         * Copy the old IMM DB values to the pm diag control data structure
         * This structure is updated by the below logic and some of the
         * information is present in the IMM DB.
         */
        port_diag_ctrl.error_mask_63_0   = old_pctrl.error_mask_63_0;
        port_diag_ctrl.error_mask_127_64 = old_pctrl.error_mask_127_64;
        port_diag_ctrl.error_control_map = old_pctrl.error_control_map;
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_ERROR_MASK_63_0)) {
        port_diag_ctrl.error_mask_63_0 = pctrl->error_mask_63_0;
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_ERROR_MASK_127_64)) {
        port_diag_ctrl.error_mask_127_64 = pctrl->error_mask_127_64;
    }

    if (SHR_BITGET(pctrl->fbmp, BCMPC_FLD_ERROR_CONTROL_MAP)) {
        port_diag_ctrl.error_control_map = pctrl->error_control_map;
    }

    PM_LOCK(pm_drv_info);
    rv = pm_drv->pm_diag_control_set(unit, &pm_acc,
                                     pm_drv_info, port_diag_ctrl);
    PM_UNLOCK(pm_drv_info);
    SHR_IF_ERR_VERBOSE_EXIT(rv);
...
static int
pmm_port_diag_control_delete (int unit, bcmpc_pport_t pport)
{
    pm_port_diag_control_t port_diag_ctrl;
    bcmpc_pmgr_drv_data_t drv_data;
    pm_info_t *pm_drv_info = NULL;
    ...
    PM_LOCK(pm_drv_info);
    SHR_IF_ERR_VERBOSE_EXIT
        (pm_drv->pm_diag_control_set(unit, &pm_acc,
                                     pm_drv_info, port_diag_ctrl));
    PM_UNLOCK(pm_drv_info);
...
bcmpc_pmgr_drv_t bcmpc_pmgr_drv_default = {
    ...
    .pm_tsc_power_on_set = pmm_tsc_power_on_set,
    ...
    .pm_phy_prbs_control_set = pmm_phy_prbs_control_set,
    .pm_phy_prbs_control_delete = pmm_phy_prbs_control_delete,
    .pm_phy_prbs_status_get = pmm_phy_prbs_status_get,
    .pm_tsc_power_cycle = pmm_pm_tsc_power_cycle,
    .pm_port_diag_control_set = pmm_port_diag_control_set,
    .pm_port_diag_control_delete = pmm_port_diag_control_delete,
};
