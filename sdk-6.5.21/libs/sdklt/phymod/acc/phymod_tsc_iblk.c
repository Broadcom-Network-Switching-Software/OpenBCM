...
/* each PML  has 4 lanes, lower 2 lanes belong to MPP0 and upper 2 lanes belong to MPP1 */
#define LANE_IS_MPP1(_ln) (_ln > 1 ? 1: 0)
...
int
phymod_1_iblk_read(const phymod_access_t *pa, uint32_t addr, uint32_t *data)
{
    int ioerr = 0;
    uint32_t devad = (addr >> 16) & 0xf;
    uint32_t blkaddr, regaddr;
 ...
 * any PMD register access should be treated as error */
int
phymod_2_iblk_read(const phymod_access_t *pa, uint32_t addr, uint32_t *data)
{
    int ioerr = 0;
    uint32_t devad = (addr >> 16) & 0xf;
    uint32_t blkaddr, regaddr;
 ...
 * any PMD register access should be treated as error */
int
phymod_4_iblk_read(const phymod_access_t *pa, uint32_t addr, uint32_t *data)
{
    int ioerr = 0;
    uint32_t devad = (addr >> 16) & 0xf;
    uint32_t blkaddr, regaddr;
    ...
    if (addr & PHYMOD_REG_ACC_AER_IBLK_FORCE_LANE) {
        ...
        lane_map = PHYMOD_ACC_LANE_MASK(pa);
        is_8_lane_port = (lane_map >> PHYMOD_8_LANE_PORT_SHIFT) & 0x1;
        if (lane_map & 0x1) {
            lane = 0;
        } else if (lane_map & 0x2) {
            lane = 1;
 ...
 * any PMD register access should be treated as error */
int
phymod_1_iblk_write(const phymod_access_t *pa, uint32_t addr, uint32_t data)
{
    uint32_t devad = (addr >> 16) & 0xf;
    uint32_t lane_map, hwlane = 0, is_hwsupport = 1;
 ...
 * any PMD register access should be treated as error */
int
phymod_2_iblk_write(const phymod_access_t *pa, uint32_t addr, uint32_t data)
{
    uint32_t devad = (addr >> 16) & 0xf;
    uint32_t lane_map, hwlane = 0, is_hwsupport = 1;
 ...
 * any PMD register access should be treated as error */
int
phymod_4_iblk_write(const phymod_access_t *pa, uint32_t addr, uint32_t data)
{
    uint32_t devad = (addr >> 16) & 0xf;
    uint32_t lane_map, hwlane = 0, is_hwsupport = 1;
