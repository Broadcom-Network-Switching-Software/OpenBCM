...
err_code_t _blackhawk_tsc_pmd_rdt_field(srds_access_t *sa__, uint16_t addr, uint8_t shift_left, uint8_t shift_right, uint16_t *val_p) {
  ...
  EFUN(blackhawk_tsc_pmd_rdt_reg(sa__, addr,val_p));
...
err_code_t _blackhawk_tsc_pmd_rdt_field_signed(srds_access_t *sa__, uint16_t addr, uint8_t shift_left, uint8_t shift_right, int16_t *val_p) {
  ...
  EFUN(blackhawk_tsc_pmd_rdt_reg(sa__, addr,(uint16_t *)val_p));
...
err_code_t _blackhawk_tsc_pmd_mwr_reg_byte(srds_access_t *sa__, uint16_t addr, uint16_t mask, uint8_t lsb, uint8_t val) {
  ...
  EFUN(blackhawk_tsc_pmd_mwr_reg(sa__, addr, mask, lsb, (uint16_t) val));

  return(ERR_CODE_NONE);
}

  err_code_t _blackhawk_tsc_pmd_rdt_field_byte(srds_access_t *sa__, uint16_t addr, uint8_t shift_left, uint8_t shift_right, uint8_t *val8_p) {

  uint16_t val = 0;

  EFUN(blackhawk_tsc_pmd_rdt_reg(sa__, addr,&val));
  ...
  *val8_p = (uint8_t) val;
  ...
  err_code_t _blackhawk_tsc_pmd_rdt_field_signed_byte(srds_access_t *sa__, uint16_t addr, uint8_t shift_left, uint8_t shift_right, int8_t *val8_p) {

  int16_t val = 0;

  EFUN(blackhawk_tsc_pmd_rdt_reg(sa__, addr,(uint16_t *) &val));
...
uint16_t _blackhawk_tsc_pmd_rde_reg(srds_access_t *sa__, uint16_t addr, err_code_t *err_code_p)
{
  ...
  EPFUN(blackhawk_tsc_pmd_rdt_reg(sa__, addr, &data));
...
uint16_t _blackhawk_tsc_pmd_rde_field(srds_access_t *sa__, uint16_t addr, uint8_t shift_left, uint8_t shift_right, err_code_t *err_code_p)
{
  ...
  EPSTM(data = _blackhawk_tsc_pmd_rde_reg(sa__, addr, err_code_p));
...
int16_t _blackhawk_tsc_pmd_rde_field_signed(srds_access_t *sa__, uint16_t addr, uint8_t shift_left, uint8_t shift_right, err_code_t *err_code_p)
{
  ...
  EPSTM(data = (int16_t) _blackhawk_tsc_pmd_rde_reg(sa__, addr, err_code_p));  /* convert it to signed */
