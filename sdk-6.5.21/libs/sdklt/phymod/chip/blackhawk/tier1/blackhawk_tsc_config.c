...
err_code_t blackhawk_tsc_ucode_pram_load(srds_access_t *sa__, char const * ucode_image, uint32_t ucode_len) {
    ...
    while (ucode_len > 0) {
        EFUN(blackhawk_tsc_pmd_wr_pram(sa__, *ucode_image));
        --ucode_len;
        ++ucode_image;
    }
...
err_code_t blackhawk_tsc_write_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t val) {
  /* Assumes the micro is not actively tuning */
    ...
    switch(param) {
    case RX_AFE_PF:
        return(blackhawk_tsc_INTERNAL_set_rx_pf_main(sa__, val));

    case RX_AFE_PF2:
        return(blackhawk_tsc_INTERNAL_set_rx_pf2(sa__, val));

    case RX_AFE_VGA:
      return(blackhawk_tsc_INTERNAL_set_rx_vga(sa__, val));
    ...
    case RX_AFE_PF3:
        return(blackhawk_tsc_INTERNAL_set_rx_pf3(sa__, val));
    default:
        return(blackhawk_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
}
...
err_code_t blackhawk_tsc_validate_full_txfir_cfg(srds_access_t *sa__, enum blackhawk_tsc_txfir_tap_enable_enum enable_taps, blackhawk_tsc_txfir_st txfir) {
    ...
    for (tap_num=0; tap_num<6; tap_num++) {
        ...
        abs_total += SRDS_ABS(txfir.tap[tap_num]);
    }
...
err_code_t blackhawk_tsc_uc_reset_with_info(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
  if (enable) {
    ...
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD200, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD201, 0x4000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD202, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD204, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD205, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD206, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD207, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD208, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD209, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD20A, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD20B, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD20C, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD20D, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD20E, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD211, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD212, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD213, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD214, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD215, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD216, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD217, 0x0003));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD218, 0xffff));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD219, 0x0e01));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD220, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD221, 0x0000));

    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD225, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD226, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD227, 0x8382));
    
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD229, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD22B, 0x0000)); /* fast_read_en = 0 for all BHK */
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD22C, 0x0000));

    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD230, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD231, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD232, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD233, 0x0000));

    {
        ...
        for (micro_idx = 0; micro_idx < num_micros; micro_idx++) {
            EFUN(blackhawk_tsc_set_micro_idx(sa__, micro_idx));
            EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD240, 0x0000)); /* Per micro registers */
            EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD241, 0x8000));
            /* EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD243, 0x0003));  Micro sets this register (lane_sel) separately */
            EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD244, 0x0000));
            EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD245, 0x000e));
            EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD24D, 0x0000));
            EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD24E, 0x0000));
        }
        EFUN(blackhawk_tsc_set_micro_idx(sa__, micro_orig));
    }
#endif /* wrc_micro_micro_s_rstb */

    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD22B, 0x0000)); /* fast_read_en = 0 for all BHK */   /*TODO: is this required ?!! */
    ...
    /* Stack Size calculation */
    {
      ...
      EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD22E, (0x8000 | (stack_size)<<2)));
    }
    ...
    /* CODE_RAM, DATA_RAM calculation */
    {
        ...
        EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD228, ram_config_val)); /* RAM CONFIG */
    }
  }
  else {
#ifdef wrc_micro_micro_s_rstb
