...
static void _blackhawk7_l8p2_display_eye_scan_ignored_lane(srds_access_t *sa__, uint8_t eye_slice) {
    if (eye_slice) {
        USR_PRINTF(("         : -"));
    } else {
        USR_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
...
err_code_t blackhawk7_l8p2_display_eye_scan(srds_access_t *sa__) {
    ...
    Exit_Status = blackhawk7_l8p2_INTERNAL_eye_scan_setup(sa__, 0, &y_max, &y_step, &usr_diag_status);
    ...
    if (y_step == 0) {
        blackhawk7_l8p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
        Exit_Status = ERR_CODE_BAD_PTR_OR_INVALID_INPUT;
        goto Exit;
    }
    CFUN(blackhawk7_l8p2_display_eye_scan_header(sa__, 1, &usr_diag_status));
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        if(enable_read) {
            Exit_Status = blackhawk7_l8p2_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (Exit_Status) {
                EFUN_PRINTF(("ERROR: blackhawk7_l8p2_read_eye_scan_stripe() returned: %s, (status = 0x%x)\n", blackhawk7_l8p2_INTERNAL_e2s_err_code(Exit_Status), status));
                goto Exit;
            }
        }
        CFUN(blackhawk7_l8p2_display_eye_scan_stripe(sa__, y, &stripe[buffer_idx], usr_diag_status));
        USR_PRINTF(("\n"));
    }
    CFUN(blackhawk7_l8p2_display_eye_scan_footer(sa__, 1, &usr_diag_status));
    /* stop acquisition */
    CFUN(blackhawk7_l8p2_meas_eye_scan_done(sa__));
Exit:
    return(blackhawk7_l8p2_error_report(sa__, Exit_Status));
}

err_code_t blackhawk7_l8p2_display_all_eye_scan(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    ...
    for ( lane=0; lane < num_lanes; lane++) {
        err_code = blackhawk7_l8p2_acc_set_lane(sa__, lane);
        if (err_code) {
            USR_PRINTF(("%s WARN: Unable to switch to lane(%d) and so ignoring lane for for 2D eye scan\n", API_FUNCTION_NAME, lane));
            continue;
        }
        err_code = blackhawk7_l8p2_INTERNAL_eye_scan_setup(sa__, 0, &i_max[lane], &i_step[lane], &usr_diag_status[lane]);
        if (err_code) {
            uint8_t core;
    ...
    if(lane_mask) {
        CFUN(blackhawk7_l8p2_display_eye_scan_header(sa__, num_lanes, &usr_diag_status[0]));
    }
    else {
        goto Exit;
    }
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                ...
                if(i[lane] >= -i_max[lane]) {
                    local_error = blackhawk7_l8p2_acc_set_lane(sa__, lane);
                    if (local_error) {
                        lane_mask &= (uint8_t)~(1<<lane);
                        _blackhawk7_l8p2_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                        continue;
                    }
                    status = 0;
                    if(eye_slice[lane]) {
                    ...
                    if(enable_read) {
                        local_error = blackhawk7_l8p2_read_eye_scan_stripe(sa__, &stripe[0], &status);
                        if (local_error) {
                            lane_mask &= (uint8_t)~(1<<lane);
                            _blackhawk7_l8p2_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                            continue;
                    ...
                    local_error = blackhawk7_l8p2_display_eye_scan_stripe(sa__, i[lane], &stripe[buffer_idx], usr_diag_status[lane]);
                    if (local_error) {
                        lane_mask &= (uint8_t)~(1<<lane);
                        _blackhawk7_l8p2_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                        continue;
    ...
    CFUN(blackhawk7_l8p2_display_eye_scan_footer(sa__, num_lanes, &usr_diag_status[0]));
    for(lane=0;lane < num_lanes;lane++) {
        if(lane_mask & (1<<lane)) {
            CFUN(blackhawk7_l8p2_acc_set_lane(sa__, lane));
            CFUN(blackhawk7_l8p2_meas_eye_scan_done(sa__));
        }
    }
Exit:

    if (blackhawk7_l8p2_acc_set_lane(sa__, lane_orig)) {
...
err_code_t blackhawk7_l8p2_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    ...
    EFUN(blackhawk7_l8p2_pmd_lock_status(sa__,&lock));
    {
        uint8_t sts;
...
err_code_t blackhawk7_l8p2_INTERNAL_poll_diag_data(srds_access_t *sa__, const srds_info_t *blackhawk7_l8p2_info_ptr, uint16_t *status,
...
err_code_t blackhawk7_l8p2_display_eye_scan_stripe(srds_access_t *sa__, int16_t y,uint32_t *buffer, uint16_t usr_diag_status) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l8p2_display_eye_scan_header(srds_access_t *sa__, uint8_t i, uint16_t *usr_diag_status) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    ...
    for(x=1;x<=i;x++) {
        {
            _blackhawk7_l8p2_display_eye_scan_ignored_lane(sa__, 0);
        }
    }
    EFUN_PRINTF(("\n"));
    return(ERR_CODE_NONE);
...
err_code_t blackhawk7_l8p2_display_eye_scan_footer(srds_access_t *sa__, uint8_t i, uint16_t *usr_diag_status) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    uint8_t stepsize_var[NUM_LANES_MAX];
    uint8_t lane, num_lanes;
...
#ifndef STANDALONE_EVENT
            if(intrusive) {
                CFUN(blackhawk7_l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], 0, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            } else {
                CFUN(blackhawk7_l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            }
#else
            margins[i] = info_out->margins[i];
 ...
 */
    ...
    if (DIAG_VERBOSE > 2) USR_PRINTF(("first good ber idx at %d, ber = 1e%f\n", first_good_ber_idx, ((first_good_ber_idx>=0) ? log10(bers[first_good_ber_idx]) : 0.0)));
    {
        ...
        if(first_small_errcnt_idx != -1 ) {
            USR_SNPRINTF(aux_str, sizeof(aux_str), "%d", total_errs[first_small_errcnt_idx]);
        } else {
    ...
    UNUSED(max_offset);
    USR_PRINTF(("This function needs SERDES_API_FLOATING_POINT define to operate \n"));
    if(!total_errs || !total_time ) {
        return(blackhawk7_l8p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    CFUN(blackhawk7_l8p2_get_uc_core_config(sa__, &core_config));
    CFUN(blackhawk7_l8p2_INTERNAL_get_osr_mode(sa__, &osr_mode));
    ...
    CFUN(blackhawk7_l8p2_start_ber_scan_test(sa__, ber_scan_mode, timer_control, max_error_control));
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        CFUN(blackhawk7_l8p2_INTERNAL_ladder_setting_to_mV(sa__, offset_start, range250, slicer, &nlmv_val));
    ...
    CFUN(blackhawk7_l8p2_INTERNAL_poll_diag_done(sa__, &sts,(uint32_t)(timer_control*2000)));
    ...
    if((sts & 0x8000) == 0) {
        ...
        blackhawk7_l8p2_error(sa__, Exit_Status);
        goto Exit;
    }
    cnt = (uint8_t)((sts & 0x00FF)/3);
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        CFUN(blackhawk7_l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        CSTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        CFUN(blackhawk7_l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        ...
        if(DIAG_VERBOSE < 5) {
            ...
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                CFUN(blackhawk7_l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)(SRDS_ABS(offset_start)-i), range250, slicer, &nlmv_val));
    ...
    USR_PRINTF(("\n"));
    CFUN(blackhawk7_l8p2_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
Exit:
...
err_code_t blackhawk7_l8p2_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    ESTM_PRINTF(("\nSilicon Version      = V%X", BLACKHAWK7_L8P2_UAPI_SWITCH));
    EFUN_PRINTF(("\n\n"));
    ...
    if((diag_level & SRDS_DIAG_TIMESTAMP) || (diag_level & SRDS_DIAG_CORE)) {
        diag_data_err = blackhawk7_l8p2_test_uc_timestamp(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
            EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_TIMESTAMP returned error %s ****\n", blackhawk7_l8p2_INTERNAL_e2s_err_code(diag_data_err)));
            diag_level_error |= SRDS_DIAG_TIMESTAMP;
        }
    }
    if(diag_level & SRDS_DIAG_CORE) {
        EFUN(blackhawk7_l8p2_display_core_state_hdr(sa__));
        diag_data_err = blackhawk7_l8p2_display_core_state_line(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    diag_data_err = blackhawk7_l8p2_display_lane_state(sa__);
    if(diag_data_err != ERR_CODE_NONE) {
        EFUN_PRINTF(("\n**** WARNING: blackhawk7_l8p2_display_lane_state() returned error %s ****\n", blackhawk7_l8p2_INTERNAL_e2s_err_code(diag_data_err)));
    }
...
Exit:
        USR_FREE(lane_st);
        if(Exit_Status != ERR_CODE_NONE) {
            EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_LANE returned error %s ****\n", blackhawk7_l8p2_INTERNAL_e2s_err_code(Exit_Status)));
            diag_level_error |= SRDS_DIAG_LANE;
            diag_data_err = blackhawk7_l8p2_error_report(sa__, Exit_Status);
		}
    }
    ...
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        ...
        uint8_t micro_num = 0;
        srds_info_t * blackhawk7_l8p2_info_ptr_tmp;
        ESTM(blackhawk7_l8p2_info_ptr_tmp = blackhawk7_l8p2_INTERNAL_get_blackhawk7_l8p2_info_ptr_with_check(sa__));
        for (; micro_num<blackhawk7_l8p2_info_ptr_tmp->micro_count; ++micro_num)
        {
            uint8_t current_lane;
            state.index = 0;
            state.line_start_index = 0;
            current_lane = blackhawk7_l8p2_acc_get_lane(sa__);
            diag_data_err = blackhawk7_l8p2_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, blackhawk7_l8p2_INTERNAL_event_log_dump_callback);
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        {
            uint8_t current_lane;
            current_lane = blackhawk7_l8p2_acc_get_lane(sa__);
            diag_data_err = blackhawk7_l8p2_read_event_log(sa__);
            EFUN(blackhawk7_l8p2_acc_set_lane(sa__,current_lane));
        }
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        diag_data_err = blackhawk7_l8p2_reg_dump(sa__, core, lane, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        diag_data_err = blackhawk7_l8p2_uc_core_var_dump(sa__, core, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
        ...
        {
        uint8_t orig_lane, num_micros, lane_tmp;

        EFUN(blackhawk7_l8p2_get_micro_num_uc_cores(sa__, &num_micros));
        orig_lane = blackhawk7_l8p2_acc_get_physical_lane(sa__);
        for (lane_tmp = 0; lane_tmp < num_micros*2; lane_tmp =(uint8_t)(lane_tmp + 2)) {
            EFUN(blackhawk7_l8p2_acc_set_physical_lane(sa__, lane_tmp));

            diag_data_err = blackhawk7_l8p2_uc_micro_var_dump(sa__, core, lane_tmp, NULL);
            if(diag_data_err != ERR_CODE_NONE) {
                EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_UC_CORE returned error %s ****\n", blackhawk7_l8p2_INTERNAL_e2s_err_code(diag_data_err)));
                diag_level_error |= SRDS_DIAG_UC_CORE;
            }
    ...
    if(diag_level & SRDS_DIAG_UC_LANE) {
        ...
        diag_data_err = blackhawk7_l8p2_uc_lane_var_dump(sa__, core, lane, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_EYE) {
        diag_data_err = blackhawk7_l8p2_display_eye_scan(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        diag_data_err = blackhawk7_l8p2_display_lane_debug_status(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        diag_data_err = blackhawk7_l8p2_INTERNAL_display_info_table(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
