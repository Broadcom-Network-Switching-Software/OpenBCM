...
static err_code_t _blackhawk7_l8p2_reg_print(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr, char reg_buffer[SRDS_DUMP_BUF_SIZE]);
...
static err_code_t _blackhawk7_l8p2_reg_print(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr, char reg_buffer[SRDS_DUMP_BUF_SIZE]) {
    INIT_SRDS_ERR_CODE
    if(reg_buffer == NULL) {
         EFUN(_blackhawk7_l8p2_reg_print_no_buf(sa__, start_addr, valid_addr));
    }
    else {
        EFUN(_blackhawk7_l8p2_reg_print_with_buf(sa__, start_addr, valid_addr, reg_buffer));
    }
    return (ERR_CODE_NONE);
}
        ...
        if(blackhawk7_l8p2_reg_dump_arr[reg_section][0] == BLACKHAWK7_L8P2_REG_PLL_START) {
            ...
            for (pll_idx = 0; pll_idx < NUM_PLLS; pll_idx++) {
                ...
                while( reg_section < BLACKHAWK7_L8P2_REG_DUMP_SECTIONS && blackhawk7_l8p2_reg_dump_arr[reg_section][0] != BLACKHAWK7_L8P2_REG_UC_START) {
                    EFUN(_blackhawk7_l8p2_reg_print(sa__, blackhawk7_l8p2_reg_dump_arr[reg_section][0], blackhawk7_l8p2_reg_dump_arr[reg_section][1], reg_buffer[buf_index]));
                    reg_section++;
...
err_code_t blackhawk7_l8p2_log_full_pmd_state_noPRBS(srds_access_t *sa__, struct blackhawk7_l8p2_detailed_lane_status_st *lane_st) {
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_pmd_lock_status(sa__,&lane_st->pmd_lock, &tmp1));
    {
        err_code_t err_code=ERR_CODE_NONE;
