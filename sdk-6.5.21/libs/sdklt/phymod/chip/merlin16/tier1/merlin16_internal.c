 ...
 */
static err_code_t _merlin16_get_divisor_ratio(enum merlin16_pll_div_enum srds_div, uint16_t *numerator, uint16_t *denominator)
{
    switch (srds_div) {
...
static err_code_t _merlin16_get_refclk_from_div_vco(uint32_t *refclk_freq_hz, enum merlin16_pll_div_enum srds_div, uint32_t vco_freq_khz, enum merlin16_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_merlin16_get_divisor_ratio(srds_div, &numerator, &denominator));
...
err_code_t merlin16_INTERNAL_get_vco_from_refclk_div(uint32_t refclk_freq_hz, enum merlin16_pll_div_enum srds_div, uint32_t *vco_freq_khz, enum merlin16_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_merlin16_get_divisor_ratio(srds_div, &numerator, &denominator));
    if (pll_option == MERLIN16_PLL_OPTION_REFCLK_DOUBLER_EN) refclk_freq_hz *= 2;
    if (pll_option == MERLIN16_PLL_OPTION_REFCLK_DIV2_EN)    refclk_freq_hz /= 2;
...
static err_code_t _merlin16_check_div(enum merlin16_pll_div_enum srds_div) {
    uint8_t i, found = 0;
    for (i=0; i<_merlin16_div_candidates_count; i++) {
        found |= srds_div == _merlin16_div_candidates[i];
    }
    if (!found) {
...
static err_code_t _merlin16_get_div(uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum merlin16_pll_div_enum *srds_div, enum merlin16_pll_option_enum pll_option) {
    ...
    for (i=0; i<_merlin16_div_candidates_count; i++) {
        ...
        if (vco_freq_khz_min_error > vco_freq_khz_error) {
            ...
            *srds_div = _merlin16_div_candidates[i];
        }
    }
...
err_code_t merlin16_INTERNAL_resolve_pll_parameters(enum merlin16_pll_refclk_enum refclk,
    ...
    const uint8_t given_param_count = (((refclk == MERLIN16_PLL_REFCLK_UNKNOWN) ? 0 : 1)
                                       + ((*srds_div == MERLIN16_PLL_DIV_UNKNOWN) ? 0 : 1)
                                       + ((*vco_freq_khz == 0) ? 0 : 1));
    ...
    if (*srds_div != MERLIN16_PLL_DIV_UNKNOWN) {
        EFUN(_merlin16_check_div(*srds_div));
    }
    ...
    if (refclk == MERLIN16_PLL_REFCLK_UNKNOWN) {
        /* Determine refclk from vco frequency and srds_div */
        EFUN(_merlin16_get_refclk_from_div_vco(refclk_freq_hz, *srds_div, *vco_freq_khz, pll_option));
    } else {
        EFUN(_merlin16_get_refclk_in_hz(refclk, refclk_freq_hz));
    }
...
err_code_t merlin16_INTERNAL_display_pll_to_divider(srds_access_t *sa__, uint32_t srds_div) {
