...
extern err_code_t blackhawk7_v1l8p1_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_v1l8p1_ber_data_st *ber_data, struct blackhawk7_v1l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl);
extern err_code_t blackhawk7_v1l8p1_get_BER_data_HW_mode(srds_access_t *sa__, struct blackhawk7_v1l8p1_ber_data_st *ber_data, struct blackhawk7_v1l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl); 
...
uint32_t blackhawk7_v1l8p1_INTERNAL_mult_with_overflow_check(uint32_t a, uint32_t b, uint8_t *of) {
    uint16_t c,d;
    uint32_t r,s;
    if (a > b) return blackhawk7_v1l8p1_INTERNAL_mult_with_overflow_check(b, a, of);
    *of = 0;
    c = (uint16_t)(b >> 16);
    d = UINT16_MAX & b;
    r = a * c;
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_v1l8p1_ber_data_st *ber_data, enum blackhawk7_v1l8p1_prbs_chk_timer_selection_enum timer_sel) {
    INIT_SRDS_ERR_CODE
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    if(timer_sel == USE_HW_TIMERS) {
        struct blackhawk7_v1l8p1_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl_bak;
        err_code_t err = ERR_CODE_NONE;
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_prbs_timeout_count_from_time(uint16_t time_ms, uint16_t * time_ms_adjusted, struct blackhawk7_v1l8p1_prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl) {
...
err_code_t blackhawk7_v1l8p1_INTERNAL_eye_scan_setup(srds_access_t *sa__, uint8_t intr_pass_b, int16_t *y_max, int16_t *y_step) {
    INIT_SRDS_ERR_CODE
    ...
    {   
        ...
        err_code = blackhawk7_v1l8p1_meas_eye_scan_start(sa__, direction);
        ...
        if (err_code) {
            if (err_code == ERR_CODE_DIAG_SCAN_NO_PMD_LOCK) {
                ...
                EFUN(blackhawk7_v1l8p1_meas_eye_scan_done(sa__));
            }
        }
    }
...
/*----------------------------------------------------*/
err_code_t blackhawk7_v1l8p1_INTERNAL_get_pam_eye_margin_est(srds_access_t *sa__, blackhawk7_v1l8p1_eye_margin_t *eye_margin) {
    INIT_SRDS_ERR_CODE
  uint8_t ladder_range = 0;
...
err_code_t blackhawk7_v1l8p1_display_pam_eye_margin_est(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    int8_t i;
    blackhawk7_v1l8p1_eye_margin_t eye_margin = {{0},{0},{0},{0}};
    enum blackhawk7_v1l8p1_rx_pam_mode_enum pam_mode = NRZ;
...
const char* blackhawk7_v1l8p1_INTERNAL_get_osr_str(uint8_t osr_val, uint8_t osr_str_length) {
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_tx_ppm(srds_access_t *sa__, int16_t *tx_ppm) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_v1l8p1_INTERNAL_read_lane_state(srds_access_t *sa__, blackhawk7_v1l8p1_lane_state_st *istate) {
  ...
  EFUN(blackhawk7_v1l8p1_INTERNAL_get_tx_ppm(sa__, &state.tx_ppm));
...
err_code_t blackhawk7_v1l8p1_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {
  ...
  if(state.osr_mode.tx_rx != 255) {
      const char *s;
      s = blackhawk7_v1l8p1_INTERNAL_get_osr_str(state.osr_mode.tx_rx, GET_OSR_STR_LONG);
      EFUN_PRINTF(("(%2s%s, 0x%04x, 0x%02x ,", (state.br_pd_en) ? "BR" : "OS", s, state.ucv_config,state.ucv_status));
  } else {
      const char *s;
      const char *r;
      s = blackhawk7_v1l8p1_INTERNAL_get_osr_str(state.osr_mode.tx, GET_OSR_STR_SHORT);
      r = blackhawk7_v1l8p1_INTERNAL_get_osr_str(state.osr_mode.rx, GET_OSR_STR_SHORT);
      {
          char *t;
          uint8_t tx, rx;
  ...
  }
  {
      ...
      EFUN(blackhawk7_v1l8p1_get_usr_ctrl_disable_startup(sa__, &dsu));
      EFUN(blackhawk7_v1l8p1_get_usr_ctrl_disable_steady_state(sa__, &dss));
      nrz_mode_en = (state.rx_pam_mode == NRZ) && ((dsu.field.eye_margin_estimation) || (dss.field.eye_margin_estimation));
      pam4_mode_en = (state.rx_pam_mode != NRZ) && ((dsu.field.pam_eye_margin_estimation) || (dss.field.pam_eye_margin_estimation));
      disable_display = nrz_mode_en || pam4_mode_en;
  ...
  EFUN(blackhawk7_v1l8p1_INTERNAL_display_BER(sa__,100));
  {
    ...
    if(state.ucv_status != 0) {
      EFUN_PRINTF(("\n"));
      EFUN(blackhawk7_v1l8p1_decode_uc_sts(sa__, state.ucv_status, decode_buf, sizeof(decode_buf), ""));
      EFUN_PRINTF(("%s", decode_buf));
    }
    if(state.ucv_status_ext != 0) {
      EFUN(blackhawk7_v1l8p1_decode_uc_sts_ext(sa__, state.ucv_status_ext, decode_buf, sizeof(decode_buf), ""));
      EFUN_PRINTF(("%s", decode_buf));
    }
  }
...
static err_code_t _blackhawk7_v1l8p1_minimize_divisor_ratio(uint32_t md, uint32_t *simple_numerator, uint32_t *simple_denominator) {
...
static err_code_t _blackhawk7_v1l8p1_get_divisor_ratio(srds_access_t *sa__, enum blackhawk7_v1l8p1_pll_div_enum srds_div, uint16_t *simple_numerator, uint16_t *simple_denominator){
    /*
    ...
    */
    ...
    if (lowest_numerator >= 0x10000){
        _blackhawk7_v1l8p1_minimize_divisor_ratio(0x10000,&lowest_numerator, &lowest_denominator);
...
static const enum blackhawk7_v1l8p1_pll_div_enum _blackhawk7_v1l8p1_div_candidates[] = {
    ...
    BLACKHAWK7_V1L8P1_PLL_DIV_137P721519,
    BLACKHAWK7_V1L8P1_PLL_DIV_140,
    BLACKHAWK7_V1L8P1_PLL_DIV_142P0253165,
    BLACKHAWK7_V1L8P1_PLL_DIV_144,
    BLACKHAWK7_V1L8P1_PLL_DIV_152,
    BLACKHAWK7_V1L8P1_PLL_DIV_165P7770801,
    BLACKHAWK7_V1L8P1_PLL_DIV_170P800628,
    BLACKHAWK7_V1L8P1_PLL_DIV_179P7348805,
    BLACKHAWK7_V1L8P1_PLL_DIV_179P7356828,
    BLACKHAWK7_V1L8P1_PLL_DIV_180P5309734,
...
err_code_t blackhawk7_v1l8p1_get_dbgfb_summary(srds_access_t *sa__, blackhawk7_v1l8p1_dbgfb_cfg_st *dbgfb_cfg) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_v1l8p1_collect_dbgfb_stats(srds_access_t *sa__, uint32_t time_in_us, blackhawk7_v1l8p1_dbgfb_stats_st *dbgfb_stats) {
    INIT_SRDS_ERR_CODE
    uint8_t y, x;
    uint8_t max_y;
    enum blackhawk7_v1l8p1_rx_pam_mode_enum pam_mode = NRZ;
    uint8_t pam4es_en = 0;
    blackhawk7_v1l8p1_dbgfb_cfg_st dbgfb_cfg = DBGFB_CFG_ST_INIT;
