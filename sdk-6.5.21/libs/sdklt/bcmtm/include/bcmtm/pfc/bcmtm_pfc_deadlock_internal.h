 ...
 */
typedef struct bcmtm_pfc_ddr_pri_info_s {

   /*! Per PFC priority deadlock recovery timer value. */
   uint8_t recovery_count_cfg;
   ...
   /*! Per PFC priority deadlock recovery mode. */
   uint8_t recovery_mode;

   /*! Per PFC priority manual deadlock recovery enable. */
   uint8_t manual;

   /*! Per PFC priority manual deadlock recovery enabled status. */
   uint8_t manual_oper;

   /*! Per PFC priority deadlock state. */
   uint8_t state;
} bcmtm_pfc_ddr_pri_info_t;
...
typedef struct bcmtm_pfc_ddr_ctrl_s {
   ...
   /*! PFC pri DDR info. */
    bcmtm_pfc_ddr_pri_info_t  pri_info[BCMDRD_CONFIG_MAX_PORTS][MAX_NUM_PFC_PRI];
...
typedef int (*bcmtm_pfc_ddr_recovery_trigger_f)(int unit,
                                        ...
                                        int32_t pfc_pri);
/*!
 * \brief TM PFC DDR recovery function with user mode.
 */
typedef int (*bcmtm_pfc_ddr_recovery_user_trigger_f)(int unit,
                                        int32_t lport,
                                        int32_t pfc_pri,
                                        int32_t user_mode);
...
typedef struct bcmtm_pfc_ddr_drv_s {
    ...
    bcmtm_pfc_ddr_recovery_user_trigger_f hw_recovery_begin;

    /*! Function for ending the deadlock recovery. */
    bcmtm_pfc_ddr_recovery_trigger_f hw_recovery_end;

    /*! Function for clearing the deadlock interrupt. */
    bcmtm_pfc_ddr_recovery_trigger_f hw_status_clear;
...
/*!
 * \brief To populate the TM_DEADLOCK_RECOVERY_STATE LT.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
...
/*!
 * \brief To populate the TM_DEADLOCK_RECOVERY_MANUAL_STATE LT.
 *
 * \param [in] unit Unit number.
 *
 * \retval SHR_E_NONE No errors.
...
bcmtm_pfc_deadlock_recovery_status_populate_imm_entry(int unit,
                                                      ...
                                                      bool up);
/*!
 * \brief To populate the TM_DEADLOCK_RECOVERY_STATE LT entry.
 *
 * \param [in] unit Unit number.
 * \param [in] lport Logical port ID.
 * \param [in] pri PFC priority.
 * \param [in] state deadlock recovery state.
...
/*!
 * \brief To populate the TM_DEADLOCK_RECOVERY_MANUAL_STATE LT entry.
 *
 * \param [in] unit Unit number.
 * \param [in] lport Logical port ID.
 * \param [in] pri PFC priority.
