...
/*! Emulation multiplication factor used for timeouts. */
...
#define MMU_CFG_F_PORT_MIN              0x01
/*! Supports per port per pool min guarentee. */
#define MMU_CFG_F_PORT_POOL_MIN         0x02
/*! Supports RQE entries. */
#define MMU_CFG_F_RQE                   0x04
/*! Supports MC queue entries. */
#define MMU_CFG_F_EGR_MCQ_ENTRY         0x08

/*! Byte to cell conversion. */
...
/*! \brief Get pfc deadlock recovery information.
 *
 * \param [in] unit Logical unit number.
 * \param [out] pfc_ddr_info Pfc deadlock recovery info.
 */
extern void
bcmtm_pfc_ddr_info_get(int unit, bcmtm_pfc_ddr_info_t **pfc_ddr_info);

/*! \brief Set pfc deadlock recovery information.
 *
 * \param [in] unit Logical unit number.
 * \param [in] pfc_ddr_info Pfc deadlock recovery info.
 */
extern int
bcmtm_pfc_ddr_info_set(int unit, bcmtm_pfc_ddr_info_t *pfc_ddr_info);

/*! \brief Transform field data to uint64 format.
 *
 * \param [in] fdata Data to be transformed.
 * \param [in] width width of fdata.
 *
 * \retval uint64_t value of the data.
 */
extern uint64_t
