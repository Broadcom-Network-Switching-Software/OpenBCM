 ...
 * \param [in] new_cfg structure rray for ports.
 *
 * \retval SHR_E_NONE No errors.
 */
typedef void (*bcmtm_drv_portmap_fill_f)(
    int unit, size_t num_ports, const bcmpc_mmu_port_cfg_t *new_cfg);
...
/*! Returns pm number and subp number from logical port. */
typedef int
(*bcmtm_drv_lport_pm_num_subp_get_f) (int unit, int lport, int *pm_num,
                                      int *subp);


/*! Returns logical port from pipe, pm and subp. */
typedef int
(*bcmtm_drv_pipe_pm_subp_lport_get_f) (int unit, int pipe, int pm_num, int subp,
                                       int *lport);
...
typedef struct bcmtm_drv_s {
   ...
   /*! MoD driver function pointers. */
    ...
    /*! Device RX Queue driver get. */
    bcmtm_drv_get_f ts_tod_drv_get;
    ...
    /*! Get OBM/PM# and subport from logical port number. */
    bcmtm_drv_lport_pm_num_subp_get_f lport_pm_subp_get;

    /*! Get logical port number from pipe, pm and subport. */
    bcmtm_drv_pipe_pm_subp_lport_get_f pm_subp_lport_get;
} bcmtm_drv_t;
