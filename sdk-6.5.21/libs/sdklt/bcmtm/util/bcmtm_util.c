...
int
bcmtm_lport_is_fae(int unit, bcmtm_lport_t lport)
{
    if (bcmdrd_lport_type_get(unit, lport) == BCMDRD_PORT_TYPE_FAE) {
        return true;
    }
    return false;
}
                      ...
                      const bcmpc_mmu_port_cfg_t *new_cfg)
{
    ...
    if (tm_drv->portmap_update) {
        tm_drv->portmap_update(unit, num_ports, new_cfg);
    }
...
bcmtm_drv_info_dump(int unit, shr_pb_t *pb)
{
    ...
    BCMDRD_PBMP_ITER(pbmp, pport) {
        ...
        if (bcmtm_lport_is_cpu(unit, lport)) {
            ...
            sys_port = "RDB";
        } else if (bcmtm_lport_is_fae(unit, lport)) {
            sys_port = "FAE";
        } else {
            sys_port = "";
        }
...
uint64_t
bcmtm_lt_field_data_to_u64_xfrm (bcmltd_field_data_t *fdata, size_t width)
{
    if (width == 1) {
        return (uint64_t)fdata->is_true;
    } else if (width <= 8) {
        return (uint64_t)fdata->u8;
    } else if (width <= 16) {
        return (uint64_t)fdata->u16;
    } else if (width <= 32) {
        return (uint64_t)fdata->u32;
    } else {
        return (uint64_t)fdata->u64;
    }
}

int
bcmtm_lport_pm_num_subp_get(int unit,
                            int lport,
                            int pports_per_pipe,
                            int pms_per_pipe,
                            int *pm_num,
                            int *subp)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_drv_get(unit, &tm_drv));
    if (tm_drv->lport_pm_subp_get) {
        SHR_IF_ERR_EXIT(tm_drv->lport_pm_subp_get(unit, lport, pm_num, subp));
    } else {
        if (pports_per_pipe == -1 || pms_per_pipe == -1) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
        SHR_IF_ERR_EXIT(bcmtm_lport_pport_get(unit, lport, &pport));
        local_phy_port = (pport - 1) % pports_per_pipe;
        *subp = local_phy_port % pms_per_pipe;
        *pm_num = local_phy_port / pms_per_pipe;
    }
exit:
    SHR_FUNC_EXIT();
}
...
int
bcmtm_pipe_pm_num_subp_lport_get(int unit, int pipe, int pm_num, int subp,
                                 int pports_per_pipe, int pms_per_pipe,
                                 int *lport)
{
    bcmtm_drv_t *tm_drv;
    int pport;
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_drv_get(unit, &tm_drv));
    if (tm_drv->pm_subp_lport_get) {
        SHR_IF_ERR_EXIT(tm_drv->pm_subp_lport_get(unit, pipe, pm_num, subp, lport));
    } else {
        if (pports_per_pipe == -1 || pms_per_pipe == -1) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
        pport = (pipe * pports_per_pipe) + (pm_num * pms_per_pipe) + subp + 1;
        SHR_IF_ERR_EXIT(bcmtm_pport_lport_get(unit, pport, lport));
    }
