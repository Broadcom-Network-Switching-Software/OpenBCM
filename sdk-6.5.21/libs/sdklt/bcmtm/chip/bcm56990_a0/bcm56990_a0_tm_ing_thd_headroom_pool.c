/*! \file bcm56990_a0_tm_ing_thd_headroom_pool.c
 *
...
static int
tm_thd_ing_headroom_pool_delete(int unit, bcmltd_sid_t sid,
                                const bcmltd_op_arg_t *op_arg, int itm, int pool)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_thd_ing_headroom_pool_update(unit, sid, op_arg, itm, pool, data_fields,
                                         &dummy_fields));
                               ...
                               bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT
                (tm_thd_ing_headroom_pool_update(unit, sid, op_arg, itm, pool,
                                                 data_fields, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_thd_ing_headroom_pool_delete(unit, sid, op_arg, itm, pool));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
static int
tm_thd_ing_headroom_pool_lookup(int unit,
                                bcmltd_sid_t sid,
                                const bcmltd_op_arg_t *op_arg,
                                void *context,
                                ...
                                bcmltd_fields_t *out)
{
    ...
    fid = TM_ING_THD_HEADROOM_POOLt_BUFFER_POOLf;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_imm_fields_get(unit, key, fid, &idx));
    itm = idx;
    fid = TM_ING_THD_HEADROOM_POOLt_TM_HEADROOM_POOL_IDf;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_imm_fields_get(unit, key, fid, &idx));
    pool = idx;

    ptid = MMU_THDI_HDRM_BUFFER_CELL_LIMIT_HPr;
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_ING_THD_HEADROOM_POOLt_LIMIT_CELLSf,
                                fval));
...
bcm56990_a0_tm_thd_ing_headroom_pool_register(int unit)
{
    ...
    tm_thd_cb_handler.op_stage = tm_thd_ing_headroom_pool_stage;
    tm_thd_cb_handler.op_lookup = tm_thd_ing_headroom_pool_lookup;
...
bcm56990_a0_tm_thd_ing_headroom_pool_populate(int unit)
{
    ...
    for (itm = 0; itm < SOC_MMU_CFG_ITM_MAX; itm++) {
        ...
        for (pool = 0; pool < SOC_MMU_CFG_SERVICE_POOL_MAX; pool++) {
            SHR_IF_ERR_EXIT
                (tm_thd_ing_headroom_pool_populate_entry(unit, itm, pool));
        }
    }
