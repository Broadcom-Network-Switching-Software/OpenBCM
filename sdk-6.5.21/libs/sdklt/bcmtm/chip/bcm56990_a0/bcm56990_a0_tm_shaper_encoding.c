/*! \file bcm56990_a0_tm_shaper_encoding.c
 *
...
#define TH4_METER_GRANULARITY_DEFAULT       3
#define TH4_METER_GRANULARITY_NUM           8
/*! 32000 bits/second */
#define TH4_METER_KBITS_SEC_QUANTUM_MIN     32
/*! 256 bytes */
#define TH4_METER_BITS_BURST_MIN            (256 * 8)
#define TH4_METER_PACKET_SEC_QUANTUM_MIN    2
/*! 0.256 packet, use 1000x */
#define TH4_METER_MMU_PACKET_BURST_MIN      128
/*! 0.512 packet, use 1000x */
#define TH4_METER_FP_PACKET_BURST_MIN       512
/*! use 1000x for calculation */
#define TH4_METER_PACKET_BURST_DIVISOR      1000
#define TH4_METER_NL_SEGMENT_GRANULARITY    256
#define TH4_METER_NL_BUCKET_POWER_SHIFT     8
#define TH4_METER_NL_BUCKET_RANGE_NUM       16
#define TH4_METER_NL_BUCKET_SEGMENT_MASK    0xff
#define TH4_METER_NL_BUCKET_POWER_MASK      0xf
/* 2 ** 0xf * (1 + 0xff/256) */
#define TH4_METER_NL_BUCKET_MAC_ENCODE_MAX  0xff80
#define TH4_REFRESH_MASK_SZ   15
#define TH4_BUCKET_MASK_SZ    20

/* Select the minimum */
#define TH4_MIN_SELECT(a, b) (a > b ? (a = b) : (a = a))
 ...
 */
static int
bcm56990_a0_tm_port_shaper_bandwidth(int unit,
                                     bcmtm_lport_t lport,
                                     uint32_t *bandwidth)
{
    bcmdrd_sid_t sid;
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_a0_tm_shaper_bucket_to_rate(unit, shaping_mode, &bucket_encode));
 ...
 */
static void
bcm56990_a0_tm_granularity_params(int unit,
                                  int granularity,
                                  ...
                                  uint32_t *burst_unit_sz)
{
    ...
    if (shaping_mode != 0) {
        ...
        *burst_unit_sz = TH4_METER_MMU_PACKET_BURST_MIN * gran_multiple;
        *rate_unit_sz = TH4_METER_PACKET_SEC_QUANTUM_MIN * gran_multiple;
    } else {
        /* Byte mode */
        gran_multiple = 1 << granularity;
        *rate_unit_sz =  TH4_METER_KBITS_SEC_QUANTUM_MIN * gran_multiple;
        *burst_unit_sz = TH4_METER_BITS_BURST_MIN * gran_multiple;
    }
}
 ...
 */
static int
bcm56990_a0_tm_default_burst_size(int unit,
                                  bcmtm_lport_t lport,
                                  uint32_t bandwidth)
{
#define TH4_PORT_MTU    9216
     ...
     */
    if (SHR_FAILURE(bcm56990_a0_tm_port_shaper_bandwidth(unit, lport, &port_bw))) {
        port_bw = 0;
    }
    ...
    if (max_port_speed > bandwidth) {
        proposed1 = (TH4_PORT_MTU * numq)/(max_port_speed/bandwidth);
    } else {
        if (((max_port_speed + bandwidth - 1) / bandwidth) == 0){
            return 0;
        }
        proposed1 = (TH4_PORT_MTU * numq)/
                                     ...
                                     bcmtm_shaper_bucket_encode_t *bucket_encode)
{
    ...
    SHR_FUNC_ENTER(unit);
    bcm56990_a0_tm_granularity_params(unit, bucket_encode->granularity,
    ...
    if (itu_mode) {
        if (bucket_encode->bucket_sz == 0) {
            ...
            segment =
                bucket_encode->bucket_sz & TH4_METER_NL_BUCKET_SEGMENT_MASK;
            power =
                (bucket_encode->bucket_sz >> TH4_METER_NL_BUCKET_POWER_SHIFT)
                & TH4_METER_NL_BUCKET_POWER_MASK;
            /* Calculate raw bits */
            bucket_sz = (1 << power) *
                (burst_unit_sz / TH4_METER_NL_SEGMENT_GRANULARITY) *
                (TH4_METER_NL_SEGMENT_GRANULARITY + segment);
        }
    } else {
    ...
    if (shaping_mode != 0) {
        /* packet mode */
        bucket_encode->burst = bucket_sz / TH4_METER_PACKET_BURST_DIVISOR;
    } else {
                                     ...
                                     bcmtm_shaper_bucket_encode_t *bucket_encode)
{
    ...
    SHR_FUNC_ENTER(unit);
    refresh_mask = 0xffffffff >> TH4_REFRESH_MASK_SZ;
    bucket_mask = 0xffffffff >> TH4_BUCKET_MASK_SZ;

    /* calculate burst rate based on bandwidth allocated */
    if (burst_auto) {
        burst_size = (bandwidth > 0) ?
            bcm56990_a0_tm_default_burst_size(unit, lport, bandwidth) : 0 ;
    }

    if ((burst_size  == 0) && (bandwidth == 0)) {
        bucket_encode->refresh_rate = 0;
        bucket_encode->bucket_sz = 0;
        bucket_encode->granularity = TH4_METER_GRANULARITY_DEFAULT;
    }

    /* Packet mode */
    if (shaping_mode != 0) {
        burst = burst_size * TH4_METER_PACKET_BURST_DIVISOR;
    } else {
        /* Byte mode */
    ...
    SHR_IF_ERR_EXIT
        (bcmcfg_field_get(unit, TM_SHAPER_CONFIGt,
                          TM_SHAPER_CONFIGt_ITU_MODEf, &itu_mode));
    /* granularity */
    for (gran = 0; gran <= TH4_METER_GRANULARITY_NUM - 1; gran++) {
        bcm56990_a0_tm_granularity_params(unit, gran, shaping_mode,
                &refresh_unit_sz, &burst_unit_sz);
        refresh_max = refresh_mask * refresh_unit_sz;
        if (itu_mode)
            bucket_max = TH4_METER_NL_BUCKET_MAC_ENCODE_MAX * burst_unit_sz;
        else
    ...
    if (gran > TH4_METER_GRANULARITY_NUM - 1) {
        /* Saturate */
        gran = TH4_METER_GRANULARITY_NUM - 1;
        TH4_MIN_SELECT(bandwidth, refresh_max);
        TH4_MIN_SELECT(burst, bucket_max);
    }
    ...
    TH4_MIN_SELECT(bucket_encode->refresh_rate, refresh_mask);
    ...
    if (itu_mode) {
        ...
        if (burst <= burst_unit_sz) {
            ...
            for (i = 0; i < TH4_METER_NL_BUCKET_RANGE_NUM;
                    i++, bucket_top *= 2) {
                 bucket_segment_size =
                    bucket_top / TH4_METER_NL_SEGMENT_GRANULARITY;
                if (burst <= (bucket_top * 2 - bucket_segment_size)) {
                    encoding =
                        (burst - bucket_top + (bucket_segment_size - 1)) /
                        bucket_segment_size;
                    encoding |= i << TH4_METER_NL_BUCKET_POWER_SHIFT;
                    break;
                }
            }
            bucket_encode->bucket_sz =
                (i < TH4_METER_NL_BUCKET_RANGE_NUM) ? encoding : bucket_mask;
        }
    } else {
