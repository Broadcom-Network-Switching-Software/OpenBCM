/*! \file bcm56990_a0_tm_pfc_deadlock.c
 *
 ...
 */
int
bcm56990_a0_pfc_deadlock_recovery_begin(int unit, int lport, int pri, int mode)
{
    int mport = -1;
...
/*!
 * \brief PFC deadlock status clear.
 *
 * The function is used to clear the deadlock interrupt reported in h/w.
 * state.
 *
 * \param [in] unit Unit number.
 * \param [in] lport device port number.
 ...
 */
int
bcm56990_a0_pfc_deadlock_hw_status_clear(int unit, int lport, int pri)
{
    int mport = -1;
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmltd_sid_t sid = TM_PFC_DEADLOCK_RECOVERYt;
    ...
    /* 1: Unmask the interrupt by setting 0. */
    rval = 0;
    fval = 0;
...
bcm56990_a0_tm_pfc_deadlock_recovery_status_populate(int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_pfc_deadlock_recovery_state_imm_populate(unit));

    SHR_IF_ERR_EXIT
        (bcmtm_pfc_deadlock_recovery_manual_state_imm_populate(unit));
...
int
bcm56990_a0_tm_pfc_deadlock_recovery_status_update(int unit,
                                                   int pport,
                                                   int up)
{
    int lport, rv, pri = 0;
        ...
        for (pri = 0; pri < MAX_NUM_PFC_PRI; pri++) {
            SHR_IF_ERR_EXIT
                (bcmtm_pfc_deadlock_recovery_state_populate_imm_entry(unit,
                                (bcmtm_lport_t )lport, pri, 0, FALSE, up));
        }
    }
...
int
bcm56990_a0_tm_pfc_deadlock_recovery_update(int unit,
                                            int pport, int up)
{
    int lport, rv, pri = 0;
    const bcmlrd_map_t *map = NULL;
