                             ...
                             mmu_sched_inst_name_e sched_inst_name)
{
    ...
    fval = th4_sched_urg_pick_spacing[freq_indx].val_400g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = URG_PICK_200G_SPACINGf;
    fval = th4_sched_urg_pick_spacing[freq_indx].val_200g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = URG_PICK_100G_SPACINGf;
    fval = th4_sched_urg_pick_spacing[freq_indx].val_100g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = URG_PICK_50G_SPACINGf;
    fval = th4_sched_urg_pick_spacing[freq_indx].val_50g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
