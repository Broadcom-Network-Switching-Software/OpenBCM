/*! \file bcm56880_a0_tm_thd_uc_q.c
 *
        ...
        soc_mmu_cfg_buf_t *buf)
{
    ...
    oper_state = PORT_INFO_UNAVAILABLE;
        ...
        if (!bcmtm_lport_is_cpu(unit, lport)) {
            num_q = bcm56880_a0_tm_num_uc_q_non_cpu_port_get(unit);
        } else {
            num_q = 0;
        }
        if (queue >= num_q) {
            ...
            SHR_IF_ERR_EXIT
                (bcmtm_field_list_set(unit, &entry,
                    TM_THD_UC_Qt_RESUME_OFFSET_CELLSf, 0, (buf_queue->pool_resume *
                    TD4_GRANULARITY)));
            SHR_IF_ERR_EXIT
                (bcmtm_field_list_set(unit, &entry,
                   ...
                   uint8_t action)
{
        ...
        switch (data_ptr->id) {
            case TM_THD_UC_Qt_MIN_GUARANTEE_CELLSf:
               ...
               /*
                ...
                if (SHR_SUCCESS
                        (bcm56880_a0_tm_shared_buffer_update(unit, 0, pool, delta,
                                                             RESERVED_BUFFER_UPDATE,
                                                             TRUE)) &&
                    SHR_SUCCESS
                        (bcm56880_a0_tm_shared_buffer_update(unit, 1, pool, delta,
                                                             RESERVED_BUFFER_UPDATE,
                                                             TRUE))) {
            ...
            case TM_THD_UC_Qt_COLOR_SPECIFIC_DYNAMIC_LIMITSf:
                ...
                if (use_dynamic_color) {
                    ...
                    if (SHR_SUCCESS
                        ...
                        limit = CEILING(limit, TD4_GRANULARITY);
                        SHR_IF_ERR_EXIT
                            (bcmtm_field_set(unit, ptid, LIMIT_YELLOWf,
                                             q_config_buf, &limit));
                    }
    ...
    if (ptid1 != INVALIDm) {
        ...
        if (ptid1 == MMU_THDO_Q_TO_QGRP_MAPD0m) {
            ...
            SHR_IF_ERR_EXIT
                (bcmtm_pt_indexed_write(unit, ptid1, sid, &pt_dyn_info, q_config_buf));
            ptid1 = MMU_THDO_Q_TO_QGRP_MAPD2m;
            SHR_IF_ERR_EXIT
                (bcmtm_pt_indexed_write(unit, ptid1, sid, &pt_dyn_info, q_config_buf));
        }
    }
...
static int
tm_thd_uc_q_delete(int unit, bcmltd_sid_t sid, int lport, int queue)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_thd_uc_q_update(unit, sid, lport, queue, data_fields, &dummy_fields,
                            FALSE));
                  ...
                  bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT
                (tm_thd_uc_q_update(unit, sid, lport, queue, data_fields,
                                    output_fields, UPDATE_ONLY));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_thd_uc_q_delete(unit, sid, lport, queue));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
 ...
 */
int
bcm56880_a0_tm_thd_uc_q_register(int unit)
{
    ...
    tm_thd_cb_handler.stage    = tm_thd_uc_q_stage;
...
int
bcm56880_a0_tm_thd_uc_q_populate(int unit)
{
    int lport, queue, rv;
    const bcmlrd_map_t *map = NULL;
    bcmtm_drv_info_t *drv_info;
    ...
    for (lport = 1; lport < TD4_MMU_NUM_LPORT; lport++) {
        td4_mmu_config_thdo_set(unit, lport, buf, &devcfg, drv_info->lossless);
        for (queue = 0; queue < TD4_MAX_NUM_UC_Q; queue++) {
            SHR_IF_ERR_EXIT(tm_thd_uc_q_populate_entry(unit, lport, queue, 1,
                        buf));
            SHR_IF_ERR_EXIT_EXCEPT_IF(rv, SHR_E_NONE);
        }
    }
...
int
bcm56880_a0_tm_thd_uc_q_update(int unit, int pport, int up)
{
    int lport, queue, rv;
    const bcmlrd_map_t *map = NULL;
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        ...
        for (queue = 0; queue < bcm56880_a0_tm_num_uc_q_non_cpu_port_get(unit);
                queue++) {
            SHR_IF_ERR_EXIT
