/*! \file bcm56880_a0_tm_mmu_flexport.c
 *
...
#include "bcm56880_a0_tm_mmu_flexport.h"
...
/*! @fn int td4_mmu_get_thdo_bst_mode(int unit, int *thdo_bst_mode)
 *  @param unit Device number
 *  @param *thdo_bst_mode Return value thdo bst mode
 *  @brief API to return the thdo bst mode programmed
 */
static int
td4_mmu_get_thdo_bst_mode(int unit, uint32_t *thdo_bst_mode)
{
    int inst = 0;
    uint32_t ltmbuf = 0;
    uint32_t fval;
...
/*!
 * @fn int td4_mmu_oqs_reconfig(int unit, int lport)
 *  @param unit Device number
 *  @param lport Logical/device port number
 *  @brief API to update the OQS during port speed reconfiguration
 */
static int
td4_mmu_oqs_reconfig(int unit, int lport)
{
    int inst, mmu_port;

    uint32_t ltmbuf;
    uint32_t port_speed_new;
    ...
    sid = MMU_OQS_AGER_DST_PORT_MAPr;

    mmu_port = port_map->lport_map[lport].mport;
    inst = mmu_port;

    if (port_map->lport_map[lport].max_speed >= 400000) {
        port_speed_new = 3;
    } else if (port_map->lport_map[lport].max_speed >= 200000) {
        port_speed_new = 2;
    } else if (port_map->lport_map[lport].max_speed >= 100000) {
        port_speed_new = 1;
    } else if (port_map->lport_map[lport].max_speed >= 1000) {
        port_speed_new = 0;
    } else {
...
/*!
 * @fn int td4_mmu_set_mmu_port_mappings(int unit,
 *                                       int lport, int is_port_down)
 *  @param unit Device number
 *  @param lport Port number
 *  @param is_port_down Port add/remove status
 *  @brief API to update the MMU port mappings for various blocks
 *         during port add and port delete operations in flexport.
 */
static int
td4_mmu_set_mmu_port_mappings(int unit, int lport, int is_port_down)
{
    bcmtm_port_map_info_t *port_map;
    int mmu_port, pport, mmu_chip_port;
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_sid_t sid ;
    ...
    SHR_IF_ERR_EXIT(td4_tdm_global_to_local_phy_num(pport, &local_phys_port));
    /*
 ...
 */
    for (sid_ind = 0; sid_ind < COUNTOF(sid_list); sid_ind++) {
        ...
        switch (sid) {
            case MMU_CRB_DEVICE_PORT_TO_MMU_PORT_MAPPINGr:
                ...
                inst = lport;
                index = 0;
                fval = (is_port_down) ? TD4_INVALID_MMU_PORT : mmu_port;
                break;
            case MMU_EBCFP_MMU_PORT_TO_DEVICE_PORT_MAPPINGr:
                fid = DEVICE_PORTf;
                inst = mmu_port;
                index = 0;
                fval = (is_port_down)  ? TD4_INVALID_DEV_PORT : lport;
                break;
            ...
            case MMU_THDO_DEVICE_PORT_MAPm:
                ...
                index = mmu_chip_port;
                inst = 0;
                fval = (is_port_down) ? TD4_THDO_INVALID_DEV_PORT : lport;
                break;
            case MMU_INTFI_MMU_PORT_TO_PHY_PORT_MAPPINGr:
                fid = PHY_PORT_NUMf;
                inst = mmu_port;
                index = 0;
                fval = (is_port_down) ? TD4_INVALID_LOCAL_PHYS_PORT
                                      : local_phys_port;
                break;
    ...
    if (bcmtm_lport_is_fp(unit, lport)) {
        ...
        fid = PHY_PORT_NUMf;
        inst = mmu_port;
        fval = (is_port_down) ? TD4_INVALID_PHYS_PORT : pport;
        ltmbuf = 0;
...
/*!
 * @fn int td4_mmu_qsch_port_flush(int unit,
 ...
 */
static int
td4_mmu_qsch_port_flush(int unit, int lport, uint32_t set_val)
{
    uint32_t temp;
    uint32_t ltmbuf;
    uint32_t new_val;
...
/*!
 * @fn int td4_mmu_ebqs_port_flush(int unit,
 *                                  int lport, uint32_t set_val)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
td4_mmu_ebqs_port_flush(int unit, int lport, uint32_t set_val)
{
    int inst;
    int phy_port;
    ...
    new_val = 1;
    lcl_mmu_port = mmu_port % TD4_MMU_PORT_PIPE_OFFSET;
    temp = TD4_MMU_FLUSH_ON;
    new_val <<= lcl_mmu_port;
...
/*!
 * @fn int td4_mmu_rqe_port_flush(int unit, int itm)
 *  @param unit Device number
 ...
 */
static int
td4_mmu_rqe_port_flush(int unit, int itm)
{
    int inst;
    int count = 0;
    uint32_t lbuf;
...
/*!
 * @fn int td4_mmu_wait_ebctm_empty(int unit,
 *                                   int lport)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
td4_mmu_wait_ebctm_empty(int unit, int lport)
{
    uint32_t ltmbuf;
    uint32_t empty_val;
    int inst, count = 0;
...
/*!
 * @fn int td4_mmu_mtro_port_metering_config(int unit,
 *                                           int lport,
 ...
 */
static int
td4_mmu_mtro_port_metering_config(int unit,
                                  int lport,
                                  uint32_t set_val_mtro)
{
...
/*!
 * @fn int td4_mmu_clear_mtro_bucket_mems(int unit,
 *                                        int lport)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
td4_mmu_clear_mtro_bucket_mems(int unit, int lport)
{
    int cos;
    int lcl_port;
    int pipe_number;
    ...
    lcl_port = port_map->lport_map[lport].mlocal_port;
    ...
    /* L0 Bucket Memory */
    for (cos = 0; cos < TD4_MMU_NUM_COS; cos++) {

        mem_indx = (lcl_port * TD4_MMU_NUM_COS) + cos;
        BCMTM_PT_DYN_INFO(pt_dyn_info, mem_indx, pipe_number);
...
/*!
 * @fn int td4_mmu_thdi_bst_clr(int unit,
 *                               int lport)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
td4_mmu_thdi_bst_clr(int unit, int lport)
{
    int pipe_number;
    int local_mmu_port;
    ...
    phy_port = port_map->lport_map[lport].pport;
    mmu_port = port_map->pport_map[phy_port].mport;
    local_mmu_port = mmu_port % TD4_MMU_PORT_PIPE_OFFSET;
    pipe_number = port_map->lport_map[lport].pipe;
...
td4_mmu_thdo_bst_clr_queue(int unit, int lport)
{
    ...
    num_queues = port_map->lport_map[lport].num_mcq +
...
/*! @fn int td4_mmu_thdo_bst_clr_port(int unit,
 ...
 */
static int
td4_mmu_thdo_bst_clr_port(int unit, int global_device_port)
{
    int itm;
    uint32_t data[BCMTM_MAX_ENTRY_WSIZE];
    bcmdrd_sid_t mem;
...
/*! @fn int td4_mmu_thdo_bst_clr_portsp(int unit,
 *                                   int pipe, int global_device_port)
 *  @param int unit Chip unit number
 *  @param int pipe Egress pipe in which port exists
 *  @param int global_device_port global device port
 ...
 */
static int
td4_mmu_thdo_bst_clr_portsp(int unit, int global_device_port)
{
    int itm;
    int sp;
    int mem_idx;
    ...
    for (sp = 0; sp < TD4_MMU_NUM_SPS; sp++) {
...
/*! @fn int td4_mmu_thdo_bst_clr(int unit,
 ...
 */
static int
td4_mmu_thdo_bst_clr(int unit, int lport)
{
    ...
    SHR_IF_ERR_EXIT
        (td4_mmu_thdo_bst_clr_queue(unit, lport));
    /* Clear per-port memories. */
    SHR_IF_ERR_EXIT
        (td4_mmu_thdo_bst_clr_port(unit, lport));
    /* Clear per-port-per-spid memories. */
    SHR_IF_ERR_EXIT
        (td4_mmu_thdo_bst_clr_portsp(unit, lport));
...
/*! @fn int td4_mmu_thdo_pktstat_clr_queue(int unit, int pipe,
 *              int global_mmu_port)
 *  @param int unit Chip unit number
 ...
 */
static int
td4_mmu_thdo_pktstat_clr_queue(int unit, int lport)
{
    int itm;
    int curr_idx;
    int num_queues;
...
/*! @fn int td4_mmu_thdo_pktstat_clr_port(int unit, int pipe,
 ...
 */
static int
td4_mmu_thdo_pktstat_clr_port(int unit, int global_device_port)
{
    int itm;
    ...
    local_device_port = global_device_port % TD4_MMU_PORTS_PER_PIPE;
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm++) {
        if (SHR_SUCCESS(bcm56880_a0_tm_check_valid_itm(unit, itm))) {
...
/*! @fn int td4_mmu_thdo_pktstat_clr(int unit,
 *              port_resource_t *port_resource_t)
 *  @param int unit Chip unit number
 *  @param port_resource_t *port_resource_t Port resource struct containing
 *         epipe and port info
 ...
 */
static int
td4_mmu_thdo_pktstat_clr(int unit, int lport)
{
    bcmtm_drv_info_t *drv_info;
    ...
    /* Clear per-queue memories. */
    SHR_IF_ERR_EXIT
        (td4_mmu_thdo_pktstat_clr_queue(unit, lport));
    /* Clear per-port memories. */
    SHR_IF_ERR_EXIT
        (td4_mmu_thdo_pktstat_clr_port(unit, lport));
exit:
    SHR_FUNC_EXIT();
}

/*! @fn int td4_mmu_clear_qsch_credit_memories(int unit,
 *              port_resource_t *port_resource_t)
 *  @param unit Device number
 ...
 */
static int
td4_mmu_clear_qsch_credit_memories(int unit, int lport)
{
    int itm;
    int lcl_port;
    int pipe_number;
    ...
    for (schq = 0; schq < TD4_MMU_NUM_L0_NODES_PER_HSP_PORT; schq++) {

        mem_indx = (lcl_port * TD4_MMU_NUM_L0_NODES_PER_HSP_PORT) + schq;
    ...
    for (schq = 0; schq < TD4_MMU_NUM_L0_NODES_PER_HSP_PORT; schq++) {
        for (itm = 0; itm < TD4_ITMS_PER_DEV; itm++) {
            if (SHR_SUCCESS(bcm56880_a0_tm_check_valid_itm(unit, itm))) {
                mem_indx =
                          (((lcl_port * TD4_MMU_NUM_L0_NODES_PER_HSP_PORT) +
                            schq) * 2) + itm;
...
/*! @fn int td4_mmu_wred_clr(int unit,
 *              port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
static int
td4_mmu_wred_clr(int unit, int lport)
{
    int q, sp;
    int itm;
    ...
    /* Per-UCQ loop */
    for (q = 0; q < TD4_MMU_NUM_Q_PER_HSP_PORT; q++) {

        cosq_mem_index = (lport * TD4_MMU_NUM_Q_PER_HSP_PORT) + q;
    ...
    for (sp = 0; sp < TD4_MMU_NUM_SPS; sp++) {
        ...
        portsp_mem_index = (lport * TD4_MMU_NUM_SPS) + sp;
...
/*!
 * @fn int td4_mmu_port_speed_reconfigure(int unit,
 *                                        int lport)
 *  @param unit Device number
 *  @param lport Port number
 *  @brief API to conifgure port speed in different MMU blocks when a new port
 *  is added.
 */

static int
td4_mmu_port_speed_reconfigure(int unit, int lport) {
    bcmtm_port_map_info_t *port_map;
    int pport;
    int mmu_port;
    bcmtm_pt_info_t pt_dyn_info = {0};
    ...
    SHR_IF_ERR_EXIT(td4_mmu_get_speed_decode(unit,
                                             port_map->lport_map[lport].max_speed,
                                             &fval));
    LOG_VERBOSE(BSL_LOG_MODULE,
              (BSL_META_U(unit,
    ...
    SHR_IF_ERR_EXIT
        (td4_mmu_oqs_reconfig(unit, lport));
exit:
    SHR_FUNC_EXIT();
}

static int
td4_mmu_stat_clear(int unit, int lport) {
    uint32_t thdi_bst_mode = 0;
    uint32_t thdo_bst_mode = 0;

    SHR_FUNC_ENTER(unit);
    td4_mmu_get_thdi_bst_mode(unit, &thdi_bst_mode);
    td4_mmu_get_thdo_bst_mode(unit, &thdo_bst_mode);
    /* Clear MTRO bucket memories. */
    LOG_DEBUG(BSL_LS_SOC_PORT,
              (BSL_META_U(unit,
                          "before td4_mmu_clear_mtro_bucket_mems\n")));
    SHR_IF_ERR_EXIT
        (td4_mmu_clear_mtro_bucket_mems(unit, lport));

    /* Clear QSCH credit memories. */
    LOG_DEBUG(BSL_LS_SOC_PORT,
              (BSL_META_U(unit,
                          "td4_mmu_clear_qsch_credit_memories\n")));
    SHR_IF_ERR_EXIT
        (td4_mmu_clear_qsch_credit_memories(unit, lport));

    /* Clear WRED Avg_Qsize instead of waiting for background process. */
    LOG_DEBUG(BSL_LS_SOC_PORT,
              (BSL_META_U(unit, "before td4_mmu_wred_clr\n")));
    SHR_IF_ERR_EXIT
        (td4_mmu_wred_clr(unit, lport));

    if (thdi_bst_mode == TD4_MMU_BST_WMARK_MODE) {
        /* Clear THDI BST in watermark mode. */
        LOG_DEBUG(BSL_LS_SOC_PORT,
                  (BSL_META_U(unit, "before td4_mmu_thdi_bst_clr\n")));
        SHR_IF_ERR_EXIT
            (td4_mmu_thdi_bst_clr(unit, lport));
    }
    if (thdo_bst_mode == TD4_MMU_BST_WMARK_MODE) {
        /* Clear THDO BST counters in watermark mode. */
        LOG_DEBUG(BSL_LS_SOC_PORT,
                  (BSL_META_U(unit, "before td4_mmu_thdo_bst_clr\n")));
        SHR_IF_ERR_EXIT
            (td4_mmu_thdo_bst_clr(unit, lport));

        /* Clear PktStat counters in THDO for Queues. */
        SHR_IF_ERR_EXIT
            (td4_mmu_thdo_pktstat_clr(unit, lport));
    }
exit:
    SHR_FUNC_EXIT();
}
...
/*!
 *  @fn int td4_flex_mmu_port_flush(int unit,
 *                                  int lport)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
td4_flex_mmu_port_flush(int unit, int lport)
{
    int itm;
    ...
    SHR_IF_ERR_EXIT
        (td4_mmu_qsch_port_flush(unit, lport, TD4_MMU_FLUSH_ON));

    SHR_IF_ERR_EXIT
        (td4_mmu_ebqs_port_flush(unit, lport, TD4_MMU_FLUSH_ON));

    LOG_DEBUG(BSL_LS_SOC_PORT,
              (BSL_META_U(unit, "before mtro_port_metering_config\n")));
    SHR_IF_ERR_EXIT
        (td4_mmu_mtro_port_metering_config(unit, lport, TD4_MMU_FLUSH_ON));
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm++) {
        if (SHR_SUCCESS(bcm56880_a0_tm_check_valid_itm(unit, itm))) {
            ...
            SHR_IF_ERR_EXIT
                (td4_mmu_rqe_port_flush(unit, itm));
        }
    }
    LOG_DEBUG(BSL_LS_SOC_PORT,
    ...
    SHR_IF_ERR_EXIT
        (td4_mmu_wait_ebctm_empty(unit, lport));
    ...
    SHR_IF_ERR_EXIT
        (td4_mmu_mtro_port_metering_config(unit, lport, TD4_MMU_FLUSH_OFF));

    SHR_IF_ERR_EXIT
        (td4_mmu_qsch_port_flush(unit, lport, TD4_MMU_FLUSH_OFF));

    SHR_IF_ERR_EXIT
        (td4_mmu_ebqs_port_flush(unit, lport, TD4_MMU_FLUSH_OFF));
...
static int
td4_port_mmu_add(int unit, int lport)
{
    ...
    /* Add UP ports */
    SHR_IF_ERR_EXIT
        (td4_mmu_tdm_port_up(unit, lport_bmp));
    SHR_IF_ERR_EXIT
        (td4_mmu_set_mmu_port_mappings(unit, lport, 0));
    SHR_IF_ERR_EXIT
        (td4_mmu_port_speed_reconfigure(unit, lport));
    /* Disable metering before clearing bucket mems */
    SHR_IF_ERR_EXIT
        (td4_mmu_mtro_port_metering_config(unit, lport, TD4_MMU_FLUSH_ON));
    SHR_IF_ERR_EXIT
        (td4_mmu_stat_clear(unit, lport));
    /* Reenable metering */
    SHR_IF_ERR_EXIT
        (td4_mmu_mtro_port_metering_config(unit, lport, TD4_MMU_FLUSH_OFF));
...
bcm56880_a0_tm_port_mmu_down(int unit, bcmtm_pport_t pport)
{
    ...
    if (lport != -1) {
        if (!sim) {
            SHR_IF_ERR_EXIT
                (td4_flex_mmu_port_flush(unit, lport));
        }
    }
...
bcm56880_a0_tm_port_mmu_delete(int unit, bcmtm_pport_t pport)
{
    ...
    if (lport != -1) {
        lport_bmp[lport] = 1;
        SHR_IF_ERR_EXIT
            (td4_mmu_tdm_port_down(unit, lport_bmp));
        SHR_IF_ERR_EXIT
            (td4_mmu_set_mmu_port_mappings(unit, lport, 1));
    }
