...
static int
td4_idb_init_idb_to_dev_pmap(int unit, int pipe)
{
    bcmdrd_sid_t mem;
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_fid_t fid ;
    ...
    for (local_dev_port = 0; local_dev_port < TD4_DEV_PORTS_PER_PIPE;
         local_dev_port++) {
        if (pipe == 0){
...
td4_idb_general_init(int unit, int pipe_num)
{
    ...
    SHR_IF_ERR_EXIT(td4_idb_init_idb_to_dev_pmap(unit,pipe_num));
