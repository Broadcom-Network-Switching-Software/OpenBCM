/*! \file bcm56880_a0_tm_ing_thd_port_pri_grp.c
 *
                                       ...
                                       soc_mmu_cfg_buf_t *buf)
{
    ...
    if ((flags & CHIP_FLAG_BW8T) == CHIP_FLAG_BW8T) {
        buffer_size = BCM56883_A0_MMU_TOTAL_SHARED_PER_ITM;
    } else {
        buffer_size = BCM56880_A0_MMU_TOTAL_SHARED_PER_ITM;
    }
    /*
                               ...
                               uint8_t action)
{
    ...
    bcmdrd_fid_t field_pgshared_limit[TD4_MMU_NUM_PG] = {
    ...
    bcmdrd_fid_t field_pgshared_dynamic[TD4_MMU_NUM_PG] = {
    ...
    bcmdrd_fid_t field_pgreset_floor[TD4_MMU_NUM_PG] = {
    ...
    bcmdrd_fid_t field_pgreset_offset[TD4_MMU_NUM_PG] = {
    ...
    bcmdrd_fid_t field_bst_hdrm_profile[TD4_MMU_NUM_PG] = {
    ...
    bcmdrd_fid_t field_bst_shared_profile[TD4_MMU_NUM_PG] = {
    ...
    while (data_ptr) {
        if (data_ptr->id == TM_ING_THD_PORT_PRI_GRPt_MIN_GUARANTEE_CELLSf) {
            ...
            SHR_IF_ERR_EXIT
                (bcm56880_a0_tm_thdi_pg_to_pool_mapping_get(unit, lport, pg,
                                                            &itm, &pool, FALSE));
...
static int
tm_thd_ing_port_pri_grp_delete(int unit, bcmltd_sid_t sid, int lport, int pg)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_thd_ing_port_pri_grp_update(unit, sid, lport, pg, data_fields,
                                        &dummy_fields, FALSE));
                              ...
                              bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_thd_ing_port_pri_grp_delete(unit, sid, lport, pg));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
int
bcm56880_a0_tm_thd_ing_port_pri_grp_register(int unit)
{
...
int
bcm56880_a0_tm_thd_ing_port_pri_grp_populate(int unit)
{
    int lport;
    int pg, rv;
    const bcmlrd_map_t *map = NULL;
    ...
    for (lport = 0; lport < TD4_MMU_NUM_LPORT; lport++) {
        td4_mmu_config_thdi_set(unit, lport, buf, &devcfg, drv_info->lossless);
        for (pg = 0; pg < TD4_MMU_NUM_PG; pg++) {
            SHR_IF_ERR_EXIT
...
int
bcm56880_a0_tm_thd_ing_port_pri_grp_update(int unit, int pport, int up)
{
    int lport, pg, rv;
    const bcmlrd_map_t *map = NULL;
