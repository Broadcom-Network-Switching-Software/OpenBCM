/*! \file bcm56880_a0_tm_thd.c
 *
 * File containing mmu threshold init sequence for bcm56880_a0
 */
/*
...
td4_mmu_init_dev_config(int unit, soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    devcfg->max_pkt_byte = TD4_MMU_MAX_PACKET_BYTES;
    devcfg->mmu_hdr_byte = TD4_MMU_PACKET_HEADER_BYTES;
    devcfg->jumbo_pkt_size = TD4_MMU_JUMBO_FRAME_BYTES;
    devcfg->default_mtu_size = TD4_MMU_DEFAULT_MTU_BYTES;
    devcfg->mmu_cell_size = TD4_MMU_BYTES_PER_CELL;

    flags = bcmdrd_dev_flags_get(unit);
    if ((flags & CHIP_FLAG_BW8T) == CHIP_FLAG_BW8T ) {
        /* BCM56883_A0 */
        devcfg->mmu_total_cell = BCM56883_A0_MMU_TOTAL_CELLS_PER_ITM;
    } else {
        /* BCM56880_A0 */
        devcfg->mmu_total_cell = BCM56880_A0_MMU_TOTAL_CELLS_PER_ITM;
    }
    devcfg->num_pg = TD4_MMU_NUM_PG;
    devcfg->num_service_pool = TD4_MMU_NUM_POOL;
    devcfg->flags = MMU_CFG_F_PORT_MIN | MMU_CFG_F_PORT_POOL_MIN |
                    MMU_CFG_F_RQE | MMU_CFG_F_EGR_MCQ_ENTRY;
    devcfg->total_mcq_entry = TD4_MMU_TOTAL_MCQ_ENTRY(unit);
    devcfg->rqe_queue_num = TD4_MMU_NUM_RQE_QUEUES;
}
...
td4_default_pg_headroom(int unit, int port)
{
    ...
    if (speed >= 400000) {
        hdrm = TD4_PG_HEADROOM_OVERSUB_400G;
    } else if (speed >= 200000) {
        hdrm = TD4_PG_HEADROOM_OVERSUB_200G;
    } else if (speed >= 100000) {
        hdrm = TD4_PG_HEADROOM_OVERSUB_100G;
    } else if (speed >= 40000) {
        hdrm = TD4_PG_HEADROOM_OVERSUB_50G;
    } else if (speed >= 25000) {
        hdrm = TD4_PG_HEADROOM_OVERSUB_25G;
    } else if (speed >= 1000) {
        hdrm = TD4_PG_HEADROOM_OVERSUB_10G;
    } else {
        hdrm = TD4_PG_HEADROOM_OVERSUB_10G;
...
static int
td4_get_shared_limit(int unit, soc_mmu_cfg_buf_t *buf, int pool_id)
{
    ...
    if ((flags & CHIP_FLAG_BW8T) == CHIP_FLAG_BW8T ) {
        /* BCM56883_A0 */
        limit = BCM56883_A0_MMU_TOTAL_CELLS_PER_ITM;
    } else {/* BCM56880_A0 */
        limit = BCM56880_A0_MMU_TOTAL_CELLS_PER_ITM;
    }
...
static int
td4_get_hdrm_limit(int unit, soc_mmu_cfg_buf_t *buf, int pool_id)
{
    soc_mmu_cfg_buf_pool_t *buf_pool;
    int hdrm, hdrm_itm[TD4_ITMS_PER_DEV];
    int itm;
...
static void
td4_mmu_config_mapping_profiles_set(int unit, soc_mmu_cfg_buf_t *buf,
                                    soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (profile_idx = 0; profile_idx < TD4_MMU_NUM_PROFILES;
            profile_idx++) {
        buf_map_profiles = &buf->mapprofiles[profile_idx];
        for (idx = 0; idx < TD4_MMU_NUM_INT_PRI; idx++) {
            if (lossless == 2) {
                /* Lossy+Lossless mode */
                if (idx == 0) {
                    inpri2pg = 7;
        ...
        for (idx = 0; idx < TD4_MMU_NUM_PG; idx++) {
...
static int
td4_mmu_config_mapping_profiles_set_hw(int unit, soc_mmu_cfg_buf_t *buf,
                                       soc_mmu_device_info_t *devcfg, int lossless)
{
    soc_mmu_cfg_buf_mapping_profile_t *buf_map_profiles;
    int idx, loop;
    ...
    bcmdrd_fid_t field_pri[TD4_MMU_NUM_INT_PRI] = {
            ...
            INPPRI15_PGf};
    bcmdrd_fid_t field_hpid[TD4_MMU_NUM_PG] = {
            PG0_HPIDf, PG1_HPIDf, PG2_HPIDf,
            PG3_HPIDf, PG4_HPIDf, PG5_HPIDf,
            PG6_HPIDf, PG7_HPIDf};
    bcmdrd_fid_t field_spid[TD4_MMU_NUM_PG] = {
            PG0_SPIDf, PG1_SPIDf, PG2_SPIDf,
            PG3_SPIDf, PG4_SPIDf, PG5_SPIDf,
            PG6_SPIDf, PG7_SPIDf};
    bcmdrd_fid_t field_pfcpri[TD4_MMU_NUM_PG] = {
    ...
    for (idx = 0; idx <= pt_index_max(unit, sid); idx++) {
        ...
        for (loop = 0; loop < TD4_MMU_NUM_INT_PRI; loop++) {
    ...
    for (idx = 0; idx <= pt_index_max(unit, sid); idx++) {
        ...
        sal_memset(ltmbuf, 0, sizeof(ltmbuf));
        for (loop = 0; loop < TD4_MMU_NUM_PG; loop++) {
                        ...
                        soc_mmu_device_info_t *devcfg,  int lossless)
{
        ...
        if (idx == TD4_MMU_NUM_PG - 1) {
            if (drv_info->skip_buf_resv == 0) {
                ...
                buf_prigroup->headroom = (lossless == 0) ? 0 :
                                ...
                                td4_default_pg_headroom(unit, port) : 0);
            } else {
                buf_prigroup->guarantee = 0;
                buf_prigroup->headroom = 0;
            }
    ...
    for (idx = 0; idx < TD4_MMU_NUM_POOL; idx++) {
                        ...
                        soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    buf_port = &buf->ports[port];
    if (bcmtm_lport_is_cpu(unit, port)) {
        num_cosq = TD4_NUM_CPU_QUEUES;
    } else {
        num_cosq = TD4_NUM_GP_QUEUES;
    }
...
static void
td4_mmu_config_thresholds_pool_set(int unit, soc_mmu_cfg_buf_t *buf,
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    bcmtm_drv_info_t *drv_info;
    soc_mmu_cfg_buf_pool_t *buf_pool;
    ...
    int itm, pipe, idx;
    int pm;
    int total_headroom_itm[TD4_ITMS_PER_DEV] = {0};
    int total_pgmin_itm[TD4_ITMS_PER_DEV] = {0};
    int total_qgrpmin_itm[TD4_ITMS_PER_DEV] = {0};
    int total_qmin_itm[TD4_ITMS_PER_DEV] = {0};
    int total_num_ports = 0;
    int ports_in_pipe[TD4_PIPES_PER_DEV] = {0};
    int ports_in_pm[TD4_PBLKS_PER_DEV] = {0};
    int itm_pipe_map;
    uint32_t flags;
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm ++) {
       for (idx = 0; idx < TD4_MMU_NUM_POOL; idx++) {
            ...
            if (idx == 0) {
                /* 100% scale up by 100 */
                buf_pool->total_mcq_entry = TD4_MMU_MCQ_ENTRY_PER_ITM;
                if ((flags & CHIP_FLAG_BW8T) == CHIP_FLAG_BW8T) {
                    /* BCM56883 */
                    buf_pool->total = BCM56883_A0_MMU_TOTAL_CELLS_PER_ITM;
                } else {
                    /* BCM56880 */
                    buf_pool->total = BCM56880_A0_MMU_TOTAL_CELLS_PER_ITM;
                }
                /* 100 Percent */
        ...
        if ((lossless == 1) || (lossless == 2)) {
            if (drv_info->skip_buf_resv == 0) {
                ...
                for (pipe = 0; pipe < TD4_PIPES_PER_DEV; pipe++) {
                    if (itm_pipe_map & (1 <<pipe)) {
                        for (pm = pipe * TD4_PBLKS_PER_PIPE;
                                pm < (pipe + 1) * TD4_PBLKS_PER_PIPE;
                                pm++) {
                            ports_in_pm[pm] = drv_info->flex_eligible[pm];
                            if (ports_in_pm[pm] > 4) {
                                /* Treat as 8x50G PM. */
                                total_headroom_itm[itm] += TD4_PG_HEADROOM_OVERSUB_50G * 8;
                            } else if (ports_in_pm[pm] > 2) {
                                /* Treat as 4x100G PM. */
                                total_headroom_itm[itm] += TD4_PG_HEADROOM_OVERSUB_100G * 4;
                            } else if (ports_in_pm[pm] == 2) {
                                /* Treat as 2x200G PM. */
                                total_headroom_itm[itm] += TD4_PG_HEADROOM_OVERSUB_200G * 2;
                            } else if (ports_in_pm[pm] == 1) {
                                /* Treat as 1x400G PM. */
                                total_headroom_itm[itm] += TD4_PG_HEADROOM_OVERSUB_400G;
                            }
            ...
            total_num_ports += TD4_NUM_CPU_PORTS +
                (TD4_NUM_LB_PORTS + TD4_NUM_MGMT_PORTS) / TD4_ITMS_PER_DEV;
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm ++) {
        ...
        if (drv_info->skip_buf_resv == 0) {
            for (pipe = 0; pipe < TD4_PIPES_PER_DEV; pipe++) {
                for (pm = pipe * TD4_PBLKS_PER_PIPE;
                     pm < (pipe + 1) * TD4_PBLKS_PER_PIPE; pm++) {
                    ports_in_pm[pm] = drv_info->flex_eligible[pm];
                    ports_in_pipe[pipe] += ports_in_pm[pm];
                }
                /* Each pipe cannot have more than 18 ports. */
        ...
        total_qgrpmin_itm[itm] = (total_num_ports + TD4_NUM_LB_PORTS +
                                  TD4_NUM_MGMT_PORTS) * (2 * default_mtu_cells);

        /* qmin for cpu * number of cpu queues */
        total_qmin_itm[itm] = default_mtu_cells * TD4_NUM_CPU_QUEUES;
    }
...
static void
td4_mmu_config_thresholds_set(int unit, soc_mmu_cfg_buf_t *buf,
                              soc_mmu_device_info_t *devcfg, int lossless)
{
    int port;

    for (port = 0; port < TD4_MMU_NUM_LPORT; port++) {
        td4_mmu_config_thdi_set(unit, port, buf, devcfg, lossless);
        td4_mmu_config_thdo_set(unit, port, buf, devcfg, lossless);
    }

    td4_mmu_config_thresholds_pool_set(unit, buf, devcfg, lossless);
}


static int
td4_mmu_config_thdi_per_itm_set_hw(int unit, soc_mmu_cfg_buf_t *buf,
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    uint32_t fval;
    bcmtm_pt_info_t pt_dyn_info = {0};
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm ++) {
        if (bcm56880_a0_tm_check_valid_itm(unit, itm) == SHR_E_NONE) {
            ...
            /* BCM56883_A0 */
            if ((flags & CHIP_FLAG_BW8T) == CHIP_FLAG_BW8T) {
                fval = BCM56883_A0_MMU_PHYSICAL_CELLS_PER_ITM;
            } else { /* BCM56880_A0*/
                fval = BCM56880_A0_MMU_PHYSICAL_CELLS_PER_ITM;
            }
            fval-= TD4_MMU_RSVD_CELLS_CFAP_PER_ITM;
    ...
    ltmbuf = 0;
    fval = TD4_SPID_OVERRIDE_ENABLE_DEFAULT;
    SHR_IF_ERR_EXIT
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm ++) {
        if (bcm56880_a0_tm_check_valid_itm(unit, itm) == SHR_E_NONE) {
            ...
            for (idx = 0; idx < TD4_MMU_NUM_POOL; idx++) {
                ...
                limit = td4_get_shared_limit(unit, buf, idx);
                headroom = td4_get_hdrm_limit(unit, buf, idx);
                ...
                ltmbuf = 0;
                fval = TD4_SPAP_YELLOW_OFFSET_DEFAULT;
                SHR_IF_ERR_EXIT
                ...
                ltmbuf = 0;
                fval = TD4_SPAP_RED_OFFSET_DEFAULT;
                SHR_IF_ERR_EXIT
...
static int
td4_mmu_config_thdo_per_itm_set_hw(int unit, soc_mmu_cfg_buf_t *buf,
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    soc_mmu_cfg_buf_pool_t *buf_pool;
    uint32_t fval;
...
soc_mmu_cfg_buf_t *soc_mmu_buf_cfg_alloc(int unit)
{
...
buf_size_tracking_init(int unit, soc_mmu_cfg_buf_t *buf)
{
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm ++) {
        buf_size = 0;
        for (pool = 0; pool < TD4_MAX_SERVICE_POOLS; pool++) {
...
bcm56880_a0_tm_mmu_config_init_thresholds(int unit)
{
    ...
    /* Default config */
    td4_mmu_config_mapping_profiles_set(unit, buf, &devcfg, drv_info->lossless);
    td4_mmu_config_thresholds_set(unit, buf, &devcfg, drv_info->lossless);
    SHR_IF_ERR_EXIT(
            td4_mmu_config_mapping_profiles_set_hw(unit, buf, &devcfg,
                drv_info->lossless));
    SHR_IF_ERR_EXIT
        (td4_mmu_config_thdi_per_itm_set_hw(unit, buf, &devcfg, drv_info->lossless));
    SHR_IF_ERR_EXIT
        (td4_mmu_config_thdo_per_itm_set_hw(unit, buf, &devcfg, drv_info->lossless));
...
bcm56880_a0_tm_rqe_threshold_init(int unit) {
    ...
    SHR_FUNC_ENTER(unit);
    for (idx = 0; idx < TD4_MMU_NUM_RQE_QUEUES; idx++) {
        reg = MMU_THDR_QE_CONFIG1_PRIQr;
        ltmbuf = TD4_THDR_QE_CONFIG1_VALUE;
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, reg, ltid,
                       (void*)&pt_dyn_info, &ltmbuf));
        fval = TD4_MMU_THDR_QE_RESET_OFFSET;
        SHR_IF_ERR_EXIT
        ...
        fval = TD4_THDR_QE_CONIFG_PRI_LIMIT;
        SHR_IF_ERR_EXIT
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, reg, ltid,
                      (void*)&pt_dyn_info, &ltmbuf));
    fval = TD4_THDR_QE_SP_SHARED_LIMIT;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, SHARED_LIMITf, &ltmbuf, &fval));
    fval = TD4_THDR_QE_SP_RESUME_LIMIT;
    SHR_IF_ERR_EXIT
                                    ...
                                    bool check_only)
{
    ...
    uint32_t pool_shared_limit[TD4_MAX_SERVICE_POOLS] = {0};
    uint32_t new_limit_array[TD4_MAX_SERVICE_POOLS] = {0};
    bcmtm_thd_info_t *thd_info;
    ...
    for (pool_idx = 0; pool_idx < TD4_MAX_SERVICE_POOLS; pool_idx++) {
        ...
        if (pool == -1) {
            ...
            if ((long int) thd_info->itm_buf_size[itm] + delta >= 0) {
                ...
                if (delta < 0 && (long int) unused_buf + delta >= 0) {
                    ...
                    remaining = new_limit;
                    for (pool_idx = TD4_MAX_SERVICE_POOLS - 1;
                         pool_idx > 0;
                         pool_idx--) {
    ...
    if (pool == -1) {
        ...
        pool_start = 0;
        pool_end = TD4_MAX_SERVICE_POOLS - 1;
    } else {
    ...
    for (; pool_start <= pool_end; pool_start++) {
        ...
        if (reason != ING_SERVICE_POOL_UPDATE) {
             ...
             */
            SHR_IF_ERR_EXIT
                (bcm56880_a0_tm_ing_pool_shared_limit_lt_update(unit, itm, pool_start,
                                                                new_limit));
        }
        ...
        if (reason != EGR_SERVICE_POOL_UPDATE) {
             ...
             */
            SHR_IF_ERR_EXIT
                (bcm56880_a0_tm_egr_pool_shared_limit_lt_update(unit, itm, pool_start,
                                                                new_limit));
        }
    }
