/*! \file bcm56880_a0_tm_egr_thd_service_pool.c
 *
 * IMM handlers for threshold LTs.
 *
 * This file contains callbcak handlers for LT TM_EGR_THD_SERVICE_POOL.
 */
/*
    ...
    while (data_ptr) {
        switch (data_ptr->id) {
            ...
            case TM_EGR_THD_SERVICE_POOLt_SHARED_LIMIT_CELLSf:
                ...
                rv = bcm56880_a0_tm_shared_buffer_update(unit, itm, pool, delta,
...
static int
tm_thd_egr_service_pool_delete(int unit, bcmltd_sid_t sid, int itm, int pool)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_thd_egr_service_pool_update(unit, sid, itm, pool, data_fields,
                                        &dummy_fields));
                              ...
                              bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT
                (tm_thd_egr_service_pool_update(unit, sid, itm, pool,
                                                data_fields, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_thd_egr_service_pool_delete(unit, sid, itm, pool));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
int
bcm56880_a0_tm_thd_egr_service_pool_register(int unit)
{
    ...
    tm_thd_cb_handler.stage    = tm_thd_egr_service_pool_stage;
...
int
bcm56880_a0_tm_thd_egr_service_pool_populate(int unit)
{
    int itm, rv;
    int pool;
    const bcmlrd_map_t *map = NULL;
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm++) {
        if (bcm56880_a0_tm_check_valid_itm(unit, itm) == SHR_E_NONE) {
            for (pool = 0; pool < TD4_MMU_NUM_POOL; pool++) {
                SHR_IF_ERR_EXIT
