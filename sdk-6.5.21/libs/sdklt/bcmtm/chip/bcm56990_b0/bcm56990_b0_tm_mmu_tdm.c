 ...
 *
 * DV file: $DV/tomahawk4/dv/ngsdk_interface/flexport/bcm56990_b0_tm_mmu_tdm.c
 */
/*
...
static const struct th4_b0_eb_sched_min_cell_spacing_within_pkt_s {
    int val_50g;
    int val_100g;
    int val_200g;
    int val_400g;
} th4_b0_eb_sched_min_cell_spacing_within_pkt[3] = {
    {9, 9, 8, 3}, /* 1325 MHz */
    {9, 9, 4, 3}, /* 1025 MHz */
    {9, 9, 4, 3}  /*  950 MHz */
};

static const struct th4_b0_eb_sched_min_cell_spacing_eop_to_sop_s {
    int val_50g;
    int val_100g;
    int val_200g;
    int val_400g;
} th4_b0_eb_sched_min_cell_spacing_eop_to_sop[3] = {
    {9, 9, 2, 2}, /* 1325 MHz */
    {9, 8, 2, 2}, /* 1025 MHz */
    {9, 8, 2, 2}  /*  950 MHz */
};

static const struct th4_b0_eb_sched_min_cell_spacing_s {
    int val_50g;
    int val_100g;
    int val_200g;
    int val_400g;
} th4_b0_eb_sched_min_cell_spacing[3] = {
    {6, 2, 2, 2}, /* 1325 MHz */
    {6, 2, 2, 2}, /* 1025 MHz */
    {6, 2, 2, 2}  /*  950 MHz */
};

static const struct th4_b0_mmu_ptsch_min_spacing_s {
    int val_50g;
    int val_100g;
    int val_200g;
    int val_400g;
} th4_b0_mmu_ptsch_min_spacing[3] = {
    {16, 16, 8, 4}, /* 1325 MHz */
    {12, 12, 6, 4}, /* 1025 MHz */
    {12, 12, 6, 4}  /*  950 MHz */
};

static const struct th4_b0_mmu_ptsch_penalty_min_spacing_s {
    int val_50g;
    int val_100g;
    int val_200g;
    int val_400g;
} th4_b0_mmu_ptsch_penalty_min_spacing[3] = {
    {200, 100, 50, 25}, /* 1325 MHz */
    {200, 100, 50, 25}, /* 1025 MHz */
    {200, 100, 50, 25}  /*  950 MHz */
};

static const struct th4_b0_tdm_mmu_eb_credit_config_vals_s {
    int val_50g;
    int val_100g;
    int val_200g;
    int val_400g;
} th4_b0_tdm_mmu_eb_credit_config_vals[3] = {
    {54, 69, 99, 160}, /* 1325 MHz */
    {58, 78, 117, 195}, /* 1025 MHz */
    {60, 81, 123, 208}, /* 950 MHz */
};

static const struct th4_b0_mmu_ebpcc_portdeficit_s {
...
th4_b0_tdm_global_to_local_phy_num(int phy_port, uint32_t *local_phy_num)
{
    *local_phy_num = 0x3f;
    ...
    if (0 <= phy_port && phy_port <= TH4_B0_PHYS_PORTS_PER_PIPE) {
        *local_phy_num = phy_port;
    } else if (((TH4_B0_PHYS_PORTS_PER_PIPE + 1) <= phy_port) &&
               (phy_port <= (TH4_B0_PHYS_PORTS_PER_PIPE * TH4_B0_PIPES_PER_DEV))) {
        *local_phy_num = ((phy_port - 1) % TH4_B0_PHYS_PORTS_PER_PIPE);
    } else if (phy_port == TH4_B0_PHY_PORT_MNG0 || phy_port == TH4_B0_PHY_PORT_MNG1) {
        *local_phy_num = TH4_B0_MGMT_LOCAL_PHYS_PORT;
    } else if (TH4_B0_PHY_PORT_LPBK0 <= phy_port && phy_port <= TH4_B0_PHY_PORT_LPBK7) {
        *local_phy_num = TH4_B0_LB_LOCAL_PHYS_PORT;
    } else {
        return SHR_E_PARAM;
    }
...
static int
th4_b0_tdm_set_cal_config(int unit,
                       int lport,
...
static int
th4_b0_tdm_set_max_spacing(int unit,
                        mmu_sched_inst_name_e sched_inst_name,
                        int pipe)
{
...
static int
th4_b0_tdm_set_feature_ctrl(int unit,
                         mmu_sched_inst_name_e sched_inst_name,
                         int pipe)
{
    int skip_instance;
...
static int
th4_b0_tdm_set_cbmg_value(int unit,
                       mmu_sched_inst_name_e sched_inst_name,
                       int pipe_num)
{
    int skip_instance;
    ...
    /*
     * TH4 update: TH4_B0_MMU_PortSch_uArch_delta.pdf
...
static int
th4_b0_tdm_set_urg_cell_spacing(int unit,
                             mmu_sched_inst_name_e sched_inst_name)
{
    bcmtm_drv_info_t *drv_info;
    bcmdrd_sid_t reg;
    ...
    fval = th4b0_sched_urg_pick_spacing[freq_indx].val_400g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = URG_PICK_200G_SPACINGf;
    fval = th4b0_sched_urg_pick_spacing[freq_indx].val_200g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = URG_PICK_100G_SPACINGf;
    fval = th4b0_sched_urg_pick_spacing[freq_indx].val_100g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = URG_PICK_50G_SPACINGf;
    fval = th4b0_sched_urg_pick_spacing[freq_indx].val_50g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
...
static int
th4_b0_tdm_set_min_spacing(int unit,
                        mmu_sched_inst_name_e sched_inst_name)
{
    bcmtm_drv_info_t *drv_info;
    bcmdrd_sid_t reg;
    ...
    fval = th4_b0_eb_sched_min_cell_spacing[freq_indx].val_50g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = ANY_CELL_100G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing[freq_indx].val_100g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = ANY_CELL_200G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing[freq_indx].val_200g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = ANY_CELL_400G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing[freq_indx].val_400g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
    ...
    fval = th4_b0_eb_sched_min_cell_spacing_eop_to_sop[freq_indx].val_50g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = INTER_PKT_100G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing_eop_to_sop[freq_indx].val_100g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = INTER_PKT_200G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing_eop_to_sop[freq_indx].val_200g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = INTER_PKT_400G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing_eop_to_sop[freq_indx].val_400g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
    ...
    fval = th4_b0_eb_sched_min_cell_spacing_within_pkt[freq_indx].val_50g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = MID_PKT_100G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing_within_pkt[freq_indx].val_100g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = MID_PKT_200G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing_within_pkt[freq_indx].val_200g;
    SHR_IF_ERR_EXIT(bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = MID_PKT_400G_MIN_SPACINGf;
    fval = th4_b0_eb_sched_min_cell_spacing_within_pkt[freq_indx].val_400g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    BCMTM_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT
 ...
 */
static int
th4_b0_tdm_set_ptsch_min_spacing(
    int unit,
    mmu_sched_inst_name_e sched_inst_name)
{
    bcmtm_drv_info_t *drv_info;
    ...
    fval = th4_b0_mmu_ptsch_min_spacing[freq_indx].val_50g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = REGULAR_100G_MIN_SPACINGf;
    fval = th4_b0_mmu_ptsch_min_spacing[freq_indx].val_100g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = REGULAR_200G_MIN_SPACINGf;
    fval = th4_b0_mmu_ptsch_min_spacing[freq_indx].val_200g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = REGULAR_400G_MIN_SPACINGf;
    fval = th4_b0_mmu_ptsch_min_spacing[freq_indx].val_400g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
    ...
    fval = th4_b0_mmu_ptsch_penalty_min_spacing[freq_indx].val_50g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = PENALTY_100G_MIN_SPACINGf;
    fval = th4_b0_mmu_ptsch_penalty_min_spacing[freq_indx].val_100g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = PENALTY_200G_MIN_SPACINGf;
    fval = th4_b0_mmu_ptsch_penalty_min_spacing[freq_indx].val_200g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));

    fid = PENALTY_400G_MIN_SPACINGf;
    fval = th4_b0_mmu_ptsch_penalty_min_spacing[freq_indx].val_400g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
...
static int
th4_b0_tdm_set_ebctm_min_spacing(
    int unit)
{
    bcmtm_drv_info_t *drv_info;
    bcmdrd_sid_t reg;
...
static int
th4_b0_tdm_set_ebpcc_tct_speed(
    int unit)
{
    bcmtm_drv_info_t *drv_info;
    bcmdrd_sid_t reg;
    ...
    fval = th4_b0_tdm_mmu_eb_credit_config_vals[freq_indx].val_50g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
    ...
    BCMTM_PT_DYN_INFO(pt_dyn_info, 0, 0);
    fid = EPORT_CELL_CREDITSf;
    fval = th4_b0_tdm_mmu_eb_credit_config_vals[freq_indx].val_100g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
    ...
    BCMTM_PT_DYN_INFO(pt_dyn_info, 0, 0);
    fid = EPORT_CELL_CREDITSf;
    fval = th4_b0_tdm_mmu_eb_credit_config_vals[freq_indx].val_200g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
    ...
    BCMTM_PT_DYN_INFO(pt_dyn_info, 0, 0);
    fid = EPORT_CELL_CREDITSf;
    fval = th4_b0_tdm_mmu_eb_credit_config_vals[freq_indx].val_400g;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
...
static int
th4_b0_tdm_set_ebpcc_tct_portdeficit(
    int unit)
{
    bcmtm_drv_info_t *drv_info;
    bcmdrd_sid_t reg;
    ...
    for (speed_indx = 0; speed_indx < 7; speed_indx++) {
        ...
        fval = th4_b0_mmu_ebpcc_portdeficit.portdeficit_sample_limit[freq_indx][speed_indx];
        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
        fid = PORTDEFICIT_SAMPLE_PERIODf;
        fval = th4_b0_mmu_ebpcc_portdeficit.portdeficit_sample_period[freq_indx][speed_indx];
        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
    ...
    for (speed_indx = 0; speed_indx < 7; speed_indx++) {
        ...
        fval = th4_b0_mmu_ebpcc_portdeficit.eport_cell_threshold[freq_indx][speed_indx];
        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
        fid = PORTDEFICIT_BACKOFF_LIMITf;
        fval = th4_b0_mmu_ebpcc_portdeficit.portdeficit_backoff_limit[freq_indx][speed_indx];
        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
    ...
    for (speed_indx = 0; speed_indx < 7; speed_indx++) {
        ...
        fval = th4_b0_mmu_ebpcc_portdeficit.warm_up_base_cnt[freq_indx][speed_indx];
        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
        fid = WARM_UP_MULT_CNTf;
        fval = th4_b0_mmu_ebpcc_portdeficit.warm_up_mult_cnt[freq_indx][speed_indx];
        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, reg, fid, &ltmbuf, &fval));
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_write(unit, reg, ltid, &pt_dyn_info, &ltmbuf));
...
static int
th4_b0_tdm_set_pksch_pkt_credits(int unit,
                              mmu_sched_inst_name_e sched_inst_name)
{
    int skip_instance;
...
static int
th4_b0_tdm_set_eb_credit_config(int unit,
                             int lport,
                             mmu_sched_inst_name_e sched_inst_name,
                             int is_port_down)
{
    ...
    phy_port = port_map->lport_map[lport].pport;
    mmu_port = port_map->pport_map[phy_port].mport;
    freq_indx = (drv_info->frequency == 1325) ? 0 :
                ((drv_info->frequency >= 1025) ? 1 : 2);
    ...
    if (is_port_down == 1) {
        ...
        switch (port_map->lport_map[lport].cur_speed) {
        case TDM_SPEED_400G:
            num_eb_credits = th4_b0_tdm_mmu_eb_credit_config_vals[freq_indx].val_400g;
            break;
        case TDM_SPEED_200G:
            num_eb_credits = th4_b0_tdm_mmu_eb_credit_config_vals[freq_indx].val_200g;
            break;
        case TDM_SPEED_100G:
            num_eb_credits = th4_b0_tdm_mmu_eb_credit_config_vals[freq_indx].val_100g;
            break;
        default: /* any speed <= 50G; */
            num_eb_credits = th4_b0_tdm_mmu_eb_credit_config_vals[freq_indx].val_50g;
            break;
        }
    }
...
/*! @fn int th4_b0_tdm_set_ebshp_port_cfg(
 *    int unit,
 ...
 */
static int
th4_b0_tdm_set_ebshp_port_cfg(int unit,
                           int lport,
                           mmu_sched_inst_name_e sched_inst_name,
                           int is_port_down)
{
    ...
    if (is_port_down == 1) {
        ...
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_get_speed_decode(unit, port_map->lport_map[lport].cur_speed,
                                      &port_speed));
    }
    ...
    LOG_DEBUG(BSL_LS_SOC_PORT,
             (BSL_META_U(unit,
                         ...
                         "phy_port = %0d, lport = %0d \n"),
              pipe, mmu_port, inst, phy_port, lport));
...
/*! @fn int th4_b0_tdm_clear_edb_credit_counter_per_port(
 *    int unit,
 *    soc_port_schedule_state_t *port_schedule_state,
 *    mmu_sched_inst_name_e sched_inst_name,
 *    int pipe,
 ...
 */
static int
th4_b0_tdm_clear_edb_credit_counter_per_port(int unit,
                                          int lport,
...
/*! @fn int th4_b0_tdm_set_mmu_port_to_phy_port_mapping(
 *    int unit,
 *    soc_port_schedule_state_t *port_schedule_state,
 *    mmu_sched_inst_name_e sched_inst_name,
 *    int pipe,
 ...
 */
static int
th4_b0_tdm_set_mmu_port_to_phy_port_mapping(int unit,
                                         int lport,
                                         mmu_sched_inst_name_e sched_inst_name,
                                         int is_port_down)
{
    ...
    if (is_port_down == 1) {
        ...
        SHR_IF_ERR_EXIT
            (th4_b0_tdm_global_to_local_phy_num(phy_port, &phy_port_num));
    }
...
static int
th4_b0_mmu_tdm_sched_general_init(
    int unit,
    mmu_sched_inst_name_e sched_inst_name)
{
    uint32_t pipe;
    ...
    SHR_IF_ERR_EXIT(th4_b0_tdm_set_ptsch_min_spacing(unit, sched_inst_name));
    /* Takes effect only in MMU EB PRT scheduler */
    SHR_IF_ERR_EXIT(th4_b0_tdm_set_min_spacing(unit, sched_inst_name));
    /* Takes effect only in MMU EB PRT scheduler */
    SHR_IF_ERR_EXIT(th4_b0_tdm_set_urg_cell_spacing(unit, sched_inst_name));
    /* Takes effect only in PKT SCH*/
    SHR_IF_ERR_EXIT(th4_b0_tdm_set_pksch_pkt_credits(unit, sched_inst_name));

    if (sched_inst_name == TDM_MMU_EB_PORT_SCHED) {
        SHR_IF_ERR_EXIT(th4_b0_tdm_set_ebctm_min_spacing(unit));
        SHR_IF_ERR_EXIT(th4_b0_tdm_set_ebpcc_tct_speed(unit));
        SHR_IF_ERR_EXIT(th4_b0_tdm_set_ebpcc_tct_portdeficit(unit));
    }
    ...
    for (pipe = 0; pipe < TH4_B0_PIPES_PER_DEV; pipe++) {
        if (pipe_map & (1 << pipe)) {
            ...
            SHR_IF_ERR_EXIT(th4_b0_tdm_set_max_spacing(unit, sched_inst_name, pipe));
            SHR_IF_ERR_EXIT(th4_b0_tdm_set_feature_ctrl(unit,sched_inst_name, pipe));
            SHR_IF_ERR_EXIT(th4_b0_tdm_set_cbmg_value(unit, sched_inst_name, pipe));
        }
    }
...
th4_b0_mmu_tdm_general_init(int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_tdm_sched_general_init(unit, TDM_MMU_MAIN_SCHED));
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_tdm_sched_general_init(unit, TDM_MMU_EB_PORT_SCHED));
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_tdm_sched_general_init(unit, TDM_MMU_EB_PKT_SCHED));
...
static int
th4_b0_tdm_port_config(int unit,
                       int lport,
                       mmu_sched_inst_name_e sched_inst_name,
                       int ports_down)
{
    ...
    SHR_IF_ERR_EXIT
        (th4_b0_tdm_set_cal_config(unit,
                                lport,
                                sched_inst_name,
                                ports_down));
    /* Configs port from mmu to phy mapping. */
    SHR_IF_ERR_EXIT
        (th4_b0_tdm_set_mmu_port_to_phy_port_mapping(unit,
                                                  lport,
                                                  sched_inst_name,
                                                  ports_down));
    ...
    if (bcmtm_lport_is_fp(unit, lport)) {
        SHR_IF_ERR_EXIT
            (th4_b0_tdm_set_ebshp_port_cfg(unit,
                                        lport,
                                        sched_inst_name,
                                        ports_down));
    }
    ...
    if (ports_down == 1) { /* remove ports from scheduler. */
        ...
        SHR_IF_ERR_EXIT
            (th4_b0_tdm_clear_edb_credit_counter_per_port(unit,
                                                       lport,
                                                       sched_inst_name,
                                                       ports_down));
        ...
        SHR_IF_ERR_EXIT
            (th4_b0_tdm_set_eb_credit_config(unit,
                                          lport,
                                          sched_inst_name,
                                          ports_down));
    }
...
th4_b0_mmu_tdm_port_delete(int unit, int lport)
{
    ...
    down = 1;
    SHR_IF_ERR_EXIT
        (th4_b0_tdm_port_config(unit, lport, TDM_MMU_MAIN_SCHED, down));
    SHR_IF_ERR_EXIT
        (th4_b0_tdm_port_config(unit, lport, TDM_MMU_EB_PORT_SCHED, down));
    SHR_IF_ERR_EXIT
        (th4_b0_tdm_port_config(unit, lport, TDM_MMU_EB_PKT_SCHED, down));
...
bcm56990_b0_tm_tdm_mmu_init(int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_tdm_general_init(unit));
