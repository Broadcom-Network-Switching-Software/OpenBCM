/*! \file bcm56990_b0_tm_shaper_encoding.c
 *
 ...
 */
static int
bcm56990_b0_tm_port_shaper_bandwidth(int unit,
                                     bcmtm_lport_t lport,
                                     uint32_t *bandwidth)
{
    bcmdrd_sid_t sid;
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_shaper_bucket_to_rate(unit, shaping_mode, &bucket_encode));
 ...
 */
static void
bcm56990_b0_tm_granularity_params(int unit,
                                  int granularity,
 ...
 */
static int
bcm56990_b0_tm_default_burst_size(int unit,
                                  bcmtm_lport_t lport,
                                  uint32_t bandwidth)
{
#define TH4_PORT_MTU    9216
     ...
     */
    if (SHR_FAILURE(bcm56990_b0_tm_port_shaper_bandwidth(unit, lport, &port_bw))) {
        port_bw = 0;
    }
                                     ...
                                     bcmtm_shaper_bucket_encode_t *bucket_encode)
{
    ...
    SHR_FUNC_ENTER(unit);
    bcm56990_b0_tm_granularity_params(unit, bucket_encode->granularity,
                                     ...
                                     bcmtm_shaper_bucket_encode_t *bucket_encode)
{
    ...
    if (burst_auto) {
        burst_size = (bandwidth > 0) ?
            bcm56990_b0_tm_default_burst_size(unit, lport, bandwidth) : 0 ;
    }
    ...
    for (gran = 0; gran <= TH4_METER_GRANULARITY_NUM - 1; gran++) {
        bcm56990_b0_tm_granularity_params(unit, gran, shaping_mode,
