 ...
 */
static int
bst_state_sid_map[BST_EVENT_CLASS_MAX][BST_EVENT_STATE_MAX][TH4_B0_ITMS_PER_DEV] =
{
 ...
 */
static int
bcm56990_b0_bst_event_notify_triggered(int unit,
                                       int buffer_pool,
 ...
 */
static int
bcm56990_b0_tm_bst_event_ing_src(int unit,
                                 bcmltd_sid_t ltid,
                                 int pipe,
                                 ...
                                 bcmltd_fields_t *in)
{
    ...
    switch (type) {
        ...
        case PORT_SERVICE_POOL:
            ...
            local_mport = (data >> 2);
            mport =  pipe * TH4_B0_MMU_PORTS_PER_PIPE + local_mport;
            SHR_IF_ERR_EXIT
 ...
 */
static int
bcm56990_b0_bst_event_ingress(int unit,
                              int buffer_pool)
{
    uint32_t pipe_bmp;
        ...
        if (pipe_bmp & 0x1) {
            ...
            for (idx = 0; idx < MAX_BST_ING_STATUS; idx++) {
                ...
                if (fval) {
                    ...
                    SHR_IF_ERR_EXIT
                        (bcm56990_b0_tm_bst_event_ing_src(unit, ltid, pipe,
                                                          idx, fval, &in));
                }
            }
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_bst_event_notify_triggered(unit, buffer_pool,
                                                BST_EVENT_CLASS_ING));
exit:
 ...
 */
static int
bcm56990_b0_bst_event_egress_source(int unit,
                                    bcmltd_sid_t ltid,
                                    ...
                                    bcmltd_fields_t *in)
{
    ...
    switch (type) {
        ...
        case QUEUE:
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_mmu_q_from_chip_q_get(unit, fval, &lport,
                                                      &q_id, &q_type));
 ...
 */
static int
bcm56990_b0_bst_event_egress(int unit, int itm)
{
    uint32_t fval;
    uint32_t ltmbuf = 0, ltmbuf1 = 0;
    ...
    if (fval) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_get(unit, sid, fid, &ltmbuf, &fval));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_bst_event_egress_source(unit, ltid, type, fval, &in));
    }
    ...
    SHR_IF_ERR_EXIT
        (bcmimm_entry_update(unit, 0, ltid, &in));
    SHR_IF_ERR_EXIT
        (bcm56990_b0_bst_event_notify_triggered(unit, itm,
                                                BST_EVENT_CLASS_EGR));
exit:
    SHR_FUNC_EXIT();
}
 ...
 */
static int
bcm56990_b0_bst_event_repl_queue(int unit, int itm)
{
    uint32_t fval;
    uint32_t ltm_buf0 = {0};
    bcmtm_pt_info_t pt_info = {0};
 ...
 */
static int
bcm56990_b0_bst_event_cfap(int unit, int itm)
{
    SHR_FUNC_ENTER(unit);
 ...
 */
static int
bcm56990_b0_tm_bst_intr_enable(int unit,
                               bcmtm_bst_event_class_t class,
                               bcmtm_intr_enable_t enable)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_mmui_interrupt_enable(unit, intr_num,
                                bcm56990_b0_tm_bst_intr_notify, enable));
exit:
    SHR_FUNC_EXIT();
}
 ...
 */
static int
bcm56990_b0_tm_bst_intr_clear(int unit,
                              int buffer_pool,
                              bcmtm_bst_event_class_t class)
{
    uint32_t intr_num;
 ...
 */
static int
bcm56990_b0_tm_bst_trigger(int unit,
                           bcmtm_bst_trigger_t *trigger_info)
{
    ...
    if (bst_dev_info->trigger_state[buffer_pool][trigger_info->class]) {
        switch (trigger_info->class) {
            ...
            case BST_EVENT_CLASS_ING:
                SHR_IF_ERR_EXIT
                    (bcm56990_b0_bst_event_ingress(unit, buffer_pool));
                break;
            /* Egress BST events */
            case BST_EVENT_CLASS_EGR:
                SHR_IF_ERR_EXIT
                    (bcm56990_b0_bst_event_egress(unit, buffer_pool));
                break;
            /* Replication queue BST events */
            case BST_EVENT_CLASS_REPL_QUEUE:
                SHR_IF_ERR_EXIT
                    (bcm56990_b0_bst_event_repl_queue(unit, buffer_pool));
                break;
            /* Device CFAP BST events */
            case BST_EVENT_CLASS_DEVICE:
                SHR_IF_ERR_EXIT
                    (bcm56990_b0_bst_event_cfap(unit, buffer_pool));
                break;
            default:
                SHR_ERR_EXIT(SHR_E_PARAM);
        }
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_bst_intr_clear(unit, buffer_pool, trigger_info->class));
    } else {
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_bst_intr_clear(unit, buffer_pool, trigger_info->class));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_bst_intr_enable(unit, trigger_info->class, INTR_ENABLE));
    }
exit:
    SHR_FUNC_EXIT();
}
 ...
 */
static int
bcm56990_b0_tm_bst_control_pt_set(int unit,
                                  bcmltd_sid_t ltid,
 ...
 */
static int
bcm56990_b0_tm_bst_event_state_enable(int unit,
                                      bcmltd_sid_t ltid,
                                      uint8_t buffer_pool,
                                      bcmtm_bst_event_class_t class,
                                      bcmtm_bst_event_state_t state)
{
    ...
    pt_info.flags |= BCMLT_ENT_ATTR_GET_FROM_HW;
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, sid, ltid, &pt_info, ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, fid, ltmbuf, &fval));
 ...
 */
static int
bcm56990_b0_tm_bst_event_state_disable(int unit,
                                       bcmltd_sid_t ltid,
                                       uint8_t buffer_pool,
                                       bcmtm_bst_event_class_t class)
{
 ...
 */
static int
bcm56990_b0_tm_bst_intr_state_disable(int unit,
                                      bcmltd_sid_t ltid,
                                      uint8_t buffer_pool,
                                      bcmtm_bst_event_class_t class)
{
 ...
 */
static int
bcm56990_b0_tm_bst_event_control_set(int unit,
                                     bcmltd_sid_t ltid,
                                     bcmtm_bst_event_cfg_t *bst_event_cfg)
{
    uint8_t buffer_pool;
    ...
    /* Disabling the BST events tracking. */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_bst_event_state_disable(unit, ltid, buffer_pool, class));
    ...
    switch (state) {
        ...
        case BST_EVENT_STATE_TRIGGERED:
            /* Interrupt is disabled for both ITMs then enable. */
            if (bst_dev_info->trigger_state[!buffer_pool & 0x1][class] == 0) {
                /*! Interrupt is enable in case of state is armed. */
                SHR_IF_ERR_EXIT
                    (bcm56990_b0_tm_bst_intr_enable(unit, class, INTR_ENABLE));
            }
        /* Fall through */
        case BST_EVENT_STATE_ARMED_PASSIVE:
            /*! Programming the event state */
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_bst_event_state_enable(unit, ltid, buffer_pool,
                                                       class, state));
            break;
            /*! BST Event state switched to OFF. */
        case BST_EVENT_STATE_OFF:
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_bst_intr_state_disable(unit, ltid,
                                                       buffer_pool, class));
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_bst_intr_clear(unit, buffer_pool, class));
            /*! Disable interrupts if both the buffers are disabled */
            if (bst_dev_info->trigger_state[!buffer_pool & 0x1][class] != 1) {
                SHR_IF_ERR_EXIT
                    (bcm56990_b0_tm_bst_intr_enable(unit, class, INTR_DISABLE));
            }
            break;
        default:
            break;
 ...
 */
void
bcm56990_b0_tm_bst_intr_notify(int unit,
                               uint32_t intr_param)
{
...
{
    bcmtm_bst_drv_t bcm56990_b0_bst_drv = {
        .bst_event_control_set = bcm56990_b0_tm_bst_event_control_set,
        .bst_control_pt_set = bcm56990_b0_tm_bst_control_pt_set,
        .bst_trigger = bcm56990_b0_tm_bst_trigger,
    };
    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(bst_drv, SHR_E_INTERNAL);
    *((bcmtm_bst_drv_t *)bst_drv) = bcm56990_b0_bst_drv;
exit:
    SHR_FUNC_EXIT();
}
