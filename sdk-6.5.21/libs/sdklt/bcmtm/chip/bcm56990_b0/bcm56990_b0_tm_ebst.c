...
#include <bcmbd/chip/bcm56990_b0_mmui_intr.h>
...
#define TH4_B0_EBST_FIFODMA_BUFFER_ENTRIES 16384
#define TH4_B0_EBST_FIFO_CHAN0 1
#define TH4_B0_EBST_FIFO_CHAN1 2
 ...
 */
static int
bcm56990_b0_tm_ebst_timestamp_init(int unit)
{
    bcmdrd_sid_t sid = 0;
    bcmdrd_fid_t fid = 0;
 ...
 */
static int
bcm56990_b0_tm_ebst_port_config_set(int unit,
                                    bcmltd_sid_t ltid,
                                    bcmtm_lport_t lport,
 ...
 */
static int
bcm56990_b0_tm_ebst_q_config_set(int unit,
                                 bcmltd_sid_t ltid,
                                 bcmtm_lport_t lport,
                                 uint16_t q_id,
                                 bcmtm_ebst_q_cfg_t *q_cfg)
{
    ...
    if (bcmtm_lport_is_cpu(unit, lport)) {
        ...
        if (q_cfg->q_type == MC_Q) {
            q_id += bcm56990_b0_tm_num_uc_q_non_cpu_port_get(unit);
        }
    }
 ...
 */
static int
bcm56990_b0_tm_ebst_scan_config_set(int unit,
                                    bcmltd_sid_t ltid,
 ...
 */
static int
bcm56990_b0_tm_ebst_data_parse(int unit,
                               uint32_t *entry,
                               ...
                               bcmtm_ebst_data_t *data)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmdrd_pt_field_get(unit, sid, entry, BST_ENTITYf, &bst_entity));
    ...
    *entity = bst_entity;
    /*
     * type == 1, BST_COUNT is egress data buf service pool use-count
     * type == 0, BST_COUNT is egress queue or egress port use-count
     */
    ...
    if (((bst_entity >> 2) > TH4_B0_DEV_PORTS_PER_DEV) &&
        ((scan_mode == EBST_SCAN_MODE_PORTSP_UC) ||
         (scan_mode == EBST_SCAN_MODE_PORTSP_MC))) {
...
static void
bcm56990_b0_tm_ebst_intr_notify(int unit, uint32_t intr_param)
{
 ...
 */
static int
bcm56990_b0_tm_ebst_enable(int unit,
                           bcmltd_sid_t ltid,
                           bool enable)
{
    uint32_t intr_num;
    ...
    if (enable) {
        SHR_IF_ERR_EXIT
            (bcmtm_mmui_interrupt_enable(unit, intr_num,
                               bcm56990_b0_tm_ebst_intr_notify, INTR_ENABLE));
    } else {
        SHR_IF_ERR_EXIT
 ...
 */
static int
bcm56990_b0_tm_ebst_map_alloc(int unit)
{
    int size;
    ...
    /* EBST queue map. */
    size = TH4_B0_TM_CHIP_QUEUES * sizeof(bcmtm_ebst_entity_map_t);
    ...
    size = TH4_B0_DEV_PORTS_PER_DEV * TH4_B0_MAX_SERVICE_POOLS *
   ...
   /* EBST service pool. */
    size = TH4_B0_MAX_SERVICE_POOLS * (sizeof(bcmtm_ebst_entity_map_t));
    sp_map = sal_alloc(size, "bcmtmEbstServicePoolMap");
    SHR_NULL_CHECK(sp_map, SHR_E_MEMORY);
    sal_memset(sp_map, 0, size);
    map_info->sp_map = sp_map;
 ...
 */
static int
bcm56990_b0_tm_ebst_portsp_index_get(int unit,
                                     bcmtm_lport_t lport,
                                     uint8_t spid,
                                     uint16_t *map_idx)
{
 ...
 */
static int
bcm56990_b0_tm_ebst_q_map_index_get(int unit,
                                    bcmtm_lport_t lport,
                                    uint16_t q_id,
                                    ...
                                    uint16_t *map_idx)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_chip_q_get(unit, lport, q_id, &chip_q));
    *map_idx = chip_q;
exit:
    SHR_FUNC_EXIT();
}
...
{
    bcmtm_ebst_drv_t bcm56990_b0_tm_ebst_drv = {
        .ebst_port_config_set = bcm56990_b0_tm_ebst_port_config_set,
        .ebst_scan_config_set = bcm56990_b0_tm_ebst_scan_config_set,
        .ebst_portsp_index_get = bcm56990_b0_tm_ebst_portsp_index_get,
        .ebst_map_alloc = bcm56990_b0_tm_ebst_map_alloc,
        .ebst_q_config_set = bcm56990_b0_tm_ebst_q_config_set,
        .ebst_q_map_index_get = bcm56990_b0_tm_ebst_q_map_index_get,
        .ebst_data_parse = bcm56990_b0_tm_ebst_data_parse,
        .ebst_enable = bcm56990_b0_tm_ebst_enable,
    };

    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(ebst_drv, SHR_E_INTERNAL);
    *((bcmtm_ebst_drv_t *)ebst_drv) = bcm56990_b0_tm_ebst_drv;
exit:
    SHR_FUNC_EXIT();
}
...
{
    bcmtm_ebst_dev_info_t *ebst_info;
    uint16_t chan_id[TH4_B0_ITMS_PER_DEV] = {TH4_B0_EBST_FIFO_CHAN0,
                                          TH4_B0_EBST_FIFO_CHAN1};
    int idx;
    ...
    /* populate ebst info */
    ebst_info->max_buffer_pool = TH4_B0_ITMS_PER_DEV;
    ebst_info->num_entries = TH4_B0_EBST_FIFODMA_BUFFER_ENTRIES;
    ebst_info->thd_entries = TH4_B0_EBST_FIFODMA_BUFFER_ENTRIES / 10;
    for (idx = 0; idx < TH4_B0_ITMS_PER_DEV; idx++) {
    ...
    if (!warm) {
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_ebst_timestamp_init(unit));
    }
exit:
    SHR_FUNC_EXIT();
}
