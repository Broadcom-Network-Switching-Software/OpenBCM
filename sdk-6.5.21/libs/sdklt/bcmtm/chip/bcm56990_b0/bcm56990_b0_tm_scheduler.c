/*! \file bcm56990_b0_tm_scheduler.c
 *
 * Scheduler related api and function for bcm56990_b0.
 */
/*
 ...
 */
static int
bcm56990_b0_tm_cos_l0_map_validate(int unit,
                                   ...
                                   int *opcode)
{
     ...
     * as per MMU_Q_MODE.
     */
    max_num_mcq = bcm56990_b0_tm_num_mc_q_non_cpu_port_get(unit);
    max_num_ucq = bcm56990_b0_tm_num_uc_q_non_cpu_port_get(unit);
 ...
 */
static void
bcm56990_b0_tm_cos_l0_map_set(int unit,
                              int profile_id,
                              bcmtm_scheduler_profile_t *profile)
{
    bcmtm_scheduler_cos_l0_map_t *l0_map =
 ...
 */
static int
bcm56990_b0_tm_mmuq_l1_map_get(int unit,
                               uint8_t profile_id,
                               ...
                               int *l1_node)
{
    ...
    if (sched_level == L1_SCHED_MULTICAST_QUEUE) {
        mcq_offset = bcm56990_b0_tm_num_uc_q_non_cpu_port_get(unit);
        mmu_q_id += mcq_offset;
    }
 ...
 */
static int
bcm56990_b0_tm_scheduler_node_profile_validate(int unit,
                                               int node_id,
                                               bcmtm_scheduler_profile_t *profile)
{
 ...
 */
static int
bcm56990_b0_tm_scheduler_profile_validate(int unit,
                                          uint8_t profile_id,
                                          int node_id,
                                          bcmtm_scheduler_profile_t *profile)
{
    ...
    sal_memset(&l0_map, -1, sizeof(bcmtm_scheduler_cos_l0_map_t));
    if (SHR_E_NONE !=
            bcm56990_b0_tm_cos_l0_map_validate(unit, profile, &l0_map, &opcode)) {
 ...
 */
static int
bcm56990_b0_tm_scheduler_node_set(int unit,
        bcmtm_lport_t lport,
        ...
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (sched_level == L0_SCHED_COS) {/* L0 Level */
        fid = SP_L0_CHILDf;
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_sched_node_l0_map_get(unit, profile_id,
                                                  sched_node_id, &node_id));
    } else {/* L1 level */
        fid = SP_L0_PARENTf;
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_mmuq_l1_map_get(unit, profile_id, sched_level,
                                           sched_node_id, &node_id));
    }
 ...
 */
static void
bcm56990_b0_tm_l1_to_l0_mapping(int unit,
                                uint32_t profile_id,
                                bcmtm_scheduler_profile_t *profile,
                                uint32_t *bmap)
{
 ...
 */
static int
bcm56990_b0_tm_scheduler_port_set(int unit,
        bcmltd_sid_t ltid,
        bcmtm_lport_t lport,
        bcmtm_scheduler_profile_t *profile,
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (profile) {
        bcm56990_b0_tm_l1_to_l0_mapping(unit, profile_id, profile, &bmap);
    }
    SHR_IF_ERR_EXIT
...
static int
bcm56990_b0_tm_scheduler_profile_reset(int unit,
                                       bcmltd_sid_t ltid,
                                       uint8_t profile_id)
{
 ...
 */
static int
bcm56990_b0_tm_scheduler_profile_set(int unit,
                                     bcmltd_sid_t ltid,
                                     uint8_t profile_id,
                                     bcmtm_scheduler_profile_t *profile)
{
    ...
    if (!profile) {
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_scheduler_profile_reset(unit, ltid, profile_id));
        l0_map = &(bcmtm_scheduler_cos_l0_map[unit][profile_id]);
        sal_memset(l0_map->map, -1, sizeof(bcmtm_scheduler_cos_l0_map_t));
        SHR_EXIT();
    } else {
        bcm56990_b0_tm_cos_l0_map_set(unit, profile_id, profile);
    }
    for (idx = 0; idx < TH4_TM_NUM_Q; idx++) {
        num_ucq = profile[idx].num_ucq;
        num_mcq = profile[idx].num_mcq;
 ...
 */
static int
bcm56990_b0_tm_scheduler_cpu_node_set(int unit,
        uint32_t sched_node_id,
        uint32_t sched_level,
        bcmltd_sid_t ltid,
        bcmtm_scheduler_cfg_t *scheduler_cfg)
...
{
    bcmtm_scheduler_drv_t bcm56990_b0_sched_drv = {
        .node_set = bcm56990_b0_tm_scheduler_node_set,
        .port_set = bcm56990_b0_tm_scheduler_port_set,
        .node_profile_validate = bcm56990_b0_tm_scheduler_node_profile_validate,
        .profile_validate = bcm56990_b0_tm_scheduler_profile_validate,
        .cpu_node_set = bcm56990_b0_tm_scheduler_cpu_node_set,
        .profile_set = bcm56990_b0_tm_scheduler_profile_set,
        .profile_init = bcm56990_b0_tm_scheduler_profile_init
    };

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(sched_drv, SHR_E_INTERNAL);
    *((bcmtm_scheduler_drv_t *)sched_drv) = bcm56990_b0_sched_drv;
exit:
    SHR_FUNC_EXIT();
}
