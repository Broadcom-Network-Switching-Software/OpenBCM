/*! \file bcm56990_b0_tm_multicast.c
 *
 ...
 */
static int
bcm56990_b0_mc_icc_set (int unit, int mc_head_id, int repl_count)
{
    bcmdrd_sid_t sid;
...
static int
bcm56990_b0_mc_icc_get (int unit, int mc_head_id, int *repl_count)
{
    bcmdrd_sid_t sid;
 ...
 */
static int
bcm56990_b0_mc_dynamic_repl_set(int unit,
...
static int
bcm56990_b0_mc_repl_group_entry_set (int unit, uint32_t mc_group_id,
...
static int
bcm56990_b0_mc_repl_group_entry_get (int unit, uint32_t mc_group_id,
                                     bcmtm_mc_repl_group_info_t *mc_group_info)
{
    bcmdrd_sid_t sid;
    bcmdrd_fid_t fid;
...
static int
bcm56990_b0_mc_repl_head_entry_set (int unit, uint32_t mc_head_id,
                                    uint32_t start_ptr)
{
    bcmdrd_sid_t sid;
    bcmdrd_fid_t fid;
...
static int
bcm56990_b0_mc_repl_head_entry_get (int unit, uint32_t mc_head_id,
                                    uint32_t *start_ptr)
{
    bcmdrd_sid_t sid;
    bcmdrd_fid_t fid;
 ...
 */
static int
bcm56990_b0_mc_repl_head_copy(int unit,
                              bcmtm_mc_repl_group_info_t *mc_group_info,
                              ...
                              int repl_count)
{
#define TH4_B0_MC_RESERVED_HEAD_ENTRY 3824
    int num_entry, icc_count = 0;
    uint32_t head_id = 0;
    uint32_t dup_head_id = TH4_B0_MC_RESERVED_HEAD_ENTRY;
    uint32_t list_ptr = 0;
    ...
    *base_ptr = dup_head_id;
    ...
    for (num_entry = 0; num_entry < mc_group_info->num_entries; num_entry++) {
        ...
        if (head_id == (uint32_t)repl_head_info->index) {
            ...
            SHR_IF_ERR_EXIT
                (bcm56990_b0_mc_repl_head_entry_get(unit, head_id, &list_ptr));
            SHR_IF_ERR_EXIT
                (bcm56990_b0_mc_icc_get(unit, head_id, &icc_count));
        }
        SHR_IF_ERR_EXIT
            (bcm56990_b0_mc_repl_head_entry_set(unit, dup_head_id, list_ptr));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_mc_icc_set(unit, dup_head_id, icc_count));
        head_id++;
        dup_head_id++;
    }
exit:
 ...
 */
static int
bcm56990_b0_mc_repl_list_update(int unit,
                                bcmtm_mc_repl_head_info_t *repl_head_info,
                                uint32_t start_ptr,
                                int repl_count)
{
     ...
     */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_mc_repl_group_entry_get(unit, mc_group_id, &mc_group_info));
    SHR_IF_ERR_EXIT
        (bcm56990_b0_mc_repl_head_copy(unit, &mc_group_info,
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_mc_repl_group_entry_set(unit, 0, &mc_group_info));
     ...
     */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_mc_dynamic_repl_set(unit, mc_group_id, 1));
    /*
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_rqe_port_flush(unit));
exit:
    SHR_FUNC_EXIT();
}
 ...
 */
static int
bcm56990_b0_mc_repl_list_update_cleanup(int unit)
{
    int idx = 0;
    bool psim, asim, emul;
    bcmtm_mc_repl_group_info_t mc_group_info;
     ...
     */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_mc_dynamic_repl_set(unit, 0, 0));
    ...
    /* Clear head list */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_mc_repl_group_entry_get(unit, 0, &mc_group_info));
    base_ptr = mc_group_info.base_ptr;
    for (idx = 0; idx < mc_group_info.num_entries; idx++) {
        SHR_IF_ERR_EXIT
            (bcm56990_b0_mc_repl_head_entry_set(unit, base_ptr, 0));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_mc_icc_set(unit, base_ptr, 0));
        base_ptr++;
    }
...
static int
bcm56990_b0_mc_repl_list_entry_set (int unit,
...
static int
bcm56990_b0_mc_repl_list_entry_get (int unit,
                                    bcmtm_mc_repl_list_entry_t *mc_list_entry)
{
    bcmdrd_sid_t sid;
    bcmdrd_fid_t fid;
...
static int
bcm56990_b0_mc_repl_list_in_use_get (int unit, int repl_entry_ptr,
                                     bool *in_use)
{
    bcmdrd_sid_t sid;
    bcmdrd_fid_t fid;
...
static int
bcm56990_b0_mc_ipmc_set (int unit, int mc_group_id,
                         bcmtm_mc_ipmc_info_t *ipmc_info)
{
    ...
    sid = EGR_IPMCm;
    fid = L3_PAYLOADf;
    fval = ipmc_info->l3_payload;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, fid, &ltmbuf1, (uint32_t *)&fval));

    fid = DONT_PRUNE_VLANf;
    fval = ipmc_info->disable_prune;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, fid, &ltmbuf1, (uint32_t *)&fval));

    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, sid, ltid, (void *)&pt_dyn_info, &ltmbuf1));
exit:
    SHR_FUNC_EXIT();
}

static int
bcm56990_b0_mc_ipmc_get (int unit, int mc_group_id,
                         bcmtm_mc_ipmc_info_t *ipmc_info)
{
    bcmdrd_sid_t sid;
    bcmdrd_fid_t fid;
    ...
    sid = EGR_IPMCm;
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, sid, ltid, (void *)&pt_dyn_info, &ltmbuf1));

    fid = L3_PAYLOADf;
    SHR_IF_ERR_EXIT
        (bcmtm_field_get(unit, sid, fid, &ltmbuf1, &fval));
    ipmc_info->l3_payload = fval;

    fid = DONT_PRUNE_VLANf;
    SHR_IF_ERR_EXIT
        (bcmtm_field_get(unit, sid, fid, &ltmbuf1, &fval));
    ipmc_info->disable_prune = fval;
...
static int
bcm56990_b0_mc_repl_head_index_count_get(int unit, int *num_index)
{
    int max, min;
    SHR_FUNC_ENTER(unit);
...
static int
bcm56990_b0_mc_nhop_index_count_get(int unit, int *num_index)
{
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(num_index, SHR_E_PARAM);

    SHR_IF_ERR_EXIT
           (bcmlrd_table_max_index_get(unit, L3_MC_HIERARCHICAL_NHOPt,
                               (uint32_t *)num_index));
...
static int
bcm56990_b0_mc_nhops_per_repl_index_sparse_mode_get(int unit, int *num_index)
{
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(num_index, SHR_E_PARAM);
    *num_index = TH4_B0_NHOPS_PER_REPL_INDEX_SPARSE_MODE;
exit:
    SHR_FUNC_EXIT();
}

static int
bcm56990_b0_mc_max_packet_replication_get(int unit, int *num_index)
{
    SHR_FUNC_ENTER(unit);
    *num_index = TH4_B0_MAX_MC_PKT_REPL;
    SHR_FUNC_EXIT();
}
 ...
 */
static int
bcm56990_b0_mc_repl_list_free_count(int unit, uint64_t *free_count)
{
    int in_use = 0;
    bcmtm_mc_dev_info_t *mc_dev;
    int tc = TH4_B0_MAX_REPL_LIST_COUNT;
    ...
    *free_count = TH4_B0_MAX_REPL_LIST_COUNT - in_use;
exit:
    SHR_FUNC_EXIT();
}
/*******************************************************************************
 ...
 */
int
bcm56990_b0_tm_multicast_init (int unit, int warm)
{
    uint32_t ltmbuf[BCMTM_MC_MAX_ENTRY_WSIZE] = {0};
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_sid_t sid;
...
int bcm56990_b0_tm_mc_drv_get(int unit, void *mc_drv)
{
    bcmtm_mc_drv_t bcm56990_b0_mc_drv = {
        .mc_repl_group_entry_set = bcm56990_b0_mc_repl_group_entry_set,
        .mc_repl_group_entry_get = bcm56990_b0_mc_repl_group_entry_get,
        .mc_repl_head_entry_set = bcm56990_b0_mc_repl_head_entry_set,
        .mc_repl_head_entry_get = bcm56990_b0_mc_repl_head_entry_get,
        .mc_repl_list_entry_set = bcm56990_b0_mc_repl_list_entry_set,
        .mc_repl_list_entry_get = bcm56990_b0_mc_repl_list_entry_get,
        .mc_repl_list_in_use_get = bcm56990_b0_mc_repl_list_in_use_get,
        .mc_repl_icc_set = bcm56990_b0_mc_icc_set,
        .mc_ipmc_set = bcm56990_b0_mc_ipmc_set,
        .mc_ipmc_get = bcm56990_b0_mc_ipmc_get,
        .mc_repl_head_index_count_get = bcm56990_b0_mc_repl_head_index_count_get,
        .mc_nhop_index_count_get = bcm56990_b0_mc_nhop_index_count_get,
        .mc_nhops_per_repl_index_sparse_mode_get =
            bcm56990_b0_mc_nhops_per_repl_index_sparse_mode_get,
        .mc_max_packet_replication_get = bcm56990_b0_mc_max_packet_replication_get,
        .mc_repl_list_update = bcm56990_b0_mc_repl_list_update,
        .mc_repl_list_update_cleanup = bcm56990_b0_mc_repl_list_update_cleanup,
        .mc_repl_list_free_count = bcm56990_b0_mc_repl_list_free_count,
    };
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(mc_drv, SHR_E_INTERNAL);

    *((bcmtm_mc_drv_t *)mc_drv) = bcm56990_b0_mc_drv;
exit:
    SHR_FUNC_EXIT();
}
