/*! \file bcm56990_b0_tm_init.c
 *
 * File containing device level and port level init sequence for bcm56990_b0.
 */
/*
...
#include "bcm56990_b0_tm_ct.h"
#include "bcm56990_b0_tm_wred.h"
#include "bcm56990_b0_tm_obm.h"
#include "bcm56990_b0_tm_ebst.h"
#include "bcm56990_b0_tm_multicast_internal.h"
...
#include "bcm56990_b0_tm_edb_flexport.h"
#include "bcm56990_b0_tm_mmu_tdm.h"
#include "bcm56990_b0_tm_mmu_flexport.h"
...
static bcmtm_port_info_t cpu_ports[TH4_B0_NUM_CPU_PORTS] = {
    /* log_port, phy_port, pipe, idb_port, mmu_port */
    { 0, 0, 0, 16, 16 }
};

static bcmtm_port_info_t lb_ports[TH4_B0_NUM_LB_PORTS] = {
...
static bcmtm_port_info_t mgmt_ports[TH4_B0_NUM_MGMT_PORTS] = {
    { 152, 257, 8,  16, 272},     /* management port 0 */
    { 50,  258, 2,  16, 80 },     /* management port 1 */
};

#define TH4_B0_MC_Q_MODE_DEFAULT 2
#define TH4_B0_NUM_CPU_QUEUES 48
#define TH4_B0_NUM_GP_QUEUES  12

#define BCMTM_TH4_B0_AUX_PORT_SPEED        10000
#define BCMTM_TH4_B0_10G_MGMT_PORT_SPEED   10000
#define BCMTM_TH4_B0_40G_MGMT_PORT_SPEED   40000
#define CLK(mhz)      (BCM56990_B0_LRD_DEVICE_CLK_FREQ_T_T_CLK_##mhz)

/* From Device_CLK_FREQ_T.type.ltl. */
static
unsigned int th4_b0_freq_retrieve_from_enum(int unit, int freq_enum) {
    uint32_t frequency;
...
port_pipe_get(int unit, int phys_port)
{
    ...
    if ((phys_port < 0) || (phys_port >= TH4_B0_NUM_PHYS_PORTS)) {
        return -1;
    }
   ...
   /* Regular ports */
   pipe = (phys_port - 1) / TH4_B0_PHYS_PORTS_PER_PIPE;
...
port_idbport_get(int unit, int phys_port, int dev_port)
{
    ...
    if ((dev_port < 0) || (dev_port >= TH4_B0_DEV_PORTS_PER_DEV)) {
        return -1;
    }
    ...
    lcl_phys_port = ((phys_port - 1) % TH4_B0_PHYS_PORTS_PER_PIPE);
    pipe_pm = lcl_phys_port >> 2;
    subp = lcl_phys_port % TH4_B0_PORTS_PER_PBLK;

    idb_port = (subp * TH4_B0_PORTS_PER_PBLK) + pipe_pm;
...
port_mmuport_get(int unit, int phys_port, int log_port)
{
    ...
    mmu_port = (pipe * TH4_B0_MMU_PORTS_PER_PIPE) + mmu_lcl_port;
    return mmu_port;
}

static int
th4_b0_uc_cosq_base_get(int unit, int pport, int lport)
{
    int pipe, pipe_queue_base[TH4_B0_PIPES_PER_DEV], pipe_index;
    int local_mmu_port;
    ...
    pipe_queue_base[0] = 0;
    for (pipe_index = 1; pipe_index < TH4_B0_PIPES_PER_DEV; pipe_index++) {
    ...
    return (pipe_queue_base[pipe] + local_mmu_port * TH4_B0_NUM_GP_QUEUES);
}

static int
th4_b0_mc_cosq_base_get(int unit, int pport, int lport)
{
    int uc_q_base, num_mcq;
    uc_q_base = th4_b0_uc_cosq_base_get(unit, pport, lport);
    ...
    num_mcq = bcm56990_b0_tm_num_mc_q_non_cpu_port_get(unit);
    return (uc_q_base + TH4_B0_NUM_GP_QUEUES - num_mcq);
}

static void
bcm56990_b0_bcmtm_dev_info_set(int unit)
{
    struct bcmtm_device_info bcm56990_b0_tm_dev_info = {
        .max_pkt_sz   = TH4_B0_MMU_MAX_PACKET_BYTES,
        .pkt_hdr_sz   = TH4_B0_MMU_PACKET_HEADER_BYTES,
        .jumbo_pkt_sz = TH4_B0_MMU_JUMBO_FRAME_BYTES,
        .default_mtu  = TH4_B0_MMU_DEFAULT_MTU_BYTES,
        .cell_sz      = TH4_B0_MMU_BYTES_PER_CELL,
        .num_cells    = TH4_B0_MMU_TOTAL_CELLS_PER_ITM,
        .num_port_pg  = SOC_MMU_CFG_PRI_GROUP_MAX,
        .num_sp       = SOC_MMU_CFG_SERVICE_POOL_MAX,
        .mcq_entry    = SOC_TH4_B0_MMU_MCQ_ENTRY_PER_ITM,
        .num_q        = TH4_B0_NUM_GP_QUEUES,
        .num_cpu_q    = TH4_B0_NUM_CPU_QUEUES,
        .num_pipe     = TH4_B0_PIPES_PER_DEV,
        .num_nhop_sparse = TH4_B0_NHOPS_PER_REPL_INDEX_SPARSE_MODE,
        .num_nhop_dense = TH4_B0_NHOPS_PER_REPL_INDEX_DENSE_MODE,
        .max_num_repl = TH4_B0_MAX_MC_PKT_REPL,
        .num_buffer_pool = TH4_B0_ITMS_PER_DEV};
    bcmtm_device_info[unit] = bcm56990_b0_tm_dev_info;
}
 ...
 */
static int
th4_b0_dpr_frequency_range_check(int unit, int dpr_freq)
{
 ...
 */
static int
th4_b0_tm_port_map_populate(int unit, bool warm)
{
    uint64_t config_value;
    bcmtm_drv_info_t *drv_info;
    int pm_id;
    uint64_t flex_port[TH4_B0_PBLKS_PER_DEV] = {0};
    size_t actual;
    ...
    SHR_IF_ERR_EXIT
        (bcmcfg_field_get(unit, TM_THD_CONFIGt, TM_THD_CONFIGt_SKIP_BUFFER_RESERVATIONf,
                          &config_value));
    drv_info->skip_buf_resv = config_value;

    SHR_IF_ERR_EXIT
        (bcmcfg_field_get(unit, DEVICE_CONFIGt, DEVICE_CONFIGt_CORE_CLK_FREQf,
                          &config_value));
    drv_info->frequency = th4_b0_freq_retrieve_from_enum(unit, config_value);
    SHR_IF_ERR_EXIT
        (bcmcfg_field_get(unit, DEVICE_CONFIGt, DEVICE_CONFIGt_PP_CLK_FREQf,
                          &config_value));
    drv_info->dpr_clock_frequency = th4_b0_freq_retrieve_from_enum(unit, config_value);
    ...
    SHR_IF_ERR_EXIT
        (bcmcfg_field_array_get(unit, TM_PM_FLEX_CONFIGt,
                                ...
                                0, TH4_B0_PBLKS_PER_DEV,
                                flex_port, &actual));
    for (pm_id = 0; pm_id < TH4_B0_PBLKS_PER_DEV; pm_id++) {
        drv_info->flex_eligible[pm_id] = flex_port[pm_id];
    }

    SHR_IF_ERR_EXIT
        (th4_b0_dpr_frequency_range_check(unit, drv_info->dpr_clock_frequency));
 ...
 */
void
bcm56990_b0_tm_drv_info_portmap_fill(int unit,
                      size_t num_ports,
                      const bcmpc_mmu_port_cfg_t *new_cfg)
{
#define BCMTM_TH4_B0_CPU_LB_PORT_SPEED 10000
    ...
    for (idx = 0; idx < (int)num_ports; idx++) {
        ...
        if (!bcmtm_lport_is_fp(unit, lport)) {
            if (port_map->lport_map[lport].cur_speed < BCMTM_TH4_B0_CPU_LB_PORT_SPEED) {
                /* set speed to minimum supported internal to TM */
                port_map->lport_map[lport].cur_speed = BCMTM_TH4_B0_CPU_LB_PORT_SPEED;
            }
        }
        ...
        chip_port =
            (port_map->lport_map[lport].pipe * TH4_B0_DEV_PORTS_PER_PIPE) +
        ...
        if (bcmtm_lport_is_cpu(unit, lport)) {
            ...
            port_map->lport_map[lport].max_speed = BCMTM_TH4_B0_CPU_LB_PORT_SPEED;
            port_map->lport_map[lport].cur_speed = BCMTM_TH4_B0_CPU_LB_PORT_SPEED;
        } else {
            port_map->lport_map[lport].num_mcq =
                        bcm56990_b0_tm_num_mc_q_non_cpu_port_get(unit);
            port_map->lport_map[lport].num_ucq =
                        bcm56990_b0_tm_num_uc_q_non_cpu_port_get(unit);
        }
        ...
        port_map->lport_map[lport].base_ucq = th4_b0_uc_cosq_base_get(unit, pport,
                port_cfg->lport);
        port_map->lport_map[lport].base_mcq = th4_b0_mc_cosq_base_get(unit, pport,
                port_cfg->lport);
...
#undef BCMTM_TH4_B0_CPU_LB_PORT_SPEED
}

static int
th4_b0_mmu_thdo_pipe_itm_config(int unit) {
    ...
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm++) {
...
static int
th4_b0_mmu_rqe_queue_number_init(int unit)
{
    bcmtm_pt_info_t pt_dyn_info = {0};
    uint32_t ltmbuf;
    bcmdrd_sid_t sid;
...
static int
th4_b0_en_mmu_refresh(int unit)
{
    int sid_index;
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_sid_t sid ;
...
static int
th4_b0_cell_pointer_init(int unit)
{
    int itm;
...
static int
th4_b0_mmu_intfi_enable(int unit)
{
...
static int
bcm56990_b0_tm_pfc_ddr_info_alloc(int unit, bool warm)
{
    ...
    alloc_sz = data_sz * TH4_B0_DEV_PORTS_PER_DEV;
    ...
    SHR_IF_ERR_EXIT
        ((alloc_sz < (data_sz * TH4_B0_DEV_PORTS_PER_DEV)) ? SHR_E_MEMORY: SHR_E_NONE);
    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(unit, "BCMTM: HA memory for pfc ddr port map %0d\n"),
            alloc_sz));
    if (!warm || (bcmissu_struct_info_get(unit,
                                          BCMTM_PFC_DDR_INFO_T_ID,
                                          BCMMGMT_TM_COMP_ID,
                                          BCMTM_HA_DEV_PFC_DEADLOCK_INFO,
                                          &offset) == SHR_E_NOT_FOUND)) {
        sal_memset(pfc_ddr_info, 0, alloc_sz);
        bcmissu_struct_info_report(unit, BCMMGMT_TM_COMP_ID,
                                   BCMTM_HA_DEV_PFC_DEADLOCK_INFO, 0,
                                   data_sz, TH4_B0_DEV_PORTS_PER_DEV,
                                   BCMTM_PFC_DDR_INFO_T_ID);
    } else {
        pfc_ddr_info = (bcmtm_pfc_ddr_info_t *)
                        ((uint8_t *)pfc_ddr_info + offset);
    }
...
static int
bcm56990_b0_tm_port_map_alloc(int unit, bool warm)
{
    bcmtm_port_map_info_t port_map;
    uint32_t data_sz, alloc_sz;
    size_t num_inst;
    ...
    num_inst = TH4_B0_DEV_PORTS_PER_DEV;
    ...
    if (!warm || (bcmissu_struct_info_get(unit,
        ...
        port_map.lport_map = (bcmtm_lport_map_t *)
                              ((uint8_t *)(port_map.lport_map) + offset);
    }

    offset = 0;
    data_sz = sizeof(bcmtm_pport_map_t);
    num_inst = TH4_B0_NUM_PHYS_PORTS;
    alloc_sz = data_sz * num_inst;
    ...
    if (!warm || (bcmissu_struct_info_get(unit,
        ...
        port_map.pport_map = (bcmtm_pport_map_t *)
                              ((uint8_t *)(port_map.pport_map) + offset);
    }

    offset = 0;
    data_sz = sizeof(bcmtm_mport_map_t);
    num_inst = TH4_B0_MMU_PORTS_PER_PIPE * TH4_B0_PIPES_PER_DEV;
    alloc_sz = data_sz * num_inst;
    ...
    if (!warm || (bcmissu_struct_info_get(unit,
        ...
        port_map.mport_map = (bcmtm_mport_map_t *)
                              ((uint8_t *)(port_map.mport_map) + offset);
    }
 ...
 */
int
bcm56990_b0_tm_chip_q_get(int unit, uint32_t lport,
...
int
bcm56990_b0_tm_mmu_q_from_chip_q_get(int unit, int chip_q,
...
int
bcm56990_b0_tm_check_valid_itm(int unit, int itm)
{
    int rv;
    uint32_t itm_map;
...
int
bcm56990_b0_tm_num_uc_q_non_cpu_port_get(int unit)
{
    int num_mcq;
    num_mcq = bcm56990_b0_tm_num_mc_q_non_cpu_port_get(unit);
    return (TH4_B0_NUM_GP_QUEUES - num_mcq);
}

int
bcm56990_b0_tm_num_mc_q_non_cpu_port_get(int unit)
{
#define TH4_B0_MC_Q_MODE_DEFAULT 2
...
int
bcm56990_b0_ip_port_forwarding_bitmap_enable(int unit, bcmpc_pport_t pport) {
    uint32_t bmp[9] = {0};
    int lport;
    bcmdrd_sid_t sid;
    bcmltd_sid_t ltid = -1; /* Requesting LT ID not available */
...
int
bcm56990_b0_ip_port_forwarding_bitmap_disable(int unit, bcmpc_pport_t pport) {
    uint32_t bmp[9] = {0};
    int lport;
    bcmdrd_sid_t sid;
    bcmltd_sid_t ltid = -1; /* Requesting LT ID not available */
 ...
 */
int
bcm56990_b0_tm_validate (int unit,
                         size_t num_ports,
                         const bcmpc_mmu_port_cfg_t *new_cfg)
{
    int pport, lport, idx;
    ...
    int num_fp_ports_pipe[TH4_B0_PIPES_PER_DEV] = {0};
    uint64_t config_value;
    uint32_t flex_port[TH4_B0_PBLKS_PER_DEV] = {0};
    bcmtm_drv_info_t *drv_info;
    ...
    for (idx = 0; idx < (int)num_ports; idx++) {
        ...
        if (bcmtm_lport_is_cpu(unit, lport) || (bcmtm_lport_is_lb(unit, lport))) {
            if ((new_cfg[idx].num_lanes != TH4_B0_NUM_LANES_AUX_PORT) ||
                (new_cfg[idx].speed_max != BCMTM_TH4_B0_AUX_PORT_SPEED)) {
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
            continue;
        }
    ...
    if (num_fp_ports > TH4_B0_MAX_NUM_FP_PORTS) {
        /* Check total number of front panel ports */
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    for (pipe = 0; pipe < TH4_B0_PIPES_PER_DEV; pipe++) {
        if (num_fp_ports_pipe[pipe] > TH4_B0_MAX_NUM_FP_PORTS_PIPE) {
            /* Check total number of front panel ports per pipe */
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    }

    for (pm = 0; pm < TH4_B0_PBLKS_PER_DEV; pm++) {
        if (flex_port[pm] > drv_info->flex_eligible[pm]) {
            /* port is port macros are more than number of port configured. */
            LOG_WARN(BSL_LS_BCMBD_DEV, (BSL_META_U(unit,
                            "Minimum buffer reservation mismatch with Port configuration..\n")));
        }
    }
    if (num_400g_ports != 0) {
        /* Check frequency */
        if (th4_b0_freq_retrieve_from_enum(unit, config_value) < 1325) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    }
...
int
bcm56990_b0_tm_chip_init(int unit, bool warm)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_port_map_alloc(unit, warm));

    SHR_IF_ERR_EXIT
        (th4_b0_tm_port_map_populate(unit, warm));

    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_pfc_ddr_info_alloc(unit, warm));
    ...
    SHR_BITSET(ltmbuf_arr, TH4_B0_CPU_LPORT);
    BCMTM_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, CPU_PBMm, -1, (void *)&pt_dyn_info,
                                ltmbuf_arr));

    bcm56990_b0_bcmtm_dev_info_set(unit);

    /* Device level init sequence. Skipping all in case of warmboot. */
    if (!warm) {
        SHR_IF_ERR_EXIT
            (th4_b0_en_mmu_refresh(unit));
        SHR_IF_ERR_EXIT
            (th4_b0_cell_pointer_init(unit));
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_intfi_enable(unit));
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_thdo_pipe_itm_config(unit));
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_rqe_queue_number_init(unit));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_idb_general_init(unit));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_tdm_mmu_init(unit));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_rqe_threshold_init(unit));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_mmu_config_init_thresholds(unit));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_wred_enable(unit));
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_ct_init(unit));
    }
    /* Scheduler shaper chip init */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_scheduler_shaper_chip_init(unit, warm));
    /* OBM threshold init. */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_obm_init(unit, warm));
    /* BST Init. */
    SHR_IF_ERR_EXIT
        (bcmtm_bst_init(unit, warm));

    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_ebst_init(unit, warm));

    /* Multicast init. */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_multicast_init(unit, warm));
    /* WRED init. */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_wred_chip_init(unit, warm));

    if (!warm) {
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_imm_populate(unit));
    }
...
int
bcm56990_b0_tm_port_delete(int unit, bcmtm_pport_t pport)
{
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_port_idb_delete(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_port_mmu_delete(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_port_edb_delete(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_imm_update(unit, pport, 0));
exit:
    SHR_FUNC_EXIT();
}

int
bcm56990_b0_tm_port_add(int unit, bcmtm_pport_t pport)
{
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_port_idb_add(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_port_mmu_add(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_port_edb_add(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_imm_update(unit, pport, 1));
exit:
    SHR_FUNC_EXIT();
}

int
bcm56990_b0_tm_check_ep_mmu_credit(int unit, int lport)
{
    int pport, mmu_port;
    bcmtm_port_map_info_t *port_map;
...
int
bcm56990_b0_tm_check_port_ep_credit(int unit, int lport)
{
    int pport;
    bcmtm_port_map_info_t *port_map;
...
int
bcm56990_b0_tm_mchip_port_lport_get(int unit, int mchip_port, int *lport)
{
    ...
    mlocal_port = mchip_port % TH4_B0_DEV_PORTS_PER_PIPE;
    pipe = mchip_port / TH4_B0_DEV_PORTS_PER_PIPE;

    mport = pipe * TH4_B0_MMU_PORTS_PER_PIPE + mlocal_port;
