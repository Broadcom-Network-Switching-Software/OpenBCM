/*! \file bcm56990_b0_tm_thd_mc_q.c
 *
                           ...
                           soc_mmu_cfg_buf_queue_t *buf_queue)
{
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        num_q = bcm56990_b0_tm_num_mc_q_non_cpu_port_get(unit);
    } else {
        num_q = SOC_TH4_B0_NUM_CPU_QUEUES;
    }
    if (queue >= num_q) {
...
bcm56990_b0_tm_thd_mc_q_populate(int unit)
{
    ...
    for (lport = 0; lport < SOC_MMU_CFG_PORT_MAX; lport++) {
        th4_b0_mmu_config_buf_queue_get(unit, lport, &buf_queue);
        if (bcmtm_lport_is_cpu(unit, lport)) {
            max_num_mcq = SOC_TH4_B0_NUM_CPU_QUEUES;
        } else {
            max_num_mcq = TH4_B0_MAX_NUM_MC_Q;
        }
