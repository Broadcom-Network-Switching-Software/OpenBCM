/*! \file bcm56990_b0_tm_thd.c
 *
 * File containing mmu threshold init sequence for bcm56990_b0
 */
/*
...
th4_b0_mmu_init_dev_config(int unit, soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    devcfg->max_pkt_byte = TH4_B0_MMU_MAX_PACKET_BYTES;
    devcfg->mmu_hdr_byte = TH4_B0_MMU_PACKET_HEADER_BYTES;
    devcfg->jumbo_pkt_size = TH4_B0_MMU_JUMBO_FRAME_BYTES;
    devcfg->default_mtu_size = TH4_B0_MMU_DEFAULT_MTU_BYTES;
    devcfg->mmu_cell_size = TH4_B0_MMU_BYTES_PER_CELL;
    devcfg->mmu_total_cell = TH4_B0_MMU_TOTAL_CELLS_PER_ITM;
    devcfg->num_pg = TH4_B0_MMU_NUM_PG;
    devcfg->num_service_pool = TH4_B0_MMU_NUM_POOL;
    devcfg->flags = MMU_CFG_F_PORT_MIN | MMU_CFG_F_PORT_POOL_MIN |
                    MMU_CFG_F_RQE | MMU_CFG_F_EGR_MCQ_ENTRY;
    devcfg->total_mcq_entry = TH4_B0_MMU_TOTAL_MCQ_ENTRY(unit);
    devcfg->rqe_queue_num = TH4_B0_MMU_NUM_RQE_QUEUES;
}
...
th4_b0_default_pg_headroom(int unit, int port)
{
    ...
    if (speed >= 400000) {
        hdrm = TH4_B0_PG_HEADROOM_OVERSUB_400G;
    } else if (speed >= 200000) {
        hdrm = TH4_B0_PG_HEADROOM_OVERSUB_200G;
    } else if (speed >= 100000) {
        hdrm = TH4_B0_PG_HEADROOM_OVERSUB_100G;
    } else if (speed >= 40000) {
        hdrm = TH4_B0_PG_HEADROOM_OVERSUB_50G;
    } else if (speed >= 25000) {
        hdrm = TH4_B0_PG_HEADROOM_OVERSUB_25G;
    } else if (speed >= 10000) {
        hdrm = TH4_B0_PG_HEADROOM_OVERSUB_10G;
    } else {
        hdrm = TH4_B0_PG_HEADROOM_OVERSUB_10G;
...
static int
th4_b0_get_shared_limit(int unit, soc_mmu_cfg_buf_t *buf, int pool_id)
{
    soc_mmu_cfg_buf_pool_t *buf_pool;
    int limit, limit_itm;
    int itm;

    limit = TH4_B0_MMU_TOTAL_CELLS_PER_ITM;
...
static int
th4_b0_get_hdrm_limit(int unit, soc_mmu_cfg_buf_t *buf, int pool_id)
{
    soc_mmu_cfg_buf_pool_t *buf_pool;
    int hdrm, hdrm_itm[TH4_B0_ITMS_PER_DEV];
    int itm;
...
static void
th4_b0_mmu_config_mapping_profiles_set(int unit, soc_mmu_cfg_buf_t *buf,
...
static int
th4_b0_mmu_config_mapping_profiles_set_hw(int unit, soc_mmu_cfg_buf_t *buf,
                                       soc_mmu_device_info_t *devcfg, int lossless)
{
    soc_mmu_cfg_buf_mapping_profile_t *buf_map_profiles;
    int idx, loop;
                              ...
                              soc_mmu_cfg_buf_qgroup_t *queue_grp)
{
    ...
    default_mtu_cells = MMU_CFG_MMU_BYTES_TO_CELLS(TH4_B0_MMU_DEFAULT_MTU_BYTES +
                            ...
                            soc_mmu_cfg_buf_port_pool_t *buf_port_pool)
{
    ...
    total_pool_size = TH4_B0_MMU_TOTAL_CELLS_PER_ITM;
    ...
    if (pg == TH4_B0_MMU_NUM_PG - 1) {
        ...
        buf_prigroup->headroom = (lossless == 0) ? 0 :
                      ((port_map->lport_map[port].cur_speed != -1) ?
                       th4_b0_default_pg_headroom(unit, port) : 0);
                        ...
                        soc_mmu_device_info_t *devcfg,  int lossless)
{
    ...
    default_mtu_cells = MMU_CFG_MMU_BYTES_TO_CELLS(devcfg->default_mtu_size +
                                                       devcfg->mmu_hdr_byte,
     ...
     */
    for (idx = 0; idx < TH4_B0_MMU_NUM_PG; idx++) {
        ...
        /* PG7 is lossless PG */
        if (idx == TH4_B0_MMU_NUM_PG - 1) {
            if (drv_info->skip_buf_resv == 0) {
                buf_prigroup->guarantee = (lossless == 0) ? 0 : default_mtu_cells;
                buf_prigroup->headroom = (lossless == 0) ? 0 :
                    ((port_map->lport_map[port].cur_speed != -1) ?
                     th4_b0_default_pg_headroom(unit, port) : 0);
    ...
    for (idx = 0; idx < TH4_B0_MMU_NUM_POOL; idx++) {
                        ...
                        soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (idx = 0; idx < TH4_B0_MMU_NUM_POOL; idx++) {
        ...
        if (idx == 0) {
            /* 100% scale up by 100 */
            buf_pool->total_mcq_entry = SOC_TH4_B0_MMU_MCQ_ENTRY_PER_ITM;
            buf_pool->total = TH4_B0_MMU_TOTAL_CELLS_PER_ITM;
            /* 100 Percent */
    ...
    for (idx = 0; idx < SOC_TH4_B0_MMU_CFG_QGROUP_MAX; idx++) {
    ...
    buf_port = &buf->ports[port];
    if (bcmtm_lport_is_cpu(unit, port)) {
        num_cosq = SOC_TH4_B0_NUM_CPU_QUEUES;
    } else {
        num_cosq = TH4_B0_NUM_GP_QUEUES;
    }
    ...
    num_cosq = port_map->lport_map[port].num_mcq +
               port_map->lport_map[port].num_ucq;
    for (idx = 0; idx < num_cosq; idx++) {
...
static void
th4_b0_mmu_config_thresholds_pool_set(int unit, soc_mmu_cfg_buf_t *buf,
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    bcmtm_drv_info_t *drv_info;
    soc_mmu_cfg_buf_pool_t *buf_pool;
    ...
    int itm, pipe, idx;
    int pm, itm_pipe_map;
    int total_headroom_itm[TH4_B0_ITMS_PER_DEV] = {0};
    int total_pgmin_itm[TH4_B0_ITMS_PER_DEV] = {0};
    int total_qgrpmin_itm[TH4_B0_ITMS_PER_DEV] = {0};
    int total_qmin_itm[TH4_B0_ITMS_PER_DEV] = {0};
    int total_num_ports = 0;
    int ports_in_pipe[TH4_B0_PIPES_PER_DEV] = {0};
    int ports_in_pm[TH4_B0_PBLKS_PER_DEV] = {0};
    ...
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm ++) {
        ...
        if ((lossless == 1) || (lossless == 2)) {
            ...
            if (drv_info->skip_buf_resv == 0) {
                for (pipe = 0; pipe < TH4_B0_PIPES_PER_DEV; pipe++) {
                    if (itm_pipe_map & (1 <<pipe)) {
                        for (pm = pipe * TH4_B0_PBLKS_PER_PIPE;
                                pm < (pipe + 1) * TH4_B0_PBLKS_PER_PIPE;
                                pm++) {
                            ports_in_pm[pm] = drv_info->flex_eligible[pm];
                            if (ports_in_pm[pm] > 4) {
                                /* Treat as 8x50G PM. */
                                total_headroom_itm[itm] += TH4_B0_PG_HEADROOM_OVERSUB_50G * 8;
                            } else if (ports_in_pm[pm] > 2) {
                                /* Treat as 4x100G PM. */
                                total_headroom_itm[itm] += TH4_B0_PG_HEADROOM_OVERSUB_100G * 4;
                            } else if (ports_in_pm[pm] == 2) {
                                /* Treat as 2x200G PM. */
                                total_headroom_itm[itm] += TH4_B0_PG_HEADROOM_OVERSUB_200G * 2;
                            } else if (ports_in_pm[pm] == 1) {
                                /* Treat as 1x400G PM. */
                                total_headroom_itm[itm] += TH4_B0_PG_HEADROOM_OVERSUB_400G;
                            }
            ...
            total_num_ports += TH4_B0_NUM_CPU_PORTS +
            (TH4_B0_NUM_LB_PORTS + TH4_B0_NUM_MGMT_PORTS) / TH4_B0_ITMS_PER_DEV;
    ...
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm ++) {
        ...
        if (drv_info->skip_buf_resv == 0) {
            ...
            for (pipe = 0; pipe < TH4_B0_PIPES_PER_DEV; pipe++) {
                if (itm_pipe_map & (1 << pipe)) {
                    for (pm = pipe * TH4_B0_PBLKS_PER_PIPE;
                            pm < (pipe + 1) * TH4_B0_PBLKS_PER_PIPE; pm++) {
                        ports_in_pm[pm] = drv_info->flex_eligible[pm];
                        ports_in_pipe[pipe] += ports_in_pm[pm];
                    }
                    /* Each pipe cannot have more than 16 ports. */
        ...
        total_qmin_itm[itm] = default_mtu_cells * SOC_TH4_B0_NUM_CPU_QUEUES;
    }
    ...
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm ++) {
        ...
        total_qgrpmin_itm[itm] = (total_num_ports + TH4_B0_NUM_LB_PORTS +
                                  TH4_B0_NUM_MGMT_PORTS) * (2 * default_mtu_cells);
    }
...
static void
th4_b0_mmu_config_thresholds_set(int unit, soc_mmu_cfg_buf_t *buf,
                              soc_mmu_device_info_t *devcfg, int lossless)
{
    int port;

    for (port = 0; port< SOC_MMU_CFG_PORT_MAX; port++) {
        th4_b0_mmu_config_thdi_set(unit, port, buf, devcfg, lossless);

        th4_b0_mmu_config_thdo_set(unit, port, buf, devcfg, lossless);
    }

    th4_b0_mmu_config_thresholds_pool_set(unit, buf, devcfg, lossless);
}


static int
th4_b0_mmu_config_thdi_per_itm_set_hw(int unit, soc_mmu_cfg_buf_t *buf,
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    uint32_t fval;
    bcmtm_pt_info_t pt_dyn_info = {0};
    ...
    jumbo_frame_cells = MMU_CFG_MMU_BYTES_TO_CELLS(devcfg->jumbo_pkt_size +
                                                       devcfg->mmu_hdr_byte,
                                                       devcfg->mmu_cell_size);
    ...
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm ++) {
        ...
        fval = TH4_B0_MMU_PHYSICAL_CELLS_PER_ITM - TH4_B0_MMU_RSVD_CELLS_CFAP_PER_ITM;
        ...
        BCMTM_PT_DYN_INFO(pt_dyn_info, 0, itm);
        fval = TH4_B0_CFAP_BANK_FULL_LIMIT;
        SHR_IF_ERR_EXIT
    ...
    ltmbuf = 0;
    fval = TH4_B0_SPID_OVERRIDE_ENABLE_DEFAULT;
    SHR_IF_ERR_EXIT
    ...
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm ++) {
        ...
        for (idx = 0; idx < TH4_B0_MMU_NUM_POOL; idx++) {
            ...
            limit = th4_b0_get_shared_limit(unit, buf, idx);
            headroom = th4_b0_get_hdrm_limit(unit, buf, idx);
            ...
            ltmbuf = 0;
            fval = TH4_B0_SPAP_YELLOW_OFFSET_DEFAULT;
            SHR_IF_ERR_EXIT
            ...
            ltmbuf = 0;
            fval = TH4_B0_SPAP_RED_OFFSET_DEFAULT;
            SHR_IF_ERR_EXIT
...
static int
th4_b0_mmu_config_thdo_per_itm_set_hw(int unit, soc_mmu_cfg_buf_t *buf,
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    soc_mmu_cfg_buf_pool_t *buf_pool;
    uint32_t fval;
    ...
    for (itm = 0; itm < SOC_MMU_CFG_ITM_MAX; itm ++) {
        ...
        /* Loop to write all instances. */
        ltmbuf = 0;
        fval = TH4_B0_LAST_PKT_ACCEPT_MODE_POOL_DEFAULT;
        SHR_IF_ERR_EXIT
        ...
        for (idx = 0; idx < TH4_B0_MMU_NUM_POOL; idx++) {
            ...
            if (buf_pool->total_mcq_entry != 0) {
                mcq_resume_limit = buf_pool->total_mcq_entry
                                   - buf_pool->mcq_entry_reserved
                                   - SOC_TH4_B0_NUM_CPU_QUEUES * default_mtu_cells
                                   - pool_resume;
            } else {
                mcq_resume_limit = 0;
            }
            ...
            if (buf_pool->total_mcq_entry != 0) {
                fval = (buf_pool->total_mcq_entry - buf_pool->mcq_entry_reserved
                        - SOC_TH4_B0_NUM_CPU_QUEUES * default_mtu_cells)/8;
            } else {
                fval = 0;
            }
            SHR_IF_ERR_EXIT
...
soc_mmu_cfg_buf_t *th4_b0_soc_mmu_buf_cfg_alloc(int unit)
{
 ...
 */
int
bcm56990_b0_tm_mmu_config_init_thresholds(int unit)
{
    soc_mmu_cfg_buf_t *buf;
    ...
    /* Default config */
    th4_b0_mmu_config_mapping_profiles_set(unit, buf, &devcfg, drv_info->lossless);
    th4_b0_mmu_config_thresholds_set(unit, buf, &devcfg, drv_info->lossless);
    SHR_IF_ERR_EXIT(
            th4_b0_mmu_config_mapping_profiles_set_hw(unit, buf, &devcfg,
                drv_info->lossless));
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_config_thdi_per_itm_set_hw(unit, buf, &devcfg, drv_info->lossless));
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_config_thdo_per_itm_set_hw(unit, buf, &devcfg, drv_info->lossless));
...
int
bcm56990_b0_tm_rqe_threshold_init(int unit) {
    uint32_t fval,idx;
    /* Requesting LT ID not available */
    bcmltd_sid_t ltid = -1;
    uint32_t ltmbuf = 0;
    ...
    for (i = 0; i < count; i++) {
        for (idx = 0; idx < TH4_B0_MMU_RQE_QUEUE_NUM; idx++) {
            BCMTM_PT_DYN_INFO(pt_dyn_info, idx, 0);
            ltmbuf = 1;
            SHR_IF_ERR_EXIT
    ...
    for (i = 0; i < count; i++) {
        for (idx = 0; idx < SOC_MMU_CFG_ITM_MAX; idx++) {
            if (bcm56990_b0_tm_check_valid_itm(unit, idx) != SHR_E_NONE) {
                continue;
            }
    ...
    for (idx = 0; idx < TH4_B0_MMU_RQE_QUEUE_NUM; idx++) {
        ...
        ltmbuf = TH4_B0_THDR_QE_CONFIG1_VALUE;
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, reg, ltid,
                       (void*)&pt_dyn_info, &ltmbuf));
        fval = TH4_B0_MMU_THDR_QE_RESET_OFFSET;
        SHR_IF_ERR_EXIT
        ...
        fval = TH4_B0_THDR_QE_CONIFG_PRI_LIMIT;
        SHR_IF_ERR_EXIT
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, reg, ltid,
                      (void*)&pt_dyn_info, &ltmbuf));
    fval = TH4_B0_THDR_QE_SP_SHARED_LIMIT;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, reg, SHARED_LIMITf, &ltmbuf, &fval));
    fval = TH4_B0_THDR_QE_SP_RESUME_LIMIT;
    SHR_IF_ERR_EXIT
                                           ...
                                           int *itm, int *pool, bool is_hdrm)
{
    ...
    *pool = fval;
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_thdi_port_to_itm_mapping_get(unit, lport, itm));
                                    ...
                                    bool check_only)
{
    ...
    for (; pool_start <= pool_end; pool_start++) {
        ...
        if (reason != ING_SERVICE_POOL_UPDATE) {
             ...
             */
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_ing_pool_shared_limit_lt_update(unit, itm, pool_start,
                                                                new_limit));
        }
        ...
        if (reason != EGR_SERVICE_POOL_UPDATE) {
             ...
             */
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_egr_pool_shared_limit_lt_update(unit, itm, pool_start,
                                                                new_limit));
        }
    }
