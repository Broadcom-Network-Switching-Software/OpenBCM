/*! \file bcm56990_b0_tm_mmu_flexport.c
 *
...
/*! @fn int th4_b0_mmu_get_thdi_bst_mode(int unit, int *thdi_bst_mode)
 *  @param unit Device number
 *  @param *thdi_bst_mode Return value thdi bst mode
 *  @brief API to return the THDI bst mode programmed
 */
static int
th4_b0_mmu_get_thdi_bst_mode(int unit, uint32_t *thdi_bst_mode)
{
    int inst = 0;
    uint32_t ltmbuf = 0;
    uint32_t fval;
...
/*! @fn int th4_b0_mmu_get_thdo_bst_mode(int unit, int *thdo_bst_mode)
 *  @param unit Device number
 *  @param *thdo_bst_mode Return value thdo bst mode
 *  @brief API to return the thdo bst mode programmed
 */
static int
th4_b0_mmu_get_thdo_bst_mode(int unit, uint32_t *thdo_bst_mode)
{
    int inst = 0;
    uint32_t ltmbuf = 0;
    uint32_t fval;
...
/*!
 * @fn int th4_b0_mmu_qsch_port_flush(int unit,
 ...
 */
static int
th4_b0_mmu_qsch_port_flush(int unit, int lport, uint32_t set_val)
{
    uint32_t temp;
    uint32_t ltmbuf;
    uint32_t new_val;
...
/*!
 * @fn int th4_b0_mmu_rqe_port_flush(int unit, int itm)
 *  @param unit Device number
 ...
 */
static int
th4_b0_mmu_rqe_port_flush(int unit, int itm)
{
    int inst;
    int count = 0;
    uint32_t lbuf;
...
/*!
 * @fn int th4_b0_mmu_wait_ebctm_empty(int unit,
 *                                   int lport)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
th4_b0_mmu_wait_ebctm_empty(int unit, int lport)
{
    uint32_t ltmbuf;
    uint32_t empty_val;
    int inst, count = 0;
...
/*!
 * @fn int th4_b0_mmu_mtro_port_metering_config(int unit,
 *                                           int lport,
 ...
 */
static int
th4_b0_mmu_mtro_port_metering_config(int unit,
                                  int lport,
                                  uint32_t set_val_mtro)
{
...
/*!
 * @fn int th4_b0_mmu_clear_mtro_bucket_mems(int unit,
 *                                        int lport)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
th4_b0_mmu_clear_mtro_bucket_mems(int unit, int lport)
{
    int cos;
    int lcl_port;
    int pipe_number;
    ...
    /* L0 Bucket Memory */
    for (cos = 0; cos < TH4_B0_MMU_NUM_COS; cos++) {

        mem_indx = (lcl_port * TH4_B0_MMU_NUM_COS) + cos;
        BCMTM_PT_DYN_INFO(pt_dyn_info, mem_indx, pipe_number);
...
/*!
 * @fn int th4_b0_mmu_thdi_bst_clr(int unit,
 *                               int lport)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
th4_b0_mmu_thdi_bst_clr(int unit, int lport)
{
    int pipe_number;
    int local_mmu_port;
    int pport, mmu_port;
    ...
    local_mmu_port = mmu_port % TH4_B0_MMU_PORT_PIPE_OFFSET;
    pipe_number = port_map->lport_map[lport].pipe;
...
th4_b0_mmu_thdo_bst_clr_queue(int unit, int lport)
{
    ...
    for (curr_idx = 0; curr_idx < num_queues; curr_idx++) {
        ...
        for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm++) {
            if (SHR_SUCCESS(bcm56990_b0_tm_check_valid_itm(unit, itm))) {
...
/*! @fn int th4_b0_mmu_thdo_bst_clr_port(int unit,
 ...
 */
static int
th4_b0_mmu_thdo_bst_clr_port(int unit, int global_device_port)
{
    int itm;
    uint32_t data[BCMTM_MAX_ENTRY_WSIZE];
    bcmdrd_sid_t mem;
...
/*! @fn int th4_b0_mmu_thdo_bst_clr_portsp(int unit,
 *                                   int pipe, int global_device_port)
 *  @param int unit Chip unit number
 *  @param int pipe Egress pipe in which port exists
 *  @param int global_device_port global device port
 ...
 */
static int
th4_b0_mmu_thdo_bst_clr_portsp(int unit, int global_device_port)
{
    int itm;
    int sp;
    int mem_idx;
    ...
    for (sp = 0; sp < TH4_B0_MMU_NUM_SPS; sp++) {
...
/*! @fn int th4_b0_mmu_thdo_bst_clr(int unit,
 ...
 */
static int
th4_b0_mmu_thdo_bst_clr(int unit, int lport)
{
    bcmtm_port_map_info_t *port_map;
    ...
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_thdo_bst_clr_queue(unit, lport));
    /* Clear per-port memories. */
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_thdo_bst_clr_port(unit, lport));
    /* Clear per-port-per-spid memories. */
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_thdo_bst_clr_portsp(unit, lport));
...
/*! @fn int th4_b0_mmu_thdo_pktstat_clr_queue(int unit, int pipe,
 *              int global_mmu_port)
 *  @param int unit Chip unit number
 ...
 */
static int
th4_b0_mmu_thdo_pktstat_clr_queue(int unit, int lport)
{
    int itm;
    int curr_idx;
    int num_queues;
...
/*! @fn int th4_b0_mmu_thdo_pktstat_clr_port(int unit, int pipe,
 ...
 */
static int
th4_b0_mmu_thdo_pktstat_clr_port(int unit, int global_device_port)
{
    int itm;
    ...
    local_device_port = global_device_port % TH4_B0_MMU_PORTS_PER_PIPE;
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm++) {
        if (SHR_SUCCESS(bcm56990_b0_tm_check_valid_itm(unit, itm))) {
...
/*! @fn int th4_b0_mmu_thdo_pktstat_clr(int unit,
 *              port_resource_t *port_resource_t)
 *  @param int unit Chip unit number
 *  @param port_resource_t *port_resource_t Port resource struct containing
 *         epipe and port info
 ...
 */
static int
th4_b0_mmu_thdo_pktstat_clr(int unit, int lport)
{
    bcmtm_port_map_info_t *port_map;
    ...
    /* Clear per-queue memories. */
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_thdo_pktstat_clr_queue(unit, lport));
    /* Clear per-port memories. */
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_thdo_pktstat_clr_port(unit, lport));
exit:
    SHR_FUNC_EXIT();
}

/*! @fn int th4_b0_mmu_clear_qsch_credit_memories(int unit,
 *              port_resource_t *port_resource_t)
 *  @param unit Device number
 ...
 */
static int
th4_b0_mmu_clear_qsch_credit_memories(int unit, int lport)
{
    int itm;
    int lcl_port;
    int pport;
    ...
    for (schq = 0; schq < TH4_B0_MMU_NUM_L0_NODES_PER_HSP_PORT; schq++) {

        mem_indx = (lcl_port * TH4_B0_MMU_NUM_L0_NODES_PER_HSP_PORT) + schq;
    ...
    for (schq = 0; schq < TH4_B0_MMU_NUM_L0_NODES_PER_HSP_PORT; schq++) {
        for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm++) {
            if (SHR_SUCCESS(bcm56990_b0_tm_check_valid_itm(unit, itm))) {
                mem_indx =
                          (((lcl_port * TH4_B0_MMU_NUM_L0_NODES_PER_HSP_PORT) +
                            schq) * 2) + itm;
...
/*! @fn int th4_b0_mmu_wred_clr(int unit,
 *              port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
static int
th4_b0_mmu_wred_clr(int unit, int lport)
{
    int q, sp;
    int itm;
    ...
    /* Per-UCQ loop */
    for (q = 0; q < TH4_B0_MMU_NUM_Q_PER_HSP_PORT; q++) {

        cosq_mem_index = (lport * TH4_B0_MMU_NUM_Q_PER_HSP_PORT) + q;
    ...
    for (sp = 0; sp < TH4_B0_MMU_NUM_SPS; sp++) {
        ...
        portsp_mem_index = (lport * TH4_B0_MMU_NUM_SPS) + sp;
...
static int
th4_b0_mmu_stat_clear(int unit, int lport) {
    uint32_t thdi_bst_mode = 0;
    uint32_t thdo_bst_mode = 0;

    SHR_FUNC_ENTER(unit);
    th4_b0_mmu_get_thdi_bst_mode(unit, &thdi_bst_mode);
    th4_b0_mmu_get_thdo_bst_mode(unit, &thdo_bst_mode);
    /* Clear MTRO bucket memories. */
    LOG_DEBUG(BSL_LS_SOC_PORT,
              (BSL_META_U(unit,
                          "before th4_b0_mmu_clear_mtro_bucket_mems\n")));
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_clear_mtro_bucket_mems(unit, lport));

    /* Clear QSCH credit memories. */
    LOG_DEBUG(BSL_LS_SOC_PORT,
              (BSL_META_U(unit,
                          "th4_b0_mmu_clear_qsch_credit_memories\n")));
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_clear_qsch_credit_memories(unit, lport));

    /* Clear WRED Avg_Qsize instead of waiting for background process. */
    LOG_DEBUG(BSL_LS_SOC_PORT,
              (BSL_META_U(unit, "before th4_b0_mmu_wred_clr\n")));
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_wred_clr(unit, lport));

    if (thdi_bst_mode == TH4_B0_MMU_BST_WMARK_MODE) {
        /* Clear THDI BST in watermark mode. */
        LOG_DEBUG(BSL_LS_SOC_PORT,
                  (BSL_META_U(unit, "before th4_b0_mmu_thdi_bst_clr\n")));
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_thdi_bst_clr(unit, lport));
    }
    if (thdo_bst_mode == TH4_B0_MMU_BST_WMARK_MODE) {
        /* Clear THDO BST counters in watermark mode. */
        LOG_DEBUG(BSL_LS_SOC_PORT,
                  (BSL_META_U(unit, "before th4_b0_mmu_thdo_bst_clr\n")));
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_thdo_bst_clr(unit, lport));

        /* Clear PktStat counters in THDO for Queues. */
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_thdo_pktstat_clr(unit, lport));
    }
exit:
    SHR_FUNC_EXIT();
}
...
/*!
 *  @fn int th4_b0_flex_mmu_port_flush(int unit,
 *                                  int lport)
 *  @param unit Device number
 *  @param lport Port number
 ...
 */
static int
th4_b0_flex_mmu_port_flush(int unit, int lport)
{
    int itm;
    ...
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_qsch_port_flush(unit, lport, TH4_B0_MMU_FLUSH_ON));

    LOG_DEBUG(BSL_LS_SOC_PORT,
              (BSL_META_U(unit, "before mtro_port_metering_config\n")));
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_mtro_port_metering_config(unit, lport, TH4_B0_MMU_FLUSH_ON));
    ...
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm++) {
        if (SHR_SUCCESS(bcm56990_b0_tm_check_valid_itm(unit, itm))) {
            ...
            SHR_IF_ERR_EXIT
                (th4_b0_mmu_rqe_port_flush(unit, itm));
        }
    }
    LOG_DEBUG(BSL_LS_SOC_PORT,
    ...
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_wait_ebctm_empty(unit, lport));
    ...
    SHR_IF_ERR_EXIT
        (th4_b0_mmu_mtro_port_metering_config(unit, lport, TH4_B0_MMU_FLUSH_OFF));

    SHR_IF_ERR_EXIT
        (th4_b0_mmu_qsch_port_flush(unit, lport, TH4_B0_MMU_FLUSH_OFF));
...
static int
th4_b0_mmu_port_map(int unit, int pport, int port_add)
{
    bcmtm_port_map_info_t *port_map;
    int mmu_port, dev_port, mmu_chip_port;
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_sid_t sid ;
    ...
    /* Valid physical port */
    dev_port = port_map->pport_map[pport].lport;
    mmu_port = port_map->pport_map[pport].mport;
    mmu_chip_port = port_map->lport_map[dev_port].mchip_port;
    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(unit, "mmu_portmapping: lport=%3d\n"), dev_port));

    SHR_IF_ERR_EXIT(th4_b0_tdm_global_to_local_phy_num(pport, &local_phys_port));
    /*
    ...
    for (sid_ind = 0; sid_ind < COUNTOF(sid_list); sid_ind++) {
        ...
        switch (sid) {
            case MMU_CRB_DEVICE_PORT_TO_MMU_PORT_MAPPINGr:
                ...
                inst = dev_port;
                index = 0;
                fval = port_add ? mmu_port : TH4_B0_INVALID_MMU_PORT;
                break;
            case MMU_EBCFP_MMU_PORT_TO_DEVICE_PORT_MAPPINGr:
                fid = DEVICE_PORTf;
                inst = mmu_port;
                index = 0;
                fval = port_add ? dev_port : TH4_B0_INVALID_DEV_PORT;
                break;
            ...
            case MMU_INTFI_MMU_PORT_TO_PHY_PORT_MAPPINGr:
                ...
                inst = mmu_port;
                index = 0;
                fval = port_add ? local_phys_port : TH4_B0_INVALID_LOCAL_PHYS_PORT;
                break;
...
static int
th4_b0_mmu_port_speed_encode(int unit, int pport)
{
    bcmtm_port_map_info_t *port_map;
    int dev_port;
    int mmu_port;
    ...
    dev_port = port_map->pport_map[pport].lport;
    mmu_port = port_map->pport_map[pport].mport;
    speed = port_map->lport_map[dev_port].cur_speed;
    LOG_VERBOSE(BSL_LOG_MODULE,
              (BSL_META_U(unit,
    ...
    SHR_IF_ERR_EXIT(th4_b0_mmu_get_speed_decode(unit, speed, &fval));
...
static int
th4_b0_mmu_port_rx_enables(int unit, int pport)
{
    bcmtm_port_map_info_t *port_map;
    int mmu_port, lport;
    bcmtm_pt_info_t pt_dyn_info = {0};
...
bcm56990_b0_tm_port_mmu_down(int unit, bcmtm_pport_t pport)
{
    ...
    if (lport != -1) {
        if (!sim) {
            SHR_IF_ERR_EXIT
                (th4_b0_flex_mmu_port_flush(unit, lport));
        }
    }
...
bcm56990_b0_tm_port_mmu_delete(int unit, bcmtm_pport_t pport)
{
    ...
    if (lport != -1) {
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_tdm_port_delete(unit, lport));
        /* Disable metering before clearing bucket mems */
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_mtro_port_metering_config(unit, lport, TH4_B0_MMU_FLUSH_ON));
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_stat_clear(unit, lport));
        /* Reenable metering */
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_mtro_port_metering_config(unit, lport, TH4_B0_MMU_FLUSH_OFF));
        SHR_IF_ERR_EXIT(th4_b0_mmu_port_map(unit, pport, 0));
    }
...
bcm56990_b0_tm_port_mmu_add (int unit, int pport) {
    ...
    if (lport != -1) {
        SHR_IF_ERR_EXIT
            (th4_b0_mmu_tdm_port_add(unit, lport));
        SHR_IF_ERR_EXIT(th4_b0_mmu_port_map(unit, pport, 1));
        SHR_IF_ERR_EXIT(th4_b0_mmu_port_rx_enables(unit, pport));
        SHR_IF_ERR_EXIT(th4_b0_mmu_port_speed_encode(unit, pport));
    }
