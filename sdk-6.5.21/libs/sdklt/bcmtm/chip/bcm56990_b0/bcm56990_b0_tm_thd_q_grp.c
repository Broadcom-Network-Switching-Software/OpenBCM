/*! \file bcm56990_b0_tm_thd_q_grp.c
 *
                    ...
                    bcmltd_fields_t *output_fields, uint8_t action)
{
    ...
    th4_b0_mmu_config_buf_qgroup_get(unit, lport, &queue_grp);
    ...
    while (data_ptr) {
        ...
        switch (data_ptr->id) {
            case TM_THD_Q_GRPt_UC_Q_GRP_MIN_GUARANTEE_CELLSf:
                ...
                if (SHR_SUCCESS
                        (bcm56990_b0_tm_shared_buffer_update(unit, 0, -1, delta,
                                                             RESERVED_BUFFER_UPDATE,
                                                             TRUE)) &&
                    SHR_SUCCESS
                        (bcm56990_b0_tm_shared_buffer_update(unit, 1, -1, delta,
                                                             RESERVED_BUFFER_UPDATE,
                                                             TRUE))) {
