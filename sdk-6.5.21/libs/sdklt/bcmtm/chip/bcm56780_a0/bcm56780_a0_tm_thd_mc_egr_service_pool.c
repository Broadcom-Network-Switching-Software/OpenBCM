/*! \file bcm56780_a0_tm_thd_mc_egr_service_pool.c
 *
...
mc_cqe_shared_buffer_update(int unit, int itm, int pool, uint32_t new_limit)
{
    ...
    uint32_t ltmbuf = 0;
    uint32_t pool_shared_limit[TD4_X9_MAX_SERVICE_POOLS] = {0};
    uint32_t unused_buf, fval;
    int pool_idx;

    SHR_FUNC_ENTER(unit);

    unused_buf = TD4_X9_MMU_MCQ_ENTRY_PER_ITM;
    for (pool_idx = 0; pool_idx < TD4_X9_MAX_SERVICE_POOLS; pool_idx++) {
                  ...
                  bcmltd_fields_t *entry, bcmltd_fields_t *ref_entry)
{
    ...
    write_flag = FALSE;
    if (CEILING(lt_red_resume, TD4_X9_GRANULARITY) <= red_limit &&
        lt_red_resume != lt_red_resume_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_red_resume, TD4_X9_GRANULARITY);
    } else {
                     ...
                     bcmltd_fields_t *entry, bcmltd_fields_t *ref_entry)
{
    ...
    write_flag = FALSE;
    if (CEILING(lt_yellow_resume, TD4_X9_GRANULARITY) <= yellow_limit &&
        lt_yellow_resume != lt_yellow_resume_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_yellow_resume, TD4_X9_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
                              ...
                              bcmltd_fields_t *entry, bcmltd_fields_t *ref_entry)
{
    ...
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
    shared_limit = shared_limit / TD4_X9_GRANULARITY;
    ...
    write_flag = FALSE;
    if (CEILING(lt_resume_limit, TD4_X9_GRANULARITY) <= shared_limit &&
        lt_resume_limit != lt_resume_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_resume_limit, TD4_X9_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    ...
    write_flag = FALSE;
    if (CEILING(lt_yellow_limit, TD4_X9_GRANULARITY) <= shared_limit &&
        lt_yellow_limit != lt_yellow_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_yellow_limit, TD4_X9_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    ...
    write_flag = FALSE;
    if (CEILING(lt_red_limit, TD4_X9_GRANULARITY) <= shared_limit &&
        lt_red_limit != lt_red_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_red_limit, TD4_X9_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
...
tm_thd_mc_egr_service_pool_populate_entry(int unit, int itm, int pool)
{
    ...
    resume_limit *= TD4_X9_GRANULARITY;
    ...
    yellow_limit *= TD4_X9_GRANULARITY;
    ...
    yellow_resume *= TD4_X9_GRANULARITY;
                               ...
                               bcmltd_fields_t *output_fields)
{
                ...
                if (rv == SHR_E_NONE) {
                    ...
                    SHR_IF_ERR_EXIT
                        (bcmtm_field_list_set(unit, output_fields,
                            TM_THD_MC_EGR_SERVICE_POOLt_SHARED_LIMIT_CELLS_OPERf,
                            0, data_ptr->data));
                    SHR_IF_ERR_EXIT
                        (bcmtm_field_list_set(unit, output_fields,
                            TM_THD_MC_EGR_SERVICE_POOLt_OPERATIONAL_STATEf,
                            0, VALID));
    ...
    /* Convert shared limit to 8-cell TD4_X9_GRANULARITY for comparison. */
    shared_limit = shared_limit / TD4_X9_GRANULARITY;
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
        ...
        switch (data_ptr->id) {
            case TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLSf:
                ...
                if (fval > shared_limit) {
                    ...
                    field_data = fval * TD4_X9_GRANULARITY;
                }
                SHR_IF_ERR_EXIT
...
static int
tm_thd_mc_egr_service_pool_delete(int unit, bcmltd_sid_t sid, int itm, int pool)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_thd_mc_egr_service_pool_update(unit, sid, itm, pool, data_fields,
                                        &dummy_fields));
                              ...
                              bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT
                (tm_thd_mc_egr_service_pool_update(unit, sid, itm, pool,
                                                data_fields, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_thd_mc_egr_service_pool_delete(unit, sid, itm, pool));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
bcm56780_a0_tm_thd_mc_egr_service_pool_register(int unit)
{
    ...
    tm_thd_cb_handler.stage    = tm_thd_mc_egr_service_pool_stage;
...
bcm56780_a0_tm_thd_mc_egr_service_pool_populate(int unit)
{
    ...
    for (pool = 0; pool < TD4_X9_MAX_SERVICE_POOLS; pool++) {
        SHR_IF_ERR_EXIT
            (tm_thd_mc_egr_service_pool_populate_entry(unit, 0, pool));
    }
exit:
    SHR_FUNC_EXIT();
}
