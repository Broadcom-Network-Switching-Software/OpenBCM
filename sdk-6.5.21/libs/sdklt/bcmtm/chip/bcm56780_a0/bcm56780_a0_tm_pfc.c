 ...
 * Local definitions
 */
#define TD4_X9_PFC_PRIORITY_MAX_INDEX         MAX_NUM_PFC_PRI - 1
#define TD4_X9_PFC_INDEX_INVALID              -1
#define TD4_X9_PFC_RX_PROFILE_NUM_ENTRIES     8
#define TD4_X9_PFC_COS_NUM                    MAX_NUM_COS
...
/*!
 * \brief Writes the PFC priority to MMU Queue,MMU Queue to EB group  mapping to H/W.
 ...
 */
static int
bcm56780_a0_tm_pfc_rx_mapping_hw_write(int unit,
                               int profile_id,
                               tm_pfc_pri_to_cos_map_config_t config_array[])
{
    ...
    for (pri = 0; pri < MAX_NUM_PFC_PRI; pri++) {
        ...
        reg_index = profile_id + pri * TD4_X9_PFC_RX_PROFILE_NUM_ENTRIES;
        SHR_BIT_ITER(cos_bmp, TD4_X9_PFC_COS_NUM, cos) {
            bcm56780_a0_tm_sched_node_mmu_q_get(unit,
                                                cos,
                                                profile, mmu_qs, q_type);
            for (count = 0; count < 2; count++) {
                if (mmu_qs[count] != -1) {
                    if (profile[cos].fc_uc &&  q_type[count] == MC_Q) {
                        continue;
                    }
                    mmuq_bmp |= 1U << mmu_qs[count];
                }
            }
        }

        /* Read the current limit from HW. */
        BCMTM_PT_DYN_INFO(pt_info, reg_index, 0);
        sal_memset(rval, 0, sizeof(rval));
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, pri_ptid, sid, &pt_info, rval));

        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, pri_ptid, mmuq_field, rval, &mmuq_bmp));

        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_write(unit, pri_ptid, sid, &pt_info, rval));
    }
exit:
    SHR_FUNC_EXIT();
}
                                      ...
                                      tm_pfc_pri_to_cos_map_config_t map[])
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56780_a0_tm_pfc_rx_mapping_hw_write(unit, profile_id, map));
exit:
    SHR_FUNC_EXIT();
}
