...
bcm56780_a0_tm_rdb_init(int unit)
{
    ...
    shr_timeout_t to;
    RDB0_FORCE_EOP_AND_STOP_ENABLEr_t rdb0_stop;
    RDB1_FORCE_EOP_AND_STOP_ENABLEr_t rdb1_stop;

    int idx, rv = 0;
    uint32_t epipe_map = 0;
    bool emul = bcmdrd_feature_enabled(unit, BCMDRD_FT_EMUL);
    bool sim = bcmdrd_feature_is_sim(unit);
    uint32_t a[2] = {0x0, 0x00008000};
    ...
    /*
     *War for 2 bit ecc error handling in rdb
     */
    for (idx = 0; idx < NUM_PP_PIPES; idx++) {
        if (epipe_map & (1 << idx)) {
            ioerr += READ_RDB0_FORCE_EOP_AND_STOP_ENABLEr(unit, idx, &rdb0_stop);
            RDB0_FORCE_EOP_AND_STOP_ENABLEr_RDB_BANK_0_ENABLEf_SET(rdb0_stop, 1);
            RDB0_FORCE_EOP_AND_STOP_ENABLEr_RDB_BANK_1_ENABLEf_SET(rdb0_stop, 1);
            RDB0_FORCE_EOP_AND_STOP_ENABLEr_RDB_BANK_2_ENABLEf_SET(rdb0_stop, 1);
            RDB0_FORCE_EOP_AND_STOP_ENABLEr_RDB_BANK_3_ENABLEf_SET(rdb0_stop, 1);
            ioerr += WRITE_RDB0_FORCE_EOP_AND_STOP_ENABLEr(unit, idx, rdb0_stop);

            ioerr += READ_RDB1_FORCE_EOP_AND_STOP_ENABLEr(unit, idx, &rdb1_stop);
            RDB1_FORCE_EOP_AND_STOP_ENABLEr_RDB_BANK_0_ENABLEf_SET(rdb1_stop, 1);
            RDB1_FORCE_EOP_AND_STOP_ENABLEr_RDB_BANK_1_ENABLEf_SET(rdb1_stop, 1);
            RDB1_FORCE_EOP_AND_STOP_ENABLEr_RDB_BANK_2_ENABLEf_SET(rdb1_stop, 1);
            RDB1_FORCE_EOP_AND_STOP_ENABLEr_RDB_BANK_3_ENABLEf_SET(rdb1_stop, 1);
            ioerr += WRITE_RDB1_FORCE_EOP_AND_STOP_ENABLEr(unit, idx, rdb1_stop);
        }
    }
...
bcm56780_a0_tm_dev_info_set(int unit)
{
    struct bcmtm_device_info bcm56780_a0_tm_dev_info = {
        ...
        .num_cells    = TD4_X9_MMU_TOTAL_CELLS_PER_ITM,
...
bcm56780_a0_tm_drv_info_populate(int unit, bool warm)
{
    ...
    fid = TM_THD_CONFIGt_SKIP_BUFFER_RESERVATIONf;
    SHR_IF_ERR_EXIT
        (bcmcfg_field_get(unit, ltid, fid, &config_value));
...
static int
bcm56780_a0_tm_refresh_enable(int unit)
{
    int idx;
    bcmtm_pt_info_t pt_info = {0};
    bcmdrd_sid_t sid[] = {MMU_MTRO_CONFIGr, MMU_WRED_REFRESH_CONTROLr};
    ...
    for (idx = 0; idx < COUNTOF(sid); idx++) {
        ...
        SHR_IF_ERR_EXIT(bcmtm_field_set(unit, sid[idx], fid, &ltmbuf, &fval));
        SHR_IF_ERR_EXIT
...
bcm56780_a0_tm_chip_init(int unit, bool warm)
{
    ...
    if (!warm) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_tm_refresh_enable(unit));
        SHR_IF_ERR_EXIT
                                          ...
                                          bool flag)
{
     ...
     */
    for (port = 0; port < TD4_X9_DEV_PORTS_PER_DEV; port++) {
        if (flag) {
            if (port == 0) {
                continue;
            }
    ...
    BCMTM_PT_DYN_INFO(pt_info, index, inst);
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, sid, ltid, &pt_info, &ltmbuf));
    SHR_IF_ERR_EXIT(bcmtm_field_set(unit, sid, fid, &ltmbuf, &fval));
    SHR_IF_ERR_EXIT
...
bcm56780_a0_tm_check_ep_mmu_credit(int unit, int lport)
{
    ...
    BCMTM_PT_DYN_INFO(pt_info, pport, 0);
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, EGR_MMU_REQUESTSm, ltid, &pt_info, &ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_field_get(unit, EGR_MMU_REQUESTSm, OUTSTANDING_MMU_REQUESTSf,
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, MMU_EBPTS_EDB_CREDIT_COUNTERr, ltid,
                                  &pt_info, &ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_field_get(unit, MMU_EBPTS_EDB_CREDIT_COUNTERr,
                         NUM_EDB_CREDITSf, &ltmbuf, &mmu_credit));
...
bcm56780_a0_tm_check_port_ep_credit(int unit, int lport)
{
    ...
    BCMTM_PT_DYN_INFO(pt_info, pport, 0);
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, EGR_PORT_REQUESTSm, -1, &pt_info,
                               ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_field_get(unit, EGR_PORT_REQUESTSm, OUTSTANDING_PORT_REQUESTSf,
                         ltmbuf, &ep_credit));
    if (bcmtm_lport_is_fp(unit, lport)) {
        BCMTM_PT_DYN_INFO(pt_info, 0, pport);
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, CDMAC_TXFIFO_STATUSr,-1, &pt_info,
                                   ltmbuf));
        SHR_IF_ERR_EXIT
        ...
        BCMTM_PT_DYN_INFO(pt_info, 0, pport);
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, XLMAC_TXFIFO_CELL_REQ_CNTr,-1, &pt_info,
                                   ltmbuf));
        SHR_IF_ERR_EXIT
