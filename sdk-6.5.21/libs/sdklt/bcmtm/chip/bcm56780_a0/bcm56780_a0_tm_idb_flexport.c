...
static int
bcm56780_a0_tm_idb_config_idb_to_dev_pmap_tbl(int unit, int pipe)
{
    bcmdrd_sid_t mem;
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_fid_t fid ;
    ...
    for (local_dev_port = 0; local_dev_port < TD4_X9_DEV_PORTS_PER_PIPE;
         local_dev_port++) {
        if (pipe == 0){
...
bcm56780_a0_tm_reset_idb(int unit, int lport, int reset_value)
{
    ...
    if (bcmtm_lport_is_fp(unit, lport)) {
        ...
        if (bcmsec_is_pm_sec_supported(unit, pm_num + 1)) {
            /*
             * Add dummy CRC for SEC ports on ingress direction.
             */
            fid = APPEND_4B_DUMMY_ON_EOPf;
            fval = reset_value ? 0 : 1;
            SHR_IF_ERR_EXIT
                (bcmtm_field_set(unit, ca_ctrl_reg, fid, &ltmbuf0, &fval));
        }
...
bcm56780_a0_tm_idb_general_init(int unit, int pipe_num)
{
    ...
    SHR_IF_ERR_EXIT(bcm56780_a0_tm_idb_config_idb_to_dev_pmap_tbl(unit,pipe_num));
