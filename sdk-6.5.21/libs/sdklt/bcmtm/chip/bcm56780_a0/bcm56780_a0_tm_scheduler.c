/*! \file bcm56780_a0_tm_scheduler.c
 *
 ...
 */
static int
bcm56780_a0_tm_cos_l0_map_validate(int unit,
                                   ...
                                   int *opcode)
{
    ...
    max_num_ucq = bcm56780_a0_tm_num_uc_q_non_cpu_port_get(unit);
    for (idx = 0; idx < TD4_X9_TM_NUM_Q; idx++) {
 ...
 */
static void
bcm56780_a0_tm_cos_l0_map_set(int unit,
                              int profile_id,
                              bcmtm_scheduler_profile_t *profile)
{
    bcmtm_scheduler_cos_l0_map_t *l0_map =
 ...
 */
static int
bcm56780_a0_tm_mmuq_l1_map_get(int unit,
                               uint8_t profile_id,
                               ...
                               int *l1_node)
{
    ...
    if (sched_level == L1_SCHED_MULTICAST_QUEUE) {
        mcq_offset = bcm56780_a0_tm_num_uc_q_non_cpu_port_get(unit);
        mmu_q_id += mcq_offset;
    }
 ...
 */
static int
bcm56780_a0_tm_scheduler_node_profile_validate(int unit,
                                               int node_id,
                                               bcmtm_scheduler_profile_t *profile)
{
 ...
 */
static int
bcm56780_a0_tm_scheduler_profile_validate(int unit,
                                          uint8_t profile_id,
                                          int node_id,
                                          bcmtm_scheduler_profile_t *profile)
{
    ...
    int num_ucq, num_mcq, total_q;
    uint8_t l1_map[TD4_X9_TM_NUM_Q]= {0};
    bcmtm_scheduler_cos_l0_map_t l0_map;
    ...
    sal_memset(&l0_map, -1, sizeof(bcmtm_scheduler_cos_l0_map_t));
    if (SHR_E_NONE !=
            bcm56780_a0_tm_cos_l0_map_validate(unit, profile, &l0_map, &opcode)) {
 ...
 */
static int
bcm56780_a0_tm_scheduler_node_set(int unit,
                                  bcmtm_lport_t lport,
                                  ...
                                  bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (sched_level == L0_SCHED_COS) {
        ...
        fid = SP_L0_CHILDf;
        SHR_IF_ERR_EXIT
            (bcm56780_a0_tm_sched_node_l0_map_get(unit, profile_id,
                                                  sched_node_id, &node_id));
    } else {
        /* L1 level */
        fid = SP_L0_PARENTf;
        SHR_IF_ERR_EXIT
            (bcm56780_a0_tm_mmuq_l1_map_get(unit, profile_id, sched_level,
                                           sched_node_id, &node_id));
    }
    ...
    if (bcmtm_lport_is_mgmt(unit, lport)) {
        index = TD4_X9_TM_MGMT_Q_OFFSET + node_id;
    } else if (bcmtm_lport_is_lb(unit, lport)) {
        index = TD4_X9_TM_LB_Q_OFFSET + node_id;
    } else {
        index = mport * TD4_X9_TM_NUM_Q + node_id;
    }
 ...
 */
static void
bcm56780_a0_tm_l1_to_l0_mapping(int unit,
                                uint32_t profile_id,
                                bcmtm_scheduler_profile_t *profile,
                                uint32_t *bmap)
{
    int idx, l0_idx, num_ucq, num_mcq, total_q;
    int l1_map[TD4_X9_TM_NUM_Q];
    bcmtm_scheduler_cos_l0_map_t *l0_map;

    l0_map = &(bcmtm_scheduler_cos_l0_map[unit][profile_id]);

    sal_memset(l1_map, -1, sizeof(int) * TD4_X9_TM_NUM_Q);
 ...
 */
static int
bcm56780_a0_tm_scheduler_port_set(int unit,
        bcmltd_sid_t ltid,
        bcmtm_lport_t lport,
        bcmtm_scheduler_profile_t *profile,
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (profile) {
        bcm56780_a0_tm_l1_to_l0_mapping(unit, profile_id, profile, &bmap);
    }
    SHR_IF_ERR_EXIT
 ...
 */
static int
bcm56780_a0_tm_scheduler_profile_reset(int unit,
                                       bcmltd_sid_t ltid,
                                       uint8_t profile_id)
{
    ...
    for (idx = 0; idx < (sizeof(sid_list)/sizeof(bcmdrd_sid_t)); idx++) {
        ...
        if (sid == MMU_PPSCH_SCHQ_MMUQ_PROFILEr) {
            ...
            SHR_IF_ERR_EXIT
                (bcmtm_pt_indexed_write(unit, sid, ltid, &pt_info, ltmbuf_2));
        }
    }
exit:
    SHR_FUNC_EXIT();
 ...
 */
static int
bcm56780_a0_tm_scheduler_profile_set(int unit,
                                     bcmltd_sid_t ltid,
                                     uint8_t profile_id,
                                     bcmtm_scheduler_profile_t *profile)
{
    ...
    uint32_t mmu_q_map[TD4_X9_TM_NUM_Q] = {0};
    uint32_t fval, ltmbuf[BCMTM_MAX_ENTRY_WSIZE] = {0};
    bcmtm_pt_info_t pt_info = {0};
    ...
    if (!profile) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_tm_scheduler_profile_reset(unit, ltid, profile_id));
        l0_map = &(bcmtm_scheduler_cos_l0_map[unit][profile_id]);
        sal_memset(l0_map->map, -1, sizeof(bcmtm_scheduler_cos_l0_map_t));
        SHR_EXIT();
    } else {
        bcm56780_a0_tm_cos_l0_map_set(unit, profile_id, profile);
    }
 ...
 */
static int
bcm56780_a0_tm_scheduler_cpu_node_set(int unit,
        uint32_t sched_node_id,
        uint32_t sched_level,
        bcmltd_sid_t ltid,
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (sched_level == L1_SCHED_MULTICAST_QUEUE &&
        ...
        /* L1 node level */
        fid = SELECTf;
        BCMTM_PT_DYN_INFO(pt_info, sched_node_id, 0);
 ...
 */
static int
bcm56780_a0_tm_scheduler_config_init(int unit)
{
...
static int
bcm56780_a0_tm_scheduler_profile_init(int unit,
                                      uint8_t profile_id,
                                      bcmtm_scheduler_profile_t *profile)
{
    int idx;
 ...
 */
void
bcm56780_a0_tm_sched_node_mmu_q_get(int unit,
                                    int sched_node,
                                    bcmtm_scheduler_profile_t *profile,
                                    int *mmu_q_num,
...
int
bcm56780_a0_tm_mmu_q_sched_node_id_get(int unit,
                                       int mmu_q_id,
                                       bcmtm_scheduler_profile_t *profile)
{
...
int
bcm56780_a0_tm_sched_node_l0_map_get(int unit,
                                     uint8_t profile_id,
                                     uint32_t sched_node,
                                     int *l0_node)
{
...
int
bcm56780_a0_tm_sched_node_l0_max_get(int unit,
                                     uint8_t profile_id)
{
    bcmtm_scheduler_cos_l0_map_t *l0_map;
    int idx = 0;
    int max_cos = TD4_X9_NUM_L0_NODE - 1;

    l0_map = &(bcmtm_scheduler_cos_l0_map[unit][profile_id]);

    /* loop across all the COS as user may skip cos in between. */
    for (idx = (TD4_X9_NUM_L0_NODE - 1) ; idx >= 0; idx--) {
...
int
bcm56780_a0_tm_scheduler_drv_get(int unit, void *sched_drv)
{
    bcmtm_scheduler_drv_t bcm56780_a0_sched_drv = {
        .node_set = bcm56780_a0_tm_scheduler_node_set,
        .port_set = bcm56780_a0_tm_scheduler_port_set,
        .node_profile_validate = bcm56780_a0_tm_scheduler_node_profile_validate,
        .profile_validate = bcm56780_a0_tm_scheduler_profile_validate,
        .cpu_node_set = bcm56780_a0_tm_scheduler_cpu_node_set,
        .profile_set = bcm56780_a0_tm_scheduler_profile_set,
        .profile_init = bcm56780_a0_tm_scheduler_profile_init
    };
    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(sched_drv, SHR_E_INTERNAL);
    *((bcmtm_scheduler_drv_t *)sched_drv) = bcm56780_a0_sched_drv;
exit:
    SHR_FUNC_EXIT();
}

int
bcm56780_a0_tm_scheduler_chip_init(int unit, bool warm)
{
    SHR_FUNC_ENTER(unit);
    ...
    if (!warm) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_tm_scheduler_config_init(unit));
    }
    SHR_IF_ERR_EXIT
        (bcm56780_a0_tm_scheduler_profile_init(unit, -1, NULL));
exit:
    SHR_FUNC_EXIT();
}
