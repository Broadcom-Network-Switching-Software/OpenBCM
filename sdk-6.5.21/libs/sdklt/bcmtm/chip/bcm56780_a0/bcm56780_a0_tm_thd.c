...
td4_x9_mmu_init_dev_config(int unit, soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    devcfg->total_mcq_entry = TD4_X9_MMU_MCQ_ENTRY_PER_ITM;
                                    ...
                                    soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (profile_idx = 0; profile_idx < TD4_X9_MMU_NUM_PROFILES;
            profile_idx++) {
        buf_map_profiles = &buf->mapprofiles[profile_idx];
        for (idx = 0; idx < TD4_X9_MMU_CFG_INT_PRI; idx++) {
            if (lossless == 2) {
                /* Lossy+Lossless mode */
                if (idx == 0) {
                    inpri2pg = 7;
                                       ...
                                       soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    bcmdrd_fid_t field_pri[TD4_X9_MMU_CFG_INT_PRI] = {
            ...
            INPPRI15_PGf};
    bcmdrd_fid_t field_hpid[TD4_X9_MMU_NUM_PG] = {
            PG0_HPIDf, PG1_HPIDf, PG2_HPIDf,
            PG3_HPIDf, PG4_HPIDf, PG5_HPIDf,
            PG6_HPIDf, PG7_HPIDf};
    bcmdrd_fid_t field_spid[TD4_X9_MMU_NUM_PG] = {
            PG0_SPIDf, PG1_SPIDf, PG2_SPIDf,
            PG3_SPIDf, PG4_SPIDf, PG5_SPIDf,
            PG6_SPIDf, PG7_SPIDf};
    bcmdrd_fid_t field_pfcpri[TD4_X9_MMU_NUM_PG] = {
    ...
    for (idx = 0; idx <= pt_index_max(unit, sid); idx++) {
        ...
        for (loop = 0; loop < TD4_X9_MMU_CFG_INT_PRI; loop++) {
    ...
    for (idx = 0; idx <= pt_index_max(unit, sid); idx++) {
        ...
        sal_memset(ltmbuf, 0, sizeof(ltmbuf));
        for (loop = 0; loop < TD4_X9_MMU_NUM_PG; loop++) {
                                   ...
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (idx = 0; idx < TD4_X9_MMU_NUM_POOL; idx++) {
        ...
        if (idx == 0) {
            /* 100% scale up by 100 */
            buf_pool->total_mcq_entry = TD4_X9_MMU_MCQ_ENTRY_PER_ITM;
    ...
    if (drv_info->skip_buf_resv == 0) {
        for (pipe = 0; pipe < TD4_X9_PIPES_PER_DEV; pipe++) {
            for (pm = pipe * TD4_X9_PBLKS_PER_PIPE;
                 pm < (pipe + 1) * TD4_X9_PBLKS_PER_PIPE; pm++) {
                ports_in_pm[pm] = drv_info->flex_eligible[pm];
                ports_in_pipe[pipe] += ports_in_pm[pm];
            }
            /* Each pipe cannot have more than 18 ports. */
    ...
    total_qmin_itm[itm] = default_mtu_cells * TD4_X9_NUM_QUEUES_CPU_PORT;
                              ...
                              soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (port = 0; port < TD4_X9_MMU_NUM_LPORT; port++) {
...
{
    int itm = 0, pool, pool_buf_size, buf_size;
    soc_mmu_cfg_buf_pool_t *buf_pool;
                                    ...
                                    bool check_only)
{
    ...
    uint32_t new_limit_array[TD4_X9_MAX_SERVICE_POOLS] = {0};
    bcmtm_thd_info_t *thd_info;
        ...
        if (pool == -1) {
            ...
            if ((long int) thd_info->itm_buf_size[itm] + delta >= 0) {
                ...
                if (delta < 0 && (long int) unused_buf + delta >= 0) {
                    ...
                    remaining = new_limit;
                    for (pool_idx = TD4_X9_MAX_SERVICE_POOLS - 1;
                         pool_idx > 0;
                         pool_idx--) {
    ...
    if (pool == -1) {
        ...
        pool_start = 0;
        pool_end = TD4_X9_MAX_SERVICE_POOLS - 1;
    } else {
