                          ...
                          bcmtm_ebst_data_t *data)
{
    ...
    switch (data->type) {
        case EBST_SCAN_MODE_QUEUE:
            map = &(map_info->q_map[*map_index]);
            break;
        case EBST_SCAN_MODE_PORTSP_UC:
            map = &(map_info->portsp_map[*map_index].uc_sp);
            break;
        case EBST_SCAN_MODE_PORTSP_MC:
            map = &(map_info->portsp_map[*map_index].mc_sp);
            break;
        case EBST_SCAN_MODE_SP:
            map = &(map_info->sp_map[*map_index]);
            break;
        default:
            break;
    }
    ...
    while (true) {
        ...
        switch (notif.msg) {
            ...
            case EBST_MSG_EXIT_THREAD:
                sal_sem_give(ebst_info->ebst_thread);
                return;
            case EBST_FIFO_FULL:
 ...
 */
int
bcmtm_ebst_start(int unit, bool resource_alloc)
{
    bcmtm_ebst_dev_info_t *ebst_info;
    bcmtm_drv_t *drv;
    bcmtm_ebst_drv_t ebst_drv;
    ...
    ebst_info->ebst_thread = sal_sem_create("bcmtmEbstThreadSemaphore",
                                            SAL_SEM_BINARY, 0);
    SHR_NULL_CHECK(ebst_info->ebst_thread, SHR_E_MEMORY);
...
bcmtm_ebst_fifodma_stop(int unit, bool resource_free)
{
    ...
    for (idx = 0; idx < ebst_info->max_buffer_pool; idx++) {
        if (ebst_info->start[idx]) {
            ebst_info->start[idx] = 0;
            bcmbd_fifodma_stop(unit, ebst_info->chan[idx]);
            sal_memset(&(ebst_info->fifodata[idx]), 0,
                                   sizeof(bcmtm_ebst_fifodata_t));
    ...
    if (resource_free) {
        if (ebst_info->pid) {
            ...
            (void) sal_sem_take(ebst_info->ebst_thread, SAL_SEM_FOREVER);
            sal_sem_destroy(ebst_info->ebst_thread);
