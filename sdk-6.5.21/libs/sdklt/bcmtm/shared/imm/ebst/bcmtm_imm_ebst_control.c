                           ...
                           bcmtm_ebst_control_cfg_t *ebst_cfg)
{
    ...
    if (ebst_cfg->enable_mode == EBST_ENABLE_MODE_START) {
        switch (prev_mode) {
            case EBST_ENABLE_MODE_OFF:
                /* Previous state is EBST_ENABLE_MODE_OFF, signifies EBST is off.
                 * Configures the EBST thread for data collection.
                 */
                SHR_IF_ERR_EXIT(bcmtm_ebst_start(unit, true));
                break;
            case EBST_ENABLE_MODE_FULL:
                break;
            case EBST_ENABLE_MODE_STOP:
                /* Enable the EBST. No resource allocation. */
                SHR_IF_ERR_EXIT(bcmtm_ebst_start(unit, false));
                break;
            default:
                break;
        }
                            ...
                            bcmtm_ebst_control_cfg_t *ebst_cfg)
{
    ...
    if (SHR_SUCCESS(bcmimm_entry_lookup(unit, ltid, &out, &out))) {
        /* Get the number of elements in out and mark the extra entry invalid. */
        if (out.count && out.count <= TM_EBST_CONTROLt_FIELD_COUNT) {
            (out.field[out.count -1])->next = NULL;
