                       ...
                       mc_group_cfg_t *old_lt_cfg)
{
    ...
    ipmc_info.valid = 1;
    ipmc_info.l3_payload = lt_cfg->l3_payload;
    ipmc_info.disable_prune = lt_cfg->disable_prune;
                            ...
                            mc_group_cfg_t *mc_group_cfg)
{
    ...
    if (rv == SHR_E_NOT_FOUND) {
        ...
        rv = bcmtm_field_list_get(unit, &out,
                        TM_MC_GROUPt_L3_PAYLOADf, 0, &fval);
        if (SHR_SUCCESS(rv)) {
            mc_group_cfg->l3_payload = (uint32_t)fval;
        }
        rv = bcmtm_field_list_get(unit, &out,
                        TM_MC_GROUPt_SKIP_VLAN_PRUNEf, 0, &fval);
        if (SHR_SUCCESS(rv)) {
            mc_group_cfg->disable_prune = (uint32_t)fval;
        }
    }
                       ...
                       mc_group_cfg_t *old_ltcfg)
{
    ...
    if (old_ltcfg->mc_group_id >= 0) {
        ...
        ltcfg->l3_payload = old_ltcfg->l3_payload;
        ltcfg->disable_prune = old_ltcfg->disable_prune;
    }
    ...
    while (gen_field) {
        ...
        case TM_MC_GROUPt_L2_PORTf:
            ...
            break;
        case TM_MC_GROUPt_L3_PAYLOADf:
            ltcfg->l3_payload = fval;
            break;
        case TM_MC_GROUPt_SKIP_VLAN_PRUNEf:
            ltcfg->disable_prune = fval;
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
