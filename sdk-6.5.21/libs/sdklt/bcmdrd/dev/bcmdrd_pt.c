 ...
 */
static bool
pt_is_valid(int unit, const bcmdrd_symbol_t *symbol, bcmdrd_sid_t sid)
{
    /* Check for SKU override */
    if (!bcmdrd_dev_sym_valid(unit, sid)) {
        return false;
    }
...
bcmdrd_pt_index_valid(int unit, bcmdrd_sid_t sid, int tbl_inst, int tbl_idx)
{
    ...
    if (!pt_is_valid(unit, symbol, sid)) {
        return false;
    }
...
bool
bcmdrd_pt_is_dmaable(int unit, bcmdrd_sid_t sid)
{
    const bcmdrd_symbol_t *symbol;
    uint32_t test_flags;
    ...
    test_flags = BCMDRD_SYMBOL_FLAG_MEMORY | BCMDRD_SYMBOL_FLAG_CACHEABLE;
    if ((symbol->flags & test_flags) != test_flags) {
        return false;
    }
    test_flags = BCMDRD_SYMBOL_FLAG_NOTEST | BCMDRD_SYMBOL_FLAG_READONLY |
                 BCMDRD_SYMBOL_FLAG_FIFO | BCMDRD_SYMBOL_FLAG_SOFT_PORT;
    if ((symbol->flags & test_flags) != 0) {
        return false;
    }
    return true;
}
