...
err_code_t merlin16_pcieg3_stop_rx_adaptation(srds_access_t *sa__, uint8_t enable) {
  ...
  if (enable) {
      err_code_t err_code;
      err_code = merlin16_pcieg3_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_GRACEFULLY,GRACEFUL_STOP_TIME);
      if(err_code) {
          uint8_t temp=0;
          USR_PRINTF(("Warning Graceful stop request returned error %d; Requesting a forceful stop\n",err_code));
          temp = merlin16_pcieg3_INTERNAL_stop_micro(sa__,0,&err_code);
          /* return value from merlin16_pcieg3_INTERNAL_stop_micro - immediate stop is not required */
...
err_code_t merlin16_pcieg3_ucode_crc_verify(srds_access_t *sa__, uint16_t ucode_len,uint16_t expected_crc_value) {
    ...
    if(calc_crc != expected_crc_value) {
        ...
        return(merlin16_pcieg3_error(sa__, ERR_CODE_UC_CRC_NOT_MATCH));
    }
...
err_code_t merlin16_pcieg3_ucode_pram_load(srds_access_t *sa__, char const * ucode_image, uint32_t ucode_len) {
    ...
    if (ucode_len > UCODE_MAX_SIZE) {                       /* uCode size should be less than UCODE_MAX_SIZE  */
        return (merlin16_pcieg3_error(sa__, ERR_CODE_INVALID_UCODE_LEN));
    }
...
err_code_t merlin16_pcieg3_read_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t *val) {
    ...
    switch(param) {
    case RX_AFE_PF:
        EFUN(merlin16_pcieg3_INTERNAL_get_rx_pf_main(sa__, &rd_val));
        *val = (int8_t)rd_val;
        break;
    case RX_AFE_PF2:
        EFUN(merlin16_pcieg3_INTERNAL_get_rx_pf2(sa__, &rd_val));
        *val = (int8_t)rd_val;
        break;
    case RX_AFE_VGA:
        EFUN(merlin16_pcieg3_INTERNAL_get_rx_vga(sa__, &rd_val));
        *val = (int8_t)rd_val;
      break;
...
err_code_t merlin16_pcieg3_uc_reset(srds_access_t *sa__, uint8_t enable) {
  ...
  return merlin16_pcieg3_uc_reset_with_info(sa__, enable, ucode_info);
}
...
err_code_t merlin16_pcieg3_init_merlin16_pcieg3_info(srds_access_t *sa__) {
    ...
    srds_info_t * merlin16_pcieg3_info_ptr = merlin16_pcieg3_INTERNAL_get_merlin16_pcieg3_info_ptr(sa__);

    USR_ACQUIRE_LOCK;
    /* Handle error cases when merlin16_pcieg3_info_ptr returned as zero */
    if (merlin16_pcieg3_info_ptr == NULL) {
        EFUN_PRINTF(("ERROR:  Info table pointer is null.\n"));
        RELEASE_LOCK_AND_RETURN (merlin16_pcieg3_error(sa__, ERR_CODE_INFO_TABLE_ERROR));
    }
    if (merlin16_pcieg3_info_ptr->signature == SRDS_INFO_SIGNATURE) {
        /*Already initalized and so check for microcode version and exit with proper status*/
        err_code = merlin16_pcieg3_INTERNAL_match_ucode_from_info(sa__);
        ...
        if (err_code == ERR_CODE_NONE) {
            ...
            RELEASE_LOCK_AND_RETURN (merlin16_pcieg3_error(sa__, ERR_CODE_MICRO_INIT_NOT_DONE));
        }
    }
    /*Never initialized so far, continue with initialization */

    err_code = merlin16_pcieg3_rdblk_uc_prog_ram(sa__, (uint8_t *)info_table, INFO_TABLE_RAM_BASE, INFO_TABLE_END);
    /* Release lock before returning from the call to avoid deadlock */
    if (ERR_CODE_NONE != err_code) {
       RELEASE_LOCK_AND_RETURN(merlin16_pcieg3_error(sa__, err_code));
    }
    ...
    if (info_table_version < 0x36) {
        ...
        merlin16_pcieg3_info_ptr->grp_ram_size = info_table[INFO_TABLE_OFFS_GROUP_RAM_SIZE / sizeof(uint32_t)] & 0xFFFF;
    }
    merlin16_pcieg3_info_ptr->ucode_version = info_table[INFO_TABLE_OFFS_UC_VERSION /sizeof(uint32_t)];
...
err_code_t merlin16_pcieg3_ucode_load_verify(srds_access_t *sa__, uint8_t *ucode_image, uint32_t ucode_len) {
    ...
    do {                                                  /* ucode_image read 16bits at a time */
        ...
        if (data_lsw != rddata_lsw) {
            ...
            return (merlin16_pcieg3_error(sa__, ERR_CODE_UCODE_VERIFY_FAIL));             /* Verify uCode FAIL */
        }
