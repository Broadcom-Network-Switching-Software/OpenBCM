...
static srds_info_t merlin16_pcieg3_info[NUM_SERDES_INFO_TABLES];
#endif


srds_info_t *merlin16_pcieg3_INTERNAL_get_merlin16_pcieg3_info_ptr(srds_access_t *sa__) {    
#if defined(SERDES_EXTERNAL_INFO_TABLE_EN)
    return merlin16_pcieg3_get_info_table_address(sa__);
#else
    uint8_t index = 0;
    index = 0;
    return &merlin16_pcieg3_info[index];
#endif
}
...
err_code_t merlin16_pcieg3_INTERNAL_match_ucode_from_info(srds_access_t *sa__) {
    ...
    if (ucode_version == merlin16_pcieg3_info_ptr->ucode_version) {
        return(ERR_CODE_NONE);
    } else {
        EFUN_PRINTF(("ERROR:  ucode version of the current thread not matching with stored merlin16_pcieg3_info->ucode_version, Expected 0x%08X, but received 0x%08X.\n",
                    merlin16_pcieg3_info_ptr->ucode_version, ucode_version));
...
err_code_t merlin16_pcieg3_INTERNAL_verify_merlin16_pcieg3_info(srds_info_t *merlin16_pcieg3_info_ptr, srds_access_t *sa__)
{
    ...
    if (merlin16_pcieg3_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        EFUN_PRINTF(("ERROR:  Mismatch in merlin16_pcieg3_info signature.  Expected 0x%08X, but received 0x%08X.\n",
                     SRDS_INFO_SIGNATURE, merlin16_pcieg3_info_ptr->signature));
        return (merlin16_pcieg3_error(sa__, ERR_CODE_INFO_TABLE_ERROR));
    }
...
static int merlin16_pcieg3_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    ...
    case MERLIN16_PCIEG3_OSX2P5:    return 2500;
#endif
    default:                    return 1000;
    }
}
...
err_code_t merlin16_pcieg3_INTERNAL_get_num_bits_per_ms(srds_access_t *sa__, uint32_t *num_bits_per_ms) {
    ...
    { 
       merlin16_pcieg3_osr_mode_st osr_mode_st;
       ENULL_MEMSET(&osr_mode_st, 0, sizeof(merlin16_pcieg3_osr_mode_st));
       EFUN(merlin16_pcieg3_INTERNAL_get_osr_mode(sa__, &osr_mode_st));
...
err_code_t merlin16_pcieg3_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string) {
    ...
    EFUN(merlin16_pcieg3_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local, USE_HW_TIMERS));
...
err_code_t merlin16_pcieg3_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, enum merlin16_pcieg3_prbs_chk_timer_selection_enum timer_sel) {
    ...
    if((time_ms <= PRBS_MAX_HW_TIMER_TIMEOUT) && (timer_sel == USE_HW_TIMERS)) {
        ...
        EFUN(merlin16_pcieg3_get_prbs_chk_hw_timer_ctrl(sa__, &prbs_chk_hw_timer_ctrl_bak));
        
        /* STEP 4: Configure hardware timer to count prbs errors */
        EFUN(merlin16_pcieg3_config_prbs_chk_hw_timer(sa__, time_ms, &time_ms_adjusted));
        ...
        EFUN(merlin16_pcieg3_prbs_chk_en_toggle(sa__));
        ...
        EFUN(merlin16_pcieg3_set_prbs_chk_hw_timer_ctrl(sa__, &prbs_chk_hw_timer_ctrl_bak));
...
err_code_t merlin16_pcieg3_INTERNAL_compute_bin(srds_access_t *sa__, char var, char bin[]) {
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (merlin16_pcieg3_error(sa__, ERR_CODE_CFG_PATT_INVALID_HEX));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t merlin16_pcieg3_INTERNAL_compute_hex(srds_access_t *sa__, char bin[],uint8_t *hex) {
  ...
  if (!USR_STRCMP(bin,"0000")) {
    ...
    *hex = 0x0;
    return (merlin16_pcieg3_error(sa__, ERR_CODE_CFG_PATT_INVALID_BIN2HEX));
  } 
  return (ERR_CODE_NONE);
}
...
err_code_t merlin16_pcieg3_INTERNAL_set_rx_pf_main(srds_access_t *sa__, uint8_t val) {
    if (val > 15) 
    {
      return (merlin16_pcieg3_error(sa__, ERR_CODE_PF_INVALID));
    }
...
err_code_t merlin16_pcieg3_INTERNAL_set_rx_vga(srds_access_t *sa__, uint8_t val) {
    ...
    if (val > RX_VGA_CTRL_VAL_MAX) {
      return (merlin16_pcieg3_error(sa__, ERR_CODE_VGA_INVALID));
    }
...
err_code_t merlin16_pcieg3_INTERNAL_set_rx_dfe1(srds_access_t *sa__, int8_t val) {
...

    {
        ...
        if (val > 63) {
            return (merlin16_pcieg3_error(sa__, ERR_CODE_DFE1_INVALID));  
        }
...
err_code_t merlin16_pcieg3_INTERNAL_set_rx_dfe2(srds_access_t *sa__, int8_t val) {
    ...
    if ((val > 31) || (val < -31)) {
      return (merlin16_pcieg3_error(sa__, ERR_CODE_DFE2_INVALID));  
    }
...
err_code_t merlin16_pcieg3_INTERNAL_set_rx_dfe3(srds_access_t *sa__, int8_t val) {
    if ((val > 31) || (val < -31)) {
      return (merlin16_pcieg3_error(sa__, ERR_CODE_DFE3_INVALID));  
    }
...
err_code_t merlin16_pcieg3_INTERNAL_set_rx_dfe4(srds_access_t *sa__, int8_t val) {
    if ((val > 15) || (val < -15)) {
      return (merlin16_pcieg3_error(sa__, ERR_CODE_DFE4_INVALID));  
    }
...
err_code_t merlin16_pcieg3_INTERNAL_set_rx_dfe5(srds_access_t *sa__, int8_t val) {
    if ((val > 15) || (val < -15)) {
      return (merlin16_pcieg3_error(sa__, ERR_CODE_DFE5_INVALID));  
    }
...
err_code_t merlin16_pcieg3_INTERNAL_get_osr_mode(srds_access_t *sa__, merlin16_pcieg3_osr_mode_st *imode) {
...
#ifdef wr_ams_rx_rxclk_div2p5    /* Analog 2p5 OSR mode */
  {
      ...
      if ((mode.tx == MERLIN16_PCIEG3_OSX1) && (osr_2p5 == 1)  && (osr_2p5_pwrup == 1)) {
          mode.tx = MERLIN16_PCIEG3_OSX2P5;
      }
      ESTM(osr_2p5       = rd_ams_rx_rxclk_div2p5());
      ESTM(osr_2p5_pwrup = rd_ams_rx_rxclk_div2p5_pwrup());
      if ((mode.rx == MERLIN16_PCIEG3_OSX1) && (osr_2p5 == 1)  && (osr_2p5_pwrup == 1)) {
          mode.rx = MERLIN16_PCIEG3_OSX2P5;
      }
  }
#endif
  *imode = mode;
...
#else
#error "Unknown core for merlin16_pcieg3_INTERNAL_pmd_lock_status()"    
#endif
    return(ERR_CODE_NONE);
}
...
err_code_t merlin16_pcieg3_INTERNAL_read_lane_state(srds_access_t *sa__, merlin16_pcieg3_lane_state_st *istate) {
  ...
  EFUN(merlin16_pcieg3_read_rx_afe(sa__, RX_AFE_PF, &state.pf_main));
  ...
  EFUN(merlin16_pcieg3_read_rx_afe(sa__, RX_AFE_VGA, &state.vga));
...

  {
      ...
      EFUN(merlin16_pcieg3_INTERNAL_pmd_lock_status(sa__,&rx_lock_at_end, &rx_lock_chg_at_end));
  ...
  if (!state.stop_state || (state.stop_state == 0xFF)) {
      /* manually check error code instead of EFUN*/
      {
          ...
          resume_status = merlin16_pcieg3_stop_rx_adaptation(sa__, 0);
          if (resume_status) {
...
err_code_t merlin16_pcieg3_INTERNAL_read_core_state(srds_access_t *sa__, merlin16_pcieg3_core_state_st *istate) {  
  ...
  EFUN(merlin16_pcieg3_version(sa__, &state.api_version));
...
err_code_t merlin16_pcieg3_INTERNAL_check_uc_lane_stopped(srds_access_t *sa__) {
  ...
  if (!is_micro_stopped) {
      return(merlin16_pcieg3_error(sa__, ERR_CODE_UC_NOT_STOPPED));
  } else {
      return(ERR_CODE_NONE);
  }
}
...
err_code_t merlin16_pcieg3_INTERNAL_calc_patt_gen_mode_sel(srds_access_t *sa__, uint8_t *mode_sel, uint8_t *zero_pad_len, uint8_t patt_length) {
  ...
  if (!(140 % patt_length)) {
    ...
    *zero_pad_len = 0;
    ...
    return (merlin16_pcieg3_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATT_LENGTH));
  }
  return(ERR_CODE_NONE);
}
...
err_code_t merlin16_pcieg3_INTERNAL_poll_uc_dsc_ready_for_cmd_equals_1(srds_access_t *sa__, uint32_t timeout_ms, enum srds_pmd_uc_cmd_enum cmd) {
    ...
    for (loop = 0; loop < 100; loop++) {
        ...
        if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
            if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
                ...
                return(merlin16_pcieg3_error(sa__, ERR_CODE_UC_CMD_RETURN_ERROR));
            }
            return (ERR_CODE_NONE);
        }
        if(loop>10) {
    ...
    }
    {
        ...
        merlin16_pcieg3_INTERNAL_print_triage_info(sa__, ERR_CODE_UC_CMD_POLLING_TIMEOUT, 1, 1, __LINE__);
...
err_code_t merlin16_pcieg3_INTERNAL_poll_dsc_state_equals_uc_tune(srds_access_t *sa__, uint32_t timeout_ms) {
  ...
  return (merlin16_pcieg3_error(sa__, ERR_CODE_POLLING_TIMEOUT));          /* Error Code for polling timeout */
}
...
err_code_t merlin16_pcieg3_INTERNAL_rdblk_callback(void *arg, uint8_t byte_count, uint16_t data) {
    ...
    ESTM(endian_offset = merlin16_pcieg3_INTERNAL_get_endian_offset(cast_arg->mem_ptr));
...
err_code_t merlin16_pcieg3_INTERNAL_display_info_table (srds_access_t *sa__) {
    srds_info_t * const merlin16_pcieg3_info_ptr = merlin16_pcieg3_INTERNAL_get_merlin16_pcieg3_info_ptr_with_check(sa__);
    EFUN(merlin16_pcieg3_INTERNAL_verify_merlin16_pcieg3_info(merlin16_pcieg3_info_ptr, sa__));

    EFUN_PRINTF(("\n********** SERDES INFO TABLE DUMP **********\n"));
    EFUN_PRINTF(("signature                 = 0x%X\n", merlin16_pcieg3_info_ptr->signature));
    EFUN_PRINTF(("diag_mem_ram_base         = 0x%X\n", merlin16_pcieg3_info_ptr->diag_mem_ram_base));
    EFUN_PRINTF(("diag_mem_ram_size         = 0x%X\n", merlin16_pcieg3_info_ptr->diag_mem_ram_size));
    EFUN_PRINTF(("core_var_ram_base         = 0x%X\n", merlin16_pcieg3_info_ptr->core_var_ram_base));
    EFUN_PRINTF(("core_var_ram_size         = 0x%X\n", merlin16_pcieg3_info_ptr->core_var_ram_size));
    EFUN_PRINTF(("lane_var_ram_base         = 0x%X\n", merlin16_pcieg3_info_ptr->lane_var_ram_base));
    EFUN_PRINTF(("lane_var_ram_size         = 0x%X\n", merlin16_pcieg3_info_ptr->lane_var_ram_size));
    EFUN_PRINTF(("trace_mem_ram_base        = 0x%X\n", merlin16_pcieg3_info_ptr->trace_mem_ram_base));
    EFUN_PRINTF(("trace_mem_ram_size        = 0x%X\n", merlin16_pcieg3_info_ptr->trace_mem_ram_size));
    EFUN_PRINTF(("micro_var_ram_base        = 0x%X\n", merlin16_pcieg3_info_ptr->micro_var_ram_base));
    EFUN_PRINTF(("lane_count                = 0x%X\n", merlin16_pcieg3_info_ptr->lane_count));
    EFUN_PRINTF(("trace_memory_descending_writes = 0x%X\n", merlin16_pcieg3_info_ptr->trace_memory_descending_writes));
    EFUN_PRINTF(("micro_count               = 0x%X\n", merlin16_pcieg3_info_ptr->micro_count));
    EFUN_PRINTF(("micro_var_ram_size        = 0x%X\n", merlin16_pcieg3_info_ptr->micro_var_ram_size));
    EFUN_PRINTF(("grp_ram_size              = 0x%X\n", merlin16_pcieg3_info_ptr->grp_ram_size));
    EFUN_PRINTF(("ucode_version             = 0x%X\n", merlin16_pcieg3_info_ptr->ucode_version));
