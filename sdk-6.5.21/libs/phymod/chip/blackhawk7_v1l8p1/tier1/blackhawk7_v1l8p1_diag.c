...
err_code_t blackhawk7_v1l8p1_test_uc_timestamp(srds_access_t *sa__) {
    ...
    EFUN(blackhawk7_v1l8p1_pmd_uc_cmd(sa__, CMD_UC_DBG,CMD_UC_DBG_TIMESTAMP,100));
...
static err_code_t _blackhawk7_v1l8p1_display_eye_scan_ignored_lane(srds_access_t *sa__) {
    EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_display_eye_scan(srds_access_t *sa__) {
    ...
    err_code_t eye_scan_err = ERR_CODE_NONE;

    eye_scan_err = blackhawk7_v1l8p1_INTERNAL_eye_scan_setup(sa__, 0, &y_max, &y_step);
    if((eye_scan_err == ERR_CODE_RX_TUNING_NOT_DONE) || ((enum srds_diag_failcodes)eye_scan_err == ERR_CODE_DIAG_SCAN_NO_PMD_LOCK)) {
        return ERR_CODE_NONE;
    }
    ...
    EFUN(blackhawk7_v1l8p1_display_eye_scan_header(sa__,1));
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        if(enable_read){
            eye_scan_err = blackhawk7_v1l8p1_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (eye_scan_err) {
                EFUN_PRINTF(("ERROR: blackhawk7_v1l8p1_read_eye_scan_stripe() returned: %s, (status = 0x%x)\n", blackhawk7_v1l8p1_INTERNAL_e2s_err_code(eye_scan_err), status));
                break;
            }
        }
        EFUN(blackhawk7_v1l8p1_display_eye_scan_stripe(sa__, y,&stripe[buffer_idx]));
        EFUN_PRINTF(("\n"));
    }
    EFUN(blackhawk7_v1l8p1_display_eye_scan_footer(sa__,1));
    /* stop acquisition */
...
err_code_t blackhawk7_v1l8p1_display_all_eye_scan(srds_access_t *sa__) {
    ...
    for(lane=0;lane < num_lanes;lane++) {
        ...
        err_code = blackhawk7_v1l8p1_INTERNAL_eye_scan_setup(sa__, 0, &i_max[lane], &i_step[lane]);
        if(err_code) {
            uint8_t core;
    ...
    EFUN(blackhawk7_v1l8p1_display_eye_scan_header(sa__, num_lanes));
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                if(i[lane] >= -i_max[lane]) {
                    ...
                    if(enable_read) {
                        EFUN(blackhawk7_v1l8p1_read_eye_scan_stripe(sa__, &stripe[0], &status));
                    }

                    EFUN(blackhawk7_v1l8p1_display_eye_scan_stripe(sa__, i[lane],&stripe[buffer_idx]));
                    ...
                    if(eye_slice[lane]) {
                        ...
                        EFUN(_blackhawk7_v1l8p1_display_eye_scan_ignored_lane(sa__));
                    }
                }
            }
            else {
    ...
    EFUN(blackhawk7_v1l8p1_display_eye_scan_footer(sa__, num_lanes));
    for(lane=0;lane < num_lanes;lane++) {
        if(lane_mask & (1<<lane)) {
            EFUN(blackhawk7_v1l8p1_acc_set_lane(sa__, lane));
            EFUN(blackhawk7_v1l8p1_meas_eye_scan_done(sa__));
...
err_code_t blackhawk7_v1l8p1_display_eye_scan_stripe(srds_access_t *sa__, int16_t y,uint32_t *buffer) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_v1l8p1_log_api_event(srds_access_t *sa__, uint8_t event_id, uint8_t data1, uint8_t data2) {
    INIT_SRDS_ERR_CODE
    ...
    if(blackhawk7_v1l8p1_info_ptr->info_table_version > 0x38) {
        ...
        EFUN(blackhawk7_v1l8p1_pmd_uc_cmd_with_data(sa__, CMD_API_EVENT, event_id, data_new, GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}
err_code_t blackhawk7_v1l8p1_display_eye_scan_header(srds_access_t *sa__, uint8_t i) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
...
err_code_t blackhawk7_v1l8p1_display_eye_scan_footer(srds_access_t *sa__, uint8_t i) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    uint8_t stepsize_var[NUM_LANES_MAX];
    uint8_t lane, num_lanes;
...
#ifndef STANDALONE_EVENT
            if(intrusive) {
                EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], 0, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            } else {
                EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            }
#else
            margins[i] = info_out->margins[i];
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, offset_start, range250, slicer, &nlmv_val));
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(blackhawk7_v1l8p1_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        ...
        if(DIAG_VERBOSE < 5) {
            ...
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)(SRDS_ABS(offset_start)-i), range250, slicer, &nlmv_val));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(blackhawk7_v1l8p1_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
...
err_code_t blackhawk7_v1l8p1_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    ESTM(rx_lock = rd_pmd_rx_lock());
    {
        ...
        micro_stop = blackhawk7_v1l8p1_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
...
Exit:
    ...
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        ...
        uint8_t micro_num = 0;
        srds_info_t * blackhawk7_v1l8p1_info_ptr;
        ESTM(blackhawk7_v1l8p1_info_ptr = blackhawk7_v1l8p1_INTERNAL_get_blackhawk7_v1l8p1_info_ptr_with_check(sa__));
    ...
    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        diag_data_err = blackhawk7_v1l8p1_reg_dump(sa__, core, lane, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
            EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_REG_CORE|LANE returned error %s ****\n", blackhawk7_v1l8p1_INTERNAL_e2s_err_code(diag_data_err)));
            diag_level_error = (uint16_t)(diag_level_error | (diag_level & (SRDS_DIAG_REG_CORE | SRDS_DIAG_REG_LANE)));
        }

    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        diag_data_err = blackhawk7_v1l8p1_uc_core_var_dump(sa__, core, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
        ...
        orig_lane = blackhawk7_v1l8p1_acc_get_physical_lane(sa__);
        for (srds_lane = 0; srds_lane < num_micros*2; srds_lane =(uint8_t)(srds_lane + 2)) {
            EFUN(blackhawk7_v1l8p1_acc_set_physical_lane(sa__, srds_lane));

            diag_data_err = blackhawk7_v1l8p1_uc_micro_var_dump(sa__, core, srds_lane, NULL);
            if(diag_data_err != ERR_CODE_NONE) {
                EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_UC_CORE returned error %s ****\n", blackhawk7_v1l8p1_INTERNAL_e2s_err_code(diag_data_err)));
                diag_level_error |= SRDS_DIAG_UC_CORE;
            }
        }
        EFUN(blackhawk7_v1l8p1_acc_set_physical_lane(sa__, orig_lane));
        }
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {

        diag_data_err = blackhawk7_v1l8p1_uc_lane_var_dump(sa__, core, lane, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if((diag_level & SRDS_DIAG_PRBS_PROJ)
        ...
        blackhawk7_v1l8p1_display_aggregate_prbs_error_analyzer_proj(sa__, 5440, SRDS_IEEE_50GE, lane_mask_aggregate, timeout_s, diag_mode);
    }
