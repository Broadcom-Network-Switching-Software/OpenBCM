...
extern err_code_t blackhawk7_l8p2_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l8p2_ber_data_st *ber_data, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl);
extern err_code_t blackhawk7_l8p2_get_BER_data_HW_mode(srds_access_t *sa__, struct blackhawk7_l8p2_ber_data_st *ber_data, struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl); 
...
err_code_t blackhawk7_l8p2_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l8p2_ber_data_st *ber_data, enum blackhawk7_l8p2_prbs_chk_timer_selection_enum timer_sel) {
    ...
    if(timer_sel == USE_HW_TIMERS) {
        struct blackhawk7_l8p2_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl_bak;
        err_code_t err = ERR_CODE_NONE;
...
err_code_t blackhawk7_l8p2_INTERNAL_read_lane_state(srds_access_t *sa__, blackhawk7_l8p2_lane_state_st *istate) {
  ...
  EFUN(blackhawk7_l8p2_INTERNAL_get_tx_ppm(sa__, &state.tx_ppm));
...
err_code_t blackhawk7_l8p2_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {
  ...
  if(state.osr_mode.tx_rx != 255) {
      const char *s = blackhawk7_l8p2_INTERNAL_get_osr_str(state.osr_mode.tx_rx, GET_OSR_STR_LONG);
      char *s_tmp;
      s_tmp = (char *)s;
      EFUN_PRINTF(("(%2s%s, 0x%04x, 0x%02x ,", (state.br_pd_en) ? "BR" : "OS", s_tmp, state.ucv_config,state.ucv_status));
  } else {
      const char *s = blackhawk7_l8p2_INTERNAL_get_osr_str(state.osr_mode.tx, GET_OSR_STR_SHORT);
      const char *r = blackhawk7_l8p2_INTERNAL_get_osr_str(state.osr_mode.rx, GET_OSR_STR_SHORT);
      char *s_tmp;
      char *r_tmp;
      s_tmp = (char *)s;
      r_tmp = (char *)r;
      {
          ...
          EFUN_PRINTF(("(%3s%s,%2s%s:%s, 0x%04x, 0x%02x_%04x,", t,(tx == rx) ? " ": "~", (state.br_pd_en) ? "BR" : "OS",
                        s_tmp, r_tmp, state.ucv_config, state.ucv_status, state.ucv_status_ext));
      }
  }
  ...
  EFUN(blackhawk7_l8p2_INTERNAL_display_BER(sa__,100));
  {
    ...
    if(state.ucv_status != 0) {
      EFUN_PRINTF(("\n"));
      EFUN(blackhawk7_l8p2_decode_uc_sts(sa__, state.ucv_status, decode_buf, sizeof(decode_buf), prepend));
      EFUN_PRINTF(("%s", decode_buf));
    }
    if(state.ucv_status_ext != 0) {
      EFUN(blackhawk7_l8p2_decode_uc_sts_ext(sa__, state.ucv_status_ext, decode_buf, sizeof(decode_buf), prepend));
      EFUN_PRINTF(("%s", decode_buf));
    }
  }
