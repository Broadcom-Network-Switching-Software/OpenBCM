...
#define TSCE7_MODEL        0x12
#define TSCE7_TECH_PROC    0x5     /* 7nm */

#define TSCE7_NOF_LANES_IN_CORE (4)
#define TSCE7_PHY_ALL_LANES (0xf)
#define TSCE7_CORE_TO_PHY_ACCESS(_phy_access, _core_access) \
    do{\
        ...
        (_phy_access)->access.lane_mask = TSCE7_PHY_ALL_LANES; \
    }while(0)
...
STATIC
int _tsce7_phy_firmware_lane_config_set(const phymod_phy_access_t* phy,
                                        phymod_firmware_lane_config_t fw_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (PHYMOD_IS_WRITE_DISABLED(&phy_copy.access, &is_warm_boot));


        if (!is_warm_boot) {
            PHYMOD_IF_ERR_RETURN
                (merlin7_lane_soft_reset_release_get(&phy_copy.access, &enable_status));
            if (enable_status) {
                PHYMOD_IF_ERR_RETURN
                    (merlin7_lane_soft_reset_release(&phy_copy.access, 0));
            }
            PHYMOD_IF_ERR_RETURN
                (merlin7_tsc_set_uc_lane_cfg(&phy_copy.access,
...
int tsce7_core_identify(const phymod_core_access_t* core,
                        uint32_t core_id,
                        uint32_t* is_identified)
{
    ...
    *is_identified = 0;
    ...
    PHYMOD_VDBG(DBG_CFG, pm_acc,
                ("%-22s: core_id=%0d adr=%0"PRIx32" lane_mask=%0"PRIx32"\n",
    ...
    if (PHYID2r_GET(id2) == TSCE7_ID0 && PHYID3r_GET(id3) == TSCE7_ID1) {
        ...
        if (model == TSCE7_MODEL) {
            if (MAIN0_SERDESIDr_TECH_PROCf_GET(serdesid) == TSCE7_TECH_PROC) {
                *is_identified = 1;
            }
        }
    }
...
#ifdef PHYMOD_DIAG
    PHYMOD_VDBG(DBG_CFG, pm_acc,
                ("%-22s: core_id=%0d identified=%0d rv=%0d adr=%0"
                PRIx32" lmask=%0"PRIx32"\n", __func__, (int)core_id,
                (int)*is_identified, rv, pm_acc->addr, pm_acc->lane_mask));
#endif
    return rv;
}

int tsce7_core_info_get(const phymod_core_access_t* phy,
                        phymod_core_info_t* info)
{
    uint32_t serdes_id;
    PHYID2r_t id2;
    PHYID3r_t id3;
    char core_name[15]="Tsce7";
    ...
    info->core_version = phymodCoreVersionTsce7;
...
int tsce7_core_lane_map_set(const phymod_core_access_t* core,
                            const phymod_lane_map_t* lane_map)
{
    ...
    PHYMOD_VDBG(DBG_TOP, pm_acc,
       ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" rx_lane_map=%0"PRIx32"%0"
         PRIx32"%0"PRIx32"%0"PRIx32" tx_lane_map=%0"PRIx32"%0"PRIx32"%0"
    ...
    /*
     * PCS lane map; PCS lane map specifies which board-side lane a
     * switch-side lane should connect to. For instance, on the bring-up board,
     * for TX, lane[0] on the switch is actually routed to lane[2] on the board;
     * lane[2] on the switch is routed to lane[0] on the board.
     * In this case, "2" should be put in lane_map_tx[0], and "0" should be
     * put in lane_map_tx[2].
     */
    if (lane_map->num_of_lanes != TSCE7_NOF_LANES_IN_CORE) {
        return PHYMOD_E_CONFIG;
    }

    for (lane = 0; lane < TSCE7_NOF_LANES_IN_CORE; lane++) {
    ...
    /*
     * PMD lane address map; PCS lane map specifies the lane map from
     * switch's point of view, while PMD lane address map specifies
     * the same lane map from board's point of view.
     * For instance, if PCS specify that lane[0] on the switch TX side
     * connects to lane[2] on the board TX side, PCS would put "2" in
     * lane_map_tx[0], and PMD would put "0" in lane_map_tx[2].
     * From PMD's point of view, this means that board lane[2] should
    ...
    PHYMOD_IF_ERR_RETURN
        (merlin7_uc_active_get(&core->access, &uc_active));

    if (uc_active) {
        PHYMOD_IF_ERR_RETURN
            (merlin7_tsc_core_dp_reset(&core->access, 1));
        ucode.stack_size = merlin7_ucode_stack_size;
        ucode.ucode_size = merlin7_ucode_len;
        ucode.crc_value  = merlin7_ucode_crc;
        PHYMOD_IF_ERR_RETURN
            (merlin7_tsc_core_reset(&core->access, ucode));
    }

    PHYMOD_IF_ERR_RETURN
        (merlin7_tsc_map_lanes(&core->access, num_lanes, pmd_tx_lane_map, pmd_rx_lane_map));

    if (uc_active) {
        PHYMOD_IF_ERR_RETURN
            (merlin7_tsc_uc_reset(&core->access, 0, ucode));
        PHYMOD_IF_ERR_RETURN
            (merlin7_tsc_core_dp_reset(&core->access, 0));
    }

    return PHYMOD_E_NONE;
}

int tsce7_core_lane_map_get(const phymod_core_access_t* core,
                            phymod_lane_map_t* lane_map)
{
    phymod_core_access_t  core_copy;
    uint32_t lane;
    uint8_t physical_tx_lane, physical_rx_lane;

    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));
    core_copy.access.lane_mask = 0x1;

    PHYMOD_MEMSET(lane_map, 0, sizeof(*lane_map));
    lane_map->num_of_lanes = TSCE7_NOF_LANES_IN_CORE;
    for (lane = 0; lane < TSCE7_NOF_LANES_IN_CORE; lane++) {
        PHYMOD_IF_ERR_RETURN
            (merlin7_tsc_get_physical_tx_addr(&core_copy.access,
            lane, &physical_tx_lane));
        PHYMOD_IF_ERR_RETURN
            (merlin7_tsc_get_physical_rx_addr(&core_copy.access,
            lane, &physical_rx_lane));
        lane_map->lane_map_tx[physical_tx_lane] = lane;
        lane_map->lane_map_rx[physical_rx_lane] = lane;
    }
...
STATIC
int _tsce7_core_firmware_load(const phymod_core_access_t* core,
                              ...
                              phymod_firmware_loader_f fw_loader)
{
    ...
    switch (load_method) {
        case phymodFirmwareLoadMethodInternal:
            PHYMOD_IF_ERR_RETURN(merlin7_tsc_ucode_load(&core->access,
                                                        merlin7_ucode,
                                                        merlin7_ucode_len));
            break;
        case phymodFirmwareLoadMethodExternal:
            ...
            PHYMOD_IF_ERR_RETURN(fw_loader(core,
                                           merlin7_ucode_len,
                                           merlin7_ucode));
        ...
        default:
            PHYMOD_RETURN_WITH_ERR(PHYMOD_E_CONFIG,
                                   (_PHYMOD_MSG("illegal fw load method %u"),
                                   load_method));
    }
...
int tsce7_phy_firmware_core_config_set(const phymod_phy_access_t* phy,
                                       phymod_firmware_core_config_t fw_config)
{
    struct merlin7_tsc_uc_core_config_st serdes_firmware_core_config;

    PHYMOD_MEMSET(&serdes_firmware_core_config,
                  0,
                  sizeof(serdes_firmware_core_config));
...
int tsce7_phy_firmware_core_config_get(const phymod_phy_access_t* phy,
                                       phymod_firmware_core_config_t* fw_config)
{
    struct merlin7_tsc_uc_core_config_st serdes_firmware_core_config;
...
int tsce7_phy_firmware_lane_config_set(const phymod_phy_access_t* phy,
                                       phymod_firmware_lane_config_t fw_config)
{
    phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;
    ...
    PHYMOD_IF_ERR_RETURN
         (_tsce7_phy_firmware_lane_config_set(phy, fw_config));
    /* Hold the per lne soft reset bit. */
    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
    ...
    PHYMOD_IF_ERR_RETURN
        (temod16_port_start_lane_get(&phy->access, &port_starting_lane, &port_num_lane));
...
int tsce7_phy_firmware_lane_config_get(const phymod_phy_access_t* phy,
                                       phymod_firmware_lane_config_t* fw_config)
{
    struct merlin7_tsc_uc_lane_config_st serdes_firmware_config;
    PHYMOD_MEMSET(&serdes_firmware_config, 0x0, sizeof(serdes_firmware_config));
...
int tsce7_phy_polarity_set(const phymod_phy_access_t* phy,
                           const phymod_polarity_t* polarity)
{
    PHYMOD_IF_ERR_RETURN
        (merlin7_polarity_set(&phy->access,
...
int tsce7_phy_polarity_get(const phymod_phy_access_t* phy,
...
{
    PHYMOD_IF_ERR_RETURN
        (merlin7_tsc_apply_txfir_cfg(&phy->access,
                                     (int8_t)tx->pre,
                                     (int8_t)tx->main,
                                     (int8_t)tx->post,
                                     (int8_t)tx->post2));
...
int tsce7_phy_power_set(const phymod_phy_access_t* phy,
                        const phymod_phy_power_t* power)
{
    ...
    if ((power->tx == phymodPowerOff) && (power->rx == phymodPowerNoChange)) {
        ...
        PHYMOD_IF_ERR_RETURN(merlin7_tsc_tx_disable(&phy->access, 1));
    }
    if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerNoChange)) {
        /* enable tx on the PMD side */
        PHYMOD_IF_ERR_RETURN(merlin7_tsc_tx_disable(&phy->access, 0));
    }
...
int tsce7_phy_power_get(const phymod_phy_access_t* phy,
                        phymod_phy_power_t* power)
{
    return PHYMOD_E_UNAVAIL;
}

int tsce7_phy_tx_lane_control_set(const phymod_phy_access_t* phy,
                                  phymod_phy_tx_lane_control_t tx_control)
{
    switch (tx_control) {
        case phymodTxTrafficDisable:
            PHYMOD_IF_ERR_RETURN
                (temod16_tx_lane_control_set(&phy->access,
                                             TEMOD16_TX_LANE_TRAFFIC_DISABLE));
            break;
        case phymodTxTrafficEnable:
            PHYMOD_IF_ERR_RETURN
                (temod16_tx_lane_control_set(&phy->access,
                                             TEMOD16_TX_LANE_TRAFFIC_ENABLE));
            break;
        case phymodTxReset:
            PHYMOD_IF_ERR_RETURN
                (temod16_tx_lane_control_set(&phy->access,
                                             TEMOD16_TX_LANE_RESET));
            break;
        case phymodTxSquelchOn:
            PHYMOD_IF_ERR_RETURN
                (temod16_tx_squelch_set(&phy->access, 1));
            break;
        case phymodTxSquelchOff:
            PHYMOD_IF_ERR_RETURN
                (temod16_tx_squelch_set(&phy->access, 0));
            break;
        default:
            break;
    }
...
int tsce7_phy_tx_lane_control_get(const phymod_phy_access_t* phy,
                                  phymod_phy_tx_lane_control_t* tx_control)
{
    ...
    if (enable) {
        PHYMOD_IF_ERR_RETURN
            (merlin7_pmd_loopback_get(&pm_phy_copy.access, &lb_enable));
        if (lb_enable) {
            enable = 0;
        }
    }
    ...
    if (enable) {
        *tx_control = phymodTxSquelchOn;
        ...
        PHYMOD_IF_ERR_RETURN
            (temod16_tx_lane_control_get(&pm_phy_copy.access,
                                         &reset,
                                         &tx_lane));
        if (!reset) {
...
int tsce7_phy_rx_lane_control_set(const phymod_phy_access_t* phy,
    ...
    switch (rx_control) {
        ...
        case phymodRxSquelchOn:
            for (i = 0; i < num_lane; i++) {
                ...
                pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_IF_ERR_RETURN
                    (temod16_rx_squelch_set(&pm_phy_copy.access, 1));
            }
            break;
        case phymodRxSquelchOff:
            for (i = 0; i < num_lane; i++) {
                ...
                pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_IF_ERR_RETURN
                    (temod16_rx_squelch_set(&pm_phy_copy.access, 0));
            }
            break;
        default:
            break;
...
int tsce7_phy_rx_lane_control_get(const phymod_phy_access_t* phy,
                                  phymod_phy_rx_lane_control_t* rx_control)
{
    int enable, reset;
    uint32_t lb_enable;
    ...
    if (enable) {
        *rx_control = phymodRxSquelchOn;
        ...
        PHYMOD_IF_ERR_RETURN
            (temod16_rx_lane_control_get(&pm_phy_copy.access, &reset));
        if (reset == 0) {
...
int tsce7_phy_interface_config_set(const phymod_phy_access_t* phy,
                                   uint32_t flags,
                                   const phymod_phy_inf_config_t* config)
{
    ...
    enum merlin7_tsc_pll_div_enum pll_cfg;
#ifdef PHYMOD_DIAG
    const phymod_access_t *pm_acc;
    pm_acc = &phy->access;
    ...
    pm_phy_copy.access.lane_mask = 0x1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (tsce7_phy_firmware_lane_config_get(&pm_phy_copy,
    ...
    switch (config->data_rate) {
        ...
        default:
            PHYMOD_RETURN_WITH_ERR
                (PHYMOD_E_CONFIG,
                 (_PHYMOD_MSG("this speed %d is not supported by tsce7"),
                 config->data_rate));
            break;
    }
    ...
    PHYMOD_IF_ERR_RETURN
        (temod16_plldiv_lkup_get(&phy->access,
                                 spd_intf,
                                 config->ref_clock,
                                 &new_pll_div,
                                 &new_speed_vec));
    ...
    PHYMOD_IF_ERR_RETURN
        (temod16_osdfe_on_lkup_get(&phy->access, spd_intf, &os_dfeon));
    PHYMOD_IF_ERR_RETURN
        (temod16_scrambling_dis_lkup_get(&phy->access,
                                         spd_intf,
                                         &scrambling_dis));
    ...
    if(PHYMOD_INTF_F_SCRAMBLE_FORCED_ON & flags) {
        ...
        or_val = 1;
        PHYMOD_IF_ERR_RETURN
            (temod16_override_set(&phy->access,
        ...
        if (scrambling_dis) {
            ...
            PHYMOD_IF_ERR_RETURN
                (temod16_override_clear(&phy->access,
                                        TEMOD16_OVERRIDE_SCR_MODE));
            PHYMOD_IF_ERR_RETURN
                (temod16_override_clear(&phy->access,
                                        TEMOD16_OVERRIDE_DESCR_MODE));
    ...
    if (new_pll_div == TEMOD16_PLL_MODE_DIV_ILLEGAL) {
        PHYMOD_RETURN_WITH_ERR
            (PHYMOD_E_CONFIG, (_PHYMOD_MSG
            ("this speed %d is not supported at this ref clock %d"),
       ...
       !(PHYMOD_INTF_F_DONT_TURN_OFF_PLL & flags)) {
        ...
        switch (new_pll_div) {
            case TEMOD16_PLL_MODE_DIV_54:
                pll_cfg = MERLIN7_TSC_PLL_DIV_54P4;
                break;
            case TEMOD16_PLL_MODE_DIV_60:
                pll_cfg = MERLIN7_TSC_PLL_DIV_60;
                break;
            case TEMOD16_PLL_MODE_DIV_64:
                pll_cfg = MERLIN7_TSC_PLL_DIV_64;
                break;
            case TEMOD16_PLL_MODE_DIV_66:
                pll_cfg = MERLIN7_TSC_PLL_DIV_66;
                break;
            case TEMOD16_PLL_MODE_DIV_70:
                pll_cfg = MERLIN7_TSC_PLL_DIV_70;
                break;
            case TEMOD16_PLL_MODE_DIV_80:
                pll_cfg = MERLIN7_TSC_PLL_DIV_80;
                break;
            case TEMOD16_PLL_MODE_DIV_92:
                pll_cfg = MERLIN7_TSC_PLL_DIV_92;
                break;
            default:
                pll_cfg = MERLIN7_TSC_PLL_DIV_66;
                break;
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
             (_tsce7_phy_firmware_lane_config_set(&pm_phy_copy,
                                                  firmware_lane_config));
    }
...
#ifdef PHYMOD_DIAG
    PHYMOD_VDBG(DBG_SPD, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"
                PRIx32" spd=%0d(%s) media=%0d\n", __func__,
                (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, spd_intf,
...
STATIC
int _tsce7_speed_id_interface_config_get(const phymod_phy_access_t* phy,
                                     int speed_id,
...
int tsce7_phy_interface_config_get(const phymod_phy_access_t* phy,
                                   uint32_t flags,
                                   ...
                                   phymod_phy_inf_config_t* config)
{
    ...
    const phymod_access_t *pm_acc;
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_LNK, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32
                " flags=%"PRIx32"\n", __func__, (void *)pm_acc, pm_acc->addr,
                pm_acc->lane_mask, flags));
#endif
    ...
    PHYMOD_IF_ERR_RETURN
        (temod16_autoneg_control_get(&pm_phy_copy.access,
                                     &an_control,
                                     &an_complete));
    ...
    PHYMOD_IF_ERR_RETURN
        (_tsce7_speed_id_interface_config_get(phy,
                                              speed_id,
                                              config,
                                              an_control.enable,
                                              &firmware_lane_config));
    ...
    PHYMOD_VDBG(DBG_CFG, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32
                " sp_id=0x%0x rate=%0d phy_intf=%0d(%s) intf_md=%0x\n",
                __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask,
                speed_id, config->data_rate, config->interface_type,
                phymod_interface_t_mapping[config->interface_type].key,
...
int tsce7_phy_autoneg_ability_set(const phymod_phy_access_t* phy,
                                  const phymod_autoneg_ability_t* an_ability)
{
    ...
    if (PHYMOD_AN_CAP_SYMM_PAUSE_GET(an_ability) &&
        !PHYMOD_AN_CAP_ASYM_PAUSE_GET(an_ability)) {
    ...
    if (PHYMOD_AN_CAP_ASYM_PAUSE_GET(an_ability) &&
        !PHYMOD_AN_CAP_SYMM_PAUSE_GET(an_ability)) {
...
int tsce7_phy_autoneg_ability_get(const phymod_phy_access_t* phy,
...
int tsce7_phy_autoneg_remote_ability_get(const phymod_phy_access_t* phy,
                                  phymod_autoneg_ability_t* an_ability_get_type)
{
    temod16_an_ability_t value;
    temod16_an_control_t an_control;
...
int tsce7_phy_autoneg_set(const phymod_phy_access_t* phy,
                          const phymod_autoneg_control_t* an)
{
    ...
    if (an->num_lane_adv == 4) {
        phy_copy.access.lane_mask = 0x1 << start_lane;
        PHYMOD_IF_ERR_RETURN
            (tsce7_phy_firmware_core_config_get(&phy_copy,
                                                &firmware_core_config_tmp));
        PHYMOD_IF_ERR_RETURN
            (merlin7_core_soft_reset_release(&phy_copy.access, 0));
        ...
        PHYMOD_IF_ERR_RETURN
            (tsce7_phy_firmware_core_config_set(&phy_copy,
                                                firmware_core_config_tmp));
        PHYMOD_IF_ERR_RETURN
            (merlin7_core_soft_reset_release(&phy_copy.access, 1));
    }
    ...
    PHYMOD_IF_ERR_RETURN
        (tsce7_phy_firmware_lane_config_get(&phy_copy, &firmware_lane_config));
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        PHYMOD_IF_ERR_RETURN
            (_tsce7_phy_firmware_lane_config_set(&phy_copy,
                                                 firmware_lane_config));
    }
    ...
    if (an->enable) {
        ...
        PHYMOD_IF_ERR_RETURN
            (temod16_set_an_port_mode(&phy->access,
                                      an->enable,
...
int tsce7_phy_autoneg_get(const phymod_phy_access_t* phy,
...
STATIC
int _tsce7_core_init_pass1(const phymod_core_access_t* core,
                           ...
                           const phymod_core_status_t* core_status)
{
    ...
    TSCE7_CORE_TO_PHY_ACCESS(&phy_access, core);
    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));
    core_copy.access.lane_mask = 0x1;
    ...
    PHYMOD_IF_ERR_RETURN
        (temod16_refclk_set(&core_copy.access,
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        if (_tsce7_core_firmware_load(&core_copy,
    ...
    for (i = 0; i < TSCE7_NOF_LANES_IN_CORE; i++) {
...
STATIC
int _tsce7_core_init_pass2(const phymod_core_access_t* core,
                           const phymod_core_init_config_t* init_config,
                           const phymod_core_status_t* core_status)
{
    phymod_phy_access_t phy_access, phy_access_copy;
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        if (!PHYMOD_CORE_INIT_F_BYPASS_CRC_CHECK_GET(init_config)) {
            PHYMOD_IF_ERR_RETURN
                (merlin7_tsc_check_ucode_crc(&core_copy.access,
                                             merlin7_ucode_crc, 200));
        }
        for (lane = 0; lane < TSCE7_NOF_LANES_IN_CORE; lane++) {
    ...
    PHYMOD_IF_ERR_RETURN
        (tsce7_core_lane_map_set(&core_copy, &init_config->lane_map));
    ...
    PHYMOD_IF_ERR_RETURN
        (tsce7_phy_firmware_core_config_get(&phy_access_copy,
                                            &firmware_core_config_tmp));
    firmware_core_config_tmp.CoreConfigFromPCS = 0;

    PHYMOD_IF_ERR_RETURN
        (tsce7_phy_firmware_core_config_set(&phy_access_copy,
                                            firmware_core_config_tmp));
    ...
    if (PHYMOD_CORE_INIT_F_TOP_DEV_REV_ID_A0_GET(init_config)) {
        PHYMOD_IF_ERR_RETURN
            (merlin7_tsc_afe_hardware_version_get(&core_copy.access, &rev_id));
        /* Disable DCC on device revision A0 (rev_id = 0). */
        if (rev_id != 0) {
            PHYMOD_IF_ERR_RETURN
                (merlin7_tsc_afe_hardware_version_set(&core_copy.access, 0));
       }
    } else {
          /*
           * All other versions set non-zero value to core.afe_hardware_version,
           * which will use HW register default value.
           */
          PHYMOD_IF_ERR_RETURN
              (merlin7_tsc_afe_hardware_version_set(&core_copy.access, 1));
    }
     ...
     * tsce7_phy_init() by calling temod16_pmd_x4_reset().
     */
    PHYMOD_IF_ERR_RETURN
...
int tsce7_core_init(const phymod_core_access_t* core,
                    const phymod_core_init_config_t* init_config,
                    const phymod_core_status_t* core_status)
{
    if ((!PHYMOD_CORE_INIT_F_EXECUTE_PASS1_GET(init_config) &&
        ...
        PHYMOD_IF_ERR_RETURN
            (_tsce7_core_init_pass1(core, init_config, core_status));
    ...
    if ((!PHYMOD_CORE_INIT_F_EXECUTE_PASS1_GET(init_config) &&
         ...
         PHYMOD_CORE_INIT_F_EXECUTE_PASS2_GET(init_config)) {
        PHYMOD_IF_ERR_RETURN
            (_tsce7_core_init_pass2(core, init_config, core_status));
    }
...
int tsce7_phy_init(const phymod_phy_access_t* phy,
                   const phymod_phy_init_config_t* init_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (tsce7_phy_polarity_set(&pm_phy_copy, &tmp_pol));
    }
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        pm_phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        PHYMOD_IF_ERR_RETURN
            (tsce7_phy_tx_set(&pm_phy_copy, &init_config->tx[i]));
        /* Set DCC configuration. */
        PHYMOD_IF_ERR_RETURN
            (merlin7_tsc_apply_dcc_en_cfg(&pm_phy_copy.access));
    }
    ...
    PHYMOD_IF_ERR_RETURN
        (temod16_tx_lane_control_set(pm_acc,
                                     TEMOD16_TX_LANE_RESET_TRAFFIC_ENABLE));
...
int tsce7_phy_cl72_status_get(const phymod_phy_access_t* phy,
...
int tsce7_phy_loopback_set(const phymod_phy_access_t* phy,
                           ...
                           uint32_t enable)
{
    ...
    PHYMOD_VDBG(DBG_LPK, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32
                " lpbk=%0d(%s) en=%0d\n", __func__, (void *)pm_acc,
                pm_acc->addr, pm_acc->lane_mask, loopback,
    ...
    switch (loopback) {
        case phymodLoopbackGlobal:
            PHYMOD_IF_ERR_RETURN
                (tsce7_phy_cl72_get(phy, &cl72_en));
            ...
            PHYMOD_IF_ERR_RETURN
                (temod16_tx_loopback_control(&phy->access,
                                             enable,
                                             start_lane,
                                             num_lane));
            break;
        case phymodLoopbackGlobalPMD:
            PHYMOD_IF_ERR_RETURN
                (temod16_tx_squelch_set(&phy_copy.access, enable));
            PHYMOD_IF_ERR_RETURN
...
int tsce7_phy_loopback_get(const phymod_phy_access_t* phy,
                           phymod_loopback_mode_t loopback,
                           uint32_t* enable)
{
    ...
    *enable = 0;
    ...
    switch (loopback) {
        case phymodLoopbackGlobal:
            PHYMOD_IF_ERR_RETURN
                (temod16_tx_loopback_get(&phy->access, &enable_core));
            *enable = (enable_core >> start_lane) & 0x1;
            break;
        case phymodLoopbackGlobalPMD:
            PHYMOD_IF_ERR_RETURN
                (merlin7_pmd_loopback_get(&phy->access, enable));
            break;
        case phymodLoopbackRemotePMD:
...
int tsce7_phy_link_status_get(const phymod_phy_access_t* phy,
                              uint32_t* link_status)
{
    PHYMOD_IF_ERR_RETURN
        (temod16_get_pcs_latched_link_status(&phy->access, link_status));

    return PHYMOD_E_NONE;
}

int tsce7_phy_reg_read(const phymod_phy_access_t* phy,
                       uint32_t reg_addr,
                       uint32_t *val)
{
...
int tsce7_phy_reg_write(const phymod_phy_access_t* phy,
                        uint32_t reg_addr,
                        uint32_t val)
{
...
int tsce7_phy_rx_pmd_locked_get(const phymod_phy_access_t* phy,
...
int tsce7_phy_rx_ppm_get(const phymod_phy_access_t* phy, int16_t* rx_ppm)
{
    ...
    pm_phy_copy.access.lane_mask = 1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (merlin7_tsc_rx_ppm(&pm_phy_copy.access, rx_ppm));

    return PHYMOD_E_NONE;
}

int tsce7_phy_synce_clk_ctrl_set(const phymod_phy_access_t* phy,
...
int tsce7_phy_synce_clk_ctrl_get(const phymod_phy_access_t* phy,
                                 phymod_synce_clk_ctrl_t *cfg)
{
    PHYMOD_IF_ERR_RETURN
...
int tsce7_phy_pll_multiplier_get(const phymod_phy_access_t* phy,
                                 uint32_t* core_vco_pll_multiplier)
{
    phymod_phy_access_t phy_access_copy;
    uint32_t merlin7_pll_div;
    PHYMOD_MEMCPY(&phy_access_copy, phy, sizeof(phy_access_copy));

    PHYMOD_IF_ERR_RETURN
        (merlin7_tsc_INTERNAL_read_pll_div(&phy_access_copy.access, &merlin7_pll_div));

    switch (merlin7_pll_div) {
        case MERLIN7_TSC_PLL_DIV_70:
            *core_vco_pll_multiplier = phymod_TSCE_PLL_DIV70;
            break;
        case MERLIN7_TSC_PLL_DIV_66:
            *core_vco_pll_multiplier = phymod_TSCE_PLL_DIV66;
            break;
        case MERLIN7_TSC_PLL_DIV_64:
            *core_vco_pll_multiplier = phymod_TSCE_PLL_DIV64;
            break;
        case MERLIN7_TSC_PLL_DIV_60:
            *core_vco_pll_multiplier = phymod_TSCE_PLL_DIV60;
            break;
        case MERLIN7_TSC_PLL_DIV_92:
            *core_vco_pll_multiplier = phymod_TSCE_PLL_DIV92;
            break;
        case MERLIN7_TSC_PLL_DIV_80:
            *core_vco_pll_multiplier = phymod_TSCE_PLL_DIV80;
            break;
        case MERLIN7_TSC_PLL_DIV_54P4:
            *core_vco_pll_multiplier = phymod_TSCE_PLL_DIV54;
            break;
        default:
            return PHYMOD_E_UNAVAIL;
