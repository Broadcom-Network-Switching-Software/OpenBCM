...
#include "blackhawk_tsc_reg_dump.h"


static err_code_t _blackhawk_tsc_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section);
static err_code_t _blackhawk_tsc_reg_print_with_buf(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
static err_code_t _blackhawk_tsc_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
...
static err_code_t _blackhawk_tsc_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section)
{
    ...
    for (addr = blackhawk_tsc_reg_dump_arr[reg_section][0]; addr < blackhawk_tsc_reg_dump_arr[reg_section][1]; addr++) 
    {
        if (!(addr & 0x000F))  
        {
            EFUN_PRINTF(("\n%04x ",addr));
        }
        EFUN(blackhawk_tsc_acc_rdt_reg(sa__, addr,&rddata));
...
static err_code_t _blackhawk_tsc_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index)
{
    if(reg_buffer == NULL)
    {
         EFUN(_blackhawk_tsc_reg_print_no_buf(sa__, reg_section));
    }
    else
    {
        EFUN(_blackhawk_tsc_reg_print_with_buf(sa__, reg_section, reg_buffer, buf_index));
    }
    return (ERR_CODE_NONE);
}
...
{    
    ...
    while(reg_section < BLACKHAWK_TSC_REG_DUMP_SECTIONS) 
    {
        if(blackhawk_tsc_reg_dump_arr[reg_section][0] == BLACKHAWK_TSC_REG_PLL_START)
        {
            ...
            for (pll_idx = 0; pll_idx < DUAL_PLL_NUM_PLLS; pll_idx++)
            {
                ...
                while( reg_section < BLACKHAWK_TSC_REG_DUMP_SECTIONS && blackhawk_tsc_reg_dump_arr[reg_section][0] != BLACKHAWK_TSC_REG_UC_START)
                {
                    EFUN(_blackhawk_tsc_reg_print(sa__, reg_section, reg_buffer, &buf_index));
                    reg_section++;
                }
            }
            EFUN(blackhawk_tsc_set_pll_idx(sa__,pll_orig));
        }
        else if (blackhawk_tsc_reg_dump_arr[reg_section][0] == BLACKHAWK_TSC_REG_UC_START)
        {
...
err_code_t blackhawk_tsc_uc_lane_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][250]) {
  ...
  /* coverity[forward_null : FALSE] */
  /* coverity[var_deref_op : FALSE] */
  lane_var_ram_size = (uint16_t)blackhawk_tsc_info_ptr->lane_var_ram_size;
...
err_code_t blackhawk_tsc_log_full_pmd_state_noPRBS (srds_access_t *sa__, struct blackhawk_tsc_detailed_lane_status_st *lane_st) {
  ...
  }
  {
    ...
    ESTM(lane_st->lms_dac4ck[0] = (int8_t)rd_ams_rx_dac4ck_lms_i());
    ESTM(lane_st->lms_dac4ck[1] = (int8_t)rd_ams_rx_dac4ck_lms_q());
    /* Convert from sign+gray to sign+bin */
    lane_st->lms_dac4ck[0] = (int8_t)((lane_st->lms_dac4ck[0]&0x8)|blackhawk_tsc_INTERNAL_gray_to_uint8(lane_st->lms_dac4ck[0]&0x07));
    lane_st->lms_dac4ck[1] = (int8_t)((lane_st->lms_dac4ck[1]&0x8)|blackhawk_tsc_INTERNAL_gray_to_uint8(lane_st->lms_dac4ck[1]&0x07));
    if (lane_st->lms_dac4ck[0]>7) lane_st->lms_dac4ck[0] = (int8_t)(-(lane_st->lms_dac4ck[0]&0x07));
    if (lane_st->lms_dac4ck[1]>7) lane_st->lms_dac4ck[1] = (int8_t)(-(lane_st->lms_dac4ck[1]&0x07));
    ESTM(lane_st->phase_dac4ck[0] = (int8_t)rd_ams_rx_dac4ck_phs_i());
    ESTM(lane_st->phase_dac4ck[1] = (int8_t)rd_ams_rx_dac4ck_phs_q());
    /* Convert from sign+gray to sign+bin */
    lane_st->phase_dac4ck[0] = (int8_t)((lane_st->phase_dac4ck[0]&0x8)|blackhawk_tsc_INTERNAL_gray_to_uint8(lane_st->phase_dac4ck[0]&0x07));
    lane_st->phase_dac4ck[1] = (int8_t)((lane_st->phase_dac4ck[1]&0x8)|blackhawk_tsc_INTERNAL_gray_to_uint8(lane_st->phase_dac4ck[1]&0x07));
    if (lane_st->phase_dac4ck[0]>7) lane_st->phase_dac4ck[0] = (int8_t)(-(lane_st->phase_dac4ck[0]&0x07));
    if (lane_st->phase_dac4ck[1]>7) lane_st->phase_dac4ck[1] = (int8_t)(-(lane_st->phase_dac4ck[1]&0x07));
    ESTM(lane_st->data_dac4ck[0] = (int8_t)rd_ams_rx_dac4ck_dat_i());
    ESTM(lane_st->data_dac4ck[1] = (int8_t)rd_ams_rx_dac4ck_dat_q());
    /* Convert from sign+gray to sign+bin */
    lane_st->data_dac4ck[0] = (int8_t)((lane_st->data_dac4ck[0]&0x8)|blackhawk_tsc_INTERNAL_gray_to_uint8(lane_st->data_dac4ck[0]&0x07));
    lane_st->data_dac4ck[1] = (int8_t)((lane_st->data_dac4ck[1]&0x8)|blackhawk_tsc_INTERNAL_gray_to_uint8(lane_st->data_dac4ck[1]&0x07));
    if (lane_st->data_dac4ck[0]>7) lane_st->data_dac4ck[0] = (int8_t)(-(lane_st->data_dac4ck[0]&0x07));
    if (lane_st->data_dac4ck[1]>7) lane_st->data_dac4ck[1] = (int8_t)(-(lane_st->data_dac4ck[1]&0x07));
    ...
    ESTM(lane_st->dummy9[0] = rdv_psv_sts_byte());
    ESTM(lane_st->dummy9[1] = rdv_psv_ctl_byte());
...
err_code_t blackhawk_tsc_disp_full_pmd_state (srds_access_t *sa__, struct blackhawk_tsc_detailed_lane_status_st const * const lane_st, uint8_t num_lanes) {
    ...
    EFUN_PRINTF(("\n**** SERDES EXTENDED LANE DIAG DATA CORE %d LANE %d DUMP ****", blackhawk_tsc_get_core(sa__), blackhawk_tsc_get_lane(sa__)));
    EFUN_PRINTF(("\n0000 "));
    ESTM_PRINTF(("%02x ", num_lanes));
    ESTM_PRINTF(("%02x ", big_endian));
    ESTM_PRINTF(("%02x ", (uint8_t)sizeof(struct blackhawk_tsc_check_platform_info_st)));
...
/*  Note: No EFUNs or ESTMs should be used in this function as this print routine is called by _error() handler. */
    ...
    if (blackhawk_tsc_version(sa__, &info.api_ver)) {
