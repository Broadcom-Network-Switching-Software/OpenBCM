 ...
 *  Summary:
 *  ========
 *  This cint example demonstrates TCB configuration to detect wred drop 
 *
 ...
 *    2) Step2 - Configuration (Done in tcb_wred_config())
 *    =======================================================
 *      a) Sets up tcb on egress port to detect wred drop
...
int rv = 0;
int unit = 0;
bcm_port_t port; /* WRED and tcb are configured on L2 queue of this egress port */
...
int i = 16; /* Number of queues assigned to port */
...
int tcb_wred_config(bcm_gport_t egr_port)
{
    int unit = 0;
...
bcm_error_t execute()
{
    ...
    if( (rrv = tcb_wred_config(port)) != BCM_E_NONE )
    {
