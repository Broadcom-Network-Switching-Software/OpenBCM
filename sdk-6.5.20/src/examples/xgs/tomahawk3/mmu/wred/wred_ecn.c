 ...
 * VLAN=2,Pri=4  cd0(1)|            |cd1(2)   VLAN=2,Pri=4
 *                     | l2:mac=0x1 |
...
bcm_error_t mmu_wred_ecn_setup(int unit) 
{
 ...
 discard_cfg.min_thresh = 1*208; /*Queue depth in bytes to begin dropping.(208 bytes = 1 cell in TH)*/
 discard_cfg.max_thresh = 4*208; /*Queue depth in bytes to drop all packets(208 bytes = 1 cell in TH)*/
 ...
 discard_cfg.min_thresh = 3*208; /*Queue depth in bytes to begin dropping.(208 bytes = 1 cell)*/
 discard_cfg.max_thresh = 5*208; /*Queue depth in bytes to drop all packets(208 bytes = 1 cell*/
 discard_cfg.drop_probability = 90; /*Drop probability at max threshold*/
 rv = bcm_cosq_gport_discard_set(unit, egr_port, cos, &discard_cfg);
 if (rv != BCM_E_NONE) {
...
void verify(int unit)
{
 ...
 if(check){
    ...
    bshell(unit,"d IP_TO_INT_CN_MAPPING");
    bshell(unit,"d INT_CN_TO_MMUIF_MAPPING");
    bshell(unit,"d chg MMU_WRED_QUEUE_CONFIG");
    bshell(unit,"d chg MMU_WRED_DROP_CURVE_PROFILE_0_0");
    bshell(unit,"d chg MMU_WRED_DROP_CURVE_PROFILE_0_3");
    bshell(unit,"d chg MMU_WRED_DROP_CURVE_PROFILE_0_5");
    bshell(unit,"d EGR_INT_CN_UPDATE");
    bshell(unit,"d EGR_INT_CN_TO_IP_MAPPING");
 }
 bshell(unit, "show c");
}
