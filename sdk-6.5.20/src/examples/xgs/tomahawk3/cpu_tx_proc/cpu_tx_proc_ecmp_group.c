 ...
 */
/*  Feature  : Cpu Tx Proc ECMP group
 *                                   
 *  Usage    : BCM.0> cint cpu_tx_proc_ecmp_group.c
 *                                                 
 *  config   : cpu_tx_proc_config.bcm              
 *                                                 
 *  Log file : cpu_tx_proc_ecmp_group_log.txt      
 *                                                 
 *  Test Topology :                                
 *                                                 
 *                   +------------------------------+  ingress_port
 ...
 *    type as "ECMP Group". This will mimic as if packet is ingress from a front panel
 *    port destined for ECMP Group and ingress pipeline will send out of one of the ECMP
 *    member egress port.                                                               
 *                                                                                      
 ...
 *                                                                                      
 *    2) Step2 - Configuration (Done in cpu_tx_proc_ecmp_group_setup())                 
 *    ============================================                                      
 *      a) There is nothing to be done in cpu_tx_proc_ecmp_group_setup                  
 *                                                                                      
 *    3) Step3 - Verification(Done in verify())                                         
 *    ==========================================                                        
 *      a) Transmit one packet with dest_ip=10.10.10.10, vlan=10 and destination type as "ecmp group"
 *         using bcm_tx() API                                                                        
 *      b) Transmit one packet with dest_ip=10.10.10.11, vlan=10 and destination type as "ecmp group"
 *         using bcm_tx() API                                                                        
 *      c) Transmit one packet with dest_ip=10.10.10.12, vlan=10 and destination type as "ecmp group"
*         using bcm_tx() API                                                                        
 *      d) Expected Result:                                                                          
 *      ===================                                                                          
 *         All 3 packets will be injected into pipeline with ingress port as source port and         
 *         transmitted to "ecmp group". The HW logic will pick one of the ecmp member based on hash logic
 *         as egress port.                                                                               
 *         CPU_MASQUERADE_COUNTER_DEST_TYPE_PIPE0(3) register count will increment by 1.                 
 ...
 */                                                      
bcm_error_t                                              
cpu_tx_proc_ecmp_group_pkt_send(int unit, uint32 ecmp_group_id, uint8 *pktData, int length)
{                                                                                          
    bcm_pkt_t  *pkt;                                                                       
    ...
    pkt->txprocmh_destination_type = 2; /*SOC_TX_PROC_OP_ECMP_GROUP*/           
    pkt->rx_port = ingress_port;                                         
    pkt->txprocmh_destination = ecmp_group_id;                           
...
/* Tx Setup */
bcm_error_t cpu_tx_proc_ecmp_group_setup(int unit)
{                                                 
    return BCM_E_NONE;                            
} 
...
{                                                         
    bcm_error_t rv=0;                                     
    rv = cpu_tx_proc_ecmp_group_pkt_send(unit, ecmp_group_id, pkt1, pkt1_SIZEOF);
    rv = cpu_tx_proc_ecmp_group_pkt_send(unit, ecmp_group_id, pkt2, pkt2_SIZEOF);
    rv = cpu_tx_proc_ecmp_group_pkt_send(unit, ecmp_group_id, pkt3, pkt3_SIZEOF);
    return rv;                                                  
} 
...
bcm_error_t verify(int unit)
{
    bcm_error_t rv = BCM_E_NONE;
    rv = test_harness(0);
    bshell(unit, "g CPU_MASQUERADE_COUNTER_DEST_TYPE_PIPE0(3)");
    sal_usleep(1000000);
    bshell(unit, "show c");
    return rv;
}
...
bcm_error_t execute()
{
    ...
    rv = cpu_tx_proc_ecmp_group_setup(unit);
    if (BCM_FAILURE(rv)) {
