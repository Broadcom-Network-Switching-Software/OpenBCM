 ...
 *
 *  Usage    : BCM.0> cint flex_cnt_l3_egress_new_api.c
 *
 *  config   : flex_cnt_config.bcm
 *
 *  Log file : flex_cnt_l3_egress_new_api_log.txt
 *  
 *  Test Topology :
 *
 *                   +------------------------------+
 ...
 *  ========
 *
 *     This cint example configures and attaches a flex counter to a EGR L3 nexthop.
 ...
 *
 *    2) Step2 - Configuration (Done in config_flex_cnt_l3_egress())
 *    ===============================================================
...
bcm_error_t config_flex_cnt_l3_egress(int unit, bcm_if_t *intf_id)
{
  bcm_error_t rv;
  bcm_if_t ingress_if;
...
/* Add L3 Route Entry another direction */
  ...
  *intf_id = l3_egr_obj_id;
  return BCM_E_NONE;
}

/* Get Egr L3 NextHop flex cnt stats*/
bcm_error_t l3egrif_flex_stat_get(int unit, bcm_if_t intf_id)
{
  bcm_error_t rv;
  ...
  rv = bcm_l3_egress_stat_counter_sync_get(unit, intf_id, bcmL3StatOutPackets, num_counters, l3egrif_flex_cnt_idx, l3egrif_flex_cnt_val);
  if (BCM_FAILURE(rv)) {
  ...
  rv = bcm_l3_egress_stat_counter_sync_get(unit, intf_id, bcmL3StatOutBytes, num_counters, l3egrif_flex_cnt_idx, l3egrif_flex_cnt_val);
  if (BCM_FAILURE(rv)) {
      printf("Error executing bcm_l3_egress_stat_counter_sync_get(): %s.\n", bcm_errmsg(rv));
      return rv;
  }
...
void verify(int unit, bcm_if_t intf_id)
{
 ...
 /* Send 3 packets with outer tag pri 0.
 *  Ethernet: dst<00:00:00:00:11:11> src<00:00:00:00:00:01> Tagged Packet ProtID<0x8100> Ctrl<0x0015> Internet Protocol (IP)
 *  IP: V(4) src<2.2.2.2> dst<1.1.1.1> hl<5> service-type<0> tl<46> id<0> frg-off<0> ttl<64> > chk-sum<0x73cc>
 */
  printf("Sending 3 packets with outer tag pri 0.\n");
  ...
  /*Retrieve Egr L3 NextHop flex counters and print them.*/
  printf("\nDumping Egr L3 NextHop counters after sending 3 packets with outer tag pri 0..\n");
  l3egrif_flex_stat_get(unit, intf_id);

  /* Send another 2 packets with outer tag pri 0.
 *  Ethernet: dst<00:00:00:00:11:11> src<00:00:00:00:00:01> Tagged Packet ProtID<0x8100> Ctrl<0x0015> Internet Protocol (IP)
 *  IP: V(4) src<2.2.2.2> dst<1.1.1.1> hl<5> service-type<0> tl<46> id<0> frg-off<0> ttl<64> > chk-sum<0x73cc>
 */
 
  printf("Sending another 2 packets with outer tag pri 0.\n");
  ...
  /*Retrieve Egr L3 NextHop flex counters and print them.*/
  printf("\nDumping Egr L3 NextHop counters after sending another 2 packets with outer tag pri 0..\n");
  l3egrif_flex_stat_get(unit, intf_id);


  /*Send 2 packets with outer tag pri 1.
   * Ethernet: dst<00:00:00:00:11:11> src<00:00:00:00:00:01> Tagged Packet ProtID<0x8100> Ctrl<0x2015> Internet Protocol (IP)
   * IP: V(4) src<2.2.2.2> dst<1.1.1.1> hl<5> service-type<0> tl<46> id<0> frg-off<0> ttl<64> > chk-sum<0x73cc>  
   */
  printf("Sending  2 packets with outer tag pri 1.\n");
  snprintf(str, 512, "tx 2 pbm=%d data=0x0000000011110000000000018100201508004500002E0000000040FF73CC0202020201010101000102030405060708090A0B0C0D0E0F1011121314151617181962D78C86; sleep 1", ingress_port);
  bshell(unit, str); 

  /*Retrieve Egr L3 NextHop flex counters and print them.*/
  printf("\nDumping Egr L3 NextHop counters after sending 2 packets with outer tag pri 1..\n");
  l3egrif_flex_stat_get(unit, intf_id);

  /*Send 2 packets with outer tag pri 2.
   * Ethernet: dst<00:00:00:00:11:11> src<00:00:00:00:00:01> Tagged Packet ProtID<0x8100> Ctrl<0x4015> Internet Protocol (IP)
   * IP: V(4) src<2.2.2.2> dst<1.1.1.1> hl<5> service-type<0> tl<46> id<0> frg-off<0> ttl<64> > chk-sum<0x73cc>
   */
  printf("Sending  2 packets with outer tag pri 2.\n");
  snprintf(str, 512, "tx 2 pbm=%d data=0x0000000011110000000000018100401508004500002E0000000040FF73CC0202020201010101000102030405060708090A0B0C0D0E0F10111213141516171819AC7A6762; sleep 1", ingress_port);
  bshell(unit, str);

  /*Retrieve Egr L3 NextHop flex counters and print them.*/
  printf("\nDumping Egr L3 NextHop counters after sending 2 packets with outer tag pri 2..\n");
  l3egrif_flex_stat_get(unit, intf_id);
 ...
 * a)test setup
 * b)actual configuration (Done in config_flex_cnt_l3_egress())
...
bcm_error_t execute()
{
  ...
  if (BCM_FAILURE((rv = config_flex_cnt_l3_egress(unit, &intf_id)))) {
    printf("config_flex_cnt_l3_egress() failed.\n");
    return -1;
  }
  printf("Completed configuration(i.e executing config_flex_cnt_l3_egress()) successfully.\n");
