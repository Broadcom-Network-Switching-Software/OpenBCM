 ...
 *
 *  config   : ptrace_lag_ecmp_config.bcm
 *
...

{
    ...
    printf("===RAW DATA===\n");
    BCM_IF_ERROR_RETURN(ptrace_raw_info_print(pkt_trace_info));
...
/*
 * Print the raw data array returned by the BCM packet trace API
 */
int
ptrace_raw_info_print(bcm_switch_pkt_trace_info_t *pkt_trace_info)
{
    int  ri;
    long long data;

    /*
     * Print BCM_SWTCH_PKT_TRACE_RAW_DATA_MAX bytes which have
     * been populated by the API. Print two double words (64-bits) per line
     */
    for (ri = 0; ri < BCM_SWITCH_PKT_TRACE_RAW_DATA_MAX; ) {
        sal_memcpy(&data, &(pkt_trace_info->raw_data[ri]), 8);
        printf("%016llx ", data);
        ri += 8;
        sal_memcpy(&data, &(pkt_trace_info->raw_data[ri]), 8);
        printf("%016llx\n", data);
