...
/*  Feature  : Subnet NAT
 *
 *  Usage    : BCM.0> cint subnet_nat.c
 *
 *  config   : subnet_nat_config.bcm
 *
 *  Log file : subnet_nat_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+  
 ...
 *  ========
 *
 *    Subnet-based NAT replaces only the subnet prefix in the IP address, leaving the rest of the IP address
 *    unchanged. Subnet-based NAT is a simple implementation of basic NAT. For example, a private network
 *    is interconnected with a public network   through a switch enabling subnet-based basic NAT. The
 *    public address for the private network (128.17.18.0/24 ) is 200.0.0.0/24. In the inbound direction, for a packet
 *    with DIP = 200.0.0.1, the subnet-based Basic NAT router will replace the subnet prefix 200.0.0/24 with
 *    new subnet prefix 128.17.18/24, and keep the host address .1/8. The translated SIP is 128.17.18.1
 *    In the outbound direction, the SIP is translated in same fashion.
 *    The BCM56770 uses the ICAP NAT policies to implement subnet-based Basic NAT.The below example shows 
 *    an inbound NAT where DIP is changed based on the Subnet
 *
 ...
 *
 *    2) Step2 - Configuration (Done in config_subnet_nat())
 *    ============================================
 ...
 *      d) Configure NAT
 *      e) Configure NAT for inbound flows
 *
 ...
 *         src_ip=10.0.0.1(0x0a000001), dst_ip=0x80111201
 *         [bcmPW.0]
 *         [bcmPW.0]Packet[2]: data[0000]: {000000000201} {000000000200} 8100 00c8
 *         [bcmPW.0]Packet[2]: data[0010]: 0800 4500 006e 0000 0000 3f06 df77 0a00 
 *         [bcmPW.0]Packet[2]: data[0020]: 0001 8011 1201 07d0 0b0c 0000 0000 0000 
 *         [bcmPW.0]Packet[2]: data[0030]: 0000 5000 0000 55e2 0000 0001 0203 0405 
 ...
 *         [bcmPW.0]Packet[2]: data[0080]: 456f 8564 f1fc 7e51 
...
int nat_subnet_inbound_create(int unit, bcm_l3_nat_id_t *out_nat_id,
		...
		bcm_ip_t pub_dip, uint16 pub_l4_dport)
{
	...
	egr_nat.flags               = BCM_L3_NAT_EGRESS_DNAT ;
	egr_nat.dip_addr            = priv_ip;
	egr_nat.dip_addr_mask       = 0xffffff00;

	BCM_IF_ERROR_RETURN(bcm_l3_nat_egress_add(unit, &egr_nat));

	printf("Subnet nat created for nat_id=%d\n", egr_nat.nat_id);
	*out_nat_id = egr_nat.nat_id;
	...
	BCM_IF_ERROR_RETURN(bcm_field_qualify_DstIp(unit, entry, pub_dip, 0xffffff00));
...
/* configure subnet nat */
bcm_error_t config_subnet_nat(int unit)
{
    ...
    rv = nat_subnet_inbound_create(0,&nat_id,0,0,0x80111200,0,0,0,0xC8000001,0);
...
test_harness(int unit)
{
    ...
    Src IP = 10.0.0.1 & Dest IP = 0x80111201 after subnet NAT
    
    Egress packet with Src IP = 0x0a000001 & Dest IP = 0x80111201 
...
bcm_error_t execute()
{
    ...
    if (BCM_FAILURE((rv = config_subnet_nat(unit)))) {
