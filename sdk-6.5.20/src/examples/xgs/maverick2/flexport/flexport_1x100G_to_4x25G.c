...
/*  Feature  : Flexport
 *                   
 *  Usage    : BCM.0> cint flexport_1x100G_to_4x25G.c
 *                                 
 *  config   : flexport_config.bcm   
 *                                 
 *  Log file : flexport_1x100G_to_4x25G_log.txt      
 *                                 
 *  Test Topology :                
 *                                 
 *                   +------------------------------+  
 ...
 *    ============================================
 *      a) Select a 100G port.
 *
 *    2) Step2 - Configuration (Done in flexport_setup())
 *    ============================================
 *      a) Disable linkscan for the selected 100G port.
 *      b) Disable the selected 100G port. 
 *
 *    3) Step3 - Verification(Done in verify())
 *    ==========================================
 *      a) Dispaly port information before flex operation.
 *      b) Flex the selected 100G port. 
 *      c) Enable linkscan for the new 25G flexed ports.
 *      d) Enable newly 25G flexed ports.
 *      d) Expected Result:
 *      ===================
 *         The 100G port will get flexed to four 25G ports. ps command output will show 
 *         new 4 25G ports.
 */
...
int logical_to_phy[4] = {0,25,29,33}; 
...
int flex_1x100g_to_4x25g(int unit , bcm_gport_t port, int phy_port)
{
    int i = 0;
    int lanes = 1; /* 1 lane for each 25G port*/
...
bcm_error_t verify(int unit)
{
    ...
    rv = flex_1x100g_to_4x25g(0,logical_port,logical_to_phy[logical_port]); 
    if(BCM_FAILURE(rv)) {
