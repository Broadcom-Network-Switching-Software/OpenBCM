...
/*
 * Maverick2 Rx Queue Channel Configuration
 *
 * The mechanism for configuring Rx Queue channels on Mavreick2 
 * (BCM56770) differs from previous devices. Previous devices used
 * bcm_tx_queue_channel_set(). This API does not apply to Maverick2. For
 * Maverick2 devices, the user needs to use a combination of cosq attach/detach
 * operations on L0 gports. On Maverick2, the L0 nodes are directly wired
 ...
 * 
 * bcm_error_t
 * mv2_rx_queue_channel_set(int unit,
 ...
 */
bcm_error_t
get_mv2_cpu_queues(int unit, cpu_queues_t * cpu_queues)
{
    int cos_idx;
    int input_idx;
    int rv = BCM_E_NONE;
 ...
 * used in place of bcm_rx_queue_channel_set() for Maverick2 device.
 */
bcm_error_t
mv2_rx_queue_channel_set(int unit,
                              cpu_queues_t * cpu_queues,
...
int rx_cpu_cosq_setup(int unit)
{
     ...
     */
    rv = get_mv2_cpu_queues(unit, &cpu_cosq_map_db);
    if(BCM_FAILURE(rv)) {
        printf("\nError in get_mv2_cpu_queues(): %s.\n",bcm_errmsg(rv));
        return rv;
    }
 ...
 *
 * Demonstrates how to map Rx PCI channels for Maverick2.
...
test_harness(int unit)
{
    ...
    */ 
    for (cos = 0; cos < max_rx_cos; cos++) {
        ...
        rv = mv2_rx_queue_channel_set(unit, &cpu_cosq_map_db, cos, rx_queue_channel);
        if(BCM_FAILURE(rv)) {
