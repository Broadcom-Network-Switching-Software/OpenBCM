...
/*
 * Feature  : FB6(56470) Vxlan Bridging , IPv4 access to network tunnel initiation Flow, Verifying port membership check based on VFI (VFI,DGLP).
 *
 * Usage    : BCM.0> cint ipv4_AtoN_Egress_VFI_DGLP_Membership.c
 *
 * config   : config_fb6_flex.bcm
 *
 * Log file : ipv4_AtoN_Egress_VFI_DGLP_Membership_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
	...
	bshell(unit, "l2 show");
        ...
        /* By default, during vfi creation, a default membership profile index(EGR_VLAN_VFI_MEMBERSHIP) with all egress ports added will be assigned to EGR_VFI table.
           So first remove the egress port from default profile index to validate {VFI,DGLP} checks
        */
	if (BCM_E_NONE != bcm_vlan_gport_delete(unit, vpn, egress_phy_gport) ) {
    	    printf("bcm_vlan_gport_delete() failed\n");
	    return -1;
        }

	printf("\n\n Enabling membership check on egress port and we will see packet get dropped on egress port \n");
	if (BCM_E_NONE != bcm_port_vlan_member_set(unit, egress_phy_gport, BCM_PORT_VLAN_MEMBER_EGRESS) ) {
    	    printf("bcm_port_vlan_member_set() failed\n");
	    return -1;
	}
	bshell(unit, "show c");

	snprintf(str1, 64, "cstat set %d tx 9 TxNotFwdDomainMemberDrop; sleep quiet 1", egress_port);
	...
	printf("\n\n Now Adding egress port  for VFI based membership check and packet get egress on network port\n");
        if (BCM_E_NONE != bcm_vlan_gport_add(unit, vpn, egress_phy_gport, BCM_VLAN_GPORT_ADD_EGRESS_ONLY) ) {
