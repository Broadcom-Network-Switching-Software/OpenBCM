 ...
 *
 * Usage    : BCM.0> cint ipv4_NtoA_acc_encap_ovid.c
 *
 * config   : config_fb6_flex.bcm
 *
 * Log file : ipv4_NtoA_acc_encap_ovid_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
Output (access):
=======
data[0000]: {00000000bbbb} {00000000aaaa} 8100 0019
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port)
{
    ...
    /*Add/Replace OVID based on OVID+DVP*/
    bcm_vlan_action_set_t action;
    action.new_outer_vlan = 0x19;
    action.ot_outer = bcmVlanActionReplace;
    rv = bcm_vlan_translate_egress_action_add(unit, *acc_flow_port, vxlan_acc->acc_vlan, 0, &action);
    if (BCM_FAILURE(rv)) {
