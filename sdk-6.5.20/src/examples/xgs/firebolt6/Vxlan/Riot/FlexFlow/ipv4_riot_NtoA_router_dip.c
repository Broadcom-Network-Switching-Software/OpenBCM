 ...
 *            this is a case when overlay IP is self or router IP and is
 *            being copied to host CPU
 *
 * Usage    : BCM.0> cint ipv4_riot_NtoA_router_dip.c
 *
 * config   : config_fb6_flex_riot.bcm
 *
 * Log file : ipv4_riot_NtoA_router_dip_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
NETWORK TO ACCESS:-
Decap-case: packet sent to network port
...
[bcmPW.0]Packet[1]: length 126 (126). rx-port 5. cos 0. prio_int 0. vlan 11. reason 0x1. Outer tagged.
[bcmPW.0]Packet[1]: dest-port 1. dest-mod 0. src-gport -1342177278. opcode 1.  matched 0. classification-tag 0.
[bcmPW.0]Packet[1]: reasons: FilterMatch
[bcmPW.0]Packet[1]: reasons: Nhop       >>> this is copy to cpu with reason
 */
...
bcm_error_t
vxlan_router_egr_obj(int unit, uint32 flag, int l3_if, bcm_mac_t nh_mac,
                     bcm_gport_t gport,int vid, bcm_if_t *egr_obj_id)
{
    bcm_l3_egress_t l3_egress;
    ...
    bcm_l3_egress_t_init(&l3_egress);
    l3_egress.flags2 |= flag;
    l3_egress.flags = BCM_L3_COPY_TO_CPU | BCM_L3_DST_DISCARD;  //send to cpu
    l3_egress.intf  = l3_if;
    sal_memcpy(l3_egress.mac_addr,  nh_mac, sizeof(nh_mac));
    l3_egress.port = gport;
    return bcm_l3_egress_create(unit, 0, &l3_egress, egr_obj_id);
                    ...
                    bcm_gport_t *acc_flow_port)
{
    ...
    printf("Access: create OL L3 egress object for case of router IP\n");
    rv = vxlan_router_egr_obj(unit,0,acc_intf_ol,vxlan_acc->acc_ol_nh_mac,
                      vxlan_acc->acc_gport,vxlan_acc->acc_vlan,&acc_egr_obj_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vxlan_router_egr_obj: %s\n", bcm_errmsg(rv));
        return rv;
    }
