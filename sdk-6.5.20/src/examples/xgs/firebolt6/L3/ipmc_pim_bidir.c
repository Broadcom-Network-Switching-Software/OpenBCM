...
int
create_ingress_l3_intf(int unit, bcm_vrf_t vrf, bcm_if_t l3_if1)
{
    ...
    l3_intf_id = l3_if1 + BCM_VLAN_MAX + 1;
...
config_l3_ipmc_pim_bidir(int unit)
{
    ...
    rv = create_ingress_l3_intf(unit, vrf, l3_if1);
    if (BCM_FAILURE(rv)) {
    ...
    rv = create_ingress_l3_intf(unit, vrf, l3_if2);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring ingress l3 interface : %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = create_ingress_l3_intf(unit, vrf, l3_if3);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring ingress l3 interface : %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = create_ingress_l3_intf(unit, vrf, l3_ingif);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring ingress l3 interface : %s.\n", bcm_errmsg(rv));
        return rv;
    }
