...
// TH3 related registers won't be dumped for Firebolt6
int opt_IsDevTomahawk3 = 0;
...
/* Populate the xe/ce/cd ports. Timestamping not supported on GE ports */
...
ConfigurePktTimestamp(int unit)
{
    ...
    intf.id = 0;
    intf.flags = BCM_TIME_ENABLE | BCM_TIME_WITH_ID;
    print bcm_time_interface_add(0,intf);
    bcm_time_capture_t time = {0};
...
bcm_error_t verify()
{
    ...
    for(i=0; i<10; ++i)
    {
        ...
        printf("\n\n\n############ %d ###########\n", i);
        sprintf(cmd, "tx %d pbm=%d data=%02X%02X%02X%02X%02X%02X0000000000CC8100%04X08004500002E0000000040063E8C%08X%08X000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1f202122232425262728292a2b2c2d2e2f30; sleep quiet 2",
