...
/*
 * Feature  : VPLS Tunnel (L2 MPLS) Initiation and Termination scenarios
 *
 * Usage    : BCM.0> cint vpls.c
 *
 * config   : mpls_config.bcm
 *
 * Log file : vpls_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *
 *   2) Step2 - Configuration (Done in config_vpls()):
 *   =========================================================
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_vpls())
...
execute(void)
{
    ...
    print "~~~ #2) config_vpls(): ** start **";
    if (BCM_FAILURE((rv = config_vpls(unit)))) {
        printf("config_vpls() failed.\n");
        return -1;
    }
    print "~~~ #2) config_vpls(): ** end **";
