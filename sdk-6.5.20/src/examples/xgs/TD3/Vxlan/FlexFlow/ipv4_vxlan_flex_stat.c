...
/*
 * Feature  : TD3(56870) Vxlan Bridging , IPv4 access to network tunnel initiation Flow with port
 * as a flow match criteria(BCM_FLOW_MATCH_CRITERIA_PORT)
 * In this script, flex stats attach at the ingress access virtual port and
 * the egress network virtual port is demonstrated.
 *
 * Usage    : BCM.0> cint ipv4_vxlan_flex_stat.c
 *
 * config   : config_td3_flex.bcm
 *
 * Log file : ipv4_vxlan_flex_stat_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
    ...
    /*Get Ing stat*/
    printf("\nGet Ingress Stat:");
    flex_stat_get_count(unit, ing_stat_id);
...
/*Flex stat at Ingress SVP */
int
config_ing_svp_flex_stat(int unit, bcm_gport_t acc_flow_port, bcm_flow_handle_t flow_handle, uint32 *ing_stat_id)
{
    bcm_error_t rv = BCM_E_NONE;
    ...
    object = bcmStatObjectIngGport;
    int stat_flag = BCM_STAT_GROUP_MODE_INGRESS;
    total_counters = 1;
    ...
    *ing_stat_id = stat_cnt_id;

    bcm_flow_stat_info_t stat_info;
    bcm_flow_stat_info_t_init(&stat_info);

    stat_info.flow_handle = flow_handle;
    stat_info.function_type = bcmFlowFuncTypeStart;
    stat_info.valid_elements = BCM_FLOW_STAT_FLOW_PORT_VALID | BCM_FLOW_STAT_MATCH_CRITERIA_VALID;
    stat_info.match_criteria = BCM_FLOW_MATCH_CRITERIA_PORT;
    stat_info.flow_port = acc_flow_port;
...
/*Flex stat at  *Egress network VP*/
int
config_egr_svp_flex_stat(int unit, bcm_gport_t net_flow_port, bcm_flow_handle_t flow_handle, uint32 *egr_stat_id)
{
    bcm_error_t rv = BCM_E_NONE;
    uint32 stat_cnt_id=0;
    int object;
    ...
    *egr_stat_id = stat_cnt_id;
    ...
    stat_info.flow_handle = flow_handle;
    stat_info.function_type = bcmFlowFuncTypeEncapSet;
    stat_info.valid_elements = BCM_FLOW_STAT_FLOW_PORT_VALID;
    stat_info.flow_port = net_flow_port;
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    rv = config_ing_svp_flex_stat(unit, acc_flow_port, flow_handle, &ing_stat_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in flex stat attach: %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /*create net_flow_port with vnid 0x12345*/
    ...
    rv = config_egr_svp_flex_stat(unit, net_flow_port, flow_handle,&egr_stat_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in flex stat attach: %s.\n", bcm_errmsg(rv));
        return rv;
    }
