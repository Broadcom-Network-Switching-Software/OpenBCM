 ...
 *
 *  Usage    : BCM.0> cint double_vlan_xc_vp2vp.c
 *
 *  config   : BCM56275_A1-PORT.bcm
 *
 *  Log file : double_vlan_xc_vp2vp_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+
 ...
 *  Summary:
 *  ========
 *    This cint example configures double vlan cross-connect between two VLAN virtual ports
 *    using BCM API.
 *
 ...
 *
 *    2) Step2 - Configuration (Done in configure_double_vlan_cross_connect_vp()):
 *    ============================================================================
 ...
 *
 *      c) Enable double VLAN cross-connect on VLAN 20 and disable learning on VLAN 20.
 *
 ...
 *
 *      e) Configure double vlan cross-connect for {outer_vlan(20), inner_vlan(10)}
 *         with two vlan virtual ports as members.
 *
 *    3) Step3 - Verification (Done in verify()):
 *    ===========================================
 *      a) Send a packet with double VLAN tagged packet{outer_vlan(20), inner_vlan(10)}
 *         on ingress port.
 *
 *      b) Expected Result:
 *      ===================
...
/* Enable double vlan cross connect */
bcm_error_t
enable_double_vlan_cross_connect(int unit, bcm_vlan_t vlan, bcm_vlan_control_vlan_t *vlan_ctrl)
{
    bcm_error_t rv = BCM_E_NONE;

    bcm_vlan_control_vlan_t_init(*vlan_ctrl);
    vlan_ctrl->flags = BCM_VLAN_LEARN_DISABLE;
    vlan_ctrl->forwarding_mode = bcmVlanForwardDoubleCrossConnect;
...
bcm_error_t
create_l2_vp(int unit, bcm_vlan_t outer_vlan, bcm_vlan_t inner_vlan, bcm_gport_t gport, bcm_vlan_port_t *vp)
{
    bcm_error_t rv = BCM_E_NONE;
    vp->flags = BCM_VLAN_PORT_INNER_VLAN_PRESERVE;
    vp->criteria = BCM_VLAN_PORT_MATCH_PORT_VLAN_STACKED;
    vp->match_vlan = outer_vlan;
    vp->match_inner_vlan = inner_vlan;
    vp->egress_vlan = outer_vlan;
...
/* Configure double vlan crosss connect */
bcm_error_t
configure_double_vlan_cross_connect(int unit, bcm_vlan_t outer_vlan, bcm_vlan_t inner_vlan,
                             bcm_vlan_port_t customer_vp,  bcm_vlan_port_t provider_vp)
{
    bcm_error_t rv = BCM_E_NONE;

    rv = bcm_vlan_cross_connect_add(unit, outer_vlan, inner_vlan,
...
bcm_error_t  configure_double_vlan_cross_connect_vp(int unit)
{
    bcm_port_t cust_port = ingress_port;
    bcm_port_t prov_port = egress_port;
    bcm_vlan_t outer_vlan = 20, inner_vlan = 10;
    bcm_error_t rv = BCM_E_NONE;
    bcm_pbmp_t pbmp, ubmp;
    ...
    rv =  bcm_vlan_control_port_set(unit, cust_port, bcmVlanPortTranslateKeySecond,
    ...
    rv =  bcm_vlan_control_port_set(unit, prov_port, bcmVlanPortTranslateKeySecond,
                                                     bcmVlanTranslateKeyPortDouble);
    if (BCM_FAILURE(rv)) {
        printf("Error executing bcm_vlan_control_port_set: %s.\n", bcm_errmsg(rv));
        return rv;
    ...
    /* VLAN Configuration */
    rv = create_vlan_add_port(unit, outer_vlan, cust_port);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring vlan : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = create_vlan_add_port(unit, outer_vlan, prov_port);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring vlan : %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = enable_double_vlan_cross_connect(unit, outer_vlan, &vlan_ctrl);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring vlan : %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = create_l2_vp(unit, outer_vlan, inner_vlan, cust_gport, customer_vp);
    if (BCM_FAILURE(rv)) {
    ...
    rv = create_l2_vp(unit, outer_vlan, inner_vlan, prov_gport, provider_vp);
    if (BCM_FAILURE(rv)) {
    ...
    rv = configure_double_vlan_cross_connect(unit, outer_vlan, inner_vlan, customer_vp, provider_vp);
...
void verify(int unit)
{
  ...
  snprintf(str, 512, "tx 1 pbm=%d data=0x101112131415000102030405810000148100000a002e00000000000000001234568a1234568b1234568c1234568d1234568e1234568f123456901234569112345692123498f05485;sleep quiet 1", ingress_port);
  bshell(unit, str);
 ...
 * a)test setup
 * b)actual configuration (Done in configure_double_vlan_cross_connect_vp())
...
bcm_error_t execute()
{
  ...
  if (BCM_FAILURE((rv = configure_double_vlan_cross_connect_vp(unit)))) {
    printf("configure_double_vlan_cross_connect_vp() failed.\n");
    return -1;
  }
  printf("Completed configuration(i.e executing configure_double_vlan_cross_connect_vp()) successfully.\n");
