 ...
 * VLAN=2,Pri=2  ge0(1)|            |ge1(2)   VLAN=2,Pri=2
 *                     | l2:mac=0x1 |
 *                     |    vlan=2  |
 *      <-------+      |    port=2  |     <-------+
 ...
 *  Summary:
 *  ========
 *  This cint example demonstrates PFC feature on HR4
 *
 ...
 *    ==========================================
 *      a) send at linerate to ge0 L2 learnt packets (vlan=1 prio=2 dmac=0x1 smac=0x2)
 *      b) Along with a), send at linerate to ge1 PFC frames with pause-control class 2 enabled
 *      c) Expected Result:
 *         ================
 *         For test case mentioned in a)
 *         On ge0 (Port-1): verify that PFC frames should be transmitted at regular intervals as
 *         egress port is congested.
 *         In 'show c' output, PFC frame Tx should be seen on ge0 (port-1)
 *
 *         On ge1 (Port-2): packets should egress at configured rate
 *
