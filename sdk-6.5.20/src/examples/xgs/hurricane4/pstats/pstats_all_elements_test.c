...
/*  Feature  : HR4 Packetized Statistic Test
 *
 *  Usage    : BCM.0> cint pstats_all_elements_test.c
 *
 *  config   : BCM56275_A1-PORT-PSTAT.bcm
 *
 ...
 *  Summary:
 *  ========
 * Hurricane4 supports a feature packetized statistics (PSTATS) which allows software
 * to read out MMU buffer use counts over SBUS and send this data via packets into the
 * Hurricane4 switch to any destination in the network in a timely manner.
 *
...
//////////////////////////////////////////////////////////////
/* HR4 COSQ related Parameters */
const int HR4_MAX_PORTS = 71;
const int HR4_NUM_CPU_Q = 48;
const int HR4_NUM_PORT_L0 = 12;
const int HR4_NUM_PORT_Q_UC = 12;
const int HR4_NUM_PORT_Q_MC = 12;

bcm_gport_t cpu_cosq_gp_l0[HR4_NUM_PORT_L0];
bcm_gport_t cpu_cosq_gp_mc[HR4_NUM_CPU_Q];

bcm_gport_t cosq_gp_l0[HR4_MAX_PORTS][HR4_NUM_PORT_L0];
bcm_gport_t cosq_gp_uc[HR4_MAX_PORTS][HR4_NUM_PORT_Q_UC];
bcm_gport_t cosq_gp_mc[HR4_MAX_PORTS][HR4_NUM_PORT_Q_MC];
...
int gport_callback(int unit, bcm_gport_t port, int numq, uint32 flags,
    ...
    else
    {
        ...
        else if (flags & BCM_COSQ_GPORT_UCAST_QUEUE_GROUP)  // 0x08
        {
            ...
            index = (index + 1) % HR4_NUM_PORT_Q_UC;
        }
        else if (flags & BCM_COSQ_GPORT_MCAST_QUEUE_GROUP)  // 0x20
        {
            ...
            index = (index + 1) % HR4_NUM_PORT_Q_MC;
        }
        else
        {
            printf("ERR: INVALID FLAG for port 0x%x(localport %d)\n",
...
int hr4_dump_gports (int port)
{
    int start = 0, end = 0, idx, g_idx, num_q = 0, n_idx, q_idx;

    if (port < -1) {
        printf ("Err. Input valid port number or -1 for all (%d) ports", HR4_MAX_PORTS);
    }

    if ((port == -1) || (port == 0)) {
    printf("\nPrinting Gport Hierarchy for CPU Port %d\n", port);
    for (n_idx = 0; n_idx < HR4_NUM_PORT_L0; n_idx++) {
            printf ("    L0.%d: 0x%x\n", n_idx, cpu_cosq_gp_l0[n_idx]);
            if (n_idx == 0) {
                for (q_idx = 0; q_idx < HR4_NUM_CPU_Q; q_idx++) {
