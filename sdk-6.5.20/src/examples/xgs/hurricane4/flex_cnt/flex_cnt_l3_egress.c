 ...
 *
 *  Usage    : BCM.0> cint flex_cnt_l3_egress.c
 *
 *  config   : BCM56275_A1-PORT.bcm
 *
 *  Log file : flex_cnt_l3_egress_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+
...
/* Add L3 Route Entry another direction */
  ...
  rv = bcm_stat_group_create(unit, bcmStatObjectEgrL3Intf, bcmStatGroupModeSingle, &l3egrif_flex_cnt_idx, &num_entries);
  if (BCM_FAILURE(rv)) {
      printf("Error executing bcm_stat_group_create(): %s.\n", bcm_errmsg(rv));
      return rv;
  }

  rv = bcm_l3_egress_stat_attach(unit, l3_egr_obj_id, l3egrif_flex_cnt_idx);
  if (BCM_FAILURE(rv)) {
...
/* Get Egr L3 intf flex cnt stats*/
bcm_error_t l3egrif_flex_stat_get(int unit, bcm_if_t intf_id)
{
  bcm_error_t rv;
  bcm_stat_value_t l3egrif_flex_cnt_val[1];
  uint32 l3egrif_flex_cnt_idx = 0;
  uint32 num_entries = 1;

  rv = bcm_l3_egress_stat_counter_sync_get(unit, intf_id, bcmL3StatOutPackets, num_entries, &l3egrif_flex_cnt_idx, l3egrif_flex_cnt_val);
  if (BCM_FAILURE(rv)) {
  ...
  printf("The value is 0x%x%x packets\n",
          COMPILER_64_HI(l3egrif_flex_cnt_val[0].packets64),
          COMPILER_64_LO(l3egrif_flex_cnt_val[0].packets64));


  rv = bcm_l3_egress_stat_counter_sync_get(unit, intf_id, bcmL3StatOutBytes, num_entries, &l3egrif_flex_cnt_idx, l3egrif_flex_cnt_val);
  if (BCM_FAILURE(rv)) {
      printf("Error executing bcm_l3_egress_stat_counter_sync_get(): %s.\n", bcm_errmsg(rv));
      return rv;
  }
  printf("The value is 0x%x%x bytes\n",
          COMPILER_64_HI(l3egrif_flex_cnt_val[0].bytes),
          COMPILER_64_LO(l3egrif_flex_cnt_val[0].bytes));
  return BCM_E_NONE;
}
 ...
 */
  ...
  printf("Sending 3 packets that hit the Egr L3 NextHop.\n");
  ...
  printf("\nDumping Egr L3 NextHop counters after sending 3 packets..\n");
 ...
 */
  ...
  printf("Sending another 2 packets that hit the same Egr L3 NextHop.\n");
  ...
  /*Retrieve Egr L3 NextHop flex counters and print them.*/
  printf("\nDumping Egr L3 NextHop counters after sending another 2 packets..\n");
  l3egrif_flex_stat_get(unit, intf_id);
}
