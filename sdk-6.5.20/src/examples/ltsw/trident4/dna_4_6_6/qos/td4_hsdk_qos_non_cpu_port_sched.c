 ...
 */
/*  Feature  : Non-CPU Port Scheduler Hierarchy 
 *
 *  Usage    : BCM.0> cint td4_hsdk_qos_non_cpu_port_sched.c 
 *
 *  config   : bcm56880_a0-generic-32x400.config.yml
 *
 *  Log file : td4_hsdk_qos_non_cpu_port_sched_log.txt
 *
 *  Test Topology : None  
 *
 *  Summary:
 *  ========
 *  This cint example demonstrate creating non-cpu port custom scheduler hierarchy
 *
 ...
 *    ======================================================
 *      a) Create scheduler profile with queue to COS(L0) mapping and
 *         and attach created scheduler profile to port.
 *    3) Step3 - Verification (Done in verify())
 *    ==========================================
 *      a) Verify the profile attachment to port using 'dump sw cosq'
 *      b) Expected Result:
 *         ================
 *         In 'dump sw cosq' output for the selected port
 *             - UCQ0-7 should be mapped to COS0-7
 *             - MCQ0-3 should be mapped to COS0-3
 *             - Other ports should have default queue to COS mapping
 *                 - UCQ0-7 mapped to COS0-COS7
 *                 - MCQ0-3 mapped to COS8-COS11  
 */
...
bcm_error_t attach_sched_profile_to_port(int unit, int port, int profile_id) 
{
  ...
  /* Disable MMU traffic and Drain before attaching new scheduler profile to port dynamically*/
  /* This control returns failure when port link is down. Hence check return status only if port is up */
  rv = bcm_port_control_set(0, port, bcmPortControlMmuTrafficEnable, FALSE);
  if (rv != BCM_E_NONE) {
      printf("attach_sched_profile_to_port: MmuTraffic Disable Error :%s.\n", bcm_errmsg(rv));
  }
  ...
  /* Enable MMU traffic on the port once profile attach is successful */
  /* This control returns failure when port link is down. Hence check return status only if port is up */
  rv = bcm_port_control_set(0, port, bcmPortControlMmuTrafficEnable, TRUE);
  if (rv != BCM_E_NONE) {
      printf("attach_sched_profile_to_port: MmuTraffic Enable Error :%s.\n", bcm_errmsg(rv));
  }
  return BCM_E_NONE;
}
