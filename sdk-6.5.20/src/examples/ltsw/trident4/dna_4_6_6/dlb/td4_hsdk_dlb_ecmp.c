...
/*  Feature  : DLB configuration for ECMP
 *
 *  config   : td4_hsdk_dlb.config.yml
 *             (Usage: rack#./bcm.user -y ./td4_hsdk_dlb.config.yml)
 *
 *  Usage    : BCM.0> cint td4_hsdk_dlb_ecmp.c
 *
 *  Log file : td4_hsdk_dlb_ecmp_log.txt
 *
 *  Test Topology :
 *      +-----------------+            +---------+
 *      |         ing-port|<-----------+         |
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of the DLB
 ...
 *        egress_ports.
 *
 *   2) Step2 - Configuration (Done in config_dlb_ecmp()):
 *   =========================================================
 *     a) Configure a basic port, vlan, flexdigest and ECMP with DLB
...
configure_HashOutSelection(int unit, bcm_port_t port)
{
    ...
    /* ECMP-DLB hashout selection */
    BCM_IF_ERROR_RETURN(bcm_switch_control_port_set(unit, port, bcmSwitchEcmpDynamicHashOffset, 48));
    return BCM_E_NONE;
}
...
bcm_error_t
configure_DLB_ECMP_setup(int unit, int port_count, bcm_port_t *port_arr, bcm_vlan_t vid, bcm_vrf_t vrf)
{
    int i;
    ...
    if (dlb_valid) {
        ecmp_grp.dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_NORMAL;
    ...
    printf("DLB ECMP Group Members status set:\n");
    ...
    if (dlb_valid) {
        for (i = 0; i < port_count; i++) {
            /* Set DLB port quality attributes. */
            bcm_l3_ecmp_dlb_port_quality_attr_t quality_attr;
            bcm_l3_ecmp_dlb_port_quality_attr_t_init(&quality_attr);
            quality_attr.scaling_factor = 400;
            quality_attr.load_weight = 50;
            quality_attr.queue_size_weight = 40;
            BCM_IF_ERROR_RETURN(bcm_l3_ecmp_dlb_port_quality_attr_set(unit, port_arr[i], &quality_attr));
        }
    }
...
config_dlb_ecmp(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(configure_DLB_ECMP_setup(unit, port_count, port_arr, vid, vrf));
...
execute(void)
{
    ...
    print "~~~ #2) config_dlb_ecmp(): ** start **";
    if (BCM_FAILURE((rv = config_dlb_ecmp(unit)))) {
        printf("config_dlb_ecmp() failed.\n");
        return -1;
    }
    print "~~~ #2) config_dlb_ecmp(): ** end **";
