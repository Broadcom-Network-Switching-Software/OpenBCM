 ...
 */
/*
 * Feature  : L2 switching for double tagged packets by configuring VFI
 *            using VFP entry match
 * Usage    : BCM.0> cint td4_hsdk_vlan_vfi_from_vfp.c
 *
 * config   : bcm56880_a0-generic-vlan.config.yml
 *
 * Log file : td4_hsdk_vlan_vfi_from_vfp_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 forwarding for ingress double tagged
 * packets on switch using VFI configuration assigned in VFP
 *
 ...
 *   [bcmPWN.0]Packet[1]: data[0010]: 9100 000a 0806 0001 0800 0604 0001 0000
 ...
 *   [bcmPWN.0]Packet[2]: data[0010]: 9100 000a 0806 0001 0800 0604 0001 0000
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000011000000000002810000149100000A080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 1", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
...
bcm_error_t
vfp_entry_setup(int unit,bcm_gport_t port,bcm_vlan_t outer_vlan,
                            bcm_vlan_t inner_vlan,bcm_vlan_t fwd_vfi)
{
    bcm_error_t rv = BCM_E_NONE;
    ...
    BCM_FIELD_ASET_ADD(group_config.aset, bcmFieldActionVpnSet);
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualify_InPort(unit,entry,port,BCM_FIELD_EXACT_MATCH_MASK));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_OuterVlanId(unit,entry,outer_vlan,0xff));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_InnerVlanId(unit,entry,inner_vlan,0xff));
    BCM_IF_ERROR_RETURN(bcm_field_action_add(unit, entry, bcmFieldActionVpnSet,fwd_vfi,0));
...
{
    bcm_vlan_t vlanId=20, default_vlan=20, pkt_outer_vlan=20, pkt_inner_vlan=10;
    bcm_error_t rv = BCM_E_NONE;
    ...
    /*configure vfp for lookup*/
    rv = vfp_entry_setup(unit,ingress_port,pkt_outer_vlan,pkt_inner_vlan,default_vlan);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vfp_entry_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }
