...
/*
 * Feature  : L3 IPv6 route/LPM/ALPM entry
 *
 * Usage    : BCM.0> cint td4_hsdk_l3_ipuc_ipv6_route.c 
 *
 * config   : bcm56880_a0-generic-32x400.config.yml 
 *
 * Log file : td4_hsdk_l3_ipuc_ipv6_route_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of the IPv6 route entry using
 *   BCM APIs.
 *
 ...
 *        'l3 egress show' and 'l3 route show v6=1'
 *
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see that dmac, smac, Hop Limit and vlan are all changed as the packet is
...
int
create_def_egr_obj(int unit, bcm_if_t l3_if, bcm_if_t *egr_if)
{
    bcm_error_t     rv = BCM_E_NONE;
...
configure_ipuc_forwarding(int unit)
{
    ...
    bcm_ip6_t       dip6_mask = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    bcm_ip6_t       def_dip6 = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    bcm_ip6_t       def_dip6_mask = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    ...
    rv = create_def_egr_obj(unit, egr_l3_if, def_egr_if);
    if (BCM_FAILURE(rv)) {
    ...
    rv = configure_route(unit, def_dip6, def_dip6_mask, def_egr_if, vrf);
    if (BCM_FAILURE(rv)) {
    ...
    rv = configure_route(unit, dip6, dip6_mask, egr_if, vrf);
    if (BCM_FAILURE(rv)) {
        printf("configure_route return error : %s.\n", bcm_errmsg(rv));
        return rv;
    }
