...
/*  Feature  : L3 Ipv6 in Ipv6 Tunnel
 *
 *  Usage    : BCM.0> cint cint td4_hsdk_l3_ip_tunnel_ip6in6.c  
 *
 *  config   : bcm56880_a0-generic-32x400.config.yml   
 *
 *  Log file : td4_hsdk_l3_ip_tunnel_ip6in6_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+
 ...
 *  Summary:
 *  ========
 *    This cint example demonstrates L3 Ipv6 in Ipv6 Tunnel initiation and termination
 *    using BCM APIs.
 *
 ...
 *
 *    2) Step2 - Configuration (Done in  config_l3_tunnel_ip6in6())
 *    =============================================================
...
/*
 * Configure Ipv6 in Ipv6 Tunnel initiation and termination.
 */
bcm_error_t config_l3_tunnel_ip6in6(int unit)
{
  bcm_error_t       rv;
  ...
  bcm_ip6_t tnl_term_dip6 = {0xAA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};/*AA00:0:0:0:0:0:0:1*/
  bcm_ip6_t tnl_init_dip6 = {0xBB, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  tnl_init_info.type = bcmTunnelTypeIp6In6; /* Choose Ipv6-in-Ipv6 tunnel type */
  ...
  tnl_term_info.type      = bcmTunnelTypeIp6In6; /* Choose Ipv6-in-Ipv6 tunnel type */
...
void verify(int unit)
{
 ...
 /* 
  *  For IPv6 tunnel initiation, send the below IPv6 packet on access_port. 
  *
  *    DA 0x1
  *    SA 0x1111
  *    VLAN 21
  ...
  */
  ...
  snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000010000000011118100001586DD6030000000023BFFAA000000000000000000000000000001BB00000000000000000000000000000100010CB1F608; sleep quiet 1", access_port);
  bshell(unit, str);
  bshell(unit, "show c");
  ...
  */
  ...
  snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000020000000022228100001686dd60300000002e290acc000000000000000000000000000001dd0000000000000000000000000000016030000000063bfebb000000000000000000000000000001aa000000000000000000000000000001000000000000114c3f2600000000; sleep quiet 1", network_port);
  bshell(unit, str);
  bshell(unit, "show c");
}
 ...
 * a)test setup
 * b)actual configuration (Done in config_l3_tunnel_ip6in6())
...
bcm_error_t execute()
{
  ...
  if (BCM_FAILURE((rv = config_l3_tunnel_ip6in6(unit)))) {
    printf("config_l3_tunnel_ip6in6() failed.\n");
    return -1;
  }
  printf("Completed configuration(i.e executing config_l3_tunnel_ip6in6()) successfully.\n");
