...
/*
 * Feature  : Vxlan Riot, VNID mismatched error packet processing in Vxlan termination flow
 *
 * Usage    : BCM.0> cint TD4_HSDK_ipv4_vxlan_riot_NtoA_uc_VNID_mismatch.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_ipv4_vxlan_riot_NtoA_uc_VNID_mismatch_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * Summary:
 * 
 * Cint example to verify the processing of Vxlan tunnel terminated packets with error VNID -
 *             - drop event monitor is added to copy VNID error packets to CPU
 *
 ...
 *     c) Expected Result:
 *     
 *       We can see the Vxlan packet with error VNID is trapped to cpu and drop.
...
Header Info
  ...
  ###[ VXLAN ]###
     flags= Instance
     reserved1= 0
     vni= 0x12348           -- error VNID, matched VNID is 0x12345
     reserved2= 0x0
  ###[ Ethernet ]###
...
Ingress Hex Packet:
tx port=2 data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d88080000000123480000000000040100000000020108004500002E000040000A06E482282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A
...
[bcmPWN.0]Packet[1]: data[0030]: 0000 0123 4800 0000 0000 0401 0000 0000
...
verify(int unit)
{
	...
	snprintf(str, 512, "tx 1 pbm=%d data=0x0000000003010000000001018100001E080045000068000040000A113466141414010A0A0A01d23412b500547d88080000000123480000000000040100000000020108004500002E000040000A06E482282828011E1E1E0181815151B1B2B3B4A1A2A3A4500F01F53A9F00001112131415161718191A; sleep quiet 3", ingress_port);
	bshell(unit, str);
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
    bshell(unit, "l2 show");
