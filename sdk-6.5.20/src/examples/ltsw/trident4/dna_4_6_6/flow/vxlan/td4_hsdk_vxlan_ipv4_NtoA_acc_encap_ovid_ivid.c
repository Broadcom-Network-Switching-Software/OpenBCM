 ...
 *
 * Usage    : BCM.0> cint TD4_HSDK_VxLAN_ipv4_NtoA_acc_encap_ovid_ivid.c 
 *
 * config   : bcm56880_a0-generic-32x400_vxlan.config.yml
 *
 * Log file : TD4_HSDK_VxLAN_ipv4_NtoA_acc_encap_ovid_ivid_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *             - egress vlan translation is used to add ovid and ivid
 *
 ...
 *     b) egress vlan translation is used to add outer and inner vlan tag after vxlan tunnel decapsulation
 *
 ...
 *     c) Expected Result:
 *     ===================
 *        The ingress vxlan packet is decapsulated with outer and inner vlan tag inserted.
 *        We can see this packet on the egress port. The contents of the egress packet are
 *        printed on screen.
...
Input Hex (network):
======


Output (access):
=======
data[0000]: {00000000bbbb} {00000000aaaa} 8100 20c8
data[0010]: 9100 0064 0800 4500 0046 0001 0000 4000 
...
struct vxlan_acc_cfg_t {
    ...
    bcm_gport_t   acc_gport; /* Output: access flow port */
};
...
struct vxlan_net_cfg_t {
    ...
    bcm_ip_t      local_ip;
    uint16 udp_sp;
    bcm_gport_t   nw_gport; /* Output: network flow port */
};
 ...
 *   080045000078000100004011A4C0C0A8
 *   01010A0A0A01003512B5006441310800
 *   00000123450000000000BBBB00000000
 *   AAAA08004500004600010000400074B2
 *   02020202010101015468697320706163
 *   6B6574206973206265696E672073656E
 *   74206279207465737420312066726F6D
 *   20434C4920746F20424D
 *
 *   Out Packet:
 *   -----------
 *   0000 0000 bbbb 0000 0000 aaaa 8100 20c8    
 *   9100 0064 0800 4500 0046 0001 0000 4000
...
void verify(int unit)
{
    ...
    snprintf(str, 1184, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port);     
    bshell(unit, str);
    bshell(unit, "show c");
    bshell(unit, "l2 show");
...
int vxlan_access_port_config(int unit, bcm_gport_t gport_acc)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_vlan_control_port_set(unit, gport_acc, bcmVlanTranslateEgressEnable, 1));
   return BCM_E_NONE;
}
...
/* Access Vxlan flow port creation */
int config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port, bcm_flow_handle_t flow_handle)
{
    ...
    /* 
     * SD_TAG operations implemented by egress vlan translation 
     * outer tag:bcmVlanActionAdd
     * inner tag:bcmVlanActionAdd
     */
    printf("Enable egress vlan action in egr_vlan_translation\n");    

    int egress_port_class = 200;
    rv = bcm_port_class_set(unit, phy_port, bcmPortClassEgress, egress_port_class);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_port_class_set: %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Adding inner and outer tag */
    ...
    action.priority = 1;
    action.inner_tag = bcmVlanActionAdd;
    action.new_inner_vlan = 100;
    rv = bcm_vlan_translate_egress_action_add(unit, egress_port_class, vxlan_cfg->vxlan_vpn, BCM_VLAN_INVALID, &action);
    if (BCM_FAILURE(rv)) {
...
/* Tunnel creation and associating it with network flow port */
...
/* vxlan network flow port creation. tunnel is not created */
int config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, bcm_flow_handle_t flow_handle)
{
    ...
    /* Enable ingress vlan action on network flow port */
