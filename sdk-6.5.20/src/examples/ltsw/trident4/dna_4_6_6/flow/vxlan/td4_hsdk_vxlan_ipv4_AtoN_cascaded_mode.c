...
/*
 * Feature  : TD4(56880) Vxlan Bridging, access to network IPv4 tunnel initiation using
 *            cascaded mode.
 *
 *
 * Usage    : BCM.0> cint TD4_HSDK_VxLAN_ipv4_AtoN_cascaded_mode.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_VxLAN_ipv4_AtoN_cascaded_mode_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 *   using cascaded egress object for tunnel encapsulation.
 *
 *   In TD4 DNA, L2 forwarding table provides both DVP and underlay next-hop.
 *   In some usecases, cascaded mode is preferred to save convergence time
 *   for any change in association between DVP and underlay next-hop.
 *   In cascaded mode, L2 forwarding table provides an index to ECMP member0
 *   table which inturn provides DVP and underlay next-hop.
 *
 ...
 *
 *     c) Perform network-side configuration.Use "BCM_FLOW_PORT_ENCAP_FLAG_EGRESS_CASCADED"
