...
/*
 * Feature  : TD4(56880) Vxlan Bridging, network-to-access IPv4 tunnel termination
 *
 * Usage    : BCM.0> cint td4_hsdk_vxlan_ipv4_NtoA_assign_access_VP_and_VPN_match_port_ovid.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_vxlan_ipv4_NtoA_assign_access_VP_and_VPN_match_port_ovid_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * Summary:
 * ========
 *   Cint example to demonstrate VxLAN Bridging configuration for VXLAN tunnel termination
 *   and adding entry into L2 table pointing to access Virtual port.
 *
 *   On Trident4(DNA: 4_5_x), on the access-side source virtual port and VFI
 ...
 *
 *     d) Add an entry into L2 table pointing to access Virtual port.
 *
 ...
 *     c) Expected Result:
 *     ===================
 *       We can see the decapsulated packet on the egress port.
 */
