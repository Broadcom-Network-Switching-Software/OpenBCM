 ...
 */
/*
 * Feature  : L2 outer and inner tpid configurations on port
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_inner_tpid_setup.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_inner_tpid_setup_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 outer and inner tpid setting on port and forwarding
 * ingress tagged packets on switch using VFI configuration based on port+outer vlan
 *
 ...
 *   [bcmPWN.0]Packet[1]: data[0010]: 88a8 000a 0806 0001 0800 0604 0001 0000
 ...
 *   [bcmPWN.0]Packet[2]: data[0010]: 88a8 000a 0806 0001 0800 0604 0001 0000
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000110000000000029100001488A8000A080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 1", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
...
setup_l2_forwarding(int unit)
{
    ...
    /*setup per port tpid*/
    rv = port_tpid_setup(unit,ingress_port,0x9100,0x88A8);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_tpid_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = port_tpid_setup(unit,egress_port,0x9100,0x88A8);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_tpid_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }
