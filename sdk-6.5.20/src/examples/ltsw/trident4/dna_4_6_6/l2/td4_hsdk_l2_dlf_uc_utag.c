 ...
 */
/*
 * Feature  : VLAN/VFI flooding for untag unicast DLF packets on TD4
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_dlf_uc_utag.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_dlf_uc_utag_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate flooding the untagged unicast dlf packet on its vlan/vfi member ports.
 *
...
verify(int unit)
{
    ...
    bshell(unit, "clear c");
    printf("\nSending untagged unicast packet to ingress port:%d\n",ingress_port);
