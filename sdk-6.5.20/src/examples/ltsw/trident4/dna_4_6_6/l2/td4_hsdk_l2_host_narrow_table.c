 ...
 */
/*
 * Feature  : L2 switching for outer tagged packets using L2 host narrow table lookup
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_host_narrow_table.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_host_narrow_table_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 forwarding by L2 host narrow table lookup for outer tagged
 * packets on switch using identity mapped VFI configuration
 *
...
verify(int unit)
{
    ...
    printf("\nverify the l2 entries learned in narrow view, not in wide view\n\n");
    bshell(unit, "bsh -c 'lt l2_host_table traverse -l'");
    bshell(unit, "bsh -c 'lt l2_host_narrow_table traverse -l'");
...
setup_l2_forwarding(int unit)
{
    ...
    /* L2 host table wide view(default) and narrow view per VLAN */
    printf("Enabling the L2 host table narrow view for vlan %d\n", vlanId);
    bcm_vlan_control_vlan_t vlan_ctrl;
    bcm_vlan_control_vlan_t_init(&vlan_ctrl);
    vlan_ctrl.l2_view = bcmVlanL2ViewNarrow;
    rv = bcm_vlan_control_vlan_selective_set(unit, vlanId, BCM_VLAN_CONTROL_VLAN_L2_VIEW_MASK, &vlan_ctrl);
    if (BCM_FAILURE(rv)) {
