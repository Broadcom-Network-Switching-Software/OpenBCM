 ...
 */
/*
 * Feature  : L2 tpid configurations on port
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_outer_tpid_setup.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_outer_tpid_setup_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 outer tpid setting on port and forwarding
 ...
 *
 *   Ingress Packet:
 *   [bcmPWN.0]Packet[1]: data[0000]: {000000000011} {000000000002} 9100 000a
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000001100000000000291000014080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 1", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
...
setup_l2_forwarding(int unit)
{
    ...
    /*setup per port tpid*/
    rv = port_tpid_setup(unit,ingress_port,0x9100,0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_tpid_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = port_tpid_setup(unit,egress_port,0x9100,0);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_tpid_setup: %s\n", bcm_errmsg(rv));
        return rv;
    }
