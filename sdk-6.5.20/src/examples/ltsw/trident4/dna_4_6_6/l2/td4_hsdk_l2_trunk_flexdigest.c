 ...
 */
/*
 * Feature  : L2 switching on trunk group based on flex digest hashing
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_trunk_flexdigest.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_trunk_flexdigest_log.txt
 *
 * Test Topology :
 ...
 * Summary:
 * ========
 * cint script to demonstrate L2 forwarding on trunk ports.
 *
...
setup_l2_forwarding(int unit)
{
    ...
    rv = add_l2_static_entry(unit, BCM_L2_TRUNK_MEMBER, dst_mac, vlanId, trunk_id);
    if (BCM_FAILURE(rv)) {
