 ...
 */
/*
 * Feature  : Protocol packets handling in TD4
 *
 * Usage    : BCM.0> cint td4_hsdk_l2_protocol.c
 *
 * config   : bcm56880_a0-generic-l2.config.yml
 *
 * Log file : td4_hsdk_l2_protocol_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate copying protocol packets like arp,icmp,igmp to cpu.
 *
 ...
 *
 * 2. ICMP echo request packet
 *    0000 0000 0d00 0000 0000 0c00 8100 0014
 *    0800 4500 002e 0000 0000 4001 5cbc 1400
 *    000a 0a00 000a 0800 afae 0000 0000 0001
 *    0203 0405 0607 0809 0a0b 0c0d 0e0f 1011
 *    ac32 e22b
 *
 *   Egress Packet:
 *   Same packets will be copied to cpu port with PROTOCOL_PKT as a reason code.
 */
...
verify(int unit)
{
    ...
    bshell(unit, "l2 show");
    bshell(unit, "clear c");
    printf("\nSending ARP request packet to ingress port:%d\n",ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000011000000000002080600010800060400010000000000020A0A0A0200000000000014141402000102030405060708090a0b0c0dbc94977f; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
    bshell(unit, "l2 show");
    ...
    bshell(unit, "clear c");
    printf("\nSending ICMP echo request packet to ingress port:%d\n",ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000D00000000000C0008004500002E0000000040015CBC1400000A0A00000A0800AFAE00000000000102030405060708090A0B0C0D0E0F1011AC32E22B; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
    printf("\nverify l2 learning:\n");
    bshell(unit, "l2 show");
...
setup_l2_forwarding(int unit)
{
    ...
    /*"protocol_pkt_forward_table" is a tcam based table so all the fields programmed in the entry
     must match all the packets fields to hit the entry and the action applied(drop/copytocpu).
     In this example we are programming two separate entries for ARP and ICMP packets to copy to cpu.
     though the API bcm_switch_pkt_protocol_control_add() will not return any error if we program
     both ARP and ICMP fields in one entry, but this entry will never hit for the ingress packet of type APR or ICMP.
     So it is user responsibility to program separate entries based on ingress packet type to hit programmed values.
    */

    printf("Configuring arp request packet to cpu \n");
    ...
    printf("Configuring icmp echo request packet to cpu \n");
    bcm_switch_pkt_control_action_t_init(&action);
    bcm_switch_pkt_protocol_match_t_init(&match);

    action.copy_to_cpu = 1;
    match.icmp_type = 8;
    match.icmp_type_mask = 0xf;
