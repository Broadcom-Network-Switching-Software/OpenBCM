 ...
 *            & outer/inner VLAN ID as match criteria
 *            (BCM_MPLS_PORT_MATCH_PORT_GROUP_VLAN_STACKED)
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpls_AtoN_MatchPortGroupVlanStacked.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_mpls_vpls_AtoN_MatchPortGroupVlanStacked_log.txt
 *
 * Test Topology :
 *
 *                    +-------------------+
 ...
 *   group & outer/inner VLAN as match criteria.
 *
 ...
 *
 *      b) Transmit packets with VLAN-11/VLAN-31 and VLAN-11 on ing_acc_port_1.
 *         The contents of the packet are printed on screen.
 *
 *      c) Expected Result:
 *      ===================
 *         We can see that customer packet with VLAN-11/VLAN-31 egress as VPLS
 *         tunnel packet on network port and packet with VLAN-11 dropped in
 *         ingress access port.
...
setup_vpls(int unit)
{
	...
	l2vpn_acc_cfg_t l2vpn_acc_1 = {
        ...
        BCM_MPLS_PORT_MATCH_PORT_GROUP_VLAN_STACKED, /* Access match criteria */
