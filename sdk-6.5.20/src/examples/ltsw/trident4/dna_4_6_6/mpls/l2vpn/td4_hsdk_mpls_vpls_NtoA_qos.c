...
/*
 * Feature  : TD4(56880) VPLS (L2 MPLS) QOS mapping in tunnel termination flow
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpls_NtoA_qos.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_mpls_vpls_NtoA_qos_log.txt
 *
 * Test Topology :
 *
 *                    +-------------------+
 ...
 *   egr_acc_port_1   |                   |
 *   <----------------+                   |
 *   Egress packet    |                   |     VLAN-22
 *                    |                   |  ing_net_port_3
 *                    |     Trident-4     +<----------------
 *      VLAN-12       |                   |  Ingress with MPLS tunnel header
 *   egr_acc_port_2   |                   |
 *   <----------------+                   |
 *   Egress packet    |                   |
 *                    |                   |
 ...
 *         three ports, one port is used as ingress port and the others as
 *         egress ports.
 *
 ...
 *      c) Expected Result:
 *      ===================
 *         We can see that customer (payload) packet out on access port 1 &
 *         access port 2 with .1P priority.
 *
 *   Ingress Packet : ing_net_port_3
 *   0000 0000 00bb 0000 0000 00aa 8100 0016
 *   8847 0022 2810 0033 3110 0000 00ff 0000
 ...
 *   0000 0000 0011 0000 0000 0022 8100 800b
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000bb0000000000aa8100001688470022281000333110000000ff0000000000110000000000228100000b0800450000360000000040ff65ac0a0000140a00000a000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f2021239412f0; sleep quiet 3", ing_net_port_3);
    bshell(unit, str);
...
/* Qos configuration */
bcm_error_t
config_l2vpn_qos (int unit, int *ing_qos, int *egr_qos)
{
    ...
    *ing_qos = ing_qos_id;
    ...
    rv = bcm_qos_map_create(unit, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_L2,
                            &egr_qos_id);
    if (BCM_FAILURE(rv)) {
    ...
    *egr_qos = egr_qos_id;
    ...
    for (i = 0; i < 8; i++) {
        ...
        egr_qos_map.pkt_pri = pkt_pri[i];
        rv = bcm_qos_map_add(unit, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_L2,
...
config_l2vpn_access (int unit, l2vpn_cfg_t *l2vpn_cfg,
                     l2vpn_acc_cfg_t *l2vpn_acc_cfg, int egr_qos_id)
{
    ...
    rv = bcm_qos_port_map_set(unit, l2vpn_acc_cfg->acc_gport, -1, egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("bcm_qos_port_map_set return error : %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
mpls_tunnel_terminator(int unit, uint32 label, uint32 flags,
                       bcm_mpls_switch_action_t action, int qos_id)
{
...
config_l2vpn_network (int unit, l2vpn_cfg_t *l2vpn_cfg,
                      l2vpn_net_cfg_t *l2vpn_net_cfg, int ing_qos_id)
{
    bcm_vlan_action_set_t  vlan_action;
    ...
    rv = mpls_tunnel_terminator(unit, l2vpn_net_cfg->tunnel_label_term,
                                ...
                                BCM_MPLS_SWITCH_ACTION_POP, ing_qos_id);
    if (BCM_FAILURE(rv)) {
...
setup_vpls(int unit)
{
    ...
    /* QOS remarking */
    rv = config_l2vpn_qos (unit, &ing_qos_id, &egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("config_l3_mpls_qos return error : %s.\n", bcm_errmsg(rv));
        return rv;
    }
	...
	rv = config_l2vpn_access(unit, &l2vpn_cfg, &l2vpn_acc_1, egr_qos_id);
	if (BCM_FAILURE(rv)) {
	...
	rv = config_l2vpn_access(unit, &l2vpn_cfg, &l2vpn_acc_2, egr_qos_id);
	if (BCM_FAILURE(rv)) {
	...
	rv = config_l2vpn_network(unit, &l2vpn_cfg, &l2vpn_net_1, ing_qos_id);
	if (BCM_FAILURE(rv)) {
