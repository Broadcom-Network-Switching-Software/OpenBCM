...
/*
 * Feature  : TD4(56880) VPLS (L2 MPLS) tunnel initiation flow with port as
 *            match criteria
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpls_AtoN_MatchPort.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_mpls_vpls_AtoN_MatchPort_log.txt
 *
 * Test Topology :
 *
 *                    +-------------------+
 ...
 * Summary:
 * ========
 *   Cint script to demonstrate VPLS (L2 MPLS) tunnel initiation flow with port
 *   as match criteria.
 *
 ...
 *
 *      b) Transmit the customer packet on ing_acc_port_1.
 *         The contents of the packet are printed on screen.
 *
 *      c) Expected Result:
 *      ===================
 *         We can see that VPLS tunnel packet out on network port 1 & 2
 *         and customer packet out on access port 2.
 *         'show c' & bcm_stat_get used to check the Tx/Rx packet
 *         stats/counters.
 *
 *   Ingress Packet : ing_acc_port_1
 *   0000 0000 0022 0000 0000 0011 8100 0006
 ...
 *   0000 0000 0011 8100 0006 0800 4500 0036
 ...
 *   8847 0055 5016 0088 8116 0000 0000 0022
 *   0000 0000 0011 8100 0006 0800 4500 0036
 *   0000 0000 40ff 65ac 0a00 0014 0a00 000a
 *   0001 0203 0405 0607 0809 0a0b 0c0d 0e0f
 *   1011 1213 1415 1617 1819 1a1b 1c1d 1e1f
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000022000000000011810000060800450000360000000040FF65AC0A0000140A00000A000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F2021239412F0; sleep quiet 3", ing_acc_port_1);
    bshell(unit, str);
