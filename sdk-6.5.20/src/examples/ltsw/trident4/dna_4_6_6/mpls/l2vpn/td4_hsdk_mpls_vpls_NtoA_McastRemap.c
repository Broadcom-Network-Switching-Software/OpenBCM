...
/*
 * Feature  : TD4(56880) VPLS (L2 MPLS) tunnel termination flow with
 *            multicast REMAP.
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpls_NtoA_McastRemap.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_mpls_vpls_NtoA_McastRemap_log.txt
 *
 * Test Topology :
 *
 *                    +-------------------+
 *                    |                   |
 *      VLAN-11       |                   |     VLAN-22
 *   egr_acc_port_1   |                   |  ing_net_port_3
 *   <----------------+                   +<----------------
 *   Egress packet    |                   |  Ingress with MPLS tunnel header
 *                    |                   |
 *                    |     Trident-4     |
 *      VLAN-12       |                   |     VLAN-23
 ...
 *
 *   Unicast entry:
 *                L2 add payload-dest-mac => Access VPort 1 (egr_acc_port_1)
 *
 *   UUC/UMC/BC multicast Group :
 *                Access port 1  - (egr_acc_port_1)
 *                Access port 2  - (egr_acc_port_2)
 *                Network port 1 - (ing_net_port_3)
 ...
 * Summary:
 * ========
 *   Cint script to demonstrate multicast REMAP in VPLS (L2 MPLS) tunnel
 *   termination flow with LABEL as match criteria.
 *
 ...
 *
 *      b) Transmit the MPLS packet on ing_net_port_3.
 *         The contents of the packet are printed on screen.
 *
 *      c) Expected Result:
 *      ===================
 *         Unicast packet :
 *         We can see that customer (payload) packet out on access port 1 as
 *         per L2 entry.
 *
 *         Non-unicast packet :
 *         We can see that VPLS tunnel packet out on network 2 and customer
 *         (payload) packet out on access port 2.
 *         Remapping multicast group prune out the access port 1.
 *
 *         'show c' & bcm_stat_get used to check the Tx/Rx packet
 *         stats/counters.
 *
 *   Ingress Unicast Packet : ing_net_port_3
 *   0000 0000 00bb 0000 0000 00aa 8100 0016
 *   8847 0022 2010 0033 3110 0000 00ff 0000
 *   0000 0011 0000 0000 0022 8100 000b 0800
 ...
 *
 *   Egress Packet : egr_acc_port_1
 *   0000 0000 0011 0000 0000 0022 8100 000b
 ...
 *
 *   Ingress Non-Unicast Packet : ing_net_port_3
 *   0000 0000 00bb 0000 0000 00aa 8100 0016
 *   8847 0022 2010 0033 3110 0000 00ff 0000
 *   0000 0055 0000 0000 0022 8100 000b 0800
 *   4500 0036 0000 0000 40ff 65ac 0a00 0014
 *   0a00 000a 0001 0203 0405 0607 0809 0a0b
 *   0c0d 0e0f 1011 1213 1415 1617 1819 1a1b
 *   1c1d 1e1f 2021 2394 12f0
 *
 *   Egress Packet : egr_acc_port_2
 *   0000 0000 0055 0000 0000 0022 8100 000b
 *   0800 4500 0036 0000 0000 40ff 65ac 0a00
 *   0014 0a00 000a 0001 0203 0405 0607 0809
 *   0a0b 0c0d 0e0f 1011 1213 1415 1617 1819
 *   1a1b 1c1d 1e1f 2021 2394 12f0
 *
 *   Egress Packet : egr_net_port_4
 *   0000 0000 00cc 0000 0000 00dd 8100 0017
 *   8847 0055 5016 0088 8116 0000 0000 0055
 *   0000 0000 0022 8100 000b 0800 4500 0036
...
test_setup(int unit)
{
    ...
    egr_acc_port_1 = port_list[0];
    egr_acc_port_2 = port_list[1];
    ing_net_port_3 = port_list[2];
    ...
    if (BCM_E_NONE != bcm_port_gport_get(unit, ing_net_port_3,
                                         &(ing_net_gport_3))){
        printf("bcm_port_gport_get() failed to get gport for port %d\n",
                                         egr_net_port_3);
        return -1;
    ...
    bcm_pbmp_t  pbmp;
    BCM_PBMP_CLEAR(pbmp);
    BCM_PBMP_PORT_ADD(pbmp, ing_net_port_3);
...
verify(int unit)
{
    ...
    printf("\nSending MPLS packet (unicast) to ingress port:%d\n", ing_net_port_3);
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000bb0000000000aa8100001688470022201000333110000000ff0000000000110000000000228100000b0800450000360000000040ff65ac0a0000140a00000a000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f2021239412f0; sleep quiet 3", ing_net_port_3);
    bshell(unit, str);

    printf("\nSending MPLS packet (non-unicast) to ingress port:%d\n", ing_net_port_3);
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000bb0000000000aa8100001688470022201000333110000000ff0000000000550000000000228100000b0800450000360000000040ff65ac0a0000140a00000a000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f2021239412f0; sleep quiet 3", ing_net_port_3);
    bshell(unit, str);
    ...
    BCM_IF_ERROR_RETURN(bcm_stat_get(unit, ing_net_port_3,
...
config_l2vpn_access (int unit, l2vpn_cfg_t *l2vpn_cfg,
                     l2vpn_acc_cfg_t *l2vpn_acc_cfg, int remap)
{
    ...
    if (remap)
    {
        /* Add the multicast encap id to the remap multicast group */
        rv = bcm_multicast_egress_add(unit, l2vpn_cfg->remap_mc_group,
                                      l2vpn_acc_cfg->acc_gport, BCM_IF_INVALID);
        if (BCM_FAILURE(rv)) {
            printf("\nError in remap bcm_multicast_egress_add : %s.\n",
                                                               bcm_errmsg(rv));
            return rv;
        }
    }
...
config_l2vpn_network (int unit, l2vpn_cfg_t *l2vpn_cfg,
                      l2vpn_net_cfg_t *l2vpn_net_cfg, int remap)
{
    bcm_vlan_action_set_t  vlan_action;
    ...
    /* Remap the multicast group */
    net_mpls_port.flags2 =  BCM_MPLS_PORT2_MULTICAST_GROUP_REMAP;
...
setup_vpls(int unit)
{
	...
	l2vpn_cfg_t l2vpn_cfg = {
		0x0,                       /* VPN */
		0x0,                       /* Multicast group */
        0x0                        /* Remap multicast group */
	};

	/* Access port 1 configuration variables */
	l2vpn_acc_cfg_t l2vpn_acc_1 = {
		egr_acc_port_1,            /* Access port */
		egr_acc_gport_1,           /* Access gport */
        0x0,                       /* Access VP gport */
		11,                        /* Access VLAN */
	...
	l2vpn_net_cfg_t l2vpn_net_1 = {
		ing_net_port_3,            /* Network port */
		ing_net_gport_3,           /* Network gport */
    ...
    /* Create a remap group for VPLS */
    rv = bcm_multicast_create(unit, BCM_MULTICAST_TYPE_VPLS,
                              &l2vpn_cfg.remap_mc_group);
    if (BCM_FAILURE(rv)) {
        printf("\nError in remap bcm_multicast_create: %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Remap multicast groups */
    rv = bcm_multicast_control_set(unit, l2vpn_cfg.bcast_group,
                                   bcmMulticastRemapGroup,
                                   l2vpn_cfg.remap_mc_group);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_multicast_control_set: %s.\n", bcm_errmsg(rv));
        return rv;
    }
	...
	rv = config_l2vpn_access(unit, &l2vpn_cfg, &l2vpn_acc_1, remap);
	if (BCM_FAILURE(rv)) {
	...
	rv = config_l2vpn_access(unit, &l2vpn_cfg, &l2vpn_acc_2, remap);
	if (BCM_FAILURE(rv)) {
	...
	rv = config_l2vpn_network(unit, &l2vpn_cfg, &l2vpn_net_1, remap);
	if (BCM_FAILURE(rv)) {
	...
	rv = config_l2vpn_network(unit, &l2vpn_cfg, &l2vpn_net_2, remap);
	if (BCM_FAILURE(rv)) {
		printf("\nError in network 1 config: %s.\n", bcm_errmsg(rv));
		return rv;
	}

	printf("Network-to-Access: Add payload-src-mac => Access VPort 1 \n");
	rv = add_to_l2_table(unit, l2vpn_acc_1.host_mac, l2vpn_cfg.mpls_vpn,
                         l2vpn_acc_1.acc_vport);
	if (BCM_FAILURE(rv)) {
