 ...
 */
/*
 * Feature  : VPWS (L2 MPLS) tunnel initiation with reserved/special 
 *            label support
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpws_AtoN_InsertReservedLabel.c
 *
 * config   : bcm56880_a0-generic-32x400.mpls-config.yml
 *
 * Log file : td4_hsdk_mpls_vpws_AtoN_InsertReservedLabel_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
...
setup_vpws(int unit)
{
    ...
    rv = mpls_insert_reserve_label(unit, vc_label_init, 0, ttl);
    if (BCM_FAILURE(rv)) {
