 ...
 */
/*
 * Feature  : VPWS (L2 MPLS) tunnel termination with reserved/special
 *            label support
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpws_NtoA_ReservedLabel.c
 *
 * config   : bcm56880_a0-generic-32x400.mpls-config.yml
 *
 * Log file : td4_hsdk_mpls_vpws_NtoA_ReservedLabel_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *
 *   Egress Packet:
 *   Drop packet get copied to cpu port
 */
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000bb0000000000aa8100001688470022201000333110000000ff0000000000110000000000228100000b0800450000360000000040ff65ac0a0000140a00000a000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f2021239412f0; sleep quiet 3", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");

    printf("\nReading drop event counter for mpls reserved label hit(bcmPktDropEventIngressMplsReservedLabelExposed)\n");
    uint64 val;
    bcm_pkt_drop_event_t drop_type = bcmPktDropEventIngressMplsReservedLabelExposed;
    print bcm_stat_drop_event_counter_get(unit, drop_type, &val);
    print val;
...
setup_vpws(int unit)
{
    ...
    rv = mpls_insert_reserve_label(unit, vc_label_term, 0, 0x10);
    if (BCM_FAILURE(rv)) {
        printf("\nError in mpls_insert_reserve_label: %s.\n", bcm_errmsg(rv));
        return rv;
    }

    printf("Copying mpls reserved label drop packets to cpu\n");
    bcm_switch_drop_event_mon_t mon;
    bcm_switch_drop_event_mon_t_init(&mon);
    mon.actions = BCM_SWITCH_MON_ACTION_COPY_TO_CPU;
    mon.drop_event = bcmPktDropEventIngressMplsReservedLabelExposed;
    rv = bcm_switch_drop_event_mon_set(unit, &mon);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_switch_drop_event_mon_set:%s\n", bcm_errmsg(rv));
        return rv;
    }
