 ...
 */
/*
 * Feature  : VPWS (L2 MPLS) tunnel initiation flow with label exp remarking
 *            (BCM_MPLS_TUNNEL_ENCAP_QOS_EXP_REMARK)
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpws_AtoN_QOS.c
 *
 * config   : bcm56880_a0-generic-32x400.mpls-config.yml
 *
 * Log file : td4_hsdk_mpls_vpws_AtoN_QOS_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *
 *   Egress Packet:
 *   0000 0000 00aa 0000 0000 00bb 8100 4016
 *   8847 0017 5410 0011 1410 0044 4110 0000
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000110000000000228100400B0800450000360000000040FF65AC0A0000140A00000A000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F2021239412F0; sleep quiet 3", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
...
bcm_error_t
mpls_customer_port_create(int unit, bcm_vpn_t vpn, uint32 vid, int ing_qos_id)
{
    ...
    cust_mpls_port.qos_map_id = ing_qos_id;
...
mpls_tunnel_initiator(int unit, uint32 label, uint8 ttl, uint32 flags,
                      bcm_gport_t *tunnel_id, int qos_id)
{
    ...
    tunnel_encap.label_array[1].label = label + 100;
    tunnel_encap.label_array[1].ttl = ttl;
    tunnel_encap.label_array[1].flags = flags;

    tunnel_encap.num_labels = 2;
    tunnel_encap.qos_map_id = qos_id;
...
vpws_qos_config(int unit, int *ing_qos, int *egr_qos)
{
    ...
    *ing_qos = ing_qos_id;
    ...
    rv = bcm_qos_port_map_set(unit, customer_gport, ing_qos_id, -1);
    if (BCM_FAILURE(rv)) {
    ...
    *egr_qos = egr_qos_id;
    ...
    for (i = 0; i < 8; i++) {
        ...
        /* TD4 supports multiple tunnel label exp's in a single QOS map */
        for(j = 0; j < BCM_QOS_MAX_TUNNEL_EXP; j++) {
            egr_qos_map.tunnel_exp[j] = tunnel_exp[i];
        }
...
setup_vpws(int unit)
{
    ...
    rv = mpls_customer_port_create(unit, vpn, customer_vid, ing_qos_id);
    if (BCM_FAILURE(rv)) {
