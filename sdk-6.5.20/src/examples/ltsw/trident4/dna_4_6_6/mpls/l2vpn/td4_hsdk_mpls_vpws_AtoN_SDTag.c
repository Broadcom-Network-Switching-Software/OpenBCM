 ...
 */
/*
 * Feature  : VPWS (L2 MPLS) tunnel initiation flow with SD-TAG action
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpws_AtoN_SDTag.c
 *
 * config   : bcm56880_a0-generic-32x400.mpls-config.yml
 *
 * Log file : td4_hsdk_mpls_vpws_AtoN_SDTag_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * payload vlan tag(SD-TAG) action
 *
 ...
 *
 *   Ingress Packet:
 *   0000 0000 0011 0000 0000 0022 8100 0005
 ...
 *   0000 0000 0022 8100 0005 0800 4500 0036
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000000011000000000022810000050800450000360000000040FF65AC0A0000140A00000A000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F2021239412F0; sleep quiet 3", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
...
bcm_error_t
vpws_sdtag_config(int unit, bcm_vpn_t vpn)
{
    /* Setup default port action to preserve ingress vlan tag */
    bcm_vlan_action_set_t default_action;
    bcm_error_t rv = BCM_E_NONE;

    bcm_vlan_action_set_t_init(default_action);
    default_action.outer_tag = bcmVlanActionNone;
...
setup_vpws(int unit)
{
    ...
    /* configure to preserve payload vlan tag */
    rv = vpws_sdtag_config(unit, vpn);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vpws_sdtag_config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
