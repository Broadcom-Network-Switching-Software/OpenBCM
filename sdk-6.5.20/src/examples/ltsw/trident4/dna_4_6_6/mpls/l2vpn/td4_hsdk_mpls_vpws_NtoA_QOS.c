 ...
 */
/*
 * Feature  : VPWS (L2 MPLS) tunnel termination flow with QOS mapping
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpws_NtoA_QOS.c
 *
 * config   : bcm56880_a0-generic-32x400.mpls-config.yml
 *
 * Log file : td4_hsdk_mpls_vpws_NtoA_QOS_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 *   8847 0022 2410 0033 3110 0000 00ff 0000
 ...
 *   Out Packet:
 *   -----------
 *   0000 0000 0011 0000 0000 0022 8100 400b
...
verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000bb0000000000aa8100001688470022241000333110000000ff0000000000110000000000228100000b0800450000360000000040ff65ac0a0000140a00000a000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f2021239412f0; sleep quiet 3", ingress_port);
    bshell(unit, str);
    print "show c";
    bshell(unit, "show c");
...
bcm_error_t
mpls_customer_port_create(int unit, bcm_vpn_t vpn, uint32 vid, int qos_id)
{
    ...
    /* cust_mpls_port.qos_map_id = qos_id; */
...
mpls_tunnel_terminator(int unit, uint32 label, uint32 flags,
                       bcm_mpls_switch_action_t action, int map_id)
{
    bcm_mpls_tunnel_switch_t info;
    bcm_error_t rv = BCM_E_NONE;

    bcm_mpls_tunnel_switch_t_init(&info);
    info.flags = BCM_MPLS_SWITCH_INT_PRI_MAP|BCM_MPLS_SWITCH_COLOR_MAP|flags;
    info.label = label;
...
/* Qos configuration */
bcm_error_t
vpws_qos_config(int unit, int *ing_qos, int *egr_qos)
{
    int ing_qos_id;
...
setup_vpws(int unit)
{
    ...
    /* Ceate an L3 interface to be used for the MPLS tunnel */
    ...
    rv = vpws_qos_config(unit, &ing_qos_id, &egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vpws_qos_config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = mpls_customer_port_create(unit, vpn, customer_vid, egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("\nError in mpls_customer_port_create: %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Attach egress QOS profile to customer port */
    rv = bcm_qos_port_map_set(unit, customer_gport, -1, egr_qos_id);
    if (BCM_FAILURE(rv)) {
