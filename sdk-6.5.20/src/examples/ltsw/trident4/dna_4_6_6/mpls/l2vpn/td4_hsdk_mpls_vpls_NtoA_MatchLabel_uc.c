...
/*
 * Feature  : TD4(56880) VPLS (L2 MPLS) tunnel termination unicast flow with
 *            LABEL as match criteria (BCM_MPLS_PORT_MATCH_LABEL)
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpls_NtoA_MatchLabel_uc.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_mpls_vpls_NtoA_MatchLabel_uc_log.txt
 *
 * Test Topology :
 *
 *                    +-------------------+
 ...
 *
 *   Unicast entry:
 *                L2 add payload-dest-mac => Access Port 1 (egr_acc_port_1)
 *
 ...
 * Summary:
 * ========
 *   Cint script to demonstrate VPLS (L2 MPLS) tunnel termination unicast flow
 *   with LABEL as match criteria.
 *
 ...
 *      c) Expected Result:
 *      ===================
 *         We can see that customer (payload) packet out on access
 *         port 1 as per L2 entry.
