 ...
 */
/*
 * Feature  : VPWS (L2 MPLS) tunnel initiation flow with failover/protection
 *            switching mechanism.
 *
 * Usage    : BCM.0> cint td4_hsdk_mpls_vpws_AtoN_FRR.c
 *
 * config   : bcm56880_a0-generic-32x400.mpls-config.yml
 *
 * Log file : td4_hsdk_mpls_vpws_AtoN_FRR_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 *                      |                   |+----> Primary provider port
 *      customer_port   |                   |
 *           ---------->+       TD4         |
 *     Ingress packet   |                   |
 ...
 *
 *   Egress Packet on primary port:
 *   0000 0000 00aa 0000 0000 00bb 8100 0016
 *   8847 0011 1010 0044 4110 0000 0000 0011
 *   0000 0000 0022 0800 4500 0036 0000 0000
 *   40ff 65ac 0a00 0014 0a00 000a 0001 0203
 *   0405 0607 0809 0a0b 0c0d 0e0f 1011 1213
 *   1415 1617 1819 1a1b 1c1d 1e1f 2021 2394
 *   12f0
 *
 *   Egress Packet on backup port:
 *   0000 0000 00cc 0000 0000 00dd 8100 0021
 *   8847 0055 5010 0088 8110 0000 0000 0011
 *   0000 0000 0022 0800 4500 0036 0000 0000
 *   40ff 65ac 0a00 0014 0a00 000a 0001 0203
 *   0405 0607 0809 0a0b 0c0d 0e0f 1011 1213
 *   1415 1617 1819 1a1b 1c1d 1e1f 2021 2394
...
bcm_port_t provider_port_backup;
bcm_gport_t customer_gport;
bcm_gport_t provider_gport;
bcm_gport_t provider_gport_backup;

bcm_port_t ingress_port;
bcm_port_t egress_port;
bcm_port_t egress_port_backup;

bcm_gport_t mpls_p_port_id_backup;
bcm_if_t egr_obj_primary;
...
bcm_error_t
egress_port_multi_setup_backup(int unit, bcm_pbmp_t pbm)
{
    bcm_field_group_t group;
    bcm_field_entry_t entry;
    bcm_port_t     port;
...
test_setup(int unit)
{
    ...
    provider_port_backup = port_list[2];

    ingress_port = customer_port;
    egress_port = provider_port;
    egress_port_backup = provider_port_backup;
    ...
    if (BCM_E_NONE != bcm_port_gport_get(unit, provider_port_backup,
                                         &provider_gport_backup)){
        printf("bcm_port_gport_get() failed to get gport for port %d\n",
                                                 provider_port_backup);
        return -1;
    }
    ...
    BCM_PBMP_CLEAR(pbmp);
    BCM_PBMP_PORT_ADD(pbmp, egress_port_backup);
    if (BCM_E_NONE != egress_port_multi_setup_backup(unit, pbmp)) {
        printf("egress_port_multi_setup_backup() failed for port %d\n",
                                                   egress_port_backup);
        return -1;
    }
...
verify(int unit)
{
    ...
    /* Now enable failover on backup port */
    printf("\nEnabling failover on backup mpls port\n");
    rv = bcm_failover_egress_status_set(unit, egr_obj_primary, 1);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_failover_egress_status_set:%s\n", bcm_errmsg(rv));
        return rv;
    }
    bshell(unit, str);
    print "show c";
...
bcm_error_t
create_egr_object_backup(int unit, uint32 flag, int l3_if, bcm_mac_t nh_mac,
                         bcm_gport_t gport,
                   int vid, bcm_if_t *egr_obj_id, bcm_if_t egr_obj_id_primary)
{
    bcm_l3_egress_t l3_egress;
    bcm_error_t rv = BCM_E_NONE;
    ...
    l3_egress.vlan = vid;
    l3_egress.port = gport;
    rv = bcm_failover_egress_set(unit, egr_obj_id_primary, &l3_egress);
    return rv;
}
...
bcm_error_t
mpls_provider_port_create_backup(int unit, bcm_vpn_t vpn, uint32 vc_label_init,
                                 uint32 vc_label_term, uint8 ttl,
                                 bcm_if_t egr_obj, bcm_if_t tunnel_id)
{
    ...
    printf("MPLS network port backup = 0x%x\n", net_mpls_port.mpls_port_id);
    mpls_p_port_id_backup = net_mpls_port.mpls_port_id;
    return rv;
}
                          ...
                          bcm_if_t tunnel_id)
{
    ...
    net_mpls_port.egress_label.flags = BCM_MPLS_EGRESS_LABEL_TTL_SET;
    net_mpls_port.tunnel_id = tunnel_id;
    net_mpls_port.failover_port_id = mpls_p_port_id_backup;
    rv = bcm_mpls_port_add(unit, vpn, &net_mpls_port);
...
setup_vpws(int unit)
{
    ...
    bcm_vlan_t  provider_vid_backup = 33;
    uint32 tunnel_label_init_backup = 0x555;
    uint32 tunnel_label_term_backup = 0x666;
    uint32 vc_label_term_backup     = 0x777;
    uint32 vc_label_init_backup     = 0x888;
    bcm_mac_t   outer_da_backup = {0x00, 0x00, 0x00, 0x00, 0x00, 0xcc};
    bcm_mac_t   outer_sa_backup = {0x00, 0x00, 0x00, 0x00, 0x00, 0xdd};
    bcm_if_t    l3_intf_backup;
    bcm_if_t    egr_obj_backup;
    bcm_gport_t tunnel_id_backup;
    ...
    printf("create vlan %d\n", provider_vid_backup);
    rv = vlan_create_add_port(unit, provider_vid_backup, provider_port_backup);
    if (BCM_FAILURE(rv)) {
        printf("Error in vlan_create_add_port: %s\n", bcm_errmsg(rv));
        return rv;
    }

    /* Create primary L3 interface to be used for the MPLS tunnel */
    rv = create_l3_interface(unit, 0, BCM_L3_INTF_UNDERLAY, outer_sa,
                             provider_vid, &l3_intf);
    if (BCM_FAILURE(rv)) {
        printf("\nError in primary create_l3_interface: %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Create backup L3 interface to be used for the MPLS tunnel */
    rv = create_l3_interface(unit, 0, BCM_L3_INTF_UNDERLAY, outer_sa_backup,
                             provider_vid_backup, &l3_intf_backup);
    if (BCM_FAILURE(rv)) {
        printf("\nError in backup create_l3_interface: %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Create primary L3 egress object for MPLS tunnel */
    rv = create_egr_object(unit, BCM_L3_FLAGS2_UNDERLAY, l3_intf, outer_da,
                           provider_gport, provider_vid, &egr_obj);
    if (BCM_FAILURE(rv)) {
        printf("\nError in primary create_egr_object: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    egr_obj_primary = egr_obj;

    /* Create backup L3 egress object for MPLS tunnel */
    rv = create_egr_object_backup(unit, BCM_L3_FLAGS2_UNDERLAY, l3_intf_backup,
                                  outer_da_backup, provider_gport_backup,
                                  provider_vid_backup, &egr_obj_backup, egr_obj);
    if (BCM_FAILURE(rv)) {
        printf("\nError in backup create_egr_object_backup: %s.\n",
                                                   bcm_errmsg(rv));
        return rv;
    }

    rv = bcm_failover_egress_protection_get(unit, egr_obj, &egr_obj_backup);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_failover_egress_protection_get: %s.\n",
                                                       bcm_errmsg(rv));
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        printf("\nError in primary mpls_tunnel_initiator:%s.\n",bcm_errmsg(rv));
        return rv;
    }

    /* Set MPLS tunnel initiator for backup port */
    rv = mpls_tunnel_initiator(unit, tunnel_label_init_backup, ttl,
                               BCM_MPLS_EGRESS_LABEL_TTL_SET,&tunnel_id_backup);
    if (BCM_FAILURE(rv)) {
        printf("\nError in backup mpls_tunnel_initiator: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        printf("\nError in primary mpls_tunnel_terminator: %s.\n",
                                                   bcm_errmsg(rv));
        return rv;
    }
    /* Add MPLS label lookup for tunnel termination */
    rv = mpls_tunnel_terminator(unit, tunnel_label_term_backup,
                                BCM_MPLS_SWITCH_COUNTED,
                                BCM_MPLS_SWITCH_ACTION_POP);
    if (BCM_FAILURE(rv)) {
        printf("\nError in backup mpls_tunnel_terminator: %s\n",bcm_errmsg(rv));
        return rv;
    }
    ...
    /* Create backup mpls provider port and bind it to vpn */
    rv = mpls_provider_port_create_backup(unit, vpn, vc_label_init_backup,
                                          vc_label_term_backup, ttl,
                                          egr_obj_backup, tunnel_id_backup);
    if (BCM_FAILURE(rv)) {
        printf("\nError in mpls_provider_port_create_backup: %s.\n",
                                                    bcm_errmsg(rv));
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_failover_init(): %s.\n", bcm_errmsg(rv));
        return rv;
    }

    /* Create primary mpls provider port and bind it to vpn */
