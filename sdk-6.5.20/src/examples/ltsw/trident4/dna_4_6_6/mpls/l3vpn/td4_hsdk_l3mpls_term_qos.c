...
/*
 * Feature  : L3 MPLS QOS mapping in tunnel termination
 *
 * Usage    : BCM.0> cint td4_hsdk_l3mpls_term_qos.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : td4_hsdk_l3mpls_term_qos_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 * Summary:
 * ========
 *   This Cint example to show configuration of L3 MPLS tunnel termination with
 *   QOS using BCM APIs.
 *
 ...
 *   =========================================================
 *     a) Configure a basic L3 MPLS tunnel termination with QOS functional
 ...
 *       routed through the egress access port with remarket .1Q PRI.
...
test_setup(int unit)
{
    ...
    if (BCM_E_NONE != egress_port_setup(unit, egr_acc_port_2)) {
...
bcm_error_t
config_l3_mpls_access (int unit, l3vpn_acc_cfg_t *l3vpn_acc_cfg, int egr_qos_id)
{
    ...
    rv = bcm_qos_port_map_set(unit, l3vpn_acc_cfg->acc_gport, -1, egr_qos_id);
    if (BCM_FAILURE(rv)) {
...
config_l3_mpls_network (int unit, l3vpn_acc_cfg_t *l3vpn_acc_cfg,
                        l3vpn_net_cfg_t *l3vpn_net_cfg, int ing_qos_id)
{
...
config_l3_mpls(int unit)
{
    ...
    rv = config_l3_mpls_access (unit, &l3vpn_acc, egr_qos_id);
    if (BCM_FAILURE(rv)) {
        printf("config_l3_mpls_access return error : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    rv = config_l3_mpls_network (unit, &l3vpn_acc, &l3vpn_net, ing_qos_id);
    if (BCM_FAILURE(rv)) {
