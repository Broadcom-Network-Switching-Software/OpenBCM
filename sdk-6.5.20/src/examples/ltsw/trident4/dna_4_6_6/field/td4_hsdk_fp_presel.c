 ...
 *
 *  Usage : BCM.0> cint fp_presel.c YES [WithPresel=0/1] [Stage=1/2/3]
 *           E.g.  cint fp_presel.c YES 0 1      <<<<< WithoutPresel   Stage: VFP
 *           E.g.  cint fp_presel.c YES 1 1      <<<<< WithPresel      Stage: VFP
 *           E.g.  cint fp_presel.c YES 0 2      <<<<< WithoutPresel   Stage: IFP
 *           E.g.  cint fp_presel.c YES 1 2      <<<<< WithPresel      Stage: IFP
 *           E.g.  cint fp_presel.c YES 0 3      <<<<< WithoutPresel   Stage: EFP
 *           E.g.  cint fp_presel.c YES 1 3      <<<<< WithPresel      Stage: EFP
 *
 *  config   : bcm56880_a0-generic-32x400.config.yml
 *
 *  Log file : fp_presel_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+
    ...
    TestFpPresel()
{
    ...
    if( opt_TestWithPresel ) {
        ...
        //if( opt_IsVfp ) BCM_FIELD_QSET_ADD(gcfg_pre.qset, bcmFieldQualifyOuterPriTaggedPkt);
        if( opt_IsVfp || opt_IsIfp ) BCM_FIELD_QSET_ADD(gcfg_pre.qset, bcmFieldQualifyL2MacInfo);
        
        if( opt_IsEfp ) BCM_FIELD_QSET_ADD(gcfg_pre.qset, bcmFieldQualifyPktType);
        //if( opt_IsEfp ) BCM_FIELD_QSET_ADD(gcfg_pre.qset, bcmFieldQualifyL2EgrIntfEfpCtrlId);
        ...
        if( 0 ) // opt_IsVfp
            BCM_IF_ERROR_RETURN( bcm_field_qualify_OuterPriTaggedPkt(unit,
                              BCM_FIELD_QUALIFY_PRESEL | presel_id1, 1, 0xFF) ); // VID == 0
        if( opt_IsEfp )
...
bcm_error_t verify()
{
    ...
    if( opt_IsVfp || opt_IsIfp )
    {
        ...
        sprintf(cmd, "tx 1 pbm=%d data=%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X8100%04X08004500LLLL000000004011SSSS%08X%08X%04X%04X0036C6B0000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F202122232425262728292A2B2C2D29197737CCCCCCCCCCCC",
            ...
            test_dmac[3], test_dmac[4], test_dmac[5],
            test_smac[0], test_smac[1], test_smac[2],
            test_smac[3], test_smac[4]|0x1, test_smac[5],
            FORM_VID(prio, 0, test_vid), test_sip, test_dip,
            test_udp_src_port,
            test_udp_dst_port );
        //printf("BEFORE: %s\n", cmd);
