...
/*
 * Feature  : TD4(56880), Verifying drop Event Mirroring 
 *
 * Usage    : BCM.0> cint TD4_HSDK_Mirror_drop_event.c
 *
 * config   : bcm56880_a0-generic-32x400.config.yml
 *
 * Log file : TD4_HSDK_Mirror_drop_event_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
 ...
 * Summary:
 * ========
 * Cint example to demonstrate drop event mirroring
 *
 * Detailed steps done in the CINT script:
 * =======================================
 *   1) Step1 - Test Setup (Done in testsetup()):
 *   =============================================
 *     a) Selects one ports and configure it in Loopback mode.
 *
 *     c) add static src L2 entry with drop bit set
 * 
 *     d) Create a mirror destination for MTP port
 *
 *     e) Configure mirror drop event to be bcmPktDropEventIngressL2SrcDiscard
 *
 *   3) Step3 - Verification (Done in testVerify()):
 *   ===========================================
 *
 *     b) Transmit the packet. 
 *        The source mac of that packet is the same as the added L2_ENTRY which has drop bit set.
 *        This will cause the drop event IngressL2SrcDiscard.
 *
 *     c) Expected Result:
 *     ===================
...
bcm_error_t configure_drop_event_mirror(int unit, bcm_gport_t mirror_dest_id)
{

    bcm_mirror_source_t_init(&source);
    source.type = bcmMirrorSourceTypePktDropEvent;
    source.drop_event = bcmPktDropEventIngressL2SrcDiscard ;
...
int testSetup(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(configure_drop_event_mirror(unit, mirror_dest_id));
...
bcm_error_t testVerify(int unit)
{
    ...
    printf("Packet comes into ingress port %d, hit the L2_ENTRY with drop bit set\n", ingress_port);
    printf("Matching drop reason IngressL2SrcDiscard, packet is mirror to port %d\n", mtp_port);
