 ...
 */
/*
 * Feature  : Flex Digest configuration along with Trunk on TD4 for non uc case
 *
 * Usage    : BCM.0> cint td4_hsdk_port_trunk_flex_digest_non_uc.c
 *
 * config   : bcm56880_a0-generic.config.yml
 *
 * Log file : td4_hsdk_port_trunk_flex_digest_non_uc_log.txt
 *
 * Test Topology :
 *
 *                     +-------------------+
 ...
 *                     |                   |
 *                     |                   |
 *    +-----------Ingressport        EgressPort3+----------------->
 *                     |                   |
 *    +---->           |                   |
 *  Ingress Pkts:      |             EgressPort4+----------------->
 *                     |                   |
 *  Set-1:             |                   |
 *  DMAC=00::01        |             EgressPort5+----------------->
 *  SMAC=00:XX         |                   |
 *  VLAN=2             |                   |         TRUNK
 *                     |                   |         ID = 0
 *                     |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate unicast LAG resolution forwarding on trunk ports.
 *
 ...
 *
 *   Egress Packet:
 *   packets based on  LAG resolution forwarding on trunk ports
 */
cint_reset();
...
verify(int unit)
{
    ...
    /*for (i = 0; i < pkt_cnt; i++) {
    snprintf(str, 512, "tx 1 pbm=%d sm=00:00:00:cc:cc:%02d dm=00:00:00:bb:bb:bb vlan=2;sleep 1;", ingress_port, i);	*/
    snprintf(str, 512, "tx 20 pbm=%d DestMac=0x11 DestMacInc=1 SourceMac=0x3214 SourceMacInc=1 vlan=2 len=80; sleep quiet 1", ingress_port);
        printf("%s\n", str);
        bshell(unit, str);
    /*}*/
