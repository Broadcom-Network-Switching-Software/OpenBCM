 ...
 *
 *  Usage    : BCM.0> cint l3_ing_l3_intf_flexctr.c
 *
 *  config   : l3_ing_l3_intf_flexctr_config.bcm
 *
 *  Log file : l3_ing_l3_intf_flexctr_log.txt
 *
 *  Test Topology : None
 *
...
void l3_route_uc_create()
{
    ...
    action.source = bcmFlexctrSourceIngL3Intf;
    action.hint = 0;
    ...
    rv = bcm_l3_ingress_stat_attach(unit, ingress_if, stat_counter_id);
    if (BCM_FAILURE(rv)) {
        printf("ERROR: bcm_l3_ingress_stat_attach() - %d : %s.\n", rv, bcm_errmsg(rv));
    }

    rv = bcm_l3_ingress_stat_id_get(unit, ingress_if, bcmL3StatInPackets, &tmp_stat_counter_id);
    if (BCM_FAILURE(rv)) {
        printf("ERROR: bcm_l3_ingress_stat_attach() - %d : %s.\n", rv, bcm_errmsg(rv));
    }
    printf("statid = %d.\n", tmp_stat_counter_id);
