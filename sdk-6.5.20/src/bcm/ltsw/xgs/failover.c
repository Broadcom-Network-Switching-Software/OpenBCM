...
xgs_ltsw_failover_clear(int unit)
{
    ...
    /* Clear TNL_MPLS_PROTECTION_ENABLE table. */
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmi_lt_clear(unit, TNL_MPLS_PROTECTION_ENABLEs));
...
xgs_ltsw_failover_init(int unit)
{
    ...
    if (bcmi_ltsw_property_get(unit, BCMI_FAILOVER_FIXED_NH_OFFSET_ENABLE, 0)) {
        /* Set NH offset to half size of EGR_L3_NEXT_HOPm */
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_lt_field_value_range_get(unit, L3_UC_NHOPs, NHOP_IDs, &min, &max));
        nh_tbl_size = max + 1;
    ...
    int enable)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmlt_entry_allocate(dunit, TNL_MPLS_PROTECTION_ENABLEs, &ent_hdl));
