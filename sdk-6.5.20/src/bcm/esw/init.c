...
/* Temporarily disable eapps modules on MV2 SVK till integration is complete  */
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) ||
        ...
        if (soc_property_get(unit, spn_QCM_FLOW_ENABLE, 0)) {
            rv = bcm_esw_cosq_burst_monitor_detach(unit);
        } else {
...
_bcm_modules_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cancun)) {
        /* Bind the supported SOC features with the running Cancun
           version */
        SOC_IF_ERROR_RETURN(soc_cancun_version_soc_feature_bind(unit));
    }
#endif
    ...
    if (SOC_IS_TRIDENT3X(unit) && soc_feature(unit, soc_feature_higig_over_ethernet)) {
        ...
        if (SOC_IS_TRIDENT3X(unit) &&
            soc_feature(unit, soc_feature_enable_flow_based_udf_extraction)) {
    ...
    init_cond2 = init_cond || (soc_property_get(unit, "skip_stat_init",
                                                0) ? FALSE : TRUE);
    _BCM_MOD_INIT_CER(unit, bcm_esw_stat_init, SHR_BPROF_BCM_STAT, init_cond2);
    ...
    if (init_knet && ((SOC_IS_TRIDENT2(unit) || SOC_IS_TOMAHAWK(unit)))) {
 ...
 */
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) ||
        ...
        soc_feature(unit, soc_feature_flex_flowtracker_ver_1)) {
        if (soc_property_get(unit, spn_QCM_FLOW_ENABLE, 0)) {
            _BCM_MOD_INIT_IER(unit, bcm_esw_cosq_burst_monitor_init, SHR_BPROF_BCM_FT, TRUE);
        } else {
    ...
    if (SOC_IS_MONTEREY(unit)) {
        SOC_IF_ERROR_RETURN(soc_monterey_gatingconfig_init(unit)); 
        _BCM_MOD_INIT_IER(unit, bcm_esw_cpri_speed_scan_init, SHR_BPROF_BCM_CPRI, TRUE);
    }
#endif
#if defined(INCLUDE_XFLOW_MACSEC)
    ...
    switch (module_number) {
         ...
         case BCM_MODULE_FLOWTRACKER:
                   if (soc_feature(unit, soc_feature_uc_flowtracker_learn) ||
                       ...
                       soc_feature(unit, soc_feature_flex_flowtracker_ver_1)) {
                       if (soc_property_get(unit, spn_QCM_FLOW_ENABLE, 0)) {
                           BCM_IF_ERROR_RETURN(bcm_esw_cosq_burst_monitor_init(unit));
                       } else {
#if defined(BCM_TOMAHAWK3_SUPPORT)
                           if (SOC_IS_TOMAHAWK3(unit)) {
