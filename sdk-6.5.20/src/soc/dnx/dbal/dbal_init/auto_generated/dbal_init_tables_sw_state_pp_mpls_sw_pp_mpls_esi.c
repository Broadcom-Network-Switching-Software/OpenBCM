...
shr_error_e
_dbal_init_tables_sw_state_pp_mpls_sw_pp_mpls_esi_mpls_label_range_info_sw_init(
int unit,
table_db_struct_t * cur_table_param,
dbal_logical_table_t * table_info)
{
    dbal_logical_table_t * table_entry = table_info + DBAL_TABLE_MPLS_LABEL_RANGE_INFO_SW;
    int is_valid = TRUE;
    SHR_FUNC_INIT_VARS(unit);
    dbal_init_table_db_struct_clear(cur_table_param);
    SHR_IF_ERR_EXIT(dbal_db_init_general_info_set(unit, table_entry, DBAL_TABLE_MPLS_LABEL_RANGE_INFO_SW , is_valid , "MPLS_LABEL_RANGE_INFO_SW" , DBAL_ACCESS_METHOD_SW_STATE ));
    ...
    
    {
        ...
        SHR_IF_ERR_EXIT(dbal_db_init_table_set_table_incompatible_image_sw_state_indication(unit, DBAL_TABLE_MPLS_LABEL_RANGE_INFO_SW, is_standard_1, is_compatible_with_all_images));
    }
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_sw_state_set_indirect_table_size(unit, cur_table_param, "DNX_DATA.mpls.general.nof_mpls_range_elements" , dnx_data_mpls.general.nof_mpls_range_elements_get(unit) ));
    SHR_IF_ERR_EXIT(dbal_db_init_table_add(unit, cur_table_param, DBAL_TABLE_MPLS_LABEL_RANGE_INFO_SW, table_info));
exit:
    SHR_FUNC_EXIT;
}
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(_dbal_init_tables_sw_state_pp_mpls_sw_pp_mpls_esi_mpls_label_range_info_sw_init(unit, cur_table_param, table_info));
exit:
    SHR_FUNC_EXIT;
}
