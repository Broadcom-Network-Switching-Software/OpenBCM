...
/**
@name: soc_firebolt6_set_tdm_soc_pkg
 ...
 */
STATIC int
soc_firebolt6_set_tdm_soc_pkg(int unit, soc_port_schedule_state_t
			*port_schedule_state, tdm_soc_t *soc_pkg, int new_or_prev)
{
    int port, phy_port, lane, index, pm_num;
    uint32 speed[FB6_NUM_EXT_PORTS];
    uint32 port_state[FB6_NUM_EXT_PORTS]; /* enum port_state_e */
    ...
    soc_pkg->unit = unit;
    soc_pkg->num_ext_ports = FB6_NUM_EXT_PORTS;
    ...
    sal_memset(speed,      0, FIREBOLT6_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));
    sal_memset(port_state, 0, FIREBOLT6_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));

    for (port = 0; port < FIREBOLT6_DEV_PORTS_PER_DEV; port++) {
...
/*Mgmt Mode updates*/
    ...
    for (port= 0; port < FIREBOLT6_TDM_PHY_PORTS_PER_DEV; port++) {
        if (port_map->log_port_speed[port] > 0 ) {
            phy_port = port_map->port_l2p_mapping[port];
          if (SOC_PBMP_MEMBER(port_map->management_pbm, port)) {
              soc_pkg->soc_vars.fb6.mgmt_mode[(phy_port-1)] = 1;
          }
          else {
              soc_pkg->soc_vars.fb6.mgmt_mode[(phy_port-1)] = 0;
          }
        }
    }
    }
    ...
    if (1 == port_schedule_state->is_flexport) {
        ...
        for (pm_num = 0; pm_num < FB6_NUM_PHY_PM; pm_num++) { /* FIREBOLT6_PBLKS_PER_PIPE * FIREBOLT6_PIPES_PER_DEV */
            soc_pkg->soc_vars.fb6.pm_num_to_pblk[pm_num] =
                  port_schedule_state->in_port_map.port_p2PBLK_inst_mapping[
									(pm_num * FIREBOLT6_PORTS_PER_PBLK) + 1];
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "\n PM NUM to PBLK DEBUG1 : pm_num %d pm_num_to_pblk %d\n"),  pm_num, soc_pkg->soc_vars.fb6.pm_num_to_pblk[pm_num] ));
        }
    }
    ...
    for (index = 0; index < FB6_NUM_EXT_PORTS; index++) {
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
...
/**
@name: soc_firebolt6_tdm_copy_prev_tables
@param: int unit, soc_port_schedule_state_t *port_schedule_state
        tdm_soc_t *soc_pkg, int new_or_prev
 * Description:
 *      Helper function to allocate & populate state and speed in a tdm_soc_t structure
 ...
 */
STATIC int
soc_firebolt6_tdm_copy_prev_tables(int unit,
			soc_port_schedule_state_t *port_schedule_state, tdm_mod_t *_tdm_pkg)
{
    ...
    sal_memcpy(_tdm_pkg->_prev_chip_data.cal_0.cal_main,
               port_schedule_state->tdm_ingress_schedule_pipe[
				...
				sizeof(int)*FIREBOLT6_TDM_LENGTH);
    ...
    /* OVS tables */
    for (grp_num = 0; grp_num < FIREBOLT6_OVS_GROUP_COUNT_PER_PIPE; grp_num ++) {
        hpipe = grp_num / FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE;
        hpipe_grp = grp_num % FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE;
        sal_memcpy(_tdm_pkg->_prev_chip_data.cal_0.cal_grp[grp_num],
                   port_schedule_state->tdm_ingress_schedule_pipe[
					0].tdm_schedule_slice[hpipe].oversub_schedule[hpipe_grp],
                   sizeof(int) * FIREBOLT6_OVS_GROUP_TDM_LENGTH);
    ...
    for (hpipe = 0; hpipe < 2; hpipe ++) {
        sal_memcpy(_tdm_pkg->_prev_chip_data.cal_0.cal_grp[
							FB6_SHAPING_GRP_IDX_0 + hpipe],
                   port_schedule_state->tdm_ingress_schedule_pipe[
					0].tdm_schedule_slice[hpipe].pkt_sch_or_ovs_space[0],
                   sizeof(int) * FB6_SHAPING_GRP_LEN);
...
/**
@name: soc_firebolt6_tdm_set_out_map
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 */
STATIC int
soc_firebolt6_tdm_set_out_map(int unit,
				soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i= 0; i< SOC_MAX_NUM_PORTS;i++) {
        ...
        port_schedule_state->out_port_map.num_subports[i]
							= port_schedule_state->in_port_map.num_subports[i];
        port_schedule_state->out_port_map.port_lc_mapping[i]
							= port_schedule_state->in_port_map.port_lc_mapping[i];
    ...
    sal_memcpy(port_schedule_state->out_port_map.port_l2p_mapping,
               ...
               sizeof(int)*FIREBOLT6_DEV_PORTS_PER_DEV);
    ...
    sal_memcpy(port_schedule_state->out_port_map.port_m2p_mapping,
               ...
               sizeof(int)*FIREBOLT6_MMU_PORTS_PER_DEV);
    ...
    sal_memcpy(&port_schedule_state->out_port_map.coe_port_pbm,
               &port_schedule_state->in_port_map.coe_port_pbm, sizeof(pbmp_t));
    ...
    if (0 == port_schedule_state->is_flexport) {
        for (i = 0; i < port_schedule_state->nport; i++) {
            ...
            if (-1 != physical_port) { /* that is, port down */
                if (1 == port_schedule_state->resource[i].coe_port) {
                    SOC_PBMP_PORT_ADD(port_schedule_state->out_port_map.coe_port_pbm,
										physical_port);
                }
	        }
        }
        ...
        for (i = 0; i < port_schedule_state->nport; i++) {
            ...
            if (-1 == physical_port) { /* that is, port down */
              ...
              port_schedule_state->out_port_map.port_p2m_mapping[
								prev_physical_port] = -1;
              port_schedule_state->out_port_map.port_lc_mapping[
	       							prev_physical_port] = -1;

              port_schedule_state->out_port_map.num_subports[
								prev_physical_port] = -1;
              ...
              SOC_PBMP_PORT_REMOVE(port_schedule_state->out_port_map.oversub_pbm,
										logical_port);
              SOC_PBMP_PORT_REMOVE(port_schedule_state->out_port_map.coe_port_pbm,
	     									prev_physical_port);
        ...
        for (i = 0; i < port_schedule_state->nport; i++) {
            ...
            if (-1 != physical_port) { /* that is, port up */
                ...
                port_schedule_state->out_port_map.port_p2l_mapping[physical_port] =
										logical_port;
                 port_schedule_state->out_port_map.num_subports[physical_port] =
					port_schedule_state->resource[i].num_subports;
                 port_schedule_state->out_port_map.port_lc_mapping[physical_port] =
    ...
    soc_firebolt6_port_schedule_speed_remap(unit, port_schedule_state);
...
/**
@name: soc_firebolt6_tdm_flexport_ovs_add_rem_port
 ...
 */
int
soc_firebolt6_tdm_flexport_ovs_add_rem_port(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int logical_port,
    int rem_add_cmd,
    int set_idb,
    int set_mmu )
{
    ...
    static const soc_reg_t idb_grp_cfg_regs[FIREBOLT6_PIPES_PER_DEV][2] = {
    ...
    static const soc_reg_t idb_grp_ovs_regs[FIREBOLT6_PIPES_PER_DEV][2][6] = {
        {
    ...
    static const soc_reg_t idb_pblk_cal_regs[FIREBOLT6_PIPES_PER_DEV][2][8] = {
        {
    ...
    static const soc_mem_t idb_pkt_shaper_mems[FIREBOLT6_PIPES_PER_DEV][2] = {
    ...
    switch(rem_add_cmd) {
        ...
        case 0: port_map = &port_schedule_state->in_port_map;
                ...
                idb_port = FIREBOLT6_TDM_IDB_UNUSED_TOKEN & 0x3f;
                mmu_port = FIREBOLT6_TDM_MMU_UNUSED_TOKEN & 0x3f;
                id = 0x7;
                ...
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "PORT DOWN   phy_port=%2d idb_port=%2d mmu_port=%2d id=%2d"), phy_port, idb_port, mmu_port, id));
                break;
        ...
        case 1: port_map = &port_schedule_state->out_port_map;
                ...
                cfg_en_PBLK_CALENDAR = 1;
                pblk_valid = 1;
                pblk_spacing = _soc_fb6_tdm_get_same_port_spacing_e(unit,
					                 port_map->log_port_speed[logical_port]);
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "PORT   UP   pipe=%1d"), pipe));
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "PORT UP phy_port=%2d idb_port=%2d mmu_port=%2d id=%2d"), phy_port, idb_port, mmu_port, id));
                break;
        ...
        case 2: port_map = &port_schedule_state->out_port_map;
                ...
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "MOVE-REMOVE phy_port=%2d idb_port=%2d mmu_port=%2d id=%2d"), phy_port, idb_port, mmu_port, id));
                break;
        ...
        case 3: port_map = &port_schedule_state->out_port_map;
                ...
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "MOVE-ADD phy_port=%2d idb_port=%2d mmu_port=%2d id=%2d"), phy_port, idb_port, mmu_port, id));
                break;
    ...
    for (hpipe_i = 0; hpipe_i < FIREBOLT6_OVS_HPIPE_COUNT_PER_PIPE; hpipe_i++) {
        for (group_i = 0; group_i < FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE; group_i++) {
            for (slot_i = 0; slot_i < FIREBOLT6_OVS_GROUP_TDM_LENGTH; slot_i++) {
                if (phy_port ==
    ...
    switch(rem_add_cmd) {
        ...
        case 2:
                ...
                curr_tdm_schedule_pipe->tdm_schedule_slice[
					hpipe].oversub_schedule[group][slot] = FB6_NUM_EXT_PORTS;
                /* If group gets empty due to remove then configure ovs_class to 0; otherwise skip*/
                cfg_en_OVR_SUB_GRP_CFG = 1;
                for (i = 0; i < FIREBOLT6_OVS_GROUP_TDM_LENGTH; i++) {
                    if (FB6_NUM_EXT_PORTS >
						curr_tdm_schedule_pipe->tdm_schedule_slice[
        ...
        case 3:
                ...
                if (cfg_en_OVR_SUB_GRP_CFG == 1)  {
                    ovs_class = _soc_fb6_tdm_get_speed_ovs_class(unit,
								port_map->log_port_speed[logical_port]);
                }
                /* Add to temp table */
                curr_tdm_schedule_pipe->tdm_schedule_slice[
    ...
    if (cfg_en_OVR_SUB_GRP_CFG == 1) {
        ...
        ovs_same_spacing = _soc_fb6_tdm_get_same_port_spacing_e(unit,
									port_map->log_port_speed[logical_port]);
        ...
        if (set_idb == 1) {
            ...
            soc_reg_field_set(unit, reg, &rval, SAME_SPACINGf,
						(ovs_class == 0) ? 0 : 6); /* on ingress, all ports have HSP same spacing */
    ...
    if (cfg_en_PBLK_CALENDAR == 1) {
        ...
        subport = (phy_port - 1) % FIREBOLT6_PORTS_PER_PBLK;
...
/**
@name: soc_firebolt6_tdm_flexport_remove_ports_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
int
soc_firebolt6_tdm_flexport_remove_ports_sel(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int set_idb,
    int set_mmu )
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if ((-1 == physical_port) && (1 == is_port_oversub)) { /* that is, OVS port going DOWN */
            soc_firebolt6_tdm_flexport_ovs_add_rem_port(unit,
                                                        port_schedule_state,
                                                        logical_port,
                                                        rem_add_cmd,
                                                        set_idb,
...
/**
@name: soc_firebolt6_tdm_flexport_add_ports_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
int
soc_firebolt6_tdm_flexport_add_ports_sel(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int set_idb,
    int set_mmu )
{
...
/**
@name: soc_firebolt6_tdm_flexport_ovs_consolidate_per_speed
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
int
soc_firebolt6_tdm_flexport_ovs_consolidate_per_speed(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int pipe,
    int hpipe,
    int ovs_class,
    ...
    int set_mmu )
{
    ...
    int ovs_class_i;
    int grp_bmp[FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE];
    int num_of_ovs_grps;
    ...
    for (group = 0; group < FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE; group++) {
        for (slot = 0; slot < FIREBOLT6_OVS_GROUP_TDM_LENGTH; slot++) {
            physical_port =
               curr_tdm_schedule_hpipe->oversub_schedule[group][slot];
            if (physical_port < FB6_NUM_EXT_PORTS) {
                logical_port =
				port_schedule_state->out_port_map.port_p2l_mapping[physical_port];
                ovs_class_i = _soc_fb6_tdm_get_speed_ovs_class(unit,
                port_schedule_state->out_port_map.log_port_speed[logical_port]);
    ...
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
			"START soc_firebolt6_tdm_flexport_ovs_consolidate_per_speed"
            ...
            for (slot = 0; slot < FIREBOLT6_OVS_GROUP_TDM_LENGTH; slot++) {
                ...
                if (physical_port < FB6_NUM_EXT_PORTS) {
                    ...
                    if (physical_port ==
                        ...
                        found = 0;
                        for (i = 0; i < FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE; i++) {
                            for (j = 0; j < FIREBOLT6_OVS_GROUP_TDM_LENGTH; j++) {
                                if (physical_port ==
				...
				"___ERROR could not find group and slot in new OVS for phy_port=%d"),
                        ...
                        if (curr_tdm_schedule_hpipe->oversub_schedule[
								dst_group][dst_slot] < FB6_NUM_EXT_PORTS) {
                            continue;
                        } else {
                            logical_port =
								port_schedule_state->out_port_map.port_p2l_mapping[
...
/**
@name: soc_firebolt6_tdm_flexport_ovs_consolidate_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
int
soc_firebolt6_tdm_flexport_ovs_consolidate_sel(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int set_idb,
    int set_mmu )
{
     ...
     * 5 : 100G
     */
    int prev_num_grp[FIREBOLT6_PIPES_PER_DEV][FIREBOLT6_OVS_HPIPE_COUNT_PER_PIPE][7];
    int  new_num_grp[FIREBOLT6_PIPES_PER_DEV][FIREBOLT6_OVS_HPIPE_COUNT_PER_PIPE][7];
        ...
        for (hpipe = 0; hpipe < FIREBOLT6_OVS_HPIPE_COUNT_PER_PIPE; hpipe++) {
            ...
            for (ovs_class = 1; ovs_class < 7; ovs_class++) {
                if (new_num_grp[pipe][hpipe][ovs_class] <= prev_num_grp[
												pipe][hpipe][ovs_class]) {
                     soc_firebolt6_tdm_flexport_ovs_consolidate_per_speed(unit,
								port_schedule_state,
                                pipe,
                                hpipe,
                                ovs_class,
...
/*! @fn int soc_firebolt6_tdm_calculate_ovs_per_pipe(int unit,
 *              soc_port_schedule_state_t *port_schedule_state,
 ...
 */
int
soc_firebolt6_tdm_calculate_ovs_per_pipe(
    ...
    int pipe, int prev_or_new)
{
    ...
    int pipe_ovs_slots;
    int hpipe_ovs_slots[FIREBOLT6_OVS_HPIPE_COUNT_PER_PIPE];
    soc_port_map_type_t *port_map;
    int pipe_ovs_ratio; /* per PIPE ovs_ratio */
    int hpipe_ovs_ratio[FIREBOLT6_OVS_HPIPE_COUNT_PER_PIPE]; /* per Half PIPE ovs_ratio */
    ...
    num_ancl_ports = (FB6_NUM_ANCL_1001_1250);
 ...
 */
int
soc_firebolt6_tdm_calculate_ovs(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        prev_or_new)
{
    int pipe;
    for (pipe = 0; pipe < FIREBOLT6_PIPES_PER_DEV; pipe++) {
        soc_firebolt6_tdm_calculate_ovs_per_pipe(unit, port_schedule_state,
                                                 pipe,
                                                 prev_or_new);
    }
    return SOC_E_NONE;
...
/**
@name: soc_firebolt6_tdm_calculation_flexport
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 *      SOC_E_*
 */
int soc_firebolt6_tdm_calculation_flexport(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    int port, pm_num;
    int pipe, hpipe;
    ...
    soc_firebolt6_tdm_set_out_map(unit, port_schedule_state);

  	/* Compute OVS ratios before FlexPort*/
    soc_firebolt6_tdm_calculate_ovs(unit, port_schedule_state, 0);
    ...
    soc_firebolt6_set_tdm_soc_pkg(unit, port_schedule_state, &soc_pkg, 1);
    ...
    if (1 == port_schedule_state->is_flexport) {
        soc_firebolt6_set_tdm_soc_pkg(unit, port_schedule_state,
					&_tdm_pkg->_prev_chip_data.soc_pkg, 0);
        /* Allocate memeory for previous TDM tables in _tdm_pkg */
        tdm_fb6_alloc_prev_chip_data(_tdm_pkg);
        soc_firebolt6_tdm_copy_prev_tables(unit, port_schedule_state, _tdm_pkg);
    }

    /* Generate the new config */
    _tdm_pkg = _soc_set_tdm_tbl(_tdm_pkg);

    /* Free the memory allocated in soc_firebolt6_set_tdm_soc_pkg calls */
    sal_free(soc_pkg.state);
    ...
    for (cal_id = 0; cal_id < FIREBOLT6_PIPES_PER_DEV; cal_id++) {
        switch (cal_id) {
            case 0:
        ...
        sal_memcpy(port_schedule_state->tdm_ingress_schedule_pipe[
                   ...
                   tdm_idb_pipe_main, sizeof(int)*FIREBOLT6_TDM_LENGTH);

        /* MMU TDM main calendar */
        sal_memcpy(port_schedule_state->tdm_egress_schedule_pipe[
					cal_id].tdm_schedule_slice[0].linerate_schedule,
                   tdm_mmu_pipe_main, sizeof(int)*FIREBOLT6_TDM_LENGTH);

        /* IDB and MMU OverSub Groups */
        for (index = 0; index < FIREBOLT6_OVS_GROUP_COUNT_PER_PIPE; index++) {
            /* Group 0:5 -> hpipe 0, Group 6:11 -> hpipe1 */
            hpipe = index / FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE;
            cal_grp = index % FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE;
            sal_memcpy(port_schedule_state->tdm_egress_schedule_pipe[
        ...
        for (hpipe = 0; hpipe < 2; hpipe ++) {
            sal_memcpy(port_schedule_state->tdm_ingress_schedule_pipe[
                    ...
                    tdm_pipe_group[hpipe+FB6_SHAPING_GRP_IDX_0], sizeof(int)
					* FB6_SHAPING_GRP_LEN);
        }
    }
    /*
    ...
    for (pm_num = 0; pm_num < FB6_NUM_PHY_PM; pm_num++) { /* FIREBOLT6_PBLKS_PER_PIPE * FIREBOLT6_PIPES_PER_DEV */
        for (port = 0; port < FIREBOLT6_PORTS_PER_PBLK; port++) {
            port_schedule_state->out_port_map.port_p2PBLK_inst_mapping[
				(pm_num * FIREBOLT6_PORTS_PER_PBLK) + 1 + port] =
                _tdm_pkg->_chip_data.soc_pkg.soc_vars.fb6.pm_num_to_pblk[pm_num];
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "PORT BLK DEBUG 2 pm_num %d : pm_num_to_pblk %d:"), pm_num,  _tdm_pkg->_chip_data.soc_pkg.soc_vars.fb6.pm_num_to_pblk[pm_num] ));
        }
    }
    ...
    soc_firebolt6_tdm_calculate_ovs(unit, port_schedule_state, 1);


    for (pipe = 0; pipe < FIREBOLT6_PIPES_PER_DEV; pipe++) {
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "Pipe %d idb_tdm:"), pipe));
        for (index = 0; index < FIREBOLT6_TDM_LENGTH; index++) {
            if (index % 16 == 0) {
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                                    "\n    ")));
            }
        ...
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "\n")));
        for (hpipe = 0; hpipe < FIREBOLT6_OVS_HPIPE_COUNT_PER_PIPE; hpipe++) {
            for (group = 0; group < FIREBOLT6_OVS_GROUP_COUNT_PER_HPIPE;
                    group++) {
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "Pipe %d hpipe %d group %d ovs_tdm"),
                            pipe, hpipe, group));
                for (index = 0; index < FIREBOLT6_OVS_GROUP_TDM_LENGTH;
                        index++) {
                    if (index == 0) {
                        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                                            "\n    ")));
	...
	_soc_fb6_tdm_print_schedule_state(unit, port_schedule_state);
...
/**
@name: soc_firebolt6_tdm_flexport_remove_ports
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
 ...
 */
int
soc_firebolt6_tdm_flexport_remove_ports(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    return (soc_firebolt6_tdm_flexport_remove_ports_sel(
...
/**
@name: soc_firebolt6_tdm_flexport_ovs_consolidate
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 */
int
soc_firebolt6_tdm_flexport_ovs_consolidate(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    return (soc_firebolt6_tdm_flexport_ovs_consolidate_sel(
				unit, port_schedule_state, 1, 1));
}


/**
@name: soc_firebolt6_tdm_flexport_add_ports
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 */
int
soc_firebolt6_tdm_flexport_add_ports(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    return (soc_firebolt6_tdm_flexport_add_ports_sel(unit,
				port_schedule_state, 1, 1));
}

/*! @fn int soc_firebolt6_tdm_flexport_idb(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_firebolt6_tdm_flexport_idb(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    SOC_IF_ERROR_RETURN(soc_firebolt6_tdm_flexport_remove_ports_sel(unit,
                                                   port_schedule_state, 1, 0));
    SOC_IF_ERROR_RETURN(soc_firebolt6_tdm_flexport_ovs_consolidate_sel(unit,
                                                   port_schedule_state, 1, 0));
    SOC_IF_ERROR_RETURN(soc_firebolt6_tdm_flexport_add_ports_sel(unit,
                                                   port_schedule_state, 1, 0));
    return SOC_E_NONE;
}

/*! @fn int soc_firebolt6_tdm_flexport_mmu(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_firebolt6_tdm_flexport_mmu(int                        unit,
                               soc_port_schedule_state_t *port_schedule_state)
{
    SOC_IF_ERROR_RETURN(soc_firebolt6_tdm_flexport_remove_ports_sel(unit,
...
/**
@name: tdm_fb6_alloc_prev_chip_data
@param:
...
tdm_fb6_alloc_prev_chip_data( tdm_mod_t *_tdm_pkg )
{
    ...
    _tdm_pkg->_prev_chip_data.cal_0.cal_main=(int *) TDM_ALLOC(
				(FB6_LR_VBS_LEN)*sizeof(int), "OLD TDM inst 0 main calendar");
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp=(int **) TDM_ALLOC(
				(FB6_OS_VBS_GRP_NUM+2)*sizeof(int *), "OLD TDM inst 0 groups");
    for (index=0; index<(FB6_OS_VBS_GRP_NUM); index++) {
        _tdm_pkg->_prev_chip_data.cal_0.cal_grp[index]=
			(int *) TDM_ALLOC((FB6_OS_VBS_GRP_LEN)*sizeof(int),
				"OLD TDM inst 0 group calendars");
    }
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp[FB6_SHAPING_GRP_IDX_0]=
			(int *) TDM_ALLOC((FB6_SHAPING_GRP_LEN)*sizeof(int),
			"OLD TDM inst 0 shaping calendars");
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp[FB6_SHAPING_GRP_IDX_1]=
			(int *) TDM_ALLOC((FB6_SHAPING_GRP_LEN)*sizeof(int),
			"OLD TDM inst 0 shaping calendars");
    /* Pipe 1 calendar group */
    _tdm_pkg->_prev_chip_data.cal_1.cal_main=
			(int *) TDM_ALLOC((FB6_LR_VBS_LEN)*sizeof(int),
			"OLD TDM inst 1 main calendar");
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp=
			(int **) TDM_ALLOC((FB6_OS_VBS_GRP_NUM+2)*sizeof(int *),
    ...
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp[FB6_SHAPING_GRP_IDX_0]=
			(int *) TDM_ALLOC((FB6_SHAPING_GRP_LEN)*sizeof(int),
			"OLD TDM inst 1 shaping calendars");
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp[FB6_SHAPING_GRP_IDX_1]=(
			int *) TDM_ALLOC((FB6_SHAPING_GRP_LEN)*sizeof(int),
			"OLD TDM inst 1 shaping calendars");
