...
do_dpp_show_stg_stp(int unit, bcm_stg_t stg)
{
    ...
    /* coverity explanation: coverity has found that port is between 0 and 616 (inclusive), Overrunning array pcfg.port.pbits of 18 4-byte elements at element index 19 (byte offset 76) using index port / 32 (which evaluates to 19) */
    /* However, SOC_DPORT_MAX is max at 571 instead of 617, so there will not be overrunning */
    /* Conclusion: We won't get overrunning here */
