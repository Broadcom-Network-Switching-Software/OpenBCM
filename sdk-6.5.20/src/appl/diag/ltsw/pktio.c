...
static ltsw_pwu_t pw_units[BCM_MAX_NUM_UNITS];
...
tx_parse(int u, args_t *a, xd_t *xd)
{
    ...
    parse_table_add(&pt, "PrioInt",     PQ_DFL|PQ_INT8,  0,
                    &xd->xd_prio_int,   NULL);
    parse_table_add(&pt, "McQType",     PQ_DFL|PQ_BOOL, 0,
                    &xd->xd_mcq,        NULL);
    parse_table_add(&pt, "COSqueue",    PQ_DFL|PQ_INT8,  0,
                    &xd->xd_cos,        NULL);
...
trace_tx(int unit, args_t *a)
{
    ...
    parse_table_add(&pt, "PortBitMap", PQ_DFL|PQ_PBMP|PQ_BCM, 0, &tx_pbmp, NULL);
    parse_table_add(&pt, "DATA", PQ_DFL|PQ_STRING, 0, &data, NULL);
    ...
    BCM_PBMP_ITER(tx_pbmp, tx_port) {
        trace_pkt.port = tx_port;
        break;
    }

    trace_pkt.pkt = pkt_data;
    trace_pkt.len = (int)pkt_len;

    rv = bcm_pktio_trace_tx(unit, &trace_pkt);
    if (BCM_FAILURE(rv)) {
        goto error;
    }
...
trace_parse(int unit, args_t *a)
{
    ...
    parse_table_init(unit, &pt);
    parse_table_add(&pt, "PortBitMap", PQ_DFL|PQ_PBMP|PQ_BCM, 0, &pbmp, NULL);
    parse_table_add(&pt, "DOP", PQ_DFL|PQ_INT, 0, &dop, NULL);
    parse_table_add(&pt, "DATA", PQ_DFL|PQ_STRING, 0, &data, NULL);
    ...
    if (data) {
        buf_data = parse_data_packet_payload(unit, data, &buf_len);
    }

    parse_arg_eq_done(&pt);

    if (buf_data) {
        if (buf_len > BCM_PKTIO_TRACE_SIZE_BYTES) {
            trace_data.len = BCM_PKTIO_TRACE_SIZE_BYTES;
            sal_memcpy(&trace_data.buf, buf_data, BCM_PKTIO_TRACE_SIZE_BYTES);
        } else {
            trace_data.len = buf_len;
            sal_memcpy(&trace_data.buf, buf_data, buf_len);
        }
    } else {
        /* Collect buf data first */
        BCM_PBMP_ITER(pbmp, port) {
            break;
        }

        rv = bcm_pktio_trace_dop_data_collect(unit, port, dop, &trace_data);
        if (BCM_FAILURE(rv)) {
            goto error;
        }
    }
    ...
    /* Parse trace buffer */
    (void)bcmi_ltsw_pktio_trace_data_dump(unit, dop, &trace_data);
...
static void
ltsw_pw_pup_free(int unit, pup_t *pup)
{
    ltsw_pwu_t *pu = &pw_units[unit];
...
static void
ltsw_pw_exit(int unit, int stat)
{
    ltsw_pwu_t *pu = &pw_units[unit];
    int rv;
    ...
    if (pu->pu_log != NULL) {
        ...
        while (pu->pu_log != NULL) {
            ...
            ltsw_pw_pup_free(unit, pu->pu_log);
            pu->pu_log = pup;
        }
    }
...
static void
ltsw_ether_dump(int unit, char *pfx, uint8 *addr, int len, int offset)
{
    int i = 0, j;
...
static void
ltsw_pw_dump_packet(int unit, pup_t *pup, uint32 report)
{
    int rv = BCM_E_NONE;
    bcm_pktio_pkt_t *pkt;
    uint8 *data;
    ...
    if (report & REPORT_RAW) {
        ltsw_ether_dump(unit, pfx, data, len, 0);
    }

    cli_out("\n");

    if (report & REPORT_PMD) {
        (void)bcmi_ltsw_pktio_rx_dump(unit, pkt, pu->pu_pmd_op);
    }
...
static void
ltsw_pw_log_packet(int unit, pup_t *pup)
{
    ltsw_pwu_t *pu = &pw_units[unit];
...
static void
ltsw_pw_process_pending(int unit)
{
    ltsw_pwu_t *pu = &pw_units[unit];
    pup_t *pup;
    ...
    while (pup) {
        ...
        ltsw_pw_log_packet(unit, (pup_t *)pup);
        ltsw_pw_dump_packet(unit, (pup_t *)pup, pu->pu_report);
        pup = tp;
        pu->last_pkt_count++;
    }
}
...
ltsw_pw_thread(void *up)
{
    ...
    /*
     * No forwaqrding null.
     * If the allocation failed, the call to ltsw_pw_exit will end the process
     * and we sould never reach here.
     */
...
static int
ltsw_pw_start(int unit, int sync)
{
    ltsw_pwu_t *pu = &pw_units[unit];
    ...
    pu->pu_pid = sal_thread_create(pw_name[unit], SAL_THREAD_STKSZ, 100,
                                   ltsw_pw_thread, INT_TO_PTR(unit));
...
static int
ltsw_pw_stop(int unit, int sync)
{
    ltsw_pwu_t *pu = &pw_units[unit];
    int pw_retry = 1000;
    ...
    while (pu->pu_pid != SAL_THREAD_ERROR) {
        if (pw_retry-- == 0) {
            cli_out("ltsw_pw_stop: pw_thread did not exit\n");
            pu->pu_pid = SAL_THREAD_ERROR;
            break;
        }
        sal_usleep(10000);
...
static cmd_result_t
ltsw_pw_dump_log(int unit, int n)
{
    ltsw_pwu_t *pu = &pw_units[unit];
    pup_t *pup;
    int abs_n;
    ...
    if (n < 0) {/* Dump last N packets */
        for (pup = pu->pu_log; abs_n-- != 0; pup = pup->pup_next) {
            ltsw_pw_dump_packet(unit, pup, pu->pu_dump_options);
        }
...
cmd_result_t
cmd_ltsw_txn(int unit, args_t *a)
{
    xd_t *xd;
    cmd_result_t rv;
...
cmd_result_t
cmd_ltsw_trace(int unit, args_t *a)
{
    char *subcmd_s;
    ...
    if (sal_strcasecmp(subcmd_s, "tx") == 0) {
        return trace_tx(unit, a);
    }

    if (sal_strcasecmp(subcmd_s, "parse") == 0) {
        return trace_parse(unit, a);
    }
...
cmd_result_t
cmd_ltsw_pcktwatch(int unit, args_t *a)
{
    char *c;
    int n;
    uint32 on = 0;
    ...
    while ((c = ARG_GET(a)) != NULL) {
        if (!sal_strcasecmp(c, "start")) {
            ...
            return (ltsw_pw_dump_log(unit, n));
    ...
    if (on & PU_F_RUN) {
        return (ltsw_pw_start(unit, TRUE));
    } else if (on & PU_F_STOP) {
        return (ltsw_pw_stop(unit, TRUE));
    }
...
int
ltsw_pw_running(int unit)
{
    return pw_units[unit].pu_flags & PU_F_RUN;
}

int
ltsw_pw_clean(int unit)
{
    return ltsw_pw_stop(unit, TRUE);
}
