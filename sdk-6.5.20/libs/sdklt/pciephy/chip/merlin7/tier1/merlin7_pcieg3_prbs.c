...
err_code_t merlin7_pcieg3_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length, const char pattern[]) {
  ...
  EFUN(merlin7_pcieg3_INTERNAL_calc_patt_gen_mode_sel(sa__, &mode_sel,&zero_pad_len,patt_length));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        EFUN(merlin7_pcieg3_INTERNAL_compute_bin(sa__, pattern[i],bin));
...
err_code_t merlin7_pcieg3_tx_shared_patt_gen_en(srds_access_t *sa__, uint8_t enable, uint8_t patt_length) {
  uint8_t zero_pad_len = 0; /* suppress warnings, changed by merlin7_pcieg3_INTERNAL_calc_patt_gen_mode_sel() */
  uint8_t mode_sel     = 0; /* suppress warnings, changed by merlin7_pcieg3_INTERNAL_calc_patt_gen_mode_sel() */
...
err_code_t merlin7_pcieg3_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    ESTM_PRINTF(("  %d ",merlin7_pcieg3_get_lane(sa__)));
    ...
    if(enabled) {
        ...
        prbs_string = (char *)merlin7_pcieg3_e2s_prbs_mode_enum[prbs_poly_mode];
