/*! \file bcm56780_a0_fp_common_presel_qual_db.c
 *
...
STATIC int
bcm56780_a0_fp_presel_matchid_quals_init(int unit,
                                         bcmfp_stage_t *stage)
{
    bcmltd_fid_t fid = 0;
...
STATIC int
bcm56780_a0_fp_presel_fixed_qual_init(int unit,
                                      bcmfp_stage_t *stage,
                                      uint32_t fid)
{
    bool unmapped = false;
                                       ...
                                       bcmfp_stage_t *stage)
{
    ...
    for (qual_idx = 0; qual_idx < pcm_info->field_count; qual_idx++) {
        ...
        if (qual_cont_info == NULL) {
             ...
             * in bcm56780_a0_fp_presel_matchid_quals_init function.
             */
            if (fid == stage->tbls.pse_tbl->drop_decision_fid) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcm56780_a0_fp_presel_fixed_qual_init(unit, stage, fid));
            }
            continue;
        }
        BCMFP_KEYGEN_QUAL_CFG_INIT(unit);
    ...
    (void) bcm56780_a0_fp_presel_matchid_quals_init(unit, stage);
exit:
    SHR_FUNC_EXIT();
}
