/*! \file bcmfp_compression_id_mgmt.c
 *
...
STATIC int
bcmfp_update_cids(int unit, bcmfp_shr_trie_md_t *trie,
                  ...
                  bcmfp_cid_update_list_t *list)
{
    ...
    bcmfp_cid_update_list_t *list_node, *list_iter;
    ...
    for (prefix = parent_node->prefix + 1;
        ...
        while (curr_node) {
            ...
            list_node = NULL;
            BCMFP_COMP_ALLOC(list_node, sizeof(bcmfp_cid_update_list_t),
                             "bcmfpCidUpdateListNode");
            sal_memcpy(&list_node->key, &curr_node->key, sizeof(bcmfp_key_t));
            ...
            if (!SHR_BITEQ_RANGE(list_node->old_cid.w, curr_node->cid.w,
                ...
                !SHR_BITEQ_RANGE(list_node->old_cid_mask.w, curr_node->cid_mask.w,
                                 0, trie->max_cid_mask) ||
                (curr_node->flags & BCMFP_TRIE_NODE_WIDTH_COMPRESSED)) {
                /*
                ...
                sal_memcpy(&list_node->new_cid_mask, &curr_node->cid_mask,
                           sizeof(bcmfp_cid_t));
                if ((curr_node->flags & BCMFP_TRIE_NODE_WIDTH_COMPRESSED) &&
                     ...
                     */
                    list_node->flags = BCMFP_TRIE_NODE_WIDTH_COMPRESSED;
                }
            ...
            rv = (bcmfp_update_cids(unit, trie, curr_node, list));
            if (SHR_FAILURE(rv)) {
                return (rv);
            }
...
{
    bcmfp_cid_update_list_t list;
    bcmfp_cid_update_list_t *list_iter, *list_tmp_node, *list_node, *list_iter_new;
    bcmfp_compress_key_t *curr_node;
    int rv;
    ...
    rv = (bcmfp_update_cids(unit, trie, parent_node, &list));
    if (SHR_FAILURE(rv)) {
        goto exit;
    }
