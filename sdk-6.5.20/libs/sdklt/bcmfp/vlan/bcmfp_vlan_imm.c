 ...
 */
static int
bcmfp_vlan_grp_config_stage(int unit,
                               bcmltd_sid_t sid,
 ...
 */
static int
bcmfp_vlan_rule_config_stage(int unit,
                                bcmltd_sid_t sid,
                                const bcmltd_op_arg_t *op_arg,
                                bcmimm_entry_event_t event_reason,
                                const bcmltd_field_t *key,
 ...
 */
static int
bcmfp_vlan_policy_config_stage(int unit,
                                  bcmltd_sid_t sid,
                                  const bcmltd_op_arg_t *op_arg,
                                  bcmimm_entry_event_t event_reason,
                                  const bcmltd_field_t *key,
 ...
 */
static int
bcmfp_vlan_entry_config_stage(int unit,
                                 bcmltd_sid_t sid,
                                 const bcmltd_op_arg_t *op_arg,
                                 bcmimm_entry_event_t event_reason,
                                 const bcmltd_field_t *key,
 ...
 */
static int
bcmfp_vlan_entry_lookup(int unit,
                           bcmltd_sid_t sid,
                           const bcmltd_op_arg_t *op_arg,
                           void *context,
                           ...
                           bcmltd_fields_t *out)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmfp_ltd_buffers_clear(unit, BCMFP_STAGE_ID_LOOKUP));
 ...
 */
static int
bcmfp_vlan_pse_lookup(int unit,
                      bcmltd_sid_t sid,
                      const bcmltd_op_arg_t *op_arg,
                      void *context,
                      bcmimm_lookup_type_t lkup_type,
 ...
 */
static int
bcmfp_vlan_pse_config_stage(int unit,
                                  bcmltd_sid_t sid,
                                  const bcmltd_op_arg_t *op_arg,
                                  bcmimm_entry_event_t event_reason,
                                  const bcmltd_field_t *key,
 ...
 */
static int
bcmfp_vlan_psg_config_stage(int unit,
                                  bcmltd_sid_t sid,
                                  const bcmltd_op_arg_t *op_arg,
                                  bcmimm_entry_event_t event_reason,
                                  const bcmltd_field_t *key,
 ...
 */
static int
bcmfp_vlan_pdd_config_stage(int unit,
                               bcmltd_sid_t sid,
                               const bcmltd_op_arg_t *op_arg,
                               bcmimm_entry_event_t event_reason,
                               const bcmltd_field_t *key,
 ...
 */
static int
bcmfp_vlan_sbr_config_stage(int unit,
                               bcmltd_sid_t sid,
                               const bcmltd_op_arg_t *op_arg,
                               bcmimm_entry_event_t event_reason,
                               const bcmltd_field_t *key,
 ...
 */
static int
bcmfp_vlan_src_class_stage(int unit,
                                   bcmltd_sid_t sid,
                                   const bcmltd_op_arg_t *op_arg,
                                   bcmimm_entry_event_t event_reason,
                                   const bcmltd_field_t *key,
...
static int
bcmfp_vlan_range_check_group_stage(int unit,
                                   bcmltd_sid_t sid,
                                   const bcmltd_op_arg_t *op_arg,
                                   bcmimm_entry_event_t event_reason,
                                   const bcmltd_field_t *key,
...
/*!
 * \brief FP vlan group LT entry IMM event callback structure.
 *
 * This structure contains callback functions that will be conresponding
 * to FP vlan group LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_grp_config_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_grp_config_stage,

  /*! Commit function. */
  .commit = bcmfp_vlan_trans_commit,

  /*! Abort function. */
  .abort = bcmfp_vlan_trans_abort
};

/*!
 * \brief FP vlan rule LT IMM event callback structure.
 *
 * This structure contains callback functions that will be conresponding
 * to FP vlan rule LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_rule_config_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_rule_config_stage,
...
/*!
 * \brief FP vlan policy LT IMM event callback structure.
 *
 * This structure contains callback functions that will be conresponding
 * to FP vlan policy LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_policy_config_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_policy_config_stage,
...
/*!
 * \brief FP vlan entry LT IMM event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to FP vlan entry LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_entry_config_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_entry_config_stage,
  ...
  /*! Lookup function. */
  .op_lookup = bcmfp_vlan_entry_lookup
};

/*!
 * \brief FP vlan presel entry LT IMM event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to FP vlan presel entry LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_pse_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_pse_config_stage,
  ...
  .abort = bcmfp_vlan_trans_abort,
   ...
   /*! Lookup function. */
  .op_lookup = bcmfp_vlan_pse_lookup
};

/*!
 * \brief FP vlan presel group LT IMM event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to FP vlan presel group LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_psg_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_psg_config_stage,
...
/*!
 * \brief FP vlan PDD LT IMM event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to FP vlan PDD LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_pdd_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_pdd_config_stage,
...
/*!
 * \brief FP vlan SBR LT IMM event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to FP vlan SBR LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_sbr_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_sbr_config_stage,
...
/*!
 * \brief FP vlan source class mode LT IMM event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to FP vlan source class mode LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_src_class_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_src_class_stage,
...
/*!
 * \brief FP vlan range check group LT IMM event callback structure.
 *
 * This structure contains callback functions that will be corresponding
 * to FP vlan range check group LT entry commit stages.
 */
static bcmimm_lt_cb_t bcmfp_vlan_range_check_group_imm_callback = {

  /*! Validate function. */
  .validate = NULL,

  /*! Staging function. */
  .op_stage = bcmfp_vlan_range_check_group_stage,
...
bcmfp_vlan_imm_register(int unit)
{
    ...
    /* Register callbacks for vlan group LT. */
    if (stage->tbls.group_tbl != NULL) {
        tbl_id = stage->tbls.group_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_grp_config_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan rule LT. */
    if (stage->tbls.rule_tbl != NULL) {
        tbl_id = stage->tbls.rule_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_rule_config_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan policy LT. */
    if (stage->tbls.policy_tbl != NULL) {
        tbl_id = stage->tbls.policy_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_policy_config_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan entry LT. */
    if (stage->tbls.entry_tbl != NULL) {
        tbl_id = stage->tbls.entry_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_entry_config_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan presel entry LT. */
    if (stage->tbls.pse_tbl != NULL) {
        tbl_id = stage->tbls.pse_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_pse_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan presel group LT. */
    if (stage->tbls.psg_tbl != NULL) {
        tbl_id = stage->tbls.psg_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_psg_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan pdd LT. */
    if (stage->tbls.pdd_tbl != NULL) {
        tbl_id = stage->tbls.pdd_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_pdd_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan sbr LT. */
    if (stage->tbls.sbr_tbl != NULL) {
        tbl_id = stage->tbls.sbr_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_sbr_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan source class mode LT. */
    if (stage->tbls.src_class_tbl != NULL) {
        tbl_id = stage->tbls.src_class_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_src_class_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }

    /* Register callbacks for vlan range check group LT. */
    if (stage->tbls.range_check_group_tbl != NULL) {
        tbl_id = stage->tbls.range_check_group_tbl->sid;
        rv = bcmlrd_map_get(unit, tbl_id, &map);
        if (SHR_SUCCESS(rv)) {
            cb = &bcmfp_vlan_range_check_group_imm_callback;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmimm_lt_event_reg(unit, tbl_id, cb, NULL));
        }
    }
