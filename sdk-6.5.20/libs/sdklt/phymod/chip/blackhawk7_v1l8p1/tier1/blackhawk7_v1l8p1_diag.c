...
static err_code_t blackhawk7_v1l8p1_eye_scan_setup(srds_access_t *sa__, uint8_t intr_pass_b, int16_t *y_max, int16_t *y_step) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_v1l8p1_display_eye_scan(srds_access_t *sa__) {
    ...
    EFUN(blackhawk7_v1l8p1_eye_scan_setup(sa__, 0, &y_max, &y_step));
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step))
    {
        ...
        if(enable_read)
        {   err_code_t err_code = blackhawk7_v1l8p1_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (err_code) {
                EFUN(blackhawk7_v1l8p1_meas_eye_scan_done(sa__));
            }
        }
...
err_code_t blackhawk7_v1l8p1_INTERNAL_poll_diag_data(srds_access_t *sa__, const srds_info_t *blackhawk7_v1l8p1_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_v1l8p1_display_eye_scan_header(srds_access_t *sa__, int8_t i) {
    INIT_SRDS_ERR_CODE
    int8_t x;
...
err_code_t blackhawk7_v1l8p1_display_eye_scan_footer(srds_access_t *sa__, int8_t i) {
    INIT_SRDS_ERR_CODE
    int8_t x;
    uint8_t stepsize_var[NUM_LANES_MAX];
    uint8_t lane, num_lanes;
...
Exit:
    ...
    if(diag_level & SRDS_DIAG_TIMESTAMP) {
        EFUN(blackhawk7_v1l8p1_test_uc_timestamp(sa__));
    }
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        ...
        for (; micro_num<blackhawk7_v1l8p1_info_ptr->micro_count; ++micro_num)
        {
            ...
            EFUN(blackhawk7_v1l8p1_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, blackhawk7_v1l8p1_INTERNAL_event_log_dump_callback));
            EFUN(blackhawk7_v1l8p1_INTERNAL_event_log_dump_callback(sa__, &state, 0, 0));
        }
    }
    if(diag_level & SRDS_DIAG_EVENT) {
        EFUN(blackhawk7_v1l8p1_read_event_log(sa__));
    }
    ...
    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(blackhawk7_v1l8p1_reg_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(blackhawk7_v1l8p1_uc_core_var_dump(sa__, core, NULL));
        ...
        for (srds_lane = 0; srds_lane < num_micros*2; srds_lane =(uint8_t)(srds_lane + 2)) {
            ...
            EFUN(blackhawk7_v1l8p1_uc_micro_var_dump(sa__, core, srds_lane, NULL));
        }
    ...
    if(diag_level & SRDS_DIAG_UC_LANE) {
        ...
        EFUN(blackhawk7_v1l8p1_uc_lane_var_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(blackhawk7_v1l8p1_display_eye_scan(sa__));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(blackhawk7_v1l8p1_display_lane_debug_status(sa__));
    }
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        EFUN(blackhawk7_v1l8p1_INTERNAL_display_info_table(sa__));
    }
    /* re enable micro */
    if (!micro_stop) {
