...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk7_v1l8p1_internal.c                                         *
...
/** @file blackhawk7_v1l8p1_internal.c
...
extern err_code_t blackhawk7_v1l8p1_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl);
extern err_code_t blackhawk7_v1l8p1_get_BER_data_HW_mode(srds_access_t *sa__, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl);


#if !defined(SERDES_EXTERNAL_INFO_TABLE_EN)
static srds_info_t blackhawk7_v1l8p1_info;
#endif

srds_info_t *blackhawk7_v1l8p1_INTERNAL_get_blackhawk7_v1l8p1_info_ptr(srds_access_t *sa__) {
#if defined(SERDES_EXTERNAL_INFO_TABLE_EN)
    return blackhawk7_v1l8p1_acc_get_info_table_address(sa__);
#else
    UNUSED(sa__);
    return &blackhawk7_v1l8p1_info;
#endif
}
...
srds_info_t *blackhawk7_v1l8p1_INTERNAL_get_blackhawk7_v1l8p1_info_ptr_with_check(srds_access_t *sa__) {
    ...
    if (blackhawk7_v1l8p1_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        err_code = blackhawk7_v1l8p1_init_blackhawk7_v1l8p1_info(sa__);
    }
    if (err_code != ERR_CODE_NONE) {
        EFUN_PRINTF(("ERROR: Serdes Info pointer not initialized correctly\n"));
    }
    return blackhawk7_v1l8p1_info_ptr;
}

err_code_t blackhawk7_v1l8p1_INTERNAL_match_ucode_from_info(srds_access_t *sa__, srds_info_t const *blackhawk7_v1l8p1_info_ptr) {
    INIT_SRDS_ERR_CODE
    ...
    if (ucode_version == blackhawk7_v1l8p1_info_ptr->ucode_version) {
        return(ERR_CODE_NONE);
    } else {
        EFUN_PRINTF(("ERROR:  ucode version of the current thread not matching with stored blackhawk7_v1l8p1_info->ucode_version, Expected 0x%08X, but received 0x%08X.\n",
                    blackhawk7_v1l8p1_info_ptr->ucode_version, ucode_version));
        return(ERR_CODE_UCODE_VERIFY_FAIL);
    }
}

/* This function is getting deprecated.
   Please use blackhawk7_v1l8p1_INTERNAL_get_blackhawk7_v1l8p1_info_ptr_with_check and blackhawk7_v1l8p1_INTERNAL_match_ucode_from_info instead */
err_code_t blackhawk7_v1l8p1_INTERNAL_verify_blackhawk7_v1l8p1_info(srds_access_t *sa__, srds_info_t const *blackhawk7_v1l8p1_info_ptr) {
    ...
    if (blackhawk7_v1l8p1_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        EFUN_PRINTF(("ERROR:  Mismatch in blackhawk7_v1l8p1_info signature.  Expected 0x%08X, but received 0x%08X.\n",
                    SRDS_INFO_SIGNATURE, blackhawk7_v1l8p1_info_ptr->signature));
        return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_INFO_TABLE_ERROR));
    }
...
static uint32_t blackhawk7_v1l8p1_osr_mode_enum_to_int_x1000(uint8_t);
static uint32_t blackhawk7_v1l8p1_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    switch(osr_mode) {
    case BLACKHAWK7_V1L8P1_OSX1:      return 1000;
    case BLACKHAWK7_V1L8P1_OSX2:      return 2000;
    case BLACKHAWK7_V1L8P1_OSX4:      return 4000;
    case BLACKHAWK7_V1L8P1_OSX8:      return 8000;
    case BLACKHAWK7_V1L8P1_OSX16P5:   return 16500;
    case BLACKHAWK7_V1L8P1_OSX20P625: return 20625;
    case BLACKHAWK7_V1L8P1_OSX16:     return 16000;
    case BLACKHAWK7_V1L8P1_OSX32:     return 32000;
    case BLACKHAWK7_V1L8P1_OSX21P25:  return 21250;
    case BLACKHAWK7_V1L8P1_OSX2P5:    return 2500;

    default:                    return 1000;
    }
}

err_code_t blackhawk7_v1l8p1_INTERNAL_get_num_bits_per_ms(srds_access_t *sa__, uint8_t select_rx, uint32_t *num_bits_per_ms) {
    INIT_SRDS_ERR_CODE
    uint8_t osr_mode = 0;
    ...
    {
        blackhawk7_v1l8p1_osr_mode_st osr_mode_st;
        ENULL_MEMSET(&osr_mode_st, 0, sizeof(blackhawk7_v1l8p1_osr_mode_st));
        EFUN(blackhawk7_v1l8p1_INTERNAL_get_osr_mode(sa__, &osr_mode_st));
        if (select_rx) {
    ...
    ESTM(prev_pll = blackhawk7_v1l8p1_acc_get_pll_idx(sa__));
    pll_select = 0;
    EFUN(blackhawk7_v1l8p1_acc_set_pll_idx(sa__,pll_select));

        EFUN(blackhawk7_v1l8p1_get_uc_core_config(sa__, &core_config));
        *num_bits_per_ms = (uint32_t)((((uint32_t)core_config.vco_rate_in_Mhz * 100000UL) / blackhawk7_v1l8p1_osr_mode_enum_to_int_x1000(osr_mode))*10);

#if defined(rd_rx_pam4_mode)
    if(pam4_mode > 0) *num_bits_per_ms <<= 1;
#endif
    EFUN(blackhawk7_v1l8p1_acc_set_pll_idx(sa__,prev_pll));
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_display_BER(srds_access_t *sa__, uint16_t time_ms) {
    INIT_SRDS_ERR_CODE
    char string[32];
      EFUN(blackhawk7_v1l8p1_INTERNAL_get_BER_string(sa__,time_ms,string, sizeof(string)));
      USR_PRINTF(("%s",string));
    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p1_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string, uint8_t string_size) {
    char string2[4];
    char string_extra[3] = "";
    ...
    err = blackhawk7_v1l8p1_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local, USE_HW_TIMERS);
    ...
    if(err == ERR_CODE_PRBS_CHK_DISABLED) {
        ...
        return blackhawk7_v1l8p1_error(sa__, err);
    }
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, enum blackhawk7_v1l8p1_prbs_chk_timer_selection_enum timer_sel) {
    ...
    if(timer_sel == USE_HW_TIMERS) {
        ...
        err_code_t err = ERR_CODE_NONE;
        err = blackhawk7_v1l8p1_config_BER_HW_mode(sa__, time_ms, ber_data, &prbs_chk_hw_timer_ctrl_bak);
        if(err != ERR_CODE_NONE) {
            return err;
        }
        EFUN(blackhawk7_v1l8p1_get_BER_data_HW_mode(sa__, ber_data, &prbs_chk_hw_timer_ctrl_bak));
    } else
    {
        uint8_t clk_gate = 0;
        UNUSED(timer_sel);
        ...
        EFUN(blackhawk7_v1l8p1_prbs_err_count_state(sa__, &err_cnt,&lcklost)); /* clear error counters */
...
err_code_t blackhawk7_v1l8p1_INTERNAL_pam4_to_bin(srds_access_t *sa__, char var, char bin[]) {
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_CFG_PATT_INVALID_PAM4));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_compute_bin(srds_access_t *sa__, char var, char bin[]) {
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_CFG_PATT_INVALID_HEX));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_compute_hex(srds_access_t *sa__, char bin[],uint8_t *hex) {
  ...
  if (!USR_STRCMP(bin,"0000")) {
    ...
    *hex = 0x0;
    return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_CFG_PATT_INVALID_BIN2HEX));
  }
  return (ERR_CODE_NONE);
}
...
uint8_t blackhawk7_v1l8p1_INTERNAL_stop_micro(srds_access_t *sa__, uint8_t graceful, err_code_t *err_code_p) {
   ...
   if (!(stop_state & 0x7F)) {
       if (graceful) {
           EPFUN2((blackhawk7_v1l8p1_stop_rx_adaptation(sa__, 1)),0xFF);
       }
       else {
           EPFUN2((blackhawk7_v1l8p1_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_IMMEDIATE,GRACEFUL_STOP_TIME)),0xFF);
       }
   }
...
uint8_t blackhawk7_v1l8p1_INTERNAL_rdb_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint32_t addr) {
    INIT_SRDS_ERR_CODE
    uint8_t rddata;
    ...
    EPSTM(rddata = blackhawk7_v1l8p1_rdb_uc_ram(sa__, err_code_p, addr)); /* Use Micro register interface for reading RAM */
    return (rddata);
}

/* Micro Global RAM Word Read */
uint16_t blackhawk7_v1l8p1_INTERNAL_rdw_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint32_t addr) {
  uint16_t rddata;
  INIT_SRDS_ERR_CODE
  ...
  EPSTM(rddata = blackhawk7_v1l8p1_rdw_uc_ram(sa__, err_code_p, addr)); /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro Global RAM Byte Write  */
err_code_t blackhawk7_v1l8p1_INTERNAL_wrb_uc_var(srds_access_t *sa__, uint32_t addr, uint8_t wr_val) {

    return (blackhawk7_v1l8p1_wrb_uc_ram(sa__, addr, wr_val));          /* Use Micro register interface for writing RAM */
}

/* Micro Global RAM Word Write  */
err_code_t blackhawk7_v1l8p1_INTERNAL_wrw_uc_var(srds_access_t *sa__, uint32_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                       /* Validate even address */
      USR_PRINTF(("Error incorrect addr x%04x\n",addr));
        return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_INVALID_RAM_ADDR));
    }
    return (blackhawk7_v1l8p1_wrw_uc_ram(sa__, addr, wr_val));          /* Use Micro register interface for writing RAM */
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_event_log_dump_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk7_v1l8p1_INTERNAL_event_log_dump_state_t * const state_ptr = (blackhawk7_v1l8p1_INTERNAL_event_log_dump_state_t *)arg;
...
err_code_t blackhawk7_v1l8p1_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
    ...
    srds_info_t const * const blackhawk7_v1l8p1_info_ptr = blackhawk7_v1l8p1_INTERNAL_get_blackhawk7_v1l8p1_info_ptr_with_check(sa__);
    uint16_t rd_idx;
    uint8_t current_lane;

    EFUN(blackhawk7_v1l8p1_INTERNAL_match_ucode_from_info(sa__, blackhawk7_v1l8p1_info_ptr));

    if (micro_num >= blackhawk7_v1l8p1_info_ptr->micro_count) {
        return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    /* Read Current lane so that it can be restored at the end of function */
    current_lane = blackhawk7_v1l8p1_acc_get_lane(sa__);
    /* Blackhawk/Osprey has 2 lanes per micro; Set lane appropriate for the desired micro */
    EFUN(blackhawk7_v1l8p1_acc_set_physical_lane(sa__,(uint8_t)(micro_num<<1)));
    EFUN_PRINTF(("\n\n********************************************\n"));
    ...
    if (bypass_micro) {
        ...
        if (blackhawk7_v1l8p1_info_ptr->trace_memory_descending_writes) {
            ++rd_idx;
            if (rd_idx >= blackhawk7_v1l8p1_info_ptr->trace_mem_ram_size) {
                rd_idx = 0;
            }
        } else {
            if (rd_idx == 0) {
                rd_idx = (uint16_t)blackhawk7_v1l8p1_info_ptr->trace_mem_ram_size;
            }
            --rd_idx;
        }
    } else {
        /* Start Read to stop uC logging and read the word at last event written by uC */
        EFUN(blackhawk7_v1l8p1_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, GRACEFUL_STOP_TIME));
        ESTM(rd_idx = rducv_trace_mem_rd_idx());
    }

    EFUN_PRINTF(( "\n  DEBUG INFO: trace memory read index = 0x%04x\n", rd_idx));

    EFUN_PRINTF(("  DEBUG INFO: trace memory size = 0x%04x\n\n", blackhawk7_v1l8p1_info_ptr->trace_mem_ram_size));

    if (blackhawk7_v1l8p1_info_ptr->trace_memory_descending_writes) {
        /* Micro writes trace memory using descending addresses.
         * So read using ascending addresses using block read
         */
        EFUN(blackhawk7_v1l8p1_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                  blackhawk7_v1l8p1_info_ptr->trace_mem_ram_base + (uint32_t)(blackhawk7_v1l8p1_info_ptr->grp_ram_size*micro_num),
                                                  (uint16_t)blackhawk7_v1l8p1_info_ptr->trace_mem_ram_size,
                                                  rd_idx,
                                                  (uint16_t)blackhawk7_v1l8p1_info_ptr->trace_mem_ram_size,
                                                  arg,
                                                  callback));
         ...
         * So read using ascending addresses using block read
         */
        EFUN(blackhawk7_v1l8p1_INTERNAL_rdblk_uc_generic_ram_descending(sa__,
                                                             blackhawk7_v1l8p1_info_ptr->trace_mem_ram_base + (uint32_t)(blackhawk7_v1l8p1_info_ptr->grp_ram_size*micro_num),
                                                             (uint16_t)blackhawk7_v1l8p1_info_ptr->trace_mem_ram_size,
                                                             rd_idx,
                                                             (uint16_t)blackhawk7_v1l8p1_info_ptr->trace_mem_ram_size,
    ...
    if (!bypass_micro) {
        ...
        EFUN(blackhawk7_v1l8p1_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 50));
    }
    EFUN(blackhawk7_v1l8p1_acc_set_lane(sa__,current_lane));
    return(ERR_CODE_NONE);
}  /* blackhawk7_v1l8p1_INTERNAL_read_event_log_with_callback() */
...
err_code_t blackhawk7_v1l8p1_INTERNAL_set_rx_pf_main(srds_access_t *sa__, uint8_t val) {
    ...
    if (val > pf_max) {
       return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_PF_INVALID));
    }
...
err_code_t blackhawk7_v1l8p1_INTERNAL_set_rx_vga(srds_access_t *sa__, uint8_t val) {
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_check_uc_lane_stopped(sa__));  /* make sure uC is stopped to avoid race conditions */

    if (val > RX_VGA_CTRL_VAL_MAX) {
      return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_VGA_INVALID));
    }
...
err_code_t blackhawk7_v1l8p1_INTERNAL_set_rx_dfe1(srds_access_t *sa__, int8_t val) {
    INIT_SRDS_ERR_CODE
    enum blackhawk7_v1l8p1_rx_pam_mode_enum pam_mode = NRZ;
    EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_rx_dfe1(srds_access_t *sa__, int8_t *val) {
    INIT_SRDS_ERR_CODE
    {
        ...
        if(pam_mode != NRZ) {
            EFUN_PRINTF(("ERROR: RX DFE1 is only available in NRZ mode (core %d lane %d)\n",blackhawk7_v1l8p1_acc_get_core(sa__),blackhawk7_v1l8p1_acc_get_lane(sa__)));
            return(ERR_CODE_INVALID_RX_PAM_MODE);
        }
    }
...
err_code_t blackhawk7_v1l8p1_INTERNAL_set_rx_dfe2(srds_access_t *sa__, int8_t val) {
    ...
    if ((val > 31) || (val < -31)) {
        ...
        return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_DFE_TAP));
    }
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_bwr(srds_access_t *sa__, uint8_t *vga1_bwr, uint8_t *vga2_bwr, uint8_t *eq1_bwr, uint8_t *eq2_bwr) {
  ...
  *eq1_bwr  = blackhawk7_v1l8p1_INTERNAL_gray_to_uint8(*eq1_bwr);
  *eq2_bwr  = blackhawk7_v1l8p1_INTERNAL_gray_to_uint8(*eq2_bwr);
  *vga1_bwr = blackhawk7_v1l8p1_INTERNAL_gray_to_uint8(*vga1_bwr);
  *vga2_bwr = blackhawk7_v1l8p1_INTERNAL_gray_to_uint8(*vga2_bwr);
  return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_set_bwr(srds_access_t *sa__, uint8_t vga1_bwr, uint8_t vga2_bwr, uint8_t eq1_bwr, uint8_t eq2_bwr, uint8_t vga_dis_ind) {
  INIT_SRDS_ERR_CODE
  eq1_bwr  = blackhawk7_v1l8p1_INTERNAL_uint8_to_gray(eq1_bwr);
  eq2_bwr  = blackhawk7_v1l8p1_INTERNAL_uint8_to_gray(eq2_bwr);
  vga1_bwr = blackhawk7_v1l8p1_INTERNAL_uint8_to_gray(vga1_bwr);
  vga2_bwr = blackhawk7_v1l8p1_INTERNAL_uint8_to_gray(vga2_bwr);
...
err_code_t blackhawk7_v1l8p1_INTERNAL_validate_full_txfir_cfg(srds_access_t *sa__, enum blackhawk7_v1l8p1_txfir_tap_enable_enum enable_taps, blackhawk7_v1l8p1_txfir_st *txfir) {
    ...
    in_nrz_range = ((enable_taps == BLACKHAWK7_V1L8P1_NRZ_LP_3TAP) || (enable_taps == BLACKHAWK7_V1L8P1_NRZ_6TAP)) ? 1 : 0;
    ...
    return (blackhawk7_v1l8p1_error(sa__, err_code));
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_apply_full_txfir_cfg(srds_access_t *sa__, enum blackhawk7_v1l8p1_txfir_tap_enable_enum enable_taps, blackhawk7_v1l8p1_txfir_st *txfir) {
    ...
    EFUN(wr_txfir_nrz_tap_range_sel(((enable_taps == BLACKHAWK7_V1L8P1_NRZ_LP_3TAP) || (enable_taps == BLACKHAWK7_V1L8P1_NRZ_6TAP)) ? 1 : 0));
    for (tap_num=0; tap_num<6; tap_num++) {
        EFUN(blackhawk7_v1l8p1_INTERNAL_set_tx_tap(sa__, tap_num, txfir->tap[tap_num]));
    }
    if ((enable_taps == BLACKHAWK7_V1L8P1_NRZ_6TAP) || (enable_taps == BLACKHAWK7_V1L8P1_PAM4_6TAP)) {
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_eye_margin_est(srds_access_t *sa__, uint16_t *left_eye_mUI, uint16_t *right_eye_mUI, uint16_t  *upper_eye_mV, uint16_t *lower_eye_mV) {
  ...
  ESTM(*left_eye_mUI  = blackhawk7_v1l8p1_INTERNAL_eye_to_mUI(sa__, eye_val));
  ESTM(eye_val = rdv_usr_sts_heye_right());
  ESTM(*right_eye_mUI = blackhawk7_v1l8p1_INTERNAL_eye_to_mUI(sa__, eye_val));
  ESTM(eye_val = rdv_usr_sts_veye_upper());
  ESTM(*upper_eye_mV = blackhawk7_v1l8p1_INTERNAL_eye_to_mV(sa__, eye_val, ladder_range));
  ESTM(eye_val = rdv_usr_sts_veye_lower());
  ESTM(*lower_eye_mV = blackhawk7_v1l8p1_INTERNAL_eye_to_mV(sa__, eye_val, ladder_range));
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_pam_eye_margin_est(srds_access_t *sa__, eye_margin_t *eye_margin) {
  ...
  for(i=0; i<NUM_PAM_EYES; i++) {
      ...
      ESTM(eye_margin->left_eye_mUI [i] = blackhawk7_v1l8p1_INTERNAL_eye_to_mUI(sa__, pam_eye_val));
      ESTM(pam_eye_val = rdv_usr_sts_pam_heye_right(i));
      ESTM(eye_margin->right_eye_mUI[i] = blackhawk7_v1l8p1_INTERNAL_eye_to_mUI(sa__, pam_eye_val));
      ESTM(pam_eye_val = rdv_usr_sts_pam_veye_upper(i));
      ESTM(eye_margin->upper_eye_mV [i] = blackhawk7_v1l8p1_INTERNAL_eye_to_mV (sa__, pam_eye_val, ladder_range));
      ESTM(pam_eye_val = rdv_usr_sts_pam_veye_lower(i));
      ESTM(eye_margin->lower_eye_mV [i] = blackhawk7_v1l8p1_INTERNAL_eye_to_mV (sa__, pam_eye_val, ladder_range));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_display_pam_eye_margin_est(srds_access_t *sa__) {
    ...
    enum blackhawk7_v1l8p1_rx_pam_mode_enum pam_mode = NRZ;
    struct blackhawk7_v1l8p1_usr_ctrl_disable_functions_st dsu, dss;

    EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));
    EFUN(blackhawk7_v1l8p1_get_usr_ctrl_disable_startup(sa__, &dsu));
    EFUN(blackhawk7_v1l8p1_get_usr_ctrl_disable_steady_state(sa__, &dss));
    EFUN(blackhawk7_v1l8p1_INTERNAL_get_pam_eye_margin_est(sa__, &eye_margin));
...
uint16_t blackhawk7_v1l8p1_INTERNAL_eye_to_mV(srds_access_t *sa__, uint8_t var, uint8_t ladder_range) {
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, var, ladder_range, slicer, &nlmv_val));
    vl = (uint16_t)nlmv_val;
    return (vl);
}

err_code_t blackhawk7_v1l8p1_INTERNAL_get_osr_mode(srds_access_t *sa__, blackhawk7_v1l8p1_osr_mode_st *imode) {
    INIT_SRDS_ERR_CODE
    blackhawk7_v1l8p1_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(blackhawk7_v1l8p1_osr_mode_st));
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_rx_pam_mode(srds_access_t *sa__, enum blackhawk7_v1l8p1_rx_pam_mode_enum *pmode) {
    ...
    if (rx_pam4_mode == 0) {
        ...
        *pmode = (rx_pam4_er_mode) ? PAM4_ER : PAM4_NR;
        ...
        return(blackhawk7_v1l8p1_error(sa__, ERR_CODE_INVALID_RX_PAM_MODE));
    }
...
#else
#error "Unknown core for blackhawk7_v1l8p1_INTERNAL_pmd_lock_status()"
#endif
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_read_lane_state(srds_access_t *sa__, blackhawk7_v1l8p1_lane_state_st *istate) {
  ...
  blackhawk7_v1l8p1_lane_state_st state;

  ENULL_MEMSET(&state, 0, sizeof(blackhawk7_v1l8p1_lane_state_st));

  if(!istate)
    return(blackhawk7_v1l8p1_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(blackhawk7_v1l8p1_INTERNAL_pmd_lock_status(sa__,&state.rx_lock, &state.rx_lock_chg));
  {
      err_code_t err_code = 0;
      state.stop_state = blackhawk7_v1l8p1_INTERNAL_stop_micro(sa__,state.rx_lock,&err_code);
...

  {
      ...
      ENULL_MEMSET(&osr_mode, 0, sizeof(blackhawk7_v1l8p1_osr_mode_st));
      EFUN(blackhawk7_v1l8p1_INTERNAL_get_osr_mode(sa__, &osr_mode));
  ...
  {
      struct blackhawk7_v1l8p1_uc_lane_config_st lane_cfg;
      EFUN(blackhawk7_v1l8p1_get_uc_lane_cfg(sa__, &lane_cfg));
  ...
  EFUN(blackhawk7_v1l8p1_INTERNAL_sigdet_status(sa__,&state.sig_det, &state.sig_det_chg));
  ...
  EFUN(blackhawk7_v1l8p1_INTERNAL_decode_br_os_mode(sa__, &state.br_pd_en));

  EFUN(blackhawk7_v1l8p1_read_rx_afe(sa__, RX_AFE_PF, &state.pf_main));
  EFUN(blackhawk7_v1l8p1_read_rx_afe(sa__, RX_AFE_PF2, &state.pf2_ctrl));
  ESTM(state.bwr_ctrl = rdv_afe_bw_adjust());
  EFUN(blackhawk7_v1l8p1_read_rx_afe(sa__, RX_AFE_VGA, &state.vga));

  ESTM(state.dc_offset = rd_dc_offset_bin());
  EFUN(blackhawk7_v1l8p1_read_rx_afe(sa__, RX_AFE_PF3, &state.pf3_ctrl));
  ESTM(state.p1_lvl = rdv_usr_main_tap_est()/32);
  EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, state.p1_lvl, 0, INVALID_SLICER, &state.p1_lvl));
  if (state.rx_pam_mode == NRZ) {
      ESTM(state.m1_lvl = rd_rx_phase02_status());
      EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)state.m1_lvl, 0, INVALID_SLICER, &state.m1_lvl));
  ...
  if (state.rx_pam_mode == NRZ) {
      EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_dfe1(sa__, &state.dfe1));
  }
  EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_dfe2(sa__, &state.dfe2));
  EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_dfe3(sa__, &state.dfe3));
  EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_dfe4(sa__, &state.dfe4));
  EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_dfe5(sa__, &state.dfe5));

  EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_dfe6(sa__, &state.dfe6));
  ...
  ESTM(state.txfir_use_pam4_range = rd_txfir_nrz_tap_range_sel() ? 0 : 1);
  {
      ...
      for (tap_num=0; tap_num<6; ++tap_num) {
          EFUN(blackhawk7_v1l8p1_INTERNAL_get_tx_tap(sa__, tap_num, &state.txfir.tap[tap_num]));
      }
  ...
  EFUN(blackhawk7_v1l8p1_INTERNAL_get_eye_margin_est(sa__, &state.heye_left, &state.heye_right, &state.veye_upper, &state.veye_lower));
  EFUN(blackhawk7_v1l8p1_INTERNAL_get_link_time(sa__, &state.link_time));
  {
      /* read lock status at end and combine them to handle transient cases */
      uint8_t rx_lock_at_end=0, rx_lock_chg_at_end=0;
      EFUN(blackhawk7_v1l8p1_INTERNAL_pmd_lock_status(sa__,&rx_lock_at_end, &rx_lock_chg_at_end));
      if (state.rx_lock != rx_lock_at_end) {
          USR_PRINTF(("WARNING: Lane %d rx_lock status changed while reading lane state (rx_lock_start=%d, rx_lock_chg_start=%d, rx_lock_at_end=%d, rx_lock_chg_at_end=%d) \n",
                      blackhawk7_v1l8p1_acc_get_lane(sa__), state.rx_lock, state.rx_lock_chg, rx_lock_at_end, rx_lock_chg_at_end));
      }
  ...
  if (!state.stop_state || (state.stop_state == 0xFF)) {
      /* manually check error code instead of EFUN*/
      {
          ...
          resume_status = blackhawk7_v1l8p1_stop_rx_adaptation(sa__, 0);
          if (resume_status) {
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_link_time(srds_access_t *sa__, uint32_t *link_time) {
    INIT_SRDS_ERR_CODE
    ...
    *link_time = (usr_sts_link_time<<BLACKHAWK7_V1L8P1_LANE_TIMER_SHIFT)/10;
    return ERR_CODE_NONE;
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {
  ...
  EFUN(blackhawk7_v1l8p1_INTERNAL_read_lane_state(sa__, &state));

  lane_idx = blackhawk7_v1l8p1_acc_get_lane(sa__);
  ...
  EFUN_PRINTF((" %1d%s", state.sig_det, state.sig_det_chg ? "*" : " "));
  {
      ...
      EFUN(blackhawk7_v1l8p1_get_rx_tuning_status(sa__, &rx_tuning_done));
  ...
  if (state.link_time == ((0xFFFFL<<BLACKHAWK7_V1L8P1_LANE_TIMER_SHIFT)/10)) {
  ...
  if(state.ucv_status != 0) {
    EFUN_PRINTF(("\n"));
    EFUN(blackhawk7_v1l8p1_decode_uc_sts(sa__, state.ucv_status));
  }
  if(state.ucv_status_ext != 0) {
    EFUN(blackhawk7_v1l8p1_decode_uc_sts_ext(sa__, state.ucv_status_ext));
  }

  return (ERR_CODE_NONE);
}  /* blackhawk7_v1l8p1_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) */


err_code_t blackhawk7_v1l8p1_INTERNAL_read_core_state(srds_access_t *sa__, blackhawk7_v1l8p1_core_state_st *istate) {
  INIT_SRDS_ERR_CODE
  blackhawk7_v1l8p1_core_state_st state;
  struct blackhawk7_v1l8p1_uc_core_config_st core_cfg = UC_CORE_CONFIG_INIT;

  ENULL_MEMSET(&state, 0, sizeof(blackhawk7_v1l8p1_core_state_st));

  if(!istate)
    return(blackhawk7_v1l8p1_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(blackhawk7_v1l8p1_get_uc_core_config(sa__, &core_cfg));
  ...
  EFUN(blackhawk7_v1l8p1_version(sa__, &state.api_version));
  ESTM(state.afe_hardware_version = rdcv_afe_hardware_version());
  ESTM(state.temp_idx             = rdcv_temp_idx());
  {  int16_t                           die_temp = 0;
     EFUN(blackhawk7_v1l8p1_read_die_temperature(sa__, &die_temp));
  ...
  EFUN(blackhawk7_v1l8p1_INTERNAL_read_pll_div(sa__, &state.pll_div));
  EFUN(blackhawk7_v1l8p1_INTERNAL_pll_lock_status(sa__, &state.pll_lock, &state.pll_lock_chg));
...
err_code_t blackhawk7_v1l8p1_INTERNAL_display_core_state_no_newline(srds_access_t *sa__) {
  ...
  blackhawk7_v1l8p1_core_state_st state;
    ENULL_MEMSET(&state     , 0, sizeof(state     ));
    {
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_read_core_state(sa__, &state));
    ...
    EFUN_PRINTF((" %02d "              ,  blackhawk7_v1l8p1_acc_get_core(sa__)));
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_display_pll_to_divider(sa__, state.pll_div));
...
err_code_t blackhawk7_v1l8p1_INTERNAL_check_uc_lane_stopped(srds_access_t *sa__) {
  ...
  if (!is_micro_stopped) {
      return(blackhawk7_v1l8p1_error(sa__, ERR_CODE_UC_NOT_STOPPED));
  } else {
      return(ERR_CODE_NONE);
  }
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_calc_patt_gen_mode_sel(srds_access_t *sa__, uint8_t *mode_sel, uint8_t *zero_pad_len, uint8_t patt_length) {
  ...
  if (!(140 % patt_length)) {
    ...
    *zero_pad_len = 0;
    ...
    return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATT_LENGTH));
  }
  return(ERR_CODE_NONE);
}
#endif /*SMALL_FOOTPRINT */
...
err_code_t blackhawk7_v1l8p1_INTERNAL_set_uc_core_config(srds_access_t *sa__, struct blackhawk7_v1l8p1_uc_core_config_st struct_val) {
    ...
    {   uint8_t reset_state;
        ...
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: blackhawk7_v1l8p1_INTERNAL_set_uc_core_config(..) called without core_dp_s_rstb=0\n"));
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_update_uc_core_config_word(&struct_val));
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_refclk_in_hz(srds_access_t *sa__, enum blackhawk7_v1l8p1_pll_refclk_enum refclk, uint32_t *refclk_in_hz) {
    switch (refclk) {
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_145P947802MHZ:  *refclk_in_hz = 145947802; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_166P7974882MHZ: *refclk_in_hz = 166797488; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_167P4107143MHZ: *refclk_in_hz = 167410714; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_245P76MHZ:      *refclk_in_hz = 245760000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_250MHZ:         *refclk_in_hz = 250000000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_425MHZ:         *refclk_in_hz = 425000000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_534MHZ:         *refclk_in_hz = 534000000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_705MHZ:         *refclk_in_hz = 705000000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_175MHZ:         *refclk_in_hz = 175000000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_400MHZ:         *refclk_in_hz = 400000000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_266P67MHZ:      *refclk_in_hz = 266670000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_100MHZ:         *refclk_in_hz = 100000000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_106P25MHZ:      *refclk_in_hz = 106250000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_122P88MHZ:      *refclk_in_hz = 122880000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_125MHZ:         *refclk_in_hz = 125000000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_155P52MHZ:      *refclk_in_hz = 155520000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_156P25MHZ:      *refclk_in_hz = 156250000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_159P375MHZ:     *refclk_in_hz = 159375000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_161P1328125MHZ: *refclk_in_hz = 161132813; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_166P67MHZ:      *refclk_in_hz = 166670000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_166P015625MHZ:  *refclk_in_hz = 166015625; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_167P38MHZ:      *refclk_in_hz = 167380000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_174P703125MHZ:  *refclk_in_hz = 174703125; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_212P5MHZ:       *refclk_in_hz = 212500000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_311P04MHZ:      *refclk_in_hz = 311040000; break;
        case BLACKHAWK7_V1L8P1_PLL_REFCLK_312P5MHZ:       *refclk_in_hz = 312500000; break;
        default:
            EFUN_PRINTF(("ERROR: Unknown refclk frequency:  0x%08X\n", (uint32_t)refclk));
            *refclk_in_hz = 0xFFFFFFFF;
            return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_REFCLK_FREQUENCY_INVALID));
    }
    return (ERR_CODE_NONE);
}
 ...
 */
static err_code_t _blackhawk7_v1l8p1_get_divisor_ratio(srds_access_t *sa__, enum blackhawk7_v1l8p1_pll_div_enum srds_div, uint16_t *numerator, uint16_t *denominator)
{
    switch (srds_div) {
        case BLACKHAWK7_V1L8P1_PLL_DIV_66:         *denominator =  33;  *numerator =  500; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_67:         *denominator =  67;  *numerator = 1000; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_70:         *denominator =   7;  *numerator =  100; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_72:         *denominator =   9;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_74P25:      *denominator = 297;  *numerator = 4000; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_76:         *denominator =  19;  *numerator =  250; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_73P6:       *denominator =  46;  *numerator =  625; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_79P2:       *denominator =  99;  *numerator = 1250; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_80:         *denominator =   2;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_82P5:       *denominator =  33;  *numerator =  400; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_84:         *denominator =  21;  *numerator =  250; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_85:         *denominator =  17;  *numerator =  200; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_87P5:       *denominator =   7;  *numerator =   80; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_89P6:       *denominator =  56;  *numerator =  625; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_90:         *denominator =   9;  *numerator =  100; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_90P75:      *denominator = 363;  *numerator = 4000; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_96:         *denominator =  12;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_100:        *denominator =   1;  *numerator =   10; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_120:        *denominator =   3;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_127P401984: *denominator = 179;  *numerator = 1405; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_128:        *denominator =  16;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_136:        *denominator =  17;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_140:        *denominator =   7;  *numerator =   50; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_144:        *denominator =  18;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_152:        *denominator =  19;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_184:        *denominator =  23;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_192:        *denominator =  24;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_198:        *denominator =  99;  *numerator =  500; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_200:        *denominator =   1;  *numerator =    5; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_224:        *denominator =  28;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_240:        *denominator =   6;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_54:         *denominator =  27;  *numerator =  500; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_108:        *denominator =  27;  *numerator =  250; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_132:        *denominator =  33;  *numerator =  250; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_147P2:      *denominator =  92;  *numerator =  625; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_158P4:      *denominator =  99;  *numerator =  625; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_160:        *denominator =   4;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_165:        *denominator =  33;  *numerator =  200; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_168:        *denominator =  21;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_170:        *denominator =  17;  *numerator =  100; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_175:        *denominator =   7;  *numerator =   40; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_180:        *denominator =   9;  *numerator =   50; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_264:        *denominator =  33;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_280:        *denominator =   7;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_330:        *denominator =  33;  *numerator =  100; break;
        case BLACKHAWK7_V1L8P1_PLL_DIV_350:        *denominator =   7;  *numerator =   20; break;
        default:
...
#ifndef SMALL_FOOTPRINT
/** Get the Refclk frequency in Hz, based on the blackhawk7_v1l8p1_pll_div_enum value and VCO frequency. */
static err_code_t _blackhawk7_v1l8p1_get_refclk_from_div_vco(srds_access_t *sa__, uint32_t *refclk_freq_hz, enum blackhawk7_v1l8p1_pll_div_enum srds_div, uint32_t vco_freq_khz, enum blackhawk7_v1l8p1_pll_option_enum pll_option) {
    INIT_SRDS_ERR_CODE
    uint16_t numerator, denominator;
    EFUN(_blackhawk7_v1l8p1_get_divisor_ratio(sa__, srds_div, &numerator, &denominator));
    *refclk_freq_hz = ((vco_freq_khz+(denominator>>1)) / denominator) * numerator;
    if (pll_option == BLACKHAWK7_V1L8P1_PLL_OPTION_REFCLK_DOUBLER_EN) *refclk_freq_hz /= 2;
    if (pll_option == BLACKHAWK7_V1L8P1_PLL_OPTION_REFCLK_DIV2_EN)    *refclk_freq_hz *= 2;
    if (pll_option == BLACKHAWK7_V1L8P1_PLL_OPTION_REFCLK_DIV4_EN)    *refclk_freq_hz *= 4;
    return (ERR_CODE_NONE);
}
#endif /*SMALL_FOOTPRINT */

/** Get the VCO frequency in kHz, based on the reference clock frequency and blackhawk7_v1l8p1_pll_div_enum value. */
...
static const enum blackhawk7_v1l8p1_pll_div_enum _blackhawk7_v1l8p1_div_candidates[] = {
    BLACKHAWK7_V1L8P1_PLL_DIV_66,
    BLACKHAWK7_V1L8P1_PLL_DIV_67,
    BLACKHAWK7_V1L8P1_PLL_DIV_70,
    BLACKHAWK7_V1L8P1_PLL_DIV_72,
    BLACKHAWK7_V1L8P1_PLL_DIV_74P25,
    BLACKHAWK7_V1L8P1_PLL_DIV_76,
    BLACKHAWK7_V1L8P1_PLL_DIV_73P6,
    BLACKHAWK7_V1L8P1_PLL_DIV_79P2,
    BLACKHAWK7_V1L8P1_PLL_DIV_80,
    BLACKHAWK7_V1L8P1_PLL_DIV_82P5,
    BLACKHAWK7_V1L8P1_PLL_DIV_84,
    BLACKHAWK7_V1L8P1_PLL_DIV_85,
    BLACKHAWK7_V1L8P1_PLL_DIV_87P5,
    BLACKHAWK7_V1L8P1_PLL_DIV_89P6,
    BLACKHAWK7_V1L8P1_PLL_DIV_90,
    BLACKHAWK7_V1L8P1_PLL_DIV_90P75,
    BLACKHAWK7_V1L8P1_PLL_DIV_96,
    BLACKHAWK7_V1L8P1_PLL_DIV_100,
    BLACKHAWK7_V1L8P1_PLL_DIV_120,
    BLACKHAWK7_V1L8P1_PLL_DIV_127P401984,
    BLACKHAWK7_V1L8P1_PLL_DIV_128,
    BLACKHAWK7_V1L8P1_PLL_DIV_136,
    BLACKHAWK7_V1L8P1_PLL_DIV_140,
    BLACKHAWK7_V1L8P1_PLL_DIV_144,
    BLACKHAWK7_V1L8P1_PLL_DIV_152,
    BLACKHAWK7_V1L8P1_PLL_DIV_184,
    BLACKHAWK7_V1L8P1_PLL_DIV_192,
    BLACKHAWK7_V1L8P1_PLL_DIV_198,
    BLACKHAWK7_V1L8P1_PLL_DIV_200,
    BLACKHAWK7_V1L8P1_PLL_DIV_224,
    BLACKHAWK7_V1L8P1_PLL_DIV_240,
    BLACKHAWK7_V1L8P1_PLL_DIV_54,
    BLACKHAWK7_V1L8P1_PLL_DIV_108,
    BLACKHAWK7_V1L8P1_PLL_DIV_132,
    BLACKHAWK7_V1L8P1_PLL_DIV_147P2,
    BLACKHAWK7_V1L8P1_PLL_DIV_158P4,
    BLACKHAWK7_V1L8P1_PLL_DIV_160,
    BLACKHAWK7_V1L8P1_PLL_DIV_165,
    BLACKHAWK7_V1L8P1_PLL_DIV_168,
    BLACKHAWK7_V1L8P1_PLL_DIV_170,
    BLACKHAWK7_V1L8P1_PLL_DIV_175,
    BLACKHAWK7_V1L8P1_PLL_DIV_180,
    BLACKHAWK7_V1L8P1_PLL_DIV_264,
    BLACKHAWK7_V1L8P1_PLL_DIV_280,
    BLACKHAWK7_V1L8P1_PLL_DIV_330,
    BLACKHAWK7_V1L8P1_PLL_DIV_350
};

static const uint8_t _blackhawk7_v1l8p1_div_candidates_count = sizeof(_blackhawk7_v1l8p1_div_candidates) / sizeof(_blackhawk7_v1l8p1_div_candidates[0]);

static err_code_t _blackhawk7_v1l8p1_check_div(srds_access_t *sa__, enum blackhawk7_v1l8p1_pll_div_enum srds_div) {
    uint8_t i, found = 0;
    for (i=0; i<_blackhawk7_v1l8p1_div_candidates_count; i++) {
        found = (uint8_t)(found | (srds_div == _blackhawk7_v1l8p1_div_candidates[i]));
    }
    if (!found) {
...
static err_code_t _blackhawk7_v1l8p1_check_vco_freq_khz(srds_access_t *sa__, uint32_t vco_freq_khz) {
    if (vco_freq_khz < SERDES_VCO_FREQ_KHZ_MIN - SERDES_VCO_FREQ_KHZ_TOLERANCE) {
        ...
        return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_VCO_FREQUENCY_INVALID));
    }
...
/** Find the entry out of _blackhawk7_v1l8p1_div_candidates that is closest to matching refclk_freq_hz and vco_freq_khz. */
static err_code_t _blackhawk7_v1l8p1_get_div(srds_access_t *sa__, uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum blackhawk7_v1l8p1_pll_div_enum *srds_div, enum blackhawk7_v1l8p1_pll_option_enum pll_option) {
    INIT_SRDS_ERR_CODE
    ...
    for (i=0; i<_blackhawk7_v1l8p1_div_candidates_count; i++) {
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_get_vco_from_refclk_div(sa__, refclk_freq_hz, _blackhawk7_v1l8p1_div_candidates[i], &actual_vco_freq_khz, pll_option));
        ...
        if (vco_freq_khz_min_error > vco_freq_khz_error) {
            ...
            *srds_div = _blackhawk7_v1l8p1_div_candidates[i];
        }
    }
    if (vco_freq_khz_min_error == (int32_t)0xFFFFFFFF) {
      return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_CONFLICTING_PARAMETERS));
    }
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    const uint8_t given_param_count = (uint8_t)(((refclk == BLACKHAWK7_V1L8P1_PLL_REFCLK_UNKNOWN) ? 0 : 1)
                                       + ((*srds_div == BLACKHAWK7_V1L8P1_PLL_DIV_UNKNOWN) ? 0 : 1)
                                       + ((*vco_freq_khz == 0) ? 0 : 1));
    const uint32_t original_vco_freq_khz = *vco_freq_khz;
    enum blackhawk7_v1l8p1_pll_div_enum auto_div = BLACKHAWK7_V1L8P1_PLL_DIV_UNKNOWN;
    ...
    if (given_param_count < 2) {
        return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_INSUFFICIENT_PARAMETERS));
    }

    /* Skip verification if option is selected. Error if all parameters not given. */
    if ((pll_option & BLACKHAWK7_V1L8P1_PLL_OPTION_DISABLE_VERIFY) == BLACKHAWK7_V1L8P1_PLL_OPTION_DISABLE_VERIFY) {
        if (given_param_count < 3) {
            ...
            COMPILER_64_SET(refclk_mhz, 0, ((((uint32_t)refclk) & 0xFFF00000) >> 20));
            COMPILER_64_UDIV_64(refclk_temp, refclk_mhz);
            *refclk_freq_hz = COMPILER_64_LO(refclk_temp);
            return(ERR_CODE_NONE);
        }
    }
    pll_option = (enum blackhawk7_v1l8p1_pll_option_enum)(pll_option & BLACKHAWK7_V1L8P1_PLL_OPTION_REFCLK_MASK);

    /* The refclk value is checked in various functions below. */

    /* Verify that the requested div value is allowed. */
    if (*srds_div != BLACKHAWK7_V1L8P1_PLL_DIV_UNKNOWN) {
        EFUN(_blackhawk7_v1l8p1_check_div(sa__, *srds_div));
    }

    /* Verify that the requested VCO frequency is allowed. */
    if (*vco_freq_khz != 0) {
        EFUN(_blackhawk7_v1l8p1_check_vco_freq_khz(sa__,*vco_freq_khz));
    }

    if (refclk == BLACKHAWK7_V1L8P1_PLL_REFCLK_UNKNOWN) {
        /* Determine refclk from vco frequency and div */
        EFUN(_blackhawk7_v1l8p1_get_refclk_from_div_vco(sa__, refclk_freq_hz, *srds_div, *vco_freq_khz, pll_option));
    } else {
     ...
     */
    EFUN(_blackhawk7_v1l8p1_get_div(sa__, *refclk_freq_hz, *vco_freq_khz, &auto_div, pll_option));
    if (*srds_div == BLACKHAWK7_V1L8P1_PLL_DIV_UNKNOWN) {
...
err_code_t blackhawk7_v1l8p1_INTERNAL_poll_diag_done(srds_access_t *sa__, uint16_t *status, uint32_t timeout_ms) {
 ...
 return(blackhawk7_v1l8p1_error(sa__, ERR_CODE_DIAG_TIMEOUT));
}
#endif /*SMALL_FOOTPRINT */
...
err_code_t blackhawk7_v1l8p1_INTERNAL_poll_uc_dsc_ready_for_cmd_equals_1(srds_access_t *sa__, uint32_t timeout_ms, enum srds_pmd_uc_cmd_enum cmd)
{
    ...
    for (loop = 0; loop < 100; loop++) {
        ...
        if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
            if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
                EFUN(blackhawk7_v1l8p1_INTERNAL_print_uc_dsc_error(sa__, cmd));
                return(blackhawk7_v1l8p1_error(sa__, ERR_CODE_UC_CMD_RETURN_ERROR));
            }
            return (ERR_CODE_NONE);
        }
        if(loop>10) {
    ...
    }
    {
        ...
        blackhawk7_v1l8p1_INTERNAL_print_triage_info(sa__, ERR_CODE_UC_CMD_POLLING_TIMEOUT, 1, 1, (uint16_t)__LINE__);
...
err_code_t blackhawk7_v1l8p1_INTERNAL_poll_dsc_state_equals_uc_tune(srds_access_t *sa__, uint32_t timeout_ms) {
  ...
  return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_POLLING_TIMEOUT));          /* Error Code for polling timeout */
}
...
err_code_t blackhawk7_v1l8p1_INTERNAL_rdblk_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk7_v1l8p1_INTERNAL_rdblk_callback_arg_t * const cast_arg = (blackhawk7_v1l8p1_INTERNAL_rdblk_callback_arg_t *)arg;
    UNUSED(sa__);
...
#ifndef SMALL_FOOTPRINT
err_code_t blackhawk7_v1l8p1_INTERNAL_clk4sync_div2_sequence(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
    ...
    if (enable) {
        ...
        ESTM(pll_orig = blackhawk7_v1l8p1_acc_get_pll_idx(sa__));
        pll_selected = 0;
        EFUN(blackhawk7_v1l8p1_acc_set_pll_idx(sa__,pll_selected));
        EFUN(blackhawk7_v1l8p1_INTERNAL_pll_lock_status(sa__, &pll_lock, &pll_lock_chg));
...
err_code_t blackhawk7_v1l8p1_INTERNAL_read_res_error(srds_access_t *sa__, int8_t res_dac4ck_err[][RES_ERROR_NUM_ILV], uint8_t sgn_chn_cnt[][RES_ERROR_NUM_ILV]) {
    INIT_SRDS_ERR_CODE
    int8_t hoff_res;
    uint8_t i,j;
...
err_code_t blackhawk7_v1l8p1_INTERNAL_poll_dbgfb_idx_lock(srds_access_t *sa__, uint8_t timeout_ms) {
  ...
  if (idx_lock) {
      ...
      return (blackhawk7_v1l8p1_error(sa__, ERR_CODE_POLLING_TIMEOUT));
  }
}
...
err_code_t blackhawk7_v1l8p1_get_dbgfb_summary(srds_access_t *sa__, dbgfb_cfg_st *dbgfb_cfg) {
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));
    if (pam_mode == NRZ) {
        EFUN_PRINTF(("!!!!ERROR!!!! rx_pam_mode is set to NRZ , blackhawk7_v1l8p1_get_dbgfb_summary cannot continue\n"));
        return(ERR_CODE_INVALID_RX_PAM_MODE);
    }
    ...
    if (dbgfb_cfg->x) {
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_poll_dbgfb_idx_lock(sa__, 100));
...
err_code_t blackhawk7_v1l8p1_collect_dbgfb_stats(srds_access_t *sa__, uint32_t time_in_us, dbgfb_stats_st *dbgfb_stats) {
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));
    if (pam_mode == NRZ) {
        EFUN_PRINTF(("!!!!ERROR!!!! rx_pam_mode is set to NRZ , blackhawk7_v1l8p1_collect_dbgfb_stats cannot continue\n"));
        return(ERR_CODE_INVALID_RX_PAM_MODE);
    }
    ...
    for (y=0; y < max_y; y++){
        for (x = 0; x < _X_; x++){
            ...
            dbgfb_cfg.data2 = 0;
            dbgfb_cfg.data3 = 0;
            EFUN(blackhawk7_v1l8p1_get_dbgfb_summary(sa__, &dbgfb_cfg));
            dbgfb_stats->time_in_us = time_in_us;
...
err_code_t blackhawk7_v1l8p1_INTERNAL_display_info_table (srds_access_t *sa__) {
    ...
    EFUN_PRINTF(("signature                 = 0x%X\n", blackhawk7_v1l8p1_info_ptr->signature));
    EFUN_PRINTF(("diag_mem_ram_base         = 0x%X\n", blackhawk7_v1l8p1_info_ptr->diag_mem_ram_base));
    EFUN_PRINTF(("diag_mem_ram_size         = 0x%X\n", blackhawk7_v1l8p1_info_ptr->diag_mem_ram_size));
    EFUN_PRINTF(("core_var_ram_base         = 0x%X\n", blackhawk7_v1l8p1_info_ptr->core_var_ram_base));
    EFUN_PRINTF(("core_var_ram_size         = 0x%X\n", blackhawk7_v1l8p1_info_ptr->core_var_ram_size));
    EFUN_PRINTF(("lane_var_ram_base         = 0x%X\n", blackhawk7_v1l8p1_info_ptr->lane_var_ram_base));
    EFUN_PRINTF(("lane_var_ram_size         = 0x%X\n", blackhawk7_v1l8p1_info_ptr->lane_var_ram_size));
    EFUN_PRINTF(("trace_mem_ram_base        = 0x%X\n", blackhawk7_v1l8p1_info_ptr->trace_mem_ram_base));
    EFUN_PRINTF(("trace_mem_ram_size        = 0x%X\n", blackhawk7_v1l8p1_info_ptr->trace_mem_ram_size));
    EFUN_PRINTF(("micro_var_ram_base        = 0x%X\n", blackhawk7_v1l8p1_info_ptr->micro_var_ram_base));
    EFUN_PRINTF(("lane_count                = 0x%X\n", blackhawk7_v1l8p1_info_ptr->lane_count));
    EFUN_PRINTF(("trace_memory_descending_writes = 0x%X\n", blackhawk7_v1l8p1_info_ptr->trace_memory_descending_writes));
    EFUN_PRINTF(("micro_count               = 0x%X\n", blackhawk7_v1l8p1_info_ptr->micro_count));
    EFUN_PRINTF(("micro_var_ram_size        = 0x%X\n", blackhawk7_v1l8p1_info_ptr->micro_var_ram_size));
    EFUN_PRINTF(("grp_ram_size              = 0x%X\n", blackhawk7_v1l8p1_info_ptr->grp_ram_size));
    EFUN_PRINTF(("ucode_version             = 0x%X\n", blackhawk7_v1l8p1_info_ptr->ucode_version));
    EFUN_PRINTF(("info_table_version        = 0x%X\n", blackhawk7_v1l8p1_info_ptr->info_table_version));

    return (ERR_CODE_NONE);
}
err_code_t blackhawk7_v1l8p1_INTERNAL_get_logical_tx_lane_addr(srds_access_t *sa__, uint8_t physical_tx_lane, uint8_t *logical_tx_lane) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_v1l8p1_INTERNAL_get_logical_rx_lane_addr(srds_access_t *sa__, uint8_t physical_rx_lane, uint8_t *logical_rx_lane) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_v1l8p1_INTERNAL_rd_breakpoint(srds_access_t *sa__) {
    ...
    UNUSED(lane);
#endif
    ESTM(core = blackhawk7_v1l8p1_acc_get_core(sa__));
