...
err_code_t blackhawk_tsc_prbs_error_analyzer_config(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint8_t prbs_errcnt_thresh, uint8_t hist_errcnt_thresh) {
    ...
    fec_size = ((prbs_err_fec_size + BLACKHAWK_TSC_PRBS_FEC_FRAME_SIZE_ROUND_UP) / BLACKHAWK_TSC_PRBS_FEC_FRAME_SIZE_MULTIPLIER);

    fec_size_bits = (fec_size * BLACKHAWK_TSC_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
...
err_code_t blackhawk_tsc_get_prbs_error_analyzer_config(srds_access_t *sa__, uint16_t *prbs_err_fec_size, uint8_t *prbs_errcnt_thresh, uint8_t *hist_errcnt_thresh) {
    ...
    *prbs_err_fec_size       = *prbs_err_fec_size * BLACKHAWK_TSC_PRBS_FEC_FRAME_SIZE_MULTIPLIER;
...
err_code_t blackhawk_tsc_display_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk_tsc_prbs_err_analyzer_errcnt_st err_analyzer) {
    ...
    t = err_analyzer.hist_errcnt_thresh - 1;                                      /* number of FEC correctable errors */
    ...
    EFUN_PRINTF((" -------------------------------------------------------------\n"));
    for (i=0; i<8; i++) {
        EFUN(_blackhawk_tsc_display_fec_frame_error(t+i, err_analyzer.prbs_errcnt[i]));
    }
    EFUN(_blackhawk_tsc_display_fec_frame_error(err_analyzer.prbs_errcnt_thresh-1, err_analyzer.prbs_errcnt[8]));
    EFUN_PRINTF((" -------------------------------------------------------------\n"));
...
err_code_t blackhawk_tsc_display_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint8_t hist_errcnt_thresh, uint32_t timeout_s) {
    ...
    USR_MEMSET(&err_analyzer, 0, sizeof(blackhawk_tsc_prbs_err_analyzer_errcnt_st));
        err_analyzer.prbs_errcnt_thresh = hist_errcnt_thresh + 8;     /* Since 20bit err_cnt register counts Frames with errors >= prbs_errcnt_thresh errors */
...
err_code_t blackhawk_tsc_display_prbs_error_analyzer_proj_auto_threshold(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint32_t timeout_s) {
    ...
    if(!prbs_lock) {
        ...
        err_analyzer.prbs_errcnt_thresh = err_analyzer.hist_errcnt_thresh + 8;     /* Since 20bit err_cnt register counts Frames with errors >= prbs_errcnt_thresh errors */
...
err_code_t blackhawk_tsc_prbs_error_analyzer_report_proj(srds_access_t *sa__, blackhawk_tsc_prbs_err_analyzer_errcnt_st err_analyzer, uint32_t timeout_s, blackhawk_tsc_prbs_err_analyzer_report_st *err_analyzer_report) {
    ...
    for (i=8; i >= 0; i--) {
        ...
        if (err_analyzer.prbs_errcnt[i] < 10*BLACKHAWK_TSC_PRBS_ERR_ANALYZER_FRAME_OVERLAP_FACTOR) {  /* "10*BLACKHAWK_TSC_PRBS_ERR_ANALYZER_FRAME_OVERLAP_FACTOR"  means  "10 error events" */
            stop_idx = i;
        }
    }
