...
extern int temod_diag_link(PHYMOD_ST *pc);
extern int temod_diag_st(PHYMOD_ST *pc, int resolved_speed);
extern int temod_diag_field_or(PHYMOD_ST *pc);
extern int temod_diag_pmd_pcs_lane_cfg(PHYMOD_ST *pc, int lane);
extern int temod_diag_speed(PHYMOD_ST *pc);
extern int temod_diag_autoneg_abilities(PHYMOD_ST *pc);
extern int temod_diag_autoneg(PHYMOD_ST *pc);
extern int temod_diag_topology(PHYMOD_ST *pc);
extern int temod_diag_general(PHYMOD_ST *pc);
extern int temod_diag_internal_tfc(PHYMOD_ST *pc);
extern int temod_diag_an_timers(PHYMOD_ST* pc);
extern int temod_diag_state(PHYMOD_ST* pc);
extern int temod_diag_debug(PHYMOD_ST* pc);
extern int temod_diag_g_rd_ram(PHYMOD_ST *pc, int mode, int lane);
extern int temod_diag_g_rd_uC(PHYMOD_ST *pc, int lane);
extern int temod_diag_g_info(PHYMOD_ST *pc);
extern int temod_diag_ieee(PHYMOD_ST *pc);
...
STATIC int tempd_an_hcd_spd_get (uint16_t spd, char *mystr, int len)
{
  char *an_spd = "UNDEF";
  ...
  PHYMOD_DEBUG_ERROR(("| ST [%d]               | SPD_ILLEGAL                 | NUM LANES : %d           |\n",      \
    ...
    PHYMOD_DEBUG_ERROR(("| ST [%d]               | 0x%02X                        | NUM LANES : %d           |\n",      \
           ...
           SC_X1_SPD_##ovr##_SPDr_NUM_LANESf_GET(sc_st##_speed)));                                                         \
	                                                                                                                          ...
	                                                                                                                          \
  if((resolved_speed == 0xff) || ((uint32_t)resolved_speed==SC_X1_SPD_##ovr##_SPDr_SPEEDf_GET(sc_st##_speed))) { /* ST has Entry */  \
