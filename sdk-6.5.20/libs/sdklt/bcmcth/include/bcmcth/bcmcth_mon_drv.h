...
/*!
 * \brief Update MON_EGR_REDIRECT_TRACE_EVENTt hardware register(s).
 *
 * This device-specific function is called by the IMM callback handler
 * in order to update the actual hardware registers associated with
 * the MON_EGR_REDIRECT_TRACE_EVENTt table.
 *
 * \param [in] unit     Unit number.
 * \param [in] trans_id Transaction ID.
 * \param [in] lt_id    Logical table ID.
 * \param [in] op       Type of operation (see \ref
 *                      bcmcth_mon_redirect_trace_op_t).
 * \param [in] entry    Redirect trace Event to update.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_IO I/O access error.
 */
typedef int (*bcmcth_mon_redirect_trace_op_f)(
    int unit,
    uint32_t trans_id,
    bcmltd_sid_t lt_id,
...
/*!
 * \brief Get device-specific metadata information.
 *
 * \param [in] unit Unit number.
 * \param [out] attrs_info Device-specific metadata information.
 *
 * \retval SHR_E_NONE No errors.
 */
typedef int (*bcmcth_mon_telemetry_attrs_info_get_f)(
    int unit,
    bcmcth_mon_inband_telemetry_metadata_attrs_t *attrs_info);

/*!
 * \brief Get device-specific metadata field information.
 *
 * \param [in] unit Unit number.
 * \param [out] chunk_info Device-specific metadata field information.
 *
 * \retval SHR_E_NONE No errors.
 */
typedef int (*bcmcth_mon_telemetry_field_info_get_f)(
    int unit,
...
typedef struct bcmcth_mon_drv_s {
    ...
    /*! Telemetry generic attributes information. */
    bcmcth_mon_telemetry_attrs_info_get_f telemetry_attrs_info;

    /*! Telemetry field information. */
    bcmcth_mon_telemetry_field_info_get_f telemetry_field_info;
    ...
    /*! Redirect Trace Event information. */
    bcmcth_mon_redirect_trace_op_f redirect_op;
 ...
 * \param [in] trans_id Transaction ID.
 * \param [in] lt_id    Logical table ID.
 * \param [in] op       Type of operation (see
 *                      \ref bcmcth_mon_redirect_trace_op_t).
 * \param [in] entry    Redirect trace Event to update.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_IO I/O access error.
