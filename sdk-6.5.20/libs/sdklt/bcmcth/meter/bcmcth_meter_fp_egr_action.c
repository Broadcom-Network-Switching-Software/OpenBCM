/*! \file bcmcth_meter_fp_egr_action.c
 *
 * This file contains EFP Meter Action custom handler functions.
 */
/*
...
/*! Device specific attach function type for METER_EGR_ACTION_TEMPLATE. */
typedef bcmcth_meter_action_drv_t *(*bcmcth_meter_egr_action_drv_get_f)(int unit);

/*! Array of device specific attach functions */
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
{ _bc##_cth_meter_egr_action_drv_get },
static struct {
    bcmcth_meter_egr_action_drv_get_f drv_get;
} meter_egr_action_drv_get[] = {
    { 0 }, /* dummy entry for type "none" */
#include <bcmdrd/bcmdrd_devlist.h>
    { 0 } /* end-of-list */
};

typedef int (*bcmcth_meter_egr_action_variant_init_f)(int unit, bcmcth_meter_action_drv_t *drv);
/* Array of device variant specific FP tables init function pointers. */
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    { _bd##_vu##_va##_di_cth_meter_egr_action_variant_init},
static struct {
    bcmcth_meter_egr_action_variant_init_f init;
} meter_egr_action_variant_init[] = {
    { 0 }, /* dummy entry for type "none" */
#include <bcmlrd/chip/bcmlrd_variant.h>
    { 0 } /* end-of-list */
};

static bcmcth_meter_action_drv_t *meter_fp_egr_action_drv[BCMDRD_CONFIG_MAX_UNITS];
static bcmcth_meter_action_pdd_sw_state_t meter_fp_egr_pdd_state[BCMDRD_CONFIG_MAX_UNITS];
static bcmcth_meter_action_pdd_sw_state_t meter_fp_egr_pdd_state_ha[BCMDRD_CONFIG_MAX_UNITS];
...
static int
bcmcth_meter_fp_egr_action_variant_init(int unit,
                                        bcmcth_meter_action_drv_t *drv)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (meter_egr_action_variant_init[variant].init(unit, drv));
...
bcmcth_meter_fp_egr_action_drv_init(int unit)
{
    ...
    meter_fp_egr_action_drv[unit] = meter_egr_action_drv_get[dev_type].drv_get(unit);

    SHR_IF_ERR_EXIT(
        bcmcth_meter_fp_egr_action_variant_init(unit,
                                                meter_fp_egr_action_drv[unit]));

    /* Driver specific init routine. */
    if (meter_fp_egr_action_drv[unit] != NULL) {
        SHR_IF_ERR_EXIT(meter_fp_egr_action_drv[unit]->init(unit));
        ...
        if ((oper_mode != METER_FP_OPER_MODE_PIPE_UNIQUE) ||
            (num_pipes == 0)) {
            meter_fp_egr_action_drv[unit]->num_pipes = 1;
        } else {
            meter_fp_egr_action_drv[unit]->num_pipes = num_pipes;
        }
...
bcmcth_meter_fp_egr_action_drv_cleanup(int unit)
{
    ...
    if (meter_fp_egr_action_drv[unit] != NULL) {
        meter_fp_egr_action_drv[unit]->num_action_lts = 0;
        meter_fp_egr_action_drv[unit]->pdd_lt_info = NULL;
        meter_fp_egr_action_drv[unit]->sbr_lt_info = NULL;
        meter_fp_egr_action_drv[unit]->action_lt_info = NULL;
    }

    meter_fp_egr_action_drv[unit] = NULL;
exit:
    SHR_FUNC_EXIT();
}
...
bcmcth_meter_fp_egr_action_init (int unit, bool warm)
{
    ...
    METER_FP_EGR_PDD_STATE_HA(unit).offset_info =
        shr_ha_mem_alloc(unit,
                         BCMMGMT_METER_COMP_ID,
                         METER_FP_EGR_PDD_OFFSET_SUB_COMP_ID,
                         "bcmcthMeterFpEgrActionPdd",
                         &ha_alloc_size);

    SHR_NULL_CHECK(METER_FP_EGR_PDD_STATE_HA(unit).offset_info, SHR_E_MEMORY);
    SHR_IF_ERR_EXIT((ha_alloc_size < ha_req_size) ?
                    SHR_E_MEMORY : SHR_E_NONE);

    if (!warm) {
        sal_memset(METER_FP_EGR_PDD_STATE_HA(unit).offset_info,
                   0,
                   ha_alloc_size);

        bcmissu_struct_info_report(unit, BCMMGMT_METER_COMP_ID,
                                         METER_FP_EGR_PDD_OFFSET_SUB_COMP_ID, 0,
                                         sizeof(bcmcth_meter_action_pdd_offset_t),
                                         (num_profiles * pdd_action_count),
                                         BCMCTH_METER_ACTION_PDD_OFFSET_T_ID);
    }

    SHR_ALLOC(METER_FP_EGR_PDD_STATE(unit).offset_info,
              ha_alloc_size,
              "bcmcthMeterFpEgrPddOffsetInfo");
    SHR_NULL_CHECK(METER_FP_EGR_PDD_STATE(unit).offset_info, SHR_E_MEMORY);
    sal_memset(METER_FP_EGR_PDD_STATE(unit).offset_info,
               0,
               ha_alloc_size);

    /* Syncing the temp space to committed SW state. */
    sal_memcpy(METER_FP_EGR_PDD_STATE(unit).offset_info,
               METER_FP_EGR_PDD_STATE_HA(unit).offset_info,
               ha_alloc_size);
    ...
    METER_FP_EGR_PDD_STATE_HA(unit).state =
        shr_ha_mem_alloc(unit,
                         BCMMGMT_METER_COMP_ID,
                         METER_FP_EGR_PDD_STATE_SUB_COMP_ID,
                         "bcmcthMeterFpEgrActionPddState",
                         &ha_alloc_size);

    SHR_NULL_CHECK(METER_FP_EGR_PDD_STATE_HA(unit).state, SHR_E_MEMORY);
    SHR_IF_ERR_EXIT((ha_alloc_size < ha_req_size) ?
                    SHR_E_MEMORY : SHR_E_NONE);

    if (!warm) {
        sal_memset(METER_FP_EGR_PDD_STATE_HA(unit).state,
                   0,
                   ha_alloc_size);
    }

    SHR_ALLOC(METER_FP_EGR_PDD_STATE(unit).state,
              ha_alloc_size,
              "bcmcthMeterFpEgrPddOffsetInfo");
    SHR_NULL_CHECK(METER_FP_EGR_PDD_STATE(unit).state, SHR_E_MEMORY);
    sal_memset(METER_FP_EGR_PDD_STATE(unit).state,
               0,
               ha_alloc_size);

    /* Syncing the temp space to committed SW state. */
    sal_memcpy(METER_FP_EGR_PDD_STATE(unit).state,
               METER_FP_EGR_PDD_STATE_HA(unit).state,
               ha_alloc_size);

exit:
    if (SHR_FUNC_ERR()) {
        if (METER_FP_EGR_PDD_STATE_HA(unit).offset_info != NULL) {
            shr_ha_mem_free(unit, METER_FP_EGR_PDD_STATE_HA(unit).offset_info);
            METER_FP_EGR_PDD_STATE_HA(unit).offset_info = NULL;
        }

        if (METER_FP_EGR_PDD_STATE(unit).offset_info != NULL) {
            SHR_FREE(METER_FP_EGR_PDD_STATE(unit).offset_info);
            METER_FP_EGR_PDD_STATE(unit).offset_info = NULL;
        }

        if (METER_FP_EGR_PDD_STATE_HA(unit).state != NULL) {
            shr_ha_mem_free(unit, METER_FP_EGR_PDD_STATE_HA(unit).state);
            METER_FP_EGR_PDD_STATE_HA(unit).state = NULL;
        }

        if (METER_FP_EGR_PDD_STATE(unit).state != NULL) {
            SHR_FREE(METER_FP_EGR_PDD_STATE(unit).state);
            METER_FP_EGR_PDD_STATE(unit).state = NULL;
        }
    }
                                   ...
                                   void *context)
{
    ...
    act_ptr = METER_FP_EGR_PDD_STATE(unit).offset_info;
    bkp_ptr = METER_FP_EGR_PDD_STATE_HA(unit).offset_info;
    ...
    act_ptr = METER_FP_EGR_PDD_STATE(unit).state;
    bkp_ptr = METER_FP_EGR_PDD_STATE_HA(unit).state;
                                    ...
                                    bcmcth_meter_action_drv_t **drv)
{
    ...
    if (meter_fp_egr_action_drv[unit] != NULL) {
        *drv = meter_fp_egr_action_drv[unit];
    } else {
        SHR_VERBOSE_EXIT(SHR_E_UNAVAIL);
    }
                                 ...
                                 bcmcth_meter_action_pdd_sw_state_t **ptr)
{
    ...
    *ptr = &meter_fp_egr_pdd_state[unit];
