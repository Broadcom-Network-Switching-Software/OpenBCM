/*! \file bcm56780_a0_cth_vlan.c
 *
 * bcm56780 a0 VLAN drivers.
 */
/*
 ...
 * Typedefs
 */
#define BCM56780_LOWER_PORTS 40
#define BCM56780_ALL_PORTS 80

/*******************************************************************************
 * Private variables
 */
static int
bcm56780_a0_cth_ing_state_profile_fields_set(int unit, uint32_t trans_id,
                                             ...
                                             vlan_ing_stg_profile_t *lt_info)
{
    ...
    if (lower) {
        for (i = 0;i < BCM56780_LOWER_PORTS; i++) {
        ...
        for (i = BCM56780_LOWER_PORTS; i < BCM56780_ALL_PORTS; i++) {
            if (lt_info->state[i] & 1) {
                BCMDRD_PBMP_PORT_ADD(bitmap_field_value, (i - BCM56780_LOWER_PORTS) * 2);
            }
            if (lt_info->state[i] & 2) {
                BCMDRD_PBMP_PORT_ADD(bitmap_field_value, (i - BCM56780_LOWER_PORTS) * 2 + 1);
            }
        }
        SHR_IF_ERR_EXIT
            (bcmdrd_pt_field_set(unit,
...
static int
bcm56780_a0_cth_block_mask_membership_fields_set(int unit, uint32_t trans_id,
                                                 bcmltd_sid_t lt_id, bool lower,
...
static int
bcm56780_a0_cth_egr_state_profile_fields_set(int unit, uint32_t trans_id,
                                             bcmltd_sid_t lt_id, bool lower,
...
static int
bcm56780_a0_cth_vlan_ing_stg_profile_set(int unit, uint32_t trans_id,
                                         bcmltd_sid_t lt_id,
                                         vlan_ing_stg_profile_t *lt_info)
{
    ...
    if (BCMCTH_VLAN_LT_FIELD_GET(lt_info->fbmp, STG_STATE)) {
        for (i = 0; i < BCM56780_ALL_PORTS; i++) {
            if (BCMCTH_VLAN_LT_FIELD_GET(
                    lt_info->state_array_bmp, i)) {
                if (i < BCM56780_LOWER_PORTS) {
    ...
    LOG_DEBUG(BSL_LOG_MODULE,
              (BSL_META_U(unit,
                          "bcm56780_a0_cth_vlan_ing_stg_profile_set.\n")));

    if (state_profile_lower_update) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_cth_ing_state_profile_fields_set(unit,
                                                          trans_id,
                                                          lt_id,
                                                          1,
                                                          lt_info));
...
static int
bcm56780_a0_cth_vlan_ing_egr_stg_member_profile_set(int unit, uint32_t trans_id,
                                                    bcmltd_sid_t lt_id,
                                                    vlan_ing_egr_stg_member_profile_t *lt_info)
{
    ...
    LOG_DEBUG(BSL_LOG_MODULE,
              (BSL_META_U(unit,
                          "bcm56780_a0_cth_vlan_ing_stg_flood_block_profile_set.\n")));

    if (block_mask_membership_lower_update) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_cth_block_mask_membership_fields_set(unit,
                                                              trans_id,
                                                              lt_id,
                                                              1,
                                                              lt_info));
...
static int
bcm56780_a0_cth_vlan_egr_stg_profile_set(int unit, uint32_t trans_id,
                                         bcmltd_sid_t lt_id,
                                         vlan_egr_stg_profile_t *lt_info)
{
    bool state_profile_lower_update = false;
    ...
    LOG_DEBUG(BSL_LOG_MODULE,
              (BSL_META_U(unit,
                          "bcm56780_a0_cth_vlan_egr_stg_profile_set.\n")));

    if (state_profile_lower_update) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_cth_egr_state_profile_fields_set(unit,
                                                      trans_id,
                                                      lt_id,
                                                      1,
                                                      lt_info));
...
static bcmcth_vlan_drv_t bcm56780_a0_cth_vlan_drv = {
           .xlate_port_grp_set = NULL,
           .ing_stg_profile_set = bcm56780_a0_cth_vlan_ing_stg_profile_set,
           .ing_egr_stg_member_profile_set = bcm56780_a0_cth_vlan_ing_egr_stg_member_profile_set,
           .egr_stg_profile_set = bcm56780_a0_cth_vlan_egr_stg_profile_set
};
/*******************************************************************************
 * Public Functions
 */
bcmcth_vlan_drv_t *
bcm56780_a0_cth_vlan_drv_get(int unit)
{
    return &bcm56780_a0_cth_vlan_drv;
}
