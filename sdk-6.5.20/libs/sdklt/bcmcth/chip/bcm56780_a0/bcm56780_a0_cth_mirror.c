...
static int
bcm56780_a0_cth_mirror_encap_type_update(
    int unit,
    ...
    bool enable)
{
     ...
     *   MIRROR_ENCAP_MIRROR_ON_DROP_V2 = 0x88,
    ...
    if (enable) {
        ...
        case BCMCTH_MIRROR_ENCAP_BASIC:
            if (entry->encap_mode ==
                BCM56780_A0_LRD_MIRROR_ENCAP_MODE_T_T_NO_ENCAP_LOOPBACK) {
                /* MIRROR_NO_ENCAP_LPBK_PORT for no-encap mirror. */
                fval = 0x8b;
            } else if (entry->encap_mode ==
                       BCM56780_A0_LRD_MIRROR_ENCAP_MODE_T_T_ENCAP_IFA_LOOPBACK) {
                /* MIRROR_ENCAP_IFA_LOOPBACK for 1st PASS packet of IFA 2.0. */
                fval = 0xc2;
            } else if (entry->encap_mode ==
                       BCM56780_A0_LRD_MIRROR_ENCAP_MODE_T_T_NO_ENCAP) {
                /* MIRROR_NO_ENCAP for MOD. */
                fval = 0x1;
            } else {
                SHR_ERR_EXIT(SHR_E_UNAVAIL);
            }
        ...
        case BCMCTH_MIRROR_ENCAP_MIRROR_ON_DROP_IPV6:
            if (entry->encap_mode ==
                BCM56780_A0_LRD_MIRROR_ON_DROP_MODE_T_T_INGRESS) {
                fval = 0x86;
            } else if (entry->encap_mode ==
                BCM56780_A0_LRD_MIRROR_ON_DROP_MODE_T_T_EGRESS) {
                fval = 0x88;
            } else {
                SHR_ERR_EXIT(SHR_E_UNAVAIL);
            }
        ...
        case BCMCTH_MIRROR_ENCAP_VXLAN_IPV6:
            ...
            break;
        case BCMCTH_MIRROR_ENCAP_IFA_1_PROBE:
            fval = 0xC1;
            break;
        default:
            fval = 0x0;
...
static int
bcm56780_a0_cth_mirror_encap_z2_shim_del(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
...
static int
bcm56780_a0_cth_mirror_encap_z2_shim_set(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
...
static int
bcm56780_a0_cth_mirror_encap_z0_edit_ins_del(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
     ...
     */
    if (variant_data[unit]->encap_data) {
        edit_id = variant_data[unit]->encap_data[index].edit_id;
    } else {
...
static int
bcm56780_a0_cth_mirror_encap_z0_edit_ins_set(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
    FLEX_EDITOR_ZONE_0_EDIT_ID_INS_1_TABLEm_t z0_edit_ins_buf;
...
static int
bcm56780_a0_cth_mirror_encap_z4_edit_ins_del(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
    FLEX_EDITOR_ZONE_4_EDIT_ID_INS_1_TABLEm_t z4_edit_ins_buf;
    uint32_t *p_z4_edit_ins_buf;
    bcmdrd_sid_t pt_id;
    uint32_t trans_id;
    bcmltd_sid_t lt_id;
    int index = 0, edit_id = 0;
    ...
    /* Zone-4 insert: reset. */
    p_z4_edit_ins_buf = (uint32_t *) &z4_edit_ins_buf;
    sal_memset(p_z4_edit_ins_buf, 0, sizeof(z4_edit_ins_buf));
    ...
    pt_id = FLEX_EDITOR_ZONE_4_EDIT_ID_INS_1_TABLEm;

    SHR_IF_ERR_EXIT
        (bcmcth_mirror_pt_write(unit, trans_id, lt_id,
                                pt_id, edit_id, p_z4_edit_ins_buf));
exit:
    SHR_FUNC_EXIT();
}

static int
bcm56780_a0_cth_mirror_encap_z4_edit_ins_set(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
    FLEX_EDITOR_ZONE_4_EDIT_ID_INS_1_TABLEm_t z4_edit_ins_buf;
    ...
    fval = ins_valid[0];
    SHR_IF_ERR_EXIT(
        bcmdrd_pt_field_set(unit, pt_id, p_z4_edit_ins_buf,
                            pt_fid, (uint32_t *) (&fval)));
...
static int
bcm56780_a0_cth_mirror_encap_0_prof_del(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
...
static int
bcm56780_a0_cth_mirror_bitp_set(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
    bool seq = FALSE;
...
static int
bcm56780_a0_cth_mirror_encap_0_prof_set(
    int unit,
    bcmcth_mirror_encap_t *entry)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcm56780_a0_cth_mirror_bitp_set(unit, entry));
 ...
 */
    ...
    if (erspan || sflow || vxlan || psamp || mirr_on_drop) {
        ...
        *(uint16_t *)p_prof_hdr = shr_htons(vlan);
         ...
         *  1 |                         Probe Marker (2)                      |
         *    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
         *  2 |   Version     | Message Type  |            Flags              |
         *    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
         *  3 |        Request Vector         |        Action Vector          |
         *    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
         *  4 |   Hop Limit   |   Hop Count   |        Must Be Zero           |
         *    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
         *  5 |         Maximum Length        |        Current Length         |
         *    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
         *  6 |        Sender's Handle        |        Sequence Number        |
         *    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
         */

            *(uint32_t *)p_prof_hdr = shr_htonl(entry->ifa_1_probe_marker1);
            p_prof_hdr += sizeof(uint32_t);

            *(uint32_t *)p_prof_hdr = shr_htonl(entry->ifa_1_probe_marker2);
            p_prof_hdr += sizeof(uint32_t);

            /* version */
            *(uint8_t *)p_prof_hdr = entry->ifa_1_version;
            p_prof_hdr += sizeof(uint8_t);

            /* msg type */
            *(uint8_t *)p_prof_hdr = entry->ifa_1_msg_type;
            p_prof_hdr += sizeof(uint8_t);
            ...
            *(uint16_t *)p_prof_hdr = shr_htons(entry->ifa_1_request_vector);
            p_prof_hdr += sizeof(uint16_t);

            *(uint16_t *)p_prof_hdr = shr_htons(entry->ifa_1_action_vector);
            p_prof_hdr += sizeof(uint16_t);

            *(uint8_t *)p_prof_hdr = entry->ifa_1_hop_limit;
            p_prof_hdr += sizeof(uint8_t);

            /* hop count as 0 */
            *(uint8_t *)p_prof_hdr = 0;
            p_prof_hdr += sizeof(uint8_t);
            ...
            *(uint16_t *)p_prof_hdr = shr_htons(entry->ifa_1_max_length);
            p_prof_hdr += sizeof(uint16_t);
    ...
    /* write: Flex editor encap profile. */
    SHR_IF_ERR_EXIT
        (bcmcth_mirror_pt_write(unit, trans_id, lt_id,
                                pt_id_1, index, p_profile_1_hdr));
    SHR_IF_ERR_EXIT
...
static int
bcm56780_a0_cth_mirror_encap_flex_editor_update(
    int unit,
    bcmcth_mirror_encap_t *entry,
    bool enable)
{
    ...
    if (enable) {
        ...
        if (entry->type == BCMCTH_MIRROR_ENCAP_IFA_1_PROBE) {
            ifa_1_probe = TRUE;
        }
        ...
        if (basic || erspan ||sflow || vxlan || psamp || mirr_on_drop) {
            SHR_IF_ERR_EXIT
                (bcm56780_a0_cth_mirror_encap_z0_edit_ins_set(unit, entry));
            SHR_IF_ERR_EXIT
                (bcm56780_a0_cth_mirror_encap_0_prof_set(unit, entry));
        }

        /* Zone-4 insert. */
        if (ifa_1_probe) {
            SHR_IF_ERR_EXIT
                (bcm56780_a0_cth_mirror_encap_z4_edit_ins_set(unit, entry));
            SHR_IF_ERR_EXIT
                (bcm56780_a0_cth_mirror_encap_0_prof_set(unit, entry));
        }
        ...
        /* Zone-2 Shim. */
        SHR_IF_ERR_EXIT
            (bcm56780_a0_cth_mirror_encap_z2_shim_set(unit, entry));
    } else {

        if (ifa_1_probe) {
            /* Zone-4 insert. */
            SHR_IF_ERR_EXIT
                (bcm56780_a0_cth_mirror_encap_z4_edit_ins_del(unit, entry));
        } else {
            /* Zone-0 insert. */
            SHR_IF_ERR_EXIT
                (bcm56780_a0_cth_mirror_encap_z0_edit_ins_del(unit, entry));
        }
        SHR_IF_ERR_EXIT
            (bcm56780_a0_cth_mirror_encap_0_prof_del(unit, entry));

        /* Zone-2 Shim. */
        SHR_IF_ERR_EXIT
            (bcm56780_a0_cth_mirror_encap_z2_shim_del(unit, entry));
    }
exit:
    SHR_FUNC_EXIT();
}
...
static int
bcm56780_a0_cth_mirror_encap_op(
    int unit,
...
static int
bcm56780_a0_cth_mirror_encap_ins_attr_update(
    int unit,
    ...
    bool enable)
{
    int index = 0;
    EPRE_EDEV_CONFIG_MIRROR_ATTRIBUTES_TABLEm_t buf;
    EDIT_CTRL_TCAM_0_Am_t tcam_buf;
    bcmdrd_sid_t pt_id;
    bcmdrd_fid_t pt_fid;
    uint32_t *buf_ptr = NULL;
    uint32_t fval;
    ...
    bcmltd_sid_t lt_id;
    int idx, idx_max;
    uint32_t trucate_valid;
    const bcmcth_mirror_encap_edit_tcam_data_t *edit_tcam_data = NULL;
    ...
    /* Update system dest port. */
    pt_fid = SYSTEM_DEST_PORTf;
    fval = 0;
    if (enable) {
        fval = entry->system_port;
    }
    SHR_IF_ERR_EXIT(
        bcmdrd_pt_field_set(unit,
                            pt_id,
    ...
    /* Update truncate enable. */
    pt_fid = TRUNCATE_ENf;
    fval = 0;
    if (enable) {
        fval = entry->truncate_en;
    }
    SHR_IF_ERR_EXIT(
        bcmdrd_pt_field_set(unit,
                            pt_id,
    ...
    /*
     * Check if EDIT TCAM mapping is available for variant.
     * To update truncate profile index for encap id.
     */
    if (variant_data[unit]->encap_data) {
        /* Get user configured/default truncate profile index. */
        if (enable &&
            (SHR_BITGET(entry->fbmp,\
                        MIRROR_ENCAP_INSTANCE_ATTRt_MIRROR_TRUNCATE_LENGTH_IDf))) {
            fval = entry->truncate_ptr;
        } else {
            /* set to default pointer */
            fval = variant_data[unit]->default_truncate_ptr;
        }

        idx_max = variant_data[unit]->encap_data[index].edit_tcam_count;
        edit_tcam_data = variant_data[unit]->encap_data[index].edit_tcam_data;

        buf_ptr = (uint32_t *)(&tcam_buf);

        /* Update all EDIT TCAM entries which has truncate as valid */
        for (idx = 0 ; idx < idx_max; idx++) {

            pt_id = edit_tcam_data[idx].pid;
            index = edit_tcam_data[idx].tcam_index;

            sal_memset(buf_ptr, 0,  sizeof(tcam_buf));

            SHR_IF_ERR_VERBOSE_EXIT
                (bcmcth_mirror_pt_read(unit, trans_id, lt_id, pt_id,
                                       index, buf_ptr));

            /* TRUNCATE_VALID is at bit 104 in DATA0. */
            bcmdrd_field_get(buf_ptr,
                             80 + 104,
                             80 + 104,
                             &trucate_valid);

            /*
             * Check if truncate valid and
             * update truncate profile index in EDIT TCAM.
             */
            if (trucate_valid) {
                /* TRUNCATE_PROFILE_PTR is at bit 100 in DATA0. */
                bcmdrd_field_set(buf_ptr,
                                 80 + 100,
                                 80 + 100 + 4 - 1,
                                 &fval);
...
static int
bcm56780_a0_cth_mirror_encap_ins_attr_op(
    int unit,
    bcmcth_mirror_encap_op_t op,
    bcmcth_mirror_encap_ins_attr_t *entry)
{
...
static int
bcm56780_a0_cth_mirror_deinit(int unit)
{
...
static bcmcth_mirror_drv_t bcm56780_a0_cth_mirror_drv = {
    .init = bcm56780_a0_cth_mirror_init,
    .deinit = bcm56780_a0_cth_mirror_deinit,
    .encap_op = &bcm56780_a0_cth_mirror_encap_op,
    .encap_ins_attr_op = &bcm56780_a0_cth_mirror_encap_ins_attr_op,
};
...
bcmcth_mirror_drv_t *
bcm56780_a0_cth_mirror_drv_get(int unit)
{
    return &bcm56780_a0_cth_mirror_drv;
}
