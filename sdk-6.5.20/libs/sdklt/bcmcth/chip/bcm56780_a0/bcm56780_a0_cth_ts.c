...
static int
bcm56780_a0_cth_synce_ctrl_validate(int unit,
...
static int
bcm56780_a0_cth_synce_ctrl_update(int unit,
                                  bcmltd_sid_t ltid,
...
static int
bcm56780_a0_cth_synce_ctrl_enable(int unit,
                                  bcmltd_sid_t ltid,
                                  const bcmltd_op_arg_t *op_arg,
                                  uint8_t inst_map,
                                  bool enable)
    ...
    ts_tod_entry_t *entry)
{
    ...
    bcmdrd_sid_t sid[2] = {EGR_TIMESTAMP_EGR_TS_TOD_CONVERSIONm,
                           EPOST_EGR_TS_TOD_CONVERSION_2m};
    bcmdrd_sid_t en_sid[2] = {EGR_TIMESTAMP_EGR_TS_TOD_HW_UPDATEr,
                              EPOST_EGR_TS_TOD_HW_UPDATE_2r};
    bcmdrd_sid_t sts_sid[2] = {EGR_TIMESTAMP_EGR_TS_TOD_HW_UPDATE_STATUSr,
                               EPOST_EGR_TS_TOD_HW_UPDATE_STATUS_2r};
    uint64_t timestamp = 0;
    uint32_t val[2] = {0}, sts[2] = {0};
    uint8_t i;
    int rv, ts_inst = 1;
    ...
    for (i = 0; i < 2; i++) {
        if (enable) {
            /* Assign current TS values */
            sal_memset(entry_buf, 0, entry_sz);
            SHR_IF_ERR_EXIT
                (bcmcth_imm_pipe_ireq_read(unit, ltid, sid[i],
                                           -1, 0, entry_buf));
            val[1] = timestamp >> 32;
            ...
            SHR_IF_ERR_EXIT
                (bcmcth_imm_pipe_ireq_write(unit, ltid, sid[i],
                                            -1, 0, entry_buf));
            ...
            sal_memset(entry_buf, 0, entry_sz);
            SHR_IF_ERR_EXIT
                (bcmcth_imm_pipe_ireq_read(unit, ltid, en_sid[i],
                                           -1, 0, entry_buf));
            val[1] = 0;
            val[0] = 1;
            bcmdrd_pt_field_set
                (unit, en_sid[i], entry_buf, ENABLEf, val);
            SHR_IF_ERR_EXIT
                (bcmcth_imm_pipe_ireq_write(unit, ltid, en_sid[i],
                                            -1, 0, entry_buf));
        } else {
            /* Disable hw update */
            sal_memset(entry_buf, 0, entry_sz);
            ...
            SHR_IF_ERR_EXIT
                (bcmcth_imm_pipe_ireq_write(unit, ltid, en_sid[i],
                                            -1, 0, entry_buf));
            /* Clear status fail if any */
            sal_memset(entry_buf, 0, entry_sz);
            SHR_IF_ERR_EXIT
                (bcmcth_imm_pipe_ireq_read(unit, ltid, sts_sid[i],
                                           -1, 0, entry_buf));
            bcmdrd_pt_field_get
                (unit, sts_sid[i], entry_buf, FAILf, sts);
            if (sts[0]) {
                val[1] = val[0] = 0;
                bcmdrd_pt_field_set
                    (unit, sts_sid[i], entry_buf, FAILf, val);
                SHR_IF_ERR_EXIT
                    (bcmcth_imm_pipe_ireq_write(unit, ltid, sts_sid[i],
                                                -1, 0, entry_buf));
            }
        }
    }
    ...
    ts_tod_entry_oper_t *entry)
{
    uint32_t entry_sz;
    uint32_t *entry_buf = NULL;
    bcmdrd_sid_t sid = EGR_TIMESTAMP_EGR_TS_TOD_CONVERSIONm;
    bcmdrd_sid_t en_sid[2] = {EGR_TIMESTAMP_EGR_TS_TOD_HW_UPDATEr,
                              EPOST_EGR_TS_TOD_HW_UPDATE_2r};
    bcmdrd_sid_t sts_sid[2] = {EGR_TIMESTAMP_EGR_TS_TOD_HW_UPDATE_STATUSr,
                               EPOST_EGR_TS_TOD_HW_UPDATE_STATUS_2r};
    ...
    for (i = 0; i < 2; i++) {
        ...
        bcmdrd_pt_field_get
            (unit, en_sid[i], entry_buf, ENABLEf, hw_en);
            ...
            if (hw_en[0]) {
                ...
                SHR_IF_ERR_EXIT
                    (bcmcth_imm_pipe_ireq_read(unit, ltid, sts_sid[i],
                                               -1, 0, entry_buf));
                bcmdrd_pt_field_get
                        (unit, sts_sid[0], entry_buf, FAILf, sts);
                if (sts[0]) {
                    update_fail |= true;
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit,"ToD hw update fail in %s\n"),
                                 (i == 0) ? "EGR_TIMESTAMP" : "EPOST"));
                    break;
                }
            }
        } else {
            if (hw_en[0]) {
                /* hw_upate is not applicable in sim */
                update_fail |= true;
            }
        }
    }
...
static bcmcth_ts_synce_drv_t bcm56780_a0_cth_synce_drv = {
    .synce_ctrl_enable = &bcm56780_a0_cth_synce_ctrl_enable,
    .synce_ctrl_update = &bcm56780_a0_cth_synce_ctrl_update,
    .synce_ctrl_validate = &bcm56780_a0_cth_synce_ctrl_validate
};

static bcmcth_ts_tod_drv_t bcm56780_a0_cth_tod_drv = {
    .tod_auto = &bcm56780_a0_cth_tod_auto,
    .tod_update = &bcm56780_a0_cth_tod_update,
    .tod_get = &bcm56780_a0_cth_tod_get,
};

static bcmcth_ts_drv_t bcm56780_a0_cth_ts_drv = {
    .synce_drv = &bcm56780_a0_cth_synce_drv,
    .tod_drv = &bcm56780_a0_cth_tod_drv
};
/*******************************************************************************
 * Public functions
 */
bcmcth_ts_drv_t *
bcm56780_a0_cth_ts_drv_get(int unit)
{
    return &bcm56780_a0_cth_ts_drv;
}
