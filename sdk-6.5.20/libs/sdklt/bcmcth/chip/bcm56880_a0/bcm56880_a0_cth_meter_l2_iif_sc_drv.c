/*! \file bcm56880_a0_cth_meter_l2_iif_sc_drv.c
 *
 * bcm56880_a0 driver for storm control meter.
 */
/*
...
#define BCM56880_A0_METER_MAX_OFFSET            (4)
/* Minimum metering rate in kbps */
#define BCM56880_A0_METER_SC_RATE_KBPS_MIN      (64) /* 64kbps */
/* Maximum value of BUCKETSIZE */
#define BCM56880_A0_METER_SC_MAX_BUCKET_SZ      (8)
/* Number of tokens refreshed in a sec when REFRESHCOUNT=1 */
#define BCM56880_A0_METER_SC_TPS_MIN            (128000)
/* Number of quantum levels, SW defined. PKT_QUANTUM range is [1 - 128000]. */
#define BCM56880_A0_METER_SC_PKT_QUANTUM_LEVELS (18)
/* Maximum REFRESHCOUNT value supported on this device. */
#define BCM56880_A0_METER_SC_MAX_REFRESH_COUNT  (0x1fffff)

/* Byte mode macros */
/* Calculate refresh count from metering rate. */
#define BCM56880_A0_METER_SC_COMPUTE_BYTE_REFRESH_COUNT(rps)        \
    ((rps + (BCM56880_A0_METER_SC_RATE_KBPS_MIN - 1)) /             \
     BCM56880_A0_METER_SC_RATE_KBPS_MIN)

/* Calculate metering rate from refresh count. */
#define BCM56880_A0_METER_SC_COMPUTE_BYTE_METER_RATE(rc)            \
    ((uint64_t)rc * (uint64_t)BCM56880_A0_METER_SC_RATE_KBPS_MIN)

/* Calculate burst size from bucket size. */
#define BCM56880_A0_METER_SC_GET_BURST_SZ_KBITS_FROM_BKT_SZ(bktsz)  \
    (bcm56880_a0_bkt_sz_to_kbit_map[bktsz])

/* Packet mode macros */
/* Calculate packets per sec from tokens per sec based on packet quantum. */
#define BCM56880_A0_METER_SC_PPS_MIN(quantum)                       \
    (BCM56880_A0_METER_SC_TPS_MIN / quantum)

/* Calcualte refresh count from metering rate and quantum. */
#define BCM56880_A0_METER_SC_COMPUTE_PKT_REFRESH_COUNT(rps, quantum)        \
    ((rps + (BCM56880_A0_METER_SC_PPS_MIN(quantum) - 1)) /                  \
     BCM56880_A0_METER_SC_PPS_MIN(quantum))

/* Calculate metering rate from refreshcount and quantum .*/
#define BCM56880_A0_METER_SC_COMPUTE_PKT_METER_RATE(rc, quantum)            \
    ((uint64_t)rc * (uint64_t)BCM56880_A0_METER_SC_PPS_MIN(quantum))

/* Calculate burst size from bucket size. */
#define BCM56880_A0_METER_SC_GET_BURST_SZ_PKTS_FROM_BKT_SZ(bktsz, quantum)  \
    (bcm56880_a0_bkt_sz_to_token_map[bktsz] / quantum)

/* Get the quantum value corresponding to the quantum level .*/
#define BCM56880_A0_METER_SC_QUANTUM_VALUE(level)                           \
    (bcm56880_a0_pkt_quantum_info[level].quantum)

/* Get the maximum metering rate supported at each quantum level. */
#define BCM56880_A0_METER_SC_QUANTUM_MAX_RATE(level)                        \
    ((uint64_t)bcm56880_a0_pkt_quantum_info[level].max_rate *               \
     (uint64_t)1000000)
...
/*!
 * \brief Token-wise bucket size definition for BCM56880_A0.
 *
 ...
 */
static uint32_t
bcm56880_a0_bkt_sz_to_token_map[BCM56880_A0_METER_SC_MAX_BUCKET_SZ] = {
    (64  * KBYTES),
    (256 * KBYTES),
    (1   * MBYTES),
    (4   * MBYTES),
...
/*!
 * \brief Kbit-wise bucket size definition for BCM56880_A0.
 *
 * Each bucket size translates to a specific bucket count.
 *
 ...
 */
static uint32_t
bcm56880_a0_bkt_sz_to_kbit_map[BCM56880_A0_METER_SC_MAX_BUCKET_SZ] = {
...
/*!
 * \brief Min-Max definitions per quantum level for BCM56880_A0.
 *
...
bcm56880_a0_pkt_quantum_info[BCM56880_A0_METER_SC_PKT_QUANTUM_LEVELS] = {
    {1, 128000, 268435},
    {2, 64000, 134217},
    {4, 32000, 67108},
    {8, 16000, 33554},
 ...
 */
static int
bcm56880_a0_meter_sc_recalc_hw_param(int unit,
                                     uint32_t meter_idx,
    ...
    for (j = 0; j < meter_idx; j++) {
        ...
        if (entry->meter_en[j] != 0) {
            entry->refresh_count[j] =
                BCM56880_A0_METER_SC_COMPUTE_PKT_REFRESH_COUNT(entry->rate_pps[j],
                                                               entry->quantum);
            if (entry->refresh_count[j] == 0) {
                entry->refresh_count[j] = 1;
            } else if (entry->refresh_count[j] >
                       BCM56880_A0_METER_SC_MAX_REFRESH_COUNT) {
                entry->refresh_count[j] = BCM56880_A0_METER_SC_MAX_REFRESH_COUNT;
            }
             ...
             */
            for (i = 0; i < BCM56880_A0_METER_SC_MAX_BUCKET_SZ; i++) {
                burst_pkts =
                    BCM56880_A0_METER_SC_GET_BURST_SZ_PKTS_FROM_BKT_SZ(
                        i,
                        entry->quantum);
            ...
            if (i == BCM56880_A0_METER_SC_MAX_BUCKET_SZ) {
                i--;
            }
            entry->bucket_size[j] = i;
            entry->bucket_count[j] = bcm56880_a0_bkt_sz_to_token_map[i];
        }
    }
 ...
 */
static int
bcm56880_a0_meter_sc_compute_pkt_quantum(int unit,
                                         uint64_t meter_rate,
                                         uint32_t burst_pkts,
                                         uint32_t *quantum)
{
     ...
     */
    for (i = (BCM56880_A0_METER_SC_PKT_QUANTUM_LEVELS - 1); i >= 0; i--) {
        max_rate = BCM56880_A0_METER_SC_QUANTUM_MAX_RATE(i);
        if (max_rate >= meter_rate) {
             ...
             */
            for (j = 0; j < BCM56880_A0_METER_SC_MAX_BUCKET_SZ; j++) {
                max_burst =
                    BCM56880_A0_METER_SC_GET_BURST_SZ_PKTS_FROM_BKT_SZ(
                        j,
                        BCM56880_A0_METER_SC_QUANTUM_VALUE(i));
    ...
    *quantum = BCM56880_A0_METER_SC_QUANTUM_VALUE(i);
 ...
 */
static int
bcm56880_a0_meter_sc_compute_hw_param (int unit,
                                       bcmcth_meter_l2_iif_sc_entry_t *entry)
{
    uint32_t *map_ptr;
    ...
    for (idx = 0; idx < max_meter_offset; idx++) {
        ...
        if (entry->byte_mode) {
            ...
            entry->refresh_count[idx] =
                BCM56880_A0_METER_SC_COMPUTE_BYTE_REFRESH_COUNT(
                    entry->rate_kbps[idx]);
        } else {
            /*
            ...
            if (entry->quantum == 0) {
                SHR_IF_ERR_VERBOSE_EXIT
                    (bcm56880_a0_meter_sc_compute_pkt_quantum(
                            unit,
                            entry->rate_pps[idx],
                ...
                */
                max_rate = BCM56880_A0_METER_SC_COMPUTE_PKT_METER_RATE(
                    BCM56880_A0_METER_SC_MAX_REFRESH_COUNT,
                    entry->quantum);
                if (max_rate < entry->rate_pps[idx]) {
                     ...
                     */
                    SHR_IF_ERR_VERBOSE_EXIT
                        (bcm56880_a0_meter_sc_recalc_hw_param(
                                unit,
                                idx,
                                entry));
                }
             ...
             */
            entry->refresh_count[idx] =
                BCM56880_A0_METER_SC_COMPUTE_PKT_REFRESH_COUNT(
                    entry->rate_pps[idx],
                    entry->quantum);
        }
         ...
         */
        if (entry->byte_mode) {
            map_ptr = bcm56880_a0_bkt_sz_to_kbit_map;
            burst_val = entry->burst_kbits[idx];
        } else {
            map_ptr = bcm56880_a0_bkt_sz_to_token_map;
        ...
        entry->bucket_count[idx] = bcm56880_a0_bkt_sz_to_token_map[i];

        /* Calculate the operating values. */
        if (entry->byte_mode) {
            entry->rate_kbps_oper[idx] =
                BCM56880_A0_METER_SC_COMPUTE_BYTE_METER_RATE(
                    entry->refresh_count[idx]);
            entry->burst_kbits_oper[idx] =
                BCM56880_A0_METER_SC_GET_BURST_SZ_KBITS_FROM_BKT_SZ(
                    entry->bucket_size[idx]);
        } else {
            entry->rate_pps_oper[idx] =
                BCM56880_A0_METER_SC_COMPUTE_PKT_METER_RATE(
                    entry->refresh_count[idx],
                    entry->quantum);
...
/* Storm control meter driver for bcm56880_a0. */
bcmcth_meter_l2_iif_sc_drv_t bcm56880_a0_meter_l2_iif_sc_drv = {
    ...
    .bc_fid = BUCKETCOUNTf,
    .bs_fid = BUCKETSIZEf,
    .num_offset = BCM56880_A0_METER_MAX_OFFSET,
    .compute_hw_param = &bcm56880_a0_meter_sc_compute_hw_param,
};

/* Return driver for bcm56880_a0 storm control meter. */
bcmcth_meter_l2_iif_sc_drv_t *
bcm56880_a0_cth_meter_l2_iif_sc_drv_get(int unit)
{
    return &bcm56880_a0_meter_l2_iif_sc_drv;
}
