    ...
    int indent)
{
    ...
    if(hasCustomPrinter) {
        ...
        for(dim_index = 0;
            ...
            dim_index++) {
            int index1 = dim_index + v->dt.desc.num_dimensions -
                v->dt.type_num_dimensions;
            sizeMultiplier *= v->dt.desc.dimensions[index1];
        }
    }
...
_cint_interpreter_include_search(const char* name, char** path_rv)
{
    ...
    for (l = __directory_list; l ; l = l->next) {
        ...
        /* directory + '/' + name + \0 */
        buf_size = CINT_STRLEN(l->directory) + 1 + len_name + 1;
        p = CINT_MALLOC(buf_size);
        if(p == NULL) {
            if(path_rv) {
                *path_rv = NULL;
            }
        ...
        *p = 0;
        ...
        CINT_STRNCAT(p, name, buf_size - CINT_STRLEN(p));
