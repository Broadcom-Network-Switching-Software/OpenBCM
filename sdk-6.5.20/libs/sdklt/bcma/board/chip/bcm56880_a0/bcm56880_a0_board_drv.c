 ...
 * h/w engg
 */
#define VOLTAGE_RANGE_UPPER_B0 875
#define VOLTAGE_RANGE_LOWER_B0 740

#define VOLTAGE_RANGE_UPPER_A0 800
#define VOLTAGE_RANGE_LOWER_A0 740
 ...
 */
static int bcma_otp_voltage_convert(int unit, uint32_t otp_val, double *voltage)
{
   bcmdrd_dev_id_t dev_id;
   int high = VOLTAGE_RANGE_UPPER_A0;
   int low = VOLTAGE_RANGE_LOWER_A0;
   int rv = SHR_E_NONE;
    ...
    if (dev_id.revision >= BCM56880_REV_B0) {
        low  = VOLTAGE_RANGE_LOWER_B0;
        high = VOLTAGE_RANGE_UPPER_B0;
    }
...
static int
bcm56880_a0_board_avs_voltage_set(int unit, uint32_t otp_voltage_val)
{
    char *regulator_name;
    int rv = SHR_E_NONE;
    double voltage;
    ...
    rv = bcma_otp_voltage_convert(unit, otp_voltage_val, &voltage);
    if (SHR_FAILURE(rv)) {
...
static int
bcm56880_a0_board_avs_configure(int unit)
{
    uint32_t otp_voltage = 0;
    shr_pb_t *pb;
    ...
    if (avs_initialized == FALSE) {
        ...
        SHR_IF_ERR_EXIT(bcmbd_otp_param_get(unit, BCMBD_OTP_PRM_AVS_VOLTAGE,
                                            &otp_voltage));

        /* Initiallize I2C */
        SHR_IF_ERR_EXIT(bcmbd_i2c_init(unit));

        if (!bcma_i2c_device_drv[unit]) {
            SHR_IF_ERR_EXIT(bcma_i2c_board_drv_init(unit));
        }

        /* Set Core voltage through I2C interface */
        SHR_IF_ERR_EXIT(bcm56880_a0_board_avs_voltage_set(unit, otp_voltage));
...
static int
bcm56880_a0_board_init(int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcm56880_a0_board_avs_configure(unit));

exit:
    SHR_FUNC_EXIT();
}

static bcma_board_drv_t bcm56880_a0_board_drv = {
    .board_init = bcm56880_a0_board_init
};

bcma_board_drv_t *
bcm56880_a0_board_drv_get(int unit)
{
    return &bcm56880_a0_board_drv;
}
