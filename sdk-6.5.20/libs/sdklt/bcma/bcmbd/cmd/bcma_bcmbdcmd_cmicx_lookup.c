/*! \file bcma_bcmbdcmd_cmicx_lookup.c
 *
...
iter_op(const bcmdrd_symbol_t *symbol, bcmdrd_sid_t symbol_id, void *vptr)
{
    ...
    rv = bcma_bcmbd_cmicx_tbl_lookup(unit, symbol, &sid, and_masks, or_masks);
    if (rv < 0) {
        return BCMA_CLI_CMD_FAIL;
    }
...
static int
cmicx_lookup(bcma_cli_t *cli, bcma_cli_args_t *args,
...
bcma_bcmbdcmd_cmicx_lookup(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    rv = cmicx_lookup(cli, args, ctoks, ctoks_num);
