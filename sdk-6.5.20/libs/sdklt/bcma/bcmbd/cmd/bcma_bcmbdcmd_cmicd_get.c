/*! \file bcma_bcmbdcmd_cmicd_get.c
 *
 * CLI 'get' command for CMICd
 */
/*
...
iter_op(const bcmdrd_symbol_t *symbol, bcmdrd_sid_t symbol_id, void *vptr)
{
    ...
    rv = bcma_bcmbd_cmicd_symop(unit, symbol, symbol_id, &sid, NULL, NULL);
    if (rv < 0) {
...
static int
cmicd_get(bcma_cli_t *cli, bcma_cli_args_t *args,
...
int
bcma_bcmbdcmd_cmicd_get(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    rv = cmicd_get(cli, args, ctoks, ctoks_num);
