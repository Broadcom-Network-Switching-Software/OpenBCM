/*! \file bcma_bcmbdcmd_cmicx_push.c
 *
...
iter_op(const bcmdrd_symbol_t *symbol, bcmdrd_sid_t symbol_id, void *vptr)
{
    ...
    rv = bcma_bcmbd_cmicx_fifo_push(unit, symbol, symbol_id, &sid,
                                    or_masks, wsize);
    if (rv < 0) {
        return BCMA_CLI_CMD_FAIL;
    }
...
static int
cmicx_push(bcma_cli_t *cli, bcma_cli_args_t *args,
...
bcma_bcmbdcmd_cmicx_push(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    rv = cmicx_push(cli, args, ctoks, ctoks_num);
