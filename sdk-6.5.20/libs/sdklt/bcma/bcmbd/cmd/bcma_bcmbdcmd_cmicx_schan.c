/*! \file bcma_bcmbdcmd_cmicx_schan.c
 *
...
bcma_bcmbdcmd_cmicx_schan(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    const char *arg;
    uint32_t data;
    uint32_t *schan_msg, schan_cmd[2];
    ...
    rv = bcmbd_cmicx_schan_raw_op(unit, NULL, 0, 0, &dwc_max);
    if (SHR_FAILURE(rv)) {
    ...
    sal_memcpy(schan_cmd, schan_msg, sizeof(schan_cmd));

    /* Successfully read all raw data to write. */
    rv = bcmbd_cmicx_schan_raw_op(unit, schan_msg, dwc_max, dwc_max, NULL);

    /* Log the operation. */
    bcma_bcmbd_symlog_schan_cmd_set(unit, schan_cmd, COUNTOF(schan_cmd),
                                    bcma_bcmbd_cmicx_schan_cmd_decode, rv);
