/*! \file bcma_bcmbdcmd_cmicd_set.c
 *
...
iter_op(const bcmdrd_symbol_t *symbol, bcmdrd_sid_t symbol_id, void *vptr)
{
    ...
    rv = bcma_bcmbd_cmicd_symop(unit, symbol, symbol_id, &sid,
                                and_masks, or_masks);
    if (rv < 0) {
        return BCMA_CLI_CMD_FAIL;
    }
...
static int
cmicd_set(bcma_cli_t *cli, bcma_cli_args_t *args,
...
bcma_bcmbdcmd_cmicd_set(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    rv = cmicd_set(cli, args, ctoks, ctoks_num);
