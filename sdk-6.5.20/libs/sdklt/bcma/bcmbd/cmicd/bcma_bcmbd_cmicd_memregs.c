/*! \file bcma_bcmbd_cmicd_memregs.c
 *
 ...
 */
static int
cmicd_cmd_regop(shr_pb_t *pb, int unit,
                ...
                uint32_t *and_masks, uint32_t *or_masks)
{
    ...
    if (write_op && (sid->flags & BCMA_BCMBD_CMD_IDF_RAW)) {
        ...
        /* Read the data */
        if (size == 1) {
            rc = bcmbd_cmicd_reg_read(unit, adext, addr, data, 1);
        } else {
            rc = bcmbd_cmicd_reg_read(unit, adext, addr, data, 2);
        }
    }
    ...
    if (write_op) {
        ...
        /* Write the data */
        if (size == 1) {
            rc = bcmbd_cmicd_reg_write(unit, adext, addr, data, 1);
        } else {
            rc = bcmbd_cmicd_reg_write(unit, adext, addr, data, 2);
        }
 ...
 */
static int
cmicd_cmd_memop(shr_pb_t *pb, int unit,
                const bcmdrd_symbol_t *symbol, bcmdrd_sid_t enum_val,
                bcma_bcmbd_id_t *sid,
                ...
                uint32_t size, uint32_t *and_masks, uint32_t *or_masks)
{
    ...
    if (write_op && (sid->flags & BCMA_BCMBD_CMD_IDF_RAW)) {
        ...
        rc = bcmbd_cmicd_mem_read(unit, adext, addr + idx, data, size);
    }
    ...
    if (write_op) {
        ...
        /* Write the data */
        rc = bcmbd_cmicd_mem_write(unit, adext, addr + idx, data, size);
...
{
    int a, b, bt = -1, p, t;
    uint32_t pipe_info = 0, pmask = 0, blk_pipe_bmp = 0;
        ...
        for (b = sid->block.start; b <= sid->block.end; b++) {
            ...
            for (t = sid->pipe.start; t <= sid->pipe.end; t++) {
                ...
                for (p = sid->port.start; p <= sid->port.end; p++) {
                    ...
                    cmicd_cmd_regop(pb, unit, symbol, enum_val, sid,
                                    a, b, p, t, size,
                                    and_masks, or_masks);
...
{
    int b, bt = -1, idx, t, p;
    uint32_t pipe_info = 0, pmask = 0, sect_size = 0, blk_pipe_bmp = 0;
    int linst = 0, acc_type = -1;
    const bcmdrd_chip_info_t *cinfo = bcmdrd_dev_chip_info_get(unit);
    shr_pb_t *pb;
    bool write_op = (and_masks || or_masks);
        ...
        for (t = sid->pipe.start; t <= sid->pipe.end; t++) {
            ...
            for (p = sid->port.start; p <= sid->port.end; p++){
                ...
                for (idx = sid->addr.start; idx <= sid->addr.end; idx++) {
                    ...
                    cmicd_cmd_memop(pb, unit, symbol, enum_val, sid,
