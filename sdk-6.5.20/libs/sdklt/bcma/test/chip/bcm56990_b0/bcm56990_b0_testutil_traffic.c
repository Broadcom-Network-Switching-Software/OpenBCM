/*! \file bcm56990_b0_testutil_traffic.c
 *
...
#define BCM56990_B0_TRAFFIC_MAX_PIPES               16
#define BCM56990_B0_TRAFFIC_PKT_HDR_LEN_ETH         20
#define BCM56990_B0_TRAFFIC_PKT_HDR_LEN_GSH         12
#define BCM56990_B0_TRAFFIC_CELL_SOP                206
#define BCM56990_B0_TRAFFIC_CELL_MOP                254

#define BCM56990_B0_TRAFFIC_MAX_FRAME_SIZE          9416
#define BCM56990_B0_TRAFFIC_MIN_FRAME_SIZE          64
#define BCM56990_B0_TRAFFIC_WORSTCASE_PKT_SIZE      64

#define BCM56990_B0_TRAFFIC_MAX_STRING_SIZE         32
#define BCM56990_B0_10G_MGMT_PORT_SPEED             10000
#define BCM56990_B0_40G_MGMT_PORT_SPEED             40000

/* Clock frequency name to value map */
static shr_enum_map_t bcm56990_b0_clk_freq_map[]={
    {"CLK_1325MHZ", 1325},
    {"CLK_1250MHZ", 1250},
    {"CLK_1175MHZ", 1175},
    {"CLK_1100MHZ", 1100},
...
typedef struct bcm56990_b0_rate_param_s {
    ...
    uint32_t pipe_ancl_mpps[BCM56990_B0_TRAFFIC_MAX_PIPES];

    /* Auxiliary bandwidth in mega cells per second */
    uint32_t pipe_ancl_mcps[BCM56990_B0_TRAFFIC_MAX_PIPES];

    /* Mgmt port speed in Mbps */
    uint32_t mgmt_port_speed[BCM56990_B0_TRAFFIC_MAX_PIPES];
...
} bcm56990_b0_rate_param_t;

static int
bcm56990_b0_rate_init_freq(int unit, bcm56990_b0_rate_param_t *rate_param)
{
    char pp_clk_freq_str[BCM56990_B0_TRAFFIC_MAX_STRING_SIZE];
    char core_clk_freq_str[BCM56990_B0_TRAFFIC_MAX_STRING_SIZE];
    ...
    SHR_IF_ERR_EXIT
        (bcma_testutil_core_clk_freq_get
            (unit, BCM56990_B0_TRAFFIC_MAX_STRING_SIZE, core_clk_freq_str));
    SHR_IF_ERR_EXIT
        (bcma_testutil_pp_clk_freq_get
            (unit, BCM56990_B0_TRAFFIC_MAX_STRING_SIZE, pp_clk_freq_str));

    map_len = COUNTOF(bcm56990_b0_clk_freq_map);
    core_clk_freq = bcma_testutil_name2id(bcm56990_b0_clk_freq_map, map_len,
                                          core_clk_freq_str);
    pp_clk_freq   = bcma_testutil_name2id(bcm56990_b0_clk_freq_map, map_len,
                                          pp_clk_freq_str);
...
static int
bcm56990_b0_rate_init(int unit, bcmdrd_pbmp_t pbmp, int pkt_size,
                      bcm56990_b0_rate_param_t *rate_param)
{
    ...
    BCMDRD_PBMP_ITER(rate_param->active_port_bmp, port) {
        rate_param->port_pkt_hdr_len[port] =
            BCM56990_B0_TRAFFIC_PKT_HDR_LEN_ETH;
    }

    /* Init cells_per_pkt */
    cells_per_pkt = 1;
    sop_size = BCM56990_B0_TRAFFIC_CELL_SOP;
    mop_size = BCM56990_B0_TRAFFIC_CELL_MOP;
    ...
    SHR_IF_ERR_EXIT
        (bcma_testutil_mgmt_port_speed_list_get
            (unit, BCM56990_B0_TRAFFIC_MAX_PIPES, rate_param->mgmt_port_speed));

    /* Init auxiliary bandwidth */
    for (pipe = 0; pipe < BCM56990_B0_TRAFFIC_MAX_PIPES; pipe++) {
        ...
        if (rate_param->mgmt_port_speed[pipe] != 0) {
            ancl_mpps = rate_param->mgmt_port_speed[pipe] /
                        (pkt_size + BCM56990_B0_TRAFFIC_PKT_HDR_LEN_ETH) / 8;
...
static int
bcm56990_b0_rate_set(int unit, bcm56990_b0_rate_param_t *rate_param)
{
    uint32_t pipe, port;
    uint32_t active_pipe_bmp;
    bcmdrd_pbmp_t active_port_bmp;
    uint32_t pipe_io_bw[BCM56990_B0_TRAFFIC_MAX_PIPES];
    ...
    uint64_t pipe_req_mpps_x1000[BCM56990_B0_TRAFFIC_MAX_PIPES];
    uint64_t pipe_req_mcps_x1000[BCM56990_B0_TRAFFIC_MAX_PIPES];
    uint64_t pipe_avail_mpps_x1000[BCM56990_B0_TRAFFIC_MAX_PIPES];
    uint64_t pipe_avail_mcps_x1000[BCM56990_B0_TRAFFIC_MAX_PIPES];
    ...
    sal_memset(pipe_io_bw, 0,
               BCM56990_B0_TRAFFIC_MAX_PIPES * sizeof(uint32_t));
    sal_memset(pipe_req_mpps_x1000, 0,
               BCM56990_B0_TRAFFIC_MAX_PIPES * sizeof(uint64_t));
    ...
    BCMDRD_PBMP_ITER(active_port_bmp, port) {
        pipe = bcmdrd_dev_logic_port_pipe(unit, port);
        if (pipe < BCM56990_B0_TRAFFIC_MAX_PIPES) {
    ...
    /* Set pipe_avail_bw */
    pipe_pp_bw_mbps   = (pkt_size + BCM56990_B0_TRAFFIC_PKT_HDR_LEN_ETH) * 8 *
                        pp_clk_freq / 2;
    pipe_core_bw_mbps = (pkt_size + BCM56990_B0_TRAFFIC_PKT_HDR_LEN_ETH) * 8 *
...
int
bcm56990_b0_traffic_get_expeceted_rate(int unit, bcmdrd_pbmp_t pbmp,
                                       bcmdrd_pbmp_t pbmp_oversub,
                                       int pkt_size, int array_size,
                                       uint64_t *ret_exp_rate)
{
    bcm56990_b0_rate_param_t *rate_param = NULL;
    int port;

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(ret_exp_rate, SHR_E_PARAM);

    SHR_ALLOC(rate_param, sizeof(bcm56990_b0_rate_param_t),
              "bcm56990TestUtilTrafficRateParam");
    SHR_NULL_CHECK(rate_param, SHR_E_MEMORY);
    sal_memset(rate_param, 0, sizeof(bcm56990_b0_rate_param_t));

    /* Init rate parameters */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_rate_init(unit, pbmp, pkt_size, rate_param));

    /* Set expected port rate */
    SHR_IF_ERR_EXIT
        (bcm56990_b0_rate_set(unit, rate_param));
...
static int
bcm56990_b0_traffic_get_cellnums(int unit, uint32_t pkt_len,
                                 uint32_t *ret_cellnums)
{
    uint32_t i = BCM56990_B0_TRAFFIC_CELL_SOP;
    ...
    *ret_cellnums = 1;
    ...
    while (i < pkt_len) {
        (*ret_cellnums)++;
        i += BCM56990_B0_TRAFFIC_CELL_MOP;
    }
...
#define BCM56990_B0_TARGET_CELL_COUNT (300)
                                          ...
                                          uint32_t *ret_pkt_cnt)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_traffic_get_cellnums(unit, pkt_len, &cellnums));

    *ret_pkt_cnt = BCM56990_B0_TARGET_CELL_COUNT / cellnums;
    ...
    *ret_pkt_cnt = 0;
    ...
    while (1) {
        ...
        do {
            ...
            pkt_len = (sal_rand() % (BCM56990_B0_TRAFFIC_MAX_FRAME_SIZE -
                      BCM56990_B0_TRAFFIC_MIN_FRAME_SIZE + 1))
                      + BCM56990_B0_TRAFFIC_MIN_FRAME_SIZE;
            SHR_IF_ERR_EXIT
                (bcm56990_b0_traffic_get_cellnums(unit, pkt_len, &cellnums));
        } while (cellnums > cellnums_limit);

        if (total_cellnums + cellnums > BCM56990_B0_TARGET_CELL_COUNT) {
            break;
        }
                                         ...
                                         uint32_t *ret_pkt_len)
{
    ...
    *ret_pkt_len = BCM56990_B0_TRAFFIC_WORSTCASE_PKT_SIZE;
