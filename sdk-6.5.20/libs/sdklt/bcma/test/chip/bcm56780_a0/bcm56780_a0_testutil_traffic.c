/*! \file bcm56780_a0_testutil_traffic.c
 *
...
#define BCM56780_A0_TRAFFIC_MAX_PIPES        32
#define BCM56780_A0_TRAFFIC_ANCL_BW          12
#define BCM56780_A0_TRAFFIC_PKT_HDR_LEN_ETH  20
#define BCM56780_A0_TRAFFIC_PKT_HDR_LEN_GSH  12
#define BCM56780_A0_TRAFFIC_CELL_SOP         190
#define BCM56780_A0_TRAFFIC_CELL_MOP         254

#define BCM56780_A0_TRAFFIC_LPORTS_PER_PIPE        20
#define BCM56780_A0_TRAFFIC_FP_LPORTS_PER_PIPE     18
#define BCM56780_A0_TRAFFIC_FP_PPORTS_PER_PIPE     40
#define BCM56780_A0_TRAFFIC_PORTS_PER_PM           8
#define BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM      4
#define BCM56780_A0_TRAFFIC_MAX_NUM_PMS            20
#define BCM56780_A0_TRAFFIC_PIPE0_FP_LPORTS_OFFSET 1
#define BCM56780_A0_TRAFFIC_MAX_FRAME_SIZE         9416

#define BCM56780_A0_TRAFFIC_MAX_STRING_SIZE        32

/* Clock frequency name to value map */
static shr_enum_map_t bcm56780_a0_clk_freq_map[]={
    {"CLK_1500MHZ", 1500},
    {"CLK_1350MHZ", 1350},
    {"CLK_950MHZ",  950},
    {NULL, 0}
...
/* Port SPEED */
typedef enum bcm56780_a0_flex_speed_e {
    SPEED_0   = 0,
    SPEED_10G = 10000,
    SPEED_25G = 25000,
    SPEED_40G = 40000,
    ...
    SPEED_200G = 200000,
    SPEED_400G = 400000
} bcm56780_a0_flex_speed_t;

/* Half PM mode */
typedef enum bcm56780_a0_flex_hpm_mode_e {
    HPM_MODE_0 = 0,
    /* VCO rate is 20.625G. */
    HPM_MODE_4X10G,
    HPM_MODE_1X40G,
    ...
    HPM_MODE_RAND,
    HPM_MODE_COUNT
} bcm56780_a0_flex_hpm_mode_t;

/* PM mode string to value map */
static const shr_enum_map_t bcm56780_a0_flex_hpm_mode_map[] = {
    {"0G",           HPM_MODE_0},
...
/* Port fec mode */
static const shr_enum_map_t bcm56780_a0_flex_fec_mode_map[]={
...
/* Flexport parameter */
typedef struct bcm56780_a0_flex_param_s {
    ...
    char pm_mode_str[BCM56780_A0_TRAFFIC_MAX_NUM_PMS]
                    [BCM56780_A0_TRAFFIC_MAX_STRING_SIZE];
    ...
    /* Port fec mode */
    char pport_fec_mode[BCMDRD_CONFIG_MAX_PORTS][BCM56780_A0_TRAFFIC_MAX_STRING_SIZE];
...
} bcm56780_a0_flex_param_t;

/* Rate calculation struct */
typedef struct bcm56780_a0_rate_param_s {
    ...
    uint32_t pipe_ancl_mpps[BCM56780_A0_TRAFFIC_MAX_PIPES];

    /* Auxiliary bandwidth in mega cells per second */
    uint32_t pipe_ancl_mcps[BCM56780_A0_TRAFFIC_MAX_PIPES];
...
} bcm56780_a0_rate_param_t;

static int
bcm56780_a0_rate_init_freq(int unit, bcm56780_a0_rate_param_t *rate_param)
{
    char pp_clk_freq_str[BCM56780_A0_TRAFFIC_MAX_STRING_SIZE];
    char core_clk_freq_str[BCM56780_A0_TRAFFIC_MAX_STRING_SIZE];
    ...
    SHR_IF_ERR_EXIT
        (bcma_testutil_core_clk_freq_get
            (unit, BCM56780_A0_TRAFFIC_MAX_STRING_SIZE, core_clk_freq_str));
    SHR_IF_ERR_EXIT
        (bcma_testutil_pp_clk_freq_get
            (unit, BCM56780_A0_TRAFFIC_MAX_STRING_SIZE, pp_clk_freq_str));

    map_len = COUNTOF(bcm56780_a0_clk_freq_map);
    core_clk_freq = bcma_testutil_name2id(bcm56780_a0_clk_freq_map, map_len,
                                          core_clk_freq_str);
    pp_clk_freq   = bcma_testutil_name2id(bcm56780_a0_clk_freq_map, map_len,
                                          pp_clk_freq_str);
...
static int
bcm56780_a0_rate_init(int unit, bcmdrd_pbmp_t pbmp, int pkt_size,
                      bcm56780_a0_rate_param_t *rate_param)
{
    ...
    BCMDRD_PBMP_ITER(rate_param->active_port_bmp, port) {
        rate_param->port_pkt_hdr_len[port] =
            BCM56780_A0_TRAFFIC_PKT_HDR_LEN_ETH;
    }

    /* Init cells_per_pkt */
    cells_per_pkt = 1;
    sop_size = BCM56780_A0_TRAFFIC_CELL_SOP;
    mop_size = BCM56780_A0_TRAFFIC_CELL_MOP;
    ...
    for (pipe = 0; pipe < BCM56780_A0_TRAFFIC_MAX_PIPES; pipe++) {
        if (((1 << pipe) & active_pipe_bmp) == 0) {
            continue;
        }
        if (pipe % 2 == 0) {
            /* Even pipe contains null/purge slots */
            ancl_mpps = BCM56780_A0_TRAFFIC_ANCL_BW;
            ancl_mcps = BCM56780_A0_TRAFFIC_ANCL_BW;
        } else {
            /* Odd pipe contains 10G management port */
            ancl_mpps = 10 * 1000 / (pkt_size +
                        BCM56780_A0_TRAFFIC_PKT_HDR_LEN_ETH) / 8;
...
static int
bcm56780_a0_rate_set(int unit, bcm56780_a0_rate_param_t *rate_param)
{
    uint32_t pipe, port;
    uint32_t active_pipe_bmp;
    bcmdrd_pbmp_t active_port_bmp;
    uint32_t pipe_io_bw[BCM56780_A0_TRAFFIC_MAX_PIPES];
    ...
    uint64_t pipe_req_mpps_x1000[BCM56780_A0_TRAFFIC_MAX_PIPES];
    uint64_t pipe_req_mcps_x1000[BCM56780_A0_TRAFFIC_MAX_PIPES];
    uint64_t pipe_avail_mpps_x1000[BCM56780_A0_TRAFFIC_MAX_PIPES];
    uint64_t pipe_avail_mcps_x1000[BCM56780_A0_TRAFFIC_MAX_PIPES];
    ...
    SHR_ALLOC(port_req_mpps_x1000, BCMDRD_CONFIG_MAX_PORTS * sizeof(uint64_t),
              "bcm56780TestUtilTrafficReqMpps");
    ...
    SHR_ALLOC(port_req_mcps_x1000, BCMDRD_CONFIG_MAX_PORTS * sizeof(uint64_t),
              "bcm56780TestUtilTrafficReqMcps");
    ...
    SHR_ALLOC(port_avail_mpps_x1000, BCMDRD_CONFIG_MAX_PORTS * sizeof(uint64_t),
              "bcm56780TestUtilTrafficAvailMpps");
    ...
    BCMDRD_PBMP_ITER(active_port_bmp, port) {
        pipe = bcmdrd_dev_logic_port_pipe(unit, port);
        if (pipe < BCM56780_A0_TRAFFIC_MAX_PIPES) {
    ...
    /* Set pipe_avail_bw */
    pipe_pp_bw_mbps   = (pkt_size + BCM56780_A0_TRAFFIC_PKT_HDR_LEN_ETH) * 8 *
                        pp_clk_freq / 2;
    pipe_core_bw_mbps = (pkt_size + BCM56780_A0_TRAFFIC_PKT_HDR_LEN_ETH) * 8 *
...
int
bcm56780_a0_traffic_get_expeceted_rate(int unit, bcmdrd_pbmp_t pbmp,
                                       bcmdrd_pbmp_t pbmp_oversub,
                                       int pkt_size, int array_size,
                                       uint64_t *ret_exp_rate)
{
    bcm56780_a0_rate_param_t *rate_param = NULL;
    int port;

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(ret_exp_rate, SHR_E_PARAM);

    SHR_ALLOC(rate_param, sizeof(bcm56780_a0_rate_param_t),
              "bcm56780TestUtilTrafficRateParam");
    SHR_NULL_CHECK(rate_param, SHR_E_MEMORY);
    sal_memset(rate_param, 0, sizeof(bcm56780_a0_rate_param_t));

    /* Init rate parameters */
    SHR_IF_ERR_EXIT(bcm56780_a0_rate_init(unit, pbmp, pkt_size, rate_param));

    /* Set expected port rate */
    SHR_IF_ERR_EXIT(bcm56780_a0_rate_set(unit, rate_param));
...
static int
bcm56780_a0_traffic_get_cellnums(int unit, uint32_t pkt_len,
                                 uint32_t *ret_cellnums)
{
#define BCM56780_A0_SOP_SIZE (190)
#define BCM56780_A0_MOP_SIZE (254)

    uint32_t i = BCM56780_A0_SOP_SIZE;
...
#define BCM56780_A0_TARGET_CELL_COUNT (300)
                                          ...
                                          uint32_t *ret_pkt_cnt)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56780_a0_traffic_get_cellnums(unit, pkt_len, &cellnums));

    *ret_pkt_cnt = BCM56780_A0_TARGET_CELL_COUNT / cellnums;
...
bcm56780_a0_traffic_get_random_packet_list(int unit, uint32_t cellnums_limit,
                                           ...
                                           uint32_t *ret_pkt_cnt)
{
#define BCM56780_A0_MIN_PKT_SIZE (64)
#define BCM56780_A0_MTU          (9416)
    ...
    *ret_pkt_cnt = 0;
    ...
    while (1) {
        ...
        do {
            ...
            pkt_len = (sal_rand() % (BCM56780_A0_MTU - BCM56780_A0_MIN_PKT_SIZE
                       + 1)) + BCM56780_A0_MIN_PKT_SIZE;
            SHR_IF_ERR_EXIT
                (bcm56780_a0_traffic_get_cellnums(unit, pkt_len, &cellnums));
        } while (cellnums > cellnums_limit);

        if (total_cellnums + cellnums > BCM56780_A0_TARGET_CELL_COUNT) {
            break;
        }
...
bcm56780_a0_traffic_get_worstcase_pktlen(int unit, int port,
                                         uint32_t *ret_pkt_len)
{
#define BCM56780_A0_ENET_WC_PKT_SIZE  (64)

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(ret_pkt_len, SHR_E_PARAM);

    *ret_pkt_len = BCM56780_A0_ENET_WC_PKT_SIZE;
...
static int
bcm56780_a0_flex_pm_mode_name2id(int unit, char *name, uint32_t *id)
{
    int map_len, pm_mode;
    SHR_FUNC_ENTER(unit);

    map_len = COUNTOF(bcm56780_a0_flex_hpm_mode_map);
    pm_mode = bcma_testutil_name2id(bcm56780_a0_flex_hpm_mode_map,
                                    map_len, name);
    if (pm_mode == -1) {
...
static int
bcm56780_a0_flex_avail_lport_get(int unit, uint32_t array_size,
                                 ...
                                 uint32_t *lport, uint32_t *status)
{
    ...
    pipe = (pport - 1) / BCM56780_A0_TRAFFIC_FP_PPORTS_PER_PIPE;
    lport_offset= (pipe == 0) ? BCM56780_A0_TRAFFIC_PIPE0_FP_LPORTS_OFFSET : 0;
    fp_lport_lo = lport_offset + pipe * BCM56780_A0_TRAFFIC_LPORTS_PER_PIPE;
    fp_lport_hi = fp_lport_lo + BCM56780_A0_TRAFFIC_FP_LPORTS_PER_PIPE - 1;
...
static int
bcm56780_a0_flex_pm_speed_get(int unit, uint32_t hpm_mode, uint32_t array_size,
                              uint32_t *speed_array)
{
    uint32_t pm_port_speed[BCM56780_A0_TRAFFIC_PORTS_PER_PM];
    uint32_t i;
    ...
    sal_memset(pm_port_speed, 0,
               BCM56780_A0_TRAFFIC_PORTS_PER_PM * sizeof(uint32_t));

    switch (hpm_mode) {
        case HPM_MODE_4X10G:
            for (i = 0; i < BCM56780_A0_TRAFFIC_PORTS_PER_PM; i++) {
                pm_port_speed[i] = SPEED_10G;
            }
            break;
        case HPM_MODE_1X40G:
            for (i = 0; i < BCM56780_A0_TRAFFIC_PORTS_PER_PM; i++) {
                if ((i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 0) {
        ...
        case HPM_MODE_2X25G_1X50G:
            for (i = 0; i < BCM56780_A0_TRAFFIC_PORTS_PER_PM; i++) {
                if ((i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 0 ||
                    (i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 1) {
                    pm_port_speed[i] = SPEED_25G;
                } else if ((i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 2) {
        ...
        case HPM_MODE_1X50G_2X25G:
            for (i = 0; i < BCM56780_A0_TRAFFIC_PORTS_PER_PM; i++) {
                if ((i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 2 ||
                    (i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 3) {
                    pm_port_speed[i] = SPEED_25G;
                } else if ((i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 0) {
                    pm_port_speed[i] = SPEED_50G;
                }
            }
            break;
        ...
        case HPM_MODE_2X100G:
            for (i = 0; i < BCM56780_A0_TRAFFIC_PORTS_PER_PM; i++) {
                if ((i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 0 ||
                    (i % BCM56780_A0_TRAFFIC_PORTS_PER_HALF_PM) == 2) {
                    pm_port_speed[i] = SPEED_100G;
                }
            }
            break;
        ...
        case HPM_MODE_1X400G:
            for (i = 0; i < BCM56780_A0_TRAFFIC_PORTS_PER_PM; i++) {
                if ((i % BCM56780_A0_TRAFFIC_PORTS_PER_PM) == 0) {
    ...
    for (i = 0; i < array_size; i++) {
        speed_array[i] = i < BCM56780_A0_TRAFFIC_PORTS_PER_PM ?
                         pm_port_speed[i] : 0;
    }
...
static int
bcm56780_a0_flex_speed_by_pm_mode_set(int unit, char *pm_mode_str,
                                      ...
                                      uint32_t *speed_array)
{
    ...
    sal_memset(pm_port_speed, 0,
               BCM56780_A0_TRAFFIC_PORTS_PER_PM * sizeof(uint32_t));
    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_pm_mode_name2id(unit, pm_mode_str, &pm_mode));
    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_pm_speed_get
            (unit, pm_mode, BCM56780_A0_TRAFFIC_PORTS_PER_PM, pm_port_speed));

    /* Set output port speed array */
    for (subp = 0; subp < BCM56780_A0_TRAFFIC_PORTS_PER_PM; subp++) {
        pport = pm_num * BCM56780_A0_TRAFFIC_PORTS_PER_PM + subp + 1;
        if (pport < array_size) {
...
static int
bcm56780_a0_flex_pport2pm_get(int unit, uint32_t pport, uint32_t *pm)
{
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(pm, SHR_E_PARAM);

    *pm = (pport - 1) / BCM56780_A0_TRAFFIC_PORTS_PER_PM;
...
static int
bcm56780_a0_flex_num_lanes_per_pm_get(int unit, uint32_t *num_lanes)
{
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(num_lanes, SHR_E_PARAM);

    *num_lanes = BCM56780_A0_TRAFFIC_PORTS_PER_PM;
...
static int
bcm56780_a0_flex_port_vco_by_pm_mode_get(int unit, char *pm_mode_str,
...
static int
bcm56780_a0_flex_port_num_lanes_get(int unit, char *pm_mode_str,
                                    uint32_t speed, uint8_t *num_lanes)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_port_vco_by_pm_mode_get(unit, pm_mode_str, &vco));
...
static int
bcm56780_a0_flex_port_fec_mode_get(int unit, uint32_t speed,
                                   uint32_t str_size, char *fec_mode_str)
{
    ...
    while (bcm56780_a0_flex_fec_mode_map[idx].name != NULL) {
        if (bcm56780_a0_flex_fec_mode_map[idx].val == (int) speed) {
            local_fec_mode_str = bcm56780_a0_flex_fec_mode_map[idx].name;
            break;
        }
        idx++;
    }
...
static int
bcm56780_a0_flex_port_down(int unit, bcmdrd_pbmp_t down_pport_bmp,
...
static int
bcm56780_a0_flex_port_up(int unit, bcmdrd_pbmp_t up_pport_bmp,
                         uint32_t array_size, uint32_t *p2l_map_array,
                         ...
                         char *pport_fec_mode_array)
{
    ...
    uint32_t max_frame_size = BCM56780_A0_TRAFFIC_MAX_FRAME_SIZE;
...
static int
bcm56780_a0_flex_init_flex_port_bmp(int unit,
                                    bcm56780_a0_flex_param_t *flex_param)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_num_lanes_per_pm_get(unit, &num_lanes_per_pm));

    /* Set port speed by PM mode */
    for (pm = 0; pm < BCM56780_A0_TRAFFIC_MAX_NUM_PMS; pm++) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_flex_speed_by_pm_mode_set
    ...
    BCMDRD_PBMP_ITER(test_pport_bmp, pport) {
        ...
        bcm56780_a0_flex_pport2pm_get(unit, pport, &pm);
    ...
    BCMDRD_PBMP_ITER(test_pport_bmp, port) {
        ...
        bcm56780_a0_flex_pport2pm_get(unit, port, &pm);
        for (subp = 0; subp < num_lanes_per_pm; subp++) {
            pport = pm * num_lanes_per_pm + 1 + subp;
    ...
    BCMDRD_PBMP_ITER(flex_param->up_pport_bmp, pport) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_flex_avail_lport_get
...
static int
bcm56780_a0_flex_init(int unit, bcm56780_a0_flex_param_t *flex_param,
                      ...
                      uint32_t str_size, char *pm_mode_str)
{
    ...
    copy_str_size = str_size < BCM56780_A0_TRAFFIC_MAX_STRING_SIZE ?
                    str_size : BCM56780_A0_TRAFFIC_MAX_STRING_SIZE;
    for (pm = 0; pm < num_pms && pm < BCM56780_A0_TRAFFIC_MAX_NUM_PMS; pm++) {
    ...
    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_init_flex_port_bmp(unit, flex_param));
...
static int
bcm56780_a0_flex_port_down_wrap(int unit, bcm56780_a0_flex_param_t *flex_param)
{
    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(flex_param, SHR_E_PARAM);

    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_port_down
...
static int
bcm56780_a0_flex_port_up_wrap(int unit, bcm56780_a0_flex_param_t *flex_param)
{
    uint32_t pport, pm;
    uint8_t num_lanes = 0;
    SHR_FUNC_ENTER(unit);
    ...
    BCMDRD_PBMP_ITER(flex_param->up_pport_bmp, pport) {
        ...
        SHR_IF_ERR_EXIT(bcm56780_a0_flex_pport2pm_get(unit, pport, &pm));
        SHR_IF_ERR_EXIT
            (bcm56780_a0_flex_port_num_lanes_get
                (unit, flex_param->pm_mode_str[pm],
                 flex_param->flex_pport_speed[pport], &num_lanes));
        flex_param->pport_num_lanes[pport] = num_lanes;

        SHR_IF_ERR_EXIT
            (bcm56780_a0_flex_port_fec_mode_get
                (unit, flex_param->flex_pport_speed[pport],
                 BCM56780_A0_TRAFFIC_MAX_STRING_SIZE - 1,
...
int
bcm56780_a0_flex_do_flex(int unit, bcmdrd_pbmp_t lport_bmp,
                         ...
                         bcmdrd_pbmp_t *up_lport_bmp)
{
    bcm56780_a0_flex_param_t *flex_param = NULL;
    ...
    SHR_ALLOC(flex_param, sizeof(bcm56780_a0_flex_param_t),
              "bcm56780TestUtilTrafficFlexPrm");
    SHR_NULL_CHECK(flex_param, SHR_E_MEMORY);
    sal_memset(flex_param, 0, sizeof(bcm56780_a0_flex_param_t));

    /* Init */
    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_init(unit, flex_param, lport_bmp, phy_pbmp_down,
                               phy_pbmp_up, num_pms, str_size,
                               &pm_mode_str[0]));

    /* Port down */
    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_port_down_wrap(unit, flex_param));

    /* Port up */
    SHR_IF_ERR_EXIT
        (bcm56780_a0_flex_port_up_wrap(unit, flex_param));
...
static bcma_testutil_flex_traffic_op_t bcm56780_a0_flex_traffic_op = {
    .flex_do_flex = bcm56780_a0_flex_do_flex
};

bcma_testutil_flex_traffic_op_t *bcm56780_a0_flex_traffic_op_get(int unit)
{
    return &bcm56780_a0_flex_traffic_op;
}
