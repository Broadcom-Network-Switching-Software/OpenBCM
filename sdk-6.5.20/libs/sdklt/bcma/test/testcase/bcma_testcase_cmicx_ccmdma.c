/*! \file bcma_testcase_cmicx_ccmdma.c
 *
 * Broadcom test cases for CMICX CCMDMA access
 */
/*
...
typedef struct cmicx_ccmdma_test_param_s {
...
} cmicx_ccmdma_test_param_t;

#define TRANS_MEM_SIZE          0x10000
#define CCMDMA_MAGIC_DATA       0xaa555a5a

static int
cmicx_ccmdma_select(int unit)
{
    return bcmdrd_feature_enabled(unit, BCMDRD_FT_CMICX);
}

static int
cmicx_ccmdma_parser(int unit, bcma_cli_t *cli, bcma_cli_args_t *a,
                    uint32_t flags, void **bp)
{
    bcma_cli_parse_table_t pt;
    cmicx_ccmdma_test_param_t *p = NULL;
    uint32_t i;
    ...
    SHR_ALLOC(p, sizeof(*p),"bcmaTestCaseCmicxCCMDMAPrm");
    if (p == NULL) {
...
static void
cmicx_ccmdma_help(int unit, void *bp)
{
    cmicx_ccmdma_test_param_t *p = (cmicx_ccmdma_test_param_t *)bp;
    if (p == NULL) {
        return;
    }
...
static void
cmicx_ccmdma_recycle(int unit, void *bp)
{
    cmicx_ccmdma_test_param_t *p = (cmicx_ccmdma_test_param_t *)bp;
    if (p == NULL) {
        return;
 ...
 */
static int
cmicx_ccmdma_mem_cmp(uint32_t *dst, uint32_t *src, uint32_t wsize)
{
    uint32_t i;
...
static int
cmicx_ccmdma_test(int unit, void *bp, uint32_t option)
{
    cmicx_ccmdma_test_param_t *p = (cmicx_ccmdma_test_param_t *)bp;
    ...
    SHR_IF_ERR_EXIT
        (bcmbd_ccmdma_xfer(unit, &p->work));
    SHR_IF_ERR_EXIT
        (cmicx_ccmdma_mem_cmp(p->dst_buf,
                              p->src_buf,
...
static bcma_test_proc_t cmicx_ccmdma_proc[] = {
    {
        .desc = "run CCM DMA access test.\n",
        .cb = cmicx_ccmdma_test,
    },
};

static bcma_test_op_t cmicx_ccmdma_op = {
    .select = cmicx_ccmdma_select,
    .parser = cmicx_ccmdma_parser,
    .help = cmicx_ccmdma_help,
    .recycle = cmicx_ccmdma_recycle,
    .procs = cmicx_ccmdma_proc,
    .proc_count = COUNTOF(cmicx_ccmdma_proc),
};

bcma_test_op_t *
bcma_testcase_cmicx_ccmdma_op_get(void)
{
    return &cmicx_ccmdma_op;
}
