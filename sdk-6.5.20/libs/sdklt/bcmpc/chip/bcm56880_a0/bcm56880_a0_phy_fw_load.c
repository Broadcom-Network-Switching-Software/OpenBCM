/*! \file bcm56880_a0_phy_fw_load.c
 *
 * BCM56880 PHY firmware loader.
 */
/*
...
#define BCM56880_CDPORT_TSC_UCMEM_DATA_SIZE 65536
                      ...
                      uint32_t fw_size, const uint8_t *fw_data)
{
    ...
    ucmem_idx_max = BCM56880_CDPORT_TSC_UCMEM_DATA_SIZE / FW_ALIGN_BYTES - 1;
...
pm_tscbh_firmware_load(int unit, int *last_bcast_port, bcmdrd_pbmp_t *pbmp)
{
    ...
    /* Check chip version. */
    SHR_IF_ERR_EXIT
        (pc->dev_revision_a0_get(unit, &is_rev_a0));
    ...
    if (fw_cfg.fw_load == PM_SERDES_FW_LOAD_SLOW ) {
        BCMDRD_PBMP_ITER(pbmp[0], port) {
            ...
            /* Set version A0 flag. */
            if (is_rev_a0) {
                PHYMOD_CORE_INIT_F_TOP_DEV_REV_ID_A0_SET(cic);
            }
            cs->pmd_active = 0;
            SHR_IF_ERR_EXIT
                (phymod_core_init(ca, cic, cs));
