/*! \file bcmptm_bcm56996_a0_rm_alpm.c
 *
...
#include "bcmptm_bcm56996_a0_rm_alpm.h"
...
#define BCM56996_ALPM_MAX_VRF   8191
#define BCM56996_ALPM_VRF_MASK  8191
#define BCM56996_ALPM_MAX_COMP_PORT     0xFFFF
#define BCM56996_ALPM_COMP_PORT_MASK    0xFFFF
...
#define KSHIFT_PATCH1(u, arg, v) ((v) + bcm56996_kshift(u, arg))
#define KSHIFT_PATCH2(u, arg, v) ((v) -= bcm56996_kshift(u, arg))
...
typedef union bcm56996_assoc_data_s {
    ASSOC_DATA_FULL_t full;
    ASSOC_DATA_REDUCED_t red;
    uint32_t v[2];
} bcm56996_assoc_data_t;
...
static int bcm56996_l2p_route_fmts[] =
    {0, 13, 1, 2, 3, 4, 5, 6, 7, 8, 9, 14, 10, 11, 12, 15};

/* Mapping physical formats to logical formats. */
static int bcm56996_p2l_route_fmts[] =
    {0, 2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 13, 14, 1, 11, 15};

static format_info_t bcm56996_route_base_entry_info[] = {
...
/* Mapping logical formats to physical formats. */
static int bcm56996_l2p_pivot_fmts[] =
    {0, 1, 2, 3, 4, 5, 9, 6, 7, 8, 10};

/* Mapping physical formats to logical formats. */
static int bcm56996_p2l_pivot_fmts[] =
    {0, 1, 2, 3, 4, 5, 7, 8, 9, 6, 10};

static format_info_t bcm56996_pivot_base_entry_info[] = {
    {.prefix_len = 0,   .full = 0, .num_entries = 0}, /* 0 (L) - 0 (P) */
...
static void
bcm56996_l1_lpm_vt_decode(int u, int sub_prio, int vrf,
                          int vrf_mask, alpm_arg_t *arg)
{
    if (sub_prio == SUB_DB_PRIORITY_OVERRIDE) {
        ...
        ALPM_ASSERT(arg->lpm_large_vrf || vrf_mask == BCM56996_ALPM_VRF_MASK);
        ALPM_ASSERT(arg->lpm_large_vrf || vrf <= BCM56996_ALPM_MAX_VRF);
...
static void
bcm56996_l1_comp_vt_decode(int u, bcmptm_cth_alpm_comp_key_type_t comp_key_type,
                           int vrf, int vrf_mask, alpm_arg_t *arg)
{
    switch (comp_key_type) {
        case COMP_KEY_L4_PORT:
            if (vrf_mask == BCM56996_ALPM_COMP_PORT_MASK) {
                arg->key.vt = VRF_PRIVATE;
                arg->key.t.w_vrf = vrf;
            } else {
                arg->key.vt = VRF_GLOBAL;
            ...
            break;
        case COMP_KEY_VRF:
            if (vrf_mask == BCM56996_ALPM_VRF_MASK) {
                arg->key.vt = VRF_PRIVATE;
                arg->key.t.w_vrf = vrf;
            } else {
                arg->key.vt = VRF_GLOBAL;
...
static void
bcm56996_l1_key_fmt0_lpm_v4_encode(int u, int vrf, int key_mode,
...
static void
bcm56996_l1_key_fmt0_lpm_v4_encode_no_vrf(int u, int key_mode,
...
static void
bcm56996_l1_key_fmt0_lpm_v6_double_encode(int u, int vrf, int key_mode,
                                          alpm_ip_ver_t v6, uint32_t *ip_addr,
...
static void
bcm56996_l1_key_fmt0_lpm_v6_double_encode_no_vrf(int u, int key_mode,
                                                 alpm_ip_ver_t v6, uint32_t *udata,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata1,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata2,
...
static void
bcm56996_l1_key_fmt0_lpm_v6_single_encode(int u, int vrf, int key_mode,
                                          alpm_ip_ver_t v6, uint32_t *ip_addr,
                                          L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
                                          L3_DEFIP_TCAM_KEY_FMT0_t *kdata1
                                          )
...
static void
bcm56996_l1_key_fmt0_lpm_v6_single_encode_no_vrf(int u, int key_mode,
                                                 alpm_ip_ver_t v6, uint32_t *udata,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata1
                                                 )
...
/* For V4, V6-0 */
static void
bcm56996_l1_key_fmt0_lpm_v4_decode(int u, int *vrf, int *key_mode,
...
/* For V4, V6-0 */
static void
bcm56996_l1_key_fmt0_lpm_v4_decode_no_vrf(int u, int *key_mode,
...
static void
bcm56996_l1_key_fmt0_lpm_v6_double_decode(int u, int *vrf, int *key_mode,
                                          int *kt, uint32_t *ip_addr,
                                          L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
                                          L3_DEFIP_TCAM_KEY_FMT0_t *kdata1,
                                          L3_DEFIP_TCAM_KEY_FMT0_t *kdata2,
...
static void
bcm56996_l1_key_fmt0_lpm_v6_double_decode_no_vrf(int u, int *key_mode,
                                                 int *kt, uint32_t *udata,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata1,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata2,
...
static void
bcm56996_l1_key_fmt0_lpm_v6_single_decode(int u, int *vrf, int *key_mode,
                                          int *kt, uint32_t *ip_addr,
                                          L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
                                          L3_DEFIP_TCAM_KEY_FMT0_t *kdata1)
{
...
static void
bcm56996_l1_key_fmt0_lpm_v6_single_decode_no_vrf(int u, int *key_mode,
                                                 int *kt, uint32_t *udata,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
                                                 L3_DEFIP_TCAM_KEY_FMT0_t *kdata1)
{
...
static void
bcm56996_l1_key_fmt1_lpm_v4_encode(int u, int vrf, int key_mode,
                                   alpm_ip_ver_t v6, uint32_t ip_addr,
...
static void
bcm56996_l1_key_fmt1_lpm_v4_encode_no_vrf(int u,  int key_mode,
                                          alpm_ip_ver_t v6, uint32_t *udata,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata)
{
    LPM_V4_KEY_t *lpm_v4 = (LPM_V4_KEY_t *)udata;
...
static void
bcm56996_l1_key_fmt1_lpm_v6_double_encode(int u, int vrf, int key_mode,
                                          alpm_ip_ver_t v6, uint32_t *ip_addr,
...
static void
bcm56996_l1_key_fmt1_lpm_v6_double_encode_no_vrf(int u, int key_mode,
                                                 alpm_ip_ver_t v6, uint32_t *udata,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata1,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata2,
...
static void
bcm56996_l1_key_fmt1_lpm_v6_single_encode(int u, int vrf, int key_mode,
                                          alpm_ip_ver_t v6, uint32_t *ip_addr,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata1
                                          )
...
static void
bcm56996_l1_key_fmt1_lpm_v6_single_encode_no_vrf(int u, int key_mode,
                                                 alpm_ip_ver_t v6, uint32_t *udata,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata1
                                                 )
...
/* For V4, V6-0 */
static void
bcm56996_l1_key_fmt1_lpm_v4_decode(int u, int *vrf, int *key_mode,
                                   int *kt, uint32_t *ip_addr,
                                   L3_DEFIP_TCAM_KEY_FMT1_t *kdata)
{
    LPM_V4_KEY_t lpm_v4 = {{0}};
...
/* For V4, V6-0 */
static void
bcm56996_l1_key_fmt1_lpm_v4_decode_no_vrf(int u, int *key_mode,
                                          int *kt, uint32_t *udata,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata)
{
    LPM_V4_KEY_t *lpm_v4 = (LPM_V4_KEY_t *)udata;
...
static void
bcm56996_l1_key_fmt1_lpm_v6_double_decode(int u, int *vrf, int *key_mode,
                                          int *kt, uint32_t *ip_addr,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata1,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata2,
...
static void
bcm56996_l1_key_fmt1_lpm_v6_double_decode_no_vrf(int u, int *key_mode,
                                                 int *kt, uint32_t *udata,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata1,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata2,
...
static void
bcm56996_l1_key_fmt1_lpm_v6_single_decode(int u, int *vrf, int *key_mode,
                                          int *kt, uint32_t *ip_addr,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                          L3_DEFIP_TCAM_KEY_FMT1_t *kdata1)
{
...
static void
bcm56996_l1_key_fmt1_lpm_v6_single_decode_no_vrf(int u, int *key_mode,
                                                 int *kt, uint32_t *udata,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                                 L3_DEFIP_TCAM_KEY_FMT1_t *kdata1)
{
...
static void
bcm56996_l1_key_fmt1_l3mc_v6_double_encode(int u, int key_mode,
                                           int kt, uint32_t *udata,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata1,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata2,
...
static void
bcm56996_l1_key_fmt1_l3mc_v4_single_encode(int u, int key_mode,
                                           int kt, uint32_t *udata,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata1
                                           )
...
static void
bcm56996_l1_key_fmt1_l3mc_v6_double_decode(int u, int *key_mode,
                                           int *kt, uint32_t *udata,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata1,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata2,
...
static void
bcm56996_l1_key_fmt1_l3mc_v4_single_decode(int u, int *key_mode,
                                           int *kt, uint32_t *udata,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata0,
                                           L3_DEFIP_TCAM_KEY_FMT1_t *kdata1)
{
...
static void
bcm56996_l1_key_fmt0_comp_v6_double_encode(int u,
...
static void
bcm56996_l1_key_fmt0_comp_v4_double_encode(int u,
                                           bcmptm_cth_alpm_comp_key_type_t comp_key_type,
                                           int vrf, int key_mode,
                                           int kt, uint32_t *ip_addr,
                                           L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
...
static void
bcm56996_l1_key_fmt0_comp_v6_double_decode(int u,
                                           bcmptm_cth_alpm_comp_key_type_t comp_key_type,
                                           int *vrf, int *key_mode,
                                           int *kt, uint32_t *ip_addr,
                                           L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
...
static void
bcm56996_l1_key_fmt0_comp_v4_double_decode(int u,
                                           bcmptm_cth_alpm_comp_key_type_t comp_key_type,
                                           int *vrf, int *key_mode,
                                           int *kt, uint32_t *ip_addr,
                                           L3_DEFIP_TCAM_KEY_FMT0_t *kdata0,
...
static void
bcm56996_ln_assoc_data_format_encode(int u, bcm56996_assoc_data_t *buf,
                                     int full,
                                     assoc_data_t *assoc)
{
...
static inline void
bcm56996_ln_assoc_data_format_decode(int u, bcm56996_assoc_data_t *buf,
                                     int full,
                                     assoc_data_t *assoc,
                                     uint8_t app)
{
...
static int
bcm56996_kshift(int u, alpm_arg_t *arg)
{
    int kshift = 0;
...
static void
bcm56996_ln_alpm2_data_format_encode(int u, ALPM2_DATA_t *a2_format,
                                     alpm2_data_t *a2_data, alpm_arg_t *arg)
{
    ...
    FMT_ALPM2_DATA_FIELD_SET(FMT0f, bcm56996_l2p_route_fmts[a2_data->fmt[0]]);
    FMT_ALPM2_DATA_FIELD_SET(FMT1f, bcm56996_l2p_route_fmts[a2_data->fmt[1]]);
    FMT_ALPM2_DATA_FIELD_SET(FMT2f, bcm56996_l2p_route_fmts[a2_data->fmt[2]]);
    FMT_ALPM2_DATA_FIELD_SET(FMT3f, bcm56996_l2p_route_fmts[a2_data->fmt[3]]);
    FMT_ALPM2_DATA_FIELD_SET(FMT4f, bcm56996_l2p_route_fmts[a2_data->fmt[4]]);
    FMT_ALPM2_DATA_FIELD_SET(FMT5f, bcm56996_l2p_route_fmts[a2_data->fmt[5]]);
    FMT_ALPM2_DATA_FIELD_SET(FMT6f, bcm56996_l2p_route_fmts[a2_data->fmt[6]]);
    FMT_ALPM2_DATA_FIELD_SET(FMT7f, bcm56996_l2p_route_fmts[a2_data->fmt[7]]);
}


static void
bcm56996_ln_alpm2_data_format_decode(int u, ALPM2_DATA_t *a2_format,
                                     alpm2_data_t *a2_data, alpm_arg_t *arg)
{
    int i;
    ...
    for (i = 0; i < ALPM2_DATA_FORMATS; i++) {
        a2_data->fmt[i] = bcm56996_p2l_route_fmts[a2_data->fmt[i]];
    }
}


static void
bcm56996_ln_alpm1_data_format_encode(int u, void *buf, alpm1_data_t *a1_data,
                                     alpm_arg_t *arg)
{
    ...
    if (a1_data->is_route) {
        FMT_ALPM1_DATA_FIELD_SET(FMT0f, bcm56996_l2p_route_fmts[a1_data->fmt[0]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT1f, bcm56996_l2p_route_fmts[a1_data->fmt[1]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT2f, bcm56996_l2p_route_fmts[a1_data->fmt[2]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT3f, bcm56996_l2p_route_fmts[a1_data->fmt[3]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT4f, bcm56996_l2p_route_fmts[a1_data->fmt[4]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT5f, bcm56996_l2p_route_fmts[a1_data->fmt[5]]);
    } else {
        FMT_ALPM1_DATA_FIELD_SET(FMT0f, bcm56996_l2p_pivot_fmts[a1_data->fmt[0]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT1f, bcm56996_l2p_pivot_fmts[a1_data->fmt[1]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT2f, bcm56996_l2p_pivot_fmts[a1_data->fmt[2]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT3f, bcm56996_l2p_pivot_fmts[a1_data->fmt[3]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT4f, bcm56996_l2p_pivot_fmts[a1_data->fmt[4]]);
        FMT_ALPM1_DATA_FIELD_SET(FMT5f, bcm56996_l2p_pivot_fmts[a1_data->fmt[5]]);
    }
}

static void
bcm56996_ln_alpm1_data_format_decode(int u, void *buf, alpm1_data_t *a1_data,
                                     alpm_arg_t *arg)
{
    int i;
    ALPM1_DATA_t *a1_format = buf;
    ...
    for (i = 0; i < ALPM1_DATA_FORMATS; i++) {
        if (a1_data->is_route) {
            a1_data->fmt[i] = bcm56996_p2l_route_fmts[a1_data->fmt[i]];
        } else {
            a1_data->fmt[i] = bcm56996_p2l_pivot_fmts[a1_data->fmt[i]];
        }
    }
}

static void
bcm56996_ln_pivot_base_entry_info(int u, uint8_t format_value,
                                  int *prefix_len,
                                  int *full,
                                  int *number)
{
    ALPM_ASSERT(format_value < COUNTOF(bcm56996_pivot_base_entry_info));
    *prefix_len = bcm56996_pivot_base_entry_info[format_value].prefix_len;
    *full = bcm56996_pivot_base_entry_info[format_value].full;
    *number = bcm56996_pivot_base_entry_info[format_value].num_entries;
...
static void
bcm56996_ln_pivot_format_full_encode2(int u, uint8_t format_value,
                                      uint32_t *base_entry,
                                      int ent_ofs,
                                      void *buf)
{
    format_value = bcm56996_l2p_pivot_fmts[format_value];
    switch (format_value) {
    case 1: {
...
static void
bcm56996_ln_pivot_format_full_encode(int u, uint8_t format_value,
...
static void
bcm56996_ln_pivot_format_full_decode2(int u, uint8_t format_value,
                                      uint32_t *base_entry,
                                      int ent_ofs,
                                      void *buf)
{
...
static void
bcm56996_ln_pivot_format_full_decode(int u, uint8_t format_value,
                                     ln_base_entry_t *base_entry,
                                     int entry_count,
                                     void *buf)
{
...
static void
bcm56996_ln_pivot_format_set_data(uint8_t format_value,
                                  uint32_t *ad_format,
                                  void *buf)
{
    format_value = bcm56996_l2p_pivot_fmts[format_value];
...
static void
bcm56996_ln_pivot_format_set(uint8_t format_value,
                             uint32_t *ad_format,
                             uint32_t *a2_format,
...
static void
bcm56996_ln_pivot_format_encode(int u, uint8_t format_value,
                                uint8_t kshift,
                                alpm_arg_t *arg,
                                void *buf)
{
    int full = 0, number;
    int max_len = 0;
    bcm56996_assoc_data_t ad_format;
    bcm56996_ln_pivot_base_entry_info(u, format_value, &max_len, &full,
                                      &number);
    bcm56996_ln_assoc_data_format_encode(u, &ad_format, full, &arg->ad);

    if (arg->encode_data_only) {
        bcm56996_ln_pivot_format_set_data(format_value,
                                          &ad_format.v[0],
                                          buf);
    } else {
        alpm_format_ip_t prefix;
        ALPM2_DATA_t a2_format;
        bcm56996_ln_alpm2_data_format_encode(u, &a2_format, &arg->a2, arg);
        bcmptm_rm_alpm_one_bit_encode(arg->key.t.max_bits,
                                      ...
                                      kshift,
                                      &prefix[0]);
        bcm56996_ln_pivot_format_set(format_value,
                                     &ad_format.v[0],
                                     &a2_format.v[0],
                                     &prefix[0],
                                     buf);
...
static void
bcm56996_ln_pivot_format_get_data(uint8_t format_value,
                                  uint32_t *ad_format,
                                  void *buf)
{
    format_value = bcm56996_l2p_pivot_fmts[format_value];
...
static void
bcm56996_ln_pivot_format_get(uint8_t format_value,
                             uint32_t *ad_format,
                             uint32_t *a2_format,
                             alpm_format_ip_t prefix,
                             void *buf)
...
static void
bcm56996_ln_pivot_format_decode(int u, uint8_t format_value,
                                uint8_t kshift,
                                alpm_arg_t *arg,
                                void *buf)
{
    ...
    int max_len;
    ALPM2_DATA_t a2_format;
    bcm56996_assoc_data_t ad_format = {{{0}}};
    alpm_format_ip_t prefix = {0};
    bcm56996_ln_pivot_base_entry_info(u, format_value, &max_len, &full,
                                      &number);
    if (arg->decode_data_only) {
        bcm56996_ln_pivot_format_get_data(format_value,
                                          &ad_format.v[0],
                                          buf);
    } else {
        bcm56996_ln_pivot_format_get(format_value,
                                     &ad_format.v[0],
                                     &a2_format.v[0],
                                     &prefix[0],
                                     buf);
        bcm56996_ln_alpm2_data_format_decode(u, &a2_format, &arg->a2, arg);
    ...
    bcm56996_ln_assoc_data_format_decode(u, &ad_format, full, &arg->ad,
                                         arg->key.t.app);
}

static void
bcm56996_ln_route_base_entry_info(int u, uint8_t format_value,
                                  int *prefix_len,
                                  int *full,
                                  int *number)
{
    ALPM_ASSERT(format_value < COUNTOF(bcm56996_route_base_entry_info));
    *prefix_len = bcm56996_route_base_entry_info[format_value].prefix_len;
    *full = bcm56996_route_base_entry_info[format_value].full;
    *number = bcm56996_route_base_entry_info[format_value].num_entries;
...
static void
bcm56996_ln_route_format_full_encode2(int u, uint8_t format_value,
                                      uint32_t *base_entry,
                                      int ent_ofs,
                                      void *buf)
{
    format_value = bcm56996_l2p_route_fmts[format_value];
    switch (format_value) {
    case 1: {
...
static void
bcm56996_ln_route_format_full_encode(int u, uint8_t format_value,
                                     ln_base_entry_t *base_entry,
                                     int entry_count,
                                     void *buf)
{
...
static void
bcm56996_ln_route_format_full_decode2(int u, uint8_t format_value,
                                      uint32_t *base_entry,
                                      int ent_ofs,
                                      void *buf)
{
...
static void
bcm56996_ln_route_format_full_decode(int u, uint8_t format_value,
                                     ln_base_entry_t *base_entry,
                                     int entry_count,
                                     void *buf)
{
...
static int
bcm56996_ln_base_entry_info(int u, int m,
                            ...
                            int *num_entry)
{
    ...
    if (format_type == FORMAT_ROUTE) {
        bcm56996_ln_route_base_entry_info(u, format_value, &a, &b, &c);
    } else {
        bcm56996_ln_pivot_base_entry_info(u, format_value, &a, &b, &c);
    }
    if (prefix_len) {
        *prefix_len = a;
    }
...
static void
bcm56996_ln_route_format_set_data(uint8_t format_value,
                                  uint32_t *ad_format,
                                  void *buf)
{
    format_value = bcm56996_l2p_route_fmts[format_value];
...
static void
bcm56996_ln_route_format_set(uint8_t format_value,
                             uint32_t *ad_format,
                             alpm_format_ip_t prefix,
                             void *buf)
{
...
static void
bcm56996_ln_route_format_encode(int u, uint8_t format_value,
                                uint8_t kshift,
                                alpm_arg_t *arg,
                                void *buf)
{
    ...
    int number;
    bcm56996_assoc_data_t ad_format;
    bcm56996_ln_route_base_entry_info(u, format_value, &max_len, &full,
                                      &number);
    bcm56996_ln_assoc_data_format_encode(u, &ad_format, full, &(arg->ad));
    if (arg->encode_data_only) {
        bcm56996_ln_route_format_set_data(format_value,
                                          &ad_format.v[0],
                                          buf);
    } else {
        alpm_format_ip_t prefix;
        bcmptm_rm_alpm_one_bit_encode(arg->key.t.max_bits,
                                      ...
                                      kshift,
                                      &prefix[0]);
        bcm56996_ln_route_format_set(format_value,
                                     &ad_format.v[0],
                                     &prefix[0],
                                     buf);
    }
...
static void
bcm56996_ln_route_format_get_data(uint8_t format_value,
                                  uint32_t *ad_format,
                                  void *buf)
{
    format_value = bcm56996_l2p_route_fmts[format_value];
...
static void
bcm56996_ln_route_format_get(uint8_t format_value,
                             uint32_t *ad_format,
                             alpm_format_ip_t prefix,
                             void *buf)
{
...
static void
bcm56996_ln_route_format_decode(int u, uint8_t format_value,
                                uint8_t kshift,
                                alpm_arg_t *arg,
                                void *buf)
{
    ...
    bcm56996_ln_route_base_entry_info(u, format_value, &max_len, &full,
                                      &number);
    if (arg->decode_data_only) {
        bcm56996_ln_route_format_get_data(format_value,
                                          &ad_format.v[0],
                                          buf);
    } else {
        bcm56996_ln_route_format_get(format_value,
                                     &ad_format.v[0],
                                     &prefix[0],
                                     buf);
        arg->valid = bcmptm_rm_alpm_one_bit_decode(arg->key.t.max_bits,
...
static int
bcm56996_ln_raw_bucket_encode(int u, int m, format_type_t format_type,
                              uint8_t format_value,
                              ln_base_entry_t *base_entry,
                              int entry_count,
                              void *bucket)
{
    if (format_type == FORMAT_ROUTE) {
        bcm56996_ln_route_format_full_encode(u, format_value, base_entry,
                                             entry_count, bucket);
    } else {
        bcm56996_ln_pivot_format_full_encode(u, format_value, base_entry,
                                             entry_count, bucket);
    }
    return SHR_E_NONE;
}
...
static int
bcm56996_ln_raw_bucket_entry_set(int u, int m, format_type_t format_type,
                                 uint8_t format_value,
                                 uint32_t *base_entry,
                                 int ent_ofs,
                                 void *bucket)
{
    if (format_type == FORMAT_ROUTE) {
        bcm56996_ln_route_format_full_encode2(u, format_value, base_entry,
                                              ent_ofs, bucket);
    } else {
        bcm56996_ln_pivot_format_full_encode2(u, format_value, base_entry,
                                              ent_ofs, bucket);
    }
    return SHR_E_NONE;
}
...
static int
bcm56996_ln_raw_bucket_decode(int u, int m, format_type_t format_type,
                              uint8_t format_value,
                              ln_base_entry_t *base_entry,
                              int entry_count,
                              void *bucket)
{
    if (format_type == FORMAT_ROUTE) {
        bcm56996_ln_route_format_full_decode(u, format_value, base_entry,
                                             entry_count, bucket);
    } else {
        bcm56996_ln_pivot_format_full_decode(u, format_value, base_entry,
                                             entry_count, bucket);
    }
    return SHR_E_NONE;
}
...
static int
bcm56996_ln_raw_bucket_entry_get(int u, int m, format_type_t format_type,
                                 uint8_t format_value,
                                 uint32_t *base_entry,
                                 int ent_ofs,
                                 void *bucket)
{
    if (format_type == FORMAT_ROUTE) {
        bcm56996_ln_route_format_full_decode2(u, format_value, base_entry,
                                              ent_ofs, bucket);
    } else {
        bcm56996_ln_pivot_format_full_decode2(u, format_value, base_entry,
                                              ent_ofs, bucket);
    }
    return SHR_E_NONE;
}
...
static int
bcm56996_ln_base_entry_encode(int u, int m, format_type_t format_type,
                              uint8_t format_value,
                              uint8_t kshift,
                              alpm_arg_t *arg,
                              uint32_t *entry)
{
    if (format_type == FORMAT_ROUTE) {
        bcm56996_ln_route_format_encode(u, format_value, kshift, arg, entry);
    } else {
        bcm56996_ln_pivot_format_encode(u, format_value, kshift, arg, entry);
    }
    return SHR_E_NONE;
}

static int
bcm56996_ln_base_entry_decode(int u, int m, format_type_t format_type,
                              uint8_t format_value,
                              uint8_t kshift,
                              alpm_arg_t *arg,
                              uint32_t *entry)
{
    if (format_type == FORMAT_ROUTE) {
        bcm56996_ln_route_format_decode(u, format_value, kshift, arg, entry);
    } else {
        bcm56996_ln_pivot_format_decode(u, format_value, kshift, arg, entry);
    }
    return SHR_E_NONE;
}

static int
bcm56996_ln_base_entry_move(int u, int m, format_type_t format_type,
                            ...
                            void *dst_bucket)
{
    ...
    SHR_IF_ERR_EXIT(
        bcm56996_ln_raw_bucket_entry_get(u, m, format_type,
                                         src_format_value,
                                         &src_base_entry[0],
                                         src_ent_ofs,
                                         src_bucket));
    ...
    if (src_format_value == dst_format_value &&
        ...
        if (0) {
            ...
            SHR_IF_ERR_EXIT(
                bcm56996_ln_base_entry_decode(u, m, format_type,
                                              src_format_value,
                                              src_kshift,
                                              arg,
                                              &src_base_entry[0]));
            ...
            SHR_IF_ERR_EXIT(
                bcm56996_ln_base_entry_encode(u, m, format_type,
                                              dst_format_value,
                                              dst_kshift,
                                              arg,
                                              &dst_base_entry[0]));
...
/*        bcmptm_rm_alpm_arg_init(u, arg);*/
    ...
    SHR_IF_ERR_EXIT(
        bcm56996_ln_raw_bucket_entry_set(u, m, format_type,
                                         dst_format_value,
                                         dst_base,
                                         dst_ent_ofs,
                                         dst_bucket));
 ...
 */
static void
bcm56996_l1_fmt0_narrow_encode_data_n1(int u, alpm_arg_t *arg, UNPAIR0_T *de,
                                       UNPAIR0_T *se, int upkm)
{
    bcm56996_assoc_data_t ad_format;
    bcm56996_ln_assoc_data_format_encode(u, &ad_format, 1, &arg->ad);
 ...
 */
static void
bcm56996_l1_fmt0_narrow_lpm_encode_n1(int u, alpm_arg_t *arg, UNPAIR0_T *de,
                                  UNPAIR0_T *se, int upkm)
{
    L3_DEFIP_TCAM_KEY_FMT0_t kdata;
    ...
    if (arg->key.vt == VRF_OVERRIDE) {
        ...
        vrf = arg->key.t.w_vrf;
        vrf_mask = BCM56996_ALPM_VRF_MASK;
    }
    ...
    if (arg->key.kt == KEY_IPV4) {
        ...
        if (large_vrf) {
            bcm56996_l1_key_fmt0_lpm_v4_encode_no_vrf(u, HALF_WIDE, IP_VER_4,
                                                      &arg->key.ip_addr[0], &kdata);
            bcm56996_l1_key_fmt0_lpm_v4_encode_no_vrf(u, 0x3, 0x1,
                                                      &arg->key.ip_mask[0], &kmask);
        } else {
            bcm56996_l1_key_fmt0_lpm_v4_encode(u, vrf, HALF_WIDE, IP_VER_4,
                                               arg->key.ip_addr[0], &kdata);
            bcm56996_l1_key_fmt0_lpm_v4_encode(u, vrf_mask, 0x3, 0x1,
                                               arg->key.ip_mask[0], &kmask);
        }
        ...
        if (large_vrf) {
            bcm56996_l1_key_fmt0_lpm_v4_encode_no_vrf(u, HALF_WIDE, IP_VER_6,
                                                      &arg->key.ip_addr[0], &kdata);
            bcm56996_l1_key_fmt0_lpm_v4_encode_no_vrf(u, 0x3, 0x1,
                                                      &arg->key.ip_mask[0], &kmask);
        } else {
            bcm56996_l1_key_fmt0_lpm_v4_encode(u, vrf, HALF_WIDE, IP_VER_6,
    ...
    bcm56996_ln_alpm1_data_format_encode(u, &a1_format, &arg->a1, arg);
    bcm56996_ln_assoc_data_format_encode(u, &ad_format, 1, &arg->ad);
...
static void
bcm56996_l1_fmt0_narrow_encode_data(int u, alpm_arg_t *arg, UNPAIR0_T *de,
                                    UNPAIR0_T *se, int upkm)
{
    bcm56996_assoc_data_t ad_format;
 ...
 */
static void
bcm56996_l1_fmt0_narrow_lpm_encode(int u, alpm_arg_t *arg, UNPAIR0_T *de,
                                   UNPAIR0_T *se, int upkm)
{
    ...
    if (arg->key.kt == KEY_IPV4) {
        ...
        if (large_vrf) {
            bcm56996_l1_key_fmt0_lpm_v6_single_encode_no_vrf(u, SINGLE_WIDE,
                                                             IP_VER_6,
                                                             &arg->key.ip_addr[0],
                                                             &kdata[0],
                                                             &kdata[1]);
            bcm56996_l1_key_fmt0_lpm_v6_single_encode_no_vrf(u, 0x3, 0x1,
                                                             &arg->key.ip_mask[0],
                                                             &kmask[0],
                                                             &kmask[1]);
        } else {
            bcm56996_l1_key_fmt0_lpm_v6_single_encode(u, vrf, SINGLE_WIDE,
                                                      IP_VER_6,
                                                      &arg->key.ip_addr[0],
                                                      &kdata[0],
                                                      &kdata[1]);
            bcm56996_l1_key_fmt0_lpm_v6_single_encode(u, vrf_mask, 0x3, 0x1,
                                                      &arg->key.ip_mask[0],
                                                      &kmask[0],
                                                      &kmask[1]);
        }
...
static void
bcm56996_l1_fmt0_wide_encode_data(int u, alpm_arg_t *arg, PAIR0_T *de, PAIR0_T *se)
{
    bcm56996_assoc_data_t ad_format;
    bcm56996_ln_assoc_data_format_encode(u, &ad_format, 1, &arg->ad);
 ...
 */
static void
bcm56996_l1_fmt0_wide_lpm_encode(int u, alpm_arg_t *arg, PAIR0_T *de, PAIR0_T *se)
{
    ...
    large_vrf = arg->lpm_large_vrf && arg->key.vt != VRF_OVERRIDE;
    if (large_vrf) {
        bcm56996_l1_key_fmt0_lpm_v6_double_encode_no_vrf(u, DOUBLE_WIDE, IP_VER_6,
                                                         &arg->key.ip_addr[0],
                                                         &kdata[0],
                                                         &kdata[1],
                                                         &kdata[2],
                                                         &kdata[3]);
        bcm56996_l1_key_fmt0_lpm_v6_double_encode_no_vrf(u, 0x3, 0x1,
                                                         &arg->key.ip_mask[0],
                                                         &kmask[0],
                                                         &kmask[1],
                                                         &kmask[2],
        ...
        bcm56996_l1_key_fmt0_lpm_v6_double_encode(u, vrf, DOUBLE_WIDE, IP_VER_6,
                                                  &arg->key.ip_addr[0],
                                                  &kdata[0],
                                                  &kdata[1],
                                                  &kdata[2],
                                                  &kdata[3]);
        bcm56996_l1_key_fmt0_lpm_v6_double_encode(u, vrf_mask, 0x3, 0x1,
                                                  &arg->key.ip_mask[0],
                                                  &kmask[0],
                                                  &kmask[1],
                                                  &kmask[2],
 ...
 */
static void
bcm56996_l1_fmt1_narrow_lpm_encode_n1(int u, alpm_arg_t *arg, UNPAIR1_T *de,
                                  UNPAIR1_T *se, int upkm)
{
    ...
    if (arg->key.kt == KEY_IPV4) {
        ...
        if (large_vrf) {
            bcm56996_l1_key_fmt1_lpm_v4_encode_no_vrf(u, HALF_WIDE, IP_VER_4,
                                                      &arg->key.ip_addr[0], &kdata);
            bcm56996_l1_key_fmt1_lpm_v4_encode_no_vrf(u, 0x3, 0x3,
                                                      &arg->key.ip_mask[0], &kmask);

        } else {
            bcm56996_l1_key_fmt1_lpm_v4_encode(u, vrf, HALF_WIDE, IP_VER_4,
                                               arg->key.ip_addr[0], &kdata);
            bcm56996_l1_key_fmt1_lpm_v4_encode(u, vrf_mask, 0x3, 0x3,
                                               arg->key.ip_mask[0], &kmask);
        ...
        if (large_vrf) {
            bcm56996_l1_key_fmt1_lpm_v4_encode_no_vrf(u, HALF_WIDE, IP_VER_6,
                                                      &arg->key.ip_addr[0], &kdata);
            bcm56996_l1_key_fmt1_lpm_v4_encode_no_vrf(u, 0x3, 0x3,
                                                      &arg->key.ip_mask[0], &kmask);
        } else {
            bcm56996_l1_key_fmt1_lpm_v4_encode(u, vrf, HALF_WIDE, IP_VER_6,
                                               arg->key.ip_addr[3], &kdata);
            bcm56996_l1_key_fmt1_lpm_v4_encode(u, vrf_mask, 0x3, 0x3,
                                               arg->key.ip_mask[3], &kmask);
        }
 ...
 */
static void
bcm56996_l1_fmt1_narrow_lpm_encode(int u, alpm_arg_t *arg, UNPAIR1_T *de,
                                   UNPAIR1_T *se, int upkm)
{
    ...
    if (arg->key.kt == KEY_IPV4) {
        ...
        if (large_vrf) {
            bcm56996_l1_key_fmt1_lpm_v6_single_encode_no_vrf(u, SINGLE_WIDE,
                                                             IP_VER_6,
                                                             &arg->key.ip_addr[0],
                                                             &kdata[0],
                                                             &kdata[1]);
            bcm56996_l1_key_fmt1_lpm_v6_single_encode_no_vrf(u, 0x3, 0x3,
                                                             &arg->key.ip_mask[0],
                                                             &kmask[0],
                                                             &kmask[1]);
        } else {
            bcm56996_l1_key_fmt1_lpm_v6_single_encode(u, vrf, SINGLE_WIDE,
                                                      IP_VER_6,
                                                      &arg->key.ip_addr[0],
                                                      &kdata[0],
                                                      &kdata[1]);
            bcm56996_l1_key_fmt1_lpm_v6_single_encode(u, vrf_mask, 0x3, 0x3,
                                                      &arg->key.ip_mask[0],
                                                      &kmask[0],
                                                      &kmask[1]);
        }
...
static void
bcm56996_l1_fmt1_wide_encode_data(int u, alpm_arg_t *arg, PAIR1_T *de, PAIR1_T *se)
{
    bcm56996_assoc_data_t ad_format;
    bcm56996_ln_assoc_data_format_encode(u, &ad_format, 1, &arg->ad);
 ...
 */
static void
bcm56996_l1_fmt1_wide_lpm_encode(int u, alpm_arg_t *arg, PAIR1_T *de, PAIR1_T *se)
{
    ...
    large_vrf = arg->lpm_large_vrf && arg->key.vt != VRF_OVERRIDE;
    if (large_vrf) {
        bcm56996_l1_key_fmt1_lpm_v6_double_encode_no_vrf(u, DOUBLE_WIDE, IP_VER_6,
                                                         &arg->key.ip_addr[0],
                                                         &kdata[0],
                                                         &kdata[1],
                                                         &kdata[2],
                                                         &kdata[3]);
        bcm56996_l1_key_fmt1_lpm_v6_double_encode_no_vrf(u, 0x3, 0x3,
                                                         &arg->key.ip_mask[0],
                                                         &kmask[0],
                                                         &kmask[1],
                                                         &kmask[2],
        ...
        bcm56996_l1_key_fmt1_lpm_v6_double_encode(u, vrf, DOUBLE_WIDE, IP_VER_6,
                                                  &arg->key.ip_addr[0],
                                                  &kdata[0],
                                                  &kdata[1],
                                                  &kdata[2],
                                                  &kdata[3]);
        bcm56996_l1_key_fmt1_lpm_v6_double_encode(u, vrf_mask, 0x3, 0x3,
                                                  &arg->key.ip_mask[0],
                                                  &kmask[0],
                                                  &kmask[1],
                                                  &kmask[2],
 ...
 */
static void
bcm56996_l1_fmt1_narrow_encode_data_n1(int u, alpm_arg_t *arg, UNPAIR1_T *de,
                                       UNPAIR1_T *se, int upkm)
{
    bcm56996_assoc_data_t ad_format;
    bcm56996_ln_assoc_data_format_encode(u, &ad_format, 1, &arg->ad);
...
static void
bcm56996_l1_fmt1_narrow_encode_data(int u, alpm_arg_t *arg, UNPAIR1_T *de,
                                    UNPAIR1_T *se, int upkm)
{
    bcm56996_assoc_data_t ad_format;
 ...
 */
static void
bcm56996_l1_fmt1_narrow_l3mc_encode(int u, alpm_arg_t *arg, UNPAIR1_T *de,
                                   UNPAIR1_T *se, int upkm)
{
    L3_DEFIP_TCAM_KEY_FMT1_t kdata[2] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT1_t kmask[2] = {{{0}}};
    ...
    UNPAIR1_FIELD_SET(FIXED_DATA0f, SUB_DB_PRIORITY_VRF);
    bcm56996_l1_key_fmt1_l3mc_v4_single_encode(u, SINGLE_WIDE,
                                              FMT1_KT_L3MC_V4,
                                              &arg->key.ip_addr[0],
                                              &kdata[0],
                                              &kdata[1]);
    bcm56996_l1_key_fmt1_l3mc_v4_single_encode(u, 0x3, 0x3,
                                              &arg->key.ip_mask[0],
                                              &kmask[0],
                                              &kmask[1]);
 ...
 */
static void
bcm56996_l1_fmt1_wide_l3mc_encode(int u, alpm_arg_t *arg, PAIR1_T *de, PAIR1_T *se)
{
    L3_DEFIP_TCAM_KEY_FMT1_t kdata[4] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT1_t kmask[4] = {{{0}}};
    bcm56996_assoc_data_t ad_format;
    ...
    PAIR1_FIELD_SET(LWR_FIXED_DATA1f, SUB_DB_PRIORITY_VRF);
    PAIR1_FIELD_SET(LWR_FIXED_DATA0f, SUB_DB_PRIORITY_VRF);
    bcm56996_l1_key_fmt1_l3mc_v6_double_encode(u, DOUBLE_WIDE, FMT1_KT_L3MC_V6,
                                              &arg->key.ip_addr[0],
                                              &kdata[0],
                                              &kdata[1],
                                              &kdata[2],
                                              &kdata[3]);
    bcm56996_l1_key_fmt1_l3mc_v6_double_encode(u, 0x3, 0x3,
                                              &arg->key.ip_mask[0],
                                              &kmask[0],
                                              &kmask[1],
                                              &kmask[2],
 ...
 */
static void
bcm56996_l1_fmt0_wide_comp_encode(int u, alpm_arg_t *arg, PAIR0_T *de, PAIR0_T *se)
{
    L3_DEFIP_TCAM_KEY_FMT0_t kdata[4] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT0_t kmask[4] = {{{0}}};
    bcm56996_assoc_data_t ad_format;
    ...
    if (arg->key.vt == VRF_OVERRIDE) {
        ...
        vrf = arg->key.t.w_vrf;
        vrf_mask = BCM56996_ALPM_COMP_PORT_MASK;
    }
    ...
    if (arg->key.t.ipv == IP_VER_4) {
        bcm56996_l1_key_fmt0_comp_v4_double_encode(u, arg->comp_key_type,
        ...
        bcm56996_l1_key_fmt0_comp_v6_double_encode(u, arg->comp_key_type,
 ...
 */
static int
bcm56996_l1_encode(int u, int m, void *varg, void *de, void *se, uint8_t l1v)
{
    alpm_arg_t *arg = varg;
    ...
    if (WIDE_PKM(arg->pkm)) {
        if (arg->key.t.app == APP_L3MC) {
            if (l1v == L1V_WIDE) {
                if (arg->encode_data_only) {
                    bcm56996_l1_fmt1_wide_encode_data(u, arg, de, se);
                } else {
                    bcm56996_l1_fmt1_wide_l3mc_encode(u, arg, de, se);
                }
            } else {
                if (arg->encode_data_only) {
                    bcm56996_l1_fmt1_narrow_encode_data(u, arg, de, se, arg->pkm);
                } else {
                    bcm56996_l1_fmt1_narrow_l3mc_encode(u, arg, de, se, arg->pkm);
                }
            }
        } else {
            if (l1v == L1V_WIDE) {
                if (arg->encode_data_only) {
                    ...
                    bcm56996_l1_fmt1_wide_lpm_encode(u, arg, de, se);
                }
            } else if (l1v == L1V_NARROW_1) {
                if (arg->encode_data_only) {
                    bcm56996_l1_fmt1_narrow_encode_data_n1(u, arg, de, se, arg->pkm);
                } else {
                    bcm56996_l1_fmt1_narrow_lpm_encode_n1(u, arg, de, se, arg->pkm);
                }
            } else {
                if (arg->encode_data_only) {
                    bcm56996_l1_fmt1_narrow_encode_data(u, arg, de, se, arg->pkm);
                } else {
                    bcm56996_l1_fmt1_narrow_lpm_encode(u, arg, de, se, arg->pkm);
                }
            }
        }
    } else {
        if (arg->key.t.app == APP_COMP1) {
            if (arg->encode_data_only) {
                bcm56996_l1_fmt0_wide_encode_data(u, arg, de, se);
            } else {
                bcm56996_l1_fmt0_wide_comp_encode(u, arg, de, se);
            }
        } else {
            if (l1v == L1V_WIDE) {
                if (arg->encode_data_only) {
                    ...
                    bcm56996_l1_fmt0_wide_lpm_encode(u, arg, de, se);
                }
            } else if (l1v == L1V_NARROW_1) {
                if (arg->encode_data_only) {
                    bcm56996_l1_fmt0_narrow_encode_data_n1(u, arg, de, se, arg->pkm);
                } else {
                    bcm56996_l1_fmt0_narrow_lpm_encode_n1(u, arg, de, se, arg->pkm);
                }
            } else {
                if (arg->encode_data_only) {
                    bcm56996_l1_fmt0_narrow_encode_data(u, arg, de, se, arg->pkm);
                } else {
                    bcm56996_l1_fmt0_narrow_lpm_encode(u, arg, de, se, arg->pkm);
                }
            }
        }
    }
 ...
 */
static void
bcm56996_l1_fmt0_narrow_decode_data_n1(int u, alpm_arg_t *arg, UNPAIR0_T *de)
{
    bcm56996_assoc_data_t ad_format;
    ALPM1_DATA_t a1_format;
    ...
    bcm56996_ln_alpm1_data_format_decode(u, &a1_format, &arg->a1, arg);
    bcm56996_ln_assoc_data_format_decode(u, &ad_format, 1, &arg->ad, arg->key.t.app);
}
 ...
 */
static void
bcm56996_l1_fmt0_narrow_decode_data(int u, alpm_arg_t *arg, UNPAIR0_T *de)
{
    bcm56996_assoc_data_t ad_format;
    ALPM1_DATA_t a1_format;
 ...
 */
static void
bcm56996_l1_fmt0_wide_decode_data(int u, alpm_arg_t *arg, PAIR0_T *de)
{
    bcm56996_assoc_data_t ad_format;
    ALPM1_DATA_t a1_format;
 ...
 */
static void
bcm56996_l1_fmt1_narrow_decode_data_n1(int u, alpm_arg_t *arg, UNPAIR1_T *de)
{
    bcm56996_assoc_data_t ad_format;
    ALPM1_DATA_t a1_format;
 ...
 */
static void
bcm56996_l1_fmt1_narrow_decode_data(int u, alpm_arg_t *arg, UNPAIR1_T *de)
{
    bcm56996_assoc_data_t ad_format;
    ALPM1_DATA_t a1_format;
 ...
 */
static void
bcm56996_l1_fmt1_wide_decode_data(int u, alpm_arg_t *arg, PAIR1_T *de)
{
    bcm56996_assoc_data_t ad_format;
    ALPM1_DATA_t a1_format;
 ...
 */
static void
bcm56996_l1_fmt0_narrow_lpm_decode_n1(int u, alpm_arg_t *arg, UNPAIR0_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT0_t kdata;
    L3_DEFIP_TCAM_KEY_FMT0_t kmask;
    int key_mode;
    ...
    if (large_vrf) {
        bcm56996_l1_key_fmt0_lpm_v4_decode_no_vrf(u, NULL, NULL,
                                                  &arg->key.ip_mask[0], &kmask);
        bcm56996_l1_key_fmt0_lpm_v4_decode_no_vrf(u, &key_mode, &kt,
                                                  &arg->key.ip_addr[0], &kdata);
    } else {
        bcm56996_l1_key_fmt0_lpm_v4_decode(u, &vrf_mask, NULL, NULL, &ip_mask,
                                           &kmask);
        bcm56996_l1_key_fmt0_lpm_v4_decode(u, &vrf, &key_mode, &kt,
                                           &ip_addr, &kdata);
    }
    arg->key.t.ipv = kt;
    if (!arg->key.t.ipv) {
    ...
    bcm56996_l1_lpm_vt_decode(u, sub_prio, vrf, vrf_mask, arg);
 ...
 */
static void
bcm56996_l1_fmt0_narrow_lpm_decode(int u, alpm_arg_t *arg, UNPAIR0_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT0_t kdata[2] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT0_t kmask[2] = {{{0}}};
    int valid[2];
    ...
    if (!arg->key.t.ipv) {
        ...
        if (large_vrf) {
            bcm56996_l1_key_fmt0_lpm_v6_single_decode_no_vrf(u, NULL, NULL,
                                                             &arg->key.ip_mask[0],
                                                             &kmask[0],
                                                             &kmask[1]);
            bcm56996_l1_key_fmt0_lpm_v6_single_decode_no_vrf(u, NULL, NULL,
            ...
            bcm56996_l1_key_fmt0_lpm_v6_single_decode(u, &vrf_mask, NULL, NULL,
                                                      &arg->key.ip_mask[0],
                                                      &kmask[0],
                                                      &kmask[1]);
            bcm56996_l1_key_fmt0_lpm_v6_single_decode(u, &vrf, NULL, NULL,
                                                      &arg->key.ip_addr[0],
                                                      &kdata[0],
                                                      &kdata[1]);
 ...
 */
static void
bcm56996_l1_fmt0_wide_lpm_decode(int u, alpm_arg_t *arg, PAIR0_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT0_t kdata[4] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT0_t kmask[4] = {{{0}}};
    int valid[4] = {0};
    ...
    large_vrf = arg->lpm_large_vrf && sub_prio != SUB_DB_PRIORITY_OVERRIDE;
    if (large_vrf) {
        bcm56996_l1_key_fmt0_lpm_v6_double_decode_no_vrf(u, NULL, &kt,
                                                         &arg->key.ip_addr[0],
                                                         &kdata[0],
                                                         &kdata[1],
                                                         &kdata[2],
        ...
        bcm56996_l1_key_fmt0_lpm_v6_double_decode_no_vrf(u, NULL, NULL,
                                                         &arg->key.ip_mask[0],
                                                         &kmask[0],
                                                         &kmask[1],
                                                         &kmask[2],
        ...
        bcm56996_l1_key_fmt0_lpm_v6_double_decode(u, &vrf, NULL, &kt,
                                                  &arg->key.ip_addr[0],
                                                  &kdata[0],
                                                  &kdata[1],
                                                  &kdata[2],
        ...
        bcm56996_l1_key_fmt0_lpm_v6_double_decode(u, &vrf_mask, NULL, NULL,
                                                  &arg->key.ip_mask[0],
                                                  &kmask[0],
                                                  &kmask[1],
                                                  &kmask[2],
 ...
 */
static void
bcm56996_l1_fmt1_narrow_l3mc_decode(int u, alpm_arg_t *arg, UNPAIR1_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT1_t kdata[2] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT1_t kmask[2] = {{{0}}};
    int valid[2];
    ...
    bcm56996_l1_key_fmt1_l3mc_v4_single_decode(u, NULL, NULL,
                                              &arg->key.ip_mask[0],
                                              &kmask[0],
                                              &kmask[1]);
    bcm56996_l1_key_fmt1_l3mc_v4_single_decode(u, NULL, NULL,
 ...
 */
static void
bcm56996_l1_fmt1_wide_l3mc_decode(int u, alpm_arg_t *arg, PAIR1_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT1_t kdata[4] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT1_t kmask[4] = {{{0}}};
    int valid[4] = {0};
...
/*    int hit[2] = {0}; */
    ...
    bcm56996_l1_key_fmt1_l3mc_v6_double_decode(u, NULL, NULL,
                                              &arg->key.ip_mask[0],
                                              &kmask[0],
                                              &kmask[1],
                                              &kmask[2],
 ...
 */
static void
bcm56996_l1_fmt1_narrow_lpm_decode_n1(int u, alpm_arg_t *arg, UNPAIR1_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT1_t kdata;
    L3_DEFIP_TCAM_KEY_FMT1_t kmask;
    int key_mode;
    ...
    large_vrf = arg->lpm_large_vrf && sub_prio != SUB_DB_PRIORITY_OVERRIDE;
    if (large_vrf) {
        bcm56996_l1_key_fmt1_lpm_v4_decode_no_vrf(u, NULL, NULL,
                                                  &arg->key.ip_mask[0], &kmask);
        bcm56996_l1_key_fmt1_lpm_v4_decode_no_vrf(u, &key_mode, &kt,
                                                  &arg->key.ip_addr[0], &kdata);

    } else {
        bcm56996_l1_key_fmt1_lpm_v4_decode(u, &vrf_mask, NULL, NULL, &ip_mask,
                                           &kmask);
        bcm56996_l1_key_fmt1_lpm_v4_decode(u, &vrf, &key_mode, &kt,
                                           &ip_addr, &kdata);
    }
 ...
 */
static void
bcm56996_l1_fmt1_narrow_lpm_decode(int u, alpm_arg_t *arg, UNPAIR1_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT1_t kdata[2] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT1_t kmask[2] = {{{0}}};
    int valid[2];
    ...
    if (kt == FMT1_KT_L3MC_V4 ||
        ...
        bcm56996_l1_fmt1_narrow_l3mc_decode(u, arg, de);
        return;
    }
    ...
    if (!arg->key.t.ipv) {
        ...
        UNPAIR1_FIELD_GET2(MASK1f, kmask[1]);
        if (large_vrf) {
            bcm56996_l1_key_fmt1_lpm_v6_single_decode_no_vrf(u, NULL, NULL,
                                                             &arg->key.ip_mask[0],
                                                             &kmask[0],
                                                             &kmask[1]);
            bcm56996_l1_key_fmt1_lpm_v6_single_decode_no_vrf(u, NULL, NULL,
            ...
            bcm56996_l1_key_fmt1_lpm_v6_single_decode(u, &vrf_mask, NULL, NULL,
                                                      &arg->key.ip_mask[0],
                                                      &kmask[0],
                                                      &kmask[1]);
            bcm56996_l1_key_fmt1_lpm_v6_single_decode(u, &vrf, NULL, NULL,
                                                      &arg->key.ip_addr[0],
                                                      &kdata[0],
                                                      &kdata[1]);
        }
 ...
 */
static void
bcm56996_l1_fmt1_wide_lpm_decode(int u, alpm_arg_t *arg, PAIR1_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT1_t kdata[4] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT1_t kmask[4] = {{{0}}};
    int valid[4] = {0};
...
/*    int hit[2] = {0}; */
    ...
    large_vrf = arg->lpm_large_vrf && sub_prio != SUB_DB_PRIORITY_OVERRIDE;
    if (large_vrf) {
        bcm56996_l1_key_fmt1_lpm_v6_double_decode_no_vrf(u, NULL, &kt,
                                                         &arg->key.ip_addr[0],
                                                         &kdata[0],
                                                         &kdata[1],
                                                         &kdata[2],
        ...
        bcm56996_l1_key_fmt1_lpm_v6_double_decode(u, &vrf, NULL, &kt,
                                                  &arg->key.ip_addr[0],
                                                  &kdata[0],
                                                  &kdata[1],
                                                  &kdata[2],
    ...
    if (kt == FMT1_KT_L3MC_V4 ||
        ...
        arg->key.t.app = APP_L3MC;
        bcm56996_l1_fmt1_wide_l3mc_decode(u, arg, de);
        return;
    }
    arg->key.t.ipv = kt;
    PAIR1_FIELD_GET2(LWR_MASK0f, kmask[0]);
    ...
    PAIR1_FIELD_GET2(UPR_MASK1f, kmask[3]);
    if (large_vrf) {
        bcm56996_l1_key_fmt1_lpm_v6_double_decode_no_vrf(u, NULL, NULL,
                                                         &arg->key.ip_mask[0],
                                                         &kmask[0],
                                                         &kmask[1],
                                                         &kmask[2],
        ...
        bcm56996_l1_key_fmt1_lpm_v6_double_decode(u, &vrf_mask, NULL, NULL,
                                                  &arg->key.ip_mask[0],
                                                  &kmask[0],
                                                  &kmask[1],
                                                  &kmask[2],
 ...
 */
static void
bcm56996_l1_fmt0_wide_comp_decode(int u, alpm_arg_t *arg, PAIR0_T *de)
{
    L3_DEFIP_TCAM_KEY_FMT0_t kdata[4] = {{{0}}};
    L3_DEFIP_TCAM_KEY_FMT0_t kmask[4] = {{{0}}};
    int valid[4] = {0};
    ...
    if (arg->key.t.ipv == IP_VER_4) {
        bcm56996_l1_key_fmt0_comp_v4_double_decode(u, arg->comp_key_type,
                                                  &vrf_mask, NULL, NULL,
                                                  &arg->key.ip_mask[0],
                                                  &kmask[0],
                                                  &kmask[1],
        ...
        bcm56996_l1_key_fmt0_comp_v6_double_decode(u, arg->comp_key_type,
                                                  &vrf_mask, NULL, NULL,
                                                  &arg->key.ip_mask[0],
                                                  &kmask[0],
                                                  &kmask[1],
    ...
    PAIR0_FIELD_GET(UPR_VALID1f, valid[3]);
    arg->valid = valid[0] & valid[1] & valid[2] & valid[3];
    bcm56996_l1_comp_vt_decode(u, arg->comp_key_type, vrf, vrf_mask, arg);
 ...
 */
static int
bcm56996_l1_decode(int u, int m, void *varg, void *de, void *se, uint8_t l1v)
{
    alpm_arg_t *arg = varg;
    ...
    if (WIDE_PKM(arg->pkm)) {
        if (l1v == L1V_WIDE) {
            if (arg->decode_data_only) {
                bcm56996_l1_fmt1_wide_decode_data(u, arg, de);
            } else {
                bcm56996_l1_fmt1_wide_lpm_decode(u, arg, de);
            }
        } else if (l1v == L1V_NARROW_1) {
            if (arg->decode_data_only) {
                bcm56996_l1_fmt1_narrow_decode_data_n1(u, arg, de);
            } else {
                bcm56996_l1_fmt1_narrow_lpm_decode_n1(u, arg, de);
            }
        } else {
            if (arg->decode_data_only) {
                bcm56996_l1_fmt1_narrow_decode_data(u, arg, de);
            } else {
                bcm56996_l1_fmt1_narrow_lpm_decode(u, arg, de);
            }
        }
    } else {
        if (FP_PKM(arg->pkm)) {
            if (arg->decode_data_only) {
                bcm56996_l1_fmt0_wide_decode_data(u, arg, de);
            } else {
                arg->key.t.app = APP_COMP1;
                bcm56996_l1_fmt0_wide_comp_decode(u, arg, de);
            }
        } else {
            if (l1v == L1V_WIDE) {
                if (arg->decode_data_only) {
                    ...
                    bcm56996_l1_fmt0_wide_lpm_decode(u, arg, de);
                }
            } else if (l1v == L1V_NARROW_1) {
                if (arg->decode_data_only) {
                    bcm56996_l1_fmt0_narrow_decode_data_n1(u, arg, de);
                } else {
                    bcm56996_l1_fmt0_narrow_lpm_decode_n1(u, arg, de);
                }
            } else {
                if (arg->decode_data_only) {
                    bcm56996_l1_fmt0_narrow_decode_data(u, arg, de);
                } else {
                    bcm56996_l1_fmt0_narrow_lpm_decode(u, arg, de);
                }
            }
        }
    }
 ...
 */
static int
bcm56996_l1_fmt0_half_entry_move(int u, void *src, void *dst, int x, int y)
{
 ...
 */
static int
bcm56996_l1_fmt1_half_entry_move(int u, void *src, void *dst, int x, int y)
{
 ...
 */
static int
bcm56996_l1_half_entry_move(int u, int m, void *src, void *dst, int x, int y, int pkm)
{

    if (WIDE_PKM(pkm)) {
        return bcm56996_l1_fmt1_half_entry_move(u, src, dst, x, y);
    } else {
        return bcm56996_l1_fmt0_half_entry_move(u, src, dst, x, y);
    }
 ...
 */
static int
bcm56996_device_info_init(int u, int m, alpm_dev_info_t *dev)
{
    SHR_FUNC_ENTER(u);

    if (dev) {
        dev->max_vrf = BCM56996_ALPM_MAX_VRF;
        dev->max_fp_op = BCM56996_ALPM_MAX_COMP_PORT;
        dev->tcam_blocks = 20;
        dev->tcam_depth = 128;
        dev->l1_blocks = 10;
        ...
        dev->feature_urpf = 0; /* no urpf support for TH4G */
        ...
        dev->l2p_route_fmts = &bcm56996_l2p_route_fmts[0];
...
int
bcm56996_a0_rm_alpm_driver_register(int u, bcmptm_rm_alpm_driver_t *drv)
{
    SHR_FUNC_ENTER(u);

    if (drv) {
        drv->l1_encode = bcm56996_l1_encode;
        drv->l1_decode = bcm56996_l1_decode;
        drv->l1_half_entry_move = bcm56996_l1_half_entry_move;
        drv->ln_base_entry_decode = bcm56996_ln_base_entry_decode;
        drv->ln_base_entry_encode = bcm56996_ln_base_entry_encode;
        drv->ln_raw_bucket_decode = bcm56996_ln_raw_bucket_decode;
        drv->ln_raw_bucket_encode = bcm56996_ln_raw_bucket_encode;
        drv->ln_raw_bucket_entry_get = bcm56996_ln_raw_bucket_entry_get;
        drv->ln_raw_bucket_entry_set = bcm56996_ln_raw_bucket_entry_set;
        drv->ln_base_entry_info = bcm56996_ln_base_entry_info;
        drv->ln_base_entry_move = bcm56996_ln_base_entry_move;
        drv->device_info_init   = bcm56996_device_info_init;
        drv->ln_hit_get = NULL;
        drv->ln_hit_set = NULL;
    } else {
