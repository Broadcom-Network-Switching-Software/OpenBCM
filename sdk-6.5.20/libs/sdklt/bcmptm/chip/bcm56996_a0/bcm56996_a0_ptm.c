...
#include "./bcmptm_bcm56996_a0_rm_tcam.h"
...
#include "bcmptm_bcm56996_a0_cth_alpm.h"
...
static bcmdrd_sid_t *s_bcm56996_a0_pt_with_ltid[BCMDRD_CONFIG_MAX_UNITS];
static bcmptm_overlay_info_t *s_bcm56996_a0_overlay_info[BCMDRD_CONFIG_MAX_UNITS];
static pt_with_dfid_t *s_bcm56996_a0_pt_with_dfid[BCMDRD_CONFIG_MAX_UNITS];

/* List of PTs which must NOT be cached */
static const bcmdrd_sid_t bcm56996_a0_dont_cache[] = {
    /* Secret registers */
    MMU_PTSCH_PIPE_PICK_TO_SAME_PORT_CPU_MIN_SPACINGr,
    MMU_PTSCH_PIPE_PICK_TO_SAME_PORT_PICK_MIN_SPACINGr,

    /* LP views - None in TH4G */
...
static const bcmdrd_sid_t bcm56996_a0_must_cache[] = {
...
static bcmdrd_sid_t bcm56996_a0_pt_with_ltid[] = {
...
/* List of PTs which need dfield_format_id.
 * Fill it on-demand basis ONLY */
static pt_with_dfid_t bcm56996_a0_pt_with_dfid[] = {
    {INVALIDm, 0}
};
 ...
 */
static bcmptm_overlay_info_t bcm56996_a0_overlay_info[] = {
     ...
     * issue.
     * No TCAM atomicity issue in TH4G. Mark them non-cacheable */
/*
 ...
 */
static const bcmdrd_sid_t bcm56996_a0_em_pt_list[] = {
    EXACT_MATCH_4m, /* 32K */
    EXACT_MATCH_2m, /* 64K */
...
static const bcmdrd_sid_t bcm56996_a0_l2_entry_uft_pt_list[] = {
    L2_ENTRY_UFT_SINGLEm, /* 128K */
    L2_ENTRY_UFT_ECCm /* 128K */
};

static const bcmdrd_sid_t bcm56996_a0_alpm_level2_pt_list[] = {
...
static const bcmdrd_sid_t bcm56996_a0_alpm_level3_pt_list[] = {
...
static int
bcm56996_a0_pt_attr_is_alpm(int unit, bcmdrd_sid_t sid, bool *is_alpm)
{
    int idx = -1;

    *is_alpm = FALSE;
    BIN_SEARCH(s_bcm56996_a0_overlay_info[unit], .n_sid,
               sid,
               COUNTOF(bcm56996_a0_overlay_info)- 2, /* don't use the last entry */
               idx);
    if (idx != -1) {
        *is_alpm = s_bcm56996_a0_overlay_info[unit][idx].is_alpm;
    }
...
static int
bcm56996_a0_pt_overlay_info_get(int unit, bcmdrd_sid_t sid,
                                const bcmptm_overlay_info_t **oinfo)
{
    ...
    BIN_SEARCH(s_bcm56996_a0_overlay_info[unit], .n_sid,
               ...
               idx);
    if (idx == -1) {
        idx = COUNTOF(bcm56996_a0_overlay_info) - 1; /* Point to the last element */
    }


    *oinfo = &s_bcm56996_a0_overlay_info[unit][idx];
...
static int
bcm56996_a0_pt_tcam_km_format(int unit, bcmdrd_sid_t sid,
...
static int
bcm56996_a0_hw_does_km_to_xy(int unit, bool *hw_does_km_to_xy)
{
...
static int
bcm56996_a0_tcam_corrupt_bits_enable(int unit, bool *tc_en)
{
     ...
     *
     * FALSE for TH4G - because TCAM error is reported via SER_FIFO
     */
    *tc_en = FALSE;
    return SHR_E_NONE;
}
...
static int
bcm56996_a0_pt_needs_dfid(int unit, bcmdrd_sid_t sid, uint8_t *dfid_bytes)
{
    int idx = -1;

    *dfid_bytes = 0;
    BIN_SEARCH(s_bcm56996_a0_pt_with_dfid[unit], .pt_sid,
               sid,
               COUNTOF(bcm56996_a0_pt_with_dfid) - 1,
               idx);

    if (idx != -1) {
        assert(s_bcm56996_a0_pt_with_dfid[unit][idx].size != 0);
        assert(s_bcm56996_a0_pt_with_dfid[unit][idx].size != 3);
        *dfid_bytes = s_bcm56996_a0_pt_with_dfid[unit][idx].size;
    }
...
static int
bcm56996_a0_pt_needs_ltid(int unit, bcmdrd_sid_t sid, bool *ltid_en)
{
    int idx = -1;

    *ltid_en = FALSE;
    BIN_SEARCH(s_bcm56996_a0_pt_with_ltid[unit], ,
               sid,
               COUNTOF(bcm56996_a0_pt_with_ltid)- 1,
               idx);
    if (idx != -1) {
...
static int
bcm56996_a0_pt_in_must_cache_list(int unit, bcmdrd_sid_t sid, bool *must_cache)
{
    uint32_t i;
    *must_cache = FALSE;
    for (i = 0; i < COUNTOF(bcm56996_a0_must_cache); i++) {
       if (bcm56996_a0_must_cache[i] == sid) {
...
static int
bcm56996_a0_pt_in_dont_cache_list(int unit, bcmdrd_sid_t sid, bool *dont_cache)
{
    uint32_t i;
    *dont_cache = FALSE;
    for (i = 0; i < COUNTOF(bcm56996_a0_dont_cache); i++) {
       if (bcm56996_a0_dont_cache[i] == sid) {
...
static int
bcm56996_a0_pt_group_info_get(int unit, uint32_t group_id, uint32_t *pt_count,
                              bool *bank_start_en, const bcmdrd_sid_t **pt_list)
{
    ...
    case PT_GROUP_DEFAULT:
        ...
        *pt_list = NULL; /* We don't have list of SIDs in DEFAULT group */
        ...
        break;
    case PT_GROUP_EM:
        *pt_count = COUNTOF(bcm56996_a0_em_pt_list);
        *pt_list = bcm56996_a0_em_pt_list;
        if (bank_start_en) {
            *bank_start_en = FALSE;
        }
        break;
    case PT_GROUP_L2_ENTRY_UFT:
        *pt_count = COUNTOF(bcm56996_a0_l2_entry_uft_pt_list);
        *pt_list = bcm56996_a0_l2_entry_uft_pt_list;
        if (bank_start_en) {
            *bank_start_en = FALSE;
        }
        break;
    case PT_GROUP_ALPM_LEVEL2:
        *pt_count = COUNTOF(bcm56996_a0_alpm_level2_pt_list);
        *pt_list = bcm56996_a0_alpm_level2_pt_list;
        if (bank_start_en) {
            *bank_start_en = FALSE;
        }
        break;
    case PT_GROUP_ALPM_LEVEL3:
        *pt_count = COUNTOF(bcm56996_a0_alpm_level3_pt_list);
        *pt_list = bcm56996_a0_alpm_level3_pt_list;
        if (bank_start_en) {
            *bank_start_en = TRUE;
        }
        break;
 ...
 */
static int
bcm56996_a0_uft_bank_get(int unit, uint8_t bank_enum,
                         uint8_t *uft_bank)
{
    int i;
 ...
 */
static int
bcm56996_a0_bank_enum_get(int unit, uint8_t uft_bank,
                          uint8_t *bank_enum)
{
    uint32_t bank_enum_val;
    const char *bank_enum_str_list[] = {
...
static int
bcm56996_a0_pt_group_index_count_set(int unit, uint32_t group_id,
    ...
    case PT_GROUP_ALPM_LEVEL3:
        ...
        if (init) {
            *bank_count = HW_MIN_BANK_COUNT_PT_GROUP_ALPM_LEVEL3;
            SHR_IF_ERR_EXIT(
                bcm56996_a0_bank_enum_get(unit, 0, /* init uft_bank */
                                          start_bank_enum));
            ...
            if (*bank_count) {
                SHR_IF_ERR_EXIT(
                    bcm56996_a0_uft_bank_get(unit, *start_bank_enum,
                                             &uft_bank_num));
            } else {
...
bcm56996_a0_pt_index_count_get(int unit, bcmdrd_sid_t sid, bool acc_for_ser,
...
static int
bcm56996_a0_pt_index_min_get(int unit, bcmdrd_sid_t sid, bool acc_for_ser)
{
    int index_min;
    int s = 0;
    if (acc_for_ser) {
...
static int
bcm56996_a0_pt_index_max_get(int unit, bcmdrd_sid_t sid, bool acc_for_ser)
{
    int index_min, tmp_rv;
    uint32_t index_count = 0;
    tmp_rv = bcm56996_a0_pt_index_count_get(unit, sid, acc_for_ser,
                                            &index_count);
    if (tmp_rv == SHR_E_NONE) {
        /* we have bank based index_count, so return bank based index_max */
        index_min = bcm56996_a0_pt_index_min_get(unit, sid, acc_for_ser);
        if (index_min >= 0) {
...
static int
bcm56996_a0_wal_info_get(int unit, bcmptm_wal_info_t *wal_info)
{
 ...
 */
int
bcmptm_bcm56996_a0_fnptr_attach(int unit, fn_ptr_all_t *fn_ptr_all)
{
    void *tmp_alloc_ptr;
    SHR_FUNC_ENTER(unit);
    fn_ptr_all->fn_rsp_info_get = NULL; /* use default logic */
    fn_ptr_all->fn_wal_info_get = &bcm56996_a0_wal_info_get;
    fn_ptr_all->fn_pt_attr_is_alpm = &bcm56996_a0_pt_attr_is_alpm;
    fn_ptr_all->fn_pt_overlay_info_get = &bcm56996_a0_pt_overlay_info_get;
    fn_ptr_all->fn_pt_tcam_km_format = &bcm56996_a0_pt_tcam_km_format;
    fn_ptr_all->fn_hw_does_km_to_xy = &bcm56996_a0_hw_does_km_to_xy;
    fn_ptr_all->fn_tcam_corrupt_bits_enable = &bcm56996_a0_tcam_corrupt_bits_enable;
    fn_ptr_all->fn_pt_needs_ltid = &bcm56996_a0_pt_needs_ltid;
    fn_ptr_all->fn_pt_needs_dfid = &bcm56996_a0_pt_needs_dfid;
    fn_ptr_all->fn_pt_in_must_cache_list = &bcm56996_a0_pt_in_must_cache_list;
    fn_ptr_all->fn_pt_in_dont_cache_list = &bcm56996_a0_pt_in_dont_cache_list;
    fn_ptr_all->fn_pt_group_index_count_set = &bcm56996_a0_pt_group_index_count_set;
    fn_ptr_all->fn_pt_index_min_get = &bcm56996_a0_pt_index_min_get;
    fn_ptr_all->fn_pt_index_max_get = &bcm56996_a0_pt_index_max_get;
    fn_ptr_all->fn_pt_group_info_get = &bcm56996_a0_pt_group_info_get;
    fn_ptr_all->fn_pt_rm_tcam_memreg_oper = &bcm56996_a0_rm_tcam_memreg_oper;
    fn_ptr_all->fn_pt_rm_tcam_prio_only_hw_key_info_init =
            &bcm56996_a0_rm_tcam_prio_only_hw_key_info_init;
    /* CCI */
    fn_ptr_all->fn_pt_cci_ctrtype_get = &bcm56996_a0_pt_cci_ctrtype_get;
    fn_ptr_all->fn_pt_cci_frmt_sym_get = &bcm56996_a0_pt_cci_frmt_sym_get;
    fn_ptr_all->fn_pt_cci_ctr_sym_map = &bcm56996_a0_pt_cci_ctr_sym_map;
    fn_ptr_all->fn_pt_cci_frmt_index_from_port =
            &bcm56996_a0_pt_cci_frmt_index_from_port;
    fn_ptr_all->fn_pt_cci_index_valid = &bcm56996_a0_pt_cci_index_valid;
    fn_ptr_all->fn_pt_cci_reg_addr_gap_get =
            &bcm56996_a0_pt_cci_reg_addr_gap_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_control_get =
            &bcm56996_a0_pt_cci_ctr_evict_control_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_enable_field_get =
            &bcm56996_a0_pt_cci_ctr_evict_enable_field_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_clr_on_read_field_get =
            &bcm56996_a0_pt_cci_ctr_evict_clr_on_read_field_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_default_enable_list_get =
            &bcm56996_a0_pt_cci_ctr_evict_default_enable_list_get;
    fn_ptr_all->fn_pt_cci_ctr_is_bypass = &bcm56996_a0_pt_cci_ctr_is_bypass;
    fn_ptr_all->fn_pt_cci_fifodma_chan_get =
            &bcm56996_a0_pt_cci_fifodma_chan_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_fifo_enable_sym_get =
            &bcm56996_a0_pt_cci_ctr_evict_fifo_enable_sym_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_fifo_sym_get =
            &bcm56996_a0_pt_cci_ctr_evict_fifo_sym_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_fifo_entry_get =
            &bcm56996_a0_pt_cci_ctr_evict_fifo_entry_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_fifo_buf_size_get =
            &bcm56996_a0_pt_cci_ctr_evict_fifo_buf_size_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_field_num_get =
            &bcm56996_a0_pt_cci_ctr_evict_field_num_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_field_width_get =
            &bcm56996_a0_pt_cci_ctr_evict_field_width_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_normal_val_get =
            &bcm56996_a0_pt_cci_ctr_evict_normal_val_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_wide_val_get =
            &bcm56996_a0_pt_cci_ctr_evict_wide_val_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_slim_val_get =
            &bcm56996_a0_pt_cci_ctr_evict_slim_val_get;
    fn_ptr_all->fn_pt_cci_ctr_evict_update_mode_num_get =
            &bcm56996_a0_pt_cci_ctr_evict_update_mode_num_get;
    /* SER */
    fn_ptr_all->fn_ser_data_driver_register = bcm56996_a0_ptm_ser_data_driver_register;
    /* SER END */

    /* UFT */
    fn_ptr_all->fn_uft_driver_register = &bcm56996_a0_uft_driver_register;
    fn_ptr_all->fn_uft_be_driver_register = &bcm56996_a0_uft_be_driver_register;
    fn_ptr_all->fn_cth_alpm_driver_register = &bcm56996_a0_cth_alpm_driver_register;
    fn_ptr_all->fn_alpm_driver_register = &bcm56996_a0_rm_alpm_driver_register;

    /* sort large arrays for quick (binary) lookup */
tmp_alloc_ptr = NULL;
    SHR_ALLOC(tmp_alloc_ptr, sizeof(bcm56996_a0_pt_with_ltid), "bcmptmPtWithLtid");
    SHR_NULL_CHECK(tmp_alloc_ptr, SHR_E_MEMORY);
    sal_memcpy(tmp_alloc_ptr, bcm56996_a0_pt_with_ltid,
               sizeof(bcm56996_a0_pt_with_ltid));
    s_bcm56996_a0_pt_with_ltid[unit] = tmp_alloc_ptr;

    tmp_alloc_ptr = NULL;
    SHR_ALLOC(tmp_alloc_ptr, sizeof(bcm56996_a0_overlay_info), "bcmptmPtOverlayInfo");
    SHR_NULL_CHECK(tmp_alloc_ptr, SHR_E_MEMORY);
    sal_memcpy(tmp_alloc_ptr, bcm56996_a0_overlay_info,
               sizeof(bcm56996_a0_overlay_info));
    s_bcm56996_a0_overlay_info[unit] = tmp_alloc_ptr;

    tmp_alloc_ptr = NULL;
    SHR_ALLOC(tmp_alloc_ptr, sizeof(bcm56996_a0_pt_with_dfid), "bcmptmPtWithDfid");
    SHR_NULL_CHECK(tmp_alloc_ptr, SHR_E_MEMORY);
    sal_memcpy(tmp_alloc_ptr, bcm56996_a0_pt_with_dfid,
               sizeof(bcm56996_a0_pt_with_dfid));
    s_bcm56996_a0_pt_with_dfid[unit] = tmp_alloc_ptr;

    sal_qsort(s_bcm56996_a0_pt_with_ltid[unit],
              COUNTOF(bcm56996_a0_pt_with_ltid),
              sizeof(bcm56996_a0_pt_with_ltid[0]),
              basic_comp);

    sal_qsort(s_bcm56996_a0_overlay_info[unit],
              COUNTOF(bcm56996_a0_overlay_info)-1, /* Keep the last element in place */
              sizeof(bcm56996_a0_overlay_info[0]),
              overlay_info_comp);

    sal_qsort(s_bcm56996_a0_pt_with_dfid[unit],
              COUNTOF(bcm56996_a0_pt_with_dfid),
              sizeof(bcm56996_a0_pt_with_dfid[0]),
              pt_with_dfid_comp);
...
int
bcmptm_bcm56996_a0_fnptr_detach(int unit)
{
    if (s_bcm56996_a0_pt_with_ltid[unit]) {
        SHR_FREE(s_bcm56996_a0_pt_with_ltid[unit]);
    }
    if (s_bcm56996_a0_overlay_info[unit]) {
        SHR_FREE(s_bcm56996_a0_overlay_info[unit]);
    }
    if (s_bcm56996_a0_pt_with_dfid[unit]) {
        SHR_FREE(s_bcm56996_a0_pt_with_dfid[unit]);
    }
    return SHR_E_NONE;
}
