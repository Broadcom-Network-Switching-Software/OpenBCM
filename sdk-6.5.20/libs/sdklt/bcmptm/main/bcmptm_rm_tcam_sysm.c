...
my_init(int unit, bool warm)
{
    ...
    tmp_rv = bcmptm_rm_tcam_ltid_identify(unit, warm);
    if (SHR_FAILURE(tmp_rv)) {
        return SHR_SYSM_RV_ERROR;
    }

    tmp_rv = bcmptm_rm_tcam_init(unit);
    if (SHR_FAILURE(tmp_rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
            (BSL_META_U(unit, "bcmptm_rm_tcam_init FAILED(rv = %0d)\n"),
             tmp_rv));
        /* return SHR_SYSM_RV_ERROR; */
    }
 ...
 */
static shr_sysm_rv_t
rm_tcam_sysm_init(shr_sysm_categories_t instance_category,
                  int unit,
                  void *comp_data,
                  bool warm,
                  uint32_t *blocking_component)
{
    ...
    tmp_rv = bcmptm_rm_tcam_init0(unit);
    if (SHR_FAILURE(tmp_rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
            (BSL_META_U(unit, "bcmptm_rm_tcam_init0 FAILED(rv = %0d)\n"),
             tmp_rv));
        /* return SHR_SYSM_RV_ERROR; */
    }
 ...
 */
static shr_sysm_rv_t
rm_tcam_sysm_comp_config(shr_sysm_categories_t instance_category,
                         int unit,
                         void *comp_data,
                         bool warm,
                         uint32_t *blocking_component)
 ...
 */
static shr_sysm_rv_t
rm_tcam_sysm_pre_config(shr_sysm_categories_t instance_category,
                        int unit,
                        void *comp_data,
                        bool warm,
                        uint32_t *blocking_component)
 ...
 */
static void
rm_tcam_sysm_run(shr_sysm_categories_t instance_category,
                 int unit,
                 void *comp_data)
{
    return;
 ...
 */
static shr_sysm_rv_t
rm_tcam_sysm_stop(shr_sysm_categories_t instance_category,
                  int unit,
                  void *comp_data,
                  bool graceful,
                  uint32_t *blocking_component)
{
    ...
    if (SHR_FAILURE(bcmptm_rm_tcam_stop(unit))) {
        return SHR_SYSM_RV_ERROR;
    }
...
/* Phase: shutdown */
static shr_sysm_rv_t
rm_tcam_sysm_shutdown(shr_sysm_categories_t instance_category,
                      int unit,
                      void *comp_data,
                      bool graceful,
                      uint32_t *blocking_component)
{
    ...
    if (SHR_FAILURE(bcmptm_rm_tcam_dyn_ptrm_tbl_cleanup(unit))) {
        return SHR_SYSM_RV_ERROR;
    }

    if (SHR_FAILURE(bcmptm_rm_tcam_cleanup(unit))) {
        return SHR_SYSM_RV_ERROR;
    }
...
bcmptm_rm_tcam_start(void)
{
    ...
    funcs.init = rm_tcam_sysm_init;
    funcs.comp_config = rm_tcam_sysm_comp_config;
    funcs.pre_config = rm_tcam_sysm_pre_config;
    funcs.run = rm_tcam_sysm_run;
    funcs.stop = rm_tcam_sysm_stop;
    funcs.shutdown = rm_tcam_sysm_shutdown;
    SHR_IF_ERR_EXIT
        (shr_sysm_register(BCMMGMT_RM_TCAM_COMP_ID,
                           SHR_SYSM_CAT_UNIT,
                           &funcs,
                           NULL));
exit:
