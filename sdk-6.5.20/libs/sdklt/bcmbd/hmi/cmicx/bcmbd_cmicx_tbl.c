/*! \file bcmbd_cmicx_tbl.c
 *
 * Table operations in CMICx
 */
/*
...
static int
cmicx_default_tbl_insert(int unit, uint32_t adext, uint32_t addr,
                         ...
                         bcmbd_tbl_resp_info_t *resp_info, uint32_t *resp_data)
{
    ...
    rv = bcmbd_cmicx_schan_op(unit, &schan_msg, 2 + wsize, 2 + wsize);
    ...
    if (LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
        ...
        bcmbd_cmicx_dump_mem(pb, unit, adext, addr + memidx, resp_data, wsize);
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "SOC table insert: %s\n"), shr_pb_str(pb)));
        shr_pb_destroy(pb);
...
static int
cmicx_default_tbl_delete(int unit, uint32_t adext, uint32_t addr,
                         uint32_t *data, size_t wsize, uint32_t bank_ignore_mask,
                         bcmbd_tbl_resp_info_t *resp_info, uint32_t *resp_data)
{
    int rv;
...
static int
cmicx_default_tbl_lookup(int unit, uint32_t adext, uint32_t addr,
                         uint32_t *data, size_t wsize, uint32_t bank_ignore_mask,
                         bcmbd_tbl_resp_info_t *resp_info, uint32_t *resp_data)
{
    int rv;
                       ...
                       bcmbd_tbl_resp_info_t *resp_info, uint32_t *resp_data)
{
    ...
    return cmicx_default_tbl_insert(unit, adext, addr, data, wsize,
                       ...
                       bcmbd_tbl_resp_info_t *resp_info, uint32_t *resp_data)
{
    ...
    return cmicx_default_tbl_delete(unit, adext, addr, data, wsize,
                                    bank_ignore_mask, resp_info, resp_data);
}
                       ...
                       bcmbd_tbl_resp_info_t *resp_info, uint32_t *resp_data)
{
    ...
    return cmicx_default_tbl_lookup(unit, adext, addr, data, wsize,
                                    bank_ignore_mask, resp_info, resp_data);
}
