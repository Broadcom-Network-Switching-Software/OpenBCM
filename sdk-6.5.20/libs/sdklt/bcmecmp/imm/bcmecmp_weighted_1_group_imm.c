/*! \file bcmecmp_weighted_1_group_imm.c
 *
 * BCMECMP ECMP_LEVEL1_WEIGHTEDt IMM handler.
 */
/*
 ...
 */
/*!
 * \brief ECMP_LEVEL1_WEIGHTED LT entry fields parse routine.
 *
 ...
 */
static int
bcmecmp_weighted_1_group_lt_fields_parse(int unit,
                                         ...
                                         bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(
                 unit,
                 "bcmecmp_weighted_1_group_lt_fields_parse:LT_SID=%d.\n"),
                  sid));

    gen_field = in;
    while (gen_field) {
        SHR_IF_ERR_EXIT
          (BCMECMP_FNCALL_EXEC(unit, grp_ul_lt_fields_fill)
 ...
 */
static int
bcmecmp_weighted_1_group_validate(int unit,
                                  bcmltd_sid_t sid,
                                  ...
                                  void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_weigthed_1_group_validate:SID=%d.\n"),
              sid));
    ...
    SHR_IF_ERR_EXIT
        (bcmecmp_weighted_1_group_lt_fields_parse(unit, key, sid, lt_entry));

    if (((event == BCMIMM_ENTRY_UPDATE) ||
        (event == BCMIMM_ENTRY_INSERT)) && (data != NULL)) {
        SHR_IF_ERR_EXIT
        (bcmecmp_weighted_1_group_lt_fields_parse(unit, data, sid, lt_entry));
    }
    ...
    if (event == BCMIMM_ENTRY_UPDATE) {
        ...
        /* Parse the input fields and retrieve the field values. */
        SHR_IF_ERR_EXIT
            (bcmecmp_weighted_1_group_lt_fields_parse(unit,
                                                 key,
                                                 sid,
                                                 cur_lt_entry));
        /* Get current fields */
 ...
 */
static int
bcmecmp_weighted_1_group_stage(int unit,
                               bcmltd_sid_t sid,
                               ...
                               bcmltd_fields_t *output_fields)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_weighted_1_group_stage: LT_SID=%d.\n"),
              sid));
    ...
    BCMECMP_FLEX_TBL_ENT_STAGED_SET
        (BCMECMP_TBL_PTR(unit, group, BCMECMP_GRP_TYPE_LEVEL1_WEIGHTED),
         hw_ecmp_id);
...
/*!
 * \brief ECMP_LEVEL1_WEIGHTED group imm commit call back
 *
 ...
 */
static int
bcmecmp_weighted_1_group_commit(int unit,
                              bcmltd_sid_t sid,
                              uint32_t trans_id,
                              void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_weighted_1_group_commit: LT_SID=%d.\n"),
              sid));
    ...
    /* Verify device is configured in Hierarchical ECMP mode. */
    if (BCMECMP_MODE_HIER != BCMECMP_MODE(unit)) {
        SHR_IF_ERR_MSG_EXIT(SHR_E_NONE,
                            (BSL_META_U(unit,
 ...
 */
static void
bcmecmp_weighted_1_group_abort(int unit,
                               bcmltd_sid_t sid,
                               uint32_t trans_id,
                               void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_weighted_1_group_abort: LT_SID=%d.\n"),
              sid));
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                     ...
                     BCMECMP_FLEX_TBL_BK_ENT_STAGED_GET
                        (BCMECMP_TBL_PTR(
                         unit,
                         group, BCMECMP_GRP_TYPE_LEVEL1_WEIGHTED),
                         ecmp_id),
                     sid));
        /*
...
/*!
 * \brief ECMP_LEVEL1_WEIGHTED In-memory event callback structure.
 *
 * This structure contains callback functions that will be conresponding
 * to ECMP_LEVEL1_WEIGHTED logical table entry commit stages.
 */
static bcmimm_lt_cb_t event_hdl = {

    /*! Validate function. */
    .validate = bcmecmp_weighted_1_group_validate,

    /*! Extended staging function. */
    .op_stage = bcmecmp_weighted_1_group_stage,

    /*! Commit function. */
    .commit = bcmecmp_weighted_1_group_commit,

    /*! Abort function. */
    .abort = bcmecmp_weighted_1_group_abort
};
 ...
 */
/*!
 * \brief ECMP_LEVEL1_WEIGHTED IMM callback register
 *
 * regist ECMP_LEVEL1_WEIGHTED LT callback in IMM module
 *
 * \param [in] unit This is device unit number.
 *
 * \retval None.
...
bcmecmp_weighted_1_group_imm_register(int unit, bcmlrd_sid_t sid)
{
    ...
    /*
     * To register callback for ECMP_LEVEL1_WEIGHTEDt here.
     */
    ...
    if (SHR_SUCCESS(rv) && map) {
        ...
        LOG_INFO(BSL_LOG_MODULE,
                 (BSL_META_U(unit,
         "Registered IMM event handler for ECMP_LEVEL1_WEIGHTED  SID:%u.\n"),
         sid));
    }
exit:
    SHR_FUNC_EXIT();
