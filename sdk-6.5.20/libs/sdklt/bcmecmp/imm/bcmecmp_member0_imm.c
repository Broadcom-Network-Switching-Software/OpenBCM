/*! \file bcmecmp_member0_imm.c
 *
 * BCMECMP ECMP_MEMBER0t IMM handler.
 */
/*
                        ...
                        bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "bcmecmp_member0_lt_entry_t_init.\n")));
    ...
    if (lt_entry) {
        ...
        lt_entry->max_paths = BCMECMP_MEMBER0_LT_MIN_NPATHS(unit);
        lt_entry->num_paths = BCMECMP_MEMBER0_LT_MIN_NPATHS(unit);
        ...
        for (i = 0; i < member_info->flds_count; i++) {
            ...
            sal_memset
                (lt_entry->member0_field[i], BCMECMP_INVALID,
 ...
 */
static int
bcmecmp_member0_lt_input_params_validate(int unit,
                                 ...
                                 bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    if (!BCMECMP_LT_FIELD_GET
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "GRP_ID field value not specified.\n")));
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    /* Verify GRP_ID is in valid range. */
    if (BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_ECMP_ID)
        && (lt_entry->ecmp_id <  BCMECMP_MEMBER0_LT_MIN_ECMP_ID(unit)
            || lt_entry->ecmp_id > BCMECMP_MEMBER0_LT_MAX_ECMP_ID (unit))) {
    ...
    SHR_IF_ERR_EXIT
        (bcmecmp_grp_in_use(unit, lt_entry->ecmp_id, TRUE, 0,
                            (bcmecmp_grp_type_t)0, &in_use, &comp_id));
    ...
    switch (opcode) {
        case BCMIMM_ENTRY_INSERT:
            ...
            if (in_use) {
                ...
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                                        "GRP_ID=%d exists.\n"),
                             lt_entry->ecmp_id));
                if (lt_entry->glt_sid == BCMECMP_MEMBER0_GRP_LT_SID(unit,
                                                          lt_entry->ecmp_id)) {
            ...
            if (BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_NUM_PATHS)
                && lt_entry->num_paths
                && lt_entry->num_paths > BCMECMP_MEMBER0_LT_MAX_NPATHS(unit)) {
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
            ...
            if (!in_use) {
                ...
                LOG_VERBOSE(BSL_LOG_MODULE,
                            (BSL_META_U(unit,
                             ...
                             lt_entry->ecmp_id, BCMECMP_MEMBER0_GRP_LT_SID(unit,
                             lt_entry->ecmp_id)));
                               ...
                               bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    old_num_paths = BCMECMP_MEMBER0_GRP_NUM_PATHS(unit, ecmp_id);
    old_mstart_idx = BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX(unit, ecmp_id);

    if (new_num_paths < old_num_paths) {
        if (BCMECMP_MEMBER0_TBL_ITBM_LIST(BCMECMP_INFO_PTR(unit))) {
            SHR_IF_ERR_EXIT
                (shr_itbm_list_block_resize(BCMECMP_MEMBER0_TBL_ITBM_LIST
        ...
        BCMECMP_MEMBER0_GRP_NUM_PATHS(unit, ecmp_id) = new_num_paths;
        SHR_EXIT();
    }
    ...
    if (new_num_paths > old_num_paths) {
        ...
        if (BCMECMP_MEMBER0_TBL_ITBM_LIST(BCMECMP_INFO_PTR(unit))) {
            ...
            if (old_mstart_idx + (int)new_num_paths
                > BCMECMP_MEMBER0_TBL_SIZE(unit)) {
                rv = SHR_E_RESOURCE;
            } else {
                rv = shr_itbm_list_block_resize
                        (BCMECMP_MEMBER0_TBL_ITBM_LIST
                            (BCMECMP_INFO_PTR(unit)),
                         old_num_paths,
                         SHR_ITBM_INVALID,
                         old_mstart_idx,
...
/*!
 * \brief ECMP_MEMBER0 LT entry fields parse routine.
 *
 * Parse ECMP_MEMBER0 logical table entry logical fields
 ...
 */
static int
bcmecmp_member0_lt_fields_parse(int unit,
                  ...
                  bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmecmp_grp_in_use(unit, grp_id, TRUE, 0, 0, &in_use, &comp_id));
    ...
    if (in_use) {
        ...
        if (BCMECMP_MEMBER0_TBL_ITBM_LIST(BCMECMP_INFO_PTR((unit)))) {
            ...
            if (lt_entry->mstart_auto) {
                ...
                if (lt_entry->mstart_idx + lt_entry->num_paths - 1 >
                    bcmecmp_flex_mbr_tbl_max(unit, BCMECMP_GRP_TYPE_LEVEL0)) {
                    rv = SHR_E_RESOURCE;
                    SHR_ERR_EXIT(rv);
                }
        ...
        BCMECMP_MEMBER0_GRP_LT_SID(unit, grp_id) = lt_entry->glt_sid;
        BCMECMP_MEMBER0_GRP_NUM_PATHS(unit, grp_id) = num_paths;
        BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX(unit, grp_id) =
                                                       lt_entry->mstart_idx;
        BCMECMP_MEMBER0_GRP_MEMB_TBL_START_AUTO(unit, grp_id) =
        ...
        LOG_DEBUG(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              ...
                              "SID[%d] != [%d].\n"),
                   grp_id,
                   BCMECMP_MEMBER0_GRP_LT_SID(unit, grp_id),
                   lt_entry->glt_sid));
    }
                     ...
                     bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit, "bcmecmp_member0_update.\n")));
    ...
    if (!BCMECMP_LT_FIELD_GET(lt_entry->fbmp, BCMECMP_LT_FIELD_NUM_PATHS)) {
        ...
        lt_entry->num_paths = BCMECMP_MEMBER0_GRP_NUM_PATHS(unit, grp_id);
    ...
    if ((lt_entry->mstart_auto == false) &&
         (lt_entry->mstart_idx != BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX
                                  (unit, grp_id))) {
            /*Move the ECMP group*/
            rv = shr_itbm_list_block_alloc_id
                    (BCMECMP_MEMBER0_TBL_ITBM_LIST
            ...
            lt_entry->mstart_idx =
                      BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX(unit, grp_id);
            lt_entry->num_paths =
                      BCMECMP_MEMBER0_GRP_NUM_PATHS(unit, grp_id);
           ...
           lt_entry->mstart_idx =
                BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX(unit, grp_id) =
                                                               new_mstart_idx;
           lt_entry->num_paths =
                BCMECMP_MEMBER0_GRP_NUM_PATHS(unit, grp_id) = new_num_paths;
    }

    if (lt_entry->num_paths != BCMECMP_MEMBER0_GRP_NUM_PATHS(unit, grp_id)) {
                     ...
                     bcmecmp_flex_lt_entry_t *lt_entry)
{
    ...
    BCMECMP_MEMBER0_GRP_LT_SID(unit, grp_id) = BCMLTD_SID_INVALID;
    BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX(unit, grp_id) = BCMECMP_INVALID;
    BCMECMP_MEMBER0_GRP_MEMB_TBL_START_AUTO(unit, grp_id) = 1;
...
static int
bcmecmp_member0_field_list_get(int unit,
...
static int
bcmecmp_member0_field_list_set(int unit,
                              ...
                              bcmltd_fields_t *out)
{
    ...
    fids = (bcmecmp_member_dest_ltd_fields_t *)BCMECMP_MEMBER0_INFO_FIDS
                                             (unit);
    ...
    bcmecmp_member0_field_list_get(unit, key_fields,  fids->ecmp_id_fid, &(grp_id));

    val = BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX(unit, grp_id);

    SHR_IF_ERR_EXIT
        (bcmecmp_member0_field_list_set(unit, out, fids->base_index_oper_fid, 0, val));

    val = BCMECMP_MEMBER0_GRP_MEMB_TBL_START_AUTO(unit, grp_id);

    SHR_IF_ERR_EXIT
        (bcmecmp_member0_field_list_set(unit, out, fids->base_index_auto_fid, 0, val));
 ...
 */
static int
bcmecmp_member0_validate(int unit,
                               bcmltd_sid_t sid,
                               ...
                               void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_member0_validate:SID=%d.\n"),
              sid));
    BCMECMP_INIT(unit);
    BCMECMP_LOCK(unit);
    ...
    SHR_IF_ERR_EXIT
        (bcmecmp_member0_lt_entry_t_init(unit, sid, lt_entry));
    lt_entry->event = event;

    /* Parse the input fields and retrieve the field values. */
    SHR_IF_ERR_EXIT
        (bcmecmp_member0_lt_fields_parse(unit, key, sid, lt_entry));

    if (((event == BCMIMM_ENTRY_UPDATE) ||
        (event == BCMIMM_ENTRY_INSERT)) && (data != NULL)) {
        SHR_IF_ERR_EXIT
        (bcmecmp_member0_lt_fields_parse(unit, data, sid, lt_entry));
    }
    ...
    SHR_IF_ERR_EXIT
        (bcmecmp_member0_lt_input_params_validate(unit, event, sid, lt_entry));
    ...
    if (event == BCMIMM_ENTRY_UPDATE) {
        ...
        /* Initialize Logical Table Entry buffer. */
        SHR_IF_ERR_EXIT
            (bcmecmp_member0_lt_entry_t_init(unit, sid, cur_lt_entry));

        cur_lt_entry->event = event;

        /* Parse the input fields and retrieve the field values. */
        SHR_IF_ERR_EXIT
            (bcmecmp_member0_lt_fields_parse(unit,
                                                 key,
                                                 sid,
                                                 cur_lt_entry));
 ...
 */
static int
bcmecmp_member0_stage(int unit,
                            bcmltd_sid_t sid,
                            ...
                            bcmltd_fields_t *output_fields)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp__member0_stage: LT_SID=%d.\n"),
              sid));
    BCMECMP_INIT(unit);
    BCMECMP_LOCK(unit);
    ...
    switch (event) {
        case BCMIMM_ENTRY_INSERT:
            SHR_IF_ERR_EXIT(bcmecmp_member0_add(unit, lt_entry));
            break;
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT(bcmecmp_member0_update(unit, lt_entry));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcmecmp_member0_delete(unit, lt_entry));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }

    BCMECMP_MEMBER0_GRP_TRANS_ID(unit, lt_entry->ecmp_id) = op_arg->trans_id;
    ...
    if (BCMECMP_ATOMIC_TRANS(unit)) {
         ...
         * operation.
         */
        BCMECMP_MEMBER0_GRP_TRANS_ID_BK(unit, lt_entry->ecmp_id) = op_arg->trans_id;
    }
 ...
 */
static int
bcmecmp_member0_commit(int unit,
                             bcmltd_sid_t sid,
                             uint32_t trans_id,
                             void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_member0_commit: LT_SID=%d.\n"),
              sid));

    BCMECMP_INIT(unit);
    BCMECMP_LOCK(unit);

    for (grp_id = BCMECMP_MEMBER0_LT_MIN_ECMP_ID(unit);
         grp_id <= BCMECMP_MEMBER0_LT_MAX_ECMP_ID(unit);
         grp_id++ ) {
        if (!BCMECMP_MEMBER0_TBL_ENT_STAGED_GET(BCMECMP_INFO_PTR(unit),
                                        grp_id)
            || (trans_id != BCMECMP_MEMBER0_GRP_TRANS_ID(unit, grp_id))) {
            continue;
        }

        BCMECMP_MEMBER0_TBL_ENT_STAGED_CLEAR(BCMECMP_INFO_PTR( unit), grp_id);
    }
    ...
    tbl_ptr = BCMECMP_MEMBER0_TBL_PTR(unit, member);
    ...
    for (grp_id = BCMECMP_MEMBER0_LT_MIN_ECMP_ID(unit);
        ...
        if (!BCMECMP_MEMBER0_TBL_BK_ENT_STAGED_GET(BCMECMP_INFO_PTR(unit),
                                                    grp_id)
            || (trans_id != BCMECMP_MEMBER0_GRP_TRANS_ID_BK(unit, grp_id))) {
            continue;
        }
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                     ...
                     grp_id,
                     trans_id,
                     BCMECMP_MEMBER0_GRP_TRANS_ID_BK(unit, grp_id),
                                                               sid));


        BCMECMP_MEMBER0_GRP_LT_SID_BK(unit, grp_id) = BCMECMP_MEMBER0_GRP_LT_SID(unit,
                                                                  grp_id);
        BCMECMP_MEMBER0_GRP_NUM_PATHS_BK(unit, grp_id) = BCMECMP_MEMBER0_GRP_NUM_PATHS(unit,
                                                                       grp_id);

        BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX_BK(unit, grp_id) =
                        BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX(unit, grp_id);

        tbl_size = (size_t)BCMECMP_MEMBER0_TBL_SIZE(unit);
        sal_memcpy(tbl_ptr->ent_bk_arr, tbl_ptr->ent_arr,
                   (sizeof(bcmecmp_flex_hw_entry_attr_t) * tbl_size));

        BCMECMP_MEMBER0_TBL_BK_ENT_STAGED_CLEAR(BCMECMP_INFO_PTR(unit),
                                                    grp_id);
    }
 ...
 */
static void
bcmecmp_member0_abort(int unit,
                            bcmltd_sid_t sid,
                            uint32_t trans_id,
                            void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_member0_abort: LT_SID=%d.\n"),
              sid));
    ...
    for (grp_id = BCMECMP_MEMBER0_LT_MIN_ECMP_ID(unit);
        ...
        BCMECMP_MEMBER0_GRP_LT_SID(unit, grp_id) = BCMECMP_MEMBER0_GRP_LT_SID_BK(
                                                             unit, grp_id);
        BCMECMP_MEMBER0_GRP_NUM_PATHS(unit, grp_id) =
                                      BCMECMP_MEMBER0_GRP_NUM_PATHS_BK(unit, grp_id);
        BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX(unit, grp_id) =
                BCMECMP_MEMBER0_GRP_MEMB_TBL_START_IDX_BK(unit, grp_id);
        ...
        BCMECMP_MEMBER0_TBL_BK_ENT_STAGED_CLEAR(BCMECMP_INFO_PTR(unit), grp_id);
    }
...
/*!
 * \brief ECMP_MEMBER0 In-memory event callback structure.
 *
...
static bcmimm_lt_cb_t event_hdl = {
    ...
    /*! Validate function. */
    .validate = bcmecmp_member0_validate,

    /*! Extended staging function. */
    .op_stage = bcmecmp_member0_stage,

    /*! Commit function. */
    .commit = bcmecmp_member0_commit,

    /*! Abort function. */
    .abort = bcmecmp_member0_abort,

    .lookup = bcmecmp_member0_get
};
...
bcmecmp_member0_imm_register(int unit, bcmlrd_sid_t sid)
{
    ...
    /*
     * To register callback for ECMP_MEMBER0t here.
     */
