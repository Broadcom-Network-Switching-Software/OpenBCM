/*! \file bcmecmp_underlay_group_imm.c
 *
 ...
 */
static int
bcmecmp_underlay_group_lt_fields_parse(int unit,
                                       ...
                                       bcmecmp_lt_entry_t *lt_entry)
{
    ...
    while (gen_field) {
        SHR_IF_ERR_VERBOSE_EXIT
          (BCMECMP_FNCALL_EXEC(unit, grp_ul_lt_fields_fill)(unit, gen_field, lt_entry));
...
static int
bcmecmp_underlay_group_rh_fields_validate(int unit,
...
static bool bcmecmp_group_rh_critical_fields_change(int unit,
                                                    bcmecmp_lt_entry_t *lt_entry)
{
    if (((BCMECMP_LT_FIELD_GET
 ...
 */
static int
bcmecmp_underlay_group_validate(int unit,
                                bcmltd_sid_t sid,
                                bcmimm_entry_event_t event,
                                ...
                                void *context)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmecmp_underlay_group_lt_fields_parse(unit, key, sid, lt_entry));

    if (((event == BCMIMM_ENTRY_UPDATE) ||
        (event == BCMIMM_ENTRY_INSERT)) && (data != NULL)) {
        SHR_IF_ERR_EXIT
        (bcmecmp_underlay_group_lt_fields_parse(unit, data, sid, lt_entry));
    }
    ...
    if (event == BCMIMM_ENTRY_UPDATE) {
        ...
        /* Parse the input fields and retrieve the field values. */
        SHR_IF_ERR_EXIT
            (bcmecmp_underlay_group_lt_fields_parse(unit, key, sid, cur_lt_entry));
    ...
    SHR_IF_ERR_EXIT
        (bcmecmp_underlay_group_rh_fields_validate(unit, event, lt_entry));
 ...
 */
static int
bcmecmp_underlay_group_stage(int unit,
                             bcmltd_sid_t sid,
                             ...
                             bcmltd_fields_t *output_fields)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_underlay_group_stage: LT_SID=%d.\n"),
              sid));
    ...
    switch (event) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            if (bcmecmp_group_rh_critical_fields_change(unit, lt_entry)) {
                SHR_IF_ERR_VERBOSE_EXIT
                ...
                if (BCMECMP_ATOMIC_TRANS(unit)) {
                    if (BCMECMP_GRP_RHG_BK_PTR(unit, hw_ecmp_id)) {
                        BCMECMP_FREE
                            (BCMECMP_GRP_RH_ENTRIES_BK_PTR(unit, hw_ecmp_id));
                        BCMECMP_FREE
                            (BCMECMP_GRP_RH_MINDEX_BK_PTR(unit, hw_ecmp_id));
                        BCMECMP_FREE
                            (BCMECMP_GRP_RH_MEMB_ENTCNT_BK_PTR(unit, hw_ecmp_id));
                        BCMECMP_FREE
                            (BCMECMP_GRP_RHG_BK_PTR(unit, hw_ecmp_id));
                    }
                }
                SHR_IF_ERR_EXIT(bcmecmp_group_add(unit, lt_entry));
                break;
            }
    ...
    BCMECMP_TBL_ENT_STAGED_SET
        (BCMECMP_TBL_PTR(unit, group, BCMECMP_GRP_TYPE_UNDERLAY), hw_ecmp_id);
 ...
 */
static int
bcmecmp_underlay_group_commit(int unit,
                              bcmltd_sid_t sid,
                              uint32_t trans_id,
                              void *context)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_underlay_group_commit: LT_SID=%d.\n"),
              sid));
    ...
    grp_list_hdl = ecmp_info->group[BCMECMP_GRP_TYPE_UNDERLAY].list_hdl;
    ...
    hw_ent_attr_base = (bcmecmp_hw_entry_attr_t *)
                           (ecmp_info->group[BCMECMP_GRP_TYPE_UNDERLAY].ent_arr);
    ...
    for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
        ...
        if (BCMECMP_GRP_NHOP_SORTED(unit, ecmp_id) &&
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                   (bcmecmp_lt_entry_t_init(unit, BCMECMP_GRP_LT_SID(unit,
                                                        ecmp_id), cur_entry));
    ...
    hw_ent_attr_base = (bcmecmp_hw_entry_attr_t *)
                       (ecmp_info->group[BCMECMP_GRP_TYPE_UNDERLAY].ent_bk_arr);
    ...
    for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
        ...
        SHR_IF_ERR_EXIT
            (bcmecmp_grp_in_use(unit, ecmp_id, FALSE, 0,
                                BCMECMP_GRP_TYPE_UNDERLAY, &in_use, &comp_id));
        SHR_IF_ERR_EXIT
            (bcmecmp_grp_in_use_bk(unit, ecmp_id, FALSE, 0,
                                   BCMECMP_GRP_TYPE_UNDERLAY, &in_use_bk,
                                   &comp_id));
 ...
 */
static void
bcmecmp_underlay_group_abort(int unit,
                             bcmltd_sid_t sid,
                             uint32_t trans_id,
                             void *context)
{
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                     ...
                     BCMECMP_TBL_BK_ENT_STAGED_GET
                        (BCMECMP_TBL_PTR(
                         unit,
                         group, BCMECMP_GRP_TYPE_UNDERLAY),
                         ecmp_id),
                     sid));
...
static bcmimm_lt_cb_t event_hdl = {
    ...
    /*! Validate function. */
    .validate = bcmecmp_underlay_group_validate,

    /*! Extended staging function. */
    .op_stage = bcmecmp_underlay_group_stage,

    /*! Commit function. */
    .commit = bcmecmp_underlay_group_commit,

    /*! Abort function. */
    .abort = bcmecmp_underlay_group_abort
};
