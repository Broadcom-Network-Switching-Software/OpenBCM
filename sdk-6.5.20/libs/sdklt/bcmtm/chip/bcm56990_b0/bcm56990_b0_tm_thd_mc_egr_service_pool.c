/*! \file bcm56990_b0_tm_thd_mc_egr_service_pool.c
 *
...
mc_cqe_shared_buffer_update(int unit, int itm, int pool, uint32_t new_limit)
{
    ...
    unused_buf = SOC_TH4_B0_MMU_MCQ_ENTRY_PER_ITM;
                  ...
                  bcmltd_fields_t *entry, bcmltd_fields_t *ref_entry)
{
    ...
    write_flag = FALSE;
    if (CEILING(lt_red_resume, TH4_B0_GRANULARITY) <= red_limit &&
        lt_red_resume != lt_red_resume_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_red_resume, TH4_B0_GRANULARITY);
    } else {
                     ...
                     bcmltd_fields_t *entry, bcmltd_fields_t *ref_entry)
{
    ...
    write_flag = FALSE;
    if (CEILING(lt_yellow_resume, TH4_B0_GRANULARITY) <= yellow_limit &&
        lt_yellow_resume != lt_yellow_resume_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_yellow_resume, TH4_B0_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
                              ...
                              bcmltd_fields_t *entry, bcmltd_fields_t *ref_entry)
{
    ...
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
    shared_limit = shared_limit / TH4_B0_GRANULARITY;
    ...
    write_flag = FALSE;
    if (CEILING(lt_resume_limit, TH4_B0_GRANULARITY) <= shared_limit &&
        lt_resume_limit != lt_resume_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_resume_limit, TH4_B0_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    ...
    write_flag = FALSE;
    if (CEILING(lt_yellow_limit, TH4_B0_GRANULARITY) <= shared_limit &&
        lt_yellow_limit != lt_yellow_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_yellow_limit, TH4_B0_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    ...
    write_flag = FALSE;
    if (CEILING(lt_red_limit, TH4_B0_GRANULARITY) <= shared_limit &&
        lt_red_limit != lt_red_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_red_limit, TH4_B0_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
...
tm_thd_mc_egr_service_pool_populate_entry(int unit, int itm, int pool)
{
    ...
    resume_limit *= TH4_B0_GRANULARITY;
    ...
    yellow_limit *= TH4_B0_GRANULARITY;
    ...
    yellow_resume *= TH4_B0_GRANULARITY;
                               ...
                               bcmltd_fields_t *output_fields)
{
    ...
    /* Convert shared limit to 8-cell TH4_B0_GRANULARITY for comparison. */
    shared_limit = shared_limit / TH4_B0_GRANULARITY;
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
    pt_dyn_info.flags = flags;
        ...
        switch (data_ptr->id) {
            case TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLSf:
                ...
                if (fval > shared_limit) {
                    ...
                    field_data = fval * TH4_B0_GRANULARITY;
                }
                SHR_IF_ERR_EXIT
