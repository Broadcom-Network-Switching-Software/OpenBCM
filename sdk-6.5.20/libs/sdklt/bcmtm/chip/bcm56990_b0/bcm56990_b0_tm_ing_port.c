/*! \file bcm56990_b0_tm_ing_port.c
 *
                   ...
                   bcmltd_fields_t *output_fields)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, output_fields,
                         TM_ING_PORTt_OPERATIONAL_STATEf, 0, VALID));
exit:
    SHR_FUNC_EXIT();
}
                  ...
                  bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT
                (tm_ing_port_update(unit, sid, op_arg, lport,
                                    data_fields, output_fields));
            break;
        default:
            /* LT has update_only attribute. */
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
                   ...
                   bcmltd_fields_t *out)
{
    ...
    SHR_IF_ERR_EXIT(
        bcmtm_field_get(unit, ptid, INPPRI_PROFILE_SELf, ltmbuf, &fval));
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out, TM_ING_PORTt_ING_PRI_MAP_IDf, fval));

    SHR_IF_ERR_EXIT(
        bcmtm_field_get(unit, ptid, PG_PROFILE_SELf, ltmbuf, &fval));
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out, TM_ING_PORTt_PRI_GRP_MAP_IDf, fval));
    SHR_IF_ERR_EXIT(
        bcmtm_field_get(unit, ptid, PAUSE_ENABLEf, ltmbuf, &fval));
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out, TM_ING_PORTt_PAUSEf, fval));
...
bcm56990_b0_tm_ing_port_register(int unit)
{
    ...
    tm_thd_cb_handler.op_stage    = tm_ing_port_stage;
    tm_thd_cb_handler.op_lookup = tm_ing_port_lookup;
...
bcm56990_b0_tm_ing_port_populate(int unit)
{
    ...
    for (lport = 0; lport < SOC_MMU_CFG_PORT_MAX; lport++) {
        th4_b0_mmu_config_thdi_set(unit, lport, buf, &devcfg, drv_info->lossless);
        SHR_IF_ERR_EXIT
...
bcm56990_b0_tm_ing_port_update(int unit, int pport, int up)
{
    ...
    if (SHR_SUCCESS(bcmimm_entry_lookup(unit, sid, &keys, &lkup_fields))) {
        ...
        SHR_IF_ERR_EXIT
            (tm_ing_port_update(unit, sid, NULL, lport, data_fields, &out_fields));
        if (up == 0) {
            SHR_IF_ERR_EXIT
