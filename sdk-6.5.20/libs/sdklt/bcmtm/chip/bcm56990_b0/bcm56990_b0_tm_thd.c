                              ...
                              soc_mmu_cfg_buf_qgroup_t *queue_grp)
{
    ...
    default_mtu_cells = TH4_B0_MMU_CFG_MMU_BYTES_TO_CELLS(TH4_B0_MMU_DEFAULT_MTU_BYTES +
                        ...
                        soc_mmu_device_info_t *devcfg,  int lossless)
{
    ...
    default_mtu_cells = TH4_B0_MMU_CFG_MMU_BYTES_TO_CELLS(devcfg->default_mtu_size +
                                                       devcfg->mmu_hdr_byte,
                                   ...
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (itm = 0; itm < TH4_B0_ITMS_PER_DEV; itm ++) {
         ...
         */
        for (pipe = 0; pipe < TH4_B0_PIPES_PER_DEV; pipe++) {
            for (pm = pipe * TH4_B0_PBLKS_PER_PIPE; pm < (pipe + 1) * TH4_B0_PBLKS_PER_PIPE; pm++) {
                ports_in_pm[pm] = drv_info->flex_eligible[pm];
                ports_in_pipe[pipe] += ports_in_pm[pm];
            }
            /* Each pipe cannot have more than 16 ports. */
                                   ...
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    jumbo_frame_cells = TH4_B0_MMU_CFG_MMU_BYTES_TO_CELLS(devcfg->jumbo_pkt_size +
                                                       devcfg->mmu_hdr_byte,
                                                       devcfg->mmu_cell_size);
