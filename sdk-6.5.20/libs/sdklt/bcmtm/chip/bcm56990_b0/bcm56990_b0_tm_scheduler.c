 ...
 * Local definitions
 */
static bcm56990_b0_tm_cos_l0_map_t
bcm56990_b0_tm_cos_l0_map[BCMDRD_CONFIG_MAX_UNITS][TH4_NUM_SCHED_PROFILE];
                              ...
                              int *opcode)
{
    ...
    bcm56990_b0_tm_cos_l0_map_t *l0_map =
            &(bcm56990_b0_tm_cos_l0_map[unit][profile_id]);
                                          ...
                                          bcmtm_scheduler_profile_t *profile)
{
    ...
    if (SHR_E_NONE !=
            bcm56990_b0_tm_cos_l0_map_set(unit, profile_id, profile, &opcode)) {
        goto opstatus_update;
    }

    l0_map = &(bcm56990_b0_tm_cos_l0_map[unit][profile_id]);
    for (idx = 0; idx < TH4_TM_NUM_Q; idx++) {
        num_ucq = profile[idx].num_ucq;
        num_mcq = profile[idx].num_mcq;
        total_q = num_mcq + num_ucq;
                                      ...
                                      bcmtm_scheduler_profile_t *profile)
{
    ...
    if (!profile) {
        for (idx = 0; idx < TH4_NUM_SCHED_PROFILE; idx++) {
            l0_map = &(bcm56990_b0_tm_cos_l0_map[unit][idx]);
            sal_memset(l0_map->map, -1, sizeof(bcm56990_b0_tm_cos_l0_map_t));
        }
    } else {
        l0_map = &(bcm56990_b0_tm_cos_l0_map[unit][profile_id]);
        SHR_IF_ERR_EXIT
