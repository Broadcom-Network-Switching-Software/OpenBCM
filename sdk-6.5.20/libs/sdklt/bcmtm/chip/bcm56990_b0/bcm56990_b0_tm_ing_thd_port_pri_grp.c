/*! \file bcm56990_b0_tm_ing_thd_port_pri_grp.c
 *
    ...
    if (!drv_info->lossless) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
                ...
                TH4_B0_MMU_TOTAL_CELLS_PER_ITM));
    } else {
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
                               ...
                               uint8_t action)
{
    ...
    th4_b0_mmu_config_buf_prigroup_get(unit, lport, pg,
    ...
    while (data_ptr) {
        if (data_ptr->id == TM_ING_THD_PORT_PRI_GRPt_MIN_GUARANTEE_CELLSf) {
            ...
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_thdi_pg_to_pool_mapping_get(unit, lport, pg,
                                                            &itm, &pool, FALSE));
