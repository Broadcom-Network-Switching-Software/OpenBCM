/*! \file bcm56990_b0_tm_mirror_on_drop.c
 *
 ...
 */
static int
bcm56990_b0_tm_mod_buffer_limit_hw_write(int unit,
                                   bcmltd_sid_t ltid,
                                   uint32_t buf_limit,
                                   uint32_t *buf_limit_opr)
{
    ...
    if (SHR_SUCCESS
            (bcm56990_b0_tm_shared_buffer_update(unit, 0, -1,
                                                 delta, RESERVED_BUFFER_UPDATE,
                                                 TRUE)) &&
             SHR_SUCCESS
                 (bcm56990_b0_tm_shared_buffer_update(unit, 1, -1,
                                                delta, RESERVED_BUFFER_UPDATE,
                                                TRUE))) {
...
static int
bcm56990_b0_tm_mod_dest_delete_hw(int unit,
                                  bcmltd_sid_t sid,
                                  uint32_t dest_id)
{
    bcmdrd_sid_t ptid;
...
static int
bcm56990_b0_tm_mod_dest_delete(int unit,
                               bcmltd_sid_t sid,
                               uint32_t dest_id)

{
    ...
    SHR_IF_ERR_EXIT
        (bcm56990_b0_tm_mod_dest_delete_hw(unit, sid, dest_id));
...
static int
bcm56990_b0_tm_mod_dest_port_configured(int unit,
                                        bcmltd_sid_t sid,
                                        uint32_t dest_id,
                                        bcmtm_lport_t lport,
                                        int32_t *configured)
...
static int
bcm56990_b0_tm_mod_dest_set_hw(int unit,
                               bcmltd_sid_t sid,
                               uint32_t dest_id,
                               uint32_t lport,
                               uint32_t queue,
 ...
 */
static int
bcm56990_b0_tm_mod_dest_set(int unit,
                   bcmltd_sid_t ltid,
                   uint32_t dest_id,
                   ...
                   uint32_t opr_old)
{
    ...
    if ((action == ACTION_INSERT) || (action == ACTION_UPDATE)) {
        ...
        if (port_valid) {
            ...
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_mod_dest_port_configured(unit, ltid, dest_id,
                                                         dest_info.lport,
                                                         &port_configured));
            if (port_configured) {
                port_valid = 0;
        ...
        if ((!port_valid) || (!queue_valid)) {
             ...
             if ((action == ACTION_UPDATE) && (opr_old == MOD_MRDP_VALID)) {
                    SHR_IF_ERR_EXIT
                        (bcm56990_b0_tm_mod_dest_delete(unit, ltid, dest_id));
             }
        }
        if (!port_valid) {
            if (port_configured) {
            ...
            *opcode = MOD_MRDP_INCORRECT_QUEUE;
            ...
            SHR_IF_ERR_EXIT
                (bcm56990_b0_tm_mod_dest_set_hw(unit, ltid, dest_id,
        ...
        SHR_IF_ERR_EXIT
            (bcm56990_b0_tm_mod_dest_delete_hw(unit, ltid, dest_id));
 ...
 */
static int
bcm56990_b0_tm_mod_hwm_cor_hw_write(int unit,
                                    bcmltd_sid_t ltid,
                                    uint32_t enable,
                                    uint32_t *dummy_opr)
{
 ...
 */
static int
bcm56990_b0_tm_mod_aging_time_hw_write(int unit,
                                       bcmltd_sid_t ltid,
                                       uint32_t aging_time,
                                       uint32_t *dummy_opr)
{
 ...
 */
static int
bcm56990_b0_tm_mod_encap_set(int unit,
                             bcmltd_sid_t ltid,
...
{
    static bcmtm_mod_drv_t bcm56990_b0_mod_drv = {
        .enable_set = NULL,
        .buffer_set = bcm56990_b0_tm_mod_buffer_limit_hw_write,
        .dest_set = bcm56990_b0_tm_mod_dest_set,
        .hwm_cor_set = bcm56990_b0_tm_mod_hwm_cor_hw_write,
        .aging_time_set = bcm56990_b0_tm_mod_aging_time_hw_write,
        .encap_set = bcm56990_b0_tm_mod_encap_set,
    };

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(mod_drv, SHR_E_INTERNAL);
    *((bcmtm_mod_drv_t *)mod_drv) = bcm56990_b0_mod_drv;
exit:
    SHR_FUNC_EXIT();
}
