/*! \file bcm56780_a0_tm_thd_uc_q.c
 *
        ...
        soc_mmu_cfg_buf_t *buf)
{
    ...
    oper_state = PORT_INFO_UNAVAILABLE;
        ...
        num_q = bcm56780_a0_tm_num_uc_q_non_cpu_port_get(unit);
        if (queue >= num_q) {
                   ...
                   uint8_t action)
{
    ...
    if (ptid1 != INVALIDm) {
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid1, sid, &pt_info, q_config_buf));
        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, ptid1, fid1, q_config_buf, &fval));
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_write(unit, ptid1, sid, &pt_info, q_config_buf));
...
bcm56780_a0_tm_thd_uc_q_populate(int unit)
{
    ...
    for (lport = 1; lport < SOC_MMU_CFG_PORT_MAX; lport++) {
        td4_x9_mmu_config_thdo_set(unit, lport, buf, &devcfg, drv_info->lossless);
        for (queue = 0; queue < TD4_X9_MAX_NUM_UC_Q; queue++) {
...
bcm56780_a0_tm_thd_uc_q_update(int unit, int pport, int up)
{
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        ...
        for (queue = 0; queue < bcm56780_a0_tm_num_uc_q_non_cpu_port_get(unit);
                queue++) {
            SHR_IF_ERR_EXIT
