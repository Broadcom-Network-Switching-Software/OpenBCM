...
#define TD4_X9_EBST_FIFODMA_BUFFER_ENTRIES 16384
#define TD4_X9_EBST_FIFO_CHAN0 1
 ...
 */
static int
bcm56780_a0_tm_ebst_timestamp_init(int unit)
{
    bcmdrd_sid_t sid = 0;
    bcmdrd_fid_t fid = 0;
 ...
 */
static int
bcm56780_a0_tm_ebst_port_config_set(int unit,
                                    bcmltd_sid_t ltid,
                                    bcmtm_lport_t lport,
 ...
 */
static int
bcm56780_a0_tm_ebst_q_config_set(int unit,
                                 bcmltd_sid_t ltid,
                                 bcmtm_lport_t lport,
                                 uint16_t q_id,
                                 bcmtm_ebst_q_cfg_t *q_cfg)
{
    ...
    if (bcmtm_lport_is_cpu(unit, lport)) {
        ...
        if (q_cfg->q_type == MC_Q) {
            q_id += bcm56780_a0_tm_num_uc_q_non_cpu_port_get(unit);
        }
    }
 ...
 */
static int
bcm56780_a0_tm_ebst_scan_config_set(int unit,
                                    bcmltd_sid_t ltid,
...
#define TM_EBST_MAX_BUF_WIDTH   2
    ...
    /* WAR JIRA-TD4_X9-5841 */
    /* disabling EBST enable */
    fid = EBST_ENABLEf;
    val = 0;
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, fid, ltmbuf, &val));
    ...
    if (ebst_cfg->scan_mode == EBST_SCAN_MODE_PORTSP_UC) {
        /* resetting EBST PTR RESET for port SP unicast EBST. */
        fid = EBST_PTR_RESETf;
        val = 1;
        SHR_IF_ERR_EXIT
            (bcmtm_field_set(unit, sid, fid, ltmbuf, &val));
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, fid, ltmbuf, &val));
    /* update the new scan round time. */
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, sid, ltid, &pt_info, ltmbuf));
 ...
 */
static int
bcm56780_a0_tm_ebst_data_parse(int unit,
                               uint32_t *entry,
 ...
 */
static int
bcm56780_a0_tm_ebst_enable(int unit,
                           bcmltd_sid_t ltid,
                           bool enable)
{
    uint32_t intr_num;
    bcmdrd_sid_t sid = MMU_GLBCFG_BST_TRACKING_ENABLEr;
    bcmdrd_fid_t fid = BST_TRACK_EN_THDO0f;
    uint32_t ltmbuf = 0, fval = 0;
    bcmtm_pt_info_t pt_info = {0};

    SHR_FUNC_ENTER(unit);
    intr_num = MMU_INTR_THDO_EBST_FIFO_FULL;

    if (enable) {
        SHR_IF_ERR_EXIT
            (bcmtm_interrupt_enable(unit, intr_num,
                                    bcmtm_ebst_intr_notify, INTR_ENABLE));
    } else {
        SHR_IF_ERR_EXIT
            (bcmtm_interrupt_enable(unit, intr_num, NULL, INTR_DISABLE));
    }
 ...
 */
static int
bcm56780_a0_tm_ebst_map_alloc(int unit)
{
    int size;
    ...
    /* EBST queue map. */
    size = TD4_X9_TM_CHIP_QUEUES * sizeof(bcmtm_ebst_entity_map_t);
    ...
    size = TD4_X9_DEV_PORTS_PER_DEV * TD4_X9_MMU_SERVICE_POOL *
 ...
 */
static int
bcm56780_a0_tm_ebst_portsp_index_get(int unit,
                                     bcmtm_lport_t lport,
                                     uint8_t spid,
                                     uint16_t *map_idx)
{
 ...
 */
static int
bcm56780_a0_tm_ebst_q_map_index_get(int unit,
                                    bcmtm_lport_t lport,
                                    uint16_t q_id,
                                    ...
                                    uint16_t *map_idx)
{
    ...
    if (q_type == UC_Q) {
        SHR_IF_ERR_EXIT
            (bcmtm_lport_ucq_base_get(unit, lport, &chip_q));
        chip_q += q_id;
    } else if (q_type == MC_Q) {
        SHR_IF_ERR_EXIT
            (bcmtm_lport_mcq_base_get(unit, lport, &chip_q));
        chip_q += q_id;
    }
    *map_idx = (uint32_t)chip_q;
exit:
    SHR_FUNC_EXIT();
}
 ...
 */
int
bcm56780_a0_tm_ebst_drv_get(int unit, void *ebst_drv)
{
    bcmtm_ebst_drv_t bcm56780_a0_tm_ebst_drv = {
        .ebst_port_config_set = bcm56780_a0_tm_ebst_port_config_set,
        .ebst_scan_config_set = bcm56780_a0_tm_ebst_scan_config_set,
        .ebst_portsp_index_get = bcm56780_a0_tm_ebst_portsp_index_get,
        .ebst_map_alloc = bcm56780_a0_tm_ebst_map_alloc,
        .ebst_q_config_set = bcm56780_a0_tm_ebst_q_config_set,
        .ebst_q_map_index_get = bcm56780_a0_tm_ebst_q_map_index_get,
        .ebst_data_parse = bcm56780_a0_tm_ebst_data_parse,
        .ebst_enable = bcm56780_a0_tm_ebst_enable
    };

    SHR_FUNC_ENTER(unit);

    SHR_NULL_CHECK(ebst_drv, SHR_E_INTERNAL);
    *((bcmtm_ebst_drv_t *)ebst_drv) = bcm56780_a0_tm_ebst_drv;
exit:
    SHR_FUNC_EXIT();
}

int
bcm56780_a0_tm_ebst_init(int unit, bool warm)
{
    bcmtm_ebst_dev_info_t *ebst_info;
    uint16_t chan_id[TD4_X9_ITMS_PER_DEV] = {TD4_X9_EBST_FIFO_CHAN0};
    int idx;
    ...
    /* populate ebst info */
    ebst_info->max_buffer_pool = TD4_X9_ITMS_PER_DEV;
    ebst_info->num_entries = TD4_X9_EBST_FIFODMA_BUFFER_ENTRIES;
    ebst_info->thd_entries = TD4_X9_EBST_FIFODMA_BUFFER_ENTRIES / 10;
    for (idx = 0; idx < TD4_X9_ITMS_PER_DEV; idx++) {
    ...
    if (!warm) {
        SHR_IF_ERR_EXIT
            (bcm56780_a0_tm_ebst_timestamp_init(unit));
    }
exit:
    SHR_FUNC_EXIT();
}
