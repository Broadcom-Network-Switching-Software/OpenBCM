 ...
 */
/*!
 * \brief Wred unicast queue related physical table configurations.
 *
 ...
 */
static int
bcm56780_a0_tm_wred_ucq_set(int unit,
                            bcmtm_lport_t lport,
                            uint32_t ucq_id,
                            bcmltd_sid_t ltid,
                            bcmtm_wred_cfg_t *wred_ucq_cfg)
{
    ...
    fid = PROFILE_INDEXf;
    if (wred_ucq_cfg->ecn_mode == AVG_Q_SIZE) {
        fval = wred_ucq_cfg->wcs_profile_id;
        SHR_IF_ERR_EXIT(bcmtm_field_set(unit, sid, fid, &ltmbuf, &fval));
    }

    /* write to hardware */
    BCMTM_PT_DYN_INFO(pt_info, ucq_id, 0);
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, sid, ltid, &pt_info, &ltmbuf));

    /* instantaneous ECN. */
    sid = MMU_WRED_ECN_MARK_CONFIG_0_0m;
    ltmbuf = fval = 0;
    fid = PROFILE_INDEXf;
    if (wred_ucq_cfg->ecn_mode == INSTANT_Q_SIZE) {
        fval = wred_ucq_cfg->wcs_profile_id;
        SHR_IF_ERR_EXIT(bcmtm_field_set(unit, sid, fid, &ltmbuf, &fval));
...
/*!
 * \brief Wred port service pool related physical table configurations.
 *
 * \param [in] unit Unit Number.
 * \param [in] lport Logical port ID.
 ...
 */
static int
bcm56780_a0_tm_wred_portsp_set(int unit,
                               bcmtm_lport_t lport,
                               uint32_t portsp_id,
                               bcmltd_sid_t ltid,
                               bcmtm_wred_cfg_t *wred_portsp_cfg)
{
    ...
    /* Write to hardware */
    BCMTM_PT_DYN_INFO(pt_info, portsp_id, 0);
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, sid, ltid, &pt_info, &ltmbuf));
exit:
    SHR_FUNC_EXIT();
...
/*!
 * \brief Wred congestion resolution related physical table configurations.
 *
 * \param [in] unit Unit Number.
 * \param [in] lport Logical port ID.
 ...
 */
static int
bcm56780_a0_tm_wred_cng_profile_set(int unit,
                                    bcmltd_sid_t ltid,
                                    bcmtm_wred_cng_profile_t *cng_profile)
{
    ...
    idx = (sp << TD4_X9_WRED_CNG_SERVICE_POOL_OFFSET)|
          (q_min << TD4_X9_WRED_CNG_Q_MIN_OFFSET)|
           q_avg;
...
/*!
 * \brief WRED congestion profile init.
 *
 * \param [in] unit Unit Number.
 *
 * \retval SHR_E_NONE No error.
 ...
 */
static int
bcm56780_a0_tm_wred_cng_profile_init(int unit)
{
    bcmltd_fields_t in;
    bcmltd_fid_t fid;
    size_t num_fid;
    int idx, profile_id = 0;
    uint32_t offset;
    ...
    for ( ; profile_id < TD4_X9_WRED_CNG_MAX_PROFILE; profile_id++) {
        cng_profile.profile_id = profile_id;
        for (idx = 0; idx < TD4_X9_WRED_CNG_ENTRY_PER_PROFILE; idx++) {
            cng_profile.q_avg = idx & 0x1;
            offset = TD4_X9_WRED_CNG_Q_MIN_OFFSET;
            cng_profile.q_min =
                 (idx & (1 << offset)) >> offset;
            cng_profile.sp = idx >> TD4_X9_WRED_CNG_SERVICE_POOL_OFFSET;
            ...
            SHR_IF_ERR_EXIT
                (bcm56780_a0_tm_wred_cng_profile_set(unit, ltid, &cng_profile));
        }
    }
exit:
 ...
 */
int
bcm56780_a0_tm_wred_enable(int unit)
{
...
int
bcm56780_a0_tm_wred_drv_get(int unit, void *wred_drv)
{

    bcmtm_wred_drv_t bcm56780_a0_wred_drv = {
        .ucq_set = bcm56780_a0_tm_wred_ucq_set,
        .portsp_set = bcm56780_a0_tm_wred_portsp_set,
        .cng_profile_set = bcm56780_a0_tm_wred_cng_profile_set
    };

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(wred_drv, SHR_E_INTERNAL);

    *((bcmtm_wred_drv_t *)wred_drv) = bcm56780_a0_wred_drv;
exit:
    SHR_FUNC_EXIT();
}

int
bcm56780_a0_tm_wred_chip_init(int unit, bool warm)
{
    ...
    if (!warm) {
        SHR_IF_ERR_EXIT(bcm56780_a0_tm_wred_cng_profile_init(unit));
    }
exit:
    SHR_FUNC_EXIT();
}
