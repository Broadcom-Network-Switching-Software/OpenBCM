/*! \file bcm56996_a0_tm_thd_uc_q.c
 *
                           ...
                           soc_mmu_cfg_buf_queue_t *buf_queue)
{
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        num_q = bcm56996_a0_tm_num_uc_q_non_cpu_port_get(unit);
    } else {
        num_q = 0;
    }
    if (queue >= num_q) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
                ...
                buf_queue->pool_resume * TH4G_GRANULARITY));
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
...
bcm56996_a0_tm_thd_uc_q_populate(int unit)
{
    ...
    for (lport = 1; lport < SOC_MMU_CFG_PORT_MAX; lport++) {
        th4g_mmu_config_buf_queue_get(unit, lport, &buf_queue);
        for (queue = 0; queue < TH4G_MAX_NUM_UC_Q; queue++) {
            SHR_IF_ERR_EXIT
...
bcm56996_a0_tm_thd_uc_q_update(int unit, int pport, int up)
{
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        ...
        for (queue = 0; queue < bcm56996_a0_tm_num_uc_q_non_cpu_port_get(unit);
                queue++) {
            SHR_IF_ERR_EXIT
