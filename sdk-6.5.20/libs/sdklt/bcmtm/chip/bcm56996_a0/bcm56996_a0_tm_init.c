/*! \file bcm56996_a0_tm_init.c
 *
 * File containing device level and port level init sequence for bcm56996_a0.
 */
/*
...
static bcmtm_port_info_t cpu_ports[TH4G_NUM_CPU_PORTS] = {
    /* log_port, phy_port, pipe, idb_port, mmu_port */
    { 0, 0, 0, 16, 16 }
};

static bcmtm_port_info_t lb_ports[TH4G_NUM_LB_PORTS] = {
...
static bcmtm_port_info_t mgmt_ports[TH4G_NUM_MGMT_PORTS] = {
    { 152, 257, 8,  16, 272},     /* management port 0 */
    { 50,  258, 2,  16, 80 },     /* management port 1 */
};

#define TH4G_MC_Q_MODE_DEFAULT 2
#define TH4G_NUM_CPU_QUEUES 48
#define TH4G_NUM_GP_QUEUES  12

#define BCMTM_TH4G_AUX_PORT_SPEED        10000
#define BCMTM_TH4G_10G_MGMT_PORT_SPEED   10000
#define BCMTM_TH4G_40G_MGMT_PORT_SPEED   40000
#define CLK(mhz)      (BCM56996_A0_LRD_DEVICE_CLK_FREQ_T_T_CLK_##mhz)

/* From Device_CLK_FREQ_T.type.ltl. */
static
unsigned int th4g_freq_retrieve_from_enum(int unit, int freq_enum) {
    uint32_t frequency;
...
port_pipe_get(int unit, int phys_port)
{
    ...
    if ((phys_port < 0) || (phys_port >= TH4G_NUM_PHYS_PORTS)) {
        return -1;
    }
   ...
   /* Regular ports */
   pipe = (phys_port - 1) / TH4G_PHYS_PORTS_PER_PIPE;
...
port_idbport_get(int unit, int phys_port, int dev_port)
{
    ...
    if ((dev_port < 0) || (dev_port >= TH4G_DEV_PORTS_PER_DEV)) {
        return -1;
    }
    ...
    lcl_phys_port = ((phys_port - 1) % TH4G_PHYS_PORTS_PER_PIPE);
    pipe_pm = lcl_phys_port >> 2;
    subp = lcl_phys_port % TH4G_PORTS_PER_PBLK;

    idb_port = (subp * TH4G_PORTS_PER_PBLK) + pipe_pm;
...
port_mmuport_get(int unit, int phys_port, int log_port)
{
    ...
    mmu_port = (pipe * TH4G_MMU_PORTS_PER_PIPE) + mmu_lcl_port;
    return mmu_port;
}

static int
th4g_uc_cosq_base_get(int unit, int pport, int lport)
{
    int pipe, pipe_queue_base[TH4G_PIPES_PER_DEV], pipe_index;
    int local_mmu_port;
    ...
    pipe_queue_base[0] = 0;
    for (pipe_index = 1; pipe_index < TH4G_PIPES_PER_DEV; pipe_index++) {
    ...
    return (pipe_queue_base[pipe] + local_mmu_port * TH4G_NUM_GP_QUEUES);
}

static int
th4g_mc_cosq_base_get(int unit, int pport, int lport)
{
    int uc_q_base, num_mcq;
    uc_q_base = th4g_uc_cosq_base_get(unit, pport, lport);
    ...
    num_mcq = bcm56996_a0_tm_num_mc_q_non_cpu_port_get(unit);
    return (uc_q_base + TH4G_NUM_GP_QUEUES - num_mcq);
}

static void
bcm56996_a0_bcmtm_dev_info_set(int unit)
{
    uint32_t dev_flags;
    struct bcmtm_device_info bcm56996_a0_tm_dev_info = {
        .max_pkt_sz   = TH4G_MMU_MAX_PACKET_BYTES,
        .pkt_hdr_sz   = TH4G_MMU_PACKET_HEADER_BYTES,
        .jumbo_pkt_sz = TH4G_MMU_JUMBO_FRAME_BYTES,
        .default_mtu  = TH4G_MMU_DEFAULT_MTU_BYTES,
        .cell_sz      = TH4G_MMU_BYTES_PER_CELL,
        .num_cells    = TH4G_MMU_TOTAL_CELLS_PER_ITM,
        .num_port_pg  = SOC_MMU_CFG_PRI_GROUP_MAX,
        .num_sp       = SOC_MMU_CFG_SERVICE_POOL_MAX,
        .mcq_entry    = SOC_TH4G_MMU_MCQ_ENTRY_PER_ITM,
        .num_q        = TH4G_NUM_GP_QUEUES,
        .num_cpu_q    = TH4G_NUM_CPU_QUEUES,
        .num_pipe     = TH4G_PIPES_PER_DEV,
        .num_nhop_sparse = TH4G_NHOPS_PER_REPL_INDEX_SPARSE_MODE,
        .num_nhop_dense = TH4G_NHOPS_PER_REPL_INDEX_DENSE_MODE,
        .max_num_repl = TH4G_MAX_MC_PKT_REPL,
        .num_buffer_pool  = TH4G_ITMS_PER_DEV,};
    bcmtm_device_info[unit] = bcm56996_a0_tm_dev_info;

    dev_flags = bcmdrd_dev_flags_get(unit);
    if (dev_flags & CHIP_FLAG_PIPE_F00F) {
        bcmtm_device_info[unit].num_buffer_pool = TH4G_ITMS_PER_DEV / 2;
        bcmtm_device_info[unit].num_pipe = TH4G_PIPES_PER_DEV / 2;
    }
}
 ...
 */
static int
th4g_dpr_frequency_range_check(int unit, int dpr_freq)
{
 ...
 */
static int
th4g_tm_drv_info_populate(int unit, bool warm)
{
    uint64_t config_value;
    bcmtm_drv_info_t *drv_info;
    int pm_id;
    uint64_t flex_port[TH4G_PBLKS_PER_DEV] = {0};
    size_t actual;
    ...
    SHR_IF_ERR_EXIT
        (bcmcfg_field_get(unit, DEVICE_CONFIGt, DEVICE_CONFIGt_CORE_CLK_FREQf,
                          &config_value));
    drv_info->frequency = th4g_freq_retrieve_from_enum(unit, config_value);
    SHR_IF_ERR_EXIT
        (bcmcfg_field_get(unit, DEVICE_CONFIGt, DEVICE_CONFIGt_PP_CLK_FREQf,
                          &config_value));
    drv_info->dpr_clock_frequency = th4g_freq_retrieve_from_enum(unit, config_value);
    ...
    SHR_IF_ERR_EXIT
        (bcmcfg_field_array_get(unit, TM_PM_FLEX_CONFIGt,
                                ...
                                0, TH4G_PBLKS_PER_DEV,
                                flex_port, &actual));

    for (pm_id = 0; pm_id < TH4G_PBLKS_PER_DEV; pm_id++) {
        drv_info->flex_eligible[pm_id] = flex_port[pm_id];
    }

    SHR_IF_ERR_EXIT
        (th4g_dpr_frequency_range_check(unit, drv_info->dpr_clock_frequency));
...
{
#define BCMTM_TH4G_CPU_LB_PORT_SPEED 10000
    ...
    for (idx = 0; idx < (int)num_ports; idx++) {
        ...
        if (!bcmtm_lport_is_fp(unit, lport)) {
            if (port_map->lport_map[lport].cur_speed < BCMTM_TH4G_CPU_LB_PORT_SPEED) {
                /* set speed to minimum supported internal to TM */
                port_map->lport_map[lport].cur_speed = BCMTM_TH4G_CPU_LB_PORT_SPEED;
            }
        }
        ...
        chip_port =
            (port_map->lport_map[lport].pipe * TH4G_DEV_PORTS_PER_PIPE) +
        ...
        if (bcmtm_lport_is_cpu(unit, lport)) {
            ...
            port_map->lport_map[lport].max_speed = BCMTM_TH4G_CPU_LB_PORT_SPEED;
            port_map->lport_map[lport].cur_speed = BCMTM_TH4G_CPU_LB_PORT_SPEED;
        } else {
            port_map->lport_map[lport].num_mcq =
                        bcm56996_a0_tm_num_mc_q_non_cpu_port_get(unit);
            port_map->lport_map[lport].num_ucq =
                        bcm56996_a0_tm_num_uc_q_non_cpu_port_get(unit);
        }
        ...
        port_map->lport_map[lport].base_ucq = th4g_uc_cosq_base_get(unit, pport,
                port_cfg->lport);
        port_map->lport_map[lport].base_mcq = th4g_mc_cosq_base_get(unit, pport,
                port_cfg->lport);
...
#undef BCMTM_TH4G_CPU_LB_PORT_SPEED
}

static int
th4g_mmu_thdo_pipe_itm_config(int unit) {
...
static int
th4g_mmu_rqe_queue_number_init(int unit)
{
    bcmtm_pt_info_t pt_dyn_info = {0};
    uint32_t ltmbuf;
    bcmdrd_sid_t sid;
...
static int
th4g_check_mmu_mem_init_done(int unit)
{
    struct {
...
th4g_init_tm_memory(int unit)
{
    ...
    if (emul) {
        SHR_IF_ERR_EXIT(th4g_check_mmu_mem_init_done(unit));
    }
...
static int
th4g_cell_pointer_init(int unit)
{
    int itm;
...
static int
th4g_mmu_intfi_enable(int unit)
{
...
static int
bcm56996_a0_tm_pfc_ddr_info_alloc(int unit, bool warm)
{
    ...
    alloc_sz = data_sz * TH4G_DEV_PORTS_PER_DEV;
    ...
    SHR_IF_ERR_EXIT
        ((alloc_sz < (data_sz * TH4G_DEV_PORTS_PER_DEV)) ? SHR_E_MEMORY: SHR_E_NONE);
    ...
    if (!warm) {
        ...
        bcmissu_struct_info_report(unit, BCMMGMT_TM_COMP_ID,
                                   ...
                                   data_sz, TH4G_DEV_PORTS_PER_DEV,
...
static int
bcm56996_a0_tm_port_map_alloc(int unit, bool warm)
{
    bcmtm_port_map_info_t port_map;
    uint32_t data_sz, alloc_sz;
    size_t num_inst;
    ...
    num_inst = TH4G_DEV_PORTS_PER_DEV;
    ...
    num_inst = TH4G_MMU_PORTS_PER_PIPE * TH4G_PIPES_PER_DEV;
    alloc_sz = data_sz * num_inst;
...
bcm56996_a0_tm_num_uc_q_non_cpu_port_get(int unit)
{
    int num_mcq;
    num_mcq = bcm56996_a0_tm_num_mc_q_non_cpu_port_get(unit);
    return (TH4G_NUM_GP_QUEUES - num_mcq);
}
...
int
bcm56996_a0_ip_port_forwarding_bitmap_enable(int unit, bcmpc_pport_t pport) {
    uint32_t bmp[9] = {0};
    int lport;
    bcmdrd_sid_t sid;
    bcmltd_sid_t ltid = -1; /* Requesting LT ID not available */
...
int
bcm56996_a0_ip_port_forwarding_bitmap_disable(int unit, bcmpc_pport_t pport) {
    uint32_t bmp[9] = {0};
    int lport;
    bcmdrd_sid_t sid;
    bcmltd_sid_t ltid = -1; /* Requesting LT ID not available */
                         ...
                         const bcmpc_mmu_port_cfg_t *new_cfg)
{
    ...
    int num_fp_ports_pipe[TH4G_PIPES_PER_DEV] = {0};
    uint64_t config_value;
    uint32_t flex_port[TH4G_PBLKS_PER_DEV] = {0};
    bcmtm_drv_info_t *drv_info;
    ...
    for (idx = 0; idx < (int)num_ports; idx++) {
        ...
        if (bcmtm_lport_is_cpu(unit, lport) || (bcmtm_lport_is_lb(unit, lport))) {
            if ((new_cfg[idx].num_lanes != TH4G_NUM_LANES_AUX_PORT) ||
                (new_cfg[idx].speed_max != BCMTM_TH4G_AUX_PORT_SPEED)) {
                SHR_ERR_EXIT(SHR_E_PARAM);
            }
            continue;
        }
    ...
    if (num_fp_ports > TH4G_MAX_NUM_FP_PORTS) {
        /* Check total number of front panel ports */
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    for (pipe = 0; pipe < TH4G_PIPES_PER_DEV; pipe++) {
        if (num_fp_ports_pipe[pipe] > TH4G_MAX_NUM_FP_PORTS_PIPE) {
    ...
    if (num_400g_ports != 0) {
        /* Check frequency */
        if (th4g_freq_retrieve_from_enum(unit, config_value) < 1325) {
            SHR_ERR_EXIT(SHR_E_PARAM);
        }
    }
...
bcm56996_a0_tm_chip_init(int unit, bool warm)
{
    ...
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_port_map_alloc(unit, warm));

    SHR_IF_ERR_EXIT
        (th4g_tm_drv_info_populate(unit, warm));

    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_pfc_ddr_info_alloc(unit, warm));
    ...
    SHR_BITSET(ltmbuf_arr, TH4G_CPU_LPORT);
    BCMTM_PT_DYN_INFO(pt_dyn_info, 0, 0);
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, CPU_PBMm, -1, (void *)&pt_dyn_info,
                                ltmbuf_arr));

    bcm56996_a0_bcmtm_dev_info_set(unit);

    /* Device level init sequence. Skipping all in case of warmboot. */
    if (!warm) {
        SHR_IF_ERR_EXIT
            (th4g_init_tm_memory(unit));
        SHR_IF_ERR_EXIT
            (th4g_en_mmu_refresh(unit));
        SHR_IF_ERR_EXIT
            (th4g_cell_pointer_init(unit));
        SHR_IF_ERR_EXIT
            (th4g_mmu_intfi_enable(unit));
        SHR_IF_ERR_EXIT
            (th4g_mmu_thdo_pipe_itm_config(unit));
        SHR_IF_ERR_EXIT
            (th4g_mmu_rqe_queue_number_init(unit));
        SHR_IF_ERR_EXIT
            (bcm56996_a0_tm_idb_general_init(unit));
        SHR_IF_ERR_EXIT
            (bcm56996_a0_tm_tdm_mmu_init(unit));
        SHR_IF_ERR_EXIT
            (bcm56996_a0_tm_rqe_threshold_init(unit));
        SHR_IF_ERR_EXIT
            (bcm56996_a0_tm_mmu_config_init_thresholds(unit));
        SHR_IF_ERR_EXIT
            (bcm56996_a0_tm_wred_enable(unit));
        SHR_IF_ERR_EXIT
            (bcm56996_a0_tm_ct_init(unit));
    }
    /* Scheduler shaper chip init */
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_scheduler_shaper_chip_init(unit, warm));
    /* OBM threshold init. */
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_obm_init(unit, warm));
    /* BST Init. */
    SHR_IF_ERR_EXIT
        (bcmtm_bst_init(unit, warm));

    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_ebst_init(unit, warm));

    /* Multicast init. */
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_multicast_init(unit, warm));
    /* WRED init. */
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_wred_chip_init(unit, warm));

    if (!warm) {
        SHR_IF_ERR_EXIT
            (bcm56996_a0_tm_imm_populate(unit));
    }
...
bcm56996_a0_tm_port_delete(int unit, bcmtm_pport_t pport)
{
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_port_idb_delete(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_port_mmu_delete(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_port_edb_delete(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_imm_update(unit, pport, 0));
exit:
    SHR_FUNC_EXIT();
}
...
bcm56996_a0_tm_port_add(int unit, bcmtm_pport_t pport)
{
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_port_idb_add(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_port_mmu_add(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_port_edb_add(unit, pport));
    SHR_IF_ERR_EXIT
        (bcm56996_a0_tm_imm_update(unit, pport, 1));
exit:
    SHR_FUNC_EXIT();
}
    ...
    if (bcmtm_lport_is_fp(unit, lport)) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_get(unit, XLMAC_TXFIFO_CELL_REQ_CNTr, REQ_CNTf, ltmbuf,
                             &mac_requests));
        /* TH4G management port, PM->EDB interface is different. Every 2 requests
...
bcm56996_a0_tm_mchip_port_lport_get(int unit, int mchip_port, int *lport)
{
    ...
    mlocal_port = mchip_port % TH4G_DEV_PORTS_PER_PIPE;
    pipe = mchip_port / TH4G_DEV_PORTS_PER_PIPE;

    mport = pipe * TH4G_MMU_PORTS_PER_PIPE + mlocal_port;
