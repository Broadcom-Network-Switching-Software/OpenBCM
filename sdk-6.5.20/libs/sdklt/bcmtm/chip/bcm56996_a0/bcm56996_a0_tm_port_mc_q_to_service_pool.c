/*! \file bcm56996_a0_tm_port_mc_q_to_service_pool.c
 *
                                            ...
                                            int up, soc_mmu_cfg_buf_t *buf)
{
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        num_q = bcm56996_a0_tm_num_mc_q_non_cpu_port_get(unit);
    } else {
        num_q = SOC_TH4G_NUM_CPU_QUEUES;
    }
    if (queue >= num_q) {
...
int
bcm56996_a0_tm_port_mc_q_to_service_pool_register(int unit)
{
...
int
bcm56996_a0_tm_port_mc_q_to_service_pool_populate(int unit)
{
    ...
    for (lport = 0; lport < SOC_MMU_CFG_PORT_MAX; lport++) {
        th4g_mmu_config_thdo_set(unit, lport, buf, &devcfg, drv_info->lossless);
        if (bcmtm_lport_is_cpu(unit, lport)) {
            max_num_mcq = SOC_TH4G_NUM_CPU_QUEUES;
        } else {
            max_num_mcq = TH4G_MAX_NUM_MC_Q;
        }
...
int
bcm56996_a0_tm_port_mc_q_to_service_pool_update(int unit, int pport, int up)
{
    int lport, queue, max_num_mcq, rv;
    const bcmlrd_map_t *map = NULL;
