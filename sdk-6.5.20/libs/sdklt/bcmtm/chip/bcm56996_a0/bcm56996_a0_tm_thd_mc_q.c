/*! \file bcm56996_a0_tm_thd_mc_q.c
 *
                           ...
                           soc_mmu_cfg_buf_queue_t *buf_queue)
{
    ...
    if (queue >= num_q) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
             TM_THD_MC_Qt_SHARED_LIMIT_DYNAMICf, 0, buf_queue->pool_scale));
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
                TM_THD_MC_Qt_COLOR_SPECIFIC_DYNAMIC_LIMITSf, 0, 1));
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
                TM_THD_MC_Qt_YELLOW_LIMIT_DYNAMICf, 0, buf_queue->yellow_limit));
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
                TM_THD_MC_Qt_RED_LIMIT_DYNAMICf, 0, buf_queue->red_limit));
    }
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
                   ...
                   uint8_t action)
{
    ...
    th4g_mmu_config_buf_queue_get(unit, lport, &buf_queue);
    ...
    while (data_ptr) {
        ...
        switch (data_ptr->id) {
            case TM_THD_MC_Qt_MIN_GUARANTEE_CELLSf:
                ...
                if (SHR_SUCCESS
                        (bcm56996_a0_tm_shared_buffer_update(unit, 0, pool, delta,
                                                             RESERVED_BUFFER_UPDATE,
                                                             TRUE)) &&
                    SHR_SUCCESS
                        (bcm56996_a0_tm_shared_buffer_update(unit, 1, pool, delta,
                                                             RESERVED_BUFFER_UPDATE,
                                                             TRUE))) {
            ...
            case TM_THD_MC_Qt_COLOR_SPECIFIC_DYNAMIC_LIMITSf:
                ...
                if (use_dynamic_color) {
                    ...
                    if (SHR_SUCCESS
                        ...
                        limit = CEILING(limit, TH4G_GRANULARITY);
                        SHR_IF_ERR_EXIT
                            (bcmtm_field_set(unit, ptid, LIMIT_YELLOWf,
                                             q_config_buf, &limit));
                    }
...
static int
tm_thd_mc_q_delete(int unit, bcmltd_sid_t sid, const bcmltd_op_arg_t *op_arg,
                   int lport, int queue)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_thd_mc_q_update(unit, sid, op_arg, lport, queue, data_fields,
                            &dummy_fields, FALSE));
                  ...
                  bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_thd_mc_q_delete(unit, sid, op_arg, lport, queue));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
                   ...
                   bcmltd_fields_t *out)
{
    ...
    fid = TM_THD_MC_Qt_PORT_IDf;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_imm_fields_get(unit, key, fid, &idx));
    lport = idx;
    fid = TM_THD_MC_Qt_TM_MC_Q_IDf;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_imm_fields_get(unit, key, fid, &idx));
    queue = idx;
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_Qt_MIN_GUARANTEE_CELLSf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_Qt_DYNAMIC_SHARED_LIMITSf,
                                fval));
    if (fval) {
        SHR_IF_ERR_EXIT(
        ...
        SHR_IF_ERR_EXIT(
            bcmcth_imm_field_update(unit, out,
                                    TM_THD_MC_Qt_SHARED_LIMIT_DYNAMICf,
                                    fval));
    } else {
        SHR_IF_ERR_EXIT(
            bcmtm_field_get(unit, ptid, SHARED_LIMITf, ltmbuf, &fval));
        SHR_IF_ERR_EXIT(
            bcmcth_imm_field_update(unit, out,
                                    TM_THD_MC_Qt_SHARED_LIMIT_CELLS_STATICf,
                                    fval));
    }
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_Qt_COLOR_SPECIFIC_LIMITSf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_Qt_COLOR_SPECIFIC_DYNAMIC_LIMITSf,
                                fval));
    if (fval) {
        SHR_IF_ERR_EXIT(
        ...
        SHR_IF_ERR_EXIT(
            bcmcth_imm_field_update(unit, out,
                                    TM_THD_MC_Qt_YELLOW_LIMIT_DYNAMICf,
                                    fval));
        SHR_IF_ERR_EXIT(
            bcmtm_field_get(unit, ptid, LIMIT_REDf, ltmbuf, &fval));
        SHR_IF_ERR_EXIT(
            bcmcth_imm_field_update(unit, out,
                                    TM_THD_MC_Qt_RED_LIMIT_DYNAMICf,
                                    fval));
    } else {
        SHR_IF_ERR_EXIT(
            bcmtm_field_get(unit, ptid, LIMIT_YELLOWf, ltmbuf, &fval));
        ...
        SHR_IF_ERR_EXIT(
            bcmcth_imm_field_update(unit, out,
                                    TM_THD_MC_Qt_YELLOW_LIMIT_CELLS_STATICf,
                                    fval));
        SHR_IF_ERR_EXIT(
            bcmtm_field_get(unit, ptid, LIMIT_REDf, ltmbuf, &fval));
        fval = fval * TH4G_GRANULARITY;
        SHR_IF_ERR_EXIT(
            bcmcth_imm_field_update(unit, out,
                                    TM_THD_MC_Qt_RED_LIMIT_CELLS_STATICf,
                                    fval));
    }
    SHR_IF_ERR_EXIT(
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_Qt_TM_EGR_BST_THD_Q_PROFILE_IDf,
                                fval));
    SHR_IF_ERR_EXIT(
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_Qt_TM_EBST_PROFILE_IDf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_Qt_RESUME_OFFSET_CELLSf,
                                fval));
...
bcm56996_a0_tm_thd_mc_q_register(int unit)
{
    ...
    tm_thd_cb_handler.op_stage = tm_thd_mc_q_stage;
    tm_thd_cb_handler.op_lookup = tm_thd_mc_q_lookup;
...
bcm56996_a0_tm_thd_mc_q_populate(int unit)
{
    ...
    for (lport = 0; lport < SOC_MMU_CFG_PORT_MAX; lport++) {
        ...
        for (queue = 0; queue < max_num_mcq; queue++) {
            SHR_IF_ERR_EXIT
                (tm_thd_mc_q_populate_entry(unit, lport, queue, 1,
                                            &buf_queue));
        }
    }
...
bcm56996_a0_tm_thd_mc_q_update(int unit, int pport, int up)
{
    ...
    for (queue = 0; queue < max_num_mcq; queue++) {
        ...
        if (SHR_SUCCESS(bcmimm_entry_lookup(unit, sid, &keys, &lkup_fields))) {
            ...
            SHR_IF_ERR_EXIT
                (tm_thd_mc_q_update(unit, sid, NULL, lport, queue, data_fields,
                                    &out_fields, up));
            if (up == 0) {
                SHR_IF_ERR_EXIT
