 ...
 */
/*!
 * \brief TH4G wred unicast queue related physical table configurations.
 *
 ...
 */
static int
bcm56996_a0_tm_wred_ucq_set(int unit,
                            bcmtm_lport_t lport,
                            uint32_t ucq_id,
                            bcmltd_sid_t ltid,
...
/*!
 * \brief TH4G wred port service pool related physical table configurations.
 *
 * \param [in] unit Unit Number.
 * \param [in] lport Logical port ID.
 ...
 */
static int
bcm56996_a0_tm_wred_portsp_set(int unit,
                               bcmtm_lport_t lport,
                               uint32_t portsp_id,
                               bcmltd_sid_t ltid,
...
/*!
 * \brief TH4G wred congestion resolution related physical table configurations.
 *
 * \param [in] unit Unit Number.
 * \param [in] lport Logical port ID.
 ...
 */
static int
bcm56996_a0_tm_wred_cng_profile_set(int unit,
                                    bcmltd_sid_t ltid,
                                    bcmtm_wred_cng_profile_t *cng_profile)
{
    ...
    idx = (sp << TH4G_WRED_CNG_SERVICE_POOL_OFFSET)|(q_min << TH4G_WRED_CNG_Q_MIN_OFFSET)|q_avg;
...
/*!
 * \brief TH4G WRED congestion profile init.
 *
 * \param [in] unit Unit Number.
 *
 * \retval SHR_E_NONE No error.
 ...
 */
static int
bcm56996_a0_tm_wred_cng_profile_init(int unit)
{
    bcmltd_fields_t in;
    bcmltd_fid_t fid;
    size_t num_fid;
    ...
    for (profile_id = 0; profile_id < TH4G_WRED_CNG_MAX_PROFILE; profile_id++) {
        cng_profile.profile_id = profile_id;
        for (idx = 0; idx < TH4G_WRED_CNG_ENTRY_PER_PROFILE; idx++) {
            cng_profile.q_avg = idx & 0x1;
            cng_profile.q_min =
                 (idx & (1 << TH4G_WRED_CNG_Q_MIN_OFFSET)) >> TH4G_WRED_CNG_Q_MIN_OFFSET;
            cng_profile.sp = idx >> TH4G_WRED_CNG_SERVICE_POOL_OFFSET;
            ...
            SHR_IF_ERR_EXIT
                (bcm56996_a0_tm_wred_cng_profile_set(unit, ltid, &cng_profile));
        }
    }
exit:
...
bcm56996_a0_tm_wred_drv_get(int unit, void *wred_drv)
{
    ...
    bcmtm_wred_drv_t bcm56996_a0_wred_drv = {
        .ucq_set = bcm56996_a0_tm_wred_ucq_set,
        .portsp_set = bcm56996_a0_tm_wred_portsp_set,
        .cng_profile_set = bcm56996_a0_tm_wred_cng_profile_set
    };

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(wred_drv, SHR_E_INTERNAL);

    *((bcmtm_wred_drv_t *)wred_drv) = bcm56996_a0_wred_drv;
exit:
    SHR_FUNC_EXIT();
}
...
bcm56996_a0_tm_wred_chip_init(int unit, bool warm)
{
    ...
    if (!warm) {
        SHR_IF_ERR_EXIT(bcm56996_a0_tm_wred_cng_profile_init(unit));
    }
exit:
    SHR_FUNC_EXIT();
}
