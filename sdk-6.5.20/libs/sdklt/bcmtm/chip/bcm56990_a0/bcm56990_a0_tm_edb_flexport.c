/*! \file bcm56990_a0_tm_edb_flexport.c
 *
...
static const th4_edb_e2e_setting_t
th4_edb_e2e_tbl[TH4_EDB_E2E_TBL_ENTRIES] = {
    ...
    */
    ...
    {TH4_EDB_E2E_TBL_SPD_LPBK,       8,    57,      57,     {{  1,   1}, { 1,  1}} },   /* LPBK  */
    {TH4_EDB_E2E_TBL_SPD_CMIC,       9,    25,      15,     {{  1,   1}, { 1,  1}} },   /* CMIC  */
    {TH4_EDB_E2E_TBL_SPD_MGMT_10G,  10,     7,       5,     {{ 26,  26}, {16, 16}} },   /* MGMT_10G */
    {TH4_EDB_E2E_TBL_SPD_MGMT_40G,  11,    27,      17,     {{101, 101}, {61, 61}} }    /* MGMT_40G */
};

/* Speed encoding index in th4_edb_e2e_tbl for special ports */
#define TH4_EDB_E2E_LPBK_SPD_ENCODE         8
#define TH4_EDB_E2E_CMIC_SPD_ENCODE         9
#define TH4_EDB_E2E_MGMT_10G_SPD_ENCODE     10
#define TH4_EDB_E2E_MGMT_40G_SPD_ENCODE     11

/***************************************************************************
 * LOCAL WRAPPER FUNCTIONS
 */
static int
th4_edb_pt_write(int unit, int pt_id, int pt_inst, bcmdrd_sid_t mem,
...
static int
th4_edb_pt_read(int unit, int pt_id, int pt_inst, bcmdrd_sid_t mem,
                bcmdrd_fid_t fid, uint32_t *fval)
{
    bcmtm_pt_info_t pt_info = {0};
    bcmltd_sid_t ltid = -1; /* Requesting LT ID not available */
 ...
 */
/*!
 * \brief Get index of table th4_edb_e2e_tbl by given lport.
 *
 ...
 */
static int
th4_edb_e2e_tbl_speed_encoding_get(int unit, int lport)
{
    int idx, speed = 0;
    int spd_encoding = 0;
    ...
    if (bcmtm_lport_is_lb(unit, lport)) {
        return TH4_EDB_E2E_LPBK_SPD_ENCODE;
    }
    /* CPU */
    else if (bcmtm_lport_is_cpu(unit, lport)) {
        return TH4_EDB_E2E_CMIC_SPD_ENCODE;
    }
    /* MGM: 10G or 40G */
    else if (bcmtm_lport_is_mgmt(unit, lport)) {
        if (port_map->lport_map[lport].cur_speed == 40000) {
            return TH4_EDB_E2E_MGMT_40G_SPD_ENCODE;
        } else {
            return TH4_EDB_E2E_MGMT_10G_SPD_ENCODE;
        }
    }
    ...
    for (idx = 0; idx < TH4_EDB_E2E_TBL_ENTRIES; idx++) {
        if (th4_edb_e2e_tbl[idx].speed >= speed) {
            spd_encoding = idx;
            break;
        }
    }
...
/*!
 * \brief Get index of xmit_cnt[] in table th4_edb_e2e_tbl.
 *
 * \param [in] unit: Device unit.
 *
 ...
 */
static int
th4_edb_e2e_tbl_opt_encoding_get(int unit)
{
    ...
    return (opt_encoding % TH4_EDB_E2E_NUM_PFC_OPT_CFG);
}

/*!
 * \brief Get CT class of the given port in th4_edb_e2e_tbl.
 *
 * \param [in] unit: Device unit.
 * \param [in] lport: Logical port.
 *
 ...
 */
static int
th4_edb_e2e_tbl_ct_class_get(int unit, int lport)
{
    int spd_encoding, ct_class;

    spd_encoding = th4_edb_e2e_tbl_speed_encoding_get(unit, lport);
    ct_class = th4_edb_e2e_tbl[spd_encoding].ct_class;

    return ct_class;
}

/*!
 * \brief Get ep2mmu credit of the given port from th4_edb_e2e_tbl.
 *
 * \param [in] unit: Device unit.
 * \param [in] lport: Logical port.
 *
 ...
 */
static int
th4_edb_e2e_tbl_mmu_credit_get(int unit, int lport)
{
    int spd_encoding, opt_encoding;
    int credit = 0;

    spd_encoding = th4_edb_e2e_tbl_speed_encoding_get(unit, lport);
    opt_encoding = th4_edb_e2e_tbl_opt_encoding_get(unit);

    if (opt_encoding == 0) {
        /* General cases */
        credit = th4_edb_e2e_tbl[spd_encoding].mmu_credit;
    } else {
        /* PFC Optimized cases for 1325MHz */
        credit = th4_edb_e2e_tbl[spd_encoding].mmu_credit_pfc_opt;
    }
 ...
 */
int
th4_ep_get_ep2mmu_credit_per_speed(int unit, int speed, uint32_t *credit)
{
    int spd_encoding, opt_encoding;
    int idx;
...
/*!
 * \brief Get SAF or CT xmit_cnt from th4_edb_e2e_tbl.
 *
 * \param [in] unit: Device unit.
 * \param [in] lport: Logical port.
 ...
 */
static int
th4_edb_e2e_tbl_xmit_cnt_get(int unit, int lport, int ct_valid)
{
    int spd_encoding, opt_encoding;
    int xmit_cnt = 0;
    ...
    if (bcmtm_lport_is_fp(unit, lport)) {
        if (ct_valid == 0) {
            xmit_cnt = th4_edb_e2e_tbl[spd_encoding].xmit_cnt[opt_encoding].saf;
        } else {
            xmit_cnt = th4_edb_e2e_tbl[spd_encoding].xmit_cnt[opt_encoding].ct;
        }
    } else if (bcmtm_lport_is_mgmt(unit, lport)) {
        /* MGM: 10G or 40G SAF */
        xmit_cnt = th4_edb_e2e_tbl[spd_encoding].xmit_cnt[opt_encoding].saf;
 ...
 */
static int
th4_edb_drain_port(int unit, int lport)
{
    bcmtm_port_map_info_t *port_map;
    ...
    SHR_IF_ERR_EXIT(th4_edb_pt_read(unit, pt_id, pt_inst, reg, fid, &fval_saved));
    SHR_IF_ERR_EXIT(th4_edb_pt_write(unit, pt_id, pt_inst, reg, fid, fval));
    ...
    do {
        ...
        SHR_IF_ERR_EXIT
            (th4_edb_pt_read(unit, pt_id, pt_inst, mem, fid, &num_entries));
 ...
 */
static int
th4_edb_port_enable_set(int unit, int lport, int rst_on)
{
    bcmtm_port_map_info_t *port_map;
    bcmdrd_sid_t mem;
    bcmdrd_fid_t fid;
    ...
    SHR_IF_ERR_EXIT(th4_edb_pt_write(unit, pt_id, pt_inst, mem, fid, fval));
 ...
 */
static int
th4_edb_port_buf_sft_rst_set(int unit, int lport, int rst_on)
{
    bcmtm_port_map_info_t *port_map;
    bcmdrd_sid_t mem;
    bcmdrd_fid_t fid;
 ...
 */
static int
th4_edb_dev2phy_mapping_set(int unit, int lport, int rst_on)
{
    bcmtm_port_map_info_t *port_map;
    bcmdrd_sid_t mem;
    bcmdrd_fid_t fid;
 ...
 */
static int
th4_edb_ep2mmu_port_credit_get(int unit, int lport)
{
    int credit = 0;

    /* FP ports */
    if (bcmtm_lport_is_fp(unit, lport)) {
        credit = th4_edb_e2e_tbl_mmu_credit_get(unit, lport);
    }
    /* AUX ports */
    else if (bcmtm_lport_is_lb(unit, lport) ||
             bcmtm_lport_is_cpu(unit, lport) ||
 ...
 */
static int
th4_edb_port_xmit_cnt_array_get(int unit, int lport,
                                int *xmit_cnt, int xmit_cnt_size)
{
    ...
    if (bcmtm_lport_is_fp(unit, lport)) {
        /* FP */
        saf_xmit_cnt = th4_edb_e2e_tbl_xmit_cnt_get(unit, lport, 0);
        ct_xmit_cnt  = th4_edb_e2e_tbl_xmit_cnt_get(unit, lport, 1);
        ...
        for (src_ct_class = 0; src_ct_class < xmit_cnt_size; src_ct_class++) {
            xmit_cnt[src_ct_class] = (src_ct_class == 0) ?
               ...
               bcmtm_lport_is_mgmt(unit, lport)) {
        /* LPBK, CPU and MGM */
        xmit_cnt[0] = th4_edb_e2e_tbl_xmit_cnt_get(unit, lport, 0);
    }
 ...
 */
static int
th4_edb_mmu_cell_credit_set(int unit, int lport, int rst_on)
{
    bcmtm_port_map_info_t *port_map;
    bcmdrd_sid_t mem;
    bcmdrd_fid_t fid;
    ...
    bcmtm_port_map_get(unit, &port_map);
    pport  = port_map->lport_map[lport].pport;
    credit = (rst_on == 1) ? 0 : (th4_edb_ep2mmu_port_credit_get(unit, lport));
 ...
 */
static int
th4_edb_ct_class_set(int unit, int lport, int rst_on)
{
    bcmdrd_sid_t mem;
    bcmdrd_fid_t fid;
    uint32_t fval;
     ...
     */
    ct_class = (rst_on == 1) ? 0 : (th4_edb_e2e_tbl_ct_class_get(unit, lport));
 ...
 */
static int
th4_edb_xmit_start_cnt_set(int unit, int lport, int rst_on)
{
    bcmtm_port_map_info_t *port_map;
    bcmdrd_sid_t mem;
    bcmdrd_fid_t fid;
    ...
    int pport, pipe_num;
    int local_lport;
    int xmit_cnt[TH4_EDB_MAX_CT_CLASSES];
    ...
    if (rst_on == 1) {
        sal_memset(xmit_cnt, 0, TH4_EDB_MAX_CT_CLASSES * sizeof(int));
    } else {
        th4_edb_port_xmit_cnt_array_get(unit, lport, xmit_cnt,
                                        TH4_EDB_MAX_CT_CLASSES);
    }
    ...
    for (ct_class = 0; ct_class < TH4_EDB_MAX_CT_CLASSES; ct_class++) {
        local_lport = lport % TH4_DEV_PORTS_PER_PIPE;
        pt_id = local_lport * TH4_EDB_MAX_CT_CLASSES + ct_class;
 ...
 */
static int
th4_port_edb_down(int unit, int lport)
{
    int rst_on = 1;
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit, "th4_port_edb_down lport=%0d\n"), lport));
     ...
     */
    SHR_IF_ERR_EXIT(th4_edb_drain_port(unit, lport));
     ...
     */
    SHR_IF_ERR_EXIT(th4_edb_port_enable_set(unit, lport, rst_on));
    SHR_IF_ERR_EXIT(th4_edb_port_buf_sft_rst_set(unit, lport, rst_on));
 ...
 */
static int
th4_port_edb_invalidate(int unit, int lport)
{
    int rst_on = 1;
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit, "th4_port_edb_invalidate lport=%0d\n"), lport));
    /*
     * Configure port related property.
     * 1. Delete Dev_to_Phy mapping.
     * 2. Reset max Ep2MMU credits to 0.
     */
    SHR_IF_ERR_EXIT(th4_edb_dev2phy_mapping_set(unit, lport, rst_on));
    SHR_IF_ERR_EXIT(th4_edb_mmu_cell_credit_set(unit, lport, rst_on));
 ...
 */
static int
th4_port_edb_add(int unit, int lport)
{
    int rst_on = 0;
    SHR_FUNC_ENTER(unit);

    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit, "th4_port_edb_add lport=%0d\n"), lport));
    ...
    SHR_IF_ERR_EXIT(th4_edb_dev2phy_mapping_set(unit, lport, rst_on));
    SHR_IF_ERR_EXIT(th4_edb_mmu_cell_credit_set(unit, lport, rst_on));
    SHR_IF_ERR_EXIT(th4_edb_ct_class_set(unit, lport, rst_on));
    SHR_IF_ERR_EXIT(th4_edb_xmit_start_cnt_set(unit, lport, rst_on));
 ...
 */
static int
th4_port_edb_up(int unit, int lport)
{
    int rst_on = 0;
    SHR_FUNC_ENTER(unit);

    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit, "th4_port_edb_up lport=%0d\n"), lport));
 ...
 */
int
bcm56990_a0_tm_port_edb_down(int unit, bcmtm_pport_t pport)
{
    bcmtm_port_map_info_t *port_map;
    int lport;
...
int
bcm56990_a0_tm_port_edb_delete(int unit, bcmtm_pport_t pport)
{
    bcmtm_port_map_info_t *port_map;
    int lport;
    ...
    lport = port_map->pport_map[pport].lport;
    SHR_IF_ERR_EXIT
        (th4_port_edb_invalidate(unit, lport));
...
int
bcm56990_a0_tm_port_edb_up(int unit, bcmtm_pport_t pport)
{
    bcmtm_port_map_info_t *port_map;
    int lport;
...
int
bcm56990_a0_tm_port_edb_add(int unit, bcmtm_pport_t pport)
{
    bcmtm_port_map_info_t *port_map;
    int lport;
