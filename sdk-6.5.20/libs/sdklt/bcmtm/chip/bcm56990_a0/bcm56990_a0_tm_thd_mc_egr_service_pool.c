/*! \file bcm56990_a0_tm_thd_mc_egr_service_pool.c
 *
 * IMM handlers for threshold LTs.
 *
 * This file contains callbcak handlers for LT TM_THD_MC_EGR_SERVICE_POOL.
 */
/*
...
static int
mc_cqe_shared_buffer_update(int unit, int itm, int pool, uint32_t new_limit)
{
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_sid_t ptid = MMU_THDO_MC_SHARED_CQE_POOL_SHARED_LIMITr;
    bcmdrd_fid_t fid = SHARED_LIMITf;
    bcmltd_sid_t sid = TM_THD_MC_EGR_SERVICE_POOLt;
    uint32_t ltmbuf = 0;
    uint32_t pool_shared_limit[SOC_MMU_CFG_SERVICE_POOL_MAX] = {0};
    uint32_t unused_buf, fval;
    int pool_idx;

    SHR_FUNC_ENTER(unit);

    unused_buf = SOC_TH4_MMU_MCQ_ENTRY_PER_ITM;
    ...
    if (new_limit > pool_shared_limit[pool] &&
        new_limit - pool_shared_limit[pool] > unused_buf) {
        return SHR_E_PARAM;
    }
...
{
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_sid_t ptid = MMU_THDO_MC_SHARED_CQE_POOL_RED_RESUME_LIMITr;
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_RESUME_LIMIT_CELLSf, 0, &lt_red_resume));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_RESUME_LIMIT_CELLS_OPERf, 0, &lt_red_resume_oper));
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
    sal_memset(ltmbuf, 0, sizeof(ltmbuf));

    write_flag = FALSE;
    if (CEILING(lt_red_resume, TH4_GRANULARITY) <= red_limit &&
        lt_red_resume != lt_red_resume_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_red_resume, TH4_GRANULARITY);
    } else {
    ...
    if (write_flag) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, entry,
                TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_RESUME_LIMIT_CELLS_OPERf,
                0, fval * TH4_GRANULARITY));
    }
...
{
    bcmtm_pt_info_t pt_dyn_info = {0};
    bcmdrd_sid_t ptid = MMU_THDO_MC_SHARED_CQE_POOL_YELLOW_RESUME_LIMITr;
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_RESUME_LIMIT_CELLSf, 0, &lt_yellow_resume));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_RESUME_LIMIT_CELLS_OPERf, 0, &lt_yellow_resume_oper));
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
    sal_memset(ltmbuf, 0, sizeof(ltmbuf));

    write_flag = FALSE;
    if (CEILING(lt_yellow_resume, TH4_GRANULARITY) <= yellow_limit &&
        lt_yellow_resume != lt_yellow_resume_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_yellow_resume, TH4_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    ...
    if (write_flag) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, entry,
                 TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_RESUME_LIMIT_CELLS_OPERf,
                 0, fval * TH4_GRANULARITY));
    }
                              ...
                              bcmltd_fields_t *entry, bcmltd_fields_t *ref_entry)
{
    ...
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
    shared_limit = shared_limit / TH4_GRANULARITY;

    /* Check shared resume limit. */
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_SHARED_RESUME_LIMIT_CELLSf, 0, &lt_resume_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_SHARED_RESUME_LIMIT_CELLS_OPERf, 0, &lt_resume_limit_oper));
    ptid = MMU_THDO_MC_SHARED_CQE_POOL_RESUME_LIMITr;
    fid = RESUME_LIMITf;
    sal_memset(ltmbuf, 0, sizeof(ltmbuf));

    write_flag = FALSE;
    if (CEILING(lt_resume_limit, TH4_GRANULARITY) <= shared_limit &&
        lt_resume_limit != lt_resume_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_resume_limit, TH4_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    ...
    if (write_flag) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, entry,
                 TM_THD_MC_EGR_SERVICE_POOLt_SHARED_RESUME_LIMIT_CELLS_OPERf,
                 0, fval * TH4_GRANULARITY));
    }
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_LIMIT_CELLSf, 0, &lt_yellow_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_LIMIT_CELLS_OPERf, 0, &lt_yellow_limit_oper));
    ptid = MMU_THDO_MC_SHARED_CQE_POOL_YELLOW_SHARED_LIMITr;
    fid = YELLOW_SHARED_LIMITf;
    sal_memset(ltmbuf, 0, sizeof(ltmbuf));

    write_flag = FALSE;
    if (CEILING(lt_yellow_limit, TH4_GRANULARITY) <= shared_limit &&
        lt_yellow_limit != lt_yellow_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_yellow_limit, TH4_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    ...
    if (write_flag) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, entry,
                 TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_LIMIT_CELLS_OPERf,
                 0, fval * TH4_GRANULARITY));
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLSf, 0, &lt_red_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_get(unit, ref_entry,
            TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLS_OPERf, 0, &lt_red_limit_oper));
    ptid = MMU_THDO_MC_SHARED_CQE_POOL_RED_SHARED_LIMITr;
    fid = RED_SHARED_LIMITf;
    sal_memset(ltmbuf, 0, sizeof(ltmbuf));

    write_flag = FALSE;
    if (CEILING(lt_red_limit, TH4_GRANULARITY) <= shared_limit &&
        lt_red_limit != lt_red_limit_oper) {
        /* Install user configured value */
        write_flag = TRUE;
        fval = CEILING(lt_red_limit, TH4_GRANULARITY);
    } else {
        /* Check current operating value */
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    ...
    if (write_flag) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, entry,
                 TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLS_OPERf,
                 0, fval * TH4_GRANULARITY));
...
static int
tm_thd_mc_egr_service_pool_populate_entry(int unit, int itm, int pool)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_BUFFER_POOLf, 0, itm));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_TM_EGR_SERVICE_POOL_IDf, 0, pool));
    ...
    ptid = MMU_THDO_MC_SHARED_CQE_POOL_SHARED_LIMITr;
    sal_memset(ltmbuf, 0, sizeof(ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    SHR_IF_ERR_EXIT
    ...
    yellow_limit *= TH4_GRANULARITY;

    /* Read pool yellow resume limit. */
    ptid = MMU_THDO_MC_SHARED_CQE_POOL_YELLOW_RESUME_LIMITr;
    sal_memset(ltmbuf, 0, sizeof(ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_field_get(unit, ptid, YELLOW_RESUME_LIMITf, ltmbuf, &yellow_resume));
    yellow_resume *= TH4_GRANULARITY;
    ...
    red_limit *= TH4_GRANULARITY;

    /* Read pool red resume limit. */
    ptid = MMU_THDO_MC_SHARED_CQE_POOL_RED_RESUME_LIMITr;
    sal_memset(ltmbuf, 0, sizeof(ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, ptid, sid, &pt_dyn_info, ltmbuf));
    SHR_IF_ERR_EXIT
        (bcmtm_field_get(unit, ptid, RED_RESUME_LIMITf, ltmbuf, &red_resume));
    red_resume *= TH4_GRANULARITY;

    /* Read per ITM color limit control bitmap. */
    ptid = MMU_THDO_MC_SHARED_CQE_POOL_CONFIGr;
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_OPERATIONAL_STATEf, 0, VALID));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_RESUME_LIMIT_CELLS_OPERf, 0, red_resume));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_RESUME_LIMIT_CELLSf, 0, red_resume));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLS_OPERf, 0, red_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLSf, 0, red_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_RESUME_LIMIT_CELLS_OPERf, 0, yellow_resume));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_RESUME_LIMIT_CELLSf, 0, yellow_resume));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_LIMIT_CELLS_OPERf, 0, yellow_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_LIMIT_CELLSf, 0, yellow_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_COLOR_SPECIFIC_LIMITSf, 0, enable_color_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_SHARED_RESUME_LIMIT_CELLS_OPERf, 0, resume_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_SHARED_RESUME_LIMIT_CELLSf, 0, resume_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_SHARED_LIMIT_CELLS_OPERf, 0, shared_limit));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
            TM_THD_MC_EGR_SERVICE_POOLt_SHARED_LIMIT_CELLSf, 0, shared_limit));
...
static int
tm_thd_mc_egr_service_pool_update(int unit,
                               bcmltd_sid_t sid,
                               ...
                               bcmltd_fields_t *output_fields)
{
    ...
    in.field[0]->id   = TM_THD_MC_EGR_SERVICE_POOLt_TM_EGR_SERVICE_POOL_IDf;
    in.field[0]->data = pool;
    in.field[1]->id   = TM_THD_MC_EGR_SERVICE_POOLt_BUFFER_POOLf;
    in.field[1]->data = itm;
    in.count = 2;
    SHR_IF_ERR_EXIT
        (bcmlrd_field_count_get(unit, sid, &num_fid));
    ...
    while (data_ptr) {
        switch (data_ptr->id) {
            case TM_THD_MC_EGR_SERVICE_POOLt_COLOR_SPECIFIC_LIMITSf:
                ptid = MMU_THDO_MC_SHARED_CQE_POOL_CONFIGr;
                ...
                SHR_IF_ERR_EXIT
                    (bcmtm_pt_indexed_write(unit, ptid, sid, &pt_dyn_info, ltmbuf));
                break;
            case TM_THD_MC_EGR_SERVICE_POOLt_SHARED_LIMIT_CELLSf:
                /* Check against other MC-CQE service pools. */
                rv = mc_cqe_shared_buffer_update(unit, itm, pool, data_ptr->data);
                if (rv == SHR_E_NONE) {
                    /*
    ...
    /* Convert shared limit to 8-cell TH4_GRANULARITY for comparison. */
    shared_limit = shared_limit / TH4_GRANULARITY;
    BCMTM_PT_DYN_INFO(pt_dyn_info, pool, itm);
    pt_dyn_info.flags = flags;
    ...
    while (data_ptr) {
        ...
        fval = data_ptr->data;
        switch (data_ptr->id) {
            case TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLSf:
                ptid = MMU_THDO_MC_SHARED_CQE_POOL_RED_SHARED_LIMITr;
                fid = RED_SHARED_LIMITf;
                fval = CEILING(fval, TH4_GRANULARITY);
                if (fval > shared_limit) {
                    /* Cannot accommodate. */
                    write_flag = FALSE;
                    field_id = TM_THD_MC_EGR_SERVICE_POOLt_OPERATIONAL_STATEf;
                    field_data = INCORRECT_COLOR_LIMIT;
                } else {
                    field_id = TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLS_OPERf;
                    field_data = fval * TH4_GRANULARITY;
                }
                SHR_IF_ERR_EXIT
                    (bcmtm_field_list_set(unit, output_fields, field_id, 0, field_data));
                break;
            case TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_LIMIT_CELLSf:
                ptid = MMU_THDO_MC_SHARED_CQE_POOL_YELLOW_SHARED_LIMITr;
                fid = YELLOW_SHARED_LIMITf;
                fval = CEILING(fval, TH4_GRANULARITY);
                if (fval > shared_limit) {
                    ...
                    field_id = TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_LIMIT_CELLS_OPERf;
                    field_data = fval * TH4_GRANULARITY;
                    SHR_IF_ERR_EXIT
                ...
                SHR_IF_ERR_EXIT
                    (bcmtm_field_list_set(unit, output_fields, field_id, 0, field_data));
                break;
            case TM_THD_MC_EGR_SERVICE_POOLt_SHARED_RESUME_LIMIT_CELLSf:
                ptid = MMU_THDO_MC_SHARED_CQE_POOL_RESUME_LIMITr;
                fid = RESUME_LIMITf;
                fval = CEILING(fval, TH4_GRANULARITY);
                if (fval > shared_limit) {
                    ...
                    field_id = TM_THD_MC_EGR_SERVICE_POOLt_SHARED_RESUME_LIMIT_CELLS_OPERf;
                    field_data = fval * TH4_GRANULARITY;
                    SHR_IF_ERR_EXIT
    ...
    while (data_ptr) {
        ...
        fval = data_ptr->data;
        switch (data_ptr->id) {
            case TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_RESUME_LIMIT_CELLSf:
                /* Red HW. */
                ptid = MMU_THDO_MC_SHARED_CQE_POOL_RED_SHARED_LIMITr;
                fid = RED_SHARED_LIMITf;
                sal_memset(ltmbuf, 0, sizeof(ltmbuf));
                ...
                SHR_IF_ERR_EXIT
                    (bcmtm_field_list_set(unit, output_fields, field_id, 0, field_data));
                break;
            case TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_RESUME_LIMIT_CELLSf:
                /* Red HW. */
                ptid = MMU_THDO_MC_SHARED_CQE_POOL_YELLOW_SHARED_LIMITr;
                fid = YELLOW_SHARED_LIMITf;
                sal_memset(ltmbuf, 0, sizeof(ltmbuf));
...
static int
tm_thd_mc_egr_service_pool_delete(int unit, bcmltd_sid_t sid,
                                  const bcmltd_op_arg_t *op_arg, int itm, int pool)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_thd_mc_egr_service_pool_update(unit, sid, op_arg, itm, pool, data_fields,
                                        &dummy_fields));
...
static int
tm_thd_mc_egr_service_pool_stage(int unit,
                              bcmltd_sid_t sid,
                              const bcmltd_op_arg_t *op_arg,
                              ...
                              bcmltd_fields_t *output_fields)
{
    ...
    while (key_ptr) {
        switch (key_ptr->id) {
            case TM_THD_MC_EGR_SERVICE_POOLt_BUFFER_POOLf:
                itm = key_ptr->data;
                break;
            case TM_THD_MC_EGR_SERVICE_POOLt_TM_EGR_SERVICE_POOL_IDf:
                pool = key_ptr->data;
                break;
            default:
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT
                (tm_thd_mc_egr_service_pool_update(unit, sid, op_arg, itm, pool,
                                                data_fields, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_thd_mc_egr_service_pool_delete(unit, sid, op_arg, itm, pool));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
static int
tm_thd_mc_egr_service_pool_lookup(int unit,
                                  bcmltd_sid_t sid,
                                  const bcmltd_op_arg_t *op_arg,
                                  void *context,
                                  ...
                                  bcmltd_fields_t *out)
{
    ...
    fid = TM_THD_MC_EGR_SERVICE_POOLt_BUFFER_POOLf;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_imm_fields_get(unit, key, fid, &idx));
    itm = idx;
    fid = TM_THD_MC_EGR_SERVICE_POOLt_TM_EGR_SERVICE_POOL_IDf;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_imm_fields_get(unit, key, fid, &idx));
    pool = idx;
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_EGR_SERVICE_POOLt_SHARED_LIMIT_CELLSf,
                                fval));
    ...
    fval = fval * TH4_GRANULARITY;
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_EGR_SERVICE_POOLt_SHARED_RESUME_LIMIT_CELLSf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_LIMIT_CELLSf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_EGR_SERVICE_POOLt_YELLOW_SHARED_RESUME_LIMIT_CELLSf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_LIMIT_CELLSf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_EGR_SERVICE_POOLt_RED_SHARED_RESUME_LIMIT_CELLSf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_THD_MC_EGR_SERVICE_POOLt_COLOR_SPECIFIC_LIMITSf,
...
int
bcm56990_a0_tm_thd_mc_egr_service_pool_register(int unit)
{
    ...
    tm_thd_cb_handler.op_stage = tm_thd_mc_egr_service_pool_stage;
    tm_thd_cb_handler.op_lookup = tm_thd_mc_egr_service_pool_lookup;

    rv = bcmlrd_map_get(unit, TM_THD_MC_EGR_SERVICE_POOLt, &map);
    ...
    SHR_IF_ERR_EXIT
        (bcmimm_lt_event_reg(unit, TM_THD_MC_EGR_SERVICE_POOLt,
...
int
bcm56990_a0_tm_thd_mc_egr_service_pool_populate(int unit)
{
    int itm, pool, rv;
    const bcmlrd_map_t *map = NULL;
    SHR_FUNC_ENTER(unit);
    ...
    for (itm = 0; itm < SOC_MMU_CFG_ITM_MAX; itm++) {
        ...
        for (pool = 0; pool < SOC_MMU_CFG_SERVICE_POOL_MAX; pool++) {
            SHR_IF_ERR_EXIT
                (tm_thd_mc_egr_service_pool_populate_entry(unit, itm, pool));
        }
    }
