/*! \file bcm56990_a0_tm_port_mc_q_to_service_pool.c
 *
                                            ...
                                            int up, soc_mmu_cfg_buf_t *buf)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
                              TM_PORT_MC_Q_TO_SERVICE_POOLt_PORT_IDf,
                              0, lport));
    SHR_IF_ERR_EXIT
        (bcmtm_field_list_set(unit, &entry,
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        num_q = bcm56990_a0_tm_num_mc_q_non_cpu_port_get(unit);
    } else {
        num_q = SOC_TH4_NUM_CPU_QUEUES;
    }
    if (queue >= num_q) {
...
static int
tm_port_mc_q_to_service_pool_delete(int unit,
                                    bcmltd_sid_t sid,
                                    const bcmltd_op_arg_t *op_arg,
                                    int lport,
                                    int queue)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_port_mc_q_to_service_pool_update(unit, sid, op_arg, lport,
                                             queue, data_fields,
                                             &dummy_fields));
                                   ...
                                   bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT
                (tm_port_mc_q_to_service_pool_update(unit, sid, op_arg, lport, queue,
                                                     data_fields, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_port_mc_q_to_service_pool_delete(unit, sid, op_arg, lport, queue));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
static int
tm_port_mc_q_to_service_pool_lookup(int unit,
                                    bcmltd_sid_t sid,
                                    const bcmltd_op_arg_t *op_arg,
                                    void *context,
                                    ...
                                    bcmltd_fields_t *out)
{
    ...
    fid = TM_PORT_MC_Q_TO_SERVICE_POOLt_PORT_IDf;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_imm_fields_get(unit, key, fid, &idx));
    lport = idx;
    fid = TM_PORT_MC_Q_TO_SERVICE_POOLt_TM_MC_Q_IDf;
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmcth_imm_fields_get(unit, key, fid, &idx));
    queue = idx;
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_PORT_MC_Q_TO_SERVICE_POOLt_TM_EGR_SERVICE_POOL_IDf,
                                fval));
    ...
    SHR_IF_ERR_EXIT(
        bcmcth_imm_field_update(unit, out,
                                TM_PORT_MC_Q_TO_SERVICE_POOLt_USE_QGROUP_MINf,
                                fval));
...
bcm56990_a0_tm_port_mc_q_to_service_pool_register(int unit)
{
    ...
    tm_thd_cb_handler.op_stage = tm_port_mc_q_to_service_pool_stage;
    tm_thd_cb_handler.op_lookup = tm_port_mc_q_to_service_pool_lookup;
    ...
    SHR_IF_ERR_EXIT
        (bcmimm_lt_event_reg(unit, TM_PORT_MC_Q_TO_SERVICE_POOLt,
...
bcm56990_a0_tm_port_mc_q_to_service_pool_populate(int unit)
{
    ...
    for (lport = 0; lport < SOC_MMU_CFG_PORT_MAX; lport++) {
        th4_mmu_config_thdo_set(unit, lport, buf, &devcfg, drv_info->lossless);
        if (bcmtm_lport_is_cpu(unit, lport)) {
            max_num_mcq = SOC_TH4_NUM_CPU_QUEUES;
        } else {
            max_num_mcq = TH4_MAX_NUM_MC_Q;
        }
...
bcm56990_a0_tm_port_mc_q_to_service_pool_update(int unit, int pport, int up)
{
    ...
    for (queue = 0; queue < max_num_mcq; queue++) {
        ...
        if (SHR_SUCCESS(bcmimm_entry_lookup(unit, sid, &keys, &lkup_fields))) {
            ...
            SHR_IF_ERR_EXIT
                (tm_port_mc_q_to_service_pool_update(unit, sid, NULL, lport, queue,
