/*! \file bcm56990_a0_tm_thd_uc_q.c
 *
                           ...
                           soc_mmu_cfg_buf_queue_t *buf_queue)
{
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        num_q = bcm56990_a0_tm_num_uc_q_non_cpu_port_get(unit);
    } else {
        num_q = 0;
    }
    if (queue >= num_q) {
        ...
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
                ...
                buf_queue->pool_resume * TH4_GRANULARITY));
        SHR_IF_ERR_EXIT
            (bcmtm_field_list_set(unit, &entry,
                   ...
                   bcmltd_fields_t *output_fields, uint8_t action)
{
    ...
    th4_mmu_config_buf_queue_get(unit, lport, &buf_queue);
    ...
    while (data_ptr) {
        ...
        switch (data_ptr->id) {
            case TM_THD_UC_Qt_MIN_GUARANTEE_CELLSf:
                ...
                if (SHR_SUCCESS
                        (bcm56990_a0_tm_shared_buffer_update(unit, 0, pool, delta,
                                                             RESERVED_BUFFER_UPDATE,
                                                             TRUE)) &&
                    SHR_SUCCESS
                        (bcm56990_a0_tm_shared_buffer_update(unit, 1, pool, delta,
                                                             RESERVED_BUFFER_UPDATE,
                                                             TRUE))) {
                ...
                if (use_dynamic_color != fval) {
                    ...
                    if (use_dynamic_color) {
                        ...
                        if (SHR_SUCCESS
                            ...
                            limit = CEILING(limit, TH4_GRANULARITY);
                            SHR_IF_ERR_EXIT
                                (bcmtm_field_set(unit, ptid, LIMIT_YELLOWf,
                                                 q_config_buf, &limit));
                        }
...
bcm56990_a0_tm_thd_uc_q_populate(int unit)
{
    ...
    for (lport = 1; lport < SOC_MMU_CFG_PORT_MAX; lport++) {
        th4_mmu_config_buf_queue_get(unit, lport, &buf_queue);
        for (queue = 0; queue < TH4_MAX_NUM_UC_Q; queue++) {
            SHR_IF_ERR_EXIT
...
bcm56990_a0_tm_thd_uc_q_update(int unit, int pport, int up)
{
    ...
    if (!bcmtm_lport_is_cpu(unit, lport)) {
        ...
        for (queue = 0; queue < bcm56990_a0_tm_num_uc_q_non_cpu_port_get(unit);
                queue++) {
            SHR_IF_ERR_EXIT
