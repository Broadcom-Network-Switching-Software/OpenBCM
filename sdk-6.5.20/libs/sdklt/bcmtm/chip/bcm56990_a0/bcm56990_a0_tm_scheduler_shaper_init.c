/*! \file bcm56990_a0_tm_scheduler_shaper_init.c
 *
 ...
 */
static int
bcm56990_a0_tm_shaper_config_init(int unit)
{
 ...
 */
static int
bcm56990_a0_tm_scheduler_config_init(int unit)
{
    ...
    for (idx = 0; idx < sizeof(sid)/sizeof(bcmdrd_sid_t); idx++) {
        ...
        for (i = 0; i < tbl_inst_num; i++) {
            if (SHR_SUCCESS(bcm56990_a0_tm_check_valid_itm(unit, i))) {
...
bcm56990_a0_tm_scheduler_shaper_chip_init(int unit, bool warm)
{
    ...
    if (!warm) {
        /* Scheduler config. */
        SHR_IF_ERR_EXIT
            (bcm56990_a0_tm_scheduler_config_init(unit));

        /* Shaper config */
        SHR_IF_ERR_EXIT
            (bcm56990_a0_tm_shaper_config_init(unit));
    }
    SHR_IF_ERR_EXIT
        (bcm56990_a0_tm_scheduler_profile_init(unit, -1, NULL));
exit:
    SHR_FUNC_EXIT();
}
