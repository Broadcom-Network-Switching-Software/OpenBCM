/*! \file bcm56990_a0_tm_scheduler.c
 *
 * Scheduler related api and function for bcm56990_a0.
 */
/*
 ...
 * Local definitions
 */
static bcm56990_a0_tm_cos_l0_map_t
bcm56990_a0_tm_cos_l0_map[BCMDRD_CONFIG_MAX_UNITS][TH4_NUM_SCHED_PROFILE];
#define BSL_LOG_MODULE  BSL_LS_BCMTM_CHIP
/*******************************************************************************
 * Private functions
 */
static int
bcm56990_a0_tm_cos_l0_map_set(int unit,
                              int profile_id,
                              ...
                              int *opcode)
{
    ...
    bcm56990_a0_tm_cos_l0_map_t *l0_map =
            &(bcm56990_a0_tm_cos_l0_map[unit][profile_id]);
     ...
     * as per MMU_Q_MODE.
     */
    max_num_mcq = bcm56990_a0_tm_num_mc_q_non_cpu_port_get(unit);
    max_num_ucq = bcm56990_a0_tm_num_uc_q_non_cpu_port_get(unit);
    for (idx = 0; idx < TH4_TM_NUM_Q; idx++) {
...
static int
bcm56990_a0_tm_mmuq_l1_map_get(int unit,
                               uint8_t profile_id,
                               ...
                               int *l1_node)
{
    ...
    if (sched_level == L1_SCHED_MULTICAST_QUEUE) {
        mcq_offset = bcm56990_a0_tm_num_uc_q_non_cpu_port_get(unit);
        mmu_q_id += mcq_offset;
    }
 ...
 */
static int
bcm56990_a0_tm_scheduler_node_profile_validate(int unit,
                                               int node_id,
                                               bcmtm_scheduler_profile_t *profile)
{
 ...
 */
static int
bcm56990_a0_tm_scheduler_profile_validate(int unit,
                                          uint8_t profile_id,
                                          int node_id,
                                          bcmtm_scheduler_profile_t *profile)
{
    ...
    int num_ucq, num_mcq, total_q;
    uint8_t l1_map[TH4_TM_NUM_Q]= {0};
    bcm56990_a0_tm_cos_l0_map_t *l0_map;

    SHR_FUNC_ENTER(unit);

    if (SHR_E_NONE !=
            bcm56990_a0_tm_cos_l0_map_set(unit, profile_id, profile, &opcode)) {
        goto opstatus_update;
    }

    l0_map = &(bcm56990_a0_tm_cos_l0_map[unit][profile_id]);
    for (idx = 0; idx < TH4_TM_NUM_Q; idx++) {
        num_ucq = profile[idx].num_ucq;
        num_mcq = profile[idx].num_mcq;
        total_q = num_mcq + num_ucq;
 ...
 */
static int
bcm56990_a0_tm_scheduler_node_set(int unit,
        bcmtm_lport_t lport,
        ...
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (sched_level == L0_SCHED_COS) {/* L0 Level */
        fid = SP_L0_CHILDf;
        SHR_IF_ERR_EXIT
            (bcm56990_a0_tm_sched_node_l0_map_get(unit, profile_id,
                                                  sched_node_id, &node_id));
    } else {/* L1 level */
        fid = SP_L0_PARENTf;
        SHR_IF_ERR_EXIT
            (bcm56990_a0_tm_mmuq_l1_map_get(unit, profile_id, sched_level,
                                           sched_node_id, &node_id));
    }
    ...
    if (bcmtm_lport_is_mgmt(unit, lport)) {
        index = TH4_TM_MGMT_Q_OFFSET + node_id;
    } else if (bcmtm_lport_is_lb(unit, lport)) {
        index = TH4_TM_LB_Q_OFFSET + node_id;
    } else {
        index = mport * TH4_TM_NUM_Q + node_id;
    }
 ...
 */
static void
bcm56990_a0_tm_l1_to_l0_mapping(int unit,
                                uint32_t profile_id,
                                bcmtm_scheduler_profile_t *profile,
                                uint32_t *bmap)
{
    int idx, l0_idx, num_ucq, num_mcq, total_q;
    int l1_map[TH4_TM_NUM_Q];
    bcm56990_a0_tm_cos_l0_map_t *l0_map;

    l0_map = &(bcm56990_a0_tm_cos_l0_map[unit][profile_id]);

    sal_memset(l1_map, -1, sizeof(int) * TH4_TM_NUM_Q);
 ...
 */
static int
bcm56990_a0_tm_scheduler_port_set(int unit,
        bcmltd_sid_t ltid,
        bcmtm_lport_t lport,
        bcmtm_scheduler_profile_t *profile,
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (profile) {
        bcm56990_a0_tm_l1_to_l0_mapping(unit, profile_id, profile, &bmap);
    }
    SHR_IF_ERR_EXIT
...
static int
bcm56990_a0_tm_scheduler_profile_reset(int unit,
                                       bcmltd_sid_t ltid,
                                       uint8_t profile_id)
{
 ...
 */
static int
bcm56990_a0_tm_scheduler_profile_set(int unit,
                                     bcmltd_sid_t ltid,
                                     uint8_t profile_id,
                                     bcmtm_scheduler_profile_t *profile)
{
    ...
    uint32_t mmu_q_map[TH4_TM_NUM_Q] = {0};
    uint32_t fval, ltmbuf[BCMTM_MAX_ENTRY_WSIZE] = {0};
    bcmtm_pt_info_t pt_dyn_info = {0};
    ...
    if (!profile) {
        SHR_IF_ERR_EXIT
            (bcm56990_a0_tm_scheduler_profile_reset(unit, ltid, profile_id));
        SHR_EXIT();
    }
    for (idx = 0; idx < TH4_TM_NUM_Q; idx++) {
        num_ucq = profile[idx].num_ucq;
 ...
 */
static int
bcm56990_a0_tm_scheduler_cpu_node_set(int unit,
        uint32_t sched_node_id,
        uint32_t sched_level,
        bcmltd_sid_t ltid,
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if ((sched_level == L0_SCHED_COS) && (sched_node_id >= TH4_NUM_L0_NODE)) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
    ...
    fid = WEIGHTf;
    fval = scheduler_cfg->weight;
    index = TH4_TM_CPU_Q_OFFSET + sched_node_id;
 ...
 */
void
bcm56990_a0_tm_sched_node_mmu_q_get(int unit,
                                    int sched_node,
                                    bcmtm_scheduler_profile_t *profile,
                                    int *mmu_q_num)
{
...
int
bcm56990_a0_tm_mmu_q_sched_node_id_get(int unit,
                                       int mmu_q_id,
                                       bcmtm_scheduler_profile_t *profile)
{
...
int
bcm56990_a0_tm_sched_node_l0_map_get(int unit,
                                     uint8_t profile_id,
                                     uint32_t sched_node,
                                     int *l0_node)
{
...
int
bcm56990_a0_tm_scheduler_profile_init(int unit,
                                      uint8_t profile_id,
                                      bcmtm_scheduler_profile_t *profile)
{
    int idx, opcode;
    ...
    if (!profile) {
        for (idx = 0; idx < TH4_NUM_SCHED_PROFILE; idx++) {
            l0_map = &(bcm56990_a0_tm_cos_l0_map[unit][idx]);
            sal_memset(l0_map->map, -1, sizeof(bcm56990_a0_tm_cos_l0_map_t));
        }
    } else {
        l0_map = &(bcm56990_a0_tm_cos_l0_map[unit][profile_id]);
        SHR_IF_ERR_EXIT
            (bcm56990_a0_tm_cos_l0_map_set(unit, profile_id, profile,
                                           &opcode));
    }
exit:
    SHR_FUNC_EXIT();
...
int
bcm56990_a0_tm_scheduler_drv_get(int unit, void *sched_drv)
{
    bcmtm_scheduler_drv_t bcm56990_a0_sched_drv = {
        .node_set = bcm56990_a0_tm_scheduler_node_set,
        .port_set = bcm56990_a0_tm_scheduler_port_set,
        .node_profile_validate = bcm56990_a0_tm_scheduler_node_profile_validate,
        .profile_validate = bcm56990_a0_tm_scheduler_profile_validate,
        .cpu_node_set = bcm56990_a0_tm_scheduler_cpu_node_set,
        .profile_set = bcm56990_a0_tm_scheduler_profile_set,
        .profile_init = bcm56990_a0_tm_scheduler_profile_init
    };

    SHR_FUNC_ENTER(unit);
    SHR_NULL_CHECK(sched_drv, SHR_E_INTERNAL);
    *((bcmtm_scheduler_drv_t *)sched_drv) = bcm56990_a0_sched_drv;
exit:
    SHR_FUNC_EXIT();
}
