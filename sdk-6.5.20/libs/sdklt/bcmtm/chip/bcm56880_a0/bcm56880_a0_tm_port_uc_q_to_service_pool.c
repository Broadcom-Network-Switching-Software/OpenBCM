/*! \file bcm56880_a0_tm_port_uc_q_to_service_pool.c
 *
                                    ...
                                    bcmltd_fields_t *output_fields)
{
    ...
    uint32_t fval;
    int pport, num_q, midx, base, mmu_queue;
    ...
    while (data_ptr) {
        ...
        switch (data_ptr->id) {
            case TM_PORT_UC_Q_TO_SERVICE_POOLt_TM_EGR_SERVICE_POOL_IDf:
                ...
                SHR_IF_ERR_EXIT
                    (bcmtm_pt_indexed_write(unit, ptid, sid, &pt_dyn_info, ltmbuf));
                ptid = MMU_THDO_Q_TO_QGRP_MAPD2m;
                SHR_IF_ERR_EXIT
                    (bcmtm_pt_indexed_write(unit, ptid, sid, &pt_dyn_info, ltmbuf));
...
static int
tm_port_uc_q_to_service_pool_delete(int unit, bcmltd_sid_t sid, int lport, int queue)
{
    ...
    SHR_IF_ERR_EXIT
        (tm_port_uc_q_to_service_pool_update(unit, sid, lport, queue, data_fields,
                                             &dummy_fields));
                          ...
                          bcmltd_fields_t *output_fields)
{
    ...
    switch (event_reason) {
        ...
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_EXIT
                (tm_port_uc_q_to_service_pool_update(unit, sid, lport, queue,
                                                     data_fields, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (tm_port_uc_q_to_service_pool_delete(unit, sid, lport, queue));
            break;
        default:
            SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
int
bcm56880_a0_tm_port_uc_q_to_service_pool_register(int unit)
{
    ...
    tm_thd_cb_handler.stage    = tm_port_uc_q_to_service_pool_stage;
...
int
bcm56880_a0_tm_port_uc_q_to_service_pool_populate(int unit)
{
    int lport, queue, rv;
    const bcmlrd_map_t *map = NULL;
...
int
bcm56880_a0_tm_port_uc_q_to_service_pool_update(int unit, int pport, int up)
{
    int lport, queue, rv;
    const bcmlrd_map_t *map = NULL;
