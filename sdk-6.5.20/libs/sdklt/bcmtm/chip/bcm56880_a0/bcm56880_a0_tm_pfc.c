 ...
 * Local definitions
 */
#define TD4_PFC_PRIORITY_MAX_INDEX         MAX_NUM_PFC_PRI - 1
#define TD4_PFC_INDEX_INVALID              -1
#define TD4_PFC_RX_PROFILE_NUM_ENTRIES     8
#define TD4_PFC_COS_NUM                    MAX_NUM_COS
 ...
 */
static int
td4_tm_pfc_rx_mapping_hw_write(int unit,
                               int profile_id,
                               tm_pfc_pri_to_cos_map_config_t config_array[])
{
    ...
    for (pri = 0; pri < MAX_NUM_PFC_PRI; pri++) {
        ...
        reg_index = profile_id + pri * TD4_PFC_RX_PROFILE_NUM_ENTRIES;
        SHR_BIT_ITER(cos_bmp, TD4_PFC_COS_NUM, cos) {
            bcm56880_a0_tm_sched_node_mmu_q_get(unit,
                                                cos,
                                                profile, mmu_qs);
            if (mmu_qs[0] != -1) {
                                      ...
                                      tm_pfc_pri_to_cos_map_config_t map[])
{
    ...
    SHR_IF_ERR_EXIT
        (td4_tm_pfc_rx_mapping_hw_write(unit, profile_id, map));
exit:
    SHR_FUNC_EXIT();
}
