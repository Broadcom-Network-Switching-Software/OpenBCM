                        ...
                        soc_mmu_device_info_t *devcfg,  int lossless)
{
    ...
    default_mtu_cells = TD4_MMU_CFG_MMU_BYTES_TO_CELLS(devcfg->default_mtu_size +
                                                       devcfg->mmu_hdr_byte,
                        ...
                        soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    buf_port = &buf->ports[port];
    if (bcmtm_lport_is_cpu(unit, port)) {
        num_cosq = SOC_TD4_NUM_CPU_QUEUES;
    } else {
                                   ...
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm ++) {
       for (idx = 0; idx < TD4_MMU_NUM_POOL; idx++) {
            ...
            if (idx == 0) {
                /* 100% scale up by 100 */
                buf_pool->total_mcq_entry = SOC_TD4_MMU_MCQ_ENTRY_PER_ITM;
                if ((flags & CHIP_FLAG_BW8T) == CHIP_FLAG_BW8T) {
                    /* BCM56883 */
    ...
    for (itm = 0; itm < TD4_ITMS_PER_DEV; itm ++) {
         ...
         */
        for (pipe = 0; pipe < TD4_PIPES_PER_DEV; pipe++) {
            for (pm = pipe * TD4_PBLKS_PER_PIPE; pm < (pipe + 1) * TD4_PBLKS_PER_PIPE; pm++) {
                ports_in_pm[pm] = drv_info->flex_eligible[pm];
                ports_in_pipe[pipe] += ports_in_pm[pm];
            }
            /* Each pipe cannot have more than 18 ports. */
        ...
        total_qmin_itm[itm] = default_mtu_cells * SOC_TD4_NUM_CPU_QUEUES;
    }
                                   ...
                                   soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    jumbo_frame_cells = TD4_MMU_CFG_MMU_BYTES_TO_CELLS(devcfg->jumbo_pkt_size +
                                                       devcfg->mmu_hdr_byte,
                                                       devcfg->mmu_cell_size);
...
bcm56880_a0_tm_rqe_threshold_init(int unit) {
    ...
    SHR_FUNC_ENTER(unit);
    for(idx = 0; idx < TD4_MMU_RQE_QUEUE_NUM; idx++) {
