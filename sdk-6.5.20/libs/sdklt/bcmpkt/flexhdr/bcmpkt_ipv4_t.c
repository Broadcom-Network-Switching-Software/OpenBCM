...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    extern const bcmpkt_ipv4_t_fget_t _bd##_vu##_va##_ipv4_t_fget;
#include <bcmlrd/chip/bcmlrd_variant.h>

#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    &_bd##_vu##_va##_ipv4_t_fget,
static const bcmpkt_ipv4_t_fget_t *ipv4_t_fget[] = {
    NULL,
#include <bcmlrd/chip/bcmlrd_variant.h>
    NULL
};
...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    extern const bcmpkt_ipv4_t_fset_t _bd##_vu##_va##_ipv4_t_fset;
#include <bcmlrd/chip/bcmlrd_variant.h>

#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    &_bd##_vu##_va##_ipv4_t_fset,
static const bcmpkt_ipv4_t_fset_t *ipv4_t_fset[] = {
    NULL,
#include <bcmlrd/chip/bcmlrd_variant.h>
    NULL
};
...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    extern const bcmpkt_ipv4_t_figet_t _bd##_vu##_va##_ipv4_t_figet;
#include <bcmlrd/chip/bcmlrd_variant.h>

#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    &_bd##_vu##_va##_ipv4_t_figet,
static const bcmpkt_ipv4_t_figet_t *ipv4_t_figet[] = {
    NULL,
#include <bcmlrd/chip/bcmlrd_variant.h>
    NULL
};
...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    &_bd##_vu##_va##_ipv4_t_view_info_get,
...
{
    BCMPKT_IPV4_T_FIELD_NAME_MAP_INIT
};
...
bcmpkt_ipv4_t_is_supported(bcmlrd_variant_t variant, bool *is_supported)
{
    ...
    if (ipv4_t_figet[variant] == NULL ||
        ipv4_t_figet[variant]->fget[BCMPKT_IPV4_T_I_SUPPORT] == NULL) {
        *is_supported = false;
    } else {
        *is_supported = ipv4_t_figet[variant]->fget[BCMPKT_IPV4_T_I_SUPPORT](NULL, NULL);
    }
exit:
    SHR_FUNC_EXIT();
}
...
{
    SHR_FUNC_ENTER(BSL_UNIT_UNKNOWN);
    SHR_NULL_CHECK(ipv4_t, SHR_E_PARAM);
    ...
    if (fid <= BCMPKT_IPV4_T_FID_INVALID || fid >= BCMPKT_IPV4_T_FID_COUNT) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (ipv4_t_fget[variant] == NULL ||
        ipv4_t_fget[variant]->fget[fid] == NULL) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }

    *val = ipv4_t_fget[variant]->fget[fid](ipv4_t);
                                     ...
                                     int fid, uint32_t val)
{
    ...
    if (ipv4_t_fset[variant] == NULL ||
        ipv4_t_fset[variant]->fset[fid] == NULL) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }

    ipv4_t_fset[variant]->fset[fid](ipv4_t, val);
...
bcmpkt_ipv4_t_field_name_get(int fid, char **name)
{
    ...
    if (fid <= BCMPKT_IPV4_T_FID_INVALID ||
        fid >= BCMPKT_IPV4_T_FID_COUNT) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
bcmpkt_ipv4_t_field_id_get(char *name, int *fid)
{
    ...
    for (i = BCMPKT_IPV4_T_FID_INVALID + 1; i < BCMPKT_IPV4_T_FID_COUNT; i++) {
                ...
                uint32_t flags, shr_pb_t *pb)
{
    ...
    for (i = BCMPKT_IPV4_T_FID_INVALID + 1; i < BCMPKT_IPV4_T_FID_COUNT; i++) {
        if (view_info.view_infos[i] == -1) {
            val = ipv4_t_fget[variant]->fget[i](ipv4_t);
            if ((val != 0) || (flags == BCMPKT_IPV4_T_DUMP_F_ALL)) {
    ...
    if (view_info.view_type_get) {
        view_enc = view_info.view_type_get(ipv4_t);
        for (i = BCMPKT_IPV4_T_FID_INVALID + 1; i < BCMPKT_IPV4_T_FID_COUNT; i++) {
