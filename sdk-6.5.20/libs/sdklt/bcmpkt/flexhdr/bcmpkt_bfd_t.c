...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    extern const bcmpkt_bfd_t_fget_t _bd##_vu##_va##_bfd_t_fget;
#include <bcmlrd/chip/bcmlrd_variant.h>

#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    &_bd##_vu##_va##_bfd_t_fget,
static const bcmpkt_bfd_t_fget_t *bfd_t_fget[] = {
    NULL,
#include <bcmlrd/chip/bcmlrd_variant.h>
    NULL
};
...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    extern const bcmpkt_bfd_t_fset_t _bd##_vu##_va##_bfd_t_fset;
#include <bcmlrd/chip/bcmlrd_variant.h>

#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    &_bd##_vu##_va##_bfd_t_fset,
static const bcmpkt_bfd_t_fset_t *bfd_t_fset[] = {
    NULL,
#include <bcmlrd/chip/bcmlrd_variant.h>
    NULL
};
...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    extern const bcmpkt_bfd_t_figet_t _bd##_vu##_va##_bfd_t_figet;
#include <bcmlrd/chip/bcmlrd_variant.h>

#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    &_bd##_vu##_va##_bfd_t_figet,
static const bcmpkt_bfd_t_figet_t *bfd_t_figet[] = {
    NULL,
#include <bcmlrd/chip/bcmlrd_variant.h>
    NULL
};
...
#define BCMLRD_VARIANT_ENTRY(_bd,_bu,_va,_ve,_vu,_vv,_vo,_vd,_r0,_r1) \
    &_bd##_vu##_va##_bfd_t_view_info_get,
...
{
    BCMPKT_BFD_T_FIELD_NAME_MAP_INIT
};
...
bcmpkt_bfd_t_is_supported(bcmlrd_variant_t variant, bool *is_supported)
{
    ...
    if (bfd_t_figet[variant] == NULL ||
        bfd_t_figet[variant]->fget[BCMPKT_BFD_T_I_SUPPORT] == NULL) {
        *is_supported = false;
    } else {
        *is_supported = bfd_t_figet[variant]->fget[BCMPKT_BFD_T_I_SUPPORT](NULL, NULL);
    }
exit:
    SHR_FUNC_EXIT();
}
...
{
    SHR_FUNC_ENTER(BSL_UNIT_UNKNOWN);
    SHR_NULL_CHECK(bfd_t, SHR_E_PARAM);
    ...
    if (fid <= BCMPKT_BFD_T_FID_INVALID || fid >= BCMPKT_BFD_T_FID_COUNT) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }

    if (bfd_t_fget[variant] == NULL ||
        bfd_t_fget[variant]->fget[fid] == NULL) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }

    *val = bfd_t_fget[variant]->fget[fid](bfd_t);
                                     ...
                                     int fid, uint32_t val)
{
    ...
    if (bfd_t_fset[variant] == NULL ||
        bfd_t_fset[variant]->fset[fid] == NULL) {
        SHR_ERR_EXIT(SHR_E_UNAVAIL);
    }

    bfd_t_fset[variant]->fset[fid](bfd_t, val);
...
bcmpkt_bfd_t_field_name_get(int fid, char **name)
{
    ...
    if (fid <= BCMPKT_BFD_T_FID_INVALID ||
        fid >= BCMPKT_BFD_T_FID_COUNT) {
        SHR_ERR_EXIT(SHR_E_PARAM);
    }
...
bcmpkt_bfd_t_field_id_get(char *name, int *fid)
{
    ...
    for (i = BCMPKT_BFD_T_FID_INVALID + 1; i < BCMPKT_BFD_T_FID_COUNT; i++) {
                ...
                uint32_t flags, shr_pb_t *pb)
{
    ...
    for (i = BCMPKT_BFD_T_FID_INVALID + 1; i < BCMPKT_BFD_T_FID_COUNT; i++) {
        if (view_info.view_infos[i] == -1) {
            val = bfd_t_fget[variant]->fget[i](bfd_t);
            if ((val != 0) || (flags == BCMPKT_BFD_T_DUMP_F_ALL)) {
    ...
    if (view_info.view_type_get) {
        view_enc = view_info.view_type_get(bfd_t);
        for (i = BCMPKT_BFD_T_FID_INVALID + 1; i < BCMPKT_BFD_T_FID_COUNT; i++) {
