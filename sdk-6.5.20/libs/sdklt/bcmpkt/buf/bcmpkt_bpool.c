...
/* 0: shared pool, 1: unit 0 pool, 2: unit 1 pool, etc. */
#define PIDX(_u)                    (_u + 1)

/* Get unit number from pool index. Unit -1 is for the shared pool. */
#define PUNIT(_i)                   (_i - 1)
...
bcmpkt_bpool_cleanup(void)
{
    ...
    for (idx = 0; idx < COUNTOF(bps); idx++) {
        if (bps[idx].mem) {
            unit = PUNIT(idx);
            SHR_IF_ERR_CONT
                (bcmpkt_bpool_destroy(unit));
        }
    }
