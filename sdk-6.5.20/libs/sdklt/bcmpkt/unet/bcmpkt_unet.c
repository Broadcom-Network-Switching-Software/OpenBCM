...
header_generate(int unit, bcmpkt_packet_t *packet)
{
    ...
    if ((hg3_support == true) && (packet->pmd.higig != NULL) &&
        ...
        SHR_IF_ERR_EXIT
            (bcmpkt_hg3_base_t_field_id_get("RESERVED_ETYPE", &fid));
        SHR_IF_ERR_EXIT
    ...
    if ((packet->pmd.txpmd != NULL) && (packet->pmd.txpmd[0] != 0)) {
        ...
        if ((packet->pmd.lbhdr != NULL) && (packet->pmd.lbhdr[0] != 0)) {
            if (glb_support == true) {
                ...
                SHR_IF_ERR_EXIT
                    (bcmpkt_generic_loopback_t_field_id_get("START_BYTE",
                                                            &fid));
                SHR_IF_ERR_EXIT
                    (bcmpkt_generic_loopback_t_field_get(variant,
                                            packet->pmd.lbhdr, fid, &lbstart));
            } else {
                SHR_IF_ERR_EXIT
