 ...
 *
 * Per-LT state extensions are selected from
 * these types
 *
 * In each bitmap state, the bit position is the Tracking Index
 * for the entry.
 * 1) Local in-use entries
 * 2) Current valid entries
 * 3) Global
 * in-use entries for shared tables.
 ...
 *
 * Some Logical Tables require state beyond
 * the basic elements for all LTs.
 * This structure links the different categories
 * of the LT state for cases where each is needed.
 * All state is stored in HA memory,
 * so HA pointers are used to reference the structure links.
 ...
 *
 * This structure assembles the runtime state of
 * a single LT.  For deployments that enable HA, this state will be updated at each
 ...
 *
 * This structure caches the information
 * that must be preserved over the duration of a transaction.  Note that only an Atomic
 * Transaction will have multiple LT entries.  The transaction ID must be identical
 * for each of the LT entries in a transaction.
