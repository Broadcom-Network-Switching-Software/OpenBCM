...
err_code_t merlin7_pcieg3_display_eye_scan(srds_access_t *sa__) {
    ...
    uint16_t   status = 0;
    int16_t     y,y_max,y_step,core,lane;

    y_max  = EYE_SCAN_NRZ_VERTICAL_IDX_MAX;
    y_step = EYE_SCAN_NRZ_VERTICAL_STEP;
    ESTM(core=merlin7_pcieg3_get_core(sa__));
    ESTM(lane=merlin7_pcieg3_get_lane(sa__));
    EFUN_PRINTF(("\n\n****  SERDES EYE SCAN CORE %d LANE %d   ****\n",core,lane));
    EFUN(merlin7_pcieg3_display_eye_scan_header(sa__,1));

    /* start horizontal acquisition */
    {   err_code_t err_code = merlin7_pcieg3_meas_eye_scan_start(sa__, EYE_SCAN_HORIZ);
        if (err_code) {
            if (err_code == ERR_CODE_DIAG_SCAN_NO_PMD_LOCK) {
                return(ERR_CODE_NONE);
            }
            else {
                EFUN((merlin7_pcieg3_meas_eye_scan_done(sa__), err_code));
            }
        }
    }
    ...
    {
        {   err_code_t err_code = merlin7_pcieg3_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (err_code) {
                EFUN((merlin7_pcieg3_meas_eye_scan_done(sa__), err_code));
            }
        }
        EFUN(merlin7_pcieg3_display_eye_scan_stripe(sa__, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(merlin7_pcieg3_meas_eye_scan_done(sa__));
    EFUN(merlin7_pcieg3_display_eye_scan_footer(sa__,1));
...
err_code_t merlin7_pcieg3_INTERNAL_poll_diag_data(srds_access_t *sa__, const srds_info_t *merlin7_pcieg3_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
...
static err_code_t merlin7_pcieg3_read_eye_scan_stripe_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    ...
    UNUSED(sa__);
    *(cast_arg->buffer_ptr++) = merlin7_pcieg3_INTERNAL_float8_to_int32((float8_t)(data & 0xFF));
    if (byte_count > 1) {
        *(cast_arg->buffer_ptr++) = merlin7_pcieg3_INTERNAL_float8_to_int32((float8_t)(data >> 8));
    }
    return (ERR_CODE_NONE);
}
...
err_code_t merlin7_pcieg3_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    const uint8_t lane = merlin7_pcieg3_get_lane(sa__);
    ...
    EFUN(merlin7_pcieg3_INTERNAL_verify_merlin7_pcieg3_info(sa__, merlin7_pcieg3_info_ptr));
        ...
        lane_diag_base = merlin7_pcieg3_info_ptr->diag_mem_ram_base + ((lane%merlin7_pcieg3_info_ptr->lane_count) * merlin7_pcieg3_info_ptr->diag_mem_ram_size) +
            merlin7_pcieg3_info_ptr->grp_ram_size*merlin7_pcieg3_INTERNAL_grp_idx_from_lane(merlin7_pcieg3_get_physical_lane(sa__));

    EFUN(merlin7_pcieg3_INTERNAL_poll_diag_data(sa__, merlin7_pcieg3_info_ptr, status, &diag_rd_ptr, stripe_size, 400));
    ...
    diag_rd_ptr = ((uint32_t)diag_rd_ptr + stripe_size) % merlin7_pcieg3_info_ptr->diag_mem_ram_size;
    EFUN(wrv_usr_diag_rd_ptr(diag_rd_ptr));
    ESTM(*status = rdv_usr_diag_status() & 0xFF);
    return(ERR_CODE_NONE);
}

err_code_t merlin7_pcieg3_display_eye_scan_stripe(srds_access_t *sa__, int16_t y,uint32_t *buffer) {

    const uint32_t limits[7] = {917504, 91750, 9175, 917, 91, 9, 1};

    int8_t x,i;
    int16_t lms_mV;
    uint8_t data_thresh = 0;
    ...
    if (data_thresh) {
        lms_mV =  (((y<<2)*(190<<2)/(29<<2))>>2);
    } else {
        lms_mV =  (((y<<2)*(136<<2)/(35<<2))>>2);
    }
    EFUN_PRINTF((" %4dmV : ",lms_mV));
...
err_code_t merlin7_pcieg3_display_eye_scan_header(srds_access_t *sa__, int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t merlin7_pcieg3_display_eye_scan_footer(srds_access_t *sa__, int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
err_code_t merlin7_pcieg3_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    {
        int16_t nlmv_val = 0;
        private_afe_override_slicer_sel_t slicer = INVALID_SLICER_;
        EFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset_start, 0, slicer, &nlmv_val));
    ...
    EFUN(merlin7_pcieg3_INTERNAL_poll_diag_done(sa__, &sts,timer_control*2000));
    ...
    do {
        if(heye == 1) {
            ...
            if(intrusive) {
                ...
                EFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250, slicer, &nlmv_val));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        EFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset_start, range250, slicer, &nlmv_val));
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        EFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        ...
        if(DIAG_VERBOSE < 5) {
            ...
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                EFUN(merlin7_pcieg3_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)SRDS_ABS(offset_start)-i, range250, slicer, &nlmv_val));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(merlin7_pcieg3_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
...
err_code_t merlin7_pcieg3_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    ESTM_PRINTF(("Core = %d; LANE = %d\n",merlin7_pcieg3_get_core(sa__),merlin7_pcieg3_get_lane(sa__)));
    ...
    ESTM(rx_lock = rd_pmd_rx_lock());
    {
        ...
        micro_stop = merlin7_pcieg3_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
    ...
    if(diag_level & SRDS_DIAG_TIMESTAMP) {
        EFUN(merlin7_pcieg3_test_uc_timestamp(sa__));
    }
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        ...
        uint8_t micro_num = 0;
        {
            ...
            EFUN(merlin7_pcieg3_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, merlin7_pcieg3_INTERNAL_event_log_dump_callback));
            EFUN(merlin7_pcieg3_INTERNAL_event_log_dump_callback(sa__, &state, 0, 0));
        }
    }
    if(diag_level & SRDS_DIAG_EVENT) {
        EFUN(merlin7_pcieg3_read_event_log(sa__));
    }
    /* currently REG_CORE and REG_LANE dump same data. */
    core = merlin7_pcieg3_get_core(sa__);
    lane = merlin7_pcieg3_get_lane(sa__);

    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(merlin7_pcieg3_reg_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(merlin7_pcieg3_uc_core_var_dump(sa__, core, NULL));

    }
    if(diag_level & SRDS_DIAG_UC_LANE) {

        EFUN(merlin7_pcieg3_uc_lane_var_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(merlin7_pcieg3_display_eye_scan(sa__));
    }
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        EFUN(merlin7_pcieg3_INTERNAL_display_info_table(sa__));
    }
    /* re enable micro */
    if (!micro_stop) {
...
/* Required Diagnostic Functions */
err_code_t merlin7_pcieg3_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(merlin7_pcieg3_pmd_rdt_reg(sa__, i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(merlin7_pcieg3_pmd_rdt_reg(sa__, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin7_pcieg3_pmd_wr_reg(sa__, addr,tmp2));
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin7_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + merlin7_pcieg3_info_ptr->lane_var_ram_base + (merlin7_pcieg3_get_physical_lane(sa__) * merlin7_pcieg3_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,merlin7_pcieg3_rdbl_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    ...
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,merlin7_pcieg3_rdwl_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    ...
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_rdbl_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin7_pcieg3_wrbl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_rdwl_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin7_pcieg3_wrwl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_INTERNAL_rdb_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin7_pcieg3_INTERNAL_wrb_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin7_pcieg3_INTERNAL_rdw_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin7_pcieg3_INTERNAL_wrw_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin7_pcieg3_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, GRACEFUL_STOP_TIME));
