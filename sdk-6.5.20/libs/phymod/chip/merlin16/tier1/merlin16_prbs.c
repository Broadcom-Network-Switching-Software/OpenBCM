...
/* Comment out display of PRBS burst error if BLACKHAWK */
  ...
  const char* merlin16_e2s_prbs_mode_enum[8] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t merlin16_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        char *prbs_string;
        EFUN(merlin16_get_tx_prbs_config(sa__, &prbs_poly_mode, &prbs_inv));
        prbs_string = (char *)merlin16_e2s_prbs_mode_enum[prbs_poly_mode];
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        char *prbs_string;
        EFUN(merlin16_get_rx_prbs_config(sa__, &prbs_poly_mode, &prbs_checker_mode, &prbs_inv));
        prbs_string = (char *)merlin16_e2s_prbs_mode_enum[prbs_poly_mode];
        ESTM_PRINTF((" %s",prbs_string));
        ESTM_PRINTF(("      %1d     ",prbs_inv));
    } else {
