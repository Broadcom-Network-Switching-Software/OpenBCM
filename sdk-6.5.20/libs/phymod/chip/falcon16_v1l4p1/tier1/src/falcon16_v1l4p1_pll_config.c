...
err_code_t falcon16_v1l4p1_INTERNAL_configure_pll(srds_access_t *sa__,
    ...
    EFUN(falcon16_v1l4p1_INTERNAL_resolve_pll_parameters(sa__, refclk, &refclk_freq_hz, &srds_div, &vco_freq_khz, pll_option));
...

    {
        ...
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: falcon16_v1l4p1_configure_pll(..) called without core_dp_s_rstb=0\n"));
    ...
    if (!int_configured) {
        ...
        if ((pll_fracn_ndiv_int < 12) || (pll_fracn_ndiv_int > 251)) {
            return (falcon16_v1l4p1_error(sa__, ERR_CODE_INVALID_PLL_CFG));
        }
        ...
        EFUN(wrc_ams_pll_ndiv_frac_vld(1));
        EFUN(wrc_ams_pll_ndiv_frac_vld(0));
    }
    ...
    /* Determine KVH and pll2rx_clkbw_val per AMS v1.1 and comments in CRSRDSAPI-137, CRSRDSFW-138, CRSRDSAPI-203 */
    {
        ...
        if (pll_option == FALCON16_V1L4P1_PLL_OPTION_REFCLK_DOUBLER_EN) {
            EFUN(wrc_ams_pll_refclk_doubler(0x1));
            EFUN(wrc_ams_pll_doubler_res(0x6));
            EFUN(wrc_ams_pll_doubler_cap(0x3));
        } else if (pll_option == FALCON16_V1L4P1_PLL_OPTION_REFCLK_DIV2_EN) {
            EFUN(wrc_ams_pll_div2(1));
            EFUN(wrc_ams_pll_div4_2_sel(1));
        } else if (pll_option == FALCON16_V1L4P1_PLL_OPTION_REFCLK_DIV4_EN) {
    ...
    if (fracn_sel) {
        ...
        *srds_div = (uint32_t)(SRDS_INTERNAL_COMPOSE_PLL_DIV(pll_fracn_ndiv_int, pll_fracn_div, pll_fracn_frac_bits));
        ...
        switch (pll_mode) {
            ...
            default:
                EFUN_PRINTF(("ERROR: falcon16_v1l4p1_INTERNAL_read_pll_div() found invalid pll_mode value:  %d\n", pll_mode));
                return (falcon16_v1l4p1_error(sa__, ERR_CODE_PLL_DIV_INVALID));
        }
    }
    return (ERR_CODE_NONE);
