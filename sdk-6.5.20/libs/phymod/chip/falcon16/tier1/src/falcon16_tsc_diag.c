...
err_code_t falcon16_tsc_test_uc_timestamp(srds_access_t *sa__) {
    ...
    EFUN(falcon16_tsc_pmd_uc_cmd(sa__, CMD_UC_DBG,CMD_UC_DBG_TIMESTAMP,100));
...
err_code_t falcon16_tsc_display_eye_scan(srds_access_t *sa__) {
    ...
    ESTM(core=falcon16_tsc_get_core(sa__));
    ESTM(lane=falcon16_tsc_get_lane(sa__));
...
err_code_t falcon16_tsc_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(falcon16_tsc_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_VSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_HORIZ) {
        EFUN(falcon16_tsc_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_HSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_SLICE) {
        EFUN(falcon16_tsc_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_EYE_SLICE,GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}
...
err_code_t falcon16_tsc_INTERNAL_poll_diag_data(srds_access_t *sa__, srds_info_t *falcon16_tsc_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    ...
    lane_diag_size = falcon16_tsc_info_ptr->diag_mem_ram_size;
    *diag_rd_ptr = 0;
    ...
    if (byte_count > (lane_diag_size / 2)) {
        ESTM_PRINTF(("\nERROR : falcon16_tsc_INTERNAL_poll_diag_data() has excessive byte count of %d.\n", byte_count));
...
err_code_t falcon16_tsc_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    EFUN(falcon16_tsc_INTERNAL_verify_falcon16_tsc_info(falcon16_tsc_info_ptr, sa__));
        ...
        lane_diag_base = falcon16_tsc_info_ptr->diag_mem_ram_base + ((lane%falcon16_tsc_info_ptr->lane_count) * falcon16_tsc_info_ptr->diag_mem_ram_size) +
            falcon16_tsc_info_ptr->grp_ram_size*falcon16_tsc_INTERNAL_grp_idx_from_lane(falcon16_tsc_get_physical_lane(sa__));

    EFUN(falcon16_tsc_INTERNAL_poll_diag_data(sa__, falcon16_tsc_info_ptr, status, &diag_rd_ptr, stripe_size, 400));
...
err_code_t falcon16_tsc_meas_eye_scan_done(srds_access_t *sa__) {
  EFUN(falcon16_tsc_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_DISABLE,GRACEFUL_STOP_TIME));
  return(ERR_CODE_NONE);
}
...
err_code_t falcon16_tsc_start_ber_scan_test(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(falcon16_tsc_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_START, ber_scan_mode,GRACEFUL_STOP_TIME));
    return(ERR_CODE_NONE);
}
...
err_code_t falcon16_tsc_read_ber_scan_data(srds_access_t *sa__, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    if((sts & 0x8000) == 0) {
        return(falcon16_tsc_error(sa__, ERR_CODE_DATA_NOTAVAIL));
    }
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
...
err_code_t falcon16_tsc_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(falcon16_tsc_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,200));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if((osr_mode.rx > FALCON16_TSC_OSX4) && (osr_mode.rx != FALCON16_TSC_OSX2P5)) {
#else
    if(osr_mode.rx > FALCON16_TSC_OSX4) {
#endif
        EFUN_PRINTF(("ERROR DIAG display_ber_data: osr mode too high\n"));
        return(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
    }
    if (osr_mode.rx > FALCON16_TSC_OSX4) { /* BER supported for only FALCON16_TSC_OSX2P5 mode when osr_mode > osx4 */
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(falcon16_tsc_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        EFUN(falcon16_tsc_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
...
err_code_t falcon16_tsc_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    EFUN(falcon16_tsc_version(sa__, &api_version));
    ...
    ESTM(rx_lock = rd_pmd_rx_lock());
    {
        ...
        micro_stop = falcon16_tsc_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
    ...
    if(diag_level & SRDS_DIAG_TIMESTAMP) {
        EFUN(falcon16_tsc_test_uc_timestamp(sa__));
    }
    ...
    core = falcon16_tsc_get_core(sa__);
    lane = falcon16_tsc_get_lane(sa__);

    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(falcon16_tsc_reg_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(falcon16_tsc_uc_core_var_dump(sa__, core, NULL));

    }
    if(diag_level & SRDS_DIAG_UC_LANE) {

        EFUN(falcon16_tsc_uc_lane_var_dump(sa__, core, lane, NULL));
    }
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        EFUN(falcon16_tsc_INTERNAL_display_info_table(sa__));
    }
...
err_code_t falcon16_tsc_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(falcon16_tsc_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + falcon16_tsc_info_ptr->lane_var_ram_base + (falcon16_tsc_get_physical_lane(sa__) * falcon16_tsc_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
    ...
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(falcon16_tsc_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, GRACEFUL_STOP_TIME));
