...
{
    TX_PI_CTL3r_t   tx_pi_reg3;

    TX_PI_CTL3r_CLR(tx_pi_reg3);
    /* ext PD select:   0 = normal mode, 1 = external PD mode */
    TX_PI_CTL3r_TX_PI_EXT_PD_INC_DEC_SELf_SET(tx_pi_reg3, ext_pd ? 1 : 0);

    MODIFY_TX_PI_CTL3r(pa, tx_pi_reg3);
...
err_code_t eagle2_tsc2pll_tx_pi_ext_pd_select_get(const phymod_access_t *pa, uint32_t *ext_pd)
{
    ...
    READ_TX_PI_CTL3r(pa, &tx_pi_reg3);

    *ext_pd = TX_PI_CTL3r_TX_PI_EXT_PD_INC_DEC_SELf_GET(tx_pi_reg3);
