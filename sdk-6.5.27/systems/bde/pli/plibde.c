...
STATIC const char*
_adapter_bde_name(void)
{
    return "dpp-adapter-bde";
}

STATIC int
_adapter_bde_num_devices(int type)
{
    return _n_devices;
}

STATIC const ibde_dev_t*
_adapter_bde_get_dev(int d)
{
    if(d < 0 || d > _n_devices) {
        return NULL;
    }
...
STATIC uint32
_adapter_bde_get_dev_type(int d)
{
    if(d < 0 || d > _n_devices) {
        return 0;
    }
...
STATIC uint32
_adapter_bde_pci_config_read(int d, uint32 addr)
{
    return (0xffffffff); /* not supported */
}

STATIC int
_adapter_bde_pci_config_write(int d, uint32 addr, uint32 data)
{
    return (-1); /* not supported */
}

STATIC void
_adapter_bde_pci_bus_features(int unit, int* be_pio, int* be_packet, int* be_other)
{
    *be_pio = 0;
    *be_packet = 0;
    *be_other = 0;
...
STATIC uint32
_adapter_bde_memory_read(int d, uint32 addr)
{
    return 0;
}
...
/*  function for mock access to device BARs (iproc and CMIC) */
STATIC int
_adapter_bde_memory_write(int d, uint32 addr, uint32 data)
{
    return 0;
}

STATIC uint32*
_adapter_bde_salloc(int d, int size, const char *name)
{
    return sal_dma_alloc(size, (char *)name);
}

STATIC void
_adapter_bde_sfree(int d, void* ptr)
{
    sal_dma_free(ptr);
}

STATIC int
_adapter_bde_sflush(int d, void* addr, int length)
{
...
STATIC int
_adapter_bde_sinval(int d, void* addr, int length)
{
...
STATIC int
_adapter_bde_interrupt_connect(int d, void (*isr)(void*), void* data)
{
    return 0;
}

STATIC int
_adapter_bde_interrupt_disconnect(int d)
{
    return 0;
}

STATIC sal_paddr_t
_adapter_bde_l2p(int d, void* laddr)
{
    return PTR_TO_INT(laddr);
}

STATIC void*
_adapter_bde_p2l(int d, sal_paddr_t paddr)
{
    return INT_TO_PTR(paddr);
}

static ibde_t _adapter_bde = {
    _adapter_bde_name,
    _adapter_bde_num_devices,
    _adapter_bde_get_dev,
    _adapter_bde_get_dev_type,
    _adapter_bde_pci_config_read,
    _adapter_bde_pci_config_write,
    _adapter_bde_pci_bus_features,
    _adapter_bde_memory_read,
    _adapter_bde_memory_write,
    _adapter_bde_salloc,
    _adapter_bde_sfree,
    _adapter_bde_sflush,
    _adapter_bde_sinval,
    _adapter_bde_interrupt_connect,
    _adapter_bde_interrupt_disconnect,
    _adapter_bde_l2p,
    _adapter_bde_p2l,
    NULL,
    NULL,
...
#define ADAPTER_DEFAULT_DEVID 0x8470
#define ADAPTER_DEVID_MAX_LENGTH 8
#define ADAPTER_DEFAULT_REVID 0x1
#define ADAPTER_REVID_MAX_LENGTH 8
extern char *getenv(const char*);

int
plibde_adapter_create(ibde_t** bde)
{
    char *dev_id_string, *rev_id_string;
    /* Use +2 to leave space for the '0x' addition */
    char dev_id_hex_string[ADAPTER_DEVID_MAX_LENGTH+2];
    char rev_id_hex_string[ADAPTER_REVID_MAX_LENGTH+2];
    unsigned int dev_id = ADAPTER_DEFAULT_DEVID;
    unsigned int rev_id = ADAPTER_DEFAULT_REVID;
    char tmp[50];
    int i;
    ...
    if (_n_devices > 0 && _n_devices < PLI_MAX_DEVICES)
    {
        ...
        {
            sal_snprintf(tmp, sizeof(tmp), "ADAPTER_DEVID_%d", i);
            dev_id_string = getenv(tmp);
            sal_snprintf(tmp, sizeof(tmp), "ADAPTER_REVID_%d", i);
            ...
            if (dev_id_string)
            {
                ...
                sal_memcpy(&(dev_id_hex_string[2]), dev_id_string, ADAPTER_DEVID_MAX_LENGTH);
                sscanf(dev_id_hex_string, "%x", &dev_id);
            }

            if (rev_id_string)
            {
                sal_memcpy(rev_id_hex_string, rev_id_string, ADAPTER_REVID_MAX_LENGTH);
            ...
            _devices[i].bde_dev.dev_unique_id = i;
        }
    }
