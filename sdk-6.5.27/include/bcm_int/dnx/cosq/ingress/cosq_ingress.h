...
typedef struct
{
     ...
     * valid in case of unified counter for dequeue to fabric and to DRAM
     */
    int deq_pkt_ovf;
    uint64 pkt_deq_cnt;
    /**
     * NOF packets de-queue to DRAM
     * valid if separate counters for dequeue to fabric and dequeue to DRAM are supported
     */
    int deq_dram_pkt_ovf;
    uint64 pkt_deq_dram_cnt;
    /**
     * NOF packets de-queue to fabric
     * valid if separate counters for dequeue to fabric and dequeue to DRAM are supported
     */
    int deq_fabric_pkt_ovf;
    uint64 pkt_deq_fabric_cnt;
