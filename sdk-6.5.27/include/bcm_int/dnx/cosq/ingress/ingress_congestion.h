...
typedef enum
{
    ...
    DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_PKT_SIZE,
    DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_GATE_STATE,
...
typedef struct
{
    ...
    /**
     *  Boolean:
     *  0 - If average queue size is larger than max_avrg_th, drop probability is 100%
     *  1 - If average queue size is larger than max_avrg_th, drop probability is max_probability.
     */
...
typedef struct
{
    ...
    uint32 guaranteed_size[DNX_INGRESS_CONGESTION_RESOURCE_NOF][DNX_COSQ_NOF_DP];       /* For colorless guaranteed use 
                                                                                         * DP0 */
...
/*
 * ingress congestion global vsq occupancy statistics
 */
typedef struct
{
    /**
     ...
     * Shared max
     */
    uint32 shared_max[DNX_DATA_MAX_INGR_CONGESTION_VSQ_POOL_NOF];
    /**
     * Guaranteed
     */
     ...
     * Guaranteed max
     */
    uint32 guaranteed_max[DNX_DATA_MAX_INGR_CONGESTION_VSQ_POOL_NOF];
    /**
     * Headroom
     */
    uint32 headroom;
     ...
     */
    uint32 headroom_max;
} dnx_ingress_congestion_gl_vsq_occupancy_info_t;
/*
...
/**
 * \brief - Function to get the VOQ programmable counter 
 *        to track certain VOQ or VOQ bundle
 * 
 * \param [in] unit - unit
 * \param [in] core - core
 * \param [out] base_voq_id - base VOQ ID
 * \param [out] nof_voqs - number of VOQs
 *
 * \return
 *   See shr_error_e
 * \remark
 ...
 *   * None
 */
shr_error_e dnx_ingress_congestion_voq_programmable_counter_configuration_get(
    int unit,
    int core,
    uint32 *base_voq_id,
    uint32 *nof_voqs);
...
/**
 *\brief run VSQ occupancy DBAL iterator
 *       The function will change the bubble width to enable CPU access
 *
 * \param [in] unit - unit index
 * \param [in] core - device core
 * \param [in] oversubscription - set to '1' to increase the access bubble pulse width. should be enabled in case of over-subscription.
 ...
 *   * None
 */
shr_error_e dnx_ingress_congestion_vsq_occupancy_iterator_get_next(
    int unit,
    int core,
...
/**
 * \brief - Function sets to 0 the SW state for a specific type.
 *
 * \param [in] unit - unit index
 * \param [in] core - device core
 * \param [in] pool_id - pool_id
 * \param [in] lossless_pool - lossless pool id (used for headroom)
 * \param [in] stat_type - cosq ingress congestion resource statistics type
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_ingress_congestion_gl_vsq_occupancy_max_value_clear(
    int unit,
    int core,
    int pool_id,
    int lossless_pool,
    bcm_cosq_icgm_resource_stat_type_t stat_type);

/**
 * \brief - Function to get all the current Global VSQ occupancy statistics.
 *
 * \param [in] unit - unit index
 * \param [in] core - device core
 * \param [out] info - occupancy info array
 ...
 *   * None
 */
shr_error_e dnx_ingress_congestion_gl_vsq_occupancy_info_get(
    int unit,
    int core,
    dnx_ingress_congestion_gl_vsq_occupancy_info_t * info);
...
/**
 * \brief -  set ocb global drop threshold
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] flags - not used.
 *   \param [in] gport - should be "0" (global configuration)
 *   \param [in] key - key containing threshold index (0: High Priority, 1: Mid Priority, 2: Low Priority)
 *   \param [in] threshold - threshold value
 *
 * \remark
 *   * None
 ...
 *   * None
 */
shr_error_e dnx_ingress_congestion_ocb_global_drop_profile_threshold_set(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
...
/**
 * \brief -  get ocb global drop threshold
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] flags - not used.
 ...
 *   * None
 */
shr_error_e dnx_ingress_congestion_ocb_global_drop_profile_threshold_get(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
...
/**
 * \brief -  Map ocb global drop profile to logical port
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] flags - not used.
 *   \param [in] gport_to_map - logical port / logical port gport
 *   \param [in] cosq - not used.
 *   \param [in] profile_id - ocb global drop profile
 *
 * \remark
 *   * None
 * \see
 *   * None
 */
shr_error_e dnx_ingress_congestion_port_ocb_global_drop_profile_map_set(
    int unit,
    uint32 flags,
...
/**
 * \brief -  Get ocb global drop profile mapped to logical port
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] flags - not used.
 *   \param [in] gport_to_map - logical port / logical port gport
 *   \param [in] cosq - not used.
 *   \param [out] profile_id - ocb global drop profile
 *
 * \remark
 *   * None
 * \see
 *   * None
 */
shr_error_e dnx_ingress_congestion_port_ocb_global_drop_profile_map_get(
    int unit,
    uint32 flags,
    bcm_gport_t gport_to_map,
    bcm_cos_queue_t cosq,
...
/**
 * \brief -  map priority to [ocb drop profile,TC,DP]
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] flags - not used.
 *   \param [in] profile - ocb global drop profile gport
 *   \param [in] key - key containing TC,DP
 ...
 *   * None
 */
shr_error_e dnx_ingress_congestion_ocb_global_drop_profile_priority_map_set(
    int unit,
    uint32 flags,
    bcm_gport_t profile,
    uint32 key,
...
/**
 * \brief -  get priority mapped to [ocb drop profile,TC,DP]
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] flags - not used.
 ...
 *   * None
 */
shr_error_e dnx_ingress_congestion_ocb_global_drop_profile_priority_map_get(
    int unit,
    uint32 flags,
    bcm_gport_t profile,
    uint32 key,
...
/*
 * brief - calculate available total bytes resources to use in dnx data.
 * if there are no active drams, we should return the max total bytes that was provided
 */
uint32 dnx_ingress_congestion_calc_available_total_bytes(
