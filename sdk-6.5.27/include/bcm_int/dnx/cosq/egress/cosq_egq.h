 ...
 *   Otherwise, see shr_error_e
 * \remark
 *   * Input port must be channelized. For not channelized port, see
 ...
 */
shr_error_e dnx_cosq_egq_gport_egress_map_get(
    int unit,
    bcm_gport_t gport,
    int count,
 ...
 *   bcm_gport_t. Identifier of traffic by its assigned gport.
 * \param [in] count - nof keys/offsets.
 * \param [in] key_array - keys (TC/DP) array. tc values must be between 0 and
 *   DNX_DEVICE_COSQ_ING_NOF_TC-1 (7). dp values must be between 0 and
 *   DNX_DEVICE_COSQ_ING_NOF_DP-1 (3).
 * \param [in] offset_array - offset (from base) array. cosq values must be between 0 and
 *   DNX_EGR_NOF_Q_PRIO-1 (7).
 * \return
 *   If zero (_SHR_E_NONE), then no error was encountered.
 *   Otherwise, see shr_error_e
 ...
 */
shr_error_e dnx_cosq_egq_gport_egress_queue_map_set(
    int unit,
    bcm_gport_t gport,
    int count,
    bcm_cosq_queue_offset_mapping_key_t * key_array,
...
/**
 * \brief - egr cosq interface controls dispatcher.
 *
 * \param [in] unit -  Unit-ID
 * \param [in] gport - description per use case, must be local interface.
 ...
 *   * None
 */
shr_error_e dnx_cosq_egq_control_egress_interface_set(
    int unit,
    bcm_gport_t gport,
    bcm_cos_queue_t cosq,
    bcm_cosq_control_t type,
 ...
 *   * None
 */
shr_error_e dnx_cosq_egq_control_egress_interface_get(
    int unit,
    bcm_gport_t gport,
    bcm_cos_queue_t cosq,
    bcm_cosq_control_t type,
 ...
 * \param [in] flags -
 *   uint32. flags.
 * \param [in] mapping_data -
 *   bcm_cosq_egress_mapping_t. struct contaid out_dp/offset per each TC and DP.
 * \return
 *   If zero (_SHR_E_NONE), then no error was encountered.
 *   Otherwise, see shr_error_e
 * \remark
 ...
 *   * dnx_egr_queuing_profile_mapping_set
 */
int dnx_egq_cosq_port_egress_mapping_set(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    int count,
 ...
 *   * bcm_dnx_cosq_gport_egress_map_set
 */
shr_error_e dnx_egq_cosq_port_egress_mapping_get(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    int count,
...
/*
 * \brief
 *   This API is used to set BCM_COSQ_MULTICAST_SCHEDULED configuration.
 *             gport = LOCAL / MODPORT
 ...
 * \param [in] offset_array - array of VOQ offsets and out dp
 ...
 */
shr_error_e dnx_cosq_port_egress_multicast_mapping_set(
    int unit,
    bcm_gport_t gport,
    int count,
    bcm_cosq_queue_offset_mapping_key_t * key_array,
...
/*
 * \brief
 *   This API is used to get BCM_COSQ_MULTICAST_SCHEDULED configuration.
 *             gport = LOCAL / MODPORT
 *               this option maps ingress_pri and ingress_dp to
 *               multicast TC and multicast DP.
 *
 ...
 * \param [in] count - number of keys to map (size of key_array and offset_array)
 * \param [in] key_array - array of keys
 * \param [out] offset_array - array of VOQ offsets and out dp
 *   For 'gport' set to 'LOCAL' or 'MODPORT', this procedure uses only
 *   two elements:
 *     'priority' -
 *       Priority to assign to outgoing multicast traffic. Must be between 0 and
 ...
 *   If traffic is enabled and the property is disabled - the function will fail.
 */
shr_error_e dnx_cosq_port_egress_multicast_mapping_get(
    int unit,
    bcm_gport_t gport,
    int count,
    bcm_cosq_queue_offset_mapping_key_t * key_array,
