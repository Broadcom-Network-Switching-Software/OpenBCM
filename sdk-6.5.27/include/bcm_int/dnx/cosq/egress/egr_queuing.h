...
/**
 * \brief -
 *   Get the current calendar configurations.
 *
 * \param [in] unit -
 *   HW Unit-ID
 * \param [in] core_id -
 *  core_id to use to access HW.
 * \param [out] current_cal -
 *  current calendar id.
 * 
 * \return
 *   If zero (_SHR_E_NONE), then no error was encountered.
 ...
 */
shr_error_e dnx_egr_queuing_nif_current_cal_get(
    int unit,
    int core_id,
    uint32 *current_cal);
...
/**
 * \brief - update credit size (increase/decrease)
 *
 * \param [in] unit -  Unit-ID
 * \param [in] port -  port
 ...
 *   * None
 */
shr_error_e dnx_egr_queuing_nif_credit_update(
    int unit,
    bcm_port_t port,
    int old_speed);
