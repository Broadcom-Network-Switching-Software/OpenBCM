...
#define DNX_ALGO_OAM_LIF_ID "OAM LIF for OAM Primary VLAN"
...
#define DNX_ALGO_OAM_SERVER_DEVICE_INDEX_TEMPLATE "OAM SERVER DEVICE INDEX"
#define DNX_ALGO_OAM_DEFAULT_MEP_PROFILE "OAM DEFAULT MEP PROFILE"
#define DNX_SLM_OAMP_TEMPLATE_TX_RATE "SLR measurement period"
...
#define DNX_OAM_OAMP_PROFILE_ICC_FLAG                       (0x200)
#define DNX_OAM_OAMP_PROFILE_TLV_FLAG                       (0x400)
#define DNX_OAM_OAMP_PROFILE_TX_PERIOD_FLAG                 (0x800)
#define DNX_OAM_OAMP_PROFILE_DELAY_HISTOGRAM_PROFILE_FLAG   (0x1000)
#define DNX_OAM_OAMP_PROFILE_SLM_MEASUREMENT_PROFILE_FLAG   (0x2000)
#define DNX_OAM_OAMP_PROFILE_STAMP_TLV_FLAG                 (0x4000)

/* UC DMAC */
#define DNX_OAM_OAMP_PROFILE_DA_MAC_MSB_FLAG                (0x8000)
#define DNX_OAM_OAMP_PROFILE_DA_MAC_LSB_FLAG                (0x10000)
 ...
 *  class/drop precedence/quality service" profiles as template.  These values
 *  are used by the OAMP to construct the ITMH and PPH header of OAM/BFD
...
/**
 * \brief - data for TLV profile map table entry
 */
typedef struct
{
    uint8 stamp_flags;
} dnx_oam_oamp_v2_stamp_tlv_profile_t;
...
typedef struct
{
    ...
    uint8 type1;
    uint8 type2;
} dnx_oam_oamp_v2_tlv_profile_t;

typedef struct
{
    uint8 delay_bin_format;
    uint8 delay_bin_multiplier_exp;
    uint8 delay_bin_multiplier_rep;
} dnx_oam_oamp_v2_delay_histogram_profile_t;

typedef union
{
    dnx_oam_group_icc_profile_data_t icc_data;
} algo_oamp_v2_oam_ch_2_small_profile_t;
...
typedef struct
{
    ...
    /*
     * DA NIC(LSB 24 bits) profile
     */
    uint8 da_mac_lsb_profile;
    bcm_mac_t da_mac;
...
void dnx_oam_oamp_oamp_trap_tcam_print_cb(
    ...
    const void *data);
/**
 * \brief - Printing callback for the OAM SERVER DEVICE INDEX templates,
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] data - Pointer to template profile data.
 * \return
 ...
 *   * None
 */
void dnx_oam_server_device_index_print_cb(
    int unit,
    const void *data);
...
/**
 * \brief - Printing callback for the STAMP TLV profile
 *        templates.  These profiles are used to
 *        construct the Flags field of TWAMP TLV.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] data - Pointer to template profile data.  The STAMP TLV
 *        profile structure dnx_oam_oamp_v2_stamp_tlv_profile_t
 ...
 *   * None
 */
void dnx_oam_oamp_v2_stamp_tlv_profile_print_cb(
    int unit,
    const void *data);

/**
 * \brief - Printing callback for the OAM TLV profile
 *        templates.  These profiles are used to
 *        construct the TLV payload in OAM packets.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] data - Pointer to template profile data.  The
 *        TLV profile structure dnx_oam_oamp_v2_tlv_profile_t
 *        is defined and described above.
 * \return
 * \remark
 *   * A print callback function is required to create a
 ...
 *   * None
 */
void dnx_oam_oamp_v2_tlv_profile_print_cb(
    int unit,
    const void *data);

/**
 * \brief - Printing callback for the OAM candidate
 *        header small profile templates.  These
 *        profiles are used to construct the 6 byte
 *        ICC field for CCM packets.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] data - Pointer to template profile data - 6 bytes.
 * \return
 * \remark
 *   * A print callback function is required to create a
 *     template.  It is later used for BCM shell diagnostics.
 ...
 *   * None
 */
void dnx_oam_oamp_v2_oam_ch_2_small_profile_print_cb(
    int unit,
    const void *data);
