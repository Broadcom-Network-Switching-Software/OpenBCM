 ...
 *        this is relevant if the HW resource that the CHM uses is in divided
 *        to banks, and the user has multiple hierarchies, and each bank can be
 *        accessed by a single hierarchy at a time.
 * \param [in] nof_banks - number of banks the HW resources are divided to,
...
/**
 * \brief - for given hierarchy, profile_type and nof_profiles check if there are sufficient resources for allocation
 *
 * \param [in] unit - unit number
 ...
 *   * None
 */
shr_error_e dnx_algo_consistent_hashing_manager_verify_sufficient_resources(
    int unit,
    uint32 chm_handle,
    int profile_type,
    uint32 hierarchy,
