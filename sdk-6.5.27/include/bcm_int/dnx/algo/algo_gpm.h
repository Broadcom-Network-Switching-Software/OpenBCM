...
/**
 * Invalid CORE indication
 */
#define DNX_ALGO_GPM_CORE_INVALID (-1)
...
/**
 * Gport Flow conversion flags
 */
#define DNX_ALGO_GPM_CONVERSION_FLAG_NONE               (0)
/** Convert to a FLOW Terminator gport - Applicable only When the in-gport is symmetric */
#define DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR    (SAL_BIT(0))
/** Convert to a FLOW Initiator gport - Applicable only When the in-gport is symmetric */
#define DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR     (SAL_BIT(1))
/** Convert from a Virtual FLOW gport to an Ingress-Only LIF sub-type */
#define DNX_ALGO_GPM_CONVERSION_FLAG_INGRESS_LIF        (SAL_BIT(2))
/** Convert from a Virtual FLOW gport to a Native gport */
#define DNX_ALGO_GPM_CONVERSION_FLAG_NATIVE             (SAL_BIT(3))
/** Calculate the conversion flags based upon flow encoding */
#define DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_CALC          (SAL_BIT(4))

#define ALGO_GPM_GPORT_L2_FWD_INFO_FLAGS_NO_ERR_MSG_NOT_FOUND (1)
...
typedef enum
{
    ...
    ALGO_GPM_GPORT_VERIFY_TYPE_FLOW_AGG,
...
/**
 * \brief structure holds local in lif value per core.
 * If local in lif is the same in all cores, nof_in_lifs=1 and the core_id[0]=SHR_CORE_ALL
 */
typedef struct
{
    int local_in_lif[DNX_DATA_MAX_DEVICE_GENERAL_NOF_CORES];
    int core_id[DNX_DATA_MAX_DEVICE_GENERAL_NOF_CORES];
    int nof_in_lifs;
} dnx_algo_gpm_local_in_lif_per_core_t;
...
typedef struct
{
    ...
    int global_out_lif;
    /**
     * Local-LIF, Invalid when does not exist or when it is not the same for all cores.
     * Note:  although local_in_lif and local_in_lif_per_core holds the similar info,
     *        we keep local_in_lif for backward compatibility.
     */
    int local_in_lif;
    ...
    /**
     * The logical phase this outlif is mapped to.
     */
    ...
    /**
     * local in lif value per core.
     */
    dnx_algo_gpm_local_in_lif_per_core_t local_in_lif_per_core;
    /**
     * virtual lif id. valid only for virtual lif type (ingress/egress)
     */
    int virtual_id;
...
/**
 * \brief - Encode a Flow gport based on various flags that
 *        determine how the gport is to be encoded.
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] global_lif_or_virtual_id - Global-LIF or a
 *          Virtual-ID.
 *   \param [in] is_terminator - Indication whether it's a
 *          Terminator or Initiator object. *
 *   \param [in] is_virtual - Indication whether the encoding
 *          should be done for a Virtual object.
 *   \param [in] is_local_lif - Indication whether the object
 *          has a Local-LIF (Not ESEM like).
 *   \param [in] is_esem_default - Indication whether it's an
 *          ESEM-Default object.
 * \return DIRECT OUTPUT:
 *   bcm_gport_t - The encoded flow gport
 */
bcm_gport_t dnx_algo_gpm_flow_gport_encode(
    int unit,
...
/**
 * \brief - Retrieve a Flow gport from a Local-LIF.
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 *   \param [in] local_lif - The local-LIF value
 *   \param [in] is_ingress - Indication whether it's a an
 *          Ingress/Egress Local-LIF
 *   \param [in] core_id - The Local-LIF's core_id in Ingress or
 *          _SHR_CORE_ALL.
 *   \param [out] flow_gport - The retrieved Flow gport
 * \return DIRECT OUTPUT:
 *   shr_error_e - Error return value
 *   _SHR_E_NOT_FOUND - The local-LIF isn't allocated
 */
shr_error_e dnx_algo_gpm_flow_gport_from_local_lif(
    int unit,
    int local_lif,
    int is_ingress,
    int core_id,
...
/**
 * Destination is for asymmetric VOQ. It will not check the core_id of the gport
 */
#define ALGO_GPM_ENCODE_DESTINATION_ASYM_VOQ                        SAL_BIT(1)

/**
  * \brief
 * check if given gport is valid destination gport
 * header profile
 * \param [in] unit -The unit number.
 * \param [in] port - gport
 * \param [out] gport_is_valid_destination - return true, if gport is valid destination. otherwise, return false
 */
shr_error_e algo_gpm_is_gport_valid_destination(
    int unit,
    bcm_gport_t port,
...
typedef struct
{
     ...
     */
    dbal_result_type_t fwd_info_result_type;
    /**
...
*  \param [in] is_replace_type_change_allowed -
*    Indication whether replacement of forward info type is
*    allowed.
*  \param [in] gport -
*    GPORT given by user
...
*  \param [in] gport -
*    GPORT given by user
*  \param [in] flags - flag indications: ALGO_GPM_GPORT_L2_FWD_INFO_FLAGS_XXX
...
*    Relevant unit.
*  \param [in] flags -
*    flags - it can be some of the following
*    ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE
*    ALGO_GPM_ENCODE_DESTINATION_ASYM_VOQ -it will allow the gport given by the user to be asymmetric VOQ-gport
*  \param [in] gport -
*    GPORT given by user
*  \param [in] encap_id -
...
/**
 * \brief
 *      Function convert lif gport (tunnel/vlan_port etc) to flow lif gport
 *      or convert flow lif gport to lif gport
 * \param [in] unit -
 *      Relevant unit.
 * \param [in] in_gport - in gport
 * \param [in] out_gport_type - determine the output gport type 
 * \param [in] conversion_flags - Flags that are required to 
 *      distinguish between a few possible conversions. 
 * \param [out] out_gport - gport converted to the type mentioned in out_gport_type
 * \remark
 *      None
 */
shr_error_e algo_gpm_gport_flow_convert(
    int unit,
