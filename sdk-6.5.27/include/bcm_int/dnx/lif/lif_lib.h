...
/**
 * \brief
 *   Deallocate all of the local outlifs.
 *
 * \param [in] unit -
 *     Identifier of the device to access.
 * \return
 ...
 *   None.
 */
shr_error_e dnx_lif_lib_all_outlifs_free(
    int unit);
...
/** see  dnx_lif_lib_free  */
shr_error_e dnx_lif_lib_free_without_releasing_sw_info(
    int unit,
    int global_lif,
    lif_mngr_local_inlif_info_t * inlif_info,
    int local_outlif);
...
/**
 * \brief
 * Writes Global to local lif mapping in ISEM, either to new entry, or updating existing one
 ...
 *   \param [in] is_update - if need to update existing entry
 *
 */
shr_error_e dnx_global_lif_reclassify_write(
    int unit,
    uint32 global_inlif,
    uint32 core_id,
    uint32 local_inlif,
