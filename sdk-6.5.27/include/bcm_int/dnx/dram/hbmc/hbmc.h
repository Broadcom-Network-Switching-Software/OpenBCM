...
typedef struct hbmc_ecc_err_cnt_s
{

    uint32 corrected_err_cnt_lower_channel[2];

    uint32 corrected_err_cnt_upper_channel[2];

    uint32 detected_err_cnt_lower_channel[2];

    uint32 detected_err_cnt_upper_channel[2];
} hbmc_ecc_err_cnt_t;
...
shr_error_e dnx_hbmc_cattrip_indication_disable(
    int unit,
    uint32 dram_start,
    uint32 dram_end,
    int *should_reenable_cattrip);

shr_error_e dnx_hbmc_cattrip_indication_restore(
    int unit,
    uint32 dram_start,
    uint32 dram_end,
...
shr_error_e dnx_hbmc_dram_and_channel_indexes_verify(
    int unit,
    int hbmc_ndx,
    int channel);
...
shr_error_e dnx_hbmc_ecc_enable_get(
    int unit,
    int hbm_index,
    int channel,
    uint32 *is_enabled);

shr_error_e dnx_hbmc_ecc_errors_get(
    int unit,
    int hbm_index,
    int channel,
    hbmc_ecc_err_cnt_t * error_counters);

shr_error_e dnx_hbmc_ecc_err_counters_clear(
    int unit);
