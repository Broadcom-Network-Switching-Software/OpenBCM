...
/** Allocation test, no changes to the swstate .*/
#define DNX_PORT_ESEM_CMD_EXCHANGE_TEST    (SAL_BIT(3))
...
/*
 * The ESEM command that are default for PP port configuration
 */
#define DNX_PORT_ESEM_CMD_IS_DEFAULT_PP_PORT_CONFIGURATION(esem_cmd) \
...
/**
 * \brief -
 *  Check whether the provided ESEM command profile is a predefined command or not (allocated at init stage)
 *
 * \param [in] unit - the relevant unit.
 * \param [in] esem_cmd_profile - ESEM command profile.
 * \param [out] is_predefined - bool, indicates whether the command is predefined or not (allocated at init stage).
 *
 * \return
 *  shr_error_e
 *
 ...
 *  None
 */
shr_error_e dnx_port_esem_cmd_is_predefine(
    int unit,
