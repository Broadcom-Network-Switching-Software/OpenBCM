...
#define PON_SUBPORT_ID_MASK(mode)                       ((mode == 1) ? 0xf : 0xe)
...
#define PON_CHANNEL_ID_GET(tunnel_id,mode)              (tunnel_id & 0xfff)
...
/**
 * \brief
 *   Check if there're ports have the same port type as the input port. 
 *   If the input port is invalid, then check if port type of any port is same as the port_info.
 *
 * \param [in] unit - unit ID
 * \param [in] port - port not to check. BCM_PORT_INVALID: check all ports
 * \param [in] port_info - type to check: DNX_PORT_TYPE_COE / DNX_PORT_TYPE_PON
 * \param [out] is_other_set - indicator of other ports is set.
 * \return
 *   shr_error_e
 *
 * \remark
 * \see
 */
shr_error_e dnx_port_sit_port_status_get(
    int unit,
    bcm_port_t port,
    int port_info,
    uint8 *is_other_set);
