...
#define DNX_DATA_FIELD_ENTRY_TCAM_EXTERNAL_CORE_ID_SHIFT    26
...
#define DNX_FIELD_ENTRY_STATE_TABLE_CORE_ID(entry_handle)        \
        (((((entry_handle) >> DNX_DATA_FIELD_ENTRY_STATE_TABLE_CORE_ID_SHIFT) & DNX_DATA_FIELD_ENTRY_CORE_ID_MASK)== DNX_FIELD_TCAM_CORE_ALL) ? \
         (BCM_CORE_ALL):(((entry_handle) >> DNX_DATA_FIELD_ENTRY_STATE_TABLE_CORE_ID_SHIFT) & DNX_DATA_FIELD_ENTRY_CORE_ID_MASK))

/** Get core from external TCAM (KBP). */
#define DNX_FIELD_ENTRY_TCAM_EXTERNAL_PER_CORE_CORE_ID(entry_handle)        \
        (((((entry_handle) >> DNX_DATA_FIELD_ENTRY_TCAM_EXTERNAL_CORE_ID_SHIFT) & DNX_DATA_FIELD_ENTRY_CORE_ID_MASK)== DNX_FIELD_TCAM_CORE_ALL) ? \
         (BCM_CORE_ALL):(((entry_handle) >> DNX_DATA_FIELD_ENTRY_TCAM_EXTERNAL_CORE_ID_SHIFT) & DNX_DATA_FIELD_ENTRY_CORE_ID_MASK))
 ...
 * ACCESS_ID        (20 bits)
 */
#define DNX_FIELD_ENTRY_TCAM_INTERNAL(access_id)  \
        ((access_id & DNX_DATA_FIELD_ENTRY_TCAM_INTERNAL_ACCESS_ID_MASK))

/** This macro encodes the entry handle that is passed to an
 *  External TCAM field group that is per core.
 *
 ...
 * ACCESS_ID        (26 bits)
 */
#define DNX_FIELD_ENTRY_TCAM_EXTERNAL_PER_CORE(access_id, core_id)  \
        ((access_id & DNX_DATA_FIELD_ENTRY_TCAM_EXTERNAL_ACCESS_ID_MASK) | \
         ((((core_id == BCM_CORE_ALL)?(DNX_FIELD_TCAM_CORE_ALL):(core_id)) & DNX_DATA_FIELD_ENTRY_TCAM_EXTERNAL_ACCESS_ID_MASK) << DNX_DATA_FIELD_ENTRY_TCAM_EXTERNAL_CORE_ID_SHIFT))

/** This macro encodes the entry handle that is passed to an
 *  External TCAM field group that is not per core.
 *
 * EXTERNAL TCAM Entry Id is encoded in the following way (from MSB to LSB):
 * NOT USED         (6 bits)
 * ACCESS_ID        (26 bits)
 ...
 * ACCESS_ID        (12 bits)
 */
#define DNX_FIELD_ENTRY_TCAM_DT(access_id, fg_id)  \
...
#define DNX_FIELD_ENTRY_STATE_TABLE(address, core_id)  \
        (((((core_id == BCM_CORE_ALL)?(DNX_FIELD_TCAM_CORE_ALL):(core_id)) & DNX_DATA_FIELD_ENTRY_CORE_ID_MASK) << DNX_DATA_FIELD_ENTRY_STATE_TABLE_CORE_ID_SHIFT) | \
...
typedef enum
{
     ...
     */
    DNX_FIELD_ENTRY_GROUP_ADD_FLAG_WITH_ID = (0x1 << 0),
    /**
     * Entry is not added, but rather updated.
     * - For TCAM entries, key/payload/valid_bit can be updated.
     */
    DNX_FIELD_ENTRY_GROUP_ADD_FLAG_UPDATE = (0x1 << 1),
} dnx_field_entry_group_add_flags_e;
...
typedef enum
{
     ...
     * Creates entry with a user-specified ID.
     */
    DNX_FIELD_ENTRY_ACE_ADD_FLAG_WITH_ID = (0x1 << 0),
    /**
     * Entry is not added, but rather updated.
     * - Only payload can be updated
     */
    DNX_FIELD_ENTRY_ACE_ADD_FLAG_UPDATE = (0x1 << 1),
    /**
     * Do not allocate the entry key.
     */
    DNX_FIELD_ENTRY_ACE_AVOID_ALLOCATION = (0x1 << 2),
} dnx_field_entry_ace_add_flags_e;

/**
 * Holds possible flags for entry creation/updating.
 * Used in dnx_field_entry_state_table_add
 */
typedef enum
{
    /**
     * Add entry to narrow state table.
     */
    DNX_FIELD_ENTRY_STATE_TABLE_ADD_NARROW = (0x1 << 0),
} dnx_field_entry_state_table_add_flags_e;
...
/**Hold the in params for dnx_field_entry_state_table_add API*/
typedef struct
{
    /** Hold the qualifier values to be filled in address key*/
    dnx_field_entry_key_t address_info;

    /** Hold the qualifier values to be filled in data key*/
    dnx_field_entry_key_t data_info;

    /** Core to add the entry on */
    int core;

} dnx_field_entry_state_table_t;
...
/**
* \brief
*  Init structure type of dnx_field_entry_state_table_t to Invalid values
* \param [in] unit            - Device ID
* \param [in] entry_add_in_p  - Pointer to structure type of dnx_field_entry_t
* \return
*   shr_error_e - Error Type
...
*   * None
*/
shr_error_e dnx_field_entry_state_table_t_init(
    int unit,
...
/**
* \brief
* Add a new entry to indicated External TCAM data base.
* On input, specify all actions and qualifiers to go into that entry in hardware:
*    Actions will go into 'actions payload' and
...
*   * None
*/
shr_error_e dnx_field_entry_external_tcam_add(
    int unit,
    uint32 flags,
    dnx_field_group_t fg_id,
    dnx_field_entry_t * entry_info_p,
...
/**
* \brief
*  Delete External TCAM Entry From HW and free all allocated resources
* \param [in] unit          - Device ID
* \param [in] fg_id         - Entry's FG ID
* \param [in] entry_handle  - unique identifier of entry
* \return
...
*   * dnx_field_group_entry_delete_all
*/
shr_error_e dnx_field_entry_external_tcam_delete(
    int unit,
...
/**
* \brief
*  Delete ACE entry From HW and free all allocated resources
* \param [in] unit          - Device ID
* \param [in] ace_entry_key - The key of the entry to delete, used as identifier.
* \return
*   shr_error_e - Error Type
* \remark
*   * None
...
/**
* \brief
*  Get info for specific external TCAM Field group by entry id
* \param [in] unit                  - Device ID
* \param [in] fg_id                 - Entry's FG ID
* \param [in] entry_handle          - unique entry handle
...
* \param [out] entry_info_p         - Pointer to dnx_field_entry_t to get info from the HW
* \return
*   shr_error_e - Error Type
* \remark
*   * None
...
*   * None
*/
shr_error_e dnx_field_entry_external_tcam_get(
    int unit,
    bcm_field_group_t fg_id,
    uint32 entry_handle,
