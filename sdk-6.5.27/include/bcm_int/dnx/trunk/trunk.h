...
/**
 * \brief - Map between System port to SPA provided one exists. This function
 *          is working only for system ports which are local to this unit.
 *
 * \param [in] unit - unit number
 * \param [in] system_port - system port for which to search for matching SPA
 * \param [in] trunk_id - trunk id in which to search for system port
 * \param [out] system_port_aggregate - returned SPA for system port and trunk_id
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_trunk_system_port_to_spa_map_get(
    int unit,
...
/**
 * \brief - add port to trunk from pp perspective
 *
 * \param [in] unit - unit number
 * \param [in] trunk_id - trunk id
 * \param [in] member_info - member info from trunk sw_db (it is possible that the info was not added yet)
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_trunk_add_port_to_pp(
    int unit,
    bcm_trunk_t trunk_id,
    trunk_group_member_info_t * member_info);

/**
 * \brief - delete port from trunk from pp perspective
 *
 * \param [in] unit - unit number
 * \param [in] trunk_id - trunk id
 * \param [in] member_info - member info from trunk sw_db (it is possible that the info was already deleted at this point)
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_trunk_delete_port_from_pp(
    int unit,
    bcm_trunk_t trunk_id,
    trunk_group_member_info_t * member_info);
