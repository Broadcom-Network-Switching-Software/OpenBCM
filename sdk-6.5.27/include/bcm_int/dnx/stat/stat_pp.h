...
#define STAT_PP_USE_DIRECT_STAT_ID BCM_STAT_ID_DIRECT
...
#define STAT_PP_PROFILE_ENGINE_GET(_unit_, profile) \
        ...
        (dnx_data_stat.stat_pp.feature_get(unit, dnx_data_stat_stat_pp_epp_combined_interfaces)) ? \
            bcmStatCounterInterfaceEgressPp : bcmStatCounterInterfaceEgressTransmitPp \
        )
#define STAT_PP_ENGINE_PROFILE_SET(encoded_profile, profile, engine) \
    ...
    { \
        ...
        { \
            ...
            ((uint32)STAT_PP_PROFILE_IRPP_ENCODED_VALUE << STAT_PP_PROFILE_ENGINE_SHIFT); \
        } \
        else if (engine == bcmStatCounterInterfaceEgressReceivePp) \
        { \
            encoded_profile = ((profile & STAT_PP_PROFILE_ID_MASK) << STAT_PP_PROFILE_ID_SHIFT) | \
            ((uint32)STAT_PP_PROFILE_ERPP_ENCODED_VALUE << STAT_PP_PROFILE_ENGINE_SHIFT); \
        } \
        else if ((engine == bcmStatCounterInterfaceEgressTransmitPp) || \
                 (engine == bcmStatCounterInterfaceEgressPp)) \
        { \
            encoded_profile = ((profile & STAT_PP_PROFILE_ID_MASK) << STAT_PP_PROFILE_ID_SHIFT) | \
            ((uint32)STAT_PP_PROFILE_ETPP_ENCODED_VALUE << STAT_PP_PROFILE_ENGINE_SHIFT); \
        } \
    } while (0)
...
#define STAT_PP_ETPP_SWITCH_CMD_SET(stat_id,stat_pp_profile,engine_source) \
    do \
    { \
        ...
        STAT_PP_ENGINE_PROFILE_SET(stat_pp_profile, stat_pp_profile, engine_source); \
...
/**
 * \brief
 *      Enable/disable the egress L2 bridge stat metadata based on fwd plus one layer
 * \param [in] unit - unit id
 * \param [in] arg - enable or disable
 * \return
 ...
 *   NONE
 */
shr_error_e dnx_stat_pp_egress_l2_fwd_plus_one_metadata_enable_set(
    int unit,
    int arg);

/**
 * \brief
 *      get if the egress L2 bridge stat metadata based on fwd plus one layer is enabled or disabled
 * \param [in] unit - unit id
 * \param [out] arg - enable or disable
 * \return
 *   \retval Non-zero (!= _SHR_E_NONE) in case of an error
 *   \retval Zero (= _SHR_E_NONE) in case of NO ERROR
 * \remark
 ...
 *   NONE
 */
shr_error_e dnx_stat_pp_egress_l2_fwd_plus_one_metadata_enable_get(
    int unit,
    int *arg);
...
/**
 * \brief - verify function, validate that the stat profile exists before performing get or removing it.
 * \param [in] unit - relevant unit
 * \param [in] engine_source - counting source (IRPP, ERPP, ETPP)
 * \param [in] profile_id - statistic profile id
 *
 * \return
 *   Negative in case of an error. See \ref shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_stat_pp_profile_get_delete_verify(
    int unit,
