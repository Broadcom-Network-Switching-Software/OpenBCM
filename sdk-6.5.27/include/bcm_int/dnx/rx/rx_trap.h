...
/**
 * \brief - Ingress traps class is not encoded in trap_id hence should call dnx_rx_trap_ingress_class_get
 *          if Egress trap, extract class from trap_id
 */
#define DNX_RX_TRAP_ID_CLASS_GET(unit, trap_id)     ((DNX_RX_TRAP_ID_BLOCK_GET(trap_id) == DNX_RX_TRAP_BLOCK_INGRESS) ? \
                                                     (dnx_rx_trap_ingress_class_get(unit, trap_id)) : ((trap_id >> DNX_RX_TRAP_CLASS_SHIFT) & DNX_RX_TRAP_CLASS_MASK))

#define DNX_RX_TRAP_ID_TYPE_GET(trap_id)    (trap_id & DNX_RX_TRAP_TYPE_MASK)

/**
 * \brief - Get trap_action_profile (3b/4b) from egress trap_id
 */
#define DNX_RX_TRAP_EGRESS_ACTION_PROFILE_MASK (dnx_data_pp.stages.is_erpp_supported_get(unit) ? 0x7:0xF)
 ...
 *          Currently the OAM trap_type is 3 bits, but we will reserve additional 2 for future use,
 *           which means that the 4 LSB hold the oam_trap_id, the following 2 are empty buffer for future use and
 *           4 MSB for action_profile.
 * (4b - action_profile | 5b - oam_trap_type(1b empty buffer | 4b oam_trap_id)) */

#define DNX_RX_TRAP_ETPP_OAM_ACTION_PROFILE_SHIFT    (5)
#define DNX_RX_TRAP_ETPP_OAM_ACTION_PROFILE_MASK     (0xF)

#define DNX_RX_TRAP_ETPP_OAM_TRAP_TYPE_MASK          (0x1F)
...
/**
 * \brief - return system headers mode
 */
#define DNX_RX_TRAP_SYSTEM_HEADERS_MODE_GET(unit) (dnx_data_headers.system_headers.system_headers_mode_get(unit))
...
typedef enum
{
    ...
    DNX_RX_TRAP_ETH_PROFILE_VTT_MY_MAC_AND_UNKNOWN_L3 = 3,
    DNX_RX_TRAP_ETH_PROFILE_VTT_MY_MAC_AND_ARP = 4
...
typedef enum
{
    DNX_RX_TRAP_ETPP_LIF_PROFILE_INVALID = -1,
    DNX_RX_TRAP_ETPP_LIF_PROFILE_DEFAULT = 0,
    DNX_RX_TRAP_ETPP_LIF_PROFILE_STACK_TRAP_1 = 1,
    DNX_RX_TRAP_ETPP_LIF_PROFILE_STACK_TRAP_2 = 2,
    DNX_RX_TRAP_ETPP_LIF_PROFILE_STACK_MIRROR = 3,
    DNX_RX_TRAP_ETPP_LIF_NOF_PROFILES = 4
} dnx_rx_trap_etpp_lif_profiles_e;
...
typedef enum
{
    ...
    /** OAM ETPP trap LMR/DMR to OAMP with priority */
    DNX_RX_TRAP_ETPP_OAM_UP_MEP_OAMP_PERFORMENCE_RESPONSE_PRIO = 7,
    /** OAM ETPP trap LMR/DMR to OAMP */
    DNX_RX_TRAP_ETPP_OAM_UP_MEP_OAMP_PERFORMENCE_RESPONSE = 8,
...
*   * None
*/
shr_error_e dnx_rx_trap_ingress_ud_name_get(
    int unit,
    int trap_type,
    uint8 *trap_name);
...
*   * None
*/
dnx_rx_trap_class_e dnx_rx_trap_ingress_class_get(
    int unit,
    int trap_type);

/**
* \brief
*  Returns the forward action profile from trap id for epmf trap action use
* \par DIRECT INPUT:
*   \param [in] unit       -  Unit Id
*   \param [in] trap_id    -  Trap Id
*   \param [out] forward_action_profile_p    -  Forward action profile
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e - Error Type
...
*   * None
*/
shr_error_e dnx_rx_trap_epmf_forward_action_profile_from_id_get(
    int unit,
    int trap_id,
...
*   * None
*/
void dnx_rx_trap_ingress_action_with_flag_and_value_hw_set(
    int unit,
...
*   * None
*/
void dnx_rx_trap_ingress_action_with_flag_hw_set(
    int unit,
    uint32 entry_handle_id,
    int flags,
    int bcm_rx_trap_flag,
...
/**
* \brief
*  Clear LIF trap action profile in HW: trap strengths and profile number.
* \par DIRECT INPUT:
*   \param [in] unit      -  Unit Id
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_ingress_action_profile_hw_clear(
    int unit,
    uint32 lif_trap_profile);

/**
* \brief
*  Allocate and Set the Ingress LIF trap Profile
* \par DIRECT INPUT:
*   \param [in] unit        - unit Id
...
*   \param [out] is_last_p - indicate if this is the last LIF pointing to old_lif_trap_profile
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e - Error type
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_ingress_trap_profile_allocate_and_hw_update(
    int unit,
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_etpp_trap_action_profile_hw_clear(
    int unit,
    uint32 lif_trap_profile);
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_etpp_profile_lif_set(
    int unit,
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_ingress_profile_set(
    int unit,
    bcm_gport_t lif_gport,
    dnx_algo_gpm_gport_hw_resources_t * lif_hw_resources_p,
    bcm_gport_t action_gport);
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_etpp_profile_lif_get(
    int unit,
    bcm_gport_t lif_gport,
    dnx_algo_gpm_gport_hw_resources_t * lif_hw_resources_p,
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_ingress_profile_get(
    int unit,
    bcm_gport_t lif_gport,
    dnx_algo_gpm_gport_hw_resources_t * lif_hw_resources_p,
    bcm_gport_t * action_gport_p);
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_mtu_profile_hw_set(
    int unit,
...
*   * None
*/
shr_error_e dnx_rx_trap_lif_mtu_profile_hw_get(
    int unit,
    dbal_tables_e dbal_table,
    dbal_fields_e dbal_key_field_type,
    uint32 outlif_dbal_result_type,
