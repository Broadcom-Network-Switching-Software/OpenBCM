 ...
 *          it toggles between stateless and regular contexts
 *
 ...
 */
shr_error_e dnx_oam_twamp_stateless_mode_set(
    int unit,
    int value);

/**
 * \brief - API gets the the stateless mode
 *
 *   \param [in] unit - unit ID
 *    \param [in] value - the value to get above modes - 1 or 0.
 *
 * \return
 *   \retval shr_error_e - Negative in case of an error, zero in case all ok.
 */
shr_error_e dnx_oam_twamp_stateless_mode_get(
    int unit,
    int *value);
