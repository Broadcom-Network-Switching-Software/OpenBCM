...
#define DNX_SAT_EVC_ID_MAX                  DNX_DATA_MAX_SAT_GENERATOR_NOF_EVC_IDS
#define DNX_SAT_GTF_ID_IS_VALID_EVC_ID(id) \
    ((((id) >> 1) < DNX_SAT_EVC_ID_MAX) && (((id) & 0x1) == 0))
...
#define DNX_SAT_DEFAULT_CREDIT_BACKPRESSURE (9)
...
#define DNX_SAT_PKT_HERDER_SIZE             DNX_DATA_MAX_SAT_GENERATOR_GTF_PKT_HEADER_MAX_LENGTH
...
#define DNX_SAT_CTF_TRAP_ID_MAX             (0x1FF)
...
#define DNX_AM_TEMPLATE_SAT_PKT_HEADER_COUNT       (DNX_DATA_MAX_SAT_GENERATOR_NOF_GTF_IDS + 1)
...
extern bcm_sat_event_cb _g_dnx_sat_event_cb[BCM_MAX_NUM_UNITS][bcmSATEventCount];

/* User data */
extern void *_g_dnx_sat_event_ud[BCM_MAX_NUM_UNITS][bcmSATEventCount];
...
{
    uint8 pkt_header[DNX_SAT_PKT_HERDER_SIZE];
 ...
 * {
 */
#define DNX_SAT_IS_SUPPORTED_CHECK(unit) \
    if (!dnx_data_sat.general.feature_get(unit, dnx_data_sat_general_is_sat_supported)) \
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "SAT is not supported on this device")
...
shr_error_e dnx_am_template_sat_pkt_header_length_get(
    int unit,
    int header_template,
    uint32 *length);
...
shr_error_e dnx_sat_pkt_header_mem_allocate(
    int unit,
    int nof_dwords,
    int *pkt_header_base);

shr_error_e dnx_sat_pkt_header_mem_free(
    int unit,
    int nof_dwords,
    int pkt_header_base);
...
shr_error_e dnx_sat_dynamic_memory_access_get(
    int unit,
    uint32 *enable);
