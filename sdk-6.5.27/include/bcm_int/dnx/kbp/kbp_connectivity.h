...
#define DNX_KBP_INST_ITER(unit, kbp_inst) \
        for (kbp_inst = 0; \
        kbp_inst < (dnx_data_elk.general.device_type_get(unit) == DNX_KBP_DEV_TYPE_BCM52321_BCM5235 ? DNX_KBP_MAX_NOF_INST : (DNX_KBP_IS_SECOND_KBP_EXIST ? DNX_OP2_MAX_NOF_INST : 1)); \
        kbp_inst++)

#define DNX_KBP_INST_MASTER 0
#define DNX_KBP_INST_SLAVE  1
#define DNX_KBP_INST_TAP 2
...
#define DNX_KBP_OP2_IS_SINGLE_PORT_MODE(unit) \
    ((dnx_data_elk.connectivity.connect_mode_get(unit) == DNX_NIF_ELK_TCAM_CONNECT_MODE_SINGLE_PORT) || \
    (dnx_data_elk.connectivity.connect_mode_get(unit) == DNX_NIF_ELK_TCAM_CONNECT_MODE_SINGLE_PORT_SMT))
...
typedef enum
{
    ...
    /*
     *  OP2 Single Port mode with SMT enabled for stats
     */
    DNX_NIF_ELK_TCAM_CONNECT_MODE_SINGLE_PORT_SMT = 4,
    /*
     *  Number of types in ARAD_NIF_ELK_TCAM_DEV_TYPE
     */
    DNX_NIF_ELK_TCAM_NOF_CONNECT_MODE = 5
