...
#define DNX_POLICER_INGRESS_COMPENSATION_DB0_TEMPLATE_NAME  "meter ingress compensation database-0 profiles"
#define DNX_POLICER_INGRESS_COMPENSATION_DB1_TEMPLATE_NAME  "meter ingress compensation database-1 profiles"
#define DNX_POLICER_INGRESS_COMPENSATION_DB2_TEMPLATE_NAME  "meter ingress compensation database-2 profiles"
#define DNX_POLICER_EGRESS_COMPENSATION_DB0_TEMPLATE_NAME   "meter egress compensation database-0 profiles"
#define DNX_POLICER_EGRESS_COMPENSATION_DB1_TEMPLATE_NAME   "meter egress compensation database-1 profiles"
...
#define DNX_POLICER_DEFAULT_CMD_ID (-1)
...
#define POLICER_COMPENSATION_MNGR_TM_CB(is_ingress, database_id, exec) ( \
        (is_ingress && (database_id == 0))         ? dnx_meter_db.policer_compensation_templates.policer_ingress_db0.exec   :\
        ((is_ingress && (database_id == 1))        ? dnx_meter_db.policer_compensation_templates.policer_ingress_db1.exec   :\
        ((is_ingress && (database_id == 2))       ? dnx_meter_db.policer_compensation_templates.policer_ingress_db2.exec   :\
        ((!is_ingress && (database_id == 0))     ? dnx_meter_db.policer_compensation_templates.policer_egress_db0.exec    :\
                                                  dnx_meter_db.policer_compensation_templates.policer_egress_db1.exec    ))))
...
{
    int header_delta;
} dnx_meter_compensation_profile_info_t;
...
*/
    ...
    /*
     * Indicate which compensation profile this meter uses
     */
...
/**
 * \brief
 *      callback print function for template manager. print the meter compensation profile
 * \param [in] unit -unit id
 * \param [in] data -const pointer to profile data
 * \return
 *  void
 ...
 *   NONE
 */
void dnx_policer_mgmt_tm_meter_compensation_profile_print_cb(
    int unit,
    const void *data);
