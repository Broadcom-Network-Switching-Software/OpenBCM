...
typedef struct rx_ctl_s {
    ...
    rx_chan_ctl_t     chan_ctl[BCM_RX_CHANNELS_MAX]; /* internal chan ctl */
...
#define FOREACH_RUNNING_CHANNEL(unit, chan) \
    for (chan = 0; chan < BCM_RX_CHANNELS_MAX; chan++) \
...
#define RX_THREAD_EXIT_COMPLETE_GET() \
    rx_control.thread_exit_complete
#define RX_THREAD_EXIT_COMPLETE_SET(value) \
    rx_control.thread_exit_complete = value
