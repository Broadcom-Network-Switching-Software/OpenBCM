...
typedef int (*_bcm_bst_hw_stat_clear_t)(int unit,
                                        ...
                                        int index);
typedef int (*_bcm_bst_hw_stat_get)(int unit, int port, int index, int buffer,
...
typedef struct _bcm_bst_cmn_unit_info_s {
    ...
    _bcm_bst_hw_stat_get            hw_stat_get;
...
#define _BCM_BST_RESOURCE_STAT_INIT_64(pres, num, rname)                       \
    do {                                                                    \
        if ((pres)->p_stat == NULL) {                                       \
            (pres)->p_stat = sal_alloc((num)*sizeof(uint64), (rname));      \
            if ((pres)->p_stat == NULL) {                                   \
                return (BCM_E_MEMORY);                                      \
            }                                                               \
        }                                                                   \
        sal_memset((pres)->p_stat, 0, sizeof(uint64)*(num));                \
        (pres)->num_stat_pp = (num);                                        \
    } while (0);
