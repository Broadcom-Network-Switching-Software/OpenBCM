...
typedef int (*bcmi_l3_iif_update_callback_t)(int unit, bcm_if_t intf_id,
                                             bcm_vrf_t vrf, void *user_data);
extern int bcmi_esw_l3_iif_update_cb_register(int unit,
                                       bcmi_l3_iif_update_callback_t callback,
                                       void *user_data);
extern int bcmi_esw_l3_iif_update_cb_unregister(int unit);
extern int bcmi_esw_l3_iif_update_callback(int unit, bcm_if_t intf_id,
                                           bcm_vrf_t vrf);
extern void bcmi_esw_l3_dependent_cleanup(int unit);
extern int bcmi_l3_init_check(int unit);
extern void bcm_xgs3_tnl_init_ref_cnt_incr(int unit, unsigned idx);
extern void bcm_xgs3_tnl_init_ref_cnt_decr(int unit, unsigned idx);
extern void bcm_xgs3_tnl_init_ref_cnt_get(int unit, unsigned idx, uint32 *ref_count);
...
typedef struct _bcm_l3_bookkeeping_s {
    ...
    int      ecmp_defrag_count;          /* Total number of times defragmentation operation
                                            has been performed on the single-level ECMP member */
    int      overlay_ecmp_defrag_count;  /* Total number of times defragmentation operation
                                            has been performed on the overlay ECMP member */
    int      underlay_ecmp_defrag_count; /* Total number of times defragmentation operation
                                            has been performed on the underlay ECMP member */
...
#ifdef BCM_RIOT_SUPPORT
#define _BCM_L3_RIOT_ALLOC_MODE_UNDERLAY_FIRST               (1UL << 0)
#define _BCM_L3_RIOT_ALLOC_MODE_DYNAMIC_BANK                 (1UL << 1)
...
#endif
    sal_mutex_t l3_iif_update_mutex; /* Mutex for l3_iif_update cb and data. */
    bcmi_l3_iif_update_callback_t l3_iif_update_cb;
    void *l3_iif_update_user_data;
...
#define BCM_L3_IIF_UPDATE_LOCK(_u_) \
        sal_mutex_take(_bcm_l3_bk_info[_u_].l3_iif_update_mutex,\
                       sal_mutex_FOREVER)
#define BCM_L3_IIF_UPDATE_UNLOCK(_u_) \
        sal_mutex_give(_bcm_l3_bk_info[_u_].l3_iif_update_mutex)
#define BCM_L3_IIF_UPDATE_CALLBACK(_u_) (_bcm_l3_bk_info[_u_]).l3_iif_update_cb
#define BCM_L3_IIF_UPDATE_USER_DATA(_u_) \
            (_bcm_l3_bk_info[_u_]).l3_iif_update_user_data
...
#define _BCM_L3_DUMP_BITMAP_INTF    (1UL << 0)
#define _BCM_L3_DUMP_BITMAP_DEFIP   (1UL << 1)
#define _BCM_L3_DUMP_BITMAP_HOST    (1UL << 2)
#define _BCM_L3_DUMP_BITMAP_ECMP    (1UL << 3)
#define _BCM_L3_DUMP_BITMAP_NH      (1UL << 4)
#define _BCM_L3_DUMP_BITMAP_TUNNEL  (1UL << 5)
#define _BCM_L3_DUMP_BITMAP_ALPM    (1UL << 6)
#define _BCM_L3_DUMP_BITMAP_DGM     (1UL << 7)

extern void _bcm_l3_sw_dump(int unit, uint32 l3_sw_dump_bitmap);
...
#define BCMI_L3_INTF_MEM_ALLOC_MODE_IS_UNDERLAY_FIRST(_u) \
    ((BCMI_RIOT_IS_ENABLED(_u)) &&            \
    ((_bcm_l3_bk_info[_u].l3_intf_overlay_alloc_mode) & _BCM_L3_RIOT_ALLOC_MODE_UNDERLAY_FIRST))
#define BCMI_L3_INTF_MEM_ALLOC_MODE_IS_DYNAMIC(_u) \
    ((BCMI_RIOT_IS_ENABLED(_u)) &&            \
    ((_bcm_l3_bk_info[_u].l3_intf_overlay_alloc_mode) & _BCM_L3_RIOT_ALLOC_MODE_DYNAMIC_BANK))
#define BCMI_L3_NH_MEM_ALLOC_MODE_IS_UNDERLAY_FIRST(_u) \
    ((BCMI_RIOT_IS_ENABLED(_u)) &&            \
    ((_bcm_l3_bk_info[_u].l3_nh_overlay_alloc_mode) & _BCM_L3_RIOT_ALLOC_MODE_UNDERLAY_FIRST))
#define BCMI_L3_NH_MEM_ALLOC_MODE_IS_DYNAMIC(_u) \
    ((BCMI_RIOT_IS_ENABLED(_u)) &&            \
    ((_bcm_l3_bk_info[_u].l3_nh_overlay_alloc_mode) & _BCM_L3_RIOT_ALLOC_MODE_DYNAMIC_BANK))
#define BCMI_L3_NH_RESERVE_FOR_ECMP(_u)  (_bcm_l3_bk_info[_u].l3_nh_reserve_for_ecmp)
