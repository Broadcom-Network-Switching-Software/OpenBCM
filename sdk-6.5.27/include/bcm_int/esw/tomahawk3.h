...
/* The following definitions to be used with
 * _bcm_th3_l2_wb_recovery_pause_resume_cb(). Values must be same as
 * SOC_L2X_WB_RECOVERY_DB_REBUILD_* macros
 */
#define _BCM_L2_WB_RECOVERY_ISSUE_CALLBACKS  (1U << 0) /* bit 0 */
#define _BCM_L2_WB_RECOVERY_PAUSE_CALLBACKS  (1U << 1) /* bit 1 */
#define _BCM_L2_WB_RECOVERY_RESUME_CALLBACKS (1U << 2) /* bit 2*/
...
extern int _bcm_th3_l2_wb_recovery_pause_resume_cb(int unit, int action);
extern void _bcm_th3_l2_register_callback(int unit,
                                          int pflags,
                                          l2x_entry_t *entry_del,
                                          l2x_entry_t *entry_add,
                                          void *fn_data);
extern void _bcm_th3_l2_pending_learn_callback(int unit, soc_l2_pending_learn_info_t *info, void *data);
...
#if defined(BCM_WARM_BOOT_SUPPORT)
extern int
_field_recover_th3_missing_extractor_codes(int unit);
#endif
...
extern int _bcm_th3_mpath_validate(int unit, int grp_id);
...
extern int _bcm_th3_get_pool_total (int unit, int itm, int *sum_pool_limit);
/* WRED API's */
...
extern int bcm_th3_port_egress_meter_hw_set(int unit, bcm_port_t port,
                                            uint32 refresh, uint32 thd_sel);
extern int bcm_th3_port_egress_meter_hw_get(int unit, bcm_port_t port,
                                            uint32 *refresh, uint32 *thd_sel);
...
#if defined(BCM_WARM_BOOT_SUPPORT)
extern int _bcm_th3_tunnel_reinit(int unit, uint8 **tnl_scache, int offset);
extern int _bcm_th3_tunnel_warm_boot_alloc(int unit, uint32 *sz);
extern int _bcm_th3_tunnel_sync(int unit, uint8 **tnl_scache, int offset);
#endif
