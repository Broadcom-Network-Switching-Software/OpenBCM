...
#define BCMI_FT_STATE_TRANSITION_ENTRY_INSTALLED        (1 << 0)

typedef struct bcmi_ft_state_transition_entry_s {
    /* Flags */
    uint8 flags;
    ...
    /* Check bitmap */
    SHR_BITDCL *check_bmp;

    /* Aggregate ALU32 profile */
    uint32 agg_alu32_ref_cnt[TOTAL_GROUP_AGG_ALU32_MEM];
#endif /* BCM_FLOWTRACKER_V3_SUPPORT */
} bcmi_ft_state_transition_entry_t;

typedef struct bcmi_ft_state_transition_info_s {
    /* Number of entries */
    uint32 num_entries;

    /* Entry */
    bcmi_ft_state_transition_entry_t **entry_arr;
} bcmi_ft_state_transition_info_t;

extern bcmi_ft_state_transition_info_t state_trans_info[BCM_MAX_NUM_UNITS];

#define BCMI_FT_STATE_TRANSITION_INFO(_unit)                \
    (&state_trans_info[(_unit)])

#define BCMI_FT_STATE_TRANSITION_SW_ENTRY_ARR(_unit)        \
    (BCMI_FT_STATE_TRANSITION_INFO(_unit)->entry_arr)

#define BCMI_FT_STATE_TRANSITION_NUM_ENTRIES(_unit)         \
    (BCMI_FT_STATE_TRANSITION_INFO(_unit)->num_entries)

#define BCMI_FT_STATE_TRANSITION_SW_ENTRY(_unit, _idx)      \
    (BCMI_FT_STATE_TRANSITION_INFO(_unit)->entry_arr[(_idx)])
...
typedef enum bcmi_ft_group_param_type_e {
    ...
    bcmiFtGroupParamTypeFlowState = 13
...
#define BCMI_FT_GROUP_TYPE_GET(_id_)                                    \
        (((_id_) >> BCM_FLOWTRACKER_GROUP_TYPE_SHIFT) & 0xF)
...
extern int
bcmi_ftv3_flow_transition_group_map_check(int unit,
        bcm_flowtracker_group_t id);
...
extern int
bcmi_ftv3_flex_state_transition_alu_trigger_set(int unit,
        bcm_flowtracker_check_t check_id,
        int index,
        int set);
...
extern int
bcmi_ft_group_check_ft_type_get(int unit,
        bcm_flowtracker_group_type_t group_type,
