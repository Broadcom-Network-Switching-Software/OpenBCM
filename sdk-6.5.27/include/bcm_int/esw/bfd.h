...
typedef struct bcm_esw_bfd_drv_s {
    ...
    int (*init)(int unit);
    /* Detach BFD module */
    int (*detach)(int unit, uint8 before_init);
    /* Create BFD endpoint */
...
/* Receive l3iif to vrf mapping notification from
 *  * L3 and send it to BFD FW eAPP */
extern int bcmi_xgs5_iif_update_callback(
    int unit,
    bcm_if_t intf_id,
    bcm_vrf_t vrf,
    void *user_data);
