...
typedef enum bcmi_keygen_ext_attr_s {
    ...
    /* Extractor cannot be used if LTID is part of qualifier
     * that needs to be extraced. */
    BCMI_KEYGEN_EXT_ATTR_NOT_WITH_PMUX_LTID,
    /* Extractor is marked as reversed for alignment and
     * cannot be allocated anymore in current extraction cycle.
     */
...
typedef struct bcmi_keygen_cfg_s {
    ...
    /*
     * Information about the qualifiers already present in group
     * before updating the QSET
     */
    bcmi_keygen_qual_info_t *prev_qual_info_arr;
    /* size of prev_qual_info_arr array */
    uint16 prev_qual_info_count;
} bcmi_keygen_cfg_t;
...
/*
 * Regenerate the extractor codes used by the groups QSET. This function
 * is mainly used in warmboot if groups extractors are not saved in the
 * HA space.
 *
 * Caller need to fill following information.
 *
 * keygen_cgf->ext_cfg_db
 * keygen_cfg->ext_ctrl_sel_info
 * keygen_cfg->ext_ctrl_sel_info_count
 * keygen_oper->qual_offset_info
 *
