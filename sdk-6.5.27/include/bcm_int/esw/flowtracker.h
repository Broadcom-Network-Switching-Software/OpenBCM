 ...
 * This is old value prior to multiple collector support.
 * Should be used only for
 *   1) scache recovery from old SDKs.
 *   2) FT V1
 *   3) Old firmwares which do not contain multiple collector support.
 */
#define BCM_INT_FT_MAX_COLLECTORS_OLD 1

/* Collector Start Id */
#define BCM_INT_FT_COLLECTOR_START_ID 1

/* Collector End Id */
#define BCM_INT_FT_COLLECTOR_END_ID(unit) \
    (BCM_INT_FT_COLLECTOR_START_ID + (FT_INFO_GET(unit)->max_collectors) - 1)

/* Default value for flowtracker_max_collectors soc property */
#define BCM_INT_FT_DEFAULT_MAX_COLLECTORS (BCM_INT_FT_MAX_COLLECTORS_OLD)

/* Maximum number of export_profiles. */
#define BCM_INT_FT_MAX_EXPORT_PROFILES 32
...
typedef struct _bcm_int_ft_export_template_info_s {
    ...
    /* Internal collector ID used for transmission of template packet */
    uint16 coll_int_id;
...
typedef struct _bcm_int_ft_flow_group_info_s {
    ...
    /* Second collector associated with the flow group */
    int collector_id2;

    /* Second export profile associated with the flow group */
    int export_profile_id2;
    ...
    /* Maximum length of original packet to be copied to drop start export */
    int drop_export_original_pkt_max_len;
