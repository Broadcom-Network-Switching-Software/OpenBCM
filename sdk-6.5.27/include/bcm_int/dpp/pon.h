 ...
 * For Jericho device & custom_feature_pon_pp_port_mapping_bypass == 0: support 32/16 PON ports according to pon_port_ptc_size configuration
 * For Jericho below device: support 8 PON ports
 *
 */
#define _BCM_PPD_NOF_PON_PHY_PORT(unit) (_BCM_PPD_PON_PP_PORT_MAPPING_BY_PASS_IN_JER(unit) ? 64 : (1 << SOC_DPP_CONFIG(unit)->pp.pon_port_ptc_size))
...
#define _BCM_PPD_NOF_PON_CHANNEL_PROFILE(unit, nof_channels_bits) (_BCM_PPD_DEFAULT_NOF_PON_CHANNEL_PROFILE(unit)/(1<<(nof_channels_bits)))

/* Default offset of PON channel profiles */
#define _BCM_PPD_GPORT_PON_PP_PORT_CHANNEL_PROFILE_DEFAULT_OFFSET(unit) (SOC_DPP_CONFIG(unit)->pp.pon_port_ptc_size)

/*  offset of PON channel profiles */
#define _BCM_PPD_GPORT_PON_PP_PORT_CHANNEL_PROFILE_OFFSET(unit, nof_channels_bits) (_BCM_PPD_GPORT_PON_PP_PORT_CHANNEL_PROFILE_DEFAULT_OFFSET(unit)+(nof_channels_bits))
