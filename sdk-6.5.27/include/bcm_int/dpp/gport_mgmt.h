...
#define _BCM_DPP_INFLIF_MATCH_INFO_GPORT_IS_FEC             (0x4)
...
int _bcm_dpp_gport_l2_intf_calculate_port_id(int unit, int act_fec, SOC_PPC_LIF_ID global_lif_id, void *gport, int gport_type, int *gport_is_fec);
...
/*
 * Function:
 *      _bcm_dpp_lif_glem_access_entry_operation
 * Purpose:
 *       Add/remove a glem entry for outlif.
 * Parameters:
 *      unit                - (IN) Device Number
 *      global_lif_id       - (IN) Global lif to be operated.
 *      egress_lif_id       - (IN) Local inlif to be operated.
 *      is_add              - (IN) TRUE: add operation; FALSE: remove operation.
 *  
 * Returns:
 *      BCM_E_XXX
 */
...
int
_bcm_dpp_lif_glem_access_entry_operation(int unit, int global_lif_id, int egress_lif_id, int is_add);
