...
/*
 *   Name
 *     bcm_dpp_counter_voq_base_val_get
 *   Purpose
 *     Get the base_val for VOQ counting. If a counter engine already
 *     configured for VOQ counting on the core, then returns the start
 *     of its counting range.
 *     Otherwise returns the start of the current range.
 *   Arguments
 *     IN int unit = the unit number on which to operate
 *     IN int core_id = the core ID
 *     IN int range_start = the start of the current range
 *     OUT int *base_val = the base_val of VOQ counting for the current core
 *   Returns
 *     BCM_E_NONE
 *   Notes
 */
