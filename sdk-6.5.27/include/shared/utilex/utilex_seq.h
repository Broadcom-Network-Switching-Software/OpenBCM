...
#define BCM_INIT_STEP_TIME_THRESH_DEFAULT_DEVIATION     10
/*
 * Per step flag
 * {
 */
 ...
 */
#define UTILEX_SEQ_STEP_F_NEVER_SKIP_DEINIT             SAL_BIT(16)

/**
 * \brief
 * Steps will use DMA for writing to memories or registers
 */
#define UTILEX_SEQ_STEP_F_USE_DMA                       SAL_BIT(17)
/**
 * @brief 
 * Steps that should be called on exiting the device.
 */
#define UTILEX_SEQ_STEP_F_MANDATORY_DEINIT              SAL_BIT(18)
...
{
  /**
   * Step memory allocation excluding resources allocated for SW state or otherwise ignored.
   */
    int non_swstate_or_ignored_memory_allocation;
...
typedef struct
{
    ...
    /**
     * Time - down_deviation_percentages
     */
    uint32 computed_thresh_down[UTILEX_SEQ_NOF_FAILED_STEPS];
    /**
     * Time + up_deviation_percentages
     */
    uint32 computed_thresh_up[UTILEX_SEQ_NOF_FAILED_STEPS];
    /*
     * Time + non-perc Up deviation
     */
    uint32 computed_thresh_non_perc_up[UTILEX_SEQ_NOF_FAILED_STEPS];
} utilex_seq_time_test_t;

/*
 * \brief
 * structure per step_id. It will be filed with dnx data before running
 * the init test
 */
typedef struct
{
    uint32 step_time;
    uint32 test_sensitivity;
    uint8 down_deviation;
    uint8 up_deviation;
    uint32 non_perc_up_deviation;
} utilex_step_time_dnx_data;
...
/**
 * \brief
 * Pointer to function which increment the dma init step enabled counter.
 */
typedef shr_error_e(
    *utilex_seq_sbusdma_desc_init_step_enabled_counter_inc_cb_f) (
    int unit);

/**
 * \brief
 * Pointer to function which decrement the dma init step enabled counter.
 */
typedef shr_error_e(
    *utilex_seq_sbusdma_desc_init_step_enabled_counter_dec_cb_f) (
    int unit);
...
typedef struct
{
   ...
   */
    ...
    */
     ...
     */
    shr_thread_manager_handler_t active_threads[UTILEX_SEQ_MAX_NOF_THREADS];
    /**
     ...
     * Pointer to function which increment the dma init step enabled counter.
     */
    utilex_seq_sbusdma_desc_init_step_enabled_counter_inc_cb_f desc_init_step_enabled_counter_inc;
    /**
     * Pointer to function which decrement the dma init step enabled counter.
     */
    utilex_seq_sbusdma_desc_init_step_enabled_counter_dec_cb_f desc_init_step_enabled_counter_dec;
    /**
     * Indicates that only the mandatory deinit steps will be runned.
     */
    int mandatory_deinit;
 ...
 * \param [in] parse_time_cb - pointer to a that function parsing time
 * \param [in] step_id_dnx_data - dnx data values per step_id
 *
 * \return
 *   See shr_error_e
 * \remark
 ...
 *   * None
 */
shr_error_e utilex_seq_step_is_skipped_get(
    int unit,
    utilex_seq_step_t * step_list,
    int step_id,
    int *is_found,
