...
/* Use memory cache for reading if available, ignore force_read_through.
 * SOC_MEM_DONT_USE_CACHE will have precedence. */
#define SOC_MEM_IGNORE_FORCE_READ_THROUGH 0x20
...
#define SOC_MEM_WRITE_INCR_NOADD   0x20  /* If set do not add to the address for each increment (Could be used for fifo)*/
...
#define soc_mem_index_max(unit, mem)      (SOC_PERSIST(unit) ? \
...
extern int soc_mem_array_read_extended(int unit, uint32 flags, 
...
int soc_is_mem_framer(int unit, soc_mem_t mem);
