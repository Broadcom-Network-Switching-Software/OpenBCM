...
typedef struct soc_cm_device_info_s {
    ...
    soc_chip_e      chip_enum; /** used to identify the chip type or family */
    const char     *dev_name;
...
typedef struct soc_cm_composite_dev_s {
    ...
    soc_cm_dev_id sub_units[SOC_MAX_NOF_COMPOSITE_SUB_DEVS]; /* sub units - indices in soc_cm_device */
...
typedef struct soc_cm_device_vectors_s {
    ...
    /*
     * Purpose:   read and writes 64 bit values from/to
     *            the device BAR containing CMIC.
     *
     * Parameters:
     *            dev     - The device handle (from cm_device_create())
     *            addr    - Address offset in the BAR.
     *            data    - 64 bit value to write.
     *
     */
    uint64 (*read64)(soc_cm_dev_t *dev, uint32 addr);
    void (*write64)(soc_cm_dev_t *dev, uint32 addr, uint64 data);
...
/*
 * Purpose: Allocate or grow a HA memory pool.
 *
 ...
 * certain size. In this case, the value returned in actual_size
 * can be greater than the requested_size value.
 *
 * Parameters:
 *      pool_ctx       - Context provided during HA memory instance
 *                       initialization.
 *      requested_size - Amount of new HA memory needed.
 *      actual_size    - Actual size of added HA memory.
 *
 * Returns: Pointer to new or existing HA memory pool.
 */
typedef void *(*soc_cm_ha_mem_pool_alloc_f)(void *pool_ctx,
                                            uint32 requested_size,
                                            uint32 *actual_size);

/*
 * Purpose: Free a HA memory pool.
 *
 * A function of this type will be called by soc_cm_ha_mem_cleanup
 * for each HA memory pool in use.
 *
 * Parameters:
 *      pool_ctx    - Context provided during HA initialization.
 *      pool_ptr    - Pointer to HA pool to be freed.
 *      pool_size   - Size of HA pool to be freed.
 *
 * Return: None.
 */
typedef void (*soc_cm_ha_mem_pool_free_f)(void *pool_ctx,
                                          void *pool_ptr,
                                          uint32 pool_size);
