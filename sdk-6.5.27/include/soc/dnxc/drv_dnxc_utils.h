...
typedef enum
{
    ...
    J2C_DEVICE = FIRST_SOC_IS_DEVICE_E,
...
shr_error_e dnx_drv_soc_event_generate(
    int unit,
...
void *dnx_drv_soc_info_framer_access_drv_get(
    int unit);

void dnx_drv_soc_info_framer_access_drv_set(
    int unit,
    void *framer_access_drv);

void *dnx_drv_soc_info_flr_sw_linkscan_info_get(
    int unit);

void dnx_drv_soc_info_flr_sw_linkscan_info_set(
    int unit,
...
int dnx_drv_soc_property_port_csv_dict_str_get(
    int unit,
    char *prop_str,
    char *key_str,
    char *value_str);
...
int dnxc_drv_soc_f_all_modules_inited(
    void);

int dnxc_drv_soc_f_inited(
    void);

int dnxc_init_is_init_done_get(
    int unit);
...
int dnxc_drv_soc_dnx_drv_sbus_broadcast_config(
    int unit);
...
shr_error_e dnxc_drv_soc_block_enable_set(
    int unit);

#ifdef BCM_DNX_SUPPORT

sal_vaddr_t dnx_drv_soc_dcb_addr_get(
    int unit,
    dcb_t * dcb);

uint32 dnx_drv_soc_dcb_xfercount_get(
    int unit,
    dcb_t * dcb);

void dnx_drv_soc_dcb_hg_set(
    int unit,
    dcb_t * dcb,
    uint32 hg);
