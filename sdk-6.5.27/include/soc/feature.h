...
SHR_G_ENTRY(packet_tdm_marking), /* fabric module is supported, but no fabric link used */ \
...
SHR_G_ENTRY(wh3_plus), /* Wolfhound3 Plus device */ \
...
SHR_G_ENTRY(sw_linkscan_flr_support), /* sw linkscan flr support */ \
...
SHR_G_ENTRY(prop_ignore), /* Allows ignoring soc property default inheritance and skipping this prop val for given deivce */ \
SHR_G_ENTRY(timesync_time_capture),  /*  Time Capture from TimeSync block. */ \
SHR_G_ENTRY(nanosync_v5),  /*  Using nanosync ver5. */ \
...
SHR_G_ENTRY(l2_wb_recover_pause_cbs), /* For TH3. Used to pause/resume the callbacks generated during wb recovery when sync thread's shadow table is synced for the first time */ \
SHR_G_ENTRY(wh3_reduced_vrf), /* Feature for WH3 reduced VRF */ \
SHR_G_ENTRY(wh3_reduced_mmu_internal_buffer), /* Feature for WH3 reduced MMU internal buffer to 1.5MB */ \
SHR_G_ENTRY(wh3_reduced_ipmc), /* Feature for WH3 reduced IPMC group, replication head and list tables */ \
SHR_G_ENTRY(wh3_reduced_iuat), /* Feature for WH3 reduced IUAT */ \
SHR_G_ENTRY(wh3_reduced_utt), /* Feature for wh3 reduced UTT with 12 bank support instead of 24 */ \
SHR_G_ENTRY(vlan_ctrl_updates_l3_iif_under_4k_disable), /* vlan control updates disabled for l3_iif table size under 4096 */ \
SHR_G_ENTRY(sbus_format_v7),	/* Sbus command format version 7	   */ \
SHR_G_ENTRY(xflow_macsec_sa_table_corruption_war),	/* WAR to address the ESEC&ISEC SA_TABLE corruption during heavy decrypt traffic.*/ \
SHR_G_ENTRY(vxlan_gbp_flex),	/* Feature for VXLAN GBP flex implementation */ \
SHR_G_ENTRY(subtag_counters),	/* Counter support for Subtag PP ports */ \
SHR_G_ENTRY(link_fault_on_fabric_ports_available), /* Does the device support local/remote fault indications on fabric links */ \
SHR_G_ENTRY(flex_flowtracker_state_transition), /* Support for Flexible state transition in flowtracker */ \
SHR_G_ENTRY(ctr_collection_preempt), /* Real time counter retrieve calls preempt periodic counter collection */ \
SHR_G_ENTRY(mirror_erspan_gre_key_enable), /* Enable to use gre key for erspan */ \
SHR_G_ENTRY(l3mc_dnapt_snat), /* Support for Multicast DNAPT and SNAT updates */ \
SHR_G_ENTRY(retain_ing_pkt_otpid), /* Enable to retain outer TPID from the ingress packet */ \
SHR_G_ENTRY(ignore_schan_ecc_error), /* Ignore schan operation ecc error */ \
SHR_G_ENTRY(vxlan_evpn_vlan_aware), /* VxLAN EVPN C-VLAN aware mode lookup for VFI assignment */ \
...
extern int soc_features_bcm88830_a0(int unit, soc_feature_t feature);
...
extern int soc_features_bcm53650_a0(int unit, soc_feature_t feature);
...
extern void soc_access_feature_init(int unit);
