...
#else
#define SOC_MEM_ELEM_SKIP(unit, mem) (SOC_MEM_ARRAY_INFO(unit, mem).element_skip)
#endif
...
#ifdef BCM_ACCESS_SUPPORT
/* Is the new access initialized for the device and therefore should be used for access */
#define SOC_IS_NEW_ACCESS_INITIALIZED(unit) ACCESS_IS_INITIALIZED(unit)
#else
...
#define SOC_MAX_NUM_IDI_BLKS            (8)
#define IDI_BLOCK(unit, instance)       ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_IDI_BLKS)) ? SOC_INFO(unit).idi_block[(instance)] : (-1))
...
#define ILKN_BLOCK(unit) (SOC_INFO(unit).ilkn_block)
...
/* Alloc memory space for SOC memState during system runtime. */
extern void
soc_memstate_soc_alloc(int unit, soc_mem_t mem);

#define SOC_MEM_STATE_INIT(unit, mem) \
        soc_memstate_soc_alloc(unit, mem)
...
/* Set value to a data member of structure soc_memstate_t. */
#define SOC_MEM_STATE_MEMBER_SET(unit, mem, _member, val) \
        do { \
            SOC_MEM_STATE_INIT(unit, mem); \
            SOC_MEM_STATE(unit, mem)->_member = val; \
        } while(0)

/* Get value from a data member of structure soc_memstate_t. */
#define SOC_MEM_STATE_MEMBER_GET(unit, mem, _member, val) \
            val = SOC_MEM_STATE(unit, mem) ? \
                SOC_MEM_STATE(unit, mem)->_member : NULL;
...
typedef struct {
    ...
    int             idi_block[SOC_MAX_NUM_IDI_BLKS]; /* IDI block */
    int             crypto_block;
...
#define SOC_PSTATS_UNLOCK(unit) \
    ...
    int pvt_mon_correction_interval;        /* PVT mon SW thread interval */
#endif
...
/* Alloc memory space for SOP memState during system runtime. */
extern void
soc_memstate_sop_alloc(int unit, soc_mem_t mem);

#define SOP_MEM_STATE_INIT(unit, mem) \
            soc_memstate_sop_alloc(unit, mem)
...
/* Set value to sop_memstate_t.count */
#define SOP_MEM_STATE_COUNT_SET(unit, mem, idx, val) \
            do { \
                SOP_MEM_STATE_INIT(unit, mem); \
                SOP_MEM_STATE(unit, mem)->count[idx] = val; \
            } while(0)

/* Get value from sop_memstate_t.count */
#define SOP_MEM_STATE_COUNT_GET(unit, mem, idx, val) \
            val = SOP_MEM_STATE(unit, mem) ? \
                SOP_MEM_STATE(unit, mem)->count[idx] : 0

/* Set value to sop_memstate_t.index_max */
#define SOP_MEM_STATE_MAX_INDEX_SET(unit, mem, val) \
            do { \
                SOP_MEM_STATE_INIT(unit, mem); \
                SOP_MEM_STATE(unit, mem)->index_max = val; \
            } while(0)

 /* Get value from sop_memstate_t.index_max */
#define SOP_MEM_STATE_MAX_INDEX_GET(unit, mem, val) \
            val = SOP_MEM_STATE(unit, mem) ? \
                SOP_MEM_STATE(unit, mem)->index_max : -1
...
#if defined(SRC_SOC_COMMON_ACTIVATE_UNUSED) || defined(BCM_PETRA_SUPPORT) || defined(BCM_JERICHO_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_DNXF_SUPPORT) || defined(INCLUDE_INTR)
/* { */
...
#if defined(SRC_SOC_COMMON_ACTIVATE_UNUSED) || defined(BCM_FIREBOLT_SUPPORT) || defined(BCM_BRADLEY_SUPPORT) || defined(BCM_ESW_SUPPORT)
/* { */
...
extern void
soc_memstate_free(int unit);
extern int
soc_memstate_alloc(int unit);
