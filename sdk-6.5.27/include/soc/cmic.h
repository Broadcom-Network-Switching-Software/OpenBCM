...
#define CMICX_GEN2_N_DMA_CHAN           16

#ifdef BCM_CMICX_GEN2_SUPPORT
#undef  MAX_N_DMA_CHAN
#define MAX_N_DMA_CHAN                  16
#endif
...
extern void soc_bar_read_reg32(int unit, soc_reg_t reg, int index, uint32 *data);
extern void soc_bar_write_reg32(int unit, soc_reg_t reg, int index, uint32 data);
#endif
...
extern void soc_direct_mem_set_cache_update(int unit, int cmic_block,
                              uint32 addr, uint32 *entry_data);

/* Set link scanning calls */
typedef enum soc_linkscan_flr_timeout_e {

    socLinkscanFlrTimeoutInvalid = 0,
    socLinkscanFlrTimeout50ms    = 1, /* Timeout value 50ms  */
    socLinkscanFlrTimeout75ms    = 2, /* Timeout value 75ms  */
    socLinkscanFlrTimeout100ms   = 3, /* Timeout value 100ms */
    socLinkscanFlrTimeout150ms   = 4, /* Timeout value 150ms */
    socLinkscanFlrTimeout250ms   = 5, /* Timeout value 250ms */
    socLinkscanFlrTimeout500ms   = 6, /* Timeout value 500ms */
    socLinkscanFlrTimeoutCount        /* Timeout type count  */

} soc_linkscan_flr_timeout_t;
...
extern int soc_linkscan_flr_enable_set(int unit, int port, int enable,
                                       soc_linkscan_flr_timeout_t timeout);
extern int soc_linkscan_flr_linkup_count_get(int unit, int port, int *count);

/*!
 * \brief Prototype of FLR link up callback function.
 *
 * The callback function must be short & fast when this is called
 * by interrupt thread. Protection of shared resources should be
 * done by the callback function itself.
 *
 * \param [in] unit Unit number.
 * \param [in] linkup Logical port bitmap with FLR link up.
 *
 */
typedef void
(*soc_linkscan_flr_linkup_callback_f) (int unit, soc_pbmp_t linkup);

/* Set call for FLR linkup event. */
extern int
...
extern int soc_dcmn_miim_cl45_write(int unit, uint32 phy_id,
                              uint32 phy_reg_addr, uint16 phy_wr_data);
extern int soc_miim_read(int unit, uint32 phy_id,
...
extern int soc_dcmn_miim_cl45_read(int unit, uint32 phy_id,
                             uint32 phy_reg_addr, uint16 *phy_rd_data);
extern int soc_sbus_mdio_write(int unit, uint32 phy_id,
                               uint32 phy_reg_addr, uint32 phy_wr_data);
extern int soc_sbus_mdio_read(int unit, uint32 phy_id,
...
extern int soc_miim_modify(int unit, uint32 phy_id,
...
extern int soc_miimc45_modify(int unit, uint32 phy_id, uint8 phy_devad,
                 uint16 phy_reg_addr, uint16 phy_rd_data, uint16 phy_rd_mask);

extern int soc_rcpu_miim_write(int unit, uint32 phy_id,
                uint8 phy_reg_addr, uint16 phy_wr_data);
extern int soc_rcpu_miim_read(int unit, uint32 phy_id,
                 uint8 phy_reg_addr, uint16 *phy_rd_data);
extern int soc_rcpu_miimc45_write(int unit, uint32 phy_id, uint8 phy_devad,
                  uint16 phy_reg_addr, uint16 phy_wr_data);
extern int soc_rcpu_miimc45_read(int unit, uint32 phy_id, uint8 phy_devad,
                 uint16 phy_reg_addr, uint16 *phy_rd_data);
...
/* An array indexed by physical port to be a hardware descriptor for
 * device-specific CMICd LED chains.
 */
typedef struct {

   /* Valid bit. */
   int valid[SOC_MAX_NUM_PORTS];

   /* LED uc number. */
   int uc[SOC_MAX_NUM_PORTS];

   /* The position of port status on a LED chain. */
   int pos[SOC_MAX_NUM_PORTS];
