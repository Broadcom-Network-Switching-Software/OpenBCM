...
typedef struct { /* the structure represents a multicast replication */
    ...
    soc_gport_t port2;                  /* Second destination. */
    soc_gport_t same_if_port;           /* Same Interface Filter port. */
    soc_if_t encap1;                    /* first encapsulation. */
    soc_if_t encap2;                    /* second encapsulation. */
    soc_gport_t same_if_encap_id;       /* Same Interface Filter encapsulation ID. */
    int network_group_id; /* Split Horizon network group identifier. */
    soc_if_t failover_id;               /* Failover Object Identifier. */
