...
#define CANCUN_HGOE_HEADER_TYPE     0x8787
...
/*
 * CANCUN branch and base version mapping
 * for feature enables
*/
typedef struct {
    soc_chip_e                   chip_type;
    uint32                       branch_cancun_ver;
    uint32                       base_cancun_ver;
} cancun_branch_version_map_t;
...
#define CANCUN_VERSION_EXCLUDE_BRANCH_ID_MASK 0x00FFFFFF
...
#define SOC_CANCUN_CEH_OBJ_ENTRY_LOC_TBL(_p) ((uint32 *) \
        ((SOC_CANCUN_CEH_HASH_HDR_TBL(_p)->table_entry)) \
         + ((SOC_CANCUN_CEH_HASH_HDR_TBL(_p)->table_entry[0]) / 4))
...
#define SOC_CANCUN_VERSION(_m,_mn,_s) (((_m) << 16) | ((_mn) << 8) | \
            (_s) | (1 << 24))
#define SOC_CANCUN_FULL_VERSION(_b,_m,_mn,_s) (((_m) << 16) | ((_mn) << 8) | \
            (_s) | (_b << 24))
...
extern int _soc_cancun_bind_soc_features(int unit, uint32 cancun_ver,
                                        soc_cancun_file_branch_id_e);
extern int _soc_cancun_get_base_ver(int unit, uint32 branch_ver, uint32* base_ver,
                               int* base_valid);
#endif /* _CANCUN_H_ */
