...
typedef enum bcm_flexdigest_match_e {
    ...
    bcmFlexDigestMatchOuterL3L4HdrIfaHeader = 75, /* Zone2: Outer L3 L4 Header: IFA
                                           header. */
    bcmFlexDigestMatchOuterL2HdrGbpEthernetShim = 76, /* Zone1: Outer L2 Header: GBP Ethernet
                                           Shim */
    bcmFlexDigestMatchCount = 77        /* Always last. Not a usable value. */
...
{ \
    ...
    "OuterL2HdrGbpEthernetShim"  \
}

/* 
 * The maximum value of Flex Digest Match ID qualifiers.
 * 
 * Use this definition to indicate the maximum value of matchid
 * qualifiers. It is used to .
 */
#define BCM_FLEXDIGEST_MATCHID_QUALIFY_MAX  (bcmFlexDigestMatchCount) /* Must be >=
                                                          bcmFlexDigestMatchCount. */
...
/* Flex digest matchid qset defines the set of fields used for lookup. */
typedef struct bcm_flexdigest_matchid_qset_s {
    SHR_BITDCL w[_SHR_BITDCLSIZE(BCM_FLEXDIGEST_MATCHID_QUALIFY_MAX)]; 
} bcm_flexdigest_matchid_qset_t;

/* 
 * The flex digest matchid qualifier set operations.
 * 
 * The flex digest matchid qualifiers are collected into qualifier sets
 * (qsets)
 * which are used to allocate flex digest groups. The type
 * bcm_flexdigest_matchid_qset_t is an opaque collection of these
 * requested
 * group capabilities.
 * 
 * The following macros are used to initialize a qualifier set and add,
 * remove or test individual qualifier bits in the matchid qualifier set.
 */
#define BCM_FLEXDIGEST_MATCHID_QSET_INIT(qset)  \
    sal_memset(&(qset), 0, sizeof(bcm_flexdigest_matchid_qset_t)) /* Clear a
                                                          bcm_flexdigest_matchid_qset_t. */
#define BCM_FLEXDIGEST_MATCHID_QSET_ADD(qset, q)  \
    SHR_BITSET(((qset).w), (q)) /* Add a matchid
                                                          qualifier into
                                                          qualifier set. */
#define BCM_FLEXDIGEST_MATCHID_QSET_REMOVE(qset, q)  \
    SHR_BITCLR(((qset).w), (q)) /* Remove a matchid
                                                          qualifier from
                                                          qualifier set. */
#define BCM_FLEXDIGEST_MATCHID_QSET_TEST(qset, q)  \
    SHR_BITGET(((qset).w), (q)) /* To test whether a
                                                          matchid qualifier
                                                          exists in qualifier
                                                          set or not. */
...
/* Flex Digest Group config create API optional flags. */
#define BCM_FLEXDIGEST_GROUP_CREATE_WITH_ID (1 << 0)   /* Create an flexdigest
                                                          group with given group
                                                          Id */

/* 
 * Flex digest group configuration structure. Used to create a flex
 * digest group with specific attributes.
 */
typedef struct bcm_flexdigest_group_config_s {
    uint32 flags;                       /* Group create flags
                                           BCM_FLEXDIGEST_GROUP_CREATE_XXX. */
    int priority;                       /* Group priority. */
    bcm_flexdigest_group_t group_id;    /* Flex digest group Id. */
    bcm_flexdigest_qset_t qset;         /* Flex digest qualifier set. */
    bcm_flexdigest_matchid_qset_t matchid_qset; /* Flex digest matchid qualifier set. */
} bcm_flexdigest_group_config_t;
...
/* Initialize the flex digest group configuration structure. */
extern void bcm_flexdigest_group_config_t_init(
    bcm_flexdigest_group_config_t *group_config);

#ifndef BCM_HIDE_DISPATCHABLE

/* Create a flex digest group with provided configuration. */
extern int bcm_flexdigest_group_config_create(
    int unit, 
    bcm_flexdigest_group_config_t *group_config);

/* Get the flex digest group configuration with provided group id. */
extern int bcm_flexdigest_group_config_get(
    int unit, 
    bcm_flexdigest_group_config_t *group_config);
