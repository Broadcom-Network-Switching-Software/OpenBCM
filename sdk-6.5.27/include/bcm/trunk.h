...
#define BCM_TRUNK_PSC_WEIGHTED_PROFILE  19         /* Use Weighted profile to
                                                      choose members. */
#define BCM_TRUNK_PSC_USER_PROFILE      20         /* Use user profile id as PSC
                                                      to choose members. */
...
#define BCM_TRUNK_PSC_WITH_ID       0x40000    /* Use user's profile_id profile
                                                  with chosen PSC. */
...
#define BCM_TRUNK_FLAG_PROFILE_CHANGE_ONLY  0x80000    /* If set, only change
                                                          the profile ID during
                                                          a trunk_set routine,
                                                          without changing the
                                                          trunk's composition */
#define BCM_TRUNK_FLAG_ALL_MATCHES          0x100000   /* perform action on all
                                                          insteances that match
                                                          the criteria */
...
#define BCM_TRUNK_MEMBER_SPA_WITH_ID        0x0400     /* used to indicate
                                                          ...
                                                          id to the newly add
                                                          member of the group */
#define BCM_TRUNK_MEMBER_WITH_ID            0x0800     /* Used to indicate
                                                          whether the user
                                                          desire a specific
                                                          member_id to the newly
                                                          add member of the
                                                          group, applicable only
                                                          to certain PSCs */
#define BCM_TRUNK_MEMBER_WITH_PROTECTION    0x1000     /* Used to indicate that
...
typedef struct bcm_trunk_psc_profile_info_s {
    ...
    int profile_id;                 /* user defined profile id */
...
typedef struct bcm_trunk_info_s {
    ...
    int dlb_source_port_prune;          /* Exclude source port from DLB member
                                           selection. */
} bcm_trunk_info_t;
...
#define BCM_TRUNK_PROFILE_WITH_ID   1          /* Trunk Profile flags */

#define BCM_TRUNK_MAX_PROFILE_SIZE  512        /* Maximal number of entries in
                                                  trunk profile */
...
/* Structure describing a trunk backup member. */
typedef struct bcm_trunk_backup_member_s {
    uint32 flags;       /* BCM_TRUNK_MEMBER_xxx */
    bcm_gport_t gport;  /* Trunk member GPORT ID. */
    uint32 spa_id;      /* spa_id of this member, provided together with
                           BCM_TRUNK_MEMBER_SPA_WITH_ID flag */
} bcm_trunk_backup_member_t;

/* Structure describing a trunk member. */
typedef struct bcm_trunk_member_s {
    uint32 flags;                       /* BCM_TRUNK_MEMBER_xxx */
    bcm_gport_t gport;                  /* Trunk member GPORT ID. */
    int dynamic_scaling_factor;         /* Dynamic load balancing threshold
                                           scaling factor. */
    ...
    int dynamic_queue_size_weight;      /* Relative weight of queue size in
                                           determining member quality. */
    uint32 spa_id;                      /* spa_id of this member, provided
                                           together with
                                           BCM_TRUNK_MEMBER_SPA_WITH_ID flag */
    int member_id;                      /* member_id of this member, provided
                                           together with
                                           BCM_TRUNK_MEMBER_WITH_ID flag. */
    bcm_trunk_backup_member_t backup_member; /* backup member info, provided together
                                           with BCM_TRUNK_MEMBER_WITH_PROTECTION
                                           flag. */
} bcm_trunk_member_t;
...
extern int bcm_trunk_member_update(
    int unit, 
    bcm_trunk_t trunk_id, 
    uint32 flags, 
...
/* Trunk profile allocation struct. */
typedef struct bcm_trunk_profile_s {
    int nof_members;                    /* number of valid entries in the
                                           members array. */
    int members_array[BCM_TRUNK_MAX_PROFILE_SIZE]; /* array of members to set/retrieved
                                           from specifide profile */
} bcm_trunk_profile_t;

/* Trunk profile allocation info struct. */
typedef struct bcm_trunk_profile_info_s {
    int max_member_id;      /* max allowed member_id to be used in the members
                               array */
    int hw_entries_used;    /* number of HW entries used for the specified
                               profile */
} bcm_trunk_profile_info_t;

#ifndef BCM_HIDE_DISPATCHABLE

/* API to create a user profile for trunk PSC */
extern int bcm_trunk_profile_create(
    int unit, 
    uint32 flags, 
    bcm_trunk_profile_t *profile, 
    int *profile_id);

/* API to destory a user profile for trunk PSC */
extern int bcm_trunk_profile_destroy(
    int unit, 
    uint32 flags, 
    int profile_id);

/* API to set members to a user profile for trunk PSC */
extern int bcm_trunk_profile_set(
    int unit, 
    uint32 flags, 
    int profile_id, 
    bcm_trunk_profile_t *profile);

/* API to get members and data from a user profile for trunk PSC */
extern int bcm_trunk_profile_get(
    int unit, 
    uint32 flags, 
    int profile_id, 
    bcm_trunk_profile_t *profile, 
...
/* Set the block gport entries on a given trunk and block types. */
extern int bcm_trunk_member_gport_block_set(
    int unit, 
    bcm_trunk_member_block_key_t *key, 
    int num_gports, 
...
/* 
 * Get the block port entries on a given trunk and block type. User will
 * have to call this API first with max_gports = 0 to get the exact
 * number of elements. num_gports will return the actual number of gports
 * present in this block trunk. In the subsequent call, user can allocate
 * memory and ask for less/equal number of gports than num_gports.
 */
extern int bcm_trunk_member_gport_block_get(
    int unit, 
    bcm_trunk_member_block_key_t *key, 
    int max_gports, 
    bcm_gport_t *block_gports, 
