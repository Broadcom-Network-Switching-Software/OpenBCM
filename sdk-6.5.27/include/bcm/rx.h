...
#define BCM_RX_CHANNELS_MAX     16         /* Max number of CMIC RX channels for
                                              different CMIC versions. */
...
typedef struct bcm_rx_cfg_s {
    ...
    int max_burst;                      /* Max. packets to be received in a
                                           single burst. */
    bcm_rx_chan_cfg_t chan_cfg[BCM_RX_CHANNELS_MAX]; /* RX channel configuration. */
...
typedef enum bcm_rx_reason_e {
    ...
    bcmRxReasonSvtag = _SHR_RX_SVTAG, 
    bcmRxReasonSvtagLookupMiss = _SHR_RX_SVTAG_LOOKUP_MISS, 
    bcmRxReasonPktDropOnPortHigig3Check = _SHR_RX_PKT_DROP_ON_PORT_HIGIG3_CHECK, 
    bcmRxReasonSrv6ProcessError = _SHR_RX_SRV6_PROCESS_ERROR, 
    bcmRxReasonSrv6ValidationError = _SHR_RX_SRV6_VALIDATION_ERROR, 
    bcmRxReasonSrv6UnknownRoutingTypeWithNonZeroSl = _SHR_RX_SRV6_UNKNOWN_RT_WITH_NONZERO_SL, 
    bcmRxReasonRcpuToCpu = _SHR_RX_RCPU_TO_CPU, 
...
#define BCM_RX_TRAP_MAX_NAME_LEN    100        /* Maximum length for a name */
...
typedef enum bcm_rx_trap_e {
    ...
    bcmRxTrapOamPerformanceY1731MplsTpAcceleratedResponse = 405, /* MPLS-TP, PWE trapping of LMR/DMR to
                                           OAMP. */
    ...
    bcmRxTrapEgTxOamUpMEPOampPerformenceResponse = 408, /* ETPP trap Up MEP trapping of LMR/DMR
                                           to OAMP. */
    bcmRxTrapTwampSender = 409,         /* TWAMP Sender packets trapping to OAMP */
    bcmRxTrapOamSlmEthAccelerated = 410, /* Down MEP trapping of SLM to OAMP. */
    bcmRxTrapOamSlrEthAccelerated = 411, /* Down MEP trapping of SLR to OAMP. */
    bcmRxTrapEgTxPpmcNotFound = 412,    /* ETPP trap for cases PPMC lookup
                                           returns not found result. */
    bcmRxTrapEgTxFabricPtp = 413,       /* ETPP trap for cases of fabric ptp
                                           error. */
    bcmRxTrapEgTxDeepEditing = 414,     /* ETPP trap for cases deep editing
                                           error. */
    bcmRxTrapEgTxFfcOutOfBound = 415,   /* ETPP trap for cases EPMF returns FFC
                                           out of bound indication. */
    bcmRxTrapOamPerformanceEthAcceleratedResponsePriority = 416, /* Down MEP trapping of LMR/DMR with
                                           priority to OAMP. */
    bcmRxTrapOamPerformanceY1731MplsTpAcceleratedResponsePriority = 417, /* MPLS-TP trapping of LM/DM with
                                           priority to OAMP. */
    bcmRxTrapItmhError = 418,           /* Trap for cases of ITMH error. */
    bcmRxTrapMplsSpeculativeParseFail = 419, /* Trap for cases of MPLS termination
                                           with incorrect speculative parsing,
                                           which cannot be fixed in the parser. */
    bcmRxTrapCount = 420                /* Must be last: enum count size. */
} bcm_rx_trap_t;
...
typedef struct bcm_rx_trap_config_s {
    ...
    int fabric_header;                  /* system header profile value to
                                           override */
    uint8 name[BCM_RX_TRAP_MAX_NAME_LEN]; /* Name of Trap */
...
typedef struct bcm_rx_cosq_mapping_s {
    ...
    uint8 rcpu_operation_code;          /* Operation code in remote CPU header. */
    uint8 rcpu_operation_code_mask;     /* Operation code mask. */
...
#define BCM_RX_TRAP_FLAGS2_REPLICATION_FAILOVER_DISABLE 0x00000100 /* Disable replication
                                                          failover. */
#define BCM_RX_TRAP_FLAGS2_FALLBACK_TO_BRIDGE_DISABLE 0x00000200 /* Disable fallback to
                                                          bridge. */
#define BCM_RX_TRAP_FLAGS2_NETWORK_GROUP_DISABLE 0x00000400 /* Disable network group
                                                          filter. */
#define BCM_RX_TRAP_FLAGS2_SAME_INTERFACE_DISABLE 0x00000800 /* Disable same interface
                                                          filter. */
