...
#define BCM_OAM_OPCODE_WITH_PRIORITY        0x800000   /* LM/DM with priority */
...
typedef enum bcm_oam_encap_db_e {
    ...
    bcmOamEncapDbLmDmBfd = 1,           /* LM/DM Down MEP, BFD and TWAMP */
    bcmOamEncapDbLmDmPriorityResponse = 2, /* LM/DM response with priority Down MEP */
    bcmOamEncapDbSlm = 3,               /* SLM Down MEP */
    bcmOamEncapDbUpMep = 4,             /* Up MEP, second pass */
    bcmOamEncapDbLmDmResponse = 5,      /* LMR/DMR Down MEP */
    bcmOamEncapDbUpMepLmDmResponse = 6, /* LMR/DMR UP MEP(second pass) */
    bcmOamEncapDbSlmResponse = 7,       /* SLR Down MEP */
    bcmOamEncapDbLast = 8               /* Always the last. Not an usable value. */
} bcm_oam_encap_db_t;
...
#define BCM_OAM_LOSS_READ_AND_RESET         0x20000    /* Indicates that the
                                                          statistics will be
                                                          reset after read */
...
typedef enum bcm_oam_control_type_e {
    ...
    bcmOamControlOampInjectedNetworkQos = 40, /* Controls priority field on headers
                                           ...
                                           fields for OAM/BFD */
    bcmOamControlUpMepDmRecyclePort = 41, /* Control the Up MEP DM recycle port */
    bcmOAMControlCount = 42             /* Always Last. Not a usable value. */
...
#define BCM_OAM_PROFILE_DM_TYPE_NONE        0x40       /* Disable stamping
                                                          ...
                                                          packets. */
#define BCM_OAM_PROFILE_LM_TYPE_SLM         0x80       /* When LM is done via
                                                          SLM. */
...
/* OAM Action flags. */
#define BCM_OAM_ACTION_NONE     0x1        /* No action will be taken on applied
                                              packets. */
...
#define BCM_OAM_REFELCTOR_ENCAP_WITH_ID     0x1        /* Allocate the specified
                                                          encap id for OAM
                                                          reflector */
#define BCM_OAM_REFELCTOR_ENCAP_SVTAG_PORT_ENABLE 0x2        /* Enable SVTAG lookup
                                                          using port for OAM
                                                          reflector */
...
/* OAM primary vlan object flags */
#define BCM_OAM_PRIMARY_VLAN_UP_FACING  0x00000001 /* Specifies primary VLAN on
                                                      this endpoint faces the
                                                      switching logic */
#define BCM_OAM_PRIMARY_VLAN_WITH_ID    0x00000002 /* Add using the specified ID */

/* OAM Primary VLAN match criteria */
typedef enum bcm_oam_primary_vlan_match_e {
    BCM_OAM_PRIMARY_VLAN_MATCH_INVALID = 0, /* Illegal. */
    BCM_OAM_PRIMARY_VLAN_MATCH_PORT_VLAN = 1, /* Mod/port/trunk + VLAN. */
    BCM_OAM_PRIMARY_VLAN_MATCH_PORT_VLAN_VLAN = 2, /* Mod/port/trunk + Outer VLAN + Inner
                                           VLAN. */
    BCM_OAM_PRIMARY_VLAN_MATCH_COUNT = 3 /* Must be last. */
} bcm_oam_primary_vlan_match_t;

/* OAM Primary VLAN info type */
typedef struct bcm_oam_primary_vlan_info_s {
    bcm_oam_primary_vlan_match_t criteria; /* Match criteria. */
    uint32 flags;                       /* BCM_OAM_PRIMARY_VLAN_xxx. */
    bcm_vlan_t match_vlan;              /* Outer VLAN ID to match. */
    bcm_vlan_t match_inner_vlan;        /* Inner VLAN ID to match. */
    bcm_gport_t match_port;             /* Gport: local or remote Physical or
                                           logical gport. */
    bcm_gport_t primary_vlan_port_id;   /* GPORT identifier. */
} bcm_oam_primary_vlan_info_t;

/* Initialize a bcm_oam_primary_vlan_info_t structure. */
extern void bcm_oam_primary_vlan_info_t_init(
    bcm_oam_primary_vlan_info_t *primary_vlan_info);

#ifndef BCM_HIDE_DISPATCHABLE

/* Create an OAM primary vlan object. */
extern int bcm_oam_primary_vlan_create(
    int unit, 
    bcm_oam_primary_vlan_info_t *primary_vlan_info);

/* Get an OAM primary vlan object. */
extern int bcm_oam_primary_vlan_get(
    int unit, 
    bcm_oam_primary_vlan_info_t *primary_vlan_info);

/* Destroy an OAM primary vlan object. */
extern int bcm_oam_primary_vlan_destroy(
    int unit, 
    bcm_oam_primary_vlan_info_t *primary_vlan_info);

#endif /* BCM_HIDE_DISPATCHABLE */

/* OAM primary vlan traverse callback function prototype */
typedef int (*bcm_oam_primary_vlan_traverse_cb)(
    int unit, 
    bcm_oam_primary_vlan_info_t *primary_vlan_info, 
    void *user_data);
...
/* Traverse routine for oam primary VLAN. */
extern int bcm_oam_primary_vlan_traverse(
    int unit, 
    bcm_oam_primary_vlan_traverse_cb trav_fn, 
    void *user_data);
