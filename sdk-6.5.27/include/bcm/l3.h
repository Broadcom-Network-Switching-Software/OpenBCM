...
/* L3 module flags3. */
#define BCM_L3_FLAGS3_DO_NOT_CUT_THROUGH    (1 << 0)   /* Enable to indicate the
                                                          packet is not eligible
                                                          for cut-through. */
#define BCM_L3_FLAGS3_EGRESS_CASCADED       (1 << 1)   /* Enable to indicate the
                                                          egress object in
                                                          cascaded mode. */
#define BCM_L3_FLAGS3_DATA_MODE_FULL        (1 << 2)   /* Specify data mode of
                                                          an ALPM route entry as
                                                          full. */
#define BCM_L3_FLAGS3_DATA_MODE_REDUCED     (1 << 3)   /* Specify data mode of
                                                          an ALPM route entry as
                                                          reduced. */
#define BCM_L3_FLAGS3_FORTE_ROUTE           (1 << 4)   /* Specify L3 using
                                                          FORTE(Flexible On-chip
                                                          Route & Tunnel
                                                          Expansion) table. */
#define BCM_L3_FLAGS3_L2_TAG_ONLY           (1 << 5)   /* L2 tag nexthop. */
#define BCM_L3_FLAGS3_VRF_VALID             (1 << 6)   /* Indicate l3a_vrf is
                                                          valid for route
                                                          delete-all operations. */
#define BCM_L3_FLAGS3_AGGREGATED_HOST_ROUTE (1 << 7)   /* Indicate the route
                                                          entry is for
                                                          aggregated host route. */
#define BCM_L3_FLAGS3_FIELD_ONLY_ROUTE      (1 << 8)   /* Specific for Field
                                                          Nexthop. Enables L3
                                                          ROUTE modification on
                                                          Egress. */
#define BCM_L3_FLAGS3_EGRESS_SVTAG_PORT_ENABLE (1 << 9)   /* Enable the SVTAG
                                                          lookup using port, for
                                                          ARP plus AC
                                                          application. */
#define BCM_L3_FLAGS3_MC_FORWARD_TYPE_UC_ROUTE (1 << 10)  /* Multicast forwarding
                                                          type is L3 unicast and
                                                          all multicast copies
                                                          share one destination
                                                          MAC. */
#define BCM_L3_FLAGS3_TRACE_EVENT_SAME_ING_EGR_INTF_OVERRIDE (1 << 11)  /* Override ingress trace
                                                          event
                                                          bcmPktTraceEventIngressL3IifEqL3Oif. */
#define BCM_L3_FLAGS3_ES_ID_INSERT          (1 << 12)  /* Enable ethernet
                                                          segment ID insertion. */
#define BCM_L3_FLAGS3_ES_FILTER             (1 << 13)  /* Enable ethernet
                                                          segment filter. */
...
#define BCM_L3_MULTICAST_L2_COPY            (1 << 9)   /* L3 multicast L2 copy */
#define BCM_L3_MULTICAST_L3_DNAT            (1 << 10)  /* L3 multicast DNAT */
#define BCM_L3_MULTICAST_L3_TYPE_NAPT       (1 << 11)  /* Used along with
                                                          BCM_L3_MULTICAST_L3_DNAT
                                                          to indicate NAT/NAPT
                                                          flow. 0: NAT, 1: NAPT */
#define BCM_L3_MULTICAST_L3_SNAT_DISABLE    (1 << 12)  /* Override L3 multicast
                                                          SNAT/SNAPT Ctrl from
                                                          ingress */
...
#define BCM_L3_INGRESS_IP6_L3_MCAST_L2      (1U << 31) /* Set bridge V6MC
                                                          ...
                                                          IPMC disable. */
#define BCM_L3_INGRESS_TUNNEL_DISABLE_NATIVE_ETH BCM_L3_INGRESS_UNKNOWN_SRC_TOCPU /*  Disable L3 routing on
                                                          native ETH packets. */
...
typedef struct bcm_l3_intf_s {
    ...
    uint16 l3a_gbp_dst_id;              /* GBP DID representing the egress
                                           interface. */
} bcm_l3_intf_t;
...
typedef enum bcm_l3_ingress_urpf_mode_e {
    ...
    bcmL3IngressUrpfV4DisableV6Loose = 3, 
    bcmL3IngressUrpfV4DisableV6Strict = 4, 
    bcmL3IngressUrpfV4LooseV6Disable = 5, 
    bcmL3IngressUrpfV4LooseV6Strict = 6, 
    bcmL3IngressUrpfV4StrictV6Disable = 7, 
    bcmL3IngressUrpfV4StrictV6Loose = 8, 
    bcmL3IngressUrpCount = 9 
...
typedef struct bcm_l3_egress_s {
    ...
    bcm_l3_nat_id_t nat_id;             /* Index into egress NAT table. */
} bcm_l3_egress_t;
...
typedef struct bcm_l3_ingress_s {
    ...
    int napt_enable;                    /* Enable Network address port
                                           translation. */
} bcm_l3_ingress_t;
...
typedef struct bcm_l3_route_s {
    ...
    bcm_if_t l3a_intf;                  /* L3 interface associated with route,
                                           or base L3 interface associated with
                                           aggregated host route. */
    bcm_if_t l3a_ul_intf;               /* Underlay L3 egress object associated
                                           with this address. */
    ...
    int l3a_opaque_ctrl_id;             /* Opaque control ID. */
    uint8 l3a_host_start_offset;        /* Host start offset for an aggregated
                                           host route. */
    uint8 l3a_host_width;               /* Host width for an aggregated host
                                           route. */
    uint8 l3a_traffic_class;            /* Traffic class used when
                                           l3_alpm_tc_mode is enabled. */
} bcm_l3_route_t;
...
typedef struct bcm_l3_info_s {
    ...
    int l3info_max_ecmp_failover_size;  /* Maximum number of members allowed in
                                           a two level 1:1 ECMP failover group. */
...
/* Tertiary ECMP load-balancing mode. */
typedef enum bcm_l3_ecmp_tertiary_lb_mode_e {
    bcmL3EcmpTertiaryLbModeRegular = 0, /* Regular load-balancing mode. */
    bcmL3EcmpTertiaryLbModeRandom = 1,  /* Random load-balancing mode. */
    bcmL3EcmpTertiaryLbModeDynamic = 2, /* Dynamic load-balancing mode. */
    bcmL3EcmpTertiaryLbModeCount = 3    /* Must be the last one. */
} bcm_l3_ecmp_tertiary_lb_mode_t;
...
typedef struct bcm_l3_egress_ecmp_s {
    ...
    int dlb_quality_adjust_profile_id;  /* DLB quality adjustment profile ID. */
    int dlb_source_port_prune;          /* Enable DLB source port pruning. */
    int dlb_reassignment_probability_threshhold; /* DLB reassignment probability
                                           threshold. */
    int dlb_reassignment_quality_delta; /* DLB reassignment quality delta. */
    int disable_sw_failover;            /* Disable software protection for given
                                           ECMP group. */
    bcm_if_t dynamic_failover_base_egress_id; /* Dynamic failover group member base
                                           egress forwarding object ID. */
...
typedef struct bcm_l3_ecmp_dlb_mon_cfg_s {
    ...
    uint64 sample_rate_64;  /* Rate of sampling DLB monitored packets with 64
                               bits */
...
#define BCM_L3_ECMP_MEMBER_WEIGHTED         0x100      /* If set, indicates it's
                                                          ...
                                                          should be with a
                                                          weight. */
#define BCM_L3_ECMP_UNDERLAY_ONLY           0x200      /* If set, the members of
                                                          the ECMP group are
                                                          configured at ECMP
                                                          Level 2 in the 56980
                                                          device. An ECMP group
...
#define BCM_L3_ECMP_MEMBER_REPLICATION_WEIGHTED BCM_L3_ECMP_WEIGHTED /* Same as
                                                          ...
                                                          achieved by member
                                                          replication. */
#define BCM_L3_ECMP_FORCED_RH_SEED          0x400      /* If set, indicates the
                                                          RH seed is derived
                                                          from rh_random_seed.
                                                          Otherwise, it is
                                                          internally generated. */
#define BCM_L3_ECMP_MEDIUM_TABLE            0x800      /* If set, indicates that
                                                          the consistent hashing
                                                          table is a medium one */
#define BCM_L3_ECMP_USER_PROFILE            0x1000     /* If set, indicates that
                                                          used ECMP profile will
                                                          be user-defined. */
#define BCM_L3_ECMP_MEMBER_RANDOM_WEIGHTED  0x2000     /* If set, indicates it's
                                                          a random weighted ECMP
                                                          group. */
#define BCM_L3_ECMP_SECONDARY               0x4000     /* If set, indicates that
                                                          the ECMP group is a
                                                          secondary ECMP group. */
...
#define BCM_L3_ECMP_DYNAMIC_MODE_VIP        8          /* ECMP dynamic group VIP
                                                          mode: Virtual IP L4
                                                          Load Balancer */
#define BCM_L3_ECMP_DYNAMIC_MODE_DLB_RANDOM 9          /* ECMP dynamic load
                                                          balancing random
                                                          member mode: always
                                                          use a random member. */
...
/* 
 * Get the L3 egress interface flex counter object value on a given L3
 * egress interface
 */
extern int bcm_l3_intf_flexctr_object_get(
    int unit, 
    bcm_if_t intf_id, 
...
/* Set flex counter object value for ECMP interface metadata. */
extern int bcm_l3_ecmp_flexctr_object_set(
    int unit, 
    bcm_if_t intf_id, 
    uint32 value);

/* Get flex counter object value for L3 ECMP interface metadata. */
extern int bcm_l3_ecmp_flexctr_object_get(
    int unit, 
    bcm_if_t intf_id, 
    uint32 *value);

/* Get the hardware index for a given L3 ECMP interface ID. */
extern int bcm_l3_ecmp_group_index_get(
    int unit, 
    bcm_if_t ecmp_intf, 
    uint32 *index);
...
/* Create an Egress ECMP user defined profile object. */
extern int bcm_l3_egress_ecmp_user_profile_create(
    int unit, 
    bcm_l3_egress_ecmp_t *ecmp, 
    int intf_count, 
    bcm_if_t *intf_array);

/* Destroy an Egress ECMP user-defined profile. */
extern int bcm_l3_egress_ecmp_user_profile_destroy(
    int unit, 
    bcm_l3_egress_ecmp_t *ecmp);

/* Get an Egress ECMP user defined profile object. */
extern int bcm_l3_egress_ecmp_user_profile_get(
    int unit, 
    bcm_l3_egress_ecmp_t *ecmp, 
    int intf_size, 
    bcm_if_t *intf_array, 
...
typedef enum bcm_l3_route_group_e {
    ...
    bcmL3RouteGroupOverrideV4 = 2,      /* L3 IPv4 UC override (global high)
                                           route group */
    bcmL3RouteGroupPrivateV6 = 3,       /* L3 IPv6 UC private VRF route group */
    bcmL3RouteGroupGlobalV6 = 4,        /* L3 IPv6 UC global low route group */
    bcmL3RouteGroupOverrideV6 = 5,      /* L3 IPv6 UC override (global high)
                                           route group */
    bcmL3RouteGroupFortePrivateV4 = 6,  /* L3 IPv4 UC private VRF route group in
                                           FORTE table */
    bcmL3RouteGroupForteGlobalV4 = 7,   /* L3 IPv4 UC global low route group in
                                           FORTE table */
    bcmL3RouteGroupForteOverrideV4 = 8, /* L3 IPv4 UC override (global high)
                                           route group in FORTE table */
    bcmL3RouteGroupFortePrivateV6 = 9,  /* L3 IPv6 UC override (global high)
                                           route group in FORTE table */
    bcmL3RouteGroupForteGlobalV6 = 10,  /* L3 IPv6 UC override (global high)
                                           route group in FORTE table */
    bcmL3RouteGroupForteOverrideV6 = 11, /* L3 IPv6 UC override (global high)
                                           route group in FORTE table */
    bcmL3RouteGroupCounter              /* L3 ALPM route group counter. This is
                                           not a group and should always be in
                                           the last */
...
/* Initialize a bcm_l3_alpm_resource_t structure. */
extern void bcm_l3_alpm_resource_t_init(
...
/* 
 * Set the L3 egress metadata flex counter object value for the given
 * egress object.
 */
...
/* Get flex counter object value for L3 egress metadata. */
...
/* 
 * Get flex counter object value for L3 egress. It is the hardware index
 * of L3 egress.
 */
extern int bcm_l3_egress_id_flexctr_object_get(
    int unit, 
    bcm_if_t intf_id, 
    bcm_flexctr_direction_t direction, 
    uint32 *value);
...
extern int bcm_l3_host_flexctr_object_set(
    int unit, 
    bcm_l3_host_t *info, 
    uint32 value);

/* Get flex counter object value for the given L3 host entry. */
extern int bcm_l3_host_flexctr_object_get(
    int unit, 
    bcm_l3_host_t *info, 
    uint32 *value);
...
/* L3 MTU adjustment profile options. */
#define BCM_L3_MTU_ADJUSTMENT_PROFILE_OPTIONS_WITH_ID 0x00000001 /* Create L3 MTU
                                                          adjustment profile
                                                          with ID. */
#define BCM_L3_MTU_ADJUSTMENT_PROFILE_OPTIONS_REPLACE 0x00000002 /* Replace existing
                                                          profile. */

/* Enum for L3 MTU adjustment mode. */
typedef enum bcm_l3_mtu_adjustment_mode_e {
    bcmL3MtuAdjustmentModeAdd = 0,  /* Add MTU adjustment value to the incoming
                                       packet length. */
    bcmL3MtuAdjustmentModeSub = 1,  /* Substract MTU adjustment value from the
                                       incoming packet length. */
    bcmL3MtuAdjustmentModeCount = 2 /* Always last. */
} bcm_l3_mtu_adjustment_mode_t;

/* L3 MTU adjustment profile. */
typedef struct bcm_l3_mtu_adjustment_profile_s {
    int profile_id;                     /* L3 MTU adjustment profile ID. */
    int length;                         /* Adjustment length. */
    bcm_l3_mtu_adjustment_mode_t mode;  /* Adjustment mode. */
} bcm_l3_mtu_adjustment_profile_t;

/* Initialize a bcm_l3_mtu_adjustment_profile_t structure. */
extern void bcm_l3_mtu_adjustment_profile_t_init(
    bcm_l3_mtu_adjustment_profile_t *adjustment);

#ifndef BCM_HIDE_DISPATCHABLE

/* Create a new L3 MTU adjustment profile. */
extern int bcm_l3_mtu_adjustment_profile_create(
    int unit, 
    uint32 options, 
    bcm_l3_mtu_adjustment_profile_t *adjustment_profile);

/* Delete an L3 MTU adjustment profile */
extern int bcm_l3_mtu_adjustment_profile_destroy(
    int unit, 
    int profile_id);

/* Get L3 MTU adjustment from an L3 MTU adjustment profile. */
extern int bcm_l3_mtu_adjustment_profile_get(
    int unit, 
    bcm_l3_mtu_adjustment_profile_t *adjustment_profile);
...
#define BCM_L3_EGRESS_MULTI_WITH_ID         (1 << 0)   /* Create multiple egress
                                                          objects with id. */
#define BCM_L3_EGRESS_MULTI_AGGREGATED_HOST_ROUTE (1 << 1)   /* Create multiple egress
                                                          objects for aggregated
                                                          host route. */
...
typedef struct bcm_l3_vpbr_entry_s {
    ...
    uint32 gport_wide_data;         /* Wide Data of the logical interface. */
    uint32 gport_wide_data_mask; 
    uint32 stat_id;                 /* Object statistics ID. */
    int stat_pp_profile;            /* Statistics profile. */
...
/* L3 Flags. */
#define BCM_L3_VPBR_IP6         (1 << 0)   /* IPv6 entries */
#define BCM_L3_VPBR_REPLACE     (1 << 1)   /* Replace flag */
#define BCM_L3_VPBR_TUNNEL      (1 << 2)   /* In case set, VPBR entries will be
...
/* 
 * Trigger reassignment of flows assigned to a egress member port in DLB
 * ECMP grp
 */
extern int bcm_l3_ecmp_dlb_trigger_reassignment(
    int unit, 
    bcm_if_t dlb_ecmp_group_num, 
    bcm_port_t port, 
    int percentage);
...
/* 
 * Adds a port that has been newly added to a trunk, to an existing DLB
 * ECMP group.
 */
extern int bcm_l3_ecmp_dlb_trunk_port_add(
    int unit, 
    bcm_if_t ecmp_intf, 
    bcm_trunk_t tid, 
    bcm_port_t port);

/* Remove a LAG port, from an existing DLB ECMP group. */
extern int bcm_l3_ecmp_dlb_trunk_port_delete(
    int unit, 
    bcm_if_t ecmp_intf, 
    bcm_trunk_t tid, 
    bcm_port_t port);
...
/* L3 ECMP DLB quality adjustment type. */
typedef enum bcm_l3_ecmp_dlb_quality_adjust_e {
    bcmL3EcmpDlbQualityAdjustAddZero = 0, /* No quality adjustment. */
    bcmL3EcmpDlbQualityAdjustAddOne = 1, /* Add one. */
    bcmL3EcmpDlbQualityAdjustAddTwo = 2, /* Add two. */
    bcmL3EcmpDlbQualityAdjustAddThree = 3, /* Add three. */
    bcmL3EcmpDlbQualityAdjustSubOne = 4, /* Subtract one. */
    bcmL3EcmpDlbQualityAdjustSubTwo = 5, /* Subtract two. */
    bcmL3EcmpDlbQualityAdjustSubThree = 6, /* Subtract three. */
    bcmL3EcmpDlbQualityAdjustSubFour = 7, /* Subtract four. */
    bcmL3EcmpDlbQualityAdjustCount      /* Always last. */
} bcm_l3_ecmp_dlb_quality_adjust_t;

/* L3 ECMP DLB quality adjustment profile options. */
#define BCM_L3_ECMP_DLB_QUALITY_ADJUST_PROFILE_OPTIONS_WITH_ID 0x00000001 /* Create an L3 ECMP DLB
                                                          quality adjustment
                                                          profile with ID. */

#ifndef BCM_HIDE_DISPATCHABLE

/* Create an L3 ECMP DLB quality adjustment profile. */
extern int bcm_l3_ecmp_dlb_quality_adjust_profile_create(
    int unit, 
    uint32 options, 
    int *profile_id);

/* Destroy an L3 ECMP DLB quality adjustment profile. */
extern int bcm_l3_ecmp_dlb_quality_adjust_profile_destroy(
    int unit, 
    int profile_id);

/* Set a port adjustment to an L3 ECMP DLB quality adjustment profile. */
extern int bcm_l3_ecmp_dlb_port_quality_adjust_set(
    int unit, 
    int profile_id, 
    bcm_port_t port, 
    bcm_l3_ecmp_dlb_quality_adjust_t adjust);

/* Get a port adjustment from an L3 ECMP DLB quality adjustment profile. */
extern int bcm_l3_ecmp_dlb_port_quality_adjust_get(
    int unit, 
    int profile_id, 
    bcm_port_t port, 
    bcm_l3_ecmp_dlb_quality_adjust_t *adjust);
...
#define BCM_L3_AACL_FLAGS_IP_SRC    (1U << 1)  /* Source IP subnet address
                                                  match. */
#define BCM_L3_AACL_FLAGS_HIT       (1U << 2)  /* IP address match. */
#define BCM_L3_AACL_FLAGS_HIT_CLEAR (1U << 3)  /* Clear hit bit. */
#define BCM_L3_AACL_FLAGS_SRC       BCM_L3_AACL_FLAGS_IP_SRC 
#define BCM_L3_AACL_FLAGS_FORTE     (1U << 4)  /* Indicate L3 AACL is on
                                                  FORTE(Flexible On-chip Route &
                                                  Tunnel Expansion) table. */
...
typedef struct bcm_l3_aacl_s {
    ...
    bcm_vrf_t vrf;              /* Virtual router ID. */
    bcm_vrf_t vrf_mask;         /* Virtual router ID mask. */
    bcm_mac_t mac;              /* Source or destination MAC address. */
    bcm_mac_t mac_mask;         /* Source or destination MAC address mask. */
    uint8 ip_protocol;          /* IP protocol field. */
    uint8 ip_protocol_mask;     /* IP protocol field mask. */
    uint16 ip_flags;            /* IP flags field. */
    uint16 ip_flags_mask;       /* IP flags field mask. */
    uint8 tcp_flags;            /* TCP flags field. */
    uint8 tcp_flags_mask;       /* TCP flags field mask. */
...
/* ECMP DLB port quality information. */
typedef struct bcm_l3_ecmp_dlb_port_quality_s {
    bcm_port_t port;    /* Port ID */
    int buffer_id;      /* Cosq buffer ID. */
    uint32 quality;     /* Quality value. */
} bcm_l3_ecmp_dlb_port_quality_t;

#ifndef BCM_HIDE_DISPATCHABLE

/* Get current ECMP DLB quality on a port. */
extern int bcm_l3_ecmp_dlb_port_quality_get(
    int unit, 
    bcm_l3_ecmp_dlb_port_quality_t *port_quality);

#endif /* BCM_HIDE_DISPATCHABLE */

/* Initialize an ECMP DLB port quality structure. */
extern void bcm_l3_ecmp_dlb_port_quality_t_init(
    bcm_l3_ecmp_dlb_port_quality_t *port_quality);
