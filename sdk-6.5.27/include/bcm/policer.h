...
typedef struct bcm_policer_config_s {
    ...
    int header_delta;                   /* delta header compensation. */
...
/* Structure holds configuration per policer region. */
typedef struct bcm_policer_exp_set_region_config_s {
    int set_size;           /* meter set size. */
    int set_base_meter_id;  /* meter set base id. */
} bcm_policer_exp_set_region_config_t;

/* Structure holds configuration per policer database. */
typedef struct bcm_policer_database_config_s {
    int expansion_enable;               /* Allowed to ingress database 0. if
                                           enabled, expansion size, filter and
                                           offsets will be  made according API
                                           bcm_policer_expansion_groups_set. */
    int is_single_bucket;               /* Single or dual bucket mode. */
    int expand_per_tc;                  /* If set and expansion is enabled,
                                           expansion will be made per TC, else
                                           it will be made per L2-Fwd Type */
    int command_id;                     /* meter command id. */
    bcm_policer_exp_set_region_config_t exp_set_region_config[2]; /* meter set size. */
...
typedef enum bcm_policer_control_type_e {
    ...
    bcmPolicerControlTypeTCSMUsePPTC = 5, /* Enable using PP TC for determining
                                           ...
                                           will be used */
    bcmPolicerControlTypeColorResolveMode = 6 /* Select the input for the resolve
                                           table: policer-profile, dp-command,
                                           color-promotion or
                                           is-incoming-color-green */
...
#define BCM_POLICER_PKT_LEN_INGRESS     1          
#define BCM_POLICER_PKT_LEN_EGRESS      2          
#define BCM_POLICER_PKT_LEN_TRUNCATE    4          
#define BCM_POLICER_PKT_LEN_ADD         8          
#define BCM_POLICER_COUNTER_BYTES       16         
#define BCM_POLICER_MTU_BYTES           32         
#define BCM_POLICER_SHAPING_BYTES       64         
...
typedef struct bcm_policer_action_config_s {
    ...
    bcm_pbmp_t ports;                   /* Policer action member ports. */
...
#define BCM_POLICER_ACTION_PROFILE_DESTROY  0x00000000 
#define BCM_POLICER_ACTION_PROFILE_DETACH_POLICER 0x00000001 

typedef struct bcm_policer_action_profile_s {
    bcm_policer_action_id_t action_id;  /* Policer action id. */
    bcm_policer_t policer_id;           /* Policer id. */
    uint32 flags;                       /* Policer action profile flags */
    bcm_policer_action_profile_id_t action_profile_id; /* Policer action profile id */
    uint32 profile_index;               /* Policer action profile index in
                                           hardware */
...
/* Destroy all policer action profile entry. */
extern int bcm_policer_action_profile_destroy_all(
    int unit);

#endif /* BCM_HIDE_DISPATCHABLE */

/* Enum holds the color-resolve-mode mux configuration. */
typedef enum bcm_policer_database_color_resolve_mode_e {
    bcmPolicerColorResolveIsProfile = 0, 
    bcmPolicerColorResolveDpCommand = 1, 
    bcmPolicerColorResolveColorPromotion = 2, 
    bcmPolicerColorResolveIsIncomingColorGreen = 3, 
    bcmPolicerColorResolveCount = 4 
} bcm_policer_database_color_resolve_mode_t;

typedef enum bcm_policer_expansion_types_e {
    bcmPolicerExpansionTypeTc = 0, 
    bcmPolicerExpansionTypeMappGlobalPointer = 1, 
    bcmPolicerExpansionTypePpMetaData = 2, 
    bcmPolicerExpansionTypeCount = 3 
} bcm_policer_expansion_types_t;

/* Structure holds configuration per expansion elemet. */
typedef struct bcm_policer_expansion_element_s {
    bcm_policer_expansion_types_t type; /* Enume describes the options of
                                           expansion metadata types. */
    uint32 bitmap;                      /* Select specific bits from the type
                                           for the expansion. */
} bcm_policer_expansion_element_t;

/* Structure holds expansion select configuration. */
typedef struct bcm_policer_expansion_select_s {
    uint32 nof_elements;                /* nof elements in expanion_elements
                                           array. */
    bcm_policer_expansion_element_t expanion_elements[bcmPolicerExpansionTypeCount]; /* Selection elements. */
} bcm_policer_expansion_select_t;

#ifndef BCM_HIDE_DISPATCHABLE

/* Set a policer expansion select entry. */
extern int bcm_policer_expansion_select_set(
    int unit, 
    int flags, 
    int policer_database_handle, 
...
extern int bcm_policer_expansion_select_get(
    int unit, 
    int flags, 
    int policer_database_handle, 
    bcm_policer_expansion_select_t *expantion_select);
...
#define BCM_POLICER_EXPANSION_DATA_MAPPING_NOF_ENTRIES 256        

typedef struct bcm_policer_expansion_mapping_key_s {
    bcm_policer_expansion_types_t expantion_type; 
    int value; 
} bcm_policer_expansion_mapping_key_t;

typedef struct bcm_policer_expansion_mapping_value_s {
    int set_region_id; 
    int offset; 
    int valid; 
} bcm_policer_expansion_mapping_value_t;

/* Structure holds configuration per expansion element. */
typedef struct bcm_policer_expansion_data_mapping_s {
    int nof_key_conditions;             /* Number of element types to consider. */
    bcm_policer_expansion_mapping_key_t key[bcmPolicerExpansionTypeCount]; /* Select specific bits from the type
                                           for the expansion. */
    bcm_policer_expansion_mapping_value_t value; 
} bcm_policer_expansion_data_mapping_t;

/* Structure holds expansion select configuration. */
typedef struct bcm_policer_set_map_s {
    int nof_entries;                    /* number of expansion variations to
                                           config. */
    bcm_policer_expansion_data_mapping_t expansion_data_mapping[BCM_POLICER_EXPANSION_DATA_MAPPING_NOF_ENTRIES]; /* Selection elements. */
} bcm_policer_set_map_t;

#ifndef BCM_HIDE_DISPATCHABLE

/* Set a policer expansion mapping entry. */
extern int bcm_policer_expansion_mapping_set(
    int unit, 
    uint32 flags, 
    int policer_database_handle, 
...
extern int bcm_policer_expansion_mapping_get(
    int unit, 
    uint32 flags, 
    int policer_database_handle, 
    bcm_policer_set_map_t *meter_set_map);
