...
typedef enum bcm_qos_control_type_e {
    ...
    bcmQosControlMplsELIIngressQosProfile = 10, /* Ingress QOS profile on MPLS ELI
                                           label. */
    bcmQosControlMplsELIIngressPhbModel = 11, /* Ingress QOS PHB model of MPLS ELI
                                           label. */
    bcmQosControlMplsELIIngressRemarkModel = 12, /* Ingress QOS Remark model of MPLS ELI
                                           label. */
    bcmQosControlMplsELIIngressTtlModel = 13, /* Ingress QOS TTL model of MPLS ELI
                                           label. */
    bcmQosControl__Count = 14           /* Always last. Not a usable value. */
...
/* Types of Drop Congestion Notification packets. */
typedef enum bcm_qos_dcn_pkt_type_e {
    bcmQosDcnPktTypeDataDrop = 0,       /* DCN data drop packets. */
    bcmQosDcnPktTypeControlCreditReturn = 1, /* DCN credit return control packets. */
    bcmQosDcnPktTypeControlOthers = 2,  /* Other DCN control packets. */
    bcmQosDcnPktTypeCount = 3           /* Must be the last entry! */
} bcm_qos_dcn_pkt_type_t;

/* QoS DCN internal priority override flags. */
#define BCM_QOS_DCN_INT_PRI_OVERRIDE_ENABLE (1 << 0)   /* Override internal
                                                          priority of DCN
                                                          packets. */

/* DCN internal priority overriding. */
typedef struct bcm_qos_dcn_int_pri_override_s {
    bcm_qos_dcn_pkt_type_t pkt_type;    /* DCN packet type. */
    uint32 flags;                       /* BCM_QOS_DCN_INT_PRI_OVERRIDE_XXX
                                           flags. */
    int int_pri;                        /* Internal priority. */
} bcm_qos_dcn_int_pri_override_t;

/* Initialize the bcm_qos_dcn_int_pri_override_t structure. */
extern void bcm_qos_dcn_int_pri_override_t_init(
    bcm_qos_dcn_int_pri_override_t *int_pri_override);

#ifndef BCM_HIDE_DISPATCHABLE

/* Set internal priority overriding for a DCN packet type. */
extern int bcm_qos_dcn_int_pri_override_set(
    int unit, 
    bcm_qos_dcn_int_pri_override_t *int_pri_override);

/* Get internal priority overriding for a DCN packet type. */
extern int bcm_qos_dcn_int_pri_override_get(
    int unit, 
    bcm_qos_dcn_int_pri_override_t *int_pri_override);
