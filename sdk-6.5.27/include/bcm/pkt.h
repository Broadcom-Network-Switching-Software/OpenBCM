...
typedef struct bcm_pkt_dnx_udh_s {
    ...
    uint32 data_ext;    /* MSB of UDH data when UDH size is 8B */
} bcm_pkt_dnx_udh_t;
...
struct bcm_pkt_s { 
    ...
    uint32 dest_pp_port;                /*  Destination packet processing port */
    bcm_gport_t rx_subport;             /* Indicates the ingress sub port if
                                           src_gport gets overridden for some
                                           reason (example: OAM UPMEP RX) */
};
