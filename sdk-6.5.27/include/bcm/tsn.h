...
typedef enum bcm_tsn_taf_control_e {
    ...
    bcmTsnTafControlTAFPTPLock = 11,    /* Indicate the PTP time is locked or
                                           ...
                                           basis. */
    bcmTsnTafControlReset = 12,         /* Reset the TAF. */
    bcmTsnTafControlCount = 13          /* This should be the last one. */
...
/* The CQF config structure */
typedef struct bcm_tsn_cqf_config_s {
    uint32 enable;                      /* 1 - enable CQF, 0 - disable CQF */
    bcm_ptp_timestamp_t ptp_base_time;  /* Specify base time in PTP time. */
    uint32 interval;                    /* CQF interval in ticks */
    uint32 start_tx;                    /* start transmit after ticks inside an
                                           interval (S1) */
    uint32 end_tx;                      /* stop transmit before ticks inside an
                                           interval (S2) */
} bcm_tsn_cqf_config_t;

/* Initialize the bcm_tsn_cqf_config_t structure */
extern void bcm_tsn_cqf_config_t_init(
    bcm_tsn_cqf_config_t *cqf_config);

#ifndef BCM_HIDE_DISPATCHABLE

/* Set CQF parameters per port. */
extern int bcm_tsn_cqf_config_set(
    int unit, 
    uint32 flags, 
    bcm_gport_t port, 
    bcm_tsn_cqf_config_t *config);

/* Get CQF parameters per port. */
extern int bcm_tsn_cqf_config_get(
    int unit, 
    uint32 flags, 
    bcm_gport_t port, 
    bcm_tsn_cqf_config_t *config);
