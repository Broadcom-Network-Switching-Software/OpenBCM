...
#define BCM_MIRROR_DEST_STAMP_IVE_ACTION_ID (1U << 21) /* stamp IVE action id */
...
#define BCM_MIRROR_DEST_FLAGS2_OVERWRITE    (1U << 12) /* Overwrite a mirror
                                                          ...
                                                          re-attach to the new
                                                          mirror destination. */
#define BCM_MIRROR_DEST_FLAGS2_INGRESS_STATEFUL_MOD (1U << 13) /* Stateful ingress
                                                          mirror on drop. */
#define BCM_MIRROR_DEST_FLAGS2_COSQ_STATEFUL_MOD (1U << 14) /* Stateful MMU mirror on
                                                          drop. */
#define BCM_MIRROR_DEST_FLAGS2_POSTCARD_TUNNEL_PSAMP (1U << 15) /* Mirror packet with
                                                          PSAMP postcard
                                                          encapsulation. */
...
typedef struct bcm_mirror_pkt_dnx_pp_header_s {
    uint8 tail_edit_profile;            /* Tail edit profile, used at ETPP for
                                           tail editing */
    uint8 bytes_to_remove;              /* Bytes to remove from the start of the
                                           packet */
    uint8 eth_header_remove;            /* Indication to remove Ethernet header
                                           from mirror copy */
    uint8 force_mirror;                 /* Indication to force mirroring
                                           processing packet at egress */
    bcm_gport_t out_vport_ext[3];       /* Extended outlif(s) */
    uint32 vsi;                         /* Virtual switch instance */
    uint8 ingress_vlan_translate_action_id; /* IVE action id */
...
typedef enum bcm_mirror_sample_type_e {
    ...
    bcmMirrorSampleTypeFlex = 1,        /* Ingress flex sample. */
    bcmMirrorSampleTypeEgressFlex = 2,  /* Egress flex sample. */
    bcmMirrorSampleTypeCount = 3        /* The last one, not valid. */
} bcm_mirror_sample_type_t;

/* 
 * Sample threshold mode, used to choose sampling packet below or above
 * sample rate.
 */
...
/* Mirror sample mode, used to choose random or one out of N sampling. */
typedef enum bcm_mirror_sample_mode_e {
    bcmMirrorSampleModeRandom = 0,  /* Random sampling. */
    bcmMirrorSampleModeOneInN = 1,  /* One out of N sampling. */
    bcmMirrorSampleModeCount = 2    /* The last one, not valid. */
...
typedef struct bcm_mirror_sample_profile_s {
    ...
    bcm_mirror_sample_mirror_mode_t mirror_mode; /* Sample threshold mode. */
    ...
    bcm_mirror_sample_mode_t sample_mode; /* Sample mode. */
...
typedef enum bcm_mirror_sample_stat_type_e {
    ...
    bcmMirrorSampleStatFlexSnapshotPkts = 2, /* Total number of eligible packets when
                                           ...
                                           sampling policy action. */
    bcmMirrorSampleStatEgressFlexPkts = 3, /* Total number of packets eligible to
                                           be sampled by flex sampler enabled by
                                           egress sampling policy action. */
    bcmMirrorSampleStatEgressFlexSampledPkts = 4, /* Total number of packets sampled by
                                           flex sampler enabled by egress
                                           sampling policy action. */
    bcmMirrorSampleStatEgressFlexSnapshotPkts = 5, /* Total number of eligible packets when
                                           the last packet was sampled by flex
                                           sampler enabled by egress sampling
                                           policy action. */
    bcmMirrorSampleStatCount = 6        /* The last one, invalid. */
...
typedef enum bcm_mirror_ingress_mod_event_e {
    ...
    bcmMirrorIngModEventPriSecNextHopDownDrop = 155, /* Enable to select packets dropped due
                                           ...
                                           or selected. */
    bcmMirrorIngModEventSvtagLookupMissDrop = 156, /* Enable to select packets dropped due
                                           to SVTAG lookup miss. */
    bcmMirrorIngModEventSvtagToCpu = 157, /* Enable to select SVTAG packet copied
                                           to CPU. */
    bcmMirrorIngModEventSvtagLookupMissToCpu = 158, /* Enable to select packet copied to CPU
                                           due to SVTAG lookup miss. */
    bcmMirrorIngModEventSRv6ProcessDrop = 159, /* Enable to select drops due to SRV6
                                           processing errors. */
    bcmMirrorIngModEventSRv6ValidationDrop = 160, /* Enable to select drops due to SRV6
                                           Validation errors. */
    bcmMirrorIngModEventSRv6UnknownRoutingTypeWithNonZeroSegmentLeft = 161, /* Enable to select drops due to unknown
                                           routing type in a SRV6 packet with
                                           non-zero SegmentsLeft field. */
    bcmMirrorIngModEventEcmpMemberTblBankCollision = 162, /* Enable to select the event of
                                           detection of ECMP member table bank
                                           collision while reading the table for
                                           primary and secondary next hop
                                           resolution. */
    bcmMirrorIngModEventProtectionPathActive = 163, /* Enable to select the event when the
                                           secondary or tertiary next hop is
                                           picked during the next hop resolution
                                           because primary or secondary next hop
                                           is either down or programatically
                                           disabled. */
    bcmMirrorIngModEventNextHopDownPktDrop = 164, /* Enable to select the event when
                                           primary, secondary and tertiary next
                                           hop selections are down or
                                           programatically disabled and a valid
                                           next hop could not be resolved. */
    bcmMirrorIngModEventNonFirstPassPkt = 165, /* Enable to select the non first pass
                                           packets. */
    bcmMirrorIngModEventStatefulMirrorOnDropSecondPassPktDrop = 166, /* Enable to select the drop event of a
                                           stateful mirror on drop second pass
                                           packet. */
    bcmMirrorIngModEventL2TnlSvpAssignFailDrop = 167, /* Enable to select the drop event due
                                           to a L2 tunnel SVP assignment lookup
                                           miss. */
    bcmMirrorIngModEventINTL3MtuCheckFail = 168, /* Enable to select the drop event of
                                           In-band Network Telemetry (INT)
                                           packets due to L3 MTU check failure. */
    bcmMirrorIngModEventINTRequestVectorMismatchDrop = 169, /* Enable to select packets dropped due
                                           to In-band Network Telemetry request
                                           vector mismatch. */
    bcmMirrorIngModEventINTDataplaneExceptionDrop = 170, /* Enable to select packets dropped due
                                           to In-band Network Telemetry
                                           Data-plane Probe (INT-DP) exception. */
    bcmMirrorIngModEventCount = 171     /* Must be last. Not a valid value. */
...
/* Flags for bcm_mirror_ingress_mod_event_profile_create/get. */
#define BCM_MIRROR_INGRESS_MOD_EVENT_PROFILE_UNCOMPRESSED_REASONS 0x00000001 /* Enable uncompressed
                                                          drop reasons to be
                                                          carried in ingress
                                                          mirror on drop
                                                          encapsulation. */
...
typedef struct bcm_mirror_ingress_mod_event_profile_s {
    ...
    uint32 priority;    /* Profile priority. Higher value means higher priority.
                           ...
                           mirror. */
    uint32 flags;       /* See BCM_MIRROR_INGRESS_MOD_EVENT_PROFILE_xxx  flags
                           definitions. */
