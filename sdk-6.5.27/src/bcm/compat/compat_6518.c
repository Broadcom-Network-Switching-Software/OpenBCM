...
_bcm_compat6518in_flow_port_encap_t(
    ...
    bcm_compat6520_flow_port_encap_t *to)
{
...
{
    int rv;
    bcm_compat6520_flow_port_encap_t *new_info = NULL;

    if (info != NULL) {
        /* Create from heap to avoid the stack to bloat */
        new_info = (bcm_compat6520_flow_port_encap_t *)
            sal_alloc(sizeof(bcm_compat6520_flow_port_encap_t), "New flow port encap");
        if (new_info == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_flow_port_encap_set(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_flow_port_encap_get(unit, new_info, num_of_fields, field);
    if (new_info != NULL) {
        /* Transform the entry from the new format to old one */
        _bcm_compat6518out_flow_port_encap_t(new_info, info);
        sal_free(new_info);
    ...
    bcm_compat6518_mpls_port_t *mpls_port)
{
    ...
    if (NULL != mpls_port) {
        /* Create from heap to avoid the stack to bloat */
        new_mpls_port = sal_alloc(sizeof(bcm_compat6525_mpls_port_t), "New mpls port");
...
{
    int rv = BCM_E_NONE;
    bcm_compat6520_ecn_traffic_map_info_t *new_map = NULL;

    if (map != NULL) {
        new_map = (bcm_compat6520_ecn_traffic_map_info_t *)
                     sal_alloc(sizeof(bcm_compat6520_ecn_traffic_map_info_t),
                     "New map");
        if (new_map == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_ecn_traffic_map_get(unit, new_map);
    ...
    bcm_compat6518_ecn_traffic_map_info_t *map)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_ecn_traffic_map_set(unit, new_map);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6520_flexctr_action_t *new_action = NULL;

    if (action != NULL) {
        new_action = (bcm_compat6520_flexctr_action_t *)
                     sal_alloc(sizeof(bcm_compat6520_flexctr_action_t),
                     "New action");
        if (new_action == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_flexctr_action_create(unit, options, new_action, stat_counter_id);
    ...
    bcm_compat6518_flexctr_action_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_flexctr_action_get(unit, stat_counter_id, new_action);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6526_flexctr_quantization_t *new_quantization = NULL;

    if (quantization != NULL) {
        new_quantization = (bcm_compat6526_flexctr_quantization_t *)
                           sal_alloc(sizeof(bcm_compat6526_flexctr_quantization_t),
                           "New quantization");
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flexctr_quantization_create(unit, options, new_quantization,
                                         quant_id);
    ...
    bcm_compat6518_flexctr_quantization_t *quantization)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_flexctr_quantization_get(unit, quant_id, new_quantization);
...
{
    sal_memset(to, 0, sizeof(bcm_compat6525_cosq_ebst_data_entry_t));
...
STATIC void
_bcm_compat6518out_cosq_ebst_data_entry_t(
    bcm_compat6525_cosq_ebst_data_entry_t *from,
...
{
    int rv = BCM_E_NONE, i;
    bcm_compat6525_cosq_ebst_data_entry_t *new_entry_array = NULL;

    /* Error case and low level driver will return BCM_E_PARAM */
    if (array_size <= 0) {
        return bcm_compat6525_cosq_ebst_data_get(unit, object_id, bid, array_size,
                                      (bcm_compat6525_cosq_ebst_data_entry_t *)entry_array,
                                      count);
    }

    if (entry_array != NULL) {
        new_entry_array = (bcm_compat6525_cosq_ebst_data_entry_t *)
                          sal_alloc(sizeof(bcm_compat6525_cosq_ebst_data_entry_t) *
                          array_size,
                          "New entry array");
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6525_cosq_ebst_data_get(unit, object_id, bid, array_size,
...
{
    int rv;
    bcm_compat6520_vlan_action_set_t *new_action = NULL;

    if (NULL != action) {
        /* Create from heap to avoid the stack to bloat */
        new_action = (bcm_compat6520_vlan_action_set_t *)
            sal_alloc(sizeof(bcm_compat6520_vlan_action_set_t), "New vlan action");
        if (NULL == new_action) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_port_default_action_get(unit, port, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_port_default_action_set(unit, port, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_port_egress_default_action_get(unit, port, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_port_egress_default_action_set(unit, port, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_port_protocol_action_add(unit, port, frame, ether, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_port_protocol_action_get(unit, port, frame, ether, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_mac_action_add(unit, mac, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_mac_action_get(unit, mac, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_action_add(unit, port, key_type,
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_action_create(unit, port, key_type,
                                          outer_vlan, inner_vlan, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_action_get(unit, port, key_type,
                                       outer_vlan, inner_vlan, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_egress_action_add(unit, port_class,
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_egress_action_get(unit, port_class, outer_vlan,
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_egress_gport_action_add(unit, port,
                                                    outer_vlan, inner_vlan,
                                                    new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_egress_gport_action_get(unit, port, outer_vlan,
                                                    inner_vlan, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_action_range_add(unit, port,
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_action_range_get(unit, port,
                                             outer_vlan_low, outer_vlan_high,
                                             inner_vlan_low, inner_vlan_high,
                                             new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_action_id_set(unit, flags, action_id, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_translate_action_id_get(unit, flags, action_id, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_ip_action_add(unit, vlan_ip, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_ip_action_get(unit, vlan_ip, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action_set)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_match_action_add(unit, options, match_info, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *action_set)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_match_action_get(unit, match_info, new_action);
...
{
    int rv, i;
    bcm_compat6520_vlan_action_set_t *new_action_set_array = NULL;

    if (NULL != action_set_array) {
        /* Create from heap to avoid the stack to bloat */
        new_action_set_array = (bcm_compat6520_vlan_action_set_t *)
            sal_alloc(size * sizeof(bcm_compat6520_vlan_action_set_t),
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_match_action_multi_get(unit, match, size, match_info_array,
    ...
    bcm_compat6518_vlan_action_set_t *vlan_action_set)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_field_action_vlan_actions_add(unit, entry, action, new_action);
    ...
    bcm_compat6518_vlan_action_set_t *vlan_action_set)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_field_action_vlan_actions_get(unit, entry, action, new_action);
...
_bcm_compat6518in_fcoe_vsan_action_set_t(
    ...
    bcm_compat6520_fcoe_vsan_action_set_t *to)
{
...
{
    int rv;
    bcm_compat6520_fcoe_vsan_action_set_t *new_action = NULL;

    if (NULL != action) {
        /* Create from heap to avoid the stack to bloat */
        new_action = (bcm_compat6520_fcoe_vsan_action_set_t *)
            sal_alloc(sizeof(bcm_compat6520_fcoe_vsan_action_set_t), "New fcoe vsan action");
        if (NULL == new_action) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_fcoe_vsan_translate_action_add(unit, key, new_action);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6520_vlan_control_vlan_t *new_control = NULL;

    if (control != NULL) {
        new_control = (bcm_compat6520_vlan_control_vlan_t *)
                     sal_alloc(sizeof(bcm_compat6520_vlan_control_vlan_t),
                     "New control");
        if (new_control == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_control_vlan_selective_get(unit, vlan, valid_fields, new_control);
    ...
    bcm_compat6518_vlan_control_vlan_t *control)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_control_vlan_selective_set(unit, vlan, valid_fields, new_control);
    ...
    bcm_compat6518_vlan_control_vlan_t *control)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_control_vlan_get(unit, vlan, new_control);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6520_vlan_control_vlan_t new_control;

    sal_memset(&new_control , 0, sizeof(bcm_compat6520_vlan_control_vlan_t));

    /* Transform the entry from the old format to new one */
    _bcm_compat6518in_vlan_control_vlan_t(&control, &new_control);
    /* Call the BCM API with new format */
    rv = bcm_compat6520_vlan_control_vlan_set(unit, vlan, new_control);
...
{
    int rv;
    bcm_compat6526_rx_cosq_mapping_t *new_rx_cosq_mapping = NULL;

    if (NULL != rx_cosq_mapping) {
        /* Create from heap to avoid the stack to bloat */
        new_rx_cosq_mapping = (bcm_compat6526_rx_cosq_mapping_t *)
            sal_alloc(sizeof(bcm_compat6526_rx_cosq_mapping_t), "New rx cos mapping");
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_rx_cosq_mapping_extended_set(unit, options, new_rx_cosq_mapping);
...

{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_rx_cosq_mapping_extended_get(unit, new_rx_cosq_mapping);
    ...
    bcm_compat6518_rx_cosq_mapping_t *rx_cosq_mapping)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_rx_cosq_mapping_extended_add(unit, options, new_rx_cosq_mapping);
    ...
    bcm_compat6518_rx_cosq_mapping_t *rx_cosq_mapping)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6526_rx_cosq_mapping_extended_delete(unit, new_rx_cosq_mapping);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6521_l2_egress_t *new_egr = NULL;

    if (egr != NULL) {
        new_egr = (bcm_compat6521_l2_egress_t *)
                     sal_alloc(sizeof(bcm_compat6521_l2_egress_t),
                     "New egr");
        if (new_egr == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6521_l2_egress_create(unit, new_egr);
    ...
    bcm_compat6518_l2_egress_t *egr)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6521_l2_egress_get(unit, encap_id, new_egr);
    ...
    bcm_if_t *encap_id)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6521_l2_egress_find(unit, new_egr, encap_id);
    ...
    bcm_compat6518_tunnel_terminator_t *info)
{
    ...
    if (info != NULL) {
        new_info = (bcm_compat6520_tunnel_terminator_t *)
                     sal_alloc(sizeof(bcm_compat6520_tunnel_terminator_t),
                     "New info");
        if (new_info == NULL) {
            return BCM_E_MEMORY;
        }
...
{
    int rv = BCM_E_NONE;
    bcm_compat6523_vxlan_port_t *new_vxlan_port = NULL;

    if (vxlan_port != NULL) {
        new_vxlan_port = (bcm_compat6523_vxlan_port_t *)
                     sal_alloc(sizeof(bcm_compat6523_vxlan_port_t),
                     "New vxlan_port");
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_vxlan_port_add(unit, l2vpn, new_vxlan_port);
    ...
    bcm_compat6518_vxlan_port_t *vxlan_port)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_vxlan_port_get(unit, l2vpn, new_vxlan_port);
...
{
    int rv = BCM_E_NONE;
    bcm_compat6523_vxlan_port_t *new_port_array = NULL;
    int i = 0;

    if (port_array != NULL && port_max > 0) {
        new_port_array = (bcm_compat6523_vxlan_port_t *)
                     sal_alloc(port_max * sizeof(bcm_compat6523_vxlan_port_t),
                     "New port_array");
        if (new_port_array == NULL) {
            return BCM_E_MEMORY;
        }
    ...
    /* Call the BCM API with new format */
    rv = bcm_compat6523_vxlan_port_get_all(unit, l2vpn, port_max, new_port_array, port_count);
