...
bcm_flexstate_quantization_t_init(bcm_flexstate_quantization_t *quant_cfg)
{
    if (quant_cfg != NULL) {
        ...
        quant_cfg->object_mask = 0xffff;
        quant_cfg->object_mask_upper = 0xffff;
    }
}
...
bcm_flexstate_action_t_init(bcm_flexstate_action_t *act_cfg)
{
    if (act_cfg != NULL) {
        ...
        act_cfg->operation_a.update_compare_opd = bcmFlexstateUpdateCompareOpdUseStoredValA;
        act_cfg->operation_b.update_compare_opd = bcmFlexstateUpdateCompareOpdUseStoredValB;
    }
}
