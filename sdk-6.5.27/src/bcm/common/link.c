...
typedef struct _ls_control_s {
    ...
    pbmp_t            pbm_flr_enable;         /* Enable FLR(Fast link recovery). */
    bcm_port_flr_timeout_t
                      flr_timeout[NUM_PORTS]; /* FLR timeout value in ms */
...
/*
 * Function:
 *      linkscan_thread_id_get
 * Purpose:
 *      Read value of 'lc->thread_id' (for the unit at hand).
 * Parameters:
 *      lc -
 *          Pointer to a structure of type _ls_control_t which is relevant for
 *          the unit at hand.
 * Returns:
 *      sal_thread_t - Value of lc->thread_id. In most cases, 'lc' would be
 *      _linkscan_control[unit]. If 'lc' is NULL
 *      then return '-1'.
 */
STATIC sal_thread_t
linkscan_thread_id_get(_ls_control_t  *lc)
{
    sal_thread_t ret;
...
/*
 * Function:
 *      linkscan_thread_id_set
 * Purpose:
 *      Write 'value' into 'lc->thread_id' (for the unit at hand).
 * Parameters:
 *      lc -
 *          Pointer to a structure of type _ls_control_t which is relevant for
 *          the unit at hand.
 *      value -
            Value to load into lc->thread_id
 * Returns:
 *      int - If lc is NULL then no action is taken (no memory is written to)
 *      and a negative  value is returned. Otherwise, zero is returned.
 */
STATIC int
linkscan_thread_id_set(_ls_control_t  *lc, sal_thread_t value)
{
    int ret;
...
#define LINKSCAN_THREAD_ID_GET(lc) linkscan_thread_id_get(lc)
#define LINKSCAN_THREAD_ID_SET(lc,value) linkscan_thread_id_set(lc,value)
/* } */
#else
/* { */
#define LINKSCAN_THREAD_ID_GET(lc) \
    lc->thread_id
#define LINKSCAN_THREAD_ID_SET(lc,value) \
    lc->thread_id = value
/* } */
#endif
...
/*
 * Function:
 *      _bcm_link_fault_is_valid
 * Purpose:
 *      Check for fault info was valid for the port.
 * Parameters:
 *      unit - unit.
 ...
 *      BCM_E_XXX
 */
STATIC int _bcm_link_fault_is_available(int unit, int port)
{
    int rv, speed;
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit))
    {
        ...
        {
            if (soc_feature(unit, soc_feature_link_fault_on_fabric_ports_available))
            {
                return TRUE;
            }
            else
    ...
    {
        if (soc_feature(unit, soc_feature_linkscan_fault_ignore_an_speed))
        {
            return TRUE;
        }
        else
...
_bcm_linkscan_thread(int unit)
{
    ...
    {
        lc->lc_warm_boot = 0;
        rv = bcm_common_linkscan_flr_wb_restore(unit);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_BCM_LINK,
                      (BSL_META_U(unit, "FLR WB restore bcm_common_linkscan_flr_wb_restore\n")));
        }
    }
    else
    {
    ...
    LINKSCAN_THREAD_ID_SET(lc,sal_thread_self());
    ...
    while ((interval = lc->interval_us) != 0) {
        ...
        /* Make sure that only valid change ports need be scaned */
    ...
    LINKSCAN_THREAD_ID_SET(lc,NULL);
    ...
    if (BCM_FAILURE(rv) && (rv != SOC_E_NOT_FOUND)) {
        LOG_ERROR(BSL_LS_BCM_LINK,
                  (BSL_META_U(unit,
                   ...
                   soc_errmsg(rv), (void *)scache_ptr, size));
...
skip_wb:
    ...
    /*
     * Initialize flr support linkscan port bitmaps
     */
    if (!SAL_BOOT_PLISIM)
    {
        BCM_IF_ERROR_RETURN(bcm_common_linkscan_flr_init(unit));
    }
...
/*
 * Function:
 *     _bcm_linkscan_port_deinit
 * Purpose:
 *     Remove the port to bitmaps in the link_control structure.
 * Parameters:
 *     unit - Device unit number
 *     port - Logical port ID
 * Returns:
 ...
 */
int
_bcm_linkscan_port_deinit(int unit, int port)
{
    _ls_control_t  *lc = LS_CONTROL(unit);
    ...
    /*
     * Remove initial scan by clear link change while pretending link
     * was initially up.
     */
    LS_LOCK(unit);
    BCM_PBMP_PORT_REMOVE(lc->pbm_link, port);
    BCM_PBMP_PORT_REMOVE(lc->pbm_link_change, port);
    BCM_PBMP_PORT_REMOVE(lc->pbm_hw_upd, port);
...
/**
 * \brief - Enable or disable the link scan feature.
...
bcm_common_linkscan_enable_set(int unit, int us)
{
    ...
    if (us) {
        ...
        if (LINKSCAN_THREAD_ID_GET(lc) != NULL) {
        ...
        if (tid == SAL_THREAD_ERROR) {
            ...
            while (LINKSCAN_THREAD_ID_GET(lc) == NULL) {
    ...
    } else if (LINKSCAN_THREAD_ID_GET(lc) != NULL) {
        lc->interval_us = 0;
...
/**
 * \brief - Retrieve the current linkscan mode.
 * \param [in] unit - Device unit number
 * \param [OUT] us - Pointer to microsecond scan time for software scanning, 0 if not enabled.
 * \return
 *   See shr_error_r
 * \remark
 *   * None
...
/**
 * \brief - Determine whether or not linkscan is managing a given port.
 * \param [in] unit - Device unit number
 * \param [in] port - Device port to check.
 * \return
 *   See shr_error_r
 * \remark
 *   * None
...
/**
 * \brief - Set the current scanning mode for the specified port.
 * \param [in] unit - Device unit number
 * \param [in] port - Device port number
 * \param [in] mode - New scan mode for specified port. The supported modes are:
 *                    BCM_PORT_LINKSCAN_HW/BCM_PORT_LINKSCAN_SW/BCM_PORT_LINKSCAN_NONE
 * \return
 *   See shr_error_r
 * \remark
 *   * None
...
exit:
    ...
    /* bcm_port_update was not necessary for DNX/DNXC devices */
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit)) {
        return rv;
    }
...
/**
 * \brief - Set the current scanning mode for the specified ports.
 * \param [in] unit - Device unit number
 * \param [in] pbm - Port bitmap indicating port to set
 * \param [in] mode - New scan mode for specified ports
 * \return
 *   See shr_error_r
 * \remark
 *   * None
...
/**
 * \brief - Get the current scanning mode for the specified port.
 * \param [in] unit - Device unit number
 * \param [in] port - Device port number
 * \param [out] mode - current scan mode for specified port
 * \return
 *   See shr_error_r
 * \remark
 *   * None
 ...
 * \param [in] unit - Device unit number
 * \param [in] f - Pointer to function registered in call to bcm_linkscan_register
 * \return
 *   See shr_error_r
 * \remark
 *   * None
...
/**
 * \brief - Register a handler to be called when a link status is to be determined by a caller provided function.
 * \param [in] unit - Device unit number
 * \param [in] port - Device port number
 * \param [in] f - Pointer to function to call for true link status
 * \return
 *   See shr_error_r
 * \remark
 *   * None
...
/**
 * \brief - Remove a previously registered handler that is used for setting link status.
 * \param [in] unit - Device unit number
 * \param [in] port - Device port number
 * \param [in] f - Pointer to function registered in call to bcm_linkscan_port_register
 * \return
 *   See shr_error_r
 * \remark
 *   * None
...
/**
 * \brief - Wait for all links in the mask to be "link up"..
 * \param [in] unit - Device unit number
 * \param [in/out] pbm - Port bitmap to wait for, mask of those link up on return
...
/*
 * Function:
 *     bcm_common_linkscan_flr_init
 * Purpose:
 *    Init FLR support pbmp.
 *
 * Parameters:
 *     unit - Device unit number
 * Returns:
 *     BCM_E_XXX
 */
int bcm_common_linkscan_flr_init(int unit)
{
    int rv = BCM_E_NONE;
    soc_pbmp_t support;
    int pport;

    if (soc_feature(unit, soc_feature_linkscan_flr_support))
    {
        SOC_PBMP_CLEAR(support);
        for (pport = 0; pport < NUM_PORTS; pport++) 
        {
            SOC_PBMP_PORT_ADD(support, pport);
        }

        rv = soc_linkscan_flr_support_set(unit, support);
    }
...
/*
 * Function:
 *      bcmi_esw_process_flr_link_up_intr
 * Description:
 *      DPC function to service FLR link up interrupt
 * Parameters:
 ...
 */
STATIC void
_bcm_common_process_flr_link_up_intrrupt(void *unit_vp, void *pbmp_vp,
                                        void *d2, void *d3, void *d4)
{
    soc_pbmp_t *flr_linkup_pbmp;
    int lport, pport;
    soc_info_t *si;
    ...
    flr_linkup_pbmp = (soc_pbmp_t*)pbmp_vp;
    if (flr_linkup_pbmp == NULL)
    {
        return;
    }

    si = &SOC_INFO(unit);
    SOC_PBMP_ITER(*flr_linkup_pbmp, pport) 
    {
        lport = si->port_p2l_mapping[pport];
        if ((lport < _SHR_PBMP_PORT_MAX) && (lport >= 0))
        {
            LOG_VERBOSE(BSL_LS_BCM_LINK, 
                        (BSL_META_U(unit, "portmod flr linkup, pport=%d lport=%d\n"), pport, lport));
#ifdef PORTMOD_SUPPORT
            portmod_port_phy_flr_link_up_event(unit, lport);
#endif
#ifdef BCM_DNX_SUPPORT
            if (SOC_IS_DNX(unit)) {
                /*
                 * For OTN port, the link configure recover is per logical lane
                 * For eth port, the link configure recover is per logical port
                 */
                if (IS_OTN_PORT(unit, lport)) {
                    dnx_port_advanced_flr_link_config_recover(unit, lport, pport);
                } else {
                    dnx_port_advanced_flr_link_config_recover(unit, lport, -1);
                }
            }
#endif
        }
    }
    sal_free(flr_linkup_pbmp);
}

/*
 * Function:
 *     _bcm_flr_link_up_interrupt
 * Purpose:
 *     FLR Link up  interrupt handler.
 * Parameters:
 *     unit - Device unit number.
 *     flr_linkup_pbmp - Logical port bit map.
 * Returns:
 *     Nothing
 * Notes:
 *     Assumes: valid unit number.
...
STATIC
void _bcm_flr_link_up_interrupt(int unit, soc_pbmp_t flr_linkup_pbmp)
{
    soc_pbmp_t *linkup_pbmp;
    int rv = 0;

    linkup_pbmp = sal_alloc(sizeof(soc_pbmp_t),"flr interrupt");

    if (linkup_pbmp == NULL) {
        return;
    }

    sal_memcpy(linkup_pbmp, &flr_linkup_pbmp, sizeof(soc_pbmp_t));

    rv = sal_dpc( _bcm_common_process_flr_link_up_intrrupt,
                  INT_TO_PTR(unit),
                  linkup_pbmp,
                  0,0,0);
    if (rv) {
        /* failed to queue dpc */
        sal_free(linkup_pbmp);
    }
}

/*
 * Function:
 *     bcm_common_linkscan_flr_enable_get
 * Purpose:
 *     Get FLR mode/timeout for the specified port.
 *
 * Parameters:
 *     unit - Device unit number
 *     port - Port number.
 *     enable - If FLR is enabled on the port.
 *     timeout - FLR timeout type (bcmPortFlrTimeoutxxms)
 * Returns:
 *     BCM_E_XXX
 */
int bcm_common_linkscan_flr_enable_get(
    int unit, 
    bcm_port_t port, 
    int *enable,
    bcm_port_flr_timeout_t *timeout)
{
    ...
    /* Check the saved data match M0 configure */

    *enable = BCM_PBMP_MEMBER(lc->pbm_flr_enable, port);
    if (*enable)
    {
        *timeout = lc->flr_timeout[port];
    }
    else
    {
        *timeout = bcmPortFlrTimeoutInvaild;
    }
...
/*
 * Function:
 *     bcm_common_linkscan_flr_enable_set
 * Purpose:
 ...
 *     BCM_E_XXX
 */
int bcm_common_linkscan_flr_enable_set(
    int unit,
    bcm_port_t port,
    int enable,
    bcm_port_flr_timeout_t timeout)
{
    ...
    LS_LOCK(unit);
    /* In case of flr enable but not configure timeout, 
       linkscan FLR was supposed be disable. */
    if (timeout == bcmPortFlrTimeoutInvaild)
    {
        enable = 0;
    }
    ...
    else
    {
        rv = soc_linkscan_flr_enable_set(unit, port, enable,
                (soc_linkscan_flr_timeout_t)timeout);
    }
    if (BCM_SUCCESS(rv))
    {
        /* Save setting for warm boot. */
        if (enable)
        {
            BCM_PBMP_PORT_ADD(lc->pbm_flr_enable, port);
            lc->flr_timeout[port] = timeout;
        }
        else
        {
            BCM_PBMP_PORT_REMOVE(lc->pbm_flr_enable, port);
            lc->flr_timeout[port] = 0;
        }
    }
    if (SAL_BOOT_PLISIM)
    {
    ...
    BCM_PBMP_COUNT(lc->pbm_flr_enable, flr_port_count);

    if (flr_port_count) {
        rv = soc_linkscan_flr_linkup_callback_set(unit,
                                              _bcm_flr_link_up_interrupt);
    } else {
        /* unregister */
        rv = soc_linkscan_flr_linkup_callback_set(unit,
...
/*
 * Function:
 *     bcm_common_linkscan_flr_wb_restore
 * Purpose:
 *     Update linkscan FLR configure on warmboot.
 *
 * Parameters:
 *     unit - Device unit number
 * Returns:
 *     void
 */
int bcm_common_linkscan_flr_wb_restore(int unit)
{
    int rv;
    int port;
    _ls_control_t  *lc;
    ...
    if (soc_feature(unit, soc_feature_linkscan_flr_support))
    {
        BCM_PBMP_ITER(lc->pbm_flr_enable, port)
        {
            rv = soc_linkscan_flr_enable_set(unit, port, 1,
                    (soc_linkscan_flr_timeout_t)lc->flr_timeout[port]);
            if (BCM_FAILURE(rv))
            {
                LOG_ERROR(BSL_LS_BCM_LINK,
                          (BSL_META_U(unit, "FLR WB restore Failed on port %d(%s)\n"),
                           port, bcm_errmsg(rv)));
            }
        }
    }
 ...
 *     bcm_common_linkscan_flr_enable_set
 * Purpose:
 *     Get FLR occur number for specified port.
 *
 * Parameters:
 *     unit - Device unit number
 *     port - Port number.
 *     count - FLR occur number.
 * Returns:
 *     BCM_E_XXX
 */
int
bcm_common_linkscan_flr_linkup_count_get(
    int unit, 
    int port, 
    int *count)
{
    ...
    rv = soc_linkscan_flr_linkup_count_get(unit, port, count);
