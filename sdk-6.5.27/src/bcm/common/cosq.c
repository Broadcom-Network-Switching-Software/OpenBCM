...
/*
 *Function:
 *   bcm_cosq_dynamic_ecn_threshold_profile_t_init
 *Purpose:
 *   Initialize bcm_cosq_dynamic_ecn_threshold_t structure
 *Parameters:
 *   threshold - Pointer to the struct to be init'ed
 **/
void bcm_cosq_dynamic_ecn_threshold_profile_t_init(
    bcm_cosq_dynamic_ecn_threshold_profile_t *profile)
{
    if (profile != NULL) {
        sal_memset(profile, 0, sizeof(bcm_cosq_dynamic_ecn_threshold_profile_t));
    }
}

/*
 *Function:
 *   bcm_cosq_dcn_drop_pkt_profile_t_init
 *Purpose:
 *   Initialize bcm_cosq_dcn_drop_pkt_profile_t structure
 *Parameters:
 *   threshold - Pointer to the struct to be init'ed
 **/
void bcm_cosq_dcn_drop_pkt_profile_t_init(
    bcm_cosq_dcn_drop_pkt_profile_t *profile)
{
    if (profile != NULL) {
        sal_memset(profile, 0, sizeof(bcm_cosq_dcn_drop_pkt_profile_t));
    }
}

/*
 *Function:
 *   bcm_cosq_hull_meter_profile_t_init
 *Purpose:
 *   Initialize bcm_cosq_hull_meter_profile_t structure
 *Parameters:
 *   threshold - Pointer to the struct to be init'ed
 */
void bcm_cosq_hull_meter_profile_t_init(
   bcm_cosq_hull_meter_profile_t *profile)
{
   if (profile != NULL) {
       sal_memset(profile, 0, sizeof(bcm_cosq_hull_meter_profile_t));
   }
}

/*
 *Function:
 *   bcm_cosq_hull_ecn_marking_profile_t_init
 *Purpose:
 *   Initialize bcm_cosq_hull_ecn_marking_profile_t structure
 *Parameters:
 *   threshold - Pointer to the struct to be init'ed
 */
void bcm_cosq_hull_ecn_marking_profile_t_init(
   bcm_cosq_hull_ecn_marking_profile_t *profile)
{
   if (profile != NULL) {
       sal_memset(profile, 0, sizeof(bcm_cosq_hull_ecn_marking_profile_t));
   }
}
