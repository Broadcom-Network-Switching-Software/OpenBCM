...
bcm_l2_addr_t_init(bcm_l2_addr_t *l2addr, const sal_mac_addr_t mac, bcm_vlan_t vid)
{
    if (NULL != l2addr) {
        ...
        l2addr->cores_group = BCM_CORES_GROUP_ALL;
    }
    return;
}
...
bcm_l2_learn_msgs_config_t_init(bcm_l2_learn_msgs_config_t *learn_msg_config)
{
    if (NULL != learn_msg_config) {
        ...
        learn_msg_config->cores_group = BCM_CORES_GROUP_ALL;
    }
    return;
}
...
bcm_l2_egress_t_init(bcm_l2_egress_t *addr)
{
    if (NULL != addr) {
        ...
        addr->egress_qos_model.egress_qos = bcmQosEgressModelInitial;
    }
    return;
}
...
/*
 * Function:
 *     bcm_l2_mac_move_mon_params_t_init
 * Description:
 *     Initialize a bcm_l2_mac_move_mon_params_t structure.
 * Parameters:
 *     mmm_cfg - (OUT) Pointer to the MAC move monitoring parameters structure.
 * Return: none
 */
void bcm_l2_mac_move_mon_params_t_init(
    bcm_l2_mac_move_mon_params_t *mmm_cfg)
{
    if (mmm_cfg != NULL) {
        sal_memset(mmm_cfg, 0, sizeof(bcm_l2_mac_move_mon_params_t));
    }
}
