    ...
    bcm_policer_database_config_t *policer_database_config)
{
    if (policer_database_config != NULL) {
        ...
        /** set command_id to DNX_POLICER_DEFAULT_CMD_ID */
        policer_database_config->command_id = -1;
    }
    return;
}
 ...
 * Purpose:
 *      Initialize the policer action profile struct
 * Parameters:
 *      act_param - Pointer to the struct to be init'ed
 */
void
bcm_policer_action_profile_t_init(bcm_policer_action_profile_t *profile_cfg)
{
    if (profile_cfg != NULL) {
        sal_memset(profile_cfg, 0, sizeof(bcm_policer_action_profile_t));
    }
    return;
}
