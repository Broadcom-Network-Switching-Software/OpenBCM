...
static _bcm_ptp_telecom_g8265_custom_clock_class_t telecom_custom_clkclass_list[PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES];
...
static void _bcm_ptp_telecom_g8265_custom_clock_class_init(
    void );

static int _bcm_ptp_telecom_g8265_custom_clock_class_lookup(
    int clock_class, int *is_found, int *custom_clkclass_list_idx);

static int _bcm_ptp_telecom_g8265_pktmaster_invalid(
    int telecom_gm_idx);
    ...
    int clock_num)
{
    ...
    _bcm_ptp_telecom_g8265_custom_clock_class_init();
    ...
    bcm_ptp_clock_port_address_t *address)
{
    ...
    /* Initialize PTSF lossAnnounce  - rest of events are inited in DPC */
    PACKET_MASTER(el).ptsf.loss_announce = 0;
    ...
    uint32 clock_port)
{
    ...
    /* Set AMT. */
    if (_bcm_ptp_telecom_g8265_pktmaster_invalid(telecom.selected_gm)) {
    ...
    void)
{
    ...
    for (el = 0; el < PTP_TELECOM_MAX_NUMBER_PKTMASTERS; ++el) {
        if (_bcm_ptp_telecom_g8265_pktmaster_invalid(el)) {
    ...
    bcm_ptp_telecom_g8265_pktmaster_t *pktmaster)
{
     ...
     * ptsf.loss_announce set is integrated with master unavailable event
...
/*
 * Function:
 *      _bcm_ptp_telecom_g8265_packet_master_ptsf_loss_announce_set
 * Purpose:
 *      Set or clear PTSF-loss_announce for the packet master.
 * Parameters:
 *      unit            - (IN)  Unit number.
 *      ptp_id          - (IN)  PTP stack ID.
 *      clock_num       - (IN)  PTP clock number.
    ...
    _bcm_ptp_event_t event_type)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_clock_cache_info_get(unit, ptp_id, clock_num, &clock_info))) {
        return rv;
    }
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_telecom_g8265_packet_master_lookup(address, &el))) {
        if (address->addr_type == bcmPTPUDPIPv4) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_PTP,
                ...
                address->address[8], address->address[9], address->address[10], address->address[11],
                address->address[12], address->address[13], address->address[14], address->address[15]));
        }

        if (event_type == _bcm_ptp_master_unavail_event) {
            /*
             * If the user removed the vaild master then ptsf due to master_unavail
             * would fail as the master is not present in telecom master dataset.
             */
            return BCM_E_NONE;
        }
        return rv;
    ...
    if (event_type == _bcm_ptp_master_unavail_event) {
        PACKET_MASTER(el).ptsf.loss_announce =
          (pktmaster->elapsed_times.announce > telecom.receipt_timeouts.announce) ? 1:0;
        /*
         * PTSF criteria met?
         * Set PTSF timestamp and PTSF occurence counter.
    ...
    } else if (event_type == _bcm_ptp_master_avail_event) {
        PACKET_MASTER(el).ptsf.loss_announce = 0;
    }
 ...
 *     _bcm_ptp_telecom_g8265_custom_clock_class_init
 * Purpose:
 *      Initialze custom clock class list.
 * Parameters:
 * Returns:
 *      None.
 * Notes:
    ...
    void)
{
    ...
    for (tmp=0; tmp < PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES; tmp++) {
        telecom_custom_clkclass_list[tmp].clock_class = 0;
        telecom_custom_clkclass_list[tmp].enable = 0;
    }
}

/*
 * Function:
 *     _bcm_ptp_telecom_g8265_custom_clock_class_lookup
 * Purpose:
 *      Look-up clock class in user defined clock class list.
 * Parameters:
 *      clock_class - (IN) Clock Class.
 * Returns:
 *      BCM_E_XXX - Function status.
 * Notes:
 */
    ...
    int clock_class, int *is_found, int *custom_clkclass_list_idx)
{
    ...
    *custom_clkclass_list_idx = -1;
    ...
    for (tmp=0; tmp < PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES; tmp++) {
        if (telecom_custom_clkclass_list[tmp].clock_class == clock_class) {
            *is_found = 1;
            *custom_clkclass_list_idx = tmp;
            break;
        }
    }
exit:
...
/*
 * Function:
 *    _bcm_ptp_telecom_g8265_pktmaster_invalid
 * Purpose:
 *      Lookup whether clock class is allowed or disallowed in custom clock class list.
 * Parameters:
 *      clock_class - (IN) Clock Class.
 * Returns:
 *      0/1 - Not allowed/Allowed.
 * Notes:
 */
static int
_bcm_ptp_telecom_g8265_pktmaster_invalid(
...
{
    int is_found = 0;
    int custom_clkclass_list_idx = 0;
    int is_pktmaster_invalid = 0;

    /* Decide packet master valid or invalid as per standard */
    is_pktmaster_invalid = PACKET_MASTER_INVALID(PACKET_MASTER(telecom_gm_idx));

    /* now override the standard decision based on custom configuration */
    _bcm_ptp_telecom_g8265_custom_clock_class_lookup(PACKET_MASTER(telecom_gm_idx).clock_class, &is_found,
        &custom_clkclass_list_idx);

    if (is_found) {
        if (telecom_custom_clkclass_list[custom_clkclass_list_idx].enable) {
            is_pktmaster_invalid = 0;
            /* Need to re-check non-QL based G.8265.1 protocol parameters
             for custom clock class list masters */
            if ( bcm_ptp_telecom_g8265_pktmaster_state_unused == PACKET_MASTER(telecom_gm_idx).state ||
                1 == PACKET_MASTER(telecom_gm_idx).selector.lockout ||
                1 == PACKET_MASTER(telecom_gm_idx).selector.wait_to_restore ||
                1 == PACKET_MASTER(telecom_gm_idx).ptsf.loss_announce ||
                1 == PACKET_MASTER(telecom_gm_idx).ptsf.loss_sync ||
                1 == PACKET_MASTER(telecom_gm_idx).ptsf.unusable ) {
                is_pktmaster_invalid = 1;
            }
...
/*
 * Function:
 *     _bcm_ptp_telecom_g8265_custom_clock_class_add
 * Purpose:
 *      Add the cloc class to user defined clock class list for G.8265.1 profile.
 * Parameters:
 *      unit        - (IN) Unit number.
 *      ptp_id      - (IN) PTP stack ID.
 *      clock_num   - (IN) PTP clock number.
    ...
    int enable)
{
    ...
    for (tmp=0; tmp < PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES; tmp++) {
        if (telecom_custom_clkclass_list[tmp].clock_class == 0) {
            if (free_idx == -1) {
                free_idx = tmp;
            }
            continue;
        }
        /* modify the existing clock class entry */
        if (telecom_custom_clkclass_list[tmp].clock_class == clock_class) {
            telecom_custom_clkclass_list[tmp].enable = enable;
            goto exit;
        }
    }

    /* clock class list is full */
    if ((tmp == PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES) && (free_idx == -1)) {
        return BCM_E_FULL;
    }

    /* add a new entry in clock class list */
    if (free_idx != -1) {
        telecom_custom_clkclass_list[free_idx].clock_class = clock_class;
        telecom_custom_clkclass_list[free_idx].enable = enable;
    }
...
/*
 * Function:
 *      _bcm_ptp_telecom_g8265_custom_clock_class_remove
 * Purpose:
 *      Remove clock class from user defined clock class list for G.8265.1 profile.
 * Parameters:
 *      unit        - (IN) Unit number.
 *      ptp_id      - (IN) PTP stack ID.
 *      clock_num   - (IN) PTP clock number.
...
/*
 * Function:
 *     _bcm_ptp_telecom_g8265_custom_clock_class_list_get
 * Purpose:
 *      Gets user defined claock class list for G.8265.1 profile.
 * Parameters:
 *      unit        - (IN) Unit number.
 *      ptp_id      - (IN) PTP stack ID.
 *      clock_num   - (IN) PTP clock number.
 *      clock_class_list - (IN/OUT) Clock Class.
 * Returns:
 *      BCM_E_XXX - Function status.
 * Notes:
 */
...
_bcm_ptp_telecom_g8265_custom_clock_class_list_get(
    ...
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_telecom_g8265_custom_clock_class_t *clock_class_list)
{
    int rv = BCM_E_NONE;
    bcm_ptp_clock_info_t clock_info;
    int tmp = 0;
    ...
    if (clock_class_list == NULL) {
        return BCM_E_PARAM;
    }

    /* assumes clock_class_list has PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES entries */
    for (tmp=0; tmp < PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES; tmp++) {
        clock_class_list[tmp].clock_class = telecom_custom_clkclass_list[tmp].clock_class;
        clock_class_list[tmp].enable = telecom_custom_clkclass_list[tmp].enable;
    }
