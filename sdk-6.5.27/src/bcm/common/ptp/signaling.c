    ...
    bcm_ptp_clock_peer_address_t *address)
{
    ...
    int inst_num = PTP_EXT_SERVO_MSTR_INST_UNKNOWN;
#endif /* BCM_PTP_EXT_SERVO_SUPPORT */
    ...
    bcm_ptp_clock_peer_t *master)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_update_peer_counts(unit, ptp_id, clock_num,
            ...
            1, 0, 0, 0, 0, 0, 0, 1, 0, NULL, 0))) {
        PTP_ERROR_FUNC("_bcm_ptp_update_peer_counts()");
        return rv;
    }

    if (BCM_FAILURE(rv = _bcm_ptp_update_packet_counts(master->local_port_number,
            pi.port_address.addr_type, 1, 0, 0))) {
    ...
    bcm_ptp_message_type_t messageType)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_update_packet_counts(entry->master.local_port_number,
            pi.port_address.addr_type, 1, 0, 0))) {
        PTP_ERROR_FUNC("_bcm_ptp_update_packet_counts()");
        return rv;
    }
    ...
    int async)
{
...

    {
        ...
        if (ethSubHeader != 0x8100 && pi.rx_packets_vlan != 0)
        {
            ...
            if (slave->peer.peer_address.addr_type == bcmPTPUDPIPv4)
            {
            ...
            } else if (slave->peer.peer_address.addr_type == bcmPTPUDPIPv6)
            {
                slave->peer.peer_address.raw_l2_header[slave->peer.peer_address.raw_l2_header_length+2] = 0x86;
                slave->peer.peer_address.raw_l2_header[slave->peer.peer_address.raw_l2_header_length+3] = 0xdd;
            }
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_update_packet_counts(slave->peer.local_port_number,
            pi.port_address.addr_type, 1, 0, 0))) {
        PTP_ERROR_FUNC("_bcm_ptp_update_packet_counts()");
        return rv;
    }
