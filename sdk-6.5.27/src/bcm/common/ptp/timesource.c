 ...
 *      bcm_common_ptp_ntp_tod_offset_set
 *      bcm_common_ptp_ntp_tod_offset_get
 */
...
int bcm_common_ptp_ntp_tod_offset_set(int unit, bcm_ptp_stack_id_t ptp_id,
            int clock_num, uint32 flags, bcm_ptp_time_spec_t *ptp_tod_offset,
            bcm_ptp_time_spec_t *ntp_tod_offset)
{
    int rv = BCM_E_UNAVAIL;
    bcm_ptp_port_identity_t portid;
    uint8 payload[PTP_MGMTMSG_PAYLOAD_PACKET_PTP_NTP_TOD_OFFSET_SIZE_OCTETS] = {0};
    uint8 *curs = &payload[0];
    uint8 resp[PTP_MGMTMSG_RESP_MAX_SIZE_OCTETS];
    int resp_len = PTP_MGMTMSG_RESP_MAX_SIZE_OCTETS;
    bcm_ptp_time_spec_t temp = {0};
    ...
    if (ptp_tod_offset == NULL) {
        ptp_tod_offset = &temp;
    }

    if (ntp_tod_offset == NULL) {
        ntp_tod_offset = &temp;
    }
    ...
    _bcm_ptp_uint32_write(curs, flags);
    curs += sizeof(uint32);

    *curs++ = ptp_tod_offset->isnegative;
    _bcm_ptp_uint64_write(curs, ptp_tod_offset->seconds);
    curs += 8;
    _bcm_ptp_uint32_write(curs, ptp_tod_offset->nanoseconds);
    curs += sizeof(uint32);

    *curs++ = ntp_tod_offset->isnegative;
    _bcm_ptp_uint64_write(curs, ntp_tod_offset->seconds);
    curs += 8;
    _bcm_ptp_uint32_write(curs, ntp_tod_offset->nanoseconds);
    curs += sizeof(uint32);

    rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num, &portid,
            PTP_MGMTMSG_SET, PTP_MGMTMSG_ID_PTP_NTP_TOD_OFFSET,
            payload, curs - payload, resp, &resp_len);

    return rv;
}

int bcm_common_ptp_ntp_tod_offset_get(int unit,bcm_ptp_stack_id_t ptp_id,
            int clock_num, uint32 flags, bcm_ptp_time_spec_t *ptp_tod_offset,
            bcm_ptp_time_spec_t *ntp_tod_offset)
{
    int rv = BCM_E_UNAVAIL;
    ...
    bcm_ptp_port_identity_t portid;
    uint8 resp[PTP_MGMTMSG_PAYLOAD_PACKET_PTP_NTP_TOD_OFFSET_SIZE_OCTETS] = {0};
    int resp_len = PTP_MGMTMSG_PAYLOAD_PACKET_PTP_NTP_TOD_OFFSET_SIZE_OCTETS;
    ...
    if (PTP_TIMESOURCE_USE_MANAGEMENT_MSG) {
        ...
        if (BCM_FAILURE(rv = _bcm_ptp_management_message_send(unit, ptp_id, clock_num,
                        &portid, PTP_MGMTMSG_GET, PTP_MGMTMSG_ID_PTP_NTP_TOD_OFFSET, 0, 0,
                        resp, &resp_len))) {
            return rv;
        }
         ...
         *                     BCM<null><null><null>.
         *    Octet 6..9     : Flags.
         *    Octet 10       : PTP ToD isnegative
         *    Octet 11..18   : PTP ToD Seconds
         *    Octet 19..22   : PTP ToD Nanoseconds
         *    Octet 23       : NTP ToD isnegative
         *    Octet 24...31  : NTP ToD seconds
         *    Octet 32..35   : NTP ToD Nanoseconds
         */
        i = 6;
        ...
        i = 10;
        if ((flags & BCM_PTP_TOD_PTP_OFFSET) && ptp_tod_offset) {
            ptp_tod_offset->isnegative = resp[i];
            i++;

            ptp_tod_offset->seconds = _bcm_ptp_uint64_read(resp+i);
            i += 8;

            ptp_tod_offset->nanoseconds = _bcm_ptp_uint32_read(resp+i);
            i += 4;
        }

        i = 23;
        if ((flags & BCM_PTP_TOD_NTP_OFFSET) && ntp_tod_offset) {
            ntp_tod_offset->isnegative = resp[i];
            i++;

            ntp_tod_offset->seconds = _bcm_ptp_uint64_read(resp+i);
            i += 8;

            ntp_tod_offset->nanoseconds = _bcm_ptp_uint32_read(resp+i);
            i += 4;
        }
    }
