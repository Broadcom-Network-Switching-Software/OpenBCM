    ...
    bcm_ptp_clock_info_t *clock_info)
{
    ...
    if (soc_feature(unit, soc_feature_ptp)) {
         ...
         *    Octet 64      : NTP ToD disable
         *    Octet 65      : PTP ToD disable
         */
        i = 0;
        payload[i++] = 'B';
        payload[i++] = 'C';
        ...
        if (PTP_UC_FEATURE_CHECK(PTP_NTP_PTP_TOD_DISABLE)) {
            i = 64;
            payload[i++] =
                ((SHR_BITGET(&(clock_info->flags), BCM_PTP_CLOCK_FLAGS_NTP_TOD_UPDATE_DISABLE)) ? 1 : 0);

            payload[i++] =
                ((SHR_BITGET(&(clock_info->flags), BCM_PTP_CLOCK_FLAGS_PTP_TOD_UPDATE_DISABLE)) ? 1 : 0);
        }
