...
void _bcm_ptp_ext_servo_phaseOffsetNotification_cb( const T_ieee1588PortIdentity *masterPortId,
                                                   T_osBigFloat phaseOffsetSeconds,
                                                   T_osBool chosenTracker )
{
    bcm_ptp_port_identity_t gps_portid = {{1,2,3,4,5,6,7,8},9};
    bcm_ptp_phase_holdover_state_t state;
    int rv = BCM_E_NONE;
    long phase_offset_ns = phaseOffsetSeconds * 1e9;

    LOG_INFO(BSL_LS_BCM_PTP,
            (BSL_META_U(0, "\n phase offset notification: [ph_off:%lf] Chosen:%d \
                MstrPortId[%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x.%d] phoff_ns: %lld\n"),
             (double)phaseOffsetSeconds, chosenTracker,
             masterPortId->clockIdentity.clockId[0], masterPortId->clockIdentity.clockId[1],
             masterPortId->clockIdentity.clockId[2], masterPortId->clockIdentity.clockId[3],
             masterPortId->clockIdentity.clockId[4], masterPortId->clockIdentity.clockId[5],
             masterPortId->clockIdentity.clockId[6], masterPortId->clockIdentity.clockId[7],
             masterPortId->portNumber,
             (long long int)phase_offset_ns));

    /* Process only non-chosen trackers. */
    if (chosenTracker) {
        if (phaseOffsetSeconds && g_apts_enabled &&
            (g_apts_current_op_mode == bcmPtpClockAptsModeSyncEGps)) {
#ifndef __KERNEL__
            double phase_off_ns = 0;
            phase_off_ns -= phase_offset_ns;
#endif
            LOG_INFO(BSL_LS_BCM_PTP,
                (BSL_META_U(0, " AptsMode SyncE-GPS. Set ph_off_ns:%lf %lld \n"),
                    (double) phase_off_ns, (long long int)phase_offset_ns));
            _bcm_ptp_servo_phase_offset_set(0, 0, 0, &phase_off_ns);
        }
        return;
    }

    /* APTS not enabled Or Mode is not GpsGps. */
    if ( (!g_apts_enabled) ||
         (g_apts_current_op_mode != bcmPtpClockAptsModeGpsGps)) {
        return;
    }

    /* Process only non-GPS trackers. */
    if (_bcm_ptp_peer_portid_compare(&gps_portid, (bcm_ptp_port_identity_t *)masterPortId)) {
        return;
    }

    rv = _bcm_ptp_phase_holdover_get(0, 0, 0, &state);
    if (rv != BCM_E_NONE) {
        LOG_INFO(BSL_LS_BCM_PTP,
                (BSL_META_U(0, "Failed to get phase_holdover \n")));
        return;
    }

    state.gps_to_ptp_offset_manual += phase_offset_ns;
    state.enable_gps_to_ptp_auto = 0;

    LOG_INFO(BSL_LS_BCM_PTP,
            (BSL_META_U(0, "\n set_gps_to_ptp_off: %lld \n"),
            (long long int)state.gps_to_ptp_offset_manual));

    rv = _bcm_ptp_phase_holdover_set(0, 0, 0, &state);
    if (rv != BCM_E_NONE) {
        LOG_INFO(BSL_LS_BCM_PTP,
                (BSL_META_U(0, "Failed to set phase_holdover \n")));
        return;
    }
...
T_userCallbacks _bcm_ptp_cb_func = {
                    ...
                    (T_phaseOffsetNotificationCallback)_bcm_ptp_ext_servo_phaseOffsetNotification_cb
                    };
