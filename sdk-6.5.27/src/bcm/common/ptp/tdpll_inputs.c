...
extern int _bcm_common_synce_phy_lane_get_port(int unit, int lanePort, int isRx, uint32 *lport);
...
extern int _bcm_common_gdpll_chan_get(int unit, int chan, bcm_gdpll_chan_t *pGdpllChan);
    ...
    bcm_tdpll_input_clock_l1mux_t *l1mux)
{
    ...
    if (SOC_IS_MONTEREY(unit)) {
        ...
        if (BCM_FAILURE(rv = _bcm_common_synce_phy_lane_get_port(0, INPUT_CLOCK(clock_index).l1mux.port, 1, &lport))) {
            PTP_ERROR_FUNC("_bcm_common_synce_phy_lane_get_port()");
            return rv;
        } else {
            LOG_DEBUG(BSL_LS_BCM_PTP,
    ...
    bcm_tdpll_input_clock_l1mux_t *l1mux)
{
    ...
    if (SOC_IS_MONTEREY(unit)) {
        ...
        int port = l1mux->port;
        if (BCM_FAILURE(rv = _bcm_common_synce_phy_port_get_lane(0, port, 1, NULL, &port))) {
            PTP_ERROR_FUNC("_bcm_common_synce_phy_port_get_lane()");
            return rv;
        } else {
            LOG_DEBUG(BSL_LS_BCM_PTP,
...
int _bcm_uc_2a_handle_ts1_offset_change_event(int unit, int64 new_ts1_offset)
{
    ...
    for (chan=0; chan<BCM_GDPLL_NUM_CHANNELS; chan++) {
        _bcm_common_gdpll_chan_get(unit, chan, &current_gdpll_chan);
