...
/* { */
/*
 * Get the current value of rx_control.thread_running
 * Added specifically to use, suppressed, under TSAN.
 */
int
   rx_control_thread_running_get(void)
{
    int ret;
    ret = rx_control.thread_running;
    return (ret);
}
/*
 * Set the value of rx_control.thread_running
 * See rx_control_thread_running_get()
 */
void
   rx_control_thread_running_set(int value)
{
    rx_control.thread_running = value;
    return;
}
/* } */
#endif
...
/* } */
    ...
    if (!bcm_rx_pool_per_unit_setup_done(unit)) {
#else
...
#ifdef BCM_PER_UNIT_RX_POOL_SUPPORT
        rv = (bcm_rx_pool_per_unit_setup(unit, number_of_packets,
    ...
    if (NULL == rx_control.system_lock) {
        ...
        if (NULL == rx_control.system_lock) {
#ifdef BCM_PER_UNIT_RX_POOL_SUPPORT
            bcm_rx_pool_per_unit_cleanup(unit);
#else
            bcm_rx_pool_cleanup();
#endif
            sal_free((void *)rx_ctrl_ptr->rc_callout);
...
int _bcm_rx_shutdown(int unit)
{
    ...
    if (i == BCM_CONTROL_MAX) {
        int rv = BCM_E_NONE;
#ifdef BCM_PER_UNIT_RX_POOL_SUPPORT
        rv = bcm_rx_pool_per_unit_cleanup(unit);
#else
...
/* } */
    ...
    if (!RX_CONTROL_THREAD_RUNNING_GET()) {
...
/* } */
    ...
    if (RX_CONTROL_THREAD_RUNNING_GET()) {
        RX_THREAD_EXIT_COMPLETE_SET(FALSE);
        RX_CONTROL_THREAD_RUNNING_SET(FALSE);
...
/* } */
        ...
        RX_THREAD_NOTIFY(unit);
        for (i = 0; i < 10; i++) {
            if (RX_THREAD_EXIT_COMPLETE_GET()) {
                break;
            }
...
_bcm_common_rx_reasons_get(int unit, bcm_rx_reasons_t *reasons)
{
    ...
    while ((map = map_array[maps_index++]) != NULL) {
        for (ix = 0; ix < max_index ; ix++) {
            ...
            if (((int)(map[ix]) >= (int)((soc_rx_reason_t)socRxReasonInvalid) ) &&
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        if (SOC_IS_TOMAHAWK3(unit)) {
            if (cmc == pci_cmc) {
                ...
                if (queue_id < 0 || queue_id > SOC_TH3_NUM_CPU_QUEUES) {
                    return BCM_E_PARAM;
                }
        ...
        if (SOC_IS_DNX(unit)) {
            ...
            if (queue_id < 0)
            {
                ...
                /** Map all  CPU logical ports to current channel */
                BCM_IF_ERROR_RETURN(dnx_algo_port_logicals_get(unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_CPU, 0, &logical_ports));
                BCM_PBMP_ITER(logical_ports, logical_port_i)
                {
                    BCM_IF_ERROR_RETURN(dnx_rx_parser_info.dma_channel_to_cpu_port.set(unit, chan_id, logical_port_i));
                    BCM_IF_ERROR_RETURN(dnx_rx_parser_info.cpu_port_to_dma_channel.set(unit, logical_port_i, chan_id));
                    BCM_IF_ERROR_RETURN(dnx_algo_gpm_gport_phy_info_get
                                    (unit, logical_port_i, DNX_ALGO_GPM_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT, &gport_info));
                    BCM_GPORT_SYSTEM_PORT_ID_SET(sys_gport, gport_info.sys_port);
                    BCM_IF_ERROR_RETURN(bcm_stk_sysport_gport_get(unit, sys_gport, &mod_gport));
                    ftmh_pp_dsp = BCM_GPORT_MODPORT_PORT_GET(mod_gport);
                    BCM_IF_ERROR_RETURN(dnx_rx_parser_info.ftmh_pp_dsp_to_cpu_port.set(unit, ftmh_pp_dsp, logical_port_i));
                }
            }
            else
            {
                ...
                /** Get CPU logical ports per input queue_id*/
                BCM_IF_ERROR_RETURN(dnx_algo_port_logicals_get(unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_CPU, 0, &logical_ports));
                BCM_PBMP_ITER(logical_ports, logical_port_i)
                {
                    BCM_IF_ERROR_RETURN(dnx_algo_port_out_channel_get(unit, logical_port_i, &channel_i));
                    BCM_IF_ERROR_RETURN(dnx_algo_port_priorities_nof_get(unit, logical_port_i,  &num_priorities));

                    for (cosq = channel_i; cosq < (channel_i + num_priorities); cosq++)
                    {
                        if (cosq == queue_id)
                        {
                            BCM_IF_ERROR_RETURN(dnx_rx_parser_info.dma_channel_to_cpu_port.set(unit, chan_id, logical_port_i));
...
rx_chain_start(int unit, int chan, dv_t *dv)
{
    ...
    if (!RX_INIT_DONE(unit) || !RX_CONTROL_THREAD_RUNNING_GET()) {
...
rx_update_dv(int unit, int chan, dv_t *dv)
{
    ...
    if (SOC_UCHAN(unit, chan) &&
        (!RX_CONTROL_THREAD_RUNNING_GET() || NO_REAL_CALLOUTS(unit))) {
...
rx_pkt_thread(void *param)
{
    ...
    RX_THREAD_EXIT_COMPLETE_SET(TRUE);
            ...
            if (SOC_IS_DNX(unit))
            {
                ...
                if (rv == BCM_E_NONE)
                {
                    if (logical_port != -1)
                    {
                        ...
                        pkt->cos = 0;
                        /*
                         * Set rx_port to queue_base + cosq(0)
                         */
...
#ifdef BCM_KATANA2_SUPPORT
        pkt->rx_subport = soc_higig2_field_get(unit, xgh2, HG_src_port);

        if (SOC_IS_KATANA2(unit)) {
            /*
             * Check if the received gport is subport type.
             * If yes, set the Gport accordingly.
             */
            if (BCM_PBMP_MEMBER(si->enabled_subtag_pp_port_pbm, pkt->rx_subport)) {
                BCM_GPORT_SUBPORT_PORT_SET(l_gport, pkt->rx_subport);
                _BCM_SUBPORT_COE_PORT_TYPE_SET(l_gport, _BCM_SUBPORT_COE_TYPE_SUBTAG);
                pkt->rx_subport = l_gport;
            } else if (BCM_PBMP_MEMBER(si->enabled_linkphy_pp_port_pbm, pkt->rx_subport)) {
                BCM_GPORT_SUBPORT_PORT_SET(l_gport, pkt->rx_subport);
                _BCM_SUBPORT_COE_PORT_TYPE_SET(l_gport, _BCM_SUBPORT_COE_TYPE_LINKPHY);
                pkt->rx_subport = l_gport;
            } else {
                pkt->rx_subport = 0;
            }
...
#ifdef BCM_KATANA2_SUPPORT
            if (SOC_IS_KATANA2(unit)) {
                pkt->dest_pp_port = SOC_DCB_ORIG_DEST_PP_PORT(unit, dcb);
            }
#endif
        }
/* } */
...
rcpu_rx_pkt_enqueue(int unit, bcm_pkt_t *pkt)
{
    ...
    if (!RX_UNIT_STARTED(unit) || !RX_CONTROL_THREAD_RUNNING_GET()) {
        return BCM_E_PARAM;
    }
...
fill_in_pkt_vpn_id(int unit, bcm_pkt_t *pkt)
{
    ...
    if (BCM_GPORT_IS_MPLS_PORT(pkt->src_gport)) {
        ...
        if (!_BCM_VPN_IS_SET(pkt->vlan)) {
            rx_svp_vpn_resolve(unit, src_vp, &pkt->vlan);
        }
    }
...
_bcm_common_rx_show(int unit)
{
    ...
    RX_PRINT(("RX Info @ time=%u: %sstarted. Last fill %u. "
              ...
              last_fill_check,
              RX_CONTROL_THREAD_RUNNING_GET() ? "" : "not "));

    /* Get global rate limiting from SW(RX_PPS(unit)) or HW. */
    BCM_IF_ERROR_RETURN(bcm_rx_rate_get(unit, &pps));

    RX_PRINT(("    Pkt Size %d. Pkts/Chain %d. All COS PPS %d. Burst %d."
              " Flags %x.\n",
              RX_PKT_SIZE(unit), RX_PPC(unit), pps, RX_BURST(unit),
              RX_USER_FLAGS(unit)));
    ...
    /* } */
            ...
            /* Get per queue rate limiting from SW(RX_COS_PPS(unit)) or HW. */
#if defined(BCM_RPC_SUPPORT) || defined(BCM_ESW_SUPPORT) || defined(BCM_LOOP_SUPPORT)
/* { */
            BCM_IF_ERROR_RETURN(bcm_rx_cos_rate_get(unit, i, &pps));
/* } */
#else
/* { */
