...
#define BS_NTP_TOD_DISABLE_SET_CMD_LEN  (1 + 1)
#define BS_NTP_TOD_DISABLE_SET_RESP_LEN (1 + 1)
int
_bcm_time_bs_ntp_tod_disable(int unit, int ntp_tod_disable)
{
    uint8  command[BS_NTP_TOD_DISABLE_SET_CMD_LEN]  = {_BCM_TIME_BS_NTP_TOD_DISABLE_SET};
    int command_len = BS_NTP_TOD_DISABLE_SET_CMD_LEN;

    uint8  response[BS_NTP_TOD_DISABLE_SET_RESP_LEN] = {0};
    int response_len = BS_NTP_TOD_DISABLE_SET_RESP_LEN;
    int rv;

    command[1] = ntp_tod_disable;
    ...
    if (response_len != BS_NTP_TOD_DISABLE_SET_RESP_LEN) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
                                "response_len != BS_NTP_TOD_DISABLE_SET_RESP_LEN\n")));
        return BCM_E_INTERNAL;
    }
    if (response[0] != command[0]) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
...
#define BS_PTP_TOD_DISABLE_SET_CMD_LEN  (1 + 1)
#define BS_PTP_TOD_DISABLE_SET_RESP_LEN (1 + 1)
int
_bcm_time_bs_ptp_tod_disable(int unit, int ptp_tod_disable)
{
    uint8  command[BS_PTP_TOD_DISABLE_SET_CMD_LEN]  = {_BCM_TIME_BS_PTP_TOD_DISABLE_SET};
    int command_len = BS_PTP_TOD_DISABLE_SET_CMD_LEN;

    uint8  response[BS_PTP_TOD_DISABLE_SET_RESP_LEN] = {0};
    int response_len = BS_PTP_TOD_DISABLE_SET_RESP_LEN;
    int rv;

    command[1] = ptp_tod_disable;
    ...
    if (response_len != BS_PTP_TOD_DISABLE_SET_RESP_LEN) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
                                "response_len != BS_PTP_TOD_DISABLE_SET_RESP_LEN\n")));
        return BCM_E_INTERNAL;
    }
    if (response[0] != command[0]) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
...
#define PTP_OFFSET_CMD_LEN  (1 + 1 + 8 + 4)
#define PTP_OFFSET_RESP_LEN (1 + 1)
int
_bcm_time_bs_ptp_offset_set(int unit, bcm_time_spec_t new_offset)
{
    uint8  command[PTP_OFFSET_CMD_LEN]  = {_BCM_TIME_PTP_OFFSET_SET};
    int command_len = PTP_OFFSET_CMD_LEN;

    uint8  response[PTP_OFFSET_RESP_LEN] = {0};
    int response_len = PTP_OFFSET_RESP_LEN;
    int rv;
