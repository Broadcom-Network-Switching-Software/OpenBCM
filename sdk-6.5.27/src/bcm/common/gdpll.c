 ...
 *
 *      _gdpll_flush
 *      bcm_common_gdpll_init
 *      bcm_common_gdpll_deinit
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
extern void _bcm_bs_process_chan_error(int unit, int chan);
#endif
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
        case bcmGdpllPortEventSynthClk:
            READ_NS_REGr(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLAr, 0, idx*4, &regVal);
            soc_reg_field_set(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLAr, &regVal,
                              UP_PERIOD_Nf, divisor);
            WRITE_NS_REGr(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLAr, 0, idx*4, regVal);
            WRITE_NS_REGr(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLAr, 0, idx*4, regVal);

            READ_NS_REGr(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLBr, 0, idx*4, &regVal);
            soc_reg_field_set(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLBr, &regVal,
                              DOWN_PERIOD_Nf, divisor_hi);
            WRITE_NS_REGr(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLBr, 0, idx*4, regVal);
            WRITE_NS_REGr(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLBr, 0, idx*4, regVal);
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
            READ_NS_REGr(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLAr, 0, idx*4, &regVal);
            soc_reg_field_set(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLAr, &regVal,
                              UP_PERIOD_Nf, divisor);
            WRITE_NS_REGr(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLAr, 0, idx*4, regVal);

            READ_NS_REGr(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLBr, 0, idx*4, &regVal);
            soc_reg_field_set(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLBr, &regVal,
                              DOWN_PERIOD_Nf, divisor_hi);
            WRITE_NS_REGr(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLBr, 0, idx*4, regVal);
    ...
    switch (input_event) {
        ...
        case bcmGdpllInputEventPORT:
            if ((port_event_type == bcmGdpllPortEventRXCDR) ||
                (port_event_type == bcmGdpllPortEventSynthClk)) {
                *pId = 35 + (port*2);
...
#if 0
/* Clean up later when confirmed not required. */
                    ...
                    bcm_port_t *pPort)
{
    int rv  = BCM_E_NONE;
#if defined(BCM_NANOSYNC_V1_SUPPORT)
    bcm_gdpll_chan_t *pCurrentGdpllChan;
#elif defined(BCM_MONTEREY_SUPPORT)
    int idx = 0;
    bcm_port_t port;
#endif
...
#if 0
    /* Clean up later when confirmed not required. */
    SOC_IF_ERROR_RETURN(_gdpll_event_get(unit, eventId, pEvent_misc, pPort));
#else
    pCurrentGdpllChan = &(pGdpllContext->dpll_chan[chan].gdpll_chan_config);
    *pPort_event_type = isFb ? pCurrentGdpllChan->event_fb.port_event_type : 
                         pCurrentGdpllChan->event_ref.port_event_type ;
    *pEvent_misc = isFb ? pCurrentGdpllChan->event_fb.input_event :
                         pCurrentGdpllChan->event_ref.input_event ;
#endif
...
#if 0
    /* Same info is already available as eventId recvd from caller. */
    SOC_IF_ERROR_RETURN(_gdpll_event_id_get(unit, event_misc, 0, 0, &idx));
    bsl_printf("\n dest misc_set: event_misc:%d idx:%d eventId:%d", event_misc, idx, eventId);
#else
    idx = eventId;
    bsl_printf("\n dest misc_set: event_misc:%d idx:%d ", event_misc, idx);
#endif
    ...
    switch(port_event_type) {
        ...
        case bcmGdpllPortEventSynthClk:
            ...
            soc_reg_field_set(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_MSBr, &regVal,
                              EVENT_IDf, eventId);
            WRITE_NS_REGr(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_MSBr, 0, (idx*8), regVal);
            bsl_printf("dest port_set: fwd_cfg_%d_msb :%x\n",idx, regVal);
            ...
            soc_reg_field_set(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_LSBr, &regVal,
                              CHK_PROFILEf, chkProfile);
            WRITE_NS_REGr(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_LSBr, 0, (idx*8), regVal);
            bsl_printf("dest port_set: fwd_cfg_%d_lsb :%x\n",idx, regVal);
            break;
                                 ...
                                 int enable)
{
    ...
    switch (port_event_type) {
        ...
        case bcmGdpllPortEventSynthClk:
            ...
            READ_NS_REGr(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLAr, 0, port*4, &regVal);
            if (soc_reg_field_get(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLAr, regVal, ENABLE_Nf) != enable) {
                soc_reg_field_set(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLAr, &regVal, ENABLE_Nf, enable ? 1:0);
                WRITE_NS_REGr(unit, NSYNC_TIMESYNC_RX_CDR_EVENT_CTRLAr, 0, port*4, regVal);
            }
#elif defined(BCM_MONTEREY_SUPPORT)
        ...
        case bcmGdpllPortEventTXPI:
            ...
            READ_NS_REGr(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLAr, 0, port*4, &regVal);
            soc_reg_field_set(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLAr, &regVal, ENABLE_Nf, enable ? 1:0);
            WRITE_NS_REGr(unit, NSYNC_TIMESYNC_TX_PI_CLK_EVENT_CTRLAr, 0, port*4, regVal);
#elif defined(BCM_MONTEREY_SUPPORT)
...
int
bcm_common_gdpll_deinit(int unit)
{
    int rv = BCM_E_NONE;
    gdpll_context_t *pGdpllContext = pGdpllCtx[unit];
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
        if (!(pGdpllContext->dpll_chan[chan].curr_state.state & BCM_GDPLL_CHAN_STATE_ERROR) &&
            (dpllParam.dpll_state.largeErrorIndicator)) {
            _bcm_bs_process_chan_error(unit, chan);
        }
#endif
...
#endif
{
    ...
    while (count < (BCM_GDPLL_NUM_CHANNELS/32)) {
        ...
        if (soc_reg_field_get(unit, ts_int_status_r, int_status, gdpll_ia_status[count])) {
            ...
            while ((chan_idx < 32) && (stat>>chan_idx)) {
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
                    READ_NS_REGr(unit, NS_GDPLL_IA_ERROR_CODEr, 0, (chan_num+chan_idx)*4, &regVal);
#else
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
                    _bcm_bs_process_chan_error(unit, (chan_num+chan_idx));
#endif
...
int
bcm_common_gdpll_init(int unit)
{
    int rv = BCM_E_NONE;
    uint32 regVal = 0;
    uint32 alloc_sz = 0;
    ...
    if ((debug_thread_pri >= 0) &&  (debug_thread_pri < 255)) {
        ...
        if (pGdpllContext->debug_sem == NULL) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                      "bcm_common_gdpll_init: Error !! failed to create the sem\n")));
              return (BCM_E_MEMORY);
        }
    ...
    if (pGdpllContext->cb_sem == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META_U(unit,
                  "bcm_common_gdpll_init: Error !! failed to create the cb_sem\n")));
          return (BCM_E_MEMORY);
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
        /* Clear the fwd cfg */
        reg = BCM_GDPLL_FWD_CFG_START;
        for (regCount = 0; regCount < (BCM_GDPLL_NUM_FWD_CFG_ENTRIES*2); regCount++) {
        ...
        for (count = 0; count < BCM_GDPLL_NUM_CHANNELS; count++) {
            ...
            READ_NS_REGr(unit, NS_GDPLL_IA_WDOG_CONFIGr, 0, (count*4), &regVal);
            soc_reg_field_set(unit, NS_GDPLL_IA_WDOG_CONFIGr, &regVal,
                              ia_wdog_interval_f, 0xFFFFF);
            soc_reg_field_set(unit, NS_GDPLL_IA_WDOG_CONFIGr, &regVal,
                              ia_wdog_enable_f, 1);
            WRITE_NS_REGr(unit, NS_GDPLL_IA_WDOG_CONFIGr, 0, (count*4), regVal);
        }
...
#if defined(BCM_DNX_SUPPORT)
            if (dnx_port_to_nanosync_output_port_get(unit,
                     pGdpllChan->port, pGdpllChan->lane_num,
                     (int *)&idx) != BCM_E_NONE) {
                rv = BCM_E_PARAM;
                goto exit;
            }
#else
            /* Add handling for non-DNX chips as required in future. */
#endif
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
            (pChanInput->port_event_type == bcmGdpllPortEventSynthClk) ||
#endif
...
{
    int rv = BCM_E_NONE;
    bcm_gdpll_input_event_t event_misc = 0;
    bcm_gdpll_port_event_t port_event_type = 0;
    bcm_port_t port = 0;
    gdpll_context_t *pGdpllContext = pGdpllCtx[unit];
                          ...
                          bcm_gdpll_chan_t *pGdpllChan, int *pChan)
{
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
#if defined(BCM_DNX_SUPPORT)
            rv = dnx_port_to_nanosync_input_port_get(unit,
                     pCurrentGdpllChan->event_fb.port,
                     pCurrentGdpllChan->event_fb.lane_num,
                     pCurrentGdpllChan->event_fb.port_event_type, &gdpll_port);
            if (rv != BCM_E_NONE) {
                rv = BCM_E_PARAM;
                goto exit;
            }
    ...
    } /* BCM_GDPLL_EVENT_CONFIG_FB */

    /* Reference event forward configuration */
    if((flags & BCM_GDPLL_EVENT_CONFIG_REF) &&
       (pCurrentGdpllChan->chan_dpll_config.phase_counter_ref == 0)) {
        if (pCurrentGdpllChan->event_ref.input_event == bcmGdpllInputEventPORT) {
#if defined(BCM_NANOSYNC_V1_SUPPORT)
#if defined(BCM_DNX_SUPPORT)
            rv = dnx_port_to_nanosync_input_port_get(unit,
                     pCurrentGdpllChan->event_ref.port,
                     pCurrentGdpllChan->event_ref.lane_num,
                     pCurrentGdpllChan->event_ref.port_event_type, &gdpll_port);
            if (rv != BCM_E_NONE) {
                rv = BCM_E_PARAM;
                goto exit;
            }
            ...
            /* Add handling for non-DNX chips as required in future. */
#endif
            bsl_printf("\nGDPLL: REF lPort:%d gdpll_port:%d\n",
...
#if defined(BCM_NANOSYNC_V1_SUPPORT)
#if defined(BCM_DNX_SUPPORT)
            rv = dnx_port_to_nanosync_output_port_get(unit,
                     pCurrentGdpllChan->port,
                     pCurrentGdpllChan->lane_num,
                     &gdpll_port);
            if (rv != BCM_E_NONE) {
                rv = BCM_E_PARAM;
                goto exit;
    ...
    /* Check and setup interpolator if required. */
    {
        uint32 regVal, iaAddr_updated = BCM_GDPLL_NUM_IA_ENTRIES;
        uint32 dest = BCM_GDPLL_EVENT_DEST_FIFO2;

        pGdpllContext->dpll_chan[chan].iA_fb = iaAddr_fb;
        pGdpllContext->dpll_chan[chan].iA_ref = iaAddr_ref;

        _bcm_gdpll_interpolator_setup(unit, pGdpllChan, pChan, &iaAddr_updated);

        if (iaAddr_updated != BCM_GDPLL_NUM_IA_ENTRIES)
        {
            /* Update dest to fwd events to firmware */
            READ_NS_REGr(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_MSBr, 0, (eventId_ref*8), &regVal);
            WRITE_NS_REGr(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_MSBr, 0, (eventId_ref*8), regVal);

            READ_NS_REGr(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_LSBr, 0, (eventId_ref*8), &regVal);
            dest |= soc_reg_field_get(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_LSBr,
                    regVal, DESTf);
            soc_reg_field_set(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_LSBr, &regVal,
                              DESTf, dest);
            WRITE_NS_REGr(unit, NS_FWD_TIMESYNC_TS_EVENT_FWD_CFG_0_LSBr, 0, (eventId_ref*8), regVal);

            /* Update IA update config */
            pGdpllContext->dpll_chan[chan].iA_ref = iaAddr_updated;
#if defined(BCM_NANOSYNC_V1_SUPPORT)
            READ_NS_REGr(unit, NS_GDPLL_IA_UPDATE_CONFIGr, 0, (chan*4), &regVal);
#else
            READ_NS_REGr(unit, NS_GDPLL_IA_UPDATE_CONFIGr, chan, 0, &regVal);
#endif
            soc_reg_field_set(unit, NS_GDPLL_IA_UPDATE_CONFIGr, &regVal, ia_update_ref_id_f, iaAddr_updated);
#if defined(BCM_NANOSYNC_V1_SUPPORT)
            WRITE_NS_REGr(unit, NS_GDPLL_IA_UPDATE_CONFIGr, 0, (chan*4), regVal);
#else
            WRITE_NS_REGr(unit, NS_GDPLL_IA_UPDATE_CONFIGr, chan, 0, regVal);
            ...
            LOG_VERBOSE(BSL_LS_BCM_PTP,
                        (BSL_META_U(unit,
                            " Updated IA_Addr:%d for evt_ref:%d \n"),
                            iaAddr_updated, eventId_ref));
        }
        else {
            LOG_VERBOSE(BSL_LS_BCM_PTP, (BSL_META_U(unit, " IA_Addr not updated. \n")));
        }
    }
...
bcm_common_gdpll_chan_state_get(int unit, int chan, uint32 *dpllState)
{
    ...
    uint8 lock;
#if defined(BCM_NANOSYNC_V1_SUPPORT)
    uint32_t m7_int_en = 0;
    uint32_t ia_int_en = 0;
#endif
    gdpll_context_t *pGdpllContext = pGdpllCtx[unit];
    ...
    m7_int_en = soc_reg_field_get(unit, ts_int_status_r, regVal, int_m7_int_status_f);
    ia_int_en = soc_reg_field_get(unit, ts_int_status_r, regVal, GDPLL_IA_STATUSf);

    if (pGdpllContext->gdpll_cb[bcmGdpllCbChanState] || m7_int_en || ia_int_en)
#else
 ...
 */
int
bcm_common_gdpll_debug_cb_register(int unit,
