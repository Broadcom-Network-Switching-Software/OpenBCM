...
/* 
 * Function:
 *      bcm_qos_dcn_int_pri_override_t_init
 * Purpose:
 *      Initialize the QoS DCN internal priority override structure
 * Parameters:
 *      int_pri_override - Pointer to the struct to be init'ed
 */
void
bcm_qos_dcn_int_pri_override_t_init(bcm_qos_dcn_int_pri_override_t *int_pri_override)
{
    if (int_pri_override != NULL) {
        sal_memset(int_pri_override, 0, sizeof(*int_pri_override));
    }
    return;
}
