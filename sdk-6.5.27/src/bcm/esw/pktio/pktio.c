...
#define PKTIO_F_HIGIG2  0x4
#define PKTIO_F_LBHDR   0x8
...
esw_pktio_init(int unit)
{
    ...
    if (!(pi->flags & PKTIO_F_HIGIG3)) {
        rv = bcmi_esw_pktio_hg2_fid_support_get(unit,
                                                &pi->support.higig2_support);
        if (SHR_SUCCESS(rv)) {
            pi->flags |= PKTIO_F_HIGIG2;
        } else if (rv != SHR_E_UNAVAIL) {
            SHR_RETURN_VAL_EXIT(rv);
        }
    }
    ...
    if (!(pi->flags & PKTIO_F_GIH)) {
        rv = bcmpkt_lbhdr_fid_support_get(pi->dev_type, &lbhdr_support);
        if (SHR_SUCCESS(rv)) {
            SHR_BITCOPY_RANGE(pi->support.lbhdr_support.pbits, 0,
                              lbhdr_support.fbits,
                              0, BCMPKT_LBHDR_FID_COUNT);
            pi->flags |= PKTIO_F_LBHDR;
        } else if (rv != SHR_E_UNAVAIL) {
            SHR_RETURN_VAL_EXIT(rv);
        }
    }
                             ...
                             int fid, uint32_t val)
{
    ...
    switch (type) {
        ...
        case bcmPktioPmdTypeHigig2:
            if (!(pi->flags & PKTIO_F_HIGIG2)) {
                SHR_RETURN_VAL_EXIT(SHR_E_UNAVAIL);
            }
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpkt_higig_get((bcmpkt_packet_t *)packet, &hghdr));
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmi_esw_pktio_hg2_field_set(unit, hghdr, fid, val));
            break;

        case bcmPktioPmdTypeLbhdr:
            if (!(pi->flags & PKTIO_F_LBHDR)) {
                SHR_RETURN_VAL_EXIT(SHR_E_UNAVAIL);
            }
                             ...
                             int fid, uint32 *val)
{
    ...
    case bcmPktioPmdTypeHigig2:
        ...
        /* Get from Module Header if Higig buffer in packet->pmd is NULL */
        if (!hghdr[0]) {
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpkt_rxpmd_get((bcmpkt_packet_t *)packet, &hghdr));
        }

        SHR_IF_ERR_VERBOSE_EXIT
            (bcmi_esw_pktio_hg2_field_get(unit, hghdr, fid, val));
    ...
    case bcmPktioPmdTypeLbhdr:
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (bcmpkt_lbhdr_field_get(pi->dev_type, lbhdr, fid, val));
