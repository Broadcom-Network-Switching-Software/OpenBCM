...
/*! For next pointer copy at the end of a free buffer. */
#define BP_BUF_NEXT_CP(_bp, _buf)   (((uint8_t **)(_buf))[_bp->buf_end])

/*! Save next pointer into a free buffer. */
#define BP_BUF_NEXT_SET(_bp, _buf, _val)   \
    (((uint8_t **)(_buf))[_bp->buf_end] = ((uint8_t **)(_buf))[0] = _val)
...
typedef struct bcmpkt_bpool_s {
    ...
    /* DMA address. */
    uint64_t dma_addr;
    /* Offset of the last word in a free buffer. */
    uint32_t buf_end;
} bcmpkt_bpool_t;
...
bcmpkt_bpool_create(int unit, int buf_size, int buf_count)
{
    ...
    bp->buf_end = bp->buf_size / sizeof(uint8_t *) - 1;
    ...
    for (i = 0; i < bp->buf_count - 1; i++) {
        ...
        BP_BUF_NEXT_SET(bp, buf, next_buf);
        buf = next_buf;
    }
    BP_BUF_NEXT_SET(bp, buf, NULL);
...
bcmpkt_bpool_alloc(int unit, uint32_t size, uint32_t *buf_size)
{
    ...
    if (!bp->active) {
        sal_spinlock_unlock(bp->lock);
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit, "Buffer pool is not created\n")));
        return NULL;
    }
    ...
    buf = bp->free_list;
    if (BP_BUF_NEXT(buf) != BP_BUF_NEXT_CP(bp, buf)) {
        sal_spinlock_unlock(bp->lock);
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "Buffer %p was corrupted %p != %p [%p,%p] %d\n"),
                   buf, BP_BUF_NEXT(buf), BP_BUF_NEXT_CP(bp, buf),
                   bp->mem, bp->mem_end, bp->buf_size));
        return NULL;
    }
...
bcmpkt_bpool_free(int unit, void *buf)
{
    ...
    BP_BUF_NEXT_SET(bp, buf, bp->free_list);
    bp->free_list = buf;
    bp->free_count++;
...
bcmpkt_bpool_status_get(int unit, bcmpkt_bpool_status_t *status)
{
    ...
    /*
     * If buffer pool created, validates lock for protection.
     * Otherwise, simply returns status.
     */
    if (bp->lock) {
        SHR_IF_ERR_EXIT
            (sal_spinlock_lock(bp->lock));
