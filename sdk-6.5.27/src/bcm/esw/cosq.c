...
/* FIXME_KP: Does the following line have to change to SOC_IS_TOMAHAWK or TOMAHAWK2? */
    }
#endif /* BCM_TOMAHAWK3_SUPPORT */
#ifdef BCM_TOMAHAWK_SUPPORT
    if (SOC_IS_TOMAHAWKX(unit)) {
 ...
 *      port - port to configure, -1 for any port.
 *      cosq - COS queue to configure, -1 for any COS queue.
 *      options - BCM_COSQ_BST_STAT_CLEAR_ON_READ.
...
int
bcm_esw_cosq_pause_queue_status_get(int unit,
                                bcm_gport_t gport,
                                uint8* pause_status)
{
#ifdef BCM_TOMAHAWK3_SUPPORT
    if (SOC_IS_TOMAHAWK3(unit)) {
        return bcm_th3_cosq_pause_queue_status_get(unit, gport, pause_status);
    }
#endif

#ifdef BCM_TOMAHAWK_SUPPORT
    if (SOC_IS_TOMAHAWKX(unit)) {
        return bcm_th_cosq_pause_queue_status_get(unit, gport, pause_status);
    }
#endif /* BCM_TOMAHAWK_SUPPORT */

#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit)) {
        return bcm_td3_cosq_pause_queue_status_get(unit, gport, pause_status);
    }
#endif /* BCM_TRIDENT3_SUPPORT */

#ifdef BCM_TRIDENT2_SUPPORT
    if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TITAN2X(unit)) {
        return bcm_td2_cosq_pause_queue_status_get(unit, gport, pause_status);
    }
#endif /* BCM_TRIDENT2_SUPPORT */
