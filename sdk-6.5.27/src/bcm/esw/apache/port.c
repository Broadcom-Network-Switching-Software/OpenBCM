        ...
        if (pr->physical_port == -1) {
            ...
            for (j = i; j < nport; j++) {
                ...
                if (resource[j].port == pr->port) {  /* Found */
                    ...
                    BCM_IF_ERROR_RETURN(bcm_esw_port_encap_get(unit, pr->port,
                                                               &cur_encap));
                    /* Check if mapping is same */
                    if (resource[j].physical_port !=
                        si->port_l2p_mapping[pr->port]) {
                        pr->flags |= SOC_PORT_RESOURCE_REMAP;
                        if (resource[j].flags & SOC_PORT_RESOURCE_SPEED) {
                            LOG_ERROR(BSL_LS_BCM_PORT,(BSL_META_U
                                (unit, "Port Mapping doesn't match\n")));
                            return BCM_E_CONFIG;
                        }
                    }

                    /* Check if lanes and encap mode are same */
                    if ((si->port_num_lanes[pr->port] != resource[j].lanes) ||
                        (cur_encap != resource[j].encap)) {
                        if (resource[j].flags & SOC_PORT_RESOURCE_SPEED) {
                            LOG_ERROR(BSL_LS_BCM_PORT,(BSL_META_U
                                (unit, "Lanes or Encap mode doesn't match\n")));
                            return BCM_E_CONFIG;
                        }
                    }
                    break;
                                    ...
                                    bcm_port_resource_t *resource)
{
    ...
    for (i = 0; i < nport; i++) {
        /*
         * Translate from public to internal flags.
         */
        if (resource[i].flags & BCM_PORT_RESOURCE_SPEED_ONLY) {
            port_resource[i].flags = SOC_PORT_RESOURCE_SPEED;
        }
    ...
    for (i = 0; i < nport; i++) {
        if ((port_resource[i].physical_port == -1) ||
            !(port_resource[i].flags & SOC_PORT_RESOURCE_SPEED)) {
            continue;
        }

        /* Need to follow remaining speed_set() sequence.
         * Will not lead to another flexport operation since
         * speed change is already done and soc structures have been updated above
         */
        rv = bcmi_esw_portctrl_speed_set(unit, resource[i].port, resource[i].speed);
        if (BCM_FAILURE(rv)) {
            sal_free(port_resource);
            return rv;
        }
 ...
 * Note:
 *      - Assumes caller has lock.
 *      - This is to be called by the bcmi_esw_portctrl_speed_set().
 */
STATIC int
_bcm_apache_port_resource_speed_set(int unit, bcm_port_t port, int speed)
{
...
bcmi_apache_port_lanes_set(int unit, bcm_port_t port, int lanes)
{
    ...
    cur_lanes = si->port_num_lanes[port];
    if (cur_lanes == lanes) {
        return BCM_E_NONE;
    }
...
typedef int bcm_esw_ap_port_not_empty; /* Make ISO compilers happy. */
