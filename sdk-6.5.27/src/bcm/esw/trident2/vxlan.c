...
_bcm_td3_vxlan_bookkeeping_t   *_bcm_td3_vxlan_bk_info[BCM_MAX_NUM_UNITS] = { 0 };
#define TD3_VXLAN_INFO(_unit_)  (_bcm_td3_vxlan_bk_info[_unit_])

/*  From src/bcm/esw/triumph2/qos.c */
#define _BCM_QOS_MAP_SHIFT                  10
#define _BCM_QOS_MAP_CHUNK_DSCP             64
/* Use the last PHB_MAPPING_TBL_2_STRENGTH_PROFILE to select L3_IIF_PHB. */
#define PHB_TBL_2_STRENGTH_PROFILE_L3_IIF   15
/* Numer of CTRL_ID used for FLEX PHB. 0 to 5 leave for FIX PHB. */
#define TUNNEL_ADAPT_PHB_CTRL_ID_FLEX_CNT   10
#endif
...
STATIC int
_bcm_td3_vxlan_phb_dscp_tunnel_term_init(int unit, bcm_tunnel_terminator_t *tnl_info, int32* phb_ctrl_id);
#endif
...
_bcm_td2_vxlan_free_resource(int unit)
{
...
#ifdef BCM_TRIDENT3_SUPPORT
    _bcm_td3_vxlan_bookkeeping_t *td3_vxlan_info;
#endif
    ...
    if (SOC_IS_TRIDENT3X(unit)) {
        if (NULL == TD3_VXLAN_INFO(unit)) {
            return;
        }

        td3_vxlan_info = TD3_VXLAN_INFO(unit);
        if (td3_vxlan_info->phb_map_bitmap) {
            sal_free(td3_vxlan_info->phb_map_bitmap);
            td3_vxlan_info->phb_map_bitmap = NULL;
        }

        if (td3_vxlan_info->phb_map_hwid_ctrl_id) {
            sal_free(td3_vxlan_info->phb_map_hwid_ctrl_id);
            td3_vxlan_info->phb_map_hwid_ctrl_id = NULL;
        }

        if (td3_vxlan_info->phb_sel_used) {
            sal_free(td3_vxlan_info->phb_sel_used);
            td3_vxlan_info->phb_sel_used = NULL;
        }

        if (td3_vxlan_info->phb_sel_refcount) {
            sal_free(td3_vxlan_info->phb_sel_refcount);
            td3_vxlan_info->phb_sel_refcount = NULL;
        }

        sal_free(TD3_VXLAN_INFO(unit));
        TD3_VXLAN_INFO(unit) = NULL;
    }
#endif
}
...
_bcm_td2_vxlan_wb_recover(int unit)
{
    ...
    if (vxlan_state != NULL) {
         ...
         *
         * Version_1_4 -> Version_1_9: + (flags, dscp_sel, dscp_map, dscp, ttl
         *                                and src_port)
         */
        ...
        /* Add flags */
        add_sz_1_9 = sizeof(uint32);

        /* Add dscp_sel if applicable */
#if defined(INCLUDE_L3)
        add_sz_1_9 += sizeof(bcm_tunnel_dscp_select_t);
#endif
        /* Add dscp_map, dscp, ttl, src_port and tnl_idx */
        add_sz_1_9 += + sizeof(int) + sizeof(int) + sizeof(int) + sizeof(uint16);

        if (recovered_ver >= BCM_WB_VERSION_1_9) {
            /* Update this size in case additional members are added. */
            additional_scache_size = 0;
        }else if (recovered_ver >= BCM_WB_VERSION_1_4) {
            additional_scache_size = num_vp * add_sz_1_9 * 2;
        ...
        if (recovered_ver >= BCM_WB_VERSION_1_4 &&
            recovered_ver <= BCM_WB_VERSION_1_8) {
            for (i = 0; i < num_vp; i++) {
                sal_memcpy(&(vxlan_info->vxlan_tunnel_term[i]),
                    vxlan_state,
                    add_sz_1_0 + add_sz_1_1 + add_sz_1_2 +
                    add_sz_1_4);
                vxlan_state += add_sz_1_0 + add_sz_1_1 + add_sz_1_2 +
                               add_sz_1_4;
            }
        ...
        /*
         * Recover match_etag_vid of each Match Key
         */
        if (recovered_ver >= BCM_WB_VERSION_1_8) {
            for (i = 0; i < num_vp; i++) {
                sal_memcpy(&(vxlan_info->match_key[i].match_etag_vlan), vxlan_state,
                           sizeof(bcm_vlan_t));
                vxlan_state += sizeof(bcm_vlan_t);
            }
        } else {
            additional_scache_size += num_vp * (sizeof(bcm_vlan_t));
        }
...
_bcm_td2_vxlan_tunnel_initiator_reinit(int unit)
{
    ...
    tnl_init_extd_fields = soc_property_get(unit,
                   spn_VXLAN_TUNNEL_INIT_EXT_FIELDS_UPDATE, 0);
    for (i = idx_min; i <= idx_max; i++) {
        ...
        if (_BCM_VXLAN_TUNNEL_TYPE ==
            ...
            if (tnl_init_extd_fields) {
                tnl_init_info.udp_src_port =
                    soc_mem_field32_get(unit, EGR_IP_TUNNELm,
                                        egr_tnl_entry, L4_SRC_PORTf);
                tnl_init_info.dscp_sel =
                    soc_mem_field32_get(unit, EGR_IP_TUNNELm,
                                        egr_tnl_entry, DSCP_SELf);
...
_bcm_td2_vxlan_reinit(int unit)
{
    ...
    int d = 0;
#ifdef BCM_TRIDENT3_SUPPORT
    int phb_ctrl_id = 0, map_id = 0;
    uint8 *phb_buf = NULL;
    soc_cancun_ceh_field_info_t fld1, fld2, fld3, fld4;
    _bcm_td3_vxlan_bookkeeping_t *td3_vxlan_info;
    uint64 matchid;
    uint32 z2_matchid_rval, z2_matchid_off, z2_matchid_mask;
    td3_vxlan_info = TD3_VXLAN_INFO(unit);
#endif
    vxlan_info = VXLAN_INFO(unit);
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if (key_type == TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN) {
            if (soc_mem_field32_get(unit, mem, entry, VIF__MPLS_ACTIONf) != 0x1) {
                continue;
            }
            vp = soc_mem_field32_get(unit, mem, entry, VIF__SOURCE_VPf);
        } else {
        ...
        if (_bcm_vp_used_get(unit, vp, _bcmVpTypeVxlan)) {
...
#endif
            {
                ...
                } else if (TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN == key_type) {
                    vxlan_info->match_key[vp].flags =
                                            _BCM_VXLAN_PORT_MATCH_TYPE_PORT_ETAG_VLAN;
                    vxlan_info->match_key[vp].match_vlan =
                        soc_mem_field32_get(unit, mem, entry, VIF__VLANf);
                    vxlan_info->match_key[vp].match_etag_vlan =
                        soc_mem_field32_get(unit, mem, entry, VIF__SRC_VIFf);
                }
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if ((key_type != _BCM_VXLAN_KEY_TYPE_TUNNEL) &&
            ...
            (key_type != _BCM_VXLAN_FLEX_KEY_TYPE_LOOKUP_DIP) &&
        ...
        #if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(unit, soc_feature_tunnel_term_dscp_phb) &&
            (key_type == _BCM_VXLAN_FLEX_KEY_TYPE_LOOKUP_DIP)) {
            if (!(phb_ctrl_id = soc_mem_field32_get(unit, mem, entry,
                VXLAN_FLEX_IPV4_DIP__TUNNEL_ADAPT_PHB_CTRL_IDf))) {
                continue;
            } else {
                SHR_BITSET(td3_vxlan_info->phb_sel_used, phb_ctrl_id);
                td3_vxlan_info->phb_sel_refcount[phb_ctrl_id]++;
            }
        }
        #endif
    }
    ...
    if (soc_feature(unit, soc_feature_tunnel_term_dscp_phb)) {
        ...
        sal_memset(&fld1, 0, sizeof(soc_cancun_ceh_field_info_t));
        rv = soc_cancun_ceh_obj_field_get(unit, "Z2_MATCH_ID", "VXLAN",
                                    &fld1);
        ...
        sal_memset(&fld2, 0, sizeof(soc_cancun_ceh_field_info_t));
        rv =  soc_cancun_ceh_obj_field_get(unit, "PKT_FLOW_ID_2",
                                    "TUNNEL_TERMINATED", &fld2);
        if (SOC_FAILURE(rv)) {
            goto clean_up;
        }

        /* Get Offset and bit width for Zone2 match id. */
        sal_memset(&fld3, 0, sizeof(soc_cancun_ceh_field_info_t));
        ...
        z2_matchid_off = fld3.offset;
        z2_matchid_mask = (((1 << fld3.width)) -1 );

        /* Recovery based on entry in MPLS tbl */

        mem = PHB_SELECT_TCAMm;
        /* Get the memory index information*/
        sal_memset(&fld4, 0, sizeof(soc_cancun_ceh_field_info_t));
        SOC_IF_ERROR_RETURN
            (soc_cancun_ceh_obj_field_get(unit,
                  "PHB_SELECT_TCAM_RANGE",
                  "PHB_SELECT_TCAM_MAX_IDX", &fld4));
        index_max = fld4.value;

        sal_memset(&fld4, 0, sizeof(soc_cancun_ceh_field_info_t));
        SOC_IF_ERROR_RETURN
             (soc_cancun_ceh_obj_field_get(unit,
                 "PHB_SELECT_TCAM_RANGE",
                 "PHB_SELECT_TCAM_MIN_IDX", &fld4));
        index_min = fld4.value;

        buf_size = SOC_MEM_TABLE_BYTES(unit, mem);
        phb_buf = soc_cm_salloc(unit, buf_size, "PHB_SELECT_ENTRY buffer");
        if (NULL == phb_buf) {
            rv = BCM_E_MEMORY;
            goto clean_up;
        }
        rv = soc_mem_read_range(unit, mem, MEM_BLOCK_ANY, index_min, index_max, phb_buf);
        if (SOC_FAILURE(rv)) {
            goto clean_up;
        }

        for (i = index_min; i <= index_max; i++) {
            entry = soc_mem_table_idx_to_pointer(unit, mem, void *, phb_buf, i);
            if (soc_mem_field32_get(unit, mem, entry, VALIDf) == 0) {
                continue;
            }

            soc_mem_field64_get(unit, mem, entry, MATCH_IDf, &matchid);

            /* Get z2 match id value */
            COMPILER_64_SHR(matchid, z2_matchid_off);
            z2_matchid_rval = COMPILER_64_LO(matchid) & (z2_matchid_mask);
            z2_matchid_rval >>= fld1.offset;

            /* Looking for entries of type vxlan tunnel term only */
            if ((z2_matchid_rval != fld1.value) ||
                   (soc_mem_field32_get(unit, mem,
                                        entry,
                                        PKT_FLOW_ID_2f)!= fld2.value)) {
                continue;
            }
            if (soc_mem_field32_get(unit, mem, entry,
                               TABLE_2_FLEX_MAPPED_PHB_ENABLEf)) {
                map_id  = soc_mem_field32_get(unit, mem, entry,
                               MAPPING_TBL_2_PHB_PROFILE_PTRf);
                SHR_BITSET(td3_vxlan_info->phb_map_bitmap, map_id);
                phb_ctrl_id = soc_mem_field32_get(unit, mem, entry,
                                     TUNNEL_ADAPT_PHB_CTRL_IDf);
                td3_vxlan_info->phb_map_hwid_ctrl_id[map_id] = phb_ctrl_id;
            }
        }
    }
    #endif
...
bcm_td2_vxlan_init(int unit)
{
    ...
    if (SOC_IS_TRIDENT3X(unit)) {
        _bcm_td3_vxlan_bookkeeping_t *td3_vxlan_info;
        int phb_map_profiles = 0;
        int phb_sel_num = 0;
        soc_field_info_t *field_info = NULL;
        int phb_ctrl_field_len;

        field_info = soc_mem_fieldinfo_get(unit, PHB_SELECT_TCAMm,
                          TUNNEL_ADAPT_PHB_CTRL_IDf);
        ...
        phb_ctrl_field_len = field_info->len ;
        phb_sel_num = (1 << phb_ctrl_field_len);

        /* Allocate BK Info */
        if (NULL == TD3_VXLAN_INFO(unit)) {
            BCMI_VXLAN_ALLOC(TD3_VXLAN_INFO(unit), sizeof(_bcm_td3_vxlan_bookkeeping_t),
                         "vxlan_phb_bk_module_data");
            if (NULL == TD3_VXLAN_INFO(unit)) {
                _bcm_td2_vxlan_free_resource(unit);
                return (BCM_E_MEMORY);
            }
        ...
        /* phb_map entry mapped to phb_select */
        phb_map_profiles = soc_mem_index_count(unit,
                           PHB_MAPPING_TBL_2m)/_BCM_QOS_MAP_CHUNK_DSCP;

        td3_vxlan_info->phb_map_bitmap =
        sal_alloc(SHR_BITALLOCSIZE(phb_map_profiles), "vxlan_phb_map_bitmap");
        if (td3_vxlan_info->phb_map_bitmap == NULL) {
            _bcm_td2_vxlan_free_resource(unit);
            return BCM_E_MEMORY;
        }
        sal_memset(td3_vxlan_info->phb_map_bitmap, 0,
                     SHR_BITALLOCSIZE(phb_map_profiles));

        /* phb_map entry mapping to phb select entry */
        td3_vxlan_info->phb_map_hwid_ctrl_id =
            sal_alloc(sizeof(uint8) * phb_map_profiles, "phb_hw_idx");
        if (td3_vxlan_info->phb_map_hwid_ctrl_id == NULL) {
            _bcm_td2_vxlan_free_resource(unit);
            return BCM_E_MEMORY;
        }
        sal_memset(td3_vxlan_info->phb_map_hwid_ctrl_id, 0,
                  (sizeof(uint8) * phb_map_profiles));

        /* for phb select tcam entry allocation */
        td3_vxlan_info->phb_sel_used =
        sal_alloc(SHR_BITALLOCSIZE(phb_sel_num), "vxlan_phb_sel_bitmap");
        if (td3_vxlan_info->phb_sel_used == NULL) {
            _bcm_td2_vxlan_free_resource(unit);
            return BCM_E_MEMORY;
        }
        sal_memset(td3_vxlan_info->phb_sel_used, 0,
                     SHR_BITALLOCSIZE(phb_sel_num));

       /* phb_map entry mapping to phb select entry */
        td3_vxlan_info->phb_sel_refcount =
            sal_alloc(sizeof(uint16) * phb_sel_num, "phb_sel_refcount");
        if (td3_vxlan_info->phb_sel_refcount == NULL) {
            _bcm_td2_vxlan_free_resource(unit);
            return BCM_E_MEMORY;
        }
        sal_memset(td3_vxlan_info->phb_sel_refcount, 0,
                  (sizeof(uint16) * phb_sel_num));
...
#ifdef BCM_TRIDENT3_SUPPORT
        /*
         * Create a PHB_MAPPING_TBL_2_STRENGTH_PROFILEm profile for
         * PHB_SELECT_TCAM to select L3_IIF_PHB.
         */
        if (SOC_IS_TRIDENT3X(unit) && rv == BCM_E_NONE &&
            (soc_feature(unit, soc_feature_tunnel_term_dscp_phb))) {
            soc_mem_t mem = PHB_MAPPING_TBL_2_STRENGTH_PROFILEm;
            uint32 entry[SOC_MAX_MEM_WORDS];
            uint32 memfld[16];

            rv = soc_mem_read(unit, mem, MEM_BLOCK_ALL, 1, entry);
            if (rv == BCM_E_NONE) {
                sal_memset(memfld, 0, sizeof(memfld));
                memfld[0] = 0x10;
                soc_mem_field_set(unit, mem, entry, PHB_SELECT_PROFILE_PTRf,
                                  memfld);
                memfld[0] = 2;
                soc_mem_field_set(unit, mem, entry, Z2_HDR_1_PHB_ENCODINGf,
                                  memfld);
                memfld[0] = 0x3f;
                soc_mem_field_set(unit, mem, entry, Z4_HDR_1_PHB_ENCODINGf,
                                  memfld);
                memfld[0] = 0x9;
                soc_mem_field_set(unit, mem, entry, Z3_HDR_3_PHB_ENCODINGf,
                                  memfld);
                memfld[0] = 0xa;
                soc_mem_field_set(unit, mem, entry, Z3_HDR_4_PHB_ENCODINGf,
                                  memfld);
                rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL,
                                   PHB_TBL_2_STRENGTH_PROFILE_L3_IIF, entry);
            }
        }
#endif
    }
...

{
    ...
    if (soc_feature(unit, soc_feature_ing_l3_next_hop_encoded_dest)) {
        ...
        dest = soc_ING_L3_NEXT_HOPm_field32_get(unit, &ing_nh, DESTINATIONf);
        if (!BCMI_L3_NH_DEST_IS_DVP(dest)) {
...
bcm_td2_vxlan_vpn_create(int unit, bcm_vxlan_vpn_config_t *info)
{
    ...
    if ((!(info->flags & (BCM_VXLAN_VPN_ELINE | BCM_VXLAN_VPN_ELAN))) ||
    ...
    if (info->flags & BCM_VXLAN_VPN_ELINE) {
        ...
        soc_VFIm_field32_set(unit, &vfi_entry, PT2PT_ENf, 0);
    ...
    bcm_tunnel_terminator_t *tnl_info, uint32 *tr_ent, uint8 out_mode, int clean_flag)
{
    ...
    if (tnl_info->flags & BCM_TUNNEL_TERM_KEEP_INNER_DSCP) {
        soc_mem_field32_set(unit, mem, tr_ent,
            BCM_VXLAN_DIP_FIELD(d,DONOT_CHANGE_INNER_HDR_DSCPf), 0x1);
    }
}
...
#if defined(BCM_TRIDENT3_SUPPORT)
/*
 * TUNNEL_ADAPT_PHB_CTRL_ID[3:0]
 *   [0] - DONOT_CHANGE_INNER_HDR_DSCP, ignore it;
 *   [2:1] - overlap with fixed PHB, but value=0x3 is not used by fixed
 *           PHB that can be used for flex PHB CTRL_ID.
 */
STATIC int
_bcm_td3_phb_ctrl_id_alloc(int unit, bcm_tunnel_terminator_t *tnl_info, SHR_BITDCL *bitmap, int map_size)
{
    int i;

    /* Leave 0 - 5 to fixed PHB. */
...
/*
 * Function:
 *      _bcm_td3_vxlan_phb_dscp_tunnel_term_init
 * Purpose:
 *      del TPID
 * Parameters:
 *      IN :  Unit
...
STATIC int
_bcm_td3_vxlan_phb_dscp_tunnel_term_init(int unit, bcm_tunnel_terminator_t *tnl_info, int32* ctrl_id){
    int phb_map_profiles;
    _bcm_td3_vxlan_bookkeeping_t *td3_vxlan_info = TD3_VXLAN_INFO(unit);
    soc_cancun_ceh_field_info_t fld1, fld2, fld3, fld4;
    int mem_index;
    uint32 entry[SOC_MAX_MEM_WORDS], all_ones[SOC_MAX_MEM_WORDS];
    uint32 memfld[16];
    uint64 matchid;
    int hw_map_idx = 0, phb_ctrl_id = 0;
    soc_mem_t mem;

    BCM_IF_ERROR_RETURN
        (_bcm_tr2_qos_id2idx(unit,
               tnl_info->qos_map_id, &hw_map_idx));

    /* phb_map entry mapped to phb_select */
    phb_map_profiles = soc_mem_index_count(unit,
                       PHB_MAPPING_TBL_2m)/_BCM_QOS_MAP_CHUNK_DSCP;

    if (hw_map_idx > phb_map_profiles) {
        LOG_ERROR(BSL_LS_BCM_VXLAN,
             (BSL_META_U(unit,
                       "Invalid map profile id(%d)"),hw_map_idx));
        return BCM_E_RESOURCE;
    }

    /*
     * Program the phb_select_tcam entry for the first tunnel
     * that uses the map
     */
    if (!SHR_BITGET(td3_vxlan_info->phb_map_bitmap, hw_map_idx)) {
        int max_ctrl_id = 64;
        int mem_idx_start = 0, mem_idx_max = 0;
        soc_cancun_ceh_field_info_t fld;
        soc_field_info_t *field_info = NULL;

        /* Get the memory index information*/
        sal_memset(&fld, 0, sizeof(soc_cancun_ceh_field_info_t));
        SOC_IF_ERROR_RETURN
            (soc_cancun_ceh_obj_field_get(unit,
                  "PHB_SELECT_TCAM_RANGE",
                  "PHB_SELECT_TCAM_MAX_IDX", &fld));
        mem_idx_max = fld.value;

        sal_memset(&fld, 0, sizeof(soc_cancun_ceh_field_info_t));
        SOC_IF_ERROR_RETURN
             (soc_cancun_ceh_obj_field_get(unit,
                 "PHB_SELECT_TCAM_RANGE",
                 "PHB_SELECT_TCAM_MIN_IDX", &fld));
        mem_idx_start = fld.value;
        ...
        max_ctrl_id = ((1 << (field_info->len)) - 1) ;

        /* profile id and not allocated phb ctrl previously */
        phb_ctrl_id = _bcm_td3_phb_ctrl_id_alloc(unit,
                   tnl_info, td3_vxlan_info->phb_sel_used, max_ctrl_id);

        if (phb_ctrl_id < 0) {
            LOG_ERROR(BSL_LS_BCM_VXLAN,
                 (BSL_META_U(unit,
                           "Invalid profile(%d)"),hw_map_idx));
            return BCM_E_RESOURCE;
        }
        td3_vxlan_info->phb_map_hwid_ctrl_id[hw_map_idx] = phb_ctrl_id;
        SHR_BITSET(td3_vxlan_info->phb_sel_used, phb_ctrl_id);
        td3_vxlan_info->phb_sel_refcount[phb_ctrl_id] = 1;

        /* use soc mem write to program the phb_select_tcam entry */
        sal_memset(&fld1, 0, sizeof(soc_cancun_ceh_field_info_t));
        SOC_IF_ERROR_RETURN
            (soc_cancun_ceh_obj_field_get(unit, "Z2_MATCH_ID", "VXLAN",
                                    &fld1));

        sal_memset(&fld2, 0, sizeof(soc_cancun_ceh_field_info_t));
        SOC_IF_ERROR_RETURN
             (soc_cancun_ceh_obj_field_get(unit,
                             "PKT_FLOW_ID_2", "TUNNEL_TERMINATED",
                             &fld2));

        sal_memset(&fld3, 0, sizeof(soc_cancun_ceh_field_info_t));
        SOC_IF_ERROR_RETURN
            (soc_cancun_ceh_obj_field_get(unit,
                             "PHB_FROM_TUNNEL_DSCP_DURING_TERM",
                             "MAPPING_TBL_2_STRENGTH_PROFILE_IDX", &fld3));
        mem_index = phb_ctrl_id + mem_idx_start;

        if (mem_index > mem_idx_max) {
            return (BCM_E_MEMORY);
        }
        ...
        /* Get Offset for Zone2 match id. */
        sal_memset(&fld4, 0, sizeof(soc_cancun_ceh_field_info_t));
        SOC_IF_ERROR_RETURN
            (soc_cancun_ceh_obj_field_get(unit, "MATCH_ID",
               "Z2_MATCH_ID", &fld4));

        /* prepare the match id field value. */
        COMPILER_64_SET(matchid, 0, 0);
        COMPILER_64_SET(matchid, 0, fld1.value);
        COMPILER_64_SHL(matchid, (fld4.offset + fld1.offset));
        soc_mem_field64_set(unit, mem, entry, MATCH_IDf, matchid);
        soc_mem_field64_set(unit, mem, entry, MATCH_ID_MASKf, matchid);

        sal_memset(memfld, 0, sizeof(memfld));
        memfld[0] = fld2.value;
        soc_mem_field_set(unit, mem, entry, PKT_FLOW_ID_2f, memfld);
        soc_mem_field_set(unit, mem, entry, PKT_FLOW_ID_2_MASKf, memfld);

        sal_memset(memfld, 0, sizeof(memfld));
        memfld[0] = phb_ctrl_id;
        soc_mem_field_set(unit, mem, entry,
                      TUNNEL_ADAPT_PHB_CTRL_IDf, memfld);

        sal_memset(memfld, 0, sizeof(memfld));
        memfld[0] = 1;
        soc_mem_field_set(unit, mem, entry,
                           TABLE_2_FLEX_MAPPED_PHB_ENABLEf, memfld);

        sal_memset(memfld, 0, sizeof(memfld));
        memfld[0] = hw_map_idx;
        soc_mem_field_set(unit, mem, entry,
                          MAPPING_TBL_2_PHB_PROFILE_PTRf, memfld);

        sal_memset(memfld, 0, sizeof(memfld));
        memfld[0] = fld3.value;
        soc_mem_field_set(unit, mem, entry,
                          MAPPING_TBL_2_STRENGTH_PROFILE_IDXf, memfld);
        ...
        memfld[0] = fld3.value;
        soc_mem_field_set(unit, mem, entry,
                          CNG_STRENGTH_PROFILE_IDXf, memfld);
        ...
        SOC_IF_ERROR_RETURN
            (soc_cancun_ceh_obj_field_get(unit,
                             "PHB_FROM_TUNNEL_DSCP_DURING_TERM",
                             "INT_PRI_STRENGTH_PROFILE_IDX", &fld3));
        memfld[0] = fld3.value;
        soc_mem_field_set(unit, mem, entry,
                          INT_PRI_STRENGTH_PROFILE_IDXf, memfld);
        ...
        soc_mem_field_width_fit_set(unit, mem,
                  entry, TUNNEL_ADAPT_PHB_CTRL_ID_MASKf, all_ones);
        ...
        if (tnl_info->flags & BCM_TUNNEL_TERM_L3_IIF_PHB) {
            sal_memset(memfld, 0, sizeof(memfld));
            /* L2. */
            soc_mem_field_set(unit, mem, entry, MY_STATION_STATUSf, memfld);
            memfld[0] = 1;
            soc_mem_field_set(unit, mem, entry, MY_STATION_STATUS_MASKf, memfld);

            SOC_IF_ERROR_RETURN
                (soc_mem_write(unit, mem, MEM_BLOCK_ALL, mem_index, entry));

            memfld[0] = PHB_TBL_2_STRENGTH_PROFILE_L3_IIF;
            soc_mem_field_set(unit, mem, entry,
                              MAPPING_TBL_2_STRENGTH_PROFILE_IDXf, memfld);
            ...
            memfld[0] = 1;
            soc_mem_field_set(unit, mem, entry, MY_STATION_STATUSf, memfld);
            mem_index += TUNNEL_ADAPT_PHB_CTRL_ID_FLEX_CNT;
        }

        SOC_IF_ERROR_RETURN
            (soc_mem_write(unit, mem, MEM_BLOCK_ALL, mem_index, entry));

        SHR_BITSET(td3_vxlan_info->phb_map_bitmap, hw_map_idx);
    } else {
        /*
         * Not the first tunnel that uses the map,
         * so increment the ref count
         */
        phb_ctrl_id = td3_vxlan_info->phb_map_hwid_ctrl_id[hw_map_idx];
        td3_vxlan_info->phb_sel_refcount[phb_ctrl_id]++;
    }
    *ctrl_id = phb_ctrl_id;
    return BCM_E_NONE;
...
STATIC int
_bcm_td3_vxlan_phb_dscp_tunnel_term_flags(int unit, bcm_tunnel_terminator_t *tnl_info, int32 phb_ctrl_id){
    soc_mem_t mem;
    uint32 memfld[16];
    uint32 entry[SOC_MAX_MEM_WORDS];
    soc_cancun_ceh_field_info_t fld;
    ...
    if (phb_ctrl_id > max_ctrl_id || mem_index > mem_idx_max) {
        return (BCM_E_INTERNAL);
    }
    ...
    soc_mem_field_get(unit, mem, entry, MY_STATION_STATUS_MASKf, memfld);
    if (memfld[0] == 1) {
        tnl_info->flags |= BCM_TUNNEL_TERM_L3_IIF_PHB;
    }
...
bcm_td2_vxlan_tunnel_terminator_create(int unit, bcm_tunnel_terminator_t *tnl_info)
{
    ...
    if (rv == SOC_E_NOT_FOUND) {
        #if defined(BCM_TRIDENT3_SUPPORT)
        if (((tnl_info->qos_map_id >> _BCM_QOS_MAP_SHIFT) ==
        ...
        #if defined(BCM_TRIDENT3_SUPPORT)
        /* Program the qos phb tables */
        /* get the hw_idx for map id */

        if ((tnl_info->qos_map_id >> _BCM_QOS_MAP_SHIFT) ==
                _BCM_QOS_MAP_TYPE_DSCP_TABLE) {
             /* Flex PHB doesn't support legacy flags. */
            if (tnl_info->flags & (BCM_TUNNEL_TERM_KEEP_INNER_DSCP |
                                   BCM_TUNNEL_TERM_USE_OUTER_DSCP |
                                   BCM_TUNNEL_TERM_USE_OUTER_PCP)) {
                LOG_ERROR(BSL_LS_BCM_VXLAN,
                    (BSL_META_U(unit,
                                "Tunnel PHB doesn't support legacy flags\n")));

                return BCM_E_PARAM;
            }
            BCM_IF_ERROR_RETURN
                (_bcm_td3_vxlan_phb_dscp_tunnel_term_init(unit,
                         tnl_info, &phb_ctrl_id));

            sal_memset(memfld, 0, sizeof(memfld));
            memfld[0] = phb_ctrl_id;
            soc_mem_field_set(unit, mem, vxlate_entry,
                         VXLAN_FLEX_IPV4_DIP__TUNNEL_ADAPT_PHB_CTRL_IDf, memfld);
        } /* Map id is dscp map */
        #endif
    ...
    if (BCM_SUCCESS(rv) && (tunnel_idx != -1)) {
       ...
       if (tnl_info->flags & BCM_TUNNEL_TERM_KEEP_INNER_DSCP) {
           vxlan_info->vxlan_tunnel_term[tunnel_idx].activate_flag |=
               _BCM_VXLAN_TUNNEL_TERM_KEEP_INNER_DSCP;
       }
    }
    ...
    if (_bcm_td2_vxlan_tunnel_terminator_reference_count(unit, tunnel_dip, 2) == 1)  {
...
#endif
        {
         ...
         if (soc_feature(unit, soc_feature_tunnel_term_dscp_phb)) {
             int mem_idx;
             soc_cancun_ceh_field_info_t fld;
             int num_map = soc_mem_index_count(unit,
                           PHB_MAPPING_TBL_2m)/_BCM_QOS_MAP_CHUNK_DSCP;

             phb_ctrl_id = soc_mem_field32_get(unit, mem,
                            vxlate_entry,
                            VXLAN_FLEX_IPV4_DIP__TUNNEL_ADAPT_PHB_CTRL_IDf);

             if (td3_vxlan_info->phb_sel_refcount[phb_ctrl_id] > 0) {
                 td3_vxlan_info->phb_sel_refcount[phb_ctrl_id]--;
                 if (td3_vxlan_info->phb_sel_refcount[phb_ctrl_id] == 0) {
                     for (hw_map_id = 0; hw_map_id < num_map; hw_map_id++) {
                         if ((SHR_BITGET(td3_vxlan_info->phb_map_bitmap,
                                     hw_map_id)) &&
                              td3_vxlan_info->phb_map_hwid_ctrl_id[hw_map_id]
                                    == phb_ctrl_id) {
                             SHR_BITCLR(td3_vxlan_info->phb_sel_used, phb_ctrl_id);
                             SHR_BITCLR(td3_vxlan_info->phb_map_bitmap, hw_map_id);
                             td3_vxlan_info->phb_map_hwid_ctrl_id[hw_map_id] = 0;
                             break;
                         }
                     }
                     ...
                     SOC_IF_ERROR_RETURN
                         (soc_cancun_ceh_obj_field_get(unit,
                                    "PHB_SELECT_TCAM_RANGE",
                                    "PHB_SELECT_TCAM_MIN_IDX", &fld));
                     mem_idx = fld.value + phb_ctrl_id;

                     /* clear the phb tcam entry. */
                     SOC_IF_ERROR_RETURN
                        (soc_mem_write(unit, PHB_SELECT_TCAMm, MEM_BLOCK_ALL,
                                       mem_idx,
                                      &soc_mem_entry_null(unit, PHB_SELECT_TCAMm)));
                     /* clear the phb tcam L3 entry. */
                     SOC_IF_ERROR_RETURN
                        (soc_mem_write(unit, PHB_SELECT_TCAMm, MEM_BLOCK_ALL,
                                       mem_idx + TUNNEL_ADAPT_PHB_CTRL_ID_FLEX_CNT,
                                      &soc_mem_entry_null(unit, PHB_SELECT_TCAMm)));
                  }
              }
          }
    ...
    if (!SOC_HW_RESET(unit) && (SOC_SWITCH_BYPASS_MODE(unit) == SOC_SWITCH_BYPASS_MODE_NONE)) {
        ...
        for (i = 0; i < nent; i++) {
            ...
            if (soc_feature(unit, soc_feature_tunnel_term_dscp_phb)) {
                ...
                phb_ctrl_id = soc_mem_field32_get(unit, vxlate_mem,
                            vtabp, VXLAN_FLEX_IPV4_DIP__TUNNEL_ADAPT_PHB_CTRL_IDf);
                if (td3_vxlan_info->phb_sel_refcount[phb_ctrl_id] > 0) {
                    if (--td3_vxlan_info->phb_sel_refcount[phb_ctrl_id] == 0) {
                       for (hw_map_id = 0; hw_map_id < num_map; hw_map_id++) {
                          if ((SHR_BITGET(td3_vxlan_info->phb_map_bitmap,
                        ...
                        /* clear the phb tcam entry. */
                        SOC_IF_ERROR_RETURN
                            (soc_mem_write(unit, PHB_SELECT_TCAMm,
                                       MEM_BLOCK_ALL, mem_idx,
                                       &soc_mem_entry_null(unit, PHB_SELECT_TCAMm)));
                        /* clear the phb tcam L3 entry. */
                        SOC_IF_ERROR_RETURN
                            (soc_mem_write(unit, PHB_SELECT_TCAMm,
                                       MEM_BLOCK_ALL, mem_idx + TUNNEL_ADAPT_PHB_CTRL_ID_FLEX_CNT,
                                       &soc_mem_entry_null(unit, PHB_SELECT_TCAMm)));
                    }
                }
            }
...
bcm_td2_vxlan_tunnel_terminator_get(int unit, bcm_tunnel_terminator_t *tnl_info)
{
    ...
    if (soc_feature(unit, soc_feature_tunnel_term_dscp_phb)) {
        int num_map = soc_mem_index_count(unit,
                      PHB_MAPPING_TBL_2m)/_BCM_QOS_MAP_CHUNK_DSCP;
        phb_ctrl_id = soc_mem_field32_get(unit,
                          mem, vxlate_entry,
                          VXLAN_FLEX_IPV4_DIP__TUNNEL_ADAPT_PHB_CTRL_IDf);
        if (td3_vxlan_info->phb_sel_refcount[phb_ctrl_id] > 0) {
            for (hw_map_id = 0; hw_map_id < num_map; hw_map_id++) {
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_tr2_qos_idx2id(unit, hw_map_id,
                             _BCM_QOS_MAP_TYPE_DSCP_TABLE,
                             &tnl_info->qos_map_id));
        }
        BCM_IF_ERROR_RETURN
            (_bcm_td3_vxlan_phb_dscp_tunnel_term_flags(unit, tnl_info, phb_ctrl_id));
    }
#endif
    return BCM_E_NONE;
}
...
/*
 * Function:
 *      _bcm_td2_current_tunnel_term_delete
 * Purpose:
 *      Delete the tunnel initiator entry associated
 *      with a tunnel
 * Parameters:
 *      unit - Device Number
 * Returns:
...
int _bcm_td2_current_tunnel_term_delete(int unit, bcm_tunnel_initiator_t *info)
{
    _bcm_td2_vxlan_bookkeeping_t *vxlan_info;
     int tunnel_idx = -1, curr_initiator_tnl_idx = 0;
     bcm_tunnel_initiator_t tmp_info;

     vxlan_info = VXLAN_INFO(unit);
     sal_memset(&tmp_info, 0, sizeof(tmp_info));

     tunnel_idx = BCM_GPORT_TUNNEL_ID_GET(info->tunnel_id);
     tmp_info.type = bcmTunnelTypeVxlan;
     tmp_info.sip = vxlan_info->vxlan_tunnel_init[tunnel_idx].sip;
     tmp_info.udp_dst_port =
         vxlan_info->vxlan_tunnel_init[tunnel_idx].tunnel_state;
     tmp_info.udp_src_port =
         vxlan_info->vxlan_tunnel_init[tunnel_idx].src_port;
     tmp_info.flags = vxlan_info->vxlan_tunnel_init[tunnel_idx].flags;
     tmp_info.dscp_sel =
         vxlan_info->vxlan_tunnel_init[tunnel_idx].dscp_sel;
     tmp_info.dscp_map =
         vxlan_info->vxlan_tunnel_init[tunnel_idx].dscp_map;
     tmp_info.dscp = vxlan_info->vxlan_tunnel_init[tunnel_idx].dscp;
     tmp_info.ttl = vxlan_info->vxlan_tunnel_init[tunnel_idx].ttl;
     if (BCM_E_NONE == _bcm_xgs3_l3_tnl_init_find(unit, &tmp_info,
                                             &curr_initiator_tnl_idx)) {
         bcm_xgs3_tnl_init_del(unit, 0, curr_initiator_tnl_idx);
     }
...
bcm_td2_vxlan_tunnel_initiator_create(int unit, bcm_tunnel_initiator_t *info)
{
    ...
    int tnl_init_extd_fields = 0, idx = 0, ref_count = 0, num_vp = 0;
    ...
    if (tnl_init_extd_fields == 1) {
        if (match_entry_present) {
            if (info->flags & BCM_TUNNEL_REPLACE) {
                if (new_idx_flag) {
                    /* Replace is being attempted, but there is no matching
                       entry, return error */
                    return BCM_E_UNAVAIL;
                }
                /* Adjust ref-counts since entry uses it now */
                bcm_xgs3_tnl_init_ref_cnt_incr(unit, tnl_idx);
                /* We have found a matching entry, delete the current one */
                BCM_IF_ERROR_RETURN(_bcm_td2_current_tunnel_term_delete(unit, info));
            } else {
                /* Adjust ref-counts since entry uses it now */
                bcm_xgs3_tnl_init_ref_cnt_incr(unit, tnl_idx);
            }
             ...
             if (info->flags & BCM_TUNNEL_REPLACE) {
                 /* Match entry is not present, delete the current one, so a new
                    entry can be alloted */
                 BCM_IF_ERROR_RETURN(_bcm_td2_current_tunnel_term_delete(unit, info));
             }
    ...
    if (soft_tnl_idx !=- 1) {
         ...
         if (tnl_init_extd_fields) {
             vxlan_info->vxlan_tunnel_init[soft_tnl_idx].flags = info->flags &
             (BCM_TUNNEL_INIT_USE_INNER_DF | BCM_TUNNEL_INIT_IPV4_SET_DF);
             vxlan_info->vxlan_tunnel_init[soft_tnl_idx].dscp_sel = info->dscp_sel;
             vxlan_info->vxlan_tunnel_init[soft_tnl_idx].dscp_map = info->dscp_map;
             vxlan_info->vxlan_tunnel_init[soft_tnl_idx].dscp = info->dscp;
             vxlan_info->vxlan_tunnel_init[soft_tnl_idx].ttl = info->ttl;
             vxlan_info->vxlan_tunnel_init[soft_tnl_idx].src_port = info->udp_src_port;
         }
...
{
    _bcm_td2_vxlan_bookkeeping_t *vxlan_info;
    int soft_tnl_idx=-1, tnl_idx=-1;
    int num_vp = 0, ref_count = 0, idx = 0;
    uint32 flags = 0;
    bcm_ip_t sip=0;
    ...
    soft_tnl_idx = BCM_GPORT_TUNNEL_ID_GET(vxlan_tunnel_id);
    _BCM_VXLAN_TUNNEL_ID_IF_INVALID_RETURN(unit, soft_tnl_idx);

    sip = vxlan_info->vxlan_tunnel_init[soft_tnl_idx].sip;
    udp_dst_port =  vxlan_info->vxlan_tunnel_init[soft_tnl_idx].tunnel_state;
    if (sip == 0) {
        return BCM_E_NOT_FOUND;
    }
    ...
    if (tnl_init_extd_fields) {
        /* Get the associated tunnel info */
        sal_memset(&info, 0, sizeof(bcm_tunnel_initiator_t));
        info.tunnel_id = vxlan_tunnel_id;

        /* Get the related fields from the soft entry */
        info.type = bcmTunnelTypeVxlan;
        info.sip = vxlan_info->vxlan_tunnel_init[soft_tnl_idx].sip;
        info.udp_dst_port =
            vxlan_info->vxlan_tunnel_init[soft_tnl_idx].tunnel_state;
        info.udp_src_port =
            vxlan_info->vxlan_tunnel_init[soft_tnl_idx].src_port;
        info.flags = vxlan_info->vxlan_tunnel_init[soft_tnl_idx].flags;
        info.dscp_sel =
            vxlan_info->vxlan_tunnel_init[soft_tnl_idx].dscp_sel;
        info.dscp_map =
            vxlan_info->vxlan_tunnel_init[soft_tnl_idx].dscp_map;
        info.dscp = vxlan_info->vxlan_tunnel_init[soft_tnl_idx].dscp;
        info.ttl = vxlan_info->vxlan_tunnel_init[soft_tnl_idx].ttl;

        /* Find the HW tunnel entry */
        if (BCM_E_NONE == _bcm_xgs3_l3_tnl_init_find(unit, &info, &tnl_idx)) {
            uint32 ref_cnt = 0;

            /* Get the ref-cnt for the HW tunnel entry */
            bcm_xgs3_tnl_init_ref_cnt_get(unit, tnl_idx, &ref_cnt);

            if (ref_cnt > 1) {
                /* Decrement the ref-cnt for the HW tunnel entry */
                bcm_xgs3_tnl_init_ref_cnt_decr(unit, tnl_idx);
            } else if (ref_cnt == 1) {
                /* Issue a delete since the ref-count is 1 */
                bcm_xgs3_tnl_init_del(unit, 0, tnl_idx);
            }
        }

        /* Reset the tunnel entry */
        vxlan_info->vxlan_tunnel_init[soft_tnl_idx].sip = 0;
        vxlan_info->vxlan_tunnel_init[soft_tnl_idx].dip = 0;
        vxlan_info->vxlan_tunnel_init[soft_tnl_idx].tunnel_state = 0;
    } else {
                                                     ...
                                                     int *hw_idx)
{
    ...
    if (tnl_init_extd_fields) {
        init_tnl_info.dip = vxlan_info->vxlan_tunnel_init[soft_idx].dip;
        init_tnl_info.flags = vxlan_info->vxlan_tunnel_init[soft_idx].flags;
        init_tnl_info.dscp_sel = vxlan_info->vxlan_tunnel_init[soft_idx].dscp_sel;
        init_tnl_info.dscp_map = vxlan_info->vxlan_tunnel_init[soft_idx].dscp_map;
        init_tnl_info.dscp = vxlan_info->vxlan_tunnel_init[soft_idx].dscp;
        init_tnl_info.ttl = vxlan_info->vxlan_tunnel_init[soft_idx].ttl;
        init_tnl_info.udp_src_port = vxlan_info->vxlan_tunnel_init[soft_idx].src_port;
    }
                                          ...
                                          int index, int *cmp_result)
{
    ...
    *cmp_result = BCM_L3_CMP_NOT_EQUAL;
    ...
    if (tnl_init_extd_fields) {
        ...
        if (tnl_entry->dscp_sel == bcmTunnelDscpMap) {
            BCM_IF_ERROR_RETURN(
                _bcm_tr2_qos_id2idx(unit, tnl_entry->dscp_map, &hw_map_idx));

            if (hw_map_idx != hw_entry.dscp_map) {
                return BCM_E_NONE;
            }
        } else {
            /* Compare dscp */
            if (tnl_entry->dscp != hw_entry.dscp) {
                return BCM_E_NONE;
            }
        }
        ...
        if (tnl_entry->udp_src_port != hw_entry.udp_src_port) {
            return BCM_E_NONE;
        }
    }
...
_bcm_td2_vxlan_tunnel_initiator_hash_calc(int unit, void *buf, uint16 *hash)
{
    ...
    if (tnl_init_extd_fields) {
        tnl_entry.udp_src_port =
            ((bcm_tunnel_initiator_t *)buf)->udp_src_port;

        tnl_entry.dscp_sel =
            ((bcm_tunnel_initiator_t *)buf)->dscp_sel;

        tnl_entry.dscp =
            ((bcm_tunnel_initiator_t *)buf)->dscp;

        tnl_entry.ttl =
            ((bcm_tunnel_initiator_t *)buf)->ttl;
    }
                         ...
                         int *tpid_index )
{
    ...
    if (vxlan_vpn_info != NULL ) {
        ...
        _bcm_td3_sd_tag_action_profile_get(unit, action_prt,
                         action_if_not_prt, &profile_index, 0, 0);
                  ...
                  _bcm_td2_vxlan_nh_info_t  *egr_nh_info, egr_l3_next_hop_entry_t* egr_nh)
{
    ...
    if ((egr_nh_info->sd_tag_action_not_present != -1) || (egr_nh_info->sd_tag_action_present != -1)) {
        ...
        _bcm_td3_sd_tag_action_profile_get(unit, egr_nh_info->sd_tag_action_present,
                                                      egr_nh_info->sd_tag_action_not_present, &profile_idx, 0, 0);
    ...
    if ((new_entry_type == BCM_VXLAN_TAG_VIEW(d)) &&
        (old_entry_type == _BCM_VXLAN_EGR_NEXT_HOP_L3_VIEW)) {
            hg_ppd_type = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                         L3__HG_HDR_SELf);
            if (SOC_MEM_FIELD_VALID(unit, EGR_L3_NEXT_HOPm, L3__CLASS_IDf)) {
                class_id = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                         L3__CLASS_IDf);
            }
            ...
            if (SOC_MEM_FIELD_VALID(unit, EGR_L3_NEXT_HOPm,
                                    BCM_VXLAN_TAG_FIELD(d, CLASS_IDf))) {
                soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                        BCM_VXLAN_TAG_FIELD(d, CLASS_IDf), class_id);
            }
        ...
        if (SOC_MEM_FIELD_VALID(unit, EGR_L3_NEXT_HOPm, L3MC__CLASS_IDf)) {
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                L3MC__CLASS_IDf, class_id);
        }
...
/*
 * Function:
 *      _bcm_td2_vxlan_match_vif_entry_update
 * Purpose:
 *      Update VXLAN Match Vlan_xlate Entry
 * Parameters:
 *  unit(IN)        :  Unit
 *  vpn(IN)         :  vpn
 *  vxlan_port(IN)  :  vxlan_port
 *  vent(IN)        :  vlan_xlate_entry_t
 *  return_ent(OUT) :  vlan_xlate_entry_t
 */
STATIC int
_bcm_td2_vxlan_match_vif_entry_update(int unit, bcm_vpn_t vpn,
                                         bcm_vxlan_port_t *vxlan_port,
                                         uint32 *vent, uint32 *return_ent)
{
    uint32  key_type=0, value=0;
    ...
    soc_mem_field32_set(unit, mem, return_ent, SOURCE_TYPEf, 1);
    value = soc_mem_field32_get(unit, mem, return_ent, VIF__MPLS_ACTIONf);
    /* SVP_VALID and L3_IIF_VALID overlay high and low bits of MPLS_ACTION. */
    if (SOC_MEM_FIELD_VALID(unit, mem, L3_IIF_VALIDf)) {
        value |= 0x1;
    } else {
    ...
    /* Retain original Keys -- Update data only */
    soc_mem_field32_set(unit, mem, return_ent, VIF__MPLS_ACTIONf, value);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        if (vxlan_port->flags & BCM_VXLAN_PORT_ENABLE_VLAN_CHECKS) {
            return BCM_E_UNAVAIL;
        }
        soc_mem_field32_set(unit, mem, return_ent, VIF__DISABLE_VLAN_CHECKSf,
                            0);
        soc_mem_field32_set(unit, mem, return_ent, VIF__VLAN_ACTION_VALIDf,
                            1);
    } else
#endif
    {
        ...
        if (!(vxlan_port->flags & BCM_VXLAN_PORT_ENABLE_VLAN_CHECKS)) {
            soc_mem_field32_set(unit, mem, return_ent,
                                VIF__VLAN_ACTION_VALIDf, 1);
        } else {
            soc_mem_field32_set(unit, mem, return_ent,
                                VIF__VLAN_ACTION_VALIDf, 0);
        }
    }
    ...
    if (soc_feature(unit, soc_feature_vp_sharing)) {
        if (vpn != BCM_VXLAN_VPN_INVALID) {
            int vfi = 0;
            _BCM_VXLAN_VPN_GET(vfi, _BCM_VPN_TYPE_VFI, vpn);
            soc_mem_field32_set(unit, mem, return_ent, VIF__VFIf, vfi);
        }
    }
#endif
    soc_mem_field32_set(unit, mem, return_ent, VIF__SOURCE_VPf, vp);
...
/*
 * Function:
 *        _bcm_td2_vxlan_match_vif_entry_set
 * Purpose:
 *       Set VXLAN Match Vxlate Entry
 * Parameters:
 *  IN :  Unit
...
STATIC int
_bcm_td2_vxlan_match_vif_entry_set(int unit, bcm_vpn_t vpn, bcm_vxlan_port_t *vxlan_port, uint32 *vent)
{
    int rv = BCM_E_NONE;
    int index = -1;
    bcm_module_t mod_out = -1;
    ...
    if (BCM_GPORT_IS_TRUNK(vxlan_port->match_port)) {
        ...
        soc_mem_field32_set(unit, mem, vent, VIF__TGIDf, trunk_id);
    } else {
    ...
    soc_mem_field32_set(unit, mem, vent, SOURCE_TYPEf, 1);
    soc_mem_field32_set(unit, mem, vent, VIF__MPLS_ACTIONf, 0x1); /* SVP */
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        ...
        soc_mem_field32_set(unit, mem, vent, VIF__DISABLE_VLAN_CHECKSf, 0);
        soc_mem_field32_set(unit, mem, vent, VIF__VLAN_ACTION_VALIDf, 1);
    } else
#endif
    {
        soc_mem_field32_set(unit, mem, vent, VIF__DISABLE_VLAN_CHECKSf, 1);
        if (!(vxlan_port->flags & BCM_VXLAN_PORT_ENABLE_VLAN_CHECKS)){
            soc_mem_field32_set(unit, mem, vent, VIF__VLAN_ACTION_VALIDf, 1);
        }
    }
    ...
    if (soc_feature(unit, soc_feature_vp_sharing)) {
        if (vpn != BCM_VXLAN_VPN_INVALID) {
            int vfi = 0;
            _BCM_VXLAN_VPN_GET(vfi, _BCM_VPN_TYPE_VFI, vpn);
            soc_mem_field32_set(unit, mem, vent, VIF__VFIf, vfi);
        }
    }
#endif
    ...
    if (BCM_E_NONE == rv) {
        if (vxlan_port->flags & BCM_VXLAN_PORT_REPLACE) {
            BCM_IF_ERROR_RETURN(
                _bcm_td2_vxlan_match_vif_entry_update(unit, vpn,
                vxlan_port, vent, return_vent));
            BCM_IF_ERROR_RETURN(
                soc_mem_write(unit, mem, MEM_BLOCK_ALL, index, return_vent));
        } else {
...
bcm_td2_vxlan_match_clear (int unit, int vp)
{
    ...
    vxlan_info->match_key[vp].match_etag_vlan = 0;
    vxlan_info->match_key[vp].trunk_id = -1;
...
/*
 * Function:
 *      _bcm_td2_vxlan_match_port_del
 * Purpose:
 *      Delete the MATCH_PORT criteria of an VXLAN port.
 * Parameters:
 *      unit    - (IN) Device Number
...
STATIC int
_bcm_td2_vxlan_match_port_del(int unit, bcm_gport_t vxlan_port_id, int vp) {
    _bcm_td2_vxlan_bookkeeping_t *vxlan_info = VXLAN_INFO(unit);
    bcm_vxlan_port_t curr;
    bcm_trunk_t trunk_id = BCM_TRUNK_INVALID;
    bcm_module_t mod_in = 0, mod_out = 0;
    bcm_port_t port_in = 0, port_out = 0;
    int src_trk_idx = 0;

    bcm_vxlan_port_t_init(&curr);
    curr.vxlan_port_id = vxlan_port_id;
    curr.criteria = BCM_VXLAN_PORT_MATCH_PORT;
    src_trk_idx = vxlan_info->match_key[vp].index;

    if (vxlan_info->match_key[vp].trunk_id != -1) {
        trunk_id = vxlan_info->match_key[vp].trunk_id;
        BCM_GPORT_TRUNK_SET(curr.match_port, trunk_id);
    } else {
        BCM_IF_ERROR_RETURN(
            _bcm_esw_src_modid_port_get(unit, src_trk_idx,
                                        &mod_in, &port_in));
        (void)_bcm_esw_stk_modmap_map(unit, BCM_STK_MODMAP_GET,
                                      mod_in, port_in,
                                      &mod_out, &port_out);
        curr.match_port =
            _bcm_esw_port_gport_get_from_modport(unit, mod_out, port_out);
    }
    curr.flags2 = (vxlan_info->match_key[vp].flags2 &
                   _BCM_VXLAN_PORT_MATCH_NO_UNTAG_OP) ?
                   BCM_VXLAN_PORT_FLAGS2_UT_VLAN_ACTION_NO_CLEAR : 0;
    BCM_IF_ERROR_RETURN(
        _bcm_td2_vxlan_match_delete(unit, vp, curr, 0));
...
_bcm_td2_vxlan_match_add(int unit, bcm_vxlan_port_t *vxlan_port, int vp, bcm_vpn_t vpn)
{
    ...
    /* stm_vp indicates the final SVP value of SOURCE_TRUNK_MAP for VP LAG. */
    int vplag_vp = 0, stm_vp = vp;
    int need_del_match = TRUE;
    ...
    if ((vxlan_port->flags & BCM_VXLAN_PORT_REPLACE) &&
        (vxlan_info->match_key[vp].flags == _BCM_VXLAN_PORT_MATCH_TYPE_PORT ||
         vxlan_info->match_key[vp].flags == _BCM_VXLAN_PORT_MATCH_TYPE_TRUNK)) {
        /* Note we need to get vplag_vp before match_delete. */
        vxlan_port_id = vxlan_port->vxlan_port_id;
        rv = bcm_td2_vxlan_port_source_vp_lag_get(unit,
                                                  vxlan_port_id,
                                                  &vplag_vp);
        if (BCM_SUCCESS(rv)) {
            stm_vp = vplag_vp;
        ...
        /*
         * For some replace operation, there is no change on the match_port,
         * Then don't do the port match delete.
         */
        if (vxlan_port->criteria == BCM_VXLAN_PORT_MATCH_PORT) {
            BCM_IF_ERROR_RETURN(
            ...
            if (BCM_GPORT_IS_TRUNK(vxlan_port->match_port)) {
                if ((vxlan_info->match_key[vp].flags ==
                    ...
                    (trunk_id == vxlan_info->match_key[vp].trunk_id)) {
                    need_del_match = FALSE;
                }
            } else {
                if ((vxlan_info->match_key[vp].flags ==
                    _BCM_VXLAN_PORT_MATCH_TYPE_PORT) &&
                    (port_out == vxlan_info->match_key[vp].port)) {
                    need_del_match = FALSE;
                }
            }
        }
        if (need_del_match) {
            BCM_IF_ERROR_RETURN(
                _bcm_td2_vxlan_match_port_del(unit, vxlan_port_id, vp));
        }
    }
    ...
    if (vxlan_port->criteria == BCM_VXLAN_PORT_MATCH_PORT_VLAN ) {
        ...
        if (BCM_GPORT_IS_TRUNK(vxlan_port->match_port)) {
            BCM_IF_ERROR_RETURN(
                bcm_td2_vxlan_match_trunk_add(
                    unit, trunk_id, stm_vp,
                    vxlan_port->flags & BCM_VXLAN_PORT_REPLACE,
                    !(vxlan_port->flags2 &
            ...
            if (temp != 0) {
                if ((temp == stm_vp) && (vxlan_port->flags & BCM_VXLAN_PORT_REPLACE)) {
                    /*continue*/
                } else {
                    return BCM_E_INTERNAL;
                }
...
match_add_clean1:
    ...
    } else if (vxlan_port->criteria == BCM_VXLAN_PORT_MATCH_PORT_ETAG_VLAN ) {
        if (!BCM_VLAN_VALID(vxlan_port->match_etag_vlan) ||
            !BCM_VLAN_VALID(vxlan_port->match_vlan)) {
             return BCM_E_PARAM;
        }

        soc_mem_field32_set(unit, mem, vent, KEY_TYPEf, TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_MEM_FIELD_VALID(unit, mem, DATA_TYPEf)) {
            soc_mem_field32_set(unit, mem, vent,
                DATA_TYPEf, TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
        }
#endif
        soc_mem_field32_set(unit, mem, vent, VIF__VLANf, vxlan_port->match_vlan);
        soc_mem_field32_set(unit, mem, vent, VIF__SRC_VIFf, vxlan_port->match_etag_vlan);

        rv = _bcm_td2_vxlan_match_vif_entry_set(unit, vpn, vxlan_port, vent);
        BCM_IF_ERROR_RETURN(rv);

        if (vxlan_info->match_key[vp].flags != _BCM_VXLAN_PORT_MATCH_TYPE_SHARED) {
            vxlan_info->match_key[vp].flags = _BCM_VXLAN_PORT_MATCH_TYPE_PORT_ETAG_VLAN;
            vxlan_info->match_key[vp].match_vlan = vxlan_port->match_vlan;
            vxlan_info->match_key[vp].match_etag_vlan = vxlan_port->match_etag_vlan;
        }
...
_bcm_td2_vxlan_match_delete(int unit,  int vp , bcm_vxlan_port_t vxlan_port, bcm_vpn_t vpn)
{
    ...
    if (vxlan_info->match_key[vp].flags == _BCM_VXLAN_PORT_MATCH_TYPE_PORT_ETAG_VLAN) {
        soc_mem_field32_set(unit, mem, vent, SOURCE_TYPEf, 1);
        soc_mem_field32_set(unit, mem, vent, KEY_TYPEf, TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
#if defined(BCM_TRIDENT3_SUPPORT)
        if (mem == VLAN_XLATE_1_DOUBLEm) {
            soc_mem_field32_set(unit, mem, vent, DATA_TYPEf, TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
        }
#endif
        soc_mem_field32_set(unit, mem, vent,
            VIF__VLANf, vxlan_info->match_key[vp].match_vlan);
        soc_mem_field32_set(unit, mem, vent,
            VIF__SRC_VIFf, vxlan_info->match_key[vp].match_etag_vlan);

        if (vxlan_info->match_key[vp].modid != -1) {
            soc_mem_field32_set(unit, mem, vent,
                VIF__MODULE_IDf, vxlan_info->match_key[vp].modid);
            soc_mem_field32_set(unit, mem, vent,
                VIF__PORT_NUMf, vxlan_info->match_key[vp].port);

        } else {
            soc_mem_field32_set(unit, mem, vent, VIF__Tf, 1);
            soc_mem_field32_set(unit, mem, vent,
                VIF__TGIDf, vxlan_info->match_key[vp].trunk_id);
        }
        rv = soc_mem_delete(unit, mem, MEM_BLOCK_ANY, &vent);
        BCM_IF_ERROR_RETURN(rv);

        bcm_td2_vxlan_port_match_count_adjust(unit, vp, -1);
    } else if (vxlan_info->match_key[vp].flags == _BCM_VXLAN_PORT_MATCH_TYPE_VLAN) {
        soc_mem_field32_set(unit, mem, vent, SOURCE_TYPEf, 1);
        soc_mem_field32_set(unit, mem, vent, KEY_TYPEf, TR_VLXLT_HASH_KEY_TYPE_OVID);
#if defined(BCM_TRIDENT3_SUPPORT)
        if (mem == VLAN_XLATE_1_DOUBLEm) {
        ...
        } else if (BCM_VXLAN_PORT_MATCH_PORT_ETAG_VLAN == vxlan_port.criteria) {
            if (!BCM_VLAN_VALID(vxlan_port.match_etag_vlan) ||
                !BCM_VLAN_VALID(vxlan_port.match_vlan)) {
                 return BCM_E_PARAM;
            }
            soc_mem_field32_set(unit, mem, vent,
                KEY_TYPEf, TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
#if defined(BCM_TRIDENT3_SUPPORT)
            if (mem == VLAN_XLATE_1_DOUBLEm) {
                soc_mem_field32_set(unit, mem, vent,
                    DATA_TYPEf, TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
...
_bcm_td2_vxlan_match_get(int unit, bcm_vxlan_port_t *vxlan_port, int vp, bcm_vpn_t vpn)
{
    ...
    } else if (vxlan_info->match_key[vp].flags & _BCM_VXLAN_PORT_MATCH_TYPE_PORT_ETAG_VLAN) {
        vxlan_port->criteria = BCM_VXLAN_PORT_MATCH_PORT_ETAG_VLAN;
        vxlan_port->match_vlan = vxlan_info->match_key[vp].match_vlan;
        vxlan_port->match_etag_vlan = vxlan_info->match_key[vp].match_etag_vlan;
        if (vxlan_info->match_key[vp].trunk_id != -1) {
             trunk_id = vxlan_info->match_key[vp].trunk_id;
             BCM_GPORT_TRUNK_SET(vxlan_port->match_port, trunk_id);
        } else {
...
_rollback:
    ...
    if (soc_feature(unit, soc_feature_base_valid)) {
        if (soc_mem_field32_get(unit, vxlate_mem, original_ment_evt, BASE_VALIDf) == 1) {
             valid = 1;
        } else {
            valid = 0;
        }
    ...
    if (valid) {
       ...
       (void) soc_mem_delete(unit, vxlate_mem, MEM_BLOCK_ALL, ment_evt);
    }

    if (soc_feature(unit, soc_feature_base_valid)) {
        if ((soc_mem_field32_get(unit, mmem, original_ment_vnid, BASE_VALID_0f) == 3) &&
            (soc_mem_field32_get(unit, mmem, original_ment_vnid, BASE_VALID_1f) == 7)) {
             valid = 1;
        } else {
            valid = 0;
        }
    } else {
        valid = soc_mem_field32_get(unit, mmem, original_ment_vnid, VALIDf);
    }
    if (valid) {
#if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(unit, soc_feature_vxlan_decoupled_mode)) {
        ...
        (void) soc_mem_delete(unit, mmem, MEM_BLOCK_ALL, ment_vnid);
    }
...
_bcm_td2_vxlan_port_vlan_get(int unit, int vp, bcm_vxlan_port_t *vxlan_port)
{
    ...
    if (_BCM_VXLAN_PORT_MATCH_TYPE_VLAN == vxlan_info->match_key[vp].flags) {
        ...
        soc_mem_field32_set(unit, mem, vent,
            XLATE__OTAGf, vxlan_info->match_key[vp].match_vlan);
    } else if (_BCM_VXLAN_PORT_MATCH_TYPE_PORT_ETAG_VLAN == vxlan_info->match_key[vp].flags) {
        soc_mem_field32_set(unit, mem, vent, SOURCE_TYPEf, 1);
        soc_mem_field32_set(unit, mem, vent, KEY_TYPEf, TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_MEM_FIELD_VALID(unit, mem, DATA_TYPEf)) {
    ...
    switch(mkey->flags) {
        ...
        case _BCM_VXLAN_PORT_MATCH_TYPE_TRUNK:
            ...
            if (local_port_count == 0)
                /* Return if no logical port index exists in source trunk. */
                return BCM_E_INTERNAL;
...
{
    int i, num_vp;
    char ip_str1[SAL_IPADDR_STR_LEN];
    char ip_str2[SAL_IPADDR_STR_LEN];
    _bcm_td2_vxlan_bookkeeping_t *vxlan_info = VXLAN_INFO(unit);

    LOG_CLI((BSL_META_U(unit,
                        "Tunnel Initiator Endpoints:\n")));
    LOG_CLI((BSL_META_U(unit, "----------------------------------------------------\n")));
    LOG_CLI((BSL_META_U(unit, "| Index |    Source IP    |     Dest IP     | VLAN |\n")));
    LOG_CLI((BSL_META_U(unit, "----------------------------------------------------\n")));
    for (i = 0; i < _BCM_MAX_NUM_VXLAN_TUNNEL; i++) {
        if (VXLAN_INFO(unit)->vxlan_tunnel_init[i].dip != 0 &&
            ...
            shr_format_ipaddr(ip_str1, vxlan_info->vxlan_tunnel_init[i].sip);
            shr_format_ipaddr(ip_str2, vxlan_info->vxlan_tunnel_init[i].dip);
            LOG_CLI((BSL_META_U(unit, "| %5d | %15s | %15s | %4d |\n"), i, ip_str1, ip_str2,
                                (int16)vxlan_info->vxlan_tunnel_init[i].vlan));
        }
    }
    LOG_CLI((BSL_META_U(unit, "----------------------------------------------------\n")));
    ...
    for (i = 0; i < _BCM_MAX_NUM_VXLAN_TUNNEL; i++) {
        if (VXLAN_INFO(unit)->vxlan_tunnel_term[i].dip != 0 &&
            ...
            shr_format_ipaddr(ip_str1, vxlan_info->vxlan_tunnel_term[i].sip);
            shr_format_ipaddr(ip_str2, vxlan_info->vxlan_tunnel_term[i].dip);
            LOG_CLI((BSL_META_U(unit, "| %5d | %15s | %15s | %4d |\n"), i, ip_str1, ip_str2,
                                (int16)vxlan_info->vxlan_tunnel_term[i].vlan));
        }
    }
    LOG_CLI((BSL_META_U(unit, "----------------------------------------------------\n")));
    ...
    LOG_CLI((BSL_META_U(unit,
                        "Match Info    : \n")));
    LOG_CLI((BSL_META_U(unit, "--------------------------------------------------------------------------------------\n")));
    LOG_CLI((BSL_META_U(unit, "| port|    Flags   | Idx | TG |Mo|Po |V  |   Flags2   |            Match             |\n")));
    LOG_CLI((BSL_META_U(unit, "| VP  |            |     | ID | d| rt| FI|            |oVLAN|iVLAN|TnlIdx|eVLAN|Count|\n")));
    LOG_CLI((BSL_META_U(unit, "--------------------------------------------------------------------------------------\n")));

    for (i = 0; i < num_vp; i++) {
        _bcm_vxlan_match_port_info_t *key = &vxlan_info->match_key[i];
        if ((key->trunk_id == 0 || key->trunk_id == -1) &&
            (key->modid == 0 || key->modid == -1) &&
            (key->port == 0) &&
            (key->flags == 0)) {
            continue;
        }
        LOG_CLI((BSL_META_U(unit, "|%5d| 0x%08x |%5d|%4d|%2d|%3d|%3d| 0x%08x |%5d|%5d|%6d|%5d|%5d|\n"),
                 i, key->flags, key->index, key->trunk_id, key->modid, key->port, key->vfi, key->flags2,
                 key->match_vlan, key->match_inner_vlan, key->match_tunnel_index, key->match_etag_vlan,
                 key->match_count));
    }
    LOG_CLI((BSL_META_U(unit, "--------------------------------------------------------------------------------------\n")));

    LOG_CLI((BSL_META_U(unit, "VxLAN VPN Info:\n")));
    LOG_CLI((BSL_META_U(unit, "-----------------------------------------------\n")));
    LOG_CLI((BSL_META_U(unit, "|  VFI  | Type |    Source IP    |    VNID    |\n")));
    LOG_CLI((BSL_META_U(unit, "-----------------------------------------------\n")));

    for (i = 0; i < soc_mem_index_count(unit, VFIm); i++) {
        _bcm_vxlan_vpn_info_t *vpn_info = &vxlan_info->vxlan_vpn_info[i];
        if (vpn_info->vnid != 0 ||
            vpn_info->vxlan_vpn_type != 0 ||
            vpn_info->sip != 0) {
            shr_format_ipaddr(ip_str1, vpn_info->sip);
            LOG_CLI((BSL_META_U(unit, "| %5d | %4d | %15s | 0x%08x |\n"), i,
                                vpn_info->vxlan_vpn_type, ip_str1,
                                vpn_info->vnid));
        }
    }
    LOG_CLI((BSL_META_U(unit, "-----------------------------------------------\n")));
    ...
    for (i = 0; i < num_vp; i++) {
        if (vxlan_info->vfi_vnid_map_count[i]) {
            LOG_CLI((BSL_META_U(unit,
                                "VxLAN VP %5d: VP-based VFI-VNID mappings count = %d\n"), i,
