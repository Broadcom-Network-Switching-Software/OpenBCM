...
/*
 * Function:
 *	_bcm_t2_vp_lag_write_ecmp_table
 * Purpose:
 *      Write the ECMP table.
 * Parameters:
 *      unit  - SOC unit number
 *      idx   - ECMP table index to write
 *      dvp   - DVP field value
 *      egress_if   - NH field value
 *      hi_ecmp_mode - hierarchical ecmp mode
 * Returns:
 *      BCM_E_xxx
 */
STATIC int
_bcm_t2_vp_lag_write_ecmp_table(int unit, int idx, int dvp, int next_hop_index,
                                int hi_ecmp_mode)
{
    int rv;
    ...
    soc_L3_ECMPm_field32_set(unit, &l3_ecmp_entry, DVPf, dvp);
...
/*
 * Function:
 *	_bcm_td2_validate_ecmp_ptr
 * Purpose:
 *      Validate that ecmp_ptr is used and is underlay
 * Parameters:
 *      unit  - SOC unit number
 *      ecmp_ptr - ECMP table index
 * Returns:
 *      BCM_E_xxx
 */
STATIC int
_bcm_td2_validate_ecmp_ptr(int unit, int ecmp_ptr)
{
    int rv = BCM_E_NONE;
...
/*
 * Function:
 *	_bcm_td2_vp_lag_member_resolve_ecmp
 * Purpose:
 *      Convert trunk ID to VP LAG ID.
 * Parameters:
 *      unit  - SOC unit number
 *      ecmp_if  - ECMP object id
 *      nh_array - (OUT)  NH member array
 *      nh_array_size (IN/OUT) - NH member array size
 *      nh_count - (OUT) number of NH member retrieved
 * Returns:
 *      BCM_E_xxx
 */
STATIC int
_bcm_td2_vp_lag_member_resolve_ecmp(int unit, bcm_if_t ecmp_if,
                                    bcm_l3_ecmp_member_t* nh_array,
                                    int nh_array_size, int* nh_count)
{
    bcm_l3_egress_ecmp_t ecmp_info;
    bcm_l3_egress_ecmp_t_init(&ecmp_info);
    ecmp_info.ecmp_intf = ecmp_if;
    *nh_count=0;

    BCM_IF_ERROR_RETURN(
        bcm_xgs3_l3_ecmp_get(unit, &ecmp_info,
             nh_array_size, nh_array, nh_count));
        ...
        bcm_trunk_info_t *trunk_info, int vp_count, int *vp_array)
{
    ...
    int next_hop_index;
    ecmp_count_entry_t l3_ecmp_count_entry;
    int old_base_ptr, old_mem_count;
    initial_l3_ecmp_group_entry_t initial_l3_ecmp_group_entry;
    int ecmp_idx = 0;
    int hi_ecmp_mode = 0;
    ...
    int rv = BCM_E_NONE;
    uint32 alloc_sz = 0;
    soc_esw_ecmp_member_info_t *mem_array = NULL;  /* member array */
    int *hi_ecmp_mode_array = NULL;  /* hierarchy ecmp mode array */
    int mem_count = 0;   /* member count */
    ...
    int nh_count=1;
    bcm_l3_ecmp_member_t *nh_array = NULL;

    /* Allocate an member array to hold all the (vp, ecmp/nh) pairs.  */
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_vp_lag_underlay_ecmp)) {
        /* This feature resolves an underlay ecmp member to nh members.
         * Allocate the largest member array possible.
         */
        alloc_sz = (BCM_XGS3_L3_ECMP_MAX(unit) * sizeof(soc_esw_ecmp_member_info_t));
    }
    else
#endif /* BCM_TRIDENT3_SUPPORT */
    {
        /* The member array is the numbers of VPs */
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_vp_lag_underlay_ecmp)) {
        alloc_sz = (BCM_XGS3_L3_ECMP_MAX(unit) * sizeof(int));
    }
    else
#endif /* BCM_TRIDENT3_SUPPORT */
    {
        alloc_sz = (vp_count * sizeof(int));
    }
    hi_ecmp_mode_array = sal_alloc(alloc_sz, "Hierarchy ECMP mode array");
    if (NULL == hi_ecmp_mode_array) {
        rv = BCM_E_MEMORY;
        goto exit;
    }
    sal_memset(hi_ecmp_mode_array, 0, alloc_sz);

    alloc_sz = sizeof(bcm_l3_ecmp_member_t) * BCM_XGS3_L3_ECMP_MAX(unit);
    nh_array = sal_alloc(alloc_sz, "NH array");
    if (NULL == nh_array) {
        rv = BCM_E_MEMORY;
        goto exit;
    }
    ...
    /* Get the number of members and next-hops */
    for (i = 0; i < vp_count; i++) {
...
#endif
        nh_count=1;
        nh_array[0].egress_if = next_hop_index + BCM_XGS3_EGRESS_IDX_MIN(unit);
#if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(unit, soc_feature_vp_lag_underlay_ecmp) &&
            (_bcm_vp_used_get(unit, vp_array[i], _bcmVpTypeVxlan) ||
             _bcm_vp_used_get(unit, vp_array[i], _bcmVpTypeL2Gre) ||
             _bcm_vp_used_get(unit, vp_array[i], _bcmVpTypeFlow))) {
            /* resolve any underlay ecmp paths */
            ecmp_flag = soc_ING_DVP_TABLEm_field32_get(unit, &ing_dvp_entry,
                                                       ECMPf);
            if (ecmp_flag) {
                bcm_if_t ecmp_if;
                ecmp_ptr = soc_ING_DVP_TABLEm_field32_get(unit, &ing_dvp_entry,
                                                          ECMP_PTRf);
                rv = _bcm_td2_validate_ecmp_ptr(unit, ecmp_ptr);
                if (BCM_FAILURE(rv)) {
                    goto exit;
                }
                ecmp_if = ecmp_ptr + BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
                rv = _bcm_td2_vp_lag_member_resolve_ecmp(unit, ecmp_if,
                         nh_array, BCM_XGS3_L3_ECMP_MAX(unit), &nh_count);
                if (BCM_FAILURE(rv)) {
                    goto exit;
                }
                if (nh_count == 0) {
        ...
        for (j=0; j<nh_count; j++) {
            if (mem_count >= alloc_sz/sizeof(soc_esw_ecmp_member_info_t)) {
                rv = BCM_E_PARAM;
                goto exit;
            }
            hi_ecmp_mode_array[mem_count] = hi_ecmp_mode ? 1 : 0;
            mem_array[mem_count].nh = nh_array[j].egress_if - BCM_XGS3_EGRESS_IDX_MIN(unit);
            mem_array[mem_count++].dvp = vp_array[i];
        }
    }
    ...
    BCM_XGS3_L3_ENT_REF_CNT_INC(tbl_op_data.tbl_ptr, base_ptr, mem_count);

    if ((_bcm_vp_used_get(unit, vp_array[0], _bcmVpTypeVxlan) ||
        _bcm_vp_used_get(unit, vp_array[0], _bcmVpTypeL2Gre) ||
        _bcm_vp_used_get(unit, vp_array[0], _bcmVpTypeFlow))) {
        rv = READ_ING_DVP_TABLEm(unit, MEM_BLOCK_ANY,
                                 vp_array[0], &ing_dvp_entry);
        if (BCM_FAILURE(rv)) {
            goto exit;
        }
    ...
    if (protected_ecmp == 1) {
        ...
        grp_info.max_paths = mem_count;
        grp_info.vp_lag = 1;
        rv = soc_esw_ecmp_protected_grp_add(unit, &grp_info, mem_array);
        if (BCM_FAILURE(rv)) {
            BCM_XGS3_L3_ENT_REF_CNT_DEC(tbl_op_data.tbl_ptr, base_ptr, mem_count);
            goto exit;
        }
        sal_free(mem_array);
        mem_array = NULL;
...
#endif /* SOC_L3_ECMP_PROTECTED_ACCESS_SUPPORT */
    {
        for (i=0; i<mem_count; i++) {
            rv = _bcm_t2_vp_lag_write_ecmp_table(unit, base_ptr+i, mem_array[i].dvp,
                                                 mem_array[i].nh,
                                                 hi_ecmp_mode_array[i]);
            if (BCM_FAILURE(rv)) {
                goto exit;
            }
        }
    ...
    old_mem_count = 1 + soc_L3_ECMP_COUNTm_field32_get(unit,
...
exit:
    ...
    if (hi_ecmp_mode_array != NULL) {
        sal_free(hi_ecmp_mode_array);
    }
        ...
        bcm_trunk_info_t *trunk_info, int vp_count, int *vp_array)
{
    ...
    for (i = 0; i < vp_count; i++) {
        /* Bypass non-access VP. */
        if (_bcm_vp_info_get(unit, vp_array[i], &vp_info) ||
            ((vp_info.flags & _BCM_VP_INFO_NETWORK_PORT) &&
             (vp_info.vp_type == _bcmVpTypeVxlan || vp_info.vp_type == _bcmVpTypeL2Gre ||
              vp_info.vp_type == _bcmVpTypeMpls || vp_info.vp_type == _bcmVpTypeFlow))) {
            continue;
        }
        ...
        int *vfi_type)
{
    ...
    if (SOC_SUCCESS(rv)) {
        ...
        if (_bcm_vp_used_get(unit, vp, _bcmVpTypeVxlan)
            ...
            || _bcm_vp_used_get(unit, vp, _bcmVpTypeFlow)
        ...
        int member_count, bcm_trunk_member_t *member_array)
{
    ...
    for (i = 0; i < member_count; i++) {
        ...
        rv = _bcm_vp_used_get(unit, vp, type);
        if (rv == BCM_E_NONE) {
            sal_free(egr_dis_vp);
            return BCM_E_PARAM;
        }
        ...
        int member_max, bcm_trunk_member_t *member_array, int *member_count)
{
    ...
    if ((VP_LAG_GROUP_INFO(unit, vp_lag_id).has_member) ||
        ...
        if (member_max == 0) {
            ...
            *member_count = 0;
            for (i = 0; i < vp_count && i < member_max; i += nh_count) {
#ifdef SOC_L3_ECMP_PROTECTED_ACCESS_SUPPORT
#if defined(BCM_TRIDENT3_SUPPORT)
                if (protected_ecmp == 1) {
                    vp = mem_array[i].dvp;
                ...
                if (soc_feature(unit, soc_feature_vp_lag_underlay_ecmp) &&
                    ...
                    int ecmp_flag = 0;
                    ing_dvp_table_entry_t ing_dvp_entry;
                    rv = READ_ING_DVP_TABLEm(unit, MEM_BLOCK_ANY, vp, &ing_dvp_entry);
                    if (BCM_FAILURE(rv)) {
                        goto exit;
                    }
                    ...
                    if (ecmp_flag) {
                        bcm_if_t ecmp_if;
                        /* DVP points to underlay ecmp-group; get the nh count */
                        int ecmp_ptr = soc_ING_DVP_TABLEm_field32_get(unit, &ing_dvp_entry,
                                                                  ECMP_PTRf);
                        rv = _bcm_td2_validate_ecmp_ptr(unit, ecmp_ptr);
                        if (BCM_FAILURE(rv)) {
                            goto exit;
                        ...
                        ecmp_if = ecmp_ptr + BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
                        rv = _bcm_td2_vp_lag_member_resolve_ecmp(unit, ecmp_if, NULL, 0, &nh_count);
                        if (BCM_FAILURE(rv)) {
                            goto exit;
                        }
                        if (nh_count == 0) nh_count = 1;
                    }
                }
#endif /* BCM_TRIDENT3_SUPPORT */
            }
