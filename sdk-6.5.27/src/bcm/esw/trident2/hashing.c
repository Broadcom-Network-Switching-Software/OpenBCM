                  ...
                  uint8 ecmp_rh_enable, uint32 ethertype)
{
    ...
    if (ecmp_rh_enable) {
        SOC_IF_ERROR_RETURN
            (READ_L3_ECMP_COUNTm(unit, MEM_BLOCK_ANY, ecmp_group, &ecmpc_entry));
        ecmp_rh_flow_set_base = soc_L3_ECMP_COUNTm_field32_get(unit, &ecmpc_entry, RH_FLOW_SET_BASEf);
        ecmp_rh_flow_set_size = soc_L3_ECMP_COUNTm_field32_get(unit, &ecmpc_entry, RH_FLOW_SET_SIZEf);
        ...
        L3_ECMPm_index_max = soc_mem_index_max(unit, L3_ECMPm);

        ecmp_index = (ecmp_ptr + ecmp_offset) & L3_ECMPm_index_max;
        SOC_IF_ERROR_RETURN
                                   ...
                                   bcm_if_t *dst_intf)
{
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_MULTIPATH)) {
        ...
        if (0 != (check_td2_ecmp_rh_enable(unit, 
                    pkt_info->mpintf - BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit), ethertype ))) { 
            ecmp_rh_enable = TRUE;
            hash_value = 0;
