...
_field_sc960_egress_qualifiers_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyPrimaryKeySelect,
                 _bcmFieldSliceSelDisable, 0, 0, 0);
                           ...
                           )
{
    ...
    if (stage_fc->stage_id == _BCM_FIELD_STAGE_INGRESS) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:field:{offset;length;range_count}\n")));

        buf[fc->scache_pos] = (uint8)range_count;
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
             "    {0x%x;1;%d}\n"), fc->scache_pos, (uint8)range_count));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:field\n")));
        fc->scache_pos++;
        ...
        if (range_count) {
            fr = stage_fc->ranges;
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                "S$BEGIN:field:{offset;length;range_id_idx,range_id}\n")));

            while (fr) {
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;4;%d;%d}\n"), fc->scache_pos, indx++,fr->rid));
       ...
       /* Write data qual count */
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:field:{offset;length;qual_count}\n")));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
             "    {0x%x;2;%d}\n"), fc->scache_pos, (int)bcmFieldQualifyCount));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:field\n")));
        ...
        if (soc_mem_is_valid(unit, FP_UDF_TCAMm)) {
            ...
            */
            ...
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                "S$BEGIN:field:{offset;length;udf_ref_count}\n")));
