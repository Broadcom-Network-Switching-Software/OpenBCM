...
/*
 * Function:
 *        _bcm_td2_l2gre_match_vif_entry_set
 * Purpose:
 *       Set L2GRE Match Vxlate Entry
 * Parameters:
 *  IN :  Unit
 ...
 */
STATIC int
_bcm_td2_l2gre_match_vif_entry_set(int unit, bcm_l2gre_port_t *l2gre_port, uint32 *vent)
{
    int rv = BCM_E_UNAVAIL;
    int index;
    bcm_module_t mod_out = -1;
...
bcm_td2_l2gre_match_add(int unit, bcm_l2gre_port_t *l2gre_port, int vp, bcm_vpn_t vpn)
{
	...
	if (soc_feature(unit, soc_feature_vp_sharing)) {
    ...
    } else if (l2gre_port->criteria == BCM_L2GRE_PORT_MATCH_PORT_ETAG_VLAN) {
        if (!BCM_VLAN_VALID(l2gre_port->match_vlan) ||
            !BCM_VLAN_VALID(l2gre_port->match_etag_vlan)) {
             return BCM_E_PARAM;
        }
...
#endif
        soc_mem_field32_set(unit, mem, vent, VIF__VLANf, l2gre_port->match_vlan);
        soc_mem_field32_set(unit, mem, vent, VIF__SRC_VIFf, l2gre_port->match_etag_vlan);
        if (BCM_GPORT_IS_TRUNK(l2gre_port->match_port)) {
            soc_mem_field32_set(unit, mem, vent, VIF__Tf, 1);
            soc_mem_field32_set(unit, mem, vent, VIF__TGIDf, trunk_id);
        } else {
        ...
        BCM_IF_ERROR_RETURN(
            _bcm_td2_l2gre_match_vif_entry_set(unit, l2gre_port, vent));
        if (l2gre_info->match_key[vp].flags != _BCM_L2GRE_PORT_MATCH_TYPE_SHARED) {
            /* Update software state */
            l2gre_info->match_key[vp].flags = _BCM_L2GRE_PORT_MATCH_TYPE_PORT_ETAG_VLAN;
            l2gre_info->match_key[vp].match_vlan = l2gre_port->match_vlan;
            l2gre_info->match_key[vp].match_etag_vlan = l2gre_port->match_etag_vlan;
            if (BCM_GPORT_IS_TRUNK(l2gre_port->match_port)) {
                l2gre_info->match_key[vp].trunk_id = trunk_id;
                l2gre_info->match_key[vp].modid = -1;
            } else {
...
bcm_td2_l2gre_match_delete(int unit,  int vp, bcm_l2gre_port_t *l2gre_port)
{
    ...
    if  (l2gre_info->match_key[vp].flags == _BCM_L2GRE_PORT_MATCH_TYPE_VLAN) {
        ...
        if (BCM_L2GRE_PORT_MATCH_PORT_VLAN == l2gre_port->criteria) {
            ...
            soc_mem_field32_set(unit, mem, vent, XLATE__OTAGf,
                                l2gre_port->match_vlan);
        } else if (BCM_L2GRE_PORT_MATCH_PORT_ETAG_VLAN == l2gre_port->criteria) {
            if (!BCM_VLAN_VALID(l2gre_port->match_vlan) ||
                !BCM_VLAN_VALID(l2gre_port->match_etag_vlan)) {
                 return BCM_E_PARAM;
            }
            soc_mem_field32_set(unit, mem, vent, KEY_TYPEf,
                                TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
            soc_mem_field32_set(unit, mem, vent, VIF__VLANf, l2gre_port->match_vlan);
            soc_mem_field32_set(unit, mem, vent, VIF__SRC_VIFf, l2gre_port->match_etag_vlan);
            is_vif_view = 1;
        } else {
        ...
        if (BCM_GPORT_IS_TRUNK(l2gre_port->match_port)) {
            soc_mem_field32_set(unit, mem, vent, is_vif_view ? VIF__Tf : XLATE__Tf, 1);
            soc_mem_field32_set(unit, mem, vent, is_vif_view ? VIF__TGIDf : XLATE__TGIDf, trunk_id);
        } else {
            soc_mem_field32_set(unit, mem, vent, is_vif_view ? VIF__MODULE_IDf : XLATE__MODULE_IDf, mod_out);
            soc_mem_field32_set(unit, mem, vent, is_vif_view ? VIF__PORT_NUMf : XLATE__PORT_NUMf, port_out);
        }
    ...
    } else if (l2gre_info->match_key[vp].flags == _BCM_L2GRE_PORT_MATCH_TYPE_PORT_ETAG_VLAN) {
        soc_mem_field32_set(unit, mem, vent, SOURCE_TYPEf, 1);
        soc_mem_field32_set(unit, mem, vent, KEY_TYPEf, TR_VLXLT_HASH_KEY_TYPE_VIF_VLAN);
#if defined(BCM_TRIDENT3_SUPPORT)
        if (mem == VLAN_XLATE_1_DOUBLEm) {
        ...
        soc_mem_field32_set(unit, mem, vent, VIF__SRC_VIFf,
                        l2gre_info->match_key[vp].match_etag_vlan);
        if (l2gre_info->match_key[vp].modid != -1) {
            soc_mem_field32_set(unit, mem, vent, VIF__MODULE_IDf,
                                        l2gre_info->match_key[vp].modid);
            soc_mem_field32_set(unit, mem, vent, VIF__PORT_NUMf,
                                        l2gre_info->match_key[vp].port);
        } else {
            soc_mem_field32_set(unit, mem, vent, VIF__Tf, 1);
            soc_mem_field32_set(unit, mem, vent, VIF__TGIDf,
                                    l2gre_info->match_key[vp].trunk_id);
        }
        rv = soc_mem_delete(unit, mem, MEM_BLOCK_ANY, vent);
        BCM_IF_ERROR_RETURN(rv);
...
bcm_td2_l2gre_match_get(int unit, bcm_l2gre_port_t *l2gre_port, int vp)
{
    ...
    if  (l2gre_info->match_key[vp].flags & _BCM_L2GRE_PORT_MATCH_TYPE_VLAN) {
        ...
        l2gre_port->criteria = BCM_L2GRE_PORT_MATCH_PORT_ETAG_VLAN;
        l2gre_port->match_vlan = l2gre_info->match_key[vp].match_vlan;
        l2gre_port->match_etag_vlan = l2gre_info->match_key[vp].match_etag_vlan;
        soc_mem_field32_set(unit, mem, vent, VIF__VLANf, l2gre_port->match_vlan);
        soc_mem_field32_set(unit, mem, vent, VIF__SRC_VIFf, l2gre_port->match_etag_vlan);
        is_vif_view = 1;
    } else {
    ...
    if (l2gre_port->criteria == BCM_L2GRE_PORT_MATCH_PORT_VLAN ||
        ...
        if (l2gre_info->match_key[vp].trunk_id != -1) {
            ...
            BCM_IF_ERROR_RETURN(_bcm_esw_stk_modmap_map(unit,
                BCM_STK_MODMAP_GET, mod_in, port_in, &mod_out, &port_out));
            BCM_GPORT_MODPORT_SET(l2gre_port->match_port, mod_out, port_out);
            soc_mem_field32_set(unit, mem, vent, is_vif_view ? VIF__MODULE_IDf : XLATE__MODULE_IDf, mod_in);
            soc_mem_field32_set(unit, mem, vent, is_vif_view ? VIF__PORT_NUMf : XLATE__PORT_NUMf, port_in);
        }
                  ...
                  source_vp_entry_t *svp, int tpid_enable, bcm_l2gre_port_t  *l2gre_port)
{
    ...
    /* Retain CML in the replace operation.It may have been changed before */
    if (!(l2gre_port->flags & BCM_L2GRE_PORT_REPLACE)) {
