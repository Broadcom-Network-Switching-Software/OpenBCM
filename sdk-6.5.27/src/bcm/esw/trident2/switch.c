...
#define FLEX_HASH_ENTRY_INSTALL_DONE    1
...
typedef uint32 bcmi_xgs4_td2_switch_wb_hash_entry_flags;
#endif
...
typedef struct _bcm_switch_hash_user_data_s {
    int                 hash_bin_bitmap;
    bcm_hash_entry_t    entry_id;
}_bcm_switch_hash_user_data_t;
#endif /* BCM_TOMAHAWK2_SUPPORT */

#if defined(BCM_TOMAHAWK2_SUPPORT)
STATIC int
_bcm_switch_hash_entry_traverse_cb(int unit, int flags, bcm_hash_entry_t entry_id,
                                   void *user_data) {
    int         rv = BCM_E_NONE;
    _bcm_switch_hash_user_data_t *cb_data;
    flex_hash_entry_t            *hash_entry;

    if (!user_data) return rv;

    cb_data = (_bcm_switch_hash_user_data_t *)user_data;
    if (cb_data->entry_id == entry_id) {
        return rv;
    }
    ...
    if (hash_entry->flags & FLEX_HASH_ENTRY_INSTALL_DONE) {
        cb_data->hash_bin_bitmap |= hash_entry->offset_bitmap;
    }
...
_bcm_th2_flex_hash_table_write(int unit, flex_hash_entry_t * hash_entry, uint8 delete)
{
        ...
        uint32          mask[2] = { 0, 0 };
        soc_reg_t       reg;
        uint64          hash_control_64=0;
        uint8           hash_bin_disable=0;
        int             hash_bin_bitmap=0;
        _bcm_switch_hash_user_data_t    cb_data;
    ...
    */
        ...
        if (SOC_IS_TOMAHAWK3(unit)) {
            ...
            if (delete) {
                cb_data.hash_bin_bitmap = 0;
                cb_data.entry_id = hash_entry->entry_id;
                bcm_td2_switch_hash_entry_traverse(unit, 0,
                                               _bcm_switch_hash_entry_traverse_cb,
                                               &cb_data);
                hash_bin_bitmap = cb_data.hash_bin_bitmap;
            }
        }
        ...
        for (fld = 0; fld < FLEX_HASH_OFFSET_NUM(unit); fld++) {
            if (hash_entry->type[fld] & BCM_SWITCH_HASH_FIELD_TYPE_UDF) {
                if (hash_entry->offset_bitmap & (0x1 << fld)) {
                    ...
                    if (SOC_IS_TOMAHAWK3(unit)) {
                        hash_bin_disable = 0;
                        if (delete && !(hash_bin_bitmap & (1 << fld))) {
                            hash_bin_disable = 1;
                        }
                        BCM_IF_ERROR_RETURN(soc_reg64_get
                                    (unit, reg, REG_PORT_ANY, 0, &hash_control_64));
                        if (SOC_REG_FIELD_VALID
                            (unit, reg, _bcm_th2_flex_hash_enable_fields[2*fld])) {
                            soc_reg64_field32_set(unit, reg, &hash_control_64,
                                      _bcm_th2_flex_hash_enable_fields[2*fld],
                                      hash_bin_disable ? 0 : 1);
                        }
                        if (SOC_REG_FIELD_VALID
                            (unit, reg, _bcm_th2_flex_hash_enable_fields[2*fld+1])) {
                            soc_reg64_field32_set(unit, reg, &hash_control_64,
                                      _bcm_th2_flex_hash_enable_fields[2*fld+1],
                                      hash_bin_disable ? 0 : 1);
                        }
...
_bcm_hash_entry_remove(int unit, bcm_hash_entry_t entry_id)
{
    ...
    hash_entry->flags &= ~FLEX_HASH_ENTRY_INSTALL_DONE;
#ifdef BCM_TOMAHAWK2_SUPPORT
    if ((SOC_IS_TOMAHAWK2(unit)) || (SOC_IS_TOMAHAWK3(unit))) {
                                     ...
                                     uint16 recovered_ver)
{
    ...
    for (i = 0; i < num_hash_entry; i++) {
        ...
        if (recovered_ver >= BCM_SWITCH_WB_FLEXHASH_VERSION_1_2) {
            hash_entry->flags = hash_entry_scache_p->entry_flags;
        }
        ...
        hash_entry_scache_p++;
        if (recovered_ver < BCM_SWITCH_WB_FLEXHASH_VERSION_1_2) {
            ptr = (uint8 *) hash_entry_scache_p;
            ptr = ptr - sizeof(bcmi_xgs4_td2_switch_wb_hash_entry_flags);
            hash_entry_scache_p = (bcmi_xgs4_td2_switch_wb_hash_entry_t *)ptr;
        }
    }
...
bcmi_td2_switch_wb_hash_entry_sync(int unit, uint8 **scache_ptr)
{
    ...
    *u32_scache_p = SWITCH_HASH_LAST_ENTRY_ID(unit);
    ...
    while (hash_entry != NULL) {
        ...
        hash_entry_scache_p->entry_flags = hash_entry->flags;
