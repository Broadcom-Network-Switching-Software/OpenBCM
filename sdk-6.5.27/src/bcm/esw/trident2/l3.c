...
extern bcm_error_t _bcm_esw_stat_flex_l3_route_restore(
    int       unit,
        ...
        if (BCM_XGS3_L3_MEM(unit, v6) == mem) {
            ...
            if (SOC_IS_TRIDENT3X(unit) || SOC_IS_TOMAHAWK3(unit))
            {
                ...
                if (soc_feature(unit, soc_feature_flex_flow) &&
                    ...
                    sal_memset(buf_p, 0, BCM_L3_MEM_ENT_SIZE(unit, mem));
                    soc_mem_field32_set(unit, mem, buf_p, DATA_TYPEf,
                             TD3_L3_HASH_DATA_TYPE_V6UC);
                    soc_mem_ip6_addr_set(unit, mem, buf_p, SRV6UC__IP_ADDR_99_0f,
                                 l3cfg->l3c_ip6, SOC_MEM_IP6_LOWER_100BIT);
                    soc_mem_ip6_addr_set(unit, mem, buf_p, SRV6UC__IP_ADDR_127_100f,
                                 l3cfg->l3c_ip6, SOC_MEM_IP6_UPPER_28BIT);
                    soc_mem_field32_set(unit, mem, buf_p, SRV6UC__VRF_IDf,
                                                      l3cfg->l3c_vrf);
                    soc_mem_field32_set(unit, mem, buf_p, KEY_TYPEf,
                                         TD3_L3_HASH_KEY_TYPE_SRV6UC);
                }
#endif /* BCM_TRIDENT3_SUPPORT */
...
#ifdef BCM_TRIDENT3_SUPPORT
        } else if ((key_type == TD3_L3_HASH_KEY_TYPE_SRV6UC) &&
                   soc_feature(unit, soc_feature_flex_flow) &&
                   _BCM_FLOW_SRV6_ENABLED(unit)) {
            l3cfg->l3c_flags = BCM_L3_IP6;
            soc_mem_ip6_addr_get(unit, mem, buf_entry,
                    SRV6UC__IP_ADDR_99_0f, l3cfg->l3c_ip6,
                    SOC_MEM_IP6_LOWER_100BIT);

            soc_mem_ip6_addr_get(unit, mem, buf_entry,
                    SRV6UC__IP_ADDR_127_100f, l3cfg->l3c_ip6,
                    SOC_MEM_IP6_UPPER_28BIT);
            rv = BCM_E_NONE;
#endif
        } else {
#if defined(BCM_TRIDENT3_SUPPORT)
        ...
        if ((l3cfg->l3c_flags & BCM_L3_L2ONLY) ||
            ...
            if (_BCM_VPN_IS_SET(l3cfg->l3c_vid)) {
                ...
                if (soc_feature(unit, soc_feature_ipmc_l2_use_vlan_vpn) && 
                    ...
                    if (_BCM_VPN_VFI_IS_SET(l3cfg->l3c_vid)) {
...
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT)
    int ecmp_idx_new, ecmp_idx_old;
#endif
...
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT)
    ecmp_idx_old = nh_idx_old;
    ecmp_idx_new = nh_idx_new;
#endif
...
#ifdef BCM_TOMAHAWK_SUPPORT
#ifdef ALPM_ENABLE
/* Store current refered route bkt_node pointer used by L3 route stat API.
 * This is multi-thread safe (L3_LOCK). Only one possible referred route bkt_node
 * exists within a single L3 route stat operation (per API and unit).
 */
uint32 *alpm2_bkt_node_ptr[SOC_MAX_NUM_DEVICES];
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
...
#endif /* BCM_TRIDENT3_SUPPORT */
        {
            ...
            if (defip.defip_sub_len > 64) {
                if (SOP_MEM_STATE(unit, defip_pair_mem) &&
                    SOP_MEM_STATE(unit, defip_pair_mem)->index_max >= 0) {
                    mem = defip_pair_mem;
                }
            } else {
                scale = 2;
    ...
    if (soc_feature(unit, soc_feature_alpm) &&
        ...
        if (defip.defip_alpm_cookie & SOC_ALPM_LOOKUP_HIT) {
            ...
            if (soc_feature(unit, soc_feature_alpm2)) {
                ...
                /* save current referred bkt_node ptr for later use
                   within a single route stat operation */
                alpm2_bkt_node_ptr[unit] = defip.bkt_node;
                index = 0;
...
/*
 * Function:
 *      _bcm_td2_l3_route_stat_detach_del
 * Description:
 *      Detach counter entries to the given l3 route index for delete.
 *
 * Parameters:
 *      unit             - (IN) unit number
 *      info		     - (IN) L3 Route Info
 ...
 *
 */
bcm_error_t  _bcm_td2_l3_route_stat_detach_del (
                            int                 unit,
                            bcm_l3_route_t      *info)
{
    uint32                     count = 0;
    ...
    for (idx = 0; idx < BCM_XGS3_L3_NH_TBL_SIZE(unit); idx++) {
        ...
        if (key_type == _BCM_NH_UPDATE_KEY_L3_INTF) {
            ...
            if (key_type == _BCM_NH_UPDATE_KEY_VLAN) {
                ...
                if (BCM_XGS3_L3_INGRESS_INTF_MAP_MODE_ISSET(unit)) {
                    /* Legacy devices have only VLAN field */
                    int check_vfi = 0;
                    int check_vlan = 1;
                    l3_iif = BCM_IF_INVALID;
                    if (soc_feature(unit, soc_feature_egr_intf_vlan_vfi_deoverlay)) {
                        /* TD3 has seperate VLAN and VFI fields - check both for
                         * matching l3_iif.
                         */
                        check_vfi = 1;
                    }
                    else if (soc_mem_field_valid(unit, BCM_XGS3_L3_MEM(unit, intf),
                                 VLAN_VFI_OVERLAY_TYPEf)) {
                        /* TD2 devices have either VLAN or VFI field */
                        int overlay_type = soc_mem_field32_get(unit,
                                             BCM_XGS3_L3_MEM(unit, intf),
                                             egr_intf_entry_ptr,
                                             VLAN_VFI_OVERLAY_TYPEf);
                        check_vfi = overlay_type;
                        check_vlan = !overlay_type;
                    }

                    if (check_vlan && BCM_VLAN_VALID(vlan)) {
                        vlan_mpls_entry_ptr =
                        ...
                        if (l3_iif == *(bcm_if_t *)val) check_vfi = 0;
                    }
                    if (check_vfi) {
                        if (soc_mem_field_valid(unit, BCM_XGS3_L3_MEM(unit, intf), VFIf) &&
                            SOC_MEM_IS_VALID(unit, vfi_mem)) {
                           vlan = soc_mem_field32_get(unit, BCM_XGS3_L3_MEM(unit, intf),
                             ...
                             soc_mem_t alpm_mem, void *alpm_entry)
{
    ...
    if (alpm_mem == L3_DEFIP_ALPM_IPV4_1m || alpm_mem == L3_DEFIP_ALPM_IPV6_64_1m) {
        if (SOC_MEM_FIELD_VALID(unit, alpm_mem, FLEX_CTR_BASE_COUNTER_IDXf)) {
            lpm_cfg->defip_flex_ctr_base_id = soc_mem_field32_get(unit, alpm_mem,
                                                                  alpm_entry,
                                                        FLEX_CTR_BASE_COUNTER_IDXf);
        }
        if (SOC_MEM_FIELD_VALID(unit, alpm_mem, FLEX_CTR_OFFSET_MODEf)) {
            lpm_cfg->defip_flex_ctr_mode = soc_mem_field32_get(unit, alpm_mem,
                                                               alpm_entry,
                                                        FLEX_CTR_OFFSET_MODEf);
        }
        if (SOC_MEM_FIELD_VALID(unit, alpm_mem, FLEX_CTR_POOL_NUMBERf)) {
            lpm_cfg->defip_flex_ctr_pool = soc_mem_field32_get(unit, alpm_mem,
                                                               alpm_entry,
    ...
    for (idx = 0; idx < idx_end; idx++) {
        ...
        for (tmp_idx = 0; tmp_idx < step_count; tmp_idx++) {
            ...
            for (bkt_idx = 0; bkt_idx < bkt_count; bkt_idx++) {
                ...
                if (lpm_cfg.defip_flex_ctr_base_id != 0 ||
                    lpm_cfg.defip_flex_ctr_mode != 0) {
                    (void)_bcm_esw_stat_flex_l3_route_restore(unit,
