...
#include <bcm_int/esw/ifa2_feature.h>
...
/*VXLAN flow aging interval multiplier */
#define BCM_XGS_IFA2_VXLAN_FLOW_AGING_INTERVAL_STEP_MSECS (1000)

/*VXLAN flow max aging interval in milisec */
#define BCM_XGS_IFA2_VXLAN_FLOW_MAX_AGING_INTERVAL_MSECS (4000000)
...
typedef struct _bcm_xgs5_ifa2_info_s {
    ...
    /* VXLAN flow aging interval */
    uint32 vxlan_flow_aging_interval;

    /* VXLAN max num flows */
    uint32 max_vxlan_term_flows;

    /* Device id */
    uint32 device_id;

    /* LNS format for tailstamp metadata. */
    uint8 tailstamp_md_lns_id;
...
_bcm_xgs5_ifa2_uc_msg_init(int unit)
{
    ...
    msg.max_vxlan_term_flows  = soc_property_get(unit, spn_IFA_MAX_VXLAN_TERM_FLOWS, 0);
...
STATIC
int _bcm_xgs5_ifa2_uc_msg_cosq_map_send(int unit, bcm_ifa_config_info_t *cfg_info)
{
    _bcm_xgs5_ifa2_info_t *ifa2_info = ifa2_global_info[unit];
    ...
    for (i = 0; i < BCM_IFA_MAX_NUM_PORTS; i++) {
        for (j = 0; j < BCM_IFA_MAX_NUM_INT_PRI; j++) {
            msg.cosq_map[i][j] = cfg_info->cosq_map[i].cosq[j];
        }
    }

    buffer_req = ifa2_info->dma_buffer;
    buffer_ptr = _bcm_xgs5_ifa2_msg_ctrl_cosq_map_pack(buffer_req, &msg);
    buffer_len = buffer_ptr - buffer_req;

    BCM_IF_ERROR_RETURN
        (_bcm_xgs5_ifa2_msg_send_receive(unit,
                                         MOS_MSG_SUBCLASS_IFA2_COSQ_MAP_SET,
                                         buffer_len, 0,
                                         MOS_MSG_SUBCLASS_IFA2_COSQ_MAP_SET_REPLY,
                                         &reply_len,
                                         SHR_IFA2_MAX_UC_MSG_TIMEOUT));
    return BCM_E_NONE;
}
 ...
 STATIC int
_bcm_xgs5_ifa2_uc_msg_config_get(int unit, shr_ifa2_msg_ctrl_config_get_t *msg)
{
    _bcm_xgs5_ifa2_info_t *ifa2_info = ifa2_global_info[unit];
    uint8 *buffer_req = NULL, *buffer_ptr = NULL;
    uint16 buffer_len = 0, reply_len = 0;

    sal_memset(msg, 0, sizeof(shr_ifa2_msg_ctrl_config_get_t));

    BCM_IF_ERROR_RETURN
        (_bcm_xgs5_ifa2_msg_send_receive(unit,
                                         MOS_MSG_SUBCLASS_IFA2_CONFIG_GET,
                                         0, 0,
                                         MOS_MSG_SUBCLASS_IFA2_CONFIG_GET_REPLY,
                                         &reply_len,
                                         SHR_IFA2_MAX_UC_MSG_TIMEOUT));

    buffer_req = ifa2_info->dma_buffer;
    buffer_ptr = _bcm_xgs5_ifa2_msg_ctrl_config_get_unpack(buffer_req, msg);
    buffer_len = buffer_ptr - buffer_req;
...
_bcm_xgs5_ifa2_uc_msg_stats_set(int unit, bcm_ifa_stat_info_t *stats)
{
    ...
    if (IFA2_UC_FEATURE_CHECK(IFA2_VXLAN_IFA_TERM_SUPPORT)) {
        msg.vxlan_term_lookup_miss_cnt_lo = stats->vxlan_term_lookup_miss_cnt;
        msg.vxlan_data_flow_table_insert_fail_cnt_lo = stats->vxlan_data_flow_table_insert_fail_cnt;
    }
...
_bcm_xgs5_ifa2_uc_msg_stats_get(int unit, bcm_ifa_stat_info_t *stats)
{
    ...
    if (IFA2_UC_FEATURE_CHECK(IFA2_VXLAN_IFA_TERM_SUPPORT)) {
        stats->vxlan_term_lookup_miss_cnt = msg.vxlan_term_lookup_miss_cnt_lo;
        stats->vxlan_data_flow_table_insert_fail_cnt = msg.vxlan_data_flow_table_insert_fail_cnt_lo;
    }
    return BCM_E_NONE;
}
...
/*
 * Function:
 *      _bcm_xgs5_ifa2_tailstamp_lns_id_set
 * Purpose:
 *      Set the LNS format of the tailstamp metadata.
 * Parameters:
 *      unit        - (IN) BCM device number
 *      lns_id      - (IN) LNS ID.
 * Returns:
 ...
 */
STATIC int
_bcm_xgs5_ifa2_tailstamp_lns_id_set(int unit, uint8 lns_id)
{
    uint32 params;
    uint32 values[1][SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_MAX_VALUES];
    uint16 num_values;

    if (!IFA2_UC_FEATURE_CHECK(IFA2_TAILSTAMP_MD_FORMAT_SET)) {
        /*
         * The app does not support setting the LNS format of tailstamped
         * metadata. Silently return if the requested LNS ID is 0 (default).
         */
        return (lns_id == 0 ? BCM_E_NONE : BCM_E_UNAVAIL);
    }

    params = SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_TAILSTAMP_LNS_ID;
    values[0][0] = lns_id;
    num_values = 1;
...
int _bcm_xgs5_ifa2_config_info_set(int unit, bcm_ifa_config_info_t *cfg_info)
{
    ...
    /*
     * Default value at boot up : 10s
     * Minimum aging interval : 1s
     * if set to 0 stop aging
     */
    if (((cfg_info->vxlan_flow_aging_interval % BCM_XGS_IFA2_VXLAN_FLOW_AGING_INTERVAL_STEP_MSECS) != 0)
        || (cfg_info->vxlan_flow_aging_interval > BCM_XGS_IFA2_VXLAN_FLOW_MAX_AGING_INTERVAL_MSECS)) {
        return BCM_E_PARAM;
    }

    params[i] = SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_RX_PKT_EXPORT_MAX_LENGTH;
    values[i][0] = cfg_info->rx_packet_export_max_length;
    num_values[i] = 1;
    i++;

    if (IFA2_UC_FEATURE_CHECK(IFA2_VXLAN_IFA_TERM_SUPPORT)) {
        params[i] = SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_VXLAN_FLOW_AGING_INTERVAL;
        values[i][0] = cfg_info->vxlan_flow_aging_interval;
        num_values[i] = 1;
        i++;
    }

    params[i] = SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_DEVICE_ID;
    values[i][0] = cfg_info->device_id;
    num_values[i] = 1;
    i++;

    BCM_IF_ERROR_RETURN
        (_bcm_xgs5_ifa2_uc_msg_cfg_update(unit, i, params, num_values, values));

    if (IFA2_UC_FEATURE_CHECK(IFA2_VXLAN_IFA_TERM_SUPPORT)) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs5_ifa2_uc_msg_cosq_map_send(unit, cfg_info));
    }

    ifa2_info->rx_pkt_export_max_length = cfg_info->rx_packet_export_max_length;
    ifa2_info->vxlan_flow_aging_interval = cfg_info->vxlan_flow_aging_interval;
...
{
    _bcm_xgs5_ifa2_info_t *ifa2_info = ifa2_global_info[unit];
    shr_ifa2_msg_ctrl_config_get_t   msg;
    int i = 0, j = 0;
    if (ifa2_info == NULL) {
        return BCM_E_INIT;
    }
    ...
    if (IFA2_UC_FEATURE_CHECK(IFA2_VXLAN_IFA_TERM_SUPPORT)) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs5_ifa2_uc_msg_config_get(unit, &msg));

        for (i = 0; i < msg.config.num_params; i++) {
            switch (msg.config.params[i]) {
                case SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_RX_PKT_EXPORT_MAX_LENGTH:
                    cfg_info->rx_packet_export_max_length = msg.config.values[i][0];
                    break;

                case SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_VXLAN_FLOW_AGING_INTERVAL:
                    cfg_info->vxlan_flow_aging_interval = msg.config.values[i][0];
                    break;

                case SHR_IFA2_MSG_CTRL_CONFIG_UPDATE_PARAM_DEVICE_ID:
                    cfg_info->device_id = msg.config.values[i][0];
                    break;
        ...
        for (i = 0; i < BCM_IFA_MAX_NUM_PORTS; i++) {
            for (j = 0; j < BCM_IFA_MAX_NUM_INT_PRI; j++) {
                cfg_info->cosq_map[i].cosq[j] = msg.cosq_map[i][j];
            }
        }
    }
    return BCM_E_NONE;
...
int _bcm_xgs5_ifa2_init(int unit, uint8 *success)
{
    ...
    *success = 0;
    ...
    ifa2_info->tailstamp_md_lns_id =
        soc_property_get(unit, spn_IFA_TAILSTAMP_METADATA_LNS_ID, 0);
    ...
    /* Send the application init message to the uC. 
       Call during warmboot also to get the FW version. */
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        /* Send the LNS format of tailstamp metadata. */
        rv = _bcm_xgs5_ifa2_tailstamp_lns_id_set(unit,
                                                 ifa2_info->tailstamp_md_lns_id);
        if (rv != BCM_E_NONE) {
            BCM_IF_ERROR_RETURN(_bcm_xgs5_ifa2_detach(unit));
            return rv;
...
_bcm_xgs5_ifa2_cfg_scache_alloc(int unit, int create)
{
    ...
    if (create) {
        ...
        if (BCM_FAILURE(rv) && (rv != BCM_E_NOT_FOUND)) {
            /* E_NOT_FOUND error means level 2 scache based warmboot is not
             * supported. Ignore  it and proceed with level 1 warmboot.
             */
            return rv;
        }
    } else {
