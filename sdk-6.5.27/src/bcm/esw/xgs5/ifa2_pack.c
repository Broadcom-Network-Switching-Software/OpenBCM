                                  ...
                                  shr_ifa2_msg_ctrl_init_t *msg)
{
    ...
    if (IFA2_UC_FEATURE_CHECK(IFA2_VXLAN_IFA_TERM_SUPPORT)) {
        _SHR_PACK_U32(buf, msg->max_vxlan_term_flows);
    }
    return buf;
}
                                       ...
                                       shr_ifa2_msg_ctrl_stats_set_t *msg)
{
    ...
    if (IFA2_UC_FEATURE_CHECK(IFA2_VXLAN_IFA_TERM_SUPPORT)) {
        _SHR_PACK_U32(buf, msg->vxlan_term_lookup_miss_cnt_hi);
        _SHR_PACK_U32(buf, msg->vxlan_term_lookup_miss_cnt_lo);
        _SHR_PACK_U32(buf, msg->vxlan_data_flow_table_insert_fail_cnt_hi);
        _SHR_PACK_U32(buf, msg->vxlan_data_flow_table_insert_fail_cnt_lo);
    }
                                         ...
                                         shr_ifa2_msg_ctrl_stats_get_t *msg)
{
    ...
    if (IFA2_UC_FEATURE_CHECK(IFA2_VXLAN_IFA_TERM_SUPPORT)) {
        _SHR_UNPACK_U32(buf, msg->vxlan_term_lookup_miss_cnt_hi);
        _SHR_UNPACK_U32(buf, msg->vxlan_term_lookup_miss_cnt_lo);
        _SHR_UNPACK_U32(buf, msg->vxlan_data_flow_table_insert_fail_cnt_hi);
        _SHR_UNPACK_U32(buf, msg->vxlan_data_flow_table_insert_fail_cnt_lo);
    }
    return buf;
}
                                      ...
                                      shr_ifa2_msg_ctrl_cosq_map_t *msg)
{
    ...
    for (i = 0; i < SHR_IFA_UC_MAX_NUM_PORTS; i++) {
        for (j = 0; j < SHR_IFA_UC_MAX_NUM_INT_PRI; j++) {
            _SHR_PACK_U8(buf, msg->cosq_map[i][j]);
        }
    }
...
uint8 *
_bcm_xgs5_ifa2_msg_ctrl_config_get_unpack(uint8 *buf,
                                          shr_ifa2_msg_ctrl_config_get_t *msg)
{
    int i, j;

    _SHR_UNPACK_U32(buf, msg->config.flags);
    _SHR_UNPACK_U16(buf, msg->config.num_params);

    for (i = 0; i < msg->config.num_params; i++) {
        _SHR_UNPACK_U32(buf, msg->config.params[i]);
        _SHR_UNPACK_U16(buf, msg->config.num_values[i]);
        for (j = 0; j < msg->config.num_values[i]; j++) {
            _SHR_UNPACK_U32(buf, msg->config.values[i][j]);
        }
    }

    for (i = 0; i < SHR_IFA_UC_MAX_NUM_PORTS; i++) {
        for (j = 0; j < SHR_IFA_UC_MAX_NUM_INT_PRI; j++) {
            _SHR_UNPACK_U8(buf, msg->cosq_map[i][j]);
        }
    }
    return buf;
}
