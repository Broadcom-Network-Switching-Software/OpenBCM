...
/* BFD Endpoint Vxlan Tunnel Configuration */
typedef struct bfd_endpoint_vxlan_tunnel_config_s {
...
} bfd_endpoint_vxlan_tunnel_config_t;
...
#if defined(BCM_EDK_SUPPORT) && defined(EDKHLIB) && defined(BCM_TOMAHAWK3_SUPPORT)
    /* EDK rProc handle */
    void *rproc;
#endif
} bfd_info_t;
...
bcmi_xgs5_bfd_encap_create(int unit, bfd_endpoint_config_t *endpoint_config,
                           uint8 *encap_data);
STATIC int
bcmi_xgs5_bfd_vxlan_encap_create(int unit, bfd_endpoint_config_t *endpoint_config,
                                 uint8 *encap_data);

STATIC int
bcmi_xgs5_bfd_endpoint_gport_resolve(int unit,
                                uint8 is_tx,
                                uint8 is_tx_raw_ingrs,
...
static uint32 _bfd_sdk_warmboot_version [][2] = {
    ...
    {((6 << 16) | (5 << 8) | 21 ), BCM_BFD_WB_VERSION_1_7}
};
...
#define BFD_MAX_EDK_MSG_TIMEOUT (5000) /* 5 secs */

/* ProcMgr APIs */
extern void *edk_procmgr_open(int unit, int core);
extern void edk_procmgr_close(void *rproc);

extern int edk_procmgr_load(void *rproc, char *exec_file, char *res_file,
                            char *core_file, char *trace_file, char *bootargs,
                            char *appargs);
extern int edk_procmgr_unload(void *rproc);

/* RPC APIs */
extern int edk_rpc_call_sync(void *rproc, char *fx_name, void *inp,
                             int insize, void *outp, int outsize,
                             int *retval, int timeoutms);

/** Notification Callback function */
typedef void(*notify_callback_t) (int event, void *cookie, uint64 params);

typedef struct {
    notify_callback_t notify_cb;
    void *cookie;
} notify_event_handler_t;

extern int edk_notify_register(void *rproc, int event, notify_callback_t callback, void *cookie);
extern void edk_notify_unregister(void *rproc, int event);
#endif
...
#define BFD_ENCAP_PKT_VXLAN                   (1 << 26)
/*
...
/* Offset of the VNID in the VxLAN header. */
#define BFD_VXLAN_HDR_VNID_OFFSET (4)
...
/* VxLAN Header */
typedef struct bfd_vxlan_header_s {
    uint32 flags; /* Flags */
    uint32 vnid;  /* VXLAN Network Identifier */
} bfd_vxlan_header_t;
...
#define BCM_BFD_ENCAP_TYPE_TNL_VXLAN(x) ((x == bcmBFDTunnelTypeVxlanIPv4InIPv4) || (x == bcmBFDTunnelTypeVxlanIPv4InIPv6) || \
        (x == bcmBFDTunnelTypeVxlanIPv6InIPv4) || (x == bcmBFDTunnelTypeVxlanIPv6InIPv6))

/*! Virtual eXtensible Local Area Network(VxLAN) header length. */
#define BFD_VXLAN_HEADER_LENGTH              8

/*! VLAN header length. */
#define BFD_VLAN_HEADER_LENGTH               4

/*! L2 ether type length. */
#define BFD_L2_ETYPE_LENGTH                  2
...
#if defined(BCM_EDK_SUPPORT) && defined(EDKHLIB) && defined(BCM_TOMAHAWK3_SUPPORT)
        if (bfd_info->rproc != NULL) {
            int ret = BCM_E_NONE;
            int reply_len;
            ret = edk_rpc_call_sync(bfd_info->rproc,
                                    "uc_bfd_event_mask_set", &event_mask,
                                    sizeof(uint32), NULL, 0,
                                    &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
            if (ret != BCM_E_NONE) {
                return BCM_E_INTERNAL;
            } else if (reply_len != sizeof(uint32)) {
                return reply_len;
            }
        } else
#endif
...
#if defined(BCM_EDK_SUPPORT) && defined(EDKHLIB) && defined(BCM_TOMAHAWK3_SUPPORT)
STATIC void bfd_event_notify_process(int event_id, void *data,
                                     uint64 ep_id_event)
{
    bfd_info_t *bfd_info = (bfd_info_t *)data;
    bfd_event_handler_t *event_handler;
    bcm_bfd_event_types_t events, cb_events;
    uint16 sess_id = ((ep_id_event >> 32) & 0xFFFF);
    uint32 event_mask = (ep_id_event & 0xFFFFFFFF);
    int invoke = 0;
...
STATIC uint8 *
bcmi_xgs5_bfd_vxlan_header_pack(uint8 *buf, bfd_vxlan_header_t *vxlan)
{
    BFD_ENCAP_PACK_U32(buf, vxlan->flags);
    BFD_ENCAP_PACK_U32(buf, vxlan->vnid);
...
#if defined(BCM_TRIDENT3_SUPPORT)
STATIC uint8 *
bcmi_xgs5_bfd_hg3_header_pack(uint8 *buffer, int unit,
                              bfd_endpoint_config_t *endpoint_config,
                              bfd_l2_header_t *l2)
{
    soc_higig3_hdr_t *hg = (soc_higig3_hdr_t *)buffer;
    int hg_size;
    int src_mod;
    int src_port;
    int dst_mod;
    ...
    sal_memset(hg, 0, hg_size);
    soc_higig3_field_set(unit, hg, HG_start, bcm_td3_tx_hg3_ethertype_get(unit));
    soc_higig3_field_set(unit, hg, HG_ppd_type, ppd);
    soc_higig3_field_set(unit, hg, HG_opcode, SOC_HIGIG_OP_UC);
    soc_higig3_field_set(unit, hg, HG_vlan_tag, vlan_val);
    soc_higig3_field_set(unit, hg, HG_dst_mod, dst_mod);
    soc_higig3_field_set(unit, hg, HG_dst_port, dst_port);
    soc_higig3_field_set(unit, hg, HG_src_mod, src_mod);
    soc_higig3_field_set(unit, hg, HG_src_port, src_port);
    soc_higig3_field_set(unit, hg, HG_pfm, pfm);
    soc_higig3_field_set(unit, hg, HG_tc, prio_val);

    if (l2->inner_vlan_tag.tpid != 0) {
        soc_higig3_field_set(unit, hg, HG_ingress_tagged, 1);
    }
                                   ...
                                   bfd_l2_header_t *l2)
{
    ...
    if ((BCM_GPORT_INVALID != endpoint_info->tx_gport)
        && (soc_feature(unit, soc_feature_visibility))) {
        src_pp_port = endpoint_config->tx_port;
    }
                               ...
                               uint8 *buffer)
{
    ...
    if (endpoint_config->is_tx_raw_ingrs) {
        BCM_IF_ERROR_RETURN
            (bcmi_xgs5_bfd_tx_raw_l2_header_get(unit, endpoint_config,
...
#if defined(BCM_TRIDENT3_SUPPORT)
    else if (soc_feature(unit, soc_feature_higig3) && (packet_flags & BFD_ENCAP_PKT_HG3)) {
        cur_ptr = bcmi_xgs5_bfd_hg3_header_pack(cur_ptr, unit,
                                                endpoint_config, &l2);
    }
#endif
    if (cur_ptr == NULL) {
                            ...
                            uint32 packet_flags)
{
    ...
    if (!(packet_flags & BFD_ENCAP_PKT_UDP__MULTI_HOP)) {
        ...
        /* For multihop endpoints IP TTL should not be 0*/
        if (endpoint_info->ip_ttl == 0) {
            return BCM_E_PARAM;
        }
    }
                                 ...
                                 uint8 *encap_data)
{
    ...
    bfd_endpoint_vxlan_tunnel_config_t bfd_tnl_info;
    bfd_info_t *bfd_info = BFD_INFO(unit);
    bcm_bfd_endpoint_info_t *endpoint_info = &endpoint_config->info;
    bfd_udp_header_t  udp;
    bfd_ipv4_header_t ipv4;
    bfd_ipv6_header_t ipv6;
    bfd_l2_header_t l2,inner_l2;
    bfd_vxlan_header_t vxlan;
    int udp_length = 0, bfd_pkt_length = 0;
    uint8 ip_tos;
    uint8 *cur_ptr = encap_data;
    uint8 *ptr_tmp = encap_data;
    bcm_port_t     tx_port;
    uint16 etype;
    int num_vlans = 0;
    int rv = BCM_E_NONE;
     ...
     * Destination IP: Either VTEP DIP or loopback address.
     */
    if ((endpoint_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv6) ||
        (endpoint_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4)) {
        return BCM_E_PARAM;
    }

    tnl_info.tunnel_id = endpoint_info->gport;
    rv = bcm_esw_vxlan_tunnel_initiator_get(unit, &tnl_info);
    if (BCM_FAILURE(rv)) {
        bcm_flow_tunnel_initiator_t_init(&tiinfo);
        tiinfo.tunnel_id = endpoint_info->gport;
        rv = bcm_esw_flow_tunnel_initiator_get(unit, &tiinfo, 0, NULL);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM, (BSL_META_U(unit,
                      "BFD tunnel initiator get failed.\n")));
            return rv;
        } else {
            sal_memcpy(bfd_tnl_info.dmac, tiinfo.dmac, sizeof(bcm_mac_t));
            sal_memcpy(bfd_tnl_info.smac, tiinfo.smac, sizeof(bcm_mac_t));
            bfd_tnl_info.vlan = tiinfo.vlan;
            bfd_tnl_info.tpid = tiinfo.tpid;
            bfd_tnl_info.pkt_pri = tiinfo.pkt_pri;
            bfd_tnl_info.pkt_cfi = tiinfo.pkt_cfi;
            bfd_tnl_info.dscp = tiinfo.dscp;
            bfd_tnl_info.ecn = tiinfo.ecn;
            bfd_tnl_info.ttl = tiinfo.ttl;
            bfd_tnl_info.dip = tiinfo.dip;
            bfd_tnl_info.sip = tiinfo.sip;
            sal_memcpy(bfd_tnl_info.dip6, tiinfo.dip6, BCM_IP6_ADDRLEN);
            sal_memcpy(bfd_tnl_info.sip6, tiinfo.sip6, BCM_IP6_ADDRLEN);
            bfd_tnl_info.udp_src_port = tiinfo.udp_src_port;
            bfd_tnl_info.udp_dst_port = tiinfo.udp_dst_port;
            endpoint_config->encap_type = SHR_BFD_ENCAP_TYPE_V4UDP;
        }
    } else {
        sal_memcpy(bfd_tnl_info.dmac, tnl_info.dmac, sizeof(bcm_mac_t));
        sal_memcpy(bfd_tnl_info.smac, tnl_info.smac, sizeof(bcm_mac_t));
        bfd_tnl_info.vlan = tnl_info.vlan;
        bfd_tnl_info.tpid = tnl_info.tpid;
        bfd_tnl_info.pkt_pri = tnl_info.pkt_pri;
        bfd_tnl_info.pkt_cfi = tnl_info.pkt_cfi;
        bfd_tnl_info.dscp = tnl_info.dscp;
        bfd_tnl_info.ecn = tnl_info.ecn;
        bfd_tnl_info.ttl = tnl_info.ttl;
        bfd_tnl_info.dip = tnl_info.dip;
        bfd_tnl_info.sip = tnl_info.sip;
        sal_memcpy(bfd_tnl_info.dip6, tnl_info.dip6, BCM_IP6_ADDRLEN);
        sal_memcpy(bfd_tnl_info.sip6, tnl_info.sip6, BCM_IP6_ADDRLEN);
        bfd_tnl_info.udp_src_port = tnl_info.udp_src_port;
        bfd_tnl_info.udp_dst_port = tnl_info.udp_dst_port;
        endpoint_config->encap_type = SHR_BFD_ENCAP_TYPE_V6UDP;
    }

    bfd_pkt_length += SHR_BFD_CONTROL_HEADER_LENGTH;
    if (endpoint_info->auth == bcmBFDAuthTypeSimplePassword) {
        bfd_pkt_length += SHR_BFD_AUTH_SP_HEADER_START_LENGTH +
                    bfd_info->auth_sp[endpoint_info->auth_index].length;
    } else if ((endpoint_info->auth == bcmBFDAuthTypeKeyedSHA1)  ||
               (endpoint_info->auth == bcmBFDAuthTypeMeticulousKeyedSHA1)) {
        bfd_pkt_length += SHR_BFD_AUTH_SHA1_LENGTH;
    }

    /* Outer L2 header */
    if ((endpoint_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4) ||
        (endpoint_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4)) {
        etype = SHR_BFD_L2_ETYPE_IPV4;
    } else {
        etype = SHR_BFD_L2_ETYPE_IPV6;
    }
    ...
    tx_port = endpoint_config->tx_port;
    BCM_IF_ERROR_RETURN(
    bcmi_xgs5_bfd_l2_header_get(unit, endpoint_info, 0,tx_port, etype, &l2));
    cur_ptr = bcmi_xgs5_bfd_l2_header_pack(cur_ptr, &l2);

    /* Inner L2 header */
    sal_memset(&inner_l2, 0, sizeof(inner_l2));
    sal_memcpy(inner_l2.dst_mac, endpoint_info->dst_mac, BFD_MAC_ADDR_LENGTH);
    sal_memcpy(inner_l2.src_mac, endpoint_info->src_mac, BFD_MAC_ADDR_LENGTH);

    if(endpoint_info->vlan_tpid) {
        inner_l2.outer_vlan_tag.tpid = endpoint_info->vlan_tpid;
        inner_l2.outer_vlan_tag.tci.vid  = endpoint_info->pkt_vlan_id & 0xFFF;
        inner_l2.outer_vlan_tag.tci.prio =  (endpoint_info->pkt_vlan_id >> 13) & 0x7;
        num_vlans++;
    }

    if (endpoint_info->inner_vlan_tpid) {
        inner_l2.inner_vlan_tag.tpid = endpoint_info->inner_vlan_tpid;
        inner_l2.inner_vlan_tag.tci.vid  = endpoint_info->pkt_inner_vlan_id & 0xFFF;
        inner_l2.inner_vlan_tag.tci.prio =  (endpoint_info->pkt_inner_vlan_id >> 13) & 0x7;
        num_vlans++;
    }

    if ((endpoint_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv6) ||
        (endpoint_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4)) {
        inner_l2.etype = SHR_BFD_L2_ETYPE_IPV4;
    } else {
        inner_l2.etype = SHR_BFD_L2_ETYPE_IPV6;
    }
    ...
    if ((endpoint_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4) ||
        ...
        /* Outer IPv4 header. */
        sal_memset(&ipv4, 0, sizeof(ipv4));
        ipv4.version = SHR_BFD_IPV4_VERSION;
        ipv4.h_length = SHR_BFD_IPV4_HEADER_LENGTH_WORDS;
        ip_tos = ((bfd_tnl_info.dscp & 0x3F << 2) | (bfd_tnl_info.ecn & 0x3));
        ipv4.dscp = ip_tos;
        ipv4.ttl = bfd_tnl_info.ttl;

        if (endpoint_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4) {
            udp_length = SHR_BFD_IPV4_HEADER_LENGTH;
        } else {
            udp_length = SHR_BFD_IPV6_HEADER_LENGTH;
        }
        udp_length += (bfd_pkt_length + (SHR_BFD_UDP_HEADER_LENGTH * 2) +
                       BFD_VXLAN_HEADER_LENGTH +
                       (SHR_BFD_MAC_ADDR_LENGTH * 2) + BFD_L2_ETYPE_LENGTH +
                       (BFD_VLAN_HEADER_LENGTH * num_vlans)+
                        SHR_BFD_IPV4_HEADER_LENGTH);

        ipv4.length = udp_length;
        ipv4.protocol = SHR_BFD_IP_PROTOCOL_UDP;
        ipv4.src = bfd_tnl_info.sip;
        ipv4.dst = bfd_tnl_info.dip;
        ptr_tmp = cur_ptr;
        cur_ptr = bcmi_xgs5_bfd_ipv4_header_pack(cur_ptr, &ipv4);

        ipv4.sum = _shr_ip_chksum(SHR_BFD_IPV4_HEADER_LENGTH, ptr_tmp);
        cur_ptr = bcmi_xgs5_bfd_ipv4_header_pack(ptr_tmp, &ipv4);

        udp_length -= SHR_BFD_IPV4_HEADER_LENGTH;
    } else {
        /* Outer IPv6 header. */
        sal_memset(&ipv6, 0, sizeof(ipv6));
        ipv6.version = SHR_BFD_IPV6_VERSION;
        ip_tos = ((bfd_tnl_info.dscp & 0x3F << 2) | (bfd_tnl_info.ecn & 0x3));
        ipv6.t_class = ip_tos;
        ipv6.f_label = 0;
        ipv6.next_header = SHR_BFD_IP_PROTOCOL_UDP;
        ipv6.hop_limit = bfd_tnl_info.ttl;
        ...
        udp_length += (bfd_pkt_length + (SHR_BFD_UDP_HEADER_LENGTH * 2) +
                       BFD_VXLAN_HEADER_LENGTH + (BFD_MAC_ADDR_LENGTH * 2) +
                       (BFD_VLAN_HEADER_LENGTH * num_vlans)+
                       BFD_L2_ETYPE_LENGTH);

        ipv6.p_length = udp_length;
        sal_memcpy(ipv6.src, bfd_tnl_info.sip6, BCM_IP6_ADDRLEN);
        sal_memcpy(ipv6.dst, bfd_tnl_info.dip6, BCM_IP6_ADDRLEN);
        cur_ptr = bcmi_xgs5_bfd_ipv6_header_pack(cur_ptr, &ipv6);
    }
    ...
    ptr_tmp = cur_ptr;
    sal_memset(&udp, 0, sizeof(udp));
    udp.src = bfd_tnl_info.udp_src_port;
    udp.dst = bfd_tnl_info.udp_dst_port;
    udp.length = udp_length;

    if ((endpoint_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv4) ||
        (endpoint_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv4)) {
        bcmi_xgs5_bfd_udp_checksum_set(0, &ipv4, 0, &udp);
    } else if ((endpoint_info->type ==bcmBFDTunnelTypeVxlanIPv4InIPv6) ||
               (endpoint_info->type == bcmBFDTunnelTypeVxlanIPv6InIPv6)) {
        bcmi_xgs5_bfd_udp_checksum_set(1, 0, &ipv6, &udp);
    } else {
        return BCM_E_PARAM;
    }
    ...
    /* VxLAN header. */
    /*
     * cur_ptr is at the start of VxLAN header, find the offset to the VNID
     * and set it as the lookup key.
     */
    endpoint_config->lkey_offset = (cur_ptr - encap_data) + SHR_BFD_VXLAN_HEADER_VNID_OFFSET;
    endpoint_config->lkey_length  = SHR_BFD_VXLAN_HEADER_VNID_LENGTH;

    sal_memset(&vxlan, 0, sizeof(vxlan));
    /*
     * I flag MUST be set to 1 for a valid VXLAN Network ID (VNI).
     * The other 7 bits are reserved fields and MUST be set to zero.
     * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
     ...
     * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
     */
    vxlan.flags = 0x8000000;
    vxlan.vnid  = endpoint_info->vxlan_vnid << 8;
    cur_ptr = bcmi_xgs5_bfd_vxlan_header_pack(cur_ptr, &vxlan);

    /* Inner L2 header */
    cur_ptr = bcmi_xgs5_bfd_l2_header_pack(cur_ptr, &inner_l2);
    ...
    if ((endpoint_info->type == bcmBFDTunnelTypeVxlanIPv4InIPv6) ||
        ...
        /* Inner SIP should be outer SIP(originating VTEP). */
        if (endpoint_info->src_ip_addr != bfd_tnl_info.sip) {
            return BCM_E_PARAM;
        }

        /* Inner DIP should be either outer DIP(VTEP) or loopback address. */
        if ((endpoint_info->dst_ip_addr != bfd_tnl_info.dip) &&
            (!IPV4_LOOPBACK(endpoint_info->dst_ip_addr))) {
            return BCM_E_PARAM;
        }

        ipv4.version = SHR_BFD_IPV4_VERSION;
        ipv4.h_length = SHR_BFD_IPV4_HEADER_LENGTH_WORDS;
        ipv4.dscp =  endpoint_info->ip_tos;
        ipv4.ttl = 0xFF;
        ipv4.protocol = SHR_BFD_IP_PROTOCOL_UDP;
        ipv4.src = endpoint_info->src_ip_addr;
        ipv4.dst = endpoint_info->dst_ip_addr;
        ipv4.length = bfd_pkt_length + SHR_BFD_UDP_HEADER_LENGTH + SHR_BFD_IPV4_HEADER_LENGTH;
        ptr_tmp = cur_ptr;
        cur_ptr = bcmi_xgs5_bfd_ipv4_header_pack(cur_ptr, &ipv4);
        ipv4.sum = _shr_ip_chksum(SHR_BFD_IPV4_HEADER_LENGTH, ptr_tmp);
        cur_ptr = bcmi_xgs5_bfd_ipv4_header_pack(ptr_tmp, &ipv4);
        ...
        /* Inner SIP should be outer SIP(originating VTEP). */
        if (sal_memcmp(endpoint_info->src_ip6_addr, bfd_tnl_info.sip6, BCM_IP6_ADDRLEN)) {
            return BCM_E_PARAM;
        }

        /* Inner DIP should be either outer DIP(VTEP) or loopback address. */
        if ((sal_memcmp(endpoint_info->dst_ip6_addr, bfd_tnl_info.dip6, BCM_IP6_ADDRLEN)) &&
            !IPV6_LOOPBACK(endpoint_info->dst_ip6_addr)) {
            return BCM_E_PARAM;
        }

        ipv6.version = SHR_BFD_IPV6_VERSION;
        ipv6.t_class = endpoint_info->ip_tos;
        ipv6.f_label = 0;
        ipv6.next_header = SHR_BFD_IP_PROTOCOL_UDP;
        ipv6.hop_limit = 0xFF;
        ipv6.p_length = bfd_pkt_length + SHR_BFD_UDP_HEADER_LENGTH;
    ...
    sal_memset(&udp, 0, sizeof(udp));
    udp.src = endpoint_info->udp_src_port;
    udp.dst = SHR_BFD_UDP_SINGLE_HOP_DEST_PORT;
    udp.length = bfd_pkt_length + SHR_BFD_UDP_HEADER_LENGTH;
    ...
    endpoint_config->encap_length = cur_ptr - encap_data;
    return BCM_E_NONE;
}
/*
 * Function:
                           ...
                           uint8 *encap_data)
{
...
#if defined(BCM_TRIDENT3_SUPPORT)
        else if (soc_feature(unit, soc_feature_higig3) &&
                 bcm_td3_is_hg3_enabled_port(unit, tx_port)) {
            packet_flags |= BFD_ENCAP_PKT_HG3;
        }
#endif
        else {
    ...
    if (endpoint_config->is_tx_raw_ingrs) {
        ...
        if (ip_flag & BFD_ENCAP_PKT_IP__V6) {
            raw_ingrs_pkt_flags = BFD_ENCAP_PKT_IP__V6 | BFD_ENCAP_PKT_UDP | BFD_ENCAP_PKT_IP;
        } else {
...
#if defined(BCM_EDK_SUPPORT) && defined(EDKHLIB) && defined(BCM_TOMAHAWK3_SUPPORT)
    if (bfd_info->rproc != NULL) {
        uint32 edk_host_ver = BFD_EDK_HOST_VERSION;
        uint32 edk_fw_ver;
        int reply_len;
        int ret = BCM_E_NONE;
        ret = edk_rpc_call_sync(bfd_info->rproc,
                                "uc_bfd_version_exchange", &edk_host_ver,
                                sizeof(uint32), &edk_fw_ver,
                                sizeof(uint32), &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if ((ret != BCM_E_NONE) ||
            (edk_fw_ver != BFD_EDK_HOST_VERSION)) {
            return BCM_E_INTERNAL;
        } else if (reply_len != sizeof(uint32)) {
            return reply_len;
        }
        bfd_firmware_version = edk_fw_ver;
    } else
#endif
    {
        uint16 reply_len;
...
bcmi_xgs5_bfd_match_id(int unit, bfd_info_t *bfd_info)
{
    ...
    rv = soc_cancun_ceh_obj_field_get(unit, "Z2_MATCH_ID", "VXLAN", &ceh_info);
    if (rv != SOC_E_NONE){
        return BCM_E_INTERNAL;
    }

    match_id_field.match_id_fields[SHR_BFD_UC_VXLAN].offset = (ceh_info.offset + zone_width[0] +
                                                 zone_width[1]);
    match_id_field.match_id_fields[SHR_BFD_UC_VXLAN].width = ceh_info.width;
    match_id_field.match_id_fields[SHR_BFD_UC_VXLAN].value = ceh_info.value;
    match_id_field.match_id_types += 1;

    rv = soc_cancun_ceh_obj_field_get(unit, "Z3_MATCH_ID", "SINGLE_OTAG", &ceh_info);
    if (rv != SOC_E_NONE){
        return BCM_E_INTERNAL;
    }

    match_id_field.match_id_fields[SHR_BFD_UC_INNER_SINGLE_TAG].offset = (ceh_info.offset + zone_width[0] +
                                                 zone_width[1] + zone_width[2]);
    match_id_field.match_id_fields[SHR_BFD_UC_INNER_SINGLE_TAG].width = ceh_info.width;
    match_id_field.match_id_fields[SHR_BFD_UC_INNER_SINGLE_TAG].value = ceh_info.value;
    match_id_field.match_id_types += 1;

    rv = soc_cancun_ceh_obj_field_get(unit, "Z3_MATCH_ID", "OTAG_ITAG", &ceh_info);
    if (rv != SOC_E_NONE){
        return BCM_E_INTERNAL;
    }

    match_id_field.match_id_fields[SHR_BFD_UC_INNER_DOUBLE_TAG].offset = (ceh_info.offset + zone_width[0] +
                                                 zone_width[1] + zone_width[2]);
    match_id_field.match_id_fields[SHR_BFD_UC_INNER_DOUBLE_TAG].width = ceh_info.width;
    match_id_field.match_id_fields[SHR_BFD_UC_INNER_DOUBLE_TAG].value = ceh_info.value;
    match_id_field.match_id_types += 1;
...
/*
 * Function:
 *      bcmi_xgs5_iif_update_callback
 * Purpose:
 *      Receive l3iif to vrf mapping notification from L3 and send it to BFD FW eAPP
 * Parameters:
 *      unit      - (IN) Unit number.
 *      intf_id   - (IN) L3 Input Interface id
 *      vrf       - (IN) vrf id
 ...
 * Notes:
 */
int bcmi_xgs5_iif_update_callback(int unit, bcm_if_t intf_id, bcm_vrf_t vrf, void *user_data)
{
#if defined(BCM_CMICM_SUPPORT) ||  defined(BCM_CMICX_SUPPORT)
    int rv;
    uint8 *buffer, *buffer_ptr;
    uint16 buffer_len, reply_len;
    bfd_info_t *bfd_info = BFD_INFO(unit);
    bfd_sdk_msg_ctrl_l3iif_vrf_map_t msg;

    FEATURE_CHECK(unit);

    if ((!BFD_INIT(unit)) || (!soc_feature(unit, soc_feature_bfd_l3iif_vrf))) {
        return BCM_E_NONE;
    }
    ...
    buffer     = bfd_info->dma_buffer;
    buffer_ptr = bfd_sdk_msg_ctrl_l3iif_vrf_map_pack(buffer, &msg);
    buffer_len = buffer_ptr - buffer;

    /* Send L3_IIF to VRF map message to uC */
    rv = bcmi_xgs5_bfd_msg_send_receive(unit,
                                        MOS_MSG_SUBCLASS_BFD_L3IIF_VRF_MAP,
                                        buffer_len, 0,
                                        MOS_MSG_SUBCLASS_BFD_L3IIF_VRF_MAP_REPLY,
                                        &reply_len, bfd_uc_msg_timeout_usecs);
                   ...
                   bcm_xgs5_bfd_hw_defs_t *hw_defs)
{
    ...
    uint8 *buffer, *buffer_ptr;
    uint16 buffer_len;
    int uc = CMICM_NUM_UCS;
    int priority;
    uint32 bfd_encap_memory_size = 0;
    uint8  session_down_on_create = 0;
    uint8  bfd_remote_discr_reset = 0;
    int    status;
    bfd_sdk_msg_ctrl_trace_log_enable_t msg_trace;
    uint32 dip_based_lookup_enable = 0;
    void *tmp_rproc = NULL;
    uint8  bfd_l3iif_vrf_lookup_enable = 0;
    uint32 vxlan_vnid_sip_lookup_enable = 0;
    ...
    if(!(SOC_IS_TRIDENT3X(unit))) {
#endif
        if ((soc_property_get(unit, spn_BFD_IFP_LOOKUP_ENABLE, 0)) != 0) {
            return BCM_E_CONFIG;
        }
#if defined(BCM_TRIDENT3_SUPPORT)
    }
    ...
    if (BFD_INIT(unit)) {
        /* Setting before_init to true since appl_init is not yet called */
        BCM_IF_ERROR_RETURN(bcmi_xgs5_bfd_detach(unit, 1));
    }
    ...
    for (uc = 0; uc < CMICM_NUM_UCS; uc++) {
        tmp_rproc = edk_procmgr_open(unit, uc);
        if (tmp_rproc != NULL) {
            uint32 edk_host_ver = BFD_EDK_HOST_VERSION;
            uint32 edk_fw_ver;
            int ret = 0;
            int reply_len;
            ret = edk_rpc_call_sync(tmp_rproc,
                                    "uc_bfd_version_exchange", &edk_host_ver,
                                    sizeof(uint32), &edk_fw_ver,
                                    sizeof(uint32), &reply_len,
                                    BFD_MAX_EDK_MSG_TIMEOUT);
            if ((ret == 0) &&
                (reply_len == sizeof(uint32)) &&
                (edk_fw_ver == BFD_EDK_HOST_VERSION)) {
                break;
            }
            edk_procmgr_close(tmp_rproc);
            tmp_rproc = NULL;
        } else {
            /* EDK Host not initialized */
            break;
        }
    }
#endif
    ...
    bfd_remote_discr_reset = soc_property_get(unit,
                                    spn_BFD_REMOTE_DISCRIMINATOR_RESET, 0);

    bfd_l3iif_vrf_lookup_enable = soc_property_get(unit,
                                        spn_BFD_L3_IIF_TO_VRF_MAP_ENABLE, 0);

    vxlan_vnid_sip_lookup_enable = soc_property_get(unit,
                                          spn_BFD_OVER_VXLAN_ZERO_DISCR_DEMUX,
                                          0);
    ...
    if (bfd_info->rproc != NULL) {
        ...
        msg_init.config_flags  |=
            (bfd_remote_discr_reset?BFD_CONFIG_REMOTE_DISCR_RESET:0);
        msg_init.config_flags  |=
            (vxlan_vnid_sip_lookup_enable?BFD_CONFIG_VXLAN_VNID_SIP_LOOKUP_ENABLE:0);
        if (0 == bfd_info->uc_num ) {
            msg_init.rx_channel = NUM_CPU_ARM_COSQ(unit, 0);
        } else if (1 == bfd_info->uc_num ) {
            msg_init.rx_channel = (NUM_CPU_ARM_COSQ(unit, 0) +
                                   soc_property_get(unit, "num_queues_uc0", 0));
        }

        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_init", &msg_init,
                                sizeof(msg_init), NULL,
                                0, &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            bcmi_xgs5_bfd_free_resource(unit);
            return BCM_E_INTERNAL;
        } else if (reply_len != sizeof(msg_init)) {
            bcmi_xgs5_bfd_free_resource(unit);
            return reply_len;
        }
    } else
#endif
    {
        ...
        if (BFD_UC_FEATURE_CHECK(BFD_FEATURE_INIT_CONFIG_FLAGS)) {
            ...
            if (BFD_UC_FEATURE_CHECK(BFD_FEATURE_L3IIF_LOOKUP_FOR_VRF)) {
                msg_init.config_flags  |=
                    (bfd_l3iif_vrf_lookup_enable?BFD_CONFIG_L3IIF_VRF_LOOKUP_ENABLE:0);
            }
        }
    ...
    if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit)) {
        int max_num_port = SOC_INFO(unit).port.max;
        int min_num_port = SOC_INFO(unit).port.min;
        int i = 0;
        ...
        if (bfd_info->rproc != NULL) {
            int ret = BCM_E_NONE;
            int reply_len;
            ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_get_port_txqnum",
                                    &queue_map, sizeof(queue_map), NULL,
                                    0, &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
            if (ret != BCM_E_NONE) {
                bcmi_xgs5_bfd_free_resource(unit);
                return BCM_E_INTERNAL;
            } else if (reply_len != queue_map.max_num_port) {
                bcmi_xgs5_bfd_free_resource(unit);
                return reply_len;
            }
        } else
    ...
    if (BCM_FAILURE(rv)) {
        BCM_IF_ERROR_RETURN(bcmi_xgs5_bfd_detach(unit, 0));
        return rv;
    }
#ifdef BCM_WARM_BOOT_SUPPORT
   } 
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        ret = edk_notify_register(bfd_info->rproc, BFD_EDK_EVENT_ID,
                                  bfd_event_notify_process,
                                  bfd_info);
        if (ret != BCM_E_NONE) {
            BCM_IF_ERROR_RETURN(bcmi_xgs5_bfd_detach(unit, 0));
            return BCM_E_INTERNAL;
    ...
    /* Register to L3 for L3_IIF to VRF map updates */
    if ((BFD_UC_FEATURE_CHECK(BFD_FEATURE_L3IIF_LOOKUP_FOR_VRF)) && 
        bfd_l3iif_vrf_lookup_enable && 
        (soc_feature(unit, soc_feature_bfd_l3iif_vrf))) {
        rv = bcmi_esw_l3_iif_update_cb_register(unit, bcmi_xgs5_iif_update_callback, NULL);
        if (rv != BCM_E_NONE) {
            BCM_IF_ERROR_RETURN(bcmi_xgs5_bfd_detach(unit, 0));
            return BCM_E_INTERNAL;
        }
 ...
 * Parameters:
 *      unit - (IN) Unit number.
 *      before_init - (IN) Is it before application init
 * Returns:
 *      BCM_E_NONE Operation completed successfully
 * Notes:
 *      In progress...
 */
int
bcmi_xgs5_bfd_detach(int unit, uint8 before_init)
{
#if defined(BCM_CMICM_SUPPORT) ||  defined(BCM_CMICX_SUPPORT)
    int rv;
    bfd_info_t *bfd_info = BFD_INFO(unit);
    ...
    /* Unregister from L3 for L3_IIF to VRF map updates */
    bfd_l3iif_vrf_lookup_enable = soc_property_get(unit,
                                       spn_BFD_L3_IIF_TO_VRF_MAP_ENABLE, 0);
    if ((BFD_UC_FEATURE_CHECK(BFD_FEATURE_L3IIF_LOOKUP_FOR_VRF)) && 
        bfd_l3iif_vrf_lookup_enable && 
        (soc_feature(unit, soc_feature_bfd_l3iif_vrf))) {
        (void)bcmi_esw_l3_iif_update_cb_unregister(unit);
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (bfd_info->rproc != NULL) {
            ...
            ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_deinit", NULL, 0, NULL,
                                    0, &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
            if (ret != BCM_E_NONE) {
                return BCM_E_INTERNAL;
            } else if (reply_len != 0) {
            ...
            edk_notify_unregister(bfd_info->rproc, BFD_EDK_EVENT_ID);
            edk_procmgr_close(bfd_info->rproc);
            bfd_info->rproc = NULL;
        } else
#endif
        {
            ...
            if (status) {
                if (before_init) {
                    /* We have not yet called application init.
                     * So we do not know if the running FW has a message thread.
                     * Keep the timeout minimal. We do not care if the FW did
                     * not respond.
                     */
                    timeout = 250000;
                } else {
...

{
    ...
    if (BCM_GPORT_IS_TRUNK(endpoint_info->gport)) {
        /*  Get trunk id from gport in case of tx_gport is member port resovle */
        if ((is_tx) && !(BCM_GPORT_IS_TRUNK(endpoint_info->tx_gport))) {
            *trunk_id = SOC_GPORT_TRUNK_GET(endpoint_info->gport);
        }
        ...
        if (((!is_tx) && member_count) || (BCM_GPORT_IS_TRUNK(endpoint_info->tx_gport))) {
            ...
            *is_trunk_bfd = 1;
        } else if (((is_tx) && member_count)
                   && !(BCM_GPORT_IS_TRUNK(endpoint_info->tx_gport))) {
            for(idx = 0;idx < member_count; idx++) {
                if (endpoint_info->tx_gport == member_array[idx].gport) {
                    /* Get the Modid and Port value using Trunk Index value. */
                    rv = _bcm_esw_gport_resolve
                             (unit, member_array[idx].gport,
                              module_id, port_id, trunk_id, local_id);
                              ...
                              bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    int update;
    int endpoint_index;
    bcm_module_t module_id = BCM_MODID_INVALID;
    ...
    if (BCM_BFD_ENCAP_TYPE_TNL_VXLAN(endpoint_info->type)){
        if(!SOC_IS_TRIDENT3X(unit)) {
            return BCM_E_UNAVAIL;
        }
    }
    ...
    /* BFD Tx Raw ingress endpoint check */
    if (bfd_info->tx_raw_ingress_enable) {
        /* Tunnel Type and Src Dst MAC Set check */
        if (endpoint_info->type == bcmBFDTunnelTypeUdp) {
            ...
            if ((sal_memcmp(endpoint_info->dst_mac, mac_zero, sizeof(bcm_mac_t)))
                && (sal_memcmp(endpoint_info->src_mac, mac_zero, sizeof(bcm_mac_t)))) {
                /* Ingress Port validation */
                if (BCM_GPORT_INVALID != endpoint_info->tx_gport) {
                    if ((!BCM_GPORT_IS_LOCAL(endpoint_info->tx_gport))
                        && (!BCM_GPORT_IS_DEVPORT(endpoint_info->tx_gport))
    ...
    if (!BCM_BFD_ENCAP_TYPE_TNL_VXLAN(endpoint_info->type)){
        ...
        if (BCM_GPORT_INVALID != endpoint_info->gport) {
            BCM_IF_ERROR_RETURN
                (bcmi_xgs5_bfd_endpoint_gport_resolve(unit, 0, endpoint_config->is_tx_raw_ingrs,
                                                      endpoint_info, &module_id, &port_id, &trunk_id, &local_id, &is_trunk_bfd, &is_micro_bfd));

        } else if ((!(endpoint_config->is_tx_raw_ingrs)) && (BCM_SUCCESS
                    (bcm_esw_l3_egress_get(unit, endpoint_info->egress_if,
                                           &l3_egress)))) {
    ...
    if (is_micro_bfd){
        /* Micro bfd can't be supported for BFD Tx Raw ingress endpoints */
        if (endpoint_config->is_tx_raw_ingrs) {
            return BCM_E_PARAM;
        }
    ...
    if (BCM_BFD_ENCAP_TYPE_TNL_VXLAN(endpoint_info->type)){
        session_flags |= SHR_BFD_SESS_SET_F_VXLAN_BFD;
    }
    ...
    if (endpoint_info->flags2 & BCM_BFD_ENDPOINT_FLAGS2_MPLS_TP_RESET_REMOTE_DESCR) {
        session_flags |= SHR_BFD_SESS_SET_F_MPLS_TP_RESET_REMOTE_DISCR;
    }

    /* Enable the BFD echo mode */
    local_echo = (endpoint_info->flags & BCM_BFD_ECHO) ? 1 : 0;

    /* Bfd echo can't be supported for BFD Tx Raw ingress endpoints */
    if ((local_echo) && (endpoint_config->is_tx_raw_ingrs)) {
        return BCM_E_PARAM;
    }
    ...
    if (update) {
        ...
        if (endpoint_config->info.local_diag !=
            ...
            if (endpoint_info->local_state == bcmBFDStateDown) {
                session_flags |= SHR_BFD_SESS_SET_F_LOCAL_STATE_DOWN;
            }
        }
        ...
        if ((endpoint_config->info.auth != endpoint_info->auth) ||
            (endpoint_config->info.auth_index != endpoint_info->auth_index)) {
    ...
    if (BCM_GPORT_INVALID != endpoint_info->tx_gport) {
        rv = bcmi_xgs5_bfd_endpoint_gport_resolve(unit, 1, endpoint_config->is_tx_raw_ingrs,
              endpoint_info, &module_id, &port_id, &trunk_id, &local_id, &is_trunk_bfd, &is_micro_bfd);

        if (BCM_FAILURE(rv)) {
            if (bfd_session_hw_set || bfd_session_hw_update) {
                BCM_IF_ERROR_RETURN
                (bcmi_xgs5_bfd_session_hw_delete(unit, endpoint_config));
    ...
    if (encap) {
        if (BCM_BFD_ENCAP_TYPE_TNL_VXLAN(endpoint_info->type)) {
            rv =   bcmi_xgs5_bfd_vxlan_encap_create(unit, endpoint_config,
                    msg_sess.encap_data);
            if (BCM_FAILURE(rv)) {
                endpoint_config->endpoint_index = -1;
                sal_memcpy(endpoint_config, &old_endpoint_config, sizeof(bfd_endpoint_config_t));
    ...
    if (soc_feature(unit, soc_feature_bfd_l3iif_vrf)) {
        msg_sess.vrf_id     = endpoint_info->vrf_id;
    }
    ...
    rv = bcm_esw_cosq_port_mapping_get(unit, msg_sess.tx_port,
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_session_set",
                                &msg_sess, sizeof(msg_sess),
                                NULL, 0, &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if ((ret != BCM_E_NONE) ||
            (reply_len != sizeof(msg_sess))) {
            if (bfd_session_hw_set || bfd_session_hw_update) {
                BCM_IF_ERROR_RETURN
                (bcmi_xgs5_bfd_session_hw_delete(unit, endpoint_config));
                if (bfd_session_hw_update) {
                           ...
                           bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_session_get",
                                &endpoint, sizeof(endpoint),
                                &msg_sess, sizeof(msg_sess),
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
        } else if (reply_len != sizeof(msg_sess)) {
            return reply_len;
        }
    } else
#endif
   ...
   if (msg_sess.local_echo) {
    ...
    if (soc_feature(unit, soc_feature_bfd_l3iif_vrf)) {
        endpoint_info->vrf_id     = msg_sess.vrf_id;
    }
...
bcmi_xgs5_bfd_tx_start(int unit)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_session_tx_pdu_start",
                                NULL, 0, NULL, 0,
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
...
bcmi_xgs5_bfd_tx_stop(int unit)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_session_tx_pdu_stop",
                                NULL, 0, NULL, 0,
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
...
bcmi_xgs5_bfd_endpoint_destroy(int unit, bcm_bfd_endpoint_t endpoint)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_session_delete",
                                &endpoint_config->endpoint_index,
                                sizeof(endpoint_config->endpoint_index), NULL,
                                0, &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
        } else if (reply_len != sizeof(endpoint_config->endpoint_index)) {
            return reply_len;
        }
    } else
#endif
...
bcmi_xgs5_bfd_endpoint_destroy_all(int unit)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_session_delete_all",
                                NULL, 0, NULL, 0,
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
...
bcmi_xgs5_bfd_endpoint_poll(int unit, bcm_bfd_endpoint_t endpoint)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_session_poll",
                                &endpoint_config->endpoint_index,
                                sizeof(endpoint_config->endpoint_index),
                                NULL, 0,
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
                                ...
                                bcm_bfd_endpoint_stat_t *ctr_info, uint32 options)
{
    ...
    if (bfd_info->rproc != NULL) {
        ...
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_session_poll",
                                &stat_req, sizeof(stat_req),
                                &stat_reply, sizeof(stat_reply),
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
        } else if (reply_len != sizeof(stat_reply)) {
            return reply_len;
        }
    } else
#endif
                            ...
                            int index, bcm_bfd_auth_sha1_t *sha1)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_auth_sha1_set",
                                &msg_auth, sizeof(msg_auth),
                                NULL, 0,
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
        } else if (reply_len != sizeof(msg_auth)) {
            return reply_len;
        }
    } else
#endif
                            ...
                            int index, bcm_bfd_auth_sha1_t *sha1)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (bfd_info->rproc != NULL) {
            int ret = BCM_E_NONE;
            int reply_len;
            ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_auth_sha1_get",
                                    &index, sizeof(index),
                                    &msg_auth, sizeof(msg_auth),
                                    &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
            if (ret != BCM_E_NONE) {
                                       ...
                                       bcm_bfd_auth_simple_password_t *sp)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_auth_sp_set",
                                &msg_auth, sizeof(msg_auth),
                                NULL, 0,
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
                                       ...
                                       bcm_bfd_auth_simple_password_t *sp)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (bfd_info->rproc != NULL) {
            int ret = BCM_E_NONE;
            int reply_len;
            ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_auth_sp_get",
                                    &index, sizeof(index),
                                    &msg_auth, sizeof(msg_auth),
                                    &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
            if (ret != BCM_E_NONE) {
           ...
           int *count)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_status_multi_get",
                                NULL, 0, buffer, buffer_len,
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
        } else if (reply_len != buffer_len) {
            return reply_len;
        }
    } else
#endif
                               ...
                               bcm_bfd_discard_stat_t *discarded_info)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_discard_stat_set",
                                NULL, 0, NULL, 0,
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
                               ...
                               bcm_bfd_discard_stat_t *discarded_info)
{
    ...
    if (bfd_info->rproc != NULL) {
        int ret = BCM_E_NONE;
        int reply_len;
        ret = edk_rpc_call_sync(bfd_info->rproc, "uc_bfd_discard_stat_get",
                                NULL, 0,
                                &msg_discard_stat, sizeof(msg_discard_stat),
                                &reply_len, BFD_MAX_EDK_MSG_TIMEOUT);
        if (ret != BCM_E_NONE) {
            return BCM_E_INTERNAL;
        } else if (reply_len != sizeof(msg_discard_stat)) {
            return reply_len;
        }
    } else
#endif
...
#if defined(BCM_EDK_SUPPORT) && defined(EDKHLIB) && defined(BCM_TOMAHAWK3_SUPPORT)
    if (bfd_info->rproc != NULL) {
        discarded_info->bfd_mpls_fm_len_err = msg_discard_stat.bfd_mpls_fm_len_err;
        discarded_info->bfd_mpls_fm_type_err = msg_discard_stat.bfd_mpls_fm_type_err;
        discarded_info->bfd_echo_ipv4_mismatch = msg_discard_stat.bfd_echo_ipv4_mismatch;
        discarded_info->bfd_echo_ipv6_mismatch = msg_discard_stat.bfd_echo_ipv6_mismatch;
        discarded_info->bfd_ip_hdr_ver_err = msg_discard_stat.bfd_ip_hdr_ver_err;
        discarded_info->bfd_mpls_no_label = msg_discard_stat.bfd_mpls_no_label;
        discarded_info->bfd_gal_additional_label_miss = msg_discard_stat.bfd_gal_additional_label_miss;
        discarded_info->bfd_unknown_ach_type = msg_discard_stat.bfd_unknown_ach_type;
        discarded_info->bfd_vxlan_i_flag_err= msg_discard_stat.bfd_vxlan_i_flag_err;
        discarded_info->bfd_ip_ttl_err = msg_discard_stat.bfd_ip_ttl_err;
        discarded_info->bfd_unsupported_echo_encap_err = msg_discard_stat.bfd_unsupported_echo_encap_err;
        discarded_info->bfd_unsupported_ethertype_err = msg_discard_stat.bfd_unsupported_ethertype_err;
        discarded_info->bfd_ether_type_len_err = msg_discard_stat.bfd_ether_type_len_err;
        discarded_info->bfd_offset_err = msg_discard_stat.bfd_offset_err;
        discarded_info->bfd_malformed_pkt_err= msg_discard_stat.bfd_malformed_pkt_err;
        discarded_info->bfd_malformed_ipv4_pkt_err= msg_discard_stat.bfd_malformed_ipv4_pkt_err;
        discarded_info->bfd_malformed_ipv6_pkt_err= msg_discard_stat.bfd_malformed_ipv6_pkt_err;
        discarded_info->bfd_malformed_mpls_pkt_err= msg_discard_stat.bfd_malformed_mpls_pkt_err;
        discarded_info->bfd_sess_id_config_err = msg_discard_stat.bfd_sess_id_config_err;
        discarded_info->bfd_session_not_enabled = msg_discard_stat.bfd_session_not_enabled;
        discarded_info->bfd_your_disc_zero = msg_discard_stat.bfd_your_disc_zero;
        discarded_info->bfd_your_disc_mismatch = msg_discard_stat.bfd_your_disc_mismatch;
        discarded_info->bfd_my_disc_mismatch = msg_discard_stat.bfd_my_disc_mismatch;
        discarded_info->bfd_pkts_discarded_admin_down = msg_discard_stat.bfd_pkts_discarded_admin_down;
        discarded_info->bfd_echo_your_disc_zero = msg_discard_stat.bfd_echo_your_disc_zero;
        discarded_info->bfd_echo_your_disc_err = msg_discard_stat.bfd_echo_your_disc_err;
        discarded_info->bfd_non_cc_cv_err = msg_discard_stat.bfd_non_cc_cv_err;
        discarded_info->bfd_mpls_fm_sess_mismatch = msg_discard_stat.bfd_mpls_fm_sess_mismatch;
    } else
#endif
    {
        if(BFD_UC_FEATURE_CHECK(BFD_FEATURE_RX_DEBUG_CTRS_ENHANCED)) {
            discarded_info->bfd_mpls_fm_len_err = msg_discard_stat.bfd_mpls_fm_len_err;
            discarded_info->bfd_mpls_fm_type_err = msg_discard_stat.bfd_mpls_fm_type_err;
            discarded_info->bfd_echo_ipv4_mismatch = msg_discard_stat.bfd_echo_ipv4_mismatch;
            discarded_info->bfd_echo_ipv6_mismatch = msg_discard_stat.bfd_echo_ipv6_mismatch;
...
/*
 * Function:
 *     _bcmi_bfd_hw_reg32_udp_portctrl_init
 * Purpose:
 *     BFD RX UDP portctl reg32 init
 * Parameters:
 *     unit  - (IN) BCM device number
 *     field - (IN) soc field
 *     value - (IN) dest port
 ...
 *     BCM_E_XXX
 */
int _bcmi_bfd_hw_reg32_udp_portctrl_init(int unit, soc_field_t field, uint32 value) {
#ifdef BCM_WARM_BOOT_SUPPORT
    if (!(SOC_WARM_BOOT(unit))) {
#endif
        uint32 val;
...
/*
 * Function:
 *     _bcmi_bfd_hw_reg64_udp_portctrl_init
 * Purpose:
 *     BFD RX UDP portctl reg64 init
 * Parameters:
 *     unit  - (IN) BCM device number
 *     field - (IN) soc field
 *     value - (IN) dest port
 ...
 *     BCM_E_XXX
 */
int _bcmi_bfd_hw_reg64_udp_portctrl_init(int unit, soc_field_t field, uint32 value) {
#ifdef BCM_WARM_BOOT_SUPPORT
    if (!(SOC_WARM_BOOT(unit))) {
#endif
        uint64 val64;
...
/*
 * Function:
 *     _bcmi_bfd_hw_reg32_rx_udp_portctrl_deinit
 * Purpose:
 *     BFD RX UDP portctl reg32 deinit
 * Parameters:
 *     unit  - (IN) BCM device number
 * Returns:
 *     BCM_E_XXX
 */
int _bcmi_bfd_hw_reg32_rx_udp_portctrl_deinit(int unit) {
#ifdef BCM_WARM_BOOT_SUPPORT
    if (!(SOC_WARM_BOOT(unit))) {
#endif
        uint32 val;
...
/*
 * Function:
 *     _bcmi_bfd_hw_reg64_rx_udp_portctrl_deinit
 * Purpose:
 *     BFD RX UDP portctl reg64 deinit
 * Parameters:
 *     unit  - (IN) BCM device number
 * Returns:
 *     BCM_E_XXX
 */
int _bcmi_bfd_hw_reg64_rx_udp_portctrl_deinit(int unit) {
#ifdef BCM_WARM_BOOT_SUPPORT
    if (!(SOC_WARM_BOOT(unit))) {
#endif
        uint64 val64;
...
bcmi_xgs5_bfd_scache_alloc(int unit)
{
    ...
    if(bfd_info->wb_next_version >= BCM_BFD_WB_VERSION_1_7) {
        /* Calculate the size of VxLAN Network Identifier */
        alloc_size += (sizeof(bcm_gport_t) * bfd_info->endpoint_count);
        /* Calculate the size of Vlan Tag Protocol Identifier */
        alloc_size += (sizeof(uint16) * bfd_info->endpoint_count);
        /* Calculate the size of Inner Vlan Tag Protocol Identifier */
        alloc_size += (sizeof(uint16) * bfd_info->endpoint_count);
    }
...
bcmi_xgs5_bfd_sync(int unit)
{
    ...
    do {
        ...
        for (idx = 0; idx < bfd_info->endpoint_count; idx++) {
             ...
             if (bfd_info->wb_next_version >= BCM_BFD_WB_VERSION_1_7) {
                 sal_memcpy(bfd_scache, &endpoint_config->info.vxlan_vnid, sizeof(bcm_gport_t));
                 bfd_scache += sizeof(bcm_gport_t);

                 sal_memcpy(bfd_scache, &endpoint_config->info.vlan_tpid, sizeof(uint16));
                 bfd_scache += sizeof(uint16);

                 sal_memcpy(bfd_scache, &endpoint_config->info.inner_vlan_tpid, sizeof(uint16));
                 bfd_scache += sizeof(uint16);
             }
             ep_not_use++;
        }
        for (idx = ep_not_use; idx < bfd_info->endpoint_count; idx++) {
             ...
             if (bfd_info->wb_next_version >= BCM_BFD_WB_VERSION_1_7) {
                 bfd_scache += sizeof(bcm_gport_t); /* VxLAN Network Identifier. */
                 bfd_scache += sizeof(uint16); /* Vlan Tag Protocol Identifier. */
                 bfd_scache += sizeof(uint16); /* Inner Vlan Tag Protocol Identifier. */
             }
        }
    }while(0);
...
bcmi_xgs5_bfd_recover(int unit, int stable_size, uint8 **bfd_scache, uint16 recovered_ver)
{
    ...
    for (idx = 0; idx < bfd_info->endpoint_count; idx++) {
        ...
        if (endpoint_config) {
            ...
            if (recovered_ver >= BCM_BFD_WB_VERSION_1_7) {
                sal_memcpy(&endpoint_config->info.vxlan_vnid, *bfd_scache, sizeof(bcm_gport_t));
                *bfd_scache += sizeof(bcm_gport_t);

                sal_memcpy(&endpoint_config->info.vlan_tpid, *bfd_scache, sizeof(uint16));
                *bfd_scache += sizeof(uint16);

                sal_memcpy(&endpoint_config->info.inner_vlan_tpid, *bfd_scache, sizeof(uint16));
                *bfd_scache += sizeof(uint16);
            }
            ...
            if (bfd_info->tx_raw_ingress_enable) {
                ...
                if (endpoint_config->info.type == bcmBFDTunnelTypeUdp) {
                    ...
                    if ((sal_memcmp(endpoint_config->info.dst_mac, mac_zero, sizeof(bcm_mac_t)))
                        && (sal_memcmp(endpoint_config->info.src_mac, mac_zero, sizeof(bcm_mac_t)))) {
                        /* Ingress Port validation */
                        if (BCM_GPORT_INVALID != endpoint_config->info.tx_gport) {
                            if ((!BCM_GPORT_IS_LOCAL(endpoint_config->info.tx_gport))
                                && (!BCM_GPORT_IS_DEVPORT(endpoint_config->info.tx_gport))
                                && (!BCM_GPORT_IS_MODPORT(endpoint_config->info.tx_gport))) {
                                return BCM_E_PARAM;
                            }
                        }
                        endpoint_config->is_tx_raw_ingrs = 1;
            ...
            /*
             * For BFD over VXLAN tunnel type, Gport was assigned as tunnel_id from the Flow Tunnel Initiator.
             * So resolving gport is not needed while endpoint creation/recovery.
             */
            if (!BCM_BFD_ENCAP_TYPE_TNL_VXLAN(endpoint_config->info.type)){
                sal_memset(&l3_egress, 0, sizeof(l3_egress));
                if (BCM_GPORT_INVALID != endpoint_config->info.gport) {
                    BCM_IF_ERROR_RETURN
                        (bcmi_xgs5_bfd_endpoint_gport_resolve(unit, 0, endpoint_config->is_tx_raw_ingrs, &endpoint_config->info,
                                           &endpoint_config->modid, &endpoint_config->port, &trunk_id, &local_id, &is_trunk_bfd,
            ...
            if (BCM_GPORT_INVALID == endpoint_config->info.tx_gport) {
                ...
                if (!BCM_BFD_ENCAP_TYPE_TNL_VXLAN(endpoint_config->info.type)){
                    BCM_IF_ERROR_RETURN
                        (bcmi_xgs5_bfd_endpoint_gport_resolve(unit, 1, endpoint_config->is_tx_raw_ingrs, &endpoint_config->info,
                                           &endpoint_config->modid, &endpoint_config->port, &trunk_id, &local_id, &is_trunk_bfd,
                                           &endpoint_config->is_micro_bfd));
                    /* Get local port used for TX BFD packet */
                    BCM_IF_ERROR_RETURN
            ...
            if (BCM_BFD_ENCAP_TYPE_TNL_VXLAN(endpoint_config->info.type)) {
                rv =   bcmi_xgs5_bfd_vxlan_encap_create(unit, endpoint_config, msg_sess.encap_data);
                if (BCM_FAILURE(rv)) {
...
bcmi_xgs5_bfd_reinit(int unit)
{
    ...
    if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit) && (stable_size > 0)) {
        ...
        if (recovered_ver < BCM_BFD_WB_VERSION_1_7) {
            realloc_size += (sizeof(bcm_gport_t) * bfd_info->endpoint_count); /* VxLAN Network Identifier. */
            realloc_size += (sizeof(uint16) * bfd_info->endpoint_count); /* Vlan Tag Protocol Identifier. */
            realloc_size += (sizeof(uint16) * bfd_info->endpoint_count); /* Inner Vlan Tag Protocol Identifier. */
        }
        SOC_SCACHE_HANDLE_SET(scache_handle, unit, BCM_MODULE_BFD, 0);
