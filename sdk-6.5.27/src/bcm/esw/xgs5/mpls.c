...
#ifdef BCM_TRIDENT3_SUPPORT
typedef struct {
    soc_field_t vxlate;  /* field from vlan_xlate_1_double */
    soc_field_t mpls;    /* field from mpls_entry */
} _td3_frr_field_trans_t;

STATIC _td3_frr_field_trans_t _td3_frr_field_trans[] = {
    {MPLS_FLEX__MPLS_LABELf,  MPLS_LABELf},
    {MPLS_FLEX__PORT_NUMf,    PORT_NUMf},
    {MPLS_FLEX__TGIDf,        TGIDf},
    {MPLS_FLEX__MODULE_IDf,   MODULE_IDf},
    {MPLS_FLEX__Tf,           Tf},
    {MPLS_FLEX__L3_IIFf,      L3_IIFf},
    {MPLS_FLEX__CLASS_IDf,    CLASS_IDf},
    {MPLS_FLEX__DESTINATIONf, DESTINATIONf},
    {MPLS_FLEX__V6_ENABLEf,   V6_ENABLEf},
    {MPLS_FLEX__V4_ENABLEf,   V4_ENABLEf},
    {MPLS_FLEX__NEW_PRIf,     NEW_PRIf},
    {MPLS_FLEX__PW_CC_TYPEf,  PW_CC_TYPEf},
    {MPLS_FLEX__SVPf,         SVPf},
    {MPLS_FLEX__FLEX_CTR_BASE_COUNTER_IDXf, FLEX_CTR_BASE_COUNTER_IDXf},
    {MPLS_FLEX__FLEX_CTR_POOL_NUMBERf,      FLEX_CTR_POOL_NUMBERf},
    {MPLS_FLEX__FLEX_CTR_OFFSET_MODEf,      FLEX_CTR_OFFSET_MODEf},
    {MPLS_FLEX__RX_PROT_GROUP_LOWERf,       RX_PROT_GROUP_LOWERf},
    {MPLS_FLEX__RX_PROT_GROUP_UPPERf,       RX_PROT_GROUP_UPPERf},
    {MPLS_FLEX__RX_PROT_GROUPf,             RX_PROT_GROUPf},
    {MPLS_FLEX__DROP_DATA_ENABLEf,          DROP_DATA_ENABLEf},
    {MPLS_FLEX__INHERITANCE_TYPEf,          INHERITANCE_TYPEf},
    {MPLS_FLEX__NEXT_FWD_LABEL_INHERITf,    NEXT_FWD_LABEL_INHERITf},
    {MPLS_FLEX__EXP_MAPPING_PTRf,           EXP_MAPPING_PTRf},
    {MPLS_FLEX__DECAP_USE_EXP_FOR_PRIf,     DECAP_USE_EXP_FOR_PRIf},
    {MPLS_FLEX__DO_NOT_CHANGE_PAYLOAD_DSCPf,DO_NOT_CHANGE_PAYLOAD_DSCPf},
    {MPLS_FLEX__DECAP_USE_EXP_FOR_INNERf,   DECAP_USE_EXP_FOR_INNERf},
    {MPLS_FLEX__DECAP_USE_TTLf,             DECAP_USE_TTLf},
    {MPLS_FLEX__MPLS_ACTION_IF_NOT_BOSf,    MPLS_ACTION_IF_NOT_BOSf},
    {MPLS_FLEX__MPLS_ACTION_IF_BOSf,        MPLS_ACTION_IF_BOSf},
    {MPLS_FLEX__EXP_TO_ECN_MAPPING_PTRf,    EXP_TO_ECN_MAPPING_PTRf},
    {MPLS_FLEX__DECAP_USE_ECN_FOR_INNERf,   DECAP_USE_ECN_FOR_INNERf},
    {MPLS_FLEX__CURR_FWD_LABEL_INHERITf,    CURR_FWD_LABEL_INHERITf},
    {MPLS_FLEX__DECAP_NEXT_FWD_LABELf,      DECAP_NEXT_FWD_LABELf},
    {MPLS_FLEX__PW_TERM_NUMf,               PW_TERM_NUMf},
    {MPLS_FLEX__PW_TERM_NUM_VALIDf,         PW_TERM_NUM_VALIDf}
};
...
#if defined(BCM_TRIDENT3_SUPPORT)
STATIC int
_bcm_td3_frr_vxlate2mpls_entry(int unit,
     vlan_xlate_1_double_entry_t *vent,
     mpls_entry_entry_t *ment)
{
    int val;
    int ix;
    soc_mem_t vmem = VLAN_XLATE_1_DOUBLEm;
    soc_mem_t mmem = MPLS_ENTRYm;
    _td3_frr_field_trans_t *field_trans = _td3_frr_field_trans;

    sal_memset(ment, 0, sizeof(mpls_entry_entry_t));
    for (ix = 0; ix < COUNTOF(_td3_frr_field_trans); ix++) {
        val = soc_mem_field32_get(unit, vmem, vent, (field_trans + ix)->vxlate);
        soc_mem_field32_set(unit, mmem, ment, (field_trans + ix)->mpls, val);
    }
    if (SOC_MEM_FIELD_VALID(unit, vmem, MPLS_FLEX__CW_CHECK_CTRLf)) {
        val = soc_mem_field32_get(unit, vmem, vent,
                  MPLS_FLEX__CW_CHECK_CTRLf);
    } else {
        if (SOC_MEM_FIELD_VALID(unit, vmem, MPLS_FLEX__CW_CHECK_CTRL_BIT_ONEf)) {
            val = soc_mem_field32_get(unit, vmem, vent,
                      MPLS_FLEX__CW_CHECK_CTRL_BIT_ONEf);
            val = val << 1;
        }

        if (SOC_MEM_FIELD_VALID(unit, vmem, MPLS_FLEX__CW_CHECK_CTRL_BIT_ZEROf)) {
            val |= soc_mem_field32_get(unit, vmem, vent,
                       MPLS_FLEX__CW_CHECK_CTRL_BIT_ZEROf);
        }
    }
    soc_mem_field32_set(unit, mmem, ment, CW_CHECK_CTRLf, val);

    soc_mem_field32_set(unit, mmem, ment, BASE_VALID_0f, 3);
    soc_mem_field32_set(unit, mmem, ment, BASE_VALID_1f, 7);
    soc_mem_field32_set(unit, mmem, ment, KEY_TYPEf, 0);
    return BCM_E_NONE;
}

STATIC int
_bcm_td3_frr_mpls2vxlate_entry(int unit,
     vlan_xlate_1_double_entry_t *vent,
     mpls_entry_entry_t *ment)
{
    int val;
    ...
    for (ix = 0; ix < COUNTOF(_td3_frr_field_trans); ix++) {
        val = soc_mem_field32_get(unit, mmem, ment, (field_trans + ix)->mpls);
        soc_mem_field32_set(unit, vmem, vent, (field_trans + ix)->vxlate, val);
    }
    val = soc_mem_field32_get(unit, mmem, ment, CW_CHECK_CTRLf);
    if (SOC_MEM_FIELD_VALID(unit, vmem, MPLS_FLEX__CW_CHECK_CTRLf)) {
        soc_mem_field32_set(unit, vmem, vent, MPLS_FLEX__CW_CHECK_CTRLf, val);
    } else {
        if (SOC_MEM_FIELD_VALID(unit, vmem, MPLS_FLEX__CW_CHECK_CTRL_BIT_ZEROf)) {
            soc_mem_field32_set(unit, vmem, vent, MPLS_FLEX__CW_CHECK_CTRL_BIT_ZEROf,
                    val & 1);
        }
        if (SOC_MEM_FIELD_VALID(unit, vmem, MPLS_FLEX__CW_CHECK_CTRL_BIT_ONEf)) {
            soc_mem_field32_set(unit, vmem, vent, MPLS_FLEX__CW_CHECK_CTRL_BIT_ONEf,
                    (val & 0x2) ? 1: 0);
        }
    }
    soc_mem_field32_set(unit, vmem, vent, BASE_VALID_0f, 3);
    soc_mem_field32_set(unit, vmem, vent, BASE_VALID_1f, 7);
    soc_mem_field32_set(unit, vmem, vent, KEY_TYPEf,
                        TD3_VLXLT_HASH_KEY_TYPE_MPLS_FLEX_SGLP_LABEL);
    soc_mem_field32_set(unit, vmem, vent, DATA_TYPEf, 23);
    return BCM_E_NONE;
}
...
bcmi_td3_php_ecmp_nhop_vcswap_delete(int unit, int nh_index, int dest_type, int mpath_flag)
{
    ...
    if (mpath_flag & BCM_L3_MULTIPATH) {
        ...
        while (i < ecmp_member_count) {
            ...
            rv = bcmi_td3_mpls_php_vcswap_info_delete(unit, index, nh_index, dest_type);
            if (BCM_FAILURE(rv)) {
                goto clean_up;
            }
            i++;
...
bcmi_xgs5_mpls_tunnel_switch_add(int unit, bcm_mpls_tunnel_switch_t *info)
{
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (info->flags2 & BCM_MPLS_SWITCH2_FRR_BOTTOM_LABEL) {
        if (SOC_IS_TRIDENT3X(unit)) {
            if (!soc_feature(unit, soc_feature_mpls_frr_bottom_label)) {
                 LOG_ERROR(BSL_LS_BCM_MPLS, (BSL_META("Unsupported cancun version for FRR_BOTTOM_LABEL\n")));
                return BCM_E_UNAVAIL;
            }
        }
    }
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_mpls_frr_bottom_label) &&
        (info->flags2 & BCM_MPLS_SWITCH2_FRR_BOTTOM_LABEL)) {
        if (info->port == BCM_GPORT_INVALID) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                (BSL_META("Port independent label not supported for FRR_BOTTOM_LABEL\n")));
            return BCM_E_PARAM;
        }
    } else
#endif /* BCM_TRIDENT3_SUPPORT */
    ...
    if (soc_feature(unit, soc_feature_mpls_frr_bottom_label) &&
        ...
        BCM_IF_ERROR_RETURN(_bcm_td3_frr_mpls2vxlate_entry(unit, &vent,
                            &ment));

        /* See if entry already exists */
        rv = soc_mem_search(unit, VLAN_XLATE_1_DOUBLEm, MEM_BLOCK_ANY,
                &idx_mpls_mem, &vent, &vent, 0);
        if (BCM_SUCCESS(rv)) {
            BCM_IF_ERROR_RETURN(_bcm_td3_frr_vxlate2mpls_entry(unit, &vent,
                            &ment));
        }
    } else
#endif /* BCM_TRIDENT3_SUPPORT */
    ...
    if (SOC_IS_TOMAHAWK2(unit) || SOC_IS_TOMAHAWK3(unit)) {
        soc_mem_field32_set(unit, mem, &ment, MPLS__DROP_DATA_ENABLEf,
                                (info->flags & BCM_MPLS_SWITCH_DROP)?1:0);
    }
#endif /* BCM_TOMAHAWK2_SUPPORT || BCM_TOMAHAWK3_SUPPORT */
    ...
    if (!tunnel_switch_update) {
        ...
        if (soc_feature(unit, soc_feature_mpls_frr_bottom_label) &&
            (info->flags2 & BCM_MPLS_SWITCH2_FRR_BOTTOM_LABEL)) {
            BCM_IF_ERROR_RETURN(_bcm_td3_frr_mpls2vxlate_entry(unit,
                                &vent, &ment));
            rv = soc_mem_insert(unit, VLAN_XLATE_1_DOUBLEm, MEM_BLOCK_ALL,
                                &vent);
        } else
#endif /* BCM_TRIDENT3_SUPPORT */
        {
        ...
        if (soc_feature(unit, soc_feature_mpls_frr_bottom_label) &&
            ...
            rv = soc_mem_write(unit, VLAN_XLATE_1_DOUBLEm, MEM_BLOCK_ALL,
                               idx_mpls_mem, &vent);
        } else
#endif /* BCM_TRIDENT3_SUPPORT */
        {
...
{   
    ...
    if (soc_feature(unit, soc_feature_mpls_frr_bottom_label) &&
        ...
        BCM_IF_ERROR_RETURN(_bcm_td3_frr_mpls2vxlate_entry(unit, &vent,
                            ment));
        rv = soc_mem_delete(unit, VLAN_XLATE_1_DOUBLEm, MEM_BLOCK_ALL, &vent);
    } else
#endif /* BCM_TRIDENT3_SUPPORT */
    {
...
#endif /* BCM_TOMAHAWK2_SUPPORT || BCM_APACHE_SUPPORT */
    {
        ...
        if (soc_feature(unit, soc_feature_mpls_frr_bottom_label) &&
            ...
            rv = soc_mem_search(unit, VLAN_XLATE_1_DOUBLEm, MEM_BLOCK_ANY,
                    &index, &vent, &vent, 0);
            if (BCM_SUCCESS(rv)) {
                BCM_IF_ERROR_RETURN(_bcm_td3_frr_vxlate2mpls_entry(unit,
                                    &vent, &ment));
            }
        } else
#endif /* BCM_TRIDENT3_SUPPORT */
        ...
        rv = _bcmi_xgs5_mpls_entry_delete(unit, &ment, frr_bt_lkup);
    }
...
bcmi_xgs5_mpls_tunnel_switch_delete_all(int unit)
{
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        rv = _bcmi_xgs5_mpls_entry_delete(unit, ment, 0);
        if (BCM_FAILURE(rv)) {
            if (mpls_entry_buf) {
                soc_cm_sfree(unit, mpls_entry_buf);
            }
    ...
    if (soc_feature(unit, soc_feature_mpls_frr_bottom_label)) {
        ...
        for (i = index_min; i <= index_max; i++) {
            vent = soc_mem_table_idx_to_pointer
                (unit, mem, vlan_xlate_1_double_entry_t *, mpls_entry_buf, i);
            ...
            if (soc_mem_field32_get(unit, mem, vent, KEY_TYPEf) !=
                TD3_VLXLT_HASH_KEY_TYPE_MPLS_FLEX_SGLP_LABEL) {
                continue;
            }
            if (soc_mem_field32_get(unit, mem, vent, MPLS_FLEX__MPLS_ACTION_IF_BOSf) ==
                    _BCM_MPLS_XGS5_MPLS_ACTION_IF_BOS_L2_SVP) {
                /* L2_SVP */
                continue;
            }

            rv = _bcm_td3_frr_vxlate2mpls_entry(unit, vent, &mpls_ent);
            if (BCM_FAILURE(rv)) {
                if (mpls_entry_buf) {
                    soc_cm_sfree(unit, mpls_entry_buf);
                }
            ...
            rv = _bcmi_xgs5_mpls_entry_delete(unit, &mpls_ent, TRUE);
            if (BCM_FAILURE(rv)) {
                if (mpls_entry_buf) {
                    soc_cm_sfree(unit, mpls_entry_buf);
                }
                                   ...
                                   void *user_data)
{
   ...
   int index_min, index_max;
    ...
    if (soc_feature(unit, soc_feature_mpls_frr_bottom_label)) {
        ...
        mem = VLAN_XLATE_1_DOUBLEm;
        sal_memset(&mpls_ent, 0, sizeof(mpls_entry_entry_t));
        vxlate_entry_buf = soc_cm_salloc(unit,
                SOC_MEM_TABLE_BYTES(unit, mem), "VXLATE_ENTRY buffer");
        if (NULL == vxlate_entry_buf) {
            goto cleanup;
        }
        ...
        index_max = soc_mem_index_max(unit, mem);
        rv = soc_mem_read_range(unit, mem, MEM_BLOCK_ANY,
                index_min, index_max, vxlate_entry_buf);
        if (SOC_FAILURE(rv)) {
            goto cleanup;
        }
        ...
        for (i = index_min; i <= index_max; i++) {
            vent = soc_mem_table_idx_to_pointer
                (unit, mem, vlan_xlate_1_double_entry_t *, vxlate_entry_buf, i);
            ...
            rv = _bcmi_xgs5_mpls_entry_get_key(unit, &mpls_ent, &info);
            if (rv < 0) {
                goto cleanup;
            }
            rv =  bcmi_td3_mpls_entry_get_data(unit, &mpls_ent, &info);
            if (rv < 0) {
                goto cleanup;
            }
            info.flags2 |= BCM_MPLS_SWITCH2_FRR_BOTTOM_LABEL;
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (vxlate_entry_buf) {
        soc_cm_sfree(unit, vxlate_entry_buf);
    }
#endif /* BCM_TRIDENT3_SUPPORT */
    return rv;
}
                                 ...
                                 bcm_mpls_egress_label_t *label_array)
{
    ...
    soc_mem_t mem;
    uint32 tnl_entry[SOC_MAX_MEM_WORDS];
    uint32 iptun_mpls_entry_format[4] = {0};
   ...
   for (i = 0; i < num_labels; i++) {
       ...
       if (((rv == BCM_E_CONFIG) || (label_array[i].label == 0)) &&
           !(label_array[i].flags & BCM_MPLS_EGRESS_LABEL_REPLACE)) {
