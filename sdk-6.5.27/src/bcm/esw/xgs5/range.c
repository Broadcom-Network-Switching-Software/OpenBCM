...
bcmi_xgs5_range_ctrl_free(int unit, bcmi_xgs5_range_ctrl_t *rangec)
{
    ...
    bcm_range_oper_mode_t oper_mode = bcmRangeOperModeGlobal;
    uint8 pipe = 0;
    uint8 found = 0;
    int idx = 0;
    int rv = 0;
    int max_groups = 0;
    bcm_range_group_config_t grp_cfg;
    ...
    if (soc_feature(unit, soc_feature_range_check_group_support)) {
        ...
        if (bcmRangeOperModePipeLocal == oper_mode) {
            max_groups = RANGE_CTRL(unit)->max_groups * NUM_PIPE(unit);
        } else {
            max_groups = RANGE_CTRL(unit)->max_groups;
        }
        for (idx = 0; idx < max_groups; idx++) {
            found = 0;
            if (bcmRangeOperModeGlobal == oper_mode) {
                if (SHR_BITGET((RANGE_CTRL(unit)->range_group_used_bitmap), idx)) {
                    found = 1;
                }
            } else {
                pipe = (idx / RANGE_CTRL(unit)->max_groups);
                if (SHR_BITGET((RANGE_CTRL(unit)->range_group_used_bitmap) + pipe, idx)) {
                    found = 1;
                }
            }
            if (found) {
                grp_cfg.range_group_id = idx;
                BCM_IF_ERROR_RETURN
                    (bcmi_xgs5_range_group_delete(unit, &grp_cfg));
            }
        }
    }
...
STATIC int
bcmi_xgs5_range_wb_reinit_2_0(int unit, uint8 **scache_ptr,
                              uint32 *scache_pos, uint16 recovered_version)
{
    uint8           *scache_p;          /* Pointer to scache */
    soc_mem_t       mem = RANGE_CTRL(unit)->range_mem;  /* Range Checker Memory ID */
    int             idx;                /* Range iteration index */
    ...
    /* Number of Ranges created */
    RANGE_CTRL(unit)->num_ranges = temp_range_count;
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;num_ranges}\n")));
    if (recovered_version >= BCM_WB_VERSION_2_1) {
        temp_range_count = *scache_p;
        scache_p++;
        temp_range_count |= (*scache_p) << 8;
        scache_p++;
        temp_range_count |= (*scache_p) << 16;
        scache_p++;
        temp_range_count |= (*scache_p) << 24;
        scache_p++;

         (*scache_pos) =  (*scache_pos) + 4;

        RANGE_CTRL(unit)->num_ranges = temp_range_count;

        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;4;%d}\n"), ((*scache_pos)-4),
              (int)(RANGE_CTRL(unit)->num_ranges)));
    } else {
        RANGE_CTRL(unit)->num_ranges = *scache_p;
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;1;%d}\n"), (*scache_pos),
    ...
    bcm_range_id_bmp_t *range_bmp)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_RANGE,
                (BSL_META_U(unit,
                            "bcmi_xgs5_range_group_range_bmp_get: Error Fetching Range Oper Mode \n")));
        return (rv);
    }

    /* Get Pipe instance from the range group ID. */
    pipe_instance = (range_group_id / RANGE_CTRL(unit)->max_groups);
    /* Get per pipe range group ID. */
    range_group_id = (range_group_id % RANGE_CTRL(unit)->max_groups);

    if ((pipe_instance < 0) || (pipe_instance >= SOC_MAX_NUM_PIPES)) {
         LOG_DEBUG(BSL_LS_BCM_RANGE,
                 (BSL_META_U(unit,
                             "bcmi_xgs5_range_group_range_bmp_get: Error Validating Range Group ID \n")));
         return (BCM_E_INTERNAL);
    }
    ...
    if (NULL == range_hw_bmap) {
        LOG_DEBUG(BSL_LS_BCM_RANGE,
                (BSL_META_U(unit,
                        "bcmi_xgs5_range_group_range_bmp_get: Error Allocating memory for range_bmap \n")));
        return (BCM_E_MEMORY);
    }

    /* Check range group ID existence per pipe */
...
bcmi_xgs5_range_wb_sync_2_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    ...
    *scache_p = RANGE_CTRL(unit)->range_oper_mode;
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;num_ranges}\n")));
    if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_2_1) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;4;%d}\n"), (*scache_pos),
              (int)(RANGE_CTRL(unit)->num_ranges)));
        ...
        *scache_p = RANGE_CTRL(unit)->num_ranges;
        scache_p++;
        *scache_p = (RANGE_CTRL(unit)->num_ranges) >> 8;
        scache_p++;
        *scache_p = (RANGE_CTRL(unit)->num_ranges) >> 16;
        scache_p++;
        *scache_p = (RANGE_CTRL(unit)->num_ranges) >> 24;
        scache_p++;
    ...
    if (BCM_E_NOT_FOUND == rv) {
        ...
        if (recovered_ver >= BCM_WB_VERSION_2_0) {
            BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_reinit_2_0(unit,
                        &scache_ptr, &scache_pos, recovered_ver));
            BCM_IF_ERROR_RETURN
        ...
        bcm_range_t rid)
{
    ...
    if (soc_feature(unit, soc_feature_range_check_group_support)) {
        for (idx = 0; idx < RANGE_CTRL(unit)->max_groups; idx++) {
            if (SHR_BITGET(((RANGE_CTRL(unit)->range_group_used_bitmap) + range_tmp.pipe_instance), idx)) {
                /* Check whether range ID is part of any range group */
                if (SHR_BITGET((RANGE_CTRL(unit)->range_group_range_bmp[idx]).w, rid)) {
                    return BCM_E_BUSY;
                }
            }
        }
    ...
    bcm_range_group_config_t *range_group_config)
{
    ...
    if (BCM_SUCCESS(rv)) {
        /* Add range group ID per pipe */
        SHR_BITSET(((RANGE_CTRL(unit)->range_group_used_bitmap) + pipe_instance),
                     range_group_id);
        /* Copy range_bmp per range group ID */
        sal_memcpy(&(RANGE_CTRL(unit)->range_group_range_bmp[range_group_config->range_group_id]),
                &(range_group_config->range_bmp), sizeof(bcm_range_id_bmp_t));
        LOG_DEBUG(BSL_LS_BCM_RANGE,
                (BSL_META_U(unit,
                            "bcmi_xgs5_range_group_update: Range Update Successful \n")));
    }
    ...
    bcm_range_group_config_t *range_group_config)
{
    ...
    sal_memcpy(&(range_group_config->range_bmp),
            &(RANGE_CTRL(unit)->range_group_range_bmp[range_group_config->range_group_id]),
            sizeof(bcm_range_id_bmp_t));

    LOG_DEBUG(BSL_LS_BCM_RANGE,
            (BSL_META_U(unit,
                        "bcmi_xgs5_range_group_get: Range Group Get Successful \n")));
