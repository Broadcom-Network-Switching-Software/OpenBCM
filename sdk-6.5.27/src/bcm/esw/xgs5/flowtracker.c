                       ...
                       shr_ft_msg_ctrl_collector_create_t *collector_create_msg)
{
    ...
    sal_memset(collector_create_msg, 0, sizeof(shr_ft_msg_ctrl_collector_create_t));
    /*
     * We have to subtract by start id, since SDK starts the ID with 1
     * and UKERNEL starts the ID with 0.
     */
    collector_create_msg->id = coll_int_id - BCM_INT_FT_COLLECTOR_START_ID;
    ...
    if (export_profile_info.wire_format == bcmCollectorWireFormatProtoBuf) {
        collector_create_msg->flags |= SHR_FT_MSG_COLLECTOR_FLAGS_PROTOBUF;
...
_bcm_xgs5_ft_collector_list_create (int unit)
{
    ...
    if (ft_collector_info_list[unit] == NULL) {
        _BCM_FT_ALLOC(ft_collector_info_list[unit], _bcm_int_ft_collector_info_t,
                      sizeof(_bcm_int_ft_collector_info_t) *
                       ((ft_info->max_collectors) + BCM_INT_FT_COLLECTOR_START_ID),
                "collector list");
                                            ...
                                            bcm_flowtracker_group_t flow_group_id)
{
    ...
    if (ft_info->cfg_mode >= SHR_FT_CFG_MODE_V2) {
        ...
        flow_group_info_list_node->collector_id2 =
                                 BCM_INT_COLLECTOR_INVALID_COLLECTOR_ID;
        flow_group_info_list_node->export_profile_id2 =
                                BCM_INT_COLLECTOR_INVALID_EXPORT_PROFILE_ID;
    }
...
STATIC int _bcm_xgs5_ft_eapp_send_features_exchange_msg(int unit)
{
    ...
    if (ft_info->cfg_mode != SHR_FT_CFG_MODE_V1) {
        send_msg.features |= SHR_FT_SDK_FEATURE_MULTIPLE_COLLECTORS;
    }
...
int _bcm_xgs5_ft_eapp_send_ctrl_init_msg (int unit) 
{
    ...
    ctrl_init_msg.max_collectors = ft_info->max_collectors;
    ...
    if (ft_info->flags & BCM_INT_FT_INFO_FLAGS_DROP_CONGESTION_MONITOR) {
        ...
        if (soc_property_get(unit, spn_FLOWTRACKER_EXPORT_IPFIX_ENABLE, 0)) {
            ctrl_init_msg.flags |= SHR_FT_MSG_INIT_FLAGS_DROP_MONITOR_IPFIX_ENABLE;
        }
...
int _bcm_xgs5_ft_eapp_send_flow_group_update_msg (int unit, 
                    bcm_flowtracker_group_t flow_group_id, uint32 flags,
...
int _bcm_xgs5_ft_eapp_send_collector_update_msg(int unit,
                                                uint8 is_detach,
                                                int collector_id,
                                                int export_profile_id)
{
    shr_ft_msg_ctrl_collector_update_t collector_update_msg;
    _bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
    uint8 *buffer_req = NULL, *buffer_ptr = NULL;
    uint16 buffer_len = 0, reply_len = 0;
    int rv = BCM_E_NONE;
    ...
    rv = _bcm_xgs5_ft_v2_collector_list_collector_get(unit, collector_id,
                                                      &coll_int_id);
    BCM_IF_ERROR_RETURN(rv);
    sal_memset(&collector_update_msg, 0, sizeof(shr_ft_msg_ctrl_collector_update_t));
    /*
     * We have to subtract by start id, since SDK starts the ID with 1
     * and UKERNEL starts the ID with 0.
     */
    collector_update_msg.id = coll_int_id - BCM_INT_FT_COLLECTOR_START_ID;
    collector_update_msg.mtu = export_profile_info.max_packet_length;
    collector_update_msg.num_records = export_profile_info.num_records;
    if (export_profile_info.wire_format == bcmCollectorWireFormatIpfix) {
        if (!is_detach) {
            collector_update_msg.domain_id = collector_info.ipfix.observation_domain_id;
        }
    }

    if (export_profile_info.flags & BCM_COLLECTOR_EXPORT_PROFILE_FLAGS_USE_NUM_RECORDS) {
        collector_update_msg.flags |= SHR_FT_MSG_COLLECTOR_FLAGS_USE_NUM_RECORDS;
    }

    if (export_profile_info.wire_format == bcmCollectorWireFormatProtoBuf) {
        collector_update_msg.flags |= SHR_FT_MSG_COLLECTOR_FLAGS_PROTOBUF;
        if (!is_detach) {
            /* Set Flowtracker export pkt Component ID */
            collector_update_msg.component_id = collector_info.protobuf.component_id;

            /* Set Flowtracker export pkt System ID length */
            collector_update_msg.system_id_length = collector_info.protobuf.system_id_length;

            /* Set Flowtracker export pkt System ID */
            memcpy(&collector_update_msg.system_id, &collector_info.protobuf.system_id,
                    collector_info.protobuf.system_id_length);
        }
    }

    buffer_req      = ft_info->dma_buffer;
    buffer_ptr      = shr_ft_msg_ctrl_collector_update_pack(buffer_req, &collector_update_msg);
    buffer_len      = buffer_ptr - buffer_req;

    BCM_IF_ERROR_RETURN(_bcm_xgs5_ft_msg_send_receive(unit, MOS_MSG_SUBCLASS_FT_COLLECTOR_UPDATE,
                                                     buffer_len, 0,
                                    MOS_MSG_SUBCLASS_FT_COLLECTOR_UPDATE_REPLY, &reply_len,
                                    SHR_FT_MAX_UC_MSG_TIMEOUT));
    return BCM_E_NONE;
}
...
int bcm_xgs5_ft_eapp_init(int unit)
{
    ...
    /* If the firmware is old and does not support multiple collectors, ignore
     * the soc property updated value of max collectors and go with minimum
     * old value of 1.
     * NOTE: The firmware would have ignored the soc property based max_collectors
     * value we sent in the init message anyway if it is not capable of it.
     */
    if (!(ft_info->uc_features & SHR_FT_UC_FEATURE_MULTIPLE_COLLECTORS)) {
...
SHR_FT_TEMPLATE_INFO_ELEM_NAMES;
...
/*
 * Function:
 *     _bcm_xgs5_ft_v2_collector_list_dump
 * Purpose:
 *     Dump the collector list.
 * Parameters:
 *     unit          - (IN) BCM device number
 ...
 */
void
_bcm_xgs5_ft_v2_collector_list_dump (int unit)
{
    _bcm_int_ft_v2_collector_info_t *collector_node = NULL;
    int i;
    ...
    for (i = BCM_INT_COLLECTOR_COLLECTOR_START_ID;
         i < BCM_INT_COLLECTOR_COLLECTOR_START_ID+BCM_INT_COLLECTOR_MAX_COLLECTORS;
         i++) {
        collector_node = &(ft_v2_collector_info_list[unit][i]);
        if (collector_node->int_id != 0) {
            LOG_CLI((BSL_META("Collector ID %d mapped to internal ID %d\r\n"),
                        i, collector_node->int_id));
        }
    }
}
                         ...
                         _bcm_int_ft_flow_group_info_t *flow_group_node)
{
    _bcm_int_ft_info_t *ft_info = FT_INFO_GET(unit);
     ...
     LOG_CLI((BSL_META("\tExport interval(ms) = %u\r\n"),
...
void _bcm_xgs5_flowtracker_sw_dump(int unit)
{
    ...
    _bcm_xgs5_ft_v2_collector_list_dump(unit);
...
int _bcm_th_ft_flow_group_drop_report_org_pkt_len_get(int unit,
                 ...
                 int* arg)
{
    ...
    rv =  _bcm_xgs5_ft_msg_send_receive(unit, MOS_MSG_SUBCLASS_FT_DROP_REPORT_ORG_PKT_MAX_LEN_GET,
            id, 0,
            MOS_MSG_SUBCLASS_FT_DROP_REPORT_ORG_PKT_MAX_LEN_GET_REPLY,
            &reply_len,
            SHR_FT_MAX_UC_MSG_TIMEOUT);
    if (rv == BCM_E_UNAVAIL) {
        return BCM_E_NONE;
