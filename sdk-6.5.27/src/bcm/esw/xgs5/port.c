                            ...
                            bcm_gport_t port, bcm_port_resource_t *resource)
{
    ...
    if (SOC_PBMP_MEMBER(si->pbmp_mmu_lossless, lport)) {
        resource->mmu_lossless = 1;
    }
    ...
    if (BCM_SUCCESS(rv) && !SAL_BOOT_SIMULATION) {
        ...
        resource->fec_type = (bcm_port_phy_fec_t) speed_config.fec;
...
#endif
                    /* If the speed is changing, check if the mmu buffer mode
                     * is manual for supported devices */
                    if (pr->op & BCMI_XGS5_PORT_RESOURCE_OP_SPEED) {
                        if (BCMI_PORT_DRV_CALL(unit)->cosq_buf_mode_get) {
                            BCM_IF_ERROR_RETURN(
                                    BCMI_PORT_DRV_CALL(unit)->cosq_buf_mode_get(
                                        unit, &buf_mode));
                            /* if manual buffer mode has been set, do not update
                             * certain mmu settings on speed change */
                            if (1 == buf_mode) {
                                pr->op |=
                                    BCMI_XGS5_PORT_RESOURCE_OP_BUFFER_MANUAL;
                            }
                        }
                    }
                                 ...
                                 soc_port_schedule_state_t *port_schedule_state)
{
    ...
    port_schedule_state->frequency = (SOC_IS_FIREBOLT6(unit)) ?
        soc_property_get(unit, spn_BCM_TDM_FREQUENCY, si->frequency) : si->frequency;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        for (port_idx = 0; port_idx < SOC_DRIVER(unit)->port_num_blktype;
            ...
            case SOC_BLK_CLPORT:
                ...
                if (((pr->encap == SOC_ENCAP_IEEE) ||
                     (pr->encap == SOC_ENCAP_HIGIG_OVER_ETHERNET)) &&
            ...
            case SOC_BLK_XLPORT:
                ...
                } else if ((pr->encap == SOC_ENCAP_IEEE) ||
                           (pr->encap == SOC_ENCAP_HIGIG_OVER_ETHERNET)) {  /* XE */
                                ...
                                soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (SOC_IS_TOMAHAWK3(unit)) {
            if ((pr->op & BCMI_XGS5_PORT_RESOURCE_OP_SPEED) &&
                !(pr->op & BCMI_XGS5_PORT_RESOURCE_OP_BUFFER_MANUAL) &&
                (pr->physical_port != -1))  {
                BCM_IF_ERROR_RETURN
                    (soc_th3_flex_reconf_thresholds(unit, pr->logical_port,
                                                    pr->pipe));
            }
        }
#endif
    ...
    */
    ...
    for (i = 0; i < nport; i++) {
        if (soc_feature(unit, soc_feature_hgoe_line_card_mode) &&
                (resource[i].encap == BCM_PORT_ENCAP_HIGIG_OVER_ETHERNET)) {
            bcm_port_encap_config_t ecfg;
            ecfg.encap = resource[i].encap;
            BCM_IF_ERROR_RETURN(
                    bcm_esw_port_encap_config_set(unit, resource[i].port, &ecfg));
        }
    }
#endif
...
/*
 * Function:
 *      bcmi_xgs5_port_cosq_buf_mode_get
 * Purpose:
 *      This function returns whether the cosq buffer mode is auto
 *      or manual, which affects whether a speed change operation
 *      will maintain manually configured MMU settings or whether
 *      those settings will be automatically determined by the SDK
 *
 * Parameters:
 *      unit     - (IN) Unit number.
 *      port     - (OUT) The MMU buffer operating mode 0 == Auto
 *                       1 == Manual
 * Returns:
 *      BCM_E_XXX
 * Notes:
 *      API to set this mode is bcm_switch_control_set() using
 *      the bcm_switch_control_t bcmSwitchBufUseMode.
 *      Supported by 56870, 56970, 56980
 */
int
bcmi_xgs5_port_cosq_buf_mode_get(int unit,
                                    int *buf_mode)
{
    if (NULL == buf_mode) {
        return BCM_E_PARAM;
    ...
    *buf_mode = SOC_INFO(unit).cosq_buf_mode;
...
int
bcmi_xgs5_port_enable_multi_set(int unit, bcm_port_t *port, int enable, int nport)
{
    soc_port_schedule_state_t *port_schedule_state;
    int i, rv;
    ...
    rv = bcmi_xgs5_port_sched_structure(unit, nport, port, 1, port_schedule_state);
    if (BCM_FAILURE(rv)) {
        sal_free(port_schedule_state);
        return rv;
    }
    ...
    if (enable && BCMI_PORT_DRV_CALL(unit)->port_enable) {
        ...
        /* Physical_port = -1 indicates bringing down a port */
        for (i = 0; i < nport; i += 1) {
            port_schedule_state->resource[i].physical_port = -1;
        }
        /* Port Down */
        rv = BCMI_PORT_DRV_CALL(unit)->port_disable(unit, port_schedule_state);
    } else {
