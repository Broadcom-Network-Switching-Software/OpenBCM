...
static bcmi_xgs5_port_func_t bcmi_th3_port_calls = {
    ...
    /* port_cosmap_update       */ NULL,
    /* cosq_buf_mode_get        */ bcmi_xgs5_port_cosq_buf_mode_get,
};
...
/*
 * Function:
 *      bcm_tomahawk3_port_buffer_pbmp_get
 * Purpose:
 *      Returns the mapped ports for the requeseted ITM
 * Parameters:
 *      unit        - (IN) Unit number
 *      itm         - (IN) The ITM requested
 *      pbmp        - (OUT) The ports on the ITM requested
 * Returns:
 *      BCM_E_XXX
 */
int
bcm_tomahawk3_port_buffer_pbmp_get(int unit, int itm, bcm_pbmp_t *pbmp)
{
    soc_info_t *si = &SOC_INFO(unit);
    uint32 pipes_map;
    int pipe;

    /* Check itm is in range of index of itm_ipipe_map */
    if ((itm < 0) || itm >= COUNTOF(si->itm_ipipe_map)) {
        return BCM_E_PARAM;
    }
    /* Check NULL pbmp */
    if (NULL == pbmp) {
    ...
    /* Check itm is mapped. 56983 only has 1 itm */
    if (0 == ((1 << itm) & si->itm_map)) {
        return BCM_E_PARAM;
    }

    /* Set the bits for the pipes belonging to the given ITM */
    pipes_map = si->itm_ipipe_map[itm];

    SOC_PBMP_CLEAR(*pbmp);

    /* Add the mapped ports that are in the pipes for the requested ITM.
     * pipe_pbm[pipe] includes CPU, Management, and LB ports. */
    for (pipe = 0; pipe < si->num_pipe; pipe++) {
        if ((1 << pipe) & pipes_map) {
            SOC_PBMP_OR(*pbmp, si->pipe_pbm[pipe]);
        }
    }
