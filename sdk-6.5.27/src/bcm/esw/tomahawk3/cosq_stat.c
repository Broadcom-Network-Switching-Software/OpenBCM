...
STATIC int
_bcm_th3_egr_pool_current_usage_bytes_get(int unit, bcm_gport_t port,
        bcm_cos_queue_t cosq, bcm_cosq_stat_t stat, uint64 *value) {
    bcm_port_t local_port = -1;
    soc_info_t *si = &SOC_INFO(unit);
    int itm, pool;
    uint32 fld_value = 0, itm_map, rval;
    uint64 val64;

    if (!SOC_REG_IS_VALID(unit, MMU_THDO_SHARED_DB_POOL_SHARED_COUNTr)) {
        return BCM_E_UNAVAIL;
    }
    BCM_IF_ERROR_RETURN
        (_bcm_th3_cosq_localport_resolve(unit, port, &local_port));
    ...
    for (itm = 0; itm < NUM_ITM(unit); itm++)  {
        if (itm_map & (1 << itm)) {
            BCM_IF_ERROR_RETURN
                (soc_reg32_get(unit, MMU_THDO_SHARED_DB_POOL_SHARED_COUNTr, itm, pool, &rval));
            fld_value = soc_reg_field_get(unit, MMU_THDO_SHARED_DB_POOL_SHARED_COUNTr, rval, COUNTf);
            COMPILER_64_ADD_32(val64, fld_value);
        }
    }
                     ...
                     bcm_cosq_stat_t stat, int sync_mode, uint64 *value)
{
    ...
    switch (stat) {
        ...
        case bcmCosqStatEgressPoolSharedBytesCurrent:
            BCM_IF_ERROR_RETURN
                (_bcm_th3_egr_pool_current_usage_bytes_get(unit, port,
                    cosq, stat, &cell_value));
            /*value = *cell_value * _TH3_MMU_BYTES_PER_CELL;*/
            COMPILER_64_ZERO(*value);
            COMPILER_64_ADD_64(*value, cell_value);
            COMPILER_64_UMUL_32(*value, _TH3_MMU_BYTES_PER_CELL);
