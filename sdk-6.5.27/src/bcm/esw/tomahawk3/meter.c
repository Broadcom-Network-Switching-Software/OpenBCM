...
uint32 _bcm_th3_default_burst_size(int unit, int port, uint32 kbits_sec)
{
    ...
    int rv = 0;
    uint32 pps, pps_burst, pps_mode = 0;
    int final = 0;
    ...
    max_port_speed *= 1000; /* Convert to kbps*/

    rv = bcm_esw_port_rate_egress_get(unit, port, &port_kbits_sec,
                             &port_kbits_burst);
    if (rv != BCM_E_NONE) {
        if (rv == BCM_E_CONFIG) {
            rv = bcm_esw_port_rate_egress_pps_get(unit, port, &pps,
                                            &pps_burst);
            if (rv != BCM_E_NONE) {
                return 0;
            }
            ...
            /* convert the pps to kbps as the calculations
               are in units of kbps, 64B pkt + 20B IFG */
            port_kbits_sec = (pps * 84 * 8 ) / 1000;

            /* convert the input pps to kbps */
            kbits_sec = (kbits_sec * 84 * 8 ) / 1000;
    ...
    if (port_kbits_sec != 0) {
         ...
         */
        if (port_kbits_sec < max_port_speed) {
            max_port_speed = port_kbits_sec;
        }
    ...
    final =  (proposed > 0) ? proposed : 1;

    /* final at above step is in kilo bits */
    if (pps_mode) {

        /* convert back to pps units, 64B pkt + 20B IFG */
        /* convert to pkts */
        proposed = proposed * 1000;
        proposed = proposed / 8;
        proposed = proposed / (64+20);  /* pkts in units of bytes */
        final =  (proposed > 0) ? proposed : 1;
    }
