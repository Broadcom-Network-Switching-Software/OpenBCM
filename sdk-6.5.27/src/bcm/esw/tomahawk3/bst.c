...
#define _TH3_BST_HW_EVENT_TRIGGER 0x1
...
STATIC int _bcm_bst_th3_sync_report(int unit, bcm_bst_stat_id_t bid,
                                   int port, int index, int flags);
...
_bcm_th3_bst_control_get(int unit, bcm_switch_control_t type, int *arg)
{
    ...
    switch (type) {
        case bcmSwitchBstEnable:
            BCM_IF_ERROR_RETURN(READ_MMU_GLBCFG_BST_TRACKING_ENABLEr(unit,
                                                                     &rval));
            if (rval == 0xff) {
                /* Return 1 only if tracking is enabled for all modules in both
                 * ITMs. */
                *arg = 1;
            } else {
                *arg = 0;
...
STATIC int
_bcm_th3_hw_intr_cb_unregister(int unit)
{
    BCM_IF_ERROR_RETURN(soc_th3_set_bst_callback(unit, NULL));
    return BCM_E_NONE;
}
...
_bcm_bst_th3_intr_to_resources(int unit, uint32 *flags)
{
    ...
    */
    ...
    for (i = 0; i < res_ct; i++) {
        BCM_IF_ERROR_RETURN(
            _bcm_bst_th3_sync_report(unit, res[i], -1, index[i], _TH3_BST_HW_EVENT_TRIGGER));
    }
                                                    ...
                                                    int cosq)
{
    ...
    int num_field, field_idx, field_idx_start, field_idx_end;
    void *pentry;
    char *dmabuf;
    ...
    if (bst_info->pre_sync) {
        BCM_IF_ERROR_RETURN(bst_info->pre_sync(unit, bid, &track_en));
    }
    if (!mmu_inst_map) {
        rv = BCM_E_PARAM;
        goto restore;
                ...
                for (idx = min_hw_idx; idx <= max_hw_idx; idx++) {
                    if (resInfo->stat_field != INVALIDf) {
                        ...
                        pentry = soc_mem_table_idx_to_pointer(unit, mem, void*,
                                                          dmabuf, hw_idx);
                        if (cosq != -1) {
                            field_idx_start = cosq;
                            field_idx_end = cosq;
                        } else {
                            field_idx_start = 0;
                            field_idx_end = num_field;
                        }
                        for (field_idx = field_idx_start;
                             field_idx < field_idx_end; field_idx++)
                            {
                                 soc_mem_field32_set(unit, mem, pentry,
...
restore:
    if (bst_info->post_sync) {
        BCM_IF_ERROR_RETURN(bst_info->post_sync(unit, bid, track_en));
    }
...
STATIC int
_bcm_th3_pre_sync(int unit, bcm_bst_stat_id_t bid, int *sync_val)
{
    soc_field_t field[2] = {INVALIDf, INVALIDf};
    _bcm_bst_resource_info_t *resInfo;
    uint32 rval;
    int field_val[2];
    ...
    if (resInfo->flags & _BCM_BST_CMN_RES_F_RES_DEV) {
        field[0] = BST_TRACK_EN_CFAP0f;
        field[1] = BST_TRACK_EN_CFAP1f;
    } else if (resInfo->flags & _BCM_BST_CMN_RES_F_RES_ING) {
        field[0] = BST_TRACK_EN_THDI0f;
        field[1] = BST_TRACK_EN_THDI1f;
    } else if (resInfo->flags & _BCM_BST_CMN_RES_F_RES_EGR) {
        field[0] = BST_TRACK_EN_THDO0f;
        field[1] = BST_TRACK_EN_THDO1f;
    } else {
        return BCM_E_PARAM;
    }

    BCM_IF_ERROR_RETURN(READ_MMU_GLBCFG_BST_TRACKING_ENABLEr(unit, &rval));
    field_val[0] = soc_reg_field_get(unit, MMU_GLBCFG_BST_TRACKING_ENABLEr, rval,
                                  field[0]);
    field_val[1] = soc_reg_field_get(unit, MMU_GLBCFG_BST_TRACKING_ENABLEr, rval,
                                  field[1]);
    soc_reg_field_set(unit, MMU_GLBCFG_BST_TRACKING_ENABLEr, &rval, field[0], 0);
    soc_reg_field_set(unit, MMU_GLBCFG_BST_TRACKING_ENABLEr, &rval, field[1], 0);
    BCM_IF_ERROR_RETURN(WRITE_MMU_GLBCFG_BST_TRACKING_ENABLEr(unit, rval));
    *sync_val = (field_val[1] << 1) | field_val[0];

    return SOC_E_NONE;
}

STATIC int
_bcm_th3_post_sync(int unit, bcm_bst_stat_id_t bid, int sync_val)
{
    soc_field_t field[2] = {INVALIDf, INVALIDf};
    _bcm_bst_resource_info_t *resInfo;
    uint32 rval;
    ...
    BCM_IF_ERROR_RETURN(READ_MMU_GLBCFG_BST_TRACKING_ENABLEr(unit, &rval));
    soc_reg_field_set(unit, MMU_GLBCFG_BST_TRACKING_ENABLEr, &rval, field[0],
                      (sync_val & 1));
    soc_reg_field_set(unit, MMU_GLBCFG_BST_TRACKING_ENABLEr, &rval, field[1],
                      (sync_val >> 1));
    BCM_IF_ERROR_RETURN(WRITE_MMU_GLBCFG_BST_TRACKING_ENABLEr(unit, rval));
...
STATIC int
_bcm_bst_th3_hw_stat_get(int unit, int port, int index, bcm_cosq_buffer_id_t buffer,
                         bcm_bst_stat_id_t bid, int cosq, int *pvalue)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
    int rv = BCM_E_NONE;
    ...
    if (buffer == -1) {
        from_inst = 0;
        to_inst = resInfo->num_instance - 1;
    } else {
        from_inst = to_inst = buffer;
    }

    for (inst = from_inst; inst <= to_inst; inst++) {
        if (!(mmu_inst_map & (1 << inst))) {
            continue;
        }
        ...
        if ((base_mem = resInfo->stat_mem[0]) != INVALIDm) {
            ...
            if(resInfo->stat_field != INVALIDf) {
                ...
                temp_stat = soc_mem_field32_get(unit, mem, entry, resInfo->stat_field);
                if (max_stat < temp_stat) {
                    max_stat = temp_stat;
                }
            } else {
                /*multiple field case */
                ...
                for (idx = 0; idx < num; idx++) {
                    field_idx = field_ids[idx];
                    temp_stat = soc_mem_field32_get(unit, mem, entry,
                                                   resInfo->stat_fields[field_idx]);
                    if (max_stat < temp_stat) {
                        max_stat = temp_stat;
                    }
                }
            ...
            temp_stat = soc_reg_field_get(unit, reg, rval, resInfo->stat_field);
            if (max_stat < temp_stat) {
                max_stat = temp_stat;
            }
        } /* if valid reg */
...
STATIC int
_bcm_bst_th3_sync_report(int unit, bcm_bst_stat_id_t bid,
                         int port, int index, int flags)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
        ...
        for (idx = 0; idx < resInfo->num_stat_pp; idx++) {
            ...
            if (bst_info->track_mode) {
                ...
                if ((resInfo->p_threshold[thd_idx] &&
                    (resInfo->p_stat[idx_offset] >=
                    (resInfo->p_threshold[thd_idx] * resInfo->threshold_gran))) ||
                     ((flags & _TH3_BST_HW_EVENT_TRIGGER) && resInfo->p_stat[idx_offset])) {
                    trigger_event = 1;
                }
            }
                    ...
                    for(j = 0; j < resInfo->num_field; j++) {
                        ...
                        if (bst_info->track_mode) {
                            ...
                            if ((resInfo->p_threshold[thd_idx] &&
                                (resInfo->p_stat[idx_offset] >=
                                (resInfo->p_threshold[thd_idx]) * resInfo->threshold_gran)) ||
                                (flags & _TH3_BST_HW_EVENT_TRIGGER)) {
                                trigger_event = 1;
                            }
                        }
...
_bcm_bst_th3_sync_hw_snapshot(int unit, bcm_bst_stat_id_t bid,
                             int port, int index)
{
    return _bcm_bst_th3_sync_report(unit, bid, port, index, 0);
}
...

{
    ...
    if (buffer >= _TH3_ITMS_PER_DEV || buffer < -1) {
        return BCM_E_PARAM;
    }
    ...
    do {
        ...
        if (options & BCM_COSQ_BST_STAT_CACHE_BYPASS) {
            for (ii = start_hw_index; ii <= end_hw_index; ii++) {
                int temp_hw_stat = 0;
                BCM_IF_ERROR_RETURN(
                    _bcm_bst_th3_hw_stat_get(unit, gport, ii, buffer,
                                             bid, cosq, &temp_hw_stat));
                COMPILER_64_ADD_32(*pvalue, temp_hw_stat);
            }
        } else {
            entries_per_inst = (resInfo->index_max + 1) / resInfo->num_instance;
            if (bst_info->port_to_mmu_inst_map) {
            ...
            if (buffer != -1 ) {
                if(!(resInfo->flags & _BCM_BST_CMN_RES_F_PIPED)) {
                    /* ITM and device stats */
                    if (!(mmu_inst_map & (1 << buffer))) {
                        return BCM_E_PARAM;
                    }
                } else {
                    /* Pipe based stats */
                    if ((si->itm_ipipe_map[buffer] & mmu_inst_map) == 0) {
                        return BCM_E_PARAM;
                    }
                }
            }
                ...
                if (buffer == -1 && cosq == -1) {
                    if (num_sw_entries > 1) {
                        for (inst = 0; inst < resInfo->num_instance; inst++) {
                            ...
                            if (mmu_inst_map & (1 << inst)) {
                                for (loop_idx = 0; loop_idx < num_sw_entries;
                                        loop_idx++) {
                                    if (sw_idx_arr[loop_idx] != -1) {
                        ...
                        for (inst = 0; inst < resInfo->num_instance; inst++) {
                            ...
                            if (mmu_inst_map & (1 << inst)) {
                                if (sw_idx_arr[0] != -1) {
                                    sw_index = sw_idx_arr[0] + (inst * entries_per_inst);
                                    temp_stat = resInfo->p_stat[sw_index];
                                    if (temp_stat > max_stat) {
                                        max_stat = temp_stat;
                                    }
                   ...
                   for (inst = 0; inst < resInfo->num_instance; inst++) {
                       ...
                       if (mmu_inst_map & (1 << inst)) {
                           if (sw_idx_arr[loop_idx] != -1) {
                               sw_index = sw_idx_arr[loop_idx] + (inst * entries_per_inst);
                               temp_stat = resInfo->p_stat[sw_index];
                               if (temp_stat > max_stat) {
                                   max_stat = temp_stat;
                               }
                  ...
                  if (sw_idx_arr[loop_idx] != -1) {
                      sw_index = sw_idx_arr[loop_idx] + (buffer * entries_per_inst);
                      temp_stat = resInfo->p_stat[sw_index];
                      if (temp_stat > max_stat) {
                          max_stat = temp_stat;
                      }
                   ...
                   COMPILER_64_ADD_32(*pvalue, max_stat);
                      ...
                      sw_index = sw_idx_arr[0] + (buffer * entries_per_inst);
                      max_stat = resInfo->p_stat[sw_index];
                      COMPILER_64_ADD_32(*pvalue, max_stat);
                }
            }
        }
    ...
    if (options & BCM_COSQ_BST_STAT_CLEAR_ON_READ) {
...
bcm_bst_th3_init(int unit)
{
    ...
    bst_info->hw_stat_get = NULL;
    bst_info->intr_enable_set = _bcm_bst_th3_intr_enable_set;
    bst_info->pre_sync = _bcm_th3_pre_sync;
    bst_info->post_sync = _bcm_th3_post_sync;
    bst_info->hw_intr_cb = _bcm_th3_set_hw_intr_cb;
    bst_info->hw_intr_cb_unregister = _bcm_th3_hw_intr_cb_unregister;
    ...
    if (mode == 1) {
        /* bst */
        for (bid = bcmBstStatIdDevice; bid < bcmBstStatIdRQEQueue; bid++) {
            if (bid == bcmBstStatIdUcast || bid == bcmBstStatIdMcast) {
                continue;
            }
            BCM_IF_ERROR_RETURN
