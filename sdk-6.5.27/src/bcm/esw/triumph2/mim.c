...
#if defined(BCM_TRIDENT3_SUPPORT)
#define _BCM_TD3_XLATE_SINGLE_ISID_KEY_TYPE(_u_) \
     ((SOC_IS_HELIX5X(_u_)) ? (25) : (24))

#define _BCM_TD3_XLATE_DOUBLE_ISID_KEY_TYPE(_u_) \
     ((SOC_IS_HELIX5X(_u_)) ? (24) : (23))
#endif
...
STATIC int
_bcm_tr2_mim_port_associated_data_recover(int unit, int vp, int stable_size, int post_scache)
{
    ...
    if (entry_type == 3) {
        ...
        /* Don't update flags if it's post scache recovery */
        if (!post_scache &&
            soc_mem_field_valid(unit, EGR_L3_NEXT_HOPm, MIM__ISID_LOOKUP_TYPEf)) {
...
_bcm_tr2_mim_reinit(int unit)
{
    ...
    void * vlan_xlate_2;
    int profile_idx;
    soc_mem_t xlate_2_mem = VLAN_XLATE_2_SINGLEm;
    uint8 *vlan_xlate_2_buf = NULL;
#endif
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        rv = _bcm_tr2_mim_port_associated_data_recover(unit, vp, stable_size, 0);
        if (BCM_FAILURE(rv)) {
            goto cleanup;
        }
    }
    ...
    if (soc_feature(unit, soc_feature_mim_decoupled_mode)) {
        ...
        /* traverse vlan_xlate_2_single to recover vlan_tag_action_profile */
        mem_buf[0] = &vlan_xlate_2_buf;
        rv = _bcm_chunk_memory_alloc(unit, xlate_2_mem, mem_buf[0]);
        if (BCM_FAILURE(rv) || (NULL == vlan_xlate_2_buf)) {
           goto cleanup;
        }

        index_min = soc_mem_index_min(unit, xlate_2_mem);
        index_max = soc_mem_index_max(unit, xlate_2_mem);
        for (i = index_min; i <= index_max; i++) {
            mem_entry[0] = &vlan_xlate_2;
            rv = _bcm_chunk_get_mem_entry(unit, xlate_2_mem, i ,
                                  vlan_xlate_2_buf, mem_entry[0]);
            if (BCM_FAILURE(rv) || (NULL == vlan_xlate_2)) {
                goto cleanup;
            }

            if (soc_mem_field32_get(unit, xlate_2_mem,
                     vlan_xlate_2, BASE_VALIDf) != 1) {
                continue;
            }
            if (soc_mem_field32_get(unit, xlate_2_mem, vlan_xlate_2,
                                            KEY_TYPEf) != 0x18) {
                continue; /* Not MIM_ISID */
            }
            profile_idx = soc_mem_field32_get(unit, xlate_2_mem, vlan_xlate_2,
    ...
    /* Recover associate data for MATCH_CRITERIA_NONE types of VPs */
    num_vp = soc_mem_index_count(unit, SOURCE_VPm);
    for (vp = 0; vp < num_vp; vp++) {
        if (MIM_INFO(unit)->port_info[vp].flags &
            _BCM_MIM_PORT_TYPE_MATCH_CRITERIA_NONE) {
            /* This type of MiM VP comes without SVP assignment,
             * so it needs to be recovered from scache info, not
             * from VLAN_XLATE/MPLS_ENTRY/SOURCE_TRUNK_MAP_TABLE etc.
             */
            rv = _bcm_tr2_mim_port_associated_data_recover(unit, vp, stable_size, 1);
            if (BCM_FAILURE(rv)) {
                goto cleanup;
            }
        }
...
cleanup:
    ...
    if (soc_feature(unit, soc_feature_mim_decoupled_mode)) {
        if (vlan_xlate_2_buf) {
            soc_cm_sfree(unit, vlan_xlate_2_buf);
        }
    }
#endif
    return rv;
                                     ...
                                     uint32 *evxlt_entry)
{
    ...
    if (mim_port != NULL) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_td3_sd_tag_action_profile_get(unit,
            sd_tag_action, sd_default_action, &profile_index, 0, 0));
        soc_mem_field32_set(unit, evxlt_mem, evxlt_entry,
             ...
             int vfi, bcm_vlan_t vlan)
{
    ...
    if (info != NULL) {
        ...
        soc_mem_field32_set(unit, vxlt_mem, vxlt_entry, KEY_TYPEf,
                            _BCM_TD3_XLATE_SINGLE_ISID_KEY_TYPE(unit));
        ...
        soc_mem_field32_set(unit, vxlt_mem, vxlt_entry, KEY_TYPEf,
                            _BCM_TD3_XLATE_DOUBLE_ISID_KEY_TYPE(unit));
        soc_mem_field32_set(unit, vxlt_mem, vxlt_entry, DATA_TYPEf, 20);
             ...
             int is_local, int *old_tpid_idx)
{
    ...
    if (mim_port->flags & BCM_MIM_PORT_EGRESS_SERVICE_VLAN_TAGGED) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_td3_sd_tag_action_profile_get(unit,
                    ...
                    egr_nh_info->sd_tag_action_not_present, &profile_index, 0, 0));
        soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                                   ...
                                   int vp, bcm_mim_vpn_t vpn)
{
    ...
    if ((mim_port->criteria == BCM_MIM_PORT_MATCH_PORT_VLAN) ||
        ...
        MIM_INFO(unit)->port_info[vp].match_tunnel_vlan =
                         mim_port->match_tunnel_vlan;
    } else if (mim_port->criteria == 0) {
        if (mim_port->flags & BCM_MIM_PORT_TYPE_BACKBONE) {
            MIM_INFO(unit)->port_info[vp].flags |= _BCM_MIM_PORT_TYPE_MATCH_CRITERIA_NONE;
        }
    }
    return rv;
}
...
{
    int rv = BCM_E_NONE;
    int nh_index = 0, vfi = -1, ipmc_id = 0, is_local = FALSE;
    source_vp_entry_t svp;
    ing_dvp_table_entry_t dvp;
    ing_l3_next_hop_entry_t ing_nh;
    vfi_entry_t vfi_entry;
    ...
    sal_memset(&mim_port, 0, sizeof(bcm_mim_port_t));
    ...
    for (vp = 0; vp < num_vp; vp++) { 
        ...
        if (soc_SOURCE_VPm_field32_get(unit, psvp, ENTRY_TYPEf) != 0) {
            ...
            if((vfi == soc_SOURCE_VPm_field32_get(unit, psvp, VFIf)) &&
               (!network_port)) {
                rv = _bcm_tr2_mim_port_delete(unit, vpn, vp);
            } else if (vpn == _BCM_MIM_VPN_INVALID &&
                       (MIM_INFO(unit)->port_info[vp].flags & _BCM_MIM_PORT_TYPE_MATCH_CRITERIA_NONE)) {
                rv = _bcm_tr2_mim_port_delete(unit, vpn, vp);
            }
            if (rv < 0) { 
    ...
    if (entry_type == 0x2) {
        ...
        if (soc_feature(unit, soc_feature_mim_bvid_insertion_control)) {
            ...
            if (soc_reg_field_get(unit, EGR_CONFIGr, regval,
                ...
                if (soc_EGR_L3_NEXT_HOPm_field32_get(unit, &egr_nh,
                                                     MIM__BVID_VALIDf) == 0) {
                    mim_port->flags |= BCM_MIM_PORT_PEER_NO_BTAG;
                }
            }
        }
#endif
...
#define MIM_PORT_TYPE_CNT   10
    char *mim_port_type_str[MIM_PORT_TYPE_CNT] = {
        "NETWORK",
        "ACCESS_PORT",
        "ACCESS_PORT_VLAN",
        "ACCESS_PORT_VLAN_STACKED",
        "ACCESS_LABEL",
        "PEER",
        "ACCESS_PORT_TRUNK",
        "ACCESS_SHARED",
    ...
    LOG_CLI((BSL_META_U(unit, "-----------------\n")));
    LOG_CLI((BSL_META_U(unit, "| MiM  VPN Info |\n")));
    LOG_CLI((BSL_META_U(unit, "---------------------------\n")));
    LOG_CLI((BSL_META_U(unit, "|    VFI     |    ISID    |\n")));
    LOG_CLI((BSL_META_U(unit, "---------------------------\n")));
    ...
    LOG_CLI((BSL_META_U(unit, "-----------------\n")));
    LOG_CLI((BSL_META_U(unit, "| MiM Port Info |\n")));
    LOG_CLI((BSL_META_U(unit, "-------------------------------------------------------------------------------------------\n")));
    LOG_CLI((BSL_META_U(unit, "| VP  |  Flags   | Index| T | M | P  |                 Match                        | Vfi |\n")));
    LOG_CLI((BSL_META_U(unit, "|     |          |      | G | o | or |Vlan|Innr| Label |            tunnel    |Count|Count|\n")));
    LOG_CLI((BSL_META_U(unit, "|     |          |      | ID| d | t  |    |Vlan|       |   Source MAC    |VLAN|     |     |\n")));
    LOG_CLI((BSL_META_U(unit, "-------------------------------------------------------------------------------------------\n")));
    for (i = 0; i < num_vp; i++) {
        ...
        LOG_CLI((BSL_META_U(unit, "|%5d"), i));
        LOG_CLI((BSL_META_U(unit, "|0x%08x"), MIM_INFO(unit)->port_info[i].flags));
        LOG_CLI((BSL_META_U(unit, "|0x%04x"), MIM_INFO(unit)->port_info[i].index));
        LOG_CLI((BSL_META_U(unit, "|%3d"), MIM_INFO(unit)->port_info[i].tgid));
        LOG_CLI((BSL_META_U(unit, "|%3d"), MIM_INFO(unit)->port_info[i].modid));
        LOG_CLI((BSL_META_U(unit, "|%4d"), MIM_INFO(unit)->port_info[i].port));
        LOG_CLI((BSL_META_U(unit, "|%4d"), MIM_INFO(unit)->port_info[i].match_vlan));
        LOG_CLI((BSL_META_U(unit, "|%4d"), MIM_INFO(unit)->port_info[i].match_inner_vlan));
        LOG_CLI((BSL_META_U(unit, "|0x%05x"), MIM_INFO(unit)->port_info[i].match_label));

        sal_memset(mac_str, 0, sizeof(mac_str));
        shr_format_macaddr(mac_str, MIM_INFO(unit)->port_info[i].match_tunnel_srcmac);
        LOG_CLI((BSL_META_U(unit, "|%-17s"), mac_str));
        LOG_CLI((BSL_META_U(unit, "|%4d"), MIM_INFO(unit)->port_info[i].match_tunnel_vlan));
        LOG_CLI((BSL_META_U(unit, "|%5d"), MIM_INFO(unit)->port_info[i].match_count));
        LOG_CLI((BSL_META_U(unit, "|%5d|\n"), MIM_INFO(unit)->port_info[i].vfi_count));
    }
    LOG_CLI((BSL_META_U(unit, "-------------------------------------------------------------------------------------------\n")));
    LOG_CLI((BSL_META_U(unit, "**************************************\n")));
    LOG_CLI((BSL_META_U(unit, "*        Flags bit definition        *\n")));
    LOG_CLI((BSL_META_U(unit, "**************************************\n")));
    for (i = 0; i < MIM_PORT_TYPE_CNT; i++) {
        LOG_CLI((BSL_META_U(unit, "* Bit %2d: %s\n"), i, mim_port_type_str[i]));
    }
    LOG_CLI((BSL_META_U(unit, "**************************************\n")));
    return;
}
