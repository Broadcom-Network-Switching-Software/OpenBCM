...
/*
 * Function:
 *      bcm_hr4_multicast_remap_xflow_macsec_set
 * Purpose:
 *      Enable/Disable xflow_macsec multicast binding.
 * Parameters:
 *      unit      - (IN) Unit number.
 ...
 */
int
bcm_hr4_multicast_remap_xflow_macsec_set(int unit, bcm_multicast_t mc_from, int enable)
{
    ...
    /* Sanity check the converted IPMC index to see if it really
       corresponds to a valid multicast group.
     */
    ...
    BCM_IF_ERROR_RETURN(READ_L3_IPMC_REMAPm(unit,
                MEM_BLOCK_ALL, ipmc_from, &l3_ipmc_remap));

    soc_L3_IPMC_REMAPm_field32_set(unit, &l3_ipmc_remap, REMAP_ENABLEf, enable);
...
/*
 * Function:
 *      bcm_hr4_multicast_remap_xflow_macsec_get
 * Purpose:
 *      Get xflow_macsec multicast binding status.
 * Parameters:
 *      unit      - (IN) Unit number.
 *      mc_from   - (IN) Multicast group from.
 *      enable    - (OUT) Pointer to xflow_macsec multicast binding status.
 * Returns:
 *      BCM_E_XXX
 * Notes:
 *      Mulitcast types are already validated as appropriate
 ...
 */
int
bcm_hr4_multicast_remap_xflow_macsec_get(int unit, bcm_multicast_t mc_from, int *enable)
{
    int rv = BCM_E_UNAVAIL;
    int ipmc_from;
    bcm_multicast_t mcgroup;
    ...
    if (SOC_SUCCESS(rv)) {
        *enable =
            soc_L3_IPMC_REMAPm_field32_get(unit, &l3_ipmc_remap, REMAP_ENABLEf);
    }
