...
STATIC _bcm_tr2_l3_ecmp_defragment_buffer_t
       *_bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[BCM_MAX_NUM_UNITS];

int bcm_tr2_overlay_defrag_buffer_set[BCM_MAX_NUM_UNITS];
                                ...
                                soc_tunnel_term_t *entry)
{
    ...
    if (tnl_type.tnl_outer_hdr_ipv6 == 1) {
        ...
        BCM_IF_ERROR_RETURN(rv);
        /* Apply mask on destination address. */
        rv = bcm_xgs3_l3_mask6_apply(tnl_info->dip6_mask, tnl_info->dip6);
        BCM_IF_ERROR_RETURN(rv);
        ...
        tnl_info->dip &= tnl_info->dip_mask;
                                  ...
                                  bcm_tunnel_terminator_t *tnl_info)
{
    ...
    if (tnl_type.tnl_outer_hdr_ipv6 == 1) {
        ...
        tnl_info->dip_mask = soc_mem_field32_get(unit, mem, entry_ptr,
                                                        DIP_MASKf);
...
_bcm_tr2_l3_intf_urpf_mode_set(int unit, bcm_vlan_t vid, bcm_vlan_urpf_mode_t urpf_mode)  
{
    ...
    iif.urpf_mode = (bcm_l3_ingress_urpf_mode_t) urpf_mode;
...
_bcm_tr2_l3_intf_urpf_mode_get(int unit, bcm_vlan_t vid, bcm_vlan_urpf_mode_t *urpf_mode)
{
    ...
    *urpf_mode = (bcm_vlan_urpf_mode_t) iif.urpf_mode;
    return (BCM_E_NONE);
}
...
bcm_tr2_l3_ecmp_defragment_buffer_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) {
        bcm_tr2_overlay_defrag_buffer_set[unit] = FALSE;
        if (NULL == _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]) {
            _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit] =
                sal_alloc(sizeof(_bcm_tr2_l3_ecmp_defragment_buffer_t),
                        "l3 ecmp overlay defragmentation buffer info");
            if (NULL == _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]) {
                return BCM_E_MEMORY;
            }
        }
        sal_memset(_bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit], 0,
                sizeof(_bcm_tr2_l3_ecmp_defragment_buffer_t));
    }
...
bcm_tr2_l3_ecmp_defragment_buffer_deinit(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) {
        if (NULL != _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]) {
            sal_free(_bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]);
            _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit] = NULL;
        }
    }
}
...
bcm_tr2_l3_ecmp_defragment_buffer_set(int unit, int size)
{
    ...
    /* In TH3 hecmp, allocate underlay defrag buffer from
       second half of ECMP member table */
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
    ...
    if (size < 0 || size > (idx_max - idx_min + 1)) {
        L3_UNLOCK(unit);
        return BCM_E_PARAM;
    }
     ...
     */
    for (i = idx_max + 1 - size; i < (idx_max + 1); i++) {
...
/*
 * Function:
 *      bcm_tr2_l3_ecmp_overlay_defragment_buffer_set
 * Purpose:
 *      Set overlay L3 ECMP table defragmentation buffer.
 * Parameters:
 *      unit - (IN) SOC unit number.
 *      size - (IN) Defragmentation buffer size.
 * Returns:
 ...
 */
int
bcm_tr2_l3_ecmp_overlay_defragment_buffer_set(int unit, int size)
{
    _bcm_l3_tbl_t *tbl_ptr;
    int old_base_ptr, old_size;
    int base_ptr;
...
#ifdef BCM_TOMAHAWK3_SUPPORT
    /* In hecmp, allocate overlay defrag buffer from
       first half of ECMP member table */
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                               ecmp_mode_hierarchical)) {
        idx_max = idx_max / 2;
    ...
    /* Free old defragmentation buffer */
    old_base_ptr = _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->base_ptr;
    old_size = _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->size;
    if (old_size > 0) {
        BCM_XGS3_L3_ENT_REF_CNT_DEC(tbl_ptr, old_base_ptr, old_size);
    }
    ...
    BCM_XGS3_L3_ENT_REF_CNT_INC(tbl_ptr, base_ptr, size);
    _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->base_ptr = base_ptr;
    _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->size = size;
...
/*
 * Function:
 *      bcm_tr2_l3_ecmp_overlay_defragment_buffer_get
 * Purpose:
 *      Get overlay L3 ECMP table defragmentation buffer size.
 * Parameters:
 *      unit - (IN)  SOC unit number.
 *      size - (OUT) Defragmentation buffer size.
 * Returns:
 ...
 */
int
bcm_tr2_l3_ecmp_overlay_defragment_buffer_get(int unit, int *size)
{
    if (!soc_mem_field_valid(unit, L3_ECMP_COUNTm, BASE_PTRf) &&
        !soc_mem_field_valid(unit, L3_ECMP_COUNTm, BASE_PTR_0f)) {
        return BCM_E_UNAVAIL;
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                                ecmp_mode_hierarchical)) {
        (*size) = _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->size;
...
_bcm_tr2_l3_ecmp_group_base_ptr_update(int unit, int group, int base_ptr)
{
    ...
    soc_mem_t initial_ecmp_group_mem, mem;
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
             ecmp_mode_hierarchical)) {
        if (group < (BCMI_L3_ECMP_OVERLAY_ENTRIES(unit))) {
            mem = ECMP_GROUP_HIERARCHICALm;
        } else if (group >= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit)) {
            phy_grp_idx = group - BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
        }
    }
#endif

    if (phy_grp_idx < 0 || phy_grp_idx > soc_mem_index_max(unit, mem)) {
        return BCM_E_PARAM;
    }

    /* Read L3 ECMP group table */
    SOC_IF_ERROR_RETURN(soc_mem_read(unit, mem, MEM_BLOCK_ANY, phy_grp_idx,
                                     &ecmp_group_entry));
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_1k_groups)) {
        ...
        if (ecmp_hash_16bits && 
            soc_mem_field_valid(unit, mem, BASE_PTRf)) {
            base_ptr_f = BASE_PTRf;
        } else if (soc_mem_field_valid(unit, mem, BASE_PTR_0f)) {
    ...
    if (base_ptr_f == BASE_PTRf) {
        soc_mem_field32_set(unit, mem, &ecmp_group_entry,
                BASE_PTRf, base_ptr);
    } else {
        soc_mem_field32_set(unit, mem, &ecmp_group_entry,
    ...
    SOC_IF_ERROR_RETURN(soc_mem_write(unit, mem, MEM_BLOCK_ALL, phy_grp_idx,
                &ecmp_group_entry));
    ...
    SOC_IF_ERROR_RETURN(soc_mem_read(unit, initial_ecmp_group_mem,
                MEM_BLOCK_ANY, phy_grp_idx, &initial_ecmp_group_entry));
    ...
    SOC_IF_ERROR_RETURN(soc_mem_write(unit, initial_ecmp_group_mem,
                MEM_BLOCK_ALL, phy_grp_idx, &initial_ecmp_group_entry));
 ...
 *                   Notes:
 *                     - HECMP: ecmp_level 1 is for overlay and
 *                     ecmp_level 2 is for underlay.
 *                     - MULTI_LEVEL_ECMP: ecmp_level 1 is for underlay and
 *                     ecmp_level 2 is for overlay.
 * Returns:
 *      BCM_E_XXX.
 */
int
bcm_tr2_l3_ecmp_defragment_no_lock(int unit, int ecmp_level)
{
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
             ecmp_mode_hierarchical)) {
        if (ecmp_level == 1) {
            ecmp_group_tbl_ptr = BCM_XGS3_L3_TBL_PTR(unit, hecmp_grp);
            mem = ECMP_GROUP_HIERARCHICALm;
        }
    }
#endif
    ...
    if (ecmp_level == 2) {
        ...
        if (hecmp_enable) {
            ...
            ecmp_max_idx = soc_mem_index_max(unit, L3_ECMPm);
    ...
    for (i = min_idx; i <= max_idx; i += ecmp_group_idx_increment) {
        ...
        rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, i, &ecmp_group_entry);
        if (SOC_FAILURE(rv)) {
            sal_free(block_array);
            return rv;
        }
        base_ptr =
            soc_mem_field32_get(unit, mem, &ecmp_group_entry, base_ptr_f);
        grp_id = i;
    ...
    if ((soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                            ecmp_mode_hierarchical) &&
        (ecmp_level == 2) &&
        (i < (soc_mem_index_count(unit, L3_ECMP_COUNTm) / 2))) {
        grp_id += BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
    }
#endif
        rv = bcm_xgs3_l3_egress_ecmp_max_paths_get(unit,
                grp_id + BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit), NULL, &max_paths);
        if (BCM_FAILURE(rv)) {
            sal_free(block_array);
            return rv;
        }
        ...
        if (hecmp_enable) {
            ...
            if ((base_ptr + max_paths) > (ecmp_max_idx + 1)) {
                continue;
            }
        }
#endif
        ...
        block_array[block_count].group = grp_id;
        block_count++;
    }
...
#ifdef BCM_TOMAHAWK3_SUPPORT
    /* For HECMP overlay defrag, it uses separate buffer in OL range */
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                                ecmp_mode_hierarchical) &&
        ecmp_level == 1) {
        reserved_block.base_ptr =
            _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->base_ptr;
        reserved_block.size =
            _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->size;
        reserved_block.group = -1;
    }
#endif
    ...
    rv = soc_defragment(unit, block_count, block_array, &reserved_block,
            ecmp_tbl_size, &member_op, &group_op, free_base_ptr, &moved_cnt);
    if (SOC_FAILURE(rv)) {
        sal_free(block_array);
        return rv;
    }

    /* Only count defragment if physically block moved (gap removed) */
    if (moved_cnt) {
        if (ecmp_level == 1) {
            if (hecmp_enable) {
                BCM_XGS3_L3_OVERLAY_ECMP_DEFRAG_COUNT(unit)++;
            } else {
                BCM_XGS3_L3_UNDERLAY_ECMP_DEFRAG_COUNT(unit)++;
            }
        } else if (ecmp_level == 2) {
            if (hecmp_enable) {
                BCM_XGS3_L3_UNDERLAY_ECMP_DEFRAG_COUNT(unit)++;
            ...
            BCM_XGS3_L3_ECMP_DEFRAG_COUNT(unit)++;
        }
    }
    sal_free(block_array);
    return BCM_E_NONE;
...
bcm_tr2_l3_ecmp_defragment_buffer_wb_alloc_size_get(int unit, int *size) 
{
    ...
    *size += sizeof(int);
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) {
        /* Allocate for overlay buffer base_ptr */
        *size += sizeof(int);

        /* Allocate for overlay buffer size */
        *size += sizeof(int);

        /* Allocate for overlay buffer set flag */
        *size += sizeof(int);
    }
...
bcm_tr2_l3_ecmp_defragment_buffer_sync(int unit, uint8 **scache_ptr, int offset)
{
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) {
        /* Store buffer base_ptr */
        value = _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->base_ptr;
        sal_memcpy((*scache_ptr), &value, sizeof(int));
        SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_overlay_defragment_buffer_base_ptr");
        SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
        SOC_SCACHE_DUMP_DATA_TAIL();
        SOC_SCACHE_DUMP_DATA_END();
        ...
        /* Store buffer size */
        value = _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->size;
        sal_memcpy((*scache_ptr), &value, sizeof(int));
        SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_overlay_defragment_buffer_size");
        SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
        SOC_SCACHE_DUMP_DATA_TAIL();
        SOC_SCACHE_DUMP_DATA_END();
        ...
        /* Store overlay defrag buffer set flag */
        value = bcm_tr2_overlay_defrag_buffer_set[unit];
        sal_memcpy((*scache_ptr), &value, sizeof(int));
        SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_overlay_defragment_buffer_set");
        SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
        SOC_SCACHE_DUMP_DATA_TAIL();
        SOC_SCACHE_DUMP_DATA_END();
...
bcm_tr2_l3_ecmp_defragment_buffer_recover(int unit, uint8 **scache_ptr,
                   int offset, int ol_defrag_buffer)
{
    int rv = BCM_E_NONE;
    int value;
    int base_ptr, size;
    ...
    /* recover L3_ECMP member table ref_count (members used)
       for single level ECMP or HECMP underlay defrag buffer */
    ecmp_tbl_ptr = BCM_XGS3_L3_TBL_PTR(unit, ecmp);
    base_ptr = _bcm_tr2_l3_ecmp_defragment_buffer_info[unit]->base_ptr;
    size = _bcm_tr2_l3_ecmp_defragment_buffer_info[unit]->size;
    if (size > 0) {
        if ((base_ptr >= ecmp_tbl_ptr->idx_min) &&
            ((base_ptr + size - 1) <= ecmp_tbl_ptr->idx_max)) {
            BCM_XGS3_L3_ENT_REF_CNT_INC(ecmp_tbl_ptr, base_ptr, size);
        } else {
            rv = BCM_E_PARAM;
        }
    }
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        ol_defrag_buffer) {
        /* Recover overlay defragmentation buffer base_ptr */
        sal_memcpy(&value, (*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_overlay_defragment_buffer_base_ptr");
        SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
        SOC_SCACHE_DUMP_DATA_TAIL();
        SOC_SCACHE_DUMP_DATA_END();
        _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->base_ptr = value;
        (*scache_ptr) += sizeof(int);

        /* Recover overlay defragmentation buffer size */
        sal_memcpy(&value, (*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_overlay_defragment_buffer_size");
        SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
        SOC_SCACHE_DUMP_DATA_TAIL();
        SOC_SCACHE_DUMP_DATA_END();
        _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->size = value;
        (*scache_ptr) += sizeof(int);

        /* Recover overlay defragmentation buffer set flag */
        sal_memcpy(&value, (*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_overlay_defragment_buffer_set");
        SOC_SCACHE_DUMP_DATA_HEAD((*scache_ptr), sizeof(int));
        SOC_SCACHE_DUMP_DATA_Vd((*scache_ptr), int);
        SOC_SCACHE_DUMP_DATA_TAIL();
        SOC_SCACHE_DUMP_DATA_END();
        bcm_tr2_overlay_defrag_buffer_set[unit] = value;
        (*scache_ptr) += sizeof(int);

        /* recover L3_ECMP member table ref_count (members used)
           for HECMP overlay defrag buffer */
        base_ptr =
            _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->base_ptr;
        size = _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->size;
        if (size > 0) {
...
bcm_tr2_l3_ecmp_defragment_buffer_sw_dump(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        ...
        LOG_CLI((BSL_META_U(unit,
            "  ECMP Underlay Defragment Buffer: base_ptr = %d, size = %d\n"),
            _bcm_tr2_l3_ecmp_defragment_buffer_info[unit]->base_ptr,
            _bcm_tr2_l3_ecmp_defragment_buffer_info[unit]->size));
        LOG_CLI((BSL_META_U(unit,
            "  ECMP Overlay Defragment Buffer: base_ptr = %d, size = %d\n"),
            _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->base_ptr,
            _bcm_tr2_l3_ecmp_overlay_defragment_buffer_info[unit]->size));
    } else
#endif
