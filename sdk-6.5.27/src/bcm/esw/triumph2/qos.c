...
bcm_tr2_qos_map_add(int unit, uint32 flags, bcm_qos_map_t *map, int map_id, int internal)
{
    ...
    case _BCM_QOS_MAP_TYPE_MIML_ING_PRI_CNG_MAP:
        ...
        if (!_BCM_QOS_ING_PRI_CNG_USED_GET(unit, id)) {
            ...
            /* Trident3 supports per priority per CFI map for untagged case. */
            if (SOC_IS_TRIDENT3X(unit)) {
                if (flags & BCM_QOS_MAP_L2_UNTAGGED) {
                    untagged = TRUE;
                }
    ...
    case _BCM_QOS_MAP_TYPE_DSCP_TABLE:
        if (!_BCM_QOS_DSCP_TABLE_USED_GET(unit, id)) {
            ...
            if (soc_property_get(unit, spn_QOS_MAP_DSCP_NONSHARABLE_MODE, 0)) {
                rv = _bcm_dscp_table_entry_ref_count_get(unit, index, &ref_cnt);
                if (BCM_FAILURE(rv)) {
                    soc_cm_sfree(unit, dscp_table);
                    break;
                }
                if (ref_cnt == 1) {
                    entries[0] = dscp_table;
                    rv = _bcm_dscp_table_entry_set(unit, entries, index);
                    if (BCM_SUCCESS(rv)) {
                        int mode;
                        rv = _bcm_dscp_table_entry_write_mode_get(unit,
                                                                  index,
                                                                  &mode);
                        ...
                        if (BCM_SUCCESS(rv) &&
                            ...
                            (void)_bcm_dscp_table_entry_delete(unit, index);
                        }
                    }
                    soc_cm_sfree(unit, dscp_table);
                    break;
                    ...
                    LOG_ERROR(BSL_LS_BCM_QOS,
                              (BSL_META_U(unit,
                                          "the DSCP map_id %d was not created!\n"),
                               map_id));
                    rv = BCM_E_RESOURCE;
                    break;
                }
                /* Else: ref_cnt > 1 may happen from nonshare mode set during warmboot.
                 * Go through legacy method.
                 */
            }
