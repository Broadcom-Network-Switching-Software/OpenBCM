                                   ...
                                   int *valid)
{
    ...
    *valid = 0;
    ...
    case bcmPortControlPfcRxStatEnable:
    ...
    case bcmPortControlTxCrcMode:
    case bcmPortControlCrcErrorFramesPurge:
        *valid = 1;
        break;
    default:
        *valid = 0;
                              ...
                              bcm_port_control_t type, int value)
{
    ...
    case bcmPortControlPfcRxStatEnable:
        if (soc_feature(unit, soc_feature_priority_flow_control)) {
            ...
            if (PORTMOD_SUCCESS(rv)) {
                /* Stats enable */
                pfc_control.stats_en = value ? 1 : 0;
                rv = portmod_port_pfc_control_set(unit, pport, &pfc_control);
            }
            PORT_UNLOCK(unit);
        }
 ...
 */
    ...
    case bcmPortControlTxCrcMode:
        {
            portmod_mac_tx_crc_mode_t crc_mode;

            if (value == bcmPortControlTxCrcModeAppend) {
                crc_mode = portmodMacTxCrcModeAppend;
            } else if (value == bcmPortControlTxCrcModeKeep) {
                crc_mode = portmodMacTxCrcModeKeep;
            } else if (value == bcmPortControlTxCrcModeReplace) {
                crc_mode = portmodMacTxCrcModeReplace;
            } else if (value == bcmPortControlTxCrcModeAuto) {
                crc_mode = portmodMacTxCrcModeAuto;
            } else {
                return BCM_E_PARAM;
            }
            PORT_LOCK(unit);
            rv = portmod_port_mac_tx_crc_mode_set(unit, pport, crc_mode);
            PORT_UNLOCK(unit);
        }
        break;
    case bcmPortControlStripCRC:
        PORT_LOCK(unit);
        rv = portmod_port_mac_strip_crc_set(unit, pport, value);
        PORT_UNLOCK(unit);
        break;
    case bcmPortControlCrcErrorFramesPurge:
        {
            uint32 rsv_mask;
            PORT_LOCK(unit);
            rv = portmod_port_mac_rsv_mask_get(unit, port, &rsv_mask);
            if (PORTMOD_SUCCESS(rv)) {
                if (value) {
                    rsv_mask |= (1 << portmodMacRsvCrcErrorPkts);
                } else {
                    rsv_mask &= ~(1 << portmodMacRsvCrcErrorPkts);
                }
                rv = portmod_port_mac_rsv_mask_set(unit, pport, rsv_mask);
            }
            PORT_UNLOCK(unit);
        }
        break;
    ...
    case bcmPortControlStripCRC:
        {
            uint32 crc_strip = 0;
            PORT_LOCK(unit);
            rv = portmod_port_mac_strip_crc_get(unit, pport, &crc_strip);
            *value = crc_strip;
            PORT_UNLOCK(unit);
        }
        break;
    case bcmPortControlTxCrcMode:
        {
            portmod_mac_tx_crc_mode_t crc_mode;
            PORT_LOCK(unit);
            rv = portmod_port_mac_tx_crc_mode_get(unit, pport, &crc_mode);
            if (PORTMOD_SUCCESS(rv)) {
                if (crc_mode == portmodMacTxCrcModeAppend) {
                    *value = bcmPortControlTxCrcModeAppend;
                } else if (crc_mode == portmodMacTxCrcModeKeep) {
                    *value = bcmPortControlTxCrcModeKeep;
                } else if (crc_mode == portmodMacTxCrcModeReplace) {
                    *value = bcmPortControlTxCrcModeReplace;
                } else if (crc_mode == portmodMacTxCrcModeAuto) {
                    *value = bcmPortControlTxCrcModeAuto;
                } else {
                    rv = BCM_E_FAIL;
                }
            }
   ...
   case bcmPortControlCrcErrorFramesPurge:
        {
            ...
            rv = portmod_port_mac_rsv_mask_get(unit, port, &rsv_mask);
            if (PORTMOD_SUCCESS(rv)) {
                if (rsv_mask & (1 << portmodMacRsvCrcErrorPkts)) {
                    *value = 1;
                } else {
                    *value = 0;
                }
...
bcmi_esw_portctrl_flr_set(int unit, bcm_port_t port, int enable, bcm_port_flr_timeout_t timeout)
{
    ...
    int phyn = 0, sys_side = 0;
    bcm_port_t local_port = -1;
    soc_linkscan_flr_timeout_t soc_flr_timeout = socLinkscanFlrTimeoutInvalid;
    ...
    switch (timeout) {
        case bcmPortFlrTimeoutInvaild:
            ...
            soc_flr_timeout = socLinkscanFlrTimeoutInvalid;

            break;
        case bcmPortFlrTimeout50ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout50ms);
            soc_flr_timeout = socLinkscanFlrTimeout50ms;
            break;
        case bcmPortFlrTimeout75ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout75ms);
            soc_flr_timeout = socLinkscanFlrTimeout75ms;
            break;
        case bcmPortFlrTimeout100ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout100ms);
            soc_flr_timeout = socLinkscanFlrTimeout100ms;
            break;
        case bcmPortFlrTimeout150ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout150ms);
            soc_flr_timeout = socLinkscanFlrTimeout150ms;
            break;
        case bcmPortFlrTimeout250ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout250ms);
            soc_flr_timeout = socLinkscanFlrTimeout250ms;
            break;
        case bcmPortFlrTimeout500ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout500ms);
            soc_flr_timeout = socLinkscanFlrTimeout500ms;
            break;
        default:
            rv = BCM_E_PARAM;
            break;
    ...
    if (BCM_SUCCESS(rv)) {
        rv = soc_esw_portctrl_flr_config_set(unit, port, enable, soc_flr_timeout);
    }
    PORT_UNLOCK(unit);
...
/*
 * Function:
 *      bcmi_esw_portctrl_flr_get
 * Purpose:
 *      get flr configuration for a given port
 * Parameters:
 *      unit - Unit #
 *      port - GPORT #
 *      enable - flr enable
 ...
 */
int
bcmi_esw_portctrl_flr_get(int unit, bcm_port_t port, int *enable, bcm_port_flr_timeout_t *timeout)
{
#ifdef PORTMOD_SUPPORT
    int rv;
    portctrl_pport_t pport;
    ...
    int phyn = 0, sys_side = 0;
    bcm_port_t local_port = -1;
    portmod_flr_timeout_t flr_timeout;
    ...
    PORT_LOCK(unit);
    rv = portmod_port_flr_config_get(unit, pport, enable, &flr_timeout);
    if (*enable) {
        switch (flr_timeout) {
            case portmodFlrTimeout50ms:
                *timeout = bcmPortFlrTimeout50ms;
                break;
            case portmodFlrTimeout75ms:
                *timeout = bcmPortFlrTimeout75ms;
                break;
            case portmodFlrTimeout100ms:
                *timeout = bcmPortFlrTimeout100ms;
                break;
            case portmodFlrTimeout150ms:
                *timeout = bcmPortFlrTimeout150ms;
                break;
            case portmodFlrTimeout250ms:
                *timeout = bcmPortFlrTimeout250ms;
                break;
            case portmodFlrTimeout500ms:
                *timeout = bcmPortFlrTimeout500ms;
                break;
            default:
    ...
    LOG_INFO(BSL_LS_BCM_PORT,
             (BSL_META_UP(unit, port,
                          "flr configuration get: u=%d p=%d enable=%d timeout=%d rv=%d\n"),
              unit, port, *enable, *timeout, rv));
...
/*
 * Function:
 *     bcmi_esw_portctrl_phy_signal_detect_status_get
 * Purpose:
 *     Get PMD signal detect status
 * Parameters:
 *      unit - (IN) Unit number.
 *      port - (IN) Port.
 *      sd_status - (OUT) The lane bitmap indicates PMD signal detect and signal change status
 * Returns:
 *      BCM_E_xxx
 * Notes:
 */
int
bcmi_esw_portctrl_phy_signal_detect_status_get(int unit, bcm_port_t port,
                                               bcm_port_phy_signal_detect_status_t *sd_status)
{
#ifdef PORTMOD_SUPPORT
    int rv;
    portctrl_pport_t pport;
    portmod_signal_detect_status_t sd_state;
    ...
    sal_memset(&sd_state, 0, sizeof(portmod_signal_detect_status_t));

    PORT_LOCK(unit);
    rv = portmod_port_signal_detect_status_get(unit, pport, &sd_state);
