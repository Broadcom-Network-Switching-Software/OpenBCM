...
static int tsn_bk_info_initialized[BCM_MAX_NUM_UNITS] = { 0 };
    ...
    int mtu_profile_id)
{
    ...
    if (tsn_bk_info[unit] == NULL && tsn_bk_info_initialized[unit] == 1) {
        return BCM_E_NONE;
    }
...
bcmi_esw_tsn_init(int unit)
{
    ...
    if (!tsn_bk_info_initialized[unit]) {
        for (i = 0; i < BCM_MAX_NUM_UNITS; i++) {
            tsn_bk_info[i] = NULL;
        }
        tsn_bk_info_initialized[unit] = 1;
    }
...
/* Check if the tsn module is initialized. */
int
bcmi_esw_tsn_inited(int unit)
{
    return tsn_bk_info_initialized[unit] ? TRUE : FALSE;
}
