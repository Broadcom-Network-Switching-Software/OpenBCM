        ...
        if (SOC_IS_HURRICANE4(unit)) {
            ...
            if (BCM_E_NONE == bcm_common_xflow_macsec_port_info_get
                ...
                if (port_info.enable) {
                    ...
                    if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, encap_id) ||
                        BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, encap_id)) {
                        /* Extract next hop index from egress object. */
                        if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, encap_id)) {
                            nh_index = encap_id - BCM_XGS3_EGRESS_IDX_MIN(unit);
                        } else {
                    ...
                    if (nh_index != -1) {
                        ...
                        data_type = soc_EGR_L3_NEXT_HOPm_field32_get(unit, &egr_nh,
                                                                     DATA_TYPEf);
                        if (data_type == 2) {
                            hg_chg_dst = soc_EGR_L3_NEXT_HOPm_field32_get(unit, &egr_nh,
                                                                          SD_TAG__HG_CHANGE_DESTINATIONf);
                        } else {
                            hg_chg_dst = soc_EGR_L3_NEXT_HOPm_field32_get(unit, &egr_nh,
                                                                          L3MC__HG_CHANGE_DESTINATIONf);
                        }
                    }
                    if (hg_chg_dst != 0) {
                        /* Macsec IPMC requires macsec port in
...
_bcm_esw_multicast_l3_create(int unit, uint32 flags, bcm_multicast_t *group)
{
    ...
    } else if (flags & BCM_MULTICAST_DISABLE_SRC_KNOCKOUT) {
        /* Disable forwarding domain match check. */
        if (soc_mem_field_valid(unit, EGR_IPMCm, DONT_PRUNE_VLANf)) {
            soc_EGR_IPMCm_field32_set(unit, &egr_ipmc, DONT_PRUNE_VLANf, 1);
        }
    }
