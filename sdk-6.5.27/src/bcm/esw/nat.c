    ...
    bcm_l3_large_nat_egress_t *nat_info)
{
    ...
    if (soc_feature(unit, soc_feature_large_scale_nat)) {
        ...
        /* must specify at least one of SNAT or DNAT or DNAPT_SNAT */
        if (!(nat_info->flags & (BCM_L3_LARGE_NAT_EGRESS_SNAT |
                                 BCM_L3_LARGE_NAT_EGRESS_DNAT |
                                 BCM_L3_LARGE_NAT_EGRESS_DNAPT_SNAT))) {
            return BCM_E_PARAM;
        }
...
#ifdef BCM_TRIDENT3_SUPPORT
        if ((nat_info->flags & BCM_L3_LARGE_NAT_EGRESS_DNAPT_SNAT) &&
            !soc_feature(unit, soc_feature_l3mc_dnapt_snat)) {
            return BCM_E_UNAVAIL;
        }
#endif /* BCM_TRIDENT3_SUPPORT */
