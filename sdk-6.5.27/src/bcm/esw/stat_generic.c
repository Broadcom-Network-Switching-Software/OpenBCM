...
{
    COMPILER_64_DEFINE_INIT(count);
    COMPILER_64_DEFINE_INIT(count1);
    int reg_valid;
    ...
    case snmpIfOutDiscards:              /* Aged packet counter */
        ...
        if (sync_mode == 1){
            BCM_IF_ERROR_RETURN(
                bcm_esw_cosq_stat_sync_get(unit, port, BCM_COS_INVALID,
    ...
    case snmpIpForwDatagrams:
        ...
        if (SOC_REG_IS_VALID(unit, TDBGC2_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, TDBGC2_64r, &count1);
        } else {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, TDBGC2r, &count1);
        }
        COMPILER_64_ADD_64(count, count1);
        COMPILER_64_ZERO(count1);

        if (SOC_REG_IS_VALID(unit, TDBGC0_64r)) {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, TDBGC0_64r, &count1);
        } else {
            BCM_STAT_REG_OPER(unit, port, sync_mode, reg_op, TDBGC0r, &count1);
        }
        COMPILER_64_SUB_64(count, count1);
        break;
    ...
    case snmpBcmIPMCBridgedPckts:
        if (_bcm_esw_stat_dbg_cntr_is_rsv(unit, snmpBcmCustomReceive1)) {
    ...
    case snmpBcmIPMCInDroppedPckts:
        if (_bcm_esw_stat_dbg_cntr_is_rsv(unit, snmpBcmCustomReceive2)) {
