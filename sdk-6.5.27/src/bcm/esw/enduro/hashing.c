                   ...
                   uint32 hash_index, bcm_gport_t *dst_gport)
{
    ...
    member_array = sal_alloc(BCM_TRUNK_MAX_PORTCNT *
    ...
    sal_memset(member_array, 0x0,
               (sizeof(bcm_trunk_member_t)) * BCM_TRUNK_MAX_PORTCNT);

    rv = bcm_esw_trunk_get(unit, tgid, &t_add_info, BCM_TRUNK_MAX_PORTCNT,
    ...
    rv = READ_TRUNK_BITMAPm(unit, MEM_BLOCK_ANY, tgid, &trunk_bitmap_entry);
    if (BCM_FAILURE(rv)) {
        sal_free(member_array);
        BCM_IF_ERROR_RETURN(rv);
    }
    ...
    rv = _bcm_esw_gport_resolve(unit, member_array[member_index].gport,
                                &modid, &port, &tid, &id);
    if (BCM_FAILURE(rv)) {
        sal_free(member_array);
        BCM_IF_ERROR_RETURN(rv);
