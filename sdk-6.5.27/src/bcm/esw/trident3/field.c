...
_bcm_field_td3_flex_qual_ceh_db_init(int unit)
{
    ...
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifyIfaProbeEntropyBits0_7,   1, 0, 0, 0, 0, 1, "UDF_1_CHUNK_5",   "\0",        "IFA_PROBE_ENTROPY_LSB_BYTE", "\0");
#if defined(BCM_HURRICANE4_SUPPORT) && defined(INCLUDE_XFLOW_MACSEC)
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifySvtag,   1, 0, 0, 0, 0, 2, "UDF_1_CHUNK_5",   "UDF_1_CHUNK_6",        "SVTAG_LOWER_TWO_BYTES", "SVTAG_UPPER_TWO_BYTES");
#endif
    ...
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifyIfaCurrentLengthValid, 0, 1, 0, 0, 0, 1, "PARSER_1_FLEX_HVE_CHECK_RESULT_15_0", "\0", "IFA_CURRENT_LENGTH_LT_MAX_LENGTH", "\0");
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifyIfaTunnelCurrentLengthValid, 0, 1, 0, 0, 0, 1, "PARSER_2_FLEX_HVE_CHECK_RESULT_15_0", "\0", "IFA_CURRENT_LENGTH_LT_MAX_LENGTH", "\0");
    ...
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifySRv6EsiLabel0to15,        0, 1, 0, 0, 0, 1, "UDF_2_CHUNK_0_BITS_15_0",             "\0", "SRV6_ESI_LOWER_TWO_BYTES", "\0");
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifySRv6EsiLabel16to31,       1, 0, 0, 0, 0, 1, "UDF_2_CHUNK_0",                       "\0", "SRV6_ESI_UPPER_TWO_BYTES", "\0");
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifySRv6ArgsLabel0to15,       0, 1, 0, 0, 0, 1, "UDF_2_CHUNK_0_BITS_15_0",             "\0", "SRV6_ARGS_LOWER_TWO_BYTES", "\0");
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifySRv6ArgsLabel16to31,      1, 0, 0, 0, 0, 1, "UDF_2_CHUNK_0",                       "\0", "SRV6_ARGS_UPPER_TWO_BYTES", "\0");
                              ...
                              _field_action_t *fa)
{
    ...
    switch (f_ent->group->stage_id) {
        ...
        case _BCM_FIELD_STAGE_EGRESS:
           switch (fa->action) {
#if defined (BCM_HURRICANE4_SUPPORT) && defined (INCLUDE_XFLOW_MACSEC)
              case bcmFieldActionMacSecSvtagColor:
                  if ((BCM_FIELD_COLOR_GREEN != fa->param[0])
                      && (BCM_FIELD_COLOR_YELLOW != fa->param[0])
                      && (BCM_FIELD_COLOR_RED != fa->param[0])) {
                      return BCM_E_PARAM;
                  }
                  return BCM_E_NONE;
...
uint32 fp_svtag_flex_data_mask[16] =
   {0xff00, 0x00ff, 0x8000, 0x3000,
    0x0c00, 0x80ff, 0x30ff, 0x0cff,
    0xb000, 0x8c00, 0x3c00, 0xb0ff,
    0x8cff, 0x3cff, 0x00ff, 0xffff};
uint32 fp_fd2_rsvd_b = 0x4300; /* FLEX_DATS_2_MASK resvered bits */
...
_field_td3_flex_data_mask_set(int unit, _field_control_t *fc)
{
    ...
    for (idx = 2; idx < 15; idx++) {
        fp_svtag_flex_data_mask[idx] |= fp_fd2_rsvd_b;
    }

    mem = EFP_FLEX_DATA_MASKm;
    index_max = soc_mem_index_max(unit, mem);

    /* Fixed programming of mask data to the mem */
    for (idx = soc_mem_index_min(unit, mem); idx <= index_max; idx++) {
        BCM_IF_ERROR_RETURN(soc_mem_write(unit, mem,
                    MEM_BLOCK_ALL, idx,
                    &(fp_svtag_flex_data_mask[idx])));
    }
...
int
_bcm_td3_flex_data_index_get(int unit,
                             _field_entry_t *f_ent,
                             uint8 fd_type,
                             uint8 *fd_idx)
{
    ...
    for (idx = 0; (idx < 16) && (mask != (fp_svtag_flex_data_mask[idx])); idx++) {
        /* Do nothing. This will end when the index is found. */
    }
                          ...
                          )
{
...
#if defined (BCM_CHANNELIZED_SWITCHING_SUPPORT) || (defined(BCM_HURRICANE4_SUPPORT) && defined(INCLUDE_XFLOW_MACSEC))
    uint32 value = 0;
    uint8 idx = 0;
    _field_action_t *fa_temp = NULL;
#endif
    ...
    switch (fa->action) {
        ...
        case bcmFieldActionPacketTraceEnable:
             ...
             * FLEX_DATA_MASK (2) = 1111111111111111
             * FLEX_DATA_MASK (3) = 0000111111111111
             */
        case bcmFieldActionMacSecSvtagColor:
            for (fa_temp = f_ent->actions;
                (NULL != fa_temp) && (bcmFieldActionMacSecSubPortNumAdd != fa_temp->action);
                fa_temp = fa_temp->next);
            value = 0;
            if (NULL != fa_temp) {
                value = BCM_XFLOW_MACSEC_SUBPORT_ID_INDEX_GET(fa_temp->param[0]);
            }
            /* Set color now */
            switch (fa->param[0]) {
            ...
            PolicySet(unit, mem, buf, FLEX_DATA_2_MASK_INDEXf, 0x3);
            PolicySet(unit, mem, buf, FLEX_DATA_2_ACTIONf, 0x1);
            PolicySet(unit, mem, buf, FLEX_DATA_2f, value);
            break;
        case bcmFieldActionMacSecSubPortNumAdd:
            for (fa_temp = f_ent->actions;
                (NULL != fa_temp) && (bcmFieldActionMacSecSvtagColor != fa_temp->action);
                fa_temp = fa_temp->next);
            value = 0;
            if (NULL != fa_temp) {
                switch (fa_temp->param[0]) {
            ...
            value |= BCM_XFLOW_MACSEC_SUBPORT_ID_INDEX_GET(fa->param[0]); 
            PolicySet(unit, mem, buf, FLEX_DATA_2_MASK_INDEXf, idx);
            PolicySet(unit, mem, buf, FLEX_DATA_2_ACTIONf, 0x1);
            PolicySet(unit, mem, buf, FLEX_DATA_2f, value);
            break;
        case bcmFieldActionMacSecUseSubportNumberFromEvxlt:
        ...
        case bcmFieldActionSvtagSignature:
            ...
            BCM_IF_ERROR_RETURN(_bcm_td3_flex_data_index_get(unit, f_ent, 1, &idx));
            PolicySet(unit, mem, buf, FLEX_DATA_1_MASK_INDEXf, idx);
            break;
        case bcmFieldActionSvtagOffset:
            for (fa_temp = f_ent->actions;
        ...
        case bcmFieldActionSvtagPktType:
            ...
            BCM_IF_ERROR_RETURN(_bcm_td3_flex_data_index_get(unit, f_ent, 2, &idx));
            PolicySet(unit, mem, buf, FLEX_DATA_2_MASK_INDEXf, idx);
            PolicySet(unit, mem, buf, FLEX_DATA_2f, value);
            break;
#endif
                               ...
                               uint32 *data, uint32 *mask)
{
    ...
    switch (type) {
       ...
       case bcmFieldPktTypeSrh:
       case bcmFieldPktTypeSrhNextHeaderL4:
            rv = (soc_cancun_ceh_obj_field_get(unit, "Z2_MATCH_ID",
                                             flex_pkt_type_arr[type].field_str,
                                             &ceh_info));
            if (SOC_FAILURE(rv)) {
                                   ...
                                   bcm_field_pkt_type_t *type)
{
    ...
    *type = bcmFieldPktTypeLastCount;
    ...
    if ((data == ceh_info.value << ceh_info.offset)
            && (mask == ((1 << ceh_info.width) - 1) << ceh_info.offset)) {
       *type = bcmFieldPktTypeSrhNextHeaderL4;
       return rv;
    }
        ...
        case bcmFieldActionEgressFlowControlEnable:
            ...
            switch (fa->param[0]) {
                ...
                case bcmFieldEgressFlowControlMplsEvpnEsiFilter:
                     ...
                     valid = 1;
                     break;
                case bcmFieldEgressFlowControlSRv6RetainSgtAppId:
                     object = "RETAIN_SGT_APPID";
                     object_mask = "RETAIN_SGT_APPID_MASK";
                     valid = 1;
                     break;
                case bcmFieldEgressFlowControlSRv6RetainSgtAppIdForNonCanonicalEncap:
                     object = "RETAIN_SGT_APPID_FOR_NON_CANONICAL_ENCAP";
                     object_mask = "RETAIN_SGT_APPID_FOR_NON_CANONICAL_ENCAP_MASK";
                     valid = 1;
                     break;
                case bcmFieldEgressFlowControlSRv6EvpnEsiFilter:
                     object = "SRV6_EVPN_ESI_FILTER";
                     object_mask = "SRV6_EVPN_ESI_FILTER_MASK";
                     valid = 1;
                     break;
                case bcmFieldEgressFlowControlVxlanGbpFlexClassifierAdd:
                     object = "ASSIGN_GBP_SID_FROM_VXLAN_GBP_HDR";
                     object_mask = "ASSIGN_GBP_SID_FROM_VXLAN_GBP_HDR_MASK";
                     valid = 1;
                     break;
                case bcmFieldEgressFlowControlHiGigGbpSrcIdInherit:
                     object = "EXTRACT_GBP_SID_FROM_GBP_TAG";
                     object_mask = "EXTRACT_GBP_SID_FROM_GBP_TAG_MASK";
                     valid = 1;
                     break;
                case bcmFieldEgressFlowControlVxlanGbpFlexPolicyDoNotApply:
                     object = "DO_NOT_APPLY_GBP_POLICY_LOOKUP";
                     object_mask = "DO_NOT_APPLY_GBP_POLICY_LOOKUP_MASK";
                     valid = 1;
                     break;
                default:
                    break;
        ...
        case bcmFieldActionEgressFlowEncapEnable:
            ...
            switch (fa->param[0]) {
                ...
                case bcmFieldEgressFlowEncapIfaHdrInsertionOverTrunkUdpUpdate:
                     ...
                     valid = 1;
                     break;
                case bcmFieldEgressFlowEncapSRv6RetainSgtAppId:
                     object = "RETAIN_SGT_APPID";
                     object_mask = "RETAIN_SGT_APPID_MASK";
                     valid = 1;
                     break;
                case bcmFieldEgressFlowEncapVxlanGbpFlexPolicyApplied:
                     object = "VXLAN_GBP_POLICY_LOOKUP_DONE";
                     object_mask = "VXLAN_GBP_POLICY_LOOKUP_DONE_MASK";
                     valid = 1;
                     break;
                default:
                    break;
                                              ...
                                              int num_pipe)
{
    ...
    int            inst = -1;
    int            valid = 0;
    char           buf_ipbm[SOC_PBMP_FMT_LEN];
    uint32         entry_buf[_BCM_FP_MEM_FIELD_BYTES] = {0};
    ...
    uint8         class_arr[SOC_MAX_NUM_PORTS] = {0};
    ...
    if (no_pbm_access == 0) {
       ...
       sal_memset(class_arr, 0, sizeof(uint8) * SOC_MAX_NUM_PORTS);
       for (inst = 0; inst < num_pipe; inst++) {
           BCM_IF_ERROR_RETURN
               (_bcm_esw_port_field_ingress_class_range_get(unit,
                                                            _f_pbmp[inst].mask,
                                                            inst,
                                                            class_arr));
       }
       for (inst = 0; inst < num_pipe; inst++) {
          BCM_PBMP_ITER(_f_pbmp[inst].mask, port) {
              ...
              lport = (inst == 0) ? port : _BCM_TD3_NUM_PIPE_PORTS + port;
              port_class = class_arr[lport];
