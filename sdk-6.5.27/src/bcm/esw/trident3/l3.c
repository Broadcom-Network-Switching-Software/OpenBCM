        ...
        bcm_port_t port, int vc_swap_index)
{
    ...
    if (soc_feature(unit, soc_feature_enable_ifa_2)) {
        BCM_IF_ERROR_RETURN(
            bcm_esw_switch_control_get(unit, bcmSwitchIFA2Enable, &ifa_en));
    }

    ifa_en |= soc_property_get(unit, spn_IFA_ENABLE, 0);

    dst_port_bits  = BCMI_TD3_L3_NH_IFA_DST_PORT_BITS(ifa_en);
    dst_modid_bits = BCMI_TD3_L3_NH_IFA_DST_MODID_BITS(ifa_en)-1;
...
*/
    ...
    if (process_next_ptr && (l3_egress->flags2 & BCM_L3_FLAGS2_IFA_DST_PORT)) {
        ...
        if (l3_egress->port > (1 << BCMI_TD3_L3_NH_IFA_DST_PORT_BITS(ifa_en))) {
            return BCM_E_PARAM;
        }
        BCM_IF_ERROR_RETURN(
        ...
        for (idx = 0; idx < max_grp_size; idx++) {
            ...
            if (soc_feature(unit, soc_feature_urpf)) {
                ...
                if (SOC_REG_FIELD_VALID(unit, L3_DEFIP_RPF_CONTROLr, DEFIP_RPF_ENABLEf)) {
                    value = soc_reg_field_get(unit, L3_DEFIP_RPF_CONTROLr, reg_val,
                                              DEFIP_RPF_ENABLEf);
                    reg_val = value;
                }
                ...
                if (reg_val) {
                    if (idx < 8) {
                        if (BCMI_XGS3_L3_MPATH_GROUP_IDX_VALID(unit, nh_list[idx])) {
                            continue;
                        }
                        ...
                        if (entry_type == 0x0) {
                            ...
                            soc_mem_field32_set(unit, L3_ECMP_COUNTm, l3_ecmp_grp_buf,
                                                l3_ecmp_oif_type_flds[arr_idx], entry_type);
                            soc_mem_field32_set(unit, L3_ECMP_COUNTm, l3_ecmp_grp_buf,
                                                l3_ecmp_oif_flds[arr_idx], value);
                        ...
                        soc_mem_field32_set(unit, L3_ECMP_COUNTm, l3_ecmp_grp_buf, URPF_COUNTf, arr_idx);
                    } else {
