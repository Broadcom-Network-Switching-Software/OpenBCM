                               ...
                               bcm_ifa_cc_config_t *config_data)
{
    ...
    uint32                   app_status[2], uc_bit_map = 0, flag = 0;
    ...
    for (i = 0; i < BCM_IFA_CC_M0_NUM_UCS; i++) {
        ...
        if ((app_status[0] == BCM_IFA_CC_APP_STATUS_1) &&
            ...
            flag = soc_uc_mem_read(unit,
                    (sram_base_offset + ifa_cc_offset(flag)));

            if ((flag & BCM_IFA_CC_IFA2) != (config_data->flag & BCM_IFA_CC_IFA2)){
                LOG_ERROR(BSL_LS_BCM_IFA, (BSL_META_U(unit,
                                "Loaded wrong IFA CC APP on M0 : %d\n"), i));
                return BCM_E_PARAM;
            }
            uc_bit_map |= (0x1 << i);
            core[num_core] = i;
