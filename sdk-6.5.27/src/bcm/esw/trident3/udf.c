                                      ...
                                      uint32 *flex_feat_num_chunks)
{
    ...
    *flex_feat_num_chunks = num_chunks;
    return BCM_E_NONE;
}
...
STATIC 
int _bcm_udf_td3_update_unavail_chunks(
        _bcm_udf_td3_abstr_pkt_fmt_hw_info_t *hw_info,
        uint32 freed_chunk_bmap,
        uint32 flex_feat_num_chunks)
{
    int num_chunks_max = 0;
    int num_chunks = 0;
    int num_cmds = 0;
    int stage = 0;
    uint8 is_extra_cmds = 0;
    uint32 base_avail_chunks = 0;
    uint32 new_avail_chunks = 0;
    uint32 resultant_chunks = 0;

    if (hw_info == NULL) {
        return BCM_E_PARAM;
    }

    if (0 == freed_chunk_bmap) {
        return BCM_E_NONE;
    }

    base_avail_chunks = (~hw_info->unavail_chunk_bmap) & 0xFFFF;
    hw_info->unavail_chunk_bmap &= freed_chunk_bmap;
    new_avail_chunks = (~hw_info->unavail_chunk_bmap) & 0xFFFF;
    resultant_chunks = (base_avail_chunks ^ new_avail_chunks) & new_avail_chunks;

    num_chunks = _bcm_udf_count_chunks(resultant_chunks);

    if (0 == num_chunks) {
        return BCM_E_NONE;
    }

    num_chunks_max = (hw_info->max_extract_bytes)/2;
    if (hw_info->num_stages > 1) {
        num_chunks_max += num_chunks * hw_info->num_stages;;
        if (num_chunks_max > _BCM_UDF_TD3_MAX_CHUNKS) {
            num_chunks_max = _BCM_UDF_TD3_MAX_CHUNKS;
        }
        hw_info->max_extract_bytes = num_chunks_max * 2;
    } else {
        if (num_chunks_max > (_BCM_UDF_TD3_MAX_CHUNKS/2)) {
            is_extra_cmds = 1;
        }
        num_chunks_max += num_chunks;
        if (is_extra_cmds) {
            if (num_chunks_max > _BCM_UDF_TD3_MAX_CHUNKS) {
                num_chunks_max = _BCM_UDF_TD3_MAX_CHUNKS;
            }
            ...
            if (num_chunks_max > (_BCM_UDF_TD3_MAX_CHUNKS/2)) {
                num_chunks_max = (_BCM_UDF_TD3_MAX_CHUNKS/2);
            }
        }
        hw_info->max_extract_bytes = num_chunks_max * 2;
    }
    ...
    for (stage = 0; stage < hw_info->num_stages; stage++) {
        num_cmds = (hw_info->num_cmds[stage]);
        num_cmds += num_chunks;
        if (num_cmds > _BCM_UDF_MAX_POLICY_CMDS) {
            num_cmds = _BCM_UDF_MAX_POLICY_CMDS;
        }
        hw_info->num_cmds[stage] = num_cmds;
    }
...
_bcm_udf_flex_feature_based_unavail_chunk_init(int unit)
{
    ...
    uint32 rsrv_bmap = 0;
    uint32 reg_value = 0;
    uint32 f_int_bmp = 0, f_int_num_chunks = 0, int_enable = -1;
    uint32 f_ifa_bmp = 0, f_ifa_num_chunks = 0;
    uint32 f_nsh_bmp = 0, f_nsh_num_chunks = 0, nsh_enable = -1;
    uint32 f_gbp_sid_bmp = 0, f_gbp_sid_num_chunks = 0, gbp_sid_enable = -1;
    uint32 f_gpe_ioam_bmp = 0, f_gpe_ioam_num_chunks = 0, gpe_ioam_enable = -1;
    uint32 f_llc_bmp = 0, f_llc_num_chunks = 0, llc_enable = -1;
    uint32 f_ipsec_spi_bmp = 0, f_ipsec_spi_num_chunks = 0, ipsec_spi_enable = -1;
    uint32 f_mpls_evpn_bmp = 0, f_mpls_evpn_num_chunks = 0, mpls_evpn_enable = -1;
    uint32 f_mim_bmp = 0, f_mim_num_chunks = 0, mim_enable = -1;
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "INT", &f_int_bmp, &f_int_num_chunks));
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "IFA", &f_ifa_bmp, &f_ifa_num_chunks));
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "NSH", &f_nsh_bmp, &f_nsh_num_chunks));
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "GBP_SID", &f_gbp_sid_bmp, &f_gbp_sid_num_chunks));
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "GPE_IOAM", &f_gpe_ioam_bmp, &f_gpe_ioam_num_chunks));
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "IPSEC_SPI", &f_ipsec_spi_bmp, &f_ipsec_spi_num_chunks));
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "MPLS_EVPN", &f_mpls_evpn_bmp, &f_mpls_evpn_num_chunks));
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "MIM", &f_mim_bmp, &f_mim_num_chunks));
    BCM_IF_ERROR_RETURN
        (_bcm_udf_flex_feature_chunk_bmap_get(unit, "LLC", &f_llc_bmp, &f_llc_num_chunks));
    ...
    if ((1 == soc_property_get(unit, spn_INT_DISABLE, 0))
    ...
    for (pkt_fmt = 0; pkt_fmt < bcmUdfAbstractPktFormatLastCount; pkt_fmt++) {
        ...
        if (0 == nsh_enable) {
            BCM_IF_ERROR_RETURN(
                    _bcm_udf_td3_update_unavail_chunks(pkt_fmt_hw_info,
                                                    ~(f_nsh_bmp & (~rsrv_bmap)),
                                                    f_nsh_num_chunks));
        }
        if (0 == mpls_evpn_enable) {
            BCM_IF_ERROR_RETURN(
                    _bcm_udf_td3_update_unavail_chunks(pkt_fmt_hw_info,
                                                    ~(f_mpls_evpn_bmp & (~rsrv_bmap)),
                                                    f_mpls_evpn_num_chunks));
        }
        if (0 == mim_enable) {
            BCM_IF_ERROR_RETURN(
                    _bcm_udf_td3_update_unavail_chunks(pkt_fmt_hw_info,
        ...
        if (0 == gpe_ioam_enable) {
            BCM_IF_ERROR_RETURN(
                    _bcm_udf_td3_update_unavail_chunks(pkt_fmt_hw_info,
                                                    ~(f_gpe_ioam_bmp & (~rsrv_bmap)),
                                                    f_gpe_ioam_num_chunks));
        }
        if (0 == llc_enable) {
            BCM_IF_ERROR_RETURN(
                    _bcm_udf_td3_update_unavail_chunks(pkt_fmt_hw_info,
        ...
        if (0 == ipsec_spi_enable) {
            BCM_IF_ERROR_RETURN(
                    _bcm_udf_td3_update_unavail_chunks(pkt_fmt_hw_info,
                                                    ~(f_ipsec_spi_bmp & (~rsrv_bmap)),
                                                    f_ipsec_spi_num_chunks));
        }
        if (0 == gbp_sid_enable) {
            BCM_IF_ERROR_RETURN(
                    _bcm_udf_td3_update_unavail_chunks(pkt_fmt_hw_info,
                                                    ~(f_gbp_sid_bmp & (~rsrv_bmap)),
                                                    f_gbp_sid_num_chunks));
        }
        if (0 == int_enable) {
            BCM_IF_ERROR_RETURN(
                    _bcm_udf_td3_update_unavail_chunks(pkt_fmt_hw_info,
                                            ~((f_ifa_bmp | f_int_bmp) & (~rsrv_bmap)),
                                            (f_ifa_num_chunks + f_ifa_num_chunks)));
        }
    }
    ...
    _udf_obj_pkt_fmt_hw_cfg_t *cfg_data)
{
   ...
   bcm_udf_abstract_pkt_format_t pkt_fmt;
   uint8 f_buf[_BCM_UDF_MAX_PARSER_STAGES][32][50];
   uint8 buf[_BCM_UDF_MAX_PARSER_STAGES][32][100];
   ...
   for (stage = 0; stage < hw_info->num_stages; stage++) {
       ...
       for (hfe_ct = 0; hfe_ct < hw_info->hfe_prof_ptr_arr_len[stage]; hfe_ct++) {
           ...
           rv = soc_mem_read(unit, policy_mem[stage], MEM_BLOCK_ANY,
                             hfe_profile_ptr[stage], (uint32 *) (buf[stage][hfe_ct]));
           BCM_IF_ERROR_RETURN(rv);

           soc_mem_field_get(unit, policy_mem[stage], (uint32 *)(buf[stage][hfe_ct]),
                             CMD_POLICY_DATAf, (uint32 *)(f_buf[stage][hfe_ct]));
           ...
           for (i = start_cmd; i < max_cmds; i++) {
               if (cmd_bmap[stage] & (1 << i)) {
                  ...
                  if (hw_info->parser == _BCM_UDF_PARSER0) {
                      ...
                      soc_format_field32_set(unit, fmt, (uint32 *)(f_buf[stage][hfe_ct]),
           ...
           if (cmd_bmap[stage] || (null_update[stage])) {
              soc_mem_field_set(unit, policy_mem[stage], (uint32 *)(buf[stage][hfe_ct]),
                             CMD_POLICY_DATAf, (uint32 *)(f_buf[stage][hfe_ct]));
           }
