...
STATIC int
soc_td3_hw_intr_cb_unregister(int unit)
{
    BCM_IF_ERROR_RETURN(soc_td3_set_bst_callback(unit, NULL));
    return BCM_E_NONE;
}
...
STATIC int
_bcm_bst_td3_hw_stat_get(int unit, int port, int index, int buffer,
                         bcm_bst_stat_id_t bid, int *pvalue)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
    int rv, pipe;
        ...
        if ((base_mem = resInfo->stat_mem[0]) != INVALIDm) {
            ...
            for (xpe = from_xpe; xpe <= to_xpe; xpe++) {
                ...
                if (bid == bcmBstStatIdEgrPortPoolSharedMcast) {
                     ...
                     */
                    int port_in, pool;
                    port_in = index / _TD3_MMU_NUM_POOL;
                    pool = index % _TD3_MMU_NUM_POOL;
#ifdef BCM_HELIX5_SUPPORT
                    if (SOC_IS_HELIX5X(unit)) {
                        p_idx = pool * _HX5_MMU_PORTS_PER_PIPE + port_in;
                    } else
#endif
                    {
                        p_idx = pool * _TD3_MMU_PORTS_PER_PIPE + port_in;
                    }
                }
...
bcm_bst_td3_init(int unit)
{
    ...
    bst_info->hw_stat_get = _bcm_bst_td3_hw_stat_get;
    ...
    bst_info->hw_intr_cb_unregister = soc_td3_hw_intr_cb_unregister;
