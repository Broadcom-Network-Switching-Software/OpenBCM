...
bcm_time_spec_t gdpll_chan_ntp_offset = {0, 0, 0};
#endif
    ...
    int *gmii_mode)
{
    ...
    return (mode) ? TRUE : FALSE;
          ...
          BCM_APACHE_SUPPORT || BCM_MAVERICK2_SUPPORT || \
          BCM_HELIX5_SUPPORT || BCM_MONTEREY_SUPPORT || \
          BCM_GREYHOUND2_SUPPORT || BCM_FIRELIGHT_SUPPORT */
                                   ...
                                   bcm_time_capture_t *time)
{
    ...
    if (SOC_HAS_TIME_V3_OR_IPROC_MANAGED_BROADSYNC(unit)) {
        ...
        if (SOC_IS_TOMAHAWK3(unit) || SOC_IS_MONTEREY(unit) ||
            ...
            /* For Nanosync devices, must read in this order: 3->2->1->0. */
            if (SOC_IS_HURRICANE4(unit) || SOC_IS_FIREBOLT6(unit)) {
...
bcm_esw_time_init (int unit)
{
    ...
    if (soc_feature(unit, soc_feature_timesync_nanosync)) {
        ...
        for (idx = misc_start; idx < misc_end; idx++) {
            BCM_TIME_READ_NS_REGr(unit, ns_reg, idx, 0, &regVal);
            soc_reg_field_set(unit, ns_reg, &regVal,
                              EVENT_IDf, eventId);
            BCM_TIME_WRITE_NS_REGr(unit, ns_reg, idx, 0, regVal);
            eventId++;
        }
    ...
    if (soc_feature(unit, soc_feature_timesync_live_wire_tod)) {
        ...
        BCM_TIME_READ_NS_REGr(unit, NS_IEEE1588_TIME_FREQ_CONTROL_UPPERr, 0, 0, &regVal);
        soc_reg_field_set(unit, NS_IEEE1588_TIME_FREQ_CONTROL_UPPERr, &regVal,
                          VALf, 0x2000);
        BCM_TIME_WRITE_NS_REGr(unit, NS_IEEE1588_TIME_FREQ_CONTROL_UPPERr, 0, 0, regVal);
         ...
         */
        BCM_TIME_READ_NS_REGr(unit, NS_IEEE1588_TIME_CONTROL_LOADr, 0, 0, &regVal);
        soc_reg_field_set(unit, NS_IEEE1588_TIME_CONTROL_LOADr, &regVal,
                          LOAD_TYPEf, 0x0);
        soc_reg_field_set(unit, NS_IEEE1588_TIME_CONTROL_LOADr, &regVal,
                          ENABLEf, 0x1);
        BCM_TIME_WRITE_NS_REGr(unit, NS_IEEE1588_TIME_CONTROL_LOADr, 0, 0, regVal);

        /* Enable TOD's counter */
        BCM_TIME_READ_NS_REGr(unit, NS_IEEE1588_TIME_CONTROLr, 0, 0, &regVal);
        soc_reg_field_set(unit, NS_IEEE1588_TIME_CONTROLr, &regVal,
                          COUNTER_ENABLEf, 0x1);
        BCM_TIME_WRITE_NS_REGr(unit, NS_IEEE1588_TIME_CONTROLr, 0, 0, regVal);
    }
    ...
    if (intf->flags & BCM_TIME_SYNC_STAMPER) {
        ...
        if (intf->flags & BCM_TIME_OFFSET) {
            ...
            if (intf->flags & BCM_TIME_NTP_OFFSET) {
                gdpll_chan_ntp_offset = intf->ntp_offset;
            }
#endif
...
bcm_esw_time_heartbeat_enable_set (int unit, bcm_time_if_t id, int enable)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        if (enable) {
            ...
            soc_cmic_intr_disable(unit, TS_INTR);
            if (!soc->time_call_ref_count) {
                soc->soc_time_callout = NULL;
            }
        }
...
_bcm_time_synce_phy_register_set(int unit, int port, int value, uint32 phy_reg)
{
    ...
    if (phy_reg != BCM_TIME_TSCF_RX_X4_SYNCE_FRACTIONAL_DIV_REG) {
        ...
        if (phy_reg == BCM_TIME_QTCE_MAIN0_SYNCE_CTL_STAGE0_REG) {
            value = (val & ~(0xc << (phy_lane * 4))) | (((value & 0x3) << 2) << (phy_lane * 4));
        } else {
...
STATIC int
_bcm_esw_time_gh2_synce_clock_set_by_extphy(int unit,
                               bcm_time_synce_clock_src_type_t clk_src,
                               bcm_time_synce_divisor_setting_t *divisor)
{
    soc_reg_t synce_ctrl_2 = TOP_MISC_CONTROL_2r;
    ...
    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, synce_ctrl_2,
                                REG_PORT_ANY, l1_rcvd_port_sel, clk_port_sel));

    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, synce_ctrl_2,
                                REG_PORT_ANY, l1_rcvd_lnksts_sel, linksts_port_sel));
                              ...
                              bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    if ((val > 1) && (val < 4)) {
        /* ExtPhy case. */
        divisor->index = (val == 2) ? 0 : 1;
        divisor->input_src = bcmTimeSynceInputSourceTypeExtPhy;
                              ...
                              bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    case bcmTimeSynceInputSourceTypePLL:
        ...
        break;
    default:
        rv = _bcm_esw_time_gh2_synce_clock_set_by_extphy(unit, clk_src, divisor);
        break;
    }
    return rv;
}
...
int
bcmi_esw_time_fl_synce_L1_rcvd_clk_port_get(int unit, int phy_port)
{
    int adjusted_phy_port;
    int base_phy_port;
    int logical_port;
    int lane;
    int num_lanes_per_core = 4; /* TSCF and TSCE */

    logical_port        = SOC_INFO(unit).port_p2l_mapping[phy_port];
    base_phy_port       = phy_port - ((phy_port-2)%num_lanes_per_core);
    adjusted_phy_port   = phy_port;
#ifdef PORTMOD_SUPPORT
    if (SOC_PORT_USE_PORTCTRL(unit, logical_port)) {
        phymod_lane_map_t lane_map;
        uint32_t flags = 0;
        ...
        SOC_IF_ERROR_RETURN
            (portmod_port_lane_map_get(unit, logical_port, flags, &lane_map));
        lane = lane_map.lane_map_rx[(phy_port - 2) % num_lanes_per_core];
        adjusted_phy_port = base_phy_port + lane;
    }
#endif

    if (adjusted_phy_port >= 2 && adjusted_phy_port <= 49) {
         /* PM4x10Q with Q mode, TSCE0_0, TSCE0_1, ..., TSCE2_3. */
        return ((adjusted_phy_port - 2)/4 + 8);
    }
    else if (adjusted_phy_port >= 50 && adjusted_phy_port <= 61) {
         /* PM4x10Q with eth mode, TSCE0, TSCE1, TSCE2 */
        return (adjusted_phy_port - 50 + 8);
    }
    else if (adjusted_phy_port >= 62 && adjusted_phy_port <= 77) {
         /* Falcon : TSCF0, TSCF1, TSCF2, TSCF3 */
        return (adjusted_phy_port - 62 + 24);
    }
    else {
         return BCMI_TIME_PORT_SEL_INVALID;
    }
    ...
    int port_sel)
{
    int adjusted_phy_port;
    int base_phy_port;
    int num_lanes_per_core = 4;
    int adjusted_lane;
    int first_logical_port;
    int lane_indx = 0;
    ...
    if (port_sel >= 8 && port_sel <= 19) {
        ...
        if (bcmi_esw_time_port_is_gmii_mode(unit, lport, NULL)) {
            adjusted_phy_port = pport_q;
        } else {
            adjusted_phy_port = pport;
        }
    }
    else if (port_sel >= 24 && port_sel <= 39) {
        /* Falcon : TSCF0, TSCF1, TSCF2, TSCF3 */
        adjusted_phy_port = (port_sel - 24 + 62);
    }
    else {
        return BCMI_TIME_PHY_PORT_INVALID;
    }

    base_phy_port = adjusted_phy_port - ((adjusted_phy_port-2)%num_lanes_per_core);
    adjusted_lane = adjusted_phy_port - base_phy_port;

    first_logical_port = SOC_INFO(unit).port_p2l_mapping[base_phy_port];
#ifdef PORTMOD_SUPPORT
    if (SOC_PORT_USE_PORTCTRL(unit, first_logical_port)) {
        phymod_lane_map_t lane_map;
        uint32_t flags = 0;
        if (!SOC_PORT_VALID(unit,first_logical_port))
        {
            return BCMI_TIME_PHY_PORT_INVALID;
        }
        SOC_IF_ERROR_RETURN
            (portmod_port_lane_map_get(unit, first_logical_port, flags, &lane_map));

        for (lane_indx = 0; lane_indx < num_lanes_per_core; lane_indx++) {
            if (lane_map.lane_map_rx[lane_indx] == adjusted_lane) {
                break;
            }
        }

        return base_phy_port + lane_indx;
    }
#endif

    return adjusted_phy_port;
}
    ...
    if ( clk_src == bcmTimeSynceClockSourcePrimary ) {
        ...
        switch (dispatch_type) {
            ...
            case phymodDispatchTypeQtce:
                /* TOP_MISC_CONTROL_2 */
                if (divisor->stage1_div == bcmTimeSynceStage1Div1) {
                    SOC_IF_ERROR_RETURN
                        (soc_reg_field32_modify(unit, TOP_MISC_CONTROL_2r, REG_PORT_ANY,
                                                L1_RCVD_FREQ_SELf, 3));
                } else {
        ...
        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, TOP_MISC_CONTROL_2r, REG_PORT_ANY,
                                    L1_RCVD_SW_OVWR_VALIDf, 0));
        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, TOP_MISC_CONTROL_2r, REG_PORT_ANY,
                                    L1_RCVD_SW_OVWR_ENf, 0));
                              ...
                              bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    if (0 == phy_port) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit,
                            "_bcm_esw_time_hurricane3_synce_clock_get : Error !! Invalid port\n")));
        return BCM_E_PARAM;
    }
...
#ifdef PHYMOD_QTCE_SUPPORT
            case phymodDispatchTypeQtce:
                if ((phy_port >= 2 && phy_port <= 5) || (phy_port >= 18 && phy_port <= 21)) { /* QTCE */
                    if (isSGMII) {
                        BCM_IF_ERROR_RETURN(_bcm_time_synce_phy_register_get(unit, divisor->index,
                                    BCM_TIME_QTCE_MAIN0_SYNCE_CTL_STAGE0_REG, &divisor->stage1_div));
                    }
                }
                break;
#endif
                             ...
                             bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    if (SOC_IS_HURRICANE3(unit)) {
      rv = _bcm_esw_time_hurricane3_synce_clock_get(unit, clk_src, divisor);
    }
#endif
    return rv;
}
                        ...
                        case phymodDispatchTypeFalcon_dpll:
                            if (SOC_IS_MONTEREY(unit)) {
                                ...
                                if (port_speed == 2457) {
                                    ...
                                    div_in.stage0_sdm_whole = 0x05;
                                    div_in.stage0_sdm_frac = 0x00;
                                } else if (port_speed == 6144) {
                                    div_in.stage0_sdm_whole = 0x06;
                                    div_in.stage0_sdm_frac = 0x40;
                                } else if (port_speed == 9830) {
                                    div_in.stage0_sdm_whole = 0x0A;
                                    div_in.stage0_sdm_frac = 0x00;
                                } else if (port_speed == 10137) {
                                    div_in.stage0_sdm_whole = 0x0A;
                                    div_in.stage0_sdm_frac = 0x50;
                                } else if (port_speed == 12165) {
                                    div_in.stage0_sdm_whole = 0x0C;
                                    div_in.stage0_sdm_frac = 0x60;
                                } else if (port_speed == 24330) {
                                    div_in.stage0_sdm_whole = 0x18;
...
_bcm_esw_time_counter_get(int unit, int ts_counter, uint64 *pTS_val)
{
    ...
    if (ts_counter == 1) {
        BCM_TIME_READ_NS_REGr(unit, ts1_reg1, 0, 0, &upper);
        BCM_TIME_READ_NS_REGr(unit, ts1_reg0, 0, 0, &lower);
    } else if(ts_counter == 0) {
        BCM_TIME_READ_NS_REGr(unit, ts0_reg1, 0, 0, &upper);
        BCM_TIME_READ_NS_REGr(unit, ts0_reg0, 0, 0, &lower);
    } else {
        return BCM_E_FAIL;
    }
        ...
        bcm_time_ts_counter_t  *counter)
{
    ...
    if (soc_feature(unit, soc_feature_timesync_nanosync)) {
        ...
        if (counter->time_format == bcmTimeFormatNTP){
            return BCM_E_UNAVAIL;
        }
        ...
        if (SOC_IS_TOMAHAWK3(unit) || SOC_IS_MONTEREY(unit) || SOC_IS_FIREBOLT6(unit) || SOC_IS_FIRELIGHT(unit) || SOC_IS_HURRICANE4(unit)) {
        ...
        bcm_time_ts_counter_t  *counter)
{
    ...
    soc_reg_t ts1_reg;
     ...
     defined(BCM_GREYHOUND_SUPPORT) || defined(BCM_GREYHOUND2_SUPPORT) || \
    ...
    if (soc_feature(unit, soc_feature_timesync_nanosync)) {
        ...
        if (ts_counter == 1) {
            BCM_TIME_READ_NS_REGr(unit, ts1_reg, 0, 0, &regval);
            counter->enable = soc_reg_field_get(unit, ts1_reg, regval, ENABLEf);
        } else if(ts_counter == 0) {
            BCM_TIME_READ_NS_REGr(unit, ts0_reg, 0, 0, &regval);
        ...
        if (SOC_IS_TOMAHAWK3(unit) || SOC_IS_HURRICANE4(unit)) {
            ...
            if (ts_counter == 1) {
                ...
                READ_NS_TIMESYNC_TS0_FREQ_CTRL_FRAC_UPPERr(unit, &freq_ctrl);
                READ_NS_TIMESYNC_TS0_FREQ_CTRL_FRAC_LOWERr(unit, &freq_ctrl_2);
            }
    ...
    bcm_time_tod_t *tod)
{
    ...
    if (!SOC_MEM_IS_VALID(unit, NTP_TIMEm)) {
        return BCM_E_UNAVAIL;
    }

    sal_memset(&entry, 0,
               sizeof(ntp_time_entry_t));
    ...
    bcm_time_tod_t *tod)
{
    ...
    if (!SOC_MEM_IS_VALID(unit, IEEE1588_TIMEm)) {
        return BCM_E_UNAVAIL;
    }
    ...
    bcm_time_tod_t *tod)
{
    ...
    BCM_TIME_READ_NS_REGr(unit, NS_PTP_TOD_A_VALUE_2r, 0, 0, &regVal);
    data_sec[1] = soc_reg_field_get(unit, NS_PTP_TOD_A_VALUE_2r, regVal, VALf);

    /* PTP.TOD[63:32] Value for CPU read. */
    BCM_TIME_READ_NS_REGr(unit, NS_PTP_TOD_A_VALUE_1r, 0, 0, &regVal);
    ...
    BCM_TIME_READ_NS_REGr(unit, NS_PTP_TOD_A_VALUE_0r, 0, 0, &regVal);
    ...
    bcm_time_tod_t *tod)
{
    ...
    BCM_TIME_READ_NS_REGr(unit, NS_NTP_TOD_VALUE_1r, 0, 0, &regVal);
    data_sec[0] = soc_reg_field_get(unit, NS_NTP_TOD_VALUE_1r, regVal, VALf);

    COMPILER_64_SET(tod->ts.seconds, data_sec[1], data_sec[0]);

    /* NTP.TOD[31:0] Value for CPU read */
    BCM_TIME_READ_NS_REGr(unit, NS_NTP_TOD_VALUE_0r, 0, 0, &regVal);
...
#if (defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT) || \
     defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_MAVERICK2_SUPPOR))
    ...
    switch (event_misc) {
        ...
        case _bcmTimeCaptureEventGPIO5:
            idx = event_misc - _bcmTimeCaptureEventGPIO0;
            {
                ...
                BCM_TIME_READ_NS_REGr(unit, ns_reg, 0, 0, &regVal);
                soc_reg_field_set(unit,  ns_reg, &regVal, DIVISORf, divisor);
                BCM_TIME_WRITE_NS_REGr(unit, ns_reg, 0, 0, regVal);
            }
            break;
                            ...
                            int enable)
{
    ...
    if (SOC_REG_IS_VALID(unit, NS_TIMESYNC_TS_EVENT_FWD_CFGr)) {
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TS_EVENT_FWD_CFGr, idx, 0, &regVal);
        ...
        BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_TS_EVENT_FWD_CFGr, idx, 0, regVal);
    } else {
        rv = BCM_E_UNAVAIL;
        goto exit;
    }
    ...
    bcm_time_port_event_t port_event_type, uint32 divisor)
{
    ...
    switch (port_event_type) {
        ...
        case bcmTimePortEventRXCDR:
            {
                BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_RX_CDR_EVENT_CTRLr, idx, 0, &regVal);
                soc_reg_field_set(unit, NS_TIMESYNC_RX_CDR_EVENT_CTRLr, &regVal,
                                DIVISORf, divisor);
                BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_RX_CDR_EVENT_CTRLr, idx, 0, regVal);
            }
            break;

        case bcmTimePortEventTXPI:
            {
                BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TX_PI_CLK_EVENT_CTRLr, idx, 0, &regVal);
                soc_reg_field_set(unit, NS_TIMESYNC_TX_PI_CLK_EVENT_CTRLr, &regVal,
                                DIVISORf, divisor);
                BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_TX_PI_CLK_EVENT_CTRLr, idx, 0, regVal);
            }
            break;
        ...
        case bcmTimePortEventROE:      /* Type-2 */
...

            {
                bsl_printf("BCM_TIME: Setting NS_TIMESYNC_MAPPER_FWD_CFG idx:%d\n",
                            idx);
                BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_MAPPER_FWD_CFGr, idx, 0, &regVal);
                ...
                soc_reg_field_set(unit, NS_TIMESYNC_MAPPER_FWD_CFGr, &regVal,
                                CHK_PROFILEf, chkProfile);
                BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_MAPPER_FWD_CFGr, idx, 0, regVal);
            }
            break;
...
_bcm_esw_time_event_roe_52b_set(int unit, bcm_port_t port, int enable)
{
    ...
    {
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_MAPPER_TYPE2_FORMATr, port, 0, &regVal);
        ...
        BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_MAPPER_TYPE2_FORMATr, port, 0, regVal);
    }
    ...
    switch (event_misc){
        ...
        case _bcmTimeCaptureEventBS1ConvTC:
...

            {
                ...
                BCM_TIME_READ_NS_REGr(unit, NS_MISC_CLK_EVENT_CTRLr, 0, 0, &regVal);
                ...
                soc_reg_field_set(unit, NS_MISC_CLK_EVENT_CTRLr, &regVal,
                                ENABLEf, reg_enable);
                BCM_TIME_WRITE_NS_REGr(unit, NS_MISC_CLK_EVENT_CTRLr, 0, 0, regVal);
            }
            break;
        ...
        case _bcmTimeCaptureEventGPIO5:
            idx = event_misc - _bcmTimeCaptureEventGPIO0;
            {
                BCM_TIME_READ_NS_REGr(unit, _bcm_time_gpio_regs[idx], 0, 0, &regVal);
#ifndef INCLUDE_GDPLL
                soc_reg_field_set(unit,  _bcm_time_gpio_regs[idx], &regVal,
                               CAPTURE_ENABLEf, enable ? 1:0);
#endif
                BCM_TIME_WRITE_NS_REGr(unit, _bcm_time_gpio_regs[idx], 0, 0, regVal);
}
            break;
        ...
        case _bcmTimeCaptureEventBS1PLL:
            idx = event_misc - _bcmTimeCaptureEventBS0PLL;
            {
                BCM_TIME_READ_NS_REGr(unit, _bcm_time_bs_pll_regs[idx], 0, 0, &regVal);
                soc_reg_field_set(unit, _bcm_time_bs_pll_regs[idx], &regVal,
                                ENABLEf, enable ? 1:0);
                BCM_TIME_WRITE_NS_REGr(unit, _bcm_time_bs_pll_regs[idx], 0, 0, regVal);
            }
            break;
        ...
        case _bcmTimeCaptureEventLCPLL3:
            idx = event_misc - _bcmTimeCaptureEventLCPLL0;
            {
                BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_SERDES_LCPLL_EVENT_CTRLr, idx, 0, &regVal);
                soc_reg_field_set(unit, NS_TIMESYNC_SERDES_LCPLL_EVENT_CTRLr, &regVal,
                                ENABLEf, enable ? 1:0);
                BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_SERDES_LCPLL_EVENT_CTRLr, idx, 0, regVal);
            }
            break;
                                 ...
                                 int enable)
{
    ...
    switch (port_event_type) {
        ...
        case bcmTimePortEventROE:
            ...
            {
                BCM_TIME_READ_NS_REGr(unit, __bcm_time_mapper_port_enable_regs[mapper_reg], 0, 0, &regVal);
                ...
                BCM_TIME_WRITE_NS_REGr(unit, __bcm_time_mapper_port_enable_regs[mapper_reg], 0, 0, regVal);
            }
        ...
        case bcmTimePortEventTXPI:
            ...
            {
                BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TX_PI_CLK_EVENT_CTRLr, port, 0, &regVal);
                soc_reg_field_set(unit, NS_TIMESYNC_TX_PI_CLK_EVENT_CTRLr, &regVal, ENABLEf, enable ? 1:0);
                BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_TX_PI_CLK_EVENT_CTRLr, port, 0, regVal);
            }
            break;
        ...
        void *user_data)
{
    ...
    if (cb_type != bcmTimeCaptureTS0CntOffsetUpdated && cb_type != bcmTimeCaptureTS1CntOffsetUpdated){
        ...
        if (SOC_IS_MONTEREY(unit) || SOC_IS_TOMAHAWK3(unit)|| SOC_IS_HURRICANE4(unit)) {
            uint32 regval;
            BCM_TIME_READ_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, &regval);
            regval |= INTR_NS_TIME_CAPTURE;
            BCM_TIME_WRITE_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, regval);
        }
#ifdef BCM_CMICX_SUPPORT
        if (soc_feature(unit, soc_feature_cmicx)) {
            soc_cmic_intr_enable(unit, TS_INTR);
        ...
        bcm_time_capture_cb_type_t cb_type)
{
    ...
    if (cb_overflow.cb == NULL && cb_fill.cb == NULL && cb_all.cb == NULL){
        ...
        if (soc_feature(unit, soc_feature_timesync_nanosync)) {
            ...
            BCM_TIME_WRITE_NS_REGr(unit, IPROC_NS_TS_INT_ENABLEr, 0, 0, 0);
            }
        }
#endif
    }
                  ...
                  void *d3, void *d4)
{
...

    {
        ...
        BCM_TIME_READ_NS_REGr(unit, NS_TS_INT_STATUSr, 0, 0, &g_int_status);
        BCM_TIME_READ_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0, &int_enable);
       ...
       /* TS0_CNT_OFFSET_UPDATED interrupt */
        if (tsfifo_flag && soc_reg_field_get(unit, NS_TS_INT_STATUSr, g_int_status, TS0_CNT_OFFSET_UPDATEDf)) {
            ...
            BCM_TIME_WRITE_NS_REGr(unit, NS_TS_INT_STATUSr, 0, 0, int_status_clear);
        }
        ...
        if (tsfifo_flag && soc_reg_field_get(unit, NS_TS_INT_STATUSr, g_int_status, TS_FIFO_NOT_EMPTYf)) {
            ...
            BCM_TIME_READ_NS_REGr(unit, NS_TS_CAPTURE_STATUSr, 0, 0, &capture_status);
        ...
        if (tsfifo_flag && soc_reg_field_get(unit, NS_TS_INT_STATUSr, g_int_status, TS_FIFO_OVERFLOWf)) {
            ...
            BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_INPUT_TIME_FIFO_STATUSr, 0, 0, &capture_status);
...
soc_esw_nanosync_ts_cmicx_intr(int unit, void *data)
{
    ...
    /* Read the GDPLL interrupt */
    BCM_TIME_READ_NS_REGr(unit, tsfifo_int_status, 0, 0, &int_status);
    BCM_TIME_READ_NS_REGr(unit, tsfifo_int_enable, 0, 0, &int_enable);
    ...
    /* TS_FIFO_NOT_EMPTY interrupt */
    {
        if (soc_reg_field_get(unit, tsfifo_int_status, int_status, tsfifo_not_empty)) {
            ...
            BCM_TIME_READ_NS_REGr(unit, tsfifo_capture_status, 0, 0, &capture_status);
            ...
            for (; fifo_depth != 0; fifo_depth--) {
                ...
                if (fifo_depth == 1) {
                    ...
                    /* Write W1TC to clear the flags in NS_TS_INT_STATUS */
                    BCM_TIME_WRITE_NS_REGr(unit, tsfifo_int_status, 0, 0, int_status_clear);

                    /*  Read it back to confirm. */
                    BCM_TIME_READ_NS_REGr(unit, tsfifo_int_status, 0, 0, &int_status_clear);
                ...
                /* For Nanosync devices, must read in this order: 3->2->1->0. */
                BCM_TIME_READ_NS_REGr(unit, tsfifo_ts[3], 0, 0, &regval3);
                BCM_TIME_READ_NS_REGr(unit, tsfifo_ts[2], 0, 0, &regval2);
                BCM_TIME_READ_NS_REGr(unit, tsfifo_ts[1], 0, 0, &regval1);
                BCM_TIME_READ_NS_REGr(unit, tsfifo_ts[0], 0, 0, &regval0);
    ...
    if (soc_reg_field_get(unit, tsfifo_int_status, int_status, tsfifo_overflow)) {
        ...
        BCM_TIME_READ_NS_REGr(unit, tsfifo_ovf_status, 0, 0, &capture_status);
    ...
    int ts_counter)
{
    ...
    if (SOC_IS_FIRELIGHT(unit))
    {
        BCM_TIME_READ_NS_REGr(unit, IPROC_NS_TIMESYNC_COUNTER_CONFIG_SELECTr, 0, 0, &regval_upper);
    } else {
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_COUNTER_CONFIG_SELECTr, 0, 0, &regval_upper);
    }
    ...
    BCM_TIME_READ_NS_REGr(unit, offset_upper, 0, 0, &regval_upper);
    BCM_TIME_READ_NS_REGr(unit, offset_lower, 0, 0, &regval_lower);
    curr_offset = (((uint64)regval_upper & 0xFFFFF) << 32) | (regval_lower);

    BCM_TIME_READ_NS_REGr(unit, ts_ctr_upper, 0, 0, &regval_upper);
    BCM_TIME_READ_NS_REGr(unit, ts_ctr_lower, 0, 0, &regval_lower);
    ...
    BCM_TIME_WRITE_NS_REGr(unit, offset_upper, 0, 0, (new_offset >> 32) & 0xFFFFF);
    BCM_TIME_WRITE_NS_REGr(unit, offset_lower, 0, 0, new_offset & 0xFFFFFFFF);

    ts_ctr_val = ((ts_ctr_val >> 4) + ts_update_intv) % 0xFFFFFFFFFFFFULL;
    BCM_TIME_WRITE_NS_REGr(unit, update_lower, 0, 0, ts_ctr_val & 0xFFFFFFFF);
    BCM_TIME_WRITE_NS_REGr(unit, update_upper, 0, 0, ((ts_ctr_val >> 32) & 0xFFFF) | 0x10000);
    rv = BCM_E_NONE;
exit:
#endif
    ...
    int ts_counter)
{
...
#if (defined (BCM_MONTEREY_SUPPORT) ||  defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_FIREBOLT6_SUPPORT) || \
    ...
    BCM_TIME_WRITE_NS_REGr(unit, offset_upper, 0, 0, 0);
    BCM_TIME_WRITE_NS_REGr(unit, offset_lower, 0, 0, 0);
...
_bcm_bs_tod_dpll_chan_create(int unit, bcm_time_format_t tod_type)
{
    ...
    if (tod_type == bcmTimeFormatNTP) {
        ...
        result_nanosec = ts.nanoseconds + gdpll_chan_ntp_offset.nanoseconds;
        prog_tod.nanoseconds = result_nanosec % 1000000000;
        COMPILER_64_ADD_32(prog_tod.seconds, (result_nanosec / 1000000000));
        COMPILER_64_ADD_64(prog_tod.seconds, gdpll_chan_ntp_offset.seconds);
...
int bcm_esw_time_ts_counter_time_init(int unit)
{
    ...
    if ((SOC_IS_KATANAX(unit)) || (SOC_IS_TRIDENT2(unit))) {
        return rv;
    }
#endif
...
#if defined(BCM_TIME_NANOSYNC_SUPPORT)
        if (soc_feature(unit, soc_feature_timesync_nanosync)) {
            SOC_IF_ERROR_RETURN(_bcm_esw_time_counter_enable(unit, 0, 0));
            SOC_IF_ERROR_RETURN(_bcm_esw_time_counter_freq_frac_set(unit, 0));
            SOC_IF_ERROR_RETURN(_bcm_esw_time_counter_set(unit, 0, 0));
            SOC_IF_ERROR_RETURN(_bcm_esw_time_counter_enable(unit, 0, 1));
        } else
#endif
        {
            READ_CMIC_TIMESYNC_TS0_COUNTER_ENABLEr(unit, &regval);
...
int bcm_esw_time_ts_time_update(int unit)
{
...
#if defined(BCM_TIME_NANOSYNC_SUPPORT)
        if (soc_feature(unit, soc_feature_timesync_nanosync) &&
            (!(SOC_IS_TOMAHAWK3(unit) || SOC_IS_HURRICANE4(unit) ||
               SOC_IS_FIRELIGHT(unit) || SOC_IS_FIREBOLT6(unit)))) {
            /* The ts counter value was already in units of ns for TH3,HR4,FL&FB6 */
            COMPILER_64_SHR(ctr.ts_counter_ns, 4);
        }
#endif
    ...
    LOG_DEBUG(BSL_LS_BCM_TIME,
                (BSL_META_U(0, " ts_time_update: 0x%016llx(%015llu) - %06llu.%09d\n"),
                (long long unsigned int)hwts,
                (long long unsigned int)hwts,
                (long long unsigned int)tod.seconds,
                tod.nanoseconds));
...
int bcm_esw_time_ts_time_convert(int unit, uint32 ts_val, bcm_time_spec_t* time)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_TIME,
                (BSL_META_U(0, " ts_time_convert: 0x%08x(%010u) - %06llu.%09d\n"),
                (unsigned int)ts_val,
                (unsigned int)ts_val,
                (long long unsigned int)time->seconds,
                time->nanoseconds));
