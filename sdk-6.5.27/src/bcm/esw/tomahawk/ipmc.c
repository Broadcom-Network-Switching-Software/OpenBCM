...
_bcm_th_repl_head_info_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_td3x1_reduced_ipmc)) {
        ...
        /* Replication Head is reduced from 16K to 8K in WH3 */
        repl_head_tbl_size_adjust = 4;
    }
#endif
...
_bcm_th_repl_list_entry_info_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_td3x1_reduced_ipmc)) {
        ...
        /* Replication Head is reduced from 8K to 4K in WH3 */
        repl_list_tbl_size_adjust = 4;
    }
#endif
...
/*    mmu_repl_group_initial_copy_count_entry_t initial_copy_count_entry;*/
    ...
    copy_count_mp_cpu = copy_count;
    /* On TD3 and MV2, HW only supports 2b ICC
     * for CPU and management ports
     */
    if (copy_count > 3) {
        copy_count_mp_cpu = 3;
    }
        ...
        if (slice == 0) {
            if (phy_port == 128 &&
                ...
                soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                        &initial_copy_count_entry, ICC_BIT_2ND_MPf, copy_count_mp_cpu);
                ...
                soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                        &initial_copy_count_entry, ICC_BIT_MPf, copy_count_mp_cpu);
            } else if (IS_CPU_PORT(unit, port)) {
                soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                        &initial_copy_count_entry, ICC_BIT_CPUf, copy_count_mp_cpu);
