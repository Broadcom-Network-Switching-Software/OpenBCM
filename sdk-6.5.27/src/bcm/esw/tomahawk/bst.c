...
#define TH_BST_HW_EVENT_TRIGGER 0x1

#define TH_BST_REG_WIDTH(unit, reg) \
    ((SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)) && \
     (reg == MMU_THDR_DB_BST_STAT_PRIQr || \
      reg == MMU_THDR_DB_BST_STAT_SPr)) ? 2 : BYTES2WORDS(soc_reg_bytes(unit, reg))
...
STATIC int _bcm_bst_th_sync_report(int unit, bcm_bst_stat_id_t bid,
                                   int port, int index, int flags,
                                   int xpe_id, int pipe_id);
...
STATIC int
soc_th_hw_intr_cb_unregister(int unit)
{
    BCM_IF_ERROR_RETURN(soc_th_set_bst_callback(unit, NULL));
    return BCM_E_NONE;
}
...
_bcm_bst_th_intr_to_resources(int unit, uint32 *flags)
{
    ...
    int xpe_id[_bcmResourceMaxCount], pipe_id[_bcmResourceMaxCount];
    _bcm_bst_cmn_unit_info_t *bst_info;
    ...
    for (i = 0; i < _bcmResourceMaxCount; i++) {
        xpe_id[i] = -1;
        pipe_id[i] = -1;
    }
    ...
    /* In peak mode, _bcmResourceDevice is checked every time to keep with legacy.
     * In current mode, only when no other triggers found, the interrupt is
     * triggered by _bcmResourceDevice.
     */
    if (res_ct == 0 || bst_info->track_mode) {
    ...
    for (i = 0; i < res_ct; i++) {
        BCM_IF_ERROR_RETURN(
            _bcm_bst_th_sync_report(unit, res[i], -1, index[i],
            TH_BST_HW_EVENT_TRIGGER, xpe_id[i], pipe_id[i]));
    }
...
STATIC int
_bcm_bst_th_hw_stat_get(int unit, int port, int index, int buffer, bcm_bst_stat_id_t bid,
                        int *pvalue)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
    ...
    soc_mem_t base_mem = INVALIDm, mem = INVALIDm;
    soc_reg_t reg;
    int pipe_num = 1, inst, stats_per_inst;
    int p_idx;
    uint32 mmu_inst_map = 0;
    ...
    for (pipe = 0; pipe < pipe_num; pipe++) {
        if ((base_mem = resInfo->stat_mem[0]) != INVALIDm) {
            int xpe = 0, from_xpe, to_xpe;
            if (buffer < 0 || buffer >= NUM_XPE(unit)) {
                from_xpe = 0;
                to_xpe = NUM_XPE(unit) - 1;
            } else {
                from_xpe = to_xpe = buffer;
            }
            for (xpe = from_xpe; xpe <= to_xpe; xpe++) {
                uint32 entry[SOC_MAX_MEM_WORDS];
                if ((port != -1) && !(mmu_inst_map & (1 << xpe))) {
                    continue;
                }
                ...
                if (bid == bcmBstStatIdEgrPortPoolSharedMcast) {
                     ...
                     */
                    int port_in, pool;
                    port_in = index / _TH_MMU_NUM_POOL;
                    pool = index % _TH_MMU_NUM_POOL;
                    p_idx = pool * _TH_MMU_PORTS_PER_PIPE + port_in;
                }
                SOC_IF_ERROR_RETURN(
                    soc_mem_read(unit, mem, MEM_BLOCK_ALL, p_idx, entry));
...
STATIC int
_bcm_bst_th_sync_report(int unit, bcm_bst_stat_id_t bid,
                        int port, int index, int flags, int xpe_id, int pipe_id)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
    ...
    int trigger_event = 0;
    int same_index = 0, trigger_times = 0;
        ...
        for (idx = 0; idx < resInfo->num_stat_pp; idx++) {
            ...
            if (bst_info->track_mode) {
                ...
                if ((resInfo->p_threshold[idx_offset] &&
                    ...
                    (flags & TH_BST_HW_EVENT_TRIGGER && (idx == index))) {
                    trigger_event = 1;
                }
            }
                ...
                for (idx = 0; idx < max_idx; idx++) {
                    ...
                    if (bst_info->track_mode) {
                        ...
                        if ((resInfo->p_threshold[idx_offset] &&
                            (resInfo->p_stat[idx_offset] >=
                             (resInfo->p_threshold[idx_offset]) * resInfo->threshold_gran)) ||
                             (flags & TH_BST_HW_EVENT_TRIGGER &&
                              same_index && (trigger_times == 1))) {
                            trigger_event = 1;
                        }
                    }
...
_bcm_bst_th_sync_hw_snapshot(int unit, bcm_bst_stat_id_t bid,
                             int port, int index)
{
    return _bcm_bst_th_sync_report(unit, bid, port, index, 0, -1, -1);
}
...
bcm_bst_th_init(int unit)
{
    ...
    bst_info->hw_stat_get = _bcm_bst_th_hw_stat_get;
    ...
    bst_info->hw_intr_cb_unregister = soc_th_hw_intr_cb_unregister;
    ...
    total_stat_count = total_thd_count; /* No per pipe copy */
    _BCM_BST_STAT_INFO(pres, stat_mem, stat_reg, stat_field);
    _BCM_BST_RESOURCE_STAT_INIT_64(pres, total_stat_count, "bst RQE queue");
    pres->index_min = 0;
    pres->index_max = total_stat_count - 1;
    ...
    total_stat_count = total_thd_count; /* No per pipe copy */
    _BCM_BST_STAT_INFO(pres, stat_mem, stat_reg, stat_field);
    _BCM_BST_RESOURCE_STAT_INIT_64(pres, total_stat_count, "bst RQE pool");
    pres->index_min = 0;
    pres->index_max = total_stat_count - 1;
    ...
    for (bid = _BCM_BST_RESOURCE_MIN; bid < _BCM_BST_RESOURCE_MAX; bid++) {
        ...
        if (reg != INVALIDr) {
            ...
            for (xpe = 0; xpe < resInfo->num_instance; xpe++) {
                ...
                desc[mi].blk = blk;
                desc[mi].width = TH_BST_REG_WIDTH(unit, reg);
            ...
            for (xpe = 0; xpe < NUM_XPE(unit); xpe++) {
                for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                    ...
                    idx_count = (resInfo->index_max + 1) / resInfo->num_instance / NUM_PIPE(unit);
                    desc[mi].mem = mem;
    ...
    for (bid = _BCM_BST_RESOURCE_MIN; bid < _BCM_BST_RESOURCE_MAX; bid++) {
        ...
        if (reg != INVALIDr) {
            for (xpe = 0; xpe < resInfo->num_instance; xpe++) {
                ...
                width = TH_BST_REG_WIDTH(unit, reg);
    ...
    if (reg != INVALIDr) {
        ...
        for (idx = 0; idx < resInfo->num_stat_pp; idx++) {
            ...
            if ((SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)) &&
                (reg == MMU_THDR_DB_BST_STAT_PRIQr ||
                 reg == MMU_THDR_DB_BST_STAT_SPr)) {
                uint64 rval64;

                COMPILER_64_ZERO(rval64);
                width = 2;
                idx_offset = idx + pipe_offset;
                sal_memcpy(&rval64, buff, width * 4);
                temp_val = soc_reg64_field32_get(unit, reg, rval64, resInfo->stat_field);
            } else {
