                                  ...
                                  uint16 *ext_ctrl_info_count)
{
    ...
    *ext_ctrl_info_count = 0;
    for (part = 0; part < parts_count; part++) {
        ...
        if (fg->ext_codes[part].udf_sel != -1) {
            (*ext_ctrl_info_count)++;
        }
        if (fg->ext_codes[part].dst_cont_a_sel != -1) {
            (*ext_ctrl_info_count)++;
        }
        if (fg->ext_codes[part].dst_cont_b_sel != -1) {
            (*ext_ctrl_info_count)++;
        }
                               ...
                               bcmi_keygen_cfg_t *keygen_cfg)
{
    ...
    for (part = 0; part < parts_count; part++) {
        ...
        if (fg->ext_codes[part].udf_sel != -1) {
            ...
            ext_ctrl_info[idx].ctrl_sel_val = fg->ext_codes[part].udf_sel;
            ext_ctrl_info[idx].part = part;
            idx++;
        }
        if (fg->ext_codes->dst_cont_a_sel != -1) {
            ext_ctrl_info[idx].ctrl_sel = BCMI_KEYGEN_EXT_CTRL_SEL_DST_A;
            ext_ctrl_info[idx].ctrl_sel_val = fg->ext_codes[part].dst_cont_a_sel;
            ext_ctrl_info[idx].part = part;
            idx++;
        }
        if (fg->ext_codes->dst_cont_b_sel != -1) {
            ext_ctrl_info[idx].ctrl_sel = BCMI_KEYGEN_EXT_CTRL_SEL_DST_B;
            ext_ctrl_info[idx].ctrl_sel_val = fg->ext_codes[part].dst_cont_b_sel;
            ext_ctrl_info[idx].part = part;
            idx++;
        }
            ...
            switch (fsm_ptr->mode) {
                case bcmFieldGroupModeSingle:
                    ...
                    if (0 == (fsm_ptr->flags & _BCM_FP_GROUP_ADD_INTRA_SLICE_ONLY)) {
                        LOG_VERBOSE(BSL_LS_BCM_FP, (BSL_META_U(unit,
                                        "Trying intra...\n")));
                        if (fsm_ptr->aset_size <= EM_MODE128_ACTION_DATA_SIZE) {
                            fg->em_mode = _FieldExactMatchMode128;
                        } else if ((fsm_ptr->aset_size <= EM_MODE160_ACTION_DATA_SIZE)) {
                            fg->em_mode = _FieldExactMatchMode160;
                        } else {
                            fsm_ptr->rv = (BCM_E_CONFIG);
                        }
                        if (BCM_E_CONFIG != fsm_ptr->rv) {
                            fg->flags = orig_group_flags | _FP_GROUP_SPAN_SINGLE_SLICE
                                                         | _FP_GROUP_INTRASLICE_DOUBLEWIDE;
                            fsm_ptr->rv = _field_th_ext_code_assign(unit, 1, fsm_ptr);
                            fg->flags = orig_group_flags | _FP_GROUP_SPAN_SINGLE_SLICE;
