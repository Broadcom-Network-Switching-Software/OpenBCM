...
_bcm_esw_vxlan_sync(int unit)
{
    ...
    /*
     * Since we can't recover match_etag_vlan from HW if we add this vxlan
     * port to VPLAG, so we need to store it for warmboot recovery.
     */
    for (i = 0; i < num_vp; i++) {
        sal_memcpy(vxlan_state, &(vxlan_info->match_key[i].match_etag_vlan),
                   sizeof(bcm_vlan_t));
        vxlan_state += sizeof(bcm_vlan_t);
    }
