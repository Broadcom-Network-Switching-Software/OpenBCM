...
#define _BCM_FLOW_L2_TUNNEL_ID_IF_INVALID_RETURN(unit, id) \
    do {                                                 \
                            ...
                            int  *index)
{
    ...
    /* End of Match */
        ...
        *mem = mem_id;
        ...
        _BCM_FLOW_L2_TUNNEL_ID_IF_INVALID_RETURN(unit, soft_tnl_idx);
                            ...
                            int  *index)
{
    ...
    if (info->function_type == bcmFlowFuncTypeL3Route) {
        ...
        if (info->encap_criteria == BCM_FLOW_ENCAP_CRITERIA_DVP) {
        ...
        } else if (info->encap_criteria == BCM_FLOW_ENCAP_CRITERIA_VLAN_PORT_GROUP) {
            if (!SOC_MEM_FIELD_VALID(unit, mem_view_id, GPP_ATTRIBUTES_VT_PORT_GROUP_IDf)) {
                 return BCM_E_PARAM;
            }
            soc_mem_field32_set(unit, mem_view_id, entry, GPP_ATTRIBUTES_VT_PORT_GROUP_IDf,
                           info->port);
            if (info->vlan >= BCM_VLAN_INVALID) {
                return  BCM_E_PARAM;
            }
            if (SOC_MEM_FIELD_VALID(unit, mem_view_id, OUTER_VLAN_IDf)) {
                soc_mem_field32_set(unit, mem_view_id, entry, OUTER_VLAN_IDf, info->vlan);
            } else {
                return BCM_E_PARAM;
            }
        } else if (info->encap_criteria == BCM_FLOW_ENCAP_CRITERIA_DGPP_VLAN ) {
            if (BCM_GPORT_IS_MODPORT(info->flow_port)) {
                ...
                (_SHR_GPORT_MODPORT_MODID_GET(info->flow_port) << 8 |
            ...
            if (info->valid_elements & BCM_FLOW_STAT_PORT_VALID) {
                BCM_IF_ERROR_RETURN(
                   _bcm_esw_gport_resolve(unit, info->port, &mod_out,
                        &port_out, &trunk_id, &gport_id));
                if (BCM_GPORT_IS_TRUNK(info->port)) {
                    if (SOC_MEM_FIELD_VALID(unit, mem_view_id, SGLP_16f &&
                        SOC_MEM_FIELD_VALID(unit, mem_view_id, SGLP_15_0f))) {
                        soc_mem_field32_set(unit, mem_view_id, entry, SGLP_16f, 1);
                        soc_mem_field32_set(unit, mem_view_id, entry, SGLP_15_0f,trunk_id);
                    } else if (!(SOC_MEM_FIELD_VALID(unit, mem_view_id, t_f) &&
                    ...
                    if (mod_out != -1) {
                        if (SOC_MEM_FIELD_VALID(unit, mem_view_id, SGLP_15_0f)) {
                            soc_mem_field32_set(unit, mem_view_id, entry, SGLP_15_0f,
                                            port_out | (mod_out << 8));
                        } else if (!SOC_MEM_FIELD_VALID(unit, mem_view_id, module_id_f)) {
                            soc_mem_field32_set(unit, mem_view_id, entry, module_id_f,
                                                mod_out);
                        }
                    }
                    if (SOC_MEM_FIELD_VALID(unit, mem_view_id, port_num_f)) {
                        soc_mem_field32_set(unit, mem_view_id, entry, port_num_f, port_out);
                    }
                }
            }
            if (SOC_MEM_FIELD_VALID(unit,  mem_view_id, SOURCE_TYPEf)) {
                soc_mem_field32_set(unit, mem_view_id, entry, SOURCE_TYPEf, 1);
            }
        }
        if (info->match_criteria == BCM_FLOW_MATCH_CRITERIA_PORT_VLAN ) {
            if (SOC_MEM_FIELD_VALID(unit, mem_view_id, OTAGf)) {
        ...
        } else if (info->match_criteria == BCM_FLOW_MATCH_CRITERIA_PORT_GROUP_VLAN) {
                if (!SOC_MEM_FIELD_VALID(unit, mem_view_id, PORT_GROUP_IDf)) {
                    return BCM_E_PARAM;
                }
                soc_mem_field32_set(unit, mem_view_id, entry, PORT_GROUP_IDf, info->port);
                if (!SOC_MEM_FIELD_VALID(unit, mem_view_id, OTAGf)) {
                    return BCM_E_PARAM;
                }
                soc_mem_field32_set(unit, mem_view_id, entry, OTAGf, info->vlan);
