...
typedef struct evxlt_key_map_s {
        char *name;   /* cancun ceh defined edit_ctrl_id names */
        int  evxlt_key;    /* bcm_vlan_translate_egress_key_t enum */
} evxlt_key_map_t;

/* Derive vxlt_control_id for egr_dvp_attribute table */
static evxlt_key_map_t evxlt_key_map[] = {
    {"EVXLT_1_DVP_GROUP_VFI_EVXLT_2_DVP_CLASS_ID",
      bcmVlanTranslateEgressKeyVpnGportGroupKey2Gport},

    {"EVXLT_1_DVP_GROUP_VFI_EVXLT_2_DVP_CLASS_ID_EGR_VFI",
      bcmVlanTranslateEgressKeyVpnGportGroupKey2VpnGport},

    {"EVXLT_1_DVP_GROUP_VFI_EVXLT_2_EGR_VFI",
      bcmVlanTranslateEgressKeyVpnGportGroupKey2Vpn},

    {"EVXLT_1_DVP_VFI_EVXLT_2_DVP_CLASS_ID",
      bcmVlanTranslateEgressKeyVpnGportKey2Gport},

    {"EVXLT_1_DVP_VFI_EVXLT_2_DVP_CLASS_ID_EGR_VFI",
      bcmVlanTranslateEgressKeyVpnGportKey2VpnGport},

    {"EVXLT_1_DVP_VFI_EVXLT_2_EGR_VFI",
      bcmVlanTranslateEgressKeyVpnGportKey2Vpn},

    {"EVXLT_1_NOOP_EVXLT_2_DVP_CLASS_ID",
      bcmVlanTranslateEgressKey2Gport},

    {"EVXLT_1_NOOP_EVXLT_2_DVP_CLASS_ID_EGR_VFI",
      bcmVlanTranslateEgressKey2VpnGport},

    {"EVXLT_1_NOOP_EVXLT_2_EGR_VFI",
      bcmVlanTranslateEgressKey2Vpn},

    {"EVXLT_1_VFI_EVXLT_2_DVP_CLASS_ID",
      bcmVlanTranslateEgressKeyVpnKey2Gport},

    {"EVXLT_1_VFI_EVXLT_2_DVP_CLASS_ID_EGR_VFI",
      bcmVlanTranslateEgressKeyVpnKey2VpnGport},

    {"EVXLT_1_VFI_EVXLT_2_EGR_VFI",
      bcmVlanTranslateEgressKeyVpnKey2Vpn},
};
#endif
...
/*
 * Function:
 *      bcmi_esw_flow_dvp_vxlt_ctrl_get
 * Purpose:
 *      Retrieve the EVXLT_CONTROL_ID value defined in CEH for a given evxlt_key_type.
 * Parameters:
 *      unit           - (IN) SOC unit number.
 *      evxlt_key_type - (IN) evxlt key select passed from API.
 *      vxlt_ctrl_id   - (OUT) EVXLT_CONTROL_ID value.
 * Returns:
 *      BCM_E_XXX
 * NOTE:
 */
STATIC int
bcmi_esw_flow_dvp_vxlt_ctrl_get(int unit,
    int evxlt_key_type,
    int *vxlt_ctrl_id)
{
    int idx;
    soc_cancun_ceh_field_info_t ceh_info;
    for (idx = 0; idx < COUNTOF(evxlt_key_map); idx++) {
        if (evxlt_key_type == evxlt_key_map[idx].evxlt_key) {
            BCM_IF_ERROR_RETURN
                (soc_cancun_ceh_obj_field_get(unit,
                 "EGR_DVP_ATTRIBUTE_VXLT_CONTROL_ID",
                  evxlt_key_map[idx].name, &ceh_info));
            *vxlt_ctrl_id = ceh_info.value;
            break;
        }
    }
    if (idx == COUNTOF(evxlt_key_map)) {
        /* not found */
...
/*
 * Function:
 *      bcmi_esw_flow_dvp_vxlt_type_get
 * Purpose:
 *      Retrieve the evxlt_key_type for a given EVXLT_CONTROL_ID value defined in CEH.
 * Parameters:
 *      unit           - (IN) SOC unit number.
 *      vxlt_ctrl_id   - (IN) EVXLT_CONTROL_ID value.
 *      evxlt_key_type - (OUT) evxlt key select passed from API.
 * Returns:
 *      BCM_E_XXX
 * NOTE:
 */
STATIC int
bcmi_esw_flow_dvp_vxlt_type_get(int unit,
    int vxlt_ctrl_id,
    int *evxlt_key_type)
{
    int idx;
    ...
    for (idx = 0; idx < COUNTOF(evxlt_key_map); idx++) {
        BCM_IF_ERROR_RETURN
            (soc_cancun_ceh_obj_field_get(unit,
                 "EGR_DVP_ATTRIBUTE_VXLT_CONTROL_ID",
                 evxlt_key_map[idx].name, &ceh_info));
        if (vxlt_ctrl_id == ceh_info.value) {
            *evxlt_key_type = evxlt_key_map[idx].evxlt_key;
            break;
        }
    }
    if (idx == COUNTOF(evxlt_key_map)) {
                                     ...
                                     int key_type)
{
    ...
    if (data_type == _BCM_FLOW_VXLAN_EGRESS_DEST_VP_TYPE) {  
       ...
       /* flex type */
        /* use vxlt_control_id */
        LOG_INFO(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
          "DVP %d is for flex flow\n"), vp));
    } else if (data_type == _BCM_FLOW_DATA_TYPE_COMMON) { 
        evxlt_field_id = COMMON__EVXLT_KEY_SELf;
    }

#if defined(BCM_TRIDENT3_SUPPORT)
        /* Get VXLT_CONTROL_ID for flex view of EGR_DVP_ATTRIBUTE table
         * programming for VXLAN GBP flex access flows
         */
        if (soc_feature(unit, soc_feature_vxlan_gbp_flex) &&
            ((key_type >= bcmVlanTranslateEgressKeyVpnGportGroupKey2Gport) &&
             (key_type <= bcmVlanTranslateEgressKeyVpnKey2Vpn))) {
            BCM_IF_ERROR_RETURN
                (bcmi_esw_flow_dvp_vxlt_ctrl_get(unit,key_type,
                                                 &key_type_value));
            BCM_IF_ERROR_RETURN
                (soc_flow_db_mem_to_view_id_get(unit,
                                                ...
                                                (uint32 *)&mem_view_id));
            /* View is flex view */
            if (SOC_MEM_FIELD_VALID(unit, mem_view_id, VXLT_CONTROL_IDf)) {
                evxlt_field_id = VXLT_CONTROL_IDf;
            }
        } else
#endif
    {
    ...
    if (evxlt_field_id != 0) {
        egr_key_type_value = soc_mem_field32_get(unit, mem_view_id, &egr_dvp_attribute,
                                        evxlt_field_id);
       
        if (key_type_value != egr_key_type_value) {
            soc_mem_field32_set(unit, mem_view_id, &egr_dvp_attribute,
                            evxlt_field_id, key_type_value);
            rv = soc_mem_write(unit, mem_view_id, MEM_BLOCK_ANY,
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(unit, soc_feature_vxlan_gbp_flex)) {
        /* Choose VXLT_CONTROL_IDf from flex view of EGR_DVP_ATTRIBUTE table
         * for VXLAN GBP flex access flows
         */
            soc_mem_t mem_view_id;
            BCM_IF_ERROR_RETURN
                (soc_flow_db_mem_to_view_id_get(unit,
            ...
            if (SOC_MEM_FIELD_VALID(unit, mem_view_id, VXLT_CONTROL_IDf)) {
                evxlt_field_id = VXLT_CONTROL_IDf;
                egr_key_type_value = soc_mem_field32_get(unit, mem_view_id,
                                         &egr_dvp_attribute, evxlt_field_id);
                rv = bcmi_esw_flow_dvp_vxlt_type_get(unit,
        ...
        if (info->valid_elements & BCM_FLOW_PORT_ENCAP_DVP_GROUP_VALID) {
            if (SOC_MEM_FIELD_VALID(unit,mem_id,VT_DVP_GROUP_IDf)) {
                soc_mem_field32_set(unit, mem_id, entry, VT_DVP_GROUP_IDf,
                        info->dvp_group);
            } else if (SOC_MEM_FIELD_VALID(unit,mem_id,VXLT_CLASS_IDf)) {
                soc_mem_field32_set(unit, mem_id, entry, VXLT_CLASS_IDf,
                                    info->dvp_group);
            } else {
                LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
                     "VT_DVP_GROUP_IDf/VXLT_CLASS_IDf not defined in memory view\n")));
                return BCM_E_PARAM;
            }
        }
    ...
    if (!_BCM_FLOW_IS_FLEX_VIEW(info)) {
        ...
        if (info->valid_elements & BCM_FLOW_PORT_ENCAP_CLASS_ID_VALID) {
            if (SOC_MEM_FIELD_VALID(unit,mem,CLASS_ID_ACTION_SETf)) {
                soc_mem_field32_set(unit, mem, entry, CLASS_ID_ACTION_SETf,
                        info->class_id);
            } else {
                LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
                                "CLASS_ID_ACTION_SETf not defined in memory view\n")));
        ...
        if (info->valid_elements & BCM_FLOW_PORT_ENCAP_DVP_GROUP_VALID) {
            if (SOC_MEM_FIELD_VALID(unit,mem,VXLT_CLASS_IDf)) {
                soc_mem_field32_set(unit, mem, entry, VXLT_CLASS_IDf,
                        info->dvp_group);
            } else {
                LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
...
_bcm_flow_linked_tbl_sd_tag_set(int unit, soc_mem_t view_id, 
              uint32 *egr_nh, uint32 sd_tag, bcm_flow_port_encap_t *pe_info)
{
    ...
    if (next_ptr_type == 3 || /* VC_AND_SWAP */
        next_ptr_type == BCMI_TD3_L3_NH_EGR_NEXT_PTR_TYPE_DVP_AND_VC_SWAP) {
    ...
    if (mem) {
        ...
        if (SOC_FAILURE(rv) && (rv != SOC_E_NOT_FOUND)) {
            ...
            if (srv6_handle && pe_info->flow_handle == srv6_handle) {
                rv = bcm_esw_flow_option_id_get(unit, srv6_handle,
                         "L2_OUTER_DIP_MSB_BITS_AND_PAYLOAD_OTAG_ACTIONS",
                         &vc_swap_option);
                BCM_IF_ERROR_RETURN(rv);
                /* initialize the vc_and_swap entry with all control fields */
                rv = soc_flow_db_ffo_to_mem_view_id_get(unit, srv6_handle,
                                   vc_swap_option,
                                   SOC_FLOW_DB_FUNC_EGRESS_LABEL_ID,
                                   &nptr_view_id);
                BCM_IF_ERROR_RETURN(rv);
                soc_flow_db_mem_view_entry_init(unit, nptr_view_id, nptr_entry);
            } else {
                return rv;
            }
        }
        ...
        if (SOC_MEM_FIELD_VALID(unit, nptr_view_id,
                                      TAG_ACTION_PROFILE_ACTION_SETf)) {
            action_present =
                soc_format_field32_get(unit, SD_TAG_ACTION_SETfmt,
                           &sd_tag, SD_TAG_ACTION_IF_PRESENTf);
            action_not_present =
                soc_format_field32_get(unit, SD_TAG_ACTION_SETfmt,
                           &sd_tag, SD_TAG_ACTION_IF_NOT_PRESENTf);
            profile_index = soc_mem_field32_get(unit, nptr_view_id, nptr_entry,
                                   TAG_ACTION_PROFILE_ACTION_SETf);
            _bcm_td3_sd_tag_action_profile_get(unit, action_present,
                               action_not_present, &profile_index, 0, 0);
            soc_mem_field32_set(unit, nptr_view_id, nptr_entry,
                   TAG_ACTION_PROFILE_ACTION_SETf, profile_index);
            vlan = soc_format_field32_get(unit, SD_TAG_ACTION_SETfmt,
                           &sd_tag, SD_TAG_VIDf);
            if (SOC_MEM_FIELD_VALID(unit, nptr_view_id, OTAG_ACTION_SETf)) {
                soc_mem_field32_set(unit, nptr_view_id, nptr_entry,
                                    OTAG_ACTION_SETf, vlan);
            }
            rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL, next_ptr, nptr_entry);
        }
        soc_mem_unlock(unit,mem);
    ...
    *sd_tag = 0;
    ...
    if (mem) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, nptr_view_id,
            ...
            *sd_tag = 0;
            soc_format_field32_set(unit, SD_TAG_ACTION_SETfmt,
                           sd_tag, SD_TAG_ACTION_IF_PRESENTf, action_present);
            soc_format_field32_set(unit, SD_TAG_ACTION_SETfmt,
                    sd_tag, SD_TAG_ACTION_IF_NOT_PRESENTf, action_not_present);
        }
              ...
              bcm_flow_port_encap_t *pe_info, int is_network_port)
{
        ...
        if (BCMI_L3_NH_FLEX_VIEW(data_type)) {
            ...
            if (soc_feature(unit, soc_feature_vxlan_decoupled_mode)) {
                ...
                if (data_type != _BCM_VXLAN_EGR_NEXT_HOP_L2OTAG_VIEW) {
                    ...
                    bcm_module_t sd_tag_modid = BCM_MODID_INVALID;
                    int sd_tag_chg_dst = -1;
                    bcm_port_t sd_tag_port = BCM_PORT_INVALID;
                    bcm_gport_t sd_tag_gport = BCM_GPORT_INVALID;

                    bcm_xflow_macsec_port_info_t_init(&port_info);

                    if (data_type == _BCM_VXLAN_EGR_NEXT_HOP_SDTAG_VIEW) {
                        sd_tag_chg_dst = soc_EGR_L3_NEXT_HOPm_field32_get(unit, egr_nh,
                                SD_TAG__HG_CHANGE_DESTINATIONf);
                        sd_tag_modid = soc_EGR_L3_NEXT_HOPm_field32_get(unit, egr_nh,
                                SD_TAG__HG_MC_DST_MODIDf);
                        sd_tag_port = soc_EGR_L3_NEXT_HOPm_field32_get(unit, egr_nh,
...
#if defined(BCM_HURRICANE4_SUPPORT) && defined(INCLUDE_XFLOW_MACSEC)
                    if (sd_tag_chg_dst != -1) {
                        BCM_GPORT_MODPORT_SET(sd_tag_gport, sd_tag_modid, sd_tag_port);
                        bcm_common_xflow_macsec_port_info_get(unit, sd_tag_gport, &port_info);
                        if (port_info.enable) {
                            /* retain the macsec config from SD_TAG view to L2_OTAG view */
                            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                                    L2_OTAG__HG_CHANGE_DESTINATION_UCf, sd_tag_chg_dst);
                            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                                    L2_OTAG__HG_CHANGE_DESTINATION_MCf, sd_tag_chg_dst);
                            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                                    L2_OTAG__HG_DST_MODIDf, sd_tag_modid);
                            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                                    L2_OTAG__HG_DST_PORT_NUMf, sd_tag_port);
                        }
                    }
#endif
                }
    ...
    if (pe_info->valid_elements & BCM_FLOW_PORT_ENCAP_FLAGS_VALID) {
        ...
        if (pe_info->flags & BCM_FLOW_PORT_ENCAP_FLAG_SERVICE_TAGGED) {
            ...
            /* Reset the the current SD tag controls */
            if ((prev_data_type != 0) &&
                (data_type == _BCM_VXLAN_EGR_NEXT_HOP_L2OTAG_VIEW)) {
                int profile_index = 0;
                ...
                soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                            L2_OTAG__VIDf, 0);
                soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                            L2_OTAG__TAG_ACTION_PROFILE_PTRf, 0);
                soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                            L2_OTAG__NEXT_PTR_TYPEf, 0);
                soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, egr_nh,
                            L2_OTAG__HG_HDR_SELf, 0);
        ...
        uint32 *entry)
{
    ...
    /* VXLAN GBP access flow */
    if (soc_feature(unit, soc_feature_vxlan_gbp_flex)) {
        if ((key_type == _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_VFI) ||
            (key_type == _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_DVP_VFI) ||
            (key_type == _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_DVP_GROUP_VFI)) {
            soc_mem_field32_set(unit, mem, entry, DATA_TYPEf, 12);
        }
    }
...
/*
 * Function:
 *      _bcm_flow_vxlan_gbp_encap_entry_key_set
 * Purpose:
 *      Set flow encap Entry Key for VXLAN GBP flow
 * Parameters:
 *      unit - (IN) Device Number
...
STATIC
int _bcm_flow_vxlan_gbp_encap_entry_key_set(int unit,
    bcm_flow_encap_config_t *info,
    soc_mem_t mem_id,
    uint32 *entry,
    uint32 vfi,
    uint32 dvp)
{
    ...
    if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_DVP_GROUP) {
        vfi_f = VXLAN_GBP_DVP_GROUP_VFI__VFIf;
        dvp_grp_f = VXLAN_GBP_DVP_GROUP_VFI__VT_DVP_GROUP_IDf;
        ...
        vfi_f = VXLAN_GBP_DVP_VFI__VFIf;
        dvp_f = VXLAN_GBP_DVP_VFI__DVPf;
        if (info->valid_elements & BCM_FLOW_ENCAP_VPN_VALID) {
            if (!SOC_MEM_FIELD_VALID(unit,mem_id,vfi_f)) {
                return BCM_E_PARAM;
            }
    ...
    int network_port)
{
    ...
    if (soc_feature(unit, soc_feature_vxlan_gbp_flex)) {
        if (info->flags & BCM_FLOW_ENCAP_FLAG_VXLAN_GBP_FLEX_ACCESS) {
         rv = _bcm_flow_vxlan_gbp_encap_entry_key_set(unit, info,
                   mem_id, entry, vfi, dvp);
         return rv;
        }
    }
    ...
    } else if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VLAN_PORT_GROUP ) {
        if (info->valid_elements & BCM_FLOW_ENCAP_PORT_GROUP_VALID ) {
            if (!SOC_MEM_FIELD_VALID(unit,mem_id,GPP_ATTRIBUTES_VT_PORT_GROUP_IDf)) {
                return BCM_E_PARAM;
            }
            soc_mem_field32_set(unit, mem_id, entry, GPP_ATTRIBUTES_VT_PORT_GROUP_IDf,
                           info->port_group);
        } else {
            return BCM_E_PARAM;
        }
    ...
    } else if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_DGPP_VLAN ) {
        if (info->valid_elements & BCM_FLOW_ENCAP_FLOW_PORT_VALID) {
            if (BCM_GPORT_IS_MODPORT(info->flow_port)) {
        ...
        if (info->flags & BCM_FLOW_ENCAP_FLAG_VXLAN_GBP_FLEX_ACCESS) {
            ...
            *network_port = FALSE;
            if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI) {
                key_type = _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_VFI;
            } else if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_DVP) {
                key_type = _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_DVP_VFI;
            } else if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_DVP_GROUP) {
                key_type = _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_DVP_GROUP_VFI;
                         ...
                         uint32 *xlate_data)
{
    ...
    if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_DVP_GROUP ||
        ...
        ((info->flags & BCM_FLOW_ENCAP_FLAG_VXLAN_GBP_FLEX_ACCESS) &&
        ...
        if (info->valid_elements & (BCM_FLOW_ENCAP_VLAN_VALID |
            ...
            if (info->flags & BCM_FLOW_ENCAP_FLAG_VXLAN_GBP_FLEX_ACCESS) {
                if (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_REPLACE) {
                    if (info->valid_elements & BCM_FLOW_ENCAP_VLAN_VALID) {
                        action.ot_outer      = bcmVlanActionReplace;
                    }
                }
                if (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_PRI_REPLACE) {
                    if (info->valid_elements & BCM_FLOW_ENCAP_VLAN_VALID) {
                        action.ot_outer      = bcmVlanActionReplace;
                    }
                    if (info->valid_elements & BCM_FLOW_ENCAP_PKT_PRI_VALID) {
                ...
                if (info->flags & BCM_FLOW_ENCAP_FLAG_VLAN_DELETE) {
...
/*
 * Function:
 *      _bcm_flow_vxlan_gbp_vxlt_data_field_get
 * Purpose:
 *      Get the vxlt data field to be used.
 * Parameters:
 *      unit  - (IN) Device Number
 *      mem   - (IN) memory ID
 *      info  - (IN) Flow encap config descriptor
 *      xlate_data_field - (OUT) vxlt data field
 * Returns:
 *      BCM_E_XXX
 */
STATIC
int _bcm_flow_vxlan_gbp_vxlt_data_field_get(int unit,
        soc_mem_t mem,
        bcm_flow_encap_config_t *info,
        soc_field_t *xlate_data_field)
{
    soc_field_t xlate_f = 0;
    if (info == NULL) {
        return BCM_E_PARAM;
    }
    switch (info->criteria) {
        case BCM_FLOW_ENCAP_CRITERIA_VFI_DVP_GROUP:
            xlate_f = VXLAN_GBP_DVP_GROUP_VFI__XLATE_DATAf;
            break;
        case BCM_FLOW_ENCAP_CRITERIA_VFI_DVP:
            xlate_f = VXLAN_GBP_DVP_VFI__XLATE_DATAf;
            break;
        case BCM_FLOW_ENCAP_CRITERIA_VFI:
            xlate_f = VXLAN_GBP_VFI__XLATE_DATAf;
            break;
        default:
            break;
    }
    if (soc_mem_field_valid(unit, mem, xlate_f)) {
        *xlate_data_field = xlate_f;
    } else {
        LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
                        "XLATE_DATAf not defined in memory view\n")));
        return BCM_E_INTERNAL;
    }
        ...
        void *key_data)
{
    ...
    if (soc_flow_db_mem_view_is_valid(unit, mem_view)){
        ...
        if (soc_mem_field_valid(unit, mem, tag_act_f)) {
            profile_index = soc_mem_field32_get(unit, mem, return_entry,
                    tag_act_f);
        }
        if (soc_feature(unit, soc_feature_vxlan_gbp_flex) &&
            (info->flags & BCM_FLOW_ENCAP_FLAG_VXLAN_GBP_FLEX_ACCESS)) {
            int xlate_format = EGR_VLAN_XLATE_XLATE_DATA_DOUBLEfmt;
            soc_field_t xlate_data_f = 0;
            uint32 xlate_data[3];
            sal_memset(xlate_data, 0, sizeof(xlate_data));
            /* Get the xlate data field to use */
            BCM_IF_ERROR_RETURN
                (_bcm_flow_vxlan_gbp_vxlt_data_field_get(
                     unit, mem, info, &xlate_data_f));

            soc_mem_field_get(unit, mem, return_entry,
                              xlate_data_f, xlate_data);
            if (soc_mem_field_valid(unit, mem, tag_act_f)) {
                profile_index = soc_format_field32_get(unit, xlate_format,
                        xlate_data, tag_act_f);
            }
        }
    }
        ...
        int network_port)
{
    ...
    if (info->action.ot_outer >= bcmVlanActionAdd &&
        info->action.ot_outer <= bcmVlanActionDelete) {
        if (info->action.dt_outer || info->action.dt_inner ||
            info->action.ot_inner || info->action.it_outer ||
            info->action.it_inner || info->action.ut_outer ||
            info->action.ut_inner) {
            return BCM_E_PARAM;
        }
        if (info->valid_elements & BCM_FLOW_ENCAP_VLAN_VALID) {
                if (info->action.new_outer_vlan >= BCM_VLAN_INVALID) {
                    return  BCM_E_PARAM;
                }
                soc_format_field32_set(unit, tag_act_fmt, &tag_act_set, vlan_f,
                                  info->action.new_outer_vlan);
        }
        if (info->valid_elements & BCM_FLOW_ENCAP_PKT_PRI_VALID) {
                if (info->action.priority > BCM_PRIO_MAX) {
                    return  BCM_E_PARAM;
                }
                soc_format_field32_set(unit, tag_act_fmt, &tag_act_set, pri_f,
                                  info->action.priority);
        }
        if (info->valid_elements & BCM_FLOW_ENCAP_PKT_CFI_VALID) {
                if (info->action.new_outer_cfi > 1) {
                    return  BCM_E_PARAM;
                }
                soc_format_field32_set(unit, tag_act_fmt, &tag_act_set, cfi_f,
                                  info->action.new_outer_cfi);
        }
        if ((info->valid_elements & BCM_FLOW_ENCAP_PKT_PRI_VALID) ||
            (info->valid_elements & BCM_FLOW_ENCAP_PKT_CFI_VALID)) {
            soc_format_field32_set(unit, tag_act_fmt, &tag_act_set, pri_cfi_sel_f, 1);
        }
        soc_mem_field32_set(unit, mem, entry, tag_act_fmt_f, tag_act_set);
        tag_profile_set = soc_mem_field32_get(unit, mem, entry,
                                             tag_profile_fmt_f);
        profile_index = soc_format_field32_get(unit, tag_profile_fmt,
                                          &tag_profile_set, tag_act_f);
        _bcm_td3_vlan_profile_get(unit, &(info->action), TRUE, &profile_index);
        soc_format_field32_set(unit, tag_profile_fmt, &tag_profile_set,
                          tag_act_f, profile_index);
        soc_mem_field32_set(unit, mem, entry, tag_profile_fmt_f, tag_profile_set);

        /* Modify Outer TPID for modify TPID action */
        if (info->action.outer_tpid_action == bcmVlanTpidActionModify) {
            tag_act_set = soc_mem_field32_get(unit, mem, entry, tag_act_fmt_f);
            old_tpid_inx = soc_format_field32_get(unit, tag_act_fmt, &tag_act_set, tpid_f);
            BCM_IF_ERROR_RETURN(_bcm_fb2_outer_tpid_entry_delete(unit,
                                old_tpid_inx));
            BCM_IF_ERROR_RETURN(_bcm_fb2_outer_tpid_entry_add(unit, info->action.outer_tpid,
                                &tpid_index));
            soc_format_field32_set(unit, tag_act_fmt, &tag_act_set, tpid_f, tpid_index);
            soc_mem_field32_set(unit, mem, entry, tag_act_fmt_f, tag_act_set);
        }
...
/*
 * Function:
 *      _bcm_flow_vxlan_gbp_encap_entry_parse
 * Description:
 *      Convert the VXLAN GBP memory entry field data to the flow match configuration
 *      parameters.
 * Parameters   :
 *      unit    - (IN) Device Number
 *      mem     - (IN)  memory id.
...
STATIC
int _bcm_flow_vxlan_gbp_encap_entry_parse(int unit,
        soc_mem_t mem_id, uint32 *entry,
        bcm_flow_encap_config_t *info)
{
    soc_field_t vfi_f = 0;
    soc_field_t dvp_f = 0;
    ...
    if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_DVP_GROUP) {
        ...
        if (info->valid_elements & BCM_FLOW_ENCAP_VPN_VALID) {
            ...
            vfi = soc_mem_field32_get(unit, mem_id, entry, vfi_f);
        ...
        if (info->valid_elements & BCM_FLOW_ENCAP_DVP_GROUP_VALID) {
            ...
            info->dvp_group = soc_mem_field32_get(unit,mem_id,
                                  entry, dvp_grp_f);
        }
    } else if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI) {
        vfi_f = VXLAN_GBP_VFI__VFIf;
        ...
        if (info->valid_elements & BCM_FLOW_ENCAP_FLOW_PORT_VALID) {
            ...
            dvp = soc_mem_field32_get(unit, mem_id, entry, dvp_f);
            _SHR_GPORT_FLOW_PORT_ID_SET(info->flow_port,dvp);
        }
    }
    BCM_IF_ERROR_RETURN(
        _bcm_flow_encap_entry_data_get(unit, info, NULL,
             0, mem_id, entry, FALSE));
                             ...
                             bcm_flow_logical_field_t *field)
{
        ...
        switch (key_type) {
            ...
            case _BCM_FLOW_ENCAP_KEY_TYPE_VLAN_XLATE_VFI:
                ...
                info->valid_elements |= BCM_FLOW_ENCAP_VPN_VALID |
                                        BCM_FLOW_ENCAP_FLOW_PORT_VALID;
                break;
            case _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_VFI:
                 /* VXLAN GBP access flow */
                info->criteria = BCM_FLOW_ENCAP_CRITERIA_VFI;
                info->valid_elements |= BCM_FLOW_ENCAP_VPN_VALID;
                info->flags |= BCM_FLOW_ENCAP_FLAG_VXLAN_GBP_FLEX_ACCESS;
            ...
            case _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_DVP_VFI:
                 /* VXLAN GBP access flow */
                info->criteria = BCM_FLOW_ENCAP_CRITERIA_VFI_DVP;
                info->valid_elements |= BCM_FLOW_ENCAP_VPN_VALID |
                                        BCM_FLOW_ENCAP_FLOW_PORT_VALID;
            ...
            case _BCM_FLOW_ENCAP_KEY_TYPE_VXLAN_GBP_DVP_GROUP_VFI:
                 /* VXLAN GBP access flow */
                info->criteria = BCM_FLOW_ENCAP_CRITERIA_VFI_DVP_GROUP;
                info->valid_elements |= BCM_FLOW_ENCAP_VPN_VALID |
                                        BCM_FLOW_ENCAP_DVP_GROUP_VALID;
    ...
    if (soc_feature(unit, soc_feature_vxlan_gbp_flex)) {
        if (info->flags & BCM_FLOW_ENCAP_FLAG_VXLAN_GBP_FLEX_ACCESS) {
         rv = _bcm_flow_vxlan_gbp_encap_entry_parse(unit, mem,
                                                    entry, info);
         return rv;
        }
    }
        ...
        int prune_enable)
{
    ...
    if (prune_enable) {
        /*
         * Remove the hardware entry if it exists to drop the traffic to support
         * prune_enable, if the entry does not exist, the removal can be
         * skipped.
         */
        sal_memset(entry, 0, sizeof(entry));
    }
