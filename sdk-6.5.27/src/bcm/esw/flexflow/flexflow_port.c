   ...
   /* ======== Allocate/Get Virtual_Port =============== */
    if (flow_port->flags & BCM_FLOW_PORT_REPLACE) {
       ...
       if (!BCM_GPORT_IS_FLOW_PORT(flow_port->flow_port_id)) {
        ...
        if (flow_port->flags & BCM_FLOW_PORT_DVP_TABLE_EXTENSION) {
            BCM_IF_ERROR_RETURN(bcmi_td3_dvp_vc_swap_entry_index_alloc(unit,
                 NULL, NULL, FALSE, &vp));
...
*/
    ...
    /* Free the VP */
    (void) _bcm_vp_free(unit, _bcmVpTypeFlow, 1, active_vp);
    return rv;
...
*/
    ...
    if (vp_info.flags & _BCM_VP_INFO_DVP_EXTENSION) {
        BCM_IF_ERROR_RETURN(bcmi_td3_dvp_vc_swap_entry_index_free(unit,
            NULL, 0, vp));
    } else {
...
_bcmi_esw_flow_port_get(int unit, bcm_vpn_t vpn, int vp, bcm_flow_port_t  *flow_port)
{
    ...
    if (vp_info.flags & _BCM_VP_INFO_DVP_EXTENSION) {
        flow_port->flags |= BCM_FLOW_PORT_DVP_TABLE_EXTENSION;
    }
...
/*
 * Function:
 *      bcm_td3_flow_port_learn_set
 * Purpose:
 *      Set the CML bits for a Flow port.
 * Returns:
 *      BCM_E_XXX
 */
int
_bcm_td3_flow_port_learn_set(int unit, bcm_gport_t flow_port_id,
                                  uint32 flags)
{
    ...
    /* Be sure the entry is used and is set for FLOW */
    if (!_bcm_vp_used_get(unit, vp, _bcmVpTypeFlow)) {
    ...
    /* Ensure that the entry_type is 1 for FLOW*/
    entry_type = soc_SOURCE_VPm_field32_get(unit, &svp, ENTRY_TYPEf);
    if (entry_type != 1){
        MEM_UNLOCK(unit, SOURCE_VPm);
        return BCM_E_NOT_FOUND;
...
/*
 * Function:
 *      _bcm_td3_flow_port_learn_get
 * Purpose:
 *      Get the CML bits for a Flow port
 * Returns:
 *      BCM_E_XXX
 */
int
_bcm_td3_flow_port_learn_get(int unit, bcm_gport_t flow_port_id,
                                  uint32 *flags)
{
