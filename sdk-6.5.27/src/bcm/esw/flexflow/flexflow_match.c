...
typedef struct _srv6_tnl_action_map_s {
        bcm_flow_tunnel_srv6_action_t action;
        char *name1;
        int  svp_profile;
} _srv6_tnl_action_map_t;
static _srv6_tnl_action_map_t _srv6_action_map[] =
{
    ...
    {bcmFlowTunnelSrv6ActionUsd,                 NULL}
};

STATIC
int _bcm_flow_match_srv6_key_ctrl_set(int unit,
    ...
    uint32 *entry)
{
    ...
    for (ix = 0; ix < COUNTOF(_srv6_action_map); ix++) {
        if (_srv6_action_map[ix].action == info->tunnel_action) {
            if (_srv6_action_map[ix].name1 != NULL) {
                rv = soc_cancun_ceh_obj_field_get(unit,
                         "UAT_EXTRACTION_CTRL_ID_ACTION_SET",
                         _srv6_action_map[ix].name1, &ceh_info);
    ...
    if (ix == COUNTOF(_srv6_action_map)) {
        LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
                  "Unsupported tunnel action ID: %d\n"),info->tunnel_action));
        return BCM_E_PARAM;
    }
    ...
    if (fwd_to_nh == FALSE) {
        if (info->tunnel_action == bcmFlowTunnelSrv6ActionTransitStdSid ||
            info->tunnel_action == bcmFlowTunnelSrv6ActionPsp) {
            soc_mem_field32_set(unit, mem_id, entry,
    ...
    if (info->tunnel_action == bcmFlowTunnelSrv6ActionUsd) {
        soc_mem_field32_set(unit, mem_id, entry, TERM_TUNNELf, 1);
    }
    return BCM_E_NONE;
}

STATIC
int _bcm_flow_match_srv6_ctrl_action_get(int unit,
    bcm_flow_match_config_t *info,
    soc_mem_t mem_id,
    uint32 *entry)
{
    ...
    action_value = soc_mem_field32_get(unit, mem_id, entry,
                       UAT_EXTRACTION_CTRL_ID_ACTION_SETf);
    ...
    for (ix = 0; ix < COUNTOF(_srv6_action_map); ix++) {
        ...
        if (action_value == ceh_info.value) {
            info->tunnel_action = _srv6_action_map[ix].action;
            break;
        }
    }
    info->flow_port = 0;
        ...
        int *result)
{
    ...
    *result = FALSE;
    ...
    rv = bcm_esw_flow_logical_field_id_get(unit, info->flow_handle,
            "GBP_SID", &lfield.id);
    ...
    if (info->valid_elements & BCM_FLOW_MATCH_FLEX_DATA_VALID) {
        BCM_IF_ERROR_RETURN(
              soc_flow_db_mem_view_field_list_get(unit, mem,
                  SOC_FLOW_DB_FIELD_TYPE_LOGICAL_POLICY_DATA,
                 _BCM_FLOW_LOGICAL_FIELD_MAX, data_ids, &cnt));
        for (ix = 0; ix < COUNTOF(gbp_handle); ix++) {
            rv = bcm_esw_flow_handle_get(unit,gbp_handle[ix], &gbp_flow_handle);
            if (BCM_SUCCESS(rv) && (info->flow_handle == gbp_flow_handle)) {
                for (i = 0; i < cnt; i++) {
                    if (data_ids[i] == lfield.id) {
                        *result = TRUE;
                        return rv;
                    }
                }
        ...
        if (info->valid_elements & BCM_FLOW_MATCH_FLEX_DATA_VALID) {
            if (SOC_MEM_FIELD_VALID(unit, sgpp_view_id, IDf)) {
                ...
                if (SOC_IS_TRIDENT3(unit)) {
                    rv = _gbp_handle_option_check(unit,sgpp_view_id,
                                                  info, &result);
                    if (BCM_FAILURE(rv)) {
                        soc_mem_unlock(unit,mem);
                        return rv;
                        ...
                        if (soc_fld) {
                            if (!clear_action) {
                                rv = soc_reg_field32_modify(unit, ING_FLEX_CONFIGr,
                                        REG_PORT_ANY, soc_fld, 1);
                                if (BCM_FAILURE(rv)) {
                                    soc_mem_unlock(unit,mem);
                                    return rv;
                                }
                                flow_info->gbp_ref_cnt++;
                            } else {
                                /* Clear the CCH register only if refcount is 1 */
                                if (flow_info->gbp_ref_cnt == 1) {
                                    rv = soc_reg_field32_modify(unit, ING_FLEX_CONFIGr,
                                            REG_PORT_ANY, soc_fld,  0);
                                    if (BCM_FAILURE(rv)) {
                                        soc_mem_unlock(unit,mem);
                                        return rv;
                                    }
                                }
                                if (flow_info->gbp_ref_cnt) {
                                    flow_info->gbp_ref_cnt--;
                                }
                            }
                        }
        ...
        int *replaced_vp)
{
    ...
    if (info->action.ot_outer >= bcmVlanActionAdd &&
        ...
        if (SOC_MEM_FIELD_VALID(unit, mem, OVLAN_ACTION_SETf)) {
                ...
                action_set = 0;
                soc_format_field32_set(unit, OVLAN_ACTION_SETfmt,
                        &action_set, VLAN_CFIf, info->action.new_outer_cfi);
                soc_format_field32_set(unit, OVLAN_ACTION_SETfmt,
                        &action_set, VLAN_PRIf , info->action.priority);
                soc_format_field32_set(unit, OVLAN_ACTION_SETfmt,
                        &action_set, VLAN_IDf, info->action.new_outer_vlan);
                soc_mem_field32_set(unit, mem, entry, OVLAN_ACTION_SETf,
                        action_set);
                ...
                if (SOC_MEM_FIELD_VALID(unit, mem, TAG_ACTION_SETf)) {
                    ...
                    profile = soc_format_field32_get(unit, TAG_ACTION_SETfmt,
                                      &action_set, ING_TAG_ACTION_PROFILE_IDXf);
                    _bcm_td3_vlan_profile_get(unit, &(info->action), FALSE, &profile);
                    soc_format_field32_set(unit, TAG_ACTION_SETfmt,
                            &action_set, ING_TAG_ACTION_PROFILE_IDXf, profile);
                    soc_mem_field32_set(unit, mem, entry, TAG_ACTION_SETf,
                            action_set);
        ...
        uint32 *entry)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit,mem,CLASS_ID_ACTION_SETf)) {
        ...
        if (info->class_id) {
            info->valid_elements |= BCM_FLOW_MATCH_CLASS_ID_VALID;
        }
    }

    if (flow_info->flow_srv6 && flow_info->flow_srv6 == info->flow_handle) {
        rv = _bcm_flow_match_srv6_ctrl_action_get(unit, info, mem, entry);
        BCM_IF_ERROR_RETURN(rv);
    }
    ...
    uint32 *entry)
{
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_PORT_VLAN ||
        ...
        if (info->valid_elements & BCM_FLOW_MATCH_PORT_VALID) {
            if (BCM_GPORT_IS_TRUNK(info->port)) {
                if (!(SOC_MEM_FIELD_VALID(unit, mem_id, t_f) &&
                    ...
                    !(SOC_MEM_FIELD_VALID(unit,mem_id,SGLP_16f) &&
                    SOC_MEM_FIELD_VALID(unit,mem_id,SGLP_15_0f))) {
                    return BCM_E_PARAM;
                }
                if (SOC_MEM_FIELD_VALID(unit,mem_id,SGLP_16f &&
                    SOC_MEM_FIELD_VALID(unit,mem_id,SGLP_15_0f))) {
                    soc_mem_field32_set(unit,mem_id, entry, SGLP_16f, 1);
                    soc_mem_field32_set(unit,mem_id, entry, SGLP_15_0f,trunk_id);
                } else {
                ...
                if (mod_out != -1) {
                    ...
                    if (SOC_MEM_FIELD_VALID(unit,mem_id,SGLP_15_0f)) {
                ...
                if (!SOC_MEM_FIELD_VALID(unit, mem_id, port_num_f) &&
                    !((mod_out != -1) & SOC_MEM_FIELD_VALID(unit,mem_id,SGLP_15_0f))) {
                    return BCM_E_PARAM;
                }
                if (SOC_MEM_FIELD_VALID(unit, mem_id, port_num_f)) {
        ...
        if (!SOC_MEM_FIELD_VALID(unit,mem_id,SOURCE_TYPEf) && !(BCM_GPORT_IS_TRUNK(info->port))
            && !((mod_out != -1) & SOC_MEM_FIELD_VALID(unit,mem_id,SGLP_15_0f))) {
            return BCM_E_PARAM;
        }
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_PORT_VLAN ) {
        if (info->valid_elements & BCM_FLOW_MATCH_OTAG_VALID) {
        ...
        if (flow_info->flow_srv6 && flow_info->flow_srv6 == info->flow_handle) {
            ...
            if (info->valid_elements & BCM_FLOW_MATCH_NEXT_HDR_VALID) {
                rv = bcm_esw_flow_logical_field_id_get(unit,
                          flow_info->flow_srv6, "SRH_NEXT_HDR", &lfield.id);
                BCM_IF_ERROR_RETURN(rv);
                soc_mem_field32_set(unit, mem_id, entry, lfield.id,
                                    info->next_hdr);
            }
            if (info->valid_elements & BCM_FLOW_MATCH_VLAN_VALID) {
                rv = bcm_esw_flow_logical_field_id_get(unit,
                           flow_info->flow_srv6, "PAYLOAD_OVID", &lfield.id);
                BCM_IF_ERROR_RETURN(rv);
        ...
        if (info->valid_elements & BCM_FLOW_MATCH_PORT_VALID) {
            if (!SOC_MEM_FIELD_VALID(unit, mem_id, PORT_GROUP_IDf)) {
                return BCM_E_PARAM;
            }
            soc_mem_field32_set(unit, mem_id, entry, PORT_GROUP_IDf, info->port);
        } else {
            return BCM_E_PARAM;
        }
        if (info->valid_elements & BCM_FLOW_MATCH_OTAG_VALID) {
    ...
    int key_ctrl)
{
    ...
    if (key_ctrl) {
        /* check if SRv6 */
       if (flow_info->flow_srv6 && flow_info->flow_srv6 == info->flow_handle) {
           rv = _bcm_flow_match_srv6_key_ctrl_set(unit, info, mem_id, entry);
           BCM_IF_ERROR_RETURN(rv);      
       }
    }
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_flow_match_key_set(unit,info,field,
                        num_of_fields,entry,&view_info, TRUE));
    ...
    /* update the iif reference counter */
        ...
        /* TD3 only check for GBP source strength selection,
         * should not include MV2 and HX5 */
        if (SOC_IS_TRIDENT3(unit) && (view_info.mem_view_id)) {
            BCM_IF_ERROR_RETURN(
                _gbp_handle_option_check(unit,
                     view_info.mem_view_id,
                     info,
                     &result));
        }
        if (result && (SOC_REG_FIELD_VALID(unit, ING_FLEX_CONFIGr,
                        LOWER_OPAQUE_1_STRENGTH_FROM_IFPf))) {
            BCM_IF_ERROR_RETURN(
                soc_reg_field32_modify(unit, ING_FLEX_CONFIGr,
                    REG_PORT_ANY, LOWER_OPAQUE_1_STRENGTH_FROM_IFPf, 1));
        }
    }
    if (!(info->options & BCM_FLOW_MATCH_OPTION_REPLACE)) {
        if (result) {
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    sal_memset(entry, 0, sizeof(entry));
    BCM_IF_ERROR_RETURN(_bcm_flow_match_key_set(unit,info,field,
                        num_of_fields,entry,&view_info, FALSE));
    if (view_info.mem == L3_TUNNELm) {
        rv = _bcm_flow_match_mem_entry_to_term_entry(unit, view_info.mem,
                    &key, entry);
        BCM_IF_ERROR_RETURN(rv);
        ...
        if (flow_info->gbp_ref_cnt == 1) {
            BCM_IF_ERROR_RETURN(
                soc_reg_field32_modify(
                    unit, ING_FLEX_CONFIGr,
                    REG_PORT_ANY,
                    LOWER_OPAQUE_1_STRENGTH_FROM_IFPf, 0));
        }
        if (flow_info->gbp_ref_cnt) {
            flow_info->gbp_ref_cnt--;
        }
                             ...
                             bcm_flow_logical_field_t *field)
{
    ...
    if (SOC_SUCCESS(rv)) {  /* flex view */
        /* tunnel termination shares the same hardware tables.
         * skip entries for termination
         */
        if ((SOC_MEM_FIELD_VALID(unit, mem_view_id, TERM_TUNNELf) &&
        ...
        if (_bcm_flow_key_is_valid(unit, mem, port_ovid_ids, 3)) {
            ...
            if (flow_info->flow_srv6 == info->flow_handle &&
                (flow_info->option_term_adapt1 == info->flow_option ||
                 ...
                 flow_info->option_psp_adapt4 == info->flow_option ||
                 flow_info->option_transit_adapt4 == info->flow_option ||
                 flow_info->option_psp_adapt2 == info->flow_option ||
                 flow_info->option_psp_adapt1  == info->flow_option)) {
                ...
                if (info->next_hdr && !info->vlan) {
                    info->criteria = BCM_FLOW_MATCH_CRITERIA_DIP_NHDR;
                    info->valid_elements |= BCM_FLOW_MATCH_NEXT_HDR_VALID;
                }
                if (info->next_hdr && info->vlan) {
                    info->criteria = BCM_FLOW_MATCH_CRITERIA_DIP_NHDR_OVID;
                    info->valid_elements |= BCM_FLOW_MATCH_NEXT_HDR_VALID;
