...
#define BCM_LINK_PBMP_SCACHE_NUM_WB_1_6 19

#define BCM_LINK_PBMP_SCACHE_NUM        BCM_LINK_PBMP_SCACHE_NUM_WB_1_6
...
_bcm_esw_link_sync(int unit)
{
    ...
    /* Introduced Fast link recovery WB ver 1.7 */
    _BCM_LINK_PBMP_SCACHE_COPY(sop->lc_pbm_flr_enable);

    /* Introduced Fast linkscan recovery timeout value ver 1.7 */
    sal_memcpy(link_scache_ptr, sop->lc_flr_timeout,
                       sizeof(uint32) * SOC_MAX_NUM_PORTS);
    link_scache_ptr += (sizeof(uint32) * SOC_MAX_NUM_PORTS);
...
_bcm_esw_link_reload(int unit)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        /* Introduced FLR port bitmap with WB ver 1.7 */
        if (recovered_ver >= BCM_WB_VERSION_1_7) {
            _BCM_LINK_PBMP_SCACHE_RECOVER(sop->lc_pbm_flr_enable);
            sal_memcpy(sop->lc_flr_timeout, link_scache_ptr,
                       sizeof(uint32) * SOC_SCACHE_SIZE_MIN(max_num_ports,
                                                            SOC_MAX_NUM_PORTS));

            link_scache_ptr += (sizeof(uint32) * max_num_ports);
        } else {
            realloc_size += ((BCM_LINK_PBMP_SCACHE_NUM_WB_1_6 -
                             BCM_LINK_PBMP_SCACHE_NUM_WB_1_5) *
                             sizeof(bcm_pbmp_t));
            realloc_size += (sizeof(uint32) * SOC_MAX_NUM_PORTS);
        }
...
#ifdef BCM_CMICX_SUPPORT
    /* Restore the CMICx link status. */
    if (soc_feature(unit, soc_feature_cmicx)) {
        BCM_PBMP_CLEAR(empty_pbmp);
        soc_linkscan_config(unit, lc->lc_pbm_hw, empty_pbmp);
    }
#endif
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        /* Only update front ports' LED status. */
        BCM_PBMP_AND(pbmp_port, PBMP_PORT_ALL(unit));

        PBMP_ITER(pbmp_port, port) {

            /* Skip if port is not front port. */
            ...
            if (soc_feature(unit, soc_feature_logical_port_num)) {
                ...
                led_link_sts.port = pport;
            }

            soc_cmicx_led_link_status(unit, &led_link_sts);
        }

        /* Recovery linkscan FLR setting. */
        SOC_IF_ERROR_RETURN
            (soc_linkscan_flr_wb_restore(unit));

        SOC_IF_ERROR_RETURN
            (soc_esw_portctrl_flr_wb_restore(unit));
    }
...
_bcm_esw_link_fault_get(int unit, int port, int *fault)
{
    ...
    if (IS_HG_PORT(unit, port) || IS_XE_PORT(unit, port) ||
        ...
        if (SOC_PORT_USE_PORTCTRL(unit, port)) {
            ...
            if (soc_feature(unit, soc_feature_cmicx)) {
                /* Get fault status after clearing old fault state.
                 * Clears intermittent faults received when ports are toggling
                 */
                BCM_IF_ERROR_NOT_UNAVAIL_RETURN
                    (bcmi_esw_portctrl_link_fault_get(unit, port,
                                                      &lcl_fault, &rmt_fault));
            ...
            /* Ignore the fault on TH3 FLR enabled ports. */
            if (SOC_IS_TOMAHAWK3(unit) &&
                SOC_PBMP_MEMBER(lc->lc_pbm_hw, port) &&
                SOC_PBMP_MEMBER(sop->lc_pbm_flr_enable, port)) {
                *fault = FALSE;
            }
...
_bcm_esw_linkscan_update_port(int unit, uint32 flags, int port)
{
    ...
    int port_enabled, autoneg, an_enabled, an_locked;
#endif
...
#if defined(BCM_TOMAHAWK3_SUPPORT) && defined(PORTMOD_SUPPORT)
    if (SOC_IS_TOMAHAWK3(unit)) {
        /* Don't allow linkscan to try to bring up a port that has been
         * disabled
         */
        BCM_IF_ERROR_RETURN(bcm_esw_port_enable_get(unit, port, &port_enabled));
        if (port_enabled == 0) {
            new_link = FALSE;
        }
    }
...
#if defined(BCM_TOMAHAWK3_SUPPORT) && defined(PORTMOD_SUPPORT)
    /* TH3's 400G autonegotiation sequence takes time to complete. Link up
     * can occur before autonegotiation has locked, so do not update the port
     * if link is coming up and autoneg has NOT locked yet
     */
    if (SOC_IS_TOMAHAWK3(unit)) {
        /* link transitioning from port down to port up */
        if (cur_link == 0 && new_link == 1) {
            ...
            if (autoneg == 1) {
                rv = bcmi_esw_portctrl_autoneg_status_get(
                                    unit, port, &an_enabled, &an_locked);
            }

            /* an is enabled, but not locked, skip link up for this port
             * for now
             */
            if ((an_enabled == 1) && (an_locked == 0)) {
                /* Indicate remote fault, since it technically is! Linkscan
                 * will rescan after linkscan interval
                 */
                BCM_PBMP_PORT_ADD(sop->lc_pbm_remote_fault, port);
                return BCM_E_NONE;
            }
        }
    }
...
#if defined (BCM_CMICX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx)) {
        /* pbm will either be all HW linkscan mode ports or SW linkscan mode ports
         * since both are handled exclusive of each other
         */
        BCM_PBMP_ASSIGN(hw_update, pbm);
        BCM_PBMP_AND(hw_update, lc->lc_pbm_hw);
        if (BCM_PBMP_NOT_NULL(hw_update)) {
            /* Pause only for update on HW linkscan ports */
...
bcm_esw_linkscan_update(int unit, bcm_pbmp_t pbm)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ls_cntl_t *lc = link_control[unit];
        pbmp_t hw_update, sw_update;
        BCM_PBMP_ASSIGN(hw_update, pbm);
        BCM_PBMP_ASSIGN(sw_update, pbm);
        BCM_PBMP_AND(hw_update, lc->lc_pbm_hw);
        BCM_PBMP_AND(sw_update, lc->lc_pbm_sw);

        _bcm_esw_linkscan_update(unit, 0 /*flags*/, hw_update);
        _bcm_esw_linkscan_update(unit, 0 /*flags*/, sw_update);
    } else
#endif
    {
...
_bcm_esw_linkscan_thread(int unit)
{
    ...
    while ((interval = lc->lc_us) != 0) {
...
#if defined (BCM_CMICX_SUPPORT)
            if (soc_feature(unit, soc_feature_cmicx)) {
                /* For case when faults may not gate link status from M0 */
                BCM_PBMP_OR(hw_update, sop->lc_pbm_remote_fault);
            }
#endif
            _bcm_esw_linkscan_update(unit, 0 /*flags*/, hw_update);
...
#if defined (BCM_CMICX_SUPPORT)
        /* Scan and process SW linkscan mode ports one-by-one.
         * In case HW linkscan interrupt is received, process that first
         */
        if (soc_feature(unit, soc_feature_cmicx) && normal_scheduled) {
            pbmp_t sw_port;
            PBMP_ITER(lc->lc_pbm_sw, port) {
                BCM_PBMP_CLEAR(sw_port);
                BCM_PBMP_PORT_SET(sw_port, port);
                _bcm_esw_linkscan_update(unit, 0 /*flags*/,  sw_port);
                if (lc->lc_hw_change) {
                    break;
                }
            }
        ...
        if (BCM_PBMP_IS_NULL(lc->lc_pbm_sw) &&
            ...
            if (soc_feature(unit, soc_feature_cmicx)) {
                /* For case when faults may not gate link status from M0 */
                if (BCM_PBMP_NOT_NULL(sop->lc_pbm_remote_fault)) {
                    lc->lc_hw_change = 1;
                }
            }
#endif
