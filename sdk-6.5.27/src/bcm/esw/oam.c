...
#if defined(INCLUDE_CCM) || defined(INCLUDE_BHH) || defined(INCLUDE_CCM_SLM)
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_fp_based_oam) && 
...
int bcm_esw_oam_pm_loss_stats_get(int unit, int loss_id, bcm_oam_pm_stats_t *stats_ptr)
{
    int result = BCM_E_UNAVAIL;
    ...
    if ((soc_feature(unit, soc_feature_uc_ccm_slm)) &&
        (soc_feature(unit, soc_feature_oam_pm))) {
        result = _bcm_mo_eth_oam_pm_loss_stats_get(unit, loss_id, stats_ptr);
    }
