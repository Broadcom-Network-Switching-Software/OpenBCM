...
int
bcmi_l3_riot_bank_set_l3_intf_hw(int unit, uint32 ol_bank_bit_value)
{
    ...
    SOC_IF_ERROR_RETURN(WRITE_EGR_L3_INTF_BANK_SELr(unit, bank_sel_reg_val));
    return BCM_E_NONE;
}

int
bcmi_l3_riot_bank_set_next_hop_hw(int unit, uint32 ol_bank_bit_value)
{
    uint32 bank_sel_reg_val     = 0;
...
bcmi_l3_riot_bank_sel(int unit)
{
    ...
    if (BCMI_L3_INTF_MEM_ALLOC_MODE_IS_UNDERLAY_FIRST(unit)) {
        ol_bank_bit_value <<= (max_l3_intf_banks - num_ol_banks);
    }

    SOC_IF_ERROR_RETURN(bcmi_l3_riot_bank_set_l3_intf_hw(unit, ol_bank_bit_value));
    ...
    if (l3->l3_nh_reserve_for_ecmp && !(BCMI_L3_NH_MEM_ALLOC_MODE_IS_UNDERLAY_FIRST(unit))) {
        num_ol_banks +=
            (BCM_XGS3_L3_ECMP_MAX_GROUPS(unit) / BCMI_L3_ECMP_PER_BANK(unit))
            + ((BCM_XGS3_L3_ECMP_MAX_GROUPS(unit) %
            BCMI_L3_ECMP_PER_BANK(unit)) ? 1 : 0);
    ...
    if (BCMI_L3_NH_MEM_ALLOC_MODE_IS_UNDERLAY_FIRST(unit)) {
        if (l3->l3_nh_reserve_for_ecmp) {
    ...
    SOC_IF_ERROR_RETURN(bcmi_l3_riot_bank_set_next_hop_hw(unit, ol_bank_bit_value));
...
/*
 * Function:
 *      bcmi_l3_nh_assoc_ol_ul_link_sw_search
 * Purpose:
 *      Searches for an underlay to overlay association in the sw list.
 * Parameters:
 *      ol_nh - overlay nh number
 *      ul_nh - underlay nh number
 * Returns:
 ...
 */
int
bcmi_l3_nh_assoc_ol_ul_link_sw_search(int unit, int ol_nh, int ul_nh)
{
    ul_nh_link_t *ul_assoc;
    ol_nh_link_t *link_nh;
    int bkt_idx = 0;
...
int bcmi_l3_nh_assoc_ol_ul_link_delete(int unit, int nh_idx)
{
    ...
    if (ent_type == BCMI_L3_EGR_NH_MCAST_ENTRY_TYPE) {
        ...
        if (next_ptr_type  == BCMI_L3_NH_EGR_NEXT_PTR_TYPE_NH) {
            ...
            if (BCM_XGS3_L3_ENT_OL_UL_LINK(BCM_XGS3_L3_TBL_PTR(unit, next_hop), nh_idx)) {
                BCM_IF_ERROR_RETURN(bcmi_l3_nh_assoc_ol_ul_link_sw_delete(unit, nh_idx,
                    next_nh_idx));
                BCM_XGS3_L3_ENT_OL_UL_LINK(BCM_XGS3_L3_TBL_PTR(unit, next_hop), nh_idx) = 0;
            }
            else {
                /*
                 * The olNH to ulNH link was setup by user.
                 * Decrement the old ulNH refcount.
                 */
                BCM_XGS3_L3_ENT_REF_CNT_DEC
                    (BCM_XGS3_L3_TBL_PTR(unit, next_hop), next_nh_idx,
                    _BCM_SINGLE_WIDE);
            }
        }
    }
...
{   
    ...
    /* If same, nothing needs to replace */
    if (old_ul == new_ul) {
        return BCM_E_NONE;
    }
    ...
    if (ul_assoc) {
        ...
        nh_idx = ol_link->o_nh_idx;
        next_link = ol_link->next_link;
        bcmi_l3_nh_assoc_ol_ul_link_sw_delete(unit, ol_link->o_nh_idx, 
            old_ul);
        bcmi_l3_nh_assoc_ol_ul_link_sw_add(unit, nh_idx,
            new_ul);
        ol_link = next_link;
        }
...
bcmi_l3_nh_assoc_ol_ul_link_reinit(int unit, int ol_nh_idx)
{
    ...
    if (entry_type == BCMI_L3_EGR_NH_MCAST_ENTRY_TYPE) { 
        ...
        if (next_ptr_type == BCMI_L3_NH_EGR_NEXT_PTR_TYPE_NH) {
            ...
            if (BCM_XGS3_L3_ENT_OL_UL_LINK(BCM_XGS3_L3_TBL_PTR(unit, next_hop), ol_nh_idx)) {
                BCM_IF_ERROR_RETURN(
                ...
                /*
                 * The olNH to ulNH link was setup by user.
                 * Increment the ulNH refcount.
                 */
                BCM_XGS3_L3_ENT_REF_CNT_INC
                    (BCM_XGS3_L3_TBL_PTR(unit, next_hop), ul_nh_idx,
                    _BCM_SINGLE_WIDE);
            }
        }
    }
    ...
    if (BCMI_L3_NH_DEST_IS_LAG(dest)) {
        ...
        if (soc_feature(unit, soc_feature_vp_lag)) {
            if (_bcm_vp_used_get(unit, vp, _bcmVpTypeVpLag)) {
                BCM_IF_ERROR_RETURN
                    (_bcm_esw_trunk_vp_lag_vp_to_tid(unit, vp, &nh->port));
                nh->flags |= BCM_L3_TGID;
            }
        }
