...
/*
 * DVP extension bit-map
 */
#define _BCM_DVP_EXTENSION_USED_GET(_u_, _vp_) \
        ((VIRTUAL_INFO(_u_)->dvp_extension_bitmap) ? \
        SHR_BITGET(VIRTUAL_INFO(_u_)->dvp_extension_bitmap, (_vp_)) : 0)
#define _BCM_DVP_EXTENSION_USED_SET(_u_, _vp_) \
        SHR_BITSET(VIRTUAL_INFO((_u_))->dvp_extension_bitmap, (_vp_))
#define _BCM_DVP_EXTENSION_USED_CLR(_u_, _vp_) \
        SHR_BITCLR(VIRTUAL_INFO((_u_))->dvp_extension_bitmap, (_vp_))
...
_bcm_virtual_free_resource(int unit, _bcm_virtual_bookkeeping_t *virtual_info)
{
    ...
    if (virtual_info->dvp_extension_bitmap) {
        sal_free(virtual_info->dvp_extension_bitmap);
        virtual_info->dvp_extension_bitmap = NULL;
    }
...
_bcm_esw_virtual_sync(int unit)
{
    ...
    if (stable_size > alloc_sz) {
        ...
        /*
         * BCM_WB_VERSION_1_15
         * dvp_extension_bitmap specifies the VP is used as the same index
         * to both dvp table and vc_and_swap table.
         */
        sal_memcpy(virtual_bitmap, VIRTUAL_INFO(unit)->dvp_extension_bitmap,
                       SHR_BITALLOCSIZE(num_vp));
        virtual_bitmap += SHR_BITALLOCSIZE(num_vp);
        ...
        /* Warm Boot */
            if (stable_size > alloc_sz) {
                ...
                if (recovered_ver >= BCM_WB_VERSION_1_15) {
                    sal_memcpy(VIRTUAL_INFO(unit)->dvp_extension_bitmap, virtual_bitmap,
                            SHR_BITALLOCSIZE(num_vp));
                    /* coverity[dead_error_condition : FALSE] */
                    if (old_num_vp) {
                        virtual_bitmap += SHR_BITALLOCSIZE(num_vp * 2);
...
_bcm_virtual_init(int unit, soc_mem_t vp_mem, soc_mem_t vfi_mem)
{
    ...
    if (NULL == virtual_info->dvp_extension_bitmap) {
        virtual_info->dvp_extension_bitmap =
            sal_alloc(SHR_BITALLOCSIZE(num_vp), "dvp_extension_bitmap");
        if (virtual_info->dvp_extension_bitmap == NULL) {
            _bcm_virtual_free_resource(unit, virtual_info);
            return BCM_E_MEMORY;
        }
    }
    ...
    sal_memset(virtual_info->dvp_extension_bitmap, 0, SHR_BITALLOCSIZE(num_vp));
        ...
        *base_vp = i;
        for (j = 0; j < count; j++) {
            ...
            if (vp_info.flags & _BCM_VP_INFO_DVP_EXTENSION) {
                _BCM_DVP_EXTENSION_USED_SET(unit, vp);
            }
        }
        VIRTUAL_UNLOCK(unit);
        return BCM_E_NONE;
...
_bcm_vp_free(int unit, _bcm_vp_type_e type, int count, int base_vp)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        _BCM_DVP_EXTENSION_USED_CLR(unit, base_vp+i);
    }
    VIRTUAL_UNLOCK(unit);
    return BCM_E_NONE;
}
...
_bcm_vp_used_set(int unit, int vp, _bcm_vp_info_t vp_info)
{
    ...
    if (vp_info.flags & _BCM_VP_INFO_DVP_EXTENSION) {
        ...
        _BCM_DVP_EXTENSION_USED_CLR(unit, vp);
    }
...
_bcm_vp_info_get(int unit, int vp, _bcm_vp_info_t *vp_info)
{
    ...
    for (vpt = _bcmVpTypeMpls; vpt < _bcmVpTypeAny; vpt++) {
        if (_bcm_vp_used_get(unit, vp, vpt)) {
            ...
            if (_BCM_DVP_EXTENSION_USED_GET(unit, vp)) {
                vp_info->flags |= _BCM_VP_INFO_DVP_EXTENSION;
            }
            return BCM_E_NONE;
        }
    }
...
_bcm_vp_encode_gport(int unit, int vp, int *gport)
{
    ...
    if(_bcm_vp_info_get(unit, vp, &vp_info) == BCM_E_NONE) {
        ...
        else if(vp_info.vp_type == _bcmVpTypeVpLag)
