            ...
            for (i = 0; i < soc_mem_index_count(unit, egr_mem); i++) {
               SOC_IF_ERROR_RETURN
                ...
                if (soc_mem_field_valid(unit, egr_mem, type_fld)) {
                    ...
                    } else if (soc_feature(unit, soc_feature_vxlan_gbp_flex)) {
                        /* VXLAN GBP flex views */
...
#if defined(BCM_HURRICANE4_SUPPORT) && defined(INCLUDE_L3)
        if (soc_feature(unit, soc_feature_flex_flow) &&
            ...
            BCM_IF_ERROR_RETURN (soc_mem_generic_lookup(unit, mem,
                        MEM_BLOCK_ALL, 0, vent, vent, &index));
            source_vp = soc_mem_field32_get(unit, mem, vent,
                    SOURCE_VPf);
            if (flow_info->vp_ref_cnt[source_vp] == 0) {
