    ...
    if (SOC_MEM_IS_VALID(unit, mem)) {
        if (NULL == info->dscp_table) {
            ...
            if (soc_property_get(unit, spn_QOS_MAP_DSCP_NONSHARABLE_MODE, 0)) {
                info->dscp_table->flags |= SOC_PROFILE_MEM_F_NO_SHARE;
            }
            if(SOC_IS_TRIDENT3X(unit)) {
...
int
_bcm_dscp_table_entry_set(int unit, void **entries, uint32 index)
{
    int rv;
    COMMON_LOCK(unit);
    rv = soc_profile_mem_set(unit, DSCP_TABLE(unit), entries, index);
    COMMON_UNLOCK(unit);
    return rv;
}
...
/*
 * Function:
 *      _bcm_dscp_table_entry_write_mode_get
 * Purpose:
 *      Internal function to get write mode of DSCP_TABLE table
 * Parameters:
 *      unit    -  (IN) Device number.
 *      index   -  (IN) Base index to get
 *      mode    -  (OUT) table write mode
 ...
 */
int
_bcm_dscp_table_entry_write_mode_get(int unit, uint32 index, int *mode)
{
    int rv;
    COMMON_LOCK(unit);
    rv = soc_profile_mem_write_mode_get(unit, DSCP_TABLE(unit), index, mode);
    COMMON_UNLOCK(unit);
    return rv;
}
