                   ...
                   void *data)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_LATENCY_MONITOR,
                (BSL_META_U(unit,"Call back completed for handle:%d Monitor:%d data:%p\n"),
                             handle, monitor_id, (void *) p_cb_data->data));
exit:
...
counter_accrue_thread(void* up)
{
    ...
    uint64          *sw_buff = NULL;
    uint32          sw_buf_idx = 0;
    _sbusdma_desc_handle_data_t *list_data = NULL;
    int             pipe = 0;
    int             pipe_lower = -1;
...
latency_monitor_read(int unit, int monitor_id)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_LATENCY_MONITOR,
                 (BSL_META_U(unit, "\npid_sbusdma[%0d] = %p"), monitor_id, (void *) pid));
    ...
    bcm_latency_monitor_config_t *config)
{
    ...
    case MONITOR_ID_0:
        ...
        stat_obj = (bcm_stat_object_t) bcmIntStatObjectEgrLatencyMonitor0;
    ...
    case MONITOR_ID_1:
        ...
        stat_obj = (bcm_stat_object_t) bcmIntStatObjectEgrLatencyMonitor1;
    ...
    case MONITOR_ID_2:
        ...
        stat_obj = (bcm_stat_object_t) bcmIntStatObjectEgrLatencyMonitor2;
    ...
    case MONITOR_ID_3:
        ...
        stat_obj = (bcm_stat_object_t) bcmIntStatObjectEgrLatencyMonitor3;
    ...
    if (latency_monitor_mem[unit][monitor_id].status != LATENCY_MONITOR_STATE_CLEANUP) {
        ...
        rv = _bcm_esw_stat_group_create(unit, stat_obj,
                (bcm_stat_group_mode_t) bcmIntStatGroupModeLatencyMonitor,
...
bcm_tomahawk3_latency_monitor_init (int unit)
{
    ...
    if (soc_feature(unit, soc_feature_latency_monitor)) {
        ...
        if (SOC_WARM_BOOT(unit)) {
            ...
            if (BCM_SUCCESS(rv)) {
                return _bcm_flex_ctr_latency_monitor_resource_clean(unit);
            }
        } else {
