...
/*
 * Function:
 *      _bcm_esw_ipmc_age_scache_size_get
 * Purpose:
 *      Get the required scache size for storing IPMC age update info.
 * Parameters:
 *      unit - device unit #
    ...
    uint32 *size)
{
    ...
    /* size of IPMC age update interval */
    *size = sizeof(IPMC_INFO(unit)->ipmc_age_interval);
...
/*
 * Function:
 *      _bcm_esw_ipmc_age_sync
 * Purpose:
 *      Record IPMC age update info into the scache.
 * Parameters:
 *      unit - device unit #
    ...
    uint8 **scache_ptr)
{
    ...
    /* Store IPMC age update interval into scache */
    sal_memcpy((*scache_ptr),
               (void*)&(IPMC_INFO(unit)->ipmc_age_interval), sizeof(sal_usecs_t));
    (*scache_ptr) += sizeof(sal_usecs_t);
...
/*
 * Function:
 *      _bcm_esw_ipmc_age_scache_recover
 * Purpose:
 *      Recover IPMC age update info from scache.
 * Parameters:
 ...
 */
STATIC int
_bcm_esw_ipmc_age_scache_recover(int unit, uint8 **scache_ptr)
{
    if (scache_ptr == NULL) {
        return BCM_E_PARAM;
    }

    sal_memcpy((void*)&(IPMC_INFO(unit)->ipmc_age_interval), (*scache_ptr),
               sizeof(IPMC_INFO(unit)->ipmc_age_interval));
    (*scache_ptr) += sizeof(IPMC_INFO(unit)->ipmc_age_interval);
...
_bcm_esw_ipmc_required_scache_size_get(int unit, uint32 *size)
{
    ...
    *size = 0;
    ...
    if ((SOC_IS_KATANA(unit)) ||
     ...
     /* size for IPMC age update info */
     BCM_IF_ERROR_RETURN
        (_bcm_esw_ipmc_age_scache_size_get(unit, &temp_size));
     *size += temp_size;
...
_bcm_esw_ipmc_sync(int unit)
{
    ...
    rv = _bcm_esw_scache_ptr_get(unit, scache_handle, FALSE,
                                 0, &ipmc_scache_ptr,
                                 BCM_IPMC_WB_DEFAULT_VERSION, NULL);
    ...
    /* Sync IPMC age update info. */
    BCM_IF_ERROR_RETURN
        (_bcm_esw_ipmc_age_sync(unit, &ipmc_scache_ptr));
...
STATIC int _bcm_esw_ipmc_reinit(int unit)
{
    ...
    rv = _bcm_esw_scache_ptr_get(unit, scache_handle, FALSE,
            0, &ipmc_scache_ptr,
            BCM_IPMC_WB_DEFAULT_VERSION, &recovered_ver);
    if (BCM_E_NOT_FOUND == rv) {
        ...
        if (recovered_ver >= BCM_IPMC_WB_VERSION_1_1) {
#ifdef BCM_TRIUMPH3_SUPPORT
            if (soc_feature(unit, soc_feature_repl_l3_intf_use_next_hop)) {
#ifdef BCM_TOMAHAWK_SUPPORT
                if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit)) {
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) || defined(BCM_TOMAHAWK_SUPPORT)
        if (recovered_ver >= BCM_IPMC_WB_VERSION_1_2) {
            if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
#if defined(BCM_TOMAHAWK_SUPPORT)
                if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit)) {
                    BCM_IF_ERROR_RETURN(
                        bcm_th_ipmc_aggid_info_recover(
                            unit, &ipmc_scache_ptr, recovered_ver));
                }
#endif
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
                if (SOC_IS_TRIDENT2PLUS(unit) || SOC_IS_APACHE(unit)) {
        ...
        if (recovered_ver >= BCM_IPMC_WB_VERSION_1_3) {
#ifdef BCM_TRIUMPH3_SUPPORT
            if (soc_feature(unit, soc_feature_repl_l3_intf_use_next_hop)) {
#ifdef BCM_TOMAHAWK_SUPPORT
                if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit)) {
        ...
        if (recovered_ver >= BCM_IPMC_WB_VERSION_1_4) {
#ifdef BCM_TRIUMPH3_SUPPORT
            if (soc_feature(unit, soc_feature_repl_l3_intf_use_next_hop)) {
#ifdef BCM_TOMAHAWK_SUPPORT
                if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit)) {
        ...
        if (recovered_ver >= BCM_IPMC_WB_VERSION_1_5) {
#if defined(BCM_KATANA_SUPPORT)
            if ((SOC_IS_KATANA(unit)) ||
                (SOC_IS_KATANA2(unit) && !SOC_IS_SABER2(unit))) {
                BCM_IF_ERROR_RETURN(
        ...
        /* IPMC age update interval */
        if (recovered_ver >= BCM_IPMC_WB_VERSION_1_7) {
            BCM_IF_ERROR_RETURN(
                _bcm_esw_ipmc_age_scache_recover(unit, &ipmc_scache_ptr));
        } else {
            uint32 temp_size;
            BCM_IF_ERROR_RETURN(
                _bcm_esw_ipmc_age_scache_size_get(unit, &temp_size));
        ...
        if (IPMC_INFO(unit)->ipmc_age_interval) {
            rv = bcm_esw_ipmc_age_timer_set(unit,
                        IPMC_INFO(unit)->ipmc_age_interval);
            if (BCM_FAILURE(rv)) {
                if (IPMC_INFO(unit)->ipmc_age_sync != NULL) {
                    sal_sem_destroy(IPMC_INFO(unit)->ipmc_age_sync);
                    IPMC_INFO(unit)->ipmc_age_sync = NULL;
                }
                return rv;
            }
        }
...
bcm_esw_ipmc_init(int unit)
{
    ...
    IPMC_INFO(unit)->ipmc_age_interval = 0;
    IPMC_INFO(unit)->ipmc_age_cb = NULL;
    IPMC_INFO(unit)->ipmc_age_cb_data = NULL;
    IPMC_INFO(unit)->ipmc_age_pid = SAL_THREAD_ERROR;

    if (IPMC_INFO(unit)->ipmc_age_sync == NULL) {
        if ((IPMC_INFO(unit)->ipmc_age_sync =
            sal_sem_create("IPMC age sync", sal_sem_BINARY, 1)) == NULL) {
            rv = BCM_E_MEMORY;
        }
    }
                 ...
                 void *user_data)
{
    ...
    if (soc_property_get(unit, spn_IPMC_AGE_MODE, 0)) {
        return BCM_E_UNAVAIL;
    }
...
/*
 * Function:
 *  bcm_esw_ipmc_age_cb_register
 * Description:
 *  Register a callback routine.
 * Parameters:
 *  unit           - (IN) BCM device number.
 *  cb             - (IN) Callback routine.
 *  user_data      - (IN) User data.
 * Returns:
 *  BCM_E_NONE      Success, handle valid
 *  BCM_E_RESOURCE  Out of resource
 */
int
bcm_esw_ipmc_age_cb_register(int unit, bcm_ipmc_age_cb_t cb, void *user_data)
{
    IPMC_INIT(unit);
    ...
    if (IPMC_INFO(unit)->ipmc_age_cb == cb &&
        IPMC_INFO(unit)->ipmc_age_cb_data == user_data) {
        IPMC_UNLOCK(unit);
        return BCM_E_NONE;
    }

    /* Currently only one callback is supported. */
    if (IPMC_INFO(unit)->ipmc_age_cb != NULL) {
        IPMC_UNLOCK(unit);
        return BCM_E_RESOURCE;
    }

    IPMC_INFO(unit)->ipmc_age_cb = cb;
    IPMC_INFO(unit)->ipmc_age_cb_data = user_data;
...
/*
 * Function:
 *  bcm_esw_ipmc_age_cb_unregister
 * Description:
 ...
 */
int
bcm_esw_ipmc_age_cb_unregister(int unit, bcm_ipmc_age_cb_t cb, void *user_data)
{
    int rv;
...
/*
 * Function:
 *  bcm_esw_ipmc_age_timer_set
 * Description:
 *  Set the age timer for IPMC entries (0 to disable timer).
 * Parameters:
 *  unit          - (IN) BCM device number.
 *  seconds       - (IN) Age timer value in seconds
 * Returns:
 *  BCM_E_NONE          Success
 ...
 */
int
bcm_esw_ipmc_age_timer_set(int unit, int seconds)
{
    int rv = BCM_E_NONE;
    ...
    if ((seconds < 0) || (seconds > _IPMC_MAX_AGE_INTERVAL)) {
        return BCM_E_PARAM;
    }

    IPMC_LOCK(unit);

    if (IPMC_INFO(unit)->ipmc_age_pid != SAL_THREAD_ERROR) {
        /* IPMC aging thread is running. */
        if (IPMC_INFO(unit)->ipmc_age_interval != seconds) {
            IPMC_INFO(unit)->ipmc_age_interval = seconds;
            /* Notify the thread of the change of the interval. */
            sal_sem_give(IPMC_INFO(unit)->ipmc_age_sync);
        }
    } else {
        /* IPMC age update thread is NOT running. */
        if (seconds) {
            /*
             * To make sure a new born aging thread will always take this
             * semaphore successfully the first time it tries.
             */
            sal_sem_give(IPMC_INFO(unit)->ipmc_age_sync);
#ifdef BCM_TRIUMPH_SUPPORT
            if (SOC_IS_TR_VL(unit)) {
                rv = bcm_tr_ipmc_age_start(unit, seconds);
            }
#endif /* BCM_TRIUMPH_SUPPORT */
       }
    }
...
/*
 * Function:
 *  bcm_esw_ipmc_age_timer_get
 * Description:
 *  Get the age timer for IPMC entries (0 for timer disable).
 * Parameters:
 *  unit      - (IN)  BCM device number.
 *  seconds   - (OUT) Age timer value in seconds
 * Returns:
 *  BCM_E_NONE      Success
 */
int
bcm_esw_ipmc_age_timer_get(int unit, int *seconds)
{
    IPMC_INIT(unit);
    ...
    IPMC_LOCK(unit);
    *seconds = IPMC_INFO(unit)->ipmc_age_interval;
    IPMC_UNLOCK(unit);
