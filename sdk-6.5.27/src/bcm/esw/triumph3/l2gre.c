...
_bcm_tr3_l2gre_wb_alloc(int unit)
{
    ...
    if ((soc_feature(unit, soc_feature_td2_l2gre) &&
        soc_feature(unit, soc_feature_vp_lag))||SOC_IS_HURRICANE4(unit)) {
        /*
         * Size of All match_key's index && gport(trunk_id, modid, port) &&
         * match_vlan && match_inner_vlan && match_etag_vlan
         */
...
_bcm_tr3_l2gre_wb_recover(int unit)
{
    ...
    if ((soc_feature(unit, soc_feature_td2_l2gre) &&
        ...
        if (recovered_ver >= BCM_WB_VERSION_1_1) {
            for (i = 0; i < num_vp; i++) {
                ...
                if (recovered_ver >= BCM_WB_VERSION_1_2) {
                    sal_memcpy(&(l2gre_info->match_key[i].match_etag_vlan),
                               l2gre_state, sizeof(bcm_vlan_t));
                    l2gre_state += sizeof(bcm_vlan_t);
                    sal_memcpy(&(l2gre_info->match_key[i].match_count),
                               l2gre_state, sizeof(int));
                    l2gre_state += sizeof(int);
                }
            }
        } else {
        ...
        if (recovered_ver < BCM_WB_VERSION_1_2) {
            /* for match_etag_vlan and match_count */
            additional_scache_size +=
                num_vp * (sizeof(int) + sizeof(bcm_vlan_t));
        }
    }
...
_bcm_tr3_l2gre_sync(int unit)
{
    ...
    if ((soc_feature(unit, soc_feature_td2_l2gre) &&
        ...
        for (i = 0; i < num_vp; i++) {
            ...
            l2gre_state += sizeof(bcm_vlan_t);
            sal_memcpy(l2gre_state,
                       &(l2gre_info->match_key[i].match_etag_vlan),
                       sizeof(bcm_vlan_t));
            l2gre_state += sizeof(bcm_vlan_t);
            sal_memcpy(l2gre_state,
                       &(l2gre_info->match_key[i].match_count),
                       sizeof(int));
            l2gre_state += sizeof(int);
        }
    }
...
_bcm_tr3_l2gre_reinit(int unit)
{
    ...
    int keytype_ovid, keytype_ivid, keytype_iovid, keytype_cfi, keytype_vif;
    ...
    if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWKX(unit)) {
        ...
        keytype_vif = VLXLT_HASH_KEY_TYPE_VIF_VLAN;
    } else
#endif
    {
        ...
        /*coverity[mixed_enums]*/
        keytype_vif = TR3_VLXLT_HASH_KEY_TYPE_VIF_VLAN;
    }
...
bcm_tr3_l2gre_vpn_create(int unit, bcm_l2gre_vpn_config_t *info)
{
    ...
    if ((!(info->flags & (BCM_L2GRE_VPN_ELINE | BCM_L2GRE_VPN_ELAN))) ||
...
/*
 * Function:
 *           _bcm_tr3_l2gre_higig_ppd_select
 * Purpose:
 ...
 */
int
_bcm_tr3_l2gre_higig_ppd_select(int unit, int nh_index, uint32 flags)
{
    egr_l3_next_hop_entry_t egr_nh;
    int entry_type = 0;
    ...
    if (entry_type == _BCM_L2GRE_EGR_NEXT_HOP_L3_VIEW) {
...
/*
 * Function:
 *      _bcm_tr3_l2gre_ecmp_higig_ppd_select
 * Purpose:
...
int
_bcm_tr3_l2gre_ecmp_higig_ppd_select(int unit, int ecmp_index, uint32 flags)
{
    ...
    while(i < ecmp_member_count) {
        ...
        rv = _bcm_tr3_l2gre_higig_ppd_select(unit, index, flags);
...
int
_bcm_tr3_l2gre_next_hop_set(int unit, int nh_index, uint32 flags, int vp, int drop)
{
    int rv=BCM_E_NONE;
    bcm_port_t port = -1;
    ...
    if (!(flags & BCM_L3_IPMC)) { /* Only for L2GRE Unicast Nexthops */
       for (idx = 0; idx < num_ports; idx++) {
        ...
        if (vp_info.flags & _BCM_VP_INFO_NETWORK_PORT) {
            ...
            egr_nh_info.dvp = vp;
            rv = _bcm_tr3_l2gre_nexthop_entry_modify(unit,
                     nh_index, drop, &egr_nh_info, _BCM_L2GRE_EGR_NEXT_HOP_L3MC_VIEW);
            if (BCM_FAILURE(rv)) {
                goto clean_up;
            }
        }
...
int
bcm_tr3_l2gre_egress_set(int unit, int nh_index, uint32 flags, int vp, int drop)
{
    ing_l3_next_hop_entry_t ing_nh;
    soc_mem_t      hw_mem;
    int rv=BCM_E_NONE;
    ...
    rv = _bcm_tr3_l2gre_next_hop_set(unit, nh_index, flags, vp, drop);
    return rv;
}
                            ...
                            bcm_vpn_t vpn, int drop)
{
    ...
    if (l2gre_port->flags & BCM_L2GRE_PORT_EGRESS_TUNNEL) {
        ...
        if (SOC_IS_TRIDENT3X(unit)) {
            ...
            if (mpath_flag == 0) {
                rv = _bcm_tr3_l2gre_higig_ppd_select(unit, vp_nh_ecmp_index, l2gre_port->flags);
               _BCM_L2GRE_CLEANUP(rv);
            } else {
                rv = _bcm_tr3_l2gre_ecmp_higig_ppd_select(unit, l2gre_port->egress_if,
                                 l2gre_port->flags);
               _BCM_L2GRE_CLEANUP(rv)
           }
        }
...
bcm_tr3_l2gre_match_tunnel_entry_reset(int unit, int vp)
{
    ...
    if (l2gre_info->l2gre_tunnel_term[tunnel_idx].dip == 0) {
        /*
         * Clear the sip field only when the tunnel terminator has been
         * destroyed. If we reset sip when the tunnel terminator is destroyed,
         * chances are we may not be able to find and remove this MPLS_ENTRY
         * later.
         */
...
_bcm_tr3_l2gre_eline_port_add(int unit, bcm_vpn_t vpn, bcm_l2gre_port_t  *l2gre_port)
{
    ...
    vp_info.vp_type = _bcmVpTypeL2Gre;
       ...
       if (rv < 0) {
            if (!(l2gre_port->flags & BCM_L2GRE_PORT_REPLACE)) {
                (void) _bcm_vp_free(unit, _bcmVpTypeL2Gre, 1, active_vp);
            }
            return rv;
       }
...
_bcm_tr3_l2gre_sw_dump(int unit)
{
    ...
    LOG_CLI((BSL_META_U(unit, "---------------------------------------------\n")));
    LOG_CLI((BSL_META_U(unit, "| Index |    Source IP    |     Dest IP     |\n")));
    LOG_CLI((BSL_META_U(unit, "---------------------------------------------\n")));
    for (i = 0; i < num_vp; i++) {
        if (L2GRE_INFO(unit)->l2gre_tunnel_init[i].dip != 0 &&
            L2GRE_INFO(unit)->l2gre_tunnel_init[i].sip != 0) {
            shr_format_ipaddr(ip_str1, l2gre_info->l2gre_tunnel_init[i].sip);
            shr_format_ipaddr(ip_str2, l2gre_info->l2gre_tunnel_init[i].dip);
            LOG_CLI((BSL_META_U(unit, "| %5d | %15s | %15s |\n"), i, ip_str1, ip_str2));
        }
    }
    LOG_CLI((BSL_META_U(unit, "---------------------------------------------\n")));
    ...
    for (i = 0; i < num_vp; i++) {
        if (L2GRE_INFO(unit)->l2gre_tunnel_term[i].dip != 0 &&
            ...
            shr_format_ipaddr(ip_str1, l2gre_info->l2gre_tunnel_term[i].sip);
            shr_format_ipaddr(ip_str2, l2gre_info->l2gre_tunnel_term[i].dip);
            LOG_CLI((BSL_META_U(unit, "| %5d | %15s | %15s |\n"), i, ip_str1, ip_str2));
        }
    }
    LOG_CLI((BSL_META_U(unit, "---------------------------------------------\n")));
    ...
    LOG_CLI((BSL_META_U(unit, "---------------------------------------------------------------------------\n")));
    LOG_CLI((BSL_META_U(unit, "| port|    Flags   | Idx | TG |Mo|Po |                Match               |\n")));
    LOG_CLI((BSL_META_U(unit, "| VP  |            |     | ID | d| rt|oVLAN|iVLAN|VPNid|TnlIdx|eVLAN|Count|\n")));
    LOG_CLI((BSL_META_U(unit, "---------------------------------------------------------------------------\n")));
    ...
    for (i = 0; i < num_vp; i++) {
        ...
        LOG_CLI((BSL_META_U(unit, "|%5d| 0x%08x |%5d|%4d|%2d|%3d|%5d|%5d|%5d|%6d|%5d|%5d|\n"),
                 i, key->flags, key->index, key->trunk_id, key->modid, key->port, key->match_vlan,
                 key->match_inner_vlan, key->match_vpnid, key->match_tunnel_index, key->match_etag_vlan,
                 key->match_count));
    }
    LOG_CLI((BSL_META_U(unit, "---------------------------------------------------------------------------\n")));
