        ...
        bcm_l3_egress_t *l3_egress) 
{
    ...
    */
    ...
    if (BCMI_RIOT_IS_ENABLED(unit) &&
        ...
        if (BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, l3_egress->encap_id)) {
            vp_nh_index = l3_egress->encap_id - BCM_XGS3_DVP_EGRESS_IDX_MIN(unit);
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                L3MC__NEXT_PTR_TYPEf, BCMI_L3_NH_EGR_NEXT_PTR_TYPE_NH);
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                L3MC__NEXT_PTRf, vp_nh_index);
            /*
             * increase the ref count of underlay next hop so that it
             * not deleted in case someone tries to delete it.
             */
            BCM_XGS3_L3_ENT_REF_CNT_INC
                (BCM_XGS3_L3_TBL_PTR(unit, next_hop), vp_nh_index,
                _BCM_SINGLE_WIDE);
        } else {
            /* Get the vp */
            SOC_IF_ERROR_RETURN
             ...
             */
             if (bcmi_l3_nh_assoc_ol_ul_link_is_required(unit, vp_nh_index)) {
                 ...
                 BCM_IF_ERROR_RETURN(
                     ...
                     vp_nh_index));
                 /* Flag to indicate the association was done by sw */
                 BCM_XGS3_L3_ENT_OL_UL_LINK(BCM_XGS3_L3_TBL_PTR(unit, next_hop), nh_index) = 1;
             } else {
                 soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
...
_bcm_tr3_l3_ipmc_nh_entry_parse(int unit, uint32 *ing_nh, 
        uint32 *egr_nh, uint32 *egr_nh_2, uint32 index,
        bcm_l3_egress_t *l3_egress)
{
    ...
    if (BCMI_RIOT_IS_ENABLED(unit)) {
        if (soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, egr_nh,
            L3MC__NEXT_PTR_TYPEf) == BCMI_L3_NH_EGR_NEXT_PTR_TYPE_NH) {
            /* This is an ol-Nh; read ul-NH */
            int ulNH = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, egr_nh,
                                L3MC__NEXT_PTRf);
            /* search the sw-managed list */
            if (!BCM_XGS3_L3_ENT_OL_UL_LINK(BCM_XGS3_L3_TBL_PTR(unit, next_hop), index)) {
                /* ulNH was supplied by user, and not derived by SDK. */
                l3_egress->encap_id = ulNH + BCM_XGS3_DVP_EGRESS_IDX_MIN(unit);
            }
        }
    }
#endif
