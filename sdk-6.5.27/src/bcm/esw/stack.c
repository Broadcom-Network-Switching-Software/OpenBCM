                    ...
                    if (soc_feature(unit, soc_feature_xflow_macsec_svtag)) {
                        ...
                        if (src_mod_port_mems[i] == MODPORT_MAP_SUBPORTm) {
                            rv = soc_mem_write_range(unit,
                                                     src_mod_port_mems[i],
                                                     MEM_BLOCK_ANY,
                                                     (SOC_HR4_MODPORT_MAP_SUBPORT_M0_SIZE - BCM_STK_HURRICANE4_NUM_LPORTS),
                                                     (SOC_HR4_MODPORT_MAP_SUBPORT_M0_SIZE - BCM_STK_HURRICANE4_NUM_LPORTS) + entry_cnt -1,
                                                     entry_buffer);
                            /* Update mirror table also */
                            rv = soc_mem_write_range(unit,
                                                     MODPORT_MAP_SUBPORT_MIRRORm,
                                                     MEM_BLOCK_ANY,
                                                     (SOC_HR4_MODPORT_MAP_SUBPORT_M0_SIZE - BCM_STK_HURRICANE4_NUM_LPORTS),
                                                     (SOC_HR4_MODPORT_MAP_SUBPORT_M0_SIZE - BCM_STK_HURRICANE4_NUM_LPORTS) + entry_cnt -1,
                                                     entry_buffer);
                            rv = _soc_xflow_macsec_modid_base_set(unit, (SOC_HR4_MODPORT_MAP_SUBPORT_M0_SIZE - BCM_STK_HURRICANE4_NUM_LPORTS));
                        }
                    }
#endif
                }
...
bcm_esw_stk_my_modid_set(int unit, int my_modid)
{
    ...
    int rv = SOC_E_NONE;
    uint32 subport_id_to_sgpp_map[SOC_MAX_MEM_FIELD_WORDS];
#endif
#endif
    ...
    case BCM_FAMILY_KATANA:
    {
        ...
        /* Update SUBPORT_ID_TO_SGPP_MAP config as mymodid has changed */
        if (soc_feature(unit, soc_feature_xflow_macsec_svtag) &&
                                    (SOC_BASE_MODID(unit) != my_modid)) {
            PBMP_ALL_ITER(unit, port) {
...
_src_modid_base_index_bk_detach(int unit) 
{
    if (src_modid_base_index_bk[unit]) {
        ...
        if (src_modid_base_index_bk[unit]->coe_module_id_list) {
            sal_free(src_modid_base_index_bk[unit]->coe_module_id_list);
            src_modid_base_index_bk[unit]->coe_module_id_list = NULL;
        }
 #if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (src_modid_base_index_bk[unit]->module_id_type_list) {
            sal_free(src_modid_base_index_bk[unit]->module_id_type_list);
            src_modid_base_index_bk[unit]->module_id_type_list = NULL;
        }
        if (src_modid_base_index_bk[unit]->multi_nexthop_module_id_list) {
            sal_free(src_modid_base_index_bk[unit]->multi_nexthop_module_id_list);
            src_modid_base_index_bk[unit]->multi_nexthop_module_id_list = NULL;
        }
#endif
