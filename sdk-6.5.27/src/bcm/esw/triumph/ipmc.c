...
extern void bcm_tr_ipmc_age_thread(int unit);
...
bcm_tr_ipmc_init(int unit)
{
    ...
    info->ipmc_age_interval = 0;
    info->ipmc_age_cb = NULL;
    info->ipmc_age_cb_data = NULL;
    info->ipmc_age_pid = SAL_THREAD_ERROR;
    if ((info->ipmc_age_sync =
...
bcm_tr_ipmc_detach(int unit)
{
    ...
    if (info->ipmc_initialized) {
        /* Stop IPMC age update thread. */
        bcm_tr_ipmc_age_stop(unit);
        IPMC_LOCK(unit);
        info->ipmc_age_cb = NULL;
        info->ipmc_age_cb_data= NULL;
        if (info->ipmc_age_sync != NULL) {
            sal_sem_destroy(info->ipmc_age_sync);
            info->ipmc_age_sync = NULL;
        }
        IPMC_UNLOCK(unit);
...
/*
 * Function:
 *  bcm_tr_ipmc_age_start
 * Purpose:
 *  Start an IPMC aging thread
 * Parameters:
 *  unit - unit number.
 ...
 */
int
bcm_tr_ipmc_age_start(int unit, int interval)
{
    int thread_prio;
    ...
    if (IPMC_INFO(unit)->ipmc_age_pid &&
    ...
    IPMC_INFO(unit)->ipmc_age_interval = interval;
    sal_snprintf(IPMC_INFO(unit)->ipmc_age_name,
                 sizeof(IPMC_INFO(unit)->ipmc_age_name),
                 "bcmIpmcAge.%d", unit);
    thread_prio = 50;

    IPMC_INFO(unit)->ipmc_age_pid =
        sal_thread_create(IPMC_INFO(unit)->ipmc_age_name, SAL_THREAD_STKSZ,
                          thread_prio,
                          (void (*)(void*))bcm_tr_ipmc_age_thread,
                          INT_TO_PTR(unit));
    if (IPMC_INFO(unit)->ipmc_age_pid == SAL_THREAD_ERROR) {
        LOG_INFO(BSL_LS_BCM_IPMC,
                 (BSL_META_U(unit,
                             "Could not start IPMC age thread.\n")));
        IPMC_UNLOCK(unit);
        return BCM_E_MEMORY;
    }
...
/*
 * Function:
 *  bcm_tr_ipmc_age_stop
 * Purpose:
 *  Stop the IPMC aging thread
 * Parameters:
 *  unit - unit number.
 * Returns:
 *  SOC_E_XXX
...
bcm_tr_ipmc_age_stop(int unit)
{
    ...
    if (IPMC_INFO(unit)->ipmc_age_pid &&
        IPMC_INFO(unit)->ipmc_age_pid != SAL_THREAD_ERROR) {
        /* Notify the thread of the change of the interval so that it could exit. */
        ...
        while (IPMC_INFO(unit)->ipmc_age_pid != SAL_THREAD_ERROR) {
            /* wait until timeout */
            if (soc_timeout_check(&to)) {
                LOG_ERROR(BSL_LS_BCM_IPMC,
                          (BSL_META_U(unit,
                                      "IPMC age thread will not exit\n")));
                rv = BCM_E_INTERNAL;
                break;
            }
        }
...
static uint32 _tr_ipmc_age_iter[SOC_MAX_NUM_DEVICES] = {0};

/*
 * Function:
 *      bcm_tr_ipmc_age_thread
 * Purpose:
 *      Update the ipmc entry by clearing the HIT bit when appropriate,
 *      an aged-out event is sent if HIT bit is not set.
 * Parameters:
 *      unit       -  (IN) BCM device number.
 * Returns:
 *      BCM_E_XXX
 */
void
bcm_tr_ipmc_age_thread(int unit)
{
    int idx;                   /* Ipmc table iteration index. */
    bcm_ipmc_addr_t entry;     /* Ipmc entry iterator.        */
    int rv = BCM_E_NONE;       /* Operation return status.    */
    sal_usecs_t interval, stime, etime;
    ipmc_entry_t   ipmc_entry;
#if defined(BCM_TRIUMPH2_SUPPORT) || defined(BCM_KATANA2_SUPPORT)
    ipmc_1_entry_t ipmc_1_entry;
#endif
    ...
    while ((interval = IPMC_INFO(unit)->ipmc_age_interval) != 0) {
        if (iter == 0) {
            goto thread_delay;
        }
        ...
        for (idx = 0; idx < IPMC_GROUP_NUM(unit); idx++) {
            if (IPMC_USED_ISSET(unit, idx)) {
                ...
                while (ipmc_l3entry != NULL) {
                    ...
                    if (entry.flags & BCM_IPMC_HIT) {
                        /* The entry is activated in the last interval. */
                        _bcm_l3_cfg_t l3cfg;
                        /* Clear hit bit by doing a lookup. */
                        sal_memset(&l3cfg, 0, sizeof(_bcm_l3_cfg_t));
                        l3cfg.l3c_flags |= BCM_L3_HIT_CLEAR;
                        l3cfg.l3c_flags |= BCM_L3_IPMC;
                        l3cfg.l3c_vrf = ipmc_l3entry->l3info.vrf;
                        ...
                        if (ipmc_l3entry->l3info.age_notified) {
                        /*
                         * Invoke the callback function to notify the application
                         * if age events have been sent, so that the application may
                         * have a chance to cancel the removal.
                         */
                            if (IPMC_INFO(unit)->ipmc_age_cb != NULL) {
                                entry.flags |= BCM_IPMC_HIT;
                                rv = (*IPMC_INFO(unit)->ipmc_age_cb)(unit,
                                        &entry, bcmIpmcAgeEventDeleteCancel,
                                        IPMC_INFO(unit)->ipmc_age_cb_data);
#ifdef BCM_CB_ABORT_ON_ERR
                                if (BCM_FAILURE(rv) && SOC_CB_ABORT_ON_ERR(unit)) {
                                    IPMC_UNLOCK(unit);
                                    goto thread_exit;
...
#endif
                                /* Clear to suppress excessive NOT age out events. */
                                ipmc_l3entry->l3info.age_notified = 0;
                            }
                        }
                    } else {
                        /*
                         * This entry has been inactive for at least the last interval.
                         * It should be aged out. Notify the application if we haven't done so.
                         * The application will decide whether to remove the entry or not.
                         */
                        if (ipmc_l3entry->l3info.age_notified == 0) {
                            /* Invoke user callback with BCM_IPMC_HIT cleared. */
                            if (NULL != IPMC_INFO(unit)->ipmc_age_cb) {
                                _BCM_MULTICAST_GROUP_SET(entry.group,
                                        _BCM_MULTICAST_TYPE_L3, entry.group);
                                _BCM_MULTICAST_GROUP_SET(entry.group_l2,
                                        _BCM_MULTICAST_TYPE_L3, entry.group_l2);
                                rv = (*IPMC_INFO(unit)->ipmc_age_cb)(unit,
                                         &entry, bcmIpmcAgeEventDelete,
                                         IPMC_INFO(unit)->ipmc_age_cb_data);
#ifdef BCM_CB_ABORT_ON_ERR
                                if (BCM_FAILURE(rv) && SOC_CB_ABORT_ON_ERR(unit)) {
                                    IPMC_UNLOCK(unit);
...
#endif
                                /* Set to suppress excessive aged out events. */
                                ipmc_l3entry->l3info.age_notified = 1;
                            }
                        }
                    }

                    /* Iterate the next entry. */
                    ipmc_l3entry = ipmc_l3entry->next;
                }
            }
        }

        /* End of the processing of the this run. */
        IPMC_UNLOCK(unit);
        ...
        LOG_VERBOSE(BSL_LS_BCM_IPMC,
                    (BSL_META_U(unit,
                                "ipmc_age_thread: \
                                 unit=%d: done in %d usec with rv=%d\n"),
                     unit, SAL_USECS_SUB(etime, stime), rv));

thread_delay:
        /* Sleep for the specified interval, unless woken up by other events. */
        rv = sal_sem_take(IPMC_INFO(unit)->ipmc_age_sync,
                          interval * _USEC_IN_ONESEC);
        if (IPMC_INFO(unit)->ipmc_age_interval &&
            (interval != IPMC_INFO(unit)->ipmc_age_interval)) {
    ...
    for (chunk_index = 0; chunk_index < num_chunks; chunk_index++) {
        ...
        for (i = 0; i < (entry_index_max - entry_index_min + 1); i++) {
            ...
            if (SOC_IS_TRIDENT3X(unit)) {
                ...
                if ((soc_mem_field32_get(unit, v6mc_mem, v6mc_entry, BASE_VALID_0f) != 1) ||
                    (soc_mem_field32_get(unit, v6mc_mem, v6mc_entry, BASE_VALID_1f) != 2) ||
                    (soc_mem_field32_get(unit, v6mc_mem, v6mc_entry, BASE_VALID_2f) != 2) ||
                    (soc_mem_field32_get(unit, v6mc_mem, v6mc_entry, BASE_VALID_3f) != 7)) {
                    continue;
                }
            } else {
