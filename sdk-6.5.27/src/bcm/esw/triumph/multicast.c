...
bcm_trx_multicast_group_get(int unit, bcm_multicast_t group, uint32 *flags)
{
    ...
    if (_BCM_MULTICAST_IS_L2(group)) {
        ...
        if (soc_mem_field_valid(unit, EGR_IPMCm, DONT_PRUNE_VLANf)) {
            ...
            BCM_IF_ERROR_RETURN(READ_EGR_IPMCm(unit, MEM_BLOCK_ANY, mc_index,
                                &egr_ipmc_entry));
            if ((soc_mem_field32_get(unit, EGR_IPMCm, &egr_ipmc_entry,
                DONT_PRUNE_VLANf) == 1) &&
                !((get_group & _BCM_VIRTUAL_TYPES_MASK) &&
                  (get_group != BCM_MULTICAST_TYPE_WLAN))) {
                *flags |= BCM_MULTICAST_DISABLE_SRC_KNOCKOUT;
            }
        }
    }
