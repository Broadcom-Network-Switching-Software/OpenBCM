...
_bcm_tr_l2_from_l2x(int unit, bcm_l2_addr_t *l2addr, l2x_entry_t *l2x_entry)
{
    ...
    if (BCM_MAC_IS_MCAST(l2addr->mac)) {
        ...
        int                     dest_type_is_l3_ecmp = 0;
        uint32                  dest_l3_ecmp = 0;
        int                     l3_ecmp_index = -1;
...
#endif /* BCM_MONTEREY_SUPPORT && INCLUDE_L3 */
#if defined(BCM_TRIDENT3_SUPPORT) && defined(INCLUDE_L3)
        if (dest_type_is_l3_ecmp) {
            l2addr->flags2 |= BCM_L2_FLAGS2_L3_ECMP;
            dest_l3_ecmp = _BCM_L2X_MEMACC_FIELD32_GET(unit, l2x_entry, DESTINATION_f);
            l3_ecmp_index = (dest_l3_ecmp & (~_BCM_L2_DEST_TYPE_MASK));
            l2addr->egress_if = BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit) + l3_ecmp_index;
        } else
#endif /* BCM_TRIDENT3_SUPPORT && INCLUDE_L3 */
#endif /* INCLUDE_L3 */
...
#endif /* BCM_MONTEREY_SUPPORT && INCLUDE_L3 */
#if defined(BCM_TRIDENT3_SUPPORT) && defined(INCLUDE_L3)
        } else if (l2addr->flags2 & BCM_L2_FLAGS2_L3_ECMP) {
            if (!soc_feature(unit, soc_feature_generic_dest)) {
                return BCM_E_UNAVAIL;
            }
            l3_ecmp_intf = l2addr->egress_if;
#endif /* BCM_TRIDENT3_SUPPORT && INCLUDE_L3 */
        } else {
            BCM_IF_ERROR_RETURN(
...
#endif /* BCM_MONTEREY_SUPPORT && INCLUDE_L3 */
#if defined(BCM_TRIDENT3_SUPPORT) && defined(INCLUDE_L3)
        } else if (-1 != l3_ecmp_intf) {
            uint32 mpath_flag = 0;
            if (!BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit, l3_ecmp_intf)) {
                return BCM_E_PARAM;
            }
            /*
             * Get ecmp group index from ECMP object and
             * increment ECMP object reference count.
             */
            BCM_IF_ERROR_RETURN(
                bcm_xgs3_get_nh_from_egress_object(unit, l3_ecmp_intf, &mpath_flag,
                                                    1, &l3_ecmp_index));
            if (l3_ecmp_index >= BCMI_L3_ECMP_OVERLAY_ENTRIES(unit)) {
                /* L2 dest must be an overlay ECMP group */
                return BCM_E_PARAM;
            }
            soc_mem_field32_dest_set(unit, L2Xm, l2x_entry,
                DESTINATIONf, SOC_MEM_FIF_DEST_ECMP, l3_ecmp_index);
#endif /* BCM_TRIDENT3_SUPPORT && INCLUDE_L3 */
#if defined(BCM_GREYHOUND2_SUPPORT) && defined(INCLUDE_L3)
        } else if (-1 != vp) {
...
#if defined(BCM_TRIDENT3_SUPPORT) && defined(INCLUDE_L3)
    if (soc_feature(unit, soc_feature_generic_dest)) {
        uint32 dest, type, index;
        dest = soc_mem_field32_get(unit, L2Xm, &l2x_entry, DESTINATIONf);
        _soc_mem_dest_value_resolve(unit, dest, &type, &index);
        if (type == SOC_MEM_FIF_DEST_ECMP) {
            int width;
...
_tr_l2x_delete_all(int unit)
{
    ...
    if (SOC_MEM_BLOCK_VALID(unit, L2Xm, copyno)) {
        SOP_MEM_STATE_COUNT_SET(unit, L2Xm, copyno, 0);
    }
        ...
        if (soc_mem_field32_get(unit, tcam_mem, &entry, VALIDf) != 0) {
            ...
            if (SOC_IS_TRIDENT3X(unit) &&
                ...
                if (tcam_mem == MY_STATION_TCAMm) {
                    ...
                    rv = soc_mem_read(unit, MY_STATION_TCAM_2m, MEM_BLOCK_ANY,
                                      s_ent->hw_index, &entry);
                    if (BCM_FAILURE(rv)) {
                        SC_UNLOCK(sc);
                        return (rv);
                    }
                    if (soc_mem_field32_get(unit, MY_STATION_TCAM_2m, &entry,
                        VALIDf) != 0) {
                        mysta_profile_id = soc_mem_field32_dest_get(unit,
                                                MY_STATION_TCAM_2m, &entry,
                                                DESTINATIONf, &dest_type);
                        if (dest_type != SOC_MEM_FIF_DEST_MYSTA) {
                            SC_UNLOCK(sc);
                            return BCM_E_FAIL;
