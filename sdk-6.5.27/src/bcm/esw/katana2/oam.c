...
/*
 * Endpoint informations like Counter id, Fault flags, Persistent fault flags,
 * Interface status, Internal priority, VLAN tag priority,
 * Inner VLAN tag priority, Source MAC address and Destination MAC address
 */
#define _BCM_KT2_OAM_EP_INFO_SCACHE_SIZE                                  \
                (sizeof(int) + sizeof(uint32) + sizeof(uint32) +          \
                 sizeof(uint8) + sizeof(bcm_cos_t) + sizeof(uint8) +      \
                 sizeof(uint8) + sizeof(bcm_mac_t) + sizeof(bcm_mac_t))
...
_bcm_kt2_oam_handle_interrupt(int unit, soc_field_t field)
{
    ...
    uint32 rmep_cur_interface_status = 0;   /* Current RMEP Interface TLV from
                                               last received RMEP PDU.  */
    ...
    for (intr = _kt2_oam_interrupts; intr->status_register != INVALIDr;
        ...
        if ((1 == intr_cur_status) && (intr_count > 0)) {
            ...
            if (INVALIDf != intr->endpoint_index_field ) {
                ...
                rmep_cur_interface_status = soc_RMEPm_field32_get(unit, &rmep_entry,
            ...
            for (e_handler = oc->event_handler_list_p; e_handler != NULL;
                ...
                if (SHR_BITGET(e_handler->event_types.w, intr->event_type)) {
                    ...
                    if ((intr->event_type == bcmOAMEventEndpointInterfaceUp) ||
                        ...
                        switch (rmep_cur_interface_status) {
                            case _BCM_OAM_TLV_INTERFACE_UP:
                                if (remote_ep_index != BCM_OAM_ENDPOINT_INVALID) {
                                    if (h_data_p->rmep_last_if_status == _BCM_OAM_TLV_INTERFACE_DOWN) {
                                        intr->event_type = bcmOAMEventEndpointInterfaceUp;
                                    } else if (h_data_p->rmep_last_if_status == _BCM_OAM_TLV_INTERFACE_TESTING) {
                                        intr->event_type = bcmOAMEventEndpointInterfaceTestingToUp;
                                    } else if (h_data_p->rmep_last_if_status == _BCM_OAM_TLV_INTERFACE_UNKNOWN) {
                                        intr->event_type = bcmOAMEventEndpointInterfaceUnknownToUp;
                                    } else if (h_data_p->rmep_last_if_status == _BCM_OAM_TLV_INTERFACE_DORMANT) {
                                        intr->event_type = bcmOAMEventEndpointInterfaceDormantToUp;
                                    } else if (h_data_p->rmep_last_if_status == _BCM_OAM_TLV_INTERFACE_NOTPRESENT) {
                                        intr->event_type = bcmOAMEventEndpointInterfaceNotPresentToUp;
                                    } else if (h_data_p->rmep_last_if_status == _BCM_OAM_TLV_INTERFACE_LLDOWN) {
                                        intr->event_type = bcmOAMEventEndpointInterfaceLLDownToUp;
                                    }
                                }
                                break;
                    ...
                    if (remote_ep_index != BCM_OAM_ENDPOINT_INVALID) {
                        h_data_p->rmep_last_if_status = rmep_cur_interface_status;
                    }
                    if ((intr->event_type == bcmOAMEventEndpointInterfaceTestingToUp) ||
                        (intr->event_type == bcmOAMEventEndpointInterfaceUnknownToUp) ||
                        (intr->event_type == bcmOAMEventEndpointInterfaceDormantToUp) ||
                        (intr->event_type == bcmOAMEventEndpointInterfaceNotPresentToUp) ||
                        (intr->event_type == bcmOAMEventEndpointInterfaceLLDownToUp)) {
...
_bcm_kt2_oam_sync(int unit)
{
    ...
    for (idx = 0; idx < oc->ep_count; idx++) {
        h_data_p = &oc->oam_hash_data[idx];
        if (h_data_p->in_use) {
            sal_memcpy(oam_scache, &(h_data_p->lm_counter_base_id), sizeof(int));
            oam_scache += sizeof(int);
            sal_memcpy(oam_scache, &(h_data_p->faults), sizeof(uint32));
            oam_scache += sizeof(uint32);
            sal_memcpy(oam_scache, &(h_data_p->persistent_faults), sizeof(uint32));
            oam_scache += sizeof(uint32);
            sal_memcpy(oam_scache, &(h_data_p->interface_state), sizeof(uint8));
            oam_scache += sizeof(uint8);
            sal_memcpy(oam_scache, &(h_data_p->int_pri), sizeof(bcm_cos_t));
            oam_scache += sizeof(bcm_cos_t);
            sal_memcpy(oam_scache, &(h_data_p->vlan_pri), sizeof(uint8));
            oam_scache += sizeof(uint8);
            sal_memcpy(oam_scache, &(h_data_p->inner_vlan_pri), sizeof(uint8));
            oam_scache += sizeof(uint8);
            sal_memcpy(oam_scache, &(h_data_p->src_mac_address), sizeof(bcm_mac_t));
            oam_scache += sizeof(bcm_mac_t);
            sal_memcpy(oam_scache, &(h_data_p->dst_mac_address), sizeof(bcm_mac_t));
            oam_scache += sizeof(bcm_mac_t);
        } else {
            oam_scache += _BCM_KT2_OAM_EP_INFO_SCACHE_SIZE;
        }
    }
...
bcm_kt2_oam_scache_alloc(int unit)
{
     ...
     * Inner VLAN tag priority, Source MAC address and Destination MAC address
     */
    alloc_sz += (_BCM_KT2_OAM_EP_INFO_SCACHE_SIZE * oc->ep_count);
...
_bcm_kt2_oam_reinit(int unit)
{
    ...
    if (recovered_ver >= BCM_WB_VERSION_1_10) {
        for (idx = 0; idx < oc->ep_count; idx++) {
            h_data_p = &oc->oam_hash_data[idx];
            if (h_data_p->in_use) {
                sal_memcpy(&(h_data_p->lm_counter_base_id), oam_scache, sizeof(int));
                oam_scache += sizeof(int);
                sal_memcpy(&(h_data_p->faults), oam_scache, sizeof(uint32));
                oam_scache += sizeof(uint32);
                sal_memcpy(&(h_data_p->persistent_faults), oam_scache, sizeof(uint32));
                oam_scache += sizeof(uint32);
                sal_memcpy(&(h_data_p->interface_state), oam_scache, sizeof(uint8));
                oam_scache += sizeof(uint8);
                sal_memcpy(&(h_data_p->int_pri), oam_scache, sizeof(bcm_cos_t));
                oam_scache += sizeof(bcm_cos_t);
                sal_memcpy(&(h_data_p->vlan_pri), oam_scache, sizeof(uint8));
                oam_scache += sizeof(uint8);
                sal_memcpy(&(h_data_p->inner_vlan_pri), oam_scache, sizeof(uint8));
                oam_scache += sizeof(uint8);
                sal_memcpy(&(h_data_p->src_mac_address), oam_scache, sizeof(bcm_mac_t));
                oam_scache += sizeof(bcm_mac_t);
                sal_memcpy(&(h_data_p->dst_mac_address), oam_scache, sizeof(bcm_mac_t));
                oam_scache += sizeof(bcm_mac_t);
            } else {
                oam_scache += _BCM_KT2_OAM_EP_INFO_SCACHE_SIZE;
            }
    ...
    */
    if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit) && (stable_size > 0)) {
        ...
        if (recovered_ver < BCM_WB_VERSION_1_10) {
            realloc_size += (_BCM_KT2_OAM_EP_INFO_SCACHE_SIZE * oc->ep_count);
        }
    ...
    if ((old->ccm_period == BCM_OAM_ENDPOINT_CCM_PERIOD_DISABLED) &&
        ...
        BCM_IF_ERROR_RETURN
            (READ_LMEP_1m(unit, MEM_BLOCK_ANY, h_data_p->local_tx_index,
                          &lmep_1_entry));
        if (new->flags & BCM_OAM_ENDPOINT_UP_FACING) {
            soc_LMEP_1m_field32_set(unit, &lmep_1_entry, PP_PORTf, h_data_p->src_pp_port);
            ...
            soc_LMEP_1m_field32_set(unit, &lmep_1_entry, PP_PORTf, h_data_p->dst_pp_port);
            soc_LMEP_1m_field32_set(unit, &lmep_1_entry, QUEUE_NUMf,
                    SOC_INFO(unit).port_uc_cosq_base[h_data_p->dst_pp_port] + new->int_pri);
        }
...
bcm_kt2_oam_endpoint_create(int unit, bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    if (1 == remote)
        hash_data->faults = endpoint_info->faults;

    hash_data->lm_counter_base_id = endpoint_info->lm_counter_base_id;
    hash_data->persistent_faults = endpoint_info->persistent_faults;
    hash_data->interface_state = endpoint_info->interface_state;
    hash_data->int_pri = endpoint_info->int_pri;
    ...
    if (bcmOAMEndpointTypeEthernet == h_data_p->type) {
        if (1 == h_data_p->is_remote) {
            ...
            if (1 == h_data_p->local_tx_enabled) {
                ...
                endpoint_info->lm_counter_base_id = h_data_p->lm_counter_base_id;
                endpoint_info->persistent_faults = h_data_p->persistent_faults;
                endpoint_info->interface_state = h_data_p->interface_state;
                endpoint_info->int_pri = h_data_p->int_pri;
