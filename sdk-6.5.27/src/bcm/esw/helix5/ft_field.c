...
/*
 * Typedef:
 *      _ft_tracking_param_qual_map_t
 * Purpose:
 *      Flow group tracking param and qualifier map.
 */
typedef struct _ft_param_qual_map_s {
    uint16 param;
...
#define _FP_FT_TRACKING_PARAM_QUAL_MAP_ADD(_unit_, _map_db_, _flags_,          \
        ...
        do {                                                                   \
            sal_memset(&_pq_map_, 0, sizeof(_ft_param_qual_map_t));            \
            _pq_map_.flags              = (_flags_);                           \
            _pq_map_.param              = (_param_);                           \
            _pq_map_.qual               = (_qual_);                            \
            _pq_map_.offset              = (_start_);                          \
            _pq_map_.width              = (_width_);                           \
            _pq_map_.ft_type_bmp        = (_ft_type_bmp_);                     \
            rv = _field_ft_tracking_param_qual_map_insert((_unit_),            \
                        (_param_), &(_pq_map_), (_map_db_));                   \
            BCMI_IF_ERROR_CLEANUP(rv);                                         \
        } while(0)

/* Macro to update default stage of the tracking param */
#define _FP_FT_TRACKING_PARAM_QUAL_MAP_STAGE_SET(_unit_, _map_db_,             \
                                                 _param_, _stage_)             \
        do {                                                                   \
            rv = _field_ft_tracking_param_qual_map_stage_set(unit,             \
                            _param_, _stage_, _map_db_);                       \
        } while(0)

/* Macro to get stage pq map */
#define _FP_FT_PQ_STAGE_MAP_GET(_stg_, _pq_map_, _pq_smap_)                    \
    do {                                                                       \
        int ii;                                                                \
        _field_ft_tracking_param_qual_stage_map_t *_t_pq_smap_;                \
        if (_pq_map_ == NULL) {                                                \
            _pq_smap_ = NULL;                                                  \
            break;                                                             \
        }                                                                      \
        _pq_smap_ = NULL;                                                      \
        _t_pq_smap_ = (_pq_map_)->pq_smap;                                     \
        for (ii = 0; ii < (_pq_map_)->num_pq_smap; ii++) {                     \
            if ((_stg_ == bcmFlowtrackerStageDefault) ||                       \
                ((_t_pq_smap_)->stage == _stg_)) {                             \
                _pq_smap_ = _t_pq_smap_;                                       \
                break;                                                         \
            }                                                                  \
            (_t_pq_smap_) = (_t_pq_smap_)->next;                               \
        }                                                                      \
    } while(0)
...
typedef struct _field_ft_group_keygen_md_s {
    ...
    /* FT Group Type */
    bcm_flowtracker_group_type_t ft_group_type;
    ...
    int *new_qual)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit, "Custom Id=%d,"
                    " UDF chunk_bmap=0x%x Param hw_bmap=0x%x\n\r"),
                custom_id, obj_info->chunk_bmap, param_udf_hw_bmap));
    ...
    int *qual_cfg_parts)
{
    ...
    for (chunk = (_FP_FT_UDF_TOTAL_CHUNKS - 1); chunk >= 0; chunk--) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit,
                        "UDF qual hw_bmap=0x%x qual_bits=%d\n\r"),
                    hw_bmap, qual_bits));
        ...
        *qual_width += qual_bits;
        ...
        if ((offset % 16) != 0) {
            ...
            width = 8;
            LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit,
                            "UDF qual blk=%d off=%d width=%d\n\r"),
                        offset/16, offset, width));
...
_field_ft_tracking_param_qual_map_insert(int unit,
                uint16 param,
                _ft_param_qual_map_t *pq_map,
                _field_ft_tracking_param_qual_map_db_t *map_db)
{
    int iter = 0;
    int alloc = FALSE;
    _field_ft_tracking_param_qual_map_t *temp_map = NULL;
    _field_ft_tracking_param_qual_stage_map_t *temp_pq_smap = NULL;
    int stage_ft_type_list[4] = { 0,
        BCMI_FT_TYPE_F_IFT | BCMI_FT_TYPE_F_AIFT,
        BCMI_FT_TYPE_F_MFT | BCMI_FT_TYPE_F_AMFT,
    ...
    map_db->param_qual_map[param]->param = param;
    if (alloc == TRUE) {
        BCMI_FT_TYPE_BMP_STAGE_GET(pq_map->ft_type_bmp,
            map_db->param_qual_map[param]->default_stage);
    }
    map_db->param_qual_map[param]->db_ft_type_bmp |= pq_map->ft_type_bmp;

    for (iter = 1; iter < COUNTOF(stage_ft_type_list); iter++) {
        if (!(stage_ft_type_list[iter] & pq_map->ft_type_bmp)) {
            continue;
        }

        /* Allocate mem */
        temp_pq_smap = NULL;
        _FP_XGS3_ALLOC(temp_pq_smap,
                sizeof(_field_ft_tracking_param_qual_stage_map_t),
                "tracking param-qual stage map");
        if (temp_pq_smap == NULL) {
            return BCM_E_MEMORY;
        }

        temp_pq_smap->stage = iter;
        temp_pq_smap->flags = pq_map->flags;
        temp_pq_smap->qual = pq_map->qual;
        temp_pq_smap->offset = pq_map->offset;
        temp_pq_smap->width = pq_map->width;

        /* Add to map db */
        temp_pq_smap->next = map_db->param_qual_map[param]->pq_smap;
        map_db->param_qual_map[param]->pq_smap = temp_pq_smap;
        map_db->param_qual_map[param]->num_pq_smap++;
    }
...
/*
 * Function:
 *   _field_ft_tracking_param_qual_map_stage_set
 * Purpose:
 *   Set Stage for tracking param-qual map in map database.
 * Parameters:
 *   unit       - (IN) BCM device number.
 *   param      - (IN) tracking param
 *   stage      - (IN) Flowtracker Stage
 *   map_db     - (IN/OUT) param and qual Map database
 * Returns:
 *   BCM_E_NONE
 */
STATIC int
_field_ft_tracking_param_qual_map_stage_set(int unit,
                uint16 param, int stage,
                _field_ft_tracking_param_qual_map_db_t *map_db)
{
    /* Validate input params. */
    if ((map_db == NULL) || (param >= _FIELD_FT_TRACKING_PARAM_MAX)) {
        return BCM_E_PARAM;
    }

    if (map_db->param_qual_map[param] == NULL) {
        return BCM_E_INTERNAL;
    }

    map_db->param_qual_map[param]->default_stage = stage;
                                             ...
                                             _field_ft_info_t *ft_info)
{
    ...
    for (param = 0; param < _FIELD_FT_TRACKING_PARAM_MAX; param++) {
        if (map_db->param_qual_map[param]) {
            ...
            while(map_db->param_qual_map[param]->pq_smap) {
                temp_pq_smap = map_db->param_qual_map[param]->pq_smap;
                map_db->param_qual_map[param]->pq_smap = temp_pq_smap->next;
                sal_free(temp_pq_smap);
            }
...
/*
 * Function:
 *   _field_ft_stage_is_qual_supported
 * Purpose:
 *   Check if gien qualifier is supported or not
 * Parameters:
 *   unit       - (IN) BCM device number.
 *   qual       - (IN) Qualifier
 *   qual_cfg_info_db - (IN) Qualifier Db
 * Returns:
 *   BCM_E_XXX
 */
        ...
        bcmi_keygen_qual_cfg_info_db_t *qual_cfg_info_db)
{
    ...
    ft_type_bmp = (BCMI_FT_TYPE_F_IFT | BCMI_FT_TYPE_F_AIFT);
    _FP_FT_TRACKING_PARAM_QUAL_MAP_ADD(unit, map_db, flags,
            bcmFlowtrackerTrackingParamTypeForwardingOuterVlanTag,
            bcmFieldQualifyForwardingOuterVlan, 0, 16, ft_type_bmp);

    ft_type_bmp = (BCMI_FT_TYPE_F_IFT | BCMI_FT_TYPE_F_AIFT);
    _FP_FT_TRACKING_PARAM_QUAL_MAP_ADD(unit, map_db, flags,
            bcmFlowtrackerTrackingParamTypeForwardingInnerVlanTag,
            bcmFieldQualifyForwardingInnerVlan, 0, 16, ft_type_bmp);
    ...
    ft_type_bmp = (BCMI_FT_TYPE_F_IFT | BCMI_FT_TYPE_F_AIFT);
    _FP_FT_TRACKING_PARAM_QUAL_MAP_ADD(unit, map_db,
            flags, bcmFlowtrackerTrackingParamTypePacketTos,
            bcmFieldQualifyTos, 0, 8, ft_type_bmp);
    ...
    _FP_FT_TRACKING_PARAM_QUAL_MAP_ADD(unit, map_db, flags,
            ...
            bcmFieldQualifyInPort, 0, 8, ft_type_bmp);
    ...
    _FP_FT_TRACKING_PARAM_QUAL_MAP_ADD(unit, map_db, flags,
            ...
            0, 0, 0, ft_type_bmp);
    _FP_FT_TRACKING_PARAM_QUAL_MAP_STAGE_SET(unit, map_db,
            bcmFlowtrackerTrackingParamTypeTimestampCheckEvent1,
            bcmFlowtrackerStageDefault);
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_state_transition)) {
            ft_type_bmp = (BCMI_FT_TYPE_F_IFT);
            flags = (_FP_FT_TRACKING_PARAM_F_NO_QUAL_MAPPED |
                     _FP_FT_TRACKING_PARAM_F_NO_FILL);
            _FP_FT_TRACKING_PARAM_QUAL_MAP_ADD(unit, map_db, flags,
                    bcmFlowtrackerTrackingParamTypeFlowState,
                    0, 0, 8, ft_type_bmp);
    }
                ...
                _field_ft_group_keygen_md_t *ftg_keygen_md)
{
    ...
    bcm_field_qualify_t qual;
    bcm_flowtracker_stage_t ft_stage;
    bcm_flowtracker_check_t check_id;
    bcm_flowtracker_check_info_t fc_info;
    bcmi_keygen_qual_info_t *qual_info_arr = NULL;
    _field_ft_tracking_param_qual_map_db_t *map_db = NULL;
    _field_ft_tracking_param_qual_stage_map_t *pq_smap = NULL;
    _field_ft_group_keydata_keygen_md_t *sdata_keygen_md = NULL;
    ...
    for (idx = 0; idx < ftg_keygen_md->num_flowchecks; idx++) {
        ...
        for (p_idx = 0; p_idx < num_params; p_idx++) {
            ...
            BCMI_FT_TYPE_BMP_STAGE_GET((chk_ft_type_bmp & ext_ft_type_flag),
                            ft_stage);
            _FP_FT_PQ_STAGE_MAP_GET(ft_stage, map_db->param_qual_map[param], pq_smap);

            if ((param > _FIELD_FT_TRACKING_PARAM_MAX) || (pq_smap == NULL)) {
                rv = BCM_E_PARAM;
                BCMI_IF_ERROR_CLEANUP(rv);
            }

            if (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_NO_QUAL_MAPPED) {
                continue;
            }

            if (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_UDF_MAPPED) {
            ...
            if (!BCM_FIELD_QSET_TEST(sdata_keygen_md->qset, qual)) {
                ...
                _field_ft_keygen_qual_info_flags_set(unit,
                        0, pq_smap->flags, udf_qual_flags,
                        &qual_info_arr[new_idx].flags);
                    ...
                    _field_ft_group_keygen_md_t *ftg_keygen_md)
{
    ...
    uint32 udf_qual_flags = 0;
    bcm_flowtracker_stage_t ft_stage;
    uint16 param_stage_ft_type_bmp = 0;
    bcmi_keygen_qual_info_t *qual_info_arr = NULL;
    _field_ft_tracking_param_qual_stage_map_t *pq_smap = NULL;
    _field_ft_tracking_param_qual_map_db_t *map_db = NULL;
    ...
    for (idx = 0; idx < num_tracking_params; idx++) {
        ...
        if (tracking_param_info->param ==
            ...
            rv = bcmi_ft_group_check_validate(unit,
                     BCMI_FT_GROUP_CHECK_MATCH_GROUP, ftg_keygen_md->ft_group_id,
                     tracking_param_info->check_id, tracking_param_info->stage);
        ...
        /* Get Final stage and ft_type_bmp */
        BCMI_FT_PARAM_FINAL_STAGE_GET(ftg_keygen_md->ft_group_type,
                tracking_param_info->stage,
                map_db->param_qual_map[param]->default_stage, ft_stage);
        BCMI_FT_STAGE_FT_TYPE_BMP_GET(ft_stage,
                map_db->param_qual_map[param]->db_ft_type_bmp,
                param_stage_ft_type_bmp);
        _FP_FT_PQ_STAGE_MAP_GET(ft_stage, map_db->param_qual_map[param], pq_smap);
        if (pq_smap == NULL) {
            continue;
        }
        ...
        if (!(param_stage_ft_type_bmp & ext_ft_type_flag)) {
            continue;
        }
        ...
        _field_ft_keygen_qual_info_flags_set(unit, is_key,
            pq_smap->flags, udf_qual_flags, &qual_info_arr[new_idx].flags);
    ...
    for (idx = 0; idx < 2; idx++) {
        if (skey_keygen_md->qual_info_count > idx) {
            ...
            if ((qual == bcmFieldQualifySrcIp6) ||
                ...
                _FP_FT_PQ_STAGE_MAP_GET(bcmFlowtrackerStageIngress,
                        map_db->param_qual_map[param_info->param], pq_smap);
                if (pq_smap == NULL) {
                    return BCM_E_INTERNAL;
                }
                qual_size = pq_smap->width/8;
            ...
            _field_ft_group_keygen_md_t *ftg_keygen_md)
{
    ...
    for (idx = 0; idx < ftg_keygen_md->num_flowchecks; idx++) {
        ...
        BCMI_FT_TYPE_BMP_STAGE_GET(ext_ft_type_flag, ft_stage);
        _FP_FT_PQ_STAGE_MAP_GET(ft_stage,
                map_db->param_qual_map[fc_info.param], pq_smap);
        if (pq_smap == NULL) {
            continue;
        }
        if (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_NO_FILL) {
            continue;
        }

        if (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_NO_QUAL_MAPPED) {
            sdata_info_key->location = 0;
            sdata_info_key->length = pq_smap->width;
        ...
        if (!BCMI_FT_IDX_IS_2ND_CHECK(check_id)) {
            ...
            BCMI_FT_TYPE_BMP_STAGE_GET(ext_ft_type_flag, ft_stage);
            _FP_FT_PQ_STAGE_MAP_GET(ft_stage,
                    map_db->param_qual_map[action_info.param], pq_smap);

            /* Skip if non IFT/AIFT stage */
            if (!(map_db->param_qual_map[action_info.param]->db_ft_type_bmp &
                        ext_ft_type_flag)) {
                continue;
            }
            ...
            if (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_NO_QUAL_MAPPED) {
                ...
                sdata_info[sd_idx].action_key.location = pq_smap->offset;
                sdata_info[sd_idx].action_key.length = pq_smap->width;
        ...
        _field_ft_group_keygen_md_t *ftg_keygen_md)
{
    ...
    while(idx < ftg_keygen_md->num_tracking_params) {
        ...
        if ((!(param_stage_ft_type_bmp & ext_ft_type_flag)) ||
            (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_NO_FILL)) {
            idx++;
            continue;
        }
        ...
        if (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_NO_QUAL_MAPPED) {
            ...
            oper_info->sdata_info[data_idx].key1.is_alu =
                    ((pq_smap->flags & _FP_FT_TRACKING_PARAM_F_ALU_REQ) ? 1 : 0);
            oper_info->sdata_info[data_idx].key1.length = pq_smap->width;
            ...
            oper_info->sdata_info[data_idx].src_ts =
                                        tracking_param_info->src_ts;
            data_idx++;
        } else {
                                    ...
                                    _field_ft_group_keygen_md_t *ftg_keygen_md)
{
    ...
    for (idx = 0; idx < ftg_keygen_md->num_flowchecks; idx++) {
        ...
        BCMI_FT_TYPE_BMP_STAGE_GET((1 << ft_type_in), ft_stage);
        if (BCMI_FT_IDX_IS_2ND_CHECK(check_id)) {
            _FP_FT_PQ_STAGE_MAP_GET(ft_stage,
                    map_db->param_qual_map[fc_info.check2.param], pq_smap);
        } else {
            _FP_FT_PQ_STAGE_MAP_GET(ft_stage,
                    map_db->param_qual_map[fc_info.check1.param], pq_smap);
        }
        ...
        sdata_info_key->location = pq_smap->offset;
        sdata_info_key->length = pq_smap->width;
        sdata_info_key->is_alu = 1;
        ...
        if (!BCMI_FT_IDX_IS_2ND_CHECK(check_id)) {
            ...
            _FP_FT_PQ_STAGE_MAP_GET(ft_stage,
                map_db->param_qual_map[fc_info.action_info.param], pq_smap);
                 ...
                 _field_ft_group_keygen_md_t *ftg_keygen_md)
{
    ...
    while(idx < ftg_keygen_md->num_tracking_params) {
        ...
        if ((!(param_stage_ft_type_bmp & (1 << ft_type_in))) ||
        ...
        oper_info->sdata_info[data_idx].key1.location = pq_smap->offset;
        oper_info->sdata_info[data_idx].key1.is_alu  =
                    ((pq_smap->flags & _FP_FT_TRACKING_PARAM_F_ALU_REQ) ? 1 : 0);
        oper_info->sdata_info[data_idx].key1.length = pq_smap->width;
        oper_info->sdata_info[data_idx].element_type1 = param;
        ...
        _field_ft_group_keygen_md_t *ftg_keygen_md)
{
    ...
    while(idx < ftg_keygen_md->num_tracking_params) {
        ...
        BCMI_FT_TYPE_BMP_STAGE_GET(ext_ft_type_flag, ft_stage);
        _FP_FT_PQ_STAGE_MAP_GET(ft_stage, pq_map, pq_smap);
        if (pq_smap == NULL) {
            idx++;
            continue;
        }
        ...
        for (qual_idx = 0; qual_idx < qual_ext_offset_info->size; qual_idx++) {
            ...
            oper_info->skey_info[key_idx].custom_id1 =
                                    tracking_param_info->custom_id;
            oper_info->skey_info[key_idx].alu_ft_type = ext_ft_type;
            key_idx++;
            break;
        }
        ...
        int *valid_commands)
{
    ...
    _FP_FT_PQ_STAGE_MAP_GET(bcmFlowtrackerStageIngress,
            map_db->param_qual_map[param], pq_smap);
    if (pq_smap == NULL) {
        return BCM_E_INTERNAL;
    }
    src_qual = pq_smap->qual;
 ...
 *   extract_pos- (OUT) Offset in ALU Data vector.
 *   actual_pos - (OUT) Offset in Packet header.
                    ...
                    int *width)
{
    ...
    for (index = 0; index < num_alu_data; index++) {
        ...
        if (param == alu_data) {
            found = TRUE;
            if (extract_pos != NULL) {
                *extract_pos = alu_data_info[index].extract_pos;
            }
            if (actual_pos != NULL) {
                *actual_pos = alu_data_info[index].offset;
            }
            if (width != NULL) {
                                ...
                                uint8 save_output)
{
    ...
    ftg_keygen_md.ft_group_type = BCMI_FT_GROUP_TYPE_GET(id);
 ...
 * Purpose:
 *   Get TCP control flag offset from extraced position val.
 * Parameters:
 *   unit        - (IN) BCM device number.
 *   extract_pos - (IN) Extract position
 *   offset      - (OUT) bit offset
 * Returns:
 *   BCM_E_XXX
 */
int
_bcm_field_ft_alu_data_offset_get(int unit,
                                int extract_pos,
                                int *offset)
{
    int rv = BCM_E_NONE;
    ...
    for (index = 0; index < num_alu_data; index++) {
        if (alu_data_info[index].extract_pos == extract_pos) {
 ...
 * Purpose:
 *   Get extract position for TCP control flag.
 * Parameters:
 *   unit        - (IN) BCM device number.
 *   offset      - (IN) bit offset
 *   extract_pos - (OUT) Extract position
 * Returns:
 *   BCM_E_XXX
 */
int
_bcm_field_ft_alu_data_extract_pos_get(int unit,
                                int offset,
                                int *extract_pos)
{
    int rv = BCM_E_NONE;
    ...
    for (index = 0; index < num_alu_data; index++) {
        if (alu_data_info[index].offset == offset) {
            *extract_pos = alu_data_info[index].extract_pos;
            break;
        }
    }
                                  ...
                                  uint32 *max_value)
{
    ...
    /* Get stage */
    if (ft_stage == bcmFlowtrackerStageDefault) {
        ft_stage = pq_map->default_stage;
    }
    _FP_FT_PQ_STAGE_MAP_GET(ft_stage, pq_map, pq_smap);

    /* Return error if not present in db */
    if ((pq_map == NULL) || (pq_smap == NULL)) {
        return BCM_E_PARAM;
    }
    ...
    if (!(pq_smap->flags & _FP_FT_TRACKING_PARAM_F_NO_QUAL_MAPPED)) {
        ...
        if ((pq_map->db_ft_type_bmp & (BCMI_FT_TYPE_F_IFT |
            ...
            keygen_cfg.qual_cfg_info_db = ft_info->flow_key_qual_cfg;
            rv = bcmi_keygen_qual_cfg_max_size_get(unit,
                    &keygen_cfg, pq_smap->qual, &max_bits);
            BCM_IF_ERROR_RETURN(rv);
        } else {
            max_bits = pq_smap->width;
        }
    }
...
_bcm_field_ft_tracking_param_type_get(int unit,
                                    bcm_flowtracker_group_type_t group_type,
                                    bcm_flowtracker_stage_t param_stage,
                                    bcm_flowtracker_tracking_param_type_t param,
                                    uint16 custom_id,
                                    uint32 flags,
                                    uint16 *ft_type_bmp)
{
    ...
    _field_stage_t *stage_fc = NULL;
    _field_ft_info_t *ft_info = NULL;
    bcm_flowtracker_stage_t final_stage;
    bcm_flowtracker_udf_info_t param_udf_info;
    _field_ft_tracking_param_qual_map_t *pq_map = NULL;
    _field_ft_tracking_param_qual_map_db_t *map_db = NULL;
    _field_ft_tracking_param_qual_stage_map_t *pq_smap = NULL;
    ...
    const char *stagelist[] = BCM_FLOWTRACKER_STAGE_STRINGS;
    ...
    BCMI_FT_PARAM_FINAL_STAGE_GET(group_type, param_stage,
                       pq_map->default_stage, final_stage);

    _FP_FT_PQ_STAGE_MAP_GET(final_stage, pq_map, pq_smap);

    /* Return error if not present in db */
    if (pq_smap == NULL) {
        if (param < _FIELD_FT_TRACKING_ELEMENT_TYPE_MAX) {
            LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
               "Tracking param <%s, %s> is not available\n"),
               stagelist[final_stage], plist[param]));
        } else if (param < _FIELD_FT_TRACKING_PARAM_MAX) {
            LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
               "Tracking param <%s, %s> is not available\n"), stagelist[final_stage],
    ...
    if (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_UDF_MAPPED) {
        ...
        if ((!(flags & _BCM_FIELD_FT_TRACKING_PARAM_TYPE_GET_F_KEY)) &&
    ...
    /* Return ft_type_bmp based on stage, flags */
    if (flags & _BCM_FIELD_FT_TRACKING_PARAM_TYPE_GET_F_STAGE_FT_TYPE_BMP) {
        if (final_stage == bcmFlowtrackerStageDefault) {
            BCMI_FT_PARAM_DEF_FT_TYPE_BMP_GET(pq_map->default_stage,
                    pq_map->db_ft_type_bmp, *ft_type_bmp);
        } else {
            BCMI_FT_STAGE_FT_TYPE_BMP_GET(final_stage,
                    pq_map->db_ft_type_bmp, *ft_type_bmp);
        }
    } else if (flags & _BCM_FIELD_FT_TRACKING_PARAM_TYPE_GET_F_IGNORE_GROUP) {
        if (param_stage == bcmFlowtrackerStageDefault) {
            BCMI_FT_PARAM_DEF_FT_TYPE_BMP_GET(pq_map->default_stage,
                    pq_map->db_ft_type_bmp, *ft_type_bmp);
        } else {
            BCMI_FT_STAGE_FT_TYPE_BMP_GET(param_stage,
                    pq_map->db_ft_type_bmp, *ft_type_bmp);
        }
    } else {
            ...
            _field_ft_tracking_param_qual_map_db_t *map_db)
{
    ...
    if (soc_format_field_valid(unit, fmt, PACKET_TOSf)) {
        ...
        if (field_info != NULL) {
            ...
            _FP_FT_TRACKING_PARAM_QUAL_MAP_ADD(unit, map_db, flags,
                    bcmFlowtrackerTrackingParamTypePacketTos,
                    0, field_info->bp, field_info->len, ft_type_bmp);
            _FP_FT_TRACKING_PARAM_QUAL_MAP_STAGE_SET(unit, map_db,
                    bcmFlowtrackerTrackingParamTypePacketTos,
                    bcmFlowtrackerStageMmu);
        }
    }
    ...
    for (param = bcmFlowtrackerTrackingParamTypeNone;
        ...
        if (pq_map->db_ft_type_bmp & ft_type_bmp) {
            LOG_CLI((BSL_META_U(unit, "  %40s | %4d\n\r"),
                   tracking_param_str[param], pq_map->pq_smap->width/8));
        }
        pq_map = NULL;
    }
    ...
    uint16 *width)
{
    ...
    map_db = ft_info->param_qual_map_db;
    pq_map = map_db->param_qual_map[param];
    if ((pq_map == NULL) || (pq_map->pq_smap == NULL)) {
        return FALSE;
    }

    /* Since Udf is supported only in ingress stage */
    if (pq_map->pq_smap->flags & _FP_FT_TRACKING_PARAM_F_UDF_MAPPED) {
        *width = pq_map->pq_smap->width;
        return TRUE;
    }
...
_bcm_field_ft_tracking_param_show(int unit)
{
    ...
    for (param = 0; param < bcmFlowtrackerTrackingParamTypeCount; param++) {
        ...
        LOG_CLI((BSL_META_U(unit, "  %40s | %4d "),
                    param_str[param], pq_map->pq_smap->width/8));

        if ((pq_map->pq_smap != NULL) &&
                (pq_map->pq_smap->flags & _FP_FT_TRACKING_PARAM_F_UDF_MAPPED)) {
...
int
_bcm_field_ft_tracking_param_def_stage_get(int unit,
        bcm_flowtracker_group_type_t group_type,
        bcm_flowtracker_tracking_param_type_t param,
        bcm_flowtracker_stage_t *def_stage)
{
    ...
    *def_stage = bcmFlowtrackerStageDefault;
    ...
    if (map_db->param_qual_map[param]) {
        ...
        BCMI_FT_PARAM_FINAL_STAGE_GET(group_type, bcmFlowtrackerStageDefault,
                map_db->param_qual_map[param]->default_stage, *def_stage);
    }
...
int
_bcm_field_ft_tracking_param_type_info_get(int unit,
        bcm_flowtracker_group_type_t group_type,
        bcm_flowtracker_tracking_param_type_t param,
        bcm_flowtracker_tracking_param_type_info_t *info)
{
    int idx = 0;
    int stage_list_count = 0;
    uint16 ft_type_bmp = 0;
    _field_stage_t *stage_fc = NULL;
    _field_ft_info_t *ft_info = NULL;
    _field_ft_tracking_param_qual_map_t *pq_map = NULL;
    _field_ft_tracking_param_qual_map_db_t *map_db = NULL;
    _field_ft_tracking_param_qual_stage_map_t *pq_smap = NULL;
    bcm_flowtracker_stage_t stage_list[bcmFlowtrackerStageCount];
    ...
    switch (group_type) {
        case bcmFlowtrackerGroupTypeNone:
            stage_list[0] = bcmFlowtrackerStageIngress;
            stage_list[1] = bcmFlowtrackerStageMmu;
            stage_list[2] = bcmFlowtrackerStageEgress;
            stage_list_count = 3;
            ft_type_bmp = BCMI_FT_TYPE_F_MICRO;
            break;
        ...
        case bcmFlowtrackerGroupTypeAggregateMmu:
            stage_list[0] = bcmFlowtrackerStageMmu;
            stage_list_count = 1;
            ft_type_bmp = BCMI_FT_TYPE_F_AMFT;
            break;
        case bcmFlowtrackerGroupTypeAggregateEgress:
            stage_list[0] = bcmFlowtrackerStageEgress;
            stage_list_count = 1;
            ft_type_bmp = BCMI_FT_TYPE_F_AEFT;
            break;
        default:
            return BCM_E_PARAM;
    }

    /* Check if ft type bmp matches */
    if ((pq_map->db_ft_type_bmp & ft_type_bmp) == 0) {
        return BCM_E_UNAVAIL;
    }

    for (idx = 0; idx < stage_list_count; idx++) {
        _FP_FT_PQ_STAGE_MAP_GET(stage_list[idx], pq_map, pq_smap);
        if (pq_smap == NULL) {
            continue;
        }

        SHR_BITSET(info->stage_bmp, stage_list[idx]);

        if (pq_smap->flags & _FP_FT_TRACKING_PARAM_F_UDF_MAPPED) {
            info->flags |= BCM_FLOWTRACKER_TRACKING_PARAM_TYPE_INFO_T_UDF_MAPPED;
        }
    }

    if (stage_list_count == 1) {
        info->default_stage = stage_list[0];
    } else {
        info->default_stage = pq_map->default_stage;
    }
