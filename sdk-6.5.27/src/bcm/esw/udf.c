...
int bcm_esw_udf_tunnel_flow_based_chunk_arrange_set(
        int unit, 
        ...
        uint32 chunk_bmap)
{
    ...
    if (soc_feature(unit, soc_feature_udf_td3x_support)) {
       ...
       _BCM_UDF_LOCK(unit);
       if (UDF_CONTROL(unit)->functions.udf_tunnel_flow_based_chunk_arrange_set != NULL) {
          rv = UDF_CONTROL(unit)->functions.udf_tunnel_flow_based_chunk_arrange_set(
                                          unit,
                                          flow_type,
                                          chunk_bmap);
       }
...
int bcm_esw_udf_tunnel_flow_based_chunk_arrange_get(
        int unit, 
        bcm_udf_tunnel_flow_type_t flow_type, 
        uint32 *chunk_bmap)
{
    ...
    if (soc_feature(unit, soc_feature_udf_td3x_support)) {
       ...
       _BCM_UDF_LOCK(unit);
       if (UDF_CONTROL(unit)->functions.udf_tunnel_flow_based_chunk_arrange_get != NULL) {
          rv = UDF_CONTROL(unit)->functions.udf_tunnel_flow_based_chunk_arrange_get(
                                          unit,
                                          flow_type,
                                          chunk_bmap);
       }
...
int 
bcm_esw_udf_tunnel_type_offset_max_get(
    int unit, 
    ...
    uint32 *offset_max)
{
    ...
    if (soc_feature(unit, soc_feature_udf_td3x_support)) {
       ...
       _BCM_UDF_LOCK(unit);
       if (UDF_CONTROL(unit)->functions.udf_tunnel_type_offset_max_get != NULL) {
          rv = UDF_CONTROL(unit)->functions.udf_tunnel_type_offset_max_get(
                                            unit,
                                            tunnel_type,
                                            offset_max);
       }
