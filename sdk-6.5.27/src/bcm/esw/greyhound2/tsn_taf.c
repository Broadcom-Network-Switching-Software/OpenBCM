...
/* The TAF profile data structure used in warmboot v1.0(pre SDK 6.5.26) */
#define BCMI_TSN_TAF_CALENDAR_TABLE_SIZE_WB_V1_0 (16)
/* The TAF profile data structure */
typedef struct bcmi_tsn_taf_profile_wb_v1_0_s {
    int num_entries;
    bcm_tsn_taf_entry_t entries[BCMI_TSN_TAF_CALENDAR_TABLE_SIZE_WB_V1_0];
    bcm_ptp_timestamp_t ptp_base_time;
    uint32 ptp_cycle_time;
    uint32 ptp_max_cycle_extension;
} bcmi_tsn_taf_profile_wb_v1_0_t;

/* restore taf info from scache and hw */
STATIC int
bcmi_gh2_taf_reload(
    int unit, uint8 *scache_ptr, uint16 wb_ver)
{
    ...
    for (entry_index = 0; entry_index < conut_profile; entry_index++) {
        ...
        if (wb_ver < BCM_TSN_WB_VERSION_1_1) {
            bcmi_tsn_taf_profile_wb_v1_0_t profile_wb_v1_0;
            bcm_tsn_taf_profile_t *new_data = NULL;
            READ_PTR_RANGE(scache_ptr, sizeof(bcmi_tsn_taf_profile_wb_v1_0_t),
                           &profile_wb_v1_0);
            new_data = &new_profile->data;
            new_data->num_entries = profile_wb_v1_0.num_entries;
            if (new_data->num_entries > \
                BCMI_TSN_TAF_CALENDAR_TABLE_SIZE_WB_V1_0) {
                return BCM_E_INTERNAL;
            }
            sal_memcpy(&(new_data->entries),
                       &profile_wb_v1_0.entries,
                       sizeof(bcm_tsn_taf_entry_t) * new_data->num_entries);
            sal_memcpy(&(new_data->ptp_base_time),
                       &profile_wb_v1_0.ptp_base_time,
                       sizeof(bcm_ptp_timestamp_t));
            new_data->ptp_cycle_time = profile_wb_v1_0.ptp_cycle_time;
            new_data->ptp_max_cycle_extension = \
                profile_wb_v1_0.ptp_max_cycle_extension;
        } else {
