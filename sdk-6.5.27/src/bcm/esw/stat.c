...
bcm_esw_stat_init(int unit)
{
    ...
    if (soc_property_get_str(unit, spn_BCM_STAT_PBMP) == NULL) {
        ...
        /* Add COE subports to counter pbmp */
        if (soc_feature(unit, soc_feature_subtag_counters)) {
            SOC_PBMP_OR(pbmp, SOC_INFO(unit).subtag_pp_port_pbm);
        }
    } else {
#if defined (BCM_KATANA2_SUPPORT)
        if (SOC_IS_KATANA2(unit)) {
    ...
    if ((!SAL_BOOT_SIMULATION) || (SAL_BOOT_BCMSIM)) {
        if ((!SAL_BOOT_BCMSIM) || (NULL != _bcm_stat_extra_counters)) {
            ...
            if (!SOC_FAST_REBOOT(unit) && !SOC_HW_RESET(unit)) {
                                 ...
                                 bcm_stat_val_t *stat_arr, uint64 *value_arr)
{
    ...
    /* Activate counter collection preempt if enabled */
    soc_counter_collection_preempt_active(unit, 1);
    ...
    /* Deactivate counter collection preempt if enabled */
    soc_counter_collection_preempt_active(unit, 0);
...
bcm_esw_stat_clear(int unit, bcm_port_t port)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe)) {
     ...
     if (soc_feature(unit, soc_feature_subtag_coe) &&
         !soc_feature(unit, soc_feature_subtag_counters)) {
...
/*
 * Function:
 *      _bcm_esw_stat_dbgc_is_rsv
 * Description:
 *      Check if identified TDBGC/RDBGC is reserved. 
 *      Reserved TDBGC/RDBGC cannot be remapped by customer.
 * Parameters:
 *      unit - StrataSwitch PCI device unit number.
 *      type  - SNMP statistics type.
 * Returns:
 *      0 for unresrevd. Otherwise for reserved.
 * Notes:
 *      The following debug counters are reserved for maintaining
 *      currently supported SNMP MIB objects:
 *
 *      snmpBcmCustomReceive0 - snmpBcmCustomReceive15
 *      snmpBcmCustomTransmit0 - snmpBcmCustomTransmit15
 */
int
_bcm_esw_stat_dbg_cntr_is_rsv(int unit, bcm_stat_val_t type)
{
    int id;
    char str[32] = {0};

    if (type >= snmpBcmCustomReceive1 && type <= snmpBcmCustomReceive2) {
        id = type - snmpBcmCustomReceive0;
        sal_sprintf(str, "%s_%d", spn_RESERVED_RDBGC, id);
        return soc_property_get(unit, str, 1);
    } else if (type >= snmpBcmCustomTransmit0 && type <= snmpBcmCustomTransmit5) {
        return 1;
    } else {
        return 0;
    }
		    ...
		    bcm_stat_val_t type, uint32 flags)
{
    ...
    if (_bcm_esw_stat_dbg_cntr_is_rsv(unit, type)) {
