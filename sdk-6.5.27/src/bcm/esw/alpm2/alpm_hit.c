...
alpm_pvt_hit_get(int u, _alpm_cb_t *acb, _alpm_pvt_node_t *pvt_node)
{
    ...
    if (ACB_HAS_TCAM(acb)) { /* TCAM pre-pivot cb=0 */
        ...
        rv = tcam_hit_get(u, pvt_node, acb->app, pkm, tcam_idx);
...
/* ALPM pivot hit clear callback if BPM = route */
static int
alpm_ppg_hit_clear_cb(alpm_lib_trie_node_t *trie, alpm_lib_trie_bpm_cb_info_t *info)
{
    ...
    rv = alpm_pvt_hit_clear(unit, acb, pvt_node);
...
/* Get TCAM entry hitbit status from pivot's tcam_idx (from hw) */
int
tcam_hit_get(int u, _alpm_pvt_node_t *pvt_node, int app, int pkm, int tcam_idx)
{
    int         rv = BCM_E_UNAVAIL;

    rv = ALPM_DRV(u)->tcam_hit_get(u, pvt_node, app, pkm, tcam_idx);
...
int
alpm_ppg_hitbit_clear(int u, int app, int vrf_id, int ipt, uint32 *key, int key_len)
{
    _alpm_ppg_data_t ppg_data;
    ...
    return alpm_ppg_op(u, ALPM_PPG_HITCLR, alpm_ppg_hit_clear_cb, &ppg_data);
}
...
bcm_esw_alpm_clear_hit(int u, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (pvt_node && bkt_node) {
        ...
        if (!ALPM_TCAM_HB_IS_PTR(u)) {
            /* propagation to clear pivot hitbit with BPM = route */
            rv = alpm_ppg_hitbit_clear(u, app, vrf_id, ipt,
...
alpm_dist_hitbit_thread(int u)
{
    int i, pid;
    _alpm_cb_t *acb;
    sal_usecs_t t1=0, t2=0, t3=0, t4=0, t5=0;
    int app = APP0;
    int dbg = ALPMC(u)->_alpm_dbg_info_bmp & _ALPM_DBG_INFO_HIT;
    ...
    while (ALPMDH(u)->interval != 0) {
        ...
        if (dbg) {
            t5 = sal_time_usecs();
            ALPM_INFO(("\nTime: %lu (sec), latency: %d %d %d %d (us)\n",
                       sal_time(), t2-t1, t3-t2, t4-t3, t5-t4));
        }
        ALPM_HIT_UNLOCK(u);
