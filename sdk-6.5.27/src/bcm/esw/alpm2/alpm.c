...
/* propagation callback function for assoc_data and hit_ptr */
static int
alpm_ppg_assoc_data_and_hitbit_cb(alpm_lib_trie_node_t *trie, alpm_lib_trie_bpm_cb_info_t *info)
{
    ...
    if (pvt_def == NULL || ppg_def == NULL || pvt_def == ppg_def) {
        ...
        /* update pivot if assoc_data or hit_ptr changed */
        if ((sal_memcmp(&pvt_def->adata, &ppg_def->adata,
                        sizeof(_alpm_bkt_adata_t)) != 0) ||
            (!ALPM_HIT_SKIP(unit) && ALPM_TCAM_HB_IS_PTR(unit) &&
             pvt_def->ent_idx != ppg_def->ent_idx)) {
            update_pvt = 1;
        }
    }
...
/* propagation for assoc_data and hit_ptr */
int
alpm_ppg_assoc_data_and_hitbit(int u, int ppg_op, _alpm_ppg_data_t *ppg_data)
{
    return alpm_ppg_op(u, ppg_op, alpm_ppg_assoc_data_and_hitbit_cb, ppg_data);
}
                ...
                _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    int found = 0;
    if (ACB_HAS_TCAM(acb)) {
        rv = bcm_esw_alpm_tcam_insert(u, lpm_cfg, ALPM_PVT_UPDATE_ALL, &found);
    } else {
...
alpm_pvt_update(int u, _alpm_cb_t *acb, _bcm_defip_cfg_t *lpm_cfg, uint32 write_flags)
{
    ...
    if (ACB_HAS_TCAM(acb)) {
        ...
        rv = bcm_esw_alpm_tcam_insert(u, lpm_cfg, write_flags, &found);
        if (BCM_FAILURE(rv)) {
...
int
alpm_bkt_pfx_shrink(int u, _alpm_cb_t *acb, _alpm_pvt_node_t *pvt_node,
                    void *user_data, int *shrink_bnks)
{
    int         i, rv = BCM_E_NONE;
    uint32      *bak_idx = NULL;
    alpm_lib_trie_t      *bkt_trie = NULL;
        ...
        if (shrink_empty_only) { /* has empty bnks */
            ...
            if (used_bnks > 1) {
                ...
                if (BCM_SUCCESS(rv)) {
                    ...
                    nbkt_release = 0; /* OK new bkt prefix merge */

                    if (shrink_bnks) {
                        *shrink_bnks = used_bnks - req_bnks;
                    }
...
bad:
    /* Release new allocated bucket if prefix merge failed */
    if (nbkt_release) {
        if (nbkt_release == 2) {
                    ...
                    int *max_spl_cnt)
{
    ...
    if (lpm_cfg->spl_key_len != 0) {
        ...
        rv = alpm_lib_trie_split(pbt, max_split_len, pvt_key, &pvt_len,
                 ...
                 ACB_BKT_FIXED_FMT(acb, vrf_id) ? alpm_lib_trie_splitable2 : NULL,
               ...
               _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (ACB_HAS_RTE(acb, ALPM_LPM_VRF_ID(u, lpm_cfg))) {
        ...
        ALPM_IER(alpm_ppg_assoc_data_and_hitbit(u, ALPM_PPG_INSERT, &pdata));
    }
    return rv;
}
                      ...
                      _bcm_defip_cfg_t *lpm_cfg, void *udata)
{
    ...
    if (vrf_id == BCM_L3_VRF_GLOBAL) {
        ...
        ALPM_ERR(("alpm_vrf_pfx_trie_add: unexpected vrf_id %d\n", vrf_id));
    }
                         ...
                         uint32 *alt_bpm_len)
{
    ...
    if (vrf_id == BCM_L3_VRF_GLOBAL) {
        ...
        ALPM_ERR(("alpm_vrf_pfx_trie_delete: unexpected vrf_id %d\n", vrf_id));
    }
              ...
              _alpm_bkt_info_t *nbkt_info, int max_spl_cnt)
{
    ...
    if (!ACB_BKT_FIXED_FMT(acb, 1) && ACB_HAS_TCAM(acb)) {
        ...
        if (rv == BCM_E_FULL) {
            /* Need reserve more than (retry_max - split_retry) free TCAM entries */
            int rsvd = ALPM_SPLIT_RETRY_MAX(u) - ACB_SPLIT_RETRY(acb);
     ...
     */
    rv = alpm_bkt_pfx_shrink(u, acb, opvt_node, &shrink_empty_only, NULL);
    ALPM_IEG(rv);
...
alpm_cb_delete(int u, _alpm_cb_t *acb, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (ACB_HAS_RTE(acb, vrf_id)) {
        ...
        rv = alpm_ppg_assoc_data_and_hitbit(u, ALPM_PPG_DELETE, &ppg_data);
        ALPM_IEG(rv);
    }
    ...
    if (ACB_BKT_WRA(acb, PVT_BKT_VRF(pvt_node))) {
        if (bkt_empty) {
            ...
            rv = alpm_bkt_pfx_shrink(u, acb, pvt_node, &shrink_empty_only, NULL);
            ALPM_IEG(rv);
        }
    } else {
...
retry_spl:
    if (rv == BCM_E_FULL && retry_count < ALPM_SPLIT_RETRY_MAX(u)) {
        int     avl_bnk_cnt = 0;
    ...
    if (ACB_HAS_RTE(acb, vrf_id)) {
        ...
        rv = alpm_ppg_assoc_data_and_hitbit(u, ALPM_PPG_INSERT, &ppg_data);
    }
    if (BCM_SUCCESS(rv) &&
        ALPMC(u)->_alpm_dbg_bmp & _ALPM_DBG_PVT_SANITY_NODE) {
        rv = alpm_cb_pvt_sanity_cb(u, acb, pvt_node, "CBInsert");
    ...
    /* Count c_spl_retry if successful, otherwise count by c_full */
    if (retry_count > 0) {
        int spl = retry_count - 1;
        if (spl > 2) {
            spl = 2;
        }
        acb->acb_cnt.c_spl_retry[spl]++;
    }
...
bcm_esw_alpm_ctrl_init(int u)
{
    ...
    /* Bucket split retry max for FIXED_FMT should be always 1 */
    ALPM_SPLIT_RETRY_MAX(u) = SOC_IS_TOMAHAWK3(u) ? 3 : 1;
...
bcm_esw_alpm_ctrl_cleanup(int u)
{
    ...
    /* reset defrag_status after delete_all */
    ALPMC(u)->_alpm_defrag_status = ALPM_DEFRAG_OK;
                             ...
                             alpm_lib_trie_traverse_state_t *state, void *info)
{
    ...
    *state = trieTraverseStateNone;
    ...
    /* Finish pfx traverse if destroyed */
    if (ALPM_VRF_TRIE(trav_st->unit, trav_st->app,
                      trav_st->vrf_id, trav_st->ipt) == NULL) {
...
#define _BKT_POOL_USAGE_PRINT(acb_idx, pid, bp_conf, ipt)                   \
        ...
        cli_out("\tSMCS221: %8d\n", acb->acb_cnt.c_spl_mp_cs221);
        cli_out("\tSMCS222: %8d\n", acb->acb_cnt.c_spl_mp_cs222);
        cli_out("\tSplitRetry: %d %d %d\n", acb->acb_cnt.c_spl_retry[0],
                acb->acb_cnt.c_spl_retry[1], acb->acb_cnt.c_spl_retry[2]);
        cli_out("\tFULL:    %8d\n", acb->acb_cnt.c_full);
        if (ALPMC(u)->_alpm_defrag_on_full) {
...
alpm_route_projection(int u, int v4v6_ratio)
{
    ...
    cli_out("\t\t Routes L1_usage L2_usage %sL1_used:total L2_used:total%s\n",
            levels > 1 ? "L3_usage " : "",
            levels > 1 ? " L3_used:total" : "");
    for (grp = 0; grp < ALPM_STD_ROUTE_GROUP_COUNT; grp++) {
        ...
        cli_out("%s:%7d  %3d.%02d  %3d.%02d",
                alpm_route_grp_name[grp],
                p->route_cnt,
                usage[0]/100, usage[0]%100,
                usage[1]/100, usage[1]%100);
        if (levels > 1) {
            cli_out("  %3d.%02d", usage[2]/100, usage[2]%100);
        }
        cli_out(" %6d:%6d  %6d:%6d", p->lvl_usage[0].cnt_used, p->lvl_usage[0].cnt_total,
                                  ...
                                  int *prj_route, int *def_lvl, int *def_usage)
{
    ...
    *prj_route = 0;
    ...
    case bcmL3RouteGroupOverrideV4:
    case bcmL3RouteGroupOverrideV6:
        if (res->lvl_usage[0].cnt_total == 0) {
            max_usage = 0;
        } else {
...
bcm_esw_alpm_resource_check_and_free(int u, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (ALPMC(u)->_alpm_defrag_status == ALPM_DEFRAG_END_NO_MERGE &&
        ...
        rv = bcm_esw_alpm_defrag(u, BCM_L3_VRF_ALL, bcmL3AlpmIptAll,
                                 ...
                                 ALPMC(u)->_alpm_defrag_max_l2cnt,
                                 ALPMC(u)->_alpm_defrag_max_l3cnt,
                                 &l2cnt, &l3cnt, &l2shk, &l3shk);

        acb->acb_cnt.c_defragonfull++;
#ifdef DEFRAG_RESULT
        cli_out("ALPM defrag on full %d (l2merge %d l2shrink %d l3merge %d l3shrink %d)\n",
                acb->acb_cnt.c_defragonfull, l2cnt, l2shk, l3cnt, l3shk);
#endif
    }
