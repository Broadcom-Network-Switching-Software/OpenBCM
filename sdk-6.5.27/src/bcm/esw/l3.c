...
typedef struct _dlb_entry_array_s {
    int pipe;
    int index;
...
#define BCM_WB_VERSION_1_22               SOC_SCACHE_VERSION(1,22)
#define BCM_WB_VERSION_1_23               SOC_SCACHE_VERSION(1,23)
#define BCM_WB_VERSION_1_24               SOC_SCACHE_VERSION(1,24)
#define BCM_WB_DEFAULT_VERSION            BCM_WB_VERSION_1_24
...
#if defined(BCM_TOMAHAWK3_SUPPORT)
    /* In TH3, DGM and RH groups are in both overlay and underlay */
...
#ifdef BCM_RIOT_SUPPORT
    if (soc_feature(unit, soc_feature_riot) &&
        (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_23)) {
        alloc_sz +=  BCM_XGS3_L3_NH_TBL_SIZE(unit) * sizeof(uint8);
    }
#endif /* BCM_RIOT_SUPPORT */
...
#define TH3_IS_HECMP_GRP_IDX(unit, idx) \
    ...
    if (soc_feature(unit, soc_feature_riot) &&
        (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_23)) {
        SOC_SCACHE_DUMP_DATA_BEGIN("idx;l3_nh_tbl_sw_link_flags");
        for (i = 0; i < BCM_XGS3_L3_NH_TBL_SIZE(unit); i++) {
            flag = BCM_XGS3_L3_ENT_OL_UL_LINK(BCM_XGS3_L3_TBL_PTR(unit, next_hop), i);
            sal_memcpy(l3_scache_ptr, &flag, sizeof(uint8));
            SOC_SCACHE_DUMP_DATA_UINT8x_TV(l3_scache_ptr, i);
            l3_scache_ptr += sizeof(uint8);
        }
...
bcm_esw_l3_init(int unit)
{
    ...
    l3->ecmp_defrag_count = 0;
    l3->overlay_ecmp_defrag_count = 0;
    l3->underlay_ecmp_defrag_count = 0;
    l3->l3_initialized = 1;
    ...
    if (intf->l3a_flags & BCM_L3_WITH_ID) {
        ...
        if (rv == BCM_E_NONE) {
            ...
            if (BCM_L3_INTF_ARL_GET(unit, intf->l3a_intf_id)) {
                if ((l3i.l3i_vid != find_if.l3a_vid) ||
                    (sal_memcmp(l3i.l3i_mac_addr, find_if.l3a_mac_addr,
                                sizeof(l3i.l3i_mac_addr)) != 0)) {
                    BCM_IF_ERROR_RETURN
    ...
    */
...
#endif /* BCM_TOMAHAWK_SUPPORT */
    {
        if (!BCM_GPORT_IS_SET(info->l3a_port_tgid)) {
            /* Only construct when GPORT is not set */
            BCM_IF_ERROR_RETURN
                (_bcm_esw_l3_gport_construct(unit, info->l3a_port_tgid,
                                             info->l3a_modid, info->l3a_port_tgid,
                                             info->l3a_flags,
...
bcm_esw_l3_route_find(int unit, bcm_l3_host_t *host, bcm_l3_route_t *route)
{
    ...
    if (!BCM_GPORT_IS_SET(route->l3a_port_tgid)) {
        /* Only construct when GPORT is not set */
        BCM_IF_ERROR_RETURN(
    ...
    bcm_l3_ecmp_member_t *ecmp_member_array)
{
    ...
    bcm_l3_egress_ecmp_t old_ecmp_info;
    int old_ecmp_member_count;
    bcm_l3_ecmp_member_t *old_ecmp_member_array = NULL;
...
#endif /* BCM_TOMAHAWK2_SUPPORT  ||  BCM_TRIDENT3_SUPPORT  */
        {
            /* Make a copy of old ecmp info and member */
            if (ecmp_info->flags & BCM_L3_REPLACE) {

                bcm_l3_egress_ecmp_t_init(&old_ecmp_info);
                old_ecmp_info.ecmp_intf = ecmp_info->ecmp_intf;

                rv = bcm_esw_l3_ecmp_get(unit, &old_ecmp_info, 0,
                                         NULL, &old_ecmp_member_count);
                if (BCM_FAILURE(rv)) {
                    goto clean_up;
                }

                if (old_ecmp_member_count > 0) {
                    alloc_size = old_ecmp_member_count * sizeof(bcm_l3_ecmp_member_t);
                    old_ecmp_member_array = sal_alloc(alloc_size, "old_ecmp_member_array");
                    if (old_ecmp_member_array == NULL) {
                        rv = BCM_E_MEMORY;
                        goto clean_up;
                    }
                    sal_memset(old_ecmp_member_array, 0, alloc_size);

                    rv = bcm_esw_l3_ecmp_get(unit, &old_ecmp_info, old_ecmp_member_count,
                                             old_ecmp_member_array, &old_ecmp_member_count);
                    if (BCM_FAILURE(rv)) {
                        goto clean_up;
                    }
                }
            ...
            /* Replace returns FULL, restore the old ecmp info and member */
            if ((ecmp_info->flags & BCM_L3_REPLACE) &&
                (rv == BCM_E_FULL || rv == BCM_E_RESOURCE)) {

                old_ecmp_info.flags |= BCM_L3_REPLACE | BCM_L3_WITH_ID;
                old_intf_count = old_ecmp_member_count;

                for (i = 0; i < old_intf_count; i++) {
                    intf_array[i] = old_ecmp_member_array[i].egress_if;
                }
                rv2 = _bcm_esw_l3_ecmp_create(unit, &old_ecmp_info, old_intf_count,
                                             intf_array, BCM_L3_ECMP_MEMBER_OP_SET,
                                             0, NULL, old_intf_count, NULL);
                if (BCM_FAILURE(rv2)) {
                    goto clean_up;
                }

                /* Configure the member's attributions */
                rv2 = _bcm_xgs3_l3_ecmp_member_set(unit, &old_ecmp_info,
                                                   old_intf_count, old_ecmp_member_array);
                goto clean_up;
            }
        }
    }
...
clean_up:
    ...
    if (old_ecmp_member_array) {
        sal_free(old_ecmp_member_array);
        old_ecmp_member_array = NULL;
    }
    L3_UNLOCK(unit);
                                         ...
                                         bcm_port_t port, int percentage)
{
    ...
    match_array_alloc_size = sizeof(int) * total_entries * NUM_PIPE(unit);
    pipe_array_alloc_size = (sizeof(_dlb_entry_array_t) * total_entries * NUM_PIPE(unit));
    dlb_entry_array = soc_cm_salloc(unit, (sizeof(_dlb_entry_array_t) *
                                   total_entries * NUM_PIPE(unit)),
                                  "Matched ECMP DLB Group Entries");
    rand_num_arr = soc_cm_salloc(unit, match_array_alloc_size,
                                  "Array for Random numbers generated");

    if (dlb_entry_array == NULL || rand_num_arr == NULL) {
        rv = BCM_E_MEMORY;
        goto error;
    }
    sal_memset(dlb_entry_array, 0, pipe_array_alloc_size);
    ...
    for (pipe = 0; pipe <NUM_PIPE(unit); pipe++) {
#ifdef BCM_TOMAHAWK3_SUPPORT
        /* Since 56983 TH3 SKU is half chip, we skip the invalid
           pipes */
...
#endif
        mem = SOC_MEM_UNIQUE_ACC(unit, DLB_ECMP_FLOWSETm)[pipe];
        for (index = index_min; index <= index_max; index++) {
            ...
            if (port_member == port && valid == 1) {
                dlb_entry_array[count].index = index;
                dlb_entry_array[count].pipe = pipe;
                count++;
            }
        }
    }
    ...
    */
     ...
     while(i < flows) {
        ...
        pipe = dlb_entry_array[rand_num].pipe;
        index = dlb_entry_array[rand_num].index;
        mem = SOC_MEM_UNIQUE_ACC(unit, DLB_ECMP_FLOWSETm)[pipe];
        SOC_IF_ERROR_RETURN
        (soc_mem_read(unit, mem, MEM_BLOCK_ANY, index, &flowset_entry));
...
error :
    if (NULL !=dlb_entry_array) {
        soc_cm_sfree(unit, dlb_entry_array);
    }
