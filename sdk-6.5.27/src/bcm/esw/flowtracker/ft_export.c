...
/* 6.5.21 Sync/Recv for Export Element info stage */
#define BCM_WB_FT_EXPORT_VERSION_1_4                SOC_SCACHE_VERSION(1,4)

/* Default warmboot version */
#define BCM_WB_FT_EXPORT_DEFAULT_VERSION            BCM_WB_FT_EXPORT_VERSION_1_4
...
#define BCMI_FTE_RECORD_ELEM_STAGE_SET(_record_, _idx_, _stage_)            \
        (_record_)->info[(_idx_)].stage = (_stage_)
...
static bcmi_ft_export_elements_map_t bcmi_ft_export_elements_map[] = {
            ...
            {   BCMI_FT_XLATE_PARAM(EgrClassId,        2  )   },
            {   BCMI_FT_XLATE_PARAM(TcpSeqNumber,      4  )   },
            {   BCMI_FT_XLATE_PARAM(Custom,            0  )   },
            {   BCMI_FT_XLATE_PARAM(PacketTos,         1  )   },
            {   BCMI_FT_XLATE_PARAM(ForwardingOuterVlanTag,      2  )   },
            {   BCMI_FT_XLATE_PARAM(ForwardingInnerVlanTag,      2  )   },
            {   BCMI_FT_XLATE_PARAM(FlowState,         1  )   }
};
...
#define BCMI_FTE_IPFIX_HDR_OBS_DOMAIN   0x0
                         ...
                         shr_ipfix_header_t *ipfix)
{
    ...
    obs_domain_id_str = soc_property_get_str(unit,
            spn_FLOWTRACKER_IPFIX_OBSERVATION_DOMAIN_ID);
    if (obs_domain_id_str != NULL) {
        ipfix->observation_id = soc_property_get(unit,
                spn_FLOWTRACKER_IPFIX_OBSERVATION_DOMAIN_ID,
                BCMI_FTE_IPFIX_HDR_OBS_DOMAIN);
    } else {
        ipfix->observation_id =
            ft_coll_info->ipfix_hdr.observation_domain_id;
    }
                          ...
                          int *actual_export_elems)
{
    ...
    for (jx = 0; jx < num_elems; jx++) {
        ...
        rv = _bcm_field_ft_tracking_param_type_get(unit,
                bcmFlowtrackerGroupTypeNone, bcmFlowtrackerStageDefault,
                bcmi_ft_export_elements_map[ix].tracking_param, elem->custom_id,
                _BCM_FIELD_FT_TRACKING_PARAM_TYPE_GET_F_IGNORE_GROUP, &ft_type_bmp);
        BCM_IF_ERROR_RETURN(rv);
    }
        ...
        bcmi_fte_template_record_info_t *record)
{
    ...
    for (key_idx = 0; key_idx < num_key_info; key_idx++) {
        ...
        BCMI_FTE_RECORD_ELEM_STAGE_SET(record, idx, bcmFlowtrackerStageIngress);
                    ...
                    bcmi_fte_template_record_info_t *record)
{
    ...
    char *stagelist[] = BCM_FLOWTRACKER_STAGE_STRINGS;
    ...
    while(ftg_template) {
        switch(ftg_template->info.param_type) {
            ...
            case bcmiFtGroupParamTypeFlowchecker:
                ...
                break;
            case bcmiFtGroupParamTypeFlowState:
                tracking_param =
        ...
        BCMI_FTE_RECORD_ELEM_STAGE_SET(record, idx, ftg_template->info.stage);

        sal_memset(info_name, 0, sizeof(char) * BCMI_FT_EXPORT_ELEMENT_NAME_LEN_MAX);
        (void) bcmi_ft_export_elem_name_get(unit, info_elem, NULL, info_name);
        LOG_DEBUG(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                        "[TemplateValidate]Element Record_Idx: %2d | Id: %2d"
                        " (%20s(%d), %5s) | Size: %02d (0x%x) | Stage %s\n\r"), idx,
                    info_elem, info_name, custom_id,
                    record_dir[ftg_template->info.direction],
                    data_bits, check_id, stagelist[ftg_template->info.stage]));
        ...
        bcmi_fte_template_record_info_t *record)
{
    ...
    for (idx = 0; idx < record->info_depth; idx++) {
        ...
        record->info[idx].stage = bcmFlowtrackerStageDefault;
    }
...
STATIC int
bcmi_ft_export_elem_stage_match(int unit,
        bcm_flowtracker_group_type_t group_type,
        bcm_flowtracker_export_element_info_t *info_elem_in,
        bcm_flowtracker_stage_t rec_stage)
{
    int ix = 0;
    ...
    for (ix = 0; ix < COUNTOF(bcmi_ft_export_elements_map); ix++) {
        if (info_elem_in->element ==
                bcmi_ft_export_elements_map[ix].export_element) {
            /* Break the loop when found. */
            break;
        }
    }
    ...
    /* Export element is not mapped to any valid param */
    if (bcmFlowtrackerTrackingParamTypeNone ==
            bcmi_ft_export_elements_map[ix].tracking_param) {
        return TRUE;
    }

    /* If timestamp related, stage is not required */
    if (bcmi_ft_tracking_param_is_timestamp(unit,
                bcmi_ft_export_elements_map[ix].tracking_param)) {
        return TRUE;
    }

    /* Skip for Flow transition state */
    if (bcmFlowtrackerTrackingParamTypeFlowState ==
            bcmi_ft_export_elements_map[ix].tracking_param) {
        return TRUE;
    }

    /* Get final stage */
    if (info_elem_in->stage == bcmFlowtrackerStageDefault) {
        (void) _bcm_field_ft_tracking_param_def_stage_get(unit,
                group_type,
                bcmi_ft_export_elements_map[ix].tracking_param,
                &final_stage);
    } else {
        final_stage = info_elem_in->stage;
    }

    if (final_stage != rec_stage) {
        return FALSE;
        ...
        int *found)
{
    ...
    *found = FALSE;
    ...
    for (ix_in = 0; ix_in < num_in_export_elements; ix_in++) {
        ...
        /* Check if export element identifier match */
        ...
        /* Skip if custom_id do not match for Custom tracking param */
        ...
        /* Skip if stage do not match */
        if (!bcmi_ft_export_elem_stage_match(unit,
                    BCMI_FT_GROUP_TYPE_GET(id),
                    info_elem_in, record_info->stage)) {
            continue;
        }
...
bcmi_ft_export_warmboot_sync(int unit, soc_scache_handle_t scache_handle)
{
    ...
    for (ix = 0; ix < BCMI_FT_EXPORT_INFO(unit)->max_templates; ix++) {
        ...
        for (jx = 0; jx < BCMI_FT_TEMPLATE_MAX_ELEMS; jx++) {
            ...
            BCMI_FT_WB_SCACHE_SYNC(ft_scache_ptr, &(cur_elem_info->stage));
        }
    }
    ...
    for (ix = 0; ix < BCMI_FT_EXPORT_INFO(unit)->max_templates; ix++) {
        ...
        for (jx = 0; jx < BCMI_FT_TEMPLATE_MAX_ELEMS; jx++) {
            if (recovered_ver <= BCM_WB_FT_EXPORT_VERSION_1_2) {
                ...
                if (recovered_ver <= BCM_WB_FT_EXPORT_VERSION_1_4) {
                    BCMI_FT_WB_SCACHE_RECOVER(ft_scache_ptr, &(cur_elem_info->stage));
                }
            }
        }
    }
...
void bcmi_ftv3_aft_export_sw_thread(void *up)
{
    ...
    sal_memset(en_group_id_list, 0, sizeof(en_group_id_list));
