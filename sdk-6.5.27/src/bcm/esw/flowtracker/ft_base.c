...
/* For 6.5.26, Added flex state transition support */
...
bcmi_ft_flow_direction_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_field_ft_alu_data_config_get(unit, -1,
        _bcmFieldFtAluDataTcpFlagsSYN, &syn_offset, NULL, NULL));

    BCM_IF_ERROR_RETURN(_bcm_field_ft_alu_data_config_get(unit, -1,
        _bcmFieldFtAluDataTcpFlagsACK, &ack_offset, NULL, NULL));
...
bcmi_ft_init(int unit, bcm_flowtracker_cfg_t *cfg)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        /* Create default state transition entries */
        result = bcmi_ft_state_transition_def_entries_add(unit);
    }
#else
    {
        /* Create default state transition entries */
...
bcmi_ft_cleanup(int unit)
{
    ...
    bcmi_ft_flow_transition_state_cleanup(unit);
...
bcmi_ft_wb_alloc_size(int unit, int *size)
{
    ...
    if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_6) {
        ...
        /* Configured */
        alloc_size += (num_entries * sizeof(uint8));
        ...
        if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
            ...
            alloc_size += (num_entries * bmp_words * sizeof(uint32));

            /* Reference count */
            alloc_size += (num_entries * TOTAL_GROUP_AGG_ALU32_MEM * \
                    sizeof(uint32));
        }
#endif
    }
...
STATIC int
bcmi_ft_sync_ver_1_6(int unit, uint8 **p_ft_scache_ptr)
{
    int idx = 0;
    uint32 num_entries;
    uint8 *ft_scache_ptr;
    ...
    for (idx = 0; idx < num_entries; idx++) {
        ...
        if (sw_ent != NULL) {
            configured = 1;
            priority = sw_ent->priority;
        } else {
            configured = 0;
            priority = 0;
        }

        sal_memcpy(ft_scache_ptr, &configured, sizeof (uint8));
        ft_scache_ptr += sizeof (uint8);

        sal_memcpy(ft_scache_ptr, &priority, sizeof (uint32));
        ft_scache_ptr += sizeof (uint32);

#if defined(BCM_FLOWTRACKER_V3_SUPPORT)

        num_checks = BCMI_FT_FLOWCHECKER_MAX_ID(unit);
        bmp_words = SHR_BITALLOCSIZE(num_checks);

        if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
            if (sw_ent != NULL) {
                sal_memcpy(ft_scache_ptr, sw_ent->check_bmp,
                        bmp_words * sizeof (uint32));
                ft_scache_ptr += (bmp_words * sizeof (uint32));

                sal_memcpy(ft_scache_ptr, sw_ent->agg_alu32_ref_cnt,
                        TOTAL_GROUP_AGG_ALU32_MEM * sizeof (uint32));
                ft_scache_ptr += (TOTAL_GROUP_AGG_ALU32_MEM * sizeof (uint32));
            } else {
                ft_scache_ptr += (bmp_words * sizeof (uint32));
                ft_scache_ptr += (TOTAL_GROUP_AGG_ALU32_MEM * sizeof (uint32));
            }
...
bcmi_ft_sync(int unit)
{
    ...
    /* Version 1_6 Sync */
    BCM_IF_ERROR_RETURN(bcmi_ft_sync_ver_1_6(unit, &ft_scache_ptr));
...
STATIC int
bcmi_ft_recover_ver_1_6(int unit, uint8 **p_ft_scache_ptr)
{
    int idx = 0;
    uint32 num_entries;
    uint8 *ft_scache_ptr;
    ...
    for (idx = 0; idx < num_entries; idx++) {
        ...
        sal_memcpy(&configured, ft_scache_ptr, sizeof (uint8));
        ft_scache_ptr += sizeof (uint8);

        sal_memcpy(&priority, ft_scache_ptr, sizeof (uint32));
        ft_scache_ptr += sizeof (uint32);

        if (configured == 1) {
            sw_ent = NULL;
            BCMI_FT_ALLOC(sw_ent, sizeof (bcmi_ft_state_transition_entry_t),
                          "ft_state_trans_ent");

            BCMI_FT_STATE_TRANSITION_SW_ENTRY(unit, idx) = sw_ent;
            ...
            if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
                BCMI_FT_ALLOC(sw_ent->check_bmp, bmp_words * sizeof (uint32),
                        "ft_state_trans_chk");
                sal_memcpy(sw_ent->check_bmp, ft_scache_ptr,
                        bmp_words * sizeof (uint32));
                ft_scache_ptr += (bmp_words * sizeof (uint32));

                sal_memcpy(sw_ent->agg_alu32_ref_cnt, ft_scache_ptr,
                        TOTAL_GROUP_AGG_ALU32_MEM * sizeof (uint32));
                ft_scache_ptr += (TOTAL_GROUP_AGG_ALU32_MEM * sizeof (uint32));
            }
#endif
...
bcmi_ft_recover(int unit)
{
    ...
    if (recovered_ver >= BCM_WB_VERSION_1_6) {
        bcmi_ft_recover_ver_1_6(unit, &ft_scache_ptr);
    }
