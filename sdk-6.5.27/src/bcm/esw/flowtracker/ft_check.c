    ...
    *ft_type_bmp = BCMI_FT_TYPE_F_ANY;
    ...
    if (options & BCM_FLOWTRACKER_CHECK_SECOND) {
        if ((flags & BCM_FLOWTRACKER_CHECK_DATA_CLEAR_ON_EXPORT) ||
            ...
            (flags & BCM_FLOWTRACKER_CHECK_STATE_TRANSITION)) {
            LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
        ...
        /* AIFT ft type supports State transition */
        if (soc_feature(unit,
                    soc_feature_flex_flowtracker_state_transition)) {
            if (flow_check_info->state_transition_flags != 0) {
                LOG_DEBUG(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                                "State transition flags unsupported for "
                                "non-zero flex state transition mode.\n")));
                return BCM_E_CONFIG;
            }
            if (flags & BCM_FLOWTRACKER_CHECK_STATE_TRANSITION) {
                *ft_type_bmp = (BCMI_FT_TYPE_F_AIFT);
            }
        } else {
            if (flags & BCM_FLOWTRACKER_CHECK_STATE_TRANSITION) {
                LOG_DEBUG(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                                "Check flag State transition unsupported for "
                                "default flex state transition mode.\n")));
                return BCM_E_CONFIG;
            }
...
#if defined(BCM_FLOWTRACKER_V3_SUPPORT)
        /* Check if associated with state transition */
        if (soc_feature(unit,
                    soc_feature_flex_flowtracker_state_transition)) {
            int is_mapped = FALSE;

            if (BCMI_FT_FLOWCHECKER_INFO(unit, index)->check1.flags &
                    BCM_FLOWTRACKER_CHECK_STATE_TRANSITION) {
                BCM_IF_ERROR_RETURN(
                        bcmi_ftv3_check_state_transition_mapped(unit,
                            flowchecker_id, &is_mapped));
                if (is_mapped == TRUE) {
                    LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
