        ...
        uint8 *ft_data)
{
    ...
    char *data_engine_str[] = { "TIMESTAMP",
        "ALU32", "ALU16", "LOAD16", "LOAD8", "FLOW_STT"};
    ...
    } else if (type == bcmiFtGroupParamTypeFlowState) {
        eng_type = 5; elem_type = 0;
        element = bcmFlowtrackerTrackingParamTypeFlowState;
        ...
        bcmi_ft_group_hw_tbl_trvrse_t *tbl_trv)
{
    ...
    if (soc_mem_field32_get(unit, pdd_mem, global_pdd_entry,
        ...
        ft_type = bcmiFtTypeIFT;
        rv = bcmi_ft_tracking_param_flow_state_is_present(unit, gid);
        if (rv == BCM_E_NONE) {
            bcmi_ft_group_session_data_element_dump(unit, 0,
                    pdd_pos[ft_type-1], 8,
                    bcmiFtGroupParamTypeFlowState,
                    NULL, ft_u8pdd_data[ft_type-1]);
        } else {
