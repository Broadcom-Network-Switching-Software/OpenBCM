...
/*
 * Function:
 *     bcmi_ft_tracking_param_flow_state_is_present
 * Purpose:
 *   check if Flow Statetracking param is present
 * Parameters:
 *   unit     - (IN) BCM device number
 ...
 */
int
bcmi_ft_tracking_param_flow_state_is_present(int unit,
        bcm_flowtracker_group_t id)
{
    int idx, num_params;
    bcm_flowtracker_tracking_param_info_t *t_info = NULL;

    num_params = BCMI_FT_GROUP_TRACK_PARAM_NUM(unit, id);
    t_info = BCMI_FT_GROUP_TRACK_PARAM(unit, id);

    for (idx = 0; idx < num_params; idx++, t_info++) {
        if (t_info->param ==
                           ...
                           int create)
{
    ...
    if (create) {
        ...
        fid_len = soc_mem_field_length(unit, BSC_DG_GROUP_TABLEm, CTRL_IDf);
        if (info->control_id > ((1 << fid_len) -1)) {
            return BCM_E_PARAM;
        }
          ...
          bcm_flowtracker_tracking_param_info_t *list_of_tracking_elements)
{
    ...
    for (iter = 0; iter < num_tracking_elements; iter++) {
        ...
        /* Validate stage */
        if ((element_info.stage < bcmFlowtrackerStageDefault) ||
                (element_info.stage >= bcmFlowtrackerStageCount)) {
            LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                            "Invalid stage.")));
            return BCM_E_PARAM;
        }
        ...
        param_flags = _BCM_FIELD_FT_TRACKING_PARAM_TYPE_GET_F_STAGE_FT_TYPE_BMP;
        if (element_info.flags & BCM_FLOWTRACKER_TRACKING_PARAM_TYPE_KEY) {
            param_flags |= _BCM_FIELD_FT_TRACKING_PARAM_TYPE_GET_F_KEY;
        }
        rv = _bcm_field_ft_tracking_param_type_get(unit,
                    group_type, element_info.stage, element_info.param,
                    element_info.custom_id, param_flags, &ft_type_bmp);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                     "param <%s, %s> is not supported in system.\n"),
                      stagelist[element_info.stage], plist[element_info.param]));
            return rv;
        }
        ...
        if (!(group_ft_type_bmp & ft_type_bmp)) {
            LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                     "param <%s, %s> is not supported for this group.\n"),
                      stagelist[element_info.stage], plist[element_info.param]));
            return BCM_E_PARAM;
        }
        ...
        if (element_info.param ==
            ...
            rv = bcmi_ft_group_check_validate(unit,
                        BCMI_FT_GROUP_CHECK_MATCH_PRIMARY, flow_group_id,
                        element_info.check_id, element_info.stage);
            BCM_IF_ERROR_RETURN(rv);
        }
        ...
        if (!(ft_type_bmp & BCMI_FT_TYPE_F_IFT)) {
            if (element_info.flags & BCM_FLOWTRACKER_TRACKING_PARAM_TYPE_KEY) {
                LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                           "key param <%s, %s> not supported in Ingress stage\n"),
                            stagelist[element_info.stage], plist[element_info.param]));
                return BCM_E_PARAM;
            }
        }

        /* key params can only belong to ingress flowtracker stage */
        if (!((element_info.stage == bcmFlowtrackerStageDefault) ||
             (element_info.stage == bcmFlowtrackerStageIngress))) {
            if (element_info.flags & BCM_FLOWTRACKER_TRACKING_PARAM_TYPE_KEY) {
                LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                           "Invalid key param <%s> stage <%s>.\n"),
                            plist[element_info.param], stagelist[element_info.stage]));
                return BCM_E_PARAM;
            }
        }
        ...
        if (element_info.param ==
            ...
            if (!soc_feature(unit,
                ...
                LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                                "Param <%s> is not supported for current"
...
bcmi_ft_group_hw_install(int unit, bcm_flowtracker_group_t id)
{
...
#if defined(BCM_FLOWTRACKER_V3_SUPPORT)
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
        rv = bcmi_ftv3_flow_transition_group_map_check(unit, id);
        BCM_IF_ERROR_RETURN(rv);
    }
#endif
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
        ...
        if (!BCMI_FT_GROUP_TYPE_IS_AGGREGATE(id) &&
                (BCMI_FT_GROUP_FTFP_DATA(unit, id).direction ==
                 bcmFlowtrackerFlowDirectionBidirectional)) {
            rv = bcmi_ft_group_flow_transition_state_set(unit, id, 1);
            BCMI_CLEANUP_IF_ERROR(rv);
        }
    }
#endif
    ...
    if (soc_feature(unit,
                soc_feature_flex_flowtracker_state_transition)) {
        rv = bcmi_ft_tracking_param_flow_state_is_present(unit, id);
        if (rv == BCM_E_NONE) {
            rv = bcmi_ft_group_flow_transition_state_set(unit, id, 1);
            BCMI_CLEANUP_IF_ERROR(rv);
        }
                                   ...
                                   int *load8_indexes)
{
    ...
    if (soc_feature(unit,
                soc_feature_flex_flowtracker_state_transition)) {
        flow_state = soc_mem_field32_get(unit, mem,
                (&(BCMI_FT_GROUP(unit, id)->pdd_entry)),
                BSD_FLEX_FLOW_STATEf);
        if (flow_state) {
            rv = bcmi_ft_group_template_list_add(unit, head,
                    running_length, 8, bcmiFtGroupParamTypeFlowState,
                    NULL);
                                ...
                                bcmi_ft_group_alu_info_t *info)
{
    ...
    if (info != NULL) {
        ...
        BCMI_FT_TYPE_BMP_STAGE_GET((1 << info->alu_ft_type),
                                    temp_node->info.stage);
        ...
        if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
            ...
            if ((info->flags & BCMI_FT_ALU_LOAD_USE_AGG_ALU32) &&
                ...
                /* Check if data is not at boundary for (LOAD / ALU for LOAD) */
                if (temp_node->info.flags & BCMI_FT_ALU_LOAD_DATA_AT_SHIFT_BITS) {
                    bcmi_ft_group_alu_action_shift_bits_get(unit,
                            bcmFlowtrackerCheckActionUpdateValue,
                            info, &shift_bits);
                    temp_node->info.data_shift = shift_bits;
            ...
            bcmi_flowtracker_check_set_t reset)
{
    ...
    group_type = BCMI_FT_GROUP_TYPE_GET(id);
    BCM_IF_ERROR_RETURN
        (bcmi_ft_group_check_ft_type_get(unit,
                group_type, &fc_info, &ft_type));
                    ...
                    bcm_flowtracker_export_element_info_t *in_export_elements)
{
    ...
    for (iter = 0; iter < num_in_export_elements; iter++) {
        ...
        rv = bcmi_ft_group_check_validate(unit, options,
                    id, check_id, export_elem_info->stage);
        BCM_IF_ERROR_RETURN(rv);
    ...
    bcm_flowtracker_group_t id)
{
     ...
     */
    mode = bcmiFtGroupModeSingle;
    bcmi_ft_group_extraction_mode_get(unit, 0, id, &mode);

    if (mode == bcmiFtGroupModeDouble) {
...
/*
 * Function:
 *   bcmi_ft_group_flow_transition_state_set
 * Purpose:
 ...
 */
int
bcmi_ft_group_flow_transition_state_set(int unit,
                    bcm_flowtracker_group_t id,
                    uint32 enable)
{
    uint32 curr_state;
        ...
        for (idx = 0; idx < map->count; idx++) {
            ...
            switch(engine_info->engine_type) {
                ...
                case bcmiFtDataEngineTypeCollectorCopy:
                    ...
                    break;
                case bcmiFtDataEngineTypeFlowState:
                    type = bcmiFtGroupParamTypeFlowState;
                    break;
                default:
                    continue;
            }
        ...
        uint32 *check_bmp_mask)
{
    ...
    for (iter = 0; iter < num_checks; iter++) {
        ...
        rv = bcmi_ft_group_check_validate(unit,
                flags, id, check_id, bcmFlowtrackerStageDefault);
        BCM_IF_ERROR_RETURN(rv);
...
/*
 * Function:
 *   bcmi_ftv3_check_in_validated_group
 * Purpose:
 *   Check if given flowtracker check is part of group which is validated.
 * Parameters:
 *   unit       - (IN) BCM device number.
 ...
 */
int
bcmi_ftv3_check_in_validated_group(int unit,
        bcm_flowtracker_check_t check_id)
{
    int rv;
    int index, num_groups;
    bcm_flowtracker_group_t group_id;
    bcm_flowtracker_group_type_t group_type;

    /* If check is not used anywhere, return not found
     * as is is not associated with any (validated) group
     */
    ...
    /* Find all groups with which check is associated */
    group_type = bcmFlowtrackerGroupTypeAggregateIngress;
    num_groups = BCMI_FT_GROUP_TYPE_MAX_COUNT(unit, group_type);

    for (index = 0; index < num_groups; index++) {
        if (!(SHR_BITGET(BCMI_FT_GROUP_BITMAP(unit, group_type), index))) {
            continue;
        }

        /* Check if check is present */
        BCM_FLOWTRACKER_GROUP_ID_SET(group_id, group_type, index);
        rv = bcmi_ft_group_is_check_valid(unit, group_id, check_id);
        if (rv != BCM_E_NONE) {
            continue;
        }

        if (BCMI_FT_GROUP_IS_VALIDATED(unit, group_id)) {
            return BCM_E_NONE;
        }
    }
