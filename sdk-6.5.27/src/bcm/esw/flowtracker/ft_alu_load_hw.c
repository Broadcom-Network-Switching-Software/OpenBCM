 ...
 */
int
bcmi_ft_alu_load_init(int unit, bcm_flowtracker_cfg_t *cfg)
{
    int rv = BCM_E_NONE;
    int i = 0, j = 0;
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_2)) {
        ...
        rv = bcmi_ftv2_alu_load_mem_config_init(unit, cfg);
        if (BCM_FAILURE(rv)) {
            bcmi_ft_alu_load_cleanup(unit);
            return rv;
        }
        ...
        rv = bcmi_ftv2_resource_info_init(unit, cfg);
        if (BCM_FAILURE(rv)) {
            bcmi_ft_alu_load_cleanup(unit);
            return rv;
        }
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
        ...
        /* allocate resources first as per config variables */
        rv = bcmi_ftv3_alu_load_mem_config_init(unit, cfg);
        if (BCM_FAILURE(rv)) {
            bcmi_ft_alu_load_cleanup(unit);
            return rv;
        }
                        ...
                        int *load16_indexes, int *index)
{
    ...
    if ((bcmi_ft_tracking_param_is_timestamp(unit, alu_info->param)) ||
        (bcmi_ft_tracking_param_is_delay(unit, alu_info->param))) {
        if (group_alu_info->src_ts != alu_info->src_ts) {
            return BCM_E_NOT_FOUND;
        }
    }
                                         ...
                                         uint32 *alu_fmt)
{
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_2)) {
        ...
        BCM_IF_ERROR_RETURN
            (bcmi_ftv2_group_alu_mask_set(unit, id, alu_info,
                                          &fc_info, alu_fmt));
    }
#endif
 ...
 */
int
bcmi_ftv2_alu_load_mem_config_init(int unit, bcm_flowtracker_cfg_t *cfg)
{
    ...
    for (num_r = 0; num_r < BCMI_FTv2_RESOURCE_NUM; num_r++) {
        ...
        if (cfg != NULL) {
            ...
            /* Resourced init - pick up resources from run time api input */
            if (num_r == BCMI_FTv2_RESOURCE_SESSION_BANK) {

                ift_r = cfg->session_data_bank[bcmFlowtrackerStageIngress];
                mft_r = cfg->session_data_bank[bcmFlowtrackerStageMmu];
                eft_r = cfg->session_data_bank[bcmFlowtrackerStageEgress];
            } else if (num_r == BCMI_FTv2_RESOURCE_ALU16) {

                ift_r = cfg->alu16[bcmFlowtrackerStageIngress];
                mft_r = cfg->alu16[bcmFlowtrackerStageMmu];
                eft_r = cfg->alu16[bcmFlowtrackerStageEgress];
            } else if (num_r == BCMI_FTv2_RESOURCE_ALU32) {

                ift_r = cfg->alu32[bcmFlowtrackerStageIngress];
                mft_r = cfg->alu32[bcmFlowtrackerStageMmu];
                eft_r = cfg->alu32[bcmFlowtrackerStageEgress];
            } else if (num_r == BCMI_FTv2_RESOURCE_LOAD8) {

                ift_r = cfg->load8[bcmFlowtrackerStageIngress];
                mft_r = cfg->load8[bcmFlowtrackerStageMmu];
                eft_r = cfg->load8[bcmFlowtrackerStageEgress];
            } else if (num_r == BCMI_FTv2_RESOURCE_LOAD16) {

                ift_r = cfg->load16[bcmFlowtrackerStageIngress];
                mft_r = cfg->load16[bcmFlowtrackerStageMmu];
                eft_r = cfg->load16[bcmFlowtrackerStageEgress];
            } else if (num_r == BCMI_FTv2_RESOURCE_TIMESTAMP_ENGINE) {

                ift_r = cfg->timestamp[bcmFlowtrackerStageIngress];
                mft_r = cfg->timestamp[bcmFlowtrackerStageMmu];
                eft_r = cfg->timestamp[bcmFlowtrackerStageEgress];
            } else {
            }
 ...
 */
int
bcmi_ftv2_resource_info_init(int unit, bcm_flowtracker_cfg_t *cfg)
{
    int ts_bank_num = 0;
    ...
    if (cfg != NULL) {
        ...
        /* Resourced init - pick up resources from run time api input */
        BCMI_FT_RESOURCE_SW_INFO_CHIP_GRAN(unit) = cfg->chip_delay_granularity;
        BCMI_FT_RESOURCE_SW_INFO_CHIP_OFFSET(unit) = cfg->chip_delay_offset;
        BCMI_FT_RESOURCE_SW_INFO_E2E_GRAN(unit) = cfg->e2e_delay_granularity;
        BCMI_FT_RESOURCE_SW_INFO_E2E_OFFSET(unit) = cfg->e2e_delay_offset;
        BCMI_FT_RESOURCE_SW_INFO_IPAT_GRAN(unit) = cfg->ipat_delay_granularity;
        BCMI_FT_RESOURCE_SW_INFO_IPAT_OFFSET(unit) = cfg->ipat_delay_offset;
        BCMI_FT_RESOURCE_SW_INFO_IPDT_GRAN(unit) = cfg->ipdt_delay_granularity;
        BCMI_FT_RESOURCE_SW_INFO_IPDT_OFFSET(unit) = cfg->ipdt_delay_offset;
...
/*
 * Function:
 *   bcmi_ftv2_resource_cfg_get
 * Purpose:
 *   Get flowtracker resource info as per user provided config.
 * Parameters:
 *   unit - (IN) BCM device id.
 * Notes:
 *   Intent of this api is to get data related to resource
 *   allocation which will be used in run time logic.
 * Returns:
 *   BCM_E_XXX
 */
int
bcmi_ftv2_resource_cfg_get(int unit, bcm_flowtracker_cfg_t *cfg)
{
    int rsrc_num = 0;
    int alu32_mem = 0;

    for (rsrc_num = 0; rsrc_num < TOTAL_GROUP_SESSION_DATA_BANK_SW_DATA_NUM; rsrc_num++) {
        if (BCMI_FT_SESSION_DATA_BANK_FT_TYPE(unit, rsrc_num) == bcmiFtTypeIFT) {
            cfg->session_data_bank[bcmFlowtrackerStageIngress]++;
        } else if (BCMI_FT_SESSION_DATA_BANK_FT_TYPE(unit, rsrc_num) == bcmiFtTypeMFT) {
            cfg->session_data_bank[bcmFlowtrackerStageMmu]++;
        } else if (BCMI_FT_SESSION_DATA_BANK_FT_TYPE(unit, rsrc_num) == bcmiFtTypeEFT) {
            cfg->session_data_bank[bcmFlowtrackerStageEgress]++;
        }
    }

    for (rsrc_num = 0; rsrc_num < TOTAL_GROUP_ALU16_MEM; rsrc_num++) {
        if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAlu16,
                    rsrc_num, 0)  == bcmiFtTypeIFT) {
            cfg->alu16[bcmFlowtrackerStageIngress]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAlu16,
                    rsrc_num, 0) == bcmiFtTypeMFT) {
            cfg->alu16[bcmFlowtrackerStageMmu]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAlu16,
                    rsrc_num, 0) == bcmiFtTypeEFT) {
            cfg->alu16[bcmFlowtrackerStageEgress]++;
        }
    }

    alu32_mem = bcmi_ft_get_alu32_mem(unit);
    for (rsrc_num = 0; rsrc_num < alu32_mem; rsrc_num++) {
        if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAlu32,
                    rsrc_num, 0)  == bcmiFtTypeIFT) {
            cfg->alu32[bcmFlowtrackerStageIngress]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAlu32,
                    rsrc_num, 0) == bcmiFtTypeMFT) {
            cfg->alu32[bcmFlowtrackerStageMmu]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAlu32,
                    rsrc_num, 0) == bcmiFtTypeEFT) {
            cfg->alu32[bcmFlowtrackerStageEgress]++;
        }
    }

    for (rsrc_num = 0; rsrc_num < TOTAL_GROUP_LOAD8_DATA_NUM; rsrc_num++) {
        if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeLoad8,
                    0, rsrc_num)  == bcmiFtTypeIFT) {
            cfg->load8[bcmFlowtrackerStageIngress]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeLoad8,
                    0, rsrc_num) == bcmiFtTypeMFT) {
            cfg->load8[bcmFlowtrackerStageMmu]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeLoad8,
                    0, rsrc_num) == bcmiFtTypeEFT) {
            cfg->load8[bcmFlowtrackerStageEgress]++;
        }
    }

    for (rsrc_num = 0; rsrc_num < TOTAL_GROUP_LOAD16_DATA_NUM; rsrc_num++) {
        if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeLoad16,
                    0, rsrc_num)  == bcmiFtTypeIFT) {
            cfg->load16[bcmFlowtrackerStageIngress]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeLoad16,
                    0, rsrc_num) == bcmiFtTypeMFT) {
            cfg->load16[bcmFlowtrackerStageMmu]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeLoad16,
                    0, rsrc_num) == bcmiFtTypeEFT) {
            cfg->load16[bcmFlowtrackerStageEgress]++;
        }
    }

    for (rsrc_num = 0; rsrc_num < TOTAL_GROUP_TIMESTAMP_ENGINE_DATA_NUM; rsrc_num++) {
        if (BCMI_FT_TIMESTAMP_ENGINE_FT_TYPE(unit, rsrc_num) == bcmiFtTypeIFT) {
                cfg->timestamp[bcmFlowtrackerStageIngress]++;
        } else if (BCMI_FT_TIMESTAMP_ENGINE_FT_TYPE(unit, rsrc_num) == bcmiFtTypeMFT) {
                cfg->timestamp[bcmFlowtrackerStageMmu]++;
        } else if (BCMI_FT_TIMESTAMP_ENGINE_FT_TYPE(unit, rsrc_num) == bcmiFtTypeEFT) {
                cfg->timestamp[bcmFlowtrackerStageEgress]++;
        }
    }

    cfg->chip_delay_granularity = BCMI_FT_RESOURCE_SW_INFO_CHIP_GRAN(unit);
    cfg->chip_delay_offset = BCMI_FT_RESOURCE_SW_INFO_CHIP_OFFSET(unit);
    cfg->e2e_delay_granularity = BCMI_FT_RESOURCE_SW_INFO_E2E_GRAN(unit);
    cfg->e2e_delay_offset = BCMI_FT_RESOURCE_SW_INFO_E2E_OFFSET(unit);
    cfg->ipat_delay_granularity = BCMI_FT_RESOURCE_SW_INFO_IPAT_GRAN(unit);
    cfg->ipat_delay_offset = BCMI_FT_RESOURCE_SW_INFO_IPAT_OFFSET(unit);
    cfg->ipdt_delay_granularity = BCMI_FT_RESOURCE_SW_INFO_IPDT_GRAN(unit);
    cfg->ipdt_delay_offset = BCMI_FT_RESOURCE_SW_INFO_IPDT_OFFSET(unit);
                             ...
                             uint32 *alu_fmt)
{
    ...
    if (alu_info->flowchecker_id == 0) {
        BCM_IF_ERROR_RETURN(bcmi_ft_element_alu_action_get(unit, id,
                    alu_info->element_type1, &alu_action));

        bcmi_ft_group_alu_action_shift_bits_get(unit, alu_action,
                alu_info, &shift_bits3);
        if (shift_bits3 != 0) {
            alu_info->flags |= BCMI_FT_ALU_LOAD_DATA_AT_SHIFT_BITS;
        }
    } else {
        bcmi_ft_group_alu_action_shift_bits_get(unit,
                fc_info->action_info.action, alu_info, &shift_bits3);
    ...
    if (fc_info != NULL) {
        ...
        if (fc_info->flags &
            ...
            /* State Transition Event */
             ...
             (fc_info->check1.flags & BCM_FLOWTRACKER_CHECK_STATE_TRANSITION)) {
            opr_result[BCMI_FT_ALU_KMAP_BIT_SEL_STATE_TRIG] = 1;
        }
...
int
bcmi_ftv3_alu_load_mem_config_init(
    int unit, bcm_flowtracker_cfg_t *cfg)
{
    int i = 0;
    int res = 0;
    int max_alu32 = 0;
    ...
    if (cfg != NULL) {
        ...
        /* Resourced init - pick up resources from run time api input */
        aift_r = cfg->agg_alu32[bcmFlowtrackerStageIngress];
        amft_r = cfg->agg_alu32[bcmFlowtrackerStageMmu];
        aeft_r = cfg->agg_alu32[bcmFlowtrackerStageEgress];
 ...
 */
int
bcmi_ftv3_resource_cfg_get(int unit, bcm_flowtracker_cfg_t *cfg)
{
    int rsrc_num = 0;
    int max_alu32 = 0;
    max_alu32 = BCMI_FTv3_AGG_SESSION_DATA_BANKS * BCMI_FTv3_AGG_ALU32_PER_BANK;

    for (rsrc_num = 0; rsrc_num < max_alu32; rsrc_num++) {
        if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAggAlu32,
                    rsrc_num, 0) == bcmiFtTypeAIFT) {
            cfg->agg_alu32[bcmFlowtrackerStageIngress]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAggAlu32,
                    rsrc_num, 0) == bcmiFtTypeAMFT) {
            cfg->agg_alu32[bcmFlowtrackerStageMmu]++;
        } else if (BCMI_FT_ALU_LOAD_MEM_FT_TYPE(unit, bcmiFtAluLoadTypeAggAlu32,
                    rsrc_num, 0) == bcmiFtTypeAEFT) {
            cfg->agg_alu32[bcmFlowtrackerStageEgress]++;
        }
    }
    ...
    int set)
{
    ...
    if (info->flowchecker_id > 0) {
        ...
        if (soc_feature(unit,
            ...
            if (fc_info.flags & BCM_FLOWTRACKER_CHECK_STATE_TRANSITION) {
                rv = bcmi_ftv3_flex_state_transition_alu_trigger_set(unit,
                        info->flowchecker_id, alu_bank, set);
                BCM_IF_ERROR_RETURN(rv);
            }
        } else {
