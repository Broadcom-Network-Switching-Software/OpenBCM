...
#define BCM_TH3_UNDERLAY_ONLY_OFFSET(_unit) \
        (soc_mem_index_count(_unit, L3_ECMP_COUNTm) / 2 + \
        BCMI_L3_ECMP_OVERLAY_ENTRIES(_unit))
                             ...
                             bcm_if_t *intf_arr, int *intf_count, int *max_paths)
{
    ...
    if (intf_arr_ptr) {
        ...
        /* If the count is 0, it indicates there are no ecmp members
           associated with this RH group so we return BCM_E_NONE instead
           of an error
         */
        if (_opt_ecmp_rh_info[unit]->rhg[ecmp_group_idx].rh_intf_count == 0) {
            if (intf_count) {
                *intf_count = 0;
            }
            if (max_paths) {
...
_bcm_opt_ecmp_rh_free_resource(int unit, int ecmp_group_idx)
{
    ...
    int phy_ecmp_group_idx = ecmp_group_idx;
    ...
    sal_memset(&ecmp_count_entry, 0, sizeof(ecmp_count_entry_t));
#ifdef BCM_TOMAHAWK3_SUPPORT
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
             ecmp_mode_hierarchical)) {
        if (ecmp_group_idx < (BCMI_L3_ECMP_OVERLAY_ENTRIES(unit))) {
            mem = ECMP_GROUP_HIERARCHICALm;
        } else if (ecmp_group_idx >= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit)) {
            phy_ecmp_group_idx -= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
        }
    }
#endif
    ...
    BCM_IF_ERROR_RETURN(soc_mem_read(unit, mem, MEM_BLOCK_ANY,
                phy_ecmp_group_idx, &ecmp_count_entry));
    ...
    if (SOC_MEM_IS_VALID(unit, INITIAL_L3_ECMP_GROUPm)) {
        BCM_IF_ERROR_RETURN(soc_mem_read(unit, INITIAL_L3_ECMP_GROUPm, MEM_BLOCK_ANY,
            phy_ecmp_group_idx, &initial_l3_ecmp_group_entry));
        ...
        BCM_IF_ERROR_RETURN(soc_mem_write(unit, INITIAL_L3_ECMP_GROUPm,
            MEM_BLOCK_ALL, phy_ecmp_group_idx, &initial_l3_ecmp_group_entry));
    }
                                               ...
                                               bcm_if_t *ex_new_intf_array)
{
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        if ((soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) &&
                  BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit,
                    (rh_intf_array[i]))) {
            rh_intf_array[i]  = new_intf_array[member_id] +  BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
        } else
#endif
        {
        ...
        if ((ecmp->flags & BCM_L3_WITH_ID)) {
            ...
            if (SOC_IS_TOMAHAWK3(unit)) {
                if (!(ecmp->flags & BCM_L3_REPLACE)) {
                    ...
                    BCM_IF_ERROR_RETURN(_bcm_opt_ecmp_rh_free_resource(unit,
                                                                ecmp_group));
                }
            } else
#endif
...
bcm_opt_l3_egress_ecmp_lb_get(int unit, bcm_l3_egress_ecmp_t *ecmp)
{
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
             ecmp_mode_hierarchical)) {
        if (ecmp_group < (BCMI_L3_ECMP_OVERLAY_ENTRIES(unit))) {
            ecmp->ecmp_group_flags |= BCM_L3_ECMP_OVERLAY;
            mem = ECMP_GROUP_HIERARCHICALm;
        } else if (ecmp_group >= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit)) {
            ecmp_group -= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
        }
    }
#endif
                        ...
                        uint8 lb_mode)
{
    ...
    BCM_IF_ERROR_RETURN(soc_mem_read(unit, mem, MEM_BLOCK_ANY, phy_ecmp_group_idx,
                &ecmp_count_entry));
    soc_mem_field32_set(unit, mem, &ecmp_count_entry,
            LB_MODEf, lb_mode);
    BCM_IF_ERROR_RETURN(soc_mem_write(unit, mem, MEM_BLOCK_ALL, phy_ecmp_group_idx,
                &ecmp_count_entry));

    if (SOC_MEM_IS_VALID(unit, INITIAL_L3_ECMP_GROUPm)) {
        BCM_IF_ERROR_RETURN(soc_mem_read(unit, INITIAL_L3_ECMP_GROUPm,
            MEM_BLOCK_ANY, phy_ecmp_group_idx, &initial_l3_ecmp_group_entry));
        soc_mem_field32_set(unit, INITIAL_L3_ECMP_GROUPm,
...
{
    ecmp_count_entry_t ecmp_count_entry;
    int ecmp_group_idx, phy_ecmp_group_idx;
    initial_l3_ecmp_group_entry_t initial_l3_ecmp_group_entry;
    soc_mem_t mem = L3_ECMP_COUNTm;
    ...
    phy_ecmp_group_idx = ecmp_group_idx;
#ifdef BCM_TOMAHAWK3_SUPPORT
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
             ecmp_mode_hierarchical)) {
        ...
        int new_ecmp_group)
{
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
       (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
             ecmp_mode_hierarchical) &&
        (new_ecmp_group < (BCMI_L3_ECMP_OVERLAY_ENTRIES(unit)))) {
        overlay = 1;
    }
#endif
...
bcm_opt_l3_ecmp_rh_member_sync(int unit, uint8 **scache_ptr, int offset)
{
    ...
    bcm_if_t *mem_ptr;
    int min_rh_idx = 0;
    int max_groups = BCM_XGS3_L3_ECMP_MAX_GROUPS(unit);
    SOC_SCACHE_DUMP_DECL;


#if defined(BCM_TOMAHAWK3_SUPPORT)
    /* In TH3, DGM and RH groups are supported in both overlay and underlay */
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) {
       if (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode != ecmp_mode_hierarchical) {
           max_groups -= soc_mem_index_count(unit, ECMP_GROUP_HIERARCHICALm);
       }
    }
#endif
    ...
    for (i = min_rh_idx; i < max_groups; i++) {
        rh_intf_arr_ptr = (_opt_ecmp_rh_info[unit]->rhg[i].rh_intf_arr);
        if (rh_intf_arr_ptr) {
    ...
    for (i = min_rh_idx; i < BCM_XGS3_L3_ECMP_MAX_GROUPS(unit); i++) {
        rh_intf_arr_ptr = (_opt_ecmp_rh_info[unit]->rhg[i].rh_intf_arr);
        if (rh_intf_arr_ptr) {
            intf_count = _opt_ecmp_rh_info[unit]->rhg[i].rh_intf_count;
            ...
            for (j = 0; j < intf_count; j++) {
                ...
                *scache_ptr += sizeof(int16);
                ...
                /* For empty member case, add 0 as invalid intf */
                *mem_ptr = _opt_ecmp_rh_info[unit]->rhg[i].rh_intf_count == 0 ?
                    0 : rh_intf_arr_ptr[j];
 ...
 *      scache_ptr - (IN/OUT) Scache pointer
 *      underlay_vers - (IN)
 *          0 (TH3 recovered_ver < BCM_WB_VERSION_1_21 or all other devices),
 *          1 (TH3 recovered_ver == BCM_WB_VERSION_1_21),
 *          2 (TH3 recovered_ver >= BCM_WB_VERSION_1_22).
 * Returns:
 *      BCM_E_XXX
 */
int
bcm_opt_l3_ecmp_rh_member_recover(int unit, uint8 **scache_ptr,
                                  int ecmp_max_paths, int offset, int underlay_vers)
{
    bcm_if_t **rh_intf_arr_ptr;
    int stable_size = 0;
    int intf_count;
    ...
    uint8 *end_scache_ptr;
    int16 *grp_ptr;
    int16 ecmp_group_index, hw_grp_idx;
    bcm_if_t *mem_ptr;
    uint8 *start_grp_ptr;
    bcm_if_t *hash_intf_array = NULL;
    int alloc_size;
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) {
       if (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode == ecmp_mode_hierarchical) {
           if (underlay_vers == 1) {
               min_rh_idx = BCMI_L3_ECMP_OVERLAY_ENTRIES(unit);
           } else if (underlay_vers == 0) {
               max_groups -= BCMI_L3_ECMP_OVERLAY_ENTRIES(unit);
           }
       } else {
           max_groups -= soc_mem_index_count(unit, ECMP_GROUP_HIERARCHICALm);
       }
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (stable_size == 0) { /* level 1 */
            ...
            for (j = min_rh_idx; j < max_groups; j++) {
#if defined(BCM_TOMAHAWK3_SUPPORT)
                if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
                    (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                         ecmp_mode_hierarchical) && (underlay_vers == 0)) {
                    int idx = j;
                    idx += BCMI_L3_ECMP_OVERLAY_ENTRIES(unit);
                    sal_memcpy(&(_opt_ecmp_rh_info[unit]->rhg[idx].max_paths),
                               (*scache_ptr),
                               sizeof(uint16));
            ...
            while ((*scache_ptr) < end_scache_ptr) {
                ...
                if (ecmp_group_index != 0) {
                    ...
                    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)  &&
                        ...
                        if (underlay_vers == 0) {
                            ecmp_group_index += BCMI_L3_ECMP_OVERLAY_ENTRIES(unit);
                            mem = L3_ECMP_COUNTm;
                        } else {
                            if (ecmp_group_index < BCMI_L3_ECMP_OVERLAY_ENTRIES(unit)) {
                                mem = ECMP_GROUP_HIERARCHICALm;
                            } else {
                                mem = L3_ECMP_COUNTm;
                                if (ecmp_group_index >= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit)) {
                                    hw_grp_idx -= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
                                }
                            }
                        }
                    }
                    ...
                    *rh_intf_arr_ptr = sal_alloc(sizeof(bcm_if_t) * intf_count,
                     ...
                     * For some releases, non-zero value may be saved for
                     * empty members.
                     */
                    if (!(BCM_XGS3_L3_EGRESS_IDX_VALID(unit, *(*rh_intf_arr_ptr))) &&
                        !(BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, *(*rh_intf_arr_ptr))) &&
                        !(BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit, *(*rh_intf_arr_ptr))))
                    {
                    ...
                    if (BCM_TH_L3_ECMP_LB_MODE_RH ==
                        soc_mem_field32_get(unit, mem, &entry, LB_MODEf)) {
