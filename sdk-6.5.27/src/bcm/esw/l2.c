...
#if defined(BCM_TRX_SUPPORT)
    if (_bcm_l2_port_cbl_profile[unit]) {
        (void )soc_profile_mem_destroy(unit, _bcm_l2_port_cbl_profile[unit]);
        sal_free(_bcm_l2_port_cbl_profile[unit]);
        _bcm_l2_port_cbl_profile[unit] = NULL;
    }
#endif /* BCM_TRX_SUPPORT */
...
_bcm_esw_delete_replace_flags_convert(int unit, uint32 flags, uint32 *repl_flags)
{
    ...
    if (flags & BCM_L2_REPLACE_IGNORE_PENDING) {
        tmp_flags |= BCM_L2_REPLACE_IGNORE_PENDING ;
    }
