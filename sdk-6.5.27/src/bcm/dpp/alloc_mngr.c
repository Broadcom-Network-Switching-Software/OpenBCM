                                             ...
                                             int is_virtual_traps_p_set)
{
    ...
    if (is_virtual_traps_p_set == FALSE) 
    {
        ...
        if (*is_allocated) {
            ...
            virtual_trap_id = *template + _BCM_NUMBER_OF_VIRTUAL_TRAPS_MC_RESERVED;
            rc = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_trap_virtual,SW_STATE_RES_ALLOC_WITH_ID,1,&virtual_trap_id);
            BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_l2_virtual_trap_res_clear(int unit)
{
    ...
    /** Free all virtual trap resources  */
    for (index = 0; index < _BCM_NUMBER_OF_VIRTUAL_TRAPS ; index++)
    {
        if (dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_res_trap_virtual, 1, index) == BCM_E_EXISTS)
        {
            BCMDNX_IF_ERR_EXIT(dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_res_trap_virtual, 1, index));
        }
    }
