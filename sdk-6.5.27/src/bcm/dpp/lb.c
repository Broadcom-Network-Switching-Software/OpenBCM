...
int bcm_petra_lb_init(int unit)
{
    ...
    soc_pbmp_t lb_ctrl_ports;
    uint32     nof_lb_ctrl_port = 0;
    uint8      is_allocated;
    ...
    if (is_coldboot) {
        rv = LB_INFO_ACCESS.is_allocated(unit, &is_allocated);
        ...
        BCMDNX_IF_ERR_EXIT(LB_INFO_ACCESS.lbg_valid_bmp.alloc_bitmap(unit, SOC_TMC_LB_NOF_LBG_WITH_CONTROL));
        ...
        for (modem_id = 0; modem_id < (SOC_TMC_LB_MODEM_MAX+1); modem_id++) {
            ...
            rv = LB_INFO_ACCESS.modem_to_ctrl_ports.set(unit, modem_id, BCM_LB_PORT_INVALID);
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_lb_ports_on_reserve_intf_get, (unit, &lb_ctrl_ports, &nof_lb_ctrl_port)));
        SOC_PBMP_ITER(lb_ctrl_ports, port) {
    ...
    bcm_gport_t port)
{
    ...
    bcm_lbg_t lbg_id;
    uint32 supported_flags = BCM_LB_FLAG_UNMAP|BCM_LB_FLAG_CONTROL_TRAFFIC;
    uint32 is_lb_enhance_support = 0;

    BCMDNX_INIT_FUNC_DEFS;
    rv = BCM_E_NONE;

    LOG_VERBOSE(BSL_LS_BCM_LB, (BSL_META_U(unit, "unit %d, %s modem(%d) to port(%d) for %s port\n"),
        unit, ((flags & BCM_LB_FLAG_UNMAP)?("disconnect"):("connect")), modem_id, port,
        ((flags & BCM_LB_FLAG_CONTROL_TRAFFIC)?("control"):("data"))));
    ...
    soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_enhancement_support_get,(unit, &is_lb_enhance_support));
    _BCM_LB_SYSTEM_UNLOCK(unit);
    if (SOC_FAILURE(soc_rc)) {
        BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_enhancement_support_get, error 0x%x\n"),
            unit, soc_rc));
    }

    if (is_lb_enhance_support == 0) {
        supported_flags = supported_flags & (~BCM_LB_FLAG_CONTROL_TRAFFIC);

        if ((flags & BCM_LB_FLAG_CONTROL_TRAFFIC) != 0) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("unit %d, control traffic is not supported in this devide\n"), unit));    
        }
    }
    ...
    if (flags & BCM_LB_FLAG_CONTROL_TRAFFIC) {
        BCMDNX_IF_ERR_EXIT(LB_INFO_ACCESS.port_to_lbg.get(unit, user_port, &lbg_id));
        if (lbg_id != SOC_TMC_LB_CONTROL) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("unit %d, unexpected port(0x%x), it's not a link bonding control port"), unit, port));
        }
        is_lb_ctrl_port = 1;
    } else {
    ...
    if (flags & BCM_LB_FLAG_UNMAP) {
        _BCM_LB_SYSTEM_LOCK(unit);
        soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_modem_to_port_unmap_set,(unit, is_lb_ctrl_port, modem_id, port));
        _BCM_LB_SYSTEM_UNLOCK(unit);
        if (SOC_FAILURE(soc_rc)) {
            BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_egr_modem_to_port_unmap_set is_lb_ctrl_port(%d) modem(%d) port(%d), error 0x%x\n"),
                unit, is_lb_ctrl_port, modem_id, port, soc_rc));
        }
    }
    else {
        _BCM_LB_SYSTEM_LOCK(unit);
        soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_modem_to_port_map_set,(unit, is_lb_ctrl_port, modem_id, port));
        _BCM_LB_SYSTEM_UNLOCK(unit);
        if (SOC_FAILURE(soc_rc)) {
            BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_egr_modem_to_port_map_set is_lb_ctrl_port(%d) modem(%d) port(%d), error 0x%x\n"),
                unit, is_lb_ctrl_port, modem_id, port, soc_rc));
        }
    }
    ...
    if (flags & BCM_LB_FLAG_CONTROL_TRAFFIC) {
        rv = LB_INFO_ACCESS.modem_to_ctrl_ports.set(unit, modem_id, port);
    } else {
    ...
    bcm_gport_t *port)
{
    ...
    soc_error_t soc_rc = SOC_E_NONE;
    bcm_port_t user_port;
    uint32 supported_flags = BCM_LB_FLAG_CONTROL_TRAFFIC;
    uint32 is_lb_enhance_support = 0;
    ...
    /* get port from SW DB */
    if (flags & BCM_LB_FLAG_CONTROL_TRAFFIC) {
        rv = LB_INFO_ACCESS.modem_to_ctrl_ports.get(unit, modem_id, &user_port);
    } else {
    ...
    int arg)
{
    ...
    if (is_global) {
        ...
        case bcmLbControlSegmentationMode:
            if (SOC_IS_QUX(unit)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("unit %d, control not supported: %d, with flag, 0x%8.8x"), unit, lb_control, BCM_LB_FLAG_GLOBAL));
            }
        ...
        case bcmLbControlNofTotalSharedBuffers:
            ...
            break;
        case bcmLbControlTPIDOfControlPackets:
            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_enhancement_support_get,(unit, &is_lb_enhance_support));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
            ...
            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_ing_control_packet_identification_config_set,(unit, 0, arg));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
                BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_ing_control_packet_identification_config_set, error 0x%x\n"),
                    unit, soc_rc));
            }

            break;
        case bcmLbControlTrafficEnable:
            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_enhancement_support_get,(unit, &is_lb_enhance_support));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
            ...
            lbg_id_l = SOC_TMC_LB_CONTROL;

            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_lbg_enable_set,(unit, lbg_id_l, arg));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
                BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_egr_lbg_enable_set lbg_id(%d) enable(%d), error 0x%x\n"),
                    unit, lbg_id_l, arg, soc_rc));
            }
    ...
    int *arg)
{
    ...
    if (is_global) {
        ...
        case bcmLbControlTPIDOfControlPackets:
            ...
            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_ing_control_packet_identification_config_get,(unit, 0, &global_tpid));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
                BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_ing_control_packet_identification_config_get, error 0x%x\n"),
                    unit, soc_rc));
            }
            *arg = global_tpid;
        ...
        case bcmLbControlTrafficEnable:
            ...
            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_lbg_enable_get,(unit, lbg_id_l, &is_enable));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
                BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_egr_lbg_enable_get lbg_id(%d), error 0x%x\n"),
                    unit, lbg_id_l, soc_rc));
            }
    ...
    int arg)
{
    ...
    case bcmLbModemControlHeaderCompensation:
        ...
        soc_shaper_config.op_flags = SOC_TMC_LB_MODEM_SHARPER_CONFIG_OP_FLAG_HDR_COMPENSATION;
    ...
    bcm_lb_modem_shaper_config_t *shaper)
{
    ...
    if (flags & BCM_LB_FLAG_MODEM_SHAPER_SEGMENT) {
        ...
        soc_bit_shaper.op_flags = SOC_TMC_LB_MODEM_SHARPER_CONFIG_OP_FLAG_SHAPER;
    }
