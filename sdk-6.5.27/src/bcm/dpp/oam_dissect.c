...
{
    int rv = BCM_E_NONE;
    uint8 is_inferred_lm = 0, is_accelerated = 0;
    ...
    /**
     * Return E_PARAM if RFC6374 is called on Q*X devices
     */
    if ((SOC_IS_QUX(unit) || SOC_IS_QAX(unit)) && is_accelerated) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                (_BSL_BCM_MSG("Accelerated RFC-6374 is not supported on QAX and QUX devices.")));
    }
...
int _bcm_oam_group_create_verify(int unit, bcm_oam_group_info_t *group_info) {
    ...
    if (_BCM_OAM_IS_CUSTOM_FEATURE_OAMP_FLEXIBLE_DA_SET(unit)) {
        if (!_BCM_OAM_GROUP_MAID_IS_EXTRA_DATA(group_info->flags)) {
             BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: Only flexible 20/48 byte MAID groups are allowed when custom feature flexible da is set\n")));
        }
    }

    if (SOC_IS_QAX(unit) || soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "maid48_enabled", 0)) {
       /*checking input parameters*/
        legal_flags |= (BCM_OAM_GROUP_FLEXIBLE_MAID_48_BYTE | BCM_OAM_GROUP_FLEXIBLE_MAID_20_BYTE | BCM_OAM_GROUP_REPLACE | BCM_OAM_GROUP_RX_NAME);
    }
    ...
    /* 
     * Flag BCM_OAM_GROUP_FLEXIBLE_MAID_20_BYTE is used to support 
     * one special MAID format where
     * 1) the first byte is "1";
     * 2) the second byte is "2";
     * 3) the second byte is "14";
     * 4) the others are flexible;
     * 5) the total legth is 17 including the first 3 bytes;
    */
    if (group_info->flags & BCM_OAM_GROUP_FLEXIBLE_MAID_20_BYTE) {
        if (group_info->name[0] != 1 || group_info->name[1] != 2 || group_info->name[2] != 14) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: the first 3 bytes must be '1' '2' '14' with flexible 20 byte MAID\n")));
        }
    }
    ...
    if (group_info->flags & BCM_OAM_GROUP_REPLACE) {
        ...
        if (((group_info->flags & BCM_OAM_GROUP_WITH_ID) == 0) ||
            (!_BCM_OAM_GROUP_MAID_IS_EXTRA_DATA(group_info->flags))) {
             BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: Only existing groups with flexible 20/48 byte MAIDs can be renamed\n")));
        }
    ...
    if (group_info->flags & BCM_OAM_GROUP_RX_NAME) {
        ...
        if (!_BCM_OAM_GROUP_MAID_IS_EXTRA_DATA(group_info->flags)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: different names for rx and tx allowed only with flexible 20/48 byte MAID\n")));
        }
    }
