  ...
  )
{
  ...
  */
  ...
  /*
   * For multicast routing into a unicast destination format A is used or format B 
   * when going into a composite object or a tunnel (i.e. identical to unicast routing). 
   */
  if ((working_fec.rpf_info.rpf_mode & SOC_PPC_FRWRD_FEC_RPF_MODE_MC_EXPLICIT) &&
      (working_fec.dest.dest_type == SOC_SAND_PP_DEST_EXPLICIT_FLOW ||
       working_fec.dest.dest_type == SOC_SAND_PP_DEST_SINGLE_PORT)) 
  {
      working_fec.type = SOC_PPC_FEC_TYPE_ROUTING;
      working_fec.eep = eep_index;
      working_fec.app_info.out_rif = out_rif;
  }
...
{ 
  ...
  /** Get the enrty info first in case of replace*/
  if (flags & BCM_L3_REPLACE) {
    ...
    sal_memcpy(&ll_encap_info, &(encap_entry_info->entry_val.ll_info), sizeof(ll_encap_info));
  }
    ...
    bcm_if_t *if_id)
{
    ...
    if (flags & ~BCM_L3_EGRESS_ONLY && _BCM_PPD_IS_VALID_FAILOVER_ID(egr->failover_id)) {
    ...
    bcm_if_t *if_id)
{
    ...
    if (is_egress) {    
        ...
        if ((encap_with_id) && (flags & BCM_L3_REPLACE)) { /* second call */
            ...
            ret = _bcm_l3_alloc_eep(unit, egr, &tunnel_local_lif, &tunnel_global_lif, &ll_local_lif, &ll_global_lif, &is_need_add_glem); 
            if (ret != BCM_E_NONE) {
        ...
        */
        ...
        if ((!encap_with_id) || ((flags & (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN | BCM_L3_REPLACE)) != (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN | BCM_L3_REPLACE))) {
            status = _bcm_ppd_eg_encap_entry_add(unit, flags, fec_index, tunnel_local_lif, ll_local_lif, &fec_eep_pointer, egr);
            if (status != BCM_E_NONE) {
                BCMDNX_IF_ERR_EXIT(status);
            }
        }
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, ll_global_lif, ll_local_lif, TRUE);
    ...
    void *user_data)
{
    ...
    if ((flags & BCM_L3_IP6)) {
        ...
        if (SOC_IS_JERICHO(unit)
#if defined(INCLUDE_KBP)
            && !(ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED)
