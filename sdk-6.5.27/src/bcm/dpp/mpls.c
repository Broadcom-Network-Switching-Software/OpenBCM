...
{    
    ...
    if(SOC_IS_JERICHO_PLUS(unit)) {
        ...
        if (dscp_preserve_with_extend_label > 0) {
            ...
            BCMDNX_IF_ERR_EXIT(rv);
            if (!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "preserving_dscp_oam_ts_disabled", 0)) {
                rv = arad_egr_prog_editor_prog_sel_outlif_profile_set(unit, SOC_OCC_MGMT_OUTLIF_APP_ADDITIONAL_LABEL,
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    int local_lif_id = 0 , global_lif_id = 0;
    ...
    for (start_indx = num_labels; start_indx > 0; start_indx = start_indx - 1 - skip_entries) {
        ...
        if(!eep_alloced) {
            ...
            if ((indx+is_bind_entries+1 == num_labels) 
                ...
                int is_2_eedb_allocated = ((num_labels == 2)&&(is_bind_entries == 0));    
                    if (SOC_IS_JERICHO_PLUS(unit)) 
                    {
                         /*
                 ...
                 /*
                  * When 2 EEDB entries created, their access stage is fixed.
                  * next pointer type is not used for access stage indication
                  */
                 if (is_2_eedb_allocated == 0) 
                 {
                     phase_stage++;
                 }
            }
            ...
            if (SOC_IS_JERICHO(unit))
            {
                ...
                local_lif_id_to_add_glem = local_lif_id;
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, global_lif_id, local_out_lif, TRUE);
        BCMDNX_IF_ERR_REPORT(rv1);
    }
