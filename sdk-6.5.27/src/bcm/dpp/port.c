...
_bcm_petra_port_add_lb_speed_set(int unit, bcm_port_t port)
{
    ...
    if ((pcnt != 0) || (SOC_PORT_IF_LBG == interface)) {
        ...
        /* update LBG FIFO, Credit Size and SW DB*/
    ...
    case bcmPortControlAllowQosInheritance:
        ...
        case bcmPortControlPreserveDscpIngress:
            if ((SOC_DPP_CONFIG(unit))->pp.routing_preserve_dscp & SOC_DPP_DSCP_PRESERVE_INLIF_CTRL_EN) {
                ...
                rv = _bcm_petra_port_inlif_profile_get(unit, port, (uint32*)value, SOC_OCC_MGMT_INLIF_APP_PRESERVE_DSCP, 0/*all_kinds*/, lif_info);
                BCMDNX_IF_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("failed getting the inlif profile")));
            } else {
                BCMDNX_IF_ERR_EXIT(BCM_E_UNAVAIL);
            }
        ...
        case bcmPortControlPreserveDscpEgress:
            if ((SOC_DPP_CONFIG(unit))->pp.routing_preserve_dscp & SOC_DPP_DSCP_PRESERVE_OUTLIF_CTRL_EN) {
                ...
                soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_occ_mgmt_app_get,
                                                  (unit, SOC_OCC_MGMT_TYPE_OUTLIF, SOC_OCC_MGMT_OUTLIF_APP_PRESERVE_DSCP, &outlif_profile,
                                                   (uint32 *)value));
                BCMDNX_IF_ERR_EXIT(soc_sand_rv);
            } else {
                BCMDNX_IF_ERR_EXIT(BCM_E_UNAVAIL);
...
int bcm_petra_port_add(int unit, bcm_port_t port, uint32 flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info)  
{
    ...
    if (SOC_IS_QAX(unit) && SOC_DPP_CONFIG(unit)->qax->lbi_en && (interface_info->interface != SOC_PORT_IF_LBG)) {
        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_lb_is_control_port_get, (unit, port, &is_lb_ctrl_port, 1)));
    }
...
int bcm_petra_port_get(int unit, bcm_port_t port, uint32 *flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info) 
{
    ...
    *flags = 0;
    ...
    if (!SOC_PORT_IS_LB_MODEM(port_info_flags))
    {
    ...
    if ((!SOC_PORT_IS_ELK_INTERFACE(port_info_flags)) && (!SOC_PORT_IS_LB_MODEM(port_info_flags)))
    {
    ...
    int *total_rate)
{
    ...
    *total_rate = 0;
    ...
    BCM_PBMP_ITER(pbmp, logical_port)
    {
        ...
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_flags_get(unit, logical_port, &port_flags));

        if ((!is_channelized) && (!SOC_PORT_IS_LB_MODEM(port_flags)))
        {
    ...
    case SOC_PPC_EG_ENCAP_ENTRY_TYPE_LL_ENCAP:
        if (get_1_set_0) {
            *outlif_profile = encap_entry_info->entry_val.ll_info.outlif_profile;
            ...
            if (set_oam_lif_valid !=-1) {
                encap_entry_info->entry_val.ll_info.oam_lif_set = 1;
            }
            if (outlif_profile) {
                encap_entry_info->entry_val.ll_info.outlif_profile = *outlif_profile; 
            }
            soc_sand_rv = soc_ppd_eg_encap_ll_entry_add(unit, gport_hw_resources.local_out_lif, &(encap_entry_info->entry_val.ll_info));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        break;
    default:
        ...
        case bcmPortControlOamDefaultProfile:
            {
                ...
                if (SOC_IS_ARADPLUS(unit)) {
                    ...
                    case 0:  /* Advanced mode */
                        ...
                        BCMDNX_IF_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("bcmPortControlOamDefaultProfile, case 0: failed getting the inlif profile")));
                    ...
                    case 1:  /* Simple mode - 1 bit */
                    case 2:  /* Simple mode - 2 bits */
                        rv = _bcm_petra_port_inlif_profile_get(unit, port, &oam_trap_profile, SOC_OCC_MGMT_INLIF_APP_OAM, 0/*all_kinds*/, &lif_entry);
                        BCMDNX_IF_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("bcmPortControlOamDefaultProfile, case 1,2: failed getting the inlif profile")));
                        break;
                    default:
                        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("OAM default profile mode unsupported (profile = %d)"), oam_lif_profile_mode));
                    }
                ...
                int unit, int lif_id, uint16 tpid, _bcm_petra_tpid_op_t oper)
{
    ...
    if ((lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_PWE) &&
        ...
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (
            _BSL_BCM_MSG("Unexpected lif id. Must be of type PWE, I-SID or IP tunnel")));
    }
    ...
    if (lif_tpid_type != _bcm_petra_tpid_profile_none) {
        /* PWE/I-SID/IP tunnel use an assigned tpid profile */
        if (lif_entry_info->type == SOC_PPC_LIF_ENTRY_TYPE_PWE) {
            tpid_profile = lif_entry_info->value.pwe.tpid_profile_index;
        }
        else if (lif_entry_info->type == SOC_PPC_LIF_ENTRY_TYPE_IP_TUNNEL_RIF){
            tpid_profile = lif_entry_info->value.ip_term_info.tpid_profile;
        }
        else { /* I-SID */
    ...
    if (lif_entry_info->type == SOC_PPC_LIF_ENTRY_TYPE_PWE) {
        ...
        lif_entry_info->value.ip_term_info.tpid_profile = new_tpid_profile;
    }
    else {
