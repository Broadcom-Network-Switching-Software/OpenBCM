...
_bcm_petra_bfd_uc_alloc_resource(int unit, int num_sessions,
                             int num_auth_sha1_keys, int num_auth_sp_keys,
                              uint8 use_localdiscr_as_sess_id, uint8 is_swstate_allocated)
{
    int rv, num_auth;
    ...
    if (use_localdiscr_as_sess_id && !is_swstate_allocated) {
                             ...
                             uint8 r_subclass, uint16 *r_len)
{
    ...
    rv = soc_cmic_uc_msg_send(unit, uc_num, &send,
                              (_BCM_BFD_UC_MSG_TIMEOUT_USECS / 2));
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_BFD,
                  (BSL_META_U(unit, "BFD Msg send failed %d\n"), rv));
        return rv;
    }

    rv = soc_cmic_uc_msg_receive(unit, uc_num, MOS_MSG_CLASS_BFD,
                                 &reply, (_BCM_BFD_UC_MSG_TIMEOUT_USECS / 2));
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_BFD,
                  (BSL_META_U(unit, "BFD Msg receive failed %d\n"), rv));
        return rv;
    }

    /* Check reply class, subclass */
    if ((reply.s.mclass != MOS_MSG_CLASS_BFD) ||
...
bcm_petra_bfd_uc_init(int unit)
{
    ...
    rv = _bcm_petra_bfd_uc_alloc_resource(unit, num_sessions, num_auth_sha1,
                                        num_auth_sp, use_localdiscr_as_sess_id,is_allocated);
    if (BCM_FAILURE(rv)) {
        _bcm_petra_bfd_uc_free_resource(unit);
        return rv;
    }
