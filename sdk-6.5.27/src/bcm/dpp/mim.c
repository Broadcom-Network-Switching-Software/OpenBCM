...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, local_out_lif_info.glif, local_out_lif_info.base_lif_id, TRUE);
    ...
    bcm_mim_port_t * mim_port)
{
    ...
    int global_in_lif_id, global_out_lif_id = 0, local_out_lif_id = 0;
    SOC_PPC_L2_LIF_ISID_KEY isid_key;
    SOC_PPC_L2_LIF_ISID_INFO isid_info;
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, local_out_lif_info.glif, local_out_lif_id, TRUE);
        BCMDNX_IF_ERR_REPORT(rv1);
    }
