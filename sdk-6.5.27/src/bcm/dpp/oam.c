...
int _bcm_dpp_oam_group_name_to_ma_name_struct_convert(int unit, uint8 *group_name, _bcm_oam_ma_name_t *ma_name_struct) {
    ...
    if (((ma_name_struct->name_type == _BCM_OAM_MA_NAME_TYPE_STRING_48_BYTE) ||
         (ma_name_struct->name_type == _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE)) &&
...
int _bcm_oam_write_group_data_to_oamp_mep_db(int unit, bcm_oam_group_info_t *group_info) {
   ...
   int data_size_in_uint32;
   int group_name_start;

   BCMDNX_INIT_FUNC_DEFS;

   if (group_info->flags & BCM_OAM_GROUP_FLEXIBLE_MAID_20_BYTE) {
       nof_extra_data_entries = 1;
       data_size_in_byte = 14;
       data_size_in_uint32 = data_size_in_byte / sizeof(uint32) + 1;
       group_name_start = 3;
   } else {
       nof_extra_data_entries = 3;
       data_size_in_byte = _BCM_OAM_MA_NAME_48;
       data_size_in_uint32 = data_size_in_byte / sizeof(uint32);
       group_name_start = 0;
   }
   ...
   if ((group_info->flags & BCM_OAM_GROUP_REPLACE) == 0) {
       ...
       for (i = 0; i < nof_extra_data_entries; i++, Entry_Index += (SOC_IS_QUX(unit) ? 0x100 : 0x400)) {
   ...
   sal_memset(&oamp_db_group_info, 0, sizeof(SOC_PPC_OAM_BFD_MEP_DB_EXT_DATA_INFO));
   for (word_index = 0; word_index < data_size_in_uint32; word_index++) {
       /* Make sure the 32 bit dword array is arranged big-endian, no matter what CPU is used */
       oamp_db_group_info.data[word_index] = (group_info->name[word_index * 4 + group_name_start] << 24) |
                                             (group_info->name[word_index * 4 + group_name_start + 1] << 16) |
                                             (group_info->name[word_index * 4 + group_name_start + 2] << 8) |
                                              group_info->name[word_index * 4 + group_name_start + 3];
   }
   oamp_db_group_info.mep_idx = -1; /* MEPs are left untouched */
   oamp_db_group_info.extension_idx = group_info->group_name_index;
   oamp_db_group_info.data_size_in_bits = data_size_in_byte * 8; /* 14 or 48 bytes */
...
int _bcm_dpp_oam_fifo_interrupt_handler(int unit, SOC_PPC_OAM_DMA_EVENT_TYPE event_type, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA * interrupt_data) {
    ...
    while (valid || interrupt_data->additional_words_to_read) {
        if (interrupt_data->additional_words_to_read == 0) { /* only handle complete event */
...
int bcm_petra_oam_group_create(int unit, bcm_oam_group_info_t *group_info) {
    ...
    else
    {
        ...
        /* fully flexible 48B MAID format */
        if ((SOC_IS_QAX(unit) || (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "maid48_enabled", 0))) && _BCM_OAM_GROUP_MAID_IS_EXTRA_DATA(group_info->flags)) { 
            if (group_info->flags & BCM_OAM_GROUP_FLEXIBLE_MAID_48_BYTE)
            {
                ma_name_struct.name_type = _BCM_OAM_MA_NAME_TYPE_STRING_48_BYTE;
            }
            else if (group_info->flags & BCM_OAM_GROUP_FLEXIBLE_MAID_20_BYTE)
            {
                ma_name_struct.name_type = _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE;
            }
            if (SOC_IS_QAX(unit)) {
...
int bcm_petra_oam_group_get(
    ...
    if (((ma_name_struct.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_48_BYTE) ||
         (ma_name_struct.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE)) && SOC_IS_QAX(unit)) {
...
STATIC int _bcm_oam_endpoint_local_create(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl)
{
    ...
    if (is_accelerated) {
        if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "maid48_enabled", 0))) {
            ...
            } else if (ma_name_struct.name_type ==  _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE) {
                mep_db_entry.maid_48_index = ma_name_struct.index;
                mep_db_entry.is_maid_20 = 1;
            }
        }
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            eth_prof.maid_check_dis = ((ma_name_struct.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_48_BYTE) ||
                                       (ma_name_struct.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE)) && SOC_IS_QAX(unit);
...
int _bcm_oam_lm_dm_pointed_add_shared(int unit, SOC_PPC_OAM_OAMP_LM_DM_MEP_DB_ENTRY *lm_dm_mep_db_entry) {
    ...
    if (ma_name_struct.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_48_BYTE) {
        ...
        shared_entries_data.is_maid_20 = 0;
    } else if (ma_name_struct.name_type == _BCM_OAM_MA_NAME_TYPE_STRING_20_BYTE) {
        shared_entries_data.is_maid_48 = 0;
        shared_entries_data.is_maid_20 = 1;
    }else {
        shared_entries_data.is_maid_48 = 0;
        shared_entries_data.is_maid_20 = 0;
    }
