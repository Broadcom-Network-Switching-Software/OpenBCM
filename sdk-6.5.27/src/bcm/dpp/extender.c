 ...
 *      is_need_add_glem - indicate is need to add glem by caller.
 * Returns:
 *      BCM_E_NONE
 *      BCM_E_XXX
 *
        ...
        bcm_extender_port_t *extender_port)
{
    ...
    if (!is_remote_lif) {
        ...
        rv = _bcm_dpp_extender_alloc(unit, specified_global_lif_id, extender_port, &extender_lif_resources, &is_need_add_glem);
...
exit:
    if (is_need_add_glem) {
        int rv1;
        rv1 = _bcm_dpp_lif_glem_access_entry_operation(unit, extender_lif_resources.global_out_lif, extender_lif_resources.local_out_lif, TRUE);
