...
#endif /* BCM_88660 */
    SOC_PPC_LIF_COS_OPCODE_TYPE opcode_type = 0 ;
...
_bcm_petra_qos_map_ingress_cos_opcode_add(int unit, uint32 flags, bcm_qos_map_t *map, int map_id)
{
    ...
    int dev_id;
    uint32 opcode_ndx, sand_rv;
    SOC_PPC_LIF_COS_OPCODE_TYPE opcode_type = 0, type;
    uint8 bit_val;
    BCMDNX_INIT_FUNC_DEFS;
            ...
            int *flags_array, int *array_count)
{
    ...
    if (array_size == 0) {
        ...
        if (BCM_SUCCESS(rv)) {
            count = 0;
            *array_count = 0;
        for(idx=1; idx < SOC_DPP_CONFIG(unit)->qos.nof_ing_lif_cos; idx++) {
        if(bcm_dpp_am_qos_ing_lif_cos_is_alloced(unit,idx) == BCM_E_EXISTS) {
            count++;
        }
        }
        for(idx=1; idx < SOC_DPP_CONFIG(unit)->qos.nof_ing_pcp_vlan; idx++) {
        if(bcm_dpp_am_qos_ing_pcp_vlan_is_alloced(unit,idx) == BCM_E_EXISTS) {
            count++;
        }
        }
        for(idx=1; idx < SOC_DPP_CONFIG(unit)->qos.nof_ing_dp; idx++) {
        ...
        for(idx=1; idx < SOC_DPP_CONFIG(unit)->qos.nof_egr_remark_id; idx++) {
        if(bcm_dpp_am_qos_egr_remark_id_is_alloced(unit,idx) == BCM_E_EXISTS) {
            count++;
        }
        }
        for(idx=1; idx < SOC_DPP_CONFIG(unit)->qos.nof_egr_mpls_php; idx++) {
        ...
        *array_count = count;
        ...
        if (BCM_SUCCESS(rv)) {
            count = 0;
            for (idx = 1; ((idx < SOC_DPP_CONFIG(unit)->qos.nof_ing_lif_cos) && 
            ...
            for (idx = 1; ((idx < SOC_DPP_CONFIG(unit)->qos.nof_ing_pcp_vlan) && 
                         (count < array_size)); idx++) {
                if (bcm_dpp_am_qos_ing_pcp_vlan_is_alloced(unit, idx) == BCM_E_EXISTS) {
            ...
            for (idx = 1; ((idx < SOC_DPP_CONFIG(unit)->qos.nof_ing_dp) && 
                         (count < array_size)); idx++) {
                if (bcm_dpp_am_qos_ing_color_is_alloced(unit, idx) == BCM_E_EXISTS) {
            ...
            for (idx = 1; ((idx < SOC_DPP_CONFIG(unit)->qos.nof_egr_remark_id) && 
                         (count < array_size)); idx++) {
                if (bcm_dpp_am_qos_egr_remark_id_is_alloced(unit, idx) == BCM_E_EXISTS) {
            ...
            for (idx = 1; ((idx < SOC_DPP_CONFIG(unit)->qos.nof_egr_mpls_php) && 
                         (count < array_size)); idx++) {
                if (bcm_dpp_am_qos_egr_mpls_php_id_is_alloced(unit, idx) == BCM_E_EXISTS) {
            ...
            for (idx = 0; ((idx < SOC_DPP_CONFIG(unit)->qos.nof_egr_pcp_vlan) && 
                ...
                if (bcm_dpp_am_qos_egr_pcp_vlan_is_alloced(unit, idx) == BCM_E_EXISTS) {
                    ...
                    _DPP_QOS_ENTRY_USED_CNT(egr_pcp_vlan_stag_bitmap, idx, _BCM_QOS_MAP_EGR_VLAN_PRI_MAX*2, entry_count);
                    if (entry_count != 0) {
