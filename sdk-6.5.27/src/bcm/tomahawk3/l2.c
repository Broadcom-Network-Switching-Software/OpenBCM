...
static int _bcm_th3_pend_lrn_en[BCM_MAX_NUM_UNITS];
...
/* Controls issuing of callbacks to the application (based on soc property) */
static int _bcm_th3_wb_recovery_cb_pause[SOC_MAX_NUM_DEVICES];
...
bcm_tomahawk3_l2_init(int unit)
{
    ...
    _bcm_th3_pend_lrn_en[unit] = soc_property_get(unit, spn_L2_SW_PENDING_LEARN,
                                                  0);

    /* Set value for default/current behavior. In 'mbcm_l2_init', if the soc
     * property is enabled and wb recovery is in progress, the value below will
     * be updated
     */
    _bcm_th3_wb_recovery_cb_pause[unit] = FALSE;
    ...
    /* On real h/w, the sync thread will be started later in the init */
    if ((SAL_BOOT_SIMULATION) && (!SOC_WARM_BOOT(unit)) &&
        (!soc_l2x_running(unit, &flags, &l2x_interval))) {
...
/*
 * Function: _bcm_th3_l2_wb_recovery_pause_resume_cb
 * Description:
 *	This function is used to set the initial states of various flags based on
 *	the setting of the soc property, spn_L2XMSG_NO_CB_DURING_TABLE_REBUILD. It
 *	flags' values are used by the learn, age and the sync threads to determine
 *	whether to temporarily pause their functions, speicially during wb recovery
 *	stage
 * Parameters:
 *	unit - StrataSwitch PCI device unit number
 *	action - Macros _BCM_L2_WB_RECOVERY_*
 * Returns:
 *	BCM_E_NONE Success
 *	BCM_E_XXX  Any failure conditions
 */
int
_bcm_th3_l2_wb_recovery_pause_resume_cb(int unit, int action)
{
    int cb_action;

    if (action == _BCM_L2_WB_RECOVERY_PAUSE_CALLBACKS) {
        cb_action = TRUE;
    } else {
        cb_action = FALSE;
    }

    /* Note we don't need need mutex for the flag below because the flag is
     * updated at different times - first in sequence is the pause operation
     * is initiated by the bcm layer and the second in sequence is in the
     * callback function, _bcm_th3_l2_register_callback. During the first step,
     * the sync thread is off, so no callback can occur
     */
    _bcm_th3_wb_recovery_cb_pause[unit] = cb_action;

    /* Set action for the sync thread to check when it starts running */
    SOC_IF_ERROR_RETURN(soc_l2x_wb_recovery_rebuild_cb_action_set(unit,
        action));

    /* The first two calls are used to inform the learn and aging threads to
     * not modify the L2 table post-warmboot until the sync thread synchronizes
     * its shadow table with the h/w. For normal init, there is no change in the
     * threads' behavior
     */
    /* Set flag for learn thread. Since it is off at the time when this flag is
     * set, it will check the flag state and modify it when it starts
     */
    SOC_IF_ERROR_RETURN(soc_th3_l2_learn_wb_recovery_rebuild_cb_action_set(
        unit, cb_action));

    /* Set flag for aging thread. Since it is off at the time when this flag is
     * set, it will check the flag state and modify it when it starts. Even if
     * aging thread is not started at all, it won't have any effect on any
     * functionality
     */
    SOC_IF_ERROR_RETURN(soc_th3_l2_age_wb_recovery_rebuild_cb_action_set(
        unit, cb_action));
                              ...
                              void *fn_data)
{
    ...
    /* No mutex needed for the flag below - check comment in
     * _bcm_th3_l2_wb_recovery_pause_resume_cb()
     */
    /* A scenario: If the L2 table is empty, this function will not be invoked.
     * So the flag below will stay in 'true' state. This is ok since it will be
     * updated in future when a new entry gets added to the L2 table. If a new
     * entry is not added at all, at some point in future L2 module, or the sdk
     * itself will get initialized, and this will reset the value of the flag to
     * the default value
     */
    if (_bcm_th3_wb_recovery_cb_pause[unit] == TRUE) {
        int rv;

        rv = soc_l2x_wb_recovery_rebuild_cb_action_get(unit, &state);
        if (rv == SOC_E_NONE) {

            /* Sync thread rebuilding its shadow table */
            if (state == SOC_L2X_WB_RECOVERY_DB_REBUILD_PAUSE_CALLBACKS) {
                return;
            }

            if (state == SOC_L2X_WB_RECOVERY_DB_REBUILD_RESUME_CALLBACKS) {
                /* Sync thread done rebuilding its shadow table, resume normal
                 * processing
                 */
                _bcm_th3_wb_recovery_cb_pause[unit] = FALSE;
            }
        } else {
            /* If BCM layer did not initiate no-callback-during-rebuild
             * behavior, we will simply follow the current/default behavior
             * of reporting callbacks to the application
             */
            _bcm_th3_wb_recovery_cb_pause[unit] = FALSE;
        }
    }
    ...
    if (pflags & (SOC_L2X_ENTRY_DUMMY | SOC_L2X_ENTRY_NO_ACTION)) {
        ...
        /* If pending learn feature is enabled, the entry was previously sent to
         * the application and the application may have populated it. If so, we
         * should not issue another callback for that entry addition from this
         * (sync) thread. Applicable to new entries only
         */
        if (_bcm_th3_pend_lrn_en[unit] && (entry_add != NULL) &&
            (entry_del == NULL)) {
            uint32 callback = TRUE;
            int rv1;

            rv1 = soc_th3_lrn_shadow_pending_entry_cb_check(unit, entry_add,
                      &callback);

            if ((rv1 != BCM_E_NONE) || (callback == FALSE)) {
                return;
            }
        }
...
/*
 * Function:
 *      _bcm_th3_l2_pending_learn_callback
 * Description:
 *      Call back to handle bcm_l2_addr_register clients
 * Parameters:
 *      unit - StrataSwitch unit number.
 *      info - Entry to be sent to application for learning
 * Notes:
 *      Only one callback per unit to the bcm layer is supported
 *      Assumes property spn_L2_SW_PENDING_LEARN is enabled
 */
void
_bcm_th3_l2_pending_learn_callback(int unit, soc_l2_pending_learn_info_t *info,
                                   void *data)
{
    bcm_l2_addr_t entry;

    bcm_l2_addr_t_init(&entry, info->mac, info->vlan);

    if (info->dst_type) {
        entry.tgid = info->portnum_or_trunk_id;
        entry.flags |= BCM_L2_TRUNK_MEMBER;
    } else {
        entry.tgid = BCM_TRUNK_INVALID;
        entry.port = info->portnum_or_trunk_id;
    }
    entry.modid = info->modid;

    if (!(entry.flags & BCM_L2_TRUNK_MEMBER)) {
        if (bcm_tomahawk3_l2_port_native(unit, entry.modid, entry.port) > 0) {
            entry.flags |= BCM_L2_NATIVE;
        }
    }
    entry.flags |= BCM_L2_PENDING; /* We are setting PENDING flag here */
    ...
    if (_bcm_th3_l2_cbs[unit] != NULL) {
        _bcm_th3_l2_cbs[unit](unit, &entry, BCM_L2_CALLBACK_ADD,
            _bcm_th3_l2_cb_data[unit]);
    }
    ...
    if (_bcm_th3_pend_lrn_en[unit] && (_bcm_th3_l2_cbs[unit] == NULL)) {
        uint32 flags = 0x0;

        /* No application callback registered, remove learn shadow entry */

        /* Note, learn shadow lock will be acquired inside the call */
        (void)soc_th3_lrn_shadow_entry_delete(unit, info->mac, info->vlan,
                  TRUE, &flags);
    }
...
_bcm_th3_l2_replace_data_mask_setup(int unit, _bcm_l2_replace_t *rep)
{
    ...
    /* BCM_L2_FLAGS2_MESH means to replace mesh-bit-set
     * L2 entries along with mesh-bit-unset L2 entries.
     */
    if (!(rep->key_l2_flags2 & BCM_L2_FLAGS2_MESH ||
          rep->int_flags & _BCM_L2_REPLACE_INT_MATCH_MESH)) {
        /* In TH3 there is no MESH bit. SW is using bit 79 in the HW entry
         * to store this MESH information. This bit is marked/named as RESERVED
         * field, hence using the RESERVEDf to denote mesh functionality. */
        soc_mem_field32_set(unit, L2Xm, &rep->match_mask, RESERVEDf, 1);
...
_bcm_th3_l2_entry_replace(int unit, void *trav_st)
{
    ...
    if (rep_st->flags & BCM_L2_REPLACE_DELETE) {
        ...
        match_static = (rep_st->flags & BCM_L2_REPLACE_MATCH_STATIC) ||
                        (rep_st->int_flags &
                         _BCM_L2_REPLACE_INT_MATCH_ONLY_STATIC);
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            if (rep_st->flags & BCM_L2_REPLACE_NO_CALLBACKS) {
                ...
                /* If BCM_L2_REPLACE_NO_CALLBACKS is not set, we should issue
                 * callback to the application; so we remove entry from
                 * 'suppress' db (if callback supress feature is enabled)
                 */
                if (soc_th3_l2x_appl_callback_ready(unit)) {
                    sal_mac_addr_t mac;
                    bcm_vlan_t vlan;
                    ...
                    /* Delete entry from application callback suppress db */
            ...
            if (match_static) {
                soc_th3_lrn_shadow_delete(unit, (l2x_entry_t *)entry, TRUE);
            }
        }
#endif /* BCM_XGS_SWITCH_SUPPORT */
        if (match_static) {
...
_soc_l2_sync_mem_cache(int unit, _bcm_l2_replace_t *rep_st)
{
    ...
    for (idx = 0; idx <= index_max; idx++) {
        ...
        if (!(soc_mem_field32_get(unit, mem, entry, STATIC_BITf) ||
              soc_mem_field32_get(unit, mem, entry, RESERVEDf))) {
            continue;
        }
        match_data = (uint32 *)&rep_st->match_data;
        match_mask = (uint32 *)&rep_st->match_mask;
       ...
       if (rv == BCM_E_NONE) {
           ...
           if (l2_pair->replace_flags & BCM_L2_REPLACE_NO_CALLBACKS) {
               ...
               if (info->flags & BCM_L2_TRUNK_MEMBER) {
                ...
                sal_mutex_take(SOC_CONTROL(unit)->l2x_lrn_shadow_mutex,
                               sal_mutex_FOREVER);
                /* Note we call shadow_delete below with 3rd arg as FALSE
                 * because call to mbcm_l2_addr_delete above has removed
                 * any static entry
                 */
                soc_th3_lrn_shadow_delete(unit, &l2x_entry, FALSE);
                sal_mutex_give(SOC_CONTROL(unit)->l2x_lrn_shadow_mutex);
           } else {
               if (soc_th3_l2x_appl_callback_ready(unit)) {
                   /* Delete entry from application callback suppress db,
                    * so that a callback will be issued by the sync thread
                    * thread
                    */
           ...
           if (soc_feature(unit, soc_feature_l2_mac_move_monitoring)) {
               ...
               if ((soc_l2_mac_move_mon_feature_state_get(unit, &state) !=
                    SOC_E_INIT) &&
                   (state & SOC_L2_MAC_MOVE_MON_STATE_INITED)) {
                   (void)soc_l2_mac_move_mon_entry_delete(unit, info->mac,
                                                          info->vid);
               }
           }
       }
...
_bcm_th3_l2_wb_alloc(int unit)
{
    ...
    if (sw_aging_support ||
       ...
       /* Ver 1_3 introduces MAC move monitoring feature.
...
_bcm_th3_l2_wb_sync(int unit)
{
    ...
    if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit)) {
        ...
        /* Ver 1_3: Store state of mac move monitoring(dampening) feature,
...
bcm_tomahawk3_l2_addr_add(int unit, bcm_l2_addr_t *l2addr)
{
    ...
    /* Entry cannot be static and mesh capable both */
