...
_bcm_th3_modules_init(int unit)
{
    ...
    if (SOC_FAST_REBOOT(unit)) {
        ...
        PBMP_PORT_ITER(unit, port_iter) {
            ports[num_ports] = port_iter;
            num_ports += 1;
        }
        BCM_IF_ERROR_RETURN(bcmi_xgs5_port_enable_multi_set(unit, ports,
                            0, num_ports));
    ...
    if (SOC_FAST_REBOOT(unit)) {
        /* Reset the credit release of EP block. */
        SOC_IF_ERROR_RETURN
            (soc_cmicx_dma_ctrl_release_cpu_credits(unit, TRUE));
        ...
        /* Do credit release of EP block. */
        SOC_IF_ERROR_RETURN
            (soc_cmicx_dma_ctrl_release_cpu_credits(unit, FALSE));
    }
    return BCM_E_NONE;
}
...
_bcm_th3_attach(int unit, char *subtype)
{
    ...
    /* Moved counter thread to bcm layer. */
