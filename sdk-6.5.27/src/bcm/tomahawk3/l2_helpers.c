...
bcm_th3_l2_init(int unit)
{
    ...
    /* During warmboot recovery, if the soc property to inhibit callbacks is
     * set, set internal variable states to indicate that the callbacks should
     * not be issued, and the learn, aging threads(if running) should not
     * modify the h/w L2 table. Note that the meaning of wb recovery is in the
     * context of the application - the application will go ahead with
     * registering callback function, enable per-port learning, aging and
     * other function, while the sync thread will start and will be running in
     * the background rebuilding the shadow table. For application, wb recovery
     * is done after all L2 functions are enabled and shadow table rebuild is
     * completed.
     * NOTE: Must be called before the soc_l2x_register below, and when
     * learning, aging, sync threads are turned off, and during wb recovery
     */
    /* Create soc layer mutex before the sync thread is started. This
     * mutex is used in non-wb case as well
     */
    if (soc_l2x_wb_recovery_rebuild_cb_mutex_create(unit) != BCM_E_NONE) {
        return BCM_E_RESOURCE;
    }

    /* Set soc property, spn_L2XMSG_NO_CB_DURING_TABLE_REBUILD value in the
     * soc layer for learning, aging threads to use later
     */
    soc_th3_l2_wb_recovery_rebuild_property_set(unit, wb_recovery_no_cb);

    action = _BCM_L2_WB_RECOVERY_ISSUE_CALLBACKS;

    if (wb_recovery_no_cb) {
        if (SOC_WARM_BOOT(unit)) {
            action = _BCM_L2_WB_RECOVERY_PAUSE_CALLBACKS;
        } else {
            action = _BCM_L2_WB_RECOVERY_ISSUE_CALLBACKS;
        }
    }

    BCM_IF_ERROR_RETURN(_bcm_th3_l2_wb_recovery_pause_resume_cb(unit, action));
...
bcm_th3_l2_addr_delete(int unit, bcm_mac_t mac, bcm_vlan_t vid)
{
    ...
    int static_ent;
    uint32 flags = 0x0;
    int found = FALSE, enable = -1, l2x_sync_del = 0;
    ...
    if (rv >= 0) {
        rv = soc_l2x_sync_delete(unit, (uint32 *) &l2x_lookup, l2_index, 0);
        l2x_sync_del = (rv == SOC_E_NONE)? 1: 0;
    }
    SOC_L2_DEL_SYNC_UNLOCK(soc);
    if (static_ent && (rv >= 0)) {
    ...
    if ((l2x_sync_del) && soc_th3_l2x_appl_callback_ready(unit)) {
        /* Record the learned entry in internal database for use by l2x sync thread;
         * the thread will suppress callback when it sees the entry in the
         * internal database
         */
        if (soc_th3_l2x_appl_callback_disable_config_state_get(unit)) {
            (void)soc_th3_l2x_appl_callback_entry_lookup(unit,
                                                         mac, vid, &found, &enable);
            if (found == FALSE) {
