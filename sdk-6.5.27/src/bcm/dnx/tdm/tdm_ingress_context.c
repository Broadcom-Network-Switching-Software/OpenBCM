    ...
    int *link_mask_id_p)
{
    ...
    SHR_IF_ERR_EXIT(algo_tdm_db.
                    link_mask.exchange(unit, 0, 1, &tdm_link_mask, old_link_mask_id, NULL, &new_link_mask_id,
    ...
    bcm_tdm_interface_config_t * interface_config)
{
    ...
    if (interface_config->stream_id_enable == FALSE)
    {
        ...
        /** verify the provided stream ID can compensate for NIT ETH channel ID */
            ...
            DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0))
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Port %d Stream base ID 0 is not supported, use a bigger base stream ID\n", port);
            }
        }
...
bcm_dnx_tdm_interface_config_set(
    ...
    bcm_tdm_interface_config_t * interface_config)
{
    int nif_interface_id = 0, core_id, sid_stamp_parsed_field_size, stream_id_base;
    uint32 shift_size;
    bcm_port_t port;
    dnx_algo_port_info_s port_info;
    ...
    stream_id_base = interface_config->stream_id_base;

    /**
     * Handle NIF ETH channel provided by NIF to IRE, in some cases the channel can be 1, which requires SW to
     * compensate for it by adjusting the base stream ID accordingly
     */
    if ((interface_config->stream_id_enable == FALSE) && DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0))
    {
        int channel_id;

        SHR_IF_ERR_EXIT(dnx_port_tdm_nif_eth_channel_get(unit, port, &channel_id));
        stream_id_base = stream_id_base - channel_id;
    }
...
/* *INDENT-OFF* */
        SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_TDM_STREAM_CONFIGURATION,
                            ...
                            GEN_DBAL_FIELD32, DBAL_FIELD_MAP_BASE, INST_SINGLE,stream_id_base,
    ...
    bcm_tdm_interface_config_t * interface_config)
{
    ...
    if ((interface_config->stream_id_enable == FALSE) && DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_tdm_nif_eth_channel_get(unit, port, &channel_id));
        interface_config->stream_id_base = interface_config->stream_id_base + channel_id;
    }
...
shr_error_e
dnx_port_tdm_nif_eth_channel_get(
    int unit,
    bcm_port_t port,
    int *channel_id)
{
    ...
    if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_ofr_tdm_is_over_high_get(unit, port, &is_tdm_over_high));
        *channel_id = is_tdm_over_high ? 1 : 0;
    }
    else
    {
        *channel_id = 0;
