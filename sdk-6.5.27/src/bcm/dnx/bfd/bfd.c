...
#define BFD_ACC_FLAGS2_V1 (BCM_BFD_ENDPOINT_FLAGS2_USE_MY_DIP_DESTINATION | BCM_BFD_ENDPOINT_FLAGS2_APPLY_JITTER | BCM_BFD_ENDPOINT_FLAGS2_TX_STATISTICS | BCM_BFD_ENDPOINT_FLAGS2_RX_STATISTICS | \
...
/** SPECIAL_DIAG_27 local_diag or remote can be map to special number */
    ...
    bfd_temp_endpoint_data_t * hw_write_data)
{
    ...
    */
    ...
    /** In case the BFD endpoint is clasified by OAM-LIF and given profile_id is valid, allocate profile in BFD namespace with given ID */
    if ((endpoint_info->profile_id != 0) && (!BFD_MEP_TYPE_CLASSIFIED_BY_DISCR(endpoint_info->type)))
    {
        profile_id = DNX_OAM_PROFILE_IDX_GET(endpoint_info->profile_id);
        alloc_flags = DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID;
    }
    ...
    bfd_temp_endpoint_data_t * hw_write_data)
{
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_REFLECTOR)
        && (!_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_IPV6)))
    {
        SHR_IF_ERR_EXIT(reflector_sw_db_info.sbfd_reflector_discriminator.set(unit, endpoint_info->local_discr));
    }
    else
    {
    ...
    bfd_temp_endpoint_data_t * hw_write_data)
{
    ...
    if (_SHR_IS_FLAG_SET(hw_write_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_IS_ACCELERATED))
    {
        ...
        if (!ENDPOINT_FROM_OAMP(endpoint_info))
        {
            ...
            temp_ep_id &= max_mep_id;
        }

        SHR_IF_ERR_EXIT(dnx_oam_acc_mep_add
                        (unit, _SHR_CORE_ALL, flags, hw_write_data->endpoint_id, 0, temp_ep_id,
                         hw_write_data->gathered_data.action_write_key.mp_profile));
    }
    else
    {
    ...
    bcm_bfd_endpoint_info_t * endpoint_info)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        if (CHECK_FLAGS_INVERTED_MASK(endpoint_info->flags2, BFD_ACC_FLAGS2_V1))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Error: some flags in flags2 are not supported; the value 0x%08X has bits that fall outside "
                         "the supported flags mask, 0x%08X\n", endpoint_info->flags2, BFD_ACC_FLAGS2_V1);
        }
    }
    ...
    if ((endpoint_info->profile_id != 0) && (BFD_MEP_TYPE_CLASSIFIED_BY_DISCR(endpoint_info->type)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error: given profile_id are not supported on this bfd type (%d)",
                     endpoint_info->type);
    }
    ...
    if (ENDPOINT_FROM_OAMP(endpoint_info))
    {
        ...
        if (endpoint_info->bfd_period_cluster > dnx_data_oam.general.bfd_nof_cluster_profile_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error: bfd_period_cluster %d not valid, valid value must be between 1 and %d",
                         endpoint_info->bfd_period_cluster, dnx_data_oam.general.bfd_nof_cluster_profile_get(unit));
        }
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            ...
            if (cluster_profile_exists
                ...
                ((tx_cnt_enable != _SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_TX_STATISTICS))
                 || (rx_cnt_enable != _SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_RX_STATISTICS))))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    const bfd_temp_endpoint_data_t * sw_read_data)
{
    ...
    if (ENDPOINT_CLASSIFIED_BY_DISCR(endpoint) || ENDPOINT_IS_ACC(endpoint))
    {
        ...
        if (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_BFD_ENDPOINT_FLAGS2_SEAMLESS_BFD_REFLECTOR)
            && (!_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_IPV6)))
        {
            SHR_IF_ERR_EXIT(reflector_sw_db_info.sbfd_reflector_discriminator.get(unit, &local_discr));
            endpoint_info->local_discr = local_discr;
        }
        else
        {
            ...
            if (DNX_OAMP_IS_V1(unit))
            {
                                       ...
                                       DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_TX_GPORT_IS_INVALID)))
                {
                    ...
                    SHR_IF_ERR_EXIT(algo_bfd_db.
                                    bfd_oamp_v1_my_discr_range_start.profile_data_get(unit, 0, &dummy, &start));
    ...
    bcm_bfd_endpoint_t endpoint)
{
        ...
        && BFD_MEP_TYPE_CLASSIFIED_BY_DISCR(hw_delete_data->endpoint_type))
    {
        ...
        {
            SHR_IF_ERR_EXIT(reflector_sw_db_info.sbfd_reflector_discriminator.set(unit, 0));
        }
        else
        {
            /**
    ...
    if (_SHR_IS_FLAG_SET(hw_delete_data->bfd_sw_state_flags, DBAL_DEFINE_BFD_ENDPOINT_SW_STATE_FLAGS_BFD_SERVER))
    {
        if (DNX_OAMP_IS_V1(unit))
        {
            SHR_IF_ERR_EXIT(algo_bfd_db.bfd_server_trap_codes.free_single(unit,
                                                                          hw_delete_data->
                                                                          deleted_data.bfd_server_trap_code_index,
                                                                          &hw_delete_data->deleted_data.
                                                                          bfd_server_trap_code_delete));
        }
