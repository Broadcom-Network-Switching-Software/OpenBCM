    ...
    int unit)
{
    ...
    uint32 min_rate, max_rate = 0, max_rate_verify = 0xFFFFFFFF;
    uint32 max_ir_mant, max_ir_exp, max_ir_rev_exp;
    int temp_exp, temp_rev_exp;
    shr_error_e retval;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dbal_tables_field_info_get(unit, DBAL_TABLE_METER_ING_PROFILE_CONFIG, DBAL_FIELD_CIR_REV_EXP,
                                               FALSE, 0, 0, &field_info));
    max_ir_rev_exp = field_info.max_value;
    ...
    /** calc the max rate allowed. the max rate determine by sw limitation: 
       ...
       */
    temp_rev_exp = dnx_data_meter.profile.rev_exp_min_get(unit);
    ...
    {
        temp_exp = 0;
        for (temp_rev_exp = dnx_data_meter.profile.rev_exp_min_get(unit); temp_rev_exp <= max_ir_rev_exp;
             temp_rev_exp++)
        {
            retval = dnx_meter_utils_compute_complex_from_mnt_exp_reverse_exp(unit, max_ir_mant,
                                                                              (uint32) temp_exp,
