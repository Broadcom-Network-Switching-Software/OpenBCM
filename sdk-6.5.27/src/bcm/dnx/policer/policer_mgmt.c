...
/** see .h file */
void
dnx_policer_compensation_default(
    int unit,
    dnx_meter_compensation_profile_info_t * cmp_cfg)
{
    cmp_cfg->header_delta = 0;
}
    ...
    int is_packets_to_kbits)
{
    ...
    if (is_packets_to_kbits)
    {
        ...
        /** covert pol_cfg->ckbits_sec to uint64 var to support big numbers */
        ckbits_sec = pol_cfg->ckbits_sec;
        pol_cfg->ckbits_sec = (uint32) DNX_POLICER_TRANSLATE_PACKETS_TO_KBITS(ckbits_sec);
    ...
    else
    {
        ...
        /** covert pol_cfg->ckbits_sec to uint64 var to support big numbers */
        ckbits_sec = pol_cfg->ckbits_sec;
        pol_cfg->ckbits_sec = (uint32) DNX_POLICER_TRANSLATE_KBITS_TO_PACKETS(ckbits_sec);
    ...
    dnx_meter_profile_info_t * profile_info)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_field_info_get(unit, DBAL_TABLE_METER_PROFILE_SET_FOR_BIG_ENGINE,
                                               DBAL_FIELD_INDEX, TRUE, 0, 0, &field_info));
    ...
    int power_down)
{
   ...
   /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_METER_SETTINGS, &entry_handle_id));
    ...
    int unit)
{
    ...
    if (dnx_data_meter.general.feature_get(unit, dnx_data_meter_general_is_used))
    {
        ...
        if (dnx_data_meter.meter_db.feature_get(unit, dnx_data_meter_meter_db_tcsm_support) == TRUE)
        {
            ...
            for (database_id = 0; database_id < dnx_data_meter.meter_db.nof_ingress_db_get(unit); database_id++)
            {
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_policer_mgmt_tc_metering_enable_set
                                    (unit, BCM_CORE_ALL, database_id, tc_idx, TRUE));
                }
            }
        }
    }
    ...
    {
        if (dnx_data_meter.general.feature_get(unit, dnx_data_meter_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_policer_power_down_set(unit, 1));
        }
    }
...
{
#ifdef DNX_SW_STATE_DIAGNOSTIC
    dnx_meter_compensation_profile_info_t *profile_info = (dnx_meter_compensation_profile_info_t *) data;
#endif
    SW_STATE_PRETTY_PRINT_INIT_VARIABLES();

    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "header_delta", profile_info->header_delta, NULL,
                                   "0x%x");
    ...
    int change_profile)
{
     ...
     */
    SHR_IF_ERR_EXIT(POLICER_MNGR_TM_CB(is_ingress, database_id,
                                       exchange(unit, core, flags, 1, meter_profile_info, old_profile, NULL,
    ...
    bcm_policer_config_t * pol_cfg)
{
    ...
    uint32 is_dual_bucket, is_single_bucket;
    ...
    int i, meter_incr, profile, nof_ref = 0, rv;
    int is_need_to_clear_CBS, is_need_to_clear_EBS, is_CBS_get_extra_credits, is_EBS_get_extra_credits;
    ...
    is_dual_bucket = !is_single_bucket;
    for (meter_incr = 0; meter_incr < nof_meters; ++meter_incr)
    {
        ...
        is_CBS_get_extra_credits = (pol_cfg->mode == bcmPolicerModeCascade
                                    || pol_cfg->mode == bcmPolicerModeCoupledCascade) && pol_cfg->max_ckbits_sec != 0;
        is_need_to_clear_CBS = meter_profile_info[meter_incr].disable_cir == 1 && (!is_CBS_get_extra_credits)
            && pol_cfg->ckbits_burst != 0;
        /** need to clear yellow bucket only if can't receive credits.
         * As credits can added directly (EIR value), or from the green bucket (if CIR and max_pkbits_sec !=0) both are checked*/
        is_EBS_get_extra_credits = pol_cfg->mode != bcmPolicerModeTrTcmDs && pol_cfg->max_pkbits_sec != 0
            && (!is_need_to_clear_CBS);
        is_need_to_clear_EBS = meter_profile_info[meter_incr].disable_eir == 1 && is_dual_bucket
            && (!is_EBS_get_extra_credits) && pol_cfg->pkbits_burst != 0;
        /** in case of disable CIR/EIR, clear bucket from credits */
        if (is_need_to_clear_CBS || is_need_to_clear_EBS)
        {
            /** clear credits in bucket by setting the burst size to 0 and then to required size*/
            cbs = meter_profile_info[meter_incr].cbs;
            ebs = meter_profile_info[meter_incr].ebs;
            meter_profile_info[meter_incr].cbs = is_need_to_clear_CBS ? 0 : cbs;
            meter_profile_info[meter_incr].ebs = is_need_to_clear_EBS ? 0 : ebs;

            SHR_IF_ERR_EXIT(dnx_policer_mgmt_profile_alloc
                            (unit, core_id, is_ingress, database_id, meter_idx + meter_incr,
                             &meter_profile_info[meter_incr], meter_profile_info[meter_incr].entropy_id,
                             change_profile));

            meter_profile_info[meter_incr].cbs = cbs;
            meter_profile_info[meter_incr].ebs = ebs;

            /** poll credits amount to validate update, time out of 1 ms with at least 1 poll. expected empty bucket */
            if (is_need_to_clear_CBS)
            {
                rv = dnx_meter_generic_buckets_empty_poll(unit, core_id, (1 * 1000), 1, is_ingress, is_dual_bucket, 0,
                                                          database_id, meter_idx + meter_incr);
                if (rv != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Clearing Green bucket failed. CIR=%d, max_ckbits_sec=%d\n",
                                 pol_cfg->ckbits_sec, pol_cfg->max_ckbits_sec);
                }
            }
            if (is_need_to_clear_EBS)
            {
                rv = dnx_meter_generic_buckets_empty_poll(unit, core_id, (1 * 1000), 1, is_ingress, is_dual_bucket, 1,
                                                          database_id, meter_idx + meter_incr);
                if (rv != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Clearing Yellow bucket failed. CIR=%d, EIR=%d, max_pkbits_sec=%d\n",
                                 pol_cfg->ckbits_sec, pol_cfg->pkbits_sec, pol_cfg->max_pkbits_sec);
                }
            }
        }
    ...
    bcm_policer_config_t * pol_cfg)
{
    ...
    if (is_global)
    {
        SHR_IF_ERR_EXIT(dnx_policer_to_meter_inter_info(unit, pol_cfg, &meter_profile_info));
            /** clear credits in bucket by setting the burst size to 0 and then to required size*/
        if (meter_profile_info.disable_cir == 1)
        {
            cbs = meter_profile_info.cbs;
            meter_profile_info.cbs = 0;
            SHR_IF_ERR_EXIT(dnx_meter_global_profile_set(unit, core_id, meter_idx, &meter_profile_info));
            meter_profile_info.cbs = cbs;
        }
        SHR_IF_ERR_EXIT(dnx_meter_global_profile_set(unit, core_id, meter_idx, &meter_profile_info));
    }
    else
    ...
    bcm_policer_database_config_t * config)
{
    ...
    bcm_policer_config_t pol_cfg;
    dnx_meter_profile_info_t meter_profile_info;
    dnx_meter_compensation_profile_info_t compensation_profile_info;
    ...
    if (config->expansion_enable == TRUE && config->expand_per_tc == FALSE)
    {
    ...
    SHR_IF_ERR_EXIT(dnx_policer_to_meter_inter_info(unit, &pol_cfg, &meter_profile_info));
    dnx_policer_compensation_default(unit, &compensation_profile_info);
    ...
    bcm_policer_database_attach_config_t * config)
{
    ...
    /** engine 0 and engine 1 are small engines and cannot be concatanated to other engines and must be used as single bucket meter in some devices*/
    is_small_engine = DNX_CRPS_METER_SHMEM_IS_METER_SMALL_ENGINE(unit, engine->engine_id);
    ...
    DNXCMN_CORES_ITER(unit, database_core, core_idx)
    {
        ...
        if (dnx_data_meter.mem_mgmt.feature_get(unit, dnx_data_meter_mem_mgmt_is_mid_engine_store_profiles)
            && is_small_engine == FALSE)
        {
            mid_engine_id =
                dnx_data_meter.mem_mgmt.base_mid_engine_for_meter_get(unit) + (engine->engine_id -
                                                                               dnx_data_meter.
                                                                               mem_mgmt.base_big_engine_for_meter_get
                                                                               (unit));
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_meter_engine_attach_detach
                            (unit, core_idx, mid_engine_id, (dnx_crps_mgmt_shmem_section_t) bcmPolicerEngineSectionAll,
                             is_ingress, TRUE));
        }

        SHR_IF_ERR_EXIT(dnx_meter_db.generic_meter_info.db_info.use_small_engine.set
                        (unit, core_idx, is_ingress, database_id, is_small_engine));
        /** for genenric banks,  update the new banks added to the database and the nof_banks */
        if (is_small_engine == FALSE)
        {
    ...
    bcm_policer_engine_t * engine)
{
    ...
    int database_id;
    int core_idx;
    int bank_id_to_remove, bank_idx_to_remove, bank_id, bank_idx, base_meter_index, bank_id_to_be_set;
        ...
        if (is_small_engine == FALSE)
        {
            ...
            for (entry_ptr_map_table = 0; entry_ptr_map_table <= index_field_info.max_value; entry_ptr_map_table++)
            {
                ...
                if (bank_id_to_remove == hw_bank_id)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_meter_generic_ptr_map_hw_set
                                    (unit, core_idx, is_ingress, database_id,
                                     (entry_ptr_map_table * dnx_data_meter.mem_mgmt.ptr_map_table_resolution_get(unit)),
                                     bank_id_to_be_set));
                    break;
                }
            }
            ...
            && is_small_engine == FALSE)
        {
            mid_engine_id =
                dnx_data_meter.mem_mgmt.base_mid_engine_for_meter_get(unit) + (engine->engine_id -
                                                                               ...
                                                                               (unit));
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_meter_engine_attach_detach
                            (unit, engine->core_id, mid_engine_id,
                             (dnx_crps_mgmt_shmem_section_t) bcmPolicerEngineSectionAll, is_ingress, FALSE));
        }

        /** clear the entire memory space */
        SHR_IF_ERR_EXIT(dnx_meter_generic_engine_clear
                        (unit, core_idx, engine, is_ingress, database_id, is_small_engine));
    }
    ...
    bcm_policer_database_attach_config_t * config)
{
    ...
    if (DNX_CRPS_METER_SHMEM_IS_METER_SMALL_ENGINE(unit, engine->engine_id) == TRUE)
    {
    ...
    bcm_policer_engine_section_t section)
{
    ...
    {
        if (section != bcmPolicerEngineSectionAll)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "engine.section=%d cannot be used for this device. Only bcmPolicerEngineSectionAll supported\n",
                         section);
        }
    }
    else
    ...
    bcm_policer_primary_color_resolution_config_t * config)
{
    ...
    is_support_counters =
        key->is_ingress ? TRUE : dnx_data_meter.general.feature_get(unit,
    ...
    bcm_policer_primary_color_resolution_config_t * config)
{
    ...
    {
        is_support_counters =
            dnx_data_meter.general.feature_get(unit, dnx_data_meter_general_egr_rslv_color_counter_support);
        /** Taking a handle */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_METER_EGR_RESOLVE_COLOR_TABLE, &entry_handle_id));
    }
...
dnx_policer_mgmt_ingress_port_compensation_delta_and_port_verify(
    ...
    int delta)
{
    const dnx_data_meter_compensation_per_ingress_pp_port_delta_range_t *delta_boundries =
        dnx_data_meter.compensation.per_ingress_pp_port_delta_range_get(unit);
...
/** return number of bit required for delta field in port compensation*/
static inline int
dnx_policer_mgmt_get_nof_delta_field(
    int unit)
{
    /** return msb on max value + 2 because delta have negative value */
    return utilex_log2_round_down(dnx_data_meter.compensation.per_ingress_pp_port_delta_range_get(unit)->max) + 2;
}
    ...
    int delta)
{
    ...
    fld_delta = UTILEX_SIGNED_NUM_TO_TWO_COMPLEMENT_METHOD(delta, dnx_policer_mgmt_get_nof_delta_field(unit));
    ...
    int *delta)
{
    ...
    *delta = UTILEX_TWO_COMPLEMENT_INTO_SIGNED_NUM(fld_delta, dnx_policer_mgmt_get_nof_delta_field(unit));
    ...
    void *user_data)
{
    ...
    int is_enabled, meter_idx, is_small_engine;
    const dnx_data_crps_engine_engines_info_t *engine_info;
        ...
        for (is_ingress_idx = FALSE; is_ingress_idx <= TRUE; is_ingress_idx++)
        {
            ...
            for (database_idx = 0; database_idx < nof_databases; database_idx++)
            {
                ...
                if (is_created == TRUE)
                {
                     ...
                     *  bank */
                    SHR_IF_ERR_EXIT(dnx_meter_db.generic_meter_info.db_info.use_small_engine.get
                                    (unit, core_idx, is_ingress_idx, database_idx, &is_small_engine));
                    if (is_small_engine == FALSE)
                    {
