    ...
    bcm_policer_database_config_t * config)
{
    ...
    if (dnx_data_meter.expansion.feature_get(unit, dnx_data_meter_expansion_enhanced_expansion_mechanisim) == TRUE)
    {
        /** expand_per_tc not supported for this device*/
        if (config->expand_per_tc == TRUE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "expand_per_tc not supported for this device\n");
        }
    }

    nof_db =
        is_ingress ? dnx_data_meter.meter_db.nof_ingress_db_get(unit) : dnx_data_meter.meter_db.nof_egress_db_get(unit);
    SHR_RANGE_VERIFY(config->command_id, DNX_POLICER_DEFAULT_CMD_ID, (nof_db - 1), _SHR_E_PARAM,
                     "Command_id max value is %d (command_id=%d).\n", (nof_db - 1), config->command_id);

    /** verify set_region_config: */
    for (i = 0; i < BCM_STAT_MAX_NUMBER_OF_COUNTER_SETS_PER_DB; i++)
    {
        SHR_RANGE_VERIFY((config->exp_set_region_config[i].set_size), 0,
                         (dnx_data_meter.expansion.expansion_size_get(unit) - 1), _SHR_E_PARAM,
                         "set_size max value is %d (set_size[%d]=%d).\n",
                         (dnx_data_meter.expansion.expansion_size_get(unit) - 1), i,
                         config->exp_set_region_config[i].set_size);
    }
...
bcm_dnx_policer_database_destroy_verify(
    ...
    int policer_database_handle)
{
    int core_id, is_ingress, database_id, use_small_engine;
    uint32 nof_banks;
    SHR_FUNC_INIT_VARS(unit);
    ...
    /*
     * All engines should be detached, nof_meters should equal 0 and use_small_engine should equal 0
     */
    ...
    if (nof_banks != 0 || use_small_engine != FALSE)
    {
    ...
    uint32 arg)
{
    ...
    switch (type)
    {
        ...
        case bcmPolicerControlTypeDpResolutionByDpCommand:
            if (dnx_data_meter.meter_db.feature_get(unit, dnx_data_meter_meter_db_resolve_table_mode_per_database) ==
                TRUE)
            {
    ...
    bcm_policer_database_attach_config_t * config)
{
    ...
    /** check that section=ALL if double mode or if device contains 1 bank in engine */
    DNXCMN_CORES_ITER(unit, _SHR_POLICER_DATABASE_HANDLE_CORE_ID_GET(config->policer_database_handle), core_idx)
    {
        if (DNX_CRPS_METER_SHMEM_IS_METER_SMALL_ENGINE(unit, engine->engine_id) == FALSE)
        {
    ...
    is_small_enigne = DNX_CRPS_METER_SHMEM_IS_METER_SMALL_ENGINE(unit, engine->engine_id);
    if (is_small_enigne == TRUE && config->object_stat_pointer_base != 0)
    {
    ...
    if (dnx_data_meter.mem_mgmt.feature_get(unit, dnx_data_meter_mem_mgmt_is_small_engine_support_dual_bucket) == FALSE)
    {
        DNXCMN_CORES_ITER(unit, _SHR_POLICER_DATABASE_HANDLE_CORE_ID_GET(config->policer_database_handle), core_idx)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_meter_db.generic_meter_info.db_info.is_single_bucket.get
                            (unit, core_idx,
                             ...
                             &is_single_bucket));
            if ((is_small_enigne == TRUE) && (is_single_bucket == FALSE))
            {
    ...
    if ((is_small_enigne == TRUE) && _SHR_POLICER_DATABASE_HANDLE_IS_INGRESS_GET(config->policer_database_handle) == 1)
    {
    ...
    DNXCMN_CORES_ITER(unit, engine->core_id, core_idx)
    {
        ...
        if (dnx_data_meter.mem_mgmt.feature_get(unit, dnx_data_meter_mem_mgmt_is_mid_engine_store_profiles)
            && !is_small_enigne)
        {
            mid_engine_id = dnx_data_meter.mem_mgmt.base_mid_engine_for_meter_get(unit) +
                (engine->engine_id - dnx_data_meter.mem_mgmt.base_big_engine_for_meter_get(unit));
            SHR_IF_ERR_EXIT(dnx_crps_meter_engine_available_verify
                            (unit, core_idx, mid_engine_id,
                             (dnx_crps_mgmt_shmem_section_t) bcmPolicerEngineSectionAll));
        }

        
        if (dnx_data_meter.mem_mgmt.feature_get(unit, dnx_data_meter_mem_mgmt_ptr_map_table_has_invalid_mapping_option))
        {
            /** verify that the engine range is not overlapping with another engine range */
            if (is_small_enigne == FALSE)
            {
...
/**
* \brief
*   verify the API bcm_dnx_policer_expansion_select_set
* \param [in] unit - unit id
* \param [in] flags - none
* \param [in] policer_database_handle - macro which holds the database id parameters
* \param [in] expantion_select  - expantion_select
* \return
*   shr_error_e - Error Type
* \remark
*   * None
...
*/
static shr_error_e
dnx_policer_expansion_select_set_verify(
    int unit,
    int flags,
    int policer_database_handle,
    bcm_policer_expansion_select_t * expantion_select)
{
    int is_global, element_index;
    SHR_FUNC_INIT_VARS(unit);

    if (dnx_data_meter.expansion.feature_get(unit, dnx_data_meter_expansion_enhanced_expansion_mechanisim) == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Unavailable for this device\n");
    }

    SHR_NULL_CHECK(expantion_select, _SHR_E_PARAM, "config");
    ...
    SHR_MAX_VERIFY((expantion_select->nof_elements), (bcmPolicerExpansionTypeCount - 1), _SHR_E_PARAM,
                   "nof_elements max value is %d (nof_elements=%d).\n", (bcmPolicerExpansionTypeCount - 1),
                   expantion_select->nof_elements);

    for (element_index = 0; element_index < bcmPolicerExpansionTypeCount; element_index++)
    {
        SHR_RANGE_VERIFY((expantion_select->expanion_elements[element_index].type), 0,
                         (bcmPolicerExpansionTypeCount - 1), _SHR_E_PARAM,
                         " expanion_elements[].type max value is %d (expanion_elements[%d].type=%d).\n",
                         (bcmPolicerExpansionTypeCount - 1), element_index,
                         expantion_select->expanion_elements[element_index].type);
    }
...
/* Set a policer expansion select entry. */
int
bcm_dnx_policer_expansion_select_set(
    int unit,
    int flags,
    int policer_database_handle,
    bcm_policer_expansion_select_t * expantion_select)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_policer_expansion_select_set_verify
                           (unit, flags, policer_database_handle, expantion_select));
...
/**
* \brief
*   verify the API bcm_dnx_policer_expansion_select_get
* \param [in] unit - unit id
* \param [in] flags - none
* \param [in] policer_database_handle - macro which holds the database id parameters
* \param [in] expantion_select  - expantion_select
...
*/
static shr_error_e
dnx_policer_expansion_select_get_verify(
    int unit,
    int flags,
    int policer_database_handle,
    bcm_policer_expansion_select_t * expantion_select)
...
/* Get a policer expansion select entry. */
int
bcm_dnx_policer_expansion_select_get(
    int unit,
    int flags,
    int policer_database_handle,
    bcm_policer_expansion_select_t * expantion_select)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_policer_expansion_select_get_verify
                           (unit, flags, policer_database_handle, expantion_select));
...
/**
* \brief
*   verify the API bcm_dnx_policer_expansion_mapping_set
* \param [in] unit - unit id
* \param [in] flags - none
* \param [in] policer_database_handle - macro which holds the database id parameters
* \param [in] meter_set_map  - meter_set_map
* \return
*   shr_error_e - Error Type
* \remark
*   * None
...
*/
static shr_error_e
dnx_policer_expansion_mapping_set_verify(
    int unit,
    uint32 flags,
    int policer_database_handle,
    bcm_policer_set_map_t * meter_set_map)
{
    int is_global, entry_index, key_index, nof_bits;
    bcm_policer_expansion_data_mapping_t entry;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_NULL_CHECK(meter_set_map, _SHR_E_PARAM, "config");
    ...
    SHR_RANGE_VERIFY((meter_set_map->nof_entries), 0, (BCM_POLICER_EXPANSION_DATA_MAPPING_NOF_ENTRIES), _SHR_E_PARAM,
                     "nof_elements max value is %d (nof_elements=%d).\n",
                     (BCM_POLICER_EXPANSION_DATA_MAPPING_NOF_ENTRIES), meter_set_map->nof_entries);

    for (entry_index = 0; entry_index < meter_set_map->nof_entries; entry_index++)
    {
        entry = meter_set_map->expansion_data_mapping[entry_index];

        SHR_RANGE_VERIFY((entry.nof_key_conditions), 0, (bcmPolicerExpansionTypeCount - 1), _SHR_E_PARAM,
                         "nof_key_conditions max value is %d (meter_set_map[%d].nof_key_conditions=%d).\n",
                         (bcmPolicerExpansionTypeCount - 1), entry_index, (entry.nof_key_conditions));

        for (key_index = 0; key_index < bcmPolicerExpansionTypeCount; key_index++)
        {
            SHR_RANGE_VERIFY((entry.key[key_index].expantion_type), 0, (bcmPolicerExpansionTypeCount - 1), _SHR_E_PARAM,
                             " expanion_elements[].type max value is %d (expanion_elements[%d].type=%d).\n",
                             (bcmPolicerExpansionTypeCount - 1), key_index, (entry.key[key_index].expantion_type));

            nof_bits =
                dnx_data_meter.expansion.source_metadata_types_get(unit, entry.key[key_index].expantion_type)->nof_bits;

            SHR_MAX_VERIFY((utilex_log2_round_down(entry.key[key_index].value)), (nof_bits - 1), _SHR_E_PARAM,
                           "expanion_elements[].value must use %d bits (expanion_elements[%d].value=%d).\n", nof_bits,
                           key_index, (entry.key[key_index].value));
        }

        SHR_RANGE_VERIFY((entry.value.set_region_id), 0, (BCM_STAT_MAX_NUMBER_OF_COUNTER_SETS_PER_DB - 1), _SHR_E_PARAM,
                         "meter_set_map[].value.set_region_id max value is %d (expanion_elements[%d].value.set_region_id=%d).\n",
                         (BCM_STAT_MAX_NUMBER_OF_COUNTER_SETS_PER_DB - 1), entry_index, (entry.value.set_region_id));
        SHR_RANGE_VERIFY((entry.value.offset), 0, (dnx_data_meter.expansion.expansion_size_get(unit) - 1), _SHR_E_PARAM,
                         "meter_set_map[].value.offset max value is %d (expanion_elements[%d].value.offset=%d).\n",
                         (dnx_data_meter.expansion.expansion_size_get(unit) - 1), entry_index, (entry.value.offset));
        SHR_BOOL_VERIFY((entry.value.valid), _SHR_E_PARAM, "meter_set_map[].value.valid  must be TRUE or FALSE");
    }
...
int
bcm_dnx_policer_expansion_mapping_set(
    int unit,
    uint32 flags,
    int policer_database_handle,
    bcm_policer_set_map_t * meter_set_map)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_policer_expansion_mapping_set_verify
                           (unit, flags, policer_database_handle, meter_set_map));
...
/**
* \brief
*   verify the API bcm_dnx_policer_expansion_mapping_get
* \param [in] unit - unit id
* \param [in] flags - none
* \param [in] policer_database_handle - macro which holds the database id parameters
* \param [in] meter_set_map  - meter_set_map
...
*/
static shr_error_e
dnx_policer_expansion_mapping_get_verify(
    int unit,
    uint32 flags,
    int policer_database_handle,
    bcm_policer_set_map_t * meter_set_map)
...
/* Get a policer expansion mapping entry. */
int
bcm_dnx_policer_expansion_mapping_get(
    int unit,
    uint32 flags,
    int policer_database_handle,
    bcm_policer_set_map_t * meter_set_map)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_policer_expansion_mapping_get_verify
                           (unit, flags, policer_database_handle, meter_set_map));
