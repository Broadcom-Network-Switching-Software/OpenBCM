    ...
    uint32 *range_expansion)
{
    ...
    if ((nof_entries == NULL) && (estimated_capacity == NULL) && (range_expansion == NULL))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Neither nof_entries nor estimated_capacity nor range_expansion is requested for %d.\n", db_id);
    }
    ...
    if (DNX_KBP_STATUS_IS_LOCKED(kbp_mngr_status))
    {
        ...
        if (range_expansion)
        {
            *range_expansion = db_stats.range_expansion;
        }
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Cannot get capacity for %d, because device isn't locked.\n", db_id);
    }
    ...
    kbp_opcode_lookup_info_t * lookup_info)
{
    ...
    for (index = 0; index < DNX_KBP_NOF_DBS_PER_CORE; index++)
    {
        ...
        uint32 app_db_id = 0;
        SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, lookup_info->dbal_table_id, &app_db_id, NULL));
        db_id[index] = app_db_id;
        /*
         * For SBC DBAL tables, each KBP core performs lookup to the same KBP DB.
         * For DPC DBAL tables, each KBP core performs lookup to different KBP DBs.
         *     The KBP DBs per core are created with sequential IDs by KBP manager in the manner db_id + core_id.
         *     In the DBAL table, app_db_id is set to db_id for core 0.
         */
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_lookup_set
                    (unit, opcode_id, lookup_id, DBAL_ENUM_FVAL_KBP_LOOKUP_TYPE_ACL, db_id,
    ...
    uint8 *new_opcode_id)
{
    ...
    /** Verify the new context and new opcode are identical, as we assume that later on. */
    if ((*new_opcode_id) != (*acl_ctx))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "new opcode %d, new context %d, expected to be equal.\n",
                     *new_opcode_id, *acl_ctx);
    }
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "dynamic opcode created %s (%d) acl context %d cascaded from %s (%d)\n"), opcode_name,
                 (*new_opcode_id), (*acl_ctx), dbal_enum_id_type_to_string(unit,
                                                                           DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES,
    ...
    for (lookup_index = 0; lookup_index < DNX_KBP_MAX_NOF_LOOKUPS; lookup_index++)
    {
        ...
        if (utilex_bitstream_test_bit(&hit_indication, hitbit_index))
        {
            ...
            SHR_IF_ERR_EXIT(dbal_table_printable_entry_get
                            (unit, dbal_table_id, NULL, DBAL_RESULT_TYPE_NOT_INITIALIZED, DBAL_CORE_ALL, result_size,
                             res_buffer, &current_kbp_entry_print_info->entry_print_info));
    ...
    kbp_printable_entry_t * kbp_entry_print_info)
{
    ...
    for (lookup_id = 0; lookup_id < DNX_KBP_MAX_NOF_LOOKUPS; lookup_id++)
    {
        if (utilex_bitstream_test_bit(&lookup_bitmap, lookup_id))
        {
            ...
            /** Skip the parsing for the KBP Dummy DB */
            if (lkp_db_id == DNX_KBP_DUMMY_HOLE_DB_ID)
            {
                continue;
            }
           /** Get db Info*/
...
shr_error_e kbp_mngr_db_capacity_get_internal(int unit, uint8 db_id, uint32 * nof_entries, uint32 * estimated_capacity, uint32 *range_expansion){DNX_KBP_DUMMY_PROTOTYPE_ERR_MSG("kbp_mngr_db_capacity_get_internal")}
