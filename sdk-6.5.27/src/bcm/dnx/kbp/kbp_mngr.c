    ...
    int unit)
{
    ...
    /** API empty need to perform de-init  */
    if (g_kbp_compiled)
    {
        SHR_EXIT();
    }
    ...
    fwd_acl_ctx_mapping[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE])
{
    ...
    sal_memset(fwd_acl_ctx_mapping, 0,
               (sizeof(fwd_acl_ctx_mapping[0]) * DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE));
    ...
    if (nof_ctxs > DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "number of contexts (%d) for opcode %d is above maximum (%d).\n",
                     nof_ctxs, opcode_id, DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE);
    }
    ...
    for (ctx_index = 0; ctx_index < nof_ctxs; ctx_index++)
    {
        ...
        /*
         * For dynamic opcode, the acl_context equals the opcode.
         * For static opcode, the acl_contexl equals the fwd_context.
         */
    ...
    uint32 *nof_ffcs)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_is_dynamic(unit, opcode_id, &is_dynamic, &dynamic_rel_position));
    if (is_dynamic)
    {
    ...
    if (is_standard_image)
    {
        ...
        if (opcode_id_enum == DBAL_ENUM_FVAL_FORWARD_APP_TYPES_NOP
            || opcode_id_enum == DBAL_ENUM_FVAL_FORWARD_APP_TYPES_MPLS)
        {
            (*nof_ffcs) = 16;
        }
        else if ((opcode_id_enum == DBAL_ENUM_FVAL_FORWARD_APP_TYPES_BRIDGE_SVL
                  || opcode_id_enum == DBAL_ENUM_FVAL_FORWARD_APP_TYPES_ETHERNET_P2P)
                 || (opcode_id_enum == DBAL_ENUM_FVAL_FORWARD_APP_TYPES_SRV6_ENDPOINT_CLASSIC)
                 || (opcode_id_enum == DBAL_ENUM_FVAL_FORWARD_APP_TYPES_SRV6_ENDPOINT_USID))
        {
            (*nof_ffcs) = 12;
        }
        else
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Function not supported for not standard image. \n");
    }
    ...
    int algo_mode)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_db_create_internal
                    (unit, dbal_table_id, 0, acl_db_as_lpm, counters_enable, initial_capacity, algo_mode, NULL));
    ...
    uint32 *range_expansion)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_db_capacity_get_internal(unit, db_id, nof_entries, estimated_capacity, range_expansion));
    ...
    uint8 *dynamic_rel_position)
{
    ...
    {
        (*is_dynamic) = TRUE;
        (*dynamic_rel_position) = opcode_id - DNX_KBP_DYNAMIC_OPCODE_BASE;
    }
    else
    {
        (*is_dynamic) = FALSE;
...
*/
shr_error_e
kbp_mngr_opcode_dynamic_index_to_opcode(
    int unit,
    uint8 dynamic_opcode_relative_position,
    uint8 *dynamic_opcode_id)
{
    SHR_FUNC_INIT_VARS(unit);

    KBP_COMPILATION_CHECK;

    *dynamic_opcode_id = DNX_KBP_DYNAMIC_OPCODE_BASE + dynamic_opcode_relative_position;

    if (*dynamic_opcode_id >= DNX_KBP_MAX_NOF_OPCODES)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "dynamic_opcode_relative_position %d translates to opcode_id %d, out of range. \n",
                     dynamic_opcode_relative_position, *dynamic_opcode_id);
    }
    ...
    uint32 *db_id)
{
    ...
    int table_is_per_core;
    int core_index;

    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(db_id, _SHR_E_PARAM, "db_id");
    DNXCMN_CORE_VALIDATE(unit, core, TRUE);

    SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, dbal_table_id, &core_mode));
    table_is_per_core = kbp_mngr_use_different_dbs_per_core(unit, core_mode);

    if (core == BCM_CORE_ALL && table_is_per_core)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "table is per core but db_id requested for BCM_CORE_ALL\n");
    }
    if (core != BCM_CORE_ALL && !table_is_per_core)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "table is not per core but db_id requested for core other than BCM_CORE_ALL\n");
    }

    SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, dbal_table_id, &first_db_id, NULL));
    ...
    /*
     * We assume the DB_IDs of the different cores are consecutive.
     */
    (*db_id) = first_db_id + core_index;
...
*/
shr_error_e
kbp_mngr_table_property_xl_db_get(
    int unit,
    dbal_tables_e dbal_table_id,
    int *xl_db)
{
    uint32 db_id, xl_db_get;
    uint32 entry_handle_id;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    KBP_COMPILATION_CHECK;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_KBP_DB_INFO, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DB_ID, db_id);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_XL_DB, INST_SINGLE, &xl_db_get);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    *xl_db = xl_db_get;
...
*/
shr_error_e
kbp_mngr_table_property_xl_db_set(
    int unit,
    dbal_tables_e dbal_table_id,
    int xl_db)
{
...
kbp_mngr_smt_enabled(
    int unit)
{
    return (((Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER] == KBP_DEVICE_OP2)
...
*/
uint8
kbp_mngr_use_different_dbs_per_core(
    int unit,
    dbal_core_mode_e core_mode)
{
    return ((core_mode == DBAL_CORE_MODE_DPC) && (kbp_mngr_smt_enabled(unit) && !DNX_KBP_IS_SECOND_KBP_EXIST &&
                                                  (dnx_data_elk.general.nof_dbs_per_core_get(unit) >
                                                   1)) ? TRUE : FALSE);
}
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_wb_file_open(unit, "kbp", Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER]));
    ...
    DNX_KBP_INST_ITER(unit, kbp_inst)
    {
        ...
        DNX_KBP_TRY(kbp_device_init
                    (Kbp_app_data[unit]->dalloc_p[kbp_inst], Kbp_app_data[unit]->device_type[kbp_inst], flags,
                     (struct kbp_xpt *) Kbp_app_data[unit]->xpt_p[kbp_inst], NULL,
                     &Kbp_app_data[unit]->device_p[kbp_inst]));
    }
    /** Configure kbp to work with two cores */
    if (kbp_mngr_smt_enabled(unit))
    {
        ...
        if (dnx_data_elk.general.device_type_get(unit) == DNX_KBP_DEV_TYPE_BCM52321_BCM5235)
        {
            DNX_KBP_TRY(kbp_device_thread_init(Kbp_app_data[unit]->device_p[DNX_KBP_INST_TAP], 0,
                                               &Kbp_app_data[unit]->smt_p[DNX_KBP_INST_TAP][0]));

            DNX_KBP_TRY(kbp_device_thread_init(Kbp_app_data[unit]->device_p[DNX_KBP_INST_TAP], 1,
                                               &Kbp_app_data[unit]->smt_p[DNX_KBP_INST_TAP][1]));
        }
    }
...
shr_error_e kbp_mngr_db_capacity_get(int unit, uint8 db_id, uint32 * nof_entries, uint32 * estimated_capacity, uint32 * range_expansion){DNX_KBP_DUMMY_PROTOTYPE_ERR_MSG("kbp_mngr_db_capacity_get_internal")}
...
shr_error_e kbp_mngr_table_db_id_get(int unit, dbal_tables_e dbal_table_id, bcm_core_t core, uint32 *db_id){DNX_KBP_DUMMY_PROTOTYPE_ERR_MSG("kbp_mngr_table_db_id_get");}
