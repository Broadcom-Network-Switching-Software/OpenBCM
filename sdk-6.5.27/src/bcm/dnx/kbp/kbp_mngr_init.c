...
#include <src/bcm/dnx/field/map/dbx_pre_compiled_fwd_app_type_auto_generated_xml_parser.h>
        ...
        && SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
    {
        ...
        *large_optimized_result_size = 0;
        for (result_type_index = 0; result_type_index < table->nof_result_types; result_type_index++)
        {
            ...
            if (res_info->result_type_hw_value[0] == DNX_KBP_OPTIMIZED_RESULT_TYPE_HW_VALUE)
            {
            ...
            else
            {
                 ...
                 * Compare to previously found regular results.
                 * All regular results need to have the same size.
                 */
                uint32 tmp_regular_result_size;
                SHR_IF_ERR_EXIT(dnx_kbp_valid_result_width
                                (unit, res_info->entry_payload_size, &tmp_regular_result_size));
                if (*regular_result_size == 0)
                {
                    *regular_result_size = tmp_regular_result_size;
                }
                else if (tmp_regular_result_size != *regular_result_size)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "KBP table %s has regular results with different sizes (%d and %d)\n",
                                 table->table_name, *regular_result_size, tmp_regular_result_size);
                }
            }
    ...
    uint8 is_standard_image)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_value_get
                    (unit, DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES, opcode_id, &opcode_id_enum_value));
    ...
    if (opcode_id != configured_opcode_id)
    {
        ...
        if (lookup_bmp && is_standard_image)
        {
            ...
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         ...
                         dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES,
    ...
    uint32 opcode_id)
{
    ...
    if ((key_size[0] == 0) && (key_size[1] == 0))
    {
        ...
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     ...
                     dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_FWD2_CONTEXT_ID, fwd_ctx_id_enum_value),
                     context_index, dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES,
                                                                opcode_id_enum_value), opcode_id);
    }
    ...
    uint8 key_segment_index[DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP])
{
    ...
    SHR_NULL_CHECK(lookup_db, _SHR_E_INTERNAL, "lookup_db");
    ...
    for (index = 0; index < DNX_KBP_NOF_DBS_PER_CORE; index++)
    {
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_LOOKUP_DB, index, lookup_db[index]);
    }
    ...
    uint8 is_standard_image)
{
    ...
    if (!disable_fwd_db)
    {
        ...
        RHDATA_ITERATOR(iter, opcode_node, "AppDB")
        {
            ...
            /** Same lookup DB for all cores, since DBs are used for FWD */
            for (db_iter = 0; db_iter < DNX_KBP_NOF_DBS_PER_CORE; db_iter++)
            {
                /** Use additional variable for DB ID due to conflicting pointer types */
                uint32 app_db_id = 0;
                SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, dbal_table_id, &app_db_id, NULL));
                db_id[db_iter] = app_db_id;
            }
    ...
    if (is_standard_image)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_value_get
                        (unit, DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES, opcode_id, &opcode_id_enum));
        if (opcode_id_enum == DBAL_ENUM_FVAL_FORWARD_APP_TYPES_IPV4_MULTICAST_PRIVATE_W_UC_RPF)
        {
        ...
        if (opcode_id_enum == DBAL_ENUM_FVAL_FORWARD_APP_TYPES_IPV6_MULTICAST_PRIVATE_W_UC_RPF)
        {
            sal_memset(&fwd_tcam_access_hash_info, 0, sizeof(fwd_tcam_access_hash_info));
            fwd_tcam_access_hash_info.expected_nof_elements =
    ...
    uint8 acl_context)
{
    ...
    if (result_bmp == 0)
    {
        ...
        for (index = 0; index < DNX_KBP_NOF_DBS_PER_CORE; index++)
        {
            dbal_entry_value_field8_set(unit, lkp_entry_handle_id, DBAL_FIELD_LOOKUP_DB, index,
    ...
    uint8 *dummy_lookup_added)
{
    ...
    *dummy_lookup_added = FALSE;
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_is_dynamic(unit, opcode_id, &opcode_is_dynamic, &dynamic_rel_position));
    ...
    for (fwd_ctx_index = 0; fwd_ctx_index < fwd_nof_contexts; fwd_ctx_index++)
    {
        ...
        for (acl_context = 0; acl_context < DNX_KBP_NOF_FWD_CONTEXTS; acl_context++)
        {
            ...
            if (rv != _SHR_E_NOT_FOUND)
            {
                ...
                SHR_IF_ERR_EXIT(kbp_mngr_opcode_is_dynamic
                                (unit, derived_opcode, &opcode_is_dynamic, &dynamic_rel_position));
        ...
        if (!sw_state_is_warm_boot(unit))
        {
            ...
            for (index = 0; index < DNX_KBP_MAX_NOF_LOOKUPS; index++)
            {
                if (utilex_bitstream_test_bit(&lookup_bmp, index))
                {
                    ...
                    /** Use smt_id to get the lookup DB per core */
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                    (unit, lkp_entry_handle_id, DBAL_FIELD_LOOKUP_DB, smt_id, &db_id));
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_init_device_type_get(unit, DNX_KBP_INST_MASTER, &device_type));
    ...
    int algo_mode)
{
    ...
    /** The feature is supported only for TCAM_BY_ID tables on devices with 2 cores and one OP2 connected */
    if (table->core_mode == DBAL_CORE_MODE_DPC)
    {
        if (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_are_two_kbps_connected))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "DPC core mode for KBP tables is only applicable for devices with one KBP");
        }
        if (dnx_data_elk.general.nof_dbs_per_core_get(unit) != 2)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "DPC core mode for KBP tables is only applicable for devices with 2 cores");
        }
        if (table->table_type != DBAL_TABLE_TYPE_TCAM_BY_ID)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "DPC core mode for KBP tables is only applicable for TCAM_BY_ID tables");
        }
    }
    ...
    if (table->table_type != DBAL_TABLE_TYPE_TCAM_BY_ID)
    {
        if (acl_db_as_lpm)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "DBs as LPM are supported only for TCAM_BY_ID table type %s",
        ...
        if (table->table_type != DBAL_TABLE_TYPE_TCAM)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "algo_mode other than -1 only applicable for TCAM tables %d %s",
                             algo_mode, dbal_table_type_to_string(unit, table->table_type));
            }
        }
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DB_IS_DPC, INST_SINGLE,
                                 (table->core_mode == DBAL_CORE_MODE_DPC ? TRUE : FALSE));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DB_CORE_ID, INST_SINGLE, 0);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_ACL_DB_AS_LPM, INST_SINGLE, acl_db_as_lpm);
    ...
    /** DB properties */
    SHR_IF_ERR_EXIT(dnx_kbp_init_device_type_get(unit, DNX_KBP_INST_MASTER, &device_type));
    xl_db = (device_type != KBP_DEVICE_OP2) ? 0 : dnx_data_elk.application.db_property_get(unit, table_id)->xl_db;
    /** Locality is not available for OP */
    locality = (device_type != KBP_DEVICE_OP2) ? 0 : dnx_data_elk.application.db_property_get(unit, table_id)->locality;
    ...
    for (key_index = 0; key_index < table->nof_key_fields; key_index++)
    {
        ...
        /*
         * Skip CORE_ID field when present
         */
    ...
    /*
     * Cloning per core DBs is not supported (and not validated here), because per core DBs are valid only for ACLs
     * and ACLs do not support cloning.
     */
    if (clone_id == 0)
    {
        ...
        /*
         * In case the DBAL table is DPC, create a second DB for the second core.
         */
        if (kbp_mngr_use_different_dbs_per_core(unit, table->core_mode))
        {
            uint8 copy_from_db_id = *db_id;
            uint32 copy_entry_handle_id;
            DBAL_HANDLE_COPY(unit, entry_handle_id, &copy_entry_handle_id);
            (*db_id)++;
            dbal_entry_key_field8_set(unit, copy_entry_handle_id, DBAL_FIELD_DB_ID, *db_id);
            dbal_entry_value_field32_set(unit, copy_entry_handle_id, DBAL_FIELD_DB_CORE_ID, INST_SINGLE, 1);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, copy_entry_handle_id, DBAL_COMMIT));

            /** Copy the key segments */
            for (key_index = 0; key_index < nof_valid_segments; key_index++)
            {
                uint32 field_nof_bytes;
                uint32 segment_type;
                char segment_name[DNX_KBP_KEY_SEGMENT_NAME_SIZE_IN_BYTES] = DNX_KBP_KEY_SEGMENT_NOT_CONFIGURED;
                ...
                SHR_IF_ERR_EXIT(kbp_mngr_db_key_segment_get
                                (unit, copy_from_db_id, key_index, &segment_type, &field_nof_bytes, segment_name));
                SHR_IF_ERR_EXIT(kbp_mngr_db_key_segment_set
                                (unit, *db_id, key_index, &segment_type, &field_nof_bytes, segment_name));
            }
        }
    }
    ...
    uint8 *clone_db_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                    (unit, entry_handle_id, DBAL_FIELD_ACL_DB_AS_LPM, INST_SINGLE, acl_db_as_lpm));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_db_create_internal
                    (unit, dbal_table_id, original_db_id, acl_db_as_lpm[0], counters_enable[0], initial_capacity,
    ...
    uint8 *db_id)
{
    ...
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_NOF_DBS, INST_SINGLE, &nof_dbs);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    nof_dbs++;
    SHR_IF_ERR_EXIT(kbp_mngr_db_info_configure
                    (unit, &nof_dbs, clone_id, table_id, acl_db_as_lpm, counters_enable, initial_capacity, algo_mode));
    ...
    while (!is_end)
    {
        ...
        for (lookup_index = 0; lookup_index < DNX_KBP_MAX_NOF_LOOKUPS; lookup_index++)
        {
            ...
            if (!is_hole_lookup)
            {
                ...
                for (core_id = 0; core_id < DNX_KBP_NOF_DBS_PER_CORE; core_id++)
                {
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                                    (unit, lkp_entry_handle_id, DBAL_FIELD_LOOKUP_DB, core_id, &db_id));

                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit, "Validating DB ID %d on opcode %d lookup index %d core %d - "),
                                 db_id, opcode_id, lookup_index, core_id));
                    res =
                    ...
                    else if (res == _SHR_E_EXISTS)
                    {
                        ...
                        if (is_standard_image)
                        {
                            ...
                            {
                                dbal_entry_value_field32_set(unit, db_entry_handle_id, DBAL_FIELD_RESULT_WIDTH,
                                                             INST_SINGLE, DNX_KBP_RESULT_SIZE_FWD);
                            }
                        }
                    ...
                    else if (res == _SHR_E_NOT_FOUND)
                    {
                        ...
                        if (res == _SHR_E_NOT_FOUND)
                        {
                            ...
                            /** Get the original DB cores info */
                            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_KBP_DB_INFO, db_entry_handle_id));
                            dbal_entry_key_field8_set(unit, db_entry_handle_id, DBAL_FIELD_DB_ID, db_id);
                            SHR_IF_ERR_EXIT(dbal_entry_get(unit, db_entry_handle_id, DBAL_GET_ALL_FIELDS));
                            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                                            (unit, db_entry_handle_id, DBAL_FIELD_DB_IS_DPC, INST_SINGLE, &db_is_dpc));
                            ...
                            {
                                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                                                (unit, db_entry_handle_id, DBAL_FIELD_DB_CORE_ID, INST_SINGLE,
                                                 &core_id));
                                dbal_entry_value_field8_set(unit, lkp_entry_handle_id, DBAL_FIELD_LOOKUP_DB, core_id,
                                                            clone_db_id);
                            }
                            else
                            {
                                for (core_id = 0; core_id < DNX_KBP_NOF_DBS_PER_CORE; core_id++)
                                {
                                    dbal_entry_value_field8_set(unit, lkp_entry_handle_id, DBAL_FIELD_LOOKUP_DB,
                                                                core_id, clone_db_id);
                                }
                            }
                            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, lkp_entry_handle_id, DBAL_COMMIT));
                            LOG_VERBOSE(BSL_LOG_MODULE,
                                        (BSL_META_U(unit, "new clone DB ID %d created\n"), clone_db_id));
                        }
    ...
    int unit)
{
    ...
    while (!is_end)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_VALID, INST_SINGLE, &is_valid));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_DB_IS_DPC, INST_SINGLE, &db_is_dpc));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_DB_CORE_ID, INST_SINGLE, &db_core_id));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_ACL_DB_AS_LPM, INST_SINGLE, &acl_db_as_lpm));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_ENTRY_COUNTERS_EN, INST_SINGLE, &counters_enable));
        ...
        if (clone_db == 0)
        {
            ...
            if (acl_db_as_lpm)
            {
                DNX_KBP_TRY_PRINT(kbp_db_set_property(db_handles.db_p, KBP_PROP_MAP_DB_TO_LPM, 1), "DB %d", db_id);
            }
            ...
            if (result_width)
            {
                db_handles.regular_result_size = result_width;
...
{
    void *catalogue = NULL;
    void *fwd2_app_type_node, *app_type_node;
    char *image_name;
    char *opcodes_path = NULL;
    uint8 is_standard_image = FALSE;
    uint32 opcode_id;
    ...
    /** reading each opcode and updating the info */
    RHDATA_ITERATOR(fwd2_app_type_node, catalogue, "FWD2_APP_TYPES")
    {
        RHDATA_ITERATOR(app_type_node, fwd2_app_type_node, "AppType")
        {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_hw_value_get
                            (unit, DBAL_FIELD_TYPE_DEF_FORWARD_APP_TYPES, opcode_id_enum, &opcode_id));
            ...
            dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_OPCODE_NAME, INST_ALL,
                                            (uint8 *) opcode_name);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

            SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_context_read
                            (unit, app_type_node, opcode_id, TRUE, is_standard_image, FALSE, &nof_contexts));
    ...
    int unit)
{
    ...
    while (table_iter != DBAL_TABLE_EMPTY)
    {
        db_id++;
        SHR_IF_ERR_EXIT(kbp_mngr_db_info_configure
                        (unit, &db_id, 0, table_iter, FALSE, FALSE, 0,
                         DNX_KBP_DB_PROPERTIES_ALGORITHMIC_MODE_DEFAULT_INDICATION));
        SHR_IF_ERR_EXIT(dbal_tables_next_table_get
    ...
    int unit)
{
     ...
     */
    smt_id_end =
        ((!DNX_KBP_OP2_IS_SINGLE_PORT_MODE(unit)
          && (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER] == KBP_DEVICE_OP2)) ? DNX_KBP_MAX_NOF_SMTS : 1);
