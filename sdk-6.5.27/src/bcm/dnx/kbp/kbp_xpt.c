    ...
    int unit)
{
    ...
    if ((!soc_sand_is_emulation_system(unit)) && (!SAL_BOOT_PLISIM))
    {
        ...
        if (dnx_data_elk.application.thread_safety_get(unit)
            && (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER] != KBP_DEVICE_OP))
        {
        ...
        DNX_KBP_INST_ITER(unit, kbp_inst)
        {
            ...
            DNX_KBP_TRY(kbp_pcie_init
                        (Kbp_app_data[unit]->device_type[kbp_inst], flags, device_no,
                         Kbp_app_data[unit]->dalloc_p[kbp_inst], &Kbp_app_data[unit]->device_config[kbp_inst], NULL,
                         &Kbp_app_data[unit]->xpt_p[kbp_inst]));
        }
