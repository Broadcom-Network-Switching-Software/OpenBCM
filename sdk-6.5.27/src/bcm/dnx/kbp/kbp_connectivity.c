...
#include <include/bcm_int/dnx/auto_generated/dnx_port_dispatch.h>
    ...
    uint16_t * value)
{
    ...
    rv = soc_cmicx_miim_operation(user_data->device, 0, DNX_KBP_MDIO_CLAUSE, user_data->kbp_mdio_id,
                                  kbp_reg_addr, value);
    ...
    uint16_t value)
{
    ...
    rv = soc_cmicx_miim_operation(user_data->device, 1, DNX_KBP_MDIO_CLAUSE, user_data->kbp_mdio_id,
                                  kbp_reg_addr, &value);
...
static shr_error_e
dnx_kbp_device_type_is_connected(
    int unit,
    int device_type,
    int *is_connected)
{
    ...
    {
        if (Kbp_app_data[unit]->device_type[kbp_inst] == device_type)
        {
            *is_connected = TRUE;
            SHR_EXIT();
        }
    ...
    dnx_kbp_user_data_t * user_data)
{
    ...
    else
    {
        ...
        {
            kbps_per_unit = DNX_KBP_MAX_NOF_INST;
        }
        else if (DNX_KBP_IS_SECOND_KBP_EXIST)
        {
            kbps_per_unit = DNX_OP2_MAX_NOF_INST;
        }
        else
        {
            kbps_per_unit = 1;
        }
        device_no = kbp_inst + unit * kbps_per_unit;
    }

    user_data->device = device_no;
    ...
    enum kbp_device_type *device_type)
{
    ...
    else if (dnx_data_elk.general.device_type_get(unit) == DNX_KBP_DEV_TYPE_BCM5235)
    {
        *device_type = KBP_DEVICE_TAP;
    }
    else if (dnx_data_elk.general.device_type_get(unit) == DNX_KBP_DEV_TYPE_BCM52321_BCM5235)
    {
        if (kbp_inst == DNX_KBP_INST_TAP)
        {
            *device_type = KBP_DEVICE_TAP;
        }
        else
    ...
    struct kbp_device_config *kbp_config)
{
    ...
    int ilkn_rate, i, elk_kbp_index = 0;
    ...
    {
        if (Kbp_app_data[unit]->device_type[kbp_inst] != KBP_DEVICE_TAP)
        {
            elk_kbp_index = kbp_inst;
            ...
            BCM_PBMP_ITER(port_pbmp, ilkn_port)
            {
                ...
                if (DNX_KBP_IS_SECOND_KBP_EXIST)
                {
                    ...
                    {
                        kbp_port_id = elk_kbp_index * 2;
                        if (dnx_data_elk.connectivity.topology_get(unit)->port_core_mapping[kbp_port_id] ==
                            temp_core_id)
                        {
                            kbp_port_id = (ilkn_id % 2 == 0) ? kbp_port_id : kbp_port_id + 1;
                        }
                        else
                        {
                            continue;
                    ...
                    {
                        if ((elk_kbp_index == 0 && ilkn_id % 2 == 0) || (elk_kbp_index == 1 && ilkn_id % 2 == 1))
                        {
                            kbp_port_id = elk_kbp_index * 2;
                            kbp_port_id =
            ...
            SHR_IF_ERR_EXIT(dnx_kbp_ilkn_interface_param_get
                            (unit, elk_kbp_index, &ilkn_port, &ilkn_num_lanes, &ilkn_rate, &ilkn_metaframe));
            ...
            if (Kbp_app_data[unit]->device_type[kbp_inst] != KBP_DEVICE_OP)
            {
        ...
        if (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_is_egw_support_rr))
        {
            kbp_config->npu_type = 1;
        }

        if (Kbp_app_data[unit]->device_type[kbp_inst] == KBP_DEVICE_TAP)
        {
            kbp_config->npu_type = 0x40000000;
        }
    }
    ...
    bcm_core_t core)
{
    ...
    int ilkn_id;
    uint32 nif_enable = 0, elk_nif_if_type = 0;
    bcm_port_t ilkn_port;
    bcm_pbmp_t port_pbmp;
    ...
    if (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_elk_nif_if_type_sel))
    {
        ...
        BCM_PBMP_ITER(port_pbmp, ilkn_port)
        {
            ...
            elk_nif_if_type = (ilkn_access_info.pm_imb_type == imbDispatchTypeImb_clu) ? 1 : 0;
    ...
    if (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_elk_nif_if_type_sel))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_TYPE_SEL, INST_SINGLE, elk_nif_if_type);
    }
    ...
    bcm_core_t core)
{
    ...
    efficiency_rate = (ilkn_total_burst_rate * (meta_frame_len - 4) * 17) / (meta_frame_len * 20);

    if (DNXC_PORT_PHY_SPEED_IS_PAM4(ilkn_rate) && num_lanes > 8)
    {
        efficiency_rate = (efficiency_rate * 974 * 95) / (1000 * 100);
    }

    spr_dly_mega = core_clk_ticks / efficiency_rate;
    ...
    int unit)
{
    ...
    int kbp_inst;
    int is_tap_exist, max_nof_stat_devices;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_kbp_device_type_is_connected(unit, KBP_DEVICE_TAP, &is_tap_exist));
    max_nof_stat_devices = is_tap_exist ? 1 : 2;

    DNX_KBP_INST_ITER(unit, kbp_inst)
    {
        if (is_tap_exist && Kbp_app_data[unit]->device_type[kbp_inst] != KBP_DEVICE_TAP)
        {
            continue;
        }

        for (port_id = (is_tap_exist ? 0 : kbp_inst); port_id < DNX_KBP_STAT_PORTS_MAX;
             port_id = port_id + max_nof_stat_devices)
        {
            ...
            if (port_speed != DNXC_PORT_INVALID_SPEED)
            {
                ...
                if (is_tap_exist)
                {
                    SHR_IF_ERR_EXIT(utilex_bitstream_set(&port_bmp, (port_id), 1));
                }
                else
                {
    ...
    int kbp_inst)
{
    ...
    DNX_KBP_TRY(kbp_device_interface_init
                (Kbp_app_data[unit]->device_type[kbp_inst], Kbp_app_data[unit]->flags,
    ...
    bcm_core_t core)
{
    ...
    if (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER] == KBP_DEVICE_OP)
    {
    ...
    int unit)
{
    ...
    DNX_KBP_INST_ITER(unit, kbp_inst)
    {
        ...
        if (Kbp_app_data[unit]->device_type[kbp_inst] == KBP_DEVICE_OP2
            && kbp_config->settings.link_training_enable == 0)
        {
            port_bmp = 0;
            ...
            for (int lane_idx = start_lane_idx; lane_idx < end_lane_idx; lane_idx++)
            {
                    ...
                    DNXC_PORT_TX_FIR_INVALID_MAIN_TAP)
                {
                    ...
                    if (lane_offset - kbp_config->port_map[0].start_lane < kbp_config->port_map[0].num_lanes)
                    {
                        port_bmp |= 1 << 0;
                    }
                    else if (lane_offset - kbp_config->port_map[1].start_lane < kbp_config->port_map[1].num_lanes)
                    {
                        port_bmp |= 1 << 2;
                    }
                }
            ...
            if (port_bmp)
            {
                DNX_KBP_TRY(kbp_device_interface_TXPCS_soft_reset_toggle(KBP_DEVICE_OP2, kbp_config, port_bmp));
            }
        }
    }

    SHR_IF_ERR_EXIT(dnx_kbp_device_type_is_connected(unit, KBP_DEVICE_OP, &is_op_exist));
    if (is_op_exist)
    {
    ...
    BCM_PBMP_ITER(elk_pbmp, elk_port)
    {
        ...
        SHR_IF_ERR_EXIT(imb_port_rx_buffers_restart(unit, elk_port));
...
shr_error_e
dnx_kbp_master_op2_instance_get(
    int unit,
    int *master_op2_intance)
{
    int tap_is_exist, op2_is_exist;
    int kbp_inst;
    SHR_FUNC_INIT_VARS(unit);

    *master_op2_intance = 0;

    SHR_IF_ERR_EXIT(dnx_kbp_device_type_is_connected(unit, KBP_DEVICE_TAP, &tap_is_exist));
    SHR_IF_ERR_EXIT(dnx_kbp_device_type_is_connected(unit, KBP_DEVICE_OP2, &op2_is_exist));

    if (!tap_is_exist || !op2_is_exist)
    {
        *master_op2_intance = DNX_KBP_INST_MASTER;
        SHR_EXIT();
    }
...
shr_error_e
dnx_kbp_slave_op2_instance_get(
    int unit,
    int *master_op2_intance)
{
    int tap_is_exist, op2_is_exist;
    int kbp_inst, first_op2_is_found = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    ...
    *master_op2_intance = 0;
    ...
    if (!tap_is_exist || !op2_is_exist)
    {
        *master_op2_intance = DNX_KBP_INST_SLAVE;
        SHR_EXIT();
    }
    ...
    else
    {
        ...
        DNX_KBP_INST_ITER(unit, kbp_inst)
        {
            if (Kbp_app_data[unit]->device_type[kbp_inst] == KBP_DEVICE_OP2 && !first_op2_is_found)
            {
                first_op2_is_found = TRUE;
            }
            else if (Kbp_app_data[unit]->device_type[kbp_inst] == KBP_DEVICE_OP2 && first_op2_is_found)
            {
                *master_op2_intance = kbp_inst;
                SHR_EXIT();
            }
    ...
    int unit)
{
    ...
    DNX_KBP_INST_ITER(unit, kbp_inst)
    {
        ...
        if (!SAL_BOOT_PLISIM)
        {
            DNX_KBP_TRY(kbp_device_interface_reset(Kbp_app_data[unit]->device_type[kbp_inst],
                                                   &Kbp_app_data[unit]->device_config[kbp_inst]));
        }
    }
exit:
