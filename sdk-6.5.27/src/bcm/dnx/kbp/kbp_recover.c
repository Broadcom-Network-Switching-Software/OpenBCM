 ...
 */
int
dnx_kbp_hard_reset_call_register_func(
    int unit)
{
    ...
    {
        LOG_ERROR_EX(BSL_LOG_MODULE, "Unit%d Device Hard Reset CB was not registed! %s%s%s\n",
    ...
    uint32 enable)
{
    ...
    int is_last_port;
    int kbp_inst;
    kbp_status kbp_err_code = KBP_OK;
    SHR_FUNC_INIT_VARS(unit);

    if (Kbp_app_data[unit] == NULL || Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER] != KBP_DEVICE_OP2)
    {
        /*
    ...
    {
        if (Kbp_app_data[unit] == NULL || Kbp_app_data[unit]->device_type[kbp_inst] != KBP_DEVICE_OP2)
        {
            continue;
        }
        /*
        ...
        BCM_PBMP_ITER(port_pbmp, elk_port)
        {
            ...
            if (DNX_KBP_IS_SECOND_KBP_EXIST)
            {
                ...
                {
                    kbp_port_id = (kbp_inst == DNX_KBP_INST_MASTER) ? 0 : 2;
                    if (dnx_data_elk.connectivity.topology_get(unit)->port_core_mapping[kbp_port_id] == core_id)
                    {
        ...
        BCM_PBMP_ITER(port_pbmp_inst, elk_port)
        {
            ...
            SHR_IF_ERR_EXIT(imb_port_enable_set(unit, elk_port, IMB_PORT_ENABLE_F_SKIP_FEC, enable));
            ...
            if (DNXC_PORT_PHY_SPEED_IS_PAM4(ilkn_rate))
            {
                ...
                kbp_err_code = kbp_device_interface_port_toggle(Kbp_app_data[unit]->device_type[kbp_inst],
                                                                &Kbp_app_data[unit]->device_config[kbp_inst],
                                                                Kbp_app_data[unit]->flags,
                                                                (kbp_port_id ? 2 : 0), enable, is_last_port, 0, 1);
                if (kbp_err_code != KBP_OK)
                {
                    LOG_ERROR_EX(BSL_LOG_MODULE, "Error %s\n failed: %s%s%s\n",
                                 shrextend_errmsg_get(dnx_kbp_error_translation(kbp_err_code)),
                                 kbp_get_status_string(kbp_err_code), EMPTY, EMPTY);
                    if (dnx_data_elk.
                        general.feature_get(unit, dnx_data_elk_general_hard_reset_after_kbp_soft_reset_fail))
                    {
                        LOG_WARN_EX(BSL_LOG_MODULE, "%s%s%s%s\n",
                                    "Calling customer registered hard rest callback", EMPTY, EMPTY, EMPTY);
                        SHR_IF_ERR_EXIT(dnx_kbp_hard_reset_call_register_func(unit));
                    }
                }
            }
        }
