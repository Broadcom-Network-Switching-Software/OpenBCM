    ...
    uint8 ivec)
{
    ...
    *tsh_offset = cur_offset_in_bytes;
    ...
    if (fhei_stamp_en || force_fhei_ive_stamp)
    {
    ...
    if (tm->ase_ext.ase_type == bcmPktDnxAseTypeSFlow)
    {
        ...
        DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, internal, fwd_layer_index, 3);
    }
    else if (mirror_header_info->pp.force_mirror)
    {

        DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, internal, parsing_start_type,
                                           (dnx_init_pp_layer_types_value_get(unit, DBAL_ENUM_FVAL_LAYER_TYPES_IPVX)));
    }
    else
    {
        DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, internal, parsing_start_type,
    ...
    if (fhei_stamp_en || force_fhei_ive_stamp)
    {
        ...
        *fhei_offset = cur_offset_in_bytes;
        if (force_fhei_ive_stamp)
        {
            DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, fhei_sz1_stamp_ive, fhei_ivec,
                                               mirror_header_info->pp.ingress_vlan_translate_action_id);
            DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, fhei_sz1_stamp_ive, fhei_ive_type, 0);
        }
    }
    ...
    if (udh_en)
    {
        ...
        DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, udh, data_type0, (udh[0].size ? 1 : 0));
        DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, udh, data_type1, (udh[1].size ? 1 : 0));
        DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, udh, data_type2, (udh[2].size ? 1 : 0));
        DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, udh, data_type3, (udh[3].size ? 1 : 0));
             ...
             ++udh_index)
        {
            ...
            if (udh[udh_index].size != 0)
            {
                ...
                {
                    case 0:
                        udh_size = dnx_data_headers.system_headers.max_udh0_block_size_get(unit);
                        break;
                    case 1:
                        udh_size = dnx_data_headers.system_headers.max_udh1_block_size_get(unit);
                        break;
                    case 2:
                        udh_size = dnx_data_headers.system_headers.max_udh2_block_size_get(unit);
                        break;
                    case 3:
                        udh_size = dnx_data_headers.system_headers.max_udh3_block_size_get(unit);
                        break;
                    default:
                        udh_size = 0;
                        break;
                ...
                if (udh_size == 8)
                {
                    ...
                    SHR_IF_ERR_EXIT(utilex_bitstream_set_field(system_header, offset, 32, udh[udh_index].data_ext));
    ...
    bcm_mirror_header_info_t * mirror_header_info)
{
    ...
    if (system_headers_mode == DNX_MIRROR_SYSTEM_HEADER_DATA_GET(unit, system_headers, system_headers_mode_jericho))
    {
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_mirror_header_system_header_buffer_set
                        (unit, flags, mirror_dest_id, mirror_header_info, total_len_in_bits,
                         ...
                         (flags & BCM_MIRROR_DEST_STAMP_IVE_ACTION_ID) ? 1 : 0,
                         mirror_header_info->pp.ingress_vlan_translate_action_id));
    ...
    if ((fhei_offset > 0) && !(flags & BCM_MIRROR_DEST_STAMP_IVE_ACTION_ID))
    {
...

    {
        ...
        for (admit_profile_index = 0; admit_profile_index < nof_admit_profiles; admit_profile_index++)
        {
                 ...
                 mod_profile_index++)
            {
                ...
                {
                    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_SNIF_MIRROR_ON_DROP_COMMAND, entry_handle_id));
    ...
    bcm_mirror_header_info_t * mirror_header_info)
{
    ...
    if (fhei_stamp_en || (fhei_size == 2))
    {
        ...
        if ((fhei_offset != cur_offset_in_bytes) && fhei_stamp_en)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "FHEI offset mismatch: is %d expected %d.\n", fhei_offset, cur_offset_in_bytes);
        }
    }

    if ((fhei_size == 2) && !fhei_stamp_en)
    {
        DNX_MIRROR_SYSTEM_HEADER_FIELD_GET(unit, fhei_sz1_stamp_ive, fhei_ive_type, val);

        if (val == 0)
        {
            DNX_MIRROR_SYSTEM_HEADER_FIELD_GET(unit, fhei_sz1_stamp_ive, fhei_ivec, val);
            pp->ingress_vlan_translate_action_id = (uint8) val;
        }
    }
    ...
    if (udh_en)
    {
             ...
             ++udh_index)
        {
            ...
            if (udh[udh_index].size != 0)
            {
                ...
                {
                    offset -= 32;
                    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(system_header, offset, 32, &udh[udh_index].data_ext));
                    cur_offset_in_bytes += 4;
                }
