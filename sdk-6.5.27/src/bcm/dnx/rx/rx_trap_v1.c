    ...
    dnx_rx_trap_etpp_action_t * etpp_action_p)
{
    ...
    if (is_user_defined)
    {
        dbal_value_field8_request(unit, entry_handle_id, etpp_profile_dbal_fields.cfg_kill_snoop_copy, INST_SINGLE,
                                  &(etpp_action_p->kill_snoop_copy));
    ...
    dnx_rx_trap_etpp_action_t * etpp_action_p)
{
    ...
    if (is_user_defined)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, etpp_profile_dbal_fields.cfg_kill_snoop_copy, INST_SINGLE,
                                     etpp_action_p->kill_snoop_copy);
    ...
    dnx_rx_trap_etpp_action_t * etpp_action_p)
{
    ...
    /** Set the values */
    if (etpp_profile_dbal_fields.cfg_rcy_prio != DBAL_FIELD_EMPTY)
    {
        dbal_entry_value_field8_set(unit, entry_handle_id, etpp_profile_dbal_fields.cfg_rcy_prio, INST_SINGLE,
                                    etpp_action_p->is_recycle_high_priority);
    }
    ...
    dnx_rx_trap_etpp_action_t * etpp_action_p)
{
    ...
    {
        etpp_action_p->is_recycle_high_priority = 1;
    }

    dbal_value_field8_request(unit, entry_handle_id, etpp_profile_dbal_fields.cfg_rcy_cmd, INST_SINGLE,
                              &etpp_action_p->recycle_cmd);

    if (etpp_profile_dbal_fields.cfg_cpu_trap_code != DBAL_FIELD_EMPTY)
    {
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    if ((etpp_config_p->action.kill_snoop_copy == TRUE) && (trap_class != DNX_RX_TRAP_CLASS_USER_DEFINED))
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "BCM_RX_TRAP_FLAGS2_SNOOP_CODE_CLEAR only supported user-defined ETPP traps\n");
    }
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    etpp_config_p->action.kill_snoop_copy = ((config_p->flags2 & BCM_RX_TRAP_FLAGS2_SNOOP_CODE_CLEAR) != 0);
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    if (etpp_config_p->action.kill_snoop_copy == TRUE)
    {
    ...
    int *ref_count)
{
    ...
    *ref_count = 0;
    ...
    for (trap_iter = 0; trap_iter < bcmRxTrapCount; trap_iter++)
    {
        ...
        if (appl_trap_info.valid && (appl_trap_info.trap_block == DNX_RX_TRAP_BLOCK_ETPP))
        {
            SHR_IF_ERR_EXIT(dnx_rx_trap_v1_etpp_action_profile_hw_get(unit, appl_trap_info.trap_id, &action_profile));
    ...
    dnx_rx_trap_action_profile_t * action_profile_p)
{
     ...
     * Set the values
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ETPP_TRAP_ACT_PROFILE, INST_SINGLE,
    ...
    dnx_rx_trap_action_profile_t * action_profile_p)
{
     ...
     * Set the values
     */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_ETPP_TRAP_ACT_PROFILE, INST_SINGLE,
    ...
    bcm_gport_t action_gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_ingress_trap_profile_allocate_and_hw_update
                    (unit, old_lif_trap_profile, action_gport, &new_lif_trap_profile, &is_last));
