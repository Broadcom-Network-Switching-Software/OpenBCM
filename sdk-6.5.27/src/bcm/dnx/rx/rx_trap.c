    ...
    int *new_recycle_cmd_p)
{
    ...
    SHR_IF_ERR_EXIT(algo_rx_db.trap_recycle_cmd.exchange
                    (unit, 0, 1, recycle_cmd_config_p, old_recycle_cmd, NULL, new_recycle_cmd_p, &is_first, &is_last));
...
/**
* \brief
*  Set user defined trap name in SW State
* \par DIRECT INPUT:
*   \param [in] unit          -  Unit id
*   \param [in] trap_type     -  Trap type
*   \param [in] trap_name     -  Trap name
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e - Error type
...
*/
static shr_error_e
dnx_rx_trap_ingress_ud_name_set(
    int unit,
    int trap_type,
    uint8 *trap_name)
{
    int sw_trap_id;
    rx_trap_user_info_t user_trap_get, user_trap_set;
    SHR_FUNC_INIT_VARS(unit);

    SHR_NULL_CHECK(trap_name, _SHR_E_PARAM, "trap_name");

    if (DNX_RX_TRAP_ID_CLASS_GET(unit, trap_type) == DNX_RX_TRAP_CLASS_USER_DEFINED)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_hw_to_sw(unit, trap_type, &sw_trap_id));
        SHR_IF_ERR_EXIT(rx_trap_info.user_trap_info.get(unit, sw_trap_id, &user_trap_get));

        if ((user_trap_get.name[0] != '\0') && (trap_name[0] != '\0')
            && (sal_strncmp((char *) trap_name, user_trap_get.name, BCM_RX_TRAP_MAX_NAME_LEN) != 0))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Trap name can only be set once!");
        }

        if (user_trap_get.name[0] == '\0')
        {
            if (trap_name[0] == '\0')
            {
                sal_snprintf(user_trap_set.name, BCM_RX_TRAP_MAX_NAME_LEN, "USER_DEFINED_%d", sw_trap_id);
            }
            else
            {
                sal_strncpy_s(user_trap_set.name, (char *) trap_name, sizeof(user_trap_set.name));
            }

            SHR_IF_ERR_EXIT(rx_trap_info.user_trap_info.set(unit, sw_trap_id, &user_trap_set));
        }
    }
...
/**
* \brief
*  Get user defined trap name from SW State
* \par DIRECT INPUT:
*   \param [in] unit          -  Unit id
*   \param [in] trap_type     -  Trap type
*   \param [in] trap_name     -  Trap name
...
*/
shr_error_e
dnx_rx_trap_ingress_ud_name_get(
    int unit,
    int trap_type,
    uint8 *trap_name)
{
    int sw_trap_id;
    rx_trap_user_info_t user_trap_get;
    SHR_FUNC_INIT_VARS(unit);

    SHR_NULL_CHECK(trap_name, _SHR_E_PARAM, "trap_name");

    sal_memset(trap_name, 0, sizeof(user_trap_get.name));

    if (DNX_RX_TRAP_ID_CLASS_GET(unit, trap_type) == DNX_RX_TRAP_CLASS_USER_DEFINED)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_hw_to_sw(unit, trap_type, &sw_trap_id));
        SHR_IF_ERR_EXIT(rx_trap_info.user_trap_info.get(unit, sw_trap_id, &user_trap_get));
        sal_strncpy_s((char *) trap_name, user_trap_get.name, sizeof(user_trap_get.name));
    }
...
/**
* \brief
*  Clear user defined trap name from SW State
* \par DIRECT INPUT:
*   \param [in] unit          -  Unit id
*   \param [in] sw_trap_id    -  SW Trap-ID
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e - Error type
...
*/
static shr_error_e
dnx_rx_trap_ingress_ud_name_clear(
    int unit,
    int sw_trap_id)
{
    rx_trap_user_info_t user_trap_init;
    SHR_FUNC_INIT_VARS(unit);
    sal_memset(&user_trap_init, 0, sizeof(rx_trap_user_info_t));
    SHR_IF_ERR_EXIT(rx_trap_info.user_trap_info.set(unit, sw_trap_id, &user_trap_init));
    ...
    int *trap_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_map_ingress_ud_alloc_get_dispatch(unit, ind, trap_id_p));
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    switch (trap_block)
    {
        case DNX_RX_TRAP_BLOCK_INGRESS:
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                             "Name can't be set for predefined traps, naming only available for user traps\n");
            }
            break;
        }
        case DNX_RX_TRAP_BLOCK_ERPP:
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    {
        /** sbfd reflector mpls-sr case, don't set overwrite bit*/
        if ((config_p->encap_id == 0) && (config_p->encap_id2 != 0))
        {
            if (BCM_GPORT_IS_SET(config_p->encap_id2))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
                                (unit, config_p->encap_id2, hw_resources_flags, &gport_hw_resources));
...
/**
* \brief
* Set configuration of SRv6 USP trap
* \par DIRECT INPUT:
*   \param [in] unit      -  Unit Id
*   \param [in] config_p  -  Structure, holding the information about the snoop and forward strength
* \par INDIRECT INPUT:
...
*/
static shr_error_e
dnx_rx_trap_ingress_srv6_usp_strength_hw_set(
    int unit,
    bcm_rx_trap_config_t * config_p)
{
    uint32 entry_handle_id;
    ...
    if (config_p->snoop_strength != 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Snoop Strength is not supported for SRv6 USP trap");
    }

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_SRV6, &entry_handle_id));

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STRENGTH_FOR_LPM_DESTINATION_FOR_SRV6_USP_TRAP,
                                 INST_SINGLE, config_p->trap_strength);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/**
* \brief
* Clear configuration of SRv6 USP trap
* \par DIRECT INPUT:
*   \param [in] unit      -  Unit Id
* \par INDIRECT INPUT:
*   * None
...
*/
static shr_error_e
dnx_rx_trap_ingress_srv6_usp_strength_hw_clear(
    int unit)
{
    uint32 entry_handle_id;
    SHR_FUNC_INIT_VARS(unit);
...
/**
* \brief
* Get configuration of SRv6 USP trap
* \par DIRECT INPUT:
*   \param [in] unit      -  Unit Id
*   \param [in] config_p  - Structure, holding the information about the snoop and forward strength
* \par INDIRECT INPUT:
...
*/
static shr_error_e
dnx_rx_trap_ingress_srv6_usp_strength_hw_get(
    int unit,
    bcm_rx_trap_config_t * config_p)
{
    uint32 entry_handle_id;
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_STRENGTH_FOR_LPM_DESTINATION_FOR_SRV6_USP_TRAP,
                               INST_SINGLE, (uint32 *) &config_p->trap_strength);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    if (!is_strengthless)
    {
        ...
        switch (trap_type)
        {
            ...
            case DBAL_ENUM_FVAL_INGRESS_TRAP_ID_TRAP_BFD_O_IPV6_UDP_CHECKSUM:
                ...
                break;
            case DBAL_ENUM_FVAL_INGRESS_TRAP_ID_PMF_SRV6_USP:
                SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_srv6_usp_strength_hw_clear(unit));
                break;
    ...
    if (!is_strengthless)
    {
        ...
        switch (trap_type)
        {
            ...
            case DBAL_ENUM_FVAL_INGRESS_TRAP_ID_TRAP_BFD_O_IPV6_UDP_CHECKSUM:
                ...
                break;
            case DBAL_ENUM_FVAL_INGRESS_TRAP_ID_PMF_SRV6_USP:
                SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_srv6_usp_strength_hw_set(unit, config_p));
                break;
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    /** Set user defined trap name */
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_name_set(unit, trap_type, config_p->name));
    ...
    bcm_rx_mtu_profile_value_t * mtu_data_p)
{
    ...
    if ((dnx_data_trap.etpp.feature_get(unit, dnx_data_trap_etpp_mtu_layer_to_compressed_layer_mapping_support) == 0) &&
        mtu_key_p->cmp_layer_type >= bcmRxTrapMtuLayerTypeCount && (flags != BCM_RX_MTU_PORT))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    if (!is_strengthless)
    {
        ...
        switch (trap_type)
        {
            ...
            case DBAL_ENUM_FVAL_INGRESS_TRAP_ID_TRAP_BFD_O_IPV6_UDP_CHECKSUM:
                ...
                break;
            case DBAL_ENUM_FVAL_INGRESS_TRAP_ID_PMF_SRV6_USP:
                SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_srv6_usp_strength_hw_get(unit, config_p));
                break;
            default:
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    /** Get user defined trap name */
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_name_get(unit, trap_type, config_p->name));
    ...
    int trap_type)
{
    ...
    switch (trap_class)
    {
        ...
        case DNX_RX_TRAP_CLASS_USER_DEFINED:
            ...
            SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_name_clear(unit, sw_trap_id));
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal trap_class=%d\n", trap_class);
            break;
    ...
    int *ref_count)
{
    ...
    *ref_count = 0;
    ...
    for (trap_iter = 0; trap_iter < bcmRxTrapCount; trap_iter++)
    {
        ...
        if (appl_trap_info.valid && (appl_trap_info.trap_block == DNX_RX_TRAP_BLOCK_ERPP))
        {
            skip_invalid_value =
...
/**
* \brief
*  Initialize user traps SW State
* \param [in] unit  - Device ID
* \return
*   shr_error_e - Error Type
* \remark
...
*/
static shr_error_e
dnx_rx_trap_ingress_user_traps_init(
    int unit)
{
    int alloc_ind;
    rx_trap_user_info_t user_trap_init;
    SHR_FUNC_INIT_VARS(unit);
    sal_memset(&user_trap_init, 0, sizeof(rx_trap_user_info_t));
    SHR_IF_ERR_EXIT(rx_trap_info.user_trap_info.alloc(unit));

    for (alloc_ind = 0; alloc_ind < dnx_data_trap.ingress.nof_user_defined_traps_get(unit); alloc_ind++)
    {
        SHR_IF_ERR_EXIT(rx_trap_info.user_trap_info.set(unit, alloc_ind, &user_trap_init));
    }
    ...
    int unit)
{
    ...
    /** Initialize rx_trap SW State */
    SHR_IF_ERR_EXIT(rx_trap_info.init(unit));
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_user_traps_init(unit));
    ...
    bcm_rx_trap_protocol_profiles_t * protocol_profiles_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_vsi_profile_get
                    (unit, resources_gport, (uint32 *) &in_lif_profile, &result_type, &is_ip_tunnel_l2vpn, intf_type));
