    ...
    bcm_srv6_sid_initiator_info_t * info)
{
    ...
    if ((info->next_encap_id != 0) && (info->next_encap_id != BCM_IF_INVALID))
    {
        ...
        res =
            dnx_algo_gpm_gport_to_hw_resources(unit, next_pointer, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS,
                                               &gport_hw_resources);
        if (res)
        {
    ...
    bcm_srv6_srh_base_initiator_info_t * info)
{
    ...
    not_supported_flags =
        ...
        BCM_SRV6_SRH_BASE_INITIATOR_VLAN_TRANSLATION;

    if (!dnx_data_headers.
        system_headers.feature_get(unit, dnx_data_headers_system_headers_system_headers_ibch1_supported))
    {
        not_supported_flags |= BCM_SRV6_SRH_BASE_INITIATOR_EXTENDED_ENCAP;
    }
    ...
    uint32 *nwk_qos_profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_global_out_lif_alloc_info_get(unit, _SHR_GPORT_TUNNEL_ID_GET(info->tunnel_id),
                                                           DNX_ALGO_LIF_EGRESS, &alloc_info));
    ...
    bcm_srv6_extension_terminator_mapping_t * info)
{
    ...
    /** 8B Alignment cannot be set with max nof_sids*/
    if ((_SHR_IS_FLAG_SET(info->flags, BCM_SRV6_EXTENSION_TERMINATOR_ADD_8B_EXTENSION)) &&
        (info->nof_sids == max_nof_terminated_usp_sids))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "SRv6 nof_sids:%d is the max nof_sids, and it is not supported wit hthe flag: BCM_SRV6_EXTENSION_TERMINATOR_ADD_8B_EXTENSION",
                     max_nof_terminated_usp_sids);
    }
    ...
    int local_inlif)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SRH_HDR_EXT_LEN, extension_length);
    ...
    bcm_srv6_extension_terminator_mapping_t * info)
{
    ...
    extension_length = SRV6_NOF_SIDS_TO_HEADER_LENGTH(info->nof_sids);
    if (_SHR_IS_FLAG_SET(info->flags, BCM_SRV6_EXTENSION_TERMINATOR_ADD_8B_EXTENSION))
    {
        extension_length += 1;
    }
    ...
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_srv6_extension_terminator_lookup_get(unit, extension_length, &local_inlif_u32));
    ...
    if (!(_SHR_IS_FLAG_SET(info->flags, BCM_SRV6_EXTENSION_TERMINATOR_REPLACE)))
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_srv6_extension_terminator_lookup_add(unit, extension_length, local_inlif));
    }
    ...
    bcm_srv6_extension_terminator_mapping_t * info)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_srv6_extension_terminator_lookup_get(unit, extension_length, &local_inlif));

    /*
     * delete nof_sids to in_lif lookup
     */
    SHR_IF_ERR_EXIT(dnx_srv6_extension_terminator_lookup_delete(unit, extension_length));
    ...
    void *user_data)
{
    ...
    if (dnx_data_flow.srv6.use_flow_lif_term_get(unit))
    {
    }
    /*
    ...
    while (!is_end)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_SRH_HDR_EXT_LEN, extension_length));

        entry_info.flags = 0;
        if (extension_length[0] & 1)
        {
            entry_info.flags = BCM_SRV6_EXTENSION_TERMINATOR_ADD_8B_EXTENSION;
        }
        entry_info.nof_sids = SRV6_HEADER_LENGTH_TO_NOF_SIDS(extension_length[0] & (~0x1));
...
shr_error_e
dnx_switch_srv6_lpm_destination_for_srv6_trap_set(
    int unit,
    int value)
{
    uint32 entry_handle_id = 0, kaps_dest = 0;
    uint8 enable_trap = 1;
    int destination_max_bit = 21;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    else if (BCM_GPORT_IS_FORWARD_PORT(value))
    {
        uint32 fec_val = BCM_GPORT_FORWARD_PORT_GET(value);
        SHR_IF_ERR_EXIT(dbal_fields_field_type_parent_field32_value_set(unit, DBAL_FIELD_TYPE_DEF_DESTINATION,
                                                                        DBAL_FIELD_FEC, &fec_val, &kaps_dest));
    }
    else if (BCM_GPORT_IS_MCAST(value))
    {
        uint32 mc_id_val = BCM_GPORT_MCAST_GET(value);
        SHR_IF_ERR_EXIT(dbal_fields_field_type_parent_field32_value_set(unit, DBAL_FIELD_TYPE_DEF_DESTINATION,
                                                                        DBAL_FIELD_MC_ID, &mc_id_val, &kaps_dest));
    }
    else if (value < (1 << destination_max_bit))
    {
        kaps_dest = value;
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Invalid Destination value for LPM destination for SRv6 USP trap. "
                     "Valid options are gports of types FORWARD_GPORT or MCAST, or raw value within %d bits.",
                     destination_max_bit);
    }

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_SRV6, &entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LPM_DESTINATION_FOR_SRV6_USP_TRAP, INST_SINGLE,
                                 kaps_dest);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENABLER_FOR_LPM_DESTINATION_FOR_SRV6_USP_TRAP,
                                 INST_SINGLE, enable_trap);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT(srv6_modes.lpm_destination_for_usp_trap.set(unit, value));
...
shr_error_e
dnx_switch_srv6_lpm_destination_for_srv6_trap_get(
    int unit,
    int *value)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(srv6_modes.lpm_destination_for_usp_trap.get(unit, value));
