    ...
    int unit)
{
    ...
    int app_db_size_svl;
    int app_db_size_pcc;
    int offset = 0, index = 0, nof_index = 0;
    uint8 vmv_size = 0;
    uint8 vmv_value = 0;
    uint32 accepted_data_with_vmv = 0;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    accepted_data_with_vmv = dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_accepted_data_with_vmv);
    ...
    /** SVL - 2msbs is masked */
    nof_index = SAL_UPTO_BIT(DBAL_APP_ID_BASIC_NOF_BITS - app_db_size_svl);
    for (index = 0; index <= nof_index; index++)
    {
        offset = (index * (SAL_UPTO_BIT(app_db_size_svl) + 1)) + app_db_id_svl;
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEM_APP_DB, offset);
        ...
        if (accepted_data_with_vmv)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_APP_DB_MAP_STRENGTH_SIZE, INST_SINGLE,
                                         vmv_size);
        }
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    if (accepted_data_with_vmv)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_APP_DB_MAP_STRENGTH_SIZE, INST_SINGLE, vmv_size);
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int unit)
{
    ...
    /*
     * Size will be 16
     * Offset will be figured out from mode and may be different for SVL
     */
    if ((dnx_data_l2.general.l2_learn_limit_mode_get(unit) == LEARN_LIMIT_MODE_VLAN))
    {
        size = MACT_VSI_TO_PROFILE_IND_SIZE_SVL;
    }
    else
    {
     ...
     * Offset will be figured out from mode and may be different for IVL
     */
    size = MACT_VSI_TO_PROFILE_IND_SIZE;
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX,
                               DBAL_ENUM_FVAL_MACT_APP_DB_INDEX_IVL);
    ...
    int limit)
{
    ...
    SHR_IF_ERR_EXIT(algo_l2_db.vsi_learning_profile.exchange
                    (unit, 0, 1, &vsi_learning_profile_info, old_vsi_learning_profile,
                     NULL, &new_vsi_learning_profile, &is_first, &is_last));
