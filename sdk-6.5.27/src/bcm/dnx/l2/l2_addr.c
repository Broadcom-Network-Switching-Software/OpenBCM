    ...
    uint32 *flags)
{
    ...
    switch (table_id)
    {
        case DBAL_TABLE_FWD_MACT:
            switch (result_type)
            {
                case DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_DEST__LIF__STAT:
                    *flags = L2_ADDR_FWD_MACT_RESULT_DEST_LIF_STAT;
                    break;
                case DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_DEST__STAT:
                    *flags = L2_ADDR_FWD_MACT_RESULT_DEST_STAT;
                    break;
                case DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_EEI__FEC:
                    *flags = L2_ADDR_FWD_MACT_RESULT_EEI_FEC;
                    break;
                case DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_DEST__XL_FGID:
                    *flags = L2_ADDR_FWD_MACT_RESULT_NO_OUTLIF;
                    break;
                case DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_DEST__LIF__XL_FGID:
        ...
        case DBAL_TABLE_FWD_MACT_IVL:
            switch (result_type)
            {
                case DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST__LIF__STAT:
                    *flags = L2_ADDR_FWD_MACT_RESULT_DEST_LIF_STAT;
                    break;
                case DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST__STAT:
                    *flags = L2_ADDR_FWD_MACT_RESULT_DEST_STAT;
                    break;
                case DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_EEI__FEC:
                    *flags = L2_ADDR_FWD_MACT_RESULT_EEI_FEC;
                    break;
                case DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST__XL_FGID:
                    *flags = L2_ADDR_FWD_MACT_RESULT_NO_OUTLIF;
                    break;
                case DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST__LIF__XL_FGID:
                    *flags = L2_ADDR_FWD_MACT_RESULT_SINGLE_OUTLIF;
                    break;
                default:
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "result_type (%d) is not supported for API bcm_l2_addr_get \r\n",
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if (_SHR_IS_FLAG_SET(l2addr->flags, ~dnx_data_l2.general.l2_add_get_apis_supported_flags_get(unit)))
    {
    ...
    {
        if ((_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_LEARN_ONLY)) &&
            (_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_FWD_ONLY)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_L2_FLAGS2_LEARN_ONLY can't be set together with BCM_L2_FLAGS2_FWD_ONLY.");
        }
    }
    /** Verify the cores_group value*/
    if ((l2addr->cores_group != BCM_CORES_GROUP_ALL) && (l2addr->cores_group >= DBAL_NOF_CORE_GROUPS))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The cores_group %d is invalid, should be BCM_CORES_GROUP_ALL or less than the number of core groups!",
                     l2addr->cores_group);
    }
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact))
    {
        if ((_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_LEARN_ONLY)) ||
            (_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_FWD_ONLY)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_L2_FLAGS2_FWD_ONLY/BCM_L2_FLAGS2_LEARN_ONLY flag must not be set!");
        }
    }
    /** Verify the cores_group value*/
    if ((l2addr->cores_group != BCM_CORES_GROUP_ALL) && (l2addr->cores_group >= DBAL_NOF_CORE_GROUPS))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The cores_group %d is invalid!", l2addr->cores_group);
    }
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if (!_SHR_IS_FLAG_SET(res_type_flags, L2_ADDR_DBAL_FIELD_GLOB_OUT_LIF_2ND))
    {
        ...
        else if (dbal_sub_field == DBAL_FIELD_LAG_ID)
        {
            ...
            /** Get the encap_id */
            SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_from_forward_information(unit, &trunk_gport, (uint32 *) (&l2addr->encap_id), &forward_info, TRUE   /* force_port_and_encap 
                                                                                                                                                         */ ));
        ...
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_from_forward_information(unit, &l2addr->port, (uint32 *) (&l2addr->encap_id), &forward_info, FALSE /* force_port_and_encap 
                                                                                                                                                         */ ));
        }
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    uint32 field_flags;
    uint32 dbal_cores_group = DBAL_CORE_GROUP_ALL;
    dnx_l2_action_type_t l2_action_type;
    uint8 is_ivl;
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_application_v2))
    {
        if (l2addr->cores_group != BCM_CORES_GROUP_ALL)
        {
            dbal_cores_group = l2addr->cores_group;
        }
    }
    ...
    if (MACT_ENTRY_STRENGTH_ENCODE_SET(l2addr->learn_strength, l2addr->age_profile) ||
        dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_application_v2))
    {
        entry_strength = l2addr->learn_strength;
        age_profile = l2addr->age_profile;
        if ((entry_strength == 0) && (age_profile == 0))
        {
            /** In case of adding transplant-able static entry, expect to return E_EXISTS error when the entry has been existed */
    ...
    table = dnx_data_l2.general.fwd_mact_info_get(unit, l2_action_type, is_ivl)->logical_data_base;
    /** Write to MAC table */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table, &entry_handle_id));

    SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, table, &core_mode));
    if (core_mode == DBAL_CORE_MODE_DPCG)
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set(unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, dbal_cores_group));
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_ENTRY_AGE_PROFILE, INST_SINGLE, age_profile);
 ...
 * \param [in] table_id - dbal table id
 * \param [in] cores_group - cores_group
 * \return
 *   int
 *
 * \remark
    ...
    int cores_group)
{
    ...
    if (core_mode == DBAL_CORE_MODE_DPCG)
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set(unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, cores_group));
    }
    /*
 ...
 * \param [in] vid - vlan ID
 * \param [in] modid - vlan value in IVL mode, zero otherwise
 * \param [in] cores_group - cores_group value
 * \param [in] separate_fwd_lrn_mact - boolean whether to delete from separate forward mac tables
 * \return
 *   int
 *
 * \remark
    ...
    uint8 separate_fwd_lrn_mact)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_l2_addr_delete_fwd_mact_hw(unit, mac, vid, modid, table_id, cores_group));
    }
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    bcm_vlan_t vid)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l2_addr_delete_fwd_mact_internal
                    (unit, 0, mac, vid, 0, DBAL_CORE_GROUP_ALL, separate_fwd_lrn_mact));
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact))
    {
        SHR_IF_ERR_EXIT_EXCEPT_IF(dnx_l2_addr_delete_fwd_mact_internal
                                  (unit, L2_ADDR_ADD_LRN, mac, vid, 0, DBAL_CORE_GROUP_ALL, separate_fwd_lrn_mact),
                                  _SHR_E_NOT_FOUND);
    }
exit:
    if (separate_fwd_lrn_mact)
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact))
    {
        if ((_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_FWD_ONLY))
            && (_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_LEARN_ONLY)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_L2_FLAGS2_LEARN_ONLY can't be set together with BCM_L2_FLAGS2_FWD_ONLY.");
        }
    }
...
{
    uint32 flags = 0;
    int cores_group = DBAL_CORE_GROUP_ALL;
    uint8 separate_fwd_lrn_mact;
    ...
    if (l2addr->cores_group != BCM_CORES_GROUP_ALL)
    {
        cores_group = l2addr->cores_group;
    }
    ...
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact))
    {
            ...
            (!(_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_FWD_ONLY))))
        {
            flags |= L2_ADDR_ADD_LRN;
            SHR_IF_ERR_EXIT(dnx_l2_addr_delete_fwd_mact_internal
                            (unit, flags, l2addr->mac, l2addr->vid, l2addr->modid, cores_group, separate_fwd_lrn_mact));
        }
        if (!(_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_LEARN_ONLY)))
        {
            SHR_IF_ERR_EXIT(dnx_l2_addr_delete_fwd_mact_internal
                            (unit, 0, l2addr->mac, l2addr->vid, l2addr->modid, cores_group, separate_fwd_lrn_mact));
        }
    }
    else
    {
    ...
    bcm_l2_addr_t * l2addr)
{
    int nof_vsis;
    uint8 is_ivl;
    /** Indicates whether the age_state of the entry is supported */
    uint32 age_state_not_updated = dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_age_state_not_updated);
    uint32 nof_core_groups = dnx_data_device.general.nof_core_groups_get(unit);
    ...
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact))
    {
        if ((nof_core_groups == 1) && (_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_FWD_ONLY)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_L2_FLAGS2_FWD_ONLY flag must not be set!");
        }
        if ((_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_LEARN_ONLY)) &&
            (_SHR_IS_FLAG_SET(l2addr->flags2, BCM_L2_FLAGS2_FWD_ONLY)))
        {
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact))
    {
        flags2 |= (l2_action_type == l2ActionTypeFwdAndNA) ? BCM_L2_FLAGS2_FWD_ONLY : BCM_L2_FLAGS2_LEARN_ONLY;
    }

    table_id = dnx_data_l2.general.fwd_mact_info_get(unit, l2_action_type, ivl_mode)->logical_data_base;
    ...
    if (core_mode == DBAL_CORE_MODE_DPCG)
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set
                        (unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, l2addr->cores_group));
    }
    ...
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, action_flags));
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_addr_get_verify(unit, vid, l2addr));

    sal_memcpy(l2addr->mac, mac_addr, sizeof(sal_mac_addr_t));
    l2addr->vid = vid;
    l2addr->cores_group = DBAL_CORE_DEFAULT;
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dnx_l2_addr_get_internal(unit, flags, l2addr), _SHR_E_NOT_FOUND);
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_addr_by_struct_get_verify(unit, l2addr));
    ...
    /** In case the core_group is not set, returned the entry from the default core group*/
    if (l2addr->cores_group == BCM_CORES_GROUP_ALL)
    {
        l2addr->cores_group = DBAL_CORE_DEFAULT;
    }
    SHR_IF_ERR_EXIT(dnx_l2_addr_get_internal(unit, flags, l2addr));
