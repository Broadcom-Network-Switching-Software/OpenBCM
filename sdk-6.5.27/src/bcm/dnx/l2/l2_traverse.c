    ...
    uint32 *rules_handle_id)
{
    ...
    {
        uint32 entry_strength_enc;

        /** Set static strength as default */
        entry_strength = DYNAMIC_MACT_ENTRY_STRENGTH;

        /** Set  static transplantable if flags are provided */
        if (_SHR_IS_FLAG_SET(replace_l2addr->flags2, BCM_L2_FLAGS2_NO_MOVE))
        {
            entry_strength = DYNAMIC_NON_TRANSPLANTABLE_MACT_ENTRY_STRENGTH;
        }

        entry_strength_enc = MACT_ENTRY_STRENGTH_ENCODE_STRENGTH_GET(entry_strength);
        SHR_IF_ERR_EXIT(dbal_iterator_field_arr32_action_add
                        (unit, *rules_handle_id, DBAL_ITER_ACTION_UPDATE, DBAL_FIELD_MAC_STRENGTH, INST_SINGLE,
                         &entry_strength_enc, &entry_strength_mask));

        age_profile = MACT_ENTRY_STRENGTH_ENCODE_AGE_PROFILE_GET(entry_strength);
        SHR_IF_ERR_EXIT(dbal_iterator_field_arr32_action_add
    ...
    if (flags & DNX_L2_REPLACE_STATIC_SET)
    {
        ...
        /** Set  static transplantable if flags are provided */
        if (_SHR_IS_FLAG_SET(replace_l2addr->flags, BCM_L2_STATIC) &&
            _SHR_IS_FLAG_SET(replace_l2addr->flags, BCM_L2_MOVE_PORT))
        {
        ...
        if (replace_l2addr
            && MACT_ENTRY_STRENGTH_ENCODE_SET(replace_l2addr->learn_strength, replace_l2addr->age_profile))
        {
            /** if provided with learn_strength and age_profile, configure strength with provided data*/
            entry_strength_enc = replace_l2addr->learn_strength;
            age_profile = replace_l2addr->age_profile;
        }
        else
        {
            /*
             * if not provided with learn_strength and age_profile, set action using encoding of entry_strength
             *     provided via flags
             */
            entry_strength_enc = MACT_ENTRY_STRENGTH_ENCODE_STRENGTH_GET(entry_strength);
            age_profile = MACT_ENTRY_STRENGTH_ENCODE_AGE_PROFILE_GET(entry_strength);
    ...
    uint32 *rules_handle_id)
{
    ...
    uint32 destination, destination_mask;
    uint32 destination_field_type_size, dest_sub_type_size;
    dbal_fields_e dbal_dest_type = 0;
    uint32 dest_val = 0;
    uint32 outlif = 0;
    uint32 outlif_mask = 0;
    ...
    if (flags & DNX_L2_MATCH_DEST)
    {
        ...
        if (_SHR_IS_FLAG_SET(match_l2addr->flags, BCM_L2_TRUNK_MEMBER) || BCM_GPORT_IS_TRUNK(match_l2addr->port))
        {
            ...
            destination_mask &= trunk_mask | ~((1 << trunk_prefix_shift) - 1);
        }

        SHR_IF_ERR_EXIT(dbal_fields_uint32_sub_field_info_get
                        (unit, DBAL_FIELD_DESTINATION, destination_mask, &dbal_dest_type, &dest_val));

        SHR_IF_ERR_EXIT(dbal_fields_max_size_get(unit, DBAL_FIELD_DESTINATION, &destination_field_type_size));
        SHR_IF_ERR_EXIT(dbal_fields_max_size_get(unit, dbal_dest_type, &dest_sub_type_size));

        /*
         * Unmask all prefix encoding bits. Value bits should not change. 
         */
        destination_mask |= (((1 << (destination_field_type_size - dest_sub_type_size)) - 1) << dest_sub_type_size);
    ...
    if ((flags & DNX_L2_MATCH_HIT) && (match_mask_l2addr->flags & BCM_L2_HIT))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_iterator_attribute_rule_add(unit, *rules_handle_id,
                                                         (DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY |
                                                          ...
                                                          DBAL_ENTRY_ATTR_FLAG_HIT_GET), hit_value));
    }
