...
#define LEARN_PAYLOAD_TYPE_WIDTH            5
    ...
    if (num_of_ports > 0)
    {
        ...
        BCM_PBMP_ITER(pbmp, port)
        {
            ...
            if ((interface_offset == olp_blk_offset) &&
                ((core >= (cores_group * nof_cores_in_group)) && (core < ((cores_group + 1) * nof_cores_in_group))))
            {
                *olp_port = port;
                break;
            }
 ...
 *
 * \param [in] unit - unit id
 * \param [in] cores_group - cores group
 ...
 */
static int
dnx_l2_learn_olp_port_header_type_get(
    int unit,
    int cores_group,
    int *olp_pp_port,
    ...
    int *olp_out_port_header_type)
{
    ...
    bcm_gport_t olp_logical_port = BCM_GPORT_INVALID;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_l2_learn_dnx_olp_port_get(unit, cores_group,
                                                  dnx_data_l2.olp.olp_fwd_block_id_get(unit), &olp_logical_port));
    if (olp_logical_port == BCM_GPORT_INVALID)
    {
    ...
    dnx_event_forwarding_profile_info_t * event_forwarding_profile_info)
{
    ...
    if (allocate_failed == _SHR_E_NONE)
    {
        ...
        SHR_IF_ERR_EXIT(algo_l2_db.vsi_learning_profile.exchange
                        (unit, 0, 1, &new_vsi_learning_profile_info, old_vsi_learning_profile,
    ...
    int lpkgv_one_outlif)
{
    ...
    current_len = LEARN_PAYLOAD_TYPE_WIDTH + 1;
    ...
    int lpkgv_one_outlif)
{
    ...
    /** LPKGV[7:3]: type(5) */
    current_len = LEARN_PAYLOAD_TYPE_WIDTH;
    current_offset -= current_len;
    learn_ffc_instruction =
    ...
    bcm_l2_learn_msgs_config_t * learn_msg_config)
{
    ...
    int olp_out_port_header_type;
    int olp_in_port_header_type_dummy;
    ...
    SHR_IF_ERR_EXIT(dnx_l2_learn_olp_port_header_type_get(unit, learn_msg_config->cores_group, &olp_pp_port,
                                                          &olp_in_port_header_type_dummy, &olp_out_port_header_type));
    ...
    SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, DBAL_TABLE_OLP_GENERAL_CONFIGURATION, &core_mode));
    if (core_mode == DBAL_CORE_MODE_DPCG)
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set
                        (unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, learn_msg_config->cores_group));
    }
    ...
    bcm_l2_learn_msgs_config_t * learn_msg_config)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l2_learn_olp_port_header_type_get(unit, learn_msg_config->cores_group,
                                                          &olp_pp_port, &olp_in_port_header_type,
                                                          &olp_out_port_header_type));
    ...
    SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, DBAL_TABLE_OLP_HEADER_CONFIGURATION, &core_mode));
    if (core_mode == DBAL_CORE_MODE_DPCG)
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set
                        (unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, learn_msg_config->cores_group));
    ...
    SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, DBAL_TABLE_OLP_DSP_ENGINE_CONFIGURATION, &core_mode));
    if (core_mode == DBAL_CORE_MODE_DPCG)
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set
                        (unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, learn_msg_config->cores_group));
    ...
    bcm_l2_learn_msgs_config_t * learn_msg_config)
{
    ...
    if ((learn_msg_config->cores_group != BCM_CORES_GROUP_ALL)
        && (learn_msg_config->cores_group >= DBAL_NOF_CORE_GROUPS))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The cores_group %d is invalid, should be BCM_CORES_GROUP_ALL or less than the number of core groups!",
                     learn_msg_config->cores_group);
    }
...
bcm_dnx_l2_learn_msgs_config_set(
    ...
    bcm_l2_learn_msgs_config_t * learn_msg_config)
{
    int core_group = learn_msg_config->cores_group;
    int core_group_iter_end = (core_group == BCM_CORES_GROUP_ALL) ?
        dnx_data_device.general.nof_core_groups_get(unit) : (core_group + 1);
    int core_group_iter = (core_group == BCM_CORES_GROUP_ALL) ? 0 : core_group;
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_l2_learn_msgs_config_set_verify(unit, learn_msg_config));

    for (; core_group_iter < core_group_iter_end; core_group_iter++)
    {
        learn_msg_config->cores_group = core_group_iter;
    ...
    /*
     * Assign the original input for cores group
     */
    learn_msg_config->cores_group = core_group;
exit:
    DNX_ERR_RECOVERY_END(unit);
    SHR_FUNC_EXIT;
}
    ...
    bcm_l2_learn_msgs_config_t * learn_msg_config)
{
    ...
    if (core_mode == DBAL_CORE_MODE_DPCG)
    {
        if (learn_msg_config->cores_group == BCM_CORES_GROUP_ALL)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "invalid parameter, cores_group can't be BCM_CORES_GROUP_ALL for get APIs");
        }
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set
                        (unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, learn_msg_config->cores_group));
    }
    ...
    switch (result_type)
    {
        ...
        case DBAL_RESULT_TYPE_OLP_HEADER_CONFIGURATION_UDH_O_ITMH_PTCH2:
        {
            ...
            {
                learn_msg_config->dest_port = gport;
            }
