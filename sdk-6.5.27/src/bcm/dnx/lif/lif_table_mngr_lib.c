 ...
 * Note: Parent and Child fields should have the same atrributes.
 */
/* *INDENT-OFF* */
static const lif_table_manager_field_actions_default_t lif_table_manager_field_actions_default[] = {
    ...
    {DBAL_FIELD_SRH_HDR_EXT_LEN,               SET_ACTION_USE_VALUE,        UNSET_ACTION_SET_TO_DEFAULT, TRUE,  TRUE},
    ...
    {DBAL_FIELD_LEARN_INFO_NO_LIF_LARGE_EXT_SYM_LIF_EXT, SET_ACTION_USE_VALUE,    UNSET_ACTION_SET_TO_DEFAULT, TRUE,  TRUE},
...
static const lif_table_manager_field_actions_per_table_t lif_table_manager_field_actions_per_table[] = {
    ...
    {DBAL_TABLE_IN_LIF_FORMAT_LSP,   DBAL_FIELD_GLOB_OUT_LIF,    SET_ACTION_USE_VALUE,        UNSET_ACTION_SET_TO_DEFAULT, TRUE,  TRUE},
    {DBAL_TABLE_IN_LIF_FORMAT_LSP,   DBAL_FIELD_DESTINATION,     SET_ACTION_USE_VALUE,        UNSET_ACTION_IGNORE,         TRUE,  TRUE},
    ...
    int local_lif_id)
{
     ...
     */
    local_lif_info.valid_fields = FLOW_LIF_MGMT_VALID_BM_UNINITIALIZED;
    ...
    int new_local_lif_id)
{
     ...
     */
    if (LIF_MNGR_DB_IS_INLIF_DPC(unit, physical_table_id))
    {
    ...
    lif_table_mngr_outlif_info_t * lif_info)
{
    ...
    if (_SHR_IS_FLAG_SET(lif_info->flags, LIF_TABLE_MNGR_LIF_INFO_REPLACE))
    {
        ...
        uint32 entry_handle_id_existing;
        dbal_tables_e dbal_table_id, existing_dbal_table_id = DBAL_TABLE_EMPTY;
    ...
    lif_table_mngr_outlif_info_t * lif_info)
{
    ...
    if (_SHR_IS_FLAG_SET(lif_info->flags, LIF_TABLE_MNGR_LIF_INFO_REPLACE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get(unit, *local_lif_id,
                                                        &dbal_table_id, &existing_result_type, NULL, NULL,
    ...
    uint8 *is_valid)
{
    ...
    dbal_tables_e dbal_table_id_get = DBAL_TABLE_EMPTY;
    uint8 found = FALSE;
    local_lif_info_t local_lif_info;
    uint32 field_ids_bitmap[2];
    ...
    else
    {
        ...
        else
        {
            ...
            {
                SHR_IF_ERR_EXIT(lif_table_mngr_db.
                                valid_fields_per_ingress_lif1.get(unit, local_lif_id, &local_lif_info));
            }
            else
            {
                SHR_IF_ERR_EXIT(lif_table_mngr_db.
                                valid_fields_per_ingress_lif2.get(unit, local_lif_id, &local_lif_info));
            }
        }
        found = (local_lif_info.valid_fields != FLOW_LIF_MGMT_VALID_BM_UNINITIALIZED) ? TRUE : FALSE;
    }
    if (!found)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
