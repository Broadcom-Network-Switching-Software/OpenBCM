    ...
    int *eth_rif_profile_egress_reserved_start_bit)
{
    ...
    else
    {
        ...
        {
            *eth_rif_profile_used_bits_mask = (1 << NOF_ETH_RIF_PROFILE_BITS) - 1;
        }
        else
        {
    ...
    int *egress_reserved_start_bit)
{
        ...
        DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO2_MODE)
    {
        *egress_reserved_nof_bits = IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS;
        ...
        {
            *in_lif_profile_used_bits_mask = (1 << NOF_IN_LIF_PROFILE_BITS) - 1;
            if (!dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_use_mapped_in_lif_profile)
                && (IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS > 0))
            {
               /** Set the egress bits to 0s */
                SHR_IF_ERR_EXIT(utilex_bitstream_reset_bit_range
                                ((uint32 *) in_lif_profile_used_bits_mask, IN_LIF_PROFILE_EGRESS_RESERVED_START_BIT,
                                 IN_LIF_PROFILE_EGRESS_RESERVED_START_BIT + IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS -
                                 1));
            }
            if (NATIVE_INDEXED_MODE_NOF_BITS > 0)
            {
                SHR_IF_ERR_EXIT(utilex_bitstream_set((uint32 *) in_lif_profile_used_bits_mask,
                                                     IN_LIF_PROFILE_NATIVE_INDEXED_MODE_START_BIT, 0));
            }
        }
        else
        {
    ...
    if (is_dealloc)
    {
        ...
        if (intf_type == ETH_RIF)
        {
            if (dnx_data_lif.
                in_lif.feature_get(unit, dnx_data_lif_in_lif_use_eth_rif_profile_range_allocation_per_pmf_profiles))
            {
                max_counter_value = 1 << egress_reserved_nof_bits;
            }
            else
            {
        ...
        else
        {
            if (dnx_data_lif.
                in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles))
            {
                max_counter_value = 1 << (egress_reserved_nof_bits + NATIVE_INDEXED_MODE_NOF_BITS);
            }
            else
            {
                max_counter_value =
                    1 << (pmf_reserved_nof_bits + egress_reserved_nof_bits + NATIVE_INDEXED_MODE_NOF_BITS);
            }
        }
    ...
    int intf_type)
{
    ...
    else
    {
        if (dnx_data_lif.
            in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles))
        {
            max_counter_value = 1 << NATIVE_INDEXED_MODE_NOF_BITS;
        }
        else
        {
    ...
    int intf_type)
{
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RPF_MODE_IPV4, INST_SINGLE,
                                     DBAL_ENUM_FVAL_RPF_MODE_DISABLE);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RPF_MODE_IPV6, INST_SINGLE,
                                     DBAL_ENUM_FVAL_RPF_MODE_DISABLE);
    }
    else
    {
    ...
    if (is_dealloc)
    {
        ...
        else
        {
            if (dnx_data_lif.
                in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles))
            {
                max_counter_value = 1 << (egress_reserved_nof_bits + native_indexed_mode_bits);
            }
            else
            {
    ...
    int intf_type)
{
    ...
    else
    {
        if (dnx_data_lif.
            in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles))
        {
            max_counter_value = 1 << native_indexed_mode_bits;
        }
        else
        {
    ...
    for (hw_write_counter = 0; hw_write_counter < max_counter_value; hw_write_counter++)
    {
        ...
        else
        {
                ...
                in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles))
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF_PROFILE,
                                           in_lif_profile | (hw_write_counter <<
                                                             (NOF_IN_LIF_PROFILE_BITS - native_indexed_mode_bits)));
            }
            else
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF_PROFILE,
...
/**
 * \see
 * in_lif_profile.h for reference
 */
shr_error_e
    ...
    in_lif_profile_info_t * in_lif_profile_info)
{
    ...
    else if (cs_lif_profile_flags_uint32 == DBAL_ENUM_FVAL_CS_LIF_PROFILE_FLAGS_SRV6_GSID)
    {
    ...
    else if (cs_lif_profile_flags_uint32 == DBAL_ENUM_FVAL_CS_LIF_PROFILE_FLAGS_VXLAN_VNI2LIF)
    {
        in_lif_profile_info->ingress_fields.cs_in_lif_profile_flags |=
            DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_VXLAN_VNI2LIF;
    }
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    dbal_tables_e inlif_dbal_table_id)
{
    ...
    if (inlif_dbal_table_id == DBAL_TABLE_IN_LIF_FORMAT_PWE)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_PWE_INLIF_CS_PROFILE, DBAL_FIELD_NOF_SD_TAGS, &pwe_nof_tags,
                         &cs_vtt_in_lif_profile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_PWE_INLIF_CS_PROFILE, DBAL_FIELD_TAG_NAMESPACE, &pwe_namespace,
                         &cs_vtt_in_lif_profile));
        /*
    ...
    else if (inlif_dbal_table_id == DBAL_TABLE_IN_LIF_FORMAT_EVPN)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_EVPN_IML_INLIF_CS_PROFILE, DBAL_FIELD_IML_ADDITIONAL_HEADERS,
                         &evpn_iml_ah, &cs_vtt_in_lif_profile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_EVPN_IML_INLIF_CS_PROFILE, DBAL_FIELD_TAG_NAMESPACE, &pwe_namespace,
                         &cs_vtt_in_lif_profile));
        ...
        if (evpn_iml_ah == DBAL_ENUM_FVAL_LIF_VTTX_CS_EVPN_IML_AH_FL_AND_CW)
        {
            in_lif_profile_info->ingress_fields.cs_in_lif_profile_flags |=
             ...
             (in_lif_profile_info->
              ingress_fields.cs_in_lif_profile_flags & DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_L2VPN))
    {
        dbal_enum_value_field_nof_sd_tags_e nof_tags = 0;
        dbal_enum_value_field_lif_vtt4_csp_1bit_pwe_tag_namespace_e tagged_namespace = 0;
        dbal_enum_value_field_lif_vtt5_csp_1bit_nat_direction_and_virtual_ip_and_srv6_gsid_e srv6_gsid_eoc = 0;

        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_IP_TUNNEL_INLIF_CS_PROFILE, DBAL_FIELD_NOF_SD_TAGS, &nof_tags,
                         &cs_vtt_in_lif_profile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_IP_TUNNEL_INLIF_CS_PROFILE, DBAL_FIELD_TAG_NAMESPACE,
                         &tagged_namespace, &cs_vtt_in_lif_profile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_IP_TUNNEL_INLIF_CS_PROFILE, DBAL_FIELD_SRV6_GSID_EOC, &srv6_gsid_eoc,
                         &cs_vtt_in_lif_profile));
        in_lif_profile_info->ingress_fields.pwe_nof_eth_tags = nof_tags;

        if (tagged_namespace == DBAL_ENUM_FVAL_LIF_VTT4_CSP_1BIT_PWE_TAG_NAMESPACE_LIF_SCOPED)
        {
            in_lif_profile_info->ingress_fields.cs_in_lif_profile_flags |=
        ...
        if (srv6_gsid_eoc ==
            DBAL_ENUM_FVAL_LIF_VTT5_CSP_1BIT_NAT_DIRECTION_AND_VIRTUAL_IP_AND_SRV6_GSID_NAT_DOWNSTREAM_VIP_ENABLED_SRV6_GSID_EOC_VNI_VXLAN)
        {
            in_lif_profile_info->ingress_fields.cs_in_lif_profile_flags |=
             ...
             || (inlif_dbal_table_id == dnx_data_l2.vsi.vsi_table_get(unit)))
    {
        ...
        dbal_enum_value_field_lif_vtt5_csp_1bit_nat_direction_and_virtual_ip_and_srv6_gsid_e
            nat_or_virtual_ip_or_srv6_gsid_or_vni_vxlan = 0;
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_ROUTING_VTT2FWD1_INLIF_CS_PROFILE,
                         DBAL_FIELD_NAT_DIRECTION_OR_VIP_MODE_OR_SRV6_GSID_OR_VNI_VXLAN,
                         &nat_or_virtual_ip_or_srv6_gsid_or_vni_vxlan, &cs_vtt_in_lif_profile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_ROUTING_VTT2FWD1_INLIF_CS_PROFILE,
                         DBAL_FIELD_MPLS_NAMESPACE_OR_AACL_OR_ESP_VXLAN, &mpls_namespace_or_aacl_or_esp_vxlan,
                         &cs_vtt_in_lif_profile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                        (unit, DBAL_FIELD_TYPE_DEF_ROUTING_VTT2FWD1_INLIF_CS_PROFILE, DBAL_FIELD_UC_DB_MODE, &uc_mode,
                         &cs_vtt_in_lif_profile));
        if ((nat_or_virtual_ip_or_srv6_gsid_or_vni_vxlan ==
             DBAL_ENUM_FVAL_LIF_VTT5_CSP_1BIT_NAT_DIRECTION_AND_VIRTUAL_IP_AND_SRV6_GSID_NAT_DOWNSTREAM_VIP_ENABLED_SRV6_GSID_EOC_VNI_VXLAN)
            && (inlif_dbal_table_id == dnx_data_l2.vsi.vsi_table_get(unit)))
        {
            in_lif_profile_info->ingress_fields.l3_ingress_flags |= BCM_L3_INGRESS_VIP_ENABLED;
        }
        else if ((nat_or_virtual_ip_or_srv6_gsid_or_vni_vxlan ==
                  DBAL_ENUM_FVAL_LIF_VTT5_CSP_1BIT_NAT_DIRECTION_AND_VIRTUAL_IP_AND_SRV6_GSID_NAT_DOWNSTREAM_VIP_ENABLED_SRV6_GSID_EOC_VNI_VXLAN)
    ...
    in_lif_intf_type_e intf_type)
{
    ...
    SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_profile_l3_enablers.exchange
                    (unit, 0, 1, routing_enablers_vector, old_routing_enablers_profile,
    ...
    in_lif_intf_type_e intf_type)
{
    ...
    SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_lb_profile_disablers_flags.exchange
                    (unit, 0, 1, &lif_lb_profile_disablers_flags_vector, old_in_lif_profile_data.in_lif_lb_profile,
    ...
    dbal_enum_value_field_cs_lif_profile_flags_e * cs_lif_profile_flags)
{
    ...
    {
        *cs_lif_profile_flags = DBAL_ENUM_FVAL_CS_LIF_PROFILE_FLAGS_SRV6_GSID;
    }
    else if (in_lif_profile_info->
             ingress_fields.cs_in_lif_profile_flags & DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_VXLAN_OVER_ESP)
    {
        *cs_lif_profile_flags = DBAL_ENUM_FVAL_CS_LIF_PROFILE_FLAGS_VXLAN_ESP;
             ...
             ingress_fields.cs_in_lif_profile_flags & DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_VXLAN_VNI2LIF)
    {
        *cs_lif_profile_flags = DBAL_ENUM_FVAL_CS_LIF_PROFILE_FLAGS_VXLAN_VNI2LIF;
    }
}
...
/**
 * \brief
 *   delete the saves in swstate CS LIF profile relevant information
 * \param [in] unit -
 *   The unit number.
 * \param [in] in_lif_profile_info -
 *   Pointer to in_lif_profile_info
 ...
 */
static shr_error_e
dnx_cs_lif_profile_sw_state_delete(
    int unit,
    in_lif_profile_info_t * in_lif_profile_info)
{
    int rv;
    ...
    /** Clear the CS LIF profile info for the given gport */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_CS_LIF_PROFILE_FLAGS_PER_GPORT_SW, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_GPORT, in_lif_profile_info->ingress_fields.lif_gport);
    ...
    uint8 *cs_in_lif_fwd_profile_value)
{
    ...
    if (inlif_dbal_table_id == DBAL_TABLE_IN_LIF_FORMAT_PWE)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_BRIDGING_FWD12FWD2_INLIF_CS_PROFILE,
                         DBAL_FIELD_EXTENDED_P2P_NOF_SD_TAGS, &pwe_nof_tags, &flp_cs_in_lif_profile));
    }
    ...
    else if (inlif_dbal_table_id == DBAL_TABLE_IN_LIF_FORMAT_EVPN)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_EVPN_IML_INLIF_CS_PROFILE, DBAL_FIELD_IML_ADDITIONAL_HEADERS,
                         &evpn_iml_ah, &vtt_cs_in_lif_proifile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_EVPN_IML_INLIF_CS_PROFILE, DBAL_FIELD_TAG_NAMESPACE, &pwe_namespace,
                         &vtt_cs_in_lif_proifile));
    }
              ...
              ingress_fields.cs_in_lif_profile_flags & DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_L2VPN))
    {
        ...
        /*
         * In this case, only DNX_IN_LIF_PROFILE_CS_PROFILE_PWE_LIF_SCOPED and
         * in_lif_profile_info_t.pwe_nof_eth_tags are relevant
         */
        if ((in_lif_profile_info->ingress_fields.l3_ingress_flags & BCM_L3_INGRESS_GLOBAL_ROUTE)
            || (in_lif_profile_info->ingress_fields.l3_ingress_flags & BCM_L3_INGRESS_MPLS_INTF_NAMESPACE))
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "invalid fields or flags for L2VPN IP Tunnel LIF\n");
        }

        nof_tags = DBAL_ENUM_FVAL_NOF_SD_TAGS_0_SD_TAGS + in_lif_profile_info->ingress_fields.pwe_nof_eth_tags;
        /*
         * Second, set the context scope (LiF/Network) scoped
         */
        if (in_lif_profile_info->
            ingress_fields.cs_in_lif_profile_flags & DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_L2VPN_LIF_SCOPED)
        {
            tagged_namespace = DBAL_ENUM_FVAL_LIF_VTT4_CSP_1BIT_PWE_TAG_NAMESPACE_LIF_SCOPED;
        }
        else
        {
            tagged_namespace = DBAL_ENUM_FVAL_LIF_VTT4_CSP_1BIT_PWE_TAG_NAMESPACE_NWK_SCOPED;
        }
        ...
        else
        {
            ...
            {
                srv6_gsid_eoc =
                    DBAL_ENUM_FVAL_LIF_VTT5_CSP_1BIT_NAT_DIRECTION_AND_VIRTUAL_IP_AND_SRV6_GSID_NAT_UPSTREAM_VIP_DISABLED_SRV6_GSID_SOC_VNI_VXLAN;
            }
        }

        /** VTT CS perofile encoding*/
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_IP_TUNNEL_INLIF_CS_PROFILE, DBAL_FIELD_SRV6_GSID_EOC,
                         &srv6_gsid_eoc, &vtt_cs_in_lif_proifile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_IP_TUNNEL_INLIF_CS_PROFILE, DBAL_FIELD_NOF_SD_TAGS, &nof_tags,
                         &vtt_cs_in_lif_proifile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_IP_TUNNEL_INLIF_CS_PROFILE, DBAL_FIELD_TAG_NAMESPACE,
                         &tagged_namespace, &vtt_cs_in_lif_proifile));

        /** FLP CS perofile encoding*/
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_BRIDGING_FWD12FWD2_INLIF_CS_PROFILE,
                         DBAL_FIELD_EXTENDED_P2P_NOF_SD_TAGS, &nof_tags, &flp_cs_in_lif_profile));
    }
    else if ((inlif_dbal_table_id == DBAL_TABLE_IN_LIF_IPvX_TUNNELS) ||
             (inlif_dbal_table_id == DBAL_TABLE_IN_LIF_FORMAT_LSP) ||
             (inlif_dbal_table_id == dnx_data_l2.vsi.vsi_table_get(unit)))
    {
        ...
        dbal_enum_value_field_lif_vtt5_csp_1bit_nat_direction_and_virtual_ip_and_srv6_gsid_e
            nat_or_virtual_ip_or_srv6_gsid_or_vni_vxlan;
        dbal_enum_value_field_lif_xxxx_csp_1bit_l3lif_mpls_namespace_and_algo_acl_or_esp_vxlan_e
        ...
        {
            if (in_lif_profile_info->
                ingress_fields.l3_ingress_flags & DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_VXLAN_VNI2LIF)
            {
                SHR_ERR_EXIT(_SHR_E_FAIL, "SRv6 GSID EOC and VXLAN VNI2LIF cannot be set together on the same LIF\n");
            }
        }
        if (in_lif_profile_info->ingress_fields.cs_in_lif_profile_flags & BCM_L3_INGRESS_VIP_ENABLED)
        {
            if (in_lif_profile_info->
                ingress_fields.l3_ingress_flags & DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_VXLAN_VNI2LIF)
            {
                SHR_ERR_EXIT(_SHR_E_FAIL, "Virtual IP and VXLAN VNI2LIF cannot be set together on the same LIF\n");
            }
        }
        if (in_lif_profile_info->ingress_fields.l3_ingress_flags & BCM_L3_INGRESS_GLOBAL_ROUTE)
        {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_ROUTING_VTT2FWD1_INLIF_CS_PROFILE,
                         DBAL_FIELD_NAT_DIRECTION_OR_VIP_MODE_OR_SRV6_GSID_OR_VNI_VXLAN,
                         &nat_or_virtual_ip_or_srv6_gsid_or_vni_vxlan, &vtt_cs_in_lif_proifile));
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_ROUTING_VTT2FWD1_INLIF_CS_PROFILE,
                         DBAL_FIELD_MPLS_NAMESPACE_OR_AACL_OR_ESP_VXLAN, &mpls_namespace_or_aacl_or_esp_vxlan,
                         &vtt_cs_in_lif_proifile));
    ...
    else if (inlif_dbal_table_id == DBAL_TABLE_IN_AC_INFO_DB
             || inlif_dbal_table_id == DBAL_TABLE_PON_DTC_CLASSIFICATION)
    {
        dbal_enum_value_field_ip_address_spoofing_check_modes_e ip_anti_spoofing_mode = 0;
        uint32 lif_on_lag_is_shown = (uint32) (in_lif_profile_info->ingress_fields.lif_on_lag_is_shown);
        uint32 oam_primary_vlan_mode = (uint32) (in_lif_profile_info->ingress_fields.oam_primary_vlan_mode);
        uint32 bridge_namespace = DBAL_ENUM_FVAL_LIF_VTT5_CSP_1BIT_BRIDGE_NAMESPACE_LIF_SCOPED;
        if (lif_on_lag_is_shown)
        {
            SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                            (unit, DBAL_FIELD_TYPE_DEF_AC_INLIF_CS_PROFILE, DBAL_FIELD_BRIDGE_NAMESPACE,
                             &bridge_namespace, &vtt_cs_in_lif_proifile));
            /** VTT CS perofile encoding*/
            SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                            (unit, DBAL_FIELD_TYPE_DEF_AC_INLIF_CS_PROFILE, DBAL_FIELD_IS_ON_LAG,
                             &lif_on_lag_is_shown, &vtt_cs_in_lif_proifile));
        }
        if (oam_primary_vlan_mode)
        {
            /** VTT CS perofile encoding*/
            SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                            (unit, DBAL_FIELD_TYPE_DEF_AC_INLIF_CS_PROFILE, DBAL_FIELD_IP_LAYER_CLASSIFICATION_MODE,
                             &oam_primary_vlan_mode, &vtt_cs_in_lif_proifile));
        }
        ip_anti_spoofing_mode = in_lif_profile_info->ingress_fields.ip_anti_spoofing_check_mode;
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, DBAL_FIELD_TYPE_DEF_AC_INLIF_CS_PROFILE, DBAL_FIELD_IP_ADDRESS_SPOOFING_CHECK_MODE,
                         &ip_anti_spoofing_mode, &vtt_cs_in_lif_proifile));
    }
...
/**
 * \brief
 *   removes cs_in_lif_profile_value according to in_lif_profile properties
 * \param [in] unit -
 *   The unit number.
 * \param [in] in_lif_profile_info -
 *   Pointer to in_lif_profile_info
 ...
 */
static shr_error_e
dnx_in_lif_profile_remove_cs_in_lif_profile(
    int unit,
    in_lif_profile_info_t * in_lif_profile_info)
{
    SHR_FUNC_INIT_VARS(unit);

    /** Remove from swstate information about the CS lif profile namespace per GPORT and stage */
    SHR_IF_ERR_EXIT(dnx_cs_lif_profile_sw_state_delete(unit, in_lif_profile_info));
 ...
 *   \param [in] unit -
 *     The unit number.
 *   \param [in] rpf_mode_ipv4 -
 *     Pointer to rpf_mode \n
 *     \b As \b output - \n
 *       the rpf_mode to return for IPv4 packets
 *   \param [in] rpf_mode_ipv6 -
 *     Pointer to rpf_mode \n
 *     \b As \b output - \n
 *       the rpf_mode to return for IPv6 packets
 *   \param [in] in_lif_profile_info -
 *     Pointer to in_lif_profile_info \n
 *     \b As \b input - \n
 *       includes the lif profile properties, only flags is used
    ...
    in_lif_profile_info_t * in_lif_profile_info)
{
    ...
    switch (in_lif_profile_info->ingress_fields.urpf_mode)
    {
        case bcmL3IngressUrpfLoose:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_LOOSE;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_LOOSE;
            break;
        case bcmL3IngressUrpfStrict:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_STRICT;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_STRICT;
            break;
        case bcmL3IngressUrpfDisable:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_DISABLE;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_DISABLE;
            break;
        case bcmL3IngressUrpfV4DisableV6Loose:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_DISABLE;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_LOOSE;
            break;
        case bcmL3IngressUrpfV4DisableV6Strict:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_DISABLE;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_STRICT;
            break;
        case bcmL3IngressUrpfV4LooseV6Disable:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_LOOSE;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_DISABLE;
            break;
        case bcmL3IngressUrpfV4LooseV6Strict:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_LOOSE;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_STRICT;
            break;
        case bcmL3IngressUrpfV4StrictV6Disable:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_STRICT;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_DISABLE;
            break;
        case bcmL3IngressUrpfV4StrictV6Loose:
            *rpf_mode_ipv4 = DBAL_ENUM_FVAL_RPF_MODE_STRICT;
            *rpf_mode_ipv6 = DBAL_ENUM_FVAL_RPF_MODE_LOOSE;
            break;
        default:
    ...
    uint8 *last_reference)
{
    ...
    SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_da_not_found_destination.exchange
                    (unit, 0, 1, default_frwrd_dst, in_lif_profile_template_data.da_not_found_dst_profile,
    ...
    in_lif_intf_type_e intf_type)
{
    ...
    if (dnx_data_switch.load_balancing.hashing_selection_per_layer_get(unit))
    {
        ...
        SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_lb_profile_disablers_flags.exchange
                        (unit, 0, 1, &in_lif_lb_profile_disablers_flags_vector, old_lif_lb_profile,
    ...
    if (intf_type == ETH_RIF)
    {
        SHR_IF_ERR_EXIT(algo_lif_profile_db.eth_rif_profile.exchange
                        (unit, 0, 1, &default_in_lif_profile_template_data,
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_da_not_found_destination.exchange
                        (unit, 0, 1, default_frwrd_dst, old_lif_flood_profile,
    ...
    {
        if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_use_mapped_in_lif_profile_field_mapping_table))
        {
        }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_remove_cs_in_lif_profile(unit, in_lif_profile_info));
    }
    ...
    in_lif_intf_type_e intf_type)
{
    ...
    if (last_profile.old_in_lif_profile != DNX_IN_LIF_PROFILE_DEFAULT)
    {
        ...
        {
            if (dnx_data_lif.
                in_lif.feature_get(unit, dnx_data_lif_in_lif_use_mapped_in_lif_profile_field_mapping_table))
            {
            }
            if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_use_mapped_in_lif_profile))
            {
    ...
    dnx_in_lif_profile_last_info_t * last_profile)
{
    ...
    int routing_enablers_profile, da_not_found_dst_profile, in_lif_lb_profile, egress_properties_nof_bits,
        first_profile_in_range = 0;
    int egress_in_lif_profile, hw_write_counter, pmf_reserved_nof_bits, egress_reserved_nof_bits, max_counter_value,
        pmf_reserved_start_bit, pmf_in_lif_profile, pmf_bits_mask =
        0, eth_rif_profile_used_bits_mask, in_lif_profile_egress_reserved_start_bit, egress_eth_rif_profile, ref_count,
        eth_rif_profile_egress_reserved_start_bit, in_lif_profile_used_bits_mask;
    uint8 rpf_mode[2] = { 0 };
    uint8 cs_vtt_in_lif_profile_value, cs_vtt_to_flp_in_lif_profile_value, is_first, routing_profile_is_first,
     ...
     * handle RPF
     */
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_algo_convert_to_rpf_mode(unit, &rpf_mode[0], &rpf_mode[1], in_lif_profile_info));
    ...
    in_lif_profile_template_data.rpf_mode_ipv4 = rpf_mode[0];
    in_lif_profile_template_data.rpf_mode_ipv6 = rpf_mode[1];
    ...
    if (((intf_type == LIF)
         && dnx_data_lif.in_lif.feature_get(unit,
    ...
    {
        in_lif_profile_template_data.field_class_id = in_lif_profile_info->field_class_id;
    }
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_use_mapped_in_lif_profile))
    {
    ...
    if (intf_type == ETH_RIF)
    {
        ...
        if (dnx_data_lif.
            in_lif.feature_get(unit, dnx_data_lif_in_lif_use_eth_rif_profile_range_allocation_per_pmf_profiles))
        {
            tag_info.resource_flags = RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE;
            first_profile_in_range = in_lif_profile_info->field_class_id << ETH_RIF_PROFILE_PMF_RESERVED_START_BIT;
            tag_info.resource_alloc_info.range_start = first_profile_in_range;
            tag_info.resource_alloc_info.range_end =
                first_profile_in_range + utilex_power_of_2(NOF_ETH_RIF_PROFILE_INGRESS_BITS);
            SHR_IF_ERR_EXIT(algo_lif_profile_db.eth_rif_profile.exchange(unit, 0, 1, &in_lif_profile_template_data,
                                                                         old_in_lif_profile &
                                                                         eth_rif_profile_used_bits_mask, &tag_info,
                                                                         new_in_lif_profile, &is_first,
                                                                         &lif_profile_is_last));
        }
        else
        {
            SHR_IF_ERR_EXIT(algo_lif_profile_db.eth_rif_profile.exchange
                            (unit, 0, 1, &in_lif_profile_template_data,
    ...
    else if (intf_type == LIF)
    {
        ...
        {
            tag_info.resource_flags = RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE;
            first_profile_in_range = in_lif_profile_info->field_class_id << IN_LIF_PROFILE_PMF_RESERVED_START_BIT;
            tag_info.resource_alloc_info.range_start = first_profile_in_range;
            if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_use_mapped_in_lif_profile))
            {
                tag_info.resource_alloc_info.range_end =
                    first_profile_in_range + utilex_power_of_2(NOF_IN_LIF_PROFILE_BITS -
                                                               IN_LIF_PROFILE_PMF_RESERVED_NOF_BITS -
                                                               NATIVE_INDEXED_MODE_NOF_BITS);
            }
            else
            {
                /** egress bits are not part of the template manager */
                tag_info.resource_alloc_info.range_end =
                    first_profile_in_range + utilex_power_of_2(NOF_IN_LIF_PROFILE_BITS -
                                                               IN_LIF_PROFILE_PMF_RESERVED_NOF_BITS -
                                                               IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS -
                                                               NATIVE_INDEXED_MODE_NOF_BITS);
            }
            SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_profile.exchange(unit, 0, 1, &in_lif_profile_template_data,
                                                                        old_in_lif_profile &
                                                                        in_lif_profile_used_bits_mask, &tag_info,
                                                                        new_in_lif_profile, &is_first,
                                                                        &lif_profile_is_last));
        }
        else
    ...
    if (is_first)
    {
        ...
        /**
         * If IPv4 and IPV6 RPF configuration is done separately, then rpf_mode[0] will hold IPV4 data, rpf_mode[1] will hold IPv6 data.
         * Otherwise, rpf_mode[0] will be used to set the unified RPF mode configuration.
         */
        if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_separate_ipv4_ipv6_urpf_mode_config))
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RPF_MODE_IPV4, INST_SINGLE,
        ...
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RPF_MODE, INST_SINGLE, (uint32) rpf_mode[0]);
        }
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CS_IN_LIF_VTT_TO_VTT_PROFILE,
        ...
        else
        {
            ...
            if (dnx_data_lif.
                in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles))
            {
                 /** Loop over the native idx mode bits, when mapped_in_lif_profile is used, egress_reserved_nof_bits = 0*/
                max_counter_value = 1 << (egress_reserved_nof_bits + NATIVE_INDEXED_MODE_NOF_BITS);
            }
            else
            {
                 /** Loop over the PMF and native idx mode bits */
                max_counter_value =
                    1 << (pmf_reserved_nof_bits + egress_reserved_nof_bits + NATIVE_INDEXED_MODE_NOF_BITS);
            }
        }
        ...
        if (dnx_data_lif.
            in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles))
        {
            /** Calculate max_counter_value. When mapped_in_lif_profile is used, egress_reserved_nof_bits = 0 */
            max_counter_value = 1 << egress_reserved_nof_bits;
        }
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_VSI_PROFILE_TABLE, &entry_handle_id));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ROUTING_ENABLERS_PROFILE, INST_SINGLE,
    ...
    if (intf_type == ETH_RIF)
    {
        if (dnx_data_lif.
            in_lif.feature_get(unit, dnx_data_lif_in_lif_use_eth_rif_profile_range_allocation_per_pmf_profiles))
        {
            *new_in_lif_profile = (egress_eth_rif_profile << eth_rif_profile_egress_reserved_start_bit) |
                *new_in_lif_profile;
        }
        else
        {
    ...
    else
    {
        ...
        {
            *new_in_lif_profile =
                (egress_in_lif_profile << in_lif_profile_egress_reserved_start_bit) |
                (native_indexed_mode_in_lif_profile << IN_LIF_PROFILE_NATIVE_INDEXED_MODE_START_BIT) |
                *new_in_lif_profile;
        }
        else
        {
    ...
    in_lif_profile_info_t * in_lif_profile_info)
{
     ...
     */
    sal_memset(in_lif_profile_info, 0, sizeof(in_lif_profile_info_t));
    ...
    in_lif_profile_info->ingress_fields.lif_on_lag_is_shown = 0;
    in_lif_profile_info->ingress_fields.oam_primary_vlan_mode = 0;
    in_lif_profile_info->ingress_fields.ip_anti_spoofing_check_mode = 0;
    ...
    in_lif_intf_type_e intf_type)
{
    ...
    dnx_default_frwrd_dst_t default_frwrd_dst[DBAL_NOF_ENUM_DESTINATION_DISTRIBUTION_TYPE_VALUES];
    bcm_l3_ingress_urpf_mode_t rpf_state_to_rpf_enum[DBAL_NOF_ENUM_RPF_MODE_VALUES][DBAL_NOF_ENUM_RPF_MODE_VALUES] =
        { {bcmL3IngressUrpfDisable, bcmL3IngressUrpfV4StrictV6Disable, bcmL3IngressUrpfV4LooseV6Disable},
    {bcmL3IngressUrpfV4DisableV6Strict, bcmL3IngressUrpfStrict, bcmL3IngressUrpfV4LooseV6Strict},
    {bcmL3IngressUrpfV4DisableV6Loose, bcmL3IngressUrpfV4StrictV6Loose, bcmL3IngressUrpfLoose}
    };
    ...
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_separate_ipv4_ipv6_urpf_mode_config))
    {
        if (in_lif_profile_template_data.rpf_mode_ipv4 != DBAL_ENUM_FVAL_RPF_MODE_LOOSE
            && in_lif_profile_template_data.rpf_mode_ipv4 != DBAL_ENUM_FVAL_RPF_MODE_STRICT
            && in_lif_profile_template_data.rpf_mode_ipv4 != DBAL_ENUM_FVAL_RPF_MODE_DISABLE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "IPv4 urpf_mode invalid : %d\n", in_lif_profile_template_data.rpf_mode_ipv4);
        }
        if (in_lif_profile_template_data.rpf_mode_ipv6 != DBAL_ENUM_FVAL_RPF_MODE_LOOSE
            && in_lif_profile_template_data.rpf_mode_ipv6 != DBAL_ENUM_FVAL_RPF_MODE_STRICT
            && in_lif_profile_template_data.rpf_mode_ipv6 != DBAL_ENUM_FVAL_RPF_MODE_DISABLE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "IPv6 urpf_mode invalid : %d\n", in_lif_profile_template_data.rpf_mode_ipv6);
        }
        in_lif_profile_info->ingress_fields.urpf_mode =
            rpf_state_to_rpf_enum[in_lif_profile_template_data.
                                  rpf_mode_ipv6][in_lif_profile_template_data.rpf_mode_ipv4];
    }
    else
    {
        if (in_lif_profile_template_data.rpf_mode_ipv4 != DBAL_ENUM_FVAL_RPF_MODE_LOOSE
        ...
        in_lif_profile_info->ingress_fields.urpf_mode =
            rpf_state_to_rpf_enum[in_lif_profile_template_data.
                                  rpf_mode_ipv4][in_lif_profile_template_data.rpf_mode_ipv4];
    }
    ...
    else
    {
                 ...
                 || inlif_dbal_table_id == DBAL_TABLE_PON_DTC_CLASSIFICATION)
        {
            ...
            /*
             * assign ip anti-spoofing check mode:enable or disable
             */
            uint32 cs_in_lif_profile = 0;
            uint32 lif_on_lag_is_shown = 0;
            dbal_enum_value_field_ip_address_spoofing_check_modes_e ip_anti_spoofing_mode = 0;

            cs_in_lif_profile = in_lif_profile_template_data.cs_vtt_in_lif_profile;
            SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                            (unit, DBAL_FIELD_TYPE_DEF_AC_INLIF_CS_PROFILE, DBAL_FIELD_IP_ADDRESS_SPOOFING_CHECK_MODE,
                             &ip_anti_spoofing_mode, &cs_in_lif_profile));

            in_lif_profile_info->ingress_fields.ip_anti_spoofing_check_mode = ip_anti_spoofing_mode;

            /** lif_on_lag_is_shown */
            SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode
                            (unit, DBAL_FIELD_TYPE_DEF_AC_INLIF_CS_PROFILE, DBAL_FIELD_IS_ON_LAG,
                             &lif_on_lag_is_shown, &cs_in_lif_profile));
            in_lif_profile_info->ingress_fields.lif_on_lag_is_shown = lif_on_lag_is_shown;
        }
    }
    ...
    {
        in_lif_profile_info->field_class_id = in_lif_profile_template_data.field_class_id;
    }
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_use_mapped_in_lif_profile))
    {
    ...
    in_lif_intf_type_e intf_type)
{
    ...
    if (is_ip_tunnel_l2vpn)
    {
        *is_ip_tunnel_l2vpn = FALSE;
    }
    ...
    else if (intf_type == LIF)
    {
        ...
        *result_type = gport_hw_resources.inlif_dbal_result_type;
        ...
        if (is_ip_tunnel_l2vpn != NULL)
        {
            int rv;
            uint32 vsi_value;
            rv = dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_VSI, INST_SINGLE, &vsi_value);
            if (rv == _SHR_E_NONE)
            {
                *is_ip_tunnel_l2vpn = TRUE;
            }
