...
/** all negative numbers represent invalid TM indexes, but can have additional meaning.
 *  The invalid TM without any other meaning has to be the smallest negative number amongst others */
#define INVALID_TM_INDEX_TO_INDICATE_DISABLED_MEMBER                (-1)
#define INVALID_TM_INDEX_BUT_NEED_ALLOCATE_ONE                      (-2)
#define INVALID_TM_INDEX_TO_INDICATE_MEMBER_IS_ONLY_PROTECTING      (-3)
#define INVALID_TM_INDEX                                            (-4)

/** Init member to trunk sw db */
void dnx_trunk_sw_db_member_init(
    int unit,
...
/**
 * \brief - set trunk member info
 *
 * \param [in] unit - unit number
 * \param [in] pool - pool index
 * \param [in] group - group index
 * \param [in] member - member index ( pp index )
 * \param [in] member_info - info to set
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_trunk_sw_db_member_set(
    int unit,
    int pool,
    int group,
    int member,
...
/**
 * \brief - return true is spa ref count is 1 for given member
 *
 * \param [in] unit - unit number
 * \param [in] pool - pool index
 * \param [in] group - group index
 * \param [in] member - member index
 * \param [out] is_last_ref - indication if last ref for spa
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_trunk_sw_db_is_last_spa_ref(
    int unit,
    int pool,
    int group,
    int member,
 ...
 * \param [in] remove_protection - remove protecting instance from member
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_trunk_sw_db_entry_remove(
    int unit,
    int pool,
    int group,
    int member,
...
/**
 * \brief - delete a protecting member from trunk sw db
 *
 * \param [in] unit - unit number
 * \param [in] pool - pool index
 * \param [in] group - group index
 ...
 *
 * \remark
 *   protecting members are managed with a ref counter, actual entry will be deleted from sw db only if the ref counter reaches 0
 * \see
 *   * None
 */
shr_error_e dnx_trunk_sw_db_protecting_member_delete(
    int unit,
    int pool,
    int group,
    int member);
 ...
 * \param [out] last_replication_index - received index of member
 *        with last TM index
 ...
 *
 * \remark
 *   * This function also finds disabled or protecting members
 * \see
 *   * None
 */
