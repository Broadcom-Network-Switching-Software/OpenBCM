 ...
 */
shr_error_e
dnx_trunk_dbal_access_trunk_members_copy(
    int unit,
    int pool,
    int group,
    int member,
    int new_member)
{
    ...
    calculated_index = pool * dnx_data_trunk.parameters.max_nof_members_in_pool_get(unit) +
        group * (dnx_data_trunk.parameters.pool_info_get(unit, pool)->max_nof_members_in_group) + new_member;
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MEMBER_INDEX, calculated_index);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DST_SYS_PORT_ID, INST_SINGLE, mapped_destination);
    if (dnx_data_trunk.protection.feature_get(unit, dnx_data_trunk_protection_backup_destination))
    {
 ...
 */
shr_error_e
dnx_trunk_dbal_access_trunk_protection_members_set(
    int unit,
    int pool,
    int group,
    int member,
 ...
 */
shr_error_e
dnx_trunk_dbal_access_trunk_protection_members_get(
    int unit,
    int pool,
    int group,
    int member,
    uint32 *system_port_id)
{
    ...
    if (!dnx_data_trunk.protection.feature_get(unit, dnx_data_trunk_protection_backup_destination))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Lag protection is not supported for this device\n");
    }
 ...
 */
shr_error_e
dnx_trunk_dbal_access_trunk_protection_members_swap(
    int unit,
    int pool,
    int group,
    int member)
{
    ...
    calculated_index = pool * dnx_data_trunk.parameters.max_nof_members_in_pool_get(unit) +
        group * (dnx_data_trunk.parameters.pool_info_get(unit, pool)->max_nof_members_in_group) + member;
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MEMBER_INDEX, calculated_index);
    /** get backup to mapped and mapped to backup */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_DST_SYS_PORT_ID, INST_SINGLE, &backup_destination);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    /** set backup to backup and mapped to mapped to complete the swap */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DST_SYS_PORT_ID, INST_SINGLE, mapped_destination);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    uint8 enable)
{
    ...
    if (dnx_data_trunk.flow_agg.feature_get(unit, dnx_data_trunk_flow_agg_member_resolution_v1))
    {
        /** Don't use new member resolution for flow aggregation */
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_NEW_MEMBER_RESOLUTION, INST_SINGLE, 0);
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
