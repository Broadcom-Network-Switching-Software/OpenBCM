...
/** see header file */
shr_error_e
dnx_algo_consistent_hashing_manager_verify_sufficient_resources(
    int unit,
    uint32 chm_handle,
    int profile_type,
    uint32 hierarchy,
    uint32 nof_profiles)
{
    ...
    /** Get CBs */
    SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.cbs.get(unit, chm_handle, &cbs));
    /** Get free nof free resources */
    SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                    def_chunk.get_free_slots(unit, chm_handle, hierarchy, &free_resources));
    /** Calculate needed size from chunk */
    ...
    /** Check against needed size */
    if (piece_alloc_size > free_resources)
    {
        SHR_ERR_EXIT(_SHR_E_FULL, "hierarchy cannot currently contain %d new profiles of type %d\n", nof_profiles,
                     profile_type);
    }
