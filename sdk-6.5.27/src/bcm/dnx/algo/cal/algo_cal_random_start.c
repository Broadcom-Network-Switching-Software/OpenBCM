...
dnx_algo_cal_min_distance_transition_check(
    ...
    int *passed)
{
    int slot_index, meeting_limit_counter = 0;
    SHR_FUNC_INIT_VARS(unit);

    *passed = TRUE;
    for (slot_index = 0; slot_index < limit_value; ++slot_index)
    {
        if (calendar[slot_index] != DNX_ALGO_CAL_ILLEGAL_OBJECT_ID
            && _SHR_PBMP_MEMBER(objects_to_check, calendar[slot_index])
            && slot_index <=
            limit_value - (calendar_length - active_calendar_last_occurrences[calendar[slot_index]][0]))
        {
    ...
    if (meeting_limit_counter > max_occurrences)
    {
        *passed = FALSE;
    }
    else
    ...
    int *passed)
{
    ...
    int slot_index, new_calendar_slots_index = 0, distance;
    ...
    *passed = TRUE;
    ...
    for (slot_index = 0; slot_index < calendar_length; ++slot_index)
    {
        ...
        {
            new_slots[new_calendar_slots_index] = slot_index;
            ++new_calendar_slots_index;
            if (new_calendar_slots_index == level)
            {
                break;
            }
        }
    ...
    {
        /** new_slots & active_calendar_last_occurrences have opposite directions and therefore opposite indexes for checking distance */
        distance = new_slots[slot_index] - active_calendar_last_occurrences[level - 1 - slot_index] + calendar_length;
        if (distance < trunc((level - deviation) * avrg_distance)
            || distance > ceil((level + deviation) * avrg_distance))
        {
            *passed = FALSE;
...
shr_error_e
dnx_algo_cal_random_start_build_impl(
    int unit,
    ...
    uint32 *calendar)
{
    ...
    int free_index;
    int swap_slot_index;
    UTILEX_RAND *rand_state = &(info->rand_state);
    ...
    /** Sort objects with compare_cb */
    _shr_sort(slots_per_object, nof_objects, sizeof(*slots_per_object),
              info->compare_cb ? info->compare_cb : &dnx_algo_cal_random_start_cmp);
    ...
    {
        obj_slots_nof = slots_per_object[obj_index].nof_slots;
        if (obj_slots_nof == 0)
        {
            break;
        }
        distance = calendar_len * 1.0 / slots_per_object[obj_index].nof_slots;
        if (trans_info && trans_info[slots_per_object[obj_index].id].slot_info[0].valid)
        {
            ...
            for (i = 0; i < free_slots_total && calendar_free_slots_p[i] <
                 trans_info[slots_per_object[obj_index].id].slot_info[0].min_first_slot; i++)
            {
            }
            start_index = i;
            for (i = start_index; i < free_slots_total && calendar_free_slots_p[i] <
                 trans_info[slots_per_object[obj_index].id].slot_info[0].max_first_slot; i++)
            {
            }
            end_index = i - 1;
        ...
        for (i = 0; i < obj_slots_nof; i++)
        {
            ...
            calendar_temp_p[curr_slot] = slots_per_object[obj_index].id;
    ...
    uint32 *calendar)
{
    ...
    for (i = 0; i < nof_objects; i++)
    {
        ...
        slots_per_object_temp[i].orig_id = i;
        slots_per_object_temp[i].nof_slots = slots_per_object[i];
        slots_per_object_temp[i].orig_nof_slots = slots_per_object[i];
    }

    SHR_IF_ERR_EXIT(dnx_algo_cal_random_start_build_impl(unit, slots_per_object_temp, nof_objects,
                                                         calendar_len, max_calendar_len, info, trans_info, calendar));
...
dnx_algo_cal_random_start_build_cb(
    ...
    uint32 *calendar)
{
    dnx_algo_cal_random_start_info_t *info = (dnx_algo_cal_random_start_info_t *) additional_info;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_algo_cal_random_start_build_impl(unit, slots_per_object, nof_objects,
                                                         calendar_len, max_calendar_len, info, NULL, calendar));
exit:
    SHR_FUNC_EXIT;
}
    ...
    for (i = 0; i < iterations; i++)
    {
        ...
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_cal_random_start_build(unit, slots_per_object, nof_objects, calendar_len,
                                                            max_calendar_len, &info, NULL, calendar));
        }
