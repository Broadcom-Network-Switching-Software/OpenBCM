    ...
    dnx_algo_gpm_gport_phy_info_t * gport_info)
{
    ...
    bcm_port_t pp_dsp = SOC_MAX_NUM_PORTS;
    int queue_id = -1;
    int trunk_id;
    int core_id;
    ...
    else if (BCM_GPORT_IS_MODPORT(gport))
    {
        ...
        if (is_local)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_pp_dsp_to_logical_get(unit, core_id, pp_dsp, &gport_info->local_port));
            /*
             * Get PP port
             */
            SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, gport_info->local_port, &port_info));
    ...
    else if (BCM_GPORT_IS_SYSTEM_PORT(gport))
    {
        ...
        if (is_local)
        {
             ...
             * Get PP port
             */
            if (trunk_db.is_init(unit, &trunk_is_init))
            {
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_fabric_logical_port_get(unit, gport, &gport_info->local_port));
        gport_info->flags |= DNX_ALGO_GPM_GPORT_INFO_F_IS_LOCAL_PORT;
    }
...
/**
 * \brief
 *   Given a flow gport, returns related global HW resources -
 *   Global-LIF or Virtual-ID. Both ingress and egress gports
 *   are returned.
 * \param [in] unit -
 ...
 *****************************************************/
static void
dnx_algo_gpm_flow_gport_to_global_resources(
    int unit,
...
{
    int flow_gport_subtype = BCM_GPORT_SUB_TYPE_GET(flow_gport);

    /** Consider defining a general sub_type macro to improve efficiency instead of calling
     *  BCM_GPORT_SUB_TYPE_xxx_GET per sub_type */
    switch (flow_gport_subtype)
    {
        case BCM_GPORT_SUB_TYPE_IN_LIF:
            *global_in_lif_id = BCM_GPORT_SUB_TYPE_IN_LIF_VAL_GET(flow_gport);
            break;
        case BCM_GPORT_SUB_TYPE_OUT_LIF:
            *global_out_lif_id = _SHR_GPORT_SUB_TYPE_OUT_LIF_VAL_GET(flow_gport);
            break;
        case BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED:
            *egr_pointed_p = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_GET(flow_gport);
            break;
        default:
            *global_in_lif_id = *global_out_lif_id = _SHR_GPORT_FLOW_LIF_VAL_GET_ID(flow_gport);
    ...
    dnx_egr_pointed_t * egr_pointed_p)
{
    ...
    *egr_pointed_p = DNX_ALGO_EGR_POINTED_INVALID;
    ...
    else if (BCM_GPORT_IS_FLOW_LIF(gport))
    {
        dnx_algo_gpm_flow_gport_to_global_resources(unit, gport, global_in_lif_id, global_out_lif_id, egr_pointed_p);
    ...
    uint32 entry_handle_id)
{
    ...
    {
        field_id = DBAL_FIELD_ESEM_NAME_SPACE;
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
         && BCM_GPORT_IS_FLOW_LIF(gport)))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "API not supported when using FLOW\n");
    }
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    if (BCM_GPORT_IS_FLOW_LIF(gport))
    {
        if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(gport))
        {
            SHR_IF_ERR_EXIT(dnx_flow_egress_virtual_sw_info_get
                            (unit, gport, &gport_hw_resources->outlif_dbal_table_id,
                             &gport_hw_resources->outlif_dbal_result_type));
        }
        else if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(gport))
        {
            ...
            gport_hw_resources->outlif_dbal_result_type = DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_AC_STAT;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected scenario, virtual gport with not virtual subtype\n");
        }
    }
    else if (BCM_GPORT_IS_VLAN_PORT(gport))
    {
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    {
        if (gport_hw_resources->local_in_lif_per_core.nof_in_lifs == 0)
        {
...
static shr_error_e
algo_gpm_dbal_in_lif_dpc_physical_table_get(
    int unit,
    dbal_physical_tables_e * physical_table_id)
{
    SHR_FUNC_INIT_VARS(unit);
...
/* Set local inlif per core */
static shr_error_e
dnx_algo_gpm_local_inlifs_get(
    int unit,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources,
    lif_mapping_local_lif_payload_t * local_lif_payload,
    int global_in_lif_dummy,
    int *local_in_lif,
    dnx_algo_gpm_local_in_lif_per_core_t * local_in_lif_per_core)
{
    int rv;

    SHR_FUNC_INIT_VARS(unit);

    rv = dnx_algo_lif_mapping_global_to_local_get(unit, DNX_ALGO_LIF_INGRESS, global_in_lif_dummy, local_lif_payload);
    ...
    if ((local_lif_payload->is_symmetric) || (!dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc)))
    {
        local_in_lif_per_core->core_id[0] = _SHR_CORE_ALL;
        local_in_lif_per_core->local_in_lif[0] = local_lif_payload->local_lifs_array.local_lifs[0];
        local_in_lif_per_core->nof_in_lifs = dnx_data_device.general.nof_cores_get(unit);

        SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                        (unit, local_in_lif_per_core->local_in_lif[0], _SHR_CORE_ALL,
                         local_lif_payload->phy_table,
                         &gport_hw_resources->inlif_dbal_table_id, &gport_hw_resources->inlif_dbal_result_type));
        *local_in_lif = local_in_lif_per_core->local_in_lif[0];
    }
    else        /* Non symmetric */
    {
        int current_core;
        uint8 valid_core_found = FALSE;

        local_in_lif_per_core->nof_in_lifs = 0;

        *local_in_lif = DNX_ALGO_GPM_LIF_INVALID;

        sal_memset(&local_in_lif_per_core->core_id, DNX_ALGO_GPM_CORE_INVALID, sizeof(local_in_lif_per_core->core_id));

        DNXCMN_CORES_ITER(unit, _SHR_CORE_ALL, current_core)
        {
            if (local_lif_payload->local_lifs_array.local_lifs[current_core] != LIF_MNGR_INLIF_INVALID)
            {
                local_in_lif_per_core->local_in_lif[local_in_lif_per_core->nof_in_lifs] =
                    local_lif_payload->local_lifs_array.local_lifs[current_core];
                local_in_lif_per_core->core_id[local_in_lif_per_core->nof_in_lifs] = current_core;
                if (!valid_core_found)
                {
                    SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                                    (unit, local_in_lif_per_core->local_in_lif[local_in_lif_per_core->nof_in_lifs],
                                     current_core, local_lif_payload->phy_table,
                                     &gport_hw_resources->inlif_dbal_table_id,
                                     &gport_hw_resources->inlif_dbal_result_type));
                    valid_core_found = TRUE;
                }
                local_in_lif_per_core->nof_in_lifs++;
            }
        }
    }
...
/**
 * \brief -
 * verify function to hw resources get function
 *
 * \param [in] unit - the relevant unit.
 * \param [in] gport - gport
 * \param [in] flags - additional flags of request
 * \param [out] gport_hw_resources - structure to fill
 * \return
 *   shr_error_e
 */
static shr_error_e
dnx_algo_gpm_gport_to_hw_resources_verify(
    int unit,
    bcm_gport_t gport,
    uint32 flags,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
     ...
     */
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS) &&
        (BCM_GPORT_IS_TUNNEL(gport) || BCM_GPORT_IS_FLOW_LIF(gport)) &&
        BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_POINTED(gport))
    {
...
/**
 *  \brief
 *  Init gport_hw_resources params
 */
static void
dnx_algo_gpm_gport_hw_resources_t_init(
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    /*
     * Init values
    ...
    gport_hw_resources->local_in_lif_per_core.nof_in_lifs = 0;
    sal_memset(gport_hw_resources->local_in_lif_per_core.core_id, DNX_ALGO_GPM_CORE_INVALID,
               sizeof(int) * DNX_DATA_MAX_DEVICE_GENERAL_NOF_CORES);
    ...
    gport_hw_resources->virtual_id = DNX_ALGO_GPM_LIF_INVALID;
}

/**
 *  \brief
 *  Update gport_hw_resources params in case of egress lif
 */
static shr_error_e
dnx_algo_gpm_hw_resources_local_lif_egress_get(
    int unit,
    bcm_gport_t gport,
    int global_out_lif_dummy,
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    if (egr_pointed_id != DNX_ALGO_EGR_POINTED_INVALID)
    {
        ...
        if (BCM_GPORT_IS_FLOW_LIF(gport))
        {
            SHR_IF_ERR_EXIT_EXCEPT_IF(dnx_flow_egr_pointed_virtual_local_lif_get
                                      (unit, egr_pointed_id, &gport_hw_resources->local_out_lif), _SHR_E_NOT_FOUND);
        }
        else
        {
            SHR_IF_ERR_EXIT_EXCEPT_IF(dnx_algo_l3_egr_pointed_to_local_lif_get
                                      (unit, egr_pointed_flags, egr_pointed_id, &gport_hw_resources->local_out_lif),
                                      _SHR_E_NOT_FOUND);
        }

        if (gport_hw_resources->local_out_lif != DNX_ALGO_GPM_LIF_INVALID)
        {
            SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get(unit, gport_hw_resources->local_out_lif,
                                                            &gport_hw_resources->outlif_dbal_table_id,
                                                            &gport_hw_resources->outlif_dbal_result_type,
                                                            &gport_hw_resources->logical_phase, NULL, NULL, NULL));
        }
    }
    else if (global_out_lif_dummy != DNX_ALGO_GPM_LIF_INVALID)
    {
        ...
        SHR_IF_ERR_EXIT_EXCEPT_IF(dnx_algo_lif_mapping_global_to_local_get
                                  (unit, DNX_ALGO_LIF_EGRESS, global_out_lif_dummy, &local_lif_payload),
                                  _SHR_E_NOT_FOUND);
        gport_hw_resources->local_out_lif = local_lif_payload.local_lifs_array.local_lifs[0];
        if (gport_hw_resources->local_out_lif != DNX_ALGO_GPM_LIF_INVALID)
        {
            SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get(unit, gport_hw_resources->local_out_lif,
                                                            &gport_hw_resources->outlif_dbal_table_id,
...
/**
 *  \brief
 *  Update gport_hw_resources params in case of ingress lif
 */
static shr_error_e
dnx_algo_gpm_hw_resources_local_lif_ingress_get(
    int unit,
    bcm_gport_t gport,
    int global_in_lif_dummy,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    {
        core_index_iterate = BCM_CORE_ALL;
    }

    if ((BCM_GPORT_IS_VLAN_PORT(gport) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS_NATIVE(gport)) ||
        (BCM_GPORT_IS_FLOW_LIF(gport) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS(gport)))
    {
        /*
         * In case of VIRTUAL GPORT, there is no global lif but local lif can be retrieved from SW state.
         */
        /** get the local lif for each core separately, and also update the legacy common local_in_lif param */
        /** if local lif was not found in any of the cores, error will be returned at the end */
        DNXCMN_CORES_ITER(unit, core_index_iterate, core_idx)
        {
            int *nof_in_lifs = &(gport_hw_resources->local_in_lif_per_core.nof_in_lifs);

            if (BCM_GPORT_IS_VLAN_PORT(gport))
            {
                rv = dnx_vlan_port_ingress_virtual_native_ac_local_lif_get(unit, core_idx, gport,
                                                                           (uint32 *)
                                                                           &gport_hw_resources->
                                                                           local_in_lif_per_core.local_in_lif
                                                                           [*nof_in_lifs]);
            }
            else
            {
                /** Flow gport */
                rv = dnx_flow_ingress_virtual_local_lif_get(unit,
                                                            BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_GET(gport),
                                                            core_idx,
                                                            &(gport_hw_resources->
                                                              local_in_lif_per_core.local_in_lif[*nof_in_lifs]));
            }
            if (rv != _SHR_E_NOT_FOUND)
            {
                SHR_IF_ERR_EXIT(rv);
                /** if it is the first core found, update the table_id and result_type */
                if (*nof_in_lifs == 0)
                {
                    uint8 is_symmetric_mode = TRUE;

                    SHR_IF_ERR_EXIT(algo_gpm_dbal_in_lif_dpc_physical_table_get(unit, &phy_table));
                    SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                                    (unit, gport_hw_resources->local_in_lif_per_core.local_in_lif[*nof_in_lifs],
                                     core_idx, phy_table, &gport_hw_resources->inlif_dbal_table_id,
                                     &gport_hw_resources->inlif_dbal_result_type));
                    gport_hw_resources->local_in_lif =
                        gport_hw_resources->local_in_lif_per_core.local_in_lif[*nof_in_lifs];

                    /** In case the allocation isn't per core, no need to traverse all the cores */
                    if (is_symmetric_mode)
                    {
                        gport_hw_resources->local_in_lif_per_core.core_id[0] = _SHR_CORE_ALL;
                        gport_hw_resources->local_in_lif_per_core.nof_in_lifs =
                            dnx_data_device.general.nof_cores_get(unit);
                        break;
                    }
                }

                if (gport_hw_resources->local_in_lif !=
                    gport_hw_resources->local_in_lif_per_core.local_in_lif[*nof_in_lifs])
                {
                    gport_hw_resources->local_in_lif = DNX_ALGO_GPM_LIF_INVALID;
                }

                gport_hw_resources->local_in_lif_per_core.core_id[*nof_in_lifs] = core_idx;
                (*nof_in_lifs)++;
            }
        }
    }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_gpm_local_inlifs_get
                        (unit, gport_hw_resources, &local_lif_payload, global_in_lif_dummy,
                         &gport_hw_resources->local_in_lif, &gport_hw_resources->local_in_lif_per_core));
    }
exit:
    SHR_FUNC_EXIT;
}
...
/**
 *  \brief
 *  Update virtual_id param in case gport sub type is virtual
 */
static shr_error_e
dnx_algo_gpm_gport_hw_resources_virtual_id_set(
    int unit,
    bcm_gport_t gport,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    if (BCM_GPORT_IS_FLOW_LIF(gport))
    {
        ...
        {
            case BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS:
                gport_hw_resources->virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_GET(gport);
                break;
            case BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH:
                gport_hw_resources->virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_GET(gport);
                break;
            case BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT:
                gport_hw_resources->virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_GET(gport);
                break;
            case BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED:
                gport_hw_resources->virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_GET(gport);
                break;
            default:
            /** If not a virtual gport - Do nothing */
                break;
        }
    }
    else
    {
        switch (gport_subtype)
        {
            case BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE:
                gport_hw_resources->virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE_GET(gport);
                break;
            case BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NON_NATIVE:
                gport_hw_resources->virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NON_NATIVE_GET(gport);
                break;
            case BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH:
                gport_hw_resources->virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_GET(gport);
                break;
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_gpm_gport_to_hw_resources_verify(unit, gport, flags, gport_hw_resources));
    /******** init function */
    dnx_algo_gpm_gport_hw_resources_t_init(gport_hw_resources);

    if ((BCM_GPORT_IS_VLAN_PORT(gport) || BCM_GPORT_IS_FLOW_LIF(gport)) &&
    ...
    else
    {
        ...
        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS))
        {
            gport_hw_resources->global_in_lif = global_in_lif_dummy;
        }
        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS))
        {
            gport_hw_resources->global_out_lif = global_out_lif_dummy;
        }
        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_FEC))
        {
            gport_hw_resources->fec_id = fec_dummy;
        }
        ...
        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS))
        {
            SHR_IF_ERR_EXIT(dnx_algo_gpm_hw_resources_local_lif_ingress_get
                            (unit, gport, global_in_lif_dummy, gport_hw_resources));
        }

        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS))
        {
            SHR_IF_ERR_EXIT(dnx_algo_gpm_hw_resources_local_lif_egress_get
                            (unit, gport, global_out_lif_dummy, egr_pointed_id, gport_hw_resources));
        }
    }

    /** update virtual id */
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_hw_resources_virtual_id_set(unit, gport, gport_hw_resources));
    ...
    dnx_algo_gpm_rif_hw_resources_t * rif_hw_resources)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.rif_alloc.rif_res_manager.is_allocated(unit, intf, &is_allocated));
    if (!is_allocated)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "RIF (0x%x) doesn't exist\n", intf);
    }

    rif_hw_resources->outlif_dbal_table_id = DBAL_TABLE_EEDB_RIF_BASIC;
    SHR_IF_ERR_EXIT(algo_l3_db.
                    out_rif_result_type_info.get(unit, intf, (int *) &rif_hw_resources->outlif_dbal_result_type));
    ...
    bcm_gport_t * gport)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "device support only IN_LIF DPC, but flag DNX_ALGO_GPM_GPORT_HW_RESOURCES_SBC_LOCAL_LIF_INGRESS is set\n");
    }
    else if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_SBC_LOCAL_LIF_INGRESS))
    {
    ...
    bcm_gport_t * gport)
{
    ...
    lif_mapping_local_lif_payload_t local_lif_payload;
    dbal_physical_tables_e dbal_physical_table = DBAL_PHYSICAL_TABLE_NONE;
     ...
     */
    egr_pointed_id = 0;
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_gpm_gport_from_lif_verify(unit, flags, core_id, lif, gport));

    sal_memset(&local_lif_info, 0, sizeof(local_lif_info));
    sal_memset(&local_lif_payload, 0, sizeof(lif_mapping_local_lif_payload_t));
    ...
    if (is_global)
    {
        ...
        shr_error = dnx_algo_lif_mapping_global_to_local_get(unit, direction_flags, global_lif, &local_lif_payload);
        ...
        local_lif = local_lif_payload.local_lifs_array.local_lifs[0];
    }
    else
    {
        /*
    ...
    if (is_ingress)
    {
        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_DPC_LOCAL_LIF_INGRESS))
        {
            SHR_IF_ERR_EXIT(algo_gpm_dbal_in_lif_dpc_physical_table_get(unit, &dbal_physical_table));
        }
        else if ((_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_SBC_LOCAL_LIF_INGRESS)))
        {
            {
        ...
        else
        {
             ...
             */
            dbal_physical_table = local_lif_payload.phy_table;
        }
    }
    else
    {
    ...
    if (is_local)
    {
        ...
        if (is_ingress)
        {
            ...
            local_lif_info.core_id = (core_id == _SHR_CORE_ALL ? DBAL_CORE_DEFAULT : core_id);
    ...
    if (is_egress && (shr_error == _SHR_E_NONE))
    {
        switch (dbal_table_id)
        {
            ...
            case DBAL_TABLE_EEDB_SRV6_DATA_SID:
            case DBAL_TABLE_EEDB_SFLOW_DATA:
            case DBAL_TABLE_EEDB_DATA_IPFIX_CODE:
            case DBAL_TABLE_EEDB_IPV6_DATA_DIP:
            ...
            case DBAL_TABLE_EEDB_TWAMP:
            case DBAL_TABLE_EEDB_OAM_REFLECTOR:
            case DBAL_TABLE_EEDB_PPPOE:
            case DBAL_TABLE_EEDB_L2TP:
            {
                /*
                 * Tunnel
                 */
            ...
            case DBAL_TABLE_EEDB_BIER:
            case DBAL_TABLE_EEDB_BIER_DATA_BFR_BITSTR:
            ...
            case DBAL_TABLE_IN_AC_INFO_DB:
            {
                ...
                if (is_symmetric && dbal_table_id == DBAL_TABLE_IN_AC_INFO_DB)
                {
                    ...
                    if (hw_res.outlif_dbal_result_type == DBAL_RESULT_TYPE_EEDB_OUT_AC_ETPS_AC_TRIPLE_TAG)
                    {
                        ...
                        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                        (unit, entry_handle_id, DBAL_FIELD_SUB_TYPE_FORMAT, INST_SINGLE,
                                         &sub_type_format));
                        /*
                         * For PON port,the port is set as AC lif
                         */
                        ...
                        /*
                         * For Extender port,the port is set as Extender lif
                         */
                        BCM_GPORT_SUB_TYPE_LIF_SET(gport_id_dummy, 0, global_lif);
                        BCM_GPORT_EXTENDER_PORT_ID_SET(*gport, gport_id_dummy);
                    }
 ...
 */
bcm_gport_t
dnx_algo_gpm_flow_gport_encode(
    int unit,
    uint32 global_lif_or_virtual_id,
    uint32 is_terminator,
    uint32 is_virtual,
    uint32 is_local_lif,
    uint32 is_esem_default)
{
    bcm_gport_t flow_gport;

    /** A Terminator application flow gport - Virtual or Not */
    if (is_terminator)
    {
        if (is_virtual)
        {
            BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_SET(flow_gport, global_lif_or_virtual_id);
        }
        else
        {
            BCM_GPORT_SUB_TYPE_IN_LIF_SET(flow_gport, global_lif_or_virtual_id);
        }
    }
    /** An Initiator application flow gport */
    else
    {
        /** An ESEM type flow gport */
        if (!is_local_lif)
        {
            if (is_esem_default)
            {
                BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_SET(flow_gport, global_lif_or_virtual_id);
            }
            else
            {
                BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_SET(flow_gport, global_lif_or_virtual_id);
            }
        }
        /** Virtual but not ESEM is Egress Pointed */
        else if (is_virtual)
        {
            BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_SET(flow_gport, global_lif_or_virtual_id);
        }
        /** Non Virtual */
        else
        {
            BCM_GPORT_SUB_TYPE_OUT_LIF_SET(flow_gport, global_lif_or_virtual_id);
        }
    }

    /** Encode the flow gport type and return */
    BCM_GPORT_FLOW_LIF_ID_SET(flow_gport, flow_gport);
    return flow_gport;
}
 ...
 */
shr_error_e
dnx_algo_gpm_flow_gport_from_local_lif(
    int unit,
    int local_lif,
    int is_ingress,
    int core_id,
    bcm_gport_t * flow_gport)
{
    uint32 is_virtual = FALSE;
    int global_lif_or_virtual_id, rv;
    lif_mapping_local_lif_key_t local_lif_info;

    SHR_FUNC_INIT_VARS(unit);

    *flow_gport = BCM_GPORT_INVALID;

    /** Fill the local_lif_info struct */
    sal_memset(&local_lif_info, 0, sizeof(local_lif_info));
    local_lif_info.local_lif = local_lif;
    ...
    if (is_ingress)
    {
        if (!dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
        {
            ...
            /** Retrieve the DBAL table in order to get the Physical table */
            SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                            (unit, local_lif, local_lif_info.core_id, 0, &dbal_table_id, &dbal_result_type));
    ...
    /** Retrieve the Global-LIF or Virtual-ID that are mapped to the Local-LIF */
    if (is_ingress)
    {
        rv = dnx_algo_lif_mapping_local_to_global_get(unit, DNX_ALGO_LIF_INGRESS, &local_lif_info,
                                                      &global_lif_or_virtual_id);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
        if (rv == _SHR_E_NOT_FOUND)
        {
            SHR_IF_ERR_EXIT(dnx_flow_ingress_virtual_gport_get
                            (unit, local_lif, local_lif_info.core_id, &global_lif_or_virtual_id));
            is_virtual = TRUE;
        }
    }
    else
    {
        rv = dnx_algo_lif_mapping_local_to_global_get(unit, DNX_ALGO_LIF_EGRESS, &local_lif_info,
                                                      &global_lif_or_virtual_id);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
        if (rv == _SHR_E_NOT_FOUND)
        {
            SHR_IF_ERR_EXIT(dnx_flow_egr_pointed_virtual_gport_get
                            (unit, local_lif, (uint32 *) &global_lif_or_virtual_id));
            is_virtual = TRUE;
        }
    }

    /** Retrieve the encoded flow gport based on the Global-LIF (or Virtual-ID) */
    *flow_gport = dnx_algo_gpm_flow_gport_encode(unit, global_lif_or_virtual_id, is_ingress, is_virtual, TRUE, FALSE);
    ...
    uint32 *destination)
{
    ...
    if (BCM_GPORT_IS_TRUNK(port))
    {
        if (dnx_data_trunk.egress_trunk.feature_get(unit, dnx_data_trunk_egress_trunk_is_supported))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_trunk_egress_get
                            (unit, BCM_GPORT_TRUNK_GET(port), &egress_trunk_id, &profile_found));
            if (!profile_found)
            {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                            (unit, egress_destination_field[ext_mode], dsp_agg_field[ext_mode],
                             (uint32 *) &egress_trunk_id, destination));
        }
        else
        {
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_pp_dsp_get(unit, gport_info.local_port, (int *) &core_id, &pp_dsp));
        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                        (unit, egress_destination_field[ext_mode], dsp_field[ext_mode], &pp_dsp, destination));
    }
    ...
    uint8 *gport_is_valid_destination)
{
    ...
    *gport_is_valid_destination = TRUE;
    ...
    {
        /** gport is flow aggregate, encode destination as flow aggregate (type of flow ID) */
        uint32 flow_agg_id;
        uint32 base_flow_id;
        flow_agg_id = BCM_GPORT_FLOW_AGG_GET(gport);

        /** transform flow_agg_id to flow_id */
        base_flow_id = flow_agg_id + dnx_data_trunk.flow_agg.first_flow_agg_id_get(unit);
        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set(unit, DBAL_FIELD_DESTINATION, DBAL_FIELD_FLOW_ID,
                                                             &base_flow_id, destination));
    }
    ...
    {
        uint32 flow_id = 0;
        if (!_SHR_IS_FLAG_SET(flags, ALGO_GPM_ENCODE_DESTINATION_ASYM_VOQ))
        {
            bcm_core_t core_id = 0;
            core_id = BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_GET(gport);
            if (core_id != BCM_CORE_ALL)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "The core in gport (0x%08X) should be set to BCM_CORE_ALL!", gport);
            }
        }
        /*
         * GPORT type is UNICAST GROUP (CosQ/Flow), Encode destination as flow id
         */
        flow_id = BCM_GPORT_UNICAST_QUEUE_GROUP_QID_GET(gport);
        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set(unit, DBAL_FIELD_DESTINATION, DBAL_FIELD_FLOW_ID,
                                                             &flow_id, destination));
    }
...
shr_error_e
algo_gpm_is_gport_valid_destination(
    int unit,
    bcm_gport_t port,
    uint8 *gport_is_valid_destination)
{
    ...
    *gport_is_valid_destination = TRUE;
    ...
    if (BCM_GPORT_IS_BLACK_HOLE(gport) || BCM_GPORT_IS_SYSTEM_PORT(gport) || BCM_GPORT_IS_LOCAL(gport) ||
        BCM_GPORT_IS_LOCAL_CPU(gport) || BCM_GPORT_IS_MODPORT(gport) || BCM_GPORT_IS_TRUNK(gport) ||
        BCM_GPORT_IS_FLOW_AGG(gport) || BCM_GPORT_IS_MCAST(gport) || BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport) ||
        BCM_GPORT_IS_MCAST_QUEUE_GROUP(gport) || BCM_GPORT_IS_TRAP(gport) || BCM_GPORT_IS_FORWARD_PORT(gport))
    {
        *gport_is_valid_destination = TRUE;
    }
    else
    ...
    uint32 *destination)
{
    ...
    else
    {
        SHR_IF_ERR_EXIT(algo_gpm_encode_destination_field_from_gport_internal
                        (unit, port, flags, destination, &gport_is_valid_destination));
        if (!gport_is_valid_destination)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "port(0X%08x) is not a valid destination gport.\r\n", port);
        }
    }
    ...
    else
    {
        ...
        else
        {
            ...
            else if (dbal_dest_type == DBAL_FIELD_FLOW_ID)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ipq_queue_is_multicast(unit, dest_val, &is_multicast));
                /*
                 * Flow ID
                 */
                if (is_multicast)
                {
                    BCM_GPORT_MCAST_QUEUE_GROUP_SET(*gport, dest_val);
                }
                else
                {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Can not retrieve port type, destination = 0x%08x\n", destination);
            }
        }
    }
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
     ...
     */
    SHR_IF_ERR_EXIT(algo_gpm_encode_destination_field_from_gport_internal
                    (unit, gport, flags, &forward_info->destination, &gport_is_valid_destination));
    /*
...
algo_gpm_gport_l2_forward_info_add(
    int unit,
    uint8 is_replace,
    uint8 is_replace_type_change_allowed,
    bcm_gport_t gport,
    dnx_algo_gpm_forward_info_t * forward_info)
{
    uint32 entry_handle_id;
    ...
    /*
     * Verify:
     * If replacing an existing gport
     */
    if (is_replace)
    {
        /*
        ...
        SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_get(unit, gport, 0, &old_forward_info));

        /*
         * learn information type change or destination change is allowed
         */
        if (is_replace_type_change_allowed)
        {
            if (sal_memcmp(&old_forward_info, forward_info, sizeof(dnx_algo_gpm_forward_info_t)) != 0)
            {
                /*
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_L2_GPORT_TO_FORWARDING_SW_INFO, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_L2_LOGICAL_GPORT, gport);
    if (_SHR_IS_FLAG_SET(flags, ALGO_GPM_GPORT_L2_FWD_INFO_FLAGS_NO_ERR_MSG_NOT_FOUND))
    {
    ...
    uint8 *is_gport_found)
{
    ...
    *is_gport_found = (rv == _SHR_E_NOT_FOUND) ? FALSE : TRUE;
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
    ...
    bcm_port_t * port)
{
    ...
    else if (BCM_COSQ_GPORT_IS_PORT_TC_SCHED(gport))
    {
        *port = BCM_COSQ_GPORT_PORT_TC_SCHED_GET(gport);
    }
    else if (BCM_GPORT_IS_MODPORT(gport))
    {
        ftmh_pp_dsp = BCM_GPORT_MODPORT_PORT_GET(gport);
    ...
    bcm_core_t * core)
{
    ...
    else
    {
        ...
        {
            dnx_algo_gpm_gport_phy_info_t gport_info;
            SHR_IF_ERR_EXIT_WITH_LOG(dnx_algo_gpm_gport_phy_info_get
                                     (unit, gport, DNX_ALGO_GPM_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info),
                                     "System port has to be local to extract a core out of it%s%s%s", EMPTY, EMPTY,
                                     EMPTY);
            gport = gport_info.local_port;
        }
    ...
    algo_gpm_gport_verify_type_e * allowed_types)
{
    ...
    {
        gport_info = ALGO_GPM_GPORT_VERIFY_TYPE_FLOW_AGG;
    }
    else if (BCM_GPORT_IS_SYSTEM_PORT(gport))
    {
        uint32 system_port;
...
static shr_error_e
algo_gpm_gport_to_flow_gport_convert(
    int unit,
    bcm_gport_t in_gport,
    uint32 conversion_flags,
    bcm_gport_t * flow_gport)
{
    int global_lif, gport_subtype, virtual_id, exc_bits;
    SHR_FUNC_INIT_VARS(unit);

    if (BCM_GPORT_IS_FLOW_LIF(in_gport))
    {
        *flow_gport = in_gport;
    }
    else
    {
        if (BCM_GPORT_IS_VLAN_PORT(in_gport))
        {
            gport_subtype = BCM_GPORT_SUB_TYPE_GET(in_gport);

            if (gport_subtype == BCM_GPORT_SUB_TYPE_LIF)
            {
                global_lif = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(in_gport);
                exc_bits = BCM_GPORT_SUB_TYPE_LIF_EXC_GET(in_gport);
                if (exc_bits == BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY)
                {
                    BCM_GPORT_SUB_TYPE_IN_LIF_SET(*flow_gport, global_lif);
                }
                else if (_SHR_IS_FLAG_SET(conversion_flags, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR))
                {
                    BCM_GPORT_SUB_TYPE_IN_LIF_SET(*flow_gport, global_lif);
                }
                else if ((exc_bits == BCM_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY) ||
                         (_SHR_IS_FLAG_SET(conversion_flags, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR)))
                {
                    BCM_GPORT_SUB_TYPE_OUT_LIF_SET(*flow_gport, global_lif);
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "algo_gpm_gport_to_flow_gport_convert error from a VLAN-Port symmetric gport (0x%x) with no flow side flag\n",
                                 in_gport);
                }
            }
            else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE)
            {
                virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE_GET(in_gport);
                BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_SET(*flow_gport, virtual_id);
            }
            else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NON_NATIVE)
            {
                virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NON_NATIVE_GET(in_gport);
                BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_SET(*flow_gport, virtual_id);
            }
            else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH)
            {
                virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_GET(in_gport);
                BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_SET(*flow_gport, virtual_id);
            }
            else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT)
            {
                virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_GET(in_gport);
                BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_SET(*flow_gport, virtual_id);
            }
            else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED)
            {
                virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_GET(in_gport);
                BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_SET(*flow_gport, virtual_id);
            }
            else if (gport_subtype == BCM_GPORT_SUB_TYPE_VLAN_TRANSLATION)
            {
                global_lif = BCM_GPORT_SUB_TYPE_L3_VLAN_TRANSLATION_GET(in_gport);
                BCM_GPORT_SUB_TYPE_OUT_LIF_SET(*flow_gport, global_lif);
            }
        }
        else if (BCM_GPORT_IS_MPLS_PORT(in_gport))
        {
            global_lif = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(in_gport);
            exc_bits = BCM_GPORT_SUB_TYPE_LIF_EXC_GET(in_gport);

            /** MPLS-Port gports are meant to be Ingress-Only or Egress-Only.
             *  Since some tests unspecify the side, we support side identification by flags as well */
            if ((exc_bits == BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY) ||
                (_SHR_IS_FLAG_SET(conversion_flags, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR)))
            {
                BCM_GPORT_SUB_TYPE_IN_LIF_SET(*flow_gport, global_lif);
            }
            else if ((exc_bits == BCM_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY) ||
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "algo_gpm_gport_to_flow_gport_convert error from an MPLS-Port with no Ingress/Egress indication (0x%x)\n",
                             in_gport);
            }
        }
        else if (BCM_GPORT_IS_EXTENDER_PORT(in_gport))
        {
            global_lif = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(in_gport);
            exc_bits = BCM_GPORT_SUB_TYPE_LIF_EXC_GET(in_gport);
            if (exc_bits == BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY)
            {
                BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_SET(*flow_gport, global_lif);
            }
            else if (_SHR_IS_FLAG_SET(conversion_flags, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR))
            {
                BCM_GPORT_SUB_TYPE_IN_LIF_SET(*flow_gport, global_lif);
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "algo_gpm_gport_to_flow_gport_convert error from a Extender-Port symmetric gport (0x%x) with no flow side flag\n",
                             in_gport);
            }
        }
        else if (BCM_GPORT_IS_TUNNEL(in_gport))
        {
            if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_POINTED(in_gport))
            {
                virtual_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_GET(in_gport);
                BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_SET(*flow_gport, virtual_id);
            }
            ...
            {
                global_lif = BCM_GPORT_TUNNEL_ID_GET(in_gport);
                BCM_GPORT_SUB_TYPE_IN_LIF_SET(*flow_gport, global_lif);
            }
            else if (_SHR_IS_FLAG_SET(conversion_flags, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR))
            {
                global_lif = BCM_GPORT_TUNNEL_ID_GET(in_gport);
                BCM_GPORT_SUB_TYPE_OUT_LIF_SET(*flow_gport, global_lif);
            }
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "algo_gpm_gport_to_flow_gport_convert error from a Tunnel gport (0x%x) with no flow side flag\n",
                             in_gport);
            }

        }
        else if (BCM_GPORT_IS_VXLAN_PORT(in_gport))
        {
            global_lif = BCM_GPORT_VXLAN_PORT_ID_GET(in_gport);
            BCM_GPORT_SUB_TYPE_IN_LIF_SET(*flow_gport, global_lif);
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Unknown GPORT type, in_gport (0x%x)\n", in_gport);
        }
...
static shr_error_e
algo_gpm_flow_gport_to_gport_convert(
    int unit,
    bcm_gport_t flow_gport,
    uint32 out_gport_type,
    uint32 conversion_flags,
    bcm_gport_t * out_gport)
{
    int global_lif_or_virtual = DNX_ALGO_GPM_LIF_INVALID, gport_subtype;
    SHR_FUNC_INIT_VARS(unit);

    if (!BCM_GPORT_IS_FLOW_LIF(flow_gport))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "flow lif gport given is not from type flow_lif (flow_gport=0x%x)\n", flow_gport);
    }

    global_lif_or_virtual = _SHR_GPORT_FLOW_LIF_VAL_GET_ID(flow_gport);
    gport_subtype = BCM_GPORT_SUB_TYPE_GET(flow_gport);

    if (out_gport_type == BCM_GPORT_TYPE_FLOW_LIF)
    {
        *out_gport = flow_gport;
    }
    else if (out_gport_type == BCM_GPORT_VLAN_PORT)
    {
        uint8 is_symmetric;
        int exc_bits;

        if (gport_subtype == BCM_GPORT_SUB_TYPE_IN_LIF)
        {
            SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_lif_id_is_symmetric
                            (unit, global_lif_or_virtual, &is_symmetric));
            exc_bits = (is_symmetric) ? 0 : BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY;
            BCM_GPORT_SUB_TYPE_LIF_SET(*out_gport, exc_bits, global_lif_or_virtual);
        }
        else if (gport_subtype == BCM_GPORT_SUB_TYPE_OUT_LIF)
        {
            SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_lif_id_is_symmetric
                            (unit, global_lif_or_virtual, &is_symmetric));
            exc_bits = (is_symmetric) ? 0 : BCM_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY;
            BCM_GPORT_SUB_TYPE_LIF_SET(*out_gport, exc_bits, global_lif_or_virtual);
            /** There's no usage for conversion to BCM_GPORT_SUB_TYPE_VLAN_TRANSLATION */
        }
        else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS)
        {
            if (_SHR_IS_FLAG_SET(conversion_flags, DNX_ALGO_GPM_CONVERSION_FLAG_NATIVE))
            {
                BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE_SET(*out_gport, global_lif_or_virtual);
            }
            else if (_SHR_IS_FLAG_SET(conversion_flags, DNX_ALGO_GPM_CONVERSION_FLAG_INGRESS_LIF))
            {
                BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NON_NATIVE_SET(*out_gport, global_lif_or_virtual);
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Error, Conversion from Flow Virtual Ingress gport 0x%x to VLAN-Port with no conversion flag\n",
                             flow_gport);
            }
        }
        else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH)
        {
            BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_SET(*out_gport, global_lif_or_virtual);
        }
        else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT)
        {
            BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_SET(*out_gport, global_lif_or_virtual);
        }
        else if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED)
        {
            BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_SET(*out_gport, global_lif_or_virtual);
        }

        BCM_GPORT_VLAN_PORT_ID_SET(*out_gport, *out_gport);
    }
    else if (out_gport_type == BCM_GPORT_MPLS_PORT)
    {
        if (gport_subtype == BCM_GPORT_SUB_TYPE_IN_LIF)
        {
            BCM_GPORT_SUB_TYPE_LIF_SET(*out_gport, BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY, global_lif_or_virtual);
        }
        else if (gport_subtype == BCM_GPORT_SUB_TYPE_OUT_LIF)
        {
            BCM_GPORT_SUB_TYPE_LIF_SET(*out_gport, BCM_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY, global_lif_or_virtual);
        }

        BCM_GPORT_MPLS_PORT_ID_SET(*out_gport, *out_gport);
    }
    else if (out_gport_type == BCM_GPORT_EXTENDER_PORT)
    {
        uint8 is_symmetric;
        int exc_bits;

        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_lif_id_is_symmetric(unit, global_lif_or_virtual, &is_symmetric));
        ...
        BCM_GPORT_EXTENDER_PORT_ID_SET(*out_gport, *out_gport);
    }
    else if (out_gport_type == BCM_GPORT_TYPE_TUNNEL)
    {
        if (gport_subtype == BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED)
        {
            BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_SET(*out_gport, global_lif_or_virtual);
        }
        else
        {
            *out_gport = global_lif_or_virtual;
        }

        BCM_GPORT_TUNNEL_ID_SET(*out_gport, *out_gport);
    }
    else if (out_gport_type == BCM_GPORT_VXLAN_PORT)
    {
        BCM_GPORT_VXLAN_PORT_ID_SET(*out_gport, global_lif_or_virtual);
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unknown out_gport_type (%d)\n", out_gport_type);
    }
...
static shr_error_e
algo_gpm_gport_flow_get_conversion_flags(
    int unit,
    int gport,
    uint32 *conversion_flags)
{
    ...
    if (BCM_GPORT_SUB_TYPE_IS_IN_LIF(gport))
    {
        *conversion_flags = DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR;
    }
    else if (BCM_GPORT_SUB_TYPE_IS_OUT_LIF(gport))
    {
        *conversion_flags = DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR;
    }
    ...
    {
        *conversion_flags = DNX_ALGO_GPM_CONVERSION_FLAG_NATIVE;
    }
    else if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS(gport))
    {
        *conversion_flags = DNX_ALGO_GPM_CONVERSION_FLAG_INGRESS_LIF;
    }
    else
    {
    ...
    bcm_gport_t * out_gport)
{
     ...
     *  Validity - if port is not valid or not set then nothing to do
     */
    if ((in_gport == BCM_GPORT_INVALID) || !BCM_GPORT_IS_SET(in_gport))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "gport given is invalid when trying to map gport to lif, gport (0x%x)\n",
                     in_gport);
    }

    /*
     * In case the calc flag is set - calculate the conversion flags by ourselves 
     */
    if (_SHR_IS_FLAG_SET(conversion_flags, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_CALC))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_get_conversion_flags(unit, in_gport, &conversion_flags));
    }

    /** convert in_gport (legacy lif) to flow lif gport */
    if (out_gport_type == BCM_GPORT_TYPE_FLOW_LIF)
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_to_flow_gport_convert(unit, in_gport, conversion_flags, out_gport));
    }
    /** convert flow lif gport to legacy gport */
    else
    {
        SHR_IF_ERR_EXIT(algo_gpm_flow_gport_to_gport_convert
                        (unit, in_gport, out_gport_type, conversion_flags, out_gport));
    }
