    ...
    int unit)
{
        ...
        && dnx_data_dev_init.general.flexe_core_drv_select_get(unit))
    {
        ...
        port_init_info.key_cmp_cb_name = "sw_state_sorted_list_cmp_uint32";
 ...
 */
shr_error_e
dnx_algo_flexe_core_port_to_group_index_get(
    int unit,
    int flexe_core_port,
    int *group_index)
{
...
dnx_algo_flexe_instance_id_alloc(
    ...
    int *nof_instances)
{
    int i, index, nof_elements = 0, instance_id_tmp;
    int orig_nof_instances = 0, orig_instance_id;
    const int *instance_id_array = NULL;
    uint8 is_allocated;

    SHR_FUNC_INIT_VARS(unit);

    if (is_check_only && (flexe_core_port != -1))
    {
        /*
         * If it is check_only operation, free the orignal resources and
         * recover the resources after checking
         */
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.nof_instances.get(unit, flexe_core_port, &orig_nof_instances));
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.instance.get(unit, flexe_core_port, &orig_instance_id));
        if (orig_nof_instances != 0)
        {
            SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.resource_alloc.
                            instance.free_several(unit, orig_nof_instances, orig_instance_id, NULL));
        }
    }
    /*
    ...
    *nof_instances = 0;
    ...
    {
        if ((*instance_id != -1) && (*instance_id != instance_id_array[i]))
        {
            /*
             * If instance id is set, check if the specified instance_id is legal.
             */
            continue;
        }
        /*
        ...
        instance_id_tmp = instance_id_array[i];
        if (instance_id_tmp != -1)
        {
            for (index = 0; index < nof_elements; ++index)
            {
                SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.resource_alloc.
                                instance.is_allocated(unit, instance_id_tmp + index, &is_allocated));
                if (is_allocated)
                {
                    break;
                }
            ...
            if (!is_allocated)
            {
                ...
                {
                    /*
                     * If it is check only, no need to allocate the resources in DB
                     */
                    SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.resource_alloc.
...
exit:
    if (is_check_only && (orig_nof_instances != 0))
    {
        /*
         * recover the orignal instance in DB
         */
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.resource_alloc.
                        instance.allocate_several(unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, orig_nof_instances, NULL,
                                                  &orig_instance_id));
    }
    SHR_FUNC_EXIT;
}
 ...
 */
shr_error_e
dnx_algo_flexe_core_port_am_transparent_set(
    int unit,
    int flexe_core_port,
    int am_transparent)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.is_am_transparent.set(unit, flexe_core_port, am_transparent));
 ...
 */
shr_error_e
dnx_algo_flexe_core_port_am_transparent_get(
    int unit,
    int flexe_core_port,
    int *am_transparent)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.is_am_transparent.get(unit, flexe_core_port, am_transparent));
 ...
 */
shr_error_e
dnx_algo_flexe_group_instances_get(
    int unit,
    int group_index,
    uint32 *instances)
{
    ...
    *instances = 0;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_port_instances_get(unit, flexe_core_port, instances_bmp_tmp));
        SHR_BITOR_RANGE(instances, instances_bmp_tmp, 0, dnx_data_nif.flexe.nof_flexe_instances_get(unit), instances);
    }
exit:
    SHR_FUNC_EXIT;
}
