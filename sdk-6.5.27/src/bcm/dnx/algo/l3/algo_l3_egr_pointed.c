    ...
    int local_lif)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_l3_egr_pointed_local_lif_mapping_add_verify
                           (unit, flags, egr_pointed_id, local_lif));
    /*
    ...
    dnx_egr_pointed_t egr_pointed_id)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_l3_egr_pointed_local_lif_mapping_remove_verify(unit, flags, egr_pointed_id));
    /*
     *  Remove mapping.
     *
    ...
    dnx_egr_pointed_t * egr_pointed_id_p)
{
     ...
     *  Check input
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_l3_local_lif_to_egr_pointed_get_verify(unit, flags, local_lif, egr_pointed_id_p));
    /*
    ...
    int *local_lif_p)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_l3_egr_pointed_to_local_lif_get_verify(unit, flags, egr_pointed_id, local_lif_p));
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGRESS_POINTED_TO_EGRESS_LIF_MAP_SW, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VIRTUAL_EGRESS_POINTED, egr_pointed_id);
    shr_error = dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS);
    if (shr_error != _SHR_E_NONE)
