    ...
    dbal_tables_e table_id)
{
    ...
    outlif_info.local_outlif = rif_id;
    ...
    uint32 fixed_entry_size)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.out_rif_result_type_info.set(unit, rif_id, rif_result_type));
    ...
    int rif_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.out_rif_result_type_info.get(unit, rif_id, &result_type));
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.out_rif_result_type_info.set(unit, rif_id, DNX_ALGO_L3_INVALID_OUT_RIF_RESULT_TYPE));
    ...
    int unit)
{
    ...
    /*
     * Initialize Out-RIF result type SW state
     */
    SHR_IF_ERR_EXIT(algo_l3_db.out_rif_result_type_info.alloc(unit));
    ...
    uint32 alloc_flags)
{
    ...
    {
        extra_create_info.max_tags_value[0] = DBAL_ENUM_FVAL_FEC_RESOURCE_TYPE_NO_PROT_W_STAT;
    }
...
/**
 * \brief
 *  Verify the FEC allocation data of the FEC pair it is being allocated in.
 *  \param [in] unit - The unit number.
 ...
 */
static shr_error_e
dnx_algo_l3_fec_allocation_verify(
    int unit,
    int fec_index,
    algo_dnx_l3_fec_data_t * fec_alloc_data)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_l3_fec_super_fec_type_get(unit, fec_in_pair, &old_fec_resource_type));

        if (old_fec_resource_type != fec_alloc_data->super_fec_type_tag)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Super FEC resource type for pair of FEC value %u is already %u, but user provided %u\n",
                         fec_index, old_fec_resource_type, fec_alloc_data->super_fec_type_tag);
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    algo_dnx_l3_fec_data_t * fec_alloc_data)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_L3_FEC_WITH_ID))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_l3_fec_allocation_verify(unit, *fec_index, fec_alloc_data));
    ...
    dbal_enum_value_field_fec_resource_type_e * super_fec_resource_type)
{
    ...
    tag_info.element = fec_index;
    SHR_IF_ERR_EXIT(algo_l3_db.fec_alloc.l3_fec_res_mngr.
                    fec_res_manager.advanced_algorithm_info_get(unit, sub_resource_id, &tag_info));

    *super_fec_resource_type = tag_info.tags[0];
