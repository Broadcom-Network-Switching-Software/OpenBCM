    ...
    uint8 *first_appear)
{
    ...
    if (with_id)
    {
        ...
        /*
         * Note: Procedure sw_state_index_rh_insert_at_index() reports 'SHR_E_FAIL' on trying to add
         * key at data_idx which is already in use for other usages.
         */
        SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(sw_state_index_htb_rh_insert_at_index
                                        (unit, multi_set->hash_table, 0, (MULTI_SET_KEY *) key, found_index),
                                        _SHR_E_FAIL);
        if (SHR_GET_CURRENT_ERR() == _SHR_E_FAIL)
        {
            /*
             * If it is a singleton, and we didn't insert or find it
             * then mark it as 'first appearance' and quit.
             * We assume the user allocated the entry before the call.
             */
            success = FALSE;
            *first_appear = FALSE;
            goto exit;
    ...
    {
        /*
         * Note: Procedure sw_state_htb_rh_insert() reports 'SHR_E_FULL' if the HTB is full.
         */
        ...
        if (SHR_GET_CURRENT_ERR() == _SHR_E_FULL)
        {
             ...
             * If it is a singleton, and we didn't insert or find it
             * then mark it as 'first appearance' and quit.
             * We assume the operation failed because the table is full.
             */
            success = FALSE;
            *first_appear = TRUE;
            found_index = UTILEX_U32_MAX;
            goto exit;
        }
    }
    success = TRUE;
    ...
    *ref_count = 0;
    ...
    SHR_IF_ERR_EXIT(multi_set_get_by_index
                    (unit, node_id, multi_set, *data_indx + multi_set->init_info.first_profile, (int *) ref_count,
                     key));
    /*
    ...
    *ref_count += adjusted_additions;
     ...
     */
    DNX_SW_STATE_MEMWRITE(unit, node_id, tmp_cnt, multi_set->ref_counter, (*data_indx) * multi_set->counter_size,
                          multi_set->counter_size, 0, "multi_set ref_counter.");
    ...
    uint32 alloc_flags)
{
    ...
    uint32 member_size;
    sw_state_htb_create_info_t hash_table_init_info;
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "extra_arguments are not used, so they should be NULL.");
    }
    ...
    max_duplications = 0;
    member_size = 0;
    sal_memset(&hash_table_init_info, 0, sizeof(sw_state_htb_create_info_t));
    ...
    DNX_SW_STATE_ALLOC(unit, node_id, *multi_set, **multi_set, /* number of elements */ 1,
                       DNXC_SW_STATE_NONE, "sw_state multi set");

    DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, &((*multi_set)->init_info), &multi_set_init_info,
    ...
    SHR_IF_ERR_EXIT(sw_state_index_htb_rh_create
                    (unit, node_id, &((*multi_set)->hash_table), &hash_table_init_info, member_size, alloc_flags));
    ...
    DNX_SW_STATE_ALLOC(unit, node_id, (*multi_set)->ref_counter, DNX_SW_STATE_BUFF,
                       multi_set_init_info.nof_members * (*multi_set)->counter_size,
                       alloc_flags, "sw_state multi set ref counter");

    DNX_SW_STATE_MEMSET(unit, node_id, (*multi_set)->ref_counter, 0, 0x00,
    ...
    void *extra_arguments)
{
    ...
    DNX_SW_STATE_FREE(unit, node_id, (*multi_set)->ref_counter, "sw state multi_set ref_counter");

    DNX_SW_STATE_FREE(unit, node_id, (*multi_set), "sw state multi_set ref_counter");
    ...
    uint8 *last_appear)
{
    ...
    *last_appear = (ref_counter == 0) ? TRUE : FALSE;
    ...
    DNX_SW_STATE_MEMWRITE(unit, node_id, tmp_cnt, multi_set->ref_counter, (_data_indx) * multi_set->counter_size,
                          multi_set->counter_size, 0, "multi_set ref_counter.");

    if (*last_appear)
    {
        SHR_IF_ERR_EXIT_EXCEPT_IF(sw_state_index_htb_rh_delete_by_index
                                  (unit, multi_set->hash_table, 0, (uint32) _data_indx), _SHR_E_NOT_FOUND);
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *data_indx)
{
    ...
    SHR_IF_ERR_EXIT_NO_MSG(sw_state_index_htb_rh_find
                           (unit, multi_set->hash_table, 0, (MULTI_SET_KEY *) key, (uint32 *) data_indx));
    ...
    void *key)
{
    ...
    SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(sw_state_index_htb_rh_get_by_index
                                    (unit, multi_set->hash_table, 0, _data_indx, (MULTI_SET_KEY *) key),
                                    _SHR_E_NOT_FOUND);
    ...
    multi_set_t multi_set)
{
     ...
     * clear mutli-set.
     */
    SHR_IF_ERR_EXIT(sw_state_index_htb_rh_delete_all(unit, multi_set->hash_table, 0));
