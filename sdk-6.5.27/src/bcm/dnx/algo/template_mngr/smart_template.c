    ...
    uint32 alloc_flags)
{
    ...
    uint32 res_flags;
    resource_tag_bitmap_utils_extra_arg_create_info_t *resource_extra_create_info;
    ...
    uint8 *first_reference)
{
    ...
    resource_tag_bitmap_utils_extra_arg_alloc_info_t *resource_alloc_info, resource_alloc_info_struct;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (allocation_needed)
    {
        ...
        if (with_id)
        {
            ...
            if (resource_alloc_info == NULL)
            {
                sal_memset(&resource_alloc_info_struct, 0, sizeof(resource_tag_bitmap_utils_extra_arg_alloc_info_t));
    ...
    uint8 *last_reference)
{
     ...
     */
    SHR_ALLOC_ERR_EXIT(old_profile_data, multi_set_template->hash_table->htb_rh->key_size,
    ...
    if (ref_count_old != 0)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_smart_template_free
                        (unit, node_id, multi_set_template, old_profile, nof_references, last_reference));
        /*
    ...
    {
        SHR_ALLOC_ERR_EXIT(new_profile_old_data, multi_set_template->hash_table->htb_rh->key_size,
        ...
        SHR_IF_ERR_EXIT(dnx_algo_smart_template_allocate
                        (unit, node_id, multi_set_template, flags, nof_references,
    ...
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_smart_template_allocate
                        (unit, node_id, multi_set_template, flags, nof_references, (void *) profile_data,
    ...
    if (SHR_FUNC_ERR() || !add_success)
    {
        ...
        if (old_profile_freed)
        {
            ...
            rv = dnx_algo_smart_template_allocate(unit, node_id, multi_set_template,
                                                  DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID, nof_references, old_profile_data,
                                                  NULL, &old_profile, &dummy_first_reference);
            if (SHR_FAILURE(rv))
            {
                /*
