    ...
    int *global_lif)
{
    ...
    if (is_ingress)
    {
        ...
        if (LIF_MNGR_DB_IS_INLIF_SBC(unit, local_lif_info->phy_table))
        {
            LIF_MNGR_LOCAL_SBC_IN_LIF_VERIFY(unit, local_lif_info->local_lifs_array.local_lifs[0]);
        }
        else if (LIF_MNGR_DB_IS_INLIF_DPC(unit, local_lif_info->phy_table))
        {
    ...
    int is_validate)
{
    ...
    else
    {
         ...
         *  Local to global
         */
        if (LIF_MNGR_DB_IS_INLIF_SBC(unit, local_lif_payload->phy_table))
        {
            SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.
...
static shr_error_e
dnx_algo_ingress_lif_mapping_init(
    int unit)
{
...
static shr_error_e
dnx_algo_egress_lif_mapping_init(
    int unit)
{
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_ingress_lif_mapping_init(unit));
    }
    SHR_IF_ERR_EXIT(dnx_algo_egress_lif_mapping_init(unit));
