    ...
    global_lif_alloc_info_t * alloc_info)
{
    ...
    if (direction == DNX_ALGO_LIF_EGRESS)
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                        egress_global_lif.advanced_algorithm_info_get(unit, (void *) &local_tag_info));
    }
    else if (direction == DNX_ALGO_LIF_INGRESS)
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                        ingress_global_lif.advanced_algorithm_info_get(unit, (void *) &local_tag_info));
    }
    else if (direction == (DNX_ALGO_LIF_INGRESS | DNX_ALGO_LIF_EGRESS))
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                        symmetric_global_lif.advanced_algorithm_info_get(unit, (void *) &local_tag_info));
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "invalid direction %d\n", direction);
    }
...
static shr_error_e
dnx_algo_global_lif_alloc_info_hw_set(
    int unit,
    int global_lif,
    global_lif_alloc_info_t * alloc_info)
{
    ...
    uint32 alloc_flags)
{
    ...
    if (create_info.tag_max_size != 0)
    {
        if (dnx_data_lif.global_lif.global_lif_bta_max_val_get(unit))
        {
            /** tag per BTA value, RIFs can receive BTA value default (0) only */
    ...
    if ((create_info.tag_max_size != 0) && (create_info.hw_set_required == TRUE) &&
        (res_tag_bitmap_alloc_info.first_references[0] == TRUE))
    {
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_alloc_info_hw_set(unit, element, &alloc_info));
    }
    ...
    int *global_lif)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_alloc_info_hw_set(unit, *global_lif, alloc_info));
    }
    ...
    int unit)
{
    ...
    create_info.hw_set_required = FALSE;
    /*
    ...
    create_info.hw_set_required = TRUE;
