    ...
    int unit)
{
    ...
    for (dbal_table_id = 1; dbal_table_id < DBAL_NOF_TABLES; dbal_table_id++)
    {
        ...
        if (dbal_table_is_in_lif(unit, table) || dbal_table_is_out_lif(table) || dbal_table_is_esem(table) ||
    ...
    uint32 *table_specific_flags)
{
    ...
    switch (result_type)
    {
        ...
        case DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_EXTENDED_P2P_W_SRC_LKP:
        {
            *table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_AC_INGRESS_EXTENDED_P2P;
            break;
        }
        default:
            break;
    ...
    uint32 *table_specific_flags)
{
    ...
    if (result_type == DBAL_RESULT_TYPE_EEDB_PWE_ETPS_IOAM_FL)
    {
        *table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_MPLS_ENCAP_IOAM_DUAL_HOM;
    }
    ...
    uint32 *table_specific_flags)
{
    ...
    else if (result_type == DBAL_RESULT_TYPE_EEDB_IPV6_TUNNEL_ETPS_IPV6_TUNNEL_GENEVE)
    {
        *table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_GENEVE_6;
    }
    ...
    else if (result_type == DBAL_RESULT_TYPE_EEDB_IPV6_TUNNEL_ETPS_IPV6_TUNNEL_SCALE)
    {
        *table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_IPV6_SCALE;
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "result type (%d) is not supported", result_type);
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_AC_INGRESS_EXTENDED_P2P))
    {
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_EXTENDED_P2P_W_SRC_LKP);
    }
    else if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_AC_INGRESS_NON_NATIVE_P2P))
    {
        /*
         * AC VLAN-PORT non-native P2P:
    ...
    else
    {
         ...
         *  - DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC__MP__XL_LEARN_INFO__PMF_DATA__PROTECTION
         */
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    {
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_PON_DTC_CLASSIFICATION_IN_LIF_AC_EXTENDED_P2P_W_SRC_LKP);
    }
    else if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_AC_INGRESS_NON_NATIVE_P2P))
    {
        /*
    ...
    else
    {
        ...
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_PON_DTC_CLASSIFICATION_IN_LIF_AC_MP_LARGE_GENERIC_DATA);
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_PON_DTC_CLASSIFICATION_IN_LIF_AC_MP_LARGE_GENERIC_DATA_AND_PROTECTION);
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
                   DBAL_RESULT_TYPE_PON_DTC_CLASSIFICATION_IN_LIF_AC_MP_OPTIMIZED_LEARN_PAYLOAD);
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_MPLS_ENCAP_IOAM_DUAL_HOM))
    {
        SHR_BITCLR_RANGE(&(table_specific_rules->allowed_result_types_bmp), 0, MAX_NOF_RESULT_TYPES);
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp), DBAL_RESULT_TYPE_EEDB_PWE_ETPS_IOAM_FL);
    }
    ...
    lif_table_mngr_table_specific_rules_t * table_specific_rules)
{
    ...
    else if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_GENEVE_6))
    {
        SHR_BITSET(&(table_specific_rules->allowed_result_types_bmp),
