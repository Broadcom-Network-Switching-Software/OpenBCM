    ...
    uint32 alloc_flags)
{
    ...
    extra_create_info.max_tags_value[0] = DNX_ALGO_INLIF_MAX_TAG_SIZE;
    ...
    alloc_info.tags[0] = DNX_ALGO_LOCAL_INLIF_ENTRY_SIZE_TO_TAG(DNX_ALGO_INLIF_MINIMUM_ENTRY_SIZE, illegal_tag);
    ...
    int *element)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_LOCAL_INLIF_RES_ALLOCATE_SET_ILLEGAL_TAG_ONLY))
    {
        ...
        SHR_IF_ERR_EXIT(resource_tag_bitmap_tag_set(unit, node_id, res_tag_bitmap, &tag_info));
    }
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_lif_local_inlif_resource_free(unit, node_id, res_tag_bitmap, *element, NULL));
        }
    ...
    void *extra_argument)
{
    ...
    entry_size = DNX_ALGO_LOCAL_INLIF_TAG_TO_ENTRY_SIZE(tag_info.tags[0]);

    SHR_INVOKE_VERIFY_DNXC(dnx_lif_local_inlif_resource_free_verify(unit, entry_size, element));

    SHR_IF_ERR_EXIT(resource_tag_bitmap_free_several(unit, node_id, res_tag_bitmap, entry_size, element, NULL));
exit:
    SHR_FUNC_EXIT;
}
    ...
    void *extra_argument)
{
    ...
    if (!DNX_ALGO_LOCAL_INLIF_TAG_IS_ILLEGAL_TAG(tag_info.tags[0]))
    {
    ...
    SHR_IF_ERR_EXIT(resource_tag_bitmap_tag_set(unit, node_id, (resource_tag_bitmap_t) res_tag_bitmap, &tag_info));
    ...
    lif_mngr_local_inlif_info_t * inlif_info)
{
    ...
    LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META("\n nROYROYROYROY INLIF table %s result type %s\n"),
                                 dbal_logical_table_to_string(unit, inlif_info->dbal_table_id),
                                 dbal_result_type_to_string(unit, inlif_info->dbal_table_id,
    ...
    int *smaller_inlif)
{
    ...
    *can_be_reused = TRUE;
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported physical table id %d", physical_table_id);
    }
