    ...
    dnx_algo_lif_local_outlif_resource_info_t * resource_info)
{
    ...
    if (outlif_info->logical_phase == DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_INVALID)
    {
        uint8 tmp;
        uint32 outlif_bank;
        SHR_IF_ERR_EXIT(dnx_algo_local_outlif_to_outlif_bank(unit, outlif_info->local_outlif, &outlif_bank, NULL));
    ...
    int *element)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_REPLACE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_local_outlif_resource_free_several(unit, node_id, res_tag_bitmap,
                                                                    resource_info.nof_outlifs_per_alternative_entry,
                                                                    *element, NULL));
    }
...
dnx_algo_local_outlif_alloc_in_used_bank(
    ...
    int ll_required,
    uint8 is_eedb_data_bank,
    int nof_eedb_bank_outlifs,
    int nof_eedb_banks_per_outlif_bank,
    lif_mngr_local_outlif_info_t * outlif_info,
    int *bank_match,
    int *bank_can_be_used)
{
    ...
    *bank_match = FALSE;
    ...
    if (is_eedb_data_bank)
    {
        if (nof_free_elements == nof_eedb_bank_outlifs)
        {
            uint8 current_eedb_bank;
            int current_eedb_bank_array_index;
            ...
            {
                SHR_IF_ERR_EXIT(local_outlif_info.outlif_bank_info.
                                eedb_data_banks.get(unit, bank, current_eedb_bank_array_index, &current_eedb_bank));

                if (current_eedb_bank == (uint8) DNX_ALGO_LOCAL_OUTLIF_INVALID_BANK)
                {
                    int eedb_bank_is_available;

                    SHR_IF_ERR_EXIT(dnx_algo_local_outlif_verify_ll_availability
                                    (unit, required_logical_phase, 1, &eedb_bank_is_available));
    ...
    else if ((rv != _SHR_E_FULL) && (rv != _SHR_E_RESOURCE))
    {
        SHR_IF_ERR_EXIT(rv);
    }
    ...
    uint32 *outlif_bank)
{
    ...
    nof_eedb_bank_outlifs = dnx_data_mdb.dh.macro_type_info_get(unit, MDB_EEDB_BANK)->nof_rows * nof_outlifs_per_entry;
    nof_eedb_banks_per_outlif_bank = DNX_ALGO_LOCAL_OUTLIF_BANK_SIZE(unit) / nof_eedb_bank_outlifs;
           ...
           && !_SHR_IS_FLAG_SET(local_lif_flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_FORCE_NEW_BANK_ALLOCATION))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_local_outlif_alloc_in_used_bank
                        (unit, current_outlif_bank, logical_phase, ll_required, TRUE, nof_eedb_bank_outlifs,
                         nof_eedb_banks_per_outlif_bank, outlif_info, &outlif_bank_match, &outlif_bank_can_be_used));
    ...
    dnx_algo_local_outlif_hw_info_t * outlif_hw_info)
{
    ...
    if (nof_allocated_elements == 0)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_local_outlif_bank_hw_info_get(unit, local_outlif, outlif_bank, outlif_hw_info));
...
{
    int current_bank, bank_match, bank_can_be_used = FALSE;
    uint8 in_use, dynamic_algo;
    ...
    for (current_bank = first_bank_for_phase; current_bank <= last_bank_for_phase; current_bank++)
    {
        ...
        if (in_use && !_SHR_IS_FLAG_SET(local_lif_flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_FORCE_NEW_BANK_ALLOCATION))
        {
            ...
            bank_can_be_used = FALSE;
            SHR_IF_ERR_EXIT(dnx_algo_local_outlif_alloc_in_used_bank
                            (unit, current_bank, logical_phase, ll_required, FALSE, 0, 0, outlif_info, &bank_match,
                             &bank_can_be_used));
...
shr_error_e
dnx_algo_local_outlif_to_outlif_bank(
    int unit,
    int local_outlif,
    uint32 *outlif_bank,
    int *outlif_in_bank)
...
shr_error_e
dnx_algo_local_outlif_is_allocated_get(
    int unit,
    int local_outlif,
    uint8 *is_allocated)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_local_outlif_to_outlif_bank(unit, local_outlif, &outlif_bank, &outlif_idx));

    SHR_IF_ERR_EXIT(lif_mngr_db.egress_local_lif.is_allocated(unit, outlif_bank, outlif_idx, is_allocated));
...
shr_error_e
dnx_algo_local_outlif_allocate_with_id(
    int unit,
    uint32 outlif_bank,
    int local_outlif,
    lif_mngr_local_outlif_info_t * outlif_info)
{
    ...
    if (!dynamic_algo)
    {
        outlif_idx = local_outlif;

    }

    SHR_IF_ERR_EXIT(lif_mngr_db.egress_local_lif.allocate_single(unit, outlif_bank,
                                                                 DNX_ALGO_RES_ALLOCATE_WITH_ID,
                                                                 (void *) outlif_info, &outlif_idx));
...
shr_error_e
dnx_algo_local_outlif_free_several(
    int unit,
    uint32 outlif_bank,
    int local_outlif,
    int deallocation_size)
{
    ...
    SHR_IF_ERR_EXIT(lif_mngr_db.egress_local_lif.free_several(unit, outlif_bank, deallocation_size, outlif_idx, NULL));
    ...
    dnx_algo_local_outlif_hw_info_t * outlif_hw_info)
{
    ...
    uint8 rt_has_ll = FALSE, dynamic_algo;
    ...
    LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META("\n nROYROYROYROY OUTLIF table %s result type %s\n"),
    ...
    if (_SHR_IS_FLAG_SET(outlif_info->local_lif_flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_REPLACE))
    {
        ...
        element = outlif_info->local_outlif;
        SHR_IF_ERR_EXIT(dnx_algo_local_outlif_to_outlif_bank(unit, element, &outlif_bank, NULL));
    ...
    else
    {
        ...
        if (_SHR_IS_FLAG_SET(outlif_info->local_lif_flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_UPDATE_LL_IN_HW))
        {
            outlif_hw_info->flags |= DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_UPDATE_LL_BANK;
            outlif_hw_info->flags |= DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_UPDATE_POINTERS;
        }
    ...
    if (_SHR_IS_FLAG_SET(outlif_hw_info->flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_UPDATE_HW))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_local_outlif_bank_hw_info_get(unit, element, outlif_bank, outlif_hw_info));
    }
    ...
    if (_SHR_IS_FLAG_SET(outlif_info->local_lif_flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_REPLACE))
    {
        ...
        if (resource_info.nof_outlifs_per_alternative_entry > resource_info.nof_outlifs_per_entry)
        {
            int nof_elements_to_free =
                resource_info.nof_outlifs_per_alternative_entry - resource_info.nof_outlifs_per_entry;
            int element_to_free = element + (resource_info.nof_outlifs_per_alternative_entry - nof_elements_to_free);

            SHR_IF_ERR_EXIT(lif_mngr_db.
                            egress_local_lif.free_several(unit, outlif_bank, nof_elements_to_free, element_to_free,
                                                          (void *) outlif_info));
        }
    }
...
dnx_algo_local_outlif_free(
    ...
    dnx_algo_local_outlif_hw_info_t * outlif_hw_info)
{
    int outlif_bank = 0, nof_free_elements, internal_local_outlif;
    int deallocation_size;
    uint8 assigned;
    uint8 dynamic_algo;
    ...
    internal_local_outlif = outlif_info->local_outlif;

    SHR_IF_ERR_EXIT(local_outlif_info.dynamic_algo.get(unit, &dynamic_algo));

    outlif_bank = DNX_ALGO_OUTLIF_LOCAL_LIF_TO_BANK(internal_local_outlif);

    SHR_IF_ERR_EXIT(dnx_algo_local_outlif_info_to_nof_outlifs_per_entry(unit, outlif_info, &deallocation_size));

    SHR_IF_ERR_EXIT(lif_mngr_db.
                    egress_local_lif.free_several(unit, outlif_bank, deallocation_size, internal_local_outlif,
                                                  (void *) outlif_info));
    ...
    else
    {
        ...
        if (nof_free_elements == DNX_ALGO_LOCAL_OUTLIF_BANK_SIZE(unit))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_local_outlif_bank_hw_info_get
                            (unit, outlif_info->local_outlif, outlif_bank, outlif_hw_info));
    ...
    int last_no_ll_bank)
{
    ...
    if (first_no_ll_bank >= 0)
    {

        SHR_IF_ERR_EXIT(local_outlif_info.logical_phase_info.
                        first_no_ll_bank.set(unit, logical_phase, first_no_ll_bank));
...
dnx_algo_local_outlif_resource_init(
    int unit,
    int nof_outlif_banks,
    int nof_rif_banks)
{
    dnx_algo_res_create_data_t res_data;
    resource_tag_bitmap_utils_tag_info_t tag_set_info;
    int *bank_address_granularities = NULL, current_granularity, minimum_granularity;
    ...
    int nof_required_outlif_banks[DBAL_NOF_ENUM_MDB_EEDB_PHASE_VALUES])
{
    ...
    for (current_physical_phase = 0; current_physical_phase < max_phase; current_physical_phase++)
    {
        ...
        for (current_macro_type = MDB_MACRO_A; current_macro_type < MDB_EEDB_BANK; current_macro_type++)
        {
...
dnx_algo_local_outlif_mdb_bank_database_init(
    ...
    int *nof_init_clusters)
{
    uint32 eedb_pair_granularities[NOF_EEDB_PHASE_PAIRS] = { 0 };
    ...
    *nof_init_clusters = 0;
    ...
    SHR_IF_ERR_EXIT(dnx_algo_local_outlif_resource_init(unit, total_required_outlif_banks, 0));
