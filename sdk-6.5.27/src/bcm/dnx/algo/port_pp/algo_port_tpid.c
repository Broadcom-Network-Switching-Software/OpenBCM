    ...
    uint32 alloc_flags)
{
    ...
    int tag_struct_range_high[bcmNofTagStructType] = {
        ...
        DBAL_ENUM_FVAL_INCOMING_TAG_STRUCTURE_S_PRIORITY
    };
    ...
    extra_create_info.grains_size[0] = 1;
    extra_create_info.max_tags_value[0] = bcmNofTagStructType + 1;
    ...
    SHR_IF_ERR_EXIT(resource_tag_bitmap_create
                    (unit, node_id, resource, &res_tag_bitmap_create_info, &extra_create_info, alloc_flags));
    ...
    for (current_tag = 0; current_tag < bcmNofTagStructType; current_tag++)
    {
        ...
        SHR_IF_ERR_EXIT(resource_tag_bitmap_tag_set(unit, node_id, *resource, &tag_info));
    }
    ...
    tag_info.tags[0] = bcmNofTagStructType;
    tag_info.force_tag = TRUE;
    tag_info.element = DBAL_NOF_ENUM_INCOMING_TAG_STRUCTURE_VALUES;
    tag_info.nof_elements = nof_vlan_tag_formats - DBAL_NOF_ENUM_INCOMING_TAG_STRUCTURE_VALUES;
    ...
    int *element)
{
    ...
    res_tag_bitmap_alloc_info.tags[0] = tag_struct_type;
    ...
    int unit)
{
    ...
    /*
     * Note:
     * First Ingress profile (see default_ingress_native_ac_llvp_profile_id) is used for Ingress Native AC default and is not managed by the template management.
     */
    data_template_mgr.nof_profiles =
        (dnx_data_port_pp.general.nof_ingress_llvp_profiles_get(unit) <<
         (dnx_data_vlan.llvp.ext_vcp_en_get(unit) ? 1 : 0)) - 1;
    ...
    /*
     * Note:
     * First Egress profile (see default_egress_native_ac_llvp_profile_id) is used for Egress Native AC default and is not managed by the template management.
     */
    data_template_mgr.nof_profiles = dnx_data_port_pp.general.nof_egress_llvp_profiles_get(unit) - 1;
    sal_strncpy(data_template_mgr.name, DNX_ALGO_PORT_TPID_LLVP_PROFILE_EGRESS_TEMPLATE,
                DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    if (!dnx_data_vlan.llvp.feature_get(unit, dnx_data_vlan_llvp_outlif_llvp_profile_zero_is_valid))
    {
        /*
         * 0: not in used
         * 1: Egress native 
         * 2-7: for user usage, handled by template manager
         */
        data_template_mgr.first_profile = 2;
        data_template_mgr.nof_profiles--;
    }
    ...
    sal_strncpy(data_res_mgr.name, DNX_ALGO_PORT_TPID_SW_TAG_STRUCT_INGRESS, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
     ...
     */
    sal_strncpy(data_res_mgr.name, DNX_ALGO_PORT_TPID_SW_TAG_STRUCT_EGRESS, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
     ...
     */
    sal_strncpy(data_res_mgr.name, DNX_ALGO_PORT_TPID_SW_TAG_STRUCT_SYMMETRIC, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    sal_strncpy(data_res_mgr.name, DNX_ALGO_PORT_TPID_HW_TAG_STRUCT_INGRESS, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
     ...
     */
    sal_strncpy(data_res_mgr.name, DNX_ALGO_PORT_TPID_HW_TAG_STRUCT_EGRESS, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
