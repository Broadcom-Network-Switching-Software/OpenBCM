    ...
    int unit)
{
    ...
    template_data.default_profile = dnx_data_esem.access_cmd.allocation_mngr_default_get(unit);
    sal_strncpy(template_data.name, DNX_ALGO_TEMPLATE_ESEM_ACCESS_CMD, DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    {
        ...
        extra_info.resource_create_info.grains_size[0] =
            dnx_data_esem.access_cmd.nof_cmds_get(unit) / DNX_ALGO_PP_PORT_ESEM_CMD_RES_NOF_TAGS;
        extra_info.resource_create_info.max_tags_value[0] = extra_info.resource_create_info.grains_size[0];
...
/**
 * \brief -
 *  The function initialize the ipvx-mpls-parsing-profile template.
 *
 * \param [in] unit - relevant unit
 *
 * \return
 ...
 */
static shr_error_e
algo_port_ipvx_mpls_parsing_profile_init(
    int unit)
{
    dnx_algo_template_create_data_t data;
    uint32 default_data;
    ...
    data.nof_profiles = 2;
    data.max_references = dnx_data_port.general.nof_pp_ports_get(unit) * dnx_data_device.general.nof_cores_get(unit);
    sal_strncpy(data.name, DNX_ALGO_PP_PORT_IPVX_O_MPLS_PARSING, DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);

    SHR_IF_ERR_EXIT(algo_port_pp_db.ipvx_o_mpls_parsing_enable_profile.create(unit, &data, NULL));
...
/**
 * \brief -
 *  The function destroy the ipvx-mpls-parsing-profile template.
 *
 * \param [in] unit - relevant unit
 *
 * \return
 ...
 */
static shr_error_e
algo_port_ipvx_mpls_parsing_profile_deinit(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    const void *data)
{
    ...
    uint32 *enable = (uint32 *) data;
#endif
    SW_STATE_PRETTY_PRINT_INIT_VARIABLES();
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "Enable/Disable", *enable, NULL, NULL);
    ...
    int unit)
{
    ...
    /*
     * Initialize IPvX over MPLS parsing profile resource
     */
    SHR_IF_ERR_EXIT(algo_port_ipvx_mpls_parsing_profile_init(unit));
    ...
    int unit)
{
    ...
    /*
     * Destroy IPvx MPLS parsing profile resource
     */
    SHR_IF_ERR_EXIT(algo_port_ipvx_mpls_parsing_profile_deinit(unit));
