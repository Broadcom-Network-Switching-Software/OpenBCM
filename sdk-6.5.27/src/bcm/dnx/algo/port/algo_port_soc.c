...
static shr_error_e dnx_algo_port_er_soc_info_port_access_rollback(
    int unit,
    ...
    dnx_algo_port_tdm_mode_e tdm_mode)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_soc_info_port_remove(unit, D3CE_PTYPE, logical_port));
    SHR_IF_ERR_EXIT(dnx_algo_port_soc_info_port_remove(unit, GE_PTYPE, logical_port));
    {
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_soc_info_port_remove(unit, OTN_PHY_PTYPE, logical_port));
    ...
    else
    {
        ...
        else if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH
                 ...
                 || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info, TRUE))
        {
            ...
            if (port_has_tx_speed && port_has_rx_speed)
            {
                ...
                if (rx_speed == tx_speed)
                {
                    ...
                    {
                        case 800000:
                            SHR_IF_ERR_EXIT(dnx_algo_port_soc_info_port_add(unit, D3CE_PTYPE, logical_port));
                            break;
                        ...
                        case 40000:
                            ...
                            break;
                        case 1000:
                            SHR_IF_ERR_EXIT(dnx_algo_port_soc_info_port_add(unit, GE_PTYPE, logical_port));
                            break;
        ...
        else if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, L1)))
        {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_soc_info_port_add(unit, OTN_PHY_PTYPE, logical_port));
        }
    }
    ...
    bcm_pbmp_t phy_ports)
{
        ...
        (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT, OTN_PHY, FRAMER_MGMT, FEEDBACK)))
    {
        ...
        if (!is_ilkn_over_fabric)
        {
            ...
            /** if master port set, set first port in bitmap as master block port - this is done to keep WB data consistent */
                ...
                {
                    int *block_port_p;
                    int first_port, is_intf_master_port = 0;
                    pbmp_t *block_bitmap_p;
                    ...
                    block_bitmap_p = &block_bitmap_p[port_block];
                    _SHR_PBMP_FIRST(*block_bitmap_p, first_port);
                    SHR_IF_ERR_EXIT(dnx_algo_port_is_master_get(unit, first_port, 0, &is_intf_master_port));
                    if (is_intf_master_port)
                    {
                    /** set first port in bitmap as master block port if the first port is master port */
                        *block_port_p = first_port;
                    }
                    else
                    {
                        *block_port_p = logical_port;
                    }
                }
                {
                    /*
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_log_entry(unit,
                                                        (uint8 *) &is_add,
                                                        sizeof(int),
                                                        (uint8 *) &soc_info_rollback,
                                                        sizeof(dnx_algo_port_er_soc_info_rollback_t),
                                                        &dnx_algo_port_er_soc_info_port_access_rollback, FALSE));
    ...
    bcm_port_t logical_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_log_entry(unit,
                                                        ...
                                                        (uint8 *) &logical_port,
                                                        sizeof(bcm_port_t),
                                                        &dnx_algo_port_er_soc_info_custom_pbmp_rollback, FALSE));
exit:
    SHR_FUNC_EXIT;
}
    ...
    dnx_algo_port_info_s port_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_log_entry(unit,
                                                        ...
                                                        (uint8 *) &soc_info_rollback,
                                                        sizeof(dnx_algo_port_er_soc_info_rollback_t),
                                                        &dnx_algo_port_er_soc_info_port_name_rollback, FALSE));
    ...
    DNXC_SERDES_REF_CLOCK_TYPE * ref_clk)
{
    ...
    {
        int link_id;
        int nof_plls = dnx_data_pll.general.fabric_pll_info_get(unit)->key_size[0];
        SHR_IF_ERR_EXIT(dnx_algo_port_fabric_link_get(unit, logical_port, &link_id));

        pll_index = link_id / (dnx_data_fabric.links.nof_links_get(unit) / nof_plls);
    ...
    else
    {
        ...
        pll_index = ethu_info.ethu_id >= dnx_data_nif.eth.max_ethu_nof_per_core_get(unit) ? 1 : 0;
