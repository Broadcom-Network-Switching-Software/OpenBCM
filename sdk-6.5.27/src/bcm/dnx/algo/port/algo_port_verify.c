    ...
    int is_out)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_RCY_MIRROR(unit, port_info))
    {
        last_channel = dnx_data_port.general.max_nof_mirror_channels_get(unit) - 1;
    }
    else
    {
        last_channel = dnx_data_port.general.max_nof_channels_get(unit) - 1;
    }
    /** verify range */
    SHR_RANGE_VERIFY(channel, 0, last_channel, _SHR_E_PARAM, "channel ID out of range for port %d.\n", logical_port);
