    ...
    switch (logicals_type)
    {
        ...
        case DNX_ALGO_PORT_LOGICALS_TYPE_NIF:
        {
            ...
            supported_flags =
                ...
                DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_OTN_PHY | DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_L1 |
                DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FRAMER_MGMT | DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FEEDBACK;
            SHR_IF_ERR_EXIT_WITH_LOG(utilex_bitstream_mask_verify(&flags, &supported_flags, 1),
            ...
            for (logical_port = 0; logical_port < SOC_MAX_NUM_PORTS; logical_port++)
            {
                ...
                if ((flags & DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FEEDBACK)
                    && (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_FEEDBACK(unit, port_info)))
                {
                    continue;
                }
        ...
        case DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_MAC_STAT:
        case DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_MAC_PROTECTION:
        {
            int include_l1, include_stat_only, include_protection;

            supported_flags = DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_REMOVED_PORT;
            if (logicals_type == DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_MAC)
            {
                supported_flags |=
            ...
            for (logical_port = 0; logical_port < SOC_MAX_NUM_PORTS; logical_port++)
            {
                ...
                if ((logicals_type == DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_MAC_STAT)
                    && !DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC_STAT(unit, port_info))
                {
                    continue;
                }

                if ((logicals_type == DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_MAC_PROTECTION)
        ...
        case DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_PHY:
        {
    ...
    dnx_algo_port_diag_info_t * diag_info)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_PP_DSP(unit, diag_info->port_info))
    {
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF
        (unit, diag_info->port_info,
         DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT, OTN_PHY, FRAMER_MGMT, FEEDBACK)))
    {
    ...
    dnx_algo_port_diag_info_all_t * diag_info_all)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_PP_DSP(unit, logical_info->port_info))
    {
    ...
    dnx_algo_port_ethu_access_info_t * ethu_access_info)
{
    ...
    else
    {
        ...
        ethu_access_info->pm_id_in_ethu = -1;
        ethu_access_info->mac_id = -1;
        ethu_access_info->mac_mode_config_id = -1;
    }
    ...
    int *nif_interface_id)
{
    ...
    switch (port_info.port_type)
    {
        ...
        case DNX_ALGO_PORT_TYPE_NIF_ETH:
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, logical_port, flags, &phy_port));
...
{
    dnx_algo_port_info_s port_info;
    int is_ilkn_over_fabric = 0, is_sfi = 0;
    int first_phy;
    int ethu_id;
    ...
    uint32 *serdes_rate)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 0) || DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
    {
