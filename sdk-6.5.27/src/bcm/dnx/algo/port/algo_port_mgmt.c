    ...
    int nif_handle)
{
    ...
    nif_db.flr.link_config.link_mask =
        (BCM_PORT_FLR_CDR_LOCK_DOWN_MASK | BCM_PORT_FLR_LOS_DOWN_MASK | BCM_PORT_FLR_CDR_OOL_DOWN_MASK);
    ...
    int flexe_handle)
{
    ...
    flexe_client_db.stat_channel = -1;
    ...
    int flexe_handle)
{
    ...
    flexe_phy_db.mac1_channel = DNX_ALGO_PORT_INVALID;
    flexe_phy_db.mac2_channel = DNX_ALGO_PORT_INVALID;
    ...
    else
    {
         ...
         */
        {
            ...
            if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
            {
                /*
                 *  counter port map init
                 *  allocate db for tx counter port map
                 */
 ...
 *   \param [in] frame_preemptable - for ETH only. specify whether port is frame preemptable
    ...
    int framer_channel)
{
    ...
    if (!valid_nif_port) /** i.e. master port */
    {
            ...
            || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info, TRUE) || DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_db.nif.flr.flr_fw_support.set(unit, nif_handle, is_flr_fw_support));
            /** Set TXPI mode */
            SHR_IF_ERR_EXIT(dnx_algo_port_db.nif.ext_txpi_enable.set(unit, nif_handle, ext_txpi_enable));
            /** Set frame preemtable status **/
            SHR_IF_ERR_EXIT(dnx_algo_port_db.nif.frame_preemptable.set(unit, nif_handle, frame_preemptable));
        }
        /*
    ...
    dnx_algo_port_if_add_t * info)
{
    ...
    indicator.is_flexe_unaware = info->eth_info.is_flexe_unaware;
    indicator.is_framer_mgmt = info->eth_info.is_framer_mgmt;
    indicator.is_tdm = info->common_info.tdm_mode == DNX_ALGO_PORT_TDM_MODE_NONE ? 0 : 1;
    indicator.is_tdm_only_if = (info->common_info.if_tdm_mode == DNX_ALGO_PORT_IF_TDM_ONLY);
    indicator.is_feedback = info->eth_info.is_feedback;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_channel_free_verify(unit, logical_port, port_info, interface_id,
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_internal_add
                    (unit, logical_port, first_phy_port, port_info, info->interface_offset, info->phys,
                     ...
                     info->ilkn_info.is_elk_if, info->is_flr_fw_support,
                     info->ext_txpi_enable, info->eth_info.frame_preemptable, info->eth_info.framer_channel));
    ...
    int mgmt_channel)
{
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, TRUE, TRUE, TRUE))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.mac.mgmt_channel.set(unit, flexe_handle, mgmt_channel));
        }
    ...
    dnx_algo_port_special_interface_add_t * info)
{
    ...
    indicator.is_flexe_protection = info->framer_info.is_flexe_protection;
        ...
        if (DNX_ALGO_PORT_TYPE_IS_CPU(unit, port_info))
        {
            ...
            DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_port_channel_free_verify
                                (unit, logical_port, port_info, cpu_interface_id, info->common_info.out_channel, TRUE));
                SHR_IF_ERR_EXIT(dnx_algo_port_channel_free_verify
                                (unit, logical_port, port_info, cpu_interface_id, info->common_info.in_channel, FALSE));
            }
        }
        else if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info))
        {
                 ...
                 interface_offset < dnx_data_egr_queuing.params.nof_egr_rcy_interfaces_get(unit); interface_offset++)
            {
                ...
                SHR_IF_ERR_EXIT_WITH_LOG(dnx_algo_port_channel_free_verify
                                         (unit, logical_port, port_info,
                                          rcy_interface_id, info->common_info.in_channel, FALSE),
                ...
                SHR_IF_ERR_EXIT_WITH_LOG(dnx_algo_port_channel_free_verify
                                         (unit, logical_port, port_info,
                                          rcy_interface_id, info->common_info.out_channel, TRUE),
    ...
    dnx_algo_port_special_interface_add_t * info)
{
    ...
    indicator.is_flexe_protection = info->framer_info.is_flexe_protection;
    indicator.is_mc = info->tm_info.is_mc;
    indicator.is_framer_mgmt = info->framer_info.is_framer_mgmt;
    /*
     * Get interface id and port type
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_interface_to_port_info_get(unit, info->interface, &indicator, &port_info));
    ...
    {
        /** FLEXE */
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_add
                        (unit, logical_port, info->framer_info.client_channel, info->framer_info.mgmt_channel));
    }
    ...
    bcm_port_t logical_port)
{
    ...
    int is_lag;
    int phy;
    int framer_channel, mgmt_channel;
    ...
    if (state != DNX_ALGO_PORT_STATE_INVALID)
    {
            ...
            || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info, TRUE) || DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
        {
            ...
            /*
             * if it is OTN port or advanced FLR is enabled
             * Recycle the OTN logical lane resources.
             */
            if (DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info) || is_advanced_flr_enabled)
            {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, MGMT))
            || DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, TRUE, FALSE, FALSE))
        {
            /*
       ...
       /** IN TM Data Base */
        ...
        if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MGMT(unit, port_info))
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_framer_mgmt_channel_get(unit, logical_port, &mgmt_channel));
            SHR_IF_ERR_EXIT(dnx_algo_framer_mgmt_channel_db_init(unit, mgmt_channel));
        }
    ...
    int *header_type)
{
    ...
    {
        /** For non in-pp ports treat as ETH (relevant for TDM bypass special case) */
 ...
 */
shr_error_e
dnx_algo_port_fabric_pbmp_to_fmac_bmp_convert(
    int unit,
    bcm_pbmp_t port_bmp,
    bcm_pbmp_t * fmac_bmp)
{
    ...
    {
        if (port >= dnx_data_port.general.fabric_port_base_get(unit))
        {
            link_id = port - dnx_data_port.general.fabric_port_base_get(unit);
        }
        else
        {
            link_id = port;
        ...
        fmac_id = link_id / dnx_data_fabric.blocks.nof_links_in_fmac_get(unit);
        BCM_PBMP_PORT_ADD(*fmac_bmp, fmac_id);
    }
    ...
    int *channel_id)
{
    ...
    *channel_id = 0;
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, TRUE)
 ...
 */
shr_error_e
dnx_algo_port_framer_mgmt_channel_get(
    int unit,
    bcm_port_t logical_port,
    int *mgmt_channel)
{
    ...
    *mgmt_channel = 0;
    ...
    {
        /*
         * Get MGMT channel for busb mgmt port
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_db.logical.flexe_mac_handle.get(unit, logical_port, &handle));
        DNX_ALGO_PORT_HANDLE_VERIFY(handle);
        SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.mac.mgmt_channel.get(unit, handle, mgmt_channel));
    }
    else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_PHY(unit, port_info))
    {
        /*
         * Get MGMT channel for busa mgmt port
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_db.logical.flexe_phy_handle.get(unit, logical_port, &handle));
        DNX_ALGO_PORT_HANDLE_VERIFY(handle);
        SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.channel.get(unit, handle, mgmt_channel));
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Port %d is expected to be of flexe type.\n", logical_port);
    ...
    switch (interface_type)
    {
        ...
        case BCM_PORT_IF_ILKN:
        {
            ...
            else
            {
                ...
                if (indicator->is_tdm &&
                    !dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_bypass_port_is_ingress_tm) &&
                    ((dnx_data_tdm.params.mode_get(unit) == TDM_MODE_STANDARD)
                     || (dnx_data_tdm.params.mode_get(unit) == TDM_MODE_OPTIMIZED)))
                {
                    /** clear ingress port indication for tdm bypass ports */
                    port_info->is_ing_tm = 0;
                    port_info->is_ing_pp = 0;
                }
            }
            break;
        }
        ...
        case BCM_PORT_IF_NIF_ETH:
        {
            ...
            else if (indicator->is_feedback)
            {
                port_info->port_type = DNX_ALGO_PORT_TYPE_NIF_ETH;
                port_info->is_nif = 1;
                port_info->is_nif_eth = 1;
                port_info->is_imb = 1;
                port_info->is_feedback = 1;
            }
            else
            {
                port_info->port_type = DNX_ALGO_PORT_TYPE_NIF_ETH;
        ...
        case BCM_PORT_IF_VIRTUAL_FLEXE_CLIENT:
        {
            ...
            if (indicator->is_flexe_mac)
            {
                ...
                else if (indicator->is_flexe_protection)
                {
                    port_info->is_flexe_protection = 1;
                }
                else
                {
                    port_info->is_ing_tm = 1;
            ...
            else if (indicator->is_flexe_sar || indicator->is_otn_sar)
            {
                ...
                {
                    if (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_bypass_port_is_ingress_tm) ||
                        !((dnx_data_tdm.params.mode_get(unit) == TDM_MODE_STANDARD)
                          || (dnx_data_tdm.params.mode_get(unit) == TDM_MODE_OPTIMIZED)))
                    {
                        port_info->is_ing_tm = 1;
                    ...
                    port_info->is_pp_dsp = 1;
                    /** when egress bypass is enabled, a mc only SAR port doesn't use pp dsp for forwarding */
                    if (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_egress_bypass_enable) &&
                        indicator->is_mc)
                    {
                        port_info->is_pp_dsp = 0;
                    }
                    /** Don't mark as egr_tm in case SAR port bypasses egress pipeline */
                    if (!dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_egress_bypass_enable))
                    {
                        port_info->is_egr_tm = 1;
                        port_info->is_egr_pp = 1;
 ...
 */
shr_error_e
dnx_algo_port_l1_eth_1588_enable_get(
    int unit,
    bcm_port_t logical_port,
    uint32 flags,
    int *enable)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.rx_1588_en.get(unit, flexe_handle, enable));
    }
    else if (flags == BCM_PORT_FLEXE_TX)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.tx_1588_en.get(unit, flexe_handle, enable));
    }
    else
    {
 ...
 */
shr_error_e
dnx_algo_port_l1_eth_1588_enable_set(
    int unit,
    bcm_port_t logical_port,
    uint32 flags,
    int enable)
{
    ...
    if (flags & BCM_PORT_FLEXE_RX)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.rx_1588_en.set(unit, flexe_handle, enable));
    }
    if (flags & BCM_PORT_FLEXE_TX)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.tx_1588_en.set(unit, flexe_handle, enable));
    }
exit:
    SHR_FUNC_EXIT;
}
 ...
 */
shr_error_e
dnx_algo_port_l1_eth_1588_mac_channel_get(
    int unit,
    bcm_port_t logical_port,
    int *mac1_channel,
    int *mac2_channel)
{
    ...
    /** get data */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.mac1_channel.get(unit, flexe_handle, mac1_channel));
    SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.mac2_channel.get(unit, flexe_handle, mac2_channel));
exit:
    SHR_FUNC_EXIT;
}
/*
 ...
 */
shr_error_e
dnx_algo_port_l1_eth_1588_mac_channel_set(
    int unit,
    bcm_port_t logical_port,
    int mac1_channel,
    int mac2_channel)
{
    ...
    /** get data */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.mac1_channel.set(unit, flexe_handle, mac1_channel));
    SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.phy.mac2_channel.set(unit, flexe_handle, mac2_channel));
exit:
    SHR_FUNC_EXIT;
}
/*
 ...
 */
shr_error_e
dnx_algo_port_flexe_client_stat_channel_get(
    int unit,
    bcm_port_t logical_port,
    int *stat_channel)
{
    ...
    /** get data */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.client.stat_channel.get(unit, flexe_handle, stat_channel));
exit:
    SHR_FUNC_EXIT;
}
/*
 ...
 */
shr_error_e
dnx_algo_port_flexe_client_stat_channel_set(
    int unit,
    bcm_port_t logical_port,
    int stat_channel)
{
    ...
    /** get data */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.flexe.client.stat_channel.set(unit, flexe_handle, stat_channel));
exit:
    SHR_FUNC_EXIT;
}
    ...
    const void *data)
{
    ...
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "first_header_size_to_skip",
                                   first_header_size->first_header_size_to_skip, NULL, NULL);
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "port_type",
                                   first_header_size->port_type, NULL, NULL);
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "is_before",
                                   first_header_size->is_before, NULL, NULL);
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "is_after",
    ...
    int *is_supported)
{
    ...
    /** get data */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.nif.flr.flr_fw_support.get(unit, nif_handle, is_supported));
 ...
 */
shr_error_e
dnx_algo_port_nif_frame_preemptable_get(
    int unit,
    bcm_port_t logical_port,
    int *frame_preemptable)
{
 ...
 */
shr_error_e
dnx_algo_port_nif_flr_link_config_get(
    int unit,
    bcm_port_t logical_port,
    bcm_port_flr_link_config_t * link_config)
{
    ...
    /** get data */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.nif.flr.link_config.get(unit, nif_handle, link_config));
exit:
    SHR_FUNC_EXIT;
}
/*
 ...
 */
shr_error_e
dnx_algo_port_nif_flr_link_config_set(
    int unit,
    bcm_port_t logical_port,
    bcm_port_flr_link_config_t link_config)
{
    ...
    /** set data */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.nif.flr.link_config.set(unit, nif_handle, link_config));
exit:
    SHR_FUNC_EXIT;
}
