    ...
    int unit)
{
     ...
     */
    {
        ...
        /** Number of ingress predefined traps (highest legal HW trap ID + 1) */
        data.nof_elements = dnx_data_trap.ingress.max_legal_predefined_trap_id_get(unit) + 1;
        data.flags = 0;
    ...
    int unit)
{
    ...
    int profile_id, ind;
    int egress_divided_to_erpp_and_etpp = dnx_data_pp.stages.is_erpp_supported_get(unit);
    ...
    if (egress_divided_to_erpp_and_etpp)
    {
        ...
        SHR_IF_ERR_EXIT(algo_rx_db.trap_recycle_cmd.exchange
                        (unit, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID, 1, &profile, 0, NULL, &profile_id, &is_first,
                         &is_last));
    }
    ...
    SHR_IF_ERR_EXIT(algo_rx_db.trap_recycle_cmd.exchange
                    (unit, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID, 1, &profile, 0, NULL, &profile_id, &is_first, &is_last));
