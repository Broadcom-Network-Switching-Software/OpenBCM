...
/*
 * define OUTER/INNER to be used as indexes for the tag/grain layers
 * OUTER being the bigger (parent) grain and INNER the child grain 
 */
#define INNER 1
#define OUTER 0
    ...
    int nof_tags_levels)
{
    if (nof_tags_levels > 1)
    {
         ...
         */
        return (max_tags_value[INNER] + 2) * (max_tags_value[OUTER] + 1) + 1;
    }
    else
    {
        /*
    ...
    uint32 tags[RESOURCE_TAG_BITMAP_MAX_NOF_TAGS_LEVELS])
{
    ...
    /*
     * If its a single layer, then just return the tag.
     * Empty list will be max_tag + 1 which is already encoded by the caller to this function
     */
    if (res_tag_bitmap_fast->resource->nof_tags_levels == 1)
    {
        return tags[OUTER];
    }

    /*
     * If OUTER level is requested as empty then we return the last head
     * It implies also that INNER level should be empty
     */
    if (tags[OUTER] == _rtb_fast_get_empty_group(res_tag_bitmap_fast, OUTER))
    {
        return (res_tag_bitmap_fast->resource->tag_level_info[INNER].max_tag_value +
                2) * (res_tag_bitmap_fast->resource->tag_level_info[OUTER].max_tag_value + 1);
    }

    /*
     * max_tag_value+2 to account for the zero tag (nof tags = max_tag_value + 1), and the empty list for the inner tag layer
     */
    return tags[OUTER] * (res_tag_bitmap_fast->resource->tag_level_info[INNER].max_tag_value + 2) + tags[INNER];
}
    ...
    int range_start)
{
    ...
    if (grain_found && grain_idx < range_end)
    {
        ...
        {
            SHR_IF_ERR_EXIT(sw_state_ll_next_node(unit, node_id, res_tag_bitmap_fast->free_grains,
...
_rtb_fast_free_verify(
    ...
    int element)
{
    int grain_idx, tag_level, grain_size, tmp_element;
    uint8 is_allocated;
    ...
    {
        grain_idx = resource_tag_bitmap_utils_get_grain_index(res_tag_bitmap_fast->resource, tag_level, element);
        grain_size =
            res_tag_bitmap_fast->
            resource->tag_level_info[resource_tag_bitmap_utils_get_higher_tag_level(res_tag_bitmap_fast->resource)].
            grain_size;
        ...
        /*
         * Return error if nof_elements_to_free is greater than the the remaining number of elements in the grain, starting from provided element.
         */
        if (nof_elements_to_free > (grain_size - (element % grain_size)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Cannot free bits in range that spans multiple grains. Attempted to free %d bits, but the grain only has %d bits starting from %d (grain size is %d bits).\n",
                         nof_elements_to_free, (grain_size - (element % grain_size)), element, grain_size);
        }

        if (nof_elements_to_free >
            res_tag_bitmap_fast->resource->tag_level_info[tag_level].nof_allocated_elements_per_grain[grain_idx])
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Cannot free more bits in a grain than are currently allocated in that grain. Attempted to free %d bits, but the grain has only %d allocated bits (grain size is %d bits).\n",
                         nof_elements_to_free,
                         res_tag_bitmap_fast->resource->
                         tag_level_info[tag_level].nof_allocated_elements_per_grain[grain_idx], grain_size);
        }

        for (tmp_element = element; tmp_element < element + nof_elements_to_free; tmp_element++)
        {
            SHR_IF_ERR_EXIT(resource_tag_bitmap_utils_is_allocated
                            (unit, node_id, res_tag_bitmap_fast->resource, tmp_element, &is_allocated));
            if (!is_allocated)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Cannot free bits that have not been allocated. Attempted to free element %d but it was already free.\n",
                             tmp_element);
            }
        }
    }
    ...
    uint8 *last_reference)
{
    ...
    uint8 grain_found = FALSE, has_forced_tag = FALSE;
    ...
    if (nof_free_elements > 0)
    {
        ...
        {
            if (res_tag_bitmap_fast->resource->
                tag_level_info[OUTER].nof_allocated_elements_per_grain[grains_idx[OUTER]] == 0)
            {
                if (_SHR_IS_FLAG_SET
                    (res_tag_bitmap_fast->resource->flags, RESOURCE_TAG_BITMAP_CREATE_ALLOW_FORCING_TAG))
                {
                    DNX_SW_STATE_BIT_GET(unit, node_id, 0,
                                         res_tag_bitmap_fast->resource->tag_level_info[OUTER].forced_tag_indication,
                                         grains_idx[OUTER], &has_forced_tag);
                }
                /*
                 * If now-empty grain has force_tag, do not move it to the {empty,empty} LL.
                 */
                if (!has_forced_tag)
                {
                    SHR_IF_ERR_EXIT(_rtb_fast_move_grain
                                    (unit, node_id, res_tag_bitmap_fast, grain_node, free_ll_head_idx,
                                     res_tag_bitmap_fast->empty_groups_ll_head_idx));
                }
            }
            /*
             * valid only for multilevel_tags_resource
             */
            else
            {
                if (_SHR_IS_FLAG_SET
                    (res_tag_bitmap_fast->resource->flags, RESOURCE_TAG_BITMAP_CREATE_ALLOW_FORCING_TAG))
                {
                    ...
                    do
                    {
                        DNX_SW_STATE_BIT_GET(unit, node_id, 0,
                                             res_tag_bitmap_fast->resource->
                                             tag_level_info[tag_level].forced_tag_indication, grains_idx[tag_level],
                                             &has_forced_tag);
                        tag_level++;
                    }
                    while ((tag_level < res_tag_bitmap_fast->resource->nof_tags_levels) && has_forced_tag);
                }
    ...
    else
    {
        ...
        else if (res_tag_bitmap_fast->resource->
                 tag_level_info[OUTER].nof_allocated_elements_per_grain[grains_idx[OUTER]] == 0)
        {
            if (_SHR_IS_FLAG_SET(res_tag_bitmap_fast->resource->flags, RESOURCE_TAG_BITMAP_CREATE_ALLOW_FORCING_TAG))
            {
                DNX_SW_STATE_BIT_GET(unit, node_id, 0,
                                     res_tag_bitmap_fast->resource->tag_level_info[OUTER].forced_tag_indication,
                                     grains_idx[OUTER], &has_forced_tag);
            ...
            /*
             * If now-empty grain has force_tag, move it to {tag,tag} LL (instead of {empty,empty} LL) even though it is now empty.
             */
            if (has_forced_tag)
            {
                SHR_IF_ERR_EXIT(sw_state_ll_add
                                (unit, node_id, res_tag_bitmap_fast->free_grains, free_ll_head_idx,
                                 &grains_idx[resource_tag_bitmap_utils_get_higher_tag_level
                                             (res_tag_bitmap_fast->resource)], NULL));
            }
            else
            {
                SHR_IF_ERR_EXIT(sw_state_ll_add
        ...
        /*
         * valid only for multilevel_tags_resource, the following condition holds:
         * res_tag_bitmap_fast->resource->tag_level_info[INNER].nof_allocated_elements_per_grain[grains_idx[INNER]] == 0
         */
        else
        {
            if (_SHR_IS_FLAG_SET(res_tag_bitmap_fast->resource->flags, RESOURCE_TAG_BITMAP_CREATE_ALLOW_FORCING_TAG))
            {
                ...
                {
                    DNX_SW_STATE_BIT_GET(unit, node_id, 0,
                                         res_tag_bitmap_fast->resource->tag_level_info[tag_level].forced_tag_indication,
                                         grains_idx[tag_level], &has_forced_tag);
                    tag_level++;
                }
                while ((tag_level < res_tag_bitmap_fast->resource->nof_tags_levels) && has_forced_tag);
            }
     ...
     */
    if (resource_tag_bitmap_utils_is_multilevel_tags_resource(res_tag_bitmap_fast->resource)
        && res_tag_bitmap_fast->resource->tag_level_info[OUTER].nof_allocated_elements_per_grain[grains_idx[OUTER]] ==
        0)
    {
        /*
...
/*
 * Check if the tag set operation should also move a grain (and if so from where).
 * It also verifies the validity of the requested tag set operation.
 * Valid cases:
 *   1- Grain is empty and the request is to force tags.
 *   2- Grain is empty and the request is not to force tags and the grain didn't already have forced tags.
 *   3- Grain is not empty and the request is to force tags and the grain didn't already have forced tags.
 * All other cases are invalid and an error will be returned.
 */
static shr_error_e
_rtb_fast_tag_set_should_move(
    int unit,
    uint32 node_id,
    ...
    uint8 *move_grain)
{
    int tag_level, is_forced;
    uint8 res, allow_set = FALSE;
    resource_tag_bitmap_utils_tag_info_t existing_tag_info;

    SHR_FUNC_INIT_VARS(unit);

    /*
     * Get the existing tags assigned to the grain (if exist).
     */
    sal_memset(&existing_tag_info, 0, sizeof(existing_tag_info));
    existing_tag_info.element = tag_set_info->element;
    SHR_IF_ERR_EXIT(resource_tag_bitmap_fast_tag_get(unit, node_id, *rtb_fast, &existing_tag_info));

    /*
     * Grain is empty.
     */
    if ((*rtb_fast)->resource->tag_level_info[resource_tag_bitmap_utils_get_higher_tag_level
                                              ((*rtb_fast)->resource)].nof_allocated_elements_per_grain[grain_idx] == 0)
    {
        /*
         * Check if the grain already has forced tags.
         */
        is_forced = 0;
        if (_SHR_IS_FLAG_SET((*rtb_fast)->resource->flags, RESOURCE_TAG_BITMAP_CREATE_ALLOW_FORCING_TAG))
        {
            for (tag_level = 0; tag_level < (*rtb_fast)->resource->nof_tags_levels; tag_level++)
            {
                res = 0;
                DNX_SW_STATE_BIT_GET(unit, node_id, 0,
                                     (*rtb_fast)->resource->tag_level_info[tag_level].forced_tag_indication, grain_idx,
                                     &res);
                if (res)
                {
                    is_forced++;
        ...
        /*
         * If the grain doesn't have forced tags (and is empty), it must be in the {empty,empty} LL and must be moved.
         */
        if (is_forced != (*rtb_fast)->resource->nof_tags_levels)
        {
            *move_grain = TRUE;
            *from_ll_head_idx = (*rtb_fast)->empty_groups_ll_head_idx;
        }
        /*
         * If the grain has forced tags and the set command also has forced_tags (and the grain is empty),
         * it must be in the {existing_tag,existing_tag} LL and moved to {new_tag,new_tag} LL (unless they are the same).
         */
        else if (tag_set_info->force_tag
                 && (_SHR_IS_FLAG_SET((*rtb_fast)->resource->flags, RESOURCE_TAG_BITMAP_CREATE_ALLOW_FORCING_TAG)))
        {
            for (tag_level = 0; tag_level < (*rtb_fast)->resource->nof_tags_levels; tag_level++)
            {
                /*
                 * Only need to move the grain if the existing tags differ from the new tags.
                 */
                if (existing_tag_info.tags[tag_level] != tag_set_info->tags[tag_level])
                {
                    *move_grain = TRUE;
                    *from_ll_head_idx = _rtb_get_ll_head_idx_by_tags(*rtb_fast, existing_tag_info.tags);
                    SHR_EXIT();
                }
            }
        }
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Cannot change the tags without force_tag of an empty grain that already had forced tags.\n");
        }
    ...
    {
        /*
         * If the request is to force_tags for the grain, check if the tags match the grain's existing tags and if the grain doesn't already have set tags.
         * If all of those conditions are met, tag set request is legal, otherwise return error.
         */
        if (tag_set_info->force_tag
            && (_SHR_IS_FLAG_SET((*rtb_fast)->resource->flags, RESOURCE_TAG_BITMAP_CREATE_ALLOW_FORCING_TAG)))
        {
            is_forced = 0;
            for (tag_level = 0; tag_level < (*rtb_fast)->resource->nof_tags_levels; tag_level++)
            {
                /*
                 * Check if the requested tags match the existing tags and if not return error.
                 */
                if (existing_tag_info.tags[tag_level] != tag_set_info->tags[tag_level])
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot change the tags of a grain that is not empty.\n\n");
                }

                /*
                 * Check if the grain already had forced tags or not.
                 */
                res = 0;
                DNX_SW_STATE_BIT_GET(unit, node_id, 0,
                                     (*rtb_fast)->resource->tag_level_info[tag_level].forced_tag_indication, grain_idx,
            ...
            /*
             * If the grain doesn't already have forced tags, tag set request is legal.
             */
            if (is_forced != (*rtb_fast)->resource->nof_tags_levels)
            {
                allow_set = TRUE;
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Cannot change the tags of a grain that is not empty, except to force_tag the existing tags if not already forced.\n\n");
        }
    }
    ...
    uint32 alloc_flags)
{
    ...
    uint32 empty_tags[RESOURCE_TAG_BITMAP_MAX_NOF_TAGS_LEVELS];
    SHR_FUNC_INIT_VARS(unit);
    ...
    /*
     * The list of the {empty,empty} group will be at the last ll head
     */
    empty_tags[INNER] = _rtb_fast_get_empty_group(*rtb_fast, INNER);
    empty_tags[OUTER] = _rtb_fast_get_empty_group(*rtb_fast, OUTER);
    (*rtb_fast)->empty_groups_ll_head_idx = _rtb_get_ll_head_idx_by_tags(*rtb_fast, empty_tags);
    ...
    int *elem)
{
    ...
    else
    {
        ...
        RESOURCE_TAG_BITMAP_ERR_EXIT(alloc_info->flags, _SHR_E_RESOURCE,
                                     ...
                                     allocated_block_length, alloc_info->align, alloc_info->offs[OUTER],
                                     alloc_info->tags[OUTER], alloc_info->tags[INNER]);
    }
exit:
    SHR_FUNC_EXIT;
}
...
shr_error_e
resource_tag_bitmap_fast_tag_set(
    int unit,
    uint32 node_id,
    void *res_tag_bitmap_fast,
    void *tag_info)
{
    int tag_level, from_ll_head_idx = 0, to_ll_head_idx, grain_idx;
    ...
    uint8 grain_found = FALSE;
    sw_state_ll_node_t grain_node = SW_STATE_LL_INVALID;
    uint8 move_grain = FALSE;

    SHR_FUNC_INIT_VARS(unit);

    /*
     * Get the index of the grain whose tags should be set.
     */
    grain_idx =
        resource_tag_bitmap_utils_get_grain_index(rtb_fast->resource,
                                                  resource_tag_bitmap_utils_get_higher_tag_level(rtb_fast->resource),
                                                  tag_set_info.element);

    /*
     * Checks whether a grain should be moved and if so from where (which LL).
     * Additionally, verifies the legality of the attempted tag set operation.
     */
    SHR_IF_ERR_EXIT(_rtb_fast_tag_set_should_move
                    (unit, node_id, &rtb_fast, &tag_set_info, grain_idx, &from_ll_head_idx, &move_grain));
    ...
    for (tag_level = 0; tag_level < rtb_fast->resource->nof_tags_levels; tag_level++)
    {
        SHR_IF_ERR_EXIT(resource_tag_bitmap_utils_tag_set(unit,
                                                          ...
                                                          tag_set_info.element - rtb_fast->resource->first_element,
                                                          tag_set_info.nof_elements, tag_set_info.first_references));
    }

    /*
     * If grain needs to be moved, move it.
     */
    if (move_grain)
    {
        to_ll_head_idx = _rtb_get_ll_head_idx_by_tags(rtb_fast, tag_set_info.tags);
        SHR_IF_ERR_EXIT(sw_state_ll_find
                        (unit, node_id, rtb_fast->free_grains, from_ll_head_idx, &grain_node, (void *) (&(grain_idx)),
                         &grain_found));

        SHR_IF_ERR_EXIT(_rtb_fast_move_grain(unit, node_id, rtb_fast, grain_node, from_ll_head_idx, to_ll_head_idx));
    }
    ...
    dnx_algo_res_dump_data_t * data)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_ll_print(unit, res_mngr->free_grains));
#endif
    ...
    void *extra_argument)
{
    ...
    DNXC_VERIFY_INVOKE(_rtb_fast_free_verify(unit, node_id, res_tag_bitmap_fast, nof_elements, elem));
...
shr_error_e
resource_tag_bitmap_fast_data_per_entry_set(
    int unit,
    uint32 node_id,
    void *res_tag_bitmap_fast,
    int element,
...
/*
 * undef these macros instead they are used in different context in other file.
 */
#undef INNER
#undef OUTER
