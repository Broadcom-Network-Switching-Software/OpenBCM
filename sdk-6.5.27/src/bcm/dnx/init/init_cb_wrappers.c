...
#if defined(BCM_WARM_BOOT_SUPPORT)
    if (dnxc_sw_state_is_auto_sync_supported_get(unit))
    {

#if defined(INCLUDE_KBP)
        SHR_IF_ERR_EXIT(kbp_mngr_wb_sync(unit));
#endif
        SHR_IF_ERR_EXIT(bcm_common_sat_wb_sync(unit, 0));
        SHR_IF_ERR_EXIT(soc_scache_commit(unit));
    }
#endif
