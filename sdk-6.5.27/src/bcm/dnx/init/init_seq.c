...
#include <bcm_int/dnx/auto_generated/dnx_gdpll_dispatch.h>
#endif
...
#include <soc/dnx/swstate/dnx_sw_state_example_linked_list.h>
 ...
 */
#define MANDATORY_DEINIT          DNX_INIT_STEP_F_MANDATORY_DEINIT
/**
 * \brief
    ...
    int *dynamic_flags)
{
    ...
    return l1_mode_skip(unit, dynamic_flags);
}
...
/** Skip all DRAM steps in devices and SKUs without DRAM */
static shr_error_e
dram_init_skip(
    int unit,
    int *dynamic_flags)
{
    ...
    *dynamic_flags = 0;
    if (dnx_data_dram.general_info.max_nof_drams_get(unit) == 0)
    {
        *dynamic_flags = DNX_INIT_STEP_F_SKIP;
    }
    ...
    int *dynamic_flags)
{
    ...
    *dynamic_flags = 0;
    if ((dnx_data_tsn.general.feature_get(unit, dnx_data_tsn_general_is_supported) == 0)
    ...
    int *dynamic_flags)
{
    if (!dnx_data_nif.framer.feature_get(unit, dnx_data_nif_framer_is_supported) &&
        !dnx_data_nif.framer.feature_get(unit, dnx_data_nif_framer_power_down_supported))
    {
        *dynamic_flags = DNX_INIT_STEP_F_SKIP | DNX_INIT_STEP_F_VERBOSE;
    }
    else
...
/**
 * Skip the initiallization of TSN module if TSN is not supported
 */
static shr_error_e
tsn_skip(
    int unit,
...
/**
 * Callback function which skips the step if we are not running tests of example linked list module
 */
static shr_error_e
dnx_sw_state_example_linked_list_flag_cb(
    int unit,
    int *flags)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    *flags = 0;
    if (!dnx_data_infra_tests.
        linked_list_example.feature_get(unit, dnx_data_infra_tests_linked_list_example_init_enable))
    {
        *flags = DNX_INIT_STEP_F_SKIP;
    }
...
static shr_error_e
dnx_error_recovery_regression_flag_cb(
    int unit,
    int *flags)
{
...
static const  dnx_init_step_t dnx_init_deinit_common_seq[] = {
  /*STEP_ID,                      STEP_NAME,       STEP_INIT,                  STEP_DEINIT,                 FLAG_CB    STEP_FLAGS                                  SUB_STEP_LIST_PTR */
  {DNX_INIT_STEP_PEMLA,           "PEMLA",         dnx_init_pemla_init,        dnx_init_pemla_deinit,       NULL,      DISABLE_WB_CHECK | TIME_PER_DEV | USE_DMA,  NULL},
...
static const  dnx_init_step_t dnx_init_deinit_tm_seq[] = {
  ...
  {DNX_INIT_STEP_DRAM,                       "Dram",               dnx_dram_init,               dnx_dram_deinit,          dram_init_skip,   WB_SKIP | TIME_PER_DEV,                            NULL},
  {DNX_INIT_STEP_DRAM_BUFFERS,               "Dram Buffers",       dnx_dram_buffers_init,       NULL,                     dram_init_skip,   WB_SKIP,                                           NULL},
  ...
  {DNX_INIT_STEP_PORT_MODULE,                "PORT",               dnx_port_init,               dnx_port_deinit,          NULL,             ACCESS_PORT | DISABLE_WB_CHECK | MANDATORY_DEINIT, NULL},
  ...
  {DNX_INIT_STEP_LINKSCAN,                   "LINKSCAN",           dnx_linkscan_init,           dnx_linkscan_deinit,      NULL,             NEVER_SKIP_DEINIT | MANDATORY_DEINIT,              NULL},
  {DNX_INIT_STEP_FRAMER,                     "Framer",             dnx_framer_init,             dnx_framer_deinit,        framer_skip,      0,                                                 NULL},
  {DNX_INIT_STEP_LAST_STEP,                  "Last Step",          NULL,                        NULL,                     NULL,             0,                                                 NULL}
};
...
static const dnx_init_step_t dnx_init_deinit_pp_seq[] = {
  ...
  {DNX_INIT_STEP_ARR,                    "ARR",                   dnx2_arr_output_init,                     NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_PP_GENERAL,             "PP General",            dnx_general_pp_init,                      NULL,                       NULL,            WB_SKIP | TIME_PER_DEV,  NULL},
  {DNX_INIT_KLEAP_INFO,                  "iKleap info",           dnx2_kleap_layers_info_init,              NULL,                       l1_mode_skip,    0,                       NULL},
  {DNX_INIT_STEP_HW_OVERWRITE,           "HW Overwrites",         dnx_hw_overwrite_init,                    NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_LIF,                    "Lif",                   dnx_lif_lib_init,                         NULL,                       l1_mode_skip,    TIME_PER_DEV,            NULL},
  {DNX_INIT_STEP_FAILOVER,               "Failover",              dnx_failover_init,                        NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_IN_LIF_PROFILE_MODULE,  "InLIF Profile",         dnx_in_lif_profile_module_init,           NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_OUT_LIF_PROFILE_MODULE, "OutLIF Profile",        dnx_out_lif_profile_module_init,          NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_L2_MODULE,              "L2",                    dnx_l2_module_init,                       dnx_l2_module_deinit,       bare_metal_skip, 0,                       NULL},
  {DNX_INIT_STEP_L3_MODULE,              "L3",                    dnx_l3_module_init,                       NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_QOS,                    "Qos",                   dnx_qos_init,                             NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_SWITCH_MODULE,          "switch",                dnx_switch_pp_init,                       NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_MPLS,                        "Mpls",                  dnx_mpls_module_init,                     NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_PP_PORT,                "Port",                  dnx_pp_port_init,                         NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_STG,                    "Stg",                   dnx_stg_init,                             NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_SRV6,                   "SRv6",                  dnx_srv6_init,                            NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  ...
  {DNX_INIT_STEP_PTP,                    "Ptp",                   dnx_ptp_init,                             dnx_ptp_deinit,             l1_mode_skip,    0,                       NULL},
#if defined(INCLUDE_GDPLL)
  {DNX_INIT_STEP_GDPLL,                  "Gdpll",                 bcm_dnx_gdpll_init,                       bcm_dnx_gdpll_deinit,       NULL,            0,                       NULL},
#endif
  ...
  {DNX_INIT_STEP_STAT_PP,                "Stat pp",               dnx_stat_pp_init,                         NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_INSTRU,                 "Instrumentation",       dnx_instru_init,                          NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_TSN,                    "TSN",                   dnx_tsn_init,                             NULL,                       tsn_skip,        WB_SKIP,                 NULL},
  {DNX_INIT_STEP_REFLECTOR,              "Reflector",             dnx_switch_reflector_init,                NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_EVENTOR,                "Eventor",               dnx_instru_eventor_init,                  NULL,                       eventor_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_FLOW,                   "flow",                  dnx_flow_init,                            dnx_flow_deinit,            l1_mode_skip,    0,                       NULL},
  {DNX_INIT_STEP_BIER_PP,                "bier",                  dnx_bier_pp_init,                         NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_VXLAN,                  "Vxlan",                 dnx_vxlan_module_init,                    NULL,                       l1_mode_skip,    WB_SKIP,                 NULL},
  {DNX_INIT_STEP_LAST_STEP,              "Last Step",             NULL,                                     NULL,                       NULL,            0,                       NULL}
};
...
static const  dnx_init_step_t dnx_init_deinit_sbus_seq[] = {
  /*STEP_ID,                        STEP_NAME,                    STEP_INIT,                                  STEP_DEINIT,                            FLAG_CB           STEP_FLAGS                   SUB_STEP_LIST_PTR */
  {DNX_INIT_STEP_SCHAN,             "Schan",                      soc_dnxc_schan_config_and_init,             soc_schan_deinit,                       NULL,             ACCESS,                      NULL},
  {DNX_INIT_STEP_ENDIANNESS_CONFIG, "Endianness configuration",   dnx_drv_soc_dnx_endian_config,              NULL,                                   NULL,             ACCESS,                      NULL},
  {DNX_INIT_STEP_RING_CONFIG,       "Sbus ring Config",           dnx_drv_soc_dnx_ring_config,                NULL,                                   NULL,             ACCESS|STANDALONE_EN,        NULL},
  {DNX_INIT_STEP_MDIO,              "MDIO",                       dnx_drv_soc_dnx_init_reset_cmic_regs,       NULL,                                   NULL,             ACCESS,                      NULL},
  {DNX_INIT_STEP_LAST_STEP,         "Last Step",                  NULL,                                       NULL,                                   NULL,             0,                           NULL}
};
...
static const  dnx_init_step_t dnx_init_deinit_access_seq[] = {
  ...
  {DNX_INIT_STEP_ADAPTER_REG_ACC,   "Adapter",           dnxc_adapter_init,              dnxc_init_adapter_reg_access_deinit,  NULL,     ACCESS | DISABLE_WB_CHECK,                       NULL},
#endif
  {DNX_INIT_STEP_CORE_CLOCK_VERIFY, "Core Clock Verify", dnx_init_core_clock_verify,     NULL,                                 NULL,     ACCESS | WB_SKIP | STANDALONE_EN,                NULL},
  {DNX_INIT_STEP_PRE_PLL,           "Pre Reset PLL",     soc_dnx_pre_reset_pll_init,     NULL,                                 NULL,     ACCESS | STANDALONE_EN | WB_SKIP,                NULL},
...
static const  dnx_init_step_t dnx_init_deinit_algo_seq[] = {
  ...
  {DNX_INIT_STEP_VLAN_ALGO_RES,         "Algo Vlan Res",                    dnx_algo_vlan_res_init,                     NULL,                     l1_mode_skip, WB_SKIP,                 NULL},
  ...
  {DNX_INIT_STEP_ALGO_L2,               "Algo L2",                          dnx_algo_l2_init,                           NULL,                     l1_mode_skip, WB_SKIP,                 NULL},
  {DNX_INIT_STEP_ALGO_L3,               "Algo L3",                          dnx_algo_l3_init,                           dnx_algo_l3_deinit,       l1_mode_skip, WB_SKIP,                 NULL},
  {DNX_INIT_STEP_ALGO_MPLS,             "Algo MPLS",                        dnx_algo_mpls_init,                         NULL,                     l1_mode_skip, WB_SKIP,                 NULL},
  {DNX_INIT_STEP_ALGO_VLAN,             "Algo VLAN",                        dnx_algo_vlan_init,                         dnx_algo_vlan_deinit,     l1_mode_skip, WB_SKIP,                 NULL},
  ...
  {DNX_INIT_STEP_ALGO_MIB,              "Algo MIB",                         dnx_algo_mib_init,                          NULL,                     NULL,         WB_SKIP,                 NULL},
  {DNX_INIT_STEP_ALGO_FLEXE_GENERAL,    "Algo FlexE General",               dnx_algo_flexe_general_init,                NULL,                     framer_skip,  WB_SKIP,                 NULL},
  {DNX_INIT_STEP_ALGO_FLEXE_CORE,       "Algo FlexE core",                  dnx_algo_flexe_core_init,                   NULL,                     framer_skip,  WB_SKIP,                 NULL},
...
static const dnx_init_step_t dnx_init_deinit_swstate_seq[] = {
  /*STEP_ID,                                   STEP_NAME,                           STEP_INIT,                             STEP_DEINIT,         FLAG_CB                                   STEP_FLAGS                                SUB_STEP_LIST_PTR */
  {DNX_INIT_STEP_SW_STATE_GENERAL,             "SW State General",                  dnx_sw_state_init,                     dnx_sw_state_deinit, NULL,                                     ACCESS | TIME_PER_DEV | MANDATORY_DEINIT, NULL},
#ifdef INCLUDE_CTEST
  ...
  {DNX_INIT_STEP_SW_STATE_EXAMPLE_LINKED_LIST, "SW State Linked List Example",      dnx_sw_state_example_linked_list_init, NULL,                dnx_sw_state_example_linked_list_flag_cb, 0,                                        NULL},
#endif
  {DNX_INIT_STEP_LAST_STEP,                    "Last Step",                         NULL,                                  NULL,                NULL,                                     0,                                        NULL}
};


static const dnx_init_step_t dnx_init_deinit_error_recovery_seq[] = {
  /*STEP_ID,                                   STEP_NAME,                            STEP_INIT,                             STEP_DEINIT,                    FLAG_CB                                STEP_FLAGS                                    SUB_STEP_LIST_PTR */
  {DNX_INIT_STEP_ERROR_RECOVERY_GENERAL,       "Error Recovery General",             dnx_err_recovery_module_init,          dnx_err_recovery_module_deinit, wb_test_mode_skip,                     0,                                            NULL},
  {DNX_INIT_STEP_ERROR_RECOVERY_REGRESSION,    "Error Recovery Regression",          dnx_er_regress_init,                   dnx_er_regress_deinit,          dnx_error_recovery_regression_flag_cb, 0,                                            NULL},
  {DNX_INIT_STEP_LAST_STEP,                    "Last Step",                          NULL,                                  NULL,                           NULL,                                  0,                                            NULL}
};
...
static const dnx_init_step_t dnx_init_deinit_appl_properties_seq[] = {
  ...
  {DNX_INIT_STEP_VISIBILITY,             "Configure visibility",        dnx_visibility_init,                dnx_visibility_deinit,  l1_mode_skip,  TIME_PER_DEV,                    NULL},
...
static const  dnx_init_step_t dnx_init_deinit_pre_soft_init_seq[] = {
  /*STEP_ID,                          STEP_NAME,             STEP_INIT,                                  STEP_DEINIT,               FLAG_CB                      STEP_FLAGS                             SUB_STEP_LIST_PTR */
  {DNX_INIT_STEP_POST_PLL,            "Post Reset PLL",      soc_dnx_post_reset_pll_init,                NULL,                      NULL,                        ACCESS | STANDALONE_EN | WB_SKIP,      NULL},
  {DNX_INIT_STEP_BLOCKS_ENABLE,       "Blocks enable",       dnxc_drv_soc_block_enable_set,              NULL,                      NULL,                        ACCESS,                                NULL},
  {DNX_INIT_STEP_DRAM_INITIAL_CONFIG, "DRAM initial config", dnx_dram_initial_config,                    NULL,                      dram_init_skip,              ACCESS | STANDALONE_EN | TIME_PER_DEV, NULL},
  ...
  {DNX_INIT_STEP_BRDC_CONFIG,         "Broadcast config",    dnxc_drv_soc_dnx_drv_sbus_broadcast_config, NULL,                      NULL,                        ACCESS | WB_SKIP,                      NULL},
  ...
  {DNX_INIT_STEP_MEM_DEFAULTS,        "Memories",            dnx_init_mems,                              NULL,                      NULL,                        WB_SKIP | TIME_PER_DEV,                NULL},
...
static const  dnx_init_step_t dnx_init_deinit_thread_seq[] = {
    /*          STEP_ID,                      STEP_NAME,                    STEP_INIT,                                STEP_DEINIT,                                 FLAG_CB             STEP_FLAGS         SUB_STEP_LIST_PTR */
    {DNX_INIT_STEP_TEMPERATURE_MONITOR,       "Dram Temperature Monitor",   dnx_hbmc_temp_monitor_init,               dnx_hbmc_temp_monitor_deinit,                dram_init_skip,     MANDATORY_DEINIT,  NULL},
    {DNX_INIT_STEP_CRPS_EVICTION,             "CRPS Eviction",              dnx_crps_eviction_init,                   dnx_crps_eviction_deinit,                    0,                  0,                 NULL},
    {DNX_INIT_STEP_MIB_THREAD,                "Mib thread",                 dnx_init_mib_thread_start,                dnx_init_mib_thread_stop,                    NULL,               MANDATORY_DEINIT,  NULL},
    {DNX_INIT_STEP_L2_FLUSH_TRAVERSE_THREAD,  "L2 Flush Traverse thread",   dnx_l2_traverse_thread_start,             dnx_l2_traverse_thread_stop,                 l1_mode_skip,       0,                 NULL},
    ...
    {DNX_INIT_STEP_TSN_THREAD,                "TSN profile manager thread", dnx_tsn_thread_init,                      dnx_tsn_thread_deinit,                       sw_tsn_thread_skip, 0,                 NULL},
    {DNX_INIT_STEP_LAST_STEP,                 "Last Step",                  NULL,                                     NULL,                                        NULL,               0,                 NULL}
};
...
#ifdef BCM_ACCESS_SUPPORT  
  /** This step has to be after DNX Data and before DBAL*/
  {DNX_INIT_STEP_MEM_SW_INIT_VALUES, "Mem SW Init Values",           access_init_mem_sw_init_values,   NULL,                            NULL,              ACCESS,                                                 NULL},
#endif
  {DNX_INIT_STEP_SW_STATE,           "SW State",                     NULL,                             NULL,                            NULL,              ACCESS | TIME_PER_DEV,                                  dnx_init_deinit_swstate_seq},
  {DNX_INIT_STEP_ERROR_RECOVERY,     "Error Recovery",               NULL,                             NULL,                            NULL,              0,                                                      dnx_init_deinit_error_recovery_seq},
