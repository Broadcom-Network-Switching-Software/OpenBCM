    ...
    int alloc_ver)
{
    ...
    static int *edk_cache[EDK_SEQ_MAX] = { 0 };
    int *scache_ptr;
#endif

    SHR_FUNC_INIT_VARS(unit);
    SHR_SET_CURRENT_ERR(BCM_E_UNAVAIL);

#ifdef BCM_WARM_BOOT_SUPPORT
    if ((size > EDK_SEQ_MAX_BYTES) || (seq >= EDK_SEQ_MAX))
    {
        SHR_ERR_EXIT(BCM_E_PARAM, "edk_init:: _edk_sdk_scache_get ERROR in param");
    }

    if (edk_cache[seq] == NULL)
    {
        SHR_ALLOC_ERR_EXIT(edk_cache[seq], EDK_SEQ_MAX_BYTES, "EDK", "%s%s%s",
                           "edk_init:: _edk_sdk_scache_get ERROR in alloc", EMPTY, EMPTY);
    }
    ...
    if ((alloc == 0) && (size == 0))
    {
        ...
        for (int idx = 0; idx < size; idx++)
        {
            ...
            {
                SHR_ERR_EXIT(BCM_E_INTERNAL, "edk_init:: _edk_sdk_scache_get ERROR in read seq:%d idx:%d", seq, idx);
            }
            scache_ptr++;
        }
    }
...
shr_error_e
dnx_edk_state_deinit(
    int unit)
{
    int rv = BCM_E_NONE;
#ifdef EDKHLIB
    char *argv_list[] = { "edk_main", "shutdown" };
    int argc = 2;

    rv = edk_main(argc, argv_list);
    if (BCM_FAILURE(rv))
    {
        cli_out("edk_init:: dnx_edk_state_deinit ERROR in edk_main() for exit, ret:%d\n", rv);
        goto exit;
    }
exit:
#endif
