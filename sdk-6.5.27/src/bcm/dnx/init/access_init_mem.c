    ...
    access_local_regmem_id_t mem)
{
    ...
    switch (rm_info->global_regmem_id)
    {
             ...
             * The following memories can not be accessed using the indirect mechanism, and do not need to be initialized with all the memories. Phymod handles them.
             */
        case mCLPORT_WC_UCMEM_DATA:
        case mSPEED_ID_TABLE:
        case mSPEED_PRIORITY_MAP_TBL:
        case mTX_TWOSTEP_1588_TS:
        case mTX_LKUP_1588_MEM_100G_KR1_2XN:
        case mRSFEC_SYMBOL_ERROR_MIB:
        ...
        case mMACSEC_ESEC_SC_MAP_TABLE:
        ...
        case mMACSEC_ISEC_SVTAG_SC_INDEX_MAP_TABLE:
        ...
        case mDC3MAC_MIB_MEM:
        case mDC3PORT_TSC_UCMEM_DATA:
        case mDC3PORT_INTR_MASK:
        case mDC3PORT_INTR_STATUS:
        case mDC3PORT_INTR_SELECT:
            return 0;
    ...
    uint32 *value)
{
    ...
    /* get indirect command timeout value */
    SHR_IF_ERR_EXIT(access_direct_regmem(unit, FLAG_ACCESS_DO_NOT_USE_CACHE, sbus_blk_inst_id, ACCESS_REG_INDIRECT_COMMAND, 
                                                                    width_in_uint32s, reg_val));
    SHR_IF_ERR_EXIT(access_field32_get(unit, rECI_INDIRECT_COMMAND_fINDIRECT_COMMAND_TIMEOUT, reg_val, &timeout));

    /* Configure timeout */
    SHR_IF_ERR_EXIT(access_field32_set(runtime_info, rECI_INDIRECT_COMMAND_fINDIRECT_COMMAND_TIMEOUT, regval, timeout));
    ...
    uint32 *mem_exclude_bitmap)
{
        ...
        if (local_mem_id != ACCESS_INVALID_LOCAL_REGMEM_ID)
        {
            ...
            for (blk_inst_id = local_block->instances_start_index; blk_inst_id <= blk_inst_end_index; blk_inst_id++)
            {
                ...
                write_operation->end_index =
                    operation_mem_index + rm_info->array_skip * (rm_info->nof_array_indices-1) + rm_info->u.mem.mem_nof_elements;
    ...
    /** continue filling memories till done */
    for (blk_inst_id = list_head; list_head != ACCESS_INVALID_LOCAL_BLOCK_INSTANCE_ID;)
    {
        ...
        if (is_done)
        {
            ...
            { /** We are still in the same memory */
                index_in_array_element = write_operation->index % rm_info->array_skip;
                
                operation_nof_entries = rm_info->u.mem.mem_nof_elements - index_in_array_element;
                
                /* 
                 * In case array_skip > mem_nof_elements, when we reach the end of current array we need to advance to the start of the next array, 
                 * and set operation_nof_entries to mem_nof_elements.
                 * In case array_skip == mem_nof_elements, when we reach the end of current array operation_nof_entries is allready set correctly 
                 * and we are at the start of the next operation
                 */
                if (operation_nof_entries == 0)
                {
                    operation_nof_entries = rm_info->u.mem.mem_nof_elements;
                    write_operation->index += (rm_info->array_skip - rm_info->u.mem.mem_nof_elements);
                }
        ...
        else
        {
            ...
            if (write_operation->not_done_counter >= 2)
            {
                ...
                {
                    rm_info = device_info->local_regs + write_operation->regmem_id;
                    LOG_ERROR(BSL_LOG_MODULE, ("Timeout on block %d, memory %s local_mem_id %d, count %d\n",
...
/**
 * \brief - Set RESETN bit for TCAM CS tables.
 * Only after reset, those tables are active.
 *
    ...
    int unit)
{
    ...
    /*
     * for simulation - avoid from reset memories - assuming all memories already initialized to zero
     */
    ...
    */
    ...
    if (SOC_IS_J2P(unit))
    {
        SHR_IF_ERR_EXIT(DNXC_ACCESS_SET_DYN_MEM_ACCESS
                        (unit, EVNT, TRANSACTION_ENGINE_BIT_MAPPING, ACCESS_ALL_BLOCK_INSTANCES, &dynamic_mem_value));
        SHR_IF_ERR_EXIT(DNXC_ACCESS_SET_DYN_MEM_ACCESS
                        (unit, EVNT, TX_EVENT_ID_TO_CONTEXT_ID_MAP, ACCESS_ALL_BLOCK_INSTANCES, &dynamic_mem_value));
        SHR_IF_ERR_EXIT(DNXC_ACCESS_SET_DYN_MEM_ACCESS
                        (unit, EVNT, CONTEXT_PROPERTIES_0, ACCESS_ALL_BLOCK_INSTANCES, &dynamic_mem_value));
        SHR_IF_ERR_EXIT(DNXC_ACCESS_SET_DYN_MEM_ACCESS
                        (unit, EVNT, CONTEXT_PROPERTIES_1, ACCESS_ALL_BLOCK_INSTANCES, &dynamic_mem_value));
    }
