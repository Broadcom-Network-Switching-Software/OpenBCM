 ...
 */
#define IPMC_IPV4_EMPTY_MASK    (UTILEX_BITS_MASK(SAL_UINT32_NOF_BITS-1, 0))
/*
...
/*
 * Number of IPMC tables
 */
#define NOF_IPMC_TABLES(unit)  ((dnx_data_l3.fwd.mc_tables_get(unit, L3_IPV4_PROTOCOL_VERSION, 1/**is_route*/)->count) + \
        (dnx_data_l3.fwd.mc_tables_get(unit, L3_IPV4_PROTOCOL_VERSION, 0 /**is_bridge*/)->count) + \
        (dnx_data_l3.fwd.mc_tables_get(unit, L3_IPV6_PROTOCOL_VERSION, 1 /**is_route*/)->count) + \
        (dnx_data_l3.fwd.mc_tables_get(unit, L3_IPV6_PROTOCOL_VERSION, 0 /**is_bridge*/)->count))
/** Number of LSBs taken from the GLOBAL IN LIF field which is separated into two for IPV6 EM and KBP tables. */
#define IPMC_IPV6_GLOB_IN_LIF_NOF_LSB  16
/** Number of MSBs taken from the GLOBAL IN LIF field which is separated into two for IPV6 EM and KBP tables. */
#define IPMC_IPV6_GLOB_IN_LIF_NOF_MSB  6
/** Used in dnx_ipmc_dbal_tables_array_get to indicate that all IPMC tables will be returned. */
#define DNX_IPMC_GET_ALL_TABLES (UTILEX_BITS_MASK(SAL_UINT32_NOF_BITS-1, 0))
/*
 * }
 */
 ...
 */
#define IPMC_IPV4_SET_GROUP(addr) (((uint32)IPMC_MC_IPV4_ADDRESS_PREFIX << IPMC_IPV4_GROUP_MASK_SIZE) | addr)
/** Apply the four MSBs to the IPV4 group mask. */
#define IPMC_IPV4_SET_GROUP_MASK(mask) (((uint32)0xF << IPMC_IPV4_GROUP_MASK_SIZE) | mask)
/*
 ...
 */
#define IPMC_IS_IPV4_HOST_ENTRY(unit, data, public_capacity) ((!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM) && (data->vid > 0 || data->ing_intf > 0) \
                                        && (data->vrf != 0 || (data->vrf == 0 && public_capacity == 0))\
                                        && IPMC_IS_IPV4_FULL_MASKING(data->s_ip_mask) && IPMC_IS_IPV4_NO_MASKING(data->mc_ip_mask))\
                                        && (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_host_entry_support)))
/*
 ...
 */
#define IPMC_IS_IPV6_HOST_ENTRY(unit, data, public_capacity) (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM) && IPMC_IS_IPV6_NO_MASKING(data->mc_ip6_mask)\
                                       && (data->vid > 0 || data->ing_intf > 0) && \
                                       (data->vrf != 0 || (data->vrf == 0 && public_capacity == 0)) && IPMC_IS_IPV6_FULL_MASKING(data->s_ip6_mask) \
 ...
 */
#define IPMC_IS_BRIDGE_EM_ENTRY(data) ((dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_host_entry_support))\
                                       && IPMC_IS_IPV4_NO_MASKING(data->mc_ip_mask)\
                                       && IPMC_IS_IPV4_FULL_MASKING(data->s_ip_mask)\
                                       && _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
/** Determine whether the entry is a default IPv4 one. */
#define IPMC_IS_IPV4_DEFAULT_ENTRY(data) ((((data->s_ip_addr & data->s_ip_mask) == 0 && (data->mc_ip_addr & data->mc_ip_mask) == 0 && (data->vid == 0) && (data->ing_intf == BCM_IF_INVALID)) || _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_DEFAULT_ENTRY))\
                                          && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
#define IPMC_IS_IPV6_DEFAULT_ENTRY(data, compressed_vrfg) ((((compressed_vrfg ? ((dnx_ipmc_ipv6_default_address(data->s_ip6_addr, data->s_ip6_mask) && data->mc_ip_addr == 0)) \
    : (dnx_ipmc_ipv6_default_address(data->s_ip6_addr, data->s_ip6_mask) &&  dnx_ipmc_ipv6_default_address(data->mc_ip6_addr, data->mc_ip6_mask))) && (data->vid == 0) && (data->ing_intf == BCM_IF_INVALID)) \
    || (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_DEFAULT_ENTRY))) && _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
...
/*
 * KBP has 32 bits MC group instead of 28 bits, IN_LIF is 24 bits instead of 22 bits due to requirement for the KBP keys
...
#define DNX_IPMC_GLOB_IN_LIF_LSB_GET(in_lif) (in_lif & (UTILEX_BITS_MASK(IPMC_IPV6_GLOB_IN_LIF_NOF_LSB-1, 0)))
/** Retrieve the 6 MSBs from the in_lif. Used for IPv6 EM and TCAM tables. */
#define DNX_IPMC_GLOB_IN_LIF_MSB_GET(in_lif) ((in_lif >> IPMC_IPV6_GLOB_IN_LIF_NOF_LSB) & UTILEX_BITS_MASK(IPMC_IPV6_GLOB_IN_LIF_NOF_MSB - 1, 0))

#define IPMC_COMPRESSED_VAL_FULL_MASK(unit) (UTILEX_BITS_MASK(dnx_data_l3.fwd.compressed_sip_svl_size_get(unit) - 1, 0))
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    uint32 vrfg_vsig_support = dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group);
    SHR_FUNC_INIT_VARS(unit);

    SHR_NULL_CHECK(data, _SHR_E_PARAM, "bcm_ipmc_addr_t");

    if (dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_public_routing_support))
    {
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "VRF %d is out of range, should be lower than %d.", data->vrf,
                     dnx_data_l3.vrf.nof_vrf_get(unit));
    }

    if (!dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_ivl_feature_support) && (data->mod_id != 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "mod_id cannot be set. IVL is not supported on this device.");
    }

    /** Verify supported flags */
    if (_SHR_IS_FLAG_SET(data->flags, ~(dnx_data_l3.fwd.ipmc_supported_flags_get(unit))))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Illegal IPMC flags are passed to bcm_ipmc_* API.");
    }
    if (dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_public_routing_support)
        && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2) && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM))
    {
        /*
         * Validate the entry based on the VRF value.
         * For host entries VRF = 0 is a valid value as long as a single KAPS DB is set up or public entries are supported.
         * If working with double KAPS DB, only private IPv6 multicast host table is supported.
         */
        if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
        {
            if (((IPMC_IS_IPV6_NO_MASKING(data->mc_ip6_mask)) && (data->vrf == 0)
                 && (data->vid > 0 || data->ing_intf > 0) && (IPMC_IS_IPV6_FULL_MASKING(data->s_ip6_mask)))
                && (kaps_public_capacity != 0))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Cannot create an IPv6 MC LEM entry with VRF = 0 when KAPS2 capacity is not 0.\n");
            }

            if ((!IPMC_IS_IPV6_HOST_ENTRY(unit, data, kaps_public_capacity)) && (data->vrf == 0)
                && (kaps_public_capacity != 0))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Cannot create an IPv6 MC LPM entry with VRF = 0 when KAPS2 capacity is not 0.\n");
            }
        }
        else
        {
            if ((data->vid > 0 || data->ing_intf > 0)
                && (IPMC_IS_IPV4_FULL_MASKING(data->s_ip_mask)) && (IPMC_IS_IPV4_NO_MASKING(data->mc_ip_mask))
                && (data->vrf == 0) && (kaps_public_capacity != 0))
            {
     ...
     * If VRFG and VSIG compression are supported, the valid fields are: s_ip6_addr, s_ip6_mask, mc_ip_addr, mc_ip_mask;
     * otherwise s_ip_addr, s_ip_mask, mc_ip6_addr, mc_ip6_mask.
     */
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        /** Check LPM inputs parameters*/
        if (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM) && !IPMC_IS_IPV6_HOST_ENTRY(unit, data, kaps_public_capacity))
        {
            if (vrfg_vsig_support)
            {
                if (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
                {
                    /** If the DIP is different than fully unmasked, then the compressed SIP/SI cannot be different than 0. */
                    if ((data->mc_ip_mask != IPMC_COMPRESSED_VAL_FULL_MASK(unit))
                        && (!IPMC_IS_IPV6_FULL_MASKING(data->s_ip6_mask)))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "Inconsistent mask of IPMC key - it is expected that if SIP is defined, Compressed VRFG should have no mask\n");
                    }
                }
                else
                {
                    if ((data->mod_id > 0) && (data->mc_ip_addr >= dnx_data_l3.fwd.nof_compressed_ivl_sip_get(unit)))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "Invalid value for IPv6 compressed VRFG, expected value which is less than %d \n",
                                     dnx_data_l3.fwd.nof_compressed_ivl_sip_get(unit));
                    }
                }
                /** Verify IPv6 compressed VRFG value for route and L2 SVL cases*/
                if (data->mc_ip_addr >= dnx_data_l3.fwd.nof_compressed_svl_sip_get(unit))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Invalid value for IPv6 compressed VRFG, expected value which is less than %d \n",
                ...
                if (!(dnx_ipmc_ipv6_default_address(data->mc_ip6_addr, data->mc_ip6_mask))
                    || !(IPMC_IS_IPV4_FULL_MASKING(data->s_ip_mask))
                    || !(IPMC_IS_IPV4_DEFAULT_ADDRESS(data->s_ip_addr)))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "The fields that do not participate in the key should not be configured(mc_ip6_mask,mc_ip6_addr,s_ip_mask,s_ip_addr).\n");
                }
                if (data->vrf > 0)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "The VRF field should not be configured.\n");
                }
            }
            else
            {
                if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
                {
                    /** Verify the cascaded IPv6 entry input - valid for LPM entries */
                    if ((data->mod_id > 0) && (data->s_ip_addr >= dnx_data_l3.fwd.nof_compressed_ivl_sip_get(unit)))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "Invalid value for IPv6 compressed SIP, expected value which is less than %d \n",
                                     dnx_data_l3.fwd.nof_compressed_ivl_sip_get(unit));
                    }
                    if ((data->mod_id == 0) && data->s_ip_addr >= dnx_data_l3.fwd.nof_compressed_svl_sip_get(unit))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "Invalid value for IPv6 compressed SIP, expected value which is less than %d \n",
                                     dnx_data_l3.fwd.nof_compressed_svl_sip_get(unit));
                ...
                else
                {
                    ...
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM, "The VID/ing_intf fields should not be configured.\n");
                    }
                    ...
                    if (data->vrf >= dnx_data_l3.vrf.nof_vrf_ipv6_get(unit))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM, "VRF %d is out of range, should be lower than %d.", data->vrf,
                                     dnx_data_l3.vrf.nof_vrf_ipv6_get(unit));
                    }
                }
                /** If the DIP is different than fully unmasked, then the compressed SIP/SI cannot be different than 0. */
                if (data->s_ip_mask != 0 && !IPMC_IS_IPV6_NO_MASKING(data->mc_ip6_mask))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Inconsistent mask of IPMC key - it is expected that if compressed SIP/compressed SI is defined, DIP should have no mask\n");
                }
                if (!IPMC_IS_IPV4_DEFAULT_ADDRESS(data->mc_ip_addr) || !(IPMC_IS_IPV4_FULL_MASKING(data->mc_ip_mask)))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "The fields that do not participate in the key should not be configured(mc_ip_addr, mc_ip_mask).\n");
                }
            }
        }

        if (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2) && _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_DEFAULT_ENTRY)
            && IPMC_IS_IPV6_HOST_ENTRY(unit, data, kaps_public_capacity))
        {
        ...
        if (!IPMC_IS_IPV6_ADDRESS_MULTICAST(data->mc_ip6_addr, data->mc_ip6_mask)
            && !(IPMC_IS_IPV6_FULL_MASKING(data->mc_ip6_mask)) && !vrfg_vsig_support)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
        ...
        if (IPMC_IS_IPV6_ADDRESS_MULTICAST(data->s_ip6_addr, data->s_ip6_mask) && vrfg_vsig_support)
        {
    ...
    else
    {
         ...
         */
        if (IPMC_IS_IPV4_HOST_ENTRY(unit, data, kaps_public_capacity) && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2)
        ...
        if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
        {
            ...
            if ((data->vid == 0)
                && (!IPMC_IS_IPV4_FULL_MASKING(data->s_ip_mask) || !IPMC_IS_IPV4_FULL_MASKING(data->mc_ip_mask)))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Illegal IPV4 bridge entry , VSI is 0, but mc_ip_mask = 0x%08X and source address mask 0x%08X values - they should be consistent.",
                             data->mc_ip_mask, data->s_ip_mask);
            }
        }
        else
        {
            ...
            if (!IPMC_IS_IPV4_HOST_ENTRY(unit, data, kaps_public_capacity)
                && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM))
            {
    ...
    if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_rif_key_participation))
    {
        ...
        {
            /*
             * BCM_IPMC_L2 is set, ing_intf should not be configured 
             */
            if (data->ing_intf != BCM_IF_INVALID)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "ing_intf field must not be set!\n");
            }
        }
    }
    else
    {
        /** In case of l2 flag, the VSI should be provided, but not ing_intf*/
        if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
        {
            if (data->ing_intf != BCM_IF_INVALID)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Ing_intf field must not be configured, only vid!\n");
            }
            if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6) && data->vid != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "VID field must not be set!\n");
            }
        }
        else
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "ETH-RIF (vid or ing_intf field) does not participate in the key construction, must not be configured!\n");
            }
        }
    }
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    uint32 cmprs_vrfg = dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group);
    ...
    if (data->l3a_intf == 0 && (IPMC_IS_IPV4_DEFAULT_ENTRY(data) || IPMC_IS_IPV6_DEFAULT_ENTRY(data, cmprs_vrfg)))
    {
    ...
    if (data->group != 0)
    {
         ...
         */
        if ((IPMC_IS_IPV4_HOST_ENTRY(unit, data, kaps_public_capacity) && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
            || (IPMC_IS_IPV6_HOST_ENTRY(unit, data, kaps_public_capacity)
                && _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6)) || IPMC_IS_BRIDGE_EM_ENTRY(data)
            || _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM))
        {
    ...
    else if (data->l3a_intf != 0)
    {
        if (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_RAW_ENTRY))
        {
            ...
            /*
             * Default FEC validation - the result type is default FEC when the key of the entry is fully masked.
             * Validation is occurring when the entry is not KBP - we check FEC ID is lower than the max value.
             * With KBP the default indication is a separate result field, which doesn't impose such limitation.
             */
            if ((IPMC_IS_IPV4_DEFAULT_ENTRY(data) && !use_kbp_ipv4)
                || (IPMC_IS_IPV6_DEFAULT_ENTRY(data, cmprs_vrfg) && !use_kbp_ipv6))
            {
                if ((fec_index > dnx_data_l3.fec.max_default_fec_get(unit))
                    && dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_default_fec_limitation))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Default FEC index %d is bigger than the maximal FEC id that can be used for default route %d \n",
    ...
    else if (port != 0)
    {
         ...
         */
        if (!IPMC_IS_IPV4_HOST_ENTRY(unit, data, kaps_public_capacity) && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM))
        {
    ...
    uint32 *entry_handle_id)
{
    ...
    if (((data->vrf > 0 || kaps_public_capacity == 0)
         && !dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group))
        || (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group)
            && (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))))
    {
 ...
 *   includes both IVL(independent vlan learning) and SVL(shared
 *   vlan learning)
 * \param [in] unit - The unit number.
 ...
 *      - mod_id     -> VLAN ID used in IVL tables
    ...
    uint32 *entry_handle_id)
{
     ...
     *      * SSM SVL - <FID, G, S>
     *      * IVL - <FID, VLAN, G>
     *      * SSM IVL - <FID, VLAN, G, S>
     * IPV6:
     *      * SSM SVL - <FID, G, S>
     *      * SSM IVL - <FID, VLAN, G, S>
     * If there's a valid vid value, then the entry will be IVL.
     * Otherwise, it will be an SVL one.
    ...
    if (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        ...
        if (IPMC_IS_BRIDGE_EM_ENTRY(data))
        {
            ...
            if (data->mod_id > 0)
            {
                dbal_entry_key_field32_set(unit, *entry_handle_id, DBAL_FIELD_VID, data->mod_id);
            }
        }
        else
        {
            ...
            if (data->mod_id > 0)
            {
                dbal_entry_key_field32_masked_set(unit, *entry_handle_id, DBAL_FIELD_VID, data->mod_id,
                                                  IPMC_GET_IN_LIF_MASK(data->mod_id));
            }
        }
    }
    else
    {
        if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group))
        {
            dbal_entry_key_field_arr8_masked_set(unit, *entry_handle_id, DBAL_FIELD_IPV6,
                                                 data->s_ip6_addr, data->s_ip6_mask);
        }
        else
        {
...
dnx_ipmc_entry_tcam_key_set(
    ...
    uint32 *entry_handle_id)
{
    uint32 ipv4_group, ipv6_group, glob_in_lif, glob_in_lif_ipv6;
    SHR_FUNC_INIT_VARS(unit);
    if (IPMC_TABLE_IS_KBP(dbal_table))
    {
        ipv4_group = DBAL_FIELD_IPV4_DIP;
        ipv6_group = DBAL_FIELD_IPV6_DIP;
        glob_in_lif = DBAL_FIELD_KBP_GLOB_IN_LIF;
        glob_in_lif_ipv6 = DBAL_FIELD_KBP_GLOBAL_IN_LIF_21_16;
    }
    else
    {
        ipv4_group = DBAL_FIELD_IPV4_GROUP;
        ipv6_group = DBAL_FIELD_IPV6_GROUP;
        glob_in_lif = DBAL_FIELD_GLOB_IN_LIF;
        glob_in_lif_ipv6 = DBAL_FIELD_GLOBAL_IN_LIF_21_16;
    }
    /*
    ...
    uint32 *result_type)
{
    ...
    uint32 mc_group;
    uint32 cmprs_vrfg = dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group);
    uint32 default_fec_limit = dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_default_fec_limitation);
    ...
    if (IPMC_TABLE_IS_KBP(dbal_table))
    {
        ...
        if ((IPMC_IS_IPV4_DEFAULT_ENTRY(data) || IPMC_IS_IPV6_DEFAULT_ENTRY(data, cmprs_vrfg)) && default_fec_limit)
        {
    ...
    else if (dbal_table_type == DBAL_TABLE_TYPE_LPM)
    {
         ...
         * However, if the 'raw' flag is set, then just use 'l3a_intf' (Actually use 'fec_index' which is, effectively,
         * the same).
         */
        ...
        else
        {
            if (data->group != 0)
            {
                dbal_entry_value_field32_set(unit, *entry_handle_id, DBAL_FIELD_MC_ID_KAPS, INST_SINGLE, mc_group);
                     ...
                     && (IPMC_IS_IPV4_DEFAULT_ENTRY(data) || IPMC_IS_IPV6_DEFAULT_ENTRY(data, cmprs_vrfg))
                     && default_fec_limit)
            {
    ...
    else
    {
        if (dbal_table_type == DBAL_TABLE_TYPE_EM)
        {
            ...
            {
                /*
                 * If mod_id value is valid, then DESTINATION_ONLY result type will be defined for the IVL table.
                 * Otherwise, DESTINATION_ONLY result will be defined for the SVL table.
                 */
                if (data->mod_id > 0)
                {
                    *result_type =
                        (data->stat_pp_profile !=
                         STAT_PP_PROFILE_INVALID) ? DBAL_RESULT_TYPE_BRIDGE_IPV4_MULTICAST_IVL_FWD_DEST_STAT :
                        DBAL_RESULT_TYPE_BRIDGE_IPV4_MULTICAST_IVL_FWD_DEST;
                    dbal_entry_value_field32_set(unit, *entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                                 *result_type);
                }
                else
 ...
 */
static shr_error_e
dnx_ipmc_dbal_table_select_with_host(
    int unit,
    bcm_ipmc_addr_t * data,
    uint32 *dbal_table)
{
     ...
     * It can be either SVL(shared vlan learning) or IVL(independent vlan learning) as for the latter,
     * the VID is used when forming the key.
    ...
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        ...
        else if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
        {
            ...
            if (data->mod_id > 0)
            {
                *dbal_table = DBAL_TABLE_BRIDGE_IPV6_MULTICAST_SOURCE_SPECIFIC_IVL;
            }
            else
            {
        ...
        else if (IPMC_IS_IPV6_HOST_ENTRY(unit, data, kaps_public_capacity))
        {
    ...
    {
        /*
         * If the vid is set, then an IVL entry will be created, otherwise - an SVL.
         * If the IPv4 group IP address is not masked and the source IP is fully masked, then an EM entry will created.
         * Otherwise, if the mask <FID,[VID,] G, S> is consistent, then it is an LPM SSM (source specific multicast) entry.
         */
        if (IPMC_IS_BRIDGE_EM_ENTRY(data) && data->mod_id > 0)
        {
            *dbal_table = DBAL_TABLE_BRIDGE_IPV4_MULTICAST_IVL;
        }
        else if (IPMC_IS_BRIDGE_EM_ENTRY(data) && data->mod_id == 0)
        {
            *dbal_table = DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SVL;
        }
        else if (data->mod_id > 0)
        {
            *dbal_table = DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SOURCE_SPECIFIC_IVL;
        }
        else
        {
    ...
    else
    {
        ...
        else if (IPMC_IS_IPV4_HOST_ENTRY(unit, data, kaps_public_capacity))
        {
 ...
 */
shr_error_e
dnx_ipmc_dbal_table_select_no_host(
    int unit,
    bcm_ipmc_addr_t * data,
    uint32 *dbal_table)
{
    ...
    uint32 *dbal_table)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_host_entry_support))
    {
        SHR_IF_ERR_EXIT(dnx_ipmc_dbal_table_select_with_host(unit, data, dbal_table));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipmc_dbal_table_select_no_host(unit, data, dbal_table));
    }
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief
 *   Retrieve an array of all valid IPMC DBAL tables for the system.
 * \param [in] unit - The unit number.
 * \param [out] dbal_tables - an array of all IPMC DBAL tables.
 * \param [out] nof_dbal_tables - the number of tables in the array
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 * \see
 ...
 */
static void
dnx_ipmc_dbal_all_tables_array_get(
    int unit,
    dbal_tables_e * dbal_tables,
    uint32 *nof_dbal_tables)
{
    uint8 bridge_enable = 0;
    uint8 fwd_mode = 0;
    uint8 nof_fwd_modes = 2;
    uint8 nof_ip_versions = 2;
    uint8 ip_versions[2] = { L3_IPV4_PROTOCOL_VERSION, L3_IPV6_PROTOCOL_VERSION };
    uint8 ip_ver;

    *nof_dbal_tables = 0;
    for (ip_ver = 0; ip_ver < nof_ip_versions; ip_ver++)
    {
        for (fwd_mode = bridge_enable; fwd_mode < nof_fwd_modes; fwd_mode++)
        {
            sal_memcpy(&dbal_tables[*nof_dbal_tables],
                       dnx_data_l3.fwd.mc_tables_get(unit, ip_versions[ip_ver], fwd_mode)->tables,
                       sizeof(dbal_tables_e) * dnx_data_l3.fwd.mc_tables_get(unit, ip_versions[ip_ver],
                                                                             fwd_mode)->count);
            *nof_dbal_tables += dnx_data_l3.fwd.mc_tables_get(unit, ip_versions[ip_ver], fwd_mode)->count;
        }
    }
}
    ...
    bcm_ipmc_addr_t * dbal_entry)
{
    ...
    if (dbal_table == DBAL_TABLE_BRIDGE_IPV4_MULTICAST_IVL
        || dbal_table == DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SOURCE_SPECIFIC_IVL
        || dbal_table == DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SVL
        || dbal_table == DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SOURCE_SPECIFIC_SVL)
    {
    ...
    /*
     * If the table is an IVL one, then the value of the VLAN field of the key will be extracted.
     */
    if (dbal_table == DBAL_TABLE_BRIDGE_IPV4_MULTICAST_IVL
        || dbal_table == DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SOURCE_SPECIFIC_IVL)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_VID, field_value));
        dbal_entry->mod_id = field_value[0];
    }
    ...
    bcm_ipmc_addr_t * dbal_entry)
{
    ...
    /**
     * Build entry key based on dbal table type.
     */
    if (dbal_table_type == DBAL_TABLE_TYPE_LPM)
    {
        if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group))
        {
            dbal_entry->mc_ip_addr = field_value;
            dbal_entry->mc_ip_mask = field_value_mask;
        }
        else
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_masked_get
                            (unit, entry_handle_id, DBAL_FIELD_COMPRESSED_SI, &field_value, &field_value_mask));
            dbal_entry->s_ip_addr = field_value;
            dbal_entry->s_ip_mask = field_value_mask;
        }
    }
    else
    {
        if ((dbal_table_type == DBAL_TABLE_TYPE_TCAM) && (!IPMC_TABLE_IS_KBP(dbal_table)))
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
        ...
        {
            /** Table type is EM or table is KBP */
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
            ...
            if (IPMC_TABLE_IS_KBP(dbal_table))
            {
                ...
                field_value = IPMC_KBP_IN_LIF_SET(((field_value << IPMC_IPV6_GLOB_IN_LIF_NOF_LSB) | field_value_lsb));
            }
            else
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                                (unit, entry_handle_id, DBAL_FIELD_GLOBAL_IN_LIF_21_16, &field_value));
                field_value = (field_value << IPMC_IPV6_GLOB_IN_LIF_NOF_LSB) | field_value_lsb;
            }
        }
    ...
    if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group))
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr8_masked_get(unit, entry_handle_id, DBAL_FIELD_IPV6,
                                                                    dbal_entry->s_ip6_addr, dbal_entry->s_ip6_mask));
    }
    else
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_VRF, &field_value));
        dbal_entry->vrf = field_value;
    ...
    bcm_ipmc_addr_t * dbal_entry)
{
    ...
    else if ((dbal_table_type == DBAL_TABLE_TYPE_LPM) || (dbal_table_type == DBAL_TABLE_TYPE_TCAM))
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_masked_get
     ...
     */
    if ((dbal_table_type == DBAL_TABLE_TYPE_LPM) || (dbal_table_type == DBAL_TABLE_TYPE_TCAM))
    {
    ...
    if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_rif_key_participation))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                            (unit, entry_handle_id, DBAL_FIELD_KBP_GLOB_IN_LIF, field_value));
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    /** Verify input and NULL check of data */
    SHR_INVOKE_VERIFY_DNXC(dnx_ipmc_verify(unit, data));
    /*
    ...
    data->group = 0;
    data->l3a_intf = 0;
    hitbit_flags |= (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_HIT_CLEAR)) ? DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR : 0;
    ...
    if (dbal_table_type == DBAL_TABLE_TYPE_TCAM)
    {
        ...
        if (!IPMC_TABLE_IS_KBP(dbal_table))
        {
            ...
            SHR_IF_ERR_EXIT_NO_MSG(dbal_entry_access_id_by_key_get
    ...
    else
    {
        ...
        else if (dbal_table_type == DBAL_TABLE_TYPE_LPM)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_tables_indication_get
                            (unit, dbal_table, DBAL_TABLE_IND_IS_HITBIT_EN, &is_hitbit_enable));
            if (is_hitbit_enable)
            {
                SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_HITBIT, &hit_bit));
            }
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ipmc_add_verify(unit, data));
    ...
    int unit)
{
    ...
    uint8 use_kbp_ipv6 = dnx_data_elk.application.feature_get(unit, dnx_data_elk_application_ipv6) ? TRUE : FALSE;
    uint32 nof_tables;
    dbal_tables_e *dbal_mc_tables = NULL;

    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    DBAL_FUNC_INIT_VARS(unit);
    SHR_ALLOC_ERR_EXIT(dbal_mc_tables, sizeof(dbal_tables_e) * NOF_IPMC_TABLES(unit), "IPMC DBAL table array", "%s%s%s",
                       "dbal_mc_tables", EMPTY, EMPTY);
    dnx_ipmc_dbal_all_tables_array_get(unit, dbal_mc_tables, &nof_tables);
    ...
    void *user_data)
{
    ...
    uint8 ip_protocol_version =
        _SHR_IS_FLAG_SET(flags, BCM_IPMC_IP6) ? L3_IPV6_PROTOCOL_VERSION : L3_IPV4_PROTOCOL_VERSION;
    uint8 route_enabled = 1;
    uint8 bridge_enabled = 0;
    uint8 fwd_mode = _SHR_IS_FLAG_SET(flags, BCM_IPMC_L2) ? bridge_enabled : route_enabled;
    dbal_tables_e *dbal_tables = NULL;

    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    DBAL_FUNC_INIT_VARS(unit);
    nof_tables = dnx_data_l3.fwd.mc_tables_get(unit, ip_protocol_version, fwd_mode)->count;
    SHR_ALLOC_ERR_EXIT(dbal_tables, nof_tables * sizeof(dbal_tables_e), "IPMC DBAL table array", "%s%s%s",
                       "dbal_tables", EMPTY, EMPTY);
    sal_memcpy(dbal_tables, dnx_data_l3.fwd.mc_tables_get(unit, ip_protocol_version, fwd_mode)->tables,
               sizeof(dbal_tables_e) * nof_tables);
        ...
        if (!IPMC_TABLE_IS_KBP(dbal_tables[table]))
        {
            ...
            if (_SHR_IS_FLAG_SET(flags, BCM_IPMC_HIT_CLEAR))
            {
                SHR_IF_ERR_EXIT(dbal_iterator_attribute_action_add
                                (unit, entry_handle_id, DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR));
            }
        }
        ...
        while (!is_end)
        {
            ...
            if (!IPMC_TABLE_IS_KBP(dbal_tables[table]))
            {
                if (dbal_table_type == DBAL_TABLE_TYPE_TCAM)
                {
                    uint32 entry_access_id;
                    uint32 tcam_handler_id;
                    SHR_IF_ERR_EXIT(dbal_tables_tcam_handler_id_get(unit, dbal_tables[table], &tcam_handler_id));
                    SHR_IF_ERR_EXIT_NO_MSG(dbal_entry_access_id_by_key_get
                                           (unit, entry_handle_id, &entry_access_id, DBAL_COMMIT));
                    SHR_IF_ERR_EXIT(dbal_entry_handle_access_id_set(unit, entry_handle_id, entry_access_id));
                    SHR_IF_ERR_EXIT(dnx_field_tcam_handler_entry_priority_get
                                    (unit, tcam_handler_id, entry_access_id, (uint32 *) &(dbal_entry.priority)));
                }
            ...
            {
                SHR_IF_ERR_EXIT(dbal_entry_attribute_get
                                (unit, entry_handle_id, DBAL_ENTRY_ATTR_PRIORITY, (uint32 *) &dbal_entry.priority));
            }
