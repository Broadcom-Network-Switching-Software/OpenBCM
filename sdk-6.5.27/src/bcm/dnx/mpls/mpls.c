...
#define MPLS_LSB_NIBBLE_GAL MPLS_LABEL_GAL
...
static const special_label_property_t special_label_property[] = {
  /** Label,                             label_profile*/
    ...
    {MPLS_LSB_NIBBLE_GAL, DBAL_ENUM_FVAL_VTT_MPLS_SPECIAL_LABEL_PROFILE_GAL},
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    {
        /*
         * Remove this limitation when P2P with VSI > 0 is supported
         */
        if (info->vpn > 0)
        {
    ...
    if (DNX_FAILOVER_IS_PROTECTION_ENABLED(info->failover_id))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_failover_id_valid_verify(unit, BCM_FAILOVER_TYPE_INGRESS, 0, info->failover_id));
    }
    ...
    if (info->action != BCM_MPLS_SWITCH_ACTION_POP &&
        _SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_EXTENDED_TERMINATION))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Couldn't use BCM_MPLS_SWITCH2_EXTENDED_TERMINATION when action isn't TERMINATION");
    }
 ...
 * \param [in] vpn -
 *      VPN number
 * \param [in] is_allocated_error_log
 *      Do we want to produce error log in case of not found
 * \return
 *      \retval Zero in case of NO ERROR.
 *      \retval Negative in case of an error, see \ref shr_error_e.
 * \remark
    ...
    uint8 is_allocated_error_log)
{
    ...
    if (!is_vsi_usage_mpls)
    {
        if (is_allocated_error_log)
        {
            SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "MPLS VPN does not exist, note that other VPN exists!\n");
        }
        else
        {
            SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
...
*/
static shr_error_e
dnx2_mpls_init_fhei_to_ees_resolution(
    int unit)
{
    uint32 entry_handle_id;
    ...
    {
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_ETPP_ETPS_FORMATS, DBAL_ENUM_FVAL_ETPP_ETPS_FORMATS_ETPS_MPLS_FHEI_PLUS,
                         &result_type_value));
    }
    else
    {
    ...
    {
        uint32 swap_command_min;
        uint32 swap_command_max;

        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_SWAP_QOS_PROFILE, DBAL_ENUM_FVAL_SWAP_QOS_PROFILE_DEFAULT,
                         &swap_command_min));
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_SWAP_QOS_PROFILE, DBAL_ENUM_FVAL_SWAP_QOS_PROFILE_MAX, &swap_command_max));
        for (mpls_command = swap_command_min; mpls_command <= swap_command_max; mpls_command++)
        {
            /**mpls command 1:1 mapping to remark profile*/
            remark_profile = mpls_command - swap_command_min;
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FHEI_SWAP_REMARK_PROFILE, mpls_command,
                                         remark_profile);
        }
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int unit)
{
    ...
    if (system_headers_mode != DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
    {
        uint32 swap_eei_identifier_min;
        uint32 swap_eei_identifier_max;

        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_SWAP_QOS_PROFILE, DBAL_ENUM_FVAL_SWAP_QOS_PROFILE_DEFAULT,
                         &swap_eei_identifier_min));
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_SWAP_QOS_PROFILE, DBAL_ENUM_FVAL_SWAP_QOS_PROFILE_MAX,
                         &swap_eei_identifier_max));
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_EEI_IDENTIFIER,
                                         swap_eei_identifier_min, swap_eei_identifier_max);
    }
    else
    {
...
*   \param [in] unit         -  Relevant unit.
*   \param [in] is_standard_image         -  Indicates whether current image is standard or bare-metal.
* \return
*   shr_error_e - Non-zero in case of an error.
*/
static shr_error_e
    ...
    uint8 is_standard_image)
{
    ...
    if (dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_fhei_init_v1))
    {
        /*
        ...
        {
            /*
             * In JR2 mode, this field is useless
             */
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FHEI_MPLS_POP_CMD, INST_SINGLE,
        ...
        if (is_standard_image)
        {
             ...
             */
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_JR1_MPLS_PUSH_COMMAND, entry_handle_id));
     ...
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_MPLS_LABEL_RANGE_PROPERTIES, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RANGE_PROFILE, 0);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS_LABEL_LOW, INST_SINGLE,
                                 MPLS_LSB_NIBBLE_RSV + 1);
    ...
    int unit)
{
    ...
    uint32 predefined_special_label_bitmap = 0;
    ...
    SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, DBAL_TABLE_IN_LIF_FORMAT_DUMMY_MPLS, &core_mode));
    ...
    sal_memset(&match_info, 0, sizeof(dnx_mpls_term_match_t));
    ...
    SHR_IF_ERR_EXIT(switch_db.predefined_mpls_special_label_bitmap.get(unit, &predefined_special_label_bitmap));
    SHR_BITSET(&predefined_special_label_bitmap, MPLS_LSB_NIBBLE_ELI);
    SHR_BITSET(&predefined_special_label_bitmap, MPLS_LSB_NIBBLE_GAL);
    SHR_BITSET(&predefined_special_label_bitmap, MPLS_LSB_NIBBLE_OAM_ALERT);
    SHR_IF_ERR_EXIT(switch_db.predefined_mpls_special_label_bitmap.set(unit, predefined_special_label_bitmap));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
...
{
    uint32 entry_handle_id;
    uint32 special_nibble_enum;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dbal_fields_enum_value_get
                    (unit, DBAL_FIELD_MPLS_SPECIAL_LABEL_NIBBLE, label_nibble, &special_nibble_enum));

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_MPLS_SPECIAL_LABEL_PROFILE_MAP, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS_SPECIAL_LABEL_NIBBLE, special_nibble_enum);
    ...
    int enable)
{
    ...
    else
    {
        ...
        /** Process for additional GAL.*/
        SHR_IF_ERR_EXIT(bcm_dnx_switch_control_get(unit, bcmSwitchOamAdditionalGalSpecialLabel, &additional_gal_label));
        if (label == additional_gal_label && additional_gal_label != MPLS_LSB_NIBBLE_GAL)
        {
            SHR_IF_ERR_EXIT(dnx_mpls_init_mpls_special_label_profile_map_single_entry
                            (unit, label, MPLS_BIT_CLEAR, DBAL_ENUM_FVAL_VTT_MPLS_SPECIAL_LABEL_PROFILE_GAL));
            SHR_IF_ERR_EXIT(dnx_mpls_init_mpls_special_label_profile_map_single_entry
                            (unit, label, MPLS_BIT_SET, DBAL_ENUM_FVAL_VTT_MPLS_SPECIAL_LABEL_PROFILE_GAL));
        }
    }
    ...
    int *enable)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_value_get
                    (unit, DBAL_FIELD_MPLS_SPECIAL_LABEL_NIBBLE, label, &special_nibble_enum));
...
*                               If not BOS, need to terminate two labels, otherwise one label.
* \return
*   shr_error_e - Non-zero in case of an error.
*/
static shr_error_e
    ...
    uint32 mpls_bos)
{
    ...
    nof_labels_to_term = mpls_bos ? ONE_MPLS_LABEL_TO_TERMINATE : TWO_MPLS_LABELS_TO_TERMINATE;
    ...
    SHR_IF_ERR_EXIT(dnx_mpls_termination_profile_allocation
                    (unit, term_profile_info, &term_profile, 0, &is_first, NULL));
    ...
    if (has_cw != FALSE)
    {
        dbal_table = DBAL_TABLE_EVPN_IML_ADDITIONAL_HEADERS_FL_AND_CW_ESI_CHECK;
    }
    else
    {
        dbal_table = DBAL_TABLE_EVPN_IML_ADDITIONAL_HEADERS_FL_ONLY_ESI_CHECK;
    }
...
/*
* \brief
*  Check if the dummy LIFs exist per inputs.
*
*   \param [in] unit         -  Relevant unit.
*   \param [in] has_cw       -  HAS_CW indicator for MPLS termiantion.
*   \param [in] mpls_bos     -  MPLS BOS indicator for MPLS termiantion.
...
*/
static shr_error_e
dnx_mpls_dual_homing_fl_termination_dummy_get(
    int unit,
    uint32 has_cw,
    uint32 mpls_bos)
{
    ...
    /** Get the local lif per given key from match table*/
    if (has_cw != FALSE)
    {
        dbal_table = DBAL_TABLE_EVPN_IML_ADDITIONAL_HEADERS_FL_AND_CW_ESI_CHECK;
    }
...
/*
* \brief
*  De-allocate and clear a dummy LIF for Flow-Label termination in EVPN IML.
*
*   \param [in] unit         -  Relevant unit.
*   \param [in] has_cw       -  HAS_CW indicator for MPLS termiantion.
*   \param [in] mpls_bos     -  MPLS BOS indicator for MPLS termiantion.
...
*/
static shr_error_e
dnx_mpls_dual_homing_fl_termination_dummy_clear(
    int unit,
    uint32 has_cw,
    uint32 mpls_bos)
{
    ...
    uint8 is_last;
    lif_mngr_local_inlif_info_t inlif_info;
    uint32 local_inlif, term_profile;
    uint8 is_std_1;
    ...
    /** Get local lif for the term profile */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_IN_LIF_FORMAT_DUMMY_MPLS, entry_handle_id));
 ...
 */
shr_error_e
dnx_mpls_dual_homing_fl_termination_enable_get(
    int unit,
    int *enablers)
{
    int rv;
    int is_fl_enabled = TRUE, is_fl_cw_enabled = TRUE;
    SHR_FUNC_INIT_VARS(unit);

    *enablers = 0;
    /*
     * Check if Flow Label is enabled
     * Since FL w/o BOS should always be set together, we check one of them
     */
    rv = dnx_mpls_dual_homing_fl_termination_dummy_get(unit, FALSE, 1);
    ...
    /*
     * Check if Flow Label and Control Word is enabled
     * Since FL w/o BOS should always be set together, we check one of them
     */
    rv = dnx_mpls_dual_homing_fl_termination_dummy_get(unit, TRUE, 1);
    if (rv != _SHR_E_NOT_FOUND)
    {
        SHR_IF_ERR_EXIT(rv);
    }
    ...
    {
        is_fl_cw_enabled = FALSE;
    }

    if (is_fl_enabled)
    {
        *enablers |= MPLS_EVPN_IML_FL_ENABLE;
    }

    if (is_fl_cw_enabled)
    {
        *enablers |= MPLS_EVPN_IML_FL_WITH_CW_ENABLE;
    }
 ...
 */
shr_error_e
dnx_mpls_dual_homing_fl_termination_enable_set(
    int unit,
    int enablers)
{
    int cur_enablers = 0;
    ...
    if (enablers > 3 || enablers < 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The choice(%d) is not valid, available range is 0 - 3!", enablers);
    }

    SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_enable_get(unit, &cur_enablers));

    /*
     * Alloc/de-alloc the local-in-lif and termination profile for Flow Lable
     * Do it only when the current status is different from input.
     */
    if (enablers & MPLS_EVPN_IML_FL_ENABLE)
    {
        if (!(cur_enablers & MPLS_EVPN_IML_FL_ENABLE))
        {
            SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_dummy_set(unit, FALSE, 1));
            SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_dummy_set(unit, FALSE, 0));
        }
    }
    else
    {
        if (cur_enablers & MPLS_EVPN_IML_FL_ENABLE)
        {
            SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_dummy_clear(unit, FALSE, 1));
            SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_dummy_clear(unit, FALSE, 0));
        }
    }

    /*
     * Alloc/de-alloc the local-in-lif and termination profile for Flow Lable and Control Word.
     * Do it only when the current status is different from input.
     */
    if (enablers & MPLS_EVPN_IML_FL_WITH_CW_ENABLE)
    {
        if (!(cur_enablers & MPLS_EVPN_IML_FL_WITH_CW_ENABLE))
        {
            SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_dummy_set(unit, TRUE, 1));
            SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_dummy_set(unit, TRUE, 0));
        }
    }
    else
    {
        if (cur_enablers & MPLS_EVPN_IML_FL_WITH_CW_ENABLE)
        {
            SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_dummy_clear(unit, TRUE, 1));
            SHR_IF_ERR_EXIT(dnx_mpls_dual_homing_fl_termination_dummy_clear(unit, TRUE, 0));
        }
    }
    ...
    int unit)
{
    ...
    /*
     * Step 1: dummy LIF for IFIT
     */
    if (dnx_data_switch.feature.feature_get(unit, dnx_data_switch_feature_silent_dummy_lif_lookup) &&
        !dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_nof_layers_to_terminate_support))
    {
        sal_memset(&inlif_info, 0, sizeof(inlif_info));
    ...
    /*
     * Step 2: dummy LIF for packet with GAL and Flow label enabled simultaneously 
     */
    ...
    int unit)
{
    ...
    if (!dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_configurable_fhei_supported))
    {
        /** initialize EGRESS_MPLS_FHEI_TO_EES_RESOLUTION table */
        SHR_IF_ERR_EXIT(dnx2_mpls_init_fhei_to_ees_resolution(unit));
    }
    else
    {
    }
    ...
    /** initialize INGRESS_MPLS_EES_TO_FHEI_RESOLUTION table */
    SHR_IF_ERR_EXIT(dnx_mpls_init_general_configuration(unit, is_standard_image));
    ...
    if (is_standard_image)
    {
        ...
        {
            /** See dnx_port_esem_default_program_init for the opposit case.*/
    ...
    bcm_mpls_tunnel_switch_t * info)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_switch_create_verify(unit, info));
    ...
    {
        /** Verification.*/
        SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_switch_term_add_verify(unit, info));
    ...
    bcm_mpls_tunnel_switch_t * info)
{
     ...
     * Verification.
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_tunnel_switch_get_verify(unit, info));
    ...
    bcm_mpls_vpn_config_t * info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_vpn_create_verify(unit, info));
    ...
    bcm_vpn_t vpn)
{
    ...
    /** check parameters */
    SHR_IF_ERR_EXIT(dnx_mpls_vpn_get_and_destroy_verify(unit, vpn, TRUE));
    ...
    bcm_mpls_vpn_config_t * info)
{
    ...
    rv = dnx_mpls_vpn_get_and_destroy_verify(unit, vpn, FALSE);
    if (rv == _SHR_E_NOT_FOUND)
    {
        /** Entry not found dont display error */
        SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
        SHR_EXIT();
    }
    else
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_MPLS_SKIP_TCAM_DB, &entry_handle_id));
    ...
    uint32 special_label_value)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_alternate_marking_special_label_set_verify(unit, special_label_value));
    ...
    /*
     * Write to additional header profile table.
     */
    SHR_IF_ERR_EXIT(egress_additional_headers_rfc8321_special_label_set(unit, special_label_value));
    ...
    uint32 label_value)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_alternate_marking_special_label_set_verify(unit, label_value));
