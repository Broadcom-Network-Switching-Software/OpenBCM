    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_REPLACE))
    {
        ...
        {
            if ((gport_hw_resources.inlif_dbal_result_type == DBAL_RESULT_TYPE_IN_LIF_FORMAT_LSP_IN_LIF_LSP)
                && _SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_CROSS_CONNECT))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot replace a LIF from non-P2P to P2P\n");
            }
            if ((gport_hw_resources.inlif_dbal_result_type ==
                 DBAL_RESULT_TYPE_IN_LIF_FORMAT_PWE_IN_LIF_PWE_P2P_NO_LEARNING_FLEXIBLE_PAYLOAD)
                && !_SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_CROSS_CONNECT))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot replace a LIF from P2P to non-P2P\n");
            }
        }
    }
    ...
    if (!DNX_MPLS_SWITCH_IS_EVPN(info) && _SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_CROSS_CONNECT)
        && (info->vpn || info->failover_id || info->failover_tunnel_id || info->qos_map_id))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "P2P case doesn't set those fields(vpn,failover_id,failover_tunnel_id,qos_map_id)\n");
    }
    ...
    uint8 *is_last_term_profile_reference)
{
    ...
    else
    {
        SHR_IF_ERR_EXIT(algo_mpls_db.termination_profile.exchange
                        (unit, 0, 1, &term_profile_info, old_term_profile, NULL, term_profile,
...
* \param [in] is_p2p - indicate lif use to p2p.
    ...
    int *in_lif_profile)
{
    ...
    if (!is_p2p && (forwarding_domain != NULL))
    {
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_REPLACE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_manager_info_to_mpls_term
                        (unit, old_entry_handle_id, &lif_info, &old_in_global_lif,
                         _SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_CROSS_CONNECT), (uint32 *) &old_term_profile,
    ...
    SHR_IF_ERR_EXIT(dnx_mpls_term_to_lif_table_manager_info(unit, sys_in_lif,
                                                            ...
                                                            qos_profile, entry_handle_id, _SHR_IS_FLAG_SET(info->flags2,
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_manager_info_to_mpls_term
                    (unit, entry_handle_id, &lif_info, &in_global_lif,
                     _SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_CROSS_CONNECT), &term_profile, &propag_profile,
                     &info->vpn, &protection_pointer[0], &info->failover_tunnel_id, &is_stat_enable, &qos_profile,
                     NULL));
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_manager_info_to_mpls_term
                    (unit, entry_handle_id, &lif_info, &in_global_lif,
                     _SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_CROSS_CONNECT), &term_profile, &propag_profile,
                     NULL, &protection_pointer, &protection_path, NULL, NULL, &in_lif_profile));
    /** in some case, global inlif is 0, and should get it from sw state*/
    if (in_global_lif == 0)
    {
        bcm_gport_t gport;
        ...
        while (!is_end)
        {
             ...
             */
            {
                ...
                SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                                (unit, local_inlif, dnx_drv_soc_core_default(), physical_table_id, &dbal_table_id,
                                 &dbal_result_type));
            }
