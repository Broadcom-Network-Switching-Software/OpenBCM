    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    {
        not_supported_flags |= BCM_MPLS_EGRESS_LABEL_PROTECTION;
    }
    ...
    {
        if (dnx_data_instru.
            alternate_marking.feature_get(unit, dnx_data_instru_alternate_marking_L_bit_profile_mapping))
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                         "Alternate marking solution is changed, alternate marking flags are no longer supported.\n");
        }
    ...
    uint8 *is_last_push_profile_reference)
{
    ...
    else
    {
        SHR_IF_ERR_EXIT(algo_mpls_db.push_profile.exchange
                        (unit, 0, 1, &push_profile_info, old_push_profile, NULL, push_profile,
