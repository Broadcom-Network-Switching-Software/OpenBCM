    ...
    bcm_mpls_label_t label_high)
{
    uint32 entry_handle_id;
    int range_profile = 0;
    int feature_mpls_term_till_bos;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    /*
     * Allocate entry
     ...
     * Set in HW
     */
    feature_mpls_term_till_bos = dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_mpls_term_till_bos);
    ...
    uint32 range_profile)
{
     ...
     * Free entry
     */
    SHR_IF_ERR_EXIT(algo_mpls_db.label_range_profile.free_single(unit, range_profile, NULL));
    /*
    ...
    bcm_mpls_range_action_t * action)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_range_action_add_verify(unit, label_low, label_high, action));
    /*
    ...
    bcm_mpls_range_action_t * action_p)
{
    ...
    SHR_NULL_CHECK(action_p, _SHR_E_PARAM, "action_p");
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_range_labels_verify(unit, label_low, label_high));
    ...
    bcm_mpls_label_t label_high)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_range_action_remove_verify(unit, label_low, label_high));
    SHR_IF_ERR_EXIT(dnx_mpls_label_range_exists(unit, label_low, label_high, &found, &is_iml, range_profile));
    if (found == FALSE)
    {
