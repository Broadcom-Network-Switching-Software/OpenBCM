    ...
    dnx_mpls_term_match_t * match)
{
    ...
    if (match->match_criteria == TERM_MATCH_CRITERIA_LABEL)
    {
        ...
        if (dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_per_port_ilm_lookup_supported))
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VMPI, INST_SINGLE, match->vmpi);
        }
    }
    ...
    if (result_type == DBAL_RESULT_TYPE_LOCAL_DPC_IN_LIF_MATCH_INFO_SW_MATCH_LABEL)
    {
        ...
        if (dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_per_port_ilm_lookup_supported))
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                            (unit, entry_handle_id, DBAL_FIELD_VMPI, INST_SINGLE, &vmpi));
            match->vmpi = vmpi;
        }
    }
    ...
    dnx_mpls_termination_profile_t * term_profile_info)
{
    ...
    if (entropy_enable &&
        (_SHR_IS_FLAG_SET(iml_flags, DNX_MPLS_L2VPN_FLAG_INGRESS_IML) ||
         _SHR_IS_FLAG_SET(iml_flags, DNX_MPLS_L2VPN_FLAG_INGRESS_IML_WITH_ESI) ||
         _SHR_IS_FLAG_SET(iml_flags, DNX_MPLS_L2VPN_FLAG_INGRESS_IML_WITHOUT_ESI)))
    {
        term_profile_info->labels_to_terminate = ONE_MPLS_LABEL_TO_TERMINATE;
    }
    else if (_SHR_IS_FLAG_SET(iml_flags, DNX_MPLS_L2VPN_FLAG_INGRESS_IML))
    {
        term_profile_info->labels_to_terminate = ONE_OR_TWO_MPLS_LABELS_TO_TERMINATE;
    }
    else if (_SHR_IS_FLAG_SET(iml_flags, DNX_MPLS_L2VPN_FLAG_INGRESS_IML_WITH_ESI))
    {
...
/*
        if (criteria == BCM_MPLS_PORT_MATCH_LABEL_CONTEXT_LABEL)
        {
    ...
    dnx_mpls_l2vpn_info_t * l2vpn)
{
    ...
    if (labels_to_terminate_val == TWO_MPLS_LABELS_TO_TERMINATE)
    {
        ...
        if ((l2vpn->type == MPLS_L2VPN_TYPE_MPLS_PORT) || (l2vpn->type == MPLS_L2VPN_TYPE_EVPN && !is_iml))
        {
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    {
        /*
         * In case failover port id (PWE FEC) is valid, only it is being learne
             ...
             (DNX_MPLS_LABEL_IN_RANGE(unit, mpls_port->egress_label.label)))
    {
        ...
        if (_SHR_IS_FLAG_SET(mpls_port->flags, BCM_MPLS_PORT_REPLACE))
        {
            SHR_IF_ERR_EXIT(dnx_mpls_eei_push_information_retrieve
                            (unit, forward_info->eei, &org_mpls_lable, &push_cmd));
        }
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_get(unit, gport, 0, &forward_info));
     ...
     *  e. In case of IOP mode and outlif is 0 when type is dest_outlif, it presents dest_outlif is converted from dest_only
     *         same logic with a, b option
     */
    if ((forward_info.fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY) ||
        ((forward_info.fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_OUTLIF) &&
         (forward_info.outlif == 0) &&
    ...
    bcm_gport_t mpls_port_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_get(unit, mpls_port_id, 0, &forward_info));
    /*
     * Jericho mode beginning
     * {
     */
    ...
    uint32 destination)
{
    ...
    if (info->is_p2p == FALSE)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SERVICE_TYPE, INST_SINGLE,
                                     DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_P2MP);
    ...
    if (info->keep_payload)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NEXT_PARSER_CONTEXT, INST_SINGLE,
                                     DBAL_ENUM_FVAL_IRPP_2ND_PARSER_PARSER_CONTEXT_TDM_BS);
    }
    SHR_EXIT();
    ...
    dnx_mpls_l2vpn_in_lif_info_t * info)
{
    ...
    uint8 is_field_on_handle = 0;
    uint32 learn_payload_context = 0;
    dbal_fields_e learn_info_field_id;
    uint32 learn_info[2] = { 0 };
    uint32 dest = 0;
...

    {
        ...
        info->is_p2p = (service_type == DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_P2P);
    }
    ...
    {
        uint32 next_parser_ctx = 0;
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                        (unit, entry_handle_id, DBAL_FIELD_NEXT_PARSER_CONTEXT, &is_field_on_handle, &next_parser_ctx));
        if (is_field_on_handle)
        {
            info->keep_payload = (next_parser_ctx == DBAL_ENUM_FVAL_IRPP_2ND_PARSER_PARSER_CONTEXT_TDM_BS) ? 1 : 0;
        }
    }
    ...
    if (is_field_on_handle)
    {
         ...
         *  IN_LIF_FORMAT_PWE only has a result(IN_LIF_PWE_MP) support learn,
         *  and the related learn info type is LEARN_INFO_ASYM_LIF_BASIC_EXT
         */
        learn_info_field_id = DBAL_FIELD_LEARN_INFO_ASYM_LIF_BASIC_EXT;
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                        (unit, entry_handle_id, learn_info_field_id, INST_SINGLE, learn_info));
        /*
         * For dest only:
         * Use NO_LIF_BASIC
         */
        if (learn_payload_context ==
            ...
            DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__FEC_OR_SPA_OR_TRAP_DEST__2K_GROUP__2L_STRENGTH__BOTH)
        {
            SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                            (unit, learn_info_field_id, DBAL_FIELD_DESTINATION, &dest, learn_info));
            info->learn_strength = SHR_BITGET(&dest, DNX_L2_LEARN_STRENGT_OFFSET_IN_LEARN_INFO(0));
        }
        else
        {
            SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                            (unit, learn_info_field_id, DBAL_FIELD_LEARN_STRENGTH, &(info->learn_strength),
                             learn_info));
        }
    }
...
*
*   \param [in] unit -  Relevant unit.
*   \param [in] inlif_dbal_table_id - dbal table id of the inlif
    ...
    dnx_mpls_l2vpn_info_t * l2vpn)
{
    ...
    else
    {
        ...
        {
            if (in_lif_profile_info.ingress_fields.pwe_nof_eth_tags == X_TAGS)
            {
    ...
    uint32 *inlif_dbal_result_type)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_allocate
                        (unit, lif_flags, DNX_ALGO_LIF_INGRESS, NULL, &l2vpn_inlif_info->global_in_lif));
    }
...
*   \param [out] last_profile - indication whether the last references to the lif-linked profiles were removed.
* \return
*   shr_error_e
*/
shr_error_e
    ...
    dnx_in_lif_profile_last_info_t * last_profile)
{
    ...
    else
    {
        ...
        if (_SHR_IS_FLAG_SET(mpls_port->flags, BCM_MPLS_PORT_SERVICE_TAGGED))
        {
            if (_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_SERVICE_TAGGED_BASED_HEADER_ONLY))
            {
                in_lif_profile_info.ingress_fields.pwe_nof_eth_tags = X_TAGS;
            }
            else
            {
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange
                    (unit, &in_lif_profile_info, old_in_lif_profile, new_in_lif_profile, LIF,
                     dbal_table_for_lif_profile, last_profile));
    ...
    dnx_mpls_l2vpn_in_lif_info_t * lif_info)
{
    ...
    learn_strength_dest = fwd_info->destination;
    learn_strength = lif_info->learn_strength;
    SHR_IF_ERR_EXIT(dbal_fields_uint32_sub_field_info_get
                    (unit, DBAL_FIELD_DESTINATION, fwd_info->destination, &dbal_sub_field, &result_value));
    if ((dbal_sub_field == DBAL_FIELD_MC_ID) || (dbal_sub_field == DBAL_FIELD_FLOW_ID))
    {
        dest_is_flow_mc = 1;
    }
    ...
    {
        /** set field id for info structure encoding */
        learn_info_field_type = DBAL_FIELD_TYPE_DEF_LEARN_INFO_ASYM_LIF_BASIC_EXT;

        /** set learn context*/
        lif_info->learn_context =
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, learn_info_field_type, DBAL_FIELD_GLOB_OUT_LIF, &fwd_info->outlif,
                         lif_info->learn_info));

        /** set learn_strength */
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, learn_info_field_type, DBAL_FIELD_LEARN_STRENGTH, &learn_strength,
                         lif_info->learn_info));
    }
    /*
     * For dest only:
    ...
    {
        /** set field id for info structure encoding */
        learn_info_field_type = DBAL_FIELD_TYPE_DEF_LEARN_INFO_NO_LIF_BASIC_SYM_LIF_BASIC;
     ...
     * Jericho mode beginning
     * {
     * Use ASYMMETRIC_LIF_EXTENDED_EEI
     */
    else if (fwd_info->fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_EEI)
    {
        learn_info_field_type = DBAL_FIELD_TYPE_DEF_LEARN_INFO_ASYM_LIF_EXT_EEI;
        /** set learn context */
        lif_info->learn_context =
        ...
        /** set EEI */
        SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                        (unit, learn_info_field_type, DBAL_FIELD_EEI, &fwd_info->eei, lif_info->learn_info));
    ...
    /** set destination */
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                    (unit, learn_info_field_type, DBAL_FIELD_DESTINATION, &learn_strength_dest, lif_info->learn_info));
    ...
    dnx_mpls_term_match_t * match)
{
    ...
    match->label = mpls_port->match_label;

    match->service_tagged = _SHR_IS_FLAG_SET(mpls_port->flags, BCM_MPLS_PORT_SERVICE_TAGGED);

    if (mpls_port->criteria == BCM_MPLS_PORT_MATCH_LABEL_CONTEXT_LABEL)
    {
        match->match_criteria = TERM_MATCH_CRITERIA_TWO_LABELS;
        match->context_label = mpls_port->context_label;
    }
    else if (mpls_port->criteria == BCM_MPLS_PORT_MATCH_LABEL_L3_INGRESS_INTF)
    {
        match->match_criteria = TERM_MATCH_CRITERIA_INTF_LABEL;
        match->context_intf = mpls_port->ingress_if;
    }
    else if (_SHR_IS_FLAG_SET(flags, DNX_MPLS_L2VPN_FLAG_INGRESS_IML_WITH_ESI))
    {
        match->match_criteria = TERM_MATCH_CRITERIA_LABEL_BOS;
        match->bos_val = 0;
        match->vmpi = mpls_port->class_id;
    }
    else if (_SHR_IS_FLAG_SET(flags, DNX_MPLS_L2VPN_FLAG_INGRESS_IML_WITHOUT_ESI))
    {
        match->match_criteria = TERM_MATCH_CRITERIA_LABEL_BOS;
        match->bos_val = 1;
        match->vmpi = mpls_port->class_id;
    ...
    dnx_mpls_l2vpn_info_t * l2vpn)
{
    ...
    l2vpn->mpls_port->class_id = match->vmpi;
    ...
    dnx_mpls_l2vpn_info_t * l2vpn_info)
{
    ...
    pwe_lif_info.keep_payload = _SHR_IS_FLAG_SET(l2vpn_info->mpls_port->flags2, BCM_MPLS_PORT2_KEEP_TTL);
     ...
     */
    SHR_IF_ERR_EXIT(dnx_mpls_l2vpn_match_info_fill(unit, l2vpn_info->flags, l2vpn_info->mpls_port, &term_match));
    ...
    if (is_replace)
    {
        ...
        if (l2vpn_info->type == MPLS_L2VPN_TYPE_EVPN)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_mpls_l2vpn_in_network_group_read
                            (unit, orig_pwe_lif_info.in_lif_profile, pwe_lif_info.dbal_table, l2vpn_info));
        }
    }
    else
    {
    ...
    {
        BCM_FAILOVER_ID_GET(protection_pointer[0], l2vpn_info->mpls_port->ingress_failover_id);
    ...
    if (l2vpn_info->type != MPLS_L2VPN_TYPE_EVPN)
    {
        ...
        if (is_replace &&
            (dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
             DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE))
        {
            SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_get
                            (unit, l2vpn_info->mpls_port->mpls_port_id, 0, &fwd_info));
        }
        else
        {
            sal_memset(&fwd_info, 0, sizeof(dnx_algo_gpm_forward_info_t));
        }
        ...
        if (pwe_lif_info.is_p2p == FALSE)
        {
            ...
            pwe_lif_info.learn_strength = l2vpn_info->mpls_port->learn_strength ? 1 : 0;
        ...
        SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_add
                        (unit, FALSE, FALSE, l2vpn_info->mpls_port->mpls_port_id, &fwd_info));
    }
    ...
    SHR_IF_ERR_EXIT(dnx_mpls_port_ingress_lif_profile_fill_and_allocation
                    (unit, l2vpn_info, orig_pwe_lif_info.in_lif_profile, &pwe_lif_info.in_lif_profile,
                     pwe_lif_info.dbal_table, &last_profile));
    ...
    /** Termination profile */
    SHR_IF_ERR_EXIT(dnx_mpls_l2vpn_to_termination_profile(unit, l2vpn_info->flags,
                                                          _SHR_IS_FLAG_SET(l2vpn_info->mpls_port->flags,
                                                                           BCM_MPLS_PORT_CONTROL_WORD),
                                                          _SHR_IS_FLAG_SET(l2vpn_info->mpls_port->flags,
                                                                           BCM_MPLS_PORT_ENTROPY_ENABLE),
                                                          l2vpn_info->mpls_port->vccv_type, &term_profile_info));
    if (is_replace)
    {
    ...
    if (_SHR_IS_FLAG_SET(l2vpn_info->mpls_port->flags2, BCM_MPLS_PORT2_EXTENDED_TERMINATION))
    {
        SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_write
                        (unit, pwe_lif_info.global_in_lif, DBAL_CORE_ALL, pwe_lif_info.local_in_lif, is_replace));
    }
    ...
    dnx_mpls_l2vpn_info_t * l2vpn_info)
{
    ...
    {
        BCM_FAILOVER_SET(l2vpn_info->mpls_port->ingress_failover_id, l2vpn_info->mpls_port->ingress_failover_id,
    ...
    l2vpn_info->mpls_port->flags2 |= (lif_info.keep_payload ? BCM_MPLS_PORT2_KEEP_TTL : 0);
    ...
    {
        /** get forwarding info */
        SHR_IF_ERR_EXIT(dnx_mpls_port_gport_to_forward_information_get
                        (unit, l2vpn_info->mpls_port->mpls_port_id, l2vpn_info->mpls_port));
        /** Get learn_strength */
        l2vpn_info->mpls_port->learn_strength = lif_info.learn_strength ? 1 : 0;
    }
     ...
     */
    SHR_IF_ERR_EXIT(dnx_mpls_inlif_profile_to_mpls_port_info
                    (unit, lif_info.dbal_table, lif_info.in_lif_profile, l2vpn_info));

    SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_get
                    (unit, gport_hw_resources.global_in_lif, DBAL_CORE_DEFAULT, &ext_term_found));
    if (ext_term_found)
    {
        l2vpn_info->mpls_port->flags2 |= BCM_MPLS_PORT2_EXTENDED_TERMINATION;
    }
    ...
    bcm_gport_t mpls_l2vpn_id)
{
     ...
     */
    /*
     * Because of MBB, the lif in match entry may not be the given one, and even may be not found.
     * So, Get the label match entry from table to check if the lif matches first.
     */
    shr_res = dnx_mpls_termination_lookup(unit, TERM_LOOKUP_EXTRA_GET, &match_info, &removing_local_in_lif);
    if (shr_res != _SHR_E_NOT_FOUND)
    {
        SHR_IF_ERR_EXIT(shr_res);
    ...
    else if (lif_info.dbal_table != DBAL_TABLE_IN_LIF_FORMAT_PWE)
    {
        /** In case of PWE, not_found is allowed.*/
        SHR_IF_ERR_EXIT(shr_res);
    }
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_dealloc
                    (unit, lif_info.in_lif_profile, &new_in_lif_profile, &in_lif_profile_info, LIF));
    ...
    if (ext_term_found)
    {
        SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_delete(unit, gport_hw_resources.global_in_lif, DBAL_CORE_ALL));
    }
