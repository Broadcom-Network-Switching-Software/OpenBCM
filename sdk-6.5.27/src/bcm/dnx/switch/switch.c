    ...
    int *arg)
{
    ...
    core_group = dnx_data_device.general.nof_core_groups_get(unit) > 1 ? (*arg >> 24) & 0xff : 0;
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, field, INST_SINGLE, &fieldval));

    *arg = (fieldval << 8) >> 8;
    *arg = *arg | core_group << 24;
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
    ...
    int switch_header_type)
{
    ...
    if (!BCM_GPORT_IS_TRUNK(port))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_tdm_get(unit, gport_info.local_port, &port_tdm_mode));

        if ((direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_IN)
            && !(DNX_ALGO_PORT_TYPE_IS_ING_PP(unit, port_info, is_lag) ||
                 port_tdm_mode == DNX_ALGO_PORT_TDM_MODE_BYPASS))
        {
    ...
    int *arg)
{
    ...
    switch (type)
    {
        ...
        case bcmSwitchUnknownVlanToCpu:
        {
             ...
             */
            int old_esem_cmd = 0;
            int dummy_app_db_id = 0;
            SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_get(unit, port, &old_esem_cmd));
            /** Check if SVTAG is in the command */
            SHR_IF_ERR_EXIT(dnx_switch_svtag_is_present_in_esem_cmd
                            (unit, old_esem_cmd, &dummy_app_db_id, (uint8 *) arg));
            break;
        }
        case bcmSwitchTrunkEgressSvtagLookupEnable:
        {
            SHR_IF_ERR_EXIT(dnx_switch_svtag_trunk_egress_lookup_enable_get(unit, port, (uint8 *) arg));
            break;
        }
    ...
    BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
    {
            ...
            && direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_IN && port_tdm_mode == DNX_ALGO_PORT_TDM_MODE_NONE)
        {
            ...
            BCM_PBMP_ITER(logical_port_channels, logical_port_i)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_port_in_lag(unit, logical_port_i, &is_lag_i));
                /*
                 * Check if the channel is non-TDM
                 */
                SHR_IF_ERR_EXIT(dnx_algo_port_tdm_get(unit, logical_port_i, &port_tdm_mode));

                if (logical_port_i != local_port && DNX_ALGO_PORT_TYPE_IS_ING_PP(unit, port_info_i, is_lag_i)
        ...
        /** Get the port`s type */
        SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, local_port, &port_info));
        SHR_IF_ERR_EXIT(dnx_algo_port_in_lag(unit, local_port, &is_lag));
        if (DNX_ALGO_PORT_TYPE_IS_ING_PP(unit, port_info, is_lag) &&
            ((direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_IN)
             || (direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_BOTH)))
        {
    ...
    int switch_header_type)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, gport_info.local_port, &port_info));
        SHR_IF_ERR_EXIT(dnx_algo_port_in_lag(unit, gport_info.local_port, &is_lag));
        is_in_pp = DNX_ALGO_PORT_TYPE_IS_ING_PP(unit, port_info, is_lag);
    }

    /** skip non ingress pp ports (TDM bypass special case) */
    if (is_in_pp &&
        (direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_BOTH || direction == DNX_SWITCH_PORT_HEADER_TYPE_INDEX_IN))
    {
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmSwitchTrunkEgressSvtagLookupEnable:
        {
            SHR_IF_ERR_EXIT(dnx_switch_svtag_trunk_egress_lookup_enable_set(unit, port, (uint8) arg));
            break;
        }
        default:
        {
...
/**
* \brief
*  Enable or disable always setting ext_statistics_valid.
* \param [in] unit        - Device Id
...
*/
static int
dnx_switch_control_ext_statistics_valid_always_set(
    int unit,
    int value)
{
    uint32 value_to_set;
    uint32 entry_handle_id;
    dbal_tables_e dbal_table = DBAL_TABLE_PEMLA_PMF;
    dbal_fields_e dbal_field = DBAL_FIELD_EXT_STAT_VALID_SET_ALWAYS;
    dbal_fields_e dbal_field_iter;
    int is_table_active;
    int found_dbal_field = FALSE;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dbal_tables_is_table_active(unit, dbal_table, &is_table_active));
    if (is_table_active == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcmSwitchExtStatAlwaysValid is not available for device.\n");
    }

    SHR_IF_ERR_EXIT(dbal_tables_next_field_get(unit, dbal_table, DBAL_FIELD_EMPTY, FALSE, 0, &dbal_field_iter));
    while (dbal_field_iter != DBAL_FIELD_EMPTY)
    {
        if (dbal_field_iter == dbal_field)
        {
            found_dbal_field = TRUE;
            break;
        }
        SHR_IF_ERR_EXIT(dbal_tables_next_field_get(unit, dbal_table, dbal_field_iter, FALSE, 0, &dbal_field_iter));
    }
    if (found_dbal_field == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcmSwitchExtStatAlwaysValid is not available for device.");
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EXT_STAT_VALID_SET_ALWAYS, INST_SINGLE,
                                 value_to_set);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
*/
static int
dnx_switch_control_ext_statistics_valid_always_get(
    int unit,
    int *value)
{
    uint32 value_get;
    ...
    dbal_value_field32_request(unit, entry_handle_id, dbal_field, INST_SINGLE, &value_get);
    ...
    int arg)
{
    ...
    {
        int current_status, current_v4, current_v6;
        int new_v4, new_v6;

        SHR_IF_ERR_EXIT(dnx_switch_control_routed_learn_get(unit, &current_status));
        current_v4 = (current_status & (BCM_SWITCH_CONTROL_L3_LEARN_IPV4_UC | BCM_SWITCH_CONTROL_L3_LEARN_IPV4_MC));
        current_v6 = (current_status & (BCM_SWITCH_CONTROL_L3_LEARN_IPV6_UC | BCM_SWITCH_CONTROL_L3_LEARN_IPV6_MC));
        new_v4 = (arg & (BCM_SWITCH_CONTROL_L3_LEARN_IPV4_UC | BCM_SWITCH_CONTROL_L3_LEARN_IPV4_MC));
        new_v6 = (arg & (BCM_SWITCH_CONTROL_L3_LEARN_IPV6_UC | BCM_SWITCH_CONTROL_L3_LEARN_IPV6_MC));
        ...
        if (nof_ipv4_host_entries && (new_v4 != current_v4))
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "Couldn't switch on/off routing learning IPv4 when host entry number:%d isn't 0",
                         (nof_ipv4_host_entries));
        }
        if (nof_ipv6_host_entries && (new_v6 != current_v6))
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "Couldn't switch on/off routing learning IPv6 when host entry number:%d isn't 0",
                         (nof_ipv6_host_entries));
        }
    }
    ...
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_IPV4_UC_LEARN, INST_SINGLE, ipv4_uc_learn_enable);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_IPV4_MC_LEARN, INST_SINGLE, ipv4_mc_learn_enable);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_IPV6_UC_LEARN, INST_SINGLE, ipv6_uc_learn_enable);
    {
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_IPV6_MC_LEARN, INST_SINGLE, ipv6_mc_learn_enable);
    }
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_MPLS_LEARN, INST_SINGLE, mpls_learn_enable);
    ...
    int arg)
{
    ...
    if (dnx_data_pp.stages.params_get(unit, DNX_PP_STAGE_VTT5)->valid)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VTT5_IS_LAST_MPLS_TERM_STAGE, INST_SINGLE,
                                     (arg == 0) ? 1 : 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VTT5_ENABLE_LIF_ARR_LOOKUP, INST_SINGLE,
                                     (arg == 0) ? 1 : 0);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    else
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmSwitchRemoteCpuDestMacOui:
        {
            if (((arg >> 24) & 0xff) > (dnx_data_device.general.nof_core_groups_get(unit) - 1))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Invalid value %d provided to switch control type=%d. Supported values are 0~0x%xffffff.\n",
                             arg, type, (dnx_data_device.general.nof_core_groups_get(unit) - 1));
            }
            if (dnx_drv_soc_feature(unit, soc_feature_rcpu_1))
            {
        ...
        case bcmSwitchControlRouteSrv6UspTrapSet:
        {
            /** set LPM Destination to be identified as SRv6 USP Trap */
            SHR_IF_ERR_EXIT(dnx_switch_srv6_lpm_destination_for_srv6_trap_set(unit, arg));
            break;
        }
        ...
        {
            SHR_IF_ERR_EXIT(dnx_vxlan_vni_miss_set(unit, arg));
            break;
        }
        case bcmSwitchEncryptedPfcTxEnable:
        {
            SHR_IF_ERR_EXIT(bcm_dnx_port_encrypted_pfc_tx_enable_set(unit, arg));
            break;
        }
        case bcmSwitchExtStatAlwaysValid:
        {
            SHR_IF_ERR_EXIT(dnx_switch_control_ext_statistics_valid_always_set(unit, arg));
            break;
        }
        default:
        {
    ...
    int *arg)
{
    ...
    switch (type)
    {
        ...
        case bcmSwitchControlRouteSrv6UspTrapSet:
        {
            /** get LPM Destination to be identified as SRv6 USP Trap */
            SHR_IF_ERR_EXIT(dnx_switch_srv6_lpm_destination_for_srv6_trap_get(unit, arg));
            break;
        }
        case bcmSwitchSRV6EgressPSPEnable:
        {
        ...
        case bcmSwitchVxlanVnIdMissToCpu:
        {
            SHR_IF_ERR_EXIT(dnx_vxlan_vni_miss_get(unit, arg));
            break;
        }
        case bcmSwitchExtStatAlwaysValid:
        {
            SHR_IF_ERR_EXIT(dnx_switch_control_ext_statistics_valid_always_get(unit, arg));
            break;
        }
        case bcmSwitchEncryptedPfcTxEnable:
        {
            SHR_IF_ERR_EXIT(bcm_dnx_port_encrypted_pfc_tx_enable_get(unit, arg));
            break;
        }
        default:
        {
...
bcm_dnx_switch_rov_get(
    ...
    uint32 *rov)
{
    uint32 entry_handle_id, core_group_idx;
    ...
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_BLOCK_INDEX, core_group_idx);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_AVS_STATUS, INST_SINGLE, &rov[core_group_idx]);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    }
...
{
    int nof_pvt_mon;
    int pvt_base, pvt_factor, pvt_offset;
    uint32 entry_handle_id;
    ...
    else
    {
        ...
        pvt_offset = dnx_data_pvt.general.pvt_offset_get(unit);
        ...
        pvt_offset = (pvt_offset * pvt_factor) / 10000;
        for (i = 0; i < nof_pvt_mon; i++)
        {
            /*
             * curr [0.1 C] = (pvt_base - curr * pvt_factor)/10000
             */
            temperature_array[i].curr = (pvt_base - current[i] * pvt_factor - pvt_offset) / 10000;
            /*
             * peak [0.1 C] = (pvt_base - curr * peak)/10000
             */
            temperature_array[i].peak = (pvt_base - peak[i] * pvt_factor - pvt_offset) / 10000;
        }
...
/**
 * \brief - Registers a call back function for switch critical events.
 * \param [in] unit - Device unit number
 * \param [in] cb - The desired call back function to register for critical events.
 * \param [in] userdata - Pointer to any user data to carry on.
 * \return
 *   See shr_error_r
 * \remark
...
/**
 * \brief - Unregisters a call back function for switch critical events.
 * \param [in] unit - Device unit number
 * \param [in] cb - The desired call back function to register for critical events.
 * \param [in] userdata - Pointer to any user data to carry on.
 * \return
...
/**
 * \brief - Set event control.
 * \param [in] unit - Device unit number
 * \param [in] event - Always be BCM_SWITCH_EVENT_DEVICE_INTERRUPT.
 * \param [in] type - Event action
 *                    type.event_id: Interrupt event operation type
 *                    type.index: If multi instances blocks represent the specific block index
 *                    type.action: Interrupt action
    ...
    uint32 value)
{
    ...
    if (event != BCM_SWITCH_EVENT_DEVICE_INTERRUPT)
    {
 ...
 *                    type.index: If multi instances blocks represent the specific block index
 *                    type.action: Interrupt action
 * \param [out] value - Point to event value
 * \return
 *   See shr_error_r
 * \remark
 *   * None
    ...
    uint8 *is_opt)
{
    ...
    switch (app_table)
    {
        ...
        case bcmSwitchExternalTableIp4RpfLargeResult:
        {
            *dbal_table_id = DBAL_TABLE_KBP_IPV4_UNICAST_PRIVATE_LPM_RPF;
            *is_opt = FALSE;
            break;
        }
        case bcmSwitchExternalTableIp6RpfLargeResult:
        {
            *dbal_table_id = DBAL_TABLE_KBP_IPV6_UNICAST_PRIVATE_LPM_RPF;
            *is_opt = FALSE;
            break;
    ...
    bcm_switch_control_info_t * value)
{
    ...
    switch (key.type)
    {
        ...
        {
            if (dnx_data_trap.etpp.feature_get(unit, dnx_data_trap_etpp_mtu_context_enable_support) == 0)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                             "The API with key.type = bcmSwitchForwardingLayerMtuFilter is not supported on this device!\n");
            }
        ...
        case bcmSwitchExemScanPeriod:
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "The key.index(%d) value is illegal! For bcmSwitchExemScanPeriod type, valid key.index value is: 0 \n",
                             key.index);
            }
        ...
        case bcmSwitchExemFlushProfilePayloadOffset:
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "The key.index(%d) value is illegal! For bcmSwitchExemFlushProfilePayloadOffset type, valid key.index value is: 0 \n",
                             key.index);
            }
