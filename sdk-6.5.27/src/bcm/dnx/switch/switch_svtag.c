...
static shr_error_e
dnx_switch_svtag_egress_entry_update_esem_command_check(
    int unit,
    bcm_gport_t gport,
    bcm_gport_t tunnel,
    uint8 *esem_cmd_update)
{
    int port_in_lag;
    dnx_algo_gpm_gport_phy_info_t gport_info;
    ...
    *esem_cmd_update = FALSE;

    if (!port_in_lag && !SWITCH_SVTAG_SUPPORTED_LIF_TYPES(tunnel))
    {
        *esem_cmd_update = TRUE;
    }
...
static shr_error_e
dnx_switch_svtag_esem_cmd_data_fill(
    int unit,
    uint8 enable,
    int old_esem_cmd,
    uint32 *flags,
    ...
    uint8 *exchange_and_set_hw)
{
    ...
    *exchange_and_set_hw = FALSE;
    ...
    {
        int app_db_id;
        SHR_IF_ERR_EXIT(dnx_switch_svtag_is_present_in_esem_cmd(unit, old_esem_cmd, &app_db_id, &svtag_present));
        if (svtag_present)
        {
...
shr_error_e
dnx_switch_svtag_esem_cmd_exchange(
    int unit,
    bcm_gport_t gport,
    int old_esem_cmd,
    uint8 enable,
    ...
    uint8 *is_last)
{
    ...
    *is_last = FALSE;
...

    {
        ...
        flags |= DNX_PORT_ESEM_CMD_UPDATE;
        SHR_IF_ERR_EXIT(dnx_switch_svtag_esem_cmd_data_fill(unit, enable, old_esem_cmd, &flags,
                                                            &esem_cmd_info, exchange_and_set_hw));

        if (*exchange_and_set_hw)
        {
    ...
    uint8 enable)
{
    ...
    uint8 is_last = FALSE;
    uint8 exchange_and_set_hw = FALSE;
    ...
    SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_get(unit, gport, &old_esem_cmd));

    SHR_IF_ERR_EXIT(dnx_switch_svtag_esem_cmd_exchange(unit, gport, old_esem_cmd, enable, 0, &new_esem_cmd,
                                                       &exchange_and_set_hw, &esem_cmd_data, &is_first, &is_last));
    ...
    if (exchange_and_set_hw)
    {
        SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_hw_set(unit, gport, new_esem_cmd));
    }
...
shr_error_e
dnx_switch_svtag_trunk_egress_lookup_enable_set(
    int unit,
    bcm_gport_t port,
    uint8 enable)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_switch_svtag_esem_cmd_set(unit, port, enable));
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The switch type bcmSwitchTrunkEgressSvtagLookupEnable is supported only for trunk gport.");
    }
exit:
    SHR_FUNC_EXIT;
}
...
shr_error_e
dnx_switch_svtag_trunk_egress_lookup_enable_get(
    int unit,
    bcm_gport_t port,
    uint8 *arg)
{
    ...
    if (BCM_GPORT_IS_TRUNK(port))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_switch_svtag_is_present_in_esem_cmd(unit, old_esem_cmd, &dummy_app_db_id, (uint8 *) arg));
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The switch type bcmSwitchHashLayersDisable is supported only for trunk gport.");
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_switch_svtag_egress_info_t * svtag_info)
{
    ...
    else
    {
        ...
        {
            uint8 esem_cmd_update = FALSE;
            dbal_action_flag = DBAL_COMMIT;

            SHR_IF_ERR_EXIT(dnx_switch_svtag_egress_entry_update_esem_command_check(unit, svtag_lookup_info->port,
                                                                                    svtag_lookup_info->tunnel,
                                                                                    &esem_cmd_update));

            if (esem_cmd_update)
            {
                SHR_IF_ERR_EXIT(dnx_switch_svtag_esem_cmd_set(unit, svtag_lookup_info->port, TRUE));
            }
        }
    ...
    bcm_switch_svtag_encapsulation_lookup_info_t * svtag_lookup_info)
{
    ...
    else
    {
        ...
        if (esem_cmd_update)
        {
            SHR_IF_ERR_EXIT(dnx_switch_svtag_esem_cmd_set(unit, svtag_lookup_info->port, FALSE));
        }
    }
...
shr_error_e
dnx_switch_svtag_egress_entry_port_exist_check(
    int unit,
    bcm_gport_t port,
    uint8 *svtag_port_exists)
{
    ...
    *svtag_port_exists = FALSE;

    sal_memset(&svtag_lookup_info, 0, sizeof(bcm_switch_svtag_encapsulation_lookup_info_t));
    svtag_lookup_info.port = port;
    rv = dnx_switch_svtag_egress_entry_get_internal(unit, &svtag_lookup_info, &svtag_info);
    if (rv == _SHR_E_NONE)
    {
        *svtag_port_exists = TRUE;
    }
        ...
        if ((_SHR_IS_FLAG_SET(flags, BCM_SWITCH_SVTAG_EGRESS_TRAVERSE_DELETE_ALL)))
        {
            ...
            if (svtag_tables[table_iter] == DBAL_TABLE_ESEM_PORT_SVTAG)
            {
                ...
                while (!is_end)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_switch_svtag_egress_entry_update_esem_command_check
                                    (unit, logical_port, BCM_GPORT_INVALID, &esem_cmd_update));
                    if (esem_cmd_update)
                    {

                        SHR_IF_ERR_EXIT(dnx_switch_svtag_esem_cmd_set(unit, logical_port, FALSE));
                    }
                    SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
                }
            }
    ...
    uint8 *found)
{
    ...
    if (esem_cmd_data.esem[esem_if].valid && (esem_cmd_data.esem[esem_if].designated_offset == esem_entry_offset))
    {
        *found = TRUE;
    ...
    uint8 enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_estimate_bta_set(unit, gport,
                                              ((enable ==
...
shr_error_e
dnx_switch_svtag_pfc_flow_control_mapping_set(
    int unit,
    int value)
{
    uint32 entry_handle_id;
    uint8 is_coe_enable = 0;
    uint8 cal_intf;
    uint16 vid_mask, vid_shift_right, vid_off, ptc_shift_left, ptc_mask;
    int ptc_size = 0;
    ...
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "SVTAG PFC FC can't support.\n");
    }
    if (!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_vid_mapping))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "SVTAG PFC FC channel ID mapping can't support.\n");
    }
    if (dnx_data_fc.coe.calendar_mode_get(unit)->mode != DNX_FC_COE_MODE_PFC)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Can't set when FC mode is not PFC!\n");
    }

    SHR_IF_ERR_EXIT(dnx_port_sit_port_status_get(unit, BCM_PORT_INVALID, DNX_PORT_TYPE_COE, &is_coe_enable));
    if (is_coe_enable)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Can't enable MACSEC PFC FC mapping when COE enabled!\n");
    }

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FC_COE_PARAMETERS, &entry_handle_id));

    for (cal_intf = 0; cal_intf < dnx_data_fc.coe.nof_coe_cal_instances_get(unit); cal_intf++)
    {
        ...
        if (value)
        {
            vid_off = vid_shift_right = vid_mask = 0;
            SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                            (unit, DBAL_TABLE_INGR_REASSEMBLY_CONTEXT_MAP_TABLE, DBAL_FIELD_PORT_TERMINATION_CONTEXT,
                             FALSE, 0, 0, &ptc_size));
            ptc_mask = (1 << ptc_size) - 1;

            ptc_shift_left = 2;
        }
        else
        {
            vid_off = dnx_data_fc.coe.coe_vid_offset_get(unit);
            vid_shift_right = dnx_data_fc.coe.coe_vid_right_shift_get(unit);
            vid_mask = dnx_data_fc.coe.coe_vid_mask_get(unit);
            ptc_mask = dnx_data_fc.coe.coe_ptc_mask_get(unit);
            ptc_shift_left = dnx_data_fc.coe.coe_ptc_left_shift_get(unit);
        }
        dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_COE_VID_OFFSET, INST_SINGLE, vid_off);
        dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_COE_VID_MASK, INST_SINGLE, vid_mask);
        dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_COE_VID_SHIFT_RIGHT, INST_SINGLE,
                                     vid_shift_right);
        dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_COE_PTC_MASK, INST_SINGLE, ptc_mask);
        dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_COE_PTC_SHIFT_LEFT, INST_SINGLE, ptc_shift_left);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
exit:
    DBAL_FUNC_FREE_VARS;
...
shr_error_e
dnx_switch_svtag_pfc_flow_control_mapping_get(
    int unit,
    int *value)
{
    uint32 entry_handle_id;
    uint16 vid_mask, ptc_shift_left, ptc_mask;
    int ptc_size = 0;
    ...
    *value = FALSE;
    ...
    if (!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_support))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "FC channel ID mapping can't support.\n");
    }
    if (dnx_data_fc.coe.calendar_mode_get(unit)->mode != DNX_FC_COE_MODE_PFC)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Can't enable when FC mode is not PFC!\n");
    }
    if (!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_vid_mapping))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "SVTAG PFC FC channel ID mapping can't support.\n");
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FC_COE_PARAMETERS, &entry_handle_id));
    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_COE_IF, 0);
    ...
    dbal_value_field16_request(unit, entry_handle_id, DBAL_FIELD_COE_PTC_SHIFT_LEFT, INST_SINGLE, &ptc_shift_left);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                    (unit, DBAL_TABLE_INGR_REASSEMBLY_CONTEXT_MAP_TABLE, DBAL_FIELD_PORT_TERMINATION_CONTEXT, FALSE, 0,
                     0, &ptc_size));

    if ((vid_mask == 0) && (ptc_mask == ((1 << ptc_size) - 1)) && (ptc_shift_left == 2))
    {
        *value = TRUE;
    }
