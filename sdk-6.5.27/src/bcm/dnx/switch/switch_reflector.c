    ...
    bcm_switch_reflector_data_t * data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_lib_logical_phase_to_encap_access_convert
                    (unit, gport_hw_resources.logical_phase, &(data->encap_access)));
    ...
    bcm_switch_reflector_data_t * data)
{
    ...
    /*
     * encap access verify:
     * Allow input bcmEncapAccessInvalid: used default access
     */
    if ((data->encap_access != bcmEncapAccessInvalid)
        && (data->encap_access != bcmEncapAccessNativeArp) && (data->encap_access != bcmEncapAccessTunnel1))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Encap_accesss can't support.\n");
    }

    if ((data->encap_access != bcmEncapAccessInvalid) && (data->encap_access != bcmEncapAccessNativeArp))
    {
        /**TWAMP not allow to change encap_access*/
        if ((data->type == bcmSwitchReflectorTwampIp4) || (data->type == bcmSwitchReflectorTwampIp6))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Encap_accesss can't changed for TWAMP.\n");
        }
    }
    ...
    bcm_switch_reflector_data_t * data)
{
    ...
    encap_access = data->encap_access;
    /**default to phase2*/
    if (data->encap_access == bcmEncapAccessInvalid)
    {
