    ...
    int unit)
{
    ...
    for (synce_index = 0; synce_index < nof_synce_types; synce_index++)
    {
...
/**
 * \brief
 *   Select clock valid signal for CDR clock
 * \param [in] unit -
 *   The unit number.
 * \param [in] port -
 *   Logical Port number.
 ...
 */
int
dnx_nanosync_nif_lane_clock_valid_set(
    int unit,
    bcm_port_t port)
{
    ...
    uint32 entry_handle_id;
    dnx_algo_port_ethu_access_info_t ethu_info;
    synce_rx_clock_valid_signal_t valid_signal;
    ...
    {
        /*
         * For OTN port, use the CDR lock
         */
        valid_signal = synce_rx_clock_signal_link;
    }
    else
    {
        valid_signal = synce_rx_clock_signal_valid_and_link;
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LINK_VALID_SEL, INST_SINGLE, valid_signal);
    ...
    DNXC_SERDES_REF_CLOCK_TYPE ref_clock_out)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        ...
        /*
         * Enable the clock gating depending on squelch soc property
         */
        gate_enable = dnx_data_synce.general.cfg_get(unit, synce_index)->squelch_enable;
        SHR_IF_ERR_EXIT(dnx_nanosync_nif_lane_gate_enable_set(unit, port, gate_enable));
        /*
    ...
    portmod_port_synce_clk_ctrl_t * config)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
    {
        ...
        {
            if (speed_config.speed >= dnx_data_nif.phys.txpi_sdm_div_get(unit, index)->speed)
            {
                config->sdm_val = dnx_data_nif.phys.txpi_sdm_div_get(unit, index)->val;
                break;
            }
        }
    }
...
/**
 * \brief
 *   Configure SDM divider for TX feedback clock
 *
 * \param [in] unit - The unit number.
 * \param [in] port - Logical Port number.
 * \param [in] divider - Integer divider.
 *
 * \return
 *   See \ref shr_error_e
 * \remark
 ...
 */
int
dnx_nif_synce_feedback_clk_sdm_divider_set(
    int unit,
    bcm_port_t port,
    uint32 divider)
{
     ...
     * Configure DBAL
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SYNCE_NIF_CDU_SDM_DIV_CTRL, &entry_handle_id));
    /*
     * Configure lane ID according to physical lane
     */
    _SHR_PBMP_ITER(phys, lane_id)
    {
        if (lane_id >= dnx_data_nif.phys.nof_phys_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Lane ID is out of bound!\n");
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_SYNCE_CLK_BLK, ethu_info.ethu_id);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LANE_ID,
                                   lane2serdes[lane_id].tx_id - ethu_info.first_lane_in_ethu);

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DIVIDER, INST_SINGLE, divider);
