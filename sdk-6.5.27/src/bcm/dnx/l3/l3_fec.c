    ...
    bcm_if_t * if_id)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_WITH_ID))
    {
        ...
        if ((fec_id > max_fec_id) || (fec_id < dnx_data_l3.fec.first_valid_fec_ecmp_id_get(unit)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "FEC ID %d is out of range - should be bigger than %d and smaller than %d\n",
                         fec_id, dnx_data_l3.fec.first_valid_fec_ecmp_id_get(unit) - 1, max_fec_id);
        }
        ...
        if (!_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_ECMP_RANGE_OVERLAP)
            && (fec_id < dnx_data_l3.ecmp.nof_ecmp_get(unit)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
        ...
        if (DNX_FAILOVER_IS_PROTECTION_ENABLED(egr->failover_id) == TRUE)
        {
            ...
            {
                if ((L3_FEC_ID_TO_EVEN_FEC_ID(fec_id) > max_fec_id)
                    || (L3_FEC_ID_TO_EVEN_FEC_ID(fec_id) < dnx_data_l3.fec.first_valid_fec_ecmp_id_get(unit)))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Primary FEC ID %d is out of range - should be bigger than %d and smaller than %d\n",
                                 L3_FEC_ID_TO_EVEN_FEC_ID(fec_id),
        ...
        if (_SHR_IS_FLAG_SET(flags, BCM_L3_REPLACE))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_l3_fec_super_fec_type_get(unit, fec_id, &super_fec_new_resource_type));
    ...
    if (DNX_FAILOVER_IS_PROTECTION_ENABLED(egr->failover_id))
    {
        ...
        {
            if (!dnx_data_failover.
                facility.feature_get(unit, dnx_data_failover_facility_protection_any_hierarchy_supported))
            {
                /** Facility protection is only allowed in 3rd hierarchy on some devices */
                if (_SHR_IS_FLAG_SET(egr->flags, BCM_L3_3RD_HIERARCHY) == FALSE)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Facility protection is only allowed in 3rd hierarchy FEC on this device.\r\n");
                }
            }
        }
    ...
    if (egr->stat_pp_profile != STAT_PP_PROFILE_INVALID)
    {
        ...
        if (egr->failover_id)
        {
            ...
            {
                if (stat_pp_profile_info.fec_profile != bcmStatFecProfileDirect)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "For protection fec, stat pp profile must be created with stat FEC profile direct");
                }
            }
        ...
        else
        {
             ...
             */
            if ((stat_pp_profile_info.fec_profile == bcmStatFecProfileDirect) &&
                (egr->stat_id != STAT_PP_USE_DIRECT_STAT_ID))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Stat PP profile is configured for direct FEC stats, but stat_id is %d (not direct)",
                             egr->stat_id);
            }
        }
    }
...
/**
 * \brief
 * Verify function for bcm_l3_egress_create with BCM_L3_INGRESS_ONLY flag after FEC is allocated.
 *
 *   \param [in] unit - Relevant unit.
 *   \param [in] flags - Similar to bcm_l3_egress_create api input
 *   \param [in] egr -Similar to bcm_l3_egress_create api input
 ...
 */
shr_error_e
dnx_l3_egress_create_fec_post_allocate_verify(
    int unit,
    uint32 flags,
    bcm_l3_egress_t * egr,
    bcm_if_t * if_id)
{
    ...
    if (egr->stat_pp_profile != STAT_PP_PROFILE_INVALID)
    {
        if (DNX_FAILOVER_IS_STANDBY(egr->failover_if_id) == FALSE)
        {
            uint32 stat_id_to_check =
                (egr->stat_id != STAT_PP_USE_DIRECT_STAT_ID) ? egr->stat_id : BCM_L3_ITF_VAL_GET(*if_id);
            SHR_IF_ERR_EXIT(dnx_stat_pp_crps_verify
                            (unit, _SHR_CORE_ALL, stat_id_to_check, egr->stat_pp_profile,
    ...
    bcm_if_t * if_id)
{
    ...
    if (is_alloc_required == TRUE)
    {
        ...
        fec_alloc_data.super_fec_type_tag = super_fec_resource_type;
        SHR_IF_ERR_EXIT(dnx_algo_l3_fec_allocate(unit, &fec_index, alloc_flags, fec_db, &fec_alloc_data));
    }
    /*
    ...
    bcm_l3_egress_t * egr)
{
    ...
    egr->flags2 |= ((BCM_L3_ITF_VAL_GET(intf)) < dnx_data_l3.ecmp.nof_ecmp_get(unit)) ?
    ...
    bcm_if_t * if_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_egress_create_fec_post_allocate_verify(unit, flags, egr, if_id));
