 ...
 */
#define L3_ECMP_GET_CONSISTENT_TABLE_TYPE(unit,use_large_table, max_path) (use_large_table ? DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE : (max_path  > SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit,DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE)->entry_size_in_bits) ? DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE : DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE))
/*
 *Verifies that the consistent table type is valid.
 * type - the consistent hashing type table
 */
#define L3_ECMP_CONSISTENT_TYPE_VERIFY(type) SHR_IF_ERR_EXIT((type >= DBAL_NOF_ENUM_ECMP_CONSISTENT_TABLE_VALUES) ? _SHR_E_INTERNAL : _SHR_E_NONE)
/*
...
/*
 * the consistent hashing manager unique key is composed from the profile id, the hierarchy and is UDEP identification.
 * This MACRO extracts the hierarchy from the key
 */
...
/*
 * the consistent hashing manager unique key is composed from the profile id, the hierarchy and is UDEP identification.
 * This MACRO extracts the profile from the key
 */
...
/*
 * the consistent hashing manager unique key is composed from the profile id, the hierarchy and is UDEP identification.
 * This MACRO extracts the is USER profile identification from the key
 */
#define L3_ECMP_USER_PROFILE_DATA_FROM_CHM_KEY_GET(unit, chm_key) ((chm_key >> L3_ECMP_USER_PROFILE_DATA_OFFSET(unit)) & L3_ECMP_USER_PROFILE_DATA_MASK)
    ...
    bcm_l3_egress_ecmp_t * ecmp)
{
    ...
    /*
     * Verify the specified dynamic_mode in the ecmp structure.
     */
    ...
    char **profile_resouce)
{
    ...
    if (hierarchy < dnx_data_mdb.direct.nof_fec_ecmp_hierarchies_get(unit))
    {
    ...
    dnx_l3_ecmp_profile_t * ecmp_profile)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ecmp_profile_hier_table.profile_data_get
                    (unit, ecmp_hierarchy, ecmp_atr.profile_index, &ref_count, ecmp_profile));
    ...
    if (ecmp_profile->ecmp_mode == DBAL_ENUM_FVAL_ECMP_MODE_MULTIPLY_AND_DIVIDE)
    {
        ...
        *member_count = ecmp_profile->group_size;
        ...
        ecmp_profile->consistent_hashing_id = 0;
    }
    else
    {
        /*
         * In case of EUDP only the first FEC need to be returned same as in the create.
         */
        if (ecmp_profile->user_defined_ecmp_profile != 0)
        {
            *member_count = 1;
            intf_array[0] = ecmp_atr.fec_base;
        }
        else
        {
            uint32 address = 0;
            ...
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_ECMP_MEMBERS_BASE, INST_SINGLE, &address);
             ...
             */
            address =
                (address + tp_table_index) * L3_ECMP_CONSISTENT_NOF_ROWS_IN_MEMBER_TABLE(unit, consistent_table_type);
    ...
    *intf_count = actual_members_count;
    ...
    if (ecmp_profile.ecmp_mode == DBAL_ENUM_FVAL_ECMP_MODE_CONSISTENT_LARGE_TABLE)
    {
        ecmp->ecmp_group_flags |= BCM_L3_ECMP_LARGE_TABLE;
             ...
             ecmp_profile.group_size <=
             SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit,
    ...
    if (ecmp_profile.user_defined_ecmp_profile != 0)
    {
        ...
        ecmp->user_profile = ecmp_profile.user_defined_ecmp_profile;
    }
    else
    {
        /*
         * The max_paths, RPF mode and the tunnel priority mode should only be set in case the ECMP doesn't use a EUDP
         * which already contains these configuration settings.
         */
        ecmp->rpf_mode = L3_ECMP_RPF_MODE_DBAL_ENUM_TO_BCM_ENUM(ecmp_profile.rpf_mode);
    ...
    dnx_l3_ecmp_profile_t * ecmp_profile)
{
    ...
    if (is_wcmp)
    {
        ecmp_profile->group_size = intf_count;

        /*
         * WCMP groups don't care about the members table hence:
         * consistent_hashing_id = L3_ECMP_FULL_MEMBERS_IN_GROUP
         */
        ecmp_profile->consistent_hashing_id = L3_ECMP_FULL_MEMBERS_IN_GROUP;
    }
    else if (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_USER_PROFILE))
    {
        dnx_l3_ecmp_user_profile_t ecmp_user_profile;
        ecmp_group_max_size = UTILEX_MAX(intf_count, ecmp->max_paths);
        SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_user_profile_data_get(unit, ecmp->user_profile, &ecmp_user_profile));

        ecmp_profile->ecmp_mode = ecmp_user_profile.ecmp_mode;
        ecmp_profile->group_size = ecmp_group_max_size;
        ecmp_profile->consistent_hashing_id =
            L3_ECMP_CHM_KEY_GET(unit, ecmp->user_profile, ecmp_user_profile.hierarchy,
                                DNX_L3_ECMP_UDEP_ALLOC_STATE_APPLY(unit, ecmp_user_profile.chm_alloc_state));
        ecmp_profile->rpf_mode = ecmp_user_profile.rpf_mode;
        ecmp_profile->user_defined_ecmp_profile = ecmp->user_profile;
        if (dnx_data_l3.ecmp.tunnel_priority_support_get(unit))
        {
            ecmp_profile->tunnel_priority_map_profile = ecmp_user_profile.tunnel_priority_map_profile;
            ecmp_profile->tunnel_priority_mode = ecmp_user_profile.tunnel_priority_mode;
        }
        ecmp->flags |= L3_EGRESS_ECMP_GET_HIER_FLAGS(ecmp_user_profile.hierarchy);
        SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_user_profile_update_ref_count(unit, ecmp->user_profile, 1));
    }
    else
    {
        ecmp_group_max_size = UTILEX_MAX(intf_count, ecmp->max_paths);
        ...
        else if (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_LARGE_TABLE))
        {
            ecmp_profile->ecmp_mode = DBAL_ENUM_FVAL_ECMP_MODE_CONSISTENT_LARGE_TABLE;
                 ...
                 SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit,
                                                                     DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE)->entry_size_in_bits))
        {
            ecmp_profile->ecmp_mode = (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_MEDIUM_TABLE)) ?
                DBAL_ENUM_FVAL_ECMP_MODE_CONSISTENT_MEDIUM_TABLE : DBAL_ENUM_FVAL_ECMP_MODE_CONSISTENT_SMALL_TABLE;
        }
        else
        {
         ...
         * 1. non consistent ECMP groups don't care about the members table hence: consistent_hashing_id = L3_ECMP_FULL_MEMBERS_IN_GROUP
        ...
        ecmp_profile->consistent_hashing_id =
            (((ecmp_profile->ecmp_mode == DBAL_ENUM_FVAL_ECMP_MODE_MULTIPLY_AND_DIVIDE)
              || (intf_count == ecmp_group_max_size))
             && (ecmp->tunnel_priority.mode == bcmL3EcmpTunnelPriorityModeDisable)
             && (!force_unique_profile)) ? L3_ECMP_FULL_MEMBERS_IN_GROUP : L3_ECMP_PROFILE_UNIQUE_KEY(unit,
                                                                                                      BCM_L3_ITF_VAL_GET
                                                                                                      (ecmp->ecmp_intf),
                                                                                                      intf_count);
    ...
    if (!_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_USER_PROFILE))
    {
...
/** Please refer to the .h file for more information */
shr_error_e
    ...
    int *profile_index)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_fill_profile
                    (unit, ecmp, intf_array[0], intf_count, force_unique_profile, FALSE, &ecmp_profile));
    ...
    if (first_reference)
    {
        ...
        if (ecmp->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT)
        {
            ...
            else
            {
                dnx_l3_ecmp_user_profile_t ecmp_user_profile;
                SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_user_profile_data_get(unit, ecmp->user_profile, &ecmp_user_profile));
                member_table_addr = ecmp_user_profile.consistent_members_table_addr;
            }
            unique_identifyer = L3_ECMP_CHM_KEY_GET(unit, *profile_index, hierarchy, 0);
            SHR_IF_ERR_EXIT(dnx_l3_ecmp_consistent_table_type_from_ecmp_mode_get
                            (unit, ecmp_profile.ecmp_mode, &consistent_table_type));
            chm_offset = member_table_addr * L3_ECMP_CONSISTENT_NOF_RESOURCES_TAKEN(unit, consistent_table_type);
            SHR_IF_ERR_EXIT(ecmp_db_info.
...
/**
 * \brief
 * This function gets the member base address and the ECMP mode
 *  of an ECMP group profile from DBAL.
 * \param [in] unit - The unit number.
 * \param [in] ecmp_profile_index -
 *   The ID of the ECMP group profile
 * \param [in] ecmp_hierarchy -
 *   The ECMP hierarchy.
 * \param [out] member_base_address -
 *   The address to the member table
 * \param [out] ecmp_mode - ECMP hashing mode
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 * \see
 ...
 */
static shr_error_e
dnx_l3_egress_ecmp_profile_mode_address_get(
    int unit,
...
/**
 * \brief
 * This function removes an ECMP group profile from DBAL.
 * \param [in] unit - The unit number.
 * \param [in] ecmp_profile_index -
 *   The ID of the ECMP group profile
 * \param [in] ecmp_hierarchy -
 ...
 */
static shr_error_e
dnx_l3_egress_ecmp_group_profile_destroy(
    int unit,
    uint32 ecmp_profile_index,
    dbal_enum_value_field_hierarchy_level_e ecmp_hierarchy)
{
    ...
    dbal_enum_value_field_hierarchy_level_e ecmp_hierarchy)
{
    ...
    if (ref_count == 1)
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_profile_mode_address_get
                        (unit, ecmp_profile_index, ecmp_hierarchy, &member_base_address, &ecmp_mode));
        /*
        ...
        if ((ecmp_mode != DBAL_ENUM_FVAL_ECMP_MODE_MULTIPLY_AND_DIVIDE))
        {
            ...
            profile_key_expected = L3_ECMP_CHM_KEY_GET(unit, ecmp_profile_index, ecmp_hierarchy, 0);
            ...
            while (DNX_SW_STATE_LL_IS_NODE_VALID(node))
            {
                ...
                SHR_IF_ERR_EXIT(ecmp_db_info.members_tbl_addr_profile_link_lists.next_node(unit, *node_ptr, node_ptr));
            ...
            if (ecmp_profile.user_defined_ecmp_profile == 0)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_consistent_hashing_manager_profile_free
                                (unit, consistent_hashing_manager_handle,
                                 L3_ECMP_CHM_KEY_GET(unit, ecmp_profile_index, ecmp_hierarchy, 0),
                                 consistent_table_type));
            }
        }
        ...
        SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_group_profile_destroy(unit, ecmp_profile_index, ecmp_hierarchy));
    ...
    if (ecmp_profile.user_defined_ecmp_profile != 0)
    {
        SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_user_profile_update_ref_count
                        (unit, ecmp_profile.user_defined_ecmp_profile, 0));
    }
    /*
    ...
    uint32 ecmp_group_flags)
{
    ...
    /*
     * If the function advanced_algorithm_info_get returns indication that the group we are currently
     * allocating is the first one in the bank of the stage map (nof_elements = 1), then we need to update the DBAL table
    ...
    SHR_IF_ERR_EXIT(DNX_L3_ECMP_GROUP_RESOURCE_CB
                    (unit, tag_info.element, advanced_algorithm_info_get(unit, &tag_info)));
    if (tag_info.nof_elements == 1)
    {
        int bank_id;
        bank_id = *ecmp_index / dnx_data_l3.ecmp.ecmp_bank_size_get(unit);
    ...
    bcm_if_t ecmp_index)
{
    ...
    /*
     * If the function advanced_algorithm_info_get returns indication that the group we are currently
     * deallocating is the last one in the bank of the hierarchy map (tag_info.nof_elements = 1), then we need to
     * update the DBAL table responsible for saving the ECMP bank to hierarchy mapping.
     */
    sal_memset(&tag_info, 0, sizeof(tag_info));
    tag_info.element = ecmp_index;
    SHR_IF_ERR_EXIT(DNX_L3_ECMP_GROUP_RESOURCE_CB
    ...
    bcm_if_t intf)
{
    ...
    uint32 max_fec_id;
    uint8 is_allocated = 0;
    uint8 new_member_protection_state;
    ...
    SHR_IF_ERR_EXIT(dnx_algo_l3_fec_protection_state_get(unit, intf, &new_member_protection_state));

    if (new_member_protection_state != ecmp_protection_state)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "New member %d protection state %d isn't matching the ECMP group protection state %d.", intf,
                     new_member_protection_state, ecmp_protection_state);
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_if_t intf)
{
    ...
    if (profile_data->user_defined_ecmp_profile != 0)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "Cannot delete member from ECMP group created with a user profile.");
    }
    ...
    bcm_if_t intf)
{
    ...
    else
    {
        if (profile_data->user_defined_ecmp_profile != 0)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "Cannot add member to an ECMP group created with a user profile.");
        }
        /*
         * Verify that the intf value is in range.
         */
 ...
 */
shr_error_e
dnx_l3_egress_ecmp_create_common_verify(
    int unit,
    uint8 is_wcmp,
    bcm_l3_egress_ecmp_t * ecmp)
{
    ...
    if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_WITH_ID))
    {
         ...
         * Verify the ecmp ID value is valid.
         */
        if ((ecmp_index >= dnx_data_l3.ecmp.nof_ecmp_get(unit)
             || ecmp_index < dnx_data_l3.fec.first_valid_fec_ecmp_id_get(unit)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "ECMP %d is out of range, ECMP id must be below %d and above 0.",
                         ecmp_index, dnx_data_l3.ecmp.nof_ecmp_get(unit));
        }

        /*
         * In case no ECMP ID was specified an unallocated ECMP ID will be taken,
         * so there is no need to check the allocation state.
         */
        ...
        if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_REPLACE))
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_hierarchy_get(unit, ecmp_index, (uint32 *) &current_ecmp_hierarchy));

            if (current_ecmp_hierarchy != L3_ECMP_GET_HIERARCHY(ecmp->flags))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
        ...
        if (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_EXTENDED)
            && (!L3_ECMP_INDEX_IS_IN_EXTENDED_RANGE(unit, ecmp_index)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
        ...
        if (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_EXTENDED) && !is_allocated)
        {
            uint8 res_mngr_is_allocated = FALSE;
            SHR_IF_ERR_EXIT(DNX_L3_ECMP_GROUP_RESOURCE_CB(unit, ecmp_index,
                                                          is_allocated(unit, ecmp_index, &res_mngr_is_allocated)));
            if (res_mngr_is_allocated)
            {
                SHR_ERR_EXIT(_SHR_E_FULL, "The bank to which the ECMP ID %d belongs to is unavailable \n", ecmp_index);
            }
        }
    }
    else
    {
        ...
        /*
         * Using the replace flag requires the "with ID" flag (to indicate the replaced ECMP member).
         */
        if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_REPLACE))
        {
    ...
    bcm_if_t * intf_array)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_create_common_verify(unit, FALSE /* is_wcmp */ , ecmp));
    ...
    {
        if (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_LARGE_TABLE))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Two table size flags were given.");
        }
        if (intf_count >
            SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit,
                                                                DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE)->entry_size_in_bits))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Illegal use of BCM_L3_ECMP_MEDIUM_TABLE flag,\nthis flag can only be used when intf_array size is %d at most.",
                         SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit,
                                                                             DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE)->entry_size_in_bits));
        }
    }
    ...
    {
        table_id = DBAL_TABLE_SUPER_FEC_1ST_HIERARCHY;
    }
    /** Get number of result types of this table */
    SHR_IF_ERR_EXIT(dbal_tables_table_nof_res_type_get(unit, table_id, &nof_super_fec_res_types));

    SHR_ALLOC_SET_ZERO(protection_per_super_fec_type, sizeof(uint8) * nof_super_fec_res_types,
                       "Protection per super FEC result types", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    for (int fec_res_type = 0; fec_res_type < nof_super_fec_res_types; fec_res_type++)
    {
        fec_resource_info = dnx_data_l3.fec.fec_resource_info_get(unit, fec_res_type);
        protection_per_super_fec_type[fec_res_type] = fec_resource_info->is_protection_type;
    }
    /*
    ...
    for (fec_iter = 0, max_fec = 0, min_fec = max_fec_id; fec_iter < intf_count; fec_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_l3_fec_super_fec_type_get
                        (unit, BCM_L3_ITF_VAL_GET(fec_index), &super_fec_resource_type));

        fec_protection = protection_per_super_fec_type[super_fec_resource_type];
    ...
    if (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_USER_PROFILE))
    {
        ...
        if (ecmp_profile_data.hierarchy != ecmp_hierarchy)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Hierarchy value %d of the EUDP must be the same as the hierarchy of the ECMP group - %d\n",
                         ecmp_profile_data.hierarchy, ecmp_hierarchy);
        }
        ...
        if (ecmp->rpf_mode != bcmL3EcmpUrpfInterfaceDefault)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "If the BCM_L3_ECMP_USER_PROFILE is set the RPF mode will be derived from the provided user profile and the ECMP configured RPF mode should be set to bcmL3EcmpUrpfInterfaceDefault\n");
        }

        if (ecmp->tunnel_priority.mode != bcmL3EcmpTunnelPriorityModeDisable)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "In case the BCM_L3_ECMP_USER_PROFILE is set the tunnel priority mode must be set to bcmL3EcmpTunnelPriorityModeDisable as the user profile will determine the tunnel priority mode\n");
        }

        if (intf_count != 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "An incorrect number of interfaces have been provided in the intf_array when flag BCM_L3_ECMP_USER_PROFILE is provided - expected 1 but found %d.\n",
                         intf_count);
        }
        if (ecmp->dynamic_mode != BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "When creating an ECMP group with a user-defined profile, dynamic mode must be RESILIENT.\n");
        }

        if (ecmp->max_paths != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "The number of used paths will be the result of the user define profile so when setting the BCM_L3_ECMP_USER_PROFILE the max_paths should be set to 0.\n");
        }
...
exit:
    SHR_FREE(protection_per_super_fec_type);
    SHR_FUNC_EXIT;
}
    ...
    bcm_if_t * intf_array)
{
    ...
    if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_REPLACE))
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_hierarchy_get(unit, ecmp->ecmp_intf, &prev_ecmp_hierarchy));
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ecmp_profile_hier_table.profile_data_get
                        (unit, prev_ecmp_hierarchy, prev_ecmp_profile_index, &ref_count, &ecmp_profile));
    ...
    bcm_l3_egress_ecmp_t * ecmp)
{
     ...
     * If mode is VIP, branch off to VIP ECMP processing 
     */
    /** allow ECMP VIP destroy */
    if (dnx_data_l3.vip_ecmp.feature_get(unit, dnx_data_l3_vip_ecmp_supported))
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_vip_is_vip_id_get(unit, ecmp, &is_vip_id));
    }
    if (is_vip_id)
    {
    ...
    bcm_if_t intf)
{
    ...
    if (current_ecmp.dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT)
    {
        ...
        if (current_ecmp.max_paths == intf_count)
        {
            ...
            unique_identifyer = L3_ECMP_CHM_KEY_GET(unit, ecmp_profile_index, ecmp_hierarchy, 0);
            /*
    ...
    dbal_enum_value_field_ecmp_consistent_table_e consistent_table_type)
{
    ...
    if (consistent_table_type >= DBAL_NOF_ENUM_ECMP_CONSISTENT_TABLE_VALUES)
    {
    ...
    uint32 *table_address)
{
    ...
    is_user_profile = L3_ECMP_USER_PROFILE_DATA_FROM_CHM_KEY_GET(unit, profile_key);

    /**
     * Get the ecmp_mode from sw state.
     * If the profile is an user-defined one, we will access the sw state structure array.
     * If it is not a UDEP, we will access the template manager.
     */
    if (is_user_profile != 0)
    {
        dnx_l3_ecmp_user_profile_t user_ecmp_profile;
        SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_user_profile_data_get(unit, ecmp_profile_indx, &user_ecmp_profile));
        ecmp_mode = user_ecmp_profile.ecmp_mode;
    }
    else
    {
        SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ecmp_profile_hier_table.profile_data_get
                        (unit, ecmp_hierarchy, ecmp_profile_indx, &ref_count, &ecmp_profile));
        ecmp_mode = ecmp_profile.ecmp_mode;
    }
    ...
    uint32 chm_resource_offset)
{
    ...
    if (is_user_profile)
    {
        int udep_profile_id = (int) ecmp_profile_indx;
        user_ecmp_profile.consistent_members_table_addr =
            chm_resource_offset / L3_ECMP_CONSISTENT_NOF_RESOURCES_TAKEN(unit, consistent_table_type);
        /** Replace ECMP profile data with the new member table address */
        SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_user_profile_allocate(unit, &udep_profile_id, &user_ecmp_profile));
    }
    else
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ECMP_GROUP_PROFILE_TABLE, &entry_handle_id));
...
/** Refer to the .h file */
shr_error_e
ecmp_chm_update_linked_list_offset(
    int unit,
    uint32 old_offset,
    uint32 new_offset)
{
    ...
    uint32 nof_consecutive_profiles)
{
    ...
    SHR_IF_ERR_EXIT(ecmp_chm_update_linked_list_offset(unit, old_offset, new_offset));
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 *chm_offset)
{
    ...
    is_user_profile = L3_ECMP_USER_PROFILE_DATA_FROM_CHM_KEY_GET(unit, unique_identifyer);
    profile_id = L3_ECMP_PROFILE_ID_FROM_CHM_KEY_GET(unit, unique_identifyer);
    if (is_user_profile != 0)
    {
        dnx_l3_ecmp_user_profile_t user_ecmp_profile;
        SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_user_profile_data_get(unit, profile_id, &user_ecmp_profile));
        ecmp_mode = user_ecmp_profile.ecmp_mode;
        member_base_address = user_ecmp_profile.consistent_members_table_addr;
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_profile_mode_address_get(unit, profile_id,
                                                                    L3_ECMP_HIERARCHY_FROM_CHM_KEY_GET(unit,
                                                                                                       unique_identifyer),
                                                                    &member_base_address, &ecmp_mode));
    }
    SHR_IF_ERR_EXIT(dnx_l3_ecmp_consistent_table_type_from_ecmp_mode_get(unit, ecmp_mode, &consistent_table_type));
    /*
    ...
    void *user_data)
{
    ...
    while (!is_end)
    {
...

        {
            SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_group_data_get
                            (unit, ecmp.ecmp_intf, &ecmp, DNX_DATA_MAX_L3_ECMP_MAX_GROUP_SIZE, intf_array,
                             &intf_count));
     ...
     */
    /** allow ECMP VIP traversal once VRs are initialized */
    if (dnx_data_l3.vip_ecmp.feature_get(unit, dnx_data_l3_vip_ecmp_supported))
    {
    ...
    bcm_l3_egress_ecmp_resilient_entry_t * entry)
{
    ...
    if (!dnx_data_l3.vip_ecmp.feature_get(unit, dnx_data_l3_vip_ecmp_supported))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcm_dnx_l3_egress_ecmp_resilient_add is not supported on this device.");
    }
    ...
    bcm_l3_egress_ecmp_resilient_entry_t * entry)
{
    ...
    if (!dnx_data_l3.vip_ecmp.feature_get(unit, dnx_data_l3_vip_ecmp_supported))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcm_dnx_l3_egress_ecmp_resilient_delete is not supported on this device.");
    }
    ...
    bcm_l3_egress_ecmp_resilient_entry_t * replace_entry)
{
    ...
    if (!dnx_data_l3.vip_ecmp.feature_get(unit, dnx_data_l3_vip_ecmp_supported))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcm_dnx_l3_egress_ecmp_resilient_replace is not supported on this device.");
    }
    ...
    void *user_data)
{
    ...
    if (!dnx_data_l3.vip_ecmp.feature_get(unit, dnx_data_l3_vip_ecmp_supported))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcm_dnx_l3_egress_ecmp_resilient_traverse is not supported on this device.");
    }
 ...
 */
shr_error_e
dnx_l3_egress_ecmp_profile_sw_get(
    int unit,
    bcm_if_t ecmp_index,
    dnx_l3_ecmp_profile_t * ecmp_profile)
{
    uint16 group_size_minus_one = 0;
    uint32 ecmp_hierarchy;
    dnx_l3_ecmp_attributes_t ecmp_atr;
    SHR_FUNC_INIT_VARS(unit);

    if (ecmp_profile != NULL)
    {
        sal_memset(ecmp_profile, 0, sizeof(dnx_l3_ecmp_profile_t));

        /*
         * Get the hierarchy of the group
         */
        SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_hierarchy_get(unit, ecmp_index, &ecmp_hierarchy));
        ...
        /*
         * Get the ECMP profile
         */
        SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ecmp_profile_hier_table.profile_data_get
                        (unit, ecmp_hierarchy, ecmp_atr.profile_index, NULL, ecmp_profile));

        /*
         * Get actual ECMP group size
         */
        SHR_IF_ERR_EXIT(ecmp_db_info.ecmp_group_size_minus_one.get(unit, ecmp_hierarchy, ecmp_atr.profile_index,
                                                                   &group_size_minus_one));
