    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        dbal_tables_field_size_get(unit, vrrp_table->table[0], DBAL_FIELD_ZERO_PADDING, 1, 0, 0, &zero_padding_size);

        /** Set FFC 0 - zero padding */
        exem_ffc = DNX_L3_VRRP_EXEM_FFC_BUILD(0, zero_padding_size - 1, DNX_L3_VRRP_FFC_GEN_VAR_OFFSET(unit) + 1);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 0, exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 0, TRUE);

        /** Set FFC 1 - next layer protocol */
        exem_ffc = DNX_L3_VRRP_EXEM_FFC_BUILD(zero_padding_size, DNX_L3_VRRP_FFC_LAYER_PROTOCOL_WIDTH - 1,
                                              DNX_L3_VRRP_FFC_LAYER_PROTOCOL_OFFSET(unit));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 1, exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 1, TRUE);
        ...
        exem_ffc =
            DNX_L3_VRRP_EXEM_FFC_BUILD(DNX_L3_VRRP_FFC_LAYER_PROTOCOL_WIDTH + zero_padding_size,
        ...
        if (dnx_data_device.general.nof_bits_for_nof_cores_get(unit) && core_mode != DBAL_CORE_MODE_DPC)
        {
            ...
            exem_ffc =
                DNX_L3_VRRP_EXEM_FFC_BUILD(DNX_L3_VRRP_FFC_LAYER_PROTOCOL_WIDTH +
                                           (dnx_data_port.general.pp_port_bits_size_get(unit)) + zero_padding_size,
                                           1 - 1, DNX_L3_VRRP_FFC_GEN_VAR_OFFSET(unit));
