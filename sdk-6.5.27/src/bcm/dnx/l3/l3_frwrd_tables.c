...
/*
 * Define the list of supported BCM_L3_FLAGS2_* flags for L3 Host APIs.
 */
#define L3_FORWARD_SUPPORTED_BCM_L3_FLAGS2_HOST 0
/*
 * }
 */
...
/*
 * Define the list of supported flags for bcm_l3_route_traverse API
 */
#define L3_SUPPORTED_BCM_L3_FLAGS_ROUTE_TRAVERSE (BCM_L3_IP6 | BCM_L3_HIT_CLEAR)

/*
 * Define the list of supported flags for L3 Host APIs
 */
#define L3_SUPPORTED_BCM_L3_FLAGS_HOST (BCM_L3_IP6 | BCM_L3_REPLACE | BCM_L3_HOST_LOCAL | BCM_L3_ENCAP_SPACE_OPTIMIZED | BCM_L3_HIT_CLEAR | BCM_L3_HIT)
 ...
 */
/*
 * The commit action according to the replace option, if replace the update will be used otherwise the force to
 * create the entry if doesn't exists.
 */
const dbal_entry_action_flags_e l3_frwrd_table_commit_actions[2] = { DBAL_COMMIT_FORCE, DBAL_COMMIT_UPDATE };
/*
 * }
 */
...
/* see .h file */
shr_error_e
dnx_l3_route_validate_result_by_fields_size(
    int unit,
    uint32 dbal_table,
    uint32 result_type,
    ...
    uint32 dest_value)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                    (unit, dbal_table, *dest_type, FALSE, result_type, INST_SINGLE, &table_field_size));
    /** Take the smaller of the two sizes */
    field_size = (field_size > table_field_size ? table_field_size : field_size);
    if (dest_value >= SAL_BIT(field_size))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Raw value for 'DESTINATION or FEC' (%d) is out of range, should be lower than %d.",
                     dest_value, SAL_BIT(field_size));
    }
    ...
    bcm_l3_route_t * info)
{
    ...
    if (_SHR_IS_FLAG_SET(info->l3a_flags, ~dnx_data_l3.fwd.uc_supported_route_flags_get(unit)))
    {
    ...
    if (_SHR_IS_FLAG_SET(info->l3a_flags2, ~dnx_data_l3.fwd.uc_supported_route_flags2_get(unit)))
    {
    ...
    dbal_fields_e * dest_type)
{
    ...
    if (!_SHR_IS_FLAG_SET(info->l3a_flags2, BCM_L3_FLAGS2_RAW_ENTRY))
    {
        ...
        *dest_type = (is_default ? DBAL_FIELD_FEC_DEFAULT : DBAL_FIELD_FEC);
    ...
    else
    {
         ...
         */
        *dest_type = DBAL_FIELD_KAPS_RESULT;

        SHR_IF_ERR_EXIT(dnx_l3_route_validate_result_by_fields_size(unit, dbal_table, 0, dest_type, fec_index));
    }
 ...
 * \param [in] table_hb_enabled -
 *  Indication whether hitbit is enabled for dbal_table.
 * \param [in] hitbit_flags -
 *  The relevant hit bit flags.
    ...
    uint8 is_traverse)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_KAPS_RESULT, INST_SINGLE, &destination_val));
    ...
    dest_type = DNX_L3_ENTRY_IS_DEFAULT(unit, info) ? DBAL_FIELD_FEC_DEFAULT : DBAL_FIELD_FEC;
    ...
    SHR_IF_ERR_EXIT(dnx_l3_route_payload_raw_resolve_get
                    (unit, info, FALSE, dest_type, dbal_sub_field, result_value, destination_val));
 ...
 * \param [in] unit - The unit number.
 * \param [in,out] info - the route structure see bcm_dnx_l3_route_get
 * \return
    ...
    bcm_l3_route_t * info)
{
    ...
    /** Verify flags */
    SHR_IF_ERR_EXIT(dnx_l3_route_flags_verify(unit, info));
    /** Verify key */
    ...
    if (table_hb_enabled)
    {
        hitbit_flags |= DBAL_ENTRY_ATTR_FLAG_HIT_GET;
        hitbit_flags |= (_SHR_IS_FLAG_SET(info->l3a_flags, BCM_L3_HIT_CLEAR)) ? DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR : 0;
    }

    SHR_IF_ERR_EXIT_NO_MSG(dnx_l3_route_payload_get(unit, info, entry_handle_id, table_hb_enabled, hitbit_flags, 0      /* is_traverse 
                                                                                                                         */ ));
    ...
    bcm_l3_route_t * info)
{
    ...
    /** Verify payload */
    SHR_IF_ERR_EXIT(dnx_l3_route_payload_verify(unit, info, is_replace, dbal_table, &dest_type));
    ...
    bcm_l3_route_t * info)
{
    ...
    {
        if (DNX_L3_ENTRY_IS_FOR_KBP(unit, info))
        {
            SHR_IF_ERR_EXIT(dnx_l3_kbp_route_add(unit, info));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_l3_route_add(unit, info));
        }
    }
exit:
    DBAL_FUNC_FREE_VARS;
    ...
    bcm_l3_route_t * info)
{
    ...
    SHR_NULL_CHECK(info, _SHR_E_PARAM, "l3_route");
    {
        if (DNX_L3_ENTRY_IS_FOR_KBP(unit, info))
        {
            rv = dnx_l3_kbp_route_get(unit, info);
        }
        else
        {
            rv = dnx_l3_route_get(unit, info);
        }
    }
    ...
    bcm_l3_route_t * info)
{
    ...
    SHR_NULL_CHECK(info, _SHR_E_PARAM, "l3_route");
    {
        if (DNX_L3_ENTRY_IS_FOR_KBP(unit, info))
        {
            SHR_IF_ERR_EXIT(dnx_l3_kbp_route_delete(unit, info));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_l3_route_delete(unit, info));
        }
    }
exit:
    DBAL_FUNC_FREE_VARS;
 ...
 * \param [in] unit - The unit number.
 * \param [in] info - The l3 route structure to be verified.
 * \param [in] is_kbp_table - Indication that the entry is for KBP.
 * \return
 *   \retval Zero if no error was encountered
 *   \retval Non-zero in case of an error.
 * \see
    ...
    int is_kbp_table)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "The EM tables are not used in FWD!\n");
    }
    ...
    /** Verify supported flags */
    if (_SHR_IS_FLAG_SET(info->l3a_flags, ~L3_SUPPORTED_BCM_L3_FLAGS_HOST))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Illegal L3 flags are passed to bcm_l3_host_* API.");
    }

    /** Verify supported flags2 */
    if (_SHR_IS_FLAG_SET(info->l3a_flags2, ~L3_FORWARD_SUPPORTED_BCM_L3_FLAGS2_HOST))
    {
    ...
    if (info->l3a_vrf == 0)
    {
        ...
        if ((kaps_public_capacity > 0) && !is_kbp_table)
        {
    ...
    int is_kbp_table)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_l3_host_verify(unit, info, is_kbp_table));
    ...
    if (!is_kbp_table)
    {
        ...
        if (_SHR_IS_FLAG_SET(info->l3a_flags, BCM_L3_IP6))
        {
            if (routed_learn_flags & (BCM_SWITCH_CONTROL_L3_LEARN_IPV6_UC | BCM_SWITCH_CONTROL_L3_LEARN_IPV6_MC))
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             "Couldn't add lem host v6 table entry when routing learning v6 is enabled.");
            }
        }
        else
        {
            if (routed_learn_flags & (BCM_SWITCH_CONTROL_L3_LEARN_IPV4_UC | BCM_SWITCH_CONTROL_L3_LEARN_IPV4_MC))
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             "Couldn't add lem host v4 table entry when routing learning v4 is enabled.");
            }
        }
    }
    ...
    uint32 flags)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    if (flags != 0 && _SHR_IS_FLAG_SET(flags, ~L3_SUPPORTED_BCM_L3_FLAGS_HOST))
    {
...
{
    SHR_FUNC_INIT_VARS(unit);
    if (flags != 0 && _SHR_IS_FLAG_SET(flags, ~L3_SUPPORTED_BCM_L3_FLAGS_ROUTE_TRAVERSE))
    {
    ...
    bcm_l3_host_t * info)
{
                  ...
                  || dbal_table_id == DBAL_TABLE_KBP_IPV6_UNICAST_PRIVATE_LPM_FORWARD)))
    {
        ...
        else
        {
            ...
            info->l3a_flags |= BCM_L3_ENCAP_SPACE_OPTIMIZED;
            SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                            (unit, DBAL_FIELD_EEI_MPLS_MULTI_SWAP, DBAL_FIELD_MPLS_LABEL, &mpls_label, &field_value));
    ...
    bcm_l3_host_t * info)
{
    uint32 entry_handle_id;
    uint32 l3_intf;
    uint32 encap_id, eei_val = 0, dest_val;
    uint32 result_type;
    ...
    /** Check whether the EM tables are used in forwarding.*/
    if (!dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_host_entry_support))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "The EM tables are not used in FWD!\n");
    }
    ...
    /** Verify the inputs.*/
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_host_add_verify(unit, info, dbal_table, system_headers_mode, is_kbp_table));
    ...
    else
    {
        ...
        else
        {
            ...
            {
                /** Below result types have identical values for IPv4 and IPv6 */
            ...
            if (frwd_with_eei)
            {
                    ...
                    BCM_FORWARD_ENCAP_ID_EEI_USAGE_GET(info->encap_id) == BCM_FORWARD_ENCAP_ID_EEI_USAGE_MPLS_PORT)
                {
                    ...
                    if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO2_MODE)
                    {
                        ...
                        uint32 remark_profile = DBAL_ENUM_FVAL_SWAP_QOS_PROFILE_DEFAULT;
                        ...
                        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                                        (unit, DBAL_FIELD_EEI, DBAL_FIELD_EEI_MPLS_MULTI_SWAP, &eei_val, &eei_val));
                    }
                    else
                    {
                        SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                        ...
                        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                                        (unit, DBAL_FIELD_EEI, DBAL_FIELD_EEI_MPLS_PUSH_COMMAND, &eei_val, &eei_val));
                    }
                }
    ...
    bcm_l3_host_t * info)
{
    ...
    uint32 dbal_table;
    uint32 hit_bit = 0;
    uint32 attr_type = DBAL_ENTRY_ATTR_FLAG_HIT_GET;
    int routed_learn_flags = 0;
    int is_kbp_table = FALSE;
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_host_verify(unit, info, is_kbp_table));
    ...
    bcm_l3_host_t * ip_addr)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_host_verify(unit, ip_addr, is_kbp_table));
    ...
    uint8 *is_valid)
{
    ...
    else
    {
        ...
        if (L3_TABLE_IS_KBP_RPF(dbal_table) && !kbp_split_fwd_rpf)
        {
            SHR_EXIT();
        }
    }
...
bcm_dnx_l3_route_delete_all(
    ...
    bcm_l3_route_t * info)
{
    const dnx_data_l3_fwd_routing_tables_t *dbal_tables_to_delete[] = { NULL, NULL };
    uint32 iter_idx;
    uint32 nof_iterations = 0;
    uint32 table_idx;
    uint8 is_valid;
    ...
    if (info != NULL)
    {
        if (_SHR_IS_FLAG_SET(info->l3a_flags, BCM_L3_IP6))
        {
            dbal_tables_to_delete[nof_iterations++] =
                dnx_data_l3.fwd.routing_tables_get(unit, L3_IPV6_PROTOCOL_VERSION);
        }
        else
        {
            dbal_tables_to_delete[nof_iterations++] =
                dnx_data_l3.fwd.routing_tables_get(unit, L3_IPV4_PROTOCOL_VERSION);
        }
    }
    else
    {
        dbal_tables_to_delete[nof_iterations++] = dnx_data_l3.fwd.routing_tables_get(unit, L3_IPV4_PROTOCOL_VERSION);
        dbal_tables_to_delete[nof_iterations++] = dnx_data_l3.fwd.routing_tables_get(unit, L3_IPV6_PROTOCOL_VERSION);
    }

    /*
     * If the pointer to bcm_l3_route_t structure is empty, all route entries are going to be cleared.
     * Otherwise, relevant tables will be cleared based on the BCM_L3_IP6 flag -
     * if it is set, then:
     *  - public and private IPv6 tables will be cleared (for publis support case)
     *  - IPv6 FWD and RPF tables will be cleared (for fwd_rpf seperated)
     * if it is not set, then:
     *  - public and private IPv4 tables will be cleared (for public support case)
     *  - IPv4 FWD and RPF tables will be cleared (for fwd_repf seperated)
     */
    /** Clear tables */
    for (iter_idx = 0; iter_idx < nof_iterations; iter_idx++)
    {
        if (dbal_tables_to_delete[iter_idx] != NULL)
        {
            table_idx = 0;
            while ((dbal_tables_to_delete[iter_idx])->tables[table_idx] != DBAL_TABLE_EMPTY)
            {
                is_valid = FALSE;
                ...
                {
                    /*
                     * The non-KBP public and private routing tables are always valid.
                     * Check is required for KBP tables.
                     */
                    is_valid = TRUE;
                    if (L3_TABLE_IS_KBP((dbal_tables_to_delete[iter_idx])->tables[table_idx]))
                    {
                        SHR_IF_ERR_EXIT(dnx_l3_route_kbp_table_is_valid
                                        (unit, info, (dbal_tables_to_delete[iter_idx])->tables[table_idx], &is_valid));
                    }
                }

                if (is_valid)
                {
                    SHR_IF_ERR_EXIT(dbal_table_clear(unit, (dbal_tables_to_delete[iter_idx])->tables[table_idx]));
                }
    ...
    bcm_l3_host_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_switch_control_routed_learn_get(unit, &routed_learn_flags));
    if (routed_learn_flags && !(use_kbp_ipv4 || use_kbp_ipv6))
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
    ...
    void *user_data)
{
    ...
    /*
     * Allocate size of array for IPv6 address
     */
    SHR_ALLOC(field_value_eight_bits, sizeof(bcm_ip6_t),
              "Array for reading table entry key or value", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    SHR_INVOKE_VERIFY_DNXC(dnx_l3_host_traverse_verify(unit, flags));
    /*
    ...
    while (!is_end)
    {
        ...
        dbal_entry.l3a_vrf = field_value;
        /*
        ...
        else
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                            (unit, entry_handle_id, DBAL_FIELD_IPV4, &field_value));
            dbal_entry.l3a_ip_addr = field_value;
        }
...
exit:
    SHR_FREE(field_value_eight_bits);
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
...
{
    uint32 entry_handle_id;
    uint32 kbp_mngr_status = 0;
    uint32 dbal_tables[MAX_NOF_L3_ROUTE_TABLES] = { 0 };
    uint32 nof_tables;
    uint32 table;
    uint32 index;
    uint32 field_value;
    ...
    uint8 separate_fwd_rpf_dbs;
    uint8 is_l3_table_public = FALSE;
    uint8 is_kbp_split_rpf_in_use = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    DBAL_FUNC_INIT_VARS(unit);

    nof_tables = 0;
    separate_fwd_rpf_dbs =
        dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_separate_fwd_rpf_dbs) ? TRUE : FALSE;

    SHR_INVOKE_VERIFY_DNXC(dnx_l3_route_traverse_verify(unit, flags));
    ...
    /*
     * Determine the correct table from which all route entries will be traversed based on the BCM_L3_IP6 flag.
     * If the BCM_L3_IP6 flag is not set, then IPv4 route table will be traversed.
     */
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_IP6))
    {
        if (separate_fwd_rpf_dbs)
        {
        }
        else
        {
    ...
    for (table = 0; table < nof_tables; table++)
    {
        ...
        is_l3_table_public = L3_TABLE_IS_PUBLIC(dbal_tables[table]);
        ...
        while (!is_end)
        {
...

            {
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_l3_route_kbp_payload_get(unit, &dbal_entry, is_kbp_split_rpf_in_use, dbal_tables[table], entry_handle_id, 1     /* is_traverse 
                                                                                                                                                         */ ));
                }
                else
                {
                    SHR_IF_ERR_EXIT(dnx_l3_route_payload_get(unit, &dbal_entry, entry_handle_id, table_hb_enabled, hitbit_flags, 1      /* is_traverse 
                                                                                                                                         */ ));
                }
            }
