    ...
    int unit)
{
    ...
    uint8 is_image_standard = FALSE;
    uint8 is_image_custom = FALSE;
    uint8 update_sa_proc_v4 = 0;
    uint8 update_sa_proc_v6 = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_pp_prgm_is_custom_standard_image(unit, &is_image_custom));
    SHR_IF_ERR_EXIT(dnx_pp_prgm_default_image_check(unit, &is_image_standard));
    if (is_image_standard)
    {
        /*
        ...
        /*
         * Going over all LPM profiles
         */
        for (lpm_ind = 0; lpm_ind < dnx_data_l3.fwd.nof_lpm_strength_profiles_get(unit); lpm_ind++)
        {
            /*
             * Going over all the KAPS interfaces
             */
            for (intf_ind = DBAL_ENUM_FVAL_KAPS_INTERFACE_INTERFACE_0;
                 intf_ind <= dnx_data_l3.fwd.last_kaps_intf_id_get(unit); intf_ind++)
            {
                range_ind = 0;
                start_range_def =
                    dnx_data_l3.fwd.lpm_profile_to_entry_strength_get(unit, lpm_ind,
                                                                      intf_ind)->prefix_len_def[range_ind];
                start_range_non_def =
                    dnx_data_l3.fwd.lpm_profile_to_entry_strength_get(unit, lpm_ind,
                                                                      intf_ind)->prefix_len_non_def[range_ind];
                end_range_def =
                    dnx_data_l3.fwd.lpm_profile_to_entry_strength_get(unit, lpm_ind,
                ...
                while (end_range_def != -1 || end_range_non_def != -1)
                {
                    /** Set KAPS interface */
                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_KAPS_INTERFACE, intf_ind);

                    /** Set LPM strength profile */
                    if (dnx_data_l3.fwd.lpm_strength_profiles_support_get(unit))
                    {
                        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LPM_PROFILE, lpm_ind);
                    }

                    /**
                     * Set the entry strength only if the prefix length range is a valid range
                     */
                    if (end_range_def != -1)
                    {
                    ...
                    if (end_range_non_def != -1)
                    {
                        ...
                        strength_non_def =
                            dnx_data_l3.fwd.lpm_profile_to_entry_strength_get(unit, lpm_ind,
                                                                              intf_ind)->entry_strength_non_def
                            [range_ind];
        ...
        /*
         * Special handling for l3_ipv6_mc_tcam_not_used
         */
        if (dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_l3_ipv6_mc_tcam_not_used))
        {
            lpm_ind = 0;
            intf_ind = DBAL_ENUM_FVAL_KAPS_INTERFACE_INTERFACE_1;
            range_ind = 5;
            start_range_non_def =
                dnx_data_l3.fwd.lpm_profile_to_entry_strength_get(unit, lpm_ind,
                                                                  intf_ind)->prefix_len_non_def[range_ind] + 1;
            ...
            dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_PREFIX_LENGTH,
                                             start_range_non_def, end_range_non_def);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STRENGTH_NON_DEFAULT,
                                         INST_SINGLE, strength_non_def + 1);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }
    ...
    if (dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_default_kaps_interface))
    {
    ...
    bcm_if_t * if_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_egress_create_verify(unit, flags, egr, if_id));
    ...
    bcm_if_t intf)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_egress_destroy_verify(unit, intf));
    ...
    bcm_l3_egress_t * egr)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_egress_get_verify(unit, intf, egr));
    /*
    ...
    void *user_data)
{
     ...
     */
    for (table = 0; table < dnx_data_mdb.direct.nof_fec_dbs_get(unit); table++)
    {
        /*
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, dnx_data_l3.fec.fec_tables_info_get(unit, info->physical_table[table])->dbal_table,
                         &entry_handle_id));
 ...
 */
static shr_error_e
dnx_l3_egress_arp_bcm_api_traverse_arp(
    int unit,
    bcm_l3_egress_traverse_cb trav_fn,
    void *user_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_egress_arp_traverse_verify(unit, trav_fn, user_data));
    ...
    void *user_data)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_arp_flow_api_traverse(unit, trav_fn, user_data));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_arp_bcm_api_traverse_arp(unit, trav_fn, user_data));
    }
