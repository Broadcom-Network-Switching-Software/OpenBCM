    ...
    bcm_if_t * intf_array)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "ECMP VIP doesn't supported on this device.\n");
    }
    ...
    if (fec_range_min == 0)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "The VIP ECMP FEC-ID range minimum must be set with bcmSwitchVIPL3EgressIdMin.\n");
    }
 ...
 */
shr_error_e
dnx_l3_egress_ecmp_vip_is_vip_id_get(
    int unit,
    bcm_l3_egress_ecmp_t * ecmp,
    int *is_vip_id)
{
     ...
     * Check that the provided intf is in range
     */
    if ((fec_range_min == 0) || (BCM_L3_ITF_VAL_GET(ecmp->ecmp_intf) < fec_range_min)
        || (BCM_L3_ITF_VAL_GET(ecmp->ecmp_intf) > fec_range_max))
 ...
 */
shr_error_e
dnx_l3_egress_ecmp_vip_group_is_allocated_verify(
    int unit,
    bcm_l3_egress_ecmp_t * ecmp)
{
    ...
    bcm_l3_egress_ecmp_t * ecmp)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_vip_group_is_allocated_verify(unit, ecmp));
    SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_vip_fec_id_min_get(unit, &fec_range_min));
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.vip_ecmp_alloc.vip_ecmp_res_manager.free_single(unit, vip_id, NULL));
    ...
    bcm_l3_egress_ecmp_resilient_entry_t * entry)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_ECMP_RESILIENT_PCC_EXTERNAL)
        && !(dnx_kbp_device_enabled(unit) && dnx_data_elk.application.feature_get(unit, dnx_data_elk_application_sllb)))
    {
        SHR_ERR_EXIT(_SHR_E_RESOURCE, "KBP_PCC_STATE_TABLE is not enabled.\n");
    }
    ecmp_intf = BCM_L3_ITF_VAL_GET(entry->ecmp.ecmp_intf);
    SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_vip_fec_id_min_get(unit, &fec_range_min));
    SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_vip_fec_id_max_get(unit, &fec_range_max));
    if ((ecmp_intf > fec_range_max) || (ecmp_intf < fec_range_min))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Resilient ECMP ID %d is out of range, allowed range is %d-%d \n", ecmp_intf,
                     fec_range_min, fec_range_max);
    }
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_ECMP_RESILIENT_PCC))
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                     DBAL_RESULT_TYPE_PCC_STATE_FWD_MACT_RESULT_DEST__XL_FGID);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENTRY_GROUPING, INST_SINGLE,
                                     LEM_PCC_STATE_ENTRY_GROUPING);
        /**
         * Set the entry as dynamic  
         * Entry_strength_encode(2) is 1MSB entry_strength(1), AGE_PROFILE(1),
         * The related configuration are 1MSB MAC_STRENGTH(1) and MACT_ENTRY_AGE_PROFILE(1).
         */
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, INST_SINGLE, 0);
    ...
    dbal_fields_e dbal_field_vip_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                    (unit, entry_handle_id, DBAL_FIELD_SIGNATURE, &entry_signature));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE, &entry_destination));
    ...
    bcm_l3_egress_ecmp_resilient_entry_t * replace_entry)
{
    ...
    /*
     * Iterate through the table looking for matching entries.
     *
     * For DBAL_TABLE_PCC_STATE in LEM may use the flush machine to provide flexible matching and accelerate table iteration 
     */
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_ECMP_RESILIENT_PCC))
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PCC_STATE, &entry_handle_id));
    ...
    while (!is_end)
    {
        rv = dnx_l3_egress_ecmp_pcc_resilient_match_check(unit, flags, match_vip_id, match_signature, match_destination,
                                                          entry_handle_id, dbal_field_vip_id);
        if (rv == _SHR_E_EXISTS)
        {
            match_count++;
            /*
    ...
    void *user_data)
{
    ...
    while (!is_end)
    {
        ...
        if (rv == _SHR_E_EXISTS)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                            (unit, entry_handle_id, DBAL_FIELD_SIGNATURE, &signature));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
 ...
 */
static shr_error_e
dnx_switch_control_sllb_table_size_verify(
    int unit,
    int table_size)
{
    int nof_vip_ecmp;
 ...
 */
static shr_error_e
dnx_switch_control_vip_l3_egress_id_min_verify(
    int unit,
    int nof_vip_ecmp,
    int vip_ecmp_fec_id_min,
    int vip_ecmp_fec_id_max)
{
    int nof_free_vip_ecmp;
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The VIP ECMP range (min/max) exceeds the valid FEC range, 1 to %d.\n",
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The VIP ECMP range (min/max) cannot overlap the ECMP FEC range, 1 to %d.\n",
    ...
    int arg)
{
    ...
    SHR_IF_ERR_EXIT(dnx_switch_control_vip_l3_egress_id_min_verify
                    (unit, nof_vip_ecmp, vip_ecmp_fec_id_min, vip_ecmp_fec_id_max));
    ...
    {
        SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fec_db.get(unit, direction, hierarchy_iter, &fec_db));
        ...
        for (fec_id = vip_ecmp_fec_id_min; fec_id <= vip_ecmp_fec_id_max; fec_id++)
        {
            if ((fec_id >= range_start) && (fec_id < (range_start + range_size)))
            {
                SHR_IF_ERR_EXIT(dnx_algo_l3_fec_allocate
                                (unit, (int *) &fec_id, DNX_ALGO_L3_FEC_WITH_ID, fec_db, &fec_alloc_data));
            }
        }
    }
