...
#include <src/bcm/dnx/flow/porting/l3_arp_flow.h>
...
/**
* \brief
*  Exchange mac source address index in SW.
...
*/
shr_error_e
dnx_l3_egress_source_address_exchange(
    int unit,
    ...
    uint8 *is_last_mymac_reference)
{
    ...
    /** Check if the MAC address is all zeros, in this case a default profile is used*/
    if (BCM_MAC_IS_ZERO(src_mac_addr))
    {
    ...
    sal_memcpy(source_address_entry.address.mac_address, src_mac_addr, sizeof(bcm_mac_t));
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.source_address_table.exchange
                    (unit, 0, 1, &source_address_entry, old_mymac_prefix, NULL, new_mymac_prefix,
...
/**
 * \brief
 *   Clear the source address index to source_address table given the source index
 * \param [in] unit -
 *      The unit number.
 * \param [in] source_mac_profile -
 *      Source MAC index to be saved to Index to the table (retrieve from the source_address template manager)
 * \return
 ...
 */
shr_error_e
dnx_l3_egress_source_address_clear(
    int unit,
    int source_mac_profile)
{
    ...
    source_address_index = L3_SOURCE_MAC_ETH_PROFILE(source_mac_profile);
...
*  \param [in] egr_prev - previous egr info for the Replcae case
*  \return
    ...
    bcm_l3_egress_t * egr_prev)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_REPLACE))
    {
        ...
        /*
         * ARP+AC selection to a result type with stats can't change due to a smaller VLAN-Edit-Profile size
         */
        if (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION)
            && _SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_EGRESS_STAT_ENABLE)
            && !(_SHR_IS_FLAG_SET(egr_prev->flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION)
                 && _SHR_IS_FLAG_SET(egr_prev->flags2, BCM_L3_FLAGS2_EGRESS_STAT_ENABLE)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Can't replace any ARP entry to an ARP+AC with stats");
        }

        if ((_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_SRC_MAC)
             && !_SHR_IS_FLAG_SET(egr_prev->flags2, BCM_L3_FLAGS2_SRC_MAC))
            || (!_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_SRC_MAC)
                && _SHR_IS_FLAG_SET(egr_prev->flags2, BCM_L3_FLAGS2_SRC_MAC)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Switching from an ARP with BCM_L3_FLAGS2_SRC_MAC indication to an ARP without BCM_L3_FLAGS2_SRC_MAC indication or vice versa is not allowed\r\n");
        }
        /** SVTAG flag can't be replaced */
        if (_SHR_IS_FLAG_SET((egr_prev->flags3 ^ egr->flags3), BCM_L3_FLAGS3_EGRESS_SVTAG_PORT_ENABLE))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "SVTAG flag can't be replaced");
        }

    }

    if (_SHR_IS_FLAG_SET(flags, BCM_L3_WITH_ID) && _SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_VIRTUAL_EGRESS_POINTED))
    {
        bcm_gport_t gport;

        BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(gport, egr->encap_id);
        if (BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_GET(gport) >=
            dnx_data_l3.egr_pointed.nof_egr_pointed_ids_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Egress pointed encap_id(0x%x) is out of range (0 - %d)", egr->encap_id,
                         dnx_data_l3.egr_pointed.nof_egr_pointed_ids_get(unit));
        }
    }

    if ((egr->vlan) > dnx_data_l3.rif.nof_rifs_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "vlan value is out of range (0 - %d)", dnx_data_l3.rif.nof_rifs_get(unit));
    }
    /*
    ...
    if (egr->qos_map_id != 0)
    {
        if (!DNX_QOS_MAP_IS_EGRESS(egr->qos_map_id) || !DNX_QOS_MAP_IS_REMARK(egr->qos_map_id))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "qos_map_id is not egress remark map id");
        }
        if (DNX_QOS_MAP_PROFILE_GET(egr->qos_map_id) >= dnx_data_qos.qos.nof_egress_profiles_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "remark profile %d is too large", DNX_QOS_MAP_PROFILE_GET(egr->qos_map_id));
        }
    }
    if (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION))
    {
        if (egr->mpls_pkt_pri > dnx_data_qos.qos.packet_max_priority_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "egr->mpls_pkt_pri %d is too large", egr->mpls_pkt_pri);
        }
        if (egr->mpls_pkt_cfi > dnx_data_qos.qos.packet_max_cfi_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "egr->mpls_pkt_cfi %d is too large", egr->mpls_pkt_cfi);
        }
    }
    else
    {
        if ((egr->mpls_pkt_pri != 0) || (egr->mpls_pkt_cfi != 0))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "egr->mpls_pkt_pri/cfi should be 0");
        }
    }
    ...
    /*
     * encap_id with BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN | BCM_L3_REPLACE flags signalizes only tunnel next pointer should be updated
     */
    if ((_SHR_IS_FLAG_SET(flags, BCM_L3_KEEP_DSTMAC) || _SHR_IS_FLAG_SET(flags, BCM_L3_KEEP_VLAN))
        && (flags & (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN | BCM_L3_REPLACE)) !=
        (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN | BCM_L3_REPLACE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Flags BCM_L3_KEEP_DSTMAC, BCM_L3_KEEP_VLAN and BCM_L3_REPLACE must be used together.\r\n");
    }
    ...
    /*
     * Verify the supported encap access options
     */
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        if ((egr->encap_access != bcmEncapAccessAc) && (egr->encap_access != bcmEncapAccessInvalid)
            && (egr->encap_access != bcmEncapAccessRif))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Not supported encap_access option!.\r\n");
        }
    }
    else
    {
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        if ((egr->encap_access == bcmEncapAccessRif) && (egr->vlan == 0))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "egr->vlan has to be set for Rif encap access in order to get VSI from ARP.\n");
        }
        ...
        {
            /*
             * Verify encap_access is valid when used with the flags BCM_L3_NATIVE_ENCAP or BCM_L3_FLAGS2_VLAN_TRANSLATION
             */
            if ((egr->encap_access != bcmEncapAccessRif) && (egr->encap_access != bcmEncapAccessInvalid))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Encap access should be bcmEncapAccessRif or bcmEncapAccessInvalid when the flags BCM_L3_NATIVE_ENCAP and BCM_L3_FLAGS2_VLAN_TRANSLATION are set.\r\n");
            }
        }
        else if (_SHR_IS_FLAG_SET(egr->flags, BCM_L3_NATIVE_ENCAP) && (egr->vlan != 0)
                 && (egr->encap_access != bcmEncapAccessInvalid))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Encap access cannot be set when vlan != 0 and BCM_L3_NATIVE_ENCAP flag is set.\n");
        }
    }

    /*
     * Verify bcmEncapAccessAc is not used with the flags BCM_L3_NATIVE_ENCAP or BCM_L3_FLAGS2_VLAN_TRANSLATION
     */
    if ((egr->encap_access == bcmEncapAccessAc)
        && (_SHR_IS_FLAG_SET(egr->flags, BCM_L3_NATIVE_ENCAP)
            || (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION))))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "bcmEncapAccessAc should not be used with BCM_L3_NATIVE_ENCAP or BCM_L3_FLAGS2_VLAN_TRANSLATION flags.\r\n");
    }

    /*
     * Verify the supported encap access options and vlan_port_id
     */
    if (egr->encap_access == bcmEncapAccessAc && egr->vlan_port_id != BCM_GPORT_INVALID)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "bcmEncapAccessAc and vlan_port_id should not be configured at the same time.\r\n");
    }

    if (_SHR_IS_FLAG_SET(egr->flags3, BCM_L3_FLAGS3_EGRESS_SVTAG_PORT_ENABLE))
    {
        /*
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "This device doesn't support SVTAG (remove the BCM_L3_FLAGS3_EGRESS_SVTAG_PORT_ENABLE flag).");
        }
        if (!(_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "SVTAG can only be added for ARP + AC entries.\r\n");
        }
    }
...
{
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_NATIVE_ENCAP))
    {
        return DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_2;
    }
    else
    {
        if (_SHR_IS_FLAG_SET(flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION) || (encap_access == bcmEncapAccessAc))
        {
            return DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_8;
        }
        return DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_7;
    }
}
        ...
        if (!this_is_egr_pointed_case)
        {
            ...
            *egr_pointed_id = DNX_ALGO_EGR_POINTED_INVALID;
            ...
            SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_allocate
                            (unit, lif_flags, DNX_ALGO_LIF_EGRESS, NULL, ll_global_lif));
        }
        else
        {
            /*
    ...
    int qos_index)
{
    ...
    outlif_info.logical_phase = dnx_l3_egress_arp_outlif_phase_get(unit, egr->flags, egr->flags2, 0     /* not relevant 
                                                                                                         */ , egr->encap_access);
    ...
    if (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION))
    {
        ...
        /*
         * Fill ESEM command field in case of ARP plus AC
         * The ESEM command's value is constructed as follows
         * we add 1 to the prefix value if SVTAG is needed else we do not add to the prefix
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE,
                                     dnx_data_esem.access_cmd.esem_cmd_arr_prefix_arp_plus_ac_get(unit)
                                     + _SHR_IS_FLAG_SET(egr->flags3, BCM_L3_FLAGS3_EGRESS_SVTAG_PORT_ENABLE));
    }
    if (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_SRC_MAC))
    {
    ...
    int *nwk_qos_idx)
{
    ...
    {
        *src_mac_prefix = SOURCE_ADDRESS_TABLE_DEFAULT_PROFILE;
    }
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
...
/* see dnx_l3_egress_arp_info_get description */
shr_error_e
dnx_l3_egress_arp_info_bcm_api_get(
    int unit,
    bcm_if_t intf,
    bcm_l3_egress_t * egr)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Error, entry not found\n");
    }
    /*
    ...
    /*
     * Check for ESEM command flag
     */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, &is_field_valid, &esem_cmd));
    if (is_field_valid == TRUE)
    {
        uint8 svtag_present;
        int app_db_id;
    ...
    if (outlif_info.logical_phase == DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_2)
    {
        egr->flags |= BCM_L3_NATIVE_ENCAP;
    }

    /** set encap_access field based on lif phase*/
    if (outlif_info.logical_phase == DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_7)
    {
        egr->encap_access = bcmEncapAccessInvalid;
    }
    else if (outlif_info.logical_phase == DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_8)
    {
        /*
         * ARP+AC
         */
        if (!_SHR_IS_FLAG_SET(egr->flags, BCM_L3_NATIVE_ENCAP)
            && (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_VLAN_TRANSLATION)))
        {
            egr->encap_access = bcmEncapAccessInvalid;
        }
    ...
    bcm_l3_egress_t * egr)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_arp_info_flow_api_get(unit, intf, egr));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_arp_info_bcm_api_get(unit, intf, egr));
    }
...
/* see dnx_l3_egress_arp_info_delete description */
shr_error_e
dnx_l3_egress_arp_info_bcm_api_delete(
    int unit,
    bcm_if_t intf)
{
    int local_out_lif = 0;
    uint32 gport;
    int global_lif = 0;
    int src_mac_profile = SOURCE_ADDRESS_TABLE_DEFAULT_PROFILE;
    int nwk_qos_index = -1;
    uint8 last_ref = 0;
    ...
    SHR_IF_ERR_EXIT(dnx_l3_egress_src_mac_prefix_and_qos_index_get
                    (unit, local_out_lif, &src_mac_profile, &nwk_qos_index));
    if (src_mac_profile != SOURCE_ADDRESS_TABLE_DEFAULT_PROFILE)
    {
        int new_profile;
        uint8 first_ref;
        source_address_entry_t source_address_entry;
        sal_memset(&source_address_entry, 0, sizeof(source_address_entry));
        /** Exchange old src_mac_prefix profile with the default one from sw algo template.*/
        SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.source_address_table.exchange
                        (unit, 0, 1, &source_address_entry, src_mac_profile, NULL, &new_profile, &first_ref,
                         &last_ref));
    }
    ...
    if (last_ref)
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_source_address_clear(unit, src_mac_profile));
    }
    /**free qos resource*/
    ...
    bcm_if_t intf)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_arp_info_flow_api_delete(unit, intf));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_arp_info_bcm_api_delete(unit, intf));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int next_local_out_lif)
{
    ...
    while (is_last_tunnel == FALSE)
    {
        ...
        if (next_tunnel_lif != 0)
        {
            ...
            {
                lif_flags =
                    (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_POINTED(next_tunnel_gport)) ?
...
*/
shr_error_e
dnx_l3_egress_bcm_api_create_arp(
    int unit,
    uint32 flags,
    bcm_l3_egress_t * egr,
    bcm_l3_egress_t * old_egr)
{
    int ll_local_lif = 0;
    int ll_global_lif = 0;
    int src_mac_addr_profile = 0, old_src_mac_addr_profile = SOURCE_ADDRESS_TABLE_DEFAULT_PROFILE;
    ...
    if (!((flags & (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN)) == (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN)))
    {
        if (_SHR_IS_FLAG_SET(flags, BCM_L3_REPLACE))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_l3_egress_src_mac_prefix_and_qos_index_get
                            (unit, ll_local_lif, &old_src_mac_addr_profile, &old_qos_index));
        }

        /** Allocate src mac addr prefix when BCM_L3_FLAGS2_SRC_MAC is set */
        if (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_SRC_MAC))
        {
            SHR_IF_ERR_EXIT(dnx_l3_egress_source_address_exchange
                            (unit, egr->src_mac_addr, old_src_mac_addr_profile,
                             &src_mac_addr_profile, &is_first_src_mac_addr_reference, &is_last_src_mac_addr_reference));
        }
        ...
        if (is_first_src_mac_addr_reference)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_l3_intf_egress_mymac_set(unit, TRUE, egr->src_mac_addr, src_mac_addr_profile));
        }
        /*
        ...
        SHR_IF_ERR_EXIT(dnx_l3_egress_create_arp_hw_write
                        (unit, flags, egr, src_mac_addr_profile, (uint32 *) &ll_local_lif, ll_global_lif,
                         nwk_qos_index));
        /*
        ...
        {
            SHR_IF_ERR_EXIT(dnx_l3_egress_source_address_clear(unit, old_src_mac_addr_profile));
        }
    ...
    bcm_l3_egress_t * egr)
{
    ...
    /*
     * In case of Replace, Get the previous ARP data
     */
     ...
     * Verify egress (ARP)
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_l3_egress_create_arp_verify(unit, flags, egr, &old_egr));

    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_flow_api_create_arp(unit, flags, egr, &old_egr));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_l3_egress_bcm_api_create_arp(unit, flags, egr, &old_egr));
    }
exit:
    SHR_FUNC_EXIT;
}
