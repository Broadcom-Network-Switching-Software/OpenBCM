...
#define PHY_ADDRESS_CHANNEL_BITS                    (2)
...
#define PHY_ADDRESS_GET_MODULE(address)             (((address & PHY_ADDRESS_CHANNEL_MASK) >> PHY_ADDRESS_CHANNEL_OFFSET) / dnx_data_dram.general_info.nof_channels_get(unit))
#define PHY_ADDRESS_GET_CHANNEL(address)             (((address & PHY_ADDRESS_CHANNEL_MASK) >> PHY_ADDRESS_CHANNEL_OFFSET) % dnx_data_dram.general_info.nof_channels_get(unit))
...
#define PHY_ADDRESS_GET_COLUMN(address)             ((address & PHY_ADDRESS_COLUMN_MASK) >> PHY_ADDRESS_COLUMN_OFFSET)
    ...
    dnx_gddr6_bist_err_cnt_t * bist_err_cnt)
{
    ...
    LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "BIST failed for channel %d in dram %d:\n"
                                          "\tbist_read_data_cnt=0x%08x%08x\n"
                                          "\tbist_data_err_cnt=0x%08x\n"
                                          "\tbist_data_err_bitmap=%s\n"),
                               channel, dram_index, read_data_counter[1], read_data_counter[0],
                               bist_err_cnt->bist_data_err_cnt, data_err_bits_str_buffer));

    SHR_IF_ERR_EXIT(dnx_gddr6_dram_err_cnt_get(unit, dram_index, channel, dram_err_cnt));
    ...
    int unit)
{
    ...
    SHR_BIT_ITER(dram_bitmap, nof_drams, dram_iter)
    {
        for (int channel_iter = 0; channel_iter < nof_channels; ++channel_iter)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_gddr6_bist_result_report
                                (unit, dram_iter, channel_iter, &dram_err_cnt, &bist_status_cnt, &bist_err_cnt));
                is_bist_failed = 1;
            }
        }
    }
    ...
    uint32 *address)
{
    ...
    uint32 base_physical_address = 0;
    ...
    {
        base_physical_address |= dnx_gddr6_xor_bits(logical_address & atm[bit]) << bit;
        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit,
                                                "%s(): logical_address=0x%x, atm[%d]=0x%x, base_physical_address_bit_%d=%d\n"),
                                     FUNCTION_NAME(), logical_address, bit, atm[bit], bit,
                                     base_physical_address >> bit));
    }

    *address = base_physical_address | ((index & GET_BIT_MASK(index_in_offset_bits, 0)) << PHY_ADDRESS_COLUMN_OFFSET);
...
shr_error_e
dnx_gddr6_dynamic_calibration_save_and_disable(
    int unit,
    dnx_gddr6_per_channel_info_t * dynamic_calibration_enable)
{
    ...
    SHR_BIT_ITER(dram_bitmap, nof_drams, dram_index)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_gddr6_dynamic_calibration_is_enabled
                            (unit, dram_index, channel, &dynamic_calibration_enable->val[dram_index][channel]));
            SHR_IF_ERR_EXIT(dnx_gddr6_dynamic_calibration_disable(unit, dram_index, channel));
        }
    }
...
shr_error_e
dnx_gddr6_dynamic_calibration_restore(
    int unit,
    dnx_gddr6_per_channel_info_t * dynamic_calibration_enable)
{
    uint32 nof_drams;
    ...
    SHR_BIT_ITER(dram_bitmap, nof_drams, dram_index)
    {
        for (channel = 0; channel < dnx_data_dram.general_info.nof_channels_get(unit); ++channel)
        {
            if (dynamic_calibration_enable->val[dram_index][channel])
            {
                SHR_IF_ERR_EXIT(dnx_gddr6_dynamic_calibration_enable(unit, dram_index, channel, 1));
            }
        }
