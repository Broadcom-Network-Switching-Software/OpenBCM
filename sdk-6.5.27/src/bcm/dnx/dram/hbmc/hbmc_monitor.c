    ...
    else
    {
        SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_iter)
        {
            ...
            if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_apd_phy))
            {
                ...
                int hbm_firmware_operation_code = dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_hbm2e_support) ?
                    DNX_HBMC_HBM2E_FIRMWARE_OPERATION_READ_TEMPERATURE : DNX_HBMC_FIRMWARE_OPERATION_READ_DEVICE_TEMP;

                SHR_IF_ERR_EXIT(dnx_hbmc_firmware_launch_operation
                                (unit, hbm_iter, DNX_HBMC_FIRMWARE_ALL_CHANNELS, hbm_firmware_operation_code));
