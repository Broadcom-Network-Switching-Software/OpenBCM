    ...
    int app_db_id_1)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_key_app_db_id_hw_set(unit, field_stage, context_id, key_id_p->id[0], app_db_id_0));

    if (key_id_p->id[1] != DNX_FIELD_KEY_ID_INVALID)
    {

        SHR_IF_ERR_EXIT(dnx_field_key_app_db_id_hw_set(unit, field_stage, context_id, key_id_p->id[1], app_db_id_1));
    }
    ...
    dnx_field_qual_attach_info_t * qual_attach_info_p)
{
    ...
    {
        dnx_field_qual_t dnx_qual_state_table_cascading;
        const dnx_field_qual_map_t *qual_map_entry_p;
        shr_error_e rv;
        rv = dnx_field_map_qual_bcm_to_dnx_int(unit, DNX_FIELD_STAGE_IPMF2, FALSE, bcmFieldQualifyStateTableData,
                                               &dnx_qual_state_table_cascading, &qual_map_entry_p);
        if (rv == _SHR_E_NONE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "State table is not supported for cascading. "
                         "bcmFieldQualifyStateTableData can be used instead. Cadcading from fg_id %d.\r\n",
                         qual_attach_info_p->input_arg);
        }
        else if (rv == _SHR_E_NOT_FOUND)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "State table is not supported for cascading. Cadcading from fg_id %d.\r\n",
                         qual_attach_info_p->input_arg);
        }
        else
        {
    ...
    switch (action_length_type)
    {
        ...
        case DNX_FIELD_ACTION_LENGTH_TYPE_INVALID:
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Field group type \"%s\" (fg_id %d) does not have payload to cascade from.\r\n",
                         dnx_field_group_type_e_get_name(fg_type), qual_attach_info_p->input_arg);
        }
        default:
        {
    ...
    dbal_enum_value_field_pmf_lookup_type_e * pmf_lookup_type_p)
{
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_DIRECT_EXTRACTION:
        {
            ...
            *pmf_lookup_type_p = DBAL_ENUM_FVAL_PMF_LOOKUP_TYPE_INVALID;
            break;
        }
        default:
    ...
    uint8 new_state)
{
    ...
    if (pmf_lookup_type == DBAL_ENUM_FVAL_PMF_LOOKUP_TYPE_INVALID)
    {
        SHR_EXIT();
    }
    ...
    uint8 new_state)
{
         ...
         (key_id_p->id[key_idx] != DNX_FIELD_KEY_ID_INVALID); key_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_key_lookup_type_get
                        (unit, field_stage, key_id_p->id[key_idx], fg_type, &pmf_lookup_type));

        SHR_IF_ERR_EXIT(dnx_field_key_lookup_set(unit, field_stage, context_id, pmf_lookup_type, new_state));
    }
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    if (is_tcam && key_id_p->id[1] != DNX_FIELD_KEY_ID_INVALID)
    {
        app_db_id_2nd_key = DNX_FIELD_TCAM_ACCESS_PROFILE_PAIRED_ID_GET(key_in_info_p->app_db_id);
    }

    SHR_IF_ERR_EXIT(dnx_field_key_app_db_id_set
                    (unit, field_stage, context_id, key_id_p, key_in_info_p->app_db_id, app_db_id_2nd_key));
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_key_lookup_key_attach(unit, context_id, key_in_info_p, TRUE, key_id_p));
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_key_lookup_key_attach(unit, context_id, key_in_info_p, FALSE, key_id_p));
...
shr_error_e
dnx_field_key_epmf_key_b_kbr_init(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_context_t context_id,
    dbal_enum_value_field_field_key_e key_id)
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_key_app_db_id_set(unit, field_stage, context_id,
                                                ...
                                                DNX_FIELD_KEY_KBR_APP_DP_ID_DEFAULT));
    ...
    if (dnx_data_field.tests.epmf_debug_rely_on_tcam0_get(unit))
    {
        ...
        if (field_stage == DNX_FIELD_STAGE_EPMF
            && (key_id_p->id[0] == DBAL_ENUM_FVAL_FIELD_KEY_B || key_id_p->id[1] == DBAL_ENUM_FVAL_FIELD_KEY_B))
        {
            SHR_IF_ERR_EXIT(dnx_field_key_epmf_key_b_kbr_init
                            (unit, DNX_FIELD_STAGE_EPMF, context_id, DBAL_ENUM_FVAL_FIELD_KEY_B));
        }
    }
exit:
    SHR_FUNC_EXIT;
