    ...
    dbal_tables_e dbal_table_id)
{
    ...
    if ((core_mode == DBAL_CORE_MODE_DPC) && (entry_info_p->core == BCM_CORE_ALL) &&
        (dnx_data_field.external_tcam.multicore_fg_supports_core_all_get(unit) == FALSE))
    {
    ...
    uint32 entry_handle)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, 0);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    SHR_IF_ERR_EXIT(dbal_tables_indication_get(unit, dbal_table_id, DBAL_TABLE_IND_IS_HITBIT_EN, &is_hitbit_enable));
    if (!is_hitbit_enable)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "invalid hitibt for fg %d.\n", fg_id);
    }
            ...
            DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
            {
                ...
                for (bank_id = 0; bank_id < nof_banks; bank_id++)
                {
                    ...
                    if (dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_hitbit_splits_banks_like_sram))
                    {
                        ...
                        {
                            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Bank %d has size %d, not divisible by %d.\r\n",
                    ...
                    for (hitibt_unit_in_bank = 0; hitibt_unit_in_bank < hitibt_units_to_alloc; hitibt_unit_in_bank++)
                    {
                        SHR_IF_ERR_EXIT(dnx_field_tcam_hit_indication.tcam_action_hit_indication.
                                        bmp.bit_range_clear(unit, core_iter, hitbit_unit_index, 0, nof_bits_to_alloc));
                        hitbit_unit_index++;
                    }
                }
            }
