    ...
    uint32 *opcode_predefined_p)
{
    ...
    uint8 dynamic_rel_position;
    int nof_user_defiend_opcodes;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(kbp_mngr_opcode_is_dynamic(unit, opcode_id, &is_user_defined, &dynamic_rel_position));
    if (is_user_defined)
    {
        SHR_IF_ERR_EXIT(dnx_field_context_apptype_opcode_cascaded_get(unit, opcode_id, opcode_predefined_p));
    }
    else
    {
        (*opcode_predefined_p) = opcode_id;
    }
    SHR_IF_ERR_EXIT(dnx_field_context_apptype_opcode_to_child_opcodes
                    (unit, *opcode_predefined_p, &nof_user_defiend_opcodes, opcodes_in_group));
    opcodes_in_group[nof_user_defiend_opcodes] = (*opcode_predefined_p);
    (*nof_opcodes_in_group_p) = nof_user_defiend_opcodes + 1;
         ...
         qual_idx >= 0; qual_idx--, lookup_seg_idx++, lookup_seg_idx_without_packing++)
    {
        ...
        if (is_shared_overlay)
        {
            ...
            if ((lookup_info.nof_segments < lookup_seg_idx_without_packing) ||
                (lookup_info.key_segment_index[lookup_seg_idx_without_packing] != ((uint8) DNX_KBP_INVALID_SEGMENT_ID)))
            {
             ...
             * Sanity check
             */
            if (segment_shared_with == ((uint32) DNX_KBP_INVALID_SEGMENT_ID))
            {
    ...
    int unit)
{
    ...
    dnx_field_kbp_opcode_info_t kbp_opcode_info;
    int nof_user_defiend_opcodes;
    uint32 user_opcodes_cascading[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_APPTYPE_USER_NOF];
    uint32 user_opcodes_used[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_APPTYPE_USER_NOF];
    int opcode_idx;
    ...
    for (predef_opcode = 0; predef_opcode < DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF; predef_opcode++)
    {
        uint8 opcode_is_valid;
        uint8 nof_contexts_per_opcode;
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.is_valid.get(unit, predef_opcode, &opcode_is_valid));
        if (opcode_is_valid == FALSE)
        {
            continue;
        }
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                        nof_fwd2_context_ids.get(unit, predef_opcode, &nof_contexts_per_opcode));
        if (nof_contexts_per_opcode <= 0)
        {
            continue;
        }
        ...
        SHR_IF_ERR_EXIT(dnx_field_context_apptype_opcode_to_child_opcodes
                        (unit, predef_opcode, &nof_user_defiend_opcodes, user_opcodes_cascading));
        /** Sanity check.*/
        if (nof_user_defiend_opcodes > DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_APPTYPE_USER_NOF)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "nof_user_defiend_opcodes %d, is bigger than maximum %d.\n",
                         nof_user_defiend_opcodes, DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_APPTYPE_USER_NOF);
        }
        nof_user_defiend_opcodes_used = 0;
        for (opcode_idx = 0; opcode_idx < nof_user_defiend_opcodes; opcode_idx++)
        {
            uint32 opcode_user = user_opcodes_cascading[opcode_idx];
            SHR_IF_ERR_EXIT(dnx_field_kbp_sw.opcode_info.is_valid.get(unit, opcode_user, &is_valid));
            if (is_valid)
            {
                user_opcodes_used[nof_user_defiend_opcodes_used] = opcode_user;
                nof_user_defiend_opcodes_used++;
            }
            else
            {
                LOG_DEBUG_EX(BSL_LOG_MODULE,
                             "User defined opcode %d was created but no field groups attached to it."
                             "%s%s%s\n", opcode_user, EMPTY, EMPTY, EMPTY);
            }
        }
        /*
        ...
        for (opcode_idx = 0; opcode_idx < nof_user_defiend_opcodes_used + 1; opcode_idx++)
        {
            ...
            {
                opcode_id_curr = user_opcodes_used[opcode_idx - 1];
            }
    ...
    uint32 *opcode_id_p)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_create(unit, base_static_opcode_id, opcode_name, &acl_context, &opcode_uint8));
    (*opcode_id_p) = opcode_uint8;

    SHR_IF_ERR_EXIT(kbp_mngr_opcode_is_dynamic(unit, *opcode_id_p, &is_dynamic, &opcode_index));
    /** Sanity check*/
    if (is_dynamic == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Created opcode %d is not dynamic. \r\n", *opcode_id_p);
    }
     ...
     * Mark the FFCs used.
     */
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_contexts_get(unit, *opcode_id_p, &fwd_nof_contexts, fwd_acl_ctx_mapping));
     ...
     */
    if (sal_strncmp(opcode_name, "", DBAL_MAX_STRING_LENGTH))
    {
        /** Set the name of the apptype to the SW state */
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.name.stringncpy(unit, opcode_index, opcode_name));
    }

    /** Set the rest of the information to SW state.*/
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.base_opcode.set(unit, opcode_index, base_static_opcode_id));
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.acl_context.set(unit, opcode_index, acl_context));
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.apptype.set(unit, opcode_index, user_defined_apptype));
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.profile_id.set(unit, opcode_index, cs_profile_id));
    ...
    uint8 *nof_fwd_segments_p)
{
    ...
    if (is_user_defined)
    {
        SHR_IF_ERR_EXIT(dnx_field_context_apptype_opcode_cascaded_get(unit, opcode_id, &opcode_predefined));
    }
    else
    {
