    ...
    if (fg_type == DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM)
    {
        ...
        if (device_locked)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_lookup_fwd_contexts
                            (unit, opcode_id, &nof_fwd_contexts, fwd_contexts));
            SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_acl_contexts
                            (unit, opcode_id, TRUE, &nof_acl_contexts, acl_contexts));
            if (nof_fwd_contexts <= 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Opcode %d has no FWD contexts (%d).\n", opcode_id, nof_fwd_contexts);
            }
            if (nof_acl_contexts <= 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Opcode %d has no ACL contexts (%d).\n", opcode_id, nof_acl_contexts);
            }
            switch (dnx_data_field.external_tcam.type_get(unit))
            {
                case DNX_FIELD_EXTERNAL_TCAM_TYPE_KBP:
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     ...
                     DBAL_TABLE_TYPE_TCAM_BY_ID, DBAL_ACCESS_METHOD_MDB, DBAL_CORE_MODE_DPC, 0, FALSE, TRUE,
