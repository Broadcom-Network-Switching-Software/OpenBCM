    ...
    switch (field_io)
    {
        ...
        default:
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Specified IO (%d \"%s\") for 'replace' must be of TCAM type only (for FEMs). Range is %d to %d (incl. edges)\r\n",
                         field_io, dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_FIELD_IO, field_io),
                         DBAL_ENUM_FVAL_FIELD_IO_INTERNAL_TCAM_0, DBAL_ENUM_FVAL_FIELD_IO_INTERNAL_TCAM_7);
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    dbal_enum_value_field_field_io_e actions_block)
{
    ...
    if (((int) actions_block < 0) || (actions_block >= DNX_DATA_MAX_FIELD_EFES_MAX_NOF_FIELD_IOS))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal value of field_io %d.\n", actions_block);
    }

    if (actions_block == DBAL_ENUM_FVAL_FIELD_IO_INVALID_INTERNAL_IO)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Invalid field_io.\n");
    }
    ...
    int *found_p)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_actions_fes_key_select_block_verify(unit, field_stage, actions_block));
    ...
    dnx_field_action_in_group_info_t actions_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_actions_place_actions_verify
    ...
    dnx_field_actions_fes_info_t fes_inst_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
         ...
         && actions_info[action_index].dnx_action != DNX_FIELD_ACTION_INVALID; action_index++)
    {
        ...
        if (action_size_without_valid >= SAL_UINT32_NOF_BITS)
        {
            fes_inst_info[inst_ndx].alloc_info.fes2msb_info[0].required_mask = 0;

            if (prefix != 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Action 0x%x has non zero prefix 0x%x, "
                             "but action size without valid bit is %d.\r\n",
                             actions_info[action_index].dnx_action, prefix, action_size_without_valid);
            }
        }
        else
        {
            fes_inst_info[inst_ndx].alloc_info.fes2msb_info[0].required_mask = prefix << action_size_without_valid;

            if (fes_inst_info[inst_ndx].alloc_info.fes2msb_info[0].required_mask >> action_size_without_valid != prefix)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Action 0x%x has prefix 0x%x, action size without valid bit is %d. "
                             "prefix does not fit.\r\n",
                             actions_info[action_index].dnx_action, prefix, action_size_without_valid);
            }
        }
    ...
    dnx_field_fes_mask_t * action_mask_p)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_value_get(unit, key_select_dbal_field_id, 0, &key_select_enum_uint32));
    ...
    (*key_select_p) = key_select_enum_uint32;
    ...
    dnx_field_actions_fes_info_t fes_inst_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
         ...
         fes_quartet_ndx++)
    {
        ...
        fes_inst_info[fes_quartet_ndx].write_info.key_select = DNX_FIELD_FES_KEY_SELECT_INVALID;
    }
    ...
    unsigned int nof_fes_quartets)
{
    ...
    SHR_ALLOC(action_found_p, sizeof(*action_found_p) * DNX_FIELD_ACTION_NOF, "action_found_p",
              "%s%s%s", EMPTY, EMPTY, EMPTY);

    for (action_type_ndx = 0; action_type_ndx < DNX_FIELD_ACTION_NOF; action_type_ndx++)
    {
        ...
        for (fes2msb_ndx = 0; fes2msb_ndx < nof_fes2msb; fes2msb_ndx++)
        {
                ...
                DNX_FIELD_ACTION_NOF)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Action type (%d) exceeds range for ACE actions (%d). "
                             ...
                             DNX_FIELD_ACTION_NOF - 1, ace_id, fes_quartet_ndx, fes2msb_ndx);
            }
    ...
    dnx_field_actions_fes_info_t fes_inst_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_actions_fes_ace_set_verify(unit, ace_id, fes_inst_info, fes_fg_in.nof_actions));
    ...
    dnx_field_actions_fes_id_get_info_t * fes_id_get_info_p)
{
    ...
    uint8 mask_id_iter;
    unsigned int fes2msb_id_ndx;
    dnx_field_fes_pgm_id_t fes_pgm_id_ndx;
    dnx_field_fes_pgm_id_t fes_pgm_id;
    dnx_field_fes_key_select_t key_select;
    dnx_algo_field_action_fes_quartet_sw_state_get_info_t fes_quartet_sw_state_info;
    dnx_field_actions_fes_common_info_fes2msb_t fes_inst_common_info_fes2msb;
    ...
    for (fes_pgm_id_ndx = 0; fes_pgm_id_ndx < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_PROGRAMS; fes_pgm_id_ndx++)
    {

        fes_id_get_info_p->fes_pgm_info[fes_pgm_id_ndx].field_group = DNX_FIELD_GROUP_INVALID;

        for (mask_id_iter = 0; mask_id_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_MASKS_PER_FES; mask_id_iter++)
        {
            fes_id_get_info_p->fes_pgm_info[fes_pgm_id_ndx].masks_id_used[mask_id_iter] =
                DNX_ALGO_FIELD_ZERO_FES_ACTION_MASK_ID;
        }
    }
...
shr_error_e
dnx_field_actions_fes_info_payload_info_get(
    int unit,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
    dnx_field_stage_e field_stage,
    dnx_field_actions_fg_payload_sw_info_t * actions_payload_info_sw_p,
    dnx_field_actions_fes_context_group_get_info_t * fes_group_info_p,
    dnx_field_action_attach_info_t attach_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
    ...
    SHR_NULL_CHECK(actions_payload_info_sw_p, _SHR_E_PARAM, "actions_payload_info_sw_p");
    SHR_NULL_CHECK(fes_group_info_p, _SHR_E_PARAM, "fes_group_info_p");
    ...
    SHR_IF_ERR_EXIT(dnx_field_actions_template_get(unit, actions_payload_info_sw_p, &actions_payload_info));
...
shr_error_e
dnx_field_actions_template_get(
    int unit,
    dnx_field_actions_fg_payload_sw_info_t * actions_payload_info_sw_p,
    dnx_field_actions_fg_payload_info_t * actions_payload_info_p)
{
    ...
    for (action_ndx = 0; action_ndx < DNX_DATA_MAX_FIELD_GROUP_NOF_ACTION_PER_FG; action_ndx++)
    {
        actions_payload_info_p->actions_on_payload_info[action_ndx] =
            actions_payload_info_sw_p->actions_on_payload_info[action_ndx];
    }
...
static shr_error_e
dnx_field_efes_action_add_allow_start_before_field_group(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_group_t fg_id,
    int *nof_bits_before_fg_p)
{
    SHR_FUNC_INIT_VARS(unit);

    (*nof_bits_before_fg_p) = 0;
    ...
    dnx_field_actions_fes_single_add_conf_t efes_encoded_extraction[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_PROG_PER_FES])
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "When adding an EFES, the priority must be a position, that is created by "
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot perform EFES add for external stage (field group ID %d).\n", fg_id);
    }
    ...
    SHR_IF_ERR_EXIT(dnx_field_efes_action_add_allow_start_before_field_group
                    (unit, field_stage, fg_id, &nof_bits_before_fg));
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Cannot acquire payload range on template. fg_payload_min_offset %d, fg_payload_max_offset %d. "
                     "fg_id %d, context_id, %d priority 0x%x.\r\n",
                     fg_payload_min_offset, fg_payload_max_offset, fg_id, context_id, action_priority);
    }

    if (fg_payload_max_offset < fg_payload_min_offset)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Payload range illogical. fg_payload_min_offset %d, fg_payload_max_offset %d. "
                     "fg_id %d, context_id, %d priority 0x%x.\r\n",
                     fg_payload_min_offset, fg_payload_max_offset, fg_id, context_id, action_priority);
    }
    ...
    for (efes_2msb_index = 0; efes_2msb_index < nof_progs_per_efes; efes_2msb_index++)
    {
        ...
        if (efes_encoded_extraction[efes_2msb_index].lsb < (fg_payload_min_offset - nof_bits_before_fg))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "input offset (%d) is smaller than the minimal bit of the field group %d "
                         "minus %d bits allowed before. "
                         "fg_id %d, context_id, %d priority 0x%x, condition %d.\r\n",
                         efes_encoded_extraction[efes_2msb_index].lsb, fg_payload_min_offset,
                         nof_bits_before_fg, fg_id, context_id, action_priority, efes_2msb_index);
        }
        if ((efes_encoded_extraction[efes_2msb_index].dont_use_valid_bit == FALSE) &&
            ((efes_encoded_extraction[efes_2msb_index].lsb - 1) < (fg_payload_min_offset - nof_bits_before_fg)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "input offset (%d) minus one for valid bit (%d) is smaller "
                         "than the minimal bit of the field group %dminus %d bits allowed before. "
                         "fg_id %d, context_id, %d priority 0x%x, condition %d.\r\n",
                         efes_encoded_extraction[efes_2msb_index].lsb,
                         efes_encoded_extraction[efes_2msb_index].lsb - 1,
                         fg_payload_min_offset, nof_bits_before_fg,
                         fg_id, context_id, action_priority, efes_2msb_index);
        }
        if ((efes_encoded_extraction[efes_2msb_index].dont_use_valid_bit == FALSE) &&
            ((efes_encoded_extraction[efes_2msb_index].lsb - 1) < fg_payload_min_offset))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         ...
                         "Valid bit is not allowed before the field group."
                         "fg_id %d, context_id, %d priority 0x%x, condition %d.\r\n",
                         efes_encoded_extraction[efes_2msb_index].lsb,
                         efes_encoded_extraction[efes_2msb_index].lsb - 1,
    ...
    dnx_field_actions_fes_single_add_conf_t efes_encoded_extraction[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_PROG_PER_FES])
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_efes_action_add_verify(unit, field_stage, fg_id, context_id, action_priority,
                                                            use_condition, efes_condition_msb,
    ...
    dnx_field_actions_fes_single_add_conf_t efes_encoded_extraction[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_PROG_PER_FES])
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_field_stage_get(unit, fg_id, &field_stage));
    SHR_INVOKE_VERIFY_DNXC(dnx_field_efes_action_info_get_verify(unit, field_stage, fg_id, context_id, action_priority,
    ...
    bcm_field_action_priority_t action_priority)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_efes_action_remove_verify(unit, field_stage, fg_id, context_id, action_priority));
    ...
    {
        {
            ...
            dnx_field_fem_program_t fem_program_max =
            ...
            dnx_field_fem_condition_t fem_condition_max =
                dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_fem_condition;
            int extraction_pointed_by_other_program[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FEM_MAP_INDEX] = { 0 };
            int fem_program_is_unallocated;

            dnx_field_fem_map_index_entry.fem_id = fem_id;
            for (fem_program_index = FIRST_AVAILABLE_FEM_PROGRAM; fem_program_index < fem_program_max;
                 fem_program_index++)
            {
                fem_program_is_unallocated = FALSE;
                SHR_IF_ERR_EXIT(FEM_INFO_FG_ID_INFO.fg_id.get(unit, fem_id, fem_program_index, &sw_fg_id));
                if (sw_fg_id == fg_id)
                {

                    if (fem_program != fem_program_index)
                    {
                        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                     "Field Group %d on fem_id %d has more than one program allocated (%d and %d).\r\n",
                                     fg_id, fem_id, fem_program, fem_program_index);
                    }
                    continue;
                }
                if (sw_fg_id == DNX_FIELD_GROUP_INVALID)
                {
                    ...
                    fem_program_is_unallocated = TRUE;
                }
                ...
                for (fem_condition_index = 0; fem_condition_index < fem_condition_max; fem_condition_index++)
                {
                    ...
                    if (dnx_field_fem_map_index_entry.fem_condition_entry.fem_action_valid)
                    {
                        ...
                        if (fem_program_is_unallocated)
                        {
                            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                         "fem_id %d program %d is not used by any field group "
                                         "but condition %d is valid.\r\n",
                                         fem_id, fem_program_index, fem_condition_index);
                        }
                        extraction_pointed_by_other_program[dnx_field_fem_map_index_entry.
                                                            fem_condition_entry.fem_map_index] = TRUE;;
                    }
                }
            }
            ...
            for (fem_map_index = 0; fem_map_index < fem_map_index_max; fem_map_index++)
            {
                if ((ignore_actions & SAL_BIT(fem_map_index)) == 0)
                {
                    ...
                    if (extraction_pointed_by_other_program[fem_map_index] == FALSE)
                    {
                        SHR_IF_ERR_EXIT(dnx_algo_field_action_pmf_a_fem_map_index_deallocate
                                        (unit, fem_id, fem_map_index));
                    }
                }
            }
        }
        ...
        SHR_IF_ERR_EXIT(dnx_algo_field_fem_priority_is_position_set(unit, fem_id, fem_program, 0));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    *ignore_actions_p = DNX_FIELD_IGNORE_ALL_ACTIONS;
    ...
    for (fem_condition_index = 0; fem_condition_index < fem_condition_max; fem_condition_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value
                        (unit, DBAL_FIELD_FIELD_FEM_ACTION_VALID,
                         fem_condition_entry[fem_condition_index].fem_action_valid, &is_enum_illegal));
        if (is_enum_illegal)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "fem_condition_entry.fem_action_valid (%d) is not a valid value.\r\n",
        ...
        {
            if ((unsigned int) (fem_condition_entry[fem_condition_index].fem_map_index) >=
                (unsigned int) (dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_fem_map_index))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "fem_condition_entry.fem_map_index (%d) is out of range (Should be between %d and %d)\r\n",
                             (fem_condition_entry[fem_condition_index].fem_map_index), 0,
                             (dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_fem_map_index - 1));
            }
        }
...
static shr_error_e
dnx_field_fem_action_find_action_types(
    int unit,
    dnx_field_group_t fg_id,
    ...
    dnx_field_fem_action_entry_t fem_action_entry[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FEM_MAP_INDEX])
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_fem_action_find_action_types(unit, fg_id, fem_encoded_actions, fem_action_entry));
    {
        ...
        if (DNX_FIELD_ACTION_PRIORITY_IS_POSITION(fem_priority))
        {

            DNX_FIELD_ACTION_PRIORITY_POSITION_FEM_ID_GET(fem_id, unit, fem_priority);
            priority_is_position = TRUE;
        }
        else
        {

            if (fem_priority != BCM_FIELD_ACTION_DONT_CARE)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "FEM priority must be position (created using BCM_FIELD_ACTION_POSITION()) or "
                             "BCM_FIELD_ACTION_DONT_CARE. Priority is 0x%x.\r\n", fem_priority);
            }
            priority_is_position = FALSE;

            SHR_IF_ERR_EXIT(dnx_algo_field_fem_position_allocate(unit, ignore_actions, fg_id, second_fg_id,
                                                                 fem_condition_entry, fem_encoded_actions,
                                                                 fem_action_entry, &fem_id));
        }
        {

            SHR_INVOKE_VERIFY_DNXC(dnx_algo_field_fem_priority_verify_contention_inside_group
                                   (unit, fg_id, ignore_actions, fem_action_entry, priority_is_position));
            SHR_INVOKE_VERIFY_DNXC(dnx_field_fem_action_add_verify
        ...
        }
        {
            ...
            else
            {
                ...
                int nof_bits_in_fem_action =
                    dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_bits_in_fem_action;
                int bit_index;
                fem_condition_ms_bit += template_shift;
                for (fem_map_index = 0; fem_map_index < fem_map_index_max; fem_map_index++)
                {
                    if (fem_encoded_actions[fem_map_index] == DNX_FIELD_ACTION_INVALID)
                    {
                    ...
                    for (bit_index = 0; bit_index < nof_bits_in_fem_action; bit_index++)
                    {
                        if (fem_action_entry[fem_map_index].fem_bit_info[bit_index].fem_bit_format ==
                            DNX_FIELD_FEM_BIT_FORMAT_FROM_KEY_SELECT)
                        {
                            fem_action_entry[fem_map_index].fem_bit_info[bit_index].fem_bit_value.bit_on_key_select +=
                                template_shift;
                        }
                    }
                }
        ...
        }
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_field_action_find_available_fem_program
                            (unit, TRUE, fem_id, &available_fem_program, &sum_allocated_actions));

            {

                SHR_IF_ERR_EXIT(dnx_algo_field_action_update_conditions
                                (unit, TRUE, fem_id, fem_condition_entry, fem_encoded_actions, fem_action_entry,
...

    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_field_fem_priority_is_position_set(unit, fem_id, fem_program, priority_is_position));
    }
...
shr_error_e
dnx_field_action_fem_is_fem_id_available(
    int unit,
    dnx_field_fem_id_t fem_id,
    dnx_field_group_t fg_id,
    dnx_field_group_t second_fg_id,
    ...
    int *fem_id_can_be_used_p)
{
    ...
    int fem_id_can_be_used = TRUE;

    SHR_FUNC_INIT_VARS(unit);

    if (fem_id < num_fems_with_short_action)
    {
        int bit_in_fem_action_small =
            dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_bits_in_fem_action_fems_0_1;
        int bit_in_fem_action_large =
            dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_bits_in_fem_action_fems_2_15;
        int bit_in_fem_action_index;
        for (fem_map_index = 0; fem_map_index < fem_map_index_max; fem_map_index++)
        {
            if ((ignore_actions_initial & SAL_BIT(fem_map_index)) != 0)
            {
                continue;
            }
            for (bit_in_fem_action_index = bit_in_fem_action_small;
                 bit_in_fem_action_index < bit_in_fem_action_large; bit_in_fem_action_index++)
            {
                if (fem_action_entry[fem_map_index].fem_bit_info[bit_in_fem_action_index].fem_bit_format !=
                    DNX_FIELD_FEM_BIT_FORMAT_INVALID)
                {
                    fem_id_can_be_used = FALSE;
                    break;
                }
            }
            if (fem_id_can_be_used == FALSE)
            {
                break;
            }
            if (fem_action_entry[fem_map_index].fem_adder != 0)
            {
                fem_id_can_be_used = FALSE;
                break;
            }
    ...
    if (fem_id_can_be_used)
    {
        ...
        dnx_field_fem_program_t available_fem_program;
        shr_error_e rv;
        dnx_field_fem_condition_entry_t fem_condition_entry_cpy[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FEM_CONDITION];
        dnx_field_action_t fem_encoded_actions_cpy[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FEM_MAP_INDEX];
        dnx_field_fem_action_entry_t fem_action_entry_cpy[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FEM_MAP_INDEX];

        sal_memcpy(fem_condition_entry_cpy, fem_condition_entry, sizeof(fem_condition_entry_cpy));
        sal_memcpy(fem_encoded_actions_cpy, fem_encoded_actions, sizeof(fem_encoded_actions_cpy));
        sal_memcpy(fem_action_entry_cpy, fem_action_entry, sizeof(fem_action_entry_cpy));
        ...
        rv = dnx_algo_field_action_find_available_fem_program
            (unit, FALSE, fem_id, &available_fem_program, &sum_allocated_actions);
        ...
        if (fem_id_can_be_used)
        {
            rv = dnx_algo_field_action_update_conditions
                (unit, FALSE, fem_id, fem_condition_entry_cpy, fem_encoded_actions_cpy, fem_action_entry_cpy,
                 sum_allocated_actions, fem_condition_entry_destination, &ignore_actions_initial,
                 &already_in_hw, fem_encoded_actions_destination, fem_action_entry_destination);
            if (rv == _SHR_E_RESOURCE)
            {
                fem_id_can_be_used = FALSE;
    ...
    (*fem_id_can_be_used_p) = fem_id_can_be_used;
...
shr_error_e
dnx_field_fem_action_entry_t_compare(
    int unit,
...
{
    int bit_index;
    dnx_field_fem_bit_format_e fem_bit_format_equivalent_1;
    dnx_field_fem_bit_value_t fem_bit_value_equivalent_1;
    dnx_field_fem_bit_format_e fem_bit_format_equivalent_2;
    dnx_field_fem_bit_value_t fem_bit_value_equivalent_2;

    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(fem_action_entry_1_p, _SHR_E_PARAM, "fem_action_entry_1_p");
    SHR_NULL_CHECK(fem_action_entry_2_p, _SHR_E_PARAM, "fem_action_entry_2_p");
    SHR_NULL_CHECK(is_equivalent_p, _SHR_E_PARAM, "is_equivalent_p");

    if (fem_action_entry_1_p->fem_action != fem_action_entry_2_p->fem_action)
    {
        (*is_equivalent_p) = FALSE;
        SHR_EXIT();
    }
    for (bit_index = 0;
         bit_index < (sizeof(fem_action_entry_1_p->fem_bit_info) / sizeof(fem_action_entry_1_p->fem_bit_info[0]));
         bit_index++)
    {
        if (fem_action_entry_1_p->fem_bit_info[bit_index].fem_bit_format == DNX_FIELD_FEM_BIT_FORMAT_INVALID)
        {
            fem_bit_format_equivalent_1 = DNX_FIELD_FEM_BIT_FORMAT_FROM_THIS_FIELD;
            fem_bit_value_equivalent_1.bit_value = 0;
        }
        else
        {
            fem_bit_format_equivalent_1 = fem_action_entry_1_p->fem_bit_info[bit_index].fem_bit_format;
            fem_bit_value_equivalent_1 = fem_action_entry_1_p->fem_bit_info[bit_index].fem_bit_value;
        }
        if (fem_action_entry_2_p->fem_bit_info[bit_index].fem_bit_format == DNX_FIELD_FEM_BIT_FORMAT_INVALID)
        {
            fem_bit_format_equivalent_2 = DNX_FIELD_FEM_BIT_FORMAT_FROM_THIS_FIELD;
            fem_bit_value_equivalent_2.bit_value = 0;
        }
        else
        {
            fem_bit_format_equivalent_2 = fem_action_entry_2_p->fem_bit_info[bit_index].fem_bit_format;
            fem_bit_value_equivalent_2 = fem_action_entry_2_p->fem_bit_info[bit_index].fem_bit_value;
        }
        if (fem_bit_format_equivalent_1 != fem_bit_format_equivalent_2)
        {
            (*is_equivalent_p) = FALSE;
            SHR_EXIT();
        }
        if (sal_memcmp(&fem_bit_value_equivalent_1, &fem_bit_value_equivalent_2, sizeof(fem_bit_value_equivalent_1)) !=
            0)
        {
            (*is_equivalent_p) = FALSE;
            SHR_EXIT();
    ...
    if (fem_action_entry_1_p->fem_adder != fem_action_entry_2_p->fem_adder)
    {
        (*is_equivalent_p) = FALSE;
        SHR_EXIT();
    }

    (*is_equivalent_p) = TRUE;
