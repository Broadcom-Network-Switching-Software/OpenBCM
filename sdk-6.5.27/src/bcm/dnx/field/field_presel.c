    ...
    dnx_field_presel_qual_data_t * qual_data_p)
{
    ...
    sal_memset(qual_data_p, 0x0, sizeof(*qual_data_p));
    ...
    if (entry_data_p->entry_valid)
    {
        ...
        while (dbal_field_cs_qual != DBAL_FIELD_EMPTY)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_masked_get(unit,
                                                                         entry_handle_id,
                                                                         dbal_field_cs_qual,
                                                                         qual_data.qual_value, qual_data.qual_mask));

            /*
             * A non-zero mask indicates that the qualifier is used.
             * Only then we count it and fill the output structure with its data.
             */
            if (utilex_bitstream_have_one_in_range(qual_data.qual_mask, 0,
                                                   WORDS2BITS(sizeof(qual_data.qual_mask) /
                                                              sizeof(qual_data.qual_mask[0])) - 1))
            {
                ...
                sal_memcpy(entry_data_p->qual_data[entry_data_p->nof_qualifiers].qual_value, qual_data.qual_value,
                           sizeof(entry_data_p->qual_data[0].qual_value));
                sal_memcpy(entry_data_p->qual_data[entry_data_p->nof_qualifiers].qual_mask, qual_data.qual_mask,
                           sizeof(entry_data_p->qual_data[0].qual_mask));
    ...
    if (entry_data_p->entry_valid)
    {
        ...
        for (qual_idx = 0; qual_idx < entry_data_p->nof_qualifiers; qual_idx++)
        {
                 ...
                 (entry_data_p->qual_data[qual_idx].qual_type_dbal_field == DBAL_FIELD_IN_LIF_PROFILE_MAP_1)))
            {
                ...
                if (utilex_bitstream_have_one_in_range(entry_data_p->qual_data[qual_idx].qual_value, nof_bits,
                                                       (WORDS2BITS(sizeof(entry_data_p->qual_data[0].qual_value) /
                                                                   sizeof(entry_data_p->qual_data[0].qual_value[0])) -
                                                        1)))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    dnx_field_presel_entry_data_t * entry_data_pruned_p)
{
    ...
    if (entry_data_p->entry_valid)
    {
        ...
        {
            if (utilex_bitstream_have_one_in_range(entry_data_p->qual_data[qual_idx_src].qual_mask, 0,
                                                   WORDS2BITS(sizeof(entry_data_p->qual_data[0].qual_mask) /
                                                              sizeof(entry_data_p->qual_data[0].qual_mask[0])) - 1))
            {
    ...
    dnx_field_presel_entry_data_t * entry_data_p)
{
    ...
    if (entry_id_p->stage == DNX_FIELD_STAGE_EXTERNAL && entry_data_intern_pruned.entry_valid)
    {
        ...
        uint32 opcode_id;
        uint32 qualifying_static_opcode;
        uint32 base_opcode_profile;
        int acl_ctx_idx;
        uint8 is_dynamic;
        uint8 dynamic_relative_position;

        opcode_id = entry_data_intern_pruned.context_id;

        SHR_IF_ERR_EXIT(dnx_field_map_opcode_is_dynamic(unit, opcode_id, &is_dynamic, &dynamic_relative_position));
        if (is_dynamic == FALSE)
        {
            /*
            ...
            qualifying_static_opcode = opcode_id;
        }
        else
        {
            /*
            ...
            SHR_IF_ERR_EXIT(dnx_field_context_apptype_opcode_cascaded_get(unit, opcode_id, &qualifying_static_opcode));
        }
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_profile_get(unit, qualifying_static_opcode, &base_opcode_profile));
        ...
        entry_data_intern_pruned.qual_data[cascaded_idx].qual_value[0] = base_opcode_profile;
        entry_data_intern_pruned.qual_data[cascaded_idx].qual_mask[0] = 0x3F;
    ...
    if (entry_id_p->stage == DNX_FIELD_STAGE_IPMF2)
    {
        ...
        if (entry_data_intern_pruned.entry_valid)
        {
            ...
            /*
             * Sanity check (default context doesn't need to be linked)
             */
            if (cascaded_from != BCM_FIELD_CONTEXT_ID_DEFAULT
                && profile_num == DNX_ALGO_FIELD_CONTEXT_LINK_PROFILE_INVALID)
            {
            ...
            entry_data_intern_pruned.qual_data[cascaded_idx].qual_value[0] = profile_num;
            entry_data_intern_pruned.qual_data[cascaded_idx].qual_mask[0] = 0x1F;
            entry_data_intern_pruned.nof_qualifiers++;
        }
    }
    ...
    dnx_field_presel_entry_data_t * entry_data_p)
{
        ...
        (entry_id_p->stage == DNX_FIELD_STAGE_IPMF2 || entry_id_p->stage == DNX_FIELD_STAGE_EXTERNAL))
    {
        ...
        for (qual_idx = 0; qual_idx < entry_data_p->nof_qualifiers; qual_idx++)
        {
            if (entry_data_p->qual_data[qual_idx].qual_type_dbal_field == dbal_field_to_remove)
            {
                ...
                removed_qual_value = entry_data_p->qual_data[qual_idx].qual_value[0];
                /*
    ...
    if (entry_data_p->entry_valid && (entry_id_p->stage == DNX_FIELD_STAGE_EXTERNAL))
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_map_profile_to_opcode_get(unit, base_apptype_profile, &base_opcode));
        if (enable_context == DNX_FIELD_PRESEL_CTX_ID_VALID_RESET)
        {
            /*
             * Predefined Apptype
             */
            opcode_id = base_opcode;
        }
        else if (enable_context == DNX_FIELD_PRESEL_CTX_ID_VALID_SET)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_lookup_fwd_contexts
                            (unit, base_opcode, &nof_fwd_contexts, fwd_contexts));
            /** Sanity check.*/
            if (nof_fwd_contexts <= 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Opcode %d has no fwd_contexts (fwd_nof_contexts %d).\n", base_opcode, nof_fwd_contexts);
            }
            SHR_IF_ERR_EXIT(dnx_field_map_contexts_to_opcode_get
                            (unit, fwd_contexts[0], entry_data_p->context_id, &opcode_id));
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
        ...
        entry_data_p->context_id = opcode_id;
    }
    ...
    dnx_field_presel_entry_data_t * dnx_entry_data_p)
{
    ...
    dbal_fields_e pem_scratch_pad_dbal_field;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (bcm_entry_data_p->entry_valid && (stage == DNX_FIELD_STAGE_EXTERNAL))
    {
        bcm_field_AppType_t app_type = bcm_entry_data_p->context_id;
        SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_opcode_bcm_to_dnx(unit, app_type, &(dnx_entry_data_p->context_id)));
    }
    else
    {
    ...
    for (qual_idx = dnx_entry_data_p->nof_qualifiers - 1; qual_idx >= 0; qual_idx--)
    {
        for (qual_idx2 = dnx_entry_data_p->nof_qualifiers - 1; qual_idx2 > qual_idx; qual_idx2--)
        {
                ...
                dnx_entry_data_p->qual_data[qual_idx2].qual_type_dbal_field)
            {
                ...
                int is_special_1, is_special_2;
                uint32 combined_mask[sizeof(dnx_entry_data_p->qual_data[0].qual_mask) /
                                     sizeof(dnx_entry_data_p->qual_data[0].qual_mask[0])] = { 0 };
                SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_dnx_dbal_to_bcm
                                (unit, stage, dnx_entry_data_p->context_id,
                                 dnx_entry_data_p->qual_data[qual_idx].qual_type_dbal_field,
                                 &qual_type1, &qual_arg1, &is_special_1));
                SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_dnx_dbal_to_bcm
                                (unit, stage, dnx_entry_data_p->context_id,
                                 dnx_entry_data_p->qual_data[qual_idx2].qual_type_dbal_field,
                                 &qual_type2, &qual_arg2, &is_special_2));
                ...
                {
                    /** Sanity check */
                    if (qual_arg1 != qual_arg2)
                    {
                        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                     "Qualifiers at indices %d and %d have the same DBAL field %d "
                                     "but different input arguments (%d and %d).\r\n",
                                     qual_idx, qual_idx2,
                                     dnx_entry_data_p->qual_data[qual_idx].qual_type_dbal_field, qual_arg1, qual_arg2);
                    }

                    sal_memcpy(combined_mask, dnx_entry_data_p->qual_data[qual_idx].qual_mask, sizeof(combined_mask));
                    SHR_IF_ERR_EXIT(utilex_bitstream_and(combined_mask,
                                                         dnx_entry_data_p->qual_data[qual_idx2].qual_mask,
                                                         sizeof(combined_mask) / sizeof(combined_mask[0])));
                    if (utilex_bitstream_have_one_in_range(combined_mask, 0,
                                                           (WORDS2BITS(sizeof(combined_mask) / sizeof(combined_mask[0]))
                                                            - 1)))
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "Qualifier at index %d and qualifier at index %d qualify on the same bit/bits.\r\n",
                                     qual_idx, qual_idx2);
                    }
                    /*
                     * Remove values outside of the mask.
                     */
                    SHR_IF_ERR_EXIT(utilex_bitstream_and(dnx_entry_data_p->qual_data[qual_idx].qual_value,
                                                         dnx_entry_data_p->qual_data[qual_idx].qual_mask,
                                                         sizeof(dnx_entry_data_p->qual_data[0].qual_value) /
                                                         sizeof(dnx_entry_data_p->qual_data[0].qual_value[0])));
                    SHR_IF_ERR_EXIT(utilex_bitstream_and(dnx_entry_data_p->qual_data[qual_idx2].qual_value,
                                                         dnx_entry_data_p->qual_data[qual_idx2].qual_mask,
                                                         sizeof(dnx_entry_data_p->qual_data[0].qual_value) /
                                                         sizeof(dnx_entry_data_p->qual_data[0].qual_value[0])));
                    /*
                     ...
                     */
                    SHR_IF_ERR_EXIT(utilex_bitstream_or(dnx_entry_data_p->qual_data[qual_idx].qual_value,
                                                        dnx_entry_data_p->qual_data[qual_idx2].qual_value,
                                                        sizeof(dnx_entry_data_p->qual_data[0].qual_value) /
                                                        sizeof(dnx_entry_data_p->qual_data[0].qual_value[0])));
                    SHR_IF_ERR_EXIT(utilex_bitstream_or(dnx_entry_data_p->qual_data[qual_idx].qual_mask,
                                                        dnx_entry_data_p->qual_data[qual_idx2].qual_mask,
                                                        sizeof(dnx_entry_data_p->qual_data[0].qual_mask) /
                                                        sizeof(dnx_entry_data_p->qual_data[0].qual_mask[0])));
    ...
    /*
     * Go over the qualifiers and check for PEM scratch pad. unite them.
     */
    pem_scratch_pad_dbal_field =
        dnx_data_field_map.qual.cs_get(unit, stage, DNX_FIELD_CS_QUAL_PEM_SCRATCH_PAD)->dbal_field;
    if (pem_scratch_pad_dbal_field != DBAL_FIELD_EMPTY)
    {
        for (qual_idx = dnx_entry_data_p->nof_qualifiers - 1; qual_idx >= 0; qual_idx--)
        {
            for (qual_idx2 = dnx_entry_data_p->nof_qualifiers - 1; qual_idx2 > qual_idx; qual_idx2--)
            {
                if ((dnx_entry_data_p->qual_data[qual_idx].qual_type_dbal_field == pem_scratch_pad_dbal_field) &&
                    (dnx_entry_data_p->qual_data[qual_idx2].qual_type_dbal_field == pem_scratch_pad_dbal_field))
                {
                    uint32 combined_mask[sizeof(dnx_entry_data_p->qual_data[0].qual_mask) /
                                         sizeof(dnx_entry_data_p->qual_data[0].qual_mask[0])] = { 0 };
                    sal_memcpy(combined_mask, dnx_entry_data_p->qual_data[qual_idx].qual_mask, sizeof(combined_mask));
    ...
    bcm_field_presel_entry_data_t * bcm_entry_data_p)
{
    int dnx_qual_idx;
    int bcm_qual_idx;
    uint32 qual_arg_uint32;
    uint8 is_std_1;
    int special_qual_found = FALSE;
    ...
    if (dnx_entry_data_p->entry_valid && (dnx_entry_id_p->stage == DNX_FIELD_STAGE_EXTERNAL))
    {
        uint32 opcide_id = dnx_entry_data_p->context_id;
        bcm_field_AppType_t app_type;
        SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_apptype_dnx_to_bcm(unit, opcide_id, &app_type));
        bcm_entry_data_p->context_id = app_type;
    }
    else
    {
     ...
     * Iterate all the used qualifiers
     */
    for (dnx_qual_idx = 0, bcm_qual_idx = 0; dnx_qual_idx < dnx_entry_data_p->nof_qualifiers; dnx_qual_idx++)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_dnx_dbal_to_bcm(unit,
                                                              dnx_entry_id_p->stage, dnx_entry_data_p->context_id,
                                                              dnx_entry_data_p->
                                                              qual_data[dnx_qual_idx].qual_type_dbal_field,
                                                              &(bcm_entry_data_p->qual_data[bcm_qual_idx].qual_type),
                                                              &qual_arg_uint32, &is_special));
        if (is_special)
        {
            int special_nof_bcm_quals;
            bcm_field_qualify_t special_bcm_quals[BCM_FIELD_MAX_NOF_CS_QUALIFIERS];
            uint32 special_qual_arg_uint32[BCM_FIELD_MAX_NOF_CS_QUALIFIERS];
            uint32 special_bcm_values[BCM_FIELD_MAX_NOF_CS_QUALIFIERS];
            uint32 special_bcm_masks[BCM_FIELD_MAX_NOF_CS_QUALIFIERS];
            int special_bcm_qual_iter;

            if (special_qual_found)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "special qualifer field found more than once.\r\n");
            }
            special_qual_found = TRUE;

            SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_special_qual_dbal_to_bcm(unit, dnx_entry_id_p->stage,
                                                                           dnx_entry_data_p->context_id,
                                                                           dnx_entry_data_p->
                                                                           qual_data[dnx_qual_idx].qual_type_dbal_field,
                                                                           dnx_entry_data_p->
                                                                           qual_data[dnx_qual_idx].qual_value,
                                                                           dnx_entry_data_p->
                                                                           qual_data[dnx_qual_idx].qual_mask,
                                                                           &special_nof_bcm_quals, special_bcm_quals,
                                                                           special_qual_arg_uint32, special_bcm_values,
                                                                           special_bcm_masks));
            if (special_nof_bcm_quals < 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "special_nof_bcm_quals %d is negative.\r\n", special_nof_bcm_quals);
            }
            if (special_nof_bcm_quals + dnx_entry_data_p->nof_qualifiers - 1 > BCM_FIELD_MAX_NOF_CS_QUALIFIERS)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Too many qualifers. Special %d, others %d, maximum %d.\r\n",
                             special_nof_bcm_quals, (dnx_entry_data_p->nof_qualifiers - 1),
                             BCM_FIELD_MAX_NOF_CS_QUALIFIERS);
            }

            for (special_bcm_qual_iter = 0; special_bcm_qual_iter < special_nof_bcm_quals; special_bcm_qual_iter++)
            {
                bcm_entry_data_p->qual_data[bcm_qual_idx].qual_type = special_bcm_quals[special_bcm_qual_iter];
                bcm_entry_data_p->qual_data[bcm_qual_idx].qual_value = special_bcm_values[special_bcm_qual_iter];
                bcm_entry_data_p->qual_data[bcm_qual_idx].qual_mask = special_bcm_masks[special_bcm_qual_iter];
                bcm_entry_data_p->qual_data[bcm_qual_idx].qual_arg = special_qual_arg_uint32[special_bcm_qual_iter];
                bcm_qual_idx++;
            }
        ...
        if (utilex_bitstream_have_one_in_range(dnx_entry_data_p->qual_data[dnx_qual_idx].qual_value,
                                               SAL_UINT32_NOF_BITS,
                                               (WORDS2BITS(sizeof(dnx_entry_data_p->qual_data[0].qual_value) /
                                                           sizeof(dnx_entry_data_p->qual_data[0].qual_value[0])) - 1)))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "CS qual %d has value beyond %d bits.\r\n",
                         dnx_qual_idx, SAL_UINT32_NOF_BITS);
        }
        if (utilex_bitstream_have_one_in_range(dnx_entry_data_p->qual_data[dnx_qual_idx].qual_mask,
                                               SAL_UINT32_NOF_BITS,
                                               (WORDS2BITS(sizeof(dnx_entry_data_p->qual_data[0].qual_mask) /
                                                           sizeof(dnx_entry_data_p->qual_data[0].qual_mask[0])) - 1)))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "CS qual %d has mask beyond %d bits.\r\n", dnx_qual_idx, SAL_UINT32_NOF_BITS);
        }

        if (bcm_entry_data_p->qual_data[bcm_qual_idx].qual_type == bcmFieldQualifyForwardingTypeRaw && is_std_1)
        {
            uint32 dbal_value;
            rv_dbal = dbal_fields_enum_value_get
                (unit, DBAL_FIELD_LAYER_TYPE, dnx_entry_data_p->qual_data[dnx_qual_idx].qual_value[0], &dbal_value);
            if ((rv_dbal == _SHR_E_INTERNAL) || (rv_dbal == _SHR_E_NOT_FOUND) || (rv_dbal == _SHR_E_PARAM))
            {
                rv = _SHR_E_NOT_FOUND;
                bcm_qual_type = bcm_entry_data_p->qual_data[bcm_qual_idx].qual_type;
            }
            else
            {
        ...
        else if (bcm_entry_data_p->qual_data[bcm_qual_idx].qual_type == bcmFieldQualifyVlanFormatRaw && is_std_1 &&
                 (dnx_entry_id_p->stage != DNX_FIELD_STAGE_EPMF))
        {
            uint32 dbal_value;
            rv_dbal = dbal_fields_enum_value_get
                (unit, DBAL_FIELD_INCOMING_TAG_STRUCTURE, dnx_entry_data_p->qual_data[dnx_qual_idx].qual_value[0],
                 &dbal_value);
            if ((rv_dbal == _SHR_E_INTERNAL) || (rv_dbal == _SHR_E_NOT_FOUND) || (rv_dbal == _SHR_E_PARAM))
            {
                rv = _SHR_E_NOT_FOUND;
        ...
        {
            bcm_qual_type = bcm_entry_data_p->qual_data[bcm_qual_idx].qual_type;
            bcm_qual_value = dnx_entry_data_p->qual_data[dnx_qual_idx].qual_value[0];
        }
        ...
        if (rv == _SHR_E_NOT_FOUND)
        {
            LOG_DEBUG_EX(BSL_LOG_MODULE,
                         ...
                         "Presel ID %d stage \"%s\".\r\n",
                         dnx_field_bcm_qual_text(unit, bcm_entry_data_p->qual_data[bcm_qual_idx].qual_type),
                         dnx_entry_data_p->qual_data[dnx_qual_idx].qual_value[0],
                         dnx_entry_id_p->presel_id, dnx_field_stage_text(unit, dnx_entry_id_p->stage));
            bcm_entry_data_p->qual_data[bcm_qual_idx].qual_value =
                dnx_entry_data_p->qual_data[dnx_qual_idx].qual_value[0];
        }
        else
        {
            bcm_entry_data_p->qual_data[bcm_qual_idx].qual_type = bcm_qual_type;
            bcm_entry_data_p->qual_data[bcm_qual_idx].qual_value = bcm_qual_value;
        }

        bcm_entry_data_p->qual_data[bcm_qual_idx].qual_arg = qual_arg_uint32;
        bcm_entry_data_p->qual_data[bcm_qual_idx].qual_mask = dnx_entry_data_p->qual_data[dnx_qual_idx].qual_mask[0];
        bcm_qual_idx++;
    }

    bcm_entry_data_p->nof_qualifiers = bcm_qual_idx;
