    ...
    uint32 parameter)
{
    ...
    if (table_id == DBAL_TABLE_MDB_19_EXEM_3)
    {
        instance = 0;
    }
    else
    ...
    uint32 *flush_profile_id)
{
     ...
     *
     *   Verify:
     *       1. maximal_value >= high_threshold_value
     *       2. high_threshold_value >= low_threshold_value
     *       3. low_threshold_value > out_value
     *       4. init_value > out_value
     */
    if (flush_profile_info->maximal_value < flush_profile_info->high_threshold_value)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! maximal_value (=%d) must be >= high_threshold_value (=%d)!\n",
                     flush_profile_info->maximal_value, flush_profile_info->high_threshold_value);
    }

    if (flush_profile_info->high_threshold_value < flush_profile_info->low_threshold_value)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! high_threshold_value (=%d) must be >= low_threshold_value (=%d)!\n",
                     flush_profile_info->high_threshold_value, flush_profile_info->low_threshold_value);
    }

    if (flush_profile_info->low_threshold_value <= flush_profile_info->out_value)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! low_threshold_value (=%d) must be > out_value (=%d)!\n",
                     flush_profile_info->low_threshold_value, flush_profile_info->out_value);
    }

    if (flush_profile_info->init_value <= flush_profile_info->out_value)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! init_value (=%d) must be > out_value (=%d)!\n",
                     flush_profile_info->init_value, flush_profile_info->out_value);
    }
...
dnx_field_flush_profile_set(
    ...
    uint32 is_disable)
{
    dbal_tables_e table_id_1;
    dbal_tables_e table_id_2;
...

    {
        if (dnx_data_mdb.em.age_profile_per_ratio_support_get(unit))
        {
            table_id_1 = DBAL_TABLE_MDB_20_EXEM_3;
            nof_supported_age_ratios = MDB_STEP_TABLE_NOF_ENTRY_SIZES;
            instance = INST_SINGLE;
        }
        else
        {
            table_id_1 = DBAL_TABLE_MDB_19_EXEM_3;
            nof_supported_age_ratios = 1;
            instance = flush_profile_id;
        }
        table_id_2 = DBAL_TABLE_MDB_21_EXEM_3;
    }

    /** allocate table handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id_1, &entry_handle_id));
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id_2, &entry_handle_id_21));
...
dnx_field_flush_profile_map_set(
    ...
    uint32 flush_profile_id)
{
    dbal_tables_e cfg_table_id = DBAL_NOF_TABLES;
    uint32 entry_handle_id;
    ...
    uint32 *flush_profile_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_profile_create_verify(unit, flags, flush_profile_info, flush_profile_id));
    ...
    bcm_field_flush_profile_info_t * flush_profile_info)
{
     ...
     *   - check profile ID is not already in use.
     */
    nof_flush_profiles = dnx_data_field.exem_learn_flush_machine.nof_flush_profiles_get(unit);
    ...
    bcm_field_flush_profile_info_t * flush_profile_info)
{
    ...
    /** get all fields */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id_21, DBAL_GET_ALL_FIELDS));
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id_21, DBAL_FIELD_MDB_ITEM_0,
                                                        (flush_profile_id * nof_supported_age_ratios),
                                                        &(flush_profile_info->init_value)));
    ...
    bcm_field_flush_profile_info_t * flush_profile_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_profile_get_verify(unit, flags, flush_profile_id, flush_profile_info));
    ...
    uint32 flush_profile_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_field_info_sw.flush_profile_id.free_single(unit, (int) flush_profile_id, NULL));
    ...
    uint32 flush_profile_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_profile_destroy_verify(unit, flags, flush_profile_id));

    /** Deallocate flush-profile ID */
    SHR_IF_ERR_EXIT(dnx_field_flush_profile_deallocate(unit, flush_profile_id));

    /** Clear HW changes done during attach. */
    {
        bcm_field_group_t attached_fg_id;
        SHR_IF_ERR_EXIT(dnx_field_flush_sw.flush_profile.mapped_fg.get(unit, flush_profile_id, &attached_fg_id));
        if (attached_fg_id != DNX_FIELD_GROUP_INVALID)
        {
            dnx_field_app_db_id_t app_db_id;
            int app_db_id_size;
            dnx_field_stage_e attached_field_stage;
            SHR_IF_ERR_EXIT(dnx_field_group_app_db_id_get(unit, attached_fg_id, &app_db_id_size, &app_db_id));
            SHR_IF_ERR_EXIT(dnx_field_group_field_stage_get(unit, attached_fg_id, &attached_field_stage));
            /** delete profile mapping: app-db to profile-id */
            SHR_IF_ERR_EXIT(dnx_field_flush_profile_map_set
                            (unit, dnx_data_field.stage.stage_info_get(unit, attached_field_stage)->exem_pdb, app_db_id,
                             0));
            /**  Delete flush profile learn mapping (flush-profile-id to app-db-id) */
            if (dnx_data_field.exem_learn_flush_machine.feature_get(unit,
                                                                    dnx_data_field_exem_learn_flush_machine_has_elephant_signals))
            {
                SHR_IF_ERR_EXIT(dnx_field_flush_profile_learn_map_set(unit, 0, 0, flush_profile_id));
            }
        }
    }
    ...
    /** Set flush-profile SW DB */
    SHR_IF_ERR_EXIT(dnx_field_flush_sw.flush_profile.mapped_fg.set(unit, flush_profile_id, DNX_FIELD_GROUP_INVALID));
    ...
    uint32 flush_profile_id)
{
    ...
    dnx_field_group_t mapped_fg_id;
    ...
    /*
     * Verify it is EXEM-3 since EXEM-4 is not supported
     */
    SHR_IF_ERR_EXIT(dnx_field_group_field_stage_get(unit, fg_id, &field_stage));
    {
        if (dnx_data_field.stage.stage_info_get(unit, field_stage)->exem_pdb != DBAL_PHYSICAL_TABLE_SEXEM_3)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Error! fg_id (=%d) - stage = \"%s\" (%d), Only SEXEM-3 is supported! \n",
                         fg_id, dnx_field_stage_text(unit, field_stage), field_stage);
        }
    }
     ...
     */
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error! high_threshold_value (=%d) doesn't fit within maximum number "
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error! low_threshold_value (=%d) doesn't fit within maximum number "
                         "of age bits (=%d) for field group %d.\n",
                         flush_profile_info.low_threshold_value, nof_age_bits, fg_id);
        }
        if ((flush_profile_info.increment_value & age_mask_inverted) != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error! increment_value (=%d) doesn't fit within maximum number "
                         "of age bits (=%d) for field group %d.\n",
                         flush_profile_info.increment_value, nof_age_bits, fg_id);
        }
        if ((flush_profile_info.decrement_value & age_mask_inverted) != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error! decrement_value (=%d) doesn't fit within maximum number "
                         "of age bits (=%d) for field group %d.\n",
                         flush_profile_info.decrement_value, nof_age_bits, fg_id);
        }
        if ((flush_profile_info.maximal_value & age_mask_inverted) != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error! maximal_value (=%d) doesn't fit within maximum number "
                         "of age bits (=%d) for field group %d.\n",
                         flush_profile_info.maximal_value, nof_age_bits, fg_id);
        }
        if ((flush_profile_info.out_value & age_mask_inverted) != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error! out_value (=%d) doesn't fit within maximum number "
                         "of age bits (=%d) for field group %d.\n", flush_profile_info.out_value, nof_age_bits, fg_id);
        }
    }

    /*
     * Flush Profile can be mapped to only one FG!
     * Verify that the flush profile is not already attached:
     */
    SHR_IF_ERR_EXIT(dnx_field_flush_sw.flush_profile.mapped_fg.get(unit, flush_profile_id, &mapped_fg_id));
    if (mapped_fg_id != DNX_FIELD_GROUP_INVALID)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error! flush_profile_id (=%d) is already attached to fg_id = %d !!!\n",
                     flush_profile_id, mapped_fg_id);
    }

    /*
     * Verify the field group is not already attached to a flush profile.
     */
    for (profile_curr = 0; profile_curr < nof_flush_profiles; profile_curr++)
    {
        SHR_IF_ERR_EXIT(dnx_field_flush_sw.flush_profile.mapped_fg.get(unit, profile_curr, &mapped_fg_id));
        if (fg_id == mapped_fg_id)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error! fg_id = %d is already attached to flush_profile_id (=%d) !!!\n",
                         mapped_fg_id, flush_profile_id);
        }
    }
    ...
    uint32 flush_profile_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_profile_attach_verify(unit, flags, fg_id, flush_profile_id));
    ...
    if (dnx_data_field.
        exem_learn_flush_machine.feature_get(unit, dnx_data_field_exem_learn_flush_machine_has_elephant_signals))
    {
        SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_profile_attach_app_db_id_verify(unit, app_db_id, flush_profile_id));
    }
    ...
    SHR_IF_ERR_EXIT(dnx_field_flush_profile_map_set(unit, access_info.physical_db_id[0], app_db_id, flush_profile_id));
        ...
        exem_learn_flush_machine.feature_get(unit, dnx_data_field_exem_learn_flush_machine_has_elephant_signals))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_flush_profile_learn_map_set(unit, app_db_id, vmv_value, flush_profile_id));
    }

    /** Set flush-profile SW DB */
    SHR_IF_ERR_EXIT(dnx_field_flush_sw.flush_profile.mapped_fg.set(unit, flush_profile_id, fg_id));
 ...
 * \param [in] table_id - DBAL table
 * \param [in] status_table_id - DBAL table
 *
 * \return
 *   shr_error_e
 *
    ...
    uint32 scan_period_us)
{
    ...
    dnxc_time_t time_for_100_clocks;
    uint64 work_reg_64;
     ...
     */
    time_for_100_clocks.time_units = DNXC_TIME_UNIT_NSEC;
    /*
     * Implement:
    ...
    SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &time_for_100_clocks, &nof_100_clocks_between_metacycles));

    /** Age machine must be stopped before changing the age time */
    SHR_IF_ERR_EXIT(dnx_field_flush_stop_age_machine(unit, exem_index, table_id, status_table_id));
    ...
    uint32 *scan_period_us)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_time_clock_cycles_to_time_get
                    (unit, nof_100_clocks_between_metacycles, DNXC_TIME_UNIT_NSEC, &time));
...
* \param [in] is_delete                  - If true, delete the entry.
...
{
    int nof_bits_entry = 0;
    int nof_bits_app_db_id_max = 0;
    dbal_tables_e table_id_45 = DBAL_NOF_TABLES;
    dbal_tables_e table_id_46 = DBAL_NOF_TABLES;
    ...
    if (is_delete == FALSE)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id_45, DBAL_FIELD_MDB_ITEM_0, INST_SINGLE, 1);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id_45, DBAL_FIELD_MDB_ITEM_4, INST_SINGLE,
                                     source_mask_flipped[0]);
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id_45, DBAL_COMMIT));
        SHR_IF_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id_46, DBAL_COMMIT));
    }
    ...
    uint32 exem_raw_entry_mask[DNX_FIELD_FLUSH_ENTRY_MAX_NOF_UINT32_IN_EXEM_ENTRY])
{
    ...
    {
        int is_large_exem;
        SHR_IF_ERR_EXIT(dnx_field_group_exem_is_lexem(unit, fg_id, &is_large_exem));
        if (is_large_exem)
        {
    ...
    max_bits_in_entry = dnx_data_mdb.em.mdb_45_info_get(unit, dbal_physical_table)->mdb_item_3_field_size;
    SHR_IF_ERR_EXIT(dnx_field_group_key_template_get(unit, fg_id, &key_template));
     ...
     */
    {
        ...
        entry_offset = 0;
        {
            ...
            soc_reg_above_64_val_t data_zero;
            uint32 sizeof_exem_entry = DNX_FIELD_FLUSH_ENTRY_MAX_NOF_UINT32_IN_EXEM_ENTRY * sizeof(exem_raw_entry[0]);

            sal_memset(mdb_entry, 0x0, sizeof(mdb_entry));
            /** Sanity check.*/
            if (sizeof_exem_entry > sizeof(data))
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Sizeof EXEM raw entry (%d) is larger than 'data' container in mdb_em_prepare_entry (%d). Quit.\n",
                             (int) sizeof_exem_entry, (int) sizeof(data));
            }
            sal_memset(data_zero, 0x0, sizeof(data_zero));
            sal_memset(data, 0, sizeof(data));
            sal_memcpy(data, exem_raw_entry, sizeof_exem_entry);
            SHR_IF_ERR_EXIT(mdb_em_prepare_entry(unit, exem_physical_table, exem_entry_app_db_id_value, FALSE,
                                                 exem_entry_key_value, key_size, exem_entry_payload_value, payload_size,
                                                 entry_encoding, entry_offset, data));
            sal_memcpy(exem_raw_entry, data, sizeof_exem_entry);
            /*
             * Sanity check.
             * Verify that nothing was written to data beyond what passes to exem_raw_entry,
             * by zeroing the copied bytes and comparing DATA to zero.
             */
            {
                sal_memset(data, 0x0, sizeof_exem_entry);
                if (sal_memcmp(data, data_zero, sizeof(data)))
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "Preparing EXEM key for flush entry used more bits than expected. fg_id %d\n", fg_id);
                }
            }

            sal_memset(mdb_entry_mask, 0x0, sizeof(mdb_entry_mask));
            sal_memset(data, 0, sizeof(data));
            sal_memcpy(data, exem_raw_entry_mask, sizeof_exem_entry);
            SHR_IF_ERR_EXIT(mdb_em_prepare_entry(unit, exem_physical_table, exem_entry_app_db_id_value, FALSE,
                                                 exem_entry_key_mask, key_size, exem_entry_payload_mask, payload_size,
                                                 entry_encoding, entry_offset, data));
            sal_memcpy(exem_raw_entry_mask, data, sizeof_exem_entry);
            /*
             * Sanity check.
             * Verify that nothing was written to data beyond what passes to exem_raw_entry_mask,
             * by zeroing the copied bytes and comparing DATA to zero.
             */
            {
                sal_memset(data, 0x0, sizeof_exem_entry);
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "Preparing EXEM mask for flush entry used more bits than expected. fg_id %d\n", fg_id);
                }
            }
        }
        exem_row_size = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, exem_physical_table)->row_width;
    ...
    uint32 exem_new_payload_raw_mask[DNX_FIELD_FLUSH_ENTRY_MAX_NOF_UINT32_IN_EXEM_ENTRY])
{
    ...
    max_payload_size_in_hw = dnx_data_field.stage.stage_info_get(unit, field_stage)->exem_max_result_size;
    SHR_IF_ERR_EXIT(dnx_field_group_actions_payload_size_get(unit, fg_id, &payload_size));
    ...
    dnx_field_flush_entry_key_t * flush_entry_key_info_p)
{
     ...
     */
    {
        ...
        entry_size = dnx_data_mdb.em.em_info_get(unit, exem_physical_table)->entry_size;
        ...
        temp_total_size =
            dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, exem_physical_table)->row_width / (1 << entry_encoding);

        zero_buffer = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, exem_physical_table)->row_width - temp_total_size;
    ...
    dnx_field_flush_entry_t * flush_entry_info_p)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "delete not supported.\n ");
    }
    if (flush_entry_info_p->payload_info.hit_bit_clear)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "hit_bit_clear not supported.\n ");
    }
    ...
    dnx_field_flush_entry_t * flush_entry_info_p)
{
    ...
    /** Verify the input.*/
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_machine_entry_add_verify(unit, flags, fg_id, entry_id, flush_entry_info_p));
    ...
    /** Write to HW.*/
    SHR_IF_ERR_EXIT(dnx_field_flush_machine_hw_set(unit, is_lexem, entry_id, FALSE,
                                                   flush_entry_info_p->key_info.hit_bit_value,
                                                   flush_entry_info_p->key_info.hit_bit_mask,
    ...
    dnx_field_flush_entry_t * flush_entry_info_p)
{
    ...
    /** Verify the input.*/
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_machine_entry_get_verify(unit, flags, fg_id, entry_id, flush_entry_info_p));
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_machine_entry_get_verify_check_fg
    ...
    dnx_field_flush_entry_id_t entry_id)
{
    ...
    /** Verify the input.*/
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_machine_entry_delete_verify(unit, flags, fg_id, entry_id));

    SHR_IF_ERR_EXIT(dnx_field_group_exem_is_lexem(unit, fg_id, &is_lexem));

    /** Write to HW default entry.*/
    SHR_IF_ERR_EXIT(dnx_field_flush_machine_hw_set(unit, is_lexem, entry_id, TRUE, 0, 0, 0, 0, 0,
                                                   NULL, NULL, NULL, NULL));
...
/** See field_flush.h */
shr_error_e
dnx_field_flush_sw_state_init(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_field_flush_sw.init(unit));
    SHR_IF_ERR_EXIT(dnx_field_flush_sw.flush_profile.alloc(unit));
