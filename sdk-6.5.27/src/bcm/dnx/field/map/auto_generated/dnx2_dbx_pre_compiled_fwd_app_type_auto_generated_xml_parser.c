...
void*
dbx_pre_compiled_dnx2_fwd_app_types_top_get(
    int unit,
    char *filepath,
    char *topname,
    int flags)
{
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        if(is_image)
        {
            curTop = dbx_xml_top_get_internal(_filepath, topname, flags, pre_compiled_dnx2_fwd_app_types_standard_1_xml_to_buf);
            if(curTop)
            {
                return curTop;
            }
