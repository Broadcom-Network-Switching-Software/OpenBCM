...
#include "dbx_pre_compiled_fwd_app_type_auto_generated_xml_parser.h"
    ...
    bcm_switch_range_result_map_t bcm_rangeresultmap)
{
    ...
    {
        rangeresultmap_name = "Invalid RangeResultMap";
    }
    else
    {
 ...
 */
char *
dnx_field_bcm_app_db_text(
    bcm_field_app_db_t pp_app)
{
    char *app_dp_name;
    char *bcm_app_db_text[bcmFieldAppDbCount] = BCM_FIELD_APP_DB_STRINGS;

    if ((pp_app < 0) || (pp_app >= bcmFieldAppDbCount))
    {
        app_dp_name = "Invalid APP DB";
    }
    else
    {
        app_dp_name = bcm_app_db_text[pp_app];
    }
    return app_dp_name;
}
    ...
    uint8 static_opcode_is_valid[DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF])
{
    ...
    RHDATA_ITERATOR(app_type_node, fwd2_app_type_node, "AppType")
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "Opcode \"%.*s\" has name with %d characters, maximum %d.\n",
        ...
        RHDATA_ITERATOR(context_iter, app_type_node, "FwdContext")
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "Context \"%.*s\" has name with %d characters, maximum %d. "
            ...
            if (sw_state_is_warm_boot(unit))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                                fwd2_context_ids.get(unit, opcode_id, context_index, &fwd_context_uint32_get));
                if (fwd_context_uint32_get != fwd_context_uint32)
                {
                    SHR_ERR_EXIT(_SHR_E_CONFIG,
                                 "Mismatch XML and SW state. For opcode_id %d, context number %d is %d "
                                 "in XML and %d in SW state.\n", opcode_id, context_index, fwd_context_uint32,
            ...
            else
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                                fwd2_context_ids.set(unit, opcode_id, context_index, fwd_context_uint32));
            }
        ...
        if (sw_state_is_warm_boot(unit))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                            nof_fwd2_context_ids.get(unit, opcode_id, &nof_context_get));
            if (nof_context_get != nof_contexts)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "Mismatch XML and SW state. For opcode_id %d, %d context in "
                             "XML and %d in SW state.\n", opcode_id, nof_contexts, nof_context_get);
            }
        }
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                            nof_fwd2_context_ids.set(unit, opcode_id, nof_contexts));
        }
     ...
     */
    {
        ...
        while (dbal_fwd2_context_enum_iterator != INVALID_ENUM)
        {
            ...
            for (i_opcode = 0;
                 (i_opcode < DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF) && (found_mapped_apptype == FALSE); i_opcode++)
            {
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                                nof_fwd2_context_ids.get(unit, i_opcode, &nof_fwd2_context_get));
                for (i_context_in_apptype = 0; i_context_in_apptype < nof_fwd2_context_get; i_context_in_apptype++)
                {
                    uint8 fwd2_context_id_sw;
                    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                                    fwd2_context_ids.get(unit, i_opcode, i_context_in_apptype, &fwd2_context_id_sw));
    ...
    int unit)
{
...

    {
        ...
        {
            if (apptype_enum_iter >= DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "The number of static opcodes (%d) is too small for opcode enum value %d.\n",
                             DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF, apptype_enum_iter);
            }
            SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get
...
#if defined(INCLUDE_KBP)
    if (DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF > DNX_KBP_MAX_NOF_OPCODES)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Number of elements in static opcode array is %d, "
                     "Exceeds maximum number of KBP opcodes %d.\n",
                     DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF, DNX_KBP_MAX_NOF_OPCODES);
    }
#endif
    ...
    {
        nof_static_apptypes = DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF;
    }
    ...
    for (i_apptype = 0; i_apptype < nof_static_apptypes; i_apptype++)
    {
        ...
        if ((int) mapped_opcode < 0 || mapped_opcode >= DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Apptype %d mapped to opcode %d. Maximum static opcode number is %d\n",
                         i_apptype, mapped_opcode, (DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF - 1));
        }
        ...
        if (!sw_state_is_warm_boot(unit))
        {
            SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.apptype.set(unit, mapped_opcode, i_apptype));
            SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.is_valid.set(unit, mapped_opcode, TRUE));
        }
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                        nof_fwd2_context_ids.get(unit, mapped_opcode, &nof_contexts));
        if (nof_contexts <= 0)
        {
    ...
    for (i_opcode = 0; i_opcode < DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF; i_opcode++)
    {
        uint8 opcode_is_valid;
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.is_valid.get(unit, i_opcode, &opcode_is_valid));
        if (opcode_is_valid)
        {
            if (!sw_state_is_warm_boot(unit))
            {
                uint8 nof_contexts;
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                                nof_fwd2_context_ids.get(unit, i_opcode, &nof_contexts));
                if (nof_contexts > 0)
                {
                ...
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                                cs_profile_id.set(unit, i_opcode, cs_profile_id_counter));
            }
            ...
            for (i_apptype = 0; i_apptype < nof_static_apptypes; i_apptype++)
            {
                ...
                if (rv != _SHR_E_NOT_FOUND)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_lookup_fwd_contexts
                                    (unit, mapped_opcode, &mapped_nof_contexts, mapped_fwd_contexts));
    ...
    dnx_field_port_porfile_type_e * port_profile_type_p)
{
    ...
    (*port_profile_type_p) = dnx_data_field_map.port_profile.port_class_profile_get(unit, bcm_port_class)->profile_type;
    ...
    dnx_field_map_port_profile_info_t * port_profile_info_p)
{
    ...
    port_profile_info_p->port_profile_type =
        dnx_data_field_map.port_profile.port_class_profile_get(unit, bcm_port_class)->profile_type;
    if (port_profile_info_p->port_profile_type == DNX_FIELD_PORT_PROFILE_TYPE_INVALID)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "BCM port class:%d is unsupported for field map.\n", bcm_port_class);
    }

    port_profile_info_p->dbal_table_id =
        dnx_data_field_map.port_profile.port_class_profile_get(unit, bcm_port_class)->dbal_table;
    port_profile_info_p->dbal_key_field =
        dnx_data_field_map.port_profile.port_class_profile_get(unit, bcm_port_class)->dbal_key;
    port_profile_info_p->dbal_result_field =
        dnx_data_field_map.port_profile.port_class_profile_get(unit, bcm_port_class)->dbal_result;
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 *opcode_id_p)
{
            ...
            case DNX_FIELD_EXTERNAL_TCAM_TYPE_KBP:
            {
                ...
                for (opcode_index = 0; opcode_index < nof_user_defined_apptypes; opcode_index++)
                {
                    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.
                                    acl_context.get(unit, opcode_index, &acl_context));
                    if (acl_context != DNX_FIELD_APPTYPE_SWSTATE_ACL_CONTEXT_INVALID)
                    {
                        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.
                                        apptype.get(unit, opcode_index, &apptype_uint8));
                        if (apptype_uint8 == app_type)
                        {
                            SHR_IF_ERR_EXIT(dnx_field_map_opcode_dynamic_index_to_opcode
                                            (unit, opcode_index, opcode_id_p));
                            found = TRUE;
                            break;
                        }
                    }
                ...
                if (found == FALSE)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Apptype %d found but could not be converted.\n", app_type);
                }
#else
    ...
    else
    {
        ...
        }
        {
            ...
            {
                /** Sanity check. */
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.apptype.get(unit, opcode_id, &mapped_apptype));
        ...
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                        nof_fwd2_context_ids.get(unit, opcode_id, &nof_contexts));
        if (nof_contexts <= 0)
        {
            SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
            SHR_EXIT();
    ...
    if (rv == _SHR_E_NOT_FOUND)
    {
        ...
        else
        {
            ...
            }
            {
                ...
                uint8 opcode_is_valid;
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                                is_valid.get(unit, opcode_id, &opcode_is_valid));
                if (opcode_is_valid == FALSE)
                {
                    SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Unmapped predefined apptype %d.\n", app_type);
                }
                else
                {
                    /** Sanity check. */
                    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                                    apptype.get(unit, opcode_id, &mapped_apptype));
    ...
    bcm_field_AppType_t * app_type_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_opcode_is_dynamic(unit, opcode_id, &is_user_defined, &opcode_index));
    if (is_user_defined)
    {
        uint8 acl_context;
        ...
        switch (dnx_data_field.external_tcam.type_get(unit))
        {
            case DNX_FIELD_EXTERNAL_TCAM_TYPE_KBP:
            {
                ...
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.apptype.get(unit, opcode_index, &apptype_uint8));
                (*app_type_p) = apptype_uint8;
#else
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
                break;
        ...
        {
            uint8 is_alloc;
            SHR_IF_ERR_EXIT(dnx_algo_field_context_apptype_is_allocated(unit, *app_type_p, &is_alloc));
            if (is_alloc == FALSE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "opcode %d translates to apptype %d, but not allocated.\r\n",
                             opcode_id, *app_type_p);
            }
        }
    }
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.apptype.get(unit, opcode_id, &app_type));
    ...
    uint32 *cs_profile_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.nof_fwd2_context_ids.get(unit, opcode_id, &nof_contexts));
    if (nof_contexts <= 0)
    {
    ...
    if (opcode_is_valid == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "opcode_id %d is invalid, despite having contexts.\n", opcode_id);
    }

    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.cs_profile_id.get(unit, opcode_id, &cs_profile));
    ...
    uint32 *cs_profile_id_p)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.
                        profile_id.get(unit, user_defined_apptype_relative_index, &profile_id_uint8));
        ...
        if ((*cs_profile_id_p) <= 0)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Dynamic opcode %d has no CS profile allocated.\n", opcode_id);
        }
    }
...
shr_error_e
dnx_field_map_profile_to_opcode_get(
    int unit,
    ...
    uint32 *opcode_id_p)
{
     ...
     */
    for (opcode_id_iter = 0; opcode_id_iter < DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF; opcode_id_iter++)
    {
        uint8 opcode_is_valid;
        uint8 nof_contexts_per_opcode;
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.is_valid.get(unit, opcode_id_iter, &opcode_is_valid));
        if (opcode_is_valid == FALSE)
        {
            continue;
        }
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                        nof_fwd2_context_ids.get(unit, opcode_id_iter, &nof_contexts_per_opcode));
        if (nof_contexts_per_opcode <= 0)
        {
            continue;
        }
        SHR_IF_ERR_EXIT(dnx_field_map_static_opcode_to_profile_get(unit, opcode_id_iter, &cs_profile_id_get));
        if (cs_profile_id == cs_profile_id_get)
        {
            (*opcode_id_p) = opcode_id_iter;
            found = TRUE;
            break;
        }
    }
    ...
    if (found == FALSE)
    {
        ...
        for (app_type_index = 0; app_type_index < nof_user_defined_apptypes; app_type_index++)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_field_map_opcode_dynamic_index_to_opcode(unit, app_type_index, opcode_id_p));
                found = TRUE;
                break;
            }
        }
 ...
 */
shr_error_e
dnx_field_map_opcode_to_lookup_fwd_contexts(
    int unit,
    uint32 opcode_id,
    int *nof_contexts_p,
    dnx_field_context_t lkp_fwd_contexts[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE])
{
    ...
    {
        /*
         * Predefined/static Opcode.
         */
        base_opcode_id = opcode_id;
    }
    else
    {
        /*
         * User defined/dynamic Opcode.
         */
        uint8 base_ocode_uint8;
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.
                        base_opcode.get(unit, user_defined_apptype_relative_index, &base_ocode_uint8));
        base_opcode_id = base_ocode_uint8;
    }

    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                    nof_fwd2_context_ids.get(unit, base_opcode_id, &nof_contexts));
    if (nof_contexts > DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Number of FWD contexts for opcode_id %d is %d, maximum is %d.\n",
                     opcode_id, nof_contexts, DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE);
    }
    for (context_index = 0; context_index < nof_contexts; context_index++)
    {
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                        fwd2_context_ids.get(unit, base_opcode_id, context_index, &fwd_context));
        /** Sanity check */
        if (((unsigned int) fwd_context) >= nof_contexts_stage)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "opcode_id %d context number %d is %d. "
                         "Number of contexts in stage is %d.\n",
                         opcode_id, context_index, fwd_context, nof_contexts_stage);
        }
 ...
 */
shr_error_e
dnx_field_map_opcode_to_cs_fwd_contexts(
    int unit,
    uint32 opcode_id,
    int *nof_contexts_p,
    dnx_field_context_t cs_fwd_contexts[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE])
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.user_def_info.
                        base_opcode.get(unit, user_defined_apptype_relative_index, &base_opcode_uint8));
        base_opcode_id = base_opcode_uint8;
    }

    if (dnx_data_field.features.external_cs_based_on_fwd2_get(unit))
    {
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.opcode_predef_info.
                        nof_fwd2_context_ids.get(unit, base_opcode_id, &nof_contexts));
        if (nof_contexts > DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE)
 ...
 */
shr_error_e
dnx_field_map_opcode_to_acl_contexts(
    int unit,
    uint32 opcode_id,
    int is_fwd2,
    int *nof_contexts_p,
    dnx_field_context_t acl_contexts[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE])
{
    ...
    if ((is_fwd2 == FALSE) && external_cs_based_on_fwd2)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "No support for ACL context in iFWD1.\r\n");
    }
    ...
    if (is_user_defined == FALSE)
    {
        ...
        if (is_fwd2)
        {
            SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_lookup_fwd_contexts(unit, opcode_id, nof_contexts_p, acl_contexts));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_cs_fwd_contexts(unit, opcode_id, nof_contexts_p, acl_contexts));
        }
    }
    else
    {
         ...
         * No difference between iFWD1 and iFWD2.
         */
        switch (dnx_data_field.external_tcam.type_get(unit))
        {
            case DNX_FIELD_EXTERNAL_TCAM_TYPE_KBP:
 ...
 */
shr_error_e
dnx_field_map_opcode_is_dynamic(
    int unit,
    uint32 opcode_id,
    ...
    switch (dnx_data_field.external_tcam.type_get(unit))
    {
        case DNX_FIELD_EXTERNAL_TCAM_TYPE_KBP:
        {
            ...
            SHR_IF_ERR_EXIT(kbp_mngr_opcode_is_dynamic
                            (unit, opcode_id, is_user_defined_p, user_defined_apptype_relative_index_p));
#else
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Using user defined opcode %d, but KBP not compiled (maximum %d).\r\n",
                             opcode_id, (DNX_FIELD_EXTERNAL_MAX_STATIC_OPCODES_NOF - 1));
            }
#endif
            break;
        }
 ...
 */
shr_error_e
dnx_field_map_opcode_dynamic_index_to_opcode(
    int unit,
    uint32 user_defined_apptype_relative_index,
    uint32 *user_defined_opcode_p)
{
    uint8 user_defined_opcode_uint8;
    SHR_FUNC_INIT_VARS(unit);
    ...
    switch (dnx_data_field.external_tcam.type_get(unit))
    {
        ...
        {
#if defined(INCLUDE_KBP)
            SHR_IF_ERR_EXIT(kbp_mngr_opcode_dynamic_index_to_opcode
                            (unit, user_defined_apptype_relative_index, &user_defined_opcode_uint8));
#else
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
            break;
    ...
    (*user_defined_opcode_p) = user_defined_opcode_uint8;
 ...
 */
shr_error_e
dnx_field_map_contexts_to_opcode_get(
    int unit,
    ...
    uint32 *opcode_id_p)
{
...

    {
        ...
        switch (dnx_data_field.external_tcam.type_get(unit))
        {
            ...
            {
#if defined(INCLUDE_KBP)
                SHR_IF_ERR_EXIT(dnx_field_kbp_contexts_to_opcode_get(unit, lkp_fwd_context, acl_context, opcode_id_p));
#else
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached KBP function but KBP not compiled.\n");
#endif
                break;
