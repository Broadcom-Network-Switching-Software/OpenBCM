...
* \param [in] flags       - BCM_FIELD_FLAG_WITH_ID - to add a field group with ID.
    ...
    bcm_field_group_t * fg_id)
{
    ...
    if (fg_info->fg_type == bcmFieldGroupTypeStateTable && dnx_data_field.state_table.supported_get(unit) == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "State table FG is not supported on this device\n");
    }
    if (fg_info->fg_type == bcmFieldGroupTypeDirectTcam && dnx_data_field.tcam.direct_supported_get(unit) == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Direct Table TCAM FG is not supported on this device\n");
    }
    /*
     * We do not fail on BCM_FIELD_FLAG_MSB_RESULT_ALIGN, even though we dont force this flag anymore.
     */
    ...
    /** Bank IDs and core IDs should be in range */
    for (bank_idx = 0; bank_idx < fg_info->tcam_info.nof_tcam_banks; bank_idx++)
    {
        if (fg_info->tcam_info.tcam_bank_ids[bank_idx] < 0
            || fg_info->tcam_info.tcam_bank_ids[bank_idx] >= dnx_data_field.tcam.nof_banks_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Given Bank ID %d at index %d during bank add/evacuate call is out of range\n",
                         fg_info->tcam_info.tcam_bank_ids[bank_idx], bank_idx);
        }

        /*
         * Verify core
         */
        DNXCMN_CORE_VALIDATE(unit, fg_info->tcam_info.core_ids[bank_idx], TRUE);

        if (fg_info->fg_type == bcmFieldGroupTypeDirectTcam && fg_info->tcam_info.core_ids[bank_idx] != BCM_CORE_ALL)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Core ID %d at index %d can specified only for TCAM field group, not for Direct TCAM\n",
                         fg_info->tcam_info.core_ids[bank_idx], bank_idx);
        }
    }
    ...
    /*
     * Verify advanced featured for external TCAM field groups.
     */
    ...
    if ((unsupported_flags = (fg_info->external_info.flags & ~(BCM_FIELD_GROUP_EXTERNAL_FLAG_ENTRIES_PER_CORE |
                                                               BCM_FIELD_GROUP_EXTERNAL_FLAG_LPM))))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "External TCAM Flags 0x%X are not supported.\n", unsupported_flags);
    }
    if (((fg_info->external_info.flags & BCM_FIELD_GROUP_EXTERNAL_FLAG_ENTRIES_PER_CORE) != 0) &&
        (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_are_two_kbps_connected)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Per core not supported for multiple KBP devices.\n");
    }
    ...
    bcm_field_group_t * fg_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_group_add_verify(unit, flags, fg_info, fg_id));
    ...
    /*
     * Copy tcam_bank_ids and core_ids arrays to DNX group info
     */
    for (bank_iter = 0; bank_iter < fg_info->tcam_info.nof_tcam_banks; bank_iter++)
    {
        ...
        dnx_field_group_info.tcam_info.core_ids[bank_iter] = fg_info->tcam_info.core_ids[bank_iter];
    }
    ...
    {
        dnx_field_group_info.external_tcam_info.min_capacity_valid = TRUE;
        dnx_field_group_info.external_tcam_info.min_capacity = fg_info->external_info.capacity;
    }
    if (fg_info->external_info.algorithmic_mode_valid)
    {
        dnx_field_group_info.external_tcam_info.algorithmic_mode_valid = TRUE;
        dnx_field_group_info.external_tcam_info.algorithmic_mode = fg_info->external_info.algorithmic_mode;
    }
    dnx_field_group_info.external_tcam_info.flags = fg_info->external_info.flags;
    ...
    bcm_field_group_info_t * fg_info)
{
    ...
    /** Copy tcam_bank_ids and core_ids arrays to BCM group info. */
    for (bank_iter = 0; bank_iter < dnx_fg_basic_info.tcam_info.nof_tcam_banks; bank_iter++)
    {
        ...
        fg_info->tcam_info.core_ids[bank_iter] = dnx_fg_basic_info.tcam_info.core_ids[bank_iter];
    }
    ...
    if (dnx_fg_basic_info.fg_type == DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM)
    {
        ...
        if (dnx_fg_basic_info.external_tcam_info.min_capacity_valid)
        {
            fg_info->external_info.capacity_valid = TRUE;
            fg_info->external_info.capacity = dnx_fg_basic_info.external_tcam_info.min_capacity;
        }
        if (dnx_fg_basic_info.external_tcam_info.algorithmic_mode_valid)
        {
            fg_info->external_info.algorithmic_mode_valid = TRUE;
            fg_info->external_info.algorithmic_mode = dnx_fg_basic_info.external_tcam_info.algorithmic_mode;
        }
        fg_info->external_info.flags = dnx_fg_basic_info.external_tcam_info.flags;
    }
    ...
    bcm_field_context_t * context_id)
{
    ...
    if (stage != bcmFieldStageIngressPMF1)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "context_compare_modes is only allowed for stage \"%s\". Stage is \"%s\".\n",
                         dnx_field_bcm_stage_text(bcmFieldStageIngressPMF1), dnx_field_bcm_stage_text(stage));
        }
    ...
    {
        if (context_info->state_collection_engine_enabled)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "state_collection_engine_enabled is only allowed for stage \"%s\". Stage is \"%s\".\n",
                         dnx_field_bcm_stage_text(bcmFieldStageIngressPMF3), dnx_field_bcm_stage_text(stage));
        }
    }
    ...
    bcm_field_context_t * context_id)
{
    ...
    /** Validate input parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_create_verify(unit, flags, stage, context_info, context_id));
    ...
    else
    {
        ...
        if (stage == bcmFieldStageIngressPMF3)
        {
            dnx_context_mode.state_table_engine_mode = context_info->state_collection_engine_enabled;
        }
    ...
    bcm_field_context_t context_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_destroy_verify(unit, stage, context_id));
    ...
    bcm_field_group_attach_info_t * attach_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_group_context_attach_verify(unit, flags, fg_id, context_id, attach_info));
    ...
    {
        SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_qual_with_attach_info_bcm_to_dnx
                                 (unit, dnx_field_stage, attach_info->key_info.qual_types[qual_iter],
                                  &(attach_info->key_info.qual_info[qual_iter]),
                                  &(dnx_attach_info.dnx_quals[qual_iter]),
                                  &(dnx_attach_info.qual_info[qual_iter])),
    ...
    if (dnx_field_stage == DNX_FIELD_STAGE_EXTERNAL)
    {
        /*
    ...
    bcm_field_group_attach_info_t * attach_info)
{
    ...
    for (qual_iter = 0; qual_iter < BCM_FIELD_NUMBER_OF_QUALS_PER_GROUP; qual_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_qual_with_attach_info_dnx_to_bcm
         ...
         dnx_attach_info.dnx_actions[dnx_action_iter] != DNX_FIELD_ACTION_INVALID; dnx_action_iter++)
    {
        /** For each action array map each DNX action to BCM action. */
        SHR_IF_ERR_EXIT(dnx_field_map_action_dnx_to_bcm(unit, dnx_field_stage,
                                                        dnx_attach_info.dnx_actions[dnx_action_iter],
                                                        &(attach_info->payload_info.action_types[dnx_action_iter])));

        attach_info->payload_info.action_info[dnx_action_iter].priority =
            dnx_attach_info.action_info[dnx_action_iter].priority;
        attach_info->payload_info.action_info[dnx_action_iter].valid_bit_polarity =
    ...
    attach_info->payload_info.nof_actions = dnx_action_iter;
    ...
    bcm_field_action_info_t * action_info)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_user_action_get(unit, action_id, &udq_action_info));
...
{
    dnx_field_qual_flags_e dnx_flags;
    ...
    uint32 hw_value[BCM_FIELD_QUAL_WIDTH_IN_WORDS])
{
    ...
    int i_data;
    uint8 cs_mapping_exists;
    uint8 cs_mapping_and_hw_support_exist;
     ...
     */
    SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_bcm_mapping_exists
                    (unit, dnx_stage, bcm_qual, &cs_mapping_exists, &cs_mapping_and_hw_support_exist));
    if (cs_mapping_and_hw_support_exist)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_presel_entry_data_t_init(unit, &dnx_cs_qual_info));
        SHR_IF_ERR_EXIT(dnx_field_presel_data_bcm_to_dnx_convert
                        (unit, 0, dnx_stage, &bcm_cs_qual_info, &dnx_cs_qual_info));
        /** Sanity check.*/
        if (dnx_cs_qual_info.nof_qualifiers != 1)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Converted one BCM qualifer to %d DNX qualifers.\n",
                         dnx_cs_qual_info.nof_qualifiers);
        }

        sal_memset(hw_value, 0x0, sizeof(hw_value[0]) * BCM_FIELD_QUAL_WIDTH_IN_WORDS);
        for (i_data = 0; i_data < MIN(BCM_FIELD_QUAL_WIDTH_IN_WORDS,
                                      (sizeof(dnx_cs_qual_info.qual_data[i_data].qual_value) /
                                       sizeof(dnx_cs_qual_info.qual_data[i_data].qual_value[0]))); i_data++)
        {
            hw_value[i_data] = dnx_cs_qual_info.qual_data[0].qual_value[0];
        }
        /*
         * Sanity check 
         */
        for (i_data = 1; i_data < (sizeof(dnx_cs_qual_info.qual_data[i_data].qual_value) /
                                   sizeof(dnx_cs_qual_info.qual_data[i_data].qual_value[0])); i_data++)
        {
            if (dnx_cs_qual_info.qual_data[0].qual_value[i_data] != 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "DNX CS qual not contained within 32 bits (index %d). "
                             "Qualifer %s stage %s.\n",
                             i_data, dnx_field_bcm_qual_text(unit, bcm_qual), dnx_field_bcm_stage_text(bcm_stage));
            }
        }
    }
    else
    {
        ...
        sal_memset(&dnx_qual_info, 0x0, sizeof(dnx_qual_info));
        SHR_IF_ERR_EXIT(dnx_field_map_qual_info_bcm_to_dnx(unit, 0, dnx_stage, &bcm_qual_info, &dnx_qual_info));
        sal_memset(hw_value, 0x0, sizeof(hw_value[0]) * BCM_FIELD_QUAL_WIDTH_IN_WORDS);
        for (i_data = 0; i_data < MIN(BCM_FIELD_QUAL_WIDTH_IN_WORDS,
                                      (sizeof(dnx_qual_info.qual_value) / sizeof(dnx_qual_info.qual_value[0])));
             i_data++)
        {
         ...
         * Sanity check 
         */
        for (; i_data < (sizeof(dnx_qual_info.qual_value) / sizeof(dnx_qual_info.qual_value[0])); i_data++)
        {
            if (dnx_qual_info.qual_value[i_data] != 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "DNX qual not contained within 32 bits (index %d). "
                             "Qualifer %s stage %s.\n",
                             i_data, dnx_field_bcm_qual_text(unit, bcm_qual), dnx_field_bcm_stage_text(bcm_stage));
            }
        }
    ...
    bcm_field_qualifier_info_get_t * qual_info)
{
    ...
    /** Initilaize the output structure.*/
...

    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size(unit, dnx_stage, dnx_qual, &nof_mappings_size, NULL, &total_size));
        qual_info->size = total_size;
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_offset(unit, dnx_stage, dnx_qual, &nof_mappings_offset, qual_offsets));
        if (nof_mappings_offset != nof_mappings_size)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Qual \"%s\" has %d mapping for size and %d for offset.\n",
                         dnx_field_bcm_qual_text(unit, qualifier_id), nof_mappings_size, nof_mappings_offset);
        }
        if (nof_mappings_offset < 1)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Qual \"%s\" has invalid number of mappings: %d.\n",
                         dnx_field_bcm_qual_text(unit, qualifier_id), nof_mappings_offset);
        }
        else if (nof_mappings_offset == 1)
        {
            qual_is_split = FALSE;
        }
        else
        ...
        if ((qual_offsets[0] & QUAL_OFFSET_SHIFT_MASK) != qual_offsets[0])
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "BCM Qual \"%s\" has number of mapping %d and offset %d. Offset too large.\n",
                         dnx_field_bcm_qual_text(unit, qualifier_id), nof_mappings_offset, qual_offsets[0]);
        }
        qual_info->offset = qual_offsets[0];
    }

    /*
     * For split qualifiers, encode the qualifier unto the offset.
     * Remove the offset value itself (but the user can add a value to it after calling this API).
     */
    if (qual_is_split)
    {
        if (dnx_qual_class == DNX_FIELD_QUAL_CLASS_USER)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Qual \"%s\" is user defined and yet is split.\n",
                         dnx_field_bcm_qual_text(unit, qualifier_id));

        }
        if (qualifier_id >= bcmFieldQualifyCount)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Qual \"%s\" is not a predefined qualifier and yet is split.\n",
                         dnx_field_bcm_qual_text(unit, qualifier_id));

        }
        if ((qualifier_id & QUAL_OFFSET_BASE_QUAL_MASK) != qualifier_id)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Qual \"%s\" (%d) does not fit within the encoding placement.\n",
                         dnx_field_bcm_qual_text(unit, qualifier_id), qualifier_id);
        }
        if ((dnx_qual_class != DNX_FIELD_QUAL_CLASS_META) && (dnx_qual_class != DNX_FIELD_QUAL_CLASS_META2))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Qual \"%s\" (%d) is not metadata but is split.\n",
                         dnx_field_bcm_qual_text(unit, qualifier_id), qualifier_id);
        }
        qual_info->offset = (qualifier_id << QUAL_OFFSET_BASE_QUAL_LSB) | (SAL_BIT(QUAL_OFFSET_BASE_QUAL_EXITS_BIT));
    }
    ...
    bcm_field_entry_t * entry_handle)
{
    ...
    if (fg_type == DNX_FIELD_GROUP_TYPE_TCAM ||
        fg_type == DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM || fg_type == DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM ||
        fg_type == DNX_FIELD_GROUP_TYPE_STATE_TABLE)
    {
        /*
         * TCAM, TCAM DT, external TCAM and state table should have entry handle
         */
    ...
    if (fg_type != DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM && fg_type != DNX_FIELD_GROUP_TYPE_TCAM &&
        fg_type != DNX_FIELD_GROUP_TYPE_STATE_TABLE && fg_type != DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM &&
        entry_info->core != DBAL_CORE_ALL)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Core ID selection is available only for TCAM, TCAM_DT or state table FGs\n");
    }
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_STATE_TABLE:
        {
            ...
            if (entry_info->core == DBAL_CORE_ALL)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "State table entries can only be added to a specific core, "
                             "not to BCM_CORE_ALL.\n");
            }
            break;
        }
    ...
    if (fg_type != DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM)
    {
        for (qual_iter = 0; qual_iter < entry_info->nof_entry_quals; qual_iter++)
        {
            for (i_data = 0; i_data < BCM_FIELD_QUAL_WIDTH_IN_WORDS; i_data++)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Max value selection is available only for external field groups. "
                                 "Qualifier number %d.\n", qual_iter);
                }
            }
        }
    ...
    if ((fg_type != DNX_FIELD_GROUP_TYPE_TCAM) && (entry_info->valid_bit != TRUE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Non TCAM field groups cannot have valid_bit for entries. "
                     "Non default value detected (%d).\n", entry_info->valid_bit);
    }
    ...
    bcm_field_entry_t * entry_handle)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_entry_add_verify(unit, flags, fg_id, entry_info, entry_handle));
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_TCAM:
        {
            dnx_entry_info.priority = entry_info->priority;
            dnx_entry_info.core = entry_info->core;
            dnx_entry_info.valid_bit = ((entry_info->valid_bit) ? (TRUE) : (FALSE));
            if (flags & BCM_FIELD_FLAG_UPDATE)
            {
                dnx_flags |= DNX_FIELD_ENTRY_GROUP_ADD_FLAG_UPDATE;
            }
            if (flags & BCM_FIELD_FLAG_WITH_ID)
            {
                dnx_flags |= DNX_FIELD_ENTRY_GROUP_ADD_FLAG_WITH_ID;
            }
        ...
        case DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM:
        {
            ...
            /** Call DNX-level entry add API */
            SHR_IF_ERR_EXIT(dnx_field_entry_external_tcam_add(unit, dnx_flags, dnx_fg_id, &dnx_entry_info,
                                                              &dnx_entry_handle));
        ...
        {
            /*
             * We do not copy existing entry handle value for update/with_id cases,
             * as there are no such cases in state table.
             */

            /** Call DNX-level entry add API */
            SHR_IF_ERR_EXIT(dnx_field_entry_state_table_add(unit, dnx_field_stage, &(dnx_entry_info.key_info),
                                                            entry_info->core, &dnx_entry_handle));
        ...
        default:
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "fg_id %d is of type \"%s\", does not support adding entries.\n",
 ...
 *  Pointer to struct of Entry info (see bcm_field_entry_info_t in field.h).
 *  Note: in case if we want to add an EXEM entry, we should provide entry key
    ...
    bcm_field_entry_info_t * entry_info)
{
    ...
    switch (fg_type)
    {
        case DNX_FIELD_GROUP_TYPE_STATE_TABLE:
        {
            uint32 address = DNX_FIELD_ENTRY_STATE_TABLE_ADDRESS(dnx_entry_handle);
            dnx_entry_info.core = DNX_FIELD_ENTRY_STATE_TABLE_CORE_ID(dnx_entry_handle);
            if (dnx_entry_info.core == BCM_CORE_ALL)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "State table entryh as BCM_CORE_ALL for core ID, not supported.\n");
            }
            /** Call DNX-level entry add API */
            SHR_IF_ERR_EXIT(dnx_field_entry_state_table_get(unit, dnx_field_stage, dnx_fg_id, dnx_entry_info.core,
                                                            address, &(dnx_entry_info.key_info)));
        }
            break;
        case DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM:
        case DNX_FIELD_GROUP_TYPE_TCAM:
        ...
        case DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM:
        {
            SHR_IF_ERR_EXIT(dnx_field_entry_external_tcam_get
                            (unit, dnx_fg_id, dnx_entry_handle, NULL, &dnx_entry_info));
        }
            break;
        case DNX_FIELD_GROUP_TYPE_EXEM:
        {
            if (input_nof_entry_quals == BCM_FIELD_INVALID)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Entry number of qualifiers (%d) is default value. For exact match entries, "
                             "key must be provided.\n", input_nof_entry_quals);
            }
            if (input_nof_entry_quals == 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Entry number of qualifiers is 0. For exact match entries, key must be provided.\n");
            }
        ...
        case DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB:
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Entry number of qualifiers (%d) is default value. For direct table MDB entries, "
                             "key must be provided.\n", input_nof_entry_quals);
            }
            if (input_nof_entry_quals == 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Entry number of qualifiers is 0. For direct table MDB entries, key must be provided.\n");
            }
            /** Number of qualifiers must be valid numbers between 0 and max value. */
            if ((input_nof_entry_quals < 0) || (input_nof_entry_quals > BCM_FIELD_NUMBER_OF_QUALS_PER_GROUP) ||
                (input_nof_entry_quals == BCM_FIELD_INVALID))
        ...
        default:
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "fg_id %d is of type \"%s\", does not support getting entry info.\n",
                         fg_id, dnx_field_group_type_e_get_name(fg_type));
        }
            break;
    }

    /** Return retrieved priority, only relevant for TCAM/external TCAM entries */
    if (dnx_entry_info.priority == DNX_FIELD_TCAM_ENTRY_PRIORITY_INVALID)
    {
        entry_info->priority = 0;
    }
    else
         ...
         dnx_action_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_action_dnx_to_bcm(unit, dnx_field_stage,
                                                        ...
                                                        action_info[dnx_action_iter].dnx_action,
                                                        &(entry_info->entry_action[dnx_action_iter].type)));
        for (ii = 0; ii < BCM_FIELD_ACTION_WIDTH_IN_WORDS; ii++)
        {
            entry_info->entry_action[dnx_action_iter].value[ii] =
    ...
    entry_info->nof_entry_actions = dnx_action_iter;
    ...
    bcm_field_entry_t entry_handle)
{
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_EXEM:
        {
            if (entry_qual_info == NULL)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Exact match entry needs to be provided with entry_qual_info.\n");
            }
        ...
        case DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB:
        {
            if (entry_qual_info == NULL)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Direct table MDB entry needs to be provided with entry_qual_info.\n");
            }
        ...
        case DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM:
        {
            SHR_IF_ERR_EXIT(dnx_field_entry_external_tcam_delete(unit, dnx_fg_id, dnx_entry_handle));
        }
            break;
        case DNX_FIELD_GROUP_TYPE_STATE_TABLE:
        {
            int core = DNX_FIELD_ENTRY_STATE_TABLE_CORE_ID(dnx_entry_handle);
            uint32 address = DNX_FIELD_ENTRY_STATE_TABLE_ADDRESS(dnx_entry_handle);
            if (core == BCM_CORE_ALL)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "State table entry has BCM_CORE_ALL for core ID, not supported.\n");
            }

            /** Call DNX-level entry add API */
            SHR_IF_ERR_EXIT(dnx_field_entry_state_table_delete(unit, dnx_field_stage, core, address));

        }
            break;
        default:
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "fg_id %d is of type \"%s\", does not support deleting entries.\n",
                         fg_id, dnx_field_group_type_e_get_name(fg_type));
        }
            break;
    }
 ...
 * \brief
 *  This function creates, sets and installs to HW
 *  a single entry for state table.
 * \param [in] unit -
 *  Device ID
 * \param [in] flags -
 *  Currently not in use!
 * \param [in] entry_info -
 *  Pointer to struct of Entry info (see bcm_field_entry_state_table_info_t in field.h)
 * \return
 *  int - Error Type, in sense of bcm_error_t, same as
 *  shr_error_t
 * \remark
 ...
 * \see
 *  Reference to:
 *  - bcm_field_entry_state_table_info_t_init
 *  - bcm_field_entry_state_table_info_get
 */
int
bcm_dnx_field_entry_state_table_add(
    int unit,
    uint32 flags,
    bcm_field_entry_state_table_info_t * entry_info)
{
 ...
 *  Reference to:
 *  - bcm_field_entry_state_table_info_t_init
 *  - bcm_field_entry_state_table_info_add
 */
int
bcm_dnx_field_entry_state_table_info_get(
    int unit,
    uint32 flags,
    bcm_field_entry_state_table_info_t * entry_info)
{
 ...
 */
int
bcm_dnx_field_entry_state_table_delete(
    int unit,
    uint32 flags,
    bcm_field_entry_state_table_info_t * entry_info)
{
    ...
    bcm_field_context_hash_info_t * hash_info)
{
    ...
    /** Validate input parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_hash_create_verify(unit, flags, stage, context_id, hash_info));
    ...
    {
        SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_qual_with_attach_info_bcm_to_dnx
                                 (unit, dnx_field_stage, hash_info->key_info.qual_types[qual_iter],
                                  &(hash_info->key_info.qual_info[qual_iter]),
                                  &(dnx_hash_info.key_info.dnx_quals[qual_iter]),
                                  &(dnx_hash_info.key_info.qual_info[qual_iter])),
                                 "Error for BCM Qualifier at index %d. stage is %s, context is %d.\n",
                                 qual_iter, dnx_field_bcm_stage_text(stage), context_id);
    }
    ...
    bcm_field_context_hash_info_t * hash_info)
{
    ...
    /** Validate input parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_hash_info_get_verify(unit, stage, context_id, hash_info));
         ...
         dnx_hash_info.key_info.dnx_quals[qual_iter] != DNX_FIELD_QUAL_TYPE_INVALID; qual_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_qual_with_attach_info_dnx_to_bcm
                        (unit, dnx_field_stage, dnx_hash_info.key_info.dnx_quals[qual_iter],
                         &(dnx_hash_info.key_info.qual_info[qual_iter]),
                         &(hash_info->key_info.qual_types[qual_iter]), &(hash_info->key_info.qual_info[qual_iter])));
    }
    ...
    for (; config_iter < DNX_DATA_MAX_FIELD_CONTEXT_NOF_HASH_KEYS &&
         dnx_hash_info.hash_config[config_iter].action_key != DBAL_ENUM_FVAL_FIELD_HASH_LB_KEY_INVALID; config_iter++)
    {
        SHR_IF_ERR_EXIT(dnx_field_map_hash_lb_key_dbal_to_bcm(unit,
                                                              dnx_hash_info.hash_config[config_iter].action_key,
                                                              &bcm_action_key));
    ...
    bcm_field_context_t context_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_hash_destroy_verify(unit, stage, context_id));
    ...
    bcm_field_context_compare_info_t * cmp_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_compare_create_verify(unit, flags, stage, context_id, pair_id, cmp_info));
    ...
    for (qual_iter = 0; qual_iter < cmp_info->first_key_info.nof_quals; qual_iter++)
    {
        SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_qual_with_attach_info_bcm_to_dnx
                                 (unit, dnx_field_stage, cmp_info->first_key_info.qual_types[qual_iter],
                                  &(cmp_info->first_key_info.qual_info[qual_iter]),
                                  &(dnx_pair_info.first_key_info.dnx_quals[qual_iter]),
                                  &(dnx_pair_info.first_key_info.qual_info[qual_iter])),
                                 "Error for BCM Qualifier at index %d first pair. stage is %s, context is %d.\n",
                                 qual_iter, dnx_field_bcm_stage_text(stage), context_id);
    }
    ...
    for (qual_iter = 0; qual_iter < cmp_info->second_key_info.nof_quals; qual_iter++)
    {
        SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_qual_with_attach_info_bcm_to_dnx
                                 (unit, dnx_field_stage, cmp_info->second_key_info.qual_types[qual_iter],
                                  &(cmp_info->second_key_info.qual_info[qual_iter]),
                                  &(dnx_pair_info.second_key_info.dnx_quals[qual_iter]),
                                  &(dnx_pair_info.second_key_info.qual_info[qual_iter])),
                                 "Error for BCM Qualifier at index %d second pair. stage is %s, context is %d.\n",
                                 qual_iter, dnx_field_bcm_stage_text(stage), context_id);
    ...
    bcm_field_context_compare_info_t * cmp_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_compare_info_get_verify(unit, stage, context_id, pair_id, cmp_info));
    ...
    uint32 pair_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_compare_destroy_verify(unit, stage, context_id, pair_id));
...
/**
 * \brief
 *  This function is used to set up a key for state table
 *  for a given context.
 * \param [in] unit -
 *  Device ID
 * \param [in] flags -
 *  Currently not in use!
 * \param [in] stage -
 *  Field stage on which the context id has been allocated.
 * \param [in] context_id -
 *  Context id on which the state table should be created.
 * \param [in] state_table_info -
 *  Pointer to struct of State table info (see bcm_field_context_state_table_info_t in field.h)
 * \return
 *  int - Error Type, in sense of bcm_error_t, same as
 *  shr_error_t
 * \remark
 ...
 * \see
 *  Reference to:
 *  - bcm_field_context_state_table_info_t_init
 */
int
bcm_dnx_field_context_state_table_create(
    int unit,
    uint32 flags,
    bcm_field_stage_t stage,
    bcm_field_context_t context_id,
    bcm_field_context_state_table_info_t * state_table_info)
{
    return BCM_E_UNAVAIL;
}
 ...
 * \brief
 *  This function is used to retrieve info about pair of keys for
 *  used for state table for a given context.
 * \param [in] unit -
 *  Device ID
 * \param [in] flags -
 *  Currently not in use!
 ...
 *  Field stage on which the context id has bean allocated.
 * \param [in] context_id -
 *  Context id for which the state table info should be retrieved.
 * \param [out] state_table_info -
 *  Pointer to struct of State table info (see bcm_field_context_state_table_info_t in field.h)
 * \return
 *  int - Error Type, in sense of bcm_error_t, same as
 *  shr_error_t
 ...
 */
int
bcm_dnx_field_context_state_table_info_get(
    int unit,
    uint32 flags,
    bcm_field_stage_t stage,
    bcm_field_context_t context_id,
...
/**
 * \brief
 *  Destroys an existing Context State table.
 * \param [in] unit -
 *  Device ID
 * \param [in] flags -
 *  Currently not in use!
 ...
 *  Field stage on which the context id has been allocated.
 * \param [in] context_id -
 *  Context id on which the state table was done.
 * \return
 *   int - Error Type, in sense of bcm_error_t, same as
 *   shr_error_t
 * \remark
 ...
 */
int
bcm_dnx_field_context_state_table_destroy(
    int unit,
    uint32 flags,
    bcm_field_stage_t stage,
    bcm_field_context_t context_id)
...
/**
 * \brief
 *  Verify function for bcm_dnx_field_tcam_bank_add().
 * \param [in] unit            - Device ID
 * \param [in] flags           - TBD
 * \param [in] info            - Banks to add/evacuate information
 *
 * \return
 ...
 */
static shr_error_e
bcm_dnx_field_tcam_bank_verify(
    int unit,
    uint32 flags,
    bcm_field_tcam_bank_info_t * info)
{
    int bank_idx;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (info->fg_id != BCM_FIELD_ID_INVALID && info->pp_app != bcmFieldAppDbInvalid)
    {
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "No FG-ID nor PP-APP-DB are given, one of them need to be valid to add/evacuate to the banks\n");
    }
    if (info->fg_id == BCM_FIELD_ID_INVALID)
    {
        int is_table_active = FALSE;
        dbal_tables_e dbal_table;

        dbal_table = (dnx_data_field_map.tcam.tcam_pp_app_get(unit, info->pp_app)->dbal_table);
        SHR_IF_ERR_EXIT(dbal_tables_is_table_active(unit, dbal_table, &is_table_active));

        if (!is_table_active)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "PP-APP %d not available on this device", info->pp_app);
        }

    }

    if (info->nof_banks <= 0 || info->nof_banks > dnx_data_field.tcam.nof_banks_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Number of banks to add/evacuate needs to be between 1-%d, value given: %d\n",
                     dnx_data_field.tcam.nof_banks_get(unit), info->nof_banks);
    }

    for (bank_idx = 0; bank_idx < info->nof_banks; bank_idx++)
    {
        if (info->tcam_bank_ids[bank_idx] < 0
            || info->tcam_bank_ids[bank_idx] >= dnx_data_field.tcam.nof_banks_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Given Bank ID %d at index %d during bank add/evacuate call is out of range\n",
                         info->tcam_bank_ids[bank_idx], bank_idx);
        }

        /*
         * Verify core
         */
        DNXCMN_CORE_VALIDATE(unit, info->core_ids[bank_idx], TRUE);

        if (info->pp_app != bcmFieldAppDbInvalid && info->core_ids[bank_idx] != BCM_CORE_ALL)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Core ID %d at index %d can specified only for field group, not for APP-DB\n",
                         info->core_ids[bank_idx], bank_idx);
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_field_tcam_bank_info_t * evac_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(bcm_dnx_field_tcam_bank_verify(unit, flags, evac_info));
    ...
    if (evac_info->fg_id != BCM_FIELD_ID_INVALID)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_group_tcam_bank_evacuate
                        (unit, dnx_fg_id, evac_info->nof_banks, evac_info->tcam_bank_ids, evac_info->core_ids));
    }
    else        /* PP-APP-DB */
    {
        dbal_tables_e dbal_table;
         ...
         */
        dbal_table = (dnx_data_field_map.tcam.tcam_pp_app_get(unit, evac_info->pp_app)->dbal_table);
        ...
        SHR_IF_ERR_EXIT(dnx_field_tcam_handler_bank_evacuate
                        (unit, tcam_handler_id, evac_info->nof_banks, evac_info->tcam_bank_ids, evac_info->core_ids));
    }
    ...
    bcm_field_tcam_bank_info_t * add_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(bcm_dnx_field_tcam_bank_verify(unit, flags, add_info));

    if (add_info->fg_id != BCM_FIELD_ID_INVALID)
    {
        dnx_field_group_t dnx_fg_id = (dnx_field_group_t) add_info->fg_id;

        SHR_IF_ERR_EXIT(dnx_field_group_tcam_bank_add
                        (unit, dnx_fg_id, add_info->nof_banks, add_info->tcam_bank_ids, add_info->core_ids));
    }
    else        /* PP-APP-DB */
    {
        dbal_tables_e dbal_table;
         ...
         */
        dbal_table = (dnx_data_field_map.tcam.tcam_pp_app_get(unit, add_info->pp_app)->dbal_table);
        SHR_IF_ERR_EXIT(dbal_tables_tcam_handler_id_get(unit, dbal_table, &tcam_handler_id));
        SHR_IF_ERR_EXIT(dnx_field_tcam_handler_bank_add
                        (unit, tcam_handler_id, add_info->nof_banks, add_info->tcam_bank_ids, add_info->core_ids));
    }
    ...
    bcm_field_ace_format_t * ace_format_id)
{
    ...
    if (flags & BCM_FIELD_FLAG_WITH_ID)
    {
        ...
        if (((flags & BCM_FIELD_FLAG_WITH_ID) || (flags & BCM_FIELD_FLAG_UPDATE)) &&
            (((*ace_format_id) < dnx_data_field.ace.ace_id_pmf_alloc_first_get(unit)) ||
             ((*ace_format_id) > dnx_data_field.ace.ace_id_pmf_alloc_last_get(unit))))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    if (dnx_data_field.ace.supported_get(unit) == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "ACE format not supported on device.\n");
    }
    ...
    bcm_field_ace_format_t * ace_format_id)
{
    ...
    /** Validate input parameters. */
    SHR_INVOKE_VERIFY_DNXC(dnx_field_ace_format_add_verify(unit, flags, ace_format_info, ace_format_id));
    ...
    dnx_ace_info.use_ppmc_range = (ace_format_info->use_mutlicast_encap_extension_range) ? (TRUE) : (FALSE);
    ...
    bcm_field_ace_format_info_t * ace_format_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_ace_format_info_get_verify(unit, ace_format_id, ace_format_info));
    ...
    ace_format_info->use_mutlicast_encap_extension_range = dnx_ace_full_info.format_basic_info.use_ppmc_range;
    ...
    for (dnx_action_iter = 0; dnx_action_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP &&
         ...
         dnx_action_iter++)
    {
        if (dnx_action_iter >= BCM_FIELD_NUMBER_OF_ACTIONS_PER_GROUP)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "There are more actions in ace format %d than the maximal number %d.\n",
                         ace_format_id, BCM_FIELD_NUMBER_OF_ACTIONS_PER_GROUP);
        }
        /** For each action array map each DNX action to BCM action. */
        SHR_IF_ERR_EXIT(dnx_field_map_action_dnx_to_bcm(unit, dnx_field_stage,
                                                        dnx_ace_full_info.
                                                        format_basic_info.dnx_actions[dnx_action_iter],
                                                        &(ace_format_info->action_types[dnx_action_iter])));
        ace_format_info->action_with_valid_bit[dnx_action_iter] =
            ((dnx_ace_full_info.format_basic_info.use_valid_bit[dnx_action_iter]) ? (TRUE) : (FALSE));
    }
    ...
    ace_format_info->nof_actions = dnx_action_iter;
    ...
    bcm_field_ace_format_t ace_format_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_ace_format_delete_verify(unit, ace_format_id));
    ...
    uint32 *entry_handle)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_ace_entry_add_verify(unit, flags, ace_format_id, entry_info, entry_handle));
    ...
    if (flags & BCM_FIELD_FLAG_WITH_ID)
    {
        dnx_flags |= DNX_FIELD_ENTRY_ACE_ADD_FLAG_WITH_ID;
    }
    if (flags & BCM_FIELD_FLAG_UPDATE)
    {
        dnx_flags |= DNX_FIELD_ENTRY_ACE_ADD_FLAG_UPDATE;
    }
    ...
    bcm_field_ace_entry_info_t * entry_info)
{
    ...
    bcm_field_ace_entry_info_t_init(entry_info);
    ...
    bcm_field_context_param_info_t * context_params)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_context_param_set_verify(unit, flags, bcm_stage, context_id, context_params));
    ...
    bcm_field_fem_action_info_t * fem_action_info)
{
    ...
    priority_is_position = DNX_FIELD_ACTION_PRIORITY_IS_POSITION(action_priority);
    if ((priority_is_position == FALSE) && (action_priority != BCM_FIELD_ACTION_DONT_CARE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "action_priority (0x%x) must represent position, as encoded by BCM_FIELD_ACTION_POSITION(), "
                     "or be given BCM_FIELD_ACTION_DONT_CARE, in which case a position will be selected for it.\r\n",
                     action_priority);
    }

    /*
     * If a position is given, verify that position.
     */
    if (priority_is_position)
    {
    ...
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "SFG for FEM is not supported! overriding_fg_id cannot be configured\r\n");
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_field_fem_action_info_t * fem_action_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_fem_action_add_verify(unit, flags, fg_id, action_priority, fem_action_info));
    SHR_ALLOC_SET_ZERO(fem_condition_entry_p, sizeof(*fem_condition_entry_p) * dnx_data_field.stage.stage_info_get(unit,
                                                                                                                   DNX_FIELD_STAGE_IPMF1)->nof_fem_condition,
                       "fem_condition_entry_p", "%s%s%s", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO(fem_encoded_actions_p,
    ...
    SHR_ALLOC_SET_ZERO(fem_action_entry_p,
    ...
    SHR_IF_ERR_EXIT(dnx_field_fem_action_add
                    (unit, action_priority, dnx_fg_id, fem_action_info->fem_input.input_offset, input_size,
    ...
    bcm_field_fem_action_info_t * fem_action_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_fem_action_info_get_verify(unit, fg_id, action_priority, fem_action_info));
    ...
    SHR_IF_ERR_EXIT(FEM_INFO_FG_ID_INFO.input_offset.get(unit, dnx_fem_id, fem_program, &input_offset));
    {
        ...
        {
            /*
             * Shift the offsets by VMV.
             */
            exem_shift = template_shift;
        }
    }
    fem_action_info->fem_input.input_offset = input_offset;
    {
         ...
         *    This is read directly from HW.
         */
        fem_action_info->condition_msb = condition_ms_bit - exem_shift;
        for (fem_condition_index = 0; fem_condition_index < fem_condition_max; fem_condition_index++)
        {
                                ...
                                output_bit[bit_in_fem_action].source_type)
                        {
                            case bcmFieldFemExtractionOutputSourceTypeKey:
                            {
                                ...
                                fem_action_info->fem_extraction[fem_map_index].output_bit[bit_in_fem_action].offset =
                                    ...
                                    fem_bit_value.bit_on_key_select - exem_shift;
                                break;
                            }
                            case bcmFieldFemExtractionOutputSourceTypeForce:
                            {
    ...
    bcm_field_efes_action_info_t * efes_action_info)
{
    ...
    for (fes_2msb_comb = 0; fes_2msb_comb < BCM_FIELD_NUMBER_OF_CONDITIONS_PER_EFES; fes_2msb_comb++)
    {
        if (efes_action_info->efes_condition_conf[fes_2msb_comb].is_action_valid)
        {
            ...
            if ((int) (efes_action_info->efes_condition_conf[fes_2msb_comb].action_type) == BCM_FIELD_INVALID)
            {
    ...
    bcm_field_efes_action_info_t * efes_action_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_efes_action_add_verify
    ...
    bcm_field_presel_entry_data_t * entry_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_presel_set_verify(unit, flags, entry_id, entry_data));
    ...
    bcm_field_range_qual_info_t * range_qualifier_info)
{
    ...
    /** Validate input parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_field_range_type_config_verify(unit, flags, stage, configurable_range_index));

    dnx_flags = 0;

    SHR_IF_ERR_EXIT(dnx_field_range_type_qual_info_t_init(unit, &dnx_qual_info));

    /*
     * Map BCM qual type/info to DNX counterparts
     */
    SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_qual_with_attach_info_bcm_to_dnx
                             (unit, DNX_FIELD_STAGE_L4_OPS, range_qualifier_info->qual_type,
                              &(range_qualifier_info->qual_info), &(dnx_qual_info.dnx_qual),
                              &(dnx_qual_info.qual_info)),
                             "Error for BCM Qualifier at configurable_range_index %d. stage is %s, context is %d.\n",
                             configurable_range_index, dnx_field_bcm_stage_text(stage), context_id);
    ...
    bcm_field_range_qual_info_t * range_qualifier_info)
{
     ...
     */
    bcm_field_range_qualifier_info_t_init(range_qualifier_info);
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_qual_with_attach_info_dnx_to_bcm
                    (unit, DNX_FIELD_STAGE_L4_OPS, dnx_qual_info.dnx_qual,
                     &(dnx_qual_info.qual_info),
                     &(range_qualifier_info->qual_type), &(range_qualifier_info->qual_info)));
...
/**
 * \brief
 *  This Function verifies input params of bcm_field_group_cache().
 * \param [in] unit -
 *  Device ID
 * \param [in] flags -
 *  Currently not in use!
 * \param [in] fg_id -
 *  Field Group id to set mode to
 * \param [in] mode -
 *  Cache mode of the field group
 * \return
 *  int - Error Type, in sense of bcm_error_t, same as
 *  shr_error_t
 * \remark
...
/**
 * \brief
 *  This Function is used to set cache mode for TCAM entries.
 * \param [in] unit -
 *  Device ID
 * \param [in] flags -
 *  Currently not in use!
 * \param [in] fg_id -
 *  Field Group ID to set cache mode to.
 * \param [in] mode -
 *  Cache mode of field group entries (see bcm_field_group_cache_mode_t in field.h).
 * \return
 *  int - Error Type, in sense of bcm_error_t, same as
 *  shr_error_t
 * \remark
 ...
 */
int
bcm_dnx_field_group_cache(
    int unit,
    uint32 flags,
    bcm_field_group_t fg_id,
    bcm_field_group_cache_mode_t mode)
{
    dnx_field_group_cache_mode_e dnx_mode;
    dnx_field_group_t dnx_fg_id;
    uint32 dnx_context_flags;
    ...
    /** Type cast the fg_id to be aligned to DNX type. */
    dnx_fg_id = (dnx_field_group_t) fg_id;
    SHR_IF_ERR_EXIT(dnx_field_map_cache_mode_bcm_to_dnx(unit, mode, &dnx_mode));

    dnx_context_flags = flags;

    /** Call DNX-level API */
    SHR_IF_ERR_EXIT(dnx_field_group_cache(unit, dnx_context_flags, dnx_fg_id, dnx_mode));
exit:
    DNX_ERR_RECOVERY_END(unit);
    SHR_FUNC_EXIT;
}
    ...
    int *offset)
{
    ...
    switch (compare_operand)
    {
        ...
        {
            *offset = dnx_data_field.compare.equal_offset_get(unit);
            break;
        }
        case bcmFieldCompareOperandNotEqual:
        {
            *offset = dnx_data_field.compare.not_equal_offset_get(unit);
            break;
        }
        case bcmFieldCompareOperandFirstKeySmaller:
        {
            *offset = dnx_data_field.compare.smaller_offset_get(unit);
            break;
        }
        case bcmFieldCompareOperandFirstKeyNotSmaller:
        {
            *offset = dnx_data_field.compare.not_smaller_offset_get(unit);
            break;
        }
        case bcmFieldCompareOperandFirstKeyBigger:
        {
            *offset = dnx_data_field.compare.bigger_offset_get(unit);
            break;
        }
        case bcmFieldCompareOperandFirstKeyNotBigger:
        {
            *offset = dnx_data_field.compare.not_bigger_offset_get(unit);
            break;
        }
        default:
        {
    ...
    uint32 ids[BCM_FIELD_NAME_TO_ID_MAX_IDS])
{
    ...
    if (((int) (name_to_id_info->name_to_id_type) < 0) || (name_to_id_info->name_to_id_type >= bcmFieldNameToIdCount))
    {
    ...
    uint32 ids[BCM_FIELD_NAME_TO_ID_MAX_IDS])
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_name_to_id_verify(unit, flags, name_to_id_info, nof_ids, ids));
    ...
    bcm_field_flush_entry_info_t * flush_entry_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_entry_add_verify(unit, flags, fg_id, entry_id, flush_entry_info));
    ...
    bcm_field_flush_entry_info_t * flush_entry_info)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_entry_get_verify(unit, flags, fg_id, entry_id, flush_entry_info));
    ...
    int entry_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_field_flush_entry_delete_verify(unit, flags, fg_id, entry_id));
...
/**
 * \brief
 *  Verify function for bcm_dnx_field_tcam_bank_status_get().
 * \param [in] unit            - Device ID
 * \param [in] core            - Core ID to get the status for (Must be specified)
 * \param [in] bank_id         - Bank ID to get the status for
 *
 * \return
 *  int - Error Type, in sense of bcm_error_t, same as shr_error_t
 * \remark
 ...
 */
static shr_error_e
bcm_dnx_field_tcam_bank_status_get_verify(
    int unit,
    bcm_core_t core,
    uint32 bank_id)
{
    ...
    /*
     * Core id should be specific core and not BCM_CORE_ALL
     */
    DNXCMN_CORE_VALIDATE(unit, core, FALSE);

    /** bank_id cannot exceed maximum number of banks in BCM*/
    if (bank_id >= BCM_FIELD_NUMBER_OF_TCAM_BANKS)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "bank_id is out of range.\n");
    }
...
/**
 * \brief
 *  Returns the list of field groups/PP APPs along with their entry count on the given
 *  TCAM bank for the given core, also returns the number of available entries per key-size
 *  on the given TCAM bank for the given core
 * \param [in] unit            - Device ID
 * \param [in] core            - Core ID to get the status for
 * \param [in] bank_id         - Bank ID to get the status for
 * \param [out] bank_info      - Bank status info for the given bank on the given core
 *
 * \return
 *  int - Error Type, in sense of bcm_error_t, same as shr_error_t
 * \remark
 ...
 */
int
bcm_dnx_field_tcam_bank_status_get(
    int unit,
    bcm_core_t core,
    uint32 bank_id,
    bcm_field_tcam_bank_status_info_t * bank_info)
{
    ...
    uint32 pp_app_iter = 0;
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);

    SHR_INVOKE_VERIFY_DNXC(bcm_dnx_field_tcam_bank_status_get_verify(unit, core, bank_id));

    SHR_IF_ERR_EXIT(dnx_field_group_tcam_bank_status_get
                    (unit, core, bank_id, bank_info->fg_ids, bank_info->nof_fg_entries));

    for (pp_app = bcmFieldAppDbFirst; pp_app < bcmFieldAppDbCount; pp_app++)
    {
        ...
        /*
         * Retrieve the PP APP's DBAL table ID, check if active, then retrieve the TCAM handler ID from the table, afterwards add the
         * returned TCAM handler ID to the given banks.
         */
        dbal_table = (dnx_data_field_map.tcam.tcam_pp_app_get(unit, pp_app)->dbal_table);
        ...
        bank_info->nof_pp_app_entries[pp_app_iter] = bank_entries_count[bank_id];
        bank_info->pp_app_ids[pp_app_iter++] = pp_app;
    }

    bank_info->pp_app_ids[pp_app_iter] = bcmFieldAppDbInvalid;

    /*
     * Fill in nof_free entries 
     */
    SHR_IF_ERR_EXIT(dnx_field_tcam_bank_available_entries_get
                    (unit, core, bank_id, DNX_FIELD_TCAM_ENTRY_SIZE_HALF, &bank_info->nof_free_half_entries));
    SHR_IF_ERR_EXIT(dnx_field_tcam_bank_available_entries_get
                    (unit, core, bank_id, DNX_FIELD_TCAM_ENTRY_SIZE_SINGLE, &bank_info->nof_free_single_entries));
    SHR_IF_ERR_EXIT(dnx_field_tcam_bank_available_entries_get
                    (unit, core, bank_id, DNX_FIELD_TCAM_ENTRY_SIZE_DOUBLE, &bank_info->nof_free_double_entries));
