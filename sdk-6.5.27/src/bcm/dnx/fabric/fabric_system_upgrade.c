    ...
    uint32 *frac)
{
    ...
    COMPILER_UINT64 frac_0, frac_x, frac_x2;

    device_core_clock_khz = DNXCMN_CORE_CLOCK_KHZ_GET(unit);
    *intg = target / device_core_clock_khz;

    COMPILER_64_SET(frac_0, 0, 0);
    COMPILER_64_SET(frac_x, 0, 0);

    COMPILER_64_SET(frac_x, 0, target);
    COMPILER_64_UMUL_32(frac_x, (1 << 19));

    COMPILER_64_COPY(frac_0, frac_x);
    UTILEX_64_UDIV_32(frac_0, device_core_clock_khz);

    if (device_core_clock_khz <= target)
    {
        COMPILER_64_SET(frac_x2, 0, *intg);
        COMPILER_64_UMUL_32(frac_x2, (1 << 19));

        COMPILER_64_SUB_64(frac_0, frac_x2);
    }
    *frac = COMPILER_64_LO(frac_0);
}
/**
 * \brief
 ...
 * \param [in] unit         - The unit number.
 * \param [in] flags        - Supported flags - 0
                                                BCM_FABRIC_SYSTEM_UPGRADE_FIRST_ITERATION
                                                BCM_FABRIC_SYSTEM_UPGRADE_REPEAT_ITERATION
 * \param [in] master_id    - An arbitrary device ID which is not used anywhere else in the system.
 * \param [in] target       - The target for system upgrade (bcmFabricSystemUpgradeTarget1)
 * \return
 *   See \ref shr_error_e
 */
int
    ...
    bcm_fabric_system_upgrade_target_t target)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_fabric_system_upgrade_phase1_verify(unit, flags, &master_id, &target, 1));
    ...
    bcm_fabric_system_upgrade_target_t * target)
{
    ...
    /** Verify input parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_fabric_system_upgrade_phase1_verify(unit, flags, master_id, target, 0));
 ...
 * \param [in] unit         - unit
 * \param [in] flags        - flags (unused)
 * \param [out] status      - The status of the system upgrade status, could be:
                                bcmFabricSystemUpgradeStatusError
                                bcmFabricSystemUpgradeStatusInProgress
                                bcmFabricSystemUpgradeStatusOk
                                bcmFabricSystemUpgradeStatusComplete
 * \return
 *   See \ref shr_error_e
 */
int
