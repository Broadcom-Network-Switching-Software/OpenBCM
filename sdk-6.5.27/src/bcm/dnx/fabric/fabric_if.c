    ...
    bcm_port_t logical_port)
{
    ...
    if (!(soc_sand_is_emulation_system(unit) && DNX_FABRIC_FMAC_EMULATION_NOT_SUPPORTED(unit)))
    {
        ...
        /** 
         * Turn on link pump when leaky bucket is down if needed. Feature mainly design for FEs
         */
        if (dnx_data_fabric.links.feature_get(unit, dnx_data_fabric_links_fmac_tx_pump_disable))
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_LINK_PUMP, INST_SINGLE, 0);
        }
        else
        {
    ...
    int *counter_interval)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_mib_counter_pbmp_get(unit, &counter_pbmp_sw));
    BCM_PBMP_OR(counter_pbmp_sw, *pbmp);
    SHR_IF_ERR_EXIT(dnx_algo_mib_counter_pbmp_set(unit, counter_pbmp_sw));

    counter_journal_entry.is_add = 1;
    counter_journal_entry.is_start_thread = 0;

    SHR_IF_ERR_EXIT(dnx_generic_state_journal_log_entry(unit,
                                                        (uint8 *) &counter_journal_entry,
                                                        sizeof(dnx_fabric_port_er_counter_journal_t),
                                                        (uint8 *) pbmp,
                                                        sizeof(bcm_pbmp_t),
                                                        &dnx_fabric_port_er_counter_rollback, FALSE));
    ...
    int *counter_interval)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_mib_counter_pbmp_get(unit, &counter_pbmp_sw));
    BCM_PBMP_REMOVE(counter_pbmp_sw, *pbmp);
    SHR_IF_ERR_EXIT(dnx_algo_mib_counter_pbmp_set(unit, counter_pbmp_sw));
    ...
    int counter_interval)
{
     ...
     * Get counter status
     */
    SHR_IF_ERR_EXIT(soc_counter_status(unit, &counter_flags, &counter_interval_tmp, &counter_pbmp));
    if (counter_interval > 0)
    {
        DNX_ERR_RECOVERY_SUPPRESS(unit);
...
/** See .h file */
shr_error_e
dnx_fabric_if_fmac_bmp_to_fsrd_bmp_convert(
    int unit,
    bcm_pbmp_t fmac_bmp,
    bcm_pbmp_t * fsrd_bmp,
    int return_only_full_fsrd)
{
    ...
    int fmac_id = 0, fsrd_id = 0, nof_fmacs_in_fsrd;
    int nof_fmacs_counted_for_fsrd[DNX_DATA_MAX_FABRIC_BLOCKS_NOF_INSTANCES_FSRD] = { 0 };

    SHR_FUNC_INIT_VARS(unit);

    BCM_PBMP_CLEAR(*fsrd_bmp);

    nof_fmacs_in_fsrd = dnx_data_fabric.blocks.nof_fmacs_in_fsrd_get(unit);

    BCM_PBMP_ITER(fmac_bmp, fmac_id)
    {
        if (fmac_id >= dnx_data_fabric.blocks.nof_instances_fmac_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Invalid fmac id provided  %d", fmac_id);
        }
        fsrd_id = fmac_id / nof_fmacs_in_fsrd;
        BCM_PBMP_PORT_ADD(*fsrd_bmp, fsrd_id);
        nof_fmacs_counted_for_fsrd[fsrd_id]++;
    }

    if (return_only_full_fsrd)
    {
        BCM_PBMP_ITER(*fsrd_bmp, fsrd_id)
        {
            /*
             * coverity explanation: BCM_PBMP_ITER teration index
             * will be never bigger max nof fsrd blocks.
             */
             /* coverity[Out-of-bounds read:FALSE]  */
            if (nof_fmacs_counted_for_fsrd[fsrd_id] != nof_fmacs_in_fsrd)
            {
                BCM_PBMP_PORT_REMOVE(*fsrd_bmp, fsrd_id);
            }
        }
    }
...
/** Power up all relevant fabric blocks */
static shr_error_e
dnx_fabric_if_port_blocks_power_up(
    int unit,
    bcm_pbmp_t pbmp_to_probe)
{
    bcm_pbmp_t sfi_pbmp, *pbmp_p, fmacs_to_be_enabled_bmp;
    ...
    /**
     * Enable FDR and FDTL dynamically if we about to add the first links
     * Only relevant for dynamic procedures
     */
    if (_SHR_PBMP_IS_NULL(sfi_pbmp))
    {
        if (dnx_data_fabric.general.connect_mode_get(unit) == DNX_FABRIC_CONNECT_MODE_SINGLE_FAP)
        {
            LOG_WARN(BSL_LOG_MODULE,
                     (BSL_META_U
                      (unit,
                       "Probing fabric links under stand alone FAP mode is not advised. There is no guarantee that the Fabric module will function properly!\n")));
        }

        /**
         * Note that powering up dynamically Fabric blocks doesn't mean that the fabric will be fully opeartional 
         * As a lot of the path configurations are set during init based on soc property
         */
        if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_clock_power_down))
        {
            /** Power up fabric blocks if dynamically all were detached and now we start to add back*/
            SHR_IF_ERR_EXIT(dnx_fabric_blocks_power_mode_set(unit, DNX_FABRIC_POWER_MODE_ON));
        }

    }

    /**
     * Power up the relevant FMACs and FSRDs for dynamic procedures
     */
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_advanced_power_down))
    {
        SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fmacs_pbmp_to_be_enabled_get
                        (unit, pbmp_to_probe, &fmacs_to_be_enabled_bmp));

        if (BCM_PBMP_NOT_NULL(fmacs_to_be_enabled_bmp))
        {
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_power_set(unit, fmacs_to_be_enabled_bmp, 1));
        }
    }
     ...
     */
    SHR_IF_ERR_EXIT(dnx_fabric_if_fsrd_fmac_last_in_chain_set(unit));

exit:
    SHR_FUNC_EXIT;
}

/*Power down all relevant fabric blocks*/
static shr_error_e
dnx_fabric_if_port_blocks_power_down(
    int unit)
{
    bcm_pbmp_t *pbmp_p;
    bcm_pbmp_t sfi_pbmp, fmacs_to_be_disabled_bmp;
    ...
    /**
     * Power down the relevant FMACs and FSRDs for dynamic procedures
     */
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_advanced_power_down))
    {
        SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fmacs_pbmp_to_be_disabled_get(unit, sfi_pbmp, &fmacs_to_be_disabled_bmp));

        if (BCM_PBMP_NOT_NULL(fmacs_to_be_disabled_bmp))
        {
            /**
             * Try to disable all provided FMACs and relevant FSRD
             * Note that all provided FMACs may not be powered down as some of them might be disabled only in per 8 lanes resolution 
             * or some other feature requires power up state
             */
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_power_set(unit, fmacs_to_be_disabled_bmp, 0));
        }
    }
    ...
    {
        /*
         * Check if dynamically all ports are beeing detached
         */
        if (_SHR_PBMP_IS_NULL(sfi_pbmp))
        {
            /** Power down unused fabric blocks if dynamically all links were removed*/
            SHR_IF_ERR_EXIT(dnx_fabric_blocks_power_mode_set(unit, DNX_FABRIC_POWER_MODE_SA));
        }
    }
    ...
    bcm_pbmp_t * okay_pbmp)
{
    ...
    /**
     * Power all relevant Fabric blocks for dynamic procedures
     * All relevant blocks were powered up during init inside dnx_fabric_if_fmac_fsrd_blocks_power_up_init or in access step
     */
    if (!is_init_sequence)
    {
        SHR_IF_ERR_EXIT(dnx_fabric_if_port_blocks_power_up(unit, pbmp));
    }
    ...
    if (!is_init_sequence)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_fabric_if_port_counter_bitmap_add(unit, okay_pbmp, &counter_interval));
        /*
         * Start counter thread
         */
        SHR_IF_ERR_EXIT(dnx_fabric_if_port_counter_thread_start(unit, counter_interval));
        /*
    ...
    bcm_pbmp_t * detached)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_fabric_if_port_counter_bitmap_remove(unit, &pbmp, &counter_interval));
    /*
    ...
    PBMP_ITER(pbmp, logical_port)
    {
        ...
        /*
         * Remove port from SW port mgmt DB
         */
    ...
    /**
     * Power down the relevant Fabric blocks like FMACs and FSRDs for dynamic procedures
     * After this step non of the port relevant blocks should be access as they will be powered down
     * Should happen after the SFI bitmap is updated so the blocks to be powered down can be calculated properly
     */
    SHR_IF_ERR_EXIT(dnx_fabric_if_port_blocks_power_down(unit));
...
/** See .h file */
shr_error_e
dnx_fabric_if_supported_fmacs_get(
    int unit,
    bcm_pbmp_t * supported_fmacs)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_fabric_pbmp_to_fmac_bmp_convert(unit, supported_links, supported_fmacs));
...
/**
 * \brief
 *   Returns a pbmp of supported fsrds
 */
shr_error_e
    ...
    bcm_pbmp_t * supported_fsrds)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fabric_if_supported_fmacs_get(unit, &supported_fmacs));

    SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_bmp_to_fsrd_bmp_convert(unit, supported_fmacs, supported_fsrds, 0));
 ...
 */
shr_error_e
dnx_fabric_if_fsrd_fmac_bmp_get(
    int unit,
    int fsrd_block,
    bcm_pbmp_t * fmac_bmp)
{
    int fmac_block;
    int first_fmac_block = fsrd_block * dnx_data_fabric.blocks.nof_fmacs_in_fsrd_get(unit);

    SHR_FUNC_INIT_VARS(unit);

    BCM_PBMP_CLEAR(*fmac_bmp);
    for (fmac_block = first_fmac_block;
         fmac_block < first_fmac_block + dnx_data_fabric.blocks.nof_fmacs_in_fsrd_get(unit); fmac_block++)
    {
        BCM_PBMP_PORT_ADD(*fmac_bmp, fmac_block);
    }
...
/**
 * \brief
 *   Get if a FSRD's clock is powered up.
 * \param [in] unit -
 *   The unit number.
 * \param [in] fsrd_id -
 *   FSRD block id
    ...
    int *enable)
{
     ...
     * request for value field
     */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_ENABLE, INST_SINGLE, &val);
    ...
    int enable)
{
    ...
    int is_master_fsrd_valid = 0, is_master_fsrd_enabled = 0;
     ...
     */
    SHR_IF_ERR_EXIT(soc_dnxc_fabric_fsrd_block_valid_get(unit, master_fsrd_block, &is_master_fsrd_valid));

    SHR_IF_ERR_EXIT(soc_dnxc_fabric_supported_lanes_get(unit, &supported_lanes));

    if (BCM_PBMP_MEMBER(supported_lanes, master_fsrd_block * dnx_data_fabric.blocks.nof_links_in_fsrd_get(unit)))
    {
        /**
         * Enable access to the master FSRD
         */
        if (!is_master_fsrd_valid)
        {
            SHR_IF_ERR_EXIT(soc_dnxc_fabric_fsrd_block_valid_set(unit, master_fsrd_block, 1));
        }

        SHR_IF_ERR_EXIT(dnx_fabric_if_fsrd_clock_power_get(unit, master_fsrd_block, &is_master_fsrd_enabled));

        if (!is_master_fsrd_enabled)
        {
            SHR_IF_ERR_EXIT(dnx_fabric_if_fsrd_clock_power_set(unit, master_fsrd_block, 1));
        }
    }
exit:
    SHR_FUNC_EXIT;
...
/**
 * \brief
 *   Power up/down a FMAC8.
 *   Accessed with FSRD ID as in SW point of view FMAC has 4 and FSRD 8 lanes
 * \param [in] unit -
 *   The unit number.
 * \param [in] fsrd_block -
 *   FSRD block id
    ...
    int enable)
{
    ...
    /** Power up/down FMAC8 block (we use FSRD as index as in SW FMAC is per 4 lanes but in power down blocks is per FMAC8-2xFMAC4)*/
...
/** See .h file */
shr_error_e
dnx_fabric_if_fmac_fsrd_block_power_set(
    int unit,
    bcm_pbmp_t fmacs_bmp,
    int power_up)
{
    int fmac_block = 0, fsrd_block = 0;
    bcm_pbmp_t fsrds_bmp;
    int return_only_full_fsrds = 0;
    SHR_FUNC_INIT_VARS(unit);

    /*
     * When power up return FSRDs even if only 1 FMAC from it is going to be enabled
     * When power down return FSRDs only if all of its FMACs are about to ot be used
     */
    return_only_full_fsrds = power_up ? 0 : 1;
    SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_bmp_to_fsrd_bmp_convert(unit, fmacs_bmp, &fsrds_bmp, return_only_full_fsrds));

    /*
     * FMACs should be disabled/enabled only in per 8 lane resolution as the FSRD Therefore remodel the FMACs bitmap 
     */
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_advanced_power_down))
    {
        BCM_PBMP_CLEAR(fmacs_bmp);
        BCM_PBMP_ITER(fsrds_bmp, fsrd_block)
        {
            int first_fmac = fsrd_block * dnx_data_fabric.blocks.nof_fmacs_in_fsrd_get(unit);
            int last_fmac = first_fmac + dnx_data_fabric.blocks.nof_fmacs_in_fsrd_get(unit);
            for (fmac_block = first_fmac; fmac_block < last_fmac; fmac_block++)
            {
                BCM_PBMP_PORT_ADD(fmacs_bmp, fmac_block);
            }
        }
    }

    /*
     * Power down FMAC (put in soft reset) and mark blocks valid invalid
     * Should happen before power down per FMAC8
     */
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_blocks_reset_required_before_power_down))
    {
        if (!power_up)
        {
            BCM_PBMP_ITER(fmacs_bmp, fmac_block)
            {
                /** Put FMACs in reset before power down */
                SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_reset(unit, fmac_block, 1));
            }
        }
    }

    /**
     * Power up/down FSRD/FMAC8
     */
    BCM_PBMP_ITER(fsrds_bmp, fsrd_block)
    {
        if (power_up)
        {
            SHR_IF_ERR_EXIT(soc_dnxc_fabric_fsrd_block_valid_set(unit, fsrd_block, power_up));

            if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fsrd_clock_power_down))
            {
        ...
        /** Power up/down FMAC8 block (we use FSRD as index as in SW FMAC is per 4 lanes but in power down blocks is per FMAC8-2xFMAC4)*/
        SHR_IF_ERR_EXIT(dnx_fabric_if_fmac8_power_set(unit, fsrd_block, power_up));
    ...
    if (power_up)
    {
        BCM_PBMP_ITER(fmacs_bmp, fmac_block)
        {
            SHR_IF_ERR_EXIT(soc_dnxc_fabric_fmac_block_valid_set(unit, fmac_block, power_up));

            SHR_IF_ERR_EXIT(dnx_fabric_if_mac_init(unit, fmac_block));
        }
    }

    /*
     * Power up FMAC (put out of soft reset)
     * Should happen after power down up FMAC8
     */
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_blocks_reset_required_before_power_down))
    {
        if (power_up)
        {
            BCM_PBMP_ITER(fmacs_bmp, fmac_block)
            {
                SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_reset(unit, fmac_block, 0));
            }
        }
    }
...
/** See .h file */
shr_error_e
dnx_fabric_if_fmac_fsrd_blocks_power_up_init(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);

    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_advanced_power_down)
        || !dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_is_used))
    {
        /*
         * Do nothing!
         * All supported FMACs and FSRDs are enabled in the access step
         */
    }
    else
    {
        ...
        /*
         * Power up all FMAC and FSRD blocks according to their supported bitmap
         * Actually only FMACs are powered up here and the FSRDs are only marked as enabled in SW state.
         * The actual FSRD power up happens in the PM code
         */
        if (BCM_PBMP_NOT_NULL(supported_fmacs))
        {
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_power_set(unit, supported_fmacs, 1));
        }
    }
...
/** See .h file */
shr_error_e
dnx_fabric_if_fmac_fsrd_blocks_power_down_init(
    int unit)
{
    int fmac_block = 0;
    bcm_pbmp_t fmacs_to_be_powered_up = { {0} }, fmacs_to_be_disabled =
    {
        {
    0}};
    int nof_fmacs = dnx_data_fabric.blocks.nof_instances_fmac_get(unit);

    SHR_FUNC_INIT_VARS(unit);

    /**
     * If not using fabric no FMACs should be powered up
     */
    if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_is_used))
    {
        /**
         * Either we convert the provided by the user pbmp, assuming that it is already validated that is within the valid range
         * Or we return all of the supported lanes based on SKU properties and if FAP is in SA mode
         */
        if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_fabric_advanced_power_down))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_fabric_pbmp_to_fmac_bmp_convert
                            (unit, fabric_ports_bitmap, &fmacs_to_be_powered_up));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_fabric_if_supported_fmacs_get(unit, &fmacs_to_be_powered_up));
        }
    }

    for (fmac_block = 0; fmac_block < nof_fmacs; ++fmac_block)
    {
        if (!BCM_PBMP_MEMBER(fmacs_to_be_powered_up, fmac_block))
        {
            BCM_PBMP_PORT_ADD(fmacs_to_be_disabled, fmac_block);
        }
    }

    /*
     * Power up down unsupported or unused FMACs and FSRDs
     */
    if (BCM_PBMP_NOT_NULL(fmacs_to_be_disabled))
    {
        SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_power_set(unit, fmacs_to_be_disabled, 0));
    }
...
/** See .h file */
shr_error_e
dnx_fabric_if_fsrd_fmac_last_in_chain_set(
    int unit)
{
    uint32 entry_handle_id;
    int blk = -1;
    ...
    {
        /*
         *  The last FSRD and FMAC in the SBUS chain are handled via access, so this procedure can be skipped
         */
        SHR_EXIT();
    }
#endif
    ...
    for (fsrd = 0; fsrd < nof_fsrds; fsrd++)
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnxc_fabric_fsrd_block_valid_get(unit, fsrd, &enable));
        if (enable)
        {
    ...
    for (fmac = 0; fmac < nof_fmacs; fmac++)
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnxc_fabric_fmac_block_valid_get(unit, fmac, &enable));
        if (enable)
        {
...
/** See .h file */
shr_error_e
dnx_fabric_if_force_traffic_fabric_set(
    int unit,
    int enable)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_RANGE_VERIFY(enable, 0, 1, _SHR_E_PARAM, "Unsupported enable value %d", enable);

    SHR_IF_ERR_EXIT(dnx_force_traffic_fabric(unit, enable));
    /*
     * Always keep force on local path disabled when enabling force on fabric. 
     */
...
/** See .h file */
shr_error_e
dnx_fabric_if_force_traffic_fabric_get(
    int unit,
    int *enable)
{
    uint32 entry_handle_id;
    ...
    int core)
{
    ...
    SHR_IF_ERR_EXIT(dnx_force_traffic_local_access(unit, core));
    ...
    uint32 *errored_token_count)
{
    ...
    *link_status = 0;
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_RX_SIGNAL_LOCK, INST_SINGLE, &rx_signal_lock);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    if (dnx_data_fabric.links.feature_get(unit, dnx_data_fabric_links_serdes_lock_determines_link_status))
    {
        /**
         * Due to Hardware some devices may wrongly indicate link up (no error) on some statuses when the link is physically disconnected.
         * The following indications are affected: Misalign, Size Error, Code Group Error, CRC Error.
         * For these devices we only allow the statuses to output no error when the serdes is locked. 
         * In other words, if serdes has an error (no lock or no sig accept), then the Misalign, Size Error, Code Group Error, CRC Error
         * Indications also output an error.
         */
        serdes_err = (rx_signal_lock & sig_acc) ? 0 : 1;
    }

    if (crc_error || serdes_err)
    {
        *link_status |= BCM_FABRIC_LINK_STATUS_CRC_ERROR;
    }
    if (size_error || serdes_err)
    {
        *link_status |= BCM_FABRIC_LINK_STATUS_SIZE_ERROR;
    }
    if (misalign || serdes_err)
    {
        *link_status |= BCM_FABRIC_LINK_STATUS_MISALIGN;
    }
    if (code_group_error || serdes_err)
    {
