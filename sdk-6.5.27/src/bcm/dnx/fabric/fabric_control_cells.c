    ...
    int is_soft_reset)
{
    ...
    {
        /**
         * Get which FMACs are currently valid and enabled and toggle the cell operation only on them
         */
        SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fmacs_enabled_get(unit, &enabled_fmacs));
    }
    ...
    if (!(soc_sand_is_emulation_system(unit) && DNX_FABRIC_FMAC_EMULATION_NOT_SUPPORTED(unit)))
    {
        ...
        for (fmac_index = 0; fmac_index < dnx_data_fabric.blocks.nof_instances_fmac_get(unit); ++fmac_index)
        {
            if (_SHR_PBMP_MEMBER(enabled_fmacs, fmac_index))
            {
                int base_link_id = fmac_index * nof_links_in_fmac;
