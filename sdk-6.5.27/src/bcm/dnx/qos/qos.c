    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        {
            if (dnx_data_qos.feature.feature_get(unit, dnx_data_qos_feature_ingress_mpls_swap_qos_ttl_model))
            {
                SHR_ERR_EXIT(_SHR_E_DISABLED,
                             "This control type is invalid since feature 'mpls_swap_qos_ttl_model' is supported\n");
            }
        ...
        case bcmQosControlMplsELIIngressQosProfile:
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "ing_map[%x] is not a valid ingress QoS map ID!\n", map_id);
            }
            SHR_IF_ERR_EXIT(dnx_qos_map_profile_verify(unit, map_id));
            break;
        }
        ...
        case bcmQosControlMplsELIIngressTtlModel:
        {
            ...
            qos_model = arg;
            if ((qos_model < bcmQosIngressModelInvalid) || (qos_model > bcmQosIngressModelUniform))
        ...
        case bcmQosControlMplsPhpTtlUseMin:
        {
            if (!dnx_data_qos.feature.feature_get(unit, dnx_data_qos_feature_ttl_uniform_small))
            {
                SHR_ERR_EXIT(_SHR_E_DISABLED,
                             "This control type is only supported for feature 'uniform if small otherwise pipe'\n");
            }
            break;
        }
        default:
    ...
    *map_id = profile_id;
    ...
    if (flags & BCM_QOS_MAP_INGRESS)
    {
        if (flags & BCM_QOS_MAP_OPCODE)
        {
            if ((flags & BCM_QOS_MAP_PHB) && (flags & BCM_QOS_MAP_REMARK))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "flags BCM_QOS_MAP_PHB and BCM_QOS_MAP_REMARK must be set separately\n");
            }
           /** set map opcode */
            DNX_QOS_OPCODE_MAP_SET(*map_id);
        }
        ...
        {
            if (((flags & BCM_QOS_MAP_PHB) && !(flags & BCM_QOS_MAP_REMARK)) ||
                (!(flags & BCM_QOS_MAP_PHB) && (flags & BCM_QOS_MAP_REMARK)))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "flags BCM_QOS_MAP_PHB and BCM_QOS_MAP_REMARK must be set at same time\n");
            }
        }
        if (flags & BCM_QOS_MAP_L2_VLAN_PCP)
        {
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmQosControlMplsELIIngressTtlModel:
        {
            SHR_IF_ERR_EXIT(dnx_qos_ingress_control_special_label_qos_attributes_set(unit, flags, type, arg));
            break;
        }
        case bcmQosControlMplsIngressPopQoSPreserve:
        {
    ...
    int *arg)
{
    ...
    switch (type)
    {
        ...
        case bcmQosControlMplsELIIngressTtlModel:
        {
            SHR_IF_ERR_EXIT(dnx_qos_ingress_control_special_label_qos_attributes_get(unit, flags, type, arg));
            break;
        }
        case bcmQosControlMplsIngressPopQoSPreserve:
        {
