    ...
    int *is_p2p)
{
    ...
    *is_p2p = FALSE;
    ...
    {
        if (service_type == DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_P2P)
        {
            *is_p2p = TRUE;
        }
    }
    ...
    bcm_gport_t gport2)
{
    ...
    if (gport2 != 0)
    {
        ...
        if (_SHR_GPORT_IS_FORWARD_PORT(gport2))
        {
            uint32 fec_ecmp = BCM_GPORT_FORWARD_PORT_GET(gport2);
            uint8 is_allocated = FALSE;
            /** Verify gport2 FEC/ECMP is created and valid */
            if (fec_ecmp < dnx_data_l3.fec.first_valid_fec_ecmp_id_get(unit))
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "Invalid configuration. The forward port is not in the valid range.");
            }
            SHR_IF_ERR_EXIT(dnx_algo_l3_fec_is_allocated(unit, fec_ecmp, &is_allocated));
            if (is_allocated == FALSE)
            {
                SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_is_allocated(unit, fec_ecmp, &is_allocated));
            }
            if (is_allocated == FALSE)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "Invalid configuration. The forward gport is not an existing destination.");
            }
        }
            ...
            (_SHR_GPORT_IS_EXTENDER_PORT(gport2)))
        {
            ...
            rv = algo_gpm_gport_l2_forward_info_get(unit, gport, 0, &forward_info2);
    ...
    bcm_vswitch_cross_connect_t * gports)
{
        ...
        && (is_valid_dest == FALSE))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, gports->port2, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
                         &gport));
    }
    ...
    if ((_SHR_IS_FLAG_SET(gports->flags, BCM_VSWITCH_CROSS_CONNECT_DIRECTIONAL) == FALSE))
    {
        ...
        {
            SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                            (unit, gports->port1, BCM_GPORT_TYPE_FLOW_LIF,
    ...
    bcm_vswitch_flexible_connect_fwd_t * port_fwd)
{
    ...
    if ((!BCM_GPORT_IS_MPLS_PORT(port_match->match_port) && !BCM_GPORT_IS_VLAN_PORT(port_match->match_port)
         && !BCM_GPORT_IS_TUNNEL(port_match->match_port))
        || (BCM_GPORT_SUB_TYPE_LIF_EXC_GET(port_match->match_port) == BCM_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The match port(0x%08X) must be an mpls/vlan port with not only egress information!\n",
