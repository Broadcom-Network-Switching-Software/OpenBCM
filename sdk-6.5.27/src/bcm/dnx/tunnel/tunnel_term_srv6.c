 ...
 */
static shr_error_e
dnx_tunnel_terminator_srv6_l2vpn_service_tagged_info_verify(
    int unit,
    bcm_tunnel_terminator_t * info)
{
    SHR_FUNC_INIT_VARS(unit);

    if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED) &&
        (!DNX_TUNNEL_TERM_SRV6_IS_L2_VPN(info->type) ||
         !_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_EXTENDED_TERMINATION)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "BCM_TUNNEL_TERM_SERVICE_TAGGED is allowed only when tunnel type is bcmTunnelTypeEthIn6"
                     " or bcmTunnelTypeEthIn6 with BCM_TUNNEL_TERM_EXTENDED_TERMINATION!");
    }

    if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED_BASED_HEADER_ONLY) &&
        !_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "BCM_TUNNEL_TERM_SERVICE_TAGGED_BASED_HEADER_ONLY is allowed only when BCM_TUNNEL_TERM_SERVICE_TAGGED is set!");
    }

    if (info->nof_service_tags != 0)
    {
        if (!_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "nof_service_tags is allowed only when BCM_TUNNEL_TERM_SERVICE_TAGGED is set!");
        }
        if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED_BASED_HEADER_ONLY))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "nof_service_tags is not allowed only when BCM_TUNNEL_TERM_SERVICE_TAGGED_BASED_HEADER_ONLY is set!");
        }
        if (info->nof_service_tags != 1 && info->nof_service_tags != 2)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "nof_service_tags %d can be set to 1 or 2 only!", info->nof_service_tags);
        }
    }
    else
    {
        if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED) &&
            !_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED_BASED_HEADER_ONLY))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "nof_service_tags should be set to 1 or 2 when BCM_TUNNEL_TERM_SERVICE_TAGGED is set!");
        }
    }
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_l2vpn_service_tagged_info_verify(unit, info));
exit:
    SHR_FUNC_EXIT;
}
...
shr_error_e
dnx_tunnel_terminator_srv6_funct_value_set(
    int unit,
    uint32 funct,
    uint32 dip_idx_type,
    bcm_tunnel_terminator_t * info)
{
    ...
    int funct_nof_bits = 0;
    uint32 start_bit;
    uint32 dip6_long[UTILEX_PP_IPV6_ADDRESS_NOF_UINT32S] = { 0 };
    uint32 dip6_long_mask[UTILEX_PP_IPV6_ADDRESS_NOF_UINT32S] = { 0 };
    uint32 mask_set = 0xffffffff;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_funct_nof_bits(unit, info, &funct_nof_bits));
    start_bit = UTILEX_PP_IPV6_ADDRESS_NOF_BITS - locator_nof_bits - funct_nof_bits;
    SHR_IF_ERR_EXIT(utilex_bitstream_set_field(dip6_long, start_bit, funct_nof_bits, funct));
    SHR_IF_ERR_EXIT(utilex_pp_ipv6_address_long_to_struct(dip6_long, info->dip6));

    SHR_IF_ERR_EXIT(utilex_bitstream_set_field(dip6_long_mask, start_bit, funct_nof_bits, mask_set));
    SHR_IF_ERR_EXIT(utilex_pp_ipv6_address_long_to_struct(dip6_long_mask, info->dip6_mask));
exit:
    SHR_FUNC_EXIT;
}
...
static void
dnx_tunnel_terminator_srv6_service_tagged_info_set(
    bcm_tunnel_terminator_t * info,
    in_lif_profile_info_t * in_lif_profile_info)
{
    if (DNX_TUNNEL_TERM_SRV6_IS_L2_VPN(info->type))
    {
        ...
        if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED))
        {
            ...
            {
                in_lif_profile_info->ingress_fields.pwe_nof_eth_tags = info->nof_service_tags;
            }
        }
        else
        {
            in_lif_profile_info->ingress_fields.pwe_nof_eth_tags = 0;
        }

        if (!_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_SERVICE_TAGGED_PLATFORM_NAMESPACE))
        {
    ...
    in_lif_profile_info_t * in_lif_profile_info)
{
    ...
    dnx_tunnel_terminator_srv6_service_tagged_info_set(info, in_lif_profile_info);
exit:
    SHR_FUNC_EXIT;
}

void
dnx_tunnel_terminator_srv6_l2vpn_in_lif_profile_init(
    bcm_tunnel_terminator_t * info,
    in_lif_profile_info_t * in_lif_profile_info)
{
    if (DNX_TUNNEL_TERM_SRV6_IS_L2_VPN(info->type))
...
static shr_error_e
dnx_tunnel_terminator_srv6_optional_service_tagged_and_gsid_eoc_info_get(
    int unit,
    bcm_tunnel_terminator_t * info,
    uint32 in_lif_profile,
    uint32 *srv6_flags)
{
    ...
    if (DNX_TUNNEL_TERM_SRV6_IS_L2_VPN(info->type))
    {
        ...
        /** Suppose the cs-profile is encoded for L2VPN in this case.*/
        in_lif_profile_info.ingress_fields.cs_in_lif_profile_flags |= DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_L2VPN;
        ...
        {
            info->flags |= BCM_TUNNEL_TERM_SERVICE_TAGGED;
            info->flags |= BCM_TUNNEL_TERM_SERVICE_TAGGED_BASED_HEADER_ONLY;
        }
        else if (in_lif_profile_info.ingress_fields.pwe_nof_eth_tags == ONE_TAG ||
                 in_lif_profile_info.ingress_fields.pwe_nof_eth_tags == TWO_TAGS)
        {
            info->flags |= BCM_TUNNEL_TERM_SERVICE_TAGGED;
            info->nof_service_tags = in_lif_profile_info.ingress_fields.pwe_nof_eth_tags;
        }
        if (!(in_lif_profile_info.ingress_fields.cs_in_lif_profile_flags &
              DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_L2VPN_LIF_SCOPED))
        {
            info->flags |= BCM_TUNNEL_TERM_SERVICE_TAGGED_PLATFORM_NAMESPACE;
        }
    }
    else
    {
    ...
    uint32 *srv6_flags)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_optional_service_tagged_and_gsid_eoc_info_get
                    (unit, info, in_lif_profile, srv6_flags));
exit:
    SHR_FUNC_EXIT;
}
