...
static shr_error_e
dnx_tunnel_term_routing_for_udp_o_tunnel_is_disabled(
    int unit,
    uint8 *routing_for_udp_o_tunnel_is_disabled)
{
    int tunnel_route_disable_bitmap = 0;
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(bcm_dnx_switch_control_get(unit, bcmSwitchTunnelRouteDisable, &tunnel_route_disable_bitmap));
    *routing_for_udp_o_tunnel_is_disabled = tunnel_route_disable_bitmap & BCM_SWITCH_TUNNEL_ROUTE_DISABLE_UDP;
...
static shr_error_e
dnx_tunnel_term_nat_dbal_table_id_get(
    int unit,
    uint32 *table_id)
{
    uint8 routing_for_udp_o_tunnel_is_disabled = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_tunnel_term_routing_for_udp_o_tunnel_is_disabled(unit, &routing_for_udp_o_tunnel_is_disabled));

    if (routing_for_udp_o_tunnel_is_disabled)
    {
        *table_id = DBAL_TABLE_IPV4_TT_MP_EM_FOR_NAT;
    }
    else
    {
    ...
    uint32 *table_id)
{
    ...
    if (DNX_TUNNEL_TERM_IS_IPV4_FOR_NAT(info))
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_term_nat_dbal_table_id_get(unit, table_id));
    }
    else if (DNX_TUNNEL_TERM_IS_IPV4_P2P(info))
    {
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    uint8 is_valid_default_tunnel_id = BCM_GPORT_IS_TUNNEL(info->default_tunnel_id);
...
static shr_error_e
dnx_tunnel_terminator_src_nat_flag_verify(
    int unit,
    bcm_tunnel_terminator_t * info)
{
    SHR_FUNC_INIT_VARS(unit);

    if (info->type != bcmTunnelTypeUdp)
    {
        if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_TUNNEL_SRC_NAT))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "flag BCM_TUNNEL_TERM_TUNNEL_SRC_NAT must be used with bcmTunnelTypeUdp type");
        }
    }
...
static shr_error_e
dnx_tunnel_terminator_dst_nat_flag_verify(
    int unit,
    bcm_tunnel_terminator_t * info)
{
    SHR_FUNC_INIT_VARS(unit);

    if (info->type != bcmTunnelTypeIpAnyIn4)
    {
        if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_TUNNEL_DST_NAT))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "flag BCM_TUNNEL_TERM_TUNNEL_DST_NAT must be used with bcmTunnelTypeIpAnyIn4 type");
        }
    }
...
static shr_error_e
dnx_tunnel_terminator_nat_flag_verify(
    int unit,
    bcm_tunnel_terminator_t * info)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_src_nat_flag_verify(unit, info));
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_dst_nat_flag_verify(unit, info));
exit:
    SHR_FUNC_EXIT;
}

static shr_error_e
dnx_tunnel_terminator_service_type_flag_verify(
    int unit,
    bcm_tunnel_terminator_t * info)
{
    int nof_enabled_service_type_flags = 0;
    uint32 service_tag_flags =
        (BCM_TUNNEL_TERM_TUNNEL_SRC_NAT | BCM_TUNNEL_TERM_TUNNEL_DST_NAT | BCM_TUNNEL_TERM_CROSS_CONNECT);
    uint32 enabled_service_tag_flag = (info->flags & service_tag_flags);
    SHR_FUNC_INIT_VARS(unit);

    SHR_BITCOUNT_RANGE(&enabled_service_tag_flag, nof_enabled_service_type_flags, 0, SAL_UINT32_NOF_BITS);
    if (nof_enabled_service_type_flags > 1)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "only one of those flags can be enabled: BCM_TUNNEL_TERM_TUNNEL_SRC_NAT,  BCM_TUNNEL_TERM_TUNNEL_DST_NAT,  BCM_TUNNEL_TERM_CROSS_CONNECT");
    }
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_nat_flag_verify(unit, info));

    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_service_type_flag_verify(unit, info));
    ...
    {
        if (DNX_TUNNEL_TERM_IS_IPV4_TCAM(info))
        {
            if ((info->priority == DNX_FIELD_TCAM_ENTRY_PRIORITY_INVALID
                 || info->priority >= dnx_data_field.tcam.max_tcam_priority_get(unit)))
                SHR_ERR_EXIT(_SHR_E_PARAM,
...
static void
dnx_tunnel_gre_terminator_type_to_ipv4_layer_qualifier_tunnel_type(
    bcm_tunnel_terminator_t * info,
    uint32 *ipv4_layer_qualifier_tunnel_type)
{
    uint8 gre_with_sn = _SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_GRE_WITH_SN);
    uint8 gre_with_lb = _SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_GRE_KEY_USE_LB);

    if (gre_with_sn && gre_with_lb)
    {
        *ipv4_layer_qualifier_tunnel_type = DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GRE12;
    }
    else if (gre_with_sn)
    {
        *ipv4_layer_qualifier_tunnel_type = DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GRE8_WITH_SN;
    }
    else if (gre_with_lb)
    {
        *ipv4_layer_qualifier_tunnel_type = DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GRE8_WITH_KEY;
    }
    else
    {
    ...
    uint32 *ipv4_layer_qualifier_tunnel_type)
{
    ...
    {
        case bcmTunnelTypeGreAnyIn4:
            dnx_tunnel_gre_terminator_type_to_ipv4_layer_qualifier_tunnel_type(info, ipv4_layer_qualifier_tunnel_type);
            break;
        case bcmTunnelTypeIpAnyIn4:
        ...
        case bcmTunnelTypeUdp:
            ...
            else if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_TUNNEL_SRC_NAT))
            {
                *ipv4_layer_qualifier_tunnel_type = DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_TUNNELED_NAT;
            }
            else
            {
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    switch (ipv4_layer_qualifier_tunnel_type)
    {
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GRE4:
            ...
            break;
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GRE8_WITH_SN:
            info->type = bcmTunnelTypeGreAnyIn4;
            info->flags |= BCM_TUNNEL_TERM_GRE_WITH_SN;
            break;
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GRE8_WITH_KEY:
            info->type = bcmTunnelTypeGreAnyIn4;
            info->flags |= BCM_TUNNEL_TERM_GRE_KEY_USE_LB;
            break;
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GRE12:
            info->type = bcmTunnelTypeGreAnyIn4;
            info->flags |= (BCM_TUNNEL_TERM_GRE_KEY_USE_LB | BCM_TUNNEL_TERM_GRE_WITH_SN);
            break;
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_RAW:
            info->type = bcmTunnelTypeIpAnyIn4;
            break;
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_UDP:
            info->type = bcmTunnelTypeUdp;
            break;
        ...
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GTPUOUDP:
            ...
            info->udp_dst_port = DNX_TUNNEL_TERM_GPTV1U_UDP_DST_PORT;
            break;
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_TUNNELED_NAT:
            info->type = bcmTunnelTypeUdp;
            break;
        ...
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_VXLAN_GPEOUDP:
            info->type = bcmTunnelTypeVxlanGpe;
            break;
        case DBAL_ENUM_FVAL_IPV4_QUALIFIER_TUNNEL_TYPE_GENEVEOUDP:
            info->type = bcmTunnelTypeGeneve;
            break;
        default:
    ...
    bcm_vlan_t default_vsi)
{
     ...
     */
    else if (DNX_TUNNEL_TYPE_IS_VXLAN_GPE(info->type) || DNX_TUNNEL_TYPE_IS_GENEVE(info->type))
    {
        if (default_vsi)
        {
            /*
    ...
    else if (DNX_TUNNEL_TERM_IS_IPV4_FOR_NAT(info))
    {
        /*
         * for NAT, where the lif is resolved only to update service type, we don't want to update the VRF. So we'll
         * set in the lif the vrf used in match 
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRF, INST_SINGLE, info->vrf);
    }
    ...
    else if (DNX_TUNNEL_TYPE_IS_VXLAN(info->type) && (default_vsi))
    {
    ...
    uint32 *local_inlif)
{
    ...
    /** set tunneled dest nat */
    if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_TUNNEL_SRC_NAT))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SERVICE_TYPE, INST_SINGLE,
                                     DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_TUNNEL_SRC_NAT);
    }
    else if (_SHR_IS_FLAG_SET(info->flags, BCM_TUNNEL_TERM_TUNNEL_DST_NAT))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SERVICE_TYPE, INST_SINGLE,
                                     DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_TUNNEL_DST_NAT);
    }
    ...
    dnx_tunnel_term_inlif_info_t * tunnel_term_inlif_info)
{
                    ...
                    (unit, _SHR_CORE_ALL, local_inlif, entry_handle_id, &lif_table_mngr_inlif_info));
    {
        ...
        if (is_field_valid == TRUE)
        {
            ...
            else if (vtt_lif_service_type == DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_TUNNEL_DST_NAT)
            {
                tunnel_term_inlif_info->flags |= BCM_TUNNEL_TERM_TUNNEL_DST_NAT;
            }
            else if (vtt_lif_service_type == DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_TUNNEL_SRC_NAT)
            {
                tunnel_term_inlif_info->flags |= BCM_TUNNEL_TERM_TUNNEL_SRC_NAT;
            }
        }
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    else
    {
        ...
        if (DNX_TUNNEL_TERM_SRV6_NEED_IN_LIF_PROFILE_UPDATE(info, in_lif_profile_info))
        {
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ipv4_layer_qualifier_tunnel_type_to_tunnel_terminator_type
                    (unit, ipv4_tunnel_type_qualifier, info));
