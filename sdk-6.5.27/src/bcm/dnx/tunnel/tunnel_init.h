...
/**
  * \brief
 * This function verifies SVTAG related input for tunnel initiator API
 * \param [in] unit -
 *     The unit number.
 * \param [in] type - tunnel type
 * \param [in] udp_dst_port - destination udp port
  *\param [in] udp_src_port - source udp port
 */
shr_error_e dnx_tunnel_initiator_create_verify_svtag(
    int unit,
