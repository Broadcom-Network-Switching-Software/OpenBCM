...
/**
 * \brief - check if port priority propagation is supported
 */
static shr_error_e
dnx_scheduler_port_priority_propagation_supported(
    int unit,
    bcm_port_t port,
    int *is_supported)
{
    ...
    *is_supported = FALSE;
    ...
    if (dnx_data_sch.general.feature_get(unit, dnx_data_sch_general_priority_propagation_on_channelized_if_only))
    {
        /** When this feature is on , we only support priority propagation on the channelized interfaces*/
...
/**
 * \brief - check if fc is supported only on channelized interfaces
 */
static shr_error_e
dnx_scheduler_port_fc_only_on_ch_supported(
    int unit,
    bcm_port_t port,
    int *is_supported)
{
    ...
    *is_supported = FALSE;
    ...
    if (dnx_data_sch.general.feature_get(unit, dnx_data_sch_general_fc_on_channelized_if_only))
    {
        /** When this feature is on , we only support priority propagation on the channelized interfaces*/
        SHR_IF_ERR_EXIT(dnx_algo_port_is_channelized_get(unit, port, &is_channelized));
        if (is_channelized)
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_E2E_SCH(unit, port_info))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_scheduler_port_priority_propagation_supported(unit, port, &is_supported));
        if (is_supported)
        {
        ...
        /** set default TC to TCG mapping */
        if (!dnx_data_sch.general.feature_get(unit, dnx_data_sch_general_tcg_mapping_hardcoded))
        {
            ...
            == DNX_SCHEDULER_PORT_PRIORITY_PROPAGATION_V1)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_scheduler_port_single_if_rate_hw_set(unit, port, FALSE, FALSE));
        }
    }
...
/**
 * \brief - Choose according to the feature if the mapping is done with qpair or with flow control context
 */
static shr_error_e
dnx_scheduler_port_flow_control_map_set_init(
    int unit,
    bcm_port_t src_port,
...
/**
 * \brief - Choose according to the feature if the clearing is done with qpair or with flow control context
 */
static shr_error_e
dnx_scheduler_port_flow_control_map_clear_init(
    int unit,
    bcm_port_t port)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_port_fc_only_on_ch_supported(unit, port, &is_fc_supported));
    ...
    {
        /** map q-pairs to HRs */
        SHR_IF_ERR_EXIT(dnx_scheduler_port_flow_control_map_set_init(unit, port, port));

        SHR_IF_ERR_EXIT(dnx_algo_port_master_get(unit, port, DNX_ALGO_PORT_MASTER_F_SCH, &master_port));

        if ((port == master_port) && is_fc_supported)
        {
    ...
    bcm_port_t port)
{
    ...
    {
        /**
         * Get HR and Core from logical port
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, port, &core));

        SHR_IF_ERR_EXIT(dnx_algo_port_base_hr_get(unit, port, &base_hr));

        SHR_IF_ERR_EXIT(dnx_algo_port_base_hr_ref_count_get(unit, core, base_hr, &base_hr_ref_count));
        if (base_hr_ref_count == 1)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_master_get(unit, port,
            ...
            if (next_master_port == DNX_ALGO_PORT_INVALID)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_scheduler_port_fc_only_on_ch_supported(unit, port, &is_fc_supported));
                if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info) && is_fc_supported)
                {
            ...
            {
                /** Configure interface calendar */
                SHR_IF_ERR_EXIT(dnx_scheduler_port_single_if_rate_hw_set(unit, port, TRUE, FALSE));
            }
        }
        ...
        if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
        {
            ...
            /** UnMap PP DSP to HR*/
            SHR_IF_ERR_EXIT(dnx_sch_dbal_port_to_base_hr_mapping_clear(unit, core, pp_dsp));
        }
    ...
    int unit)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_port_flow_control_map_clear_init(unit, port));
    }
    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
       ...
       /** This function is not invoked under warm boot */
        SHR_IF_ERR_EXIT(dnx_scheduler_flow_allocate
                        (unit, core, nof_remote_cores, DNX_ALGO_RES_ALLOCATE_WITH_ID, FALSE, FALSE,
                         DNX_COSQ_RESERVED_FLOW_NOF, DNX_SCH_ALLOC_FLOW_TYPE_CONNECTOR,
                         DNX_SCHEDULER_SHARED_SHAPER_ORDER_NONE, src_modid, &reserved_flow_id));
    }
    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        SHR_IF_ERR_EXIT(dnx_sch_convert_se_to_flow_id(unit, core, default_se_id, &default_se_flow_id));

        /** additional SE is allocated to hookup objects just created */
        bcm_cosq_scheduler_gport_params_t_init(&params);
        params.core = core;
    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        SHR_IF_ERR_EXIT(dnx_sch_convert_se_to_flow_id(unit, core, default_se_id, &default_se_flow_id[0]));
        SHR_IF_ERR_EXIT(dnx_sch_flow_shaper_token_count_set(unit, core, default_se_flow_id, 1, 0, 0));
    }
    ...
    int *is_reserved)
{
    ...
    *is_reserved = FALSE;
    ...
    if (is_se)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core, flow_id, &se_id));
    ...
    int unit)
{
    ...
    if (hr != -1)
    {
        ...
        {
            /** se_id to flow id */
            SHR_IF_ERR_EXIT(dnx_sch_convert_se_to_flow_id(unit, core, se_id, &flow_id));
...
/**
 * \brief -
 * initialized non_default_flow_order swstate value
 */
static shr_error_e
dnx_sch_swstate_non_default_flow_order_init(
    int unit)
{
    int core;
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        /** assume default flow order */
        SHR_IF_ERR_EXIT(dnx_scheduler_db.general.non_default_flow_order.set(unit, core, 0));

        if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_flex_quartet_supported))
        {
            group_start =
                DNX_SCH_QUARTET_GROUP_INDEX_FROM_REGION_INDEX_GET(dnx_data_sch.
                                                                  flow.nof_connectors_only_regions_get(unit));
            ...
            for (group_index = group_start; group_index < group_end; group_index++)
            {
                ...
                {
                    /** non-default flow order found */
                    SHR_IF_ERR_EXIT(dnx_scheduler_db.general.non_default_flow_order.set(unit, core, 1));
                    break;
                }
            }
        }
...
/**
 * \brief -
 * initialized is_composite_fq_supported swstate value
 */
static shr_error_e
dnx_sch_swstate_is_composite_fq_supported_init(
    int unit)
{
    int core;
    int group_start, group_end;
    ...
    {
        /** assume default flow order */
        SHR_IF_ERR_EXIT(dnx_scheduler_db.general.is_composite_fq_supported.set(unit, core, 0));
        ...
        for (group_index = group_start; group_index < group_end; group_index++)
        {
            ...
            /**
             * CL-FQ-CL-FQ and CL-CL-FQ-FQ do not support composite FQ.
             * The rest of the quratet types support composite FQ.
             */
            if ((quartet_type != DNX_SCH_FLOW_QUARTET_TYPE_CL_FQ_CL_FQ)
                && ((quartet_type != DNX_SCH_FLOW_QUARTET_TYPE_CL_CL_FQ_FQ)))
            {
                /** non-default flow order found */
                SHR_IF_ERR_EXIT(dnx_scheduler_db.general.is_composite_fq_supported.set(unit, core, 1));
                break;
            }
        }
    }
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_db.port_shaper.alloc(unit));
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_flow_virtual.alloc(unit));
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_flow_virtual.alloc_bitmap(unit, core));
    }

    SHR_IF_ERR_EXIT(dnx_sch_swstate_non_default_flow_order_init(unit));
    SHR_IF_ERR_EXIT(dnx_sch_swstate_is_composite_fq_supported_init(unit));
    SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.alloc(unit));
    ...
    int unit)
{
    ...
    {
        if (dnx_data_sch.general.feature_get(unit, dnx_data_sch_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_sch_power_down_set(unit, 1));
        }
    }
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
             ...
             region_indx < dnx_data_sch.flow.nof_regions_get(unit); region_indx++)
        {
            ...
            for (i = 0; i < nof_se_in_quartet; i++)
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core, region_indx * region_size + i, &se_id_start));
                SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id
                                (unit, core, (region_indx + 1) * region_size - 4 + i, &se_id_end));
    ...
    int unit)
{
    ...
    if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_flex_quartet_supported))
    {
        /** configure quartet order */
        SHR_IF_ERR_EXIT(dnx_scheduler_region_quartet_order_init(unit));
    }
    ...
    /** configure dummy PP DSP and map it to reserved HR*/
    if (dnx_data_port.general.reserved_pp_dsp_get(unit) != -1)
    {
        SHR_IF_ERR_EXIT(dnx_sch_dbal_dummy_pp_dsp_set(unit, dnx_data_port.general.reserved_pp_dsp_get(unit)));
        SHR_IF_ERR_EXIT(dnx_sch_dbal_port_to_base_hr_mapping_set(unit, BCM_CORE_ALL,
                                                                 dnx_data_port.general.reserved_pp_dsp_get(unit),
...
dnx_scheduler_connector_flow_id_calc(
    ...
    int called_by_egress,
    int flow[2],
    int *nof_subflows)
{
    int flow_region = 0;
    dnx_scheduler_region_odd_even_mode_t odd_even_mode = DNX_SCHEDULER_REGION_ODD;
    int factor = 1;
    ...
    else
    {
        ...
        if (is_non_contiguous && is_composite && odd_even_mode == FALSE && called_by_egress)
        {
    ...
    bcm_gport_t * gport)
{
    ...
    uint32 allowed_flags =
        ...
        BCM_COSQ_VOQ_CONNECTOR_GPORT_F_RESERVED | BCM_COSQ_VOQ_CONNECTOR_GPORT_F_VIRTUAL;
    ...
    if ((flags & BCM_COSQ_VOQ_CONNECTOR_GPORT_F_NON_CONTIGUOUS) &&
        !dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_interdigitated_supported))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Device does not support interdigiated regions\n");
    }

    if ((flags & BCM_COSQ_VOQ_CONNECTOR_GPORT_F_LOW_RATE) &&
        dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_multi_low_rate_ranges))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Device does not support BCM_COSQ_VOQ_CONNECTOR_GPORT_F_LOW_RATE flag,\n"
                     "For low rate VOQ connector please use BCM_COSQ_VOQ_CONNECTOR_GPORT_F_WITH_ID\n");
    }

    /** verify if provided flow id is correct*/
    if (flags & BCM_COSQ_VOQ_CONNECTOR_GPORT_F_WITH_ID)
    {
        flow_id = BCM_COSQ_GPORT_VOQ_CONNECTOR_ID_GET(*gport);
        if (flow_id < 0 || flow_id > (dnx_data_sch.flow.nof_flows_get(unit) - 1))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Provided flow id is not in valid range\n");
        }
    }

    /** verify if we can allocate virtual flows*/
    if (flags & BCM_COSQ_VOQ_CONNECTOR_GPORT_F_VIRTUAL)
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_enabled.get(unit, &is_enabled));
        if (is_enabled == SCH_VIRTUAL_FLOWS_DISABLED)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Since base VOQ %d is mapped to a VOQ connector, virtual flows are disabled\n",
                         dnx_data_sch.flow.virtual_flow_mapped_queue_get(unit));
        }
        if (flags & (BCM_COSQ_VOQ_CONNECTOR_GPORT_F_COMPOSITE | BCM_COSQ_VOQ_CONNECTOR_GPORT_F_NON_CONTIGUOUS))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Some of provided flags are not supported for virtual flows\n");
        }
    }
exit:
    SHR_FUNC_EXIT;
...
/*
 * \brief - add virtual flow
 */
static shr_error_e
dnx_scheduler_connector_gport_virtual_add(
    int unit,
    int core,
    int flow_id,
    int nof_remote_cores,
    ...
    bcm_gport_t * voq_connector_gport)
{
    ...
    /** Enable virtual flows HW*/
    SHR_IF_ERR_EXIT(dnx_sch_dbal_virtual_flow_enable_set(unit, TRUE));

    /** Update SW state , updated only once first time virtual flow is added */
    SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_enabled.get(unit, &is_enabled));
    if (is_enabled == SCH_VIRTUAL_FLOWS_UNINITALIZED)
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_enabled.set(unit, SCH_VIRTUAL_FLOWS_ENABLED));
    }

    /** Generate FSM for each member of the VOQ bundle to activate it
     *  In case of multiple remote cores, active the VOQ bundle on the first core */
    for (cur_connector = 0; cur_connector < numq; cur_connector++)
    {
        actual_flow_id = DNX_SCH_FLOW_ID_FROM_REMOTE_CORE(flow_id + cur_connector, nof_remote_cores, 0);
        SHR_IF_ERR_EXIT(dnx_sch_dbal_manual_fsm_generate
                        (unit, core, actual_flow_id, DBAL_ENUM_FVAL_FLOW_SLOW_STATUS_NORMAL));
        SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_flow_virtual.bit_set(unit, core,
                                                                               actual_flow_id /
                                                                               dnx_data_sch.flow.
                                                                               min_connector_bundle_size_get(unit)));
    }

    /** Map all voq connectors to VOQ 0*/
    sal_memset(&gport_connect, 0, sizeof(bcm_cosq_gport_connection_t));
    gport_connect.flags = BCM_COSQ_GPORT_CONNECTION_EGRESS;
    gport_connect.voq_connector = *voq_connector_gport;
    BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_QUEUE_SET(voq_gport, core,
                                                 dnx_data_sch.flow.virtual_flow_mapped_queue_get(unit));
    gport_connect.voq = voq_gport;
    SHR_IF_ERR_EXIT(dnx_scheduler_connection_set(unit, &gport_connect, TRUE));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_gport_t * gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_connector_gport_add_verify(unit, core, src_modid, nof_remote_cores,
    ...
    if (flags & BCM_COSQ_VOQ_CONNECTOR_GPORT_F_WITH_ID)
    {
        ...
        /** when multi low rate ranges - low rate connector can be allocated with WITH_ID flag only*/
        if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_multi_low_rate_ranges))
        {
            SHR_IF_ERR_EXIT(dnx_sch_flow_is_low_rate_get(unit, core, flow_id, &is_low_rate));
        }
    }
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_flow_allocate
                    (unit, core, nof_remote_cores, res_flags, is_non_contiguous, is_low_rate, numq, flow_type,
                     DNX_SCHEDULER_SHARED_SHAPER_ORDER_NONE, src_modid, &flow_id));
    ...
    /** if connector is allocated as low rate , we need to update the default token count */
    if (is_low_rate)
    {
        ...
        {
            flow_id_i = DNX_SCH_FLOW_ID_FROM_REMOTE_CORE(flow_id, nof_remote_cores, i);
            SHR_IF_ERR_EXIT(dnx_sch_flow_shaper_token_count_set
                            (unit, core, &flow_id_i, 1, rate_factor * dnx_data_sch.dbal.token_count_default_get(unit),
                             0));
        }
    }
    ...
    if (flags & BCM_COSQ_VOQ_CONNECTOR_GPORT_F_VIRTUAL)
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_connector_gport_virtual_add(unit, core, flow_id, nof_remote_cores, numq, gport));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int is_non_contiguous)
{
    ...
    for (cur_connector = 0; cur_connector < numq; cur_connector++)
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_connector_flow_id_calc(unit, core, base_flow_id, cur_connector,
                                                             is_non_contiguous, is_composite, FALSE, TRUE,
    ...
    bcm_gport_t gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_base_flow_id_cosq_verify(unit, core, base_flow_id, 0, TRUE));
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_voq_connector_detached_verify(unit, core, base_flow_id, numq,
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_is_voq_connector_virtual(unit, gport, &is_virtual));
    ...
    for (cur_connector = 0; cur_connector < numq; cur_connector++)
    {
        ...
        for (subflow_indx = 0; subflow_indx < nof_subflows; subflow_indx++)
        {
            ...
            /** If flow is virtual , we also need to send FSM to deactivate it
             *  We need to deactivate only the first bundle, since it is the only activated when virtual flow is created */
            if (is_virtual)
            {
                SHR_IF_ERR_EXIT(dnx_sch_dbal_manual_fsm_generate
                                (unit, core, actual_flow_id[0], DBAL_ENUM_FVAL_FLOW_SLOW_STATUS_OFF));
                SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_flow_virtual.bit_clear(unit, core,
                                                                                         actual_flow_id[i] /
                                                                                         dnx_data_sch.flow.
                                                                                         min_connector_bundle_size_get
                                                                                         (unit)));
            }
        }
    }
    ...
    int *base_flow_id)
{
    ...
    if (BCM_COSQ_GPORT_IS_E2E_PORT(gport) || BCM_COSQ_GPORT_IS_E2E_PORT_TC(gport))
    {
        ...
        else
        {
            /** Port-TC */
            /** validate priority */
            SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_port_priority_verify(unit, logical_port, cosq));
        ...
        SHR_IF_ERR_EXIT(dnx_sch_convert_se_to_flow_id(unit, tmp_core, se_id, base_flow_id));
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_base_flow_id_cosq_verify(unit, *core, *base_flow_id, cosq, is_connector));
    ...
    int *flow_id)
{
    ...
    if (!is_connector)
    {
        ...
        SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_element_shared_shaper_mode_verify(unit, gport, *core, base_flow_id));
    }
    ...
    dnx_scheduler_flow_info_t * flow_info)
{
    ...
    if (BCM_COSQ_GPORT_IS_VOQ_CONNECTOR(gport))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_scheduler_connector_flow_id_calc(unit, *core, base_flow_id, cosq,
                                                             is_interdigitated, is_composite, FALSE, TRUE,
    ...
    int is_parent_se)
{
    ...
    /** get parent SE id */
    SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, parent_core_id, parent_flow_id, &parent_se_id));
    ...
    bcm_cos_queue_t cosq)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_attach_verify(unit, parent_port, input_port, cosq));
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_attach_verify_config(unit, child_core, child_flow_id, is_child_se,
                                                                    core, parent_flow_id, is_parent_se));

    SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core, parent_flow_id, &parent_se_id));
    ...
    int parent_flow_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, parent_core, parent_flow_id, &parent_se_id));
    ...
    bcm_cos_queue_t cosq)
{
    ...
    /** Verify configuration */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_detach_verify_config(unit, child_core, child_flow_id,
                                                                    core, parent_flow_id));
    ...
    bcm_gport_t * parent_port)
{
    ...
    /** Verify child port */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_child_flow_verify(unit, child_port, cos));
    ...
    if (parent_se_id == sw_state_default_se)
    {
        ...
        else
        {
            ...
            if (is_child_se)
            {
                SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core, child_flow_id, &child_se_id));
    ...
    {
        SHR_IF_ERR_EXIT(dnx_sch_convert_se_to_flow_id(unit, core, parent_se_id, &parent_flow_id));
    ...
    int *weight)
{
    ...
    /** Verify child port */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_child_flow_verify(unit, gport, cosq));
    ...
    int weight)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_cl_weight_set_param_verify(unit, port, cosq, weight));

    /** get flow id and core */
    SHR_IF_ERR_EXIT(dnx_scheduler_gport_core_flow_id_get(unit, port, -1, &core, &flow_id));

    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_flow_is_cl_verify(unit, core, flow_id));
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_cl_weight_set_weight_verify(unit, core, cl_profile, weight_ap));
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_allocators_cl_profile_exchange
                    (unit, core, flags, 1, &cl_template_data, cl_profile, NULL, &new_profile, &is_allocated, &is_last));
    ...
    int *weight)
{
    ...
    /** verify parameter */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_cl_weight_param_verify(unit, port, cosq));
    ...
    int slow_rate_id)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_slow_profile_set_verify(unit, port, cosq, slow_rate_id));
    ...
    int *slow_rate_id)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_voq_connector_gport_verify(unit, port, cosq));
    ...
    int max_burst)
{
    ...
    /*
     * verify port and cosq 
     */
    SHR_IF_ERR_EXIT(dnx_scheduler_gport_cosq_verify(unit, port, cosq));

    SHR_IF_ERR_EXIT(dbal_tables_field_max_value_get(unit, DBAL_TABLE_SCH_SHAPER_DESCRIPTOR_TABLE, DBAL_FIELD_MAX_BURST,
                                                    0, 0, 0, &max_field_value));
    ...
    /*
     * Check if max burst is not over max allowed value
     */
    if (max_burst_in_credit_worth > max_field_value)
    {
    ...
    int max_burst)
{
    ...
    int flow_id;
    int real_max_burst = 0;
    int is_under_traffic = 0;
    uint32 under_traffic_mask = BCM_COSQ_CONTROL_BANDWIDTH_BURST_MAX_UNDER_TRAFFIC;
    ...
    SHR_FUNC_INIT_VARS(unit);
    is_under_traffic = (max_burst & under_traffic_mask) ? TRUE : FALSE;
    real_max_burst = (max_burst & ~under_traffic_mask);

    /** Verify parameters*/
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_flow_max_burst_set_verify(unit, port, cosq, real_max_burst));
    ...
    max_burst_in_credit_worth = UTILEX_DIV_ROUND_UP(real_max_burst * token_factor, credit_worth);

    /** 2 flows per remote core are written inside this function for VOQ connector */
    SHR_IF_ERR_EXIT(dnx_sch_flow_shaper_max_burst_set
                    (unit, core, flow_id, is_under_traffic, max_burst_in_credit_worth));
    ...
    int *max_burst)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_cosq_verify(unit, port, cosq));
    ...
    int group)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_se_group_set_verify(unit, port, cosq, group));
    ...
    int *group)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_group_se_gport_verify(unit, port, cosq));
...
dnx_scheduler_fraction_is_bigger(
    ...
    int *is_bigger)
{
    COMPILER_UINT64 val64_1, val64_2;
    ...
    COMPILER_64_SET(val64_1, 0, numerator1);
    COMPILER_64_UMUL_32(val64_1, denominator2);

    /** numerator2*denominator1 */
    COMPILER_64_SET(val64_2, 0, numerator2);
    COMPILER_64_UMUL_32(val64_2, denominator1);

    if (COMPILER_64_GT(val64_1, val64_2))
    {
        *is_bigger = TRUE;
    }
    else
    ...
    uint32 round_up)
{
    ...
    uint32 peak_rate_man_arr[1];

    COMPILER_UINT64 intermediate_numerator;
    ...
    int nof_flipflops;
    uint32 set_bits;
    uint64 max_rate_in_kbits_per_sec;
    ...
    COMPILER_64_ZERO(max_rate_in_kbits_per_sec);
    COMPILER_64_SET(max_rate_in_kbits_per_sec, 0, (credit_worth * UTILEX_NOF_BITS_IN_BYTE));
    COMPILER_64_UMUL_32(max_rate_in_kbits_per_sec, DNXCMN_CORE_CLOCK_KHZ_GET(unit));
    /** Rate should be in [Kbits/sec] */
    UTILEX_64_UDIV_32(max_rate_in_kbits_per_sec, dnx_data_sch.general.cycles_per_credit_token_get(unit));
    ...
    else
    {
        ...
        /*
         * shaper_weight_numerator = max_rate_in_kilo_bits_per_sec * 64 / rate_in_kilo_bits_per_sec, aka
         * shaper_weight_numerator / shaper_weight_denominator = max_rate_in_kilo_bits_per_sec / rate_in_mega_bits_per_sec
         *
         * this is the number we calculate fractional mantissa-exponent representation for
         */

        COMPILER_64_COPY(intermediate_numerator, max_rate_in_kbits_per_sec);
        COMPILER_64_UMUL_32(intermediate_numerator, shaper_weight_denominator);
        UTILEX_64_UDIV_32(intermediate_numerator, rate_in_kbits_per_sec);

        if (COMPILER_64_HI(intermediate_numerator) != 0)
        {
            /*
             * with Jericho 2 typical numbers, we should never get here 
             */

            COMPILER_64_SET(intermediate_numerator, 0, 0xFFFFFFFF);
        }
        shaper_weight_numerator = COMPILER_64_LO(intermediate_numerator);
        ...
        /** use peak_rate_man_arr to make Coverity happy */
        peak_rate_man_arr[0] = peak_rate_man;
        while (peak_rate_exp > (nof_flipflops - mantissa_field_size) &&
               (SHR_BITTEST_RANGE(peak_rate_man_arr, 0, peak_rate_exp - (nof_flipflops - mantissa_field_size),
                                  set_bits)))
        {
            peak_rate_man_arr[0] = (round_up ? UTILEX_DIV_ROUND_UP(peak_rate_man_arr[0], 2) :
                                    peak_rate_man_arr[0] >> 1);
            peak_rate_exp--;
        }

        *rate_man = peak_rate_man_arr[0];
    ...
    uint32 *rate_in_kbits_per_sec)
{
    ...
    UTILEX_64_UDIV_32(u64_calc, (1000 * dnx_data_sch.general.cycles_per_credit_token_get(unit)));
    ...
    else
    {
        ...
        if (shaper_weight)
        {
            ...
            UTILEX_64_UDIV_32(intermediate_val, shaper_weight);
        }
    ...
    uint32 flags)
{
    ...
    /** check if SE is shared shaper*/
    if (BCM_COSQ_GPORT_IS_SHARED_SHAPER_ELEM(gport))
    {
        supported_flags |= BCM_COSQ_BW_SHAPER_DISABLED;
    }
    SHR_MASK_VERIFY(flags, supported_flags, _SHR_E_PARAM, "flag not supported\n");

    /** Verify it is not port-HR */
    if (BCM_GPORT_IS_SCHEDULER(gport))
    {
        ...
        if (is_port_hr)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "port HR %d can't be used for configuring flow shaper\n", flow_id);
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    uint32 flags)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_flow_bandwidth_set_verify(unit, gport, cosq,
                                                                         kbits_sec_min, kbits_sec_max, flags));

    /** get flow id and core */
    SHR_IF_ERR_EXIT(dnx_scheduler_gport_core_flow_id_get(unit, gport, cosq, &core, &flow_id));

    /** If BCM_COSQ_BW_SHAPER_DISABLED is used, directly set mantissa and exponend to 0 */
    if (flags & BCM_COSQ_BW_SHAPER_DISABLED)
    {
        peak_rate_mantissa = 0;
        peak_rate_exp = 0;
    }
    else
    {
    ...
    uint32 *flags)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_flow_bandwidth_get_verify(unit, gport, cosq,
                                                                         kbits_sec_min, kbits_sec_max, flags));
    ...
    /** Check if shaper was disabled. In this case we need to return BCM_COSQ_BW_SHAPER_DISABLED flag*/
    if ((peak_rate_mantissa == 0) && (peak_rate_exp == 0))
    {
        *flags |= BCM_COSQ_BW_SHAPER_DISABLED;
        *kbits_sec_max = peak_rate_mantissa;
    }
    else
    {
    ...
    int flow_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, params->core, flow_id, &se_id));
...
/**
 * \brief - return shared shaper order according to scheduler_gport_add parameters
 * 
 */
static shr_error_e
dnx_scheduler_element_params_to_shared_shaper_order_get(
    int unit,
    bcm_cosq_scheduler_gport_params_t * params,
    ...
    if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_flex_quartet_supported))
    {
        switch (params->mode)
        {
            case bcmCosqSchedulerGportModeSharedDual:
                ...
                switch (params->shared_order)
                {
                    case bcmCosqSchedulerGportSharedOrderNone:
                    case bcmCosqSchedulerGportSharedOrderClFq:
                        *shared_order = DNX_SCHEDULER_SHARED_SHAPER_ORDER_CL_FQ;
                        break;
                    case bcmCosqSchedulerGportSharedOrderClCl:
                        *shared_order = DNX_SCHEDULER_SHARED_SHAPER_ORDER_CL_CL;
                        break;
                    case bcmCosqSchedulerGportSharedOrderFqCl:
                        *shared_order = DNX_SCHEDULER_SHARED_SHAPER_ORDER_FQ_CL;
                        break;
                    default:
                        SHR_ERR_EXIT(_SHR_E_PARAM, "Unexpected shared_order mode %d\n", params->shared_order);
                        break;
                }
                break;
            ...
            case bcmCosqSchedulerGportModeEnhancedClHighPrioFQ:
                *shared_order = DNX_SCHEDULER_SHARED_SHAPER_ORDER_NONE;
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_PARAM, "Unexpected mode %d\n", params->mode);
                break;
        }
    }
    else
...
/**
 * \brief - return scheduler_gport_add shared_order order according to flow type
 *
 */
static shr_error_e
dnx_scheduler_element_flow_type_to_shared_order_params_get(
    int unit,
    int core,
    int flow_id,
    dnx_sch_alloc_flow_type_e flow_type,
    bcm_cosq_scheduler_gport_shared_order_t * shared_order)
{
    dnx_scheduler_shared_shaper_order_t dnx_shared_order = DNX_SCHEDULER_SHARED_SHAPER_ORDER_NONE;
    uint8 non_default_flow_order = 0;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_scheduler_db.general.non_default_flow_order.get(unit, core, &non_default_flow_order));

    if (non_default_flow_order == 1)
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_flow_shared_order_from_flow_type_get
                        (unit, core, flow_type, flow_id, &dnx_shared_order));

        switch (dnx_shared_order)
        {
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_NONE:
                *shared_order = bcmCosqSchedulerGportSharedOrderNone;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_CL_FQ:
                *shared_order = bcmCosqSchedulerGportSharedOrderClFq;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_CL_CL:
                *shared_order = bcmCosqSchedulerGportSharedOrderClCl;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_FQ_CL:
                *shared_order = bcmCosqSchedulerGportSharedOrderFqCl;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_CL_FQ_CL_FQ:
                *shared_order = bcmCosqSchedulerGportSharedOrderClFqClFq;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_CL_CL_FQ_FQ:
                *shared_order = bcmCosqSchedulerGportSharedOrderClClFqFq;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_CL_FQ_FQ_CL:
                *shared_order = bcmCosqSchedulerGportSharedOrderClFqFqCl;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_FQ_CL_FQ_CL:
                *shared_order = bcmCosqSchedulerGportSharedOrderFqClFqCl;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_FQ_FQ_CL_CL:
                *shared_order = bcmCosqSchedulerGportSharedOrderFqFqClCl;
                break;
            case DNX_SCHEDULER_SHARED_SHAPER_ORDER_FQ_CL_CL_FQ:
                *shared_order = bcmCosqSchedulerGportSharedOrderFqClClFq;
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_PARAM, "Unexpected shared order %d\n", dnx_shared_order);
                break;
        }
    }
    else
            ...
            switch (params->mode)
            {
                ...
                case bcmCosqSchedulerGportModeEnhancedClHighPrioFQ:
                    *flow_type = DNX_SCH_ALLOC_FLOW_TYPE_ENHANCED_CL;
                    break;
                case bcmCosqSchedulerGportModeSharedDual:
                    *flow_type = DNX_SCH_ALLOC_FLOW_TYPE_SHARED_SE_2;
                    break;
                case bcmCosqSchedulerGportModeSharedQuad:
...
dnx_scheduler_element_flow_type_to_params_get(
    ...
    int flow_id,
    dnx_sch_alloc_flow_type_e flow_type,
    int specific_element_in_shared_shaper,
    bcm_cosq_scheduler_gport_params_t * params)
{
    dnx_sch_element_se_type_e se_type;
    int cl_flow_id;
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_element_flow_type_to_shared_order_params_get
                    (unit, core, flow_id, flow_type, &params->shared_order));
    ...
    switch (flow_type)
    {
        ...
        case DNX_SCH_ALLOC_FLOW_TYPE_SHARED_SE_8:
            if (!specific_element_in_shared_shaper)
            {
                /*
                 * Note that shared FQ-FQ is currently not supported
                 * If it would be supported, need to handle FQ here
                 */
                switch (params->shared_order)
                {
                    case bcmCosqSchedulerGportSharedOrderFqCl:
                    case bcmCosqSchedulerGportSharedOrderFqClFqCl:
                    case bcmCosqSchedulerGportSharedOrderFqClClFq:
                        cl_flow_id = flow_id + 1;
                        break;
                    case bcmCosqSchedulerGportSharedOrderFqFqClCl:
                        cl_flow_id = flow_id + 2;
                        break;
                    default:
                        cl_flow_id = flow_id;
                        break;
                }
                SHR_IF_ERR_EXIT(dnx_scheduler_element_cl_params_get(unit, core, cl_flow_id, flow_type, params));
            }
            else
            {
    ...
    int *flow_id)
{
    ...
    if (with_id)
    {
        ...
        /** low rate flow can be allocated with WITH_ID flag only*/
        SHR_IF_ERR_EXIT(dnx_sch_flow_is_low_rate_get(unit, core, *flow_id, &is_low_rate));
    }

    SHR_IF_ERR_EXIT(dnx_scheduler_flow_allocate
                    (unit, core, 1, alloc_flags, FALSE, is_low_rate, 1, flow_type, shared_order, -1, flow_id));
    ...
    bcm_gport_t * gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_element_params_to_shared_shaper_order_get(unit, params, &shared_order));
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_element_flow_allocate(unit, core, flow_type, shared_order, with_id, &flow_id));
    ...
    int *hr_id)
{
    ...
    else
    {
        ...
        for (prio = 0; prio < nof_priorities; prio++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_scheduler_element_flow_allocate
                            (unit, core, flow_type, DNX_SCHEDULER_SHARED_SHAPER_ORDER_NONE, TRUE, &flow_id));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int flow_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_flow_real_flow_ids_get(unit, core, 1, flow_type, flow_id,
                                                         allocated_flows, &nof_flows));

    /** Configure only the first element for composite */
    nof_flows = is_composite ? 1 : nof_flows;
    ...
    int flow_id)
{
    ...
    for (i = 0; i < nof_subflows; i++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core, subflow[i], &se_id));
    ...
    int flow_id)
{
    ...
    /** Configure only the first element for composite */
    nof_subflows_to_config = is_composite ? 1 : nof_subflows;

    for (i = 0; i < nof_subflows_to_config; i++)
    {
        /** update flow attributes with default values */
    ...
    is_shared_shaper = (flow_type == DNX_SCH_ALLOC_FLOW_TYPE_SHARED_SE_2 ||
                        flow_type == DNX_SCH_ALLOC_FLOW_TYPE_SHARED_SE_4 ||
                        flow_type == DNX_SCH_ALLOC_FLOW_TYPE_SHARED_SE_8);
    ...
    bcm_gport_t gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_element_delete_verify(unit, core, flow_id));
    ...
    int specific_element_in_shared_shaper)
{
    ...
    /** when flow is not a specific element in shared shaper, we should verify that is the base flow id*/
    if (!specific_element_in_shared_shaper)
    {
        /** determine the allocated flow type */
        SHR_IF_ERR_EXIT(dnx_scheduler_element_flow_type_hw_get(unit, core, flow_id, &flow_type));
        SHR_IF_ERR_EXIT(dnx_scheduler_flow_real_flow_ids_get
                        (unit, core, 1, flow_type, flow_id, subflow, &nof_subflows));
        ...
        switch (flow_type)
        {
            ...
            case DNX_SCH_ALLOC_FLOW_TYPE_SHARED_SE_8:
                if (flow_id % nof_subflows != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Flow %d is not the base flow id in shared shaper\n", flow_id);
                }
                break;
            default:
                break;
    ...
    bcm_cosq_scheduler_gport_params_t * params)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_get_verify(unit, core, flow_id, specific_element_in_shared_shaper));

    /** determine the allocated flow type */
    SHR_IF_ERR_EXIT(dnx_scheduler_element_flow_type_hw_get(unit, core, flow_id, &flow_type));

    SHR_IF_ERR_EXIT(dnx_scheduler_element_flow_type_to_params_get(unit, core, flow_id, flow_type,
                                                                  specific_element_in_shared_shaper, params));
    ...
    bcm_cosq_voq_connector_gport_t * config)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_connector_gport_get_verify(unit, gport, config));
    ...
    if (!dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_multi_low_rate_ranges))
    {
        SHR_IF_ERR_EXIT(dnx_sch_flow_is_low_rate_get(unit, core, base_flow_id, &is_low_rate));
        flags |= (is_low_rate) ? BCM_COSQ_VOQ_CONNECTOR_GPORT_F_LOW_RATE : 0;
    }

    /** determine if connector is virtual flow */
    SHR_IF_ERR_EXIT(dnx_scheduler_is_voq_connector_virtual(unit, gport, &is_virtual));
    flags |= (is_virtual) ? BCM_COSQ_VOQ_CONNECTOR_GPORT_F_VIRTUAL : 0;
    ...
    int weight)
{
    ...
    /** check if TCG to TC mapping feature is supported, if not return an error */
    if (!dnx_data_egr_queuing.params.feature_get(unit, dnx_data_egr_queuing_params_tcg_mapping))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "TCG to TC mapping is not supported on this device\n");
    }
    ...
    int weight)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tc_sched_set_verify(unit, gport, cosq, mode, weight));
    ...
    int *weight)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tc_sched_get_verify(unit, gport, cosq, mode, weight));
    ...
    int weight)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tcg_sched_set_verify(unit, gport, cosq, mode, weight));
    ...
    int *weight)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tcg_sched_get_verify(unit, gport, cosq, mode, weight));
...
/*
 * \ brief -
 * calculate minimum max burst
 */
static int
dnx_scheduler_min_max_burst_calc(
    int unit,
    uint32 quanta,
    uint32 *min_max_burst)
{
    uint32 actual_credit_worth;
    SHR_FUNC_INIT_VARS(unit);

    /** get credit worth*/
    SHR_IF_ERR_EXIT(dnx_sch_port_shaper_credit_divider_get(unit, &actual_credit_worth));

    /** calculate min max burst*/
    *min_max_burst = quanta + dnx_data_sch.ps.assigned_credit_worth_multiplier_get(unit) * actual_credit_worth;
...
/*
 * \ brief -
 * verify if minimal max burst requirement is violated when we set max_burst
 */
static int
dnx_scheduler_min_max_burst_verify(
    int unit,
    bcm_gport_t gport,
    bcm_cos_queue_t cosq,
    int is_tc,
    uint32 max_burst)
{
    ...
    uint32 internal_max_burst, quanta, min_value;
    uint32 min_max_burst = 0;
    ...
    /** if we want to verify the max burst we need to get quanta from HW*/
    if (is_tc)
    {
        SHR_IF_ERR_EXIT(dnx_sch_port_tc_bandwidth_get(unit, core, base_hr + cosq, &quanta));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_sch_port_tcg_bandwidth_get(unit, core, base_hr + cosq, &quanta));
    }

    SHR_IF_ERR_EXIT(dnx_scheduler_port_shaper_max_burst_to_internal(unit, max_burst, &internal_max_burst));

    SHR_IF_ERR_EXIT(dnx_scheduler_min_max_burst_calc(unit, quanta, &min_value));

    /*
     * If restriction is violated , throw an error
     */
    if (internal_max_burst < min_value)
    {
        /** convert max burst internal representation to bytes */
        SHR_IF_ERR_EXIT(dnx_scheduler_port_shaper_max_burst_from_internal(unit, min_value, &min_max_burst));
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid max burst value %u . The minimal max burst size is %u \n",
                     max_burst, min_max_burst);
    }
...
/*
 * \ brief -
 * check if minimal max_burst requirement is violated when we set the rate
 * if it is violated adjust max_burst to minimal value
 */
static int
dnx_scheduler_adjust_min_max_burst(
    int unit,
    bcm_gport_t gport,
    bcm_cos_queue_t cosq,
    int is_tc,
    uint32 kbits_sec_max)
{
    ...
    port_calendar_type = ((is_tc) ? DNX_SCH_PORT_TC_CALENDAR : DNX_SCH_PORT_TCG_CALENDAR);
    ...
    /** if we want to verify the quanta we need to get max burst from HW*/
    if (is_tc)
    {
        SHR_IF_ERR_EXIT(dnx_sch_port_tc_max_burst_get(unit, core, base_hr + cosq, &internal_max_burst));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_sch_port_tcg_max_burst_get(unit, core, base_hr + cosq, &internal_max_burst));
    }

    SHR_IF_ERR_EXIT(dnx_scheduler_port_rate_kbits_per_sec_to_quanta(unit, port_calendar_type, kbits_sec_max, &quanta));

    SHR_IF_ERR_EXIT(dnx_scheduler_min_max_burst_calc(unit, quanta, &min_value));

    /*
     * If restriction is violated, adjust the max burst to the minimal value and print warning
     * When we disable the shaper, we don't check the restriction
     */
    if (internal_max_burst != 0)
    {
        if (internal_max_burst < min_value)
        {
            if (is_tc)
            {
                SHR_IF_ERR_EXIT(dnx_sch_port_tc_max_burst_set(unit, core, base_hr + cosq, min_value));
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_sch_port_tcg_max_burst_set(unit, core, base_hr + cosq, min_value));
            }
            /** convert max burst internal representation to bytes */
            SHR_IF_ERR_EXIT(dnx_scheduler_port_shaper_max_burst_from_internal(unit, min_value, &min_max_burst));

            LOG_WARN(BSL_LOG_MODULE,
                     (BSL_META_U(unit, "warning: the shaper rate defines a minimal value for max burst size. "
                                 "Max burst size was adjusted to the minimal value %u\n"), min_max_burst));
        }
    }
...
/** 
 * \brief -
 * verify if maximal port rate is correct
 */
static int
dnx_scheduler_max_port_rate_verify(
    int unit,
    uint32 kbits_sec_max)
{
    uint64 temp;
    ...
    COMPILER_64_SET(temp, 0, dnx_data_sch.ps.max_port_rate_mbps_get(unit));
    COMPILER_64_UMUL_32(temp, 1000);
    if (COMPILER_64_HI(temp) == 0)
    {
        if (kbits_sec_max > COMPILER_64_LO(temp))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Invalid parameter: kbits_sec_max %u is bigger than maximal port rate %d\nMaximal port rate is determined by maximal_port_bandwidth soc property\n",
                         kbits_sec_max, COMPILER_64_LO(temp));
        }
    }
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_max_port_rate_verify(unit, kbits_sec_max));
    ...
    uint32 flags)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tc_bandwidth_set_verify(unit, gport, cosq,
                                                                                kbits_sec_min, kbits_sec_max, flags));
    /** Check if minimal rate requirement is preserved */
    SHR_IF_ERR_EXIT(dnx_scheduler_adjust_min_max_burst(unit, gport, cosq, TRUE, kbits_sec_max));
    ...
    uint32 *flags)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tc_bandwidth_get_verify(unit, gport, cosq,
                                                                                kbits_sec_min, kbits_sec_max, flags));
    ...
    uint32 flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tcg_bandwidth_set_verify(unit, gport, cosq,
                                                                                 kbits_sec_min, kbits_sec_max, flags));

    /** Check if minimal rate requirement is preserved */
    SHR_IF_ERR_EXIT(dnx_scheduler_adjust_min_max_burst(unit, gport, cosq, FALSE, kbits_sec_max));
    ...
    uint32 *flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tcg_bandwidth_get_verify(unit, gport, cosq,
                                                                                 kbits_sec_min, kbits_sec_max, flags));
    ...
    uint32 max_burst)
{
    ...
    if (max_burst != 0)
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_min_max_burst_verify(unit, gport, cosq, TRUE, max_burst));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int arg)
{
    ...
    switch (type)
    {
        case bcmCosqControlBandwidthBurstMax:
            ...
            /** Validate gport */
            SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tc_max_burst_verify(unit, port, cosq, arg));
        ...
        case bcmCosqControlGroup:
            /** Validate gport */
            SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tc_group_set_verify(unit, port, cosq, arg));
    ...
    int *arg)
{
    ...
    switch (type)
    {
        case bcmCosqControlBandwidthBurstMax:
            ...
            /** Validate gport */
            SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tc_verify(unit, port, cosq));
    ...
    uint32 max_burst)
{
    ...
    if (max_burst != 0)
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_min_max_burst_verify(unit, gport, cosq, FALSE, max_burst));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int arg)
{
    ...
    switch (type)
    {
        case bcmCosqControlBandwidthBurstMax:
            ...
            /** Validate gport */
            SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tcg_max_burst_verify(unit, port, cosq, arg));
    ...
    int *arg)
{
    ...
    switch (type)
    {
        case bcmCosqControlBandwidthBurstMax:
            ...
            /** Validate gport */
            SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_tcg_verify(unit, port, cosq));
    ...
    int arg)
{
    ...
    if (dnx_data_sch.general.feature_get(unit, dnx_data_sch_general_fc_on_channelized_if_only))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Interface priority mapping is not supported on this device \n");
    }
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_interface_priority_mapping_set_verify(unit, gport, cosq, arg));
    ...
    int *arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_interface_priority_mapping_get_verify(unit, gport, cosq, arg));
    ...
    int port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
    {
        ...
        /** Map PP DSP to HR*/
        SHR_IF_ERR_EXIT(dnx_sch_dbal_port_to_base_hr_mapping_set(unit, core, pp_dsp, base_hr));
    }
    ...
    uint32 flags)
{
    ...
    COMPILER_64_SET(temp, 0, dnx_data_sch.interface.max_if_rate_mbps_get(unit));
    COMPILER_64_UMUL_32(temp, 1000);
    if (COMPILER_64_HI(temp) == 0)
    {
        if (kbits_sec_max > COMPILER_64_LO(temp))
    ...
    supported_flags |= BCM_COSQ_BW_NOT_COMMIT | BCM_COSQ_BW_ALT_CALC;

    /** currently only BCM_COSQ_BW_NOT_COMMIT and BCM_COSQ_BW_ALT_CALC flag are supported */
    ...
    uint32 flags)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_bandwidth_set_validate(unit, gport, cosq,
                                                                               kbits_sec_min, kbits_sec_max, flags));
    ...
    if (!priority_propagation_enable ||
        dnx_data_sch.general.port_priority_propagation_version_get(unit) == DNX_SCHEDULER_PORT_PRIORITY_PROPAGATION_V2)
    {
        ...
        {
            /** Configure interface calendar */
            SHR_IF_ERR_EXIT(dnx_scheduler_port_rate_hw_set(unit, (flags & BCM_COSQ_BW_ALT_CALC ? TRUE : FALSE)));
        }
    }
    ...
    uint32 *flags)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_port_bandwidth_get_validate(unit, gport, cosq,
                                                                               kbits_sec_min, kbits_sec_max, flags));
    ...
    uint32 flags)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_interface_bandwidth_set_validate(unit, gport, cosq,
    ...
    uint32 *flags)
{
    ...
    /** Validate */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_gport_e2e_interface_bandwidth_get_validate(unit, gport, cosq,
                                                                                    kbits_sec_min, kbits_sec_max,
                                                                                    flags));
    /** retrieve port */
    SHR_IF_ERR_EXIT(algo_gpm_tm_gport_to_port_get(unit, 0, gport, &logical_port));
    ...
    uint32 *hr_id)
{
    ...
    for (index = 0; index < hr_nof; index++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core, flow_id[index], &se_id));
    ...
    int is_virtual)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_base_flow_id_cosq_verify(unit, egress_core, base_cid, 0, TRUE));
    ...
    {
        /** when is virtual flow, we need to map all voq connectors to VOQ 0*/
        if (is_virtual)
        {
            curr_queue_quad = dnx_data_sch.flow.virtual_flow_mapped_queue_get(unit);
        }
        else
        {
            curr_queue_quad = DNX_COSQ_IPQ_QUEUE_TO_QUAD_GET(base_qid +
                                                             (cur_quad *
                                                              dnx_data_ipq.queues.min_bundle_size_get(unit)));
        }
    ...
    /** check if we need to disable virtual flows
     *  When connector is connected to VOQ 0 , we need to disable virtual flows
     *  updated only once first time when connector is connected to VOQ 0 */
    SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_enabled.get(unit, &is_enabled));
    if ((base_qid == dnx_data_sch.flow.virtual_flow_mapped_queue_get(unit))
        && (is_enabled == SCH_VIRTUAL_FLOWS_UNINITALIZED))
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.is_enabled.set(unit, SCH_VIRTUAL_FLOWS_DISABLED));
    }
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_base_flow_id_cosq_verify(unit, core_id, base_cid, 0, TRUE));
    ...
    bcm_gport_t * gport)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_sched_gport_add_verify(unit, port, numq, flags, gport));
    ...
    bcm_cosq_fc_endpoint_t * source)
{
    ...
    /** source port + cosq */
    if (BCM_GPORT_IS_LOCAL(source->port))
    {
        if (!dnx_data_sch.fc.feature_get(unit, dnx_data_sch_fc_context_based_mapping))
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Port level flow control is not supported on this device \n");
        }
        if ((source->priority != BCM_COSQ_LOW_PRIORITY) && (source->priority != BCM_COSQ_HIGH_PRIORITY))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid priority %d for port level flow control!", source->priority);
        }
    }
    else if (!BCM_COSQ_GPORT_IS_PORT_TC(source->port))
    {
    ...
    int enable)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_internal_fc_map_set_verify(unit, source, target));
    ...
    SHR_IF_ERR_EXIT(dnx_sch_dbal_qpair_to_hr_mapping_set(unit, sch_core, egq_core,
                                                         base_q_pair + source->cosq, base_hr + target->cosq, enable));
    ...
    int *target_count)
{
    ...
    /** verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_internal_fc_map_get_verify(unit, source, target_max, target, target_count));
 ...
 */
static shr_error_e
dnx_scheduler_low_rate_flow_range_set_verify(
    int unit,
    bcm_gport_t gport,
    uint32 flags,
    ...
    bcm_cosq_range_t * range)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_flow_low_rate_range_id_get(unit, type, &new_range_id));

    if (range->is_enabled)
    {
        /** if range is disabled, range limits are ignored and not checked */
        SHR_RANGE_VERIFY(range->range_start, 0, dnx_data_sch.flow.nof_low_rate_regions_get(unit) - 1,
                         _SHR_E_PARAM, "range_start (%d) is out of range (0-%d)\n", range->range_start,
                         dnx_data_sch.flow.nof_low_rate_regions_get(unit) - 1);

        SHR_RANGE_VERIFY(range->range_end, 0, dnx_data_sch.flow.nof_low_rate_regions_get(unit) - 1,
    ...
    DNXCMN_CORES_ITER(unit, core, core_i)
    {
        ...
        if (range->is_enabled)
        {
            /** verify no overlaps with other ranges */
            for (int range_id = 0; range_id < dnx_data_sch.flow.nof_low_rate_ranges_get(unit); range_id++)
            {
                SHR_IF_ERR_EXIT(dnx_sch_dbal_low_rate_flow_range_get
                                (unit, core_i, range_id, &flow_range_start, &flow_range_end, &pattern));
                if ((pattern != 0) && (range_id != new_range_id))
                {
                    ...
                    /** check if overlap */
                    if ((range->range_end >= current_range_start) && (range->range_start <= current_range_end))
                    {
                        SHR_ERR_EXIT(_SHR_E_RESOURCE, "range %d is overlapping range %d)\n", new_range_id, range_id);
                    }
                }
            }

            SHR_IF_ERR_EXIT(dnx_sch_dbal_low_rate_flow_range_get
                            (unit, core_i, new_range_id, &flow_range_start, &flow_range_end, &pattern));

            current_range_start = flow_range_start / dnx_data_sch.flow.region_size_get(unit);
            current_range_end = flow_range_end / dnx_data_sch.flow.region_size_get(unit);
            is_current_enabled = pattern == 0 ? 0 : 1;

            if (is_current_enabled)
            {
            ...
            {
                /** verify no flow is allocated in range */
                SHR_IF_ERR_EXIT(dnx_scheduler_flow_alloc_is_allocated_in_range(unit, core_i,
                                                                               range->range_start, range->range_end,
                                                                               &is_allocated_in_range));
        ...
        else
        {
            ...
            if (is_current_enabled)
            {
                /** verify no flow is allocated in current range */
                SHR_IF_ERR_EXIT(dnx_scheduler_flow_alloc_is_allocated_in_range(unit, core_i,
                                                                               current_range_start, current_range_end,
                                                                               &is_allocated_in_range));
...
/** see header file */
shr_error_e
dnx_scheduler_low_rate_flow_range_set(
    int unit,
    bcm_gport_t gport,
    uint32 flags,
    bcm_cosq_control_range_type_t type,
    bcm_cosq_range_t * range)
{
    bcm_core_t core, core_i;
    int region_size = dnx_data_sch.flow.region_size_get(unit);
    int range_id, flow_start, flow_end;
    uint32 pattern;

    SHR_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_low_rate_flow_range_set_verify(unit, gport, flags, type, range));

    SHR_IF_ERR_EXIT(dnx_sch_flow_low_rate_range_id_get(unit, type, &range_id));
    ...
    DNXCMN_CORES_ITER(unit, core, core_i)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.pattern.get(unit, core_i, range_id, &pattern));
        }
        ...
        SHR_IF_ERR_EXIT(dnx_sch_dbal_low_rate_flow_range_set(unit, core_i, range_id, flow_start, flow_end, pattern));

        /*
         * Update SW state 
         */
        SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.is_enabled.set(unit, core_i, range_id, range->is_enabled));
        SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.range_start.set(unit, core_i, range_id, range->range_start));
        SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.range_end.set(unit, core_i, range_id, range->range_end));
    }
...
/**
 * \brief - Verify function for low rate flow get
 *
 * \param [in] unit - unit ID
 * \param [in] gport - core gport
 * \param [in] flags - flags
 * \param [in] range - range values for low rate flow
 *
 * \return
 *   see shr_error_e
 * \see
 ...
 */
static shr_error_e
dnx_scheduler_low_rate_flow_range_get_verify(
    int unit,
    bcm_gport_t gport,
    uint32 flags,
    bcm_cosq_range_t * range)
...
/** see header file */
shr_error_e
dnx_scheduler_low_rate_flow_range_get(
    int unit,
    bcm_gport_t gport,
    uint32 flags,
    bcm_cosq_control_range_type_t type,
    bcm_cosq_range_t * range)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_low_rate_flow_range_get_verify(unit, gport, flags, range));
    SHR_IF_ERR_EXIT(dnx_sch_flow_low_rate_range_id_get(unit, type, &range_id));

    core = (gport == 0 ? 0 : BCM_COSQ_GPORT_CORE_GET(gport));
    SHR_IF_ERR_EXIT(dnx_sch_dbal_low_rate_flow_range_get
                    (unit, core, range_id, &flow_range_start, &flow_range_end, &pattern));

    range->range_start = flow_range_start / dnx_data_sch.flow.region_size_get(unit);
    range->range_end = flow_range_end / dnx_data_sch.flow.region_size_get(unit);
    range->is_enabled = pattern == 0 ? 0 : 1;
    ...
    int factor)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_flow_is_low_rate_enable_get(unit, &is_enabled));
    ...
    int factor)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_low_rate_factor_set_verify(unit, factor));
    ...
    bcm_cos_queue_t cosq)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_attach_port_to_lag_scheduler_verify(unit, sched_port, input_port, cosq));

   /** retrieve egq port */
    SHR_IF_ERR_EXIT(algo_gpm_tm_gport_to_port_get(unit, 0, input_port, &src_port));
    SHR_IF_ERR_EXIT(dnx_algo_port_pp_dsp_get(unit, src_port, &egq_core, &pp_dsp));

    /** retrieve target port */
    SHR_IF_ERR_EXIT(algo_gpm_tm_gport_to_port_get(unit, 0, sched_port, &target_port));
    SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, target_port, &sch_core));

    /** mark the port is member of LAG  and is on different core than its scheduler */
    SHR_IF_ERR_EXIT(dnx_sch_dbal_lag_port_config_set(unit, egq_core, pp_dsp, TRUE, sch_core));

    /** make default qpair -> HR mapping */
    SHR_IF_ERR_EXIT(dnx_scheduler_port_flow_control_map_clear_init(unit, src_port));
    SHR_IF_ERR_EXIT(dnx_scheduler_port_flow_control_map_set_init(unit, src_port, target_port));
    ...
    bcm_cos_queue_t cosq)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_detach_port_from_lag_scheduler_verify(unit, sched_port, input_port, cosq));
    ...
    /** mark the port is not member of LAG and is not on different core than its scheduler */
    SHR_IF_ERR_EXIT(dnx_sch_dbal_lag_port_config_clear(unit, egq_core, pp_dsp));

    /** change qpair -> HR mapping  to point to src port Scheduler*/
    SHR_IF_ERR_EXIT(dnx_scheduler_port_flow_control_map_clear_init(unit, src_port));
    SHR_IF_ERR_EXIT(dnx_scheduler_port_flow_control_map_set_init(unit, src_port, src_port));

    /** enable interface level Flow Control */
    SHR_IF_ERR_EXIT(dnx_scheduler_port_fc_only_on_ch_supported(unit, src_port, &is_fc_supported));
    if (is_fc_supported)
    {
    ...
    bcm_gport_t * parent_port)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_port_lag_scheduler_get_verify(unit, child_port, cos, parent_port));
...
/**
 * \brief - returns if voq_connector is virtual
 */
shr_error_e
dnx_scheduler_is_voq_connector_virtual(
    int unit,
    bcm_gport_t voq_connector_gport,
    int *is_virtual)
{
    int core, base_flow_id;
    uint8 is_flow_virtual;
    ...
    *is_virtual = FALSE;
    ...
    core = BCM_COSQ_GPORT_VOQ_CONNECTOR_CORE_GET(voq_connector_gport);
    base_flow_id = BCM_COSQ_GPORT_VOQ_CONNECTOR_ID_GET(voq_connector_gport);

    SHR_IF_ERR_EXIT(dnx_scheduler_db.virtual_flows.
                    is_flow_virtual.bit_get(unit, core, base_flow_id / min_bundle_size, &is_flow_virtual));
    if (is_flow_virtual)
    {
        *is_virtual = TRUE;
    }
...
/**
 * \brief - Verify function for low rate pattern get
 *
 * \param [in] unit - unit ID
 * \param [in] flags - sould be 0
 * \param [in] gport - core gport
 * \param [in] key - represent range id
 * \param [in] arg - range id pattern
 *
 * \return
 *   see shr_error_e
 * \see
 ...
 */
static shr_error_e
dnx_scheduler_low_rate_pattern_get_verify(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
    uint32 *arg)
{
    ...
    SHR_NULL_CHECK(arg, _SHR_E_PARAM, "arg");
    SHR_MAX_VERIFY(key, dnx_data_sch.flow.nof_low_rate_ranges_get(unit) - 1, _SHR_E_PARAM, "Key out of bound - %d",
                   key);
    SHR_MASK_VERIFY(flags, 0, _SHR_E_PARAM, "no flags supported\n");

    if (gport != 0 && !BCM_COSQ_GPORT_IS_CORE(gport))
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "Invalid port (%x). Expected gport 0 or core gport only\n", gport);
    }
...
/** see header file */
shr_error_e
dnx_scheduler_low_rate_pattern_get(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
    uint32 *arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_low_rate_pattern_get_verify(unit, flags, gport, key, arg));

    core = (gport == 0 ? 0 : BCM_COSQ_GPORT_CORE_GET(gport));
    SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.pattern.get(unit, core, key, arg));
...
/**
 * \brief - Verify function for low rate pattern set
 *
 * \param [in] unit - unit ID
 * \param [in] flags - sould be 0
 * \param [in] gport - core gport
 ...
 */
static shr_error_e
dnx_scheduler_low_rate_pattern_set_verify(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
    uint32 arg)
{
    ...
    SHR_MAX_VERIFY(key, dnx_data_sch.flow.nof_low_rate_ranges_get(unit) - 1, _SHR_E_PARAM, "Key out of bound - %d",
                   key);
    SHR_MASK_VERIFY(flags, 0, _SHR_E_PARAM, "no flags supported\n");
    SHR_MASK_VERIFY(arg, dnx_data_sch.flow.low_rate_pattern_mask_get(unit), _SHR_E_PARAM,
                    "arg is bigger then the allowed pattern\n");
...
/** see header file */
shr_error_e
dnx_scheduler_low_rate_pattern_set(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
    uint32 arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_low_rate_pattern_set_verify(unit, flags, gport, key, arg));

    core = (gport == 0 ? BCM_CORE_ALL : BCM_COSQ_GPORT_CORE_GET(gport));
    DNXCMN_CORES_ITER(unit, core, core_i)
    {
        SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.pattern.set(unit, core_i, key, arg));
    }
