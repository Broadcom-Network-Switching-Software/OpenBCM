    ...
    )
{
    ...
    else
    {
        ...
        COMPILER_64_SET(calc2, 0, calc);
        COMPILER_64_UMUL_32(calc2, quanta);

        tmp = COMPILER_64_LO(calc2);
        if (COMPILER_64_HI(calc2))
        {
            /** Overflow */
...
/**
 * \brief - get scheduler reserved interface
 */
static shr_error_e
dnx_scheduler_port_sch_reserved_interface_get(
    int unit,
    int *res_sch_if)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot use reserved SCH interface when priority propagation is enabled!\n");
    }
    else
    {
        *res_sch_if = dnx_data_sch.interface.reserved_get(unit);
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_port_t port)
{
    ...
    if (port == master_port)
    {
        ...
        alloc_type = (!is_channelized ? DNX_SCHEDULER_ALLOCATORS_INTERFACE_TYPE_NON_CHANNELIZED :
                      (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 0) && !priority_propagation_enable ?
    ...
    bcm_port_t port)
{
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_scheduler_port_sch_reserved_interface_get(unit, &e2e_if));
        }
    ...
    bcm_port_t port)
{
    ...
    if (dnx_data_sch.general.port_priority_propagation_enable_get(unit))
    {
        ...
        for (i = 0; i < nof_interfaces; i++)
        {
            int invalid_egress_interface_id;
            ...
            {
                int egr_interfaces_field_size =
                    utilex_log2_round_up(dnx_data_egr_queuing_v1.egress_queues.nof_egr_queues_get(unit));
                invalid_egress_interface_id = utilex_max_value_by_size(egr_interfaces_field_size);
            }
            else
            {
                invalid_egress_interface_id = dnx_data_egr_queuing.params.reserved_if_get(unit);
            }
            SHR_IF_ERR_EXIT(dnx_sch_dbal_sch_if_to_egq_if_map_set
                            (unit, core, e2e_if + i, invalid_egress_interface_id, FALSE));
        }
    }
    else
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_scheduler_port_sch_reserved_interface_get(unit, &res_sch_if));
            SHR_IF_ERR_EXIT(dnx_sch_egq_if_to_sch_if_map_set(unit, core, egq_if, res_sch_if));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    uint32 *rate)
{
    ...
    uint32 sch_cal[DNX_DATA_MAX_SCH_INTERFACE_MAX_CALENDAR_SIZE];
    int base_hr;
    ...
    int alt_cal_alg)
{
    ...
    int rate;
    uint64 rates_per_hr[DNX_DATA_MAX_SCH_FLOW_NOF_HR];
    uint32 calendar[DNX_DATA_MAX_SCH_INTERFACE_MAX_CALENDAR_SIZE];
    ...
    sal_memset(rates_per_hr, 0, sizeof(uint64) * dnx_data_sch.flow.nof_hr_get(unit));
    sal_memset(calendar, 0, sizeof(uint32) * dnx_data_sch.interface.max_calendar_size_get(unit));
    ...
    if (is_channelized_calendar)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_channels_get(unit, port, DNX_ALGO_PORT_CHANNELS_F_NON_L1_ONLY, &ifs_bmp));
        ...
        else
        {
            ...
            BCM_PBMP_ITER(ifs_bmp, port_i)
            {
                ...
                if (sch_offset == sch_offset_i)
                {
                    ...
                    if (is_valid)
                    {
                        ...
                        COMPILER_64_SET(rates_per_hr[base_hr], 0, rate);
                        COMPILER_64_ADD_64(total_if_rate, rates_per_hr[base_hr]);
                    }
                }
            }
            ...
            /** Calculate calendar*/
            SHR_IF_ERR_EXIT(dnx_algo_cal_simple_from_rates_to_calendar(unit, alt_cal_alg, rates_per_hr,
    ...
    int alt_cal_alg)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        BCM_PBMP_ITER(ports_bmp, port)
        {
            ...
            /** configure the rate */
            SHR_IF_ERR_EXIT(dnx_scheduler_port_single_if_rate_hw_set(unit, port, FALSE, alt_cal_alg));
        }
    }
