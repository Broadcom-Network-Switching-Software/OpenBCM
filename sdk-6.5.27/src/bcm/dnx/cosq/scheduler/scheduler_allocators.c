    ...
    uint8 *last_reference)
{
    ...
    SHR_IF_ERR_EXIT(sch_alloc_db.
                    cl_class_alloc_mngr.exchange(unit, core_id, flags, nof_references, profile_data, old_profile,
...
dnx_scheduler_interface_allocator_create(
    int unit)
{
    resource_tag_bitmap_utils_extra_arg_create_info_t create_extra_info;
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        if (reserved_sch_if < create_data.nof_elements
            && !dnx_data_sch.general.port_priority_propagation_enable_get(unit))
        {
            ...
            SHR_IF_ERR_EXIT(sch_alloc_db.interface.allocate_several(unit, core_id, DNX_ALGO_RES_ALLOCATE_WITH_ID,
                                                                    1, (void *) &alloc_info, &reserved_sch_if));
