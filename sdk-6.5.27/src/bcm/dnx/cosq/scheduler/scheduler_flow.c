    ...
    int *is_low_rate)
{
    ...
    *is_low_rate = FALSE;
    ...
    {
        uint32 is_enabled, region_start, region_end;
        int flow_range_start, flow_range_end;

        SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.is_enabled.get(unit, core, range_id, &is_enabled));
        SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.range_start.get(unit, core, range_id, &region_start));
        SHR_IF_ERR_EXIT(dnx_scheduler_db.low_rate_ranges.range_end.get(unit, core, range_id, &region_end));

        flow_range_start = region_start * dnx_data_sch.flow.region_size_get(unit);
        flow_range_end =
            region_end * dnx_data_sch.flow.region_size_get(unit) + dnx_data_sch.flow.region_size_get(unit) - 1;

        if ((is_enabled) && UTILEX_IS_IN_RANGE(flow_id, flow_range_start, flow_range_end))
        {
            *is_low_rate = TRUE;
            break;
        }
...
/** see header file */
shr_error_e
dnx_sch_flow_is_low_rate_enable_get(
    int unit,
    int *is_enable)
{
    bcm_core_t core_id;
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        for (int range_id = 0; range_id < dnx_data_sch.flow.nof_low_rate_ranges_get(unit); range_id++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_sch_dbal_low_rate_flow_range_get
                            (unit, core_id, range_id, &flow_range_start, &flow_range_end, &pattern));
            if (pattern != 0)
            {
                *is_enable = TRUE;
                break;
...
/** see header file */
shr_error_e
dnx_sch_flow_low_rate_range_id_get(
    int unit,
    int type,
    int *range_id)
{
    ...
    switch (type)
    {
        ...
        case (bcmCosqRangeLowRate_2):
            *range_id = 2;
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "unsupported type\n");
            break;
    }
