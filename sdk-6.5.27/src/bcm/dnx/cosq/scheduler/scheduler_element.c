    ...
    uint32 *cos_weight)
{
    ...
    uint32 temp_weight, factored_weight, empty_exp;
    uint32 ratio;
    int mnt_field_size;
    ...
    switch (ipf_config_mode)
    {
        ...
        case DNX_SCH_IPF_CONFIG_MODE_INVERSE:
            ...
            COMPILER_64_SET(max_weight, 0, dnx_sch_cl_info[DBAL_ENUM_FVAL_CL_MODE_5].max_independent_weight);
    ...
    int group)
{
    ...
    if (is_composite)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core_id, composite_flow_id, &composite_se_id));
    ...
    dnx_sch_flow_credit_source_cos_data_t * se_cos)
{
    ...
    if (DNX_SCHEDULER_ELEMENT_IS_HR(unit, parent_se_id))
    {
        ...
        SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_element_hr_mode_weight_verify(unit, hr_mode, child_mode, child_weight));
    ...
    else if (DNX_SCHEDULER_ELEMENT_IS_CL(unit, parent_se_id))
    {
        ...
        /** validate mode and weight */
        SHR_INVOKE_VERIFY_DNXC(dnx_scheduler_element_cl_mode_weight_verify(unit, &cl_config, child_mode, child_weight));
