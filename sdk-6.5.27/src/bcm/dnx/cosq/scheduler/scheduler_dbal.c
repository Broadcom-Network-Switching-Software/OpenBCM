...
/** see header file */
shr_error_e
dnx_sch_dbal_low_rate_flow_range_set(
    int unit,
    int core_id,
    int range_id,
    int flow_min,
    ...
    uint32 pattern)
{
    ...
    /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SCH_LOW_RATE_CONFIGURATION, &entry_handle_id));

    /** Setting key fields */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RANGE_ID, range_id);

    /** Setting value fields */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOW_RATE_MIN, INST_SINGLE,
                                 flow_min / dnx_data_sch.flow.low_rate_flow_resolution_get(unit));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOW_RATE_MAX, INST_SINGLE,
                                 flow_max / dnx_data_sch.flow.low_rate_flow_resolution_get(unit));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOW_RATE_PATTERN, INST_SINGLE, pattern);
...
/** see header file */
shr_error_e
dnx_sch_dbal_low_rate_flow_range_get(
    int unit,
    int core_id,
    int range_id,
    int *flow_min,
    ...
    uint32 *pattern)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_LOW_RATE_PATTERN, INST_SINGLE, pattern);

    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    *flow_min = hw_min * dnx_data_sch.flow.low_rate_flow_resolution_get(unit);
    /** last flows are always lost in the conversion from SW to HW and back */
    *flow_max = hw_max * dnx_data_sch.flow.low_rate_flow_resolution_get(unit);
    if (hw_max != 0)
    {
        *flow_max += dnx_data_sch.flow.low_rate_flow_resolution_get(unit) - 1;
    }
