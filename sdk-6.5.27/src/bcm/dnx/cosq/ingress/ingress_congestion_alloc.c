    ...
    int *is_allocated)
{
    ...
    switch (vsq_group)
    {
        case DNX_INGRESS_CONGESTION_VSQ_GROUP_STTSTCS_TAG:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.vsq_rate_class.
                            vsq_rate_cls_d.exchange(unit, core_id, 0, 1, &data_rate_cls->data.vsq_a_d_rate_class_info,
                                                    tmp_old_rate_cls, NULL, new_rate_cls, &tmp_is_alloc, &tmp_is_last));
            break;
        case DNX_INGRESS_CONGESTION_VSQ_GROUP_SRC_PORT:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.vsq_rate_class.
                            vsq_rate_cls_source_port.exchange(unit, core_id, 0, 1,
                                                              &data_rate_cls->data.vsq_e_rate_class_info,
                                                              tmp_old_rate_cls, NULL, new_rate_cls, &tmp_is_alloc,
                                                              &tmp_is_last));
            break;
        case DNX_INGRESS_CONGESTION_VSQ_GROUP_PG:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.vsq_rate_class.vsq_rate_cls_pg.exchange(unit, core_id, 0, 1,
                                                                                              &data_rate_cls->
