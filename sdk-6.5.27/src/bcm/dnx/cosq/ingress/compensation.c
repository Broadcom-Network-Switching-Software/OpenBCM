    ...
    int delta)
{
    ...
    DNXCMN_CORES_ITER(unit, core, core_idx)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_compensation_db.
                        compensation.exchange(unit, core_idx, 0, 1, &delta, old_comp_profile, NULL, &new_comp_profile,
                                              &is_first, &is_last));
...
/**
 * \brief - Configure CRC not to be ignored by compensation
 *
...
static shr_error_e
dnx_cosq_ingress_compensation_crc_init(
    int unit)
{
    uint32 entry_handle_id;
    ...
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_COMPENSATION_CRC_CONFIG, &entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IGNORE_CRC, INST_SINGLE, 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int unit)
{
    ...
    /*
     * IRE delta configuration init
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_crc_init(unit));
