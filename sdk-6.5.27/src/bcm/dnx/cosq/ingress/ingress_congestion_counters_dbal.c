 ...
 */
shr_error_e
dnx_ingress_congestion_voq_programmable_counter_configuration_get(
    int unit,
    int core,
    uint32 *base_voq_id,
    uint32 *nof_voqs)
{
   ...
   /** Select the core */
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_VOQ_ID, INST_SINGLE, base_voq_id);

    /** Get maximum value for the mask field */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_VOQ_MASK, INST_SINGLE, nof_voqs);
...
/**
 * Get SW state for Global VSQ occupancy max counters
 */
static shr_error_e
dnx_ingress_congestion_gl_vsq_occupancy_swstate_get(
    int unit,
    int core,
    int nof_pools,
    dnx_ingress_congestion_gl_vsq_occupancy_info_t * info_get)
{
    int pool_idx;
    ...
    for (pool_idx = 0; pool_idx < nof_pools; pool_idx++)
    {
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.
                        shared_size_max.get(unit, core, pool_idx, &info_get->shared_max[pool_idx]));
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.
                        guaranteed_size_max.get(unit, core, pool_idx, &info_get->guaranteed_max[pool_idx]));
    }

    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.
                    headroom_size_max.get(unit, core, &info_get->headroom_max));
...
/**
 * Set SW state for Global VSQ occupancy max counters
 */
static shr_error_e
dnx_ingress_congestion_gl_vsq_occupancy_swstate_set(
    int unit,
    int core,
    int nof_pools,
    dnx_ingress_congestion_gl_vsq_occupancy_info_t * info_set)
{
    int pool_idx;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.
                        shared_size_max.set(unit, core, pool_idx, info_set->shared_max[pool_idx]));
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.
                        guaranteed_size_max.set(unit, core, pool_idx, info_set->guaranteed_max[pool_idx]));
    }

    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.
                    headroom_size_max.set(unit, core, info_set->headroom_max));
...
/*
 * This function updates the max values in Sw State if needed,
 * compares HW and SW state and retrieves the higher values
 * Note:
 * dnx_ingress_congestion_gl_vsq_occupancy_info_t info is in\out
 */
static shr_error_e
dnx_ingress_congestion_gl_vsq_occupancy_max_value_update(
    int unit,
    int core,
    dnx_ingress_congestion_gl_vsq_occupancy_info_t * info)
{
    ...
    /** Read SW state */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_gl_vsq_occupancy_swstate_get(unit, core, nof_pools, &info_get));

    /** compare and set the higher value */
    for (pool_idx = 0; pool_idx < nof_pools; pool_idx++)
    {
        /** compare and set the higher value */
        info->shared_max[pool_idx] =
            info->shared_max[pool_idx] >
            info_get.shared_max[pool_idx] ? info->shared_max[pool_idx] : info_get.shared_max[pool_idx];
        info->guaranteed_max[pool_idx] =
            info->guaranteed_max[pool_idx] >
            info_get.guaranteed_max[pool_idx] ? info->guaranteed_max[pool_idx] : info_get.guaranteed_max[pool_idx];
    }

    info->headroom_max = info->headroom_max > info_get.headroom_max ? info->headroom_max : info_get.headroom_max;

    /** Write updated values in SW state */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_gl_vsq_occupancy_swstate_set(unit, core, nof_pools, info));
exit:
    SHR_FUNC_EXIT;
}

/*
 * This function sets to 0 the SW state for a specific type
 */
shr_error_e
dnx_ingress_congestion_gl_vsq_occupancy_max_value_clear(
    int unit,
    int core,
    int pool_id,
    int lossless_pool,
    bcm_cosq_icgm_resource_stat_type_t stat_type)
{
    ...
    switch (stat_type)
    {
        ...
        case bcmCosqICgmMaxSharedBytes:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.shared_size_max.set(unit, core, pool_id, 0));
            break;
        case bcmCosqICgmMaxGrntdBytes:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.
                            guaranteed_size_max.set(unit, core, pool_id, 0));
            break;
        case bcmCosqICgmMaxHdrmBytes:
            if (pool_id == lossless_pool)
            {
                SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.global_vsq_occupancy.headroom_size_max.set(unit, core, 0));
            }
            break;
        default:
            break;
 ...
 */
shr_error_e
dnx_ingress_congestion_gl_vsq_occupancy_info_get(
    int unit,
    int core,
    dnx_ingress_congestion_gl_vsq_occupancy_info_t * info)
{
    ...
    /** Get key to the dbal table */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_CONG_CGM_GLOBAL_VSQ_OCCUPANCY, &entry_handle_id));
    ...
    /** Shared and Guaranteed are per pool*/
    for (pool_idx = 0; pool_idx < nof_pools; pool_idx++)
    {
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_SHARED_SIZE, pool_idx, &info->shared[pool_idx]);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_SHARED_SIZE_MAX, pool_idx,
                                   &info->shared_max[pool_idx]);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_GUARANTEED_SIZE, pool_idx,
                                   &info->guaranteed[pool_idx]);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_GUARANTEED_SIZE_MAX, pool_idx,
                                   &info->guaranteed_max[pool_idx]);
    }

    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_HEADROOM_SIZE, INST_SINGLE, &info->headroom);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_HEADROOM_SIZE_MAX, INST_SINGLE, &info->headroom_max);
    /** Get values from HW */
    ...
    for (pool_idx = 0; pool_idx < nof_pools; pool_idx++)
    {
        info->shared[pool_idx] *= dnx_data_ingr_congestion.info.words_resolution_get(unit);
        info->shared_max[pool_idx] *= dnx_data_ingr_congestion.info.words_resolution_get(unit);
        info->guaranteed[pool_idx] *= dnx_data_ingr_congestion.info.words_resolution_get(unit);
        info->guaranteed_max[pool_idx] *= dnx_data_ingr_congestion.info.words_resolution_get(unit);
    }
    info->headroom *= dnx_data_ingr_congestion.info.words_resolution_get(unit);
    info->headroom_max *= dnx_data_ingr_congestion.info.words_resolution_get(unit);

    /** Update SW state */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_gl_vsq_occupancy_max_value_update(unit, core, info));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
