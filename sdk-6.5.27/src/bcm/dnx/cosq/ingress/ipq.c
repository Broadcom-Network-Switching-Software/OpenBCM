...
#define DNX_COSQ_IPQ_QUEUE_BUNDLE_BITMAP_ALLOC_PATTERN(unit, core) (core == BCM_CORE_ALL ? dnx_data_device.general.valid_cores_bitmap_get(unit) : (1 << core))
    ...
    int unit)
{
    ...
    {
        if (dnx_data_ipq.general.feature_get(unit, dnx_data_ipq_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_ipq_power_down_set(unit, 1));
        }
    }
...
/**
 * brief - clear mapping from system TC to VOQ offset, for all TCs
 */
static shr_error_e
dnx_cosq_ipq_tc_to_voq_offset_map_clear(
    int unit,
    int in_core,
    uint32 profile,
    uint32 is_flow,
    uint32 is_fmq)
{
    ...
    for (sys_tc = 0; sys_tc < DNX_COSQ_NOF_TC; ++sys_tc)
    {
        for (dp = 0; dp < nof_dps; dp++)
        {
            SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_tc_to_voq_offset_map_clear
                            (unit, core, sys_tc, dp, profile, is_flow, is_fmq));
        }
    }
...
/**
 * brief - determine if VOQ is flow based
 */
static shr_error_e
dnx_cosq_ipq_is_voq_flow_based(
    int unit,
    int core,
    int base_voq,
    int *is_flow_based)
{
    ...
    uint32 base_voq_quartet, sysport = 0, dest_base_queue, is_dest_valid;
    ...
    *is_flow_based = FALSE;
    ...
    /*
     * If dest_base_queue != base_voq, then we are in flow based mapping
     */
    ...
    int *is_allocated)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_voq_flow_map.exchange(unit, core, flags, 1, tc_mapping, *old_map_profile, NULL,
                                                 new_map_profile, (uint8 *) is_allocated, (uint8 *) is_last));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_sysport_map.exchange(unit, flags, 1, tc_mapping, *old_map_profile, NULL, new_map_profile,
    ...
    int *base_queue)
{
    ...
    rv = dnx_ipq_alloc_mngr_db.ipq_alloc_mngr.allocate_single(unit, flags, (void *) &alloc_info, &element);
    if (_SHR_E_FAILURE(rv))
    {
        if (is_with_id)
        {
            SHR_ERR_EXIT(_SHR_E_RESOURCE, "Can't allocate for base_queue %d", *base_queue);
        }
        else
        {
    ...
    int base_queue)
{
    ...
    for (bundle_iter = 0; bundle_iter < nof_bundles_to_free; bundle_iter++)
    {
        DNXCMN_CORES_ITER(unit, core, core_idx)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.ipq_alloc_mngr.free_single(unit, element, NULL));
        }
    }
    ...
    int *queue_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_fabric_multicast_queue_range_get_verify(unit, queue_id));
    ...
    int queue_id)
{
     ...
     * Verify input
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_fabric_multicast_queue_range_set_verify(unit, queue_id));
    ...
    bcm_cosq_queue_offset_t offset_array[])
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_tc_mapping_verify(unit, gport));
    ...
    DNXCMN_CORES_ITER(unit, core, core_idx)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_map_profile_exchange
                        (unit, flags, core_idx, dest_ndx, is_flow, &tc_mapping, &old_profile, &new_profile, &is_last,
                         &is_allocated));
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
    ...
    /** get pp dsp from old modport**/
    ftmh_pp_dsp = BCM_GPORT_MODPORT_PORT_GET(mod_port_gport);

    /** Find PP DSP for the new modport **/
    SHR_IF_ERR_EXIT(dnx_stk_sys_modport_to_pp_dsp_convert(unit, gport_connect->remote_modid, ftmh_pp_dsp,
                                                          &remote_core, &pp_dsp));
    SHR_IF_ERR_EXIT(dnx_algo_port_pp_dsp_to_logical_get(unit, remote_core, pp_dsp, &logical_port));
    ...
    DNXCMN_CORES_ITER(unit, ingress_core, core_id)
    {
        ...
        for (quad = 0; quad < nof_quads; quad++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_scheduler_connector_flow_id_calc(unit, core_id, base_cid,
                                                                 ...
                                                                 is_composite, is_remove, FALSE, flow_subflow,
                                                                 &nof_subflows));
            /*
    ...
    uint32 *is_stk)
{
    ...
    *is_stk = FALSE;
     ...
     */
    SHR_ALLOC_ERR_EXIT(local_modid, sizeof(uint32) * dnx_data_device.general.max_nof_fap_ids_per_core_get(unit),
                       "local_modid", "%s%s%s", "local_modid", EMPTY, EMPTY);
    ...
    bcm_cosq_ingress_queue_bundle_gport_config_t * config)
{
    ...
    for (cosq = 0; cosq < config->numq; cosq++)
    {
        ...
        if (!dnx_data_iqs.credit.feature_get(unit, dnx_data_iqs_credit_profile_is_rate_class))
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_iqs_credit_request_profile_map_set
                            (unit, core, base_voq, cosq, delay_tolerance_profile));
        }
    ...
    bcm_gport_t * gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_ucast_gport_add_verify(unit, config, gport));
    ...
    bcm_gport_t * gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_mcast_gport_add_verify(unit, config, gport));
    ...
    uint32 *flags)
{
    ...
    int base_voq = 0, is_mc = FALSE, core = BCM_CORE_ALL, is_flow_based;
    SHR_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_ucast_gport_verify(unit, gport));

    *flags = 0;
    base_voq = BCM_GPORT_UNICAST_QUEUE_GROUP_QID_GET(gport);
    core = BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_GET(gport);

    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_gport_get_verify(unit, core, base_voq));

    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_get(unit, core, base_voq, is_mc, physical_port, num_cos_levels, flags));

    SHR_IF_ERR_EXIT(dnx_cosq_ipq_is_voq_flow_based(unit, core, base_voq, &is_flow_based));
    ...
    uint32 *flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_mcast_gport_verify(unit, gport));
...
/**
 * brief - Resets the default tc to offset flow based mapping
 * \param [in] unit -The unit number.
 * \param [in] core -device core id.
 * \param [in] base_voq - base queue of the voq bundle
 * \param [in] numq - number of queues allocated
 *
 * \return
 *   \retval Zero if no error was detected
 ...
 */
static shr_error_e
dnx_cosq_ipq_default_flow_based_offset_mapping_set(
    int unit,
    int core,
    uint32 base_voq,
    int numq)
{
    ...
    uint32 old_profile;
    int core_idx;
    int is_last = FALSE, is_allocated = FALSE;
    int nof_quartet, quartet_index, destination;
    dnx_ipq_tc_map_t tc_mapping = { {{0}} };
    uint32 flags;
    int new_profile = dnx_data_ipq.tc_map.default_voq_flow_profile_idx_get(unit);

    SHR_FUNC_INIT_VARS(unit);

    nof_quartet = numq / min_bundle_size;
    ...
    DNXCMN_CORES_ITER(unit, core, core_idx)
    {
         ...
         * Update sw state
         */
        flags = DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID | DNX_ALGO_TEMPLATE_EXCHANGE_IGNORE_DATA;
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_map_profile_exchange
                        (unit, flags, core_idx, base_voq, TRUE, &tc_mapping, &old_profile, &new_profile, &is_last,
                         &is_allocated));
        /**
         * Update hw
         */
        for (quartet_index = 0; quartet_index < nof_quartet; quartet_index++)
        {
             ...
             * Set new mapping.
             */
            destination = base_voq + (quartet_index * min_bundle_size);
            SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_profile_set(unit, core_idx, destination, TRUE, new_profile));
        }
        ...
        if (is_last == TRUE)
        {
             ...
             * Set new profile data
             */
            SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_to_voq_offset_map_clear(unit, core_idx, old_profile, TRUE, FALSE));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int is_mc)
{
    ...
    {
        /**
         * reset system TC and DP to VOQ offset mapping
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_default_flow_based_offset_mapping_set(unit, core, base_voq, numq));
    ...
    for (cosq = 0; cosq < numq; cosq++)
    {
        ...
        if (!dnx_data_iqs.credit.feature_get(unit, dnx_data_iqs_credit_profile_is_rate_class))
        {
            /** get the delete profile id */
            SHR_IF_ERR_EXIT(dnx_iqs_delete_profile_get(unit, &profile_id, &valid));
            if (!valid)
            {
                /** clear credit request profile */
                SHR_IF_ERR_EXIT(dnx_iqs_credit_request_profile_map_clear(unit, core, base_voq, cosq));
            }
            else
            {
                /*
                 * use delete credit request profile to clear the queue's credit balance:
                 *    1. Set the queue's credit request profile into the delete profile.
                 *       When the queue will get empty, the credit balance will be cleared.
                 *    2. Try to send a manual credit. If queue is empty, queue credit balance should be cleared.
                 *       This should cover the case of deleting a queue that was already empty.
                 */

                /** set credit request profile to delete profile */
                SHR_IF_ERR_EXIT(dnx_iqs_credit_request_profile_map_set(unit, core, base_voq, cosq, profile_id));

                DNXCMN_CORES_ITER(unit, core, core_idx)
                {
                    /** wait for previous manual credit generation to be done */
                    rv = dnx_iqs_queue_manual_credit_trigger_down_polling(unit, core_idx);
                    if (rv == _SHR_E_TIMEOUT)
                    {
                        LOG_WARN(BSL_LOG_MODULE,
                                 (BSL_META_U(unit, "manual credit generation skipped for queue %d (core %d)\n"),
                                  base_voq + cosq, core_idx));
                    }
                    else
                    {
                        SHR_IF_ERR_EXIT(rv);
                    ...
                    if (rv == _SHR_E_NONE)
                    {
                        /** set queue id to generate credit manually */
                        SHR_IF_ERR_EXIT(dnx_iqs_queue_manual_operation_queue_id_set(unit, core_idx, base_voq + cosq));

                        /** generate credit manually. if queue is empty, queue credit balance should be cleared */
                        SHR_IF_ERR_EXIT(dnx_iqs_queue_manual_credit_trigger_set(unit, core_idx, TRUE));
                    }
                }
            }
        }
    ...
    bcm_gport_t gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_mcast_gport_delete_verify(unit, gport));
    ...
    bcm_gport_t gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_ucast_gport_delete_verify(unit, gport));
    ...
    uint32 alloc_flags)
{
    ...
    extra_create_info.grains_size[0] = region_nof_quartets * DNX_COSQ_IPQ_BITMAP_ELEMS_PER_QUEUE_BUNDLE(unit);

    /*
     * Tag is actually the queue type. so max tag value is [nof_queue_types - 1]
     */
    extra_create_info.max_tags_value[0] = NUM_DNX_ALGO_IPQ_QUEUE_TYPE - 1;
    ...
    SHR_IF_ERR_EXIT(resource_tag_bitmap_create
                    (unit, module_id, res_tag_bitmap, &res_tag_bitmap_create_info, &extra_create_info, alloc_flags));
    ...
    int mc_prio)
{
    ...
    /** Verify arguments */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_tc_to_multicast_prio_map_set_verify(unit, cosq, mc_prio));
    ...
    int *mc_prio)
{
    ...
    /** Verify arguments */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_tc_to_multicast_prio_map_get_verify(unit, cosq, mc_prio));
    ...
    int enable)
{
    ...
    if (!dnx_data_ipq.queues.feature_get(unit, dnx_data_ipq_queues_invalid_queue_support))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Invalid queue not supported on this device");
    }
    ...
    int enable)
{
    ...
    /** verify enable param*/
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_invalid_destination_queue_set_verify(unit, enable));
    ...
    int *enable)
{
    ...
    *enable = TRUE;
    ...
    /** verify enable param*/
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_invalid_destination_queue_get_verify(unit, enable));
    ...
    int enable)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_ipq_sysport_ingress_queue_map_enable_validate(unit, sysport_gport));
