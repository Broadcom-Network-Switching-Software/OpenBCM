    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    else
    {
        ...
        {
            allowed_flags |= BCM_COSQ_DISCARD_CONTINUOUS_DROP_PROBABILITY;
        }
    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_discard_set_verify(unit, gport, cosq, discard));
    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_discard_get_verify(unit, gport, cosq, discard));
    ...
    bcm_cos_queue_t cosq)
{
    ...
    /** Validate parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_vsq_add_verify(unit, vsq, queue, cosq));
    ...
    bcm_gport_t * vsq_gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_vsq_create_verify(unit, vsq_info, vsq_gport));
    ...
    bcm_gport_t * pg_base_vsq_gport)
{
    ...
    int enable;
    int is_over_fabric;
    int src_port_vsq_id, pg_base_vsq_id;
    int nof_phys;
    int non_nif_vsq_f_nof;
    ...
    /** Check if we try to allocate NIF port with id only when feature is disabled */
    if (!dnx_data_ingr_congestion.vsq.feature_get(unit, dnx_data_ingr_congestion_vsq_vsq_with_id_on_nif_port_support))
    {
        if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF))
            && (BCM_COSQ_SRC_VSQS_GPORT_WITH_ID & config->flags))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_COSQ_SRC_VSQS_GPORT_WITH_ID is not supported for NIF ports\n");
        }
    }

    if (BCM_COSQ_SRC_VSQS_GPORT_WITH_ID & config->flags)
    {
        if (!BCM_COSQ_GPORT_IS_VSQ_SRC_PORT(*src_port_vsq_gport))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Gport(0x%08x) must be a vsq src gport.", *src_port_vsq_gport);
        }

        if (!BCM_COSQ_GPORT_IS_VSQ_PG(*pg_base_vsq_gport))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Gport(0x%08x) must be a pg base vsq gport.", *pg_base_vsq_gport);
        }

        src_port_vsq_id = BCM_COSQ_GPORT_VSQ_SRC_PORT_GET(*src_port_vsq_gport);
        if (src_port_vsq_id >= dnx_data_ingr_congestion.vsq.vsq_e_nof_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Add src vsq gport(0x%08x) with_id failed, queue %d is out of range",
                         *src_port_vsq_gport, src_port_vsq_id);
        }

        if (config->numq > 0)
        {
            pg_base_vsq_id = BCM_COSQ_GPORT_VSQ_PG_GET(*pg_base_vsq_gport);
            if ((pg_base_vsq_id + config->numq) > dnx_data_ingr_congestion.vsq.vsq_f_nof_get(unit))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Add pg vsq gport(0x%08x) with_id failed, queue %d is out of range",
                             *pg_base_vsq_gport, pg_base_vsq_id);
            }
        }
    }

    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_nof_phys_per_core_get(unit, core_id, &nof_phys));
    non_nif_vsq_f_nof = dnx_data_ingr_congestion.vsq.vsq_f_nof_get(unit) - nof_phys * BCM_COSQ_VSQ_NOF_PG;
    if (!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF)) && config->numq != 0 &&
        non_nif_vsq_f_nof == 0)
    {
        if (!(BCM_COSQ_SRC_VSQS_GPORT_WITH_ID & config->flags))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "PG VSQ cannot be created for non NIF port on this core (%d). config.numq should be 0 (%d)\n",
                         core_id, config->numq);
        }
    }
    ...
    bcm_gport_t * pg_base_vsq_gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_src_vsqs_gport_add_verify(unit, port, config,
    ...
    /** Check if we want to allocate with ID */
    if (config->flags & BCM_COSQ_SRC_VSQS_GPORT_WITH_ID)
    {
    ...
    if (config->numq > 0)
    {
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_vsq_f_id_alloc(unit, config->flags, src_port, config->numq, &pg_base));
    }

    SHR_IF_ERR_EXIT(dnx_ingress_congestion_vsq_e_id_alloc(unit, config->flags, src_port, &src_port_vsq_index));
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_port_based_vsq_sw_state_update(unit, core_id, config->numq, src_port,
    ...
    bcm_gport_t * pg_base_vsq_gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_src_vsqs_gport_get_verify(unit, port, config,
                                                              src_port_vsq_gport, pg_base_vsq_gport));
    ...
    bcm_gport_t vsq_gport)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_vsq_destroy_verify(unit, vsq_gport));
    ...
    bcm_cosq_resource_amounts_t * amounts)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_resource_allocation_get_verify(unit, flags, resource, target, amounts));
...
{
    dnx_ingress_congestion_resource_type_e rsrc_type;
    uint32 allowed_flags = BCM_COSQ_RSRC_ALLOC_COLOR_BLIND;
    ...
    {
        allowed_flags |= BCM_COSQ_RSRC_ALLOC_NO_COLOR;
    }
    ...
    bcm_cosq_resource_amounts_t * amounts)
{
    ...
    /** Input validation */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_resource_allocation_set_verify(unit, flags, resource, target, amounts));
    ...
    bcm_cosq_gport_size_t * gport_size)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_color_size_verify(unit, gport, cosq, color, flags, gport_size));
    ...
    bcm_cosq_inst_congestion_info_data_t * data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_inst_congestion_info_get_verify(unit, flags, key, data));
