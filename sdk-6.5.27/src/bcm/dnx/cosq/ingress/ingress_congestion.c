    ...
    int *vsq_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_gport_vsq_get_verify(unit, vsq_gport, cosq));
    ...
    dnx_ingress_congestion_vsq_tc_pg_mapping_t * tc_pg_mapping)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.vsq_pg_tc_mapping.exchange(unit, core_id, 0, 1, tc_pg_mapping,
    ...
    int pg_base)
{
    ...
    if (pg_base != -1)
    {
        ...
        {
            vsq_id = pg_base + cosq;
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.port_based_vsq.vsq_f_mapping.
                            src_port.set(unit, core_id, vsq_id, src_port));
        }
    }
exit:
    SHR_FUNC_EXIT;
...
/**
 * \brief - Return core and NIF interface ID that will be used as VSQ-E ID
 *
 */
static shr_error_e
dnx_ingress_congestion_port_to_vsq_e_id_get(
    int unit,
    ...
    int *vsq_e_id)
{
    ...
    int interface_offset, phy_port, nof_eth_if, ilkn_start;
    ...
    switch (port_info.port_type)
    {
        case DNX_ALGO_PORT_TYPE_NIF_ILKN:
            ...
            *vsq_e_id = ilkn_start + interface_offset;
            break;

        case DNX_ALGO_PORT_TYPE_NIF_ETH:
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, logical_port, 0, &phy_port /* zero based */ ));
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, phy_port, vsq_e_id));
            break;
    ...
    int *vsq_e_id)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF)))
    {
        ...
        if (dnx_data_ingr_congestion.vsq.feature_get(unit, dnx_data_ingr_congestion_vsq_vsq_with_id_on_nif_port_support)
            && (BCM_COSQ_SRC_VSQS_GPORT_WITH_ID & flags))
        {
            /** use VSQ-E index provided by user */
            alloc_flags |= DNX_ALGO_RES_ALLOCATE_WITH_ID;
        }
        else
        {
            /** VSQ-E index is mapped 1-1 to NIF interface ID */
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_port_to_vsq_e_id_get(unit, src_port, &core_id, vsq_e_id));
            alloc_flags |= DNX_ALGO_RES_ALLOCATE_WITH_ID;
        }
    }
    else
    {
        /** Non NIF port -- check if we want to allocate with or without ID */
        if (BCM_COSQ_SRC_VSQS_GPORT_WITH_ID & flags)
        {
            alloc_flags |= DNX_ALGO_RES_ALLOCATE_WITH_ID;
        }
        else
        {
            /** When we allocate without id we want to allocate from the extra VSQs */
            alloc_flags |= RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE;

            /*
             * VSQEs allocated for NIF ports include VSQE per nif SerDes and VSQE per ILKN Data interface.
             * Start allocating VSQE for non-NIF ports after VSQEs allocated for NIF.
             */
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_nof_phys_per_core_get(unit, core_id, &nof_phys));
            nof_ilkn_if = dnx_data_nif.ilkn.ilu_nof_get(unit) * dnx_data_nif.ilkn.ilkn_unit_if_nof_get(unit);
            alloc_info.range_start = nof_phys + nof_ilkn_if;
            alloc_info.range_end = dnx_data_ingr_congestion.vsq.vsq_e_nof_get(unit);
        }
    }
    /** Allocate VSQ-E id */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.algo_res.vsqe_id.allocate_single
                    (unit, core_id, alloc_flags, (void *) &alloc_info, vsq_e_id));
    ...
    int vsq_e_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.algo_res.vsqe_id.free_single(unit, core_id, vsq_e_id, NULL));
    ...
    int *vsq_f_id)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF)))
    {
        ...
        if (dnx_data_ingr_congestion.vsq.feature_get(unit, dnx_data_ingr_congestion_vsq_vsq_with_id_on_nif_port_support)
            && (BCM_COSQ_SRC_VSQS_GPORT_WITH_ID & flags))
        {
            /** use VSQ-F index provided by user */
            alloc_flags |= DNX_ALGO_RES_ALLOCATE_WITH_ID;
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, src_port, 0, &phy_port));
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, phy_port, &local_phy_index)); /** VSQ id is local per core */
    ...
    else
    {
        ...
        else
        {
            ...
            alloc_flags |= RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE;
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_nof_phys_per_core_get(unit, core_id, &nof_phys));
            alloc_info.range_start = (nof_phys * BCM_COSQ_VSQ_NOF_PG);
            alloc_info.range_end = dnx_data_ingr_congestion.vsq.vsq_f_nof_get(unit);
        }
    }

    /** Allocate VSQ-F id */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.algo_res.vsqf_id.
                    allocate_several(unit, core_id, alloc_flags, numq, (void *) &alloc_info, vsq_f_id));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int vsq_f_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.algo_res.vsqf_id.free_several(unit, core_id, numq, vsq_f_id, NULL));
    ...
    int unit)
{
    ...
    data.nof_elements = dnx_data_ingr_congestion.vsq.vsq_e_nof_get(unit);
     ...
     */
    data.first_element = 0;
    data.nof_elements = dnx_data_ingr_congestion.vsq.vsq_f_nof_get(unit);
    ...
    dnx_ingress_congestion_reject_mask_t * reject_mask)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_reject_bit_set(unit, DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_PKT_SIZE,
                                                              1, reject_mask));
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_reject_bit_set(unit, DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_GATE_STATE,
                                                              1, reject_mask));
    }
exit:
    SHR_FUNC_EXIT;
...
dnx_ingress_congestion_admission_tests_default_set(
    int unit)
{
    dnx_ingress_congestion_reject_mask_t reject_mask, taf_reject_mask;
    dnx_ingress_congestion_reject_mask_key_t reject_key;
    int is_voq;
    int i;
    ...
    for (i = 0; i < DNX_INGRESS_CONGESTION_REJECT_BIT_NOF; i++)
    {
        if (i == DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_PKT_SIZE
            || i == DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_GATE_STATE)
        {
            /** TAF bits are disabled in regular masks */
            reject_mask.mask[i] = 0;
        }
        else
        {
    ...
    if (dnx_data_tsn.general.feature_get(unit, dnx_data_tsn_general_is_supported))
    {
        sal_memcpy(&taf_reject_mask, &reject_mask, sizeof(dnx_ingress_congestion_reject_mask_t));
        /** Set TAF bits*/
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_reject_bit_set(unit, DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_PKT_SIZE,
                                                              1, &taf_reject_mask));
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_reject_bit_set(unit, DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_GATE_STATE,
                                                              1, &taf_reject_mask));
        reject_key.pp_admit_profile = dnx_data_tsn.taf.taf_admission_profile_get(unit);
        /** set mask default PP admission profile + TAF rejects (2)  */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_reject_mask_set(unit, DBAL_TABLE_INGRESS_CONG_PP_RJCT_MASK,
                                                                    &reject_key, &taf_reject_mask));
    }
    ...
    int unit)
{
    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_is_freeze_boot_supported))
    {
    ...
    int unit)
{
    ...
    for (vsq_type = DNX_INGRESS_CONGESTION_VSQ_GROUP_CTGRY; vsq_type < DNX_INGRESS_CONGESTION_VSQ_GROUP_NOF; vsq_type++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_vsq_rate_class_hw_set(unit, BCM_CORE_ALL, 0, vsq_type,
                                                  &vsq_rate_class_default_data, &vsq_exact_rate_class_info, FALSE));
    }
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_ING_TM(unit, port_info))
    {
        ...
        if (tdm_mode != DNX_ALGO_PORT_TDM_MODE_BYPASS ||
            DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF)))
        {
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_vsq_shared_free_drop_threshold_arg_verify
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    /** Verify API arguments */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_global_free_sram_drop_threshold_arg_verify(unit,
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    /** Verify API arguments */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_global_free_dram_drop_threshold_arg_verify(unit,
                                                                                             index_type, index_info,
                                                                                             thresh_info, threshold));
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    if (drop_priority >= DNX_COSQ_NOF_DP)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Drop priority is out of range, must be min:0 max:%d", (DNX_COSQ_NOF_DP - 1));
    }
    ...
    switch (fwd_action)
    {
        case bcmCosqFwdTypeFwd:
        case bcmCosqFwdTypeSnoop:
        case bcmCosqFwdTypeMirror:
        case bcmCosqFwdTypeStatSample:
            /** expected value */
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Unexpected forward action %d", fwd_action);
            break;
    }
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    /** Verify API arguments */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_cmd_fifo_sram_free_drop_threshold_arg_verify(unit,
                                                                                               cast, priority,
                                                                                               fwd_action,
                                                                                               drop_priority,
                                                                                               thresh_info, threshold));
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_cmd_fifo_sram_reject_threshold_hw_get
                    (unit, fifo_id, fwd_action, drop_priority, resource_type,
                     (uint32 *) &threshold->thresh_set, (uint32 *) &threshold->thresh_clear));
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    switch (index_type)
    {
        case DNX_COSQ_THRESHOLD_INDEX_CAST_PRIO_FDWACT_DP:
    ...
    bcm_cosq_vsq_info_t * vsq_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_gport_vsq_get_verify(unit, vsq, 0));
    ...
    bcm_gport_t vsq_gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_port_based_vsq_sw_state_update(unit, core_id, numq, DNX_ALGO_PORT_INVALID,
                                                                          src_port_vsq_index, pg_base));
    ...
    dnx_ingress_congestion_wred_info_t * wred_data)
{
    ...
    wred_data->continuous_probability = discard->flags & BCM_COSQ_DISCARD_CONTINUOUS_DROP_PROBABILITY ? 1 : 0;
    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    if (wred_data->continuous_probability)
    {
        discard->flags |= BCM_COSQ_DISCARD_CONTINUOUS_DROP_PROBABILITY;
    }
    ...
    uint32 global_drop_pool_dp[][DNX_COSQ_NOF_DP])
{
    ...
    int resources_left, dp, updated_voq_total_guaranteed;
    uint32 equivalent_global_drop, equivalent_global_drop_hw;
    int core_idx;
    ...
    DNXCMN_CORES_ITER(unit, core, core_idx)
    {
        ...
        updated_voq_total_guaranteed = voq_total_guaranteed + guaranteed_change;

        /*
         * resources lest after the requested change (guaranteed_change can be < 0)
         */
        resources_left = max_res - updated_voq_total_guaranteed - equivalent_global_drop_hw;
        ...
        for (dp = 0; dp < DNX_COSQ_NOF_DP; ++dp)
        {
            ...
            if (resources_left)
            {
                ...
                reject_st_thr = (resources_left * drop_percentage / 100) + updated_voq_total_guaranteed;
            }
            else
            {
                /*
         ...
         */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.guaranteed.
                        voq_guaranteed.set(unit, core_idx, res_type, updated_voq_total_guaranteed));
    }
    ...
    bcm_cosq_ingress_queue_bundle_gport_config_t * config)
{
    ...
    for (cosq = 0; cosq < numq; cosq++)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Rate class ID out of range - must be between 0 and %d",
                         dnx_data_ingr_congestion.voq.nof_rate_class_get(unit) - 1);
        }
    }
    ...
    int rate_class)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_voq_to_rate_class_profile_map_set_verify
                           (unit, core, voq, rate_class));
    ...
    bcm_cosq_fadt_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_dram_bound_fadt_threshold_verify(unit, fadt_info, threshold, is_set));
    ...
    bcm_cosq_fadt_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_voq_congestion_notification_fadt_threshold_verify
    ...
    int is_set)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_rate_class_gport_verify(unit, gport, cosq, is_vsq_allowed));
    ...
    bcm_cosq_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_dram_recovery_threshold_verify(unit, gport, cosq, threshold, is_set));
    ...
    bcm_cosq_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_gport_threshold_verify(unit, gport, cosq, threshold));
    /*
...
dnx_ingress_congestion_dram_usage_stop(
    int unit)
{
    int dram_in_use, rate_class, resource_type, is_res_range, is_mutex_locked = FALSE, dp;
    ...
    {
        /** initialize dram_thresholds structure */
        sal_memset(&dram_thresholds[resource_type], 0, sizeof(bcm_cosq_fadt_threshold_t));
...
{
    uint32 nof_rate_class = dnx_data_ingr_congestion.voq.nof_rate_class_get(unit);
    int rate_class, dram_in_use, resource_type, is_dram_bound_mutex_locked = FALSE, is_fadt_mutex_locked = FALSE, dp;
    SHR_FUNC_INIT_VARS(unit);
    ...
    int is_ocb_only)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_voq_ocb_only_set_verify(unit, gport, cosq, is_ocb_only));
    ...
    bcm_cosq_gport_size_t * gport_size)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_threshold_verify(unit, res_type, "size_max", gport_size->size_max));
    ...
    bcm_cosq_pfc_config_t * pfc_threshold)
{
    ...
    /** Determine the port */
    if (BCM_COSQ_GPORT_IS_VSQ_SRC_PORT(gport) || BCM_COSQ_GPORT_IS_VSQ_PG(gport))
    {
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_vsq_info_get(unit, gport, &vsq_info));
        port = vsq_info.src_port;

        SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
        if (!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, MGMT)))
        {
            SHR_ERR_EXIT(_SHR_E_PORT, "Unsupported port %d, must be NIF", port);
        }
    ...
    bcm_cosq_pfc_config_t * pfc_threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_vsq_fc_threshold_set_verify(unit, gport, cosq, flags, pfc_threshold));
    ...
    bcm_cosq_pfc_config_t * pfc_threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_vsq_fc_threshold_verify(unit, gport, cosq, flags, pfc_threshold));
...
/**
 * \brief
 * Get the resource to allocate accoring to target
 */
static shr_error_e
dnx_ingress_congestion_src_vsq_resource_allocation_config_get(
    int unit,
    uint32 flags,
    int *configure_guaranteed,
    int *configure_shared_headroom)
{
    SHR_FUNC_INIT_VARS(unit);

    if (!dnx_data_ingr_congestion.vsq.feature_get(unit, dnx_data_ingr_congestion_vsq_colorless_guaranteed_support))
    {
        *configure_guaranteed = TRUE;
        *configure_shared_headroom = TRUE;
    }
    else
    {
        if (flags & BCM_COSQ_RSRC_ALLOC_NO_COLOR)
        {
            *configure_guaranteed = TRUE;
            *configure_shared_headroom = FALSE;
        }
        else
        {
            *configure_guaranteed = FALSE;
            *configure_shared_headroom = TRUE;
        }
    }
    ...
    bcm_cosq_resource_amounts_t * amounts)
{
    ...
    if (dnx_data_ingr_congestion.vsq.feature_get(unit, dnx_data_ingr_congestion_vsq_colorless_guaranteed_support))
    {
        if ((flags & BCM_COSQ_RSRC_ALLOC_NO_COLOR) && (flags & BCM_COSQ_RSRC_ALLOC_COLOR_BLIND))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "color blind cannot be configured in no-color mode\n");
        }

        if (flags & BCM_COSQ_RSRC_ALLOC_NO_COLOR)
        {
            if (amounts->max_shared_pool != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "max_shared_pool cannot be defined with BCM_COSQ_RSRC_ALLOC_NO_COLOR");
            }

            if (amounts->max_headroom != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "max_headroom cannot be defined with BCM_COSQ_RSRC_ALLOC_NO_COLOR");
            }

            if (vsq_type == DNX_INGRESS_CONGESTION_VSQ_GROUP_PG)
            {
                if (amounts->shared_pool_fadt.thresh_max != 0 || amounts->shared_pool_fadt.thresh_min != 0
                    || amounts->shared_pool_fadt.alpha != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "shared_pool_fadt cannot be defined with BCM_COSQ_RSRC_ALLOC_NO_COLOR");
                }

                if (amounts->nominal_headroom != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "nominal_headroom cannot be defined with BCM_COSQ_RSRC_ALLOC_NO_COLOR");
                }
            }
        }
        else
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "reserved should be defined with BCM_COSQ_RSRC_ALLOC_NO_COLOR");
            }
        }
    }
    ...
    bcm_cosq_resource_amounts_t * amounts)
{
    ...
    int configure_shared_headroom = FALSE, configure_guaranteed = FALSE;

    SHR_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_src_vsq_resource_allocation_set_verify(unit, flags, rsrc_type,
                                                                                         target, amounts));
    ...
    /** retrieve resource type to be configured */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_src_vsq_resource_allocation_config_get
                    (unit, flags, &configure_guaranteed, &configure_shared_headroom));
    ...
    if (configure_shared_headroom == TRUE)
    {
    ...
    if (configure_guaranteed == TRUE)
    {

        if (dnx_data_ingr_congestion.vsq.feature_get(unit, dnx_data_ingr_congestion_vsq_colorless_guaranteed_support))
        {
            /** For colorless guaranteed, configure dp #0 only */
            start_color = 0;
            end_color = 1;
        }
        ...
        SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_vsq_total_guaranteed_verify(unit, core_id, rsrc_type,
    ...
    bcm_cosq_resource_amounts_t * amounts)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_global_vsq_resource_allocation_set_verify(unit, flags, rsrc_type,
                                                                                            target, amounts));
    ...
    uint32 flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_discard_set_verify(unit, flags));
    ...
    bcm_cosq_mirror_on_drop_group_info_t * group_info)
{
    ...
    for (reject_reason_id = 0; reject_reason_id < group_info->drop_reasons_count; reject_reason_id++)
    {
        ...
        if (((int) (group_info->reasons_array[reject_reason_id]) < 0) ||
    ...
    bcm_cosq_mirror_on_drop_group_info_t * group_info)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_mirror_on_drop_group_set_verify(unit, flags, group_id, group_info));
    ...
    bcm_cosq_mirror_on_drop_group_info_t * group_info)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_mirror_on_drop_group_get_verify(unit, flags, group_id, group_info));
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_mirror_on_drop_aging_set_verify(unit, type, arg));
    ...
    int *arg)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_mirror_on_drop_aging_get_verify(unit, type, arg));
    ...
    uint8 is_set)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "This index is not used,index must be set to 0");
    }
    ...
    if (is_set)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "thresh_set and tresh_clear must have equal values");
        }
    }
        ...
        case bcmResourceDramBundleDescriptorsBuffers:
            if (is_set)
            {
                SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_threshold_encode(unit, threshold->thresh_set,
                                                                             (uint32 *)
                                                                             &threshold_internal.thresh_set));
                /** Get the appropriate DBAL table and fields */
                SHR_IF_ERR_EXIT(dnx_cosq_voq_gl_ecn_resource_to_dbal_info
                                (unit, thresh_info->resource_type, &table, &thresh_field));
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_voq_gl_ecn_marking_threshold_arg_verify
                           (unit, core_id, index_type, index_info, thresh_info, threshold, TRUE));

    /** set hw*/
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_ecn_marking_threshold_hw_set
                    (unit, core_id, thresh_info->resource_type, threshold->thresh_set));
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_ingress_congestion_voq_gl_ecn_marking_threshold_arg_verify
                           (unit, core_id, index_type, index_info, thresh_info, threshold, FALSE));
...
/** \brief - Function to verify the arguments of dnx_ingress_congestion_ocb_global_drop_profile_threshold_set/get */
static shr_error_e
dnx_ingress_congestion_ocb_global_drop_profile_threshold_verify(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
...
{
    int priority = 0;
    int nof_drop_tresholds;
    int max_sram_buffers;

    SHR_FUNC_INIT_VARS(unit);

    nof_drop_tresholds = dnx_data_ingr_congestion.sram_buffer.drop_tresholds_info_get(unit)->key_size[0];
    max_sram_buffers =
        dnx_data_ingr_congestion.info.resource_get(unit, DNX_INGRESS_CONGESTION_RESOURCE_SRAM_BUFFERS)->max;

    SHR_MASK_VERIFY(flags, 0, _SHR_E_PARAM, "flags must be 0\n");

    if (gport != 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "gport type not supported %d\n", gport);
    }

    priority = BCM_COSQ_GENERIC_KEY_PRIO_GET(key);
    SHR_RANGE_VERIFY(priority, 0, nof_drop_tresholds - 1, _SHR_E_PARAM, "Invalid priority %d\n", priority);

    if (is_set)
    {
        SHR_MAX_VERIFY(threshold, max_sram_buffers, _SHR_E_PARAM, "Invalid threshold value %d\n", threshold);
    }
...
/** see .h file */
shr_error_e
dnx_ingress_congestion_ocb_global_drop_profile_threshold_set(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
    uint32 threshold)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_ocb_global_drop_profile_threshold_verify
                    (unit, flags, gport, key, threshold, TRUE));

    priority = BCM_COSQ_GENERIC_KEY_PRIO_GET(key);
    SHR_IF_ERR_EXIT(dnx_spb_drop_threshold_set(unit, priority, threshold));
...
/** see .h file */
shr_error_e
dnx_ingress_congestion_ocb_global_drop_profile_threshold_get(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 key,
    uint32 *threshold)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_ocb_global_drop_profile_threshold_verify(unit, flags, gport, key, 0, FALSE));

    priority = BCM_COSQ_GENERIC_KEY_PRIO_GET(key);
    SHR_IF_ERR_EXIT(dnx_spb_drop_threshold_get(unit, priority, threshold));

exit:
    SHR_FUNC_EXIT;
}

/** \brief - Function to verify the arguments of dnx_ingress_congestion_port_ocb_global_drop_profile_map_set/get */
static shr_error_e
dnx_ingress_congestion_port_ocb_global_drop_profile_map_verify(
    int unit,
    uint32 flags,
    ...
    int is_set)
{
    ...
    /** verify local port / local port gport */
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
                    (unit, gport_to_map, DNX_ALGO_GPM_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info));

    if (cosq != -1)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "invalid cosq value %d\n", cosq);
    }

    if (is_set)
    {
        SHR_RANGE_VERIFY(profile_id, 0, dnx_data_ingr_congestion.sram_buffer.nof_drop_profiles_get(unit) - 1,
                         _SHR_E_PARAM, "profile id %d out of range\n", profile_id);
    }
...
/** see .h file */
shr_error_e
dnx_ingress_congestion_port_ocb_global_drop_profile_map_set(
    int unit,
    uint32 flags,
    bcm_gport_t gport_to_map,
    bcm_cos_queue_t cosq,
    int profile_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_port_ocb_global_drop_profile_map_verify
                    (unit, flags, gport_to_map, cosq, profile_id, TRUE));
    ...
    /** map profile to reassembly contexts */
    for (i = 0; i < DNX_DATA_MAX_INGR_REASSEMBLY_PRIORITY_CGM_PRIORITIES_NOF; i++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_spb_drop_reassembly_context_profile_map_set
                            (unit, core, reassembly_context[i], (uint32) profile_id));
        }
    }
...
/** see .h file */
shr_error_e
dnx_ingress_congestion_port_ocb_global_drop_profile_map_get(
    int unit,
    uint32 flags,
    bcm_gport_t gport_to_map,
    bcm_cos_queue_t cosq,
    int *profile_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_port_ocb_global_drop_profile_map_verify
                    (unit, flags, gport_to_map, cosq, 0, FALSE));
    ...
    *profile_id = -1;
    ...
    /** get profile mapped to reassembly contexts */
    for (i = 0; i < DNX_DATA_MAX_INGR_REASSEMBLY_PRIORITY_CGM_PRIORITIES_NOF; i++)
    {
        if ((reassembly_context[i] != DNX_PORT_INGR_REASSEMBLY_NON_INGRESS_PORT_CONTEXT)
            && (reassembly_context[i] != invalid_context))
        {
            SHR_IF_ERR_EXIT(dnx_spb_drop_reassembly_context_profile_map_get
                            (unit, core, reassembly_context[i], (uint32 *) profile_id));
            break;
        }
    }
    ...
    if (*profile_id < 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "logical port not mapped to reassembly context %d\n", local_port);
    }
...
/** \brief - Function to verify the arguments of dnx_ingress_congestion_ocb_global_drop_profile_priority_map_set */
shr_error_e
dnx_ingress_congestion_ocb_global_drop_profile_priority_map_verify(
    int unit,
    uint32 flags,
    bcm_gport_t profile,
    uint32 key,
    ...
    int is_set)
{
    ...
    if (!BCM_GPORT_IS_OCB_GLOBAL_DROP_PROFILE(profile))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "illegal profile gport type %d\n", profile);
    }

    tc = BCM_COSQ_GENERIC_KEY_TC_DP_TC_GET(key);
    SHR_RANGE_VERIFY(tc, 0, DNX_COSQ_NOF_TC - 1, _SHR_E_PARAM, "Invalid TC %d\n", tc);

    dp = BCM_COSQ_GENERIC_KEY_TC_DP_DP_GET(key);
    SHR_RANGE_VERIFY(dp, 0, DNX_COSQ_NOF_DP - 1, _SHR_E_PARAM, "Invalid DP %d\n", dp);

    if (is_set)
    {
        SHR_MAX_VERIFY(priority, nof_drop_tresholds - 1, _SHR_E_PARAM, "Invalid priority %d\n", priority);
    }
...
/** see .h file */
shr_error_e
dnx_ingress_congestion_ocb_global_drop_profile_priority_map_set(
    int unit,
    uint32 flags,
    bcm_gport_t profile,
    uint32 key,
    uint32 priority)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_ocb_global_drop_profile_priority_map_verify
                    (unit, flags, profile, key, priority, TRUE));

    profile_id = BCM_GPORT_OCB_GLOBAL_DROP_PROFILE_GET(profile);
    tc = BCM_COSQ_GENERIC_KEY_TC_DP_TC_GET(key);
    dp = BCM_COSQ_GENERIC_KEY_TC_DP_DP_GET(key);

    SHR_IF_ERR_EXIT(dnx_spb_drop_profile_priority_map_set(unit, profile_id, tc, dp, priority));
...
/** see .h file */
shr_error_e
dnx_ingress_congestion_ocb_global_drop_profile_priority_map_get(
    int unit,
    uint32 flags,
    bcm_gport_t profile,
    uint32 key,
    uint32 *priority)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_ocb_global_drop_profile_priority_map_verify
                    (unit, flags, profile, key, 0, FALSE));

    profile_id = BCM_GPORT_OCB_GLOBAL_DROP_PROFILE_GET(profile);
    tc = BCM_COSQ_GENERIC_KEY_TC_DP_TC_GET(key);
    dp = BCM_COSQ_GENERIC_KEY_TC_DP_DP_GET(key);

    SHR_IF_ERR_EXIT(dnx_spb_drop_profile_priority_map_get(unit, profile_id, tc, dp, priority));
...
/** see h file */
uint32
dnx_ingress_congestion_calc_available_total_bytes(
    uint32 max_total_bytes,
    uint32 nof_active_drams,
    uint32 max_nof_drams)
{
    COMPILER_UINT64 temp_calc;

    if (nof_active_drams == 0)
    {
        return max_total_bytes;
    }
    else
    {
        COMPILER_64_SET(temp_calc, 0, max_total_bytes);
        COMPILER_64_UMUL_32(temp_calc, nof_active_drams);
        COMPILER_64_UDIV_32(temp_calc, max_nof_drams);

        return COMPILER_64_LO(temp_calc);
    }
}
