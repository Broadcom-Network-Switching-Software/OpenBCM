    ...
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle)
{
    ...
    SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_port_profile.exchange
                    (unit, port_profile_handle->core, 0, 1, &info, old_profile, NULL,
    ...
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle)
{
    ...
    if (!dnx_data_ecgm.queue_resources.feature_get(unit, dnx_data_ecgm_queue_resources_no_profile_management))
    {
        ...
        nof_offsets = DNX_COSQ_NOF_TC;
    }
    else
    {
        base_qpair_or_port_profile = port_profile_handle->base_q_pair;
        nof_offsets = port_profile_handle->num_priorities;
    }

    /*
     * set port profile info per unicast and multicast queue
     */
    for (queue_offset = 0; queue_offset < nof_offsets; ++queue_offset)
    {
    ...
    dnx_ecgm_profile_mgmt_interface_handle_t * interface_profile_handle)
{
    ...
    SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_interface_profile.exchange
                    (unit, interface_profile_handle->core, 0, 1, &interface_profile_handle->info, old_profile,
