...
static shr_error_e dnx_ofp_rates_calcal_config(
    int unit,
    int core,
    dnx_ofp_rates_cal_set_e cal2set,
    int alt_cal_alg);

static shr_error_e dnx_ofp_rates_set_select_get(
    int unit,
    int core,
    dnx_ofp_rates_egq_cal_type_e cal_type,
    uint32 *set_select_p);

static shr_error_e switch_active_chan_arb_calendar(
    int unit,
    int core);
/* } */
...
/**
 * \brief
 *   This function clear the OTM Calendar, setting the default
 *   values for Max burst and all other fields to '0'
 *
 * \param [in] unit -Identifier of HW platform.
 * \param [in] core - Core to apply new setup for.
 * \param [in] cal_set - Calendar to set
 *
 * \return
 *   \retval Zero - On success
 *   \retval Error - Identifier as per shr_error_e
 ...
 */
static shr_error_e
dnx_ofp_rates_otm_calendar_clear(
    int unit,
    int core,
    dnx_ofp_rates_cal_set_e cal_set)
{

    uint32 hp_default_max_burst;
    uint32 lp_default_max_burst;

    SHR_FUNC_INIT_VARS(unit);

        /**
         * Set default values to the EGQ_SHAPING_OTM_CRDT_TABLE
         * For HP set number of bytes per burst multiplied by a conversion coefficient between credits and bits.
         * For LP burst size needs to be multiplied by 2.
         */
    hp_default_max_burst = DNX_EGQ_MAX_BURST_IN_BYTES * dnx_data_egr_queuing.params.cal_burst_res_get(unit);
    lp_default_max_burst = hp_default_max_burst * 2;
    SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_SHAPING_OTM_CRDT_TABLE, 3, 4,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, core,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CALENDAR_SET, cal_set,
                                     GEN_DBAL_RANGE32, DBAL_FIELD_CHAN_ARB_CALENDAR, DBAL_RANGE_ALL, DBAL_RANGE_ALL,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_OTM_MAX_BURST, INST_SINGLE, hp_default_max_burst,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_OTM_LP_MAX_BURST, INST_SINGLE, lp_default_max_burst,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_OTM_INDEX, INST_SINGLE, 0,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_OTM_CRDT_TO_ADD, INST_SINGLE, 0,
                                     GEN_DBAL_FIELD_LAST_MARK));
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 internal_rate)
{
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_SHAPER_OTM_CALENDAR_CRDT_TABLE, 3, 1,
                                     ...
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CALENDAR_SET, cal2set,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, core,
                                     /** values */
...
/*
 * \brief -
 *   Get from HW the internal rate for a specified interface.
 * \param [in] unit -
...
dnx_ofp_rates_if_calendar_internal_rate_get(
    int unit,
    int core,
    dnx_ofp_rates_cal_set_e cal2get,
    int calendar_id,
    uint32 *internal_rate)
{
    uint32 org_if_val, org_otm_val;
     ...
     */
    SHR_IF_ERR_EXIT(dnx_dbal_gen_get(unit, DBAL_TABLE_EGQ_SHAPER_OTM_CALENDAR_CRDT_TABLE, 3, 1,
                                     /** keys */
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, core,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CALENDAR_ID, calendar_id,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CALENDAR_SET, (uint32) (cal2get),
                                     /** values */
    ...
    if (dnx_data_egr_queuing.general.feature_get(unit, dnx_data_egr_queuing_general_is_used))
    {
        ...
        }
        {
            ...
            SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_SHAPER_OTM_CALENDAR_CRDT_TABLE,
                                             ...
                                             GEN_DBAL_RANGE32, DBAL_FIELD_CORE_ID, DBAL_RANGE_ALL, DBAL_RANGE_ALL,
                                             GEN_DBAL_FIELD32, DBAL_FIELD_OTM_CAL_CRDT_TO_ADD, INST_SINGLE,
                                             field32_value, GEN_DBAL_FIELD32, DBAL_FIELD_OTM_CAL_MAX_BURST, INST_SINGLE,
                                             field32_value, GEN_DBAL_FIELD_LAST_MARK));
        }
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
        {
            /**
             * Clear the OTM calendar and set default values for max burst
             */
            for (cal_set = 0; cal_set < DNX_OFP_NOF_RATES_CAL_SETS; cal_set++)
            {
                SHR_IF_ERR_EXIT(dnx_ofp_rates_otm_calendar_clear(unit, core, cal_set));
            }
        ...
        }
        {
            ...
            SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_SHAPER_GLOBAL_CONFIGURATION,
                                             ...
                                             GEN_DBAL_RANGE32, DBAL_FIELD_CORE_ID, DBAL_RANGE_ALL, DBAL_RANGE_ALL,
                                             GEN_DBAL_FIELD32, DBAL_FIELD_QP_CRDT_TABLE_READ_EN, INST_SINGLE,
                                             field32_value, GEN_DBAL_FIELD32, DBAL_FIELD_OTM_CRDT_TABLE_READ_EN,
                                             INST_SINGLE, field32_value, GEN_DBAL_FIELD32,
                                             DBAL_FIELD_TCG_CRDT_TABLE_READ_EN, INST_SINGLE, field32_value,
                                             GEN_DBAL_FIELD_LAST_MARK));
        }
    }
 ...
 *   This is done in case of setting BW for a channelized local port, because in
 ...
 * \param [in] cal2set -
 *   Int. Calendar to set, must be the non-active one..
 * \return
 *   If zero (_SHR_E_NONE), then no error was encountered.
 *   Otherwise, see shr_error_e
 * \remark
    ...
    dnx_ofp_rates_cal_set_e cal2set)
{
    ...
    dnx_ofp_rates_cal_set_e act_cal;
    ...
    /** Determine the active calendar */
    act_cal = ((cal2set == DNX_OFP_RATES_CAL_SET_A) ? DNX_OFP_RATES_CAL_SET_B : DNX_OFP_RATES_CAL_SET_A);
...
static shr_error_e
dnx_ofp_rates_if_calendar_copy(
    int unit,
    int core,
    dnx_ofp_rates_cal_set_e cal2set)
{
    ...
    for (calendar_id = 0; calendar_id < dnx_data_egr_queuing.params.nof_calendars_get(unit); calendar_id++)
    {
        /** Get interface shaper rate from the active calendar */
        SHR_IF_ERR_EXIT(dnx_ofp_rates_if_calendar_internal_rate_get(unit, core, act_cal, calendar_id, &internal_rate));
        /** Set interface shaper rate to the inactive calendar */
        SHR_IF_ERR_EXIT(dnx_ofp_rates_if_calendar_internal_rate_set(unit, core, cal2set, calendar_id, internal_rate));
    }
    ...
    int alt_cal_alg)
{
    ...
    bcm_pbmp_t port_bm;
    int calendar_id;
    dnx_ofp_rates_cal_set_e active_cal;
    dnx_ofp_rates_cal_set_e cal2set;
    SHR_FUNC_INIT_VARS(unit);

    /** Determine the non-active calendar */
    SHR_IF_ERR_EXIT(dnx_ofp_rates_set_select_get(unit, core, DNX_OFP_RATES_EGQ_CAL_CHAN_ARB, &active_cal));
    cal2set = (active_cal == DNX_OFP_RATES_CAL_SET_A) ? DNX_OFP_RATES_CAL_SET_B : DNX_OFP_RATES_CAL_SET_A;

    /** If if_recalc is TRUE need to recalculate calcal and interface shapers */
    if (if_recalc == TRUE)
    {
        /** recalculate calcal */
        SHR_IF_ERR_EXIT(dnx_ofp_rates_calcal_config(unit, core, cal2set, alt_cal_alg));
        ...
        BCM_PBMP_ITER(port_bm, logical_port)
        {
            ...
            if (!COMPILER_64_IS_ZERO(cal_rate))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ofp_rates_calculate_credits_from_rate
                                (unit, calcal_length, calcal_instances, 1, (uint32) packet_mode, cal_rate,
                                 &internal_rate));

                /** Set interface shaper rate */
                SHR_IF_ERR_EXIT(dnx_ofp_rates_if_calendar_internal_rate_set
                                (unit, core, cal2set, calendar_id, internal_rate));
            }
        }
    }
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ofp_rates_calcal_copy(unit, core, cal2set));

        /** Copy existing IF Shaper */
        SHR_IF_ERR_EXIT(dnx_ofp_rates_if_calendar_copy(unit, core, cal2set));
    }

    /** Recalculate all port shapers */
    SHR_IF_ERR_EXIT(dnx_ofp_rates_port_rate_set(unit, core, cal2set));

    /** Switch the active calendar */
    SHR_IF_ERR_EXIT(switch_active_chan_arb_calendar(unit, core));
    ...
    uint32 *if_shaper_rate_p)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ofp_rates_if_calendar_internal_rate_get(unit, core, active_cal, calendar_id, &internal_rate));
    /*
    ...
    /** Get packet mode in order to correctly translate internal rate (credits/cycle) to Kbps rate*/
    SHR_IF_ERR_EXIT(dnx_egr_queuing_interface_packet_mode_get(unit, logical_port, &packet_mode));

    /*
     * Translate from internal rate (credits/cycle) to Kbps
     */
    SHR_IF_ERR_EXIT(dnx_ofp_rates_calculate_rate_from_credits
                    (unit, calcal_length, calcal_instances, 1, packet_mode, &internal_rate));
            ...
            case DNX_OFP_RATES_EGQ_CAL_CHAN_ARB:
            {
                ...
                *egq_cal_len_p = (1 + *egq_cal_len_p);
                    ...
                    params.feature_get(unit, dnx_data_egr_queuing_params_calendar_low_range_config))
                {
                    dbal_field_id =
                        (cal2set ==
                         DNX_OFP_RATES_CAL_SET_A) ? DBAL_FIELD_PORT_OFFSET_LOW_A : DBAL_FIELD_PORT_OFFSET_LOW_B;
                    SHR_IF_ERR_EXIT(dnx_dbal_gen_get(unit, DBAL_TABLE_EGQ_SHAPER_OTM_SHAPER_LENGTH,
                                                     2, 1,
                                                     GEN_DBAL_FIELD32, DBAL_FIELD_CALENDAR_ID, (uint32) (offset),
                                                     GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, core,
                ...
                else
                {
                    if (offset)
                    {
                        ...
                        SHR_IF_ERR_EXIT(dnx_dbal_gen_get(unit, DBAL_TABLE_EGQ_SHAPER_OTM_SHAPER_LENGTH,
                                                         ...
                                                         (uint32) (offset - 1), GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID,
                                                         core, GEN_DBAL_FIELD32, dbal_field_id, INST_SINGLE,
                                                         &field32_value, GEN_DBAL_FIELD_LAST_MARK));
    ...
    uint32 egq_cal_len)
{
    uint32 field32_value;
    uint32 offset;
    dbal_fields_e dbal_field_id, dbal_field_low_id;
    ...
    switch (cal_info_p->cal_type)
    {
        case DNX_OFP_RATES_EGQ_CAL_CHAN_ARB:
        {
            ...
            offset = cal_info_p->chan_arb_id;
            if (dnx_data_egr_queuing.params.feature_get(unit, dnx_data_egr_queuing_params_calendar_low_range_config))
            {
                dbal_field_low_id =
                    (cal2set == DNX_OFP_RATES_CAL_SET_A) ? DBAL_FIELD_PORT_OFFSET_LOW_A : DBAL_FIELD_PORT_OFFSET_LOW_B;
                field32_value = cal_info_p->arb_slot_id;
                SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_SHAPER_OTM_SHAPER_LENGTH,
                                                 2, 1,
                                                 GEN_DBAL_FIELD32, DBAL_FIELD_CALENDAR_ID, (uint32) (offset),
                                                 GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, core,
                                                 GEN_DBAL_FIELD32, dbal_field_low_id, INST_SINGLE, field32_value,
                                                 GEN_DBAL_FIELD_LAST_MARK));
            }
            else
            {
    ...
    uint32 *offset_of_calendar_start_p)
{
    ...
    *offset_of_calendar_start_p = 0;
    ...
    {
        dbal_field_id =
            (cal_set == DNX_OFP_RATES_CAL_SET_A) ? DBAL_FIELD_PORT_OFFSET_LOW_A : DBAL_FIELD_PORT_OFFSET_LOW_B;
        SHR_IF_ERR_EXIT(dnx_dbal_gen_get(unit, DBAL_TABLE_EGQ_SHAPER_OTM_SHAPER_LENGTH,
                                         2, 1,
                                         GEN_DBAL_FIELD32, DBAL_FIELD_CALENDAR_ID, (uint32) (cal_info_p->chan_arb_id),
                                         GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, core,
                                         GEN_DBAL_FIELD32, dbal_field_id, INST_SINGLE, &offset,
                                         GEN_DBAL_FIELD_LAST_MARK));
    }
    else
    {
        if ((cal_info_p->cal_type == DNX_OFP_RATES_EGQ_CAL_CHAN_ARB) && (cal_info_p->chan_arb_id > 0))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_dbal_gen_get(unit, DBAL_TABLE_EGQ_SHAPER_OTM_SHAPER_LENGTH, 2, 1,
                                              ...
                                              */
                                             GEN_DBAL_FIELD32, DBAL_FIELD_CALENDAR_ID,
                                             (uint32) (cal_info_p->chan_arb_id - 1), GEN_DBAL_FIELD32,
                                             DBAL_FIELD_CORE_ID, core,
                                             /*
    ...
    uint32 *port_credits_p)
{
    ...
    if (COMPILER_64_IS_ZERO(rate_kbps))
    {
        *port_credits_p = 0;
        SHR_EXIT();
    }
     ...
     */
    COMPILER_64_COPY(u64_1, rate_kbps);
    ...
    uint32 *ports_rates)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ofp_rates_calculate_rate_from_credits
                    (unit, calcal_length, calcal_instances, calendar_len, packet_mode, &ports_rates[0]));
    ...
    int alt_cal_alg)
{
    uint32 idx;
    uint32 chan_arb_i;
    uint64 egq_calcal_chan_arb_rate_requested;
    uint32 egq_calcal_calendar_len_actual = 0;
    uint32 egq_calcal_calendar_len_for_hw = 0;
    uint64 egq_calcal_rates[DNX_DATA_MAX_EGR_QUEUING_PARAMS_NOF_CALENDARS];
    ...
    dbal_fields_e dbal_field_id;
    SHR_FUNC_INIT_VARS(unit);
    SHR_ALLOC_SET_ZERO(egq_calcal_calendar_p, sizeof(*egq_calcal_calendar_p), "egq_calcal_calendar_p", "%s%s%s\r\n",
                       EMPTY, EMPTY, EMPTY);

    /** Clear */
    COMPILER_64_ZERO(egq_calcal_chan_arb_rate_requested);
    ...
    for (chan_arb_i = 0; chan_arb_i < dnx_data_egr_queuing.params.nof_calendars_get(unit); chan_arb_i++)
    {
        ...
        COMPILER_64_ADD_64(egq_calcal_chan_arb_rate_requested, egq_calcal_rates[chan_arb_i]);
    }

    /** Convert rates to calendar only if the total rate that is requested is different than 0.
      * In case of egq_calcal_chan_arb_rate_requested, the slots, cal len and calcal onstances will all be 0 too.
      */
    if (egq_calcal_chan_arb_rate_requested != 0)
    {
        SHR_IF_ERR_EXIT(dnx_algo_cal_simple_from_rates_to_calendar
                        (unit, alt_cal_alg, egq_calcal_rates, (dnx_data_egr_queuing.params.nof_calendars_get(unit)),
                         egq_calcal_chan_arb_rate_requested, DNX_OFP_RATES_CALCAL_LEN_EGQ_MAX,
                         egq_calcal_calendar_p->slots, &(egq_calcal_calendar_len_actual)));
    }
    ...
    /** The value that is written to HW represents the last valid calcal index, so it is actual_len-1 */
    egq_calcal_calendar_len_for_hw = (egq_calcal_calendar_len_actual == 0) ? 0 : (egq_calcal_calendar_len_actual - 1);
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_SHAPER_GLOBAL_CONFIGURATION, 1, 1,
                                     ...
                                     (uint32) egq_calcal_calendar_len_for_hw, GEN_DBAL_FIELD_LAST_MARK));
    SHR_IF_ERR_EXIT(dnx_ofp_rate_db.calcal_len.set(unit, core, egq_calcal_calendar_len_actual));
    ...
    for (idx = 0; idx < egq_calcal_calendar_len_actual; ++idx)
    {
 ...
 */
static shr_error_e
switch_active_chan_arb_calendar(
    int unit,
    int core)
{
    /*
    ...
    /*
     * Set the non-active calendar to be active
     */
    SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, DBAL_TABLE_EGQ_SHAPER_GLOBAL_CONFIGURATION, 1, 1,
                                     GEN_DBAL_FIELD32, DBAL_FIELD_CORE_ID, core,
 ...
 *   Core to apply new setup for.
 * \param [in] cal2set -
 *   calendar to set, should be the non-active one.
 * \return
 *   If zero (_SHR_E_NONE), then no error was encountered.
 *   Otherwise, see shr_error_e
 * \remark
    ...
    dnx_ofp_rates_cal_set_e cal2set)
{
    ...
    /** Reset non-active calendar to default */
    SHR_IF_ERR_EXIT(dnx_ofp_rates_otm_calendar_clear(unit, core, cal2set));
        ...
        if (nof_instances > 0)
        {
            ...
            if (cal_slots)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ofp_rates_egress_shaper_cal_len_set
                                (unit, core, &cal_info, cal2set, egq_calendar_len));
                ...
                for (cal_slots = 0; cal_slots < egq_calendar_len; cal_slots++)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, logical_port_i, &core));
                    SHR_IF_ERR_EXIT(dnx_algo_port_base_q_pair_get(unit, logical_port_i, &base_q_pair));
                    ...
                    COMPILER_64_SET(egq_rate64, 0, egq_rate);
                    SHR_IF_ERR_EXIT(dnx_ofp_rates_calculate_credits_from_rate
                                    (unit, calcal_length, calcal_instances, egq_calendar_len, (uint32) packet_mode,
                                     egq_rate64, &(cal_entry.credit)));
                    ...
                    SHR_IF_ERR_EXIT(dnx_ofp_rates_calendar_entry_set
                                    (unit, core, cal_info.cal_type, cal2set, (offset + cal_slots), &egq_data));
                }
            }
        }
    ...
    uint32 max_burst)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ofp_rates_egq_tcg_shaper_verify(unit,
                                                               core,
                                                               logical_port,
                                                               tcg_ndx,
                                                               max_burst,
    ...
    uint32 tcg_rate)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_ofp_rates_egq_tcg_shaper_verify(unit,
                                                               ...
                                                               tcg_ndx,
                                                               tcg_rate,
                                                               DNX_OFP_RATES_GENERIC_FUNC_STATE_TCG_EGQ_RATE));
    {
        /*
         * Getting the fap port's base_q_pair
         */
    ...
    int core)
{
    ...
    SHR_ALLOC(egq_calendar, sizeof(dnx_ofp_rates_cal_egq_t), "egq_calendar", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, port_i, &core_i));
...
{
    int field_size;
    uint32 new_comp_hw = 0;
    uint32 old_comp_hw = 0;
    uint32 entry_handle_id;
    uint32 entry_handle_id2;
    int old_comp = 0;
    int nof_ports_negative_comp = 0;
    uint32 allow_above_max = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /** Get compensation field size. The value in HW is in two's compliment, so we need the
     * size in order to convert to signed number. */
    ...
    /** Calculate the new compensation in two's compliment */
    new_comp_hw = UTILEX_SIGNED_NUM_TO_TWO_COMPLEMENT_METHOD(comp, field_size);

    if (dnx_data_egr_queuing.
        params.feature_get(unit, dnx_data_egr_queuing_params_credits_above_max_burst_for_negative_comp))
    {
        /** Get the old compensation from HW. */
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CR_ADJUST_VALUE, INST_SINGLE, &old_comp_hw);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

        /** Calculate the old compensation in signed number */
        old_comp = UTILEX_TWO_COMPLEMENT_INTO_SIGNED_NUM(old_comp_hw, field_size);

        /** If the old compensation was negative and new compensation is positive, or
         * the old compensation is positive and new compensation is negative, adjust the SW counter.
         * If both old and new compensations are with the same sign, no need to update. */
        if (((old_comp < 0) && (comp >= 0)) || ((old_comp >= 0) && (comp < 0)))
        {
             /** Get the the number of ports with negative compensation on the current core. */
            SHR_IF_ERR_EXIT(dnx_egr_db.nof_negative_compensation_ports.get(unit, core, &nof_ports_negative_comp));

             /** Update the SW counter */
            nof_ports_negative_comp = ((old_comp < 0)
                                       && (comp >= 0)) ? (nof_ports_negative_comp - 1) : (nof_ports_negative_comp + 1);

             /** If ports with negative compensation exist on the current core, need to not allow shapers
              * to accumulate credits above max burst */
            allow_above_max = (nof_ports_negative_comp > 0) ? FALSE : TRUE;
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_EGQ_SHAPER_GLOBAL_CONFIGURATION, entry_handle_id));
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
            /** The below fields are relevant only in case burst_size_below_mtu
             * is disabled. Else, this configuration is set to 0 on init and should not be changed. */
            if (!dnx_data_egr_queuing.params.burst_size_below_mtu_get(unit))
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OTM_TOTAL_ALLOW_ACC_ABOVE_MAX,
                                             INST_SINGLE, allow_above_max);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OTM_ALLOW_ACC_ABOVE_MAX, INST_SINGLE,
                                             allow_above_max);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_QP_ALLOW_ACC_ABOVE_MAX, INST_SINGLE,
                                             allow_above_max);
            }
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TCG_ALLOW_ACC_ABOVE_MAX, INST_SINGLE,
                                         allow_above_max);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

             /** Set the the number of ports with negative compensation on the current core. */
            SHR_IF_ERR_EXIT(dnx_egr_db.nof_negative_compensation_ports.set(unit, core, nof_ports_negative_comp));
        }
    }

    /** Set the new compensation to HW. */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGQ_SHAPING_OTM_ATTRIBUTES, &entry_handle_id2));
    /** Key construction */
    dbal_entry_key_field32_set(unit, entry_handle_id2, DBAL_FIELD_QPAIR, base_q_pair);
    dbal_entry_key_field32_set(unit, entry_handle_id2, DBAL_FIELD_CORE_ID, core);
    /** Write the value to HW in 2's complement */
    dbal_entry_value_field32_set(unit, entry_handle_id2, DBAL_FIELD_CR_ADJUST_VALUE, INST_SINGLE, new_comp_hw);
...
dnx_egr_port_compensation_set(
    ...
    int comp)
{
    int base_q_pair, egq_if;
    bcm_core_t core;
    SHR_FUNC_INIT_VARS(unit);

    /** Get core and Egress ID of the port */
    SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, logical_port, &core));
    SHR_IF_ERR_EXIT(dnx_algo_port_egr_if_get(unit, logical_port, &egq_if));

    /** Set compensation on the base Q-pair of the port */
    SHR_IF_ERR_EXIT(dnx_algo_port_base_q_pair_get(unit, logical_port, &base_q_pair));
    ...
    int *comp_p)
{
    ...
    /** Get the base Q-pair and core of the port */
    SHR_IF_ERR_EXIT(dnx_algo_port_base_q_pair_get(unit, logical_port, &base_q_pair));
    SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, logical_port, &core));
    ...
    uint32 flags)
{
    ...
    if ((flags & ~(BCM_COSQ_BW_NOT_COMMIT | BCM_COSQ_BW_ALT_CALC)) != 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     ...
                     "The only supported flags are 0x%08X", flags,
                     (uint32) BCM_COSQ_BW_NOT_COMMIT | BCM_COSQ_BW_ALT_CALC);
    }
    if (kbits_sec_max > DNX_IF_MAX_RATE_KBPS(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Invalid parameter: kbits_sec_max: %u. Maximal value allowed is %u",
    ...
    uint32 flags)
{
    ...
    int base_q_pair;
    int calendar_id;
    uint32 prev_kbits_sec_max;
    uint64 total_rate;
    uint64 prev_kbits_sec_max64;
    SHR_FUNC_INIT_VARS(unit);

    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_egq_ofp_rates_port_bandwidth_set_verify
    ...
    if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
    {
        ...
        if (!channelized)
        {
            ...
            COMPILER_64_SET(prev_kbits_sec_max64, 0, prev_kbits_sec_max);
            if (COMPILER_64_LT(total_rate, prev_kbits_sec_max64))
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "total_rate(=%d) <  prev_kbits_sec_max(=%d)", COMPILER_64_LO(total_rate),
                             prev_kbits_sec_max);
            }
            ...
            COMPILER_64_SUB_32(total_rate, prev_kbits_sec_max);
            COMPILER_64_ADD_32(total_rate, kbits_sec_max);
        ...
        if (!(flags & BCM_COSQ_BW_NOT_COMMIT))
        {
            ...
            DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ofp_rates_egq_otm_shapers_set(unit, core, if_recalc,
                                                                  (flags & BCM_COSQ_BW_ALT_CALC ? TRUE : FALSE)));
            }
        }
    }
exit:
