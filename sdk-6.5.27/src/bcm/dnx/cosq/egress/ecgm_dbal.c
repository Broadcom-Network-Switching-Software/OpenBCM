...
static shr_error_e dnx_ecgm_dbal_internal_out_tm_port_to_port_profile_set(
    uint32 unit,
    uint32 core_id,
    uint32 out_tm_port,
    uint32 port_profile);
    ...
    uint32 mc_pd_sp)
{
    ...
    if (!dnx_data_ecgm.queue_resources.feature_get(unit, dnx_data_ecgm_queue_resources_no_profile_management))
    {
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ECGM_PORT_PROFILE, base_qpair_or_port_profile);
    ...
    else
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, DBAL_TABLE_ECGM_MULTICAST_QUEUE_MAP_TO_SP_WO_PROFILE, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_QPAIR, base_qpair_or_port_profile + queue_offset);
    }
    ...
    uint32 interface_profile)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_EGQ_IF_WO_FLEXE, egq_interface);
    ...
    uint32 pd_drop)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    if (!dnx_data_ecgm.
        interface_resources.feature_get(unit, dnx_data_ecgm_interface_resources_per_priority_is_not_supported))
    {
...
*/
shr_error_e
dnx_ecgm_dbal_out_tm_port_to_port_profile_set(
    uint32 unit,
    uint32 core_id,
    uint32 out_tm_port,
    uint32 base_q_pair,
    uint32 port_profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_internal_out_tm_port_to_port_profile_set(unit, core_id, out_tm_port, port_profile));
...
*/
shr_error_e
dnx_ecgm_dbal_out_tm_port_to_port_profile_get(
    uint32 unit,
    uint32 core_id,
    uint32 out_tm_port,
    uint32 *port_profile)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ECGM_OUT_TM_PORT_TO_PORT_PROFILE_MAP, &entry_handle_id));
    ...
    uint32 reserved_pd_drop)
{
    ...
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, DBAL_TABLE_ECGM_MULTICAST_QUEUE_TC_THRESHOLDS_WO_PROFILE, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_QPAIR, base_qpair_or_port_profile + queue_offset);
    }
...
dnx_ecgm_dbal_unicast_port_thresholds_set(
    uint32 unit,
    uint32 core_id,
    uint32 out_tm_port_or_portprofile,
    const dnx_ecgm_uc_thresholds_t * thresholds)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ECGM_PORT_PROFILE, out_tm_port_or_portprofile);
    ...
    const dnx_ecgm_uc_thresholds_t * thresholds)
{
    ...
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ECGM_UNICAST_QUEUE_THRESHOLDS_WO_PROFILE, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_QPAIR, base_qpair_or_port_profile + queue_offset);
    }
...
dnx_ecgm_dbal_multicast_port_thresholds_set(
    uint32 unit,
    uint32 core_id,
    uint32 tm_port_or_port_profile,
    const dnx_ecgm_mc_drop_thresholds_t * thresholds)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ECGM_PORT_PROFILE, tm_port_or_port_profile);
    ...
    const dnx_ecgm_mc_drop_thresholds_t * thresholds)
{
    ...
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, DBAL_TABLE_ECGM_MULTICAST_QUEUE_TC_DP_THRESHOLDS_WO_PROFILE, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_QPAIR, base_qpair_or_port_profile + queue_offset);
    }
...
{
    uint32 entry_handle_id;
    uint32 bubble_pulse_width_default;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /*
     * Getting the default bubble configuration. 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGQ_GLOBAL_CONFIGURATION, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_ECGM_FORCE_BUBBLE_PULSE_WIDTH, INST_SINGLE,
                               &bubble_pulse_width_default);

    /*
     * Setting the higher bubble value to ensure proper access during high traffic. 
     */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ECGM_FORCE_BUBBLE_PULSE_WIDTH, INST_SINGLE,
                                 dnx_data_egr_queuing.params.bubble_pulse_width_for_polling_get(unit));
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    /*
     * Polling of queue pd count. 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_ECGM_QUEUE_PD_COUNT, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_QPAIR, queue_pair);
    ...
    /*
     * Restore the default bubble configuration.
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_EGQ_GLOBAL_CONFIGURATION, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ECGM_FORCE_BUBBLE_PULSE_WIDTH, INST_SINGLE,
                                 bubble_pulse_width_default);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
static shr_error_e
dnx_ecgm_dbal_internal_out_tm_port_to_port_profile_set(
    uint32 unit,
    uint32 core_id,
    uint32 out_tm_port,
    uint32 port_profile)
...
shr_error_e
dnx_ecgm_dbal_global_fc_indications_get(
    uint32 unit,
    uint32 core_id,
    uint8 *device,
    uint8 *erp,
    uint8 *erp_tc)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ECGM_EGQ_2_SCH_GLOBAL_FC_DEBUG, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_DEVICE_FC_INDICATION, INST_SINGLE, device);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_ERP_FC_INDICATION, INST_SINGLE, erp);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_ERP_TC_FC_INDICATION, INST_SINGLE, erp_tc);
...
shr_error_e
dnx_ecgm_dbal_per_qpair_fc_indication_get(
    uint32 unit,
    uint32 core_id,
    uint32 qpair,
    uint8 *fc_indication)
{
    ...
    select = qpair / dnx_data_ecgm.info.fc_q_pair_vector_nof_presented_get(unit);
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ECGM_EGQ_2_SCH_QPAIR_FC_DEBUG_CFG, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SELECT, INST_SINGLE, select);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ECGM_EGQ_2_SCH_QPAIR_FC_DEBUG, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_QPAIR, qpair);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_FC_INDICATION, INST_SINGLE, fc_indication);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
...
shr_error_e
dnx_ecgm_dbal_per_interface_fc_indication_get(
    uint32 unit,
    uint32 core_id,
    uint32 interface,
    uint32 priority,
    uint8 *fc_indication)
{
    ...
    if (dnx_data_ecgm.info.feature_get(unit, dnx_data_ecgm_info_fc_egr_if_vector_select))
    {
        select =
            (interface +
             priority * (dnx_data_egr_queuing.params.first_lp_egr_interface_get(unit) -
                         dnx_data_egr_queuing.params.nof_dedicated_flexe_interfaces_get(unit))) /
            dnx_data_ecgm.info.fc_egr_if_vector_nof_presented_get(unit);
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ECGM_EGQ_2_SCH_INTERFACE_FC_DEBUG_CFG, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SELECT, INST_SINGLE, select);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ECGM_EGQ_2_SCH_INTERFACE_FC_DEBUG, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_EGQ_IF, interface);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ECGM_PRIORITY, priority);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_FC_INDICATION, INST_SINGLE, fc_indication);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
exit:
