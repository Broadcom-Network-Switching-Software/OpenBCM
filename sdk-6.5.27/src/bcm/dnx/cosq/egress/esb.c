    ...
    uint32 *esb_tdm_or_lp_queue_id)
{
    ...
    */
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, 0, 0, 0)
...
/**
 * \brief - Power down the ESB modules
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit -  Unit-ID
    ...
    int power_down)
{
   ...
   /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ESB_SETTINGS, &entry_handle_id));
    ...
    uint32 unit)
{
    ...
    if (!dnx_data_esb.general.feature_get(unit, dnx_data_esb_general_is_used))
    {
        if (dnx_data_esb.general.feature_get(unit, dnx_data_esb_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_esb_power_down_set(unit, 1));
        }
    }
    else
    {
    ...
    uint8 is_tx_preemption)
{
    ...
    int nif_if, egr_if_hp, egr_if_to_config;
    bcm_port_t master_port;
    int nif_unit_id, internal_offset;
    ...
    while (current_queue != ESB_QUEUE_ID_INVALID)
    {
            ...
            egr_if_to_config =
                (current_queue ==
                 esb_tdm_or_lp_queue_id) ? (egr_if_hp +
                                            dnx_data_egr_queuing.params.first_lp_egr_interface_get(unit)) : egr_if_hp;
        }
        else
        {
         ...
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ESB_EGQ_IF_2_Q_MAP, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_Q_NUM, INST_SINGLE,
                                     ((is_tx_preemption == TRUE) ? current_queue : esb_queue_id));
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 * \param [in] unit -  Unit-ID
 * \param [in] logical_port - Logical port
 * \param [in] is_tx_preemption - Is preemption enabled for the port
 *
 * \return
 *   shr_error_e
 * \remark
    ...
    uint8 is_tx_preemption)
{
    ...
    */
     ...
     */
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, 0) || DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, 0, 0, 0))
    {
        SHR_IF_ERR_EXIT(dnx_esb_to_fqp_mapping_set(unit, logical_port, is_tx_preemption));
    }
...
/**
 * \brief - calculate ESB txi irdy threshold
 */
static shr_error_e
dnx_esb_port_txi_irdy_thr_calc(
    uint32 unit,
    bcm_port_t logical_port,
    int is_add,
    uint32 *txi_irdy_th)
    ...
    int is_add)
{
    ...
    /** calcualte new threshold according to speed */
    SHR_IF_ERR_EXIT(dnx_esb_port_txi_irdy_thr_calc(unit, logical_port, is_add, &txi_irdy_th));
    ...
    bcm_port_t logical_port)
{
    ...
    uint32 esb_tdm_or_lp_queue_id = 0, esb_queue_id = 0;
    uint32 sar_queue = 0;
    int egr_if_hp, ilkn_id, nof_channels, egr_if_to_config;
    int is_interleave, core;
    bcm_port_t master_port;
    uint32 current_queue;
    int nif_if_id;
 ...
 */
shr_error_e
dnx_esb_nif_current_calendar_get(
    uint32 unit,
    bcm_core_t core,
    uint32 *current_calendar)
{
    ...
    SHR_NULL_CHECK(current_calendar, _SHR_E_PARAM, "current_calendar");
    /**
     * Allocate a handle to the DBAL table
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ESB_CALENDER_SET, &entry_handle_id));
    ...
    {
        /**
         * Determine which is the currently inactive calendar so the configuration can be set to it
         */
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CALENDAR_SWITCH_STATUS, INST_SINGLE,
                                   current_calendar);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    }
    else
...
/** see .h file */
shr_error_e
dnx_esb_dbal_frame_preemption_set(
    int unit,
    int egq_nif_if,
    int core_id,
    uint32 enable)
{
    ...
    /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ESB_FRAME_PREEMPTION_ENABLE, &entry_handle_id));
    /** Setting key fields */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_EGQ_ETH_NIF_IF, egq_nif_if);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FRAME_PREEMPTION_ENABLE, INST_SINGLE, enable);
    /** Performing the action */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
exit:
    DBAL_FUNC_FREE_VARS;
...
/** see .h file */
shr_error_e
dnx_esb_dbal_frame_preemption_get(
    int unit,
    int egq_nif_if,
    int core_id,
    uint32 *enable)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FRAME_PREEMPTION_ENABLE, INST_SINGLE, enable);
    /** Performing the action */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
exit:
    DBAL_FUNC_FREE_VARS;
