...
/**
 * \brief - Get the correct information of nof egress queues, depending on which device is used
 */
#define DEVICE_NOF_EGR_QUEUES (dnx_data_egr_queuing_v1.general.feature_get(unit, dnx_data_egr_queuing_v1_general_v1_supported) ? dnx_data_egr_queuing_v1.egress_queues.nof_egr_queues_get(unit) : dnx_data_egr_queuing.params.nof_q_pairs_get(unit));
 ...
 */
shr_error_e
dnx_fc_port_pass_control_frames_set(
    int unit,
    int port,
    int enable)
{
    ...
    rx_ctrl.pass_control_frames = enable;
 ...
 */
shr_error_e
dnx_fc_port_pass_control_frames_get(
    int unit,
    int port,
    int *enable)
{
    ...
    SHR_IF_ERR_EXIT(portmod_port_rx_control_get(unit, port, &rx_ctrl));
    *enable = rx_ctrl.pass_control_frames;
 ...
 */
shr_error_e
dnx_fc_port_pass_pause_frames_set(
    int unit,
    int port,
    int enable)
{
    ...
    rx_ctrl.pass_pause_frames = enable;
    rx_ctrl.flags = PORTMOD_MAC_PASS_PAUSE_FRAME;
    SHR_IF_ERR_EXIT(portmod_port_rx_control_set(unit, port, &rx_ctrl));
 ...
 */
shr_error_e
dnx_fc_port_pass_pause_frames_get(
    int unit,
    int port,
    int *enable)
{
    ...
    SHR_IF_ERR_EXIT(portmod_port_rx_control_get(unit, port, &rx_ctrl));
    *enable = rx_ctrl.pass_pause_frames;
 ...
 */
shr_error_e
dnx_fc_port_pass_pfc_frames_set(
    int unit,
    int port,
    int enable)
{
    ...
    rx_ctrl.pass_pfc_frames = enable;
    rx_ctrl.flags = PORTMOD_MAC_PASS_PFC_FRAME;
    SHR_IF_ERR_EXIT(portmod_port_rx_control_set(unit, port, &rx_ctrl));
 ...
 */
shr_error_e
dnx_fc_port_pass_pfc_frames_get(
    int unit,
    int port,
    int *enable)
{
    ...
    SHR_IF_ERR_EXIT(portmod_port_rx_control_get(unit, port, &rx_ctrl));
    *enable = rx_ctrl.pass_pfc_frames;
    ...
    int bitmap_index_new)
{
    ...
    if (bitmap_index_old != bitmap_index_new)
    {
        ...
        {
            nof_qpairs = DEVICE_NOF_EGR_QUEUES;
            for (qpair = 0; qpair < nof_qpairs; qpair++)
            {
    ...
    int *bitmap_index_new)
{
        ...
        || (map_sel == dnx_data_fc.calendar_common.rx_dest_pfc_generic_bmp_get(unit)))
    {
        *bitmap_index_old = map_val;
        ...
        SHR_IF_ERR_EXIT(flow_control_db.fc_generic_bmp.exchange(unit, 0, 1, &new_data, (int) map_val, NULL,
                                                                bitmap_index_new, &is_first, &is_last));
    }
    else
    {
        /** Exchange the default profile with a new one*/
        SHR_IF_ERR_EXIT(flow_control_db.fc_generic_bmp.exchange(unit, 0, 1, &new_data, bitmap_index_default, NULL,
                                                                bitmap_index_new, &is_first, &is_last));
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    int bitmap_index_default = GENERIC_BMP_DEFAULT_PROFILE_ID;
    uint32 nif_priority = 0;
    uint32 map_sel_temp = 0;
    uint32 map_val_temp = 0;
    uint32 valid = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    else
    {
        ...
        if (is_bitmap)
        {
            ...
            if (is_delete)
            {
                ...
                SHR_IF_ERR_EXIT(flow_control_db.fc_generic_bmp.exchange(unit, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID, 1,
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
        /** If the target is EGQ Q-pair, there is a limitation that only one PFC can be mapped to each Q-pair.
          * Need to verify that no other PFC is mapped to the same Q-pair */
        if ((map_sel == DNX_REC_PFC_MAP_SEL_PFC) && (!is_delete))
        {
            for (nif_priority = 0; nif_priority <= BCM_COS_MAX; nif_priority++)
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_PRIORITY, nif_priority);
                dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MAP_SEL, INST_SINGLE, &map_sel_temp);
                dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MAP_VAL, INST_SINGLE, &map_val_temp);
                dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MAP_VALID, INST_SINGLE, &valid);
                SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
                if ((valid == TRUE) && (map_val_temp == map_val) && (map_sel_temp == map_sel))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "The Q-pair is already mapped to PFC priority %d!", nif_priority);
                }
            }
        }
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_PRIORITY, source->cosq);
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    else
    {
        ...
        if (target->cosq == -1)
        {
            ...
            map_val = (is_delete ? 0 : (base_q_pair + nof_qpairs * core));
        }
        else
        {
            if (target->cosq < num_priorities)
            {
                ...
                map_val = (is_delete ? 0 : (base_q_pair + nof_qpairs * core + target->cosq));
                dest_sel =
                    (is_delete ? dnx_data_fc.calendar_common.rx_dest_empty_get(unit) : dnx_data_fc.
                     calendar_common.rx_dest_pfc_get(unit));
            }
...
/**
 * \brief - This function is to verify that for NIF port VSQ id is mapped 1-1 to serdes (multiplied by nof PGs)
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit id
 ...
 */
static shr_error_e
dnx_fc_nif_port_pfc_vsq_id_verify(
    int unit,
    bcm_cosq_fc_endpoint_t * source)
{
    bcm_cosq_vsq_info_t vsq_info;
    bcm_port_t src_local_port = -1;
    int phy_port = 0, local_phy_index, pg_base;
    dnx_algo_port_info_s port_info;
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, src_local_port, &port_info));
    ...
    {
        pg_base = BCM_COSQ_GPORT_VSQ_PG_GET(source->port);
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, src_local_port, 0, &phy_port));
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_global_to_local_phy_get(unit, phy_port, &local_phy_index)); /** VSQ id is local per core */
        if (pg_base != (BCM_COSQ_VSQ_NOF_PG * local_phy_index))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "PG-VSQ base should be mapped 1-1 to serdes (multiplied by nof PGs).");
        }
    }
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    /** Check if this mapping is supported */
    if (!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_pfc_to_stop_pm_support))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Defining PFC vector to stop interface is not supported!");
    }
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    /** Verify VSQ id */
    SHR_IF_ERR_EXIT(dnx_fc_nif_port_pfc_vsq_id_verify(unit, source));
    ...
    int *target_count)
{
             ...
             || (dest_sel == dnx_data_fc.calendar_common.rx_dest_port_8_prio_get(unit)))
    {
        ...
        nof_qpairs = DEVICE_NOF_EGR_QUEUES;
        core = (map_val >= nof_qpairs) ? 1 : 0;
        q_pair = map_val - (nof_qpairs * core);
    ...
    int power_down)
{
   ...
   /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FC_SETTINGS, &entry_handle_id));
    ...
    int unit)
{
    ...
    if (!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_is_used))
    {
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_fc_power_down_set(unit, 1));
        }
    }
    else
    {
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_qmlf_rmcs_thresholds_per_profile_support))
        {
            ...
            sal_strncpy(rmc_thresholds.name, "Template for QMLF FC Thresholds",
        ...
        }
        {
            ...
            if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_pfc_remapping))
            {
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_oob_support))
        {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SPI_OOB_RX_TO_NIF_FAST_LLFC_EN, INST_SINGLE,
                                         1);
            ...
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_RX_TO_NIF_FAST_LLFC_EN, INST_SINGLE,
                                             1);
            }
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PP_ILKN_RX_TO_NIF_FAST_LLFC_EN, INST_SINGLE,
                                         1);
        }
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_additional_enablers))
        {
            ...
            if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_oob_support))
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_OOB_RX_TO_NIF_FAST_LLFC_EN,
                                             INST_SINGLE, 1);
            }
        }
            ...
            for (interface_number = 0; interface_number < dnx_data_fc.oob.nof_oob_ids_get(unit); interface_number++)
            {
                    ...
                    (dnx_data_fc.oob.port_get(unit, interface_number)->mode == DNX_FC_CAL_MODE_TX_RX_ENABLE))
                {
                    ...
                    if (dnx_data_fc.oob.port_get(unit, interface_number)->type == DNX_OOB_TYPE_ILKN)
                    {
                        ...
                        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_OOB_RX_RSTN, INST_SINGLE,
                                                     1);
                    }
                    ...
                    if (dnx_data_fc.oob.port_get(unit, interface_number)->type == DNX_OOB_TYPE_SPI)
                    {
                        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SPI_OOB_POLARITY, INST_SINGLE,
                                                     dnx_data_fc.oob.indication_invert_get(unit,
                                                                                           interface_number)->spi);
                        ...
                        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SPI_OOB_RX_CAL_M, INST_SINGLE,
                                                     dnx_data_fc.oob.calender_rep_count_get(unit,
                                                                                            interface_number)->rx);
                    ...
                    (dnx_data_fc.oob.port_get(unit, interface_number)->mode == DNX_FC_CAL_MODE_TX_RX_ENABLE))
                {
                    ...
                    if (dnx_data_fc.oob.port_get(unit, interface_number)->type == DNX_OOB_TYPE_ILKN)
                    {
                        ...
                        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ILKN_OOB_TX_RSTN, INST_SINGLE,
                                                     1);
                    }
                    ...
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OOB_TX_OUT_ROUGH_PHASE, INST_SINGLE,
                                                 1);
                }
                SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
            }
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_support))
        {
                 ...
                 interface_number++)
            {
                ...
                /** If working in both PFC and Pause modes for COE are not supported, need to choose the mode the device will be working in */
                if (!dnx_data_fc.coe.feature_get(unit, dnx_data_fc_coe_both_modes_support))
                {
