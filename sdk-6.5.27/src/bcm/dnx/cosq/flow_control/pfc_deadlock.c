    ...
    int unit)
{
    ...
    {
        /** Flow control is not used, no need to initialize this module */
        SHR_EXIT();
    }
    ...
    int unit)
{
    ...
    if (!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_is_used))
    {
        /** Flow control is not used, no need to de-initialize this module */
        SHR_EXIT();
    }
...
/**    COMPILER_UINT64 time64; **/
    ...
    if (is_set == TRUE)
    {
        ...
        /** If the ports for PFC instrumentation logic are limited, check if the user input is one of the configured ports */
    if (dnx_data_instru.synced_counters.feature_get(unit, dnx_data_instru_synced_counters_limited_ports_for_pfc))
    {
        ...
        {
            /** Get port from SW state and check if it is matching the uuser input */
            SHR_IF_ERR_EXIT(instru_sync_counters.ports_for_pfc_counters.
                            port.get(unit, current_port_idx, &current_port));
            if (current_port == port)
            {
                port_found = TRUE;
                /** Port is found, now check if the required TC is set */
                SHR_IF_ERR_EXIT(instru_sync_counters.ports_for_pfc_counters.tc_bmp.get(unit, current_port_idx, tc_bmp));
                tc_found = SHR_BITGET(tc_bmp, pfc_priority) ? TRUE : FALSE;
                if (tc_found == FALSE)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "TC %d is not configured for PFC Insturumentation!\n", pfc_priority);
                }
                break;
            }
        }
        ...
        if (port_found == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Port %d is not configured for PFC Insturumentation!\n", port);
        }
    }
    ...
    int *event_id)
{
    ...
    if (ethu_info.imb_type == imbDispatchTypeImb_cdu_shr)
    {
        /** ETHU type is OFR. IMB type index is not used, because there is only one OFR block for all ETHUs */
        *index = 0;
    ...
    else if (ethu_info.imb_type == imbDispatchTypeImb_cdu)
    {
        ...
        if (ethu_info.ethu_id_in_core < cdum_nof_per_core)
        {
            ...
            *index = ethu_info.imb_type_id - (cdu_nof_per_core - cdum_nof_per_core) * core;
