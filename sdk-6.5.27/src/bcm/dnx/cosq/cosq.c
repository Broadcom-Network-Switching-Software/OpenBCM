    ...
    bcm_cosq_range_t * range)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqRangeLowRateVoqConnector:
            ...
            {
                SHR_IF_ERR_EXIT(dnx_scheduler_low_rate_flow_range_set(unit, port, flags, type, range));
            }
            else
            {
        ...
        case bcmCosqRangeLowRate_0:
        case bcmCosqRangeLowRate_1:
        case bcmCosqRangeLowRate_2:
        {
            if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_multi_low_rate_ranges))
            {
                SHR_IF_ERR_EXIT(dnx_scheduler_low_rate_flow_range_set(unit, port, flags, type, range));
    ...
    bcm_cosq_range_t * range)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqRangeLowRateVoqConnector:
            if (!dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_multi_low_rate_ranges))
            {
                SHR_IF_ERR_EXIT(dnx_scheduler_low_rate_flow_range_get(unit, port, flags, type, range));
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "unsupported type %d.\n", type);
 ...
 *  - port - 0
 *  - cosq - 0
 *  - type - bcmCosqLowRateVoqConnectorFactor / bcmCosqLowRateFactor
 *  - arg - 32 or 64
 *
 * \param [in] unit -  Unit-ID
 * \param [in] port - description per use case.
    ...
    int is_set)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqGenericControlLowRatePattern:
    ...
    uint32 arg)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqGenericControlLowRatePattern:
            ...
            {
                SHR_IF_ERR_EXIT(dnx_scheduler_low_rate_pattern_set(unit, flags, gport, key, arg));
            }
            break;
        default:
    ...
    uint32 *arg)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqGenericControlLowRatePattern:
            ...
            {
                SHR_IF_ERR_EXIT(dnx_scheduler_low_rate_pattern_get(unit, flags, gport, key, arg));
            }
            break;
        default:
    ...
    int arg)
{
    ...
    {
        case bcmCosqTASControlTASReset:
            SHR_IF_ERR_EXIT(dnx_tsn_tas_control_reset(unit, port, arg));
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "type not supported %d", type);
    }
...
/*
 * brief - verify function for bcm_dnx_cosq_gport_queue_offset_mapping_set
 */
static shr_error_e
    ...
    bcm_cosq_queue_offset_t * offset_array)
{
    ...
    if ((flags != BCM_COSQ_QUEUE_OFFSET_MAPPING_EGRESS_MULTICAST) &&
        (flags != BCM_COSQ_QUEUE_OFFSET_MAPPING_EGRESS_UNICAST))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unsopported flag type \n");
    }
    ...
    {
        SHR_RANGE_VERIFY(key_array[i].dp, 0, DNX_COSQ_NOF_DP - 1, _SHR_E_PARAM, "Invalid dp %d\n", key_array[i].dp);
        SHR_RANGE_VERIFY(key_array[i].tc, BCM_COS_MIN, BCM_COS_MAX, _SHR_E_PARAM, "Invalid tc %d\n", key_array[i].tc);
        SHR_RANGE_VERIFY(offset_array[i].cosq, BCM_COS_MIN, nof_priorities - 1, _SHR_E_PARAM,
                         "Invalid cosq %d\n", offset_array[i].cosq);

        if (flags & BCM_COSQ_QUEUE_OFFSET_MAPPING_EGRESS_MULTICAST)
        {
            SHR_RANGE_VERIFY(offset_array[i].out_dp, 0, DNX_COSQ_NOF_DP - 1, _SHR_E_PARAM,
                             "Invalid out_dp%d\n", offset_array[i].out_dp);
        }
    ...
    bcm_cosq_queue_offset_t * offset_array)
{
        ...
        || (flags & BCM_COSQ_QUEUE_OFFSET_MAPPING_EGRESS_UNICAST))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_egq_cosq_port_egress_mapping_set(unit, flags, gport, count, key_array, offset_array));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipq_cosq_gport_queue_offset_mapping_set
                        (unit, flags, gport, count, key_array, offset_array));
    }
    ...
    bcm_cosq_queue_offset_t * offset_array)
{
    ...
    if (!BCM_GPORT_IS_LOCAL(gport) && !BCM_GPORT_IS_TRUNK(gport) && !BCM_GPORT_IS_MODPORT(gport))
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "Unsopported Gport type \n");
    }
    ...
    bcm_cosq_queue_offset_t * offset_array)
{
        ...
        || (flags & BCM_COSQ_QUEUE_OFFSET_MAPPING_EGRESS_UNICAST))
    {
        SHR_INVOKE_VERIFY_DNXC(dnx_cosq_gport_queue_offset_mapping_get_verify(unit, flags, gport, count,
                                                                              key_array, offset_array));
        SHR_IF_ERR_EXIT(dnx_egq_cosq_port_egress_mapping_get(unit, flags, gport, count, key_array, offset_array));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipq_cosq_gport_queue_offset_mapping_get
                        (unit, flags, gport, count, key_array, offset_array));
    }
exit:
    SHR_FUNC_EXIT;
