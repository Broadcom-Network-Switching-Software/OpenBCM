...
#include <include/bcm_int/dnx/init/init_time_analyzer.h>
...
dnx_crps_ctrs_rate_calculation_next_expected_counter_update(
    ...
    int engine_id)
{
    uint32 eviction_range_start, eviction_range_end, next_seq_counter_idx;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_crps_db.proc.rate.next_seq_counter_idx.get(unit, core_id, engine_id, &next_seq_counter_idx));
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.eviction.eviction_range.end.get(unit, core_id, engine_id, &eviction_range_end));
    if (next_seq_counter_idx == eviction_range_end)
    {
        SHR_IF_ERR_EXIT(dnx_crps_db.proc.eviction.eviction_range.
                        start.get(unit, core_id, engine_id, &eviction_range_start));

        next_seq_counter_idx = eviction_range_start;
    }
    else
    {
    ...
    if (!is_zero || rate_calculation_is_enable)
    {
        ...
        switch (format)
        {
            ...
            case bcmStatCounterFormatMaxSize:
                ...
                ctr_size_first_section = rcd_format->ctr_value_size / nof_counters_per_shaddow;
...
{
    int nof_valid_counter_indexes = 0;
    int valid_counter_indexes[DNX_CRPS_MGMT_MAX_NSTAT] = { 0 };
    int lock_taken = FALSE;
    int rate_calculation_is_enable;
    int min_val;
    int max_val;
    int first;
    int count;
    uint64 values[DNX_CRPS_MGMT_MAX_NSTAT * DNX_CRPS_MGMT_SUB_COUNT] = { 0 };
    SHR_FUNC_INIT_VARS(unit);

    DNX_INIT_TIME_ANALYZER_CRPS_START(unit, DNX_INIT_TIME_ANALYSER_CRPS_EVICTION);
    ...
    for (int cnt_idx = 0; cnt_idx < nstat; cnt_idx++)
    {
        if ((counters_id[cnt_idx] == -1) || (sub_count[cnt_idx] == -1))
        {
            continue;
        }
        valid_counter_indexes[nof_valid_counter_indexes] = cnt_idx;
        ++nof_valid_counter_indexes;
    }

    /** Get minimal and maximal counter_ids */
    /** Set both to first */
    min_val = counters_id[valid_counter_indexes[0]];
    max_val = counters_id[valid_counter_indexes[0]];
    /** iterate others to search minimal/maximal */
    for (int valid_counter_index_index = 1; valid_counter_index_index < nof_valid_counter_indexes;
         ++valid_counter_index_index)
    {
        int current_counter_id = counters_id[valid_counter_indexes[valid_counter_index_index]];
        if (min_val > current_counter_id)
        {
            min_val = current_counter_id;
        }
        else if (max_val < current_counter_id)
        {
            max_val = current_counter_id;
        }
    }

    /** Read counter range, optimized for reading all the range */
    first = min_val * DNX_CRPS_MGMT_SUB_COUNT;
    count = (max_val - min_val + 1) * DNX_CRPS_MGMT_SUB_COUNT;
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.counter.range_read(unit, core_id, engine_id, first, count, values));

    /** Iterate valid entries and get/clear values */
    for (int valid_counter_index_index = 0; valid_counter_index_index < nof_valid_counter_indexes;
         ++valid_counter_index_index)
    {
        int counter_index = valid_counter_indexes[valid_counter_index_index];
        int current_counter_id = counters_id[counter_index];
        int current_sub_counter = sub_count[counter_index];
        int value_offset = (current_counter_id - min_val) * DNX_CRPS_MGMT_SUB_COUNT + current_sub_counter;
        COMPILER_64_COPY(output_data->value_arr[counter_index].value, values[value_offset]);

        if (_SHR_IS_FLAG_SET(flags, BCM_STAT_COUNTER_CLEAR_ON_READ))
        {
            COMPILER_64_ZERO(values[value_offset]);
        }
    }

    /** write values back in case that need to clear on read */
    if (_SHR_IS_FLAG_SET(flags, BCM_STAT_COUNTER_CLEAR_ON_READ))
    {
        SHR_IF_ERR_EXIT(dnx_crps_db.proc.counter.range_write(unit, core_id, engine_id, first, count, values));
    }

    if (rate_calculation_is_enable)
    {
        /** Get range for rate calculations*/
        uint8 seq_number_values[DNX_CRPS_MGMT_MAX_NSTAT * DNX_CRPS_MGMT_SUB_COUNT] = { 0 };
        SHR_IF_ERR_EXIT(dnx_crps_db.proc.rate.
                        sequence_num.range_read(unit, core_id, engine_id, first, count, seq_number_values));
             ...
             ++valid_counter_index_index)
        {
            ...
            int current_counter_id = counters_id[counter_index];
            int current_sub_counter = sub_count[counter_index];
            int seq_number_value_offset = (current_counter_id - min_val) * DNX_CRPS_MGMT_SUB_COUNT;
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_rate_seq_shadow_get
                            (unit, core_id, engine_id,
                             (current_counter_id * DNX_CRPS_MGMT_SUB_COUNT) + current_sub_counter,
                             &last_period_change));

            COMPILER_64_COPY(output_data->value_arr[counter_index].last_period_change, last_period_change);
            output_data->value_arr[counter_index].sequence_number = seq_number_values[seq_number_value_offset];
        }
    }
...
exit:
    ...
    DNX_INIT_TIME_ANALYZER_CRPS_STOP(unit, DNX_INIT_TIME_ANALYSER_CRPS_EVICTION);
    SHR_FUNC_EXIT;
}
...
static shr_error_e
dnx_crps_eviction_is_enabled_hw_get(
    int unit,
    bcm_stat_engine_t * engine,
    uint32 *is_enabled)
{
    ...
    /** Set dbal request */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CRPS_ENGINE_SCAN_ENABLE, INST_SINGLE, is_enabled);
...
static shr_error_e
dnx_crps_eviction_is_enabled_hw_set(
    int unit,
    bcm_stat_engine_t * engine,
    int is_enabled)
{
    ...
    /** Set dbal field */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CRPS_ENGINE_SCAN_ENABLE, INST_SINGLE, is_enabled);
...
static shr_error_e
dnx_crps_eviction_boundaries_hw_set(
    int unit,
    bcm_stat_engine_t * engine,
    bcm_stat_eviction_boundaries_t * boundaries)
{
    ...
    /** fisrt addres to scan sequentialy is always zero, unless the user asked something else explicitly. */
...
static shr_error_e
dnx_crps_eviction_boundaries_sw_set(
    int unit,
    bcm_stat_engine_t * engine,
    bcm_stat_eviction_boundaries_t * boundaries)
{
    ...
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.eviction.eviction_range.start.set
                    (unit, engine->core_id, engine->engine_id, boundaries->start));

    SHR_IF_ERR_EXIT(dnx_crps_db.proc.eviction.eviction_range.end.set
                    (unit, engine->core_id, engine->engine_id, boundaries->end));
...
dnx_crps_eviction_boundaries_set(
    ...
    bcm_stat_eviction_boundaries_t * boundaries)
{
    bcm_stat_eviction_boundaries_t boundaries_set;
    uint32 was_initially_enabled = 0;
    uint32 first, last;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
     ...
     *  boundaries -> enable the scan */
     /** Taking a handle - engine scan disable */
    SHR_IF_ERR_EXIT(dnx_crps_eviction_is_enabled_hw_get(unit, engine, &was_initially_enabled));

    if (was_initially_enabled)
    {
        /** disable the scan */
        SHR_IF_ERR_EXIT(dnx_crps_eviction_is_enabled_hw_set(unit, engine, FALSE));
    }

    if (flags & BCM_STAT_EVICTION_RANGE_ALL)
    {
        boundaries_set.start = 0;
        ...
        boundaries_set.end = (last - first);
    }
    /** set the user given values */
    else
    {
        boundaries_set.start = boundaries->start;
        boundaries_set.end = boundaries->end;
    }

    SHR_IF_ERR_EXIT(dnx_crps_eviction_boundaries_hw_set(unit, engine, &boundaries_set));
    SHR_IF_ERR_EXIT(dnx_crps_eviction_boundaries_sw_set(unit, engine, &boundaries_set));

    if (was_initially_enabled)
    {
        /** enable the scan */
        SHR_IF_ERR_EXIT(dnx_crps_eviction_is_enabled_hw_set(unit, engine, TRUE));
    ...
    bcm_stat_eviction_boundaries_t * boundaries)
{
    ...
    while (engine_id != DNX_CRPS_MGMT_ENGINE_INVALID)
    {
        ...
        else
        {
            ...
            /** calculate the counter id according to the nof counter per
             *  counter entry */
