    ...
    bcm_stat_counter_enable_t * enable)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_database_enable_set_verify(unit, flags, database, enable));
    ...
    bcm_stat_counter_enable_t * enable)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_database_enable_get_verify(unit, flags, database, enable));
    ...
    uint32 arg)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_database_control_set_verify(unit, flags, database, control, arg));
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        database_main.core_id = core_idx;
        database_main.database_id = database->database_id;
    ...
    uint32 *arg)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_database_control_get_verify(unit, flags, database, control, arg));
    database_main.core_id = (database->core_id == BCM_CORE_ALL) ? 0 : database->core_id;
    database_main.database_id = database->database_id;
    ...
    uint32 arg)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_engine_control_set_verify(unit, flags, engine, control, arg));
    DNXCMN_CORES_ITER(unit, engine->core_id, core_idx)
    {
        ...
        switch (control)
        {
            ...
            case bcmStatCounterSequentialSamplingInterval:
                /** check for correct source if the eviction process is allowed */
                if (!dnx_data_crps.src_interface.crps_eviction_get(unit, engine_source)->valid_for_eviction)
                {
    ...
    uint32 *arg)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_engine_control_get_verify(unit, flags, engine, control, arg));
    ...
    bcm_stat_counter_interface_t * config)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_interface_set_verify(unit, flags, database, config));
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        database_main.core_id = core_idx;
        database_main.database_id = database->database_id;
    ...
    bcm_stat_counter_interface_t * config)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_interface_get_verify(unit, flags, database, config));
    database_main.core_id = (database->core_id == BCM_CORE_ALL) ? 0 : database->core_id;
    database_main.database_id = database->database_id;
...
static void
dnx_stat_counter_expansion_select_array_copy(
    int unit,
    bcm_stat_counter_interface_type_t interface_source,
    bcm_stat_expansion_select_t * expansion_select_src,
    bcm_stat_expansion_select_t * expansion_select_dest)
{
    int i;
    int nof_bits;
    ...
    /** this will help later in the driver implementation to cosider the bitmap for all types */
    /** (because all types will have "full" bitmap according to the type size, except the ones that allowed to be not "full" */
    for (i = 0; i < expansion_select_src->nof_elements; i++)
    {
        nof_bits =
            dnx_data_crps.expansion.source_metadata_types_get(unit, interface_source,
                                                              expansion_select_src->expansion_elements[i].
                                                              type)->nof_bits;

        expansion_select_dest->expansion_elements[i].type = expansion_select_src->expansion_elements[i].type;
        expansion_select_dest->expansion_elements[i].bitmap = expansion_select_src->expansion_elements[i].bitmap;
        ...
        if (!DNX_CRPS_EXPANSION_TYPE_BITMAP_IS_RELEVANT(expansion_select_src->expansion_elements[i].type))
        {
            SHR_BITSET_RANGE(&expansion_select_dest->expansion_elements[i].bitmap, 0, nof_bits);
        }
    }
    ...
    bcm_stat_expansion_select_t * expansion_select)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_expansion_select_set_verify(unit, flags, interface, expansion_select));

    sal_memcpy(&expansion_select_copy, expansion_select, sizeof(bcm_stat_expansion_select_t));
    ...
    DNXCMN_CORES_ITER(unit, interface->core_id, core_idx)
    {
        ...
        dnx_stat_counter_expansion_select_array_copy
            (unit, interface->interface_source, expansion_select, &expansion_select_copy);
    ...
    bcm_stat_expansion_select_t * expansion_select)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_expansion_select_get_verify(unit, flags, interface, expansion_select));
    ...
    bcm_stat_counter_set_map_t * counter_set_map)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_set_mapping_set_verify(unit, flags, database, counter_set_map));
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        database_main.core_id = core_idx;
        database_main.database_id = database->database_id;
    ...
    bcm_stat_counter_set_map_t * counter_set_map)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_set_mapping_get_verify(unit, flags, database, counter_set_map));
    database_main.core_id = (database->core_id == BCM_CORE_ALL) ? 0 : database->core_id;
    database_main.database_id = database->database_id;
    ...
    bcm_stat_eviction_t * eviction)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_eviction_set_verify(unit, flags, engine, eviction));
    DNXCMN_CORES_ITER(unit, engine->core_id, core_idx)
    {
        engine_main.core_id = core_idx;
        engine_main.engine_id = engine->engine_id;
    ...
    bcm_stat_eviction_t * eviction)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_eviction_get_verify(unit, flags, engine, eviction));
    engine_main.core_id = (engine->core_id == BCM_CORE_ALL) ? 0 : engine->core_id;
    engine_main.engine_id = engine->engine_id;
...
/**
* \brief
*   API which sets the eviction configuration per database.
*   it verifies the input parameters, and sets the parameters in the sw state
* \par DIRECT INPUT:
*   \param [in] unit    -  unit ID
*   \param [in] flags   -  NONE
*   \param [in] database  -  pointer to structure which contain the database_id and core_id
...
*/
int
bcm_dnx_stat_counter_database_eviction_set(
    int unit,
    uint32 flags,
    bcm_stat_counter_database_t * database,
    bcm_stat_eviction_t * eviction)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_database_eviction_set_verify(unit, flags, database, eviction));

    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        /** go over all cores */
        SHR_IF_ERR_EXIT(dnx_crps_db.database.base_engine.get(unit, core_idx, database->database_id, &engine_idx));
        /**    go over the linked list of the database */
        while (engine_idx != DNX_CRPS_MGMT_ENGINE_INVALID)
        {
            engine.engine_id = engine_idx;
            engine.core_id = core_idx;
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_counter_eviction_set(unit, flags, &engine, eviction));
            SHR_IF_ERR_EXIT(dnx_crps_db.proc.interface.next_engine_id.get(unit, core_idx, engine_idx, &engine_idx));
        }
    }
...
/**
* \brief
*   API which gets the eviction configuration per database.
*   it verifies the input parameters, and gets the parameters from the sw state
* \par DIRECT INPUT:
*   \param [in] unit    -  unit ID
*   \param [in] flags   -  NONE
*   \param [in] database  -  pointer to structure which contain the database_id and core_id
...
*/
int
bcm_dnx_stat_counter_database_eviction_get(
    int unit,
    uint32 flags,
    bcm_stat_counter_database_t * database,
    bcm_stat_eviction_t * eviction)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_database_eviction_get_verify(unit, flags, database, eviction));

    engine.core_id = (database->core_id == BCM_CORE_ALL) ? 0 : database->core_id;
    SHR_IF_ERR_EXIT(dnx_crps_db.database.base_engine.get(unit, engine.core_id, database->database_id, &engine_id));
    /**    all engines in the database should have same setting, this was verified in set */
    engine.engine_id = engine_id;
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_counter_eviction_get(unit, flags, &engine, eviction));
    ...
    bcm_stat_counter_output_data_t * stat_counter_output_data)
{
    ...
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_get_verify
    ...
    bcm_stat_counter_output_data_t * stat_counter_output_data)
{
    ...
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_explicit_get_verify
                           (unit, flags, stat_counter_input_data, stat_counter_output_data));
    ...
    bcm_stat_eviction_boundaries_t * boundaries)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_eviction_boundaries_set_verify(unit, flags, engine, boundaries));
    DNXCMN_CORES_ITER(unit, engine->core_id, core_idx)
    {
        engine_main.core_id = core_idx;
        engine_main.engine_id = engine->engine_id;
    ...
    bcm_stat_eviction_boundaries_t * boundaries)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_eviction_boundaries_get_verify(unit, flags, engine, boundaries));
    engine_main.core_id = (engine->core_id == BCM_CORE_ALL) ? 0 : engine->core_id;
    engine_main.engine_id = engine->engine_id;
    ...
    int *database_id)
{
    ...
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_counter_database_create_verify(unit, flags, core_id, database_id));
    ...
    bcm_stat_counter_database_t * database)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_counter_database_destroy_verify(unit, flags, database));
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        database_main.core_id = core_idx;
        database_main.database_id = database->database_id;
    ...
    int engine_id)
{
    ...
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_counter_engine_attach_verify(unit, flags, database, engine_id));
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        database_main.core_id = core_idx;
        database_main.database_id = database->database_id;
    ...
    int engine_id)
{
    ...
    DNX_ERR_RECOVERY_NO_SUPPORT_BEGIN(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_counter_engine_detach_verify(unit, flags, database, engine_id));
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
    ...
    bcm_stat_eviction_boundaries_t * boundaries)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_database_eviction_boundaries_set_verify
                           (unit, flags, database, type_id, boundaries));
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        database_main.core_id = core_idx;
        database_main.database_id = database->database_id;
    ...
    bcm_stat_eviction_boundaries_t * boundaries)
{
    ...
    DNX_ERR_RECOVERY_START(unit);
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_crps_database_eviction_boundaries_get_verify
                           (unit, flags, database, type_id, boundaries));
    database_main.core_id = (database->core_id == BCM_CORE_ALL) ? 0 : database->core_id;
    database_main.database_id = database->database_id;
