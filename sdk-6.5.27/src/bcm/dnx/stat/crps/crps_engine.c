...
static shr_error_e
dnx_crps_meter_shared_b_and_c_bank_hw_get(
    int unit,
    int core_id,
    dbal_tables_e dbal_table_id,
    int bank_id,
    ...
    dbal_enum_value_field_meter_crps_smem_src_types_e src_type)
{
    ...
    int small_engine = DNX_CRPS_METER_SHMEM_IS_METER_SMALL_ENGINE(unit, engine_id);
    dbal_tables_e dbal_table_id;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    else
    {
        ...
        else
        {
            ...
            {
                base_bank = dnx_crps_mgmt_shmem_section_all;
                last_bank = dnx_crps_mgmt_shmem_section_all;
            }

            else if (section == dnx_crps_mgmt_shmem_section_all)
            {
    ...
    dbal_enum_value_field_meter_crps_smem_src_types_e * src_type)
{
    ...
    /** engine 0 and 1 are small engines and behave differently from big engines.
               each engine has one bank. engine 0 may be used for CRPS or ingress meter, engine 1 may be used for CRPS or egress meter */
    if (DNX_CRPS_METER_SHMEM_IS_METER_SMALL_ENGINE(unit, engine_id))
    {
        /** Taking a handle */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_CRPS_METER_SMALL_ENGINE_SHARED_MEM, &entry_handle_id));
        /** Setting key fields */
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_crps_meter_shared_b_and_c_bank_hw_get(unit, core_id, dbal_table_id, bank_id, src_type));
    }
