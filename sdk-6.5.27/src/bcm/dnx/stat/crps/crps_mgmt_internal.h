...
#define DNX_CRPS_MGMT_ENGINE_FIELDS_INVALID (-1)

#define DNX_CRPS_MGMT_SET_INVALID (-1)

#define DNX_CRPS_MGMT_MAX_NSTAT (DNX_DATA_MAX_CRPS_ENGINE_MAX_COUNTER_SET_SIZE*2)

#define DNX_CRPS_MGMT_COUNTER_FORMAT_DEFAULT (bcmStatCounterFormatPacketsAndBytes)

#define DNX_CRPS_MGMT_COUNTER_MODE_DEFAULT (bcmStatCounterModeNone)
...
/**
* \brief
*   whether or not two stat-ids related to the same counter shadow
* \par DIRECT INPUT:
*   \param [in] unit    -  unid id
*   \param [in] format_type  -  counter format
...
*   * None
*/
shr_error_e dnx_crps_mgmt_nof_counters_in_counter_shaddow_get(
    int unit,
...
/**
* \brief
*   calculate the number of counters from the given format can be located in the given engine
* \par DIRECT INPUT:
*   \param [in] unit    -  unid id
*   \param [in] engine_id    -  engine_id
*   \param [in] format_type  -  counter format
*   \param [in] counter_size - counter size, relevant for state counters only
*   \param [out] nof_counters  -  number of counters from the given format.
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e -
...
*   * None
*/
shr_error_e dnx_crps_mgmt_nof_counters_per_engine(
    int unit,
    int engine_id,
    bcm_stat_counter_format_type_t format_type,
    int counter_size,
...
/**
 * \brief
 *      Update HW on network eviction packet size
 * \param [in] unit - unit id
 * \param [in] pkt_size_in_KB - packet size [KB]
 * \return
 *   \retval Non-zero (!= _SHR_E_NONE) in case of an error
 *   \retval Zero (= _SHR_E_NONE) in case of NO ERROR
 * \remark
 ...
 *   NONE
 */
shr_error_e dnx_crps_mgmt_network_eviction_pkt_size_set(
    int unit,
    int pkt_size_in_KB);

/**
 * \brief
 *      get network eviction packet size from HW
 * \param [in] unit - unit id
 * \param [out] pkt_size_in_KB - packet size [KB]
 * \return
 *   \retval Non-zero (!= _SHR_E_NONE) in case of an error
 *   \retval Zero (= _SHR_E_NONE) in case of NO ERROR
 * \remark
 ...
 *   NONE
 */
shr_error_e dnx_crps_mgmt_network_eviction_pkt_size_get(
    int unit,
    int *pkt_size_in_KB);
 ...
 * \param [in] engine_id -engine_id
 * \param [in] type_id -type_id 
 * \param [in] stat_object_id -the counter pointer that the source send to CRPS.
