    ...
    bcm_stat_counter_interface_t * config)
{
    ...
    {
        if (config->source != bcmStatCounterInterfaceEgressTransmitPp
            && config->source != bcmStatCounterInterfaceEgressPp)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "EGRESS_LATENCY flags is valid with source=%d only. \n", config->source);
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "EGRESS_LATENCY flags is valid only for source=%d, command_id=%d \n",
                         config->source, dnx_data_crps.latency.etpp_command_id_get(unit));
        }
    ...
    bcm_stat_counter_output_data_t * stat_counter_output_data)
{
    ...
    /** check for correct source - if no eviction, can't read counters */
    if (!dnx_data_crps.src_interface.crps_eviction_get(unit, engine_source)->valid_for_eviction)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
