 ...
 */
const dnx_stif_mgmt_report_format_element_t ingress_element_enum_to_dbal_fields_binding_table[] = {
    ...
    int power_down)
{
   ...
   /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_STIF_SETTINGS, &entry_handle_id));
    ...
    int unit)
{
    ...
    if (!dnx_data_stif.general.feature_get(unit, dnx_data_stif_general_is_used))
    {
        if (dnx_data_stif.general.feature_get(unit, dnx_data_stif_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_stif_power_down_set(unit, 1));
        }
    }
    else
    {
    ...
    int instance_id)
{
    ...
    if (port_get != port)
    {
        ...
        if (!dnx_data_stif.config.feature_get(unit, dnx_data_stif_config_is_stif_hard_wire_to_nif))
        {
    ...
    int unit)
{
    ...
    nof_metadata_elements = dnx_data_stif.report.feature_get(unit, dnx_data_stif_report_stif_record_nif_port_rate_bin) ?
        DBAL_NOF_ENUM_STIF_BILLING_REPORT_METADATA_ELEMENT_VALUES :
        DBAL_NOF_ENUM_STIF_BILLING_REPORT_METADATA_ELEMENT_VALUES - 1;

    for (element = 0; element < nof_metadata_elements; element++)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, DBAL_CORE_ALL);
    ...
    dbal_enum_value_field_stif_billing_report_metadata_element_e * dbal_value)
{
    ...
    nof_metadata_elements = sizeof(egress_metadata_element_enum_to_dbal_fields_binding_table) /
        sizeof(dnx_stif_mgmt_report_metadata_element_t);

    for (i = 0; i < nof_metadata_elements; i++)
    {
    ...
    int *elements_size_array)
{
    ...
    dbal_fields_e shift_field;
    int element_id, object_id, field_size, nof_metadata_elements;
    ...
    for (metadata_element = 0; metadata_element < nof_metadata_elements; metadata_element++)
    {
        dbal_entry_key_field32_set(unit, metadata_handle, DBAL_FIELD_CORE_ID, DBAL_CORE_ALL);
        dbal_entry_key_field32_set(unit, metadata_handle, DBAL_FIELD_STIF_BILLING_REPORT_METADATA_ELEMENT,
                                   metadata_element);
