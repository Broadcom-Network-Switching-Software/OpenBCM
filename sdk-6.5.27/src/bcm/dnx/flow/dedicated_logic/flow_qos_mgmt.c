    ...
    uint8 *is_qos_valid)
{
    ...
    if (is_field_valid == TRUE)
    {
        ...
        if (ttl_mode == DBAL_ENUM_FVAL_TTL_MODE_PIPE)
        {
            ...
            /** Get TTL PIPE profile */
            SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                            (unit, entry_handle_id, DBAL_FIELD_TTL_PIPE_PROFILE, &is_field_valid, &ttl_pipe_profile));
            if (is_field_valid == TRUE)
            {
        ...
        else    /* updating or 1st writing the ttl */
        {
            ...
            if (((is_pipe_mode_old) == TRUE) && ((is_pipe_mode_new) == TRUE) && (was_old_ttl_profile_set == TRUE))
            {
                ...
                SHR_IF_ERR_EXIT(algo_qos_db.qos_egress_ttl_pipe_profile.exchange
                                (unit, 0, 1, (int *) &initiator_info->ttl, old_profile, NULL,
...
{
    uint8 field_idx;
    uint8 nwk_pri_cfi_en = FALSE, nwk_qos_enabled = FALSE;
    uint8 nwk_qos_pri = 0, nwk_qos_cfi = 0;
    uint32 nwk_qos_pipe_value = 0;
    ...
    if (special_fields)
    {
        for (field_idx = 0; field_idx < special_fields->actual_nof_special_fields; field_idx++)
        {
            ...
            {
                nwk_qos_pipe_value = special_fields->special_fields[field_idx].shr_var_uint32;
    ...
    {
        nwk_qos_pipe_value = DNX_QOS_IDX_PCP_DEI_TO_NWK_QOS(nwk_qos_pri, nwk_qos_cfi);
        nwk_qos_enabled = TRUE;
    }
    ...
    if (nwk_qos_enabled)
    {
            ...
            (initiator_info->egress_qos_model.egress_qos == bcmQosEgressModelPipeNextNameSpace))
        {
            ...
            uint8 is_first_pipe_profile_ref = FALSE;
            dbal_tables_e table_id;

            SHR_IF_ERR_EXIT(dbal_entry_handle_related_table_get(unit, entry_handle_id, &table_id));

            if ((initiator_info->egress_qos_model.egress_qos == bcmQosEgressModelPipeNextNameSpace) &&
                nwk_pri_cfi_en && (table_id != DBAL_TABLE_EEDB_ARP))
            {
                nwk_qos_profile = DNX_QOS_EXPLICIT_IDX_L2(nwk_qos_pri, nwk_qos_cfi);
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_allocate
                                (unit, nwk_qos_pipe_value, nwk_qos_pipe_value, 0, (int *) &nwk_qos_profile,
                                 &is_first_pipe_profile_ref));
            }

            if (is_first_pipe_profile_ref)
            {
                SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_set
                                (unit, nwk_qos_profile, nwk_qos_pipe_value, nwk_qos_pipe_value, 0));
            }
...
{
    uint8 was_nwk_qos_set = FALSE;
    uint8 is_first_profile_ref = FALSE, is_last_profile_ref = FALSE;
    uint8 field_idx;
    uint8 is_qos_model_valid = FALSE, nwk_qos_cleared = FALSE;
    uint8 is_nwk_qos_index_valid = FALSE;
    uint8 nwk_pri_en = FALSE, nwk_cfi_en = FALSE, exp0_en = FALSE, exp1_en = FALSE;
    uint8 nwk_qos_update = FALSE, is_pipe_mode_old = FALSE, is_pipe_mode_new = FALSE;
    uint8 old_nwk_qos_piped = 0, old_nwk_qos_dp_piped = 0, old_dp_piped = 0;
    uint32 old_profile = 0, new_profile = 0;
    uint32 nwk_qos_pri = 0, nwk_qos_cfi = 0, exp0_value = 0, exp1_value = 0, nwk_qos_pipe_value = 0;
    dbal_enum_value_field_encap_qos_model_e encap_qos_model;
    bcm_flow_special_field_t special_field_data = { 0 };
    ...
    {
        uint8 exp0_found = FALSE, exp1_found = FALSE, more_than_one_label = FALSE;
        shr_error_e rv;

        rv = dnx_flow_special_field_data_get(unit, special_fields, FLOW_S_F_MPLS_LABEL_2, &special_field_data);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
        more_than_one_label = (rv == _SHR_E_NONE) ? TRUE : FALSE;
        ...
        for (field_idx = 0; field_idx < special_fields->actual_nof_special_fields; field_idx++)
        {
            ...
            if (special_fields->special_fields[field_idx].field_id == FLOW_S_F_QOS_EXP)
            {
                ...
                {
                    exp0_value = special_fields->special_fields[field_idx].shr_var_uint32;
                    exp0_en = TRUE;
                    exp0_found = TRUE;
                    if ((!more_than_one_label) || exp1_found)
                    {
                        break;
                    }
                }
            }
            else if (special_fields->special_fields[field_idx].field_id == FLOW_S_F_QOS_EXP_1)
            {
                if (special_fields->special_fields[field_idx].is_clear)
                {
                    nwk_qos_cleared = TRUE;
                ...
                {
                    exp1_value = special_fields->special_fields[field_idx].shr_var_uint32;
                    exp1_en = TRUE;
                    exp1_found = TRUE;
                    if (exp0_found)
                    {
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get(unit, get_entry_handle_id,
                                                                       ...
                                                                       &is_qos_model_valid, &encap_qos_model));

    if (is_qos_model_valid == TRUE)
    {
        /**in case qos_model is pipe, get old value*/
            ...
            (encap_qos_model == DBAL_ENUM_FVAL_ENCAP_QOS_MODEL_PIPE_ECN))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get(unit, get_entry_handle_id,
                                                                               ...
                                                                               &is_nwk_qos_index_valid, &old_profile));
            if (is_nwk_qos_index_valid)
            {
                was_nwk_qos_set = TRUE;
                /**old NWK_QOS_IDX is explicit*/
                if (old_profile & (DNX_QOS_EXPLICIT_IDX_FLAG << DNX_QOS_EXPLICIT_IDX_OFFSET))
                {
                    is_pipe_mode_old = FALSE;
                    old_nwk_qos_piped = old_profile;
                }
                else
                {
                    SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_attr_get(unit, old_profile,
                                                                                     &old_nwk_qos_piped,
                                                                                     &old_nwk_qos_dp_piped,
                                                                                     &old_dp_piped));
                }
            }
        }
    ...
    {
        if ((is_qos_model_valid == TRUE) && (is_nwk_qos_index_valid == TRUE))
        {
            if (nwk_pri_en == FALSE)
            {
                /** pri value was not replaced, take the old value */
                nwk_qos_pri = DNX_QOS_IDX_NWK_QOS_PCP_GET(old_nwk_qos_piped);
            }
            if (nwk_cfi_en == FALSE)
            {
                /** cfi value was not replaced, take the old value */
                nwk_qos_cfi = DNX_QOS_IDX_NWK_QOS_CFI_GET(old_nwk_qos_piped);
            }
        }
        ...
        {
            if ((initiator_info->egress_qos_model.egress_qos == bcmQosEgressModelPipeNextNameSpace) &&
                (table_id != DBAL_TABLE_EEDB_ARP))
            {
                new_profile = DNX_QOS_EXPLICIT_IDX_L2(nwk_qos_pri, nwk_qos_cfi);
            }
        }
        nwk_qos_update = TRUE;
    }

    if ((exp0_en == TRUE) || (exp1_en == TRUE))
    {
        if ((is_qos_model_valid == TRUE) && (is_nwk_qos_index_valid == TRUE))
        {
            if (exp0_en == FALSE)
            {
                exp0_value = DNX_QOS_IDX_NWK_QOS_GET_EXP0(old_nwk_qos_piped);
            }
            if (exp1_en == FALSE)
            {
                exp1_value = DNX_QOS_IDX_NWK_QOS_GET_EXP1(old_nwk_qos_piped);
            }
        }
        else if (exp0_en == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "cannot set special_field FLOW_S_F_QOS_EXP_1 without setting special field FLOW_S_F_QOS_EXP");
        }
        nwk_qos_pipe_value = DNX_QOS_IDX_EXP_TO_NWK_QOS(exp0_value, exp1_value);
        nwk_qos_update = TRUE;
    }
    ...
    else
    {
        if (initiator_info->valid_elements_set & BCM_FLOW_INITIATOR_ELEMENT_QOS_EGRESS_MODEL_VALID)
        {
            /**  pipeNextNameSpace mode,  AC uses explicit nwk_qos_idx except arp_plus-ac*/
            if ((initiator_info->egress_qos_model.egress_qos == bcmQosEgressModelPipeMyNameSpace) ||
                ((initiator_info->egress_qos_model.egress_qos == bcmQosEgressModelPipeNextNameSpace) &&
                 ((!(nwk_pri_en == TRUE) && !(nwk_cfi_en == TRUE)) || (table_id == DBAL_TABLE_EEDB_ARP))))
            {
        ...
        if (nwk_qos_update)
        {
            ...
            if (is_pipe_mode_old && is_pipe_mode_new && (was_nwk_qos_set == TRUE))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_update
                                (unit, (uint16) nwk_qos_pipe_value, (uint16) nwk_qos_pipe_value, 0,
            ...
            else if (is_pipe_mode_new)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_allocate
                                (unit, (uint8) nwk_qos_pipe_value, (uint8) nwk_qos_pipe_value, 0, (int *) &new_profile,
    ...
    {
        SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_set
                        (unit, new_profile, (uint8) nwk_qos_pipe_value, (uint8) nwk_qos_pipe_value, 0));
    }
    ...
    uint8 *is_qos_valid)
{
    uint8 is_field_valid;
    uint8 idx;
    uint8 more_than_one_label = FALSE;
    dbal_enum_value_field_encap_qos_model_e encap_qos_model;
    ...
    if (is_field_valid == TRUE)
    {
            ...
            (encap_qos_model == DBAL_ENUM_FVAL_ENCAP_QOS_MODEL_PIPE_ECN))
        {
            ...
            uint8 exp0_found = FALSE;
            uint8 exp1_found = FALSE;
            ...
            if (nwk_qos_idx & (DNX_QOS_EXPLICIT_IDX_FLAG << DNX_QOS_EXPLICIT_IDX_OFFSET))
            {
                /*
                 * explicit 
                 */
            ...
            else
            {
                SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_attr_get
                                (unit, nwk_qos_idx, &nwk_qos, &qos_profile_get_dummy_var, &qos_profile_get_dummy_var));
            }
            ...
            for (idx = 0; flow_app_info->special_fields[idx] != FLOW_S_F_EMPTY; idx++)
            {
                ...
                else if (flow_app_info->special_fields[idx] == FLOW_S_F_QOS_EXP_1)
                {
                    SPECIAL_FIELD_UINT32_DATA_ADD(special_fields, FLOW_S_F_QOS_EXP_1,
                                                  DNX_QOS_IDX_NWK_QOS_GET_EXP1(nwk_qos));
                    exp1_found = TRUE;
                    if (exp0_found)
                    {
                        break;
                ...
                else if (flow_app_info->special_fields[idx] == FLOW_S_F_QOS_PRI)
                {
                    SPECIAL_FIELD_UINT32_DATA_ADD(special_fields, FLOW_S_F_QOS_PRI,
                                                  DNX_QOS_IDX_NWK_QOS_PCP_GET(nwk_qos));
                    pri_cfi_found++;
                    if (pri_cfi_found > 1)
                    {
                        break;
                ...
                else if (flow_app_info->special_fields[idx] == FLOW_S_F_QOS_CFI)
                {
                    SPECIAL_FIELD_UINT32_DATA_ADD(special_fields, FLOW_S_F_QOS_CFI,
                                                  DNX_QOS_IDX_NWK_QOS_CFI_GET(nwk_qos));
                    pri_cfi_found++;
                    if (pri_cfi_found > 1)
                    {
                        break;
    ...
    if (is_set)
    {
            ...
            (encap_qos_model == DBAL_ENUM_FVAL_ENCAP_QOS_MODEL_PIPE_ECN))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get(unit, entry_handle_id,
                                                                               DBAL_FIELD_NWK_QOS_IDX,
                                                                               &is_field_valid, &profile));
            if (!(profile & (DNX_QOS_EXPLICIT_IDX_FLAG << DNX_QOS_EXPLICIT_IDX_OFFSET)))
            {
                if (is_field_valid)
                {
                    /** Free old QOS profile template */
    ...
    bcm_flow_special_fields_t * special_fields)
{
        ...
        (!_SHR_IS_FLAG_SET(bcm_app_info->valid_elements_set, BCM_FLOW_INITIATOR_ELEMENT_QOS_EGRESS_MODEL_VALID)))
    {
        ...
        if (_SHR_IS_FLAG_SET(bcm_app_info->valid_elements_set, BCM_FLOW_INITIATOR_ELEMENT_TTL_VALID))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
             ...
             (_SHR_IS_FLAG_SET(bcm_app_info->valid_elements_set, BCM_FLOW_INITIATOR_ELEMENT_QOS_EGRESS_MODEL_VALID)))
    {
        /** if BCM_FLOW_INITIATOR_ELEMENT_QOS_EGRESS_MODEL_VALID was set, 
           check that either both pri and cfi were set, or none of them */
        uint8 pri_found, cfi_found;
        uint8 exp_found, exp1_found;
        ...
        /** if BCM_FLOW_INITIATOR_ELEMENT_QOS_EGRESS_MODEL_VALID was set, 
           check EXP_1 field must be set following EXP field*/
        rv = dnx_flow_special_field_data_get(unit, special_fields, FLOW_S_F_QOS_EXP, &special_field_data);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
        exp_found = (rv == _SHR_E_NONE) ? TRUE : FALSE;

        rv = dnx_flow_special_field_data_get(unit, special_fields, FLOW_S_F_QOS_EXP_1, &special_field_data);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
        exp1_found = (rv == _SHR_E_NONE) ? TRUE : FALSE;

        if (exp1_found)
        {
            if (!more_than_one_label)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "special_fields FLOW_S_F_QOS_EXP_1 couldn't be set in case only one label");
            }
            else if (!exp_found)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "special_fields FLOW_S_F_QOS_EXP_1 must be set following FLOW_S_F_QOS_EXP field");
            }
        }
    }
        ...
        _SHR_IS_FLAG_SET(flow_app_info->valid_common_fields_bitmap, BCM_FLOW_INITIATOR_ELEMENT_QOS_EGRESS_MODEL_VALID))
    {
        ...
        uint8 exp_clear = FALSE, exp1_clear = FALSE;
        ...
        if (rv == _SHR_E_NONE)
        {
            exp_clear = special_field_data.is_clear ? TRUE : FALSE;
        }
        /** in replace, cannot clear exp or exp_1 without clearing the other if existing*/
        /**set exp2_clear same with exp1_clear as default*/
        exp1_clear = exp_clear;
        rv = dnx_flow_special_field_data_get(unit, special_fields, FLOW_S_F_QOS_EXP_1, &special_field_data);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
        if (rv == _SHR_E_NONE)
        {
            exp1_clear = special_field_data.is_clear ? TRUE : FALSE;
        }

        if (exp_clear != exp1_clear)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "special_fields FLOW_S_F_QOS_EXP and FLOW_S_F_QOS_EXP_1 must be cleared together");
        }
    }
    ...
    uint8 *qos_propagation_prof_last_ref)
{
    ...
    if (!_SHR_IS_FLAG_SET(terminator_info->valid_elements_clear, BCM_FLOW_TERMINATOR_ELEMENT_QOS_INGRESS_MODEL_VALID))
    {
        ...
        {
            if (flow_app_info->flow_table[0] == DBAL_TABLE_IN_LIF_IPvX_TUNNELS)
            {
                ecn_propagation_type = DNX_QOS_PROPAGATION_TYPE_STUCK;
            }
        }
    }
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    if (is_set)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_PROPAGATION_PROFILE,
                                                            INST_SINGLE, &propagation_prof));
