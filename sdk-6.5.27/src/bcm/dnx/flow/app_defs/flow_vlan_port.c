...
#define VLAN_PORT_LL_TERMINATOR_INDICATIONS (SAL_BIT(FLOW_APP_TERM_IND_QOS_PHB_DEFAULT_PIPE) | \
                                             ...
                                             SAL_BIT(FLOW_APP_TERM_IND_L2_LEARN_OPTIMIZATION))
    ...
    for (idx = 0; idx < special_fields->actual_nof_special_fields; idx++)
    {
        if (special_fields->special_fields[idx].field_id == FLOW_S_F_PORT)
        {
                     ...
                     || (flow_cmd_control->flow_cb_type == FLOW_CB_TYPE_TRAVERSE))
            {
                 ...
                 */
                core_id = (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc)) ?
                    DBAL_CORE_DEFAULT : _SHR_CORE_ALL;
                SHR_IF_ERR_EXIT(dnx_algo_gpm_flow_gport_from_local_lif
                                (unit, default_in_lif, TRUE, core_id, &(flow_handle_info->flow_id)));
            }
            /*
