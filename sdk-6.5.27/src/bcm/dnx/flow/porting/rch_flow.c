    ...
    bcm_l2_egress_t * egr)
{
    ...
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, egr->vlan_port_id, BCM_GPORT_TYPE_FLOW_LIF,
    ...
    if ((egr->encap_access == bcmEncapAccessInvalid) && (!_SHR_IS_FLAG_SET(egr->flags, BCM_L2_EGRESS_REPLACE)))
    {
        if (egr->recycle_app == bcmL2EgressRecycleAppExtendedEncap)
        {
            flow_init_info.encap_access = bcmEncapAccessArp;
        }
    }
    else
    {
        flow_init_info.encap_access = egr->encap_access;
    }
    ...
    bcm_l2_egress_t * egr)
{
    ...
    for (int i = 0; i < special_fields.actual_nof_special_fields; i++)
    {
        ...
        if (special_fields.special_fields[i].field_id == FLOW_S_F_RCH_ADDITIONAL_BYTES_TO_STRIP)
        {
            egr->additional_egress_termination_size = special_fields.special_fields[i].shr_var_uint32;
        }
    }

    /** set Invalid encap_access instead of its real value, if it match to one of the creation conditions.*/
    egr->encap_access = flow_init_info.encap_access;
    if ((flow_init_info.encap_access == bcmEncapAccessArp && egr->recycle_app == bcmL2EgressRecycleAppExtendedEncap) ||
        (flow_init_info.encap_access == bcmEncapAccessAc))
    {
