    ...
    int value)
{
    ...
    /**Get the already allocated terminator and store the other L2/L3 ingress property fields*/
    SHR_IF_ERR_EXIT(bcm_dnx_flow_terminator_info_get(unit, &flow_handle_info, &flow_term_info_get, &special_fields));

    if (flow_term_info_get.valid_elements_set & BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID)
    {
        lif_is_l2 = TRUE;
    }
    else if (flow_term_info_get.valid_elements_set & BCM_FLOW_TERMINATOR_ELEMENT_L3_INGRESS_INFO_VALID)
    {
        lif_is_l2 = FALSE;
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "LIF should be either L2 or L3 one \n");
    }
    ...
    if (lif_is_l2)
    {
        switch (port_control_type)
        {
            case bcmPortControlBridge:
            {
                ...
                flow_term_info.l2_ingress_info.oam_default_profile =
                    flow_term_info_get.l2_ingress_info.oam_default_profile;
                break;
            }
            ...
            case bcmPortControlOuterPolicerRemark:
            {
                ...
                flow_term_info.l2_ingress_info.inner_policer_remark =
                    flow_term_info_get.l2_ingress_info.inner_policer_remark;
                flow_term_info.l2_ingress_info.same_interface_mode =
                    flow_term_info_get.l2_ingress_info.same_interface_mode;
                flow_term_info.l2_ingress_info.oam_default_profile =
                    flow_term_info_get.l2_ingress_info.oam_default_profile;
                break;
            }
            ...
            case bcmPortControlOamDefaultProfile:
            {
                ...
                flow_term_info.l2_ingress_info.same_interface_mode =
                    flow_term_info_get.l2_ingress_info.same_interface_mode;
                flow_term_info.l2_ingress_info.oam_default_profile = value;
                break;
            }
            default:
            {
    ...
    else
    {
        switch (port_control_type)
        {
            case bcmPortControlOamDefaultProfile:
            {
                flow_term_info.valid_elements_set |= BCM_FLOW_TERMINATOR_ELEMENT_L3_INGRESS_INFO_VALID;
                flow_term_info.l3_ingress_info.oam_default_profile = value;
                break;
            }
            default:
            {
        ...
        flow_handle_info.flags = BCM_FLOW_HANDLE_INFO_REPLACE | BCM_FLOW_HANDLE_INFO_WITH_ID;
        flow_term_info.l3_ingress_info.urpf_mode = flow_term_info_get.l3_ingress_info.urpf_mode;
        flow_term_info.l3_ingress_info.l3_ingress_flags = flow_term_info_get.l3_ingress_info.l3_ingress_flags;
        flow_term_info.l3_ingress_info.service_type = flow_term_info_get.l3_ingress_info.service_type;
    }

    /**Replace the flow terminator with only the proper element, based on the switch control type, changed*/
    ...
    int *value)
{
    ...
    switch (port_control_type)
    {
        ...
        case bcmPortControlOamDefaultProfile:
        {
            if (_SHR_IS_FLAG_SET(common_field_bit_set, BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID))
            {
                common_field_valid_bit = BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID;
                value_ptr = &(flow_term_info.l2_ingress_info.oam_default_profile);
                value_bytes = sizeof(flow_term_info.l2_ingress_info.oam_default_profile);
            }
            else
            {
                common_field_valid_bit = BCM_FLOW_TERMINATOR_ELEMENT_L3_INGRESS_INFO_VALID;
                value_ptr = &(flow_term_info.l3_ingress_info.oam_default_profile);
                value_bytes = sizeof(flow_term_info.l3_ingress_info.oam_default_profile);
            }
            break;
        }
        default:
    ...
    bcm_port_flood_group_t * flood_groups)
{
    ...
    bcm_flow_special_fields_t special_fields;
    int field_idx;
    uint8 uuc_found = FALSE, umc_found = FALSE, bc_found = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    ...
    for (field_idx = 0; field_idx < special_fields.actual_nof_special_fields; field_idx++)
    {
        ...
        if (uuc_found == TRUE && umc_found == TRUE && bc_found == TRUE)
        {
            break;
        }
    }

    /** If the fields are not found, return the default value for legacy APIs.*/
    if (uuc_found == FALSE)
    {
