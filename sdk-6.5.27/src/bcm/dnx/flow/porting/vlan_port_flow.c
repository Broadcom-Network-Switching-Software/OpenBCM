...
static shr_error_e dnx_vswitch_port_flow_add_internal(
    int unit,
    bcm_vlan_t vsi,
    int learn_enable,
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    vlan_port_flow_term->l2_learn_info.learn_strength = vlan_port->learn_strength;
    vlan_port_flow_term->l2_learn_info.l2_learn_info_flags = 0;

    /** Virtual gport learn info requires setting the BCM_FLOW_L2_LEARN_INFO_DEST_ONLY flag */
    ...
    /** Check whether Optimized learn can be applied. but only if the flush_group isn't set as there's no support for
     *  Extended Optimized learn info. */
    if ((vlan_port->failover_port_id == 0) && (vlan_port_flow_term->l2_learn_info.flush_group == 0))
    {
        ...
        if ((dbal_dest_type == DBAL_FIELD_PORT_ID) || (dbal_dest_type == DBAL_FIELD_LAG_ID))
        {
            ...
            {
                vlan_port_flow_term->l2_learn_info.l2_learn_info_flags |= BCM_FLOW_L2_LEARN_INFO_OPTIMIZED;
            }
        }
    }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    else
    {
        if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_RECYCLE))
        {
            ...
            if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_flow_terminator_l2_learn_info_set(unit, flow_handle_info,
                                                                                vlan_port_flow_term, vlan_port));
            }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_REPLACE))
    {
         ...
         */
        if (!(_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE) &&
              _SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_VLAN_TRANSLATION) && vlan_port->vsi == 0))
        {
    ...
    bcm_vlan_port_t * old_vlan_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vlan_port_flow_terminator_common_fields_set
                    (unit, &flow_handle_info, &vlan_port_flow_term, vlan_port));
    ...
    bcm_flow_handle_info_t * flow_handle_info)
{
    ...
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
    {
        ...
        else if (!(_SHR_IS_FLAG_SET(vlan_port_flow_term.l2_learn_info.l2_learn_info_flags,
                                    BCM_FLOW_L2_LEARN_INFO_OPTIMIZED)))
        {
            /*
    ...
    vlan_port->learn_strength = vlan_port_flow_term.l2_learn_info.learn_strength;
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    /*
     * delete SW databases if exist. Might be exist if the LIF was replaced from MP to P2P (part of vswitch cross
     * connect API porting). 
     */
    rv = algo_gpm_gport_l2_forward_info_get(unit, flow_handle_info.flow_id,
                                            ALGO_GPM_GPORT_L2_FWD_INFO_FLAGS_NO_ERR_MSG_NOT_FOUND, &forward_info);
    ...
    void *data)
{
    ...
    if (call_cb == TRUE)
    {
        if (flow_user_data->is_vswitch_delete == TRUE)
        {
            /*
             * Check VSI value:
             * Note: vsi ==0 means "ALL VSIs"
             */
            if ((flow_user_data->vsi == 0) || (flow_user_data->vsi == terminator_info->vsi))
            {
                SHR_IF_ERR_EXIT(dnx_vswitch_port_flow_add_internal(unit, DNX_VSI_DEFAULT, FALSE,
    ...
    void *user_data)
{
    ...
    flow_user_data.is_vswitch_delete = is_vswitch_delete;
    flow_user_data.vsi = vsi;
    ...
    void *data)
{
    ...
    if (gport_hw_resources.local_in_lif != flow_user_data->vtt4_default_0_tag_in_lif)
    {
        if (flow_user_data->is_vswitch_delete == TRUE)
        {
             ...
             * Avoid the entries that their result type is "without VSI
             */
    ...
    void *user_data)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_vlan_port_flow_traverse_ingress_non_native
                        (unit, is_traverse_all, is_vswitch_delete, 0, trav_fn, user_data));
    }
    ...
    if ((is_traverse_all == TRUE) || (is_native == TRUE && is_virtual == FALSE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_vlan_port_flow_traverse_ingress_native(unit, is_vswitch_delete, 0, trav_fn, user_data));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_vlan_port_flow_traverse_ingress_native_virtual
                        (unit, is_vswitch_delete, 0, trav_fn, user_data));
    }
...
/**
  * \brief
 * Internal function for associating a given flow_id  with a VSI by updating the vsi
 * attribute of an In-LIF using flow module
 */
static shr_error_e
dnx_vswitch_port_flow_add_internal(
    int unit,
    bcm_vlan_t vsi,
    int learn_enable,
    bcm_flow_handle_t flow_handle,
    bcm_gport_t flow_id)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
                    (unit, flow_handle_info.flow_id, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS,
                     &gport_hw_resources));
    ...
    bcm_gport_t port)
{
    ...
    /** Encode VLAN-PORT gport back to FLOW gport */
    SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                    (unit, port, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR, &flow_id));

    SHR_IF_ERR_EXIT(dnx_vswitch_port_flow_add_internal(unit, vsi, learn_enable, flow_handle, flow_id));
 ...
 */
shr_error_e
dnx_vswitch_port_flow_delete_all(
    int unit,
    bcm_vlan_t vsi)
{
    int is_symmetric_traverse;
    ...
    is_symmetric_traverse = TRUE;
    is_vswitch_delete = TRUE;

    /*
     * Traverse on Ingress non-native LIFs:
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_flow_traverse_ingress_non_native
                    (unit, is_symmetric_traverse, is_vswitch_delete, vsi, NULL, NULL));

    /*
     * Traverse on Ingress native non-virtual LIFs:
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_flow_traverse_ingress_native(unit, is_vswitch_delete, vsi, NULL, NULL));

    /*
     * Traverse on Ingress native virtual LIFs:
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_flow_traverse_ingress_native_virtual(unit, is_vswitch_delete, vsi, NULL, NULL));
