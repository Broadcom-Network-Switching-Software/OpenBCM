...
#include "src/bcm/dnx/algo/lif_mngr/dbx_pre_compiled_lif_mdb_auto_generated_xml_parser.h"
...
typedef enum
{
    FLOW_MISMATCH_COMMON_FIELDS_SET_MISMATCH = 0,
    FLOW_MISMATCH_COMMON_FIELDS_CLEAR_MISMATCH = 1,
    FLOW_MISMATCH_COMMON_FIELDS_SET_CLEAR_MISMATCH = 2
} flow_mismatch_common_fields_e;
...
********************************************************************************************/
static shr_error_e
flow_no_special_fields_validate(
    int unit,
    dnx_flow_app_config_t * flow_app_info)
{
    uint8 field_idx;
    ...
    {
        SHR_IF_ERR_EXIT(flow_special_field_info_get(unit, flow_app_info->special_fields[field_idx], &curr_field));

        if (dnx_flow_special_field_is_ind_set(unit, curr_field, FLOW_SPECIAL_FIELD_IND_LIF_MANDATORY) &&
            (curr_field->payload_type != FLOW_PAYLOAD_TYPE_ENABLER))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Missing Mandatory special field %s in create setting",
...
/*
 * this function is called ONLY when an invalid common fields was found during the common_fields_validate
 * it will pass one field at the time and print a list of the invalid fields
 */
static shr_error_e
common_fields_invalid_fields_print(
    int unit,
    flow_mismatch_common_fields_e invalid_type,
    dnx_flow_app_config_t * flow_app_info,
    uint32 selected_bitmap,
    uint32 invalid_bitmap)
{
    uint32 enabler = 1;
    uint8 first_field = TRUE;
    char print_str[STR_SIZE] = { 0 };
    ...
    if (!FLOW_APP_IS_LIF(flow_app_info->flow_app_type))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Function can only used for LIF applications\n");
    }

    if (invalid_type == FLOW_MISMATCH_COMMON_FIELDS_SET_MISMATCH)
    {
        sal_snprintf(print_str, STR_SIZE,
                     "Unsupported common bitmap (0x%x) was set to application %s, valid common bitmap is 0x%x. The invalid fields are: ",
                     selected_bitmap, flow_app_info->app_name, flow_app_info->valid_common_fields_bitmap);
    }
    else if (invalid_type == FLOW_MISMATCH_COMMON_FIELDS_CLEAR_MISMATCH)
    {
        sal_snprintf(print_str, STR_SIZE,
                     "Unsupported common bitmap (0x%x) was cleared to application %s, valid common bitmap is 0x%x. The invalid fields are: ",
                     selected_bitmap, flow_app_info->app_name, flow_app_info->valid_common_fields_bitmap);
    }
    else if (invalid_type == FLOW_MISMATCH_COMMON_FIELDS_SET_CLEAR_MISMATCH)
    {
        sal_snprintf(print_str, STR_SIZE,
                     "Invalid configuration, for application %s, fields can either be set (0x%x) or cleared (0x%x), not both. The invalid fields are: ",
                     flow_app_info->app_name, selected_bitmap, invalid_bitmap);
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid error type %d\n", invalid_type);
    }

    while (selected_bitmap != 0)
    {
        char str_tmp[STR_SIZE];
        if ((selected_bitmap & 0x1) & (invalid_bitmap & 0x1))
        {
            sal_memset(str_tmp, 0, STR_SIZE);
            if (first_field == TRUE)
            {
                sal_snprintf(str_tmp, STR_SIZE, "%s",
                             dnx_flow_common_field_enabler_to_string(unit, flow_app_info->flow_app_type, enabler));
                first_field = FALSE;
            }
            else
            {
            ...
            sal_strncat_s(print_str, str_tmp, STR_SIZE);
        }

        /** update the loop params */
        selected_bitmap = (selected_bitmap >> 1);
        invalid_bitmap = (invalid_bitmap >> 1);
        enabler = (enabler << 1);
    }
    SHR_ERR_EXIT(_SHR_E_PARAM, "%s\n", print_str);
    ...
    void *bcm_app_info)
{
    ...
    {
        SHR_IF_ERR_EXIT(common_fields_invalid_fields_print(unit, FLOW_MISMATCH_COMMON_FIELDS_SET_MISMATCH,
                                                           flow_app_info, valid_elements_set,
                                                           (~flow_app_info->valid_common_fields_bitmap)));
    ...
    {
        SHR_IF_ERR_EXIT(common_fields_invalid_fields_print(unit, FLOW_MISMATCH_COMMON_FIELDS_CLEAR_MISMATCH,
                                                           flow_app_info, valid_elements_clear,
                                                           (~flow_app_info->valid_common_fields_bitmap)));
    }
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_FLOW_HANDLE_INFO_REPLACE))
    {
        ...
        {
            SHR_IF_ERR_EXIT(common_fields_invalid_fields_print(unit, FLOW_MISMATCH_COMMON_FIELDS_SET_CLEAR_MISMATCH,
                                                               flow_app_info, valid_elements_set,
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (bcm_app_info->valid_elements_set & BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID)
    {
        uint32 nof_network_group_id = dnx_data_lif.in_lif.in_lif_profile_allocate_orientation_get(unit);

        if ((bcm_app_info->l2_ingress_info.ingress_network_group_id < 0) ||
            ((bcm_app_info->l2_ingress_info.ingress_network_group_id >= nof_network_group_id) &&
             (bcm_app_info->l2_ingress_info.ingress_network_group_id != 0)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error - ingress_network_group_id = %d, is out of range [0:%d].\n",
                         bcm_app_info->l2_ingress_info.ingress_network_group_id, (nof_network_group_id - 1));
        }
    }

    /** l2 and l3 ingress info cannot be cleared */
    if (bcm_app_info->valid_elements_clear & BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "cannot clear BCM_FLOW_TERMINATOR_ELEMENT_L2_INGRESS_INFO_VALID");
    }
    if (bcm_app_info->valid_elements_clear & BCM_FLOW_TERMINATOR_ELEMENT_L3_INGRESS_INFO_VALID)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "cannot clear BCM_FLOW_TERMINATOR_ELEMENT_L3_INGRESS_INFO_VALID");
    }

    /** restrictions which are relevant only for application that have both l2_ingress_info and l3_ingress_info */
    if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_inlif_legacy_profile_logic)) &&
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** outlif group cannot be changed when replacing */
    if (_SHR_IS_FLAG_SET(flow_handle_info->flags, BCM_FLOW_HANDLE_INFO_REPLACE) &&
        _SHR_IS_FLAG_SET(bcm_app_info->valid_elements_set, BCM_FLOW_INITIATOR_ELEMENT_OUTLIF_PROFILE_GROUP_VALID))
    {
        ...
        if (alloc_info.outlif_group != bcm_app_info->outlif_profile_group)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "common field outlif_group cannot be changed in replace");
        }
    }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "common field outlif_group %d cannot be above max value %d",
                     bcm_app_info->outlif_profile_group, dnx_data_lif.global_lif.global_lif_group_max_val_get(unit));
    }
...
static shr_error_e
dnx_flow_special_fields_unchangeable_verify(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    for (idx = 0; idx < special_fields->actual_nof_special_fields; idx++)
    {
        ...
        {
            shr_error_e rv;
            bcm_flow_special_field_t special_field = { 0 };

            rv = flow_special_fields_get_one_field(unit, flow_handle_info, flow_app_info, gport_hw_resources,
                                                   get_entry_handle_id, user_field, curr_field, &(special_field));

            SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
            if (rv == _SHR_E_NONE)
            {
                if (sal_memcmp(&special_field, &(special_fields->special_fields[idx]), sizeof(bcm_flow_special_field_t))
                    != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot change unchangeable field %s",
                                 dnx_flow_special_field_to_string(unit, user_field));
                }
            }
        }
...
static shr_error_e
dnx_flow_initiator_info_replace_verify(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources,
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (special_fields)
    {
        SHR_IF_ERR_EXIT(dnx_flow_special_fields_unchangeable_verify
                        (unit, flow_handle_info, flow_app_info, gport_hw_resources, get_entry_handle_id,
                         special_fields));
    }
...
static shr_error_e
dnx_flow_terminator_info_replace_verify(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources,
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** in case of create and no special fields were given, insure the flow app has no mandatory fields - relevant only for lif apps */
    ...
    {
        SHR_IF_ERR_EXIT(flow_no_special_fields_validate(unit, flow_app_info));
    }
    ...
    if ((flow_app_type == FLOW_APP_TYPE_INIT)
        ...
        && !dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_egress_virtual_supported))
    {
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /*
     * In case this is a match application, need to validate the special fields also, as they are the key 
     */
    if (FLOW_APP_IS_MATCH(flow_app_info->flow_app_type))
    {
        SHR_NULL_CHECK(special_fields, _SHR_E_PARAM, "key_special_fields");
        SHR_IF_ERR_EXIT(flow_special_fields_validate(unit, flow_handle_info, flow_app_info, special_fields));
    }
...
**********************************************************************************************/
/*
 * the extended termination feature, is used for recycled packets (pass from egress back to ingress)
 * when extended termination is set, the recycles packet, in ingress, will get the local lif of its 1st passing
 */
static shr_error_e
    ...
    int global_lif_id)
{
    ...
    else        /* not symmetric */
    {
        ...
        {
            if (local_lif_payload.local_lifs_array.local_lifs[core_id] == LIF_MNGR_INLIF_INVALID)
            {
                continue;
            }
            /** setting the current local_lif */
            SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_write(unit, global_lif_id, core_id,
                                                            local_lif_payload.local_lifs_array.local_lifs[core_id],
                                                            FALSE));
        }
    }
    ...
    uint32 *terminato_flags)
{
    ...
    else        /* not symmetric */
    {
        ...
        for (core_id = 0; core_id < nof_cores; core_id++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_get(unit, global_lif_id, core_id, &is_entry_written));
            break;
        }
    }
    ...
    else        /* not symmetric */
    {
        ...
        if (is_entry_written == TRUE)
        {
            for (; core_id < nof_cores; core_id++)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_delete(unit, global_lif_id, core_id));
            }
        }
    }
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_lif_mngr_max_inlif_id_get(unit, &nof_sbc_lifs, &nof_dpc_lifs));

    sal_memset(&hash_tbl_init_info, 0, sizeof(sw_state_htbl_init_info_t));
    hash_tbl_init_info.max_nof_elements = nof_sbc_lifs + nof_dpc_lifs;
    hash_tbl_init_info.expected_nof_elements = hash_tbl_init_info.max_nof_elements;

    SHR_IF_ERR_EXIT(flow_db.ingress_gport_to_match_info_htb.create(unit, &hash_tbl_init_info));
    ...
    int unit)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_flow_match_init(unit));
        }
    }
    ...
    if ((!dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc)) && !sw_state_is_warm_boot(unit))
    {
        /** initialize only virtual inlif algo for legacy devices, in the regular sequence init is done by flow algo init */
        dnx_algo_res_create_data_t ing_virtual_res_data;
        uint8 is_init;

        /** Initialize algo_flow_lif sw state. */
        SHR_IF_ERR_EXIT(algo_flow_lif.is_init(unit, &is_init));

        if (!is_init)
        {
            SHR_IF_ERR_EXIT(algo_flow_lif.init(unit));
        }

        sal_memset(&ing_virtual_res_data, 0, sizeof(ing_virtual_res_data));

        /*
         * Create a resource allocation pool for ingress virtual gport IDs 
         */
        sal_memset(&ing_virtual_res_data, 0, sizeof(ing_virtual_res_data));
        ing_virtual_res_data.first_element = 0;
        ing_virtual_res_data.nof_elements = dnx_data_lif.virtual_lif.flow_ingress_ids_get(unit);
        ing_virtual_res_data.flags = 0;
        sal_strncpy(ing_virtual_res_data.name, "FLOW INGRESS VIRTUAL GPORT ID", DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
        SHR_IF_ERR_EXIT(algo_flow_lif.flow_ingress_virtual_gport_id.create(unit, &ing_virtual_res_data, NULL));
    }
...
/** get the special field value according to the values of in the entry handle, in case the field was not found
 *  returns _SHR_E_NOT_FOUND */
shr_error_e
flow_special_fields_get_one_field(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources,
    ...
    bcm_flow_special_field_t * special_field)
{
    ...
    {
        /** need to skip this field */
        SHR_IF_ERR_EXIT_NO_MSG(_SHR_E_NOT_FOUND);
    }
    if (curr_field->get != NULL)
    {
        rv = curr_field->get(unit, entry_handle_id, flow_handle_info, flow_app_info, gport_hw_resources,
                             special_field_id, special_field);
        if ((rv != _SHR_E_NOT_FOUND) && (rv != _SHR_E_NONE))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "issue when getting field %s via field CB", curr_field->name);
        }
        else
        {
            SHR_SET_CURRENT_ERR(rv);
    ...
    {
        int is_virtual_match_key = dnx_flow_special_field_is_ind_set(unit, curr_field, FLOW_SPECIAL_FIELD_IND_IS_KEY);
        if (!is_virtual_match_key)
        {
            rv = flow_special_field_direct_mapping_get(unit, curr_field, entry_handle_id, special_field);
        }
        else
        {
            rv = flow_special_field_key_direct_mapping_get(unit, (flow_special_field_info_t *) curr_field,
                                                           entry_handle_id, DBAL_TABLE_TYPE_NONE, special_field);
        }
        ...
        else
        {
            if (rv != _SHR_E_NOT_FOUND)
            {
                SHR_ERR_EXIT(rv, "issue when getting field %s via direct mapping", curr_field->name);
            }
        }
        SHR_SET_CURRENT_ERR(rv);
        SHR_EXIT();
    ...
    /** Reaching here means that the field was not handled, this is wrong */
    SHR_ERR_EXIT(_SHR_E_PARAM, "No mapping for field %s ", curr_field->name);
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    if (flow_app_info->special_fields[0] != FLOW_S_F_EMPTY)
    {
        ...
        for (ii = 0; flow_app_info->special_fields[ii] != FLOW_S_F_EMPTY; ii++)
        {
            ...
            rv = flow_special_fields_get_one_field(unit, flow_handle_info, flow_app_info, gport_hw_resources,
                                                   entry_handle_id, flow_app_info->special_fields[ii], curr_field,
                                                   &(special_fields->special_fields[idx]));

            SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
            if (rv == _SHR_E_NONE)
            {
                /** field was found, need to increament the idx */
                idx++;
            }
        }
    ...
    uint32 entry_handle_id)
{
    ...
    while (valid_elements_set != 0)
    {
        if (valid_elements_set & 0x1)
        {
            ...
            {
                /** Call to the common field set callback */
    ...
    uint32 get_entry_handle_id)
{
    ...
    for (field = 0; field < num_common_fields; field++)
    {
        ...
        if (_SHR_IS_FLAG_SET(valid_elements_set, field_valid_bit))
        {
            if (common_fields_arr[field]->field_cbs.field_set_cb)
            {
                /** For standard fields - perform simple set cb */
                SHR_IF_ERR_EXIT(common_fields_arr[field]->
                                field_cbs.field_set_cb(unit, entry_handle_id, flow_app_type, flow_handle_info,
                                                       gport_hw_resources, flow_app_info, bcm_struct_info));
            }
        ...
        {
            /** If no flag was set - keep original field value. Perform get on original entry. If exists - set previous value.
             *  Otherwise - leave default value */
            if ((!common_fields_arr[field]->is_profile)
                && (common_fields_arr[field]->mapped_dbal_field != DBAL_FIELD_EMPTY))
            {
                uint32 uint32_arr_value[BCM_FLOW_SPECIAL_FIELD_UIN32_ARR_MAX_SIZE] = { 0 };

                /** direct mapping to DBAL field */
                rv = dbal_entry_handle_value_field_arr32_get
                    (unit, get_entry_handle_id, common_fields_arr[field]->mapped_dbal_field, INST_SINGLE,
                     uint32_arr_value);
                if (rv == _SHR_E_NONE)
                {
                    dbal_entry_value_field_arr32_set(unit, entry_handle_id, common_fields_arr[field]->mapped_dbal_field,
                                                     INST_SINGLE, uint32_arr_value);
                }
            }
            else if (common_fields_arr[field]->field_cbs.field_get_cb)
            {
        ...
        else    /* Valid elements clear - in case of a delete cb - call this cb to free the field */
        {
        ...
        if (temp_common_bitmap & 0x1)
        {
            if (common_fields_arr[field]->field_cbs.field_get_cb)
            {
                rv = common_fields_arr[field]->field_cbs.field_get_cb(unit, entry_handle_id,
                                                                      flow_app_type,
                                                                      flow_handle_info,
                                                                      gport_hw_resources, flow_app_info,
                                                                      bcm_struct_info);
                if (rv == _SHR_E_NONE)
                {
                    if (flow_app_type == FLOW_APP_TYPE_TERM)
...
bcm_gport_t
dnx_flow_gport_encode_by_app(
    int unit,
    uint32 global_lif_or_virtual_id,
    uint32 is_virtual,
    const dnx_flow_app_config_t * flow_app_info)
{
    dnx_flow_app_type_e flow_app_type = flow_app_info->flow_app_type;
    uint32 is_terminator, is_local_lif, is_esem_default;

    is_terminator = ((flow_app_type == FLOW_APP_TYPE_TERM)
                     || (flow_app_type == FLOW_APP_TYPE_TERM_MATCH)) ? TRUE : FALSE;
    is_local_lif = (dnx_flow_app_is_ind_set(unit, flow_app_info, FLOW_APP_INIT_IND_NO_LOCAL_LIF)) ? FALSE : TRUE;
    is_esem_default = (FLOW_LIF_DBAL_TABLE_GET(flow_app_info) == DBAL_TABLE_ESEM_DEFAULT_RESULT_TABLE) ? TRUE : FALSE;

    return dnx_algo_gpm_flow_gport_encode(unit, global_lif_or_virtual_id, is_terminator, is_virtual, is_local_lif,
                                          is_esem_default);
}
    ...
    global_lif_alloc_info_t * alloc_info)
{
    ...
    uint32 alloc_flags = 0, is_terminator;
    ...
    is_terminator = (flow_app_info->flow_app_type == FLOW_APP_TYPE_TERM) ? TRUE : FALSE;
    flow_handle_info->flow_id = dnx_algo_gpm_flow_gport_encode(unit, global_lif, is_terminator, FALSE, TRUE, FALSE);
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** for replace calling verify just after get */
    SHR_INVOKE_VERIFY_DNXC(dnx_flow_terminator_info_replace_verify
                           (unit, flow_handle_info, flow_app_info, &gport_hw_resources, get_entry_handle_id,
                            terminator_info, special_fields));
    ...
    /** update the inlif_proile */
    SHR_IF_ERR_EXIT(dnx_flow_dl_inlif_profile_replace(unit, flow_app_info, entry_handle_id, get_entry_handle_id,
                                                      flow_handle_info->flow_id, &gport_hw_resources, terminator_info,
                                                      special_fields, &last_profile));
    ...
    /** Note: get_entry_handle_id after the following replace will have a DBAL_CORE_ALL in symmetric core mode */
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_terminator_info_replace
                    (unit, flow_handle_info, terminator_info, &gport_hw_resources, entry_handle_id, get_entry_handle_id,
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    /** destroy the inlif_profile */
    SHR_IF_ERR_EXIT(dnx_flow_dl_inlif_profile_destroy(unit, entry_handle_id, flow_handle_info->flow_id,
                                                      gport_hw_resources));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** get the common fields derived from inlif_profile */
    SHR_IF_ERR_EXIT(dnx_flow_dl_inlif_profile_get(unit, entry_handle_id, flow_app_info,
                                                  flow_handle_info->flow_id, gport_hw_resources,
                                                  terminator_info, special_fields));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    /** update the inlif_profile */
    SHR_IF_ERR_EXIT(dnx_flow_dl_inlif_profile_create(unit, flow_app_info, entry_handle_id,
                                                     flow_handle_info->flow_id, terminator_info, special_fields));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_terminator_info_create(unit, flow_handle_info, terminator_info, entry_handle_id,
                                                             selectable_result_types));

    /** update the reclassify table if required*/
    if ((terminator_info->flags & BCM_FLOW_TERMINATOR_INFO_EXTENDED_TERMINATION) && !is_virtual)
    {
...
dnx_flow_terminator_info_get(
    ...
    bcm_flow_terminator_info_t * terminator_info,
    bcm_flow_special_fields_t * special_fields,
    uint32 *retrieved_entry_handle_id)
{
    ...
    if (retrieved_entry_handle_id == NULL)
    {
        /** If no retrieved_entry_handle_id supplied, call a function that retrieves the entry values first */
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_terminator_info_get
                        (unit, flow_app_info, flow_handle_info, &gport_hw_resources, entry_handle_id));
    }
    else
    {
        /** Get the values of valid field from retrieved_entry_handle_id and copy them to entry_handle_id */
        SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_info_from_entry_handle_get
                        (unit, flow_app_info, flow_handle_info, &gport_hw_resources,
                         *retrieved_entry_handle_id, entry_handle_id));
    }
    ...
    dnx_flow_traverse_operation_e traverse_operation)
{
    ...
    int global_lif, global_lif_or_virtual_id;
    ...
    while (!is_end)
    {
        ...
        if (rv == _SHR_E_NONE)
        {
            global_lif_or_virtual_id = global_lif;
            is_virtual = FALSE;
        }
        else
        {
            ...
            SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
            global_lif_or_virtual_id = virtual_id;
            is_virtual = TRUE;
        }

        if (rv == _SHR_E_NONE)
        {
            /** Encode the flow gport */
            flow_handle_info->flow_id =
                dnx_algo_gpm_flow_gport_encode(unit, global_lif_or_virtual_id, TRUE, is_virtual, TRUE, FALSE);
    ...
    bcm_encap_access_t * encap_access)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get(unit,
                                                    ...
                                                    &dbal_table_id, NULL, &logical_phase, NULL, NULL, NULL));
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    int last_network_qos_profile = FLOW_PROFILE_INVALID,
        ...
        last_esem_cmd_profile = FLOW_PROFILE_INVALID;
    ...
    /** for replace calling verify just after get */
    SHR_INVOKE_VERIFY_DNXC(dnx_flow_initiator_info_replace_verify
                           (unit, flow_handle_info, flow_app_info, &gport_hw_resources, get_entry_handle_id,
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_initiator_info_replace
                    (unit, flow_handle_info, &gport_hw_resources, entry_handle_id, get_entry_handle_id,
                     selectable_result_types));
    ...
    {
        /** Clear the ETPP outlif profile as there are no more references to it*/
        SHR_IF_ERR_EXIT(dnx_etpp_out_lif_profile_hw_clear(unit, last_outlif_profile));
    }
    if (last_esem_cmd_profile != FLOW_PROFILE_INVALID)
    {
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_data_clear(unit, last_esem_cmd_profile));
    }
    ...
    while (!is_end)
    {
        ...
        else
        {
            SHR_IF_ERR_EXIT(dnx_flow_initiator_info_get
                            (unit, flow_handle_info, &flow_cmd_control, &initiator_info, &special_fields, NULL));
    ...
    dnx_flow_traverse_operation_e traverse_operation)
{
    ...
    while (!is_end)
    {
        ...
        SHR_IF_ERR_EXIT_EXCEPT_IF(dnx_algo_gpm_flow_gport_from_local_lif(unit, key_field, FALSE, _SHR_CORE_ALL, &gport),
                                  _SHR_E_NOT_FOUND);
    ...
    bcm_flow_special_fields_t * special_fields)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_terminator_info_get
                    (unit, flow_handle_info, &flow_cmd_control, terminator_info, special_fields, NULL));
    ...
    bcm_flow_special_fields_t * key_special_fields)
{
    ...
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dnx_flow_match_info_get
...
int
bcm_dnx_flow_match_info_by_gport_multi_get(
    int unit,
    bcm_gport_t gport,
    int size,
    ...
    int *count)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_match_info_by_gport_multi_get(unit, gport, size, match_array, count));
