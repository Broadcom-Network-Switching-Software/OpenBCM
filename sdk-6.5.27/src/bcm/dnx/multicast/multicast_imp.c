    ...
    for (ref_num = 0; ref_num < DNX_MULTICAST_MAX_NOF_REF_PER_REP; ref_num++)
    {
        SHR_IF_ERR_EXIT(dnx_multicast_hash_key_construct
                        (unit, group_id, is_ingress, core_id, dest_gport, dest2_gport, entry_val, ref_num,
                         &hash_key_temp));
        SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(multicast_db.key_2_mcdb_id_hash.find(unit, &hash_key_temp, &entry_id_temp),
                                        _SHR_E_NOT_FOUND);
    ...
    *found = FALSE;
    ...
    for (ref_num = 0; ref_num < DNX_MULTICAST_MAX_NOF_REF_PER_REP; ref_num++)
    {
        SHR_IF_ERR_EXIT(dnx_multicast_hash_key_construct
                        (unit, group_id, is_ingress, core_id, dest_gport, dest2_gport, entry_val, ref_num,
                         &hash_key_temp));
        SHR_IF_ERR_EXIT_EXCEPT_IF(multicast_db.key_2_mcdb_id_hash.find(unit, &hash_key_temp, &entry_id_temp),
                                  _SHR_E_NOT_FOUND);
        if ((SHR_GET_CURRENT_ERR() == _SHR_E_NONE) && (entry_id_temp == entry_id))
        {
    ...
    bcm_multicast_replication_t * rep_array)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        for (rep_id = 0; rep_id < rep_count; rep_id++)
        {
            ...
            SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(multicast_db.key_2_mcdb_id_hash.replace(unit, &hash_key, &cur_entry_id),
                                            _SHR_E_NOT_FOUND);
            if (SHR_GET_CURRENT_ERR() == _SHR_E_NOT_FOUND)
            {
                SHR_SET_CURRENT_ERR(_SHR_E_NONE);
                SHR_IF_ERR_EXIT_WITH_LOG(multicast_db.key_2_mcdb_id_hash.insert(unit, &hash_key, &cur_entry_id),
    ...
    uint32 new_entry_id)
{
    ...
    SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(multicast_db.key_2_mcdb_id_hash.delete(unit, &hash_key), _SHR_E_NOT_FOUND);
    if (SHR_GET_CURRENT_ERR() == _SHR_E_NOT_FOUND)
    {
        SHR_SET_CURRENT_ERR(_SHR_E_NONE);
    }
    /** Re-map it with the mew MCDB Index */
    SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(multicast_db.key_2_mcdb_id_hash.replace(unit, &hash_key, &new_entry_id),
                                    _SHR_E_NOT_FOUND);
    if (SHR_GET_CURRENT_ERR() == _SHR_E_NOT_FOUND)
    {
        SHR_SET_CURRENT_ERR(_SHR_E_NONE);
        SHR_IF_ERR_EXIT(multicast_db.key_2_mcdb_id_hash.insert(unit, &hash_key, &new_entry_id));
    }
    ...
    bcm_multicast_replication_t * rep_array)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        for (rep_id = 0; rep_id < rep_count_in; rep_id++)
        {
            ...
            SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(multicast_db.key_2_mcdb_id_hash.find(unit, &hash_key, &cur_entry_id),
                                            _SHR_E_NOT_FOUND);
        ...
        while (cur_entry_id != DNX_MULTICAST_END_OF_LIST_PTR)
        {
            ...
            switch (action)
            {
                ...
                case DNX_MULTICAST_ENTRY_ITER_REMOVE_ALL:
                {
                    ...
                    SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(multicast_db.key_2_mcdb_id_hash.delete(unit, &hash_key),
                                                    _SHR_E_NOT_FOUND);
                    if (SHR_GET_CURRENT_ERR() == _SHR_E_NOT_FOUND)
                    {
                        SHR_SET_CURRENT_ERR(_SHR_E_NONE);
    ...
    int unit)
{
    ...
    uint32 entry_handle_id;
    uint32 bier_size;
    sw_state_htb_create_info_t hash_info;
    ...
    else
    {
        ...
        hash_info.hash_func = SWSTATE_HTB_SMALL_HASH;
