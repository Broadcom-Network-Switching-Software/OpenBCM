    ...
    bcm_if_t * encap_id)
{
    ...
    {
        supported_flags |= BCM_OAM_REFELCTOR_ENCAP_SVTAG_PORT_ENABLE;
    }
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_OAM_REFELCTOR_ENCAP_WITH_ID))
    {
        /*
         * range check
         */
    ...
    bcm_if_t * encap_id)
{
    ...
    if (dnx_data_switch.svtag.supported_get(unit))
    {
        uint32 esem_cmd = (_SHR_IS_FLAG_SET(flags, BCM_OAM_REFELCTOR_ENCAP_SVTAG_PORT_ENABLE)) ?
            dnx_data_esem.access_cmd.port_svtag_get(unit) : dnx_data_esem.access_cmd.no_action_get(unit);
