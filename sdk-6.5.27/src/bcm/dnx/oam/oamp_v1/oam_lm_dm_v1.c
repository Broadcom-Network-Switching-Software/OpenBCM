    ...
    if (exchange)
    {
        ...
        if (sal_memcmp(old_oamp_mac_da_lsb, oamp_mac_da_lsb, sizeof(bcm_mac_t)))
        {
            ...
            SHR_IF_ERR_EXIT(algo_oam_db.
                            oam_da_mac_lsb_profile_template.exchange(unit, 0 /* No flags */ , 1, oamp_mac_da_lsb,
    ...
    dnx_oam_loss_delay_oamp_hw_write_data_t * hw_write_data)
{
    ...
    if (exchange)
    {
        ...
        if (sal_memcmp(old_oamp_mac_da_msb, oamp_mac_da_msb, sizeof(bcm_mac_t)))
        {
             ...
             */
            SHR_IF_ERR_EXIT(algo_oam_db.
                            oam_da_mac_msb_profile_template.exchange(unit, 0 /* No flags */ , 1, oamp_mac_da_msb,
    ...
    dnx_oam_loss_delay_oamp_hw_write_data_t * hw_write_data)
{
     ...
     */
    SHR_IF_ERR_EXIT(algo_oam_db.
                    oam_mep_profile.exchange(unit, 0 /* No flags */ , 1, mep_profile_data, old_mep_profile_id,
                                             NULL /* No extra args */ ,
    ...
    bcm_oam_loss_t * loss_ptr)
{
    ...
    /*
     * Check if LM is already configured on this endpoint for non priority mode
     */
    if (!update)
    {
        SHR_IF_ERR_EXIT(dnx_oamp_v1_mep_db_lm_endpoint_get
                        (unit, loss_ptr->id, lm_mep_db_entry, &lm_exists, &lm_stat_exists));
        if ((loss_ptr->pkt_pri_bitmap == 0xff) && (loss_ptr->pm_id == 0) && lm_exists)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error: Loss entry is already allocated with non per priority mode.\n");
        }
    }
    /*
    ...
    dnx_oam_loss_delay_oamp_hw_write_data_t * hw_write_data)
{
    ...
    {
        /**
         * get recycle port from sw table 
        */
    ...
    bcm_oam_delay_t * delay_ptr)
{
        ...
        && (dnx_data_oam.feature.feature_get(unit, dnx_data_oam_feature_up_mep_tod_recycle_enable)))
    {
        SHR_IF_ERR_EXIT(dnx_up_mep_dm_recycle_port_get(unit, 0, &up_mep_tod_recyle_port_core0));
        SHR_IF_ERR_EXIT(dnx_up_mep_dm_recycle_port_get(unit, 1, &up_mep_tod_recyle_port_core1));
        if (!up_mep_tod_recyle_port_core0 || !up_mep_tod_recyle_port_core1)
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                         "Error: please call bcm_oam_control_indexed_set(bcmOamControlUpMepDmRecyclePort) to configure up mep DM recycle port\n");
        }
    ...
    if (!update)
    {
        ...
        if ((delay_ptr->pkt_pri_bitmap == 0xff) && (delay_ptr->pm_id == 0) && dm_exists)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error: delay entry is already allocated with non per priority mode.\n");
        }
    }
    if ((delay_ptr->pkt_pri_bitmap != 0xff) && (delay_ptr->pkt_pri_bitmap != 0))
    {
