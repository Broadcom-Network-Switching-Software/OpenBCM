        ...
        case DNX_OAMP_OAM_GROUP_MD_FORMAT:
            ...
            switch (group_info->name[1])
            {
                case DNX_OAMP_OAM_GROUP_MA_NAME_FORMAT_SHORT:  /* MA_NAME_FORMAT SHORT */
                    ...
                    if (dnx_data_oam.general.oam_sub_type_v2_get(unit) == 0)
                    {
    ...
    dnx_oam_group_temp_data_t * hw_write_data)
{
    ...
    if (DNX_OAMP_IS_V2(unit) || name_type == DNX_OAMP_OAM_MA_NAME_TYPE_ICC)
    {
...
/* { */
    if (DNX_OAM_GROUP_MAID_IS_EXTRA_DATA(group_info->flags) && DNX_OAMP_IS_V1(unit))
    {
...
/* } */
    ...
    if (DNX_OAM_GROUP_MAID_IS_EXTRA_DATA(group_info->flags) &&
        (group_info->group_name_index != 0) && (group_info->group_name_index != -1) && DNX_OAMP_IS_V1(unit))
    {
        /** Valid group_name_index value */
        SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mep_db_48b_maid_set(unit, group_info, calculated_crc));
    }
    else
    {
    }
...
/* } */
    ...
    {
        /*
         * Flags that are only on Jer2. 20, 40 byte group names are not available 
         */
        /*
         * RX name not available since group can be compared against CRC in Jer2, unlike Jer3. 
         */
    ...
    if (group_info->flags & BCM_OAM_GROUP_WITH_ID)
    {
        ...
        if (_SHR_IS_FLAG_SET(group_info->flags, BCM_OAM_GROUP_REPLACE))
        {
            ...
            if (DNX_OAMP_IS_V1(unit))
            {
                ...
                if (existing_group_info->group_name_index == 0 && DNX_OAMP_IS_V1(unit))
                {
    ...
    bcm_oam_group_info_t * group_info)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_group_create_verify(unit, group_info));
...
/* { */
    if ((group_info->group_name_index != 0) && (group_info->group_name_index != -1) && DNX_OAMP_IS_V1(unit))
    {
        /** Valid group_name_index value */
        SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mep_db_48b_maid_clear(unit, group_info->group_name_index));
    }
/* } */
    /** verify group info is allocated/exsits */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_group_verify_is_allocated(unit, group));
    ...
    SHR_IF_ERR_EXIT(algo_oam_db.oam_group_id.free_single(unit, group, NULL));
