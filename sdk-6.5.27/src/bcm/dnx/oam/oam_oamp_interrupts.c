        ...
        {
            {
                ...
                else if (report_data.event_size == 5)
                {
                    ...
                    else if (sw_endpoint_info.sw_state_flags &
                             DBAL_DEFINE_OAM_ENDPOINT_SW_STATE_FLAGS_DELAY_ONE_WAY_STATS)
                    {
                        /*
...
shr_error_e
dnx_oam_oamp_stat_event_data_get_through_signal(
    int unit,
    dnx_oam_oamp_interrupt_jr2_report_data_t * event)
{
    signal_output_t *event_data = NULL;
    uint32 mep_id = 0;
    uint32 event_size = 0;
    uint32 mep_event_type = 0;
    uint32 frame_delay_msbs = 0;
    int dword_index, bit_index;
    uint32 data_dword;

    SHR_FUNC_INIT_VARS(unit);

    if (sand_signal_output_find(unit, 0, 0, "", "", "", "stat_event_data", &event_data) == _SHR_E_NONE)
    {
        event->is_valid = 1;
        utilex_bitstream_get_field(event_data->value, 240, 16, &mep_id);
        event->mep_id = (uint16) mep_id;
        utilex_bitstream_get_field(event_data->value, 256, 3, &event_size);
        event->event_size = (uint8) event_size;
        utilex_bitstream_get_field(event_data->value, 259, 1, &mep_event_type);
        event->mep_event_type = (uint8) mep_event_type;
        utilex_bitstream_get_field(event_data->value, 260, 16, &frame_delay_msbs);
        event->frame_delay_msbs = frame_delay_msbs;
        bit_index = 208;
        for (dword_index = 0; dword_index < 7; dword_index++)
        {
            utilex_bitstream_get_field(event_data->value, bit_index, 32, &data_dword);
            event->data[dword_index] = data_dword;
            bit_index -= 32;
        }
        utilex_bitstream_get_field(event_data->value, 0, 16, &data_dword);
        event->data[7] = data_dword;
    }

    SHR_EXIT();
exit:
    sand_signal_output_free(event_data);
    SHR_FUNC_EXIT;
}
...
#ifdef ADAPTER_SERVER_MODE
    SHR_IF_ERR_EXIT(dnx_oam_oamp_stat_event_data_get_through_signal(unit, event));
    *event_type = DNX_OAM_OAMP_REPORT_TYPE;
    SHR_EXIT();
#endif
    /*
     * Use DBAL interface to read interrupt message data and update the interrupt_data structure
    ...
    void *host_entry)
{
    ...
    {
        /*
         * Adapter workaround: the data is not written to the FIFO,
         * so the "interrupt" is triggered without any data.  The
         * data is later read directly from Adapter signals.
         */
        additional_data_to_read = 1;
    }
#endif
