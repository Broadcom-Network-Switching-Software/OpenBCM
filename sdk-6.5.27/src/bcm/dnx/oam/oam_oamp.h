...
/** ETH OAM CCM sent by Unicast DMAC Enable
 *  When set, CCM ETH Unicast DMAC is enabled on this endpoint
 */
#define DNX_OAMP_OAM_CCM_MEP_ETH_UC_DA      SAL_BIT(23)
...
#define BCM_OAMP_MEP_DB_SCAN_RATE_TO_SW_ENTRY_PERIOD(unit,scan_rate) \
    (dnx_data_oam.oamp.feature_get(unit, dnx_data_oam_oamp_oamp_v2) ? \
        UTILEX_DIV_ROUND((scan_rate * OAMP_SCAN_INTERVAL_uS(unit)), 1000) \
...
typedef struct
{
    ...
    /** ETH OAM CCM sent by Unicast DMAC Enable
     *  The 24 lst bits of the destination MAC address
     */
