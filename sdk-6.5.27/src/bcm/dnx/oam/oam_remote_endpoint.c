    ...
    bcm_oam_endpoint_info_t * endpoint_info)
{
    ...
    if ((endpoint_info->ccm_period * 1000) <= 6668 && !DNX_OAMP_IS_V1(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error: detection period must be higher then 6.67ms.\n");
    }
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        if (dnx_data_oam.oamp.rmep_db_non_zero_lifetime_units_limitation_get(unit))
        {
            ...
            dnx_oam_oamp_v1_period_to_lifetime(unit, (endpoint_info->ccm_period * 1000), &lifetime, &lifetime_units);
     ...
     */
    name_max = (1 << dnx_data_oam.oamp.mep_id_in_rx_pkt_nof_bits_get(unit)) - 1;
    if (endpoint_info->name > name_max)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error: name %d not valid, value must be in range of 0 to %d.\n",
                     endpoint_info->name, name_max);
    }
    ...
    dnx_oam_rmep_punt_profile_temp_data_t * hw_write_data)
{
    ...
    if (is_update)
    {
        SHR_IF_ERR_EXIT(algo_oam_db.oamp_punt_event_prof_template.exchange
                        (unit, 0, 1, &punt_profile_data, old_profile_id, NULL, &profile_id, &write_hw, &is_last));
