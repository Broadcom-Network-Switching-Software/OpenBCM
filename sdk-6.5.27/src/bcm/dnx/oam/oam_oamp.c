    ...
    dnx_oam_oamp_rmep_db_entry_t * new_entry_values)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        dnx_oam_oamp_v1_period_to_lifetime(unit, new_entry_values->period, &new_lifetime, &new_lifetime_units);
    }
    else
    {
    }
    ...
    dnx_oam_oamp_rmep_db_entry_t * rmep_entry)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        if (rmep_table == DBAL_TABLE_OAMP_RMEP_DB)
        {
            SHR_IF_ERR_EXIT(dbal_entry_attribute_set(unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, 0));
        }
    }
    else
    {
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        dnx_oam_oamp_v1_period_to_lifetime(unit, rmep_entry->period, &lifetime, &lifetime_units);
    }
    else
    {
    }
    ...
    dnx_oam_oamp_rmep_db_entry_t * rmep_entry)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        dnx_oam_oamp_v1_lifetime_to_period(unit, lifetime, lifetime_units, &rmep_entry->period);
    }
    else
    {
    }
    ...
    if (is_update && old_profile_id != 0)
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(algo_oamp_db.oamp_punt_good_profile.exchange
                            (unit, 0, 1, &hw_number_of_scans, old_profile_id, NULL, &profile_id, &write_hw, &is_last));
        }
    ...
    dnx_oam_oamp_ccm_endpoint_t * entry_values)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        if (entry_values->mep_type == DBAL_ENUM_FVAL_OAMP_MEP_TYPE_ETH_OAM)
        {
            ...
            if (_SHR_IS_FLAG_SET(entry_values->flags, DNX_OAMP_OAM_CCM_MEP_ETH_UC_DA))
            {
                UTILEX_SET_BIT(oamp_params->flags, DBAL_ENUM_FVAL_DA_UC_MC_TYPE_UC, OAMP_MEP_MC_UC_SEL);
            }
        }
        ...
        if (entry_values->mep_type == DBAL_ENUM_FVAL_OAMP_MEP_TYPE_ETH_OAM)
        {
            oamp_params->exclusive.oam_only.oam_lmm_da_profile = entry_values->da_lsb_profile;
        }
    ...
    oam_oamp_profile_hw_data_t * mep_hw_profiles_write_data)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        if (mep_hw_profiles_write_data->flags & DNX_OAM_OAMP_PROFILE_DA_MAC_MSB_FLAG)
        {
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mac_da_msb_profile_set(unit,
                                                                   mep_hw_profiles_write_data->
                                                                   mep_profile.oamp_mac_da_oui_prof,
                                                                   mep_hw_profiles_write_data->da_mac));
        }

        if (mep_hw_profiles_write_data->flags & DNX_OAM_OAMP_PROFILE_DA_MAC_LSB_FLAG)
        {
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mac_da_lsb_profile_set(unit,
                                                                   mep_hw_profiles_write_data->da_mac_lsb_profile,
                                                                   mep_hw_profiles_write_data->da_mac));
        }
    }
