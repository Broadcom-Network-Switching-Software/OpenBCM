...
/** shift on top of normal prefix for trunk member */
#define PREFIX_TRUNK_MEMBER_SHIFT 3
#define PREFIX_OAM_PRIMARY_VLAN_SHIFT 3
...
/** Flag that indicates that EP is a down-MEP with a valid in-LIF (not egress-only MPLS) */
#define USE_GLOB_IN_LIF(endpoint_info, lif_resolution) \
...
#define DNX_OAMP_IS_V2(unit) (dnx_data_oam.oamp.feature_get(unit, dnx_data_oam_oamp_oamp_v2) == 1)
/* Indicate if OAMP v1 is supported. */
#define DNX_OAMP_IS_V1(unit) (dnx_data_oam.oamp.feature_get(unit, dnx_data_oam_oamp_oamp_v2) == 0)
...
#define DNX_OAM_PROFILE_SET(profile_idx, profile_type) \
            (((uint32)(profile_idx & DNX_OAM_PROFILE_IDX_MASK) << DNX_OAM_PROFILE_IDX_SHIFT) | \
            ((uint32)(profile_type & DNX_OAM_PROFILE_TYPE_MASK) << DNX_OAM_PROFILE_TYPE_SHIFT) | \
            ((uint32)1 << DNX_OAM_PROFILE_VALID_SHIFT))
...
/* Check if none action is set */
#define DNX_OAM_ACTION_IS_NONE(flags) \
    (_SHR_IS_FLAG_SET((flags), BCM_OAM_ACTION_NONE))
...
/* For MEP on trunk member port, OAM LIF is local port */
#define DNX_OAM_LIF_IS_LOCAL_PORT(oam_lif) BCM_GPORT_IS_LOCAL(oam_lif)

/* OAM LIF is oam primary vlan port */
#define DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(oam_lif) \
    (BCM_GPORT_IS_VLAN_PORT(oam_lif) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_OAM_PRIMARY_VLAN(oam_lif))

/* OAM LIF is vlan port ingress only */
#define DNX_OAM_LIF_IS_VLAN_PORT_INGRESS(oam_lif) \
    (BCM_GPORT_IS_VLAN_PORT(oam_lif) && \
    (BCM_GPORT_SUB_TYPE_LIF_EXC_GET(BCM_GPORT_VLAN_PORT_ID_GET(oam_lif)) == BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY))

/* Is endpoint_info struct represents an accelerated CCM endpoint */
#define DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED_CCM(p_endpoint_info) \
...
/* Is endpoint_info struct represents a LMM endpoint */
#define DNX_OAM_DISSECT_IS_LMM(p_endpoint_info) \
                          (((p_endpoint_info)->opcode_flags & BCM_OAM_OPCODE_LMM_IN_HW) == BCM_OAM_OPCODE_LMM_IN_HW)

/* Is endpoint_info struct represents a DMM  endpoint */
#define DNX_OAM_DISSECT_IS_DMM(p_endpoint_info) \
                          (((p_endpoint_info)->opcode_flags & BCM_OAM_OPCODE_DMM_IN_HW) == BCM_OAM_OPCODE_DMM_IN_HW)

/* Does endpoint_info struct represent a 1DM  endpoint */
#define DNX_OAM_DISSECT_IS_1DM(p_endpoint_info) \
                          (((p_endpoint_info)->opcode_flags & BCM_OAM_OPCODE_1DM_IN_HW) == BCM_OAM_OPCODE_1DM_IN_HW)

/* Is endpoint_info struct represents a SLM endpoint */
#define DNX_OAM_DISSECT_IS_SLM(p_endpoint_info) \
                          (((p_endpoint_info)->opcode_flags & BCM_OAM_OPCODE_SLM_IN_HW) == BCM_OAM_OPCODE_SLM_IN_HW)

/* Is endpoint_info struct represents a dual ended endpoint */
#define DNX_OAM_DISSECT_IS_DUAL_ENDED(p_endpoint_info) \
                          (((p_endpoint_info)->opcode_flags & BCM_OAM_OPCODE_DUAL_ENDED_LM) == BCM_OAM_OPCODE_DUAL_ENDED_LM)

/* Is DM/LM endpoint used w/wo priority */
#define DNX_OAM_DISSECT_IS_WITH_PRIORITY(p_endpoint_info) \
                          (((p_endpoint_info)->opcode_flags & BCM_OAM_OPCODE_WITH_PRIORITY) == BCM_OAM_OPCODE_WITH_PRIORITY)

/* Does endpoint_info struct represent a DMM endpoint with statistics? */
#define DNX_OAM_DISSECT_IS_DMM_ONE_WAY_STATS(p_endpoint_info) \
                          (((p_endpoint_info)->opcode_flags & BCM_OAM_OPCODE_DMM_ONE_WAY_STATS) == BCM_OAM_OPCODE_DMM_ONE_WAY_STATS)

/* Accelerated endpoint types for OAMP V1 */
#define DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED_V1(p_endpoint_info) DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED_CCM(p_endpoint_info)

/* Accelerated endpoint types for OAMP V2 */
#define DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED_V2(p_endpoint_info) \
   (DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED_CCM(p_endpoint_info) || \
    DNX_OAM_DISSECT_IS_LMM(p_endpoint_info) || \
    DNX_OAM_DISSECT_IS_DMM(p_endpoint_info) || \
    DNX_OAM_DISSECT_IS_SLM(p_endpoint_info) || \
    DNX_OAM_DISSECT_IS_1DM(p_endpoint_info) || \
    DNX_OAM_DISSECT_IS_DUAL_ENDED(p_endpoint_info) || \
    DNX_OAM_DISSECT_IS_DMM_ONE_WAY_STATS(p_endpoint_info))

/* Is endpoint accelerated? */
#define DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED(p_endpoint_info) \
   (DNX_OAMP_IS_V2(unit) ? \
    DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED_V2(p_endpoint_info) : \
    DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED_V1(p_endpoint_info))
...
/* Is endpoint_info struct represents an down mep egress injection with deleting one VLAN tag */
#define DNX_OAM_DISSECT_IS_DOWN_MEP_EGRESS_INJECTION_IVEC_DELETE_ONE_VLAN_TAG(p_endpoint_info) \
                          (((p_endpoint_info)->flags2 & BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_ONE_VLAN_TAG) == BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_ONE_VLAN_TAG)

/* Is endpoint_info struct represents an down mep egress injection with deleting two VLAN tags */
#define DNX_OAM_DISSECT_IS_DOWN_MEP_EGRESS_INJECTION_IVEC_DELETE_TWO_VLAN_TAGS(p_endpoint_info) \
                          (((p_endpoint_info)->flags2 & BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_TWO_VLAN_TAGS) == BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_TWO_VLAN_TAGS)
...
#define DNX_OAM_CLIENT_MEP_INDEX_BIT_0 21
#define DNX_OAM_CLIENT_MEP_INDEX_BIT_1 23
#define DNX_OAM_CLIENT_MEP_INDEX_BIT_2 24
#define DNX_OAM_CLIENT_MEP_INDEX_BIT_3 26
#define DNX_OAM_CLIENT_MEP_INDEX_BIT_4 31

/* Get server device index from endpoint id */
#define DNX_OAM_CLIENT_MEP_SERVER_DEVICE_GET(endpoint)  \
    ((((endpoint) >> DNX_OAM_CLIENT_MEP_INDEX_BIT_0) & 0x1) |  \
     ((((endpoint) >> DNX_OAM_CLIENT_MEP_INDEX_BIT_1) & 0x1) << 1) |  \
     ((((endpoint) >> DNX_OAM_CLIENT_MEP_INDEX_BIT_2) & 0x1) << 2) |  \
     ((((endpoint) >> DNX_OAM_CLIENT_MEP_INDEX_BIT_3) & 0x1) << 3) |  \
     ((((endpoint) >> DNX_OAM_CLIENT_MEP_INDEX_BIT_4) & 0x1) << 4))

/* Encode server device index into endpoint id */
#define DNX_OAM_CLIENT_MEP_SERVER_DEVICE_SET(endpoint, server_device_index)  \
    ((((server_device_index) & 0x1) << DNX_OAM_CLIENT_MEP_INDEX_BIT_0) |  \
     ((((server_device_index) >> 1) & 0x1) << DNX_OAM_CLIENT_MEP_INDEX_BIT_1) |  \
     ((((server_device_index) >> 2) & 0x1) << DNX_OAM_CLIENT_MEP_INDEX_BIT_2) |  \
     ((((server_device_index) >> 3) & 0x1) << DNX_OAM_CLIENT_MEP_INDEX_BIT_3) |  \
     ((((server_device_index) >> 4) & 0x1) << DNX_OAM_CLIENT_MEP_INDEX_BIT_4) |  \
     (endpoint))

/* Clear server device index from endpoint id */
#define DNX_OAM_CLIENT_MEP_SERVER_DEVICE_CLR(endpoint)  \
    ((~(1 << DNX_OAM_CLIENT_MEP_INDEX_BIT_0)) &  \
     (~(1 << DNX_OAM_CLIENT_MEP_INDEX_BIT_1)) &  \
     (~(1 << DNX_OAM_CLIENT_MEP_INDEX_BIT_2)) &  \
     (~(1 << DNX_OAM_CLIENT_MEP_INDEX_BIT_3)) &  \
     (~(1U << DNX_OAM_CLIENT_MEP_INDEX_BIT_4)) & \
     (endpoint))
...
extern bcm_oam_event_cb _g_oam_event_cb[BCM_MAX_NUM_UNITS][bcmOAMEventCount];

/* Callbacks are not supported by warmboot. Need to re-register after warmboot */
extern bcm_oam_performance_event_cb _g_oam_performance_event_cb[BCM_MAX_NUM_UNITS][bcmOAMEventCount];

/*User Data*/
extern void *_g_oam_event_ud[BCM_MAX_NUM_UNITS][bcmOAMEventCount];
...
typedef struct
{
    ...
    */
     ...
     *  (OAMA)
     *
     *  oama_counter_disable_access_0 is used as the only counter disable
     *  indication on devices without hlm support
     */
...
typedef struct
{
    ...
    *  OPCODE_FLAGS, LM_FLAGS) */
    ...
    uint8 tx_signal;
    /** Duplicated MEP ID for ccm piggy-back with priority */
    uint16 duplicated_mep_id;
    /** OAM server device index */
    uint8 server_device_index;
...
typedef struct
{
    ...
    uint16 oam_id;
    /*
     * Used in OAMP_v2 only, UP/DOWN mep indication 
     */
    uint32 type;
 ...
 *   * None
 */
shr_error_e dnx_oam_mpls_tp_bfd_ach_header_form(
    int unit,
    uint32 *channel,
    uint32 *ach_header);
 ...
 *   * None
 */
shr_error_e dnx_oam_mpls_tp_bfd_channel_value_get_from_ach_header(
    int unit,
    uint32 *ach_header,
    uint32 *channel);
 ...
 *        is marked as "valid" and LSBs Range Min/Max. The data is read from the dbal
 ...
 * \param [out] my_cfm_mac_lsb_min - MAC address 8 LSBs Range Min Value
 * \param [out] my_cfm_mac_lsb_max - MAC address 8 LSBs Range Max Value
 *
 * \return
 *   shr_error_e
 *
...
/**
 * \brief - This function updates a MAC address to the my_cfm_mac
 *        setting of a port. If the port has none, any adress
 *        is accepted. If the port has one or more, the MSB of
 *        the MAC must match. New LSB range is set.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] flags - currently only one in use,
 *          DMX_OAM_CLASSIFIER_INGRESS: ingress DB lookup
 ...
 *        _SHR_CORE_ALL (to access both tables.)
 * \param [in] port_id - number of configured port
 * \param [in] my_cfm_mac - MAC address to update.
 * \param [in] my_cfm_mac_lsb_min - MAC address 8 LSBs Range Min Value.
 * \param [in] my_cfm_mac_lsb_max - MAC address 8 LSBs Range Max Value.
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_oam_my_cfm_mac_lsb_range_add(
    int unit,
    uint32 flags,
    int core_id,
    uint32 port_id,
...
/**
 * \brief - This function clear my_mac_cfm from a port's.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] flags - currently only one in use,
 *          DMX_OAM_CLASSIFIER_INGRESS: ingress DB lookup
 ...
 *   * None
 */
shr_error_e dnx_oam_my_cfm_mac_range_clear(
    int unit,
    uint32 flags,
    int core_id,
    uint32 port_id);
 ...
 *
 */
shr_error_e dnx_oam_channel_type_rx_set_entry(
    int unit,
 ...
 *
 */
shr_error_e dnx_oam_channel_type_table_get_number_of_entries(
    int unit,
 ...
 * \param [in] is_oam - pointer to flag indicated if its oam.
 * \return
 *   void
 */
 ...
 *   * None
 */
shr_error_e dnx_oam_oamp_v1_endpoint_workaround_fill_id(
    int unit,
    bcm_oam_endpoint_info_t * endpoint_info,
...
/**
* \brief
*  Utility callback function to add entries indicated
*  by duplicated mep id in remote index db
*
* \param [in] unit  - Relevant unit.
...
*   * None
*/
shr_error_e dnx_oam_lmep_rmep_index_db_add_traverse_callback(
    int unit,
    bcm_oam_endpoint_t local_endpoint,
    bcm_oam_endpoint_t remote_endpoint);
...
/**
* \brief
*  Utility callback function to remove entries indicated
*  by duplicated mep id in remote index db
*
* \param [in] unit  - Relevant unit.
* \param [in] local_endpoint  - local endpoint
...
*   * None
*/
shr_error_e dnx_oam_lmep_rmep_index_db_remove_traverse_callback(
    int unit,
    bcm_oam_endpoint_t local_endpoint,
    bcm_oam_endpoint_t remote_endpoint);
 ...
 *
 */
shr_error_e dnx_oam_local_endpoint_fill_id(
    int unit,
    bcm_oam_endpoint_info_t * endpoint_info,
    const oam_lif_resolution_t * lif_resolution);
 ...
 *   * None
 */
shr_error_e dnx_oam_trap_tcam_entry_clear(
    int unit,
    const oam_oamp_profile_hw_data_t * mep_hw_profiles_data);

shr_error_e dnx_oam_sw_state_oam_lif_info_delete(
    int unit,
    uint32 oam_lif,
    uint8 is_ingress);

/**
 * \brief - Get current report mode
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [out] report_mode - Value: Will be
 *            bcmOamReportModeTypeNormal or
 *            bcmOamReportModeTypeCompact or 
 *            bcmOamReportModeTypeRaw
 *
 * \return
 *    shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_oam_report_mode_get(
    int unit,
    uint32 *report_mode);

/**
 * \brief
 *   The function adds the OAM secondary endpoint to relevant
 *   primary OAM endpoint linked list.  OAMP v2 allows multiple
 *   endpoints with different opcodes (CCM, LM, DM) on the same
 *   LIF, level and direction, so after the first (primary)
 *   endpoint is created, the rest (secondaries) are added to a
 *   linked list.
 * \param [in] unit  -
 *   Relevant unit.
 * \param [in] primary_endpoint_id -
 *  ID of the primary OAM endpoint
 * \param [in] secondary_endpoint_id -
 *  Id of secondary endpoint to add to linked list
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected.
 * \remark
 ...
 *  * None
*/
shr_error_e dnx_oam_sw_db_pmep_to_smep_insert_smep(
    int unit,
    bcm_oam_endpoint_t primary_endpoint_id,
    bcm_oam_endpoint_t secondary_endpoint_id);

/**
 * \brief
 *   The function removes the OAM secondary endpoint from relevant
 *   OAM primary OAM endpoint list linked list.
 *   OAMP v2 allows multiple
 *   endpoints with different opcodes (CCM, LM, DM) on the same
 *   LIF, level and direction, so after the first (primary)
 *   endpoint is created, the rest (secondaries) are added to a
 *   linked list.
 ...
 *  ID of the primary OAM endpoint
 * \param [in] secondary_endpoint_id -
 *  Id of secondary endpoint to remove from linked list
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected.
 * \remark
 ...
 *  * None
*/
shr_error_e dnx_oam_sw_db_pmep_to_smep_delete_smep(
    int unit,
    bcm_oam_endpoint_t primary_endpoint_id,
    bcm_oam_endpoint_t secondary_endpoint_id);
...
/**
 * \brief
 *   The function traverses the primary endpoint linked list
 *   and calls the callback function for each secondary
 *   endpoint in the linked list.
 * \param [in] unit  -
 *   Relevant unit.
 * \param [in] primary_endpoint_id-
 *  ID of the primary endpoint
 * \param [in] cb -
 *  Callback function called for each secondary endpoint in the list.
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected.
 * \remark
 ...
 *  * None
*/
shr_error_e dnx_oam_sw_db_pmep_to_smep_traverse_list(
    int unit,
    bcm_oam_endpoint_t primary_endpoint_id,
    dnx_oam_sw_db_lmep_to_rmep_traverse_cb_f cb);
 ...
 *   * None
 */
shr_error_e dnx_oam_accelerated_endpoint_destroy_delete_acc_mep_db_entry(
    int unit,
    const bcm_oam_endpoint_info_t * endpoint_info,
    const oam_lif_resolution_t * lif_resolution,
 ...
 *   * None
 */
shr_error_e dnx_oam_handle_acceleraed_endpoint_group_config(
    int unit,
    const uint8 *group_name,
 ...
 *   * None
 */
shr_error_e dnx_oam_accelerated_endpoint_destroy_delete_profiles(
    int unit,
    const bcm_oam_endpoint_info_t * endpoint_info,
    oam_oamp_profile_hw_data_t * mep_hw_profiles_write_data);

/**
 * \brief - Helper function for oam.
 *          This function gets hw resources
 *          for a given flow lif
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] algo_gpm_flags - algo flags
 * \param [in] gport - flow type lif. May be in lif or out lif.
 * \param [out] gport_hw_resources - return hw resources
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_oam_get_flow_lif_hw_resources(
    int unit,
    int algo_gpm_flags,
    bcm_gport_t gport,
