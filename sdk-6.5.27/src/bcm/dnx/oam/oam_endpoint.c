    ...
    const bcm_oam_endpoint_info_t * endpoint_info)
{
...

    {
         ...
         * ETH OAM CCM sent by Unicast DMAC Enable,skip.
         */
        if (!((endpoint_info->type == bcmOAMEndpointTypeEthernet) &&
              ((endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW) == BCM_OAM_OPCODE_CCM_IN_HW)))
        {
    ...
    const bcm_oam_group_info_t * group_info)
{
    ...
    /*
     *Checks if period is less than zero
     */
    ...
    const bcm_oam_endpoint_info_t * endpoint_info)
{
    ...
    if (existing_endpoint_info->opcode_flags != endpoint_info->opcode_flags)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error: opcode cannot be updated.\n"
                     "Current opcode: 0x%04X.  New opcode: 0x%04X.\n",
                     existing_endpoint_info->opcode_flags, endpoint_info->opcode_flags);
    }
    ...
    oam_lif_resolution_t * lif_resolution)
{
    ...
    {
        if (!DNX_OAM_LIF_IS_LOCAL_PORT(endpoint_info->gport)
            && !DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(endpoint_info->gport)
            && !DNX_OAM_LIF_IS_VLAN_PORT_INGRESS(endpoint_info->gport))
        {
            if (endpoint_info->type == bcmOAMEndpointTypeEthernet)
            {
            ...
            if (BCM_GPORT_IS_FLOW_LIF(endpoint_info->gport))
            {
                SHR_IF_ERR_EXIT(dnx_oam_get_flow_lif_hw_resources
                                (unit, flags, endpoint_info->gport, &gport_hw_resources));
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
                                (unit, endpoint_info->gport, flags, &gport_hw_resources));
            }
        ...
        {
            lif_resolution->local_out_lif = endpoint_info->gport;
        }
        else
        {
            /*
             * This branch is specfic for OAM on trunk member port and DNX_OAM_LIF_IS_VLAN_PORT_INGRESS.
    ...
    int oam_lif)
{
    ...
    else
    {
        ...
        {
            /*
             * we have down or both_directions, because we enter twice here, once for the INGRESS part * and once for
             * the EGRESS part and we add the both_direction flag every time * 
             */
                ...
                || configured_lif->mep_dir[endpoint_info->level] == MEP_DIRECTION_IS_BOTH_DIRECTIONS)
            {
                ...
                /*
                 * when we have one mep only, it should have active and passive side 
                 */
            {
                if (is_ingress)
                {
 ...
 *        on that lif
 * \param [in] oam_lif - oam_lif of the entry to be added
 * \param [in] nof_meps - number of meps
 *
 * \retval Error indication.
 *
 */
    ...
    int nof_meps)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                /*
                 * we have down or both_directions, because we enter twice here, once for the INGRESS part * and once
                 * for the EGRESS part and we add the both_direction flag every time 
                 */
...
dnx_oam_remove_endpoint_to_lif_db_hw_wo_hlm(
    ...
    const oam_lif_configured_lif_t * configured_lif)
{
    int is_ingress, level, nof_meps;

    SHR_FUNC_INIT_VARS(unit);

    nof_meps = 0;
    for (is_ingress = 0; is_ingress < 2; is_ingress++)
    {
        for (level = 0; level <= DNX_OAM_MAX_MDL; level++)
        {
            /*
             * We have MIP and is_ingress, so we can count the MIP only once, not twice. 
             * Otherwise it is counted both on the ingress and egress.
             */
            if ((configured_lif->mdl_mp_type[is_ingress][level] == DBAL_ENUM_FVAL_MDL_MP_TYPE_ACTIVE_MEP)
                || ((configured_lif->mdl_mp_type[is_ingress][level] == DBAL_ENUM_FVAL_MDL_MP_TYPE_MIP) && (is_ingress)))
            {
                nof_meps++;
            }
        }
    ...
    if (lif_resolution->global_in_lif != DNX_OAM_INVALID_LIF)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_lif_db_remove_endpoint_wo_hlm
                        (unit, endpoint_info, DNX_OAM_CLASSIFIER_INGRESS, configured_lif,
                         lif_resolution->global_in_lif, nof_meps));
    }
    ...
    if (lif_resolution->local_out_lif != DNX_OAM_INVALID_LIF)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_lif_db_remove_endpoint_wo_hlm
                        (unit, endpoint_info, DNX_OAM_CLASSIFIER_EGRESS, configured_lif,
                         lif_resolution->local_out_lif, nof_meps));
    }
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_oam_endpoint_info_t * endpoint_info)
{
    ...
    bcm_oam_endpoint_t classifier_ep_id, temp_ep_id;
    ...
    {
        /*
         * FLOW lifs already marked as oam 
         */
        if (!(_SHR_GPORT_IS_FLOW_LIF(endpoint_info->gport)))
        {
        ...
        {
            {
                ...
                SHR_IF_ERR_EXIT(dnx_oam_acc_mep_add
                                (unit, _SHR_CORE_ALL,
                                 ...
                                 oam_lif, mdl, temp_ep_id, DNX_OAM_PROFILE_IDX_GET(endpoint_info->acc_profile_id)));
            }
        }
    }
