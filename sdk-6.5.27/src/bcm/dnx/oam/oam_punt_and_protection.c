...
#define OAMP_V1_PROTECTION_HEADERS_FIRST_PART_SIZE 0x10
#define OAMP_V2_PROTECTION_HEADERS_FIRST_PART_SIZE 0x40
    ...
    else
    {
        ...
        else if (BCM_GPORT_IS_SYSTEM_PORT(gport))
        {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_field_type_parent_field32_value_set
                            (unit, DBAL_FIELD_TYPE_DEF_DESTINATION_JR1, DBAL_FIELD_PORT_ID, &sys_port, destination));
        }
        else if (BCM_GPORT_IS_LOCAL(gport) || BCM_GPORT_IS_LOCAL_CPU(gport) || BCM_GPORT_IS_MODPORT(gport))
        {
             ...
             */
            SHR_IF_ERR_EXIT(dbal_fields_field_type_parent_field32_value_set
                            (unit, DBAL_FIELD_TYPE_DEF_DESTINATION_JR1, DBAL_FIELD_PORT_ID, &gport_info.sys_port,
                             destination));
        }
        else if (BCM_GPORT_IS_MCAST(gport))
        {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_field_type_parent_field32_value_set
                            (unit, DBAL_FIELD_TYPE_DEF_DESTINATION_JR1, DBAL_FIELD_MC_ID_JR1, &mc_id, destination));
        }
        else
        {
            /*
    ...
    switch (type)
    {
        ...
        case bcmRxTrapOampPuntNextGoodPacket:
            *error_type = DBAL_ENUM_FVAL_OAMP_ERROR_TYPE_DEBUG;
            break;
        case bcmRxTrapOampChecksumErr:
            break;
        case bcmRxTrapOampRmepEccErr:
            break;
        case bcmRxTrapOampErrorIndicationMiss:
            break;
        default:
           /**
    ...
    int trap_id_p)
{
    ...
    else
    {
        ...
        {
            /** Configure OAMP_PORT */
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_init_cpu_port_configuration(unit));
        }
        else
        {
        }
    ...
    if (type == bcmRxTrapOampErrorIndicationMiss)
    {
    }
    ...
    if (*is_oam_punt_or_protection == TRUE)
    {
        if (flags & BCM_RX_TRAP_WITH_ID)
        {
            ...
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "OAMP Error traps may not be created with ID. User-defined trap code will be allocated.");
...
{
    uint32 pp_ssp = 232, reserved = 0, prt_qual = DBAL_ENUM_FVAL_PRT_QUALIFIER_DEFAULT, ppc = JR2_PPC_DEFAULT;
    ...
    uint32 *total_header_len)
{
    ...
    uint32 itmh_ext_present = 0, tc = 0, dp = 0, snoop_cmd = 0, in_mirr_dis = 0;
    ...
    uint32 *total_header_len)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                    (unit, DBAL_FIELD_TYPE_DEF_FHEI_5B_TRAP_SNIFF, DBAL_FIELD_FHEI_TYPE, &fhei_type, fhei_header_tmp));
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                    (unit, DBAL_FIELD_TYPE_DEF_FHEI_5B_TRAP_SNIFF, DBAL_FIELD_CODE, &trap_code, fhei_header_tmp));
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                    (unit, DBAL_FIELD_TYPE_DEF_FHEI_5B_TRAP_SNIFF, DBAL_FIELD_QUALIFIER, &qualifier, fhei_header_tmp));
    ...
    uint32 *total_header_len)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                    (unit, DBAL_FIELD_TYPE_DEF_FHEI_3B_TRAP_JR1, DBAL_FIELD_CPU_TRAP_CODE_JR1, &trap_code,
                     fhei_header_dword));
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode
                    (unit, DBAL_FIELD_TYPE_DEF_FHEI_3B_TRAP_JR1, DBAL_FIELD_CPU_TRAP_CODE_QUALIFIER, &qualifier,
                     fhei_header_dword));
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    dbal_enum_value_field_oamp_error_type_e error_type;
    uint32 is_oam_protection = FALSE, is_bfd_protection = FALSE;
    uint32 udh_size = 0;
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_IS_PROTECTION, INST_SINGLE, &is_oam_protection);

    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    if (is_oam_protection == TRUE || is_bfd_protection == TRUE)
    {
        /** Protection trap */
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            uint8 oamp_v1_zeros[OAMP_V1_PROTECTION_HEADERS_FIRST_PART_SIZE] = { 0 };
            ...
            dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_PROTECTION_PACKET_HEADER_PART_2,
                                            INST_SINGLE, headers);

            dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_PROTECTION_PACKET_HEADER_PART_1,
                                            INST_SINGLE, oamp_v1_zeros);
    ...
    bcm_pkt_blk_t * packet_header)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        SHR_ALLOC_SET_ZERO(first_part, OAMP_V1_PROTECTION_HEADERS_FIRST_PART_SIZE, "Array for over 32-bit dbal fields",
                           "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    }
    else
    {
    ...
    if (packet_header->len > dnx_data_oam.oamp.protection_packet_max_header_size_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Illegal OAMP protection packet header length. Value provided is %d, maximum is %d\n",
                     packet_header->len, dnx_data_oam.oamp.protection_packet_max_header_size_get(unit));
    }
    ...
    for (index = 0; index < packet_header->len; ++index)
    {
        ...
        else
        {
            ...
            first_part[dnx_data_oam.oamp.protection_packet_max_header_size_get(unit) - index - 1] =
                packet_header->data[index];
        }
    }
