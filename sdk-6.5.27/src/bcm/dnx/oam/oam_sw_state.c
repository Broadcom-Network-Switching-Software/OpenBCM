    ...
    uint8 *found)
{
    ...
    *found = FALSE;
    ...
    while (DNX_SW_STATE_LL_IS_NODE_VALID(node))
    {
        ...
        SHR_IF_ERR_EXIT(oam_sw_db_info.oam_group_sw_db_info.
                        oam_group_array_of_linked_lists.next_node(unit, *node, node));
...
static shr_error_e
dnx_oam_sw_db_pmep_to_smep_create(
    int unit)
{
    sw_state_ll_init_info_t init_info;
    uint32 nof_meps, half_nof_mep;

    SHR_FUNC_INIT_VARS(unit);

    sal_memset(&init_info, 0, sizeof(sw_state_ll_init_info_t));
    nof_meps = dnx_data_oam.oamp.nof_mep_db_endpoint_entries_get(unit);
    half_nof_mep = nof_meps / 2;

    /*
     * Maximum case - half_nof_meps primaries, half_nof_meps secondaries
     */
    init_info.max_nof_elements = half_nof_mep;
    init_info.nof_heads = dnx_data_oam.oamp.max_value_of_mep_id_get(unit);

    SHR_IF_ERR_EXIT(oam_sw_db_info.oam_endpoint_sw_db_info.
                    oam_endpoint_array_of_mep_linked_lists.create_empty(unit, &init_info));
    ...
    int unit)
{
   ...
   /** init the SW DB of OAM EPs linked to each OAM group */
    ...
    /** Initialize the SW DB of OAM EPs with different opcodes on the same LIF, level and direction */
    SHR_IF_ERR_EXIT(dnx_oam_sw_db_pmep_to_smep_create(unit));
    ...
    int unit)
{
    ...
    nof_mep_db_endpoint_entries = dnx_data_oam.oamp.max_value_of_mep_id_get(unit);
    ...
    dnx_oam_sw_db_lmep_to_rmep_traverse_cb_f cb)
{
    ...
    while (DNX_SW_STATE_LL_IS_NODE_VALID(node))
    {
        ...
        oam_sw_db_info.oam_endpoint_sw_db_info.oam_endpoint_array_of_rmep_linked_lists.next_node(unit, node, &node);
    }
