...
#define DNX_OAM_ENDPOINT_TYPE_PER_MP_TYPE(mp_type) (                                                \
            (mp_type == DBAL_ENUM_FVAL_MP_TYPE_BELOW_MIP ||                                         \
...
/*
 * mp_type DBAL_ENUM_FVAL_MP_TYPE_BELOW_MIP and DBAL_ENUM_FVAL_MP_TYPE_BETWEEN_MIP_AND_MEP
 * can be mapped to both bcmOAMMatchTypeLowMDLMIP and bcmOAMMatchTypeLowMDL
 */
#define DNX_OAM_IS_MP_TYPE_MATCH_ENDPOINT_TYPE(mp_type, ep_type) (                                            \
            (ep_type != bcmOAMMatchTypeLowMDLMIP) ? (DNX_OAM_ENDPOINT_TYPE_PER_MP_TYPE(mp_type) == ep_type) : \
            (mp_type == DBAL_ENUM_FVAL_MP_TYPE_BELOW_MIP ||                                                   \
             mp_type == DBAL_ENUM_FVAL_MP_TYPE_BETWEEN_MIP_AND_MEP) ? TRUE : FALSE)
    ...
    uint8 is_ingress)
{
    ...
    sal_memset(&oam_lif_info, 0, sizeof(oam_lif_info));
    ...
    bcm_oam_profile_t * profile_index)
{
    ...
    legal_flags |=
        ...
        BCM_OAM_PROFILE_DM_TYPE_NONE;
    if (dnx_data_oam.general.oam_slm_dedicated_profile_supported_get(unit))
    {
        legal_flags |= BCM_OAM_PROFILE_LM_TYPE_SLM;
    }
     ...
     */
    if ((int) oam_profile_type < 0 || oam_profile_type >= bcmOAMProfileCount)
    {
    ...
    bcm_oam_profile_t * profile_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_profile_create_verify(unit, flags, oam_profile_type, profile_id));
    ...
    if (flags & BCM_OAM_PROFILE_DM_TYPE_NONE)
    {
        SHR_IF_ERR_EXIT(dnx_oam_sw_db_dm_mode_set
                        (unit, profile_index, is_ingress, is_acc, DBAL_ENUM_FVAL_DM_MODE_DM_MODE_IS_NONE,
                         profile_is_section));
    }
    else if (flags & BCM_OAM_PROFILE_DM_TYPE_NTP)
    {
        /*
    ...
    bcm_oam_profile_t profile_id)
{
    ...
    SHR_IF_ERR_EXIT(DNX_OAM_RESOURCE_CB_PER_PROFILE_TYPE(profile_type, free_single(unit, profile_index, NULL)));
    ...
    bcm_oam_profile_t * enc_profile_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_profile_id_get_by_type_verify
                           (unit, profile_id, oam_profile_type, enc_profile_id, flags));
    ...
    *flags = 0;
    ...
    {
        *flags |= BCM_OAM_PROFILE_DM_TYPE_NONE;
    }
    ...
    bcm_oam_profile_t * egress_oam_action_profile)
{
    ...
    /** Validity checks */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_profile_get_verify(unit, flags, gport, ingress_oam_action_profile,
                                                          egress_oam_action_profile));
    ...
    bcm_oam_profile_t egress_oam_action_profile)
{
    ...
    else
    {
       ...
       /** Validate profile */
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "gport are invalid\r\n");
        }
        if (ingress_oam_action_profile != BCM_OAM_PROFILE_INVALID)
        {
        ...
        {
            algo_gpm_flags |=
                (DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS | DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS);
        }
       /** Get Gport information */
        if (BCM_GPORT_IS_FLOW_LIF(gport))
        {
            SHR_IF_ERR_EXIT(dnx_oam_get_flow_lif_hw_resources(unit, algo_gpm_flags, gport, &gport_hw_resources));
        }
        else if (!DNX_OAM_LIF_IS_LOCAL_PORT(gport) && !DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(gport))
        {
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, gport, algo_gpm_flags, &gport_hw_resources));
        }
        else if (DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(gport))
        {
            gport_hw_resources.local_out_lif = gport;
            gport_hw_resources.global_in_lif = DNX_OAM_INVALID_LIF;
        }
        else
        {
            /** For OAM on trunk member port only */
            SHR_IF_ERR_EXIT(dnx_algo_port_in_lag(unit, BCM_GPORT_LOCAL_GET(gport), &is_lag));
            if (!is_lag)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Parameter error: OAM LIF can be is local port only when local port is a trunk member.\n");
            }
            else
            {
                gport_hw_resources.global_in_lif = gport;
                gport_hw_resources.local_out_lif = DNX_OAM_INVALID_LIF;
            }
        }

       /** Validity checks */
        SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_profile_set_verify(unit, flags, gport_hw_resources.global_in_lif,
       ...
       /** HW related changes */
        ...
        /** To remove entries from the OAM-LIF-DB, both SW and HW tables.*/
        if (ingress_oam_action_profile == BCM_OAM_PROFILE_INVALID
            && egress_oam_action_profile == BCM_OAM_PROFILE_INVALID && !DNX_OAM_LIF_IS_LOCAL_PORT(gport)
            && !DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(gport))
        {
            /** clear SW_STATE and Hw table for ing/eg */
            if (gport_hw_resources.global_in_lif != BCM_GPORT_INVALID)
            {
                is_ingress = 1;
                SHR_IF_ERR_EXIT(dnx_oam_sw_state_oam_lif_info_delete
                                (unit, gport_hw_resources.global_in_lif, is_ingress));
                for (prefix = PREFIX_HIGH; prefix < lif_analysis.lif_db.occupied_prefixes[DNX_OAM_CLASSIFIER_INGRESS];
                     prefix++)
                {
                    oam_lif_db_key.oam_key_prefix = prefix;
                    ...
                    oam_lif_db_key.flags = DNX_OAM_CLASSIFIER_INGRESS;
                    oam_lif_db_key.oam_lif_id = gport_hw_resources.global_in_lif;
                    SHR_IF_ERR_EXIT(dnx_oam_lif_delete(unit, &oam_lif_db_key));
                }
            }
            if (gport_hw_resources.local_out_lif != BCM_GPORT_INVALID)
            {
                is_ingress = 0;
                SHR_IF_ERR_EXIT(dnx_oam_sw_state_oam_lif_info_delete
                                (unit, gport_hw_resources.local_out_lif, is_ingress));
        ...
        /** For OAM on trunk member port only */
        else if (ingress_oam_action_profile == BCM_OAM_PROFILE_INVALID && DNX_OAM_LIF_IS_LOCAL_PORT(gport))
        {
            is_ingress = 1;
            SHR_IF_ERR_EXIT(dnx_oam_sw_state_oam_lif_info_delete(unit, gport_hw_resources.global_in_lif, is_ingress));
            for (prefix = PREFIX_HIGH; prefix < lif_analysis.lif_db.occupied_prefixes[DNX_OAM_CLASSIFIER_INGRESS];
                 prefix++)
            {
                oam_lif_db_key.oam_key_prefix = prefix;
        ...
        else if (egress_oam_action_profile == BCM_OAM_PROFILE_INVALID && (DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(gport)))
        {
            is_ingress = 0;
            SHR_IF_ERR_EXIT(dnx_oam_sw_state_oam_lif_info_delete(unit, gport_hw_resources.local_out_lif, is_ingress));
    ...
    oam_action_content_t * action_content)
{
        ...
        internal_opcode == DBAL_ENUM_FVAL_OAM_INTERNAL_OPCODE_ETH_1DM)
    {
        ...
        if (dm_type == DBAL_ENUM_FVAL_DM_MODE_DM_MODE_IS_NONE)
        {
            action_content->oam_sub_type = DBAL_ENUM_FVAL_OAM_SUB_TYPE_NULL;
        }
        else if (dm_type == DBAL_ENUM_FVAL_DM_MODE_DM_MODE_IS_NTP)
        {
    ...
    {
        {
            int reflector_trap_id, trap_id = 0;
            /*
    ...
    {
/** In OAMPv2 there is no separate sub_type for passive side and level error */
        if (DNX_OAMP_IS_V1(unit))
        {
            /** Set subtype for OAM level and passive trap */
            int oam_level_trap_id, oam_passive_trap_id, trap_id = 0;
    ...
    else
    {
        ...
        {
            /** Snooped packets should have subtype NULL */
            action_content->oam_sub_type = DBAL_ENUM_FVAL_OAM_SUB_TYPE_NULL;
        }
    }
    ...
    bcm_oam_action_result_t * oam_action_result)
{
    ...
    /** Validate flags */
    legal_flags = BCM_OAM_ACTION_NONE;
    if (flags & ~legal_flags)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Flags combination is not supported\r\n");
    }

    /** Validate destination when action none is set */
    if (DNX_OAM_ACTION_IS_NONE(flags))
    {
        if (oam_action_result->destination != BCM_GPORT_INVALID)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Destination should be invalid when action none is set.\r\n");
        }
    }
    ...
    bcm_oam_action_result_t * oam_action_result)
{
    ...
    /** Validity checks */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_profile_action_common_verify(unit, flags, profile_id, oam_action_key,
                                                                oam_action_result));
    ...
    bcm_oam_action_result_t * oam_action_result)
{
    ...
    uint8 oama_disable = 0;

    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);

    /** Validity checks */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_profile_action_get_verify(unit, flags, profile_id, oam_action_key,
                                                             oam_action_result));
    ...
    /*
     * For HLM supported devices, take the first_access result
     * For non HLM devices, take the only result
     */
    oama_disable = action_content.oama_counter_disable_access_0;
 ...
 */
static shr_error_e
dnx_oam_profile_counter_disable_calc_hlm(
    int unit,
    bcm_oam_profile_t profile_id,
 ...
 */
static shr_error_e
dnx_oam_profile_counter_disable_calc_wo_hlm(
    int unit,
    bcm_oam_profile_t profile_id,
    dbal_enum_value_field_mp_type_e mp_type,
    uint8 count_enable,
    ...
    bcm_oam_action_result_t * oam_action_result)
{
    ...
    /** Determine setting action none */
    is_action_none = DNX_OAM_ACTION_IS_NONE(flags);

    /** Validity checks */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_profile_action_set_verify(unit, is_ingress, flags, profile_id, oam_action_key,
                                                             oam_action_result));
    ...
    else
    {
        ...
        if (BCM_GPORT_IS_TRAP(oam_action_result->destination))
        {
            ...
            {
                /**Extract trap code.
                 * This may also double up as the snoop code in case snoop_strength is also >0 */
                action_content.table_specific_fields.egress_only_fields.egress_action_profile =
                    DNX_RX_TRAP_ETPP_OAM_ACTION_PROFILE_GET(BCM_GPORT_TRAP_GET_ID(oam_action_result->destination));
            }
            else
            {
                /**Forwarding strength is 0 - Extract snoop code*/
                action_content.table_specific_fields.egress_only_fields.egress_action_profile =
                    BCM_GPORT_TRAP_GET_ID(oam_action_result->destination);
            }
    ...
    if (!DNX_OAM_MP_TYPE_IS_RFC_6374(oam_action_key->flags))
    {
        ...
        {
            if (DNX_OAM_IS_MP_TYPE_MATCH_ENDPOINT_TYPE(mp_type, oam_action_key->endpoint_type))
            {
            ...
            /** Calculate counter disable values based on mp_type */
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_oam_profile_counter_disable_calc_hlm(unit, profile_id, mp_type,
                                                                             oam_action_result->counter_increment,
                                                                             is_section, is_hlm_disable,
                                                                             &action_content));
                }
                else
                {
                    SHR_IF_ERR_EXIT(dnx_oam_profile_counter_disable_calc_wo_hlm(unit, profile_id, mp_type,
                                                                                oam_action_result->counter_increment,
                                                                                &action_content));
                }

                /** Set the entry on OAM action table */
                if (!is_ingress && action_key.is_inject)
                {
                    action_content.oam_sub_type = DBAL_ENUM_FVAL_OAM_SUB_TYPE_NULL;
                }
                ...
                if ((DNX_OAM_IS_MP_TYPE_MATCH_ENDPOINT_TYPE(mp_type, bcmOAMMatchTypeMEP))
                    && (action_key.is_inject == 0))
                {
                    ...
                    /** When up mep tod feature enabled, set subtype to NULL since the packet doesn't need tod process in the 2nd pass (it's stamped in 1st pass) */
                    if (dnx_data_oam.feature.feature_get(unit, dnx_data_oam_feature_up_mep_tod_recycle_enable)
                        && is_ingress && (action_key.is_inject == 1)
                        && (action_content.oam_sub_type == DBAL_ENUM_FVAL_OAM_SUB_TYPE_DELAY_MEASUREMENT_1588))
                    {
                        action_content.oam_sub_type = DBAL_ENUM_FVAL_OAM_SUB_TYPE_NULL;
                    }
