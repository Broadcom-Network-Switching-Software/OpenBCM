 ...
 */
shr_error_e
dnx_oam_get_flow_lif_hw_resources(
    int unit,
    int algo_gpm_flags,
    bcm_gport_t gport,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    bcm_gport_t temp_gport = gport;
    bcm_gport_t temp_lif;
    uint32 flags = 0;
    ...
    {
        /*
         * For FLOW lifs the gport represent either symetric in-lif or symetric out-lif. For out-lif we first get
         * egress lifs and then use temp_lif to get the associated in-lif by setting the lif-sub-type but keeping the
         * lif. Then we get the ingress lifs using the temp_lif. For in-lifs we do the other way around to get the
         * out-lif. That way we can support both symetric-in-lif and symetric out-lif as gport 
         */
        if (BCM_GPORT_SUB_TYPE_IS_OUT_LIF(gport))
        {
            if (algo_gpm_flags == DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_AND_GLOBAL_LIF)
            {
                flags =
            ...
            {
                flags = algo_gpm_flags;
            }
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, gport, flags, gport_hw_resources));
            temp_lif = gport_hw_resources->local_out_lif;

            temp_gport =
                (gport & ((-1) ^ (_SHR_GPORT_SUB_TYPE_OUT_LIF << _SHR_GPORT_SUB_TYPE_SHIFT))) |
                (_SHR_GPORT_SUB_TYPE_IN_LIF << _SHR_GPORT_SUB_TYPE_SHIFT);
            ...
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, temp_gport, flags, gport_hw_resources));
            gport_hw_resources->local_out_lif = temp_lif;
        }
        else
        {
            if (algo_gpm_flags == DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_AND_GLOBAL_LIF)
            ...
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, gport, flags, gport_hw_resources));
            temp_lif = gport_hw_resources->global_in_lif;

            temp_gport =
                (gport & ((-1) ^ (_SHR_GPORT_SUB_TYPE_IN_LIF << _SHR_GPORT_SUB_TYPE_SHIFT))) |
                (_SHR_GPORT_SUB_TYPE_OUT_LIF << _SHR_GPORT_SUB_TYPE_SHIFT);
            ...
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, temp_gport, flags, gport_hw_resources));
            gport_hw_resources->global_in_lif = temp_lif;
        }
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error: 0x%x is not a FLOW lif \n", gport);
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    dnx_oam_lif_entry_key_t * oam_lif_entry_key_out)
{
    ...
    /** Convertion specific for OAM primary VLAN port. */
    if (DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(oam_lif_entry_key_out->oam_lif_id))
    {
        /** Prefix shifted */
        oam_lif_entry_key_out->oam_key_prefix += PREFIX_OAM_PRIMARY_VLAN_SHIFT;
        oam_lif_entry_key_out->oam_lif_id = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_OAM_PRIMARY_VLAN_GET
            (BCM_GPORT_VLAN_PORT_ID_GET(oam_lif_entry_key_out->oam_lif_id));
    }

    /** Convertion specific for OAM primary VLAN port. */
    if (DNX_OAM_LIF_IS_VLAN_PORT_INGRESS(oam_lif_entry_key_out->oam_lif_id))
    {
        /** Prefix shifted */
        oam_lif_entry_key_out->oam_key_prefix += PREFIX_OAM_PRIMARY_VLAN_SHIFT;
        oam_lif_entry_key_out->oam_lif_id = BCM_GPORT_SUB_TYPE_LIF_VAL_GET
            (BCM_GPORT_VLAN_PORT_ID_GET(oam_lif_entry_key_out->oam_lif_id));
    }
    ...
    uint32 *acc_mp_profile)
{
    ...
    else if (DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(oam_lif))
    {
        oam_lif_tmp = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_OAM_PRIMARY_VLAN_GET(BCM_GPORT_VLAN_PORT_ID_GET(oam_lif));
    }
    else if (DNX_OAM_LIF_IS_VLAN_PORT_INGRESS(oam_lif))
    {
        oam_lif_tmp = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(BCM_GPORT_VLAN_PORT_ID_GET(oam_lif));
    }
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_OAM_CLASSIFIER_BFD) || DNX_OAM_LIF_IS_LOCAL_PORT(oam_lif)
        || DNX_OAM_LIF_IS_OAM_PRIMARY_VLAN_PORT(oam_lif))
    {
    ...
    const oam_action_content_t * action_content)
{
    ...
    else
    {
        if (action_content->table_specific_fields.egress_only_fields.egress_action_profile >
            SAL_UPTO_BIT(dnx_data_oam.general.oam_trap_action_profile_nof_bits_get(unit)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Illegal trap_code value %d. Max allowed value: %d\n",
