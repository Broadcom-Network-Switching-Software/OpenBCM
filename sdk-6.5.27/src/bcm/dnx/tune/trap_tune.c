    ...
    int unit)
{
    ...
    /** Initialize trap for an ITMH error (unsupported PPH extension) */
    SHR_IF_ERR_EXIT(dnx_tune_trap_ingress_configure_init
                    (unit, BCM_RX_TRAP_UPDATE_DEST, bcmRxTrapItmhError, 0, BCM_GPORT_BLACK_HOLE, 0, 0));

    /** Fwd Action profile used if Fwd-Domain(PD-1) lookup wasn't found */
    if (dnx_data_trap.ingress.feature_get(unit, dnx_data_trap_ingress_fwd_domain_mode_trap))
    {
        SHR_IF_ERR_EXIT(dnx_tune_trap_ingress_configure_init
                        (unit, BCM_RX_TRAP_UPDATE_DEST, bcmRxTrapFwdDomainAssignmentModeTrap,
                         dnx_data_trap.strength.default_trap_strength_get(unit), BCM_GPORT_BLACK_HOLE, 0, 0));
    }
