    ...
    bcm_cosq_delay_tolerance_t * delay_tolerance)
{
    ...
    flag_mask = BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_PUSH_QUEUE | BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_DELETE;
    if (!(preset_attr->flags & flag_mask))
    {
        /** bandwidth level */
        ...
        delay_tolerance->credit_request_satisfied_backoff_enter_thresh =
            slow_enable ? ((128 - 64) * credit_resolution) *
            1024 : delay_tolerance->credit_request_satisfied_empty_queue_max_balance_thresh +
            (2 * preset_attr->credit_size);
                                                                                                                /** bytes */
        ...
        delay_tolerance->credit_request_hungry_off_to_normal_thresh =
            slow_enable ? (2 * (max_inflight_credits + 2) * preset_attr->credit_size) : (-1 *
                                                                                         delay_tolerance->
    ...
    /** credit watchdog delete threshold. watchdog is disabled for push-queue */
    flag_mask = BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_PUSH_QUEUE;
    if (!(preset_attr->flags & flag_mask))
    {
