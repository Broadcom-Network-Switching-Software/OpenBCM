    ...
    bcm_vlan_control_vlan_t * control)
{
    ...
    if (group_id > dnx_data_multicast.params.max_ing_mc_groups_get(unit) || group_id < 0)
    {
        /**
         * Reset VSI(0) unknown dest to default value(mc_id is black hole gport) in black hole supported devices.
         */
        if (vlan != BCM_VLAN_NONE || no_black_hole_for_unknown_destination)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Ingress multicast ID (%d) is not at the valid range <0-%u>\n",
    ...
    if (_SHR_IS_FLAG_SET(control->flags2, BCM_VLAN_FLAGS2_UNKNOWN_DEST))
    {
        ...
        if (no_black_hole_for_unknown_destination && BCM_GPORT_IS_BLACK_HOLE(control->unknown_dest))
        {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Can't update VSI(0) unknown destination when no_black_hole_for_unknown_destination = 1 in the device!\n");
        }
    }
    ...
    {
        /**
         * Set VSI(0) unknown dest in black host support devices.
         */
        if (vlan != BCM_VLAN_NONE || no_black_hole_for_unknown_destination)
        {
           ...
           /** Get in_lif_profile_info */
            SHR_IF_ERR_EXIT(in_lif_profile_info_t_init(unit, &in_lif_profile_info));
            SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data
                            (unit, old_vsi_profile, &in_lif_profile_info, table_id, ETH_RIF));
 ...
 */
static shr_error_e
dnx_vlan_control_port_mac_lookup_set(
    int unit,
    int port,
    uint32 value)
{
    ...
    if (value != 0)
    {
        if (llr_cs_profile == DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_SA_BASED_CLASSIFICATION_PP_PORT)
        {
            SHR_EXIT();
        }
        else if (llr_cs_profile == DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_TYPICAL_PORT)
        {
            llr_cs_profile = DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_SA_BASED_CLASSIFICATION_PP_PORT;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Can not enable the SA VLAN feature due to llr_cs_profile is %d. Reject request with error\r\n",
                         llr_cs_profile);
        }
    }
    else
    {
        ...
        else if (llr_cs_profile == DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_SA_BASED_CLASSIFICATION_PP_PORT)
        {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Can not disable the SA VLAN feature due to llr_cs_profile is %d. Reject request with error\r\n",
                         llr_cs_profile);
        }
    }
...
*/
static shr_error_e
dnx_vlan_control_port_mac_lookup_get(
    int unit,
    int port,
    int *value)
{
    ...
    bcm_vlan_control_vlan_t control)
{
    ...
    /*
     * Set the VSI profile with field bits, exchange in_lif_profile
     */
    SHR_IF_ERR_EXIT(dnx_field_port_profile_vsi_set
                    (unit, control.if_class, vlan, &in_lif_profile_info, old_vsi_profile, &new_vsi_profile,
                     &last_profile));
    ...
    else
    {
        ...
        BCM_GPORT_MCAST_SET(gport_unknown_dest, mc_id);
    }
    ...
    int *arg)
{
    ...
    switch (type)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_vlan_control_port_mac_lookup_get(unit, port, arg));
            break;
        }
    ...
    int arg)
{
    ...
    switch (type)
    {
        case bcmVlanLookupMACEnable:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_control_port_mac_lookup_set(unit, port, UTILEX_NUM2BOOL(arg)));
            break;
        }
        case bcmVlanPortIgnorePktTag:
        {
    ...
    bcm_pbmp_t list_pbmp)
{
    ...
    *listp = NULL;
    ...
    for (idx = (BCM_VLAN_MIN + 1); idx < BCM_VLAN_COUNT; idx++)
    {
        /** Make sure VSI is allocated */
        is_vsi_allocated = 0;
        rv = vlan_db.vsi.is_allocated(unit, idx, &is_vsi_allocated);
        if (BCM_FAILURE(rv))
        {
            SHR_FREE(list);
            SHR_IF_ERR_EXIT(rv);
