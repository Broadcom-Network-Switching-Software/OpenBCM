...
#define DNX_CRITERIA_IS_MAC_PORT(_criteria_) ((_criteria_ == BCM_VLAN_PORT_MATCH_PORT)|| \
                                         (_criteria_ == BCM_VLAN_PORT_MATCH_MAC_PORT)|| \
                                         (_criteria_ == BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED) || \
                                         (_criteria_ == BCM_VLAN_PORT_MATCH_MAC_PORT_VLAN))
...
/**
 * \brief
 * The vlan port protection pointer bits number in large group AC result 
 */
#define BCM_DNX_VLAN_PORT_PROTECTION_PTR_BITS_IN_LARGE_GROUP 10
...
extern int bcm_dnx_rx_trap_type_get(
    int,
    int,
    bcm_rx_trap_t,
    int *);
    ...
    uint8 *is_allocated)
{
    ...
    *is_allocated = FALSE;
    ...
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, tmp_gport, flags, &gport_hw_resources));
...
/**
 * \brief - function returns whether a given VLAN-Port gport is DPC in-lif
 */
static shr_error_e
dnx_vlan_port_is_dpc_in_lif(
    int unit,
    bcm_vlan_port_t * vlan_port,
    int *is_dpc_in_lif)
{
        ...
        if (DNX_CRITERIA_IS_PON(vlan_port->criteria))
        {
            ...
            if (is_pon_enable)
            {
                SHR_IF_ERR_EXIT(bcm_vlan_control_port_get
                                (unit, vlan_port->port, bcmVlanPortDoubleLookupEnable, &is_pon_dtc));
            }
        }
        else if (DNX_CRITERIA_IS_MAC_PORT(vlan_port->criteria))
        {
            SHR_IF_ERR_EXIT(bcm_vlan_control_port_get(unit, vlan_port->port, bcmVlanLookupMACEnable, &is_mac_port));
        }
    }
    *is_dpc_in_lif = is_native_ac || is_pon_dtc || is_mac_port;
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    switch (vlan_port->criteria)
    {
        ...
        case BCM_VLAN_PORT_MATCH_MAC_PORT:
        case BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED:
        case BCM_VLAN_PORT_MATCH_MAC_PORT_VLAN:
            if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error ! criteria =  BCM_VLAN_PORT_MATCH_MAC_PORT_*(%d) is not supported for native! (flag BCM_VLAN_PORT_NATIVE (0x%08X) is set!) flags = 0x%08X\n",
                             vlan_port->criteria, BCM_VLAN_PORT_NATIVE, vlan_port->flags);
            }
        ...
        default:
            ...
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         ...
                         "BCM_VLAN_PORT_MATCH_PORT_TUNNEL_UNTAGGED(%d), BCM_VLAN_PORT_MATCH_MAC_PORT(%d), "
                         "BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED(%d), BCM_VLAN_PORT_MATCH_MAC_PORT_VLAN(%d) are supported\n",
                         ...
                         BCM_VLAN_PORT_MATCH_PORT_TUNNEL_UNTAGGED, BCM_VLAN_PORT_MATCH_MAC_PORT,
                         BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED, BCM_VLAN_PORT_MATCH_MAC_PORT_VLAN);
            break;
    }
    ...
    switch (vlan_port->criteria)
    {
        ...
        case BCM_VLAN_PORT_MATCH_NAMESPACE_VSI:
        {
            ...
            int nof_vsis;
            dbal_tables_e table_id = DBAL_TABLE_ESEM_FORWARD_DOMAIN_MAPPING_DB;
            dbal_fields_e field_id = DBAL_FIELD_VLAN_DOMAIN;

            SHR_IF_ERR_EXIT(dbal_tables_field_max_value_get
                            (unit, table_id, field_id, TRUE, 0, 0, (int *) &network_domain_max_val));
    ...
    int is_virtual_ac)
{
    ...
    supported_flags =
        ...
        | BCM_VLAN_PORT_CREATE_EGRESS_ONLY | BCM_VLAN_PORT_VSI_BASE_VID |
    ...
    {
        supported_flags |= BCM_VLAN_PORT_EGRESS_PROTECTION;
    }
    ...
    /** Verify only supported flags2 are set */
    supported_flags2 =
        BCM_VLAN_PORT_FLAGS2_OAM_TRUNK_MEMBER | BCM_VLAN_PORT_FLAGS2_MAC_SAV | BCM_VLAN_PORT_FLAGS2_OAM_PRIMARY_VLAN;
    if (vlan_port->flags2 & (~supported_flags2))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! Unsupported flag: flags2 = 0x%08x. Supported flags are: 0x%08x\n",
                     vlan_port->flags2, supported_flags2);
    }
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_ALLOC_SYMMETRIC))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "BCM_VLAN_PORT_ALLOC_SYMMETRIC and BCM_VLAN_PORT_CREATE_INGRESS_ONLY cannot be used together");
        }
    ...
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
    {
        ...
        else
        {
            ...
            uint8 is_virtual_trunk = 0;
            bcm_trunk_t master_tid;

            /**allow for virtual trunk without pp_port*/
            SHR_IF_ERR_EXIT(dnx_trunk_sw_db_group_gport_is_virtual_trunk
                            (unit, vlan_port->port, &is_virtual_trunk, &master_tid));
            if (!is_virtual_trunk)
            {
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
                                (unit, vlan_port->port, DNX_ALGO_GPM_GPORT_TO_PHY_OP_PP_PORT_IS_MANDATORY,
                                 &gport_info));
            }
        }
    }
    ...
    if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
    {
        ...
        if (vlan_port->group == RING_PROTECTION_MACT_GROUP_IN_JR1_MODE)
        {
            ...
            if (!(vlan_port->flags & BCM_VLAN_PORT_INGRESS_WIDE)
                && !_SHR_GPORT_IS_FORWARD_PORT(vlan_port->failover_port_id))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Error, Ring-Protection needs group = 1 and failover_port_id is fec gport");
            }
        }

        if (vlan_port->learn_strength)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Doesn't support learn strength setting at Jericho System header mode");
        }
    }

    if (vlan_port->learn_strength != TRUE && vlan_port->learn_strength != FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "learn_strength shall be 0 or 1");
    }

    if (DNX_L2_IS_LARGE_ENTRY_GROUP(vlan_port->group))
    {
        uint32 max_lif_protection_ptr = SAL_UPTO_BIT(BCM_DNX_VLAN_PORT_PROTECTION_PTR_BITS_IN_LARGE_GROUP);
        uint32 protection_ptr = 0, failover_id = 0;
        uint32 lif_protection_ptr_nof_msb = dnx_data_failover.path_select.in_lif_protection_pointer_nof_msb_get(unit);

        BCM_FAILOVER_ID_GET(failover_id, vlan_port->ingress_failover_id);
        if ((failover_id % (1 << lif_protection_ptr_nof_msb)) != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error, the ingress_failover_id must be multiple of 4 when group > 15");
        }

        DNX_FAILOVER_ID_PROTECTION_POINTER_ENCODE(protection_ptr, failover_id);
        if (protection_ptr > max_lif_protection_ptr)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "lif protection pointer exceeded allowed range [0:%d] when group > 15\n",
                         (max_lif_protection_ptr << lif_protection_ptr_nof_msb));
        }
    }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    /** Verify the Egress Protection value at Replace according to the device support */
    if (is_egress == TRUE)
    {
        if (DNX_FAILOVER_IS_PROTECTION_ENABLED(vlan_port->egress_failover_id))
        {
            SHR_IF_ERR_EXIT(dnx_failover_id_valid_verify
                            (unit, BCM_FAILOVER_TYPE_ENCAP, 0, vlan_port->egress_failover_id));
        }
    }
    ...
    bcm_vlan_port_t * old_vlan_port)
{
    ...
    uint32 replaceable_flags;
    int is_support_change_protect;
    int is_virtual_trunk = 0;
    ...
    replaceable_flags =
        ...
        BCM_VLAN_PORT_STAT_INGRESS_ENABLE | BCM_VLAN_PORT_STAT_EGRESS_ENABLE | BCM_VLAN_PORT_EXTENDED;

    /*
     * Verify flags are same as previous configuration (apart from the above flags that we allow to change configuration of during replace):
     */
    if ((new_vlan_port->flags & ~replaceable_flags) != ((old_vlan_port->flags & ~replaceable_flags)))
    {
    ...
    if (is_ingress)
    {
        ...
        if (BCM_GPORT_IS_TRUNK(new_vlan_port->port))
        {
            SHR_IF_ERR_EXIT(dnx_trunk_utils_trunk_id_to_id_info_convert(unit, new_vlan_port->port, &id_info));
            SHR_IF_ERR_EXIT(dnx_trunk_sw_db_group_flags_get
                            (unit, id_info.pool_index, id_info.group_index, &trunk_flags));
        ...
        if (is_physical_port)
        {
            ...
            /**virtual trunk doesn't allocate pp_port*/
            if (!is_virtual_trunk)
            {
                /*
        ...
        else
        {
            ...
            if (is_native_ac)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ingress_native_ac_interface_namespace_check
                                (unit, new_vlan_port->port, &gport_hw_resources, &new_next_layer_nwk_domain,
                                 &new_is_intf_namespace));
                ...
                /** get vlan_domain and LIF or NETWORK scope for new vlan-port*/
                SHR_IF_ERR_EXIT(dnx_ingress_native_ac_interface_namespace_check
                                (unit, old_vlan_port->port, &gport_hw_resources, &old_next_layer_nwk_domain,
                                 &old_is_intf_namespace));
    ...
    if (is_egress)
    {
        ...
        else if (dnx_data_multicast.
                 params.feature_get(unit, dnx_data_multicast_params_mc_pruning_actions_not_supported))
        {
            /*
    ...
    bcm_gport_t gport)
{
     ...
     * Verify that the gport is allocated:
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_is_allocated(unit, gport, &is_allocated));
    if (is_allocated == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Wrong setting. gport = 0x%08X is not allocated!!!\n", gport);
    }
 ...
 */
shr_error_e
dnx_vlan_port_dbal_ethertype_convert(
    int unit,
    ...
    uint32 *layer_type_val)
{
    ...
    {
        case ETHERTYPE_INITIALIZATION:
            *layer_type_val = DBAL_ENUM_FVAL_LAYER_TYPES_INITIALIZATION;
            break;
        case ETHERTYPE_MPLS:
            *layer_type_val = DBAL_ENUM_FVAL_LAYER_TYPES_MPLS;
            break;
        case ETHERTYPE_IPV4:
            *layer_type_val = DBAL_ENUM_FVAL_LAYER_TYPES_IPV4;
            break;
        case ETHERTYPE_IPV6:
            *layer_type_val = DBAL_ENUM_FVAL_LAYER_TYPES_IPV6;
            break;
        case ETHERTYPE_ARP:
            *layer_type_val = DBAL_ENUM_FVAL_LAYER_TYPES_ARP;
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported match ethertype - 0x%x\n", match_ethertype);
            break;
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int is_local_in_lif_update)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ingress_native_ac_interface_namespace_check
                    (unit, port, &gport_hw_resources_match, &vlan_domain, &is_intf_namespace));
 ...
 */
shr_error_e
dnx_vlan_port_create_ingress_match_mac_port(
    int unit,
    bcm_gport_t port,
    bcm_mac_t match_srcmac,
    int is_tagged,
    bcm_gport_t local_in_lif,
    int is_local_in_lif_update)
{
     ...
     * Ingress default action
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, is_tagged ? DBAL_TABLE_PP_PORT_SA_BASED_TAGGED :
                                      DBAL_TABLE_PP_PORT_SA_BASED_DEFAULT, &entry_handle_id));
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        uint32 pp_port = gport_info.internal_port_pp_info.pp_port[pp_port_index];
        uint32 core = gport_info.internal_port_pp_info.core_id[pp_port_index];
        uint32 mapped_pp_port = pp_port | (core << dnx_data_port.general.pp_port_bits_size_get(unit));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MAPPED_PP_PORT, mapped_pp_port);
        dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, match_srcmac);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, INST_SINGLE, local_in_lif);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, dbal_flags));
    }
 ...
 */
shr_error_e
dnx_vlan_port_create_ingress_match_mac_port_vlan(
    int unit,
    bcm_gport_t port,
    bcm_mac_t match_srcmac,
    bcm_vlan_t match_vlan,
    ...
    int is_local_in_lif_update)
{
     ...
     * Ingress default action
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PP_PORT_SA_BASED_VLAN_SPECIFIC, &entry_handle_id));
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        uint32 pp_port = gport_info.internal_port_pp_info.pp_port[pp_port_index];
        uint32 core = gport_info.internal_port_pp_info.core_id[pp_port_index];
    ...
    bcm_vlan_port_t * vlan_port)
{
     ...
     * 1) Non native AC;
     * 2) Gliful symmetric Native AC with criteria of _MATCH_NONE.
     */
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
    {
    ...
    dnx_ingress_ac_table_info_t * ingress_ac_table_info)
{
    ...
    if (!(_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE)))
    {
        ...
        if (_SHR_IS_FLAG_SET(vlan_port->flags2, BCM_VLAN_PORT_FLAGS2_MAC_SAV))
        {
            ingress_ac_table_info->learn_info_type = LIF_LEARN_INFO_REQ_TYPE_EXTENDED_LIF;
        }
    }
    ...
    dnx_ingress_ac_table_info_t * ingress_ac_table_info)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_vlan_port_is_dpc_in_lif(unit, vlan_port, &is_dpc_in_lif));
...
dnx_vlan_port_create_learn_info_set(
    ...
    int is_native)
{
    uint32 full_dbal_struct_data[2] = { 0 };
    dbal_enum_value_field_learn_payload_context_e learn_context;
    uint32 learn_strength = ingress_ac_table_info->learn_strength;
    uint32 learn_strength_dest = ingress_ac_table_info->learn_info;
    uint8 learn_info_set = TRUE;
    uint8 is_opt_learn_context = FALSE;
    ...
    if ((learn_context ==
         DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__SYMMETRIC_DEST__2K_GROUP__2L_STRENGTH__IVL)
        || (learn_context ==
            DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__SYMMETRIC_DEST__2K_GROUP__2L_STRENGTH__SVL)
        || (learn_context == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__SYMMETRIC_DEST__2K_GROUP__2L_STRENGTH__BOTH))
    {
        is_opt_learn_context = TRUE;
    }

    /*
     * learn_strength bit in dest[20] 
     */
    if (learn_context != DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF0__ANY_DEST__2K_GROUP__2L_STRENGTH__SVL)
    {
        if (learn_strength)
        {
    ...
    if (is_native == FALSE)
    {
        ...
        switch (ingress_ac_table_info->learn_info_type)
        {
            case LIF_LEARN_INFO_REQ_TYPE_EXTENDED_LIF:
                if (DNX_L2_IS_LARGE_ENTRY_GROUP(ingress_ac_table_info->flush_group))
                {
                    learn_info_field_id = DBAL_FIELD_LEARN_INFO_NO_LIF_LARGE_EXT_SYM_LIF_EXT;
                    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                    (unit, learn_info_field_id, DBAL_FIELD_DESTINATION,
                                     &ingress_ac_table_info->learn_info, full_dbal_struct_data));
                    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                    (unit, learn_info_field_id, DBAL_FIELD_LEARN_STRENGTH, &learn_strength,
                                     full_dbal_struct_data));
                    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                    (unit, learn_info_field_id, DBAL_FIELD_LARGE_FLUSH_GROUP,
                ...
                else
                {
                    ...
                    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                    (unit, learn_info_field_id, DBAL_FIELD_DESTINATION,
                                     &learn_strength_dest, full_dbal_struct_data));
                    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                    (unit, learn_info_field_id, DBAL_FIELD_FLUSH_GROUP,
                                     &ingress_ac_table_info->flush_group, full_dbal_struct_data));
                }
                break;
            ...
            case LIF_LEARN_INFO_REQ_TYPE_OPTIMIZED_LIF:
                ...
                full_dbal_struct_data[0] = learn_strength;
                break;
            default:
        ...
        if (is_opt_learn_context)
        {
            full_dbal_struct_data[0] = learn_strength;
            if (learn_info_field_id == DBAL_FIELD_LEARN_INFO_NO_LIF_EXT_SYM_LIF_EXT)
            {
                full_dbal_struct_data[0] |= (ingress_ac_table_info->flush_group << 1);
            }
        }

        if (learn_info_set)
        {
            dbal_entry_value_field_arr32_set(unit, entry_handle_id, learn_info_field_id, INST_SINGLE,
                                             full_dbal_struct_data);
        }
    }
     ...
     *  The only supported native result type IN_ETH_AC_MP, 
     *  uses the NO_LIF__FEC_or_SPA_or_TRAP_DEST__16_GROUP__2L_STRENGTH__BOTH(NO_LIF_EXTENDED)
     *  or NO_LIF__FEC_or_SPA_or_TRAP_DEST__16_GROUP__2L_STRENGTH__BOTH(NO_LIF_EXTENDED)
     */
    else
    {
        SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
        ...
        SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                        (unit, DBAL_FIELD_LEARN_INFO_NO_LIF_EXT_SYM_LIF_EXT, DBAL_FIELD_FLUSH_GROUP,
                         &ingress_ac_table_info->flush_group, full_dbal_struct_data));
        /** 1b NO_GROUP or 12b 2K_GROUP, learn_strength is Learn_Data[0] and GROUP is Learn_Data[11:1], dest is SSPA() */
        if (is_opt_learn_context)
        {
            full_dbal_struct_data[0] = learn_strength;
            full_dbal_struct_data[0] |= (ingress_ac_table_info->flush_group << 1);
...
dnx_vlan_port_create_ingress_lif_info_set(
    ...
    int *is_p2p)
{
    int is_native, has_vsi, is_native_virtual, is_virtual, is_extended_p2p_w_src_lkp, has_global_lif, has_learning,
        has_protection;
    ...
    is_extended_p2p_w_src_lkp = _SHR_IS_FLAG_SET(vlan_port->flags2, BCM_VLAN_PORT_FLAGS2_MAC_SAV);
    ...
    *is_p2p = FALSE;
    ...
    has_learning = ((!(*is_p2p)) && (!is_native_virtual)
                    && (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_RECYCLE))) || is_extended_p2p_w_src_lkp;
    ...
    if (has_learning)
    {
        SHR_IF_ERR_EXIT(dnx_vlan_port_create_learn_info_set(unit,
                                                            ...
                                                            is_native));
        /*
         * Enable the learning for Extended P2P with SA lookup 
         */
        if (is_extended_p2p_w_src_lkp)
        {
    ...
    if (!DNX_L2_IS_LARGE_ENTRY_GROUP(vlan_port->group))
    {
        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_INGRESS_WIDE))
        {
            ...
            {
                /*
                 * DBAL_FIELD_LIF_GENERIC_DATA_1 is supported only When protection is disabled.
                 */
                if (!has_protection)
                {
    ...
    else
    {
        ...
        {
            /*
             * For LARGE_ENTRY_GROUP, set LIF_GENERIC_DATA_1 only. Since it is the only possible option.
             */
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_1, INST_SINGLE, 0);
        }
        else
    ...
    dnx_in_lif_profile_last_info_t * last_profile)
{
    ...
    /*
     * Update lif_on_lag_is_shown
     */
    in_lif_profile_info.ingress_fields.lif_on_lag_is_shown = (uint8) lif_on_lag_is_shown;

    /*
     * Update oam_primary_vlan_mode
     */
    in_lif_profile_info.ingress_fields.oam_primary_vlan_mode = (uint8) oam_primary_vlan_mode;
    ...
    if (!is_replace)
    {
        ...
        switch (vlan_port->criteria)
        {
            ...
            case BCM_VLAN_PORT_MATCH_PORT:
            {
                ...
                /*
                 * Ethertype is not supported for Ingress Native and DPC.
                 */
                if (!is_dpc_in_lif)
                {
            ...
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                             DBAL_RESULT_TYPE_LOCAL_DPC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_MAC_PORT);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GPORT, INST_SINGLE, vlan_port->port);
                dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, INST_SINGLE,
                                                vlan_port->match_srcmac);
                break;
            }
            case BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED:
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                             DBAL_RESULT_TYPE_LOCAL_DPC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_MAC_PORT_TAGGED);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GPORT, INST_SINGLE, vlan_port->port);
                dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, INST_SINGLE,
                                                vlan_port->match_srcmac);
                break;
            ...
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                             DBAL_RESULT_TYPE_LOCAL_DPC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_MAC_PORT_VLAN);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GPORT, INST_SINGLE, vlan_port->port);
                dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, INST_SINGLE,
                                                vlan_port->match_srcmac);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUTER_VLAN_ID, INST_SINGLE,
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    if (is_native_ac || is_pon_dtc_ac || dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
    {
        ...
        switch (result_type)
        {
            ...
            case DBAL_RESULT_TYPE_LOCAL_DPC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_MAC_PORT:
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id, DBAL_FIELD_GPORT, INST_SINGLE, &tmp32bit));
                vlan_port->port = (bcm_gport_t) tmp32bit;
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                                (unit, entry_handle_id, DBAL_FIELD_L2_MAC, INST_SINGLE, vlan_port->match_srcmac));
                vlan_port->criteria = BCM_VLAN_PORT_MATCH_MAC_PORT;
                break;
            }
            case DBAL_RESULT_TYPE_LOCAL_DPC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_MAC_PORT_TAGGED:
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id, DBAL_FIELD_GPORT, INST_SINGLE, &tmp32bit));
                vlan_port->port = (bcm_gport_t) tmp32bit;
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                                (unit, entry_handle_id, DBAL_FIELD_L2_MAC, INST_SINGLE, vlan_port->match_srcmac));
                vlan_port->criteria = BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED;
                break;
            }
            case DBAL_RESULT_TYPE_LOCAL_DPC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_MAC_PORT_VLAN:
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id, DBAL_FIELD_GPORT, INST_SINGLE, &tmp32bit));
                vlan_port->port = (bcm_gport_t) tmp32bit;
                ...
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id, DBAL_FIELD_OUTER_VLAN_ID, INST_SINGLE, &tmp32bit));
                vlan_port->match_vlan = (bcm_vlan_t) tmp32bit;
                vlan_port->criteria = BCM_VLAN_PORT_MATCH_MAC_PORT_VLAN;
                break;
            }
            default:
            {
    ...
    uint32 system_port)
{
    ...
    etpp_out_lif_profile_info_t out_lif_profile_info;
    uint8 is_last = 0;
    uint8 is_esem_cmd_field_on_handle = 0, is_out_lif_profile_field_on_handle = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_field_exist_on_dbal_handle
                    (unit, entry_handle_id, DBAL_FIELD_OUT_LIF_PROFILE, &is_out_lif_profile_field_on_handle));

    if (is_out_lif_profile_field_on_handle)
    {
        /*
         * Get out_lif_profile:
         */
    ...
    dnx_ingress_ac_table_info_t * ingress_ac_table_info)
{
    ...
    /*
     * Set the learn enable to TRUE except the standard Service-AC.
    ...
    else
    {
        ...
        if (BCM_GPORT_IS_TRUNK(vlan_port->port))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_trunk_utils_trunk_id_to_id_info_convert(unit, vlan_port->port, &id_info));
            SHR_IF_ERR_EXIT(dnx_trunk_sw_db_group_flags_get
                            (unit, id_info.pool_index, id_info.group_index, &trunk_flags));
     ...
     */
    if (vlan_port->group || !is_optimized_allowed)
    {
        /*
         * group: SYMMETRIC_LIF_EXTENDED_SVL, SYMMETRIC_LIF_EXTENDED_IVL
         * !is_optimized_allowed:SYMMETRIC_LIF_BASIC_SVL, SYMMETRIC_LIF_BASIC_IVL 
         */
        if (is_ivl)
        {
            if (dest_is_flow_mc)
            {
                ingress_ac_table_info->learn_context =
                    DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__TMF_OR_MC_DEST__2K_GROUP__2L_STRENGTH__IVL;
            }
            else
            {
                ingress_ac_table_info->learn_context =
                    DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__FEC_OR_SPA_OR_TRAP_DEST__2K_GROUP__2L_STRENGTH__IVL;
            }
        }
        else
        {
            if (DNX_L2_IS_LARGE_ENTRY_GROUP(vlan_port->group))
            {
                ...
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__ANY_DEST__2K_GROUP__2L_STRENGTH__SVL;
                }
                else
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__ANY_DEST__2K_GROUP__2L_STRENGTH__SVL;
                }
            }
            else
            {
                ...
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__TMF_OR_MC_DEST__2K_GROUP__2L_STRENGTH__SVL;
                }
                else
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__FEC_OR_SPA_OR_TRAP_DEST__2K_GROUP__2L_STRENGTH__SVL;
                }
            }
        }
    }
    ...
    {
        /*
         * SYMMETRIC_LIF_OPTIMIZED_SVL, SYMMETRIC_LIF_OPTIMIZED_IVL 
         */
        if (is_ivl)
        {
            ingress_ac_table_info->learn_context =
        ...
        (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE))
    {
         ...
         * SYMMETRIC_LIF_BASIC_SVL, SYMMETRIC_LIF_BASIC_IVL
         */
        if (dest_is_flow_mc)
        {
            ingress_ac_table_info->learn_context =
    ...
    if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO2_MODE)
    {
            ...
            || (forward_info->fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY))
        {
            ...
            {
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__TMF_OR_MC_DEST__2K_GROUP__2L_STRENGTH__SVL:
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__TMF_OR_MC_DEST__2K_GROUP__2L_STRENGTH__IVL:
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__TMF_OR_MC_DEST__2K_GROUP__2L_STRENGTH__BOTH;
                    break;
                }
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__FEC_OR_SPA_OR_TRAP_DEST__2K_GROUP__2L_STRENGTH__SVL:
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__FEC_OR_SPA_OR_TRAP_DEST__2K_GROUP__2L_STRENGTH__IVL:
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__FEC_OR_SPA_OR_TRAP_DEST__2K_GROUP__2L_STRENGTH__BOTH;
                    break;
                }
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__SYMMETRIC_DEST__2K_GROUP__2L_STRENGTH__SVL:
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__SYMMETRIC_DEST__2K_GROUP__2L_STRENGTH__IVL:
                {
                    ingress_ac_table_info->learn_context =
                        DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__SYMMETRIC_DEST__2K_GROUP__2L_STRENGTH__BOTH;
                    break;
                }
                default:
                {
    ...
    else if (is_native == FALSE)
    {
            ...
            (forward_info->outlif == 0))
        {
            ...
            {
                ingress_ac_table_info->learn_context =
                    DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF0__ANY_DEST__2K_GROUP__2L_STRENGTH__SVL;
            }
        }
    }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    int indexed_mode_enable = 0, native_indexed_mode = 0, lif_on_lag_is_shown = 0, oam_primary_vlan_mode = 0;
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags2, BCM_VLAN_PORT_FLAGS2_OAM_TRUNK_MEMBER))
    {
        if (BCM_GPORT_IS_TRUNK(vlan_port->port))
        {
            lif_on_lag_is_shown = 1;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "BCM_VLAN_PORT_FLAGS2_OAM_TRUNK_MEMBER (0x%08X) is supported only when LIF is created on turnk.\n",
                         BCM_VLAN_PORT_FLAGS2_OAM_TRUNK_MEMBER);
        }
    }

    if (_SHR_IS_FLAG_SET(vlan_port->flags2, BCM_VLAN_PORT_FLAGS2_OAM_PRIMARY_VLAN))
    {
        oam_primary_vlan_mode = 1;
    }
    ...
    if (!is_replace)
    {
        SHR_IF_ERR_EXIT(dnx_ingress_inlif_profile_alloc
                        (unit, vlan_port->ingress_network_group_id, lif_on_lag_is_shown, oam_primary_vlan_mode,
                         native_indexed_mode, DNX_IN_LIF_PROFILE_DEFAULT, &(ingress_ac_table_info->in_lif_profile),
                         ingress_ac_table_info->dbal_table_id, &last_profile));
    }
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_inlif_profile_alloc
                        (unit, vlan_port->ingress_network_group_id, lif_on_lag_is_shown, oam_primary_vlan_mode,
                         native_indexed_mode, old_in_lif_profile, &(ingress_ac_table_info->in_lif_profile),
                         ingress_ac_table_info->dbal_table_id, &last_profile));
    }
    ...
    /*
     * learn_strength information
     */
    ingress_ac_table_info->learn_strength = vlan_port->learn_strength ? 1 : 0;
    ...
    bcm_vlan_port_t * old_vlan_port)
{
    ...
    int is_virtual_ac;
    int is_p2p;
    bcm_vlan_port_t vlan_port_created;
    uint8 is_virtual_trunk = 0;
    bcm_trunk_t master_tid;
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags2, BCM_VLAN_PORT_FLAGS2_MAC_SAV))
    {
        lif_info.table_specific_flags = DNX_ALGO_LIF_TABLE_SPECIFIC_AC_INGRESS_EXTENDED_P2P;
    }
    /** For P2P REPLACE of non-native - need to set table-specific-flags */
    else if ((is_p2p == TRUE) && (!is_native_ac))
    {
    ...
    if (!is_replace)
    {
        vlan_port_created = *vlan_port;

        SHR_IF_ERR_EXIT(dnx_trunk_sw_db_group_gport_is_virtual_trunk
                        (unit, vlan_port->port, &is_virtual_trunk, &master_tid));
        /**if virtual trunk, replace master trunk for match*/
        if (is_virtual_trunk)
        {
            BCM_GPORT_TRUNK_SET(vlan_port_created.port, master_tid);
        }

        SHR_IF_ERR_EXIT(dnx_vlan_port_match_inlif_set(unit, &vlan_port_created, local_inlif, FALSE));
    }
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    uint32 default_outlif_profile;
    uint32 old_esem_cmd_profile = 0;
    uint8 is_esem_cmd_last_reference = FALSE, is_last = 0;
    uint32 old_out_lif_profile = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    if (!is_replace)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_egress_outlif_profile_alloc
                        (unit, vlan_port->egress_network_group_id, default_outlif_profile,
                         &(egress_ac_table_info->out_lif_profile), outlif_profile_dbal_table, 0, &is_last));
        /** egress qos*/
    ...
    else
    {
        ...
        /** ESEM command should not change in replace */
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_field_exist_on_dbal_handle
                        (unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, &is_field_on_handle));
        if (is_field_on_handle)
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE,
                             (uint32 *) &egress_ac_table_info->esem_cmd));
        }
    }
    ...
    if (is_last)
    {
        SHR_IF_ERR_EXIT(dnx_etpp_out_lif_profile_hw_clear(unit, (int) old_out_lif_profile));
    }
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    uint32 entry_handle_id)
{
    ...
    if (!is_native)
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_get(unit, gport, 0, &sw_state_forward_info));
    }
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_rx_trap_type_get(unit, 0, bcmRxTrapDfltDroppedPacket, (int *) &trap_id));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_egress_outlif_profile_alloc
                    (unit, egress_network_group_id, DNX_OUT_LIF_PROFILE_DEFAULT, &out_lif_profile,
                     DBAL_TABLE_ESEM_DEFAULT_RESULT_TABLE, 0, &is_last));
    dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_OUT_LIF_PROFILE, INST_SINGLE, out_lif_profile);
    ...
    uint32 *out_lif_is_zero)
{
    dnx_algo_gpm_forward_info_t forward_info;
    bcm_gport_t destination;
    bcm_gport_t gport = vlan_port->vlan_port_id;
    SHR_FUNC_INIT_VARS(unit);
    ...
    int *local_in_lif)
{
    ...
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT), _SHR_E_NOT_FOUND);
    ...
    bcm_gport_t * vlan_port_id)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        /** convert the flow gport back to legacy if dnx data is_flow_enabled_for_legacy_applications=1 */
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, gport, BCM_GPORT_VLAN_PORT, DNX_ALGO_GPM_CONVERSION_FLAG_NATIVE, vlan_port_id));
    }
    else
    {
        *vlan_port_id = gport;
...
/*
 * This function returns the local-lif value of the port X sa ISEM entry
 * \see
 * bcm_dnx_vlan_port_find
 */
static shr_error_e
dnx_vlan_port_find_ingress_match_mac_port(
    int unit,
    bcm_gport_t port,
    bcm_mac_t match_srcmac,
    int is_tagged,
...
/*
 * This function returns the local-lif value of the port X sa X vlan ISEM entry
 * \see
 * bcm_dnx_vlan_port_find
 */
static shr_error_e
dnx_vlan_port_find_ingress_match_mac_port_vlan(
    int unit,
    bcm_gport_t port,
    bcm_mac_t match_srcmac,
    bcm_vlan_t match_vlan,
    ...
    dnx_ingress_ac_fields_table_info_get_t * ingress_ac_fields_info)
{
    ...
    uint8 is_opt_learn_context = 0;
    uint8 is_large_learn_info_field = 0;
    uint32 large_learn_info[2] = { 0 };
    ...
    if ((!is_native) || is_native_non_virtual)
    {
        ...
        if (in_lif_profile_info.ingress_fields.lif_on_lag_is_shown)
        {
            vlan_port->flags2 |= BCM_VLAN_PORT_FLAGS2_OAM_TRUNK_MEMBER;
        }
    }
    else
    {
    ...
    if (is_field_on_handle)
    {
        ...
        if ((dbal_struct_data ==
            ...
            || (dbal_struct_data ==
                DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF__SYMMETRIC_DEST__2K_GROUP__2L_STRENGTH__IVL)
            || (dbal_struct_data ==
                DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_ASYMMETRIC_LIF__ANY_DEST__16_GROUP__2L_STRENGTH__IVL))
        {
            vlan_port->flags |= BCM_VLAN_PORT_IVL;
        }
        /**For SYMMETRIC_LIF0__ANY_DEST__16_GROUP__2L_STRENGTH__SVL, the destination hasn't learn_strength */
        if (dbal_struct_data !=
            DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF0__ANY_DEST__2K_GROUP__2L_STRENGTH__SVL)
        {
            if ((dbal_struct_data ==
                ...
                || (dbal_struct_data ==
                    DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF__SYMMETRIC_DEST__2K_GROUP__2L_STRENGTH__BOTH))
            {
                is_opt_learn_context = 1;
            }
            /*
             * Get learn_strength when learn_payload_context exists 
             */
            SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_field_exist_on_dbal_handle
                            (unit, entry_handle_id, DBAL_FIELD_LEARN_INFO_NO_LIF_LARGE_EXT_SYM_LIF_EXT,
                             &is_large_learn_info_field));

            if (is_large_learn_info_field == FALSE)
            {

                dbal_struct_data = 0;
                SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                                (unit, entry_handle_id, DBAL_FIELD_LEARN_INFO_NO_LIF_EXT_SYM_LIF_EXT,
                                 &is_field_on_handle, &dbal_struct_data));
            }
            else
            {
                /** Get LEARN_INFO_NO_LIF_LARGE_EXT_SYM_LIF_EXT value  */
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                                (unit, entry_handle_id, DBAL_FIELD_LEARN_INFO_NO_LIF_LARGE_EXT_SYM_LIF_EXT, INST_SINGLE,
                                 large_learn_info));
                /*
                 * Retrieve flush group from large learn information:
                 */
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                                (unit, DBAL_FIELD_LEARN_INFO_NO_LIF_LARGE_EXT_SYM_LIF_EXT, DBAL_FIELD_LARGE_FLUSH_GROUP,
                                 &(vlan_port->group), large_learn_info));
            }
            if (is_field_on_handle == FALSE)
            {
                dbal_struct_data = 0;
                SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                                (unit, entry_handle_id, DBAL_FIELD_LEARN_INFO_NO_LIF_BASIC_SYM_LIF_BASIC,
            ...
            if (is_large_learn_info_field)
            {
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                                (unit, DBAL_FIELD_LEARN_INFO_NO_LIF_LARGE_EXT_SYM_LIF_EXT, DBAL_FIELD_LEARN_STRENGTH,
                                 &learn_strength, large_learn_info));
            }
            else
            {
                /*
                 * learn_strength could be learn_data[20] or learn_data[0] 
                 */
                learn_strength =
                    SHR_BITGET(&dbal_struct_data, DNX_L2_LEARN_STRENGT_OFFSET_IN_LEARN_INFO(is_opt_learn_context));
            }
            vlan_port->learn_strength = learn_strength ? 1 : 0;
        }
    }
    ...
    uint8 not_found_log_enable)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        /** convert the flow gport back to legacy if dnx data is_flow_enabled_for_legacy_applications=1 */
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, gport, BCM_GPORT_VLAN_PORT, DNX_ALGO_GPM_CONVERSION_FLAG_NONE,
                         &vlan_port->vlan_port_id));
    }
    else
    {
        ...
        switch (vlan_port->criteria)
        {
            ...
            case BCM_VLAN_PORT_MATCH_PORT_VLAN:
            {
                if (is_native_ac == FALSE)
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dnx_vlan_port_find_ingress_match_port_vlan
                                                           (unit, vlan_port->port, vlan_port->match_vlan,
                                                            &local_in_lif), _SHR_E_NOT_FOUND);
                    }
                }
                else
                {
            ...
            case BCM_VLAN_PORT_MATCH_MAC_PORT:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_mac_port
                                (unit, vlan_port->port, vlan_port->match_srcmac, 0, &local_in_lif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_mac_port
                                (unit, vlan_port->port, vlan_port->match_srcmac, 1, &local_in_lif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_MAC_PORT_VLAN:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_mac_port_vlan
                                (unit, vlan_port->port, vlan_port->match_srcmac, vlan_port->match_vlan, &local_in_lif));
                break;
            }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    if (vlan_port->vlan_port_id == 0)
    {
        SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dnx_vlan_port_find_by_lookup(unit, vlan_port), _SHR_E_NOT_FOUND);
...
/**
 * \brief - Clear PORT x SA match in ingress configuration.
 *          ISEM lookup is cleared.
 * \see
 *  bcm_dnx_vlan_port_destroy_ingress
 */
static shr_error_e
dnx_vlan_port_destroy_ingress_match_mac_port_clear(
    int unit,
    bcm_gport_t port,
    bcm_mac_t match_srcmac,
    int is_tagged)
...
/**
 * \brief - Clear PORT x SA x VLAN match in ingress configuration.
 *          ISEM lookup is cleared.
 * \see
 *  bcm_dnx_vlan_port_destroy_ingress
 */
static shr_error_e
dnx_vlan_port_destroy_ingress_match_mac_port_vlan_clear(
    int unit,
    bcm_gport_t port,
    bcm_mac_t match_srcmac,
    bcm_vlan_t match_vlan)
    ...
    if (BCM_GPORT_IS_TRUNK(vlan_port->port))
    {
        ...
        if (trunk_flags & BCM_TRUNK_FLAG_VP)
        {
            SHR_IF_ERR_EXIT(dnx_trunk_sw_db_group_master_trunk_id_get
                            (unit, id_info.pool_index, id_info.group_index, (bcm_trunk_t *) & port));
            BCM_GPORT_TRUNK_SET(port, port);
        }
    }
    ...
    switch (vlan_port->criteria)
    {
        ...
        case BCM_VLAN_PORT_MATCH_PORT:
        {
            if (vlan_port->match_ethertype)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_port_vlan_vlan_pcp_ethertype_clear
                                (unit, port, BCM_VLAN_INVALID, BCM_VLAN_INVALID,
            ...
            else
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_port_default(unit, port, initial_default_in_lif));
            }
            break;
        }
        case BCM_VLAN_PORT_MATCH_PORT_VLAN:
        {
            if (vlan_port->match_ethertype)
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_port_vlan_vlan_pcp_ethertype_clear
                                (unit, port, vlan_port->match_vlan, BCM_VLAN_INVALID,
                                 vlan_port->match_ethertype, BCM_DNX_PCP_INVALID));
            }
            else
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_port_vlan_clear(unit, port, vlan_port->match_vlan));
            }
            break;
        }
        case BCM_VLAN_PORT_MATCH_PORT_CVLAN:
        {
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_port_cvlan_clear
                                (unit, port, vlan_port->match_vlan));
            }
            break;
        }
        case BCM_VLAN_PORT_MATCH_PORT_VLAN_STACKED:
        {
            if (vlan_port->match_ethertype)
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_port_vlan_vlan_pcp_ethertype_clear
                                (unit, port, vlan_port->match_vlan, vlan_port->match_inner_vlan,
                                 vlan_port->match_ethertype, BCM_DNX_PCP_INVALID));
            }
            else
            {
        ...
        case BCM_VLAN_PORT_MATCH_PORT_UNTAGGED:
        {
            ...
            else
            {
                 ...
                 * Clear the ISEM entry
                 */
                SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_port_untagged_clear(unit, port));
            }
        ...
        case BCM_VLAN_PORT_MATCH_PORT_TUNNEL:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_port_tunnel_unmatched_clear
                            (unit, port, vlan_port->match_tunnel_value));
            break;
        }
        case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_VLAN:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_port_tunnel_svlan_clear
                            (unit, port, vlan_port->match_tunnel_value, vlan_port->match_vlan));
            break;
        }
        case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_CVLAN:
        {
        ...
        case BCM_VLAN_PORT_MATCH_MAC_PORT:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_mac_port_clear(unit, port, vlan_port->match_srcmac, 0));
            break;
        }
        case BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_mac_port_clear(unit, port, vlan_port->match_srcmac, 1));
            break;
        }
        case BCM_VLAN_PORT_MATCH_MAC_PORT_VLAN:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_match_mac_port_vlan_clear
                            (unit, port, vlan_port->match_srcmac, vlan_port->match_vlan));
            break;
        }
        default:
        {
    ...
    void *user_data)
{
    ...
    while (!is_end)
    {
        ...
        if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
        {
            /** convert the flow gport back to legacy if dnx data is_flow_enabled_for_legacy_applications=1 */
            SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                            (unit, vlan_port_id, BCM_GPORT_VLAN_PORT, DNX_ALGO_GPM_CONVERSION_FLAG_NONE,
                             &vlan_port.vlan_port_id));
        }
        else
        {
    ...
    int is_local_in_lif_update)
{
    ...
    switch (vlan_port->criteria)
    {
        ...
        case BCM_VLAN_PORT_MATCH_MAC_PORT:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_mac_port
                            (unit, vlan_port->port, vlan_port->match_srcmac, 0, local_in_lif_id,
                             is_local_in_lif_update));
            break;
        }
        case BCM_VLAN_PORT_MATCH_MAC_PORT_TAGGED:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_mac_port
                            (unit, vlan_port->port, vlan_port->match_srcmac, 1, local_in_lif_id,
                             is_local_in_lif_update));
            break;
        }
        case BCM_VLAN_PORT_MATCH_MAC_PORT_VLAN:
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_mac_port_vlan
                            (unit, vlan_port->port, vlan_port->match_srcmac, vlan_port->match_vlan, local_in_lif_id,
                             is_local_in_lif_update));
            break;
        }
        default:
