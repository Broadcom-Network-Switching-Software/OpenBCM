    ...
    uint32 flags)
{
    ...
    /*
     * For L1 ETH, always configure the Rate adapter
     */
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, TRUE) || DNX_ALGO_PORT_TYPE_IS_FLEXE_UNAWARE(unit, port_info))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_speed_get(unit, port, DNX_ALGO_PORT_SPEED_F_TX, &tx_speed));
        /*
         * Get the rate adapter bus type
         */
        flexe_core_rate_adpt_info_t_init(unit, &rate_adpt_info);
        if (DNX_ALGO_PORT_TYPE_IS_FLEXE_UNAWARE(unit, port_info))
        {
            rate_adpt_info.bus_type = flexe_core_rateadpt_cl82_adj_bypass;
        }
        else if (IMB_FLEXE_AM_TRANSPARENT_GET(flags))
        {
            if ((tx_speed == 100000) || (tx_speed == 50000))
            {
                rate_adpt_info.bus_type = flexe_core_rateadpt_adj_shift_bypass;
            }
            else if ((tx_speed == 200000) || (tx_speed == 400000))
            {
                rate_adpt_info.bus_type = flexe_core_rateadpt_cl82_adj_bypass;
            }
            else
            {
                rate_adpt_info.bus_type = flexe_core_rateadpt_cl49_adj_bypass;
            }
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_l1_eth_1588_enable_get(unit, port, BCM_PORT_FLEXE_TX, &is_1588_enable));
            if (is_1588_enable)
            {
                /*
                 * If 1588 is enabled, rate adapter should be bypassed
                 */
                rate_adpt_info.bus_type = flexe_core_rateadpt_adj_shift_bypass;
            }
            else
            {
                rate_adpt_info.bus_type = flexe_core_rateadpt_normal;
            }
        }
    ...
    if (curr_active_cal != active_cal)
    {
        ...
        _SHR_PBMP_ITER(all_channels_in_group, client_channel)
        {
            ...
            if (ts_config_db[client_channel].nof_flexe_core_ports != 0)
            {
                ...
                if (dnx_data_nif.global.feature_get(unit, dnx_data_nif_global_is_g_hao_supported))
                {
                    SHR_IF_ERR_EXIT(flexe_core_oam_timeslot_add
                                    (unit, client_channel, BCM_PORT_FLEXE_TX, rate_adpt_info.speed));
                }
            }
            else
            {
                ...
                if (dnx_data_nif.global.feature_get(unit, dnx_data_nif_global_is_g_hao_supported))
                {
                    SHR_IF_ERR_EXIT(flexe_core_oam_timeslot_delete(unit, client_channel, BCM_PORT_FLEXE_TX));
                }
            }
        }
    }
...
dnx_port_flexe_group_cal_active_rx_set(
    ...
    bcm_port_flexe_group_cal_t active_cal)
{
    int group_index, max_nof_clients, group_speed;
    int nof_slots, slot_index, client_channel;
    int calendar_slots[DNX_DATA_MAX_NIF_FLEXE_MAX_NOF_SLOTS];
    int ts_num, speed;
    ...
    bcm_port_flexe_phy_slot_mode_t slot_mode;
    dnx_algo_flexe_client_ts_config_t *ts_config_db = NULL;
    bcm_pbmp_t all_channels_in_group;
     ...
     * Get current active calendar
     */
    SHR_IF_ERR_EXIT(dnx_algo_flexe_demux_active_cal_id_get(unit, group_index, &curr_active_cal));
    /*
     * If there is calendar switching, need to update rate adapter accordingly.
     */
    if (curr_active_cal != active_cal)
    {
        ...
        /*
         * Update OAM Rx slots for all the clients in the group:
         *
         * - If there is no slot for the client, always delete it
         * - If there are slots for the client, always add it.
         */
        _SHR_PBMP_ITER(all_channels_in_group, client_channel)
        {
            SHR_BITCOUNT_RANGE(ts_config_db[client_channel].ts_mask, ts_num, 0, BCM_PORT_FLEXE_MAX_NOF_SLOTS);
            speed = ts_num * dnx_data_nif.flexe.client_speed_granularity_get(unit, slot_mode)->val;

            if (ts_config_db[client_channel].nof_flexe_core_ports != 0)
            {
                SHR_IF_ERR_EXIT(flexe_core_oam_timeslot_add(unit, client_channel, BCM_PORT_FLEXE_RX, speed));
            }
            else
            {
                SHR_IF_ERR_EXIT(flexe_core_oam_timeslot_delete(unit, client_channel, BCM_PORT_FLEXE_RX));
            }
        }
    }
    /*
     * Swtich schedule calendar
     */
    SHR_IF_ERR_EXIT(flexe_core_schedule_calendar_switch(unit, group_index, flags, active_cal));
    /*
     * Update DB. The saved cal ID is used to check if the client
     * is going to be added into active calendar. If yes, need to update
     * the schedule calendar.
     */
    SHR_IF_ERR_EXIT(dnx_algo_flexe_demux_active_cal_id_set(unit, group_index, active_cal));
exit:
    SHR_FREE(ts_config_db);
    SHR_FUNC_EXIT;
}
    ...
    int *calendar_slots)
{
    ...
    int speed;
    bcm_gport_t gport;
    bcm_port_flexe_group_cal_t active_rx_cal, active_tx_cal;
    dnx_algo_flexe_client_ts_config_t *ts_config_db = NULL;
    ...
    if (DNX_ALGO_PORT_FLEXE_TX_GET(flags) && !is_bypass)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_dnx_port_flexe_group_cal_active_get(unit, gport, BCM_PORT_FLEXE_TX, &active_tx_cal));
    }
    if (DNX_ALGO_PORT_FLEXE_TX_GET(flags)
        && dnx_data_nif.global.feature_get(unit, dnx_data_nif_global_is_g_hao_supported))
    {
        /*
         * Get the Rx active calendar from DB. API bcm_dnx_port_flexe_group_cal_active_get
         * gets active calendar from C bit, which might be not reliable.
         */
        SHR_IF_ERR_EXIT(dnx_algo_flexe_demux_active_cal_id_get(unit, group_index, &active_rx_cal));
    }
    /*
     * Alloc space
     */
    ...
    for (count = 0; count < sizeof(slots_direction) / sizeof(uint32); ++count)
    {
        ...
        _SHR_PBMP_ITER(changed_clients, client_channel)
        {
            if (orig_ts_config_db[client_channel].nof_flexe_core_ports != 0)
            {
                ...
                SHR_BITCOUNT_RANGE(orig_ts_config_db[client_channel].ts_mask, ts_num, 0, BCM_PORT_FLEXE_MAX_NOF_SLOTS);
                speed = ts_num * dnx_data_nif.flexe.client_speed_granularity_get(unit, slot_mode)->val;
                /*
                 * Need to update the rate adpater is configuring the active calendar
                 * Always update the rate adpater for bypass mode
                 */
                if (DNX_ALGO_PORT_FLEXE_TX_GET(slots_direction[count]) && ((calendar_id == active_tx_cal) || is_bypass))
                {
                    rate_adpt_info.speed = speed;
                    SHR_IF_ERR_EXIT(flexe_core_rateadpt_delete(unit, client_channel, &rate_adpt_info));
                }
                /*
                 * Need to update the OAM Rx/Tx timeslots
                 */
                if (((DNX_ALGO_PORT_FLEXE_RX_GET(slots_direction[count]) && (calendar_id == active_rx_cal)) ||
                     ((DNX_ALGO_PORT_FLEXE_TX_GET(slots_direction[count]) && (calendar_id == active_tx_cal)))) &&
                    dnx_data_nif.global.feature_get(unit, dnx_data_nif_global_is_g_hao_supported))
                {
                    SHR_IF_ERR_EXIT(flexe_core_oam_timeslot_delete(unit, client_channel, slots_direction[count]));
                }
            }
        }
        _SHR_PBMP_ITER(changed_clients, client_channel)
        {
            if (ts_config_db[client_channel].nof_flexe_core_ports != 0)
            {
                ...
                if (((DNX_ALGO_PORT_FLEXE_RX_GET(slots_direction[count]) && (calendar_id == active_rx_cal)) ||
                    ...
                    dnx_data_nif.global.feature_get(unit, dnx_data_nif_global_is_g_hao_supported))
                {
                    SHR_IF_ERR_EXIT(flexe_core_oam_timeslot_add(unit, client_channel, slots_direction[count], speed));
                }
                /*
        ...
        if (dnx_data_nif.global.feature_get(unit, dnx_data_nif_global_is_g_hao_supported))
        {
            if (_SHR_PBMP_NOT_NULL(changed_clients) &&
                ((DNX_ALGO_PORT_FLEXE_TX_GET(slots_direction[count]) && (calendar_id == active_tx_cal)) ||
                 (DNX_ALGO_PORT_FLEXE_RX_GET(slots_direction[count]) && (calendar_id == active_rx_cal))))
            {
    ...
    int unit)
{
    ...
    if (!dnx_data_nif.otn.feature_get(unit, dnx_data_nif_otn_is_supported))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FRW40_POWER_DOWN, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    /*
     * Power down FRWA/FRWBC blocks if Framer is not supported
     */
    if (!dnx_data_nif.framer.feature_get(unit, dnx_data_nif_framer_is_supported))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FRWBC_POWER_DOWN, INST_SINGLE, 1);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FRWA_POWER_DOWN, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    else
    {
    ...
    uint32 flags)
{
    ...
    for (count = 0; count < sizeof(slots_direction) / sizeof(uint32); ++count)
    {
        ...
        if (slots_direction[count] == BCM_PORT_FLEXE_RX)
        {
            /*
             * Update DB. The saved cal ID is used to check if the client
             * is going to be added into active calendar. If yes, need to update
             * the schedule calendar.
             */
            SHR_IF_ERR_EXIT(dnx_algo_flexe_demux_active_cal_id_set(unit, group_index, target_cal_id));
        }
    }
exit:
    SHR_FUNC_EXIT;
