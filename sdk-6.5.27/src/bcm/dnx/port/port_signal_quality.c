    ...
    uint32 *value_p)
{
    ...
    *value_p = 0;
    ...
    if (DNX_ALGO_PORT_TYPE_IS_FABRIC(unit, port_info))
    {
        ...
        {
            int link_id = 0;
            /** Translate port to link id */
            SHR_IF_ERR_EXIT(dnx_algo_port_fabric_link_get(unit, port, &link_id));

            /** get the last calculated BER */
            SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.prev_error_rate.get(unit, link_id, value_p));
        }
    }
    else
    {
        ...
        if (dnx_data_nif.signal_quality.feature_get(unit, dnx_data_nif_signal_quality_is_supported))
        {
            ...
            if (is_enabled)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_port_db.signal_quality.phy_state.
                                prev_error_rate.get(unit, first_phy, value_p));
            }
        }
    }
...
static void
dnx_port_signal_quality_fabric_ber_fec_compensate(
    int unit,
    ...
    uint64 *compensated_count)
{
    ...
    /** get compensation data based on FEC type */
    fec_entry = dnx_data_port.general.fec_compensation_info_get(unit, fec_type);

    /** if the FEC requires compensation, do the math */
    if (fec_entry && fec_entry->total_bits != 0)
    {
        *compensated_count *= (fec_entry->total_bits / fec_entry->counted_bits);
    }
}

static shr_error_e
dnx_port_signal_quality_fabric_ber_error_rate_get(
    int unit,
    bcm_port_t port,
    int interval,
    uint64 er_cnt,
    ...
    float *error_rate)
{
    ...
    *error_rate = 0;
    ...
    /** Get rate from sw-state*/
    /** Can we assume all ports are on the same rate */
    SHR_IF_ERR_EXIT(soc_dnxc_port_speed_sw_get(unit, port, &lane_rate));

    /** Get fec from sw-state*/
    ...
    if (uncorrectable_er_cnt)
    {
        ...
        /** minimum bad symbols in codeword */
        min_nof_bad_symbols =
            dnx_data_port.general.fec_compensation_info_get(unit, fec_type)->max_nof_correctable_symbols + 1;

        er_cnt += uncorrectable_er_cnt * min_nof_bad_symbols * DNX_PORT_SIGNAL_QUALITY_ER_BITS_PER_SYMBOL;
    }

    total_cnt = (uint64) lane_rate /** Mbps */  * (uint64) interval /** Micro-sec */ ;

    /** In some FEC types, bit errors are not counted over parity region */
    dnx_port_signal_quality_fabric_ber_fec_compensate(unit, fec_type, &er_cnt);
...
static shr_error_e
dnx_port_signal_quality_fabric_ber_db_lane_clear(
    int unit,
    int link_id)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.prev_error_rate.set(unit, link_id, 0));
    SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.prev_cnt.set(unit, link_id, 0));
    SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.prev_uncorrectable_cnt.set(unit, link_id, 0));
...
shr_error_e
dnx_port_signal_quality_fabric_ber_db_clear(
    int unit)
{
    bcm_pbmp_t sfi_pbmp, *pbmp_p;
    bcm_port_t port;
    ...
    _SHR_PBMP_ITER(sfi_pbmp, port)
    {
        ...
        /** clear state of corresponding first_phy */
        SHR_IF_ERR_EXIT(dnx_port_signal_quality_fabric_ber_db_lane_clear(unit, link_id));
    }
...
static shr_error_e
dnx_port_signal_quality_fabric_ber_db_update(
    int unit,
    int link_id,
    uint32 encoded_ber_val,
    uint64 er_cnt,
    uint64 prev_uncorrectable_cnt)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.prev_error_rate.set(unit, link_id, encoded_ber_val));
    SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.prev_cnt.set(unit, link_id, er_cnt));
    SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.prev_uncorrectable_cnt.set(unit,
                                                                                       link_id,
                                                                                       prev_uncorrectable_cnt));
exit:
    SHR_FUNC_EXIT;
 ...
 */
static void
dnx_port_signal_quality_fabric_bit_err_rate_set(
    int unit)
{
    bcm_port_t port;
    int iteration_interval;
    ...
    _SHR_PBMP_ITER(sfi_pbmp, port)
    {
        int link_id, has_speed;
        float error_rate;
        dnx_fabric_signal_quality_lane_state_t lane_state;
        uint64 er_cnt = 0, curr_er_cnt = 0;
        uint64 uncorrectable_er_cnt = 0, curr_uncorrectable_er_cnt = 0;
        uint32 error_rate_encoded = 0;

        /** Translate port to link id */
        SHR_IF_ERR_EXIT(dnx_algo_port_fabric_link_get(unit, port, &link_id));

        /** Skip ports without speed and set all values to zero */
        SHR_IF_ERR_EXIT(dnx_algo_port_has_speed(unit, port, 0, &has_speed));
        if (!has_speed)
        {
            SHR_IF_ERR_EXIT(dnx_port_signal_quality_fabric_ber_db_update
                            (unit, link_id, error_rate_encoded, curr_er_cnt, curr_uncorrectable_er_cnt));
            continue;
        }

        SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.get(unit, link_id, &lane_state));

        /** read current bit error value */
        SHR_IF_ERR_EXIT(soc_counter_get(unit, port, dnx_mib_counter_rx_rs_fec_bit_error, 0, &curr_er_cnt));

        /** check if uncorrectable FEC */
        SHR_IF_ERR_EXIT(soc_counter_get
                        (unit, port, dnx_mib_counter_rx_lfec_fec_uncorrrectable_errors, 0, &curr_uncorrectable_er_cnt));

        uncorrectable_er_cnt = curr_uncorrectable_er_cnt - lane_state.prev_uncorrectable_cnt;
        er_cnt = curr_er_cnt - lane_state.prev_cnt;

        /** calculate error rate*/
        SHR_IF_ERR_EXIT(dnx_port_signal_quality_fabric_ber_error_rate_get
                        (unit, port, iteration_interval, er_cnt, uncorrectable_er_cnt, &error_rate));

        /** encode error rate into mantissa and exponent */
        soc_dnxc_port_signal_quality_float_to_encoded_val_get(unit, error_rate, &error_rate_encoded);

        /** store current counters values and BER */
        SHR_IF_ERR_EXIT(dnx_port_signal_quality_fabric_ber_db_update
                        (unit, link_id, error_rate_encoded, curr_er_cnt, curr_uncorrectable_er_cnt));
    }
...
shr_error_e
dnx_port_signal_quality_fabric_ber_init(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (!dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_is_ber_calc_supported))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "BER calculation is not supported for current device \n");
    }

    /** Allocate SW-STATE to hold counters and BER data*/
    SHR_IF_ERR_EXIT(dnx_fabric_db.signal_quality.lane_state.alloc(unit));
...
shr_error_e
dnx_port_signal_quality_fabric_ber_register(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    /** add cb to counter thread*/
    SHR_IF_ERR_EXIT(soc_counter_extra_register(unit, dnx_port_signal_quality_fabric_bit_err_rate_set));
