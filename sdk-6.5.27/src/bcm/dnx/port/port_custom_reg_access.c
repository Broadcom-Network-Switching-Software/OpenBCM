    ...
    uint32 *data)
{
    ...
    else if (soc_is_reg_framer(unit, reg))
    {
        sal_memset(data_32, 0, sizeof(data_32));
        SHR_IF_ERR_EXIT(dnx_framer_reg_get(unit, reg, index, data_32));
        *data = data_32[0];
    }
    else
    {
    ...
    uint32 data)
{
    ...
    else if (soc_is_reg_framer(unit, reg))
    {
        sal_memset(data_32, 0, sizeof(data_32));
        data_32[0] = data;
        SHR_IF_ERR_EXIT(dnx_framer_reg_set(unit, reg, index, data_32));
    }
    else
    {
    ...
    soc_reg_above_64_val_t data)
{
    ...
    {
        sal_memset(data, 0, sizeof(soc_reg_above_64_val_t));
        SHR_IF_ERR_EXIT(dnx_framer_reg_get(unit, reg, index, data));
    }
    else
    {
    ...
    soc_reg_above_64_val_t data)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_framer_reg_set(unit, reg, index, data));
    }
    else
    {
...
{
    SHR_FUNC_INIT_VARS(unit);
    /** Currently, only FlexeCore(Q2A)/Framer(J2X) have custom memory */
    if (soc_is_mem_framer(unit, mem))
    {
        SHR_IF_ERR_EXIT(dnx_framer_mem_array_read(unit, 0, mem, copyno, array_index, index, entry_data));
    }
    else
    {
    ...
    void *entry_data)
{
    ...
    if (soc_is_mem_framer(unit, mem))
    {
        SHR_IF_ERR_EXIT(dnx_framer_mem_array_write(unit, 0, mem, copyno, array_index, index, entry_data));
    }
    else
    {
    ...
    const void *entry_data)
{
    ...
    if (soc_is_mem_framer(unit, mem))
    {
        SHR_IF_ERR_EXIT(dnx_framer_mem_array_fill_range
