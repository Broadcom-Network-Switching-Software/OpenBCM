...
#define DNX_PORT_ADD_SUPPORTED_FLAGS (BCM_PORT_ADD_BASE_Q_PAIR_WITH_ID | BCM_PORT_ADD_CGM_AUTO_ADJUST_DISABLE | \
                                      ...
                                      BCM_PORT_ADD_FLEXE_UNAWARE | BCM_PORT_ADD_FRAME_PREEMPTABLE)

#define DNX_PORT_ADD_IF_INFO_SUPPORTED_FLAGS  (BCM_PORT_ADD_FLEXE_PROTECTION | BCM_PORT_ADD_TX_FEEDBACK_ONLY)
    ...
    dnx_algo_port_info_indicator_t * indicator)
{
    ...
    if (flags & BCM_PORT_ADD_FLEXE_UNAWARE)
    {
        indicator->is_flexe_unaware = 1;
    }
    if (interface_info->flags & BCM_PORT_ADD_FLEXE_PROTECTION)
    {
        indicator->is_flexe_protection = 1;
    }
    ...
    {
        indicator->is_tdm_only_if = 1;
    }
    if (interface_info->flags & BCM_PORT_ADD_TX_FEEDBACK_ONLY)
    {
        indicator->is_feedback = 1;
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int is_ilkn_over_fabric)
{
     ...
     * 1. NIF ports (excluding ELK, STIF, MGMT and INKN over fabric)
    ...
    if (((!DNX_ALGO_PORT_TYPE_IS_NIF
          (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, FLEXE, OTN_PHY, FRAMER_MGMT, FEEDBACK)))
        ...
        || is_ilkn_over_fabric)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Only NIF port (excluding ELK / STIF/ ILKNoFabric / MGMT), RCY, mirror-recycle, FlexE client and SCH only can be dymamically added or removed. port=%d\n",
                     port);
    }
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_port_mapping_info_t * mapping_info)
{
    ...
    int is_ext_txpi_configurable = 0;
    bcm_pbmp_t pm_phys;
    ...
    SHR_MASK_VERIFY(interface_info->flags, DNX_PORT_ADD_IF_INFO_SUPPORTED_FLAGS, _SHR_E_PARAM, "unexpected flags.\n");
    ...
    /** Make sure TX feedback only is configured correctly */
    if (_SHR_IS_FLAG_SET(interface_info->flags, BCM_PORT_ADD_TX_FEEDBACK_ONLY))
    {
        if (!dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_tx_fdbk_port_supported))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Tx feedback only port is not supported on the device!\n");
        }
        if (interface_info->interface != BCM_PORT_IF_NIF_ETH)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Tx feedback only port can be only ETH port type!\n");
        }
        /** Validate the nof lanes */
        _SHR_PBMP_COUNT(interface_info->phy_pbmp, nof_lanes);
        if (nof_lanes != 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Only single lane is supported for feedback port!\n");
        }
    }
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FLEXE_PHY) || _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_CROSS_CONNECT)
        ...
        || _SHR_IS_FLAG_SET(interface_info->flags, BCM_PORT_ADD_FLEXE_PROTECTION)
        || _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FLEXE_UNAWARE)
        ...
        || (interface_info->interface == BCM_PORT_IF_VIRTUAL_FLEXE_CLIENT))
    {
        ...
        else
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "BCM_PORT_ADD_FLEXE_MAC/SAR/PROTECTION flag is supported only for BCM_PORT_IF_VIRTUAL_FLEXE_CLIENT interface type.\n");
            }
            /** Check if cross_connect flags is only set if the interface type is ETH/ILKN/BusB */
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_PORT_ADD_CROSS_CONNECT flag is supported for ETH/ILKN/FLEXE_MAC.\n");
            }
            /** Check if L1 ILKN is supported */
            if (dnx_data_nif.sar_client.feature_get(unit, dnx_data_nif_sar_client_is_tdm_port) &&
                (interface_info->interface == BCM_PORT_IF_ILKN) && _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_CROSS_CONNECT))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "BCM_PORT_ADD_CROSS_CONNECT flag is not supported for ILKN interface. The path between ILKN and SAR should be TDM path.\n");
            }
            /** make sure BCM_PORT_ADD_FLEXE_PROTECTION/BCM_PORT_ADD_STAT_ONLY should be use together with BCM_PORT_ADD_FLEXE_MAC */
            if ((_SHR_IS_FLAG_SET(interface_info->flags, BCM_PORT_ADD_FLEXE_PROTECTION)
                 || _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_STAT_ONLY))
                && !_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FLEXE_MAC))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "BCM_PORT_ADD_FLEXE_PROTECTION/BCM_PORT_ADD_STAT_ONLY should be used together with BCM_PORT_ADD_FLEXE_MAC.\n");
            }
            ...
            /** make sure that BCM_PORT_ADD_FLEXE_UNAWARE flag is set correctly */
            if (_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FLEXE_UNAWARE)
                && !dnx_data_nif.flexe.feature_get(unit, dnx_data_nif_flexe_unaware_support))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "The FlexE unaware interface can only be enabled when the device supports it.\n");
            }
            /** make sure flexe unaware flag should be used together with flexe phy flag */
            if (_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FLEXE_UNAWARE) && !_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FLEXE_PHY))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "BCM_PORT_ADD_FLEXE_UNAWARE flag should be used together with BCM_PORT_ADD_FLEXE_PHY.\n");
            }
            /** make sure that FlexE MAC L1 port can be enable only if the device supports it */
    ...
    {
        /** Common validation for L1 only mode */
        if (!DNX_ALGO_PORT_TYPE_IS_L1(unit, port_info) && !DNX_ALGO_PORT_TYPE_IS_FRAMER_PHY(unit, port_info) &&
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Only L1/ILKN_TDM/FRAMER_PHY ports are allowed in L1 only mode.\n");
        }
        /** Specific valation for different L1 only mode */
        if (dnx_data_nif.global.l1_only_mode_get(unit) == DNX_PORT_NIF_L1_ONLY_MODE_WITHOUT_ILKN)
        {
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN
                (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE_ELK | DNX_ALGO_PORT_TYPE_INCLUDE_L1))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "ILKN ports are not allowed in L1 only without ILKN mode.\n");
            }
            if (dnx_data_fabric.general.connect_mode_get(unit) == DNX_FABRIC_CONNECT_MODE_SINGLE_FAP)
            {
                /** For single FAP mode, SAR is not relevant, so the monitor client is also not relevant */
                if (DNX_ALGO_PORT_TYPE_IS_FRAMER_SAR(unit, port_info, TRUE) ||
                    DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC_STAT(unit, port_info))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "SAR and Framer MAC stat client are not allowed in L1 only without ILKN and single FAP mode.\n");
                }
            }
        }
        else
        {
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 0)
                && !_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_IF_TDM_ONLY))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Only ILKN TDM only ports are supported in L1 only with ILKN mode.\n");
            }
        }
    }
    ...
    if (is_init_done)
    {
        SHR_IF_ERR_EXIT(dnx_port_dynamic_verify(unit, port, port_info,
                                                _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_ILKN_OVER_FABRIC_PORT)));
    }
...

    {
        if (DNX_ALGO_PORT_TYPE_IS_IMB(unit, port_info, FALSE))
        {
            ...
            if (BCM_PBMP_IS_NULL(interface_info->phy_pbmp))
            {
                ...
                if (!dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_legacy_interfaces_support))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Port (%d), can't add interface type (%d). Legacy interfaces are no longer supported."
                                 "Instead, provide a valid lanes bitmap in phy_pbmp argument. \n", port,
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, MGMT, FEEDBACK))
            || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info, TRUE) || DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
        {
            ...
            {
                /*
                 * For OTN, TXPI is mandatory. So BCM_PORT_ADD_TXPI_MODE_EXT is not required.
                 */
                is_ext_txpi_configurable = dnx_data_nif.eth.pm_properties_get(unit, pm_index)->ext_txpi_support;
            }
            /*
         ...
         */
        if (!is_ext_txpi_configurable && _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_TXPI_MODE_EXT))
        {
    ...
    if (mapping_info->mgmt_channel != -1 && !DNX_ALGO_PORT_TYPE_IS_FRAMER_MGMT(unit, port_info))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Port %d - mgmt_channel can only be configured when the port type is framer management port.\n",
                     port);
    }
        ...
        || DNX_ALGO_PORT_TYPE_IS_FRAMER_MGMT(unit, port_info))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_flexe_channel_verify
                        (unit, port_info, mapping_info->channel, mapping_info->mgmt_channel));
    }
    ...
    if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info)
        && dnx_data_egr_queuing_v1.general.feature_get(unit, dnx_data_egr_queuing_v1_general_v1_supported))
    {
        int egress_priorities = mapping_info->num_priorities;
        int configuration_is_supported = 0;
        if (_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_CONFIG_CHANNELIZED))
        {
            configuration_is_supported =
                dnx_data_egr_queuing_v1.egress_queues.supported_nof_priorities_get(unit,
                                                                                   egress_priorities)->supported_for_channelized;
        }
        else
        {
            configuration_is_supported =
                dnx_data_egr_queuing_v1.egress_queues.supported_nof_priorities_get(unit,
                                                                                   egress_priorities)->supported_for_not_channelized;
        }

        if (!configuration_is_supported)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Port %d - cannot support %d priorities\n", port, egress_priorities);
        }
    }
    ...
    if (is_channelized)
    {
        ...
        /*
         * in_channel and out_channel can be different only for recycle ports on devices that don't use ibch1 header
         * to carry recycle metadata (such as ptc that is used to identify source context instead of in_channel)
         */
        if ((mapping_info->channel == -1 && mapping_info->in_channel != mapping_info->out_channel) &&
            (!DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info) ||
             dnx_data_headers.system_headers.feature_get(unit,
                                                         dnx_data_headers_system_headers_system_headers_ibch1_supported)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FRAME_PREEMPTABLE)
        && !dnx_data_tsn.general.feature_get(unit, dnx_data_tsn_general_is_supported))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Port %d - device does not support frame preemption\n", port);
    }

    if (_SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FRAME_PREEMPTABLE) && !DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Port %d - cannot enable frame preemption on non-NIF Ethernet ports\n", port);
    }

#ifdef ADAPTER_SERVER_MODE
    if (dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit) != 0)
    {
        if (mapping_info->core != dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Port %d, core (=%d). Invalid core id. For adapter core_swap mode, the core should be %d.\n",
                         port, mapping_info->core,
    ...
    uint32 flags)
{
   ...
   /** 
    ...
    SHR_IF_ERR_EXIT(dnx_port_dynamic_verify(unit, port, port_info, is_ilkn_over_fabric));

    if (DNX_ALGO_PORT_TYPE_IS_FRAMER(unit, port_info))
    {
        SHR_IF_ERR_EXIT(dnx_port_framer_port_remove_verify(unit, port));
    }
    /*
    ...
    bcm_port_mapping_info_t * mapping_info)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF
        (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT, OTN_PHY, FRAMER_MGMT, FEEDBACK)))
    {
        ...
        if_info.is_flr_fw_support = _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FLR_FIRMWARE_SUPPORT);
        if_info.ext_txpi_enable = _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_TXPI_MODE_EXT);
        if_info.eth_info.frame_preemptable = _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_FRAME_PREEMPTABLE);
        if_info.eth_info.framer_channel =
            _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_IF_FRAMER_MGMT) ? mapping_info->mgmt_channel : framer_channel;
        if_info.eth_info.is_flexe_unaware = indicator.is_flexe_unaware;
        if_info.eth_info.is_framer_mgmt = _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_IF_FRAMER_MGMT);
        if_info.eth_info.is_feedback = indicator.is_feedback;
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_TM(unit, port_info) || DNX_ALGO_PORT_TYPE_IS_FRAMER_CLIENT(unit, port_info, TRUE) ||
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_framer_mgmt_channel_add(unit, mapping_info->mgmt_channel, port_info));
    }
    /** Add to port per port type */
    if (DNX_ALGO_PORT_TYPE_IS_NIF
        (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT, OTN_PHY, FRAMER_MGMT, FEEDBACK)))
             ...
             DNX_ALGO_PORT_TYPE_IS_E2E_SCH(unit, port_info))
    {
        ...
        special_info.framer_info.is_flexe_protection = indicator.is_flexe_protection;
        special_info.framer_info.is_framer_mgmt = indicator.is_framer_mgmt;
        special_info.framer_info.mgmt_channel = mapping_info->mgmt_channel;
        special_info.tm_info.is_mc = indicator.is_mc;
    ...
    *flags = 0;
        ...
        (unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT, OTN_PHY, FRAMER_MGMT, FEEDBACK)))
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0))
        {
            ...
            {
                *flags |= BCM_PORT_ADD_FRAME_PREEMPTABLE;
            }
        }
    }
    ...
    /** Get Framer MGMT channel */
    if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MGMT(unit, port_info))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_framer_mgmt_channel_get(unit, port, (int *) &(mapping_info->mgmt_channel)));
    }
    /** is ELK port flag */
    if (indicator.is_elk)
    {
    ...
    if (indicator.is_flexe_protection)
    {
        interface_info->flags |= BCM_PORT_ADD_FLEXE_PROTECTION;
    }
    if (indicator.is_feedback)
    {
    ...
    bcm_port_mapping_info_t * mapping_info)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF
        ...
        && interface_info->interface != BCM_PORT_IF_NIF_ETH && interface_info->interface != BCM_PORT_IF_ILKN
        && interface_info->interface != BCM_PORT_IF_OTN)
    {
    ...
    const dnx_data_port_static_add_ucode_port_t * ucode_port)
{
    ...
    if (ucode_port->interface != BCM_PORT_IF_NULL)
    {
        ...
        mapping_info.mgmt_channel = ucode_port->framer_mgmt_channel;
        ...
        if (ucode_port->is_flexe_unaware)
        {
            flags |= BCM_PORT_ADD_FLEXE_UNAWARE;
        }
        ...
        if (ucode_port->frame_preemptable)
        {
            flags |= BCM_PORT_ADD_FRAME_PREEMPTABLE;
        }
        if (ucode_port->is_framer_mgmt)
        {
            flags |= BCM_PORT_ADD_IF_FRAMER_MGMT;
        }
        if (ucode_port->is_feedback)
        {
            if_info.flags |= BCM_PORT_ADD_TX_FEEDBACK_ONLY;
        }
        /** add the port */
        if (init_db_only)
        {
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, MGMT)))
    {
        ...
        /** Nothing to verify if ethu is not supporting MAC padding */
        ...
        if (dnx_data_nif.eth.mac_padding_get(unit, imb_type)->is_supported)
        {
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, MGMT))
        || DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info, FALSE))
    {
