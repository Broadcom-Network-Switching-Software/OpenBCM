    ...
    int enable)
{
    ...
    else
    {
         ...
         */
        dbal_entry_value_field_predefine_value_set(unit, entry_handle_id, DBAL_FIELD_MAC_MODE_CONFIG, INST_SINGLE,
    ...
    int enable)
{
     ...
     * set value field
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_400GE_ENABLE, INST_SINGLE, enable);
    /*
     * commit value
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
int
imb_cdu_frame_preemption_enable_set(
    int unit,
    bcm_core_t core,
    int cdu_id,
    int enable)
...
*/
int
imb_cdu_internal_instru_traffic_pfc_deadlock_counters_ofr_reset(
    int unit,
    uint32 pm_index,
    ...
    uint32 nof_port_indexes)
{
    ...
    for (port_index_current = port_index_base; port_index_current < (port_index_base + nof_port_indexes);
         port_index_current++)
    {
        /** Take into reset */
...
*/
int
imb_cdu_internal_instru_synced_counters_per_record_ofr_get(
    int unit,
    uint32 port_index_base,
    uint32 nof_port_indexes,
    uint32 nof_tcs,
    ...
    uint64 counter_values[])
{
    ...
    uint32 port_index_current = 0;
    int counter_index;
    uint8 remaining_tcs_to_return = 0;
    int counters_per_tc = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /** Set the number of TCs for which to return counters */
    remaining_tcs_to_return = nof_tcs;

    /** Calculate the number of counters per TC */
    counters_per_tc = nof_counters / dnx_data_instru.synced_counters.nof_tcs_per_port_get(unit);

    /** Iterate over the port's indexes, starting at base index */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_INST_SYNCED_COUNTERS_RECORDS_NIF_PFC_OFR, &entry_handle_id));
    for (port_index_current = port_index_base; port_index_current < (port_index_base + nof_port_indexes);
         port_index_current++)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PFC_PORT_IDX, port_index_current);
        ...
        /** For each of the port's indexes, iterate over the counters */
        for (counter_index = 0; counter_index < nof_counters; counter_index++)
        {
            dbal_value_field64_request(unit, entry_handle_id, dbal_fields[counter_index], INST_SINGLE,
                                       &counter_values[counter_index +
                                                       ((port_index_current - port_index_base) * nof_counters)]);

            /** When we returned all counters for a TC, lower the numbed of remaining TCs to return */
            if (((counter_index + 1) % counters_per_tc) == 0)
            {
                /** Lower the number of remaining TCs, all counters for the current TC have been returned. */
                remaining_tcs_to_return--;
            }

            /** If no more TCs need to be returned, break */
            if (remaining_tcs_to_return == 0)
            {
                break;
            }
        }
 ...
 */
int
imb_cdu_hw_linkscan_link_mux_clear(
    int unit,
    bcm_core_t core,
    uint32 cdu_id,
    int lane_id_cdu)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_CDU_LINK_MUX_CTRL, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CDU, cdu_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LANE_IN_CDU, lane_id_cdu);
 ...
 */
int
imb_cdu_hw_linkscan_link_mux_set(
    int unit,
    bcm_core_t core,
    uint32 cdu_id,
    int lane_id_cdu,
    uint32 link_sel)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LINK_SEL, INST_SINGLE, link_sel);
    ...
    int *in_reset)
{
    ...
    /** set key fields */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CDU, ethu_id_in_core);
 ...
 */
int
imb_cdu_hw_per_lane_link_status_change_intr_mask_set(
    int unit,
    bcm_core_t core,
    ...
    uint32 mask)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                    (unit, DBAL_TABLE_NIF_CDU_LANE_LINK_STATUS_CHANGE_INTERRUPT_MASK, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, ethu_id_in_core);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LANE_IN_CDU, lane_id_cdu);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INTERRUPT_MASK, INST_SINGLE, ! !mask);
 ...
 */
int
imb_cdu_hw_per_cdu_instance_link_status_change_intr_mask_set(
    int unit,
    bcm_core_t core,
    uint32 ethu_id_in_core,
    uint32 mask)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                    (unit, DBAL_TABLE_NIF_CDU_GLOBAL_LINK_STATUS_CHANGE_INTERRUPT_MASK, &entry_handle_id));
