    ...
    uint32 flags)
{
    ...
    if (IMB_PORT_ADD_F_SKIP_SETTINGS_GET(flags) == FALSE)
    {
        ...
        else if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, L1, MGMT, FEEDBACK)))
        {
    ...
    uint8 reset)
{
    ...
    {
        SHR_IF_ERR_EXIT(imbm.imb.get(unit, imb_id, &imb));
        if (__imb__dispatch__[imb.type]->f_imb_instru_counters_reset_set != NULL)
        {
            SHR_IF_ERR_EXIT(__imb__dispatch__[imb.type]->f_imb_instru_counters_reset_set(unit, imb_id, reset));
        }
    }
exit:
    SHR_FUNC_EXIT;
