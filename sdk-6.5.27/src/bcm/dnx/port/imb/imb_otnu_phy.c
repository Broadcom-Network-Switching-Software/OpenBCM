...
static int
imb_otnu_phy_port_mlane_member_reset_set(
    int unit,
    int port,
    uint32 flags,
    int nof_lanes,
...
{
    int i;
    uint64 mlane_rx_members;
    uint64 mlane_tx_members;
    uint32 entry_handle_id;
     ...
     * Get field value
     */
    dbal_value_field64_request(unit, entry_handle_id, DBAL_FIELD_MLANE_GROUP_MEMBERS_RX_RESET, INST_SINGLE,
                               &mlane_rx_members);
    dbal_value_field64_request(unit, entry_handle_id, DBAL_FIELD_MLANE_GROUP_MEMBERS_TX_RESET, INST_SINGLE,
                               &mlane_tx_members);
    /*
     * Get the value
     */
    ...
    if (flags & IMB_COMMON_DIRECTION_RX)
    {
        for (i = 0; i < nof_lanes; i++)
        {
            if (out_reset)
            {
                COMPILER_64_BITSET(mlane_rx_members, lanes[i]);
            }
            else
            {
                COMPILER_64_BITCLR(mlane_rx_members, lanes[i]);
            }
        }
        dbal_entry_value_field64_set(unit, entry_handle_id, DBAL_FIELD_MLANE_GROUP_MEMBERS_RX_RESET, INST_SINGLE,
                                     mlane_rx_members);
    }
    if (flags & IMB_COMMON_DIRECTION_TX)
    {
        for (i = 0; i < nof_lanes; i++)
        {
            if (out_reset)
            {
                COMPILER_64_BITSET(mlane_tx_members, lanes[i]);
            }
            else
            {
                COMPILER_64_BITCLR(mlane_tx_members, lanes[i]);
            }
        }
        dbal_entry_value_field64_set(unit, entry_handle_id, DBAL_FIELD_MLANE_GROUP_MEMBERS_TX_RESET, INST_SINGLE,
                                     mlane_tx_members);
    }
    /*
     * commit the value
...
static int
imb_otnu_phy_port_mlane_reset_set(
    int unit,
    int port,
    uint32 flags,
    int in_reset)
{
    int nof_otn_lanes, mlane_enabled = 0;
    ...
    SHR_IF_ERR_EXIT(imb_otnu_phy_port_mlane_group_enable_set(unit, port, nof_otn_lanes, otn_lanes, 1, &mlane_enabled));
    if (mlane_enabled)
    {
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_mlane_member_reset_set
                        (unit, port, flags, nof_otn_lanes, otn_lanes, in_reset));
        if (out_reset)
        {
            sal_usleep(1000);
        }
...
/**
 * \brief - Wether inverting the synth clock
 *    holdover signal in Framer
 *
 * \param [in] unit - chip unit id
 * \param [in] core - core ID
 * \param [in] lane_id - lane ID
 * \param [in] invert - Invert or not invert
 *
 * \return
 *   int - see _SHR_E_*
 *
 ...
 */
static int
imb_otnu_phy_port_synth_clock_holdover_invert_set(
    int unit,
    bcm_core_t core,
    int lane_id,
    int invert)
{
     ...
     * alloc DBAL table handle
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_FRAMER_SYNTH_CLOCK_HOLDOVER_INVERT, &entry_handle_id));
    /*
     * Set Key
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LANE_IN_CORE, lane_id);
    /*
     * set value fields
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INVERT, INST_SINGLE, invert);
    /*
     * commit the value
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 *    Linkscan. The Link single is from CDR lock per
 *    physical lane
 */
static int
imb_otnu_phy_port_link_status_mux_set(
    int unit,
    bcm_port_t port)
{
    int lane_id, map_size;
    bcm_pbmp_t phys;
    ...
    _SHR_PBMP_ITER(phys, lane_id)
    {
        ...
        SHR_IF_ERR_EXIT(imb_cdu_hw_linkscan_link_mux_set(unit, ethu_info.core, ethu_info.ethu_id_in_core,
                                                         lane_id - ethu_info.first_lane_in_ethu,
                                                         lane2serdes[lane_id].rx_id - ethu_info.first_lane_in_ethu));
    }
exit:
    SHR_FUNC_EXIT;
}
...
{
    int core_id, lane_index;

    SHR_FUNC_INIT_VARS(unit);
    /*
     * Invert the holdover signal from Framer
     */
    for (core_id = 0; core_id < dnx_data_device.general.nof_cores_get(unit); ++core_id)
    {
        for (lane_index = 0; lane_index < dnx_data_nif.phys.max_phys_in_core_get(unit); ++lane_index)
        {
            SHR_IF_ERR_EXIT(imb_otnu_phy_port_synth_clock_holdover_invert_set(unit, core_id, lane_index, 1));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    uint32 flags)
{
    ...
    {
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_link_status_mux_set(unit, port));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_flr_fw_support_get(unit, port, &is_flr_fw_support));
    /*
     * Get xpmd enable flags
     */
    if (is_xpmd_enabled)
    {
        xpmd_flags = IMB_COMMON_DIRECTION_RX;
        /*
         * For non-OTN interface, only Rx is relevant for FLR
         */
        if (DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
        {
            xpmd_flags |= IMB_COMMON_DIRECTION_TX;
        }
    }
    if (!enable)
    {
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_reset_set(unit, port, IMB_COMMON_IN_RESET));
        if (is_xpmd_enabled && !is_flr_fw_support)
        {
            SHR_IF_ERR_EXIT(imb_cdu_port_xpmd_lane_enable(unit, port, xpmd_flags, 0));
        }
    }
    /*
    ...
    if (enable)
    {
        if (is_xpmd_enabled)
        {
            SHR_IF_ERR_EXIT(imb_cdu_port_xpmd_lane_enable(unit, port, xpmd_flags, 1));
        }
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_reset_set(unit, port, IMB_COMMON_IN_RESET));
    ...
    int *link)
{
    ...
    *link = 0;
    ...
    BCM_PBMP_ITER(nif_phys, lane)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_port_phy_control_get(unit, lane_port, BCM_PORT_PHY_CONTROL_RX_SEQ_DONE, &lane_seq_done));
        rx_seq_done |= lane_seq_done << phy_count;
        phy_count++;
    }

    if (rx_seq_done == ((1 << phy_count) - 1))
    {
        *link = 1;
    }
    ...
    int link)
{
    ...
    if (link)
    {
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_mlane_reset_set(unit, port, IMB_COMMON_DIRECTION_RX, IMB_COMMON_IN_RESET));
        sal_usleep(1000);
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_mlane_reset_set
                        (unit, port, IMB_COMMON_DIRECTION_RX, IMB_COMMON_OUT_OF_RESET));
    }
    ...
    int *loopback)
{
    ...
    *loopback = 0;
    ...
    {
        if (portmod_loopback != portmodLoopbackPhyGloopPMD && portmod_loopback != portmodLoopbackPhyRloopPMD)
        {
            continue;
        }
    ...
    int nof_lanes)
{
    ...
    nof_lanes_per_group = dnx_data_nif.otn.nof_lanes_per_group_get(unit);
    lane_speed = DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info) ? speed : (speed / nof_phys);
 ...
 * \param [in] lane_id - logical lane in the port
    ...
    const bcm_port_flr_link_config_t * config)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_get(unit, port, 0, &phys));
    _SHR_PBMP_ITER(phys, tmp_lane_id)
    {
        if (tmp_lane_id >= dnx_data_nif.phys.nof_phys_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "port %d: The lane ID is out of bound! \n ", port);
        }
        if ((lane_id != -1) && (lane2serdes[tmp_lane_id].rx_id != lane2serdes[lane_id].rx_id))
        {
            continue;
        }
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_flr_link_mask_set
                        (unit, 0, lane2serdes[tmp_lane_id].rx_id, config->link_mask));
        SHR_IF_ERR_EXIT(imb_otnu_phy_port_flr_link_timer_set
                        (unit, 0, lane2serdes[tmp_lane_id].rx_id, config->link_stable_timer));
    }
exit:
    SHR_FUNC_EXIT;
}
