...
const int imb_ile_verbose_link_status_change = 0;
...
/**
 * \brief
 *   Rollback for portmod_ilkn_block_phys_set
 */
static shr_error_e
imb_ile_port_portmod_ilkn_block_phys_set_er_rollback(
    int unit,
...
{
    int *enable = NULL;
    struct ilkn_block_phys_set_s
    {
        int ilkn_core;
        portmod_pbmp_t ilkn_phys;
    }  *ilkn_block_phys = NULL;

    SHR_FUNC_INIT_VARS(unit);

    enable = (int *) metadata;
    ilkn_block_phys = (struct ilkn_block_phys_set_s *) payload;

    SHR_IF_ERR_EXIT(portmod_ilkn_block_phys_set
                    (unit, ilkn_block_phys->ilkn_core, ilkn_block_phys->ilkn_phys, *enable ? 0 : 1));
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief
 *   Insert portmod_ilkn_block_phys_set journal to generic ER database
 */
static shr_error_e
imb_ile_port_portmod_ilkn_block_phys_set_er_journal_insert(
    int unit,
    int ilkn_core,
    portmod_pbmp_t ilkn_phys,
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_log_entry(unit,
                                                        (uint8 *) &enable,
                                                        sizeof(int),
                                                        (uint8 *) &ilkn_block_phys,
                                                        sizeof(ilkn_block_phys),
                                                        &imb_ile_port_portmod_ilkn_block_phys_set_er_rollback, FALSE));
    ...
    int enable)
{
    ...
    for (fsrd_block = 0; fsrd_block < nof_fsrd; fsrd_block++)
    {
        ...
        {
            bcm_pbmp_t fmac_bmp;
            SHR_IF_ERR_EXIT(dnx_fabric_if_fsrd_fmac_bmp_get(unit, fsrd_block, &fmac_bmp));
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_fsrd_block_power_set(unit, fmac_bmp, enable));
    ...
    int enable)
{
    ...
    /** note: to make L1 -  SAR protocol work, TX and RX segments should be enabled/disabled together */
    SHR_IF_ERR_EXIT(imb_ile_port_segments_enable_set(unit, port, nof_segments, enable));
...
imb_ile_port_attach(
    ...
    uint32 flags)
{
    uint8 is_xpmd_enabled, ilkn_aligner_supported, ilkn_over_fabric_only;
    portmod_port_add_info_t add_info;
    ...
    /*
     * Get whether this port is master port or channelized
     */
    ...
    if (is_master_port)
    {
        ...
        /*
         * Check if bypass is supported for this port
         */
        ...
        ilkn_over_fabric_only = dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_ilkn_over_fabric_only);

        /*
         * If bypass supported - disable Rx&Tx bypass
         */
        if ((ilkn_aligner_supported && !is_xpmd_enabled) || (!ilkn_aligner_supported && !ilkn_over_fabric_only))
        {
        ...
        /*
         * Disable Rx&Tx wrapper
         */
        SHR_IF_ERR_EXIT(imb_ile_port_wrapper_enable_set
                        (unit, port, IMB_COMMON_DIRECTION_RX | IMB_COMMON_DIRECTION_TX, 0));
        ...
        if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_is_ilu_supported) &&
            dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_is_elk_supported))
        {
        ...
        if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_frmr_ilkn_selector))
        {
            SHR_IF_ERR_EXIT(imb_ile_port_frmr_ilkn_select_set(unit, port, TRUE));
            sal_usleep(10000);
        }
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN_ELK(unit, port_info))
        {
            ...
            SHR_IF_ERR_EXIT(imb_ile_port_elk_rx_reset_set(unit, port, IMB_COMMON_OUT_OF_RESET));
        }
        ...
        if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_ensure_tx_rx_reset_order))
        {
            SHR_IF_ERR_EXIT(imb_ile_port_lock_status_interrupt_enable_set(unit, port, 1));
        }
    }
    ...
    if (is_master_port)
    {
        PORTMOD_PBMP_ASSIGN(ilkn_phys_tmp, add_info.phys);
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(portmod_ilkn_block_phys_set(unit, ilkn_info.ilkn_core, ilkn_phys_tmp, 1));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
        }
        SHR_IF_ERR_EXIT(imb_ile_port_portmod_ilkn_block_phys_set_er_journal_insert
                        (unit, ilkn_info.ilkn_core, ilkn_phys_tmp, 1));
    }
    ...
    {
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(portmod_port_enable_set(unit, port, 0, 0));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
    ...
    bcm_port_t port)
{
    ...
    if (nof_channels == 1)
    {
        ...
        /*
         * 3. Disable lane lock status change interrupt
         */
        if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_ensure_tx_rx_reset_order))
        {
            SHR_IF_ERR_EXIT(imb_ile_port_lock_status_interrupt_enable_set(unit, port, 0));
        }

        /*
         * 4. Remove the PM mapping if port is master
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_get(unit, port, 0, &port_phys));
        PORTMOD_PBMP_CLEAR(ilkn_phys_tmp);
        BCM_PBMP_ITER(port_phys, phy)
        {
            PORTMOD_PBMP_PORT_ADD(ilkn_phys_tmp, phy);
        }
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(portmod_ilkn_block_phys_set(unit, ilkn_info.ilkn_core, ilkn_phys_tmp, 0));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
        ...
        SHR_IF_ERR_EXIT(imb_ile_port_portmod_ilkn_block_phys_set_er_journal_insert
                        (unit, ilkn_info.ilkn_core, ilkn_phys_tmp, 0));
        ...
        {
            /*
             *  5. Disable fabric SerDes for ILKN over fabric
             */
            SHR_IF_ERR_EXIT(imb_ile_port_fsrd_enable(unit, port, 0));
        }

        /*
         * 6. Reset CLU/CDU ilkn Data MUX
         */
        if ((dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_cdu_ilkn_selectors))
            || (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_clu_ilkn_selectors)))
        {
        ...
        /*
         * 7. Unselect ILKN in framer
         */
        if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_frmr_ilkn_selector))
        {
            sal_usleep(10000);
            SHR_IF_ERR_EXIT(imb_ile_port_frmr_ilkn_select_set(unit, port, FALSE));
        }
    ...
    int enable)
{
    ...
    ilkn_core_info = dnx_data_nif.ilkn.ilkn_cores_get(unit, ilkn_access_info.ilkn_core);

    /*
     * Enable/Disable ILKN RX lanes if there's fec dependency
     */
    if (ilkn_core_info->fec_dependency)
    {
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(portmod_ilkn_port_lane_enable(unit, port, IMB_COMMON_DIRECTION_RX, enable));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
    ...
    if (!enable)
    {
        ...
        if (is_xpmd_enabled)
        {
            SHR_IF_ERR_EXIT(imb_ile_port_xpmd_lane_enable
                            (unit, port, IMB_COMMON_DIRECTION_RX | IMB_COMMON_DIRECTION_TX, 0));
        }
        if (is_xpmd_enabled && ilkn_aligner_supported)
        {
            ...
            SHR_IF_ERR_EXIT(imb_ile_port_async_fifo_reset
    ...
    if (enable)
    {
        if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_ilkn_over_fabric_only))
        {
            sal_usleep(9000);
        }
...
imb_ile_port_enable_get(
    ...
    int *enable)
{
    int phy_enable = 0, core_enable = 0;
    SHR_FUNC_INIT_VARS(unit);

    /*
     * Get the indication from Portmod
     */
    SHR_IF_ERR_EXIT(portmod_port_enable_get(unit, port, 0, &phy_enable));
    ...
    int loopback)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_port_enable_set(unit, port, IMB_PORT_ENABLE_F_SKIP_FEC, 0));
    ...
    if (enable)
    {
        SHR_IF_ERR_EXIT(imb_port_enable_set(unit, port, IMB_PORT_ENABLE_F_SKIP_FEC, 1));
    }
exit:
    SHR_FUNC_EXIT;
    ...
    uint64 *counter_val)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_in_channel_get(unit, port, &in_channel));
    SHR_IF_ERR_EXIT(dnx_algo_port_out_channel_get(unit, port, &out_channel));

    switch (counter_type)
    {
        case dnx_mib_counter_ilkn_rx_pkt:
            SHR_IF_ERR_EXIT(imb_ile_port_ilkn_rx_counter_get_prepared_by_channel(unit, port, in_channel));
        ...
        case dnx_mib_counter_ilkn_tx_pkt:
            SHR_IF_ERR_EXIT(imb_ile_port_ilkn_tx_counter_get_prepared_by_channel(unit, port, out_channel));
            /** get the relevant counter*/
        ...
        case dnx_mib_counter_nif_rx_stats_discards:
            SHR_IF_ERR_EXIT(imb_ilu_burst_drop_count_get(unit, port, counter_val));
            break;
        default:
    ...
    bcm_port_t port)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id_rx, DBAL_FIELD_ADDR, INST_SINGLE, in_channel);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id_tx, DBAL_FIELD_ADDR, INST_SINGLE, out_channel);
    ...
    bcm_port_resource_t * resource)
{
    ...
    if (resource->phy_lane_config == BCM_PORT_RESOURCE_DEFAULT_REQUEST)
    {
        ...
        BCM_PORT_RESOURCE_PHY_LANE_CONFIG_MEDIUM_SET(resource->phy_lane_config,
                                                     _SHR_PORT_RESOURCE_PHY_LANE_CONFIG_MEDIUM_BACKPLANE);
 ...
 * \param [in] nof_lanes - array size of lane_order
 *
 * \return
 *   int - see _SHR_E_*
 *
...
{
    int lane_id;
    int pre_dev_lane_id, dev_lane_id;
    int is_over_fabric;
    int map_size;
    ...
    for (lane_id = 0; lane_id < nof_lanes; lane_id++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_lane_map_ilkn_lane_info_get
                        (unit, port, lane_map_type, lanes[lane_id], &ilkn_lane_info));
        ilkn_lane_id[lane_id] = ilkn_lane_info.ilkn_id;
        ...
        /*
         * Check the ilkn lane id must be pair becasue FEC block are connected two adjacent lane 
         */
        if (!DNX_PORT_ADDR_IS_PAIRS_MATCH(ilkn_lane_id[lane_id - 1], ilkn_lane_id[lane_id]))
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "port %d: on the ILKN core level mapping, the lanes should be in pairs!\n",
                         port);
        }

        pre_dev_lane_id = lanes[lane_id - 1];
        dev_lane_id = lanes[lane_id];
        if (pre_dev_lane_id >= map_size || dev_lane_id >= map_size)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "port:%d map_size(%d) must be higher than lane_id(%d and %d)",
                         port, map_size, pre_dev_lane_id, dev_lane_id);
        }
        /*
         * Check the outgoing PHY lane swap (PMD address)
         */
        if (!DNX_PORT_ADDR_IS_PAIRS_NATUNAL_MATCH(lane2serdes[pre_dev_lane_id].tx_id, lane2serdes[dev_lane_id].tx_id) ||
            !DNX_PORT_ADDR_IS_PAIRS_MATCH(lane2serdes[pre_dev_lane_id].rx_id, lane2serdes[dev_lane_id].rx_id))
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
    ...
    int speed)
{
    ...
    int phy_id, phy_in_core, idx, core, nof_phys;
    bcm_pbmp_t phys;
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
    int lanes[DNX_DATA_MAX_NIF_ILKN_LANES_MAX_NOF];
    dnx_algo_port_ilkn_access_info_t ilkn_info;
    ...
    nof_phys_per_core = (is_over_fabric) ? dnx_data_fabric.links.nof_links_per_core_get(unit) : nof_phys;
    ...
    if (fec_type != bcmPortPhyFecNone)
    {
        ...
        if (!dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_12lanes_mode))
        {
            BCM_PBMP_ITER(phys, phy_id)
            {
                ...
                if (phy_in_core >=
                    dnx_data_nif.ilkn.fec_units_per_core_nof_get(unit) *
                    dnx_data_nif.ilkn.nof_lanes_per_fec_unit_get(unit))
                {
        ...
        SHR_IF_ERR_EXIT(bcm_dnx_port_ilkn_lane_map_get(unit, 0, port, nof_lanes, lanes));
        /*
         * Check the PHY lane swap (PMD address) also considering ILKN lane Id mapping
         */
        SHR_IF_ERR_EXIT(imb_ile_port_fec_pmd_laneswap_check(unit, port, lanes, nof_lanes));
    ...
    int lane_order_size)
{
    ...
    {
        SHR_IF_ERR_EXIT(imb_ile_port_fec_pmd_laneswap_check(unit, port, lanes, lane_order_size));
    }
    ...
    switch (type)
    {
        ...
        case BCM_PORT_PHY_CONTROL_TX_PPM_ADJUST:
        {
            if ((pm_type == portmodDispatchTypePm8x50)
                && !dnx_data_nif.phys.feature_get(unit, dnx_data_nif_phys_txpi_supported_in_pm8x50))
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "This control is not supported on PM8x50.\n");
            }
            break;
        }
...
/**
 * \brief - reset ELK RX buffers
 */
int
imb_ile_port_rx_buffers_restart(
    int unit,
    bcm_port_t port)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(imb_ile_port_elk_rx_reset_set(unit, port, IMB_COMMON_IN_RESET));
    SHR_IF_ERR_EXIT(imb_ile_port_elk_rx_reset_set(unit, port, IMB_COMMON_OUT_OF_RESET));
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief - check whether there is a PM lock on all of the port's lanes
 *
 * \param [in] unit - chip unit id.
 * \param [in] port - logical port
 * \param [out] is_all_locked - TRUE if all lanes are locked, FALSE otherwise
 *
 * \return
 *   int - see _SHR_E_*
 *
...
static int
imb_ile_port_pm_lock_status_check(
    int unit,
    bcm_port_t port,
    int *is_all_locked)
{
    int lane_in_port, lane;
    bcm_pbmp_t nif_phys;
    ...
    /*
     * Check whether all lanes in port are locked
     */
    lane_in_port = 0;
    *is_all_locked = TRUE;
    ...
    BCM_PBMP_ITER(nif_phys, lane)
    {
         ...
         */
        BCM_PHY_GPORT_LANE_PORT_SET(lane_port, lane_in_port, port);
...
/**
 * \brief - handler for PM lock status change
 *          If the locked/unlocked status is stable on all port lanes,
 *          enable/disable RX wrapper and bypass (respectively)
 *
 * \param [in]  unit             - chip unit id.
 * \param [in]  port             - logical port
 *
...
imb_ile_port_pm_lock_status_change_event(
    ...
    bcm_port_t port)
{
    int is_all_locked = FALSE;
    uint32 live_status = FALSE;
    SHR_FUNC_INIT_VARS(unit);

    /*
     * Get PM lock status for all lanes
     */
    SHR_IF_ERR_EXIT(imb_ile_port_pm_lock_status_check(unit, port, &is_all_locked));
    if (!is_all_locked)
    {
        SHR_EXIT();
    }
    ...
    /*
     * If alignment wasn't achieved but all lanes are locked, it might be that Rx was enabled before peer's Tx.
     * Therefore, to handle this scenario, we toggle the port's Rx.
     */
    if (!live_status)
    {
        /*
         * Disable port Rx in wrapper
         */
        SHR_IF_ERR_EXIT(imb_ile_port_wrapper_enable_set(unit, port, IMB_COMMON_DIRECTION_RX, 0));
        /*
         * Disable port Rx bypass
         */
        SHR_IF_ERR_EXIT(imb_ile_port_bypass_if_enable_set(unit, port, IMB_COMMON_DIRECTION_RX, 0));

        /*
         * Enable Rx bypass interface
         */
        SHR_IF_ERR_EXIT(imb_ile_port_bypass_if_enable_set(unit, port, IMB_COMMON_DIRECTION_RX, 1));

        /*
         * Enable port Rx in wrapper
         */
        SHR_IF_ERR_EXIT(imb_ile_port_wrapper_enable_set(unit, port, IMB_COMMON_DIRECTION_RX, 1));
    }
    ...
    bcm_port_t port)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_ile_port_alignment_live_status_get(unit, port, &live_status));
    if (imb_ile_verbose_link_status_change)
    {
        LOG_WARN(BSL_LOG_MODULE, (BSL_META_U(unit, "time %u usec: port %d link status=%d\n"),
                                  sal_time_usecs(), port, live_status));
    }
    ...
    /*
     * Wait to check alignment stability
     */
    sal_usleep(dnx_data_nif.ilkn.link_stable_wait_get(unit));
    ...
    if (is_alignment_stable)
    {
        if (imb_ile_verbose_link_status_change)
        {
            LOG_WARN(BSL_LOG_MODULE, (BSL_META_U(unit, "time %u usec: port %d link stable\n"), sal_time_usecs(), port));
        }
        /*
         * Get port type
         */
        ...
        /** Reset RX buffers (HRF in case of ILU)  to clean up the buffers from the junk*/
        SHR_IF_ERR_EXIT(imb_port_rx_buffers_restart(unit, port));
    ...
    int enable)
{
    ...
    bypass_flags = (type == bcmPortControlPrbsRxEnable ? IMB_COMMON_DIRECTION_RX : IMB_COMMON_DIRECTION_TX);
    ...
    int power_down)
{
    ...
    if (ilkn_access_info.pm_imb_type == imbDispatchTypeImb_cdu)
    {
        ...
        {
            /*
             * we want to power down the unit only if there are no other ILKN ports on the same unit
             */
            if (power_down)
            {
                SHR_IF_ERR_EXIT(imb_ile_port_is_only_ilkn_port_on_cdu_get(unit, port, ethu_id, &is_only_port_on_ethu));
                if (!is_only_port_on_ethu)
                {
                    continue;
                }
            }
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_global_to_local_and_core_get
                            (unit, ethu_id, &core, &ethu_id_in_core));
            SHR_IF_ERR_EXIT(imb_cdu_ilkn_logic_power_down_set(unit, core, ethu_id_in_core, power_down));
        }
    }
    ...
    int fec_enable)
{
    ...
    SHR_IF_ERR_EXIT(imb_ile_internal_fec_global_enable_set(unit, ilkn_core, fec_enable));
