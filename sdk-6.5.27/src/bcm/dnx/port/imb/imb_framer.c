...
/**
 * \brief - Disable and flush egq. If global MAC
 *    loopback is enabled, need to set MAC loopback
 *    before flushing egq.
 *
 * \param [in] unit - chip unit id
 * \param [in] port - logical port
 ...
 */
static int
imb_framer_port_egr_queuing_disable_and_flush(
    int unit,
    bcm_port_t port)
{
    int is_loopback_en = 0, is_global_loopback_en = 0;
    bcm_port_t logical_port;
    bcm_pbmp_t mac_clients;

    SHR_FUNC_INIT_VARS(unit);

    if (dnx_data_nif.mac_client.feature_get(unit, dnx_data_nif_mac_client_is_loopback_supported))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_framer_mac_loopback_enable_get(unit, port, &is_loopback_en));
        /*
         * Check if loopback is enabled on the other ports
         */
        if (!is_loopback_en)
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                            (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_MAC,
                             DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_L1 |
                             DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FLEXE_MAC_STAT_ONLY |
                             DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FLEXE_MAC_PROTECTION, &mac_clients));
            _SHR_PBMP_ITER(mac_clients, logical_port)
            {
                SHR_IF_ERR_EXIT(dnx_algo_port_framer_mac_loopback_enable_get(unit, logical_port, &is_loopback_en));
                if (is_loopback_en)
                {
                    is_global_loopback_en = 1;
                    break;
                }
            }
        }
    }
    if (is_global_loopback_en)
    {
        /*
         * The TinyMAC loopback is a global loopback.
         * If global loopback is enabled and the port is not in loopback calendar,
         * there is no credit sent to aribter. In order to flush egq, need to
         * enable the MAC loopback to enable the credit.
         */
        SHR_IF_ERR_EXIT(imb_framer_port_loopback_set(unit, port, 1));
    }
    /*
     * Disable EGQ and flush
     */
    ...
    if (is_global_loopback_en)
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_framer_port_loopback_set(unit, port, 0));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int enable)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, TRUE, TRUE, TRUE))
    {
        ...
        else
        {
            ...
            {
                SHR_IF_ERR_EXIT(imb_framer_port_egr_queuing_disable_and_flush(unit, port));
            }
            if (IMB_FRAMER_FEU_IS_VALID(unit))
            {
                /*
 ...
 * \param [in] port - logical port
 * \param [in] speed - speed
 * \param [in] is_ghao - if speed change is for G.hao
 *
 * \return
 *   int - see _SHR_E_*
 *
...
imb_framer_port_mac_speed_set(
    ...
    int is_ghao)
{
    uint32 dummy, orig_numb_credit, val[IMB_FRAMER_MAC_SPEED_NOF_FIELDS];
    dbal_fields_e field[IMB_FRAMER_MAC_SPEED_NOF_FIELDS] = { DBAL_FIELD_SPEED_GROUP, DBAL_FIELD_NUMB_CREDIT };

    SHR_FUNC_INIT_VARS(unit);
    /*
     * Get original numb_credit
     */
    SHR_IF_ERR_EXIT(imb_framer_mac_get
                    (unit, port, DBAL_TABLE_TINY_MAC_CTRL, DBAL_FIELD_NUMB_CREDIT, &orig_numb_credit));
    /*
     * Get numb_credit and speed group according to new speed
     */
    SHR_IF_ERR_EXIT(imb_framer_utils_mac_tx_speed_config_get(unit, speed, &val[0], &val[1], &dummy));
    /*
     * Check if there is speed change
     */
    if (val[1] == orig_numb_credit)
    {
        SHR_EXIT();
    }
    ...
    if (is_ghao)
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_framer_mac_polling
                        (unit, -1, DBAL_TABLE_TINY_MAC_TXFIFO_STATUS, DBAL_FIELD_IS_SWITCH_DONE, 1));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    const portmod_speed_config_t * speed_config)
{
    ...
    if (dnx_data_nif.mac_client.feature_get(unit, dnx_data_nif_mac_client_is_speed_supported))
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_framer_port_mac_speed_set(unit, port, speed_config->speed, 0));
        /*
    ...
    int speed)
{
     ...
     * Program the "speed group" register (FLEXMAC2_CTRL.NUMB_CREDIT)
     */
    SHR_IF_ERR_EXIT(imb_framer_port_mac_speed_set(unit, port, speed, 1));
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief - Configure uniform calendar for SB RX/TX
 *
 * \param [in] unit - chip unit id
 * \param [in] flags - Indicate calendar actions
 * \param [in] calendar_type - Indicate which calendar (Rx/Tx) will be set
 *
 * \return
 *   int - see _SHR_E_*
 *
 ...
 */
static shr_error_e
imb_framer_sb_shared_arch_calendar_set(
    int unit,
    uint32 flags,
    dnx_port_nif_calendar_type_e calendar_type)
{
    ...
    int cal_to_config = -1;
    imb_framer_calendar_type_e imb_calendar_type;
    SHR_FUNC_INIT_VARS(unit);

    imb_calendar_type =
        (calendar_type == DNX_PORT_NIF_FRAMER_SB_RX_CALENDAR) ? IMB_FRAMER_SB_RX_CALENDAR : IMB_FRAMER_SB_TX_CALENDAR;
    ...
    if (IMB_FLEXE_RESOURCE_PREPARE_GET(flags))
    {
        ...
        SHR_IF_ERR_EXIT(imb_framer_internal_shadow_calendar_config
                        (unit, imb_calendar_type, calendar_length, calendar, &cal_to_config));
    }
    ...
    {
        SHR_IF_ERR_EXIT(imb_framer_internal_calendar_switch(unit, imb_calendar_type, cal_to_config));
    }
    ...
    uint32 flags)
{
    ...
    if (IMB_FLEXE_DIRECTION_RX_GET(flags))
    {
        ...
        {
            SHR_IF_ERR_EXIT(imb_framer_sb_shared_arch_calendar_set(unit, flags, DNX_PORT_NIF_FRAMER_SB_RX_CALENDAR));
        }
        else
        {
    ...
    if (IMB_FLEXE_DIRECTION_TX_GET(flags))
    {
        ...
        if (dnx_data_nif.global.feature_get(unit, dnx_data_nif_global_is_shared_arch))
        {
            SHR_IF_ERR_EXIT(imb_framer_sb_shared_arch_calendar_set(unit, flags, DNX_PORT_NIF_FRAMER_SB_TX_CALENDAR));
        }
        else
        {
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_FRAMER_MAC,
                     DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_REMOVED_PORT, &all_tmac_ports));
    /*
    ...
    if (total_slot > 0)
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_framer_mac_set
                        (unit, -1, DBAL_TABLE_TINY_MAC_COMMON_CTRL, DBAL_FIELD_LOCAL_LPBK_WRAP_PTR,
                         calendar_length - 1));
        /*
