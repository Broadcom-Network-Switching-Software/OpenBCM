    ...
    int enable)
{
    ...
    if (enable)
    {
        ...
        if (IMB_ETHU_IS_RX_RMC_PORT(unit, port_info) || (tdm_mode == DNX_ALGO_PORT_TDM_MODE_BYPASS))
        {
            /*
             * take the port out of reset in ETHU RX
             */
        ...
        for (prio_group = 0; prio_group < nof_prio_groups_supported; ++prio_group)
        {
            ...
            if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_port_ofr_rmc_machine_id_internal_get
                                (unit, port, rmc.sch_priority, is_over_high, &rmc_machine_id));
                SHR_IF_ERR_EXIT(dnx_port_link_list_db_double_priority_allocated_list_info_get
                                (unit, DNX_PORT_NIF_OFR_LINK_LIST, nif_interface_id, rmc_machine_id, &list_head,
                                 &rmc_fifo_size));
        ...
        if (rmc_info.rmc_id != -1)
        {
                    ...
                    BCM_COSQ_THRESHOLD_ETH_PORT_HIGH_FIFO))))
            {
                if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_qmlf_rmcs_thresholds_per_profile_support))
                {
                    ...
                    SHR_IF_ERR_EXIT(flow_control_db.
                                    rmc_thresholds.exchange(unit, 0, 1, &rmc_thresholds, profile_old, NULL,
                                                            &profile_new, &is_first, &is_last));
    ...
    int enable_mode)
{
    ...
    if (enable_mode == bcmCosqIngressPortDropDisable)
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_ethu_port_priority_config_default_get(unit, port, rmc_arr, &nof_rmc));
        SHR_IF_ERR_EXIT(imb_port_rx_rmc_priority_config_set(unit, port, rmc_arr, nof_rmc));
    }
    else
    {
        /*
