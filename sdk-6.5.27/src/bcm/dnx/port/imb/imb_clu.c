    ...
    portmod_pm_create_info_t * pm_info)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_soc_to_phymod_ref_clk(unit, ref_clk, &(pm_info->pm_specific_info.pm4x25.ref_clk)));
    ...
    SHR_IF_ERR_EXIT(soc_physim_check_sim
                    (unit, sim_type, &(pm_info->pm_specific_info.pm4x25.access), (imb_info->inst_id << 16), 1,
                     &is_sim));
    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    ethu_info.core = core;
    ethu_info.ethu_id_in_core = ethu_id;

    SHR_ALLOC_ERR_EXIT(user_acc, nof_pms_in_clu * sizeof(portmod_default_user_access_t), "CLU user access", "%s%s%s",
                       "user_acc", EMPTY, EMPTY);
    ...
    portmod_port_add_info_t * add_info)
{
    ...
    SHR_ALLOC_ERR_EXIT(lane2serdes,
                       (nof_lanes_in_pm * sizeof(soc_dnxc_lane_map_db_map_t)),
                       "lane2serdes", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    /*
    ...
    int enable)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_ethu_access_info_get(unit, port, &ethu_info));
    SHR_IF_ERR_EXIT(imb_clu_rx_pm_port_mode_hw_set(unit, ethu_info.imb_type_id, ethu_info.pm_id_in_ethu, mode, enable));
    ...
    bcm_port_t port)
{
    ...
    SHR_IF_ERR_CONT(portmod_port_mode_set(unit, port, &port_mode_info));
    ...
    uint32 flags)
{
    ...
    if (is_master_port)
    {
        ...
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(portmod_port_pfc_control_set(unit, port, &pfc_control));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
    ...
    bcm_port_resource_t * resource)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(imb_clu_port_resource_default_get_verify(unit, port, flags, resource));
    ...
    /*
     * Get the FLR firmware status. FLR firmware doesn't support Link traning
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_flr_fw_support_get(unit, port, &is_flr_fw_support));
    /*
    ...
    const bcm_port_speed_ability_t * abilities)
{
    ...
    SHR_IF_ERR_EXIT(imb_clu_mgu_port_autoneg_ability_verify(unit, port, num_ability, abilities));
 ...
 */
int
imb_clu_pm_reset_get(
    int unit,
    int ethu_id,
    int *in_reset)
{
    int clu_id, pm_i, in_reset_current_pm_state;
    SHR_FUNC_INIT_VARS(unit);

    clu_id = dnx_data_nif.eth.ethu_properties_get(unit, ethu_id)->type_index;

    for (pm_i = 0; pm_i < dnx_data_nif.eth.nof_pms_in_clu_get(unit); pm_i++)
    {
        in_reset_current_pm_state = 0;
        SHR_IF_ERR_EXIT(imb_clu_internal_pm_reset_get(unit, clu_id, pm_i, &in_reset_current_pm_state));
        *in_reset |= in_reset_current_pm_state;
    }
