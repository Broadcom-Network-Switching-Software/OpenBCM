 ...
 * 
 *   Note: 
 *   This result of this function is used in two scenarios:
 *      1. When MDIO is used for HW access instead of LPI.
 *      2. ALWAYS used for PHYSIM since address is needed for creating PHYSIM instances per PM.
 */
int
    ...
    int *local_fault)
{
    ...
    int link = 0;
    uint32 err_token_count, fec_err;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        /*
         * In hardware we can read the local fault from the Link Integrity register, but it is slow because of a Watchdog Timer, which makes it
         * unsuitable for the Linkscan indication (which should be fast). This is why, instead, we read the local fault as:
         * Local Fault = MAC Leaky bucket indication && FEC (Loss of sync) signal
         */
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_NOF_ERRORED_TOKENS, INST_SINGLE, &err_token_count);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MISALIGN, INST_SINGLE, &fec_err);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

        if (fec_err || (err_token_count < 63))
        {
            *local_fault = 1;
        }
    }
    ...
    bcm_port_t port)
{
    ...
    /**
     * The linkscan thread clears the local/remote fault after reading it. The local fault on fabric ports is based on 
     * MAC Leaky bucket indication && FEC (Loss of sync) signal
     * However the leaky bucket is read-only, so we only clear the FEC interrupt 
     */
    ...
    bcm_port_t port)
{
    ...
    /**
     * The SFI ports remote fault is a read-only signal and can't be cleared.
     * The linkscan thread clears the local/remote fault after reading it, so this empty function exists in order to return BCM_E_NONE
     */
...
/**
 * \brief
 *   Get fabric link status (up or down)
 *
 * \param [in] unit - chip unit id
 * \param [in] port - logical port
    ...
    int *link)
{
    ...
    {
        /** Convert logical port to fabric link */
        SHR_IF_ERR_EXIT(dnx_algo_port_fabric_link_get(unit, port, &link_idx));

        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FABRIC_IF_LINK_STATUS, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FABRIC_LINK, link_idx);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_LINK_STATUS, INST_SINGLE, (uint32 *) link);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    }
    else
    {
        SHR_IF_ERR_EXIT(portmod_port_link_get(unit, port, flags, link));
    }
