 ...
 */
int
imb_ile_port_is_only_ilkn_port_on_cdu_get(
    int unit,
    bcm_port_t port,
    int cdu_id,
    int *is_only_port_on_ethu)
{
    ...
    if (ilkn_access_info.pm_imb_type != imbDispatchTypeImb_cdu &&
        ilkn_access_info.pm_imb_type != imbDispatchTypeImb_cdu_shr)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "This function is only meant for CDUs (single-PM ETHU)\n");
    }
    ...
    pm_index = dnx_data_nif.eth.ethu_properties_get(unit, cdu_id)->pms[0];
    phys = dnx_data_nif.eth.pm_properties_get(unit, pm_index)->phys;

    BCM_PBMP_AND(phys, other_ilkn_phys);
    BCM_PBMP_COUNT(phys, phys_in_cdu_count);
    *is_only_port_on_ethu = (phys_in_cdu_count == 0) ? TRUE : FALSE;
...
/**
 * \brief - Configure the PM lock status mux to receive interrupt when lock status changes
 */
int
imb_ile_port_lock_status_interrupt_enable_set(
    int unit,
    bcm_port_t port,
    int enable)
{
    ...
    /*
     * iterate over all lanes in logical port
     */
    _SHR_PBMP_ITER(ilkn_phys, iter_phy)
    {

        SHR_IF_ERR_EXIT(dnx_algo_port_ilkn_lane_info_get
                        (unit, ethu_info.core, iter_phy, &ethu_info.ethu_id_in_core, &lane));
        BCM_PBMP_PORT_ADD(cdu_bmp, ethu_info.ethu_id);
        ...
        {
            /*
             * When the N instance of the mux is set to M, the interrupt
             * CDPM_LINK_STATUS_CHANGE_LINK_STATUS_CHANGE_INTERRUPT_REGISTER_N
             * is triggered by lock status change of lane M. Setting the mux
             * value to the lane number causes the interrupt index to correspond to the lane index.
             */
            SHR_IF_ERR_EXIT(imb_cdu_hw_linkscan_link_mux_set
                            (unit, ethu_info.core, ethu_info.ethu_id_in_core, lane, lane));
        }
        else
        {
            SHR_IF_ERR_EXIT(imb_cdu_hw_linkscan_link_mux_clear(unit, ethu_info.core, ethu_info.ethu_id_in_core, lane));
        }

        SHR_IF_ERR_EXIT(imb_cdu_hw_per_lane_link_status_change_intr_mask_set(unit, ethu_info.core,
                                                                             ethu_info.ethu_id_in_core, lane, enable));
    }

    _SHR_PBMP_ITER(cdu_bmp, global_ethu_id)
    {
        SHR_IF_ERR_EXIT(imb_ile_port_is_only_ilkn_port_on_cdu_get(unit, port, global_ethu_id, &is_only_port_on_ethu));

        if (enable || is_only_port_on_ethu)
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_global_to_local_and_core_get
                            (unit, global_ethu_id, &core, &ethu_id_in_core));
            SHR_IF_ERR_EXIT(imb_cdu_hw_per_cdu_instance_link_status_change_intr_mask_set
                            (unit, core, ethu_id_in_core, enable));
        }
    }
