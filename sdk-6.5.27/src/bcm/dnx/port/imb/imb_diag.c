...
#define IMB_DIAG_RX_CLOCK_SIGNAL_LINK             (1)
#define IMB_DIAG_RX_CLOCK_SIGNAL_LINK_AND_VALID   (2)
    ...
    imb_diag_synce_config_t * synce)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
         ...
         * set value fields
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COUNTER1_CLK_SEL, INST_SINGLE,
                                     synce->counter_clk_sel);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CAPTURE_MODE, INST_SINGLE, synce->capture_mode);
        ...
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        /*
         * cdpm gate enable=0 and valid select
         */
    ...
    imb_diag_synce_config_t * synce)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CAPTURE_MODE, INST_SINGLE, &synce->capture_mode);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_COUNTER1_CLK_SEL, INST_SINGLE,
                                   &synce->counter_clk_sel);
        /*
         * commit request
         */
    ...
    uint32 *synce_cnt)
{
    ...
    {
        table_id = DBAL_TABLE_SYNCE_COUNTER;
    }
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
         ...
         * set value request
         */
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_COUNTER1, INST_SINGLE, synce_cnt);
    }
    else
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SYNCE_INDEX, 0);
    ...
    soc_dnxc_port_phy_measure_t * phy_measure)
{
    ...
    if (!dnx_data_nif.eth.pm_properties_get(unit, pm_index)->measure_rate_on_down_port_supported)
    {
    ...
    soc_dnxc_port_phy_measure_t * phy_measure)
{
    ...
    SHR_IF_ERR_EXIT(phymod_phy_speed_config_get(&phy_access[0], &phy_speed_config));
    /*
    ...
    soc_dnxc_port_phy_measure_t * phy_measure)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        ...
        synce_config.link_valid_sel =
            DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit,
                                          port_info) ? IMB_DIAG_RX_CLOCK_SIGNAL_LINK :
            IMB_DIAG_RX_CLOCK_SIGNAL_LINK_AND_VALID;
        synce_config.clk_chain_sel = 1;
        synce_config.capture_mode = 1;
    ...
    /*
     * Since the divider is big when txpi enable, in order to measure more accurate,
     * multi 50 times
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ext_txpi_enable_get(unit, port, &is_ext_txpi_en));
    sample_period = is_ext_txpi_en ? 20 * IMB_DIAG_GTIMER_NOF_CYCLES : IMB_DIAG_GTIMER_NOF_CYCLES;

    SHR_IF_ERR_EXIT(dnx_gtimer_set(unit, sample_period, block_type, instance));
    /*
     * Wait Gtimer until it reset
     */
    SHR_IF_ERR_EXIT(dnx_gtimer_wait(unit, block_type, instance));
    ...
    tot_time_measured_int = (((clock_speed_int * 10000) + clock_speed_remainder) * sample_period) / 10000;
    tot_time_measured_remainder =
        ((int) (((clock_speed_int * 10000) + clock_speed_remainder) * (sample_period)) % 10000);
    /*
     * Read syncE counter
     */
    SHR_IF_ERR_EXIT(imb_diag_synce_counter_get(unit, ethu_info.core, is_mgu, ethu_info.ethu_id, &synce_cnt));
    /*
...
exit:
    ...
    {
        if ((_func_rv = dnx_gtimer_clear(unit, block_type, instance)) < 0)
        {
