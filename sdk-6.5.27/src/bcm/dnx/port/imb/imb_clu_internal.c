    ...
    int enable)
{
    ...
    if (enable)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PM_PORTS_CONFIG, INST_SINGLE, mac_mode);
    }
    else
    {
        /*
         ...
         */
        dbal_entry_value_field_predefine_value_set(unit, entry_handle_id, DBAL_FIELD_PM_PORTS_CONFIG, INST_SINGLE,
    ...
    int *in_reset)
{
    ...
    /** set key fields */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CLU, clu_id);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PM_IN_CLU, pm_in_clu);

    /** request for value field */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_PM_RESET, INST_SINGLE, &in_reset_dbal);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CLU_PM_LANES_RESET, INST_SINGLE, &in_lanes_reset_dbal);

    /** commit value */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    *in_reset = ((in_reset_dbal == DBAL_ENUM_FVAL_NIF_RESET_REVERSE_LOGIC_IN_RESET)
                 || (in_lanes_reset_dbal == DBAL_ENUM_FVAL_NIF_CLU_PM_RESET_REVERSE_LOGIC_IN_RESET)) ? 1 : 0;
