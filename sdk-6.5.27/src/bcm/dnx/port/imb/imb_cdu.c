    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        ethu_info.core = core;
        ethu_info.ethu_id_in_core = cdu_id;
        /*
         * Disable data to Portmacro
         */
        SHR_IF_ERR_EXIT(imb_ethu_tx_data_disable_traffic(unit, &ethu_info));

        /*
         * Refclk termination SW correction
         */
        if (dnx_data_nif.global.feature_get(unit, dnx_data_nif_global_bh_refclk_termination_inverted))
        {
            if (dnx_data_nif.eth.pm_properties_get(unit, cdu_id)->is_master)
            {
                SHR_IF_ERR_EXIT(imb_cdu_refclk_termination_set(unit, core, cdu_id, TRUE));
            }
        }
        ...
        {
            /** Power down the unit */
            SHR_IF_ERR_EXIT(imb_ethu_power_down_set(unit, &ethu_info, TRUE));
        }
         ...
         */
        SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_set(unit, &ethu_info, bcmPortNifSchedulerLow, 0));
        SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_set(unit, &ethu_info, bcmPortNifSchedulerHigh, 0));
        if (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_supported))
        {
            SHR_IF_ERR_EXIT(imb_ethu_scheduler_config_set(unit, &ethu_info, bcmPortNifSchedulerTDM, 0));
        }
        ...
        if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_rmc_flush_support))
        {
             ...
             */
            SHR_IF_ERR_EXIT(imb_ethu_sch_cnt_dec_thres_hw_set(unit, &ethu_info, 0));
        }
        /*
        ...
        {
            SHR_IF_ERR_EXIT(imb_ethu_port_rmc_thr_after_ovf_hw_set(unit, &ethu_info, rmc, 0));
            if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_rmc_flush_support))
            {
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_rmc_flush_mode_enable_hw_set(unit, &ethu_info, rmc, 0));
            }
        }
        /*
         ...
         */
        SHR_IF_ERR_EXIT(imb_ethu_prd_bypass_hw_set(unit, &ethu_info, 0));
        ...
        {
            SHR_IF_ERR_EXIT(imb_ethu_tdm_mask_hw_set(unit, &ethu_info, 0));
        }
    ...
    portmod_port_add_info_t * add_info)
{
    ...
    /*
     * Check if need to enable FLR firmware
     */
    ...
    portmod_pm_create_info_t * pm_info)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_soc_to_phymod_ref_clk(unit, ref_clk, &(pm_info->pm_specific_info.pm8x50.ref_clk)));
    ...
    uint32 flags)
{
    ...
    if (is_master_port)
    {
        ...
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(portmod_port_rx_control_set(unit, port, &rx_ctrl));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
        ...
        SHR_IF_ERR_EXIT(imb_cdu_port_cdmac_valid_enable(unit, port, 1));
    }
    ...
    if (dnx_data_nif.framer.feature_get(unit, dnx_data_nif_framer_is_supported))
    {
        ...
        SHR_IF_ERR_EXIT(imb_ethu_port_credit_mask_set
                        (unit, port, (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, FALSE) ? 1 : 0)));
        /** Configure average IPG for L1 port */
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, FALSE))
        {
            average_ipg_in_bits = dnx_data_nif.mac_client.average_ipg_for_l1_get(unit);
            DNX_ERR_RECOVERY_SUPPRESS(unit);
            SHR_IF_ERR_CONT(portmod_port_tx_average_ipg_set(unit, port, average_ipg_in_bits));
            DNX_ERR_RECOVERY_UNSUPPRESS(unit);
            if (SHR_FUNC_ERR())
            {
                SHR_EXIT();
    ...
    {
        /** enable the MACSec core clk */
        SHR_IF_ERR_EXIT(dnx_xflow_macsec_control_power_down_set(unit, port, DNX_MACSEC_POWER_DOWN_CORE, FALSE));
    }
#endif
    /*
     * 2. Configure the SCH
    ...
    bcm_port_t port)
{
    ...
    DNX_ERR_RECOVERY_EXPR_SUPPRESS_IF_ERR_EXIT(imb_port_autoneg_get(unit, port, 0, &an));
    if (an.enable)
    {
        an.enable = 0;
        DNX_ERR_RECOVERY_EXPR_SUPPRESS_IF_ERR_EXIT(imb_port_autoneg_set(unit, port, 0, &an));
    }
    /*
     ...
     */
    SHR_IF_ERR_EXIT(imb_ethu_mac_nof_ports_get(unit, port, &mac_nof_ports));
    /*
     * get number of ports on the MAC mode config
     */
    SHR_IF_ERR_EXIT(imb_ethu_mac_mode_config_nof_ports_get(unit, port, &mac_mode_config_nof_ports));
    /*
    ...
    if (nof_channels == 1)
    {
        ...
        SHR_IF_ERR_EXIT(imb_cdu_port_cdmac_valid_enable(unit, port, 0));
    }
...
#ifdef INCLUDE_XFLOW_MACSEC
    /** disable the MACSec core clk */
    if (dnx_data_macsec.general.feature_get(unit, dnx_data_macsec_general_macsec_block_exists))
    {
        /** disable the MACSec core clk ONLY in case this is the last port in the entire MACSec instance. */
        SHR_IF_ERR_EXIT(dnx_xflow_macsec_control_power_down_set(unit, port, DNX_MACSEC_POWER_DOWN_CORE, TRUE));
    }
#endif
exit:
    SHR_FUNC_EXIT;
    ...
    int *actual_num_ability)
{
    ...
    *actual_num_ability = 0;
    ...
    {
        SHR_IF_ERR_EXIT(imb_ethu_port_fec_speed_is_valid_get(unit, pm_type, raw_abilities[i].num_lanes,
                                                             (bcm_port_phy_fec_t) raw_abilities[i].fec_type,
                                                             raw_abilities[i].speed, TRUE, &is_valid));
        if (is_valid)
        {
    ...
    bcm_port_resource_t * resource)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    if ((resource->flags != 0) && !DNX_ALGO_PORT_TYPE_IS_NIF_ETH_L1(unit, port_info, TRUE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "port %d: flags fields of bcm_port_resource_t are not in use. It should all be initialized to 0",
                     port);
    }
    if ((resource->physical_port != 0) || (resource->encap != 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "port %d: physical_port and encap fields of bcm_port_resource_t are not in use. They should all be initialized to 0",
                     port);
    }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "port %d: Link training and FLR cannot be enabled together. \r\n", port);
    }
    /*
    ...
    if (resource->phy_lane_config == BCM_PORT_RESOURCE_DEFAULT_REQUEST)
    {
         ...
         * FORCE_ES must be cleared if FLR firmware is downloaded
         */
    ...
    {
        /*
         * Force ES cannot be enabled if FLR firmware is downloaded
         */
        if (BCM_PORT_RESOURCE_PHY_LANE_CONFIG_FORCE_ES_GET(resource->phy_lane_config) && is_flr_fw_support)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "port %d: Force ES and FLR cannot be enabled together. \r\n", port);
        }
    }
    resource->port = port;
    ...
    bcm_pbmp_t port_to_add_phys)
{
    ...
    {
        SHR_IF_ERR_EXIT(imb_ethu_port_mac_mode_calc
    ...
    soc_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(imb_cdu_portmod_soft_reset(unit, port, portmodCallBackActionTypeLock));
    ...
    soc_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(imb_cdu_portmod_soft_reset(unit, port, portmodCallBackActionTypeUnlock));
 ...
 * \param [out] in_reset - port number internal for the ETHU ID
 * \return
 *   int - see _SHR_E_ *
 *
 * \remarks
    ...
    int *in_reset)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ethu_global_to_local_and_core_get(unit, ethu_id, &ethu_core, &ethu_id_in_core));

    SHR_IF_ERR_EXIT(imb_cdu_internal_pm_reset_get(unit, ethu_core, ethu_id_in_core, in_reset));
