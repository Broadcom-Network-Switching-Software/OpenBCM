...
static int imb_ilu_rx_sch_config_get(
    int unit,
    dnx_algo_port_ilkn_access_info_t * ilkn_access_info,
    imb_ilu_sch_config_t * sch_config);
    ...
    uint32 ilkn_core_id)
{
    ...
    {
        /*
         * Clear priority configuration per ILKN port
         */
        SHR_IF_ERR_EXIT(imb_ilu_port_nmg_sch_config_hw_init(unit, ilkn_core_id));
    }
    else
    {
        /*
         * Clear priority configuration  per ILKN core
         */
    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    for (ilkn_iter = 0; ilkn_iter < dnx_data_nif.ilkn.ilkn_unit_if_nof_get(unit); ilkn_iter++)
    {
        ...
        if (!sw_state_is_warm_boot(unit) && !dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
            /*
             * Init all schedulers in the ilkn core (Do nothing for OFR scheduler because it is initialized in OFR code)
             */
            SHR_IF_ERR_EXIT(imb_ilu_nmg_sch_config_init(unit, imb_info->inst_id));
        }
    }
    ...
    pm_info.pm_specific_info.os_ilkn.fec_dependency = ilkn_core_info->fec_dependency;
...
/**
 * \brief
 *   Take ILU Rx in NMG out of reset.
 */
static int
imb_ilu_rx_nmg_sch_reset_out_of_reset(
    int unit,
    bcm_port_t port)
{
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
    ...
    else
    {
        ...
        if (is_tdm_or_l1)
        {
            ...
            /*
             * Take NMG Rx TDM counter out of reset
             */
 ...
 */
static int
imb_ilu_rx_nmg_sch_reset_in_reset(
    int unit,
    bcm_port_t port)
{
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
...
/**
 * \brief
 *   Take NMG ILU Rx scheduler into reset / out of reset
 */
static int
imb_ilu_rx_nmg_sch_reset(
    int unit,
    bcm_port_t port,
    int in_reset)
{
    ...
    {
        SHR_IF_ERR_EXIT(imb_ilu_rx_nmg_sch_reset_in_reset(unit, port));
    }
    else
    {
        SHR_IF_ERR_EXIT(imb_ilu_rx_nmg_sch_reset_out_of_reset(unit, port));
    }
...
/**
 * \brief
 *   Take OFR ILU Rx scheduler into reset / out of reset
 */
static int
imb_ilu_rx_ofr_sch_reset(
    int unit,
    bcm_port_t port,
    int in_reset)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_tdm_get(unit, port, &tdm_mode));
    is_tdm = (tdm_mode == DNX_ALGO_PORT_TDM_MODE_BYPASS);
    ...
    if (in_reset == IMB_COMMON_IN_RESET)
    {
        ...
        /*
         * Reset scheduler if no channels with same priority left on this interface
         */
        if (BCM_PBMP_IS_NULL(channels))
        {
            SHR_IF_ERR_EXIT(dnx_port_ofr_ilkn_rx_counter_reset_set(unit, ilkn_access_info.ilkn_id,
                                                                   is_tdm, IMB_COMMON_IN_RESET));
        }
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_port_ofr_ilkn_rx_counter_reset_set(unit, ilkn_access_info.ilkn_id,
                                                               is_tdm, IMB_COMMON_OUT_OF_RESET));
    }
...
/**
 * \brief
 *   Take ILU Rx scheduler into reset / out of reset
 */
static int
imb_ilu_rx_sch_reset(
    int unit,
    bcm_port_t port,
    int in_reset)
{
    ...
    {
        SHR_IF_ERR_EXIT(imb_ilu_rx_ofr_sch_reset(unit, port, in_reset));
    }
    else
    {
        SHR_IF_ERR_EXIT(imb_ilu_rx_nmg_sch_reset(unit, port, in_reset));
    }
    ...
    bcm_port_t port)
{
    ...
    /*
     * At this point, in case of master port, we assume that ilkn data port uses half of 
     * number of segments. The exact number of segments will be later determined by the 
     * portmod according to number of lanes and serdes rate. Then extended memory mode will
     * be determined, and all configuration will be set accordingly.
     * In case of not master port, setting the number of segments based on the master port.
     */
    if (is_master_port)
    {
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_ilkn_nof_segments_get(unit, master_port, &nof_segments));
    }
    ...
    /*
     * Put ILU Rx scheduler out of reset
     */
    SHR_IF_ERR_EXIT(imb_ilu_rx_sch_reset(unit, port, IMB_COMMON_OUT_OF_RESET));
    /*
    ...
    bcm_port_t port)
{
    ...
    /*
     * Put ILU Rx scheduler into reset
     */
    SHR_IF_ERR_EXIT(imb_ilu_rx_sch_reset(unit, port, IMB_COMMON_IN_RESET));
    /*
    ...
    const portmod_speed_config_t * speed_config)
{
    ...
    if (nof_segments != current_nof_segments)
    {
        ...
        SHR_IF_ERR_EXIT(imb_ile_port_segments_enable_set(unit, port, current_nof_segments, FALSE));
        SHR_IF_ERR_EXIT(imb_ile_port_segments_enable_set(unit, port, nof_segments, TRUE));
    ...
    int enable)
{
    ...
    /*
     * Get data scheduler priority of the ILKN Interface to which the port is connected.
     */
    SHR_IF_ERR_EXIT(imb_ilu_rx_sch_config_get(unit, &ilkn_access_info, &sch_config));
    ...
    if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_scheduler_context_per_port_support))
    {
        ...
        /** Check if it is the last channel */
        SHR_IF_ERR_EXIT(dnx_algo_port_channels_nof_get(unit, port, &nof_channels));
        sch_config.is_tdm_priority_enabled = ((nof_channels == 1) && (enable == 0)) ? 0 : 1;
        ...
        if (enable)
        {
            ...
            if (sch_prio == bcmPortNifSchedulerLow)
            {
                ...
                /*
                 * Enable Low scheduler priority
                 */
                sch_config.is_low_priority_enabled = 1;
                sch_config.is_high_priority_enabled = 0;
            }
            else
            {
                /*
                ...
                /*
                 * Enable High scheduler priority
                 */
                sch_config.is_low_priority_enabled = 0;
                sch_config.is_high_priority_enabled = 1;
            }
        }
        else
        {
         ...
         */
        SHR_IF_ERR_EXIT(imb_common_dbal_sch_weight_get(unit, nof_weight_bits_low_sch, &sch_config.dbal_sch_weight_low));
        ...
        if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
            SHR_IF_ERR_EXIT(dnx_port_ofr_ilkn_rx_config_hw_set
                            (unit, port, sch_config.is_tdm_priority_enabled,
                             sch_config.is_high_priority_enabled, sch_config.is_low_priority_enabled));
        }

        else
        {
            SHR_IF_ERR_EXIT(imb_ilu_port_nmg_sch_config_hw_set
                            (unit, ilkn_access_info.ilkn_core, ilkn_access_info.port_in_core,
                             sch_config.is_tdm_priority_enabled, sch_config.is_high_priority_enabled,
                             sch_config.dbal_sch_weight_low));
        }
    ...
    else
    {
         ...
         */
        sch_config.is_high_priority_enabled = (nof_high_sch_prio_ports > 0) ? 1 : 0;
        /*
         ...
         * For TDM scheduler priorities we set maximum weight.
         */
        sch_config.is_tdm_priority_enabled = ((nof_high_sch_prio_ports + nof_low_sch_prio_ports == 0)
                                              && (enable == 0)) ? 0 : 1;
         ...
         */
        SHR_IF_ERR_EXIT(imb_ilu_core_scheduler_config_hw_set
                        (unit, ilkn_access_info.ilkn_core, sch_config.is_tdm_priority_enabled,
                         sch_config.is_high_priority_enabled, sch_config.dbal_sch_weight_low));
        ...
        if (sch_prio == bcmPortNifSchedulerLow)
        {
             ...
             * Convert number of weight bits to DBAL weight
             */
            SHR_IF_ERR_EXIT(imb_common_dbal_sch_weight_get
                            (unit, nof_weight_bits_low_sch, &sch_config.dbal_sch_weight_low));
    ...
    imb_ilu_sch_config_t * sch_config)
{
    ...
    if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        SHR_IF_ERR_EXIT(dnx_port_ofr_ilkn_rx_config_hw_get(unit, ilkn_access_info->ilkn_id,
                                                           &(sch_config->is_tdm_priority_enabled),
                                                           &(sch_config->is_high_priority_enabled),
                                                           &(sch_config->is_low_priority_enabled)));
    }
    else
    {
        if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_scheduler_context_per_port_support))
        {
            SHR_IF_ERR_EXIT(imb_ilu_port_nmg_sch_config_hw_get(unit, ilkn_access_info->ilkn_core,
...
/*
 * \brief - Disable write to HRF and wait until current write operation is finished
 */
static int
imb_ilu_hrf_ingoing_data_enable_set(
    int unit,
    dnx_algo_port_ilkn_access_info_t * ilkn_access_info,
    dbal_enum_value_field_ilkn_hrf_type_e ilkn_hrf_type,
    int is_enable)
{
     ...
     */
    SHR_IF_ERR_EXIT(imb_ilu_internal_port_rx_drop_data_hw_set(unit,
                                                              ilkn_access_info->ilkn_core,
    ...
    {
        /*
         * In case SOP entered the fifo, we need to wait until all the packet enters.
...
/**
 * \brief - Check if scheduler needs to be synced with HRF before HRF reset
 */
static int
imb_ilu_hrf_is_sync_with_sch_required_get(
    int unit,
    dnx_algo_port_ilkn_access_info_t * ilkn_access_info,
    dbal_enum_value_field_ilkn_hrf_type_e ilkn_hrf_type,
    int *is_sync_with_sch_required)
{
    ...
    *is_sync_with_sch_required = (counter > 0);
...
/**
 * \brief - configure rx scheduler
 */
static int
imb_ilu_rx_sch_config_set(
    int unit,
    bcm_port_t port,
    imb_ilu_sch_config_t * sch_config)
{
    ...
    if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_ofr_ilkn_rx_config_hw_set(unit, port,
                                                           sch_config->is_tdm_priority_enabled,
                                                           sch_config->is_high_priority_enabled,
                                                           sch_config->is_low_priority_enabled));

    }
    else if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_scheduler_context_per_port_support))
    {
        SHR_IF_ERR_EXIT(imb_ilu_port_nmg_sch_config_hw_set
                        (unit, ilkn_access_info.ilkn_core, ilkn_access_info.port_in_core,
                         sch_config->is_tdm_priority_enabled, sch_config->is_high_priority_enabled,
    ...
    {
        SHR_IF_ERR_EXIT(imb_ilu_core_scheduler_config_hw_set(unit, ilkn_access_info.ilkn_core,
                                                             sch_config->is_tdm_priority_enabled,
                                                             sch_config->is_high_priority_enabled,
                                                             sch_config->dbal_sch_weight_low));
    }
...
/**
 * \brief - Synchronize scheduler counter with HRF that is about to be reset
 */
static int
imb_ilu_hrf_outgoing_data_disable(
    int unit,
    bcm_port_t port,
    dbal_enum_value_field_ilkn_hrf_type_e ilkn_hrf_type,
    imb_ilu_sch_config_t * current_sch_config)
{
    imb_ilu_sch_config_t sch_config_copy;
    SHR_FUNC_INIT_VARS(unit);

    sal_memcpy(&sch_config_copy, current_sch_config, sizeof(imb_ilu_sch_config_t));

    if (ilkn_hrf_type == DBAL_ENUM_FVAL_ILKN_HRF_TYPE_TDM)
    {
        sch_config_copy.is_tdm_priority_enabled = FALSE;
    }
    else
    {
        sch_config_copy.is_high_priority_enabled = FALSE;
        sch_config_copy.is_low_priority_enabled = FALSE;
        sch_config_copy.dbal_sch_weight_low = DBAL_ENUM_FVAL_NIF_SCH_WEIGHT_WEIGHT_0;
    }

    SHR_IF_ERR_EXIT(imb_ilu_rx_sch_config_set(unit, port, &sch_config_copy));
...
/**
 * \brief - Synchronize Rx scheduler counter with HRF that is about to be reset
 */
static int
imb_ilu_hrf_sch_sync_enable_set(
    int unit,
    bcm_port_t port,
    dbal_enum_value_field_ilkn_hrf_type_e ilkn_hrf_type,
    dbal_enum_value_field_nif_sch_priority_e hrf_sch_priority,
    int is_start_sync)
{
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
    int ilkn_nof_ports;
    ...
    if (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_hrf_flush_support) && ilkn_nof_ports > 1)
    {
        ...
        if (is_start_sync)
        {
            ...
            SHR_IF_ERR_EXIT(imb_ilu_internal_port_rx_nmg_flush_context_enable_hw_set
                            (unit, ilkn_access_info.ilkn_core, hrf_sch_priority, TRUE));
        ...
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(imb_ilu_internal_port_rx_nmg_flush_context_enable_hw_set
                            (unit, ilkn_access_info.ilkn_core, hrf_sch_priority, FALSE));
        }

    }
    /** there is no additional port on this core - direct scheduler counter reset can be used */
    else
    {
        if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
        ...
        {
            SHR_IF_ERR_EXIT(imb_ilu_internal_nmg_rx_core_reset(unit, ilkn_access_info.ilkn_core,
                                                               hrf_sch_priority, is_start_sync));
        }
    }
    ...
    dbal_enum_value_field_ilkn_hrf_type_e ilkn_hrf_type)
{
    ...
    dbal_enum_value_field_nif_sch_priority_e hrf_sch_priority;
    imb_ilu_sch_config_t sch_config;
    int is_sync_with_sch_required = FALSE;
    int is_config_restore_required = FALSE;
    ...
    /*
     * Get HRF priority as DBAL enum value
     */
    if (ilkn_hrf_type == DBAL_ENUM_FVAL_ILKN_HRF_TYPE_TDM)
    {
        hrf_sch_priority = DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_TDM;
    }
    else
    {
        hrf_sch_priority = sch_config.is_high_priority_enabled ?
            DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_HIGH : DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_LOW;
    }

    /*
     * Disable writes to HRF
     */
    SHR_IF_ERR_EXIT(imb_ilu_hrf_ingoing_data_enable_set(unit, &ilkn_access_info, ilkn_hrf_type, FALSE));

    /*
     * Check if there are entries in HRF, if not - no need to sync with scheduler
     */
    SHR_IF_ERR_EXIT(imb_ilu_hrf_is_sync_with_sch_required_get(unit, &ilkn_access_info, ilkn_hrf_type,
                                                              &is_sync_with_sch_required));

    if (is_sync_with_sch_required)
    {
        is_config_restore_required = TRUE;

        /*
         * Disable reads from HRF
         */
        SHR_IF_ERR_EXIT(imb_ilu_hrf_outgoing_data_disable(unit, port, ilkn_hrf_type, &sch_config));

        /*
         * Ensure scheduler will stay synced with HRF after HRF is reset
         */
        SHR_IF_ERR_EXIT(imb_ilu_hrf_sch_sync_enable_set(unit, port, ilkn_hrf_type, hrf_sch_priority, TRUE));
     ...
     * Put HRF into reset
     */
    SHR_IF_ERR_EXIT(imb_ilu_hrf_controller_rx_hw_set(unit, ilkn_access_info.ilkn_core, ilkn_access_info.port_in_core,
                                                     ilkn_hrf_type, IMB_COMMON_IN_RESET));

    if (is_sync_with_sch_required)
    {
        /*
         * Disable sync with scheduler
         */
        SHR_IF_ERR_EXIT(imb_ilu_hrf_sch_sync_enable_set(unit, port, ilkn_hrf_type, hrf_sch_priority, FALSE));

        /*
         * Restore original scheduler configuration
         */
        SHR_IF_ERR_EXIT(imb_ilu_rx_sch_config_set(unit, port, &sch_config));
    }

    /*
     * Re-enable writes to HRF
     */
    SHR_IF_ERR_EXIT(imb_ilu_hrf_ingoing_data_enable_set(unit, &ilkn_access_info, ilkn_hrf_type, TRUE));

exit:
    if (is_config_restore_required && SHR_GET_CURRENT_ERR() != BCM_E_NONE)
    {
        SHR_IF_ERR_EXIT(imb_ilu_rx_sch_config_set(unit, port, &sch_config));
    }
    SHR_FUNC_EXIT;
    ...
    bcm_port_t port)
{
    ...
    for (ilkn_hrf_type = DBAL_ENUM_FVAL_ILKN_HRF_TYPE_DATA; ilkn_hrf_type < DBAL_NOF_ENUM_ILKN_HRF_TYPE_VALUES;
         ++ilkn_hrf_type)
    {
        SHR_IF_ERR_EXIT(imb_ilu_hrf_controller_rx_hrf_flush_and_reset(unit, port, ilkn_hrf_type));
    }
...
/**
 * \brief - reset ILU RX buffers -- HRF controller + HRF segmentation
 */
int
    ...
    bcm_port_t port)
{
    ...
    /** Flush (if available) and reset the Rx HRF controller */
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_ilkn_ports_in_core_get(unit, port, &ilkn_ports));
    BCM_PBMP_COUNT(ilkn_ports, nof_ports_on_core);
    is_complete = (nof_ports_on_core == 1);
    SHR_IF_ERR_EXIT(imb_ilu_hrf_rx_segmentation_reset(unit, port, is_complete, IMB_COMMON_IN_RESET));

    /*
     * OUT OF RESET 
     */
    SHR_IF_ERR_EXIT(imb_ilu_hrf_rx_segmentation_reset(unit, port, is_complete, IMB_COMMON_OUT_OF_RESET));
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_ilkn_priority_get(unit, port, &priority));
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    ...
    if (enable)
    {
        ...
        /*
         * Enable ILKN context in OFR
         */
        if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
        {
            SHR_IF_ERR_EXIT(dnx_port_ofr_ilkn_rx_sch_context_enable(unit, port, priority, enable));
        }
    ...
    else
    {
         ...
         * HRF RX Segmentation Reset
         */
        SHR_IF_ERR_CONT(imb_ilu_port_nof_active_ilkn_ports_in_core_get(unit, port, &nof_active_port));
    ...
    uint64 *count)
{
    ...
    /*
     * if port is TDM and we are asking for data traffic or vice versa then exit
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_tdm_get(unit, port, &tdm_mode));
    if ((tdm_mode == DNX_ALGO_PORT_TDM_MODE_BYPASS && sch_priority != bcmPortNifSchedulerTDM) ||
        (tdm_mode == DNX_ALGO_PORT_TDM_MODE_PACKET && sch_priority != bcmPortNifSchedulerTDM) ||
        (tdm_mode == DNX_ALGO_PORT_TDM_MODE_NONE && sch_priority == bcmPortNifSchedulerTDM))
    {
        SHR_EXIT();
    }
