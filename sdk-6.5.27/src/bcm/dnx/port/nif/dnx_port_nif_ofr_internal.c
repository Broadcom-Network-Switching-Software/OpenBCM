...
#define MAX_NOF_PRD_PRIORITIES 4
    ...
    int in_reset)
{
    ...
    for (priority_group = 0; priority_group < priority_config->nof_priority_groups; ++priority_group)
    {
        if (DNX_PORT_NIF_OFR_PRIORITY_IS_LOW_OR_HIGH(priority_config->priority_groups[priority_group]))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_port_ofr_quantum_counter_reset_set(unit, port,
                                                                   priority_config->
                                                                   priority_groups[priority_group].sch_priority,
                                                                   in_reset));
        }
    }
    ...
    int *rmc_machine_id)
{
    ...
    if (port_is_tdm && (sch_prio == bcmPortNifSchedulerTDM))
    {
        *rmc_machine_id = is_tdm_over_high ? 0 : 1;
    }
    else
    {
    ...
    int *rmc_id)
{
    ...
    int nif_interface_id, core, is_over_high;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_algo_port_nif_interface_id_get(unit, port, 0, &core, &nif_interface_id));
    SHR_IF_ERR_EXIT(dnx_port_ofr_tdm_is_over_high_internal_get(unit, port, priority_config, &is_over_high));
    SHR_IF_ERR_EXIT(dnx_port_ofr_rmc_machine_id_internal_get
                    (unit, port, priority_config->priority_groups[priority_group].sch_priority, is_over_high,
                     &rmc_machine_id));
    ...
    const bcm_port_prio_config_t * priority_config)
{
    ...
    /**
     * Determine TDM RMC priority (either high or low).
    ...
    /** for TDM only ports set single RMC and its chosen priority (over high or over low) */
    {
        /*
         * alloc DBAL table handle
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_NIF_OFR_PORT_IS_SINGLE_RMC, entry_handle_id));
        ...
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_SINGLE_RMC, INST_SINGLE, TRUE);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_LOW, INST_SINGLE, !is_over_high);
        }
        else
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_SINGLE_RMC, INST_SINGLE, FALSE);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_LOW, INST_SINGLE, FALSE);
        }
...
shr_error_e
dnx_port_ofr_rmc_thresholds_hw_set(
    int unit,
    int rmc_id,
    int thr_after_ovf)
{
    ...
    const bcm_port_prio_config_t * priority_config)
{
    ...
    int list_head, nof_sections_allocated, priority_group, core, nif_interface_id, priority_index = 0;
    int is_over_high;
    SHR_FUNC_INIT_VARS(unit);
    ...
    for (priority_group = 0; priority_group < priority_config->nof_priority_groups; ++priority_group)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_link_list_db_double_priority_allocated_list_info_get
                        (unit, DNX_PORT_NIF_OFR_LINK_LIST, nif_interface_id, priority_index, &list_head,
         ...
         */
        rmc_fifo_size = nof_sections_allocated * dnx_data_nif.ofr.nof_bytes_per_memory_section_get(unit);
        ...
        /*
         * set thr_after_ovf to hw.
         */
        SHR_IF_ERR_EXIT(dnx_port_ofr_rmc_thresholds_hw_set(unit, rmc.rmc_id, rmc.thr_after_ovf));
    }
    ...
    int in_reset)
{
        ...
        (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, FALSE, FALSE, FALSE)))
    {
        ...
        for (priority_group = 0; priority_group < priority_config.nof_priority_groups; ++priority_group)
        {
            if (DNX_PORT_NIF_OFR_PRIORITY_IS_TDM(priority_config.priority_groups[priority_group]))
            {
                /**
                 * Determine TDM RMC priority (either high or low).
                 * - For TDM hybrid ports the TDM priority is determine according to the 'other' priority
                 * - For TDM only ports use high priority as this is the only RMC used by the port.
                 */
                SHR_IF_ERR_EXIT(dnx_port_ofr_tdm_is_over_high_internal_get(unit, port, &priority_config,
                                                                           &is_over_high));
                priority = is_over_high ? 0 : 1;
            }
            else
            {   /** DNX_PORT_NIF_OFR_PRIORITY_IS_LOW_OR_HIGH */
    ...
    int enable)
{
    ...
    for (priority_group = 0; priority_group < priority_config->nof_priority_groups; ++priority_group)
    {
        if (DNX_PORT_NIF_OFR_PRIORITY_IS_LOW_OR_HIGH(priority_config->priority_groups[priority_group]))
        {
            SHR_IF_ERR_EXIT(dnx_port_ofr_quantum_load_enable_set
                            (unit, port, priority_config->priority_groups[priority_group].sch_priority, enable));
        }
    }
    ...
    int enable)
{
    ...
    for (priority_group = 0; priority_group < priority_config->nof_priority_groups; ++priority_group)
    {
        if (DNX_PORT_NIF_OFR_PRIORITY_IS_TDM(priority_config->priority_groups[priority_group]))
        {
            /** TDM - make sure to hold the chosen priority in reset since it's used for TDM traffic only */
            in_reset = IMB_COMMON_IN_RESET;
            SHR_IF_ERR_EXIT(dnx_port_ofr_tdm_is_over_high_internal_get(unit, port, priority_config, &is_over_high));
            priority = is_over_high ? bcmPortNifSchedulerHigh : bcmPortNifSchedulerLow;
        }
        else
        {
            in_reset = (enable) ? IMB_COMMON_OUT_OF_RESET : IMB_COMMON_IN_RESET;
            /** High or Low priority */
            priority = priority_config->priority_groups[priority_group].sch_priority;
        }

        SHR_IF_ERR_EXIT(dnx_port_ofr_deficit_rr_config_reset_set(unit, port, priority, in_reset));
    }
...
shr_error_e
dnx_port_ofr_quantum_load_enable_set(
    int unit,
    bcm_port_t port,
    bcm_port_nif_scheduler_t priority,
    int enable)
{
    int dbal_priority, core, nif_interface_id;
    uint32 entry_handle_id;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_algo_port_nif_interface_id_get(unit, port, 0, &core, &nif_interface_id));
    dbal_priority = (priority == bcmPortNifSchedulerLow ? 1 : 0);
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PRIORITY, dbal_priority);
...
shr_error_e
dnx_port_ofr_quantum_counter_reset_set(
    int unit,
    bcm_port_t port,
    bcm_port_nif_scheduler_t priority,
    int in_reset)
...
shr_error_e
dnx_port_ofr_deficit_rr_config_reset_set(
    int unit,
    bcm_port_t port,
    bcm_port_nif_scheduler_t priority,
    int in_reset)
...
{
    int free_size, rmc_machine_id, is_over_high;
    bcm_port_prio_config_t priority_config;
    ...
    for (priority_group = 0; priority_group < priority_config.nof_priority_groups; ++priority_group)
    {
        SHR_IF_ERR_EXIT(dnx_port_ofr_tdm_is_over_high_internal_get
                        (unit, logical_port, &priority_config, &is_over_high));
        SHR_IF_ERR_EXIT(dnx_port_ofr_rmc_machine_id_internal_get
                        (unit, logical_port, priority_config.priority_groups[priority_group].sch_priority, is_over_high,
                         &rmc_machine_id));
    ...
    uint32 phy_id)
{
     ...
     * set value fields
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HIGH_PRIORITY_RMC, INST_SINGLE, phy_id);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOW_PRIORITY_RMC, INST_SINGLE, phy_id);
    ...
    uint32 phy_id)
{
     ...
     * set value fields
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PHY_ID, INST_SINGLE, phy_id);
    /*
     * commit the value
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
shr_error_e
dnx_port_ofr_preemption_hw_init(
    int unit)
{
    uint32 entry_handle_id;
    SHR_FUNC_INIT_VARS(unit);
     ...
     * alloc DBAL table handle
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_OFR_PREEMPTION_ENABLE, &entry_handle_id));

    /*
     * set value fields
     */

    /** Enable frame preemption (device level enabler) */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PREEMPTION_ENABLE, INST_SINGLE, 1);
    /*
     * commit the value
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
shr_error_e
dnx_port_ofr_preemption_enable_hw_set(
    int unit,
    bcm_port_t port,
    uint32 enable)
{
     ...
     * set key fields
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PORT_HAS_XPRESS, INST_SINGLE, enable);
    /*
     * commit value
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
shr_error_e
dnx_port_ofr_preemption_enable_hw_get(
    int unit,
    bcm_port_t port,
    uint32 *enable)
{
     ...
     * request for value field
     */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_PORT_HAS_XPRESS, INST_SINGLE, enable);
...
shr_error_e
dnx_port_nif_ofr_link_list_client_post_switch(
    int unit,
    int rmc_id,
    int nof_sections,
    uint32 *threshold,
    ...
    int is_hard_stage_enable)
{
    ...
    /*
     * Setting new threshold after overflow value
     */
    SHR_IF_ERR_EXIT(dnx_port_ofr_rmc_thresholds_hw_set(unit, rmc_id, thr_after_ovf));

    /*
     * Setting new fifo size
     */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ofr_prd_threshold_hw_set(unit, rmc_id, priority, threshold[priority]));
    }

    /*
     * Setting prd enable bit
     */
    SHR_IF_ERR_EXIT(dnx_ofr_prd_hard_stage_enable_hw_set(unit, rmc_id, is_hard_stage_enable));
...
shr_error_e
dnx_port_ofr_tdm_is_over_high_internal_get(
    int unit,
    bcm_port_t port,
    const bcm_port_prio_config_t * priority_config,
    int *is_over_high)
{
    ...
    /** By default use Low RMC for TDM only ports - thus using channel 0 when sending TDM only packet to IRE */
    *is_over_high = 0;

    /**
     * Determine TDM RMC priority (either high or low) for TDM hybrid ports according to the 'other' priority.
     */
