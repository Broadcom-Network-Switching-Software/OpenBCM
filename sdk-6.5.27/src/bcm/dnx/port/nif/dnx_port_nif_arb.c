    ...
    bcm_port_t logical_port)
{
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, FALSE, FALSE, FALSE))
    {
        /*
         * Free link list
         */
        SHR_IF_ERR_EXIT(dnx_port_arb_tx_tmac_link_list_set(unit, logical_port, FALSE));
    }
...
shr_error_e
dnx_port_arb_shadow_calendar_build(
    int unit,
    bcm_pbmp_t calendars)
{
    bcm_port_t calendar;
    ...
    /*
     * Build Arbiter's shadow calendars (rx or tx according to calendar's type)
     */
    _SHR_PBMP_ITER(calendars, calendar)
    {
        SHR_IF_ERR_EXIT(dnx_port_shadow_calendar_build(unit, calendar));
    }
...
shr_error_e
dnx_port_arb_calendar_switch_active_id(
    int unit,
    bcm_pbmp_t calendars)
{
    bcm_port_t calendar;
    ...
    /*
     * Switch Arbiter's active calendars' id (rx or tx according to calendar's type)
     */
    _SHR_PBMP_ITER(calendars, calendar)
    {
        SHR_IF_ERR_EXIT(dnx_port_calendar_switch_active_id(unit, calendar));
    }
...
shr_error_e
dnx_port_arb_tx_tmac_link_list_switch(
    int unit,
    bcm_port_t logical_port)
{
    int speed = 0, nof_sections;
    int client_id;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_port_link_list_switch(unit, DNX_PORT_NIF_ARB_LINK_LIST, nof_sections, client_id));
    ...
    bcm_port_t logical_port)
{
     ...
     * Enable stif bit.
     */
    SHR_IF_ERR_EXIT(dnx_port_arb_stif_enable_set(unit, instance_id, TRUE));

    /*
     * Set offset of stif port.
     */
    SHR_IF_ERR_EXIT(dnx_port_arb_stif_port_set(unit, instance_id, ethu_info.first_lane_in_port));

    /*
     * Take out of reset the stif async fifo.
     */
    SHR_IF_ERR_EXIT(dnx_port_arb_stif_reset_set(unit, instance_id, IMB_COMMON_OUT_OF_RESET));
...
dnx_port_arb_port_enable(
    ...
    int enable)
{
    int in_reset, stif_instance_core, stif_instance_id;
    uint8 is_connected;
    dnx_algo_port_info_s port_info;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        /*
         * Get stif instance id
         */
        SHR_IF_ERR_EXIT(dnx_stif_mgmt_logical_port_is_connected
                        (unit, logical_port, &stif_instance_core, &stif_instance_id, &is_connected));

        /*
         * Reset stif port
         */
        SHR_IF_ERR_EXIT(dnx_port_arb_stif_reset_set(unit, stif_instance_id, in_reset));
    }
    ...
    bcm_port_t logical_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_arb_credits_init_set(unit, logical_port, 1));
    SHR_IF_ERR_EXIT(dnx_port_arb_credits_init_set(unit, logical_port, 0));
    ...
    int enable)
{
    ...
    if (enable)
    {
        SHR_IF_ERR_EXIT(dnx_port_arb_credits_init(unit, logical_port));
    }
exit:
    SHR_FUNC_EXIT;
}
