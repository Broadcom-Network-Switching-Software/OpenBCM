    ...
    uint32 threshold)
{
    ...
    uint32 rmc_fifo_resolution, max_threshold, priority_group;
    int nif_interface_id, core, list_head, rmc_fifo_size;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_port_link_list_db_double_priority_allocated_list_info_get
                    (unit, DNX_PORT_NIF_OFR_LINK_LIST, nif_interface_id, priority_group, &list_head, &rmc_fifo_size));
    ...
    if (prd_profile != DNX_PORT_NIF_OFR_DEFAULT_PRD_PROFILE)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                        (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF,
                         DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_ELK | DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FLEXE_PHY |
                         DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_OTN_PHY, &nif_pbmp));
    ...
    uint8 *is_single_port_on_profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF,
                     ...
                     DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_FLEXE_PHY | DNX_ALGO_PORT_LOGICALS_F_EXCLUDE_OTN_PHY, &nif_pbmp));
 ...
 */
int
dnx_ofr_prd_global_drop_count_get(
    int unit,
    uint32 priority,
    ...
    uint64 *drop_count)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ofr_prd_global_drop_count_hw_get(unit, priority, priority_group, drop_count));
...
/**
 * \brief - maps a NIF port to a PRD counter
 *
 * \param [in] unit - chip unit id
 * \param [in] port - nif port
 * \param [in] priority_group - priority group to count
 * \param [in] counter - the counter to map to the port
 *
 * \return
 *   int - see _SHR_E_*
 *
 ...
 */
int
dnx_ofr_prd_port_counter_map_set(
    int unit,
    uint32 counter,
    bcm_port_t port,
    uint32 priority_group)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ofr_prd_port_counter_hw_set(unit, counter, nif_port, priority_group));
...
int
dnx_ofr_prd_rmc_drop_count_get(
    int unit,
    uint32 counter,
    uint64 *drop_count)
{
    uint64 prio_0_drop_count, prio_1_drop_count, prio_2_drop_count, prio_3_drop_count;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_ofr_prd_counter_drop_count_hw_get
                    (unit, counter, &prio_0_drop_count, &prio_1_drop_count, &prio_2_drop_count, &prio_3_drop_count));
    *drop_count = prio_0_drop_count + prio_1_drop_count + prio_2_drop_count + prio_3_drop_count;
