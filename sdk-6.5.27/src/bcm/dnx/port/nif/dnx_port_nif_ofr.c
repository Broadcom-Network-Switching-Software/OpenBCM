...
{
    int nof_prio_groups_supported, ii, nof_entries, prd_priority_mask, rmc_machine_id, prd_prio;
    int prio_found[DNX_DATA_MAX_NIF_ETH_PRIORITY_GROUPS_NOF] = { 0 };
    uint32 profile_rmc_group[DNX_DATA_MAX_NIF_PRD_NOF_PRIORITIES], prd_profile;
    ...
    for (ii = 0; ii < priority_config->nof_priority_groups; ++ii)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_ofr_rmc_machine_id_get(unit, port, priority_config, ii, &rmc_machine_id));
        if (prio_found[rmc_machine_id])
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Port %d, Having two priority groups with the same scheduler priority (RMC) is not supported\n",
                         port);
        }
        prio_found[rmc_machine_id] = 1;
        ...
        if ((prd_profile != DNX_PORT_NIF_OFR_DEFAULT_PRD_PROFILE) && !is_first_port_on_profile)
        {
            ...
            {
                if (rmc_machine_id == profile_rmc_group[prd_prio])
                {
...
dnx_port_ofr_double_link_list_calc(
    ...
    uint8 **alloc_info)
{
    int alloc_index = 0, max_num_entries, num_of_entries, core, nif_interface_id;
    bcm_port_prio_config_t priority_config;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_port_nif_db.
                        priority_group_num_of_entries.get(unit, priority_group, nif_interface_id, &num_of_entries));
        nof_sections_in_group =
            (num_of_entries == -1) ? nof_sections : priority_config.priority_groups[priority_group].num_of_entries;
    ...
    const bcm_port_prio_config_t * priority_config)
{
    ...
    for (priority_group = 0; priority_group < priority_config->nof_priority_groups; ++priority_group)
    {
        ...
        /** Saves configured num_of_entries to tell whether user set -1 (sections according to port's speed) or actual num_of_entries. */
        if (port < DNX_DATA_MAX_NIF_OFR_NOF_RMC_PER_PRIORITY_GROUP)
        {
            int core, nif_interface_id;
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_interface_id_get(unit, port, 0, &core, &nif_interface_id));
            SHR_IF_ERR_EXIT(dnx_port_nif_db.priority_group_num_of_entries.set(unit, priority_group, nif_interface_id,
                                                                              priority_config->priority_groups
                                                                              [priority_group].num_of_entries));
        }
    }
...
shr_error_e
dnx_port_ofr_ilkn_rx_sch_context_enable(
    int unit,
    bcm_port_t port,
    bcm_port_nif_scheduler_t priority,
    int enable)
{
    int in_reset = enable ? IMB_COMMON_OUT_OF_RESET : IMB_COMMON_IN_RESET;
    bcm_port_nif_scheduler_t priority_to_disable =
        (priority == bcmPortNifSchedulerLow) ? bcmPortNifSchedulerHigh : bcmPortNifSchedulerLow;
    SHR_FUNC_INIT_VARS(unit);

    /*
     *  Set the Deficit RR configuration. For each ilkn_id there is only one data HRF,
     *  so if low priority is enabled, high should be disabled and vice versa.
     */
    SHR_IF_ERR_EXIT(dnx_port_ofr_deficit_rr_config_reset_set(unit, port, priority, in_reset));
    SHR_IF_ERR_EXIT(dnx_port_ofr_deficit_rr_config_reset_set(unit, port, priority_to_disable, IMB_COMMON_IN_RESET));

    /*
     * if enabling - toggle quantum load for this context according to its priority
     */
    if (enable)
    {
        SHR_IF_ERR_EXIT(dnx_port_ofr_quantum_load_enable_set(unit, port, priority, 1));
        SHR_IF_ERR_EXIT(dnx_port_ofr_quantum_load_enable_set(unit, port, priority, 0));
    }
    ...
    int is_low_priority_enabled)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_ofr_internal_ilkn_rx_sched_request_enable_set
                    (unit, ilkn_access_info.ilkn_id, DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_TDM, is_tdm_priority_enabled));
    SHR_IF_ERR_EXIT(dnx_port_ofr_internal_ilkn_rx_sched_request_enable_set
                    (unit, ilkn_access_info.ilkn_id, DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_HIGH, is_high_priority_enabled));
    SHR_IF_ERR_EXIT(dnx_port_ofr_internal_ilkn_rx_sched_request_enable_set
                    (unit, ilkn_access_info.ilkn_id, DBAL_ENUM_FVAL_NIF_SCH_PRIORITY_LOW, is_low_priority_enabled));
    SHR_IF_ERR_EXIT(dnx_port_ofr_internal_ilkn_rx_is_high_priority_config_set
                    (unit, ilkn_access_info.ilkn_id, is_high_priority_enabled));

    /*
     * priority is either low or high or neither (ILKN port can't have both).
     * Put quantum counter into / out of reset accordingly.
     */
    SHR_IF_ERR_EXIT(dnx_port_ofr_quantum_counter_reset_set
                    (unit, port, bcmPortNifSchedulerLow,
                     is_low_priority_enabled ? IMB_COMMON_OUT_OF_RESET : IMB_COMMON_IN_RESET));
    SHR_IF_ERR_EXIT(dnx_port_ofr_quantum_counter_reset_set
                    (unit, port, bcmPortNifSchedulerHigh,
                     is_high_priority_enabled ? IMB_COMMON_OUT_OF_RESET : IMB_COMMON_IN_RESET));
    ...
    bcm_port_t port)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_interface_id_get(unit, port, 0, &core, &phy_id));

    /*
     * Reset Counter
     */
    SHR_IF_ERR_EXIT(dnx_port_ofr_rx_fifo_status_hw_get(unit, 0, &fifo_level));
    SHR_IF_ERR_EXIT(dnx_port_ofr_rx_fifo_status_hw_get(unit, 1, &fifo_level));

    SHR_IF_ERR_EXIT(dnx_port_ofr_rx_fifo_status_counter_hw_map(unit, phy_id));
    ...
    uint32 *mapped)
{
    ...
    /*
     * No tracking is available for max_occupancy - we always set it to zero
     */
    *max_occupancy = 0;

    /*
     * Verify port is mapped to fifo counter.
     * If port is mapped to a counter then retrieve the fifo level for the port, otherwise return 0.
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_db.port_rx_fifo_counter.port.get(unit, &mapped_port));
    if (mapped_port == port)
    {

        SHR_IF_ERR_EXIT(dnx_port_ofr_rx_fifo_status_hw_get(unit, sch_priority, fifo_level));
        *mapped = 1;
    }
    else
    {
    ...
    bcm_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_ofr_tx_fifo_status_counter_hw_map(unit, counter, phy_id));
...
shr_error_e
dnx_port_ofr_tdm_is_over_high_get(
    int unit,
    bcm_port_t port,
    int *is_over_high)
{
