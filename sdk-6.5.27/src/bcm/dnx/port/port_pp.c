...
#include <bcm_int/dnx/auto_generated/dnx_rx_dispatch.h>
...
/* parsing context for DNX2-INJECTED-2-J1 is built as follow: {0,0,prt_qual(1:0),PTCH(msb bit)}
 * prt_qual=2, is reserved value to indicates J1 mode. We share this value with SET_VISIBILITY prt_qual enum (due to lack of resources),
 * and the assumptions is that it is okay to allow visibility for these flows.
 */
#define DNX2_INJECTED_2_J1_PARSING_CONTEXT_MSBS_RESERVED (DBAL_ENUM_FVAL_PRT_QUALIFIER_SET_VISIBILITY)

/** Number of Load balancing disable hash layers flags. */
#define NOF_LB_PROFILE_FLAGS 6

/** The types of gport can be used in dnx_port_flood_group_set*/
#define DNX_PORT_FLOOD_DEST_IS_LEGAL(default_dest)  \
     (BCM_GPORT_IS_BLACK_HOLE(default_dest) ||\
      BCM_GPORT_IS_MCAST(default_dest) ||\
      BCM_GPORT_IS_TRAP(default_dest) ||\
      BCM_GPORT_IS_FORWARD_PORT(default_dest) ||\
      (default_dest == BCM_GPORT_INVALID))
/** Estimated BTR */
#define DNX_PORT_PP_ESTIMATED_BTR 110
/** Estimated BTR in SRv6 JR2 compatible mode */
#define DNX_PORT_PP_ESTIMATED_BTR_SRV6_JR2_COMP_MODE 90
/** Invalid profile for the use of ESEM command allocation profile **/
#define DNX_PORT_PP_INVALID_PROFILE -1
...
/**
 * \brief
 *   Verify function for dnx_port_pp_preserve_dscp_set()
 */
static shr_error_e
dnx_port_pp_preserve_dscp_set_verify(
    int unit,
    bcm_port_t gport,
    int preserve_dscp_mode)
{
    ...
    {
        /** Preserve DSCP is supported with 1 bit of orientation in Jericho mode*/
        if ((same_interface_number_of_values) || (outer_dp_number_of_values) || (inner_dp_number_of_values) ||
            (in_lif_profile_orientation_number_of_values > 2))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Unsupported combination of soc properties, only preserve dscp should be set, or preserve dscp with 2 values for orientation!\n");
        }
    }
    if ((preserve_dscp) != 1)
    {
...
/**
 *  Verify function for dnx_port_pp_preserve_dscp_get()
 */
static shr_error_e
dnx_port_pp_preserve_dscp_get_verify(
    int unit,
    bcm_port_t gport,
    int *preserve_dscp_mode)
{
    ...
    uint32 vlan_domain)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_vlan_domain_set_verify(unit, port, vlan_domain));
    ...
    uint32 *vlan_mem_if)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_vlan_membership_if_get_verify
    ...
    uint32 vlan_mem_if)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_vlan_membership_if_set_verify(unit, port, vlan_mem_if));
    ...
    uint32 *vlan_domain)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_vlan_domain_get_verify(unit, port, vlan_domain));
    ...
    int map_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_port_control_flow_lif_property_set
                        (unit, port,
                         (type ==
                          QOS_DP_PROFILE_OUTER ? bcmPortControlOuterPolicerRemark : bcmPortControlInnerPolicerRemark),
                         map_id));
        SHR_EXIT();
    }
    /*
    ...
    int *map_id)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_port_control_flow_lif_property_get
                        (unit, port,
                         (type ==
                          QOS_DP_PROFILE_OUTER ? bcmPortControlOuterPolicerRemark : bcmPortControlInnerPolicerRemark),
                         map_id));
    ...
    bcm_vlan_t * vid)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_untagged_vlan_get_verify(unit, port, vid));
    ...
    bcm_vlan_t vid)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_untagged_vlan_set_verify(unit, port, vid));
    ...
    uint32 header_type)
{
    ...
    switch (header_type)
    {
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_IBCH1_MODE:
        {
            ...
            else if (header_type == BCM_SWITCH_PORT_HEADER_TYPE_STACKING)
            {
                parser_context = DBAL_ENUM_FVAL_IRPP_1ST_PARSER_PARSER_CONTEXT_ITMH_A1;
            }
            /** for IBCH1_MODE, the first header and context is unknown in advance, and it is taken from ibch1.prt_command */
            else if (header_type == BCM_SWITCH_PORT_HEADER_TYPE_IBCH1_MODE)
            {
                parser_context = DBAL_ENUM_FVAL_IRPP_1ST_PARSER_PARSER_CONTEXT_NOP;
            }
 ...
 */
int
dnx_port_initial_eth_properties_per_pp_port_unset(
    int unit,
    uint32 pp_port,
    bcm_core_t core_id,
    uint32 header_type,
    uint8 tpid_ref_dealloc)
{
    ...
    if (header_type == BCM_SWITCH_PORT_HEADER_TYPE_NONE)
    {
        SHR_EXIT();
    }
    ...
    switch (header_type)
    {
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_IBCH1_MODE:
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_port_tpid_class_ingress_default_per_pp_port_deinit
                            (unit, pp_port, core_id, tpid_ref_dealloc));
    ...
    SHR_IF_ERR_EXIT(dnx_port_packet_processing_properties_remove_internal(unit, pp_port, core_id, TRUE, FALSE));
 ...
 */
int
dnx_port_pp_src_system_port_get(
    int unit,
    bcm_core_t core_id,
    uint32 pp_port,
    uint32 *src_system_port)
{
    ...
    /*
     * Get SOURCE_SYSTEM_PORT from dbal table INGRESS_PP_PORT
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PP_PORT, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PP_PORT, pp_port);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
 ...
 */
int
dnx_port_pp_ingress_ptc_port_mapping_set(
    int unit,
    uint32 ptc,
    bcm_core_t core_id,
    uint32 src_system_port,
    ...
    uint32 flags)
{
    ...
    /*
     * Configure SOURCE_SYSTEM_PORT and PP_PORT in dbal table INGRESS_PTC_PORT
     */
    ...
    uint32 flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_vlan_member_set_verify(unit, port, flags));
    ...
    uint32 *flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_vlan_member_get_verify(unit, port, flags));
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, gport_hw_resources.inlif_dbal_table_id, &core_mode));
    ...
    uint32 flags)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_learn_set_verify(unit, is_physical_port, flags));
    ...
    uint32 *flags)
{
     ...
     * Verification the inputs
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_learn_get_verify(unit, flags));
    ...
    uint32 vmpi)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID,
                                  gport_info.internal_port_pp_info.core_id[pp_port_index]);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VMPI, INST_SINGLE, vmpi);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    uint32 *vmpi)
{
    ...
    /*
     * Get the atrributes from the entry
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PP_PORT, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PP_PORT, gport_info.internal_port_pp_info.pp_port[0]);
    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, gport_info.internal_port_pp_info.core_id[0]);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_VMPI, INST_SINGLE, vmpi);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    uint32 remove)
{
     ...
     * Verification the inputs
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_learn_modify_verify(unit, port, is_physical_port, add, remove));
    ...
    int filter_flags)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_lif_same_interface_disable_set_verify(unit, port, filter_flags));

    /** Flow-lif procedures */
    if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
         (BCM_GPORT_IS_VLAN_PORT(port) || BCM_GPORT_IS_FLOW_LIF(port))) ||
        (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
         (BCM_GPORT_IS_MPLS_PORT(port) || BCM_GPORT_IS_FLOW_LIF(port))))
    {
        SHR_IF_ERR_EXIT(dnx_port_control_flow_lif_property_set(unit, port, bcmPortControlBridge, filter_flags));
        SHR_EXIT();
    }
    ...
    int *filter_flags)
{
    ...
    if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
        ...
        (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
         (BCM_GPORT_IS_MPLS_PORT(port) || BCM_GPORT_IS_FLOW_LIF(port))))
    {
        SHR_IF_ERR_EXIT(dnx_port_control_flow_lif_property_get(unit, port, bcmPortControlBridge, filter_flags));
        SHR_EXIT();
    }
    ...
    int in_lif_same_interface_mode)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_in_lif_same_interface_set_verify(unit, gport, in_lif_same_interface_mode));

    /** Flow-lif procedures */
    if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
         (BCM_GPORT_IS_VLAN_PORT(gport) || BCM_GPORT_IS_FLOW_LIF(gport))) ||
        (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
         (BCM_GPORT_IS_MPLS_PORT(gport) || BCM_GPORT_IS_FLOW_LIF(gport))))
    {
        SHR_IF_ERR_EXIT(dnx_port_control_flow_lif_property_set
                        (unit, gport, bcmPortControlLogicalInterfaceSameFilter, in_lif_same_interface_mode));
        SHR_EXIT();
    }
    ...
    int preserve_dscp_mode)
{
    ...
    dnx_in_lif_profile_last_info_t last_profile;
    uint8 is_ip_tunnel_l2vpn = 0;

    SHR_FUNC_INIT_VARS(unit);
    dnx_in_lif_profile_last_info_t_init(&last_profile);
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_preserve_dscp_set_verify(unit, gport, preserve_dscp_mode));

    if (DNX_PORT_PP_GPORT_IS_RIF(unit, gport))
    {
        /** It's a RIF*/
        gport = BCM_GPORT_TUNNEL_ID_GET(gport);
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_vsi_profile_get
                    (unit, gport, (uint32 *) &old_in_lif_profile, &result_type, &is_ip_tunnel_l2vpn, intf_type));

    SHR_IF_ERR_EXIT(in_lif_profile_info_t_init(unit, &in_lif_profile_info));
    in_lif_profile_info.ingress_fields.cs_in_lif_profile_flags |=
        (is_ip_tunnel_l2vpn ? DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_L2VPN : 0);
    ...
    int *preserve_dscp_mode)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_preserve_dscp_get_verify(unit, gport, preserve_dscp_mode));
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_vsi_profile_get
                    (unit, gport, (uint32 *) &in_lif_profile, &result_type, &is_ip_tunnel_l2vpn, intf_type));
 ...
 */
shr_error_e
dnx_port_pp_speculative_parsing_set(
    int unit,
    bcm_port_t port,
    int enable)
{
    ...
    if (llr_cs_profile == DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_MPLS_WITH_IP_SPECULATIVE_PARSING)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The port is configured with bcmPortControlMplsIpVXParseEnable, thus it cannot be set with bcmPortControlMplsSpeculativeParse=0\n");
    }

    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
                    (unit, port, DNX_ALGO_GPM_GPORT_TO_PHY_OP_PP_PORT_IS_MANDATORY, &gport_info));

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEM_PER_PORT_DB, &entry_handle_id));
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS_SPECULATIVE_PARSING, INST_SINGLE, enable);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
 ...
 */
shr_error_e
dnx_port_pp_speculative_parsing_get(
    int unit,
    bcm_port_t port,
    int *enable)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_PP_PORT, pp_port);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MPLS_SPECULATIVE_PARSING, INST_SINGLE, &tmp32bit);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
...
static shr_error_e
dnx_port_pp_ipvx_parsing_over_mpls_set_verify(
    int unit,
    bcm_port_t port)
{
    dbal_enum_value_field_llr_cs_port_cs_profile_e old_llr_cs_profile = 0;
    int spec_parse_enable = 0;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_port_pp_llr_cs_profile_get(unit, port, &old_llr_cs_profile));

    /*
     * The API can be called only on ports which their LLR profile is typical or IPVX_PARSING
     */
    if ((old_llr_cs_profile != DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_MPLS_WITH_IP_SPECULATIVE_PARSING) &&
        (old_llr_cs_profile != DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_TYPICAL_PORT))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The port has some LLR configuration, thus it cannot be set with bcmPortControlMplsIpVXParseEnable\n");
    }

    /*
     * The API cannot be called on ports with Speculative parsign disable.
     */
    SHR_IF_ERR_EXIT(dnx_port_pp_speculative_parsing_get(unit, port, &spec_parse_enable));
    if (!spec_parse_enable)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The port is configured with bcmPortControlMplsSpeculativeParse=0, thus cannot be set with bcmPortControlMplsIpVXParseEnable\n");
    }
 ...
 */
shr_error_e
dnx_port_pp_ipvx_parsing_over_mpls_set(
    int unit,
    bcm_port_t port,
    uint8 enable)
{
    ...
    uint32 enable_uint = enable;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_port_pp_ipvx_parsing_over_mpls_set_verify(unit, port));
    SHR_IF_ERR_EXIT(dnx_port_pp_llr_cs_profile_get(unit, port, &old_llr_cs_profile));
     ...
     */
    SHR_IF_ERR_EXIT(algo_port_pp_db.ipvx_o_mpls_parsing_enable_profile.exchange
                    (unit, 0, 1, &enable_uint, old_mode, NULL, &new_mode, &is_first, &is_last));

    if (is_first && enable)
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_ipvx_over_mpls_parsing_predefined_entries_add(unit));
    }

    SHR_IF_ERR_EXIT(dnx_port_pp_llr_cs_profile_set(unit, port, enable ?
                                                   DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_MPLS_WITH_IP_SPECULATIVE_PARSING
                                                   : DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_TYPICAL_PORT));

    if (is_last && !enable)
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_ipvx_over_mpls_parsing_clear(unit));
    }
 ...
 */
shr_error_e
dnx_port_pp_ipvx_parsing_over_mpls_get(
    int unit,
    bcm_port_t port,
    uint8 *enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_pp_llr_cs_profile_get(unit, port, &llr_cs_profile));
    *enable = llr_cs_profile == DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_MPLS_WITH_IP_SPECULATIVE_PARSING;
...
/**
 * \brief - Verify the value of oam_default_profile
 *
 * \param [in] unit - Unit ID
 * \param [in] oam_default_profile - OAM default profile
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
dnx_port_pp_oam_default_profile_set_verify(
    int unit,
    int oam_default_profile)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_RANGE_VERIFY(oam_default_profile, 0, DNX_OAM_DEFAULT_PROFILE_MAX_VALUE, _SHR_E_PARAM,
                     "OAM default profile is out of range, it should not be bigger than %d",
    ...
    int oam_default_profile)
{
    ...
    DBAL_FUNC_INIT_VARS(unit);
    dnx_in_lif_profile_last_info_t_init(&last_profile);
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_oam_default_profile_set_verify(unit, oam_default_profile));
    ...
    bcm_core_t * core)
{
    ...
    /*
     * for COE channelized port, PTC is taken from channel 0's
     * Allow to enable COE on RCY_MIRROR port to support COE tag remove for egress mirror(from COE port)
     */
    if ((flags & (DNX_ALGO_PORT_APP_FLAG_COE | DNX_ALGO_PORT_APP_FLAG_PON))
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_ptc_get(unit, port_tmp, core, (int *) ptc));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_in_tm_port_get(unit, port_tmp, core, ptc));
    }
    ...
    bcm_port_t * port)
{
    ...
    *port = -1;
    ...
    BCM_PBMP_ITER(logical_ports, port_iter)
    {
        SHR_IF_ERR_EXIT(dnx_port_ptc_get(unit, port_iter, &ptc_iter, &core_iter));
        if (ptc_iter == ptc && core_iter == core)
        {
            *port = port_iter;
            SHR_EXIT();
        }
    ...
    dbal_enum_value_field_port_termination_ptc_profile_e * prt_profile)
{
    ...
    /*
     * Get the prt ptc_profile per PTC, using first local port in case of LAG
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PTC_PORT, &entry_handle_id));
    /** Key fields */
 ...
 */
static shr_error_e
dnx_port_pp_recycle_app_ptc_profile_info_set(
    int unit,
    bcm_port_t port,
    dnx_port_pp_ingress_port_ptc_info_t * ptc_info,
    uint8 vtt1_routing_enablers_valid)
{
    uint32 entry_handle_id;
    dnx_algo_gpm_gport_phy_info_t gport_info;
    uint32 ptc;
    ...
    BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
    {
        ...
        if (dnx_data_pp.stages.params_get(unit, DNX_PP_STAGE_VTT5)->valid)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PTC_CS_PROFILE_VTT5, INST_SINGLE,
                                         ptc_info->vtt5_ptc_cs_profile);
        }
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PTC_CS_PROFILE_FWD1, INST_SINGLE,
                                     ptc_info->fwd1_ptc_cs_profile);
        if (vtt1_routing_enablers_valid)
        {
            dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_VTT1,
                                        INST_SINGLE, ptc_info->vtt1_ptc_routing_enable_profile);
        }
        if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_ecologic_support))
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ECOL_PTC_PROFILE, INST_SINGLE,
                                         ptc_info->eco_logic_ptc_profile);
        }
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    int recycle_app_type)
{
    ...
    if (recycle_app_type == bcmPortControlRecycleAppReflector)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Value bcmPortControlRecycleAppReflector(%d) is not supported\n",
                     bcmPortControlRecycleAppReflector);
    }
    if (!
    ...
    if ((recycle_app_type == bcmPortControlRecycleAppSbfdReflector) &&
        (!dnx_data_port_pp.general.feature_get(unit, dnx_data_port_pp_general_recycle_app_sbfd_reflector)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Value bcmPortControlRecycleAppSbfdReflector(%d) is not supported\n",
                     bcmPortControlRecycleAppSbfdReflector);
    }
    if ((recycle_app_type == bcmPortControlRecycleAppBridgeFallback) &&
        (!dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_mc_bridge_fallback)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Value bcmPortControlRecycleAppBridgeFallback(%d) is not supported\n",
                     bcmPortControlRecycleAppBridgeFallback);
    }
    ...
    int recycle_app_type)
{
    ...
    uint8 vtt1_ptc_routing_enable_valid;
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_recycle_app_set_verify(unit, port, recycle_app_type));
     ...
     * initialize ingress ptc info with default values
     */
    vtt1_ptc_routing_enable_valid = FALSE;
    ingress_ptc_info.vtt1_ptc_cs_profile = DBAL_ENUM_FVAL_PTC_CS_PROFILE_VTT1_DEFAULT;
    ingress_ptc_info.vtt2_ptc_cs_profile = DBAL_ENUM_FVAL_PTC_CS_PROFILE_VTT2_DEFAULT;
    ingress_ptc_info.vtt3_ptc_cs_profile = DBAL_ENUM_FVAL_PTC_CS_PROFILE_VTT3_DEFAULT;
    ingress_ptc_info.vtt4_ptc_cs_profile = DBAL_ENUM_FVAL_PTC_CS_PROFILE_VTT4_DEFAULT;
    ingress_ptc_info.vtt5_ptc_cs_profile = DBAL_ENUM_FVAL_PTC_CS_PROFILE_VTT5_DEFAULT;
    ingress_ptc_info.fwd1_ptc_cs_profile = DBAL_ENUM_FVAL_PTC_CS_PROFILE_FWD1_STANDARD;
    ingress_ptc_info.vtt1_ptc_routing_enable_profile = DBAL_ENUM_FVAL_PTC_ROUTING_ENABLE_PROFILE_VTT1_DEFAULT;
    ingress_ptc_info.eco_logic_ptc_profile = DBAL_ENUM_FVAL_ECOL_PTC_PROFILE_ENABLE;

    /** General Note:
     * When ibch1_supported, it is possible to run several application on the same recycle port.
     * This is possible if the header_type_in=IBCH1_MODE and the recycle_app is Default.
     * The following applications are excluded: bcmPortControlRecycleAppExtendedEncap
     */
    /*
    ...
    switch (recycle_app_type)
    {
        ...
        case bcmPortControlRecycleAppExtendedEncap:
        {
                ...
                system_headers.feature_get(unit, dnx_data_headers_system_headers_system_headers_ibch1_supported))
            {
                ...
                ingress_ptc_info.fwd1_ptc_cs_profile =
                    DBAL_ENUM_FVAL_PTC_CS_PROFILE_FWD1_2ND_PASS_EXTENDED_ENCAPSULATION;
            }
            ingress_ptc_info.eco_logic_ptc_profile = DBAL_ENUM_FVAL_ECOL_PTC_PROFILE_DISABLE;
            break;
        }
        case bcmPortControlRecycleAppExtendedTerm:
        {
            if (!dnx_data_headers.
                system_headers.feature_get(unit, dnx_data_headers_system_headers_system_headers_ibch1_supported))
            {
                ingress_ptc_info.fwd1_ptc_cs_profile = DBAL_ENUM_FVAL_PTC_CS_PROFILE_FWD1_EXTENDED_TERMINATION;
            }
            break;
        }
        case bcmPortControlRecycleAppBridgeFallback:
        {
            vtt1_ptc_routing_enable_valid = dnx_data_l3.routing_enablers.feature_get
                (unit, dnx_data_l3_routing_enablers_ptc_routing_enable_profile_support) ? TRUE : FALSE;
            ingress_ptc_info.vtt1_ptc_routing_enable_profile =
    ...
    SHR_IF_ERR_EXIT(dnx_port_pp_recycle_app_ptc_profile_info_set(unit, port, &ingress_ptc_info,
                                                                 vtt1_ptc_routing_enable_valid));

    /*
     * Egress side handling
     */
    egress_port_info.fwd_code_port_profile = DBAL_ENUM_FVAL_OUT_PP_PORT_EGRESS_FORWARD_CODE_SELECTION_PROFILE_NORMAL;
    ...
    egress_port_info.enc1_port_profile = DBAL_ENUM_FVAL_ETPP_ENC1_PORT_CS_VAR_ETHERNET_TYPICAL;
    egress_port_info.enc3_port_profile = DBAL_ENUM_FVAL_ETPP_ENC3_PORT_CS_VAR_ETHERNET_TYPICAL;
    egress_port_info.enc4_port_profile = DBAL_ENUM_FVAL_ETPP_ENC4_PORT_CS_VAR_ETHERNET_TYPICAL;
    ...
    switch (recycle_app_type)
    {
        ...
        {
            if (dnx_data_port_pp.
                filters.feature_get(unit, dnx_data_port_pp_filters_egress_filters_enable_per_recycle_port_supported))
            {
        ...
        case bcmPortControlRecycleAppExtendedEncap:
        {
            egress_port_info.enc1_port_profile = DBAL_ENUM_FVAL_ETPP_ENC1_PORT_CS_VAR_EXTENDED_ENCAPSULATION;
            egress_port_info.enc3_port_profile = DBAL_ENUM_FVAL_ETPP_ENC3_PORT_CS_VAR_EXTENDED_ENCAPSULATION;
            egress_port_info.enc4_port_profile = DBAL_ENUM_FVAL_ETPP_ENC4_PORT_CS_VAR_EXTENDED_ENCAPSULATION;
            if (!dnx_data_esem.access_cmd.esem_cmd_predefine_allocations_get(unit))
            {
                egress_port_info.default_esem_cmd = dnx_data_esem.access_cmd.no_action_get(unit);
                esem_cmd_modify = 1;
        ...
        case bcmPortControlRecycleAppBridgeFallback:
        {
            ...
            egress_port_info.fwd_code_port_profile =
                DBAL_ENUM_FVAL_OUT_PP_PORT_EGRESS_FORWARD_CODE_SELECTION_PROFILE_RAW_PROCESSING;
            if (!dnx_data_headers.
                system_headers.feature_get(unit, dnx_data_headers_system_headers_system_headers_ibch1_supported))
            {
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id,
                                     DBAL_FIELD_OUT_PP_PORT_EGRESS_FORWARD_CODE_SELECTION_PROFILE, INST_SINGLE,
                                     egress_port_info.fwd_code_port_profile);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENC_1_CONTEXT_PORT_PROFILE, INST_SINGLE,
                                     egress_port_info.enc1_port_profile);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENC_3_CONTEXT_PORT_PROFILE, INST_SINGLE,
                                     egress_port_info.enc3_port_profile);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENC_4_CONTEXT_PORT_PROFILE, INST_SINGLE,
                                     egress_port_info.enc4_port_profile);
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SPLIT_HORIZON_ENABLE, INST_SINGLE,
                                     egress_port_info.egress_pp_filters_enable);
        if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_glem_filter_support))
        {
        ...
        else
        {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_LOOPBACK_SRC_OR_DST_FILTER_ENABLE,
                                         INST_SINGLE, egress_port_info.egress_pp_filters_enable);
            if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_ipv6_hop_by_hop_filter_support))
            {
        ...
        if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_acceptable_frame_type_filter_support))
        {
            ...
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ACCEPTABLE_FRAME_TYPE_ENABLE_ERPP,
                                             INST_SINGLE, egress_port_info.egress_pp_filters_enable);
            }
        }
 ...
 */
shr_error_e
dnx_port_pp_recycle_app_set_wrapper(
    int unit,
    bcm_port_t port,
    int recycle_app_type)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_port_pp_recycle_app_set(unit, port, recycle_app_type));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *recycle_app_type)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_recycle_app_get_verify(unit, port, recycle_app_type));
    ...
    uint32 vlan_domain)
{
    ...
    {
        /** the following is not relevent when porting to flow, as flow allow result_type change */
        if (!dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
        {
            ...
            SHR_IF_ERR_EXIT(dbal_tables_field_predefine_value_get(unit,
                                                                  ...
                                                                  FALSE, gport_hw_resources.inlif_dbal_result_type,
            ...
            SHR_IF_ERR_EXIT(dbal_tables_field_predefine_value_get(unit,
                                                                  ...
                                                                  DBAL_FIELD_NEXT_LAYER_NETWORK_DOMAIN,
                                                                  FALSE, gport_hw_resources.inlif_dbal_result_type,
                                                                  0, DBAL_PREDEF_VAL_MIN_VALUE,
                                                                  &next_layer_network_domain_min_value));

            if ((vlan_domain > next_layer_network_domain_max_value)
    ...
    uint32 vlan_domain)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_lif_vlan_domain_set_verify(unit, gport_lif, pclass, vlan_domain));

    /** Flow-lif procedures */
    if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
         BCM_GPORT_IS_MPLS_PORT(gport_lif)) ||
        (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
         BCM_GPORT_IS_TUNNEL(gport_lif)))
    {
        SHR_IF_ERR_EXIT(dnx_port_class_flow_lif_property_set(unit, gport_lif, pclass, vlan_domain));
        SHR_EXIT();
    }
    ...
    uint32 *vlan_domain)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_lif_vlan_domain_get_verify(unit, gport_lif, vlan_domain));
    ...
    if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
        ...
        (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
         BCM_GPORT_IS_TUNNEL(gport_lif)))
    {
        SHR_IF_ERR_EXIT(dnx_port_class_flow_lif_property_get(unit, gport_lif, bcmPortClassIngress, vlan_domain));
        SHR_EXIT();
    }
    ...
    uint32 *vlan_domain)
{
    ...
    if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
        ...
        (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
         && BCM_GPORT_IS_TUNNEL(gport_lif)))
    {
        SHR_IF_ERR_EXIT(dnx_port_class_flow_lif_property_get(unit, gport_lif, bcmPortClassEgress, vlan_domain));
        SHR_EXIT();
    }
...
/**
 * \brief - Configure EGRESS_PP_PORT table with egress_port_info
  *
 * \param [in] unit - Unit number
 * \param [in] pp_port - PP port
 * \param [in] core_id - Core id
 * \param [in] egress_port_info - Egress PP port information
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
dnx_port_pp_egress_hw_per_pp_port_set(
    int unit,
    uint32 pp_port,
    uint32 core_id,
    dnx_port_pp_egress_port_info_t * egress_port_info)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE,
                                 egress_port_info->default_esem_cmd);
    if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_dss_stacking_filter_support))
    {
    ...
    if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_lag_egress_multicast_filter_support))
    {
    ...
    if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_exclude_source_filter_support))
    {
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENC_1_CONTEXT_PORT_PROFILE,
                                 INST_SINGLE, egress_port_info->enc1_port_profile);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENC_2_CONTEXT_PORT_PROFILE,
                                 INST_SINGLE, egress_port_info->enc2_port_profile);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENC_3_CONTEXT_PORT_PROFILE,
                                 INST_SINGLE, egress_port_info->enc3_port_profile);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENC_4_CONTEXT_PORT_PROFILE,
                                 INST_SINGLE, egress_port_info->enc4_port_profile);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENC_5_CONTEXT_PORT_PROFILE,
                                 INST_SINGLE, egress_port_info->enc5_port_profile);
    if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_acceptable_frame_type_filter_support))
    {
        /*
...
/**
 * \brief - ESEM command related actions for the action of egres pp port settings
  *
 * \param [in] unit - Unit number
 * \param [in] pp_port - PP port
 * \param [in] core - Core ID
 * \param [in] remove_svtag_configuration - indicates if SVTAG configuration should be removed from ESEM
 *                                          command in case it exist
 * \param [out] free_profile - in case there is a need to free ESEM command profile this will contain
 *                             the profile to be freed
 * \return
 *   shr_error_e
 *
 * \remark
 ...
 */
static shr_error_e
dnx_port_pp_egress_hw_esem_cmd(
    int unit,
    uint32 pp_port,
    bcm_core_t core,
    ...
    int *free_profile)
{
    ...
    bcm_gport_t gport_dummy = BCM_GPORT_INVALID;
    dnx_esem_cmd_data_t esem_cmd_data;
    SHR_FUNC_INIT_VARS(unit);

    /** Get old ESEM command */
    SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_per_pp_port_get(unit, pp_port, core, &old_esem_cmd));

    /*
     * Set the "new ESEM command" to be the old ESEM command as a default, in case the below exchange is not necessary 
     */
    new_esem_cmd = old_esem_cmd;
    SHR_IF_ERR_EXIT(dnx_switch_svtag_is_present_in_esem_cmd(unit, old_esem_cmd, &app_db_id_dummy, &found));

    if (found && remove_svtag_configuration)
    {

        /** In case SVTAG configuration is found and remove_svtag_configuration is TRUE */
        /** Simulate exchange to not have SVTAG configuration to see if we get one of the default PP configurations */
        SHR_IF_ERR_EXIT(dnx_switch_svtag_esem_cmd_exchange
                        (unit, gport_dummy, old_esem_cmd, FALSE /* enable */ , DNX_PORT_ESEM_CMD_EXCHANGE_TEST,
                         &new_esem_cmd, &exchange_and_set_hw, &esem_cmd_data, &is_first, &is_last));

    }
    /** If the old ESEM command is not one of the default commands then there is an uncleared configuration */
    if (!DNX_PORT_ESEM_CMD_IS_DEFAULT_PP_PORT_CONFIGURATION(new_esem_cmd))
    {

        SHR_ERR_EXIT(_SHR_E_FAIL, "PP port %d on core %d, has uncleared ESEM configuration,"
                     "please clear the ESEM configuration before calling this API", pp_port, core);
    }

    if (found && remove_svtag_configuration)
    {
        /** If it is configure new command data */
        /** Is first is not dealt with because we should receive a default pre-allocated configuration at this point */
        SHR_IF_ERR_EXIT(dnx_switch_svtag_esem_cmd_exchange(unit, gport_dummy, old_esem_cmd, FALSE /* enable */ ,
                                                           0 /* Flags */ , &new_esem_cmd, &exchange_and_set_hw,
                                                           &esem_cmd_data, &is_first, &is_last));
        if (is_last)
        {
            *free_profile = old_esem_cmd;
        }
    }
...
/**
 * \brief - Configure EGRESS_PP_PORT table and exchange ESEM command
  *
 * \param [in] unit - Unit number
 * \param [in] pp_port - PP port
 * \param [in] core - Core ID
 ...
 */
static shr_error_e
dnx_port_pp_egress_per_pp_port_set(
    int unit,
    uint32 pp_port,
    bcm_core_t core,
    dnx_port_pp_egress_port_info_t * egress_port_info,
    uint8 remove_svtag_configuration)
{
    int free_profile = DNX_PORT_PP_INVALID_PROFILE;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_port_pp_egress_hw_esem_cmd(unit, pp_port, core, remove_svtag_configuration, &free_profile));

    SHR_IF_ERR_EXIT(dnx_port_pp_egress_hw_per_pp_port_set(unit, pp_port, core, egress_port_info));

    if (free_profile != DNX_PORT_PP_INVALID_PROFILE)
    {
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_data_clear(unit, free_profile));
    }
...
/**
 * \brief - Initialization function for the egress per port filters enable and trap context port profile
 *
 * \param [in] unit - Relevant unit
 * \param [in] port - Port id
 * \param [in] remove_svtag_configuration - Switch header type value according to BCM_SWITCH_PORT_HEADER_TYPE_XXX
 * \param [in] egress_port_info - info for the initialization
 *
 * \return
 *   shr_error_e
 *
    ...
    dnx_port_pp_egress_port_info_t * egress_port_info)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        SHR_IF_ERR_EXIT(dnx_port_pp_egress_per_pp_port_set(unit,
                                                           gport_info.internal_port_pp_info.pp_port[pp_port_index],
                                                           gport_info.internal_port_pp_info.core_id[pp_port_index],
                                                           egress_port_info, remove_svtag_configuration));
    }

    if (dnx_data_port_pp.filters.feature_get
        (unit, dnx_data_port_pp_filters_rqp_discard_and_invalid_otm_filters_support))
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PP_ATTRIBUTES_PER_PP_DSP_TABLE, &entry_handle_id));
        for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_pp_to_pp_dsp_pbmp_get(unit,
                                                                gport_info.internal_port_pp_info.core_id[pp_port_index],
                                                                gport_info.internal_port_pp_info.pp_port[pp_port_index],
                                                                &pbmp_pp_dsp));
            BCM_PBMP_ITER(pbmp_pp_dsp, pp_dsp)
            {
                dbal_entry_key_field16_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID,
                                           gport_info.internal_port_pp_info.core_id[pp_port_index]);
    ...
    int switch_header_type)
{
    ...
    egress_port_info.estimated_btr_system_headers_termination_enable = TRUE;
    egress_port_info.estimated_btr_network_headers_termination_enable = TRUE;
    system_headers_mode = dnx_data_headers.system_headers.system_headers_mode_get(unit);
    ...
    switch (switch_header_type)
    {
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_RAW_WITH_ORIGINAL_SYSTEM_HEADERS:
        case BCM_SWITCH_PORT_HEADER_TYPE_RCH_SRV6_USP_PSP:
        {
            if (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_RAW_WITH_ORIGINAL_SYSTEM_HEADERS)
            {
                egress_port_info.trap_context_port_profile = DBAL_ENUM_FVAL_ETPP_TRAP_CONTEXT_PORT_PROFILE_APPEND_64B;
            }
            ...
            egress_port_info.fwd_code_port_profile = (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_RAW
                                                      || switch_header_type ==
                                                      BCM_SWITCH_PORT_HEADER_TYPE_RAW_WITH_ORIGINAL_SYSTEM_HEADERS) ?
                DBAL_ENUM_FVAL_OUT_PP_PORT_EGRESS_FORWARD_CODE_SELECTION_PROFILE_RAW_PROCESSING :
                DBAL_ENUM_FVAL_OUT_PP_PORT_EGRESS_FORWARD_CODE_SELECTION_PROFILE_NORMAL;

            if (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_RAW
                || switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_RAW_WITH_ORIGINAL_SYSTEM_HEADERS)
            {
                egress_port_info.estimated_btr_system_headers_termination_enable = TRUE;
                egress_port_info.estimated_btr_network_headers_termination_enable = FALSE;
                egress_port_info.prp2_port_profile = DBAL_ENUM_FVAL_ETPP_PRP2_PORT_CS_VAR_CANCEL_SYSHDR_ENC;
            }

            if ((switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_RCH_SRV6_USP_PSP)
                && dnx_data_dev_init.general.feature_get(unit,
                                                         dnx_data_dev_init_general_egress_estimated_bta_btr_hw_config))
            {
                ...
                {
                    egress_port_info.estimated_btr = DNX_PORT_PP_ESTIMATED_BTR_SRV6_JR2_COMP_MODE;
                }
                else
                {
                    egress_port_info.estimated_btr = DNX_PORT_PP_ESTIMATED_BTR;
                }
            }
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_ENCAP_EXTERNAL_CPU:
        {
            ...
            egress_port_info.fwd_code_port_profile =
                DBAL_ENUM_FVAL_OUT_PP_PORT_EGRESS_FORWARD_CODE_SELECTION_PROFILE_CPU_PORT;
            ...
            egress_port_info.estimated_btr_system_headers_termination_enable = FALSE;
            egress_port_info.estimated_btr_network_headers_termination_enable = FALSE;
            break;
        }
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_IBCH1_MODE:
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_SWITCH_PORT_HEADER_TYPE_IBCH1_MODE is not supported on egress!");
        }
        default:
        {
    ...
    if (count_oamp > 0 && (!BCM_GPORT_IS_TRUNK(port)))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_port_oamp_v1_ptch_header_init
                            (unit, (uint32) gport_info_oamp.internal_port_pp_info.pp_port[0]));
        }
    ...
    SHR_IF_ERR_EXIT(dnx_port_pp_egress_hw_set(unit, port, FALSE /* remove_svtag_configuration */ , &egress_port_info));
 ...
 */
shr_error_e
dnx_port_pp_egress_per_pp_port_unset(
    int unit,
    uint32 pp_port,
    bcm_core_t core_id,
    int switch_header_type,
    ...
    uint8 remove_svtag_configuration)
{
    ...
    if (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_NONE)
    {
        SHR_EXIT();
    }
    ...
    switch (switch_header_type)
    {
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_MPLS_RAW:
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_port_tpid_class_egress_default_per_pp_port_deinit
                            (unit, pp_port, core_id, tpid_ref_dealloc));
            break;
        }
        case BCM_SWITCH_PORT_HEADER_TYPE_INJECTED_2_PP:
    ...
    egress_port_info.forward_port_profile = DBAL_ENUM_FVAL_ETPP_FORWARD_PORT_CS_VAR_ETHERNET_TYPICAL;
    egress_port_info.enc1_port_profile = DBAL_ENUM_FVAL_ETPP_ENC1_PORT_CS_VAR_ETHERNET_TYPICAL;
    egress_port_info.enc2_port_profile = DBAL_ENUM_FVAL_ETPP_ENC2_PORT_CS_VAR_ETHERNET_TYPICAL;
    egress_port_info.enc3_port_profile = DBAL_ENUM_FVAL_ETPP_ENC3_PORT_CS_VAR_ETHERNET_TYPICAL;
    egress_port_info.enc4_port_profile = DBAL_ENUM_FVAL_ETPP_ENC4_PORT_CS_VAR_ETHERNET_TYPICAL;
    egress_port_info.enc5_port_profile = DBAL_ENUM_FVAL_ETPP_ENC5_PORT_CS_VAR_ETHERNET_TYPICAL;
    egress_port_info.exclude_src_enable = FALSE;

    SHR_IF_ERR_EXIT(dnx_port_pp_egress_per_pp_port_set(unit, pp_port, core_id, &egress_port_info,
                                                       remove_svtag_configuration));

    /** Remove PP port properties */
    SHR_IF_ERR_EXIT(dnx_port_packet_processing_properties_remove_internal(unit, pp_port, core_id, FALSE, TRUE));
    ...
    int switch_header_type)
{
    ...
    {
        /*
         * egress_acceptable_frame_type_profile, port_pp_tpid_profile_egress and esem_cmd template managers should remove only
         * one reference in case of LAG. Do it when the function is accessed by the first pp port in LAG
         */
        SHR_IF_ERR_EXIT(dnx_port_pp_egress_per_pp_port_unset
                        (unit, gport_info.internal_port_pp_info.pp_port[pp_port_index],
                         gport_info.internal_port_pp_info.core_id[pp_port_index], switch_header_type, ref_dealloc,
                         FALSE));
        ref_dealloc = FALSE;
    }
    ...
    uint32 header_type)
{
    ...
    if ((header_type != BCM_SWITCH_PORT_HEADER_TYPE_ETH) && (header_type != BCM_SWITCH_PORT_HEADER_TYPE_TDM_ETH)
        ...
        && (header_type != BCM_SWITCH_PORT_HEADER_TYPE_IBCH1_MODE)
    ...
    uint32 header_type)
{
    ...
    uint32 pp_port_index;
    int system_headers_mode = 0;
    uint32 prt_qualifier = DBAL_ENUM_FVAL_PRT_QUALIFIER_DEFAULT;
    dbal_enum_value_field_port_termination_ptc_profile_e old_prt_ptc_profile = 0;
    int ref_count = 0;
    dnx_algo_skip_first_header_info_t profile_data;
    uint32 new_ptc_profile_ud_port_type = 0;
    uint32 ptc_profile_overwrite = 1;
    uint32 flags = 0;
    uint32 parser_context = DBAL_ENUM_FVAL_IRPP_1ST_PARSER_PARSER_CONTEXT_ITMH_A1;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_prt_ptc_profile_internal_set_verify(unit, gport_info, header_type));

    switch (header_type)
    {
        case BCM_SWITCH_PORT_HEADER_TYPE_ETH:
        {
            SHR_IF_ERR_EXIT(dnx_port_sit_port_flag_get(unit, gport_info->local_port, &flags));
            if (DNX_PORT_IS_COE_PORT(flags))
            {
                port_termination_ptc_profile = DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_COE;
            }
            else
            {
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_RAW:
        {
            ...
            {
                port_termination_ptc_profile = DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_COE;
                prt_qualifier = DBAL_ENUM_FVAL_PRT_QUALIFIER_COE_RESERVED;
            }
            else
            {
                port_termination_ptc_profile = DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_RAW;
        ...
        case BCM_SWITCH_PORT_HEADER_TYPE_INJECTED_2_PP_JR1_MODE:
        {
            ...
            if (dnx_data_port_pp.general.feature_get
                (unit, dnx_data_port_pp_general_prt_qualifier_injected_2_jr1_reserved_parsing_context_support))
            {
                prt_qualifier = DNX2_INJECTED_2_J1_PARSING_CONTEXT_MSBS_RESERVED;
            }
            break;
        }
        case BCM_SWITCH_PORT_HEADER_TYPE_INJECTED_2:
    ...
    BCM_PBMP_ITER(gport_info->local_port_bmp, local_port)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_first_header_size_to_skip_port_type_from_port_header_type_convert
                        (unit, local_port, header_type, &new_ptc_profile_ud_port_type));
            ...
            || (old_prt_ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_UD6))
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_db.
                            skip_first_user_defined_header_size_profile_data.profile_data_get(unit, old_prt_ptc_profile,
                                                                                              &ref_count,
                                                                                              &profile_data));
            if (profile_data.port_type == new_ptc_profile_ud_port_type)
            {
                /** In case that the user defined header is from the same type as the pre_defined header, the ptc should not be changed */
                ptc_profile_overwrite = 0;
            }
        }
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PTC_PORT, &entry_handle_id));
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PRT_QUALIFIER, INST_SINGLE, prt_qualifier);
        if ((header_type == BCM_SWITCH_PORT_HEADER_TYPE_MPLS_RAW) &&
            (dnx_data_mpls.general.feature_get(unit, dnx_data_mpls_general_mpls_raw_vtt_handling_supported)))
        {
            /*
             ...
             */
            if (dnx_data_l3.routing_enablers.feature_get
                (unit, dnx_data_l3_routing_enablers_ptc_routing_enable_profile_support))
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_VTT1,
                                             INST_SINGLE,
                 ...
                 || (header_type == BCM_SWITCH_PORT_HEADER_TYPE_RCH_SRV6_USP_PSP))
        {
            ...
            {
                parser_context = DBAL_ENUM_FVAL_IRPP_1ST_PARSER_PARSER_CONTEXT_RCH_SRV6_USP_PSP;
            }
        }
        else if (header_type == BCM_SWITCH_PORT_HEADER_TYPE_STACKING)
        {
            /*
             * Set llr ptc profile as FTMH, so VT1 FTMH context will be selected
             */
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PTC_CS_PROFILE_VTT1,
                                         INST_SINGLE, DBAL_ENUM_FVAL_PTC_CS_PROFILE_VTT1_FTMH);
            /*
            ...
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_VTT1,
                                             INST_SINGLE, DBAL_ENUM_FVAL_PTC_ROUTING_ENABLE_PROFILE_VTT1_DEFAULT);
            }
        }
        else
        {
        ...
        if (!DNX_PORT_IS_COE_PORT(flags))
        {
            for (pp_port_index = 0; pp_port_index < gport_info->internal_port_pp_info.nof_pp_ports; pp_port_index++)
            {
                ...
                else if (gport_info->internal_port_pp_info.nof_pp_ports - 1 == pp_port_index)
                {
                    ...
                    SHR_ERR_EXIT(_SHR_E_PARAM, "No PP port corresponds to PTC %d on core %d", ptc, tm_core_id);
                }
            }
        }
    }
...
* \param [out] is_generic_data_1_only - indicates that only field GENERIC_DATA_1 exist.
    ...
    *is_generic_data_1_only = FALSE;
    ...
    if (gport_hw_resources->inlif_dbal_table_id != DBAL_TABLE_INNER_ETH_VLAN_EDIT_CLASSIFICATION &&
        ...
        (gport_hw_resources->inlif_dbal_table_id != DBAL_TABLE_IN_LIF_FORMAT_PWE ||
         !dnx_data_lif.feature.feature_get(unit, dnx_data_lif_feature_in_lif_tunnel_wide_data)) &&
    ...
    {
        if (dnx_data_lif.feature.feature_get(unit, dnx_data_lif_feature_in_lif_tunnel_wide_data))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "generic data is supported only for AC/PWE/IPvx_Tun IN-LIF types \n");
        }
        else
        {
    ...
    if (rv == _SHR_E_NOT_FOUND)
    {
        if (dbal_tables_field_info_get_no_err
            (unit, gport_hw_resources->inlif_dbal_table_id, DBAL_FIELD_LIF_GENERIC_DATA_1, 0,
             gport_hw_resources->inlif_dbal_result_type, 0, &dbal_info_tmp) == _SHR_E_NOT_FOUND)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "generic data is not supported for the given gport \n");
        }
        else
        {
            *is_generic_data_1_only = TRUE;
        }
    }
...
{
    dbal_table_field_info_t dbal_info;
    int is_generic_data_1_only = 0;
    uint32 mask_not = 0;
    SHR_FUNC_INIT_VARS(unit);

    /** Verify parameters for flags and gport information */
    sal_memset(&dbal_info, 0, sizeof(dbal_table_field_info_t));
    SHR_IF_ERR_EXIT(dnx_port_pp_wide_data_common_verify
                    (unit, gport_hw_resources, flags, &is_generic_data_1_only, &dbal_info));

    if (is_generic_data_1_only == FALSE)
    {
    ...
    {
        /*
         * if only GENERIC_DATA_1 exist, it means that need to copy just the user data that belongs to GENERIC_DATA_1
         * (bits 16 and above up to the the size of GENERIC_DATA_1). 
         */
        mask_not = (~(((1 << dbal_info.field_nof_bits) - 1) <<
                      dnx_data_lif.in_lif.generic_data_1_start_position_get(unit)));
        if ((COMPILER_64_HI(data) != 0) || ((mask_not & COMPILER_64_LO(data)) != 0))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "data={0x%X, 0x%X}. Only bits {%d...%d} allowed to set.\n",
                         COMPILER_64_HI(data), COMPILER_64_LO(data),
                         dnx_data_lif.in_lif.generic_data_1_start_position_get(unit),
                         (dbal_info.field_nof_bits + dnx_data_lif.in_lif.generic_data_1_start_position_get(unit)));
        }
    }
...
*
* \param [in] unit - Unit ID
* \param [in] gport_hw_resources - GPORT hardware resources information
* \param [out] data - wide data
* \param [out] glob_lif_data - global inlif and PMF additional data, could be NULL if needn't this output
*
* \return
*   See shr_error_r
* \remark
    ...
    uint32 *glob_lif_data)
{
    ...
    type = gport_hw_resources.inlif_dbal_result_type;
    ...
    if (!SHR_FAILURE(rv))
    {
        rv = dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_LIF_GENERIC_DATA_0,
    ...
    if (is_generic_data_1_only)
    {
        /*
         * if only generic_data_1 exists, it means that the user data contains only FIELD CS info, in its specific
         * position, and the rest is zero. Need to align it back to its postion. 
         */
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_1, INST_SINGLE, &data_tmp));
        *data = (data_tmp << dnx_data_lif.in_lif.generic_data_1_start_position_get(unit));
    }
    else
    {
        rv = dbal_tables_field_info_get_no_err(unit, table_id, DBAL_FIELD_LIF_GENERIC_DATA_1, 0, type, 0,
                                               &field_info_1);
        if (!SHR_FAILURE(rv))
        {
    ...
    uint32 data)
{
    ...
    int local_in_lif;
    shr_error_e rv;
    int is_generic_data_1_only = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    if (is_generic_data_1_only)
    {
        /** if only generic_data_1 exists, copy only the user data that belong to generic_data_1.*/
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_1, INST_SINGLE,
                                     (data >> dnx_data_lif.in_lif.generic_data_1_start_position_get(unit)));
    }
    else
    {
        /*
    ...
    uint64 data)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_wide_data_set_flow(unit, gport, flags, data));
        SHR_EXIT();
    }
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_wide_data_set_verify(unit, &gport_hw_resources, flags, data));

    /** write to HW the data. */
    SHR_IF_ERR_EXIT(dnx_port_pp_wide_data_hw_set(unit, gport_hw_resources, COMPILER_64_LO(data)));
    ...
    uint64 *data)
{
    dnx_algo_gpm_gport_hw_resources_t gport_hw_resources;
    uint32 value = 0;
    uint32 global_lif_data = 0;
    bcm_gport_t gport_hw_resources_get = gport;
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, gport, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_TERMINATOR,
                         &gport_hw_resources_get));
    }
    /** get the local in-lif hw resource associated to the gport */
    SHR_IF_ERR_REPLACE_AND_EXIT(dnx_algo_gpm_gport_to_hw_resources
                                (unit, gport_hw_resources_get, (DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS |
                                                                DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS),
                                 &gport_hw_resources), _SHR_E_NOT_FOUND, _SHR_E_PARAM);

    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_wide_data_get_verify(unit, &gport_hw_resources, flags, data));

    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(dnx_wide_data_get_flow(unit, gport, flags, data));
        SHR_EXIT();
    }

    /** get from HW the data. */
    SHR_IF_ERR_EXIT(dnx_port_pp_wide_data_hw_get(unit, gport_hw_resources, &value, &global_lif_data));
...
{
    uint8 bc_same_as_unknown_mc;
    uint8 no_trap_for_unknown_destination;
    ...
    if (!DNX_PORT_FLOOD_DEST_IS_LEGAL(flood_groups->unknown_unicast_group) ||
        !DNX_PORT_FLOOD_DEST_IS_LEGAL(flood_groups->unknown_multicast_group) ||
        !DNX_PORT_FLOOD_DEST_IS_LEGAL(flood_groups->broadcast_group))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Only black hole, multicast, trap, FEC and invalid gport are supported!\n");
    }

    no_trap_for_unknown_destination =
        dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_no_trap_for_unknown_destination);
    if (no_trap_for_unknown_destination
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Trap for default destination is not supported in current device!\n");
    }

    no_black_hole_for_unknown_destination =
    ...
    if (no_black_hole_for_unknown_destination
        && (BCM_GPORT_IS_BLACK_HOLE(flood_groups->unknown_unicast_group)
            || BCM_GPORT_IS_BLACK_HOLE(flood_groups->unknown_multicast_group)
            || BCM_GPORT_IS_BLACK_HOLE(flood_groups->broadcast_group)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Black hole can not be set as a destination in current device!\n");
    }
...
/**
 * \brief -
 *  See the .h for detail
 */
shr_error_e
dnx_port_flood_dst_gport_to_dst_offset(
    int unit,
    ...
    dnx_default_frwrd_dst_t * default_frwrd_dst)
{
     ...
     */
    if (BCM_GPORT_IS_BLACK_HOLE(dst_gport))
    {
        default_frwrd_dst->add_vsi_dst = FALSE;
        default_frwrd_dst->offset = 0;
        SHR_IF_ERR_EXIT(algo_gpm_encode_destination_field_from_gport
                        (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, BCM_GPORT_BLACK_HOLE,
                         &default_frwrd_dst->destination));
    }
    else if (BCM_GPORT_IS_TRAP(dst_gport) || BCM_GPORT_IS_FORWARD_PORT(dst_gport))
    {
        default_frwrd_dst->add_vsi_dst = FALSE;
        default_frwrd_dst->offset = 0;
        SHR_IF_ERR_EXIT(algo_gpm_encode_destination_field_from_gport
                        (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, dst_gport, &default_frwrd_dst->destination));
    }
    else
    {
        default_frwrd_dst->add_vsi_dst = TRUE;
        default_frwrd_dst->offset = _BCM_MULTICAST_ID_GET(dst_gport);
        default_frwrd_dst->destination = 0;
    }
    ...
    dnx_default_frwrd_dst_t * default_frwrd_dst)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_port_flood_dst_gport_to_dst_offset
                        (unit, defaut_dst_offset[da_type], &default_frwrd_dst[da_type]));
    }
 ...
 */
shr_error_e
dnx_port_flood_dst_gport_from_dst_offset(
    int unit,
    dnx_default_frwrd_dst_t default_frwrd_dst,
    bcm_gport_t * dst_gport)
{
    ...
    SHR_NULL_CHECK(dst_gport, _SHR_E_PARAM, "A valid pointer is needed!");

    /** Translate the destination to gport*/
    if (default_frwrd_dst.add_vsi_dst == FALSE)
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_from_encoded_destination_field
                        (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, default_frwrd_dst.destination, dst_gport));
    }
    else
    {
        /** If it's offset to destination, it must be mc-id*/
        BCM_GPORT_MCAST_SET(*dst_gport, default_frwrd_dst.offset);
    }

    /** Check the data retrieved is valid */
    if ((default_frwrd_dst.add_vsi_dst == FALSE) &&
        ((*dst_gport != BCM_GPORT_BLACK_HOLE) && !BCM_GPORT_IS_TRAP(*dst_gport)
         && !BCM_GPORT_IS_FORWARD_PORT(*dst_gport)))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Invalid flooding destination gport (0x%08X) is found!\n", *dst_gport);
    }
    ...
    bcm_port_flood_group_t * flood_groups)
{
    ...
    {
        /** Translate the destination to gport*/
        SHR_IF_ERR_EXIT(dnx_port_flood_dst_gport_from_dst_offset
                        (unit, default_frwrd_dst[da_type], &defaut_dst_offset[da_type]));
    }
    ...
    bcm_port_flood_group_t * flood_groups)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_flood_group_set_verify(unit, port, is_phy_port, flags, flood_groups));
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_port_pp_flow_lif_flood_set(unit, port, flood_groups));
            SHR_EXIT();
        }
    ...
    bcm_port_flood_group_t * flood_groups)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_is_physical(unit, port, &is_phy_port));
    SHR_INVOKE_VERIFY_DNXC(dnx_port_flood_group_get_verify(unit, port, is_phy_port, flags, flood_groups));
    ...
    else
    {
        ...
        if ((dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications) &&
            ...
            (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications)
             && BCM_GPORT_IS_TUNNEL(port)))
        {
            SHR_IF_ERR_EXIT(dnx_port_pp_flow_lif_flood_get(unit, port, flood_groups));
            SHR_EXIT();
        }
    ...
    bcm_port_t logical_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_switch_header_type_in_get(unit, logical_port, FALSE, &switch_port_header_type));
    ...
    int forward_context_port_profile)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_pp_term_context_port_profile_set_verify(unit, port, term_context_port_profile));
    ...
    int *enabled)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_force_forward_get_verify(unit, port, egr_port, enabled));
    ...
    *enabled = (trap_id == -1) ? 0 : 1;
    ...
    if (*enabled)
    {
        if (trap_index != -1)
        {
            ...
            {
                if (BCM_GPORT_IS_TRUNK(trap_config.dest_port))
                {
    ...
    int enable)
{
    ...
    {
        SHR_IF_ERR_EXIT(algo_gpm_encode_destination_field_from_gport
                        (unit, ALGO_GPM_ENCODE_DESTINATION_ASYM_VOQ, egr_port, &trap_data));
    }
    ...
    int enable)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_force_forward_set_verify(unit, port, egr_port, enable));

    nof_trap_ids = dnx_data_trap.ingress.nof_entries_action_table_get(unit);
        ...
        if (enable)
        {
            ...
            else
            {
                ...
                if (is_first)
                {
                    ...
                    /** set forward action */
                    bcm_rx_trap_config_t_init(&trap_config);
                    sal_snprintf((char *) trap_config.name, BCM_RX_TRAP_MAX_NAME_LEN, "FORCE_FORWARD_FROM_%d_TO_%d",
                                 port, egr_port);
            ...
            if ((prev_trap_index != -1) && (prev_trap_id < nof_trap_ids))
            {
    ...
    uint8 *is_last)
{
    ...
    SHR_IF_ERR_EXIT(algo_port_pp_db.pp_port_lb_profile.exchange
                    (unit, 0, 1, &pp_lb_profile_enablers_vector, old_port_profile, NULL, new_port_profile, is_first,
                     is_last));
...
/**
 *  Enable/Disable SA multicast ingress trap per port
 */
shr_error_e
dnx_port_pp_sa_mc_trap_set(
    int unit,
    bcm_port_t port,
    int enable)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        /** Set values */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FILTER_SA_MC_ENABLE, INST_SINGLE, enable);
...
/**
 *  Get the value of Enable/Disable SA multicast ingress trap per port
 */
shr_error_e
dnx_port_pp_sa_mc_trap_get(
    int unit,
    bcm_port_t port,
    int *enable)
{
    ...
    /** Get the trap enabler value */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_IRPP_LLR_CONTEXT_PROPERTIES, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_PP_PORT,
                               gport_info.internal_port_pp_info.pp_port[0]);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, gport_info.internal_port_pp_info.core_id[0]);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_FILTER_SA_MC_ENABLE, INST_SINGLE, &sa_mc_en);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int size)
{
    ...
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ESTIMATE_DELTA, &entry_handle_id));

        if (BCM_GPORT_IS_TRUNK(port))
        {
            /**set for each member port*/
            DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
            {
                bcm_pbmp_t local_port_pbmp_per_core;
                bcm_port_t member_local_port;
                bcm_core_t member_core;

                BCM_PBMP_CLEAR(local_port_pbmp_per_core);
                /**get local member on this core*/
                SHR_IF_ERR_EXIT(dnx_trunk_local_port_bmp_get
                                (unit, BCM_GPORT_TRUNK_GET(port), core, &local_port_pbmp_per_core));
                BCM_PBMP_ITER(local_port_pbmp_per_core, member_local_port)
                {
                    SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get(unit, member_local_port, &member_core, &out_tm_port));
                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OUT_TM_PORT, out_tm_port);
                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, member_core);
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESTIMATE_BTA, INST_SINGLE, size);
                    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
                }
            }
        }
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get
                            (unit, BCM_GPORT_IS_SET(port) ? BCM_GPORT_LOCAL_GET(port) : port, &core, &out_tm_port));
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OUT_TM_PORT, out_tm_port);
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESTIMATE_BTA, INST_SINGLE, size);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/** see port_pp.h for more information */
shr_error_e
dnx_port_estimate_header_mode_set(
    int unit,
    bcm_port_t port,
    int out_header_type)
{
    ...
    bcm_core_t core;
    uint32 out_tm_port;
    uint8 sys_header_mode, net_header_mode;
    dnx_algo_gpm_gport_phy_info_t gport_info;
    bcm_port_t local_port;
    int estimated_btr = 0;
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_egress_estimated_bta_btr_hw_config))
    {
        /**header mode depends on out header type*/
        sys_header_mode = TRUE;
        net_header_mode = TRUE;

        if ((out_header_type == BCM_SWITCH_PORT_HEADER_TYPE_RAW)
            || (out_header_type == BCM_SWITCH_PORT_HEADER_TYPE_RAW_WITH_ORIGINAL_SYSTEM_HEADERS))
        {
            sys_header_mode = TRUE;
            net_header_mode = FALSE;
        }
        else if ((out_header_type == BCM_SWITCH_PORT_HEADER_TYPE_CPU)
                 || (out_header_type == BCM_SWITCH_PORT_HEADER_TYPE_ENCAP_EXTERNAL_CPU))
        {
            sys_header_mode = FALSE;
            net_header_mode = FALSE;
        }

        if ((out_header_type == BCM_SWITCH_PORT_HEADER_TYPE_RCH_SRV6_USP_PSP)
            && dnx_data_dev_init.general.feature_get(unit,
                                                     dnx_data_dev_init_general_egress_estimated_bta_btr_hw_config))
        {
            if (dnx_data_srv6.general.feature_get(unit, dnx_data_srv6_general_jr2_compatible))
            {
                estimated_btr = DNX_PORT_PP_ESTIMATED_BTR_SRV6_JR2_COMP_MODE;
            }
            else
            {
                estimated_btr = DNX_PORT_PP_ESTIMATED_BTR;
            }
        }
        ...
        /**set only when local port*/
        if (gport_info.flags & DNX_ALGO_GPM_GPORT_INFO_F_IS_LOCAL_PORT)
        {
            if (gport_info.flags & DNX_ALGO_GPM_GPORT_INFO_F_IS_LAG)
            {
                BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
                {
                    SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get(unit, local_port, &core, &out_tm_port));
                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OUT_TM_PORT, out_tm_port);
                    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESTIMATE_BTR, INST_SINGLE,
                                                 estimated_btr);
                    dbal_entry_value_field32_set(unit, entry_handle_id,
                                                 DBAL_FIELD_ESTIMATE_BTR_TERMINATE_SYSTEM_HEADERS, INST_SINGLE,
                                                 sys_header_mode);
                    dbal_entry_value_field32_set(unit, entry_handle_id,
                                                 DBAL_FIELD_ESTIMATE_BTR_TERMINATE_NETWORK_HEADERS, INST_SINGLE,
                                                 net_header_mode);
                    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
                }
            }
            else
            {
                ...
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESTIMATE_BTR, INST_SINGLE,
                                             estimated_btr);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESTIMATE_BTR_TERMINATE_SYSTEM_HEADERS,
                                             INST_SINGLE, sys_header_mode);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESTIMATE_BTR_TERMINATE_NETWORK_HEADERS,
                                             INST_SINGLE, net_header_mode);
                SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
            }
        }
    }
...
/**
 * \brief - Verify function for dnx_vlan_domain_port_pp_get_verify
 */
static shr_error_e
dnx_vlan_domain_port_pp_get_verify(
    int unit,
    uint32 vlan_domain,
    bcm_port_t * port)
{
 ...
 */
shr_error_e
dnx_vlan_domain_port_pp_get(
    int unit,
    uint32 vlan_domain,
    bcm_port_t * port)
{
    uint32 entry_handle_id;
    uint32 vlan_domain_get;
    uint32 pp_port;
    bcm_core_t pp_port_core;
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_domain_port_pp_get_verify(unit, vlan_domain, port));

    number_of_ports = dnx_data_port.general.nof_pp_ports_get(unit);

    found = 0;
    for (pp_port = 0; pp_port < number_of_ports; pp_port++)
    {
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, pp_port_core)
        {
            /*
             * Read INGRESS PORT table
             */
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PP_PORT, &entry_handle_id));

            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PP_PORT, pp_port);
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, pp_port_core);

            dbal_value_field_arr32_request(unit, entry_handle_id, DBAL_FIELD_VLAN_DOMAIN, INST_SINGLE,
                                           &vlan_domain_get);

            /*
             * getting the entry values
             */
            SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

            if (vlan_domain_get == vlan_domain)
            {
                found = 1;
                SHR_IF_ERR_EXIT(dnx_algo_port_pp_to_logical_get(unit, pp_port_core, pp_port, port));
                break;
            }
        }
        if (found == 1)
    ...
    if (pp_port == number_of_ports)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "No port was found!\n");
    }
