    ...
    int *is_required)
{
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(MGMT)))
    {
        *is_required = 1;
    }
    else
    ...
    int *channel)
{
    ...
    *channel = DNX_ALGO_PORT_INVALID;
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF)) ||
        DNX_ALGO_PORT_TYPE_IS_FRAMER_MAC(unit, port_info, 0, 0, 0))
    {
        *channel = priority;
    }
    else
    {
        ...
        if (is_channelized)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_in_channel_get(unit, port, channel));
        }
        else
        {
            *channel = DNX_ALGO_PORT_INVALID;
    ...
    uint32 *context_map_address)
{
    ...
    context_info = dnx_data_ingr_reassembly.context.context_map_get(unit, port_info.port_type, port_info.is_mgmt);
    ...
    switch (port_info.port_type)
    {
        ...
        case DNX_ALGO_PORT_TYPE_RCY_MIRROR:
            *context_map_address = context_map_start_address
                + priority * dnx_data_port.general.max_nof_mirror_channels_get(unit);
            break;
        default:
            *context_map_address = context_map_start_address;
            break;
    ...
    uint32 *port_termination_context)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_in_tm_port_get(unit, port, &core_id, port_termination_context));
    ...
    uint32 reassembly_context)
{
    ...
    /** Setting key fields */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_REASSEMBLY_CONTEXT_MAP_ADDR, context_map_address);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    ...
    int unit)
{
    ...
    for (interface_type = 0; interface_type < DBAL_NOF_ENUM_REASSEMBLY_SPECIAL_INTERFACE_TYPE_VALUES; interface_type++)
    {
        ...
        if (port_type != DNX_ALGO_PORT_TYPE_INVALID)
        {
            ...
            {
                max_nof_channels = dnx_data_port.general.max_nof_mirror_channels_get(unit);
            }
            else
            {
                max_nof_channels = dnx_data_port.general.max_nof_channels_get(unit);
            }
            context_info = dnx_data_ingr_reassembly.context.context_map_get(unit, port_type, 0);
            base_address = context_info->start_index + priority * max_nof_channels;
    ...
    int unit)
{
    ...
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_REASSEMBLY_CONTEXT_MAP_ADDR,
 ...
 */
static shr_error_e
dnx_port_ingr_reassembly_timeout_hw_init(
    int unit)
{
    uint32 entry_handle_id;
    uint32 timeout;
    ...
    timeout = dnx_data_ingr_reassembly.context.reassembly_timeout_get(unit);
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGR_REASSEMBLY_TIMEOUT, &entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_REASSEMBLY_TIMEOUT, INST_SINGLE, timeout);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_timeout_hw_init(unit));

    if (!dnx_data_ingr_reassembly.context.feature_get(unit, dnx_data_ingr_reassembly_context_direct_context_table))
    {
    ...
    uint32 reassembly_context)
{
    ...
    if (is_interleaved && !dnx_data_nif.ilkn.interleaved_error_drop_single_context_get(unit))
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, port, &core_id));
    ...
    int is_dealloc)
{
    ...
    if (is_dealloc)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 0) && is_interleaved)
        {
            int interleaved_id;
    ...
    if (is_ingress_channelized || is_dealloc)
    {
        ...
        if (dnx_data_ingr_reassembly.
            context.feature_get(unit, dnx_data_ingr_reassembly_context_port_termination_in_context_table))
        {
            SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_port_termination_hw_set(unit, port, context_map_address, 0));
        }
    }
    ...
    uint32 reassembly_context[DNX_DATA_MAX_INGR_REASSEMBLY_PRIORITY_CGM_PRIORITIES_NOF])
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info) && !dnx_data_ingr_reassembly.context.rcy_unique_context_get(unit))
    {
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_context_to_in_cgm_port_hw_get(
    int unit,
    int core,
    int reassembly_ctxt,
    uint32 *in_cgm_port)
{
    ...
    /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGR_REASSEMBLY_TO_IN_CGM_PORT_MAPPING, &entry_handle_id));
    ...
    /** Getting value fields */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_IN_CGM_PORT, INST_SINGLE, in_cgm_port);
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_context_to_in_cgm_port_hw_set(
    int unit,
    int core,
    int reassembly_ctxt,
    uint32 in_cgm_port)
{
    ...
    /** Setting value fields */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_CGM_PORT, INST_SINGLE, in_cgm_port);
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_context_to_in_cgm_port_hw_clear(
    int unit,
    int core,
    int reassembly_ctxt)
{
...
static shr_error_e
dnx_port_ingr_reassembly_context_power_down_set(
    int unit,
    int power_down)
{
    uint32 entry_handle_id;
   ...
   /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGR_REASSEMBLY_POWER_DOWN, &entry_handle_id));
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_context_in_cgm_port_get(
    int unit,
    bcm_port_t port,
    int *nof_entries,
    uint32 *in_cgm_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_for_cgm_get_all(unit, port, reassembly_context));

    for (prio = 0; prio < DNX_DATA_MAX_INGR_REASSEMBLY_PRIORITY_CGM_PRIORITIES_NOF; prio++)
    {
        if ((reassembly_context[prio] != DNX_PORT_INGR_REASSEMBLY_NON_INGRESS_PORT_CONTEXT)
            && (reassembly_context[prio] != invalid_context))
        {
            SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_to_in_cgm_port_hw_get
                            (unit, core_id, reassembly_context[prio], in_cgm_port));
            *nof_entries = 1;
            break;
        }
    }
 ...
 */
static shr_error_e
dnx_port_ingr_reassembly_context_in_cgm_port_set(
    int unit,
    bcm_port_t port,
    uint32 in_cgm_port)
{
    ...
    for (prio = 0; prio < DNX_DATA_MAX_INGR_REASSEMBLY_PRIORITY_CGM_PRIORITIES_NOF; prio++)
    {
        if ((reassembly_context[prio] != DNX_PORT_INGR_REASSEMBLY_NON_INGRESS_PORT_CONTEXT)
            && (reassembly_context[prio] != invalid_context))
        {
            SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_to_in_cgm_port_hw_set
                            (unit, core_id, reassembly_context[prio], in_cgm_port));
        }
    }
    ...
    int priority)
{
    ...
    if (call_alloc)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_reassembly_context_alloc
                        (unit, port, priority, &reassembly_context, &is_allocated));

        /** at the end -- write the context to the HW */
        SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_set(unit, port, priority, reassembly_context));

        if (dnx_data_ingr_reassembly.
            context.feature_get(unit, dnx_data_ingr_reassembly_context_reassembly_to_in_cgm_port_mapping))
        {
            if (is_allocated)
            {
                /** Allocate mapped port */
                SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_mapped_port_alloc(unit, port, priority, &mapped_port));

                /** write the allocated port to the HW */
                SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_in_cgm_port_set(unit, port, mapped_port));
            }
        }
    }
    ...
    int priority)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_ingr_reassembly_priority_verify(unit, port));
    ...
    if (reassembly_context != dnx_data_ingr_reassembly.context.invalid_context_get(unit))
    {
            ...
            context.feature_get(unit, dnx_data_ingr_reassembly_context_reassembly_to_in_cgm_port_mapping))
        {
            if (is_deallocated)
            {
                ...
                /** get mapped port */
                SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_to_in_cgm_port_hw_get
                                (unit, core, reassembly_context, &mapped_port));

                /** De-Allocate mapped port */
                SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_mapped_port_dealloc(unit, port, priority, mapped_port));

                /** on reassembly de-allocation - clear internal CGM port mapping */
                SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_to_in_cgm_port_hw_clear
                                (unit, core, reassembly_context));
            }
        }
    }
    ...
    int unit)
{
    ...
    {
        if (dnx_data_ingr_reassembly.general.feature_get(unit, dnx_data_ingr_reassembly_general_power_down_supported))
        {
            SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_context_power_down_set(unit, 1));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int *enable)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_port_ingr_reassembly_enable_verify(unit, port));
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1)))
    {
        ...
        /** Setting value fields */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DISABLE_SHORT_SOP_CHECK, INST_SINGLE, TRUE);
