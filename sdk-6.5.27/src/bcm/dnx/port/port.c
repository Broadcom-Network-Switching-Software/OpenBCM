    ...
    int orientation)
{
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_vsi_profile_get
                    (unit, port, (uint32 *) &old_in_lif_profile, &result_type, &is_ip_tunnel_l2vpn, LIF));

    /** If orientation is being set on non l2vpn tunnel - exit with error */
    if (!is_ip_tunnel_l2vpn)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Orientation should not be set on non L2VPN tunnel(0x%08X)\n", port);
    }
    /** Get the data stored for this in_lif_profile*/
    SHR_IF_ERR_EXIT(in_lif_profile_info_t_init(unit, &in_lif_profile_info));

    /** Since we have continued with the function execution, this is an L2VPN tunnel */
    ...
    uint32 first_header_size_value)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_db.skip_first_user_defined_header_size_profile_data.exchange(unit, flags, 1,
                                                                                               &port_first_header_size_data,
    ...
    uint32 is_set)
{
    ...
    switch (type)
    {
        ...
        case bcmPortControlPFCPassFrames:
        {
            ...
            if (((type == bcmPortControlPassControlFrames) || (type == bcmPortControlPFCPassFrames)))
            {
                if (!dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_pass_control_frames_support))
                {
                    SHR_ERR_EXIT(_SHR_E_PORT, "Type is not supported!\n");
                }
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PORT, "Type is supported only on MGMT ports!\n");
                }
            }
            break;
        }
    ...
    int value)
{
    ...
    switch (type)
    {
        case bcmPortControlLowLatencyPacketProcessing:
        {
            /** Switch control name which is in use for this feature should be updated. */
            dbal_enum_value_field_llr_cs_port_cs_profile_e llr_cs_profile;
            if (value != 0)
            {
                llr_cs_profile = DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_IPOE__AI_ML_OPTIMIZED;
            }
            else
            {
        ...
        case bcmPortControlMplsIpVXParseEnable:
        {
            uint8 is_set = (value != 0) ? TRUE : FALSE;
            /*
             * The API can be called only devices which parse IPvx over MPLS in VTT TCAM
             */
            if (!dnx_data_tunnel.feature.feature_get(unit, dnx_data_tunnel_feature_ipvx_tunnel_over_mpls_use_vtt_tcam))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "bcmPortControlMplsIpVXParseEnable is not supported on that device\n");
            }
            SHR_IF_ERR_EXIT(dnx_port_pp_ipvx_parsing_over_mpls_set(unit, port, is_set));
            break;
        }
        ...
        case bcmPortControlPassControlFrames:
        {
            ...
            port_local = (BCM_GPORT_IS_LOCAL(port)) ? BCM_GPORT_LOCAL_GET(port) : port;
            SHR_IF_ERR_EXIT(dnx_fc_port_pass_control_frames_set(unit, port_local, value));
            SHR_IF_ERR_EXIT(dnx_fc_port_pass_pause_frames_set(unit, port_local, value));
            break;
        }
        case bcmPortControlPFCPassFrames:
        {
            SHR_BOOL_VERIFY(value, _SHR_E_PARAM, "Value must be TRUE or FALSE");
            port_local = (BCM_GPORT_IS_LOCAL(port)) ? BCM_GPORT_LOCAL_GET(port) : port;
            SHR_IF_ERR_EXIT(dnx_fc_port_pass_pfc_frames_set(unit, port_local, value));
            break;
        }
        case bcmPortControlPFCTransmit:
        case bcmPortControlLLFCTransmit:
        ...
        {
            if (dnx_data_headers.
                general.feature_get(unit, dnx_data_headers_general_jericho_mode_sys_header_is_supported))
            {
                /**
                * At OLP TX:
    ...
    int *value_p)
{
    ...
    switch (type)
    {
        ...
        case bcmPortControlMplsIpVXParseEnable:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_port_pp_ipvx_parsing_over_mpls_get(unit, port, &is_set));
            *value_p = (is_set) ? 1 : 0;
            break;
        }
        case bcmPortControlEgressFilterDisable:
        ...
        {
            port_local = (BCM_GPORT_IS_LOCAL(port)) ? BCM_GPORT_LOCAL_GET(port) : port;
            /** No need to also get the pass PAUSE frames indication, it is the same */
            SHR_IF_ERR_EXIT(dnx_fc_port_pass_control_frames_get(unit, port_local, value_p));
            break;
        }
        case bcmPortControlPFCPassFrames:
        {
            port_local = (BCM_GPORT_IS_LOCAL(port)) ? BCM_GPORT_LOCAL_GET(port) : port;
            SHR_IF_ERR_EXIT(dnx_fc_port_pass_pfc_frames_get(unit, port_local, value_p));
            break;
        }
        case bcmPortControlPFCTransmit:
        case bcmPortControlLLFCTransmit:
...
/**
 * \brief - Configure a port for egress rate shaping in packet mode.
 * 
 * \param [in] unit - unit
 * \param [in] port - port
 * \param [in] pps - packets per second (0 disables the shaper)
 * \param [in] burst - Max packets at each period before enabling the shaper
 * \return int 
 */
int
 ...
 * \param [in] port - port
 * \param [in] pps - packets per second (0 disables the shaper)
 * \param [out] burst - Max packets at each period before enabling the shaper
 * \return int 
 */
int
...
/**
 * \brief -
 *  Register a callback function that will be invoked when an interrupt occurs.
 *
 * \param [in] unit - the relevant unit
 * \param [in] user_func - a callback function of prototype bcm_port_signal_quality_callback_t
...
/**
 * \brief -
 *  Unregister a callback function that will be invoked when an interrupt occurs.
 *
 * \param [in] unit - the relevant unit
 * \param [in] user_func - a callback function of prototype bcm_port_signal_quality_callback_t
 * \param [in] user_data - user custom data
    ...
    int is_input)
{
    ...
    if (!DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info) &&
        !DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, FEEDBACK)))
    {
    ...
    SHR_RANGE_VERIFY(lane_num, 0, nof_phys - 1, _SHR_E_PARAM, "lane_num is out of range.\n");
    /*
     * Verify event type
     */
    if (is_input)
    ...
    int *ns_port)
{
    ...
    _SHR_PBMP_ITER(phys, lane_id)
    {
        if (index == lane_num)
        {
            if (event_type == bcmGdpllPortEventRXCDR)
            {
                *ns_port = lane2serdes[lane_id].rx_id;
            }
            else if (event_type == bcmGdpllPortEventTXPI)
            {
                *ns_port = lane2serdes[lane_id].tx_id;
            }
            else
            {
...
/* Allocate port class per information */
int
bcm_dnx_port_class_allocation_create(
    int unit,
    bcm_port_class_allocation_info_t * class_info)
{
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
...
/* Get port class per information */
int
bcm_dnx_port_class_allocation_get(
    int unit,
    bcm_port_class_allocation_info_t * class_info)
{
    SHR_FUNC_INIT_VARS(unit);
...
/* Destroy port class per information */
int
bcm_dnx_port_class_allocation_destroy(
    int unit,
    bcm_port_class_allocation_info_t * class_info)
{
    SHR_FUNC_INIT_VARS(unit);
