    ...
    uint8 *is_last)
{
    ...
    while (rv != _SHR_E_NOT_FOUND)
    {
            ...
            (tmp_new_esem_cmd < extra_alloc_info.resource_alloc_info.range_end))
        {
            ...
            SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.exchange
                            (unit, flags, 1, &esem_cmd_data_i, esem_cmd_i, (void *) &extra_alloc_info, new_esem_cmd,
                             is_first, is_last));
    ...
    SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.exchange
                    (unit, flags, 1, esem_cmd_data, tmp_old_esem_cmd, (void *) &extra_alloc_info, new_esem_cmd,
                     is_first, is_last));
    ...
    int unit)
{
    ...
    {
        /*
         * Create ESEM command for SVTAG port, used by ARP+AC LIF and OAM reflector
         */
        esem_cmd_info.nof_accesses = 0;
        sal_memset(&esem_cmd_data, 0, sizeof(dnx_esem_cmd_data_t));
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_access_add(unit, 0, ESEM_ACCESS_TYPE_SVTAG,
                                                     ...
                                                     DNX_PORT_ESEM_DEFAULT_RESULT_PROFILE_INVALID, &esem_cmd_info));
        esem_cmd_alloc = dnx_data_esem.access_cmd.port_svtag_get(unit);
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_exchange(unit, flags, DNX_ALGO_ESEM_CMD_SUFFIX_SIZE_BIT_2
    ...
    uint8 *is_last)
{
    ...
    /** Convert ESEM command flags to allocation manager flags */
    algo_flags = _SHR_IS_FLAG_SET(flags, DNX_PORT_ESEM_CMD_ALLOCATE_WITH_ID) ? DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID : 0;
    algo_flags |= _SHR_IS_FLAG_SET(flags, DNX_PORT_ESEM_CMD_EXCHANGE_TEST) ? DNX_ALGO_TEMPLATE_EXCHANGE_TEST : 0;
    ...
    uint8 *is_last)
{
    ...
    *is_last = FALSE;
    if (free_needed == TRUE)
    {
        ...
        SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.exchange
                        (unit, 0, 1, &esem_cmd_data, esem_cmd, NULL, &tmp_esem_cmd, &is_first, is_last));
