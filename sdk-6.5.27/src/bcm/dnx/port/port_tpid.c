    ...
    uint32 flags)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_PORT_TPID_CLASS_EGRESS_IP_TUNNEL))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! BCM_PORT_TPID_CLASS_EGRESS_IP_TUNNEL (0x%08X) is not supported! flags = 0x%08X\n",
                     BCM_PORT_TPID_CLASS_EGRESS_IP_TUNNEL, flags);
    }
    ...
    if ((is_ingress_only == TRUE) && ((is_egress_native == TRUE) || (is_inner_c_tag == TRUE)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Wrong flags setting. INGRESS_ONLY(0x%08X) was combined with one of the Egress only flags: NATIVE_EVE(0x%08X)/INNER_C(0x%08X)!!! flags = 0x%08X\n",
                     BCM_PORT_TPID_CLASS_INGRESS_ONLY, BCM_PORT_TPID_CLASS_NATIVE_EVE,
                     BCM_PORT_TPID_CLASS_INNER_C, flags);
    }
    ...
    if ((is_outer_c_tag || is_inner_c_tag) && (is_egress_native))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Wrong flags setting. One of the C_TAG flags OUTER_C(0x%08X) or INNER_C(0x%08X) are combined at Egress with a Native flag NATIVE_EVE(0x%08X)!!! flags = 0x%08X\n",
                     BCM_PORT_TPID_CLASS_OUTER_C, BCM_PORT_TPID_CLASS_INNER_C, BCM_PORT_TPID_CLASS_NATIVE_EVE, flags);
    }
...
dnx_port_tpid_class_port_verify(
    ...
    bcm_gport_t port)
{
    int is_ingress_only, is_egress_only, is_ingress_native, is_egress_native;
    ...
    if ((is_ingress_native == TRUE) || (is_egress_native == TRUE))
    {
        /*
         * Native IVE/EVE ; port is irrelevant, verify that it is not set:
         */
        if (port != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Wrong port setting. For Ingress Native / Egress Native, port is irrelevent thus must be set to zero! port = 0x%08X flags = 0x%08X\n",
                         port, flags);
        }
    }
    else
    ...
    uint32 acceptable_frame_type_profile)
{
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ACCEPTABLE_FRAME_TYPE_PROFILE_ERPP, INST_SINGLE,
                                     acceptable_frame_type_profile);
    }
    ...
    bcm_gport_t logical_port)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(algo_port_pp_db.port_pp_tpid_profile_ingress.exchange
                        (unit, 0, 1, default_template_data, old_llvp_profile, NULL, &llvp_profile,
    ...
    if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_acceptable_frame_type_filter_support))
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(algo_port_pp_db.egress_acceptable_frame_type_profile.exchange
                            (unit, 0, 1, &default_acceptable_frame_type_template,
    ...
    bcm_port_tpid_class_t * tpid_class)
{
    ...
    uint8 is_ingress, is_egress, is_ingress_native, is_egress_native;
    ...
    if ((is_ingress == TRUE) && (is_ingress_native == FALSE))
    {
        ...
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(algo_port_pp_db.port_pp_tpid_profile_ingress.exchange
                            (unit, 0, 1, tpid_class_buffer, old_llvp_profile, NULL, &new_llvp_profile,
                             &first_reference, &is_last));
        }
        /*
     ...
     */
    if ((is_egress == TRUE) && (is_egress_native == FALSE))
    {
        /*
         * Read SW template of this port
         */
        ...
        if (dnx_data_port_pp.filters.feature_get(unit, dnx_data_port_pp_filters_acceptable_frame_type_filter_support))
        {
            ...
            if (is_physical_port)
            {
                ...
                is_last = FALSE;
                SHR_IF_ERR_EXIT(algo_port_pp_db.egress_acceptable_frame_type_profile.exchange
                                (unit, 0, 1, &acceptable_frame_type_template,
