...
/**
 * \brief -
 * Take the pfc counters out of reset for PFC deadlock detection mechanism
 *
 * \param [in] unit -  Unit-ID
 * \return
 *   See shr_error_e
 ...
 */
static int
dnx_instru_synced_counters_pfc_counters_out_of_reset(
    int unit)
{
    uint32 entry_handle_id;
    int port_index_current = 0;
    int pm_index_current;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /** Take HW out of reset for the instrumentation counters for PFC deadlock detection mechanism */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_INSTRU_RSTN_PFC_CNTS, &entry_handle_id));
    for (pm_index_current = 0; pm_index_current < dnx_data_instru.synced_counters.nof_pms_for_pfc_get(unit);
         pm_index_current++)
    {

        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PFC_PM_IDX, pm_index_current);
        for (port_index_current = 0; port_index_current < dnx_data_instru.synced_counters.nof_ports_for_pfc_get(unit);
             port_index_current++)
        {
            /** Take out of reset */
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PFC_PORT_IDX, port_index_current);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESET_CNTS_PFC, INST_SINGLE,
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_instru_synced_counters_pfc_counters_out_of_reset(unit));
    }
    else
    {
    ...
    uint8 is_set)
{
    ...
    /** If the number of ports for PFC instrumentation counters are limited, check if they were configured */
    if (dnx_data_instru.synced_counters.feature_get(unit, dnx_data_instru_synced_counters_limited_ports_for_pfc)
        && (source_type == bcmInstruSyncedCountersTypeNif))
    {
        int ports_count_temp = 0;
        SHR_IF_ERR_EXIT(instru_sync_counters.nof_ports_for_pfc_counters.get(unit, &ports_count_temp));
        if (ports_count_temp == 0)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "Ports for PFC instrumentation counters are not configured!\n");
        }
    }
    ...
    int *nof_counter_types)
{
    ...
    /** Initialize the retrieced records structure to 0 */
    sal_memset(&retrieved_records, 0, sizeof(retrieved_records));
    ...
    {
        /*
         * For non-continuous case, there is a check above to make sure that:
         * (first_record + num_records_read - 1) < nof_intervals_hw
         * This is checked because we go over the intervals only one time. In this case:
         * current record index == record_index (so we do not need record_index % nof_intervals_hw)
         *
         * For continuous case there is a check that ensures num_records_read < nof_intervals_hw,
         * but the first_record can be any number from 0 to nof_intervals_hw, we can read the given 
         * number of records, no matter the offset (first_record) because it will just wrap around.In this case:
         * current record index == record_index % nof_intervals_hw
         */
        SHR_IF_ERR_EXIT(dnx_instru_synced_counters_per_record_get(unit, source_type, core_id, logical_port,
                                                                  (record_index % nof_intervals_hw),
                                                                  nof_counters_per_record, counters_dbal_fields,
                                                                  retrieved_records[record_index - first_record]));
    }
    ...
    if (num_records_read > 0)
    {
        ...
        for (record_index = 0; record_index < num_records_read; record_index++)
        {
            ...
            sal_memcpy(record_arr[record_index].value_arr,
                       retrieved_records[record_index], sizeof(retrieved_records[0]));
...
static int
dnx_instru_synced_counters_ports_for_pfc_get_verify(
    int unit,
    uint32 flags,
    int ports_count_in,
    ...
    int *ports_count_out)
{
    ...
    if (!dnx_data_instru.synced_counters.feature_get(unit, dnx_data_instru_synced_counters_limited_ports_for_pfc))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "This API is not supported on current device!\n");
    }

    /** NULL Check for ports_for_pfc array */
    SHR_NULL_CHECK(ports_for_pfc, _SHR_E_PARAM, "Parameter ports_for_pfc is NULL!\n");

    /** For 'get' context - NULL Check for ports_count_out */
    SHR_NULL_CHECK(ports_count_out, _SHR_E_PARAM, "Parameter ports_count is NULL!\n");

    /** Verify input flags */
    SHR_VAL_VERIFY(flags, 0, _SHR_E_PARAM,
                   "Unsupported flags for synchronized counters - ports for PFC configuration!\n");

    /** Verify ports_count_in value.
     * For 'get' context this is the max number of ports to be returned.
     * It should be between 0 and the max allowed value. */
    SHR_RANGE_VERIFY(ports_count_in, 0, dnx_data_instru.synced_counters.nof_ports_for_pfc_get(unit), _SHR_E_PARAM,
                     "Number of input ports over the limit!\n");
...
static int
dnx_instru_synced_counters_ports_for_pfc_set_verify(
    int unit,
    uint32 flags,
    int ports_count_in,
    bcm_instru_synced_counters_port_for_pfc_t * ports_for_pfc,
    int *ports_count_out)
{
    int index = 0;
    int first_phy = 0;
    int pm_index, current_pm, phy_map_granularity;
    uint8 pm_already_in_array, found_free_slot;
    int nof_tcs_in_bmp, total_nof_bmp = 0;
    uint32 tc_bmp;
    int pms[DNX_DATA_MAX_INSTRU_SYNCED_COUNTERS_NOF_PMS_FOR_PFC];
    SHR_FUNC_INIT_VARS(unit);
    ...
    /** Verify ports_count_in value.
     * For 'set' context this is the number of input ports.
     * It should be between 0 and the max allowed value. */
    SHR_RANGE_VERIFY(ports_count_in, 0, dnx_data_instru.synced_counters.nof_ports_for_pfc_get(unit), _SHR_E_PARAM,
                     "Number of input ports over the limit!\n");

    /** Verify that the total number of TCs does not go over the limitation */
    for (index = 0; index < ports_count_in; index++)
    {
        if (ports_for_pfc[index].tc_bmp == 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "TC bitmap can not be empty!\n");
        }
        /** Count the number of TCs in the current bitmap */
        tc_bmp = ports_for_pfc[index].tc_bmp;
        SHR_BITCOUNT_RANGE(&tc_bmp, nof_tcs_in_bmp, 0, BCM_COS_COUNT);

        /** Add the current TCs to the total */
        total_nof_bmp += nof_tcs_in_bmp;

        /** Check if we are going over the limitation */
        if (total_nof_bmp >
            (dnx_data_instru.synced_counters.nof_ports_for_pfc_get(unit) *
             dnx_data_instru.synced_counters.nof_tcs_per_port_get(unit)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Number of port+TC combinations over the limit! Max number of combinations is %d\n",
                         (dnx_data_instru.synced_counters.nof_ports_for_pfc_get(unit) *
                          dnx_data_instru.synced_counters.nof_tcs_per_port_get(unit)));
        }
    }
    ...
    /** Init the array to -1 */
    for (pm_index = 0; pm_index < dnx_data_instru.synced_counters.nof_pms_for_pfc_get(unit); pm_index++)
    {
        pms[pm_index] = -1;
    }

    /** Verify that all ports are spanning on up to the max number of PMs */
    for (index = 0; index < ports_count_in; index++)
    {
    /** Get the PM of the current port */
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, ports_for_pfc[index].port, 0, &first_phy));
        phy_map_granularity = dnx_data_nif.eth.phy_map_granularity_get(unit);
        current_pm = dnx_data_nif.eth.phy_map_get(unit, first_phy / phy_map_granularity)->pm_index;

        /** Check if the current PM already exists in the PMs array */
        pm_already_in_array = FALSE;
        for (pm_index = 0; pm_index < dnx_data_instru.synced_counters.nof_pms_for_pfc_get(unit); pm_index++)
        {
            if (pms[pm_index] == current_pm)
            {
                pm_already_in_array = TRUE;
                break;
            }
        }

        /** If the current PM is not already in the array, find a free slot to add it */
        found_free_slot = FALSE;
        if (pm_already_in_array == FALSE)
        {
            for (pm_index = 0; pm_index < dnx_data_instru.synced_counters.nof_pms_for_pfc_get(unit); pm_index++)
            {
                if (pms[pm_index] == -1)
                {
                    pms[pm_index] = current_pm;
                    found_free_slot = TRUE;
                    break;
                }
            }

            /** If the current PM is not already in the array and there was no free slot, return an error */
            if ((pm_already_in_array == FALSE) && (found_free_slot == FALSE))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Number of PMs over the limit! Ports must span over maximum of %d PMs!\n",
                             dnx_data_instru.synced_counters.nof_pms_for_pfc_get(unit));
            }
        }
    }
exit:
...
static int
dnx_instru_synced_counters_ports_for_pfc_to_hw_convert(
    int unit,
    int ports_count,
    bcm_instru_synced_counters_port_for_pfc_t * ports_for_pfc,
    bcm_instru_synced_counters_port_for_pfc_hw_t * ports_for_pfc_for_hw)
{
    int current_port;
    int port_entry_index;
    int pm_array_current_index = 0;
    int tc_index = 0;
    int port_first_phy = 0;
    int pm_index, current_pm;
    uint8 pm_already_in_array;
    int nof_tcs_in_bmp;
    uint32 tc_bmp[1];
    uint32 internal_offset_in_pm;
    SHR_FUNC_INIT_VARS(unit);

    port_entry_index = 0;
    for (current_port = 0; current_port < ports_count; current_port++)
    {
        /** Get the PM of the current port and its offset internal to the PM */
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, ports_for_pfc[current_port].port, 0, &port_first_phy));
        current_pm =
            dnx_data_nif.eth.phy_map_get(unit,
                                         port_first_phy / dnx_data_nif.eth.phy_map_granularity_get(unit))->pm_index;
        internal_offset_in_pm =
            port_first_phy - dnx_data_nif.eth.phy_map_get(unit,
                                                          port_first_phy /
                                                          dnx_data_nif.eth.phy_map_granularity_get(unit))->phy_index;

        /** Set in SW State the current port's PM index and Port base index */
        SHR_IF_ERR_EXIT(instru_sync_counters.ports_for_pfc_counters.
                        pfc_pm_idx.set(unit, current_port, pm_array_current_index));
        SHR_IF_ERR_EXIT(instru_sync_counters.ports_for_pfc_counters.
                        pfc_port_idx_base.set(unit, current_port, port_entry_index));

        /** Get the number of TCs for the current port and based on it determine how many entries it will take up */
        tc_bmp[0] = ports_for_pfc[current_port].tc_bmp;
        SHR_BITCOUNT_RANGE(tc_bmp, nof_tcs_in_bmp, 0, BCM_COS_COUNT);
        SHR_BIT_ITER(tc_bmp, BCM_COS_COUNT, tc_index)
        {
            /** Populate the port - one time every nof_tcs */
            if ((nof_tcs_in_bmp % dnx_data_instru.synced_counters.nof_tcs_per_port_get(unit)) == 0)
            {
                /** Set the value in the array, as global offsed in the chosen PMs */
                ports_for_pfc_for_hw->nif_ports_in_pms[port_entry_index] =
                    internal_offset_in_pm + (pm_array_current_index * dnx_data_nif.eth.nof_lanes_in_cdu_get(unit));
            }

            /** Populate the TCs per port */
            ports_for_pfc_for_hw->tcs_per_port[port_entry_index][nof_tcs_in_bmp %
                                                                 dnx_data_instru.
                                                                 synced_counters.nof_tcs_per_port_get(unit)] = tc_index;

             /** Lower the remaining number of TCs */
            nof_tcs_in_bmp--;

            /** Update the entry index of the next entry based on the remaining number of TCs */
            if ((nof_tcs_in_bmp % dnx_data_instru.synced_counters.nof_tcs_per_port_get(unit)) == 0)
            {
                port_entry_index++;
            }
        ...
        for (pm_index = 0; pm_index < dnx_data_instru.synced_counters.nof_pms_for_pfc_get(unit); pm_index++)
        {
            if (ports_for_pfc_for_hw->pms[pm_index] == current_pm)
            {
                pm_already_in_array = TRUE;
                pm_array_current_index = pm_index;
                break;
            }
        }

                /** If the PM does not already exist add it at the next free index */
        if (pm_already_in_array == FALSE)
        {
            ports_for_pfc_for_hw->pms[pm_array_current_index] = current_pm;
            pm_array_current_index++;
        }
    }
...
static int
dnx_synced_counters_ports_for_pfc_sw_get(
    int unit,
    int ports_count_max,
    bcm_instru_synced_counters_port_for_pfc_t * ports_for_pfc,
    int *ports_count)
{
    ...
    SHR_IF_ERR_EXIT(instru_sync_counters.nof_ports_for_pfc_counters.get(unit, &ports_count_temp));
    if (ports_count_temp > ports_count_max)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Array size too small. Array size %d, actual ports count %d!\n", ports_count_max,
                     ports_count_temp);
    }
    *ports_count = ports_count_temp;

    /** Get the ports and TCs bitmaps to SW State */
    for (current_port = 0; current_port < ports_count_temp; current_port++)
    {
        SHR_IF_ERR_EXIT(instru_sync_counters.ports_for_pfc_counters.port.get(unit, current_port, &port_temp));
        SHR_IF_ERR_EXIT(instru_sync_counters.ports_for_pfc_counters.tc_bmp.get(unit, current_port, &tc_bmp_temp));
        ports_for_pfc[current_port].port = port_temp;
        ports_for_pfc[current_port].tc_bmp = tc_bmp_temp;
    }
...
static int
dnx_synced_counters_ports_for_pfc_sw_set(
    int unit,
    bcm_instru_synced_counters_port_for_pfc_t * ports_for_pfc,
    int ports_count)
{
    ...
    /** Set the number of ports in the SW State */
    SHR_IF_ERR_EXIT(instru_sync_counters.nof_ports_for_pfc_counters.set(unit, ports_count));

    /** Set the ports and TCs bitmaps to SW State */
    port_max = (ports_count == -1) ? DNX_DATA_MAX_INSTRU_SYNCED_COUNTERS_NOF_PORTS_FOR_PFC : ports_count;
    for (current_port = 0; current_port < port_max; current_port++)
    {
        SHR_IF_ERR_EXIT(instru_sync_counters.ports_for_pfc_counters.
                        port.set(unit, current_port, ports_for_pfc[current_port].port));
        SHR_IF_ERR_EXIT(instru_sync_counters.ports_for_pfc_counters.
                        tc_bmp.set(unit, current_port, ports_for_pfc[current_port].tc_bmp));
    }
...
static int
dnx_synced_counters_ports_for_pfc_hw_set(
    int unit,
    bcm_instru_synced_counters_port_for_pfc_hw_t * ports_for_pfc_for_hw)
{
    uint32 entry_handle_id = 0;
    int pm_index, current_port, tc_index;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /** Set the PMs to HW */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_NIF_INSTRU_PM_FOR_PFC_SEL, &entry_handle_id));
    for (pm_index = 0; pm_index < dnx_data_instru.synced_counters.nof_pms_for_pfc_get(unit); pm_index++)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PFC_PM_IDX, pm_index);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PM_ID, INST_SINGLE,
                                     ports_for_pfc_for_hw->pms[pm_index]);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }

    /** Set the ports and TCs to HW */
    for (current_port = 0; current_port < dnx_data_instru.synced_counters.nof_ports_for_pfc_get(unit); current_port++)
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_NIF_INSTRU_PORT_FOR_PFC_SEL, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PFC_PORT_IDX, current_port);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PFC_2xPM_OFFSET, INST_SINGLE,
                                     ports_for_pfc_for_hw->nif_ports_in_pms[current_port]);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

        for (tc_index = 0; tc_index < dnx_data_instru.synced_counters.nof_tcs_per_port_get(unit); tc_index++)
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_NIF_INSTRU_TC_FOR_PFC_SEL, entry_handle_id));
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PFC_PORT_IDX, current_port);
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PFC_TC_IDX, tc_index);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TC, INST_SINGLE,
                                         ports_for_pfc_for_hw->tcs_per_port[current_port][tc_index]);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }
    }
...
int
bcm_dnx_instru_synced_counters_ports_for_pfc_set(
    int unit,
    uint32 flags,
    int ports_count,
    bcm_instru_synced_counters_port_for_pfc_t * ports_for_pfc)
{
    ...
    /** Verify input parameters of API */
    SHR_INVOKE_VERIFY_DNXC(dnx_instru_synced_counters_ports_for_pfc_set_verify
                           (unit, flags, ports_count, ports_for_pfc, NULL));

    /** Convert the array in the appropriate format to write to HW */
    sal_memset(&ports_for_pfc_for_hw, 0x0, sizeof(ports_for_pfc_for_hw));
    if (ports_count != 0)
    {
        SHR_IF_ERR_EXIT(dnx_instru_synced_counters_ports_for_pfc_to_hw_convert
                        (unit, ports_count, ports_for_pfc, &ports_for_pfc_for_hw));
    }

    /** Write the array to HW */
    SHR_IF_ERR_EXIT(dnx_synced_counters_ports_for_pfc_hw_set(unit, &ports_for_pfc_for_hw));

    /** Update SW State */
    SHR_IF_ERR_EXIT(dnx_synced_counters_ports_for_pfc_sw_set(unit, ports_for_pfc, ports_count));
...
int
bcm_dnx_instru_synced_counters_ports_for_pfc_get(
    int unit,
    uint32 flags,
    int ports_count_max,
    bcm_instru_synced_counters_port_for_pfc_t * ports_for_pfc,
    int *ports_count)
{
    ...
    /** Verify input parameters of API */
    SHR_INVOKE_VERIFY_DNXC(dnx_instru_synced_counters_ports_for_pfc_get_verify
                           (unit, flags, ports_count_max, ports_for_pfc, ports_count));

    /** Get values from SW State */
    SHR_IF_ERR_EXIT(dnx_synced_counters_ports_for_pfc_sw_get(unit, ports_count_max, ports_for_pfc, ports_count));
