...
#include "src/bcm/dnx/flow/porting/mpls_port_flow.h"
    ...
    bcm_mpls_port_t * mpls_port)
{
        ...
        (mpls_port->criteria != BCM_MPLS_PORT_MATCH_LABEL_L3_INGRESS_INTF))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "match criteria must be BCM_MPLS_PORT_MATCH_LABEL"
                     " or BCM_MPLS_PORT_MATCH_LABEL_L3_INGRESS_INTF.");
    }
    ...
    if (_SHR_IS_FLAG_SET(mpls_port->flags, BCM_MPLS_PORT_SERVICE_TAGGED) &&
        !_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_SERVICE_TAGGED_BASED_HEADER_ONLY))
    {
        /*
         * PWE tagged mode(ONE_TAG, TWO_TAGS). Make sure there are tags.
         */
    ...
    if (_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_SERVICE_TAGGED_BASED_HEADER_ONLY)
        && !_SHR_IS_FLAG_SET(mpls_port->flags, BCM_MPLS_PORT_SERVICE_TAGGED))
    {
        /*
         * PWE tagged x-tags mode.
         */
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "BCM_MPLS_PORT2_SERVICE_TAGGED_BASED_HEADER_ONLY shall co-work with BCM_MPLS_PORT_SERVICE_TAGGED");
    }
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    /** verifications that are made in the flow module, are skipped here if is_flow_enabled_for_legacy_applications feature is active */
    if (!dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        ...
        if ((mpls_port->egress_label.encap_access != bcmEncapAccessInvalid)
            ...
            && (mpls_port->egress_label.encap_access != bcmEncapAccessTunnel3)
            && (mpls_port->egress_label.encap_access != bcmEncapAccessNativeArp)
            && (mpls_port->egress_label.encap_access != bcmEncapAccessRif))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "mpls_port->egress_label.encap_access (%d) is invalid, supported values are %d - %d",
    ...
    if (mpls_port->egress_label.estimated_encap_size < -1)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "estimated_encap_size=%d. Invalid value.",
                     mpls_port->egress_label.estimated_encap_size);
    }
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    supported_flags2 =
        ...
        BCM_MPLS_PORT2_STAT_ENABLE | BCM_MPLS_PORT2_ALLOC_SYMMETRIC |
        ...
        BCM_MPLS_PORT2_SERVICE_TAGGED_BASED_HEADER_ONLY | BCM_MPLS_PORT2_KEEP_TTL | BCM_MPLS_PORT2_EXTENDED_TERMINATION;

    if (!dnx_data_vlan.llvp.feature_get(unit, dnx_data_vlan_llvp_outlif_llvp_from_outlif_profile))
    {
        supported_flags2 |= BCM_MPLS_PORT2_NO_TPID_CLASS;
    }
    if (dnx_data_multicast.params.feature_get(unit, dnx_data_multicast_params_mc_pruning_actions_not_supported))
    {
        supported_flags2 |= BCM_MPLS_PORT2_EGRESS_PROTECTION;
    }
    ...
    if (!dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        ...
        {
            if (mpls_port->egress_label.exp > dnx_data_qos.qos.packet_max_exp_get(unit))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "EXP %d is out of range.", mpls_port->egress_label.exp);
            }
        }
    ...
    if (_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_EGRESS_ONLY))
    {
        ...
        if (_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_EXTENDED_TERMINATION))
        {
            /** Egress objects cannot be configured with EXTENDED_TERMINATION flag */
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "BCM_MPLS_PORT2_EXTENDED_TERMINATION (0x%x) should not be set for egress only MPLS Port. "
                         "flags2 = 0x%08x\n", BCM_MPLS_PORT2_EXTENDED_TERMINATION, mpls_port->flags2);
        }
    ...
    if (_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_ALLOC_SYMMETRIC))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "BCM_MPLS_PORT2_ALLOC_SYMMETRIC and BCM_MPLS_PORT2_INGRESS_ONLY cannot be used together");
        }
    }
    ...
    if (mpls_port->learn_strength != TRUE && mpls_port->learn_strength != FALSE)
    {
    ...
    bcm_mpls_port_t * mpls_port)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_port_add_verify(unit, vpn, mpls_port));
    ...
    bcm_gport_t mpls_port_id)
{
     ...
     * BCM level verification
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_port_delete_verify(unit, vpn, mpls_port_id));
    if (BCM_GPORT_SUB_TYPE_IS_MPLS_PUSH_PROFILE(mpls_port_id))
    {
    ...
    bcm_mpls_port_t * mpls_port)
{
     ...
     * BCM level verification
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_port_get_verify(unit, vpn, mpls_port));
    ...
    void *user_data)
{
     ...
     * Verify
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_mpls_port_traverse_verify(unit, additional_info, trav_fn));
