    ...
    bcm_port_extender_mapping_info_t * mapping_info)
{
    ...
    {
        if (!dnx_data_fc.coe.fc_support_get(unit) && (mapping_info->fc_channel_id != FC_CHAN_ID_INVALID))
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                         "Flow Control doesn't work on COE port on this device, fc_channel_id should be FC_CHAN_ID_INVALID!");
        }
    ...
    int is_update)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO_ASSIGNMENT_MODE, INST_SINGLE, 0);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_ENABLE, INST_SINGLE, 0);
