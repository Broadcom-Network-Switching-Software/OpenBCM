 ...
 * {
 */
static shr_error_e dnx_vxlan_fodo_to_vni_set_query_key_set(
    int unit,
    ...
    dnx_fwd_domain_t * fwd_domain)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_uint32_sub_field_info_get(unit, DBAL_FIELD_FODO, fodo, &sub_field_id, &vsi_or_vrf));
    fwd_domain->is_vsi = (sub_field_id == DBAL_FIELD_VSI);
    if (fwd_domain->is_vsi)
    {
        fwd_domain->vsi = vsi_or_vrf;
    }
    else
    {
        fwd_domain->vrf = vsi_or_vrf;
    ...
    uint32 *value)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_VXLAN_VSI2VNI_MAPPING_CONFIGURED_BY_API,
                               INST_SINGLE, value);
    ...
    uint8 *vsi_to_vni_is_created_by_ac)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, vlan_port_id, BCM_GPORT_TYPE_FLOW_LIF, DNX_ALGO_GPM_CONVERSION_FLAG_FLOW_INITIATOR,
                         &local_gport));
    }
    ...
    uint8 *vsi_to_vni_is_created_by_ac)
{
    ...
    {
        algo_esem_entry_source_e source = DNX_ALGO_ESEM_ENTRY_SOURCE_AC;
        ...
        {
            *vsi_to_vni_is_created_by_ac = TRUE;
        }
        else
        {
...
static shr_error_e
dnx_vxlan_vpn_esem_owner_remove(
    int unit,
    uint32 entry_handle_id,
    int update_only_if_not_exists)
{
    algo_esem_entry_status_e entry_status;
    algo_esem_entry_source_e source = DNX_ALGO_ESEM_ENTRY_SOURCE_VXLAN_VPN;
    ...
    {
        if (!update_only_if_not_exists)
        {
            /** ALGO_ESEM_ENTRY_STATUS_NOT_EXISTS in this case it may that the entry was added by AC */
            SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "entry not found, status %d. \n", entry_status);
        }
    }
...
static shr_error_e
dnx_vxlan_vpn_esem_owner_update(
    int unit,
    uint32 entry_handle_id,
    uint32 update_only_if_not_exists)
{
    algo_esem_entry_status_e entry_status;
    algo_esem_entry_source_e source = DNX_ALGO_ESEM_ENTRY_SOURCE_VXLAN_VPN;
    ...
    else if (entry_status == ALGO_ESEM_ENTRY_STATUS_NOT_EXISTS)
    {
        SHR_IF_ERR_EXIT(dnx_esem_entry_sw_source_create(unit, entry_handle_id, source));
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Entry already exists. \n");
    }
...
/** update SWSTATE for egress, VSI to VNI owner (AC / VXLAN_VPN / CONFIG_ADD) this will help to sync between the
 *  modules for example used in dnx_vxlan_vsi_to_vni_is_created_by_ac() or dnx_vxlan_virtual_ac_gport_is_exist */
static shr_error_e
dnx_vxlan_vpn_esem_entry_owner_set(
    int unit,
    int vsi,
    uint32 network_domain,
    int is_remove,
    int update_only_if_not_exists)
{
    ...
    if (dnx_data_esem.feature.feature_get(unit, dnx_data_esem_feature_esem_sw_managment))
    {
        ...
        if (is_remove)
        {
            /** in case the entry add with source VXLAN_NWK_DOMAIN, it was deleted by the AC, so no need to remove the owner
                 *  again */
            SHR_IF_ERR_EXIT_WITH_LOG(dnx_vxlan_vpn_esem_owner_remove(unit, entry_handle_id, update_only_if_not_exists),
                                     "Failed to remove entry Vsi %d network_domain=%d %s", vsi, network_domain, EMPTY);
        }
        else
        {
            SHR_IF_ERR_EXIT_WITH_LOG(dnx_vxlan_vpn_esem_owner_update(unit, entry_handle_id, update_only_if_not_exists),
                                     "Failed to update entry Vsi %d network_domain=%d %s", vsi, network_domain, EMPTY);
        }
    }
    ...
    uint32 value)
{
    ...
    /** updare the swstate about VXLAN VSI-VNI mapping owner (VXLAN_VPN / CONFIG_ADD) */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_VSI_TYPE_SW, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI, vsi);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_VXLAN_VSI2VNI_MAPPING_CONFIGURED_BY_API,
                                INST_SINGLE, value);
...
static int
dnx_vsi_to_vni_result_type_wo_vlan_get(
    int unit)
{
    return DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_L2_FODO;
}

static int
dnx_vsi_to_vni_result_type_with_vlan_get(
    int unit)
{
    return DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_L2_FODO_VLANS;
}
...
static shr_error_e
dnx_vxlan_fodo_to_vni_traverse_query_key_get(
    int unit,
    uint32 entry_handle_id,
    dnx_fwd_domain_t * fwd_domain,
    uint32 *network_domain)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                    (unit, entry_handle_id, network_domain_dbal_field, network_domain));

    SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_FODO, &fodo));
    SHR_IF_ERR_EXIT(dnx_fwd_domain_fwd_domain_set(unit, fodo, fwd_domain));
exit:
    SHR_FUNC_EXIT;
...
int
dnx_vxlan_port_tunnel_term_gport_hw_resources_get(
    int unit,
    bcm_vxlan_port_t * vxlan_port,
    ...
    bcm_vxlan_port_t * vxlan_port)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_get(unit, vxlan_port->vxlan_port_id, 0, &forward_info));
    ...
    dnx_in_lif_profile_last_info_t * last_profile)
{
     ...
     */
    in_lif_profile_info.egress_fields.in_lif_orientation = vxlan_port->network_group_id;
    if (_SHR_IS_FLAG_SET(vxlan_port->flags2, BCM_VXLAN_PORT_FLAGS2_VNI_FLOW_TERMINATOR))
    {
    ...
    dnx_in_lif_profile_last_info_t * last_profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vxlan_port_ingress_inlif_profile_exchange_internal
                    (unit, vxlan_port, tunnel_term_gport_hw_resources->inlif_dbal_table_id, old_in_lif_profile,
    ...
    bcm_vxlan_port_t * vxlan_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vxlan_port_ingress_inlif_profile_exchange
                    (unit, &tunnel_term_gport_hw_resources, vxlan_port, &in_lif_profile, &last_profile));
    if (is_out_lif)
    {
    ...
    bcm_vxlan_port_t * vxlan_port)
{
    ...
    if (_SHR_IS_FLAG_SET(in_lif_profile_info.ingress_fields.cs_in_lif_profile_flags,
                         DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_WITH_VXLAN_VNI2LIF))
    {
    ...
    uint8 display_err)
{
    ...
    if (rv == _SHR_E_NONE)
    {
        ...
        if (((*result_type) == dnx_vsi_to_vni_result_type_wo_vlan_get(unit)) ||
            ((*result_type) == dnx_vsi_to_vni_result_type_with_vlan_get(unit)))
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    uint32 *result_type)
{
    ...
    {
        *result_type = dnx_vsi_to_vni_result_type_wo_vlan_get(unit);
    }
}
    ...
    uint32 *result_type)
{
    ...
    *existing_entry = UTILEX_NUM2BOOL(rv_vsi2vni != _SHR_E_NOT_FOUND);
    ...
    {
        dnx_vxlan_vpn_vsi_to_vni_resolve_result_type_for_new_entry(unit, qos_map_id, result_type);
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    dnx_vxlan_vpn_mapping_t * mapping_info)
{
    ...
    if (!mapping_info->has_egress_mapping)
    {
        dnx_vxlan_vpn_vsi_to_vni_resolve_result_type_for_new_entry(unit, DNX_QOS_INITIAL_MAP_ID,
                                                                   &(mapping_info->egress_mapping_result_type));
    }
    ...
    bcm_vxlan_vpn_config_t * info)
{
    ...
    if (!update)
    {
        ...
        if (info->vnid != BCM_VXLAN_VNI_INVALID)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vxlan_vpn_esem_entry_owner_set(unit, info->vpn, info->match_port_class, FALSE, FALSE));
        }
    }
    ...
    if (update)
    {
        ...
        if (info->flags & BCM_VXLAN_VPN_REPLACE)
        {
                ...
                && (info->vnid != BCM_VXLAN_VNI_INVALID))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_vxlan_vpn_esem_entry_owner_set
                                (unit, info->vpn, info->match_port_class, FALSE, FALSE));
            }
        }
    }
    else
    ...
    bcm_vpn_t l2vpn)
{
    ...
    if (vxlan_vpn_create != DBAL_ENUM_FVAL_VXLAN_VSI2VNI_MAPPING_CONFIGURED_BY_API_UNKNOWN)
    {
        /** only if entry was actually added by vxlan API, need to remove it */
        SHR_IF_ERR_EXIT(dnx_vxlan_vpn_create_type_set
                        (unit, l2vpn, DBAL_ENUM_FVAL_VXLAN_VSI2VNI_MAPPING_CONFIGURED_BY_API_UNKNOWN));

        SHR_IF_ERR_EXIT(dnx_vxlan_vpn_esem_entry_owner_set(unit, l2vpn, network_domain, TRUE, TRUE));
    }
    ...
    bcm_vxlan_network_domain_config_t * config)
{
    ...
    if (is_l2)
    {
        ...
        {
            /** */
            SHR_IF_ERR_EXIT(dnx_vxlan_vpn_esem_entry_owner_set(unit, config->vsi, config->network_domain, FALSE, TRUE));
        }
    }
    ...
    void *user_data)
{
    ...
    uint32 dbal_tables[] = { 0, 0, 0 }, vni, vrf;
    uint32 nof_tables, vxlan_network_domain_add;
    int is_end, table;
    bcm_vxlan_network_domain_config_t config;
    dbal_tables_e fodo_to_vni_table = dnx_fodo_to_vni_dbal_table_get(unit);
    dnx_fwd_domain_t fwd_domain;
    ...
    {
        dbal_tables[0] = fodo_to_vni_table;;
        nof_tables = 1;
    }
    ...
    else
    {
        dbal_tables[0] = DBAL_TABLE_VNI2VSI;
        dbal_tables[1] = DBAL_TABLE_VNI2VRF;
        dbal_tables[2] = fodo_to_vni_table;
        nof_tables = 3;
    }
    ...
    for (table = 0; table < nof_tables; table++)
    {
        ...
        if ((dbal_tables[table] != fodo_to_vni_table))
        {
        ...
        while (!is_end)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_vxlan_fodo_to_vni_traverse_query_key_get
                                (unit, entry_handle_id, &fwd_domain, &config.network_domain));
                if (!fwd_domain.is_vsi)
                {
                    config.flags =
...
/**
 * \brief
 * Allocate the default in lif.
 *
 * \param [in] unit - Relevant unit
 * \param [out] default_in_lif - default in lif
 * \return
 ...
 */
static shr_error_e
dnx_vxlan_vni_default_in_lif_id_allocate(
    int unit,
    uint32 *default_in_lif)
{
    ...
    inlif_info.dbal_table_id = DBAL_TABLE_IN_LIF_FORMAT_VNI;
    inlif_info.dbal_result_type = DBAL_RESULT_TYPE_IN_LIF_FORMAT_VNI_IN_LIF_VNI;
    ...
    *default_in_lif = inlif_info.local_inlif;
...
/**
 * \brief
 * Set the default in lif.
 *
 * \param [in] unit - Relevant unit
 * \param [in] default_in_lif - default in lif
 * \return
 *  * Error indication according to shr_error_e enum
 * \remark
 * None
 */
static shr_error_e
dnx_vxlan_vni_default_in_lif_id_set(
    int unit,
    uint32 default_in_lif)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_VXLAN_VNI_DEFAULT_LIF, &entry_handle_id));

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VXLAN___VNI___DEFAULT_LIF, INST_SINGLE,
                                 default_in_lif);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
static shr_error_e
dnx_vxlan_vni_default_in_lif_id_get(
    int unit,
    uint32 *default_in_lif)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_VXLAN_VNI_DEFAULT_LIF, &entry_handle_id));
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_VXLAN___VNI___DEFAULT_LIF, INST_SINGLE,
                               default_in_lif);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    SHR_IF_ERR_EXIT(DBAL_HANDLE_FREE(unit, entry_handle_id));
 ...
 */
static shr_error_e
dnx_vxlan_vni_default_in_lif_add(
    int unit,
    uint32 default_in_lif)
{
    uint32 entry_handle_id;
    ...
    /*
     * Step 1: Take table handle:
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_IN_LIF_FORMAT_VNI, &entry_handle_id));

    /*
     * Step 2: Set keys:
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, default_in_lif);

    /*
     * Step 3: Set values:
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                 DBAL_RESULT_TYPE_IN_LIF_FORMAT_VNI_IN_LIF_VNI);
 ...
 */
static shr_error_e
dnx_vxlan_vni_default_in_lif_trap_update(
    int unit,
    uint32 default_in_lif,
    int trap_gport)
{
 ...
 */
static shr_error_e
dnx_vxlan_vni_default_in_lif_trap_get(
    int unit,
    uint32 default_in_lif,
    int *trap_gport)
{
 ...
 */
shr_error_e
dnx_vxlan_vni_miss_get(
    int unit,
    int *trap_gport)
{
    uint32 vni_default_in_lif = 0;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /*
     * Get default in lif id
     */
    SHR_IF_ERR_EXIT(dnx_vxlan_vni_default_in_lif_id_get(unit, &vni_default_in_lif));

    /*
     * Take table handle:
     */
    SHR_IF_ERR_EXIT(dnx_vxlan_vni_default_in_lif_trap_get(unit, vni_default_in_lif, trap_gport));
    ...
    int trap_gport)
{
    ...
    /*
     * Step 1: Get default in lif id
     */
    SHR_IF_ERR_EXIT(dnx_vxlan_vni_default_in_lif_id_get(unit, &vni_default_in_lif));

    /*
     * Step 2: Update the default in lif trap:
     */
    SHR_IF_ERR_EXIT(dnx_vxlan_vni_default_in_lif_trap_update(unit, vni_default_in_lif, trap_gport));
 ...
 */
static shr_error_e
dnx_vxlan_vni_default_in_lif_create(
    int unit)
{
    uint8 is_std_1;
    uint32 vni_default_in_lif = 0;
    ...
    /*
     * Step 1: Allocate local LIF
     */
    SHR_IF_ERR_EXIT(dnx_vxlan_vni_default_in_lif_id_allocate(unit, &vni_default_in_lif));

    /*
     * Step 2: Write lif value to virtual register of default lif
     */
    SHR_IF_ERR_EXIT(dnx_vxlan_vni_default_in_lif_id_set(unit, vni_default_in_lif));

    /*
     * Step 2: Add the default lif
     */
    SHR_IF_ERR_EXIT(dnx_vxlan_vni_default_in_lif_add(unit, vni_default_in_lif));
 ...
 */
shr_error_e
dnx_vxlan_module_init(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);

    /** initialize VNI default LIF*/
    SHR_IF_ERR_EXIT(dnx_vxlan_vni_default_in_lif_create(unit));
