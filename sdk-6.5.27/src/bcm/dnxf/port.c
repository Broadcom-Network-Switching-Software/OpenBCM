    ...
    int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        SHR_IF_ERR_EXIT(dnxf_port_features_init(unit));
    ...
    bcm_port_resource_t * resource)
{
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnxf_drv_link_to_block_mapping
                        (unit, resource[resource_index].port, &block_id, &port_index, SOC_DNXF_DRV_BLOCK_TYPE_FSRD));
        ...
        for (port_index = 0; port_index < DNXC_PORT_FABRIC_LANES_PER_CORE; ++port_index)
        {
            if (pm_config[pm_id].ports_to_set[port_index] == 1)
            {
                ...
                SHR_IF_ERR_EXIT(soc_dnxf_drv_block_to_link_mapping
                                (unit, pm_id, port_index, SOC_DNXF_DRV_BLOCK_TYPE_FSRD, &port));
    ...
    int *is_powered_down)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_port_internal_blocks_power_down_get, (unit, block_type, &bmp)));
