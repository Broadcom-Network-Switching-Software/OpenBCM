    ...
    int arg)
{
    ...
    if (!DNXF_IS_PRIORITY_VALID(arg) && arg != soc_dnxf_fabric_priority_invalid)
    {
    ...
    int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        if (dnxf_data_fabric.cell.feature_get(unit, dnxf_data_fabric_cell_fe2_control_cells_filter_required))
        {
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmFabricMcMidPrioMax:
            if (!dnxf_data_fabric.
                multicast.feature_get(unit, dnxf_data_fabric_multicast_mc_prio_per_mc_range_supported))
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Setting MC cell priority per MC ID is not supported for this device!");
            }
            if (!(dnxf_data_fabric.multicast.priority_map_enable_get(unit) == 1))
            {
        ...
        case bcmFabricEnableMcLowPrioDrop:
                ...
                multicast.feature_get(unit, dnxf_data_fabric_multicast_mc_prio_per_mc_range_supported))
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                             "Please use bcmFabricMcLowPrioDropSelect to enable/disable MC low priority drop. Use value of bcmFabricPriorityAll to disable the feature.");
            }
            SHR_RANGE_VERIFY(arg, 0, 1, _SHR_E_PARAM, "Unsupported enable value %d", arg);
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
        ...
        case bcmFabricMcLowPrioDropThDown:
            ...
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                             "Please use bcm_fabric_profile_threshold_set/get with control of bcmFabricRxMcLowPrioDropLow/HighTh to configure this threshold.");
            }
        ...
        case bcmFabricLocalRouteControlCellsEnable:
            ...
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcmFabricLocalRouteControlCellsEnable is not supported for this device.");
            }
            else
            {
                SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                                (unit, mbcm_dnxf_fabric_topology_local_route_control_cells_enable_set, (unit, arg)));
            }
            break;
        default:
    ...
    int *arg)
{
    ...
    switch (type)
    {
        ...
        case bcmFabricEnableMcLowPrioDrop:
            ...
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                             "Please use bcmFabricMcLowPrioDropSelect to enable/disable MC low priority drop.");
            }
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
    ...
    int value)
{
    ...
    switch (control)
    {
        ...
        {
            if (dnxf_data_fabric.multicast.feature_get(unit, dnxf_data_fabric_multicast_static_mode_supported))
            {
                SHR_IF_ERR_EXIT(dnxf_fabric_link_topology_destination_to_local_get(unit, modid, &local_modid));
                SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_topology_mc_sw_control_set,
                                                      (unit, local_modid, (value) ? (1) : (0))));
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Static mode is not supported on this device.\n");
            }
            break;
        }
        default:
    ...
    switch (control)
    {
        case bcmFabricModuleControlMcTopologySwControlEnable:
        {
            if (dnxf_data_fabric.multicast.feature_get(unit, dnxf_data_fabric_multicast_static_mode_supported))
            {
                SHR_IF_ERR_EXIT(dnxf_fabric_link_topology_destination_to_local_get(unit, modid, &local_modid));
                SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                                (unit, mbcm_dnxf_fabric_topology_mc_sw_control_get, (unit, local_modid, value)));
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Static mode is not supported on this device.\n");
...
static int
dnxf_fabric_link_topology_set_verify(
    int unit,
    bcm_module_t destination,
    int links_count,
    bcm_port_t * links_array)
{
    ...
    if (links_count < 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Links_count can't be less than 0.");
    }

    if (links_count > dnxf_data_port.general.nof_links_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Links count not within the valid range!");
    }
    ...
    bcm_port_t * links_array)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                     "bcm_fabric_link_topology_config_set is not supported on devices that do not support static mode Multicast. Use bcm_fabric_link_topology_set instead");
    }

    SHR_INVOKE_VERIFY_DNXC(dnxf_fabric_link_topology_set_verify(unit, destination, links_count, links_array));
...
static int
dnxf_fabric_link_topology_all_config_set(
    int unit,
    uint32 flags,
    bcm_module_t local_id,
    int links_count,
    ...
    bcm_port_t * links_array)
{
    ...
    switch (type)
    {
        ...
        case bcmFabricTopologyAll:
        {
            SHR_IF_ERR_EXIT(dnxf_fabric_link_topology_all_config_set(unit, flags, local_id, links_count, links_array));
            break;
        }
        default:
        {
    ...
    bcm_port_t * links_array)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnxf_fabric_link_topology_set_verify(unit, destination, links_count, links_array));
    SHR_IF_ERR_EXIT(dnxf_fabric_link_topology_destination_to_local_get(unit, destination, &local_id));
    SHR_IF_ERR_EXIT(dnxf_fabric_link_topology_all_config_set(unit, 0, local_id, links_count, links_array));
...
static int
dnxf_fabric_link_topology_get_verify(
    int unit,
    bcm_module_t destination,
    int links_count_max,
    int *links_count,
    ...
    bcm_port_t * links_array)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                     "bcm_fabric_link_topology_config_get is not supported on devices that do not support static mode Multicast. Use bcm_fabric_link_topology_get instead.");
    }

    SHR_INVOKE_VERIFY_DNXC(dnxf_fabric_link_topology_get_verify
                           (unit, destination, links_count_max, links_count, links_array));
    ...
    int set_get)
{
    ...
    if (!(dnxf_data_fabric.congestion.thresholds_info_get(unit, threshold_type)->is_valid))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid threshold provided %d !", threshold_type);
    }
    ...
    switch (dnxf_data_fabric.congestion.thresholds_info_get(unit, threshold_type)->index_type)
    {
        ...
        case _shr_dnxf_cgm_index_type_global:
            if (threshold_id != -1)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Global index can be only -1!");
            }
            break;
        default:
