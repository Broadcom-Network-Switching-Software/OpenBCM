    ...
    int unit)
{
    ...
    {
        case 1:
            nof_table_entries = sizeof(cgm_init_thresholds_single_pipe_mode) / sizeof(threshold_descriptor_t);
            cgm_init_values_table = cgm_init_thresholds_single_pipe_mode;
            break;

        case 2:
            nof_table_entries = sizeof(cgm_init_thresholds_dual_pipes_mode) / sizeof(threshold_descriptor_t);
            cgm_init_values_table = cgm_init_thresholds_dual_pipes_mode;
            break;

        case 3:
            nof_table_entries = sizeof(cgm_init_thresholds_thriple_pipes_mode) / sizeof(threshold_descriptor_t);
    ...
    for (table_entry_idx = 0; table_entry_idx < nof_table_entries; table_entry_idx++)
    {
        ...
        {
            if (!dnxf_data_fabric.congestion.thresholds_info_get(unit, threshold_type)->is_tx)
            {
                /**
                 * Multiply by a factor of the reduced fifo size for DCH/RX buffers
                 */
                value =
                    value * dnxf_data_fabric.fifos.reduced_rx_resources_get(unit) /
                    dnxf_data_fabric.fifos.rx_resources_get(unit);
            }
        }
        ...
        {
            if (threshold_type == bcmFabricRxLLFCFcPipeTh)
            {
                threshold_id = BCM_FABRIC_TH_INDEX_PIPE_LINK_SET(BCM_FABRIC_PIPE_ALL, -1);
            }
        }
        SHR_IF_ERR_EXIT(bcm_dnxf_fabric_profile_threshold_set
                        (unit, BCM_DNXF_CGM_TUNE_PROFILE_0, threshold_id, threshold_type, flags, value));
    }
    ...
    if (nof_pipes == 1)
    {
        ...
        {
            nof_table_entries =
                sizeof(cgm_init_thresholds_single_pipe_mode_fe13_multistage) / sizeof(threshold_descriptor_t);
        }
        else
        {
            nof_table_entries = 0;
    ...
    else
    {
        ...
        switch (dnxf_data_fabric.pipes.map_get(unit)->type)
        {
            case soc_dnxc_fabric_pipe_map_triple_uc_mc_tdm:
                nof_table_entries =
                    sizeof(cgm_init_thresholds_pipe_map_triple_uc_mc_tdm) / sizeof(threshold_descriptor_t);
                cgm_init_values_table = cgm_init_thresholds_pipe_map_triple_uc_mc_tdm;

                nof_table_entries_lr =
                    sizeof(cgm_init_thresholds_pipe_map_triple_uc_mc_tdm_lr) / sizeof(threshold_descriptor_t);
            ...
            case soc_dnxc_fabric_pipe_map_triple_uc_hp_mc_lp_mc:
                nof_table_entries =
                    sizeof(cgm_init_thresholds_pipe_map_triple_uc_hp_mc_lp_mc) / sizeof(threshold_descriptor_t);
                cgm_init_values_table = cgm_init_thresholds_pipe_map_triple_uc_hp_mc_lp_mc;

                nof_table_entries_lr =
                    sizeof(cgm_init_thresholds_pipe_map_triple_uc_hp_mc_lp_mc_lr) / sizeof(threshold_descriptor_t);
            ...
            case soc_dnxc_fabric_pipe_map_dual_tdm_non_tdm:
                nof_table_entries =
                    sizeof(cgm_init_thresholds_pipe_map_dual_tdm_non_tdm) / sizeof(threshold_descriptor_t);
            ...
            case soc_dnxc_fabric_pipe_map_dual_uc_mc:
                ...
                cgm_init_values_table_lr = cgm_init_thresholds_dual_pipes_mode_lr;
                break;
            case soc_dnxc_fabric_pipe_map_dual_lr_uc_nlr:
     ...
     */
    if (dnxf_data_fabric.general.local_routing_enable_uc_get(unit)
        || dnxf_data_fabric.general.local_routing_enable_mc_get(unit))
    {
    ...
    SHR_IF_ERR_EXIT(bcm_dnxf_fabric_link_profile_set
                    (unit, BCM_DNXF_CGM_TUNE_PROFILE_0, profile_attach_flags, links_count, links));
