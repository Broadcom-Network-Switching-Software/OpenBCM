...
/* Legacy variable to track if init is done. Currently only the soc flags should be used */
 ...
 */
#define ACCESS          DNXF_INIT_STEP_F_ACCESS
 ...
 *          for the DNXF BCM init from SOC properties.
 *
...
/**
 * \brief Init dnxf_init_info_t with default values.
 */
static shr_error_e
    ...
    utilex_seq_step_t * step_list)
{
    ...
    for (step_index = 0; step_list[step_index].step_id != UTILEX_SEQ_STEP_LAST; step_index++)
    {
        ...
        {
            SHR_IF_ERR_CONT(dnxf_init_step_list_destory(unit, step_list[step_index].step_sub_array));
        }
    }
    ...
    utilex_seq_step_t ** step_list)
{
    ...
    *step_list = (utilex_seq_step_t *) sal_alloc(sizeof(utilex_seq_step_t) * list_size, "dnxf init sequence list");
    ...
    for (step_index = 0; step_index < list_size; step_index++)
    {
        ...
        if (dnxf_step_list[step_index].step_sub_array != NULL)
        {
            SHR_IF_ERR_EXIT(dnxf_init_step_list_convert
                            (unit, dnxf_step_list[step_index].step_sub_array, &(step->step_sub_array)));
        }
    }
    ...
    utilex_step_time_dnx_data * step_id_dnx_data)
{
    ...
    if (step_id == UTILEX_SEQ_STEP_INVALID)
    {
        ...
        step_id_dnx_data->step_time = dnxf_data_dev_init.time.init_total_time_get(unit);
    ...
    else
    {
        ...
        step_id_dnx_data->step_time = dnxf_data_dev_init.time.step_time_get(unit, step_id)->time;
        step_id_dnx_data->down_deviation = dnxf_data_dev_init.time.step_time_get(unit, step_id)->down_deviation;
        step_id_dnx_data->up_deviation = dnxf_data_dev_init.time.step_time_get(unit, step_id)->up_deviation;
        step_id_dnx_data->test_sensitivity = dnxf_data_dev_init.time.test_sensitivity_get(unit);
        step_id_dnx_data->non_perc_up_deviation =
            dnxf_data_dev_init.time.step_time_get(unit, step_id)->non_perc_up_deviation;
    }
...
int
dnxf_init_skip_print_decider_cb(
    int unit)
{
    ...
    int forward)
{
    ...
    seq->desc_init_step_enabled_counter_inc = NULL;
    seq->desc_init_step_enabled_counter_dec = NULL;
    ...
    int forward)
{
     ...
     */
    {
        SHR_IF_ERR_EXIT(dnxf_init_seq_convert
                        (unit, flags, dnxf_init_info, dnxf_init_deinit_seq, seq_ptr_arr[unit], forward));
    }
...
/**
 * This function is used by include/bcm_int/bcm_dlist.h
 * @param unit
 * @param subtype
 * @return
 */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnxf_linkscan_thread_stop(unit));
    SHR_IF_ERR_EXIT(dnxf_stat_deinit(unit));
    ...
    char *subtype_b)
{
    ...
    SHR_IF_ERR_EXIT(sal_strncmp(subtype_a, subtype_b, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH));
exit:
    SHR_FUNC_EXIT;
}
 ...
 */
shr_error_e
dnxf_adapter_reg_access_deinit(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
...
/**
 * \brief - A wrapper for soc_dnxf_soc_init_done
 * together with BCM later API
 *
 * \param [in] unit - unit
 *
 * \return
 ...
 * Init HA modules
 */
const dnxf_init_step_t dnxf_ha_modules_init_deinit_seq[] = {
    /* STEP_ID,                          STEP_NAME,              STEP_INIT,                    STEP_DEINIT,                         STEP_FLAGS                  SUB_STEP_LIST_PTR     */
    {DNXF_INIT_STEP_HA,                  "HA Init",              soc_dnxf_ha_init,             soc_dnxf_ha_deinit,                  ACCESS | TIME_PER_DEV,      NULL},
    {DNXF_INIT_STEP_SW_STATE,            "SW_State",             soc_dnxf_sw_state_init,       soc_dnxf_sw_state_deinit,            ACCESS | TIME_PER_DEV,      NULL},
    {DNXF_INIT_STEP_LAST_STEP,           "Last Step",            NULL,                         NULL,                                0,                          NULL}
};
/*
 * Soc init sequence
 */
const dnxf_init_step_t dnxf_soc_init_deinit_seq[] = {
   /* STEP_ID,                          STEP_NAME,             STEP_INIT,                     STEP_DEINIT,                         STEP_FLAGS       SUB_STEP_LIST_PTR     */
    {DNXF_INIT_STEP_HA_MODULES,        "HA Modules",           NULL,                          NULL,                                0,               dnxf_ha_modules_init_deinit_seq},
    {DNXF_INIT_STEP_SOC_INFO_LEGACY,   "Soc Info",             soc_dnxf_soc_info_get,         NULL,                                TIME_PER_DEV,    NULL},
#ifdef ADAPTER_SERVER_MODE
    {DNXF_INIT_STEP_ADAPTER_REG_ACC,   "Adapter",              dnxf_adapter_reg_access_init,  dnxf_adapter_reg_access_deinit,      0,               NULL},
#endif
    {DNXF_INIT_STEP_DEVICE_RESET,      "Device reset",         soc_dnxf_drv_device_reset_init,soc_dnxf_drv_device_reset_deinit,    TIME_PER_DEV,    NULL},
    {DNXF_INIT_STEP_LINK_MAPPING,      "Link Mapping",         soc_dnxf_init_link_mapping,    NULL,                                TIME_PER_DEV,    NULL},
    {DNXF_INIT_STEP_BLOCKS_RESET,      "Blocks Reset",         soc_dnxf_init_blocks_config,   NULL,                                TIME_PER_DEV,    NULL},
    {DNXF_INIT_STEP_COUNTER_ATTACH,    "Counter attach",       soc_dnxf_counter_attach,       soc_dnxf_counter_dettach,            TIME_PER_DEV,    NULL},
    {DNXF_INIT_STEP_INTERRUPTS,        "Interrupts",           soc_dnxf_drv_interrupts_init,  soc_dnxf_drv_interrupts_deinit,      TIME_PER_DEV,    NULL},
    {DNXF_INIT_STEP_SOC_INIT_DONE,     "SOC Init Done",        _soc_dnxf_soc_init_done,       soc_dnxf_soc_init_done_deinit,       TIME_PER_DEV,    NULL},
    {DNXF_INIT_STEP_LAST_STEP,         "Last Step",            NULL,                          NULL,                                0,               NULL}
 ...
 */
static const  dnxf_init_step_t dnxf_init_deinit_thread_seq[] = {
    /*          STEP_ID,               STEP_NAME,                 STEP_INIT,                              STEP_DEINIT,                            STEP_FLAGS                                       SUB_STEP_LIST_PTR */
    /** Stat's STEP_DEINIT is null since 'stat' thread is terminated in _bcm_dnxf_detach */
    {DNXF_INIT_STEP_STAT,              "Stat",                    dnxf_stat_init,                         NULL,                                   TIME_PER_DEV,                                    NULL},
    {DNXF_INIT_STEP_LAST_STEP,         "Last Step",               NULL,                                   NULL,                                   0,                                               NULL}
};
...
const dnxf_init_step_t dnxf_init_deinit_seq[] = {
  ...
  /*                                                             \ /                           / \                                                                      */
  /*                                                              V                             |                                                                       */
    {DNXF_INIT_STEP_MARK_NOT_INITED,   "Soc Flags",            dnxf_soc_flags_init,           dnxf_soc_flags_deinit,      0,                      NULL},
    {DNXF_INIT_STEP_DNX_DATA,          "DNXF_Data",            _dnxf_data_init,               dnxf_data_deinit,           ACCESS,                 NULL},
    {DNXF_INIT_STEP_SOC,               "SOC Init",             NULL,                          NULL,                       0,                      dnxf_soc_init_deinit_seq},
    {DNXF_INIT_STEP_PORT,              "Port",                 dnxf_port_init,                dnxf_port_deinit,           TIME_PER_DEV,           NULL},
    {DNXF_INIT_STEP_SYNCE,             "SyncE",                dnxf_synce_init,               NULL,                       TIME_PER_DEV | WB_SKIP, NULL},
    {DNXF_INIT_STEP_LINKSCAN,          "Linkscan",             dnxf_linkscan_init,            dnxf_linkscan_deinit,       TIME_PER_DEV,           NULL},
    {DNXF_INIT_STEP_STACK,             "Stack",                dnxf_stk_init,                 dnxf_stk_deinit,            TIME_PER_DEV,           NULL},
    {DNXF_INIT_STEP_MULTICAST,         "Multicast",            dnxf_multicast_init,           dnxf_multicast_deinit,      TIME_PER_DEV,           NULL},
    {DNXF_INIT_STEP_FABRIC,            "Fabric",               dnxf_fabric_init,              dnxf_fabric_deinit,         TIME_PER_DEV,           NULL},
    {DNXF_INIT_STEP_RX,                "RX",                   dnxf_rx_init,                  dnxf_rx_deinit,             TIME_PER_DEV,           NULL},
    {DNXF_INIT_STEP_TUNE,              "Tune",                 dnxf_tune_init,                NULL,                       TIME_PER_DEV | WB_SKIP, NULL},
    {DNXF_INIT_STEP_INIT_DONE,         "Init Done",            dnxf_init_done_init,           dnxf_init_done_deinit,      TIME_PER_DEV,           NULL},
    {DNXF_INIT_STEP_THREADS,           "Threads",              NULL,                          NULL,                       TIME_PER_DEV,           dnxf_init_deinit_thread_seq},
    {DNXF_INIT_STEP_LAST_STEP,         "Last Step",            NULL,                          NULL,                       0,                      NULL}
};
/* *INDENT-ON* */
