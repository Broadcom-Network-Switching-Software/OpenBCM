...
typedef struct pup_s {
    ...
    sal_usecs_t ts;            /* Pkt Rx timestamp in usec */
...
#   define	PU_LOG_CNT	64	/* Default # log entries */
    ...
    uint32  pu_packet_discarded;/* count discarded pkts due to out of buffers */
...
char pw_usage[] =
    ...
    "\t        rx       - Use the RX API\n"
#ifdef INCLUDE_KNET
    "\tdebugpkt [on | off] - set KNET debug pkt mode\n"
#endif
#endif
    ;
...
#ifdef INCLUDE_KNET
STATIC int
pw_debugpkt_start(int unit)
{
    pwu_t *pu = &pw_units[unit]; /* Control this unit */
    pu->pu_report |= REPORT_RAW; /* default to raw dump */

    if (soc_knet_debug_pkt_set(unit, TRUE) < 0) {
       cli_out("Failed to enable KNET debug pkt trace\n");
    }
    return CMD_OK;
}

STATIC int
pw_debugpkt_stop(int unit)
{
    if (soc_knet_debug_pkt_set(unit, FALSE) < 0) {
       cli_out("Failed to disable KNET debug pkt trace\n");
    }
    return CMD_OK;
}
#endif
    ...
    while ((c = ARG_GET(a)) != NULL) {
        if (!sal_strcasecmp(c, "start")) {
            ...
            cli_out("%s: Received %d packets [(%d),%d,%d,%d,%d] (%d/sec), "
                    "last %d packet(s) logged, %d pkts discarded\n",
                    pw_name[unit],
...
#ifdef INCLUDE_KNET
    } else if (!sal_strcasecmp(c, "debugpkt")) {
        c = ARG_GET(a);
        if (c) {
            if (!sal_strcasecmp(c, "on")) {
                if (pw_debugpkt_start(unit)) {
                    return CMD_FAIL;
                }
                if (!(pu->pu_flags & PU_F_RUN)) {
                    on |= PU_F_RUN;
...
pw_dump_packet_rx(int unit, pup_t *pup, uint32 report)
{
    ...
    /* Assume Tx pkt if src port is CPU port 0 */
    if (report) {
        if (pkt->rx_port == 0) {
            cli_out("%sCPU Pkt Tx Timestamp: %01d.%06d\n", pfx, pup->ts / SECOND_USEC, pup->ts % SECOND_USEC);
        } else {
            cli_out("%sCPU Pkt Rx Timestamp: %01d.%06d\n", pfx, pup->ts / SECOND_USEC, pup->ts % SECOND_USEC);
        }
    }
}
