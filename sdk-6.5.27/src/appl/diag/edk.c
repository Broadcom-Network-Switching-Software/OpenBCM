 ...
 */
{
    ...
    if (edkhost_pid == -1) {
        ...
        edkhost_pid = fork();
        if (edkhost_pid == 0) {
            /* Separate out child to a separate PGID */
            setpgid(edkhost_pid, edkhost_pid);
            ...
            /* Set the edkhost as the foreground process on STDIN */
            tcsetpgrp(0, edkhost_pid);
            /* Parent process waits for child process signal */
            waitpid(edkhost_pid, &wstatus, WUNTRACED);
            if (WIFSTOPPED(wstatus)) {
                /* Ignore the SIGTTOU that would be received because of tcsetpgrp below */
                signal(SIGTTOU, SIG_IGN);
                /* Set the SDK as the foreground process on STDIN */
                tcsetpgrp(0, getpid());
