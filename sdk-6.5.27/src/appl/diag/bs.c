...
#if defined (BCM_MONTEREY_SUPPORT) || defined(BCM_LTSW_SUPPORT) || defined(BCM_J2X_SUPPORT)
#define DUAL_BS
#endif
...
char cmd_broadsync_usage[] =
    ...
    "                   [NtpTodSec=<seconds>] [NtpTodNSec]=<nanoseconds> [BSTimeSec=<seconds>] [BSTimeNSec=<nanoseconds>]\n"
    "                   [NtpTodDisable=<0/1>] [PtpTodDisable=<0/1>] [PtpTodSec=<seconds>] [PtpTodNSec=<nanoseconds>]\n"
...
cmd_result_t cmd_broadsync(int unit, args_t *args)
{
    ...
    if (parse_cmp("CONFig", arg_string_p, 0)) {
        ...
        uint32 bs_time_nsec = 0;
        int64 ntp_sec = 0;
        uint32 ntp_nsec = 0;
        int def_bitclock_hz = 10000000;
        int def_heartbeat_hz = 4000;
        int ntp_tod_disable = 0;
        int ptp_tod_disable = 0;
        int64 ptp_sec = 0;
        uint32 ptp_nsec = 0;
        ...
        parse_table_add(&parse_table, "NtpTodSec", PQ_DFL | PQ_INT64, NULL, &ntp_sec, NULL);
        parse_table_add(&parse_table, "NtpTodNSec", PQ_DFL | PQ_INT, NULL, &ntp_nsec, NULL);
        parse_table_add(&parse_table, "BSTimeSec", PQ_DFL | PQ_INT, NULL, &bs_time_sec, NULL);
        parse_table_add(&parse_table, "BSTimeNSec", PQ_DFL | PQ_INT, NULL, &bs_time_nsec, NULL);
        parse_table_add(&parse_table, "NtpTodDisable", PQ_INT, (void*)0, &ntp_tod_disable, NULL);
        parse_table_add(&parse_table, "PtpTodDisable", PQ_INT, (void*)0, &ptp_tod_disable, NULL);
        parse_table_add(&parse_table, "PtpTodSec", PQ_DFL | PQ_INT64, NULL, &ptp_sec, NULL);
        parse_table_add(&parse_table, "PtpTodNSec", PQ_DFL | PQ_INT, NULL, &ptp_nsec, NULL);
        ...
        if (bs_time_sec != 0) {
            ...
            while (bs_time_nsec >= BS_NANOSECONDS_PER_SECOND) {
                bs_time_nsec -= BS_NANOSECONDS_PER_SECOND;
            }

            intf.bs_time.isnegative = 0;
            COMPILER_64_SET(intf.bs_time.seconds, 0, bs_time_sec);
            intf.bs_time.nanoseconds = bs_time_nsec;
        } else {
        ...
        if ((ntp_tod_disable || ptp_tod_disable) && bstime_intf_info.bs_initialized) {
            cli_out("BroadSync initialized!! Cannot disable NTP/PTP ToD updates\n");
        } else {
            if (ntp_tod_disable) {
                intf.flags |= BCM_TIME_BS_NTP_TOD_UPDATE_DISABLE;
            }

            if (ptp_tod_disable) {
                intf.flags |= BCM_TIME_BS_PTP_TOD_UPDATE_DISABLE;
            }
        }

        if(BS_UC_FEATURE_CHECK(BS_NTP_PTP_TOD_OFFSET_CONFIG)) {
            if (ptp_sec != 0 || ptp_nsec != 0) {
                intf.flags |= BCM_TIME_PTP_OFFSET;

                intf.ptp_offset.isnegative = 0;

                if (ptp_sec < 0) {
                    intf.ptp_offset.isnegative = 1;
                    ptp_sec = -ptp_sec;
                }
                ...
                while (ptp_nsec >= BS_NANOSECONDS_PER_SECOND) {
                    ptp_nsec -= BS_NANOSECONDS_PER_SECOND;
                }

                COMPILER_64_SET(intf.ptp_offset.seconds, 0, ptp_sec);
                intf.ptp_offset.nanoseconds = ptp_nsec;
            } else {
                intf.flags &= ~(BCM_TIME_PTP_OFFSET);
            }
        }
        ...
        intf.flags |= (BCM_TIME_WITH_ID | BCM_TIME_SYNC_STAMPER | BCM_TIME_HEARTBEAT | BCM_TIME_BS_TIME);
...
#if !defined(MONTEREY) && !defined(BCM_J2X_SUPPORT)
...
#endif
        {
            ...
            if (arg_string_p == NULL) {
                uint32 flags;
                if (BCM_FAILURE(_bcm_mbox_debug_flag_get(unit, &flags))) {
            ...
            if (parse_cmp("ON", arg_string_p, 0)) {
                _bcm_mbox_debug_flag_set(unit, 0xff);
            } else if (parse_cmp(arg_string_p, "OFF", 0)) {
                _bcm_mbox_debug_flag_set(unit, 0);
            } else {
                return CMD_USAGE;
            }
#endif
        ...
        parse_table_add(&parse_table, "Debug", PQ_DFL | PQ_INT, (void*)0xFF, &arg_debug, NULL);
        ...
        if (SOC_IS_LTSW(unit)) {
            ...
            bs_log_cfg.debug_mask = arg_debug;
            bs_log_cfg.udp_log_mask = udp_log_mask;
            sal_memcpy(bs_log_cfg.src_mac, src_mac, 6);
            sal_memcpy(bs_log_cfg.dest_mac, dest_mac, 6);
            bs_log_cfg.tpid = tpid;
            bs_log_cfg.vid = vlan;
            bs_log_cfg.ttl = ttl;
            bs_log_cfg.src_addr = src_ip;
            bs_log_cfg.dest_addr = dest_ip;
            bs_log_cfg.udp_port = udp_port;
            rv = bcm_time_bs_log_configure_set (unit, bs_log_cfg);
            if (rv != BCM_E_NONE) {
...
#endif
        {
            rv = _bcm_time_bs_log_configure(unit, arg_debug, udp_log_mask,
...
#if defined(INCLUDE_GDPLL) && (defined (BCM_MONTEREY_SUPPORT) || defined(BCM_J2X_SUPPORT))
