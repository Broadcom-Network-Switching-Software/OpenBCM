...
#include <ltsw/xflow_macsec_diag.h>
#endif /* INCLUDE_XFLOW_MACSEC */
#endif

#if defined(BCM_CTSW_SUPPORT)
#include <appl/diag/ctsw/cmdlist.h>
#endif
...
cmd_t bcm_cmd_common[] = {
...
#endif
#if defined(BCM_CMICM_SUPPORT) || defined(BCM_LTSW_SUPPORT)
    ...
    {"extdpll",    cmd_extdpll,       cmd_extdpll_usage,   "External DPLL Clock Management"},
#endif
...
#endif
#if defined(BCM_SAND_SUPPORT) && (defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__) || defined(__DUNE_SLK_BCM_CPU__) || defined(__DUNE_GTS_BCM_CPU__))
    {"i2c_smbus", cmd_sand_i2c_smbus, cmd_sand_i2c_smbus_usage, "Read/Write I2C smbus device via cpu"},
#endif
    ...
    {"KNETSYNC",      cmd_knetsync,             cmd_knetsync_usage,    "KnetSync Diag commands"},
#endif
/*
 ...
 */
    ...
    {"LOGGER",   cint_logger_cmd,  cint_logger_usage,
     "Commands for the Logger functionality"},
#endif
    ...
    {"NOEcho",          sh_noecho,              sh_noecho_usage,
     "Ignore command line"},
#if defined(BCM_CMICM_SUPPORT)
    {"ONESYNC",       cmd_onesync,              cmd_onesync_usage,     "OneSync Diag commands"},
#endif
#ifndef NO_SAL_APPL
...
#if defined(BCM_CMICM_SUPPORT) && defined(INCLUDE_GDPLL)
...
#if defined(INCLUDE_I2C) && defined(BCM_ESW_SUPPORT)
...
"Manage XGS4 MPLS"},
    ...
    {"PortSAMp",        if_port_samp,           if_port_samp_usage,     "Set/Display sflow port sampling characteristics"},
#endif
...
static cmd_t bcm_ltsw_cmd_list[] = {
...
#if defined(INCLUDE_BFD)
    {"BFD",             cmd_ltsw_bfd,           cmd_ltsw_bfd_usage,         "BFD endpoint info"},
#endif
    ...
    {"DebugSHell",      cmd_bcmlt_debug_shell,  cmd_bcmlt_debug_shell_usage,"Run SDKLT debug shell"},
    {"DETach",          sh_detach,              sh_detach_usage,            "Detach SOC device(s)" },
    ...
    {"FFB",             cmd_lt_ffb,             cmd_lt_ffb_usage,           "Fast fast boot"},
    ...
    {"FlowTracker",     cmd_ltsw_flowtracker,   cmd_ltsw_flowtracker_usage, "Flowtracker sw state"},
    {"HA",              cmd_lt_ha,              cmd_lt_ha_usage,            "HA memory usage and associate information."},
    {"HeaderMode",      cmd_ltsw_hdr_mode,      cmd_ltsw_hdr_mode_usage,    "Get or set packet tx header mode" },
    {"INIT",            cmd_lt_init,            cmd_lt_init_usage,          "Initialize SOC and S/W"},
    ...
    {"MOD",             cmd_ltsw_mod,           cmd_ltsw_mod_usage,         "Display mirror on drop information"},
    ...
    {"PERQ",            cmd_ltsw_perq,          cmd_ltsw_perq_usage,        "Per Port/Per Queue counter utility"},
    {"Policer",         cmd_ltsw_policer_proc,  cmd_ltsw_policer_usage,     "Display Policer information."},
...
#if defined(INCLUDE_XFLOW_MACSEC)
    {"SEC",             cmd_ltsw_xflow_macsec,  cmd_ltsw_xflow_macsec_usage, "Display xflow_macsec information."},
#endif /* INCLUDE_XFLOW_MACSEC */
    ...
    {"TaBLe",           cmd_lt_table,           cmd_lt_table_usage,         "Get table information"},
    {"TOD",             cmd_ltsw_tod,           cmd_ltsw_tod_usage,         "Set/Get TOD"},
...
#if defined(BCM_CTSW_SUPPORT)
/* This is the list of commands that can be executed only in CTSW mode */
static cmd_t bcm_ctsw_cmd_list[] = {
    {"ALPM",            cmd_ctsw_alpm,          cmd_ctsw_alpm_usage,            "ALPM info" },
    {"ASF",             cmd_ctsw_asf,           cmd_ctsw_asf_usage,             "ASF (cut-through) Mode" },
    {"BcmltSHell",      cmd_bcmlt_shell,        cmd_bcmlt_shell_usage,          "Run BCM LT shell"},
    {"CLEAR",           cmd_ctsw_clear,         cmd_ctsw_clear_usage,           "Clear a memory table or counters"},
    {"COS",             cmd_ctsw_cos,           cmd_ctsw_cos_usage,             "Manage classes of service"},
    {"CounTeR",         cmd_ctsw_counter,       cmd_ctsw_counter_usage,         "Enable/disable counter collection"},
    {"DebugSHell",      cmd_bcmlt_debug_shell,  cmd_bcmlt_debug_shell_usage,    "Run BCM LT debug shell"},
    {"DETach",          sh_detach,              sh_detach_usage,                "Detach SOC device(s)" },
    {"Dump",            cmd_ctsw_dump,          cmd_ctsw_dump_usage,            "Dump information" },
    {"EventSTAT",       cmd_ctsw_event_stat,    cmd_ctsw_event_stat_usage,      "Display drop/trace event counters" },
    {"FieldProcessor",  cmd_ctsw_field_proc,    cmd_ctsw_field_proc_usage,      "Display Field Processor information."},
    {"FlexCTR",         cmd_ctsw_flexctr,       cmd_ctsw_flexctr_usage,         "Manage flex counter"},
    {"FlexDigest",      cmd_ctsw_flexdigest,    cmd_ctsw_flexdigest_usage,      "Manage Flex Digest"},
    {"FLEXport",        cmd_ctsw_flexport,      cmd_ctsw_flexport_usage,        "Flexport"},
    {"HA",              cmd_lt_ha,              cmd_lt_ha_usage,                "HA memory usage and associate information."},
    {"INIT",            cmd_lt_init,            cmd_lt_init_usage,              "Initialize SOC and S/W"},
    {"L2",              cmd_ctsw_l2,            cmd_ctsw_l2_usage,              "Manage L2 (MAC) addresses"},
    {"L3",              cmd_ctsw_l3,            cmd_ctsw_l3_usage,              "Manage L3 (IP) addresses"},
    {"LINKscan",        if_ctsw_linkscan,       if_ctsw_linkscan_usage,         "Configure/Display link scanning"},
    {"MIRror",          cmd_ctsw_mirror,        cmd_ctsw_mirror_usage,          "Manage port mirroring"},
    {"MultiCast",       cmd_ctsw_multicast,     cmd_ctsw_multicast_usage,       "Manage multicast operation" },
    {"PacketWatcher",   cmd_ctsw_pcktwatch,     cmd_ctsw_pcktwatch_usage,       "Monitor ports for packets"},
    {"PBMP",            cmd_ctsw_pbmp,          cmd_ctsw_pbmp_usage,            "Convert port bitmap string to hex"},
    {"Policer",         cmd_ctsw_policer_proc,  cmd_ctsw_policer_usage,         "Display Policer information."},
    {"PORT",            cmd_ctsw_port,          cmd_ctsw_port_usage,            "Set/Display port characteristics"},
    {"PortStat",        cmd_ctsw_port_stat,     cmd_ctsw_port_stat_usage,       "Display port status in table"},
    {"PVlan",           cmd_ctsw_pvlan,         cmd_ctsw_pvlan_usage,           "Port VLAN settings" },
    {"SHOW",            cmd_ctsw_show,          cmd_ctsw_show_usage,            "Show information on a subsystem" },
    {"STG",             cmd_ctsw_stg,           cmd_ctsw_stg_usage,             "Manage spanning tree groups" },
    {"SwitchControl",   cmd_ctsw_switch_control,cmd_ctsw_switch_control_usage, "General switch control"},
    {"TaBLe",           cmd_lt_table,           cmd_lt_table_usage,             "Get table information"},
    {"TRace",           cmd_ctsw_trace,         cmd_ctsw_trace_usage,           "Packet trace"},
    {"TRUNK",           cmd_ctsw_trunk,         cmd_ctsw_trunk_usage,           "Manage port aggregation"},
    {"TX",              cmd_ctsw_txn,           cmd_ctsw_txn_usage,             "Transmit one or more packets"},
    {"VLAN",            cmd_ctsw_vlan,          cmd_ctsw_vlan_usage,            "Manage VLAN usage"},
    {"WARMBOOT",        sh_warmboot,            sh_warmboot_usage,              "Optionally boot warm"},
};

int bcm_ctsw_cmd_cnt = COUNTOF(bcm_ctsw_cmd_list);

#endif /* BCM_CTSW_SUPPORT */
...
cmdlist_init(void)
{
...
#endif
#if defined(BCM_CTSW_SUPPORT)
    for (i = 1; i < bcm_ctsw_cmd_cnt; i++) {
        if (sal_strcasecmp(bcm_ctsw_cmd_list[i].c_cmd,
                           bcm_ctsw_cmd_list[i - 1].c_cmd) <= 0) {
            cli_out("WARNING: bcm ctsw command %s not alphabetized\n",
                    bcm_ctsw_cmd_list[i].c_cmd);
        }
    }
#endif
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        if(!soc_attached(unit)) {
            ...
            command_mode_set(unit, CTSW_CMD_MODE);
        }
#endif /* BCM_CTSW_SUPPORT */
...
/*
 * Calibrate (initialize and adjust) the command tree
 * Note - this should be run after running the init sequence,
 *  in order to be able to differentiate between available and unavailable commands based on DNX/F data
 */
void
cmdlist_calibrate(void)
{
    int i;
    int unit;
    ...
    for (i = 0; i < soc_ndev; i++)
    {
        ...
        if((unit < 0) || (unit >= SOC_MAX_NUM_DEVICES))
        {
            ...
            command_mode_set(unit, DNX_CMD_MODE);
            /** Calibrate DNX command tree */
            if(diag_sand_error_get(cmd_dnx_sh_calibrate(unit)) != CMD_OK)
            {
                cli_out("Error: Problem while calibrating DNX command tree\n");
            }
        }
#endif /* BCM_DNX_SUPPORT */
            ...
            command_mode_set(unit, DNXF_CMD_MODE);
            /** Calibrate DNXF command tree */
            if(diag_sand_error_get(cmd_dnxf_sh_calibrate(unit)) != CMD_OK)
            {
                cli_out("Error: Problem while initiating DNXF command tree\n");
            }
        }
#endif /* BCM_DNXF_SUPPORT */
    }
...
#if defined(BCM_CTSW_SUPPORT)
    case CTSW_CMD_MODE:
        cur_cmd_list[unit] = bcm_ctsw_cmd_list;
        cur_cmd_cnt[unit] = bcm_ctsw_cmd_cnt;
        break;
#endif
