 ...
 */
{
    ...
    num_ucs = SOC_INFO(unit).num_ucs;
#ifdef INCLUDE_GDPLL
    /* Monterey had the uc's count accounted in SOC_INFO for M7 */
    if (!SOC_IS_MONTEREY(unit))
        num_ucs += 1;
#endif
    ucnum = parse_integer(c);
    if (ucnum < 0 || ucnum >= num_ucs) {
    ...
    /* "bus error" issue that SDK configure the RSVD field of MHOST_0_CR5_CFG_CTRL,
     * then uKernel polling this register field to kill or stop the thread. 
     */
#if defined(BCM_JERICHO_SUPPORT)
...
extern int _gdpll_m7_status (int unit, uint8 *pBuff);
#endif
