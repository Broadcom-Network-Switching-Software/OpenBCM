    ...
    diag_dnxf_thresholds_iteration_params_t * iteration_parameters)
{
    ...
    iteration_parameters->last_stage = soc_dnxf_threshold_stage_nof - 1;
...
static shr_error_e
diag_dnxf_global_type_threshold_print(
    int unit,
    int threshold_idx,
    uint32 th_validity_bmp,
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(bcm_fabric_profile_threshold_get
                    (unit, 0, -1, (bcm_fabric_threshold_type_t) threshold_idx, flags, &threshold_value));
    PRT_CELL_SET("%d", threshold_value);
    ...
    for (stage = iteration_parameters.first_stage; stage <= iteration_parameters.last_stage; stage++)
    {
             ...
             threshold_idx++)
        {
            ...
            if (!(dnxf_data_fabric.congestion.thresholds_info_get(unit, threshold_idx)->is_to_be_displayed) ||
                !(dnxf_data_fabric.congestion.thresholds_info_get(unit, threshold_idx)->is_valid))
            {
                continue;
            }
            /*
            ...
            /*
             * GLOBAL type threshold
             */
            if (dnxf_data_fabric.congestion.thresholds_info_get(unit, threshold_idx)->index_type ==
                _shr_dnxf_cgm_index_type_global)
            {
                SHR_IF_ERR_EXIT(diag_dnxf_global_type_threshold_print
                                (unit, threshold_idx, th_validity_bmp, sand_control));
            }
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int pipe_idx, block_idx, fifo_idx, dfl_bank_idx, ifm_block_idx, aligned_ifm_idx, aligned_block_idx,
        aligned_dtm_fifo_idx, aligned_inner_link;
    int nof_links_in_dch_link_group, nof_links_in_dtm_fifo, virtual_dch_idx, virtual_dch_inner_link;
    ...
    soc_dnxf_queues_info_t *fe_queues_info = NULL;
    ...
    SHR_ALLOC_SET_ZERO(fe_queues_info, sizeof(soc_dnxf_queues_info_t), "queues_info", "%s%s%s\r\n", EMPTY, EMPTY,
                       EMPTY);
    ...
    /*
     * Get active block instances
     */
    SHR_IF_ERR_EXIT(soc_dnxf_block_active_instance_bmp_get(unit, SOC_DNXF_DRV_BLOCK_TYPE_DCH, &dch_bmp));
    SHR_IF_ERR_EXIT(soc_dnxf_block_active_instance_bmp_get(unit, SOC_DNXF_DRV_BLOCK_TYPE_DCML, &dcml_bmp));
    ...
    if (queue_stage_selected == DIAG_DNXC_OPTION_ALL || queue_stage_selected == soc_dnxf_dch_stage_option)
    {
        ...
        for (pipe_idx = 0; pipe_idx < nof_pipes; pipe_idx++)
        {
            ...
            {
                if (!BCM_PBMP_MEMBER(dch_bmp, block_idx))
                {
                    continue;
                }

                SHR_IF_ERR_EXIT(soc_dnxf_drv_virtual_block_get
                                (unit, SOC_DNXF_DRV_BLOCK_TYPE_DCML, block_idx, 0, &aligned_block_idx,
                                 &aligned_inner_link));
                /*
                ...
                {
                    SHR_IF_ERR_EXIT(soc_dnxf_drv_virtual_block_get
                                    (unit, SOC_DNXF_DRV_BLOCK_TYPE_DCH, block_idx,
                                     ifm_block_idx * nof_links_in_dch_link_group, &virtual_dch_idx,
                                     &virtual_dch_inner_link));
                    aligned_ifm_idx = virtual_dch_inner_link / nof_links_in_dch_link_group;
                    /*
                     * Display vallues only if there is any traffic through the link group
                     */
                    current_dch_link_group =
                        &fe_queues_info->dch_queues_info[pipe_idx].nof_dch[block_idx].dch_link_group[aligned_ifm_idx];
                    if (current_dch_link_group->max_occupancy_value)
                    {
                        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
                        PRT_CELL_SET("DCH%d", aligned_block_idx);
                        PRT_CELL_SET("%d", pipe_idx);
                        if (dnxf_data_port.
                            lane_map.feature_get(unit, dnxf_data_port_lane_map_is_flexible_link_mapping_supported))
                        {
                            shr_pbmp_range_format(current_dch_link_group->link_group_links, print_buf, 40);
                            PRT_CELL_SET("%s", print_buf);
                        }
                        else
                        {
    ...
    if (queue_stage_selected == DIAG_DNXC_OPTION_ALL || queue_stage_selected == soc_dnxf_dtm_stage_option)
    {
        ...
        nof_links_in_dtm_fifo =
            dnxf_data_device.blocks.nof_links_in_dcml_get(unit) / dnxf_data_device.blocks.nof_dtm_fifos_get(unit);

        for (pipe_idx = 0; pipe_idx < nof_pipes; pipe_idx++)
        {
            for (block_idx = 0; block_idx < dnxf_data_device.blocks.nof_instances_dcml_get(unit); block_idx++)
            {
                if (!BCM_PBMP_MEMBER(dcml_bmp, block_idx))
                {
                    continue;
                }
                SHR_IF_ERR_EXIT(soc_dnxf_drv_virtual_block_get
                ...
                {
                    SHR_IF_ERR_EXIT(soc_dnxf_drv_virtual_block_get
                                    (unit, SOC_DNXF_DRV_BLOCK_TYPE_DCH, block_idx, fifo_idx * nof_links_in_dtm_fifo,
                                     &virtual_dch_idx, &virtual_dch_inner_link));
                    aligned_dtm_fifo_idx = virtual_dch_inner_link / nof_links_in_dtm_fifo;

                    current_fifo_max_occupancy =
                        fe_queues_info->dtm_queues_info[pipe_idx].
                        nof_dtm[block_idx].fifo_max_occupancy_value[aligned_dtm_fifo_idx];
                    /*
                    ...
                    {
                        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
                        PRT_CELL_SET("DTM%d", aligned_block_idx);
                        PRT_CELL_SET("%d", pipe_idx);
        ...
        for (pipe_idx = 0; pipe_idx < nof_pipes; pipe_idx++)
        {
            for (block_idx = 0; block_idx < dnxf_data_device.blocks.nof_instances_dcml_get(unit); block_idx++)
            {
                ...
                if (current_dtl_block->max_occupancy_value)
                {
                    ...
                    PRT_CELL_SET("DTL%d", aligned_block_idx);
                    PRT_CELL_SET("%d", pipe_idx);
                    if (dnxf_data_port.
                        lane_map.feature_get(unit, dnxf_data_port_lane_map_is_flexible_link_mapping_supported))
                    {
                        shr_pbmp_range_format(current_dtl_block->link_group_links, print_buf, 200);
                        PRT_CELL_SET("%s", print_buf);
                    }
                    else
                    {
    ...
    if (queue_stage_selected == DIAG_DNXC_OPTION_ALL || queue_stage_selected == soc_dnxf_dfl_stage_option)
    {
        ...
        SOC_PBMP_ITER(dcml_bmp, block_idx)
        {
            /*
            ...
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("DFL%d", aligned_block_idx);
