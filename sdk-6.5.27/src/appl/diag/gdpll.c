...
#ifdef BCM_J2X_SUPPORT
#define BCM_NANOSYNC_V1_SUPPORT
#endif
...
char cmd_gdpll_usage[] =
    ...
    "gdpll debug <enable:0/1> <file_name/UDP> [DestMAC=<mac>] [DestAddr=<addr>] [SrcMAC=<mac>] [SrcAddr=<addr>] [UDPSrcPort=< port>] [UDPDestPort=< port>] [VLan=<vlan>] [TTL=<ttl>] [sizeMB]"
    ...
    "            <LogIndex>, <Channel_num+ChannelState>, <FB-TS>, <REF-TS>, <PERROR>, <NCO>, <LoopFilter>, <LockDetFilter>, <NominalLoopFilter>\n"
    "            NOTE: LogIndex is only added if file size limit set in gdpll debug enable cmd\n"
...
extern int _timestamper_get(int unit, int timestamper, uint64 *pTs_52);
extern int _timestamper_enable(int unit, int timestamper, int enable);
extern int _timestamper_set(int unit, int timestamper, uint64 ts_52);
extern int _gdpll_chan_debug_mode_get(int unit, int chan, bcm_gdpll_debug_mode_t *pDebugMode);
extern int _gdpll_debug_get (int unit, int *pEnable);
extern int _gdpll_debug_dump (int unit, dpll_dump_t *pChanDump);
...
#ifndef NO_FILEIO
#define GDPLL_DEBUG_LOG_ELEMENT_SIZE_BYTES          (8) /*64 bit elements*/
#define GDPLL_DEBUG_LOG_MB_TO_BYTES                 (1024*1024)
#define GDPLL_DEBUG_LOG_NUM_ELEMENTS                (8)
#define GDPLL_DEBUG_LOG_NUM_ELEMENTS_WITH_LOG_IDX   (GDPLL_DEBUG_LOG_NUM_ELEMENTS + 1)
#define BCM_GDPLL_DEBUG_BUFFER_SIZE_B               (40*1024)

uint32 *pDebugBuff = NULL;
FILE * volatile fp = NULL;
static int file_size_bytes = 0; /*Zero means no limit*/
static int file_offset_from_start_bytes = 0;
static uint64 file_logIndex;
#endif
...
/********************************************************************
 * Function:
 *      _bcm_gdpll_debug_log_copy
 * Purpose:
 *      This API will push the debug log to GDPLL debug buffers
 *      NONE
 * Returns:
 *      None
 ********************************************************************/
void _bcm_gdpll_debug_log_copy(uint64 log, uint32 *upper, uint32 *lower)
{
     uint32 ts_upp, ts_low;
#if defined (LE_HOST)
     ts_low = COMPILER_64_LO(log);
     ts_upp = COMPILER_64_HI(log);
     ts_upp = ts_upp << 16 | ts_upp >> 16;
     ts_low = ts_low << 16 | ts_low >> 16;
     *upper = ts_upp;
     *lower = ts_low;
#else
     ts_upp = COMPILER_64_HI(log);
...
int gdpll_debug_cb(int unit, void *user_data, uint32 debug_buff, uint32 debug_buf_size)
{
    ...
    //bsl_printf("\n gdpll_debug_cb: addr:0x%x size:%d", debug_buff, debug_buf_size);
    if (file_size_bytes == 0) {
        /* Write the debug buffer to a file registered */
        if (fp) {
            for (i=0; i<debug_buf_size; i++, rd_addr+=4) {
#if defined(BCM_NANOSYNC_V1_SUPPORT)
                pDebugBuff[wrcount++] = swap32(soc_pci_read(unit, rd_addr));
#else
                pDebugBuff[wrcount++] = swap32(soc_cm_iproc_read(unit, rd_addr));
#endif
            }

            /* Dump the data */
            if (wrcount) {
                if (sal_fwrite(pDebugBuff, 4*wrcount, 1, fp) != 1) {
                    cli_out("Error writing to file in non-snapshot mode\n");
                    rv = BCM_E_FAIL;
                }
            }
        }
    } else {
        /* Snapshot logging mode enabled */
        /* Write the debug buffer to a file registered */
        if (fp) {
            for (i=0; i<debug_buf_size; i++, rd_addr+=4) {
                if ((i % ((GDPLL_DEBUG_LOG_ELEMENT_SIZE_BYTES * GDPLL_DEBUG_LOG_NUM_ELEMENTS)/4)) == 0) {
                    uint32 upper, lower;

                    if (file_offset_from_start_bytes >= (file_size_bytes - (GDPLL_DEBUG_LOG_ELEMENT_SIZE_BYTES * GDPLL_DEBUG_LOG_NUM_ELEMENTS_WITH_LOG_IDX))) {
                        /* Flush to file and reset the counter */
                        if (wrcount) {
                            if (sal_fwrite(pDebugBuff, 4*wrcount, 1, fp) != 1) {
                                cli_out("Error writing to file in snapshot mode for a roll-over\n");
                                rv = BCM_E_FAIL;
                            }
                        }
                        wrcount = 0;

                        if (sal_fseek(fp, 0, SEEK_SET)) {
                            cli_out("Error while moving to start of file\n");
                            rv = BCM_E_FAIL;
                        }
                        file_offset_from_start_bytes = 0;
                    }

                    _bcm_gdpll_debug_log_copy(file_logIndex, &upper, &lower);

                    pDebugBuff[wrcount++] = upper;
                    pDebugBuff[wrcount++] = lower;

                    COMPILER_64_ADD_32(file_logIndex, 1);
                    file_offset_from_start_bytes += GDPLL_DEBUG_LOG_ELEMENT_SIZE_BYTES;
                }

                pDebugBuff[wrcount++] = swap32(soc_cm_iproc_read(unit, rd_addr));
                file_offset_from_start_bytes += 4;

            }
            /* Dump the data */
            if (wrcount) {
                if (sal_fwrite(pDebugBuff, 4*wrcount, 1, fp) != 1) {
                    cli_out("Error writing to file in snapshot mode\n");
                    rv = BCM_E_FAIL;
                }
            }
        }
...
{
    uint32 l, u;
    READ_NS_REGr(unit, NS_TIMESYNC_TS1_TIMESTAMP_UPPER_STATUSr, 0, 0, &u);
    READ_NS_REGr(unit, NS_TIMESYNC_TS1_TIMESTAMP_LOWER_STATUSr, 0, 0, &l);
...
static uint64 rsvd1_if_ts;
static uint64 rsvd1_rf_ts;
...
uint32 gdpll_config(bcm_gdpll_chan_t *pGdpllChan, char *pStr, char *pNumStr)
{
    ...
    if (!isint(pNumStr)) {
        if (pNumStr)
        cli_out("### gdpll_config: Error: config number for str:%s NumStr:%s\n", pStr, pNumStr);
        else
    ...
    if (strcmp(pStr, "event_ref_input_event") == 0)
    ...
    else if (strcmp(pStr, "event_ref_lane") == 0)
            pGdpllChan->event_ref.lane_num = num;
     ...
     else if (strcmp(pStr, "event_ref_event_divisor_hi") == 0)
            pGdpllChan->event_ref.event_divisor_hi = num;
     else if (strcmp(pStr, "event_ref_event_dest_bmp") == 0)
            pGdpllChan->event_ref.event_dest_bitmap = num;
    ...
    else if (strcmp(pStr, "event_fb_lane") == 0)
        pGdpllChan->event_fb.lane_num = num;
    ...
    else if (strcmp(pStr, "event_fb_event_divisor_hi") == 0)
        pGdpllChan->event_fb.event_divisor_hi = num;
    else if (strcmp(pStr, "event_fb_event_dest_bmp") == 0)
        pGdpllChan->event_fb.event_dest_bitmap = num;
    ...
    /* Channel specifics */
        ...
        pGdpllChan->port = num;
    else if (strcmp(pStr, "out_lane") == 0)
        pGdpllChan->lane_num = num;
...
_bcm_future_cpri_enable(int unit, int port, uint64 offset)
{
...
#ifdef CPRIMOD_SUPPORT
    rv = cprimod_tx_framer_tgen_enable_set(unit, port, 0/*enable*/);
    if (BCM_FAILURE(rv)) {
    ...
    rv = cprimod_tx_framer_tgen_enable_set(unit, port, 1/*enable*/);
    if (BCM_FAILURE(rv)) {
...
roe_pkt_cb_handler(int unit, bcm_pkt_t *info, void *cookie)
{
    ...
    _timestamper_get(unit, timestamper, &T1);
...
void chan_debug_dump(int unit, int chan)
{
    ...
    chan_dump.chan = chan;
    if (BCM_E_NONE != _gdpll_debug_dump(unit, &chan_dump)) {
    ...
    if (parse_cmp("Flush", arg_string_p, 0)) {
        ...
        if (enable) {
            ...
            if (strcmp(filename, "UDP") == 0) {
                ...
                result = bcm_gdpll_debug_cb_register (unit, gdpll_debug_udp_cb, NULL);
                if (BCM_FAILURE(result)) {
                    cli_out("Command failed. %s\n", bcm_errmsg(result));
                    return CMD_FAIL;
                }
                ...
                /* Register the callback function */
                result = bcm_gdpll_debug_cb_register (unit, gdpll_debug_cb, NULL);
                if (BCM_FAILURE(result)) {
                    cli_out("Command failed. %s\n", bcm_errmsg(result));
                    return CMD_FAIL;
                } else if (!pDebugBuff) {
                    /* Allocate mem for debug buffer read
                     * Since it will be word read, allocate maximum of half the word size
                     */
                    pDebugBuff = sal_alloc(BCM_GDPLL_DEBUG_BUFFER_SIZE_B/4, "GDPLL debug buffer");
                    if (pDebugBuff == NULL) {
                        cli_out("Debug buffer allocation failed\n");
                        return CMD_FAIL;
                    }
                }
                ...
                if (c == NULL) {
                    /* File size not set i.e., no limit*/
                    file_size_bytes = 0;
                } else if (!isint(c)) {
                   cli_out("%s: Error: file size \n", ARG_CMD(args));
                   return(CMD_USAGE);
                } else {
                    int file_size_MB = parse_integer(c);
                    file_size_bytes = file_size_MB * GDPLL_DEBUG_LOG_MB_TO_BYTES;
                    file_offset_from_start_bytes = 0;
                    COMPILER_64_SET(file_logIndex, 0, 0);
                    cli_out("Snapshot logging mode enabled. Max log file size: %d MB\n", file_size_MB);
                }
            }
#endif
        }
        ...
        if (parse_cmp("Destroy", arg_string_p, 0)) {
            ...
            READ_NS_REGr(unit, NS_GDPLL_IA_UPDATE_CONFIGr, chan, 0, &regVal1);
            ...
            WRITE_NS_REGr(unit, NS_GDPLL_IA_UPDATE_CONFIGr, chan, 0, regVal1);
...
#define ia_ts_pair_available0_r     NS_GDPLL_IA_TS_PAIR_AVAILABLE_0r
{
            int idx = chan/32;
            int pos = chan%32;
            int max_gdpll_ch = 288;
            if (chan < 0 || chan >=max_gdpll_ch) {
                cli_out("Error: Invalid chan number %d\n", chan);
                return(CMD_USAGE);
            } else {
                READ_NS_REGr(unit, ia_ts_pair_available0_r, 0, (idx*4), &pair_avail);
                pair_avail = pair_avail & (1 << pos);
                WRITE_NS_REGr(unit, ia_ts_pair_available0_r, 0, (idx*4), pair_avail);
                cli_out("\ngdpll_chan_dump: chan:%d idx:%d pos:%d pair_avail:%d\n", chan, idx, pos, pair_avail);
            }
}
#else
            if (chan < 32) {
                READ_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE0r, 0, 0, &pair_avail);
                pair_avail = pair_avail & (1 << chan);
                WRITE_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE0r, 0, 0, pair_avail);
            } else if (chan < 64) {
                READ_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE1r, 0, 0, &pair_avail);
                pair_avail = pair_avail & (1 << (chan-32));
                WRITE_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE1r, 0, 0, pair_avail);
            } else if (chan < 96) {
                READ_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE2r, 0, 0, &pair_avail);
                pair_avail = pair_avail & (1 << (chan-64));
                WRITE_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE2r, 0, 0, pair_avail);
            } else if (chan < 128) {
                READ_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE3r, 0, 0, &pair_avail);
                pair_avail = pair_avail & (1 << (chan-96));
                WRITE_NS_REGr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE3r, 0, 0, pair_avail);
            ...
            _gdpll_chan_debug_mode_get(unit, chan, &chan_debug_mode);
            _gdpll_debug_get (unit, &enable);

            /* Check for phaseCounter Ref */
#if defined(BCM_NANOSYNC_V1_SUPPORT)
            READ_NS_REGr(unit, NS_GDPLL_IA_UPDATE_CONFIGr, 0, (chan*4), &regVal1);
            field1 = soc_reg_field_get(unit, NS_GDPLL_IA_UPDATE_CONFIGr, regVal1, PHASECOUNTERREF_Nf);
#else
            READ_NS_REGr(unit, NS_GDPLL_IA_UPDATE_CONFIGr, chan, 0, &regVal1);
            ...
            if (use_case_flag == 1)
            {
		        ...
		        READ_NS_REGr(unit, NS_MISC_EVENT_STATUSr, 0, 0, &rval);
                        soc_reg_field_set(unit, NS_MISC_EVENT_STATUSr, &rval, field, 1);
		        WRITE_NS_REGr(unit, NS_MISC_EVENT_STATUSr, 0, 0, rval);
                    ...
                    _timestamper_get(unit, gdpll_chan.event_fb.ts_counter, &TS);
                ...
                _timestamper_enable(unit, gdpll_chan.event_fb.ts_counter, 0);

                u64_H(ts_reset) = 0; u64_L(ts_reset) = 0;
                _timestamper_set(unit, gdpll_chan.event_fb.ts_counter, ts_reset);
                ...
                if ((gdpll_chan.event_fb.input_event == bcmGdpllInputEventBS0HB) ||
                    ...
                    if (gdpll_chan.event_fb.input_event == bcmGdpllInputEventBS0HB) {
			READ_NS_REGr(unit, NS_BS0_BS_CONFIGr, 0, 0, &rval);
                        soc_reg_field_set(unit, NS_BS0_BS_CONFIGr, &rval, MODEf, 0);
			WRITE_NS_REGr(unit, NS_BS0_BS_CONFIGr, 0, 0, rval);

			READ_NS_REGr(unit, NS_MISC_CLK_EVENT_CTRLr, 0, 0, &rval);
                        rval = rval | 0x2;
			WRITE_NS_REGr(unit, NS_MISC_CLK_EVENT_CTRLr, 0, 0, rval);

                    } else {
			READ_NS_REGr(unit, NS_BS1_BS_CONFIGr, 0, 0, &rval);
                        soc_reg_field_set(unit, NS_BS1_BS_CONFIGr, &rval, MODEf, 0);
			WRITE_NS_REGr(unit, NS_BS1_BS_CONFIGr, 0, 0, rval);
			...
			READ_NS_REGr(unit, NS_MISC_EVENT_STATUSr, 0, 0, &rval);
...
#if defined(GDPLL_SAL_HAS_TIME)
                    time = (uint32)get_time(mydate, mytime);
                ...
                _timestamper_enable(unit, gdpll_chan.event_fb.ts_counter, 1);
                ...
                if (gdpll_chan.event_fb.input_event == bcmGdpllInputEventBS0PLL) {
                    ...
                    READ_NS_REGr(unit, NS_BS0_BS_HEARTBEAT_DOWN_DURATIONr, 0, 0, &regVal);
                    ...
                    READ_NS_REGr(unit, NS_COMMON_CTRLr, 0, 0, &regVal);
                    ...
                    READ_NS_REGr(unit, NS_BS1_BS_HEARTBEAT_DOWN_DURATIONr, 0, 0, &regVal);
            ...
            if (use_case_flag != 10 /* SyncE/SyncO */) {
                /* Reset portType so that Interpolator is bypassed.
                 * Only if use_case_flag is set to 10 we decided on
                 * interpolator config during gdpll_channel_create.
                 */
